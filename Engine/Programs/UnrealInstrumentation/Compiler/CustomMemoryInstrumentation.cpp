...
// std::atomic_ref<T> instantiates the same _Atomic_storage / _Atomic_integral /
// _Atomic_pointer / _Atomic_floating templates that std::atomic<T> uses, but
// with a reference type (T&) as the first template argument. The _Storage
// member is then a reference field, which at the ABI level is a pointer, so
// the `this` argument of the resulting load/store/etc. call points at a
// pointer to T rather than at T itself. Detect this by walking the first
// template argument of the matched `std::_Atomic_*<...>` and checking whether
// it ends in `&` (after stripping trailing whitespace). The walk balances
// nested angle brackets, so atomic_ref<MyTemplate<int>> is handled correctly
// and atomic<MyTemplate<int>> is not misclassified.
bool IsMSVCAtomicRefCallTarget(StringRef DemangledName) {
  auto IsAsciiSpace = [](char C) {
    return C == ' ' || C == '\t' || C == '\n' || C == '\r';
  };
  auto FirstArgEndsInRef = [&](size_t Start, size_t End) {
    while (End > Start && IsAsciiSpace(DemangledName[End - 1])) {
      --End;
    }
    return End > Start && DemangledName[End - 1] == '&';
  };

  size_t NamePos = DemangledName.find("std::_Atomic_");
  if (NamePos == StringRef::npos) {
    return false;
  }
  size_t OpenAngle = DemangledName.find('<', NamePos);
  if (OpenAngle == StringRef::npos) {
    return false;
  }
  int Depth = 1;
  for (size_t i = OpenAngle + 1; i < DemangledName.size(); ++i) {
    char C = DemangledName[i];
    if (C == '<') {
      ++Depth;
    } else if (C == '>') {
      if (--Depth == 0) {
        return FirstArgEndsInRef(OpenAngle + 1, i);
      }
    } else if (C == ',' && Depth == 1) {
      return FirstArgEndsInRef(OpenAngle + 1, i);
    }
...
void CustomMemoryInstrumentationPass::cacheInstrumentationFunctions(Module &M) {
  ...
  for (size_t i = 1; i <= MAX_ATOMIC_SIZE; i *= 2) {
    ...
    AtomicStoreInstrumentFunctions[FunctionIndexFromSize(i)] =
        M.getOrInsertFunction(
            ...
            Builder.getPtrTy(),
#else
            Builder.getIntNTy(i * 8)->getPointerTo(),
#endif // Atomic pointer.
  ...
  for (size_t i = 1; i <= MAX_ATOMIC_SIZE; i *= 2) {
    for (unsigned b = 0; b < RMWBinOp::LAST_BINOP; ++b) {
...
#ifdef LLVM_HAS_AUTORTFM
  // Mark all __Instrument_* functions as autortfm_mode_internal so that
  // AutoRTFM does not generate or execute closed variants of these
  // instrumentation functions.
  for (auto &F : M) {
    if (F.getName().starts_with("__Instrument_")) {
      F.addFnAttr(Attribute::get(
          M.getContext(), Attribute::AutoRTFM,
          static_cast<uint64_t>(autortfm_mode_internal)));
    }
  }
#endif
}
...
#if LLVM_VERSION_MAJOR >= 20
  InstrumentationIRBuilder Builder(&*F.getEntryBlock().getFirstNonPHIIt());
...
bool CustomMemoryInstrumentationPass::instrumentMSVCAtomicCallSite(
  ...
  Value *RawPtr = Inst->getArgOperand(CallSite.PtrOperand);

  // For std::atomic_ref<T> (i.e. _Atomic_*<T &, ...>), the `this` pointer
  // points at a reference field rather than at T. The reference is stored as
  // a pointer at the ABI level, so load it once to recover the address that
  // the atomic operation actually targets — otherwise the runtime would record
  // accesses at the atomic_ref object's stack slot instead of at &T.
  if (IsMSVCAtomicRefCallTarget(
          demangle(Inst->getCalledFunction()->getName().str()))) {
    RawPtr = Builder.CreateAlignedLoad(
        Builder.getPtrTy(), RawPtr,
        Align(CurrentModule->getDataLayout().getPointerSize()));
  }

  Value *Ptr =
#if LLVM_VERSION_MAJOR >= 20
      Builder.CreatePointerCast(RawPtr, Builder.getPtrTy());
#else
      Builder.CreatePointerCast(RawPtr,
