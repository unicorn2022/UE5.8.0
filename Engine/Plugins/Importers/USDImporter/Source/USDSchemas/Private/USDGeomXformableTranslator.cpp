...
USceneComponent* FUsdGeomXformableTranslator::CreateComponentsEx(TOptional<TSubclassOf<USceneComponent>> ComponentType, TOptional<bool> bNeedsActor)
{
	...
	if (SceneComponent)
	{
		...
		const bool bIsCamera = SceneComponent->GetOwner() && SceneComponent->GetOwner()->IsA(ACameraActor::StaticClass());
		...
		if (bIsAssociating || bParentIsMovable || bIsCamera || SceneComponent->IsA<USkeletalMeshComponent>() || UsdUtils::IsAnimated(Prim))
		{
...
bool FUsdGeomXformableTranslator::CollapsesChildren(ECollapsingType CollapsingType) const
{
	...
	if (Model)
	{
		...
		const bool bUseSchemaForCollapsing = true;
		bCollapsesChildren = UsdUtils::PrimCollapses(
			Prim,
...
bool FUsdGeomXformableTranslator::CanBeCollapsed(ECollapsingType CollapsingType) const
{
	...
	if (UsdUtils::IsAnimated(UsdPrim) || 
		...
		UsdUtils::GetAppliedDrawMode(UsdPrim) != EUsdDrawMode::Default
	)
	{
		return false;
	}
