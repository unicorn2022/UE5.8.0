...
void FUsdLuxLightTranslator::CreateAssets()
{
	...
	const FString ResolvedDomeTexturePath = UsdUtils::GetResolvedAssetPath(TextureFileAttr);
	if (ResolvedDomeTexturePath.IsEmpty())
	{
		FScopedUsdAllocs Allocs;

		pxr::SdfAssetPath TextureAssetPath;
		TextureFileAttr.Get<pxr::SdfAssetPath>(&TextureAssetPath);
		...
		USD_LOG_USERWARNING(FText::Format(
			LOCTEXT("FailToFindTexture", "Failed to find texture '{0}' used for DomeLight '{1}'!"),
			FText::FromString(TargetAssetPath),
			FText::FromString(UsdToUnreal::ConvertPath(Prim.GetPath()))
		));
	...
	{
		TextureUserData->PrimPaths.AddUnique(UsdToUnreal::ConvertPath(Prim.GetPath()));
	}
