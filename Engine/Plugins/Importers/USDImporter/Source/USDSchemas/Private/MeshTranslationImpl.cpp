		...
		for (int32 SlotIndex = 0; SlotIndex < Slots.Num(); ++SlotIndex, ++GlobalResolvedMaterialIndex)
		{
			...
			switch (Slot.AssignmentType)
			{
				...
				case UsdUtils::EPrimAssignmentType::MaterialPrim:
				{
					...
					bool bUsingTwoSidedPreviewSurfaceAsFallback = false;
					...
					for (UMaterialInterface* ExistingMaterial : ExistingMaterials)
					{
						...
						if (bSidednessMatches)
						{
							...
							bUsingTwoSidedPreviewSurfaceAsFallback = false;
						...
						{
							// If we're searching for a one sided material and just ran into an existing two-sided one we
							// should keep it just in case, but continue iterating: If a two-sided material is within
							// ExistingMaterials, its one-sided reference material should also be in there, so we'll find
							// something eventually. The only exception is in case the user manually set a two-sided material
							// as a one-sided preview surface parent material, which we'll keep track of and warn about
							if (!Material)
							{
								Material = ExistingMaterial;
							}
							if (!Slot.bMeshIsDoubleSided && ExistingMaterial && bExistingIsDirectReference && ExistingMaterial->IsTwoSided())
							{
								bUsingTwoSidedPreviewSurfaceAsFallback = true;
							}
						}
					}

					if (bUsingTwoSidedPreviewSurfaceAsFallback)
					{
						USD_LOG_USERWARNING(FText::Format(
							LOCTEXT(
								"TwoSidedAsOneSidedParent",
								"Using two-sided material '{0}' for a one-sided mesh prim '{1}' as there is no better alternative. Make sure the ReferencePreviewSurfaceMaterial material set on the project settings is one-sided. You can assign a two-sided material on ReferencePreviewSurfaceTwoSidedMaterial instead."
							),
