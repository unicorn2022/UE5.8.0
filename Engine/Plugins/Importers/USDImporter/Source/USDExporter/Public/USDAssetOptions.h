...
struct FUsdMeshAssetOptions
{
	...
	UE_DEPRECATED(all, "This option is now deprecated as UE material assignments are only visible in the 'unreal' render context anyway")
	UPROPERTY(config, meta = (EditCondition = bBakeMaterials))
	bool bRemoveUnrealMaterials_DEPRECATED = false;
