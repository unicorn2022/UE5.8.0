...
namespace UE::LevelSequenceExporterUSD::Private
{
	...
	void ExportAudioTrack(
		...
		UE::FUsdPrim& Prim,
		UsdUtils::TCaseSensitiveStringMap<int32>& AudioTracksPerPrim
	)
	{
	...
	)
	{
		...
		for (const TPair<UObject*, FSpawnedInstanceKey>& Pair : BoundObjects)
		{
			...
			auto GetPrimForComponent = [&Context, &UsdStage](const USceneComponent& Component, FString* PrimPathStr = nullptr) -> UE::FUsdPrim
			{
				...
				// For the spot light case we also have to author our API schema here, or else our property bakers will fail to see this
				// as a valid prim for a spot light
				if (Prim && Component.IsA<USpotLightComponent>())
				{
					Prim.ApplyAPI(TEXT("ShapingAPI"));
				}
