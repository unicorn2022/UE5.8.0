		...
		if (pxr::UsdStageRefPtr PrimUsdStage = PxrUsdPrim.GetStage())
		{
			for (const pxr::SdfPrimSpecHandle& Spec : PxrUsdPrim.GetPrimStack())
			{
				if (Spec && Spec->HasReferences())
				{
					...
					{
						// At this point we know this prim has a spec with references on this edit target layer
						//
						// For us to be able to interact with it however, it must be a non-ancestral reference, so we need to check if we
						// have any on this layer and prim path.
						//
						// Ancestral references are reference composition arcs introduced by an ancestor prim (so e.g. a parent prim has a 
						// reference, that contributes reference specs to this prim). We can't interact with those, as they're "encapsulated" 
						// within the reference (see https://openusd.org/release/usdfaq.html#list-edited-composition-arcs)
						pxr::UsdPrimCompositionQuery PrimCompositionQuery = pxr::UsdPrimCompositionQuery::GetDirectReferences(PxrUsdPrim);
						for (const pxr::UsdPrimCompositionQueryArc& CompositionArc : PrimCompositionQuery.GetCompositionArcs())
						{
							if (CompositionArc.HasSpecs() && 
								CompositionArc.GetIntroducingLayer() == SpecLayer && 
								CompositionArc.GetIntroducingPrimPath() == PxrUsdPrim.GetPrimPath())
							{
								return true;
							}
						}
		...
		if (pxr::UsdStageRefPtr PrimUsdStage = PxrUsdPrim.GetStage())
		{
			for (const pxr::SdfPrimSpecHandle& Spec : PxrUsdPrim.GetPrimStack())
			{
				if (Spec && Spec->HasPayloads())
				{
					...
					if (PrimUsdStage->GetEditTarget().GetLayer() == SpecLayer && PrimUsdStage->HasLocalLayer(SpecLayer))
					{
						// See analogous comment on HasReferencesOnLocalLayer

						// There's no pxr::UsdPrimCompositionQuery::GetDirectPayloads()...
						...
						Filter.arcTypeFilter = pxr::UsdPrimCompositionQuery::ArcTypeFilter::Payload;
						pxr::UsdPrimCompositionQuery PrimCompositionQuery{PxrUsdPrim, Filter};
