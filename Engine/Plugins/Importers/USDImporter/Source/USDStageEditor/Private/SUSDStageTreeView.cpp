		...
		PrimOptions.AddMenuEntry(
			...
			FUIAction(
				...
				FCanExecuteAction::CreateLambda(
					[this]()
					{
						return GetSelectedItems().Num() == 1 && GetSelectedItems()[0]->UsdPrim && !GetSelectedItems()[0]->UsdPrim.IsPseudoRoot();
					}
				)
			),
			NAME_None,
...
void SUsdStageTreeView::OnPastePrim()
{
	...
	else if (MySelectedItems.Num() > 0)
	{
		// Only paste for the first selection, for simplicity
		ParentPrim = MySelectedItems[0]->UsdPrim.GetParent();
	}
...
void SUsdStageTreeView::OnAddReference()
{
	...
	if (OptionsPtr->bInternalReference)
	{
		...
		OptionsPtr->bUseDefaultPrim = false;
	}
...
bool SUsdStageTreeView::CanPastePrim() const
{
	...
	for (FUsdPrimViewModelRef SelectedItem : MySelectedItems)
	{
		if (!SelectedItem->UsdPrim || SelectedItem->UsdPrim.IsPseudoRoot())
		{
			// Don't let us paste as siblings of the pseudoroot as that makes no sense
			return false;
		}
	}
...
void SUsdStageTreeView::OnPrimNameCommitted(const FUsdPrimViewModelRef& ViewModel, const FText& InPrimName)
{
	...
	if (bRenamingExistingPrim)
	{
		...
		// preserve our expansion states
		{
			...
			UsdUtils::FCaseSensitiveStringSet EntriesToAdd;
			for (UsdUtils::FCaseSensitiveStringSet::TIterator It{ExpandedPrimPaths}; It; ++It)
			{
