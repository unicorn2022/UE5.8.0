...
struct FUsdStageImportContext
{
	...
	UE_DEPRECATED(all, "Use the 'UsdAssetCache' member instead, which is of the new UUsdAssetCache3 type")
	UPROPERTY()
	TObjectPtr<UUsdAssetCache2> AssetCache_DEPRECATED;

	UE_DEPRECATED(all, "The import process now always builds its own InfoCache, so this member is no longer used")
	TSharedPtr<FUsdInfoCache> InfoCache_DEPRECATED;
