...
)
{
	...
	if (!bIsAutomated)
	{
		...
		// Update the boundaries of the subdivision level slider according to the cvar
#if WITH_EDITOR
		...
		if (FProperty* Property = UUsdStageImportOptions::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UUsdStageImportOptions, SubdivisionLevel)))
		{
