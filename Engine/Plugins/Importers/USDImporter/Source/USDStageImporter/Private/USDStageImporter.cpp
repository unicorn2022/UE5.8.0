...
namespace UE::USDStageImporter::Private
{
	...
	)
	{
		...
		if (bMustDuplicate || (ExistingAsset != nullptr && ExistingAsset != Asset && ReplacePolicy == EReplaceAssetPolicy::Replace))
		{
			...
			{
				ensure(Asset->Rename(*TargetAssetName, Package, REN_DontCreateRedirectors | REN_NonTransactional | REN_DoNotDirty | REN_AllowPackageLinkerMismatch));
				MovedAsset = Asset;
			}
		...
		// If we swapped the asset inside of a package we need to invalidate the linker and disconnect
		// the package from the file on disk. Otherwise some code paths that go through it to find the
		// package's asset will find our old asset in Engine/Transient instead of the new reimported asset
		// (Repro for this: Import static mesh and place it into the level -> Save everything -> Reimport
		// static mesh -> Reload level -> Without this code, the reloaded level will point at the old static mesh)
		if (ExistingPackage)
		{
	...
	void CopyOriginalMaterialAssignment(FUsdStageImportContext& ImportContext, UObject* ExistingAsset, UObject* NewAsset)
	{
		...
		{
			TArray<FStaticMaterial>& NewMaterials = NewMesh->GetStaticMaterials();
			TArray<FStaticMaterial>& ExistingMaterials = ExistingMesh->GetStaticMaterials();
			int32 NumExistingMaterials = ExistingMaterials.Num();
			int32 NumNewMaterials = NewMaterials.Num();

			for (int32 NewMaterialIndex = 0; NewMaterialIndex < NumNewMaterials && NewMaterialIndex < NumExistingMaterials; ++NewMaterialIndex)
			{
				...
				NewMaterials[NewMaterialIndex].MaterialInterface = ExistingMaterial;
			}

			// Clear out any other assignments we may have
			for (int32 Index = NumExistingMaterials; Index < NumNewMaterials; ++Index)
			{
				NewMaterials[Index].MaterialInterface = nullptr;
			}
	...
	)
	{
		...
		// Get all in-memory referencers.
		//
		// We need this as we need to remap even references internal to other loaded assets, like a blueprint component's reference
		// or a spawnable reference, etc.
		//
		// We don't need to search inside unloaded assets because those would presumably already find the proper remapped
		// asset via path whenever they are loaded. By the same logic we don't need to do the same for soft object path references,
		// and only really need to update live UObject references
		{
			TSet<UObject*> OldAssets;
			OldAssets.Reserve(ObjectsToRemap.Num());
			for (const TPair<UObject*, UObject*>& Pair : ObjectsToRemap)
			{
				UObject* OldAsset = Pair.Key;
				if (!OldAsset)
				{
					continue;
				}
			...
			TArray<UObject*> FoundReferencerObjects = FReferencerFinder::GetAllReferencers(
				OldAssets,
				nullptr,
				EReferencerFinderFlags::SkipInnerReferences
			);

			Referencers.Append(FoundReferencerObjects);
		}
		...
		// Actually fixup all the referencers that we've collected.
...
)
{
	...
	bool bFoundReimportPrim = false;
	{
		...
		if (TargetPrim)
		{
			bFoundReimportPrim = true;
			UE::USDStageImporter::Private::CreateAssetsForPrims({TargetPrim}, TranslationContext.Get(), LOCTEXT("CreateAssets", "Creating assets"));
		}
		else
		{
			USD_LOG_USERERROR(FText::Format(
				LOCTEXT("ReimportMissingPrim", "Failed to find prim '{0}' in stage '{1}' to use for the reimport of asset '{2}'!"),
				FText::FromString(OriginalPrimPath),
				FText::FromString(ImportContext.FilePath),
				FText::FromString(OriginalAsset->GetPathName())
			));
		}
	}
	TranslationContext->CompleteTasks();
	...
	else if (bFoundReimportPrim)
	{
		// If we're in here we did find the prim we wanted on the stage, but for some reason it didn't produce the
		// expected asset type
		USD_LOG_USERERROR(FText::Format(
			LOCTEXT(
				"ReimportMissingCompatibleAsset",
				"Failed to produce a '{0}' asset from prim '{1}' in stage '{2}' that could be used for the reimport of '{3}'!"
			),
			FText::FromString(OriginalAsset->GetClass()->GetName()),
			FText::FromString(OriginalPrimPath),
			FText::FromString(ImportContext.FilePath),
			FText::FromString(OriginalAsset->GetPathName())
		));
