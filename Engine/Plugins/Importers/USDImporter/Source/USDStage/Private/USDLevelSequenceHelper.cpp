...
namespace UE::USDLevelSequenceHelper::Private
{
	static TSet<FName> TrackedCameraProperties = {
		...
		UnrealIdentifiers::DepthOfFieldFstopPropertyName,
		UnrealIdentifiers::CameraISOPropertyName,
		UnrealIdentifiers::CameraShutterSpeedPropertyName
	};
}
...
void FUsdLevelSequenceHelperImpl::AddLightTracks(const UUsdPrimTwin& PrimTwin, const UE::FUsdPrim& Prim, const TSet<FName>& PropertyPathsToRead)
{
	...
	for (const TPair<FName, ETrackType>& Pair : PropertyPathToTrackType)
	{
		...
		UsdUtils::DensifyTimeSamplesIfNeeded(Prim, PropertyPath, UnionedTimeSamples);
...
void FUsdLevelSequenceHelperImpl::RemoveSequenceForPrim(ULevelSequence& Sequence, const UUsdPrimTwin& PrimTwin)
{
	...
	if (PrimPathsForSequence.Find(PrimTwin.PrimPath) != INDEX_NONE)
	{
		...
		if (!PrimPathByLevelSequenceName.Contains(Sequence.GetFName()) && !LocalLayersSequences.Contains(Sequence.GetFName()))
		{
			...
			FString OldIdentifier;
			if (IdentifierByLevelSequence.RemoveAndCopyValue(&Sequence, OldIdentifier) && !OldIdentifier.IsEmpty())
			{
				LevelSequencesByIdentifier.Remove(OldIdentifier);
			}

			LayerIdentifierByLevelSequenceName.Remove(Sequence.GetName());
			...
			if (UsdPrimTwin)
			{
				...
				if (UE::FUsdPrim UsdPrim = UsdStage.GetPrimAtPath(UE::FSdfPath(*UsdPrimTwin->PrimPath)))
				{
					...
					if (bIsCamera)
					{
						...
						RemoveTimeSamplesForPropertyIfNeeded(UsdPrim, Guid, UnrealIdentifiers::DepthOfFieldFstopPropertyName);
						RemoveTimeSamplesForPropertyIfNeeded(UsdPrim, Guid, UnrealIdentifiers::CameraISOPropertyName);
						RemoveTimeSamplesForPropertyIfNeeded(UsdPrim, Guid, UnrealIdentifiers::CameraShutterSpeedPropertyName);
					}
					else if (bIsLight)
					{
