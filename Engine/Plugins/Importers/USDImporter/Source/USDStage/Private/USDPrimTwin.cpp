...
UUsdPrimTwin& UUsdPrimTwin::AddChild(const FString& InPrimPath)
{
	...
	TObjectPtr<UUsdPrimTwin>& ChildPrim = Children.Add(UsdUtils::AddHashToPrimPath(ChildPrimName));
...
UUsdPrimTwin* UUsdPrimTwin::Find(const FString& InPrimPath)
{
	...
	const FString& HashedChildName = UsdUtils::AddHashToPrimPath(ChildPrimName);
	if (Children.Contains(HashedChildName))
	{
		if (RestOfPrimPathToFind.IsEmpty())
		{
			return Children[HashedChildName];
		}
		else
		{
			return Children[HashedChildName]->Find(RestOfPrimPathToFind);
		}
	}
