				...
				if (!ExistingMaterial)
				{
					...
					if (MaterialAsset)
					{
						...
						{							
							...
							{
								FoundMaterialInterface->SetMaterialUsage(MATUSAGE_SkeletalMesh);
								FoundMaterialInterface->SetMaterialUsage(MATUSAGE_MorphTargets);
							}
							else if (ImportSettings->ImportType == EAlembicImportType::GeometryCache)
							{
								FoundMaterialInterface->SetMaterialUsage(MATUSAGE_GeometryCache);
							}
						}
					}
					else
