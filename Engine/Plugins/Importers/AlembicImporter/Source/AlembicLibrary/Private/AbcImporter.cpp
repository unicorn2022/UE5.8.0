		...
		if (NumPolyMeshes != 0)
		{
			...
			if (ImportSettings->GeometryCacheSettings.bFlattenTracks)
			{
				...
				TFunction<void(int32, FAbcFile*)> Callback = [this, &Tracks, &PolyMeshes, &PreviousNumVertices, &FrameTimes, &SlowTask, &CompletedFrames, &TotalFrames, &AssetName, NumFrames](int32 FrameIndex, const FAbcFile* InAbcFile)
				{
					...
					int32 FrameTimeIndex = FMath::Max(0, FrameIndex - ImportSettings->SamplingSettings.FrameStart);
				...
				{
					UE_LOGF(LogAbcImporter, Warning, "Alembic geometry cache import was interrupted");
				}
	...
	if (NumPolyMeshesToCompress)
	{
		if (InCompressionSettings.bMergeMeshes)
		{
...

			{
				...
				if (bTriggerWarning)
				{
					UE_LOGF(LogAbcImporter, Warning, "Vertex matrix has %llu elements because the mesh has %d vertices and the animation has %d frames. This can cause the import to take a long time and use a lot of memory.",
			...
			{
				UE_LOGF(LogAbcImporter, Warning, "Alembic skeletal mesh import was interrupted");
				return false;
			}
			SlowTask.Reset();
	...
	TArray64<float>& OutU, TArray64<float>& OutNormalsU, TArray64<float>& OutV)
{
	...
	UE_LOGF(LogAbcImporter, Log, "Decomposed animation and reconstructed with %i number of bases (full %lli, percentage %f, calculated %i)", NumUsedSingularValues, OutS.Num(), PercentageBasesUsed * 100.0f, NumUsedSingularValues);	
