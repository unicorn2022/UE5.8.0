...
bool FAbcPolyMesh::ReadFirstFrame(const float InTime, const int32 FrameIndex)
{
	...
	// Only try to read velocities when importing a geometry cache with ImportAbcVelocitiesAsMotionVectors
	const bool bReadVelocities = File->GetImportSettings()->ImportType == EAlembicImportType::GeometryCache &&
		File->GetImportSettings()->GeometryCacheSettings.MotionVectors == EAbcGeometryCacheMotionVectorsImport::ImportAbcVelocitiesAsMotionVectors;

	if (!bReadVelocities)
	{
		EnumRemoveFlags(SampleReadFlags, ESampleReadFlags::Velocities);
	}
	...
	// Remove the velocities flag again since GenerateAbcMeshSampleReadFlags generates a new SampleReadFlags from scratch
	if (!bReadVelocities)
	{
		EnumRemoveFlags(SampleReadFlags, ESampleReadFlags::Velocities);
	}
