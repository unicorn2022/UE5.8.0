...
public:
	...
	void TriggerReinitializationIfNeeded(const FPropertyChangedEvent& InEvent);
...
protected:
	...
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnNeedsReinitialized, const FName& /*UniqueStructName*/, const FPropertyChangedEvent& /*Modified Property*/);
	FOnNeedsReinitialized NeedsReinitializedDelegate;

	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnPreEditProperty, const FName& /*UniqueStructName*/, const FProperty* /*PropertyAboutToChange*/);
	FOnPreEditProperty PreEditPropertyDelegate;

	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnPostEditProperty, const FName& /*UniqueStructName*/, const FPropertyChangedEvent& /*Modified Property*/);
	FOnPostEditProperty PostEditPropertyDelegate;
...
public:
	...
	// attach a delegate to be notified whenever a property is edited that requires the system to be reinitialized
	// NOTE: This happens in a few hard-coded special cases or for any property marked with the meta tag: ReinitializeOnEdit
	FOnNeedsReinitialized& OnNeedsReinitialized(){ return NeedsReinitializedDelegate; };
	
	// attach a delegate to be notified BEFORE a property is edited
	FOnPreEditProperty& OnPreEditProperty(){ return PreEditPropertyDelegate; };
	
	// attach a delegate to be notified AFTER a property is edited
	FOnPostEditProperty& OnPostEditProperty(){ return PostEditPropertyDelegate; };	
};
