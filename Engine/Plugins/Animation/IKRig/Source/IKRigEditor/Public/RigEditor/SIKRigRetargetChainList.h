...
private:
	...
	/** pelvis / root bone widgets */
	void OnPelvisBoneComboSelectionChanged(FName InName);
	void OnRootBoneComboSelectionChanged(FName InName);
	FName GetPelvisBoneName(bool& bMultipleValues) const;
	FName GetRootBoneName(bool& bMultipleValues) const;
	const FReferenceSkeleton& GetReferenceSkeleton() const;
