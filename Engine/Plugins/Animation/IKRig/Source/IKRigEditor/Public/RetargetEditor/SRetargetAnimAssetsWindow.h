...
public:
	...
	void SetOverrideSets(const TArray<FName>& InOverrideSets);
...
public:
	...
	// Override set from the retargeter to apply during preview and export.
	// Only available when using a manually supplied retargeter asset.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Retargeter", meta = (EditCondition = "!bAutoGenerateRetargeter", GetOptions = "GetOverrideSetOptions"))
	FName OverrideSetToApply;

	UFUNCTION()
	TArray<FString> GetOverrideSetOptions() const;
...
private:
	...
	void ApplyOverrideSetToPreview();
