	...
	const bool bSuppressWarnings) const
{
	...
	if (!bSuppressWarnings)
	{
		...
		{
			if (!Processor->IsBoneMapped(Bone, SourceOrTarget))
			{
	...
	const ERetargetSourceOrTarget SourceOrTarget) const
{
	...
	if (!Processor->IsBoneMapped(BoneToAlign, SourceOrTarget))
	{
		return;
	}
	...
	FName ChainToAlign;
	FName ChainToMatch;
	if (!Processor->GetFirstChainMappingForBone(BoneToAlign, SideToAlign, ChainToAlign, ChainToMatch))
	{
		ensureMsgf(false, TEXT("Bone should never pass IsBoneMapped but not have both chains."));
		return;
	}
	
	// get the normalized chain parameter to match
	const float Param = Processor->GetParamOfBoneInChain(BoneToAlign, SideToAlign, ChainToAlign);
...
void FRetargetAutoPoseGenerator::SnapToGround(const FName BoneToPutOnGround, const ERetargetSourceOrTarget SourceOrTarget) const
{
	...
	if (BoneToPutOnGround == NAME_None)
	{
		...
		{
			if (!Processor->IsBoneMapped(Skeleton.BoneNames[BoneIndex], SourceOrTarget))
			{
				continue;
			}
	...
	ERetargetSourceOrTarget SourceOrTarget) const
{
	...
	{
		if (Processor->IsBoneMapped(Child, SourceOrTarget))
		{
			return false;
		}
	}
...
bool FRetargetAutoPoseGenerator::CheckReadyToAlignBones() const
{
	...
	// NOTE: we have to initialize a processor to "resolve" the retarget asset setup onto actual skeletons to do the alignment
	FRetargetProfile RetargetProfile;
	RetargetProfile.FillProfileWithAssetSettings(Controller->GetAsset());
	FRetargetInitParameters Params;
	Params.SourceSkeletalMesh = Controller->GetPreviewMesh(ERetargetSourceOrTarget::Source);
	Params.TargetSkeletalMesh = Controller->GetPreviewMesh(ERetargetSourceOrTarget::Target);
	Params.RetargeterAsset = Controller->GetAsset();
	Params.CustomProfile = &RetargetProfile;
	Params.bSuppressWarnings = true;
