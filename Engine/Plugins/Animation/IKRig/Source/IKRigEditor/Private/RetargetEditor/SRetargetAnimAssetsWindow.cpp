...
void SRetargetPoseViewport::SetOverrideSets(const TArray<FName>& InOverrideSets)
{
	if (TargetAnimInstance)
	{
		TargetAnimInstance->SetOverrideSets(InOverrideSets);
	}
}
	...
	: FEditorViewportClient(nullptr, &InPreviewScene, StaticCastSharedRef<SEditorViewport>(InRetargetPoseViewport))
{
	...
	DefaultProfile.LoadProfileObjects();
...
void FProceduralRetargetAssets::AutoGenerateIKRetargetAsset()
{
	...
	const FName TargetPelvis = TargetCharacterizationResults.AutoRetargetDefinition.RetargetDefinition.PelvisBone;
	if (TargetSkeletonRoot != TargetPelvis)
	{
		...
		const FName SourcePelvis = SourceCharacterizationResults.AutoRetargetDefinition.RetargetDefinition.PelvisBone;
...
SRetargetAnimAssetsWindow::SRetargetAnimAssetsWindow()
{
	...
	// reset settings to defaults when opening a new dialog
	Settings->bAutoGenerateRetargeter = true;
	Settings->RetargetAsset = nullptr;
	Settings->OverrideSetToApply = NAME_None;
...
void SRetargetAnimAssetsWindow::OnFinishedChangingSelectionProperties(const FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (ChangedRetargeter)
	{
		...
		// clear override set if it no longer exists on the new retargeter
		if (Settings->RetargetAsset)
		{
			const TMap<FName, FRetargetOverrideSet>& ValidSets = Settings->RetargetAsset->GetOverrideSets();
			if (!Settings->OverrideSetToApply.IsNone() && !ValidSets.Contains(Settings->OverrideSetToApply))
			{
				Settings->OverrideSetToApply = NAME_None;
			}
		}
	...
	const bool ChangedOverrideSet = PropertyChangedEvent.Property->GetName() == "OverrideSetToApply";
	if (ChangedOverrideSet)
	{
		ApplyOverrideSetToPreview();
	}
}
...
FReply SRetargetAnimAssetsWindow::OnExportAnimations()
{
	...
	// apply override set to batch context
	BatchContext.OverrideSetNames.Reset();
	if (!Settings->OverrideSetToApply.IsNone())
	{
		BatchContext.OverrideSetNames.Add(Settings->OverrideSetToApply);
	}
...
FReply SRetargetAnimAssetsWindow::OnExportRetargetAssets()
{
	...
	auto SaveAssetToDisk = [](UObject* Asset, FString& AssetName, const FString& FolderPath)
	{
		...
		const FString BasePackageName = FolderPath / AssetName;
		const FString Suffix = "";
		FString UniquePackageName, UniqueAssetName;
		IAssetTools& AssetTools = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools").Get();
		AssetTools.CreateUniqueAssetName(BasePackageName, Suffix, UniquePackageName, UniqueAssetName);
		
		// duplicate the asset
		FObjectDuplicationParameters ObjParameters(Asset, GetTransientPackage());
		ObjParameters.DestName = MakeUniqueObjectName(GetTransientPackage(), Asset->GetClass(), *UniqueAssetName);
		...
		AssetToSave->Rename(*UniqueAssetName, Package);
...
TArray<FString> UBatchRetargetSettings::GetOverrideSetOptions() const
{
	...
	if (RetargetAsset)
	{
		const TMap<FName, FRetargetOverrideSet>& AllSets = RetargetAsset->GetOverrideSets();
		for (const TTuple<FName, FRetargetOverrideSet>& Entry : AllSets)
		{
...
void SRetargetAnimAssetsWindow::ApplyOverrideSetToPreview()
{
	BatchContext.OverrideSetNames.Reset();
	if (!Settings->OverrideSetToApply.IsNone())
	{
	...
	if (Viewport.IsValid())
	{
		Viewport->SetOverrideSets(BatchContext.OverrideSetNames);
	}
}
