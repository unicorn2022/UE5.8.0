    ...
    [
		...
		[
			...
			[
				...
				.Visibility_Lambda([this]()
				{
					const bool bIsVisible = IsChainMappingEnabled();
...
void SIKRetargetChainMapList::RefreshView()
{
	...
	// record selected item to restore
	FName SelectedChainName = NAME_None;
	TArray<TSharedPtr<FRetargetChainMapElement>> SelectedItems = ListView->GetSelectedItems();
	if (!SelectedItems.IsEmpty())
	{
		SelectedChainName = SelectedItems[0].Get()->TargetChainName;	
	}
	...
	for (const FRetargetChainPair& ChainPair : ChainMap)
	{
		...
		// restore selection
		if (ChainItem.Get()->TargetChainName == SelectedChainName)
		{
			ListView->SetSelection(ChainItem);
		}
	}
