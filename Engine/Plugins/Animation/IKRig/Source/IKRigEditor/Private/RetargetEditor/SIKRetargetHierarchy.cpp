...
FName FIKRetargetHierarchyElement::GetChainName() const
{
	...
	return Controller->GetChainDisplayNameForBone(Name, Controller->GetSourceOrTarget());
}
...
void SIKRetargetHierarchy::ShowItemAfterSelection(FName ItemName)
{
	...
	{
		TWeakPtr<FIKRetargetHierarchyElement> ItemToExpand = ItemToShow->Parent;
		while(ItemToExpand.IsValid())
		{
			TreeView->SetItemExpansion(ItemToExpand.Pin(), true);
			ItemToExpand = ItemToExpand.Pin()->Parent;
		}
	}
...
void SIKRetargetHierarchy::RefreshTreeView(bool IsInitialSetup)
{
	...
	for (const FName BoneName : BoneNames)
	{
		...
		FName ChainName = Controller->GetChainDisplayNameForBone(BoneName, Controller->GetSourceOrTarget());
	...
	for (int32 BoneIndex=0; BoneIndex<BoneNames.Num(); ++BoneIndex)
	{
		...
		TWeakPtr<FIKRetargetHierarchyElement> BoneTreeElement = AllElements[BoneTreeElementIndices[BoneName]];

		if (BoneTreeElement.IsValid() && BoneTreeElement.Pin()->bIsHidden)
		{
			continue;
		}
		
		// find first parent that is not filtered
		TWeakPtr<FIKRetargetHierarchyElement> FirstNonHiddenParentElement;
		...
		if (FirstNonHiddenParentElement.IsValid())
		{
			// store pointer to child on parent
			FirstNonHiddenParentElement.Pin()->Children.Add(BoneTreeElement);
			// store pointer to parent on child
			BoneTreeElement.Pin()->Parent = FirstNonHiddenParentElement;
		}
		else
		{
			// has no parent, store a root element
			RootElements.Add(BoneTreeElement.Pin());
		}
	}
...
{
	for (TWeakPtr<FIKRetargetHierarchyElement>& Child : InItem->Children)
	{
