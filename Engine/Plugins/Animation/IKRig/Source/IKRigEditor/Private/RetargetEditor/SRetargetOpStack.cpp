...
void SRetargetOpItem::Construct(const FArguments& InArgs)
{
	...
	[
		SNew(SBorder)
		 .BorderImage(FAppStyle::GetBrush("NoBorder")) // Invisible but hit-testable
		 .Visibility(EVisibility::Visible)
		 .OnMouseButtonUp_Lambda([this](const FGeometry& MyGeometry, const FPointerEvent& MouseEvent) -> FReply
		 {
			...
			// build a menu for this op
			FMenuBuilder MenuBuilder(true, nullptr);
			GenerateRightClickMenu(MenuBuilder);

			// summon the menu at the cursor's location
		...
		[
			...
			{
				const UScriptStruct* OpType = Element.Pin()->GetType();
				if (!OpType)
				{
					return FText::FromString(TEXT("Unknown Op type. Resave asset to clean."));
				}
					
				return FText::Format(
				FText::FromString(TEXT("Op Type: {0} \nIndex in Stack: {1}\n")),
				FText::FromString(OpType->GetStructCPPName()),
			...
			[
				...
				{
					FIKRetargetEditorController* EditorController = GetEditorController();
			...
			[
				...
				.Text_Lambda([this]() -> FText
				{
					FIKRetargetEditorController* EditorController = GetEditorController();
					FIKRetargetOpBase* Op = EditorController->AssetController->GetRetargetOpByIndex(Element.Pin()->GetIndexInStack());
					const double Milliseconds = (Op && Op->EditorInstance ? Op->EditorInstance->Timer.GetRunningAverage() : 0.0f) * 1000.0;
...
void SRetargetOpItem::GenerateRightClickMenu(FMenuBuilder& InMenuBuilder)
{
	FIKRetargetEditorController* EditorController = GetEditorController();
	if (!EditorController)
	{
	...
	TObjectPtr<UIKRetargeterController> AssetController = EditorController->AssetController;
	FIKRetargetOpBase* Op = AssetController->GetRetargetOpByIndex(Element.Pin()->GetIndexInStack());
	if (!ensure(Op))
	{
		return;
	}

	const FName SelectedOverrideSet = EditorController->GetSelectedOverrideSet();
	const FName SelectedOpName = Op->GetName();

	if (SelectedOverrideSet == NAME_None)
	{
		InMenuBuilder.BeginSection("WarningSection", LOCTEXT("WarningHeading", "Notice"));
		{
			InMenuBuilder.AddWidget(
				SNew(STextBlock)
				.Text(LOCTEXT("NoSelectionWarning", "No profile selected to add overrides to. \nOpen the Retarget Profiles tab to edit a profile."))
				.ColorAndOpacity(FAppStyle::GetColor("ErrorReporting.WarningBackgroundColor"))
				.Margin(FMargin(16.0f, 4.0f))
	...
	InMenuBuilder.BeginSection("ProfileActions", FText::Format(LOCTEXT("ProfileHeading", "Edit Profile: {0}"), FText::FromName(SelectedOverrideSet)));

	InMenuBuilder.AddMenuEntry(
		LOCTEXT("AddChangedLabel", "Add Changed Properties"),
		LOCTEXT("AddChangedTooltip", "Record all modifications to the op settings into the currently selected override set."),
		FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Plus"),
		FUIAction(FExecuteAction::CreateLambda([this, SelectedOverrideSet, SelectedOpName]()
			{
				GetEditorController()->AssetController->OverrideAllEditedProperties(SelectedOverrideSet, SelectedOpName);
			})));

	InMenuBuilder.AddMenuEntry(
		LOCTEXT("AddAllLabel", "Add All Properties"),
		LOCTEXT("AddAllTooltip", "Record all the op settings into the currently selected override set."),
		FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Plus"),
		FUIAction(FExecuteAction::CreateLambda([this, SelectedOverrideSet, SelectedOpName]()
			{
				GetEditorController()->AssetController->OverrideAllProperties(SelectedOverrideSet, SelectedOpName);
			})));

	InMenuBuilder.AddMenuEntry(
		LOCTEXT("ClearAllLabel", "Clear All Overrides"),
		LOCTEXT("ClearAllTooltip", "Removes all sparse overrides from this overrise set."),
		FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Delete"),
		FUIAction(FExecuteAction::CreateLambda([this, SelectedOverrideSet, SelectedOpName]()
			{
				GetEditorController()->AssetController->RemoveOpOverrides(SelectedOverrideSet, SelectedOpName);
			})));
	...
	{
		InMenuBuilder.EndSection();
		InMenuBuilder.BeginSection("OpProperties", LOCTEXT("ProfileOverrideHeading", "Add Overrides to Profile"));
		MakeOverrideSubMenu(InMenuBuilder, RootNode);
		InMenuBuilder.EndSection();
	}
}

FIKRetargetEditorController* SRetargetOpItem::GetEditorController() const
{
	return ListView.IsValid() ? ListView.Pin()->EditorController.Pin().Get() : nullptr;
}

void SRetargetOpItem::MakeOverrideSubMenu(FMenuBuilder& SubMenuBuilder, TSharedPtr<FRetargetPropertyOverrideNode> Node)
{
	const FName ProfileName = GetEditorController()->GetSelectedOverrideSet();
	FName OpName = GetEditorController()->AssetController->GetOpName(Element.Pin()->GetIndexInStack());
	
	// add leaf properties first
	for (const FRetargetPropertyOverrideNode::FRetargetPropertyOverrideEntry& Entry : Node->LeafProperties)
	{
		SubMenuBuilder.AddMenuEntry(
			Entry.DisplayName,
			FText::Format(LOCTEXT("OverrideTipLabel", "Add or Remove {0} override in current override set."), FText::FromString(Entry.PropertyPath)),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this, ProfileName, OpName, Entry]()
				{
					TObjectPtr<UIKRetargeterController> AssetController = GetEditorController()->AssetController;
					if (AssetController->HasPropertyOverride(ProfileName, OpName, Entry.PropertyPath))
					{
						// remove it
						AssetController->RemovePropertyOverrideFromOp(ProfileName, OpName, Entry.PropertyPath);
					}
					else
					{
						// add it
						AssetController->AddPropertyOverrideToOp(ProfileName, OpName, Entry.PropertyPath);	
					}
				}),
				FCanExecuteAction(),
				FIsActionChecked::CreateLambda([this, ProfileName, OpName, Entry]()
				{
					TObjectPtr<UIKRetargeterController> AssetController = GetEditorController()->AssetController;
					return AssetController->HasPropertyOverride(ProfileName, OpName, Entry.PropertyPath);
				})
			),
			NAME_None,
	...
	// add sub-nodes (nested submenus)
	for (TTuple<FName, TSharedPtr<FRetargetPropertyOverrideNode>>& Pair : Node->SubNodes)
	{
		SubMenuBuilder.AddSubMenu(
			FText::FromName(Pair.Key),
			FText::GetEmpty(),
			FNewMenuDelegate::CreateSP(this, &SRetargetOpItem::MakeOverrideSubMenu, Pair.Value)
		);
	}
}
...
void SRetargetOpList::CacheOpTypeMetaData()
{
	...
	for (const FInstancedStruct& TempOpStruct : TempInstancedOps)
	{
		...
		// filter deprecated ops
		static const FName DeprecatedStruct = TEXT("Deprecated");
		if (TempOp->GetType()->HasMetaData(DeprecatedStruct))
		{
			continue;
		}

		// disallow adding abstract types to the stack
		if (TempOp->IsAbstract())
		{
			continue;
		}
