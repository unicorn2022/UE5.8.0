...
void FIKRetargetEditor::BindCommands()
{
	...
	ToolkitCommands->MapAction(
		Commands.ShowOverrideSets,
		FExecuteAction::CreateLambda([this]()
		{
			TSharedPtr<SDockTab> Tab = TabManager->TryInvokeTab(FIKRetargetOverridesTabSummoner::TabID);
			if (Tab.IsValid())
			{
				Tab->ActivateInParent(ETabActivationCause::SetDirectly);
				
				if (TSharedPtr<SWindow> ParentWindow = Tab->GetParentWindow())
				{
		...
		FIsActionChecked::CreateLambda([this]() ->bool
		{
			TSharedPtr<SDockTab> ExistingTab = TabManager->FindExistingLiveTab(FIKRetargetOverridesTabSummoner::TabID);
			return ExistingTab.IsValid() && ExistingTab->IsForeground();
		}));
	...
	ToolkitCommands->MapAction(
		...
		FExecuteAction::CreateLambda([this]()
		{
			TSharedPtr<SDockTab> Tab = TabManager->TryInvokeTab(FIKRetargetVariablesTabSummoner::TabID);
			if (Tab.IsValid())
			{
				Tab->ActivateInParent(ETabActivationCause::SetDirectly);
		...
		FIsActionChecked::CreateLambda([this]() ->bool
		{
			TSharedPtr<SDockTab> ExistingTab = TabManager->FindExistingLiveTab(FIKRetargetVariablesTabSummoner::TabID);
			return ExistingTab.IsValid() && ExistingTab->IsForeground();
		}));
...
void FIKRetargetEditor::FillToolbar(FToolBarBuilder& ToolbarBuilder)
{
	...
	ToolbarBuilder.BeginSection("Show Settings");
	{
		...
		ToolbarBuilder.AddToolBarButton(
		FIKRetargetCommands::Get().ShowOverrideSets,
		NAME_None,
		TAttribute<FText>(),
		TAttribute<FText>(),
		FSlateIcon(FIKRetargetEditorStyle::Get().GetStyleSetName(),"IKRetarget.AssetSettings"));
		...
		ToolbarBuilder.AddToolBarButton(
		FIKRetargetCommands::Get().ShowVariables,
		NAME_None,
		TAttribute<FText>(),
		TAttribute<FText>(),
		FSlateIcon(FIKRetargetEditorStyle::Get().GetStyleSetName(),"IKRetarget.AssetSettings"));
...
{	
	...
	// show flags only control the source mesh, suppress the default trajectory mode on target mesh to avoid drawing a second trajectory.
	EditorController->TargetSkelMeshComponent->SetVisualizeRootMotionMode(EVisualizeRootMotionMode::None);
