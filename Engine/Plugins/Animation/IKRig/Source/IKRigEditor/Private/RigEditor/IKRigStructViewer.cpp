...
void UIKRigStructViewer::TriggerReinitializationIfNeeded(const FPropertyChangedEvent& InEvent)
{
	if (!InEvent.Property)
	{
		return;
	...
	const bool bIsInteractive = InEvent.ChangeType == EPropertyChangeType::Interactive;
	const bool bNeedsReinit = InEvent.Property->HasMetaData(IKRigReinitOnEditMetaLabel);

	// Allow reinitialization during interactive slider drag only for properties explicitly
	// marked ReinitializeOnEdit — this enables live viewport updates (e.g. foot dimension
	// debug drawing) while still avoiding unnecessary reinits for non-marked properties.
	if (bIsInteractive && !bNeedsReinit)
	{
		return;
	}
	...
	if (bNeedsReinit)
	{
		OnNeedsReinitialized().Broadcast(StructToView.UniqueName, InEvent);
		return;
	}
...
void UIKRigStructViewer::SetupPropertyEditingCallbacks(const TSharedPtr<IPropertyHandle>& InProperty)
{
	...
	InProperty->SetOnPropertyValueChangedWithData(TDelegate<void(const FPropertyChangedEvent&)>::CreateLambda([this](const FPropertyChangedEvent& InEvent)
	{
		...
		// Push wrapper values to the underlying struct before triggering reinitialization,
		// so the retargeter reads the latest values when it reinitializes.
		if (UIKRigStructWrapperBase* Wrapper = Cast<UIKRigStructWrapperBase>(this))
		{
			Wrapper->UpdateWrappedStructWithLatestValues();
		}
		TriggerReinitializationIfNeeded(InEvent);
	}));
...
USkeleton* UIKRigStructViewer::GetSkeleton(bool& bInvalidSkeletonIsError, const IPropertyHandle* PropertyHandle)
{
	...
	if (StructToView.Type->IsChildOf(FIKRetargetOpSkeletonProvider::StaticStruct()))
	{
		FName PropertyName = PropertyHandle->GetProperty()->GetFName();
		uint8* StructMemory = StructToView.MemoryProvider();
		FIKRetargetOpSkeletonProvider* SkeletonProvider = reinterpret_cast<FIKRetargetOpSkeletonProvider*>(StructMemory);
		USkeleton* Skeleton = SkeletonProvider->GetSkeleton(PropertyName);
		if (!Skeleton)
		{
			// if GetSkeleton(FName) returns null, try the alternative function that takes a property handle
			Skeleton = SkeletonProvider->GetSkeleton(PropertyHandle);
		}
		return Skeleton;
	}
	...
	UIKRetargeterController* InRetargeterController)
{
	...
	OnNeedsReinitialized().AddUObject(InRetargeterController, &UIKRetargeterController::OnOpPropertyChanged);
}
...
void UIKRigStructWrapperBase::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	// Paste operations (and other struct-level changes) come through PostEditChangeProperty rather than the
	// IPropertyHandle callbacks set up in SetupPropertyEditingCallbacks. This means that pasting property
	// values (e.g. right-click -> Paste on the Foot Settings section) correctly updates the underlying struct
	// data but does NOT trigger the reinitialization broadcast needed to refresh the viewport.
	// Calling TriggerReinitializationIfNeeded here ensures the viewport updates after a paste.
	TriggerReinitializationIfNeeded(PropertyChangedEvent);
}
