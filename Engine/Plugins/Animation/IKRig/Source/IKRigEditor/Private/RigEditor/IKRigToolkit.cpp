...
bool FIKRigEditorToolkit::MatchesContext(const FTransactionContext& InContext, const TArray<TPair<UObject*, FTransactionObjectEvent>>& TransactionObjectContexts) const
{
	if (InContext.PrimaryObject == EditorController->AssetController)
	{
		return true;
	}
	for (const TPair<UObject*, FTransactionObjectEvent>& ObjectTransactionPair : TransactionObjectContexts)
	{
		if (ObjectTransactionPair.Get<0>() == EditorController->AssetController)
		{
			return true;
		}
	}	
