...
bool UIKRigController::AddBoneSetting(const FName BoneName, int32 SolverIndex) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Bone settings refused by solver. Reason: %ls", *ErrorMessage.ToString());
...
bool UIKRigController::RemoveBoneSetting(const FName BoneName, int32 SolverIndex) const
{
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Solver does not exist at index: %d.", SolverIndex);
		return false; // solver doesn't exist
	}

	if (Asset->Skeleton.GetBoneIndexFromName(BoneName) == INDEX_NONE)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Cannot remove setting on unknown bone, %ls.", *BoneName.ToString());
...
FName UIKRigController::AddRetargetChainInternal(const FBoneChain& BoneChain) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Could not create retarget chain. Start Bone does not exist, %ls.", *BoneChain.StartBone.BoneName.ToString());
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Could not create retarget chain. End Bone does not exist, %ls.", *BoneChain.EndBone.BoneName.ToString());
...
bool UIKRigController::RemoveRetargetChain(const FName ChainName) const
{
	...
	UE_LOGF(LogIKRigEditor, Warning, "Retarget chain not found: %ls.", *ChainName.ToString());
	return false;
}
...
bool UIKRigController::SetRetargetChainStartBone(const FName ChainName, const FName StartBoneName) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Bone does not exist, %ls.", *StartBoneName.ToString());
		return false; // bone doesn't exist
	}
...
bool UIKRigController::SetRetargetChainEndBone(const FName ChainName, const FName EndBoneName) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Bone does not exist, %ls.", *EndBoneName.ToString());
		return false; // bone doesn't exist
	}
...
bool UIKRigController::SetRetargetRoot(const FName InPelvisBoneName) const
{
	check(Asset)

	FName NewPelvisBone = InPelvisBoneName;
	if (InPelvisBoneName != NAME_None && Asset->Skeleton.GetBoneIndexFromName(InPelvisBoneName) == INDEX_NONE)
	{
		NewPelvisBone = NAME_None;
	}

	FScopedTransaction Transaction(LOCTEXT("SetPelvisBone_Label", "Set Pelvis Bone"));
	FScopedReinitializeIKRig Reinitialize(this);
	Asset->Modify();
	Asset->RetargetDefinition.PelvisBone = NewPelvisBone;
	return true;
}
...
FName UIKRigController::GetRetargetRoot() const
{
	...
	return Asset->RetargetDefinition.PelvisBone;
}

bool UIKRigController::SetRootMotionBone(const FName InRootBoneName) const
{
	check(Asset)

	FName NewRootBone = InRootBoneName;
	if (InRootBoneName != NAME_None && Asset->Skeleton.GetBoneIndexFromName(InRootBoneName) == INDEX_NONE)
	{
...
FName UIKRigController::GetRootMotionBone() const
{
	check(Asset);
...
bool UIKRigController::SetSkeletalMesh(USkeletalMesh* SkeletalMesh) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to initialize IKRig with a Skeleton that is missing required bones. See output log. %ls", *Asset->GetName());
		FScopedReinitializeIKRig Reinitialize(this);
		return false;
	}
...
int32 UIKRigController::AddSolver(const FString InIKRigSolverType) const
{
	...
	if (!SolverType)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Specified solver type was not found, %ls.", *InIKRigSolverType);
		return INDEX_NONE;
	}
...
bool UIKRigController::SetBoneExcluded(const FName BoneName, const bool bExclude) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to exclude non-existant bone, %ls.", *BoneName.ToString());
		return false;
	}
...
{	
	...
	if (BoneIndex == INDEX_NONE)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Tried to get the ref pose of bone that is not loaded into this rig.");
...
int32 UIKRigController::AddSolver(UScriptStruct* InIKRigSolverType) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Could not add solver to IK Rig. No solver type specified.");
		return INDEX_NONE;
	}

	if (!InIKRigSolverType->IsChildOf(FIKRigSolverBase::StaticStruct()))
	{
		UE_LOGF(LogIKRigEditor, Warning, "Could not add solver to IK Rig. Invalid solver type specified. Must be child of FIKRigSolverBase.");
		return INDEX_NONE;
	}
...
bool UIKRigController::RemoveSolver(const int32 SolverIndex) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Solver not removed. Invalid index, %d.", SolverIndex);
		return false;
	}
...
bool UIKRigController::MoveSolverInStack(int32 SolverToMoveIndex, int32 TargetSolverIndex) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Solver not moved. Invalid source index, %d.", SolverToMoveIndex);
		return false;
	}
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Solver not moved. Invalid target index, %d.", TargetSolverIndex);
		return false;
	}

	if (SolverToMoveIndex == TargetSolverIndex)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Solver not moved. Source and target index cannot be the same.");
		return false;
	}
...
bool UIKRigController::SetSolverEnabled(int32 SolverIndex, bool bIsEnabled) const
{
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Solver not enabled or disabled. Invalid index, %d.", SolverIndex);
		return false;
	}
...
bool UIKRigController::GetSolverEnabled(int32 SolverIndex) const
{
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Invalid solver index, %d.", SolverIndex);
		return false;
	}
...
bool UIKRigController::SetStartBone(const FName RootBoneName, int32 SolverIndex) const
{
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Start bone not set. Invalid solver index, %d.", SolverIndex);
		return false; // solver doesn't exist
	}

	if (Asset->Skeleton.GetBoneIndexFromName(RootBoneName) == INDEX_NONE)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Start bone not set. Invalid bone specified, %ls.", *RootBoneName.ToString());
		return false; // bone doesn't exist
	}
...
FName UIKRigController::GetStartBone(int32 SolverIndex) const
{
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Could not query root bone. Invalid solver index, %d.", SolverIndex);
...
bool UIKRigController::SetEndBone(const FName EndBoneName, int32 SolverIndex) const
{
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "End bone not set. Invalid solver index, %d.", SolverIndex);
		return false; // solver doesn't exist
	}

	if (Asset->Skeleton.GetBoneIndexFromName(EndBoneName) == INDEX_NONE)
	{
		UE_LOGF(LogIKRigEditor, Warning, "End bone not set. Invalid bone specified, %ls.", *EndBoneName.ToString());
		return false; // bone doesn't exist
	}

	if (!Solver->UsesEndBone())
	{
		UE_LOGF(LogIKRigEditor, Warning, "End bone not set. Specified solver does not support end bones.");
		return false; //
	}
...
FName UIKRigController::GetEndBone(int32 SolverIndex) const
{
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "End bone not queried. Invalid solver index, %d.", SolverIndex);
		return NAME_None; // solver doesn't exist
	}
...
FName UIKRigController::AddNewGoal(const FName GoalName, const FName BoneName) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to create a Goal that already exists, %ls.", *GoalName.ToString());
		return NAME_None;
	}
	...
	if (BoneIndex == INDEX_NONE)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to create Goal on unknown bone, %ls.", *BoneName.ToString());
		return NAME_None;
	}
...
bool UIKRigController::ConnectGoalToSolver(const FName GoalName, int32 SolverIndex) const
{
	...
	if (GoalIndex == INDEX_NONE)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to connect unknown Goal, %ls to a solver.", *GoalName.ToString());
		return false;
	}
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to connect Goal, %ls to a unknown solver with index, %d.", *GoalName.ToString(), SolverIndex);
		return false;
	}
...
bool UIKRigController::DisconnectGoalFromSolver(const FName GoalToRemove, int32 SolverIndex) const
{
	...
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to disconnect unknown Goal, %ls.", *GoalToRemove.ToString());
		return false;
	}
	...
	if (!Solver)
	{
		UE_LOGF(LogIKRigEditor, Warning, "Trying to disconnect Goal, %ls from an unknown solver with index, %d.", *GoalToRemove.ToString(), SolverIndex);
		return false;
	}
