...
const FReferenceSkeleton& SIKRigRetargetChainRow::GetReferenceSkeleton() const
{
	...
	{
		return ChainList.Pin()->GetReferenceSkeleton();	
	}
...
void SIKRigRetargetChainList::Construct(const FArguments& InArgs, TSharedRef<FIKRigEditorController> InEditorController)
{
    ...
    [
        ...
        [
			...
			[
				SNew(SBoneSelectionWidget)
				.OnBoneSelectionChanged(this, &SIKRigRetargetChainList::OnPelvisBoneComboSelectionChanged)
				.OnGetSelectedBone(this, &SIKRigRetargetChainList::GetPelvisBoneName)
				.OnGetReferenceSkeleton(this, &SIKRigRetargetChainList::GetReferenceSkeleton)
			]
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("RootLabel", "Root Motion:"))
				.TextStyle(FAppStyle::Get(), "NormalText")
			]
			...
			[
				SNew(SBoneSelectionWidget)
				.OnBoneSelectionChanged(this, &SIKRigRetargetChainList::OnRootBoneComboSelectionChanged)
				.OnGetSelectedBone(this, &SIKRigRetargetChainList::GetRootBoneName)
				.OnGetReferenceSkeleton(this, &SIKRigRetargetChainList::GetReferenceSkeleton)
			]
        ]
...
void SIKRigRetargetChainList::OnPelvisBoneComboSelectionChanged(FName InName)
{
	...
	Controller->AssetController->SetRetargetRoot(InName);
	RefreshView();
}

void SIKRigRetargetChainList::OnRootBoneComboSelectionChanged(FName InName)
{
	const TSharedPtr<FIKRigEditorController> Controller = EditorController.Pin();
	if (!Controller.IsValid())
	{
	...
	Controller->AssetController->SetRootMotionBone(InName);
	RefreshView();
}

FName SIKRigRetargetChainList::GetPelvisBoneName(bool& bMultipleValues) const
{
	const TSharedPtr<FIKRigEditorController> Controller = EditorController.Pin();
	if (!Controller.IsValid())
	{
	...
	bMultipleValues = false;
	return Controller->AssetController->GetRetargetRoot();
}

FName SIKRigRetargetChainList::GetRootBoneName(bool& bMultipleValues) const
{
	const TSharedPtr<FIKRigEditorController> Controller = EditorController.Pin();
	if (!Controller.IsValid())
	{
	...
	bMultipleValues = false;
	return Controller->AssetController->GetRootMotionBone();
}

const FReferenceSkeleton& SIKRigRetargetChainList::GetReferenceSkeleton() const
{
	static const FReferenceSkeleton DummySkeleton;
