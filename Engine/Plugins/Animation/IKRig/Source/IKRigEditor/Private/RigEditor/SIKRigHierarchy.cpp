...
void SIKRigHierarchy::OnSelectionChanged(TSharedPtr<FIKRigTreeElement> Selection, ESelectInfo::Type SelectInfo)
{
	...
	// update details view — reorder so the last clicked item is first, ensuring
	// ShowDetailsForElements pivots on the correct type when a mixed selection is made
	TArray<TSharedPtr<FIKRigTreeElement>> SelectedItems = TreeView->GetSelectedItems();
	if (const TSharedPtr<FIKRigTreeElement> LastSelected = TreeView->LastSelected.Pin())
	{
		const int32 LastSelectedIndex = SelectedItems.IndexOfByPredicate(
			[&LastSelected](const TSharedPtr<FIKRigTreeElement>& Item) { return Item == LastSelected; });
		if (LastSelectedIndex > 0)
		{
			SelectedItems.Swap(0, LastSelectedIndex);
		}
	}
