...
void FIKRigEditorController::Initialize(TSharedPtr<FIKRigEditorToolkit> Toolkit, const UIKRigDefinition* IKRigAsset)
{
	...
	BoneDetails.Reset(); // populated on demand in ShowDetailsForBones()
...
void FIKRigEditorController::HandleIKRigNeedsInitialized(UIKRigDefinition* ModifiedIKRig) const
{
	...
	// update all bone details objects so they can pull on the current data
	for (const TObjectPtr<UIKRigBoneDetails>& BoneDetail : BoneDetails)
	{
		BoneDetail->AnimInstancePtr = AnimInstance;
		BoneDetail->AssetPtr = ModifiedIKRig;
	}
...
void FIKRigEditorController::AutoGenerateFBIK() const
{
	...
	case EAutoFBIKResult::UnknownSkeletonType:
		...
		break;
	case EAutoFBIKResult::MissingPelvisBone:
		{
...
void FIKRigEditorController::ShowDetailsForBones(const TArray<FName>& BoneNames) const
{
	// grow the owned pool to match the number of bones to display
	while (BoneDetails.Num() < BoneNames.Num())
	{
		BoneDetails.Add(NewObject<UIKRigBoneDetails>());
	}

	TArray<UObject*> BoneDetailObjects;
	for (int32 i = 0; i < BoneNames.Num(); ++i)
	{
		BoneDetails[i]->SetBone(BoneNames[i]);
		BoneDetails[i]->AnimInstancePtr = AnimInstance;
		BoneDetails[i]->AssetPtr = AssetController->GetAsset();
		BoneDetailObjects.Add(BoneDetails[i]);
	}

	if (BoneDetailObjects.Num() == 1)
	{
		DetailsView->SetObject(BoneDetailObjects[0]);
	}
	else
	{
		DetailsView->SetObjects(BoneDetailObjects);
	}
}
	...
	case EIKRigTreeElementType::BONE:
		{
			...
			for (const TSharedPtr<FIKRigTreeElement>& Item : ToEdit)
			{
				BoneNames.Add(Item->BoneName);
			}
			ShowDetailsForBones(BoneNames);
		}
		break;
