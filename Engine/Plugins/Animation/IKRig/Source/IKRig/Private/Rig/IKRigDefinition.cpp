...
bool FRetargetDefinition::Serialize(FArchive& Ar)
{
...
void FRetargetDefinition::PostSerialize(const FArchive& Ar)
{
	if (Ar.IsLoading())
	{
		if (Ar.CustomVer(FIKRigObjectVersion::GUID) < FIKRigObjectVersion::AddedRootBoneToRetargetDefinition)
		{
			// we used to call "Pelvis" the "RetargetRoot"
			// but now we also want to characterize the actual skeleton "root" bone itself
			PelvisBone = RootBone;
		}
	}
}
...
void UIKRigDefinition::PostLoad()
{
	...
	for (TObjectPtr<UIKRigSolver> Solver : Solvers_DEPRECATED)
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "IK Rig: unable to load old UObject based solver type. Conversion failed for type:  %ls", *Solver->GetName());
		}
	}
	...
	// clean out null goals
	Goals.RemoveAll([](const TObjectPtr<UIKRigEffectorGoal>& InGoal)
	{
		return !InGoal;
	});
}
