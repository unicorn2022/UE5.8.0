	...
	const FIKRigGoalContainer& InOptionalGoals)
{
	...
	AssetInitializedWith = InRigAsset;
...
void FIKRigProcessor::Solve(const FTransform& ComponentToWorld)
{
	...
	if (GoalContainer.NeedsInitialized())
	{
		// save input pose
		TArray<FTransform> InputPoseGlobal = Skeleton.CurrentPoseGlobal;
		// make a copy of the goal container in it's current state to reinitialize with
		FIKRigGoalContainer CopiedGoalContainer = GoalContainer;
		// reinitialize
		Initialize(AssetInitializedWith, Skeleton.SkeletalMesh, CopiedGoalContainer);
		// restore input pose after reinit
		if (IsInitialized())
		{
			SetInputPoseGlobal(InputPoseGlobal);
		}
	}
