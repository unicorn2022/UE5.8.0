...
bool FRetargetOpProfile::CopySettingsToOp(FInstancedStruct& InOutOpStruct, ECopyOpSettingsContext InApplyContext) const
{
	...
	if (!ensure(Op))
	{
		return false; // should never get here with an invalid op struct
	}
	FIKRetargetOpSettingsBase* OpSettings = Op->GetSettings();
	if (!ensure(OpSettings))
	{
		return false; // no settings to modify
	}
	const UScriptStruct* OpSettingsType = Op->GetSettingsType();
	if (!ensure(OpSettingsType))
	{
		return false; // unknown settings type
	}

	if (!SettingsToApply.IsValid() || OpSettingsType != SettingsToApply.GetScriptStruct())
	{
		return false; // invalid settings data or incorrect type
	}
	
	// apply wholesale op settings struct overrides
	...
	case ECopyOpSettingsContext::Runtime:
		// apply the profile settings to an op in the stack through the CopySettingsAtRuntime interface
		// this gives each op a chance to digest the new settings in a way that won't require reinitialization
		OpSettings->CopySettingsAtRuntime(&OpSettingsFromProfile);
		break;
...
void FRetargetProfile::MergeWithOtherProfile(const FRetargetProfile& OtherProfile)
{
	...
	for (const FRetargetOpProfile& OtherOpProfile : OtherProfile.RetargetOpProfiles)
	{
		...
		{
			// found an op profile for the same op (same index and type) so overwrite it
	...
	MergeDeprecatedProperties(OtherProfile);
}
...
{
	const FIKRetargetOpBase* OpPtr = InOutOpStruct.GetPtr<FIKRetargetOpBase>();
	if (!OpPtr)
	{
		return false;
	}
	const FName OpName = OpPtr->GetName();
...
void FRetargetProfile::MergeDeprecatedProperties(const FRetargetProfile& OtherProfile)
{
	//
