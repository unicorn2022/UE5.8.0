...
void FIKRetargetRootMotionOpSettings::CopySettingsAtRuntime(const FIKRetargetOpSettingsBase* InSettingsToCopyFrom)
{
	...
	const TArray<FName> PropertiesToIgnore = {
		GET_MEMBER_NAME_CHECKED(FIKRetargetRootMotionOpSettings, SourceRoot),
		GET_MEMBER_NAME_CHECKED(FIKRetargetRootMotionOpSettings, TargetRoot),
		GET_MEMBER_NAME_CHECKED(FIKRetargetRootMotionOpSettings, TargetPelvis)};
...
void FIKRetargetRootMotionOp::OnAddedToStack(const UIKRetargeter* InRetargetAsset, const FIKRetargetOpBase* InParentOp)
{
	...
	// first try and get the root bone from the characterization data on the IK Rig assets
	if (const UIKRigDefinition* SourceIKRig = InRetargetAsset->GetIKRig(ERetargetSourceOrTarget::Source))
	{
		Settings.SourceRoot = SourceIKRig->GetRoot();
	}
	if (const UIKRigDefinition* TargetIKRig = InRetargetAsset->GetIKRig(ERetargetSourceOrTarget::Target))
	{
		Settings.TargetRoot = TargetIKRig->GetRoot();
		// get the target pelvis bone too
		Settings.TargetPelvis = TargetIKRig->GetPelvis();
	}

	// if source root bone was not registered, fallback to the root of the SOURCE skeleton
	if (Settings.SourceRoot.BoneName == NAME_None)
	{
	...
	// if target root bone was not registered, fallback to the root of the TARGET skeleton
	if (Settings.TargetRoot.BoneName == NAME_None)
	{
