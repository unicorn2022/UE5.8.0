...
void FIKRetargetIKChainsOpSettings::CopySettingsAtRuntime(const FIKRetargetOpSettingsBase* InSettingsToCopyFrom)
{
	...
	const TArray<FName> PropertiesToIgnore = {GET_MEMBER_NAME_CHECKED(FIKRetargetIKChainsOpSettings, ChainsToRetarget)};
	...
	IKRetargetOpUtils::CopyChainSettingsAtRuntime(NewSettings->ChainsToRetarget, ChainsToRetarget);
}
	...
	FIKRigLogger& InLog)
{
	...
	for (FRetargetIKChainSettings& ChainSettings : Settings.ChainsToRetarget)
	{
		...
		const FResolvedBoneChain* OutSourceBoneChain = nullptr;
		const FResolvedBoneChain* OutTargetBoneChain = nullptr;
		...
		const bool bChainInitialized = IKChainRetargeter.Initialize(*OutSourceBoneChain, *OutTargetBoneChain, ChainSettings, InLog);
		if (!bChainInitialized)
		{
			InLog.LogWarning( FText::Format(
			LOCTEXT("IKChainOpBadChain", "IK Chain Op: could not initialize a mapped retarget chain for IK, '{0}."),
...
{
	IKRetargetOpUtils::ResetChainSettingsToDefault(Settings.ChainsToRetarget, InChainName);
}

bool FIKRetargetIKChainsOp::AreChainSettingsAtDefault(const FName& InChainName)
{
	return IKRetargetOpUtils::AreChainSettingsAtDefault(Settings.ChainsToRetarget, InChainName);
}

uint8* FIKRetargetIKChainsOp::GetChainSettingsMemory(const FName InChainName)
{
	return IKRetargetOpUtils::GetChainSettingsMemory(Settings.ChainsToRetarget, InChainName);
}
