...
void FIKRetargetPinBoneOpSettings::CopySettingsAtRuntime(const FIKRetargetOpSettingsBase* InSettingsToCopyFrom)
{
	...
	const TArray<FName> PropertiesToIgnore = {GET_MEMBER_NAME_CHECKED(FIKRetargetPinBoneOpSettings, BonesToPin)};
	...
	TArray<FTransform>& OutTargetGlobalPose) const
{
	...
	case EPinBoneRotationMode::MaintainOffsetFromBoneToCopyFrom:
		{
			// get global retarget pose of from/to bones
			...
			const FQuat& SourceRefRot = FromRetargetPose[PinData.BoneToCopyFrom.BoneIndex].GetRotation();
			const FQuat& TargetRefRot = ToRetargetPose[PinData.BoneToCopyTo.BoneIndex].GetRotation();

			// get rotation delta of from bone
			const FQuat& SourceCurrentRot = PoseToCopyFrom[PinData.BoneToCopyFrom.BoneIndex].GetRotation();
			const FQuat SourceDelta = SourceCurrentRot * SourceRefRot.Inverse();

			// apply delta to the "to" retarget pose
			Result.SetRotation(SourceDelta * TargetRefRot);
