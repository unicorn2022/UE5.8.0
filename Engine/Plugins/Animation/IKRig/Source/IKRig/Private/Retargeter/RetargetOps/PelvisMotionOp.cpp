...
void FIKRetargetPelvisMotionOpSettings::CopySettingsAtRuntime(const FIKRetargetOpSettingsBase* InSettingsToCopyFrom)
{
	...
	const TArray<FName> PropertiesToIgnore = {
		GET_MEMBER_NAME_CHECKED(FIKRetargetPelvisMotionOpSettings, SourcePelvisBone),
		GET_MEMBER_NAME_CHECKED(FIKRetargetPelvisMotionOpSettings, TargetPelvisBone)};
...
{
	FTransform NewPelvisGlobal;
	RetargetPelvisTransform(InSourceGlobalPose, NewPelvisGlobal);
	
	// update global transforms below pelvis
	FTargetSkeleton& TargetSkeleton = Processor.GetTargetSkeleton();
	TargetSkeleton.SetGlobalTransformAndUpdateChildren(Target.BoneIndex, NewPelvisGlobal, TargetSkeleton.OutputGlobalPose);

#if WITH_EDITOR
	CurrentPelvisTransform = NewPelvisGlobal;
#endif
}
...
void FIKRetargetPelvisMotionOp::RetargetPelvisTransform(
	const TArray<FTransform>& SourceGlobalPose,
	FTransform& OutPelvisGlobalPose)
{
	// get source pelvis transform
	...
	FVector Position;
	{
		// generate pelvis motion scaled by the relative height ratio of the target to the source height
		// ie, if source is 2x taller than target, motion is scaled by 0.5
		const FVector NormalizedSourcePosition = Source.CurrentPosition * Source.InitialHeightInverse;
		const FVector PositionScaled = NormalizedSourcePosition * Target.InitialHeight;

		// generate pelvis motion as absolute delta
		const FVector AbsoluteDelta = Source.CurrentPosition - Source.InitialPosition;
		const FVector PositionAbsolute = Target.InitialPosition + AbsoluteDelta;

		// blend between scaled and absolute motion
		Position = FMath::Lerp(PositionScaled, PositionAbsolute, Settings.BlendToAbsoluteOffset);
		...
		if (!FMath::IsNearlyZero(Settings.FloorConstraintWeight))
		{
			...
			{
				// calculate the constrained height as a proportional target height relative to the source
				...
				// optionally apply ground proximity falloff to the floor constraint weight
				float FinalWeight = Settings.FloorConstraintWeight;
				if (Settings.bUseGroundFalloff)
				{
					// get how close we are to the ground (1.0 = Touching Floor, 0.0 = Standing)
					const float FalloffStartHeight = Target.InitialHeight * Settings.GroundFalloffHeightPercent;
					const float ProximityFalloff = 1.0f - FMath::Clamp(Position.Z, 0.0f, FalloffStartHeight) / FalloffStartHeight;
					FinalWeight *= ProximityFalloff;
				}
				
				Position.Z = FMath::Lerp(Position.Z, ConstrainedHeight, FinalWeight);
			}
		}

		// record the retargeted position before applying any modifications
		const FVector RetargetedPosition = Position;
