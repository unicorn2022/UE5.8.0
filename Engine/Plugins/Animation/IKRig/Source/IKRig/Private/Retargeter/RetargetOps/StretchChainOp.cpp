...
FChainStretcher::FChainStretcher(
	...
	const FResolvedBoneChain& InTargetBoneChain)
{
	...
	// lazy initialized to avoid wasting memory
	bIsInitialized = false;
}

void FChainStretcher::Initialize(const FRetargetSkeleton& InTargetSkeleton)
{
...
void FIKRetargetStretchChainOpSettings::CopySettingsAtRuntime(const FIKRetargetOpSettingsBase* InSettingsToCopyFrom)
{
	...
	IKRetargetOpUtils::CopyChainSettingsAtRuntime(NewSettings->ChainsToStretch, ChainsToStretch);
}
	...
	FIKRigLogger& InLog)
{
	...
	for (const FRetargetStretchChainSettings& ChainSettings : Settings.ChainsToStretch)
	{
		...
		// store a chain stretcher
		ChainStretchers.Emplace(&ChainSettings, *SourceBoneChain, *TargetBoneChain);
	}
...
{
	IKRetargetOpUtils::ResetChainSettingsToDefault(Settings.ChainsToStretch, InChainName);
}

bool FIKRetargetStretchChainOp::AreChainSettingsAtDefault(const FName& InChainName)
{
	return IKRetargetOpUtils::AreChainSettingsAtDefault(Settings.ChainsToStretch, InChainName);
}

uint8* FIKRetargetStretchChainOp::GetChainSettingsMemory(const FName InChainName)
{
	return IKRetargetOpUtils::GetChainSettingsMemory(Settings.ChainsToStretch, InChainName);
}
#endif
