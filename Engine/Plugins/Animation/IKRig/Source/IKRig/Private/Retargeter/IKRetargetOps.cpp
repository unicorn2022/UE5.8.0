...
void FIKRetargetOpSettingsBase::AssignSkeletonAssets(const USkeleton* InSourceSkeleton, const USkeleton* InTargetSkeleton)
{
	SourceSkeletonAsset = InSourceSkeleton;
	TargetSkeletonAsset = InTargetSkeleton;
}
...
FText FIKRetargetOpBase::GetWarningMessage() const
{
	if (bIsInitialized)
	{
		return IsEnabled() ? LOCTEXT("OpReadyAndOn", "Running.") : LOCTEXT("OpReadyAndOff", "Ready, but disabled.");
	}
	
	return LOCTEXT("OpNotReady", "Not initialized. See output log.");
}

void FOpExecutionTimer::Reset()
{
	TimeThisFrame = 0.0f;
	RunningAverageTime = 0.0f;
}

void FOpExecutionTimer::AddTimeThisFrame(const double InSeconds)
{
	TimeThisFrame += InSeconds;
}

void FOpExecutionTimer::StartNewFrame()
{
	constexpr double FrameWindow = 30.0;
	constexpr double Alpha = 2.0 / (FrameWindow + 1.0);
	RunningAverageTime += Alpha * (TimeThisFrame - RunningAverageTime);

	TimeThisFrame = 0.0f;
}

double FOpExecutionTimer::GetRunningAverage() const
{
	return RunningAverageTime;
}

void FIKRetargetOpBase::GetOverrideableProperties(TSharedPtr<FRetargetPropertyOverrideNode>& OutRootNode) const
{
	const UScriptStruct* SettingsStruct = GetSettingsType();
	const uint8* SettingsData = reinterpret_cast<const uint8*>(GetSettingsConst());
	if (!ensure(SettingsStruct && SettingsData))
	{
		return;
	}

	OutRootNode = MakeShared<FRetargetPropertyOverrideNode>();
	OutRootNode->NodeName = TEXT("Root");
	OutRootNode->AddPropertiesFromStruct(SettingsStruct, SettingsData, TEXT(""));
}
