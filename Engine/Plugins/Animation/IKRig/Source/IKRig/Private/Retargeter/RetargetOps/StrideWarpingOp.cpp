...
void FIKRetargetStrideWarpingOpSettings::CopySettingsAtRuntime(const FIKRetargetOpSettingsBase* InSettingsToCopyFrom)
{
	...
	const TArray<FName> PropertiesToIgnore = {GET_MEMBER_NAME_CHECKED(FIKRetargetStrideWarpingOpSettings, ChainSettings)};
	...
	// copy chains
	const FIKRetargetStrideWarpingOpSettings* NewSettings = reinterpret_cast<const FIKRetargetStrideWarpingOpSettings*>(InSettingsToCopyFrom);
	IKRetargetOpUtils::CopyChainSettingsAtRuntime(NewSettings->ChainSettings, ChainSettings);
}
	...
	FIKRigLogger& InLog)
{
	...
	for (const FRetargetStrideWarpChainSettings& ChainSettings : Settings.ChainSettings)
	{
		...
		// store goal to warp
		GoalsToWarp.Emplace(Goal->Name, TargetGlobalRetargetPose[GoalBoneIndex], ChainSettings);
	}
	...
	TArray<FTransform>& OutTargetGlobalPose)
{
	...
	for (FStrideWarpGoalData& GoalToWarp : GoalsToWarp)
	{
		...
		// accumulate a warped position to apply to the goal
		FVector WarpedPosition = Goal->Position;
		...
		WarpedPosition = GoalProjOnFwdPlane + ((WarpedPosition - GoalProjOnFwdPlane) * Settings.WarpForwards);
		...
		WarpedPosition += Side * Settings.SidewaysOffset * GoalSideMultiplier;

		// splay warping
		FVector SplayOrigin = CurrentBodyPosition;
		SplayOrigin.Z = WarpedPosition.Z;
		WarpedPosition = SplayOrigin + (WarpedPosition - SplayOrigin) * Settings.WarpSplay;

		// alpha blend the goal location towards the warped position
		const float Alpha = GoalToWarp.Settings->Alpha * Settings.Alpha;
		Goal->Position = FMath::Lerp(Goal->Position, WarpedPosition, Alpha);
...
{
	IKRetargetOpUtils::ResetChainSettingsToDefault(Settings.ChainSettings, InChainName);
}

bool FIKRetargetStrideWarpingOp::AreChainSettingsAtDefault(const FName& InChainName)
{
	return IKRetargetOpUtils::AreChainSettingsAtDefault(Settings.ChainSettings, InChainName);
}

uint8* FIKRetargetStrideWarpingOp::GetChainSettingsMemory(const FName InChainName)
{
	return IKRetargetOpUtils::GetChainSettingsMemory(Settings.ChainSettings, InChainName);
}
#endif
