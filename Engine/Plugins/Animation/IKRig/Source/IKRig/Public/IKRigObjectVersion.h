...
struct FIKRigObjectVersion
{
	...
	enum Type
	{
		...
		// the "Retarget IK Goals" op is deprecated and features are split into separate ops
		IKChainsOpSplitIntoSeparateOps,

		// IK Rig retarget definition now stores separate root bone and pelvis bone
		AddedRootBoneToRetargetDefinition,

		// the op settings now contain bEnabled instead of the ops themselves
		EnabledStateMovedFromOpToOpSettings,
