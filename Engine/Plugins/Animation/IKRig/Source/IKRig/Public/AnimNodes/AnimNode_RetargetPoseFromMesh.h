...
struct FAnimNode_RetargetPoseFromMesh : public FAnimNode_CustomProperty
{
	GENERATED_BODY()
	...
	// If true, will use the custom list of named override sets to apply. These are stored in the retarget asset.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, meta=(PinHiddenByDefault))
	bool bUseCustomOverrideSets = false;

	/** Apply one or more override sets stored in the retarget asset. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, meta=(EditCondition="bUseCustomOverrideSets", PinHiddenByDefault))
	...
	// END FAnimNode_CustomProperty interface
...
private:
	...
	// the runtime storage for variable values
	UPROPERTY(Transient)
	FInstancedPropertyBag RuntimeVariables;

	// a map of curves bound to property overrides (Name, float)
	UPROPERTY(Transient)
	TMap<FName, float> BoundCurveValues;
	int32 CachedOverrideVersion = INDEX_NONE;

	// pulls runtime values of variables out of anim instance and stores in RuntimeVariables
	void CopyInputPropertiesToVariables(const UObject* InSourceInstance);

	// fills BoundCurves from the anim instance
	void CopyBoundCurveValues(const UObject* InSourceInstance);
