...
USTRUCT()
struct FRetargetVariableContainer
{
	GENERATED_BODY()
...
public:
	...
	// Get a retarget op by name non-const
	// NOTE: be careful with this, general op-editing should be done through a controller
	UE_API FIKRetargetOpBase* GetRetargetOpByNameNonConst(const FName& InOpName);
	// Get index of a retarget op
	UE_API int32 GetIndexOfRetargetOp(const FIKRetargetOpBase* InOp) const;
	...
	// Get read-only access to all the override sets
	const TMap<FName, FRetargetOverrideSet>& GetOverrideSets() const { return OverrideSets; };
	// Get read-write access to all override sets (caution, all modifications should go through the controller)
	TMap<FName, FRetargetOverrideSet>& GetOverrideSetsWriteable() { return OverrideSets; };
	// Get all the override sets that are enabled
	UE_API const TArray<FName>& GetOverrideSetsToApply() const;
	// Get a set of overrides by name (may be null) 
	UE_API const FRetargetOverrideSet* GetOverrideSetByName(const FName& OverrideName) const;
	// get current version of the property overrides data (to compare against running processor instances)
	int32 GetOverrideVersion() const { return OverridesVersion; };

	// get read-write access to the variable bag (caution here, all editing should go through controller) 
	FRetargetVariableContainer& GetVariables() { return Variables; };
...
private:
	...
	UE_API void PostLoadSplitIKChainsOp();
	UE_API void PostLoadSplitOps();
#endif
...
private:
	...
	// whole setting struct profiles stored in this asset
	UPROPERTY()
	TMap<FName, FRetargetProfile> Profiles;
	UPROPERTY()
	FName CurrentProfile = NAME_None;

	// sparse property overrides to apply at runtime
	UPROPERTY()
	TMap<FName, FRetargetOverrideSet> OverrideSets;
	// the default set of override sets to be applied at runtime
	mutable TArray<FName> OverridesToApply;
	// updated anytime overrides are modified
	mutable int32 OverridesVersion = 0;
	
	// storage for all variables (used to dynamically adjust property overrides)
	UPROPERTY()
	FRetargetVariableContainer Variables;
