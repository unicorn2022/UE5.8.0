...
struct FIKRigInputSkeleton
{
	...
	UE_API FIKRigInputSkeleton(const USkeletalMesh* InSkeletalMesh);

	UE_API void Initialize(const USkeletalMesh* InSkeletalMesh);
...
struct FIKRigSkeleton
{
	...
	/** The skeletal mesh this was initialized with. */
	...
	UE_API void SetInputSkeleton(const USkeletalMesh* InSkeletalMesh, const TArray<FName>& InExcludedBones);
