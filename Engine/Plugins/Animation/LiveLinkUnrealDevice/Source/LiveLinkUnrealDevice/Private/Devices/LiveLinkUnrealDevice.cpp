...
{
	if (bSuppressSlateNameSend)
	{
		return;
	}

	// Authoritative editors own their take metadata; the Hub must not push
	// slate changes back to them.
	if (GetSettings()->bHasTakeMetadataAuthority)
	{
		return;
	}
...
{
	if (bSuppressTakeNumberSend)
	{
		return;
	}

	// Authoritative editors own their take metadata; the Hub must not push
	// take-number changes back to them (otherwise the Hub's post-stop
	// auto-increment double-bumps the editor's already-incremented take).
	if (GetSettings()->bHasTakeMetadataAuthority)
	{
		return;
	}
...
void ULiveLinkUnrealDevice::HandleMessageCatchall(const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	else if (MessageTypeInfo->IsChildOf(FLiveLinkTakeRecorderMessageBase::StaticStruct()))
	{
		...
		else if (MessageTypeInfo == FLiveLinkTakeRecorderCmd_SetSlateName::StaticStruct())
		{
			const FLiveLinkTakeRecorderCmd_SetSlateName* Cmd =
				static_cast<const FLiveLinkTakeRecorderCmd_SetSlateName*>(InContext->GetMessage());

			UE_LOGFMT(LogLiveLinkUnrealDevice, Log, "{DeviceName}: SetSlateName(\"{SlateName}\") from {RemoteAddress}",
				GetSettings()->DisplayName, Cmd->SlateName, InContext->GetSender().ToString());

			if (GetDeviceSettings<ULiveLinkUnrealDeviceSettings>()->bHasTakeMetadataAuthority)
			{
				ILiveLinkRecordingSession& Session = ILiveLinkRecordingSession::Get();
				const FString PrevSlateName = Session.GetSlateName();

				bSuppressSlateNameSend = true;
				Session.SetSlateName(Cmd->SlateName);
				bSuppressSlateNameSend = false;

				UE_LOGFMT(LogLiveLinkUnrealDevice, Log, "{DeviceName}: Has take metadata authority; "
					"PrevSlateName={PrevSlateName}, NewSlateName={NewSlateName}",
					GetSettings()->DisplayName, PrevSlateName, Cmd->SlateName);
			}
		}
		else if (MessageTypeInfo == FLiveLinkTakeRecorderCmd_SetTakeNumber::StaticStruct())
		{
			const FLiveLinkTakeRecorderCmd_SetTakeNumber* Cmd =
				static_cast<const FLiveLinkTakeRecorderCmd_SetTakeNumber*>(InContext->GetMessage());

			UE_LOGFMT(LogLiveLinkUnrealDevice, Log, "{DeviceName}: SetTakeNumber({TakeNumber}) from {RemoteAddress}",
				GetSettings()->DisplayName, Cmd->TakeNumber, InContext->GetSender().ToString());

			if (GetDeviceSettings<ULiveLinkUnrealDeviceSettings>()->bHasTakeMetadataAuthority)
			{
				ILiveLinkRecordingSession& Session = ILiveLinkRecordingSession::Get();
				const int32 PrevTakeNumber = Session.GetTakeNumber();

				bSuppressTakeNumberSend = true;
				Session.SetTakeNumber(Cmd->TakeNumber);
				bSuppressTakeNumberSend = false;

				UE_LOGFMT(LogLiveLinkUnrealDevice, Log, "{DeviceName}: Has take metadata authority; "
					"PrevTakeNumber={PrevTakeNumber}, NewTakeNumber={NewTakeNumber}",
					GetSettings()->DisplayName, PrevTakeNumber, Cmd->TakeNumber);
			}
		}
	}
}
