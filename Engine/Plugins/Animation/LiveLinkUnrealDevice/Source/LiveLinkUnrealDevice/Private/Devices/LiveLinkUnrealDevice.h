...
public:
	...
	{
		DisplayName = NSLOCTEXT("LiveLinkUnrealDevice", "DefaultDisplayName", "Unreal Editor").ToString();
	}
	...
	/** If true, slate/take changed events from this device will also cause Live Link Hub to update. 
	 * If true, this device will not accept updates to the slate/take sent from Live Link Hub.
	 */
	UPROPERTY(EditAnywhere, Category="Unreal Device", AdvancedDisplay, DisplayName="Has Slate Name & Take Number Authority")
	bool bHasTakeMetadataAuthority = true;
};
...
private:
	...
	/** Attempts to connect to the configured `ClientId`. */
	void Connect();
...
private:
	...
	/**
	 * Guards against feedback loops when receiving slate/take changes from UE.
	 * Set true while processing an incoming command so that the resulting
	 * ILiveLinkRecordingSession delegate doesn't re-send the same command back.
	 */
	bool bSuppressSlateNameSend = false;
	bool bSuppressTakeNumberSend = false;
};
