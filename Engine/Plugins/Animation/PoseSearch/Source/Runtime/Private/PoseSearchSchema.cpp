...
int8 UPoseSearchSchema::FindSchemaBoneIdx(const FName& BoneName, const UE::PoseSearch::FRole& Role, bool bDefaultWithRootBone) const
{
	...
	if (!RoledSkeleton)
	{
		UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::FindSchemaBoneIdx: couldn't find data for the requested Role '%ls' in UPoseSearchSchema '%ls'", *Role.ToString(), *GetNameSafe(this));
	...
	if (BoneName == NAME_None)
	{
		...
		{
			if (RoledSkeleton->BoneReferences.IsEmpty())
			{
				return InvalidSchemaBoneIdx;
			}

			return RootSchemaBoneIdx;
		}
		
		return TrajectorySchemaBoneIdx;
	}

	for (int32 SchemaBoneIdx = 0; SchemaBoneIdx < RoledSkeleton->BoneReferences.Num(); ++SchemaBoneIdx)
	{
		const FBoneReference& BoneReference = RoledSkeleton->BoneReferences[SchemaBoneIdx];
		if (BoneReference.BoneName == BoneName)
		{
			check(SchemaBoneIdx < 128);
...
int8 UPoseSearchSchema::AddBoneReference(const FBoneReference& BoneReference, const UE::PoseSearch::FRole& Role, bool bDefaultWithRootBone)
{
	...
	if (!RoledSkeleton)
	{
		UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::AddBoneReference: couldn't find data for the requested Role '%ls' in UPoseSearchSchema '%ls'", *Role.ToString(), *GetNameSafe(this));
		return InvalidSchemaBoneIdx;
	}
	...
	if (!Skeleton)
	{
		UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::AddBoneReference: couldn't find Skeleton with Role '%ls' in UPoseSearchSchema '%ls'", *Role.ToString(), *GetNameSafe(this));
		return InvalidSchemaBoneIdx;
	}
	...
	if (TempBoneReference.BoneName != NAME_None)
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::AddBoneReference: couldn't initialize FBoneReference '%ls' with Skeleton '%ls' with Role '%ls' in UPoseSearchSchema '%ls'",
...
int8 UPoseSearchSchema::AddCurveReference(const FName& CurveReference, const UE::PoseSearch::FRole& Role)
{
	...
	if (!RoledSkeleton)
	{
		UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::AddCurveReference: couldn't find data for the requested Role '%ls' in UPoseSearchSchema '%ls'", *Role.ToString(), *GetNameSafe(this));
	...
	if (!Skeleton)
	{
		UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::AddCurveReference: couldn't find Skeleton with Role '%ls' in UPoseSearchSchema '%ls'", *Role.ToString(), *GetNameSafe(this));
		return InvalidSchemaCurveIdx;
	}
...
void UPoseSearchSchema::Finalize()
{
	...
	for (int32 RoledSkeletonIndex = 0; RoledSkeletonIndex < Skeletons.Num(); ++RoledSkeletonIndex)
	{
		...
		for (int32 ComparisonIndex = RoledSkeletonIndex + 1; ComparisonIndex < Skeletons.Num(); ++ComparisonIndex)
		{
			...
			{
				UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::Finalize: couldn't Finalize '%ls' because of duplicate Role '%ls' in Skeletons", *GetNameSafe(this), *RoledSkeleton.Role.ToString());
		...
		{
			UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::Finalize: couldn't Finalize '%ls' because couldn't initialize root bone properly", *GetNameSafe(this));
	...
	for (const TObjectPtr<UPoseSearchFeatureChannel>& ChannelPtr : Channels)
	{
		if (ChannelPtr)
		{
			...
			if (!ChannelPtr->Finalize(this))
			{
				...
				UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::Finalize: couldn't Finalize '%ls' because of Channel '%ls'", *GetNameSafe(this), *Label);
				#else // WITH_EDITOR
				UE_LOGF(LogPoseSearch, Error, "UPoseSearchSchema::Finalize: couldn't Finalize '%ls' because of Channel '%ls'", *GetNameSafe(this), *GetNameSafe(ChannelPtr));
				#endif // WITH_EDITOR
