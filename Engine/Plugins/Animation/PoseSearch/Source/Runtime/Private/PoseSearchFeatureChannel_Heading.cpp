...
void UPoseSearchFeatureChannel_Heading::FindOrAddToSchema(UPoseSearchSchema* Schema, float SampleTimeOffset, const FName& BoneName, const UE::PoseSearch::FRole& Role, EHeadingAxis HeadingAxis, EPermutationTimeType PermutationTimeType, bool bDefaultWithRootBone)
{
	using namespace UE::PoseSearch;

	check(Schema);
	const int8 SchemaBoneIdx = Schema->FindSchemaBoneIdx(BoneName, Role, bDefaultWithRootBone);
	if (SchemaBoneIdx == InvalidSchemaBoneIdx)
	{
		UE_LOGF(LogPoseSearch, Error, "UPoseSearchFeatureChannel_Heading::FindOrAddToSchema: couldn't find SchemaBoneIdx for Bone '%ls' with Role '%ls' in UPoseSearchSchema '%ls'", *BoneName.ToString(), *Role.ToString(), *GetNameSafe(Schema));
		return;
	}

	if (!Schema->FindChannel([SchemaBoneIdx, SampleTimeOffset, Role, HeadingAxis, PermutationTimeType](const UPoseSearchFeatureChannel* Channel) -> const UPoseSearchFeatureChannel_Heading*
		{
	...
	{
		const TConstArrayView<FBoneReference> BoneReferences = Schema->GetBoneReferences(Role);

		UPoseSearchFeatureChannel_Heading* Heading = NewObject<UPoseSearchFeatureChannel_Heading>(Schema, NAME_None, RF_Transient);
		Heading->Bone.BoneName = BoneReferences.IsValidIndex(SchemaBoneIdx) ? BoneReferences[SchemaBoneIdx].BoneName : NAME_None;
		Heading->SampleRole = Role;
		Heading->OriginBone.BoneName = BoneReferences.IsValidIndex(RootSchemaBoneIdx) ? BoneReferences[RootSchemaBoneIdx].BoneName : NAME_None;
		Heading->OriginRole = Role;
		...
		Heading->HeadingAxis = HeadingAxis;
		Heading->PermutationTimeType = PermutationTimeType;
		Heading->bDefaultWithRootBone = false;
...
void UPoseSearchFeatureChannel_Heading::AddDependentChannels(UPoseSearchSchema* Schema) const
{
	if (Schema->bInjectAdditionalDebugChannels)
	{
		...
		UPoseSearchFeatureChannel_Position::FindOrAddToSchema(Schema, SampleTimeOffset, Bone.BoneName, SampleRole, SamplePermutationTimeType, bDefaultWithRootBone);
		
		// injecting 2 Heading channels to be able to reconstruct the Bone rotation
		...
		UPoseSearchFeatureChannel_Heading::FindOrAddToSchema(Schema, OriginTimeOffset, Bone.BoneName, OriginRole, EHeadingAxis::X, OriginPermutationTimeType, bDefaultWithRootBone);
		UPoseSearchFeatureChannel_Heading::FindOrAddToSchema(Schema, OriginTimeOffset, Bone.BoneName, OriginRole, EHeadingAxis::Y, OriginPermutationTimeType, bDefaultWithRootBone);
	}
}
...
FQuat UPoseSearchFeatureChannel_Heading::BP_GetWorldRotationFromContext_Implementation(FChooserEvaluationContext& Context) const
{
	using namespace UE::PoseSearch;
	...
	if (const UAnimInstance* AnimInstance = Cast<UAnimInstance>(GetAnimContext(&Context)))
	{
		...
		return BP_GetWorldRotation(AnimInstance);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	
	UE_LOGF(LogPoseSearch, Warning, "UPoseSearchFeatureChannel_Heading::BP_GetWorldRotationFromContext_Implementation - unsupported null UAnimInstance: WIP support for AnimNext!");
...
void UPoseSearchFeatureChannel_Heading::BuildQuery(UE::PoseSearch::FSearchContext& SearchContext) const
{
	...
	if (bUseBlueprintQueryOverride)
	{
		...
		const FQuat BoneRotationWorld = ensure(ChooserContext) ? BP_GetWorldRotationFromContext(*ChooserContext) : FQuat::Identity;
