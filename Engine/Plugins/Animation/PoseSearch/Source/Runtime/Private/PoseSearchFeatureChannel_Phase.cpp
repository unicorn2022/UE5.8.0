...
bool UPoseSearchFeatureChannel_Phase::IndexAsset(UE::PoseSearch::FAssetIndexer& Indexer) const
{
	...
	if (Indexer.GetNumIndexedPoses() <= 1)
	{
		...
		UE_LOGF(LogPoseSearch, Error, "UPoseSearchFeatureChannel_Phase::IndexAsset - Attempt to index animation asset with single pose is not supported. Asset name: %ls", *Indexer.GetAssetName());
		return false;
	}
