...
void UPoseSearchFeatureChannel_Velocity::AddDependentChannels(UPoseSearchSchema* Schema) const
{
	if (Schema->bInjectAdditionalDebugChannels)
	{
		...
		UPoseSearchFeatureChannel_Position::FindOrAddToSchema(Schema, SampleTimeOffset, Bone.BoneName, SampleRole, DependentChannelsPermutationTimeType, bDefaultWithRootBone);
	}
}

FVector UPoseSearchFeatureChannel_Velocity::BP_GetWorldVelocityFromContext_Implementation(FChooserEvaluationContext& Context) const
{
	using namespace UE::PoseSearch;
	...
	if (const UAnimInstance* AnimInstance = Cast<UAnimInstance>(GetAnimContext(&Context)))
	{
		...
		return BP_GetWorldVelocity(AnimInstance);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	
	UE_LOGF(LogPoseSearch, Warning, "UPoseSearchFeatureChannel_Velocity::BP_GetWorldVelocityFromContext_Implementation - unsupported null UAnimInstance: WIP support for AnimNext!");
...
void UPoseSearchFeatureChannel_Velocity::BuildQuery(UE::PoseSearch::FSearchContext& SearchContext) const
{
	...
	if (bUseBlueprintQueryOverride)
	{
		...
		const FVector LinearVelocityWorld = ensure(ChooserContext) ? BP_GetWorldVelocityFromContext(*ChooserContext) : FVector::ZeroVector;
...
void UPoseSearchFeatureChannel_Velocity::DebugDraw(const UE::PoseSearch::FDebugDrawParams& DrawParams, TConstArrayView<float> PoseVector) const
{
	...
	if (DrawParams.IsAnyWeightRelevant(this))
	{
		...
		const float ArrowScale = 0.2f * LinearVelocityScale;
		...
		const EPermutationTimeType OriginPermutationTimeType = SamplePermutationTimeType;

		const FVector FeaturesVector = FFeatureVectorHelper::DecodeVector(PoseVector, ChannelDataOffset, ComponentStripping);
		// velocity is encoded in OriginRole's root-bone space at OriginTime; rotate it back to world using that frame
		const FQuat OriginRotation = DrawParams.ExtractRotation(PoseVector, OriginTimeOffset, RootSchemaBoneIdx, OriginRole, OriginPermutationTimeType, SamplingAttributeId, PermutationOriginTimeOffset);
		const FVector LinearVelocity = OriginRotation.RotateVector(FeaturesVector);
		...
		const FVector EndPos = BonePos + LinearVelocity * LinearVelocityScale;

		DrawParams.DrawLine(BonePos, EndPos, Color);

		const FVector SideVector = LinearVelocity.Cross(OriginRotation.RotateVector(FVector::UpVector));
		DrawParams.DrawLine(EndPos, EndPos - (LinearVelocity - SideVector) * ArrowScale, Color);
		DrawParams.DrawLine(EndPos, EndPos - (LinearVelocity + SideVector) * ArrowScale, Color);
	}
}
