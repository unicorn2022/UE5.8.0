...
const UObject* GetAnimContext(const FChooserEvaluationContext* Context)
{
	check(Context);
	return Context->GetFirstObjectParam();
}
...
static FAnimInstanceProxy* GetAnimInstanceProxy(TConstArrayView<FChooserEvaluationContext*> Contexts)
{
	if (!Contexts.IsEmpty())
	{
		return UAnimInstanceProxyProvider::GetAnimInstanceProxy(Cast<UAnimInstance>(GetAnimContext(Contexts[0])));
	}
	return nullptr;
}

static const USkinnedMeshComponent* GetMesh(TConstArrayView<FChooserEvaluationContext*> Contexts, int32 RoleIndex = 0)
{
	return Cast<USkinnedMeshComponent>(GetAnimContext(Contexts[RoleIndex]));
}
...
//////////////////////////////////////////////////////////////////////////
// FDebugDrawParams
FDebugDrawParams::FDebugDrawParams(TConstArrayView<FChooserEvaluationContext*> InAnimContexts, TConstArrayView<const IPoseHistory*> InPoseHistories, const FRoleToIndex& InRoleToIndex, const UPoseSearchDatabase* InDatabase, EDrawContext InDrawContext)
: FDebugDrawParams()
{
	Init(InAnimContexts, InPoseHistories, InRoleToIndex, InDatabase, InDrawContext);
}
...
void FDebugDrawParams::Init(TConstArrayView<FChooserEvaluationContext*> InAnimContexts, TConstArrayView<const IPoseHistory*> InPoseHistories, const FRoleToIndex& InRoleToIndex, const UPoseSearchDatabase* InDatabase, EDrawContext InDrawContext)
{
...
void FDebugDrawParams::DrawWedge(const FVector& Origin, const FVector& Direction, float InnerRadius, float OuterRadius, float WidthInDegrees, int32 Segments, const FColor& Color, float Thickness) const
{
	FVector NormalizedDirection = Direction;
	if (Color.A > 0 && Segments > 1 && NormalizedDirection.Normalize())
	{
		float AngleDeg = -WidthInDegrees * 0.5f;
		const float AngleDegIncrement = WidthInDegrees / (Segments - 1);
...
: FSearchContext(InDesiredPermutationTimeOffset, InPoseJumpThresholdTime, InEventToSearch)
{
...
#if !NO_LOGGING
	if (!bSuccess && !PoseHistory->IsEmpty())
	{
		UE_LOGF(LogPoseSearch, Warning, "FSearchContext::GetSampleCurveValueInternal - Couldn't find Curve %ls in the input IPoseHistory requested by %ls. Consider adding it to the Pose History!", *CurveName.ToString(), *Schema->GetName());
	}
#endif // !NO_LOGGING
...
{
	if (PoseHistories.Num() > 0)
	{
		const IPoseHistory* PoseHistory = PoseHistories[0]; 
		check(PoseHistory);
	...
	else // if (BoneIndexType != RootBoneIndexType)
	{
		...
		if (!PoseHistory->GetTransformAtTime(SampleTime, LocalBoneTransform, Skeleton, BoneIndexType, RootBoneIndexType))
		{
			if (Skeleton)
			{
				...
				{
					UE_LOGF(LogPoseSearch, Warning, "FSearchContext::GetWorldBoneTransformAtTime - Couldn't find BoneIndexType %d (%ls) for Skeleton %ls in the input IPoseHistory requested by %ls. Consider adding it to the Pose History!", BoneIndexType, *Skeleton->GetReferenceSkeleton().GetBoneName(BoneIndexType).ToString(), *Skeleton->GetName(), *Schema->GetName());
				}
			}
			else
			{
				UE_LOGF(LogPoseSearch, Warning, "FSearchContext::GetWorldBoneTransformAtTime - Schema '%ls' Skeleton is not properly set", *Schema->GetName());
			}
		}
...
void FSearchContext::Track(const UPoseSearchDatabase* Database, int32 PoseIdx, EPoseCandidateFlags PoseCandidateFlags, TConstArrayView<float> DynamicWeightsSqrt, float ContinuingPoseCostAddend, float ContinuingInteractionCostAddend, float ContinuingContextInteractionCostAddend)
{
	...
	if (PoseIdx < SearchIndex.GetNumPoses())
	{
		...
		const FPoseSearchCost PoseCost(DissimilarityCost, SearchIndex.PoseMetadata[PoseIdx].GetCostAddend(), ContinuingPoseCostAddend, ContinuingInteractionCostAddend, ContinuingContextInteractionCostAddend);
