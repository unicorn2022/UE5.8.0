...
static bool GVarAnimPoseHistoryDebugDrawBeforeCollection = false;
static FAutoConsoleVariableRef CVarAnimPoseHistoryDebugDrawBeforeCollection(TEXT("a.AnimNode.PoseHistory.DebugDrawBeforeCollection"), GVarAnimPoseHistoryDebugDrawBeforeCollection, TEXT("Debug Draw Pose History before collection"));
...
void FAnimNode_PoseSearchHistoryCollector_Base::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	...
	if (ComponentSpacePoseProvider.GetSkeletonAsset())
	{
		PoseHistoryPtr->EvaluateComponentSpace_AnyThread(0.f, ComponentSpacePoseProvider, bStoreScales,
			...
			GetRequiredBones(Context.AnimInstanceProxy), FBlendedCurve(), CollectedCurves
#if WITH_EDITORONLY_DATA
			, GetContextTransform(AnimContext, false)
...
TArray<FBoneIndexType> FAnimNode_PoseSearchHistoryCollector_Base::GetRequiredBones(const FAnimInstanceProxy* AnimInstanceProxy) const
{
	...
	return GetRequiredBones(AnimInstanceProxy->GetSkelMeshComponent());
}

TArray<FBoneIndexType> FAnimNode_PoseSearchHistoryCollector_Base::GetRequiredBones(const USkeletalMeshComponent* SkeletalMeshComponent) const
{
	TArray<FBoneIndexType> RequiredBones;
	if (!CollectedBones.IsEmpty() && SkeletalMeshComponent)
	{
...
void FAnimNode_PoseSearchHistoryCollector::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	const UObject* AnimContext = Output.AnimInstanceProxy->GetAnimInstanceObject();
	check(AnimContext);
	CheckInteractionThreadSafety(AnimContext);
	...
	if (FPoseHistory* PoseHistory = GetPoseHistoryPtr())
	{
#if ENABLE_DRAW_DEBUG && ENABLE_ANIM_DEBUG
		if (GVarAnimPoseHistoryDebugDrawBeforeCollection)
		{
			FColor Color;
#if WITH_EDITORONLY_DATA
		...
		PoseHistory->EvaluateComponentSpace_AnyThread(Output.AnimInstanceProxy->GetDeltaSeconds(), ComponentSpacePoseProvider, bStoreScales,
		...
		{
			PoseHistoryAttribute->PoseHistory = PoseHistory;
