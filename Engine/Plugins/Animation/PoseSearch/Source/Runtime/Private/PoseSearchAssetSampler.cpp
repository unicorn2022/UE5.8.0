...
static FTransform ExtractRootTransformInternal(const UAnimMontage* AnimMontage, float StartTime, float EndTime, bool bEnforceCompressedDataSampling)
{
	...
	{
		UE_LOGF(LogPoseSearch, Error, "ExtractRootTransformInternal: so far we support only montages with one SlotAnimTracks. %ls has %d", *AnimMontage->GetName(), AnimMontage->SlotAnimTracks.Num());
...
void FAnimationAssetSampler::Init(const UAnimationAsset* InAnimationAsset, const FTransform& InRootTransformOrigin, const FVector& InBlendParameters, int32 InRootTransformSamplingRate, bool bPreProcessRootTransform, bool bInEnforceCompressedDataSampling)
{
	...
	{		
		if (!AnimationAssetPtr->GetPackage()->HasAnyPackageFlags(PKG_Cooked))
		{
			...
			if (bEnforceCompressedDataSampling)
			{
				ensureAlwaysMsgf(AnimSequence->HasCompressedDataForPlatform(TargetPlatform), TEXT("AnimationAssetSampler initialized to forcefully sample compressed animation data, but none is currently available for the target-platform"));
			}
		}
	}
#endif
...
float FAnimationAssetSampler::WrapOrClampTime(float Time, float PlayLength, bool bLoopable)
{
	...
	if (bLoopable)
	{
		const float Result = FMath::Fmod(Time, PlayLength);
		return (Result < 0.f) ? (Result + PlayLength) : Result;
	}
		
	return FMath::Clamp(Time, 0.f, PlayLength);
}
...
float FAnimationAssetSampler::ToNormalizedTime(float RealTime) const
{
	...
	if (CachedPlayLength > UE_KINDA_SMALL_NUMBER && Cast<UBlendSpace>(AnimationAssetPtr.Get()))
	{
		...
		UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler::ToNormalizedTime: requested RealTime %f is greater than CachedPlayLength %f for UBlendSpace %ls!", RealTime, CachedPlayLength, *AnimationAssetPtr->GetName());
...
FTransform FAnimationAssetSampler::GetTotalRootTransform() const
{
	...
	if (const UAnimMontage* AnimMontage = Cast<UAnimMontage>(AnimationAssetPtr.Get()))
	{
		...
		if (AnimMontage->SlotAnimTracks.Num() != 1)
		{
			UE_LOGF(LogPoseSearch, Error, "FAssetSamplerBase::GetTotalRootTransform: so far we support only montages with one SlotAnimTracks. %ls has %d", *AnimMontage->GetName(), AnimMontage->SlotAnimTracks.Num());
			return FTransform::Identity;
		}
...
void FAnimationAssetSampler::ExtractPose(const FAnimExtractContext& ExtractionCtx, FAnimationPoseData& OutAnimPoseData) const
{
	...
	if (const UBlendSpace* BlendSpace = Cast<UBlendSpace>(AnimationAssetPtr.Get()))
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FAnimMontageSampler::ExtractPose: UBlendSpace %ls couldn't return a valid pose for BlendParameters (%.2f, %.2f, %.2f)", *BlendSpace->GetName(), BlendParameters.X, BlendParameters.Y, BlendParameters.Z);
	...
	else if (const UAnimMontage* AnimMontage = Cast<UAnimMontage>(AnimationAssetPtr.Get()))
	{
		...
		if (AnimMontage->SlotAnimTracks.Num() != 1)
		{
			UE_LOGF(LogPoseSearch, Error, "FAnimMontageSampler::ExtractPose: so far we support only montages with one SlotAnimTracks. %ls has %d", *AnimMontage->GetName(), AnimMontage->SlotAnimTracks.Num());
			OutAnimPoseData.GetPose().ResetToRefPose();
		}
		else
		{
...
void FAnimationAssetSampler::ExtractPose(float Time, FCompactPose& OutPose) const
{
	...
	const bool bLoopable = IsLoopable();
	const float WrappedOrClampedTime = WrapOrClampTime(Time, GetPlayLength(), bLoopable);

	FDeltaTimeRecord DeltaTimeRecord;
	DeltaTimeRecord.Set(WrappedOrClampedTime, 0.f);
	FAnimExtractContext ExtractionCtx(double(WrappedOrClampedTime), false, DeltaTimeRecord, bLoopable);
...
void FAnimationAssetSampler::ExtractAnimNotifyContext(float Time, FAnimNotifyContext& PreAllocatedNotifyContext) const
{
	...
	if (const UBlendSpace* BlendSpace = Cast<UBlendSpace>(AnimationAssetPtr.Get()))
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler::ExtractAnimNotifyContext: Unsupported BlendSpace NotifyTriggerMode for '%ls'", *BlendSpace->GetName());
		}
	}
	else if (const UAnimSequenceBase* SequenceBase = Cast<UAnimSequenceBase>(AnimationAssetPtr.Get()))
	{
...
{
	ExtractAnimNotifyContext(Time, PreAllocatedNotifyContext);
...
void FAnimationAssetSampler::ExtractAnimNotifyEvents(float Time, FAnimNotifyContext& PreAllocatedNotifyContext, const TFunction<bool(const FAnimNotifyEvent*)>& ProcessAnimNotifyEvent) const
{
	ExtractAnimNotifyContext(Time, PreAllocatedNotifyContext);
	...
	for (const FAnimNotifyEventReference& EventReference : PreAllocatedNotifyContext.ActiveNotifies)
	{
		if (const FAnimNotifyEvent* NotifyEvent = EventReference.GetNotify())
		{
			if (!ProcessAnimNotifyEvent(NotifyEvent))
			{
				break;
			}
		}
...
TConstArrayView<FAnimNotifyEvent> FAnimationAssetSampler::GetAllAnimNotifyEvents() const
{
	if (const UBlendSpace* BlendSpace = Cast<UBlendSpace>(AnimationAssetPtr.Get()))
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler::GetAllAnimNotifyEvents: Unsupported BlendSpace NotifyTriggerMode for '%ls'", *BlendSpace->GetName());
		}
	}
	else if (const UAnimSequenceBase* SequenceBase = Cast<UAnimSequenceBase>(AnimationAssetPtr.Get()))
	{
