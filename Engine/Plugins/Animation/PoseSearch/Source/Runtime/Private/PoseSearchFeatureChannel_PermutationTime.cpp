...
void UPoseSearchFeatureChannel_PermutationTime::FindOrAddToSchema(UPoseSearchSchema* Schema)
{
	if (Schema->FindFirstChannelOfType<UPoseSearchFeatureChannel_PermutationTime>() == nullptr)
	{
		...
		UE_LOGF(LogPoseSearch, Warning, "required UPoseSearchFeatureChannel_PermutationTime has been added by the Schema '%ls' with Weight of zero. Explicitly add one of this channel to the Schema to be able to tune its weight", *Schema->GetName());
	}
}
