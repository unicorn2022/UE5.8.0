	...
	, ValidatingIsland(nullptr)
{
	...
	if (!MainActor)
	{
		UE_LOGF(LogPoseSearch, Error, "FInteractionValidator invalid MainActor! How did he die? Did you rebuild the animation blue print while PIE was running?");
	...
	if (Actor == MainActor)
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "Non thread safe call! Why is there any other actor running while we schedule the MainActor (%ls)? %d", *MainActor->GetName(), PreviousCounter);
			ValidatingIsland->LogTickDependencies();
		}
	}
	else
	{
		...
		if (PreviousCounter < 0)
		{
			UE_LOGF(LogPoseSearch, Error, "Non thread safe call! The MainActor (%ls) is running! Nobody else (%ls) in the same island should be running! %d", *MainActor->GetName(), *Actor->GetName(),  PreviousCounter);
			ValidatingIsland->LogTickDependencies();
		}
	}
}
	...
	, ValidatingIsland(InValidatingIsland)
{
	...
	if (PreviousCounter != 0)
	{
		UE_LOGF(LogPoseSearch, Error, "Non thread safe call! TickFunctions running concurrently? %d", PreviousCounter);
		ValidatingIsland->LogTickDependencies();
	}
}
...
FInteractionValidator::~FInteractionValidator()
{
	...
	{
		UE_LOGF(LogPoseSearch, Error, "FInteractionValidator why did the InteractionIsland changed?");
		return;
	}
	...
	if (!MainActor)
	{
		UE_LOGF(LogPoseSearch, Error, "FInteractionValidator invalid MainActor! How did he died? Did you rebuild the animation blue print while PIE was running?");
		return;
	}
	...
	if (Actor == MainActor)
	{
		...
		if (PreviousCounter >= 0)
		{
			UE_LOGF(LogPoseSearch, Error, "Non thread safe call! Why was there some other actor running while we ended the scheduling the MainActor (%ls)? %d", *MainActor->GetName(), PreviousCounter);
		}
	}
	else
	{
