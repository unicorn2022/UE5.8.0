...
static bool GVarMotionMatchTestInvalidateCache = false;
static FAutoConsoleVariableRef CVarMotionMatchTestInvalidateCache(TEXT("a.MotionMatch.Test.InvalidateCache"), GVarMotionMatchTestInvalidateCache, TEXT("Invalidate DDC cache on a frame basis (to stress test DDC cancellation while tasks are flying if !WaitForTaskCompletion)"));

static bool GVarMotionMatchTestWaitForTaskCompletion = false;
static FAutoConsoleVariableRef CVarMotionMatchTestWaitForTaskCompletion(TEXT("a.MotionMatch.Test.WaitForTaskCompletion"), GVarMotionMatchTestWaitForTaskCompletion, TEXT("Wait synchronously for DDC tasks to complete"));

static bool GVarMotionMatchTestForceIndexing = false;
static FAutoConsoleVariableRef CVarMotionMatchTestForceIndexing(TEXT("a.MotionMatch.Test.ForceIndexing"), GVarMotionMatchTestForceIndexing, TEXT("we'll force the database re-indexing and compare the result SearchIndex with the one retrieved via DDC"));

static bool GVarMotionMatchTestDeterminismKDTreeConstruct = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismKDTreeConstruct(TEXT("a.MotionMatch.Test.Determinism.KDTreeConstruct"), GVarMotionMatchTestDeterminismKDTreeConstruct, TEXT("test KDTree Construct determinism"));

static bool GVarMotionMatchTestKDTreeConstruct = false;
static FAutoConsoleVariableRef CVarMotionMatchTestKDTreeConstruct(TEXT("a.MotionMatch.Test.KDTreeConstruct"), GVarMotionMatchTestKDTreeConstruct, TEXT("validating the kdtree construction"));

static bool GVarMotionMatchTestDeterminismVPTreeConstruct = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismVPTreeConstruct(TEXT("a.MotionMatch.Test.Determinism.VPTreeConstruct"), GVarMotionMatchTestDeterminismVPTreeConstruct, TEXT("test VPTree Construct determinism"));

static bool GVarMotionMatchTestVPTreeConstruct = false;
static FAutoConsoleVariableRef CVarMotionMatchTestVPTreeConstruct(TEXT("a.MotionMatch.Test.VPTreeConstruct"), GVarMotionMatchTestVPTreeConstruct, TEXT("validating the vptree construction"));

static bool GVarMotionMatchTestDeterminismIndexDatabase = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismIndexDatabase(TEXT("a.MotionMatch.Test.Determinism.IndexDatabase"), GVarMotionMatchTestDeterminismIndexDatabase, TEXT("test IndexDatabase determinism"));

static bool GVarMotionMatchTestDeterminismPruneDuplicateValues = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismPruneDuplicateValues(TEXT("a.MotionMatch.Test.Determinism.PruneDuplicateValues"), GVarMotionMatchTestDeterminismPruneDuplicateValues, TEXT("test PruneDuplicateValues determinism"));

static bool GVarMotionMatchTestDeterminismPruneDuplicatePCAValues = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismPruneDuplicatePCAValues(TEXT("a.MotionMatch.Test.Determinism.PruneDuplicatePCAValues"), GVarMotionMatchTestDeterminismPruneDuplicatePCAValues, TEXT("test PruneDuplicatePCAValues determinism"));

static bool GVarMotionMatchTestDDCConsistency = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDDCConsistency(TEXT("a.MotionMatch.Test.DDCConsistency"), GVarMotionMatchTestDDCConsistency, TEXT("validating the data we gave to DDC is stored correctly"));

static bool GVarMotionMatchTestDeterminismSynchronizeWithExternalDependencies = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismSynchronizeWithExternalDependencies(TEXT("a.MotionMatch.Test.Determinism.SynchronizeWithExternalDependencies"), GVarMotionMatchTestDeterminismSynchronizeWithExternalDependencies, TEXT("validating SynchronizeWithExternalDependencies doesn't alter the database AnimationAssets order"));

static bool GVarMotionMatchTestDeterminismAssetSampler = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismAssetSampler(TEXT("a.MotionMatch.Test.Determinism.AssetSampler"), GVarMotionMatchTestDeterminismAssetSampler, TEXT("test FAnimationAssetSampler determinism"));

static bool GVarMotionMatchTestDeterminismAssetSamplerFromPreviousExecution = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismAssetSamplerFromPreviousExecution(TEXT("a.MotionMatch.Test.Determinism.AssetSamplerFromPreviousExecution"), GVarMotionMatchTestDeterminismAssetSamplerFromPreviousExecution, TEXT("test FAnimationAssetSampler determinism across multiple editor executions. It'll store some bin files in \\Engine\\TestAssetSamplerDeterminism"));

static bool GVarMotionMatchTestDeterminismDDCKey = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismDDCKey(TEXT("a.MotionMatch.Test.Determinism.DDCKey"), GVarMotionMatchTestDeterminismDDCKey, TEXT("test DDC key generation determinism"));

static int32 GVarMotionMatchTestNumIterations = 10;
static FAutoConsoleVariableRef CVarMotionMatchTestNumIterations(TEXT("a.MotionMatch.Test.NumIterations"), GVarMotionMatchTestNumIterations, TEXT("Test Motion Matching Num Iterations"));

static bool GVarMotionMatchTestDeterminismPartialKeyHashesSaveFile = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismPartialKeyHashesSaveFile(TEXT("a.MotionMatch.Test.Determinism.PartialKeyHashesSaveFile"), GVarMotionMatchTestDeterminismPartialKeyHashesSaveFile, TEXT("save file \\Engine\\TestPartialKeyHashesDeterminism\\PartialKeyHashes.bin for testing PartialKeyHashes determinism across multiple editor executions"));

static bool GVarMotionMatchTestDeterminismPartialKeyHashes = false;
static FAutoConsoleVariableRef CVarMotionMatchTestDeterminismPartialKeyHashes(TEXT("a.MotionMatch.Test.Determinism.PartialKeyHashes"), GVarMotionMatchTestDeterminismPartialKeyHashes, TEXT("test PartialKeyHashes determinism across multiple editor executions from stored file \\Engine\\TestPartialKeyHashesDeterminism\\PartialKeyHashes.bin"));
...
static bool GVarMotionMatchUseRawAnimationData = false;
static FAutoConsoleVariableRef CVarMotionMatchUseRawAnimationData(TEXT("a.MotionMatch.UseRawAnimationData"), GVarMotionMatchUseRawAnimationData, TEXT("Use Raw Animation Data"), FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable)
	{
...
#if ENABLE_ANIM_DEBUG
struct FSerializablePartialKeyHashes
{
	FSerializablePartialKeyHashes() = default;

	explicit FSerializablePartialKeyHashes(const FPartialKeyHashes& PartialKeyHashes)
	{
		Entries.Reserve(PartialKeyHashes.GetEntries().Num());
		for (const FPartialKeyHashes::FPair& Pair : PartialKeyHashes.GetEntries())
		{
			if (UObject* Object = Pair.Key.ResolveObjectPtr())
			{
				FString ObjectName = Object->GetFullName();
				if (Entries.Find(ObjectName))
				{
					UE_LOGF(LogPoseSearch, Error, "FSerializablePartialKeyHashes duplicate asset name '%ls'", *ObjectName);
				}
				else
				{
					FEntry& Entry = Entries.Add(ObjectName);

					Entry.Hash = Pair.Value.Hash;
					Entry.Dependencies.Reserve(Pair.Value.Dependencies.Num());

					for (const FObjectKey& DependencyKey : Pair.Value.Dependencies)
					{
						if (UObject* Dependency = DependencyKey.ResolveObjectPtr())
						{
							FString DependencyName = Dependency->GetFullName();
							Entry.Dependencies.Add(DependencyName);
						}
						else
						{
							Entry.Dependencies.Add("");
						}
					}
				}
			}
	...
	// checking if the intersection of entries between this and Other are consistent
	bool LogDifferencesAgains(const FSerializablePartialKeyHashes& Other) const
	{
		bool bSuccess = true;
		for (const TPair<FString, FEntry>& Pair : Entries)
		{
			if (const FEntry* OtherEntry = Other.Entries.Find(Pair.Key))
			{
				if (Pair.Value.Hash != OtherEntry->Hash)
				{
					UE_LOGF(LogPoseSearch, Error, "FSerializablePartialKeyHashes hash mismatch for asset '%ls'", *Pair.Key);
					bSuccess = false;
				}

				if (Pair.Value.Dependencies != OtherEntry->Dependencies)
				{
					UE_LOGF(LogPoseSearch, Warning, "FSerializablePartialKeyHashes Dependencies mismatch for asset '%ls'", *Pair.Key);
					bSuccess = false;
				}
			}
		}
	...
	friend FArchive& operator<<(FArchive& Ar, FSerializablePartialKeyHashes& SerializablePartialKeyHashes)
	{
		Ar << SerializablePartialKeyHashes.Entries;
		return Ar;
	}
...
static bool FindValidRanges(const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset, const FVector& BlendParameters, const FFloatInterval& ExcludeFromDatabaseParameters, FValidRanges& ValidRanges)
{
	check(DatabaseAsset);
	...
	bool bAnyExclusionNotifyStateFound = false;
	bool bAnySamplingRangeNotifyStateFound = false;
		...
		if (UAnimSequenceBase* SequenceBase = Cast<UAnimSequenceBase>(DatabaseAsset->GetAnimationAssetForRole(Role)))
		{
			...
			for (const FAnimNotifyEventReference& EventReference : NotifyContext.ActiveNotifies)
			{
				if (const FAnimNotifyEvent* NotifyEvent = EventReference.GetNotify())
				{
					...
					{
						if (bAnySamplingRangeNotifyStateFound)
						{
							UE_LOGF(LogPoseSearch, Error, "FindValidRanges conflicting UAnimNotifyState_PoseSearchExcludeFromDatabase and UAnimNotifyState_PoseSearchSamplingRange found in '%ls'.", *DatabaseAsset->GetAnimationAsset()->GetName());
							ValidRanges.Reset();
							return false;
						}

						bAnyExclusionNotifyStateFound = true;
						const FFloatRange ExclusionRange = FFloatRange::Inclusive(NotifyEvent->GetTime(), NotifyEvent->GetTime() + NotifyEvent->GetDuration());
					...
					else if (const UAnimNotifyState_PoseSearchSamplingRange* SamplingRangeNotifyState = Cast<const UAnimNotifyState_PoseSearchSamplingRange>(NotifyEvent->NotifyStateClass))
					{
						if (bAnyExclusionNotifyStateFound)
						{
							UE_LOGF(LogPoseSearch, Error, "FindValidRanges conflicting UAnimNotifyState_PoseSearchExcludeFromDatabase and UAnimNotifyState_PoseSearchSamplingRange found in '%ls'.", *DatabaseAsset->GetAnimationAsset()->GetName());
							ValidRanges.Reset();
							return false;
						...
						FFloatRange InclusionRange = FFloatRange::Inclusive(NotifyEvent->GetTime(), NotifyEvent->GetTime() + NotifyEvent->GetDuration());
						if (!bAnySamplingRangeNotifyStateFound)
						{
							ValidRanges.Reset();
							ValidRanges.Add(InclusionRange);
							bAnySamplingRangeNotifyStateFound = true;
						}
						else
						{
							for (int32 RangeIdx = ValidRanges.Num() - 1; RangeIdx >= 0; --RangeIdx)
							{
								const FFloatRange EvaluatedRange = ValidRanges[RangeIdx];
								if (InclusionRange.Contiguous(EvaluatedRange))
								{
									ValidRanges.RemoveAt(RangeIdx);
									const TArray<FFloatRange> Union = FFloatRange::Union(InclusionRange, EvaluatedRange);
									if (ensure(Union.Num() == 1))
									{
										InclusionRange = Union[0];
									}
								}
							}
...
static bool IsSchemaCompatible(const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset, const UPoseSearchSchema* Schema)
{
	const int32 DatabaseAssetNumRoles = DatabaseAsset->GetNumRoles();
	if (DatabaseAssetNumRoles != Schema->GetRoledSkeletons().Num())
	{
		return false;
	}

	for (int32 DatabaseAssetRoleIndex = 0; DatabaseAssetRoleIndex < DatabaseAssetNumRoles; ++DatabaseAssetRoleIndex)
	{
		const FRole DatabaseAssetRole = DatabaseAsset->GetRole(DatabaseAssetRoleIndex);
		if (!Schema->GetRoledSkeleton(DatabaseAssetRole))
		{
			return false;
		}
	}
	...
	for (int32 AnimationAssetIndex = 0; AnimationAssetIndex < DatabaseToLookForAssets->GetNumAnimationAssets(); ++AnimationAssetIndex)
	{
		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = DatabaseToLookForAssets->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
			...
			for (int32 RoleIndex = 0; RoleIndex < NumRoles; ++RoleIndex)
			{
				...
				if (bIsAlreadyInSet)
				{
					UE_LOGF(LogPoseSearch, Error, "DatabaseAsset '%ls' contains duplicate Role '%ls'", *DatabaseAsset->GetAnimationAsset()->GetName(), *Role.ToString());
					bAnyErrors = true;
				}
			}
			...
			for (const FPoseSearchRoledSkeleton& RoledSkeleton : Schema->GetRoledSkeletons())
			{
				...
				{
					UE_LOGF(LogPoseSearch, Error, "DatabaseAsset '%ls' doesn't support Role '%ls' required by Schema '%ls' Skeletons", *DatabaseAsset->GetAnimationAsset()->GetName(), *RoledSkeleton.Role.ToString(), *Schema->GetName());
			...
			DatabaseAsset->IterateOverSamplingParameter(
				[DatabaseAsset, ExcludeFromDatabaseParameters, &ValidRanges, Schema, bAddUnmirrored, bAddMirrored, 
				bIsLooping, bIsBlendSpace, bDisableReselection, AnimationAssetIndex, &TotalPoses, &SearchIndex, &bAnyErrors]
				(const FVector& BlendParameters)
				{
					if (FindValidRanges(DatabaseAsset, BlendParameters, ExcludeFromDatabaseParameters, ValidRanges))
					{
...
static bool PreprocessSearchIndexWeights(FSearchIndex& SearchIndex, const UPoseSearchSchema* Schema, TConstArrayView<float> Deviation)
{
	...
	for (const TObjectPtr<UPoseSearchFeatureChannel>& ChannelPtr : Schema->GetChannels())
	{
		...
		const int32 ChannelDataStart = ChannelPtr->GetChannelDataOffset();
		const int32 ChannelDataEnd = ChannelDataStart + ChannelPtr->GetChannelCardinality();

		for (int32 ChannelDataIndex = ChannelDataStart; ChannelDataIndex < ChannelDataEnd; ++ChannelDataIndex)
		{
			if (SearchIndex.WeightsSqrt[ChannelDataIndex] < 0.f)
			{
				UE_LOGF(LogPoseSearch, Error, "Error in PreprocessSearchIndexWeights: channel '%ls' in schema '%ls' has a negative weight!", *ChannelPtr->GetName(), *Schema->GetName());
				return false;
			}
		}
	}
	...
	if (DataPreprocessor != EPoseSearchDataPreprocessor::None)
	{
		for (int32 Dimension = 0; Dimension != NumDimensions; ++Dimension)
		{
			...
			if (Deviation[Dimension] > UE_SMALL_NUMBER)
			{
...
{
#if ENABLE_ANIM_DEBUG
	if (GVarMotionMatchTestKDTreeConstruct)
	{
		...
		{
			Values[PoseIndex * DataCardinality + 0] = -5.54383405e-07f;
			Values[PoseIndex * DataCardinality + 1] = 2.77555756e-16f;
		}
	...
	if (PoseSearchMode == EPoseSearchMode::PCAKDTree && NumDimensions > 0 && NumPoses > 0 && NumberOfPrincipalComponents > 0)
	{
		...
		if (ComputationInfo == Eigen::Success)
		{
...
#if ENABLE_ANIM_DEBUG
			if (GVarMotionMatchTestKDTreeConstruct && NumberOfPrincipalComponents == NumDimensions)
			{
			...
			SearchIndex.PCAExplainedVarianceEditorOnly = TotalVariance > UE_KINDA_SMALL_NUMBER ? AccumulatedVariance / TotalVariance : 0.f;
#endif // WITH_EDITORONLY_DATA
...
static void PreprocessSearchIndexKDTree(FSearchIndex& SearchIndex, const UPoseSearchDatabase* Database)
{
	...
	if (NumDimensions > 0 && PoseSearchMode == EPoseSearchMode::PCAKDTree)
	{
		...
		if (GVarMotionMatchTestDeterminismKDTreeConstruct)
		{
			TAlignedArray<float> PCAValuesTest;
			for (int32 Iteration = 0; Iteration < GVarMotionMatchTestNumIterations; ++Iteration)
			{
				...
				{
					UE_LOGF(LogPoseSearch, Warning, "PreprocessSearchIndexKDTree - FKDTree::Construct is not deterministic");
				}
			}
		}
		...
		if (GVarMotionMatchTestKDTreeConstruct)
		{
			...
			for (int32 PointIndex = 0; PointIndex < NumPCAValuesVectors; ++PointIndex)
			{
				...
				if (!bFound)
				{
					UE_LOGF(LogPoseSearch, Error, "PreprocessSearchIndexKDTree - kdtree for %ls is not properly constructed! Couldn't find the Point %d in it", *Database->GetName(), PointIndex);
				}
			}

			if (MaxNumNeighborToFindAPoint >= KDTreeQueryNumNeighbors)
			{
				UE_LOGF(LogPoseSearch, Warning, "Not enough 'KDTreeQueryNumNeighbors' (%d) for database '%ls'. Pose values projected in PCA space have too many duplicates, so try to prune duplicates by tuning 'PCAValuesPruningSimilarityThreshold' or increase 'KDTreeQueryNumNeighbors' at least to %d", KDTreeQueryNumNeighbors, *Database->GetName(), MaxNumNeighborToFindAPoint);
			}
				...
				for (int32 PointIndex = 0; PointIndex < NumPCAValuesVectors; ++PointIndex)
				{
					...
					for (; ResultIndex < NumResults; ++ResultIndex)
					{
						if (PointIndex == Results[ResultIndex].Index)
						{
							...
							{
								UE_LOGF(LogPoseSearch, Error, "PreprocessSearchIndexKDTree - kdtree for %ls is not properly constructed! Couldn't find the Point %d in it within UE_KINDA_SMALL_NUMBER tolerance, after PCA projection", *Database->GetName(), PointIndex);
							}
							break;
						}
					}
					...
					{
						UE_LOGF(LogPoseSearch, Error, "PreprocessSearchIndexKDTree - kdtree for %ls is not properly constructed! Couldn't find the Point %d in it, after PCA projection", *Database->GetName(), PointIndex);
					}
				}
			}
		}
...
static void PreprocessSearchIndexVPTree(FSearchIndex& SearchIndex, const UPoseSearchDatabase* Database, int32 RandomSeed)
{
	...
	if (PoseSearchMode == EPoseSearchMode::VPTree && NumValuesVectors > 0)
	{
		...
		if (GVarMotionMatchTestDeterminismVPTreeConstruct)
		{
			if (!SearchIndex.VPTree.TestConstruct(DataSource))
			{
				UE_LOGF(LogPoseSearch, Error, "PreprocessSearchIndexVPTree - FVPTree construction failed");
			}
			...
			for (int32 Iteration = 0; Iteration < NumIterations; ++Iteration)
			{
				...
				{
					UE_LOGF(LogPoseSearch, Warning, "PreprocessSearchIndexVPTree - FVPTree construction is not deterministic");
				}
			}
		}

		if (GVarMotionMatchTestVPTreeConstruct)
		{
				...
				for (int32 PointIndex = 0; PointIndex < NumValuesVectors; ++PointIndex)
				{
					...
					for (const FIndexDistance& Result : ResultSet.GetUnsortedResults())
					{
						if (Result.Index == PointIndex)
						{
							...
							{
								UE_LOGF(LogPoseSearch, Error, "PreprocessSearchIndexVPTree - VPTree for %ls is malformed because foud PointIndex %d distance %f from itself (distance should be zero)!", *Database->GetName(), PointIndex, Result.Distance);
							}
					...
					if (!bFound)
					{
						UE_LOGF(LogPoseSearch, Error, "PreprocessSearchIndexVPTree - VPTree for %ls is malformed and couldn't find PointIndex %d", *Database->GetName(), PointIndex);
					}
				}
			}
			else
			{
				UE_LOGF(LogPoseSearch, Warning, "PreprocessSearchIndexVPTree - cannot VPTreeConstruct for %ls if there could be potential duplicate poses: set PosePruningSimilarityThreshold > 0 to enforce duplicate pruning", *Database->GetName());
			}
		}
#endif // ENABLE_ANIM_DEBUG
	}
...
static void InitRoledBoneContainers(TMap<FRole, FBoneContainer>& RoledBoneContainers, const UPoseSearchDatabase* DatabaseToLookForAssets, const UPoseSearchSchema* Schema)
{
	auto AddBoneIndex = [](int32 BoneIndex, TArray<uint16>& BoneIndicesWithParents, const UMirrorDataTable* MirrorDataTable)
	{
		...
		if (MirrorDataTable)
		{
			...
			{
				UE_LOGF(LogPoseSearch, Warning, "InitBoneContainersFromRoledSkeleton: MirrorDataTable %ls doesn't contain bone with index %d.", *MirrorDataTable->GetName(), BoneIndex);
			}
		}
	};
	...
	for (const FPoseSearchRoledSkeleton& RoledSkeleton : Schema->GetRoledSkeletons())
	{
		...
		RoledBoneContainer.SetUseRAWData(GVarMotionMatchUseRawAnimationData);
	}
}

static bool IndexDatabase(FSearchIndexBase& SearchIndexBase, const UPoseSearchDatabase* DatabaseToLookForAssets, const UPoseSearchSchema* Schema, const FAssetSamplingContext& SamplingContext, const FFloatInterval& AdditionalExtrapolationTime)
{
	if (!ensure(Schema))
	{
		return false;
	...
	for (int32 AssetIdx = 0; AssetIdx != SearchIndexBase.Assets.Num(); ++AssetIdx)
	{
		...
		for (int32 RoleIndex = 0; RoleIndex < NumRoles; ++RoleIndex)
		{
			...
			if (!SamplerMap.Contains(SamplerMapKey))
			{
				...
				Samplers.Emplace(AnimationAsset, RootTransformOrigin, BlendParameters, FAnimationAssetSampler::DefaultRootTransformSamplingRate, false, !GVarMotionMatchUseRawAnimationData);
			}
		}
	}
...
static FString GetDatabaseAndOuterName(const UPoseSearchDatabase* Database)
{
	if (Database)
	{
		const UObject* Outer = Database->GetOuter();
		if (Outer && !Outer->IsA<UPackage>())
		{
			return Database->GetName() + " [" + Outer->GetName() + "]";
		}
...
static bool ValidateSearchInfoAgainstDatabase(const FSearchIndex& SearchIndex, const UPoseSearchDatabase* Database, const UE::DerivedData::FCacheKey& FullIndexKey)
{
	...
	{
		UE_LOGF(LogPoseSearch, Warning, "%ls - %ls BuildIndex SearchIndex Corrupted! SchemaCardinality mismatch %d vs %d", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(Database), SearchIndex.GetNumDimensions(), Database->Schema->SchemaCardinality);
		return false;
	}
	...
	for (const FSearchIndexAsset& SearchIndexAsset : SearchIndex.Assets)
	{
		...
		if (SourceAssetIdx < 0 || SourceAssetIdx >= NumAnimationAssets)
		{
			UE_LOGF(LogPoseSearch, Warning, "%ls - %ls BuildIndex SearchIndex Corrupted! SearchIndex.Assets referencing missing asset with index %d", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(Database), SourceAssetIdx);
			return false;
		}

		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = Database->GetDatabaseAnimationAsset(SourceAssetIdx))
		{
			if (const UObject* AnimationAsset = DatabaseAsset->GetAnimationAsset())
			{
				if (!DatabaseAsset->IsEnabled())
				{
					UE_LOGF(LogPoseSearch, Warning, "%ls - %ls BuildIndex SearchIndex Corrupted! Found references to disabled asset %ls in the SearchIndex", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(Database), *AnimationAsset->GetName());
					return false;
				}
			}
		}
		...
		{
			UE_LOGF(LogPoseSearch, Warning, "%ls - %ls BuildIndex SearchIndex Corrupted! null FPoseSearchDatabaseAnimationAssetBase asset at index %d!?", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(Database), SourceAssetIdx);
			return false;
		}
	}
...
static bool IndexDatabase(const UPoseSearchDatabase* DependentDatabase, const UPoseSearchSchema* DependentDatabaseSchema, FSearchIndexBase& SearchIndexBase, 
	const UPoseSearchDatabase* MainDatabase, const UE::DerivedData::FCacheKey& FullIndexKey)
{
	check(DependentDatabase);

	const FString DependentDatabaseName = GetDatabaseAndOuterName(DependentDatabase);
	...
	{
		if (DependentDatabase == MainDatabase)
		{
			UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because of invalid Schema", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));
		}
		else
		{
			UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because dependent database '%ls' has an invalid Schema", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *DependentDatabaseName);
		}
		return false;
	}
	...
	for (const FPoseSearchRoledSkeleton& RoledSkeleton : DependentDatabaseSchema->GetRoledSkeletons())
	{
		if (RoledSkeleton.MirrorDataTable)
		{
			...
			{
				UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because '%ls' schema MirrorDataTable Skeleton is not set for Role '%ls' ", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *DependentDatabaseName, *RoledSkeleton.Role.ToString());
				return false;
			}
		}
		else
		...
		for (int32 AnimationAssetIndex = 0; AnimationAssetIndex < DependentDatabase->GetNumAnimationAssets(); ++AnimationAssetIndex)
		{
			if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = DependentDatabase->GetDatabaseAnimationAsset(AnimationAssetIndex))
			{
				if (DatabaseAsset->GetMirrorOption() == EPoseSearchMirrorOption::MirroredOnly || DatabaseAsset->GetMirrorOption() == EPoseSearchMirrorOption::UnmirroredAndMirrored)
				{
					...
					UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because '%ls' schema requires MirrorDataTable(s) to sample mirrored animation assets", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *DependentDatabaseName);
					return false;
				}
			}
		}
	...
	for (int32 AnimationAssetIndex = 0; AnimationAssetIndex < DependentDatabase->GetNumAnimationAssets(); ++AnimationAssetIndex)
	{
		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = DependentDatabase->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
			...
			{
				UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - Database %ls entry %d has no associated animation asset.", *DatabaseAsset->GetName(), AnimationAssetIndex);
			}
			else if (!DatabaseAsset->GetAnimationAsset()->GetPackage() || DatabaseAsset->GetAnimationAsset()->HasAnyFlags(RF_MirroredGarbage))
			{
				UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because '%ls' referenced asset '%ls' is garbage or is not contained in a package", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *DependentDatabaseName, *DatabaseAsset->GetAnimationAsset()->GetName());
				return false;
			}
			else if (AnimationAsset && AnimationAsset->GetSkeleton() == nullptr)
			{
				UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because '%ls' referenced asset '%ls' which has an invalid (null) Skeleton", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *DependentDatabaseName, *DatabaseAsset->GetAnimationAsset()->GetName());
				return false;
			}
			else if (!IsSchemaCompatible(DatabaseAsset, MainDatabase->Schema))
			{
				UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - %ls is not compatible with the schema %ls", *DatabaseAsset->GetName(), *GetNameSafe(MainDatabase->Schema));
			}
			else if (const UBlendSpace* BlendSpace = Cast<UBlendSpace>(DatabaseAsset->GetAnimationAsset()))
			{
				...
				{
					UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - %ls's bShouldMatchSyncPhases flag is not enabled. This is required for properly pose matching blendspaces.", *DatabaseAsset->GetName());
				}
				else
				{
					...
					for (int i = 0; i < BlendSamples.Num() - 1; ++i)
					{
						...
						if (CurrSample.Animation && NextSample.Animation)
						{
							...
							if (bWarning)
							{
								UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - %ls's samples don't share the same layout of sync markers. This is required for properly pose matching blendspaces.", *DatabaseAsset->GetName());
							}
						}
					}
				}
	...
	{
		UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because of invalid assets", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));
		return false;
	}

	if (!IndexDatabase(SearchIndexBase, DependentDatabase, DependentDatabaseSchema, DependentSamplingContext, DependentAdditionalExtrapolationTime))
	{
		UE_LOGF(LogPoseSearch, Log, "%ls - %ls BuildIndex Failed in IndexDatabase", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));
		return false;
	}

#if ENABLE_ANIM_DEBUG
	if (GVarMotionMatchTestDeterminismIndexDatabase)
	{
		for (int32 Iteration = 0; Iteration < GVarMotionMatchTestNumIterations; ++Iteration)
		{
			FSearchIndexBase TestSearchIndexBase = SearchIndexBase;
			if (IndexDatabase(TestSearchIndexBase, DependentDatabase, DependentDatabaseSchema, DependentSamplingContext, DependentAdditionalExtrapolationTime))
			{
				if (TestSearchIndexBase != SearchIndexBase)
				{
					TStringBuilder<0> Message;
					CompareSearchIndexBase(TestSearchIndexBase, SearchIndexBase, DependentDatabaseSchema, Message);
					UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - IndexDatabase is not deterministic\n%ls", *Message);
				}
			}
		}
	}
	...
	, DerivedDataKey(FIoHash::Zero)
{
	...
	{
		UE_LOGF(LogPoseSearch, Log, "Delaying DDC until on the game thread    - %ls", *Database->GetName());
	}
}
...
void FPoseSearchDatabaseAsyncCacheTask::StartNewRequestIfNeeded(bool bPerformConditionalPostLoadIfRequired, FPartialKeyHashes& PartialKeyHashes)
{
	...
	// composing the key
	FKeyBuilder KeyBuilder(Database.Get(), true, bPerformConditionalPostLoadIfRequired, &PartialKeyHashes, !GVarMotionMatchUseRawAnimationData);
	if (KeyBuilder.AnyAssetNotFullyLoaded())
	{
		DerivedDataKey = FIoHash::Zero;
		SetState(EState::Notstarted);
	
		UE_LOGF(LogPoseSearch, Log, "Delaying DDC until dependents are fully loaded - %ls", *Database->GetName());
	}
	else
	{
		...
		if (bHasKeyChanged)
		{
			if (!KeyBuilder.AnyAssetNotReady())
			{
				...
				UE_LOGF(LogPoseSearch, Log, "%ls - %ls BeginCache", *LexToString(DerivedDataKey), *GetDatabaseAndOuterName(Database.Get()));
			...
			else
			{
				DerivedDataKey = FIoHash::Zero;
				SetState(EState::Notstarted);
				UE_LOGF(LogPoseSearch, Log, "Delaying DDC until dependents are ready - %ls", *Database->GetName());
			}
...
void FPoseSearchDatabaseAsyncCacheTask::PreCancelIfDependsOn(const UObject* Object)
{
	...
	if (DatabaseDependencies.Contains(Object))
	{
		...
		FString DatabaseName = IsValid() ? *GetDatabaseAndOuterName(Database.Get()) : TEXT("Garbage Collected Database");
		UE_LOGF(LogPoseSearch, Log, "%ls - %ls PreCancelled because of %ls", *LexToString(DerivedDataKey), *DatabaseName, *Object->GetName());
...
void FPoseSearchDatabaseAsyncCacheTask::Cancel()
{
	...
	FString DatabaseName = IsValid() ? *GetDatabaseAndOuterName(Database.Get()) : TEXT("Garbage Collected Database");
	UE_LOGF(LogPoseSearch, Log, "%ls - %ls Cancelled", *LexToString(DerivedDataKey), *DatabaseName);
...
void FPoseSearchDatabaseAsyncCacheTask::OnGetComplete(UE::DerivedData::FCacheGetResponse&& Response)
{
	...
	if (Response.Status == EStatus::Ok)
	{
		...
		{
			UE_LOGF(LogPoseSearch, Log, "%ls - %ls BuildIndex From Cache", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(Database.Get()));
		}
		else
		{
	...
	{
		ResetSearchIndex();
		UE_LOGF(LogPoseSearch, Log, "%ls - %ls BuildIndex Cancelled", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(Database.Get()));
	}
	
	bool bForceBuildIndex = false;
#if ENABLE_ANIM_DEBUG
	bForceBuildIndex = GVarMotionMatchTestForceIndexing;
#endif // ENABLE_ANIM_DEBUG
		...
		Owner.LaunchTask(TEXT("PoseSearchDatabaseBuild"), [this, FullIndexKey, bCompareSearchIndex]
			{
				...
				if (!MainDatabase)
				{
					UE_LOGF(LogPoseSearch, Log, "%ls - BuildIndex Cancelled because associated Database weak pointer has been released.", *LexToString(FullIndexKey.Hash));
					ResetSearchIndex();
					return;
				}
				...
				for (int32 IndexBaseIdx = 0; IndexBaseIdx < IndexBaseDatabases.Num(); ++IndexBaseIdx)
				{
					...
					Schemas[IndexBaseIdx] = bNormalizeWithCommonSchema ? MainDatabaseSchema : DependentDatabase->Schema.Get();

					if (!IndexDatabase(DependentDatabase, Schemas[IndexBaseIdx], SearchIndexBases[IndexBaseIdx], MainDatabase, FullIndexKey))
					{
						ResetSearchIndex();
						return;
					}
				...
				if (GVarMotionMatchTestDeterminismPruneDuplicateValues)
				{
					const int32 NumIterations = GVarMotionMatchTestNumIterations;
					...
					for (int32 Iteration = 0; Iteration < NumIterations; ++Iteration)
					{
						...
						{
							UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - PruneDuplicateValues is not deterministic");
						}

						if (TestSearchIndexA.ValuesVectorToPoseIndexes != TestSearchIndexB.ValuesVectorToPoseIndexes)
						{
							UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - PruneDuplicateValues ValuesVectorToPoseIndexes generation is not deterministic");
						}
					}
				}
#endif // ENABLE_ANIM_DEBUG
				...
				if (!PreprocessSearchIndexWeights(SearchIndex, MainDatabaseSchema, Deviation))
				{
					UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed while preprocessing weights for schema '%ls'", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *MainDatabaseSchema->GetName());
					ResetSearchIndex();
					return;
				}
				...
				if (Owner.IsCanceled())
				{
					UE_LOGF(LogPoseSearch, Log, "%ls - %ls BuildIndex Cancelled", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));
					ResetSearchIndex();
					return;
				}
				...
				if (ComputationInfo != Eigen::Success)
				{
					...
					UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because of '%ls' while calculating PCA data. Try with a different dataset or change the database 'Pose Search Mode'", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *Reason);
				...
				if (GVarMotionMatchTestDeterminismPruneDuplicatePCAValues)
				{
					FSearchIndex TestSearchIndexA = SearchIndex;
					...
					for (int32 Iteration = 0; Iteration < GVarMotionMatchTestNumIterations; ++Iteration)
					{
						...
						{
							UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - PruneDuplicatePCAValues is not deterministic");
						}

						if (TestSearchIndexA.PCAValuesVectorToPoseIndexes != TestSearchIndexB.PCAValuesVectorToPoseIndexes)
						{
							UE_LOGF(LogPoseSearch, Warning, "OnGetComplete - PruneDuplicatePCAValues PCAValuesVectorToPoseIndexes generation is not deterministic");
						}
					}
				}
#endif // ENABLE_ANIM_DEBUG
				...
				UE_LOGF(LogPoseSearch, Log, "%ls - %ls BuildIndex Succeeded", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));
				...
				if (bCompareSearchIndex && SearchIndexCompare != SearchIndex)
				{
					...
					UE_LOGF(LogPoseSearch, Warning, "%ls - %ls BuildIndex mismatch with DDC Index\n%ls", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *Message);
				}

				if (!ValidateSearchInfoAgainstDatabase(SearchIndex, Database.Get(), FullIndexKey))
				{
					UE_LOGF(LogPoseSearch, Error, "%ls - %ls BuildIndex Failed because of corrupted SearchIndex!", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(Database.Get()));
					ResetSearchIndex();
					return;
				}
#endif // ENABLE_ANIM_DEBUG
				...
				GetCache().Put({ { { MainDatabase->GetPathName() }, Builder.Build() } }, Owner, [this, MainDatabaseSchema, MainDatabase, FullIndexKey](FCachePutResponse&& Response)
					{
						switch (Response.Status)
						{
						case EStatus::Error:
							UE_LOGF(LogPoseSearch, Log, "%ls - %ls Failed to store DDC", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));
							break;
						case EStatus::Canceled:
							UE_LOGF(LogPoseSearch, Log, "%ls - %ls Canceled to store DDC", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));
							break;
						case EStatus::Ok:
							UE_LOGF(LogPoseSearch, Log, "%ls - %ls BuildIndex stored to DDC", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase));

#if ENABLE_ANIM_DEBUG
							if (GVarMotionMatchTestDDCConsistency)
							{
								const FCacheKey CacheKey{ Bucket, DerivedDataKey };
								const FCacheGetRequest CacheRequest = { { Database->GetPathName() }, CacheKey, ECachePolicy::Default };
								GetCache().Get(MakeArrayView(&CacheRequest, 1), Owner, [this, MainDatabaseSchema, MainDatabase](FCacheGetResponse&& Response)
									{
										const FCacheKey& FullIndexKey = Response.Record.GetKey();
										...
										if (Response.Status == EStatus::Ok)
										{
											...
											if (TestSearchIndex != SearchIndex)
											{
												...
												UE_LOGF(LogPoseSearch, Warning, "%ls - %ls DDC Index mismatch with BuildIndex\n%ls", *LexToString(FullIndexKey.Hash), *GetDatabaseAndOuterName(MainDatabase), *Message);
											}
										}
									});
							}
...
{
	PreModified(Object, FKeyBuilder::ShouldIncludeFlagsToCheck);
}

void FAsyncPoseSearchDatabasesManagement::PreModified(UObject* Object, EObjectFlags ObjectFlagsToCheck)
{
	if (FKeyBuilder::ShouldInclude(Object, ObjectFlagsToCheck))
	{
		ensure(IsInGameThread());

		FScopeLock Lock(&Mutex);

		PartialKeyHashes.InvalidateKeysFor(Object);
...
void FAsyncPoseSearchDatabasesManagement::PostModified(UObject* Object, EObjectFlags ObjectFlagsToCheck)
{
	if (FKeyBuilder::ShouldInclude(Object, ObjectFlagsToCheck))
	{
		ensure(IsInGameThread());
...
{
	PostModified(Object, FKeyBuilder::ShouldIncludeFlagsToCheck);
}
...
void FAsyncPoseSearchDatabasesManagement::OnPackageReloaded(const EPackageReloadPhase InPackageReloadPhase, FPackageReloadedEvent* InPackageReloadedEvent)
{
	...
	else if (InPackageReloadPhase == EPackageReloadPhase::PrePackageFixup)
	{
		...
		{
			PreModified(RepointPair.Key, FKeyBuilder::ShouldIncludeFlagsToCheckOnReload);
		}
	}
	else if (InPackageReloadPhase == EPackageReloadPhase::PostPackageFixup)
	{
		...
		for (const TPair<UObject*, UObject*>& RepointPair : RepointedObjects)
		{
			PostModified(RepointPair.Key, FKeyBuilder::ShouldIncludeFlagsToCheckOnReload);
		}
	}
}
...
{
	PreModified(InObject, FKeyBuilder::ShouldIncludeFlagsToCheck);
}
...
{
	PostModified(InObject, FKeyBuilder::ShouldIncludeFlagsToCheck);
}
...
void FAsyncPoseSearchDatabasesManagement::Tick(float DeltaTime)
{
	...
	if (GVarMotionMatchTestDeterminismPartialKeyHashesSaveFile || GVarMotionMatchTestDeterminismPartialKeyHashes)
	{
		FSerializablePartialKeyHashes SerializablePartialKeyHashes(PartialKeyHashes);
		FString FileName = FString::Printf(TEXT("%s//TestPartialKeyHashesDeterminism//PartialKeyHashes.bin"), *FPaths::EngineDir());

		if (GVarMotionMatchTestDeterminismPartialKeyHashesSaveFile)
		{
			TArray<uint8> RawBytes;
			RawBytes.Reserve(1024 * 1024);
			FMemoryWriter Writer(RawBytes);
			Writer << SerializablePartialKeyHashes;
			FFileHelper::SaveArrayToFile(RawBytes, *FileName);

			GVarMotionMatchTestDeterminismPartialKeyHashesSaveFile = false;
			UE_LOGF(LogPoseSearch, Log, "FAsyncPoseSearchDatabasesManagement::Tick PartialKeyHashes serialized in '%ls'", *FileName);
		}

		if (GVarMotionMatchTestDeterminismPartialKeyHashes)
		{
			TArray<uint8> LoadedData;
			const bool bLoadedFile = FFileHelper::LoadFileToArray(LoadedData, *FileName, FILEREAD_Silent);
			if (bLoadedFile)
			{
				FMemoryReaderView Reader(LoadedData);
				FSerializablePartialKeyHashes SerializablePartialKeyHashesFromFile;
				Reader << SerializablePartialKeyHashesFromFile;
				if (SerializablePartialKeyHashes.LogDifferencesAgains(SerializablePartialKeyHashesFromFile))
				{
					UE_LOGF(LogPoseSearch, Log, "FAsyncPoseSearchDatabasesManagement::Tick PartialKeyHashes determinism validation succeeded");
				}
			}
			else
			{
				UE_LOGF(LogPoseSearch, Error, "FAsyncPoseSearchDatabasesManagement::Tick Failed to load '%ls' with serialized PartialKeyHashes", *FileName);
			}
			GVarMotionMatchTestDeterminismPartialKeyHashes = false;
		}
	}

	// testing sampler determinism
	const bool bTestAssetSamplerDeterminism = GVarMotionMatchTestDeterminismAssetSampler;
	const bool bTestAssetSamplerDeterminismFromPreviousExecution = GVarMotionMatchTestDeterminismAssetSamplerFromPreviousExecution;
	if (bTestAssetSamplerDeterminism || bTestAssetSamplerDeterminismFromPreviousExecution)
	{
								...
								DatabaseAsset->IterateOverSamplingParameter([Database, DatabaseAsset, AnimationAssetIndex, &TestSamples, &RoledBoneContainers](const FVector& BlendParameters)
									{
										...
										for (int32 RoleIndex = 0; RoleIndex < NumRoles; ++RoleIndex)
										{
											...
											if (const FBoneContainer* BoneContainer = RoledBoneContainers.Find(Role))
											{
												...
												constexpr int32 NUM_TESTING_SAMPLES_PER_DATABASE_ASSET = 100; // make sure it's greater than 1
		...
		int32 NumIterations = GVarMotionMatchTestNumIterations;
		ParallelFor(TestSamples.Num(), [&TestSamples, NumIterations, bTestAssetSamplerDeterminism, bTestAssetSamplerDeterminismFromPreviousExecution](int32 TestSampleIndex)
			{
				...
				const FAnimationAssetSampler AssetSampler(TestSample.AnimationAsset, FTransform::Identity, FVector::ZeroVector, FAnimationAssetSampler::DefaultRootTransformSamplingRate, true, !GVarMotionMatchUseRawAnimationData);
				...
				if (bTestAssetSamplerDeterminism)
				{
					for (int32 IterationIndex = 0; IterationIndex < NumIterations; ++IterationIndex)
					{
						...
						{
							UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler - ExtractRootTransform is not deterministic");
						}

						const TConstArrayView<FTransform> TestBones = TestPose.GetBones();
						if (Bones.Num() != TestBones.Num())
						{
							UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler - ExtractPose is not deterministic");
						}
						else
						{
		...
		if (bTestAssetSamplerDeterminismFromPreviousExecution)
		{
			for (const FTestSample& TestSample : TestSamples)
			{
				...
				if (bLoadedFile)
				{
					...
					{
						UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler - Loaded the wrong amount of data!");
					}
					else if (TestSample.SerializedData.Num() != LoadedData.Num())
					{
						UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler - Loaded data mismatch expected amount of data!");
					}
					else if (FMemory::Memcmp(TestSample.SerializedData.GetData(), LoadedData.GetData(), TestSample.SerializedData.Num()) != 0)
					{
						...
						for (int32 TransformIndex = 0; TransformIndex < NumTransforms; ++TransformIndex)
						{
							if (FMemory::Memcmp(&LoadedTransforms[TransformIndex], &SerializedTransforms[TransformIndex], sizeof(FTransform)) != 0)
							{
								...
								UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler - ExtractPose is not deterministic for %ls for Bone %d", *FileName, BoneIndex);
							}
						}
					}
				}
				else
				{
					...
					if (!bSavedFile)
					{
						UE_LOGF(LogPoseSearch, Error, "FAnimationAssetSampler - Failed to save comparison file!");
					}
				}
			}
		}
	...
	if (GVarMotionMatchTestInvalidateCache)
	{
		if (GVarMotionMatchTestWaitForTaskCompletion)
		{
			...
			for (int32 TaskIndex = Tasks.Num() - 1; TaskIndex >= 0; --TaskIndex)
			{
				...
				{
					UE_LOGF(LogPoseSearch, Log, "%ls - %ls Removed because of InvalidateCache with WaitForTaskCompletion", *LexToString(Tasks[TaskIndex]->GetDerivedDataKey()), *GetDatabaseAndOuterName(Tasks[TaskIndex]->GetDatabase()));
		...
		else
		{
			for (TUniquePtr<FPoseSearchDatabaseAsyncCacheTask>& TaskPtr : Tasks)
			{
				UE_LOGF(LogPoseSearch, Log, "%ls - %ls Cancelled because of InvalidateCache", *LexToString(TaskPtr->GetDerivedDataKey()), *GetDatabaseAndOuterName(TaskPtr->GetDatabase()));
			}
			Tasks.Reset();
		}
	}

	if (GVarMotionMatchTestDeterminismSynchronizeWithExternalDependencies)
	{
	...
	for (int32 TaskIndex = Tasks.Num() - 1; TaskIndex >= 0; --TaskIndex)
	{
		...
		{
			UE_LOGF(LogPoseSearch, Log, "%ls - Removed because IsValid", *LexToString(Tasks[TaskIndex]->GetDerivedDataKey()));
			Tasks.RemoveAtSwap(TaskIndex, EAllowShrinking::No);
		}
		else if (Tasks[TaskIndex]->GetState() == FPoseSearchDatabaseAsyncCacheTask::EState::Cancelled)
		{
			const UPoseSearchDatabase* TaskDatabase = Tasks[TaskIndex]->GetDatabase();

			UE_LOGF(LogPoseSearch, Log, "%ls - %ls Removed because is Cancelled", *LexToString(Tasks[TaskIndex]->GetDerivedDataKey()), *GetDatabaseAndOuterName(TaskDatabase));
			Tasks.RemoveAtSwap(TaskIndex, EAllowShrinking::No);

#if ENABLE_ANIM_DEBUG
			if (GVarMotionMatchReindexCancelledDatabases)
			{
				RequestAsyncBuildIndex(TaskDatabase, ERequestAsyncBuildFlag::NewRequest);
			}
#endif // ENABLE_ANIM_DEBUG
		}
		else
...
#if ENABLE_ANIM_DEBUG
	if (GVarMotionMatchTestDeterminismDDCKey)
	{
		for (int32 TaskIndex = 0; TaskIndex < Tasks.Num(); ++TaskIndex)
		{
			if (const UPoseSearchDatabase* Database = Tasks[TaskIndex]->GetDatabase())
			{
				FKeyBuilder KeyBuilder(Database, false, false, nullptr, !GVarMotionMatchUseRawAnimationData);
				const FIoHash IoHash = KeyBuilder.Finalize();

				for (int32 IterationIndex = 0; IterationIndex < GVarMotionMatchTestNumIterations; ++IterationIndex)
				{
					FKeyBuilder TestKeyBuilder(Database, false, false, &PartialKeyHashes, !GVarMotionMatchUseRawAnimationData);
					const FIoHash TestIoHash = TestKeyBuilder.Finalize();

					if (!KeyBuilder.ValidateAgainst(TestKeyBuilder))
					{
						UE_LOGF(LogPoseSearch, Error, "FKeyBuilder - key generation is not deterministic: %ls / %ls for asset %ls", *LexToString(IoHash), *LexToString(TestIoHash), *Database->GetName());
					}
...
EAsyncBuildIndexResult FAsyncPoseSearchDatabasesManagement::RequestAsyncBuildIndexInternal(const UPoseSearchDatabase* Database, ERequestAsyncBuildFlag Flag)
{
	...
	if (Database->GetPackage()->HasAnyPackageFlags(PKG_Cooked))
	{
		...
		check(!Database->GetSearchIndex().IsEmpty());
...
void FAsyncPoseSearchDatabasesManagement::Clear()
{
	FScopeLock Lock(&Mutex);
	...
	for (TUniquePtr<FPoseSearchDatabaseAsyncCacheTask>& TaskPtr : This.Tasks)
	{
		UE_LOGF(LogPoseSearch, Log, "%ls - %ls Cancelled because of Clear", *LexToString(TaskPtr->GetDerivedDataKey()), *GetDatabaseAndOuterName(TaskPtr->GetDatabase()));
	}
	This.Tasks.Reset();
}
