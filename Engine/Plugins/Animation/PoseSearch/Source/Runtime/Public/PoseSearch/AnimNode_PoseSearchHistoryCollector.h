...
public:
	...
	// GetPoseHistory got deprecated in favor of GetPoseHistoryPtr, since FAnimNode_PoseSearchHistoryCollector_Base::PoseHistoryPtr can be invalid during blue print
	// recompilation when the pose history node exists in the graph but on an inactive branch, for instance on a never activated branch of a blend by bool node
	const UE::PoseSearch::FPoseHistory* GetPoseHistoryPtr() const { return PoseHistoryPtr.Get(); }
	UE::PoseSearch::FPoseHistory* GetPoseHistoryPtr() { return PoseHistoryPtr.Get(); }
	...
	TArray<FBoneIndexType> GetRequiredBones(const USkeletalMeshComponent* SkeletalMeshComponent) const;
