...
struct FVPTreeResultSet
{
        ...
        : NumNeighbors(InNumNeighbors)
    {
		...
		{
			UE_LOGF(LogPoseSearch, Warning, "FVPTreeResultSet - preallocated Heap data size 'HeapDefaultMaxSize' of %d is less than requested %d num neighbors. Performances will be negatively impacted", HeapDefaultMaxSize, InNumNeighbors);
		}
#endif // !NO_LOGGING
    }
	...
	)
    {
        ...
        else if (Distance < GetWorstDistance())
        {
			...
			{
				UE_LOGF(LogPoseSearch, Error, "FVPTreeResultSet::AddPoint - we removed the best node from the ResultSet...");
			}
#endif // VALIDATE_FINDNEIGHBORS
		...
		while (!NodesToSearch.IsEmpty())
		{
			...
			if (!bBestNodeIndexFound)
			{
				...
				if (!bNodeIndexContainsBestNodeIndex)
				{
					...
					if (!bOtherNodeIndexContainsBestNodeIndex)
					{
						UE_LOGF(LogPoseSearch, Error, "FVPTree::FindNeighbors - we couldn't find the best node...");
					}
				}
			}
#endif // VALIDATE_FINDNEIGHBORS
			...
			{
				UE_LOGF(LogPoseSearch, Error, "FVPTree::FindNeighbors - we had the best node in our hands and let it slipped away...");
			}
			...
			if (Node.LeftIndex != INDEX_NONE && QueryDistance < Node.Distance + Tau)
			{
				...
				{
					UE_LOGF(LogPoseSearch, Warning, "FVPTree::FindNeighbors - requested more than preallocated NodesToSearch data size 'NodesToSearchDefaultMaxSize' (%d / %d)", NodesToSearch.Num(), NodesToSearchDefaultMaxSize);
				}
#endif // !NO_LOGGING
			...
			{
				UE_LOGF(LogPoseSearch, Error, "FVPTree::FindNeighbors - we lost strack of the best node...");
			}
#endif // VALIDATE_FINDNEIGHBORS
		}
    }
