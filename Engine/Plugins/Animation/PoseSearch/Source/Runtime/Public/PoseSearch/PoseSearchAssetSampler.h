...
struct FAnimationAssetSampler
{
	...
	UE_DEPRECATED(5.8, "This ExtractPose signatures will be made private. Use other signatures")
	...
	// Populates FAnimNotifyContext from the animation asset at Time
	POSESEARCH_API void ExtractAnimNotifyContext(float Time, FAnimNotifyContext& PreAllocatedNotifyContext) const;
	// Extracts notify states present in the animation asset at Time
	...
	// Extracts notify events present in the animation asset at Time
	POSESEARCH_API void ExtractAnimNotifyEvents(float Time, FAnimNotifyContext& PreAllocatedNotifyContext, const TFunction<bool(const FAnimNotifyEvent*)>& ProcessAnimNotifyEvent) const;
	...
	POSESEARCH_API static float WrapOrClampTime(float Time, float PlayLength, bool bLoopable);
