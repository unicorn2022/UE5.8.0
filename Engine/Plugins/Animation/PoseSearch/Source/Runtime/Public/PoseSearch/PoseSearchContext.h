...
{
	bool ContainsAnyOf(TConstArrayView<const UObject*> Objects) const
	{
...
// usually used as set of asset indexes to consider (per database)
// Experimental, this feature might be removed without warning, not for production use
typedef TSet<int32, DefaultKeyFuncs<int32>, TInlineSetAllocator<8, TMemStackSetAllocator<>>> FStackAssetIndexes;
// usually used as container to map database to what asset indexes to consider
typedef TMap<const UPoseSearchDatabase*, FStackAssetIndexes, TInlineSetAllocator<8, TMemStackSetAllocator<>>> FStackDatabaseToAssetIndexes;

// Utils functions
const UObject* GetAnimContext(const FChooserEvaluationContext* Context);
// Experimental, this feature might be removed without warning, not for production use
UE_API const FTransform& GetContextTransform(const UObject* AnimContext, bool bCheckIsInGameThread = true);
...
struct FDebugDrawParams
{
	...
	UE_API FDebugDrawParams(TConstArrayView<FChooserEvaluationContext*> InAnimContexts, TConstArrayView<const IPoseHistory*> InPoseHistories, const FRoleToIndex& InRoleToIndex, const UPoseSearchDatabase* InDatabase, EDrawContext InDrawContext = EDrawContext::DrawQuery);
	...
	UE_API void Init(TConstArrayView<FChooserEvaluationContext*> InAnimContexts, TConstArrayView<const IPoseHistory*> InPoseHistories, const FRoleToIndex& InRoleToIndex, const UPoseSearchDatabase* InDatabase, EDrawContext InDrawContext = EDrawContext::DrawQuery);
	...
	UE_API void DrawWedge(const FVector& Origin, const FVector& Direction, float InnerRadius, float OuterRadius, float WidthInDegrees, int32 Segments, const FColor& Color, float Thickness = 0.f) const;
	...
	EDrawContext GetDrawContext() const { return DrawContext; }
...
private:
	...
	EDrawContext DrawContext = EDrawContext::DrawCandidate;
...
struct FSearchContext
{
	...
	void SetAssetIndexesToConsiderPerDatabase(const FStackDatabaseToAssetIndexes* InAssetIndexesToConsiderPerDatabase)
	{
		AssetIndexesToConsiderPerDatabase = InAssetIndexesToConsiderPerDatabase;
	}

	const FStackDatabaseToAssetIndexes* GetAssetIndexesToConsiderPerDatabase() const
	{
		return AssetIndexesToConsiderPerDatabase;
	}
...
private:
	...
	const FStackDatabaseToAssetIndexes* AssetIndexesToConsiderPerDatabase = nullptr;
...
public:
	...
	UE_API void Track(const UPoseSearchDatabase* Database, int32 PoseIdx, EPoseCandidateFlags PoseCandidateFlags, TConstArrayView<float> DynamicWeightsSqrt, float ContinuingPoseCostAddend, float ContinuingInteractionCostAddend, float ContinuingContextInteractionCostAddend);
