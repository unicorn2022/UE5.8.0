...
private:
	...
	FTransform GetTransform(float SampleTime, const FRole& Role, int8 SchemaBoneIdx = RootSchemaBoneIdx);
	FTransform GetTransform(float SampleTime, int32 RoleIndex, const FBoneReference& BoneReference);
	...
	FTransform GetComponentSpaceTransform(float SampleTime, const FRole& Role, int8 SchemaBoneIdx = RootSchemaBoneIdx);
	...
	bool GetSamplePositionInternal(FVector& OutSamplePosition, float SampleTime, float OriginTime, int8 SchemaSampleBoneIdx, int8 SchemaOriginBoneIdx, const FRole& SampleRole, const FRole& OriginRole, int32 SamplingAttributeId);
