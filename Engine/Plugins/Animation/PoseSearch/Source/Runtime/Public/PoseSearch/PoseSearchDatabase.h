...
struct FPoseSearchDatabaseAnimationAssetBase
{
	...
	// Experimental, this feature might be removed without warning, not for production use
	UE_API virtual bool IsMirrorOptionCompatible(TObjectPtr<const UPoseSearchSchema> InSchema) const;
	// Experimental, this feature might be removed without warning, not for production use
...
public:
	...
	// Cost added to poses from this database (if setup with UMultiAnimAsset assets in "interaction mode") as continuation of a previous interaction (same anim contexts with the same assigned roles)
	...
	// behaving like ContinuingInteractionCostBias, but for continuing interactions with the same anim contexts (e.g. same actors)
	// Experimental, this feature might be removed without warning, not for production use
	UPROPERTY(EditAnywhere, Category = "Database", meta = (ExcludeFromHash))
	float ContinuingContextInteractionCostBias = 0.f;
...
public:
	...
	UE_DEPRECATED(5.8, "no longer necessary API")
	...
	UE_API int32 GetPoseIndex(const UObject* AnimationAsset, float AnimationAssetTime, bool bMirrored, const FVector& BlendParameters, const UE::PoseSearch::FStackAssetIndexes* AssetIndexesToConsider = nullptr) const;
	...
	UE_API void SetAnimationAssetAt(const FPoseSearchDatabaseAnimationAsset& AnimationAsset, int32 AnimationAssetIndex);
#endif // WITH_EDITOR

	UE_API const FPoseSearchDatabaseAnimationAsset* GetDatabaseAnimationAsset(int32 AnimationAssetIndex) const;

	UE_DEPRECATED(5.8, "Use GetDatabaseAnimationAsset(SearchIndexAsset.GetSourceAssetIdx()) instead")
	...
	UE_DEPRECATED(5.8, "Use SetAnimationAssetAt(AnimationAsset, AnimationAssetIndex) instead")
	UE_API FPoseSearchDatabaseAnimationAsset* GetMutableDatabaseAnimationAsset(int32 AnimationAssetIndex);
	UE_DEPRECATED(5.8, "Use SetAnimationAssetAt(AnimationAsset, SearchIndexAsset.GetSourceAssetIdx()) instead")
...
private:
	...
	void PopulateSelectableAssetIdx(FSelectableAssetIdx& SelectableAssetIdx, const UE::PoseSearch::FStackDatabaseToAssetIndexes* AssetIndexesToConsiderPerDatabase) const;
	...
	void ExtractCostAddends(const UE::PoseSearch::FSearchContext& SearchContext, bool bIsContinuingPoseSearch, float& ContinuingPoseCostAddend, float& ContinuingInteractionCostAddend, float& ContinuingContextInteractionCostAddend) const;
};
...
inline const TDatabaseAnimationAsset* UPoseSearchDatabase::GetDatabaseAnimationAsset(int32 AnimationAssetIndex) const
{
		...
		std::is_same<TDatabaseAnimationAsset, FPoseSearchDatabaseAnimationAssetBase>::value)
	{
		...
		return reinterpret_cast<const TDatabaseAnimationAsset*>(GetDatabaseAnimationAsset(AnimationAssetIndex));
	}
...
inline TDatabaseAnimationAsset* UPoseSearchDatabase::GetMutableDatabaseAnimationAsset(int32 AnimationAssetIndex)
{
	...
	{
		// using reinterpret_cast to be able to compile this method even with other types (that will return null)
		return reinterpret_cast<TDatabaseAnimationAsset*>(GetMutableDatabaseAnimationAsset(AnimationAssetIndex));
	}
