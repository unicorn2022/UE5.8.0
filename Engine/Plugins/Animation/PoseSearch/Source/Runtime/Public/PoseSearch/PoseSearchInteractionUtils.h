...
// Experimental, this feature might be removed without warning, not for production use
POSESEARCH_API void CalculateFullAlignedTransforms(const FPoseSearchBlueprintResult& Result, float Time, float TimeOffset, bool bWarpUsingRootBone, TArrayView<FTransform> OutFullAlignedTransforms);
...
static void GenerateCombinationsRecursive(int32 DataCardinality, int32 DataIndex, TArray<int32, TInlineAllocator<PreallocatedRolesNum>>& Combination, int32 CombinationIndex, EvaluateCombinationType EvaluateCombination)
{
