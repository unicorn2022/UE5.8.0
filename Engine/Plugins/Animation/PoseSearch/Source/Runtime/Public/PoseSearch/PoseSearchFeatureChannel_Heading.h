...
public:
	...
	UE_DEPRECATED(5.8, "Use BP_GetWorldRotationFromContext instead")
	...
	UFUNCTION(BlueprintPure, BlueprintNativeEvent, meta=(BlueprintThreadSafe, DisplayName = "Get World Rotation From Context"), Category = "Settings")
	UE_API FQuat BP_GetWorldRotationFromContext(UPARAM(Ref) FChooserEvaluationContext& Context) const;
...
#if WITH_EDITORONLY_DATA
	virtual FLinearColor GetDebugColor() const override { return DebugColor; }
#endif // WITH_EDITORONLY_DATA

	UE_API FVector GetAxis(const FQuat& Rotation) const;
	static UE_API void FindOrAddToSchema(UPoseSearchSchema* Schema, float SampleTimeOffset, const FName& BoneName, const UE::PoseSearch::FRole& Role, EHeadingAxis HeadingAxis = EHeadingAxis::X, EPermutationTimeType PermutationTimeType = EPermutationTimeType::UseSampleTime, bool bDefaultWithRootBone = true);
};
