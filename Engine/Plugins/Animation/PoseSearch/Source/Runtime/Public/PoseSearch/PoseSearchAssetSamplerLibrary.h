...
struct FPoseSearchAssetSamplerPose
{
	...
	FPoseSearchAssetSamplerPose() = default;

	FPoseSearchAssetSamplerPose(const FPoseSearchAssetSamplerPose& Other)
	: RootTransform(Other.RootTransform)
	, BoneContainer(Other.BoneContainer)
	, Pose(Other.Pose)
	, Curve(Other.Curve)
	, ComponentSpacePose(Other.ComponentSpacePose)
	{
		Pose.SetBoneContainer(&BoneContainer);
		const_cast<FCompactHeapPose&>(ComponentSpacePose.GetPose()).SetBoneContainer(&BoneContainer);
	}

	FPoseSearchAssetSamplerPose& operator=(const FPoseSearchAssetSamplerPose& Other)
	{
		if (this != &Other)
		{
			this->~FPoseSearchAssetSamplerPose();
			new(this) FPoseSearchAssetSamplerPose(Other);
		}
		return *this;
	}
...
public:
	UFUNCTION(BlueprintPure, Category = "Animation|Pose Search|Experimental", meta = (BlueprintThreadSafe, AdvancedDisplay = "AnimInstance"))
	static UE_API FPoseSearchAssetSamplerPose SamplePose(const FPoseSearchAssetSamplerInput Input, const UAnimInstance* AnimInstance = nullptr);

	static UE_API FPoseSearchAssetSamplerPose SamplePose(const FBoneContainer& BoneContainer, const FPoseSearchAssetSamplerInput Input);
	...
	static FTransform GetTransform(FComponentSpacePoseType& ComponentSpacePose, const FTransform& RootTransform, FCompactPoseBoneIndex CompactPoseBoneIndex = FCompactPoseBoneIndex(INDEX_NONE), EPoseSearchAssetSamplerSpace Space = EPoseSearchAssetSamplerSpace::World)
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "UPoseSearchAssetSamplerLibrary::GetTransform invalid Pose");
		...
		if (CompactPoseBoneIndex.GetInt() == INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogPoseSearch, Error, "UPoseSearchAssetSamplerLibrary::GetTransform invalid Space %ls to get the RootTransform. Expected space: %ls", *UEnum::GetDisplayValueAsText(Space).ToString(), *UEnum::GetDisplayValueAsText(EPoseSearchAssetSamplerSpace::World).ToString());
			}
		...
		{
			UE_LOGF(LogPoseSearch, Error, "UPoseSearchAssetSamplerLibrary::GetTransform invalid CompactPoseBoneIndex %d", CompactPoseBoneIndex.GetInt());
			return FTransform::Identity;
		}
