...
void FInteractionAssetViewModel::Tick(float DeltaSeconds)
{
	...
	else
	{
		...
		const int32 NumRoles = InteractionAsset->GetNumRoles();
		...
		bool bAnyAddedOrRemovedActors = false;
		...
		if (PreviewActors.Num() == NumRoles)
		{
			...
			for (int32 PreviewActorIndex = 0; PreviewActorIndex < PreviewActors.Num(); ++PreviewActorIndex)
			{
				...
				AnimContexts[PreviewActorIndex] = PreviewActor.GetAnimPreviewInstance();
				Roles[PreviewActorIndex] = PreviewActor.GetRole();
			}

			if (PreviewActorRoleToIndex.Num() == NumRoles)
			{
				// testing CalculateWarpTransforms
#if WITH_EDITORONLY_DATA
				if (InteractionAsset->bEnableDebugWarp)
				{
					TArray<FTransform, TInlineAllocator<PreallocatedRolesNum>> ActorTransforms;
					ActorTransforms.SetNum(NumRoles);
					for (int32 InteractionAssetRoleIndex = 0; InteractionAssetRoleIndex < NumRoles; ++InteractionAssetRoleIndex)
					{
					...
					FullAlignedActorTransforms.SetNum(NumRoles);
					const TConstArrayView<const UMirrorDataTable*> MirrorDataTables;

					InteractionAsset->CalculateWarpTransforms(PlayTime, 0.f, ActorTransforms, FullAlignedActorTransforms, MirrorDataTables, AnimContexts);
...
#if ENABLE_ANIM_DEBUG
						// FullAlignedActorTransforms drawing 
						const FVector FullAlignedActorPosition = FullAlignedActorTransforms[InteractionAssetRoleIndex].GetTranslation();
						const FQuat FullAlignedActorRotation = FullAlignedActorTransforms[InteractionAssetRoleIndex].GetRotation();
						DrawDebugCircle(DebugWorld, FMatrix(FVector::ZAxisVector, FVector::XAxisVector, FVector::YAxisVector, FullAlignedActorPosition), 5.f, 16, FColorList::Yellow, false, 0.f, SDPG_Foreground, 0.f, false);
						DrawDebugLine(DebugWorld, FullAlignedActorPosition, FullAlignedActorPosition + FullAlignedActorRotation.RotateVector(FVector::XAxisVector) * 10, FColorList::Yellow, false, 0.f, SDPG_Foreground, 0.f);
#endif // ENABLE_ANIM_DEBUG
...
#if ENABLE_ANIM_DEBUG
					if (InteractionAsset->DebugTimeOffset > UE_KINDA_SMALL_NUMBER)
					{
						InteractionAsset->CalculateWarpTransforms(PlayTime, InteractionAsset->DebugTimeOffset, ActorTransforms, FullAlignedActorTransforms, MirrorDataTables, AnimContexts);
						...
						for (int32 InteractionAssetRoleIndex = 0; InteractionAssetRoleIndex < NumRoles; ++InteractionAssetRoleIndex)
						{
							...
							const FVector FullAlignedActorPosition = FullAlignedActorTransforms[InteractionAssetRoleIndex].GetTranslation();
							const FQuat FullAlignedActorRotation = FullAlignedActorTransforms[InteractionAssetRoleIndex].GetRotation();
							DrawDebugCircle(DebugWorld, FMatrix(FVector::ZAxisVector, FVector::XAxisVector, FVector::YAxisVector, FullAlignedActorPosition), 5.f, 16, FColorList::Pink, false, 0.f, SDPG_Foreground, 0.f, false);
							DrawDebugLine(DebugWorld, FullAlignedActorPosition, FullAlignedActorPosition + FullAlignedActorRotation.RotateVector(FVector::XAxisVector) * 10, FColorList::Pink, false, 0.f, SDPG_Foreground, 0.f);
						}
					}
#endif // ENABLE_ANIM_DEBUG
				}
				...
				// extracting and drawing Constraints

				// @todo: fill up those parameters
				const FVector BlendParameters = FVector::ZeroVector;
				const bool bIsMirrored = false;

				TArray<FPoseSearchConstraint> Constraints;
				UPoseSearchInteractionLibrary::UpdateConstraints(Constraints, InteractionAsset, StepDeltaTime, PlayTime, bIsMirrored, BlendParameters, AnimContexts, Roles);

				if (const UWorld* World = GetWorld())
				{
					for (const FPoseSearchConstraint& Constraint : Constraints)
					{
						DrawDebugLine(World, Constraint.FromSocketTransform.GetTranslation(), Constraint.ToSocketTransform.GetTranslation(), FColor::Red, false, -1.f, SDPG_Foreground);
					}
				}
			}
		}
