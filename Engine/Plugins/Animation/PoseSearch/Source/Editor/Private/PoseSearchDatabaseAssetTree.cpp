...
namespace UE::PoseSearch
{
			...
			[
				...
				[
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("DragDropHint", "Drag and drop Animation Sequences, Anim Composites, Blendspaces, or Anim Montages"))
	...
	bool SDatabaseAssetTree::SupportsKeyboardFocus() const
	{
		return true;
	}

	FReply SDatabaseAssetTree::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
	{
		// When the database is empty the STreeView has no rows, so STableRow::OnMouseButtonDown never fires
		// and focus is never acquired. Claim focus here so that keyboard shortcuts (e.g. Ctrl+V) work even
		// on an empty database. This override is only reached when no deeper widget handled the click, which
		// is exactly the empty-tree case; non-empty clicks are consumed by STableRow before bubbling here.
	...
	TSharedRef<SWidget> SDatabaseAssetTree::GenerateFilterBoxWidget()
	{
		...
		SAssignNew(SearchBox, SSearchBox)
			...
			.ToolTipText(LOCTEXT("AssetFilterTooltip", "Enter Asset Filter..."))
	...
	void SDatabaseAssetTree::RecoverSelection(const TArray<TSharedPtr<FDatabaseAssetTreeNode>>& PreviouslySelectedNodes)
	{
		...
		for (const TSharedPtr<FDatabaseAssetTreeNode>& Node : RootNodes)
		{
	...
	void SDatabaseAssetTree::OnCopySelectedNodesToClipboard() const
	{
		...
		if (!SelectedNodes.IsEmpty())
		{
					...
					if (SelectedNode->SourceAssetIdx != INDEX_NONE)
					{
						if (UPoseSearchDatabase* Database = ViewModel->GetPoseSearchDatabase())
						{
							if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAnimationAsset = Database->GetDatabaseAnimationAsset(SelectedNode->SourceAssetIdx))
							{
								...
								{
									UE_LOGF(LogPoseSearchEditor, Log, "Failed to copy %ls. Asset(s) with BranchIn notifies do not have clipboard support.", *DatabaseAnimationAsset->GetName())
								}
							}
						}
					}
			...
			{
				UE_LOGF(LogPoseSearchEditor, Warning, "Failed create clipboard object while attempting to copy data");
			}
		}
	}
	...
	void SDatabaseAssetTree::OnPasteNodesFromClipboard()
	{
		...
		{
			UE_LOGF(LogPoseSearchEditor, Warning, "Failed to get valid clipboard data while attempting to paste data");
		}
	}
	...
	void SDatabaseAssetTree::OnConvertToBranchIn()
	{
		...
		if (UPoseSearchDatabase* PoseSearchDatabase = ViewModel->GetPoseSearchDatabase())
		{
			...
			if (!SelectedNodes.IsEmpty())
			{
				...
				for (const TSharedPtr<FDatabaseAssetTreeNode>& SelectedNode : SelectedNodes)
				{
					if (const FPoseSearchDatabaseAnimationAsset* DatabaseAnimationAsset = PoseSearchDatabase->GetDatabaseAnimationAsset(SelectedNode->SourceAssetIdx))
					{
						if (UAnimSequenceBase* AnimSequenceBase = Cast<UAnimSequenceBase>(DatabaseAnimationAsset->GetAnimationAsset()))
						{
							if (!bModified)
							{
								PoseSearchDatabase->Modify();
							...
							const FFloatInterval SamplingRange = DatabaseAnimationAsset->GetEffectiveSamplingRange(FVector::ZeroVector);
							...
							NewDatabaseAnimationAsset.BranchInId = PoseSearchBranchIn->GetBranchInId();

							PoseSearchDatabase->SetAnimationAssetAt(NewDatabaseAnimationAsset, SelectedNode->SourceAssetIdx);
						}
					}
				}
	...
	void SDatabaseAssetTree::SetSelectedItem(int32 SourceAssetIdx, bool bClearSelection)
	{
		...
		if (SourceAssetIdx >= 0)
		{
			for (TSharedPtr<FDatabaseAssetTreeNode>& Node : RootNodes)
			{
