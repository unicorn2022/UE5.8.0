...
namespace UE::PoseSearch
{
	...
	static FText GetWarningsForDatabaseAsset(const FPoseSearchDatabaseAnimationAssetBase& InDatabaseAsset, const UPoseSearchDatabase& InDatabase)
	{
		...
		if (!InDatabaseAsset.IsSkeletonCompatible(InDatabase.Schema))
		{
		...
		if (!InDatabaseAsset.IsMirrorOptionCompatible(InDatabase.Schema))
		{
			return FText::Format(LOCTEXT("ErrorIncompatibleMirrorOption", "{0}'s MirrorOption is not compatible with the schema's MirrorDataTable(s)."), FText::FromString(InDatabaseAsset.GetName()));
		}
		
		if (const UBlendSpace* BlendSpace = Cast<UBlendSpace>(InDatabaseAsset.GetAnimationAsset()))
		{
		...
		if (const UObject* ThisAnimAsset = InDatabaseAsset.GetAnimationAsset())
		{
			const EPoseSearchMirrorOption ThisMirrorOption = InDatabaseAsset.GetMirrorOption();
			const FFloatInterval ThisSamplingRange = InDatabaseAsset.GetSamplingRange();
			const int32 NumAssets = InDatabase.GetNumAnimationAssets();
			for (int32 OtherIdx = 0; OtherIdx < NumAssets; ++OtherIdx)
			{
				const FPoseSearchDatabaseAnimationAsset* Other = InDatabase.GetDatabaseAnimationAsset(OtherIdx);
				if (Other 
					&& static_cast<const FPoseSearchDatabaseAnimationAssetBase*>(Other) != &InDatabaseAsset
					&& Other->GetAnimationAsset() == ThisAnimAsset
					&& Other->GetMirrorOption() == ThisMirrorOption
					&& Other->GetSamplingRange() == ThisSamplingRange)
				{
					return FText::Format(LOCTEXT("ErrorDuplicateAnimationAsset", "{0} duplicates another entry with the same animation asset, mirror option, and sampling range."), FText::FromString(InDatabaseAsset.GetName()));
				}
			}
		}
	...
	public:
		...
		{
			const TSharedPtr<FDatabaseAssetTreeNode> ThisNode = WeakAssetTreeNode.Pin();
			const TSharedPtr<FDatabaseViewModel> ViewModel = EditorViewModel.Pin();
			const TSharedPtr<SDatabaseAssetTree> SkeletonViewPtr = SkeletonView.Pin();

			if (ThisNode && ViewModel && SkeletonViewPtr && InMouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
			{
				TArray<TSharedPtr<FDatabaseAssetTreeNode>> SelectedNodes = SkeletonViewPtr->GetSelectedNodes();
				if (!SelectedNodes.Contains(ThisNode))
				{
					SelectedNodes = { ThisNode };
				}

				const FScopedTransaction Transaction(LOCTEXT("OnClickEditMirrorOptionPoseSearchDatabase", "Edit Mirror Option"));

				// Next mirror option is driven by the clicked item's current state
				...
				const EPoseSearchMirrorOption NextOption = OptionArray[(static_cast<int32>(ViewModel->GetMirrorOption(ThisNode->SourceAssetIdx)) + 1) % OptionArray.Num()];

				for (const TSharedPtr<FDatabaseAssetTreeNode>& Node : SelectedNodes)
				{
					if (Node->SourceAssetIdx != INDEX_NONE)
					{
						ViewModel->SetMirrorOption(Node->SourceAssetIdx, NextOption);
					}
				}

				SkeletonViewPtr->RefreshTreeView(false, true);
	...
	void SDatabaseAssetListItem::OnDisableReselectionChanged(ECheckBoxState NewCheckboxState)
	{
		const TSharedPtr<FDatabaseViewModel> ViewModelPtr = EditorViewModel.Pin();
		const TSharedPtr<SDatabaseAssetTree> SkeletonViewPtr = SkeletonView.Pin();
		if (!ViewModelPtr || !SkeletonViewPtr)
		{
			return;
		}
		...
		{
			TArray<TSharedPtr<FDatabaseAssetTreeNode>> SelectedNodes = SkeletonViewPtr->TreeView->GetSelectedItems();
			...
			const FScopedTransaction Transaction(LOCTEXT("DisableReselectionChangedForAssetInPoseSearchDatabase", "Update disable reselection flag for item from Pose Search Database"));
			...
			for (const TSharedPtr<FDatabaseAssetTreeNode>& Node : SelectedNodes)
			{
				if (Node->SourceAssetIdx != INDEX_NONE)
				{
					ViewModelPtr->SetDisableReselection(Node->SourceAssetIdx, NewCheckboxState == ECheckBoxState::Checked);
				}
			}

			PoseSearchDatabase->PostEditChange();
			SkeletonViewPtr->RefreshTreeView(false, true);
		}
	}
	...
	void SDatabaseAssetListItem::OnAssetIsEnabledChanged(ECheckBoxState NewCheckboxState)
	{
		...
		if (UPoseSearchDatabase* PoseSearchDatabase = ViewModelPtr->GetPoseSearchDatabase())
		{
			...
			// If the clicked item is not part of the selection, apply only to it
			const TSharedPtr<FDatabaseAssetTreeNode> ThisNode = WeakAssetTreeNode.Pin();
			if (!ThisNode)
			{
				return;
			...
			for (const TSharedPtr<FDatabaseAssetTreeNode>& Node : SelectedNodes)
			{
				if (Node->SourceAssetIdx != INDEX_NONE)
				{
					ViewModelPtr->SetIsEnabled(Node->SourceAssetIdx, NewCheckboxState == ECheckBoxState::Checked);
				}
			}
