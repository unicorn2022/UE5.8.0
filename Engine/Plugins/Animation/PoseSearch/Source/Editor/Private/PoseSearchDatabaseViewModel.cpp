...
namespace
{
	static USceneComponent* FindExistingPreviewAttachment(USceneComponent* Parent, UObject* Asset, FName AttachedTo)
	{
		if (!Parent || !Asset)
		{
			return nullptr;
		...
		{
			if (Child && Child->GetAttachSocketName() == AttachedTo &&
				FComponentAssetBrokerage::GetAssetFromComponent(Child) == Asset)
			{
				return Child;
			}
		}
	...
	static void AttachPreviewAssetToMesh(USkeletalMeshComponent* Mesh, UObject* Asset, FName AttachedTo)
	{
		if (!Mesh || !Asset || FindExistingPreviewAttachment(Mesh, Asset, AttachedTo))
		{
			return;
		}
		...
		USceneComponent* SceneComponent = NewObject<USceneComponent>(Mesh->GetOwner() ? (UObject*)Mesh->GetOwner() : (UObject*)Mesh, ComponentClass, NAME_None, RF_Transient);
		FComponentAssetBrokerage::AssignAssetToComponent(SceneComponent, Asset);

		if (UParticleSystemComponent* PSysComp = Cast<UParticleSystemComponent>(SceneComponent))
		{
			PSysComp->SetTickGroup(TG_PostUpdateWork);
		}

		SceneComponent->SetupAttachment(Mesh, AttachedTo);
		SceneComponent->RegisterComponent();
	}

	static void RefreshPreviewAttachedAssets(UDebugSkelMeshComponent* Mesh)
	{
		if (!Mesh)
		{
			return;
		...
		// Detach any prior preview attachments so a mesh swap doesn't leave stale ones behind.
		// Iterate backwards because DestroyComponent mutates the AttachChildren array.
		const TArray<TObjectPtr<USceneComponent>> ChildrenSnapshot = Mesh->GetAttachChildren();
		for (int32 Index = ChildrenSnapshot.Num() - 1; Index >= 0; --Index)
		{
			USceneComponent* Child = ChildrenSnapshot[Index];
			if (Child && FComponentAssetBrokerage::GetAssetFromComponent(Child))
			{
				Child->DestroyComponent();
			}
		}

		// Reapply attachments declared on the skeletal mesh asset...
		if (USkeletalMesh* SkeletalMesh = Mesh->GetSkeletalMeshAsset())
		{
			FPreviewAssetAttachContainer& Container = SkeletalMesh->GetPreviewAttachedAssetContainer();
			for (int32 Index = 0; Index < Container.Num(); ++Index)
			{
				const FPreviewAttachedObjectPair& Pair = Container[Index];
				AttachPreviewAssetToMesh(Mesh, Pair.GetAttachedObject(), Pair.AttachedTo);
			}
			...
			if (const USkeleton* Skeleton = SkeletalMesh->GetSkeleton())
			{
				...
				{
					const FPreviewAttachedObjectPair& Pair = Skeleton->PreviewAttachedAssetContainer[Index];
					AttachPreviewAssetToMesh(Mesh, Pair.GetAttachedObject(), Pair.AttachedTo);
				}
			}
		}
...
bool FDatabasePreviewActor::SpawnPreviewActor(UWorld* World, const UPoseSearchDatabase* PoseSearchDatabase, int32 IndexAssetIdx, const FRole& Role, const FTransform& SamplerRootTransformOrigin, int32 PoseIdxForTimeOffset)
{
	...
	if (!Skeleton)
	{
		UE_LOGF(LogPoseSearchEditor, Log, "Couldn't spawn preview Actor for asset %ls because its Role '%ls' is missing in Schema '%ls'", *GetNameSafe(DatabaseAnimationAsset->GetAnimationAsset()), *Role.ToString(), *PoseSearchDatabase->Schema->GetName());
		return false;
	}
	...
	RefreshPreviewAttachedAssets(Mesh);
	...
	Sampler.Init(PreviewAsset, SamplerRootTransformOrigin, IndexAsset.GetBlendParameters(), FAnimationAssetSampler::DefaultRootTransformSamplingRate, true, false);
...
bool FDatabasePreviewActor::DrawPreviewActors(TConstArrayView<FDatabasePreviewActor> PreviewActors, const UPoseSearchDatabase* PoseSearchDatabase, bool bDisplayRootMotionSpeed, bool bDisplayBlockTransition, bool bDisplayEventData, TConstArrayView<float> QueryVector)
{
	...
	FDebugDrawParams DrawParams(AnimContexts, PoseHistories, RoleToIndex, PoseSearchDatabase, FDebugDrawParams::EDrawContext::DrawQuery);
	...
	if (UPoseSearchDatabase* Database = GetPoseSearchDatabase())
	{
		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAnimationAssetBase = Database->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
			...
			if (DatabaseAnimationAssetBase->IsSynchronizedWithExternalDependency())
			{
				...
				{
					UE_LOGF(LogPoseSearchEditor, Error, "found DatabaseAnimationAssetBase with valid BranchInId, but invalid AnimSequenceBase in %ls", *Database->GetName());
				}
			}
...
void FDatabaseViewModel::SetDisableReselection(int32 AnimationAssetIndex, bool bEnabled)
{
	if (UPoseSearchDatabase* Database = GetPoseSearchDatabase())
	{
		if (const FPoseSearchDatabaseAnimationAsset* DatabaseAnimationAsset = Database->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
			Database->Modify();
			
			FPoseSearchDatabaseAnimationAsset NewDatabaseAnimationAsset = *DatabaseAnimationAsset;
			NewDatabaseAnimationAsset.SetDisableReselection(bEnabled);
			Database->SetAnimationAssetAt(NewDatabaseAnimationAsset, AnimationAssetIndex);
...
void FDatabaseViewModel::SetIsEnabled(int32 AnimationAssetIndex, bool bEnabled)
{
	if (UPoseSearchDatabase* Database = GetPoseSearchDatabase())
	{
		if (const FPoseSearchDatabaseAnimationAsset* DatabaseAnimationAsset = Database->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
			...
			FPoseSearchDatabaseAnimationAsset NewDatabaseAnimationAsset = *DatabaseAnimationAsset;
			NewDatabaseAnimationAsset.SetIsEnabled(bEnabled);
			Database->SetAnimationAssetAt(NewDatabaseAnimationAsset, AnimationAssetIndex);
	...
	if (AnimAsset)
	{
		if (UPoseSearchDatabase* Database = GetPoseSearchDatabase())
		{
			if (const FPoseSearchDatabaseAnimationAsset* DatabaseAnimationAsset = Database->GetDatabaseAnimationAsset(AnimationAssetIndex))
			{
				...
				FPoseSearchDatabaseAnimationAsset NewDatabaseAnimationAsset = *DatabaseAnimationAsset;
				NewDatabaseAnimationAsset.AnimAsset = AnimAsset;
				Database->SetAnimationAssetAt(NewDatabaseAnimationAsset, AnimationAssetIndex);
...
void FDatabaseViewModel::SetMirrorOption(int32 AnimationAssetIndex, EPoseSearchMirrorOption InMirrorOption)
{
	if (UPoseSearchDatabase* Database = GetPoseSearchDatabase())
	{
		if (const FPoseSearchDatabaseAnimationAsset* DatabaseAnimationAsset = Database->GetDatabaseAnimationAsset(AnimationAssetIndex))
		{
			...
			FPoseSearchDatabaseAnimationAsset NewDatabaseAnimationAsset = *DatabaseAnimationAsset;
			NewDatabaseAnimationAsset.MirrorOption = InMirrorOption;
			Database->SetAnimationAssetAt(NewDatabaseAnimationAsset, AnimationAssetIndex);
...
void FDatabaseViewModel::SetSelectedNodes(const TArrayView<TSharedPtr<FDatabaseAssetTreeNode>>& InSelectedNodes)
{
	...
	if (const UPoseSearchDatabase* Database = GetPoseSearchDatabase())
	{
		if (EAsyncBuildIndexResult::Success == FAsyncPoseSearchDatabasesManagement::RequestAsyncBuildIndex(Database, ERequestAsyncBuildFlag::ContinueRequest))
		{
			...
			const int32 MaxPreviewActors = UPoseSearchSettings::Get().MaxDatabaseEditorPreviewActors;
			int32 SpawnedActorCount = 0;
			bool bHitPreviewActorCap = false;

			const FSearchIndex& SearchIndex = Database->GetSearchIndex();
			for (int32 IndexAssetIndex = 0; IndexAssetIndex < SearchIndex.Assets.Num() && !bHitPreviewActorCap; ++IndexAssetIndex)
			{
				const FSearchIndexAsset& IndexAsset = SearchIndex.Assets[IndexAssetIndex];
				if (AssociatedAssetIndices.Find(IndexAsset.GetSourceAssetIdx()))
				{
					...
					for (int32 RoleIndex = 0; RoleIndex < DatabaseAnimationAsset->GetNumRoles(); ++RoleIndex)
					{
						if (SpawnedActorCount >= MaxPreviewActors)
						{
							bHitPreviewActorCap = true;
							break;
						}
			...
			if (bHitPreviewActorCap)
			{
				UE_LOG(LogPoseSearchEditor, Warning, TEXT("Pose Search Database editor preview actor cap reached (%d). Increase MaxDatabaseEditorPreviewActors in Project Settings -> Pose Search to preview more assets simultaneously."), MaxPreviewActors);
			}
		...
		if (EAsyncBuildIndexResult::Success == FAsyncPoseSearchDatabasesManagement::RequestAsyncBuildIndex(Database, ERequestAsyncBuildFlag::ContinueRequest))
		{
			...
			for (const FSearchIndexAsset& IndexAsset : SearchIndex.Assets)
			{
				if (IndexAsset.GetSourceAssetIdx() == SourceAssetIdx)
				{
					...
					if (const UObject* AnimationAsset = Database->GetDatabaseAnimationAsset(IndexAsset.GetSourceAssetIdx())->GetAnimationAsset())
					{
