...
namespace UE::PoseSearch
{
	...
	FDatabaseEditor::~FDatabaseEditor()
	{
		...
		{
			for (TWeakObjectPtr<UObject>& Reflection : SelectionWidget.SelectedReflections)
			{
		...
		if (CachedStatistics && UObjectInitialized())
		{
			CachedStatistics->RemoveFromRoot();
			CachedStatistics = nullptr;
		}
	...
	void FDatabaseEditor::OnFinishedChangingSelectionProperties(const FPropertyChangedEvent& PropertyChangedEvent)
	{
		if (UPoseSearchDatabase* Database = GetPoseSearchDatabase())
		{
			if (IsValid(Database))
			{
				...
				for (TWeakObjectPtr<UObject>& SelectedReflectionPtr : SelectionWidget.SelectedReflections)
				{
					if (UPoseSearchDatabaseAnimationAssetReflection* SelectionReflection = Cast<UPoseSearchDatabaseAnimationAssetReflection>(SelectedReflectionPtr.Pin().Get()))
					{
						// adding some edit constraints to SelectionReflection
						if (UObject* AnimationAsset = SelectionReflection->DatabaseAnimationAsset.GetAnimationAsset())
						{
							if (AnimationAsset->IsA<UAnimSequenceBase>())
							{
								const float PlayLength = SelectionReflection->DatabaseAnimationAsset.GetPlayLength(FVector::ZeroVector);
								const FFloatInterval ClampedSamplingRange(FMath::Clamp(SelectionReflection->DatabaseAnimationAsset.SamplingRange.Min, 0.0f, PlayLength), FMath::Clamp(SelectionReflection->DatabaseAnimationAsset.SamplingRange.Max, 0.0f, PlayLength));
								if (ClampedSamplingRange != SelectionReflection->DatabaseAnimationAsset.SamplingRange)
								{
									SelectionReflection->DatabaseAnimationAsset.SamplingRange = ClampedSamplingRange;
								}
							}
						}

						if (const FPoseSearchDatabaseAnimationAsset* DatabaseAnimationAssetPtr = Database->GetDatabaseAnimationAsset(SelectionReflection->SourceAssetIdx))
						{
							if (*DatabaseAnimationAssetPtr != SelectionReflection->DatabaseAnimationAsset)
							{
								if (!bModified)
								{
									Database->Modify();
								...
								Database->SetAnimationAssetAt(SelectionReflection->DatabaseAnimationAsset, SelectionReflection->SourceAssetIdx);
							}
						}
					}
				}
			...
			for (TSharedPtr<FDatabaseAssetTreeNode>& SelectedItem : SelectedItems)
			{
				if (SelectedItem.IsValid())
				{
					if (const FPoseSearchDatabaseAnimationAsset* DatabaseAnimationAsset = PoseSearchDatabase->GetDatabaseAnimationAsset(SelectedItem->SourceAssetIdx))
					{
						...
						NewSelectionReflection->SourceAssetIdx = SelectedItem->SourceAssetIdx;
	...
	void FDatabaseEditor::RefreshStatisticsWidgetInformation()
	{
		...
		CachedStatistics = NewObject<UPoseSearchDatabaseStatistics>();
		CachedStatistics->AddToRoot();
		CachedStatistics->Initialize(GetPoseSearchDatabase());
		StatisticsOverviewWidget->SetObject(CachedStatistics);
	}
