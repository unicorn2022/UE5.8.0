...
void FEditorModule::ShutdownModule()
{
	...
	if (DebuggerTrackCreator.Get() != nullptr)
	{
		IModularFeatures::Get().UnregisterModularFeature(FDebuggerTrackCreator::ModularFeatureName, DebuggerTrackCreator.Get());
	}

	if (TraceModule.Get() != nullptr)
	{
...
void FEditorModule::RegisterPropertyTypeCustomizations()
{
	...
	RegisterCustomPropertyTypeLayout("PoseSearchColumnRowReflection",     FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FPoseSearchColumnRowReflectionCustomization::MakeInstance));
}
