	...
	if (const TraceServices::IAnalysisSession* AnalysisSession = IRewindDebugger::Instance()->GetAnalysisSession())
	{
		if (const IGameplayProvider* GameplayProvider = AnalysisSession->ReadProvider<IGameplayProvider>("GameplayProvider"))
		{
			FString PathName;
			{
				...
				{
					PathName = DatabaseInfo->PathName;
				}
			}

			if (!PathName.IsEmpty())
			{
				Database = TSoftObjectPtr<UPoseSearchDatabase>(FSoftObjectPath(PathName)).LoadSynchronous();
			}
		}
	}
	return Database;
	...
	int32 DbPoseIdx, EPoseCandidateFlags PoseCandidateFlags, TConstArrayView<uint32> PoseToPCAValuesVectorIndexes, const FPoseSearchCost& Cost = FPoseSearchCost())
{
	...
	if (const FSearchIndexAsset* SearchIndexAsset = SearchIndex.GetAssetForPoseSafe(DbPoseIdx))
	{
		...
		{
			// @todo: gather ContinuingPoseCostAddend, ContinuingInteractionCostAddend, and ContinuingContextInteractionCostAddend properly to reconstruct the real FPoseSearchCost
			const float ContinuingPoseCostAddend = 0.f;
			const float ContinuingInteractionCostAddend = 0.f;
			const float ContinuingContextInteractionCostAddend = 0.f;
			Row->PoseCost = FPoseSearchCost(ArraySum(Row->CostVector, 0, Database->Schema->SchemaCardinality), SearchIndex.PoseMetadata[DbPoseIdx].GetCostAddend(), ContinuingPoseCostAddend, ContinuingInteractionCostAddend, ContinuingContextInteractionCostAddend);
		}
		...
		if (const FPoseSearchDatabaseAnimationAssetBase* DatabaseAsset = Database->GetDatabaseAnimationAsset(SearchIndexAsset->GetSourceAssetIdx()))
		{
...
void SDebuggerDatabaseView::Update(const FTraceMotionMatchingStateMessage& State)
{
	...
	InvalidDatabaseRows.Reset();
	InvalidDatabaseRows.Add(MakeShared<FDebuggerDatabaseRowData>(MakeShared<FDebuggerDatabaseSharedData>()));
	...
	if (bHasPCACostColumn != bAddPCACost || bWasVerbose != bIsVerbose || !CostBreakDownData.AreLabelsEqualTo(OldLabels))
	{
		...
		if (bIsVerbose)
		{
			...
			AddColumn(MakeShared<FContinuingContextInteractionCost>(ColumnIdx++));
		}
		else
		{
...
void SDebuggerDatabaseView::PopulateViewRows()
{
	...
	if (UnfilteredDatabaseRowsNum > 0)
	{
		...
		if (UPoseSearchDebuggerConfig::Get().bShowOnlyBestAssetPose)
		{
			...
			Algo::Sort(SortIndex, [this](int32 IndexA, int32 IndexB)
			{
				...
				{
					const bool bIsRowDataAValid = EnumHasAnyFlags(RowDataA.PoseCandidateFlags, EPoseCandidateFlags::AnyValidMask);
					const bool bIsRowDataBValid = EnumHasAnyFlags(RowDataB.PoseCandidateFlags, EPoseCandidateFlags::AnyValidMask);

					if (bIsRowDataAValid == bIsRowDataBValid)
					{
						return RowDataA.PoseCost < RowDataB.PoseCost;
					}
					return bIsRowDataAValid;
				}
	...
	{
		ActiveView.Rows.Add(InvalidDatabaseRows[0]);
	}

	if (ContinuingPoseView.Rows.IsEmpty())
	{
		ContinuingPoseView.Rows.Add(InvalidDatabaseRows[0]);
	}
}
			...
			[
				...
				[
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("ActivePoseLabel", "Active Pose"))
						]
					]
				]
			...
			[
				...
				[
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("ContinuingPoseLabel", "Continuing Pose"))
						]
					]
				]
			...
			[
				...
				[
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("PoseCandidatesLabel", "Pose Candidates"))
						]
					]
					.AutoWidth()
