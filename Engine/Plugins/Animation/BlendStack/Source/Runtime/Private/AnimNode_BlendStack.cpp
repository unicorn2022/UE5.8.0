...
void FBlendStackAnimPlayer::Initialize(const FAnimationInitializeContext& Context, UAnimationAsset* AnimationAsset, float AccumulatedTime, bool bLoop,
	bool bMirrored, UMirrorDataTable* MirrorDataTable, float BlendTime, const UBlendProfile* InBlendProfile, EAlphaBlendOption InBlendOption,
	const FVector& BlendParameters, float PlayRate, float ActivationDelay, int32 InPoseLinkIdx, FName GroupName, EAnimGroupRole::Type GroupRole, EAnimSyncMethod GroupMethod, bool bOverridePositionWhenJoiningSyncGroupAsLeader)
{
	...
	{
		UE_LOGF(LogBlendStack, Error, "FBlendStackAnimPlayer::Initialize failed for '%ls'. Mirroring will not work because MirrorDataTable is missing", *GetNameSafe(AnimationAsset));
	}
	...
	{
		UE_LOGF(LogBlendStack, Error, "FBlendStackAnimPlayer::Initialize failed for '%ls'. Skeleton has no bones?!", *GetNameSafe(AnimationAsset));
	}
	else if (BlendTime > UE_KINDA_SMALL_NUMBER)
	{
		// handling BlendTime > 0 and RootBoneBlendTime >= 0
		if (InBlendProfile != nullptr)
		{
			TotalBlendInTimePerBone.SetNumUninitialized(NumSkeletonBones);
			InBlendProfile->FillSkeletonBoneDurationsArray(TotalBlendInTimePerBone, BlendTime, Skeleton);
		}
	}
	...
	if (bUnsupportedAnimAsset)
	{
		...
		UE_LOGF(LogBlendStack, Error, "FBlendStackAnimPlayer::Initialize unsupported AnimationAsset %ls", *GetNameSafe(AnimationAsset));
	}
...
float FBlendStackAnimPlayer::GetSecondsFromEnd() const
{
	float TimeFromEndInSeconds = 0.f;

	if (SequencePlayerNode.GetSequence())
	{
		const float AccumulatedTime = SequencePlayerNode.GetAccumulatedTime();
		TimeFromEndInSeconds = SequencePlayerNode.GetTimeFromEnd(AccumulatedTime);
	}
	else if (const UBlendSpace* BlendSpace = BlendSpacePlayerNode.GetBlendSpace())
	{
		int32 TriangulationIndex = 0;
		TArray<FBlendSampleData> BlendSamples;
		const FVector BlendParameters = BlendSpacePlayerNode.GetPosition();
		...
		const float AccumulatedTime = BlendSpacePlayerNode.GetAccumulatedTime();
		TimeFromEndInSeconds = BlendSpacePlayerNode.GetTimeFromEnd(AccumulatedTime) * PlayLength;
	}

	return TimeFromEndInSeconds;
}
...
EAnimSyncMethod FBlendStackAnimPlayer::GetGroupMethod() const
{
	if (SequencePlayerNode.GetSequence())
	{
		return SequencePlayerNode.GetGroupMethod();
	}
	
	if (BlendSpacePlayerNode.GetBlendSpace())
	{
		return BlendSpacePlayerNode.GetGroupMethod();
	}

	return EAnimSyncMethod::DoNotSync;
}

FName FBlendStackAnimPlayer::GetGroupName() const
{
	if (SequencePlayerNode.GetSequence())
	{
		return SequencePlayerNode.GetGroupName();
	}
	
	if (BlendSpacePlayerNode.GetBlendSpace())
	{
		return BlendSpacePlayerNode.GetGroupName();
	}

	return FName();
}

EAnimGroupRole::Type FBlendStackAnimPlayer::GetGroupRole() const
{
	if (SequencePlayerNode.GetSequence())
	{
		return SequencePlayerNode.GetGroupRole();
	}
	
	if (BlendSpacePlayerNode.GetBlendSpace())
	{
		return BlendSpacePlayerNode.GetGroupRole();
	}

	return EAnimGroupRole::Type::CanBeLeader;
}

bool FBlendStackAnimPlayer::GetOverridePositionWhenJoiningSyncGroupAsLeader() const
{
	if (SequencePlayerNode.GetSequence())
	{
		return SequencePlayerNode.GetOverridePositionWhenJoiningSyncGroupAsLeader();
	}
	
	if (BlendSpacePlayerNode.GetBlendSpace())
	{
		return BlendSpacePlayerNode.GetOverridePositionWhenJoiningSyncGroupAsLeader();
	}

	return false;
}

const UBlendProfile* FBlendStackAnimPlayer::GetBlendProfile() const
{
	return BlendProfile.Get();
}

UMirrorDataTable* FBlendStackAnimPlayer::GetMirrorDataTable() const
{
	return MirrorNode.GetMirrorDataTable();
}
...
void FAnimNode_BlendStack_Standalone::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogBlendStack, Error, "FAnimNode_BlendStack_Standalone::Evaluate_AnyThread couldn't evaluate its last sample. Defaulting to RefPose");
			Output.ResetToRefPose();
		}
		...
		if (PlayerIndex == (MaxActiveBlends - 1))
		{
			...
			{
				UE_LOGF(LogBlendStack, Warning, "FAnimNode_BlendStack_Standalone::Evaluate_AnyThread dropping animplayer with blend in at %.2f", AnimPlayers.Last().GetBlendInPercentage());
			}
#endif // !NO_LOGGING
		}
...
void FAnimNode_BlendStack_Standalone::UpdateAssetPlayer(const FAnimationUpdateContext& Context)
{
	...
	// NoTe that it's safe to delete all those players, because we didn't call SamplePlayer.Update_AnyThread 
...
bool FAnimNode_BlendStack_Standalone::StitchTo(const FAnimationUpdateContext& Context, UAnimationAsset* AnimationAsset, float AccumulatedTime, bool bLoop,
	...
	const FVector& BlendParameters, float PlayRate, float ActivationDelay)
{
	...
	{
		UE_LOGF(LogBlendStack, Error, "FAnimNode_BlendStack_Standalone::BlendTo cannot use StitchDatabase '%ls', since MaxActiveBlends should be at least 1. Defaulting to regular blend", *GetNameSafe(StitchDatabase));
		return false;
	}
		
	IPoseSearchProvider* PoseSearchProvider = IPoseSearchProvider::Get();
	if (!PoseSearchProvider)
	{
		UE_LOGF(LogBlendStack, Error, "FAnimNode_BlendStack_Standalone::BlendTo cannot use StitchDatabase '%ls', because of missing IPoseSearchProvider (is PoseSearch plugin enabled?). Defaulting to regular blend", *GetNameSafe(StitchDatabase));
		return false;
	}
	...
	UAnimationAsset* StitchAnimationAsset = Cast<UAnimationAsset>(SearchResult.SelectedAsset);
	if (!StitchAnimationAsset)
	{
		UE_LOGF(LogBlendStack, Error, "FAnimNode_BlendStack_Standalone::BlendTo cannot use StitchDatabase '%ls', because of missing IPoseSearchProvider::Search couldn't select a StitchAnimationAsset. Defaulting to regular blend", *GetNameSafe(StitchDatabase));
		return false;
	}

	if (SearchResult.Dissimilarity > StitchBlendMaxCost)
	{
		UE_LOGF(LogBlendStack, Display, "FAnimNode_BlendStack_Standalone::BlendTo StitchDatabase '%ls' search cost is %f, above StitchBlendMaxCost %f. Defaulting to regular blend", *GetNameSafe(StitchDatabase), SearchResult.Dissimilarity, StitchBlendMaxCost);
		return false;
	}
	...
	InternalBlendTo(Context, StitchAnimationAsset, SearchResult.TimeOffsetSeconds, false, SearchResult.bMirrored, MirrorDataTable,
		StitchBlendTime, BlendProfile, BlendOption, bUseInertialBlend, InertialBlendNodeTag, BlendParameters, SearchResult.WantedPlayRate, ActivationDelay,
		NAME_None, EAnimGroupRole::CanBeLeader, EAnimSyncMethod::DoNotSync, false, NAME_None, EAnimGroupRole::CanBeLeader, EAnimSyncMethod::DoNotSync);
	...
	InternalBlendTo(Context, AnimationAsset, AccumulatedTime + BlendTime - StitchBlendTime, bLoop, bMirrored, MirrorDataTable,
		StitchBlendTime, BlendProfile, BlendOption, bUseInertialBlend, InertialBlendNodeTag, BlendParameters, PlayRate, BlendTime - StitchBlendTime + ActivationDelay,
		NAME_None, EAnimGroupRole::CanBeLeader, EAnimSyncMethod::DoNotSync, false, NAME_None, EAnimGroupRole::CanBeLeader, EAnimSyncMethod::DoNotSync);
...
void FAnimNode_BlendStack_Standalone::BlendTo(const FAnimationUpdateContext& Context, UAnimationAsset* AnimationAsset, float AccumulatedTime, bool bLoop,
	...
	FName PrevGroupName, EAnimGroupRole::Type PrevGroupRole, EAnimSyncMethod PrevMethod)
{
	if (!StitchTo(Context, AnimationAsset, AccumulatedTime, bLoop, bMirrored, MirrorDataTable, BlendTime, 
		BlendProfile, BlendOption, bUseInertialBlend, InertialBlendNodeTag, BlendParameters, PlayRate, ActivationDelay))
	{
		InternalBlendTo(Context, AnimationAsset, AccumulatedTime, bLoop, bMirrored, MirrorDataTable,
			BlendTime, BlendProfile, BlendOption, bUseInertialBlend, InertialBlendNodeTag, BlendParameters, PlayRate, ActivationDelay, 
			GroupName, GroupRole, GroupMethod, bOverridePositionWhenJoiningSyncGroupAsLeader, PrevGroupName, PrevGroupRole, PrevMethod);
	}
}
...
void FAnimNode_BlendStack_Standalone::InternalBlendTo(const FAnimationUpdateContext& Context, UAnimationAsset* AnimationAsset, float AccumulatedTime, bool bLoop,
	bool bMirrored, UMirrorDataTable* MirrorDataTable, float BlendTime, const UBlendProfile* BlendProfile, EAlphaBlendOption BlendOption, bool bUseInertialBlend, FName InertialBlendNodeTag,
	const FVector& BlendParameters, float PlayRate, float ActivationDelay, FName GroupName, EAnimGroupRole::Type GroupRole, EAnimSyncMethod GroupMethod, bool bOverridePositionWhenJoiningSyncGroupAsLeader,
	FName PrevGroupName, EAnimGroupRole::Type PrevGroupRole, EAnimSyncMethod PrevGroupMethod)
{
	...
	{
		// returns AnimationAsset EAdditiveAnimationType. AAT_MAX if AnimationAsset is a UBlendSpace with inconsistent EAdditiveAnimationType
		auto GetAdditiveAnimType = [](UAnimationAsset* AnimationAsset) -> EAdditiveAnimationType
			{
				EAdditiveAnimationType AdditiveAnimationType = AAT_None;
				if (const UAnimSequenceBase* AnimSequenceBase = Cast<UAnimSequenceBase>(AnimationAsset))
				{
					AdditiveAnimationType = AnimSequenceBase->GetAdditiveAnimType();
				}
				else if (const UBlendSpace* BlendSpace = Cast<UBlendSpace>(AnimationAsset))
				{
					AdditiveAnimationType = AAT_MAX;
					for (const FBlendSample& BlendSample : BlendSpace->GetBlendSamples())
					{
						if (AdditiveAnimationType == AAT_MAX)
						{
							AdditiveAnimationType = BlendSample.Animation->GetAdditiveAnimType();
						}
						else if (AdditiveAnimationType != BlendSample.Animation->GetAdditiveAnimType())
						{
							AdditiveAnimationType = AAT_MAX;
							break;
						}
				...
				return AdditiveAnimationType;
			};

		const EAdditiveAnimationType OldAdditiveAnimationType = GetAdditiveAnimType(AnimPlayers[0].GetAnimationAsset());
		const EAdditiveAnimationType NewAdditiveAnimationType = GetAdditiveAnimType(AnimationAsset);
		if (NewAdditiveAnimationType != OldAdditiveAnimationType)
		{
			UE_LOGF(LogBlendStack, Error, "FAnimNode_BlendStack_Standalone::InternalBlendTo AdditiveAnimType mismatch between input animation '%ls' and currently playing animation '%ls'", *GetNameSafe(AnimationAsset), *GetNameSafe(AnimPlayers[0].GetAnimationAsset()));
		}
	}
#endif // WITH_EDITOR
	...
	{
		// replacing AnimPlayers[0] with this new BlendTo request
		UE_LOGF(LogBlendStack, Verbose, "FAnimNode_BlendStack_Standalone::InternalBlendTo '%ls' replaced by '%ls' because blend time in is less than MaxBlendInTimeToOverrideAnimation (%.2f / %.2f)", *AnimPlayers[0].GetAnimationName(), *GetNameSafe(AnimationAsset), AnimPlayers[0].GetCurrentBlendInTime(), MaxBlendInTimeToOverrideAnimation);
	}
	else if (AnimPlayers.Num() <= MaxActiveBlends + 2)
	{
		...
		if (!bBlendStackIsEmpty)
		{
			const float SecondsFromEnd = AnimPlayers[0].GetSecondsFromEnd();
			if (SecondsFromEnd < BlendTime)
			{
				UE_LOGF(LogBlendStack, Verbose, "FAnimNode_BlendStack_Standalone::InternalBlendTo remaining playback time for '%ls' is only %f sec, while the requested blend time is %f sec. This will result in a poor quality blend, since the currently playing asset will end and freeze its pose, curves, and attributes before the new animation is fully relevant", *AnimPlayers[0].GetAnimationName(), SecondsFromEnd, BlendTime);
			}
		}
#endif // !NO_LOGGING
	...
	else
	{
		...
		{
			const float SecondsFromEnd = AnimPlayers[1].GetSecondsFromEnd();
			if (SecondsFromEnd < BlendTime)
			{
				UE_LOGF(LogBlendStack, Verbose, "FAnimNode_BlendStack_Standalone::InternalBlendTo remaining playback time for '%ls' is only %f sec, while the requested blend time is %f sec. This will result in a poor quality blend, since the currently playing asset will end and freeze its pose, curves, and attributes before the new animation is fully relevant", *AnimPlayers[0].GetAnimationName(), SecondsFromEnd, BlendTime);
			}
		...
		UE_LOGF(LogBlendStack, Warning, "FAnimNode_BlendStack_Standalone::InternalBlendTo multiple BlendTo requests during the same frame: only the last request will be put on this BlendStack");
	}

	if (AnimPlayers.Num() > 1)
	{
		FBlendStackAnimPlayer& PrevMainAnimPlayer = AnimPlayers[1];
		if (FAnimNode_AssetPlayerBase* PrevAssetPlayerNode = PrevMainAnimPlayer.GetAssetPlayerNode())
		{
			PrevAssetPlayerNode->SetGroupMethod(PrevGroupMethod);
			PrevAssetPlayerNode->SetGroupRole(PrevGroupRole);
			PrevAssetPlayerNode->SetGroupName(PrevGroupName);
		}

		// Clear up any sync group of other AnimPlayers
		for (int32 i = 2; i < AnimPlayers.Num(); ++i)
		{
			FBlendStackAnimPlayer& AnimPlayer = AnimPlayers[i];
	...
	FBlendStackAnimPlayer& MainAnimPlayer = AnimPlayers[0];

	FAnimationInitializeContext InitContext(Context.AnimInstanceProxy, Context.SharedContext);
	MainAnimPlayer.Initialize(InitContext, AnimationAsset, AccumulatedTime, bLoop, bMirrored, MirrorDataTable, BlendTime, BlendProfile, BlendOption, BlendParameters, PlayRate, ActivationDelay, NewSamplePoseLinkIndex, GroupName, GroupRole, GroupMethod, bOverridePositionWhenJoiningSyncGroupAsLeader);
	InitializeSample(InitContext, MainAnimPlayer);

	LastBlendToFrameCounter = GFrameCounter;
}
...
bool FAnimNode_BlendStack_Standalone::AnyNewBlendToThisFrame() const
{
	return LastBlendToFrameCounter == GFrameCounter;
}
...
void FAnimNode_BlendStack::UpdateAssetPlayer(const FAnimationUpdateContext& Context)
{
	...
	if (bForceBlendTo && !AnyNewBlendToThisFrame())
	{
		check(!AnimPlayers.IsEmpty());
		const FBlendStackAnimPlayer& MainAnimPlayer = AnimPlayers[0];

		// blending into the new main anim player with the new integrated time to avoid pops on this discontinuity
		InternalBlendTo(Context, MainAnimPlayer.GetAnimationAsset(), MainAnimPlayer.GetAccumulatedTime(), MainAnimPlayer.IsLooping(), MainAnimPlayer.GetMirror(),
			MainAnimPlayer.GetMirrorDataTable(), BlendTime,
			MainAnimPlayer.GetBlendProfile(), MainAnimPlayer.GetBlendOption(), bUseInertialBlend, 
			InertialBlendNodeTag, MainAnimPlayer.GetBlendParameters(), MainAnimPlayer.GetPlayRate(), 0.f,
			GetGroupName(), GetGroupRole(), GetGroupMethod(), GetOverridePositionWhenJoiningSyncGroupAsLeader(),
			GetPrevGroupName(), GetPrevGroupRole(), GetPrevGroupMethod());
	}
...
FName FAnimNode_BlendStack::GetPrevGroupName() const
{
	return GET_ANIM_NODE_DATA(FName, PrevGroupName);
}

EAnimGroupRole::Type FAnimNode_BlendStack::GetPrevGroupRole() const
{
	return GET_ANIM_NODE_DATA(TEnumAsByte<EAnimGroupRole::Type>, PrevGroupRole);
}

EAnimSyncMethod FAnimNode_BlendStack::GetPrevGroupMethod() const
{
	return GET_ANIM_NODE_DATA(EAnimSyncMethod, PrevMethod);
}
