...
public:
	...
	/** Returns the FDNAConfig the reader was constructed with, with all per-platform
	 *  fields resolved to scalar values for the host platform that built it.
	 *  Use this to compare host config against a cook-target config without going
	 *  through TPerPlatformProperty::GetValue() (which reads OnGetPreviewPlatform,
	 *  an FNotThreadSafeDelegate that the cook server may rebind concurrently).
	 */
	virtual const FDNAConfig& GetConfig() const = 0;

	/** Returns true if this reader is wrapped in the legacy adapter (FLegacyDNAReader),
	 *  which applies the legacy Maya->UE coordinate swizzle on geometry queries regardless
	 *  of FDNAConfig. Used by callers that must preserve UEMHC backwards-compatible behavior
	 *  (e.g. RestoreLegacyUEMHCCompatibility) to detect when re-wrapping is required.
	 */
	virtual bool IsLegacyWrapped() const { return false; }
...
public:
	...
	virtual ERotationSequence GetRotationSequence() const = 0;
	virtual FRotationSign GetRotationSign() const = 0;
	virtual EFaceWindingOrder GetFaceWindingOrder() const = 0;
...
{
protected:
	virtual ~IMachineLearnedBehaviorExtReader() = default;

public:
	virtual uint16 GetMLTypeCount() const = 0;
	virtual uint16 GetMLOperationSetCount(uint16 MLTypeIndex) const = 0;
	virtual uint16 GetMLOperationCount(uint16 MLTypeIndex, uint16 MLOperationSetIndex) const = 0;
	virtual EMachineLearnedBehaviorOperationType GetMLOperationType(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const = 0;
	virtual TArrayView<const uint32> GetMLOperationParameters(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const = 0;
	virtual TArrayView<const uint16> GetMLOperationDependencyOperationSetIndices(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const = 0;
	virtual TArrayView<const uint16> GetMLOperationDependencyOperationIndices(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const = 0;
	virtual uint16 GetMLOperationIndexListCount(uint16 MLTypeIndex, uint16 MLOperationSetIndex) const = 0;
	virtual TArrayView<const uint16> GetMLOperationIndicesForLOD(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 LOD) const = 0;
	virtual TArrayView<const uint16> GetMLJointsInputIndices() const = 0;
	virtual TArrayView<const uint16> GetMLJointsOutputIndices() const = 0;
	virtual TArrayView<const uint16> GetMLJointsParameterKeys() const = 0;
	virtual TArrayView<const uint16> GetMLJointsParameterValues() const = 0;
...
*/
class IDNAReader : public IRBFBehaviorReader, public IGeometryReader, public IMachineLearnedBehaviorExtReader, public IJointBehaviorMetadataReader, public ITwistSwingBehaviorReader
{
public:
