...
class UE_DEPRECATED(5.8, "FSkelMeshDNAReader is obsolete and will be removed. FDNAReader can be used instead. Delete any usages.") FSkelMeshDNAReader: public IDNAReader
{
public:
	...
	UE_API ERotationSequence GetRotationSequence() const override;
	UE_API FRotationSign GetRotationSign() const override;
	UE_API EFaceWindingOrder GetFaceWindingOrder() const override;
	...
	UE_API EMachineLearnedBehaviorOperationType GetMLOperationType(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const override;
	UE_API TArrayView<const uint32> GetMLOperationParameters(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const override;
	UE_API TArrayView<const uint16> GetMLOperationDependencyOperationSetIndices(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const override;
	UE_API TArrayView<const uint16> GetMLOperationDependencyOperationIndices(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const override;
	UE_API uint16 GetMLOperationIndexListCount(uint16 MLTypeIndex, uint16 MLOperationSetIndex) const override;
	UE_API TArrayView<const uint16> GetMLOperationIndicesForLOD(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 LOD) const override;
	UE_API TArrayView<const uint16> GetMLJointsInputIndices() const override;
	UE_API TArrayView<const uint16> GetMLJointsOutputIndices() const override;
	UE_API TArrayView<const uint16> GetMLJointsParameterKeys() const override;
	UE_API TArrayView<const uint16> GetMLJointsParameterValues() const override;

	UE_API void Unload(EDNADataLayer /*unused*/) override;

	UE_API const FDNAConfig& GetConfig() const override;

private:
	UE_API dna::Reader* Unwrap() const override { return nullptr; };

private:
	
	TSharedPtr<IDNAReader> DNAReader = nullptr;
};
