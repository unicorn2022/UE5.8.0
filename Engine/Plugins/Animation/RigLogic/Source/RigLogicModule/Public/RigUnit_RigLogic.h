...
/* A helper struct used inside of the RigUnit_RigLogic to store arrays of arrays of integers. */
USTRUCT(meta = (DocumentationPolicy = "Strict"))
struct FRigUnit_RigLogic_IntMatrix
{
	GENERATED_BODY()

	// The values stored within this array.
	UPROPERTY(transient)
	TArray<FRigUnit_RigLogic_IntArray> Values;
};
...
struct FRigUnit_RigLogic_Data
{
	...
	**/
	...
	UPROPERTY(transient)
	TArray<int32> MLMaskCurveIndices;
	...
	void MapMLMaskCurveIndices(const URigHierarchy* InHierarchy);
