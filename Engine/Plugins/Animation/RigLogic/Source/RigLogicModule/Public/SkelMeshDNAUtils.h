...
public:
	...
	//UE_DEPRECATED(5.8, "Use GetMeshDNAAsset instead which returns the new UDNA asset type")
	static UE_API UDNAAsset* GetMeshDNA(USkeletalMesh* InSkelMesh);

	/** Gets the DNA asset embedded in the mesh */
	static UE_API UDNA* GetMeshDNAAsset(USkeletalMesh* InSkelMesh);

	/** Gets DNA Reader either from old version of UDNAAsset or new UDNA, because the reader is widely used so having that in function can be useful*/
	static UE_API TSharedPtr<IDNAReader> GetDNAReader(USkeletalMesh* InSkelMesh);
	 ...
	 */
	static UE_API void SetDNAReader(USkeletalMesh* InSkelMesh, TSharedPtr<IDNAReader> InDNAReader,
