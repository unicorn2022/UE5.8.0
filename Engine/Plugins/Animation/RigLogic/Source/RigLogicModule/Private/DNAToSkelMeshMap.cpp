...
bool FDNAToSkelMeshMap::InitFromDNAAsset(USkeletalMesh* InSkelMesh)
{
	...
	if(UDNAAssetUserData* DNAAssetUserData = InSkelMesh->GetAssetUserData<UDNAAssetUserData>())
	{
		if (UDNA* DNA = DNAAssetUserData->DNAAsset)
		{
			InitBaseMesh(DNA->GetDNAReader().Get(), InSkelMesh);
			return true;
		}
		else
		{
			FString PathName = GetPathNameSafe(InSkelMesh);
			UE_LOGF(LogDNAToSkelMeshMap, Warning, "No DNA asset is referenced on DNAAssetUserData for %ls", *PathName);
			return false;
		}
	}
	else
	{
		if(UDNAAsset* DNAAsset = InSkelMesh->GetAssetUserData<UDNAAsset>())
		{
			InitBaseMesh(DNAAsset->GetDNAReader().Get(), InSkelMesh);
			return true;
		}
		else
		{
			FString PathName = GetPathNameSafe(InSkelMesh);
			UE_LOGF(LogDNAToSkelMeshMap, Warning, "Could not find DNAAsset user data for %ls", *PathName);
			return false;
		}
	}
...
void FDNAToSkelMeshMap::InitVertexMap(IDNAReader* InDNAReader)
{
	...
	for (int32 LODIndex = 0; LODIndex < NumEngineLods && LODIndex < NumLODs; LODIndex++)
	{
		...
		for (int32 LODMeshVtxIndex = 0; LODMeshVtxIndex < LODMeshVtxCount; LODMeshVtxIndex++)
		{
			...
			{
#ifdef DEBUG
				UE_LOGF(LogDNAToSkelMeshMap, Warning, "Not sorted fbx vertex found %d", FbxVertexIndex);
#endif
	...
	UE_LOGF(LogDNAToSkelMeshMap, Log, "	InitVertexMap:	%.6f", TimeToMap * 1000.0f);
}
...
void FDNAToSkelMeshMap::MapJoints(IDNAReader* InDNAReader)
{
	...
	UE_LOGF(LogDNAToSkelMeshMap, Log, "	Map joints:	%.6f", TimeToMap * 1000.0f);
}
...
void FDNAToSkelMeshMap::MapMorphTargets(IDNAReader* InDNAReader)
{
	...
	for (int16 MeshIndex = 0; MeshIndex < MeshCount; MeshIndex++)
	{
		...
		for (int16 TargetIndex = 0; TargetIndex < TargetCount; TargetIndex++)
		{
			...
			{
#ifdef DEBUG
				UE_LOGF(LogDNAToSkelMeshMap, Error, "Could not find morph target %ls (probably below threshold) Channel %d, Target %d", *MorphTargetStr, ChannelIndex, TargetIndex);
#endif
			}
		}
	}
	...
	//UE_LOGF(LogDNAToSkelMeshMap, Log, "	MapMorphTargets:	%.6f", TimeToMap * 1000.0f);
}
#endif // WITH_EDITORONLY_DATA