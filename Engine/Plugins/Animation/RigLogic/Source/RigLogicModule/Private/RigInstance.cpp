...
TArrayView<float> FRigInstance::GetGUIControlValues()
{
	rl4::ArrayView<float> Values = RigInstance->getGUIControlValues();
	return TArrayView<float>{Values.data(), static_cast<int32>(Values.size())};
}
...
TArrayView<float> FRigInstance::GetRawControlValues()
{
	rl4::ArrayView<float> Values = RigInstance->getRawControlValues();
	return TArrayView<float>{Values.data(), static_cast<int32>(Values.size())};
}
...
uint16 FRigInstance::GetMLTypeCount() const
{
	return RigInstance->getMLTypeCount();
}

uint16 FRigInstance::GetMLOperationSetCount(uint16 MLTypeIndex) const
{
	return RigInstance->getMLOperationSetCount(MLTypeIndex);
}

uint16 FRigInstance::GetMLOperationCount(uint16 MLTypeIndex, uint16 MLOperationSetIndex) const
{
	return RigInstance->getMLOperationCount(MLTypeIndex, MLOperationSetIndex);
}

TArrayView<float> FRigInstance::GetMLMaskValues()
{
	rl4::ArrayView<float> Values = RigInstance->getMLMaskValues();
	return TArrayView<float>{Values.data(), static_cast<int32>(Values.size())};
}

TArrayView<const float> FRigInstance::GetMLMaskValues() const
{
	rl4::ConstArrayView<float> Values = RigInstance->getMLMaskValues();
	return TArrayView<const float>{Values.data(), static_cast<int32>(Values.size())};
}
