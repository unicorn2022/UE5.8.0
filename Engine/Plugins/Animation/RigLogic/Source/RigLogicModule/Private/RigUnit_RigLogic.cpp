...
static FString ConstructCurveName(const FString& NameToSplit, const FString& FormatString)
{
	...
	if (!NameToSplit.Split(".", &ObjectName, &AttributeName))
	{
		...
		UE_LOGF(LogRigLogicUnit, Error, "RigUnit_R: Missing '.' in '%ls'",
			*NameToSplit);
		return TEXT("");
	}
...
FRigUnit_RigLogic_Data& FRigUnit_RigLogic_Data::operator=(const FRigUnit_RigLogic_Data& Other)
{
	...
	MLMaskCurveIndices = Other.MLMaskCurveIndices;
...
void FRigUnit_RigLogic_Data::InitializeRigLogic(const URigHierarchy* InHierarchy, TSharedPtr<FSharedRigRuntimeContext> NewContext)
{
	...
	{
		UE_LOGF(LogRigLogicUnit, Warning, "No valid DNA file found, abort initialization.");
		return;
	}
	...
	if (!RigInstance.IsValid())
	{
		...
		{
			MapMLMaskCurveIndices(InHierarchy);
		}
...
void FRigUnit_RigLogic_Data::MapInputCurveIndices(const URigHierarchy* InHierarchy)
{
	...
	const IDNAReader* DNABehavior = LocalRigRuntimeContext->DNAReader.Get();
...
void FRigUnit_RigLogic_Data::MapMLMaskCurveIndices(const URigHierarchy* InHierarchy)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	QUICK_SCOPE_CYCLE_COUNTER(STAT_RigUnit_RigLogic_MapMLMaskCurveIndices);

	const IDNAReader* DNABehavior = LocalRigRuntimeContext->DNAReader.Get();

	MLMaskCurveIndices.Empty();
	...
	for (uint16 MeshIndex = 0; MeshIndex < MeshCount; ++MeshIndex)
	{
		...
		MLMaskCurveIndices.Reserve(MLMaskCurveIndices.Num() + MeshRegionCount);
		for (uint16 RegionIndex = 0; RegionIndex < MeshRegionCount; ++RegionIndex)
		{
			...
			MLMaskCurveIndices.Add(CurveIndex);
		}
	}
}
...
void FRigUnit_RigLogic_Data::CalculateRigLogic(const URigHierarchy* InHierarchy, TArrayView<const float> NeutralJointValues)
{
	...
	if (RigLogicConfig.LoadMachineLearnedBehavior)
	{
		...
		for (int32 MaskIndex = 0; MaskIndex < MLMaskCurveIndices.Num(); ++MaskIndex)
		{
			const int32 CurveIndex = MLMaskCurveIndices[MaskIndex];
			if (InHierarchy->IsCurveValueSetByIndex(CurveIndex))
			{
				MaskBuffer[MaskIndex] = InHierarchy->GetCurveValue(CurveIndex);
			}
		}
	}
...
void FRigUnit_RigLogic_Data::UpdateBlendShapeCurves(URigHierarchy* InHierarchy, TArrayView<const float> BlendShapeValues)
{
	...
	{
		UE_LOGF(LogRigLogicUnit, Warning, "Invalid LOD Index for the BlendShapes. Ensure your curve is set up correctly!");
	}
}
...
void FRigUnit_RigLogic_Data::UpdateAnimMapCurves(URigHierarchy* InHierarchy, TArrayView<const float> AnimMapOutputs)
{
	...
	{
		UE_LOGF(LogRigLogicUnit, Warning, "Invalid LOD Index for the AnimationMaps. Ensure your curve is set up correctly!");
	}	
}

TSharedPtr<FSharedRigRuntimeContext> FRigUnit_RigLogic::GetSharedRigRuntimeContext(USkeletalMesh* SkelMesh)
{
	TSharedPtr<FSharedRigRuntimeContext> SharedRigRuntimeContext = nullptr;
	...
	if (UDNAAssetUserData* DNAAssetUserData = SkelMesh->GetAssetUserData<UDNAAssetUserData>())
	{
	...
	{
		if (UDNAAsset* DNAAsset = SkelMesh->GetAssetUserData<UDNAAsset>())
		{
			SharedRigRuntimeContext = DNAAsset->GetRigRuntimeContext();
		}
	}

	return SharedRigRuntimeContext;
}
...
FRigUnit_RigLogic_Execute()
{
	...
	if (Hierarchy)
	{
		if(!bIsInitialized)
		{
			...
			//UE_LOGF(LogRigLogicUnit, Warning, "RigLogic::Init execution time: %f", delta);
		}
		...
		//const double delta = FPlatformTime::Seconds() - startTime;
		//UE_LOGF(LogRigLogicUnit, Warning, "RigLogic::Update execution time: %f", delta);
	}
}
