...
#include <cstdlib>
    ...
    (defined(_GLIBCXX_HAVE_ALIGNED_ALLOC) || defined(_LIBCPP_HAS_ALIGNED_ALLOC) || defined(_LIBCPP_HAS_C11_FEATURES))
    #define ALIGNED_ALLOC(ptr, alignment, size)                                                                                  \
        ptr = std::aligned_alloc(alignment, (size % alignment == 0) ? size : (size + alignment - (size % alignment)))
