...
MachineLearnedBehaviorExtWriter::~MachineLearnedBehaviorExtWriter() = default;
...
static void copyDescriptor(const DescriptorReader* source, DescriptorWriter* destination, MemoryResource* /*unused*/) {
    ...
    destination->setRotationSequence(source->getRotationSequence());
    destination->setRotationSign(source->getRotationSign());
    destination->setFaceWindingOrder(source->getFaceWindingOrder());
...
static void copyDefinition(const DefinitionReader* source, DefinitionWriter* destination, MemoryResource* memRes) {
    ...
    copyLODIndices(std::bind(&DefinitionReader::getJointIndicesForLOD, source, _1),
                   ...
                   lodCount,
                   memRes);
    copyLODIndices(std::bind(&DefinitionReader::getBlendShapeChannelIndicesForLOD, source, _1),
                   ...
                   lodCount,
                   memRes);
    copyLODIndices(std::bind(&DefinitionReader::getAnimatedMapIndicesForLOD, source, _1),
                   ...
                   lodCount,
                   memRes);
    copyLODIndices(std::bind(&DefinitionReader::getMeshIndicesForLOD, source, _1),
    ...
    auto translations = collect_n<Vector3Vector>(
        jointCount,
        [source](std::size_t index) { return source->getNeutralJointTranslation(static_cast<std::uint16_t>(index)); },
        memRes);
    destination->setNeutralJointTranslations(translations.data(), static_cast<std::uint16_t>(translations.size()));

    auto rotations = collect_n<Vector3Vector>(
        jointCount,
        [source](std::size_t index) { return source->getNeutralJointRotation(static_cast<std::uint16_t>(index)); },
        memRes);
...
static void copyBehavior(const BehaviorReader* source, BehaviorWriter* destination, MemoryResource* /*unused*/) {
    ...
    for (std::uint16_t jointGroupIndexPlusOne = source->getJointGroupCount(); jointGroupIndexPlusOne > 0u;
        ...
        destination->setJointGroupInputIndices(jointGroupIndex,
                                               jointGroupInputIndices.data(),
        ...
        destination->setJointGroupOutputIndices(jointGroupIndex,
                                                jointGroupOutputIndices.data(),
                                                static_cast<std::uint16_t>(jointGroupOutputIndices.size()));

        auto jointGroupValues = source->getJointGroupValues(jointGroupIndex);
        destination->setJointGroupValues(jointGroupIndex,
                                         jointGroupValues.data(),
        ...
        destination->setJointGroupJointIndices(jointGroupIndex,
                                               jointGroupJointIndices.data(),
...
static void copyGeometry(const GeometryReader* source, GeometryWriter* destination, MemoryResource* memRes) {
    ...
    for (std::uint16_t meshIndexPlusOne = source->getMeshCount(); meshIndexPlusOne > 0u; --meshIndexPlusOne) {
        ...
        auto positions = collect_n<Vector3Vector>(
            vertexCount,
        ...
        auto textureCoordinates = collect_n<TextureCoordinateVector>(
            textureCoordinateCount,
            [source, meshIndex](std::size_t index) {
                return source->getVertexTextureCoordinate(meshIndex, static_cast<std::uint32_t>(index));
            },
            memRes);
        destination->setVertexTextureCoordinates(meshIndex,
        ...
        auto normals = collect_n<Vector3Vector>(
            normalCount,
            [source, meshIndex](std::size_t index) {
        ...
        auto layouts = collect_n<VertexLayoutVector>(
            layoutCount,
            [source, meshIndex](std::size_t index) {
        ...
        for (std::uint32_t faceIndexPlusOne = source->getFaceCount(meshIndex); faceIndexPlusOne > 0u; --faceIndexPlusOne) {
            ...
            destination->setFaceVertexLayoutIndices(meshIndex,
                                                    faceIndex,
                                                    faceVertices.data(),
        ...
        for (std::uint32_t skinWeightsIndexPlusOne = skinWeightsCount; skinWeightsIndexPlusOne > 0u; --skinWeightsIndexPlusOne) {
            ...
            destination->setSkinWeightsValues(meshIndex,
                                              skinWeightsIndex,
                                              skinWeights.data(),
            ...
            destination->setSkinWeightsJointIndices(meshIndex,
                                                    skinWeightsIndex,
                                                    skinWeightsJoints.data(),
...
static void copyBlendShapeTargets(const GeometryReader* source, GeometryWriter* destination, MemoryResource* memRes) {
    ...
    for (std::uint16_t meshIndexPlusOne = source->getMeshCount(); meshIndexPlusOne > 0u; --meshIndexPlusOne) {
        ...
        for (std::uint16_t blendShapeTargetIndexPlusOne = source->getBlendShapeTargetCount(meshIndex);
            ...
            auto deltas = collect_n<Vector3Vector>(
                deltaCount,
                [source, meshIndex, blendShapeTargetIndex](std::size_t index) {
                    return source->getBlendShapeTargetDelta(meshIndex, blendShapeTargetIndex, static_cast<std::uint32_t>(index));
                },
                memRes);
            destination->setBlendShapeTargetDeltas(meshIndex,
            ...
            destination->setBlendShapeTargetVertexIndices(meshIndex,
                                                          blendShapeTargetIndex,
                                                          deltasVertices.data(),
...
static void copyMachineLearnedBehavior(const MachineLearnedBehaviorReader* source,
    ...
    using namespace std::placeholders;
    copyLODIndices(std::bind(&MachineLearnedBehaviorReader::getNeuralNetworkIndicesForLOD, source, _1),
    ...
    for (std::uint16_t netIndexPlusOne = source->getNeuralNetworkCount(); netIndexPlusOne > 0u; --netIndexPlusOne) {
        ...
        destination->setNeuralNetworkInputIndices(netIndex, inputIndices.data(), static_cast<std::uint16_t>(inputIndices.size()));

        const auto outputIndices = source->getNeuralNetworkOutputIndices(netIndex);
        destination->setNeuralNetworkOutputIndices(netIndex,
                                                   outputIndices.data(),
        ...
        for (std::uint16_t layerIndexPlusOne = layerCount; layerIndexPlusOne > 0u; --layerIndexPlusOne) {
            ...
            const auto activationFunctionParams = source->getNeuralNetworkLayerActivationFunctionParameters(netIndex, layerIndex);
            destination->setNeuralNetworkLayerActivationFunctionParameters(
                netIndex,
                layerIndex,
                activationFunctionParams.data(),
                static_cast<std::uint16_t>(activationFunctionParams.size()));
            ...
            destination->setNeuralNetworkLayerWeights(netIndex,
                                                      layerIndex,
                                                      weights.data(),
...
static void copyRBFBehavior(const RBFBehaviorReader* source, RBFBehaviorWriter* destination, MemoryResource* memRes) {
    ...
    using namespace std::placeholders;
    copyLODIndices(std::bind(&RBFBehaviorReader::getRBFSolverIndicesForLOD, source, _1),
    ...
    for (std::uint16_t posePlusOne = source->getRBFPoseCount(); posePlusOne > 0u; --posePlusOne) {
        ...
        destination->setRBFPoseInputControlIndices(pi,
                                                   inputControlIndices.data(),
        ...
        destination->setRBFPoseOutputControlIndices(pi,
                                                    outputControlIndices.data(),
        ...
        destination->setRBFPoseOutputControlWeights(pi,
                                                    outputControlWeights.data(),
    ...
    for (std::uint16_t solverPlusOne = source->getRBFSolverCount(); solverPlusOne > 0u; --solverPlusOne) {
        ...
        destination->setRBFSolverRawControlIndices(si,
                                                   rawControlIndices.data(),
        ...
        destination->setRBFSolverRawControlValues(si,
                                                  rawControlValues.data(),
...
static void copyTwistSwingBehavior(const TwistSwingBehaviorReader* source,
                                   TwistSwingBehaviorWriter* destination,
                                   MemoryResource* /*unused*/) {
    ...
    for (std::uint16_t twistIndexPlusOne = source->getTwistCount(); twistIndexPlusOne > 0u; --twistIndexPlusOne) {
        ...
        destination->setTwistInputControlIndices(ti,
                                                 twistInputControlIndices.data(),
        ...
        destination->setTwistOutputJointIndices(ti,
                                                twistOutputJointIndices.data(),
                                                static_cast<std::uint16_t>(twistOutputJointIndices.size()));
        const auto twistBlendWeights = source->getTwistBlendWeights(ti);
        destination->setTwistBlendWeights(ti, twistBlendWeights.data(), static_cast<std::uint16_t>(twistBlendWeights.size()));
    }
    ...
    for (std::uint16_t swingIndexPlusOne = source->getSwingCount(); swingIndexPlusOne > 0u; --swingIndexPlusOne) {
        ...
        destination->setSwingInputControlIndices(si,
                                                 swingInputControlIndices.data(),
        ...
        destination->setSwingOutputJointIndices(si,
                                                swingOutputJointIndices.data(),
...
static bool hasMachineLearnedBehaviorExt(const MachineLearnedBehaviorExtReader* source) {
    // Heuristic for determining whether source DNA actually has any ML behavior ext data
    const auto mlTypeCount = source->getMLTypeCount();
    const auto neuralNetCount = source->getNeuralNetworkCount();

    // Check if this is the old ML layer accessed through the new API
    if (mlTypeCount == 1u) {
        const auto mlOperationSetCount = source->getMLOperationSetCount(0u);
        if (mlOperationSetCount == 3u) {
            const auto mlOperationCount1 = source->getMLOperationCount(0u, 0u);
            const auto mlOperationCount2 = source->getMLOperationCount(0u, 1u);
            const auto mlOperationCount3 = source->getMLOperationCount(0u, 2u);
            if ((mlOperationCount1 == neuralNetCount) && (mlOperationCount2 == neuralNetCount) &&
                (mlOperationCount3 == neuralNetCount)) {
                return false;
            }
        }
    }

    return (mlTypeCount > 0u);
}

static void copyMachineLearnedBehaviorExt(const MachineLearnedBehaviorExtReader* source,
                                          MachineLearnedBehaviorExtWriter* destination,
                                          MemoryResource* memRes) {
    destination->clearMLExtData();
    if (!hasMachineLearnedBehaviorExt(source)) {
        // Source DNA was loaded without machine learned behavior extension layer
        return;
    }

    using namespace std::placeholders;
    for (std::uint16_t mlTypeIndex = 0; mlTypeIndex < source->getMLTypeCount(); ++mlTypeIndex) {
        for (std::uint16_t i = source->getMLOperationSetCount(mlTypeIndex); i > 0u; --i) {
            const auto opSetIdx = static_cast<std::uint16_t>(i - 1u);

            copyLODIndices(
                std::bind(&MachineLearnedBehaviorExtReader::getMLOperationIndicesForLOD, source, mlTypeIndex, opSetIdx, _1),
                std::bind(&MachineLearnedBehaviorExtWriter::setMLOperationIndices,
                          destination,
                          mlTypeIndex,
                          opSetIdx,
                          _1,
                          _2,
                          _3),
                std::bind(&MachineLearnedBehaviorExtWriter::setLODMLOperationMapping, destination, mlTypeIndex, opSetIdx, _1, _2),
                source->getLODCount(),
                memRes);

            for (std::uint16_t j = source->getMLOperationCount(mlTypeIndex, opSetIdx); j > 0u; --j) {
                const auto opIdx = static_cast<std::uint16_t>(j - 1u);
                destination->setMLOperationType(mlTypeIndex,
                                                opSetIdx,
                                                opIdx,
                                                source->getMLOperationType(mlTypeIndex, opSetIdx, opIdx));

                const auto parameters = source->getMLOperationParameters(mlTypeIndex, opSetIdx, opIdx);
                destination->setMLOperationParameters(mlTypeIndex,
                                                      opSetIdx,
                                                      opIdx,
                                                      parameters.data(),
                                                      static_cast<std::uint16_t>(parameters.size()));

                const auto depOpSetIndices = source->getMLOperationDependencyOperationSetIndices(mlTypeIndex, opSetIdx, opIdx);
                destination->setMLOperationDependencyOperationSetIndices(mlTypeIndex,
                                                                         opSetIdx,
                                                                         opIdx,
                                                                         depOpSetIndices.data(),
                                                                         static_cast<std::uint16_t>(depOpSetIndices.size()));

                const auto depOpIndices = source->getMLOperationDependencyOperationIndices(mlTypeIndex, opSetIdx, opIdx);
                destination->setMLOperationDependencyOperationIndices(mlTypeIndex,
                                                                      opSetIdx,
                                                                      opIdx,
                                                                      depOpIndices.data(),
                                                                      static_cast<std::uint16_t>(depOpIndices.size()));
            }
        }
    }

    const auto parameterKeys = source->getMLJointsParameterKeys();
    destination->setMLJointsParameterKeys(parameterKeys.data(), static_cast<std::uint16_t>(parameterKeys.size()));

    const auto parameterValues = source->getMLJointsParameterValues();
    destination->setMLJointsParameterValues(parameterValues.data(), static_cast<std::uint16_t>(parameterValues.size()));

    const auto inputIndices = source->getMLJointsInputIndices();
    destination->setMLJointsInputIndices(inputIndices.data(), static_cast<std::uint16_t>(inputIndices.size()));

    const auto outputIndices = source->getMLJointsOutputIndices();
    destination->setMLJointsOutputIndices(outputIndices.data(), static_cast<std::uint16_t>(outputIndices.size()));
}
...
void Writer::setFrom(const Reader* source, DataLayer layer, UnknownLayerPolicy /*unused*/, MemoryResource* memRes) {
    ...
    if (contains(bitmask, DataLayerBitmask::MachineLearnedBehavior)) {
        ...
        copyMachineLearnedBehaviorExt(source, this, memRes);
    }
