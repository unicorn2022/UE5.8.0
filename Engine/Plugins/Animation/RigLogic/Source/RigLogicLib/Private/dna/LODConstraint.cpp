...
LODConstraint::LODConstraint(std::uint16_t maxLOD, std::uint16_t minLOD, MemoryResource* memRes) :
    lods{memRes} {
...
LODConstraint::LODConstraint(ConstArrayView<std::uint16_t> lods_, MemoryResource* memRes) :
    lods{lods_.begin(), lods_.end(), memRes} {
...
void LODConstraint::clampTo(std::uint16_t lodCount) {
    extd::filter(lods, [lodCount](std::uint16_t lod, std::size_t /*unused*/) { return lod < lodCount; });
}
