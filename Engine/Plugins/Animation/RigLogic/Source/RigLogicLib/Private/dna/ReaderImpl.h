...
public:
    ...
    RotationSequence getRotationSequence() const override;
    RotationSign getRotationSign() const override;
    FaceWindingOrder getFaceWindingOrder() const override;
    ...
    Delta getBlendShapeTargetDelta(std::uint16_t meshIndex,
    ...
    ActivationFunction getNeuralNetworkLayerActivationFunction(std::uint16_t netIndex, std::uint16_t layerIndex) const override;
...
#endif
template<class TReaderBase>
inline ReaderImpl<TReaderBase>::ReaderImpl(MemoryResource* memRes_) :
    BaseImpl{memRes_},
    cache{memRes_} {
}
...
inline CoordinateSystem ReaderImpl<TReaderBase>::getCoordinateSystem() const {
    return {static_cast<Direction>(dna.descriptor.coordinateSystem.xAxis),
            static_cast<Direction>(dna.descriptor.coordinateSystem.yAxis),
            static_cast<Direction>(dna.descriptor.coordinateSystem.zAxis)};
}

template<class TReaderBase>
inline RotationSequence ReaderImpl<TReaderBase>::getRotationSequence() const {
    return dna.descriptorExt.rotationSequence;
}

template<class TReaderBase>
inline RotationSign ReaderImpl<TReaderBase>::getRotationSign() const {
    return dna.descriptorExt.rotationSign;
}

template<class TReaderBase>
inline FaceWindingOrder ReaderImpl<TReaderBase>::getFaceWindingOrder() const {
    return dna.descriptorExt.faceWindingOrder;
}
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t>
ReaderImpl<TReaderBase>::getMeshBlendShapeChannelMappingIndicesForLOD(std::uint16_t lod) const {
...
inline ConstArrayView<float> ReaderImpl<TReaderBase>::getVertexTextureCoordinateUs(std::uint16_t meshIndex) const {
    if (meshIndex < dna.geometry.meshes.size()) {
        return dna.geometry.meshes[meshIndex].textureCoordinates.us;
    }
    return {};
}
...
inline ConstArrayView<float> ReaderImpl<TReaderBase>::getVertexTextureCoordinateVs(std::uint16_t meshIndex) const {
    if (meshIndex < dna.geometry.meshes.size()) {
        return dna.geometry.meshes[meshIndex].textureCoordinates.vs;
    }
    return {};
}
...
template<class TReaderBase>
inline ConstArrayView<std::uint32_t>
ReaderImpl<TReaderBase>::getVertexLayoutTextureCoordinateIndices(std::uint16_t meshIndex) const {
    if (meshIndex < dna.geometry.meshes.size()) {
...
template<class TReaderBase>
inline ConstArrayView<std::uint32_t>
ReaderImpl<TReaderBase>::getBlendShapeTargetVertexIndices(std::uint16_t meshIndex, std::uint16_t blendShapeTargetIndex) const {
    const auto& meshes = dna.geometry.meshes;
    if ((meshIndex < meshes.size()) && (blendShapeTargetIndex < meshes[meshIndex].blendShapeTargets.size())) {
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t>
ReaderImpl<TReaderBase>::getNeuralNetworkIndicesForMeshRegion(std::uint16_t meshIndex, std::uint16_t regionIndex) const {
...
template<class TReaderBase>
inline ConstArrayView<float>
ReaderImpl<TReaderBase>::getNeuralNetworkLayerActivationFunctionParameters(std::uint16_t netIndex,
                                                                           std::uint16_t layerIndex) const {
    const auto& neuralNets = dna.machineLearnedBehavior.neuralNetworks;
    if ((netIndex < neuralNets.size()) && (layerIndex < neuralNets[netIndex].layers.size())) {
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t>
ReaderImpl<TReaderBase>::getRBFPoseBlendShapeChannelOutputIndices(std::uint16_t poseIndex) const {
    const auto& poses = dna.rbfBehavior.poses;
    if (poseIndex < poses.size()) {
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t> ReaderImpl<TReaderBase>::getRBFPoseAnimatedMapOutputIndices(std::uint16_t poseIndex) const {
    const auto& poses = dna.rbfBehavior.poses;
    if (poseIndex < poses.size()) {
...
template<class TReaderBase>
inline StringView ReaderImpl<TReaderBase>::getRBFPoseControlName(std::uint16_t poseControlIndex) const {
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t> ReaderImpl<TReaderBase>::getRBFSolverRawControlIndices(std::uint16_t solverIndex) const {
    if (dna.rbfBehavior.solvers.size() > solverIndex) {
...
template<class TReaderBase>
inline bool ReaderImpl<TReaderBase>::isMLExtLoaded() const {
    return !dna.machineLearnedBehaviorExt.mlbTypeData.empty();
}

template<class TReaderBase>
inline std::uint16_t ReaderImpl<TReaderBase>::getMLTypeCount() const {
    if (!isMLExtLoaded()) {
        return dna.machineLearnedBehavior.neuralNetworks.empty() ? 0u : 1u;
    }
    return static_cast<std::uint16_t>(dna.machineLearnedBehaviorExt.mlbTypeData.size());
}

template<class TReaderBase>
inline std::uint16_t ReaderImpl<TReaderBase>::getMLOperationSetCount(std::uint16_t mlTypeIndex) const {
    if (!isMLExtLoaded()) {
        return dna.machineLearnedBehavior.neuralNetworks.empty() ? 0u : 3u;
    }
    if (dna.machineLearnedBehaviorExt.mlbTypeData.size() > mlTypeIndex) {
        return static_cast<std::uint16_t>(dna.machineLearnedBehaviorExt.mlbTypeData[mlTypeIndex].operations.size());
    }
    return {};
}

template<class TReaderBase>
inline std::uint16_t ReaderImpl<TReaderBase>::getMLOperationCount(std::uint16_t mlTypeIndex,
                                                                  std::uint16_t mlOperationSetIndex) const {
    if (!isMLExtLoaded()) {
        return getNeuralNetworkCount();
    }
    if (mlTypeIndex < dna.machineLearnedBehaviorExt.mlbTypeData.size()) {
        const auto& mlbType = dna.machineLearnedBehaviorExt.mlbTypeData[mlTypeIndex];
        if (mlOperationSetIndex < mlbType.operations.size()) {
            return static_cast<std::uint16_t>(mlbType.operations[mlOperationSetIndex].size());
        }
    }
    return {};
}
...
template<class TReaderBase>
inline MachineLearnedBehaviorOperationType ReaderImpl<TReaderBase>::getMLOperationType(std::uint16_t mlTypeIndex,
                                                                                       std::uint16_t mlOperationSetIndex,
                                                                                       std::uint16_t mlOperationIndex) const {
    if (!isMLExtLoaded()) {
        static const MachineLearnedBehaviorOperationType types[] = {MachineLearnedBehaviorOperationType::Gather,
                                                                    MachineLearnedBehaviorOperationType::MLP,
                                                                    MachineLearnedBehaviorOperationType::Scatter};
        if (mlOperationSetIndex < getMLOperationSetCount(mlTypeIndex)) {
    ...
    if (mlTypeIndex < dna.machineLearnedBehaviorExt.mlbTypeData.size()) {
        const auto& mlbType = dna.machineLearnedBehaviorExt.mlbTypeData[mlTypeIndex];
        if (mlOperationSetIndex < mlbType.operations.size()) {
            const auto& mlOperations = mlbType.operations[mlOperationSetIndex];
            if (mlOperationIndex < mlOperations.size()) {
                return static_cast<MachineLearnedBehaviorOperationType>(mlOperations[mlOperationIndex].operationType);
            }
        }
    }
    return {};
...
template<class TReaderBase>
inline ConstArrayView<std::uint32_t> ReaderImpl<TReaderBase>::getMLOperationParameters(std::uint16_t mlTypeIndex,
                                                                                       std::uint16_t mlOperationSetIndex,
                                                                                       std::uint16_t mlOperationIndex) const {
    if (!isMLExtLoaded()) {
        if (cache.mlOperationParameters.empty()) {
            cache.populateMLOperationParameters(this);
        }
        if (mlOperationSetIndex < cache.mlOperationParameters.size()) {
            if (mlOperationIndex < cache.mlOperationParameters[mlOperationSetIndex].size()) {
                return cache.mlOperationParameters[mlOperationSetIndex][mlOperationIndex];
            }
        }
        return {};
    }
    if (mlTypeIndex < dna.machineLearnedBehaviorExt.mlbTypeData.size()) {
        ...
        if (mlOperationSetIndex < mlbType.operations.size()) {
            const auto& mlOperations = mlbType.operations[mlOperationSetIndex];
            if (mlOperationIndex < mlOperations.size()) {
                return mlOperations[mlOperationIndex].parameters;
            }
        }
    }
    return {};
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t>
ReaderImpl<TReaderBase>::getMLOperationDependencyOperationSetIndices(std::uint16_t mlTypeIndex,
                                                                     std::uint16_t mlOperationSetIndex,
                                                                     std::uint16_t mlOperationIndex) const {
    if (!isMLExtLoaded()) {
        if (cache.mlDependencyOperationSetIndices.empty()) {
            cache.populateMLDependencyOperationSetIndices(this);
        }
        if (mlOperationSetIndex < cache.mlDependencyOperationSetIndices.size()) {
            if (mlOperationIndex < cache.mlDependencyOperationSetIndices[mlOperationSetIndex].size()) {
                return cache.mlDependencyOperationSetIndices[mlOperationSetIndex][mlOperationIndex];
            }
        }
        return {};
    }
    if (mlTypeIndex < dna.machineLearnedBehaviorExt.mlbTypeData.size()) {
        ...
        if (mlOperationSetIndex < mlbType.operations.size()) {
            const auto& mlOperations = mlbType.operations[mlOperationSetIndex];
            if (mlOperationIndex < mlOperations.size()) {
                return mlOperations[mlOperationIndex].mlOperationSetDependencyIndices;
            }
        }
    }
    return {};
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t>
ReaderImpl<TReaderBase>::getMLOperationDependencyOperationIndices(std::uint16_t mlTypeIndex,
                                                                  std::uint16_t mlOperationSetIndex,
                                                                  std::uint16_t mlOperationIndex) const {
    if (!isMLExtLoaded()) {
        if (cache.mlDependencyOperationIndices.empty()) {
            cache.populateMLDependencyOperationIndices(this);
        }
        if (mlOperationSetIndex < cache.mlDependencyOperationIndices.size()) {
            if (mlOperationIndex < cache.mlDependencyOperationIndices[mlOperationSetIndex].size()) {
                return cache.mlDependencyOperationIndices[mlOperationSetIndex][mlOperationIndex];
            }
        }
        return {};
    }
    if (mlTypeIndex < dna.machineLearnedBehaviorExt.mlbTypeData.size()) {
        ...
        if (mlOperationSetIndex < mlbType.operations.size()) {
            const auto& mlOperations = mlbType.operations[mlOperationSetIndex];
            if (mlOperationIndex < mlOperations.size()) {
                return mlOperations[mlOperationIndex].mlOperationDependencyIndices;
            }
        }
    }
    return {};
...
template<class TReaderBase>
inline std::uint16_t ReaderImpl<TReaderBase>::getMLOperationIndexListCount(std::uint16_t mlTypeIndex,
                                                                           std::uint16_t mlOperationSetIndex) const {
    if (!isMLExtLoaded()) {
        return getNeuralNetworkIndexListCount();
    }

    if (mlTypeIndex < dna.machineLearnedBehaviorExt.mlbTypeData.size()) {
        const auto& mlbType = dna.machineLearnedBehaviorExt.mlbTypeData[mlTypeIndex];
        if (mlOperationSetIndex < mlbType.lodMLOperationMappings.size()) {
            return mlbType.lodMLOperationMappings[mlOperationSetIndex].getIndexListCount();
        }
    }
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t> ReaderImpl<TReaderBase>::getMLOperationIndicesForLOD(std::uint16_t mlTypeIndex,
                                                                                          std::uint16_t mlOperationSetIndex,
                                                                                          std::uint16_t lod) const {
    if (!isMLExtLoaded()) {
        return getNeuralNetworkIndicesForLOD(lod);
    }

    if (mlTypeIndex < dna.machineLearnedBehaviorExt.mlbTypeData.size()) {
        const auto& mlbType = dna.machineLearnedBehaviorExt.mlbTypeData[mlTypeIndex];
        if (mlOperationSetIndex < mlbType.lodMLOperationMappings.size()) {
            return mlbType.lodMLOperationMappings[mlOperationSetIndex].getIndices(lod);
        }
    }
...
template<class TReaderBase>
inline ConstArrayView<std::uint16_t> ReaderImpl<TReaderBase>::getMLJointsInputIndices() const {
    if (!isMLExtLoaded()) {
        return {};
    }
    return dna.machineLearnedBehaviorExt.mlbJoints.inputIndices;
}

template<class TReaderBase>
inline ConstArrayView<std::uint16_t> ReaderImpl<TReaderBase>::getMLJointsOutputIndices() const {
    if (!isMLExtLoaded()) {
        return {};
    }
    return dna.machineLearnedBehaviorExt.mlbJoints.outputIndices;
}

template<class TReaderBase>
inline ConstArrayView<std::uint16_t> ReaderImpl<TReaderBase>::getMLJointsParameterKeys() const {
    if (!isMLExtLoaded()) {
        return {};
    }
    return dna.machineLearnedBehaviorExt.mlbJoints.parameterKeys;
}

template<class TReaderBase>
inline ConstArrayView<std::uint16_t> ReaderImpl<TReaderBase>::getMLJointsParameterValues() const {
    if (!isMLExtLoaded()) {
        return {};
    }
    return dna.machineLearnedBehaviorExt.mlbJoints.parameterValues;
}
