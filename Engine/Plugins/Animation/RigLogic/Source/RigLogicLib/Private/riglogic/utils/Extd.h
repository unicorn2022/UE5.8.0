...
inline void filter(std::vector<T, Args...>& source, Predicate pred) {
    source.erase(std::remove_if(std::begin(source),
                                std::end(source),
                                [&source, &pred](const T& value) {
...
class LUTFilter {
public:
    explicit LUTFilter(const TLUT& lut_) :
        lut{lut_} {
    }

    template<typename T, LUTStrategy ChosenStrategy = Strategy>
    typename std::enable_if<ChosenStrategy == LUTStrategy::ByValue, bool>::type operator()(const T& value,
                                                                                           std::size_t /*unused*/) {
        return contains(lut, value);
    }

    template<typename T, LUTStrategy ChosenStrategy = Strategy>
    typename std::enable_if<ChosenStrategy == LUTStrategy::ByPosition, bool>::type operator()(const T& /*unused*/,
                                                                                              std::size_t index) {
...
inline typename TContainer::value_type minOf(const TContainer& container) {
    using ValueType = typename TContainer::value_type;
    const auto compare = [](const ValueType& lhs, const ValueType& rhs) { return lhs < rhs; };
...
inline void copy(av::ArrayView<T> source, av::ArrayView<U> destination) {
    #if defined(_MSC_VER) && !defined(__clang__) && (_MSC_VER < 1938)
        #if (_MSC_VER >= 1900) && (__cplusplus >= 202002L)
    std::copy(source.begin(), source.end(), std::span{destination.data(), destination.size()}.begin());
        #else
    if ((source.data() != nullptr) || (destination.data() != nullptr)) {
        std::copy(source.begin(), source.end(), stdext::make_checked_array_iterator(destination.data(), destination.size()));
    }
        #endif
    #else
