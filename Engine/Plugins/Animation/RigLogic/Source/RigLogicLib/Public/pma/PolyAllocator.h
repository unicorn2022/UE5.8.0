...
public:
    ...
    PolyAllocator(MemoryResource* memRes) :
        PolyAllocator{} {
        if (memRes != nullptr) {
            pMemRes = memRes;
        }
    ...
    PolyAllocator(std::nullptr_t /*unused*/) :
        PolyAllocator{} {
    }

    template<class U, std::size_t UAlignment, class UDefaultMemoryResource>
    PolyAllocator(const PolyAllocator<U, UAlignment, UDefaultMemoryResource>& rhs) :
        pMemRes{rhs.pMemRes} {
    }
...
public:
    ...
    template<class U, std::size_t UAlignment, class UDefaultMemoryResource>
    PolyAllocator(const impl::PolyAllocator<U, UAlignment, UDefaultMemoryResource>& rhs) :
        Base{rhs} {
    }
};
...
template<typename T,
         ...
         class TDefaultMemoryResource,
         typename U,
         std::size_t UAlignment,
         class UDefaultMemoryResource>
bool operator==(const PolyAllocator<T, TAlignment, TDefaultMemoryResource>& lhs,
