...
struct F128 {
    ...
    explicit F128(__m128 value) :
        data{value} {
    }

    explicit F128(float value) :
        F128{_mm_set1_ps(value)} {
    }
...
TRIMD_TARGET_F16C inline F128 F128::fromAlignedSource(const std::uint16_t* source) {
    return F128{_mm_cvtph_ps(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(source)))};
}

TRIMD_TARGET_F16C inline F128 F128::fromUnalignedSource(const std::uint16_t* source) {
    return F128{_mm_cvtph_ps(_mm_loadu_si64(reinterpret_cast<const __m128i*>(source)))};
}

TRIMD_TARGET_F16C inline F128 F128::loadSingleValue(const std::uint16_t* source) {
    return F128{_mm_cvtph_ps(_mm_loadu_si16(source))};
}

TRIMD_TARGET_F16C inline void F128::alignedLoad(const std::uint16_t* source) {
    data = _mm_cvtph_ps(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(source)));
}

TRIMD_TARGET_F16C inline void F128::unalignedLoad(const std::uint16_t* source) {
    data = _mm_cvtph_ps(_mm_loadu_si64(reinterpret_cast<const __m128i*>(source)));
}

TRIMD_TARGET_F16C inline void F128::alignedStore(std::uint16_t* dest) const {
        #if defined(__clang__) || defined(__GNUC__)
            #pragma GCC diagnostic push
            #pragma GCC diagnostic ignored "-Wold-style-cast"
...
TRIMD_TARGET_F16C inline void F128::unalignedStore(std::uint16_t* dest) const {
        #if defined(__clang__) || defined(__GNUC__)
            #pragma GCC diagnostic push
            #pragma GCC diagnostic ignored "-Wold-style-cast"
            #if !defined(__clang__)
