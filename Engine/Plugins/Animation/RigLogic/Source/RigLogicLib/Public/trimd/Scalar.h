...
struct T128 {
    ...
    T128(value_type v1, value_type v2, value_type v3, value_type v4) :
        data({v1, v2, v3, v4}) {
    }

    explicit T128(value_type value) :
...
inline T128<T> operator==(const T128<T>& lhs, const T128<T>& rhs) {
    return T128<T>{bitcast<T>(static_cast<std::uint32_t>(-(lhs.data[0] == rhs.data[0]))),
...
inline T128<T> operator~(const T128<T>& rhs) {
    return T128<T>(bitcast<T>(~bitcast<std::uint32_t>(rhs.data[0])),
