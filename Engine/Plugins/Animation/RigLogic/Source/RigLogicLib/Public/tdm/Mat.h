...
public:
    ...
    explicit mat(U scalar) {
        apply([scalar](row_type& row, dim_t /*unused*/) { row = row_type{scalar}; });
    }
    ...
    mat(Us... scalars) {
        ...
        apply([&tmp](row_type& row, dim_t ri) {
            row.apply([&tmp, ri](value_type& value, dim_t ci) { value = tmp[ri * columns() + ci]; });
        });
    }

    template<typename U>
    mat(const mat<R, C, U>& rhs) {
        apply([&rhs](row_type& row, dim_t ri) { row = rhs[ri]; });
    }
    ...
    template<typename... Us, typename std::enable_if<sizeof...(Us) == R>::type* = nullptr>
    static mat from_rows(const vec<C, Us>&... vs) {
        return mat{vs...};
    }

    template<typename... Us, typename std::enable_if<sizeof...(Us) == C>::type* = nullptr>
    static mat from_columns(const vec<R, Us>&... vs) {
    ...
    static typename std::enable_if<H == W, mat>::type diagonal(U scalar) {
        mat ret;
        ret.apply([scalar](row_type& row, dim_t ri) { row[ri] = scalar; });
        return ret;
    }
    ...
    static typename std::enable_if<H == W, mat>::type diagonal(const vec<W, U>& scalars) {
        mat ret;
        ret.apply([&scalars](row_type& row, dim_t ri) { row[ri] = scalars[ri]; });
        return ret;
    }
    ...
    mat& operator++() {
        return apply([](row_type& row, dim_t /*unused*/) { ++row; });
    }
    ...
    template<typename U>
    mat& operator+=(U rhs) {
        return apply([rhs](row_type& row, dim_t /*unused*/) { row += rhs; });
    }

    template<typename U>
    mat& operator+=(const mat<R, C, U>& rhs) {
        return apply([&rhs](row_type& row, dim_t ri) { row += rhs[ri]; });
    }
    ...
    mat& negate() {
        apply([](row_type& row, dim_t /*unused*/) { row.negate(); });
        return *this;
    }
    ...
    column_type column(dim_t index) const {
        column_type col;
        apply([&col, index](const row_type& row, dim_t ri) { col[ri] = row[index]; });
        return col;
    }
    ...
    #pragma GCC diagnostic ignored "-Wattributes"
        ...
        ret.apply([this, y, x](typename mat<H, W, T>::row_type& row, dim_t ri) {
            row.apply([this, y, x, ri](value_type& value, dim_t ci) { value = values[y + ri][x + ci]; });
        });
#if !defined(__clang__) && defined(__GNUC__)
...
inline bool operator==(const mat<R, C, T>& lhs, const mat<R, C, T>& rhs) {
    ...
    bool retval = true;
    lhs.apply([&rhs, &retval](const row_type& row, dim_t ri) { retval = retval && (row == rhs[ri]); });
    return retval;
}
...
inline typename mat<R, C, T>::row_type operator*(const typename mat<R, C, T>::column_type& lhs, const mat<R, C, T>& rhs) {
    using row_type = typename mat<R, C, T>::row_type;
    row_type ret;
    rhs.apply([&ret, &lhs](const row_type& row, dim_t ri) { ret += (row * row_type{lhs[ri]}); });
    return ret;
}
...
inline typename mat<R, C, T>::column_type operator*(const mat<R, C, T>& lhs, const typename mat<R, C, T>::row_type& rhs) {
    ...
    column_type ret;
    rhs.apply([&ret, &lhs](value_type value, dim_t ci) { ret += (lhs.column(ci) * column_type{value}); });
    return ret;
}
...
inline mat<R, C, T> operator*(const mat<R, S, T>& lhs, const mat<S, C, T>& rhs) {
    using row_type = typename mat<R, C, T>::row_type;
    mat<R, C, T> ret;
    ret.apply([&lhs, &rhs](row_type& row, dim_t ri) { row = (lhs[ri] * rhs); });
    return ret;
}
...
inline mat<R, C, T> operator/(T lhs, const mat<R, C, T>& rhs) {
    using row_type = typename mat<R, C, T>::row_type;
    mat<R, C, T> tmp{rhs};
    tmp.apply([lhs](row_type& row, dim_t /*unused*/) { row = lhs / row; });
    return tmp;
}
