...
#include <cstdint>
    ...
    #include <immintrin.h>
        ...
        (defined(BYTE_ORDER) && BYTE_ORDER == LITTLE_ENDIAN) /*Apple,MingW*/ || defined(__LITTLE_ENDIAN__) /*GCC Mac*/ ||        \
        defined(__ARMEL__) /*GCC,Clang*/ || defined(__THUMBEL__) /*GCC,Clang*/ || defined(__AARCH64EL__) /*GCC,Clang*/ ||        \
        defined(_MIPSEL) /*GCC,Clang*/ || defined(__MIPSEL) /*GCC,Clang*/ || defined(__MIPSEL__) /*GCC,Clang*/ ||                \
        defined(_M_IX86) /*MSVC*/ || defined(_M_X64) /*MSVC*/ || defined(_M_IA64) /*MSVC*/ || defined(_M_AMD64) /*MSVC*/ ||      \
        defined(_M_ARM) /*MSVC*/ || defined(_M_ARM64) /*MSVC*/
        ...
        (defined(BYTE_ORDER) && BYTE_ORDER == BIG_ENDIAN) /*Apple,MingW*/ || defined(_M_PPC) /*MSVC for XBox-360*/ ||            \
        defined(__BIG_ENDIAN__) /*GCC Mac*/ || defined(__ARMEB__) /*GCC,Clang*/ || defined(__THUMBEB__) /*GCC,Clang*/ ||         \
        defined(__AARCH64EB__) /*GCC,Clang*/ || defined(_MIPSEB) /*GCC,Clang*/ || defined(__MIPSEB) /*GCC,Clang*/ ||             \
        defined(__MIPSEB__) /*GCC,Clang*/
...
static inline std::uint16_t bswap16(std::uint16_t x) {
    return (((x & std::uint16_t{0x00FF}) << 8) | ((x & std::uint16_t{0xFF00}) >> 8));
}

static inline std::uint32_t bswap32(std::uint32_t x) {
    return (((x & std::uint32_t{0x000000FF}) << 24) | ((x & std::uint32_t{0x0000FF00}) << 8) |
            ((x & std::uint32_t{0x00FF0000}) >> 8) | ((x & std::uint32_t{0xFF000000}) >> 24));
}

static inline std::uint64_t bswap64(std::uint64_t x) {
    return (((x & std::uint64_t{0x00000000000000FF}) << 56) | ((x & std::uint64_t{0x000000000000FF00}) << 40) |
            ((x & std::uint64_t{0x0000000000FF0000}) << 24) | ((x & std::uint64_t{0x00000000FF000000}) << 8) |
            ((x & std::uint64_t{0x000000FF00000000}) >> 8) | ((x & std::uint64_t{0x0000FF0000000000}) >> 24) |
            ((x & std::uint64_t{0x00FF000000000000}) >> 40) | ((x & std::uint64_t{0xFF00000000000000}) >> 56));
}
...
static inline void bswap32x4(std::uint32_t* source) {
    const __m128i v = _mm_load_si128(reinterpret_cast<__m128i*>(source));
    const __m128i swapped = _mm_shuffle_epi8(v, _mm_set_epi8(12, 13, 14, 15, 8, 9, 10, 11, 4, 5, 6, 7, 0, 1, 2, 3));
    _mm_store_si128(reinterpret_cast<__m128i*>(source), swapped);
}

static inline void bswap64x2(std::uint64_t* source) {
    const __m128i v = _mm_load_si128(reinterpret_cast<__m128i*>(source));
    const __m128i swapped = _mm_shuffle_epi8(v, _mm_set_epi8(8, 9, 10, 11, 12, 13, 14, 15, 0, 1, 2, 3, 4, 5, 6, 7));
    _mm_store_si128(reinterpret_cast<__m128i*>(source), swapped);
}
