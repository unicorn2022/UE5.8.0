...
protected:
    ...
    typename std::enable_if<traits::has_versioned_save_member<T, V>::value, void>::type process(Versioned<T, V>&& source) {
        process(source);
    }
    ...
    template<typename T, typename V>
    typename std::enable_if<traits::has_versioned_serialize_member<T, V>::value, void>::type process(Versioned<T, V>&& source) {
        process(source);
    }
    ...
    template<typename T, typename V>
    typename std::enable_if<traits::has_versioned_save_function<T, V>::value, void>::type process(Versioned<T, V>&& source) {
        process(source);
    }
    ...
    template<typename T, typename V>
    typename std::enable_if<traits::has_versioned_serialize_function<T, V>::value, void>::type process(Versioned<T, V>&& source) {
        process(source);
    }
    ...
    typename std::enable_if<
        !traits::has_versioned_save_member<T, V>::value && !traits::has_versioned_serialize_member<T, V>::value &&
            !traits::has_versioned_save_function<T, V>::value && !traits::has_versioned_serialize_function<T, V>::value,
        void>::type
    process(Versioned<T, V>&& dest) {
        process(dest);
    }
    ...
    template<typename T>
    typename std::enable_if<traits::has_save_member<T>::value, void>::type process(const T& source) {
        const bool transparent = popTransparency();
        if (!transparent) {
            preStructOutput();
        }
    ...
    template<typename T>
    typename std::enable_if<traits::has_serialize_member<T>::value, void>::type process(const T& source) {
        const bool transparent = popTransparency();
        if (!transparent) {
            preStructOutput();
        }
    ...
    template<typename T>
    typename std::enable_if<traits::has_save_function<T>::value, void>::type process(const T& source) {
        const bool transparent = popTransparency();
        if (!transparent) {
            preStructOutput();
        }
    ...
    template<typename T>
    typename std::enable_if<traits::has_serialize_function<T>::value, void>::type process(const T& source) {
        const bool transparent = popTransparency();
        if (!transparent) {
            preStructOutput();
        }
    ...
    void process(const Blob<T, Args...>& source) {
        ...
        std::basic_string<char, std::char_traits<char>, typename Blob<T, Args...>::allocator_type> buffer{encodedSize,
                                                                                                          '\0',
...
class JSONOutputArchive
    : public ExtendableJSONOutputArchive<JSONOutputArchive<TStream, TSize, TOffset>, TStream, TSize, TOffset> {
public:
...
public:
    JSONOutputArchive(TStream* stream_, std::uint32_t indentWidth_) :
        BaseArchive{this, stream_, indentWidth_} {
    }
