...
template<class TContainer>
static auto test_push_back_member(std::int32_t)
    -> true_sink<decltype(std::declval<TContainer>().push_back(std::declval<typename TContainer::value_type>()))>;
...
static auto test_versioned_serialize_member(std::int32_t)
    -> true_sink<decltype(std::declval<T>().serialize(std::declval<T&>(), V{}))>;
...
template<class T, class V>
static auto test_versioned_serialize_function(std::int32_t)
    -> true_sink<decltype(serialize(std::declval<T&>(), V{}, std::declval<T&>()))>;
