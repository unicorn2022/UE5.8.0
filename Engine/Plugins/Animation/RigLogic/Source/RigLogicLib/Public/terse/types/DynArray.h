...
public:
    ...
    DynArray(const DynArray& rhs) :
        DynArray{rhs.size(), rhs.get_allocator()} {
    ...
    void resize(std::size_t size, const value_type& value) {
        if (size > sz) {
            pointer_type old{ptr.release(), [this](value_type* p) { alloc.deallocate(p, sz); }};
...
private:
    static pointer_type create(std::size_t size, allocator_type alloc) {
        return pointer_type{alloc.allocate(size), [alloc, size](value_type* p) mutable { alloc.deallocate(p, size); }};
    }
