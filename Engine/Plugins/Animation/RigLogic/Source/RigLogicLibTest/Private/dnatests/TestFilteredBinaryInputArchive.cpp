...
protected:
    void SetUp() override {
        ...
        dna::LODConstraint lod{dnaParams.maxLOD, dnaParams.minLOD, &amr};
        dna::FilteredBinaryInputArchive archive{stream.get(), dna::DataLayer::All, std::move(lod), &amr};
        archive >> *dnaInstance;
    }
...
INSTANTIATE_TEST_SUITE_P(FilteredDNAInputArchiveTest,
                         FilteredDNAInputArchiveTest,
                         ::testing::Values(DNAParameters{dna::FileVersion::v21, 0u, 1u},
...
TEST_P(GeometryFilteringTest, IncludeBlendShapeTargets) {
    dna::LODConstraint lod{0u, std::numeric_limits<std::uint16_t>::max(), &amr};
    dna::FilteredBinaryInputArchive archive{stream.get(), dna::DataLayer::Geometry, std::move(lod), &amr};
    archive >> *dnaInstance;
...
TEST_P(GeometryFilteringTest, IgnoreBlendShapeTargets) {
    dna::LODConstraint lod{0u, std::numeric_limits<std::uint16_t>::max(), &amr};
    dna::FilteredBinaryInputArchive archive{stream.get(), dna::DataLayer::GeometryWithoutBlendShapes, std::move(lod), &amr};
    archive >> *dnaInstance;
...
INSTANTIATE_TEST_SUITE_P(GeometryFilteringTest,
                         GeometryFilteringTest,
...
public:
    FilterLODsDNAReader(dna::MemoryResource* memRes = nullptr) :
        jointNames{memRes},
        jointIndicesPerLOD{memRes} {
        lodCount = 6u;
        jointNames = {{"body_joint1", memRes},
                      {"body_joint2", memRes},
                      {"body_joint3", memRes},
                      {"body_joint4", memRes},
                      {"body_joint5", memRes},
...
protected:
    void SetUp() override {
        ...
        config.lods = lods;
        readerSpecificLODs = dna::makeScoped<dna::BinaryStreamReader>(stream.get(), config);
...
INSTANTIATE_TEST_SUITE_P(FilteredDNAInputArchiveTest,
                         FilterLODsTest,
                         ::testing::Values(std::vector<std::uint16_t>{0},
                                           std::vector<std::uint16_t>{2},
                                           std::vector<std::uint16_t>{5},
                                           std::vector<std::uint16_t>{3, 1},
                                           std::vector<std::uint16_t>{4, 3, 5},
                                           std::vector<std::uint16_t>{0, 1, 2, 3, 4}));

namespace {

class FilterSkinWeightsDNAReader : public dna::FakeDNAReader {
public:
    FilterSkinWeightsDNAReader(dna::MemoryResource* memRes = nullptr) :
        jointNames{memRes},
        meshNames{memRes},
        jointIndicesPerLOD{memRes},
        meshesPerLOD{memRes},
        ...
        skinWeightsValues = {{// Mesh-0
                             ...
                             {// Mesh-1
                              ...
                              {0.8f, 0.2f},
                              {0.1f, 0.9f}}};
        skinWeightsJointIndices = {{// Mesh-0
                                    {0, 1, 2},
                                    {3, 4},
                                    {6, 9}},
                                   {// Mesh-1
...
protected:
    void SetUp() override {
        ...
        dna::Configuration config;
        config.lods = lods;
        readerLOD2 = dna::makeScoped<dna::BinaryStreamReader>(stream.get(), config);
        readerLOD2->read();
    }
