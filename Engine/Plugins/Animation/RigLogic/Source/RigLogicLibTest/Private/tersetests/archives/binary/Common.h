...
struct Root {
    ...
    explicit Root(pma::MemoryResource* memRes) :
        children{memRes} {
    }
...
struct OffsetUtilizer {
    ...
    template<class Archive>
    void serialize(Archive& archive) {
        archive(layerStart, first, second, a, b, c, layerMarker, firstMarker, firstInt, secondMarker, secondInt);
    }
};
