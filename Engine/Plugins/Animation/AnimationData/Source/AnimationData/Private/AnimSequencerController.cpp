...
void UAnimSequencerController::OpenBracket(const FText& InTitle, bool bShouldTransact /*= true*/)
{
	...
	if (BracketDepth == 0)
	{
		bShouldTransactBracket = bShouldTransact;
	}

	if (UE::FChangeTransactor::CanTransactChanges() && !ChangeTransactor.IsTransactionPending() && ShouldTransact(bShouldTransact))
	{
...
void UAnimSequencerController::CloseBracket(bool bShouldTransact /*= true*/)
{
	...
	if (BracketDepth == 0)
	{
		if (UE::FChangeTransactor::CanTransactChanges() && ShouldTransact(bShouldTransact && bShouldTransactBracket))
		{
...
bool UAnimSequencerController::ShouldTransact(bool bInLocalShouldTransact) const
{
	return bInLocalShouldTransact && (BracketDepth == 0 || bShouldTransactBracket);
}
...
bool UAnimSequencerController::RemoveBoneTracksMissingFromSkeleton(const USkeleton* Skeleton, bool bShouldTransact /*= true*/)
{
	...
	if (Skeleton)
	{
		...
		{
			TSharedPtr<IControlRigObjectBinding> Binding = ControlRig->GetObjectBinding();
			const UObject* BoundControlRigObject = Binding.IsValid() ? Binding->GetBoundObject() : nullptr;
			if (BoundControlRigObject && BoundControlRigObject == Skeleton)
			{
			...
			{
				if (!BoundControlRigObject)
				{
					ReportErrorf(LOCTEXT("MissingControlRigBinding", "ControlRig has no bound object; cannot validate against Skeleton {0}"),  FText::FromString(Skeleton->GetName()));
				}
				else
				{
					ReportErrorf(LOCTEXT("SkeletonNotBoundToControlRig", "Provided Skeleton {0} does not match bound ControlRigObject {1}"), FText::FromString(Skeleton->GetName()), FText::FromString(BoundControlRigObject->GetName()));
				}
			}
		}
	}
...
void UAnimSequencerController::UpdateAttributesFromSkeleton(const USkeleton* Skeleton, bool bShouldTransact)
{
	...
	if (Skeleton)
	{
		...
		if (ToRemoveIdentifiers.Num() || ToDuplicateIdentifiers.Num())
		{
			...
			{
				RemoveAttribute(Identifier, bShouldTransact);
			}
			...
			for (const TPair<FAnimationAttributeIdentifier, int32>& Pair : ToDuplicateIdentifiers)
			{
				...
				DuplicateAttribute(Pair.Key, NewIdentifier, bShouldTransact);
				RemoveAttribute(Pair.Key, bShouldTransact);
			}	
		}
	}
	else
...
bool UAnimSequencerController::SetCurveKey(const FAnimationCurveIdentifier& CurveId, const FRichCurveKey& Key, bool bShouldTransact)
{
	...
	if (FRichCurve* RichCurve = Model->GetMutableRichCurve(CurveId))
	{
		...
		const FKeyHandle Handle = RichCurve->FindKey(Key.Time);
...
bool UAnimSequencerController::RemoveCurveKey(const FAnimationCurveIdentifier& CurveId, float Time, bool bShouldTransact)
{
	...
	if (FRichCurve* RichCurve = Model->GetMutableRichCurve(CurveId))
	{
		...
		const FKeyHandle Handle = RichCurve->FindKey(Time);
		if (Handle != FKeyHandle::Invalid())
		{
			IAnimationDataModel::FEvaluationAndModificationLock Lock(*ModelInterface);
