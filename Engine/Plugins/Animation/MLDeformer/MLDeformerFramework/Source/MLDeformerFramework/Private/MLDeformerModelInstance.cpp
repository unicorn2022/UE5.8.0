...
void UMLDeformerModelInstance::Init(USkeletalMeshComponent* SkelMeshComponent)
{
	...
	if (SkelMeshComponent == nullptr)
	{
		...
		if (Asset && Asset->GetModel()->GetInputInfo())
		{
			...
			{
				UE_LOGF(LogMLDeformer, Error, "ML Deformer '%ls' is trained on skeletal mesh '%ls', which cannot be found on actor!", *Asset->GetName(), *SkelMeshPath.ToString());
			}
		}
...
FString UMLDeformerModelInstance::CheckCompatibility(USkeletalMeshComponent* InSkelMeshComponent, bool bLogIssues)
{
	...
	if (SkelMesh && !InputInfo->IsCompatible(InSkelMeshComponent) && Model->GetDeformerAsset() && Model->IsTrained())
	{
		...
		if (bLogIssues)
		{
			UE_LOGF(LogMLDeformer, Error, "ML Deformer '%ls' isn't compatible with Skeletal Mesh '%ls'.\nReason(s):\n%ls\n", 
