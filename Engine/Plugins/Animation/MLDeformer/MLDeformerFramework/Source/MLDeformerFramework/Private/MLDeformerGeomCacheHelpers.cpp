		...
		for (int32 TrackIndex = 0; TrackIndex < GeomCache->Tracks.Num(); ++TrackIndex)
		{
			...
			for (int32 GeoPartIndex = 0; GeoPartIndex < NumSourceGeoParts; ++GeoPartIndex)
			{
					...
					(IsPotentialMatch(Track->GetName(), SkelMeshName) || bIsSoloMesh))
				{
					...
					if (!bSuppressLog)
					{
						UE_LOGF(LogMLDeformer, Verbose, "Mapped geom cache track '%ls' to SkelMesh submesh '%ls'", *Track->GetName(), *SkelMeshName);
					}
					break;
			...
			if (Track && !bFoundMatch)
			{
				...
				if (!bSuppressLog)
				{
					UE_LOGF(LogMLDeformer, Warning, "Geometry cache '%ls' cannot be matched with a mesh inside the Skeletal Mesh.", *Track->GetName());
				}
			}
		} // For all tracks.
	}
