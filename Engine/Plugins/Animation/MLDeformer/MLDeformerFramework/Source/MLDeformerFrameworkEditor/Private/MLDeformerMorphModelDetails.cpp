		...
		if (SkelMesh)
		{
			...
			for (const int32 MaterialIndex : MaterialIndices)
			{
				if (Materials.IsValidIndex(MaterialIndex))
				{
					...
					if (MaterialInterface && !MaterialInterface->GetUsageByFlag(MATUSAGE_MorphTargets))
					{
						UE_LOGF(LogMLDeformer, Warning, "Material '%ls' (Index=%d) has the 'Used with Morph Targets' property disabled, while no deformer graph or skin cache is used. This can cause issues with ML Deformer.",
