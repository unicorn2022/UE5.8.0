...
namespace UE::MLDeformer
{
	...
	void SMLDeformerInputBonesWidget::OnCopyInputBones() const
	{
		...
		{
			UE_LOGF(LogMLDeformer, Error, "Failed to serialize JSON on Copy action in the Input Bones widget.");
			return;
		}
	...
	void SMLDeformerInputBonesWidget::OnPasteInputBones()
	{
		...
		if (GetInputWidget()->HasValidClipBoardData(TEXT("InputBones"), JsonObject) && JsonObject.IsValid())
		{
			...
			{
				UE_LOGF(LogMLDeformer, Error, "Failed to get bone names during paste operation.");
			}
	...
	void SMLDeformerInputBonesWidget::OnAddInputBones()
	{
		...
		{
			UE_LOGF(LogMLDeformer, Warning, "No skeleton is available to pick bones from");
		}
	}
