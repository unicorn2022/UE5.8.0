...
TArray<float> UMLDeformerTrainingModel::GetTrainingInputAnimMaskData(FName MaskName) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogMLDeformer, Warning, "Failed to get vertex mask data for mask '%ls'. A fully painted mask will be used.", *MaskName.ToString());
		}
...
{
	UE_LOGF(LogMLDeformer, Warning, "Please override the SampleNextFrame method in your UMLDeformerTrainingModel inherited class.");
	return false;
}
