...
namespace UE::MLDeformer
{
	...
	void SMLDeformerInputCurvesWidget::OnCopyInputCurves() const
	{
		...
		{
			UE_LOGF(LogMLDeformer, Error, "Failed to serialize JSON on Copy action in the Input Curves widget.");
			return;
		}
	...
	void SMLDeformerInputCurvesWidget::OnPasteInputCurves()
	{
		...
		if (GetInputWidget()->HasValidClipBoardData(TEXT("InputCurves"), JsonObject) && JsonObject.IsValid())
		{
			...
			{
				UE_LOGF(LogMLDeformer, Error, "Failed to get curve names during paste operation.");
			}
	...
	void SMLDeformerInputCurvesWidget::OnAddInputCurves()
	{
		...
		{
			UE_LOGF(LogMLDeformer, Warning, "No skeletal mesh is available to pick curves from");
		}
	}
