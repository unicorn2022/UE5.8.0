...
namespace UE::MLDeformer
{
		...
		bool bInvertMaskChannel)
	{
		...
		UE_LOGF(LogMLDeformer, Display, "Initializing %d engine morph targets of %d vertices each", NumMorphTargets, Deltas.Num() / NumMorphTargets);
	...
	void FMLDeformerMorphModelEditorModel::TransferMorphTargets(TArray<UMorphTarget*> MorphTargetsLODZero)
	{
		...
		UE_LOGF(LogMLDeformer, Display, "Finished Morph Target LOD generation in %.2f seconds", TotalTime);
	}
	...
	void FMLDeformerMorphModelEditorModel::ApplyMaskInfoToBuffer(const USkeletalMesh* SkeletalMesh, const FMLDeformerMaskInfo& MaskInfo, TArrayView<float> ItemMaskBuffer)
	{
		...
		if (MaskInfo.MaskMode == EMLDeformerMaskingMode::Generated)
		{
			...
			for (const FName MaskBoneName : MaskInfo.BoneNames)
			{
				...
				{
					UE_LOGF(LogMLDeformer, Warning, "Mask contains a bone named '%ls', which cannot be found in the ref skeleton of skeletal mesh '%ls'.",
		...
		else // We're using a painted mask.
		{
			...
			if (!VertexAttributeName.IsNone())
			{
				...
				{
					UE_LOGF(LogMLDeformer, Warning, "Mask references a vertex attribute '%ls' which doesn't exist on skeletal mesh %ls.", *VertexAttributeName.ToString(), *SkeletalMesh->GetName());
					FillMaskValues(ItemMaskBuffer, 1.0f);
				}
			}
			else
			{
				UE_LOGF(LogMLDeformer, Warning, "Mask is set to use a vertex attribute, but none is specified.");
				FillMaskValues(ItemMaskBuffer, 1.0f);
			}
		}
	}
