...
namespace UE::MLDeformer
{
	...
	{
		UE_LOGF(LogMLDeformer, Error, "Please implement your Train method.");
	...
	void FMLDeformerGeomCacheEditorModel::OnObjectModified(UObject* Object)
	{
		...
		for (int32 AnimIndex = 0; AnimIndex < NumAnims; ++AnimIndex)
		{
			...
			{
				UE_LOGF(LogMLDeformer, Display, "Detected a modification in training anim sequence %ls, reinitializing inputs.", *Object->GetName());
				bNeedsAssetReinit = true;
			}

			if (Anim.GetGeometryCache() == Object)
			{
				UE_LOGF(LogMLDeformer, Display, "Detected a modification in training geometry cache %ls, reinitializing inputs.", *Object->GetName());
				bNeedsAssetReinit = true;
			}
		}
		...
		{
			UE_LOGF(LogMLDeformer, Display, "Detected a modification in test ground truth geometry cache %ls, reinitializing inputs.", *Object->GetName());
			bNeedsAssetReinit = true;
		}
	}
