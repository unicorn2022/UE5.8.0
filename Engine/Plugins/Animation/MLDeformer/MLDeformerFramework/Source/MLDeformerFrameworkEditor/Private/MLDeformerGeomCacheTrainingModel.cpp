...
bool UMLDeformerGeomCacheTrainingModel::SampleNextFrame()
{
	...
	UE_LOGF(LogMLDeformer, Verbose, "Sampling frame %d of anim %d", CurFrameToSample, SampleAnimIndex);
...
bool UMLDeformerGeomCacheTrainingModel::GenerateBasicInputsAndOutputBuffers(const FString& InputsFilePath, const FString& OutputsFilePath)
{
	...
	if (!InputsArchive)
	{
		UE_LOGF(LogMLDeformer, Error,
		       "UMLDeformerGeomCacheTrainingModel::GenerateBasicInputsAndOutputBuffers() - Failed to create inputs buffer file %ls",
		       *InputsFilePath);
		return false;
	}
	...
	{
		UE_LOGF(LogMLDeformer, Error,
		       "UMLDeformerGeomCacheTrainingModel::GenerateBasicInputsAndOutputBuffers() - Failed to create output buffer file %ls",
		       *OutputsFilePath);
		return false;
	}
	...
	for (int32 SampleIndex = 0; SampleIndex < NumFrames; SampleIndex++)
	{
		...
		{
			UE_LOGF(LogMLDeformer, Error, "Failed to write sampled input data!");
			return false;
		}
		...
		{
			UE_LOGF(LogMLDeformer, Error, "Failed to write sampled output data!");
			return false;
		}
	...
	UE_LOGF(LogMLDeformer, Display, "Sampling finished in %.0f seconds", FPlatformTime::Seconds() - StartTime);
	return true;
}
