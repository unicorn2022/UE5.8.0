...
namespace UE::MLDeformer::TrainingDataProcessor
{
	...
	FReply FTrainingDataProcessorSettingsDetailCustomization::OnCreateNewButtonClicked() const
	{
		...
		{
			UE_LOGF(LogMLDeformer, Warning, "User canceled the save dialog.");
			return FReply::Handled();
		}
		...
		if (NewAnimSequence)
		{
			...
			UE_LOGF(LogMLDeformer, Display, "Successfully created AnimSequence asset at: %ls", *SaveObjectPath);
		}
		else
		{
			FMessageDialog::Open(EAppMsgCategory::Error, EAppMsgType::Ok,
			                     ...
			                     MessageTitle);
			UE_LOGF(LogMLDeformer, Error, "Failed to create the anim instance asset at: %ls", *SaveObjectPath);
		}
