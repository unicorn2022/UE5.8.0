...
namespace UE::MLDeformer
{
	...
	void FMLDeformerEditorModel::OnObjectModified(UObject* Object)
	{
		...
		{
			UE_LOGF(LogMLDeformer, Display, "Detected a modification in skeletal mesh %ls, reinitializing inputs.", *Object->GetName());
			bNeedsAssetReinit = true;
		}

		if (Model->GetVizSettings()->GetTestAnimSequence() == Object)
		{
			UE_LOGF(LogMLDeformer, Display, "Detected a modification in test anim sequence %ls, reinitializing inputs.", *Object->GetName());
			bNeedsAssetReinit = true;
		}
	}
	...
	void FMLDeformerEditorModel::UpdateLODMappings()
	{
		...
		UE_LOGF(LogMLDeformer, Display, "Finished Updating ML Deformer LOD mappings in %.2f seconds", TotalTime);
	}
	...
	void FMLDeformerEditorModel::AddAnimatedBonesToBonesIncludeList(const UAnimSequence* AnimSequence)
	{
		...
		{
			UE_LOGF(LogMLDeformer, Warning, "Cannot initialize bone list as no Anim Sequence has been picked.");
			return;
		}
		...
		if (!DataModel)
		{
			UE_LOGF(LogMLDeformer, Warning, "Anim sequence has no data model.");
			return;
		}
		...
		for (int32 Index = 0; Index < NumBones; ++Index)
		{
			...
			if (!BoneTransforms.IsEmpty())
			{
				...
				if (!bIsAnimated)
				{
					UE_LOGF(LogMLDeformer, Display, "Bone '%ls' has keyframes but isn't animated.", *BoneName.ToString());
				}
			}
	...
	void FMLDeformerEditorModel::AddAnimatedBonesToBonesIncludeList()
	{
		if (!Model->GetSkeletalMesh())
		{
			UE_LOGF(LogMLDeformer, Warning, "Skeletal Mesh has not been set.");
			UpdateEditorInputInfo();
			return;
		}
		...
		if (!Skeleton)
		{
			UE_LOGF(LogMLDeformer, Warning, "Skeletal Mesh has no skeleton.");
			UpdateEditorInputInfo();
			return;
		}
	...
	void FMLDeformerEditorModel::AddAnimatedCurvesToCurvesIncludeList(const UAnimSequence* AnimSequence)
	{
		if (AnimSequence == nullptr)
		{
			UE_LOGF(LogMLDeformer, Warning, "Cannot initialize curve list as no Anim Sequence has been picked.");
			return;
		}
		...
		if (SkeletalMesh)
		{
			...
			for (const FBoneReference& BoneReference : Model->GetBoneIncludeList())
			{
				if (BoneReference.BoneName.IsValid())
				{
					...
					{
						UE_LOGF(LogMLDeformer, Warning, "Bone '%ls' in the bones include list doesn't exist, ignoring it.", *BoneName.ToString());
						continue;
					}
	...
	UMeshDeformer* FMLDeformerEditorModel::LoadDefaultDeformerGraph() const
	{
		...
		if (!GraphAssetPath.IsEmpty())
		{
			...
			{
				UE_LOGF(LogMLDeformer, Warning, "Failed to load default Mesh Deformer from: %ls", *GraphAssetPath);
			}
			else
			{
				UE_LOGF(LogMLDeformer, Verbose, "Loaded default Mesh Deformer from: %ls", *GraphAssetPath);
			}
	...
	{ 
		UE_LOGF(LogMLDeformer, Warning, "Please override the FMLDeformerEditorModel::Train() method inside your derived editor model class.");
	...
	{
		NumTrainingFrames = 0;
		UE_LOGF(LogMLDeformer, Warning, "Please override the FMLDeformerEditorModel::UpdateNumTrainingFrames() inside your model.");
	}
	...
	{
		UE_LOGF(LogMLDeformer, Warning, "Please override the FMLDeformerEditorModel::GetNumTrainingInputAnims() method inside your derived editor model class.");
		return 0;
	}

	FMLDeformerTrainingInputAnim* FMLDeformerEditorModel::GetTrainingInputAnim(int32 Index) const
	{
		UE_LOGF(LogMLDeformer, Warning, "Please override the FMLDeformerEditorModel::GetTrainingInputAnim(int32) method inside your derived editor model class.");
		return nullptr;
	}
	...
	void FMLDeformerEditorModel::BroadcastPropertyChanged(const FName PropertyName)
	{
		...
		if (Property == nullptr)
		{
			UE_LOGF(LogMLDeformer, Error, "Failed to find property '%ls' in class '%ls'", *PropertyName.ToString(), *Model->GetName());
			return;
		}
