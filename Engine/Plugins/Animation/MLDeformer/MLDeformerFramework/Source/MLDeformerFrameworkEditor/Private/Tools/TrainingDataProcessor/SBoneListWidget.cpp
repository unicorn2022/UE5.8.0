...
namespace UE::MLDeformer::TrainingDataProcessor
{
	...
	void SBoneListWidget::OnAddBones() const
	{
		if (!GetBoneNames.IsBound())
		{
			UE_LOGF(LogMLDeformer, Warning, "Please set a GetBoneNames to your SBoneListWidget when creating your SBoneListWidget.");
			return;
		}
		...
		if (!BoneNames)
		{
			UE_LOGF(LogMLDeformer, Warning, "There are no bones that can be added.");
			return;
		}
