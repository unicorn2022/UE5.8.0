...
namespace UE::Chaos::FleshGenerator
{
	namespace Private
	{
		...
		FTimeScope::~FTimeScope()
		{
			...
			UE_LOGF(LogChaosFleshGeneratorPrivate, Log, "%ls took %f secs", *Name, Duration.GetTotalSeconds());
		}
		...
		TArray<int32> ParseFrames(const FString& FramesString)
		{
			...
			{
			    UE_LOGF(LogChaosFleshGeneratorPrivate, Error, "Input contains invalid characters.");
			    return Result;
			}
		    ...
		    for (const FString& Segment : Segments)
		    {
		    	...
		    	if (!bSegmentValid)
		    	{
		    	    UE_LOGF(LogChaosFleshGeneratorPrivate, Error, "Invalid format in segment: %ls", *Segment);
		    	}
		    }
		...
		TOptional<TArray<int32>> GetMeshImportVertexMap(const USkinnedAsset& SkinnedMeshAsset, const UFleshAsset& FleshAsset)
		{
			...
			if (Map.IsEmpty())
			{
				UE_LOGF(LogChaosFleshGeneratorPrivate, Warning, "MeshToImportVertexMap is empty. MLDeformer Asset should be an imported SkeletalMesh (e.g. from fbx).");
				return None;
			}
			...
			if (SkeletalMeshAsset && FleshCollection && RestVertices)
			{
				...
				if (NumSections != 1)
				{
					UE_LOGF(LogChaosFleshGeneratorPrivate, Warning, "SkeletalMeshAsset should have only one section.");
					return None;
				}
				...
				for (int32 SectionIndex = 0; SectionIndex < NumSections; ++SectionIndex)
				{
					...
					{
						UE_LOGF(LogChaosFleshGeneratorPrivate, Warning, "SkeletalMeshAsset and FleshAsset have different number of vertices in section %d. Check if the assets have the same mesh.", SectionIndex);
						return None;
					}
					...
					for (int32 VertexIndex = 0; VertexIndex < MLDSection.NumVertices; ++VertexIndex)
					{
						...
						{
							UE_LOGF(LogChaosFleshGeneratorPrivate, Warning, "SkeletalMeshAsset and FleshAsset have different vertex positions. Check if the assets have the same vertex order.");
							return None;
						}
					}
				}
