...
UNeuralMorphMLPLayer* UNeuralMorphMLP::LoadNetworkLayer(FArchive& FileReader)
{
	...
	{
		UE_LOGF(LogNeuralMorphModel, Error, "Failed to load a layer header for the network!");
	...
	if (FileReader.IsError())
	{
		UE_LOGF(LogNeuralMorphModel, Error, "Failed to load layer weights of the network!");
		Layer->ConditionalBeginDestroy();
		return nullptr;
	}
	...
	if (FileReader.IsError())
	{
		UE_LOGF(LogNeuralMorphModel, Error, "Failed to load layer biases of the network!");
		Layer->ConditionalBeginDestroy();
		return nullptr;
	}
...
bool UNeuralMorphMLP::Load(FArchive& FileReader)
{
	...
	if (FileReader.IsError())
	{
		UE_LOGF(LogNeuralMorphModel, Error, "Failed to read the FOURCC!");
		Empty();
		return false;
	}
	...
	{
		UE_LOGF(LogNeuralMorphModel, Error, "The file is not a valid valid neural morph MLP file type!");
		Empty();
		return false;
	}
	...
	if (FileReader.IsError())
	{
		UE_LOGF(LogNeuralMorphModel, Error, "Failed to load version");
		Empty();
		return false;
	}
	...
	if (Version != 1)
	{
		UE_LOGF(LogNeuralMorphModel, Error, "The Neural Morph MLP file '%ls' is of an unknown version (Version=%d)!", *FileReader.GetArchiveName(), Version);
		Empty();
		return false;
	}
	...
	if (FileReader.IsError())
	{
		UE_LOGF(LogNeuralMorphModel, Error, "Failed to load number of layers");
		Empty();
		return false;
	}
	...
	for (int32 LayerIndex = 0; LayerIndex < NumLayers; ++LayerIndex)
	{
		...
		Layers.Add(Layer);
		UE_LOGF(LogNeuralMorphModel, Verbose, "MLP Layer %d --> NumWeights=%d (%dx%dx%d)   NumBiases=%d", LayerIndex, Layer->Weights.Num(), Layer->NumInputs, Layer->NumOutputs, Layer->Depth, Layer->Biases.Num());
	}

	UE_LOGF(LogNeuralMorphModel, Display, "Successfully loaded neural morph MLP from file '%ls'", *FileReader.GetArchiveName());
	return true;
}
...
{
	UE_LOGF(LogNeuralMorphModel, Display, "Loading Neural Morph Network from file '%ls'", *Filename);
	...
	if (FFileHelper::LoadFileToArray(FileData, *Filename))
	{
		...
		{
			UE_LOGF(LogNeuralMorphModel, Error, "The file is not a valid neural morph network file!");
			return false;
		}
		...
		{
			UE_LOGF(LogNeuralMorphModel, Error, "The file is not a supported version of neural morph network file!");
			return false;
		}
		...
		UE_LOGF(LogNeuralMorphModel, Display, "Successfully loaded neural morph network from file '%ls'", *Filename);
		return true;
	}
	else
	{
		UE_LOGF(LogNeuralMorphModel, Error, "Failed to read file '%ls'!", *Filename);
		return false;
	}
}
