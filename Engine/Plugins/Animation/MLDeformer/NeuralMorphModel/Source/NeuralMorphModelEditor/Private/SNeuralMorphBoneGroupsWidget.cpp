...
namespace UE::NeuralMorphModel
{
	...
	void SNeuralMorphBoneGroupsWidget::OnCopyBoneGroups() const
	{
		...
		{
			UE_LOGF(LogMLDeformer, Error, "Failed to serialize JSON on Copy action in the Input Bone Groups widget.");
			return;
		}
		...
		if (GetInputWidget()->HasValidClipBoardData(TEXT("BoneGroups"), JsonObject) && JsonObject.IsValid())
		{
			...
			if (JsonObject->TryGetArrayField(TEXT("BoneGroups"), BoneGroupsArray))
			{
				...
				for (const TSharedPtr<FJsonValue>& GroupValue : *BoneGroupsArray)
				{
					...
					{
						UE_LOGF(LogNeuralMorphModel, Error, "Skipping invalid group object when pasting bone groups.");
						continue;
					}
					...
					{
						UE_LOGF(LogNeuralMorphModel, Error, "Failed to read bone group name when pasting bone groups.");
						continue;
					}
					...
					{
						UE_LOGF(LogNeuralMorphModel, Error, "Failed to read bone group bone names array when pasting bone groups.");
						continue;
					}
