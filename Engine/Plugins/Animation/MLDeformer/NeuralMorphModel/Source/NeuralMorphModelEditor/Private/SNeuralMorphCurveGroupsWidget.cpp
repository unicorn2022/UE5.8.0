...
namespace UE::NeuralMorphModel
{
	...
	void SNeuralMorphCurveGroupsWidget::OnCopyCurveGroups() const
	{
		...
		{
			UE_LOGF(LogMLDeformer, Error, "Failed to serialize JSON on Copy action in the Input Curve Groups widget.");
			return;
		}
		...
		if (GetInputWidget()->HasValidClipBoardData(TEXT("CurveGroups"), JsonObject) && JsonObject.IsValid())
		{
			...
			if (JsonObject->TryGetArrayField(TEXT("CurveGroups"), CurveGroupsArray))
			{
				...
				for (const TSharedPtr<FJsonValue>& GroupValue : *CurveGroupsArray)
				{
					...
					{
						UE_LOGF(LogNeuralMorphModel, Error, "Skipping invalid group object when pasting curve groups.");
						continue;
					}
					...
					{
						UE_LOGF(LogNeuralMorphModel, Error, "Failed to read curve group name when pasting curve groups.");
						continue;
					}
					...
					{
						UE_LOGF(LogNeuralMorphModel, Error, "Failed to read curve group curve names array when pasting curve groups.");
						continue;
					}
	...
	bool SNeuralMorphCurveGroupsWidget::BroadcastModelPropertyChanged(const FName PropertyName)
	{
		...
		if (Property == nullptr)
		{
			UE_LOGF(LogNeuralMorphModel, Error, "Failed to find property '%ls' in class '%ls'", *PropertyName.ToString(), *Model->GetName());
			return false;
		}
