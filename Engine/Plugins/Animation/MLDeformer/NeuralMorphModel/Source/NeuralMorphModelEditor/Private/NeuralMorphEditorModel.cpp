...
namespace UE::NeuralMorphModel
{
	...
	bool FNeuralMorphEditorModel::LoadTrainedNetwork() const
	{
		...
		{
			UE_LOGF(LogNeuralMorphModel, Error, "Failed to load neural morph network from file '%ls'!", *NetworkFilename);
		...
		if (SkeletalMesh)
		{
			...
			for (int32 BoneGroupIndex = 0; BoneGroupIndex < ModelBoneGroups.Num(); ++BoneGroupIndex)
			{
				...
				for (int32 Index = 0; Index < NumBonesInGroup; ++Index)
				{
					...
					{
						UE_LOGF(LogNeuralMorphModel, Warning, "Invalid or 'None' bone detected inside curve group %d, please fix this.", BoneGroupIndex);
						continue;
					}

					if (RefSkeleton.FindBoneIndex(BoneRef.BoneName) == INDEX_NONE)
					{
						UE_LOGF(LogNeuralMorphModel, Warning, "Bone '%ls' inside bone group %d doesn't exist, please fix this.", *BoneRef.BoneName.ToString(), BoneGroupIndex);
						continue;
					}

					if (!InputInfo->GetBoneNames().Contains(BoneRef.BoneName))
					{
						UE_LOGF(LogNeuralMorphModel, Warning, "Bone '%ls' inside bone group %d isn't included in the bone list that are input to the model.", *BoneRef.BoneName.ToString(), BoneGroupIndex);
						continue;
					}
		...
		if (SkeletalMesh)
		{
			...
			{				
				...
				for (int32 Index = 0; Index < NumCurvesInGroup; ++Index)
				{
					...
					{
						UE_LOGF(LogNeuralMorphModel, Warning, "Invalid or 'None' curve detected inside curve group %d, please fix this.", CurveGroupIndex);
						continue;
					}

					if (!InputInfo->GetCurveNames().Contains(CurveRef.CurveName))
					{
						UE_LOGF(LogNeuralMorphModel, Warning, "Curve '%ls' inside curve group %d isn't included in the curve list that are input to the model.", *CurveRef.CurveName.ToString(), CurveGroupIndex);
						continue;
					}
	...
	void FNeuralMorphEditorModel::GenerateBoneMaskInfo(int32 InputInfoBoneIndex, int32 HierarchyDepth)
	{
		...
		{
			UE_LOGF(LogNeuralMorphModel, Warning, "Bone '%ls' cannot be found in the SkeletalMesh '%ls', ignoring during mask generation.", *BoneName.ToString(), *SkeletalMesh->GetName());
			return;
		}
	...
	void FNeuralMorphEditorModel::GenerateBoneGroupMaskInfo(int32 InputInfoBoneGroupIndex, int32 HierarchyDepth)
	{
		...
		for (int32 BoneIndex = 0; BoneIndex < BoneGroup.BoneNames.Num(); ++BoneIndex)
		{
			...
			if (BoneName == NAME_None)
			{
				UE_LOGF(LogNeuralMorphModel, Warning, "Bone index %d inside bone group %d has its name set to None, ignoring the bone inside the group's mask.", BoneIndex, InputInfoBoneGroupIndex);
				continue;
			}
			...
			if (SkeletonBoneIndex == INDEX_NONE)
			{
				UE_LOGF(LogNeuralMorphModel, Warning, "Bone '%ls' cannot be found in the SkeletalMesh '%ls', ignoring the bone inside the group's mask.", *BoneName.ToString(), *SkeletalMesh->GetName());
				continue;
			}
