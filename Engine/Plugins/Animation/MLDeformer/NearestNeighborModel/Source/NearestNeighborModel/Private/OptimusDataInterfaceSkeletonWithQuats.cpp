...
{
	if (SkinnedMeshComponent && SkinnedMeshComponent->IsRegistered())
	{
		SceneInterface = SkinnedMeshComponent->GetScene();
		ComponentId = SkinnedMeshComponent->GetPrimitiveSceneId();
	}
...
void FOptimusSkeletonWithQuatsDataProviderProxy::AllocateResources(FRDGBuilder& GraphBuilder, FAllocationData const& InAllocationData)
{
	...
	const int32 NumUniqueSections = bUnifiedBoneMap ? 1 : NumSections;

	PerSectionRefToLocalQuats.SetNum(NumUniqueSections);
	PerSectionRefToLocalQuatsSRVs.SetNum(NumUniqueSections);
...
void FOptimusSkeletonWithQuatsDataProviderProxy::GatherDispatchData(FDispatchData const& InDispatchData)
{
	...
	for (int32 InvocationIndex = 0; InvocationIndex < ParameterArray.Num(); ++InvocationIndex)
	{
		...
		Parameters.BoneQuats = PerSectionRefToLocalQuatsSRVs.IsValidIndex(BoneBufferIndex) ? PerSectionRefToLocalQuatsSRVs[BoneBufferIndex] : nullptr;
