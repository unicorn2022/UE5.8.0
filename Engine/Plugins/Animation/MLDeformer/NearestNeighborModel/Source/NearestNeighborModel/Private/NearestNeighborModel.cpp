...
namespace UE::NearestNeighborModel
{
	...
	namespace Private
	{
		TOptional<TArray<int32>> ParseIntegers(const FString& String)
		{
			...
			{
			    UE_LOGF(LogNearestNeighborModel, Error, "Input contains invalid characters.");
			    return None;
			}
		    ...
		    for (const FString& Segment : Segments)
		    {
		    	...
		    	if (!bSegmentValid)
		    	{
		    	    UE_LOGF(LogNearestNeighborModel, Error, "Invalid format in segment: %ls", *Segment);
					return None;
		    	}
		    }
		...
		TOptional<TArray<float>> ReadTxt(const FString& FilePath)
		{
			...
			if (FFileHelper::LoadFileToStringArray(Lines, *FilePath))
			{
				...
				for (const FString& Line : Lines)
				{
					...
					{
						UE_LOGF(LogNearestNeighborModel, Error, "Invalid value in file: %ls", *Line);
						return Empty;
					}
				}
				return Result;
			...
			{
				UE_LOGF(LogNearestNeighborModel, Error, "Failed to read file: %ls", *FilePath);
				return Empty;
			}
		}
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateForTraining()
{
	...
	if (!GetModel())
	{
		UE_LOGF(LogNearestNeighborModel, Error, "The parent is not a NearestNeighborModel.");
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateForInference()
{
	...
	if (!IsBasisValid())
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Basis is invalid. Please re-train your model");
		ResetBasisData();
		return EOpFlag::Error;
	}
	if (!UpdateRuntimeNeighbors())
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Failed to update runtime neighbors.");
		return EOpFlag::Error;
	}
	if (!IsNearestNeighborValid())
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Nearest neighbor data is invalid. Please re-train your model to update basis data or re-update your model to update nearest neighbor data.");
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateVertexWeights()
{
	...
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Invalid WeightMapCreationMethod.");
	}
	using namespace UE::NearestNeighborModel;
	if (OpFlag::HasError(Result))
	{
	...
	if (DoesUsePCA())
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "NumVertices %d needs to be larger than NumBasis %d.", NumVertices, GetNumBasis());
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateVertexWeightsFromText()
{
	...
	if (!GetModel())
	{
		UE_LOGF(LogNearestNeighborModel, Error, "NearestNeighborModel is invalid.");
		return EOpFlag::Error;
	}
	...
	{
		UE_LOGF(LogNearestNeighborModel, Error, "SkeletalMesh has no imported model.");
		return EOpFlag::Error;
	}
	constexpr int32 LODIndex = 0;
	if (!SkeletalMesh->GetImportedModel()->LODModels.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogNearestNeighborModel, Error, "SkeletalMesh has no LODModel.");
		return EOpFlag::Error;
	}
	...
	if (VertexMapOpt.IsSet())
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "MaxIndex %d is invalid, SkeletalMesh has %d vertices.", MaxIndex, SkeletalMesh->GetNumImportedVertices());
			VertexMap.Reset();
			return EOpFlag::Error;
		}
	...
	{
		UE_LOGF(LogNearestNeighborModel, Error, "VertexMapString is invalid.");
		return EOpFlag::Error;
	}
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateVertexWeightsSelectedBones()
{
	...
	for (const FName& BoneName : BoneNames)
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Bone %ls does not exist in SkeletalMesh.", *BoneName.ToString());
			return EOpFlag::Error;
		}
	...
	for (const FSkelMeshSection& Section : Sections)
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Section %d has invalid soft vertices.", Section.MaterialIndex);
			return EOpFlag::Error;
		}
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateVertexWeightsVertexAttributes()
{
	...
	if (!SkeletalMesh->GetImportedModel()->LODModels.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogNearestNeighborModel, Error, "SkeletalMesh has no LODModel at LOD 0.");
		return EOpFlag::Error;
	}
	if (!SkeletalMesh->HasMeshDescription(LODIndex))
	{
		UE_LOGF(LogNearestNeighborModel, Error, "SkeletalMesh has no Mesh Description at LOD 0.");
		return EOpFlag::Error;
	}
	const FSkeletalMeshLODModel& LODModel = SkeletalMesh->GetImportedModel()->LODModels[LODIndex];
	...
	{
		UE_LOGF(LogNearestNeighborModel, Error, "There is no float attribute %ls in SkeletalMesh.", *AttributeName.ToString());
		return EOpFlag::Error;
	}
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateVertexWeightsExternalTxt()
{
	...
	{
		UE_LOGF(LogNearestNeighborModel, Error, "SkeletalMesh has no vertices.");
		return EOpFlag::Error;
	}
	...
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Number of weights %d does not match number of vertices %d.", Weights.Num(), NumBaseMeshVerts);
		return EOpFlag::Error;
	}
	VertexMap.Reset();
	VertexWeights.Reset();
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::NormalizeVertexWeights()
{
	if (!Model)
	{
		UE_LOGF(LogNearestNeighborModel, Error, "NearestNeighborModel in Section is invalid");
		return EOpFlag::Error;
	}
	const TArray<float>& WeightSum = Model->GetVertexWeightSum();
	if (WeightSum.Num() != Model->GetNumBaseMeshVerts())
	{
		UE_LOGF(LogNearestNeighborModel, Error, "VertexWeightSum is not initialized. Call UNearestNeighborModel::UpdateForTraining() instead.");
		return EOpFlag::Error;
	}
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModel::CheckHiddenLayerDims()
{
	...
	if (HiddenLayerDims.IsEmpty())
	{
		...
		UE_LOGF(LogNearestNeighborModel, Error, "HiddenLayerDims is empty, which will lead to bad results. Default hidden layer dims are added for you. Please double check and train model again.");
		return EOpFlag::Error;
	}
	for (int32 Index = 0; Index < HiddenLayerDims.Num(); ++Index)
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "HiddenLayerDims[%d] is %d. Non-positive numbers will lead to bad results", Index, HiddenLayerDims[Index]);
			return EOpFlag::Error;
		}
	}
	return EOpFlag::Success;
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModel::UpdateForTraining()
{
	...
	if (InputDim == 0)
	{
		UE_LOGF(LogNearestNeighborModel, Error, "InputDim is 0. Please add bones to the include list.");
		return EOpFlag::Error;
	}
	UpdateNetworkOutputDim();
	if (OutputDim == 0)
	{
		UE_LOGF(LogNearestNeighborModel, Error, "OutputDim is 0. Please create at least one section.");
		return EOpFlag::Error;
	}
	...
	if (GetNumSections() == 0)
	{
		UE_LOGF(LogNearestNeighborModel, Error, "At least one section is required. Please create a section using the '+' button.");
		return EOpFlag::Error;
	}
	for (FSection* Section : Sections)
	{
...
bool UNearestNeighborModel::LoadOptimizedNetworkFromFile(const FString& Filename)
{
	...
	{
		UE_LOGF(LogNearestNeighborModel, Display, "Loading Network file '%ls'...", *FullPath);
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Failed to load Network file '%ls'!", *FullPath);
			Result->ConditionalBeginDestroy();
		}
	}
	else
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Network file '%ls' does not exist!", *FullPath);
	}
	return false;
}
