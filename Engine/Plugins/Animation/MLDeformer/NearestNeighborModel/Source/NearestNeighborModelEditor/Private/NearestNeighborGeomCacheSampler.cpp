...
namespace UE::NearestNeighborModel
{
	...
	bool FNearestNeighborGeomCacheSampler::CustomSample(int32 Frame)
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Warning, "AnimSequence only has %d keys, but being sampled with key %d", NumKeys, Frame);
			return false;
		}
	...
	EOpFlag FNearestNeighborGeomCacheSampler::GenerateMeshMappings()
	{
		...
		if (SkeletalMeshComponent && SkeletalMesh && GeometryCacheComponent && GeometryCache)
		{
			...
			{
				UE_LOGF(LogNearestNeighborModel, Warning, "%ls is skipped because it has different vertex counts in skeletal mesh and geometry cache.", *VertexMisMatchNames[i]);
			}
	...
	EOpFlag FNearestNeighborGeomCacheSampler::CheckMeshMappingsEmpty() const
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "MeshMappings is empty. Unable to match skeletal mesh with geometry cache.");
