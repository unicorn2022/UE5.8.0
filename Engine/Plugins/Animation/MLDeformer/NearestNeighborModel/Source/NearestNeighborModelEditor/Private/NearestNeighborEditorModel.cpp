...
namespace UE::NearestNeighborModel
{
	...
	ETrainingResult FNearestNeighborEditorModel::Train()
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Training frames (%d) must be greater than the number of basis (%d)", NumFrames, NumBasis);
			return ETrainingResult::FailPythonError;
		}
	...
	EOpFlag FNearestNeighborEditorModel::Update()
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Model is not ready for training. Please check if training data is not empty or reload MLDeformer editor.");
			return EOpFlag::Error;
		}
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Morph target set is empty");
	...
	EOpFlag FNearestNeighborEditorModel::CheckNetwork()
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Network is empty. Please (re-)train model.");
			return EOpFlag::Error;
		}
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Network output dimension %d does not equal to total number of basis %d. Network needs to be re-trained.", NumNetworkWeights, NumBasis);
			return EOpFlag::Error;
		}
	...
	EOpFlag FNearestNeighborEditorModel::UpdateNearestNeighborData()
	{
		...
		if (!IsTrained())
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Network is not trained. Nearest neighbor data cannot be updated.");
			return EOpFlag::Error;
		}
		if (NearestNeighborModel->GetNumNetworkOutputs() != NearestNeighborModel->GetTotalNumBasis())
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Network output dimension %d does not equal to total number of basis %d. Please re-train network.", NearestNeighborModel->GetNumNetworkOutputs(), NearestNeighborModel->GetTotalNumBasis());
			return EOpFlag::Error;
		}
	...
	EOpFlag FNearestNeighborEditorModel::UpdateMorphDeltas()
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "SkeletalMesh is empty. No morph targets are generated");
			return EOpFlag::Error;
		}
	
		if (Model->GetVertexMap().IsEmpty())
		{
			UE_LOGF(LogNearestNeighborModel, Error, "VertexMap of the skeletal mesh is empty. No morph targets are generated");
			return EOpFlag::Error;
		}
		...
		if (NumSections == 0)
		{
			UE_LOGF(LogNearestNeighborModel, Error, "There are no cloth sections. No morph targets are generated");
			return EOpFlag::Error;
		}
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Vertex count mismatch: imported model of SkeletalMesh has %d vertices and cached SkeletalMesh has %d vertices", NumImportedModelVerts, NumBaseMeshVerts);
			return EOpFlag::Error;
		}
		...
		for (int32 SectionIndex = 0; SectionIndex < NearestNeighborModel->GetNumSections(); ++SectionIndex)
		{
			...
			{
				UE_LOGF(LogNearestNeighborModel, Warning, "Section %d has empty vertex map. No morph targets are generated for this section", SectionIndex);
		...
		for (int32 SectionIndex = 0; SectionIndex < NearestNeighborModel->GetNumSections(); ++SectionIndex)
		{
			...
			{
				UE_LOGF(LogNearestNeighborModel, Warning, "Section %d is not ready for inference. No morph targets are generated for this section", SectionIndex);
				Result |= EOpFlag::Warning;
				continue;
			}
		...
		if (Deltas.Num() == 0)
		{
			UE_LOGF(LogNearestNeighborModel, Error, "All sections are empty. No morph targets are generated.");
			return EOpFlag::Error;
		}
