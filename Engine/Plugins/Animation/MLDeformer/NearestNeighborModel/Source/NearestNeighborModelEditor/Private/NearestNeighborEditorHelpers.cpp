...
void UNearestNeighborAnimStream::Init(USkeleton* InSkeleton)
{
	if (!InSkeleton)
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Skeleton is None when initializing NearestNeighborAnimStream");
		return;
	}
	Skeleton = InSkeleton;
	const FReferenceSkeleton& ReferenceSkeleton = Skeleton->GetReferenceSkeleton();
...
bool UNearestNeighborAnimStream::AppendFrames(const UAnimSequence* Anim, TArray<int32> Frames)
{
	...
	for (int32 Index = 0; Index < NewFrames; ++Index)
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Frame %d not found in AnimSequence", Frame);
			return false;
		}
...
void UNearestNeighborGeometryCacheStream::Init(const UGeometryCache* InTemplateCache)
{
	if (!InTemplateCache)
	{
		UE_LOGF(LogNearestNeighborModel, Error, "TemplateCache is None when initializing NearestNeighborGeometryCacheStream");
		return;
	}
	...
	for (int32 TrackIndex = 0; TrackIndex < TemplateNumTracks; ++TrackIndex)
	{
		...
		if (NumVertices == 0)
		{
			UE_LOGF(LogNearestNeighborModel, Warning, "Track %d has no vertices", TrackIndex);
		}
...
bool UNearestNeighborGeometryCacheStream::AppendFrames(const UGeometryCache* Cache, TArray<int32> Frames)
{
	...
	{
		UE_LOGF(LogNearestNeighborModel, Error, "Number of tracks in Cache (%d) is different from TemplateCache (%d)", NumTracks, TemplateNumTracks);
		return false;
	}
	for (int32 Frame : Frames)
	{
		...
		{
			UE_LOGF(LogNearestNeighborModel, Error, "Frame %d not found in GeometryCache", Frame);
			return false;
		}
		for (int32 TrackIndex = 0; TrackIndex < NumTracks; ++TrackIndex)
		{
			...
			{
				UE_LOGF(LogNearestNeighborModel, Error, "Frame %d cannot be retrieved in GeometryCache", Frame);
				return false;
			}
			TArray<FVector3f>& Positions = MeshData.Positions;
			if (Positions.Num() != TemplateTrackNumVertices[TrackIndex])
			{
				UE_LOGF(LogNearestNeighborModel, Error, "Number of vertices in Frame %d of Track %d (%d) is different from TemplateCache (%d)", Frame, TrackIndex, Positions.Num(), TemplateTrackNumVertices[TrackIndex]);
				return false;
			}
...
bool UNearestNeighborGeometryCacheStream::ToGeometryCache(UGeometryCache* OutCache)
{
	if (!OutCache)
	{
		UE_LOGF(LogNearestNeighborModel, Error, "OutCache is None when converting NearestNeighborGeometryCacheStream to GeometryCache");
		return false;
	}
	if (!IsValid())
	{
		UE_LOGF(LogNearestNeighborModel, Error, "TemplateCache is None when converting NearestNeighborGeometryCacheStream to GeometryCache");
		return false;
	}
