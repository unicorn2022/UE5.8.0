...
FRewindDebuggerAnimation::FSpawnedMeshComponentInfo* FRewindDebuggerAnimation::SpawnMesh(uint64 ObjectId, const IGameplayProvider* GameplayProvider)
{
	...
	if (UWorld* World = RewindDebugger->GetWorldToVisualize())
	{
		...
		{
			ComponentName = ObjectInfo->Name;
		}
		
		MeshComponentInfo->Component = NewObject<USkeletalMeshComponent>(MeshComponentInfo->Actor.Get(), ComponentName);
			...
			if (bFrameFound)
			{
				...
				if (AnimationProvider && GameplayProvider)
				{
					...
					// - apply the recorded pose for the current Frame
					{
						...
						const bool ResetOrSpawnMeshes = !RewindDebugger->IsPIESimulating() && RewindDebugger->GetAnalysisSession() != nullptr;

						TraceServices::FProviderReadScopeLock AnimProviderReadScope(*AnimationProvider);
						...
						[this, ResetOrSpawnMeshes, RewindDebugger, &Frame, AnimationProvider, GameplayProvider, &bSearchForActor, &DebuggedObjects, &bNewActor](uint64 ObjectId, const IAnimationProvider::SkeletalMeshPoseTimeline& TimelineData)
						{
							if (bSearchForActor)
							{
