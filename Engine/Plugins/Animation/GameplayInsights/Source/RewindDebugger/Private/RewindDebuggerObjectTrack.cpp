...
bool FRewindDebuggerObjectTrack::HandleDoubleClickInternal()
{
	...
	if (const TraceServices::IAnalysisSession* Session = RewindDebugger->GetAnalysisSession())
	{
		...
		// Cache the class hierarchy names under the lock
		TArray<FName> ClassHierarchyNames;
		{
			TraceServices::FProviderReadScopeLock ProviderReadScope(*GameplayProvider);
			...
			{
				const FClassInfo& ClassInfo = GameplayProvider->GetClassInfo(ClassId);
				ClassHierarchyNames.Add(ClassInfo.Name);
				ClassId = ClassInfo.SuperId;
			}
		}

		// Iterate handlers outside the lock since HandleDoubleClick may perform heavy operations
		bool bHandled = false;
		const int32 NumExtensions = ModularFeatures.GetModularFeatureImplementationCount(HandlerFeatureName);

		for (int32 HierarchyIndex = 0; HierarchyIndex < ClassHierarchyNames.Num() && !bHandled; ++HierarchyIndex)
		{
			for (int32 ExtensionIndex = 0; ExtensionIndex < NumExtensions; ++ExtensionIndex)
			{
				...
				if (Handler->GetTargetTypeName() == ClassHierarchyNames[HierarchyIndex])
				{
...
TOptional<double> FRewindDebuggerObjectTrack::GetStepFrameTimeInternal(const EStepMode StepMode, const FScrubTimeInformation& CurrentScrubTime) const
{
...
bool FRewindDebuggerObjectTrack::UpdateInternal()
{
	...
	const IGameplayProvider* GameplayProvider = Session->ReadProvider<IGameplayProvider>("GameplayProvider");
	{
		...
		const TRange<double> Existence = GameplayProvider->GetObjectRecordingLifetime(ObjectId);

		ExistenceRange->EventWindows.SetNum(0, EAllowShrinking::No);
		if (Existence.HasLowerBound() && Existence.HasUpperBound())
		{
			...
			ExistenceRange->EventWindows.Add({ Existence.GetLowerBoundValue(), EndTime, LOCTEXT("Object Existence","Object Existence"), LOCTEXT("Object Existence","Object Existence"), FLinearColor(0.1f,0.11f,0.1f) });
		}
	...
	// add child objects
	{
		...
		auto AddChildObject = [this, &FoundObjects, &bChanged, &ViewRange, GameplayProvider](const FObjectId& SubObjectId)
		{
		...
		TraceServices::FProviderReadScopeLock ProviderReadScope(*GameplayProvider);
		GameplayProvider->EnumerateSubobjects(ObjectId, [this, AddChildObject](const FObjectId& SubObjectId)
		{
			AddChildObject(SubObjectId);
		});

#if OBJECT_TRACE_ENABLED
		// for now we'll hard code the level sequence special case.
		// the level sequence is not part of the same world - and it is not using an outer
		// relationship to the level sequence actor. We introduce a fake parenting relationship
		// here to allow inspection of the level sequence under the level sequence actor.
		if (const FObjectInfo* ObjectInfo = GameplayProvider->FindObjectInfo(ObjectId))
		{
			if (const FClassInfo* LevelSequenceActorClassInfo = GameplayProvider->FindClassInfo(*ALevelSequenceActor::StaticClass()->GetPathName()))
			{
				const uint64 LevelSequenceActorClassId = LevelSequenceActorClassInfo->Id;
				bool bIsLevelSequenceActor = false;
	
				uint64 ClassId = ObjectInfo->ClassId;
				while (ClassId != FObjectId::InvalidId)
				{
					if (ClassId == LevelSequenceActorClassId)
					{
						bIsLevelSequenceActor = true;
						break;
					}
					const FClassInfo& ClassInfo = GameplayProvider->GetClassInfo(ClassId);
					ClassId = ClassInfo.SuperId;
				...
				if (bIsLevelSequenceActor)
				{
					const ALevelSequenceActor* LevelSequenceActor = Cast<ALevelSequenceActor>(FObjectTrace::GetObjectFromId(ObjectId.GetMainId()));
					if (LevelSequenceActor)
					{
						const FObjectId LevelSequenceObjectId(FObjectTrace::GetObjectId(LevelSequenceActor->LevelSequenceAsset));
						AddChildObject(LevelSequenceObjectId);
					}
				}
			}
		}
	...
	if (bChanged)
	{
		...
		if (UserSettings->ObjectTrackSortMode == ERewindDebuggerObjectSortMode::Name)
		{
		...
		{
			// sort child object tracks by Id which will generally make older objects first because ids are allocated as increasing integers
			Children.Sort([](const TSharedPtr<FRewindDebuggerTrack>& A, const TSharedPtr<FRewindDebuggerTrack>& B)
				{
					return A->GetUObjectId() < B->GetUObjectId();
				});
		}
	}

	{
		...
		for (const TSharedPtr<FRewindDebuggerTrack>& Child : Children)
		{
			if (Child->Update())
			{
				UE_LOGF(LogRewindDebugger, Verbose, "List changed by: '%ls'", *Child->GetDisplayName().ToString());
				bChanged = true;
			}
		}
	}
	{
		...
		for (FTrackCreatorAndTrack& Creator : ChildTrackCreators)
		{
			if (Creator.Track.IsValid())
			{
				...
				{
					UE_LOGF(LogRewindDebugger, Verbose, "List changed by: '%ls'", *Creator.Track->GetDisplayName().ToString());
					bChanged = true;
				}
			}
		}
