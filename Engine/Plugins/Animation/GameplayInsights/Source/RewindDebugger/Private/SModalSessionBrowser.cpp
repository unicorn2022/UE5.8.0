...
SModalSessionBrowser::FTraceSessionInfo SModalSessionBrowser::GetSelectedTraceInfo()
{
	if (ensure(CurrentTraceSessionSelected.IsValid()))
	{
		...
		{
			UE_LOGF(LogSessionBrowser, Error, "Selected session [%ls] is no longer available", *CurrentTraceSessionSelected.ToString())
		}
	}
	else
	{
		UE_LOGF(LogSessionBrowser, Error, "Selected session [%ls] is invalid", *CurrentTraceSessionSelected.ToString())
	}
...
void SModalSessionBrowser::UpdateCurrentSessionInfoMap()
{
	...
	const TUniquePtr<FStoreClient> StoreClient = FStoreClient::Create(*CurrentTraceStoreAddress);

	if (!StoreClient)
	{
		UE_LOGF(LogSessionBrowser, Error, "Failed to connect to trace store at [%ls]", *CurrentTraceStoreAddress)
		return;
	}
