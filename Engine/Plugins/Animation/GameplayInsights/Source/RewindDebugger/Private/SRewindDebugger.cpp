...
SRewindDebugger::~SRewindDebugger()
{
	...
	// Sessions discovery no longer needed after closing the main debugger window
	using namespace UE::RewindDebugger;
	if (TSharedPtr<UE::TraceBasedDebuggers::FRemoteSessionsManager> RemoteSessionManager = FRewindDebuggerEngineEditorBridge::Get().GetSessionsManager())
	{
		RemoteSessionManager->StopSessionDiscovery(DebuggerGuid);
	}
...
void SRewindDebugger::CenterView(const double Time)
{
	const double ViewSize = ViewRange.GetUpperBoundValue() - ViewRange.GetLowerBoundValue();
	const double HalfViewSize = ViewSize * 0.5;
	SetViewRange(TRange<double>(Time - HalfViewSize, Time + HalfViewSize));
}
...
void SRewindDebugger::SortObjectsByName()
{
	Settings.ObjectTrackSortMode = ERewindDebuggerObjectSortMode::Name;
	ForceRefreshTracks.ExecuteIfBound();
}

bool SRewindDebugger::ShouldSortObjectsByName() const
{
	return Settings.ObjectTrackSortMode == ERewindDebuggerObjectSortMode::Name;
}

void SRewindDebugger::SortObjectsByTime()
{
	Settings.ObjectTrackSortMode = ERewindDebuggerObjectSortMode::Time;
	ForceRefreshTracks.ExecuteIfBound();
}

bool SRewindDebugger::ShouldSortObjectsByTime() const
{
	return Settings.ObjectTrackSortMode == ERewindDebuggerObjectSortMode::Time;
}
...
TSharedRef<SWidget> SRewindDebugger::MakeObjectSelectionMenu()
{
	...
	if (const IRewindDebugger* RewindDebugger = IRewindDebugger::Instance())
	{
		if (const TraceServices::IAnalysisSession* Session = RewindDebugger->GetAnalysisSession())
		{
			...
			if (GameplayProvider)
			{
				// Gather allowed type names from settings outside the provider lock
				TArray<FString> AllowedTypeNames;
	
				const URewindDebuggerProjectSettings* ProjectSettings = GetDefault<URewindDebuggerProjectSettings>();
				Algo::Transform(ProjectSettings->SelectorAllowedTypes, AllowedTypeNames, [](const FSoftClassPath& Path) { return Path.ToString(); });
				Algo::Transform(ProjectSettings->SelectorAllowedStructTypes, AllowedTypeNames, [](const FInstancedStruct& Type) { return GetPathNameSafe(Type.GetScriptStruct()); });
	
				const URewindDebuggerSettings* UserSettings = GetDefault<URewindDebuggerSettings>();
				Algo::Transform(UserSettings->SelectorAllowedTypes, AllowedTypeNames, [](const FSoftClassPath& Path) { return Path.ToString(); });
				Algo::Transform(UserSettings->SelectorAllowedStructTypes, AllowedTypeNames, [](const FInstancedStruct& Type) { return GetPathNameSafe(Type.GetScriptStruct()); });
	
				// Cache object data from provider under a tight lock scope
				struct FCachedObjectEntry
				{
					FString Name;
					FSlateIcon Icon;
				};
				TArray<FCachedObjectEntry> CachedObjects;
	
				{
					TraceServices::FProviderReadScopeLock ProviderReadScope(*GameplayProvider);
	
					TArray<const uint64> AllowedTypeIds;
					AllowedTypeIds.Reserve(AllowedTypeNames.Num());
					for (const FString& AllowedTypeName : AllowedTypeNames)
					{
						if (const FClassInfo* ClassInfo = GameplayProvider->FindClassInfo(*AllowedTypeName))
						{
							AllowedTypeIds.AddUnique(ClassInfo->Id);
						}
					}
	
					auto IsClassOrChildOfAllowed = [&AllowedTypeIds, GameplayProvider](const uint64 ClassId)
						{
							if (AllowedTypeIds.Num() > 0)
							{
								for (const uint64 AllowedTypeId : AllowedTypeIds)
								{
									if (GameplayProvider->IsSubClassOf(ClassId, AllowedTypeId))
									{
										return true;
									}
								}
							...
							// Allow everything if allowed class list is not used
							return true;
						};
	
					GameplayProvider->EnumerateObjects([&CachedObjects, GameplayProvider, &IsClassOrChildOfAllowed](const FObjectInfo& ObjectInfo)
						{
							if (!IsClassOrChildOfAllowed(ObjectInfo.ClassId))
							{
								return;
							}
	
							CachedObjects.Add({ ObjectInfo.Name, GameplayProvider->FindIconForClass(ObjectInfo.ClassId) });
						});
				}
	
				// Build menu entries outside the lock
				for (const FCachedObjectEntry& Entry : CachedObjects)
				{
					const FText SelectedLabel = FText::FromString(Entry.Name);
	
					MenuBuilder.AddMenuEntry(SelectedLabel, FText(), Entry.Icon, FExecuteAction::CreateLambda([this, Name = Entry.Name]()
						{
							FSlateApplication::Get().DismissAllMenus();
...
{
	ForceRefreshTracks = InArgs._ForceRefreshTracks;
	...
	TSharedPtr<SWidget> StatusBarWidget = SNullWidget::NullWidget;
	...
	{
		// Start remote sessions discovery
		RemoteSessionManager->StartSessionDiscovery(DebuggerGuid);

		// Add status bar for connection statistics updates
		if (UStatusBarSubsystem* StatusBarSubsystem = GEditor ? GEditor->GetEditorSubsystem<UStatusBarSubsystem>() : nullptr)
		{
			...
			constexpr ECreateStatusBarOptions StatusBarOptions =
				ECreateStatusBarOptions::HideContentBrowser
				| ECreateStatusBarOptions::HideOutputLog
				| ECreateStatusBarOptions::HideSourceControl;

			StatusBarWidget = StatusBarSubsystem->MakeStatusBarWidget(FRewindDebuggerModule::MainStatusBarName, ConstructUnderMajorTab, StatusBarOptions);

			const FName StatusBarToolbarID = FName(FRewindDebuggerModule::MainStatusBarName.ToString() + TEXT(".Toolbar"));
			if (UToolMenu* StatusToolBarMenu = UToolMenus::Get()->ExtendMenu(StatusBarToolbarID))
			{
				...
				ConnectionStatusSection.AddEntry(FToolMenuEntry::InitWidget("ConnectionStatusBar"
					, SNew(SConnectionStatusBar, UE::RewindDebugger::DebuggerGuid, RemoteSessionManager.ToSharedRef())
					, FText::GetEmpty()
					, bNoIndent
					, bSearchable));
			}
	...
	const TSharedRef<SWidget> ToolBar = UToolMenus::Get()->GenerateWidget(FRewindDebuggerModule::MainToolBarName, ToolMenuContext);
	const TSharedRef<SWidget> RightToolBar = UToolMenus::Get()->GenerateWidget(FRewindDebuggerModule::RightToolBarName, ToolMenuContext);
...
bool FilterTrack(const TSharedPtr<RewindDebugger::FRewindDebuggerTrack>& Track, const FString& FilterString, bool bRemoveNoData, const TArray<FName>& FilteredTrackTypes, bool bParentFilterPassed = false)
{
	...
	Track->IterateSubTracks([&bAnyChildVisible, bThisFilterPassed, FilterString, bRemoveNoData, FilteredTrackTypes](const TSharedPtr<FRewindDebuggerTrack>& ChildTrack)
		{
...
TSharedRef<SWidget> SRewindDebugger::MakeFilterMenu()
{
	...
	Builder.AddMenuEntry(
		LOCTEXT("Sort Objects By Name", "Sort Objects By Name"),
		LOCTEXT("Sort Objects By Name Tooltip", "Object Tracks will be sorted alphabetically"),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateRaw(this, &SRewindDebugger::SortObjectsByName),
			FCanExecuteAction(),
			FIsActionChecked::CreateRaw(this, &SRewindDebugger::ShouldSortObjectsByName)),
		NAME_None,
		EUserInterfaceActionType::ToggleButton
	);
		Builder.AddMenuEntry(
    		LOCTEXT("Sort Objects By Id", "Sort Objects By Time"),
    		LOCTEXT("Sort Objects By Id Tooltip", "Object Tracks will be sorted by Id, making older objects display first"),
    		FSlateIcon(),
    		FUIAction(
    			FExecuteAction::CreateRaw(this, &SRewindDebugger::SortObjectsByTime),
    			FCanExecuteAction(),
    			FIsActionChecked::CreateRaw(this, &SRewindDebugger::ShouldSortObjectsByTime)),
    		NAME_None,
    		EUserInterfaceActionType::ToggleButton
    	);
