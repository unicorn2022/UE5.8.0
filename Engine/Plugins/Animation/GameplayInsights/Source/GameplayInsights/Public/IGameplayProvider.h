...
class IGameplayProvider : public TraceServices::IProvider, public TraceServices::IEditableProvider
{
public:
	...
	virtual const UStruct* FindType(uint64 InTypeId, bool bSearchBlueprints) const = 0;
	UE_DEPRECATED(5.8, "Use FindType returning a UStruct instead.")
	virtual const UClass* FindClass(const uint64 InClassId, const bool bSearchBlueprints) const final
	{
		return Cast<UClass>(FindType(InClassId, bSearchBlueprints));
	}
