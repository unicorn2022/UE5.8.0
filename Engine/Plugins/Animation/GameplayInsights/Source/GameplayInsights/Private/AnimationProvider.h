...
namespace TraceServices
{
	...
	extern thread_local FProviderLock::FThreadLocalState GAnimationProviderLockState;
}
class FGameplayProvider;
struct FObjectInfo;
class USkeletalMesh;

class FAnimationProvider : public IAnimationProvider, public TraceServices::IEditableProvider
{
public:
	...
	{
		Lock.BeginRead(TraceServices::GAnimationProviderLockState);
	}

	virtual void EndRead() const override
	{
		Lock.EndRead(TraceServices::GAnimationProviderLockState);
	}

	virtual void ReadAccessCheck() const override
	{
		Lock.ReadAccessCheck(TraceServices::GAnimationProviderLockState);
	}

	//~ IEditableProvider (edit lock)
	virtual void BeginEdit() const override
	{
		Lock.BeginWrite(TraceServices::GAnimationProviderLockState);
	}

	virtual void EndEdit() const override
	{
		Lock.EndWrite(TraceServices::GAnimationProviderLockState);
	}

	virtual void EditAccessCheck() const override
	{
		Lock.WriteAccessCheck(TraceServices::GAnimationProviderLockState);
	}
	...
	void AppendTickRecord(uint64 InAnimInstanceId, double InProfileTime, double InRecordingTime, uint64 InAssetId, int32 InNodeId, float InBlendWeight, float InPlaybackTime, float InRootMotionWeight, float InPlayRate, float InBlendSpacePositionX, float InBlendSpacePositionY, float InBlendSpaceFilteredPositionX, float InBlendSpaceFilteredPositionY, uint16 InFrameCounter, bool bInLooping, bool bInIsBlendSpace, uint32 InSyncGroupName, bool bInIsSyncGroupLeader, float InSyncLeaderScore, float TimeToPrevMarker, float TimeToNextMarker, int32 PrevMarkerIndex, int32 NextMarkerIndex);
...
private:
	...
	void UpdateSessionDurationSeconds(double InTime) const;
