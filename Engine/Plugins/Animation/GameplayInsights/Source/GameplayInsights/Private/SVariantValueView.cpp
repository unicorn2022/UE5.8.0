...
{ 
	...
	case EAnimNodeValueType::Object:
	{
		...
		if(GameplayProvider)
		{
			...
			return
				SNew(SHyperlink)
				.Text(FText::FromString(ObjectName))
				.TextStyle(&FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("SmallText"))
				.ToolTipText(FText::Format(LOCTEXT("AssetHyperlinkTooltipFormat", "Open asset '{0}'"), FText::FromString(ObjectPathName)))
				.OnNavigate_Lambda([ObjectPathName, InValue]()
				{
					UObject* Asset = nullptr;

					FString PackagePathString = FPackageName::ObjectPathToPackageName(ObjectPathName);
					...
					if (Package)
					{
						...
						FString AssetName = FPaths::GetBaseFilename(ObjectPathName);
						Asset = FindObject<UObject>(Package, *AssetName);
					}
					else
					{
						// fallback for unsaved assets
						Asset = FindObject<UObject>(nullptr, *ObjectPathName);
					}
			...
			return
				...
				.Font(FCoreStyle::Get().GetFontStyle("SmallFont"))
				.Text(FText::FromString(ObjectName))
				.ToolTipText(FText::FromString(ObjectPathName))
				.HighlightText(InHighlightText);
#endif
		}
	}
	...
	case EAnimNodeValueType::Class:
	{
		...
		if(GameplayProvider)
		{
			...
			return
				...
				.TextStyle(&FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("SmallText"))
				.ToolTipText(FText::Format(LOCTEXT("ClassHyperlinkTooltipFormat", "Open class '{0}'"), FText::FromString(ClassPathName)))
				.OnNavigate_Lambda([ClassName, ClassPathName]()
				{
					...
					GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(ClassPathName);
				});
#else
			return
				SNew(STextBlock)
				.Font(FCoreStyle::Get().GetFontStyle("SmallFont"))
				.Text(FText::FromString(ClassName))
				.ToolTipText(FText::FromString(ClassPathName))
				.HighlightText(InHighlightText);
#endif
		}
	}
	...
	case EAnimNodeValueType::AnimNode:
	{
		...
		if (GameplayProvider && AnimationProvider)
		{
			...
			FString AnimInstanceClassPathName;
			{
				...
				{
					AnimNodeName = AnimNodeInfo->Name;
					AnimNodeId = AnimNodeInfo->Id;
					AnimInstanceClassPathName = AnimInstanceClassInfo->PathName;
				}
			}

			if (AnimNodeId != INDEX_NONE)
			{
#if WITH_EDITOR
				return
					SNew(SHyperlink)
					.Text(FText::FromString(AnimNodeName))
					.TextStyle(&FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("SmallText"))
					.ToolTipText(FText::Format(LOCTEXT("NodeHyperlinkTooltipFormat", "Open node '{0}'"), FText::FromString(AnimNodeName)))
					.OnNavigate_Lambda([AnimNodeId, AnimInstanceClassPathName, InValue]()
					{
				...
				return
					...
					.Font(FCoreStyle::Get().GetFontStyle("SmallFont"))
					.Text(FText::FromString(AnimNodeName))
					.ToolTipText(FText::FromString(AnimNodeName))
					.HighlightText(InHighlightText);
#endif
			}
		}
