...
void FAnimationAnalyzer::OnAnalysisBegin(const FOnAnalysisContext& Context)
{
	...
	Builder.RouteEvent(RouteId_TickRecord3, "Animation", "TickRecord3");
...
bool FAnimationAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	FProviderEditScopeLock ProviderEditScope(AnimationProvider);

	const auto& EventData = Context.EventData;
	switch (RouteId)
	{
	case RouteId_TickRecord3:
	{
		...
		uint32 SyncGroupNameId = EventData.GetValue<uint32>("SyncGroupNameId");
		bool IsSyncGroupLeader = EventData.GetValue<bool>("IsSyncGroupLeader");
		float SyncLeaderScore = EventData.GetValue<float>("SyncLeaderScore");
		float TimeToPrevMarker = EventData.GetValue<float>("TimeToPrevMarker");
		float TimeToNextMarker = EventData.GetValue<float>("TimeToNextMarker");
		int32 PrevMarkerIndex = EventData.GetValue<int32>("PrevMarkerIndex");
		int32 NextMarkerIndex = EventData.GetValue<int32>("NextMarkerIndex");
			
		AnimationProvider.AppendTickRecord(AnimInstanceId, Context.EventTime.AsSeconds(Cycle), RecordingTime, AssetId, NodeId, BlendWeight, PlaybackTime, RootMotionWeight, PlayRate, BlendSpacePositionX, BlendSpacePositionY, BlendSpaceFilteredPositionX, BlendSpaceFilteredPositionY, FrameCounter, bLooping, bIsBlendSpace, SyncGroupNameId, IsSyncGroupLeader, SyncLeaderScore, TimeToPrevMarker, TimeToNextMarker, PrevMarkerIndex, NextMarkerIndex);
		break;
	}
