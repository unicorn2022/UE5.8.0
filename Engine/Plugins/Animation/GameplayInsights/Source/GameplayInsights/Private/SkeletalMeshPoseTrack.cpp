...
void FSkeletalMeshPoseTrack::InitTooltip(FTooltipDrawState& Tooltip, const ITimingEvent& HoveredTimingEvent) const
{
	...
	FindSkeletalMeshPoseMessage(SearchParameters, [this, &Tooltip](double InFoundStartTime, double InFoundEndTime, uint32 InFoundDepth, const FSkeletalMeshPoseMessage& InMessage)
	{
		...
		FString PathName;
		{
			...
			if (SkeletalMeshObjectInfo)
			{
				PathName = SkeletalMeshObjectInfo->PathName;
			}
		}

		if (!PathName.IsEmpty())
		{
			Tooltip.AddNameValueTextLine(LOCTEXT("Mesh", "Mesh").ToString(), PathName);

			FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
			if(!AssetRegistryModule.Get().GetAssetByObjectPath(FSoftObjectPath(PathName)).IsValid())
			{
...
void FSkeletalMeshPoseTrack::BuildContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	{
		FString ActorName;
		RewindDebugger::FObjectId ActorObjectId;
		{
			TraceServices::FProviderReadScopeLock ProviderReadScope(*GameplayProvider);
			...
			if(ComponentObjectInfo != nullptr)
			{
				...
				{
					ActorName = ActorObjectInfo->Name;
					ActorObjectId = ActorObjectInfo->GetId();
				}
			}
		}

		if(ActorObjectId.IsSet())
		{
			{
				MenuBuilder.BeginSection(TEXT("DrawingSection"), FText::Format(LOCTEXT("DrawingActor", "Drawing ({0})"), FText::FromString(ActorName)));
				{
					MenuBuilder.AddMenuEntry(
						...
						FSlateIcon(),
						FUIAction(
							FExecuteAction::CreateLambda([this, ActorObjectId, GameplayProvider]()
							{
								TraceServices::FProviderReadScopeLock ProviderReadScope(*GameplayProvider);

								bool bSetDrawPose = true;
								SharedData.EnumerateSkeletalMeshPoseTracks([GameplayProvider, ActorObjectId, &bSetDrawPose](const TSharedRef<FSkeletalMeshPoseTrack>& InTrack)
								{
									...
									if(OtherComponentObjectInfo && OtherComponentObjectInfo->GetOuterId() == ActorObjectId)
									{
							...
							FCanExecuteAction(),
							FIsActionChecked::CreateLambda([this, ActorObjectId, GameplayProvider]()
							{
								TraceServices::FProviderReadScopeLock ProviderReadScope(*GameplayProvider);

								bool bDrawPoseSet = true;
								SharedData.EnumerateSkeletalMeshPoseTracks([GameplayProvider, ActorObjectId, &bDrawPoseSet](const TSharedRef<FSkeletalMeshPoseTrack>& InTrack)
								{
									const FObjectInfo* OtherComponentObjectInfo = GameplayProvider->FindObjectInfo(InTrack->GetGameplayTrack().GetAssociatedObjectId());
									if(OtherComponentObjectInfo && OtherComponentObjectInfo->GetOuterId() == ActorObjectId)
									{
					...
					MenuBuilder.AddMenuEntry(
						...
						FUIAction(
							FExecuteAction::CreateLambda([this, ActorObjectId, GameplayProvider]()
							{
								...
								SharedData.EnumerateSkeletalMeshPoseTracks([GameplayProvider, ActorObjectId, &bSetDrawSkeleton](const TSharedRef<FSkeletalMeshPoseTrack>& InTrack)
								{
									const FObjectInfo* OtherComponentObjectInfo = GameplayProvider->FindObjectInfo(InTrack->GetGameplayTrack().GetAssociatedObjectId());
									if(OtherComponentObjectInfo && OtherComponentObjectInfo->GetOuterId() == ActorObjectId)
									{
							...
							FIsActionChecked::CreateLambda([this, ActorObjectId, GameplayProvider]()
							{
								...
								SharedData.EnumerateSkeletalMeshPoseTracks([GameplayProvider, ActorObjectId, &bDrawSkeletonSet](const TSharedRef<FSkeletalMeshPoseTrack>& InTrack)
								{
									const FObjectInfo* OtherComponentObjectInfo = GameplayProvider->FindObjectInfo(InTrack->GetGameplayTrack().GetAssociatedObjectId());
									if(OtherComponentObjectInfo && OtherComponentObjectInfo->GetOuterId() == ActorObjectId)
									{
