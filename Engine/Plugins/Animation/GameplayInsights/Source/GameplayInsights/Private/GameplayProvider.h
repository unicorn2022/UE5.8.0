...
extern thread_local FProviderLock::FThreadLocalState GGameplayProviderLockState;
}
...
public:
	...
	explicit FGameplayProvider(TraceServices::IAnalysisSession& InSession);

	//~ IProvider
	virtual void BeginRead() const override
	{
		Lock.BeginRead(TraceServices::GGameplayProviderLockState);
	}

	virtual void EndRead() const override
	{
		Lock.EndRead(TraceServices::GGameplayProviderLockState);
	}

	virtual void ReadAccessCheck() const override
	{
		Lock.ReadAccessCheck(TraceServices::GGameplayProviderLockState);
	}

	//~ IEditableProvider
	virtual void BeginEdit() const override
	{
		Lock.BeginWrite(TraceServices::GGameplayProviderLockState);
	}

	virtual void EndEdit() const override
	{
		Lock.EndWrite(TraceServices::GGameplayProviderLockState);
	}

	virtual void EditAccessCheck() const override
	{
		Lock.WriteAccessCheck(TraceServices::GGameplayProviderLockState);
	}
	...
	virtual const UStruct* FindType(uint64 InTypeId, bool bSearchBlueprints = true) const override;
...
private:
	...
	/** The target type specified in the current analysis session */
