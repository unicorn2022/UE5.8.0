...
{
thread_local FProviderLock::FThreadLocalState GAnimationProviderLockState;
}
...
void FAnimationProvider::UpdateSessionDurationSeconds(const double InTime) const
{
...
void FAnimationProvider::AppendTickRecord(uint64 InAnimInstanceId, double InProfileTime, double InRecordingTime, uint64 InAssetId, int32 InNodeId, float InBlendWeight, float InPlaybackTime, float InRootMotionWeight, float InPlayRate, float InBlendSpacePositionX, float InBlendSpacePositionY, float InBlendSpaceFilteredPositionX, float InBlendSpaceFilteredPositionY, uint16 InFrameCounter, bool bInLooping, bool bInIsBlendSpace)
{
	...
	UpdateSessionDurationSeconds(InProfileTime);
}
...
void FAnimationProvider::AppendTickRecord(uint64 InAnimInstanceId, double InProfileTime, double InRecordingTime, uint64 InAssetId, int32 InNodeId, float InBlendWeight, float InPlaybackTime, float InRootMotionWeight, float InPlayRate, float InBlendSpacePositionX, float InBlendSpacePositionY, float InBlendSpaceFilteredPositionX, float InBlendSpaceFilteredPositionY, uint16 InFrameCounter, bool bInLooping, bool bInIsBlendSpace, uint32 InSyncGroupName, bool bInIsSyncGroupLeader, float InSyncLeaderScore, float TimeToPrevMarker, float TimeToNextMarker, int32 PrevMarkerIndex, int32 NextMarkerIndex)
{
	...
	Message.bLooping = bInLooping;
	Message.bIsBlendSpace = bInIsBlendSpace;
	Message.bSyncGroupNameId = InSyncGroupName;
	Message.bIsSyncGroupLeader = bInIsSyncGroupLeader;
	Message.SyncLeaderScore = InSyncLeaderScore;
	Message.PrevMarkerIndex = PrevMarkerIndex;
	Message.NextMarkerIndex = NextMarkerIndex;
	Message.TimeToNextMarker = TimeToNextMarker;
	Message.TimeToPrevMarker = TimeToPrevMarker;
...
void FAnimationProvider::AppendSkeletalMeshFrame(uint64 InObjectId, double InTime, uint16 InFrameCounter)
{
	...
	UpdateSessionDurationSeconds(InTime);
}
...
void FAnimationProvider::AppendAnimGraph(uint64 InAnimInstanceId, double InStartTime, double InEndTime, int32 InNodeCount, uint16 InFrameCounter, uint8 InPhase)
{
	...
	UpdateSessionDurationSeconds(InStartTime);
}
