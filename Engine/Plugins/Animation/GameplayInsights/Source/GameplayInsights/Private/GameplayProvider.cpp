...
// This is copied from EditorUtilitySubsystem (where it is not public)
// Should probably be somewhere shared
static UStruct* FindTypeByPathName(const FString& RawTargetPathName, const bool bCheckBlueprintClasses)
{
	...
	UStruct* ResultClass = nullptr;
	if (bIsValidClassName)
	{
		ResultClass = FindObject<UStruct>(nullptr, *TargetName);
	}
...
{
thread_local FProviderLock::FThreadLocalState GGameplayProviderLockState;
}
...
const UStruct* FGameplayProvider::FindType(uint64 InTypeId, bool bSearchBlueprints) const
{
#if WITH_EDITOR
	if (const FClassInfo* ClassInfo = FindClassInfo(InTypeId))
	{
		return FindTypeByPathName(ClassInfo->PathName, bSearchBlueprints);
	}
	return nullptr;
#else
	return nullptr;
...
FSlateIcon FGameplayProvider::FindIconForClass(uint64 ClassId) const
{
	...
	while (ClassId)
	{
		...
		if (const UStruct* FoundType = FindType(ClassId, false))
		{
...
void FGameplayProvider::AppendRecordingInfo(uint64 InWorldId, double InProfileTime, uint32 InRecordingIndex, uint32 InFrameIndex, double InElapsedTime)
{
	...
	if (!TargetType.IsSet())
	{
		...
		Session.EnumerateMetadata(
			[&TargetType = TargetType](const TraceServices::FTraceSessionMetadata& Metadata)
			{
				if (Metadata.Name == FName("TargetType"))
				{
					TargetType = static_cast<EBuildTargetType>(Metadata.Int64Value);
				}
			});
	}

	if (const int32* WorldIndex = WorldIdToIndexMap.Find(InWorldId))
	{
		if (WorldInfos.IsValidIndex(*WorldIndex))
		{
			...
			if (WorldInfos[*WorldIndex].NetMode == FWorldInfo::ENetMode::Standalone
				|| WorldInfos[*WorldIndex].NetMode == FWorldInfo::ENetMode::Client
				|| (TargetType == EBuildTargetType::Server
					&& WorldInfos[*WorldIndex].NetMode == FWorldInfo::ENetMode::DedicatedServer))
			{
