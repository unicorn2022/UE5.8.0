...
bool FBlendWeightsTrack::UpdateInternal()
{
	...
	if (AnimationProvider)
	{
...

		{
			...
			AnimationProvider->ReadTickRecordTimeline(ObjectId, [&UniqueTrackIds, StartTime, EndTime](const FAnimationProvider::TickRecordTimeline& InTimeline)
			{
		...
		UniqueTrackIds.StableSort([](const TrackId &A, const TrackId &B)
			{
...
#if WITH_EDITOR
					if (FoundType->IsChildOf(UBlendSpace::StaticClass()))
					{
						bIsBlendSpace = true;	
					}
#endif
...
bool FBlendWeightTrack::HandleDoubleClickInternal()
{
	...
	if (const TraceServices::IAnalysisSession* AnalysisSession = RewindDebugger->GetAnalysisSession())
	{
		...
		{
			TraceServices::FProviderReadScopeLock AnimationProviderReadScope(*AnimationProvider);
			AnimationProvider->ReadTickRecordTimeline(ObjectId, [this, &bMessageFound, &PlaybackTime, &BlendSpaceX, &BlendSpaceY, &Frame](const FAnimationProvider::TickRecordTimeline& InTimeline)
			{
				InTimeline.EnumerateEvents(Frame.StartTime, Frame.EndTime, [this, &bMessageFound, &PlaybackTime, &BlendSpaceX, &BlendSpaceY, &Frame](double InStartTime, double InEndTime, uint32 InDepth, const FTickRecordMessage& InMessage)
				{
