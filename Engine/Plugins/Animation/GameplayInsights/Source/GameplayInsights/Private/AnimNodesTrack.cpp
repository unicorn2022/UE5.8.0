						...
						if(UAnimBlueprint* AnimBlueprint = Cast<UAnimBlueprint>(InstanceClass->ClassGeneratedBy))
						{
							...
							if (GameplayProvider)
							{
								uint64 OuterUObjectId = 0;
								{
									...
									{
										OuterUObjectId = AnimInstanceObjectInfo->GetOuterUObjectId();
									}
								}
								...
								if (OuterUObjectId != 0)
								{
									...
									TSharedPtr<FSkeletalMeshPoseTrack> SkeletalMeshPoseTrack = SharedData.FindSkeletalMeshPoseTrack(OuterUObjectId);
						...
						InGraphTimeline.EnumerateEvents(InFrame.StartTime, InFrame.EndTime, [this, AnimationProvider, GameplayProvider, &DebugData, NodeCount](double InGraphStartTime, double InGraphEndTime, uint32 InDepth, const FAnimGraphMessage& InMessage)
						{
							...
							if(InMessage.NodeCount == NodeCount)
							{
								...
								if(InMessage.Phase == EAnimGraphPhase::Update)
								{
									...
									AnimationProvider->ReadAnimSequencePlayersTimeline(GetGameplayTrack().GetObjectId(), [InGraphStartTime, InGraphEndTime, &DebugData](const FAnimationProvider::AnimSequencePlayersTimeline& InSequencePlayersTimeline)
									{
