...
static void ShowEditorMessage(ELogVerbosity::Type InMessageType, const FText& InMessage, const FText* InLogMessage)
{
	...
	case ELogVerbosity::Warning:
		UE_LOGF(LogSkeletalMeshModelingToolsMeshConverter, Warning, "%ls", InLogMessage ? *InLogMessage->ToString() : *InMessage.ToString());
		break;
	case ELogVerbosity::Error:
		State = SNotificationItem::CS_Fail;
		UE_LOGF(LogSkeletalMeshModelingToolsMeshConverter, Error, "%ls", InLogMessage ? *InLogMessage->ToString() : *InMessage.ToString());
		break;
	default:
		checkNoEntry();
	}
	...
	case EReferenceSkeletonImportOption::UseExistingSkeleton:
		{
			...
			if (!Skeleton)
			{
				UE_LOGF(LogSkeletalMeshModelingToolsMeshConverter, Error, "No valid skeleton given as a reference. Aborting conversion.");
				return false;
			}
			ReferenceSkeleton = Skeleton->GetReferenceSkeleton();
	...
	case EReferenceSkeletonImportOption::UseExistingSkeletalMesh:
		{
			...
			if (!SkeletalMesh)
			{
				UE_LOGF(LogSkeletalMeshModelingToolsMeshConverter, Error, "No valid skeletal mesh given as reference. Aborting conversion.");
				return false;
			}
