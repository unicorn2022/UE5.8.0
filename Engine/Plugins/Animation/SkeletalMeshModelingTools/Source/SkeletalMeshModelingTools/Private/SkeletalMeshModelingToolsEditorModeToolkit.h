...
public:
	...
	void ShowToolSkeletonTree();
	
	void EnableDynamicMeshSkeletonCommands();
	void EnableSkeletalMeshSkeletonCommands();
	// When tool has its own skeleton, the editing cache skeleton is hidden and shouldn't receive commands like reset bone transforms
	void DisableEditingCacheSkeletonCommands();

	bool BindToolSkeletonTree(ISkeletalMeshEditingInterface* InTool);
	void UnbindToolSkeletonTree();
	
	void RefreshDynamicMeshSkeletonTreeIfNeeded();
private:
	void SetupCommands();
	...
	void UpdateSelectionColors(UObject* Object, FPropertyChangedEvent& PropertyChangedEvent) const;
	...
	TSharedPtr<SBorder> DynamicMeshSkeletonWidget;
	TSharedPtr<SReferenceSkeletonTree> DynamicMeshSkeletonTree;

	TSharedPtr<SBorder> ToolSkeletonTreeHost;
	...
	TUniquePtr<FSkeletalMeshNotifierBindScope> DynamicMeshSkeletonTreeNotifierBindScope;
	...
	enum ESkeletonTabState
	{
		DisplayDefaultSkeletalMeshSkeletonTree,
		DisplayDynamicMeshSkeletonTree,
		DisplayToolSkeletonTree,
	};
	
	ESkeletonTabState SkeletonTreeTabState = ESkeletonTabState::DisplayDefaultSkeletalMeshSkeletonTree;
};
