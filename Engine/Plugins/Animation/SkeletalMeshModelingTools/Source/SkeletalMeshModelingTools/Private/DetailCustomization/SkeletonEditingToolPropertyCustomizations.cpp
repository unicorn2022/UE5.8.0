...
void FMirroringPropertiesDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	DetailBuilder.RegisterInstancedCustomPropertyTypeLayout(
		FMirrorOptions::StaticStruct()->GetFName(),
		FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FSKMMTMirroringPropertiesAxisCustomization::MakeInstance));

	const TWeakObjectPtr<UMirroringProperties> Properties = GetCustomizedObject<UMirroringProperties>(DetailBuilder);
	if (!GetParentTool(Properties).IsValid())
	{
		return;
	}
	...
	MirrorCategory.AddCustomRow(LOCTEXT("MirrorButtonRow", "Mirror"))
	[
		SNew(SHorizontalBox)
		+SHorizontalBox::Slot()
		.Padding(2.f, 4.f)
...
void FOrientingPropertiesDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	DetailBuilder.RegisterInstancedCustomPropertyTypeLayout(
		FOrientOptions::StaticStruct()->GetFName(),
		FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FSKMMTOrientPropertiesAxisCustomization::MakeInstance));
	...
	IDetailCategoryBuilder& OrientCategory = DetailBuilder.EditCategory(OrientCategoryName, FText::GetEmpty(), ECategoryPriority::Important);
	OrientCategory.AddCustomRow(LOCTEXT("OrientButtonRow", "Orient"), false)
	[
		SNew(SHorizontalBox)
		+SHorizontalBox::Slot()
		.Padding(2.f, 4.f)
