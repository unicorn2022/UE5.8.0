...
{
	TWeakPtr<FSkinWeightDetailCustomization> WeakThis = SharedThis(this);
	...
	// Mesh Element Display category
	IDetailCategoryBuilder& MeshElementDisplayCategory =
		DetailBuilder.EditCategory("MeshElementDisplay", LOCTEXT("MeshElementDisplayCategory", "Mesh Element Display"), ECategoryPriority::Important);

	auto MakeOnPropertyChangedDelegate = [this, WeakThis](TFunction<void(USkinWeightsPaintToolProperties&, UMeshElementsVisualizerProperties&)> ApplyFn)
	{
		return FSimpleDelegate::CreateLambda([this, WeakThis, ApplyFn]()
		{
			if (!WeakThis.IsValid())
			{
				return;
			}
			USkinWeightsPaintToolProperties* Settings = ToolSettings.Get();
			USkinWeightsPaintTool* WeightTool = Tool.Get();
			if (!Settings)
			{
				return;
			}

			UMeshElementsVisualizer* Visualizer = WeightTool ? WeightTool->GetMeshElementsDisplay() : nullptr;
			if (Visualizer && Visualizer->Settings)
			{
				ApplyFn(*Settings, *Visualizer->Settings);
			}
			Settings->SaveConfig();
		});
	};

	TSharedRef<IPropertyHandle> ShowGroupBoundariesHandle =
		DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(USkinWeightsPaintToolProperties, bShowGroupBoundaries));
	MeshElementDisplayCategory.AddProperty(ShowGroupBoundariesHandle)
		.DisplayName(LOCTEXT("ShowPolygroupEdgesLabel", "Show Polygroup Edges"));
	ShowGroupBoundariesHandle->SetOnPropertyValueChanged(MakeOnPropertyChangedDelegate(
		[](USkinWeightsPaintToolProperties& ToolProperties, UMeshElementsVisualizerProperties& VisualizerProperties)
		{
			VisualizerProperties.bVisible = ToolProperties.bShowGroupBoundaries;
		}));

	TSharedRef<IPropertyHandle> GroupBoundaryColorHandle =
		DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(USkinWeightsPaintToolProperties, GroupBoundaryColor));
	MeshElementDisplayCategory.AddProperty(GroupBoundaryColorHandle)
		.DisplayName(LOCTEXT("PolygroupEdgeColorLabel", "Polygroup Edge Color"));
	GroupBoundaryColorHandle->SetOnPropertyValueChanged(MakeOnPropertyChangedDelegate(
	[](USkinWeightsPaintToolProperties& ToolProperties, UMeshElementsVisualizerProperties& VisualizerProperties)
		{
			VisualizerProperties.GroupBoundaryColor = ToolProperties.GroupBoundaryColor;
		}));

	TSharedRef<IPropertyHandle> GroupBoundaryThicknessHandle =
		DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(USkinWeightsPaintToolProperties, GroupBoundaryThickness));
	MeshElementDisplayCategory.AddProperty(GroupBoundaryThicknessHandle)
		.DisplayName(LOCTEXT("PolygroupEdgeThicknessLabel", "Polygroup Edge Thickness"));
	GroupBoundaryThicknessHandle->SetOnPropertyValueChanged(MakeOnPropertyChangedDelegate(
	[](USkinWeightsPaintToolProperties& ToolProperties, UMeshElementsVisualizerProperties& VisualizerProperties)
		{
			VisualizerProperties.ThicknessScale = ToolProperties.GroupBoundaryThickness;
		}));
	...
	PropertiesToHide.Add(GET_MEMBER_NAME_CHECKED(USkinWeightsPaintToolProperties, bShowGroupBoundaries));
	PropertiesToHide.Add(GET_MEMBER_NAME_CHECKED(USkinWeightsPaintToolProperties, GroupBoundaryColor));
	PropertiesToHide.Add(GET_MEMBER_NAME_CHECKED(USkinWeightsPaintToolProperties, GroupBoundaryThickness));
...
void FSkinWeightDetailCustomization::AddBrushUI(IDetailLayoutBuilder& DetailBuilder) const
{
	...
	[
		...
		[
			...
			.OnValueChanged_Lambda([this](float NewValue)
			{
				if (ToolSettings.IsValid())
				{
					...
					// Honour the user's Sync toggle: lockstep across modes when on,
					// per-mode save when off. OnPropertyModified will also see this via
					// PostEditChangeProperty and applies the same gate; the explicit write
					// here keeps the live slider responsive without depending on that round-trip.
					if (ToolSettings->bSyncBrushRadiusAcrossModes)
					{
						ToolSettings->SetSharedRadius(NewValue);
					}
					else
					{
		...
		[
			...
			{
				const bool bSync = ToolSettings.IsValid() && ToolSettings->bSyncBrushRadiusAcrossModes;
				return bSync ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
			})
			.OnCheckStateChanged_Lambda([this](const ECheckBoxState NewState)
			{
				if (!ToolSettings.IsValid()) return;
				const bool bNewSync = (NewState == ECheckBoxState::Checked);
				ToolSettings->bSyncBrushRadiusAcrossModes = bNewSync;
				// Snap-on-enable: when re-linking, push the live radius to every per-mode config
				// so toggling on never silently drops the active value.
				if (bNewSync)
				{
					ToolSettings->SetSharedRadius(ToolSettings->BrushRadius);
				}
				ToolSettings->SaveConfig();
			})
			.Padding(2.0f)
	...
	{
		const bool bIsRelax = ToolSettings.IsValid() && ToolSettings->BrushMode == EWeightEditOperation::Relax;
	...
	[
		...
		.ToolTipText(LOCTEXT("BrushAccumulateTooltip",
			"When enabled, repeated strokes over the same area continue to smooth. "
			"When disabled, each stroke produces a single bounded result."))
	]
	.ValueContent()
	[
		SNew(SCheckBox)
		...
		{
			const bool bChecked = ToolSettings.IsValid() && ToolSettings->RelaxBrushAdvancedConfig.bAccumulate;
		...
		.OnCheckStateChanged_Lambda([this](ECheckBoxState NewState)
		{
			if (ToolSettings.IsValid())
			{
				ToolSettings->RelaxBrushAdvancedConfig.bAccumulate = (NewState == ECheckBoxState::Checked);
				ToolSettings->SaveConfig();
			}
		})
	];
...
void FSkinWeightDetailCustomization::AddSelectionUI(IDetailLayoutBuilder& DetailBuilder) const
{
	...
	auto AddToggleButtonForBool = [&ToolbarBuilder, this](EComponentSelectionMode Mode, const FText& Label, const FText& Tooltip, const FName IconName)
	{
		...
		FExecuteAction::CreateLambda([this, Mode]()
		{
			if (ToolSettings.IsValid())
			{
				const bool bConvertSelection = FSlateApplication::Get().GetModifierKeys().IsControlDown();
				ToolSettings->SetComponentSelectionMode(Mode, bConvertSelection);
			}
		}),
	...
	AddToggleButtonForBool(
		...
		LOCTEXT("VerticesTooltip", "Select mesh vertices. Ctrl-clicking converts the current selection to vertices."),
		"PolyEd.SelectCorners");
	AddToggleButtonForBool(
		EComponentSelectionMode::Edges,
		LOCTEXT("EdgesLabel", "Edges"),
		LOCTEXT("EdgesTooltip", "Select mesh edges. Ctrl-clicking converts the current selection to edges."),
		"PolyEd.SelectEdges");
	AddToggleButtonForBool(
		EComponentSelectionMode::Faces,
		LOCTEXT("FacesLabel", "Faces"),
		LOCTEXT("FacesTooltip", "Select mesh faces. Ctrl-clicking converts the current selection to faces."),
		"PolyEd.SelectFaces");
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(SCheckBox)
					.Style(FSkeletalMeshModelingToolsStyle::Get(), "SkeletalMeshModelingTools.ToggleButtonCheckbox")
					.HAlign(HAlign_Center)
					...
					.IsEnabled_Lambda([this]()
					{
						if (Tool.IsValid())
						{
						   ...
						   const bool bAlreadyIsolatingSelection = !Tool->GetSelectionIsolator()->CanIsolate();
					...
					[
						...
						.Text_Lambda([this]()
						{
							if (!Tool.IsValid() || !Tool->GetSelectionIsolator()->CanIsolate())
							{
	...
	[
		...
		[
			...
			.ToolTipText(LOCTEXT("PruneButtonTooltip",
				...
				"Pruned bones will no longer receive weight when a vertex is normalized.\n"
				"This command operates on the selected vertices."))
			.OnClicked_Lambda([this]()
			{
				if (ToolSettings.IsValid())
...
TSharedRef<SWidget> SVertexWeightItem::GenerateWidgetForColumn(const FName& ColumnName)
{
	if (ColumnName == ColumnName_Bone)
	{
		...
		return SNew(STextBlock)
			.Text_Lambda([Tool = ParentTable->Tool, BoneName, BoneIndex = Element->BoneIndex]()
				{
					bool bLocked = Tool.IsValid() ? Tool->GetFinalLockedBoneIndices().Contains(BoneIndex) : false;
					return FText::Format(LOCTEXT("VertexWeightBoneNameFormat", "{0}{1}"),
						FText::FromName(BoneName),
						bLocked ? LOCTEXT("VertexWeightBoneNameLockedText", " (Locked)") : FText());
				});
	}
...
void SVertexWeightEditor::Construct(const FArguments& InArgs, USkinWeightsPaintTool* InSkinTool)
{
	...
	[
		...
		[
			...
			.OnSelectionChanged(this, &SVertexWeightEditor::OnSelectionChanged)
...
void SVertexWeightEditor::OnSelectionChanged(TSharedPtr<FWeightEditorElement> Selection, ESelectInfo::Type SelectInfo)
{
	...
	const USkinWeightsPaintTool& ToolRef = *Tool.Get();
	
	if (const TSharedPtr<SReferenceSkeletonTree>& SkeletonTree = ToolRef.GetToolSkeletonTree())
	{
		const FName BoneName = ToolRef.GetBoneNameFromIndex(Selection->BoneIndex);

		constexpr bool bFrameSelection = true;
		SkeletonTree->SelectItemFromNames({ BoneName }, bFrameSelection);
	}
}
