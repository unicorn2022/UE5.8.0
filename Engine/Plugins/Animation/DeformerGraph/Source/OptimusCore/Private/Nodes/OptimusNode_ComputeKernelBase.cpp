...
) const
{
...
#if 0
	UE_LOGF(LogOptimusCore, Log, "Kernel: %ls [%ls]", *GetNodePath(), *GetNodeName().ToString());
	UE_LOGF(LogOptimusCore, Log, "Cooked Source:\n%ls\n", *CookedSource);
	if (!OutInputDataBindings.IsEmpty())
	{
		UE_LOGF(LogOptimusCore, Log, "Input Bindings:");
		for (const TPair<int32, FOptimus_InterfaceBinding>& Binding: OutInputDataBindings)
		{
			...
			UE_LOGF(LogOptimusCore, Log, "  K[%d] %ls -> %ls@%d [%ls]", Binding.Key, *Binding.Value.BindingFunctionName,
	...
	{
		UE_LOGF(LogOptimusCore, Log, "Output Bindings:");
