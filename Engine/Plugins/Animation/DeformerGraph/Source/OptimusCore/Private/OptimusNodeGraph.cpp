	...
	)
{
	...
	for (const UOptimusNode* Node: InNodes)
	{
		...
		{
			UE_LOGF(LogOptimusCore, Warning, "Nodes to duplicate have to all belong to the same graph.");
			return false;
		}
	}
...
UOptimusNode* UOptimusNodeGraph::ConvertCustomKernelToFunction(UOptimusNode* InCustomKernel)
{
	...
	if (!CustomKernelNode)
	{
		UE_LOGF(LogOptimusCore, Error, "%ls: Not a custom kernel node.", *InCustomKernel->GetName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogOptimusCore, Error, "%ls: Need at least one input binding and one output binding.", *CustomKernelNode->GetName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogOptimusCore, Error, "%ls: Node has an error on it.", *CustomKernelNode->GetName());
		return nullptr;
	}
...
UOptimusNode* UOptimusNodeGraph::ConvertFunctionToCustomKernel(UOptimusNode* InKernelFunction)
{
	...
	{
		UE_LOGF(LogOptimusCore, Error, "%ls: Not a kernel function node.", *InKernelFunction->GetName());
		return nullptr;
	}
	...
	)
{
	...
	{
		UE_LOGF(LogOptimusCore, Warning, "Only subgraphs can be added to other graphs");
		return nullptr;
	}
