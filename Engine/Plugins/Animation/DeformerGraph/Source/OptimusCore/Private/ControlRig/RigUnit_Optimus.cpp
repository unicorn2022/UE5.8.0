		...
		struct Local
		{
			static TArray<USkeletalMeshComponent*> GetComponentsToProcess(USkeletalMeshComponent* RigMeshComponent, bool DeformChildComponents, FName ExcludeChildComponentsWithTag)
			{
				...
				if (DeformChildComponents)
				{
					...
					const AActor* RigMeshActor = RigMeshComponent->GetOwner();
					for (USceneComponent* Component : ChildComponents)
					{
						if (USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(Component))
						{
							const AActor* ChildMeshActor = SkeletalMeshComponent->GetOwner();						
#if WITH_EDITOR
							const bool bIsTempActor = ChildMeshActor && ChildMeshActor != RigMeshActor && ChildMeshActor->bIsEditorPreviewActor;
#else
							constexpr bool bIsTempActor = false;
#endif						
							if (!bIsTempActor)
							{
				...
				if (!DeformerTrait->DeformerGraph.IsNull())
				{
					...
					if (UOptimusDeformer* DeformerGraph = DeformerTrait->DeformerGraph.Get())
					{
						ExecuteOnGameThread(UE_SOURCE_LOCATION,
							[AddDeformerToComponentsIfNeeded, WeakDeformer = TWeakObjectPtr<UOptimusDeformer>(DeformerGraph)]()
								{
