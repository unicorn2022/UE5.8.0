...
void FOptimusDataTypeRegistry::RegisterBuiltinTypes()
{
	...
	Registry.RegisterType(
	    *FBoolProperty::StaticClass(),
		...
		[](FFieldVariant InScope, FName InName) {
		    FBoolProperty* Property = new FBoolProperty(InScope, InName);
	...
	Registry.RegisterType(
	    *FIntProperty::StaticClass(),
	    ...
	    [](FFieldVariant InScope, FName InName) {
		    FIntProperty* Property = new FIntProperty(InScope, InName);
	...
	Registry.RegisterType(
		*FUInt32Property::StaticClass(),
		...
		[](FFieldVariant InScope, FName InName) {
			FUInt32Property* Property = new FUInt32Property(InScope, InName);
			Property->SetPropertyFlags(CPF_HasGetValueTypeHash);
			return Property;
		},
		ConvertPropertyValuePOD<uint32, uint32>,
		FName(TEXT("uint")), FLinearColor(0.0275f, 0.733f, 0.820f, 1.0f), 
		EOptimusDataTypeUsageFlags::Resource | EOptimusDataTypeUsageFlags::Variable | EOptimusDataTypeUsageFlags::DataInterfaceOutput | EOptimusDataTypeUsageFlags::PinType);
	...
	*/
	...
	Registry.RegisterType(
	    *FFloatProperty::StaticClass(),
	    ...
	    [](FFieldVariant InScope, FName InName) {
		    FFloatProperty* Property = new FFloatProperty(InScope, InName);
		    Property->SetPropertyFlags(CPF_HasGetValueTypeHash);
#if WITH_EDITOR
	...
	Registry.RegisterType(
	    *FDoubleProperty::StaticClass(),
	    ...
	    FShaderValueType::Get(EShaderFundamentalType::Float),
	    [](FFieldVariant InScope, FName InName) {
		    FDoubleProperty* Property = new FDoubleProperty(InScope, InName);
		    Property->SetPropertyFlags(CPF_HasGetValueTypeHash);
#if WITH_EDITOR
	    	Property->SetMetaData(TEXT("UIMin"), TEXT("0.0"));
			Property->SetMetaData(TEXT("UIMax"), TEXT("1.0"));
	...
	Registry.RegisterType(
		*FNameProperty::StaticClass(),
		...
		[](FFieldVariant InScope, FName InName) {
			FNameProperty* Property = new FNameProperty(InScope, InName);
			return Property;
		},
		{},
		FName(TEXT("name")), {},
	...
	)
{
	...
	{
		UE_LOGF(LogOptimusCore, Error, "Invalid type name.");
		return false;
	}

	if (RegisteredTypes.Contains(InTypeName))
	{
		UE_LOGF(LogOptimusCore, Error, "Type '%ls' is already registered.", *InTypeName.ToString());
		return false;
	}
...
bool FOptimusDataTypeRegistry::RegisterStructType(UScriptStruct* InStructType)
{
	...
	{
		auto Property = new FStructProperty(InScope, InName);
...
bool FOptimusDataTypeRegistry::RegisterArrayTypeIfApplicable(FOptimusDataTypeHandle InElementDataType)
{
	...
	{
		FArrayProperty* ArrayProperty = new FArrayProperty(InScope, InName);
	...
	if (ensure(InStructType))
	{
		...
		if (bInShowElements)
		{
			for (const FProperty* Property : TFieldRange<FProperty>(InStructType))
			{
				...
				{
					UE_LOGF(LogOptimusCore, Error, "Found un-registered sub-element '%ls' when registering '%ls'",
		...
		if (EnumHasAnyFlags(InUsageFlags, EOptimusDataTypeUsageFlags::Variable))
		{
			...
			for (const FProperty* Property : TFieldRange<FProperty>(InStructType))
			{
				...
				{
					UE_LOGF(LogOptimusCore, Error, "Found un-registered sub-element '%ls' when converting '%ls'",
						*(Property->GetClass()->GetName()), *InStructType->GetName());
					return false;
				}
				...
				{
					UE_LOGF(LogOptimusCore, Error, "Sub-element '%ls' has no conversion when converting '%ls'",
						*(Property->GetClass()->GetName()), *InStructType->GetName());
					return false;
				}
	...
	EOptimusDataTypeUsageFlags InUsageFlags)
{
	if (ensure(InStructType))
	{
		...
		if (EnumHasAnyFlags(InUsageFlags, EOptimusDataTypeUsageFlags::Variable))
		{
			...
			PropertyCreateFunc = [bIsHashable, InStructType](FFieldVariant InScope, FName InName) -> FProperty *
			{
				FStructProperty* Property = new FStructProperty(InScope, InName);
				Property->Struct = InStructType;
				Property->SetElementSize(InStructType->GetStructureSize());
				if (bIsHashable)
				{
	...
	if (ensure(InClassType))
	{
		...
		if (EnumHasAnyFlags(InUsageFlags, EOptimusDataTypeUsageFlags::Variable))
		{
			PropertyCreateFunc = [InClassType](FFieldVariant InScope, FName InName) -> FProperty* {
				FObjectProperty* Property = new FObjectProperty(InScope, InName);
	...
	)
{
	if (EnumHasAnyFlags(InUsageFlags, EOptimusDataTypeUsageFlags::Variable))
	{
		UE_LOGF(LogOptimusCore, Error, "Can't register '%ls' for use in variables when there is no associated native type.", *InTypeName.ToString());
		return false;
	}
	...
	) const
{
	...
	if (!InfoPtr)
	{
		UE_LOGF(LogOptimusCore, Fatal, "CreateProperty: Invalid type name.");
		return nullptr;
	}
