...
UOptimusNodeGraph* UOptimusDeformer::GetUpdateGraph() const
{
	...
	UE_LOGF(LogOptimusCore, Fatal, "No upgrade graph on deformer (%ls).", *GetPathName());
	return nullptr;
}
	...
	)
{
	...
	{
		UE_LOGF(LogOptimusCore, Error, "Invalid data type for variables.");
		return nullptr;
	}
	...
	)
{
	...
	{
		UE_LOGF(LogOptimusCore, Error, "Variable not owned by this deformer.");
		return false;
	}
	...
	)
{
	...
	if (InNewName.IsNone())
	{
		UE_LOGF(LogOptimusCore, Error, "Invalid resource name.");
		return false;
	}
	...
	)
{
	...
	if (InVariableDesc->GetOuter() != Variables)
	{
		UE_LOGF(LogOptimusCore, Error, "Resource not owned by this deformer.");
		return false;
	}
	
	if (!InDataType.IsValid())
	{
		UE_LOGF(LogOptimusCore, Error, "Invalid data type");
		return false;
	}
	...
	)
{
	...
	{
		UE_LOGF(LogOptimusCore, Error, "Invalid data type for resources.");
		return nullptr;
	}
	...
	)
{
	...
	{
		UE_LOGF(LogOptimusCore, Error, "Component binding not owned by this deformer.");
		return false;
	}
	if (InBinding->bIsPrimaryBinding)
	{
		UE_LOGF(LogOptimusCore, Error, "The primary binding cannot be removed.");
		return false;
	}
	...
	)
{
	...
	if (InBinding->GetOuter() != Bindings)
	{
		UE_LOGF(LogOptimusCore, Error, "Binding not owned by this deformer.");
		return false;
	}
	
	if (InNewName.IsNone())
	{
		UE_LOGF(LogOptimusCore, Error, "Invalid binding name");
		return false;
	}
	...
	)
{
	...
	if (!InComponentSource)
	{
		UE_LOGF(LogOptimusCore, Error, "Invalid component source");
		return false;
	}
...
void UOptimusDeformer::PostLoad()
{
	...
	GetObjectsWithOuter(this, Objects, EGetObjectsFlags::None);
		...
		ObjectVersion < FOptimusObjectVersion::LatestVersion)
	{
		...
		if (ObjectVersion < FOptimusObjectVersion::FunctionGraphUseGuid)
		{
			...
			if (FunctionNodes.Num() > 0)
			{
				...
				UE_LOGF(LogOptimusCore, Warning, "Deformer Graph should be recompiled and resaved to avoid issues in the packaged build, Asset: %ls",  *GetPackage()->GetName());
			}
		}
...
void UOptimusDeformer::PostLoadRemoveDeprecatedValueContainerGeneratorClass()
{
	...
	GetObjectsWithOuter(GetPackage(), ObjectsInPackage, EGetObjectsFlags::None);
