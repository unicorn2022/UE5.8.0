	...
	)
{
	if (!bDynamicPins)
	{
		UE_LOGF(LogOptimusCore, Error, "Attempting to add a pin to a non-dynamic node: %ls", *GetNodePath());
		return nullptr;
	}
	...
	if (InBeforePin)
	{
		...
		{
			UE_LOGF(LogOptimusCore, Error, "Attempting to place a pin before one that does not belong to this node: %ls", *InBeforePin->GetPinPath());
			return nullptr;
		}
		if (InBeforePin->GetParentPin() != nullptr || (InBeforePin->GetParentPin() && !InBeforePin->GetParentPin()->IsGroupingPin()))
		{
			UE_LOGF(LogOptimusCore, Error, "Attempting to place a pin before one that is not a top-level pin or inside a grouping pin: %ls", *InBeforePin->GetPinPath());
			return nullptr;
		}
	}
	if (InGroupingPin)
	{
		...
		{
			UE_LOGF(LogOptimusCore, Error, "Attempting to place a pin under a group pin that does not belong to this node: %ls", *InGroupingPin->GetPinPath());
			return nullptr;
		}

		if (InBeforePin && !InGroupingPin->GetSubPins().Contains(InBeforePin))
		{
			UE_LOGF(LogOptimusCore, Error, "Before pin is not a part of the given group: %ls", *InBeforePin->GetPinPath());
			return nullptr;
		}
	}
	...
	)
{
	...
	if (InBeforePin)
	{
		...
		{
			UE_LOGF(LogOptimusCore, Error, "Attempting to place a pin before one that is not a top-level pin: %ls", *InBeforePin->GetPinPath());
			return nullptr;
		}
	}
...
bool UOptimusNode::RemovePin(UOptimusNodePin* InPin)
{
	if (!bDynamicPins)
	{
		UE_LOGF(LogOptimusCore, Error, "Attempting to remove a pin from a non-dynamic node: %ls", *GetNodePath());
		return false;
	}

	if (InPin->GetParentPin() != nullptr && !InPin->GetParentPin()->IsGroupingPin())
	{
		UE_LOGF(LogOptimusCore, Error, "Attempting to remove a non-root or non-group pin: %ls", *InPin->GetPinPath());
		return false;
	}
...
bool UOptimusNode::RemovePinDirect(UOptimusNodePin* InPin)
{
	...
	for (const UOptimusNodePin* Pin: PinsToRemove)
	{
		...
		{
			UE_LOGF(LogOptimusCore, Warning, "Attempting to remove a connected pin: %ls", *Pin->GetPinPath());
			return false;
		}
	}
	...
	)
{
	...
	{
		UE_LOGF(LogOptimusCore, Warning, "Attempting to move a pin before a non-sibling pin: %ls", *InPinToMove->GetPinPath());
		return false;
	}
	...
	)
{
	for (const FProperty* Property : TFieldRange<FProperty>(InStruct))
	{
		...
		else if (Property->HasMetaData(PropertyMeta::Input))
		{
			...
			{
				UE_LOGF(LogOptimusCore, Error, "Pin on %ls.%ls marked both input and output. Ignoring it as output.",
	...
	)
{
	...
	if (!DataType.IsValid())
	{
		UE_LOGF(LogOptimusCore, Error, "No registered type found for pin '%ls'.", *InProperty->GetName());
		return nullptr;
	}
