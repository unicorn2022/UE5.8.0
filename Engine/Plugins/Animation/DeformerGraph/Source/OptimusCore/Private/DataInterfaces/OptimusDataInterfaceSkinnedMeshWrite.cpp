...
void UOptimusSkinnedMeshWriteDataProvider::InitializePerKernelInputOutputMasks(TMap<int32, FInputOutputMask>&& InPerKernelMasks)
{
	PerKernelMasks = MoveTemp(InPerKernelMasks);
}
...
FComputeDataProviderRenderProxy* UOptimusSkinnedMeshWriteDataProvider::GetRenderProxy()
{
	...
	if (SkinnedMesh.IsValid() && SkinnedMesh->IsRegistered())
	{
		Proxy->SceneInterface = SkinnedMesh->GetScene();
		Proxy->ComponentId = SkinnedMesh->GetPrimitiveSceneId();
	}
	Proxy->OutputMask = OutputMask;
	Proxy->PerKernelMasks = PerKernelMasks;

	if (WeakDeformerInstance.IsValid())
	{
		Proxy->ExecutionGroup = WeakDeformerInstance->CurrentExecutionGroupName;
	}
...
void UOptimusSkinnedMeshWriteDataProvider::SetDeformerInstance(UOptimusDeformerInstance* InInstance)
{
...
void FOptimusSkinnedMeshWriteDataProviderProxy::PostSubmit(FComputeContext& Context) const
{
	FSkeletalMeshObject* SkeletalMeshObject = FSkeletalMeshDeformerHelpers::GetSkeletalMeshObject(SceneInterface, ComponentId);
	if (!SkeletalMeshObject)
	{
	...
	const uint64 KernelOutputMask = PerKernelMasks.FindChecked(Context.GraphKernelIndex).OutputMask;

	EMeshDeformerOutputBuffer DeformerOutputBuffers = EMeshDeformerOutputBuffer::None;

	if (KernelOutputMask & static_cast<uint64>(ESkinnedMeshWriteDataInterfaceOutputSelectorMask::Position))
	{
		DeformerOutputBuffers |= EMeshDeformerOutputBuffer::SkinnedMeshPosition;
	}
	if ((KernelOutputMask & static_cast<uint64>(ESkinnedMeshWriteDataInterfaceOutputSelectorMask::TangentX)) ||
		(KernelOutputMask & static_cast<uint64>(ESkinnedMeshWriteDataInterfaceOutputSelectorMask::TangentZ)))
	{
		DeformerOutputBuffers |= EMeshDeformerOutputBuffer::SkinnedMeshTangents;
	}
	if (KernelOutputMask & static_cast<uint64>(ESkinnedMeshWriteDataInterfaceOutputSelectorMask::Color))
	{
		DeformerOutputBuffers |= EMeshDeformerOutputBuffer::SkinnedMeshVertexColor;
	}

	FOptimusSkinnedMeshTrackingExtension::MarkDeformed(Context.GraphBuilder, SceneInterface, ComponentId, SkeletalMeshObject, DeformerOutputBuffers, ExecutionGroup);
}
...
void FOptimusSkinnedMeshWriteDataProviderProxy::GetReadbackData(TArray<FReadbackData>& OutReadbackData) const
{
	...
	if (GeometryReadbackRequests.Num() > 0)
	{
		...
		FSkeletalMeshObject* ReadbackMeshObject = FSkeletalMeshDeformerHelpers::GetSkeletalMeshObject(SceneInterface, ComponentId);
		GeometryReadback->LodIndex = ReadbackMeshObject ? ReadbackMeshObject->GetLOD() : 0;
