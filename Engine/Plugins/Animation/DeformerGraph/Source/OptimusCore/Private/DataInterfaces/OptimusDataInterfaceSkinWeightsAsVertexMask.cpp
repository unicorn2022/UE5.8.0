	...
	if (SkeletalMesh.Get() && SkeletalMesh->MeshObject)
	{
		...
		else
		{
			if (!bIsInitialized)
			{
				...
				const bool bUnifiedBoneMap = SkeletalMeshRenderData.HasUnifiedBoneMap();
				...
				for (int32 LODIndex = 0; LODIndex < SkeletalMeshRenderData.LODRenderData.Num(); LODIndex++)
				{
					...
					for (int32 SectionIndex = 0; SectionIndex < LocalLodRenderData.RenderSections.Num(); ++SectionIndex)
					{
						...
						// Only process first section when unified.
						if (bUnifiedBoneMap)
						{
							break;
						}
					...
					// Unified bone map is cross-LOD, so LOD 0 / Section 0 covers all bones. Break the LOD loop too.
					if (bUnifiedBoneMap)
					{
						break;
					}
			...
			{
#if WITH_EDITOR
				ExecuteOnGameThread(UE_SOURCE_LOCATION, [ WeakThis = TWeakObjectPtr(this)]()
					{
						if (WeakThis.IsValid())
						{
			...
			if (SkeletalMesh.IsValid() && SkeletalMesh->IsRegistered())
			{
				Proxy->SceneInterface = SkeletalMesh->GetScene();
				Proxy->ComponentId = SkeletalMesh->GetPrimitiveSceneId();
			}
...
bool FOptimusSkinWeightsAsVertexMaskDataProviderProxy::IsValid(FValidationData const& InValidationData) const
{
	...
	// This data interface does not support unified dispatch (because bone maps can be per-section) 
	// so the invocation count has to match the render sections
...
void FOptimusSkinWeightsAsVertexMaskDataProviderProxy::AllocateResources(FRDGBuilder& GraphBuilder, FAllocationData const& InAllocationData)
{
	...
	FSkeletalMeshLODRenderData const* LodRenderData = &SkeletalMeshRenderData.LODRenderData[LodIndex];
	const bool bUnifiedBoneMap = SkeletalMeshRenderData.HasUnifiedBoneMap();
	const int32 NumUniqueSections = bUnifiedBoneMap ? 1 : LodRenderData->RenderSections.Num();

	BoneIsSelectedBuffersPerSection.AddDefaulted(NumUniqueSections);
	BoneIsSelectedBufferSRVsPerSection.AddDefaulted(NumUniqueSections);

	for (int32 InvocationIndex = 0 ; InvocationIndex < NumUniqueSections; InvocationIndex++)
	{
		const TArray<uint32>& BoneIsSelected = bUnifiedBoneMap ? BoneIsSelectedPerSectionPerLod[0][0] : BoneIsSelectedPerSectionPerLod[LodIndex][InvocationIndex];

		// When using unified bone map, the BoneIsSelected array was built from LOD 0's bone map.
		// The current LOD may have a different bone count. Use the smaller of the two to avoid OOB.
		int32 NumBones = FMath::Min(LodRenderData->RenderSections[InvocationIndex].BoneMap.Num(), BoneIsSelected.Num());
...
void FOptimusSkinWeightsAsVertexMaskDataProviderProxy::GatherDispatchData(FDispatchData const& InDispatchData)
{
	...
	for (int32 InvocationIndex = 0; InvocationIndex < InDispatchData.NumInvocations; ++InvocationIndex)
	{
		...
		Parameters.BoneIsSelected = BoneIsSelectedBufferSRVsPerSection[bUnifiedBoneMap ? 0 : InvocationIndex];
	}
}
