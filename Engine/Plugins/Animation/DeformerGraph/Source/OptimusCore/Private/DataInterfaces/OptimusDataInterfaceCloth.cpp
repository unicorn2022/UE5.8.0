...
{
	if (SkinnedMeshComponent != nullptr && SkinnedMeshComponent->IsRegistered())
	{
		SceneInterface = SkinnedMeshComponent->GetScene();
		ComponentId = SkinnedMeshComponent->GetPrimitiveSceneId();
		FrameNumber = SkinnedMeshComponent->GetScene() != nullptr ? SkinnedMeshComponent->GetScene()->GetFrameNumber() + 1 : 0; // +1 matches the logic for FrameNumberToPrepare in FSkeletalMeshObjectGPUSkin::Update()
