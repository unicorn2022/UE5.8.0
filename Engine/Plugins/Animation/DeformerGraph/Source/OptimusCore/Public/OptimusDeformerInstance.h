...
/**
 * Delegate called per non-primary binding during GetComponentBindings to supply an explicit component.
 * If bound and returns non-null, the result bypasses tag/class auto-resolution for that binding. 
 */
DECLARE_DELEGATE_RetVal_OneParam(UActorComponent*, FOptimusComponentBindingResolver, FName /*BindingName*/)
...
public:
	...
	FOptimusComponentBindingResolver ComponentResolver;
...
public:
	...
	/** The execution group name for the current EnqueueWork call (e.g. EndOfFrameUpdate, BeginInitViews).
	 *  Set on game thread before GetRenderProxy() calls, used by read/write proxies to inform the tracking extension. */
	FName CurrentExecutionGroupName = NAME_None;

	/** Get the primary mesh component that owns this deformer instance. */
