...
void SRigHierarchy::Construct(const FArguments& InArgs, TSharedRef<IControlRigBaseEditor> InControlRigEditor, const FName& InViewName)
{
	ViewName = InViewName;
	...
	[
		...
		[
			...
			[
				...
				[
					SAssignNew(TreeView, SRigHierarchyTreeView, ViewName)
...
void SRigHierarchy::OnEditorClose(IControlRigBaseEditor* InEditor, FControlRigAssetInterfacePtr InBlueprint)
{
	...
	if (InEditor)
	{
		UControlRig* ControlRig = InEditor->GetControlRig();
		if (ControlRig && TreeView.IsValid())
		{
			// Store the tree state so it gets restored when the editor is opened anew
			FRigHierarchyTreePersistentStateStore::Get().StorePersistentState(*ControlRig, *TreeView, ViewName);
		}
...
void SRigHierarchy::BindCommands()
{
	...
	constexpr bool bFrameInViewport = true;
	CommandList->MapAction(
		Commands.FrameSelection,
		FExecuteAction::CreateSP(this, &SRigHierarchy::HandleFrameSelection, bFrameInViewport),
		FCanExecuteAction::CreateSP(this, &SRigHierarchy::IsMultiSelected, true));
...
FReply SRigHierarchy::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	{
		if(const TSharedPtr<FRigHierarchyTreeElement>* ItemPtr = TreeView->FindItemAtPosition(MouseEvent.GetScreenSpacePosition()))
		{
			if(const TSharedPtr<FRigHierarchyTreeElement>& Item = *ItemPtr)
			{
...
void SRigHierarchy::RefreshTreeView(bool bRebuildContent)
{
	...
	bIsChangingRigHierarchy = true;
	
	TreeView->RefreshTreeView(bRebuildContent,
		FOnRigHierarchyTreePersistentStateRestored::CreateLambda([WeakThis = AsWeak(), this]()
			{
				if (WeakThis.IsValid())
				{
					bIsChangingRigHierarchy = false;
...
{
	TArray<TSharedPtr<FRigHierarchyTreeElement>> SelectedItems = TreeView->GetSelectedItems();
	
	TArray<FRigHierarchyKey> SelectedKeys;
	for (const TSharedPtr<FRigHierarchyTreeElement>& SelectedItem : SelectedItems)
	{
...
void SRigHierarchy::OnSelectionChanged(TSharedPtr<FRigHierarchyTreeElement> Selection, ESelectInfo::Type SelectInfo)
{
	...
	switch(InNotif)
	{
		...
		case ERigHierarchyNotification::ComponentDeselected:
		{
			if(InElement || InComponent)
			{
				...
				{
					TSharedPtr<FRigHierarchyTreeElement> Found = TreeView->FindElement(Key, TreeView->RootElements[RootIndex]);
					if (Found.IsValid())
					{
						...
						{
							constexpr bool bFrameInViewport = false;
							HandleFrameSelection(bFrameInViewport);
						}
		...
		case ERigHierarchyNotification::SocketColorChanged:
		{
				...
				(InElement->GetType() == ERigElementType::Socket)))
			{
				for (int32 RootIndex = 0; RootIndex < TreeView->RootElements.Num(); ++RootIndex)
				{
					TSharedPtr<FRigHierarchyTreeElement> TreeElement = TreeView->FindElement(InElement->GetKey(), TreeView->RootElements[RootIndex]);
					if (TreeElement.IsValid())
					{
						const FRigHierarchyTreeDisplaySettings& Settings = TreeView->GetRigTreeDelegates().GetDisplaySettings();
		...
		case ERigHierarchyNotification::ComponentContentChanged:
		{
			if(InComponent)
			{
				for (int32 RootIndex = 0; RootIndex < TreeView->RootElements.Num(); ++RootIndex)
				{
					TSharedPtr<FRigHierarchyTreeElement> TreeElement = TreeView->FindElement(InComponent->GetKey(), TreeView->RootElements[RootIndex]);
					if (TreeElement.IsValid())
					{
						const FRigHierarchyTreeDisplaySettings& Settings = TreeView->GetRigTreeDelegates().GetDisplaySettings();
						TreeElement->RefreshDisplaySettings(InHierarchy, Settings);
			...
			if(ControlRigEditor.IsValid())
			{
				if(UControlRig* ControlRig = ControlRigEditor.Pin()->GetControlRig())
				{
					if(URigHierarchy* Hierarchy = ControlRig->GetHierarchy())
					{
						...
						bool bScrollIntoView = !FRigHierarchyTreePersistentStateStore::IsFeatureEnabled();

						for(const FRigElementKey& Key : Keys)
						{
							if(const TSharedPtr<FRigHierarchyTreeElement> TreeElement = TreeView->FindElement(Key))
							{
...
void SRigHierarchy::HandleRefreshEditorFromBlueprint(FRigVMEditorAssetInterfacePtr InBlueprint)
{
	if (bIsChangingRigHierarchy)
	{
		return;
...
void SRigHierarchy::OnItemClicked(TSharedPtr<FRigHierarchyTreeElement> InItem)
{
	URigHierarchy* Hierarchy = GetHierarchy();
	check(Hierarchy);
...
void SRigHierarchy::OnItemDoubleClicked(TSharedPtr<FRigHierarchyTreeElement> InItem)
{
...
void SRigHierarchy::OnSetExpansionRecursive(TSharedPtr<FRigHierarchyTreeElement> InItem, bool bShouldBeExpanded)
{
				...
				if (SRigHierarchy* RigHierarchyPanel = MainContext->GetRigHierarchyPanel())
				{
					...
					ElementsSection.AddSubMenu(TEXT("NewElement"), LOCTEXT("NewElement", "New Element"), LOCTEXT("NewElement_ToolTip", "Create New Elements"),
						FNewToolMenuDelegate::CreateLambda([Commands, RigHierarchyPanel](UToolMenu* InSubMenu)
						{
							...
							TArray<TSharedPtr<FRigHierarchyTreeElement>> SelectedItems = RigHierarchyPanel->TreeView->GetSelectedItems();
...
void SRigHierarchy::HandleNewItem(ERigElementType InElementType, bool bIsAnimationChannel)
{
	...
	if (Hierarchy)
	{
		...
		// It's important to also modify the rig to restore the hierarchy after undo/redo
		if (UControlRig* ControlRig = ControlRigEditor.Pin()->GetControlRig())
		{
			ControlRig->Modify();
		}
...
void SRigHierarchy::HandleMirrorItem()
{
	...
	if (Hierarchy)
	{
		...
		TSharedPtr<FStructOnScope> StructToDisplay = MakeShared<FStructOnScope>(FRigVMMirrorSettings::StaticStruct(), (uint8*)&Settings->MirrorSettings);
#if WITH_RIGVMLEGACYEDITOR
		TSharedRef<SKismetInspector> KismetInspector = SNew(SKismetInspector);
#else
		TSharedRef<SRigVMDetailsInspector> KismetInspector = SNew(SRigVMDetailsInspector);
		...
		if (MirrorDialog->ShowModal() == 0)
		{
			ClearDetailPanel();
			{
				...
				Controller->MirrorElements(KeysToMirror, Settings->MirrorSettings, true, true, true);
			}
			ControlRigBlueprint->PropagateHierarchyFromBPToInstances();
...
public:
	...
	{
		UE_LOGF(LogControlRig, Error, "Error importing transforms to Hierarchy: %ls", V);
		NumErrors++;
	}
};
...
FReply SRigHierarchy::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (MouseEvent.IsMouseButtonDown(EKeys::LeftMouseButton) && DraggedElements.Num() > 0)
	{
		if (TSharedPtr<IControlRigBaseEditor> EditorPtr = ControlRigEditor.Pin())
		{
			...
			TSharedRef<FRigHierarchyElementDragDropOp> DragDropOp = FRigHierarchyElementDragDropOp::New(MoveTemp(DraggedElements));
...
TOptional<EItemDropZone> SRigHierarchy::OnCanAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TSharedPtr<FRigHierarchyTreeElement> TargetItem)
{
	const TOptional<EItemDropZone> InvalidDropZone;
	TOptional<EItemDropZone> ReturnDropZone;

	const TSharedPtr<FRigHierarchyElementDragDropOp> RigDragDropOp = DragDropEvent.GetOperationAs<FRigHierarchyElementDragDropOp>();
	...
	const TSharedPtr<FModularRigHierarchyElementDragDropOp> ModuleDropOp = DragDropEvent.GetOperationAs<FModularRigHierarchyElementDragDropOp>();
...
FReply SRigHierarchy::OnAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TSharedPtr<FRigHierarchyTreeElement> TargetItem)
{
	...
	TSharedPtr<FRigHierarchyElementDragDropOp> RigDragDropOp = DragDropEvent.GetOperationAs<FRigHierarchyElementDragDropOp>();
	if (RigDragDropOp.IsValid())
	{
	...
	if (AssetDragDropOp.IsValid())
	{
		for (const FAssetData& AssetData : AssetDragDropOp->GetAssets())
		{
			...
			if(FControlRigAssetInterfacePtr AssetBlueprint = AssetData.GetAsset())
			{
				ControlRigAssetReference = AssetBlueprint->GetControlRigAssetReference();
			...
			{
				ControlRigAssetReference = GeneratedClass->GetControlRigAssetReference();
				ModuleSettings = GeneratedClass->GetRigModuleSettings();
			}
			...
			if(ControlRigAssetReference.IsValid())
			{
				if (UModularRigController* Controller = ControlRigBlueprint->GetModularRigController())
				{
					...
					const FName ModuleName = Controller->AddModuleFromAssetReference(DesiredModuleName.GetFName(), ControlRigAssetReference, NAME_None);
...
void SRigHierarchy::UpdateConnectorMatchesOnDrag(const TArray<FRigHierarchyKey>& InDraggedKeys)
{
	...
	// fade in all items
	for(const TPair<FRigHierarchyKey, TSharedPtr<FRigHierarchyTreeElement>>& Pair : TreeView->ElementMap)
	{
	...
	for(const TPair<FRigElementKey, FModularRigResolveResult>& Pair: DragRigResolveResults)
	{
		...
		{
			if(const TSharedPtr<FRigHierarchyTreeElement>* TreeElementPtr = TreeView->ElementMap.Find(ExcludedElement.GetKey()))
			{
...
FReply SRigHierarchy::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	const TSharedPtr<FRigHierarchyTreeElement>* ItemAtMouse = TreeView->FindItemAtPosition(DragDropEvent.GetScreenSpacePosition());
...
void SRigHierarchy::HandleZeroControls()
{
	...
	const TScriptInterface<IControlRigEditorAssetInterface> Blueprint = PinnedControlRigEditor.IsValid() ? PinnedControlRigEditor->GetControlRigAssetInterface() : nullptr;
...
{
	const TScriptInterface<IControlRigEditorAssetInterface> Blueprint = ControlRigEditor.Pin()->GetControlRigAssetInterface();
	URigHierarchy* DefaultHierarchy = GetDefaultHierarchy();
	URigHierarchy* DebuggedHierarchy = GetHierarchy();
...
void SRigHierarchy::HandleFrameSelection(const bool bFrameInViewport)
{
	TArray<TSharedPtr<FRigHierarchyTreeElement>> SelectedItems = TreeView->GetSelectedItems();
	for (TSharedPtr<FRigHierarchyTreeElement> SelectedItem : SelectedItems)
	{
