...
void SRigConnectorTargetComboButton::Construct(const FArguments& InArgs)
{
	...
	RigTreeDelegates.OnGetSelection.BindLambda([WeakThis = AsWeak(), this]() -> TArray<FRigHierarchyKey>
	{
		if (WeakThis.IsValid())
		{
	...
	RigTreeDelegates.OnGetTagDisplayMode = FOnRigTreeGetTagDisplayMode::CreateLambda([]() 
		{
			return ERigHierarchyConnectorTagDisplayMode::Single;
		}
	);

	constexpr const TCHAR* ConnecterTargetRigHierarchyView = TEXT("ConnecterTargetRigHierarchyView");

	SearchableTreeView = SNew(SRigHierarchySearchableTreeView, ConnecterTargetRigHierarchyView)
...
void SRigConnectorTargetComboButton::PopulateButtonBox()
{
	...
	const TWeakObjectPtr<const URigHierarchy> WeakHierarchy = Hierarchy;
	auto GetIconAndColor = [WeakHierarchy, this]() -> TPair<const FSlateBrush*, FSlateColor>
		{
			if (WeakHierarchy.IsValid())
			{
				...
				return SRigHierarchyItem::GetBrushForElementType(WeakHierarchy.Get(), CurrentTargetKey);
			}

			return TPair<const FSlateBrush*, FSlateColor>(nullptr, FSlateColor::UseForeground());
		};
	...
	[
		...
		[
			...
			[
				...
				.Text_Lambda([WeakHierarchy, this]()
					{
						if (WeakHierarchy.IsValid())
						{
							const EElementNameDisplayMode NameDisplayMode = RigTreeDelegates.GetDisplaySettings().NameDisplayMode;
							return WeakHierarchy->GetDisplayNameForUI(TargetKey.Get(), NameDisplayMode);
						}
...
void SRigConnectorTargetComboButton::OnConnectorTargetPicked(TSharedPtr<FRigHierarchyTreeElement> Selection, ESelectInfo::Type SelectInfo)
{
...
void SRigConnectorTargetWidget::Construct(const FArguments& InArgs)
{
	...
	if(!RigTreeDelegates.OnRigTreeIsItemVisible.IsBound())
	{
		...
		if(URigHierarchy* Hierarchy = RigTreeDelegates.GetHierarchy())
		{
			if(const UControlRig* ControlRig = Hierarchy->GetTypedOuter<UControlRig>())
			{
				...
				{
					const UModularRig* ModularRig = Cast<UModularRig>(ControlRig);

					// Only filter by module if this is a modular rig, otherwise 
					// use a nullptr module for it to be ignored when finding matches
					const FRigModuleInstance* Module = ModularRig ? ModularRig->FindModule(ConnectorElement) : nullptr;

					if (ModularRig == nullptr || Module != nullptr)
					{
						const UModularRigRuleManager* RuleManager = Hierarchy->GetRuleManager();
						TArray<FRigElementResolveResult> Matches = RuleManager->FindMatches(ConnectorElement, Module, ControlRig->GetElementKeyRedirector()).GetMatches();
	...
	if(bIsArray)
	{
		...
		DetailsViewArgs.ColumnWidth = .93f;

		const TSharedRef<IDetailsView> DetailsView = PropertyEditorModule.CreateDetailView(DetailsViewArgs);

		// Define a min height. The details view is painted one tick after it was constructed
		// and resizes the widget. This causes the outer widget to resize, e.g. when shown in 
		// the modular rig hierarchy tree view, it leaves the tree with a borked scrollbar 
		// towards the bottom unless the height is defined ahead of time.
		constexpr float MinHeight = 28.f;
                                                             ...
                                                             IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	{
		const TWeakPtr<IPropertyHandle> WeakStructPropertyHandle = InStructPropertyHandle;
		...
		if(TargetsDetailWrapper->TargetArray.IsValidIndex(ArrayIndex))
		{
			...
			[
				...
				.ContentPadding(FMargin(0, 3, 3, 0))
				.OnSetTarget_Lambda([WeakStructPropertyHandle, this, ArrayIndex](FRigElementKey InTarget) -> bool
					{
						if (WeakStructPropertyHandle.IsValid() &&
							WeakStructPropertyHandle.Pin()->IsValidHandle())
						{
							TargetsDetailWrapper->TargetArray[ArrayIndex] = InTarget;
							WeakStructPropertyHandle.Pin()->NotifyFinishedChangingProperties();
							return true;
						}
