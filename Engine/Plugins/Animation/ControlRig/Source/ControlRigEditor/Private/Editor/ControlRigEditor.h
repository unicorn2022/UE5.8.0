...
protected:
	...
	virtual void InitRigVMEditorSuper(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, FRigVMEditorAssetInterfacePtr InRigVMBlueprint) = 0;
	...
	virtual void HandleRefreshEditorFromBlueprintSuper(FRigVMEditorAssetInterfacePtr InBlueprint) = 0;
...
protected:
	void InitRigVMEditorImpl(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, FRigVMEditorAssetInterfacePtr InRigVMBlueprint);
	...
	virtual ~FControlRigBaseEditor() = default;
...
protected:
	...
	/** Sets the Event Queue that was set prior to the current one */
	void SetPriorEventQueue();

	/** Toggles between forwards solve and construction */
	void ToggleForwardsSolveAndConstruction();
	...
	void HandleToggleSchematicViewportShowEmptySocketsOnly();
	bool IsSchematicViewportShowEmptySocketsOnlyActive() const;
	void HandleToggleCompileResetsEventQueue();
	bool IsToggleCompileResetsEventQueueActive() const;
	EVisibility GetSchematicOverlayVisibility() const;

	bool IsRunningForwardSolve() const;
	void OnRunGraphOnceToggledImpl(bool bRunGraphOnceEnabled);
	...
	virtual void HandleRefreshEditorFromBlueprintImpl(FRigVMEditorAssetInterfacePtr InBlueprint);
	...
	/**
	 * When true, the editor will try to frame the rig's visible controls in the preview viewport on the next
	 * tick. Set during init when no preview mesh is assigned (e.g. Face_ControlBoard_CtrlRig) so the camera
	 * doesn't sit somewhere with no rig content visible. Cleared once the framing has been requested or after
	 * a small number of attempts (in case the rig hierarchy never populates).
	 * See: MH issue "Control Rigs displaced out of their Blueprint Viewports".
	 */
	bool bPendingInitialViewportFocus = false;
	int32 PendingInitialViewportFocusTickCount = 0;
	...
	void OnPostForwardsSolve_AnyThread(UControlRig* InRig, const FName& InEventName);
	...
	void OnEarlyExit_AnyThread(URigVMHost* InHost, URigVMHost::EEarlyExitReason InReason);
	...
	FRigPose BonePoseFromLastEvaluation;
