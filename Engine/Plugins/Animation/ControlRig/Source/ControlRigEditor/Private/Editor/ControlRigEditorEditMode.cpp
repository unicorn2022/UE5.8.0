...
FControlRigEditorEditMode::FControlRigEditorEditMode()
{
...
void FControlRigEditorEditMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	if (FControlRigAssetInterfacePtr EditorAsset = ControlRig->GetAssetReference().GetEditorAsset())
	{
		HierarchyForSelection = EditorAsset->GetHierarchy();
	}
	...
	for (int32 ElementIndex = 0; ElementIndex < Hierarchy->Num(); ElementIndex++)
	{
		...
		// Only draw procedurally generated bones, other bones are painted by persona
		const bool bPaintedByPersona = !Hierarchy->IsProcedural(BoneElement);
		if (bPaintedByPersona)
		{
			continue;
		}
...
bool FControlRigEditorEditMode::IsDisplayControlsEnabled() const
{
	return !GetDefault<UControlRigEditorSettings>()->bHideControlShapes;
}

void FControlRigEditorEditMode::ToggleDisplayControlsEnabled()
{
...
bool FControlRigEditorEditMode::IsControlsAsOverlayEnabled() const
{
	return GetDefault<UControlRigEditorSettings>()->bShowControlsAsOverlay;
}

void FControlRigEditorEditMode::ToggleControlsAsOverlay()
{
	UControlRigEditorSettings* Settings = GetMutableDefault<UControlRigEditorSettings>();
	Settings->bShowControlsAsOverlay = !Settings->bShowControlsAsOverlay;
	Settings->SaveConfig();
	UControlRigEditModeSettings::OnSettingsChange.Broadcast(GetDefault<UControlRigEditModeSettings>());
}

bool FControlRigEditorEditMode::IsDisplayNullsEnabled() const
{
	return GetDefault<UControlRigEditorSettings>()->bDisplayNulls;
}

void FControlRigEditorEditMode::ToggleDisplayNulls()
{
	UControlRigEditorSettings* Settings = GetMutableDefault<UControlRigEditorSettings>();
...
bool FControlRigEditorEditMode::IsDisplaySocketsEnabled() const
{
	return GetDefault<UControlRigEditorSettings>()->bDisplaySockets;
}

void FControlRigEditorEditMode::ToggleDisplaySockets()
{
	UControlRigEditorSettings* Settings = GetMutableDefault<UControlRigEditorSettings>();
...
bool FControlRigEditorEditMode::IsDisplayAxesOnSelectionEnabled() const
{
	return GetDefault<UControlRigEditorSettings>()->bDisplayAxesOnSelection;
}

void FControlRigEditorEditMode::ToggleDisplayAxesOnSelection()
{
	UControlRigEditorSettings* Settings = GetMutableDefault<UControlRigEditorSettings>();
...
float FControlRigEditorEditMode::GetAxesOnSelectionScale() const
{
