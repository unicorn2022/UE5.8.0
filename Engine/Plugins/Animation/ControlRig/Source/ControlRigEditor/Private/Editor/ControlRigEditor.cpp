		...
		if (AssetDragDropOperation)
		{
			for (const FAssetData& AssetData : AssetDragDropOperation->GetAssets())
			{
				...
				else if (UControlRigBlueprintGeneratedClass* GeneratedClass = Cast<UControlRigBlueprintGeneratedClass>(AssetData.GetAsset()))
				{
					...
					{
						AssetObject.Set(TSubclassOf<UControlRig>(GeneratedClass));
					}
				}
				else if(UControlRigRuntimeAsset* RuntimeAsset = Cast<UControlRigRuntimeAsset>(AssetData.GetAsset()))
				{
					if (RuntimeAsset->IsRigModule())
					{
						AssetObject.Set(RuntimeAsset);
					}
				}
				FControlRigAssetStrongReference ControlRigClass(AssetObject);
				if(ControlRigClass.IsValid())
				{
					FSlateApplication::Get().DismissAllMenus();

					UModularRigController* Controller = GetControlRigAssetInterface()->GetModularRigController();
					FString ClassName = ControlRigClass.Get()->GetName();
					ClassName.RemoveFromEnd(TEXT("_C"));
					const FRigName DesiredModuleName = Controller->GetSafeNewName(FRigName(ClassName));
					const FName ModuleName = Controller->AddModuleFromAssetReference(DesiredModuleName, ControlRigClass, NAME_None);
...
{
	// If the EdGraph does not exist, create it
	if (FControlRigAssetInterfacePtr ControlRigEditorAsset = GetControlRigAssetInterface())
	{
		if (FRigVMEditorAssetInterfacePtr EditorAsset = ControlRigEditorAsset->GetRigVMAssetInterface())
		{
			if (URigVMGraph* Graph = InController->GetGraph())
			{
				UEdGraph* EdGraph = EditorAsset->GetEdGraph(Graph);
				if (!EdGraph)
				{
	...
	if (!InController->GetAllEventNames().Contains(FRigUnit_BeginExecution::EventName))
	{
...
void FControlRigBaseEditor::InitRigVMEditorImpl(const EToolkitMode::Type Mode, const TSharedPtr<class IToolkitHost>& InitToolkitHost, FRigVMEditorAssetInterfacePtr InRigVMBlueprint)
{
	...
	IControlRigEditorAssetInterface::sCurrentlyOpenedRigBlueprints.AddUnique(ControlRigBlueprint);

	// When the asset has no preview mesh, the default viewport camera doesn't auto-focus on anything visible
	// (the rig's control shapes can be far from world origin -- Face_ControlBoard_CtrlRig is the canonical
	// example). Mark a one-shot request to frame the visible controls once the rig has populated. The actual
	// framing is performed in TickImpl, which gives the hierarchy a chance to compile and the viewport client
	// to be available.
	bPendingInitialViewportFocus = (ControlRigBlueprint->GetPreviewMesh() == nullptr);
	PendingInitialViewportFocusTickCount = 0;
...
void FControlRigBaseEditor::FillToolbarImpl(FToolBarBuilder& ToolbarBuilder, bool bEndSection)
{
	...
	
	{
		...
		{
			TWeakInterfacePtr<IControlRigEditorAssetInterface> WeakBlueprint = GetControlRigAssetInterface();
...
void FControlRigBaseEditor::SetEventQueueImpl(TArray<FName> InEventQueue, bool bCompile)
{
	...
	if (FControlRigEditMode* EditMode = GetEditMode())
	{
		...
		UControlRigEditorSettings* EditorSettings = GetMutableDefault<UControlRigEditorSettings>();
		EditorSettings->bDisplayNulls = IsConstructionModeEnabled();
	}
...
void FControlRigBaseEditor::HandleSetObjectBeingDebuggedImpl(UObject* InObject)
{
	...
	if(UControlRig* PreviouslyDebuggedControlRig = Cast<UControlRig>(GetRigVMAssetInterface()->GetObjectBeingDebugged()))
	{
		if(!URigVMHost::IsGarbageOrDestroyed(PreviouslyDebuggedControlRig))
		{
			...
			PreviouslyDebuggedControlRig->OnPostForwardsSolve_AnyThread().RemoveAll(this);
			...
			PreviouslyDebuggedControlRig->OnEarlyExit_AnyThread().RemoveAll(this);
	...
	if (DebuggedControlRig)
	{
		...
		FControlRigAssetInterfacePtr Asset = GetControlRigAssetInterface();
		Asset->GetHierarchy()->HierarchyForSelectionPtr = DebuggedControlRig->DynamicHierarchy;
		...
		DebuggedControlRig->OnPostForwardsSolve_AnyThread().RemoveAll(this);
		...
		DebuggedControlRig->OnEarlyExit_AnyThread().RemoveAll(this);
		...
		DebuggedControlRig->OnPostForwardsSolve_AnyThread().AddSP(SharedRef(), &FControlRigBaseEditor::OnPostForwardsSolve_AnyThread);
		...
		DebuggedControlRig->OnEarlyExit_AnyThread().AddSP(SharedRef(), &FControlRigBaseEditor::OnEarlyExit_AnyThread);
		...
		ObjectFilter->OnAddOverride().BindLambda([RigBlueprint](const FOverrideStatusSubject& InSubject)
			{
				...
				InSubject.ForEach<UControlRig>(
					[RigBlueprint, InSubject](const FOverrideStatusObjectHandle<UControlRig>& InModuleRig)
					{
						...
						{
							const FControlRigOverrideValue Value(InSubject.GetPropertyPathString(), InModuleRig->GetVariablesStruct(), const_cast<UControlRig*>(InModuleRig.GetObject())->GetVariablesMemory());
							if(Value.IsValid())
							{
						...
						else
						{
							...
							{
								const FString PropertyPath = Variable.GetName().ToString();
								const FControlRigOverrideValue Value(PropertyPath, InModuleRig->GetVariablesStruct(), const_cast<UControlRig*>(InModuleRig.GetObject())->GetVariablesMemory());
								RigBlueprint->GetModularRigController()->SetConfigValueInModule(InModuleRig->GetFName(), Value);
							}
						}
					}
					...
					[RigBlueprint, InSubject](const FOverrideStatusObjectHandle<UControlRig>& InModuleRig)
					{
						if(const FRigModuleReference* ModuleReference = RigBlueprint->GetModularRigModel().FindModule(InModuleRig->GetFName()))
						{
							...
							else
							{
								...
								for(const FRigVMExternalVariable& Variable : Variables)
								{
									if(FProperty* Property = InModuleRig->GetClass()->FindPropertyByName(Variable.GetName()))
									{
							...
							uint8* CDO = AssetReference.GetVariablesMemory();
...
{	
	...
	TUniquePtr<const FScopedRestoreEventQueue> ScopedRestoreEventQueue;
	if (!UControlRigEditorSettings::Get()->bCompileResetsEventQueue)
	{
		ScopedRestoreEventQueue = MakeUnique<FScopedRestoreEventQueue>(*this);
	}

	{
		DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()

		TUniquePtr<IControlRigEditorAssetInterface::FControlValueScope> ValueScope;
		if (!UControlRigEditorSettings::Get()->bResetControlsOnCompile) // if we need to retain the controls
		{
			ValueScope = MakeUnique<IControlRigEditorAssetInterface::FControlValueScope>(GetControlRigAssetInterface());
		}
...
void FControlRigBaseEditor::HandleVMCompiledEventImpl(UObject* InCompiledObject, URigVM* InVM, FRigVMExtendedExecuteContext& InContext)
{
	...
	if(FControlRigAssetInterfacePtr ControlRigBlueprint = GetControlRigAssetInterface())
	{
		...
		{
			ControlRigBlueprint->UpdateElementKeyRedirector(nullptr);
		}
	}
}
...
void FControlRigBaseEditor::SaveAsset_ExecuteImpl()
{
	...
	{
		if (RigBlueprint->GetRigVMAssetInterface()->IsBlueprintAsset())
		{
	...
	ActionDatabase.ClearAssetActions(UControlRigEditorAssetInterface::StaticClass());
	ActionDatabase.RefreshClassActions(UControlRigEditorAssetInterface::StaticClass());
}
...
void FControlRigBaseEditor::TickImpl(float DeltaTime)
{
	...
	// Initial viewport focus for assets opened with no preview mesh. We want to make sure the rig's visible
	// controls are on screen by default; without this the camera can sit in empty space and the user has no
	// idea where the rig content is (Face_ControlBoard_CtrlRig is the canonical example).
	// We do this in the tick (rather than HandleViewportCreated) because controls only exist after the
	// blueprint has compiled. EditMode::FrameItems() handles the deferral if the viewport client isn't ready
	// yet by storing the keys and processing them on the next EditMode::Tick.
	if (bPendingInitialViewportFocus)
	{
		// Safety bail: if the hierarchy never populates (e.g. compile error, modular rig with no resolved
		// connectors) we don't want to keep retrying every tick forever. Give it a small budget.
		static constexpr int32 MaxPendingFocusTicks = 60;
		++PendingInitialViewportFocusTickCount;

		// Skip the first tick: this gives the rig at least one frame to run its construction event and
		// evaluate, so control transforms reflect their resolved positions and not stale defaults. From
		// the second tick onward we attempt the framing each tick until it succeeds or we hit the budget.
		if (PendingInitialViewportFocusTickCount > 1)
		{
			if (FControlRigEditorEditMode* EditMode = GetEditMode())
			{
				if (URigHierarchy* Hierarchy = GetHierarchyBeingDebugged())
				{
					TArray<FRigElementKey> VisibleControlKeys;
					Hierarchy->ForEach<FRigControlElement>([&VisibleControlKeys](const FRigControlElement* ControlElement) -> bool
					{
						if (!ControlElement || !ControlElement->Settings.IsVisible())
						{
							return true;
						}

						// Proxy controls with BasedOnSelection visibility are currently hidden until the user
						// selects something they're proxying for; including them would pull the camera off the
						// actually-visible part of the rig.
						...
						VisibleControlKeys.Add(ControlElement->GetKey());
						return true;
					});

					if (VisibleControlKeys.Num() > 0)
					{
						EditMode->FrameItems(VisibleControlKeys);
						bPendingInitialViewportFocus = false;
					}
				}
			}
		}

		if (PendingInitialViewportFocusTickCount >= MaxPendingFocusTicks)
		{
			bPendingInitialViewportFocus = false;
		}
	}
...
void FControlRigBaseEditor::HandleViewportCreated(const TSharedRef<class IPersonaViewport>& InViewport)
{
	...
	UE::ControlRigEditor::PopulateControlRigViewportToolbarControlRigSubmenu();
	...
	if (FAnimationViewportClient* AnimViewportClient = static_cast<FAnimationViewportClient*>(&ViewportClient))
	{
		...
		TWeakPtr<FControlRigBaseEditor> WeakThis = SharedRef();
		AnimViewportClient->OnSetBoneSize.BindLambda([WeakThis](float InBoneSize)
		{
			if (TSharedPtr<FControlRigBaseEditor> PinnedThis = WeakThis.Pin())
			{
				if (FControlRigAssetInterfacePtr RigBlueprint = PinnedThis->GetControlRigAssetInterface())
				{
					RigBlueprint->Modify();
		...
		AnimViewportClient->OnGetBoneSize.BindLambda([WeakThis]() -> float
		{
			if (TSharedPtr<FControlRigBaseEditor> PinnedThis = WeakThis.Pin())
			{
				if (FControlRigAssetInterfacePtr RigBlueprint = PinnedThis->GetControlRigAssetInterface())
...
void FControlRigBaseEditor::SetNextEventQueue()
{
	const TArray<FName> EventQueue = GetEventQueue();
...
void FControlRigBaseEditor::SetPriorEventQueue()
{
	const TArray<FName> PriorEventQueue = GetLastEventQueue();

	if (!PriorEventQueue.IsEmpty())
	{
		constexpr bool bCompile = false;
		SetEventQueueSuper(PriorEventQueue, bCompile);
	}
}

void FControlRigBaseEditor::ToggleForwardsSolveAndConstruction()
{
	const TArray<FName> EventQueue = GetEventQueue();
...
void FControlRigBaseEditor::HandleToggleControlVisibility()
{
	if (FControlRigEditMode* EditMode = GetEditMode())
	{
		EditMode->ToggleDisplayControlsEnabled();
	}
}
...
bool FControlRigBaseEditor::AreControlsVisible() const
{
	if (FControlRigEditMode* EditMode = GetEditMode())
	{
		return EditMode->IsDisplayControlsEnabled();
	}
	return false;
}
...
void FControlRigBaseEditor::HandleToggleControlsAsOverlay()
{
	if (FControlRigEditMode* EditMode = GetEditMode())
	{
		EditMode->ToggleControlsAsOverlay();
	}
}
...
bool FControlRigBaseEditor::AreControlsAsOverlay() const
{
	if (FControlRigEditMode* EditMode = GetEditMode())
	{
		return EditMode->IsControlsAsOverlayEnabled();
	}
	return false;
}
...
void FControlRigBaseEditor::HandleToggleSchematicViewportShowEmptySocketsOnly()
{
	if (SchematicViewport.IsValid())
	{
		UControlRigEditorSettings::Get()->bSchematicViewportShowEmptySocketsOnly = !UControlRigEditorSettings::Get()->bSchematicViewportShowEmptySocketsOnly;
		UControlRigEditorSettings::Get()->SaveConfig();
...
bool FControlRigBaseEditor::IsSchematicViewportShowEmptySocketsOnlyActive() const
{
	return 
		IsSchematicViewportActive() &&
		UControlRigEditorSettings::Get()->bSchematicViewportShowEmptySocketsOnly;
}

void FControlRigBaseEditor::HandleToggleCompileResetsEventQueue()
{
	UControlRigEditorSettings* EditorSettings = GetMutableDefault<UControlRigEditorSettings>();
	EditorSettings->bCompileResetsEventQueue = !EditorSettings->bCompileResetsEventQueue;

	EditorSettings->SaveConfig();
}

bool FControlRigBaseEditor::IsToggleCompileResetsEventQueueActive() const
{
	return UControlRigEditorSettings::Get()->bCompileResetsEventQueue;
}
...
bool FControlRigBaseEditor::IsRunningForwardSolve() const
{
	return GetEventQueue() == ForwardsSolveEventQueue;
}

void FControlRigBaseEditor::OnRunGraphOnceToggledImpl(bool bRunGraphOnceEnabled)
{
	if (UControlRig* ControlRig = GetControlRig())
	{
		if (bRunGraphOnceEnabled)
		{
			ControlRig->SetEvaluationsLeft(1);
		}
		else
		{
			ControlRig->ResetEvaluationsLeft();
		}
	}	
}
...
{
	if (const UControlRigEditorSettings* Settings = GetDefault<UControlRigEditorSettings>())
	{
...
{
	if (UControlRigEditorSettings* Settings = GetMutableDefault<UControlRigEditorSettings>())
	{
...
{
	const FControlRigEditMode* EditMode = GetEditMode();
	return EditMode ? EditMode->IsDisplayNullsEnabled() : false;
}
...
{
	const FControlRigEditMode* EditMode = GetEditMode();
	return EditMode ? EditMode->IsDisplaySocketsEnabled() : false;
}
...
void FControlRigBaseEditor::HandlePreviewSceneCreated(const TSharedRef<IPersonaPreviewScene>& InPersonaPreviewScene)
{
	...
	{
		UE_LOGF(LogControlRig, Error, "Control Rig editor ground asset not found :%ls", GroundAssetPath);
	}
...
void FControlRigBaseEditor::UpdateRigVMHostImpl()
{
	...
	// For backwards compatibility, we cannot assume the runtime asset interface is a IControlRigRuntimeAssetInterface (older assets created URigVMBlueprintGeneratedClass instead 
	// of UControlRigBlueprintGeneratedClass)
	TScriptInterface<IRigVMRuntimeAssetInterface> RuntimeAsset = Blueprint->GetRuntimeAssetInterface().GetObject();
	if (!RuntimeAsset)
	{
		if (UControlRigBlueprint* CRBP = Cast<UControlRigBlueprint>(Blueprint.GetObject()))
		{
			RuntimeAsset = CRBP->GetRigVMBlueprintGeneratedClass();
		}
	}
	if (RuntimeAsset)
	{
		...
		if (AnimInstance && ControlRig)
		{
			...
			if (TScriptInterface<IControlRigRuntimeAssetInterface> CRRuntimeAsset = Blueprint->GetRuntimeAssetInterface())
			{
				CRRuntimeAsset->GetShapeLibraries() = Blueprint->GetShapeLibraries();
			}
			else
			{
				if (UControlRig* CDO = Cast<UControlRig>(RuntimeAsset.GetObject()->GetClass()->GetDefaultObject()))
				{
			...
			ControlRig->OnEarlyExit_AnyThread().RemoveAll(this);
			...
			ControlRig->OnPostForwardsSolve_AnyThread().AddSP(SharedRef(), &FControlRigBaseEditor::OnPostForwardsSolve_AnyThread);
			ControlRig->OnEarlyExit_AnyThread().AddSP(SharedRef(), &FControlRigBaseEditor::OnEarlyExit_AnyThread);
...
FVector2D FControlRigBaseEditor::ComputePersonaProjectedScreenPos(const FVector& InWorldPos, bool bClampToScreenRectangle)
{
	if (PreviewViewport.IsValid())
	{
		...
		const FMatrix& WorldToView = SceneView->ViewMatrices.GetWorldToView();
		const FMatrix& ViewToProj = SceneView->ViewMatrices.GetViewToClip();
...
void FControlRigBaseEditor::GenerateEventQueueMenuContentImpl(FMenuBuilder& MenuBuilder)
{
	...
	MenuBuilder.BeginSection(TEXT("Settings"), LOCTEXT("EventQueueSettings", "Settings"));
	MenuBuilder.AddMenuEntry(FControlRigEditorCommands::Get().ToggleCompileResetsEventQueue, TEXT("ToggleCompileResetsEventQueue"), TAttribute<FText>(), TAttribute<FText>(), GetEventQueueIconImpl(BackwardsAndForwardsSolveEventQueue));
	MenuBuilder.EndSection();
...
void FControlRigBaseEditor::SetRigElementTransform(const FRigElementKey& InElement, const FTransform& InTransform, bool bLocal)
{
	...
	switch (InElement.Type)
	{
		...
		default:
		{
			ensureMsgf(false, TEXT("Unsupported RigElement Type : %d"), EnumToUnderlyingType(InElement.Type));
			break;
		}
	}
...
bool FControlRigBaseEditor::HandleRequestDirectManipulation(ERigControlType InControlType) const
{
	...
	for(const FRigDirectManipulationTarget& Target : Targets)
	{
		if(Target.ControlType == InControlType || Target.ControlType == ERigControlType::EulerTransform)
		{
			...
			if(FControlRigAssetInterfacePtr Blueprint = GetControlRigAssetInterface())
			{
				// Mark this node as interactive so the compiler gives it its own literal
				// registers rather than sharing them with other nodes that have the same
				// default values. Only recompile if the node is not already in the set.
				if(DirectManipulationSubject.IsValid())
				{
					auto& ASTSettings = Blueprint->GetRigVMAssetInterface()->GetVMCompileSettings().ASTSettings;
					if(!ASTSettings.InteractiveNodes.Contains(FObjectKey(DirectManipulationSubject.Get())))
					{
						ASTSettings.InteractiveNodes.Add(FObjectKey(DirectManipulationSubject.Get()));
						Blueprint->GetRigVMAssetInterface()->RecompileVM();
					}
				}
...
void FControlRigBaseEditor::OnDirectManipulationChanged(TSharedPtr<FString> NewValue, ESelectInfo::Type SelectInfo)
{
	...
	// default values. Only recompile if the node is not already in the set.
	{
		auto& ASTSettings = ControlRigBlueprint->GetRigVMAssetInterface()->GetVMCompileSettings().ASTSettings;
		if(!ASTSettings.InteractiveNodes.Contains(FObjectKey(UnitNode)))
		{
			ASTSettings.InteractiveNodes.Add(FObjectKey(UnitNode));
...
const TArray<FRigDirectManipulationTarget> FControlRigBaseEditor::GetDirectManipulationTargets() const
{
	if(DirectManipulationSubject.IsValid())
	{
		if (UControlRig* DebuggedControlRig = Cast<UControlRig>(GetRigVMAssetInterface()->GetObjectBeingDebugged()))
		{
			if (DebuggedControlRig->GetVM() && !DebuggedControlRig->GetVM()->IsNativized())
			{
...
EVisibility FControlRigBaseEditor::GetPreviewingNodeVisibility() const
{
	if (UControlRig* DebuggedControlRig = Cast<UControlRig>(GetRigVMAssetInterface()->GetObjectBeingDebugged()))
	{
		if (DebuggedControlRig->GetDebugInfo().GetStepCondition().OriginInstruction != INDEX_NONE)
		{
			return EVisibility::Visible;
		}
	}
...
void FControlRigBaseEditor::OnPreviewingNodeJumpTo()
{
	...
	const int32 InstructionToJumpTo = DebuggedControlRig->GetDebugInfo().GetStepCondition().OriginInstruction;
...
void FControlRigBaseEditor::BindCommandsImpl()
{
	...
	GetToolkitCommands()->MapAction(
		FControlRigEditorCommands::Get().SetNextEventQueue,
		FExecuteAction::CreateSP(SharedEditor, &FControlRigBaseEditor::SetNextEventQueue),
		FCanExecuteAction());

	GetToolkitCommands()->MapAction(
		FControlRigEditorCommands::Get().SetPriorEventQueue,
		FExecuteAction::CreateSP(SharedEditor, &FControlRigBaseEditor::SetPriorEventQueue),
		FCanExecuteAction());

	GetToolkitCommands()->MapAction(
		FControlRigEditorCommands::Get().ToggleForwardsSolveAndConstruction,
		FExecuteAction::CreateSP(SharedEditor, &FControlRigBaseEditor::ToggleForwardsSolveAndConstruction),
		FCanExecuteAction());
	...
	GetToolkitCommands()->MapAction(
		FControlRigEditorCommands::Get().ToggleSchematicViewportShowEmptySocketsOnly,
		FExecuteAction::CreateSP(SharedEditor, &FControlRigBaseEditor::HandleToggleSchematicViewportShowEmptySocketsOnly),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(SharedEditor, &FControlRigBaseEditor::IsSchematicViewportShowEmptySocketsOnlyActive));
	...
	GetToolkitCommands()->MapAction(
		FControlRigEditorCommands::Get().ToggleCompileResetsEventQueue,
		FExecuteAction::CreateSP(SharedEditor, &FControlRigBaseEditor::HandleToggleCompileResetsEventQueue),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(SharedEditor, &FControlRigBaseEditor::IsToggleCompileResetsEventQueueActive));
}
...
void FControlRigBaseEditor::UnbindCommandsImpl()
{
	...
	GetToolkitCommands()->UnmapAction(FControlRigEditorCommands::Get().SetNextEventQueue);
	GetToolkitCommands()->UnmapAction(FControlRigEditorCommands::Get().SetPriorEventQueue);
	GetToolkitCommands()->UnmapAction(FControlRigEditorCommands::Get().ToggleForwardsSolveAndConstruction);
	...
	GetToolkitCommands()->UnmapAction(FControlRigEditorCommands::Get().ToggleCompileResetsEventQueue);
}
...
void FControlRigBaseEditor::HandlePostCompileModularRigs(FRigVMEditorAssetInterfacePtr InBlueprint)
{
	RefreshDetailView();
}
...
void FControlRigBaseEditor::SwapModuleWithinAsset()
{
	...
	{
		return IControlRigEditorAssetInterface::GetRigType(AssetData) == EControlRigType::RigModule;
	});
	...
	WidgetArgs
		.EnableUndo(true)
		.CloseOnSuccess(true)
		.OnGetReferences_Lambda([Blueprint, Asset](const FAssetData& InReferencedAsset) -> TArray<FSoftObjectPath>
		{
			TArray<FSoftObjectPath> Result;
			FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
			IAssetRegistry& AssetRegistry = AssetRegistryModule.GetRegistry();

			FControlRigAssetStrongReference ReferencedAsset;
			if (FControlRigAssetInterfacePtr ReferencedBlueprint = InReferencedAsset.GetAsset())
			{
				ReferencedAsset = ReferencedBlueprint->GetControlRigAssetReference();
			}
			...
			if (Blueprint)
			{
				if (Blueprint->IsModularRig())
				{
					TArray<const FRigModuleReference*> Modules = Blueprint->GetModularRigModel().FindModuleInstancesOfClass(ReferencedAsset.GetEditorAsset());
		...
		.OnSwapReference_Lambda([](const FSoftObjectPath& ModulePath, const FAssetData& NewModuleAsset) -> bool
		{
			...
			{
				NewModuleClass.Set(TSubclassOf<UControlRig>(GeneratedClass));
			}
			else if (UControlRigRuntimeAsset* RuntimeAsset = Cast<UControlRigRuntimeAsset>(NewModuleAsset.GetAsset()))
			{
				NewModuleClass.Set(RuntimeAsset);
			}
			if (NewModuleClass.IsValid())
			{
				if (FControlRigAssetInterfacePtr RigBlueprint = ModulePath.GetWithoutSubPath().ResolveObject())
				{
					...
					{
						return RigBlueprint->GetModularRigController()->SwapModuleSource(Module->Name, NewModuleClass);
					}
				}
			}
			return false;
...
void FControlRigBaseEditor::SwapModuleAcrossProject()
{
	...
	WidgetArgs
		...
		{
			return IControlRigEditorAssetInterface::GetReferencesToRigModule(Asset);
		})
		.OnSwapReference_Lambda([](const FSoftObjectPath& ModulePath, const FAssetData& NewModuleAsset) -> bool
		{
			FControlRigAssetStrongReference NewModuleClass;
...
void FControlRigBaseEditor::OnGraphNodeDropToPerformImpl(TSharedPtr<FDragDropOperation> InDragDropOp, UEdGraph* InGraph, const FVector2f& InNodePosition, const FVector2f& InScreenPosition)
{
	...
	if (InDragDropOp->IsOfType<FRigHierarchyElementDragDropOp>())
	{
		TSharedPtr<FRigHierarchyElementDragDropOp> RigHierarchyOp = StaticCastSharedPtr<FRigHierarchyElementDragDropOp>(InDragDropOp);
...
void FControlRigBaseEditor::HandleRefreshEditorFromBlueprintImpl(FRigVMEditorAssetInterfacePtr InBlueprint)
{
	OnHierarchyChanged();
...
{
	// make sure to reapply this in case
	if (SharedRigVMEditorRef()->IsRunGraphOnceOn())
	{
		OnRunGraphOnceToggledImpl(true);
	}
...
void FControlRigBaseEditor::OnPostForwardsSolve_AnyThread(UControlRig* InRig, const FName& InEventName)
{
	const UControlRig* ControlRig = GetControlRig();
	if (!ControlRig || ControlRig != InRig)
	{
		return;
	}

	// this is the last evaluation
	if (ControlRig->GetEvaluationsLeft() == 1)
	{
		if (const URigHierarchy* Hierarchy = ControlRig->GetHierarchy())
		{
			static const FRigElementKeyCollection EmptyCollection;
			BonePoseFromLastEvaluation = Hierarchy->GetPose(false, ERigElementType::Bone, EmptyCollection, false);
		}
	}
	else
	{
		BonePoseFromLastEvaluation.Reset();
	}
}
	...
	if(FControlRigAssetInterfacePtr RigBlueprint = GetControlRigAssetInterface())
	{
		...
		if(bShouldExecute && RigBlueprint->IsModularRig())
		{
			...
			if(RigBlueprint->GetModularRigModel().Connections.IsEmpty() && RigBlueprint->GetModularRigModel().Modules.Num() == 1 && RigBlueprint->GetHierarchy()->Num(ERigElementType::Bone) > 0)
			{
				...
				if (DefaultRootModuleObj)
				{
					ControlRigAssetReference = FControlRigAssetStrongReference(DefaultRootModuleObj);
				}
				
				if(ControlRigAssetReference.IsValid())
				{
					if(ControlRigAssetReference.Get() == RootModule.ControlRigAssetReference.Get())
					{
...
void FControlRigBaseEditor::OnEarlyExit_AnyThread(URigVMHost* InHost, URigVMHost::EEarlyExitReason InReason)
{
	if (InReason != URigVMHost::EEarlyExitReason_NoEvaluationsLeft)
	{
		return;
	}
	
	UControlRig* ControlRig = GetControlRig();
	if (!ControlRig || ControlRig != InHost)
	{
		return;
	}
	...
	if (BonePoseFromLastEvaluation.Num() == 0)
	{
		return;
	}

	Hierarchy->SetPose(BonePoseFromLastEvaluation, ERigTransformType::CurrentGlobal);
}
