...
void SRigCurveContainer::Construct(const FArguments& InArgs, TSharedRef<IControlRigBaseEditor> InControlRigEditor)
{
	...
	const TSharedRef<IRigVMEditor> RigVMEditor = InControlRigEditor->SharedRigVMEditorRef();
	const TScriptInterface<IRigVMEditorAssetInterface> AssetInterface = RigVMEditor->GetRigVMAssetInterface();
	if (AssetInterface)
	{
		EdGraphNodeFunctionRegistry = FRigVMEdGraphNodeRegistry::GetOrCreateRegistry(AssetInterface, URigVMUnitNode::StaticClass());
	}
...
void SRigCurveContainer::CreateRigCurveList( const FString& SearchText )
{
	...
	if (Hierarchy)
	{
		...
		Hierarchy->ForEach<FRigCurveElement>([this](FRigCurveElement* CurveElement) -> bool
		{
			...
			const FSlateColor ItemTextColor = IsCurveElementUsedInGraph(*CurveElement) ?
				FSlateColor::UseForeground() :
				FSlateColor::UseSubduedForeground();

            const TSharedRef<FDisplayedRigCurveInfo> NewItem = FDisplayedRigCurveInfo::Make(CurveElement->GetFName(), ItemTextColor);
...
bool SRigCurveContainer::IsCurveElementUsedInGraph(const FRigCurveElement& CurveElement) const
{
	if (!ensure(EdGraphNodeFunctionRegistry.IsValid()))
	{
		return false;
	}

	const TArray<TWeakObjectPtr<URigVMEdGraphNode>>& WeakConnectedEdGraphNodes = EdGraphNodeFunctionRegistry->GetConnectedEdGrapNodes();
	
	return 
		WeakConnectedEdGraphNodes.ContainsByPredicate(
			[&CurveElement](const TWeakObjectPtr<URigVMEdGraphNode>& WeakEdGraphNode)
				{
					const URigVMUnitNode* UnitNode = WeakEdGraphNode.IsValid() ? Cast<URigVMUnitNode>(WeakEdGraphNode->GetModelNode()) : nullptr;
					const UScriptStruct* ScriptStruct = UnitNode ? UnitNode->GetScriptStruct() : nullptr;
            					
					if (UnitNode &&	ScriptStruct)
					{
						if (ScriptStruct == FRigUnit_CurveExists::StaticStruct())
						{
							const URigVMPin* CurvePin = UnitNode->FindPin(GET_MEMBER_NAME_STRING_CHECKED(FRigUnit_CurveExists, Curve));
							return CurvePin && CurvePin->GetDefaultValue() == CurveElement.GetName();
						}    
						else if (ScriptStruct == FRigUnit_GetCurveValue::StaticStruct())
						{
							const URigVMPin* CurvePin = UnitNode->FindPin(GET_MEMBER_NAME_STRING_CHECKED(FRigUnit_GetCurveValue, Curve));
							return CurvePin && CurvePin->GetDefaultValue() == CurveElement.GetName();
						}
						else if (ScriptStruct == FRigUnit_SetCurveValue::StaticStruct())
						{
							const URigVMPin* CurvePin = UnitNode->FindPin(GET_MEMBER_NAME_STRING_CHECKED(FRigUnit_SetCurveValue, Curve));
							return CurvePin && CurvePin->GetDefaultValue() == CurveElement.GetName();
						}            		
						else if (ScriptStruct == FRigUnit_UnsetCurveValue::StaticStruct())
						{
							const URigVMPin* CurvePin = UnitNode->FindPin(GET_MEMBER_NAME_STRING_CHECKED(FRigUnit_UnsetCurveValue, Curve));
							return CurvePin && CurvePin->GetDefaultValue() == CurveElement.GetName();
						}
					}
...
void SRigCurveContainer::HandleRefreshEditorFromBlueprint(FRigVMEditorAssetInterfacePtr InBlueprint)
{
	RefreshCurveList_AnyThread();
}
