...
FControlRigLegacyEditor::~FControlRigLegacyEditor()
{
	...
	if (RigBlueprint)
	{
		...
		if (RigBlueprint->IsModularRig())
		{
			...
			RigBlueprint->IRigVMEditorAssetInterface::OnSetObjectBeingDebugged().RemoveAll(&SchematicModel);
...
void FControlRigLegacyEditor::InitToolMenuContext(FToolMenuContext& MenuContext)
{
	...
	if (FRigVMEditorAssetInterfacePtr RigVMAsset = GetRigVMAssetInterface())
	{
