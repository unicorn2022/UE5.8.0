...
public:
	...
	static TSharedRef<FDisplayedRigCurveInfo> Make(const FName& InCurveName, const FSlateColor& InTextColor)
	{
		return MakeShareable(new FDisplayedRigCurveInfo(InCurveName, InTextColor));
	}
...
protected:
	...
	FDisplayedRigCurveInfo(const FName& InCurveName, const FSlateColor& InTextColor)
...
private:
	...
	*/
	...
	/** Returns true if the curve is used in the graph */
	bool IsCurveElementUsedInGraph(const FRigCurveElement& CurveElement) const;
	...
	/** Function graph node registry to find coneected curves */
