...
#include "SpacePicker/RigFloatingSpacePickerManager.h"
...
TAutoConsoleVariable<bool> CVarMarqueePivotSelect(
	TEXT("ControlRig.Sequencer.MarqueePivotSelect"),
	false,
	TEXT("If true, marquee selection (BoxSelect/FrustumSelect) in Control Rig edit mode ")
	TEXT("additionally picks any control whose pivot lies inside the marquee region, ")
	TEXT("in addition to the existing bounds/hit-proxy tests. Catches controls whose ")
	TEXT("gizmo mesh is asymmetric, sparse, or occluded. Set to false to restore the ")
	TEXT("prior bounds-only behavior."));
	...
	, bIsConstructionEventRunning(false)
{
	...
	bShowControlsAsOverlay = IsControlsAsOverlayEnabled();
...
bool FControlRigEditMode::SetSequencer(TWeakPtr<ISequencer> InSequencer)
{
	if (InSequencer != WeakSequencer)
	{
		...
		{
			constexpr bool bDisable = false;
			const TSharedPtr<ISequencer> PreviousSequencer = WeakSequencer.Pin();
			PreviousSequencer->GetSequencerWidgetInterface().EnablePendingFocusOnHovering(bDisable);
		}
		...
		if (InSequencer.IsValid())
		{
			...
			if (AnimDetailsProxyManager)
			{
				AnimDetailsProxyManager->RequestUpdateProxies();
			}
			
			Sequencer->GetSequencerWidgetInterface().EnablePendingFocusOnHovering(Private::bFocusMode);
		}
...
void FControlRigEditMode::Enter()
{
	...
	if (UsesToolkits())
	{
		...
		if (!Toolkit.IsValid())
		{
			...
			Toolkit->GetToolkitCommands()->Append(CommandBindings.ToSharedRef());
		}
	...
	if (UControlRigEditModeSettings* Settings = GetMutableSettings())
	{
		...
		if (!Settings->OnSettingChanged().IsBoundToObject(this))
		{
			Settings->OnSettingChanged().AddSP(this, &FControlRigEditMode::OnSettingsPropertyChanged);
		}
	}

	if (IsInLevelEditor() && !ViewportSelectabilityDelegateHandle.IsValid())
	{
		ViewportSelectabilityDelegateHandle = FEditorViewportSelectability::RegisterObjectSelectablePredicate(
			FOnIsObjectSelectableInViewport::CreateRaw(this, &FControlRigEditMode::IsViewportObjectSelectable)
		);
	}
	...
	if (UModeManagerInteractiveToolsContext* const InteractiveToolsContext = GetModeManager()->GetInteractiveToolsContext())
	{
		if (!BuildViewportInteractionsDelegate.IsValid())
		{
			BuildViewportInteractionsDelegate = InteractiveToolsContext->OnBuildViewportInteractions().AddSPLambda(this, [](UViewportInteractionsBehaviorSource* Source)
			{
				// Add interactions specific to Control Rig
				Source->AddInteraction<UControlRigFrustumSelectInteraction>();
			});
		}
	}
...
{
	if (ILevelEditor* LevelEditor = Private::GetLevelEditor())
	{
		FString ActiveToolName =
			LevelEditor->GetEditorModeManager().GetInteractiveToolsContext()->ToolManager->GetActiveToolName(EToolSide::Left);
		if (ActiveToolName == TEXT("SequencerPivotTool"))
		{
			LevelEditor->GetEditorModeManager().GetInteractiveToolsContext()->ToolManager->DeactivateTool(
				EToolSide::Left, EToolShutdownType::Completed
			);
		}
	}
}
...
{
	AnimDetailsProxyManager = nullptr;
	...
	if (IsInLevelEditor())
	{
		...
		if (const ILevelEditor* const LevelEditor = Private::GetLevelEditor())
		{
			if (UModeManagerInteractiveToolsContext* const InteractiveToolsContext = LevelEditor->GetEditorModeManager().GetInteractiveToolsContext())
			{
				InteractiveToolsContext->OnBuildViewportInteractions().Remove(BuildViewportInteractionsDelegate);
				BuildViewportInteractionsDelegate.Reset();
			}
		}
	}
	...
	if (ViewportSelectabilityDelegateHandle.IsValid())
	{
		FEditorViewportSelectability::UnregisterObjectSelectablePredicate(ViewportSelectabilityDelegateHandle);
		ViewportSelectabilityDelegateHandle.Reset();
	}

	UnregisterPendingFocusMode();
	DirectControl.UnBind();
...
void FControlRigEditMode::Tick(FEditorViewportClient* ViewportClient, float DeltaTime)
{
	...
	// Ensure transient control shape actors are selected while DM is active.
	// RecreateControlShapeActors() may be called from paths outside this Tick
	// (e.g. OnPostConstruction_AnyThread), which resets RecreateControlShapesRequired
	// before the selection loop above can run, leaving the shape actor unselected.
	// ComputePivotFromShapeActors gates entirely on IsSelected(), so an unselected
	// shape actor causes the gizmo to fall back to the world origin.
	for (TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RuntimeControlRigs)
	{
		if (UControlRig* ControlRig = RuntimeRigPtr.Get())
		{
...
void FControlRigEditMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	const UControlRigEditModeSettings* EditModeSettings = GetSettings();
	if (!IsDisplayControlsEnabled())
	{
		return;
	}
	...
	const bool bIsEditorWorld = WorldType == EWorldType::Editor || WorldType == EWorldType::EditorPreview;
	
	const bool bIsAssetEditor =
		(WorldType == EWorldType::Editor || WorldType == EWorldType::EditorPreview) &&
		EditorViewportClient &&
		!EditorViewportClient->IsLevelEditorClient();
	...
	for (TWeakObjectPtr<UControlRig>& ControlRigPtr : RuntimeControlRigs)
	{
		...
		if (ControlRig && ControlRig->GetControlsVisible())
		{
			...
			if (EditModeSettings->bDisplayHierarchy || bHasFKRig)
			{
			...
			const float AxisScale = GetAxesOnSelectionScale();

			if (bIsEditorWorld && (IsDisplayNullsEnabled() || ControlRig->IsConstructionModeEnabled()))
			{
				...
				ControlRig->DrawInterface.DrawAxes(FTransform::Identity, SpaceTransforms, AxisScale);
				ControlRig->DrawInterface.DrawAxes(FTransform::Identity, SelectedSpaceTransforms, FLinearColor(1.0f, 0.34f, 0.0f, 1.0f), AxisScale);
			}

			const bool bDisplayAxesOnSelection = IsDisplayAxesOnSelectionEnabled();

			if (PDI &&
				bDisplayAxesOnSelection &&
				AxisScale > SMALL_NUMBER)
			{
				TArray<FRigElementKey> SelectedRigElements = GetSelectedRigElements(ControlRig);
				...
				for (const FRigElementKey& SelectedElement : SelectedRigElements)
				{
					...
					PDI->DrawLine(ElementTransform.GetTranslation(), ElementTransform.TransformPosition(FVector(AxisScale, 0.f, 0.f)), FLinearColor::Red, SDPG_Foreground);
					PDI->DrawLine(ElementTransform.GetTranslation(), ElementTransform.TransformPosition(FVector(0.f, AxisScale, 0.f)), FLinearColor::Green, SDPG_Foreground);
					PDI->DrawLine(ElementTransform.GetTranslation(), ElementTransform.TransformPosition(FVector(0.f, 0.f, AxisScale)), FLinearColor::Blue, SDPG_Foreground);
				}
			}
			...
			if (PDI &&
				bIsEditorWorld &&
				(IsDisplaySocketsEnabled() || ControlRig->IsConstructionModeEnabled()) &&
				AxisScale > SMALL_NUMBER)
			{
				const float Scale = AxisScale;
...
void FControlRigEditMode::DrawHUD(FEditorViewportClient* ViewportClient, FViewport* Viewport, const FSceneView* View, FCanvas* Canvas)
{
	...
	// Render any text instructions that were queued into the draw interfaces of the active rigs.
	// These get pushed by rigs via FRigVMDrawInterface::DrawText and can't be drawn via PDI.
	for (TWeakObjectPtr<UControlRig>& ControlRigPtr : RuntimeControlRigs)
	{
		if (UControlRig* ControlRig = ControlRigPtr.Get())
		{
			...
			ControlRig->DrawIntoCanvas(Canvas, View, ComponentTransform);
		}
	}
}
		...
		if (AreEditingControlRigDirectly())
		{
			for (TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RuntimeControlRigs)
			{
				if (UControlRig* ControlRig = RuntimeRigPtr.Get())
				{
					...
					if (!Blueprint)
					{
						Blueprint = ControlRig->GetGeneratedByAsset().GetObject();
					}
					if (Blueprint)
					{
...
bool FControlRigEditMode::GetCursor(EMouseCursor::Type& OutCursor) const
{
	if (!GetOnlySelectRigControls())
	{
		return false;
	}

	const FEditorModeTools* const ModeManager = GetModeManager();
	const FEditorViewportClient* const HoveredViewportClient = ModeManager ? ModeManager->GetHoveredViewportClient() : nullptr;
	if (!HoveredViewportClient || !HoveredViewportClient->Viewport)
	{
		return false;
	}

	HHitProxy* const HitProxy = HoveredViewportClient->Viewport->GetHitProxy(
		HoveredViewportClient->Viewport->GetMouseX(),
		HoveredViewportClient->Viewport->GetMouseY()
	);
	if (!HitProxy)
	{
	...
	const bool bSelectable = IsViewportHitProxySelectable(HitProxy);
	if (!bSelectable)
	{
		OutCursor = EMouseCursor::Default;
		return true;
...
bool FControlRigEditMode::BoxSelect(FBox& InBox, bool InSelect)
{
	...
	if (LevelViewportClient->IsInGameView() == true || !IsDisplayControlsEnabled())
	{
		...
		if (TStrongObjectPtr<UControlRig> ControlRig = WeakControlRig.Pin())
		{
			if (ControlRig && ControlRig->GetControlsVisible())
			{
				for (AControlRigShapeActor* ShapeActor : ShapeActors)
				{
					...
					const bool bPivotInBox = CVarMarqueePivotSelect.GetValueOnGameThread()
						&& InBox.IsInsideOrOn(ShapeActor->GetActorLocation());

					if (bPivotInBox || IntersectsBox(*ShapeActor, InBox, LevelViewportClient, bStrictDragSelection))
					{
...
bool FControlRigEditMode::FrustumSelect(const FConvexVolume& InFrustum, FEditorViewportClient* InViewportClient, bool InSelect)
{
	...
	if (bMalformedFrustum || InViewportClient->IsInGameView() == true || !IsDisplayControlsEnabled())
	{
	...
	if (CVarMarqueePivotSelect.GetValueOnGameThread())
	{
		for (const auto& [WeakControlRig, ShapeActors] : ControlRigShapeActors)
		{
			if (TStrongObjectPtr<UControlRig> ControlRig = WeakControlRig.Pin())
			{
				if (ControlRig && ControlRig->GetControlsVisible())
				{
					for (AControlRigShapeActor* ShapeActor : ShapeActors)
					{
						...
						if (InFrustum.IntersectPoint(ShapeActor->GetActorLocation()))
						{
							ControlsToSelect.FindOrAdd(ControlRig.Get()).AddUnique(ShapeActor->GetElementKey());
						}
					}
				}
			}
	...
	if (bIsAssetEditor)
	{
		...
		for(TWeakObjectPtr<UControlRig> WeakControlRig : RuntimeControlRigs)
		{
			if(UControlRig* ControlRig = WeakControlRig.Get())
			{
				if (IsDisplayNullsEnabled() || ControlRig->IsConstructionModeEnabled())
				{
					if(URigHierarchy* Hierarchy = ControlRig->GetHierarchy())
					{
						...
						for(int32 Index = 0; Index < Nulls.Num(); Index++)
						{
							...
							const float AxisScale = GetAxesOnSelectionScale();
							if(InFrustum.IntersectSphere(Origin, MaxScale * AxisScale))
							{
								bSomethingSelected = true;
...
bool FControlRigEditMode::InputDelta(FEditorViewportClient* InViewportClient, FViewport* InViewport, FVector& InDrag, FRotator& InRot, FVector& InScale)
{
	...
	const bool bEnableMMB = UEditorInteractiveGizmoManager::AreEditorGizmosAllowed(InViewportClient->GetModeTools());
	...
	DirectControl.bHasTransformChanged = bOtherSelectedObjects || bManipulatorMadeChange; 
...
bool FControlRigEditMode::ShouldDrawWidget() const
{
	for (const TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RuntimeControlRigs)
	{
		if (UControlRig* ControlRig = RuntimeRigPtr.Get())
		{
			...
			if (Binding && Binding->GetBoundObject())
			{
				if (AreRigElementSelectedAndMovable(ControlRig))
				{
					return true;
...
void FControlRigEditMode::ClearRigElementSelection(uint32 InTypes)
{
	auto GetController = [bControlRigEditor = AreEditingControlRigDirectly()](UControlRig* InControlRig) -> URigHierarchyController*
	{
		if (InControlRig)
		{
			...
			else if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = InControlRig->GetGeneratedByAsset().GetObject())
			{
				return RuntimeAsset->GetHierarchy()->GetController(true);
			}
		}
...
void FControlRigEditMode::UpdatePivotTransforms()
{
	...
	if (IsInLevelEditor() && InteractionScopes.IsEmpty() && !bSequencerPlaying)
	{
...
void FControlRigEditMode::UpdatePivotTransformsIfNeeded(UControlRig* InControlRig, FTransform& InOutTransform) const
{
	...
	if (!InControlRig || !InControlRig->GetHierarchy())
	{
		return;
	}
	...
	if (auto* ShapeActors = ControlRigShapeActors.Find(InControlRig))
	{
		...
		for (AControlRigShapeActor* ShapeActor : *ShapeActors)
		{
			if (FRigTransformElement* ControlElement = InControlRig->FindControl(ShapeActor->ControlName))
			{
				const FTransform Transform = InControlRig->GetHierarchy()->GetTransform(ControlElement, ERigTransformType::CurrentGlobal);
...
void FControlRigEditMode::HandleSelectionChanged()
{
	...
	for (const auto& ShapeActors : ControlRigShapeActors)
	{
		...
		{
			ShapeActor->ForEachComponent<UPrimitiveComponent>(bFromChildActors, [](UPrimitiveComponent* PrimitiveComponent) 
			{
...
void FControlRigEditMode::BindCommands()
{
	...
	CommandBindings->MapAction(
		...
		FExecuteAction::CreateRaw(this, &FControlRigEditMode::ZeroTransforms, true, false, (UControlRig*) nullptr));
	CommandBindings->MapAction(
		Commands.ZeroAllTransforms,
		FExecuteAction::CreateRaw(this, &FControlRigEditMode::ZeroTransforms, false, false, (UControlRig*) nullptr));
	CommandBindings->MapAction(
	...
	CommandBindings->MapAction(
		Commands.IsolateLinkedAnimTracks,
		FExecuteAction::CreateRaw(this, &FControlRigEditMode::IsolateLinkedAnimTracksForAllBindings, true));

	CommandBindings->MapAction(
		Commands.UnIsolateLinkedAnimTracks,
		FExecuteAction::CreateRaw(this, &FControlRigEditMode::IsolateLinkedAnimTracksForAllBindings, false));
	...
	CommandBindings->MapAction(
		Commands.SelectAnimLayerWeightChannel,
		FExecuteAction::CreateRaw(this, &FControlRigEditMode::SelectAnimLayerWeightChannel));

	CommandBindings->MapAction(
		Commands.OpenSpacePickerWidget,
		FExecuteAction::CreateRaw(this, &FControlRigEditMode::OpenTabMenu));
...
void FControlRigEditMode::IsolateLinkedAnimTracksForAllBindings(bool bIsolate)
{
	TSharedPtr<ISequencer> Sequencer = WeakSequencer.Pin();
	...
	UE::Sequencer::FCommonAnimationTrackEditor::IsolateLinkedAnimTracks(Sequencer, Guids, bIsolate);
...
void FControlRigEditMode::FrameSelection()
{
	...
	else
	{
		...
		for (TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RuntimeControlRigs)
		{
			if (UControlRig* ControlRig = RuntimeRigPtr.Get())
			{
				SelectedRigElements.Append(GetSelectedRigElements(ControlRig));
			}
		}

		FrameItems(SelectedRigElements);
	}
}
...
void FControlRigEditMode::SelectAnimLayerWeightChannel()
{
	ISequencer* Sequencer = WeakSequencer.Pin().Get();
	if (Sequencer)
	{
		if (UAnimLayers* AnimLayers = UAnimLayers::GetAnimLayers(Sequencer))
		{
			const bool bClearSelection = !(FSlateApplication::Get().GetModifierKeys().IsShiftDown());
			for (UAnimLayer* AnimLayer : AnimLayers->AnimLayers)
			{
				if (AnimLayer->GetSelectedInList())
				{
					AnimLayer->SelectWeightChannel(bClearSelection);
				}
			}
		}
	}
...
void FControlRigEditMode::OpenTabMenu()
{
	using namespace UE::ControlRigBlutility;
		
	if (!SupportUtilityActions())
	{
		return OpenSpacePickerWidget();
	}
	
	auto GetViewportWidget = [ModeTools = GetModeManager()]() -> TWeakPtr<SViewport>
	{
		// NOTE: remove ModeTools == &GLevelEditorModeTools() to extend the tab menu in the ControlRig editor as well.
		if (ModeTools && ModeTools == &GLevelEditorModeTools())
		{
			if (FEditorViewportClient* HoveredVPC = ModeTools->GetHoveredViewportClient())
			{
				const TSharedPtr<SEditorViewport>& EditorViewport = HoveredVPC->GetEditorViewportWidget();
				const TSharedPtr<FSceneViewport>& SceneViewport = EditorViewport ? EditorViewport->GetSceneViewport() : nullptr;
				return SceneViewport ? SceneViewport->GetViewportWidget() : nullptr;
			}
		}
		return nullptr;
	};
	...
	const TWeakPtr<SViewport>& ViewportWidget = GetViewportWidget();
	if (!ViewportWidget.IsValid() ||
		(Actions = FControlRigUtilityActions::GetControlRigUtilityActions(WorldPtr, RuntimeControlRigs), Actions.UtilityAndRigIndices.IsEmpty()))
	{
		return OpenSpacePickerWidget();
	}
	...
	// add space picker widget
	{
		using namespace UE::ControlRigEditor;
		if (const TSharedPtr<SRigSpacePickerMenuEntry> SpacePickerMenuEntry = GetSpacePickerManager().CreateSpacePickerMenuEntry())
		{
			MenuBuilder.BeginSection("SpacePicker", LOCTEXT("SpacePicker", "Switch Space"));
			MenuBuilder.AddWidget(SpacePickerMenuEntry.ToSharedRef(), FText::GetEmpty());
	...
	// add utility actions
	{
		const FBlutilityMenuExtensions::FMenuParams MenuParams({
		   LOCTEXT("ControlRigMode_Actions", "Control Rig Actions"),
		   LOCTEXT("ControlRigMode_Tooltip", "Control Rig Actions Tooltip"),
			   NAME_None,
			   false});

		const FBlutilityMenuExtensions::FObjectActionParams ActionParams({
		MoveTemp(Actions.SupportedRigs),
			MoveTemp(Actions.UtilityAndRigIndices),
			[](const FProperty*) { return true; }
		});
	
		FBlutilityMenuExtensions::CreateObjectBlutilityActionsMenu(MenuBuilder, MenuParams, ActionParams);
	}
	
	// create menu
	FSlateApplication::Get().PushMenu(
			ViewportWidget.Pin().ToSharedRef(),
			FWidgetPath(),
...
{
	GetSpacePickerManager().SummonSpacePickerAtCursor();
}

UE::ControlRigEditor::FRigFloatingSpacePickerManager& FControlRigEditMode::GetSpacePickerManager()
{
	using namespace UE::ControlRigEditor;
	
	if (!FloatingSpacePickerManager)
	{
		FloatingSpacePickerManager = MakePimpl<FRigFloatingSpacePickerManager>(*this);
	}
	
	return *FloatingSpacePickerManager;
}
...
static void ToggleTool(const FString& InToolName)
{
	if (ILevelEditor* LevelEditor = Private::GetLevelEditor())
	{
		...
		{
			LevelEditor->GetEditorModeManager().GetInteractiveToolsContext()->ToolManager->SelectActiveToolType(
				EToolSide::Left, InToolName
			);
...
void FControlRigEditMode::ToggleModuleManipulators()
{
	...
	if (!AreEditingControlRigDirectly() && IsDisplayControlsEnabled())
	{
		for (TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RuntimeControlRigs)
		{
			if (UControlRig* ControlRig = RuntimeRigPtr.Get())
...
void FControlRigEditMode::OpenPoseLibraryTab()
{
	if (UsesToolkits() && Toolkit.IsValid())
	{
		StaticCastSharedPtr<FControlRigEditModeToolkit>(Toolkit)->TryInvokeToolkitUI(FControlRigEditModeToolkit::PoseTabName);
	}
}

void FControlRigEditMode::ClosePoseLibraryTab()
{
	if (UsesToolkits() && Toolkit.IsValid())
	{
		StaticCastSharedPtr<FControlRigEditModeToolkit>(Toolkit)->TryCloseToolkitUI(FControlRigEditModeToolkit::PoseTabName);
	}
}
...
bool FControlRigEditMode::IsDisplayControlsEnabled() const
{
	return !GetDefault<UControlRigEditModeSettings>()->bHideControlShapes;
}

void FControlRigEditMode::ToggleDisplayControlsEnabled()
{
...
bool FControlRigEditMode::IsControlsAsOverlayEnabled() const
{
	return GetDefault<UControlRigEditModeSettings>()->bShowControlsAsOverlay;
}
...
bool FControlRigEditMode::IsDisplayNullsEnabled() const
{
	return GetDefault<UControlRigEditModeSettings>()->bDisplayNulls;
}

void FControlRigEditMode::ToggleDisplayNulls()
{
	UControlRigEditModeSettings* Settings = GetMutableDefault<UControlRigEditModeSettings>();
...
bool FControlRigEditMode::IsDisplaySocketsEnabled() const
{
	return GetDefault<UControlRigEditModeSettings>()->bDisplaySockets;
}

void FControlRigEditMode::ToggleDisplaySockets()
{
	UControlRigEditModeSettings* Settings = GetMutableDefault<UControlRigEditModeSettings>();
...
bool FControlRigEditMode::IsDisplayAxesOnSelectionEnabled() const
{
	return GetDefault<UControlRigEditModeSettings>()->bDisplayAxesOnSelection;
}

void FControlRigEditMode::ToggleDisplayAxesOnSelection()
{
	UControlRigEditModeSettings* Settings = GetMutableDefault<UControlRigEditModeSettings>();
...
float FControlRigEditMode::GetAxesOnSelectionScale() const
{
...
bool FControlRigEditMode::AreControlsVisible() const
{
	...
	return IsDisplayControlsEnabled();
}
...
void FControlRigEditMode::ZeroTransforms(bool bSelectionOnly, bool bIncludeChannels, UControlRig* InControlRig)
{
	// Gather up the control rigs for the selected controls
	TArray<UControlRig*> ControlRigs;
	if (InControlRig)
	{
		ControlRigs = {InControlRig};
	}
	else
	{
		ControlRigs = GetControlRigsBasedOnSelection(bSelectionOnly);
	}
...
void FControlRigEditMode::ZeroTransforms(UControlRig* ControlRig, const FRigControlModifiedContext& Context,  bool bSelectionOnly, bool bIncludeChannels)
{
	...
	for (const FRigElementKey& ElementToReset : TransformElementsToReset)
	{
		...
		if (FControlRigAssetInterfacePtr Asset = AssetReference.GetEditorAsset())
		{
			if (Asset->GetDebuggedControlRig() == ControlRig)
			{
				Asset->GetHierarchy()->SetLocalTransform(ElementToReset, InitialLocalTransform);
			}
		}
	}
...
TArray<UControlRig*> FControlRigEditMode::GetControlRigsBasedOnSelection(bool bSelectionOnly) const
{
	TArray<UControlRig*> SelectedControlRigs;
	TArray<UControlRig*> AllControlRigs;
	for (const TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RuntimeControlRigs)
	{
		if (UControlRig* ControlRig = RuntimeRigPtr.Get())
		{
			...
			AllControlRigs.Add(ControlRig);
		}
	}

	if (SelectedControlRigs.Num() > 0 || bSelectionOnly)
	{
		return SelectedControlRigs;
	}
	return AllControlRigs;
}

void FControlRigEditMode::InvertInputPose(bool bSelectionOnly, bool bIncludeChannels)
{
	// Gather up the control rigs for the selected controls
	TArray<UControlRig*> ControlRigs = GetControlRigsBasedOnSelection(bSelectionOnly);
	if (ControlRigs.Num() == 0)
	{
		return;
	}
...
{
	// compare with the last cache to track useless synthesized mouse moves
	if (!MouseMoveCache.Update(InViewport, InX, InY))
	{
		return false;
	}
	...
	{	
		HHitProxy* const HitProxy = InViewport->GetHitProxy(InX, InY);
		...
		for (const auto& [ControlRig, Shapes] : ControlRigShapeActors)
		{
			for (AControlRigShapeActor* ShapeActor : Shapes)
			{
				if (ShapeActor)
				{
					ShapeActor->SetHovered(ShapeActor->IsSelected() ? true : IsHovered(ShapeActor));
				}
			}
		}
	}
...
bool FControlRigEditMode::MouseLeave(FEditorViewportClient* /*InViewportClient*/, FViewport* /*InViewport*/)
{
	...
	for (auto& ShapeActors : ControlRigShapeActors)
	{
		...
		{
			ShapeActor->SetHovered(ShapeActor->IsSelected());
		}
	}
	...
	if (ensure(UseFocusMode))
	{
		auto OnFocusModeChanged = [this](IConsoleVariable*)
		{
			...
			if (WeakSequencer.IsValid())
			{
				...
				PreviousSequencer->GetSequencerWidgetInterface().EnablePendingFocusOnHovering(Private::bFocusMode);
			}
		};
...
void FControlRigEditMode::UnregisterPendingFocusMode()
{
	...
	{
		Sequencer->GetSequencerWidgetInterface().EnablePendingFocusOnHovering(bDisable);
	}
...
EEditAction::Type FControlRigEditMode::GetActionDragDuplicate()
{
	// Halt edit duplicate from a gizmo alt-drag action.
...
bool FControlRigEditMode::TryUpdatingControlsShapes(UControlRig* InControlRig)
{
	...
	// Prevent evaluating the rig while we collect and use raw FRigControlElement* pointers.
	// Evaluation (e.g. construction) can reallocate internal hierarchy storage, dangling those pointers.
	// Both CreateShapeActors and this function call GetControlsEligibleForShapes
	...
	for (int32 ShapeActorIndex = 0; ShapeActorIndex < NumShapes; ShapeActorIndex++)
	{
		...
		if (ShapeActor && ControlElement)
		{
			...
			if (const FControlRigShapeDefinition* ShapeDef = UControlRigShapeLibrary::GetShapeByName(ControlElement->Settings.ShapeName, ShapeLibraries, InControlRig->ShapeLibraryNameMap))
			{
				...
				{
					UPrimitiveComponent* Component = ShapeActor->GetProxyComponent(ShapeProxy);	
					if (Component && ShapeDef->PostSetupFunction)
					{
						ShapeDef->PostSetupFunction(ShapeActor);
					}
				}

				if (UPrimitiveComponent* ProxyComponent = ShapeActor->ProxyComponent.Get())
				{
			...
			else
			{
				if (UPrimitiveComponent* ProxyComponent = ShapeActor->ProxyComponent.Get())
				{
					ProxyComponent->SetRelativeTransform(ShapeTransform);
				}
			}
		}
	}
	...
	const FShapeUpdateParams Params(InControlRig, ComponentTransform, IsControlRigSkelMeshVisible(InControlRig), IsInLevelEditor(), !IsDisplayControlsEnabled());
	for (int32 ShapeActorIndex = 0; ShapeActorIndex < NumShapes; ShapeActorIndex++)
	{
		AControlRigShapeActor* ShapeActor = Shapes[ShapeActorIndex].Get();
		FRigControlElement* ControlElement = ControlPerShapeActor[ShapeActorIndex];
...
void FControlRigEditMode::CreateShapeActors(UControlRig* InControlRig)
{
	...
	TArray<const FControlRigShapeDefinition*> ShapeDefinitions;
	for (FRigControlElement* ControlElement : Controls)
	{
		...
		const FControlRigShapeDefinition* ShapeDef = UControlRigShapeLibrary::GetShapeByName(ControlSettings.ShapeName, ShapeLibraries, InControlRig->ShapeLibraryNameMap);
		if (ShapeDef)
		{
			Param.MeshTransform = ShapeDef->Transform;
			Param.Proxy = ShapeDef->GetShapeProxy();
			...
			Param.HoveredParameterName = ShapeDef->Library->MaterialHoveredParameter;
			Param.HoveredColorParameterName = ShapeDef->Library->MaterialHoveredColorParameter;
			if (Settings)
			{
				Param.HoveredColor = Settings->HoveredColor;
			}
			Param.PostSetupFunction = ShapeDef->PostSetupFunction;
		}
		...
		if (AActor* PreviewActor = Component ? Component->GetOwner() : nullptr)
		{
			for (int32 Index = 0; Index < ShapeActors->Num(); Index++)
			{
				if (AControlRigShapeActor* ShapeActor = ShapeActors->operator[](Index))
				{
					...
					// use the root component explicitly to avoid circular dependencies for actors where the default
					...
					if (const FControlRigShapeDefinition* ShapeDefinition = ShapeDefinitions[Index])
					{
						if (ShapeDefinition->PostSetupFunction)
						{
							ShapeDefinition->PostSetupFunction(ShapeActor);
						}
					}
				}
			}
...
bool IsControlSelectedAndTransformable(const URigHierarchy* InHierarchy, const FRigElementKey& InSelectedKey)
{
    ...
    return ControlElement->Settings.IsSelectable(bRespectVisibility);
}
...
void FControlRigEditMode::OnHierarchyModified(ERigHierarchyNotification InNotif, URigHierarchy* InHierarchy, const FRigNotificationSubject& InSubject)
{
	...
	// Recreate shape actors when the hierarchy is reset, even when the element is null.
	// This handles cases where another rig is compiled and somehow affects the shape actors of this rig.
	if (InNotif == ERigHierarchyNotification::HierarchyReset)
	{
		RequestToRecreateControlShapeActors();
	}
			...
			if (Key.Type == ERigElementType::Control)
			{
				if (const FRigControlElement* ControlElement = Cast<FRigControlElement>(InElement))
				{
					if (AControlRigShapeActor* ShapeActor = GetControlShapeFromControlName(ControlRig,Key.Name))
					{
						...
						const bool bHideManipulators = !IsDisplayControlsEnabled();
						if (ShapeActor->UpdateControlSettings(InNotif, ControlRig, ControlElement, bHideManipulators, !AreEditingControlRigDirectly()))
						{
							break;
						}
					}
		...
		case ERigHierarchyNotification::ControlDrivenListChanged:
		{
			if (!AreEditingControlRigDirectly())
			{
				...
				if (FRigControlElement* ControlElement = InHierarchy->Find<FRigControlElement>(InElement->GetKey()))
				{
					...
					const UAnimDetailsSelection* Selection = AnimDetailsProxyManager ? AnimDetailsProxyManager->GetAnimDetailsSelection() : nullptr;
...
void FControlRigEditMode::OnSettingsChanged(const UControlRigEditModeSettings* InSettings)
{
	...
	if(bShowControlsAsOverlay != IsControlsAsOverlayEnabled())
	{
		bShowControlsAsOverlay = IsControlsAsOverlayEnabled();
		for (TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RuntimeControlRigs)
		{
...
void FControlRigEditMode::OnSettingsPropertyChanged(UObject* InObject, FPropertyChangedEvent& PropertyChangedEvent)
{
	UControlRigEditModeSettings* Settings = Cast<UControlRigEditModeSettings>(InObject);
	if (!Settings)
	{
		return;
	}
	
	if (PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UControlRigEditModeSettings, HoveredColor))
	{
		if (RecreateControlShapesRequired == ERecreateControlRigShape::RecreateNone)
		{
			for (const auto&[ControlRig, ShapeActors]: ControlRigShapeActors)
			{
				if (ControlRig.IsValid())
				{
					for (AControlRigShapeActor* ShapeActor : ShapeActors)
					{
						if (ShapeActor)
						{
							ShapeActor->SetHoveredColor(Settings->HoveredColor);
						}
					}
				}
			}
	...
	TArray<TFunction<void()>>& OutTasks)
{
	...
	if (!ShapeActor->GetAttachParentActor())
	{
		if (USceneComponent* SceneComponent = GetHostingSceneComponent(ControlRig))
		{
			if (AActor* OwnerActor = SceneComponent->GetOwner())
			{
				...
				// use the root component explicitly to avoid circular dependencies for actors where the default
				// attachment component isn't the root
				USceneComponent* ParentComponent = OwnerActor->GetRootComponent();
				if (ParentComponent)
				{
					ShapeActor->AttachToComponent(ParentComponent, FAttachmentTransformRules::KeepWorldTransform);
				}				
	...
	if (ShapeActor->ProxyComponent.Get())
	{
	...
	{
		// If the shape proxy is not valid, we cannot rely on the shape's transform.
...
void FControlRigEditMode::ChangeControlShapeTransform(AControlRigShapeActor* InShapeActor, const FControlRigInteractionTransformContext& InContext, const FTransform& ToWorldTransform)
{
	...
	if (UPrimitiveComponent* ProxyComponent = InShapeActor->ProxyComponent.Get())
	{
		ProxyComponent->SetRelativeTransform(MeshTransform * ShapeTransform);
	}
}
...
{
	auto UpdateSkeletalMeshComponent = [](USkeletalMeshComponent* InSkeletalMeshComponent)
	{
		if (InSkeletalMeshComponent && !InSkeletalMeshComponent->IsPostEvaluatingAnimation() && InSkeletalMeshComponent->PoseTickedThisFrame())
		{
			...
			constexpr bool bDontNeedValidRootMotion = false;
			
			InSkeletalMeshComponent->TickAnimation(DeltaTime, bDontNeedValidRootMotion);
	...
	for (const TWeakObjectPtr<UControlRig>& RuntimeRigPtr : RigsToTick)
	{
		if (UControlRig* ControlRig = RuntimeRigPtr.Get())
		{
			const TGuardValue<bool> GuardEvaluationType(ControlRig->bEvaluationTriggeredFromInteraction, true);
			...
			else if (USkeletalMeshComponent* MeshComponent = Cast<USkeletalMeshComponent>(SceneComponent))
			{
				if(!ControlRig->ContainsSimulation())
				{
					...
					{
						UpdateSkeletalMeshComponent(SkeletalMeshToUpdate);
					}
				}
			}
		}
...
void FControlRigEditMode::OnWorldCleanup(UWorld* World, bool bSessionEnded, bool bCleanupResources)
{
	...
	if ((WorldPtr == World) || (Owner && Owner->GetWorld() == World))
	{
		DestroyShapesActors(nullptr);
	}
}
...
FTransform FControlRigEditMode::GetHostingSceneComponentTransform(const UControlRig* ControlRig) const
{
	...
	if(IsInLevelEditor())
	{
		...
		FTransform Result = HostingComponent ? HostingComponent->GetComponentTransform() : FTransform::Identity;

		// When root motion is extracted from a control rig layer and applied to the actor,
		// the control hierarchy's cached global transforms still include the extracted root
		// motion while the component transform also includes it (via the moved actor).
		// Subtract the root motion offset so gizmos are not drawn at a double offset.
		if (TSharedPtr<ISequencer> Sequencer = WeakSequencer.Pin())
		{
			const UMovieSceneEntitySystemLinker* EntityLinker =
				Sequencer->GetEvaluationTemplate().GetEntitySystemLinker();
			if (EntityLinker && HostingComponent)
			{
				TArray<IMovieSceneRootMotionOffsetProvider*> Providers =
					IModularFeatures::Get().GetModularFeatureImplementations<IMovieSceneRootMotionOffsetProvider>(
						IMovieSceneRootMotionOffsetProvider::GetModularFeatureName());
				for (IMovieSceneRootMotionOffsetProvider* Provider : Providers)
				{
					if (!Provider)
					{
						continue;
					}
					FTransform Offset = Provider->GetRootMotionOffset(EntityLinker, HostingComponent, ControlRig);
					if (!Offset.Equals(FTransform::Identity))
					{
						Result = Offset.Inverse() * Result;
						break;
					}
				}
			}
...
void FControlRigEditMode::PostPoseUpdate(const FEditorViewportClient* InViewportClient, const TArray<TWeakObjectPtr<UControlRig>>& InRigs) const
{
	...
	auto UpdateShapes = [this, bAreEditingControlRigDirectly](const TWeakObjectPtr<UControlRig> WeakControlRig, const TArray<TObjectPtr<AControlRigShapeActor>>& ShapeActors)
	{
		if(TStrongObjectPtr<UControlRig> ControlRig = WeakControlRig.Pin())
		{
			...
			FShapeUpdateParams Params(ControlRig.Get(), ComponentTransform, IsControlRigSkelMeshVisible(ControlRig.Get()), IsInLevelEditor(), !IsDisplayControlsEnabled());
			for (AControlRigShapeActor* ShapeActor : ShapeActors)
			{
...
bool FControlRigEditMode::IsViewportHitProxySelectable(HHitProxy* const InHitProxy) const
{
	if (!InHitProxy)
	{
		return false;
	...
	if (InHitProxy->IsA(HWidgetAxis::StaticGetType()) || InHitProxy->IsA(HFKRigBoneProxy::StaticGetType()))
	{
		return true;
	}

	if (const HActor* const ActorHitProxy = HitProxyCast<HActor>(InHitProxy))
	{
		return IsViewportObjectSelectable(ActorHitProxy->Actor);
	}

	return false;
}

bool FControlRigEditMode::IsViewportObjectSelectable(UObject* const InObject) const
{
	if (!IsInLevelEditor())
	{
		return true;
	...
	const UControlRigEditModeSettings* const Settings = GetSettings();
	if (!Settings || !Settings->bOnlySelectRigControls)
	{
		return true;
	}
	...
	return !Actor || Actor->IsA<AControlRigShapeActor>();
}
...
void FControlRigEditMode::SelectMirroredControls()
{
	...
	for (UControlRig* ControlRig : ControlRigs)
	{
		...
		{
			ControlRig->Modify();
			TempPose->SavePose(ControlRig, false, false/*bSaveValues*/);
...
void FControlRigEditMode::SelectAllControls()
{
	...
	for (UControlRig* ControlRig : ControlRigs)
	{
		...
		{
			ControlRig->Modify();
			TempPose->SavePose(ControlRig, true, false/*bSaveValues*/); //this will save the whole pose
...
bool FDetailKeyFrameCacheAndHandler::IsPropertyKeyable(const UClass* InObjectClass, const IPropertyHandle& InPropertyHandle) const
{
	...
	if (InObjectClass->IsChildOf(UAnimDetailsProxyEnum::StaticClass()) &&
		PropertyName == GET_MEMBER_NAME_CHECKED(UAnimDetailsProxyEnum, Enum))
	{
		return true;
	}
...
void FDetailKeyFrameCacheAndHandler::SetDelegates(TWeakPtr<ISequencer>& InWeakSequencer, FControlRigEditMode* InEditMode)
{
	...
	if (TSharedPtr<ISequencer> Sequencer = WeakSequencer.Pin())
	{
		...
		Sequencer->OnChannelChangedWithTime().AddRaw(this, &FDetailKeyFrameCacheAndHandler::OnChannelChangedWithTime);
...
void FDetailKeyFrameCacheAndHandler::UnsetDelegates()
{
	if (TSharedPtr<ISequencer> Sequencer = WeakSequencer.Pin())
	{
		...
		Sequencer->OnGlobalTimeChanged().RemoveAll(this);
		Sequencer->OnEndScrubbingEvent().RemoveAll(this);
		Sequencer->OnChannelChangedWithTime().RemoveAll(this);
		Sequencer->OnStopEvent().RemoveAll(this);
	}
}
...
void FDetailKeyFrameCacheAndHandler::OnChannelChangedWithTime(const TArray<FFrameNumber>&, const FMovieSceneChannelMetaData*, UMovieSceneSection*)
{
	ResetCachedData();
}
...
void FControlRigEditMode::BindDirectControl()
{
	DirectControl.Bind(Owner);
	
	auto GetSequencer = [this]()
	{
		return WeakSequencer.IsValid() ? WeakSequencer.Pin() : nullptr;
	};
	
	auto ShouldSelectControlsOnly = [this]()
	{
		return GetOnlySelectRigControls();
	};
	
	DirectControl.HitProxyFilterFunction = [GetSequencer, ShouldSelectControlsOnly](HHitProxy* HitProxy)
	{
		if (HitProxy)
		{
			const HActor* ActorHitProxy = HitProxyCast<HActor>(HitProxy);
			...
			if (ShouldSelectControlsOnly() && !ActorHitProxy->Actor->IsA<AControlRigShapeActor>())
			{
				return false;
			}
			...
			{
				using SequencerSelectionGuard = TGuardValue_Bitfield_Cleanup<TFunction<void()>>;
				const SequencerSelectionGuard NotificationGuard([Sequencer, bSelectability = Sequencer->IsViewportSelectionLimited()]()
				{
					Sequencer->SetViewportSelectionLimited(bSelectability);
				});
				Sequencer->SetViewportSelectionLimited(true);
				return Sequencer->IsObjectSelectableInViewport(ActorHitProxy->Actor);
			}
	...
	DirectControl.SequencerFunction = [this]()
	{
		return WeakSequencer.IsValid() ? WeakSequencer.Pin() : nullptr;
	};
}
