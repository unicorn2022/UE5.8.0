...
uint32 GetHitProxySize()
{
	// see FViewport constructor and FViewport::GetHitProxy(int32 X,int32 Y)
	static uint32 HitProxySize = 0;
	if (HitProxySize == 0)
	{
		HitProxySize = 5;
		if (GIsEditor && GConfig) 
		{
			int32 HitProxySizeFromConfig = 5;
			if (GConfig->GetInt( TEXT("UnrealEd.HitProxy"), TEXT("HitProxySize"), HitProxySizeFromConfig, GEditorIni ))
			{
				HitProxySize = static_cast<uint32>(FMath::Clamp(HitProxySizeFromConfig, 1, MAX_HITPROXYSIZE));
			}
		}
	}
	return HitProxySize;
...
TOptional<FIntRect> FSelectionHelper::RectangleFromFrustum(const FConvexVolume& InFrustum) const
{
	...
	FPlane NearPlane;
	if (!SceneView->ViewMatrices.GetWorldToClip().GetFrustumNearPlane(NearPlane))
	{
		return InvalidBox;
	}
	...
	if (Intersections.Num() == 4)
	{
		...
		{
			FVector2D& TopLeft = ScreenPos[3], BottomRight = ScreenPos[1];

			const int32 HitProxySize = Private::GetHitProxySize();
			const double dHitProxySize(HitProxySize);

			if (BottomRight.X - TopLeft.X < 2.0 * dHitProxySize)
			{
				const double MidX = 0.5 * (BottomRight.X + TopLeft.X);
				TopLeft.X = MidX - dHitProxySize;
				BottomRight.X = MidX + dHitProxySize;
			}

			if (BottomRight.Y - TopLeft.Y < 2.0 * dHitProxySize)
			{
				const double MidY = 0.5 * (BottomRight.Y + TopLeft.Y);
				TopLeft.Y = MidY - dHitProxySize;
				BottomRight.Y = MidY + dHitProxySize;
			}
			...
			const int32 MinXClamp = (MinX + HitProxySize) > (ViewportSize.X-1) ? MinX + 1 : MinX + HitProxySize;
			const int32 MinYClamp = (MinY + HitProxySize) > (ViewportSize.Y-1) ? MinY + 1 : MinY + HitProxySize;
			
			const int32 MaxX = FMath::Clamp(FMath::CeilToInt32(BottomRight.X), MinXClamp, ViewportSize.X);
			const int32 MaxY = FMath::Clamp(FMath::CeilToInt32(BottomRight.Y), MinYClamp, ViewportSize.Y);
