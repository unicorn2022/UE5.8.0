...
/** Specifies the selection policy applied when using click-and-drag interactions. */
UENUM(MinimalAPI, meta=(Experimental))
enum class EClickAndDragBehavior : uint8
{
	// click-and-drag interaction is disabled.
	Disabled = 0,
	// temporarily selects the hovered element during the interaction (if nothing is already selected).
	TransientSelection,
	// selects the hovered element and preserves the selection after the interaction ends.
	PersistentSelection
};
...
public:
	...
	/**
	 * Enables the tab menu customization so that users can register contextual utility actions.
	 */
	UPROPERTY(config, EditAnywhere, Category = "Animation Settings|Workflow")
	bool bSupportUtilityActions = true;
	
	/**
	 * Enables or disables click-and-drag selection and defines how it behaves.
	 */
	UPROPERTY(config, EditAnywhere, Category = "Animation Settings|Interaction", meta=(Experimental))
	EClickAndDragBehavior ClickAndDragBehavior = EClickAndDragBehavior::Disabled;
	
	/** Hovered color used when hovering a control shape. */
    UPROPERTY(config, EditAnywhere, Category = "Animation Settings", meta = (EditCondition = "!bHideControlShapes"))
    FLinearColor HoveredColor = FLinearColor(0.5f, 0.5f, 0.5f, 0.25f);
