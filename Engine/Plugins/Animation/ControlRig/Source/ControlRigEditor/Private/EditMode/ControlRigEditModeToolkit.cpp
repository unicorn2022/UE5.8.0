...
void FControlRigEditModeToolkit::BuildToolBar(FToolBarBuilder& InToolBarBuilder)
{
	...
	InToolBarBuilder.AddToolBarButton(
		FUIAction(
			...
			{
				const TWeakPtr<ISequencer>& WeakSequencer = UE::AnimationEditMode::GetSequencer();
				if (const TSharedPtr<ISequencer>& Sequencer = WeakSequencer.IsValid() ? WeakSequencer.Pin() : nullptr)
				{
					Sequencer->SetShowCurveEditor(!Sequencer->GetCurveEditorIsVisible());
				}
			}),
			...
			FIsActionChecked::CreateLambda([]()
			{
				...
				if (const TSharedPtr<ISequencer>& Sequencer = WeakSequencer.IsValid() ? WeakSequencer.Pin() : nullptr)
				{
					return Sequencer->GetCurveEditorIsVisible();
				}
				return false;
			})
		),
		NAME_None,
		LOCTEXT("CurveEditor", "Curve Editor"),
		LOCTEXT("CurveEditorTooltip", "Toggle the Curve Editor"),
		FSlateIcon(FAppStyle::GetAppStyleSetName(), TEXT("GenericCurveEditor")),
