...
float SControlRigPoseView::SplitterSlotValues[3] = {0.33f, 0.33f, 0.33f};
...
void SControlRigPoseView::Construct(const FArguments& InArgs)
{
	...
	[
		...
		+SVerticalBox::Slot()
			...
			.Padding(0, 0, 0, 4)
			[
			SAssignNew(PoseSplitter, SSplitter)
				.OnSplitterFinishedResizing(this, &SControlRigPoseView::OnSplitterFinishedResizing)

				+ SSplitter::Slot()
				.Value(SplitterSlotValues[0])
				[
					SNew(SBorder)
...
{	
	if (PoseAsset.IsValid())
	{
		...
		TArray<UControlRig*> ControlRigs = GetControlRigs(bIsSelected);
		if (ControlRigs.Num() == 0)
		{
			bIsSelected = false;
			ControlRigs = GetControlRigs(bIsSelected);
		}
		for (UControlRig* ControlRig : ControlRigs)
		{
			if (ControlRig)
...
TArray<UControlRig*> SControlRigPoseView::GetControlRigs(bool bIsSelected)
{
	...
	if (bIsSelected)
	{
		...
		for (TWeakObjectPtr<UControlRig> ControlRigPtr : CurrentControlRigs)
		{
			if (ControlRigPtr.IsValid() && ControlRigPtr->CurrentControlSelection().Num() > 0)
			{
				SelectedControlRigs.Add(ControlRigPtr.Get());
			}
		}
		if (SelectedControlRigs.Num() > 0)
		{
...
void SControlRigPoseView::OnSplitterFinishedResizing()
{
	if (PoseSplitter.IsValid())
	{
		for (int32 SlotIndex = 0; SlotIndex < 3; ++SlotIndex)
		{
			SplitterSlotValues[SlotIndex] = PoseSplitter->SlotAt(SlotIndex).GetSizeValue();
		}
	}
}
