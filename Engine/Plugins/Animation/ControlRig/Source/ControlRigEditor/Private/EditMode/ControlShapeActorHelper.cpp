...
FShapeUpdateParams::FShapeUpdateParams(
	const UControlRig* InControlRig, 
	const FTransform& InComponentTransform, 
	const bool InSkeletalMeshVisible, 
	const bool IsInLevelEditor, 
	const bool bInHideControlShapes)
	...
	, bHideControlShapes(bInHideControlShapes)
{
	if (IsValid())
	{
		bControlsHiddenInViewport = bHideControlShapes || !ControlRig->GetControlsVisible() || !bIsSkeletalMeshVisible;
	}
}
...
void UpdateControlShape(AControlRigShapeActor* InShapeActor, FRigControlElement* InControlElement, const FShapeUpdateParams& InUpdateParams)
{
	...
	const bool bControlsHiddenInViewport =
		InUpdateParams.bHideControlShapes ||
