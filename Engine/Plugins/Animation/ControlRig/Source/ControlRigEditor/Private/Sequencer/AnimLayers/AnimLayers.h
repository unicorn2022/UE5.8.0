...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "State")
	void SelectWeightChannel(bool bClearSelection);

	int32 GetAnimLayerIndex() const { return LayerIndex; }
	...
	/** Sets sequencer selection for this layer's currently selected controls */
	void ApplyControlSelectionToSequencer(bool bSelect = true) const;
	
	/** Returns true if the layer has sections that match the current editor selection */
	bool HasSectionsInCurrentEditorSelection() const;
...
public:
	...
	bool ReorderAnimLayers(ISequencer* InSequencer, int32 OldIndex, int32 NewIndex);
...
private:
	...
	//Set the anim layer index values, if bResetSectionOrders is true we will also go through and make sure the sections are
	//also in order
	void SetAnimLayerIndices(bool bResetSectionOrders);
	//get set of tracks and their sections for each anim layer
	void GetTracksAndSections(TMap<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>>& OutTracksAndOrderedSections);
	//setup track information (name, mute state etc), for sections on the track per anim layer
	void SetTrackRowForSections(const TMap<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>>& TracksAndOrderedSections);
