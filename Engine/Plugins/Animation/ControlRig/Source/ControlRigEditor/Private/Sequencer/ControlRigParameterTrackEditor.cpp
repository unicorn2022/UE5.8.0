...
TArray <TPair<FControlRigAssetStrongReference, TArray<FName>>> FControlRigParameterTrackEditor::PreviousSelectedControlRigs;
...
CVarSelectedKeysSelectControls(TEXT("ControlRig.Sequencer.SelectedKeysSelectControls"), false, TEXT("When true when we select a key in Sequencer it will select the Control, by default false."));
...
TAutoConsoleVariable<bool> CVarSelectChildChannels(TEXT("ControlRig.Sequencer.SelectChildChannels"), true, TEXT("When true selecting a control will search for child channels and constraint controls and select them also, default to true."));
...
void ClearTimer(FTimerHandle& InOutHandle)
{
	if (ensure(GEditor))
	{
		...
		{
			GEditor->GetTimerManager()->ClearTimer(InOutHandle);
		}
	}
	else
	{
...
void DeferToNextTick(FTimerHandle& InOutHandle, TFunction<void(void)>&& Callback)
{
	ClearTimer(InOutHandle);
	if (ensure(GEditor))
	{
		InOutHandle = GEditor->GetTimerManager()->SetTimerForNextTick(MoveTemp(Callback));
	}
}
	
void DeferToNextTick(TFunction<void(void)>&& Callback)
{
	if (ensure(GEditor))
	{
		GEditor->GetTimerManager()->SetTimerForNextTick(MoveTemp(Callback));
	}
}
...
FControlRigParameterTrackEditor* FControlRigParameterTrackEditor::CurrentEditor = nullptr;
...
void FControlRigParameterTrackEditor::OnInitialize()
{
	...
	FControlRigParameterTrackEditor::CurrentEditor = this;
	...
	OnChannelChangedHandle = Sequencer->OnChannelChangedWithTime().AddRaw(this, &FControlRigParameterTrackEditor::OnChannelChanged);
	...
	if (MovieScene)
	{
		OnMovieSceneChannelChangedHandle = MovieScene->OnChannelChangedWithTime().AddRaw(this, &FControlRigParameterTrackEditor::OnChannelChanged);
	}
...

	{
		...
		const FDelegateHandle OnObjectsReplacedHandle = FCoreUObjectDelegates::OnObjectsReplaced.AddLambda([this](const TMap<UObject*, UObject*>& ReplacementMap)
		{
			...
			if (OldToNewControlRigs.Num() > 0)
			{
				...
				IterateTracks([this, &OldToNewControlRigs, ControlRigEditMode, &bRequestEvaluate](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
				{
					if (UControlRig* OldControlRig = Track->GetControlRig())
					{
						...
						if (FoundNewControlRig)
						{
							...
							{
								Track->Modify();
								Track->ReplaceControlRig(NewControlRig, OldControlRig->GetAssetReference() != NewControlRig->GetAssetReference());
								BindControlRig(NewControlRig,InGuid);
							...
							if (ControlRigEditMode)
							{
								...
								auto UpdateSelectionDelegate = [this, SelectedControls, NewControlRig, bRequestEvaluate]()
								{
									...
									if (!(FSlateApplication::Get().HasAnyMouseCaptor() || GUnrealEd->IsUserInteracting()))
									{
										...
										if (NewControlRig)
										{
											DeferToNextTick([WeakRig = TWeakObjectPtr<UControlRig>(NewControlRig), SelectedControls]()
											{
												if (UControlRig* Rig = WeakRig.Get())
												{
													Rig->ClearControlSelection();
													for (const FName& ControlName : SelectedControls)
													{
														Rig->SelectControl(ControlName, true);
													}
												}
											});
										}
										...
										ClearTimer(UpdateSelectionTimerHandle);
									}
								};
	...
	IterateTracks([this](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
	{
		...
		if (UControlRig* ControlRig = Track->GetControlRig())
		{
			...
			BindControlRig(ControlRig, InGuid);
		}
...
FControlRigParameterTrackEditor::~FControlRigParameterTrackEditor()
{
	...
	FControlRigParameterTrackEditor::CurrentEditor = nullptr;

	FMovieSceneToolsModule::Get().UnregisterAnimationBakeHelper(this);

	UE::ControlRigEditor::Private::ClearTimer(DeactivateModeHandle);
	UE::ControlRigEditor::Private::ClearTimer(ActivateModeHandle);
}


void FControlRigParameterTrackEditor::BindControlRig(UControlRig* ControlRig, FGuid InGuid)
{
	if (ControlRig)
	{
		if (!BoundControlRigs.Contains(ControlRig))
		{
			...
			BoundControlRigs.Add(ControlRig, InGuid);
		}
...
void FControlRigParameterTrackEditor::UnbindAllControlRigs()
{
	...
	TMap<TWeakObjectPtr<UControlRig>,FGuid> ControlRigs = BoundControlRigs;
	for (TPair<TWeakObjectPtr<UControlRig>, FGuid>& ObjectPtr : ControlRigs)
	{
		if (ObjectPtr.Key.IsValid())
		{
			UControlRig* ControlRig = ObjectPtr.Key.Get();
			UnbindControlRig(ControlRig);
		}
	}
	BoundControlRigs.Reset();
}
...
void FControlRigParameterTrackEditor::ObjectImplicitlyAdded(UObject* InObject)
{
	...
	if (ControlRig)
	{
		IterateTracks([this, ControlRig](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
			{
...
void FControlRigParameterTrackEditor::OnRelease()
{
	...
	if (ControlRigEditMode)
	{
		...
		for (TPair<TWeakObjectPtr<UControlRig>,FGuid>& ControlRig : BoundControlRigs)
		{
			if (ControlRig.Key.IsValid())
			{
				TPair<FControlRigAssetStrongReference, TArray<FName>> ClassAndName;
				ClassAndName.Key = ControlRig.Key->GetAssetReference();
				ClassAndName.Value = ControlRig.Key->CurrentControlSelection();
	...
	if (Sequencer)
	{
		...
		{
			Sequencer->OnChannelChangedWithTime().Remove(OnChannelChangedHandle);
		...
		if (OnMovieSceneChannelChangedHandle.IsValid())
		{
			...
			{
				MovieScene->OnChannelChangedWithTime().Remove(OnMovieSceneChannelChangedHandle);
			}
...
const FSlateBrush* FControlRigParameterTrackEditor::GetIconBrush() const
{
	static const FName StyleSetName(TEXT("ControlRigEditorStyle"));
	const ISlateStyle* Style = FSlateStyleRegistry::FindSlateStyle(StyleSetName);
	return Style ? Style->GetOptionalBrush(TEXT("ClassIcon.ControlRigBlueprint")) : nullptr;
}
...
TSharedPtr<SWidget> FControlRigParameterTrackEditor::BuildOutlinerEditWidget(const FGuid& ObjectBinding, UMovieSceneTrack* Track, const FBuildEditWidgetParams& Params)
{
	TWeakPtr<ISequencer> WeakSequencer = GetSequencer();

	const int32 RowIndex = Params.TrackInsertRowIndex;
	UE::Sequencer::TWeakViewModelPtr<UE::Sequencer::FViewModel> WeakViewModel = Params.ViewModel.AsWeak();
...
void FControlRigParameterTrackEditor::BuildObjectBindingContextMenu(FMenuBuilder& MenuBuilder, const TArray<FGuid>& ObjectBindings, const UClass* ObjectClass)
{
		...
		ObjectClass->IsChildOf(UChildActorComponent::StaticClass()))
	{
		...
		{
			const TSharedPtr<ISequencer> SequencerForBake = Sequencer;
			const FGuid BakeObjectBinding = ObjectBindings[0];

			MenuBuilder.BeginSection("Control Rig", LOCTEXT("ControlRig", "Control Rig"));
			{
				UE::ControlRig::BuildBakeToControlRigMenu(
					MenuBuilder, Skeleton, bFilterAssetBySkeleton,
					// PopulateDelegate: export from skel mesh via sequencer evaluation
					UE::ControlRig::FPopulateAnimSequenceDelegate::CreateLambda(
						[SequencerForBake](UAnimSequence* AnimSeq, UAnimSeqExportOption* ExportOpts, USkeletalMeshComponent* Mesh) -> bool
						{
							UMovieSceneSequence* OwnerSeq = SequencerForBake->GetFocusedMovieSceneSequence();
							UMovieSceneSequence* RootSeq = SequencerForBake->GetRootMovieSceneSequence();
							FMovieSceneSequenceTransform RootToLocal = SequencerForBake->GetFocusedMovieSceneSequenceTransformUnwarped();
							FAnimExportSequenceParameters AESP;
							AESP.Player = SequencerForBake.Get();
							AESP.RootToLocalTransform = RootToLocal;
							AESP.MovieSceneSequence = OwnerSeq;
							AESP.RootMovieSceneSequence = RootSeq;
							AESP.bForceUseOfMovieScenePlaybackRange = SequencerForBake->GetSequencerSettings()->ShouldEvaluateSubSequencesInIsolation();
							return MovieSceneToolHelpers::ExportToAnimSequence(AnimSeq, ExportOpts, AESP, Mesh);
						}),
					SequencerForBake, BakeObjectBinding, BoundObject, SkelMeshComp,
					// TrackFactory: create standalone track on the binding
					[this, BakeObjectBinding](UMovieScene* MovieScene) -> UMovieSceneControlRigParameterTrack*
					{
						// Remove existing CR tracks
						TArray<UMovieSceneTrack*> ExistingTracks = MovieScene->FindTracks(UMovieSceneControlRigParameterTrack::StaticClass(), BakeObjectBinding, NAME_None);
						for (UMovieSceneTrack* Existing : ExistingTracks)
						{
							MovieScene->RemoveTrack(*Existing);
						}
						return Cast<UMovieSceneControlRigParameterTrack>(AddTrack(MovieScene, BakeObjectBinding, UMovieSceneControlRigParameterTrack::StaticClass(), NAME_None));
					},
					[this, BakeObjectBinding, WeakSkelMeshComp = TWeakObjectPtr<USkeletalMeshComponent>(SkelMeshComp)]
					(const UE::ControlRig::FBakeToControlRigResult& Result)
					{
						const TSharedPtr<ISequencer> Seq = GetSequencer();
						if (!Seq) { return; }
						UMovieScene* MovieScene = Seq->GetFocusedMovieSceneSequence() ? Seq->GetFocusedMovieSceneSequence()->GetMovieScene() : nullptr;
						if (!MovieScene) { return; }

						TArray<UMovieSceneTrack*> TracksToDisable;
						auto CollectSkelAnimAndMixerTracks = [&TracksToDisable, MovieScene](const FGuid& InBindingGuid)
						{
							const FMovieSceneBinding* Binding = MovieScene->FindBinding(InBindingGuid);
							if (!Binding) { return; }
							for (UMovieSceneTrack* Track : Binding->GetTracks())
							{
								if (Cast<UMovieSceneSkeletalAnimationTrack>(Track))
								{
									TracksToDisable.Add(Track);
								}
								else if (Track && Track->GetClass()->GetName() == TEXT("MovieSceneAnimationMixerTrack"))
								{
									TracksToDisable.Add(Track);
								}
							}
						};
						CollectSkelAnimAndMixerTracks(BakeObjectBinding);
						if (USkeletalMeshComponent* SkelMeshComp = WeakSkelMeshComp.Get())
						{
							CollectSkelAnimAndMixerTracks(Seq->FindObjectId(*SkelMeshComp, Seq->GetFocusedTemplateID()));
						}

						for (UMovieSceneTrack* Track : TracksToDisable)
						{
							MovieSceneHelpers::DisableTrack(Track);
						}
						...
						if (ControlRigEditMode) { ControlRigEditMode->AddControlRigObject(Result.ControlRig, Seq); }
						BindControlRig(Result.ControlRig, BakeObjectBinding);

						if (Result.Section)
						{
							Seq->EmptySelection();
							Seq->SelectSection(Result.Section);
							Seq->ThrobSectionSelection();
						}
...
static bool AssetReferenceSortPredicate(const TSharedRef<FControlRigAssetSoftReference> A, const  TSharedRef<FControlRigAssetSoftReference> B)
{
	...
	{
		return A->Get() == UFKControlRig::StaticClass();
	}
}
...
void FControlRigParameterTrackEditor::BakeToControlRigSubMenu(FMenuBuilder& MenuBuilder, FGuid ObjectBinding, UObject* BoundObject, USkeletalMeshComponent* SkelMeshComp, USkeleton* Skeleton)
{
	if (Skeleton)
	{
		...
		MenuBuilder.AddWidget(SNew(SControlRigAssetReferencePicker)
			.OnSelectionChanged_Lambda([this, ObjectBinding, BoundObject, SkelMeshComp, Skeleton](FControlRigAssetSoftReference InReference)
				{
					BakeToControlRig(InReference.LoadStrongReference(), ObjectBinding, BoundObject, SkelMeshComp, Skeleton);
				})
			.Filter(ClassFilter)
			.ExtraAssets(ExtraAssets)
			.SortPredicate(AssetReferenceSortPredicate)
		,FText::GetEmpty());
	}
}
...
void FControlRigParameterTrackEditor::BakeToControlRig(const FControlRigAssetStrongReference& InAssetReference, FGuid ObjectBinding, UObject* BoundActor, USkeletalMeshComponent* SkelMeshComp, USkeleton* Skeleton)
{
	FSlateApplication::Get().DismissAllMenus();
	
	const TSharedPtr<ISequencer> Sequencer = GetSequencer();

	if (InAssetReference.IsValid() && Sequencer)
	{
		UMovieSceneSequence* OwnerSequence = Sequencer->GetFocusedMovieSceneSequence();
		...
		
		{
			...
			if (OptionWindow.Get()->ShouldExport())
			{
				...
				// Remove existing control rig tracks on this binding
				TArray<UMovieSceneTrack*> ExistingCRTracks = OwnerMovieScene->FindTracks(UMovieSceneControlRigParameterTrack::StaticClass(), ObjectBinding, NAME_None);
				for (UMovieSceneTrack* AnyOleTrack : ExistingCRTracks)
				{
					if (UMovieSceneControlRigParameterTrack* CRTrack = Cast<UMovieSceneControlRigParameterTrack>(AnyOleTrack))
					{
						OwnerMovieScene->RemoveTrack(*CRTrack);
					}
				}

				// Create the track and initialize the control rig via the shared helper
				...
				if (Track)
				{
					UE::ControlRig::FBakeToControlRigResult Result = UE::ControlRig::InitControlRigFromAnimSequence(
						Sequencer, OwnerMovieScene, Track, InAssetReference,
					...
					if (Result.bSuccess)
					{
						...
						{
							SkelTrack->Modify();
							MovieSceneHelpers::DisableTrack(SkelTrack);
						}

						// Finish setup: edit mode + delegate binding
						FControlRigEditMode* ControlRigEditMode = GetEditMode();
						if (!ControlRigEditMode)
						{
							ControlRigEditMode = GetEditMode(true);
						...
						if (ControlRigEditMode)
						{
							ControlRigEditMode->AddControlRigObject(Result.ControlRig, Sequencer);
						}
						BindControlRig(Result.ControlRig, ObjectBinding);
...
static void IterateTracksInMovieScene(const UMovieScene& InMovieScene, TFunctionRef<bool(UMovieSceneControlRigParameterTrack*, FGuid)> InCallback)
{
	...
	for (const FMovieSceneBinding& Binding : Bindings)
	{
		...
		for(UMovieSceneTrack* Track : FoundTracks)
		{
			...
			{
				InCallback(CRTrack, Binding.GetObjectGuid());
			}
		}

		// Also check for Control Rig child tracks within animation tracks (e.g., Animation Mixer)
		TArray<UMovieSceneTrack*> AllTracks = InMovieScene.FindTracks(UMovieSceneCommonAnimationTrack::StaticClass(), Binding.GetObjectGuid(), NAME_None);
		for (UMovieSceneTrack* Track : AllTracks)
		{
			if (UMovieSceneCommonAnimationTrack* AnimTrack = Cast<UMovieSceneCommonAnimationTrack>(Track))
			{
				TArray<TObjectPtr<UMovieSceneTrack>> ChildTracks;
				AnimTrack->GetAllChildTracks(ChildTracks);
				for (UMovieSceneTrack* ChildTrack : ChildTracks)
				{
					if (UMovieSceneControlRigParameterTrack* CRTrack = Cast<UMovieSceneControlRigParameterTrack>(ChildTrack))
					{
						InCallback(CRTrack, Binding.GetObjectGuid());
					}
				}
	...
	for (UMovieSceneTrack* Track : InMovieScene.GetTracks())
	{
		...
		{
			FGuid Empty;
			InCallback(CRTrack,Empty);
		}
	}
}

void FControlRigParameterTrackEditor::IterateTracks(TFunctionRef<bool(UMovieSceneControlRigParameterTrack*, FGuid)> Callback) const
{
	const TSharedPtr<ISequencer> Sequencer = GetSequencer();
	UMovieSceneSequence* OwnerSequence = Sequencer ? Sequencer->GetFocusedMovieSceneSequence() : nullptr;
	UMovieScene* MovieScene = OwnerSequence ? OwnerSequence->GetMovieScene() : nullptr;
...
void FControlRigParameterTrackEditor::BakeInvertedPose(UControlRig* InControlRig, UMovieSceneControlRigParameterTrack* Track)
{
	...
	{
		UE_LOGF(LogMovieScene, Error, "FControlRigParameterTrackEditor::BakeInvertedPose All parameters must be valid.");
		return;
	}
	...
	UMovieSceneControlRigParameterTrack* InTrack)
{
	...
	SettingsForNewControlRig = InTrack->GetCurrentRigSettings();
	if (!SettingsForNewControlRig.IsValid())
	{
		return;
	}
...
void FControlRigParameterTrackEditor::FRecreateRigOperator::RefreshSettingsFromControlRig()
{
	...
	WeakTrack.Get()->SyncRigSettingsToCurrentRig(SettingsForNewControlRig);
	
	UControlRig* ControlRig = WeakTrack.Get()->GetControlRig();


	for (const FPropertyBagPropertyDesc& PropertyDesc : SettingsForNewControlRig.GetPropertyBagStruct()->GetPropertyDescs())
	{
		FRigVMExternalVariable Variable = ControlRig->GetPublicVariableByName(PropertyDesc.Name);
		if (Variable.IsValid())
		{
			const uint8* SourceAddress  = Variable.GetProperty()->ContainerPtrToValuePtr<uint8>(ControlRig->GetVariablesMemory());
			uint8* TargetAddress = SettingsForNewControlRig.GetMutableValue().GetMemory() + PropertyDesc.CachedProperty->GetOffset_ForInternal();
			FRigVMMemoryStorageStruct::CopyProperty(
...
{
	// Control Rig menu entries are now added via PopulateObjectBindingAnimationMenu,
	// which is called by FCommonAnimationTrackEditor::BuildObjectBindingTrackMenu
}

void FControlRigParameterTrackEditor::PopulateObjectBindingAnimationMenu(FMenuBuilder& MenuBuilder, const TArray<FGuid>& ObjectBindings, const UClass* ObjectClass, bool bIsInsideSubmenu)
{
	if(!ObjectClass)
	{
		return;
		...
		ObjectClass->IsChildOf(UChildActorComponent::StaticClass()))
	{
		...
		static const FName ControlRigStyleName(TEXT("ControlRigEditorStyle"));
		const FSlateIcon ControlRigIcon(ControlRigStyleName, "ClassIcon.ControlRigBlueprint");
		...
		if (Skeleton)
		{
			...
			TFunction<UMovieSceneControlRigParameterTrack* (FGuid)> InTrackCreationCallback = nullptr;
			MenuBuilder.AddSubMenu(
				LOCTEXT("ControlRigText", "Control Rig"),
				FText(),
				FNewMenuDelegate::CreateSP(this, &FControlRigParameterTrackEditor::HandleAddTrackSubMenu, ObjectBindings, Track, InTrackCreationCallback),
...
void FControlRigParameterTrackEditor::HandleAddTrackSubMenu(FMenuBuilder& MenuBuilder, TArray<FGuid> ObjectBindings, UMovieSceneTrack* Track, TFunction<UMovieSceneControlRigParameterTrack* (FGuid)> InTrackCreationCallback)
{
	...
	if (Skeleton)
	{
		TArray<FControlRigAssetStrongReference> ExistingRigs;
		USkeletalMeshComponent* SkeletalMeshComponent = AcquireSkeletalMeshFromObject(BoundObject, ParentSequencer);
		IterateTracks([&ExistingRigs, SkeletalMeshComponent](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid) -> bool
		{
			if (UControlRig* ControlRig = Track->GetControlRig())
			{
				if (TSharedPtr<IControlRigObjectBinding> ObjectBinding = ControlRig->GetObjectBinding())
				{
					...
					{
						ExistingRigs.Add(ControlRig->GetAssetReference());
					}
				}
			}
			return true;
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &FControlRigParameterTrackEditor::AddFKControlRig, BoundObject, ObjectBindings[0], InTrackCreationCallback),
		...
		FAssetPickerConfig AssetPickerConfig;
		{
			...
			AssetPickerConfig.OnAssetSelected = FOnAssetSelected::CreateRaw(this, &FControlRigParameterTrackEditor::AddControlRig, BoundObject, ObjectBindings[0], InTrackCreationCallback);
			AssetPickerConfig.OnAssetEnterPressed = FOnAssetEnterPressed::CreateRaw(this, &FControlRigParameterTrackEditor::AddControlRig, BoundObject, ObjectBindings[0], InTrackCreationCallback);
...
const UClass* FControlRigParameterTrackEditor::GetHandledMixerItemClass() const
{
	return UMovieSceneControlRigParameterTrack::StaticClass();
}

void FControlRigParameterTrackEditor::PopulateAddMixerItemMenu(FMenuBuilder& MenuBuilder, TArray<FGuid> ObjectBindings, UMovieSceneTrack* Track, TSharedPtr<ISequencer> Sequencer, int32 RowIndex)
{
	UMovieSceneCommonAnimationTrack* CommonTrack = Cast<UMovieSceneCommonAnimationTrack>(Track);
	if (!CommonTrack)
	{
		return;
	}

	// Create track creation callback that adds the track as a child of the mixer track
	auto TrackCreationCallback = [CommonTrack, RowIndex](FGuid ObjectBinding) -> UMovieSceneControlRigParameterTrack*
	{
		// Use the provided row index if valid, otherwise compute next available
		int32 NewRowIndex = (RowIndex != INDEX_NONE) ? RowIndex : CommonTrack->ComputeNextAvailableRowIndex();
		return Cast<UMovieSceneControlRigParameterTrack>(CommonTrack->AddChildTrack(ObjectBinding, UMovieSceneControlRigParameterTrack::StaticClass(), NewRowIndex));
	};

	// Wrap the control rig menu in a submenu with mixer-specific track creation
	static const FName ControlRigStyleName(TEXT("ControlRigEditorStyle"));
	MenuBuilder.AddSubMenu(
		LOCTEXT("AddControlRig", "Control Rig"),
		LOCTEXT("AddControlRigTooltip", "Adds a control rig track with FK Control Rig option and asset picker"),
		FNewMenuDelegate::CreateLambda([this, ObjectBindings, Track, TrackCreationCallback](FMenuBuilder& SubMenuBuilder)
		{
			HandleAddTrackSubMenu(SubMenuBuilder, ObjectBindings, Track, TrackCreationCallback);
		}),
		/*bInOpenSubMenuOnClick=*/ false,
		FSlateIcon(ControlRigStyleName, "ClassIcon.ControlRigBlueprint")
	);
}

bool FControlRigParameterTrackEditor::IsControlRigAllowed(const FAssetData& AssetData, TArray<FControlRigAssetStrongReference> ExistingRigs, USkeleton* Skeleton)
{
	...
	if (FControlRigAssetInterfacePtr LoadedControlRig = AssetData.FastGetAsset())	
	{
		if (ExistingRigs.Contains(LoadedControlRig->GetControlRigAssetReference()))
		{
			if (!AssetData.GetTagValueRef<bool>(GET_MEMBER_NAME_CHECKED(UControlRigBlueprint, bAllowMultipleInstances)))
			{
				return false;
	...
	else if (TScriptInterface<IControlRigRuntimeAssetInterface> LoadedRuntimeAsset = AssetData.FastGetAsset())
	{
		if (ExistingRigs.Contains(LoadedRuntimeAsset.GetObject()))
		{
...
static TArray<UMovieSceneControlRigParameterTrack*> GetExistingControlRigTracksForSkeletalMeshComponent(UMovieScene* MovieScene, USkeletalMeshComponent* SkeletalMeshComponent)
{
	...
	IterateTracksInMovieScene(*MovieScene, [&ExistingControlRigTracks, SkeletalMeshComponent](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid) -> bool
	{
		if (UControlRig* ControlRig = Track->GetControlRig())
		{
			if (TSharedPtr<IControlRigObjectBinding> ObjectBinding = ControlRig->GetObjectBinding())
...
static UMovieSceneControlRigParameterTrack* AddControlRig(TSharedPtr<ISequencer> SharedSequencer , UMovieSceneSequence* Sequence, const FControlRigAssetStrongReference InSource, FGuid ObjectBinding, UControlRig* InExistingControlRig, bool bIsAdditiveControlRig, TFunction<UMovieSceneControlRigParameterTrack* (FGuid)> InTrackCreationCallback)
{
	FSlateApplication::Get().DismissAllMenus();
	if (!InSource.IsValid() || 
	...
	if (bIsAdditiveControlRig && InSource.GetRigClass() != UFKControlRig::StaticClass() && !InSource.GetSupportedEvents().Contains(FRigUnit_InverseExecution::EventName))
	{
		UE_LOGF(LogControlRigEditor, Error, "Cannot add an additive control rig which does not contain a backwards solve event.");
		return nullptr;
	}
	...
	UMovieSceneControlRigParameterTrack* Track = InTrackCreationCallback ? InTrackCreationCallback(ObjectBinding) : Cast<UMovieSceneControlRigParameterTrack>(OwnerMovieScene->AddTrack(UMovieSceneControlRigParameterTrack::StaticClass(), ObjectBinding));
	if (Track)
	{
		...
		FString ObjectName = InSource.GetName(); //GetDisplayNameText().ToString();
		...
		{
			ControlRig = InSource.CreateInstance(Track, FName(*ObjectName), RF_Transactional);
...
static UMovieSceneTrack* FindOrCreateControlRigTrack(TSharedPtr<ISequencer>& Sequencer, UWorld* World, const FControlRigAssetStrongReference ControlRigAssetReference, const FMovieSceneBindingProxy& InBinding, bool bIsLayeredControlRig, TFunction<UMovieSceneControlRigParameterTrack*(FGuid)> InTrackCreationCallback)
{
	...
	if (MovieScene && InBinding.BindingID.IsValid())
	{
		...
		{
			if (!DoesControlRigAllowMultipleInstances(FTopLevelAssetPath(ControlRigAssetReference.GetPathName())))
			{
				...
				for (UMovieSceneTrack* AnyOleTrack : Tracks)
				{
					...
					if (Track && Track->GetControlRig() && ControlRigAssetReference.IsSourceOf(Track->GetControlRig()))
					{
						return Track;
					}
				}
			...
			UMovieSceneControlRigParameterTrack* Track = AddControlRig(Sequencer, InBinding.Sequence, ControlRigAssetReference, InBinding.BindingID, nullptr, bIsLayeredControlRig, InTrackCreationCallback);
...
void FControlRigParameterTrackEditor::AddControlRig(const FControlRigAssetStrongReference InSource, UObject* BoundActor, FGuid ObjectBinding, UControlRig* InExistingControlRig, TFunction<UMovieSceneControlRigParameterTrack*(FGuid)> InTrackCreationCallback)
{
	TSharedPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer)
	{
	...
	if (UMovieSceneTrack* Track = FindOrCreateControlRigTrack(Sequencer, World, InSource, BindingProxy, bIsLayeredControlRig, InTrackCreationCallback))
	{
		BindControlRig(CastChecked<UMovieSceneControlRigParameterTrack>(Track)->GetControlRig(), ObjectBinding);
	}
}

void FControlRigParameterTrackEditor::AddControlRig(const FAssetData& InAsset, UObject* BoundActor, FGuid ObjectBinding, TFunction<UMovieSceneControlRigParameterTrack*(FGuid)> InTrackCreationCallback)
{
	FControlRigAssetStrongReference RigSource = InAsset.GetAsset();
	if (!RigSource.IsValid())
	{
		if (FControlRigAssetInterfacePtr EditorAsset = InAsset.GetAsset())
		{
			RigSource = EditorAsset->GetRuntimeAssetInterface().GetObject();
		}
	}
	if (RigSource.IsValid())
	{
		AddControlRig(RigSource, BoundActor, ObjectBinding, nullptr, InTrackCreationCallback);
	}
}

void FControlRigParameterTrackEditor::AddControlRig(const TArray<FAssetData>& InAssets, UObject* BoundActor, FGuid ObjectBinding, TFunction<UMovieSceneControlRigParameterTrack*(FGuid)> InTrackCreationCallback)
{
	if (InAssets.Num() > 0)
	{
		AddControlRig(InAssets[0], BoundActor, ObjectBinding, InTrackCreationCallback);
	}
}

void FControlRigParameterTrackEditor::AddFKControlRig(UObject* BoundActor, FGuid ObjectBinding, TFunction<UMovieSceneControlRigParameterTrack*(FGuid)> InTrackCreationCallback)
{
	AddControlRig(UFKControlRig::StaticClass(), BoundActor, ObjectBinding, nullptr, InTrackCreationCallback);
}

void FControlRigParameterTrackEditor::AddControlRig(FControlRigAssetStrongReference InSource, UObject* BoundActor, FGuid ObjectBinding)
{
	if (InSource.Get() == UFKControlRig::StaticClass())
	{
		AcquireSkeletonFromObjectGuid(ObjectBinding, &BoundActor, GetSequencer());
	}
	AddControlRig(InSource, BoundActor, ObjectBinding, nullptr);
}
...
void FControlRigParameterTrackEditor::SetKeysOnControls(const TMap<UControlRig*, TArray<FRigElementKey>>& SelectedControls, EMovieSceneTransformChannel Channel)
{
	...
	for (const TPair<UControlRig*, TArray<FRigElementKey>>& Selection : SelectedControls)
	{
		...
		const FName ControlRigName(*ControlRig->GetName());

		// Cache FRigControlElement* for each selected control to avoid repeated FindControl lookups.
		TArray<FControlRigKeyAddParams> Params;
		Params.Reserve(Selection.Value.Num());
		for (const FRigElementKey& ControlKey : Selection.Value)
		{
			if (FRigControlElement* ControlElement = ControlRig->FindControl(ControlKey.Name))
			{
				FControlRigKeyAddParams& Param = Params.AddDefaulted_GetRef();
				Param.RigControlName = ControlKey.Name;
		...
		// Single call: resolves handle/track once, Modify()s each section once, keys all controls.
		AddControlKeys(Object, ControlRig, ControlRigName, Params,
			ChannelsToKey, ESequencerKeyMode::ManualKeyForced, FLT_MAX, bInConstraintSpace);

		for (const FControlRigKeyAddParams& Param : Params)
		{
			FControlRigEditMode::NotifyDrivenControls(ControlRig, Param.ControlElement->GetKey(), NotifyDrivenContext);
		}
	}
}
...
void FControlRigParameterTrackEditor::OnAddTransformKeysForSelectedObjects(EMovieSceneTransformChannel Channel)
{
	...
	SetKeysOnControls(SelectedControls, Channel);

}

void FControlRigParameterTrackEditor::OnDeleteTransformKeysForSelectedObjects(EMovieSceneTransformChannel Channel)
{
	const TSharedPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer || !Sequencer->IsAllowedToChange())
	{
	...
	FScopedTransaction DeleteTransaction(LOCTEXT("DeleteKeysOnControls", "Delete Keys On Controls"), !GIsTransacting);

	const EControlRigContextChannelToKey ChannelsToDelete = static_cast<EControlRigContextChannelToKey>(Channel);
	const FFrameNumber CurrentFrame = Sequencer->GetLocalTime().Time.FrameNumber;
	const TRange<FFrameNumber> FrameRange(
		TRangeBound<FFrameNumber>::Inclusive(CurrentFrame),
		TRangeBound<FFrameNumber>::Inclusive(CurrentFrame));

	auto DeleteKeyFromChannel = [&FrameRange](auto* TypedChannel, UMovieSceneSection* Section, bool& bModified)
	{
		TArray<FFrameNumber> KeyTimes;
		TArray<FKeyHandle> KeyHandles;
		TypedChannel->GetData().GetKeys(FrameRange, &KeyTimes, &KeyHandles);
		if (KeyHandles.Num() > 0)
		{
			if (!bModified)
			{
			...
			TypedChannel->DeleteKeys(MakeArrayView(KeyHandles));
		}
	};
	...
	for (const TPair<UControlRig*, TArray<FRigElementKey>>& Selection : SelectedControls)
	{
		...
		for (UMovieSceneSection* BaseSection : Track->GetAllSections())
		{
			...
			TArrayView<FMovieSceneByteChannel*>    ByteChannels    = Section->GetChannelProxy().GetChannels<FMovieSceneByteChannel>();
			TArrayView<FMovieSceneIntegerChannel*> IntegerChannels = Section->GetChannelProxy().GetChannels<FMovieSceneIntegerChannel>();
			...
			for (const FRigElementKey& ControlKey : Selection.Value)
			{
				FRigControlElement* ControlElement = ControlRig->FindControl(ControlKey.Name);
				if (!ControlElement || !Hierarchy->IsAnimatable(ControlElement))
				{
					continue;
				}

				if (Section->GetControlNameMask(ControlElement->GetFName()) == false)
				{
					continue;
				}
				...
				//IF All then delete space and transform keys, do this first to handle previous keys etc..
				// Delete space switch key at this frame for this control
				if (Channel == EMovieSceneTransformChannel::AllTransform || Channel == EMovieSceneTransformChannel::All)
				{
					if (FSpaceControlNameAndChannel* SpaceControlChannel = Section->GetSpaceChannel(ControlElement->GetFName()))
					{
						DeleteKeyFromChannel(&SpaceControlChannel->SpaceCurve, Section, bModified);
					}

					// Delete constraint keys at this frame for this control
					for (FConstraintAndActiveChannel& ConstraintAndChannel : Section->GetConstraintsChannels())
					{
						if (Section->FindControlNameFromConstraintChannel(&ConstraintAndChannel.ActiveChannel) == ControlElement->GetFName())
						{
							DeleteKeyFromChannel(&ConstraintAndChannel.ActiveChannel, Section, bModified);
						}
					}
				}
				...
				{
					if (ChannelIndex < BoolChannels.Num())
					{
						DeleteKeyFromChannel(BoolChannels[ChannelIndex], Section, bModified);
					}
					break;
				}
				case ERigControlType::Integer:
				{
					...
					{
						if (ChannelIndex < ByteChannels.Num())
						{
							DeleteKeyFromChannel(ByteChannels[ChannelIndex], Section, bModified);
						}
					}
					else
					{
						if (ChannelIndex < IntegerChannels.Num())
						{
							DeleteKeyFromChannel(IntegerChannels[ChannelIndex], Section, bModified);
						}
					}
					break;
				}
				...
				{
					if (ChannelIndex < FloatChannels.Num())
					{
						DeleteKeyFromChannel(FloatChannels[ChannelIndex], Section, bModified);
					}
					break;
				}
				case ERigControlType::Vector2D:
				{
					// Ignore channel mask for Vector2D — delete both axes
					if (ChannelIndex < FloatChannels.Num())     DeleteKeyFromChannel(FloatChannels[ChannelIndex],     Section, bModified);
					if (ChannelIndex + 1 < FloatChannels.Num()) DeleteKeyFromChannel(FloatChannels[ChannelIndex + 1], Section, bModified);
					break;
				}
				case ERigControlType::Position:
				{
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::TranslationX) && ChannelIndex < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::TranslationY) && ChannelIndex + 1 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 1], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::TranslationZ) && ChannelIndex + 2 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 2], Section, bModified);
					break;
				}
				case ERigControlType::Rotator:
				{
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::RotationX) && ChannelIndex < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::RotationY) && ChannelIndex + 1 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 1], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::RotationZ) && ChannelIndex + 2 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 2], Section, bModified);
					break;
				}
				case ERigControlType::Scale:
				{
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::ScaleX) && ChannelIndex < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::ScaleY) && ChannelIndex + 1 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 1], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::ScaleZ) && ChannelIndex + 2 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 2], Section, bModified);
					break;
				}
				...
				case ERigControlType::TransformNoScale:
				{
					...
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::TranslationZ) && ChannelIndex + 2 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 2], Section, bModified);
					// Rotation X, Y, Z
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::RotationX) && ChannelIndex + 3 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 3], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::RotationY) && ChannelIndex + 4 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 4], Section, bModified);
					if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::RotationZ) && ChannelIndex + 5 < FloatChannels.Num())
						DeleteKeyFromChannel(FloatChannels[ChannelIndex + 5], Section, bModified);
					// Scale X, Y, Z
					if ((ControlElement->Settings.ControlType) != ERigControlType::TransformNoScale)
					{
						if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::ScaleX) && ChannelIndex + 6 < FloatChannels.Num())
							DeleteKeyFromChannel(FloatChannels[ChannelIndex + 6], Section, bModified);
						if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::ScaleY) && ChannelIndex + 7 < FloatChannels.Num())
							DeleteKeyFromChannel(FloatChannels[ChannelIndex + 7], Section, bModified);
						if (EnumHasAnyFlags(ChannelsToDelete, EControlRigContextChannelToKey::ScaleZ) && ChannelIndex + 8 < FloatChannels.Num())
							DeleteKeyFromChannel(FloatChannels[ChannelIndex + 8], Section, bModified);
					}
					break;
				}
				default:
...
static void EvaluateThisControl(const FFrameRate& TickResolution, UMovieSceneControlRigParameterSection* Section, const FName& ControlName, const FFrameTime& FrameTime)
{
	if (!Section)
	{
		return;
	...
	{ 
		...
		FRigControlModifiedContext Context(EControlRigSetKey::Never);
		double Time = TickResolution.AsSeconds(FrameTime);
		Context.LocalTime = (float)(Time);
		...
		switch (ControlElement->Settings.ControlType)
		{
			case ERigControlType::Bool:
			{
				if (Section->GetBlendType().Get() != EMovieSceneBlendType::Additive)
				{
					...
					if (Value.IsSet())
					{
						ControlRig->SetControlValue<bool>(ControlName, Value.GetValue(), true, Context, bSetupUndo);
					}
				}
				break;
			}
				...
				if (Section->GetBlendType().Get() != EMovieSceneBlendType::Additive)
				{
					if (ControlElement->Settings.ControlEnum)
					{
						...
						if (Value.IsSet())
						{
							...
							ControlRig->SetControlValue<int32>(ControlName, IVal, true, Context, bSetupUndo);
						}
					}
					else
					{
						...
						if (Value.IsSet())
						{
							ControlRig->SetControlValue<int32>(ControlName, Value.GetValue(), true, Context, bSetupUndo);
						}
					}
				}
				break;
			...
			case ERigControlType::ScaleFloat:
			{
				...
				if (Value.IsSet())
				{
					ControlRig->SetControlValue<float>(ControlName, Value.GetValue(), true, Context, bSetupUndo);
				}
				break;
			}
			case ERigControlType::Vector2D:
			{
				...
				if (Value.IsSet())
				{
					ControlRig->SetControlValue<FVector2D>(ControlName, Value.GetValue(), true, Context, bSetupUndo);
				}
				break;
			}
			case ERigControlType::Position:
			...
			case ERigControlType::Rotator:
			{
				...
				if (Value.IsSet())
				{
					...
					ControlRig->SetControlValue<FVector3f>(ControlName, FloatVal, true, Context, bSetupUndo);
				}
				break;
			}
			...
			case ERigControlType::EulerTransform:
			{
				...
				if (Value.IsSet())
				{
					if (ControlElement->Settings.ControlType == ERigControlType::Transform)
					{
						...
						ControlRig->SetControlValue<FRigControlValue::FTransform_Float>(ControlName, Value.GetValue().ToFTransform(), true, Context, bSetupUndo);
					}
					else if (ControlElement->Settings.ControlType == ERigControlType::TransformNoScale)
					{
						...
						FVector EulerAngle(Value.GetValue().Rotation.Roll, Value.GetValue().Rotation.Pitch, Value.GetValue().Rotation.Yaw);
						RigHierarchy->SetControlSpecifiedEulerAngle(ControlElement, EulerAngle);
						ControlRig->SetControlValue<FRigControlValue::FTransformNoScale_Float>(ControlName, NoScale, true, Context, bSetupUndo);
					}
					else if (ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
					{
						...
						ControlRig->SetControlValue<FRigControlValue::FEulerTransform_Float>(ControlName, Transform, true, Context, bSetupUndo);
		...
		//make sure control rig time is correct
		ControlRig->SetAbsoluteTime(Time); 
...
void FControlRigParameterTrackEditor::OnChannelChanged(const TArray<FFrameNumber>& Frames, const FMovieSceneChannelMetaData* MetaData, UMovieSceneSection* InSection)
{
	// During Undo / Redo, this interferes with the Undo / Redo key data that is applied. See CL description.
	if (GIsTransacting)
	{
		return;
	}
			...
			if (FRigControlElement* ControlElement = Section->GetControlRig()->FindControl(ControlName))
			{
				if (ControlElement->Settings.ControlType == ERigControlType::Bool)
				{
					if (Frames.Num() <= 0)
					{
						...
						EvaluateThisControl(TickResolution, Section, ControlName, Time);
					}
					else
					{
						for (const FFrameNumber& Frame : Frames)
						{
							EvaluateThisControl(TickResolution,Section, ControlName, Frame);
						}
					}
				}
			}
			...
			if (Object != nullptr && (Object->IsA<UControlRigBlueprint>() || Object->IsA<UControlRigComponent>() || Object->IsA<URigVMBlueprintGeneratedClass>()))
			{
				if (TSharedPtr<ISequencer> Sequencer = GetSequencer())
				{
					...
					if (Binding.IsValid())
					{
						...
						if (Track == nullptr)
						{
							FControlRigAssetStrongReference RigSource;
							if (FControlRigAssetInterfacePtr EditorAsset = Object)
							{
								RigSource = EditorAsset->GetRuntimeAssetInterface().GetObject();
							}
							else
							...
							if (RigSource.IsValid())
							{
								AddControlRig(RigSource, InComponent, Binding);
							}
						}
					}
				}
...
void FControlRigParameterTrackEditor::OnActivateSequenceChanged(FMovieSceneSequenceIDRef ID)
{
	...
	if (bControlRigEditModeWasOpen && Sequencer && Sequencer->IsLevelEditorSequencer())
	{
		...
		DeferToNextTick(DeactivateModeHandle, [WeakThis = SharedThis(this).ToWeakPtr()]()
		{
			TSharedPtr<FControlRigParameterTrackEditor> SharedThis = WeakThis.IsValid() ? WeakThis.Pin() : nullptr;
			...
			{
				auto GetActiveLevelViewportClient = []() -> FLevelEditorViewportClient* 
				{
				...
				if (FLevelEditorViewportClient* ViewportClient = GetActiveLevelViewportClient())
				{
					if (FEditorModeTools* EditorModeTools = SharedThis->GetEditorModeTools())
					{
						EditorModeTools->Tick(ViewportClient, 0.033f);
					}
				}
				
				// now we can recreate it
				DeferToNextTick(SharedThis->ActivateModeHandle, [WeakThis]()
				{
					TSharedPtr<FControlRigParameterTrackEditor> SharedThis = WeakThis.IsValid() ? WeakThis.Pin() : nullptr;
					if (!SharedThis.IsValid())
					{
					...
					constexpr bool bForceActivate = true;
					if (FControlRigEditMode* ControlRigEditMode = SharedThis->GetEditMode(bForceActivate))
					{
						bool bSequencerSet = false;

						TSharedPtr<ISequencer> Sequencer = SharedThis->GetSequencer();
						for (TPair<TWeakObjectPtr<UControlRig>,FGuid>& ControlRig : SharedThis->BoundControlRigs)
						{
							if (UControlRig* Rig = ControlRig.Key.Get())
							{
								ControlRigEditMode->AddControlRigObject(Rig, Sequencer);
								bSequencerSet = true;

								for (int32 Index = 0; Index < PreviousSelectedControlRigs.Num(); ++Index)
								{
									if (Rig->GetAssetReference() == PreviousSelectedControlRigs[Index].Key)
									{
					...
					SharedThis->ActivateModeHandle.Invalidate();
				});
			}
				
			SharedThis->DeactivateModeHandle.Invalidate();
		});
	}
	...
	if (DataChangeType == EMovieSceneDataChangeType::MovieSceneStructureItemRemoved || DataChangeType == EMovieSceneDataChangeType::Unknown)
	{
		...
		for (UControlRig* ControlRig : ControlRigs)
		{
			if (ControlRig)
			{
				...
				IterateTracks([&bControlRigInTrack, ControlRig](const UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
				{
...
void FControlRigParameterTrackEditor::PostEvaluation(UMovieScene* MovieScene, FFrameNumber Frame)
{
	if (MovieScene)
	{
		IterateTracksInMovieScene(*MovieScene, [this](const UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
		{
...
static void GetAllKeyAreas(TSharedPtr<UE::Sequencer::FViewModel> DataModel, TArray<TSharedPtr<IKeyArea>>& Channels)
{
	using namespace UE::Sequencer;
...
//mz new
static void GetJustSelectedKeyArea(TSharedPtr<ISequencer>& Sequencer, TArray<const IKeyArea*>& OutSelectedKeyAreas)
{
	using namespace UE::Sequencer;
	TSharedPtr<FSequencerEditorViewModel> SequencerViewModel = Sequencer->GetViewModel();
	if (TSharedPtr<FSequencerSelection> Selection = SequencerViewModel ? SequencerViewModel->GetSelection() : nullptr)
	{
		TArray<TSharedRef<FViewModel>> NodesToKey;
		for (FViewModelPtr Node : Selection->Outliner)
		{
			if (Node->IsA<FCategoryGroupModel>() || Node->IsA<FChannelGroupModel>() || Node->IsA<FChannelModel>())
			{
				TArray<TSharedPtr<IKeyArea>> Channels;
				GetAllKeyAreas(Node, Channels);
				//just care about the first key area
				if (Channels.Num() > 0)
				{
					const IKeyArea* KeyAreaPtr = Channels[0].Get();
...
void FControlRigParameterTrackEditor::OnSelectionChanged(TArray<UMovieSceneTrack*> InTracks)
{
	...
	// Some control rig systems set the selection in Curve Editor. Depending on user settings, Curve Editor updates Sequencer selection.
	// If a control rig system started the selection change, don't do any further processing.
	const bool bControlRigDoingSelection = ControlRigEditMode && UE::ControlRig::IsAnimDetailsChangingSelection(*ControlRigEditMode);
	if (bControlRigDoingSelection)
	{
		return;
	}
	...
	const TGuardValue<bool> Guard(bIsDoingSelection, true);
	...
	const bool bEditModeExisted = ControlRigEditMode != nullptr;
	UControlRig* ControlRig = nullptr;

	TArray<const IKeyArea*> KeyAreas;
	const bool bSelectChildChannels = CVarSelectChildChannels.GetValueOnGameThread();
	if (bSelectChildChannels)
	{
	...
	{
		GetJustSelectedKeyArea(Sequencer, KeyAreas);
	}
	...
	{ 
		...
		if (bSelectedSectionSetsSectionToKey)
		{
			...
			{
				if (TViewModelPtr<ISectionOwnerExtension> SectionOwnerExtension = ViewModel.ImplicitCast())
				{
					for (UMovieSceneSection* Section : SectionOwnerExtension->GetSections())
					{
		...
		if (pChannelIndex != nullptr)
		{
			...
			else
			{
				...
				for (const FMovieSceneChannelEntry& Entry : ChannelProxy.GetAllEntries())
				{
					...
					{
						constexpr bool bDoNotSelectParentInstead = false;
						const FMovieSceneChannelHandle Channel = ChannelProxy.MakeHandle(ChannelTypeName, pChannelIndex->ChannelIndex);
						Sequencer->SelectByChannels(ParamSection, {Channel}, bDoNotSelectParentInstead, bSelected);
						break;
					}
				}
			}
...
FMovieSceneTrackEditor::FFindOrCreateTrackResult FControlRigParameterTrackEditor::FindOrCreateControlRigTrackForObject(FGuid ObjectHandle, UControlRig* ControlRig, FName PropertyName, bool bCreateTrackIfMissing)
{
	...
	IterateTracks([&Result, &bTrackExisted, ControlRig](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
	{
		if (Track->GetControlRig() == ControlRig)
		{
			Result.Track = Track;
...
UMovieSceneControlRigParameterTrack* FControlRigParameterTrackEditor::FindTrack(const UControlRig* InControlRig) const
{
	...
	IterateTracks([InControlRig, &FoundTrack](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
	{
...
void FControlRigParameterTrackEditor::ClearOutAllSpaceAndConstraintDelegates(const UControlRig* InOptionalControlRig) const
{
	...
	IterateTracks([InOptionalControlRig](const UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
	{
...
bool FControlRigParameterTrackEditor::ShouldBeSelected(URigHierarchy* Hierarchy, FRigControlElement* ControlElement, FRigControlElement* OtherControlElement)
{
...
bool FControlRigParameterTrackEditor::BakeTimeWarp(TSharedPtr<ISequencer>& Sequencer, UMovieSceneControlRigParameterSection* Section)
{
	using namespace UE::AIE;
	if (Sequencer && Section  && Section->GetControlRig() && Section->HasTimeWarp())
	{
		...
		TArray<FFrameNumber>  Frames = CurrentFrameTimes.GetFullFrameArray();

		int32 NumOfTransforms = CurrentFrameTimes.NumFrames;
		UControlRig* ControlRig = Section->GetControlRig();
		TArray<FRigControlElement*> Controls;
		ControlRig->GetControlsInOrder(Controls);
		TArray<FName> ControlNames;
		ControlNames.Reserve(Controls.Num());
		for (FRigControlElement* ControlElement : Controls)
		{
			if (ControlElement == nullptr || !ControlRig->GetHierarchy()->IsAnimatable(ControlElement))
			{
				continue;
			}
		...
		EMovieSceneTimeUnit TimeUnit = EMovieSceneTimeUnit::TickResolution;
		EControlRigTransformSpace Space = EControlRigTransformSpace::Global;
		ULevelSequence* LevelSequence = Cast<ULevelSequence>(Sequencer->GetFocusedMovieSceneSequence());
		TArray<FArrayOfRigControlTransforms> RigControlTransforms = UControlRigSequencerEditorLibrary::BatchGetControlTransforms(LevelSequence, ControlRig, ControlNames, Frames,
			Space, TimeUnit);

		//delete all keys, cept space and constraints
		Section->RemoveAllKeys(false);

		//delete timewarp
		Section->DeleteTimeWarp();

		//set world transforms back
		UControlRigSequencerEditorLibrary::BatchSetControlTransforms(LevelSequence, ControlRig, RigControlTransforms, Frames,
			Space, Section, TimeUnit);
...
void FControlRigParameterTrackEditor::HandleControlSelected(UControlRig* Subject, FRigControlElement* ControlElement, bool bSelected)
{
	if (!IsInGameThread())
	{
		if (Subject && ControlElement)
		{
			const TWeakObjectPtr<UControlRig> WeakSubject = Subject;
			const FRigElementKey ControlKey = ControlElement->GetKey();

			ExecuteOnGameThread(UE_SOURCE_LOCATION,
				[WeakSubject, ControlKey, bSelected, WeakThis = SharedThis(this).ToWeakPtr()]()
				{
					const TSharedPtr<FControlRigParameterTrackEditor> StrongThis = WeakThis.Pin();
					UControlRig* StrongSubject = WeakSubject.Get();
					URigHierarchy* Hierarchy = StrongSubject ? StrongSubject->GetHierarchy() : nullptr;
					FRigControlElement* StrongControlElement = Hierarchy ? Hierarchy->Find<FRigControlElement>(ControlKey) : nullptr;

					if (StrongThis.IsValid() &&
						StrongSubject &&
						Hierarchy &&
						StrongControlElement)
					{
						StrongThis->HandleControlSelected(StrongSubject, StrongControlElement, bSelected);
					}
				});
		}
	...
	if (!bIsSelectingIndirectControl)
	{
		...
		{			
			...
			if (bSelectChildChannels)
			{
				...
				{
					if (FControlRigParameterTrackEditor::ShouldBeSelected(Hierarchy, ControlElement, OtherControlElement))
					{
	...
	if (bSelected) //if selected
	{
		if (FGuid* BindingID = BoundControlRigs.Find(Subject))
		{
			if (USequencerBakeSubsystem* SequencerBakeSubsystem = GEditor->GetEditorSubsystem<USequencerBakeSubsystem>())
			{
				if (TSharedPtr<UE::Sequencer::FSequencerBaker> SequencerBaker = SequencerBakeSubsystem->GetSequencerBaker(Sequencer))
				{
					constexpr bool bIsolate = false;
					SequencerBaker->IsolateRecorderWithGuid(*BindingID, bIsolate);
				}
			}
		}
	}
...
void FControlRigParameterTrackEditor::HandleControlModified(UControlRig* ControlRig, FRigControlElement* ControlElement, const FRigControlModifiedContext& Context)
{
	...
	IterateTracks([this, ControlRig, ControlElement, Context](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
	{
...
void FControlRigParameterTrackEditor::HandleOnObjectBoundToControlRig(UObject* InObject)
{
	...
	for (TPair<TWeakObjectPtr<UControlRig>, FGuid>& ControlRigPair : BoundControlRigs)
	{
		TStrongObjectPtr<UControlRig> ControlRig = ControlRigPair.Key.Pin();
		...
		if (Selection.Num() > 0)
		{
			ReselectIfNeeded.Add(ControlRigPair.Key, Selection);
		}
		...
		{
			if (const UMovieSceneControlRigParameterTrack* Track = FindTrack(ControlRig.Get()))
			{
				for (UMovieSceneSection* BaseSection : Track->GetAllSections())
				{
					if (UMovieSceneControlRigParameterSection* Section = Cast<UMovieSceneControlRigParameterSection>(BaseSection))
...
void FControlRigParameterTrackEditor::GetControlRigKeys(
	UControlRig* InControlRig,
	FControlRigSectionParamMap& InOutSectionParamMap,
	EControlRigContextChannelToKey ChannelsToKey,
	ESequencerKeyMode KeyMode,
	const bool bInConstraintSpace)
{
	const TSharedPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer || InOutSectionParamMap.IsEmpty())
	{
		return;
	}
	...
	// Create one GeneratedKeys per section and build a flat name → section map for O(1) lookup.
	TMap<FName, UMovieSceneControlRigParameterSection*> ControlToSection;
	for (auto& [Section, SectionData] : InOutSectionParamMap)
	{
		SectionData.GeneratedKeys = MakeShared<FGeneratedTrackKeys>();
		for (const FControlRigKeyAddParams& Param : SectionData.Params)
		{
			ControlToSection.Add(Param.RigControlName, Section);
		}
	}
	...
	// SpaceChannelIndex mirrors the original per-call behaviour: increments only for
	// batch-keyed controls that have a space channel.
	int32 SpaceChannelIndex = 0;
	...
	for (FRigControlElement* ControlElement : Controls)
	{
		...
		UMovieSceneControlRigParameterSection** SectionPtr = ControlToSection.Find(ControlElement->GetFName());
		if (!SectionPtr)
		{
			continue;
		}

		UMovieSceneControlRigParameterSection* SectionToKey = *SectionPtr;
		FGeneratedTrackKeys& OutGeneratedKeys = *InOutSectionParamMap[SectionToKey].GeneratedKeys;

		FChannelMapInfo* pChannelIndex = SectionToKey->ControlChannelMap.Find(ControlElement->GetFName());
		if (!pChannelIndex)
		{
			continue;
		}
		...
		const bool bMaskKeyOut = (SectionToKey->GetControlNameMask(ControlElement->GetFName()) == false);
		const bool bSetKey = !bMaskKeyOut;
		if (!bSetKey)
		{
			continue;
		}
		...
		case ERigControlType::Bool:
		{
			const bool Val = ControlValue.Get<bool>();
			pChannelIndex->GeneratedKeyIndex = OutGeneratedKeys.Num();
			OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneBoolChannel>(ChannelIndex, Val, bSetKey));
			break;
		}
		case ERigControlType::Integer:
		{
			if (ControlElement->Settings.ControlEnum)
			{
				const uint8 Val = (uint8)ControlValue.Get<uint8>();
				pChannelIndex->GeneratedKeyIndex = OutGeneratedKeys.Num();
				OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneByteChannel>(ChannelIndex, Val, bSetKey));
			}
			else
			{
				const int32 Val = ControlValue.Get<int32>();
				pChannelIndex->GeneratedKeyIndex = OutGeneratedKeys.Num();
				OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneIntegerChannel>(ChannelIndex, Val, bSetKey));
			}
			break;
		...
		case ERigControlType::ScaleFloat:
		{
			const float Val = ControlValue.Get<float>();
			pChannelIndex->GeneratedKeyIndex = OutGeneratedKeys.Num();
			OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneFloatChannel>(ChannelIndex, Val, bSetKey));
			break;
		}
		case ERigControlType::Vector2D:
		{
			const bool bKeyX = bSetKey && EnumHasAnyFlags(ChannelsToKey, EControlRigContextChannelToKey::TranslationX);
			const bool bKeyY = bSetKey && EnumHasAnyFlags(ChannelsToKey, EControlRigContextChannelToKey::TranslationY);
			const FVector3f Val = ControlValue.Get<FVector3f>();
			pChannelIndex->GeneratedKeyIndex = OutGeneratedKeys.Num();
			OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneFloatChannel>(ChannelIndex++, Val.X, bKeyX));
			OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneFloatChannel>(ChannelIndex++, Val.Y, bKeyY));
			break;
		...
		case ERigControlType::EulerTransform:
		{
			...
			if (ControlElement->Settings.ControlType == ERigControlType::TransformNoScale)
			{
				const FTransformNoScale NoScale = ControlValue.Get<FRigControlValue::FTransformNoScale_Float>().ToTransform();
				Translation = NoScale.Location;
			}
			else if (ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
			{
				const FEulerTransform Euler = ControlValue.Get<FRigControlValue::FEulerTransform_Float>().ToTransform();
				Translation = Euler.Location;
				Scale = Euler.Scale;
			}
			else
			{
				const FTransform Val = ControlValue.Get<FRigControlValue::FTransform_Float>().ToTransform();
				Translation = Val.GetTranslation();
				Scale = Val.GetScale3D();
			}
			...
			{
				const uint32 ControlHash = UTransformableControlHandle::ComputeHash(InControlRig, ControlElement->GetFName());
				const TOptional<FTransform> Transform = UE::TransformConstraintUtil::GetRelativeTransform(InControlRig->GetWorld(), ControlHash);
				if (Transform)
				{
					Translation = Transform->GetTranslation();
					if (InControlRig->GetHierarchy()->GetUsePreferredRotationOrder(ControlElement))
			...
			if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::TranslationX)) { bKeyX = false; }
			if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::TranslationY)) { bKeyY = false; }
			if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::TranslationZ)) { bKeyZ = false; }
			...
			{
				FMovieSceneControlRigSpaceBaseKey NewKey;
				const int32 RealSpaceChannelIndex = pChannelIndex->SpaceChannelIndex != -1 ? pChannelIndex->SpaceChannelIndex : SpaceChannelIndex;
				++SpaceChannelIndex;
				OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneControlRigSpaceChannel>(RealSpaceChannelIndex, NewKey, false));
			}
			...
			if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::RotationX)) { bKeyX = false; }
			if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::RotationY)) { bKeyY = false; }
			if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::RotationZ)) { bKeyZ = false; }
			...
			if (ControlElement->Settings.ControlType == ERigControlType::Transform || ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
			{
				...
				if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::ScaleX)) { bKeyX = false; }
				if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::ScaleY)) { bKeyY = false; }
				if (!EnumHasAnyFlags(TransformMask, EMovieSceneTransformChannel::ScaleZ)) { bKeyZ = false; }
				OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneFloatChannel>(ChannelIndex++, CurrentVector.X, bKeyX));
				OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneFloatChannel>(ChannelIndex++, CurrentVector.Y, bKeyY));
				OutGeneratedKeys.Add(FMovieSceneChannelValueSetter::Create<FMovieSceneFloatChannel>(ChannelIndex++, CurrentVector.Z, bKeyZ));
			}
...
FKeyPropertyResult FControlRigParameterTrackEditor::AddKeysToControlRigBatch(
	UObject* InObject, UControlRig* InControlRig,
	FGuid ObjectHandle, UMovieSceneControlRigParameterTrack* Track,
	FFrameNumber KeyTime, FFrameNumber EvaluateTime,
	FControlRigSectionParamMap& InOutSectionParamMap,
	ESequencerKeyMode KeyMode, FName ControlRigName)
{
	FKeyPropertyResult CombinedResult;
	...
	// Each section is visited exactly once: one RemapTime, one bounds check, one weight calculation,
	// one interrogation, and one AddKeysToSection using the section's single GeneratedKeys.
	for (auto& [ParamSection, SectionData] : InOutSectionParamMap)
	{
		if (!SectionData.GeneratedKeys.IsValid() || SectionData.GeneratedKeys->Num() <= 0)
		{
			continue;
		}

		// Time remap handles time-warped sections.
		const FFrameNumber RemappedTime = ParamSection->RemapTime(FFrameTime(KeyTime)).GetFrame();

		// Extend section bounds once.
		if (ParamSection->HasEndFrame() && ParamSection->GetExclusiveEndFrame() < RemappedTime)
		{
			ParamSection->SetEndFrame(RemappedTime);
		}
		else if (ParamSection->HasStartFrame() && ParamSection->GetInclusiveStartFrame() > RemappedTime)
		{
			ParamSection->SetStartFrame(RemappedTime);
		}

		if (!ParamSection->GetRange().Contains(RemappedTime))
		{
			continue;
		}

		// One weight calculation per section.
		const float Weight = MovieSceneHelpers::CalculateWeightForBlending(ParamSection, RemappedTime);

		// One track interrogation for all controls in this section.
		ModifyOurGeneratedKeysByCurrentAndWeight(InObject, InControlRig, Track, ParamSection, EvaluateTime, SectionData, Weight);

		// GeneratedKeys already holds all controls' keys — pass it directly, no combining needed.
		FKeyPropertyResult KeyResult = AddKeysToSection(ParamSection, RemappedTime, *SectionData.GeneratedKeys, KeyMode, EKeyFrameTrackEditorSetDefault::SetDefaultOnAddKeys);
		CombinedResult |= KeyResult;
	}

	// Compensate spaces and constraints per control.
	if (CombinedResult.bKeyCreated)
	{
		for (auto& [ParamSection, SectionData] : InOutSectionParamMap)
		{
			UControlRig* SectionControlRig = ParamSection->GetControlRig();
			if (!SectionControlRig || SectionControlRig->IsEvaluating())
			{
				continue;
			}
			...
			for (const FControlRigKeyAddParams& Param : SectionData.Params)
			{
				TOptional<FFrameNumber> OptionalKeyTime = RemappedTime;
				...
				const uint32 ControlHash = UTransformableControlHandle::ComputeHash(SectionControlRig, Param.RigControlName);
	...
	ESequencerKeyMode KeyMode, TSubclassOf<UMovieSceneTrack> TrackClass, FName ControlRigName, FName RigControlName)
{
	...
	const TSharedPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer || GeneratedKeys.Num() <= 0)
	{
		return KeyPropertyResult;
	}
	...
	const bool bInConstraintSpace)
{
	...
	auto OnKeyProperty = [=, this](FFrameNumber Time) -> FKeyPropertyResult
	{
		if (const TSharedPtr<ISequencer> Sequencer = GetSequencer())
		{
			...
			LocalTime = ParamSection->RemapTime(FFrameTime(LocalTime)).GetFrame();
...
void FControlRigParameterTrackEditor::AddControlKeys(
	...
	UControlRig* InControlRig,
	FName ControlRigName,
	TArray<FControlRigKeyAddParams>& InOutParams,
	EControlRigContextChannelToKey ChannelsToKey,
	ESequencerKeyMode KeyMode,
	float InLocalTime,
	const bool bInConstraintSpace)
{
	const TSharedPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer || KeyMode == ESequencerKeyMode::ManualKey || !Sequencer->IsAllowedToChange())
	{
		return;
	}

	// Resolve handle and track once for all controls in the batch.
	constexpr bool bCreateTrack = false;
	FFindOrCreateHandleResult HandleResult = FindOrCreateHandleToObject(InObject, InControlRig);
	...
	if (InLocalTime != FLT_MAX)
	{
		const FFrameTime LocalFrameTime = Sequencer->GetFocusedTickResolution().AsFrameTime((double)InLocalTime);
		BeginKeying(LocalFrameTime.RoundToFrame());
	}

	// Group controls by section. Modify() each section once when it first appears in the map.
	FControlRigSectionParamMap SectionParamMap;
	for (const FControlRigKeyAddParams& Param : InOutParams)
	{
		UMovieSceneSection* Section = Track->GetSectionToKey(Param.RigControlName);
		UMovieSceneControlRigParameterSection* ParamSection = Cast<UMovieSceneControlRigParameterSection>(Section);
		if (!ParamSection || ParamSection->GetDoNotKey())
		{
			continue;
		}
		FControlRigSectionKeyData& SectionData = SectionParamMap.FindOrAdd(ParamSection);
		if (SectionData.Params.IsEmpty())
		{
			// First control for this section — mark it dirty before any modifications.
			ParamSection->Modify();
		}
		SectionData.Params.Add(Param);
	}

	if (SectionParamMap.IsEmpty())
	{
		EndKeying();
		return;
	}

	// Single pass over the rig hierarchy; each section gets one shared GeneratedKeys.
	GetControlRigKeys(InControlRig, SectionParamMap, ChannelsToKey, KeyMode, bInConstraintSpace);

	// Drop sections whose GeneratedKeys are empty.
	for (auto It = SectionParamMap.CreateIterator(); It; ++It)
	{
		const TSharedPtr<FGeneratedTrackKeys>& Keys = It->Value.GeneratedKeys;
		if (!Keys.IsValid() || Keys->Num() <= 0)
		{
			It.RemoveCurrent();
		}
	}
	...
	// Move the map into the lambda — AnimatablePropertyChanged is synchronous, but owning it is cleaner.
	auto OnKeyProperty = [InObject, InControlRig, InLocalTime, KeyMode, ControlRigName, ObjectHandle, Track,
		SectionMap = MoveTemp(SectionParamMap), this](FFrameNumber Time) mutable -> FKeyPropertyResult
	{
		const TSharedPtr<ISequencer> LocalSequencer = GetSequencer();
		if (!LocalSequencer)
		{
			static const FKeyPropertyResult Dummy;
			return Dummy;
		...
		FFrameNumber LocalTime = Time;
		FFrameNumber EvaluateTime = LocalSequencer->GetLastEvaluatedLocalTime().RoundToFrame();
		if (InLocalTime != FLT_MAX)
		{
			const FFrameTime LocalFrameTime = LocalSequencer->GetFocusedTickResolution().AsFrameTime((double)InLocalTime);
			LocalTime = LocalFrameTime.RoundToFrame();
			EvaluateTime = LocalTime;
		}

		return AddKeysToControlRigBatch(InObject, InControlRig, ObjectHandle, Track,
			LocalTime, EvaluateTime, SectionMap, KeyMode, ControlRigName);
	};
...
bool FControlRigParameterTrackEditor::ModifyOurGeneratedKeysByCurrentAndWeight(UObject* Object, UControlRig* InControlRig, FName RigControlName, UMovieSceneTrack* Track, UMovieSceneSection* SectionToKey, FFrameNumber EvaluateTime, FGeneratedTrackKeys& GeneratedTotalKeys, float Weight) const
{
	...
	{
		UMovieSceneControlRigParameterTrack* ControlRigParameterTrack = CastChecked<UMovieSceneControlRigParameterTrack>(Track);
		...
		const TUniquePtr<FMovieSceneEvaluationTrack>& MovieSceneEvaluationTrack = ControlRigParameterTrack->GetLegacyEvaluationTrack();
		if (MovieSceneEvaluationTrack.IsValid())
		{
			MovieSceneEvaluationTrack->Interrogate(Context, InterrogationData, Object);
		}
		else		
		{
			FMovieSceneEvaluationTrack EvalTrack = ControlRigParameterTrack->GenerateTrackTemplate(Track);
	...
	//evaluate time will be in Sequencer unwarped time since we interrogated to get the 'final' value
	//we know need to convert it to warped time so we can blend the delta with the modified value/weight
	EvaluateTime = Section->RemapTime(FFrameTime(EvaluateTime)).GetFrame();
	...
	for (FRigControlElement* ControlElement : Controls)
	{
		...
		if (!pChannelIndex || pChannelIndex->GeneratedKeyIndex == INDEX_NONE || RigControlName != ControlName)
		{
			continue;
		}
...
bool FControlRigParameterTrackEditor::ModifyOurGeneratedKeysByCurrentAndWeight(
	UObject* Object, UControlRig* InControlRig,
	UMovieSceneTrack* Track, UMovieSceneSection* SectionToKey, FFrameNumber EvaluateTime,
	FControlRigSectionKeyData& InOutSectionKeyData, float Weight) const
{
	using namespace UE::MovieScene;

	TSharedPtr<ISequencer> Sequencer = GetSequencer();
	if (!Sequencer || !InOutSectionKeyData.GeneratedKeys.IsValid())
	{
		return false;
	}

	// Build a set of the control names being keyed for fast membership checks.
	TSet<FName> KeyedControlNames;
	KeyedControlNames.Reserve(InOutSectionKeyData.Params.Num());
	for (const FControlRigKeyAddParams& Param : InOutSectionKeyData.Params)
	{
		KeyedControlNames.Add(Param.RigControlName);
	}

	// Run a single track interrogation for the whole section — this is the expensive step.
	FControlRigParameterValues ParameterBufferValues(EControlRigParameterBufferIndexStability::Stable);
	const FFrameRate TickResolution = Sequencer->GetFocusedTickResolution();
	...
	TArray<FRigControlElement*> Controls = InControlRig->AvailableControls();
	TArray<FMovieSceneChannelValueSetter, TInlineAllocator<1>>& GeneratedKeys = *InOutSectionKeyData.GeneratedKeys;
	...
	for (FRigControlElement* ControlElement : Controls)
	{
		...
		if (!KeyedControlNames.Contains(ControlName))
		{
			continue;
		}
		...
		case ERigControlType::ScaleFloat:
		{
			...
			if (ParameterBufferValues.Find<float>(ControlName, Value))
			{
				GeneratedKeys[pChannelIndex->GeneratedKeyIndex]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&Value, Weight);
			}
			break;
		}
		case ERigControlType::Bool:
		case ERigControlType::Integer:
		{
			// No blending for bools or ints/enums.
			break;
		}
		case ERigControlType::Vector2D:
		{
			...
			if (ParameterBufferValues.Find(ControlName, Value))
			{
				GeneratedKeys[pChannelIndex->GeneratedKeyIndex    ]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&Value.X, Weight);
				GeneratedKeys[pChannelIndex->GeneratedKeyIndex + 1]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&Value.Y, Weight);
			}
			break;
		}
		case ERigControlType::Position:
		...
		case ERigControlType::Rotator:
		{
			...
			if (ParameterBufferValues.Find(ControlName, Value))
			{
				GeneratedKeys[pChannelIndex->GeneratedKeyIndex    ]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&Value.X, Weight);
				GeneratedKeys[pChannelIndex->GeneratedKeyIndex + 1]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&Value.Y, Weight);
				GeneratedKeys[pChannelIndex->GeneratedKeyIndex + 2]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&Value.Z, Weight);
			}
			break;
		}
		case ERigControlType::Transform:
		...
		case ERigControlType::EulerTransform:
		{
			...
			if (ParameterBufferValues.Find(ControlName, Value))
			{
				...
				const int32 ChannelIndex = pChannelIndex->bDoesHaveSpace ? pChannelIndex->GeneratedKeyIndex + 1 : pChannelIndex->GeneratedKeyIndex;

				GeneratedKeys[ChannelIndex    ]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentPos.X, Weight);
				GeneratedKeys[ChannelIndex + 1]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentPos.Y, Weight);
				GeneratedKeys[ChannelIndex + 2]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentPos.Z, Weight);

				GeneratedKeys[ChannelIndex + 3]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentRot.Roll,  Weight);
				GeneratedKeys[ChannelIndex + 4]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentRot.Pitch, Weight);
				GeneratedKeys[ChannelIndex + 5]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentRot.Yaw,   Weight);

				if (ControlElement->Settings.ControlType == ERigControlType::Transform || ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
				{
					FVector3f CurrentScale(Value.GetScale3D());
					GeneratedKeys[ChannelIndex + 6]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentScale.X, Weight);
					GeneratedKeys[ChannelIndex + 7]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentScale.Y, Weight);
					GeneratedKeys[ChannelIndex + 8]->ModifyByCurrentAndWeight(Proxy, EvaluateTime, (void*)&CurrentScale.Z, Weight);
				}
			}
			break;
		}
...
void FControlRigParameterTrackEditor::ProcessKeyOperation(FFrameNumber InKeyTime, const UE::Sequencer::FKeyOperation& Operation, ISequencer& InSequencer, TArray<UE::Sequencer::FAddKeyResult>* OutResults)
{
	...
	if (UMovieSceneControlRigParameterTrack::ShouldUseLegacyTemplate())
	{
		TMap<UMovieSceneControlRigParameterTrack*, TUniquePtr<FScopedLegacyEvaluationTrack>> CachedEvaluationTracks;
	
		// Cache the evaluation tracks if requested
		if (UMovieSceneControlRigParameterTrack::ShouldCacheLegacyTemplate())
		{
			IterateTracks([&CachedEvaluationTracks](UMovieSceneControlRigParameterTrack* Track, FGuid InGuid)
		   {
			   if (Track && !CachedEvaluationTracks.Contains(Track))
			   {
				   CachedEvaluationTracks.Emplace(Track, new FScopedLegacyEvaluationTrack(Track));
			   }
			   return true;
		   });
		}
...
void FControlRigParameterTrackEditor::ProcessKeyOperation(UObject* ObjectToKey, UMovieSceneControlRigParameterTrack* Track, TArrayView<const UE::Sequencer::FKeySectionOperation> SectionsToKey, ISequencer& InSequencer, FFrameNumber KeyTime, TArray<UE::Sequencer::FAddKeyResult>* OutResults)
{
	...
	for (const FKeySectionOperation& Operation : SectionsToKey)
	{
		...
		for (TSharedPtr<IKeyArea> KeyArea : Operation.KeyAreas)
		{
			...
			FControlRigValueView DesiredValue = RecomposedValues.Find(ChannelMetaData.GetControlName(), ChannelMetaData.GetType());
...
UE::MovieScene::FControlRigParameterValues FControlRigParameterTrackEditor::GetRecomposedControlValues(UMovieSceneControlRigParameterTrack* Track, UMovieSceneControlRigParameterSection* Section, FFrameNumber KeyTime)
{
	...
	{
		FControlRigValueView Value = CurrentValues.Find(Scalar.ParameterName, EControlRigControlType::Parameter_Scalar);
		if (Value)
		{
	...
	{
		FControlRigValueView Value = CurrentValues.Find(Vector.ParameterName, EControlRigControlType::Parameter_Vector);
		if (Value)
		{
	...
	{
		FControlRigValueView Value = CurrentValues.Find(Transform.ParameterName, EControlRigControlType::Parameter_Transform);
		if (Value)
		{
...
void FControlRigParameterTrackEditor::BuildTrackContextMenu(FMenuBuilder& MenuBuilder, UMovieSceneTrack* InTrack)
{
	...
	if (bIsSection)
	{
		...
		for (UE::Sequencer::TViewModelPtr<UE::Sequencer::ISectionOwnerExtension> SectionOwnerExtension : GetSequencer()->GetViewModel()->GetSelection()->Outliner.Filter<UE::Sequencer::ISectionOwnerExtension>())
		{
			for (UMovieSceneSection* Section : SectionOwnerExtension->GetSections())
			{
	...
	if (!bIsSection)
	{
		MenuBuilder.BeginSection("Control Rig", LOCTEXT("ControlRig", "Control Rig"));
		{
			// Don't show priority order for child tracks, it's handled by the layers in the anim mixer
			if (Track->ParentTrack == nullptr)
			{
...
{
	if (!IControlRigEditorAssetInterface::IsAControlRigAsset(Asset))
	{
		return false;
	}
	...
	if (FControlRigAssetInterfacePtr EditorAsset = Asset)
	{
		RigSource = EditorAsset->GetControlRigAssetReference();
		SkeletalMesh = EditorAsset->GetPreviewMesh();
	}
	else if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = Asset)
	{
		RigSource = RuntimeAsset.GetObject();
		SkeletalMesh = RuntimeAsset->GetPreviewSkeletalMesh().Get();
	}
	else
	{
		return false;
	...
	if (Track == nullptr)
	{
		AddControlRig(RigSource, SpawnedSkeletalMeshActor->GetSkeletalMeshComponent(), NewGuid);
	}
		...
		if (bValid && AutoRig)
		{
			...
			if (Track && Sequencer)
			{
				...
				for (UMovieSceneSection* IterSection : Sections)
				{
					...
					if (Section)
					{
						constexpr bool bReconstructChannelProxy = false;
						for (const FFKBoneCheckInfo& Info : BoneCheckArray)
						{
							Section->SetControlNameMask(Info.BoneName, Info.bActive, bReconstructChannelProxy);
						}
...
bool FControlRigParameterTrackEditor::CollapseAllLayers(TSharedPtr<ISequencer>&SequencerPtr, UMovieSceneTrack * OwnerTrack, const FBakingAnimationKeySettings &InSettings)
{
	...
	else
	{
		if (SequencerPtr.IsValid() && OwnerTrack)
		{
			...
			if (Sections.Num() < 1)
			{
				UE_LOGF(LogControlRigEditor, Log, "CollapseAllSections::No sections on track");
				return false;
			}
			if (UMovieSceneControlRigParameterSection* ParameterSection = Cast<UMovieSceneControlRigParameterSection>(Sections[0]))
			{
				...
				{
					UE_LOGF(LogControlRigEditor, Log, "CollapseAllSections:: First section is not additive");
					return false;
				}
			}
			else
			{
				UE_LOGF(LogControlRigEditor, Log, "CollapseAllSections:: No Control Rig section");
				return false;
			}
		}
		UE_LOGF(LogControlRigEditor, Log, "CollapseAllSections:: Sequencer or track is invalid");
	}
	return false;
}
...
void FControlRigParameterSection::BuildSectionContextMenu(FMenuBuilder& MenuBuilder, const FGuid& InObjectBinding)
{
	...
	MenuBuilder.EndSection();
	MenuBuilder.BeginSection(NAME_None, LOCTEXT("TimeWarp", "TimeWarp"));
	{
		using namespace UE::Sequencer;

		TSharedPtr<ISequencer> SequencerPtr = WeakSequencer.Pin();
		FSectionModelStorageExtension* SectionModelStorage = SequencerPtr->GetViewModel()->GetRootModel()->CastDynamic<FSectionModelStorageExtension>();
		check(SectionModelStorage);

		TSharedPtr<FSectionModel> SectionHandle = SectionModelStorage->FindModelForSection(ParameterSection);
		TWeakViewModelPtr<FViewModel> WeakViewModel = SectionHandle->AsWeak();
		FSequencerUtilities::MakeTimeWarpMenuEntry(MenuBuilder, SectionHandle);

		if (ParameterSection->HasTimeWarp())
		{
			MenuBuilder.AddMenuEntry(
				LOCTEXT("DeleteTimeWarp", "Delete Time Warp"),
				LOCTEXT("DeleteTimeWarp_ToolTip", "Delete Time Warp"),
				FSlateIcon(),
				FUIAction(FExecuteAction::CreateLambda([this]
					{
						if (UMovieSceneControlRigParameterSection* ParameterSection = CastChecked<UMovieSceneControlRigParameterSection>(WeakSection.Get()))
						{
							FScopedTransaction Transaction(LOCTEXT("DeleteTimeWarp", "Delete Time Warp"));
							ParameterSection->DeleteTimeWarp();
			...
			MenuBuilder.AddMenuEntry(
				LOCTEXT("Bake Time Warp", "Bake Time Warp"),
				LOCTEXT("BakeTimeWarp_ToolTip", "Bake Time Warp and delete it"),
				FSlateIcon(),
				FUIAction(FExecuteAction::CreateLambda([this] 
					{
						if (TSharedPtr<ISequencer> SequencerPtr = WeakSequencer.Pin())
						{
							if (UMovieSceneControlRigParameterSection* ParameterSection = CastChecked<UMovieSceneControlRigParameterSection>(WeakSection.Get()))
							{
								FScopedTransaction Transaction(LOCTEXT("BakeTimeWarp", "Bake Time Warp"));
								FControlRigParameterTrackEditor::BakeTimeWarp(SequencerPtr, ParameterSection);
							}
						}
					}))
			);
...
void FControlRigParameterSection::ShowSelectedControlsChannels()
{
	...
	if (ParameterSection && ControlRig && SequencerPtr.IsValid())
	{
		...
		for (const FRigControlElement* RigControl : Controls)
		{
			...
			if (FChannelMapInfo* pChannelIndex = ParameterSection->ControlChannelMap.Find(RigName))
			{ 
				constexpr bool bReconstructChannelProxy = false;
				const bool bIsSelected = ControlRig->IsControlSelected(RigName);
				ParameterSection->SetControlNameMask(RigName, bIsSelected, bReconstructChannelProxy);
			}
		}
		ParameterSection->ReconstructChannelProxy();
		SequencerPtr->NotifyMovieSceneDataChanged(EMovieSceneDataChangeType::MovieSceneStructureItemsChanged);
	}
}
...
bool FControlRigParameterSection::RequestDeleteKeyArea(const TArray<FName>& KeyAreaNamePaths)
{
	...
	UMovieSceneControlRigParameterSection* ControlRigSection = CastChecked<UMovieSceneControlRigParameterSection>(WeakSection.Get());
	
	if (!Sequencer || !ControlRigSection || KeyAreaNamePaths.Num() <= 0)
	{
		return false;
	}

	const FName KeyAreaLeafName = KeyAreaNamePaths.Last();
	if (KeyAreaLeafName == TEXT("TimeWarp") || KeyAreaLeafName == TEXT("PlayRate"))
	{
		ControlRigSection->DeleteTimeWarp();
		return true;
	}

	if (KeyAreaNamePaths.Num() != 2)
	{
		return false;
	}

	// KeyAreaNamePaths[0] is the control's name
	const FName ConstraintName = KeyAreaNamePaths[1];
	UTickableConstraint* Constraint = ControlRigSection->FindConstraint(ConstraintName);
	if (!Constraint)
	{
		return false;
	}
	...
	{
		const FScopedTransaction Transaction(LOCTEXT("DeleteConstraintChannel", "Delete Constraint Channel"));
		if (ControlRigSection->TryModify())
		{
			FConstraintsManagerController& Controller = FConstraintsManagerController::Get(World);
			if (Controller.RemoveConstraint(Constraint))
			{
				Sequencer->NotifyMovieSceneDataChanged(EMovieSceneDataChangeType::MovieSceneStructureItemsChanged);
				return true;
			}
