...
#include "Sequencer/SaveScopedControlRigSelection.h"
...
ECheckBoxState UAnimLayer::GetKeyed() const
{
	...
	return State.Type == EAnimLayerType::Base ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}
		...
		if (Pair.Key != nullptr)
		{
			for (const FAnimLayerSectionItem& SectionItem : Pair.Value.SectionItems)
			{
				if (SectionItem.Section.IsValid())
				{
					if (UMovieSceneTrack* Track = Cast<UMovieSceneTrack>(SectionItem.Section.Get()->GetTypedOuter< UMovieSceneTrack>()))
					{
						const bool bActive = !Track->IsRowEvalDisabled(SectionItem.Section->GetRowIndex());

						//make sure other track rows match state, and if active the sections are active
						if (CurrentVal.IsSet() && CurrentVal.GetValue() != bActive)
						{
							Track->SetRowEvalDisabled(!CurrentVal.GetValue(), SectionItem.Section->GetRowIndex());
						}
						//make sure section is active
						if (bActive  && SectionItem.Section->IsActive() == false)
						{
							SectionItem.Section->SetIsActive(bActive);
						}
						if (CurrentVal.IsSet() == false)
						{
							CurrentVal = bActive;
...
void UAnimLayer::PostLoad()
{
	Super::PostLoad();

	TWeakObjectPtr<UAnimLayer> WeakThis = this;

	GEditor->GetTimerManager()->SetTimerForNextTick(FTimerDelegate::CreateLambda([WeakThis, this]()
		{
			if (WeakThis.IsValid())
			{
				const bool bTrackRowDisabled = !WeakThis->State.bActive;
				for (const TPair<TWeakObjectPtr<UObject>, FAnimLayerItem>& Pair : WeakThis->AnimLayerItems)
				{
					if (Pair.Key != nullptr)
					{
						for (const FAnimLayerSectionItem& SectionItem : Pair.Value.SectionItems)
						{
							if (SectionItem.Section.IsValid())
							{
								if (UMovieSceneTrack* Track = Cast<UMovieSceneTrack>(SectionItem.Section.Get()->GetTypedOuter< UMovieSceneTrack>()))
								{ 
									const int32 SectionRowIndex = SectionItem.Section->GetRowIndex();
									bool bTrackModified = false;
									if (Track->IsRowEvalDisabled(SectionItem.Section->GetRowIndex()) != bTrackRowDisabled)
									{
										bTrackModified = true;
										Track->Modify();
										Track->SetRowEvalDisabled(bTrackRowDisabled, SectionRowIndex);
									}
									//make sure all of the sections are active
									const TArray<UMovieSceneSection*>& Sections = Track->GetAllSections();
									if (Sections.Num() > 0)
									{
										TArray<UMovieSceneSection*> SectionsToDelete;
										for (UMovieSceneSection* Section : Sections)
										{
											if (Section)
											{
												if (Section != SectionItem.Section.Get() && Section->GetRowIndex() == SectionRowIndex)
												{
										...
										if (SectionsToDelete.Num() > 0)
										{
											if (bTrackModified == false)
											{
												Track->Modify();
											}
											for (UMovieSceneSection* SectionToDelete : SectionsToDelete)
											{
												Track->RemoveSection(*SectionToDelete);
											}
										}
									}
								}
		...
		if (Pair.Key != nullptr)
		{
			for (const FAnimLayerSectionItem& SectionItem : Pair.Value.SectionItems)
			{
				if (SectionItem.Section.IsValid())
				{
					...
					{
						Track->Modify();
						Track->SetRowEvalDisabled(!State.bActive, SectionItem.Section->GetRowIndex());
						if (State.bActive)
						{
							//make sure to turn on, may be off from legacy
						...
						if (TSharedPtr<ISequencer> SequencerPtr = UAnimLayers::GetSequencerFromAsset())
						{
							...
							if (ControlRig)
							{
								if (UMovieSceneControlRigParameterSection* CRSection = Cast<UMovieSceneControlRigParameterSection>(SectionItem.Section.Get()))
								{
									for (const TPair<FName, FAnimLayerPropertyAndChannels>& ControlName : SectionItem.AnimLayerSet.Names)
									{
										if (State.bActive && ControlRig->IsControlSelected(ControlName.Key) == false)
										{
										...
										if (pChannelIndex != nullptr)
										{
											if (pChannelIndex->ParentControlIndex == INDEX_NONE)
											{
												...
												{
													SequencerPtr->SelectByNthCategoryNode(CRSection, CategoryIndex, State.bActive);
												}
											}
										}
									}
							...
							if (State.bActive == false || ControlRig == nullptr) //if not a control rig we select the whole trackrow, always make sure to deselect it
							{
...
void UAnimLayer::ApplyControlSelectionToSequencer(bool bSelect) const
{
	using namespace UE::Sequencer;

	if (TSharedPtr<ISequencer> Sequencer = UAnimLayers::GetSequencerFromAsset())
	{
		for (const TPair<TWeakObjectPtr<UObject>, FAnimLayerItem>& Pair : AnimLayerItems)
		{
			if (!Pair.Key.IsValid())
			...
			for (const FAnimLayerSectionItem& SectionItem : Pair.Value.SectionItems)
			{
				...
				UMovieSceneControlRigParameterTrack* CRTrack = Cast<UMovieSceneControlRigParameterTrack>(CRSection->GetTypedOuter<UMovieSceneTrack>());
				if (!CRTrack)
				{
					continue;
				}

				for (const TPair<FName, FAnimLayerPropertyAndChannels>& ControlEntry : SectionItem.AnimLayerSet.Names)
				{
					const FName& ControlName = ControlEntry.Key;

					// When selecting, only show currently selected controls.
					// When deselecting, clear all, control selection may have changed since the last select pass.
					if (bSelect && 
						!ControlRig->IsControlSelected(ControlName))
					{
						continue;
					}

					// Only select sections to key
					if (CRTrack->GetSectionToKey(ControlName) != CRSection)
					{
						continue;
					}

					// Only select root controls, SelectByNthCategoryNode selects children implicitly
					FChannelMapInfo* ChannelMapInfo = CRSection->ControlChannelMap.Find(ControlName);
					if (!ChannelMapInfo || 
						ChannelMapInfo->ParentControlIndex != INDEX_NONE)
					{
						continue;
					}

					const int32 CategoryIndex = CRSection->GetActiveCategoryIndex(ControlName);
					if (CategoryIndex != INDEX_NONE)
					{
						Sequencer->SelectByNthCategoryNode(CRSection, CategoryIndex, bSelect);
					}
				}
			}
		}
...
bool UAnimLayer::HasSectionsInCurrentEditorSelection() const
{
	TArray<AActor*> SelectedActors;
	USelection* EditorSelection = GEditor ? GEditor->GetSelectedActors() : nullptr;
	if (EditorSelection)
	{
		EditorSelection->GetSelectedObjects<AActor>(SelectedActors);
	}

	for (const TPair<TWeakObjectPtr<UObject>, FAnimLayerItem>& LayerItem : AnimLayerItems)
	{
		if (!LayerItem.Key.IsValid())
		{
			continue;
		}

		for (const FAnimLayerSectionItem& SectionItem : LayerItem.Value.SectionItems)
		{
			UMovieSceneSection* Section = SectionItem.Section.Get();
			if (!Section)
			{
				continue;
			}

			UMovieSceneControlRigParameterTrack* ControlRigParameterTrack = Section->GetTypedOuter<UMovieSceneControlRigParameterTrack>();
			if (ControlRigParameterTrack)
			{
				const UControlRig* ControlRig = ControlRigParameterTrack->GetControlRig();
				if (!ControlRig)
				{
					continue;
				}
				...
				for (const TPair<FName, FAnimLayerPropertyAndChannels>& ControlName : SectionItem.AnimLayerSet.Names)
				{
					if (ControlRig->IsControlSelected(ControlName.Key))
					{
						return true;
					}
				}
			}
			else if (SectionItem.AnimLayerSet.BoundObject.IsValid())
			{
				if (AActor* OuterActor = SectionItem.AnimLayerSet.BoundObject->GetTypedOuter<AActor>())
				{
					if (SelectedActors.Contains(OuterActor))
					{
						return true;
					}
				}
...
void UAnimLayer::SelectWeightChannel(bool bClearSelection)
{
	TSharedPtr<ISequencer> SequencerPtr = UAnimLayers::GetSequencerFromAsset();
	if (!SequencerPtr)
	{
	...
	const TArray<FName> WeightChannelNames = { FName("Weight") };
	for (const TPair<TWeakObjectPtr<UObject>, FAnimLayerItem>& Pair : AnimLayerItems)
	{
		if (!Pair.Key.IsValid())
		{
		...
		for (const FAnimLayerSectionItem& SectionItem : Pair.Value.SectionItems)
		{
			...
			UMovieSceneSection* Section = SectionItem.Section.Get();
			if (Cast<UMovieSceneControlRigParameterSection>(Section) || Cast<UMovieScene3DTransformSection>(Section))
			{
				SequencerPtr->SelectByChannels(Section, WeightChannelNames, false, true);
			}
		}
	}
}
...
{
	Super::PostEditUndo();
	SetAnimLayerIndices(false /*bResetSectionOrders*/);
	TMap<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>> TracksAndOrderedSections;
	GetTracksAndSections(TracksAndOrderedSections);
	SetTrackRowForSections(TracksAndOrderedSections);
	AnimLayerListChangedBroadcast();
}
#endif

void UAnimLayers::PostLoad()
{
	Super::PostLoad();
	SetAnimLayerIndices(false /*bResetSectionOrders*/);
	TMap<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>> TracksAndOrderedSections;
...
void UAnimLayers::GetTracksAndSections(TMap<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>>& OutTracksAndOrderedSections)
{
			...
			if (Pair.Key != nullptr)
			{
				for (const FAnimLayerSectionItem& SectionItem : Pair.Value.SectionItems)
				{
					if (SectionItem.Section.IsValid())
					{
						if (UMovieSceneTrack* Track = SectionItem.Section->GetTypedOuter<UMovieSceneTrack>())
						{
							TArray<TPair<UAnimLayer*, UMovieSceneSection*>>& TrackSections = OutTracksAndOrderedSections.FindOrAdd(Track);
							TPair<UAnimLayer*, UMovieSceneSection*> NameSection;
							NameSection.Key = AnimLayer;
							NameSection.Value = SectionItem.Section.Get();
							TrackSections.Add(NameSection);
						}
					}
				}
			}
...
void UAnimLayers::SetTrackRowForSections(const TMap<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>>& TracksAndOrderedSections)
{
	for (const TPair<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>>& TrackSections : TracksAndOrderedSections)
	{
		if (UMovieSceneTrack* Track = TrackSections.Key)
		{
			UMovieSceneNameableTrack* NameableTrack = Cast<UMovieSceneNameableTrack>(Track);
			int32 Index = 0;
			for (const TPair<UAnimLayer*, UMovieSceneSection*>& Pair : TrackSections.Value)
			{
				Pair.Value->SetOverlapPriority(Index);
				Pair.Value->SetRowIndex(Index);
				if (NameableTrack)
				{
					NameableTrack->SetTrackRowDisplayName(Pair.Key->GetName(), Index);
				}
				Track->SetRowEvalDisabled(!Pair.Key->State.bActive, Index);
				++Index;
			}
			Track->UpdateEasing();
		}
...
void UAnimLayers::SetAnimLayerIndices(bool bResetSectionOrders)
{
	for (int32 Index = 0; Index < AnimLayers.Num(); ++Index)
	{
		if (UAnimLayer* AnimLayer = AnimLayers[Index].Get())
		{
			AnimLayer->Modify();
			AnimLayer->LayerIndex = Index;
		}
	}
	if (bResetSectionOrders)
	{
		//go through and cache out the sections in order, we need to do this since there may be missing sections
		//since every layer doesn't contain every section 
		
		TMap<UMovieSceneTrack*, TArray<TPair<UAnimLayer*,UMovieSceneSection*>>> TracksAndOrderedSections;
		GetTracksAndSections(TracksAndOrderedSections);
		//now go through and set up tracks
		TArray<UMovieSceneSection*> Sections;
		int32 Index = 0;
		for (TPair<UMovieSceneTrack*, TArray<TPair<UAnimLayer*, UMovieSceneSection*>>>& TrackSections : TracksAndOrderedSections)
		{
			if (UMovieSceneTrack* Track = TrackSections.Key)
			{
				Track->Modify();
				...
				Sections.SetNum(TrackSections.Value.Num());
				Index = 0;
				for (TPair<UAnimLayer*, UMovieSceneSection*>& Pair : TrackSections.Value)
				{
					Sections[Index++] = Pair.Value;
				}
				MovieSceneHelpers::ResetSectionOrder(Track, Sections);
			}
		}
		SetTrackRowForSections(TracksAndOrderedSections);
	}
...
bool UAnimLayers::ReorderAnimLayers(ISequencer* SequencerPtr, int32 OriginalIndex, int32 NewIndex)
{
	//can't move over  or move base index
	if (NewIndex == 0 || OriginalIndex == 0 || SequencerPtr == nullptr)
	{
		return false;
	}
	if(OriginalIndex != NewIndex && AnimLayers.IsValidIndex(OriginalIndex) && AnimLayers.IsValidIndex(NewIndex))
	{ 
		const FScopedTransaction Transaction(LOCTEXT("ReorderLayers_Transaction", "Reorder Layers"), !GIsTransacting);
		Modify();
		UAnimLayer* AnimLayer = AnimLayers[OriginalIndex];
		AnimLayers.RemoveAt(OriginalIndex);
		AnimLayers.Insert(AnimLayer, NewIndex);
		//reset layer indices need to go through and set every
		SetAnimLayerIndices(true /*bResetSectionOrders*/);
		SequencerPtr->NotifyMovieSceneDataChanged(EMovieSceneDataChangeType::MovieSceneStructureItemRemoved);
		AnimLayerListChangedBroadcast();
		return true;
	}
...
int32 UAnimLayers::DuplicateAnimLayer(ISequencer* SequencerPtr, int32 Index)
{
	...
	if (Index >= 1 && Index < AnimLayers.Num()) //don't duplicate base
	{
		if (UAnimLayer* ExistingAnimLayer = AnimLayers[Index])
		{
			...
			{
				UE_LOGF(LogControlRig, Error, "Anim Layers: Can not duplicate empty layer");
				return INDEX_NONE;
			}
							...
							if (SectionIndex != INDEX_NONE)
							{
								...
								if (bHasSelected == false)
								{
									if (UMovieSceneControlRigParameterSection* CRSection = Cast<UMovieSceneControlRigParameterSection>(SectionItem.Section.Get()))
									{
										...
										{
											//passthrough and base is separate case we call edit mode function since it does lots of special case stuff
...
bool UAnimLayers::MergeAnimLayers(TSharedPtr<ISequencer>& InSequencerPtr, const TArray<int32>& Indices, const FMergeAnimLayerSettings* InSettings)
{
	...
	const UE::ControlRig::FSaveScopedControlRigSelection SaveScopedControlRigSelection(InSequencerPtr);
	...
	const FAnimLayersScopedSelection ScopedSelection(*this);
...
void UAnimLayers::SetUpBaseLayerSections()
{
	if (AnimLayers.Num() > 0)
	{
		if (UAnimLayer* BaseAnimLayer = AnimLayers[0])
		{
			...
			// Track controls in the base layer for any control present in the absolute section
			// but not covered by any non-base layer — so the base layer is visible for those controls.
			for (TTuple<TWeakObjectPtr<UObject>, FAnimLayerItem>& Item : BaseAnimLayer->AnimLayerItems)
			{
				if (!Item.Key.IsValid() || 
					!Item.Key->IsA<UControlRig>())
				{
					continue;
				}

				for (FAnimLayerSectionItem& BaseSectionItem : Item.Value.SectionItems)
				{
					if (!BaseSectionItem.Section.IsValid())
					{
						continue;
					}
					
					UMovieSceneControlRigParameterSection* AbsSection = Cast<UMovieSceneControlRigParameterSection>(BaseSectionItem.Section.Get());
					if (!AbsSection)
					{
						continue;
					}

					for (const TPair<FName, FChannelMapInfo>& ChannelPair : AbsSection->ControlChannelMap)
					{
						if (!BaseSectionItem.AnimLayerSet.Names.Contains(ChannelPair.Key))
						{
							FAnimLayerPropertyAndChannels Channels;
							Channels.Name = ChannelPair.Key;
							Channels.Channels = (uint32)EControlRigContextChannelToKey::AllTransform;

							BaseSectionItem.AnimLayerSet.Names.Add(ChannelPair.Key, Channels);
						}
					}
				}
			}
...
void UAnimLayers::SetUpControlRigSection(UMovieSceneControlRigParameterSection* ParameterSection, TArray<FName>& ControlNames)
{
	if (ParameterSection)
	{
		...
		if (ParameterSection && ControlRig)
		{
			...
			{
				ParameterSection->SetControlNameMask(RigName, true, bReconstructChannelProxy);
			}
	...
	for (int32 Index = 0; Index < BaseChannels.Num(); ++Index)
	{
		...
		for (FKeyInterval& KeyInterval : KeyIntervals)
		{
			...
			for (TPair<FFrameNumber, double>& TimeValue : PercentageDifferences)
			{
				...
				ValueType ValueTypeValue(CurrentValue);
				AddKeyToChannel(LayerChannel, TimeValue.Key, ValueTypeValue, DefaultInterpolation);
...
void AdjustmentBlendWorldSpace(ISequencer* Sequencer,  TArray<FAnimLayerItem*>& LayersToMute,  FAnimLayerItem& AnimLayerItem)
{
	if (!Sequencer)
	{
		return;
	...
	//set up Base to evaluate from current
	...
	for (FAnimLayerItem* MuteLayerItem : LayersToMute)
	{
		if (MuteLayerItem)
		{
			MuteLayerItem->SetSectionsActive(false);
		}
	}
	...
	AdditiveKeyIndices.SetNum(2);
	AdditiveKeyIndices[0] = 0;
	AdditiveKeyIndices[1] = CurrentFrameTimes.NumFrames - 1;

	AnimLayerItem.SetSectionsActive(true);
	for (FAnimLayerItem* UnMuteLayerItem : LayersToMute)
	{
		if (UnMuteLayerItem)
		{
			UnMuteLayerItem->SetSectionsActive(true);
		}
	}
	FEvalHelpers::CalculateWorldTransforms(World, Sequencer, CurrentFrameTimes,
...
bool UAnimLayers::AdjustmentBlendLayers(ISequencer* InSequencer, int32 LayerIndex)
{
	...
	TArray<FAnimLayerItem*> OtherActiveLayers;
	for (int32 Index = 0; Index < AnimLayers.Num(); ++Index)
	{
		if (Index != 0 && Index != LayerIndex && AnimLayers[Index] != nullptr && AnimLayers[Index]->State.bActive)
		{
			for (TPair<TWeakObjectPtr<UObject>, FAnimLayerItem>& Pair : AnimLayers[Index]->AnimLayerItems)
			{
				if (Pair.Key != nullptr)
				{
					OtherActiveLayers.Add(&Pair.Value);
				}
			}
		}
	}
	...
	for (TPair<TWeakObjectPtr<UObject>, FAnimLayerItem>& Pair : AnimLayer->AnimLayerItems)
	{
		if (Pair.Key != nullptr)
		{
			if (Pair.Key->IsA<UControlRig>())
			{
				...
				{
					AdjustmentBlendWorldSpace(InSequencer, OtherActiveLayers, Pair.Value);
				}
			}
			else
			{
					...
					if (SectionItem.Section.IsValid())
					{
						...
						if (BaseSectionItem && BaseSectionItem->Section.IsValid())
						{
							if (SectionItem.Section->IsActive())//active sections merge them
							{
								...
								if (BaseDoubleChannels.Num() > 0)
								{
									AdjustmentBlend<FMovieSceneDoubleChannel, FMovieSceneDoubleValue>(SectionItem.Section.Get(), BaseDoubleChannels, LayerDoubleChannels, InSequencer);
								}
								else if (BaseFloatChannels.Num() > 0)
								{
									AdjustmentBlend<FMovieSceneFloatChannel, FMovieSceneFloatValue>(SectionItem.Section.Get(), BaseFloatChannels, LayerFloatChannels, InSequencer);
								}
							}
						}
					}
