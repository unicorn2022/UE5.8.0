...
#include "MovieSceneScriptingConstraint.generated.h"
...
/**
* Exposes a Constraint Channel key to Python/Blueprints.
* Stores a reference to the data so changes to this class are forwarded onto the underlying data structures.
*/
UCLASS(BlueprintType)
class UMovieSceneScriptingConstraintChannelKey : public UMovieSceneScriptingKey, public TMovieSceneScriptingKey<FMovieSceneConstraintChannel, bool>
{
	GENERATED_BODY()
public:
	/**
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Get Time (Constraint Key)"))
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Set Time (Constraint Key)"))
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Get Value (Constraint Key)"))
	bool GetValue() const
	{
		bool Value = GetValueFromChannel(KeyHandle);
		return Value;
	}

	/**
	* Sets the value for this key, reflecting it in the owning channel.
	* @param InNewValue	The constraint key
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Set Value (Constraint Key)"))
...
UCLASS(BlueprintType)
class UMovieSceneScriptingConstraintChannel : public UMovieSceneScriptingChannel
{
	GENERATED_BODY()

	using Impl = TMovieSceneScriptingChannel<FMovieSceneConstraintChannel, UMovieSceneScriptingConstraintChannelKey, bool>;
...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Add Key (Constraint Key)"))
	UMovieSceneScriptingConstraintChannelKey* AddKey(const FFrameNumber InTime, bool NewValue, float SubFrame = 0.f, EMovieSceneTimeUnit TimeUnit = EMovieSceneTimeUnit::DisplayRate)
	{
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Remove Key (Constraint Key)"))
	...
	* @return	An array of UMovieSceneScriptingConstraintChannelKeys contained by this channel.
	*			Returns all keys even if clipped by the owning section's boundaries or outside of the current sequence play range.
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Get Keys (Constraint Key)"))
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Get Keys By Index (Constraint Key)"))
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Evaluate Keys (Constraint Key)"))
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Set Default (Constraint Key)"))
	void SetDefault(bool InDefaultValue)
	{
		bool ReferenceKey = bool(InDefaultValue);
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Get Default (Constraint Key)"))
	bool GetDefault() const
	{
		TOptional<bool> DefaultValue = Impl::GetDefaultFromChannel(ChannelHandle);
		return DefaultValue.IsSet() ? DefaultValue.GetValue() : bool();
	}

	/**
	* Remove this channel's default value causing the channel to have no effect where no keys are present
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Remove Default (Constraint Key)"))
	void RemoveDefault()
	{
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Has Default (Constraint Key)"))
	 ...
	 *
	 */
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Transform (Constraint Key)"))
	...
	/**
	* @return Get the tickable constraint associated with this channel
	*/
	UFUNCTION(BlueprintCallable, Category = "Sequencer|Keys", meta = (DisplayName = "Get Constraint (Constraint Channel)"))
	UTickableConstraint* GetConstraint()
	{
		if(IMovieSceneConstrainedSection* Section = Cast<IMovieSceneConstrainedSection>(OwningSection.Get()))
		{
			FMovieSceneConstraintChannel* Channel = ChannelHandle.Get();
			if (Channel)
			{
				return Section->FindConstraint(Channel);
			}
		}
		return nullptr;
	}
