...
void FControlRigAnimNodeEventNameDetails::UpdateEntryNameList()
{
	...
	{
		FControlRigAssetStrongReference RigSource = AnimNodeBeingCustomized->GetControlRigAssetReference();
		if(RigSource.IsValid())
		{
			TArray<FName> SupportedEvents = RigSource.GetSupportedEvents();
