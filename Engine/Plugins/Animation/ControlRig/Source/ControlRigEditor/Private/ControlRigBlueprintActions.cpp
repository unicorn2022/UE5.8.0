...
UControlRigBlueprint* FControlRigBlueprintActions::CreateControlRigFromSkeletalMeshOrSkeleton(UObject* InSelectedObject, const bool bModularRig)
{
	...
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
	const TOptional<FString> DefaultControlRigName = AssetToolsModule.Get().GetDefaultAssetNameForClass(UControlRigBlueprint::StaticClass(), nullptr, InSelectedObject);

	FString ControlRigName = DefaultControlRigName.Get(FString::Printf(TEXT("%s_CtrlRig"), *InSelectedObject->GetName()));
	...
	GEditor->GetTimerManager()->SetTimerForNextTick([InObject, InAsset]()
	{
		...
		if (WeakSequencer.IsValid())
		{
			...
			for (FGuid ActorTrackGuid : ActorTracks)
			{
				...
				if (!ControlRig || ControlRig->GetAssetReference() != ControlRigClass)
				{
					...
					ControlRig = ControlRigClass.CreateInstance(Track, FName(*ObjectName), RF_Transactional);
