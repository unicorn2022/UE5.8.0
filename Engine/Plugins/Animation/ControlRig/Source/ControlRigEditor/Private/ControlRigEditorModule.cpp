...
#include "Sequencer/SequencerSimpleViewExtension.h"
...
void FControlRigEditorModule::StartupModule()
{
	...
	PropertiesToUnregisterOnShutdown.Add(FControlRigAssetSoftReference::StaticStruct()->GetFName());
	PropertyEditorModule.RegisterCustomPropertyTypeLayout(PropertiesToUnregisterOnShutdown.Last(), FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FControlRigAssetReferenceDetails::MakeInstance));
	
	PropertiesToUnregisterOnShutdown.Add(FControlRigAssetStrongReference::StaticStruct()->GetFName());
	PropertyEditorModule.RegisterCustomPropertyTypeLayout(PropertiesToUnregisterOnShutdown.Last(), FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FControlRigAssetReferenceDetails::MakeInstance));
	...
	RegisterAssetTypeAction(MakeShareable(new FControlRigAssetActions()));
	RegisterAssetTypeAction(MakeShareable(new FControlRigEditorAssetActions()));
	...
	FControlRigAssetActions::ExtendSketalMeshToolMenu();
	...
	UActorFactorySkeletalMesh::RegisterDelegatesForAssetClass(
		UControlRigRuntimeAsset::StaticClass(),
		FGetSkeletalMeshFromAssetDelegate::CreateStatic(&FControlRigAssetActions::GetSkeletalMeshFromControlRigAsset),
		FPostSkeletalMeshActorSpawnedDelegate::CreateStatic(&FControlRigAssetActions::PostSpawningSkeletalMeshActor)
	);

	UThumbnailManager::Get().RegisterCustomRenderer(UControlRigBlueprint::StaticClass(), UControlRigThumbnailRenderer::StaticClass());
	UThumbnailManager::Get().RegisterCustomRenderer(UControlRigRuntimeAsset::StaticClass(), UControlRigThumbnailRenderer::StaticClass());
	...
	auto RegisterSpaceChannel = [](UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType, TArray<UMovieSceneScriptingChannel*>& OutChannels) -> bool
		{
			if (ChannelType == UMovieSceneScriptingChannel::StaticClass() ||
				ChannelType == UMovieSceneScriptingSpaceChannel::StaticClass())
			{
				FMovieSceneChannelProxy& ChannelProxy = Section->GetChannelProxy();
				TWeakObjectPtr<UMovieSceneSequence> Sequence = Section->GetTypedOuter<UMovieSceneSequence>();
				GetScriptingChannelsForChannel<FMovieSceneControlRigSpaceChannel, UMovieSceneScriptingSpaceChannel>(ChannelProxy, Sequence, Section, OutChannels);
				return true;
			}
			return false;
		};
	auto RegisterConstraintChannel = [](UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType, TArray<UMovieSceneScriptingChannel*>& OutChannels)-> bool
		{
			if (ChannelType == UMovieSceneScriptingChannel::StaticClass() ||
				ChannelType == UMovieSceneScriptingConstraintChannel::StaticClass())
			{
				FMovieSceneChannelProxy& ChannelProxy = Section->GetChannelProxy();
				TWeakObjectPtr<UMovieSceneSequence> Sequence = Section->GetTypedOuter<UMovieSceneSequence>();
				GetScriptingChannelsForChannel<FMovieSceneConstraintChannel, UMovieSceneScriptingConstraintChannel>(ChannelProxy, Sequence, Section, OutChannels);
				return true;
			}
			return false;
		};

	SpaceCustomChannelGuid = UMovieSceneSectionExtensions::SetGetCustomChannelsFunction(RegisterSpaceChannel);
	ConstraintCustomChannelGuid = UMovieSceneSectionExtensions::SetGetCustomChannelsFunction(RegisterConstraintChannel);
		...
		if (FToolMenuSection* Section = Menu->FindSection("CommonAssetActions"))
		{
			Section->AddDynamicEntry("CreateControlRigAsset", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
			{
				...
				if (Context)
				{
					...
					if (SelectedAssetData.GetClass() != UControlRigBlueprint::StaticClass())
					{
						// We aren't dealing with a URigVMExampleBlueprint 
						return;
					}

					
					FSoftObjectPath SoftObjectPath = SelectedAssetData.GetSoftObjectPath();
					InSection.AddMenuEntry("CreateAsset", LOCTEXT("CreateAsset", "Create Control Rig asset"), LOCTEXT("CreateAsset_ToolTip", "Create a Control Rig blueprint-independent asset from this blueprint"), FSlateIcon(FRigVMEditorStyle::Get().GetStyleSetName(), "RigVM", "RigVM.Unit"), FExecuteAction::CreateLambda([SoftObjectPath]()
						{
							...
							if (UControlRigBlueprint* Blueprint = Cast<UControlRigBlueprint>(SelectedObject))
							{
								FControlRigAssetActions::CreateNewControlRigAsset(Blueprint);
							}
						}));
				}
			}));
	...
	UE::ControlRig::SimpleView::FSequencerSimpleViewExtension::Register();
}

void FControlRigEditorModule::ShutdownModule()
{
	UE::ControlRig::SimpleView::FSequencerSimpleViewExtension::Unregister();
	...
	UMovieSceneSectionExtensions::RemoveGetCustomChannelsFunction(SpaceCustomChannelGuid);
	UMovieSceneSectionExtensions::RemoveGetCustomChannelsFunction(ConstraintCustomChannelGuid);
}
...
UClass* FControlRigEditorModule::GetRigVMAssetClass() const
{
	if (CVarRigVMBlueprintIndependentAssets->GetBool())
	{
		return UControlRigRuntimeAsset::StaticClass();
	}
...
UClass* FControlRigEditorModule::GetRigVMLegacyBlueprintClass() const
{
	return UControlRigBlueprint::StaticClass();
}
						...
						FToolMenuEntry& MenuEntry = Section.AddMenuEntry(
							*Target.Name,
							...
							FUIAction(FExecuteAction::CreateLambda([ControlRigBlueprint, UnitNode, Target]() {
								...
								auto& ASTSettings = ControlRigBlueprint->VMCompileSettings.ASTSettings;
...
TSharedRef< SWidget > FControlRigEditorModule::GenerateAnimationMenu(TWeakPtr<IAnimationEditor> InAnimationEditor)
{
	...
	if(InAnimationEditor.IsValid())
	{
		...
		if (Skeleton && SkeletalMesh && AnimSequence)
		{
			...
			FUIAction ModifyLinkSettingsAction(
				FExecuteAction::CreateStatic(&FControlRigEditorModule::ModifyLevelSequenceLinkSettings, AnimSequence),
				FCanExecuteAction::CreateLambda([AnimSequence]()
					{
						if (AnimSequence)
						{
			...
			MenuBuilder.AddMenuEntry(LOCTEXT("ModifyExistingLinkSettings", "Modify Existing Link Settings"),
				FText(), FSlateIcon(), ModifyLinkSettingsAction, NAME_None, EUserInterfaceActionType::Button);
...
void LogBakeToControlRigError(const FString& InError)
{
	...
	UE_LOGF(LogControlRigEditor, Error, "%ls", *Notification.ToString());
				...
				(bool bKeyReduce, float KeyReduceTolerance, FFrameRate BakeFrameRate, bool bResetControls)
				{
					...
					if (WeakSequencer.IsValid())
					{
						...
						if (IInterface_AssetUserData* AssetUserDataInterface = Cast< IInterface_AssetUserData >(LevelSequence))
						{
							...
							LevelAnimLinkItem.bSetRetargetSourceAsset = false;
							...
							LevelAnimLinkItem.bSkipCurvesWithZeroValue = true;
							LevelAnimLinkItem.bRemoveExcludedCurves = false;
...
void FControlRigEditorModule::ModifyLevelSequenceLinkSettings(UAnimSequence* AnimSequence)
{
	if (IInterface_AssetUserData* AnimAssetUserData = Cast<IInterface_AssetUserData>(AnimSequence))
	{
		UAnimSequenceLevelSequenceLink* AnimLevelLink = AnimAssetUserData->GetAssetUserData<UAnimSequenceLevelSequenceLink>();
		...
		IInterface_AssetUserData* LSUserData = Cast<IInterface_AssetUserData>(LevelSequence);
		if (!LSUserData)
		{
			return;
		}
		ULevelSequenceAnimSequenceLink* LevelAnimLink = LSUserData->GetAssetUserData<ULevelSequenceAnimSequenceLink>();
		if (!LevelAnimLink)
		{
			return;
		}

		FLevelSequenceAnimSequenceLinkItem* FoundItem = nullptr;
		for (FLevelSequenceAnimSequenceLinkItem& Item : LevelAnimLink->AnimSequenceLinks)
		{
			if (Item.ResolveAnimSequence() == AnimSequence)
			{
				FoundItem = &Item;
				break;
			}
		...
		TWeakObjectPtr<UAnimSequence> WeakAnimSeq = AnimSequence;
		TWeakObjectPtr<ULevelSequence> WeakLevelSeq = LevelSequence;
		TWeakObjectPtr<ULevelSequenceAnimSequenceLink> WeakLevelAnimLink = LevelAnimLink;

		FModifyLevelSequenceLinkDelegate Delegate;
		Delegate.BindLambda([WeakAnimSeq, WeakLevelSeq, WeakLevelAnimLink](const FLevelSequenceAnimSequenceLinkItem& EditedItem)
		{
			if (!WeakAnimSeq.IsValid() || !WeakLevelSeq.IsValid() || !WeakLevelAnimLink.IsValid())
			{
				return;
			}
			const FScopedTransaction Transaction(LOCTEXT("ModifyLevelSequenceLinkSettings", "Modify Level Sequence Link Settings"));
			WeakLevelSeq->Modify();
			for (FLevelSequenceAnimSequenceLinkItem& Item : WeakLevelAnimLink->AnimSequenceLinks)
			{
				if (Item.ResolveAnimSequence() == WeakAnimSeq.Get())
				{
					const FGuid SavedGuid = Item.SkelTrackGuid;
					const FSoftObjectPath SavedPath = Item.PathToAnimSequence;
					Item = EditedItem;
					Item.SkelTrackGuid = SavedGuid;
					Item.PathToAnimSequence = SavedPath;
					break;
				}
			}
			WeakLevelSeq->MarkPackageDirty();
		});

		FSlateApplication::Get().DismissAllMenus();
		ModifyLevelSequenceLinkDialog::ShowDialog(*FoundItem, MoveTemp(Delegate));
	}
}
...
TSharedRef<IControlRigEditor> FControlRigEditorModule::CreateControlRigEditor(const EToolkitMode::Type Mode, const TSharedPtr<IToolkitHost>& InitToolkitHost, UControlRigRuntimeAsset* InAsset)
{
	TSharedRef< FControlRigEditor > NewControlRigEditor(new FControlRigEditor());
	NewControlRigEditor->InitRigVMEditor(Mode, InitToolkitHost, InAsset->GetEditorOnlyData());
	return NewControlRigEditor;
}
