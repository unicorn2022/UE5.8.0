...
{
	if (ControlRig == nullptr || ControlRig->GetHierarchy() == nullptr)
	{
		UE_LOGF(LogControlRig, Error, "No Control Rig or Hierarchy");
		return false;
	}
	if (Frames.Num() > ParentTransforms.Num())
	{
		UE_LOGF(LogControlRig, Error, "Number of Frames %d to Snap greater  than Parent Frames %d", Frames.Num(), ParentTransforms.Num());
		return false;
	}
	FRigTransformElement* ControlElement = ControlRig->FindControl(ControlName);
	...
	if (UMovieScene* MovieScene = MovieSceneSequence->GetMovieScene())
	{
		...
		const float CachingDeltaTime = DisplayRate > 0.f ? 1.f / DisplayRate : 1.f / 30.f;
		...
		for (int32 Index = 0; Index < Frames.Num(); ++Index)
		{
			...
			if ((Index != 0) || (CurrentFrame.IsSet() == false || CurrentFrame.GetValue() != FrameNumber))
			{
				...
				{
					const UE::Anim::FEvaluationForCachingScope CachingScope(CachingDeltaTime);
			...
			{
				ControlRig->SetDeltaTime(CachingDeltaTime);
				ControlRig->Evaluate_AnyThread();
			}
			OutTransforms[Index] = ControlRig->GetHierarchy()->GetTransform(ControlElement, ERigTransformType::CurrentGlobal) * ParentTransforms[Index];
		}
	}
	return true;
}
	...
	const TArray<FFrameNumber>& Frames, TArray<FTransform>& OutParentWorldTransforms)
{
	...
	if (UMovieScene* MovieScene = MovieSceneSequence ? MovieSceneSequence->GetMovieScene() : nullptr)
	{
		...
		for (const FControlRigForWorldTransforms& ControlRigAndSelection : ParentToSnap.ControlRigs)
		{
			if (UControlRig* ControlRig = ControlRigAndSelection.ControlRig.Get())
			{
				if (const TSharedPtr<IControlRigObjectBinding> ObjectBinding = ControlRig->GetObjectBinding())
				{
					...
					{
						OutParentWorldTransforms.Init(FTransform::Identity, Frames.Num());

						const FName& ControlName = ControlRigAndSelection.ControlNames[0];
						...
						MovieSceneToolHelpers::RunSequencerTask(Sequencer, Actor->GetWorld(), Frames, 
						[&OutParentWorldTransforms, &Provider, ControlName](int32 Index)
						{
							if (OutParentWorldTransforms.IsValidIndex(Index))
							{
								constexpr bool bUpdateAnimation = true;
								OutParentWorldTransforms[Index] = Provider.GetTransform(ControlName, bUpdateAnimation);
							}
						});
						return true;
					}
	...
	const FControlRigSnapperSelection& ParentToSnap, const UControlRigSnapSettings* SnapSettings)
{
	...
	if (InSequencer.IsValid() && InSequencer.Pin()->GetFocusedMovieSceneSequence() && ActorToSnap.IsValid())
	{
		...
		else
		{
			...
			FFrameNumber LocalStartFrame = StartFrame;
			FFrameNumber LocalEndFrame = EndFrame;
			if (EndFrame < StartFrame)
			{
				Swap<FFrameNumber>(LocalStartFrame, LocalEndFrame);
			}
			GetTransformFrames(Sequencer, ParentToSnap, LocalStartFrame, LocalEndFrame, FrameMap);
			GetTransformFrames(Sequencer, ActorToSnap, LocalStartFrame, LocalEndFrame, FrameMap);
			FrameMap.GenerateKeyArray(Frames);
			if (LocalStartFrame == EndFrame) //reverse it
			{
				Algo::Reverse(Frames);
			}
		...
		if (Frames.Num() == 0)
		{
			UE_LOGF(LogControlRig, Error, "No Frames To Snap, probably no keys within time range");
			return false;
		}
		...
		{
			UE_LOGF(LogControlRig, Error, "Number of Frames %d to Snap different than Parent Frames %d", Frames.Num(),WorldTransformToSnap.Num());
				...
				if (TSharedPtr<IControlRigObjectBinding> ObjectBinding = ControlRig->GetObjectBinding())
				{
					...
					for (const FName& Name : ControlRigAndSelection.ControlNames)
					{
						...
						for (int32 Index = 0; Index < WorldTransformToSnap.Num(); ++Index)
						{
							...
							{
								UE::AIE::FSetTransformHelpers::SetControlGlobalTransform(GlobalTransform, ControlRig, ControlElement, Context);
							}
