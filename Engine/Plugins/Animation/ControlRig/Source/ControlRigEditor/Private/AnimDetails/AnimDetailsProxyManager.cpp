...
#include "AnimDetails/Customizations/AnimDetailsPropertyRowExtensions.h"
...
UAnimDetailsProxyManager::UAnimDetailsProxyManager()
{
	if (!IsTemplate())
	{
		...
		UE::ControlRigEditor::FAnimDetailsPropertyRowExtensions::Get().RegisterRowExtensions();
	}
}

void UAnimDetailsProxyManager::BeginDestroy()
{
	Super::BeginDestroy();

	if (!IsTemplate())
	{
		UE::ControlRigEditor::FAnimDetailsPropertyRowExtensions::Get().UnregisterRowExtensions();
	}
}
...
void UAnimDetailsProxyManager::SetupBingings()
{
	...
	{
		if (!Sequencer->OnMovieSceneBindingsChanged().IsBoundToObject(this))
		{
			Sequencer->OnMovieSceneBindingsChanged().AddUObject(this, &UAnimDetailsProxyManager::RequestUpdateProxies);
		}
...
{
	FControlRigEditMode* EditMode = static_cast<FControlRigEditMode*>(GLevelEditorModeTools().GetActiveMode(FControlRigEditMode::ModeName));
	const bool bInNotChangingSelection = AnimDetailsSelection && !AnimDetailsSelection->IsAnimLayersChangingSelection();
	if (EditMode && !EditMode->AreEditingControlRigDirectly() && bInNotChangingSelection)
	{
		RequestUpdateProxies();
	}
}
...
void UAnimDetailsProxyManager::ForceUpdateProxies()
{
	...
	FTSTicker::RemoveTicker(RequestUpdateProxiesTickerHandle);
	...
	// Update control rig proxies
	{
		if (!EditMode->AreEditingControlRigDirectly())
		{
			...
			// Add grouped children to selected controls
			for (TPair <UControlRig*, TArray<FRigElementKey>>& SelectedKeys : ControlRigToSelectedKeysMap)
			{
				if (SelectedKeys.Key != nullptr)
				{
					if (URigHierarchy* Hierarchy = SelectedKeys.Key->GetHierarchy())
					{
						for (int32 Index = 0; Index < SelectedKeys.Value.Num(); ++Index)
						{
							if (FRigControlElement* ParentControl = Hierarchy->Find<FRigControlElement>(SelectedKeys.Value[Index]))
							{
								TArrayView<FRigBaseElement*> Children = Hierarchy->GetChildren(ParentControl);
								for (FRigBaseElement* BaseElement : Children)
								{
									if (FRigControlElement* Control = Cast<FRigControlElement>(BaseElement))
									{
										//if groupable and not contained/selected add it to the array
										FRigElementKey ChildKey = Control->GetKey();
										if (SelectedKeys.Value.Contains(ChildKey) == false && Hierarchy->ShouldBeGrouped(Control))
										{
											Index = SelectedKeys.Value.Insert(ChildKey, Index);
										}
									}
								}
								//now also check the WHOLE hierarchy to see if it's a hosted or whatever channel
								//similar to what's done in the track editor

								Hierarchy->ForEach<FRigControlElement>([Hierarchy, ParentControl, &SelectedKeys,&Index](FRigControlElement* OtherControlElement) -> bool
								{
									FRigElementKey ChildKey = OtherControlElement->GetKey();
									if (SelectedKeys.Value.Contains(ChildKey) == false && FControlRigParameterTrackEditor::ShouldBeSelected(Hierarchy, ParentControl, OtherControlElement))
									{
										Index = SelectedKeys.Value.Insert(ChildKey, Index);
									}
									return true;
			...
			for (const TTuple<UControlRig*, TArray<FRigElementKey>>& ControlRigToSelectedKeysPair : ControlRigToSelectedKeysMap)
			{
				...
				// In order of the hierarchy
				TArray<FRigControlElement*> OrderedControlElements;
				ControlRig->GetControlsInOrder(OrderedControlElements);

				Algo::SortBy(ControlRigToSelectedKeysPair.Value,
					[&OrderedControlElements](const FRigElementKey& ElementKey)
					{
						return OrderedControlElements.IndexOfByPredicate(
							[&ElementKey](const FRigControlElement* ControlElement)
							{
								return
									ControlElement &&
									ControlElement->GetKey() == ElementKey;
							});
					});
	...
	{
		// Just care about the outliner selection
...
TMap<FGuid, TArray<UMovieScenePropertyTrack*>> UAnimDetailsProxyManager::GetPropertyTracks(const TSharedRef<ISequencer>& Sequencer, const TArray<FGuid>& ObjectGuids) const
{
	...
	const TSharedPtr<FSequencerEditorViewModel> SequencerViewModel = Sequencer->GetViewModel();
	const FViewModelPtr RootModel = SequencerViewModel.IsValid() ? SequencerViewModel->GetRootModel() : nullptr;
	const FTrackModelStorageExtension* TrackStorage = RootModel.IsValid() ? RootModel->CastDynamic<FTrackModelStorageExtension>() : nullptr;
	...
	const auto FindPropertyTracks = [&](const UMovieScene* MovieScene)
		{
			if (!MovieScene)
			{
				return;
			...
			for (const FGuid& ObjectGuid : ObjectGuids)
			{
				...
				// Sort in sequencer order
				for (TTuple<FGuid, TArray<UMovieScenePropertyTrack*>>& ObjectGuidToPropertyTracksPair : ObjectGuidToPropertyTracksMap)
				{
					ObjectGuidToPropertyTracksPair.Value.Sort(
						[&](UMovieScenePropertyTrack& TrackA, UMovieScenePropertyTrack& TrackB)
						{
							const TSharedPtr<FTrackModel> TrackModelA = TrackStorage->FindModelForTrack(&TrackA);
							const TSharedPtr<FTrackModel> TrackModelB = TrackStorage->FindModelForTrack(&TrackB);

							if (TrackModelA && TrackModelB)
							{
								return FSortingKey::CompareCustomOrderFirst(TrackModelA->GetSortingKey(), TrackModelB->GetSortingKey());
							}
