...
namespace UE::ControlRigEditor
{
		...
		IPropertyTypeCustomizationUtils& InStructCustomizationUtils)
	{
		...
		const bool bHasMultipleChildHandles = SortedChildHandles.Num() > 1;

		// Details view deferred actions execute before RestoreAllExpandedItems(), so use a one-shot
		// next-frame callback to collapse after the saved expansion state has been restored.
		if (bHasMultipleChildHandles
			&& !GetDefault<UAnimDetailsSettings>()->bExpandTransformProperties
			&& ShouldUseTransformPropertyExpansion(InStructPropertyHandle))
		{
			const TWeakPtr<IPropertyHandle> WeakStructPropertyHandle = InStructPropertyHandle;
			FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
				[WeakStructPropertyHandle](const float InDeltaTime)
				{
					const TSharedPtr<IPropertyHandle> StructPropertyHandle = WeakStructPropertyHandle.Pin();
					if (StructPropertyHandle.IsValid()
						&& StructPropertyHandle->IsValidHandle()
						&& StructPropertyHandle->IsExpanded())
					{
						StructPropertyHandle->SetExpanded(false);
					}
					return false;
				}));
		}
		...
		for (int32 ChildIndex = 0; ChildIndex < SortedChildHandles.Num(); ++ChildIndex)
		{
			...
			const FText PropertyDisplayText = [bHasMultipleChildHandles, FirstProxy, &ChildHandle, this]()
				{
	...
	void FAnimDetailsValueCustomization::OnResetToDefaultClicked(const TWeakPtr<IPropertyHandle> WeakPropertyHandle)
	{
		...
		// Reset all then Propagate all values so all is evaluated in the new state
		...
		for (UObject* OuterObject : OuterObjects)
		{
			if (UAnimDetailsProxyBase* Proxy = Cast<UAnimDetailsProxyBase>(OuterObject))
			{
				Proxy->PropagateValues();
			}
		}
	}

	bool FAnimDetailsValueCustomization::ShouldUseTransformPropertyExpansion(const TSharedRef<IPropertyHandle>& InStructPropertyHandle)
	{
		const FStructProperty* const StructProperty = CastField<FStructProperty>(InStructPropertyHandle->GetProperty());
		if (StructProperty && StructProperty->Struct)
		{
			return StructProperty->Struct == FAnimDetailsLocation::StaticStruct()
				|| StructProperty->Struct == FAnimDetailsRotation::StaticStruct()
				|| StructProperty->Struct == FAnimDetailsScale::StaticStruct();
		}
		return false;
	}
