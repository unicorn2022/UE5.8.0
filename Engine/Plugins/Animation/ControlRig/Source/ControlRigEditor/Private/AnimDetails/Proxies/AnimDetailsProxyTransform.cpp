...
void UAnimDetailsProxyTransform::SetSequencerBindingValueFromCurrent(const FRigControlModifiedContext& Context)
{
	...
	if (SceneComponentThatChanged)
	{
		...
		if (Context.SetKey != EControlRigSetKey::Never)
		{
			...
			if (EnumHasAnyFlags(ChannelsToKey, EControlRigContextChannelToKey::TranslationX))
			{
				StructProperty = CastField<FStructProperty>(FindFProperty<FProperty>(USceneComponent::StaticClass(), USceneComponent::GetRelativeLocationPropertyName()));
			...
			if (EnumHasAnyFlags(ChannelsToKey, EControlRigContextChannelToKey::RotationX))
			{
				StructProperty = CastField<FStructProperty>(FindFProperty<FProperty>(USceneComponent::StaticClass(), USceneComponent::GetRelativeRotationPropertyName()));
			...
			if (EnumHasAnyFlags(ChannelsToKey, EControlRigContextChannelToKey::ScaleX))
			{
				StructProperty = CastField<FStructProperty>(FindFProperty<FProperty>(USceneComponent::StaticClass(), USceneComponent::GetRelativeScale3DPropertyName()));
				AxisProperty = FindFProperty<FDoubleProperty>(TBaseStructure<FVector>::Get(), GET_MEMBER_NAME_CHECKED(FVector, X));
			}
			if (EnumHasAnyFlags(ChannelsToKey, EControlRigContextChannelToKey::ScaleY))
			{
		...
		if (StructProperty && AxisProperty)
		{
			const UE::ControlRigEditor::FAnimDetailsScopedPropertyChangedEvent ScopedPropertyChangedEvent(*BoundObject, *StructProperty, *AxisProperty);
