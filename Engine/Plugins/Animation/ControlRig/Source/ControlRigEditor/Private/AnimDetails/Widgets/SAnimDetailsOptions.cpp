...
namespace UE::ControlRigEditor
{
	...
	void SAnimDetailsOptions::PopulateMenu(UToolMenu* InMenu)
	{
		...
		FToolMenuSection& OptionsSection = InMenu->AddSection("Options");
		{
			...
			// Expand transform properties option
			{
				const TSharedRef<SWidget> ExpandTransformPropertiesOptionWidget =
					SNew(SBox)
					...
					[
						SNew(SCheckBox)
						.HAlign(HAlign_Right)
						.IsChecked(This, &SAnimDetailsOptions::GetExpandTransformPropertiesCheckState)
						.OnCheckStateChanged(This, &SAnimDetailsOptions::OnExpandTransformPropertiesCheckStateChanged)
					];

				OptionsSection.AddEntry(
					FToolMenuEntry::InitWidget(
						"ExpandTransformPropertiesOptionWidget",
						ExpandTransformPropertiesOptionWidget,
						LOCTEXT("ExpandTransformPropertiesLabel", "Expand Transform Properties"),
						bIndent,
						bSearchable,
						bNoPadding,
						LOCTEXT("ExpandTransformPropertiesTooltip", "When checked, expands Location, Rotation, and Scale properties in anim details"))
					);
			}
	...
	ECheckBoxState SAnimDetailsOptions::GetExpandTransformPropertiesCheckState() const
	{
		return GetDefault<UAnimDetailsSettings>()->bExpandTransformProperties ?
			ECheckBoxState::Checked : ECheckBoxState::Unchecked;
	}

	void SAnimDetailsOptions::OnExpandTransformPropertiesCheckStateChanged(const ECheckBoxState InCheckBoxState)
	{
		UAnimDetailsSettings* const Settings = GetMutableDefault<UAnimDetailsSettings>();
		Settings->bExpandTransformProperties = InCheckBoxState == ECheckBoxState::Checked;
		Settings->SaveConfig();
