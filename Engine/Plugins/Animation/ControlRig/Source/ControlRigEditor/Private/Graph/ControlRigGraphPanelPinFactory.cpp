...
{
	return IControlRigEditorAssetInterface::ControlRigPanelNodeFactoryName;
}
			...
			if (URigVMPin* ModelPin = RigNode->GetModelPinFromPinPath(InPin->GetName()))
			{
				...
				else if (CustomWidgetName == TEXT("ComponentName"))
				{
					...
					return SNew(SRigVMGraphPinNameList, InPin)
						...
						{
							if (FControlRigAssetInterfacePtr Asset = IControlRigEditorAssetInterface::GetInterfaceOuter(RigGraph))
							{
