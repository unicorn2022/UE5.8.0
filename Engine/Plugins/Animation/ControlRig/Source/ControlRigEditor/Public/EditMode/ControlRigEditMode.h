...
namespace UE::ControlRigEditor { class FRigFloatingSpacePickerManager; }
...
struct FDetailKeyFrameCacheAndHandler: public IDetailKeyframeHandler
{
	...
	void OnChannelChangedWithTime(const TArray<FFrameNumber>&, const FMovieSceneChannelMetaData*, UMovieSceneSection*);
...
public:
	...
	UE_API virtual bool GetCursor(EMouseCursor::Type& OutCursor) const override;
...
public:
	UE_API virtual bool IsDisplayControlsEnabled() const;
	UE_API virtual void ToggleDisplayControlsEnabled();

	UE_API virtual bool IsControlsAsOverlayEnabled() const;
	UE_API virtual void ToggleControlsAsOverlay();

	UE_API virtual bool IsDisplayNullsEnabled() const;
	UE_API virtual void ToggleDisplayNulls();

	UE_API virtual bool IsDisplaySocketsEnabled() const;
	UE_API virtual void ToggleDisplaySockets();

	UE_API virtual bool IsDisplayAxesOnSelectionEnabled() const;
	UE_API virtual void ToggleDisplayAxesOnSelection();

	UE_API virtual float GetAxesOnSelectionScale() const;
	...
	UE_API void OpenPoseLibraryTab();

	/** Closes the Control Rig Pose Library panel */
	UE_API void ClosePoseLibraryTab();
	...
	UE_API void SelectAnimLayerWeightChannel();
	...
	/** Reset Transforms to Default(which may not be zero if initial values are present*/
	UE_API void ZeroTransforms(bool bSelectionOnly, bool bIncludeChannels = true, UControlRig* InControlRig = nullptr);
	...
	/** Isolate or un-isolate linked anim tracks for every binding in the current Sequencer */
	UE_API void IsolateLinkedAnimTracksForAllBindings(bool bIsolate);

private:

	/** Opens the space picker or a contextual menu with the picker and utility actions if any. */
	void OpenTabMenu();
...
protected:
	...
	TPimplPtr<UE::ControlRigEditor::FRigFloatingSpacePickerManager> FloatingSpacePickerManager;
	
	/** Lazy creation of the picker manager. */
	UE::ControlRigEditor::FRigFloatingSpacePickerManager& GetSpacePickerManager();
	...
	UE_DEPRECATED(5.8, "The ControlRigEditMode-Specific gizmo scale feature has been removed.")
...
private:
	/** Adapts hovered viewport hit proxies to the controls-only selectability policy used for cursor feedback. */
	UE_API bool IsViewportHitProxySelectable(HHitProxy* const InHitProxy) const;

	/** Returns whether an object is selectable under the current Control Rig viewport policy, including controls-only mode. */
	UE_API bool IsViewportObjectSelectable(UObject* const InObject) const;
	...
	FDelegateHandle ViewportSelectabilityDelegateHandle;
...
private:
	...
	UE_API void OnSettingsPropertyChanged(UObject* InObject, FPropertyChangedEvent& PropertyChangedEvent);
...
protected:
	...
	//if selection flag is set, get control rigs with selected controls, if not get all control rigs if there are none selected
	TArray<UControlRig*> GetControlRigsBasedOnSelection(bool bSelectionOnly) const;
...
private:
	...
	FDelegateHandle BuildViewportInteractionsDelegate;
	
	// direct control system for viewport click-and-drag interactions
	UE::AnimationEditMode::FDirectControl DirectControl;
	
	// binds the direct control system to the gizmo
	void BindDirectControl();
	
	// This structure is used to cache the last mouse screen position and last viewport in FControlRigEditMode::MouseMove
	// to prevent synthesized mouse movements from triggering useless hover updates. 
	struct FMouseMoveCache
	{
		bool Update(const FViewport* InViewport, const int32 InX, const int32 InY)
		{
			const bool bUpdated = InViewport != Viewport || InX != X || InY != Y;
			if (bUpdated)
			{
				Viewport = InViewport;
				X = InX;
	...
	FMouseMoveCache MouseMoveCache;
