...
struct  FFrameTimeByIndex
{
	...
	TArray<FFrameNumber> GetFullFrameArray() const
	{
		TArray<FFrameNumber> FrameArray;
		if (NumFrames > 0)
		{
			FrameArray.SetNum(NumFrames);
			for (int32 Index = 0; Index < NumFrames; ++Index)
			{
				FrameArray[Index] = CalculateFrame(Index);
			}
		}
		return FrameArray;
	}
...
struct FSetTransformHelpers
{
	...
	//set a control transform from a local transform, will set a constrainted transform if one is needed, otherwise will just set a normal one.
	static UE_API void SetControlTransform(const FEulerTransform& LocalTransform, UControlRig* ControlRig, FRigControlElement* ControlElement, const FRigControlModifiedContext& Context);
	//set a control transform from a global transform, will call SetControlTransform but handles doing the converstion to local space
	static UE_API void SetControlGlobalTransform(const FTransform& GlobalTransform, UControlRig* ControlRig, FRigControlElement* ControlElement, const FRigControlModifiedContext& Context);
...
struct FControlRigWorldTransformProvider
{
	FControlRigWorldTransformProvider(AActor* InActor,  USceneComponent* InComponent,  UControlRig* InControlRig, float InDeltaTime);

	bool IsValid() const;
	FTransform GetTransform(const FName& InControlName, const bool bUpdateAnimation = false) const;
	FTransform GetComponentTransform(const bool bUpdateAnimation = false) const;
