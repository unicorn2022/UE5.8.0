...
(
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& InParams, UWorld* InWorld)
	{
		...
		if(InParams.Num() == 0)
		{
			...
			UE_LOGF(LogVisualGraphUtils, Error, "Example: VisualGraphUtilsControlRig.TraverseHierarchy /Game/Animation/ControlRig/BasicControls_CtrlRig event=update");
			UE_LOGF(LogVisualGraphUtils, Error, "Provide one path name to an instance of a control rig.");
			UE_LOGF(LogVisualGraphUtils, Error, "Optionally provide the event name (%ls, %ls or %ls).", *BeginExecuteEventName.ToString(), *PrepareForExecuteEventName.ToString(), *InverseExecuteEventName.ToString());
			return;
		}
		...
		for(const FString& ObjectPathName : ObjectPathNames)
		{
			if(UObject* Object = FindObject<UObject>(nullptr, *ObjectPathName, EFindObjectFlags::None))
			{
				...
				{
					UE_LOGF(LogVisualGraphUtils, Error, "Object is not a hierarchy / nor a Control Rig / or short name was provided: '%ls'", *ObjectPathName);
					return;
				}
			}
			else
		...
		{
			UE_LOGF(LogVisualGraphUtils, Error, "No hierarchy found.");
			return;
		}
