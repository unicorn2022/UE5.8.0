...
void UControlRigGraph::InitializeFromAsset(FRigVMEditorAssetInterfacePtr InAsset)
{
...
const TArray<TSharedPtr<FRigVMStringWithTag>> UControlRigGraph::GetSelectedElementsNameList() const
{
	...
	TArray<FRigElementKey> Keys = Blueprint->GetHierarchy()->GetSelectedKeys();
...
{
	FControlRigAssetInterfacePtr RigVMBlueprint = InEdGraph->GetAsset()->GetObject();
	...
	{
		URigHierarchy* Hierarchy = RigVMBlueprint->GetHierarchy();
