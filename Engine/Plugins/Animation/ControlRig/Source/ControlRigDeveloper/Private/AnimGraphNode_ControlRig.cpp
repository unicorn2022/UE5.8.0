	...
	, ControlRigIOMapping(MakeShared<FControlRigIOMapping>(Node.InputMapping, Node.OutputMapping, CustomPinProperties))
{
	...
	ControlRigIOMapping->GetOnGetTargetAssetReferenceDelegate().BindUObject(this, &UAnimGraphNode_ControlRig::GetTargetAssetReference);
}
...
void UAnimGraphNode_ControlRig::CreateCustomPins(TArray<UEdGraphPin*>* OldPins)
{
	...
	FControlRigAssetStrongReference AssetReference = GetTargetAssetReference();
	if (AssetReference.IsValid())
	{
		if (UObject* ReferencedObj = AssetReference.GetEditorAsset())
		{
			if (ReferencedObj->HasAllFlags(RF_NeedPostLoad))
			{
				if(OldPins)
				{
	...
	for (const TPair<FName, FRigVMExternalVariable>& VariablePair : ControlRigIOMapping->GetInputVariables())
	{
		...
		if (CurrentlyExposedNames.Contains(PropertyName))
		{
			...
			{ 
				TArray<FRigVMExternalVariable> ExternalVariables = AssetReference.GetExternalVariables();
				for (FRigVMExternalVariable& ExternalVariable : ExternalVariables)
				{
					const FProperty* Property = ExternalVariable.GetProperty();
					if (Property->GetFName() == PropertyName)
					{
						...
						FBlueprintEditorUtils::PropertyValueToString_Direct(Property, Variable.GetMemory(), DefaultValue, this);
...
void UAnimGraphNode_ControlRig::ValidateAnimNodeDuringCompilation(USkeleton* ForSkeleton, FCompilerResultsLog& MessageLog)
{
	...
	if (AssetReference.IsValid())
	{
		...
		if (FControlRigAssetInterfacePtr EditorAsset = AssetReference.GetEditorAsset())
		{
			Hierarchy = EditorAsset->GetHierarchy();
		}
		if (Hierarchy)
		{
			if (ForSkeleton)
...
void UAnimGraphNode_ControlRig::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	TSharedRef<IPropertyHandle> ClassHandle = DetailBuilder.GetProperty(TEXT("Node.ControlRigAssetReference"), GetClass());
...
void UAnimGraphNode_ControlRig::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (ChangedProperty)
	{
		if (ChangedProperty->GetFName() == GET_MEMBER_NAME_CHECKED(FAnimNode_ControlRig, ControlRigAssetReference))
		{
							...
							if (AssetReference.IsValid())
							{
								...
								for (FRigVMExternalVariable& ExternalVariable : ExternalVariables)
								{
									...
									if (Property->GetFName() == PropertyName)
									{
										// The format the graph pins editor use is different of what property exporter produces, so we use BlueprintEditorUtils to generate the default string
										// Variable.Memory here points to the corresponding property in the Control Rig BP CDO, it was initialized in UAnimGraphNode_ControlRig::RebuildExposedProperties 
										FBlueprintEditorUtils::PropertyValueToString_Direct(Property, RigVMExternalVariable->GetMemory(), DefaultValue, this);
										break;
									}
								}
							}
...
void UAnimGraphNode_ControlRig::PreloadRequiredAssets()
{
	...
	if (UObject* CDO = Node.ControlRigAssetReference.Get())
	{
		PreloadObject(CDO);
		const FModularRigModel& Model = Node.ControlRigAssetReference.GetModularRigModel();
		Model.ForEachModule([this](const FRigModuleReference* Module) -> bool
		{
				// Need to make sure rig modules are loaded before compiling the AnimBP
PRAGMA_DISABLE_DEPRECATION_WARNINGS
				if (!Module->Class_DEPRECATED.IsNull())
				{
					(void)Module->Class_DEPRECATED.LoadSynchronous();
				}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
				else if (Module->ControlRigAssetReference.IsValid())
				{
					constexpr bool bForceLoad = true;
					(void)Module->ControlRigAssetReference.LoadSynchronous(bForceLoad);
				}
			return true;
		});
	}
...
UObject* UAnimGraphNode_ControlRig::GetJumpTargetForDoubleClick() const
{
	const FControlRigAssetStrongReference& RigSource = GetTargetAssetReference();
	if (RigSource.IsValid())
	{
		return RigSource.GetEditorAsset();
	}

	return nullptr;
}

UStruct* UAnimGraphNode_ControlRig::GetTargetSkeletonStruct() const
{
	FControlRigAssetStrongReference RigSource = GetTargetAssetReference();
	if (RigSource.IsValid())
	{
		if (RigSource.GetBlueprintClass())
		{
			UClass* GeneratedClass = RigSource.GetBlueprintClass();
			if(GeneratedClass && GeneratedClass->ClassGeneratedBy)
			{
		...
		if (RigSource.GetControlRigAsset())
		{
			return RigSource.GetVariablesStruct();
		}
	}
...
UStruct* UAnimGraphNode_ControlRig::GetTargetStruct() const
{
	const FControlRigAssetStrongReference& RigSource = GetTargetAssetReference();
	if (RigSource.IsValid())
	{
...
FControlRigAssetStrongReference UAnimGraphNode_ControlRig::GetTargetAssetReference() const
{
	return Node.GetControlRigAssetReference();
}
