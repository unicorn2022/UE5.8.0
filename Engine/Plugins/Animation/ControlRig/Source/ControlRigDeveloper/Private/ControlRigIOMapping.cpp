...
bool FControlRigIOMapping::CreateVariableMappingWidget(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (!GetTargetAssetReference().IsValid())
	{
		return false;
	}
...
void FControlRigIOMapping::RebuildExposedProperties()
{
	...
	const FControlRigAssetStrongReference RigSource = GetTargetAssetReference();
	if (!RigSource.IsValid())
	{
		return;
	}
	
	if (UObject* EditorAsset = RigSource.GetEditorAsset())
	{
		if (EditorAsset->HasAllFlags(RF_NeedPostLoad))
		{
			return;
		}
	}
	...
	GetVariables(RigSource, true, bIgnoreVariablesWithNoMemory, InputVariables);
	// we still update OUtputvariables
	// we don't want output to be exposed
	GetVariables(RigSource, false, bIgnoreVariablesWithNoMemory, OutputVariables);
...
void FControlRigIOMapping::SetIOMapping(bool bInput, const FName& SourceProperty, const FName& TargetCurve)
{
	...
	const FControlRigAssetStrongReference& TargetSource = GetTargetAssetReference();
	if (TargetSource.IsValid())
	{
		TMap<FName, FName>& MappingData = (bInput) ? InputMapping : OutputMapping;
		TArray<FRigVMExternalVariable> ExternalVariables = TargetSource.GetExternalVariables();
		FRigVMExternalVariable* Variable = ExternalVariables.FindByPredicate([SourceProperty](const FRigVMExternalVariable& Variable)
			{
				return Variable.GetName() == SourceProperty;
			});
		if (Variable)
		{
			// if it's valid as of now, we add it
			bool bIsReadOnly = Variable->IsReadOnly();
...
bool FControlRigIOMapping::IsAvailableToMapToCurve(const FName& PropertyName, bool bInput) const
{
	...
	if (Variable)
	{
		return Variable->GetExtendedCPPType() == TEXT("float");
	}
...
{
	const FControlRigAssetStrongReference ControlRigAssetReference = GetTargetAssetReference();
	USkeleton* TargetSkeleton = GetTargetSkeleton();
	return RigControlsData.GetControls(ControlRigAssetReference, TargetSkeleton);
}

const TArray<FControlRigIOMapping::FControlsInfo>& FControlRigIOMapping::FRigControlsData::GetControls(const FControlRigAssetStrongReference& ControlRigAssetReference, USkeleton* TargetSkeleton) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()

	if (ControlsInfoAssetReference != ControlRigAssetReference)
	{
		ControlsInfo.Reset();
		ControlsInfoAssetReference = ControlRigAssetReference;

		if (ControlRigAssetReference.IsValid())
		{
			if (TargetSkeleton != nullptr)
			{
				UControlRig* TemplateRig = ControlRigAssetReference.CreateInstance(GetTransientPackage(), NAME_None, RF_Transient);
...
void FControlRigIOMapping::GetVariables(const FControlRigAssetStrongReference& TargetSource, bool bInput, bool bIgnoreVariablesWithNoMemory, TMap<FName, FRigVMExternalVariable>& OutVariables)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	...
	if (TargetSource.IsValid())
	{
		...
		{
			if (ExternalVariable.IsPublic())
			{
				if (!bInput || !ExternalVariable.IsReadOnly())
				{
					if (bIgnoreVariablesWithNoMemory && ExternalVariable.GetMemory() == nullptr)
					{
						continue;
					}
					OutVariables.Add(ExternalVariable.GetName(), ExternalVariable);
				}
			}
		}
	}
...
FControlRigAssetStrongReference FControlRigIOMapping::GetTargetAssetReference() const
{
	return (OnGetTargetAssetReferenceDelegate.IsBound()) ? OnGetTargetAssetReferenceDelegate.Execute() : nullptr;
}
