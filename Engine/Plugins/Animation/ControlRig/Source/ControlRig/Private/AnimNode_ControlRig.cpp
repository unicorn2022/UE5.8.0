...
void FAnimNode_ControlRig::OnInitializeAnimInstance(const FAnimInstanceProxy* InProxy, const UAnimInstance* InAnimInstance)
{
	...
	if(DefaultControlRigAssetReference.IsValid())
	{
		ControlRigAssetReference = FControlRigAssetStrongReference();
	}
	...
	InitializeProperties(InAnimInstance, GetTargetAssetReference());
}
...
void FAnimNode_ControlRig::GatherDebugData(FNodeDebugData& DebugData)
{
	...
	DebugLine += FString::Printf(TEXT("(%s)"), *GetNameSafe(ControlRigAssetReference.Get()));
...
void FAnimNode_ControlRig::Update_AnyThread(const FAnimationUpdateContext& Context)
{
	...
	TRACE_ANIM_NODE_VALUE(Context, TEXT("Class"), *GetNameSafe(ControlRigAssetReference.Get()));
}
...
void FAnimNode_ControlRig::CacheBones_AnyThread(const FAnimationCacheBonesContext& Context)
{
	...
	{
		ControlRigVariableMappings.InitializeProperties(AnimInstance->GetClass(), TargetInstance, GetTargetAssetReference(), SourcePropertyNames, DestPropertyNames);
	}
...
void FAnimNode_ControlRig::PostSerialize(const FArchive& Ar)
{
	...
	if (DefaultControlRigClass_DEPRECATED)
	{
		DefaultControlRigAssetReference.Set(DefaultControlRigClass_DEPRECATED);
		DefaultControlRigClass_DEPRECATED = nullptr;
	}
}

UClass* FAnimNode_ControlRig::GetTargetClass() const
{
	FControlRigAssetStrongReference RigSource = GetTargetAssetReference();
	return RigSource.GetRigClass();
}

FControlRigAssetStrongReference FAnimNode_ControlRig::GetTargetAssetReference() const
{
	if(ControlRigAssetReference.IsValid())
	{
		return ControlRigAssetReference;
	}
	return DefaultControlRigAssetReference;
}
...
{
	if(DefaultControlRigAssetReference == nullptr)
	{
		DefaultControlRigAssetReference = ControlRigAssetReference;
	}

	// this may be setting an invalid runtime rig class,
	// which will be validated during UpdateControlRigIfNeeded
	ControlRigAssetReference = InControlRigClass;
}
...
{
	FControlRigAssetStrongReference RigSource = GetTargetAssetReference();
	if (RigSource.IsValid() && RigSource.Get() != nullptr)
	{
		if(ControlRig != nullptr)
		{
			if(!RigSource.IsSourceOf(ControlRig))
			{
				UControlRig* NewControlRig = nullptr;

				auto ReportErrorAndSwitchToDefaultRig = [this, InAnimInstance, InRequiredBones, RigSource](const FString& InMessage) -> bool
				{
					static constexpr TCHAR Format[] =  TEXT("[%s] Cannot switch to runtime rig class '%s' - reverting to default. %s");
					UE_LOG(LogControlRig, Warning, Format, *InAnimInstance->GetPathName(), *RigSource.GetPathName(), *InMessage);

					// mark the class to be known - and nullptr - indicating that it is not supported.
					ControlRigPerClass.FindOrAdd(RigSource, nullptr);

					// fall back to the default control rig and switch to that
					ControlRigAssetReference = nullptr;
				...
				if((ControlRigAssetReference == RigSource) &&
					DefaultControlRigAssetReference.IsValid() &&
					(RigSource != DefaultControlRigAssetReference))
				{
					// check if we already created this before
					if(const TObjectPtr<UControlRig>* ExistingControlRig = ControlRigPerClass.Find(ControlRigAssetReference))
					{
					...
					{
						if(RigSource.IsNative())
						{
							static constexpr TCHAR Format[] = TEXT("Class '%s' is not supported (is it native).");
							return ReportErrorAndSwitchToDefaultRig(FString::Printf(Format, *RigSource.GetPathName()));
						}
						
						// compare the two classes and make sure that the expected class is a super set in terms of
						// user defined properties.
						ControlRigAssetReference.ForEachVariableProperty([RigSource, ReportErrorAndSwitchToDefaultRig](const FProperty* OldProperty) -> bool
							{
								...
								const FProperty* NewProperty = RigSource.FindVariablePropertyByName(OldProperty->GetFName());
						...
						// create a new control rig using the new class
						{
							...
							NewControlRig = RigSource.CreateInstance(InAnimInstance->GetOwningComponent());
							...
							NewControlRig->SetObjectBinding(MakeShared<FControlRigObjectBinding>());
							NewControlRig->GetObjectBinding()->BindToObject(InAnimInstance->GetOwningComponent());
						...
						FControlRigAssetStrongReference NewSource = NewControlRig->GetAssetReference();
						ControlRigPerClass.FindOrAdd(NewSource, NewControlRig);
					}
				}

				// stop listening to the rig, store it for reuse
				ControlRig->OnInitialized_AnyThread().RemoveAll(this);
				FControlRigAssetStrongReference NewSource = ControlRig->GetAssetReference();
				ControlRigPerClass.FindOrAdd(NewSource, ControlRig);
				ControlRig = nullptr;
		...
		if(ControlRig == nullptr)
		{
			...
			ControlRig = RigSource.CreateInstance(InAnimInstance->GetOwningComponent());
			if (!ControlRig)
			{
				return false;
			}
...
{
	FControlRigAssetStrongReference Reference(InTargetClass);
	InitializeProperties(InSourceInstance, Reference);
}

void FAnimNode_ControlRig::InitializeProperties(const UObject* InSourceInstance, FControlRigAssetStrongReference InTargetClass)
{
