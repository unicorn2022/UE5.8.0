...
FRigUnit_SetControlVisibility_Execute()
{
	...
	if (Hierarchy)
	{
		auto UpdateVisibility = [&ExecuteContext, Hierarchy, bNewVisibility = bVisible](const FCachedRigElement& CachedControlIndex)
		{
...
#endif
				Hierarchy->SetControlVisibility(CachedControlIndex, bNewVisibility);
			}
		};
		...
		if (Item.IsValid())
		{
			...
			if (CachedControlIndices.Num() != 1)
			{
				CachedControlIndices.Reset();
				CachedControlIndices.SetNumZeroed(1);
			}

			if (CachedControlIndices[0].UpdateCache(Item, Hierarchy))
			{
				UpdateVisibility(CachedControlIndices[0]);
			} 
			return;
		}
		...
		if (!CachedControlIndices.IsEmpty())
		{
			for (int32 Index = 0; Index < Keys.Num(); Index++)
			{
				if (CachedControlIndices[Index].UpdateCache(Keys[Index], Hierarchy))
				{
					UpdateVisibility(CachedControlIndices[Index]);
				}
			}
		}
	}
