...
#define LOCTEXT_NAMESPACE "RigUnit_UserData"
...
FText FRigDispatch_GetUserData::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if (InArgumentName == ArgNameSpaceName)
	{
		return LOCTEXT("GetUserDataNameSpaceToolTip", "The namespace to look up the argument within");
	}
	if (InArgumentName == ArgPathName)
	{
		return LOCTEXT("GetUserDataPathToolTip", "The path to the argument within the namespace");
	}
	if (InArgumentName == ArgDefaultName)
	{
		return LOCTEXT("GetUserDataDefaultToolTip", "The default value for the argument. This is returned if the argument could not be found.");
	}
	if (InArgumentName == ArgResultName)
	{
		return LOCTEXT("GetUserDataResultToolTip", "The resulting value for the argument. This may be the default value if the argument could not be found.");
	}
	if (InArgumentName == ArgFoundName)
	{
		return LOCTEXT("GetUserDataFoundToolTip", "A boolean flag indicating if the argument could be found.");
	}
