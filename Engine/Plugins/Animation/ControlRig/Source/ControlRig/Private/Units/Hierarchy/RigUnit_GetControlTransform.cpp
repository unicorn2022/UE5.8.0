...
FRigUnit_GetControlPreferredEulerAngles_Execute()
{
    DECLARE_SCOPE_HIERARCHICAL_COUNTER_RIGUNIT()
	const URigHierarchy* Hierarchy = ExecuteContext.Hierarchy;
	...
	if (Hierarchy)
	{
		...
		{
			RotationOrder = Hierarchy->GetControlPreferredEulerRotationOrderByIndex(CachedControlIndex, false);
			Angles = Hierarchy->GetControlPreferredEulerAnglesByIndex(CachedControlIndex, RotationOrder);
		}
	}
}
