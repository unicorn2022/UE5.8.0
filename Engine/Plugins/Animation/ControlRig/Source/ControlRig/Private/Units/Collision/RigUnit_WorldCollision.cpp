...
FRigUnit_SphereTraceByProfile_Execute()
{
	...
	Hash = HashCombine(Hash, GetTypeHash(Start));
	Hash = HashCombine(Hash, GetTypeHash(End));
	Hash = HashCombine(Hash, GetTypeHash(ProfileName));
	Hash = HashCombine(Hash, GetTypeHash(Radius));
...
FRigUnit_LineTraceByProfile_Execute()
{
	FRigUnit_SphereTraceByProfile::StaticExecute(ExecuteContext, Start, End, 0.0f, ProfileName, bHit, HitLocation, HitNormal, WorkData);
}
