...
FRigUnit_SetControlInteger_Execute()
{
	...
	FRigUnit_SetControlIntegerFloatWeight::StaticExecute(ExecuteContext, Control, FloatWeight, IntegerValue, CachedControlIndex);	
}

FRigVMStructUpgradeInfo FRigUnit_SetControlInteger::GetUpgradeInfo() const
{
	FRigUnit_SetControlIntegerFloatWeight NewNode;
	NewNode.Control = Control;
	NewNode.Weight = (float)Weight;
	NewNode.IntegerValue = IntegerValue;

	return FRigVMStructUpgradeInfo(*this, NewNode);
}

FRigUnit_SetControlIntegerFloatWeight_Execute()
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_RIGUNIT()
	URigHierarchy* Hierarchy = ExecuteContext.Hierarchy;
	if (Hierarchy)
