...
FRigVMTemplateTypeMap FRigDispatch_ComponentBase::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex,
	FRigVMRegistryHandle& InRegistry) const
{
	FRigVMTemplateTypeMap Types;

	if (ensure(InArgumentName == ComponentArgName))
	{
		if (IsTypeSupported(InTypeIndex, InRegistry))
		{
			...
			{
				if (Info.Name == ComponentArgName)
				{
					Types.Add(Info.Name, InTypeIndex);
				}
				else
				{
					const FRigVMTemplateArgument& Argument = Info.GetArgument_NoLock(InRegistry);
					check(Argument.IsSingleton_NoLock({}, InRegistry));
					Types.Add(Info.Name, Argument.GetTypeIndex_NoLock(0, InRegistry));
				}
			}
		}
	}
