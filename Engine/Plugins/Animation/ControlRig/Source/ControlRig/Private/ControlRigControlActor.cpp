...
void AControlRigControlActor::Refresh()
{
	...
	if (ControlRig == nullptr)
	{
		...
		Hierarchy->ForEach<FRigControlElement>([this, ShapeLibraries, Hierarchy, &ProviderRegistry](FRigControlElement* ControlElement) -> bool
        {
			...
			switch (ControlElement->Settings.ControlType)
			{
				...
				case ERigControlType::EulerTransform:
				{
					if (const FControlRigShapeDefinition* ShapeDef = UControlRigShapeLibrary::GetShapeByName(ControlElement->Settings.ShapeName, ShapeLibraries, ControlRig->ShapeLibraryNameMap))
					{
						...
						UPrimitiveComponent* Component = [ShapeDef, Outer = ActorRootComponent, &ProviderRegistry]() -> UPrimitiveComponent*
						{
							if (UObject* Proxy = ShapeDef->GetShapeProxy())
							{
								return ProviderRegistry.GetNewProxyComponent(Proxy, Outer);	
							}
							return nullptr;
						}();
	...
	for (int32 GizmoIndex = 0; GizmoIndex < ControlNames.Num(); GizmoIndex++)
	{
		...
		USceneComponent* Component = Components[GizmoIndex];
