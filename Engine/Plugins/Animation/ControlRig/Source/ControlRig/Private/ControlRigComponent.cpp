...
void UControlRigComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.MemberProperty && PropertyChangedEvent.MemberProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UControlRigComponent, ControlRigAssetReference))
	{
		ControlRig = nullptr;
...
void UControlRigComponent::Initialize()
{
		...
		if (MappedElement.SceneComponent)
		{
			...
			if(!TickDependencyErrorMessage.IsEmpty())
			{
				...
				if(!MappedElement.ComponentReference.ComponentProperty.IsNone() || MappedElement.ComponentReference.OtherActor.IsValid())
				{
					...
					{
						UE_LOGF(LogControlRig, Warning, "%ls", *TickDependencyErrorMessage);
	...
	for (const TObjectPtr<USceneComponent>& ErrorComponent : MappedComponentsWithErrors)
	{
		...
		UE_LOGF(LogControlRig, Warning, "%ls: %ls", *GetPathName(), *Message);
	}
#endif
}
...
UControlRig* UControlRigComponent::SetupControlRigIfRequired()
{
	...
	{
		if (!ControlRigAssetReference.IsSourceOf(ControlRig))
		{
	...
	{
		UControlRig* NewControlRig = ControlRigAssetReference.CreateInstance(this);
...
{
	FControlRigAssetStrongReference AssetReference(InControlRigClass);
	SetControlRigAssetReference(AssetReference);
}

void UControlRigComponent::SetControlRigAssetReference(FControlRigAssetStrongReference InControlRigAssetReference)
{
	if (ControlRigAssetReference != InControlRigAssetReference)
	{
		SetControlRig(nullptr);
...
{
	UE_LOGF(LogControlRig, Warning, "%ls: %ls", *GetPathName(), *InMessage);
