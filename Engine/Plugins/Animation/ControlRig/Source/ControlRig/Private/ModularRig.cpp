...
void FRigModuleInstance::SetRig(UControlRig* InRig)
{
	...
	if (InRig)
	{
		RigAssetReference = InRig->GetAssetReference();
	}
	else
	{
		RigAssetReference.Reset();
	}
...
void FRigModuleInstance::PostSerialize(const FArchive& Ar)
{
	if (Ar.IsLoading())
	{
		if (RigClass_DEPRECATED)
		{
			RigAssetReference.Set(RigClass_DEPRECATED);
			RigClass_DEPRECATED = nullptr;
		}
	}
}
...
void UModularRig::GetPreloadDependencies(TArray<UObject*>& OutDeps)
{
	...
	{
		OutDeps.AddUnique(Module->ControlRigAssetReference.Get());
		return true;
	});
}
...
void UModularRig::UpdateModuleHierarchyFromCDO()
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		Model.ForEachModule([this, &Model](const FRigModuleReference* InModuleReference) -> bool
		{
			...
			FControlRigAssetStrongReference AssetReference = InModuleReference->ControlRigAssetReference.Get();
			if (IsInGameThread() && !AssetReference.IsValid())
			{
				AssetReference = InModuleReference->ControlRigAssetReference.LoadStrongReference(/*bForceLoad=*/true);
			}
...
bool UModularRig::Execute_Internal(const FName& InEventName)
{
	if (VM)
	{
		...
		{
			UE::TWriteScopeLock WriteLock(LastExecutionQueueLock);
...
const FModularRigSettings& UModularRig::GetModularRigSettings() const
{
	...
	else if (const UModularRig* CDO = Cast<UModularRig>(GetClass()->GetDefaultObject()))
	{
	...
	while(ExecutionQueue.IsValidIndex(ExecutionQueueFront))
	{
		...
		if (FRigModuleInstance* ModuleInstance = ExecutionElement.ModuleInstance)
		{
			...
			if (UControlRig* ModuleRig = ModuleInstance->GetRig())
			{
				...
				RigPublicContext.SetEventName(ExecutionElement.EventName);
				...
				if(ModuleRig->IsInitRequired())
				{
					...
					{
						(void) ModuleReference->ConfigOverrides.CopyToSubject(ModuleRig->GetVariablesMemory(), ModuleRig->GetVariablesStruct(), ModuleRig->GetFName());
					}
				}
				...
				// Make sure the module's rig has the correct user data
				...
				// - outer modular rig
				// - external variables
				if (RigPublicContext.IsRunningAConstructionEvent())
				{
					...
					{
						RigPublicContext.AssetUserData.Reserve(ControlRigUserDataArray->Num());
...
#if WITH_EDITOR
				if (!LastExecutedElements.IsEmpty())
				{
...
{
	UE::TReadScopeLock ReadLock(LastExecutionQueueLock);
...
const FModularRigModel& UModularRig::GetModularRigModel() const
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = GetGeneratedByAsset().GetObject())
		{
...
void UModularRig::GenerateRequiredPluginsData(FRigVMExtendedExecuteContext& Context)
{
	...
	ForEachModule([this](const FRigModuleInstance* Module) -> bool
	{
		if(UControlRig* ModuleRig = Module->GetRig())
		{
			ModuleRig->GenerateRequiredPluginsData(ModuleRig->GetRigVMExtendedExecuteContext());

			const TArray<FString>& RequiredPluginsForModule = ModuleRig->GetRequiredPlugins();
			for (const FString& RequiredPluginForModule : RequiredPluginsForModule)
			{
				RequiredPlugins.AddUnique(RequiredPluginForModule);
			}
		}
		return true;
	});
...
FRigModuleInstance* UModularRig::AddModuleInstance(const FName& InModuleName, FControlRigAssetStrongReference InModuleSource, const FRigModuleInstance* InParent,
                                                   const FRigElementKeyRedirector::FKeyMap& InConnectionMap, const FControlRigOverrideContainer& InConfigValues) 
{
	...
	if(UControlRig** ExistingModuleRigPtr = PreviousModuleRigs.Find(InModuleName))
	{
		if(UControlRig* ExistingModuleRig = *ExistingModuleRigPtr)
		{
			...
			if(ExistingModuleRig->GetFName() == InModuleName && InModuleSource.IsSourceOf(ExistingModuleRig))
			{
	...
	{
		NewModuleRig = InModuleSource.CreateInstance(this, InModuleName);
	}
	else
	{
		if (TScriptInterface<IRigVMRuntimeAssetInterface> RuntimeAsset = NewModuleRig->GetGeneratedByAsset())
		{
			TArray<FRigVMExternalVariable> ExternalVariables = RuntimeAsset->GetExternalVariables();
			for (FRigVMExternalVariable Variable : ExternalVariables)
			{
				const FProperty* Property = Variable.GetProperty();
				uint8* TargetData = Property->ContainerPtrToValuePtr<uint8>(NewModuleRig->GetVariablesMemory());
				Property->CopyCompleteValue(TargetData, Variable.GetMemory());
			}
		}
		else
		{
	...
	// Configure module
	{
		...
		InConfigValues.CopyToSubject(ModuleRig->GetVariablesMemory(), ModuleRig->GetVariablesStruct(), ModuleRig->GetFName());
	}
...
bool UModularRig::SetModuleVariableBindings(const FName& InModuleName, const TMap<FName, FString>& InVariableBindings)
{
	if(FRigModuleInstance* Module = const_cast<FRigModuleInstance*>(FindModule(InModuleName)))
	{
		...
		for (const TPair<FName, FString>& Pair : InVariableBindings)
		{
			...
			{
				// Use GetVariableByName directly: it already routes through GetVariablesMemory(),
				// which for BP-independent rigs points at the dynamic UPropertyBag (Variables.GetMutableValue().GetMemory())
				SourceVariable = GetVariableByName(*SourceVariableName);
			}
			...
			if(SourceVariable.GetProperty() == nullptr)
			{
				// todo: report error
				return false;
			}
			
			SourceVariable.SetName(*Pair.Value); // Adapt the name of the variable to contain the full path
...
void UModularRig::UpdateModuleVariables(const FRigModuleInstance* InModule)
{
	if(const UControlRig* ModuleRig = InModule->GetRig())
	{
		...
		{
			FRigVMExternalVariable TargetVariable = ModuleRig->GetPublicVariableByName(Pair.Key);
			if(ensure(TargetVariable.GetProperty()))
			{
				if (RigVMTypeUtils::AreCompatible(Pair.Value.GetProperty(), TargetVariable.GetProperty()))
				{
					Pair.Value.GetProperty()->CopyCompleteValue(TargetVariable.GetMemory(), Pair.Value.GetMemory());
				}
			}
		}
	}
...
UControlRig* UModularRig::GetModuleRig(FString InModulePath)
{
	if(const FRigModuleInstance* Module = FindModule_Deprecated(InModulePath))
	{
		...
		UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't contain a rig instance.", *InModulePath);
		return nullptr;
	}
	UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't exist.", *InModulePath);
	return nullptr;
}
...
UControlRig* UModularRig::GetModuleRigByName(FName InModuleName)
{
	if(const FRigModuleInstance* Module = FindModule(InModuleName))
	{
		...
		UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't contain a rig instance.", *InModuleName.ToString());
		return nullptr;
	}
	UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't exist.", *InModuleName.ToString());
	return nullptr;
}
...
TArray<FName> UModularRig::ExecuteEventOnAllModules(FName InEvent)
{
	...
	{
		UE_LOGF(LogControlRig, Error, "Event '%ls' is not supported by any module on this modular rig.", *InEvent.ToString());
	...
	for(const FRigModuleExecutionElement& ExecutionElement : ExecutionQueue)
	{
		...
		{
			UE_LOGF(LogControlRig, Error, "Module '%ls' did not run event '%ls' successfully.", *ExecutionElement.ModuleName.ToString(), *InEvent.ToString());
		}
	}
...
bool UModularRig::ExecuteEventOnModuleForBP(FName InEvent, FString InModulePath)
{
	if(FRigModuleInstance* Module = FindModule_Deprecated(InModulePath))
	{
		if(const UControlRig* ModuleRig = Module->GetRig())
		{
			...
			UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't support the event '%ls'.", *InModulePath, *InEvent.ToString());
			return false;
		}
		UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't contain a rig instance.", *InModulePath);
		return false;
...
bool UModularRig::ExecuteEventOnModuleByNameForBP(FName InEvent, FName InModuleName)
{
	if(FRigModuleInstance* Module = FindModule(InModuleName))
	{
		if(const UControlRig* ModuleRig = Module->GetRig())
		{
			...
			UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't support the event '%ls'.", *InModuleName.ToString(), *InEvent.ToString());
			return false;
		}
		UE_LOGF(LogControlRig, Error, "Module '%ls' doesn't contain a rig instance.", *InModuleName.ToString());
		return false;
