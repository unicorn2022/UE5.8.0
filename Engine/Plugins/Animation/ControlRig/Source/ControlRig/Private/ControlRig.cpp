...
void UControlRig::BeginDestroy()
{
	...
	if (!HasAnyFlags(RF_ClassDefaultObject) && !IsEngineExitRequested())
	{
		if(const UClass* Class = GetClass())
		{
			if(IsValid(Class))
			{
				URigHierarchy* DefaultHierarchy = nullptr;
				if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = GeneratedBy.GetObject())
				{
					if (!IsGarbageOrDestroyed(RuntimeAsset.GetObject()))
					{
						DefaultHierarchy = RuntimeAsset->GetHierarchy();
					}
				}
				else if(UControlRig* CDO = Class->GetDefaultObject<UControlRig>())
				{
					if (!IsGarbageOrDestroyed(CDO))
					{
						DefaultHierarchy = CDO->GetHierarchy();
					}
				}
				
				if (DefaultHierarchy)
				{
					DefaultHierarchy->UnregisterListeningHierarchy(GetHierarchy());
				}
			}
		}
	}
...
void UControlRig::Initialize(bool bRequestInit)
{
	...
	// Runtime-asset-backed rigs loaded from disk might arrive here with DynamicHierarchy == nullptr:
	// PostInitProperties runs before GeneratedBy is serialized, so PostInitInstance is skipped
	// at construction and PostLoad does not make up for it. InitializeVMs below would create
	// the hierarchy (via InitializeFromCDO → PostInitInstanceIfRequired), but we access
	// GetHierarchy() earlier than that, so ensure it's ready up front.
	PostInitInstanceIfRequired();

	if (!IsConstructionRequired())
	{
		SelectionBeforeInit = GetHierarchy()->GetSelectedHierarchyKeys();
	}
...
void UControlRig::RestoreShapeLibrariesFromCDO()
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = GeneratedBy.GetObject())
		{
			ShapeLibraries = RuntimeAsset->GetShapeLibraries();
		}
		else if(UControlRig* CDO = GetClass()->GetDefaultObject<UControlRig>())
		{
...
void UControlRig::Evaluate_AnyThread()
{
	if (bIsAdditive)
	{
		...
		// Let's set up a queue so that the rig knows when to reset things. We manually
		// run the events in the queue here - but the queue is still required to make sure
		// the delta time isn't reset, logs are being collected correctly etc.
		// Note: It's ok that the interaction event is in here - since we are manually
		// running the queue below it won't be run if there's no interaction.
		static const TArray<FName> AdditiveControlRigQueue = {
			FRigUnit_InverseExecution::EventName,
			FRigUnit_InteractionExecution::EventName,
			FRigUnit_PreBeginExecution::EventName,
			FRigUnit_BeginExecution::EventName,
			FRigUnit_PostBeginExecution::EventName
		};
		const TGuardValue<TArray<FName>> EventQueueGuard(EventQueueToRun, AdditiveControlRigQueue);
...
{
	// Flag the rig as needing sequencer re-initialization. Scoped to Runtime Asset-backed
	// rigs because they re-initialize the same UObject in place (same GetUniqueID()), so
	// sequencer's HasControlRigTrack(UniqueID) check would otherwise skip the refresh.
	// BP rigs get a new UObject via FBlueprintCompileReinstancer, so they don't need this.
	if (!HasAnyFlags(RF_ClassDefaultObject) && GetGeneratedByAsset().GetObject() != nullptr)
	{
		bNeedsInstanceInitialization = true;
	}
	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		if (!IsRigModuleInstance())
		{
			...
			URigHierarchy* DefaultHierarchy = nullptr;
			FRigHierarchySettings* DefaultHierarchySettings = nullptr;
			FRigElementKeyRedirector* DefaultElementKeyRedirector = nullptr;
			FRigModuleSettings* DefaultRigModuleSettings = nullptr;
			if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = GeneratedBy.GetObject())
			{
				DefaultHierarchy = RuntimeAsset->GetHierarchy();
				DefaultHierarchySettings = &RuntimeAsset->GetHierarchySettings();
				DefaultElementKeyRedirector = &RuntimeAsset->GetElementKeyRedirector();
				DefaultRigModuleSettings = &RuntimeAsset->GetRigModuleSettings();
			}
			else if (UControlRig* CDO = GetClass()->GetDefaultObject<UControlRig>())
			{
				DefaultHierarchy = CDO->GetHierarchy();
				DefaultHierarchySettings = &CDO->HierarchySettings;
				DefaultElementKeyRedirector = &CDO->ElementKeyRedirector;
				DefaultRigModuleSettings = &CDO->RigModuleSettings;
			}
			URigHierarchy* Hierarchy = GetHierarchy();
			...
			// copy hierarchy
			{
				...
				FRigHierarchyValidityBracket ValidityBracketB(DefaultHierarchy);
			
				TGuardValue<bool> Guard(Hierarchy->GetSuspendNotificationsFlag(), true);
				Hierarchy->CopyHierarchy(DefaultHierarchy);
			...
			DefaultHierarchy->RegisterListeningHierarchy(Hierarchy);
#endif
			...
			if (DefaultHierarchySettings)
			{
				HierarchySettings = *DefaultHierarchySettings;
			}
			if (DefaultElementKeyRedirector)
			{
				ElementKeyRedirector = FRigElementKeyRedirector(*DefaultElementKeyRedirector, Hierarchy);
			} 
			if (DefaultRigModuleSettings)
			{
				RigModuleSettings = *DefaultRigModuleSettings;
			}
		
			// increment the procedural limit based on the number of elements in the CDO
			if(const URigHierarchy* CDOHierarchy = DefaultHierarchy)
			{
...
bool UControlRig::AllConnectorsAreResolved(FString* OutFailureReason, FRigElementKey* OutConnector) const
{
	if(const URigHierarchy* Hierarchy = GetHierarchy())
	{
		...
		for(const FRigModuleConnector& Connector : GetRigModuleSettings().ExposedConnectors)
		{
			...
			if(const FRigElementKeyRedirector::FCachedKeyArray* Cache = ElementKeyRedirector.Find(ConnectorKey))
			{
				...
				{
					const bool bIsConnectorValid = const_cast<FCachedRigElement*>(&CachedRigElement)->UpdateCache(Hierarchy);
					if (!bIsConnectorValid)
					{
						if(OutFailureReason)
						{
...
bool UControlRig::Execute(const FName& InEventName)
{
	...
	if(bRequiresInitExecution && !IsReadOnly())
	{
	...
	PublicContext.SetEventName(InEventName);
	...
	if (bEnabledDuringGame || !bIsRunningInPIE || UE_TRACE_CHANNELEXPR_IS_ENABLED(RigVMChannel))
	{
	...
	if (GeneratedBy)
	{
		DrawContainer = GeneratedBy->GetDrawContainer();
	}
	else if (UControlRig* CDO = GetClass()->GetDefaultObject<UControlRig>())
	{
	...
	if(!HasAnyFlags(RF_ClassDefaultObject))
	{
		if (PublicContext.Hierarchy)
		{
			if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = GeneratedBy.GetObject())
			{
				if(URigHierarchy* DefaultHierarchy = RuntimeAsset->GetHierarchy())
				{
					PublicContext.Hierarchy->DefaultHierarchyPtr = DefaultHierarchy;
				}
			}
			else if (UControlRig* CDO = Cast<UControlRig>(GetClass()->GetDefaultObject()))
			{
	...
	if(Hierarchy && !IsReadOnly())
	{
		...
		bool bShouldResetRecords = !bIsEventInQueue || bIsEventFirstInQueue;
		if (!bShouldResetRecords && bIsAdditive)
		{
			// In the case of layered rigs, the forward solve is not the first event in the queue, the backward solve is
			// So lets just force the reset right before the pre forward solve
			bShouldResetRecords = InEventName == FRigUnit_PreBeginExecution::EventName;
		}
	...
	if(IsExecuting())
	{
		UE_LOGF(LogControlRig, Warning, "%ls: Execute is being called recursively.", *GetPathName());
		return false;
	}
	if(bIsConstructionEvent || bIsPostConstructionEvent)
	{
		...
		{
			UE_LOGF(LogControlRig, Warning, "%ls: Construction is being called recursively.", *GetPathName());
			return false;
		}
	}
	...
	if (bIsConstructionEvent)
	{
		...
		// Reset our flag on construction - nodes etc are expected to set this if they contain
		// state/would behave badly with some optional updates.
		bContainsSimulation = false;
		...
		const bool bConstructionModeEnabled = IsConstructionModeEnabled();
		{
			...
			TGuardValue<bool> DisableSelectionNotifications(Hierarchy->GetController(true)->bSuspendSelectionNotifications, true);
			{
				...
				if (bResetCurrentTransformsAfterConstruction && !IsReadOnly())
				{
...
#endif
					if (!bHierarchyInitializedThisFrame && !IsReadOnly())
					{
				...
				{
					Hierarchy->GetController()->SetHierarchySelection(SelectionBeforeInit);
					ApplyElementOverrides();
					EmitPostConstructionEventFinished();
				}

				// Reset the input pose after construction
				if (CurrentPose.Num() > 0 && !IsReadOnly())
				{
					const TGuardValue<bool> DisableRecordingCurveChanges(Hierarchy->GetRecordCurveChangesFlag(), false);
		...
		if (bConstructionModeEnabled && !IsReadOnly())
		{
#if WITH_EDITOR
			TUniquePtr<FTransientControlPoseScope> TransientControlPoseScope;
			if (bConstructionModeEnabled)
		...
		SendSelectionBeforeInitNotifications();
	}
	else
	{
#if WITH_EDITOR
...
bool UControlRig::Execute_Internal(const FName& InEventName)
{
	...
	if (VM)
	{
		...
		if(ProfilingRunsLeft == 0)
		{
			...
			UE_LOGF(LogControlRig, Display, "%ls: %d runs took %.03lfms.", *GetClass()->GetName(), UE_RIGVM_PROFILE_EXECUTE_UNITS_NUM, Milliseconds);
		}
#endif
...
FControlRigAssetStrongReference UControlRig::GetAssetReference() const
{
	FControlRigAssetStrongReference Result;
	if (GeneratedBy)
	{
		if (UControlRigRuntimeAsset* RuntimeAsset = Cast<UControlRigRuntimeAsset>(GeneratedBy.GetObject()))
		{
			Result.Set(RuntimeAsset);
		}
	}
	// for backwards compatibility, we cast to URigVMBlueprintGeneratedClass instead of UControlRigBlueprintGeneratedClass
	else if (URigVMBlueprintGeneratedClass* GeneratedClass = Cast<URigVMBlueprintGeneratedClass>(GetClass()))
	{
		Result.Set(TSubclassOf<UControlRig>(GeneratedClass));
	}
	else if (GetClass() == UFKControlRig::StaticClass())
	{
		Result.Set(TSubclassOf<UControlRig>(UFKControlRig::StaticClass()));
	}
	return Result;
}
...
const FRigModuleSettings& UControlRig::GetRigModuleSettings() const
{
	...
	else if (const UControlRig* CDO = Cast<UControlRig>(GetClass()->GetDefaultObject()))
	{
...
const FString& UControlRig::GetRigModulePrefix() const
{
	if(IsRigModule())
	{
		auto& [CachedPrefixName, CachedPrefixString] = RigModulePrefix;
		if (IsRigModuleInstance() || GetRigModuleSettings().Identifier.Name.IsEmpty())
		{
			if (!CachedPrefixName.IsEqual(GetFName(), ENameCase::CaseSensitive) || CachedPrefixString.IsEmpty())
			{
				CachedPrefixString = GetFName().ToString() + FRigHierarchyModulePath::ModuleNameSuffix;
				CachedPrefixName = GetFName();
			}
		}
		else
		{
			CachedPrefixString = GetRigModuleSettings().Identifier.Name + FRigHierarchyModulePath::ModuleNameSuffix;
		}
...
void UControlRig::CreateRigControlsForCurveContainer()
{
	...
	if(bCreateFloatControls && DynamicHierarchy)
	{
		...
		DynamicHierarchy->ForEach<FRigCurveElement>([this, Controller](FRigCurveElement* CurveElement) -> bool
        {
			...
			if (Name.Contains(CtrlPrefix) && !DynamicHierarchy->Contains(FRigElementKey(*Name, ERigElementType::Curve)))
			{
...
{
	// make sure we can run this rig as we are interacting with it.
	ResetEvaluationsLeft();
...
const TArray<TSoftObjectPtr<UControlRigShapeLibrary>>& UControlRig::GetShapeLibraries() const
{
	...
	if (ShapeLibraries.IsEmpty())
	{
		if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = GeneratedBy.GetObject())
		{
			LibrariesPtr = &RuntimeAsset->GetShapeLibraries();
		}
		else if(!GetClass()->IsNative())
		{
...
TArray<FName> UControlRig::GetSortedControls() const
{
	TArray<FRigControlElement*> SortedControls;
	GetControlsInOrder(SortedControls);
	TArray<FName> ControlNames;
	ControlNames.Reserve(SortedControls.Num());
...
void UControlRig::SendSelectionBeforeInitNotifications()
{
	URigHierarchy* Hierarchy = GetHierarchy();
	...
	{
		if(!SelectionBeforeInit.Contains(SelectedHierarchyKey))
		{
	...
	for(const FRigHierarchyKey& PreviouslySelectedKey : SelectionBeforeInit)
	{
...
UObject* UControlRig::GetBoundOuterForTrace() const
{
	if (ObjectBinding.IsValid())
	{
		if (UObject* Owner = ObjectBinding->GetBoundObject())
		{
			return Owner;
		}
	}
	return Super::GetBoundOuterForTrace();
}

void UControlRig::TraceExecuteData(FRigVMTraceArchiveWriter& InArchive) const
{
	InArchive.UsingCustomVersion(FControlRigObjectVersion::GUID);
	
	Super::TraceExecuteData(InArchive);

	URigHierarchy* Hierarchy = GetHierarchy();
	bool bHasHierarchy = Hierarchy != nullptr;
	InArchive << bHasHierarchy;
	if (bHasHierarchy)
	{
		int64 PosBeforeHierarchy = InArchive.Tell();
		int64 PosAfterHierarchy = 0;
		InArchive << PosAfterHierarchy;
		
		Hierarchy->Serialize(InArchive);

		PosAfterHierarchy = InArchive.Tell();
		InArchive.Seek(PosBeforeHierarchy);
		InArchive << PosAfterHierarchy;
		InArchive.Seek(PosAfterHierarchy);
	}
}

void UControlRig::LoadTracedExecuteData(FRigVMTraceArchiveReader& InArchive)
{
	Super::LoadTracedExecuteData(InArchive);
...
void UControlRig::GetControlsInOrder(TArray<FRigControlElement*>& SortedControls) const
{
	...
	SortedControls = DynamicHierarchy->GetElementsOfType<FRigControlElement>(true, true);
}
...
const FRigInfluenceMap* UControlRig::FindInfluenceMap(const FName& InEventName)
{
	if (TScriptInterface<IControlRigRuntimeAssetInterface> RuntimeAsset = GeneratedBy.GetObject())
	{
		return RuntimeAsset->GetInfluences().Find(InEventName);
	}
	else if (UControlRig* CDO = GetClass()->GetDefaultObject<UControlRig>())
	{
...
const TArray<UAssetUserData*>* UControlRig::GetAssetUserDataArray() const
{
	...
	if (GeneratedBy)
	{
		CombinedAssetUserData.Append(*GeneratedBy->GetAssetUserDataArray());
	}
	else if (UControlRig* CDO = Cast<UControlRig>(GetClass()->GetDefaultObject(false)))
	{
	...
	{
		if(ExternalVariable.GetMemory() != nullptr)
		{
			if(const UClass* Class = Cast<UClass>(ExternalVariable.GetCPPTypeObject()))
			{
				if(Class->IsChildOf(UDataAsset::StaticClass()))
				{
					TObjectPtr<UDataAsset>& DataAsset = *(TObjectPtr<UDataAsset>*)ExternalVariable.GetMemory();
					if(IsValid(DataAsset) && !DataAsset->GetFName().IsNone())
					{
						if(const TObjectPtr<UDataAssetLink>* ExistingDataAssetLink =
							ExternalVariableDataAssetLinks.Find(ExternalVariable.GetName()))
						{
							if(!IsValid(*ExistingDataAssetLink) ||
								(*ExistingDataAssetLink)->HasAnyFlags(EObjectFlags::RF_BeginDestroyed))
							{
								ExternalVariableDataAssetLinks.Remove(ExternalVariable.GetName());
							}
						}
						if(!ExternalVariableDataAssetLinks.Contains(ExternalVariable.GetName()))
						{
						...
						TObjectPtr<UDataAssetLink>& DataAssetLink = ExternalVariableDataAssetLinks.FindChecked(ExternalVariable.GetName());
						DataAssetLink->NameSpace = ExternalVariable.GetName().ToString();
...
void UControlRig::PostInitInstanceIfRequired()
{
	if(GetHierarchy() == nullptr || VM == nullptr)
	{
		...
		else
		{
			if (GeneratedBy)
			{
				ensure(VM == nullptr || VM == GeneratedBy->GetVM());
				PostInitInstance(nullptr);
			}
			else
			{
...
void UControlRig::PostInitInstance(URigVMHost* InCDO)
{
	...
	{
		// If this is not a CDO, it should have never saved its hierarchy. However, we have found that some rigs in the past
		...
		{
			FGCScopeGuard Guard;
			// Don't replace the DynamicHierarchy object if it's not Local. We need to keep Remote/LocalNotReady DynamicHierarchy object so that we don't accidentally create another one with the same remote id when migrating from another server
			URigHierarchy* ObjectFound = Cast<URigHierarchy>(StaticFindObjectFast(URigHierarchy::StaticClass(), this, TEXT("DynamicHierarchy"), EFindObjectFlags::ExactClass, RF_DefaultSubObject, EInternalObjectFlags::Remote | EInternalObjectFlags::LocalNotReady));
			if (ObjectFound)
			{
	...
	if (ControlElement->GetFName() != InControlName)
	{
		...
		if (DynamicHierarchy->ReceivedNameSpaceBasedKeys.Contains(ControlKey))
		{
			UE_LOGF(
				LogControlRig,
				Warning,
				"%ls: control '%ls' has been accessed using a namespace based key ('%ls') when creating the control handle. Please consider updating your code.",
				*GetName(),
...
void UControlRig::GenerateRequiredPluginsData(FRigVMExtendedExecuteContext& Context)
{
	Super::GenerateRequiredPluginsData(Context);

	if (const URigHierarchy* Hierarchy = GetHierarchy())
	{
		for (int32 Index = 0; Index < Hierarchy->ElementComponents.Num(); Index++)
		{
			if(Hierarchy->ElementComponents[Index].IsValid())
			{
				const FRigBaseComponent* Component = Hierarchy->ElementComponents[Index].GetPtr<FRigBaseComponent>();
				const FString PluginName = RigVMTypeUtils::GetPluginName(Component->GetScriptStruct());
