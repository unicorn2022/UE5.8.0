...
namespace UE::MovieScene
{
	...
	bool GUseScopedLegacyControlRigTemplate = false;
	FAutoConsoleVariableRef CVarUseScopedLegacyControlRigTemplate(
		TEXT("ControlRig.UseScopedLegacySequencerTemplate"),
		GUseScopedLegacyControlRigTemplate,
		TEXT("(Default: false) Specifies whether to use scoped legacy template evaluation for control rig tracks in Sequencer."),
		ECVF_Default
	);
...
bool UMovieSceneControlRigParameterTrack::ShouldCacheLegacyTemplate()
{
	return ShouldUseLegacyTemplate() && UE::MovieScene::GUseScopedLegacyControlRigTemplate;
}
...
void UMovieSceneControlRigParameterTrack::OnPreBeginPIE(bool bIsSimulating)
{
	OnCleanupGameControlRigs();
}
	...
	for (const auto& [WeakGameWorldPtr, GameRigPtr]: GameWorldControlRigs)
	{
		...
		if (World && GameRig)
		{
			auto ResetHandle = [GameRig, DefaultRig](const TObjectPtr<UTransformableHandle>& Handle)
			{
				...
				{
					ControlHandle->UpdateControlRig(DefaultRig);
				}						
			};
...
void UMovieSceneControlRigParameterTrack::OnCleanupGameControlRigs()
{
	...
	for (const TPair<TWeakObjectPtr<UWorld>,TObjectPtr<UControlRig>>& Pair : GameWorldControlRigs)
	{
		if (!Pair.Key.IsValid() || Pair.Key.IsStale())
		{
			...
			if (UControlRig* GameWorldControlRig = Pair.Value)
			{
				...
				{
					UnbindGameWorldControlRigDelegates(GameWorldControlRig);
...
void UMovieSceneControlRigParameterTrack::RemoveAllConstraints() const
{
	// no need to compensate, we just want to get rig of all the animation data
	...
	for (UMovieSceneSection* Section : Sections)
	{
		...
		{
			const UControlRig* Rig = RigSection->GetControlRig();
			if (UWorld* World = Rig ? Rig->GetWorld() : nullptr)
			{
				RigSection->RemoveAllConstraints(World, bDoNotCompensate);
			}
		}
	}
	
	// also remove the constraints related to this ControlRig object that live within other constrained sections
	UWorld* World = ::IsValid(ControlRig) ? ControlRig->GetWorld() : nullptr;
	const UMovieScene* OuterMovieScene = GetTypedOuter<UMovieScene>();
	if (World && OuterMovieScene)
	{
		...
		const UObject* ThisTrack = this;
		const FSoftObjectPath RigPath(ControlRig);
		TSet<UTickableConstraint*> ConstraintsToRemove;
		
		auto ProcessSection = [ThisTrack, &ConstraintsToRemove, &RigPath](UMovieSceneSection* InSection)
		{
			if (InSection && InSection->GetOuter() != ThisTrack)
			{
				if (IMovieSceneConstrainedSection* ConstrainedSection = Cast<IMovieSceneConstrainedSection>(InSection))
				{
					for (const FConstraintAndActiveChannel& ConstraintChannel: ConstrainedSection->GetConstraintsChannels())
					{
						if (UTickableTransformConstraint* TransformConstraint = Cast<UTickableTransformConstraint>(ConstraintChannel.GetConstraint()))
						{
							if (UTransformableControlHandle* ControlHandle = ::IsValid(TransformConstraint->ParentTRSHandle) ?
								Cast<UTransformableControlHandle>(TransformConstraint->ParentTRSHandle.Get()) : nullptr)
							{
								const FSoftObjectPath& HandleRigPath = ControlHandle->ControlRig.ToSoftObjectPath();
								if (HandleRigPath == RigPath)
								{
									ConstraintsToRemove.Add(TransformConstraint);
								}
							}
						}
					}
		...
		auto ProcessTrack = [&ProcessSection, ThisTrack](const UMovieSceneTrack* InTrack)
		{
			if (InTrack && InTrack != ThisTrack)
			{
		...
		// remove constraints
		for (UTickableConstraint* ConstraintToRemove: ConstraintsToRemove)
		{
			FConstraintsManagerController::Get(World).RemoveConstraint(ConstraintToRemove, bDoNotCompensate);
		}
	}
}
...
void UMovieSceneControlRigParameterTrack::OnRemovedFromMovieSceneImpl()
{
	RemoveAllConstraints();
}
...
void UMovieSceneControlRigParameterTrack::PostLoad()
{
	...
	if (ControlRig && !ControlRig->GetAssetReference().IsNative())
	{
...
void UMovieSceneControlRigParameterTrack::BindControlRigDelegates()
{
	if (IsValid(ControlRig))
	{
		...
		{
			ControlRig->OnPreConstruction_AnyThread().AddUObject(this,  &UMovieSceneControlRigParameterTrack::HandleOnPreConstruct_AnyThread);
...
void UMovieSceneControlRigParameterTrack::BindGameWorldControlRigDelegates(UControlRig* GameWorldControlRig)
{
	if (IsValid(GameWorldControlRig))
	{
		if (!GameWorldControlRig->OnPreConstruction_AnyThread().IsBoundToObject(this))
		{
			GameWorldControlRig->OnPreConstruction_AnyThread().AddUObject(this,  &UMovieSceneControlRigParameterTrack::HandleOnPreConstruct_AnyThread);
		}
	}	
}

void UMovieSceneControlRigParameterTrack::UnbindGameWorldControlRigDelegates(UControlRig* GameWorldControlRig)
{
	if (IsValid(GameWorldControlRig))
	{
		GameWorldControlRig->OnPreConstruction_AnyThread().RemoveAll(this);
	}
}

void UMovieSceneControlRigParameterTrack::HandleOnPreConstruct_AnyThread(UControlRig* Subject, const FName& InEventName)
{
	if (IsValid(Subject))
	{
		ApplyControlRigSettingsOverrides(Subject);
	}
}
...
#if WITH_EDITOR
void UMovieSceneControlRigParameterTrack::BindPIEDelegates()
{
	OnPreBeginPIEHandle = FEditorDelegates::PreBeginPIE.AddUObject(this, &UMovieSceneControlRigParameterTrack::OnPreBeginPIE);
...
void UMovieSceneControlRigParameterTrack::UnbindPIEDelegates()
{
...
void UMovieSceneControlRigParameterTrack::HandlePackageDone(const FEndLoadPackageContext& Context)
{
	...
	// make sure the asset reference exists
	UObject* AssetReference = ControlRig->GetAssetReference().Get();
	if (!AssetReference)
	{
		return;
	}
	
	if (const UPackage* ControlRigPackage = Cast<UPackage>(AssetReference->GetOutermost()))
	{
...
void UMovieSceneControlRigParameterTrack::ApplyControlRigSettingsOverrides(UControlRig* InControlRig)
{
	if (!InControlRig)
	{
		return;
	...
	{
		FRigVMExternalVariable Variable = InControlRig->GetPublicVariableByName(PropertyDesc.Name);
		if (Variable.IsValid())
		{
			uint8* TargetAddress = Variable.GetProperty()->ContainerPtrToValuePtr<uint8>(InControlRig->GetVariablesMemory());
			const uint8* SourceAddress = ControlRigSettingsOverrides.GetValue().GetMemory() + PropertyDesc.CachedProperty->GetOffset_ForInternal();
...
void UMovieSceneControlRigParameterTrack::RecreateControlRig()
{
	Modify();
	ControlRig->Modify();
...
FInstancedPropertyBag UMovieSceneControlRigParameterTrack::GetCurrentRigSettings()
{
	if (!ControlRig)
	{
		return {};
	...
	{
		const FProperty* SourceProperty = ControlRig->GetAssetReference().FindVariablePropertyByName(Variable.GetName());
		SourceProperties.Add(SourceProperty);
		
		BagProperties.Add(FPropertyBagPropertyDesc(Variable.GetName(), SourceProperty));
	}
	...
	FInstancedPropertyBag Settings;
	Settings.AddProperties(BagProperties);

	SyncRigSettingsToCurrentRig(Settings);

	return Settings;
}

void UMovieSceneControlRigParameterTrack::SyncRigSettingsToCurrentRig(FInstancedPropertyBag& InSettings)
{
	if (!ControlRig)
	{
		return;
	...
	for (const FPropertyBagPropertyDesc& PropertyDesc : InSettings.GetPropertyBagStruct()->GetPropertyDescs())
	{
		...
		if (Variable.IsValid())
		{
			...
			uint8* TargetAddress = InSettings.GetMutableValue().GetMemory() + PropertyDesc.CachedProperty->GetOffset_ForInternal();
			FRigVMMemoryStorageStruct::CopyProperty(
				PropertyDesc.CachedProperty,
				TargetAddress,
				Variable.GetProperty(),
...
FInstancedPropertyBag UMovieSceneControlRigParameterTrack::BuildNewRigSettings(const TMap<FName, FString>& InSettingsToOverride)
{
	FInstancedPropertyBag Result = ControlRigSettingsOverrides;
	if (!Result.IsValid())
	{
		Result = GetCurrentRigSettings();
	}

	const UPropertyBag* BagStruct = Result.GetPropertyBagStruct();

	bool bSuccess = true;
	for (const TPair<FName, FString>& Pair : InSettingsToOverride)
	{
		const FPropertyBagPropertyDesc* Desc = Result.FindPropertyDescByName(Pair.Key);
		if (!Desc)
		{
			bSuccess = false;
			break;
		...
		// Making sure string parsing is the same as current Control Rig APIs
		FControlRigOverrideValue StringParser(Pair.Key.ToString(), BagStruct, Result.GetValue().GetMemory(), TrackName);
		const FControlRigOverrideValueErrorPipe::TReportFunction ReportFunction = [this, Pair, &bSuccess](const TCHAR* V, ELogVerbosity::Type Verbosity)
			{
				bSuccess = false;
				UE_LOGF(LogControlRig, Warning, "Problem during import of property '%ls' for Control Rig Track '%ls': %ls", *Pair.Key.ToString(), *TrackName.ToString(), V);
			};
		StringParser.SetFromString(Pair.Value, ReportFunction);
		if (!StringParser.IsValid())
		{
			bSuccess = false;
			break;
		}

		(void)StringParser.CopyToSubject(Result.GetMutableValue().GetMemory(), BagStruct);
	}

	// Avoid changing anything unless all overrides can be applied correctly
	if (!bSuccess)
	{
		return {};
	}
...
UControlRig* UMovieSceneControlRigParameterTrack::GetGameWorldControlRig(UWorld* InWorld, UObject* InBoundObject) 
{
	if (GameWorldControlRigs.Find(InWorld) == nullptr && ControlRig)
	{
		...
		UControlRig* NewGameWorldControlRig = AssetReference.CreateInstance(this, NAME_None, RF_Transient);
		BindGameWorldControlRigDelegates(NewGameWorldControlRig);

		if (!ControlRig->IsA<UFKControlRig>())
		{
			if (InBoundObject)
			{
				NewGameWorldControlRig->SetObjectBinding(MakeShared<FControlRigObjectBinding>());
				NewGameWorldControlRig->GetObjectBinding()->BindToObject(InBoundObject);
			}
		}
...
const TUniquePtr<FMovieSceneEvaluationTrack>& UMovieSceneControlRigParameterTrack::GetLegacyEvaluationTrack() const
{
	return LegacyEvaluationTrack;
}

FScopedLegacyEvaluationTrack::FScopedLegacyEvaluationTrack(UMovieSceneControlRigParameterTrack* InTrack)
	: WeakTrack(InTrack)
	, bCacheTemplate(UMovieSceneControlRigParameterTrack::ShouldCacheLegacyTemplate())
{
	if (InTrack && bCacheTemplate)
	{
		if (InTrack->LegacyEvaluationTrackCount == 0)
		{
			ensure(!InTrack->LegacyEvaluationTrack.IsValid());
			FMovieSceneEvaluationTrack EvaluationTrack = InTrack->GenerateTrackTemplate(InTrack);
			InTrack->LegacyEvaluationTrack = MakeUnique<FMovieSceneEvaluationTrack>(MoveTemp(EvaluationTrack));
		}
		else
		{
			ensure(InTrack->LegacyEvaluationTrack.IsValid());
		}
		InTrack->LegacyEvaluationTrackCount++;
	}
}

FScopedLegacyEvaluationTrack::FScopedLegacyEvaluationTrack(const UMovieSceneSection* InSection)
	: FScopedLegacyEvaluationTrack(InSection ? Cast<UMovieSceneControlRigParameterTrack>(InSection->GetOuter()) : nullptr)
{}

FScopedLegacyEvaluationTrack::~FScopedLegacyEvaluationTrack()
{
	if (UMovieSceneControlRigParameterTrack* Track = WeakTrack.Get())
	{
		ensure(bCacheTemplate == UMovieSceneControlRigParameterTrack::ShouldCacheLegacyTemplate());
		if (bCacheTemplate)
		{
			Track->LegacyEvaluationTrackCount--;
			ensure(Track->LegacyEvaluationTrackCount >= 0);
			if (Track->LegacyEvaluationTrackCount == 0)
			{
				Track->LegacyEvaluationTrack.Reset();
			}
		}
	}
}
