...
void FPreAnimatedControlRigTraits::RestorePreAnimatedValue(const FObjectKey& InObject, FPreAnimatedControlRigState& State, const FRestoreStateParams& Params)
{
	...
	{
		// When the anim mixer is the pose producer it owns the anim instance and skel-mesh restore,
		// so skip the evaluate / sequencer-instance unbind / skel-mesh tick path.
		if (!State.bAnimMixerPoseProducer)
		{
...
struct FEvaluateBaseControlRigs
{
		...
		FBaseControlRigEvalData& InOutBaseEvalData) const
	{
		...
		InOutBaseEvalData.WarpedTime = InOutBaseEvalData.Section->RemapTime(EvalTime);
		const FFrameRate TickResolution = InOutBaseEvalData.Section->GetTypedOuter<UMovieScene>()->GetTickResolution();
		float EvalTimeSeconds = TickResolution.AsSeconds(InOutBaseEvalData.WarpedTime);
		...
		if (InOutBaseEvalData.bIsActive)
		{
			if (USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(BoundObject))
			{
				...
				if (!AnimInstance && !InOutBaseEvalData.bAnimMixerPoseProducer)
				{
					// The ControlRigLayerInstance may have been destroyed by pre-animated state
					// restoration during PostInstantiation (e.g. when skeletal animation entities
					// are unlinked after a BakeToControlRig operation). Re-bind to recreate it.
					FControlRigBindingHelper::BindToSequencerInstance(ControlRig);
					AnimInstance = Cast<UControlRigLayerInstance>(SkeletalMeshComponent->GetAnimInstance());
				}
				if (AnimInstance)
				{
					float Weight = InOutBaseEvalData.Section->EvaluateEasing(InOutBaseEvalData.WarpedTime);
					if (EnumHasAllFlags(InOutBaseEvalData.Section->TransformMask.GetChannels(), EMovieSceneTransformChannel::Weight))
					{
						float ManualWeight = 1.f;
						InOutBaseEvalData.Section->Weight.Evaluate(InOutBaseEvalData.WarpedTime, ManualWeight);
						Weight *= ManualWeight;
					}
					...
					// Only update for Absolute sections - additive sections handle weight through control value blending
					if (InOutBaseEvalData.Section->GetBlendType() == EMovieSceneBlendType::Absolute)
					{
		...
		const FFrameTime RoundTime = InOutBaseEvalData.WarpedTime.RoundToFrame();
		...
		const TSharedRef<FSharedPlaybackState>& InSharedPlaybackState)
	{
		...
		const bool bIsRigInstanceSectionRig = InControlRigInstance == SectionRig;
		...
		for (const TWeakObjectPtr<UTickableConstraint>& TickConstraint : Constraints)
		{
			...
			if (ParentControlHandle && ShouldResolveParent(ParentControlHandle))
			{
				if (bIsGameWorld)
				{
					...
					{
						FConstraintDependencyScope Scope(TransformConstraint);
				...
				{
					// resolve to the incoming rig instance if it's the default rig for the section or if the previous one was a game instance
					if (WasAGameInstance(ParentControlHandle->ControlRig.Get()) || bIsRigInstanceSectionRig)
					{
						FConstraintDependencyScope Scope(TransformConstraint);
						ParentControlHandle->ResolveBoundObjects(InSequenceID, InSharedPlaybackState, InControlRigInstance);
						TransformConstraint->EnsurePrimaryDependency(BoundObjectWorld);
...
{
	static void ForEachEntity(const FMovieSceneControlRigSpaceChannel* Channel,
		const FControlRigSourceData& ControlRigSource, FFrameTime EvalTime, FMovieSceneControlRigSpaceBaseKey& OutValue)
	{
		UMovieSceneControlRigParameterSection* Section = ControlRigSource.Section.Get();
		if (!Section)
		{
			return;
		}

		// Apply the section's time remapping (play rate), matching what FEvaluateBaseControlRigs does.
		const FFrameTime WarpedTime = Section->RemapTime(EvalTime);
		...
		// its transform. (float/double channels will interpolate to a value pretty close to the one at 24400 as its based
		// on that 0.999928 subframe value.
		Channel->Evaluate(WarpedTime.RoundToFrame(), OutValue);
	}
};
...
struct FGatherControlRigParameterValues
{
	...
	void ForEachAllocation(FEntityAllocationIteratorItem Item, const FAccumulatedControlEntryIndex* EntryIndices, const FName* ParameterNames, 
...
void UMovieSceneControlRigParameterEvaluatorSystem::OnInstantiation()
{
	...
	else
	{
		...
		.PassthroughFilter(FEntityComponentFilter().All({ BuiltInComponents->WeightAndEasingResult }))
		.PassthroughFilter(FEntityComponentFilter().All({ TracksComponents->Tags.AnimMixerPoseProducer}))
		.FilterNone({ BuiltInComponents->Tags.NeedsUnlink })
		.Iterate_PerEntity(&Linker->EntityManager, [this, &EncounteredRigs](FMovieSceneEntityID EntityID, const FGuid& ObjectBindingID, FRootInstanceHandle RootInstanceHandle, const FMovieSceneSequenceID* OptSequenceID,
				const FControlRigSourceData& ControlRigSource, FBaseControlRigEvalData& OutBaseData, bool bWantsRestoreState, bool bHasWeight, bool bAnimMixerPoseProducer)
			{

				OutBaseData.bIsActive = false;
				OutBaseData.bHasWeight = bHasWeight;
				OutBaseData.bAnimMixerPoseProducer = bAnimMixerPoseProducer;
	...
	// --------------------------------------------------------------------------------------------------------
	// Check for parameter entities whose sections were reconstructed during rig binding
	// (e.g., procedural rigs adding controls). Repair stale channels if possible or mark stale entities for reimport.
	RepairOrInvalidateStaleParameterEntities();
...
void UMovieSceneControlRigParameterEvaluatorSystem::RepairOrInvalidateStaleParameterEntities()
{
	using namespace UE::MovieScene;
	...
	// Collect entities that need to be unlinked (can't add/remove components during iteration)
	TArray<FMovieSceneEntityID> EntitiesToUnlink;

	// Iterate all control rig parameter entities and check if their section has been
	// reconstructed since they were imported. This can happen when procedural rigs
	// add controls during binding (in InitializeBaseRigComponent), which causes
	// RecreateWithThisControlRig to be called, potentially invalidating channel pointers.
	// We repair stale entities by updating their channel pointers directly.
	FEntityTaskBuilder()
	.ReadEntityIDs()
	.Write(ControlRigComponents->ControlRigSource)
	.WriteOptional(ControlRigComponents->SpaceChannel)
	.WriteOptional(BuiltInComponents->BoolChannel)
	.WriteOptional(BuiltInComponents->ByteChannel)
	.WriteOptional(BuiltInComponents->IntegerChannel)
	.WriteOptional(BuiltInComponents->FloatChannel[0])
	.WriteOptional(BuiltInComponents->FloatChannel[1])
	.WriteOptional(BuiltInComponents->FloatChannel[2])
	.WriteOptional(BuiltInComponents->FloatChannel[3])
	.WriteOptional(BuiltInComponents->FloatChannel[4])
	.WriteOptional(BuiltInComponents->FloatChannel[5])
	.WriteOptional(BuiltInComponents->FloatChannel[6])
	.WriteOptional(BuiltInComponents->FloatChannel[7])
	.WriteOptional(BuiltInComponents->FloatChannel[8])
	.FilterAll({ ControlRigComponents->Tags.ControlRigParameter })
	.FilterNone({ BuiltInComponents->Tags.NeedsUnlink, BuiltInComponents->BlendChannelOutput })
	.Iterate_PerEntity(&Linker->EntityManager,
		[&EntitiesToUnlink](
			...
			FSourceBoolChannel* BoolChannel,
			FSourceByteChannel* ByteChannel,
			FSourceIntegerChannel* IntegerChannel,
			FSourceFloatChannel* FloatChannel0,
			FSourceFloatChannel* FloatChannel1,
			FSourceFloatChannel* FloatChannel2,
			FSourceFloatChannel* FloatChannel3,
			FSourceFloatChannel* FloatChannel4,
			FSourceFloatChannel* FloatChannel5,
			FSourceFloatChannel* FloatChannel6,
			FSourceFloatChannel* FloatChannel7,
			FSourceFloatChannel* FloatChannel8)
		{
			UMovieSceneControlRigParameterSection* Section = ControlRigSource.Section.Get();
			if (!Section)
			{
			...
			uint32 CurrentHash = Section->GetLastControlsHash();
			if (CurrentHash == ControlRigSource.ImportedControlsHash)
			{
				// Not stale, nothing to do
				return;
			}

			// Section was reconstructed since this entity was imported - repair channel pointers
			int32 EntityIndex = 0;
			EControlRigEntityType EntityType;
			DecodeControlRigEntityID(ControlRigSource.ImportedEntityID, EntityIndex, EntityType);
			...
			case EControlRigEntityType::Space:
			{
				...
				if (SpaceChannel && EntityIndex < SpaceChannels.Num())
				{
					*SpaceChannel = &SpaceChannels[EntityIndex].SpaceCurve;
					ControlRigSource.ImportedControlsHash = CurrentHash;
					bFoundChannel = true;
				}
				break;
			}
			case EControlRigEntityType::BoolParameter:
			{
				...
				if (BoolChannel && EntityIndex < BoolParams.Num())
				{
					BoolChannel->Source = &BoolParams[EntityIndex].ParameterCurve;
					ControlRigSource.ImportedControlsHash = CurrentHash;
					bFoundChannel = true;
				}
				break;
			...
			{
				TArray<FEnumParameterNameAndCurve>& EnumParams = Section->GetEnumParameterNamesAndCurves();
				if (ByteChannel && EntityIndex < EnumParams.Num())
				{
					ByteChannel->Source = &EnumParams[EntityIndex].ParameterCurve;
					ControlRigSource.ImportedControlsHash = CurrentHash;
					bFoundChannel = true;
				}
				break;
			...
			{
				TArray<FIntegerParameterNameAndCurve>& IntParams = Section->GetIntegerParameterNamesAndCurves();
				if (IntegerChannel && EntityIndex < IntParams.Num())
				{
					IntegerChannel->Source = &IntParams[EntityIndex].ParameterCurve;
					ControlRigSource.ImportedControlsHash = CurrentHash;
					bFoundChannel = true;
				}
				break;
			...
			{
				TArray<FScalarParameterNameAndCurve>& ScalarParams = Section->GetScalarParameterNamesAndCurves();
				if (FloatChannel0 && EntityIndex < ScalarParams.Num())
				{
					FloatChannel0->Source = &ScalarParams[EntityIndex].ParameterCurve;
					ControlRigSource.ImportedControlsHash = CurrentHash;
					bFoundChannel = true;
				}
				break;
			...
			{
				// Vector entities can be Vector2D or Vector3 based on index
				TArray<FVector2DParameterNameAndCurves>& Vector2DParams = Section->GetVector2DParameterNamesAndCurves();
				if (EntityIndex < Vector2DParams.Num())
				{
					FVector2DParameterNameAndCurves& Vec2D = Vector2DParams[EntityIndex];
					// Update whichever float channels exist on the entity.
					// Channels are conditionally added based on HasAnyData() at import time,
					// so a valid Vector2D entity may have only FloatChannel0 or FloatChannel1.
					if (FloatChannel0)
					{
						FloatChannel0->Source = &Vec2D.XCurve;
					}
					if (FloatChannel1)
					{
						FloatChannel1->Source = &Vec2D.YCurve;
					}
					if (FloatChannel0 || FloatChannel1)
					{
						ControlRigSource.ImportedControlsHash = CurrentHash;
						bFoundChannel = true;
					}
				...
				{
					int32 VectorIndex = EntityIndex - Vector2DParams.Num();
					TArray<FVectorParameterNameAndCurves>& VectorParams = Section->GetVectorParameterNamesAndCurves();
					if (VectorIndex >= 0 && VectorIndex < VectorParams.Num())
					{
						if (FloatChannel0 && FloatChannel1 && FloatChannel2)
						{
							FVectorParameterNameAndCurves& Vec = VectorParams[VectorIndex];
							FloatChannel0->Source = &Vec.XCurve;
							FloatChannel1->Source = &Vec.YCurve;
							FloatChannel2->Source = &Vec.ZCurve;
							ControlRigSource.ImportedControlsHash = CurrentHash;
							bFoundChannel = true;
						}
					}
			...
			{
				TArray<FTransformParameterNameAndCurves>& TransformParams = Section->GetTransformParameterNamesAndCurves();
				if (EntityIndex < TransformParams.Num())
				{
					if (FloatChannel0 && FloatChannel1 && FloatChannel2 &&
						FloatChannel3 && FloatChannel4 && FloatChannel5 &&
						FloatChannel6 && FloatChannel7 && FloatChannel8)
					{
						FTransformParameterNameAndCurves& Transform = TransformParams[EntityIndex];
						FloatChannel0->Source = &Transform.Translation[0];
						FloatChannel1->Source = &Transform.Translation[1];
						FloatChannel2->Source = &Transform.Translation[2];
						FloatChannel3->Source = &Transform.Rotation[0];
						FloatChannel4->Source = &Transform.Rotation[1];
						FloatChannel5->Source = &Transform.Rotation[2];
						FloatChannel6->Source = &Transform.Scale[0];
						FloatChannel7->Source = &Transform.Scale[1];
						FloatChannel8->Source = &Transform.Scale[2];
						ControlRigSource.ImportedControlsHash = CurrentHash;
						bFoundChannel = true;
					}
				}
			...
			if (!bFoundChannel)
			{
				// Channel no longer exists - collect for unlinking outside iteration
				EntitiesToUnlink.Add(EntityID);
			}
		}
	);

	// Mark entities whose channels no longer exist for unlink
	for (FMovieSceneEntityID EntityID : EntitiesToUnlink)
	{
		if (Linker->EntityManager.HasComponent(EntityID, BuiltInComponents->Tags.NeedsLink))
		{
			Linker->EntityManager.RemoveComponent(EntityID, BuiltInComponents->Tags.NeedsLink);
		}
		Linker->EntityManager.AddComponent(EntityID, BuiltInComponents->Tags.NeedsUnlink);
	}
}
...
void UMovieSceneControlRigParameterEvaluatorSystem::OnSchedulePersistentTasks(UE::MovieScene::IEntitySystemScheduler* TaskScheduler)
{
	...
	// Evaluate space channels. Space entities have SpaceChannel, ControlRigSource, and EvalTime
	// (EvalTime is added via mutual inclusion with SpaceChannel, see MovieSceneControlRigComponentTypes.cpp).
	// We read ControlRigSource to apply the section's time remapping (play rate) inside the task.
	...
	TMap<UControlRig*, UE::MovieScene::FBaseControlRigEvalData*>& OutBaseComponentTracker)
{
	...
	if (!ControlRig)
	{
		if (GameWorld)
		{
			ControlRig = Track->GetGameWorldControlRig(GameWorld, BoundObject);
		}
		else
		{
	...
	// The anim-mixer case is differentiated inside RestorePreAnimatedValue via FPreAnimatedControlRigState::bAnimMixerPoseProducer.
	...
	// Stamp bAnimMixerPoseProducer onto the state so the restore path can branch the same way the init path did.
	this->ControlRigStorage->CachePreAnimatedValue(
		FCachePreAnimatedValueParams(), ControlRig,
		[bAnimMixerPoseProducer](const FObjectKey& InObject)
		{
			FPreAnimatedControlRigState State = FPreAnimatedControlRigTraits::CachePreAnimatedValue(InObject);
			State.bAnimMixerPoseProducer = bAnimMixerPoseProducer;
			return State;
		});
...
#if WITH_EDITOR	
	// The anim mixer handles binding to the skeletal mesh component
	TWeakObjectPtr<UAnimInstance> PreviousAnimInstanceWeakPtr = nullptr;
	if (!bAnimMixerPoseProducer)
	{
		if (ControlRig->GetObjectBinding())
		{
	...
	// We bind to sequencer instance if we're not an anim mixer pose producer or if our bound object is not a skm component.
	bool bWasCreated = false;
	if (!bAnimMixerPoseProducer || (ControlRig->GetObjectBinding() && Cast<USkeletalMeshComponent>(ControlRig->GetObjectBinding()->GetBoundObject()) == nullptr))
	{
		bWasCreated = FControlRigBindingHelper::BindToSequencerInstance(ControlRig);
	}
	
	this->AccumulatedValues.InitializeRig(Track, ControlRig);

#if WITH_EDITOR
	if (!bAnimMixerPoseProducer && GEditor && bWasCreated)
	{
		if (ControlRig->GetObjectBinding())
		{
			if (USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(ControlRig->GetObjectBinding()->GetBoundObject()))
