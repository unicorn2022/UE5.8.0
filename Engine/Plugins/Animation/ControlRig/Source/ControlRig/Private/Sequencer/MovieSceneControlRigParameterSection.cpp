...
namespace UE::MovieScene
{
	...
	void InterrogateTrack(UMovieSceneTrack* Track, const FMovieSceneContext& Context, FMovieSceneInterrogationData& Container, UObject* BindingOverride)
	{
		if (Track)
		{
			UMovieSceneControlRigParameterTrack* ControlRigTrack = Cast<UMovieSceneControlRigParameterTrack>(Track);
			if (ControlRigTrack && ControlRigTrack->GetLegacyEvaluationTrack())
			{
				return ControlRigTrack->GetLegacyEvaluationTrack()->Interrogate(Context, Container, BindingOverride);
			}

			FMovieSceneEvaluationTrack EvalTrack = CastChecked<IMovieSceneTrackTemplateProducer>(Track)->GenerateTrackTemplate(Track);
			return EvalTrack.Interrogate(Context, Container, BindingOverride);
		}
	}
		...
		float& OutValue, float& OutWeight)
	{
		...
		if (Track)
		{
			FMovieSceneInterrogationData InterrogationData;
			{
				...
				UE::MovieScene::InterrogateTrack(Track, Context, InterrogationData, Object);
			}
...
struct FParameterTransformChannelEditorData
{
		...
		float& OutValue, float& OutWeight)
	{
		FMovieSceneInterrogationData InterrogationData;
		{
			RootTemplate.CopyActuators(InterrogationData.GetAccumulator());
			FMovieSceneContext Context(FMovieSceneEvaluationRange(KeyTime, TickResolution));
			UE::MovieScene::InterrogateTrack(SectionToKey->GetTypedOuter<UMovieSceneTrack>(), Context, InterrogationData, Object);
		}
...
UMovieSceneControlRigParameterSection::UMovieSceneControlRigParameterSection() :bDoNotKey(false)
{
	...
	// If we are in the process of loading, we can't be certain that we aren't already inside one of these delegate's broadcasts
...
FMovieSceneTimeWarpVariant* UMovieSceneControlRigParameterSection::GetTimeWarp()
{
	return &PlayRate;
}

bool UMovieSceneControlRigParameterSection::HasTimeWarp() const
{
	UMovieSceneTimeWarpGetter* Getter = PlayRate.GetType() == EMovieSceneTimeWarpType::Custom
		? PlayRate.AsCustom()
		: nullptr;
...
void UMovieSceneControlRigParameterSection::BeginDestroy()
{
	Super::BeginDestroy();
#if WITH_EDITOR
...
{
	// When loading older assets, make sure we carry over the Control Rig class it was built with.
	if (ControlRigClass_DEPRECATED)
	{
		ControlRigAssetReference = FControlRigAssetStrongReference(ControlRigClass_DEPRECATED);
	}
	...
	BindEditorDelegates();
#endif
	PostLoadConstraintChannels();
}

void UMovieSceneControlRigParameterSection::PostLoadConstraintChannels()
{
	if (ControlRig)
	{
		const FSoftObjectPath RigPath(ControlRig);
		auto UpdateHandle = [&RigPath, NewControlRig = ControlRig.Get(), this](const TObjectPtr<UTransformableHandle>& Handle)
		{
			if (UTransformableControlHandle* ControlHandle = ::IsValid(Handle) ? Cast<UTransformableControlHandle>(Handle.Get()) : nullptr)
			{
				const FSoftObjectPath& HandleRigPath = ControlHandle->ControlRig.ToSoftObjectPath();
				if (RigPath.IsSubobject() && HandleRigPath.IsSubobject())
				{
					const FSoftObjectPath HandleRigPathInThisAsset(RigPath.GetAssetPath(), HandleRigPath.GetSubPathUtf8String());					
					if (HandleRigPathInThisAsset == RigPath)
					{
						ControlHandle->UpdateControlRig(NewControlRig);						
					}
				}
			}						
		};

		// update the constraints' handles related to this ControlRig object that live within this section 
		for (FConstraintAndActiveChannel& ConstraintChannel : ConstraintsChannels)
		{
			if (UTickableTransformConstraint* TransformConstraint = Cast<UTickableTransformConstraint>(ConstraintChannel.GetConstraint()))
			{
				UpdateHandle(TransformConstraint->ChildTRSHandle);
				UpdateHandle(TransformConstraint->ParentTRSHandle);
			}
		}

		// update the constraints' handles related to this ControlRig object that live within other constrained sections
		if (const UMovieScene* OuterMovieScene = GetTypedOuter<UMovieScene>())
		{
			static const TSet< TSubclassOf<UMovieSceneTrack> > ConstrainedTracks(
			{
				UMovieSceneControlRigParameterTrack::StaticClass(),
				UMovieScene3DTransformTrack::StaticClass()
			});

			const UObject* OuterTrack = GetOuter();
			auto ProcessSection = [OuterTrack, &UpdateHandle](UMovieSceneSection* InSection)
			{
				if (InSection && InSection->GetOuter() != OuterTrack)
				{
					if (IMovieSceneConstrainedSection* ConstrainedSection = Cast<IMovieSceneConstrainedSection>(InSection))
					{
			...
			auto ProcessTrack = [&ProcessSection, OuterTrack](const UMovieSceneTrack* InTrack)
			{
				if (InTrack && InTrack != OuterTrack)
				{
					if (ConstrainedTracks.Contains(InTrack->GetClass()))
					{
			...
			for (const FMovieSceneBinding& Binding : OuterMovieScene->GetBindings())
			{
				for (const TSubclassOf<UMovieSceneTrack>& TrackClass: ConstrainedTracks)
				{
					for (UMovieSceneTrack* Track: OuterMovieScene->FindTracks(TrackClass, Binding.GetObjectGuid()))
					{
						ProcessTrack(Track);
					}
				}
			...
			for (UMovieSceneTrack* Track: OuterMovieScene->GetTracks())
			{
				ProcessTrack(Track);
			}
		}
...
void UMovieSceneControlRigParameterSection::AddConstraintChannel(UTickableConstraint* InConstraint)
{
	if (InConstraint && !HasConstraintChannel(InConstraint->ConstraintID))
	{
		...
		InConstraint->Rename(nullptr, this, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
...
uint32 UMovieSceneControlRigParameterSection::ComputeControlsHash(const TArray<FRigControlElement*>& Controls)
{
	uint32 Hash = GetTypeHash(Controls.Num());
	for (const FRigControlElement* Control : Controls)
	{
		if (Control)
		{
			Hash = HashCombine(Hash, GetTypeHash(Control->GetFName()));
			Hash = HashCombine(Hash, GetTypeHash(static_cast<uint8>(Control->Settings.ControlType)));
		}
	}
	return Hash;
}
...
{
	return ComputeControlsHash(NewControls) != LastControlsHash;
}
...
{
	LastControlsHash = ComputeControlsHash(NewControls);
}
...
void UMovieSceneControlRigParameterSection::HACK_FixMultipleParamsWithSameName()
{
	...
	if (bHasDup)
	{
		...
		if (Choice == EAppReturnType::Yes)
		{
			...
			{
				UE_LOGF(LogControlRig, Warning, "There were duplicated curves found possibly by 40400084, resave to remove duplicates and keep first one");
...
EMovieSceneChannelProxyType UMovieSceneControlRigParameterSection::CacheChannelProxy()
{
	...
	if (ControlRig)
	{
		...
		{
			if (PlayRate.GetType() == EMovieSceneTimeWarpType::Custom)
			{
				UMovieSceneTimeWarpGetter* Custom = PlayRate.AsCustom();
				if (Custom)
				{
			...
			int32 MaskIndex = 0;
			int32 SortOrder = 2; //start with 2 so Timewarp then Weight are first 
						...
						if (Handle && Handle->GetHash() == ControlHash)
						{
							...
							if (WeakConstraint.IsValid())
							{
								...
								ConstraintChannel.ActiveChannel.ExtraLabel = [WeakConstraint]
								{
									if (WeakConstraint.IsValid())
									{
										constexpr TCHAR ParentChildDelimiter[] = TEXT("|");
										FString ParentStr; WeakConstraint->GetLabel().Split(ParentChildDelimiter, &ParentStr, nullptr);
...
bool UMovieSceneControlRigParameterSection::RenameParameterName(const FName& OldParameterName, const FName& NewParameterName, TOptional<ERigControlType> ControlType)
{
	...
	for (FSpaceControlNameAndChannel& SpaceChannel : SpaceChannels)
	{
		if (SpaceChannel.ControlName == OldParameterName)
		{
			SpaceChannel.ControlName = NewParameterName;
			bWasReplaced = true;
		}
		//update spaces
		TMovieSceneChannelData<FMovieSceneControlRigSpaceBaseKey> ChannelData = SpaceChannel.SpaceCurve.GetData();
		TArrayView<const FFrameNumber> Times = ChannelData.GetTimes();
		TArrayView<FMovieSceneControlRigSpaceBaseKey> Values = ChannelData.GetValues();
		for (FMovieSceneControlRigSpaceBaseKey& Key : Values)
		{
			if (Key.ControlRigElement.Name == OldParameterName)
			{
				Key.ControlRigElement.Name = NewParameterName;
			}
		}
	}
	for (FConstraintAndActiveChannel& ConstraintChannel : ConstraintsChannels)
	{
		if (UTickableTransformConstraint* TransformConstraint = Cast<UTickableTransformConstraint>(ConstraintChannel.GetConstraint()))
		{
			...
			{
				if (Handle->ControlName == OldParameterName)
				{
					Handle->ControlName = NewParameterName;
					bWasReplaced = true;
				}
			}
		}
...
void UMovieSceneControlRigParameterSection::BindEditorDelegates()
{
	OnControlRigEditorSettingChangedHandle = UControlRigEditorSettings::Get()->OnSettingChanged().AddUObject(this, &UMovieSceneControlRigParameterSection::OnControlRigEditorSettingChanged);
}

void UMovieSceneControlRigParameterSection::UnbindEditorDelegates()
{
	UControlRigEditorSettings::Get()->OnSettingChanged().Remove(OnControlRigEditorSettingChangedHandle);
}
#endif
...
void UMovieSceneControlRigParameterSection::SetControlRig(UControlRig* InControlRig)
{
	...
	ControlRig = InControlRig;
	ControlRigAssetReference = ControlRig ? ControlRig->GetAssetReference() : nullptr;
                                                                       ...
                                                                       const  TOptional<EEulerRotationOrder>& NewOrder, EMovieSceneKeyInterpolation Interpolation)
{
	...
	if (FRigControlElement* ControlElement = GetControlRig()->FindControl(InControlName))
	{
			...
			ControlElement->Settings.ControlType == ERigControlType::TransformNoScale)
		{
			...
			if (!(FloatChannels.IsValidIndex(XIndex) && FloatChannels.IsValidIndex(YIndex) && FloatChannels.IsValidIndex(ZIndex)))
			{
				return;
			}
			...
			auto UpdateRotator = [&OldOrder, &NewOrder](FRotator& InOutRotator)
			{
				constexpr bool bUseUEHandiness = true;
				FQuat Quat;
				
				//if set use animation core conversion, else use rotator conversion
				if (OldOrder.IsSet())
				{
					const FVector Vector = InOutRotator.Euler();
					Quat = AnimationCore::QuatFromEuler(Vector, OldOrder.GetValue(), bUseUEHandiness);
				}
				else
				{
					Quat = FQuat(InOutRotator);
				}
				
				if (NewOrder.IsSet())
				{
					const FVector Vector = AnimationCore::EulerFromQuat(Quat, NewOrder.GetValue(), bUseUEHandiness);
					InOutRotator = FRotator::MakeFromEuler(Vector);
				}
				else
				{
					InOutRotator = FRotator(Quat);
				}
			};
			
			// no keys, update defaults
			if (KeyTimes.IsEmpty())
			{
				const TOptional<float>& OptRoll = FloatChannels[XIndex]->GetDefault();
				const TOptional<float>& OptPitch = FloatChannels[YIndex]->GetDefault();
				const TOptional<float>& OptYaw = FloatChannels[ZIndex]->GetDefault();
				if (OptRoll || OptPitch || OptYaw)
				{
					FRotator Rotator(OptPitch ? *OptPitch : 0.f, OptYaw ? *OptYaw : 0.f, OptRoll ? *OptRoll : 0.f);
					UpdateRotator(Rotator);
					
					FloatChannels[XIndex]->SetDefault(Rotator.Roll);
					FloatChannels[YIndex]->SetDefault(Rotator.Pitch);
					FloatChannels[ZIndex]->SetDefault(Rotator.Yaw);
				}
				return;
			}
...
void UMovieSceneControlRigParameterSection::SetControlNameMask(const FName& Name, bool bValue, bool bReconstructChannel)
{
	if (bValue == false)
	{
	...
	if (FRigControlElement* ControlElement = GetControlRig()->FindControl(ControlName))
	{
			...
			ControlElement->Settings.ControlType == ERigControlType::TransformNoScale)
		{
			...
			for (int32 Index = 0; Index < 3; ++Index)
			{
				...
				FloatChannels[RealIndex]->AutoSetTangents();
			}
		}
	}
}
...
namespace UE::Private
{
	void SetOrAddKey(TMovieSceneChannelData<FMovieSceneFloatValue>& ChannelData, FFrameNumber Time, float Value, const EMovieSceneKeyInterpolation Interpolation)
	{
		...
		{
			FMovieSceneFloatValue& FloatValue = ChannelData.GetValues()[ExistingIndex];
...
void UMovieSceneControlRigParameterSection::RecordControlRigKey(FFrameNumber FrameNumber, bool bSetDefault, EMovieSceneKeyInterpolation InInterpMode, bool bbOntoSelectedControls)
{
	if (ControlRig && ControlRig->GetHierarchy())
	{
		...
		auto WindRotation = [&FloatChannels](FVector& InOutRotation, const int32 ChannelIndex, const uint8 Axis)
		{
			if ((Axis < 3) && FloatChannels.IsValidIndex(ChannelIndex+Axis))
			{
				FMovieSceneFloatChannel* RotChannel = FloatChannels[ChannelIndex+Axis];
				const int32 NumKeys = RotChannel->GetNumKeys();
				if (NumKeys > 0)
				{
					const float LastVal = RotChannel->GetValues().Last().Value;
					FMath::WindRelativeAnglesDegrees(LastVal, InOutRotation[Axis]);
				}
			}
		};
		...
		for (FRigControlElement* ControlElement : Controls)
		{
			...
			const FRigControlValue CurrentValue = ControlRig->GetControlValue(ControlElement, ERigControlValueType::Current);
			
			switch (ControlElement->Settings.ControlType)
			{
				case ERigControlType::Bool:
				{
					bool Val = CurrentValue.Get<bool>();
					if (bSetDefault)
					{
					...
					if (ExistingIndex != INDEX_NONE)
					{
						bool& BValue = BoolChannels[ChannelIndex]->GetData().GetValues()[ExistingIndex];
						BValue = Val;
					}
					else
					{
				...
				case ERigControlType::Integer:
				{
					if (ControlElement->Settings.ControlEnum)
					{
						uint8 Val = (uint8)CurrentValue.Get<uint8>();
						if (bSetDefault)
						{
						...
						if (ExistingIndex != INDEX_NONE)
						{
							uint8& BValue = EnumChannels[ChannelIndex]->GetData().GetValues()[ExistingIndex];
							BValue = Val;
						}
						else
						{
					...
					{
						int32 Val = CurrentValue.Get<int32>();
						if (bSetDefault)
						{
						...
						if (ExistingIndex != INDEX_NONE)
						{
							int32& BValue = IntChannels[ChannelIndex]->GetData().GetValues()[ExistingIndex];
							BValue = Val;
						}
						else
						{
				...
				case ERigControlType::Vector2D:
				{
					FVector3f Val = CurrentValue.Get<FVector3f>();
					if (bSetDefault)
					{
						FloatChannels[ChannelIndex]->SetDefault(Val.X);
						FloatChannels[ChannelIndex + 1]->SetDefault(Val.Y);
				...
				case ERigControlType::Rotator:
				{
					...
					if (ControlElement->Settings.ControlType == ERigControlType::Rotator)
					{
						FVector Rotation = Hierarchy->GetControlSpecifiedEulerAngle(ControlElement);
						
						WindRotation(Rotation, ChannelIndex, 0);
						WindRotation(Rotation, ChannelIndex, 1);
						WindRotation(Rotation, ChannelIndex, 2);
						
						Val.X = static_cast<float>(Rotation.X);
						Val.Y = static_cast<float>(Rotation.Y);
						Val.Z = static_cast<float>(Rotation.Z);
					}
				...
				case ERigControlType::EulerTransform:
				{
					...
					if (ControlElement->Settings.ControlType == ERigControlType::TransformNoScale)
					{
						FTransformNoScale NoScale = CurrentValue.Get<FRigControlValue::FTransformNoScale_Float>().ToTransform();
						Val = NoScale;
					}
					else if (ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
					{
						FEulerTransform Euler = CurrentValue.Get<FRigControlValue::FEulerTransform_Float>().ToTransform();
						Val = Euler.ToFTransform();
					}
					else
					{
						Val = CurrentValue.Get<FRigControlValue::FTransform_Float>().ToTransform();
					}
					
					// set translation
					{
						const FVector Translation = Val.GetTranslation();
						if (bSetDefault)
						{
							FloatChannels[ChannelIndex]->SetDefault(Translation.X);
							FloatChannels[ChannelIndex + 1]->SetDefault(Translation.Y);
							FloatChannels[ChannelIndex + 2]->SetDefault(Translation.Z);
						}
						AddVectorKeyToFloatChannels(ChannelIndex, FrameNumber, Translation);
					}
					...
					// set rotation
					{
						...
						if (bSetDefault)
						{
							FloatChannels[ChannelIndex]->SetDefault(Rotation.X);
							FloatChannels[ChannelIndex + 1]->SetDefault(Rotation.Y);
							FloatChannels[ChannelIndex + 2]->SetDefault(Rotation.Z);
						}
						AddVectorKeyToFloatChannels(ChannelIndex, FrameNumber, Rotation);
					}
						...
						ControlElement->Settings.ControlType == ERigControlType::EulerTransform)
					{
						...
						if (bSetDefault)
						{
							FloatChannels[ChannelIndex]->SetDefault(Scale.X);
							FloatChannels[ChannelIndex + 1]->SetDefault(Scale.Y);
							FloatChannels[ChannelIndex + 2]->SetDefault(Scale.Z);
						}
						AddVectorKeyToFloatChannels(ChannelIndex, FrameNumber, Scale);
					}
					break;
				}
			}
...
bool UMovieSceneControlRigParameterSection::LoadAnimSequenceIntoThisSection(UAnimSequence* AnimSequence, const FFrameNumber& SequenceStart, UMovieScene* MovieScene, UObject* BoundObject, const FLoadAnimSequenceData& LoadData, EMovieSceneKeyInterpolation InInterpolation)
{
	...
	*/
	...
	{
		if(!ControlRig->GetAssetReference().IsNative())
		{
			SourceHierarchy->CopyHierarchy(ControlRig->GetAssetReference().GetDefaultHierarchy());
		}
	}
	...
	for (; Index <= EndIndex; ++Index)
	{
		...
		//do not retarget if FKControlRig 
		EvaluationOptions.bShouldRetarget = (EvaluationOptions.OptionalSkeletalMesh && AutoRig == nullptr) ? LoadData.bShouldRetarget : false;
		if (EvaluationOptions.bShouldRetarget == true)
		{
			if (USkeleton* NewSkeleton = EvaluationOptions.OptionalSkeletalMesh->GetSkeleton())
			{
				if (NewSkeleton == AnimSequence->GetSkeleton() && NewSkeleton->GetGuid() == AnimSequence->GetSkeletonGuid())
				{
...
UControlRig* UMovieSceneControlRigParameterSection::GetControlRig() const
{
	return GetControlRig(nullptr, nullptr);
}

UControlRig* UMovieSceneControlRigParameterSection::GetControlRig(UWorld* InGameWorld, UObject* InBoundObject) const
{
	...
	if (UMovieSceneControlRigParameterTrack* Track = GetTypedOuter<UMovieSceneControlRigParameterTrack>())
	{
		return Track->GetGameWorldControlRig(InGameWorld, InBoundObject);
	}
	return nullptr;
}
...
FFrameTime UMovieSceneControlRigParameterSection::RemapTime(const FFrameTime& InFrameTime) const
{
	UMovieSceneTimeWarpGetter* Custom = PlayRate.GetType() == EMovieSceneTimeWarpType::Custom
		? PlayRate.AsCustom()
		: nullptr;

	if (Custom && Custom->IsMuted() == false)
	{
		return PlayRate.RemapTime(InFrameTime);
	}
	return InFrameTime;
}

void UMovieSceneControlRigParameterSection::DeleteTimeWarp()
{
	if (HasTimeWarp())
	{
		Modify();

		// Reset to the default 1:1 play rate so this section no longer reports a custom time warp
		PlayRate.Set(1.0);
...
void UMovieSceneControlRigParameterSection::ImportEntityImpl(UMovieSceneEntitySystemLinker* EntityLinker, const FEntityImportParams& Params, FImportedEntity* OutImportedEntity)
{
	...
	ControlRigSource.Section = const_cast<UMovieSceneControlRigParameterSection*>(this);
	ControlRigSource.ImportedControlsHash = GetLastControlsHash();
	ControlRigSource.ImportedEntityID = Params.EntityID;
	...
	switch (EntityType)
	{
		...
		case EControlRigEntityType::BoolParameter:
		{
			...
			OutImportedEntity->AddBuilder(
				...
				.AddDefaulted(BuiltInComponentTypes->EvalTime)
				.AddConditional(BuiltInComponentTypes->TimeWarp, Custom, Custom && Custom->IsMuted() == false)
				.AddTag(ControlRigComponents->Tags.ControlRigParameter)
