...
void UTransformableControlHandle::SetGlobalTransform(const FTransform& InGlobal) const
{
	...
	const FTransform ComponentTransform = TransformableHandleUtils::SkipTicking() ?
		TransformableHandleUtils::GetGlobalTransform(BoundComponent, NAME_None) : 
		BoundComponent->GetComponentTransform();
...
bool UTransformableControlHandle::HasIndirectDependencyWith(const UTransformableHandle& InOther) const
{
	const uint32 OtherHash = InOther.GetHash();
	if (OtherHash == 0 || OtherHash == GetHash())
	{
		return false;
	}
	...
	const UTransformableControlHandle* OtherCtrlHandle = InOther.IsValid() ? Cast<UTransformableControlHandle>(&InOther) : nullptr;
	const FRigControlElement* OtherControlElement = OtherCtrlHandle ? OtherCtrlHandle->GetControlElement() : nullptr;
	if (!OtherCtrlHandle || !OtherControlElement || OtherCtrlHandle->ControlRig != ControlRig)
	{
		return false;
	}
	...
	const FRigBaseElementParentArray& Parents = Hierarchy->GetParents(ControlElement, bRecursive);
	const FRigBaseElementParentArray& OtherParents = Hierarchy->GetParents(OtherControlElement, bRecursive);

	int32 Index = Parents.Num(), OtherIndex = OtherParents.Num();
	auto HasCommonAncestor = [&Index, &Parents, &OtherIndex, &OtherParents, OtherControlElement, ControlElement]()
	{
	...
	if (OtherControlElement->GetIndex() < ControlElement->GetIndex())
	{
		return true;
	}
...
{
	if (!InControlRig || !InControl || ControlName == NAME_None)
	{
		return;
	}
	...
	const bool bThisControl = InControl->GetFName() == ControlName;
	const EHandleEvent Event = InContext.bConstraintUpdate ? EHandleEvent::GlobalTransformUpdated : EHandleEvent::LocalTransformUpdated;
	if (Event == EHandleEvent::LocalTransformUpdated && !bThisControl)
	{
		return;
	}
	
	UControlRig* Rig = ControlRig.Get();
	if (!Rig || Rig != InControlRig)
	{
		return;
	}

	if (HandleModified().IsBound())
	{
		if (bThisControl)
		{
			...
			const UE::Private::RigGuard NotificationGuard([Rig]()
			{
				UE::Private::NotifyingRigs.Remove(Rig);
			});
			UE::Private::NotifyingRigs.Add(Rig);
...
void UTransformableControlHandle::UpdateControlRig(UControlRig* InControlRig)
{
	if (InControlRig && InControlRig != ControlRig)
	{
		UnregisterDelegates();
...
{
	if (!ControlRig.IsValid() || ControlName == NAME_None || !ControlRig->GetHierarchy())
	{
		return false;
	}

	FRigControlElement* ControlElement = ControlRig.Get()->FindControl(ControlName);
	if(!ControlElement)
	{
		return false;
	}
	...
	const bool bUpdateDefault = NumFrames == 0 && NumTransforms == 1;
	if (NumFrames == 0)
	{
		if (!bUpdateDefault)
		{
	...
	else if (NumTransforms != NumFrames)
	{
		return false;
	}

	auto KeyframeFunc = [this, bLocal](const FTransform& InTransform, const FRigControlModifiedContext& InKeyframeContext, FRigControlElement* InControlElement)
	{
		...
		if (bLocal)
		{
			const FVector EulerAngle = InTransform.GetRotation().Rotator().Euler();
			
			InControlRig->GetHierarchy()->SetControlSpecifiedEulerAngle(InControlElement, EulerAngle);
	...
	const FScopedLegacyEvaluationTrack ScopedLegacyEvaluationTrack(InSection);
	
	if (bUpdateDefault)
	{
		KeyframeFunc(InTransforms[0], KeyframeContext, ControlElement);
	}
	else
	{
		...
		for (int32 Index = 0; Index < InFrames.Num(); ++Index)
		{
			...
			KeyframeFunc(InTransforms[Index], KeyframeContext, ControlElement);
		}
...
void UTransformableControlHandle::ResolveBoundObjects(FMovieSceneSequenceID LocalSequenceID, TSharedRef<UE::MovieScene::FSharedPlaybackState> SharedPlaybackState, UObject* SubObject)
{
	if (UControlRig* InControlRig = Cast<UControlRig>(SubObject))
	{
		...
		if (ControlRig && ControlRig->GetAssetReference() != InControlRig->GetAssetReference())
		{
			return;
		}
...
FString UTransformableControlHandle::GetLabel() const
{
	...
	if (!BoundComponent)
	{
		if (ControlRig.IsValid())
		{
			const FString ControlRigLabel = ControlRig->GetName();
			if (const FRigControlElement* Control = ControlRig->FindControl(ControlName))
			{
				return FString::Printf(TEXT("%s(unbound)/%s"), *ControlRigLabel, *Control->GetName() );	
			}
			return FString::Printf(TEXT("%s(unbound)/%s(not found)"), *ControlRigLabel, *ControlName.ToString() );			
		}
...
FString UTransformableControlHandle::GetFullLabel() const
{
	...
	if (ControlRig.IsValid())
	{
		...
		const FString BoundLabel = Actor ? Actor->GetActorLabel() : BoundComponent->GetName();
		return FString::Printf(TEXT("%s/%s/%s"), *BoundLabel, *ControlRig->GetName(), *ControlName.ToString() );
	}
	
	const AActor* Actor = BoundComponent->GetOwner();
	const FString BoundLabel = Actor ? Actor->GetActorLabel() : BoundComponent->GetName();
	return FString::Printf(TEXT("%s/%s"), *BoundLabel, *ControlName.ToString() );
}
...
void UTransformableControlHandle::OnObjectsReplaced(const TMap<UObject*, UObject*>& InOldToNewInstances)
{
	if (UObject* NewObject = InOldToNewInstances.FindRef(ControlRig.Get()))
	{
		...
		{
			UpdateControlRig(NewControlRig);
		}
	}
}
