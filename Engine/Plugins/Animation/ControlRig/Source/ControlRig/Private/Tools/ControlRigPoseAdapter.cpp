	...
	, bool bInResetInputPoseToInitial)
{
	...
	{
		ParentPoseIndices.AddDefaulted();
		...
		{
			ParentPoseIndices[Index] = ParentBoneIndex;
		}
...
const FTransform& FControlRigPoseAdapter::GetLocalTransform(int32 InIndex)
{
	...
	if (LocalPoseIsDirty[InIndex])
	{
		...
		{
			const FCompactPoseBoneIndex ParentIndex = ParentPoseIndices[InIndex];
			...
			{
				LocalPose[InIndex] = GetGlobalTransform(InIndex).GetRelativeTransform(GetGlobalTransform(ParentIndex.GetInt()));
			}
...
const FTransform& FControlRigPoseAdapter::GetGlobalTransform(int32 InIndex)
{
	...
	if (GlobalPoseIsDirty[InIndex])
	{
		...
		else
		{
			...
			{
				GlobalPose[InIndex] = GetLocalTransform(InIndex) * GetGlobalTransform(ParentIndex.GetInt());
			}
