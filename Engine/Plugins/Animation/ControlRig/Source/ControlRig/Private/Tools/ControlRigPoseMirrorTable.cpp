...
{
	FControlRigPoseMirrorContext Context;
	Context.bDoLocal = bDoLocal;
	Context.bIsMatched = bIsMatched;
	GetMirrorTransform(ControlCopy, Context, OutGlobalTransform, OutLocalTransform);
}

void FControlRigPoseMirrorTable::GetMirrorTransform(const FRigControlCopy& ControlCopy, const FControlRigPoseMirrorContext& InContext, FTransform& OutGlobalTransform, FTransform& OutLocalTransform) const
{
	...
	if (Settings)
	{
		if (!InContext.bIsMatched) //still do it if not a matching, need to mirror translation
		{
		...
		{
			const FVector Identity(1.0, 1.0, 1.0);
			if (InContext.TranslationFactor.Equals(Identity, KINDA_SMALL_NUMBER) == false)
			{
				FVector Translate = GlobalTransform.GetTranslation();
				Translate *= InContext.TranslationFactor;
				OutGlobalTransform.SetTranslation(Translate);
				Translate = LocalTransform.GetTranslation();
				Translate *= InContext.TranslationFactor;
				OutLocalTransform.SetTranslation(Translate);
			}
			if (InContext.RotationFactor.Equals(Identity, KINDA_SMALL_NUMBER) == false)
			{
				FVector Euler = GlobalTransform.GetRotation().Euler();
				Euler *= InContext.RotationFactor;
				FQuat Quat;
				Quat = Quat.MakeFromEuler(Euler);
				OutGlobalTransform.SetRotation(Quat);

				Euler = LocalTransform.GetRotation().Euler();
				Euler *= InContext.RotationFactor;
				Quat = Quat.MakeFromEuler(Euler);
				OutLocalTransform.SetRotation(Quat);
			}
		}
	}
}
