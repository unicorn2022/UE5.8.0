...
void FControlRigVariableMappings::InitializeProperties(const UClass* InSourceClass, UObject* TargetInstance,
	FControlRigAssetStrongReference InTargetClass, const TArray<FName>& InSourcePropertyNames, TArray<FName>& InDestPropertyNames)
{
		...
		if (bUseFunctions)
		{
			if (TargetControlRig && TargetHierarchy && SourceProperty)
			{
				...
				else
				{
					...
					if (Variable.IsValid() && !Variable.IsReadOnly())
					{
...
{
	FControlRigAssetStrongReference TargetSource;
	TargetSource.Set(TSubclassOf<UControlRig>(InTargetClass));
	InitializeProperties(InSourceClass, TargetInstance, TargetSource, InSourcePropertyNames, InDestPropertyNames);
}
...
void FControlRigVariableMappings::UpdateCurveInputs(UControlRig* InControlRig, const TMap<FName, FName> &InputMapping, const FBlendedCurve& InCurveData) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	...
	if (InputMapping.Num() > 0 && InControlRig)
	{
		UE::Anim::FCurveUtils::BulkGet(InCurveData, InputCurveMappings,
			[&InControlRig](const FControlRigCurveMapping& InBulkElement, float InValue)
			{
				...
				if (!Variable.IsReadOnly() && Variable.GetExtendedCPPType() == TEXT("float"))
				{
					Variable.SetValue<float>(InValue);
				}
				else
				{
					UE_LOGF(LogAnimation, Warning, "[%ls] Missing Input Variable [%ls]", *GetNameSafe(InControlRig->GetClass()), *InBulkElement.SourceName.ToString());
				}
			});
	}
}

void FControlRigVariableMappings::UpdateCurveOutputs(UControlRig* InControlRig, const TMap<FName, FName> &OutputMapping, FBlendedCurve& InCurveData) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	...
	if (OutputMapping.Num() > 0 && InControlRig)
	{
		UE::Anim::FCurveUtils::BulkSet(InCurveData, OutputCurveMappings,
			...
			{
				FRigVMExternalVariable Variable = InControlRig->GetPublicVariableByName(InBulkElement.SourceName);
				if (Variable.GetExtendedCPPType() == TEXT("float"))
				{
					return Variable.GetValue<float>();
				}
				else
				{
					UE_LOGF(LogAnimation, Warning, "[%ls] Missing Output Variable [%ls]", *GetNameSafe(InControlRig->GetClass()), *InBulkElement.SourceName.ToString());
				}
...
void FControlRigVariableMappings::PropagateInputPropertiesNoCache(const UObject* InSourceInstance, const UControlRig* InTargetControlRig, URigHierarchy* InTargetHierarchy, TArray<FName>& InDestPropertyNames)
{
	...
	for (int32 PropIdx = 0; PropIdx < SourceProperties.Num(); ++PropIdx)
	{
		...
		if (Variable.IsValid())
		{
			...
			if (CastField<FBoolProperty>(CallerProperty) != nullptr && Variable.GetExtendedCPPType() == RigVMTypeUtils::BoolTypeName)
			{
				const bool Value = *(const bool*)SrcPtr;
				Variable.SetValue<bool>(Value);
			}
			else if (CastField<FFloatProperty>(CallerProperty) != nullptr && (Variable.GetExtendedCPPType() == RigVMTypeUtils::FloatTypeName || Variable.GetExtendedCPPType() == RigVMTypeUtils::DoubleTypeName))
			{
				const float Value = *(const float*)SrcPtr;
				if (Variable.GetExtendedCPPType() == RigVMTypeUtils::FloatTypeName)
				{
			...
			else if (CastField<FDoubleProperty>(CallerProperty) != nullptr && (Variable.GetExtendedCPPType() == RigVMTypeUtils::FloatTypeName || Variable.GetExtendedCPPType() == RigVMTypeUtils::DoubleTypeName))
			{
			...
			else if (CastField<FIntProperty>(CallerProperty) != nullptr && Variable.GetExtendedCPPType() == RigVMTypeUtils::Int32TypeName)
			{
				const int32 Value = *(const int32*)SrcPtr;
				Variable.SetValue<int32>(Value);
			}
			else if (CastField<FNameProperty>(CallerProperty) != nullptr && Variable.GetExtendedCPPType() == RigVMTypeUtils::FNameTypeName)
			{
				const FName Value = *(const FName*)SrcPtr;
				Variable.SetValue<FName>(Value);
			}
			else if (CastField<FNameProperty>(CallerProperty) != nullptr && Variable.GetExtendedCPPType() == RigVMTypeUtils::FStringTypeName)
			{
			...
			{
				if (StructProperty->Struct == Variable.GetCPPTypeObject())
				{
					StructProperty->Struct->CopyScriptStruct(Variable.GetMemory(), SrcPtr, 1);
				}
			}
			else if (FArrayProperty* ArrayProperty = CastField<FArrayProperty>(CallerProperty))
			{
				if (ensure(ArrayProperty->SameType(Variable.GetProperty())))
				{
					ArrayProperty->CopyCompleteValue(Variable.GetMemory(), SrcPtr);
				}
			}
			else if (FObjectProperty* ObjectProperty = CastField<FObjectProperty>(CallerProperty))
			{
				if (ensure(ObjectProperty->SameType(Variable.GetProperty())))
				{
					ObjectProperty->CopyCompleteValue(Variable.GetMemory(), SrcPtr);
				}
			}
			else if (FEnumProperty* EnumProperty = CastField<FEnumProperty>(CallerProperty))
			{
				if (ensure(EnumProperty->SameType(Variable.GetProperty())))
				{
					EnumProperty->CopyCompleteValue(Variable.GetMemory(), SrcPtr);
				}
			}
			else if (FByteProperty* ByteProperty = CastField<FByteProperty>(CallerProperty))
			{
				if (ensure(ByteProperty->SameType(Variable.GetProperty())))
				{
					ByteProperty->CopyCompleteValue(Variable.GetMemory(), SrcPtr);
				}
			}
			else
			{
...
void FControlRigVariableMappings::AddVariableFunction(const FRigVMExternalVariable& InVariable, FProperty* InSourceProperty)
{
	...
	if (CastField<FBoolProperty>(InSourceProperty) && InVariable.GetExtendedCPPType() == RigVMTypeUtils::BoolTypeName)
	{
	...
	else if (CastField<FFloatProperty>(InSourceProperty) && (InVariable.GetExtendedCPPType() == RigVMTypeUtils::FloatTypeName || InVariable.GetExtendedCPPType() == RigVMTypeUtils::DoubleTypeName))
	{
		if(InVariable.GetExtendedCPPType() == RigVMTypeUtils::FloatTypeName)
		{
			return AddUpdateFunction([InSourceProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
			{
	...
	else if (CastField<FDoubleProperty>(InSourceProperty) && (InVariable.GetExtendedCPPType() == RigVMTypeUtils::FloatTypeName || InVariable.GetExtendedCPPType() == RigVMTypeUtils::DoubleTypeName))
	{
		if(InVariable.GetExtendedCPPType() == RigVMTypeUtils::FloatTypeName)
		{
			return AddUpdateFunction([InSourceProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
	...
	else if (CastField<FIntProperty>(InSourceProperty) && InVariable.GetExtendedCPPType() == RigVMTypeUtils::Int32TypeName)
	{
		return AddUpdateFunction([InSourceProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
		{
	...
	else if (CastField<FNameProperty>(InSourceProperty) && InVariable.GetExtendedCPPType() == RigVMTypeUtils::FNameTypeName)
	{
		return AddUpdateFunction([InSourceProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
		{
	...
	else if (CastField<FNameProperty>(InSourceProperty) && InVariable.GetExtendedCPPType() == RigVMTypeUtils::FStringTypeName)
	{
		return AddUpdateFunction([InSourceProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
		{
	...
	else if (FStructProperty* StructProperty = CastField<FStructProperty>(InSourceProperty))
	{
		return AddUpdateFunction([StructProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
		{
			...
			StructProperty->Struct->CopyScriptStruct(Variables[VariableIndex].GetMemory(), SrcPtr, 1);
		});
	}
	else if(FArrayProperty* ArrayProperty = CastField<FArrayProperty>(InSourceProperty))
	{
		if(ensure(ArrayProperty->SameType(InVariable.GetProperty())))
		{
			return AddUpdateFunction([ArrayProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
			{
				...
				ArrayProperty->CopyCompleteValue(Variables[VariableIndex].GetMemory(), SrcPtr);
			});
		}
	}
	else if(FObjectProperty* ObjectProperty = CastField<FObjectProperty>(InSourceProperty))
	{
		if(ensure(ObjectProperty->SameType(InVariable.GetProperty())))
		{
			return AddUpdateFunction([ObjectProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
			{
				...
				ObjectProperty->CopyCompleteValue(Variables[VariableIndex].GetMemory(), SrcPtr);
			});
		}
	}
	else if(FEnumProperty* EnumProperty = CastField<FEnumProperty>(InSourceProperty))
	{
		if(ensure(EnumProperty->SameType(InVariable.GetProperty())))
		{
			return AddUpdateFunction([EnumProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
			{
				...
				EnumProperty->CopyCompleteValue(Variables[VariableIndex].GetMemory(), SrcPtr);
			});
		}
	}
	else if (FByteProperty* ByteProperty = CastField<FByteProperty>(InSourceProperty))
	{
		if (ensure(ByteProperty->SameType(InVariable.GetProperty())))
		{
			return AddUpdateFunction([ByteProperty, this, VariableIndex](URigHierarchy* InTargetHierarchy, const UObject* InSourceInstance)
				{
					...
					ByteProperty->CopyCompleteValue(Variables[VariableIndex].GetMemory(), SrcPtr);
				});
		}
	}
	else
...
void FControlRigVariableMappings::AddCustomVariableFunction(const FRigVMExternalVariable& InVariable, const FProperty* InSourceProperty, const uint8* InSourcePropertyMemory)
{
	...
	else if (const FStructProperty* StructProperty = CastField<FStructProperty>(InSourceProperty))
	{
		AddCustomUpdateFunction([StructProperty, InSourcePropertyMemory, Variable = InVariable]()
			{
				StructProperty->Struct->CopyScriptStruct(const_cast<uint8*>(Variable.GetMemory()), InSourcePropertyMemory, 1);
			});
	}
	else if (const FArrayProperty* ArrayProperty = CastField<FArrayProperty>(InSourceProperty))
	{
		if (ensure(ArrayProperty->SameType(InVariable.GetProperty())))
		{
			AddCustomUpdateFunction([ArrayProperty, InSourcePropertyMemory, Variable = InVariable]()
				{
					ArrayProperty->CopyCompleteValue(const_cast<uint8*>(Variable.GetMemory()), InSourcePropertyMemory);
				});
		}
	}
	else if (const FObjectProperty* ObjectProperty = CastField<FObjectProperty>(InSourceProperty))
	{
		if (ensure(ObjectProperty->SameType(InVariable.GetProperty())))
		{
			AddCustomUpdateFunction([ObjectProperty, InSourcePropertyMemory, Variable = InVariable]()
				{
					ObjectProperty->CopyCompleteValue(const_cast<uint8*>(Variable.GetMemory()), InSourcePropertyMemory);
				});
		}
	}
	else if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(InSourceProperty))
	{
		if (ensure(EnumProperty->SameType(InVariable.GetProperty())))
		{
			AddCustomUpdateFunction([EnumProperty, InSourcePropertyMemory, Variable = InVariable]()
				{
					EnumProperty->CopyCompleteValue(const_cast<uint8*>(Variable.GetMemory()), InSourcePropertyMemory);
				});
		}
	}
	else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(InSourceProperty))
	{
		if (ensure(ByteProperty->SameType(InVariable.GetProperty())))
		{
			AddCustomUpdateFunction([ByteProperty, InSourcePropertyMemory, Variable = InVariable]()
				{
					ByteProperty->CopyCompleteValue(const_cast<uint8*>(Variable.GetMemory()), InSourcePropertyMemory);
				});
		}
	}
	else
