...
const FRigConnectorElement* FRigConnectionRuleInput::FindPrimaryConnector(FText* OutErrorMessage) const
{
	...
	const bool bIsModularRig = Hierarchy->GetTypedOuter<UModularRig>() != nullptr;
	if (bIsModularRig)
	{
		check(Module);
	}
	...
	Hierarchy->ForEach<FRigConnectorElement>(
		[this, &PrimaryConnector, bIsModularRig](FRigConnectorElement* Connector) -> bool
		{
			if (Connector->IsPrimary())
			{
				...
				if (!bIsModularRig ||
					Module->Name == ConnectorModuleName)
				{
	...
	if (PrimaryConnector == nullptr)
	{
		if (OutErrorMessage)
		{
			...
			{
				static constexpr TCHAR Format[] = TEXT("No primary connector found in Control Rig Module.");
			}
		}
	}
...
TArray<const FRigConnectorElement*> FRigConnectionRuleInput::FindSecondaryConnectors(bool bOptional, FText* OutErrorMessage) const
{
	...
	Hierarchy->ForEach<FRigConnectorElement>(
		[this, bOptional, bIsModularRig, &SecondaryConnectors](const FRigConnectorElement* Connector) -> bool
		{
...
////////////////////////////////////////////////////////////////////////////////
// FRigArraySizeConnectionRule
////////////////////////////////////////////////////////////////////////////////

FRigElementResolveResult FRigArraySizeConnectionRule::Resolve(const FRigBaseElement* InTarget, const FRigConnectionRuleInput& InRuleInput) const
{
	// The Array Size Rule does not restrict targets
	FRigElementResolveResult Result(InTarget->GetKey());
	Result.SetPossibleTarget();
...
TOptional<int32> FRigArraySizeConnectionRule::GetMinNumConnections() const
{
	if (bMinEnabled)
	{
...
TOptional<int32> FRigArraySizeConnectionRule::GetMaxNumConnections() const
{
	if (bMaxEnabled)
	{
