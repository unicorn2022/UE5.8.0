...
public:
	...
	CONTROLRIG_API static bool ShouldCacheLegacyTemplate();
...
private:
	...
	void BindGameWorldControlRigDelegates(UControlRig* GameWorldControlRig);
	void UnbindGameWorldControlRigDelegates(UControlRig* GameWorldControlRig);
	
	//UControlRig Delegates
	void HandleOnPreConstruct_AnyThread(UControlRig* Subject, const FName& InEventName);
...
public:
	...
	*/
	...
	/**
	 * Set the section we want to key and receive globally changed values.
	 *
	...
	/**
	 * Finds a section we want to key and receive globally changed values.
	...
	*/
	...
	CONTROLRIG_API void ApplyControlRigSettingsOverrides(UControlRig* InControlRig);
	CONTROLRIG_API void RecreateControlRig();
	

	CONTROLRIG_API FInstancedPropertyBag GetCurrentRigSettings();
	CONTROLRIG_API void SyncRigSettingsToCurrentRig(FInstancedPropertyBag& InSettings);
	CONTROLRIG_API FInstancedPropertyBag BuildNewRigSettings(const TMap<FName, FString>& InSettingsToOverride);
...
private:
	...
	void RemoveAllConstraints() const;
...
public:
	CONTROLRIG_API UControlRig* GetGameWorldControlRig(UWorld* InWorld, UObject* InBoundObject);
	bool IsAGameInstance(const UControlRig* InControlRig, const bool bCheckValidWorld = false) const;
	
	CONTROLRIG_API const TUniquePtr<FMovieSceneEvaluationTrack>& GetLegacyEvaluationTrack() const;
...
private:
	...
	// Cached FMovieSceneEvaluationTrack used to speed up legacy evaluation
	int32 LegacyEvaluationTrackCount = 0;
	TUniquePtr<FMovieSceneEvaluationTrack> LegacyEvaluationTrack; 

	friend struct FControlRigParameterTrackSectionToKeyRestore;
	friend struct FScopedLegacyEvaluationTrack;
};

/**
 * FScopedLegacyEvaluationTrack allows FMovieSceneEvaluationTrack to be cached when bulk-adding keyframes which significantly improves performance.
 * Note that this code can be called multiple times for the same track but is not thread-safe as is.
 */

struct FScopedLegacyEvaluationTrack
{
	CONTROLRIG_API FScopedLegacyEvaluationTrack(UMovieSceneControlRigParameterTrack* InTrack);
	CONTROLRIG_API FScopedLegacyEvaluationTrack(const UMovieSceneSection* InSection);
	CONTROLRIG_API ~FScopedLegacyEvaluationTrack();
private:
	UE_NONCOPYABLE(FScopedLegacyEvaluationTrack);
	TWeakObjectPtr<UMovieSceneControlRigParameterTrack> WeakTrack;
	bool bCacheTemplate = false;
};

//override the section to key, section to key per control, temporarily
