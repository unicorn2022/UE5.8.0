...
public:
	...
	CONTROLRIG_API FControlRigAssetStrongReference GetAssetReference() const;
...
public:
	...
	/** Get cached control pose after backwards solve*/
	const FRigPose& GetControlsAfterBackwardsSolve() const { return  ControlsAfterBackwardsSolve; }
...
public:	
	...
	/**
	 * Returns true if this rig instance needs the sequencer to re-run its instance-level
	 * initialization (re-binding, re-transferring bones from the skeletal mesh, etc.) on
	 * the next bind. Set by InitializeFromCDO when the rig is backed by a Runtime Asset —
	 * since those re-initialize the same UObject in place, the sequencer's track-map check
	 * (which is keyed by GetUniqueID()) would otherwise skip the refresh. BP rigs don't
	 * need this flag because BP reinstancing produces a new UObject with a new UniqueID.
	 */
	bool NeedsInstanceInitialization() const { return bNeedsInstanceInitialization; }
	void ClearInstanceInitializationPending() { bNeedsInstanceInitialization = false; }
	...
	UFUNCTION(BlueprintPure, Category = "Control Rig")
	CONTROLRIG_API virtual TArray<FName> GetSortedControls() const;
...
protected:
	...
	CONTROLRIG_API void SendSelectionBeforeInitNotifications();

	/**
	 * Session-local flag set by InitializeFromCDO when the rig is backed by a Runtime Asset.
	 * Consumed by BindToSequencerInstance (and similar paths) to force a re-bind/refresh
	 * even though the rig's UniqueID is unchanged. Cleared after the refresh completes.
	 */
	bool bNeedsInstanceInitialization = false;

#if RIGVM_TRACE_ENABLED
public:
	CONTROLRIG_API virtual UObject* GetBoundOuterForTrace() const override;
	
protected:
	CONTROLRIG_API virtual void TraceExecuteData(FRigVMTraceArchiveWriter& InArchive) const override;
	CONTROLRIG_API virtual void LoadTracedExecuteData(FRigVMTraceArchiveReader& InArchive) override;
#endif
...
protected:
	...
	TArray<FRigHierarchyKey> SelectionBeforeInit;
...
protected:
	...
	FControlRigOverrideAssetsChanged OverrideAssetsChangedDelegate;
 	...
 	CONTROLRIG_API virtual void GenerateRequiredPluginsData(FRigVMExtendedExecuteContext& Context) override;
...
public:
	...
	// We no longer support the idea of ControlRig hosting simulations - that can be done by adding
	// them to elements or components (etc). 
	UE_DEPRECATED(5.8, "Control Rig core no longer supports owning simulations")
	FRigPhysicsSimulationBase* GetPhysicsSimulation(const FRigComponentKey& InComponentKey) { return nullptr ;}

	UE_DEPRECATED(5.8, "Control Rig core no longer supports owning simulations")
	const FRigPhysicsSimulationBase* GetPhysicsSimulation(const FRigComponentKey& InComponentKey) const { return nullptr; }

	UE_DEPRECATED(5.8, "Control Rig core no longer supports owning simulations")
	bool RegisterPhysicsSimulation(
		TSharedPtr<FRigPhysicsSimulationBase> PhysicsSimulation, const FRigComponentKey& InComponentKey) { return false; }
...
private:
	...
	mutable TPair<FName, FString> RigModulePrefix = {NAME_None, FString()};
...
public:
	...
	// Nodes/components etc that contain "simulation" (i.e. contain significant amounts of state,
	// and won't behave nicely when given extra "updates") should set this on the control rig during
	// construction or during execution. It will be used to prevent additional updates of the
	// control rig that are there primarily to make it feel responsive when editing. Note that the
	// flag will be reset automatically before construction etc.
	void SetContainsSimulation() { bContainsSimulation = true; }
	bool ContainsSimulation() const { return bContainsSimulation; }
