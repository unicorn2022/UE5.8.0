...
struct FControlRigStats
{
	bool Enabled;
	TArray<FName> Stack;
	// Values are heap-allocated so that `double&` returned from RetainCounter remains valid
	// when later inserts cause the TMap to rehash. A scoped counter may outlive subsequent
	// FindOrAdd calls; storing the double inline would dangle on rehash.
	TMap<FName, TUniquePtr<double>> Counters;
