...
/*
 * Base class for all pure collection nodes
 */
USTRUCT(meta = (Abstract, NodeColor = "0.4627450108528137 1.0 0.3294120132923126", Category = "Items", DocumentationPolicy = "Strict"))
...
/*
 * Base class for all mutable collection nodes
 */
USTRUCT(meta = (Abstract, NodeColor = "0.4627450108528137 1.0 0.3294120132923126", Category = "Items", DocumentationPolicy = "Strict"))
...
struct FRigUnit_CollectionNameSearchArray : public FRigUnit_CollectionBase
{
	...
	// The resulting array of elements matching the filters
	UPROPERTY(meta = (Output))
	TArray<FRigElementKey> Items;
};
...
struct FRigUnit_CollectionChildrenArray : public FRigUnit_CollectionBase
{
	...
	// The parent to search the children for
	UPROPERTY(meta = (Input, ExpandByDefault))
	FRigElementKey Parent;

	// If True the parent will be included in the list of results
	UPROPERTY(meta = (Input))
	bool bIncludeParent;

	// If True children of children will be included
	UPROPERTY(meta = (Input))
	bool bRecursive;
	...
	// The type of children to look for
	UPROPERTY(meta = (Input))
	ERigElementType TypeToSearch;

	// The resulting array of children
	UPROPERTY(meta = (Output))
	TArray<FRigElementKey> Items;
};
...
struct FRigUnit_CollectionReplaceItemsArray : public FRigUnit_CollectionBase
{
	...
	// The input array of items to replace
	UPROPERTY(meta = (Input))
	TArray<FRigElementKey> Items;

	// The old name to look for
	UPROPERTY(meta = (Input))
	FName Old;
	...
	// If True items that don't exist (after the replace operation) will be removed from the list of results
	UPROPERTY(meta = (Input))
	bool RemoveInvalidItems;

	// If True the list may contain duplicates
	UPROPERTY(meta = (Input))
	bool bAllowDuplicates;

	// The resulting array of items
	UPROPERTY(meta = (Output))
...
struct FRigUnit_CollectionItems : public FRigUnit_CollectionBase
{
	...
	// The input array of items
	UPROPERTY(meta = (Input, ExpandByDefault))
	TArray<FRigElementKey> Items;

	// If True the collection may contain duplicates 
	UPROPERTY(meta = (Input))
	bool bAllowDuplicates;
...
struct FRigUnit_CollectionGetItems : public FRigUnit_CollectionBase
{
	...
	// The input collection to retrieve the items from
	UPROPERTY(meta = (Input))
	FRigElementKeyCollection Collection;

	// The resulting items in the collection 
	UPROPERTY(meta = (Output))
	TArray<FRigElementKey> Items;
};
...
struct FRigUnit_CollectionGetParentIndices : public FRigUnit_CollectionBase
{
	...
	// The collection to retrieve the parent indices from
	UPROPERTY(meta = (Input))
	FRigElementKeyCollection Collection;

	// The resulting parent indices
	UPROPERTY(meta = (Output))
...
struct FRigUnit_CollectionGetParentIndicesItemArray : public FRigUnit_CollectionBase
{
	...
	// The input array of items to retrieve the parent indices for
	UPROPERTY(meta = (Input))
	TArray<FRigElementKey> Items;

	// The resulting array of parent indices
	UPROPERTY(meta = (Output))
	TArray<int32> ParentIndices;
};
