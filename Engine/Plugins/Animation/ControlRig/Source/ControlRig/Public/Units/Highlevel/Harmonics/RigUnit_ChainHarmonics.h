...
struct FRigUnit_ChainHarmonicsPerItem : public FRigUnit_HighlevelBaseMutable
{
	...
	// The root of the chain to apply the harmonics to
	UPROPERTY(meta = (Input, ExpandByDefault))
	FRigElementKey ChainRoot;

	// The speed of the harmonics effects
	UPROPERTY(meta = (Input))
	FVector Speed;

	// The reach settings
	// Reach lets the chain "lean" towards the target trying to reach it.
	UPROPERTY(meta = (Input))
	FRigUnit_ChainHarmonics_Reach Reach;

	// The wave settings
	// A wave is a rocking back and forth motion to be applied to all / some axes.
	UPROPERTY(meta = (Input))
	FRigUnit_ChainHarmonics_Wave Wave;

	// The curve to use when evaluating the wave
	UPROPERTY(meta = (Input, Constant))
	FRuntimeFloatCurve WaveCurve;

	// The pendulum settings
	// The harmonic pendulum uses a simple spring interpolation to allow to follow secondary motion.
	UPROPERTY(meta = (Input))
	FRigUnit_ChainHarmonics_Pendulum Pendulum;

	// if True the debug drawing will be enabled
	UPROPERTY(meta = (Input))
	bool bDrawDebug;

	// The world offset to be used when debug drawing
	UPROPERTY(meta = (Input))
	FTransform DrawWorldOffset;
