...
struct FRigUnit_DebugHierarchy : public FRigVMFunction_DebugBase
{
	...
	// The scale to apply to the drawn transform
	UPROPERTY(meta = (Input))
	float Scale;
	...
	// The line thickness to use when drawing
	UPROPERTY(meta = (Input))
	float Thickness;

	// The world offset to pre-multiply the drawn transform with
	UPROPERTY(meta = (Input))
