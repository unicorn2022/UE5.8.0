...
struct FRigUnit_GetControlBool : public FRigUnit
{
	...
	// The current bool of the given control - or identity in case it wasn't found.
 ...
 */
USTRUCT(meta=(DisplayName="Get Euler Angles From Animation", Category="Controls", DocumentationPolicy = "Strict", Keywords="GetControlRotator,PreferredEulerAngles", TemplateName="GetControlValue", NodeColor="0.462745, 1,0, 0.329412", Varying))
struct FRigUnit_GetControlPreferredEulerAngles : public FRigUnit
{
	GENERATED_BODY()

	FRigUnit_GetControlPreferredEulerAngles()
		: Angles(FVector::ZeroVector)
		, RotationOrder(FRigPreferredEulerAngles::DefaultRotationOrder)
		, CachedControlIndex(FCachedRigElement())
	{
	}
	...
	/**
	 * The name of the Control to retrieve the rotation for.
	 */
	UPROPERTY(meta = (Input, CustomWidget = "ControlName" ))
	FName Control;

	// The euler angles provided by sequencer
	UPROPERTY(meta=(Output))
	FVector Angles;

	// The rotation order used to represent the rotation
	UPROPERTY(meta = (Output))
