...
struct FRigUnit_ParentConstraint : public FRigUnit_HighlevelBaseMutable
{
	...
	// If True the offset will be computed based on the current delta between each child and parent
	UPROPERTY(EditAnywhere, Category = "Constraint", meta = (Input))
	bool bMaintainOffset;

	// The filter to apply on the transform
	UPROPERTY(EditAnywhere, Category = "Constraint", meta = (Input)) 
	FTransformFilter Filter;

	// The array of parents to constrain the child to
	UPROPERTY(EditAnywhere, Category = "Constraint", meta = (Input, ExpandByDefault, DefaultArraySize = 1))
	TArray<FConstraintParent> Parents;

	// The advanced settings of the constraint operation
	UPROPERTY(EditAnywhere, Category = "Constraint", meta = (Input)) 
	FRigUnit_ParentConstraint_AdvancedSettings AdvancedSettings;

	// The weight to use when applying the resulting transform to the child
	UPROPERTY(EditAnywhere, Category = "Constraint", meta = (Input))
	float Weight;
...
struct FRigUnit_ParentConstraintMath : public FRigUnit_HighlevelBase
{
	...
	*/
	...
	// The array of parents to compute the constrained transform for
	UPROPERTY(EditAnywhere, Category = "Constraint", meta = (Input, ExpandByDefault, DefaultArraySize = 1))
	TArray<FConstraintParent> Parents;

	// The advanced settings to use for the constraint compute
	UPROPERTY(EditAnywhere, Category = "Constraint", meta = (Input)) 
	FRigUnit_ParentConstraintMath_AdvancedSettings AdvancedSettings;

	// The resulting constrained transform
