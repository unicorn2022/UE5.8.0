...
public:
	...
	// returns the path this value was serialized with
	UE_API const FString& GetSerializedPath() const;
	...
	UE_API FProperty* GetLeafProperty();
...
private:
	...
	static UE_API FProperty* FindProperty(const UStruct* InStruct, const FString& InNameOrDisplayName, bool& bWasRedirected);

	UE_API bool SetPropertiesFromPath(const FString& InPath, const UStruct* InOwnerStruct);
	UE_API FString GetPathFromProperties() const;
	...
	UE_API uint32 GetPropertyHash(const FProperty* InProperty) const;
	...
	TOptional<FString> SerializedPath;
	...
	UE_API static FString GetPathFromProperties(const TArray<FPropertyInfo>& InProperties);
...
public:
	...
	UE_API static UControlRigOverrideAsset* CreateOverrideAssetInDeveloperFolder(const UControlRig* InSubject);
