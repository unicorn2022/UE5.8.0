...
void UOrientationWarpingModifier::OnApply_Implementation(UAnimSequence* Animation)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "OrientationWarpingModifier failed. Reason: Invalid Animation");
		return;
	}
	...
	if (Skeleton == nullptr)
	{
		UE_LOGF(LogAnimation, Error, "OrientationWarpingModifier failed. Reason: Animation with invalid Skeleton. Animation: %ls",
	...
	{
		UE_LOGF(LogAnimation, Error, "OrientationWarpingModifier failed. Reason: Name==None on curve. Animation: %ls",
			*GetNameSafe(Animation));
		return;
	}
