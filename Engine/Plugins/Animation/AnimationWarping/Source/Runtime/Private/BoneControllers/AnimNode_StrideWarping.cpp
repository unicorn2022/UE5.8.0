...
void FAnimNode_StrideWarping::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (Mode == EWarpingEvaluationMode::Graph)
	{
		...
		{
			UE_LOGF(LogAnimation, Error, "FAnimNode_StrideWarping, Graph-driven mode expected a valid root motion delta provider interface");
			return;
		}
	}
	...
	if (!bGraphDrivenWarping)
	{
		// If we aren't using graph driven warping, we need to validate the input variables are valid
		if (StrideDirection.ContainsNaN())
		{
			UE_LOGF(LogAnimation, Error, "Invalid stride direction in FAnimNode_StrideWarping");
			return;
		}

		if (!FMath::IsFinite(StrideScale))
		{
			UE_LOGF(LogAnimation, Error, "Invalid stride scale in FAnimNode_StrideWarping");
			return;
		}
	}
	...
	FVector ResolvedFloorNormal = FloorNormalDirection.AsComponentSpaceDirection(AnimInstanceProxy, IKFootRootTransform);
	if (ResolvedFloorNormal.ContainsNaN())
	{
		UE_LOGF(LogAnimation, Error, "FAnimNode_StrideWarping, Resolved FloorNormal contains NaN (%ls); falling back to UpVector", *ResolvedFloorNormal.ToCompactString());
		ResolvedFloorNormal = FVector::UpVector;
	}
	FVector ResolvedGravityDirection = GravityDirection.AsComponentSpaceDirection(AnimInstanceProxy, IKFootRootTransform);
	if (ResolvedGravityDirection.ContainsNaN())
	{
		UE_LOGF(LogAnimation, Error, "FAnimNode_StrideWarping, Resolved GravityDirection contains NaN (%ls); falling back to DownVector", *ResolvedGravityDirection.ToCompactString());
		ResolvedGravityDirection = FVector::DownVector;
	}
	...
	{
#if WITH_EDITORONLY_DATA
		CachedRootMotionDeltaSpeed = !FMath::IsNearlyZero(CachedDeltaTime) ? (CachedRootMotionDeltaTranslation.Size() / CachedDeltaTime) : 0.f;
#else
		const float CachedRootMotionDeltaSpeed = !FMath::IsNearlyZero(CachedDeltaTime) ? (CachedRootMotionDeltaTranslation.Size() / CachedDeltaTime) : 0.f;
#endif

		if (CachedRootMotionDeltaSpeed <= MinRootMotionSpeedThreshold || FMath::IsNearlyZero(CachedRootMotionDeltaSpeed))
		{
	...
	if (!FMath::IsFinite(ActualStrideScale))
	{
		UE_LOGF(LogAnimation, Error, "FAnimNode_StrideWarping, ActualStrideScale is non-finite (%f); falling back to 1.0", ActualStrideScale);
		ActualStrideScale = 1.0f;
	}
	...
	if (bGraphDrivenWarping)
	{
		...
		{
			UE_LOGF(LogAnimation, Error, "FAnimNode_StrideWarping, Graph-driven mode expected a root motion delta to be present in the attribute stream prior to warping/overriding it");
			return;
		}
	}
	...
	for (auto& Foot : FootData)
	{
		...
		check(!WarpedLocation.ContainsNaN());
