...
void UAnimationWarpingLibrary::ConvertToOffsetRootBoneNodePure(const FAnimNodeReference& Node, FOffsetRootBoneAnimNodeReference& OffsetRootBoneNode,
	bool& Result)
{
	EAnimNodeReferenceConversionResult ConversionResult;
	OffsetRootBoneNode = FAnimNodeReference::ConvertToType<FOffsetRootBoneAnimNodeReference>(Node, ConversionResult);
	Result = (ConversionResult == EAnimNodeReferenceConversionResult::Succeeded);
}

void UAnimationWarpingLibrary::ApplyDeltaToOffsetRootBone(const FOffsetRootBoneAnimNodeReference& OffsetRootBoneNode, FVector TranslationWorld,
                                                          FQuat RotationWorld)
{
	if (FAnimNode_OffsetRootBone* Node = OffsetRootBoneNode.GetAnimNodePtr<FAnimNode_OffsetRootBone>())
	{
		FVector SimulatedTranslation = Node->GetOffsetRootTranslation();
		FQuat SimulatedRotation = Node->GetOffsetRootRotation();

		SimulatedTranslation += TranslationWorld;
		SimulatedRotation = RotationWorld * SimulatedRotation;

		Node->SetOffsetRootTranslation(SimulatedTranslation);
		Node->SetOffsetRootRotation(SimulatedRotation);
	}
}
