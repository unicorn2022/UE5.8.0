...
struct FAnimNode_OffsetRootBone : public FAnimNode_Base
{
	...
	// When OnGround is true, root motion velocities will be projected onto the ground surface
	UPROPERTY(EditAnywhere, Category = Settings, meta = (FoldProperty, PinHiddenByDefault))
	bool bOnGround = true;
...
public:
	...
	UE_API bool GetResetOnTeleport() const;
	...
	const FVector& GetOffsetRootTranslation() const { return SimulatedTranslation; }
	void SetOffsetRootTranslation(const FVector& InTranslation) { SimulatedTranslation = InTranslation; }
	
	const FQuat& GetOffsetRootRotation() const { return SimulatedRotation; };
	void SetOffsetRootRotation(const FQuat& InRotation) { SimulatedRotation = InRotation; }
