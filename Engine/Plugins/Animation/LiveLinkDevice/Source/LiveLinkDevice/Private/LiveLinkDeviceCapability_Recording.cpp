...
ULiveLinkDeviceCapability_Recording::ULiveLinkDeviceCapability_Recording()
	: Column_RecordingStatus(RegisterTableColumn("RecordingStatus"))
{
	RegisterGlobalTableColumnHandler(UE::LiveLink::DefaultColumn::Action);
}
...
void ULiveLinkDeviceCapability_Recording::OnDeviceSubsystemInitialized()
{
	...
	{
		ILiveLinkRecordingSession::Get().OnRecordingStarted().AddUObject(
			this, &ULiveLinkDeviceCapability_Recording::HandleRecordingStarted
		);

		ILiveLinkRecordingSession::Get().OnRecordingStopped().AddUObject(
...
void ULiveLinkDeviceCapability_Recording::OnDeviceSubsystemDeinitializing()
{
	...
	if (ModularFeatures.IsModularFeatureAvailable(ILiveLinkRecordingSession::GetModularFeatureName()))
	{
		ILiveLinkRecordingSession::Get().OnRecordingStarted().RemoveAll(this);
		ILiveLinkRecordingSession::Get().OnRecordingStopped().RemoveAll(this);
	}
}
