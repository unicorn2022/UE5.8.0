...
void ULiveLinkDeviceSubsystem::OnExtraDataSessionSaving(ULiveLinkHubSessionExtraData* InExtraData)
{
	...
	for (const TPair<FGuid, TObjectPtr<ULiveLinkDevice>>& DevicePair : Devices)
	{
		DeviceData->Devices.Emplace(FLiveLinkDevicePreset{
			...
			.DeviceCapabilityData = DevicePair.Value->OnSaveDeviceCapabilityData()
		});
	}
}
...
void ULiveLinkDeviceSubsystem::OnExtraDataSessionLoaded(const ULiveLinkHubSessionExtraData* InExtraData)
{
	...
	for (const FLiveLinkDevicePreset& DevicePreset : DeviceData->Devices)
	{
		...
		{
			FLiveLinkDeviceCreateResult Result = CreateResult.StealValue();
			Result.Device->OnLoadDeviceCapabilityData(DevicePreset.DeviceCapabilityData);
		}
		else
		{
...
)
{
	...
	NewSettings->DisplayName = GenerateUniqueDisplayName(NewSettings->DisplayName);
...
bool ULiveLinkDeviceSubsystem::IsDisplayNameInUse(const FString& InDisplayName) const
{
	for (const TPair<FGuid, TObjectPtr<ULiveLinkDevice>>& DevicePair : Devices)
	{
		if (DevicePair.Value->GetDisplayName().ToString() == InDisplayName)
		{
			return true;
		}
	}
...
FString ULiveLinkDeviceSubsystem::GenerateUniqueDisplayName(const FString& InBaseName) const
{
	if (!IsDisplayNameInUse(InBaseName))
	{
		return InBaseName;
	}

	int32 Suffix = 1;
	FString Candidate = FString::Printf(TEXT("%s %d"), *InBaseName, Suffix);

	while (IsDisplayNameInUse(Candidate))
	{
		++Suffix;
		Candidate = FString::Printf(TEXT("%s %d"), *InBaseName, Suffix);
	}
...
void ULiveLinkDeviceSubsystem::RegisterDeviceClass(TSubclassOf<ULiveLinkDevice> InDeviceClass)
{
	...
	for (UClass* TempClass = InDeviceClass; TempClass; TempClass = TempClass->GetSuperClass())
	{
		for (TArray<FImplementedInterface>::TConstIterator It(TempClass->Interfaces); It; ++It)
		{
