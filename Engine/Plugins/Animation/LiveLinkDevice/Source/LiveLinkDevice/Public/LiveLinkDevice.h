...
public:
	...
	/** Unique display name for this device */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "LiveLink Device")
	FString DisplayName;
};
...
public:
	...
	/** Called when the device is being saved to the settings */
	virtual TMap<TSubclassOf<class ULiveLinkDeviceCapability>, TObjectPtr<UObject>> OnSaveDeviceCapabilityData() const { return {}; }

	/** Called when the device is being loaded from the settings */
	virtual void OnLoadDeviceCapabilityData(TMap<TSubclassOf<class ULiveLinkDeviceCapability>, TObjectPtr<UObject>> InCapabilityDataMap) { }
...
private:
	...
	/** Provides an image for the status column in the combined list view. */
	const struct FSlateBrush* GetHealthImage() const;
