...
float URootMotionModifier_PrecomputedWarp::GetWeight(float CurrentTime, const FPrecomputedWarpCurve& WarpCurve) const
{
	...
	else
	{
		...
		float CurrentRelativeTime;
		if (StartTimeWithRatio < EndTimeWithRatio - UE_SMALL_NUMBER)
		{
			CurrentRelativeTime = FMath::Clamp((CurrentTime - StartTimeWithRatio) / (EndTimeWithRatio - StartTimeWithRatio), 0, 1);
		}
		else
		{
			CurrentRelativeTime = 1.f;
		}
...
TSharedPtr<FRootMotionModifierSwapState> URootMotionModifier_PrecomputedWarp::GetSwapState() const
{
	TSharedPtr<FRootMotionModifierSwapState> Result = MakeShared<FRootMotionModifier_PrecomputedWarpSwapState>();
	PopulateSwapState(Result);
	return Result;
}

void URootMotionModifier_PrecomputedWarp::PopulateSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState) const
{
	Super::PopulateSwapState(InSwapState);

	TSharedPtr<FRootMotionModifier_PrecomputedWarpSwapState> PrecomputedWarpSwapState = StaticCastSharedPtr<FRootMotionModifier_PrecomputedWarpSwapState>(InSwapState);

	PrecomputedWarpSwapState->bFirstFrame = bFirstFrame;
	PrecomputedWarpSwapState->ActualStartTime = ActualStartTime;
	PrecomputedWarpSwapState->RoundedEndTime = RoundedEndTime;
	PrecomputedWarpSwapState->PreviousFrame = PreviousFrame;
	PrecomputedWarpSwapState->StartingRootTransform = StartingRootTransform;
	PrecomputedWarpSwapState->TargetTransform = TargetTransform;
	PrecomputedWarpSwapState->FilteredSteeringTarget = FilteredSteeringTarget;
	PrecomputedWarpSwapState->TargetSmoothingState = TargetSmoothingState;
	PrecomputedWarpSwapState->WarpedTrajectory = WarpedTrajectory;
	PrecomputedWarpSwapState->AnimTrajectoryData = AnimTrajectoryData;
}

void URootMotionModifier_PrecomputedWarp::RestoreFromSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState)
{
	Super::RestoreFromSwapState(InSwapState);

	TSharedPtr<FRootMotionModifier_PrecomputedWarpSwapState> PrecomputedWarpSwapState = StaticCastSharedPtr<FRootMotionModifier_PrecomputedWarpSwapState>(InSwapState);

	bFirstFrame = PrecomputedWarpSwapState->bFirstFrame;
	ActualStartTime = PrecomputedWarpSwapState->ActualStartTime;
	RoundedEndTime = PrecomputedWarpSwapState->RoundedEndTime;
	PreviousFrame = PrecomputedWarpSwapState->PreviousFrame;
	StartingRootTransform = PrecomputedWarpSwapState->StartingRootTransform;
	TargetTransform = PrecomputedWarpSwapState->TargetTransform;
	FilteredSteeringTarget = PrecomputedWarpSwapState->FilteredSteeringTarget;
	TargetSmoothingState = PrecomputedWarpSwapState->TargetSmoothingState;
	WarpedTrajectory = PrecomputedWarpSwapState->WarpedTrajectory;
	AnimTrajectoryData = PrecomputedWarpSwapState->AnimTrajectoryData;
}