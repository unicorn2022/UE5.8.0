...
FTransform URootMotionModifier_SkewWarp::ProcessRootMotion(const FTransform& InRootMotion, float DeltaSeconds)
{
	...
	if (bWarpTranslation)
	{
		...
		// use a larger threshold than UE_KINDA_SMALL_NUMBER to ensure that when taking the animated codepath,
		// other small number thresholds inside WarpTranslation will not fail
		constexpr double RootMotionThreshold = 2e-4;

		// if there is translation in the animation, warp it
		if (!TotalRootMotionWithinWindow.GetTranslation().IsNearlyZero(RootMotionThreshold))
		{
			if (!DeltaTranslation.IsNearlyZero(RootMotionThreshold))
			{
				const FTransform MeshTransform = FTransform(MeshRotationOffset, MeshTranslationOffset) * OwnerAdapter->GetCurrentTransform();
	...
	if (DebugLevel == 2 || DebugLevel == 3)
	{
		...
		if (const AActor* OwnerAsActor = OwnerAdapter->GetActor())
		{
