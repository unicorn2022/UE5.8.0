...
int32 UMotionWarpingComponent::AddModifier(URootMotionModifier* Modifier)
{
	...
	{
		UE_LOGF(LogMotionWarping, Verbose, "MotionWarping: RootMotionModifier added. NetMode: %d WorldTime: %f Char: %ls Animation: %ls [%f %f] [%f %f] Loc: %ls Rot: %ls",
...
void UMotionWarpingComponent::UpdateWithContext(const FMotionWarpingUpdateContext& Context, float DeltaSeconds)
{
	...
	// Currently just uses existing modifiers and swaps state, no need to create
	// Also We can predict outside of gamethread which means creating Modifiers / UObjects.
	if (!bPredictingTrajectory && Context.Animation.IsValid())
	{
		...
		for (const FAnimNotifyEvent& NotifyEvent : Animation->Notifies)
		{
			...
			if (MotionWarpingNotify)
			{
				...
				{
					UE_LOGF(LogMotionWarping, Warning, "MotionWarpingComponent::Update. A motion warping window in %ls doesn't have a valid root motion modifier!", *GetNameSafe(Animation));
					continue;
				}
						...
						if (const UAnimSequenceBase* AnimReference = AnimSegment->GetAnimReference())
						{
							for (const FAnimNotifyEvent& NotifyEvent : AnimReference->Notifies)
							{
								...
								if (MotionWarpingNotify)
								{
									if (MotionWarpingNotify->RootMotionModifier == nullptr)
									{
										UE_LOGF(LogMotionWarping, Warning, "MotionWarpingComponent::Update. A motion warping window in %ls doesn't have a valid root motion modifier!", *GetNameSafe(AnimReference));
										continue;
									}
	...
	if (Modifiers.Num() > 0)
	{
		...
		Modifiers.RemoveAll([this](const URootMotionModifier* Modifier)
		{
			...
			{
				UE_LOGF(LogMotionWarping, Verbose, "MotionWarping: RootMotionModifier removed. NetMode: %d WorldTime: %f Char: %ls Animation: %ls [%f %f] [%f %f] Loc: %ls Rot: %ls",
					GetWorld()->GetNetMode(), GetWorld()->GetTimeSeconds(), *GetNameSafe(GetOwner()), *GetNameSafe(Modifier->Animation.Get()), Modifier->StartTime, Modifier->EndTime, Modifier->PreviousPosition, Modifier->CurrentPosition,
					*GetOwner()->GetActorLocation().ToString(), *GetOwner()->GetActorRotation().ToCompactString());
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	const int32 DebugLevel = FMotionWarpingCVars::CVarMotionWarpingDebug.GetValueOnAnyThread();
...
int32 UMotionWarpingComponent::RemoveWarpTarget(FName WarpTargetName)
{
	...
	if (NumRemoved > 0 && !bPredictingTrajectory)
	{
		MARK_PROPERTY_DIRTY_FROM_NAME(UMotionWarpingComponent, WarpTargets, this);
	}
...
void UMotionWarpingComponent::AddSwitchOffCondition(FName WarpTargetName, UMotionWarpingSwitchOffCondition* Condition)
{
	...
	{
		UE_LOGF(LogMotionWarping, Error, "Trying to add invalid switch off condition")
	}
}
...
void UMotionWarpingComponent::BeginTrajectoryPredictions(const FAnimMontageInstance* MontageInstance)
{
	check(!bPredictingTrajectory);

	// @TODO: We don't support SwitchOffConditions yet, TBD if we will
	ensure(SwitchOffConditions.IsEmpty());

	bPredictingTrajectory = true;

	CachedOwnerAdapter = OwnerAdapter;

	UMotionWarpingMontageTrajectoryAdapter* MontageTrajectoryAdapter = nullptr;
	if (!PredictionAdapter)
	{
		// Create prediction adapter and assign as owner adapter
		MontageTrajectoryAdapter = CreateOwnerAdapter<UMotionWarpingMontageTrajectoryAdapter>();
		PredictionAdapter = MontageTrajectoryAdapter;
	}
	else
	{
		OwnerAdapter = PredictionAdapter;
		MontageTrajectoryAdapter = CastChecked<UMotionWarpingMontageTrajectoryAdapter>(OwnerAdapter);
	}

	MontageTrajectoryAdapter->InitializeFrom(CachedOwnerAdapter, MontageInstance);
}

void UMotionWarpingComponent::EndTrajectoryPredictions()
{
	check(bPredictingTrajectory);

	// Restore Pre-prediction owner
	OwnerAdapter = CachedOwnerAdapter;

	// Restore Warp State to pre-prediction
	if (UMotionWarpingMontageTrajectoryAdapter* MontageTrajectoryAdapter = Cast<UMotionWarpingMontageTrajectoryAdapter>(PredictionAdapter))
	{
		MontageTrajectoryAdapter->RestoreState();
	}

	bPredictingTrajectory = false;
}

bool UMotionWarpingComponent::IsPredictingTrajectory() const
{
	return bPredictingTrajectory;
}
...
void UMotionWarpingComponent::AddOrUpdateWarpTargetFromComponent(FName WarpTargetName, const USceneComponent* Component, FName BoneName, bool bFollowComponent, EWarpTargetLocationOffsetDirection LocationOffsetDirection, FVector LocationOffset, FRotator RotationOffset)
{
	...
	{
		UE_LOGF(LogMotionWarping, Warning, "AddOrUpdateWarpTargetFromComponent has failed!. Reason: Invalid Component");
		return;
	}
