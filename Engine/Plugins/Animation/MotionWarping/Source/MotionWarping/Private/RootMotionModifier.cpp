...
namespace FMotionWarpingRuntimeCVars
{
	// @todo: Switch this to default true after more testing.
	bool bUseMeshScale = false;
	static FAutoConsoleVariableRef CVarMotionWarping(
		TEXT("a.MotionWarping.UseMeshScale"),
		bUseMeshScale,
		TEXT("Account for mesh scale when calculating warp points (see CachedOffsetFromWarpPoint)."));
}
...
FTransform FMotionWarpingTarget::GetTargetTransformFromComponent(const USceneComponent* Comp, const FName& BoneName)
{
	if (Comp == nullptr)
	{
		UE_LOGF(LogMotionWarping, Warning, "FMotionWarpingTarget::GetTargetTransformFromComponent: Invalid Component");
		return FTransform::Identity;
	}

	if (Comp->DoesSocketExist(BoneName) == false)
	{
		UE_LOGF(LogMotionWarping, Warning, "FMotionWarpingTarget::GetTargetTransformFromComponent: Invalid Bone or Socket. Comp: %ls Owner: %ls BoneName: %ls",
...
void URootMotionModifier_Warp::Serialize(FArchive& Ar)
{
	...
	// Now load so any other deprecation fixup can work properly
	Super::Serialize(Ar);

	if (Ar.IsLoading())
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		if (CustomVersion < FFortniteMainBranchObjectVersion::RootMotionModifierRotationOffset)
		{
			if (RotationType == EMotionWarpRotationType::OppositeDefault)
			{
				RotationType = EMotionWarpRotationType::Default;
				AdditionalRotationOffset.Yaw += 180.0f;
				AdditionalRotationOffset.Normalize();
			}
			else if ( RotationType == EMotionWarpRotationType::OppositeFacing )
			{
...
void FMotionWarpingTarget::CacheOffset(const FTransform& InTransform)
{
	...
	if (LocationOffsetDirection == EWarpTargetLocationOffsetDirection::VectorFromTargetToOwner)
	{
		...
		{
			UE_LOGF(LogMotionWarping, Warning, "Motion warping offset is set to VectorFromTargetToOwner but avator actor is invalid")
		}
	}
}
...
void FMotionWarpingTarget::RecalculateOffset(FTransform& InTransform) const
{
	...
	switch (LocationOffsetDirection)
	{
		...
		case EWarpTargetLocationOffsetDirection::VectorFromTargetToOwner:
			...
			{
				UE_LOGF(LogMotionWarping, Warning, "Motion warping offset is set to VectorFromOwnerToTarget but avator actor is invalid")
			}
			break;
...
TSharedPtr<FRootMotionModifierSwapState> URootMotionModifier::GetSwapState() const
{
	TSharedPtr<FRootMotionModifierSwapState> Result = MakeShared<FRootMotionModifierSwapState>();
	PopulateSwapState(Result);
	return Result;
}

void URootMotionModifier::PopulateSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState) const
{
	InSwapState->StartTime = StartTime;
	InSwapState->EndTime = EndTime;
	InSwapState->PreviousPosition = PreviousPosition;
	InSwapState->CurrentPosition = CurrentPosition;
	InSwapState->Weight = Weight;
	InSwapState->PlayRate = PlayRate;
	InSwapState->StartTransform = StartTransform;
	InSwapState->ActualStartTime = ActualStartTime;
	InSwapState->TotalRootMotionWithinWindow = TotalRootMotionWithinWindow;
	InSwapState->State = State;
}

void URootMotionModifier::RestoreFromSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState)
{
	StartTime = InSwapState->StartTime;
	EndTime = InSwapState->EndTime;
	PreviousPosition = InSwapState->PreviousPosition;
	CurrentPosition = InSwapState->CurrentPosition;
	Weight = InSwapState->Weight;
	PlayRate = InSwapState->PlayRate;
	StartTransform = InSwapState->StartTransform;
	ActualStartTime = InSwapState->ActualStartTime;
	TotalRootMotionWithinWindow = InSwapState->TotalRootMotionWithinWindow;
	State = InSwapState->State;
}
...
void URootMotionModifier::Update(const FMotionWarpingUpdateContext& Context)
{
	...
	{
		UE_LOGF(LogMotionWarping, Verbose, "MotionWarping: Marking RootMotionModifier for removal. Reason: Animation is not valid. Char: %ls Current Animation: %ls. Window: Animation: %ls [%f %f] [%f %f]",
	...
	{
		UE_LOGF(LogMotionWarping, Verbose, "MotionWarping: Marking RootMotionModifier for removal. Reason: Window has ended. Char: %ls Animation: %ls [%f %f] [%f %f]",
	...
	if (State == ERootMotionModifierState::Active && PreviousPosition < EndTime && (CurrentPosition > EndTime || CurrentPosition < StartTime))
	{
		...
		{
			UE_LOGF(LogMotionWarping, Verbose, "MotionWarping: Marking RootMotionModifier for removal. Reason: CurrentPosition manually changed. PrevPos: %f CurrPos: %f DeltaTime: %f ExpectedDelta: %f ActualDelta: %f",
...
void URootMotionModifier_Warp::Update(const FMotionWarpingUpdateContext& Context)
{
	...
	if (OwnerComp && GetState() == ERootMotionModifierState::Active)
	{
		...
		{
			UE_LOGF(LogMotionWarping, Verbose, "MotionWarping: Marking RootMotionModifier as Disabled. Reason: Invalid Warp Target (%ls). Char: %ls Animation: %ls [%f %f] [%f %f]",
		...
		if (WarpPointAnimProvider != EWarpPointAnimProvider::None)
		{
			if (!CachedOffsetFromWarpPoint.IsSet())
			{
				if (const UMotionWarpingBaseAdapter* OwnerAdapter = GetOwnerAdapter())
				{
					...
					// Scale the animation-space offset by mesh world scale so the target accounts for the scaled skeleton
					if (FMotionWarpingRuntimeCVars::bUseMeshScale && CachedOffsetFromWarpPoint.IsSet())
					{
						if (const USkeletalMeshComponent* Mesh = OwnerAdapter->GetMesh())
						{
							CachedOffsetFromWarpPoint.GetValue().ScaleTranslation(Mesh->GetComponentTransform().GetScale3D());
						}
					}
				}
			}
...
void URootMotionModifier_Warp::OnTargetTransformChanged()
{
 	if (const UMotionWarpingBaseAdapter* WarpingAdapter = GetOwnerAdapter())
	{
		...
		const FQuat CurrentRotation = WarpingAdapter->GetCurrentRotation();
...
TSharedPtr<FRootMotionModifierSwapState> URootMotionModifier_Warp::GetSwapState() const
{
	TSharedPtr<FRootMotionModifierSwapState> Result = MakeShared<FRootMotionModifier_WarpSwapState>();
	PopulateSwapState(Result);
	return Result;
}

void URootMotionModifier_Warp::PopulateSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState) const
{
	Super::PopulateSwapState(InSwapState);

	TSharedPtr<FRootMotionModifier_WarpSwapState> WarpSwapState = StaticCastSharedPtr<FRootMotionModifier_WarpSwapState>(InSwapState);

	WarpSwapState->AdditionalRotationOffset = AdditionalRotationOffset;
	WarpSwapState->CachedTargetTransform = CachedTargetTransform;
	WarpSwapState->RootMotionRemainingAfterNotify = RootMotionRemainingAfterNotify;
	WarpSwapState->CachedOffsetFromWarpPoint = CachedOffsetFromWarpPoint;
	WarpSwapState->RotationOffset = RotationOffset;
	WarpSwapState->bWarpingPaused = bWarpingPaused;
	WarpSwapState->bRootMotionPaused = bRootMotionPaused;
}

void URootMotionModifier_Warp::RestoreFromSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState)
{
	Super::RestoreFromSwapState(InSwapState);

	TSharedPtr<FRootMotionModifier_WarpSwapState> WarpSwapState = StaticCastSharedPtr<FRootMotionModifier_WarpSwapState>(InSwapState);

	AdditionalRotationOffset = WarpSwapState->AdditionalRotationOffset;
	CachedTargetTransform = WarpSwapState->CachedTargetTransform;
	RootMotionRemainingAfterNotify = WarpSwapState->RootMotionRemainingAfterNotify;
	CachedOffsetFromWarpPoint = WarpSwapState->CachedOffsetFromWarpPoint;
	RotationOffset = WarpSwapState->RotationOffset;
	bWarpingPaused = WarpSwapState->bWarpingPaused;
	bRootMotionPaused = WarpSwapState->bRootMotionPaused;
}
...
FQuat URootMotionModifier_Warp::GetTargetRotation() const
{
	switch (RotationType)
	{
		case EMotionWarpRotationType::Default:
			return FQuat::MakeFromRotator(AdditionalRotationOffset) * CachedTargetTransform.GetRotation();
		...
		case EMotionWarpRotationType::Facing:
			if (const AActor* ActorOwner = GetActorOwner())
			{
				...
				return FQuat::MakeFromRotator(AdditionalRotationOffset) * FRotationMatrix::MakeFromXZ(ToSyncPoint, FVector::UpVector).ToQuat();
			}
			break;
...
FQuat URootMotionModifier_Warp::WarpRotation(const FTransform& RootMotionDelta, const FTransform& RootMotionTotal, float DeltaSeconds)
{
	...
	if (const UMotionWarpingBaseAdapter* WarpingAdapter = GetOwnerAdapter())
	{
		CurrentRotation = WarpingAdapter->GetCurrentRotation() * WarpingAdapter->GetBaseVisualRotationOffset();
	...
	FQuat DeltaOut = TargetRotThisFrame * TotalRootMotionRotation.Inverse();
	DeltaOut = DeltaOut.Equals(FQuat::Identity) ? FQuat::Identity : DeltaOut;
...
void URootMotionModifier_Warp::PrintLog(const FString& Name, const FTransform& OriginalRootMotion, const FTransform& WarpedRootMotion) const
{
	...
	if (ActorOwner && SkelMesh)
	{
		...
		UE_LOGF(LogMotionWarping, Log, "%ls NetMode: %d Char: %ls Anim: %ls Win: [%f %f][%f %f] DT: %f WT: %f ETOffset: %f Dist2D: %f Z: %f FDist2D: %f FZ: %f Dot: %f Delta: %ls (%f) FDelta: %ls (%f) Speed: %f Loc: %ls FLoc: %ls Rot: %ls FRot: %ls",
...
FTransform URootMotionModifier_Blueprintable::ProcessRootMotion(const FTransform& InRootMotion, float DeltaSeconds)
{
	check(IsInGameThread()); // checking since BP_ProcessRootMotion is not threadsafe
	if (const UMotionWarpingBaseAdapter* Adapter = GetOwnerAdapter())
	{
		return BP_ProcessRootMotion(Adapter->GetActor(), Adapter->GetMesh(), InRootMotion, DeltaSeconds);
	}
