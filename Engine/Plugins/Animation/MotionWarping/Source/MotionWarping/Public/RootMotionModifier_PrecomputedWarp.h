...
UCLASS(Experimental, Blueprintable, MinimalAPI, meta = (DisplayName = "Precomputed Warp", Tooltip="(Experimental) Precomputed warp computes the full path to alignment on the first frame of execution and then on following frames just applies that root motion.  Only Suitable for stationary targets."))
...
public:
	...
	MOTIONWARPING_API virtual TSharedPtr<FRootMotionModifierSwapState> GetSwapState() const override;
	MOTIONWARPING_API virtual void PopulateSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState) const override;
	MOTIONWARPING_API virtual void RestoreFromSwapState(TSharedPtr<FRootMotionModifierSwapState> InSwapState) override;
...
struct FRootMotionModifier_PrecomputedWarpSwapState : public FRootMotionModifier_WarpSwapState
{
	bool bFirstFrame = true;
	float ActualStartTime = 0.f;
	float RoundedEndTime = 0.f;
	...
	URootMotionModifier_PrecomputedWarp::FAnimTrajectoryData AnimTrajectoryData;
};
