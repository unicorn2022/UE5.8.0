...
using TReaderWriterPairs =
    ::testing::Types<std::tuple<raf::RegionAffiliationBinaryStreamReaderImpl, raf::RegionAffiliationBinaryStreamWriterImpl>,
                     std::tuple<raf::RegionAffiliationJSONStreamReaderImpl, raf::RegionAffiliationJSONStreamWriterImpl>>;
...
TYPED_TEST(RegionAffiliationReaderWriterTest, ClearJointRegionAffiliations) {
    ...
    this->writer->setJointRegionAffiliation(jointIndex1,
                                            expectedAffiliations1.data(),
                                            static_cast<std::uint16_t>(expectedAffiliations1.size()));
    this->writer->setJointRegionIndices(jointIndex1,
                                        expectedRegionIndices1.data(),
                                        static_cast<std::uint16_t>(expectedRegionIndices1.size()));
    this->writer->setJointRegionAffiliation(jointIndex2,
                                            expectedAffiliations2.data(),
                                            static_cast<std::uint16_t>(expectedAffiliations2.size()));
    this->writer->setJointRegionIndices(jointIndex2,
                                        expectedRegionIndices2.data(),
...
TYPED_TEST(RegionAffiliationReaderWriterTest, VertexRegionAffiliation) {
    ...
    std::uint16_t meshIndex = 0U;
    std::uint32_t vertexIndex = 3U;
    this->writer->setVertexRegionAffiliation(meshIndex,
                                             vertexIndex,
                                             expectedValues.data(),
                                             static_cast<std::uint16_t>(expectedValues.size()));
    this->writer->write();
...
TYPED_TEST(RegionAffiliationReaderWriterTest, ClearVertexRegionAffiliations) {
    ...
    this->writer->setVertexRegionAffiliation(meshIndex1,
                                             vertexIndex1,
                                             expectedAffiliations1.data(),
                                             static_cast<std::uint16_t>(expectedAffiliations1.size()));
    this->writer->setVertexRegionIndices(meshIndex1,
                                         vertexIndex1,
                                         expectedRegionIndices1.data(),
                                         static_cast<std::uint16_t>(expectedRegionIndices1.size()));
    this->writer->setVertexRegionAffiliation(meshIndex2,
                                             vertexIndex2,
                                             expectedAffiliations2.data(),
                                             static_cast<std::uint16_t>(expectedAffiliations2.size()));
    this->writer->setVertexRegionIndices(meshIndex2,
                                         vertexIndex2,
                                         expectedRegionIndices2.data(),
                                         static_cast<std::uint16_t>(expectedRegionIndices2.size()));
