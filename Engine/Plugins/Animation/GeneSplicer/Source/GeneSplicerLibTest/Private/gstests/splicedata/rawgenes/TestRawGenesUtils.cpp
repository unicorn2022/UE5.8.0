...
TEST_F(TestRawGenesUtils, getJointValuesForOutputIndex) {
    ...
    std::vector<std::pair<std::uint16_t, std::vector<float>>> expectedOutputValuesPair{{1u, {0.1f, 0.1f}},
                                                                                       {3u, {0.3f, 0.3f}},
                                                                                       {7u, {0.7f, 0.7f}},
                                                                                       {13u, {}}};
...
TEST_F(TestRawGenesUtils, getOutputIndicesIntroducedByLOD) {
    ...
    std::vector<std::pair<std::uint16_t, std::vector<uint16_t>>> expectedOutputLODPair{{0u, {11, 12, 13}},
                                                                                       {1u, {}},
                                                                                       {2u, {0, 1, 2, 6}}};

    for (auto expected : expectedOutputLODPair) {
        auto actualOutputIndices = getOutputIndicesIntroducedByLOD(ConstArrayView<std::uint16_t>{jointGroup.outputIndices},
