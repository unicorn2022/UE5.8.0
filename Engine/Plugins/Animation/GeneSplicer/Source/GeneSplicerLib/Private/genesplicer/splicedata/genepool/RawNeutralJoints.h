...
struct RawNeutralJoints {
    ...
    RawNeutralJoints(const Reader* dna, MemoryResource* memRes) :
        translations{dna->getNeutralJointTranslationXs(),
                     ...
                     memRes},
        rotations{dna->getNeutralJointRotationXs(), dna->getNeutralJointRotationYs(), dna->getNeutralJointRotationZs(), memRes},
        coordinateSystem{dna->getCoordinateSystem()},
        rotationSequence{dna->getRotationSequence()},
        rotationSigns{dna->getRotationSign()} {

        auto toRadians = [](float& r) { r = tdm::radians(r); };
    ...
    void serialize(Archive& archive) {
        archive(translations, rotations, coordinateSystem, rotationSequence, rotationSigns);
    }

    RawVector3Vector translations;
    RawVector3Vector rotations;
    dna::CoordinateSystem coordinateSystem;
    dna::RotationSequence rotationSequence;
    dna::RotationSign rotationSigns;
};
