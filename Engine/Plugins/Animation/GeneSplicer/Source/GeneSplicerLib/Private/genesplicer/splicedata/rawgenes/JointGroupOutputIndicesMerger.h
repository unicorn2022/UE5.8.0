...
public:
    ...
    void add(const RawJointGroup& jointGroup) {
        add(ConstArrayView<std::uint16_t>{jointGroup.outputIndices}, ConstArrayView<std::uint16_t>{jointGroup.lods});
    }
    ...
    TOutputIndicesOuputIter merge(TOutputIndicesOuputIter outputIndicesIter, ULODReverseOutputIter lodsOnePastLast) {
        ...
        auto maxOutputIndex = static_cast<std::uint16_t>((*std::max_element(jointIndices.begin(), jointIndices.end()) + 1u) * 9u);
        ...
        for (std::size_t lodIdxPlusOne = lods[0].size(); lodIdxPlusOne > 0; --lodIdxPlusOne) {
            ...
            for (std::uint16_t i = 0; i < outputIndices.size(); i++) {
                outputIndicesForLOD[i] = getOutputIndicesIntroducedByLOD(outputIndices[i], lods[i], lodIdx);
            }
            onePastLastAdded = mergeIndices({outputIndicesForLOD}, maxOutputIndex, onePastLastAdded, memRes);
            lodsOnePastLast--;
