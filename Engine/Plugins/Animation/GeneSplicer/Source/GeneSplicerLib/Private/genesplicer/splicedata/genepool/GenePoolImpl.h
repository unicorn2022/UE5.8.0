...
public:
    struct SectionLookupTable {
        ...
        void serialize(Archive& archive) {
            archive(neutralMeshes, blendShapes, neutralJoints, skinWeights, jointBehavior, metadata);
        }
    };
    ...
    struct MetaData {
        ...
        template<class Archive>
        void serialize(Archive& archive) {
            archive(names, genders, ages, jointNames, dbMaxLOD, dbName, dbComplexity, vertexCountPerMesh, genePoolMask);
        }
    };
...
public:
    ...
    void save(Archive& archive) {
        ...
        archive(terse::proxy(sections.metadata),
                metadata,
                terse::proxy(sections.neutralMeshes),
                neutralMeshes,
                terse::proxy(sections.blendShapes),
                blendShapes,
                terse::proxy(sections.neutralJoints),
                neutralJoints,
                terse::proxy(sections.skinWeights),
                skinWeights,
                terse::proxy(sections.jointBehavior),
                jointBehavior,
                getEoF());
    }
