...
void NeutralJointSplicer<CT>::splice(const SpliceDataInterface* spliceData, GeneSplicerDNAReader* output_) {
    ...
    auto splice = [spliceData, output](JointAttribute jointAttribute,
    ...
    neutralJoints.translations =
        splice(JointAttribute::Translation, [](const PoolSpliceParamsImpl* poolParams) { return poolParams->getScale(); });
    // we want to avoid scaling of rotations, so we are pegging it to 1.0f
    neutralJoints.rotations = splice(JointAttribute::Rotation, [](const PoolSpliceParamsImpl* /*unused*/) { return 1.0f; });
    neutralJoints.coordinateSystem = output->getCoordinateSystem();
    neutralJoints.rotationSequence = output->getRotationSequence();
    neutralJoints.rotationSigns = output->getRotationSign();
    ...
    auto toDegrees = [](float& angle) { angle = tdm::degrees(angle); };
