...
fmat4 getTransformationMatrix(const RawNeutralJoints& neutralJoints, std::uint16_t jointIndex) {
    ...
    const tdm::fvec3 translation{translations.xs[jointIndex], translations.ys[jointIndex], translations.zs[jointIndex]};
    ...
    return tdm::rotate(rotation, neutralJoints.rotationSequence, neutralJoints.rotationSigns) * tdm::translate(translation);
}
