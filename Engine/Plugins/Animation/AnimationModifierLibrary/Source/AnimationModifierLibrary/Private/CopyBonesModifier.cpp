...
void UCopyBonesModifier::OnApply_Implementation(UAnimSequence* Animation)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "CopyBonesModifier failed. Reason: Invalid Animation");
		return;
	}
	...
	if (Model == nullptr)
	{
		UE_LOGF(LogAnimation, Error, "CopyBonesModifier failed. Reason: Invalid Data Model. Animation: %ls", *GetNameSafe(Animation));
		return;
	}
	...
	for (int32 Index = 0; Index < NumBoneCopies; ++Index)
	{
		...
		// If there are no pre-existing curves for the target bone we have to add them before being able to set the keys
