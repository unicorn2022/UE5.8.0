...
void UMirrorModifier::OnApply_Implementation(UAnimSequence* Animation)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "MirrorModifier failed. Reason: Invalid Animation");
		return;
	}
	...
	if (Model == nullptr)
	{
		UE_LOGF(LogAnimation, Error, "MirrorModifier failed. Reason: Invalid Data Model. Animation: %ls", *GetNameSafe(Animation));
		return;
	}

	if (!MirrorDataTable)
	{
		UE_LOGF(LogAnimation, Error, "MirrorModifier failed. Reason: Invalid Mirror Data table");
		return;
	}
