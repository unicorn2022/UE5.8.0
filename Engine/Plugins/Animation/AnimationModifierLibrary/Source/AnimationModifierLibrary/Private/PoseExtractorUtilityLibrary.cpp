	...
	, Model(nullptr)
{
	if (!Animation)
	{
		UE_LOGF(LogAnimation, Error, "FPoseExtractorUtilityLibrary failed. Reason: Invalid Animation");
		return;
	}
	...
	if (!AnimationModel)
	{
		UE_LOGF(LogAnimation, Error, "FPoseExtractorUtilityLibrary failed. Reason: Invalid Data Model. Animation: %ls", *GetNameSafe(Animation));
		return;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "FPoseExtractorUtilityLibrary failed. Reason: Invalid Skeleton. Animation: %ls", *GetNameSafe(Animation));
		return;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "FPoseExtractorUtilityLibrary failed. Reason: Ref Skeleton. Animation: %ls", *GetNameSafe(Animation));
		return;
	}
...
bool FPoseExtractorUtilityLibrary::AddBone(const FBoneReference& Bone)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "FPoseExtractorUtilityLibrary::AddBone failed. Reason: Bone %ls has invalid setup", *TempBone.BoneName.ToString());
		return false;
	}
...
bool FPoseExtractorUtilityLibrary::AddBone(const FName& BoneName, int32 BoneIndex)
{
	...
	UE_LOGF(LogAnimation, Error, "FPoseExtractorUtilityLibrary::AddBone failed. Reason: Bone %ls has invalid BoneIndex %d", *BoneName.ToString(), BoneIndex);
	return false;
}
...
bool FPoseExtractorUtilityLibrary::CacheGlobalBoneTransforms()
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "FPoseExtractorUtilityLibrary::CacheGlobalBoneTransforms no weighted bones?");
		return false;
	}
