...
void UDistanceCurveModifier::OnApply_Implementation(UAnimSequence* Animation)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "DistanceCurveModifier failed. Reason: Invalid Animation");
		return;
	}

	if (!Animation->HasRootMotion())
	{
		UE_LOGF(LogAnimation, Error, "DistanceCurveModifier failed. Reason: Root motion is disabled on the animation (%ls)", *GetNameSafe(Animation));
		return;
	}
