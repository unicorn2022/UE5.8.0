		...
		if (AnimSequence != nullptr)
		{
			...
			if (!FMath::IsNearlyZero(UE::Anim::DistanceMatchingUtility::GetDistanceRange(AnimSequence, CurveName)))
			{
				...
				while ((AccumulatedDistance < DistanceTraveled) && (bAllowLooping || (NewTime + StepTime < SequenceLength)))
				{
					...
					{
						UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Failed to advance distance of (%.2f) after (%.2f) seconds on anim sequence (%ls). Aborting.", 
			...
			{
				UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Anim sequence (%ls) is missing a distance curve or doesn't cover enough distance for GetTimeAfterDistanceTraveled.", *GetNameSafe(AnimSequence));
			}
		}
		else
		{
			UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Invalid AnimSequence passed to GetTimeAfterDistanceTraveled");
		}
			...
			if (const FAnimationUpdateContext* AnimationUpdateContext = UpdateContext.GetContext())
			{
				...
				if (DeltaTime > 0 && DistanceTraveled > 0)
				{
					if (const UAnimSequenceBase* AnimSequence = Cast<UAnimSequence>(InSequenceEvaluator.GetSequence()))
					{
						...
						{
							UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Could not set explicit time on sequence evaluator, value is not dynamic. Set it as Always Dynamic.");
						}
					}
					else
					{
						UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Sequence evaluator does not have an anim sequence to play.");
					}
				}
			}
			else
			{
				UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "AdvanceTimeByDistanceMatching called with invalid context");
			}
		});
			...
			if (const UAnimSequence* AnimSequence = Cast<UAnimSequence>(InSequencePlayer.GetSequence()))
			{
				...
				if (!FMath::IsNearlyZero(AnimLength))
				{
					...
					if (!FMath::IsNearlyZero(RootMotionDistance))
					{
						...
						{
							UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Could not set play rate on sequence player, value is not dynamic. Set it as Always Dynamic.");
						}
					}
					else
					{
						UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Unable to adjust playrate for animation with no root motion delta (%ls).", *GetNameSafe(AnimSequence));
					}
				}
				else
				{
					UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Unable to adjust playrate for zero length animation (%ls).", *GetNameSafe(AnimSequence));
				}
			}
			else
			{
				UE_LOGF(LogAnimDistanceMatchingLibrary, Warning, "Sequence player does not have an anim sequence to play.");
			}
		});
