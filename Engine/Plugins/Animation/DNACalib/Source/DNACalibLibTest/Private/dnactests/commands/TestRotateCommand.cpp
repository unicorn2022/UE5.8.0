...
public:
    ...
    dnac::Vector3 getNeutralJointRotation(std::uint16_t index) const override {
        return dnac::Vector3{neutralJointRotations.xs[index], neutralJointRotations.ys[index], neutralJointRotations.zs[index]};
    }
    ...
    dnac::Vector3 getNeutralJointTranslation(std::uint16_t index) const override {
        return dnac::Vector3{neutralJointTranslations.xs[index],
    ...
    dnac::Vector3 getVertexPosition(std::uint16_t /*unused*/, std::uint32_t vertexIndex) const override {
        return dnac::Vector3{vertexPositions.xs[vertexIndex], vertexPositions.ys[vertexIndex], vertexPositions.zs[vertexIndex]};
    }
        ...
        if (meshIndex < getMeshCount()) {
            if (blendShapeTargetIndex < getBlendShapeTargetCount(meshIndex)) {
                if (deltaIndex < getBlendShapeTargetDeltaCount(meshIndex, blendShapeTargetIndex)) {
                    return dnac::Vector3{bsTargetDeltas[meshIndex][blendShapeTargetIndex].xs[deltaIndex],
...
TEST_F(RotateCommandTest, AlongZAxis) {
    ...
    ASSERT_ELEMENTS_NEAR(output->getVertexPositionXs(0), expectedVertexPositionXs, expectedVertexPositionXs.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getVertexPositionYs(0), expectedVertexPositionYs, expectedVertexPositionYs.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getVertexPositionZs(0), expectedVertexPositionZs, expectedVertexPositionZs.size(), 0.0001);
    ...
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaXs(0, 0), expectedDelta0Xs, expectedDelta0Xs.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaYs(0, 0), expectedDelta0Ys, expectedDelta0Ys.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaZs(0, 0), expectedDelta0Zs, expectedDelta0Zs.size(), 0.0001);
    ...
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaXs(0, 1), expectedDelta1Xs, expectedDelta1Xs.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaYs(0, 1), expectedDelta1Ys, expectedDelta1Ys.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaZs(0, 1), expectedDelta1Zs, expectedDelta1Zs.size(), 0.0001);
    ...
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaXs(0, 2), expectedDelta2Xs, expectedDelta2Xs.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaYs(0, 2), expectedDelta2Ys, expectedDelta2Ys.size(), 0.0001);
    ASSERT_ELEMENTS_NEAR(output->getBlendShapeTargetDeltaZs(0, 2), expectedDelta2Zs, expectedDelta2Zs.size(), 0.0001);
}
