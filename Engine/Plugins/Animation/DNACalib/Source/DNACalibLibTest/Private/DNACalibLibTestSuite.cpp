	...
	: public ::testing::EmptyTestEventListener
{
	...
	{
		UE_LOGF(LogDNACalibLibTest, Verbose, "Test %ls.%ls Starting", *FString(InTestInfo.test_suite_name()), *FString(InTestInfo.name()));
	}
	...
	virtual void OnTestPartResult(const ::testing::TestPartResult& InTestPartResult) override
	{
		...
		{
			UE_LOGF(LogDNACalibLibTest, Error, "FAILED in %ls:%d\n%ls", *FString(InTestPartResult.file_name()), InTestPartResult.line_number(), *FString(InTestPartResult.summary()));
		}
		else
		{
			UE_LOGF(LogDNACalibLibTest, Verbose, "Succeeded in %ls:%d\n%ls", *FString(InTestPartResult.file_name()), InTestPartResult.line_number(), *FString(InTestPartResult.summary()));
		}
	}

	virtual void OnTestEnd(const ::testing::TestInfo& InTestInfo) override
	{
		UE_LOGF(LogDNACalibLibTest, Verbose, "Test %ls.%ls Ending", *FString(InTestInfo.test_suite_name()), *FString(InTestInfo.name()));
	}
};
