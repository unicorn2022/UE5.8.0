...
FDNACalibDNAReader::FDNACalibDNAReader(IDNAReader* Source) :
	ReaderPtr{new FDNAReader<dnac::DNACalibDNAReader>{dnac::DNACalibDNAReader::create(Source->Unwrap(), FMemoryResource::Instance()), Source->GetConfig()}}
{
}
...
ERotationSequence FDNACalibDNAReader::GetRotationSequence() const
{
	return ReaderPtr->GetRotationSequence();
}

FRotationSign FDNACalibDNAReader::GetRotationSign() const
{
	return ReaderPtr->GetRotationSign();
}

EFaceWindingOrder FDNACalibDNAReader::GetFaceWindingOrder() const
{
...
uint16 FDNACalibDNAReader::GetMLTypeCount() const
{
	return ReaderPtr->GetMLTypeCount();
}

uint16 FDNACalibDNAReader::GetMLOperationSetCount(uint16 MLTypeIndex) const
{
	return ReaderPtr->GetMLOperationSetCount(MLTypeIndex);
}

uint16 FDNACalibDNAReader::GetMLOperationCount(uint16 MLTypeIndex, uint16 MLOperationSetIndex) const
{
	return ReaderPtr->GetMLOperationCount(MLTypeIndex, MLOperationSetIndex);
}

EMachineLearnedBehaviorOperationType FDNACalibDNAReader::GetMLOperationType(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const
{
...
TArrayView<const uint32> FDNACalibDNAReader::GetMLOperationParameters(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const
{
	return ReaderPtr->GetMLOperationParameters(MLTypeIndex, MLOperationSetIndex, MLOperationIndex);
}

TArrayView<const uint16> FDNACalibDNAReader::GetMLOperationDependencyOperationSetIndices(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const
{
	return ReaderPtr->GetMLOperationDependencyOperationSetIndices(MLTypeIndex, MLOperationSetIndex, MLOperationIndex);
}

TArrayView<const uint16> FDNACalibDNAReader::GetMLOperationDependencyOperationIndices(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 MLOperationIndex) const
{
	return ReaderPtr->GetMLOperationDependencyOperationIndices(MLTypeIndex, MLOperationSetIndex, MLOperationIndex);
}

uint16 FDNACalibDNAReader::GetMLOperationIndexListCount(uint16 MLTypeIndex, uint16 MLOperationSetIndex) const
{
	return ReaderPtr->GetMLOperationIndexListCount(MLTypeIndex, MLOperationSetIndex);
}

TArrayView<const uint16> FDNACalibDNAReader::GetMLOperationIndicesForLOD(uint16 MLTypeIndex, uint16 MLOperationSetIndex, uint16 LOD) const
{
	return ReaderPtr->GetMLOperationIndicesForLOD(MLTypeIndex, MLOperationSetIndex, LOD);
}

TArrayView<const uint16> FDNACalibDNAReader::GetMLJointsInputIndices() const
{
	return ReaderPtr->GetMLJointsInputIndices();
}

TArrayView<const uint16> FDNACalibDNAReader::GetMLJointsOutputIndices() const
{
	return ReaderPtr->GetMLJointsOutputIndices();
}

TArrayView<const uint16> FDNACalibDNAReader::GetMLJointsParameterKeys() const
{
	return ReaderPtr->GetMLJointsParameterKeys();
}

TArrayView<const uint16> FDNACalibDNAReader::GetMLJointsParameterValues() const
{
	return ReaderPtr->GetMLJointsParameterValues();
}

const FDNAConfig& FDNACalibDNAReader::GetConfig() const
{
