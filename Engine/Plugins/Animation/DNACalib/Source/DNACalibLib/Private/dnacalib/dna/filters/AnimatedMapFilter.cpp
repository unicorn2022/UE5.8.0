...
void AnimatedMapFilter::configure(std::uint16_t animatedMapCount,
...
void AnimatedMapFilter::apply(RawDefinition& dest) {
    ...
    dest.lodAnimatedMapMapping.mapIndices([this](std::uint16_t value) { return remappedIndices.at(value); });
    ...
    extd::filter(dest.animatedMaps.conditionals.outputIndices,
                 [this, &indicesToDelete, &dest](std::uint16_t outputIndex, std::size_t index) {
                     if (!passes(outputIndex)) {
                         ...
                         for (std::uint16_t lodIndex = 0; lodIndex < static_cast<std::uint16_t>(animatedMapLODIndices.size());
                              ++lodIndex) {
