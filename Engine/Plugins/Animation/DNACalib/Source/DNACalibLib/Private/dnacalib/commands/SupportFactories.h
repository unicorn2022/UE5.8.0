...
struct WeightGetterFactory {
    ...
    static GetterFunc create(const Vector<float>& masks) {
        if (masks.empty()) {
            return [](float* /*unused*/, std::size_t /*unused*/) { return 1.0f; };
        }
        return [](float* pMasks, std::size_t index) { return pMasks[index]; };
    }
};
...
struct OperationFactory {
    ...
    static OpFunc create(VectorOperation operation) {
        ...
        case VectorOperation::Interpolate: {
            return [](float a, float b, float weight) { return extd::interpolate(a, b, weight); };
        }
        case VectorOperation::Add: {
            return [](float a, float b, float weight) { return a + (b * weight); };
        }
