...
private:
    ...
    void computeBlendShapeTargetDeltas(FOperation op, FWeightGetter getWeight, DNACalibDNAReaderImpl* output) {
        ...
        if (vertexIndices.empty()) {
            if (deltas.size() != bsVertexIndices.size()) {
                const auto message = formatString(output->getMemoryResource(),
                                                  "No vertex indices set. Current vertex indices in DNA will not be used, as "
                                                  "their number (%hu) differs from the number of set deltas (%hu).",
        ...
        output->setBlendShapeTargetVertexIndices(meshIndex,
                                                 blendShapeTargetIndex,
...
#endif
sc::StatusProvider SetBlendShapeTargetDeltasCommand::Impl::status{VertexIndicesOutOfBoundsError,
...
SetBlendShapeTargetDeltasCommand::SetBlendShapeTargetDeltasCommand(MemoryResource* memRes) :
