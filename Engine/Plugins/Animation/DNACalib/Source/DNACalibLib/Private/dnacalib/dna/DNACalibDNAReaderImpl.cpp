...
void DNACalibDNAReaderImpl::setNeutralJointRotations(ConstArrayView<float> xs,
                                                     ConstArrayView<float> ys,
                                                     ConstArrayView<float> zs) {
    ...
    for (auto& mesh : dna.geometry.meshes) {
        for (auto& bst : mesh.blendShapeTargets) {
            ...
            for (std::size_t si{}; si < bst.deltas.size(); ++si) {
                const float magnitude2 = (bst.deltas.xs[si] * bst.deltas.xs[si]) + (bst.deltas.ys[si] * bst.deltas.ys[si]) +
...
void DNACalibDNAReaderImpl::removeMeshes(ConstArrayView<std::uint16_t> meshIndices) {
    ...
    dna.definition.lodMeshMapping.filterIndices(
        [meshIndices](std::uint16_t value) { return (!extd::contains(meshIndices, value)); });
...
void DNACalibDNAReaderImpl::removeJoints(ConstArrayView<std::uint16_t> jointIndices) {
    ...
    dna.definition.lodJointMapping.filterIndices(
        [jointIndices](std::uint16_t value) { return (!extd::contains(jointIndices, value)); });
    // Collect all distinct element position indices that are referenced by the present LODs
...
void DNACalibDNAReaderImpl::removeBlendShapes(ConstArrayView<std::uint16_t> blendShapeIndices) {
    ...
    dna.definition.lodBlendShapeMapping.filterIndices(
        [blendShapeIndices](std::uint16_t value) { return (!extd::contains(blendShapeIndices, value)); });
...
void DNACalibDNAReaderImpl::removeAnimatedMaps(ConstArrayView<std::uint16_t> animatedMapIndices) {
    ...
    dna.definition.lodAnimatedMapMapping.filterIndices(
        [animatedMapIndices](std::uint16_t value) { return (!extd::contains(animatedMapIndices, value)); });
    ...
    animatedMapFilter.configure(static_cast<std::uint16_t>(dna.definition.animatedMapNames.size()),
                                std::move(allowedAnimatedMapIndices),
                                std::move(lodIndices));
