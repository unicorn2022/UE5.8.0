...
public:
    template<typename TIter,
             typename std::enable_if<std::is_same<unqualified_iter_value_type<TIter>, fvec2>::value, void>::type* = nullptr>
    ...
    bool overlaps(const BoundingBox& other) const {
        return (max[0] >= other.min[0] && other.max[0] >= min[0]) && (max[1] >= other.min[1] && other.max[1] >= min[1]);
    }
