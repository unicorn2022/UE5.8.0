...
void FPhysicsControlAssetEditorEditMode::HitNothing(FEditorViewportClient* InViewportClient)
{
	...
	if (TSharedPtr<FPhysicsControlAssetEditor> Editor = PhysicsControlAssetEditor.Pin())
	{
		Editor->RefreshHierarchyTree();
	}
}
...
void FPhysicsControlAssetEditorEditMode::OpenBodyMenu(FEditorViewportClient* InViewportClient)
{
	...
	OpenContextMenu(Editor.ToSharedRef(), InViewportClient,
		[Editor](FMenuBuilder& InMenuBuilder)
		{
			Editor->BuildMenuWidgetBody(InMenuBuilder);
			Editor->BuildMenuWidgetSelection(InMenuBuilder);
		});
}
