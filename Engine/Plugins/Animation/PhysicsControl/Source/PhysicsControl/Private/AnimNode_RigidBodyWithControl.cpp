...
FAnimNode_RigidBodyWithControl::FAnimNode_RigidBodyWithControl()
	...
	, bWorldGeometryCollisionEnabledHasPhysics(true)
	, bWorldGeometryCollisionEnabledHasQuery(false)
	, bWorldGeometryCollisionEnabledHasProbe(false)
...
{
	SimulationSpaceTransform = SpaceToWorldTransform(SimulationSpace, ComponentToWorld, BoneToComponent);
	...
	FVector&                 SpaceAngularAcc)
{
	...
	FVector PrevSpaceLinearVel = Chaos::FVec3::CalculateVelocity(
		PreviousPreviousSimulationSpaceTransform.GetTranslation(), PreviousSimulationSpaceTransform.GetTranslation(), Dt);
	SpaceLinearVel = Chaos::FVec3::CalculateVelocity(
		PreviousSimulationSpaceTransform.GetTranslation(), SimulationSpaceTransform.GetTranslation(), Dt);
	SpaceLinearAcc = (SpaceLinearVel - PrevSpaceLinearVel) / Dt;

	// World-space component angular velocity and acceleration
	FVector PrevSpaceAngularVel = Chaos::FRotation3::CalculateAngularVelocity(
		PreviousPreviousSimulationSpaceTransform.GetRotation(), PreviousSimulationSpaceTransform.GetRotation(), Dt);
	SpaceAngularVel = Chaos::FRotation3::CalculateAngularVelocity(
		PreviousSimulationSpaceTransform.GetRotation(), SimulationSpaceTransform.GetRotation(), Dt);
...
void FAnimNode_RigidBodyWithControl::SetupControls(USkeletalMeshComponent* const SkeletalMeshComponent)
{
	...
	if (SkeletalMeshComponent)
	{
		...
		{
			UE_LOGF(LogPhysicsControl, Warning, "Invalid Skeletal Mesh");
		}
	}
	else
	{
		UE_LOGF(LogPhysicsControl, Warning, "Invalid Skeletal Mesh Component");
	}
}
...
void FAnimNode_RigidBodyWithControl::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (bEnabled && Simulation)	
	{
		...
		if ((EvaluationResetTime > 0.0f) && !bFirstEvalSinceReset)
		{
			...
			{
				UE_LOGF(LogPhysicsControl, Verbose, "%ls Time-Based Reset", *Output.AnimInstanceProxy->GetAnimInstanceName());
				ResetSimulatedTeleportType = ETeleportType::ResetPhysics;
			}
		}
		...
		// Now we know the base bone, we can calculate the simulation space transform
		SimulationSpaceTransform = SpaceToWorldTransform(SimulationSpace, CompWorldSpaceTM, BaseBoneTM);
		...
		if (bResetOrTeleportBodies)
		{
			...
			switch(ResetSimulatedTeleportType)
			{
				...
				{
					UE_LOGF(LogPhysicsControl, Verbose, "%ls TeleportPhysics (Scale: %f %f %f)", *Output.AnimInstanceProxy->GetAnimInstanceName(), CompWorldSpaceTM.GetScale3D().X, CompWorldSpaceTM.GetScale3D().Y, CompWorldSpaceTM.GetScale3D().Z);
				...
				{
					UE_LOGF(LogPhysicsControl, Verbose, "%ls ResetPhysics (Scale: %f %f %f)", *Output.AnimInstanceProxy->GetAnimInstanceName(), CompWorldSpaceTM.GetScale3D().X, CompWorldSpaceTM.GetScale3D().Y, CompWorldSpaceTM.GetScale3D().Z);
		...
		if (bNeedsSimulationTick)
		{
			...
			if (bHaveSetupControls)
			{
				...
				ApplyForcesAndTorques(CompWorldSpaceTM, BaseBoneTM);
			}
			...
			ApplyKinematicTargets(CompWorldSpaceTM, BaseBoneTM);
			...
			UpdateWorldObjects(SimulationSpaceLinearVelocity, SimulationSpaceAngularVelocity, DeltaSeconds);
			UpdateClothColliderObjects();

			Simulation->UpdateSimulationSpace(
				SimulationSpaceTransform, SimulationSpaceLinearVelocity, SimulationSpaceAngularVelocity,
			...
			CollisionDetectorSettings.BoundsVelocityInflation = BoundsVelocityMultiplier;
			CollisionDetectorSettings.MaxVelocityBoundsExpansion = MaxVelocityBoundsExpansion;
			CollisionDetectorSettings.bAllowCCD = bAllowCCD;
...
void FAnimNode_RigidBodyWithControl::InitPhysics(const UAnimInstance* InAnimInstance)
{
	...
	if(bEnabled)
	{
		...
		for (FBoneIndexType InsertBone : InsertionOrder)
		{
			...
			{
				const UBodySetup* BodySetup = PhysicsAssetToUse->SkeletalBodySetups[BodyInstance->InstanceBodyIndex];
				...
				if (ActorHandle)
				{
					ActorHandle->SetCCDEnabled(BodySetup->DefaultInstance.bUseCCD);
...
void FAnimNode_RigidBodyWithControl::UpdateClothColliderObjects()
{
	for (FClothCollider& ClothCollider : ClothColliders)
	{
		const USkeletalMeshComponent* const SkeletalMeshComp = ClothCollider.SkeletalMeshComponent.Get();
		if (ClothCollider.ActorHandle && SkeletalMeshComp)
		{
			// Calculate the sim-space transform of this object
			const FTransform CompWorldTransform = SkeletalMeshComp->GetBoneTransform(ClothCollider.BoneIndex);
			FTransform CompSpaceTransform;
			CompSpaceTransform.SetTranslation(SimulationSpaceTransform.InverseTransformPosition(
				CompWorldTransform.GetLocation()));
			CompSpaceTransform.SetRotation(SimulationSpaceTransform.InverseTransformRotation(
...
inline bool IsComponentCollisionEnabled(const FAnimNode_RigidBodyWithControl* Node, const UPrimitiveComponent* const Component)
{
	if (Component->GetCollisionResponseToChannel(ECC_PhysicsBody) != ECollisionResponse::ECR_Block)
	{
		return false;
	}

	const ECollisionEnabled::Type CollisionEnabled = Component->GetBodyInstance()->GetCollisionEnabled();

	return (Node->bWorldGeometryCollisionEnabledHasPhysics && CollisionEnabledHasPhysics(CollisionEnabled))
		|| (Node->bWorldGeometryCollisionEnabledHasQuery && CollisionEnabledHasQuery(CollisionEnabled))
		|| (Node->bWorldGeometryCollisionEnabledHasProbe && CollisionEnabledHasProbe(CollisionEnabled));
}

inline bool IsComponentDesiredInSim(const FAnimNode_RigidBodyWithControl* Node, const UPrimitiveComponent* Component)
{
	...
	if(!IsComponentCollisionEnabled(Node, Component))
	{
		return false;
	}
	return true;
		...
		for (const FOverlapResult& Overlap : Overlaps)
		{
			if (UPrimitiveComponent* OverlapComp = Overlap.GetComponent())
			{
				...
				else
				{
					...
					if (!bIsSelf && IsComponentDesiredInSim(this, OverlapComp))
					{
						...
						ComponentsInSim.Add(
							OverlapComp, FWorldObject(ActorHandle, ComponentsInSimTick, Overlap.Component));
						// We would need this: 
...
void FAnimNode_RigidBodyWithControl::ExpireWorldObjects()
{
	...
	for (auto& WorldEntry : ComponentsInSim)
	{
		...
		const UPrimitiveComponent* WorldComp = WorldObject.Component.Get();
		...
		if (((ComponentsInSimTick - WorldObject.LastSeenTick) > ExpireTickCount)	// Haven't seen this object for a while
			|| !IsComponentDesiredInSim(this, WorldComp) )
		{
...
void FAnimNode_RigidBodyWithControl::UpdateWorldObjects(
	const FVector& LinearVel, const FVector& AngularVel, const float DeltaSeconds)
{
	...
	if (bCalculateVelocitiesForWorldGeometry)
	{
		...
		const FVector PrevSimSpacePosition = 
			SimulationSpaceTransform.GetLocation() - LinearVel * DeltaSeconds;
		...
		const FQuat PrevSimSpaceOrientation = 
			SimSpaceDeltaOrientation.Inverse() * SimulationSpaceTransform.GetRotation();
		...
		{ 
			const UPrimitiveComponent* OverlapComp = WorldEntry.Value.Component.Get();
			if (OverlapComp != nullptr)
			{
				...
				const FVector CompWorldPosition = CompWorldTransform.GetLocation();
				const FQuat CompWorldOrientation = CompWorldTransform.GetRotation();

				const FVector CompSimSpacePosition = SimulationSpaceTransform.InverseTransformPosition(CompWorldPosition);
				const FQuat CompSimSpaceOrientation = SimulationSpaceTransform.InverseTransformRotation(CompWorldOrientation);

				// TODO - sort out scale for world objects in local sim
				const FTransform CompSimSpaceTransform(CompSimSpaceOrientation, CompSimSpacePosition, FVector::OneVector);
				...
				const FVector PrevCompWorldPosition = CompWorldPosition - Velocity * DeltaSeconds;
				const FQuat DeltaWorldOrientation = FQuat::MakeFromRotationVector(AngularVelocity * DeltaSeconds);
				const FQuat PrevCompWorldOrientation = DeltaWorldOrientation.Inverse() * CompWorldOrientation;

				const FVector PrevCompSimSpacePosition = PrevSimSpaceTransform.InverseTransformPosition(CompWorldPosition);
				const FQuat PrevCompSimSpaceOrientation = PrevSimSpaceTransform.InverseTransformRotation(CompWorldOrientation);
				const FTransform PrevCompSimSpaceTransform(PrevCompSimSpaceOrientation, PrevCompSimSpacePosition, FVector::OneVector);

				ActorHandle->InitWorldTransform(PrevCompSimSpaceTransform);
				ActorHandle->SetKinematicTarget(CompSimSpaceTransform);
			}
		}
	}
	else
	{
		for (const auto& WorldEntry : ComponentsInSim)
		{
			...
			if (OverlapComp != nullptr)
			{
				...
				ActorHandle->InitWorldTransform(CompSimSpaceTransform);
			}
		}
	}
}
...
void FAnimNode_RigidBodyWithControl::InitializeBoneReferences(const FBoneContainer& RequiredBones) 
{
	...
	{
		UE_LOGF(LogPhysicsControl, Warning, "FAnimNode_RigidBodyWithControl::InitializeBoneReferences: Required bones is invalid for Skeleton %ls / SkeletalMesh %ls.",
	...
	if (!BaseBoneRef.HasValidSetup())
	{
		...
		UE_LOGF(LogPhysicsControl, Log, "FAnimNode_RigidBodyWithControl: RigidBodyWithControl Simulation Base Bone \'%ls\' does not exist on SkeletalMesh %ls.", *BaseBoneRef.BoneName.ToString(), *GetNameSafe(RequiredBones.GetSkeletalMeshAsset()));
	}
