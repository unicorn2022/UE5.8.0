...
// Overrides the per-constraint LimitViolationResponse for all controls. Useful for comparing
// responses without having to change each constraint individually.
// -1 = no override (use per-constraint setting)
//  0 = None
//  1 = ClampLinear
//  2 = ClampExact
//  3 = WidenLimits
static TAutoConsoleVariable<int32> CVarPhysicsControlLimitViolationResponseOverride(
	TEXT("p.PhysicsControl.LimitViolationResponseOverride"),
	-1,
	TEXT("Overrides the LimitViolationResponse on all physics controls: -1=off, 0=None, 1=ClampLinear, 2=ClampExact, 3=WidenLimits"),
	ECVF_Default);
...
{
	if (!ensure(InSkeletalMeshComponent))
	{
		return false;
	}
	OutPoseData = nullptr;
	const USkeletalMesh* SkeletalMesh = InSkeletalMeshComponent->GetSkeletalMeshAsset();
	if (!SkeletalMesh)
	{
		return false;
	}
	...
	{
		UE_LOGF(LogPhysicsControl, Warning, "Failed to find BoneIndex for %ls in %ls", 
			*InBoneName.ToString(), GetOwner() ? *GetOwner()->GetName() : TEXT("Unknown"));
		return false;
	}
		...
		!CachedSkeletalMeshData->BoneDatas.IsEmpty())
	{
		...
		UE_LOGF(LogPhysicsControl, Warning, 
			"BoneIndex is out of range in %ls", GetOwner() ? *GetOwner()->GetName() : TEXT("Unknown"));

	}
	UE_LOGF(LogPhysicsControl, Warning, "Failed to find bone data for %ls in %ls",
		*InBoneName.ToString(), GetOwner() ? *GetOwner()->GetName() : TEXT("Unknown"));
	return false;
}
	...
	const FName                     InBoneName)
{
		...
		!CachedSkeletalMeshData->BoneDatas.IsEmpty())
	{
		...
		UE_LOGF(LogPhysicsControl, Warning, "BoneIndex is out of range in %ls", 
...
void UPhysicsControlComponent::UpdateCachedSkeletalBoneData(float DeltaTime)
{
		...
		CachedSkeletalMeshDataPair : CachedPoseDatas)
	{
		...
		{
			// Ensure any in-flight parallel animation evaluation or physics blend task has
			// completed before we read the editable component space transforms. Without this,
			// SwapEvaluationContextBuffers() (called during parallel work) can temporarily replace
			// the buffer that GetEditableComponentSpaceTransforms() returns, causing us to read
			// stale or invalid data. This can occur when tick prerequisites push this component
			// into a later tick group (e.g. PostPhysics), overlapping with the skeletal mesh's 
...
void UPhysicsControlComponent::ApplyKinematicTarget(const FPhysicsBodyModifierRecord& Record) const
{
	...
	else
	{
		...
		const FTransform CurrentBodyTM = Component->GetComponentToWorld();
		FTransform KinematicTargetWS = CurrentBodyTM;
	...
	USkeletalMeshComponent* InSkeletalMeshComponent)
{
	...
	{
		UE_LOGF(LogPhysicsControl, Warning, "Invalid skeletal mesh component in %ls", 
			GetOwner() ? *GetOwner()->GetName() : TEXT("Unknown"));
		return false;
	}
	...
	UE_LOGF(LogPhysicsControl, Warning, "Failed to remove skeletal mesh component reference for caching");
	return false;
}
	...
	USkeletalMeshComponent* InSkeletalMeshComponent)
{
	...
	UE_LOGF(LogPhysicsControl, Warning, "Failed to remove skeletal mesh component reference for modifier");
	return false;
}
	...
	FPhysicsControlRecord& Record, FConstraintInstance* ConstraintInstance)
{
	...
	ConstraintInstance->SetLinearDriveAccelerationMode(Data.bUseAccelerationDriveMode);
	ConstraintInstance->SetAngularDriveAccelerationMode(Data.bUseAccelerationDriveMode);
...
//======================================================================================================================
void UPhysicsControlComponent::ApplyControl(
	...
	const FConstraintInstance* DefaultConstraintInstance)
{
	...
	{
		// PhysicsAssetConstraintInstance and DefaultConstraintInstance were looked up once in UpdateControls before this call.

		// Resolve the response up front. When switching from WidenLimits to ClampLinear/ClampExact,
		// the clamp must see the authored limit values, not previously widened ones, so restore here
		// before computing the target.
		EAngularDriveLimitViolationResponse Response = EAngularDriveLimitViolationResponse::None;
		if (PhysicsAssetConstraintInstance)
		{
			Response = PhysicsAssetConstraintInstance->ProfileInstance.AngularDrive.LimitViolationResponse;
			const int32 Override = CVarPhysicsControlLimitViolationResponseOverride.GetValueOnGameThread();
			if (Override >= 0 && Override <= (int32)EAngularDriveLimitViolationResponse::WidenLimits)
			{
				Response = (EAngularDriveLimitViolationResponse)Override;
			}
			if (Record.bJointLimitsWidened && DefaultConstraintInstance &&
				(Response == EAngularDriveLimitViolationResponse::ClampLinear ||
				 Response == EAngularDriveLimitViolationResponse::ClampExact))
			{
				PhysicsAssetConstraintInstance->RestoreAngularLimitsToDefault(*DefaultConstraintInstance);
				Record.bJointLimitsWidened = false;
			}
		}
		...
		bool bUsePreviousSkeletalTargetTM = CurrentUpdateCounter.HasEverBeenUpdated() && (CurrentUpdateCounter.Get() == Record.ExpectedUpdateCounter.Get());
		CalculateControlTargetData(TargetTM, SkeletalTargetTM, TargetVelocity, TargetAngularVelocity, Record, bUsePreviousSkeletalTargetTM);
		...
		// Apply the limit-violation response to the final composed drive target. Both clamp and
		// widen operate on TargetTM (not just the skeletal component) so they act on the rotation
		// that is actually sent to the physics constraint as the drive target.
		if (PhysicsAssetConstraintInstance && DefaultConstraintInstance)
		{
			if (Response == EAngularDriveLimitViolationResponse::ClampLinear ||
				Response == EAngularDriveLimitViolationResponse::ClampExact)
			{
				QUICK_SCOPE_CYCLE_COUNTER(STAT_PhysicsControl_ClampDriveTarget);
				const bool bUseExact = (Response == EAngularDriveLimitViolationResponse::ClampExact);
				TargetTM.SetRotation(PhysicsAssetConstraintInstance->ClampDriveTargetToLimits(TargetTM.GetRotation(), bUseExact));
			}
			else if (Response == EAngularDriveLimitViolationResponse::WidenLimits)
			{
				QUICK_SCOPE_CYCLE_COUNTER(STAT_PhysicsControl_WidenLimits);
				Record.bJointLimitsWidened = PhysicsAssetConstraintInstance->WidenLimitsForDriveTarget(
					TargetTM.GetRotation(), *DefaultConstraintInstance);
			}
			else if (Record.bJointLimitsWidened)
			{
				// Response is None; restore any previously widened limits.
				PhysicsAssetConstraintInstance->RestoreAngularLimitsToDefault(*DefaultConstraintInstance);
				Record.bJointLimitsWidened = false;
			}
		}
...
void UPhysicsControlComponent::ApplyBodyModifier(FPhysicsBodyModifierRecord& Record)
{
	...
	if (BodyInstance)
	{
		...
		case EPhysicsMovementType::Default:
			...
			break;
		default:
			UE_LOGF(LogPhysicsControl, Warning, "Invalid movement type %d in %ls",
...
bool UPhysicsControlComponent::DestroyControl(const FName Name, const EDestroyBehavior DestroyBehavior)
{
	...
	{
		UE_LOGF(LogPhysicsControl, Warning, "DestroyControl - invalid control name %ls for %ls",
...
bool UPhysicsControlComponent::DestroyBodyModifier(const FName Name, const EDestroyBehavior DestroyBehavior)
{
	...
	if (bWarnAboutInvalidNames)
	{
		UE_LOGF(LogPhysicsControl, Warning, "DestroyBodyModifier - invalid modifier name %ls for %ls",
			*Name.ToString(), GetOwner() ? *GetOwner()->GetName() : TEXT("Unknown"));
	}
	return false;
}
