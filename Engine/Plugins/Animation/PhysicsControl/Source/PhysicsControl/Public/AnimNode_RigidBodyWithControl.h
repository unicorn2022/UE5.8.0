...
// Adjusts the constraint spring drive settings to reflect the control data. Also enabled or
// disables the underlying constraint based on whether there is any strength/damping. Returns true
// if there is some control, false if the drive has no effect
...
PHYSICSCONTROL_API void SetPhysicsJointEnabled(
	ImmediatePhysics::FJointHandle& JointHandle, const bool bIsEnabled);

// Deprecated versions of the functions above

UE_DEPRECATED(5.8, "Use the version of UpdateBodyFromModifierData taking references instead instead")
PHYSICSCONTROL_API inline void UpdateBodyFromModifierData(
	ImmediatePhysics::FActorHandle*    ActorHandle,
	ImmediatePhysics::FSimulation*     PhysicsSimulation,
	const FPhysicsControlModifierData& ModifierData,
	const FVector&                     SimSpaceGravity)
{
	if (ActorHandle && PhysicsSimulation)
	{
		UpdateBodyFromModifierData(*ActorHandle, *PhysicsSimulation, ModifierData, SimSpaceGravity);
	}
}

UE_DEPRECATED(5.8, "Use the version of UpdateDriveSpringDamperSettings taking references instead instead")
PHYSICSCONTROL_API inline bool UpdateDriveSpringDamperSettings(
	ImmediatePhysics::FJointHandle* JointHandle,
	const Chaos::FPBDJointSettings& Settings,
	const FPhysicsControlData& Data,
	const FPhysicsControlMultiplier& Multiplier)
{
	if (JointHandle)
	{
		return UpdateDriveSpringDamperSettings(*JointHandle, Settings, Data, Multiplier);
	}
	return false;
}

UE_DEPRECATED(5.8, "Use the version of CreatePhysicsJoint taking references instead instead")
PHYSICSCONTROL_API inline ImmediatePhysics::FJointHandle* CreatePhysicsJoint(
	ImmediatePhysics::FSimulation* Simulation,
	ImmediatePhysics::FActorHandle* ChildActorHandle,
	ImmediatePhysics::FActorHandle* ParentActorHandle)
{
	...
	{
		return CreatePhysicsJoint(*Simulation, *ChildActorHandle, *ParentActorHandle);
	}
	return nullptr;
}

UE_DEPRECATED(5.8, "Use the version of SetPhysicsJointEnabled taking references instead instead")
PHYSICSCONTROL_API inline void SetPhysicsJointEnabled(ImmediatePhysics::FJointHandle* JointHandle, const bool bIsEnabled)
{
	if (JointHandle)
	{
		SetPhysicsJointEnabled(*JointHandle, bIsEnabled);
	}
}
...
public:
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = CollisionSettings, meta = (ClampMin="1.0", ClampMax="2.0"))
	...
	UPROPERTY(EditAnywhere, Category = CollisionSettings, meta = (editcondition = "bEnableWorldGeometry"))
...
public:
	...
	UPROPERTY(EditAnywhere, Category = CollisionSettings)
	uint8 bWorldGeometryCollisionEnabledHasPhysics : 1;
	
	UPROPERTY(EditAnywhere, Category = CollisionSettings)
	uint8 bWorldGeometryCollisionEnabledHasQuery : 1;
	
	UPROPERTY(EditAnywhere, Category = CollisionSettings)
	uint8 bWorldGeometryCollisionEnabledHasProbe : 1;
	...
	*/
	...
	/**
	 * Forces/torques that should be applied to dynamic bodies that are under the influence of a body modifier.
	 */
	UPROPERTY(EditAnywhere, Category = Controls, meta = (PinShownByDefault))
	FRigidBodyForcesAndTorques ForcesAndTorques;
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = CollisionSettings, meta = (ClampMin = 0))
	float BoundsVelocityMultiplier = 0.0f;
	 ...
	 */
	UPROPERTY(EditAnywhere, Category = CollisionSettings, meta = (ClampMin = 0))
	float MaxVelocityBoundsExpansion = 0.0f;
...
private:
	...
	UE_API void UpdateWorldForces(const FTransform& ComponentToWorld, const FTransform& BaseBoneTM, const float DeltaSeconds);
...
protected:
	...
	UE_API void ApplyForcesAndTorques(
		const FTransform& ComponentToWorld, const FTransform& BaseBoneTM);

	UE_API void ApplyForceAndTorque(
	...
	UE_API void ApplyKinematicTargets(const FTransform& CompWorldSpaceTM, const FTransform& BaseBoneTM);
	...
	// Gather cloth collision sources from the supplied Skeletal Mesh and add a kinematic actor
	...
	UE_API void UpdateClothColliderObjects();
	...
	UE_API void UpdateWorldObjects(const FVector& LinearVel, const FVector& AngularVel, const float DeltaSeconds);
...
private:
	...
	struct FWorldObject
	{
		...
		FWorldObject(
			...
			int32                               InLastSeenTick, 
			TWeakObjectPtr<UPrimitiveComponent> InComponent)
			: Component(InComponent), ActorHandle(InActorHandle), LastSeenTick(InLastSeenTick), bExpired(false){}
	...
	struct FClothCollider
	{
		...
		TWeakObjectPtr<const USkeletalMeshComponent> SkeletalMeshComponent; // Parent skeleton.
	...
	// List of actors in the sim that represent objects collected from the world. Note that the key
	// should never be dereferenced, as it is not a smart pointer, and might have been deleted.
	// Instead it should just be used for comparisons, and the actual pointer to the component is
	// stored in the WorldObject.
