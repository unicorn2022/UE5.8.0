...
// Calculates the angular velocity that would go from PrevQ to CurrentQ in Dt. Returns zero if Dt is invalid.
FORCEINLINE FVector CalculateAngularVelocity(const FQuat& PrevQ, const FQuat& CurrentQ, float Dt)
{
	...
	return (Dt > UE_SMALL_NUMBER) ? DeltaQ.ToRotationVector() / Dt : FVector::ZeroVector;
}

// Calculates the linear velocity that would go from PrevP to CurrentP in Dt. Returns zero if Dt is invalid.
FORCEINLINE FVector CalculateLinearVelocity(const FVector& PrevP, const FVector& CurrentP, float Dt)
{
	return (Dt > UE_SMALL_NUMBER) ? (CurrentP - PrevP) / Dt : FVector::ZeroVector;
}

// Converts strength/damping ratio/extra damping into spring stiffness/damping. Clamps strength to >= 0.
