...
struct FPhysicsControlModifierSparseData
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = PhysicsControl, meta = (editcondition = "bEnablebEnableCCD"))
	uint8 bEnableCCD : 1;
	...
	UPROPERTY(EditAnywhere, Category = PhysicsControl, meta = (InlineEditConditionToggle))
	uint8 bEnablebEnableCCD : 1;
};

// Specifies a space for queries, forces etc
UENUM(BlueprintType)
enum class EPhysicsControlSpace : uint8
{
	// World space. For a control rig, this is the space of the control rig, rather than the "real"
	// world, so is the same as Component.
	World,
	// If a skeleton/bone is being used, the target is in the space of that component. If not then
	// it is in the space of the owning component (e.g. PhysicsControl, or ControlRig etc).
	Component,
...
struct FPhysicsControlModifierData
{
	...
	// The space in which kinematic targets should operate. Note that some target spaces are more
	// expensive than others due to the calculations: IgnoreTarget is the cheapest,
	// OffsetInBoneSpace and OffsetInComponentSpace are cheap, but OffsetInWorldSpace is relatively expensive.
...
struct FPhysicsControlSparseData
{
	...
	FPhysicsControlSparseData()
		...
		, bUseAccelerationDriveMode(true)
		// the flags
		...
		, bEnablebUseAccelerationDriveMode(true)
	{
	}
	...
	/**
	 * When true (the default), strength values are interpreted as accelerations (mass-independent).
	 * When false they are interpreted as forces (heavier bodies respond more slowly).
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = PhysicsControl, meta = (editcondition = "bEnablebUseAccelerationDriveMode"))
	uint8 bUseAccelerationDriveMode : 1;
	...
	UPROPERTY(EditAnywhere, Category = PhysicsControl, meta = (InlineEditConditionToggle))
	uint8 bEnablebUseAccelerationDriveMode : 1;
};
...
struct FPhysicsControlData
{
	...
	bool bOnlyControlChildObject;
	 ...
	 // When true (the default), the strength values are interpreted as accelerations - so the same setting
	 // produces a comparable response regardless of body mass. When false, the strength values are
	 // interpreted as forces - so heavier bodies respond more slowly to the same setting. Switching this
	 // value changes the effective strength dramatically, so retune after toggling.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = PhysicsControl)
	bool bUseAccelerationDriveMode;
...
UENUM(BlueprintType)
enum class EPhysicsControlForceType : uint8
{
	Force,
	Acceleration,
	Impulse,
...
UE_API EPhysicsControlForceType ConvertForceType(ImmediatePhysics_Shared::EForceType InForceType);
UE_API ImmediatePhysics_Shared::EForceType ConvertForceType(EPhysicsControlForceType InForceType);

USTRUCT(BlueprintType)
struct FPhysicsControlForceAndTorqueData
{
	GENERATED_BODY();

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	EPhysicsControlSpace LocationSpace = EPhysicsControlSpace::Body;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	EPhysicsControlSpace ForceAndTorqueSpace = EPhysicsControlSpace::World;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	EPhysicsControlForceType Type = EPhysicsControlForceType::Force;

	/// The linear force/acceleration/impulse/velocity change
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	FVector Linear = FVector::ZeroVector;

	/// The angular torque/acceleration/impulse/velocity change
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	FVector Angular = FVector::ZeroVector;

	// The position at which the linear force will be applied
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	FVector Location = FVector::ZeroVector;
};

USTRUCT(BlueprintType)
struct FPhysicsControlNamedForceAndTorqueData
{
	GENERATED_BODY();

	FPhysicsControlNamedForceAndTorqueData() {}

	FPhysicsControlNamedForceAndTorqueData(const FName InName, const FPhysicsControlForceAndTorqueData& InData)
		: Name(InName), ForceAndTorqueData(InData) {}

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	FName Name;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Force)
	FPhysicsControlForceAndTorqueData ForceAndTorqueData;
};
