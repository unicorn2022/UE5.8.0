	...
	bool bBuildAdditiveBase, ACLPhantomTrackMode PhantomTrackMode)
{
	...
	{
		UE_LOGF(LogAnimationCompression, Fatal, "Animation sequence is additive but it is not baked into the raw data, this is not supported. [%ls]", *CompressibleAnimData.FullName);
	...
	for (int32 UETrackIndex = 0; UETrackIndex < NumUETracks; ++UETrackIndex)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAnimationCompression, Warning, "Animation sequence has phantom tracks that do not map to any skeleton bone. Track %d in [%ls]", UETrackIndex, *CompressibleAnimData.FullName);
			}
