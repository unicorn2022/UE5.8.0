...
void UAnimationCompressionLibraryDatabase::BuildDatabase(TArray<uint8>& OutCompressedBytes, TArray<uint64>& OutAnimSequenceMappings, TArray<uint8>& OutBulkData, bool bStripLowestTier)
{
	...
	for (UAnimSequence* AnimSeq : AnimSequences)
	{
		...
		{
			UE_LOGF(LogAnimationCompression, Warning, "ACL Database mapping is stale. '%ls' no longer references it. [%ls]", *AnimSeq->GetPathName(), *GetPathName());
			continue;
		}
		...
		if (!AnimData.IsValid())
		{
			UE_LOGF(LogAnimationCompression, Warning, "Cannot include a invalid sequence '%ls' in the ACL database. [%ls]", *AnimSeq->GetPathName(), *GetPathName());
			continue;
		}
	...
	if (MergeResult.any())
	{
		...
		UE_LOGF(LogAnimationCompression, Error, "ACL failed to merge databases: %ls [%ls]", ANSI_TO_TCHAR(MergeResult.c_str()), *GetPathName());
		return;
	}
	...
	if (SplitResult.any())
	{
		...
		UE_LOGF(LogAnimationCompression, Error, "ACL failed to split database: %ls [%ls]", ANSI_TO_TCHAR(SplitResult.c_str()), *GetPathName());
	...
	if (bStripLowestTier && SplitDB->has_bulk_data(acl::quality_tier::lowest_importance))
	{
		...
		if (StripResult.any())
		{
			...
			UE_LOGF(LogAnimationCompression, Warning, "ACL failed to strip lowest database tier: %ls [%ls]", ANSI_TO_TCHAR(StripResult.c_str()), *GetPathName());
		}
		else
		{
	...
	UE_LOGF(LogAnimationCompression, Log, "ACL DB [%ls] Sequences (%u) went from %.2f MB -> %.2f MB. DB is %.2f MB",
		*GetPathName(), NumSequences, BytesToMB(TotalSizeSeqOld), BytesToMB(TotalSizeSeqNew), BytesToMB(SplitDB->get_total_size()));
	UE_LOGF(LogAnimationCompression, Log, "    DB metadata is %.2f MB", BytesToMB(SplitDB->get_size()));
	UE_LOGF(LogAnimationCompression, Log, "    DB medium tier is %.2f MB", BytesToMB(BulkDataSizeMedium));
	UE_LOGF(LogAnimationCompression, Log, "    DB lowest tier is %.2f MB%ls", BytesToMB(BulkDataSizeLow), bStripLowestTier ? TEXT(" (stripped)") : TEXT(""));
...
uint32 UAnimationCompressionLibraryDatabase::SetVisualFidelityImpl(ACLVisualFidelity VisualFidelity, ACLVisualFidelityChangeResult* OutResult)
{
	...
	UE_LOGF(LogAnimationCompression, Log, "ACL database is requesting visual fidelity %ls [%ls]", VisualFidelityToString(VisualFidelity), *GetPathName());
...
static void LogRequestResult(const UAnimationCompressionLibraryDatabase& Database, acl::database_stream_request_result Result)
{
	...
	{
	default:
		UE_LOGF(LogAnimationCompression, Log, "Unknown ACL database stream request result: %u [%ls]", uint32(Result), *Database.GetPathName());
		break;
	case acl::database_stream_request_result::done:
		UE_LOGF(LogAnimationCompression, Log, "ACL database streaming is done [%ls]", *Database.GetPathName());
		break;
	case acl::database_stream_request_result::dispatched:
		break;
	case acl::database_stream_request_result::streaming_in_progress:
		UE_LOGF(LogAnimationCompression, Log, "ACL database streaming is already in progress [%ls]", *Database.GetPathName());
		break;
	case acl::database_stream_request_result::context_not_initialized:
		UE_LOGF(LogAnimationCompression, Log, "ACL database context not initialized [%ls]", *Database.GetPathName());
		break;
	case acl::database_stream_request_result::invalid_database_tier:
		UE_LOGF(LogAnimationCompression, Log, "Specified ACL database tier is invalid [%ls]", *Database.GetPathName());
		break;
	case acl::database_stream_request_result::no_free_streaming_requests:
		UE_LOGF(LogAnimationCompression, Log, "Failed to find a free ACL database streaming request [%ls]", *Database.GetPathName());
		break;
	}
}
...
bool UAnimationCompressionLibraryDatabase::UpdateVisualFidelityTicker(float DeltaTime)
{
	...
	while (FidelityChangeRequests.Num() != 0)
	{
		...
		{
			UE_LOGF(LogAnimationCompression, Log, "ACL database is changing visual fidelity from %ls to %ls [%ls]", VisualFidelityToString(CurrentVisualFidelity), VisualFidelityToString(Request.Fidelity), *GetPathName());
