...
void FACLDatabaseCompressedAnimData::Bind(const TArrayView<uint8> BulkData)
{
	...
	else
	{
		...
		UE_LOGF(LogAnimationCompression, Warning, "ACL Database mapping is stale. [0x%X] should be contained but isn't.", SequenceNameHash);
...
void UAnimBoneCompressionCodec_ACLDatabase::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	{
		UE_LOGF(LogAnimationCompression, Error, "ACL database codec must be the only codec in its parent bone compression settings asset. [%ls]", *Settings->GetPathName());
	}
}
...
void UAnimBoneCompressionCodec_ACLDatabase::DecompressPose(FAnimSequenceDecompressionContext& DecompContext, const BoneTrackArray& RotationPairs, const BoneTrackArray& TranslationPairs, const BoneTrackArray& ScalePairs, TArrayView<FTransform>& OutAtoms) const
{
	...
	{
		UE_LOGF(LogAnimationCompression, Warning, "ACL failed initialize decompression context, database won't be used");
