...
void FACLPlugin::ListCodecs(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogAnimationCompression, Log, "===== Bone Compression Setting Assets =====");
	for (const UAnimBoneCompressionSettings* Settings : BoneSettings)
	{
		...
		UE_LOGF(LogAnimationCompression, Log, "%ls ...", *Settings->GetPathName());
		UE_LOGF(LogAnimationCompression, Log, "    used by %d / %d (%.1f %%) anim sequences", NumReferences, AnimSequences.Num(), Percentage(NumReferences, AnimSequences.Num()));
		UE_LOGF(LogAnimationCompression, Log, "    uses %.2f MB / %.2f MB (%.1f %%)", BytesToMB(UsedSize), BytesToMB(TotalSize), Percentage(UsedSize, TotalSize));
	}

	UE_LOGF(LogAnimationCompression, Log, "===== Bone Compression Codecs =====");
	for (const UAnimBoneCompressionCodec* Codec : BoneCodecs)
	{
		...
		{
			UE_LOGF(LogAnimationCompression, Log, "%ls ...", *Codec->GetPathName());
		}
		else
		{
			UE_LOGF(LogAnimationCompression, Log, "%ls (%ls) ...", *Codec->GetPathName(), *Codec->Description);
		}
	...
	UE_LOGF(LogAnimationCompression, Log, "===== Curve Compression Setting Assets =====");
	...
	UE_LOGF(LogAnimationCompression, Log, "===== Curve Compression Codecs =====");
	...
	UE_LOGF(LogAnimationCompression, Log, "===== Animation Compression Library Database Assets =====");
	for (const UAnimationCompressionLibraryDatabase* Database : Databases)
	{
		...
		UE_LOGF(LogAnimationCompression, Log, "%ls ...", *Database->GetPathName());
		UE_LOGF(LogAnimationCompression, Log, "    used by %d / %d (%.1f %%) anim sequences", NumReferences, AnimSequences.Num(), Percentage(NumReferences, AnimSequences.Num()));
		UE_LOGF(LogAnimationCompression, Log, "    sequences use %.2f MB", BytesToMB(SequencesSize));
		UE_LOGF(LogAnimationCompression, Log, "    database uses %.2f MB (%.2f MB streamable)", BytesToMB(DatabaseTotalSize), BytesToMB(DatabaseBulkDataSize));
	}
...
void FACLPlugin::ListAnimSequences(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogAnimationCompression, Log, "===== Anim Sequence Assets =====");
	for (const UAnimSequence* AnimSeq : AnimSequences)
	{
		UE_LOGF(LogAnimationCompression, Log, "%ls ...", *AnimSeq->GetPathName());
		...
		{
			UE_LOGF(LogAnimationCompression, Log, "    uses bone codec %ls", *CompressedData.BoneCompressionCodec->GetPathName());
		}
		else
		{
			UE_LOGF(LogAnimationCompression, Log, "    uses bone codec %ls (%ls)", *CompressedData.BoneCompressionCodec->GetPathName(), *CompressedData.BoneCompressionCodec->Description);
		}

		const SIZE_T BoneDataSize = GetCompressedBoneSize(CompressedData);
		UE_LOGF(LogAnimationCompression, Log, "    has %.2f KB of bone data", BytesToKB(BoneDataSize));

#if WITH_EDITORONLY_DATA
		if (CompressedData.CompressedDataStructure)
		{
			UE_LOGF(LogAnimationCompression, Log, "    has a bone error of %.4f cm", CompressedData.CompressedDataStructure->BoneCompressionErrorStats.MaxError);
		}
#endif

		UE_LOGF(LogAnimationCompression, Log, "    uses curve codec %ls", *CompressedData.CurveCompressionCodec->GetPathName());

		const SIZE_T CurveDataSize = GetCompressedCurveSize(CompressedData);
		UE_LOGF(LogAnimationCompression, Log, "    has %.2f KB of curve data", BytesToKB(CurveDataSize));

		BoneDataTotalSize += BoneDataSize;
		CurveDataTotalSize += CurveDataSize;
	}

	UE_LOGF(LogAnimationCompression, Log, "Total bone data size: %.2f MB", BytesToMB(BoneDataTotalSize));
	UE_LOGF(LogAnimationCompression, Log, "Total curve data size: %.2f MB", BytesToMB(CurveDataTotalSize));
...
void FACLPlugin::SetDatabaseVisualFidelity(const TArray<FString>& Args)
{
	...
	{
		UE_LOGF(LogAnimationCompression, Warning, "Invalid visual fidelity: %ls", *Args[0]);
	}
...
#if WITH_EDITORONLY_DATA
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FACLPlugin::OnPostEngineInit);
#endif
}
