...
void FACLCompressedAnimDataBase::SerializeCompressedData(UObject* DataOwner, FArchive& Ar)
{
	...
	if (bCompressionFailed && (Ar.IsLoading() || Ar.IsCooking()))
	{
		...
		UE_LOGF(LogAnimationCompression, Error,
			"ACL failed to compress an anim sequence and will output the bind pose at runtime: %ls",
...
bool UAnimBoneCompressionCodec_ACLBase::Compress(const FCompressibleAnimData& CompressibleAnimData, FCompressibleAnimDataResult& OutResult)
{
	...
	UE_LOGF(LogAnimationCompression, Verbose, "ACL Animation raw size: %u bytes [%ls]", ACLTracks.get_raw_size(), *CompressibleAnimData.FullName);
	...
	if (!CompressionResult.empty())
	{
		...
		UE_LOGF(LogAnimationCompression, Error, "ACL failed to compress anim sequence: %ls [%ls]", ANSI_TO_TCHAR(CompressionResult.c_str()), *CompressibleAnimData.FullName);
	...
	if (bEnableErrorReporting && LogAnimationCompression.GetVerbosity() >= ELogVerbosity::Verbose)
	{
		...
		UE_LOGF(LogAnimationCompression, Verbose, "ACL Animation compressed size: %u bytes [%ls]", CompressedClipDataSize, *CompressibleAnimData.FullName);
		UE_LOGF(LogAnimationCompression, Verbose, "ACL Animation error: %.4f cm (bone %u @ %.3f) [%ls]", TrackError.error, TrackError.index, TrackError.sample_time, *CompressibleAnimData.FullName);
	}
#endif
