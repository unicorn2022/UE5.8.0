...
bool UAnimCurveCompressionCodec_ACL::Compress(const FCompressibleAnimData& AnimSeq, FAnimCurveCompressionResult& OutResult)
{
	...
	if (CompressionResult.any())
	{
		...
		UE_LOGF(LogAnimationCompression, Error, "ACL failed to compress curves: %ls [%ls]", ANSI_TO_TCHAR(CompressionResult.c_str()), *AnimSeq.FullName);
	...
	if (bEnableErrorReporting && LogAnimationCompression.GetVerbosity() >= ELogVerbosity::Verbose)
	{
		...
		UE_LOGF(LogAnimationCompression, Verbose, "ACL Curves compressed size: %u bytes [%ls]", CompressedDataSize, *AnimSeq.FullName);
		UE_LOGF(LogAnimationCompression, Verbose, "ACL Curves error: %.4f (curve %u @ %.3f) [%ls]", Error.error, Error.index, Error.sample_time, *AnimSeq.FullName);
	}
#endif
...
bool UAnimCurveCompressionCodec_ACL::ValidateCompressedData(UObject* DataOwner, const FCompressedAnimSequence& AnimSeq) const
{
	...
	if (CompressedTracks == nullptr || CompressedTracks->is_valid(false).any())
	{
		UE_LOGF(LogAnimationCompression, Error,
			"ACL compressed curve data is missing or corrupted for an anim sequence: %ls",
	...
	{
		UE_LOGF(LogAnimationCompression, Error,
			"ACL failed to compress curves for an anim sequence and will output the default curve values at runtime: %ls",
			DataOwner != nullptr ? *DataOwner->GetPathName() : TEXT("[Unknown Sequence]"));
		return false;
	}
