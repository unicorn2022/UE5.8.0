...
public:
	...
	virtual void stream_in(uint32_t Offset, uint32_t Size, bool CanAllocateBulkData, acl::quality_tier Tier, acl::streaming_request_id RequestID) override
	{
		...
		{
			UE_LOGF(LogAnimationCompression, Log, "ACL completed the stream in request!");
		...
		UE_LOGF(LogAnimationCompression, Log, "ACL starting a new stream in request!");

		// Allocate our bulk data buffer on the first stream in request
		if (CanAllocateBulkData)
		{
			UE_LOGF(LogAnimationCompression, Log, "ACL is allocating the database bulk data!");
		...
		{
			UE_LOGF(LogAnimationCompression, Warning, "ACL failed to initiate database stream in request!");
			cancel(RequestID);
		}
	}
	...
	virtual void stream_out(uint32_t Offset, uint32_t Size, bool CanDeallocateBulkData, acl::quality_tier Tier, acl::streaming_request_id RequestID) override
	{
		...
		UE_LOGF(LogAnimationCompression, Log, "ACL is streaming out a database!");

		// Free our bulk data on the last stream out request
		if (CanDeallocateBulkData)
		{
			// TODO: Make this optional?
			UE_LOGF(LogAnimationCompression, Log, "ACL is deallocating the database bulk data!");
