...
struct CompressAnimationsFunctor
{
	...
	void DoIt(UCommandlet* Commandlet, UPackage* Package, const TArray<FString>& Tokens, const TArray<FString>& Switches)
	{
		...
		for (int32 SequenceIndex = 0; SequenceIndex < NumAnimSequences; ++SequenceIndex)
		{
			...
			if (StatsCommandlet->PerformCompression)
			{
				UE_LOGF(LogAnimationCompression, Verbose, "Compressing: %ls (%d / %d)", *UEClip->GetPathName(), SequenceIndex, NumAnimSequences);
			...
			{
				UE_LOGF(LogAnimationCompression, Verbose, "Extracting: %ls (%d / %d)", *UEClip->GetPathName(), SequenceIndex, NumAnimSequences);
				...
				if (Error.any())
				{
					UE_LOGF(LogAnimationCompression, Warning, "Failed to write ACL clip file: %ls", ANSI_TO_TCHAR(Error.c_str()));
				}
			}
...
int32 UACLStatsDumpCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOGF(LogAnimationCompression, Error, "Missing commandlet argument: -output=<path/to/output/directory>");
		return 0;
	}
	...
	{
		UE_LOGF(LogAnimationCompression, Error, "Cannot compress and extract clips at the same time");
		return 0;
	}

	if (!PerformCompression && !PerformClipExtraction)
	{
		UE_LOGF(LogAnimationCompression, Error, "Must compress or extract clips");
		return 0;
	}

	if (PerformClipExtraction && ParamsMap.Contains(TEXT("input")))
	{
		UE_LOGF(LogAnimationCompression, Error, "Cannot use an input directory when extracting clips");
		return 0;
	}
	...
	else
	{
		...
		for (const FString& Filename : Files)
		{
			...
			UE_LOGF(LogAnimationCompression, Verbose, "Compressing: %ls", *Filename);
