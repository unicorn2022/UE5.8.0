...
FRigUnit_SetSplinePoints_Execute()
{
	...
	{
		UE_LOGF(LogControlRig, Error, "Invalid input spline.");
		return;
	}
	
	if (Spline.SplineData->Spline == nullptr)
	{
		UE_LOGF(LogControlRig, Error, "Invalid input spline implementation.");
		return;
	}
	...
	if (!Spline.SplineData->bClosed)
	{
		...
		{
			UE_LOGF(LogControlRig, Error, "Number of input points does not match the number of point in the spline.");
			return;
		}
	}
