...
public:
	FLiveLinkHubCrashRecovery()
	{
		...
		{
			UE_LOGF(LogLiveLinkHub, Display, "Detected LiveLinkHub canary file.");
			ReadCanaryFile();
		}
		else
		{
...
private:
	...
	void CreateCanaryFile()
	{
		...
		{
			UE_LOGF(LogLiveLinkHub, Warning, "Could not read canary file (%ls) when attempting to recover from a crash.", *CanaryFilePath);
		}
	}
	...
	void PromptRecoverLastSave(uint32 ProcessId)
	{
		...
		if (bRecoverLastSave)
		{
			...
			UE_LOGF(LogLiveLinkHub, Display, "Recovering last autosave.");
		}
	}
	...
	void RestoreLastAutosave(uint32 CompatibleProcessId)
	{
		...
		for (const FLiveLinkHubSessionFile& File : SaveFiles)
		{
			if (TStrongObjectPtr<ULiveLinkHubSessionData> SessionDataPtr = FLiveLinkHub::Get()->GetSessionManager()->ReadSessionFile(File.FilePath))
			{
				...
				{
					UE_LOGF(LogLiveLinkHub, Display, "Restoring session from save %ls", *File.FileName);
		...
		UE_LOGF(LogLiveLinkHub, Display, "Could not find a valid autosave file to recover from crash.");
	}
