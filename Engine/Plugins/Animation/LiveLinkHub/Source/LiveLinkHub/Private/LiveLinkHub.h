...
class FLiveLinkHubMediaProfileHandler;
...
public:
	...
	/** 
	 * Upgrade a recording and save to a new file or over the existing file.
	 */
	void UpgradeAndSaveRecording(ULiveLinkUAssetRecording* InRecording, bool bUpgradeInPlace = false);
...
private:
	...
	void OnPlaybackReadyForUpgrade(bool bUpgradeInPlace);
...
private:
	...
	/** Handle for media profile changed delegate. */
	FDelegateHandle MediaProfileChangedHandle;
	/** Handle for object property changed delegate (to detect edits within the active MediaProfile). */
	FDelegateHandle ObjectPropertyChangedHandle;
	/** Handle for engine custom time step changed delegate (used to keep the Hub off the MediaProfile's genlock). */
	FDelegateHandle CustomTimeStepChangedHandle;

	/** Manages the transient MediaProfile and its session serialization. */
	TPimplPtr<FLiveLinkHubMediaProfileHandler> MediaProfileHandler;

	/** Called when the active MediaProfile changes. Re-broadcasts timecode/genlock settings to connected clients. */
	void OnMediaProfileChanged(UMediaProfile* PreviousProfile, UMediaProfile* NewProfile);

	/** Called when any UObject property changes. Filters for the active MediaProfile and its timecode/genlock subobjects. */
	void OnObjectPropertyChanged(UObject* Object, FPropertyChangedEvent& PropertyChangedEvent);

	/**
	 * Called when GEngine's custom time step changes. Hub shouldn't ever run with a custom timestep,
	 * so when the MediaProfile installs its custom time step we synchronously clear it.
	 */
	void OnEngineCustomTimeStepChanged();
