...
FSlateColor SLiveLinkTimecode::GetIconColor() const
{
	...
	const bool bCustomTimeStepEnabled = TimeAndSyncSettings->bUseLiveLinkHubAsCustomTimeStepSource;

	TSharedPtr<FSlateStyleSet> Style = ILiveLinkModule::Get().GetStyle();
	if (bCustomTimeStepEnabled)
	{
		const UMediaProfile* Profile = IMediaProfileManager::Get().GetCurrentMediaProfile();
		const bool bHasCustomTimeStep = Profile && Profile->GetCustomTimeStep() != nullptr;
		return bHasCustomTimeStep ? Style->GetSlateColor("LiveLink.Color.Valid") : Style->GetSlateColor("LiveLink.Color.Error");
	}
...
{
	const ULiveLinkHubTimeAndSyncSettings* TimeAndSyncSettings = GetDefault<ULiveLinkHubTimeAndSyncSettings>();
	const bool bTimecodeSourceEnabled = TimeAndSyncSettings->bUseLiveLinkHubAsTimecodeSource;
	...
	{
		const UMediaProfile* Profile = IMediaProfileManager::Get().GetCurrentMediaProfile();
		const bool bHasTimecodeProvider = Profile && Profile->GetTimecodeProvider() != nullptr;
		return bHasTimecodeProvider ? Style->GetSlateColor("LiveLink.Color.Valid") : Style->GetSlateColor("LiveLink.Color.Error");
	}
