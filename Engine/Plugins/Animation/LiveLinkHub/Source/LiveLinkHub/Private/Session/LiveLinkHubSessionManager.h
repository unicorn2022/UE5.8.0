...
class FLiveLinkHubSessionManager : public ILiveLinkHubSessionManager
	, public ILiveLinkRecordingSession
{
public:
	FLiveLinkHubSessionManager()
	{
		IModularFeatures::Get().RegisterModularFeature(ILiveLinkRecordingSession::GetModularFeatureName(), this);
	...
	{
		IModularFeatures::Get().UnregisterModularFeature(ILiveLinkRecordingSession::GetModularFeatureName(), this);
	}
	...
	virtual void SaveCurrentSession(FString SavePath = TEXT("")) override
	{
		...
		// Save recording mount point state
		if (FLiveLinkHubRecordingMountManager* MountManager = LiveLinkHubModule.GetRecordingMountManager())
		{
			LiveLinkHubSessionData->bUseCustomRecordingDirectory = MountManager->IsCustomDirectoryMounted();
			if (MountManager->IsCustomDirectoryMounted())
			{
				LiveLinkHubSessionData->CustomRecordingDirectory = MountManager->GetMountedDirectory();
				LiveLinkHubSessionData->CustomRecordingMountPoint = MountManager->GetMountPoint();
			}
			else
			{
				LiveLinkHubSessionData->CustomRecordingDirectory.Empty();
				LiveLinkHubSessionData->CustomRecordingMountPoint.Empty();
			}
		}
	...
	virtual void RestoreSession(ULiveLinkHubSessionData* SessionData)
	{
		...
		if (FLiveLinkHubRecordingMountManager* MountManager = HubModule.GetRecordingMountManager())
		{
			if (SessionData->bUseCustomRecordingDirectory && !SessionData->CustomRecordingDirectory.IsEmpty())
			{
				...
				FText ErrorText;
				if (!MountManager->MountCustomDirectory(SessionData->CustomRecordingDirectory, OutMountPoint, ErrorText))
				{
					UE_LOGF(LogLiveLinkHub, Warning, "Failed to restore custom recording directory '%ls': %ls",
						*SessionData->CustomRecordingDirectory, *ErrorText.ToString());
					// Clear the flag so we fall back to default directory
					SessionData->bUseCustomRecordingDirectory = false;
				}
			}
			else
			{
				// Ensure any existing mount point is cleaned up
				FText ErrorText;
				MountManager->UnmountCustomDirectory(ErrorText);
			}
		}
...
private:
	...
	{
		// Unmount any custom recording directory
		const FLiveLinkHubModule& HubModule = FModuleManager::Get().GetModuleChecked<FLiveLinkHubModule>("LiveLinkHub");
		if (FLiveLinkHubRecordingMountManager* MountManager = HubModule.GetRecordingMountManager())
		{
			FText ErrorText;
		...
		{
			UE_LOGF(LogLiveLinkHub, Warning, "Could not remove all existing sources in time. Sources may still be getting cleaned up.");
		}
