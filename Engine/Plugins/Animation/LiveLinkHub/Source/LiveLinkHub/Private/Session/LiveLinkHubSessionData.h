...
public:
	...
	/** Whether to use a custom recording directory instead of the default project content folder */
	UPROPERTY()
	bool bUseCustomRecordingDirectory = false;

	/** Custom recording directory absolute path (only used if bUseCustomRecordingDirectory is true) */
	UPROPERTY()
	FString CustomRecordingDirectory;

	/** Mount point for custom recording directory (e.g., /Game/LiveLinkRecordings_eeaf31) */
	UPROPERTY()
	FString CustomRecordingMountPoint;
