...
namespace UE::LiveLinkHub::Compression
{
	static const FName CompressionAlgo = NAME_Oodle;

	/** Compress data with a bias for memory. */
	void Compress(TArray<uint8>& InBytes)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(ULiveLinkUAssetRecording::Compress);

		TArray<uint8> OutCompressedData;
		ECompressionFlags CompressFlags = COMPRESS_BiasMemory;
		int32 CompressedSize = FCompression::CompressMemoryBound(CompressionAlgo, InBytes.Num(), CompressFlags);
		OutCompressedData.SetNumUninitialized(CompressedSize);

		if (FCompression::CompressMemory(CompressionAlgo, OutCompressedData.GetData(), CompressedSize, InBytes.GetData(), InBytes.Num(), CompressFlags))
		{
			OutCompressedData.SetNum(CompressedSize, EAllowShrinking::No);
			InBytes = MoveTemp(OutCompressedData);
		}
		else
		{
			ensureAlways(false);
	...
	/** Decompress data. */
	TArray<uint8> Decompress(FName InCompressionMethod, TArray<uint8>& InBytes, int32 UncompressedSize)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(ULiveLinkUAssetRecording::Decompress);

		TArray<uint8> UncompressedData;
		UncompressedData.SetNumUninitialized(UncompressedSize);

		ensure(FCompression::UncompressMemory(InCompressionMethod, UncompressedData.GetData(), UncompressedData.Num(), InBytes.GetData(), InBytes.Num(), COMPRESS_BiasMemory));
...
void ULiveLinkUAssetRecording::LoadEntireRecording()
{
	auto LoadEntireRecordingLambda = [this]()
	{
		...
		if (SavedMaxFrames <= 0)
		{
			UE_LOGF(LogLiveLinkHub, Warning, "Could not load the recording because the max frame size is 0.");
			return;
		}
...
void ULiveLinkUAssetRecording::LoadRecordingData(int32 InInitialFrame, int32 InNumFramesToLoad)
{
	...
	if (InitialFrameToStream != InInitialFrame)
	{
		...
		UE_LOGF(LogLiveLinkHub, Verbose, "Stream initial frame changed from: %d to %d ", InInitialFrame, InitialFrameToStream);
	}
...
void ULiveLinkUAssetRecording::UnloadRecordingData()
{
	...
	{
		UE_LOGF(LogLiveLinkHub, Warning, "Attempted to unload %ls while the package was still being saved", *GetName());
		return;
	}
...
void ULiveLinkUAssetRecording::SaveFrameData(FArchive* InFileWriter, const FLiveLinkSubjectKey& InSubjectKey, FLiveLinkRecordingBaseDataContainer& InBaseDataContainer)
{
	...
	*InFileWriter << NumFrames;
	...
	if (NumFrames == 0)
	{
		UE_LOGF(LogLiveLinkHub, Error, "No frames recorded.");
		return;
	}
	...
	*InFileWriter << InBaseDataContainer.Timestamps;
	...
	FString CompressionMethod = UE::LiveLinkHub::Compression::CompressionAlgo.ToString();
	*InFileWriter << CompressionMethod;
	...
	// Memory used for the intermediate step of serializing data before compressing it.
	TArray<uint8> Backing;
	...
	for (int32 FrameIdx = 0; FrameIdx < NumFrames; ++FrameIdx)
	{
		...
		*InFileWriter << FrameIdx;
		...
		// Write the entire frame data to memory
		FMemoryWriter MemoryWriter{ Backing, true };
		FObjectAndNameAsStringProxyArchive StructAr(MemoryWriter, false);

		Frame->Serialize(StructAr);

		int32 UncompressedSize = Backing.Num();
		*InFileWriter << UncompressedSize;

		// Then compress it and write it to the file. 
		// Note: We compress every frame independently to make it easier for the loading code to stream the data.
		UE::LiveLinkHub::Compression::Compress(Backing);
		InFileWriter->Serialize(Backing.GetData(), Backing.Num());
...
void ULiveLinkUAssetRecording::LoadRecordingAsync(int32 InStartFrame, int32 InCurrentFrame, int32 InNumFramesToLoad)
{
	...
	UE_LOGF(LogLiveLinkHub, Verbose, "Loading recording StartFrame: %d, CurrentFrame: %d, EndFrame: %d ", InStartFrame, InCurrentFrame, EndFrame);
	...
	if (bInitialLoad)
	{
		...
		{
			UE_LOGF(LogLiveLinkHub, Log, "Converting version %d to %d", LoadedRecordingVersion, CurrentRecordingVersion);
		}
	...
	while (true)
	{
		...
		for (TTuple<FLiveLinkSubjectKey, FrameData::Private::FFrameMetaData>& FrameDataKeyVal : FrameFileData)
		{
			...
			if (FrameData.BufferIterationData.Status <= FrameData::Private::FFrameBufferIterationData::Active)
			{
				...
				{
					UE_LOGF(LogLiveLinkHub, Error, "FrameDataSubjectKey is missing for recording %ls.", *GetName());
					CompletedTasks++;
				}
			}
...
bool ULiveLinkUAssetRecording::LoadInitialFrameData(UE::LiveLinkHub::FrameData::Private::FFrameMetaData& OutFrameData)
{
...

	{
		...
		{	
			...
			{
				UE_LOGF(LogLiveLinkHub, Error, "Script struct type '%ls' not found.", *StructTypeName);
				return false;
			}
		...
		if (RecordingVersionBeingLoaded >= UE::LiveLinkHub::Private::RecordingVersions::CompressionSupport)
		{
			FString CompressionMethodName;
			Reader->GetMemoryReader() << CompressionMethodName;

			OutFrameData.CompressionMethodName = *CompressionMethodName;
		}
	}
                                             ...
                                             int32 RequestedStartFrame, int32 RequestedInitialFrame, int32 RequestedFramesToLoad, bool bForceSequential)
{
	...
	if (MaxFrames > 0)
	{
		...
		TRACE_CPUPROFILER_EVENT_SCOPE(ULiveLinkUAssetRecording::LoadFrames)
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(ULiveLinkUAssetRecording::LoadFrameFromDisk);
	...
	{
		UE_LOGF(LogLiveLinkHub, Error, "Frame index mismatch: expected %d, got %d", InFrame, ParsedFrameIdx);
		return false;
	}
	...
	{
		UE_LOGF(LogLiveLinkHub, Error, "Script struct type not found.");
		return false;
	}

	{
		...
		{
			int32 FrameSize = InFrameData.FrameDiskSizes[InFrame].Get<1>();
			int32 UncompressedSize = 0;

			Reader->GetMemoryReader() << UncompressedSize;

			const int32 HeaderSize = sizeof(FrameSize) + sizeof(UncompressedSize);

			// We've already read the header, so substract that amount from the frame size we're going to read next.
			FrameSize -= HeaderSize;
			check(FrameSize > 0);

			TArray<uint8> CompressedData;
			CompressedData.AddZeroed(FrameSize);

			{
				TRACE_CPUPROFILER_EVENT_SCOPE(ULiveLinkUAssetRecording::ReadCompressedData);
				Reader->GetMemoryReader().Serialize(CompressedData.GetData(), FrameSize);
			}

			TArray<uint8> UncompressedData = UE::LiveLinkHub::Compression::Decompress(InFrameData.CompressionMethodName, CompressedData, UncompressedSize);

			FMemoryReader UncompressedReader{ UncompressedData };

			FObjectAndNameAsStringProxyArchive StructAr(UncompressedReader, true);
			OutFrame = MakeShared<FInstancedStruct>(LoadedStruct);

			TRACE_CPUPROFILER_EVENT_SCOPE(ULiveLinkUAssetRecording::DeserializeFrame);
			OutFrame->Serialize(StructAr);
		}
	}
