...
namespace UE::AssetRecorderUtils::Private
{
	/** Get the current recording mount point (custom or default /Game) */
	FString GetRecordingMountPoint()
	{
		FString RecordingMountPoint = TEXT("/Game");
		...
		if (TSharedPtr<ILiveLinkHubSessionManager> SessionManager = HubModule.GetSessionManager())
		{
			...
			{
				if (ULiveLinkHubSessionData* SessionData = Session->GetSessionData())
				{
					if (SessionData->bUseCustomRecordingDirectory && !SessionData->CustomRecordingMountPoint.IsEmpty())
					{
						RecordingMountPoint = SessionData->CustomRecordingMountPoint;
					}
				}
			}
		}
		return RecordingMountPoint;
	}
...
bool FLiveLinkUAssetRecorder::CanRecord(FText* OutErrorMessage) const
{
	...
	const ILiveLinkRecordingSession& SessionInfo = ILiveLinkRecordingSession::Get();
...
void FLiveLinkUAssetRecorder::SaveFromExistingRecording(ULiveLinkRecording* InRecording, const FString& InFilePath, bool bUpgradeInPlace)
{
	ULiveLinkUAssetRecording* UAssetRecording = CastChecked<ULiveLinkUAssetRecording>(InRecording);

	if (bUpgradeInPlace)
	{
		SaveRecordingInPlace(UAssetRecording);
	}
	else
	{
		SaveRecording(MoveTemp(UAssetRecording->RecordingData), InRecording->LengthInSeconds, /*bIncrementTake*/ false,
...
{
	// Determine the recording mount point (custom or default /Game)
	const FString RecordingMountPoint = UE::AssetRecorderUtils::Private::GetRecordingMountPoint();
	...
	ensure(FPackageName::TryConvertLongPackageNameToFilename(RecordingMountPoint, ContentDir));
	const FString DialogStartPath = FPaths::Combine(RecordingMountPoint, DefaultFolder);
	...
	if (!PlatformFile.DirectoryExists(*AbsoluteFolderPath))
	{
		...
		{
			UE_LOGF(LogLiveLinkHub, Error, "Failed to create directory %ls.", *AbsoluteFolderPath);
			return false;
		}

		FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
		AssetRegistryModule.Get().ScanPathsSynchronous({ RecordingMountPoint }, true);
	}
	...
	{
		NewPackageName = RecordingMountPoint / UniqueAssetName;
	}
	...
	const FFrameRate* InFrameRate, const FString& InFilePath)
{
	...
	ULiveLinkUAssetRecording* Recording = nullptr;

	Package = CreatePackage(*PackageName);
	Recording = NewObject<ULiveLinkUAssetRecording>(Package, *NewAssetName, RF_Public | RF_Standalone);

	if (Recording)
	{
		Recording->InitializeNewRecordingData(MoveTemp(InRecordingData), InRecordingLength);
		...
		if (InPreset)
		{
			...
			UEngine::CopyPropertiesForUnrelatedObjects(InPreset, Recording->RecordingPreset);
		}

		if (InFrameRate)
		{
			Recording->FrameRate = *InFrameRate;
		}

		Recording->MarkPackageDirty();
		FAssetRegistryModule::AssetCreated(Recording);

		const TStrongObjectPtr<ULiveLinkUAssetRecording> NewRecordingStrongPtr(Recording);
		...
		FAsyncTask<FLiveLinkSaveRecordingAsyncTask>& AsyncTask = *AsyncSaveTasks.Add(NewRecordingStrongPtr,
			MakeUnique<FAsyncTask<FLiveLinkSaveRecordingAsyncTask>>(Recording, this));
...
void FLiveLinkUAssetRecorder::SaveRecordingInPlace(ULiveLinkUAssetRecording* InRecording)
{
	if (!InRecording)
	{
		return;
	...
	InRecording->SavedRecordingVersion = UE::LiveLinkHub::Private::RecordingVersions::Latest;
	InRecording->MarkPackageDirty();

	const TStrongObjectPtr<ULiveLinkUAssetRecording> NewRecordingStrongPtr(InRecording);

	// Create a task to run on a separate thread for saving all frame data and writing the final UAsset to disk.
	// We use a container rather than just one task on the chance a save operation is still running when another recording is being saved.
	FAsyncTask<FLiveLinkSaveRecordingAsyncTask>& AsyncTask = *AsyncSaveTasks.Add(NewRecordingStrongPtr,
		MakeUnique<FAsyncTask<FLiveLinkSaveRecordingAsyncTask>>(InRecording, this));
	AsyncTask.StartBackgroundTask();
}
...
void FLiveLinkUAssetRecorder::RecordInitialStaticData()
{
	...
	{
		TOptional<FLiveLinkRecordingStaticDataContainer> StaticDataContainer = UE::AssetRecorderUtils::Private::CreateStaticDataContainerFromFrameData(Subject);
...
void FLiveLinkUAssetRecorder::OnRecordingDataSaved_GameThread(FLiveLinkSaveRecordingAsyncTask* InTask)
{
	...
	if (ensure(Recording.IsValid()))
	{
		...
		{
			UE_LOGF(LogLiveLinkHub, Error, "Package '%ls' was not saved", *PackageFileName);
		}
	}
...
void FLiveLinkUAssetRecorder::OnRecordingSaveThreadFinished_GameThread(FLiveLinkSaveRecordingAsyncTask* InTask)
{
	...
	if (ensure(Recording.IsValid()))
	{
		...
		{
			UE_LOGF(LogLiveLinkHub, Error, "Could not find save task for recording: '%ls'", *Recording->GetName());
		}
...
void FLiveLinkUAssetRecorder::FLiveLinkSaveRecordingAsyncTask::DoWork()
{
	...
	{
		UE_LOGF(LogLiveLinkHub, Error, "Timed out waiting for package save.");
	}
