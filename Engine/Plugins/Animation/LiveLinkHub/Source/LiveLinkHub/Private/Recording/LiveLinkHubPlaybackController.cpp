...
namespace UE::LiveLinkHub 
{
	...
	{
		UE::LiveLinkHubTimingUtils::Private::PlaybackEvaluationOverride = ELiveLinkTimecodeProviderEvaluationType::Latest;
		GetMutableDefault<ULiveLinkHubTimeAndSyncSettings>()->OnToggleTimecodeSettings();
	}

	static void ResetTimecodeEvaluationType()
	{
		UE::LiveLinkHubTimingUtils::Private::PlaybackEvaluationOverride.Reset();
		GetMutableDefault<ULiveLinkHubTimeAndSyncSettings>()->OnToggleTimecodeSettings();
	}
}
...
void FLiveLinkHubPlaybackController::PreparePlayback(ULiveLinkRecording* InLiveLinkRecording)
{
	...
	{
		UE_LOGF(LogLiveLinkHub, Warning, "Started a recording playback with an invalid recording.");
	}
		...
		auto PreparePlaybackCallback = [this, RecordingStrongPtr]()
		{
			if (RecordingStrongPtr.IsValid())
			{
				...
				if (!RecordingPlayer->PreparePlayback(RecordingToPlay.Get()))
				{
					...
					UE_LOGF(LogLiveLinkHub, Error, "Playback failed for '%ls', the file may be corrupted or unsupported.", *GetRecordingName());
					return;
				}
				...
				if (RecordingLength.AsSeconds() <= 0.0)
				{
					...
					UE_LOGF(LogLiveLinkHub, Warning, "Recording '%ls' is empty.", *GetRecordingName());
					return;
				}
...
bool FLiveLinkHubPlaybackController::SyncToFrame(const FQualifiedFrameTime& InFrameTime)
{
	...
	if (NextFrames.Num() == 0)
	{
		UE_LOGF(LogLiveLinkHub, Warning, "No frame loaded for frame number %d", InFrameTime.Time.GetFrame().Value);
	}
	for (const FLiveLinkRecordedFrame& NextFrame : NextFrames)
	{
