...
private:
	...
	void OnImportRecording(const FAssetData& AssetData)
	{
		if (const TSharedPtr<FLiveLinkHub> HubPtr = LiveLinkHub.Pin())
		{
			...
			{
				UE_LOGF(LogLiveLinkHub, Warning, "Failed to import recording %ls", *AssetData.AssetName.ToString());
				return;
			}
			...
			if (UE::IsSavingPackage(nullptr) && !ImportedRecording->IsFullyLoaded())
			{
				...
				UE_LOGF(LogLiveLinkHub, Warning, "Can't start recording because a package is saving");
				return;
			}
