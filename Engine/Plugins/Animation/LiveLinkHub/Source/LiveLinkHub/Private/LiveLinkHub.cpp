...
void FLiveLinkHub::Initialize(FLiveLinkHubTicker& Ticker)
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FLiveLinkHub::InitializePostEngineInit);
...
void FLiveLinkHub::InitializePostEngineInit()
{
	...
	// Create the handler that manages the transient MediaProfile and its session serialization.
	MediaProfileHandler = MakePimpl<FLiveLinkHubMediaProfileHandler>();

	// Timecode and genlock are now applied via the active MediaProfile.
	// Observe MediaProfile changes to re-broadcast settings to connected clients.
	MediaProfileChangedHandle = IMediaProfileManager::Get().OnMediaProfileChanged().AddSP(this, &FLiveLinkHub::OnMediaProfileChanged);

	// Also observe property edits within the active MediaProfile (e.g. changing the timecode provider's subject name).
	ObjectPropertyChangedHandle = FCoreUObjectDelegates::OnObjectPropertyChanged.AddSP(this, &FLiveLinkHub::OnObjectPropertyChanged);

	// Live Link Hub application broadcasts the MediaProfile's custom time step to its clients but should never run a custom timestep itself. 
	// Hook into GEngine's OnCustomTimeStepChanged so we can synchronously undo any application of the MediaProfile's custom time step inside the 
	// same call stack as UMediaProfile::Apply() — covering OnMediaProfileChanged, OnMediaProxiesChanged-style re-applies, and any other path 
	// that goes through GEngine->SetCustomTimeStep().
	if (GEngine)
	{
		CustomTimeStepChangedHandle = GEngine->OnCustomTimeStepChanged().AddSP(this, &FLiveLinkHub::OnEngineCustomTimeStepChanged);

		// Defend against: if a startup MediaProfile was already applied before this delegate was hooked,
		// clear it now. Subsequent applications of custom timestep will be caught by the delegate.
		UE::LiveLinkHubTimingUtils::Private::ClearMediaProfileCustomTimeStepFromEngine();
	}
...
void FLiveLinkHub::OnEnginePreExit()
{
	...
	// Tearing down modes before engine tears down singleton objects
	if (CurrentAppModePtr.IsValid())
	{
		CurrentAppModePtr->PreDeactivateMode();
	}
	
	RemoveAllApplicationModes();
	CurrentAppModePtr.Reset();
}
...
FLiveLinkHub::~FLiveLinkHub()
{
	...
	if (CustomTimeStepChangedHandle.IsValid() && GEngine)
	{
		GEngine->OnCustomTimeStepChanged().Remove(CustomTimeStepChangedHandle);
	}

	// Destroy the media profile handler before other systems are torn down.
	MediaProfileHandler.Reset();
...
void FLiveLinkHub::OnMediaProfileChanged(UMediaProfile* PreviousProfile, UMediaProfile* NewProfile)
{
	// When the active MediaProfile changes, re-broadcast timecode/genlock settings to all connected clients.
	UE::LiveLinkHubTimingUtils::Private::BroadcastCurrentSettings();
}

void FLiveLinkHub::OnEngineCustomTimeStepChanged()
{
	// Synchronously undo the MediaProfile's application of its custom time step on GEngine.
	// This fires inside GEngine->SetCustomTimeStep() before any tick can observe the new value.
	UE::LiveLinkHubTimingUtils::Private::ClearMediaProfileCustomTimeStepFromEngine();
}

void FLiveLinkHub::OnObjectPropertyChanged(UObject* Object, FPropertyChangedEvent& PropertyChangedEvent)
{
	if (!Object || PropertyChangedEvent.ChangeType == EPropertyChangeType::Unspecified)
	{
		return;
	}

	const UMediaProfile* ActiveProfile = IMediaProfileManager::Get().GetCurrentMediaProfile();
	if (!ActiveProfile)
	{
		return;
	}

	// Check if the changed object is the active MediaProfile itself, or one of its timecode/genlock subobjects.
	const bool bIsActiveProfile = (Object == ActiveProfile);
	const bool bIsTimecodeProvider = (ActiveProfile->GetTimecodeProvider() && Object == ActiveProfile->GetTimecodeProvider());
	const bool bIsCustomTimeStep = (ActiveProfile->GetCustomTimeStep() && Object == ActiveProfile->GetCustomTimeStep());

	if (bIsActiveProfile || bIsTimecodeProvider || bIsCustomTimeStep)
	{
		UE::LiveLinkHubTimingUtils::Private::BroadcastCurrentSettings();
	}
}
...
void FLiveLinkHub::UpgradeAndSaveRecording(ULiveLinkUAssetRecording* InRecording, bool bUpgradeInPlace)
{
	GetPlaybackController()->OnPlaybackReady().AddSP(this, &FLiveLinkHub::OnPlaybackReadyForUpgrade, bUpgradeInPlace);
...
{
	UE_LOGF(LogLiveLinkHub, Verbose, "Removed subject %ls", *InSubjectKey.SubjectName.ToString());
...
void FLiveLinkHub::OnPlaybackReadyForUpgrade(bool bUpgradeInPlace)
{
	...
	FString RecordingPath = PlaybackController->GetRecording()->GetPackage()->GetPathName();
	if (!bUpgradeInPlace)
	{
	...
	CurrentRecording->GetOnRecordingFullyLoaded().BindLambda([this, CurrentRecordingWeakPtr, RecordingPath, bUpgradeInPlace]()
	{
		...
		{
			RecordingController->RecordFromExistingAndSave(CurrentRecordingPin.Get(), RecordingPath, bUpgradeInPlace);
		}
	});
