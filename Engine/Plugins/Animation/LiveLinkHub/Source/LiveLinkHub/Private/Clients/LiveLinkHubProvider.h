...
public:
	...
	/**
	 * Push the hub's preferred evaluation mode to connected Editor clients, overwriting their hub source's local Mode.
	 * @Note If ClientId is not provided or invalid, the message will be broadcast to all connected clients.
	 */
	void UpdateSourceEvaluationMode(ELiveLinkSourceMode Mode, const FLiveLinkHubClientId& ClientId = FLiveLinkHubClientId{});
...
private:
	...
	/** Send a source evaluation mode override to connected UE Clients. */
	void SendSourceEvaluationMode(ELiveLinkSourceMode Mode, const FLiveLinkHubClientId& ClientId);
