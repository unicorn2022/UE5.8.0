...
namespace LiveLinkHubConnectionManager
{
	...
	static bool CanConnectTo(const FString& MachineName, const T& ObjectWithAnnotations, const FLiveLinkHubInstanceId& InstanceId)
	{
		...
		// An explicit connect request (the user picked this client in the LLH "Add Client" menu) bypasses the auto-connect filter.
		const bool bExplicitConnect = Annotations.Contains(FLiveLinkHubMessageAnnotation::ExplicitConnectAnnotation);

		const bool bAutoConnectMatchResult = !bSameInstance &&
			(bExplicitConnect
				|| AutoConnectMode == ELiveLinkHubAutoConnectMode::All
				|| (AutoConnectMode == ELiveLinkHubAutoConnectMode::LocalOnly && bSameHost));
		if (!bAutoConnectMatchResult)
		{
			const FText AutoConnectModeName = UEnum::GetDisplayValueAsText(AutoConnectMode);
			UE_LOGF(LogLiveLinkHubConnectionManager, Verbose, "Refusing connection from incoming instance since it was in mode: %ls", *AutoConnectModeName.ToString());
		}
	...
	static bool ShouldAcceptConnectionFrom(ELiveLinkTopologyMode InHostMode, const FProviderPollResultPtr& InPollResult, const FLiveLinkHubInstanceId& InstanceId)
	{
		...
		{
			UE_LOGF(LogLiveLinkHubConnectionManager, Verbose, "Refusing connection from incoming instance in %ls mode. This app is in %ls mode.", *IncomingModeName.ToString(), *ModeName.ToString());
		}
...
public:
		...
		, DiscoveryRequestDelegate(OnDiscoveryRequest)
	{
		...
		FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FLiveLinkHubConnectionManager::StartDiscovery);
...
private:
	...
	void LookForLiveLinkHubConnection()
	{
		...
		UE_LOGF(LogLiveLinkHubConnectionManager, Verbose, "Polling discovery results.");
	...
	{
		UE_LOGF(LogLiveLinkHubConnectionManager, Verbose, "Discovered new source.");
		...
		if (ModularFeatures.IsModularFeatureAvailable(ILiveLinkClient::ModularFeatureName))
		{
			...
			for (const FGuid& SourceId : LiveLinkClient->GetSources())
			{
				if (LiveLinkClient->GetSourceType(SourceId).ToString() == PollResult->Name && LiveLinkClient->GetSourceMachineName(SourceId).ToString() == PollResult->MachineName)
				{
					...
					{
						UE_LOGF(LogLiveLinkHubConnectionManager, Verbose, "Rejecting poll result since source %ls already exists.", *PollResult->Name);
						return;
					}
				}
			}
		...
		{
			UE_LOGF(LogLiveLinkHubConnectionManager, Warning, "LiveLink modular feature was unavailable.");
		}
	}
