...
public:
	...
	void InitializeMessageEndpoint(FMessageEndpoint::Builder& Builder)
	{
		...
		Builder.Handling<FLiveLinkHubDiscoveryMessage>(GameThreadDispatch)
			...
			.Handling<FLiveLinkHubSourceEvaluationModeMessage>(GameThreadDispatch)
	...
	void OnSourceShutdown(const TSharedPtr<FLiveLinkHubMessageBusSource>& InSource)
	{
		...
		{
			UE_LOGF(LogLiveLinkHubMessaging, Verbose, "MessageBusSource(%ls) : Sending disconnect message to %ls", *InSource->GetAddress().ToString(), *InSource->GetConnectionAddress().ToString());
...
private:
	...
	/**
	 * Checks if an incoming LiveLinkHub discovery should replace existing sources.
	 * Returns the GUID of any existing source that should be removed, or invalid GUID if none.
	 * This method is only called after confirming the incoming connection is a LiveLinkHub provider.
	 */
	FGuid FindStaleLiveLinkHubSource(const FLiveLinkHubDiscoveryMessage& IncomingMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context) const
	{
		ILiveLinkClient* Client = IModularFeatures::Get().IsModularFeatureAvailable(ILiveLinkClient::ModularFeatureName)
		...
		// This method is only called for confirmed LiveLinkHub providers
		// Search existing sources for LiveLinkHub instances from the same machine
		TArray<FGuid> Sources = Client->GetSources();

		for (const FGuid& SourceId : Sources)
		{
			FText SourceType = Client->GetSourceType(SourceId);
			FText SourceMachineName = Client->GetSourceMachineName(SourceId);

			// Look for existing LiveLinkHub sources from the same machine
			// We check both the source type and machine name to identify potential duplicates
			bool bIsLiveLinkHubSource = SourceType.ToString().Contains(TEXT("Live Link Hub")) ||
			                           SourceType.ToString().Contains(UE::LiveLinkHub::Private::LiveLinkHubProviderType.ToString());

			if (bIsLiveLinkHubSource && SourceMachineName.ToString() == IncomingMessage.MachineName)
			{
				// Check if the source is still valid
				bool bIsStillValid = Client->IsSourceStillValid(SourceId);

				if (!bIsStillValid)
				{
					// Source is invalid (disconnected/timed out), replace it
					UE_LOGF(LogLiveLinkHubMessaging, Verbose, "Found invalid LiveLinkHub source %ls from %ls, will replace",
						*SourceId.ToString(), *IncomingMessage.MachineName);
					return SourceId;
				}
				else if (SourceType.ToString() == IncomingMessage.ProviderName)
				{
					// Same provider name from same machine - likely a reconnection
					UE_LOGF(LogLiveLinkHubMessaging, Verbose, "Found duplicate LiveLinkHub source %ls from %ls, will replace",
						*SourceId.ToString(), *IncomingMessage.MachineName);
					return SourceId;
				}
			}
		...
		return FGuid(); // No stale source found
	}
	...
	/** Handle a source evaluation mode message — apply the hub's preferred mode to the relevant hub source(s) on this client. */
	template <>
	void Handle(const FLiveLinkHubSourceEvaluationModeMessage& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
	{
		...
		auto ApplyTo = [&Client, &Message](const FGuid& SourceId)
		{
			if (ULiveLinkSourceSettings* Settings = Client.GetSourceSettings(SourceId))
			{
				Settings->Mode = Message.Mode;
			}
		};

		if (Message.SourceGuid.IsValid())
		{
			ApplyTo(Message.SourceGuid);
			return;
		}

		// No explicit target. In Source-mode channel we own a single hub source; otherwise apply to every
		// hub-typed source known to the client. "Hub wins" policy — we overwrite any local Mode.
		if (ChannelMode == EChannelMode::Source)
		{
			if (CachedSourceId.IsValid())
			{
				ApplyTo(CachedSourceId);
			}
			return;
		}

		const FString LiveLinkHubProviderTypeString = UE::LiveLinkHub::Private::LiveLinkHubProviderType.ToString();
		for (const FGuid& SourceId : Client.GetSources())
		{
			const FString SourceTypeString = Client.GetSourceType(SourceId).ToString();
			if (SourceTypeString.Contains(TEXT("Live Link Hub")) || SourceTypeString.Contains(LiveLinkHubProviderTypeString))
			{
				ApplyTo(SourceId);
			}
		}
	...
	void Handle(const FLiveLinkHubDiscoveryMessage& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
	{
		...
		{
			// Check if the existing source is still valid/connected before rejecting the discovery request
			// This allows reconnection after timeout or when debugging causes the connection to become unresponsive
			...
			if (Client)
			{
				// Check all sources to see if any are still valid for this address
				TArray<FGuid> Sources = Client->GetSources();
				for (const FGuid& SourceId : Sources)
				{
					if (Client->GetSourceType(SourceId).ToString() == Message.ProviderName)
					{
						// Found a matching source - check if it's still valid
						if (Client->IsSourceStillValid(SourceId))
						{
							bSourceStillValid = true;
							break;
						}
					}
				}
			...
			{
				UE_LOGF(LogLiveLinkHubMessaging, Display, "Ignoring discovery request from %ls because we're already tracking an active connection.", *Context->GetSender().ToString());
				return;
			}
			else
			{
				// Source is no longer valid (timed out, disconnected, etc.) - remove it and allow reconnection
				UE_LOGF(LogLiveLinkHubMessaging, Display, "Removing invalid tracked provider %ls to allow reconnection.", *Context->GetSender().ToString());
				TrackedProviders.Remove(Context->GetSender());
			}
		}
		...
		if (bShouldConnect)
		{
			// Only check for stale sources if this is actually a LiveLinkHub instance, not a regular UE client

			// Check the provider type annotation to determine if this is a Hub.

			const TMap<FName, FString>& Annotations = Context->GetAnnotations();
			const FString* ProviderTypeAnnotation = Annotations.Find(FLiveLinkHubMessageAnnotation::ProviderTypeAnnotation);

			bool bIsLiveLinkHubProvider = false;

			if (ProviderTypeAnnotation && *ProviderTypeAnnotation == UE::LiveLinkHub::Private::LiveLinkHubProviderType.ToString())
			{
				bIsLiveLinkHubProvider = true;
			}
			else if (Message.Mode == ELiveLinkTopologyMode::Hub && Message.ProviderName.Contains(TEXT("Live Link Hub")))
			{
				// No annotation, but Mode is Hub and name suggests it's LiveLinkHub
				// This handles older LLH versions or cases where annotation is missing.
				bIsLiveLinkHubProvider = true;
			}

			if (bIsLiveLinkHubProvider)
			{
				// Check for stale sources that should be replaced
				FGuid StaleSourceId = FindStaleLiveLinkHubSource(Message, Context);
				if (StaleSourceId.IsValid())
				{
					UE_LOGF(LogLiveLinkHubMessaging, Display, "Removing stale LiveLinkHub source from %ls", *Message.MachineName);
					ILiveLinkClient& Client = IModularFeatures::Get().GetModularFeature<ILiveLinkClient>(ILiveLinkClient::ModularFeatureName);
					Client.RemoveSource(StaleSourceId);
				}
			}
			else
			{
				// This is a regular UE client so we always allow multiple connections
				UE_LOGF(LogLiveLinkHubMessaging, Verbose, "Adding UE client connection from %ls (no deduplication)", *Message.MachineName);
			}
