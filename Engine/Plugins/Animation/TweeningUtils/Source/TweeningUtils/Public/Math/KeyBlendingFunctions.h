 ...
 *	unless bAllowExtrapolation is true, in which case it must handle any X value.
 * @param bAllowExtrapolation If true, InEvaluate is called for ShiftedX values outside the blended range instead of clamping to the edge.
 *	Use this when the curve has cyclic extrapolation and InEvaluate already handles out-of-range X via the curve's infinity settings.
 *
 * @return The new Y value that the current key should have.
 */
