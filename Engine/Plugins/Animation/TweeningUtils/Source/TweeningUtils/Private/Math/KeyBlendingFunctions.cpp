...
static TAutoConsoleVariable<float> CVarEaseSlopeMultiplier(
	  TEXT("Tweening.Ease.SlopeMultiplier"),
	  6.0f,
	  TEXT("Controls the maximum slope of the Ease blend curve (at 100% slider). Higher = sharper peak curve.")
	  );

static TAutoConsoleVariable<float> CVarEaseSlopeExponent(
	TEXT("Tweening.Ease.SlopeExponent"),
	2.0f,
	TEXT("Controls how aggressively the Ease slope grows with slider position. "
		 "1.0 = linear, 2.0 = smoother behavior (gentler at low slider, sharper only at high slider).")
		 );
	...
	)
{
	...
	const float SlopeMultiplier = Private::BlendToEase::CVarEaseSlopeMultiplier.GetValueOnAnyThread();
	const float SlopeExponent   = Private::BlendToEase::CVarEaseSlopeExponent.GetValueOnAnyThread();
	const double AbsValue = FMath::Abs(InBlendValue);
	const double Slope = SlopeMultiplier * FMath::Pow(AbsValue, SlopeExponent);
	...
	)
{
	...
	// When bAllowExtrapolation is true (e.g. cyclic curves), delegate to InEvaluate which handles the curve's infinity settings correctly.
	if (ShiftedX >= InLastBlended.X)
	{
		return bAllowExtrapolation ? InEvaluate(ShiftedX) : BlendEdge(InLastBlended, InAfterBlendRange);
	}
	if (ShiftedX <= InFirstBlended.X)
	{
		return bAllowExtrapolation ? InEvaluate(ShiftedX) : BlendEdge(InFirstBlended, InBeforeBlendRange);
	}
