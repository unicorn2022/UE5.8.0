...
FTweenControllers::FTweenControllers(
	...
	, MoveSliderByHotkeyController(ToolbarController.MakeSelectedTweenModelAttr(), InCommandList)
{
...
TOptional<float> FTweenControllers::GetCurrentSliderPosition() const
{
	if (const TOptional<float> MouseOverride = MouseSlidingController.GetCurrentSliderPosition())
	{
		return MouseOverride;
	}
	
	if (const TOptional<float> CommandOverride = MoveSliderByHotkeyController.GetCurrentSliderPosition())
	{
