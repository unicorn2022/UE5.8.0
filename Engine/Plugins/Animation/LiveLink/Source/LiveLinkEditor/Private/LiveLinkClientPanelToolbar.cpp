...
void SLiveLinkClientPanelToolbar::Construct(const FArguments& Args, FLiveLinkClient* InClient)
{
	...
	[
		...
		[
			...
			[
				SNew(SButton)
				.ToolTipText(LOCTEXT("SavePreset_Tip", "Save the current Live Link preset"))
				.ForegroundColor(FSlateColor::UseForeground())
				.ButtonStyle(FAppStyle::Get(), "HoverHintOnly")
				.OnClicked(this, &SLiveLinkClientPanelToolbar::OnSavePreset)
				.IsEnabled_Lambda([this]() 
					{ 
						return !Client->GetSources().IsEmpty(); 
					})
				.Visibility(Args._ShowPresetPicker ? EVisibility::Visible : EVisibility::Collapsed)
				[
					SNew(STextBlock)
...
void SLiveLinkClientPanelToolbar::OnSaveAsPreset()
{
	...
	Client->SetLastSavedPreset(NewPreset);
}
...
void SLiveLinkClientPanelToolbar::OnImportPreset(const FAssetData& InPreset)
{
	...
	Client->SetLastSavedPreset(ImportedPreset);
}

FReply SLiveLinkClientPanelToolbar::OnSavePreset()
{
	ULiveLinkPreset* CurrentPreset = Client->GetLastSavedPreset();
	if (!CurrentPreset)
	{
		OnSaveAsPreset();
	...
	FEditorFileUtils::PromptForCheckoutAndSave({ CurrentPreset->GetPackage() }, false, false);
...
{
	return Client->GetLastSavedPreset() != nullptr;
}
