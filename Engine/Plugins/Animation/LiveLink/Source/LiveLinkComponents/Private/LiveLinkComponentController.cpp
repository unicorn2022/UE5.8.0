...
void ULiveLinkComponentController::InitializeController(ULiveLinkControllerBase* InController)
{
	...
	if (AActor* OuterActor = GetOwner())
	{
		...
		{
			UE_LOGF(LogLiveLinkComponents, Warning, "The desired component class for %ls is %ls, but %ls does not have a component of that type.", *InController->GetName(), *InController->GetDesiredComponentClass()->GetName(), *OuterActor->GetActorLabel(false));
		}
		...
		{
			UE_LOGF(LogLiveLinkComponents, Verbose, "%ls has no editable components of type %ls.", *OuterActor->GetActorLabel(false), *InController->GetDesiredComponentClass()->GetName());
		}
	}
#endif //WITH_EDITOR
}
