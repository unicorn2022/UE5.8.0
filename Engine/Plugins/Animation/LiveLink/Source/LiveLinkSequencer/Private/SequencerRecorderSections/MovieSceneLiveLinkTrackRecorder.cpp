...
static void AlwaysRecordLiveLinkTimecodeSinkFunction()
{
	...
	if (AlwaysRecordLiveLinkTimecode != CachedAlwaysRecordLiveLinkTimecode)
	{
		...
		UE_LOGF(LogLiveLinkSequencer, Warning, "Sequencer.AlwaysRecordLiveLinkTimecode is no longer in use, please set the Always Use Timecode value on the Live Link Source.");
	}
}
...
void UMovieSceneLiveLinkTrackRecorder::CreateTracks()
{
	...
	{
		UE_LOGF(LogLiveLinkSequencer, Warning, "Error: Could not create live link track. LiveLink module is not available.");
		return;
	}

	if (SubjectName == NAME_None)
	{
		UE_LOGF(LogLiveLinkSequencer, Warning, "Error: Could not create live link track. Desired subject name is empty.");
		return;
	}
	...
	{
		UE_LOGF(LogLiveLinkSequencer, Warning, "Error: Could not create live link track. Could not find an enabled subject with subject name '%ls'.", *SubjectName.ToString());
		return;
	}
	...
	if(bRegistered && StaticData->IsValid())
	{
		...
		{
			UE_LOGF(LogLiveLinkSequencer, Warning, "Error Creating LiveLink MovieScene Section for subject '%ls' with role '%ls", *SubjectName.ToString(), *SubjectRole->GetFName().ToString());
		}
	}
	else
	{
		UE_LOGF(LogLiveLinkSequencer, Warning, "Error: Could not register to SubjectName '%ls' from LiveLink client.", *(SubjectName.ToString()));
	}
}
		...
		if (FramesToProcess.Num() > 0)
		{
			...
			for (const FLiveLinkFrameDataStruct& Frame : FramesToProcess)
			{
				...
				if (bSyncedOrForced && CurrentFrameTime.IsSet())
				{
					...
					UE_LOGF(LogLiveLinkSequencer, VeryVerbose, "LiveLinkFrameTime: [%ls] at %ls for subject '%ls'.",
				...
				else
				{
					...
					UE_LOGF(LogLiveLinkSequencer, VeryVerbose, "LiveLinkFrameTime (Unsynced): %f, for subject '%ls'.", Frame.GetBaseData()->WorldTime.GetOffsettedTime(), *(SubjectName.ToString()));
				}
				...
				{
					UE_LOGF(LogLiveLinkSequencer, Warning, "Discarded buffered frame: [%ls] outside of start frame: [%ls] for subject '%ls'.",
...
{
	UE_LOGF(LogLiveLinkSequencer, Warning, "Static data changed for subject '%ls' while recording. This is not supported and could cause problems with associated frame data", *(SubjectName.ToString()));
}
...
void UMovieSceneLiveLinkTrackRecorder::OnFrameDataReceived(FLiveLinkSubjectKey InSubjectKey, TSubclassOf<ULiveLinkRole> InSubjectRole, const FLiveLinkFrameDataStruct& InFrameData)
{
	...
	{
		UE_LOGF(LogLiveLinkSequencer, Warning, "Received frame for Subject '%ls' but was expecting subject '%ls'", *(InSubjectKey.SubjectName.Name.ToString()), *(SubjectName.ToString()));
		return;
	}

	if (InSubjectRole != SubjectRole)
	{
		UE_LOGF(LogLiveLinkSequencer, Warning, "Received frame for Subject '%ls' for role '%ls' but was expecting role '%ls'"
...
{
	UE_LOGF(LogLiveLinkSequencer, Warning, "Loading recorded file for live link tracks is not supported.");
	return false;
}
