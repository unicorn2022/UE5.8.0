...
public:
	...
	/** Custom serializer that compresses property data before serializing it when doing persistent serialization. */
	bool Serialize(FArchive& Archive);
};


template<>
struct TStructOpsTypeTraits<FLiveLinkSubSectionData> : public TStructOpsTypeTraitsBase2<FLiveLinkSubSectionData>
{
	enum
	{
		WithSerializer = true
