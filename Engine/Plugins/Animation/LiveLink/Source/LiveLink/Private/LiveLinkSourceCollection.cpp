...
void FLiveLinkSourceCollection::RemoveSubject(FLiveLinkSubjectKey InSubjectKey)
{
	...
	TOptional<FLiveLinkSubjectKey> ReEnabledSubject;
	{
		UE::TUniqueLock Lock(SubjectsLock);
		...
		{
			bWasEnabled = Subjects[IndexOf].bEnabled;
			Subjects.RemoveAtSwap(IndexOf);
		}

		// If the removed subject was enabled and the setting is on, re-enable another subject
		// with the same name so it can take over. This prevents the case where removing a subject
		// that was shadowing another (same name, different source) leaves the subject disabled.
		if (bWasEnabled && GetDefault<ULiveLinkSettings>()->bReEnableSubjectOnRemoval)
		{
			for (FLiveLinkCollectionSubjectItem& SubjectItem : Subjects)
			{
				if (SubjectItem.Key.SubjectName == InSubjectKey.SubjectName && !SubjectItem.bPendingKill)
				{
					SubjectItem.bEnabled = true;
					ReEnabledSubject = SubjectItem.Key;
					break;
				}
			}
		}
	...
	if (ReEnabledSubject)
	{
		BroadcastOnGameThread(OnLiveLinkSubjectEnabledChanged(), *ReEnabledSubject, true);
	}
