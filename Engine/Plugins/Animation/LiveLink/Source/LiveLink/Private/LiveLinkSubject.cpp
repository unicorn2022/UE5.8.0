...
void FLiveLinkSubject::Update()
{
	...
	if (bPaused)
	{
		...
		FWriteScopeLock WriteLock(FrameSnapshotRWLock);
		if (FrameSnapshot.StaticData.IsValid() && FrameSnapshot.FrameData.IsValid())
		{
	...
	// Protect FrameSnapshot with write lock during snapshot construction
	{
		FWriteScopeLock WriteLock(FrameSnapshotRWLock);
		...
		if (FrameData.Num() > 0)
		{
			...
			case ELiveLinkSourceMode::EngineTime:
			{
				...
				const double ReadTime = Time.CurrentTime - CachedSettings.BufferSettings.EngineTimeOffset - CachedSettings.BufferSettings.EngineTimeClockOffset - CachedSettings.BufferSettings.SmoothEngineTimeOffset;

				UE_LOGF(LogLiveLink, Verbose, "Subject '%ls' Read Time: %lf    User Offset: %.4lf    Clock Offset: %.4lf    Smooth Offset: %.4lf"
		...
		else
		{
			...
			UE_LOGF(LogLiveLink, Verbose, "Subject '%ls' created snapshot with frame ID %d - Timecode:[%ls.%0.3f] - SourceTime: %0.4f, Offset: %0.6f, CorrectedTime: %0.4f", *SubjectKey.SubjectName.ToString(), FrameSnapshot.FrameData.GetBaseData()->FrameId, *FrameTC.ToString(), FrameSnapshot.FrameData.GetBaseData()->MetaData.SceneTime.Time.GetSubFrame(), FrameSnapshot.FrameData.GetBaseData()->WorldTime.GetSourceTime(), FrameSnapshot.FrameData.GetBaseData()->WorldTime.GetOffset(), FrameSnapshot.FrameData.GetBaseData()->WorldTime.GetOffsettedTime());
		}
	}

	UpdateCachedOldestDataTime();
	UpdateCachedNewestDataTime();
	UdpateCachedDataTimes();
...
bool FLiveLinkSubject::EvaluateFrame(TSubclassOf<ULiveLinkRole> InDesiredRole, FLiveLinkSubjectFrameData& OutFrame)
{
	// Protect FrameSnapshot access with read lock to allow concurrent reads from multiple animation threads
	FReadScopeLock ReadLock(FrameSnapshotRWLock);
...
void FLiveLinkSubject::AddFrameData(FLiveLinkFrameDataStruct&& InFrameData)
{
	...
	if (FrameIndex >= 0)
	{
		...
		// FrameId should be populated by LiveLinkClient framework before reaching here
		if (FLiveLinkBaseFrameData* BaseData = InFrameData.GetBaseData())
		{
			if (BaseData->FrameId == INDEX_NONE)
			{
				UE_LOGF(LogLiveLink, 
					Error, 
					"FLiveLinkSubject::AddFrameData: Subject '%ls': FrameId was unexpectedly not set at this point in the pipeline, skipping it.", 
					*SubjectKey.SubjectName.ToString()
				);

				IncreaseFrameDroppedStat();

				FrameIndex = -1; // Prevents insertion into FrameData.
			}
			else
			{
				ReceivedOrderedFrames.Enqueue(BaseData->FrameId);
			}
		}
...
bool FLiveLinkSubject::GetFrameAtWorldTime_Interpolated(const double InSeconds, FLiveLinkSubjectFrameData& OutFrame)
{
	...
	UE_LOGF(LogLiveLink, Verbose, "Subject '%ls' interpolating between frames %d and %d"
		, *SubjectKey.SubjectName.ToString()
...
void FLiveLinkSubject::UpdateState()
{
	...
	const bool bHasValidFrame = (Time.CurrentTime - GetLastPushTime() < GetDefault<ULiveLinkSettings>()->GetTimeWithoutFrameToBeConsiderAsInvalid());
			...
			if (NewRemapper.IsValid())
			{
				...
				if (bRecreateRemapper)
				{
					...
					if (SubjectSetting->Remapper->IsValidRemapper())
					{
						...
						{
							// Protect FrameSnapshot access during remapping
							{
								FWriteScopeLock WriteLock(FrameSnapshotRWLock);
								if (FrameSnapshot.StaticData.IsValid() && FrameSnapshot.FrameData.IsValid())
								{
						...
						{
							UE_LOGF(LogLiveLink, Warning, "Remapper is not compatible with this static data.");
						}
					}
					else
					{
...
ETimedDataInputState FLiveLinkSubject::GetState() const
{
	...
	switch (State)
	{
		...
		default:
			ensureMsgf(false, TEXT("Unhandled ELiveLinkSubjectState::%d"), EnumToUnderlyingType(State));
...
{
	FScopeLock Lock(&CachedTimedDataMonitorInfoCS);
	return CachedTimeDataMonitorInfo.OldestDataTime;
}

void FLiveLinkSubject::UpdateCachedOldestDataTime()
{
	FScopeLock Lock(&CachedTimedDataMonitorInfoCS);

	CachedTimeDataMonitorInfo.OldestDataTime = FTimedDataChannelSampleTime();
	...
	if (FrameData.Num() > 0)
	{
		...
		CachedTimeDataMonitorInfo.OldestDataTime = FTimedDataChannelSampleTime(OffsettedOldestTime, OldestSceneTime);
	}
}
...
{
	FScopeLock Lock(&CachedTimedDataMonitorInfoCS);
	return CachedTimeDataMonitorInfo.NewestDataTime;
}

void FLiveLinkSubject::UpdateCachedNewestDataTime()
{
	FScopeLock Lock(&CachedTimedDataMonitorInfoCS);

	CachedTimeDataMonitorInfo.NewestDataTime = FTimedDataChannelSampleTime();
	...
	if (FrameData.Num() > 0)
	{
		...
		CachedTimeDataMonitorInfo.NewestDataTime = FTimedDataChannelSampleTime(OffsettedNewestTime, NewestSceneTime);
	}
}
...
{
	FScopeLock Lock(&CachedTimedDataMonitorInfoCS);
	return CachedTimeDataMonitorInfo.DataTimes;
}

void FLiveLinkSubject::UdpateCachedDataTimes()
{
	FScopeLock Lock(&CachedTimedDataMonitorInfoCS);

	CachedTimeDataMonitorInfo.DataTimes.Reset();
	...
	if (FrameData.Num() > 0)
	{
		...
		CachedTimeDataMonitorInfo.DataTimes.Reserve(FrameData.Num());
		for (const FLiveLinkFrameDataStruct& Data : FrameData)
		{
			...
			CachedTimeDataMonitorInfo.DataTimes.Emplace(OffsettedEngineTime, AdjustedSceneTime);
		}
	}
}
