...
void ULiveLinkAnimationVirtualSubject::Update()
{
	...
	if (!IsPaused())
	{
		bLastSnapshotIsComplete = BuildSubjectSnapshot(SubjectSnapshot);

		// BuildSkeleton/BuildFrame assert that every entry's StaticData/FrameData is valid. Skip the rebuild when
		// BuildSubjectSnapshot bailed out because a source had no data and no cached fallback was available.
		if (SubjectSnapshot.Num() == Subjects.Num())
		{
			BuildSkeleton(SubjectSnapshot);
			BuildFrame(SubjectSnapshot);

			LastFrameSnapshots = MoveTemp(SubjectSnapshot);
		}
	}
...
bool ULiveLinkAnimationVirtualSubject::BuildSubjectSnapshot(TArray<FLiveLinkSubjectFrameData>& OutSnapshot)
{
	...
	bool bAllSnapshotsSucceeded = true;

	uint32 SubjectIndex = 0;
	for (const FName& SubjectName : Subjects)
	{
		...
		{
			bAllSnapshotsSucceeded = false;
			if (LastFrameSnapshots.IsValidIndex(SubjectIndex))
			{
				// Snapshots are non-copyable, so this should be fine since the resulting snapshot array will be used
				// to build the next LastFrameSnapshots list.
				NextSnapshot = MoveTemp(LastFrameSnapshots[SubjectIndex]);
			}
			else
			{
				// First evaluation after session load: no cached snapshot exists yet. Bail out — leaving NextSnapshot
				// default-initialised would trip the StaticData/FrameData IsValid checks in BuildSkeleton/BuildFrame.
				OutSnapshot.Reset();
				return false;
			}
		}
...
void ULiveLinkAnimationVirtualSubject::BuildFrame(const TArray<FLiveLinkSubjectFrameData>& InSubjectSnapshots)
{
	...
	if (NewSnapshotFrameData->MetaData.SceneTime.Time == 0)
	{
		...
		FAppTime AppTime = FAppTime::Get_UnsafeNoCheck();
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		if (TOptional<FQualifiedFrameTime> FrameTime = AppTime.CurrentFrameTime)
		{
...
void ULiveLinkAnimationVirtualSubject::SortSubjects()
{
	...
	{
		UE_LOGF(LogTemp, Warning, "Circular dependency present in attachments.");
	}
