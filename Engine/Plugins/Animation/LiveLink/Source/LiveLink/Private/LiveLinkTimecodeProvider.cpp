...
{
static const ULiveLinkCustomTimeStep* GetLiveLinkGenlock()
{
	if (!GEngine)
	{
		return nullptr;
	...
	return Cast<ULiveLinkCustomTimeStep>(GEngine->GetCustomTimeStep());
}
}


ULiveLinkTimecodeProvider::ULiveLinkTimecodeProvider()
	: Evaluation(ELiveLinkTimecodeProviderEvaluationType::Latest)
...
FQualifiedFrameTime ULiveLinkTimecodeProvider::GetQualifiedFrameTime() const
{
	...
	if (SubjectFrameTimes.Num() > 0)
	{
		...
		{
			// When using Latest mode, prefer genlocked timecode unless sourcing from sequencer
			const bool bIsSequencer = bIsSequencerSourceAnyThread.load(std::memory_order_relaxed);

			if (Evaluation == ELiveLinkTimecodeProviderEvaluationType::Latest && !bIsSequencer)
			{
				if (const ULiveLinkCustomTimeStep* LiveLinkCTS = UE::LiveLink::Private::GetLiveLinkGenlock())
				{
					const FQualifiedFrameTime GenlockTime = LiveLinkCTS->GetGenlockTriggerSceneTime();
					return ConvertTo(GenlockTime);
				}
			}

			// Fall back to buffered latest frame
...
void ULiveLinkTimecodeProvider::UnregisterSubject()
{
	...
	// Also unregister from external subject if we registered to it
	if (ExternalSubjectFrameDataReceivedHandle.IsValid() && OriginalExternalSubjectKey.Source.IsValid())
	{
		LiveLinkClient->UnregisterForFrameDataReceived(OriginalExternalSubjectKey, FDelegateHandle(), ExternalSubjectFrameDataReceivedHandle);
		ExternalSubjectFrameDataReceivedHandle.Reset();
	}
...
{
	if (!LiveLinkClient || InSubjectKey.SubjectName != SubjectKey.SubjectName)
	{
		return;
	}

	// Check if already registered to a subject with same name - unregister first
	if (RegisteredSubjectKey.SubjectName == InSubjectKey.SubjectName && RegisteredSubjectKey.Source.IsValid())
	{
		UnregisterSubject();
	}

	// Check if this is a sequencer source before registering
	FText SourceType = LiveLinkClient->GetSourceType(InSubjectKey.Source);
	FString SourceTypeStr = SourceType.ToString();
	bool bIsSequencer = SourceTypeStr.Contains(UE::LiveLink::SourceTypes::SequencerSourcePrefix, ESearchCase::IgnoreCase);

	if (bIsSequencer)
	{
		// Store the original external key if this is the first sequencer takeover
		if (!OriginalExternalSubjectKey.Source.IsValid())
		{
			OriginalExternalSubjectKey = SubjectKey;
		}

		// Register to the sequencer source without updating SubjectKey (keeps UI showing external source)
		{
			FScopeLock ScopeLock(&SubjectFrameLock);
			SubjectFrameTimes.Reset();
		}

		RegisteredSubjectKey = InSubjectKey;
		bIsSequencerSourceAnyThread.store(true, std::memory_order_relaxed);
		...
		// Also register to the disabled external subject to read its live timecode
		if (OriginalExternalSubjectKey.Source.IsValid() && LiveLinkClient->GetSubjectSettings(OriginalExternalSubjectKey))
		{
			FDelegateHandle TmpExternal;
			LiveLinkClient->RegisterForFrameDataReceived(
				OriginalExternalSubjectKey,
				FOnLiveLinkSubjectStaticDataReceived::FDelegate(),
				FOnLiveLinkSubjectFrameDataReceived::FDelegate::CreateUObject(this, &ULiveLinkTimecodeProvider::OnLiveLinkFrameDataReceived_AnyThread),
				TmpExternal,
				ExternalSubjectFrameDataReceivedHandle);

			UE_LOGF(LogLiveLink, Log, "LiveLinkTimecodeProvider: Registered to both sequencer and external subject for timecode");
		}
	}
	else
	{
		// External source - update SubjectKey and register normally
		OriginalExternalSubjectKey = InSubjectKey;
...
void ULiveLinkTimecodeProvider::OnLiveLinkSubjectRemoved(FLiveLinkSubjectKey InSubjectKey)
{
	if (InSubjectKey.SubjectName == RegisteredSubjectKey.SubjectName)
	{
		...
		// If sequencer source was removed, try to re-register to the original external subject
		if (bWasSequencerSource && LiveLinkClient && OriginalExternalSubjectKey.Source.IsValid()
			&& LiveLinkClient->GetSubjectSettings(OriginalExternalSubjectKey))
		{
			SubjectKey = OriginalExternalSubjectKey;
			RegisterSubject();
