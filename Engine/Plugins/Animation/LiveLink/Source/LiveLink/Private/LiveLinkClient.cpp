...
bool FLiveLinkClient::HandleSubjectRebroadcast(ILiveLinkSubject* InSubject, FLiveLinkSubjectFrameData&& SubjectFrameData)
{
	...
	if (InSubject->IsRebroadcasted())
	{
		if (InSubject->GetStaticData().IsValid() && SubjectFrameData.FrameData.IsValid())
		{
			...
			{
				UE_LOGF(LogLiveLink, Warning, "Rebroadcaster doesn't exist, but was requested and failed");
			}
		}
	}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FAppTime Time = FAppTime::Get_UnsafeNoCheck();
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	if (Time.CurrentFrameTime.IsSet())
	{
		CachedEngineFrameTime = Time.CurrentFrameTime;
	}
#endif
}

void FLiveLinkClient::SetLastSavedPreset(ULiveLinkPreset* InPreset)
{
	LastSavedPreset = InPreset;
}

ULiveLinkPreset* FLiveLinkClient::GetLastSavedPreset() const
{
	return LastSavedPreset.Get();
}
...
void FLiveLinkClient::Shutdown()
{
	...
	if (Collection)
	{
		...
		while(bContinue)
		{
			...
			{
				bContinue = false;
				UE_LOGF(LogLiveLink, Warning, "Force shutdown LiveLink after %f seconds. One or more sources refused to shutdown.", Timeout);
			}
		}
	}
}
...
void FLiveLinkClient::OnSubjectRemovedCallback(FLiveLinkSubjectKey InSubjectKey)
{
	...
	{
		FScopeLock Lock(&SubjectFrameCountersLock);
		SubjectFrameCounters.Remove(InSubjectKey);
	}
}
...
ELiveLinkSubjectState FLiveLinkClient::GetSubjectState(FLiveLinkSubjectName InSubjectName) const
{
	...
	if (!LiveSubject)
	{
		if (ULiveLinkVirtualSubject* VirtualSubject = SubjectItem->GetVirtualSubject())
		{
			...
			{
				return VirtualSubject->GetState();
			}
		}
		else
		{
	...
	default:
		ensureMsgf(false, TEXT("Unhandled ETimedDataInputState::%d"), EnumToUnderlyingType(InputState));
...
bool FLiveLinkClient::CreateSource(const FLiveLinkSourcePreset& InSourcePreset)
{
	...
	else
	{
		...
		Source = InSourcePreset.Settings->Factory.Get()->GetDefaultObject<ULiveLinkSourceFactory>()->CreateSource(InSourcePreset);
		if (!Source.IsValid())
		{
...
{
	CachedPresetSubjectSettings.Reset();
...
void FLiveLinkClient::PushSubjectStaticData_Internal(FPendingSubjectStatic&& SubjectStaticData)
{
	...
	if (LiveLinkSubject)
	{
		// If new static data was received, make sure it's rebroadcast
	...
	else
	{
...

		{
			...
			bool bRestoredFromPresetCache = false;
			if (!SubjectSettings)
			{
				// Check if we have cached preset settings for this subject.
				// This allows preprocessors and other settings to survive source disconnection/reconnection.
				if (TStrongObjectPtr<ULiveLinkSubjectSettings>* CachedSettings = CachedPresetSubjectSettings.Find(SubjectStaticData.SubjectKey.SubjectName))
				{
					FObjectDuplicationParameters Parameters = InitStaticDuplicateObjectParams(CachedSettings->Get(), GetTransientPackage());
					Parameters.bSkipPostLoad = true;
				
					// Cannot call ordinary DuplicateObject() because it calls PostLoad for legacy reasons,
					// which is not allowed anywhere except the GameThread.
					SubjectSettings = TStrongObjectPtr{ Cast<ULiveLinkSubjectSettings>(StaticDuplicateObjectEx(Parameters)) };

					// PATCH until UE-250259 bug is fixed
					SubjectSettings->ClearFlags(EObjectFlags::RF_NeedPostLoad | EObjectFlags::RF_NeedPostLoadSubobjects);

					bRestoredFromPresetCache = true;
				}
				else
				{
			...
			// Skip default interpolation/preprocessor setup if we restored settings from a preset cache,
			// since the cached settings already have the correct processors configured.
			if (!bRestoredFromPresetCache)
			{
...
void FLiveLinkClient::PushSubjectFrameData_AnyThread(const FLiveLinkSubjectKey& InSubjectKey, FLiveLinkFrameDataStruct&& InFrameData)
{
	...
	// Assign FrameId using uint32 wraparound arithmetic.
	if (FLiveLinkBaseFrameData* BaseData = SubjectFrame.FrameData.GetBaseData())
	{
		if (BaseData->FrameId == INDEX_NONE)
		{
			FScopeLock Lock(&SubjectFrameCountersLock);
			FLiveLinkFrameIdentifier& Counter = SubjectFrameCounters.FindOrAdd(InSubjectKey, 0);
			BaseData->FrameId = Counter;
			Counter = int32(uint32(Counter) + 1u);
		}
	}
...
void FLiveLinkClient::PushSubjectFrameData_Internal(FPendingSubjectFrame&& SubjectFrameData)
{
	...
	if (SubjectFrameData.FrameData.GetBaseData())
	{
		...
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		// Have to use the unsafe method since we're calling this outside the game thread.
		FAppTime Time = FAppTime::Get_UnsafeNoCheck();
		PRAGMA_ENABLE_DEPRECATION_WARNINGS

		const TOptional<FQualifiedFrameTime>& CurrentTime = Time.CurrentFrameTime;
				...
				if (SourceItem.Setting->ParentSubject.Name == SubjectKey.SubjectName)
				{
					...
					if (ILiveLinkSubject* LiveSubject = SubjectItem.GetLiveSubject())
					{
						...
						if (SubjectItem.GetLiveSubject()->EvaluateFrameAtWorldTime(SubjectFrameData.FrameData.GetBaseData()->WorldTime.GetSourceTime(), SubjectItem.GetLinkSettings()->Role, ChildData))
						{
							...
							UE_LOGF(LogLiveLink, Verbose, "LiveLinkHub Parent (%ls) - Child '%ls' adding frame with Timecode:[%ls.%0.3f] - SourceTime: %0.4f, Offset: %0.6f, CorrectedTime: %0.4f", *SubjectKey.SubjectName.ToString(), *SubjectItem.Key.SubjectName.ToString(), *FrameTC.ToString(), FrameData.GetBaseData()->MetaData.SceneTime.Time.GetSubFrame(), FrameData.GetBaseData()->WorldTime.GetSourceTime(), FrameData.GetBaseData()->WorldTime.GetOffset(), FrameData.GetBaseData()->WorldTime.GetOffsettedTime());
...
bool FLiveLinkClient::CreateSubject(const FLiveLinkSubjectPreset& InSubjectPreset)
{
	...
	// Cache the preset settings so they can be restored if the subject is recreated later
	// (e.g., after source disconnection/reconnection).
	if (InSubjectPreset.Settings)
	{
		CachedPresetSubjectSettings.Add(InSubjectPreset.Key.SubjectName,
			TStrongObjectPtr{DuplicateObject<ULiveLinkSubjectSettings>(InSubjectPreset.Settings, GetTransientPackage())});
	}
...
void FLiveLinkClient::PauseSubject_AnyThread(FLiveLinkSubjectName SubjectName)
{
	if (Collection)
	{
		if (const FLiveLinkCollectionSubjectItem* SubjectItem = Collection->FindSubject(SubjectName))
		{
			...
			{
				UE_LOGF(LogLiveLink, Warning, "Could not pause subject %ls since it's not in a valid state.", *SubjectName.ToString());
				return;
			}
...
bool FLiveLinkClient::IsSubjectValid(const FLiveLinkSubjectKey& InSubjectKey) const
{
	if (const FLiveLinkCollectionSubjectItem* SubjectItem = Collection->FindSubject(InSubjectKey))
	{
		if (SubjectItem->GetSubject()->HasValidFrameSnapshot())
		{
			if (ULiveLinkVirtualSubject* VSubject = SubjectItem->GetVirtualSubject())
			{
				return VSubject->GetState() == ELiveLinkSubjectState::Connected;
			}
...
void FLiveLinkClient::SetSubjectEnabled(FLiveLinkSubjectName SubjectName, bool bInEnabled)
{
	if (const FLiveLinkSubjectKey* FoundSubjectKey = EnabledSubjects.Find(SubjectName))
	{
		Collection->SetSubjectEnabled(*FoundSubjectKey, bInEnabled);
	}
}
...
FText FLiveLinkClient::GetSourceNameOverride(const FLiveLinkSubjectKey& SubjectKey) const
{
	...
	ULiveLinkSubjectSettings* SubjectSettings = Cast<ULiveLinkSubjectSettings>(GetSubjectSettings(SubjectKey));
	ULiveLinkSourceSettings* SourceSettings = GetSourceSettings(SubjectKey.Source);

	if (SubjectSettings && SourceSettings)
	{
		SourceNameOverride = SourceSettings->GetSourceNameOverride(SubjectSettings, SourceType);
	}
...
bool FLiveLinkClient::EvaluateFrame_AnyThread(FLiveLinkSubjectName InSubjectName, TSubclassOf<ULiveLinkRole> InDesiredRole, FLiveLinkSubjectFrameData& OutFrame)
{
	...
	if (FLiveLinkSubjectKey* FoundSubjectKey = EnabledSubjects.Find(InSubjectName))
	{
		...
		{
			FLiveLinkTime RequestedTime = FLiveLinkTime(FPlatformTime::Seconds(), CachedEngineFrameTime.Get(FQualifiedFrameTime()));
	...
	{
		UE_LOGF(LogLiveLink, Verbose, "Subject '%ls' is not enabled or doesn't exist", *InSubjectName.ToString());
	}
