...
namespace
{
	...
	static FAutoConsoleCommand LiveLinkPresetClearCmd(
		TEXT("LiveLink.Preset.Clear"),
		TEXT("Remove all LiveLink sources from the client"),
		FConsoleCommandDelegate::CreateLambda([]()
			{
				if (IModularFeatures::Get().IsModularFeatureAvailable(ILiveLinkClient::ModularFeatureName))
				{
					...
					LiveLinkClient.Tick();
					FLiveLinkLog::Info(TEXT("Removed all LiveLink sources"));
				}
				else
				{
					FLiveLinkLog::Error(TEXT("LiveLink client not available"));
				}
			})
		);
}
...
void ULiveLinkPreset::ApplyToClientLatent(TFunction<void(bool)> CompletionCallback)
{
	...
	ApplyToClientEndFrameHandle = FCoreDelegates::OnEndFrame.AddLambda([WeakThis = TWeakObjectPtr<ULiveLinkPreset>(this), WrappedCallback = MoveTemp(CompletionCallback), Count = ApplyCount]()
	{
		if (ensure(WeakThis->ApplyToClientPollingOperation))
		{
			...
			if (Result != FApplyToClientPollingOperation::EApplyToClientUpdateResult::Pending)
			{
				...
				UE_LOGF(LogTemp, Verbose, "Clearing preset %ls (%d)", *WeakThis->GetFullName(), Count);
	...
	UE_LOGF(LogTemp, Verbose, "Applying preset %ls (%d)", *GetFullName(), ApplyCount++);
}
