...
protected:
	...
	//~ LiveLinkVirtualSubject Interface
	ELiveLinkSubjectState GetState() const override
	{
		if (IsPaused())
		{
			return ELiveLinkSubjectState::Paused;
		}
		else if (!bLastSnapshotIsComplete)
		{
...
private:
	...
	/** Cached snapshots, used when the subjects are invalid in order to maintain tranmission of data. */
	TArray<FLiveLinkSubjectFrameData> LastFrameSnapshots;

	/** 
	 * Returns whether all snapshots were successfully created. 
	 * Will be false if no snapshot was created, or if one of the subject was invalid, causing the subject to use the previous frame's data. 
	 */
	bool bLastSnapshotIsComplete = false;
