...
enum class ELiveLinkTimecodeProviderEvaluationType
{
	...
	/** Use the newest frame that was received. When LiveLink Custom Time Step is active, uses the timecode captured at the genlock trigger point. */
	Latest,
};
...
private:
	...
	/** Stores the original external subject key before sequencer takes over, so we can restore it when sequencer stops */
	FLiveLinkSubjectKey OriginalExternalSubjectKey;

	/** When sequencer takes over, we also register to the disabled external subject to read its timecode */
	FDelegateHandle ExternalSubjectFrameDataReceivedHandle;
