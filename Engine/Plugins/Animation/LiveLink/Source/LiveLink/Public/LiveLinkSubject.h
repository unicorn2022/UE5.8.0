...
{
	FTimedDataChannelSampleTime OldestDataTime;
	FTimedDataChannelSampleTime NewestDataTime;
	TArray<FTimedDataChannelSampleTime> DataTimes;
};
...
public:
	UE_API virtual bool EvaluateFrame(TSubclassOf<ULiveLinkRole> InDesiredRole, FLiveLinkSubjectFrameData& OutFrame) override;
...
private:
	...
	//~ Updates cached timed data monitor information from the LiveLink thread (either Game Thread or dedicated thread in Live Link Hub)
	void UpdateCachedOldestDataTime();
	void UpdateCachedNewestDataTime();
	void UdpateCachedDataTimes();
...
private:
	...
	/** 
	 * This struct is updated on the LiveLink thread (either GameThread or dedicated thread in Live Link Hub).
	 * Then it's accessed by TimedDataMonitor on the GameThread.
	 */
	FTimedDataMonitorInfo CachedTimeDataMonitorInfo;

	/** Critical section used to protect access to the cached timed data monitor information. */
	mutable FCriticalSection CachedTimedDataMonitorInfoCS;
	...
	/** Read-write lock to protect FrameSnapshot access. Allows multiple concurrent readers (EvaluateFrame_AnyThread) but exclusive writer access (Update). */
	mutable FRWLock FrameSnapshotRWLock;
};
