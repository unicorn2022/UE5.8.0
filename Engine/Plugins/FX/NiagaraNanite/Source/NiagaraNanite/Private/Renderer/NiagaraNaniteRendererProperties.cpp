...
void UNiagaraNaniteRendererProperties::GetUsedMeshes(const FNiagaraEmitterInstance* InEmitter, TArray<UObject*>& OutMeshes) const
{
	for (const FNiagaraNaniteMeshRendererMeshProperties& Mesh : Meshes)
	{
		if (UStaticMesh* ResolvedMesh = Mesh.GetResolvedMesh(InEmitter))
		{
			OutMeshes.AddUnique(ResolvedMesh);
		}
	}
}
...
TArray<FNiagaraVariable> UNiagaraNaniteRendererProperties::GetBoundAttributes() const
{
	...
	{
		for (const FNiagaraNaniteMaterialOverride& MaterialOverride : OverrideMaterials)
		{
			if (MaterialOverride.UserParamBinding.Parameter.IsValid())
			{
				BoundAttributes.AddUnique(MaterialOverride.UserParamBinding.Parameter);
			}
		}
	}
