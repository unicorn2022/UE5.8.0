...
void UMovieSceneNiagaraTrackRecorder::OnRecordFrame(float DeltaSeconds)
{
	if (bRecordingEnabled && NiagaraCacheSection.IsValid() && SystemToRecord.IsValid())
	{
		...
		{
			UE_LOGF(LogNiagaraSimCachingEditor, Warning, "Recording sim cache for frame %i: %ls", RecordingFrameNumber.Value, *Warning);
		}

		for (const FString& Error : FeedbackContext.Errors)
		{
			UE_LOGF(LogNiagaraSimCachingEditor, Warning, "Unable to record sim cache for frame %i: %ls", RecordingFrameNumber.Value, *Error);
		}
	}
