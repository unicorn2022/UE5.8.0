...
struct FNiagaraCacheExecutionToken : IMovieSceneExecutionToken
{
	...
	float MapTimeToAnimation(const FMovieSceneBaseCacheParams& BaseParams, float ComponentDuration, FFrameTime InPosition, FFrameRate InTickResolution, const FMovieSceneNiagaraSectionTemplateParameter& Section) const
	{
		FFrameNumber SectionStartTime = Section.GetInclusiveStartFrame();
		FFrameNumber SectionEndTime = Section.GetExclusiveEndFrame();
		
		const float SequenceLength = Section.Params.SectionStretchMode == ENiagaraSimCacheSectionStretchMode::TimeDilate ? static_cast<float>(InTickResolution.AsSeconds(SectionEndTime - SectionStartTime)) : ComponentDuration;
		const FFrameTime AnimationLength = SequenceLength * InTickResolution;
		...
		const float SeqLength = SequenceLength - static_cast<float>(InTickResolution.AsSeconds(BaseParams.StartFrameOffset + BaseParams.EndFrameOffset));

		float AnimPosition = static_cast<float>(FFrameTime::FromDecimal((InPosition - SectionStartTime).AsDecimal() * AnimPlayRate) / InTickResolution);
		AnimPosition += static_cast<float>(InTickResolution.AsSeconds(BaseParams.FirstLoopStartFrameOffset));
		if (SeqLength > 0.f && (bLooping || !FMath::IsNearlyEqual(AnimPosition, SeqLength, 1e-4f)))
		{
			AnimPosition = FMath::Fmod(AnimPosition, SeqLength);
		}
		AnimPosition += static_cast<float>(InTickResolution.AsSeconds(BaseParams.StartFrameOffset));
