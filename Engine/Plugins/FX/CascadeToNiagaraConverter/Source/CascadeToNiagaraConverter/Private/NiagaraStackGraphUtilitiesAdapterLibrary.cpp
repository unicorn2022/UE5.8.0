...
FAssetData UFXConverterUtilitiesLibrary::CreateAssetData(FString InPath)
{
	...
	{
		UE_LOGF(LogScript, Error, "Failed to make path %ls" , *InPath);
	}
...
void UNiagaraEmitterConversionContext::InternalFinalizeStackEntryAddActions()
{
	...
	auto ApplySetParameterAction = [&StackItemGroups, &GetStackItemGroupForStackEntryID](const FStackEntryAddAction& SetParameterAction) {
		...
		{
			UE_LOGF(LogFXConverter, Error, "%ls", *PasteWarning.ToString());
			return;
		}
	};
	...
	if (ensureMsgf(RendererStackItemGroupPtr != nullptr, TEXT("Failed to find renderer stack items group for Emitter!")))
	{
		...
		for (auto It = RendererNameToStagedRendererPropertiesMap.CreateIterator(); It; ++It)
		{
			...
			if (PasteWarning.IsEmpty() == false)
			{
				UE_LOGF(LogFXConverter, Warning, "%ls", *PasteWarning.ToString());
			}
		}
	}
...
void UNiagaraEmitterConversionContext::SetDetailBitMask(int32 BitMask)
{
	...
	for (int32 i=0; i < int32(EParticleDetailMode::PDM_MAX); ++i)
	{
		...
		{
			UE_LOGF(LogFXConverter, Error, "Failed to find Niagara quality level for \"%ls\", this will be ignored!", *CascadeDetailText.ToString());
			continue;
		}
...
void UNiagaraEmitterConversionContext::SetRendererBinding(UNiagaraRendererProperties* InRendererProperties, FName BindingName, FName VariableToBindName, ENiagaraRendererSourceDataMode SourceDataMode)
{
	if (UNiagaraComponentRendererProperties* ComponentRendererProperties = Cast<UNiagaraComponentRendererProperties>(InRendererProperties))
	{
		...
		UE_LOGF(LogFXConverter, Error, "Tried to set component renderer binding \"%ls\" but it was not supported for component!", *BindingName.ToString());
		return;
	}
	...
	if (!ObjectProp)
	{
		UE_LOGF(LogFXConverter, Error, "Tried to set renderer binding \"%ls\" but the property could not be found!", *BindingName.ToString());
		return;
	}
	...
	if (!ObjectStructProp)
	{
		UE_LOGF(LogFXConverter, Error, "Tried to set renderer binding \"%ls\" but the property could be cast to a struct property!", *BindingName.ToString());
		return;
	}

	if (!ObjectStructProp->Struct)
	{
		UE_LOGF(LogFXConverter, Error, "Tried to set renderer binding \"%ls\" but the struct property did not have a valid UStruct!", *BindingName.ToString());
		return;
	}

	if (!ObjectStructProp->Struct->IsChildOf(FNiagaraVariableAttributeBinding::StaticStruct()))
	{
		UE_LOGF(LogFXConverter, Error, "Tried to set renderer binding \"%ls\" but the ustruct of the property was not a child of FNiagaraVariableAttributeBinding static struct!", *BindingName.ToString());
		return;
	}
	...
	if(!AttributeBinding)
	{
		UE_LOGF(LogFXConverter, Error, "Tried to set renderer binding \"%ls\" but failed to get the container to value ptr of the FPropertyStruct!", *BindingName.ToString());
		return;
	}
