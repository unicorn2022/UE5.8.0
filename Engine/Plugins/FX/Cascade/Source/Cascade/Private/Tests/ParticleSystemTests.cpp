	...
	*/
	static UObject* CreateAsset(UFactory* CreateFactory, UClass* AssetClass, const FString& AssetName)
	{
		...
		if (CreatedAsset)
		{
			...
			UE_LOGF(LogParticleEditorPromotionTests, Display, "Created asset %ls (%ls)", *AssetName, *AssetClass->GetName());
		}
		else
		{
			UE_LOGF(LogParticleEditorPromotionTests, Error, "Unable to create asset of type %ls", *AssetClass->GetName());
		}
		...
		*/
		bool ContentBrowser_CreateAParticleSystem_Part1()
		{
			...
			if (CreatedPS)
			{
				...
				UE_LOGF(LogParticleEditorPromotionTests, Display, "Opened the cascade editor");
			}
			else
			{
				UE_LOGF(LogParticleEditorPromotionTests, Error, "Failed to create a new ParticleSystem");
			}
		...
		*/
		bool ContentBrowser_CreateAParticleSystem_Part2()
		{
			...
			if (CreatedPS)
			{
				...
				if (bModifiedSize)
				{
					UE_LOGF(LogParticleEditorPromotionTests, Display, "Modified ParticleSystem StartSize (Min and Max)");
					...
					UE_LOGF(LogParticleEditorPromotionTests, Display, "Saved the particle system");
				}
				else
				{
					UE_LOGF(LogParticleEditorPromotionTests, Error, "Failed to modify ParticleSystem StartSize");
				}

				// @TODO: Close only Cascade instead of all editors
				GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->CloseAllAssetEditors();
				UE_LOGF(LogParticleEditorPromotionTests, Display, "Closed the cascade editor");
			}
