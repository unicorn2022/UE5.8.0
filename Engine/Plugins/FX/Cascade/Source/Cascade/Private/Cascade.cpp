...
FCascade::~FCascade()
{
	UE_LOGF(LogCascade,Log,"Quitting Cascade. FXSystem=0x%p",GetFXSystem());
	...
	if (PreviewViewport.IsValid() && PreviewViewport->GetViewportClient().IsValid())
	{
		...
		if (FloorComponent)
		{
			...
			if (FloorComponent->GetStaticMesh())
			{
				...
				{
					UE_LOGF(LogCascade, Warning, "Unable to locate Cascade floor mesh outer...");
					EditorOptions->FloorMesh = TEXT("");
				}
...
void FCascade::OnNewEmitter()
{
	...
	if (ParticleSystem->Emitters.Num() > 0)
	{
		...
		if (ExistingEmitter->LODLevels.Num() > 1)
		{
			...
			{
				UE_LOGF(LogCascade, Warning, "Failed to autogenerate lowest LOD level!");
			}
		}

		if (ExistingEmitter->LODLevels.Num() > 2)
		{
			UE_LOGF(LogCascade, Log, "Generating existing LOD levels...");
	...
	if (DupObject)
	{
		...
		if (DraggedModule == pkSourceModule)
		{
			...
			for (int32 LODIndex = 1; LODIndex < pkTargetEmitter->LODLevels.Num(); LODIndex++)
			{
				...
				{
					UE_LOGF(LogCascade, Warning, "Missing dragged module!");
				}
			}
		}
...
bool FCascade::DuplicateEmitter(UParticleEmitter* SourceEmitter, UParticleSystem* DestSystem, bool bShare)
{
	...
	if (SourceOuter != DestSystem)
	{
		if (bShare == true)
		{
			UE_LOGF(LogCascade, Warning, "Can't share modules across particle systems!");
			bShare = false;
		}
	}
...
void FCascade::AddNewEmitter(int32 PositionOffset)
{
	...
	if (EmitterIndex != -1)
	{
		UE_LOGF(LogCascade, Log, "Insert New Emitter Before %d", EmitterIndex);
...
void FCascade::RegenerateLowestLOD(bool bDupeHighest)
{
	...
	{
		UE_LOGF(LogCascade, Log, "Regenerate Lowest LOD levels!");
	...
	{
		UE_LOGF(LogCascade, Log, "CANCELLED Regenerate Lowest LOD levels!");
	}
...
void FCascade::AddLOD(bool bBeforeCurrent)
{
	...
	if (ParticleSystem->Emitters.Num() > 0)
	{
		...
		UE_LOGF(LogCascade, Log, "Inserting LOD level at %d", CurrentLODIndex);
...
bool FCascade::EndTransaction(const FText& Description)
{
	...
	{
		UE_LOGF(LogCascade, Log, "Cascade -   EndTransaction = %ls --- Curr = %ls", *Description.ToString(), *TransactionDescription.ToString());
		return false;
	}
...
void FCascade::OnSetRandomSeed()
{
	if ((SelectedModule != NULL) && (SelectedModule->SupportsRandomSeed()))
	{
		...
		{
			UE_LOGF(LogCascade, Warning, "Failed to set random seed entry on module %ls", *(SelectedModule->GetClass()->GetName()));
		}
...
void FCascade::OnConvertToSeeded()
{
	if ((SelectedModule != NULL) && (SelectedModule->SupportsRandomSeed() == false))
	{
		...
		UE_LOGF(LogCascade, Log, "Non-seeded module %ls", *ClassName);
		...
		if (SeededClass != NULL)
		{
			...
			if (BaseLODLevel != NULL)
			{
				...
				{
					UE_LOGF(LogCascade, Warning, "Failed to convert module!");
				}
				EndTransaction(Transaction);
...
void ParticleSystem_DumpInfo(UParticleSystem* InParticleSystem)
{
	...
	{
		UE_LOGF(LogCascade, Log, "Dumping info for %ls", *(InParticleSystem->GetPathName()));
		UE_LOGF(LogCascade, Log, "\tEmitterCount = %d", InParticleSystem->Emitters.Num());
		for (int32 EmitterIdx = 0; EmitterIdx < InParticleSystem->Emitters.Num(); EmitterIdx++)
		{
			UE_LOGF(LogCascade, Log, "\t\tEmitter %d", EmitterIdx);
			UParticleEmitter* Emitter = InParticleSystem->Emitters[EmitterIdx];
			if (Emitter != NULL)
			{
				UE_LOGF(LogCascade, Log, "\t\t\tLODLevels %d", Emitter->LODLevels.Num());
				for (int32 LODIdx = 0; LODIdx < Emitter->LODLevels.Num(); LODIdx++)
				{
					UParticleLODLevel* LODLevel = Emitter->LODLevels[LODIdx];
					if (LODLevel != NULL)
					{
						UE_LOGF(LogCascade, Log, "\t\t\t\tLODLevel %d", LODIdx);
						...
						UE_LOGF(LogCascade, Log, "%ls", *ModuleDump);
					}
					else
					{
						UE_LOGF(LogCascade, Log, "\t\t\t\t*** NULL");
					}
				}
			}
			else
