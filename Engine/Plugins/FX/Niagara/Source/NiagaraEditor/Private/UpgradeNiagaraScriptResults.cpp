...
TArray<UNiagaraClipboardFunctionInput*> UUpgradeNiagaraScriptResults::CopyOldInputs() const
{
	return CopyInputs(OldInputs);
}

TArray<UNiagaraClipboardFunctionInput*> UUpgradeNiagaraScriptResults::CopyNewInputs() const
{
	return CopyInputs(NewInputs);
}

TArray<UNiagaraClipboardFunctionInput*> UUpgradeNiagaraScriptResults::CopyInputs(const TArray<TObjectPtr<UNiagaraPythonScriptModuleInput>>& Source) const
{
	TArray<UNiagaraClipboardFunctionInput*> Result;
	for (const UNiagaraPythonScriptModuleInput* Input : Source)
	{
		if (Input->Input)
		{
			if (UNiagaraClipboardFunctionInput* NewInput = Cast<UNiagaraClipboardFunctionInput>(StaticDuplicateObject(Input->Input, GetTransientPackage())))
			{
				NewInput->CopyValuesFrom(Input->Input);
				Result.Add(NewInput);
			}
		}
	}
