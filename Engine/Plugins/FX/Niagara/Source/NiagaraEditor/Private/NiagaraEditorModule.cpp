...
static FAutoConsoleCommand LogFolderPermissionsCommand = FAutoConsoleCommand(
	...
	FConsoleCommandWithArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args)
		{
			...
			{
				UE_LOGF(LogNiagaraEditor, Display, "Trying to find tagged Niagara assets under path %ls", *CurrentPath.GetInternalPathString());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
				...
				{
					UE_LOGF(LogNiagaraEditor, Warning, "Didn't find any assets to resave under %ls", *CurrentPath.GetInternalPathString());
				}
			}
			else
			{
				UE_LOGF(LogNiagaraEditor, Warning, "No valid path found");	
			}
		}));
...
class FNiagaraEditorOnlyDataUtilities : public INiagaraEditorOnlyDataUtilities
{
	...
	{
		return FNiagaraSystemStateDataResolver::TryResolve(System);
	}
	...
	TSharedPtr<class SGraphPin> InternalCreatePin(class UEdGraphPin* InPin) const
	{
		if (const UEdGraphSchema_Niagara* NSchema = Cast<UEdGraphSchema_Niagara>(InPin->GetSchema()))
		{
			if (InPin->PinType.PinCategory == UEdGraphSchema_Niagara::PinCategoryType || InPin->PinType.PinCategory == UEdGraphSchema_Niagara::PinCategoryStaticType)
			{
				...
				{
					UE_LOGF(LogNiagaraEditor, Warning, "Pin type is invalid! Pin Name '%ls' Owning Node '%ls'. Turning into standard int definition!", *InPin->PinName.ToString(),
			...
			else if (InPin->PinType.PinCategory == UEdGraphSchema_Niagara::PinCategoryEnum || InPin->PinType.PinCategory == UEdGraphSchema_Niagara::PinCategoryStaticEnum)
			{
				...
				if (Enum == nullptr)
				{
					UE_LOGF(LogNiagaraEditor, Warning, "Pin states that it is of Enum type, but is missing its Enum! Pin Name '%ls' Owning Node '%ls'. Turning into standard int definition!", *InPin->PinName.ToString(),
						*InPin->GetOwningNode()->GetFullName());
...
void DumpParameterStore(const FNiagaraParameterStore& ParameterStore)
{
	...
	for (const FNiagaraVariable& ParameterVariable : ParameterVariables)
	{
		...
		UE_LOGF(LogNiagaraEditor, Log, "%ls\t%ls\t%ls", *Name, *Type, *Value);
	}
}
...
void DumpRapidIterationParametersForScript(UNiagaraScript* Script, const FString& HeaderName)
{
	...
	UE_LOGF(LogNiagaraEditor, Log, "%ls - %ls - %ls", *Script->GetPathName(), *HeaderName, *UsageName);
	...
	if (Arguments.Num() == 1)
	{
		...
		if (Asset != nullptr)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogNiagaraEditor, Warning, "DumpRapidIterationParameters - Only niagara system and niagara emitter assets are supported");
				}
			}
		}
		else
		{
			UE_LOGF(LogNiagaraEditor, Warning, "DumpRapidIterationParameters - Asset not found");
		}
	}
	else
	{
		UE_LOGF(LogNiagaraEditor, Warning, "DumpRapidIterationParameters - Must supply an asset path to dump");
	}
}
	...
	if (Arguments.Num() > 0)
	{
		...
		if (SystemAsset.IsValid())
		{
			...
			if (System != nullptr)
			{
				...
				UE_LOGF(LogNiagaraEditor, Log, "%ls", *Dump);
			}
			else
			{
				UE_LOGF(LogNiagaraEditor, Warning, "Could not load system asset for argument: %ls", *Arguments[0]);
			}
		}
		else
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Could not find asset for argument: %ls", *Arguments[0]);
		}
	}
	else
	{
		UE_LOGF(LogNiagaraEditor, Warning, "Command required an asset reference to be passed in.");
	}
}
...
void ExecuteInvalidateNiagaraCachedScripts(const TArray< FString >& Args)
{
	...
	if (FileHandle)
	{
		...
		UE_LOGF(LogConsoleResponse, Display, "fx.InvalidateCachedScripts succeeded\n\"NiagaraShaderVersion.ush\" was updated.\n");
	}
	else
	{
...
{
	UE_LOGF(LogConsoleResponse, Display, "fx.RebuildDirtyScripts started.\n");
...
void FNiagaraEditorModule::StartupModule()
{
	...
	ContentBrowserModule.RegisterDynamicTagAssetClass(UNiagaraSystem::StaticClass()->GetClassPathName());
	ContentBrowserModule.RegisterDynamicTagAssetClass(UNiagaraEmitter::StaticClass()->GetClassPathName());
	...
	// Preload all parameter definition & collection assets so that they will be postloaded before postload calls to scripts/emitters/systems that rely on them.
	{
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(FNiagaraEditorModule::OnFilesLoaded);
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(FNiagaraEditorModule::OnAssetsAdded);
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FNiagaraEditorModule::OnPostEngineInit);
	...
	RegisterExternalObjectUtilities(*UDistribution::StaticClass(), true, MakeShared<FNiagaraCascadeDistributionExternalObjectUtilities, ESPMode::ThreadSafe>());
...
void FNiagaraEditorModule::ShutdownModule()
{
	...
	UnregisterDynamicCustomizations();
	...
	if (FContentBrowserModule* ContentBrowserModule = FModuleManager::GetModulePtr<FContentBrowserModule>("ContentBrowser");
		(ContentBrowserModule != nullptr) && UObjectInitialized())
	{
		ContentBrowserModule->UnregisterDynamicTagAssetClass(UNiagaraSystem::StaticClass()->GetClassPathName());
		ContentBrowserModule->UnregisterDynamicTagAssetClass(UNiagaraEmitter::StaticClass()->GetClassPathName());
	}
}
...
void FNiagaraEditorModule::OnPostEngineInit()
{
	...
	if (GEditor)
	{
		...
		PreviewPlatformChangedHandle = EditorEngine->OnPreviewShaderPlatformChanged().AddRaw(this, &FNiagaraEditorModule::OnPreviewPlatformChanged);
		...
		if (UImportSubsystem* ImportSubsystem = GEditor->GetEditorSubsystem<UImportSubsystem>())
		{
			ImportSubsystem->OnAssetReimport.AddLambda(
				...
				{
					UNiagaraMeshRendererProperties::OnMeshChanged(ObjectReimported);
				}
			);
		}
	}
	...
	{
		UE_LOGF(LogNiagaraEditor, Log, "GEditor isn't valid! Particle reset commands will not work for Niagara components!");
	}
...
void FNiagaraEditorModule::OnPreviewPlatformChanged(EShaderPlatform NewShaderPlatform)
{
	FNiagaraPlatformSet::InvalidateCachedData();

	UNiagaraScript::SetPreviewFeatureLevel(GetMaxSupportedFeatureLevel(NewShaderPlatform));
...
{
	UE::TScopeLock ScopeLock(EditorUtilitiesGuard);
...
void FNiagaraEditorModule::RegisterExternalObjectUtilities(const UClass& InClass, bool bHandleSubclasses, TSharedRef<INiagaraEditorExternalObjectUtilities, ESPMode::ThreadSafe> InExternalObjectUtilities)
{
	UE::TScopeLock ScopeLock(EditorUtilitiesGuard);
	if (RegisteredExternalObjectUtilities.ContainsByPredicate([&InClass]
		(const FRegisteredExternalObjectUtilities& RegisteredUtilities) { return RegisteredUtilities.Class == &InClass; }) == false)
	{
		FRegisteredExternalObjectUtilities& AddedUtilities = RegisteredExternalObjectUtilities.AddDefaulted_GetRef();
		AddedUtilities.Class = &InClass;
		AddedUtilities.bHandleSubclasses = bHandleSubclasses;
		AddedUtilities.ExternalObjectUtilities = InExternalObjectUtilities;
	}
}
...
TSharedPtr<INiagaraEditorTypeUtilities, ESPMode::ThreadSafe> FNiagaraEditorModule::GetTypeUtilities(const FNiagaraTypeDefinition& Type) const
{
	const TSharedRef<INiagaraEditorTypeUtilities, ESPMode::ThreadSafe>* TypeUtilities = nullptr;
	{
		UE::TScopeLock ScopeLock(EditorUtilitiesGuard);
		TypeUtilities = TypeToEditorUtilitiesMap.Find(Type);
	}

	if(TypeUtilities != nullptr)
	{
...
TSharedPtr<INiagaraEditorPropertyUtilities, ESPMode::ThreadSafe> FNiagaraEditorModule::GetPropertyUtilities(const UScriptStruct& Struct) const
{
	const TSharedRef<INiagaraEditorPropertyUtilities, ESPMode::ThreadSafe>* PropertyUtilities = nullptr;
	{
		UE::TScopeLock ScopeLock(EditorUtilitiesGuard);
		PropertyUtilities = StructToPropertyUtilitiesMap.Find(&Struct);
	}
	return PropertyUtilities != nullptr ? *PropertyUtilities : TSharedPtr<INiagaraEditorPropertyUtilities>();
}

TSharedPtr<const INiagaraEditorExternalObjectUtilities, ESPMode::ThreadSafe> FNiagaraEditorModule::GetExternalObjectUtilities(const UClass& Class) const
{
	const FRegisteredExternalObjectUtilities* MatchingRegisteredUtilities = nullptr;
	{
		UE::TScopeLock ScopeLock(EditorUtilitiesGuard);
		MatchingRegisteredUtilities = RegisteredExternalObjectUtilities.FindByPredicate([&Class](const FRegisteredExternalObjectUtilities& RegisteredUtilities)
		{
			return RegisteredUtilities.Class == &Class || (RegisteredUtilities.bHandleSubclasses && Class.IsChildOf(RegisteredUtilities.Class));
		});
	}
	return MatchingRegisteredUtilities != nullptr ? MatchingRegisteredUtilities->ExternalObjectUtilities : TSharedPtr<const INiagaraEditorExternalObjectUtilities>();
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FNiagaraEditorModule::OnAssetRegistryLoadComplete);
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
	IAssetRegistry& AssetRegistry = AssetRegistryModule.Get();
	...
	// NDC is only for when running in the Editor with UI
...
void FNiagaraEditorModule::ConditionalRegisterDynamicStruct(const UScriptStruct* Struct)
{
	...
	if (Struct->IsChildOf(FNDCAccessContextBase::StaticStruct()))
	{
		...
		if (RegisteredDynamicStructs.Contains(StructName))
		{
			return;
		}
		...
		RegisteredDynamicStructs.Add(StructName);
	}
}
...
void FNiagaraEditorModule::OnModulesChanged(FName Module, EModuleChangeReason Reason)
{
	...
	{
		// if we are running a cook commandlet we don't need to setup the NDC types or refresh them
		if (!IsRunningCookCommandlet())
		{
			if (FModuleManager::Get().DoesLoadedModuleHaveUObjects(Module))
			{
				FNameBuilder ScriptPackageName(FPackageName::GetModuleScriptPackageName(Module));
				if (const UPackage* ScriptPackage = FindPackage(nullptr, ScriptPackageName.ToString()))
				{
					ForEachObjectWithPackage(ScriptPackage, [this](UObject* Object) -> bool
					{
						if (const UScriptStruct* Struct = Cast<const UScriptStruct>(Object))
						{
							ConditionalRegisterDynamicStruct(Struct);
						}
...
void FNiagaraEditorModule::RegisterDynamicCustomizations()
{
	...
	{
		ConditionalRegisterDynamicStruct(*It);
	}
}

void FNiagaraEditorModule::UnregisterDynamicCustomizations()
{
	if (!RegisteredDynamicStructs.IsEmpty())
	{
		if (FModuleManager::Get().IsModuleLoaded("PropertyEditor"))
		{
			FPropertyEditorModule& PropertyModule = FModuleManager::LoadModuleChecked<FPropertyEditorModule>("PropertyEditor");
			for (const FName& StructName : RegisteredDynamicStructs)
			{
				PropertyModule.UnregisterCustomPropertyTypeLayout(StructName);
			}
		}

		RegisteredDynamicStructs.Reset();
	}
}
							...
							FExecuteAction::CreateLambda([InSelectedAssets = Context->SelectedAssets]()
								{
									...
									{
										UE_LOGF(LogNiagaraEditor, Warning, "Didn't find any assets to resave");
									}
								}));
					}
				}
	...
	for (UNiagaraEffectType* EffectType : BaselinesToGenerate)
	{
		if (EffectType->IsPerfBaselineValid() == false && EffectType->GetPerfBaselineController())
		{
			if (!BaselineViewport->AddBaseline(EffectType))
			{
				...
				UE_LOGF(LogNiagaraEditor, Warning, "Failed to add baseline! %ls", *EffectType->GetName());
			}
		}
	}
}
