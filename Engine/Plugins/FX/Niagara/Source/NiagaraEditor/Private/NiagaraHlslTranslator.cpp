...
static int GNiagaraCompilationUseNewMissingDefaultTraversal = 1;
static FAutoConsoleVariableRef CVarNiagaraCompilationUseNewMissingDefaultTraversal(
	TEXT("fx.Niagara.Compilation.UseNewMissingDefaultTraversal"),
	GNiagaraCompilationUseNewMissingDefaultTraversal,
	TEXT("Whether or not to use the new missing default traversal when compiling scripts."),
	ECVF_Default
);
...
void FNiagaraShaderQueueTickable::ProcessQueue()
{
	...
	for (FNiagaraCompilationQueue::NiagaraCompilationQueueItem &Item : FNiagaraCompilationQueue::Get()->GetQueue())
	{
		...
		if (ShaderScript == nullptr) // This script has been removed from the pending queue post submission... just skip it.
		{
			...
			UE_LOGF(LogNiagaraEditor, Log, "GPU shader compile skipped. Id %d", NewShaderMap->GetCompilingId());
			continue;
		}
...
void TNiagaraHlslTranslator<GraphBridge>::GenerateFunctionSignature(ENiagaraScriptUsage ScriptUsage, FStringView InName, FName InFullName, FStringView InFunctionNameSuffix, const FGraph* FuncGraph, TConstArrayView<const FInputPin*> CallInputs, TArray<int32>& Inputs,
	FString& FunctionComment, FNiagaraFunctionSignature& OutSig) const
{
	NIAGARA_SCOPE_CYCLE_COUNTER(STAT_NiagaraEditor_Module_NiagaraHLSLTranslator_GenerateFunctionSignature);

	TArray<const FInputNode*, TInlineAllocator<16>> InputNodes;
	{
		...
		InputNodes = GraphBridge::GetGraphInputNodes(FuncGraph, Options);

		if (Inputs.Num() != InputNodes.Num())
		{
			const_cast<TNiagaraHlslTranslator<GraphBridge>*>(this)->Error(FText::Format(LOCTEXT("GenerateFunctionSignatureFail", "Generating function signature for {0} failed.  The function call is providing a different number of inputs than the function graph supplies."),
				FText::FromString(InFullName.ToString())));
			return;
		}
	}

	TArray<FNiagaraVariable, TInlineAllocator<16>> ConstantLiteralInputVars;
	TArray<FNiagaraVariable, TInlineAllocator<16>> InputVars;
	TArray<FNiagaraVariableBase, TInlineAllocator<16>> OutputVars;

	bool bHasDIParameters = false;
	{
		const int32 InputCount = Inputs.Num();

		ConstantLiteralInputVars.Reserve(InputCount);
		InputVars.Reserve(InputCount);

		for (int32 InputIt = 0; InputIt < InputCount; ++InputIt)
		{
			if (Inputs[InputIt] == INDEX_NONE)
			{
				continue;
			}

			FNiagaraVariable InputVar = GraphBridge::GetInputVariable(InputNodes[InputIt]);
			if (GetLiteralConstantVariable(InputVar))
			{
				ConstantLiteralInputVars.Emplace(InputVar);
				Inputs[InputIt] = INDEX_NONE;
			}
			else
			{
				if (!bHasDIParameters && InputVar.GetType().IsDataInterface())
				{
					bHasDIParameters = true;
				}
				InputVars.Emplace(InputVar);
			}
		}
	}
	...
	{
		const TArray<FNiagaraVariable> OutputNodeVariables = GraphBridge::GetGraphOutputNodeVariables(FuncGraph, ScriptUsage);
		OutputVars.Reserve(OutputNodeVariables.Num());
		Algo::Transform(OutputNodeVariables, OutputVars, [](const FNiagaraVariable& Var) -> FNiagaraVariableBase { return Var; });
	}
	...
	const bool bHasParameterMapParameters = GraphBridge::GraphHasParametersOfType(FuncGraph, FNiagaraTypeDefinition::GetParameterMapDef());
	const bool bScriptUsageRequiresUnique = ScriptUsage == ENiagaraScriptUsage::Module
		|| ScriptUsage == ENiagaraScriptUsage::DynamicInput
		|| ScriptUsage == ENiagaraScriptUsage::EmitterSpawnScript
		|| ScriptUsage == ENiagaraScriptUsage::EmitterUpdateScript;
	const bool bParametersRequiresUnique = bHasParameterMapParameters || bHasDIParameters;
	const bool bHasUniqueNaming = (ModuleAliasStr != nullptr || EmitterAliasStr != nullptr);

	if (bHasUniqueNaming && (bScriptUsageRequiresUnique || bParametersRequiresUnique))
	{
		FNameBuilder SignatureName;
		...
		{
			SignatureName << TEXT('_');
			SignatureName << InFunctionNameSuffix;
		}
		NiagaraHlslTranslatorImpl::ReplaceInline(SignatureName, 0, TEXT("."), TEXT("_"));
		OutSig = FNiagaraFunctionSignature(*SignatureName, InputVars, OutputVars, true, false);
	}
	else
	{
		// compose a string containing all of the decorations to the function and boil it down into a Guid that we can append to the function name
		FGuid PermutationGuid;
		{
			...
			for (const FNiagaraVariableBase& StaticSwitchInput : GraphBridge::GraphGetStaticSwitchInputs(FuncGraph))
			{
				const FInputPin* const* FoundInputPinPtr = CallInputs.FindByPredicate([PinName = StaticSwitchInput.GetName()](const FInputPin* Pin) -> bool
				{
					return Pin && PinName.IsEqual(Pin->PinName);
				});

				if (FoundInputPinPtr)
				{
					const FInputPin& InputPin = **FoundInputPinPtr;
					FunctionAliasContext.AddPermutationIdentifier(InputPin.PinName, InputPin.DefaultValue);
				}
			}

			FuncGraph->UpdateFunctionAliasByContext(FunctionAliasContext);

			FNameBuilder FunctionAliasSuffix;
			ResolveGraphFunctionAlias(FunctionAliasContext, FunctionAliasSuffix);

			for (const FNiagaraVariable& LiteralConstant : ConstantLiteralInputVars)
			{
				checkf(LiteralConstant.GetType().IsSameBaseDefinition(FNiagaraTypeDefinition::GetBoolDef()), TEXT("Only boolean types are currently supported for literal constants."));
				FunctionAliasSuffix << TEXT(',');
				LiteralConstant.GetName().AppendString(FunctionAliasSuffix);
				FunctionAliasSuffix << TEXT('_');
				FunctionAliasSuffix << (LiteralConstant.GetValue<bool>() ? TEXT("true") : TEXT("false"));
			}

			if (!InputVars.IsEmpty())
			{
				FunctionAliasSuffix << TEXT(",Input - [");
				for (const FNiagaraVariableBase& InputVar : InputVars)
				{
					InputVar.GetType().GetFName().AppendString(FunctionAliasSuffix);
					FunctionAliasSuffix << TEXT(" ");
					InputVar.GetName().AppendString(FunctionAliasSuffix);
					FunctionAliasSuffix << TEXT(";");
				}
				FunctionAliasSuffix << TEXT("]");
			}
			...
			{
				FunctionAliasSuffix << TEXT(",Output - [");
				for (const FNiagaraVariableBase& OutputVar : OutputVars)
				{
					OutputVar.GetType().GetFName().AppendString(FunctionAliasSuffix);
					FunctionAliasSuffix << TEXT(" ");
					OutputVar.GetName().AppendString(FunctionAliasSuffix);
					FunctionAliasSuffix << TEXT(";");
				}
				FunctionAliasSuffix << TEXT("]");
			}

			if (FunctionAliasSuffix.Len() > 0)
			{
				PermutationGuid = FGuid::NewDeterministicGuid(FunctionAliasSuffix.ToView());
				FunctionComment = FString(InName, FunctionAliasSuffix.Len());
				FunctionComment.Append(FunctionAliasSuffix.ToView());
			}
		}

		FNameBuilder NameBuilder;
		NameBuilder.Append(InName);

		if (PermutationGuid.IsValid())
		{
			NameBuilder << TEXT('_');
			PermutationGuid.AppendString(NameBuilder, EGuidFormats::Base36Encoded);
		}

		OutSig = FNiagaraFunctionSignature(*NameBuilder, InputVars, OutputVars, true, false);
	}
...
template<typename GraphBridge>
void TNiagaraHlslTranslator<GraphBridge>::ResolveGraphFunctionAlias(const FNiagaraGraphFunctionAliasContext& FunctionAliasContext, FStringBuilderBase& FunctionAliasSuffix) const
{
	FunctionAliasContext.AppendPermutationString(FunctionAliasSuffix);

	TConstArrayView<FNiagaraGraphFunctionAliasContext::FStaticSwitchVariable> PendingStaticSwitchVariables = FunctionAliasContext.ReadPendingStaticSwitchVariables();
	if (!PendingStaticSwitchVariables.IsEmpty())
	{
		FNiagaraFixedConstantResolver ConstantResolver(this);

		for (FNiagaraGraphFunctionAliasContext::FStaticSwitchVariable CompilerVariable : PendingStaticSwitchVariables)
		{
			int32 SwitchValue = INDEX_NONE;

			if (ConstantResolver.ResolveConstant(CompilerVariable.Variable))
			{
				ENiagaraStaticSwitchType SwitchType = static_cast<ENiagaraStaticSwitchType>(CompilerVariable.SwitchType);
				if (SwitchType == ENiagaraStaticSwitchType::Bool)
				{
					SwitchValue = CompilerVariable.Variable.GetValue<bool>() ? 1 : 0;
				}
				else if (SwitchType == ENiagaraStaticSwitchType::Integer || SwitchType == ENiagaraStaticSwitchType::Enum)
				{
					SwitchValue = CompilerVariable.Variable.GetValue<int32>();
				}
			}

			if (SwitchValue != INDEX_NONE)
			{
				FunctionAliasSuffix.AppendChar(TEXT(','));
				CompilerVariable.Variable.GetName().AppendString(FunctionAliasSuffix);
				FunctionAliasSuffix.AppendChar(TEXT('='));
				FunctionAliasSuffix.Append(FString::FormatAsNumber(SwitchValue));
			}
		}
	}
}
...
FString FNiagaraHlslTranslator::GetFunctionDefinitions()
{
	...
	for (const auto& FuncPair : Functions)
	{
		...
		if (!FuncPair.Value.Body.IsEmpty())
		{
			...
			if (!FuncPair.Value.Comments.IsEmpty())
			{
				TArray<FStringView, TInlineAllocator<16>> CommentLines;
				UE::String::ParseTokens(FuncPair.Value.Comments, TEXT(","), [&CommentLines](FStringView Token) { CommentLines.Add(Token); });

				for (FStringView CommentLine : CommentLines)
				{
					DefinitionsString += TEXT("// ");
					DefinitionsString += CommentLine;
					DefinitionsString += TEXT("\n");
				}
			}
...
void TNiagaraHlslTranslator<GraphBridge>::BuildMissingDefaults()
{
	...
	if (TranslationStages[ActiveStageIdx].ShouldDoSpawnOnlyLogic() || TranslationStages[ActiveStageIdx].bShouldUpdateInitialAttributeValues)
	{
		...
		for (FNiagaraVariable& Var : DeferredVariablesMissingDefault)
		{
			...
			bool bIsKnownMissingDefault = ParamMapHistories[ActiveStageIdx].Variables.Contains(Var);
			if (bIsKnownMissingDefault ||
				GNiagaraCompilationUseNewMissingDefaultTraversal == 0)
			{
			...
			{
				// Missing defaults that are unknown to the current parameter map need to be handled differently since their parameter history is unrelated to the
				// current stage.  Calling parameter map read in this case can result in calling compile on a pin with mismatched parameter map history and graph
				// pins which can result in compile errors or incorrect code generation.  If the unknown default can not be resolved we use the struct default.
				int32 MissingDefaultIdx = HandleReadUnknownMissingDefault(Var, DefaultPin);
				if (MissingDefaultIdx == INDEX_NONE)
				{
					FNiagaraVariable DefaultVar = Var;
					FNiagaraEditorUtilities::ResetVariableToDefaultValue(DefaultVar);
					MissingDefaultIdx = GetConstant(DefaultVar);
				}
				FString ParameterMapInstanceName = GetParameterMapInstanceName(ActiveStageIdx);
				AddBodyChunk(ParameterMapInstanceName + TEXT(".") + BuildSanitizedSymbolName(Var.GetName()), TEXT("{0}"), Var.GetType(), MissingDefaultIdx, false);
			}
		}
...
template<typename GraphBridge>
int32 TNiagaraHlslTranslator<GraphBridge>::HandleReadUnknownMissingDefault(const FNiagaraVariable& DefaultVar, const FInputPin* DefaultPin)
{
	// Validate inputs
	...
	const FNode* DefaultPinNode = GraphBridge::GetOwningNode(DefaultPin);
	if (DefaultPinNode == nullptr)
	{
		return INDEX_NONE;
	}
	const FGraph* DefaultPinGraph = GraphBridge::GetOwningGraph(DefaultPinNode);
	if (DefaultPinGraph == nullptr)
	{
		return INDEX_NONE;
	}

	// Get the default mode
	const FNiagaraVariableBase* AliasedVar = UniqueVarToAliasedVar.Find(DefaultVar);
	FNiagaraScriptVariableBinding DefaultBinding;
	TOptional<ENiagaraDefaultMode> DefaultMode = GraphBridge::GetGraphDefaultMode(DefaultPinGraph, AliasedVar ? *AliasedVar : FNiagaraVariableBase(DefaultVar), DefaultBinding);

	int32 DefaultChunkIndex = INDEX_NONE;
	if (DefaultMode == ENiagaraDefaultMode::Binding && DefaultBinding.IsValid())
	{
		// Bound defaults that can be handled are either local to the script being translated which will be in the UniqueVarToChunk map, or
		// are in an external constant namespace which can be handled safely with parameter map read.
		FNiagaraVariable BoundVar = FNiagaraVariable(DefaultVar.GetType(), DefaultBinding.GetName());
		int32* ExistingDefault = UniqueVarToChunk.Find(BoundVar);
		if (ExistingDefault != nullptr)
		{
			DefaultChunkIndex = *ExistingDefault;
		}
		else if (FNiagaraParameterUtilities::IsExternalConstantNamespace(BoundVar, CompileOptions.TargetUsage, CompileOptions.GetTargetUsageBitmask()))
		{
			HandleParameterRead(ActiveStageIdx, BoundVar, nullptr, nullptr, DefaultChunkIndex);
		}
	}
	else if (DefaultMode == ENiagaraDefaultMode::Custom)
	{
		// For custom defaults we only support linked parameter map gets.  First we handle the linked variable as if it was a
		// default binding, and if that doesn't work we recursively evaluate the linked default pin if it exists.
		const FOutputPin* LinkedOutputPin = GraphBridge::GetLinkedOutputPin(DefaultPin);
		if (LinkedOutputPin != nullptr)
		{
			const FNode* LinkedNode = GraphBridge::GetOwningNode(LinkedOutputPin);
			if (LinkedNode != nullptr)
			{
				const FParamMapGetNode* LinkedGetNode = GraphBridge::AsParamMapGetNode(LinkedNode);
				if (LinkedGetNode != nullptr)
				{
					FNiagaraVariable LinkedVar = GraphBridge::GetPinVariable(LinkedOutputPin, true, ENiagaraStructConversion::Simulation);
					int32* ExistingDefault = UniqueVarToChunk.Find(LinkedVar);
					if (ExistingDefault != nullptr)
					{
						DefaultChunkIndex = *ExistingDefault;
					}
					else if (FNiagaraParameterUtilities::IsExternalConstantNamespace(LinkedVar, CompileOptions.TargetUsage, CompileOptions.GetTargetUsageBitmask()))
					{
						HandleParameterRead(ActiveStageIdx, LinkedVar, nullptr, nullptr, DefaultChunkIndex);
					}
					else
					{
						const FInputPin* LinkedDefaultPin = GraphBridge::GetDefaultPin(LinkedGetNode, LinkedOutputPin);
						if (LinkedDefaultPin != nullptr)
						{
							DefaultChunkIndex = HandleReadUnknownMissingDefault(LinkedVar, LinkedDefaultPin);
						}
					}
				}
			}
	...
	if (DefaultChunkIndex == INDEX_NONE)
	{
		// If a default was not found through a binding or through custom traversal, check if the pin itself has a default.
		FNiagaraVariable PinVariable = GraphBridge::GetPinVariable(DefaultPin, true, ENiagaraStructConversion::Simulation);
		DefaultChunkIndex = PinVariable.IsDataAllocated() ? GetConstant(PinVariable) : INDEX_NONE;
	}

	return DefaultChunkIndex;
}
...

		{
			...
			for (int32 FoundIdx = 0; FoundIdx < FoundOutputNodes.Num(); FoundIdx++)
			{
				...
				if (HistoryOutputNode && GraphBridge::GetOutputNodeScriptType(HistoryOutputNode) == ENiagaraScriptUsage::ParticleSimulationStageScript)
				{
					...
					FNameBuilder StageNameBuilder;
					StageNameBuilder.AppendChar(TEXT('_'));
					AppendSanitizedFunctionNameSuffix(FNameBuilder(CompileSimStageData.StageName).ToView(), StageNameBuilder);
					...
					SimStage.PassNamespace = FString::Printf(TEXT("MapSimStage%d%s"), DestSimStageIndex, StageNameBuilder.ToString());
...
void TNiagaraHlslTranslator<GraphBridge>::HandleSimStageSetupAndTeardown(int32 InWhichStage, FString& OutHlsl)
{
	...
	if (CDO && CDO->CanExecuteOnTarget(ENiagaraSimTarget::GPUComputeSim))
	{
		...
		TranslationStage.CustomReadFunction = FString::Printf(TEXT("SetupFromIterationSource_%s"), *TranslationStage.PassNamespace);
		TranslationStage.CustomWriteFunction = FString::Printf(TEXT("TeardownFromIterationSource_%s"), *TranslationStage.PassNamespace);
		...
		if (bNeedsSetupAndTeardown)
		{
			...
			{
				Sig.Name = *FString::Printf(TEXT("SetupFromIterationSource_%s_GeneratedSetup"), *TranslationStage.PassNamespace);
			...
			{
				Sig.Name = *FString::Printf(TEXT("TeardownFromIterationSource_%s_GeneratedTeardown"), *TranslationStage.PassNamespace);
				FNiagaraFunctionSignature SignatureOut = Sig;
		...
		if (bNeedsAttributeRead)
		{
			...
			{
				Sig.Name = *FString::Printf(TEXT("SetupFromIterationSource_%s_GeneratedReadAttributes"), *TranslationStage.PassNamespace);
				FNiagaraFunctionSignature SignatureOut = Sig;
		...
		if (bNeedsAttributeWrite)
		{
			...
			{
				Sig.Name = *FString::Printf(TEXT("TeardownFromIterationSource_%s_GeneratedWriteAttributes"), *TranslationStage.PassNamespace);
				FNiagaraFunctionSignature SignatureOut = Sig;
...
void FNiagaraHlslTranslator::ConvertCompileInfoToParamInfo(const FNiagaraScriptDataInterfaceCompileInfo& Info, FNiagaraDataInterfaceGPUParamInfo& DIInstanceInfo, TArray<FNiagaraFunctionSignature>& GeneratedFunctionSignatures)
{
	...
	for (const FNiagaraFunctionSignature& OriginalSig : Info.RegisteredFunctions)
	{
		...
		SeenFunctions.FindOrAdd(OriginalSig, &bAlreadySeen);
		if (bAlreadySeen)
		{
			continue;
		}
...
void FNiagaraHlslTranslator::AppendSanitizedFunctionNameSuffix(FStringView Name, FStringBuilderBase& StringBuilder)
{
	FNameBuilder SanitizedName;

	{ // single character replacements
		const FStringView::ElementType UnderscoreKeys[] = { TEXT('.'), TEXT('\\'), TEXT('/'), TEXT(','), TEXT('-'), TEXT(':') };
		const FStringView::ElementType SkipKeys[] = { TEXT('\t'), TEXT(' ') };

		for (FStringView::ElementType InChar : Name)
		{
			if (!Algo::Contains(SkipKeys, InChar))
			{
				if (Algo::Contains(UnderscoreKeys, InChar))
				{
					SanitizedName.AppendChar(TEXT('_'));
				}
				else
				{
					SanitizedName.AppendChar(InChar);
				}
			}
		}
	}
	...
	{ // multi character replacements
		struct FReplacementPair
		{
			const TCHAR* Key = nullptr;
			const TCHAR* Value = nullptr;

			const int32 KeyLen = 0;
			const int32 ValueLen = 0;

			FReplacementPair(const TCHAR* InKey, const TCHAR* InValue)
			: Key(InKey)
			, Value(InValue)
			, KeyLen(FCString::Strlen(InKey))
			, ValueLen(FCString::Strlen(InValue))
			{}
		};

		const FReplacementPair MultiCharReplacements[] =
		{
			{ TEXT("__"), TEXT("ASC95ASC95") }
		};

		for (const FReplacementPair& Replacement : MultiCharReplacements)
		{
			int32 SearchPosition = 0;

			while (SearchPosition != INDEX_NONE)
			{
				SearchPosition = SanitizedName.ToView().Find(Replacement.Key, SearchPosition);
				if (SearchPosition != INDEX_NONE)
				{
					SanitizedName.ReplaceAt(SearchPosition, Replacement.KeyLen, Replacement.Value);
					SearchPosition += Replacement.ValueLen;
				}
			}
		}
	}

	NiagaraHlslTranslatorImpl::AppendConvertedToAscii(SanitizedName.ToView(), StringBuilder);
}
...
int32 TNiagaraHlslTranslator<GraphBridge>::GetRapidIterationParameter(const FNiagaraVariable& Parameter)
{
	...
	{
		if (FNiagaraStackGraphUtilities::IsRapidIterationType(Parameter.GetType()))
		{
	...
	else
	{
		...
		if (FoundIdx != INDEX_NONE)
		{
			...
			//UE_LOGF(LogNiagaraEditor, Display, "Converted parameter %ls to constant %ls for script %ls", *RapidIterationConstantVar.GetName().ToString(), *DebugConstantStr, *CompileOptions.FullName);
			return OutputChunkId;
		}
	}
	return INDEX_NONE;
		...
		else // These are the pins that we are setting on the parameter map.
		{
			...
			if (Var.IsDataInterface())
			{
				...
				if (ParamMapHistoryIdx < ParamMapHistories.Num() && ParamMapHistories[ParamMapHistoryIdx].IsPrimaryDataSetOutput(Var, CompileOptions.TargetUsage, bAllowDataInterfaces))
				{
					...
					{
						Error(FText::Format(LOCTEXT("ParameterMapDataInterfaceNotFoundErrorFormat", "Data interface could not be found for parameter map set.  Parameter: {0}"), FText::FromName(Var.GetName())), SetNode, Inputs[i].Pin);
						continue;
					}
			...
			else if ( Var.IsUObject() )
			{
				...
				if (ParamMapHistoryIdx < ParamMapHistories.Num() && ParamMapHistories[ParamMapHistoryIdx].IsPrimaryDataSetOutput(Var, CompileOptions.TargetUsage, bAllowDataInterfaces))
				{
					...
					{
						Error(FText::Format(LOCTEXT("ParameterMapUObjectNotFoundErrorFormat", "UObject could not be found for parameter map set.  Parameter: {0}"), FText::FromName(Var.GetName())), SetNode, Inputs[i].Pin);
						continue;
					}
...
void FNiagaraHlslTranslator::SetConstantByStaticVariable(int32& OutValue, const FNiagaraVariable& Var, FString* DebugString)
{
	...
	if (VarDefault.GetType().IsStatic())
	{
		...
		if (VarWithValue.IsDataAllocated())
		{
			...
			//UE_LOGF(LogNiagaraEditor, Log, "SetConstantByStaticVariable Added %ls (%d)", *Var.GetName().ToString(), OutValue);
		}
		else
		{
...
void TNiagaraHlslTranslator<GraphBridge>::Emitter(const FEmitterNode* EmitterNode, TArray<int32>& Inputs, TArray<int32>& Outputs)
{
...

	{
		...
		RegisterFunctionCall(ScriptUsage, Name, FName(FullName), EmitterNode->NodeGuid, GraphBridge::GetEmitterHandleIdString(EmitterNode), EmitterGraph, Signature, false, FString(), {}, Inputs, CallInputs, CallOutputs, Signature);
...
void TNiagaraHlslTranslator<GraphBridge>::HandleParameterRead(int32 ParamMapHistoryIdx, const FNiagaraVariable& InVar, const FInputPin* DefaultPin, const FNode* ErrorNode, int32& OutputChunkId, TOptional<ENiagaraDefaultMode> DefaultMode, TOptional<FNiagaraScriptVariableBinding> DefaultBinding, bool bTreatAsUnknownParameterMap, bool bIgnoreDefaultSetFirst)
{
	...
	else if (FNiagaraParameterUtilities::IsAliasedModuleParameter(Var) && ActiveHistoryForFunctionCalls.InTopLevelFunctionCall(CompileOptions.TargetUsage))
	{
		...
		else if (InputPin != nullptr && GraphBridge::GetLinkedOutputPin(InputPin) == nullptr && FNiagaraStackGraphUtilities::IsRapidIterationType(Var.GetType()))
		{
			bIsCandidateForRapidIteration = true;
		}
	}
		...
		if (LastSetChunkIdx == INDEX_NONE && bIsCandidateForRapidIteration)
		{
			...
			{		
				...
				if (FoundIdx != INDEX_NONE)
				{
					...
					UE_LOGF(LogNiagaraEditor, VeryVerbose, "Converted parameter %ls to constant %ls for script %ls", *Var.GetName().ToString(), *DebugConstantStr, *CompileOptions.FullName);
					return;
				}
				else if (InputPin != nullptr && !InputPin->bDefaultValueIsIgnored) // Use the default from the input pin because this variable was previously never encountered.
				{
					...
					UE_LOGF(LogNiagaraEditor, VeryVerbose, "Converted default value of parameter %ls to constant %ls for script %ls. Likely added since this system was last compiled.", *Var.GetName().ToString(), *DebugConstantStr, *CompileOptions.FullName);
					return;
				}
			...
			else if (InputPin != nullptr) // Default was found, trace back its inputs.
			{
				...
				if (LinkedOutputPin == nullptr && bIsCandidateForRapidIteration && !TranslationOptions.bParameterRapidIteration)
				{
					...
					if (FoundIdx != INDEX_NONE)
					{
						FString DebugConstantStr;
						OutputChunkId = GetConstant(TranslationOptions.OverrideModuleConstants[FoundIdx], &DebugConstantStr);
						UE_LOGF(LogNiagaraEditor, Display, "Converted parameter %ls to constant %ls for script %ls", *Var.GetName().ToString(), *DebugConstantStr, *CompileOptions.FullName);
						return;
					}
					else if (!InputPin->bDefaultValueIsIgnored) // Use the default from the input pin because this variable was previously never encountered.
					{
						...
						FString DebugConstantStr;
						OutputChunkId = GetConstant(PinVar, &DebugConstantStr);
						UE_LOGF(LogNiagaraEditor, Display, "Converted default value of parameter %ls to constant %ls for script %ls. Likely added since this system was last compiled.", *Var.GetName().ToString(), *DebugConstantStr, *CompileOptions.FullName);
						return;
					}
...
int32 TNiagaraHlslTranslator<GraphBridge>::RegisterDataInterface(const FNiagaraVariable& Var, const UNiagaraDataInterface* DataInterface, bool bPlaceholder, bool bAddParameterMapRead)
{
	...
	//UE_LOGF(LogNiagaraEditor, Log, "RegisterDataInterface %ls %ls %ls '%ls' %ls", *Var.GetName().ToString(), *Var.GetType().GetName(), *PathName,
...
void TNiagaraHlslTranslator<GraphBridge>::FunctionCall(const FFunctionCallNode* FunctionNode, TArray<int32>& Inputs, TArray<int32>& Outputs)
{
	...
	//UE_LOGF(LogNiagaraEditor, Log, "Function Call: %ls %d", *FunctionNode->GetFunctionName(), ActiveStageIdx);
	// We need the generated string to generate the proper signature for now.
	...
	RegisterFunctionCall(ScriptUsage, Name, FName(FullName), FunctionNode->NodeGuid, FString(), FunctionGraph, Signature, bCustomHlsl, CustomHlsl, CustomHlslIncludeFilePaths, Inputs, CallInputs, CallOutputs, OutputSignature);
			...
			for (int32 TokenIndex = 0; TokenIndex < Tokens.Num();)
			{
				...
				if (Info.UserPtrIdx != INDEX_NONE && CompilationTarget != ENiagaraSimTarget::GPUComputeSim)
				{
					...
					if (TokenIndex < Tokens.Num())
					{
						...
						// We only insert a comma if there are valid parameters, i.e. non-class, non-parameter map arugments
						// The first input argument is the instance data so we don't evaluate that
						// For example a impure function may have the instance data / exec pin / data interface as inputs, then an exec pin for output, none of which will translate to real parameters
						const auto IsRealHlslArg = [](const FNiagaraTypeDefinition& Type) { return !Type.GetClass() && Type != FNiagaraTypeDefinition::GetParameterMapDef(); };

						bool bHasRemainingArgs = false;
						for (int32 j = 1; j < Sig.Inputs.Num() && !bHasRemainingArgs; ++j)
						{
							bHasRemainingArgs = IsRealHlslArg(Sig.Inputs[j].GetType());
						}
						for (int32 j = 0; j < Sig.Outputs.Num() && !bHasRemainingArgs; ++j)
						{
							bHasRemainingArgs = IsRealHlslArg(Sig.Outputs[j].GetType());
						}

						if (bHasRemainingArgs)
						{
	...
	FNiagaraFunctionSignature& OutSignature, TArray<int32>& Inputs)
{
...

	{
		...
		for (const FPin* Pin : Pins)
		{
			...
			const uint32 PinVariableTypeHash = GetPersistentTypeHash(PinVariable);
...
void TNiagaraHlslTranslator<GraphBridge>::HandleDataInterfaceCall(FNiagaraScriptDataInterfaceCompileInfo& Info, const FNiagaraFunctionSignature& InMatchingSignature)
{
	...
	//UE_LOGF(LogNiagaraEditor, Log, "HandleDataInterfaceCall %d %ls %ls %ls", ActiveStageIdx, *InMatchingSignature.Name.ToString(), InMatchingSignature.bWriteFunction ? TEXT("true") : TEXT("False"), *Info.Name.ToString());
...
template<typename GraphBridge>
void TNiagaraHlslTranslator<GraphBridge>::RegisterFunctionCall(ENiagaraScriptUsage ScriptUsage, const FString& InName, FName InFullName, const FGuid& CallNodeId, const FString& InFunctionNameSuffix, const FGraph* SourceGraph,
                                                  const FNiagaraFunctionSignature& InSignature, bool bIsCustomHlsl, const FString& InCustomHlsl, const TArray<FNiagaraCustomHlslInclude>& InCustomHlslIncludeFilePaths, TArray<int32>& Inputs, TConstArrayView<const FInputPin*> CallInputs, TConstArrayView<const FOutputPin*> CallOutputs,
                                                  FNiagaraFunctionSignature& OutSignature)
{
	...
	if (SourceGraph)
	{
		...
		GenerateFunctionSignature(ScriptUsage, InName, InFullName, InFunctionNameSuffix, SourceGraph, CallInputs, Inputs, FunctionComment, OutSignature);
		...
		if (!FuncBody)
		{
...

			{
				...
				FunctionBody.Comments = MoveTemp(FunctionComment);
				...
				FunctionBody.StageWriteTargets = ActiveStageWriteTargets.Top();
				FunctionBody.StageReadTargets = ActiveStageReadTargets.Top();
			}
		...
		else
		{
			...
			for (const FName& Entry : FuncBody->StageWriteTargets)
			{
				const int32 SourceSimStage = TranslationStages[ActiveStageIdx].SimulationStageIndex;
			...
			for (const FName& Entry : FuncBody->StageReadTargets)
			{
				const int32 SourceSimStage = TranslationStages[ActiveStageIdx].SimulationStageIndex;
...
FString FNiagaraHlslTranslator::GetFunctionIncludeStatement(const FNiagaraCustomHlslInclude& Include)
{
	TStringBuilder<128> IncludeStatement;

	if (Include.bIsVirtual)
	{
		if ( !CheckVirtualShaderFilePath(Include.FilePath) )
		{
			Error(FText::FromString("Invalid virtual include " + Include.FilePath));
		}
		else
		{
		...
		while (bSearch)
		{
			if (InputCompileResult != INDEX_NONE)
			{
				if (CodeChunks.IsValidIndex(InputCompileResult))
				{
					...
					{
						//UE_LOGF(LogNiagaraEditor, Log, "CompilerTag %ls looking for uniform with symbol %ls", *Variable.GetName().ToString(), *CodeChunks[InputCompileResult].SymbolName);
						FString VarSymbol;
						for (const FNiagaraVariable& UniformVar : CompilationOutput.ScriptData.Parameters.Parameters)
						{
							...
							{
								//UE_LOGF(LogNiagaraEditor, Log, "Match Found: %ls == %ls", *VarSymbol, *UniformVar.GetName().ToString());
								Value = UniformVar.GetName().ToString();
								break;
							}
							else
							{
								//UE_LOGF(LogNiagaraEditor, Log, "No Match: %ls != %ls", *VarSymbol, *UniformVar.GetName().ToString());
							}
						}
