...
void FNiagaraDebugger::ExecConsoleCommand(const TCHAR* Cmd, bool bRequiresWorld)
{
	...
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Sending console command %ls. | Session: %ls | Instance: %ls |", Cmd, *Client.SessionId.ToString(), *Client.InstanceId.ToString());
...
void FNiagaraDebugger::UpdateDebugHUDSettings()
{
	...
	if (const UNiagaraDebugHUDSettings* Settings = GetDefault<UNiagaraDebugHUDSettings>())
	{
		...
		auto SendSettingsUpdate = [&](FNiagaraDebugger::FClientInfo& Client)
		{
			...
			UE_LOGF(LogNiagaraDebugger, Log, "Sending updated debug HUD settings. | Session: %ls | Instance: %ls |", *Client.SessionId.ToString(), *Client.InstanceId.ToString());
...
void FNiagaraDebugger::RequestUpdatedClientInfo()
{
	...
	auto RequestUpdate = [&](FNiagaraDebugger::FClientInfo& Client)
	{
		...
		UE_LOGF(LogNiagaraDebugger, Log, "Requesting updated simple client info. | Session: %ls | Instance: %ls |", *Client.SessionId.ToString(), *Client.InstanceId.ToString());
		MessageEndpoint->Send(Message, EMessageFlags::Reliable, nullptr, TArrayBuilder<FMessageAddress>().Add(Client.Address), FTimespan::Zero(), FDateTime::MaxValue());
	};
	...
	if (UNiagaraOutliner* Outliner = GetMutableDefault<UNiagaraOutliner>())
	{
		if(Outliner->CaptureSettings.bTriggerCapture)
		{
			...
			auto SendSettingsUpdate = [&](FNiagaraDebugger::FClientInfo& Client)
			{
				...
				UE_LOGF(LogNiagaraDebugger, Log, "Sending updated outliner settings. | Session: %ls | Instance: %ls |", *Client.SessionId.ToString(), *Client.InstanceId.ToString());
				MessageEndpoint->Send(Message, EMessageFlags::Reliable, nullptr, TArrayBuilder<FMessageAddress>().Add(Client.Address), FTimespan::Zero(), FDateTime::MaxValue());
			};
...
void FNiagaraDebugger::TriggerSimCacheCapture(FName ComponentName, int32 CaptureDelay, int32 CaptureFrames)
{
	...
	auto SendCaptureRequest = [&](FNiagaraDebugger::FClientInfo& Client)
	{
		...
		UE_LOGF(LogNiagaraDebugger, Log, "Sending request to capture sim cache for component %ls. | Session: %ls | Instance: %ls |", *ComponentName.ToString(), *Client.SessionId.ToString(), *Client.InstanceId.ToString());
		MessageEndpoint->Send(Message, EMessageFlags::Reliable, nullptr, TArrayBuilder<FMessageAddress>().Add(Client.Address), FTimespan::Zero(), FDateTime::MaxValue());
	};
...
void FNiagaraDebugger::SessionManager_OnSessionSelectionChanged(const TSharedPtr<ISessionInfo>& Session)
{
	...
	if (Session.IsValid())
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Session selection changed. Dropping connections not from this session. | Session: %ls (%ls)", *Session->GetSessionId().ToString(), *Session->GetSessionName());
	...
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Session selection changed. Dropping all connections.");
...
void FNiagaraDebugger::SessionManager_OnInstanceSelectionChanged(const TSharedPtr<ISessionInstanceInfo>& Instance, bool Selected)
{
	...
	if (MessageEndpoint.IsValid())
	{
		if (Selected)
		{
			...
			{
				UE_LOGF(LogNiagaraDebugger, Log, "Session Instance selection callback for existing active connection. Ignored. | Session: %ls | Instance: %ls (%ls).", *SessionId.ToString(), *InstanceId.ToString(), *Instance->GetInstanceName());
				return;
			}
			...
			{
				UE_LOGF(LogNiagaraDebugger, Log, "Session Instance selection callback for existing pending connection. Ignored. | Session: %ls | Instance: %ls (%ls).", *SessionId.ToString(), *InstanceId.ToString(), *Instance->GetInstanceName());
				return;
			}
			...
			UE_LOGF(LogNiagaraDebugger, Log, "Establishing connection. | Session: %ls | Instance: %ls (%ls).", *SessionId.ToString(), *InstanceId.ToString(), *Instance->GetInstanceName());
...
void FNiagaraDebugger::CloseConnection(FGuid SessionId, FGuid InstanceId)
{
	...
	if (FoundPending != INDEX_NONE)
	{
		...
		UE_LOGF(LogNiagaraDebugger, Log, "Closing pending connection. | Session: %ls | Instance: %ls |", *SessionId.ToString(), *InstanceId.ToString());
	...
	if (FoundActive != INDEX_NONE)
	{
		...
		UE_LOGF(LogNiagaraDebugger, Log, "Closing active connection. | Session: %ls | Instance: %ls |", *SessionId.ToString(), *InstanceId.ToString());
...
void FNiagaraDebugger::HandleConnectionAcceptedMessage(const FNiagaraDebuggerAcceptConnection& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	if (FoundActive != INDEX_NONE)
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Received connection accepted message from an already connected client. Ignored. | Session: %ls | Instance: %ls |", *Message.SessionId.ToString(), *Message.InstanceId.ToString());
		return;
	}
	...
	if (FoundPending != INDEX_NONE)
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Connection accepted. | Session: %ls | Instance: %ls |", *Message.SessionId.ToString(), *Message.InstanceId.ToString());
		PendingClients.RemoveAtSwap(FoundPending);
	...
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Received connection accepted message from a client that is not in our pending list. Ignored. | Session: %ls | Instance: %ls |", *Message.SessionId.ToString(), *Message.InstanceId.ToString());
	}
}
...
void FNiagaraDebugger::HandleConnectionClosedMessage(const FNiagaraDebuggerConnectionClosed& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	if (FoundPending != INDEX_NONE)
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Pending connection closed by the client. | Session: %ls | Instance: %ls |", *Message.SessionId.ToString(), *Message.InstanceId.ToString());
		OnConnectionClosedDelegate.Broadcast(PendingClients[FoundPending]);
		PendingClients.RemoveAtSwap(FoundPending);
	}
	if (FoundActive != INDEX_NONE)
	{
		UE_LOGF(LogNiagaraDebugger, Log, "Active connection closed by the client. | Session: %ls | Instance: %ls |", *Message.SessionId.ToString(), *Message.InstanceId.ToString());
...
{
	UE_LOGF(LogNiagaraDebugger, Log, "Received outliner update. | Sender: %ls ", *Context->GetSender().ToString());
	if(UNiagaraOutliner* Outliner = GetOutliner())
	{
...
{
	UE_LOGF(LogNiagaraDebugger, Log, "Received simple client info update. | Sender: %ls ", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogNiagaraDebugger, Log, "Received Niagara Sim Cache Capture Reply. | Sender: %ls ", *Context->GetSender().ToString());
