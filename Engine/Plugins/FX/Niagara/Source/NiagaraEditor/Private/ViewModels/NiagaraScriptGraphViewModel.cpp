...
void FNiagaraScriptGraphViewModel::CopySelectedNodes()
{
	...
	for(FNiagaraVariable& Var : Variables)
	{
		...
		{
			UE_LOGF(LogNiagaraEditor, Log, "Variable %ls was encountered as parameter during a copy nodes operation, but the corresponding script variable could not be found.", *(Var.GetName().ToString()));
		}
	}
