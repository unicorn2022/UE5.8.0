...
void UNiagaraStackRendererItem::OpenSourceCodeLocation() const
{
	if (UNiagaraRendererProperties* RendererProps = RendererProperties.Get())
	{
		FSourceCodeNavigation::NavigateToClass(RendererProps->GetClass());
	}
}
...
{
	ClipboardContent->Renderers.Add(UNiagaraClipboardRenderer::CreateRenderer(ClipboardContent, RendererProperties.Get(), GetStackNoteData(), GetAlternateDisplayName()));
}
...
void UNiagaraStackRendererItem::RefreshIssues(TArray<FStackIssue>& NewIssues)
{
	...
	if (RendererProperties->GetIsEnabled())
	{
		...
		// Query feedback from the renderer
		RendererProperties->GetRendererFeedback(GetEmitterViewModel()->GetEmitter(), Errors, Warnings, Infos);

		// Query feedback from the material validation system
		UMaterialEditorValidationSubsystem* MaterialValidationSubsystem = GEditor ? GEditor->GetEditorSubsystem<UMaterialEditorValidationSubsystem>() : nullptr;
		if (MaterialValidationSubsystem)
		{
			TArray<UMaterialInterface*> RendererMaterials;
			RendererProperties->GetUsedMaterials(nullptr, RendererMaterials);
			...
			for (UMaterialInterface* Material : RendererMaterials)
			{
				...
				FMaterialEditorValidatorContext MaterialValidatorContext;
				MaterialValidationSubsystem->Validate(Material, MaterialValidatorContext);
				if (MaterialValidatorContext.Message.IsEmpty() == false)
				{
					Warnings.Emplace(
						MaterialValidatorContext.Message,
						FText::Format(LOCTEXT("MaterialValidationMessage", "Material '{0}' has validation issues."), FText::FromName(Material->GetFName()))
					);
				}
			}
		}

		// Convert feedback into stack issues
...
TArray<FNiagaraStackItemCustomIcon> UNiagaraStackRendererItem::GetCustomIcons() const
{
	TArray<FNiagaraStackItemCustomIcon> CustomIcons;
	...
	// Gather all our material parameter bindings
	TArray<FNiagaraRendererMaterialParameters*> MaterialParameters;
	for (TFieldIterator<FStructProperty> PropIt(Renderer->GetClass()); PropIt; ++PropIt)
	{
		FStructProperty* StructProp = *PropIt;
		if (!StructProp || !StructProp->Struct || !StructProp->Struct->IsChildOf(FNiagaraRendererMaterialParameters::StaticStruct()))
		{
			continue;
		}

		MaterialParameters.Add(StructProp->ContainerPtrToValuePtr<FNiagaraRendererMaterialParameters>(Renderer));
	}

	if (MaterialParameters.Num() > 0)
	{
		FNiagaraStackItemCustomIcon& Icon = CustomIcons.AddDefaulted_GetRef();

		Icon.IconBrush			= FNiagaraEditorStyle::Get().GetBrush(TEXT("NiagaraEditor.Overview.HasMaterialParameterBindings"));
		Icon.ToolTipText		= LOCTEXT("BoundMaterialParametersTooltip", "Renderer has bound material parameters.  See the bindings section on the renderer for more details.");
		Icon.VisibleFunction	= TAttribute<EVisibility>::CreateLambda(
			[MaterialParameters, WeakRenderer=RendererProperties]()
			{
				if (WeakRenderer.Get())
				{
					for (FNiagaraRendererMaterialParameters* Parameters : MaterialParameters)
					{
						if (Parameters->HasAnyBindings())
						{
