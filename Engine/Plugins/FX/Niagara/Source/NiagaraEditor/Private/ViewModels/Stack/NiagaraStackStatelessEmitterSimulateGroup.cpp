...
#include "ViewModels/Stack/NiagaraStackStatelessCommon.h"
...
void UNiagaraStackStatelessModuleItem::RefreshChildrenInternal(const TArray<UNiagaraStackEntry*>& CurrentChildren, TArray<UNiagaraStackEntry*>& NewChildren, TArray<FStackIssue>& NewIssues)
{
	...
	if (StatelessModule != nullptr)
	{
		...
		{
			const bool bIsInTopLevelObject = true;
			const bool bHideTopLevelCategories = FNiagaraStackStatelessCommon::ShouldHideTopLevelCategories(StatelessModule->GetClass());
		...
		if (bGeneratedHeaderValueHandlers == false)
		{
			...
			if (StatelessModule->CanDebugDraw())
			{
				...
				if ( FBoolProperty* DebugDrawProperty = GetDebugDrawProperty() )
				{
...
bool UNiagaraStackStatelessModuleItem::SupportsDebugDraw() const
{
	UNiagaraStatelessModule* StatelessModule = StatelessModuleWeak.Get();
	return StatelessModule ? StatelessModule->CanDebugDraw() : false;
}

TOptional<FText> UNiagaraStackStatelessModuleItem::GetDebugDrawTooltip() const
{
	TOptional<FText> Tooltip;
	if (FBoolProperty* DebugDrawProperty = GetDebugDrawProperty())
	{
		Tooltip = DebugDrawProperty->GetToolTipText();
	}
	return Tooltip;
}

bool UNiagaraStackStatelessModuleItem::IsDebugDrawEnabled() const
{
	UNiagaraStatelessModule* StatelessModule = StatelessModuleWeak.Get();
	return StatelessModule ? StatelessModule->IsDebugDrawEnabled() : false;
}

void UNiagaraStackStatelessModuleItem::SetDebugDrawEnabled(bool bInEnabled)
{
	UNiagaraStatelessModule* StatelessModule = StatelessModuleWeak.Get();
	FBoolProperty* DebugDrawProperty = GetDebugDrawProperty();
	if (StatelessModule && DebugDrawProperty)
	{
		FScopedTransaction ScopedTransaction(FText::Format(LOCTEXT("SetDebugDrawEnabled", "Change {0}"), DebugDrawProperty->GetDisplayNameText()));
		StatelessModule->Modify();

		DebugDrawProperty->SetPropertyValue(DebugDrawProperty->ContainerPtrToValuePtr<void>(StatelessModule), bInEnabled);
		
		FPropertyChangedEvent PropertyChangedEvent(DebugDrawProperty);
		StatelessModule->PostEditChangeProperty(PropertyChangedEvent);
...
const UNiagaraStackEntry::FCollectedUsageData& UNiagaraStackStatelessModuleItem::GetCollectedUsageData() const
{
	if (CachedCollectedUsageData.IsSet() == false)
	{
		...
		const UNiagaraStatelessModule* StatelessModule = StatelessModuleWeak.Get();
		const UNiagaraStatelessEmitter* StatelessEmitter = StatelessModule ? StatelessModule->GetTypedOuter<const UNiagaraStatelessEmitter>() : nullptr;
		if ( ParamVM && StatelessModule && StatelessEmitter )
		{
			StatelessEmitter->GatherSimulationAttributeReferences(
				StatelessModule,
				[this, &ParamVM](FName WhereForm, const FNiagaraVariableBase& Variable, bool bRead, bool bWrite)
				{
					if (ParamVM->IsVariableSelected(Variable))
					{
						CachedCollectedUsageData.GetValue().bHasReferencedParameterRead |= bRead;
						CachedCollectedUsageData.GetValue().bHasReferencedParameterWrite |= bWrite;
					}
				}
			);
		}
...
FBoolProperty* UNiagaraStackStatelessModuleItem::GetDebugDrawProperty() const
{
	UNiagaraStatelessModule* StatelessModule = StatelessModuleWeak.Get();
	if (StatelessModule == nullptr)
	{
	...
	UClass* ModuleClass = StatelessModule->GetClass();
	return CastField<FBoolProperty>(ModuleClass->FindPropertyByName(UNiagaraStatelessModule::PrivateMemberNames::bDebugDrawEnabled));
}
