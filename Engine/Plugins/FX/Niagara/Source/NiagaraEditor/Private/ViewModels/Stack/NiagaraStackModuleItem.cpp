...
void UNiagaraStackModuleItem::ReassignModuleScript(UNiagaraScript* ModuleScript)
{
		...
		TEXT("Can not reassign the module script when the module isn't a valid function call module.")))
	{
		...
		if (OldScriptData->bUsePythonScriptConversion && bIsPythonAvailable && OldClipboardContent != nullptr)
		{
		...
		else if (NewScriptData->ConversionUtility != nullptr && OldClipboardContent != nullptr)
		{
			...
			if (ConversionUtility)
			{
				...
				if (ConversionUtility->UsesLegacyConversion())
				{
					...
					ConversionUtility->Convert(OldScript, OldClipboardContent, ModuleScript, InputRoot, NewClipboardContent, FunctionCallNode, ConvertMessage);
					PRAGMA_ENABLE_DEPRECATION_WARNINGS
				}
				else if (ConversionUtility->ConvertInputs(OldScript, OldClipboardContent, ModuleScript, NewClipboardContent, ConvertMessage))
				{
