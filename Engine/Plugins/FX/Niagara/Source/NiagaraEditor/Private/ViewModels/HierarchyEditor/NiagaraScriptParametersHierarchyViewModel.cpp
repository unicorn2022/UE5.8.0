...
TSharedRef<FHierarchyDragDropOp> UNiagaraScriptParametersHierarchyViewModel::CreateDragDropOp(const FDataHierarchyDragDropContext& DragContext)
{
	TSharedRef<FNiagaraHierarchyScriptParameterDragDropOp> DragDropOp = MakeShared<FNiagaraHierarchyScriptParameterDragDropOp>(DragContext);
...
void UNiagaraScriptParametersHierarchyViewModel::OnParametersChanged(TOptional<TInstancedStruct<FNiagaraParametersChangedData>> ParametersChangedData)
{
	if(ParametersChangedData.IsSet())
	{
		...
		if(ChangedData.GetScriptStruct() == FNiagaraParameterRenamedData::StaticStruct())
		{
			...
			const UNiagaraScriptVariable* OldScriptVariable = RenamedData.OldScriptVariable.Get();
			const UNiagaraScriptVariable* NewScriptVariable = RenamedData.NewScriptVariable.Get();
			if (ensureMsgf(OldScriptVariable && NewScriptVariable, TEXT("UNiagaraScriptParametersHierarchyViewModel::OnParametersChanged - OldVariable(%p) NewVariable(%p)"), OldScriptVariable, NewScriptVariable))
			{
				if(OldScriptVariable->GetIsStaticSwitch() && NewScriptVariable->GetIsStaticSwitch())
				{
					...
					UNiagaraHierarchyScriptParameter** FoundHierarchyScriptParameter = AllHierarchyScriptParameters.FindByPredicate([Guid = OldScriptVariable->Metadata.GetVariableGuid()](UNiagaraHierarchyScriptParameter* Candidate)
					{
					...
					if(FoundHierarchyScriptParameter)
					{
						...
						(*FoundHierarchyScriptParameter)->Initialize(*NewScriptVariable);
					}
				}
			}
		}
...
TSharedRef<SWidget> FNiagaraHierarchyScriptParameterDragDropOp::CreateCustomDecorator() const
{
	...
	{
		if(const UNiagaraHierarchyScriptParameter* ScriptParameter = Cast<UNiagaraHierarchyScriptParameter>(TopLevelEntries[0].Element.Pin()->GetData()))
		{
	...
	for(const FDraggedElementEntry& Entry : TopLevelEntries)
	{
		...
		{
			if(const UNiagaraHierarchyScriptParameter* ScriptParameter = Cast<UNiagaraHierarchyScriptParameter>(Entry.Element.Pin()->GetData()))
			{
				TOptional<FNiagaraVariable> Variable = ScriptParameter->GetVariable();
				if(Variable.IsSet())
				{
					...
					.Padding(1.f)
					[
						FNiagaraParameterUtilities::GetParameterWidget(Variable.GetValue(), false, false)
					];
				}
			}
		}
