...
{
	if(OnGetWorkflowModeDelegate.IsBound())
	{
...
{
	if(OnChangeWorkflowModeDelegate.IsBound())
	{
...
void FNiagaraSystemViewModel::NotifyPreSave()
{
	...
	{
		UE_LOGF(LogNiagaraEditor, Log, "System %ls has pending compile jobs. Waiting for that code to complete before Saving..", *System->GetName());
...
void FNiagaraSystemViewModel::NotifyPreClose()
{
	...
	{
		UE_LOGF(LogNiagaraEditor, Log, "System %ls has pending compile jobs. Waiting for that code to complete before Closing..", *GetSystem().GetName());
...
const TArray<FNiagaraStackModuleData>& FNiagaraSystemViewModel::GetStackModuleDataByModuleEntry(const UNiagaraStackEntry* ModuleEntry)
{
...
void FNiagaraSystemViewModel::AddParameter(const FNiagaraVariableBase& InUserParameter)
{
	FScopedTransaction RenameParameterTransaction(LOCTEXT("AddParameterTransaction", "Add parameter"));
	System->Modify();
	System->GetExposedParameters().AddParameter(InUserParameter);
}
...
bool FNiagaraSystemViewModel::DeleteParameter(const FNiagaraVariableBase Parameter)
{
	...
	{
		if (UNiagaraEditorParametersAdapter* EditorParametersAdapter = GetEditorOnlyParametersAdapter())
		{
			auto MatchingParameter = [&Parameter](const UNiagaraScriptVariable* ScriptVariable) -> bool
			{
				return ScriptVariable && ScriptVariable->Variable.IsEquivalent(Parameter);
			};

			EditorParametersAdapter->Modify();
			EditorParametersAdapter->GetParameters().RemoveAll(MatchingParameter);
		}
	}
...
bool FNiagaraSystemViewModel::SetParameterDefaultValue(const FNiagaraVariableBase& InUserParameter, const FNiagaraVariant& InDefaultValue)
{
	int32 ParameterOffset = System->GetExposedParameters().IndexOf(InUserParameter);
	if (ParameterOffset == INDEX_NONE)
	{
		return false;
	}

	const FNiagaraTypeDefinition& ParameterType = InUserParameter.GetType();
	if (ParameterType.IsDataInterface() && (InDefaultValue.GetDataInterface() == nullptr || InDefaultValue.GetDataInterface()->GetClass() != ParameterType.GetClass()))
	{
		return false;
	}
	if (ParameterType.GetStruct() != nullptr && InDefaultValue.GetNumBytes() != ParameterType.GetSize())
	{
		return false;
	}

	FScopedTransaction Transaction(LOCTEXT("SetUserParameterDefaultValueTransaction", "Set User Parameter Default"));
	System->Modify();
	if (ParameterType.IsDataInterface())
	{
		UNiagaraDataInterface* DataInterface = System->GetExposedParameters().GetDataInterface(ParameterOffset);
		DataInterface->Modify();
		InDefaultValue.GetDataInterface()->CopyTo(DataInterface);
	}
	else if (ParameterType.IsUObject())
	{
		System->GetExposedParameters().SetUObject(InDefaultValue.GetUObject(), ParameterOffset);
	}
	else
	{
		if (InUserParameter.GetType() == FNiagaraTypeDefinition::GetPositionDef() && InDefaultValue.GetNumBytes() == sizeof(FVector))
		{
			FVector* PositionVector = (FVector*)InDefaultValue.GetBytes();
			System->GetExposedParameters().SetPositionParameterValue(*PositionVector, InUserParameter.GetName());
		}
		else
		{
			System->GetExposedParameters().SetParameterData(InDefaultValue.GetBytes(), ParameterOffset, InDefaultValue.GetNumBytes());
		}
	}
	return true;
}
		...
		for (UNiagaraNodeOutput* OutputNode : OutputNodes)
		{
			...
			*Builder.ConstantResolver = ConstantResolver;
			...
			if (Builder.Histories.Num() != 1)
			{
				...
				UE_LOGF(LogNiagaraEditor, Warning, "Encountered more than one parameter map history when collecting graph parameter reference collections for system parameter panel view model!");
			}
...
void FNiagaraSystemViewModel::NotifyObjectAssetChanged(UNiagaraNode& OwningNode, FName VariableReadName, UObject* NewValue)
{
	...
	if (ensureMsgf(CompiledScripts.Num() > 0, TEXT("Could not find compiled scripts for UObject input node.")))
	{
		...
		if ( EmitterHandle != nullptr )
		{
			...
			EmitterHandleId = EmitterHandle->GetId();
		}
		...
		for (const UNiagaraScript* CompiledScript : CompiledScripts)
		{
			for (const FNiagaraScriptUObjectCompileInfo& CompileInfo : CompiledScript->GetCachedDefaultUObjects())
			{
				...
				const FNiagaraScriptRuntimeData* ScriptRuntimeData = NiagaraSystem.GetScriptRuntimeData(EmitterHandle, *CompiledScript).Get();
				if (ScriptRuntimeData == nullptr)
				{
					continue;
				}

				for (const FNiagaraResolvedUObjectInfo& ResolvedInfo : ScriptRuntimeData->GetResolvedUObjects())
				{
