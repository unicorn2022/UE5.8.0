...
{
	TSharedPtr<FHierarchyCategoryViewModel> CategoryViewModel = CategoryViewModelWeakPtr.Pin();
	if (ensure(CategoryViewModel.IsValid()))
	{
		return CategoryViewModel->GetCategoryName();
	}
	return FText();
}
		...
		if(UNiagaraHierarchyModuleInput* ModuleInput = Cast<UNiagaraHierarchyModuleInput>(Data))
		{
			if(GetEmitterViewModel().IsValid())
			{
				...
				if(ModuleBaseInputData.IsSet())
				{
					...
					{
						UNiagaraNodeFunctionCall* FunctionCallNode = CurrentInput->GetInputFunctionCallNodePtr();
						return FunctionCallNode != nullptr
							&& CurrentInput->GetInputParameterHandle() == ModuleBaseInputData->InputName && CurrentInput->GetInputType() == ModuleBaseInputData->Type
							...
							&& FunctionCallNode->NodeGuid == OwningFunctionCallNode->NodeGuid;
					});
					...
					for(UNiagaraHierarchyModuleInput* ChildInput : ChildInputs)
					{
						...
						if(ChildInputData.IsSet())
						{
							...
							UNiagaraStackFunctionInput* StackChildInput = FindCurrentChildOfTypeByPredicate<UNiagaraStackFunctionInput>(CurrentChildren, [&](UNiagaraStackFunctionInput* CurrentInput)
							{
								UNiagaraNodeFunctionCall* FunctionCallNode = CurrentInput->GetInputFunctionCallNodePtr();
								return FunctionCallNode != nullptr
									&& CurrentInput->GetInputParameterHandle() == ChildInputData->InputName && CurrentInput->GetInputType() == ChildInputData->Type
									&& CurrentInput->GetInputFunctionCallInitialScript() == ChildFunctionNode->FunctionScript
									// we are checking for node guid as we could have 2x the same input from 2 identical modules
									&& FunctionCallNode->NodeGuid == ChildFunctionNode->NodeGuid;
							});
...
bool UNiagaraStackSummaryCategory::IsTopLevelCategory() const
{
	...
	if (ensure(CategoryViewModel.IsValid()))
	{
		return CategoryViewModel->GetData()->GetOuter()->IsA<UHierarchyRoot>();
	}
	return false;
}
...
FText UNiagaraStackSummaryCategory::GetTooltipText() const
{
	...
	if (ensure(CategoryViewModel.IsValid()))
	{
		if (const UHierarchyCategory* Category = Cast<const UHierarchyCategory>(CategoryViewModel->GetData()))
		{
