...
void FNiagaraSystemToolkitParameterPanelViewModel::DeleteParameters(const TArray<FNiagaraParameterPanelItem>& ItemsToDelete) 
{
	...
	else
	{
		for (const FNiagaraParameterPanelItem& ItemToDelete : ItemsToDelete)
		{
			...
			if (SystemViewModel->DeleteParameter(ItemToDelete.GetVariable()))
			{
				bAnyChange = true;
			}
		}
...
TSharedPtr<SWidget> FNiagaraSystemToolkitParameterPanelViewModel::CreateContextMenuForItems(const TArray<FNiagaraParameterPanelItem>& Items, const TSharedPtr<FUICommandList>& ToolkitCommands)
{
	...
	if (Items.Num() == 1)
	{
		...
		MenuBuilder.BeginSection("Edit", LOCTEXT("EditMenuHeader", "Edit"));
		{
			...
			MenuBuilder.AddMenuEntry(FNiagaraParameterPanelCommands::Get().DeleteItem, NAME_None, TAttribute<FText>(), DeleteToolTip, FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Delete"));
...
TArray<FNiagaraParameterPanelItem> FNiagaraSystemToolkitParameterPanelViewModel::GetViewedParameterItems() const
{
	...
	UNiagaraSystemSelectionViewModel* SelectionViewModel = SystemViewModel->GetSelectionViewModel();
	if(SelectionViewModel->GetSystemIsSelected() == false && SelectionViewModel->GetSelectedEmitterHandleIds().Num() == 0)
	{	
	...
	if (bForceSystem)
	{
			...
			for (UNiagaraNodeOutput* OutputNode : OutputNodes)
			{
				...
				*Builder.ConstantResolver = ConstantResolver;
				...
				auto AddReadReference = [GraphToCheck](const FNiagaraParameterMapHistory::FReadHistory& History, FNiagaraParameterPanelItem& Item)
				{
					...
					Source.SourceObject = GraphToCheck;
				...
				auto AddWriteReference = [GraphToCheck](const FNiagaraParameterMapHistory::FModuleScopedPin& History, FNiagaraParameterPanelItem& Item)
				{
					...
					Item.AddToWriteCount(FNiagaraParameterReferencePath(GraphToCheck, History.ModuleName));
				};
				...
				for (int32 VariableIndex = 0; VariableIndex < Builder.Histories[0].Variables.Num(); VariableIndex++)
				{
					...
					FNiagaraParameterPanelItem* ItemPtr = VisitedParameterToItemMap.Find(Var);
					...
					if (ItemPtr == nullptr)
					{
						...
						ItemPtr = &VisitedParameterToItemMap.Add(Var, Item);
					}

					if (!ItemPtr->bExternallyReferenced)
					{
						...
						ItemPtr->bExternallyReferenced = !bVarOnlyInTopLevelGraph;
					}
		...
		// Stateless emitter usage
		for (const TSharedRef<FNiagaraEmitterHandleViewModel>& EmitterViewModel : SystemViewModel->GetEmitterHandleViewModels())
		{
			...
			if (SelectionViewModel->GetSelectedEmitterHandleIds().Num() != 0)
			{
				if (!SelectionViewModel->GetSelectedEmitterHandleIds().Contains(EmitterHandle->GetId()))
				{
					continue;
				}
			}

			StatelessEmitter->GatherSimulationAttributeReferences(
				[&](FName OwnerName, const FNiagaraVariableBase& Variable, bool bRead, bool bWrite)
				{
					if (VisitedInvalidParameters.Contains(Variable))
					{
						return;
					}

					FNiagaraParameterPanelItem* ItemPtr = VisitedParameterToItemMap.Find(Variable);
					if (ItemPtr == nullptr)
					{
						UNiagaraScriptVariable* ScriptVariable = NewObject<UNiagaraScriptVariable>(GetTransientPackage());
						ScriptVariable->AddToRoot();
						ScriptVariable->Init(Variable, FNiagaraVariableMetaData());
						TransientParameterToScriptVarMap.Add(Variable, ScriptVariable);

						FNiagaraNamespaceMetadata CandidateNamespaceMetaData = FNiagaraEditorUtilities::GetNamespaceMetaDataForVariableName(Variable.GetName());
						if (CachedCurrentCategories.Contains(FNiagaraParameterPanelCategory(CandidateNamespaceMetaData)) == false)
						{
							VisitedInvalidParameters.Add(Variable);
							return;
						}
						...
						Item.NamespaceMetaData = CandidateNamespaceMetaData;
						ItemPtr = &VisitedParameterToItemMap.Add(Variable, Item);
					}

					if (bRead)
					{
						ItemPtr->AddToReadCount(FNiagaraParameterReferencePath(StatelessEmitter, OwnerName));
					}
					if (bWrite)
					{
						ItemPtr->AddToWriteCount(FNiagaraParameterReferencePath(StatelessEmitter, OwnerName));
					}
				}
			);
		}
		...
		for (TSharedRef<FNiagaraEmitterHandleViewModel> EmitterVMS : SystemViewModel->GetEmitterHandleViewModels())
		{
			...
			FNiagaraEmitterHandle* EmitterHandle = EmitterVMS.Get().GetEmitterHandle();
			UObject* SourceObject = EmitterHandle->GetEmitterBase();
			if (const FVersionedNiagaraEmitterData* EmitterData = EmitterVMS.Get().GetEmitterHandle()->GetEmitterData())
			{
				if (UNiagaraScriptSource* ScriptSource = Cast<UNiagaraScriptSource>(EmitterData->GraphSource))
				{
					SourceObject = ScriptSource->NodeGraph;
				}
			}

			FNiagaraAliasContext ResolveAliasesContext(FNiagaraAliasContext::ERapidIterationParameterMode::EmitterOrParticleScript);
			ResolveAliasesContext.ChangeEmitterNameToEmitter(EmitterHandle->GetUniqueInstanceName());

			EmitterHandle->ForEachEnabledRendererWithIndex(
				[&](const UNiagaraRendererProperties* RenderProperties, int32 RendererIndex)
				{
					TArray<FNiagaraVariable> BoundAttributes = RenderProperties->GetBoundAttributes();

					// Find attributes first
					for (const FNiagaraVariableAttributeBinding* AttributeBinding : RenderProperties->GetAttributeBindings())
					{
						const FNiagaraVariable BoundAttribute = RenderProperties->GetBoundAttribute(AttributeBinding);
						if (BoundAttribute.IsValid() == false)
						{
							continue;
						}
						BoundAttributes.RemoveSwap(BoundAttribute, EAllowShrinking::No);

						const FNiagaraVariable ResolvedAttribute = FNiagaraUtilities::ResolveAliases(BoundAttribute, ResolveAliasesContext);
						if (FNiagaraParameterPanelItem* ItemPtr = VisitedParameterToItemMap.Find(ResolvedAttribute))
						{
							const FName SourceModuleName(RenderProperties->GetWidgetDisplayName().ToString() + " (" + AttributeBinding->GetDataSetBindableVariable().GetName().ToString() + " Binding)");
							ItemPtr->AddToReadCount(FNiagaraParameterReferencePath(SourceObject, SourceModuleName));
						}
					}

					// Add remaining bindings
					for (const FNiagaraVariable& BoundAttribute : BoundAttributes)
					{
						if (BoundAttribute.IsValid() == false)
						{
							continue;
						...
						if (FNiagaraParameterPanelItem* ItemPtr = VisitedParameterToItemMap.Find(ResolvedAttribute))
						{
							const FName SourceModuleName(*RenderProperties->GetWidgetDisplayName().ToString());
							ItemPtr->AddToReadCount(FNiagaraParameterReferencePath(SourceObject, SourceModuleName));
						}
					}
				}
		...
		// Find all data interfaces bindings
		{
			TSet<UNiagaraDataInterface*> DiscoveredDIs;
			const auto FindDataInterfaceBindings =
				[&](const UObject* Context, UNiagaraDataInterface* DataInterface, const FNiagaraVariableBase& Variable)
				{
					if (!DataInterface || DiscoveredDIs.Contains(DataInterface))
					{
						return true;
					}
					DiscoveredDIs.Add(DataInterface);

					for (TFieldIterator<FStructProperty> PropertyIt(DataInterface->GetClass()); PropertyIt; ++PropertyIt)
					{
						...
						if (StructProperty->Struct->IsChildOf(FNiagaraUserParameterBinding::StaticStruct()))
						{
							FNiagaraUserParameterBinding* ParameterBinding = StructProperty->ContainerPtrToValuePtr<FNiagaraUserParameterBinding>(DataInterface);
							if (ParameterBinding->Parameter.IsValid())
							{
								if (FNiagaraParameterPanelItem* ItemPtr = VisitedParameterToItemMap.Find(ParameterBinding->Parameter))
								{
									ItemPtr->AddToReadCount(FNiagaraParameterReferencePath(Context, Variable.GetName()));
								}
							}
						}
						else if (StructProperty->Struct->IsChildOf(FNiagaraParameterBinding::StaticStruct()))
						{
							FNiagaraParameterBinding* ParameterBinding = StructProperty->ContainerPtrToValuePtr<FNiagaraParameterBinding>(DataInterface);
							if (ParameterBinding->ResolvedParameter.IsValid())
							{
								if (FNiagaraParameterPanelItem* ItemPtr = VisitedParameterToItemMap.Find(ParameterBinding->ResolvedParameter))
								{
									ItemPtr->AddToReadCount(FNiagaraParameterReferencePath(Context, Variable.GetName()));
								}
							}
			...
			{
				const FNiagaraParameterStore& UserParameterStore = SystemViewModel->GetSystem().GetExposedParameters();
				for (const FNiagaraVariableWithOffset& UserVariable : UserParameterStore.ReadParameterVariables())
				{
					if (UserVariable.GetType().IsDataInterface() == false)
					{
						continue;
					}
					FindDataInterfaceBindings(&SystemViewModel->GetSystem(), UserParameterStore.GetDataInterface(UserVariable.Offset), UserVariable);
				}
			}

			FNiagaraDataInterfaceUtilities::ForEachDataInterface(
				&SystemViewModel->GetSystem(),
				[&](const FNiagaraDataInterfaceUtilities::FDataInterfaceUsageContext& UsageContext)
				{
					FindDataInterfaceBindings(UsageContext.OwnerObject, UsageContext.DataInterface, UsageContext.Variable);
					return true;
				}
			);
		}
