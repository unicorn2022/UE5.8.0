...
{
	bool bNamespaceGroupingEnabled = true;
	static FAutoConsoleVariableRef CVarNamespaceGroupingEnabled(
		TEXT("fx.Niagara.SimCacheViewer.NamespaceGroupingEnabled"),
		bNamespaceGroupingEnabled,
		TEXT("When enabled namespaces will be part of the attribute tree view."),
		ECVF_Default
	);
	...
	void TraverseStructItems(const FString& StructPath, const FNiagaraTypeDefinition& StructTypeDef, const TFunction<void(const FString&, const FNiagaraTypeDefinition&)>& ItemCallback)
	{
		UScriptStruct* Struct = StructTypeDef.GetScriptStruct();
		if (Struct == nullptr)
		{
			return;
		}
		...
		for (TFieldIterator<FProperty> PropertyIt(Struct, EFieldIteratorFlags::IncludeSuper); PropertyIt; ++PropertyIt)
		{
			...
			const FString PropertyPath = StructPath + "." + Property->GetName();

			if (const FStructProperty* StructProperty = CastField<FStructProperty>(Property))
			{
				if (UScriptStruct* FriendlyStruct = FNiagaraTypeHelper::FindNiagaraFriendlyTopLevelStruct(StructProperty->Struct, ENiagaraStructConversion::Simulation))
				{
					TraverseStructItems(PropertyPath, FNiagaraTypeDefinition(FriendlyStruct), ItemCallback);
				}
			}
			if (Property->IsA(FIntProperty::StaticClass()))
			{
				ItemCallback(PropertyPath, FNiagaraTypeDefinition::GetIntDef());
			}
			else if (Property->IsA(FFloatProperty::StaticClass()))
			{
				ItemCallback(PropertyPath, FNiagaraTypeDefinition::GetFloatDef());
			}
			else if (Property->IsA(FBoolProperty::StaticClass()))
			{
				ItemCallback(PropertyPath, FNiagaraTypeDefinition::GetBoolDef());
			}
		}
	}

	namespace NamespaceGroupedTree
	{
		void AddItemToTreeInternal(TWeakPtr<SNiagaraSimCacheTreeView> WeakOwnerTreeView, TArray<TSharedRef<FNiagaraSimCacheTreeItem>>& ComponentTreeItems, FString ComponentPath, const FNiagaraTypeDefinition ComponentType)
		{
			TSharedPtr<FNiagaraSimCacheTreeItem> ParentItem = ComponentTreeItems[0];

			FString CurrentPath;

			TArray<FString> ComponentPathParts;
			ComponentPath.ParseIntoArray(ComponentPathParts, TEXT("."));

			for (int32 i = 0; i < ComponentPathParts.Num(); ++i)
			{
				const FString PathPart = ComponentPathParts[i];
				if (CurrentPath.Len() > 0)
				{
					CurrentPath.AppendChar('.');
				}
				CurrentPath.Append(PathPart);

				TSharedRef<FNiagaraSimCacheTreeItem>* CurrentPathItem = ComponentTreeItems.FindByPredicate(
					[&CurrentPath](const TSharedPtr<FNiagaraSimCacheTreeItem>& Item)
					{
						return Item->GetFilterName() == CurrentPath;
					}
				);

				if (CurrentPathItem == nullptr)
				{
					TSharedRef<FNiagaraSimCacheComponentTreeItem> NewItem = MakeShared<FNiagaraSimCacheComponentTreeItem>(WeakOwnerTreeView);
					NewItem->SetDisplayName(FText::FromString(PathPart));
					NewItem->SetFilterName(CurrentPath);

					const bool bIsLastPart = i == ComponentPathParts.Num() - 1;
					if (bIsLastPart)
					{
						NewItem->TypeDef = ComponentType;
					}

					ComponentTreeItems.Emplace(NewItem);
					CurrentPathItem = &ComponentTreeItems.Last();
				}
				ParentItem->AddChild(*CurrentPathItem);
				ParentItem = *CurrentPathItem;
			}
		}

		void AddItemToTree(TWeakPtr<SNiagaraSimCacheTreeView> WeakOwnerTreeView, TArray<TSharedRef<FNiagaraSimCacheTreeItem>>& ComponentTreeItems, const FNiagaraSimCacheVariable& Variable)
		{
			const FNiagaraTypeDefinition VariableTypeDef = Variable.Variable.GetType();
			const FString VariableName = Variable.Variable.GetName().ToString();

			AddItemToTreeInternal(WeakOwnerTreeView, ComponentTreeItems, VariableName, VariableTypeDef);
			if (!VariableTypeDef.IsEnum() && !FNiagaraTypeDefinition::IsScalarDefinition(VariableTypeDef))
			{
				TraverseStructItems(
					VariableName,
					VariableTypeDef,
					[&WeakOwnerTreeView, &ComponentTreeItems](const FString& StructPath, const FNiagaraTypeDefinition& StructTypeDef)
					{
						AddItemToTreeInternal(WeakOwnerTreeView, ComponentTreeItems, StructPath, StructTypeDef);
					}
				);
			}
		}
	...
	namespace TypeGroupedTree
	{
		TSharedRef<FNiagaraSimCacheTreeItem> AddItemToTreeInternal(TWeakPtr<SNiagaraSimCacheTreeView> WeakOwnerTreeView, const TArray<TSharedRef<FNiagaraSimCacheTreeItem>>& ComponentTreeItems, FString ComponentPath, const FNiagaraTypeDefinition ComponentType)
		{
			TSharedPtr<FNiagaraSimCacheTreeItem> ParentItem = ComponentTreeItems.Last();

			TSharedRef<FNiagaraSimCacheComponentTreeItem> NewItem = MakeShared<FNiagaraSimCacheComponentTreeItem>(WeakOwnerTreeView);
			NewItem->SetFilterName(ComponentPath);
			NewItem->TypeDef = ComponentType;

			if (ComponentTreeItems.Num() > 1)
			{
				const FString DisplayPath = ComponentPath.Mid(ParentItem->GetFilterName().Len() + 1);
				NewItem->SetDisplayName(FText::FromString(DisplayPath));
			}
			else
			{
				NewItem->SetDisplayName(FText::FromString(ComponentPath));
			}
		...
		void AddItemToTree(TWeakPtr<SNiagaraSimCacheTreeView> WeakOwnerTreeView, TArray<TSharedRef<FNiagaraSimCacheTreeItem>>& ComponentTreeItems, const FNiagaraSimCacheVariable& Variable)
		{
			...
			TSharedRef<FNiagaraSimCacheTreeItem> ParentItem = AddItemToTreeInternal(WeakOwnerTreeView, ComponentTreeItems, VariableName, VariableTypeDef);
			if (!VariableTypeDef.IsEnum() && !FNiagaraTypeDefinition::IsScalarDefinition(VariableTypeDef))
			{
				ComponentTreeItems.Add(ParentItem);
				TraverseStructItems(
					VariableName,
					VariableTypeDef,
					[&WeakOwnerTreeView, &ComponentTreeItems, &ParentItem](const FString& StructPath, const FNiagaraTypeDefinition& StructTypeDef)
					{
						AddItemToTreeInternal(WeakOwnerTreeView, ComponentTreeItems, StructPath, StructTypeDef);
					}
				);
				ComponentTreeItems.Pop();
			}
		}
	} //namespace TypeGroupedTree
...
void FNiagaraSimCacheViewModel::BuildTreeItemChildren(TSharedPtr<FNiagaraSimCacheTreeItem> TreeItem, TWeakPtr<SNiagaraSimCacheTreeView> OwningTreeView)
{
	...
	{
		TArray<TSharedRef<FNiagaraSimCacheTreeItem>> ComponentTreeItems;
		ComponentTreeItems.Emplace(TreeItem.ToSharedRef());

		SimCache->ForEachEmitterAttribute(
			CacheEmitterIndex.GetValue(),
			[OwningTreeView, &ComponentTreeItems](const FNiagaraSimCacheVariable& Variable)
			{
				if (NiagaraSimCacheViewModelPrivate::bNamespaceGroupingEnabled)
				{
					NiagaraSimCacheViewModelPrivate::NamespaceGroupedTree::AddItemToTree(OwningTreeView, ComponentTreeItems, Variable);
				}
				else
				{
					NiagaraSimCacheViewModelPrivate::TypeGroupedTree::AddItemToTree(OwningTreeView, ComponentTreeItems, Variable);
				}
				return true;
			}
		);
