...
OutputNodeType* GetEmitterOutputNodeForStackNodeInternal(InputNodeType& StackNode)
{
	...
	while (NodesToCheck.Num() > 0)
	{
		...
		for (const UEdGraphPin* OutputPin : OutputPins)
		{
			...
			{
				InputNodeType* LinkedNiagaraNode = LinkedPin ? Cast<UNiagaraNode>(LinkedPin->GetOwningNodeUnchecked()) : nullptr;
		...
		if (OutAllNodes.Contains(CurrentStartNode) == false)
		{
			...
			if (ParameterMapInputPin != nullptr)
			{
				...
				for (UEdGraphPin* InputPin : InputPins)
				{
					if (InputPin != ParameterMapInputPin)
					{
						...
						{
							UNiagaraNode* LinkedNode = Cast<UNiagaraNode>(InputLinkedPin->GetOwningNodeUnchecked());
			...
			if (CurrentStartNode != EndNode)
			{
				...
				for (UEdGraphPin* OutputPin : OutputPins)
				{
					for (UEdGraphPin* LinkedPin : OutputPin->LinkedTo)
					{
						UNiagaraNode* LinkedNode = Cast<UNiagaraNode>(LinkedPin->GetOwningNodeUnchecked());
						if (LinkedNode != nullptr)
						{
...
void FNiagaraStackGraphUtilities::GetStackNodeGroups(UNiagaraNode& StackNode, TArray<FNiagaraStackGraphUtilities::FStackNodeGroup>& OutStackNodeGroups)
{
	...
	if (OutputNode != nullptr)
	{
		...
		if (InputNode != nullptr)
		{
			...
			{
				UNiagaraNode& LastEndNode = *OutStackNodeGroups.Last().EndNode;
				UEdGraphPin* PreviousOutputPin = GetParameterMapOutputPin(LastEndNode);
				if (!ensureMsgf(PreviousOutputPin, TEXT("Could not find a valid parameter map output pin for %s"), *LastEndNode.GetPathName()))
				{
					OutStackNodeGroups.Empty();
					return; // unrecoverable, sorry.
				}
				FStackNodeGroup ModuleGroup;
				for (UEdGraphPin* PreviousOutputPinLinkedPin : PreviousOutputPin->LinkedTo)
				{
					if (PreviousOutputPinLinkedPin)
					{
						ModuleGroup.StartNodes.Add(CastChecked<UNiagaraNode>(PreviousOutputPinLinkedPin->GetOwningNode()));
					}
					else
					{
						UE_LOGF(LogNiagaraEditor, Log, "Invalid PreviousOutputPinLinkedPin in %ls", *ModuleNode->GetPathName());
					}
				}
				// previous tests could make this malformed.
				if (!ensureMsgf(!ModuleGroup.StartNodes.IsEmpty(), TEXT("ModuleGroup for %s has no StartNodes"), *ModuleNode->GetPathName()))
				{
					OutStackNodeGroups.Empty();
					return;
				}
			...
			for (UEdGraphPin* PreviousOutputPinLinkedPin : PreviousOutputPin->LinkedTo)
			{
				...
				{
					UE_LOGF(LogNiagaraEditor, Log, "Invalid PreviousOutputPinLinkedPin in %ls", *StackNode.GetPathName());
				}
			}
			// previous tests could make this malformed.
			if (!ensureMsgf(!OutputGroup.StartNodes.IsEmpty(), TEXT("OutputGroup for %s has no StartNodes"), *StackNode.GetPathName()))
			{
				OutStackNodeGroups.Empty();
				return;
			}
...
void FNiagaraStackGraphUtilities::ExtractInputPinsFromHistory(FNiagaraParameterMapHistory& History, UEdGraph* FunctionGraph, FNiagaraStackGraphUtilities::ENiagaraGetStackFunctionInputPinsOptions Options, TArray<const UEdGraphPin*>& OutPins)
{
...
UNiagaraNodeFunctionCall* FNiagaraStackGraphUtilities::FindFunctionCallNode(FGuid FunctionCallGuid,	TSharedRef<FNiagaraEmitterViewModel> EmitterViewModel)
{
	...
	if (UNiagaraGraph* Graph = GraphViewModel->GetGraph())
	{
		TArray<UNiagaraNodeFunctionCall*> FunctionCalls;
...
void FNiagaraStackGraphUtilities::GetStackFunctionOutputVariables(UNiagaraNodeFunctionCall& FunctionCallNode, FCompileConstantResolver ConstantResolver, TArray<FNiagaraVariable>& OutOutputVariables, TArray<FNiagaraVariable>& OutOutputVariablesWithOriginalAliasesIntact)
{
	...
	*Builder.ConstantResolver = ConstantResolver;
	...
	{
		UE_LOGF(LogNiagaraEditor, Log, "Invalid Stack Graph - Function call node has invalid history count!");
	}
}
...
void FNiagaraStackGraphUtilities::SetCustomExpressionForFunctionInput(UEdGraphPin& OverridePin, const FString& CustomExpression, UNiagaraNodeCustomHlsl*& OutDynamicInputFunctionCall, const FGuid& NewNodePersistentId)
{
	...
	if(UNiagaraNodeParameterMapSet* OverrideNode = Cast<UNiagaraNodeParameterMapSet>(OverridePin.GetOwningNode()))
	{
		UEdGraph* Graph = OverrideNode->GetGraph();
	...
	{
		OutDynamicInputFunctionCall = nullptr;
	}
}
...
{
	UE_LOGF(LogNiagaraEditor, Log, "Move module %ls", *ModuleToMove.GetPathName());
	...
	UE_LOGF(LogNiagaraEditor, Log, "Finished moving!");
}
