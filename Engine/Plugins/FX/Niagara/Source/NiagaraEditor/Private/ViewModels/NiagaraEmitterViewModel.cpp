...
bool FNiagaraEmitterViewModel::Initialize(const FVersionedNiagaraEmitter& InEmitter, TWeakPtr<FNiagaraEmitterInstance, ESPMode::ThreadSafe> InSimulation)
{
	...
	SummaryViewHierarchyViewModel->GetOnHierarchyChangedDelegate().AddSP(this, &FNiagaraEmitterViewModel::OnSummaryViewHierarchyChanged);
...
void FNiagaraEmitterViewModel::Reset()
{
	...
	SummaryViewHierarchyViewModel->GetOnHierarchyChangedDelegate().RemoveAll(this);
...
FNiagaraEmitterViewModel::~FNiagaraEmitterViewModel()
{
	...
	//UE_LOGF(LogNiagaraEditor, Warning, "Deleting Emitter view model %p", this);
}
	...
	if (InEmitter == Emitter)
	{
		...
		for (UNiagaraScript* GPUScript : GPUScripts)
		{
			if (GPUScript)
			{
				...
				for (int32 i = 0; i < Scripts.Num(); i++)
				{
					if (CompileStatuses.Num() > i && CompileErrors.Num() > i && Scripts[i] != nullptr && GPUScript->ContainsUsage(Scripts[i]->Usage))
					{
...
void FNiagaraEmitterViewModel::OnSummaryViewHierarchyChanged(const TInstancedStruct<FHierarchyChangedPayload>& Payload)
{
