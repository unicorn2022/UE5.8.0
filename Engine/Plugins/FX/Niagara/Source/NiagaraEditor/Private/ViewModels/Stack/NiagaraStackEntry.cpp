...
namespace
{
	// Build a locale-independent hash input for an FText. LOCTEXT-derived text carries a stable
	// (Namespace, Key) pair that survives translation - we prefer that so two LOCTEXT entries
	// that translate to the same string (or even share an English source string) still produce
	// distinct identifiers. Source-string fallback covers FText::FromString / FText::Format
	// results that have no key; ToString() is a final fallback for FText with neither.
	FString MakeStableIdentifierInput(const FText& Text)
	{
		const TOptional<FString> Namespace = FTextInspector::GetNamespace(Text);
		const TOptional<FString> Key = FTextInspector::GetKey(Text);
		if (Key.IsSet() && !Key->IsEmpty())
		{
			return FString::Printf(TEXT("%s::%s"),
				Namespace.IsSet() ? **Namespace : TEXT(""),
				**Key);
		}
...
UNiagaraStackEntry::FStackIssueFix::FStackIssueFix(FText InDescription, FStackIssueFixDelegate InFixDelegate, EStackIssueFixStyle InFixStyle)
	...
	, Style(InFixStyle)
	, UniqueIdentifier(FMD5::HashAnsiString(*MakeStableIdentifierInput(InDescription)))
{
...
UNiagaraStackEntry::FStackIssue::FStackIssue(EStackIssueSeverity InSeverity, FText InShortDescription, FText InLongDescription, FString InStackEditorDataKey, bool bInCanBeDismissed, const TArray<FStackIssueFix>& InFixes, const FSimpleDelegate& InDismissHandler)
	...
	, UniqueIdentifier(FMD5::HashAnsiString(*FString::Printf(TEXT("%s-%s-%s"), *InStackEditorDataKey, *MakeStableIdentifierInput(InShortDescription), *MakeStableIdentifierInput(InLongDescription))))
