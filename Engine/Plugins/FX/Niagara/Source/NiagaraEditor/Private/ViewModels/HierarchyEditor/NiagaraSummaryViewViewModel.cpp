...
void FNiagaraModuleInputViewModel::AddNativeChildrenInputs()
{
	...
	if(GetModuleNode().IsValid() && GetInputData().IsSet())
	{
		...
		HierarchyViewModel->OnHierarchyChanged(TInstancedStruct<FHierarchyChangedPayload>::Make("Native children inputs added"));
	}
}
...
TSharedRef<SWidget> FNiagaraHierarchyInputParameterHierarchyDragDropOp::CreateCustomDecorator() const
{
	...
	{
		TSharedPtr<FNiagaraModuleInputViewModel> InputViewModel = StaticCastSharedPtr<FNiagaraModuleInputViewModel>(TopLevelEntries[0].Element.Pin());
		if(TOptional<FInputData> InputData = InputViewModel->GetInputData())
		{
	...
	for(const FDraggedElementEntry& Entry : TopLevelEntries)
	{
		...
		{
			TSharedPtr<FNiagaraModuleInputViewModel> InputViewModel = StaticCastSharedPtr<FNiagaraModuleInputViewModel>(Entry.Element.Pin());
			if(TOptional<FInputData> InputData = InputViewModel->GetInputData())
			{
				...
				.Padding(1.f)
				[
					FNiagaraParameterUtilities::GetParameterWidget(FNiagaraVariable(InputData->Type, InputData->InputName), false, false)
				];
			}
		}
	}
...
TSharedRef<FHierarchyDragDropOp> UNiagaraSummaryViewViewModel::CreateDragDropOp(const FDataHierarchyDragDropContext& DragContext)
{
	// Only use the specialized op when all top-level entries are module inputs
	bool bAllModuleInputs = !DragContext.IsEmpty();
	for(const FDraggedElementEntry& Entry : DragContext.GetTopLevelEntries())
	{
		if(!Entry.Element.IsValid() || !Entry.Element.Pin()->GetData()->IsA<UNiagaraHierarchyModuleInput>())
		{
			bAllModuleInputs = false;
			break;
		}
	}
	...
	if(bAllModuleInputs)
	{
		TSharedRef<FNiagaraHierarchyInputParameterHierarchyDragDropOp> DragDropOp = MakeShared<FNiagaraHierarchyInputParameterHierarchyDragDropOp>(DragContext);
