...
void UNiagaraStackSummaryViewCollection::Initialize(FRequiredEntryData InRequiredEntryData,FVersionedNiagaraEmitterWeakPtr InEmitter, FString InOwningStackItemEditorDataKey)
{
	...
	GetEmitterViewModel()->GetSummaryHierarchyViewModel()->GetOnHierarchyChangedDelegate().AddUObject(this, &UNiagaraStackSummaryViewCollection::OnHierarchyChanged);
...
void UNiagaraStackSummaryViewCollection::FinalizeInternal()
{
	...
	{
		EmitterViewModelPtr->GetSummaryHierarchyViewModel()->GetOnHierarchyChangedDelegate().RemoveAll(this);
	}
...
void UNiagaraStackSummaryViewCollection::OnHierarchyChanged(const TInstancedStruct<FHierarchyChangedPayload>& Payload)
{
	// Deferred to the next tick because OnHierarchyChangedDelegate is broadcast synchronously from inside the hierarchy
	// view model's mutation paths (e.g. DeleteChild). Calling RefreshChildren() inline would re-enter SyncViewModelsToData
	// on the root while the outer walk is still iterating, corrupting Children. Same-scope defer keeps the work local to
	// this collection rather than escalating to a full stack refresh.
	if (IsFinalized() || GEditor == nullptr)
	{
		return;
	}

	TWeakObjectPtr<UNiagaraStackSummaryViewCollection> WeakThis(this);
	GEditor->GetTimerManager()->SetTimerForNextTick([WeakThis]()
	{
		if (UNiagaraStackSummaryViewCollection* StrongThis = WeakThis.Get())
		{
			if (!StrongThis->IsFinalized())
			{
				StrongThis->OnViewStateChanged();
			}
		}
	});
}
