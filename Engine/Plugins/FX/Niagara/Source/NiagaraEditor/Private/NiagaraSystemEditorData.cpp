...
const UNiagaraScriptVariable* UNiagaraSystemEditorData::FindUserScriptVariable(FNiagaraVariable UserParameter, const UNiagaraSystem& System) const
{
	bool bSuccess = System.GetExposedParameters().RedirectUserVariable(UserParameter);
	if (bSuccess)
	{
		const TObjectPtr<UNiagaraScriptVariable>* ExistingScriptVariable = UserParameterMetaData.FindByPredicate([UserParameter]
			(const UNiagaraScriptVariable* ScriptVariable) { return ScriptVariable->Variable == UserParameter; });
		if (ExistingScriptVariable != nullptr)
		{
			return *ExistingScriptVariable;
		}
