...
struct FNiagaraMaterialParameterValueHelper : public TSharedFromThis<FNiagaraMaterialParameterValueHelper>
{
	FNiagaraMaterialParameterValueHelper(TSharedPtr<IPropertyHandle> InNamePropertyHandle, TSharedPtr<IPropertyHandle> InValuePropertyHandle, TWeakObjectPtr<UNiagaraRendererProperties> InWeakRenderProperties)
		: NamePropertyHandle(InNamePropertyHandle.ToSharedRef())
		, ValuePropertyHandle(InValuePropertyHandle.ToSharedRef())
		, WeakRenderProperties(InWeakRenderProperties)
	{
	}

	template<typename TType>
	TOptional<TType> GetMaterialDefaultValue(UMaterialInterface* Material, FName ParameterName) const;

	template<>
	TOptional<float> GetMaterialDefaultValue(UMaterialInterface* Material, FName ParameterName) const
	{
		float DefaultValue;
		if (Material->GetScalarParameterDefaultValue(ParameterName, DefaultValue))
		{
			return DefaultValue;
		}
		return {};
	...
	template<>
	TOptional<FLinearColor> GetMaterialDefaultValue(UMaterialInterface* Material, FName ParameterName) const
	{
		FLinearColor DefaultValue;
		if (Material->GetVectorParameterDefaultValue(ParameterName, DefaultValue))
		{
			return DefaultValue;
		}
		return {};
	...
	template<>
	TOptional<UTexture*> GetMaterialDefaultValue(UMaterialInterface* Material, FName ParameterName) const
	{
		UTexture* DefaultValue;
		if (Material->GetTextureParameterDefaultValue(ParameterName, DefaultValue))
		{
			return DefaultValue;
		}
		return {};
	...
	void AppendValueToString(float V, FString& ValuesString) const
	{
		ValuesString.Appendf(TEXT("%f"), V);
	}

	void AppendValueToString(const FLinearColor& V, FString& ValuesString) const
	{
		ValuesString.Appendf(TEXT("{R=%f, G=%f, B=%f, A=%f}"), V.R, V.G, V.B, V.A);
	}

	void AppendValueToString(const UTexture* V, FString& ValuesString) const
	{
		ValuesString.Append(*GetNameSafe(V));
	}

	template<typename TType>
	FText FromValuesAsText(TArray<TType> Values) const
	{
		FString ValuesString;
		for (const TType& V : Values)
		{
			if (!ValuesString.IsEmpty())
			{
				ValuesString.Append(TEXT(", "));
			}
			AppendValueToString(V, ValuesString);
		}
		return FText::FromString(ValuesString);
	}

	FName GetParameterName() const
	{
		FName ParameterName;
		NamePropertyHandle->GetValue(ParameterName);
		return ParameterName;
	}

	template<typename TType>
	TArray<TType> TryResolveMaterialDefaultValue() const
	{
		TArray<TType> DefaultValues;

		if (UNiagaraRendererProperties* RenderProperties = WeakRenderProperties.Get())
		{
			const FName ParameterName = GetParameterName();
			...
			for (UMaterialInterface* Material : UsedMaterials)
			{
				...
				TOptional<TType> DefaultValue = GetMaterialDefaultValue<TType>(Material, ParameterName);
				if (DefaultValue.IsSet())
				{
					DefaultValues.AddUnique(DefaultValue.GetValue());
				}
			}
		}
	...
	{
		const TArray<TType> Values = TryResolveMaterialDefaultValue<TType>();
		const FText ParameterNameText = FText::FromName(GetParameterName());
		if (Values.Num() == 0)
		{
			return FText::Format(LOCTEXT("GetDefaultMaterialValue_InvalidBinding", "Parameter '{0}' is either invalid or not found on the material"), ParameterNameText);
		}
		else if (Values.Num() == 1)
		{
			return FText::Format(LOCTEXT("GetDefaultMaterialValue_Value", "Reset To Default\nSet to parameter '{0}' default value '{1}'"), ParameterNameText, FromValuesAsText(Values));
		}
		else
		{
			return FText::Format(LOCTEXT("GetDefaultMaterialValue_Conflict", "Reset To Default\nParameter '{0}' has conflicting values '{1}'"), ParameterNameText, FromValuesAsText(Values));
		}
	}

	template<typename TType>
	bool GetPropertyValue(TType& OutValue) const
	{
		return ValuePropertyHandle->GetValue(OutValue) == FPropertyAccess::Success;
	}

	template<>
	bool GetPropertyValue(UTexture*& OutValue) const
	{
		UObject* TempValue = nullptr;
		bool bSuccess = ValuePropertyHandle->GetValue(TempValue) == FPropertyAccess::Success;
		OutValue = Cast<UTexture>(TempValue);
		return bSuccess;
	}

	template<>
	bool GetPropertyValue(FLinearColor& OutValue) const
	{
		TArray<void*> RawDatas;
		ValuePropertyHandle->AccessRawData(RawDatas);

		if (RawDatas.Num() == 1 && RawDatas[0] != nullptr)
		{
			FMemory::Memcpy(&OutValue, RawDatas[0], sizeof(FLinearColor));
			return true;
		}
		return false;
	}
	...
	{
		TType Value;
		if (GetPropertyValue(Value))
		{
			const TArray<TType> Values = TryResolveMaterialDefaultValue<TType>();
			if (Values.Num() > 0 && Value != Values[0])
			{
	...
	template<typename TType>
	void SetPropertyValue(TType Value) const
	{
		ValuePropertyHandle->SetValue(Value);
	}

	template<>
	void SetPropertyValue(FLinearColor Value) const
	{
		TArray<void*> RawDatas;
		ValuePropertyHandle->AccessRawData(RawDatas);
		...
		for (void* RawData : RawDatas)
		{
			if (RawData)
			{
				FMemory::Memcpy(RawData, &Value, sizeof(FLinearColor));
			}
		}
	}
	...
	FReply OnClicked() const
	{
		...
		if (Values.Num() > 0)
		{
			FScopedTransaction Transaction(LOCTEXT("SetValueToMaterialDefault", "Set Value To Material Default"));

			TArray<UObject*> OuterObjects;
			ValuePropertyHandle->GetOuterObjects(OuterObjects);
	...
	template<typename TType>
	TSharedRef<SWidget> CreatePropertyWidget()
	{
		return ValuePropertyHandle->CreatePropertyValueWidget();
	}

	template<>
	TSharedRef<SWidget> CreatePropertyWidget<FLinearColor>()
	{
		return SNew(SNiagaraColorEditor)
			.ShowAlpha(true)
			.ShowExpander(true)
			.Color(this, &FNiagaraMaterialParameterValueHelper::GetColorValue)
			.OnColorChanged(this, &FNiagaraMaterialParameterValueHelper::SetColorValue)
			.OnBeginEditing(this, &FNiagaraMaterialParameterValueHelper::BeginContinuousTransaction)
			.OnEndEditing(this, &FNiagaraMaterialParameterValueHelper::EndContinuousTransaction)
			.OnCancelEditing(this, &FNiagaraMaterialParameterValueHelper::CancelContinuousTransaction)
		;
	}
	...
	{
		const FText TransactionText = LOCTEXT("SetColorValue", "Set Color Value");
		...
		const bool bIsContinousTransaction = ContinuousTransactionIndex.IsSet();

		FScopedTransaction Transaction(TransactionText, !bIsContinousTransaction);
		...
		SetPropertyValue(InColorValue);

		ValuePropertyHandle->NotifyPostChange(bIsContinousTransaction ? EPropertyChangeType::Interactive : EPropertyChangeType::ValueSet);
	}

	void BeginContinuousTransaction()
	{
		bContinuousTransactionPending = true;
	}

	void EndContinuousTransaction()
	{
		bContinuousTransactionPending = false;
	...
	void CancelContinuousTransaction(FLinearColor OriginalValue)
	{
		bContinuousTransactionPending = false;
		if (ContinuousTransactionIndex.IsSet())
		{
	...
	template<typename TType>
	bool CreateWidget(IDetailChildrenBuilder& ChildBuilder)
	{
		ChildBuilder.AddCustomRow(ValuePropertyHandle->GetPropertyDisplayName())
		.NameWidget
		[
		...
		[
			...
			[
				SNew(SButton)
				.IsFocusable(false)
				.ToolTipText(this, &FNiagaraMaterialParameterValueHelper::GetToolTipText<TType>)
				.ButtonStyle(FAppStyle::Get(), "NoBorder")
				.ContentPadding(0)
				.Visibility(this, &FNiagaraMaterialParameterValueHelper::IsVisible<TType>)
				.OnClicked(this, &FNiagaraMaterialParameterValueHelper::OnClicked<TType>)
				.Content()
				[
					SNew(SImage)
	...
	TSharedRef<IPropertyHandle>					NamePropertyHandle;
	TSharedRef<IPropertyHandle>					ValuePropertyHandle;
...
{
	UE_LOGF(LogNiagaraEditor, Warning, "Reset to default!");
}
	...
	if (Objects.Num() == 1)
	{
		...
		if (UNiagaraEmitterBase* EmitterBase = VersionedEmitter.Emitter)
		{
			...
			if ( SystemViewModel.IsValid() )
			{
				...
				{
					UE_LOGF(LogNiagaraEditor, Warning, "EmitterHandleViewModel was not valid. This shouldn't happen and implies some mismatch in emitter names.");
				}
			}
...
TArray<FName> FNiagaraUserParameterBindingCustomization::GetNames() const
{
	...
	if (IsValid() && TargetUserParameterBinding)
	{
		...
		AllowedTypes.Add(TargetUserParameterBinding->Parameter.GetType());
		{
			TArray<FString> AdditionalAllowedClasses;
			const FString& AdditionalAllowedClassesMetadata = PropertyHandle->GetProperty()->GetMetaData(TEXT("AdditionalAllowedClasses"));
			if (!AdditionalAllowedClassesMetadata.IsEmpty())
			{
				AdditionalAllowedClassesMetadata.ParseIntoArray(AdditionalAllowedClasses, TEXT(","), true);
				for (const FString& AllowedClass : AdditionalAllowedClasses)
				{
					if ( UClass* Class = FindObject<UClass>(nullptr, *AllowedClass) )
					{
						AllowedTypes.Add(FNiagaraTypeDefinition(Class));
					}
				}
			}
		}
		...
		{
			if (FNiagaraParameterUtilities::IsUserParameter(Var) && AllowedTypes.Contains(Var.GetType()))
			{
				Names.AddUnique(Var.GetName());
			}
		}
...
TArray<TPair<FNiagaraVariableBase, FNiagaraVariableBase> > FNiagaraMaterialAttributeBindingCustomization::GetNiagaraNames() const
{
	...
	//-TODO:stateless:Remove and unify emitter
	{
		...
		TArray<FNiagaraScriptDataKey> ScriptDataKeys;
		ScriptDataKeys.Add(FNiagaraScriptDataKey(nullptr, *BaseSystem->GetSystemSpawnScript()));
		ScriptDataKeys.Add(FNiagaraScriptDataKey(nullptr, *BaseSystem->GetSystemUpdateScript()));
		if (FVersionedNiagaraEmitterData* EmitterData = BaseEmitter.GetEmitterData())
		{
			...
			{
				ScriptDataKeys.Add(FNiagaraScriptDataKey(BaseEmitterHandleId, Script->GetUsage(), Script->GetUsageId()));
			}
		}
		...
		auto FindCachedDI = 
			[&](const FNiagaraVariableBase& BaseVariable) -> UNiagaraDataInterface*
			{
				...
				if (BaseVariable.IsInNameSpace(FNiagaraConstants::EmitterNamespaceString))
				{
					...
					//-TODO:stateless:Remove and unify emitter
					ResolvedVariable = FNiagaraUtilities::ResolveAliases(BaseVariable, FNiagaraAliasContext()
						.ChangeEmitterToEmitterName(BaseEmitter.Emitter->GetUniqueEmitterName()));
				}
				else
				{
					ResolvedVariable = BaseVariable;
				}

				for (const FNiagaraScriptDataKey& ScriptDataKey : ScriptDataKeys)
				{
					const FNiagaraScriptRuntimeData* ScriptRuntimeData = BaseSystem->GetScriptRuntimeData(ScriptDataKey).Get();
					...
					{
						if (ResolvedDataInterfaceData.ParameterStoreVariable == ResolvedVariable)
						{
							return ResolvedDataInterfaceData.ResolvedDataInterface;
						}
					}
				}
...
void FNiagaraMaterialAttributeBindingCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> StructPropertyHandle, class IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	if (Objects.Num() == 1)
	{
		...
		if (RenderProps)
		{
			...
			if (BaseEmitter.Emitter != nullptr)
			{
				const FNiagaraEmitterHandle* EmitterHandle = BaseSystem != nullptr 
					? BaseSystem->GetEmitterHandles().FindByPredicate([this]
						(const FNiagaraEmitterHandle& EmitterHandle) { return EmitterHandle.GetInstance().Emitter == BaseEmitter.Emitter; })
					: nullptr;
				BaseEmitterHandleId = EmitterHandle != nullptr ? EmitterHandle->GetId() : FGuid();
			}
			//-TODO:stateless:Remove and unify emitter
...
bool FNiagaraRendererMaterialScalarParameterCustomization::CustomizeChildProperty(IDetailChildrenBuilder& ChildBuilder, TSharedPtr<IPropertyHandle> PropertyHandle)
{
	if (NamePropertyHandle.IsValid() && PropertyHandle->GetProperty()->GetFName() == GET_MEMBER_NAME_CHECKED(FNiagaraRendererMaterialScalarParameter, Value))
	{
		ValueHelper = MakeShared<FNiagaraMaterialParameterValueHelper>(NamePropertyHandle, PropertyHandle, WeakRenderProperties);
		return ValueHelper->CreateWidget<float>(ChildBuilder);
	}
	return false;
}
...
bool FNiagaraRendererMaterialVectorParameterCustomization::CustomizeChildProperty(IDetailChildrenBuilder& ChildBuilder, TSharedPtr<IPropertyHandle> PropertyHandle)
{
	if (NamePropertyHandle.IsValid() && PropertyHandle->GetProperty()->GetFName() == GET_MEMBER_NAME_CHECKED(FNiagaraRendererMaterialVectorParameter, Value))
	{
		ValueHelper = MakeShared<FNiagaraMaterialParameterValueHelper>(NamePropertyHandle, PropertyHandle, WeakRenderProperties);
		return ValueHelper->CreateWidget<FLinearColor>(ChildBuilder);
	}
	return false;
}
...
bool FNiagaraRendererMaterialTextureParameterCustomization::CustomizeChildProperty(IDetailChildrenBuilder& ChildBuilder, TSharedPtr<IPropertyHandle> PropertyHandle)
{
	if (NamePropertyHandle.IsValid() && PropertyHandle->GetProperty()->GetFName() == GET_MEMBER_NAME_CHECKED(FNiagaraRendererMaterialTextureParameter, Texture))
	{
		ValueHelper = MakeShared<FNiagaraMaterialParameterValueHelper>(NamePropertyHandle, PropertyHandle, WeakRenderProperties);
		return ValueHelper->CreateWidget<UTexture*>(ChildBuilder);
	}
	return false;
}
