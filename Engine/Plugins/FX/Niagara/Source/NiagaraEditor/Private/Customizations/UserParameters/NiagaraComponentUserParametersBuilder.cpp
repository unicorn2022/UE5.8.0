...
FNiagaraComponentUserParametersNodeBuilder::~FNiagaraComponentUserParametersNodeBuilder()
{
	if (Component.IsValid() && bDelegatesInitialized)
	{
		...
		if(UNiagaraSystem* System = Component->GetAsset())
		{
			...
			{
				SystemViewModel->GetUserParametersHierarchyViewModel()->GetOnHierarchyChangedDelegate().RemoveAll(this);
			}
		}
	}
}
...
void FNiagaraComponentUserParametersNodeBuilder::RegisterRebuildOnHierarchyChanged()
{
	if(Component.IsValid())
	{
		if(UNiagaraSystem* Asset = Component->GetAsset())
		{
			...
			{
				SystemViewModel->GetUserParametersHierarchyViewModel()->GetOnHierarchyChangedDelegate().RemoveAll(this);
				SystemViewModel->GetUserParametersHierarchyViewModel()->GetOnHierarchyChangedDelegate().Add(UDataHierarchyViewModelBase::FOnHierarchyChanged::FDelegate::CreateSP(this, &FNiagaraComponentUserParametersNodeBuilder::RebuildOnHierarchyChanged));
			}
		}
	}
}
