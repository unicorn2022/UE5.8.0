...
void FNiagaraPlatformSetCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> InPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	while (CurrHandle)
	{
		...
		{
			PlatformSetArrayProperty = CurrHandle->GetParentHandle();
			PlatformSetArray = PlatformSetArrayProperty->AsArray();
			PlatformSetArrayIndex = ThisIndex;
		}
		else
		{
			PlatformSetArrayProperty.Reset();
			PlatformSetArray.Reset();
			PlatformSetArrayIndex = INDEX_NONE;
		}
		...
		if (FProperty* CurrProperty = CurrHandle->GetProperty())
		{
			if (UStruct* CurrStruct = CurrProperty->GetOwnerStruct())
			{
				...
				else if (PlatformSetArray.IsValid() && PlatformSetArrayProperty->HasMetaData("NiagaraOrthogonalPlatformSets"))
				{
					break;
				}
			}
...
void FNiagaraPlatformSetCustomization::UpdateCachedConflicts()
{
	...
	else if (PlatformSetArray.IsValid())
	{
		//Try to handle the general case.
		FName PlatformSetPropertyName = PropertyHandle->GetProperty()->GetFName();

		auto AddPlatformSetProperty = [&PlatformSets](TSharedPtr<IPropertyHandle> Handle)
			{
				FProperty* Prop = Handle->GetProperty();
				if (const FStructProperty* StructProp = CastField<FStructProperty>(Prop))
				{
					if (StructProp->Struct == FNiagaraPlatformSet::StaticStruct())
					{
						TArray<void*> Raw;
						Handle->AccessRawData(Raw);
						if (Raw.Num() == 1 && Raw[0] != nullptr)
						{
							if (FNiagaraPlatformSet* FoundPlatformSet = reinterpret_cast<FNiagaraPlatformSet*>(Raw[0]))
							{
								PlatformSets.Add(FoundPlatformSet);
							}
						}
					}
				}
		...
		PlatformSetArray->GetNumElements(NumElements);
		for (uint32 i = 0; i < NumElements; ++i)
		{
			TSharedPtr<IPropertyHandle> ElemProperty = PlatformSetArray->GetElement(i);
			if (TSharedPtr<IPropertyHandle> ElemPlatformSetPropertyHandle = FindChildPlatformSet(ElemProperty))
			{
				AddPlatformSetProperty(ElemPlatformSetPropertyHandle);
			}
		}
	}
