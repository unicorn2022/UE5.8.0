...
void UNiagaraNodeSelect::ChangeValuePinType(int32 Index, FNiagaraTypeDefinition Type)
{
	...
	{
		UE_LOGF(LogNiagaraEditor, Error, "Tried assigning type %ls to index %i. Out of bounds.", *Type.GetName(), Index);
		return;
	}
...
TArray<UEdGraphPin*> UNiagaraNodeSelect::GetOptionPins(int32 Index) const
{
	...
	if (Index < 0 || Index >= OutputVars.Num())
	{
		UE_LOGF(LogNiagaraEditor, Error, "Tried retrieving option pins for index index %i. Out of bounds.", Index);
		return OptionPins;
	}
	...
	{
		UE_LOGF(LogNiagaraEditor, Error, "Could not retrieve %i option pins for output pin %i. Retrieved %i pins instead.", NumOptionsPerVariable, Index, OptionPins.Num());
		return {};
	}
