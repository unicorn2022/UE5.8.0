...
namespace NiagaraDebuggerMeshCaptureSpawnTab
{
	static const FName TabName = FName(TEXT("NiagaraDebuggerMeshCapture"));
		...
		TabManager->RegisterTabSpawner(
			...
			FOnSpawnTab::CreateLambda(
				[=](const FSpawnTabArgs&)
				{
					return SNew(SDockTab)
						.TabRole(ETabRole::PanelTab)
						.Label(LOCTEXT("NiagaraDebuggerMeshCapture", "Mesh Capture"))
						[
							SNew(SNiagaraDebuggerMeshCapture)
						];
				}
			)
		)
		.SetDisplayName(LOCTEXT("NiagaraDebuggerMeshCaptureTitle", "Mesh Capture"))
		.SetTooltipText(LOCTEXT("NiagaraDebuggerMeshCaptureTooltipText", "Open the mesh capture tab."));
	}
}
...
void SNiagaraDebugger::Construct(const FArguments& InArgs)
{
	...
	NiagaraDebuggerMeshCaptureSpawnTab::RegisterTabSpawner(TabManager);
#endif
		...
		(
			...
			(
				...
				(
					...
					->AddTab(NiagaraDebuggerMeshCaptureSpawnTab::TabName, ETabState::OpenedTab)
#endif
