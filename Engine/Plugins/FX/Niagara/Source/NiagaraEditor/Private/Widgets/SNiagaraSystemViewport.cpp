...
public:
	...
	void FocusViewport(FBox FocusBounds, bool bInstant);
...
void FNiagaraSystemViewportClient::Draw(FViewport* InViewport, FCanvas* Canvas)
{
	...
	if (Component && SystemInstanceController.IsValid() && SystemInstanceController->GetAge() > 0.0)
	{
		...
		{
			// Provide some minimum bounds for the instant snap
			Bounds += Bounds.GetCenter() - FVector(100.0);
			Bounds += Bounds.GetCenter() + FVector(100.0);

			const bool bCachedShouldOrbitCamera = ShouldOrbitCamera();
			FocusViewport(Bounds, true);
...
void FNiagaraSystemViewportClient::FocusViewport(FBox FocusBounds, bool bInstant)
{
	if (FocusBounds.IsValid == false)
	{
		return;
	}

	UNiagaraSystemEditorData* SystemEditorData = GetSystemEditorData();
	if (SystemEditorData && SystemEditorData->GetViewportManualFocusDistanceEnabled())
	{
		const float Distance = SystemEditorData->GetViewportManualFocusDistance();
		const FVector HDistance = FVector(Distance * 0.5f);
		FocusBounds = FBox(FocusBounds.GetCenter() - HDistance, FocusBounds.GetCenter() + HDistance);
	}
...
void SNiagaraSystemViewport::Construct(const FArguments& InArgs, TSharedRef<FNiagaraSystemViewModel> InSystemViewModel)
{
	...
	// Register for preview platform changes
	if (UEditorEngine* EditorEngine = Cast<UEditorEngine>(GEngine))
	{
		OnPreviewFeatureLevelChangedHandle = EditorEngine->OnPreviewShaderPlatformChanged().AddLambda(
			[WeakWorld=TWeakObjectPtr<UWorld>(Client->GetWorld())](EShaderPlatform NewShaderPlatform)
			{
				if (UWorld* World = WeakWorld.Get())
				{
					World->ShaderPlatformChanged(NewShaderPlatform);
				}
			}
		);
	}
...
SNiagaraSystemViewport::~SNiagaraSystemViewport()
{
	if (OnPreviewFeatureLevelChangedHandle.IsValid())
	{
		if (UEditorEngine* EditorEngine = Cast<UEditorEngine>(GEngine))
		{
			EditorEngine->OnPreviewShaderPlatformChanged().Remove(OnPreviewFeatureLevelChangedHandle);
		}
	}
}
...
bool SNiagaraSystemViewport::IsFloorEnabled() const
{
	return AdvancedPreviewScene ? AdvancedPreviewScene->IsFloorEnabled() : false;
}

void SNiagaraSystemViewport::ToggleFloorEnabled()
{
	if ( AdvancedPreviewScene )
	{
		AdvancedPreviewScene->SetFloorVisibility(!AdvancedPreviewScene->IsFloorEnabled());
	}
}

float SNiagaraSystemViewport::GetFloorOffset() const
{
	return AdvancedPreviewScene ? AdvancedPreviewScene->GetFloorOffset() : 0.0f;
}

void SNiagaraSystemViewport::SetFloorOffset(float Offset)
{
	if (AdvancedPreviewScene)
	{
		AdvancedPreviewScene->SetFloorOffset(Offset);
		RefreshViewport();
	}
}

bool SNiagaraSystemViewport::IsManualFocusDistanceEnabled() const
{
	UNiagaraSystemEditorData* SystemEditorData = SystemViewportClient->GetSystemEditorData();
	return SystemEditorData ? SystemEditorData->GetViewportManualFocusDistanceEnabled() : false;
}

void SNiagaraSystemViewport::ToggleManualFocusDistance()
{
	if ( UNiagaraSystemEditorData* SystemEditorData = SystemViewportClient->GetSystemEditorData() )
	{
		SystemEditorData->SetViewportManualFocusDistanceEnabled(!SystemEditorData->GetViewportManualFocusDistanceEnabled());
	}
}

void SNiagaraSystemViewport::SetManualFocusDistance(float Distance)
{
	if (UNiagaraSystemEditorData* SystemEditorData = SystemViewportClient->GetSystemEditorData())
	{
		SystemEditorData->SetViewportManualFocusDistance(Distance);
	}
}

float SNiagaraSystemViewport::GetManualFocusDistance() const
{
	UNiagaraSystemEditorData* SystemEditorData = SystemViewportClient->GetSystemEditorData();
	return SystemEditorData ? SystemEditorData->GetViewportManualFocusDistance() : 0.0f;
}
...
void SNiagaraSystemViewport::OnFocusViewportToSelection()
{
	if( PreviewComponent )
	{
		...
		const bool bIsOrbit = SystemViewportClient->ShouldOrbitCamera();

		SystemViewportClient->FocusViewport(PreviewComponent->Bounds.GetBox(), false);
...
bool SNiagaraBaselineViewport::AddBaseline(UNiagaraEffectType* EffectType)
{
	...
	if (UNiagaraBaselineController* Controller = EffectType->GetPerfBaselineController())
	{
		if (UNiagaraSystem* System = Controller->GetSystem())
		{
			...
			{
				UE_LOGF(LogNiagaraEditor, Warning, "Niagara System shouldn't be used as a perf baseline as it does not have fixed bounds. %ls", *System->GetName());
			}
		...
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Baseline Niagara System missing!. Effect Type: %ls", *EffectType->GetName());
			return false;
		}
	}
	return false;
