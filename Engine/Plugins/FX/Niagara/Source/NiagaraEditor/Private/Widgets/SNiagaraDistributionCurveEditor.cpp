...
namespace
{
	...
	class FNiagaraDistributionCurveEditorModel : public FRichCurveEditorModel
	{
	public:
		FNiagaraDistributionCurveEditorModel(TSharedRef<INiagaraDistributionAdapter> InDistributionAdapter, int32 InChannelIndex)
			: FRichCurveEditorModel(GetOwnerFromDistribution(InDistributionAdapter.Get()))
			, DistributionAdapter(InDistributionAdapter)
			, ChannelIndex(InChannelIndex)
		{
		}

		virtual bool IsValid() const override { return DistributionAdapter.IsValid() && DistributionAdapter->GetCurveValue(ChannelIndex) != nullptr; }
		virtual FRichCurve& GetRichCurve() { return *DistributionAdapter->GetCurveValue(ChannelIndex); }
		virtual const FRichCurve& GetReadOnlyRichCurve() const override { return *DistributionAdapter->GetCurveValue(ChannelIndex); }

	private:
		static UObject* GetOwnerFromDistribution(INiagaraDistributionAdapter& InDistribution)
		{
			TArray<UObject*> Owners;
			InDistribution.GetOwners(Owners);
	...
	{
		void Initialize(TSharedRef<INiagaraDistributionAdapter> InDistributionAdapter, int InChannelIndex)
		{
			DistributionAdapter = InDistributionAdapter;
			ChannelIndex = InChannelIndex;
		...
		virtual void CreateCurveModels(TArray<TUniquePtr<FCurveModel>>& OutCurveModels) override
		{
			...
			DistributionAdapter->GetOwners(OwnerObjects);
			if (OwnerObjects.Num() != 1)
			{
				return;
			}

			TUniquePtr<FNiagaraDistributionCurveEditorModel> NewCurve = MakeUnique<FNiagaraDistributionCurveEditorModel>(DistributionAdapter.ToSharedRef(), ChannelIndex);
	...
	private:
		void OnCurveChanged()
		{
			DistributionAdapter->NotifyCurveValueChangedExternally(ChannelIndex);
		}
...
void SNiagaraDistributionCurveEditor::Construct(const FArguments& InArgs, TSharedRef<INiagaraDistributionAdapter> InDistributionAdapter)
{
	...
	{
		TSharedRef<FNiagaraDistributionCurveEditorDetailsTreeItem> TreeItem = MakeShared<FNiagaraDistributionCurveEditorDetailsTreeItem>();
		TreeItem->Initialize(DistributionAdapter.ToSharedRef(), iChannel);
