...
{
public:
	virtual ~INiagaraDistributionAdapter() = default;
	...
	*/
	...
	/** Gets a curve value for the specified channel */
	virtual FRichCurve* GetCurveValue(int32 ChannelIndex) = 0;

	/** Notifies this distribution adapter that a curve from GetCurveValue has been modified externally. */
	virtual void NotifyCurveValueChangedExternally(int32 ChannelIndex) = 0;
	...
	/** Gets the UEnum to be used in lookup value mode. */
	virtual const UEnum* GetLookupValueModeEnum() const = 0;
	...
	/** A delegate that is called whenever the value of the distribution is changed. */
	virtual FSimpleMulticastDelegate& OnDistributionValueChanged() = 0;

	/** Gets the owning objects for this distribution. */
	virtual void GetOwners(TArray<UObject*>& OutOwners) const = 0;
