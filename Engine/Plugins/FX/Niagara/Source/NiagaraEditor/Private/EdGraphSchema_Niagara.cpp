...
FNiagaraVariable UEdGraphSchema_Niagara::PinToNiagaraVariable(const UEdGraphPin* Pin, bool bNeedsValue, ENiagaraStructConversion StructConversion)
{
	...
	if (Pin->bDefaultValueIsIgnored == false && Pin->DefaultValue.IsEmpty() == false)
	{
		...
		if (TypeEditorUtilities.IsValid() && TypeEditorUtilities->CanHandlePinDefaults())
		{
			...
			if (bHasValue == false)
			{
				...
				UE_LOGF(LogNiagaraEditor, Warning, "PinToNiagaraVariable: Failed to convert default value '%ls' to type %ls. Owning node path: %ls", *Pin->DefaultValue, *Var.GetType().GetName(), *OwningNodePath);
			}
		}
		else
		{
			...
			{
				FString OwningNodePath = Pin->GetOwningNode() != nullptr ? Pin->GetOwningNode()->GetPathName() : TEXT("Unknown");
				UE_LOGF(LogNiagaraEditor, Warning, "Pin had default value string, but default values aren't supported for variables of type {%ls}. Owning node path: %ls", *Var.GetType().GetName(), *OwningNodePath);
			}
		}
	}
	...
	if (bNeedsValue && bHasValue == false)
	{
		...
		{
			UE_LOGF(LogNiagaraEditor, Warning, "ResetVariableToDefaultValue called, but failed on var %ls type %ls. ", *Var.GetName().ToString(), *Var.GetType().GetName());
		}
	}
...
bool UEdGraphSchema_Niagara::TryGetPinDefaultValueFromNiagaraVariant(const FNiagaraVariableBase& Variable, const FNiagaraVariant& Value, FString& OutPinDefaultValue) const
{
	bool bValid = Value.IsValid(Variable.GetType());
	OutPinDefaultValue.Reset();
	if (Variable.IsDataInterface())
	{
		if (bValid)
		{
			OutPinDefaultValue = FSoftObjectPath(Value.GetDataInterface()).ToString();
			return true;
		}
	}
	else if (Variable.IsUObject())
	{
		if (bValid)
		{
			OutPinDefaultValue = FSoftObjectPath(Value.GetUObject()).ToString();
			return true;
		}
	}
	else
	{
		FNiagaraVariable PinDefaultVariable = Variable;//Just deal with data in the variable since that's what other code deals with
		if (bValid)
		{
			PinDefaultVariable.AllocateData();
			FMemory::Memcpy(PinDefaultVariable.GetData(), Value.GetBytes(), PinDefaultVariable.GetType().GetSize());
		}
		else
		{
			FNiagaraEditorUtilities::ResetVariableToDefaultValue(PinDefaultVariable);
...
FNiagaraTypeDefinition UEdGraphSchema_Niagara::PinToTypeDefinition(const UEdGraphPin* Pin, ENiagaraStructConversion StructConversion)
{
	...
	if ((Pin->PinType.PinCategory == PinCategoryType || Pin->PinType.PinCategory == PinCategoryStaticType) && Pin->PinType.PinSubCategoryObject.IsValid())
	{
		...
		if (Struct == nullptr)
		{
			UE_LOGF(LogNiagaraEditor, Error, "Pin states that it is of struct type, but is missing its struct object. This is usually the result of a registered type going away. Pin Name '%ls' Owning Node '%ls'.",
		...
		if (Struct && !FNiagaraTypeHelper::IsNiagaraFriendlyTopLevelStruct(Struct, StructConversion)) // LWC swapover support
		{
			...
			{
				UE_LOGF(LogNiagaraEditor, Verbose, "Pin states that it is not a niagara friendly struct, but didn't get converted by PostLoad yet, it still has RF_NeedPostLoad . Pin Name '%ls' Owning Node '%ls'.",
					*Pin->PinName.ToString(), OwningNode ? *OwningNode->GetPathName() : TEXT("Invalid"));
			}
			else 
			{
				UE_LOGF(LogNiagaraEditor, Verbose, "Pin states that it is not a niagara friendly struct, but didn't get converted by PostLoad yet, it does not have RF_NeedPostLoad though. Pin Name '%ls' Owning Node '%ls'.",
					*Pin->PinName.ToString(), OwningNode ? *OwningNode->GetPathName() : TEXT("Invalid"));
			}
	...
	else if (Pin->PinType.PinCategory == PinCategoryClass || Pin->PinType.PinCategory == PinCategoryStaticClass)
	{
		...
		if (Class == nullptr)
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Pin states that it is of class type, but is missing its class object. This is usually the result of a registered type going away. Pin Name '%ls' Owning Node '%ls'.",
	...
	else if (Pin->PinType.PinCategory == PinCategoryEnum || Pin->PinType.PinCategory == PinCategoryStaticEnum)
	{
		...
		if (Enum == nullptr)
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Pin states that it is of Enum type, but is missing its Enum! Pin Name '%ls' Owning Node '%ls'. Turning into standard int definition!", *Pin->PinName.ToString(),
...
FNiagaraTypeDefinition UEdGraphSchema_Niagara::PinTypeToTypeDefinition(const FEdGraphPinType& PinType)
{
	if ((PinType.PinCategory == PinCategoryType || PinType.PinCategory == PinCategoryStaticType) && PinType.PinSubCategoryObject.IsValid())
	{
		...
		if (Struct == nullptr)
		{
			UE_LOGF(LogNiagaraEditor, Error, "Pin states that it is of struct type, but is missing its struct object. This is usually the result of a registered type going away.");
			return FNiagaraTypeDefinition();
		}
	...
	else if (PinType.PinCategory == PinCategoryClass || PinType.PinCategory == PinCategoryStaticClass)
	{
		...
		if (Class == nullptr)
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Pin states that it is of class type, but is missing its class object. This is usually the result of a registered type going away.");
			return FNiagaraTypeDefinition();
		}
	...
	else if (PinType.PinCategory == PinCategoryEnum || PinType.PinCategory == PinCategoryStaticEnum)
	{
		...
		if (Enum == nullptr)
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Pin states that it is of Enum type, but is missing its Enum! Turning into standard int definition!");
			return FNiagaraTypeDefinition(FNiagaraTypeDefinition::GetIntDef());
		}
...
void UEdGraphSchema_Niagara::GetContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	...
	else if (InGraphNode)
	{
		...
		Section.AddMenuEntry("RefreshNode", LOCTEXT("RefreshNode", "Refresh Node"), LOCTEXT("RefreshNodeTooltip", "Refresh this node."), FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Refresh"),
