...
void UNiagaraNodeDataSetBase::PostLoad()
{
	...
	/*if (ExternalStructAsset != nullptr)
	{
		UE_LOGF(LogNiagaraEditor, Display, "Niagara script '%ls' references struct asset '%ls'", *GetFullName(), *ExternalStructAsset->GetFullName());
	}*/
...
bool UNiagaraNodeDataSetBase::IsSynchronizedWithStruct(bool bIgnoreConditionVariable, FString* Issues, bool bLogIssues) const
{
		...
		// First check to see if any variables have been added...
		{
			for (TFieldIterator<FProperty> PropertyIt(ExternalStructAsset, EFieldIteratorFlags::IncludeSuper); PropertyIt; ++PropertyIt)
			{
				...
				if (VarFound == nullptr)
				{
					...
					if (bLogIssues)
					{
						UE_LOGF(LogNiagaraEditor, Warning, "%ls Unable to find matching variable for struct property: '%ls' ... possible add?", *GetPathNameSafe(this), *GetPathNameSafe(Property));
					}
					if (Issues != nullptr)
					{
				...
				else
				{
					...
					if (GetSupportedNiagaraTypeDef(Property, TypeDefFound))
					{
						if (VarFound->GetType() != TypeDefFound)
						{
							...
							if (bLogIssues)
							{
								UE_LOGF(LogNiagaraEditor, Warning, "%ls Matching variable for struct property '%ls', but different type:  Existing Type:'%ls' vs New Type:'%ls' ... possible type change in user-defined struct?", *GetPathNameSafe(this), *GetPathNameSafe(Property), *VarFound->GetType().GetName(), *TypeDefFound.GetName());
							}
							if (Issues != nullptr)
							{
					...
					else
					{
						...
						if (bLogIssues)
						{
							UE_LOGF(LogNiagaraEditor, Warning, "%ls Matching variable for struct property '%ls', but different type:  Existing Type:'%ls' vs New Type:'Unsupported' ... possible type change in user-defined struct?", *GetPathNameSafe(this), *GetPathNameSafe(Property), *VarFound->GetType().GetName());
						}
						if (Issues != nullptr)
						{
		...
		// Now check to see if any variables have been removed..
		{
			for (const FNiagaraVariable& Var : Variables)
			{
				...
				if (!bFound)
				{
					...
					if (bLogIssues)
					{
						UE_LOGF(LogNiagaraEditor, Warning, "%ls Unable to find matching struct property for variable: '%ls' ... possible removal?", *GetPathNameSafe(this), *Var.GetName().ToString());
					}
	...
	else
	{
		if (bLogIssues)
		{
			UE_LOGF(LogNiagaraEditor, Warning, "%ls Unable to find matching Niagara Type: %ls", *GetPathNameSafe(this), *DataSet.Name.ToString());
		}
...
bool UNiagaraNodeDataSetBase::SynchronizeWithStruct()
{
	...
	for (TFieldIterator<FProperty> PropertyIt(ExternalStructAsset, EFieldIteratorFlags::IncludeSuper); PropertyIt; ++PropertyIt)
	{
		...
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Could not add property %ls in struct %ls to NiagaraNodeDataSetBase, class %ls", *Property->GetName(), *ExternalStructAsset->GetName(), ExternalStructAsset->GetClass() ? *ExternalStructAsset->GetClass()->GetName() : *FString("nullptr"));
		}
	}
