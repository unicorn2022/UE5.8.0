...
namespace SNiagaraParameterNamePrivate
{
	void TryResolveParameterCollectionPath(FName CollectionName, FNiagaraNamespaceMetadata& NamespaceMetadata)
	{
		for (TObjectIterator<UNiagaraParameterCollection> NPCIt; NPCIt; ++NPCIt)
		{
			UNiagaraParameterCollection* NPC = *NPCIt;
			if (::IsValid(NPC) && NPC->IsAsset() && (NPC->GetNamespace() == CollectionName))
			{
				CollectionName = FName(*NPC->GetPathName());
				break;
			}
		}

		NamespaceMetadata.SetDescription(FText::Format(NSLOCTEXT("NiagaraParameterName", "NPCAssetToolTipFormat", "References parameter collection '{0}'."), FText::FromName(CollectionName)));
	}
}
		...
		for(int32 NamePartIndex = NamespaceNamePartCount; NamePartIndex < NameParts.Num() - 1; NamePartIndex++)
		{
			...
			else
			{
				...
				else
				{
					...
					if (NamePartIndex == 1 && NameParts[0] == FNiagaraConstants::ParameterCollectionNamespace)
					{
						SNiagaraParameterNamePrivate::TryResolveParameterCollectionPath(NameParts[1], NamespaceModifierMetadata);
					}
				}
