...
const FName FNiagaraSystemToolkitModeBase::DefaultModeName(TEXT("NiagaraSystemToolkitMode_Default"));
const FName FNiagaraSystemToolkitModeBase::ScalabilityModeName(TEXT("NiagaraSystemToolkitMode_Scalability"));
...
void FNiagaraSystemToolkitModeBase::OnActiveDocumentChanged(TSharedPtr<SDockTab> NewActiveTab)
{
	...
	if (ActiveTab.IsValid())
	{
		...
		{
			LastSelectionUpdateDelegate = ScratchScriptVM->GetGraphViewModel()->GetNodeSelection()->OnSelectedObjectsChanged().AddSP(this, &FNiagaraSystemToolkitModeBase::UpdateSelectionForActiveDocument);
			if (INiagaraParameterPanelViewModel* ParameterPanelViewModel = Toolkit->GetSystemViewModel()->GetParameterPanelViewModel())
			{
				LastParamPanelSelectionUpdateDelegate = ParameterPanelViewModel->GetVariableObjectSelection()->OnSelectedObjectsChanged().AddSP(this, &FNiagaraSystemToolkitModeBase::OnParameterPanelViewModelExternalSelectionChanged);
			}
			...
			LastGraphEditDelegate = ScratchScriptVM->GetGraphViewModel()->GetGraph()->AddOnGraphNeedsRecompileHandler(FOnGraphChanged::FDelegate::CreateSP(this, &FNiagaraSystemToolkitModeBase::OnEditedScriptGraphChanged));
		}
	}
}
...
void FNiagaraSystemToolkitModeBase::RegisterTabFactories(TSharedPtr<FTabManager> InTabManager)
{
	...
	TSharedPtr<FNiagaraSystemToolkit> PinnedSystemToolkit = SystemToolkit.Pin();
	PinnedSystemToolkit->RegisterToolbarTab(InTabManager.ToSharedRef());
...
// 		.SetGroup(WorkspaceMenuCategory.ToSharedRef());
	...
	TSharedPtr<FNiagaraSystemToolkit> SystemToolkitPinned = SystemToolkit.Pin();
	if (SystemToolkitPinned.IsValid())
	{
		const UNiagaraEditorSettings* NiagaraEditorSettings = GetDefault<UNiagaraEditorSettings>();
		NiagaraEditorSettings->GetExternalTabSpawnersForAsset(SystemToolkitPinned.ToSharedRef(), SystemToolkitPinned->GetSystemViewModel()->GetSystem(), ExternalTabSpawners);
		if(ExternalTabSpawners.Num() > 0)
		{
			for (TSharedRef<FWorkflowTabFactory>& ExternalTabSpawner : ExternalTabSpawners)
			{
				ExternalTabSpawner->RegisterTabSpawner(InTabManager.ToSharedRef(), this);
			}
		}
	}

	RegisterTabFactoriesWithAppAndManager(PinnedSystemToolkit.Get(), InTabManager.ToSharedRef());
}
