...
FNiagaraSystemToolkitMode_Default::FNiagaraSystemToolkitMode_Default(TWeakPtr<FNiagaraSystemToolkit> InSystemToolkit) : FNiagaraSystemToolkitModeBase(FNiagaraSystemToolkitModeBase::DefaultModeName, InSystemToolkit)
{
	TabLayout = FTabManager::NewLayout("Standalone_Niagara_System_Layout_v30")
		->AddArea
		(
	...
	NiagaraEditorSettings->ExternalExtendLayoutForAsset(LayoutExtender.ToSharedRef(), SystemToolkit.Pin()->GetSystemViewModel()->GetSystem());
...
void FNiagaraSystemToolkitMode_Default::ExtendToolbar()
{
	struct Local
	{
		...
		static TSharedRef<SWidget> FillSettingsMenu(FNiagaraSystemToolkit* Toolkit)
		{
			FMenuBuilder MenuBuilder(true, Toolkit->GetToolkitCommands());
			if (FProperty* AutoPlayProperty = UNiagaraEditorSettings::StaticClass()->FindPropertyByName("bAutoPlay"))
			{
				MenuBuilder.AddMenuEntry(
					AutoPlayProperty->GetDisplayNameText(),
					AutoPlayProperty->GetToolTipText(),
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateLambda(
							[]()
							{
								...
								EditorSettings->SetAutoPlay(!EditorSettings->GetAutoPlay());
							}
						),
						FCanExecuteAction(),
						FIsActionChecked::CreateLambda(
							[]() -> bool
							{
								return GetDefault<UNiagaraEditorSettings>()->GetAutoPlay();
							}
						)
					),
					NAME_None,
		...
		static void FillToolbar(FToolBarBuilder& ToolbarBuilder, FNiagaraSystemToolkit* Toolkit)
		{
			...
			{
				FUIAction ScalabilityToggleAction(FExecuteAction::CreateRaw(Toolkit, &FNiagaraSystemToolkit::SetCurrentMode, FNiagaraSystemToolkitModeBase::ScalabilityModeName));
				ScalabilityToggleAction.GetActionCheckState = FGetActionCheckState::CreateLambda([Toolkit]()
				{
					return Toolkit->GetCurrentMode() == FNiagaraSystemToolkitModeBase::ScalabilityModeName ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
				});
			...
			ToolbarBuilder.BeginSection("Settings");
			{
				ToolbarBuilder.AddComboButton(
					FUIAction(),
					FOnGetContent::CreateStatic(Local::FillSettingsMenu, Toolkit),
					FText(),
					LOCTEXT("NiagaraEditorGlobalSettings", "Niagara Editor Settings"),
					FSlateIcon(FAppStyle::GetAppStyleSetName(), "DetailsView.ViewOptions")
				);
			}
			ToolbarBuilder.EndSection();
