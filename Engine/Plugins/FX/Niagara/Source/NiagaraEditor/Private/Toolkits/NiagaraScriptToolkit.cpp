...
void FNiagaraScriptToolkit::Initialize( const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UNiagaraScript* InputScript )
{
	...
	ParametersHierarchyViewModel->GetOnHierarchyChangedDelegate().AddSP(this, &FNiagaraScriptToolkit::OnHierarchyChanged);
...
void FNiagaraScriptToolkit::OnHierarchyChanged(const TInstancedStruct<FHierarchyChangedPayload>& Payload)
{
...
void FNiagaraScriptToolkit::OnApply()
{
	if(GetOpenMethod() == EAssetOpenMethod::Edit)
	{
		...
		UE_LOGF(LogNiagaraEditor, Log, "Applying Niagara Script %ls", *GetEditingObjects()[0]->GetName());
...
{
	UE_LOGF(LogNiagaraEditor, Log, "Saving and Compiling NiagaraScript %ls", *GetEditingObjects()[0]->GetName());
