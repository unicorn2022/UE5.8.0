...
void FNiagaraSystemToolkit::InitializeInternal(const EToolkitMode::Type Mode, const TSharedPtr<IToolkitHost>& InitToolkitHost, const FGuid& MessageLogGuid)
{
	...
	AddApplicationMode(FNiagaraSystemToolkitModeBase::DefaultModeName, MakeShared<FNiagaraSystemToolkitMode_Default>(SharedThis(this)));
	AddApplicationMode(FNiagaraSystemToolkitModeBase::ScalabilityModeName, MakeShared<FNiagaraSystemToolkitMode_Scalability>(SharedThis(this)));
	...
	SetCurrentMode(FNiagaraSystemToolkitModeBase::DefaultModeName);
...
void FNiagaraSystemToolkit::SaveAsset_Execute()
{
	...
	if (SystemToolkitMode == ESystemToolkitMode::Emitter)
	{
		UE_LOGF(LogNiagaraEditor, Log, "Saving and Compiling NiagaraEmitter %ls", *GetEditingObjects()[0]->GetName());
