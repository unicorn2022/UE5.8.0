...
static int GNiagaraCompilationForceOneThread = 0;
static FAutoConsoleVariableRef CVarNiagaraCompilationForceOneThread(
	TEXT("fx.Niagara.Compilation.ForceOneThread"),
	GNiagaraCompilationForceOneThread,
	TEXT("Forces the compile tasks to run inline on one thread."),
	ECVF_Default
);
...
namespace NiagaraSystemCookStats
{
	...
	static std::atomic<int32> ScriptTranslationCount(0);
	...
	{
		AddStat(TEXT("Niagara"), FCookStatsManager::CreateKeyValueArray(TEXT("NiagaraScriptTranslationCount"), ScriptTranslationCount.load(std::memory_order_relaxed)));
...
namespace NiagaraCompilationTasksImpl
{
	...
	static bool BinaryToExecData(const UNiagaraScript* Script, FSharedBuffer SharedBuffer, FNiagaraVMExecutableData& OutExecData)
	{
		...
		{
			UE_LOGF(LogNiagaraEditor, Display, "Failed to validate FNiagaraVMExecutableData received from DDC, rejecting!  Reasons:\nDeprecated object version");
			return false;
		}
	...
	static FSharedBuffer ExecToBinaryData(const UNiagaraScript* Script, FNiagaraVMExecutableData& InExecData)
	{
		...
		{
			UE_LOGF(LogNiagaraEditor, Error, "Failed to validate FNiagaraVMExecutableData being pushed to DDC, rejecting!  Errors:\n%ls", *ValidationErrors);
			return FSharedBuffer();
		}
...
void FNiagaraSystemCompilationTask::FCompileTaskInfo::Translate(FNiagaraSystemCompilationTask* SystemCompileTask, const FCompileGroupInfo& GroupInfo)
{
	...
	COOK_STAT(NiagaraSystemCookStats::ScriptTranslationCount.fetch_add(1, std::memory_order_relaxed));
...
void FNiagaraSystemCompilationTask::FCompileTaskInfo::RetrieveTranslateResult()
{
	...
	for (const FNiagaraCompileEvent& Message : TranslateResults.CompileEvents)
	{
		...
		UE_LOGF(LogNiagaraEditor, Log, "%ls", *Message.Message);
#endif
...
{
	UE_LOG_CONTEXT("NiagaraSystem", SystemInfo.GetSystemPath());
	switch (CompilationState)
	{
		case EState::WaitingForProcessing:
		{
...
void FNiagaraSystemCompilationTask::Precompile()
{
	...
	for (int32 EmitterIt = 0; EmitterIt < EmitterCount; ++EmitterIt)
	{
		...
		UE_LOG_CONTEXT("NiagaraEmitter", EmitterInfo.UniqueEmitterName);
...
void FNiagaraSystemCompilationTask::IssueCompilationTasks()
{
	...
	EExtendedTaskPriority TaskExtendedPriority = (GNiagaraCompilationForceOneThread > 0 || FApp::ShouldUseThreadingForPerformance() == false)
	...
	if (HasOutstandingCompileTasks())
	{
		...
		{
			COOK_STAT(auto Timer = NiagaraSystemCookStats::UsageStats.TimeSyncWork(); Timer.TrackCyclesOnly(););
			UE_LOG_CONTEXT("NiagaraSystem", Self->SystemInfo.GetSystemPath());

			Self->Precompile();
		}, TaskPriority, TaskExtendedPriority);
		...
		for (FCompileGroupInfo& GroupInfo : CompileGroups)
		{
			if (GroupInfo.HasOutstandingCompileTasks(*this))
			{
				...
				}, PrecompileTask, TaskPriority, TaskExtendedPriority);
				...
				for (int32 CompileTaskIndex : GroupInfo.CompileTaskIndices)
				{
					...
					if (CompileTask.IsOutstanding(*this))
					{
						...
						IssuedCompilationTasks.Add(Launch(UE_SOURCE_LOCATION, [Self = AsShared(), &GroupInfo, &CompileTask]
						{
							...
							UE_LOG_CONTEXT("NiagaraSystem", TaskPtr->SystemInfo.GetSystemPath());
						...
						}, InstantiateGraphTask, TaskPriority, TaskExtendedPriority));
						...
						}, *CompileTask.CompileResultsReadyEvent, TaskPriority, TaskExtendedPriority);
	...
	}, IssuedCompilationTasks, TaskPriority, TaskExtendedPriority);

	Launch(UE_SOURCE_LOCATION, [Self = AsShared()]
	{
		UE_LOG_CONTEXT("NiagaraSystem", Self->SystemInfo.GetSystemPath());
		Self->IssuePostResultsProcessedTasks();
	}, CompilationTasks, TaskPriority, TaskExtendedPriority);
}
