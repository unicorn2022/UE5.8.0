	...
	, OverridePin(&InOverridePin)
{
	...
	else if (OverridePin->LinkedTo.Num() == 1)
	{
		...
		{
			UE_LOGF(LogNiagaraEditor, Error, "Invalid Stack Graph - Unsupported input node connection. Owning Node: %ls", *OverrideNode->GetPathName());
		}
	}
	else
	{
		UE_LOGF(LogNiagaraEditor, Error, "Invalid Stack Graph - Input had multiple connections. Owning Node: %ls", *OverrideNode->GetPathName());
	}
}
...
FNiagaraStackFunctionMergeAdapter::FNiagaraStackFunctionMergeAdapter(const FVersionedNiagaraEmitter& InOwningEmitter, UNiagaraScript& InOwningScript, UNiagaraNodeFunctionCall& InFunctionCallNode, int32 InStackIndex)
{
	...
	UNiagaraNodeOutput* FunctionCallOutputNode = FNiagaraStackGraphUtilities::GetEmitterOutputNodeForStackNode(*FunctionCallNode);
	if (FunctionCallOutputNode == nullptr)
	{
		UE_LOGF(LogNiagaraEditor, Warning, "FunctionCallOutputNode is nullptr in FNiagaraStackFunctionMergeAdapter, results may be incorrect");
	}
	FCompileConstantResolver ConstantResolver = FunctionCallOutputNode ? FCompileConstantResolver(InOwningEmitter, FunctionCallOutputNode->GetUsage()) : FCompileConstantResolver();
...
INiagaraMergeManager::FMergeEmitterResults FNiagaraScriptMergeManager::MergeEmitter(const FVersionedNiagaraEmitter& Parent, const FVersionedNiagaraEmitter& ParentAtLastMerge, const FVersionedNiagaraEmitter& Instance) const
{
	...
	else if (State == EmitterMergeState::CopyProperties) //-V547
	{
		...
		// Make sure we resync to interpolated spawning usage as it could have been adjusted previously and it needs to reflect the emitter not the parent
		if (FVersionedNiagaraEmitterData* MergedDataForUsageSync = VersionedMergedInstance.GetEmitterData())
		{
			if (UNiagaraScript* MergedSpawnScript = MergedDataForUsageSync->SpawnScriptProps.Script)
			{
				const ENiagaraScriptUsage RequiredUsage = MergedDataForUsageSync->UsesInterpolatedSpawning() ? ENiagaraScriptUsage::ParticleSpawnScriptInterpolated : ENiagaraScriptUsage::ParticleSpawnScript;
				if (MergedSpawnScript->GetUsage() != RequiredUsage)
				{
					MergedSpawnScript->Modify();
					MergedSpawnScript->SetUsage(RequiredUsage);
					if (MergedDataForUsageSync->GraphSource)
					{
						MergedDataForUsageSync->GraphSource->MarkNotSynchronized(TEXT("Resync SpawnScript Usage with InterpolatedSpawnMode after merge"));
					}
				}
			}
		}
...
#if 0
		UE_LOGF(LogNiagaraEditor, Log, "A");
		//for (FNiagaraEmitterHandle& EmitterHandle : EmitterHandles)
		{
			...
			UE_LOGF(LogNiagaraEditor, Log, "Spawn Parameters");
			SpawnScript->GetVMExecutableData().Parameters.DumpParameters();
			UE_LOGF(LogNiagaraEditor, Log, "Spawn RI Parameters");
			SpawnScript->RapidIterationParameters.DumpParameters();
			UE_LOGF(LogNiagaraEditor, Log, "Update Parameters");
			UpdateScript->GetVMExecutableData().Parameters.DumpParameters();
			UE_LOGF(LogNiagaraEditor, Log, "Update RI Parameters");
...
#if 0
		UE_LOGF(LogNiagaraEditor, Log, "B");
		//for (FNiagaraEmitterHandle& EmitterHandle : EmitterHandles)
		{
			//UE_LOGF(LogNiagara, Log, "Emitter Handle: %ls", *EmitterHandle.GetUniqueInstanceName());
			UNiagaraScript* UpdateScript = MergedInstance->GetScript(ENiagaraScriptUsage::ParticleUpdateScript, FGuid());
...
#if 0
		UE_LOGF(LogNiagaraEditor, Log, "C");
		//for (FNiagaraEmitterHandle& EmitterHandle : EmitterHandles)
		{
			//UE_LOGF(LogNiagara, Log, "Emitter Handle: %ls", *EmitterHandle.GetUniqueInstanceName());
			UNiagaraScript* UpdateScript = MergedInstance->GetScript(ENiagaraScriptUsage::ParticleUpdateScript, FGuid());
			...
			if(ParentIdentity.IsValid())
			{
				...
				else
				{
					...
					if(BaseRoot->FindChildWithIdentity(AddedInput->GetPersistentIdentity(), true) == nullptr)
					{
						...
						if(NewChild == nullptr)
						{
							UE_LOGF(LogNiagaraEditor, Log, "Item %ls could not be added during merge process", *AddedInput->ToString());
						}
						else
						{
							// see above
