...
void FNiagaraEditorUtilities::WriteTextFileToDisk(FString SaveDirectory, FString FileName, FString TextToSave, bool bAllowOverwriting)
{
	...
	if (PlatformFile.CreateDirectoryTree(*SaveDirectory))
	{
		...
		if (bAllowOverwriting || !PlatformFile.FileExists(*AbsoluteFilePath))
		{
			...
			{
				UE_LOGF(LogNiagaraEditor, Log, "Wrote file to %ls", *AbsoluteFilePath);
				return;
			}
	...
	for (TFieldIterator<FProperty> PropertyIt(Struct, IteratorFlags, EFieldIteratorFlags::ExcludeDeprecated); PropertyIt; ++PropertyIt)
	{
		...
		else if (Property->IsA(FMapProperty::StaticClass()))
		{
			...
			if (MapHelper.GetKeyProperty())
			{
				...
				if (MapHelper.GetKeyProperty()->GetClass() == FNameProperty::StaticClass())
				{
					...
					if (MapHelper.GetValueProperty()->IsA(FStructProperty::StaticClass()))
					{
						...
						for (FScriptMapHelper::FIterator It(MapHelper); It; ++It)
						{
							...
							{
								UE_LOGF(LogNiagaraEditor, Warning, "Skipping %ls because it is a map value property of unsupported underlying type, please add \"meta = (SkipForCompileHash=\"true\")\" to avoid this warning in the future or handle it yourself in NestedPropertiesAppendCompileHash!", *Property->GetName());
							}
						}
					}
					else
					{
						for (FScriptMapHelper::FIterator It(MapHelper); It; ++It)
						{
							...
							{
								UE_LOGF(LogNiagaraEditor, Warning, "Skipping %ls because it is a map value property of unsupported underlying type, please add \"meta = (SkipForCompileHash=\"true\")\" to avoid this warning in the future or handle it yourself in PODPropertyAppendCompileHash!", *Property->GetName());
							}
						}
					}
				}
				...
				{
					UE_LOGF(LogNiagaraEditor, Warning, "Skipping %ls because it is a map property of unsupported key type, please add \"meta = (SkipForCompileHash=\"true\")\" to avoid this warning in the future or handle it yourself in NestedPropertiesAppendCompileHash!", *Property->GetName());
				}
			}
		}
		else if (Property->IsA(FArrayProperty::StaticClass()))
		{
			...
			if (CastProp->Inner->IsA(FStructProperty::StaticClass()))
			{
				...
				for (int32 ArrayIdx = 0; ArrayIdx < ArrayHelper.Num(); ArrayIdx++)
				{
					...
					{
						UE_LOGF(LogNiagaraEditor, Warning, "Skipping %ls because it is an array property of unsupported underlying type, please add \"meta = (SkipForCompileHash=\"true\")\" to avoid this warning in the future or handle it yourself in NestedPropertiesAppendCompileHash!", *Property->GetName());
					}
				}
			}
			else
		...
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Skipping %ls because it is a UText property, please add \"meta = (SkipForCompileHash=\"true\")\" to avoid this warning in the future or handle it yourself in NestedPropertiesAppendCompileHash!", *Property->GetName());
			return true;
		}
		else
		{
...
{
	if (ensureMsgf(Node->GetOutermost() == GetTransientPackage(), TEXT("Can not fix up numerics on non-transient node %s"), *Node->GetPathName()) == false)
	{
		return;
	}
...
{
	if (ensureMsgf(Graph->GetOutermost() == GetTransientPackage(), TEXT("Can not preprocess non-transient function graph %s"), *Graph->GetPathName()) == false)
	{
		return;
	}
...
TTuple<EScriptSource, FText> FNiagaraEditorUtilities::GetScriptSource(const FAssetData& ScriptAssetData)
{
	...
	const auto ResolvePluginName =
		[PackagePathLocal]() -> TOptional<FText>
		{
			int32 ContentFoundIndex = PackagePathLocal.Find(TEXT("/Content"));
			if (ContentFoundIndex != INDEX_NONE)
			{
				const FString LeftPart = PackagePathLocal.Left(ContentFoundIndex);
				const bool bFound = LeftPart.FindLastChar('/', ContentFoundIndex);

				if (bFound)
				{
					const FString PluginName = LeftPart.RightChop(ContentFoundIndex + 1);
	...
	{
		TOptional<FText> PluginName = ResolvePluginName();
		if (PluginName.IsSet())
		{
			return TTuple<EScriptSource, FText>(EScriptSource::Niagara, PluginName.GetValue());
		}
	}
	...
	if(FPaths::IsUnderDirectory(PackagePathLocal, FPaths::EnginePluginsDir()) || FPaths::IsUnderDirectory(PackagePathLocal, FPaths::ProjectPluginsDir()))
	{
		...
		if (PluginName.IsSet())
		{
			return TTuple<EScriptSource, FText>(EScriptSource::Plugins, PluginName.GetValue());
		}
	}

	if (FPaths::IsRestrictedPath(PackagePathLocal))
	{
		const TArray<FString> PluginSourcePaths = { FPaths::ProjectDir(), FPaths::EngineDir() };
		for (const FString& RestrictedPath : FPaths::GetRestrictedFolderNames())
		{
			for (const FString& PluginSourcePath : PluginSourcePaths)
			{
				const FString& RestrictedPluginPath = FPaths::Combine(PluginSourcePath, TEXT("Restricted/"), RestrictedPath, TEXT("Plugins/"));
				if (FPaths::IsUnderDirectory(PackagePathLocal, RestrictedPluginPath))
				{
					TOptional<FText> PluginName = ResolvePluginName();
					if (PluginName.IsSet())
					{
...
const UNiagaraScriptVariable* FNiagaraEditorUtilities::UserParameters::FindScriptVariableForUserParameter(const FGuid& UserParameterGuid, const UNiagaraSystem& System)
{
 ...
 const UNiagaraScriptVariable* FNiagaraEditorUtilities::UserParameters::FindScriptVariableForUserParameter(const FNiagaraVariable& UserParameter, const UNiagaraSystem& System)
 {
	return Cast<UNiagaraSystemEditorData>(System.GetEditorData())->FindUserScriptVariable(UserParameter, System);
 }
...
void FNiagaraEditorUtilities::WarnWithToastAndLog(FText WarningMessage)
{
	...
	UE_LOGF(LogNiagaraEditor, Warning, "%ls", *WarningMessage.ToString());
}
...
void FNiagaraEditorUtilities::InfoWithToastAndLog(FText InfoMessage, float ToastDuration)
{
	...
	FSlateNotificationManager::Get().AddNotification(WarningNotification);
	UE_LOGF(LogNiagaraEditor, Log, "%ls", *InfoMessage.ToString());
}
...
UNiagaraClipboardContent* FNiagaraEditorUtilities::RunPythonConversionScript(FVersionedNiagaraScriptData& NewScriptVersionData, UNiagaraClipboardContent* NewScriptInputs, FVersionedNiagaraScriptData& OldScriptVersionData, UNiagaraClipboardContent* OldScriptInputs, FText& OutWarnings)
{
	...
	{
		UE_LOGF(LogNiagaraEditor, Error, "Unable to save python script to file %ls", *TempScriptFile);
...
void FNiagaraEditorUtilities::RunPythonUpgradeScripts(UNiagaraNodeFunctionCall* SourceNode,	const TArray<FVersionedNiagaraScriptData*>& UpgradeVersionData, const FNiagaraScriptVersionUpgradeContext& UpgradeContext, FString& OutWarnings)
{
	...
	for (int i = 1; i < UpgradeVersionData.Num(); i++)
	{
		...
		UNiagaraVersionUpgradeBase* VersionUpdateUtility = NewData->UpdateUtilityClass.GetDefaultObject();
		if (PythonScript.IsEmpty() && VersionUpdateUtility == nullptr)
		{
			continue;
		}

		bool bLoggedWarning = false;
		bool bIsPythonAvailable = IPythonScriptPlugin::Get()->IsPythonAvailable() && !PythonScript.IsEmpty();
		if (!bIsPythonAvailable && VersionUpdateUtility == nullptr)
		{
			UE_LOGF(LogNiagaraEditor, Log, "Python is not available update script skipped");
		...
		{
			NiagaraVersionUpgrade::ScriptInputs OldInputs;
			OldInputs.Version = PreviousData->Version;
			OldInputs.FunctionInputs = Results->CopyOldInputs();
			
			NiagaraVersionUpgrade::ScriptInputs NewInputs;
			NewInputs.Version = NewData->Version;
			NewInputs.FunctionInputs = Results->CopyNewInputs();
			
			FText OutMessage;
			if (VersionUpdateUtility->ConvertScript(OldInputs, NewInputs, OutMessage))
			{
				UNiagaraClipboardContent* ClipboardContent = UNiagaraClipboardContent::Create();
				for (UNiagaraClipboardFunctionInput* ModuleInput : NewInputs.FunctionInputs)
				{
					ClipboardContent->FunctionInputs.Add(ModuleInput);
				}
				if (ClipboardContent->FunctionInputs.Num() > 0)
				{
					FText Warnings;
			...
			{
				UE_LOGFMT(LogNiagaraEditor, Error, "Script upgrade ended with error! {ErrorMessage}", OutMessage.ToString());
				AddStackWarning(PreviousData->Version, NewData->Version, OutMessage.ToString(), bLoggedWarning, OutWarnings);
			}
		}
	}
	...
	if (OuterInputNode != nullptr)
	{
		...
		if (ensureMsgf(CompiledScripts.Num() > 0, TEXT("Could not find compiled scripts for data interface input node.")))
		{
			for (const UNiagaraScript* CompiledScript : CompiledScripts)
			{
				...
				const FNiagaraEmitterHandle* EmitterHandleForScript = UNiagaraScript::IsSystemScript(CompiledScript->GetUsage()) ? nullptr : EmitterHandle;
				const FNiagaraScriptRuntimeData* ScriptRuntimeData = OwningSystem.GetScriptRuntimeData(EmitterHandleForScript, *CompiledScript).Get();
				if (ScriptRuntimeData != nullptr)
				{
					for (const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDI : ScriptRuntimeData->GetResolvedDataInterfaces())
					{
...
namespace FNiagaraEditorUtilities::UserParameters::Internal
{
	TSharedRef<FNiagaraSystemViewModel> CreateDataProcessingOnlyViewModel(UNiagaraSystem& InSystem)
	{
		...
		SystemViewModel->Initialize(InSystem, Options);
		return SystemViewModel;
	}
}

void FNiagaraEditorUtilities::UserParameters::AddUserParameterToSystem(UNiagaraSystem& System, const FNiagaraVariableBase& UserParameter)
{
	...
	{
		SystemViewModel = Internal::CreateDataProcessingOnlyViewModel(System);
	}
	SystemViewModel->AddParameter(UserParameter);
}

void FNiagaraEditorUtilities::UserParameters::RemoveUserParameterFromSystem(UNiagaraSystem& System, const FNiagaraVariableBase& UserParameter)
{
	TSharedPtr<FNiagaraSystemViewModel> SystemViewModel = TNiagaraViewModelManager<UNiagaraSystem, FNiagaraSystemViewModel>::GetExistingViewModelForObject(&System);
	if (SystemViewModel.IsValid() == false)
	{
	...
	SystemViewModel->DeleteParameter(UserParameter);
}

void FNiagaraEditorUtilities::UserParameters::RenameUserParameterForSystem(UNiagaraSystem& System, const FNiagaraVariableBase& OldParameter, const FNiagaraVariableBase& NewParameter)
{
	TSharedPtr<FNiagaraSystemViewModel> SystemViewModel = TNiagaraViewModelManager<UNiagaraSystem, FNiagaraSystemViewModel>::GetExistingViewModelForObject(&System);
	if (SystemViewModel.IsValid() == false)
	{
	...
	SystemViewModel->RenameParameter(OldParameter, NewParameter.GetName(), ENiagaraGetGraphParameterReferencesMode::AllGraphs);
}

void FNiagaraEditorUtilities::UserParameters::GetUserParameterDefaultForSystem(const UNiagaraSystem& System, const FNiagaraVariableWithOffset& UserParameterWithOffset, FNiagaraVariant& OutDefault)
{
	if (UserParameterWithOffset.IsDataInterface())
	{
		OutDefault.SetDataInterface(System.GetExposedParameters().GetDataInterface(UserParameterWithOffset));
	}
	else if (UserParameterWithOffset.IsUObject())
	{
		OutDefault.SetUObject(System.GetExposedParameters().GetUObject(UserParameterWithOffset));
	}
	else
	{
		OutDefault.SetBytes(System.GetExposedParameters().GetParameterData(UserParameterWithOffset), UserParameterWithOffset.GetType().GetSize());
	}
}

TOptional<FNiagaraVariant> FNiagaraEditorUtilities::UserParameters::FindUserParameterDefaultForSystem(const UNiagaraSystem& System, const FNiagaraVariableBase& UserParameter)
{
	int32 ParameterIndex = System.GetExposedParameters().IndexOf(UserParameter);
	if (ParameterIndex != INDEX_NONE)
	{
		FNiagaraVariableWithOffset UserParameterWithOffset = FNiagaraVariableWithOffset(UserParameter, ParameterIndex, FNiagaraLwcStructConverter());
		FNiagaraVariant Default;
		GetUserParameterDefaultForSystem(System, UserParameterWithOffset, Default);
		return Default;
	}
	return TOptional<FNiagaraVariant>();
}

void FNiagaraEditorUtilities::UserParameters::SetUserParameterDefaultForSystem(UNiagaraSystem& System, const FNiagaraVariableBase& UserParameter, const FNiagaraVariant& Default)
{
	TSharedPtr<FNiagaraSystemViewModel> SystemViewModel = TNiagaraViewModelManager<UNiagaraSystem, FNiagaraSystemViewModel>::GetExistingViewModelForObject(&System);
	if (SystemViewModel.IsValid() == false)
	{
	...
	SystemViewModel->SetParameterDefaultValue(UserParameter, Default);
}
...
TMap<FGuid, TArray<FNiagaraVariableBase>> FNiagaraEditorUtilities::Scripts::Validation::ValidateScriptVariableIds(UNiagaraScript* Script, FGuid VersionGuid)
{
	...
	if(UNiagaraScriptSourceBase* SourceBase = Script->GetSource(VersionGuid))
	{
		...
		if(bReturnResults)
		{
			UE_LOGF(LogNiagaraEditor, Error, "---- Duplicate variable guids found in script %ls, Version %ls ---- :", *Script->GetName(), *VersionGuid.ToString());
			for(const auto& Result : Results)
			{
				if(Result.Value.Num() > 1)
				{
					...
					UE_LOGF(LogNiagaraEditor, Error, "%ls", *ResultString);
				}
			}
	...
	UE_LOGF(LogNiagaraEditor, Log, "---- No duplicate variable guids in script %ls, version %ls ----", *Script->GetName(), *VersionGuid.ToString());
	return {};
}
...
TMap<FNiagaraVariableBase, FGuid> FNiagaraEditorUtilities::Scripts::Validation::FixupDuplicateScriptVariableGuids(UNiagaraScript* Script)
{
	...
	{
		UE_LOGF(LogNiagaraEditor, Log, "----------- Variables with new guids in script %ls ---------", *Script->GetName());
	}
	
	for(const auto& RemappedGuid : RemappedGuids)
	{
		UE_LOGF(LogNiagaraEditor, Log, "Variable: %ls. Guid: %ls -> %ls", *RemappedGuid.Key.GetName().ToString(), *OldGuids[RemappedGuid.Key].ToString(), *RemappedGuids[RemappedGuid.Key].ToString())
	}
