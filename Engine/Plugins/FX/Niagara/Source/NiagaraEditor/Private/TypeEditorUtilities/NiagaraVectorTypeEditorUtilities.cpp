...
FString FNiagaraEditorVector2TypeUtilities::GetPinDefaultStringFromValue(const FNiagaraVariable& AllocatedVariable) const
{
	...
	// NOTE: We can not use ToString() here since the vector pin control doesn't use the standard 'X=0,Y=0' syntax. The string parsing breaks without the parentesis which are not added by default.
	FVector2f Value = AllocatedVariable.GetValue<FVector2f>();
