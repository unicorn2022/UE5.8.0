...
static void ForEachEmitterInContext(const FNiagaraValidationContext& Context, TFunctionRef<void(const FVersionedNiagaraEmitter&, UNiagaraStackEmitterPropertiesItem*)> Callback)
{
	...
	{
		const FVersionedNiagaraEmitter VersionedEmitter = EmitterHandleViewModel->GetEmitterHandle()->GetInstance();
		if (IsValid(VersionedEmitter.Emitter) == false)
		{
			continue;
		}

		UNiagaraStackEmitterPropertiesItem* EmitterProperties = NiagaraValidation::GetStackEntry<UNiagaraStackEmitterPropertiesItem>(EmitterHandleViewModel->GetEmitterStackViewModel());
		Callback(VersionedEmitter, EmitterProperties);
	}
}
...
TSharedPtr<FNiagaraEmitterHandleViewModel> NiagaraValidation::GetEmitterViewModel(const FNiagaraValidationContext& Context, const UNiagaraEmitter* NiagaraEmitter)
{
...
void UNiagaraValidationRule_NoWarmupTime::CheckValidity(const FNiagaraValidationContext& Context, TArray<FNiagaraValidationResult>& Results)  const
{
	...
	if (System.NeedsWarmup() == false)
	{
		return;
	}

	FNiagaraValidationResult& Result = Results.Emplace_GetRef(
		Severity,
		LOCTEXT("WarumupSummary", "Warmup is enabled and not allowed."),
		LOCTEXT("WarmupDescription", "Warmup is enabled on this system and it not allowed due to in game performance cost."),
	...
	Result.Fixes.Emplace(
		LOCTEXT("WarmupSetToZero", "Set Warmup Time to 0"),
		FNiagaraValidationFixDelegate::CreateLambda(
			[WeakSystem = MakeWeakObjectPtr(&System)]()
			{
				if (UNiagaraSystem* NiagaraSystem = WeakSystem.Get())
				{
					const FScopedTransaction Transaction(LOCTEXT("SetWarmupToZero", "Set Warmup Time to 0.0"));
					NiagaraSystem->Modify();
					NiagaraSystem->SetWarmupTime(0.0f);
				}
			}
		)
	);
...
void UNiagaraValidationRule_BannedDataInterfaces::CheckValidity(const FNiagaraValidationContext& Context, TArray<FNiagaraValidationResult>& Results)  const
{
	...
	FNiagaraDataInterfaceUtilities::ForEachDataInterface(
		...
		[&](const FNiagaraDataInterfaceUtilities::FDataInterfaceUsageContext& UsageContext) -> bool
		{
			...
			if (const UNiagaraEmitter* NiagaraEmitter = Cast<UNiagaraEmitter>(UsageContext.OwnerObject))
			{
...
void UNiagaraValidationRule_GpuUsage::CheckValidity(const FNiagaraValidationContext& Context, TArray<FNiagaraValidationResult>& OutResults) const
{
	...
	for (TSharedRef<FNiagaraEmitterHandleViewModel> EmitterHandleModel : Context.ViewModel->GetEmitterHandleViewModels())
	{
		...
		FNiagaraValidationResult& ValidationResult = OutResults.Emplace_GetRef(
			Severity,
			LOCTEXT("GpuUsageInfo", "GPU compute may not be available."),
			FText::Format(LOCTEXT("GpuUsageInfoDetails", "GPU compute may not be enabled on platforms '{0}' consider making a fallback CPU emitter."), FText::FromString(PlatformConflictsString)),
		...
		for (UNiagaraStackFunctionInput* Input : StackInputs)
		{
			if (Input->GetInputType() == FNiagaraTypeDefinition::GetPositionDef())
			{
				...
				if ((Input->GetValueMode() == UNiagaraStackFunctionInput::EValueMode::Linked ||
					Input->GetValueMode() == UNiagaraStackFunctionInput::EValueMode::LinkedShared) && Settings->bEnforceStrictStackTypes)
				{
...
void UNiagaraValidationRule_TickDependencyCheck::CheckValidity(const FNiagaraValidationContext& Context, TArray<FNiagaraValidationResult>& OutResults) const
{
	...
	NiagaraSystem->ForEachScriptWithRuntimeData(
		[&](const UNiagaraScript* Script, const FNiagaraScriptRuntimeData* ScriptRuntimeData)
		{
...
void UNiagaraValidationRule_RequireLatestParentEmitterVersion::CheckValidity(const FNiagaraValidationContext& Context, TArray<FNiagaraValidationResult>& OutResults) const
{
	ForEachEmitterInContext(Context, [&, this](const FVersionedNiagaraEmitter& VersionedEmitter, UNiagaraStackEmitterPropertiesItem* EmitterProperties)
	{
		VersionedEmitter.Emitter->ForEachVersionData([&, this](const FVersionedNiagaraEmitterData& VersionedEmitterData)
		{
			const UNiagaraEmitter* ParentEmitter = VersionedEmitterData.VersionedParent.Emitter.Get();
			if (IsValid(ParentEmitter) == false)
			{
				return;
			}

			const FGuid UsedParentVersionGuid = VersionedEmitterData.VersionedParent.Version;
			const FVersionedNiagaraEmitterData* UsedParentVersionData = ParentEmitter->GetEmitterData(UsedParentVersionGuid);
			if (ensureMsgf(UsedParentVersionData != nullptr, TEXT("Using unknown or invalid parent emitter version")) == false)
			{
				return;
			}

			if (const FVersionedNiagaraEmitterData* LatestParentVersionData = VersionedEmitterData.VersionedParent.Emitter->GetLatestEmitterData())
			{
				if (LatestParentVersionData->Version <= UsedParentVersionData->Version)
				{
					return;
				}
				...
				FNiagaraValidationResult& Result = OutResults.AddDefaulted_GetRef();
				Result.Severity = Severity;
				Result.SummaryText = FText::Format(LOCTEXT("NotLatestParentVersion_Title", "Not using latest version of parent emitter '{0}'"), FText::FromName(*GetPathNameSafe(ParentEmitter)));
				Result.Description = FText::Format(LOCTEXT("NotLatestParentVersion_Description", "Using version '{0}.{1}', but latest is '{2}.{3}'. Please consider updating to the latest version"),
					UsedParentVersionData->Version.MajorVersion, UsedParentVersionData->Version.MinorVersion,
					LatestParentVersionData->Version.MajorVersion, LatestParentVersionData->Version.MinorVersion);
				Result.SourceObject = TWeakObjectPtr { EmitterProperties };

				FNiagaraValidationFix& UpdateToLatestParentFix = Result.Fixes.AddDefaulted_GetRef();
				UpdateToLatestParentFix.Description = LOCTEXT("UpgradeToLatestParentVersion", "Upgrade to latest parent version.");
				UpdateToLatestParentFix.FixDelegate = FNiagaraValidationFixDelegate::CreateWeakLambda(EmitterProperties, [EmitterProperties, LatestParentVersionGuid = LatestParentVersionData->Version.VersionGuid]()
				{
					TSharedRef<FNiagaraEmitterViewModel> EmitterViewModel = EmitterProperties->GetEmitterViewModel().ToSharedRef();
					TSharedRef<FNiagaraSystemViewModel> SystemViewModel = EmitterProperties->GetSystemViewModel();
					FNiagaraEditorUtilities::SwitchParentEmitterVersion(EmitterViewModel, SystemViewModel, LatestParentVersionGuid);
				});
			}
		});
	});
...
static bool ObjectIsInSoftObjectArray(const TArray<TSoftObjectPtr<ObjectType>>& ObjectArray, const ObjectType* ObjectToFind)
{
	const TSoftObjectPtr<const ObjectType> SoftObjectToFindPtr { ObjectToFind };
	return ObjectArray.Contains(SoftObjectToFindPtr);
}

template<class T>
static FString JoinSoftObjectArrayAsString(const TArray<TSoftObjectPtr<T>>& ObjectArray, const TCHAR* Separator)
{
	FString Result;
	...
	Result = ObjectArray[0].ToString();
	for (int32 Idx = 1; Idx < ObjectArray.Num(); ++Idx)
	{
		Result += Separator + ObjectArray[Idx].ToString();
	}

	return Result;
}

void UNiagaraValidationRule_RequireParentEmitter::CheckValidity(const FNiagaraValidationContext& Context, TArray<FNiagaraValidationResult>& OutResults) const
{
	if (AllowedParentEmitters.IsEmpty())
	{
		return;
	}

	const FString AllowedParentEmitterString = JoinSoftObjectArrayAsString(AllowedParentEmitters, TEXT(", "));
	const FText AllowedParentEmittersText = FText::FromString(AllowedParentEmitterString);

	ForEachEmitterInContext(Context, [&, this](const FVersionedNiagaraEmitter& VersionedEmitter, UNiagaraStackEmitterPropertiesItem* EmitterProperties)
	{
		VersionedEmitter.Emitter->ForEachVersionData([&, this](const FVersionedNiagaraEmitterData& VersionedEmitterData)
		{
			// Ignore lightweight (stateless) emitters
			TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel = Context.ViewModel->GetEmitterHandleViewModelForEmitter(VersionedEmitter);
			if (EmitterHandleViewModel.IsValid() &&
			    EmitterHandleViewModel->GetEmitterHandle() != nullptr &&
			    EmitterHandleViewModel->GetEmitterHandle()->GetEmitterMode() == ENiagaraEmitterMode::Stateless)
			{
				return;
			}
			...
			if (IsValid(ParentEmitter) == false)
			{
				if (bMustHaveParentEmitter)
				{
					FNiagaraValidationResult& Result = OutResults.AddDefaulted_GetRef();
					Result.Severity = Severity;
					Result.SummaryText = LOCTEXT("NoParentEmitter_Title", "No parent emitter set");
					Result.Description = LOCTEXT("NoParentEmitter_Description", "The emitter does not have a parent emitter. Please set one");
					Result.SourceObject = TWeakObjectPtr { EmitterProperties };
				}
				return;
			}

			if (ObjectIsInSoftObjectArray(AllowedParentEmitters, ParentEmitter))
			{
				return;
			}
			...
			FNiagaraValidationResult& Result = OutResults.AddDefaulted_GetRef();
			Result.Severity = Severity;
			Result.SummaryText = FText::Format(LOCTEXT("InvalidParentEmitter_Title", "Parent emitter '{0}' is not allowed"), FText::FromName(*GetPathNameSafe(ParentEmitter)));
			Result.Description = FText::Format(LOCTEXT("InvalidParentEmitter_Description", "Please recreate the emitter using one of the following as the parent: {0}"), AllowedParentEmittersText);
			Result.SourceObject = TWeakObjectPtr { EmitterProperties };
		});
	});
}
