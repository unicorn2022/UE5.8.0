...
void UNiagaraNodeOp::PostLoad()
{
	...
	{
		UE_LOGF(LogNiagaraEditor, Log, "OpNode: Converted %ls to %ls, Package: %ls", *OriginalOpName.ToString(), *OpName.ToString(), *GetOutermost()->GetName());
	}
		...
		if (OpInfo && OpInfo->StaticVariableResolveFunction.IsBound())
		{
			...
			if (bAllPinsStatic)
			{
				...
				if (Vars.Num() > 0)
				{
					...
					{
						UE_LOGF(LogNiagaraEditor, Log, "Inputs Static Node Op: %ls", *GetNodeTitle(ENodeTitleType::FullTitle).ToString());
