...
void FNiagaraCompilationGraph::UpdateFunctionAliasByContext(FNiagaraGraphFunctionAliasContext& FunctionAliasContext) const
{
	...
	for (const TUniquePtr<FNiagaraCompilationNode>& Node : Nodes)
	{
		switch (Node->NodeType)
		{
			case FNiagaraCompilationNode::ENodeType::StaticSwitch:
			{
				...
				if (!bSetByCompiler)
				{
					UE_LOGF(LogNiagaraEditor, Warning, "Encountered StaticSwitch that should have been resolved.  Graph will likely fail to compile.  Task: %ls Node %ls",
	...
	if (InNode)
	{
		...
		for (int32 PinIt = 0; PinIt < PinCount; ++PinIt)
		{
			...
			if (Pin->bOrphanedPin)
			{
				...
				UE_LOGF(LogNiagaraEditor, Warning, "Node pin is no longer valid.  This pin must be disconnected or reset to default so it can be removed.");
				continue;
			}
...
void FNiagaraCompilationNodeInput::UpdateFunctionAliasForContext(FNiagaraGraphFunctionAliasContext& InOutFunctionAliasContext) const
{
	if (Usage == ENiagaraInputNodeUsage::TranslatorConstant && InputVariable == TRANSLATOR_PARAM_CALL_ID)
	{
		...
		static const FName NAME_ScriptUsage = TEXT("ScriptUsage");
		InOutFunctionAliasContext.AddPermutationIdentifier(NAME_ScriptUsage, FString::FormatAsNumber(static_cast<uint8>(InOutFunctionAliasContext.ScriptUsage)));
	}
}
	...
	if (OpInfo && OpInfo->StaticVariableResolveFunction.IsBound())
	{
		...
		if (bAllPinsStatic)
		{
			...
			if (Vars.Num() > 0)
			{
				...
				{
					UE_LOGF(LogNiagaraEditor, Log, "Inputs Static Node Op: %ls", *FullTitle);
					for (int32 i = 0; i < Vars.Num(); i++)
					{
						UE_LOGF(LogNiagaraEditor, Log, "[%d] %d", i, Vars[i]);
					}
					UE_LOGF(LogNiagaraEditor, Log, "Result: %d", Result);
				}
	...
	for (const FNiagaraCompilationInputPin& InputPin : InputPins)
	{
		...
		else if (InputPin.Variable.GetType() == ParameterMapType)
		{
			if (InputPin.LinkedTo)
			{
				...
				if (UNiagaraScript::LogCompileStaticVars > 0)
				{
					UE_LOGF(LogNiagaraEditor, Log, "Build Parameter Map History: NiagaraCompilationNodeOutput %ls PMapIdx: %d", *FullTitle, ParamMapIdx);
				}
...
void FNiagaraCompilationNodeStaticSwitch::BuildParameterMapHistory(FParameterMapHistoryBuilder& Builder, bool bRecursive, bool bFilterForCompilation) const
{
	...
	if (bSetByPin)
	{
		...
		if (UNiagaraScript::LogCompileStaticVars != 0)
		{
			UE_LOGF(LogNiagaraEditor, Log, "Static Switch Value: %d  Name: \"%ls\"  SwitchInputParameterName:\"%ls\"", SelectorValue, *FullTitle, *InputParameterName.ToString());
		}
	}
...
void FNiagaraCompilationNodeStaticSwitch::Compile(FTranslator* Translator, TArray<int32>& Outputs) const
{
	...
	if (bSetByCompiler)
	{
		...
		if (const FNiagaraVariable* Found = FNiagaraConstants::FindStaticSwitchConstant(SwitchConstant))
		{
			FNiagaraVariable Constant = *Found;
			if (ConstantResolver.ResolveConstant(Constant))
			{
...
void FNiagaraCompilationNodeStaticSwitch::UpdateFunctionAliasForContext(FNiagaraGraphFunctionAliasContext& InOutFunctionAliasContext) const
{
	if (bSetByCompiler)
	{
		if (const FNiagaraVariable* Found = FNiagaraConstants::FindStaticSwitchConstant(SwitchConstant))
		{
			InOutFunctionAliasContext.AddPendingStaticSwitchVariable(*Found, static_cast<uint8>(SwitchType));
		}
	}
}
...
void FNiagaraCompilationNodeUsageSelector::UpdateFunctionAliasForContext(FNiagaraGraphFunctionAliasContext& InOutFunctionAliasContext) const
{
	static const FName NAME_Usage = TEXT("Usage");
	switch (InOutFunctionAliasContext.CompileUsage)
	{
		case ENiagaraScriptUsage::SystemSpawnScript:
		case ENiagaraScriptUsage::SystemUpdateScript:
			InOutFunctionAliasContext.AddPermutationIdentifier(NAME_Usage, TEXT("System"));
			break;
		case ENiagaraScriptUsage::EmitterSpawnScript:
		case ENiagaraScriptUsage::EmitterUpdateScript:
			InOutFunctionAliasContext.AddPermutationIdentifier(NAME_Usage, TEXT("Emitter"));
			break;
		...
		case ENiagaraScriptUsage::ParticleGPUComputeScript:
			InOutFunctionAliasContext.AddPermutationIdentifier(NAME_Usage, TEXT("Particle"));
			break;
	}
}
