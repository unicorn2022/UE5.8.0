...
void UNiagaraNode::PostLoad()
{
	...
	for (UEdGraphPin* Pin : Pins)
	{
		if(Pin == nullptr)
		{
			...
			UE_LOGF(LogNiagaraEditor, Log, "Pin of node %ls in asset %ls is nullptr!", *NodeName, *AssetPath);
			continue;
		}
		if(Pin->LinkedTo.Num() > 0)
		{
			for(UEdGraphPin* LinkedPin : Pin->LinkedTo)
			{
				if(LinkedPin == nullptr)
				{
					...
					FString ScriptUsageName = TEXT("Unknown Usage");
					FString AssetPath = "Undetermined";
					FString PinName = Pin->PinName.ToString();
					FString NodeName = "Undetermined";

					if (UNiagaraScriptSource* ScriptSource = GetSource())
					{
						if (UNiagaraScript* Script = ScriptSource->GetTypedOuter<UNiagaraScript>())
						{
							const UEnum* UsageEnum = StaticEnum<ENiagaraScriptUsage>();
							ScriptUsageName = UsageEnum->GetDisplayNameTextByValue(int64(Script->GetUsage())).ToString();
						}
					}

					if ( OwningGraph )
					{
						AssetPath = OwningGraph->GetPathName();
					}

					FString OwningNodeDetailed;
					if (UEdGraphNode* OwningNode = Pin->GetOwningNode())
					{
						NodeName = FString::Printf(TEXT("%s:%s"), *GetNameSafe(OwningNode->GetClass()), *OwningNode->GetNodeTitle(ENodeTitleType::FullTitle).ToString());

						OwningNodeDetailed.Appendf(TEXT("\nOwningNodePath - %s"), *GetFullNameSafe(OwningNode));
						for (int32 i=0; i < OwningNode->Pins.Num(); ++i)
						{
							if (UEdGraphPin* OwningNodePin = OwningNode->Pins[i])
							{
								OwningNodeDetailed.Appendf(TEXT("\nPin[%d] - %s | %s"), i, *OwningNodePin->GetDisplayName().ToString(), *OwningNodePin->GetFName().ToString());
							}
							else
							{
								OwningNodeDetailed.Appendf(TEXT("\nPin[%d] - nullptr"), i);
					...
					UE_LOGF(LogNiagaraEditor, Warning, "Connected pin of Pin '%ls' in '%ls' of node '%ls' in asset '%ls' is nullptr!%ls", *PinName, *ScriptUsageName, *NodeName, *AssetPath, *OwningNodeDetailed);
				}
			}
		}
...
bool UNiagaraNode::PinAppendCompileHash(const UEdGraphPin* InPin, FNiagaraCompileHashVisitor* InVisitor) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogNiagaraEditor, Warning, "AppendCompileHash can't handle default objects properly, please investigate!");
		}
	}
...
{
	//UE_LOGF(LogNiagaraEditor, Warning, "SetPinDefaultToTypeDefaultIfUnset");
	...
	for (int32 OldPinIndex = 0; OldPinIndex < OldPins.Num(); ++OldPinIndex)
	{
		...
		if (MatchingNewPin)
		{
			...
			if (OldPin->bOrphanedPin)
			{
				...
				UE_LOGF(LogNiagaraEditor, Log, "Pin \"%ls\" in node \"%ls\" was orphaned, but is now matched. De-orphaning.", *OldPin->GetName(), *GetFullName());
			}
...
void UNiagaraNode::AutowireNewNode(UEdGraphPin* FromPin)
{
	...
	if (FromPin != nullptr)
	{
		...
		{
			if (UEdGraphNode* FromPinNode = FromPin->GetOwningNodeUnchecked())
			{
				FromPinNode->NodeConnectionListChanged();
			}
		}
	}
}
...
void UNiagaraNode::MarkNodeRequiresSynchronization(FString Reason, bool bRaiseGraphNeedsRecompile)
{
	Modify();
	ChangeId = FGuid::NewGuid();
	//UE_LOGF(LogNiagaraEditor, Verbose, "Node %ls was marked requires synchronization.  Reason: %ls", *GetPathName(), *Reason);
