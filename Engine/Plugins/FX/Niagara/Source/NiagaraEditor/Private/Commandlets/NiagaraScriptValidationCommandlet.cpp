...
void UNiagaraScriptValidationCommandlet::ProcessNiagaraScripts()
{
	...
	for (const FAssetData& AssetIt : AssetList)
	{
		...
		if (PackageName != LastPackageName)
		{
			...
			{
				UE_LOGF(LogNiagaraScriptValidation, Warning, "Failed to load package %ls processing %ls", *PackageName, *SystemName);
		...
		{
			UE_LOGF(LogNiagaraScriptValidation, Warning, "Failed to load Niagara script %ls", *SystemName);
			continue;
		}
	...
	UE_LOGF(LogNiagaraScriptValidation, Log, "Took %5.3f seconds to process referenced Niagara scripts...", ProcessNiagaraSystemsTime);
}
