...
void UNiagaraDumpModuleInfoCommandlet::ProcessNiagaraParameterDefinitions()
{
	...
	for (const FAssetData& AssetIt : AssetList)
	{
		...
		if (PackageName != LastPackageName)
		{
			...
			{
				UE_LOGF(LogNiagaraDumpModuleInfoCommandlet, Warning, "Failed to load package %ls processing %ls", *PackageName, *SystemName);
		...
		{
			UE_LOGF(LogNiagaraDumpModuleInfoCommandlet, Warning, "Failed to load NiagaraParameterDefinitions %ls", *SystemName);
			continue;
		}
...
void UNiagaraDumpModuleInfoCommandlet::ProcessNiagaraScripts()
{
	...
	for (const FAssetData& AssetIt : AssetList)
	{
		...
		{
			UE_LOGF(LogNiagaraDumpModuleInfoCommandlet, Warning, "Failed to load Niagara script %ls", *SystemName);
			continue;
		}
	...
	UE_LOGF(LogNiagaraDumpModuleInfoCommandlet, Log, "Took %5.3f seconds to process referenced Niagara scripts...", ProcessNiagaraSystemsTime);
}
...
/*void UNiagaraDumpModuleInfoCommandlet::DumpByteCode(const UNiagaraScript* Script, const FString& FilePath)
{
	...
	if (Script)
	{
		...
		if (!FileArchive)
		{
			UE_LOGF(LogNiagaraDumpModuleInfoCommandlet, Warning, "Failed to create output stream %ls", *FullFilePath);
		}
