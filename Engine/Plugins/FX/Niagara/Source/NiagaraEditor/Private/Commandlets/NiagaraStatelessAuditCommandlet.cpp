...
namespace NiagaraStatelessAuditCommandletPrivate
{
	...
	UNiagaraSystem* LoadSystem(const FAssetData& AssetData)
	{
		...
		if (Package == nullptr)
		{
			UE_LOGF(LogNiagaraStatelessAuditCommandlet, Warning, "Failed to load package %ls processing %ls", *PackageName, *SystemName);
			return nullptr;
		}
		Package->FullyLoad();
...
int32 UNiagaraStatelessAuditCommandlet::Main(const FString& Params)
{
	...
	for (const FAssetData& AssetData : AssetsToProcess)
	{
		...
		//	UE_LOGF(LogNiagaraStatelessAuditCommandlet, Warning, "%ls = %ls = %d", *AssetData.PackageName.ToString(), *NumStatelessEmittersString, NumStatelessEmitters);
		//}
...
TUniquePtr<FArchive> UNiagaraStatelessAuditCommandlet::GetOutputFile(const TCHAR* Filename) const
{
	...
	{
		UE_LOGF(LogNiagaraStatelessAuditCommandlet, Warning, "Failed to create output stream %ls", *FullPath);
	}
