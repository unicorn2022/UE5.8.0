...
int32 UNiagaraTraversalCacheAuditCommandlet::Main(const FString& Params)
{
	...
	if (Switches.Contains(TEXT("pause")))
	{
		...
		UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Display, "Pausing for %i seconds.", WaitSeconds);
		...
		UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Display, "Pause finished.");
	}
...
void UNiagaraTraversalCacheAuditCommandlet::AuditScript(UNiagaraSystem& AuditSystem, FScriptData& AuditScriptData, TArray<FAuditResultScriptData>& OutScriptResults) const
{
	...
	for (FFunctionData& AuditFunctionData : AuditScriptData.FunctionDatas)
	{
		...
		if (TraversalCacheFunctionData == nullptr)
		{
			...
			UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Error, "System: %ls - Script %ls - Function %ls - Not found in traversal cache data.",
...
void UNiagaraTraversalCacheAuditCommandlet::LogSystemResult(const FAuditResultSystemData& SystemResult, int32 SystemResultIndex) const
{
	...
	{
		UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Error, "[%i] System: %ls Emitter: %ls",
		...
		for (const FAuditResultFunctionData& FunctionResult : ScriptResult.FunctionDatas)
		{
			...
			UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Error, "[%i] Script: %ls Function: %ls%ls Function Script: %ls Difference: %ls",
				SystemResultIndex + 1,
...
void UNiagaraTraversalCacheAuditCommandlet::AuditSystems(const TArray<FString>& SystemPaths, FAuditResults& Results) const
{
	...
	for (int32 BatchIndex = 0; BatchIndex < NumBatches && (MaxErrorCount == 0 || Results.ErrorCount < MaxErrorCount); BatchIndex++)
	{
		...
		for (int32 SystemPathIndex = SystemPathStartIndex; SystemPathIndex < SystemPathEndIndex; SystemPathIndex++)
		{
			...
			{
				UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Warning, "[%i / %i] Load failed - %ls", SystemPathIndex + 1, SystemPaths.Num(), *SystemData.SystemPath);
			}
			else
			{
				UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Display, "[%i / %i] Loaded - %ls", SystemPathIndex + 1, SystemPaths.Num(), *SystemData.SystemPath);
		...
		for (int32 BatchSystemIndex = 0; BatchSystemIndex < BatchSystems.Num(); BatchSystemIndex++)
		{
			...
			UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Display, "[%i / %i] Collecting Legacy Data - %ls", SystemPathIndex + 1, SystemPaths.Num(), *SystemPaths[SystemPathIndex]);
		...
		ParallelFor(BatchSystems.Num(), [&](int32 BatchSystemIndex)
		{
			...
			{
				UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Warning, "[%i / %i] Skipping Validation - %ls wasn't loaded", SystemPathIndex + 1, SystemPaths.Num(), *SystemPaths[SystemPathIndex]);
				return;
			}

			UE_LOGF(LogNiagaraTraversalCacheAuditCommandlet, Display, "[%i / %i] Validating - %ls", SystemPathIndex + 1, SystemPaths.Num(), *SystemPaths[SystemPathIndex]);
