...
void UNiagaraNodeStaticSwitch::UpdateFunctionAliasForContext(FNiagaraGraphFunctionAliasContext& InOutFunctionAliasContext) const
{
	if (IsSetByCompiler())
	{
		if (const FNiagaraVariable* Found = FNiagaraConstants::FindStaticSwitchConstant(SwitchTypeData.SwitchConstant))
		{
			InOutFunctionAliasContext.AddPendingStaticSwitchVariable(*Found, static_cast<uint8>(SwitchTypeData.SwitchType));
		}
	}
}
...
void UNiagaraNodeStaticSwitch::UpdateCompilerConstantValue(FTranslator* Translator)
{
	if (IsSetByPin() && Translator)
	{
		...
		if (SelectorPin)
		{
			...
			{
				UE_LOGF(LogNiagaraEditor, Log, "Static Switch Value: %d  Name: \"%ls\"", SwitchValue, *GetPathName());
			}
			return;
		}
	}
...
{	
	...
	for (int i = 0; i < MainOutputPins.Num(); i++)
	{
		...
		for (int32 j = 0; j < OptionValues.Num(); j++)
		{
			...
			{
				UE_LOGF(LogNiagaraEditor, Warning, "Invalid index on UNiagaraNodeStaticSwitch::ResolveNumerics %ls, OptionIdx: %d VarIdx: %d MaxIndex: %d", *GetPathName(), j, VarIdx, MainInputPins.Num())
			}
		}
...
void UNiagaraNodeStaticSwitch::PostLoad()
{
	...
	if (GetInputType().IsValid() && InputParameterName.IsValid())
	{
		if (UNiagaraGraph* NiagaraGraph = GetNiagaraGraph())
		{
			...
			{
				UE_LOGF(LogNiagaraEditor, Log, "Static switch constant \"%ls\" in \"%ls\" didn't have static switch meta-data conversion set properly. Fixing now.", *InputParameterName.ToString(), *GetPathName())
...
void UNiagaraNodeStaticSwitch::BuildParameterMapHistory(FNiagaraParameterMapHistoryBuilder& OutHistory, bool bRecursive /*= true*/, bool bFilterForCompilation /*= true*/) const
{
	...
	else
	{
		if (IsSetByPin())
		{
			...
			if (UNiagaraScript::LogCompileStaticVars != 0)
			{
				UE_LOGF(LogNiagaraEditor, Log, "Static Switch Value: %d  Name: \"%ls\"  SwitchInputParameterName:\"%ls\"", SelectorValue, *GetPathName(), *InputParameterName.ToString());
			}
		}
	}
