...
bool FNiagaraBakerRendererOutputSparseVolumeTexture::BeginBake(FNiagaraBakerFeedbackContext& FeedbackContext, UNiagaraBakerOutput* InBakerOutput)
{
	...
	{
		UE_LOGF(LogNiagaraBaker, Error, "Cannot initialize SVT for baking");
		return false;
	}
...
void FNiagaraBakerRendererOutputSparseVolumeTexture::BakeFrame(FNiagaraBakerFeedbackContext& FeedbackContext, UNiagaraBakerOutput* InBakerOutput, int FrameIndex, const FNiagaraBakerRenderer& BakerRenderer)
{
	...
	else if (DataInterface.Grid3DDataInterface != nullptr)
	{
		...
		{
			UE_LOGF(LogNiagaraBaker, Error, "Cannot bake SVTs from non RGBA Grid3D Collections");
		}		
		RT_Proxy_Grid3D = DataInterface.Grid3DProxy;	
	}
	else
	{
		UE_LOGF(LogNiagaraBaker, Error, "Cannot bake from data interface");
	}
		...
		(
			...
			RT_TextureData = &TextureData, RT_VolumeResolution = &VolumeResolution, RT_VolumeFormat = &VolumeFormat](FRHICommandListImmediate& RHICmdList)
			{
				...
				if (RT_Proxy_RenderTargetVolume)
				{
					...
					{
						UE_LOGF(LogNiagaraBaker, Error, "No valid volume RT DI to do readback from");
						return;
					}
				}
				else if (RT_Proxy_Grid3D)
				{
					if (const FGrid3DCollectionRWInstanceData_RenderThread* InstanceData_RT = RT_Proxy_Grid3D->SystemInstancesToProxyData_RT.Find(RT_InstanceID))
					{
						...
						{
							UE_LOGF(LogNiagaraBaker, Error, "No valid grid DI to do readback from");
							return;
						}
					}
					else
	...
	if (TextureData.Num() > 0)
	{
		...
		if (!Success)
		{
			UE_LOGF(LogNiagaraBaker, Error, "Cannot create SVT for data interface");
		...
		{
			UE_LOGF(LogNiagaraBaker, Error, "Cannot append frame to SVT");
		}
#endif			
	}
...
void FNiagaraBakerRendererOutputSparseVolumeTexture::EndBake(FNiagaraBakerFeedbackContext& FeedbackContext, UNiagaraBakerOutput* InBakerOutput)
{
	...
	{
		UE_LOGF(LogNiagaraBaker, Error, "Cannot EndInitialize on creating SVT");
	}

	SVTAsset->ConditionalPostLoad();
	...
	{		
		...
		{
			UE_LOGF(LogNiagaraBaker, Error, "Cannot initialize looped SVT for baking");
			return;
		}
		...
		if (TotalNumFrames <= 0)
		{
			UE_LOGF(LogNiagaraBaker, Error, "SVT sequence to loop must have > 0 frames");
			return;
		}

		if (StartFrame < 0)
		{
			UE_LOGF(LogNiagaraBaker, Error, "Start frame must be greater than or equal to 0");
			return;
		}

		if (BlendFrames <= 0)
		{
			UE_LOGF(LogNiagaraBaker, Error, "Blend amount must be greater than 0");
			return;
		}

		if (LoopedFrames <= 0)
		{
			UE_LOGF(LogNiagaraBaker, Error, "Cannot loop SVT due to insufficient frames.  Please reduce blend amount and start time or bake more frames.");
			return;
		}
		...
		for (int i = 0; i < LoopedFrames; ++i)
		{
			...
			{
				UE_LOGF(LogNiagaraBaker, Error, "Invalid frame from baked SVT for looping");
				return;
			}
			...
			ENQUEUE_RENDER_COMMAND(NDIRenderTargetVolumeUpdate)
				(
					...
					RT_SVTRenderResources_B = SparseVolumeTextureFrameB->GetTextureRenderResources()](FRHICommandListImmediate& RHICmdList)
					{
						...
						{
							UE_LOGF(LogNiagaraBaker, Error, "Null svt resource");
							return;
						}
						...
						{
							UE_LOGF(LogNiagaraBaker, Error, "Null svt texture");
							return;
						}
						...
						{
							UE_LOGF(LogNiagaraBaker, Error, "Readback failed and returned null locked data");
							return;
						}

						if (RowPitchInPixels == 0 || BufferHeight == 0)
						{
							UE_LOGF(LogNiagaraBaker, Error, "Readback failed and returned data with zero pitch/buffer height");
							return;
						}
						...
						{
							UE_LOGF(LogNiagaraBaker, Error, "Output looped texture data has no elements");
							return;
						}
			...
			if (TextureData.Num() == 0)
			{
				UE_LOGF(LogNiagaraBaker, Error, "Readback failed when trying to add looped render target to SVT");
				return;
			}
			...
			if (!Success)
			{
				UE_LOGF(LogNiagaraBaker, Error, "Cannot create looped SVT output");
				return;
			}
			...
			{
				UE_LOGF(LogNiagaraBaker, Error, "Cannot append frame to looped SVT");
				return;
			}
#endif
		}		
		...
		{
			UE_LOGF(LogNiagaraBaker, Error, "Cannot EndInitialize on creating looped SVT");
		}

		LoopedSVTAsset->ConditionalPostLoad();
	}
}
