...
const UNiagaraClipboardRenderer* UNiagaraClipboardRenderer::CreateRenderer(UObject* InOuter, UNiagaraRendererProperties* Renderer, TOptional<FNiagaraStackNoteData> StackNoteData, TOptional<FText> AlternateDisplayName)
{
	...
	if (StackNoteData.IsSet() && StackNoteData.GetValue().IsValid())
	{
		NewRenderer->StackNoteData = StackNoteData;
	}
	if (AlternateDisplayName.IsSet() && !AlternateDisplayName.GetValue().IsEmpty())
	{
		NewRenderer->AlternateDisplayName = AlternateDisplayName;
	}
...
{
	FPlatformApplicationMisc::ClipboardCopy(*ClipboardContentToString(ClipboardContent));
}
...
const UNiagaraClipboardContent* FNiagaraClipboard::GetClipboardContent() const
{
	...
	return ClipboardContentFromString(ClipboardText);
}

FString FNiagaraClipboard::ClipboardContentToString(UNiagaraClipboardContent* ClipboardContent)
{
...
UNiagaraClipboardContent* FNiagaraClipboard::ClipboardContentFromString(const FString& ClipboardText)
{
	...
	if (ClipboardText.IsEmpty() == false)
	{
		...
		// a valid portable value so we construct an empty clipboard object and set the portable data.
		UNiagaraClipboardContent* ClipboardContent = UNiagaraClipboardContent::Create();

		// Check to see if the clipboard string can represent an external object reference and if so, check to see if we have a registered
		// handler.
		FSoftObjectPath ClipboardObjectPath = FSoftObjectPath(ClipboardText);
		UObject* ClipboardObject = ClipboardObjectPath.ResolveObject();
		if (ClipboardObject != nullptr)
		{
			TSharedPtr<const INiagaraEditorExternalObjectUtilities, ESPMode::ThreadSafe> ExternalObjectUtilities = FNiagaraEditorModule::Get().GetExternalObjectUtilities(*ClipboardObject->GetClass());
			FNiagaraClipboardPortableValue ObjectValue;
			if (ExternalObjectUtilities.IsValid() &&
				ExternalObjectUtilities->SupportsClipboardPortableValues() &&
				ExternalObjectUtilities->TryUpdateClipboardPortableValueFromObject(*ClipboardObject, ObjectValue))
			{
				ClipboardContent->PortableValues.Add(ObjectValue);
			}
		}

		// If it was not handled by an external object handler, just put the raw clipboard text in the portable values.
		if (ClipboardContent->PortableValues.Num() == 0)
		{
