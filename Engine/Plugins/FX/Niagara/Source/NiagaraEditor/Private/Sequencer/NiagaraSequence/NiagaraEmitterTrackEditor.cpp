...
private:
	...
	FReply OnToggleIsolateButtonClicked()
	{
		if (EmitterTrack.IsValid())
		{
			TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel = EmitterTrack->GetEmitterHandleViewModel();
			if (EmitterHandleViewModel.IsValid())
			{
				...
				{
					EmitterTrack->GetSystemViewModel().ToggleIsolateEmitterAndSelectedEmitters(EmitterHandleViewModel->GetId());
				}
			}
		}
...
TSharedPtr<SWidget> FNiagaraEmitterTrackEditor::BuildOutlinerColumnWidget(const FBuildColumnWidgetParams& Params, const FName& ColumnName)
{
	...
	{
		UMovieSceneNiagaraEmitterTrack* Track = nullptr;
		if (TViewModelPtr<ITrackExtension> TrackModel = Params.ViewModel->FindAncestorOfType<ITrackExtension>(true))
		{
			Track = CastChecked<UMovieSceneNiagaraEmitterTrack>(TrackModel->GetTrack());
		}

		return Track ? SNew(SEmitterTrackWidget, *Track) : SNullWidget::NullWidget;;
	}
