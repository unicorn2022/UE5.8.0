...
void UNiagaraNodeAssignment::PostLoad()
{
	...
	if (AssignmentTarget_DEPRECATED.IsValid() && AssignmentTargets.Num() == 0)
	{
		...
		UE_LOGF(LogNiagaraEditor, Log, "Found old Assignment Node, converting variable \"%ls\" in \"%ls\"", *AssignmentTarget_DEPRECATED.GetName().ToString(), *GetFullName());
		...
		if (Pin != nullptr && Pin->LinkedTo.Num() == 1)
		{
				...
				for (UEdGraphPin* InputPin : InputPins)
				{
					...
					if (Var.GetName() == RemapVar.GetName())
					{
						if (NewVar != Var)
						{
							...
							UE_LOGF(LogNiagaraEditor, Log, "Converted Set pin variable \"%ls\" to \"%ls\" in \"%ls\"", *Var.GetName().ToString(), *NewVar.GetName().ToString(), *GetFullName());
			...
			else if (Pin->LinkedTo[0]->GetOwningNode())
			{
				...
				UE_LOGF(LogNiagaraEditor, Log, "Found node \"%ls\" attached to assignment \"%ls\" variable %ls", *Pin->LinkedTo[0]->GetOwningNode()->GetFullName(), *GetFullName(), *NewVar.GetName().ToString());
			}
		}
		...
		{
			UE_LOGF(LogNiagaraEditor, Log, "Found old Assignment Node, nothing was attached???? variable \"%ls\" in \"%ls\"", *AssignmentTarget_DEPRECATED.GetName().ToString(), *GetFullName());
		}
	}
	else
	{
