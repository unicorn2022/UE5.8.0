...
public:
	static NIAGARAEDITOR_API const UNiagaraClipboardRenderer* CreateRenderer(UObject* InOuter, UNiagaraRendererProperties* Renderer, TOptional<FNiagaraStackNoteData> StackNoteData = TOptional<FNiagaraStackNoteData>(), TOptional<FText> AlternateDisplayName = TOptional<FText>());

	UPROPERTY()
	TObjectPtr<UNiagaraRendererProperties> RendererProperties;

	UPROPERTY()
	TOptional<FNiagaraStackNoteData> StackNoteData;
...
struct FNiagaraClipboardPortableValue
{
	...
	static FNiagaraClipboardPortableValue CreateFromStructValue(const StructType& InStructValue)
	{
		return CreateFromStructValue(*StructType::StaticStruct(), (uint8*)&InStructValue);
	}
	...
	template<typename StructType>
	bool TryUpdateStructValue(StructType& OutStructValue) const 
	{
		return TryUpdateStructValue(*StructType::StaticStruct(), (uint8*)&OutStructValue);
	}
...
public:
	...
	NIAGARAEDITOR_API static FString ClipboardContentToString(UNiagaraClipboardContent* ClipboardContent);

	NIAGARAEDITOR_API static UNiagaraClipboardContent* ClipboardContentFromString(const FString& ClipboardText);
};
