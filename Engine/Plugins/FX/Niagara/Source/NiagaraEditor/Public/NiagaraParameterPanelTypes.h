...
{
	FNiagaraParameterReferencePath() = default;
	FNiagaraParameterReferencePath(const UObject* InSourceObject, FName InModuleName)
	...
	{
		return Other.SourceObject == SourceObject && Other.ModuleName == ModuleName;
	};
};
...
public:
	...
	{
		if ( ensure(OnRequestRenameDelegate.IsBound()) )
		{
			OnRequestRenameDelegate.Execute();
		}
	}

	void RequestRenameNamespaceModifier() const
	{
		if ( ensure(OnRequestRenameNamespaceModifierDelegate.IsBound()) )
		{
			OnRequestRenameNamespaceModifierDelegate.Execute();
		}
	}
