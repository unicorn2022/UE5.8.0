...
struct FNiagaraStackFunctionInputActions
{
	FText			Label;
	FText			ToolTip;
...
public:
	...
	enum class EValueMode
	{
		...
		Linked,
		/** The value is linked to a shared parameter defined in the top level script. */
		LinkedShared,
...
public:
	...
	/** Gets the function call node which owns this input, or nullptr if the node has been garbage collected. */
	NIAGARAEDITOR_API UNiagaraNodeFunctionCall* GetInputFunctionCallNodePtr() const;
	...
	/** Get a list of custom actions that can be executed on the input from the right click context menu */
	NIAGARAEDITOR_API TArray<FNiagaraStackFunctionInputActions> GetCustomActions();
	...
	/** Sets this input's data interface value via a caller-supplied function. The function receives
	 *  the placeholder DI; the graph-side override is synced via the manager's OnChanged handler. */
	NIAGARAEDITOR_API void SetDataInterfaceValueExternal(TSubclassOf<UNiagaraDataInterface> InDataInterfaceType, TFunction<void(UNiagaraDataInterface*)> ExternalSetFunction);
	...
	/** Gets the variable that serves as a visible condition for this input. */
	NIAGARAEDITOR_API TOptional<FNiagaraVariable> GetVisibleConditionVariable() const;
	...
	/** Gets available target script usages which can be used to create shared linked parameters. */
	NIAGARAEDITOR_API void GetAvailableScriptUsagesForSharedParameters(TArray<ENiagaraScriptUsage>& OutUsages) const;

	/** Gets a stack function input to display and edit a linked value, when this input is linked to a shared linked parameter, otherwise returns null */
	NIAGARAEDITOR_API UNiagaraStackFunctionInput* GetSharedLinkedValueInput();

	/** Gets whether or not this input was created by another input to display and edit a shared linked value. */
	bool GetIsSharedLinkedValueInput() const { return bIsSharedLinkedValueInput; }

	/** Moves the current value of this input, to a shared parameter and then changes the value of this input to link to that new parameter.  If a target usage is provided
		the shared input parameter will be added to that script, otherwise the shared parameter will be added to a transient parameter in the current script. */
	NIAGARAEDITOR_API void MoveValueToLinkedSharedParameter(TOptional<ENiagaraScriptUsage> LinkedSharedParameterTargetUsage);

	/** Breaks the link to a shared parameter and copies the value of the shared parameter this input. */
	NIAGARAEDITOR_API void UnlinkFromSharedParameter();
...
protected:
	...
	NIAGARAEDITOR_API TOptional<float> GetMaterialDynamicParamDefaultValue() const;
	NIAGARAEDITOR_API FStackIssueFixDelegate GetUpgradeDynamicInputVersionFix();

	/** Sets whether or not this input was created by another input to display and edit a shared linked value. */
	void SetIsSharedLinkedValueInput(bool bInIsSharedLinkedValueInput) { bIsSharedLinkedValueInput = bInIsSharedLinkedValueInput; }
...
private:
	struct FInputValues
	{
		...
		EValueMode Mode = EValueMode::None;
		TSharedPtr<FStructOnScope> LocalStruct;
		FNiagaraVariableBase LinkedParameter;
		TWeakObjectPtr<UNiagaraNodeAssignment> SharedSourceAssignmentNode;
...
private:
	...
	/** A stack function input to display and edit a linked value, when this input is linked to a shared linked parameter. */
	UPROPERTY()
	TObjectPtr<UNiagaraStackFunctionInput> SharedLinkedInput;
	/** Whether or not this input was created by another input to display and edit a shared linked value. */
	bool bIsSharedLinkedValueInput = false;
