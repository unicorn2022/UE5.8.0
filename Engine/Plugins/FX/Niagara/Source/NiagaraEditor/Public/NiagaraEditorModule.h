...
class INiagaraEditorExternalObjectUtilities;
...
public:
	...
	/** Registers niagara editor external object utilities for a specific class type, optionally allowing handling subtypes. */
	void RegisterExternalObjectUtilities(const UClass& InClass, bool bHandleSubclasses, TSharedRef<INiagaraEditorExternalObjectUtilities, ESPMode::ThreadSafe> ExternalObjectUtilities);
	...
	TSharedPtr<INiagaraEditorTypeUtilities, ESPMode::ThreadSafe> NIAGARAEDITOR_API GetTypeUtilities(const FNiagaraTypeDefinition& Type) const;

	/** Gets Niagara editor property utilities for a specific struct if there are any registered. */
	TSharedPtr<INiagaraEditorPropertyUtilities, ESPMode::ThreadSafe> NIAGARAEDITOR_API GetPropertyUtilities(const UScriptStruct& Struct) const;

	/** Gets Niagara editor external object utilities for a specific class if there are any registered. */
	TSharedPtr<const INiagaraEditorExternalObjectUtilities, ESPMode::ThreadSafe> NIAGARAEDITOR_API GetExternalObjectUtilities(const UClass& Class) const;
...
private:
	...
	{
	public:
		virtual ~FDeferredDestructionContainerBase() = default;
	};
	...
	void UnregisterDynamicCustomizations();
	
	void RegisterMenuExtensions();
	
	void RegisterMenuExtension_MigrateToUserAssetTags();

	void ConditionalRegisterDynamicStruct(const UScriptStruct* Struct);
...
private:
	...
	mutable UE::FMutex EditorUtilitiesGuard;
	...
	struct FRegisteredExternalObjectUtilities
	{
		const UClass* Class = nullptr;
		bool bHandleSubclasses = false;
		TSharedPtr<const INiagaraEditorExternalObjectUtilities, ESPMode::ThreadSafe> ExternalObjectUtilities;
	};

	TArray<FRegisteredExternalObjectUtilities> RegisteredExternalObjectUtilities;
