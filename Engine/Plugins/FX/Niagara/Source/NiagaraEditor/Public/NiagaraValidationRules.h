...
namespace NiagaraValidation
{
	...
	NIAGARAEDITOR_API TSharedPtr<FNiagaraEmitterHandleViewModel> GetEmitterViewModel(const FNiagaraValidationContext& Context, const UNiagaraEmitter* NiagaraEmitter);
...
/** Checks that all emitters with a parent emitter are using the latest version of the parent. */
UCLASS(Category = "Validation", DisplayName = "Require Latest Parent Emitter Version")
class UNiagaraValidationRule_RequireLatestParentEmitterVersion : public UNiagaraValidationRule
{
	GENERATED_BODY()
...
/** Checks that all emitters with a parent emitter are using one of a set of allowed parent emitters. */
UCLASS(Category = "Validation", DisplayName = "Require Parent Emitter(s)")
class UNiagaraValidationRule_RequireParentEmitter : public UNiagaraValidationRule
{
	GENERATED_BODY()

public:

	/** The collection of allowed parent emitters */
	UPROPERTY(EditAnywhere, Category = Validation)
	TArray<TSoftObjectPtr<UNiagaraEmitter>> AllowedParentEmitters;

	/** True if emitters must have a parent emitter, false to allow no parent emitter */
	UPROPERTY(EditAnywhere, Category = Validation)
	bool bMustHaveParentEmitter = true;
