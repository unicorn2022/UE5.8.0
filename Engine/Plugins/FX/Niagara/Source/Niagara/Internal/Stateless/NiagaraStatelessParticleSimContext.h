...
public:
	...
	const FVector3f GetLocalToWorldScale() const;
...
public:
	...
	static FVector2f	SafeNormalize(const FVector2f& v, const FVector2f& Fallback) { const float l2 = v.SquaredLength(); return l2 < UE_KINDA_SMALL_NUMBER ? Fallback : v * (1.0f / FMath::Sqrt(l2)); }
	static FVector3f	SafeNormalize(const FVector3f& v, const FVector3f& Fallback) { const float l2 = v.SquaredLength(); return l2 < UE_KINDA_SMALL_NUMBER ? Fallback : v * (1.0f / FMath::Sqrt(l2)); }
	static FVector2f	SafeNormalize(const FVector2f& v) { return SafeNormalize(v, FVector2f(1.0f, 0.0f)); }
	static FVector3f	SafeNormalize(const FVector3f& v) { return SafeNormalize(v, FVector3f(1.0f, 0.0f, 0.0f)); }

	static float		SafeRcpSqrt(const float v) { return v > UE_SMALL_NUMBER ? FMath::InvSqrt(v) : 0.0f; }
	static FVector3f	SafeRcpSqrt(const FVector3f& v) { return FVector3f(SafeRcpSqrt(v.X), SafeRcpSqrt(v.Y), SafeRcpSqrt(v.Z)); }
	...
	template<typename T>
	const T* ReadSharedBuiltData() const
	{
		const FName DataName = T::GetName();
		for (const TPair<FName, uint32>& Pair : SharedBuiltDataOffsets)
		{
			if (Pair.Key == DataName)
			{
				return reinterpret_cast<const T*>(SharedBuiltData.GetData() + Pair.Value);
			}
		}
		return nullptr;
	}
...
private:
	...
	TConstArrayView<uint8>					SharedBuiltData;
	TConstArrayView<TPair<FName, uint32>>	SharedBuiltDataOffsets;
