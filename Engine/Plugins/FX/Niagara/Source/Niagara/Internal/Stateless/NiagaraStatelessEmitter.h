...
*/
UCLASS(MinimalAPI, EditInlineNew, meta = (ShowTopLevelCategories))
...
#if WITH_EDITOR
	// Callback is owner name, variable, read, write
	NIAGARA_API void GatherSimulationAttributeReferences(const FNiagaraStatelessSpawnInfo& SpawnInfo, TFunction<void(FName, const FNiagaraVariableBase&, bool, bool)> ReferenceFound) const;
	NIAGARA_API void GatherSimulationAttributeReferences(const UNiagaraStatelessModule* Module, TFunction<void(FName, const FNiagaraVariableBase&, bool, bool)> ReferenceFound) const;
	NIAGARA_API void GatherSimulationAttributeReferences(TFunction<void(FName, const FNiagaraVariableBase&, bool, bool)> ReferenceFound) const;
...
protected:
	...
	*/
	UPROPERTY(EditAnywhere, Category = "Emitter", AdvancedDisplay)
	...
	You should not need to modify this with the exception of debugging / testing and as it will impact cooked performance and memory
	*/
	UPROPERTY(EditAnywhere, Category = "Emitter", AdvancedDisplay, meta = (EditCondition = "!bForceOutputAllAttributes"))
	uint32 bForceOutputUniqueID : 1 = false;
#endif

	UPROPERTY(EditAnywhere, Category = "Emitter", AdvancedDisplay, meta = (Bitmask, BitMaskEnum = "/Script/Niagara.ENiagaraStatelessFeatureMask"))
