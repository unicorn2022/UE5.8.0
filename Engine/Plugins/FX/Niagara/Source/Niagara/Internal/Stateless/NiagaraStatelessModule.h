...
public:
	...
	// The feature mask is used to define when they can be used
	// For example if the module does not support GPU make sure to remove the GPU feature mask
	// Module & template authors should be careful to not get into a situation where the feature mask is none
	virtual ENiagaraStatelessFeatureMask GetFeatureMask() const { return ENiagaraStatelessFeatureMask::All; }

	// Called during the data build process on each module sequentially
	// Modules add build data that contain all the information needed to setup shader parameters for simulation
	virtual void BuildEmitterData(const FNiagaraStatelessEmitterDataBuildContext& BuildContext) const {}

	// Called during the data build process to create the shader parameter strucutre
	// Modules pack structure or loose parameters that will be used to simulate
	virtual void BuildShaderParameters(FNiagaraStatelessShaderParametersBuilder& ShaderParametersBuilder) const {}

	// Called at runtime to set the shader parameter structures
	// Modules should keep this simple and used the build process to optimize data as much as possible
