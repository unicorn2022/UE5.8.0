...
void FNiagaraGpuComputeDispatch::ProcessPendingTicksFlush(FRHICommandListImmediate& RHICmdList, bool bForceFlush)
{
	...
	switch (FNiagaraGpuComputeDispatchLocal::GTickFlushMode)
	{
		...
		case 0:
		{
			//UE_LOGF(LogNiagara, Log, "FNiagaraGpuComputeDispatch: Queued ticks (%d) are building up, this may cause a stall when released.", Ticks_RT.Num());
			break;
		}
		...
		case 1:
		{
			//UE_LOGF(LogNiagara, Log, "FNiagaraGpuComputeDispatch: Queued ticks are being Processed due to not rendering.  This may result in undesirable simulation artifacts.");

			// Early out if we have no pending ticks to process
			if (MaxPendingTicks == 0)
			{
				GpuReadbackManagerPtr->Tick(RHICmdList);
				return;
			}
		...
		case 2:
		{
			//UE_LOGF(LogNiagara, Log, "FNiagaraGpuComputeDispatch: Queued ticks are being Destroyed due to not rendering.  This may result in undesirable simulation artifacts.");
...
void FNiagaraGpuComputeDispatch::DumpDebugFrame()
{
	...
	// Dump Frame
	UE_LOGF(LogNiagara, Warning, "====== BatcherFrame(%d)", GFrameNumberRenderThread);
	...
	for (int iTickStage = 0; iTickStage < ENiagaraGpuComputeTickStage::Max; ++iTickStage)
	{
		...
		UE_LOGF(LogNiagara, Warning, "==== TickStage(%d) TotalGroups(%d)", iTickStage, DispatchList.DispatchGroups.Num());
		...
		for ( int iDispatchGroup=0; iDispatchGroup < DispatchList.DispatchGroups.Num(); ++iDispatchGroup )
		{
			...
			{
				UE_LOGF(LogNiagara, Warning, "====== TicksWithPerInstanceData(%d)", DispatchGroup.TicksWithPerInstanceData.Num());
				for (FNiagaraGPUSystemTick* Tick : DispatchGroup.TicksWithPerInstanceData)
				{
					for (const auto& Pair : Tick->DIInstanceData->InterfaceProxiesToOffsets)
					{
						...
						UE_LOGF(LogNiagara, Warning, "Proxy(%ls)", *Proxy->GetSourceDIName().ToString());
					}
				}
			}

			UE_LOGF(LogNiagara, Warning, "====== DispatchGroup(%d)", iDispatchGroup);
			for ( const FNiagaraGpuDispatchInstance& DispatchInstance : DispatchGroup.DispatchInstances )
			{
				...
				Builder.Appendf(TEXT("Iteration(%d | %s) "), SimStageData.IterationIndex, SimStageData.AlternateIterationSource ? *SimStageData.AlternateIterationSource->GetSourceDIName().ToString() : TEXT("Particles"));
				...
				UE_LOGF(LogNiagara, Warning, "%ls", Builder.ToString());
			}

			if (DispatchGroup.FreeIDUpdates.Num() > 0)
			{
				UE_LOGF(LogNiagara, Warning, "====== FreeIDUpdates");
				for (const FNiagaraGpuFreeIDUpdate& FreeIDUpdate : DispatchGroup.FreeIDUpdates)
				{
					UE_LOGF(LogNiagara, Warning, "ComputeContext(%p) Emitter(%ls)", FreeIDUpdate.ComputeContext, FreeIDUpdate.ComputeContext->GetDebugSimName());
				}
			}
		}
		if (DispatchList.CountsToRelease.Num() > 0)
		{
			UE_LOGF(LogNiagara, Warning, "====== CountsToRelease");
			...
			for ( int32 i=0; i < DispatchList.CountsToRelease.Num(); ++i )
			{
				...
				if ( bLast )
				{
					UE_LOGF(LogNiagara, Warning, "%ls", StringBuilder.ToString());
					StringBuilder.Reset();
				}
			}
		}
...
void FNiagaraGpuComputeDispatch::UpdateInstanceCountManager(FRHICommandListImmediate& RHICmdList)
{
	...
	// Consume any pending readbacks that are ready
	{
		...
		if ( const uint32* Counts = GPUInstanceCounterManager.GetGPUReadback() )
		{
			...
			{
				UE_LOGF(LogNiagara, Warning, "====== BatcherFrame(%d) Readback Complete", GFrameNumberRenderThread);
			}
			...
			for (int iTickStage=0; iTickStage < ENiagaraGpuComputeTickStage::Max; ++iTickStage)
			{
				for (FNiagaraSystemGpuComputeProxy* ComputeProxy : ProxiesPerStage[iTickStage])
				{
					for ( FNiagaraComputeExecutionContext* ComputeContext : ComputeProxy->ComputeContexts )
					{
						...
						const uint32 CurrentInstanceCount = ComputeContext->GetCurrentNumInstances();
						if ( DeadInstanceCount <= CurrentInstanceCount)
						{
							ComputeContext->SetCurrentNumInstances(CurrentInstanceCount - DeadInstanceCount);
						}
						if (FNiagaraGpuComputeDispatchLocal::GDebugLogging)
						{
							UE_LOGF(LogNiagara, Warning, "ComputeContext(%p) Emitter(%ls) DeadInstances(%d) CountReleased(0x%08x)", ComputeContext, ComputeContext->GetDebugSimName(), DeadInstanceCount, ComputeContext->EmitterInstanceReadback.GPUCountOffset);
						}
...
void FNiagaraGpuComputeDispatch::PrepareTicksForProxy(FRHICommandListImmediate& RHICmdList, FNiagaraSystemGpuComputeProxy* ComputeProxy, FNiagaraGpuDispatchList& GpuDispatchList)
{
	...
	for ( int32 iTickToProcess=0; iTickToProcess < NumTicksToProcess; ++iTickToProcess )
	{
		...
		for (FNiagaraComputeInstanceData& InstanceData : Tick.GetInstances())
		{
			...
			if (InstanceData.bResetData)
			{
				ComputeContext->SetCurrentNumInstances(0);
			...
			const uint32 PrevNumInstances = ComputeContext->GetCurrentNumInstances();

			uint32 NewInstanceCount = PrevNumInstances;
			NewInstanceCount = NewInstanceCount + InstanceData.SpawnInfo.SpawnRateInstances + InstanceData.SpawnInfo.EventSpawnTotal;
			NewInstanceCount = FMath::Min(NewInstanceCount, MaxBufferInstances);

			// Calculate new maximum count
			ComputeContext->CurrentMaxInstances_RT = FMath::Max(ComputeContext->CurrentMaxInstances_RT, NewInstanceCount);

			if (!GNiagaraBatcherFreeBufferEarly || (NewInstanceCount > 0))
			{
				ComputeContext->CurrentMaxAllocateInstances_RT = FMath::Max3(ComputeContext->CurrentMaxAllocateInstances_RT, NewInstanceCount, InstanceData.SpawnInfo.MaxParticleCount);
			}
			else
			{
				ComputeContext->CurrentMaxAllocateInstances_RT = FMath::Max(ComputeContext->CurrentMaxAllocateInstances_RT, NewInstanceCount);
			}

			ComputeContext->SetCurrentNumInstances(NewInstanceCount);
			...
			for (const FNiagaraComputeInstanceData::FPerStageInfo& PerStageInfo : InstanceData.PerStageInfo)
			{
				...
				for ( int32 IterationIndex=0; IterationIndex < NumIterations; ++IterationIndex )
				{
					...
					if (!SimStageData.StageMetaData->bWritesParticles)
					{
						...
						SimStageData.SourceNumInstances = NewInstanceCount;
						SimStageData.Destination = nullptr;
						SimStageData.DestinationCountOffset = ComputeContext->CountOffset_RT;
						SimStageData.DestinationNumInstances = NewInstanceCount;
					}
					...
					else
					{
						...
						SimStageData.SourceNumInstances = SimStageIndex == 0 && IterationIndex == 0 ? PrevNumInstances : NewInstanceCount;
	...
	for (FNiagaraComputeExecutionContext* ComputeContext : ComputeProxy->ComputeContexts)
	{
		...
		if ((ComputeProxy->GetComputeTickStage() == ENiagaraGpuComputeTickStage::PreInitViews) || (ComputeProxy->GetComputeTickStage() == ENiagaraGpuComputeTickStage::PostInitViews))
		{
			...
			FinalBuffer->SetNumInstances(ComputeContext->GetCurrentNumInstances());
...
void FNiagaraGpuComputeDispatch::ExecuteTicks(FRDGBuilder& GraphBuilder, TConstStridedView<FSceneView> Views, ENiagaraGpuComputeTickStage::Type TickStage)
{
	...
	SubstratePublicGlobalUniformParams = UE::FXRenderingUtils::GetSubstratePublicGlobalUniformBuffer(GraphBuilder, SimulationSceneViews);
	
	FNiagaraGpuComputeDataManagerTickContext Context(GraphBuilder);
	for (auto& DataManager : GpuDataManagers)
	{
		if (DataManager.Value)
		{
			DataManager.Value->PreTick(Context, TickStage);
		}
	}
...
void FNiagaraGpuComputeDispatch::DispatchStage(FRDGBuilder& GraphBuilder, const FNiagaraGPUSystemTick& Tick, const FNiagaraComputeInstanceData& InstanceData, const FNiagaraSimStageData& SimStageData)
{
	...
	switch (SimStageData.StageMetaData->IterationSourceType)
	{
		...
		case ENiagaraIterationSource::DirectSet:
		{
			...
			switch (DispatchType)
			{
				...
				case ENiagaraGpuDispatchType::ThreeD:
					...
					break;
				default:
					UE_LOGF(LogNiagara, Fatal, "FNiagaraGpuComputeDispatch: Unknown DispatchType(%d)", int(DispatchType));
					break;
			}
		...
		default:
			UE_LOGF(LogNiagara, Fatal, "FNiagaraGpuComputeDispatch: Unknown IterationSouce(%d)", int(SimStageData.StageMetaData->IterationSourceType));
			return;
	}
	...
	if (SimStageData.StageMetaData->bGpuIndirectDispatch)
	{
		...
		GraphBuilder.AddPass(
			...
			[this, ShaderParametersMetadata, DispatchParameters, ComputeShader, TickPtr=&Tick, InstanceDataPtr=&InstanceData, SimStageDataPtr=&SimStageData](FRHICommandList& RHICmdList)
			{
				...
				FComputePipelineStateInitializer Initializer(RHIComputeShader, ComputeShader.GetSpecializationConstants());
				SetComputePipelineState(RHICmdList, Initializer);
	...
	else
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "FNiagaraGpuComputeDispatch: Invalid ThreadGroupdCount(%d, %d, %d) for ElementCount(%d, %d, %d) Stage (%ls)",
...
void FNiagaraGpuComputeDispatch::PreInitViews(FRDGBuilder& GraphBuilder, bool bAllowGPUParticleUpdate, const TArrayView<const FSceneViewFamily*>& ViewFamilies, const FSceneViewFamily* CurrentFamily)
{
	...
	GpuReadbackManagerPtr->Tick(GraphBuilder.RHICmdList);
#if NIAGARA_COMPUTEDEBUG_ENABLED
	if ( FNiagaraGpuComputeDebug* GpuComputeDebug = GpuComputeDebugPtr.Get() )
	{
	...
	if (FNiagaraGpuComputeDispatchLocal::GDumpSortedSimulations > 0)
	{
		...
		UE_LOGF(LogNiagara, Log, "Niagara SimulationsToSort:");
		for (const FNiagaraGPUSortInfo& SortInfo : SimulationsToSort)
		{
			...
			UE_LOGF(LogNiagara, Log, " - Owner(%ls) ParticleCount(%d) %ls", *SortInfo.OwnerName.ToString(), SortInfo.ParticleCount, EnabledFeatures.ToString());
		}
	}
#endif
	...
	if (FNiagaraUtilities::AllowGPUParticles(GetShaderPlatform()))
	{
		if ( bAllowGPUParticleUpdate )
		{
			...
			for (auto& DataManager : GpuDataManagers)
			{
				if (DataManager.Value)
				{
					DataManager.Value->BeginFrame(Context);
				}
			}
...
void FNiagaraGpuComputeDispatch::PostRenderOpaque(FRDGBuilder& GraphBuilder, TConstStridedView<FSceneView> Views, FSceneUniformBuffer &SceneUniformBuffer, bool bAllowGPUParticleUpdate)
{
	...
	GraphBuilder.AddPostExecuteCallback(
		[this, &RHICmdList=GraphBuilder.RHICmdList, &GraphBuilder, bAllowGPUParticleUpdate]
		{
		#if WITH_NIAGARA_GPU_PROFILER
			...
			for (auto& DataManager : GpuDataManagers)
			{
				if (DataManager.Value)
				{
					DataManager.Value->EndFrame(Context);
				}
			}
		}
	);
...
void FNiagaraGpuComputeDispatch::ProcessDebugReadbacks(FRHICommandList& RHICmdList, bool bWaitCompletion)
{
	...
	for (const FDebugReadbackInfo& DebugReadback : GpuDebugReadbackInfos)
	{
		...
		GpuReadbackManagerPtr->EnqueueReadbacks(
			...
			{
				checkf(4 + (CountOffset * 4) <= BufferData[CountBufferIndex].Value, TEXT("CountOffset %d is out of bounds (%d)"), CountOffset, BufferData[CountBufferIndex].Value);
...
void FNiagaraGpuComputeDispatch::GenerateSortKeys(FRHICommandListImmediate& RHICmdList, int32 BatchId, int32 NumElementsInBatch, EGPUSortFlags Flags, FRHIUnorderedAccessView* KeysUAV, FRHIUnorderedAccessView* ValuesUAV)
{
	...
	for (const FNiagaraGPUSortInfo& SortInfo : SimulationsToSort)
	{
		if (SortInfo.AllocationInfo.SortBatchId == BatchId)
		{
			...
			const int32 NumThreadGroups = FMath::DivideAndRoundUp(SortInfo.ParticleCount, NIAGARA_KEY_GEN_THREAD_COUNT);
