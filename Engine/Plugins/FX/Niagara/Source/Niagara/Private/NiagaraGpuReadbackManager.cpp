...
void FNiagaraGpuReadbackManager::Tick(FRHICommandListImmediate& RHICmdList)
{
	TickInternal(RHICmdList, false);
}

void FNiagaraGpuReadbackManager::TickInternal(FRHICommandListImmediate& RHICmdList, bool bAssumeGpuIdle)
{
	...
	while (FPendingReadback * Readback = PendingReadbacks.Peek())
	{
		...
		{
			void* DataPtr = RHICmdList.LockStagingBuffer(StagingBuffer.Key, Readback->Fence.GetReference(), 0, StagingBuffer.Value);
		...
		for (const TPair<FStagingBufferRHIRef, uint32>& StagingBuffer : Readback->StagingBuffers)
		{
			RHICmdList.UnlockStagingBuffer(StagingBuffer.Key);
		}
