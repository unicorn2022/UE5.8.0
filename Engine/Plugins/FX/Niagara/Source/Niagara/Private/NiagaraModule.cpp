...
void INiagaraModule::OnUseGlobalFXBudgetChanged(IConsoleVariable* Variable)
{
	...
	if (UseGlobalFXBudget() && FFXBudget::Enabled() == false && !IsRunningCommandlet())
	{
		...
		UE_LOGF(LogNiagara, Warning, "Niagara has enabled fx.Niagara.UseGlobalFXBudget but fx.Budget.Enabled is false so global FX budgetting will still be disabled. Consider also enabling fx.Budget.Enabled.");
	}
}
...
void INiagaraModule::StartupModule()
{
	...
	FNiagaraRuntimeTypeUtilities::Initialize();
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &INiagaraModule::OnPostEngineInit);
...
{
#if WITH_EDITOR
	UNiagaraMeshRendererProperties::RemoveMeshChangedHandlers();
#endif
...
bool FNiagaraTypeDefinition::AppendCompileHash(FNiagaraCompileHashVisitor* InVisitor) const
{
	...
	{
		UE_LOGF(LogNiagara, Error, "Invalid data in niagara type definition, might be due to broken serialization or missing DI implementation");
		return false;
	}
	...
	else if (UClass* TDClass = GetClass())
	{
		...
		if (TempDI)
		{
			if (!TempDI->AppendCompileHash(InVisitor))
			{
				...
				UE_LOGF(LogNiagara, Warning, "Unable to generate AppendCompileHash for DI %ls", PathName.ToString());
			}
		}
	}
	...
	if (!IsRunningCommandlet())
	{
		...
		for (FSoftObjectPath AssetRef : TotalStructAssets)
		{
			...
			if (Obj != nullptr)
			{
				...
				{
					UE_LOGF(LogNiagara, Warning, "Additional parameter/payload enum has moved from where it was in settings (this may cause errors at runtime): Was: \"%ls\" Now: \"%ls\"", *AssetRefPathNamePreResolve.ToString(), *Obj->GetPathName());
				}

			}
			else
			{
				UE_LOGF(LogNiagara, Warning, "Could not find additional parameter/payload type: %ls", *AssetRef.ToString());
			}
		}
		...
		for (FSoftObjectPath AssetRef : Settings->AdditionalParameterEnums)
		{
			...
			{
				UE_LOGF(LogNiagara, Warning, "Could not find additional parameter/payload enum: %ls", *AssetRef.ToString());
			}
		}
	}
}
