...
void UNiagaraComponentRendererProperties::InitCDOPropertiesAfterModuleStartup()
{
	...
	{
		if (UNiagaraComponentRendererProperties* RendererProperties = WeakComponentRendererProperties.Get())
		{
			RendererProperties->InitRendererBaseBindings();
			if (!RendererProperties->EnabledBinding.IsValid())
			{
				RendererProperties->EnabledBinding = FNiagaraConstants::GetAttributeDefaultBinding(SYS_PARAM_PARTICLES_COMPONENTS_ENABLED);
			}
			if (!RendererProperties->RendererVisibilityTagBinding.IsValid())
			{
