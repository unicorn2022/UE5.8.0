...
void FNiagaraUserRedirectionParameterStore::SanityCheckData(bool bInitInterfaces)
{
	...
	for (const FNiagaraVariable& Var : Vars)
	{
		...
		if (Offset == -1)
		{
			UE_LOGF(LogNiagara, Log, "User parameter redirect exists but no real value! Param: %ls Owner:%ls", *Var.GetName().ToString(), GetOwner() != nullptr ? *GetOwner()->GetPathName() : TEXT("Unknown owner"));
		}
	}
}
