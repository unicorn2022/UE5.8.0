...
namespace NiagaraDataInterfaceRenderTargetCommon
{
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& Args, UWorld*)
			{
				...
				if ( ensure(TextureRenderTargetFormatEnum) )
				{
					...
					UE_LOGF(LogNiagara, Log, "Niagara RenderTarget Override is '%ls' with format '%ls'.", GNiagaraRenderTargetOverrideFormatEnabled ? TEXT("Enabled") : TEXT("Disabled"), *TextureRenderTargetFormatEnum->GetNameStringByIndex(int32(GNiagaraRenderTargetOverrideFormat)));
				}
			}
		)
	);
	...
	const ETextureRenderTargetFormat GetRenderTargetFormatFromPixelFormat(EPixelFormat InPixelFormat)
	{
		...
		UE_LOGF(LogNiagara, Error, "No mapping from pixel format to render target format is possible");
