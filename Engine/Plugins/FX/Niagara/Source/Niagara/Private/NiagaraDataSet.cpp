...
void NiagaraWarnGpuBufferSize(const int32 ByteSize, const TCHAR* DebugSimName)
{
	...
	if ((GNiagaraGPUDataWarningSize > 0) && (ByteSize > GNiagaraGPUDataWarningSize) && (MaxEncountered < ByteSize))
	{
		...
		UE_LOGF(LogNiagara, Warning, "GpuBuffer Allocation (%d bytes) from %ls", ByteSize, DebugSimName);
	}
}
...
void FNiagaraDataSet::Allocate(int32 NumInstances, bool bMaintainExisting)
{
	...
	if (RequiresPersistentIDs())
	{
		...
		{
			//UE_LOGF(LogNiagara, Warning, "Growing ID Table! OldSize:%d | NewSize:%d", ExistingNumIDs, RequiredIDs);
			...
			//UE_LOGF(LogNiagara, Warning, "DataSetAllocate: Adding New Free IDs: %d - ", NewNumIds);
		}
#if 0
		else if (RequiredIDs < ExistingNumIDs >> 1)//Configurable?
		{
			...
			//UE_LOGF(LogNiagara, Warning, "DataSetAllocate: Shrinking ID Table! OldSize:%d | NewSize:%d", ExistingNumIDs, RequiredIDs);
			for (int32 CheckedFreeID = 0; CheckedFreeID < NumFreeIDs;)
			{
				...
				{
					//UE_LOGF(LogNiagara, Warning, "RemoveSwap FreeID: Removed:%d | Swapped:%d", FreeIDsTable[CheckedFreeID], FreeIDsTable.Last());		
...
// 		UE_LOGF(LogNiagara, Warning, "DataSetAllocate: NumInstances:%d | ID Table Size:%d | NumFreeIDs:%d | FreeTableSize:%d", NumInstances, DestinationData->GetIDTable().Num(), NumFreeIDs, FreeIDsTable.Num());
// 		UE_LOGF(LogNiagara, Warning, "== FreeIDs %d ==", NumFreeIDs);
// 		for (int32 i=0; i< NumFreeIDs;++i)
// 		{
// 			UE_LOGF(LogNiagara, Warning, "%d", FreeIDsTable[i]);
// 		}
	}
...
void FNiagaraDataBuffer::TransferGPUToCPUImmediate(FRHICommandListImmediate& RHICmdList, FNiagaraGpuComputeDispatchInterface* ComputeInterface, FNiagaraDataBuffer* CPUBuffer) const
{
	...
	{
		const uint32* DataPtr = static_cast<const uint32*>(RHICmdList.LockStagingBuffer(CountReadbackBuffer, 0, sizeof(int32)));
		GPUNumInstances = *DataPtr;
		RHICmdList.UnlockStagingBuffer(CountReadbackBuffer);
	}
	...
	if (FloatReadbackBuffer)
	{
		...
		const uint8* GPUData	= reinterpret_cast<uint8*>(RHICmdList.LockStagingBuffer(FloatReadbackBuffer, 0, GPUBufferFloat.NumBytes));
		...
		RHICmdList.UnlockStagingBuffer(FloatReadbackBuffer);
	}

	if (HalfReadbackBuffer)
	{
		const uint32 GPUStride = HalfStride;
		const uint8* GPUData = reinterpret_cast<uint8*>(RHICmdList.LockStagingBuffer(HalfReadbackBuffer, 0, GPUBufferHalf.NumBytes));
		...
		RHICmdList.UnlockStagingBuffer(HalfReadbackBuffer);
	}

	if (Int32ReadbackBuffer)
	{
		const uint32 GPUStride = Int32Stride;
		const uint8* GPUData = reinterpret_cast<uint8*>(RHICmdList.LockStagingBuffer(Int32ReadbackBuffer, 0, GPUBufferInt.NumBytes));
		...
		RHICmdList.UnlockStagingBuffer(Int32ReadbackBuffer);
	}
}
...
void FNiagaraDataBuffer::Dump(int32 StartIndex, int32 InNumInstances, const FString& Label, const FName& SortParameterKey)const
{
	...
	UE_LOGF(LogNiagara, Log, "%ls", *Sep);
	UE_LOGF(LogNiagara, Log, " %ls ", *Label);
	UE_LOGF(LogNiagara, Log, "%ls", *Sep);
	// 	UE_LOGF(LogNiagara, Log, "%ls", *HeaderStr);
	// 	UE_LOGF(LogNiagara, Log, "%ls", *Sep);
	if (bKeyFound)
	{
		for (TPair<float, FString>& Str : Lines)
		{
			UE_LOGF(LogNiagara, Log, "[%f] %ls", Str.Key, *Str.Value);
		}
	}
	else
	{
		for (TPair<float, FString>& Str : Lines)
	{
			UE_LOGF(LogNiagara, Log, "%ls", *Str.Value);
		}
	}
	if (IDToIndexTable.Num() > 0)
	{
		UE_LOGF(LogNiagara, Log, "== ID Table ==");
		for (int32 i = 0; i < IDToIndexTable.Num(); ++i)
		{
			UE_LOGF(LogNiagara, Log, "%d = %d", i, IDToIndexTable[i]);
		}
	}
	UE_LOGF(LogNiagara, Log, "%ls", *Sep);
