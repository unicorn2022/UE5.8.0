...
bool FNiagaraSystemSimulation::Init(UNiagaraSystem* InSystem, UWorld* InWorld, bool bInIsSolo, ETickingGroup InTickGroup, const FNiagaraParameterCollectionResolver& ParameterCollectionResolver)
{
	...
	if (bCanExecute)
	{
...

		{
			...
			bCanExecute &= SpawnExecContext->Init(nullptr, SpawnScript, System->GetScriptRuntimeData(nullptr, *SpawnScript));
			bCanExecute &= UpdateExecContext->Init(nullptr, UpdateScript, System->GetScriptRuntimeData(nullptr, *UpdateScript));
		}

		{
			//SCOPE_CYCLE_COUNTER(STAT_NiagaraSystemSim_Init_BindParams);

			ParameterCollectionResolver.BindCollections(SpawnScript, SpawnExecContext->Parameters);
			ParameterCollectionResolver.BindCollections(UpdateScript, UpdateExecContext->Parameters);
...
void FNiagaraSystemSimulation::TransferInstance(FNiagaraSystemInstance* Instance)
{
	...
	switch ( Instance->SystemInstanceState)
	{
		...
		case ENiagaraSystemInstanceState::Spawning:
			UE_LOGF(LogNiagara, Fatal, "Attempting to transfer an instance that is in the spawning state");
			break;
...
void FNiagaraSystemSimulation::DumpInstance(const FNiagaraSystemInstance* Inst)const
{
	...
	UE_LOGF(LogNiagara, Log, "==  %ls (%d) ========", *Inst->GetSystem()->GetFullName(), Inst->SystemInstanceIndex);
	...
	UE_LOGF(LogNiagara, Log, "................. System Instance .................");
...
void FNiagaraSystemSimulation::SimCacheTick_GameThread(UNiagaraSimCache* SimCache, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	TArray<FNiagaraSystemInstance*>& RunningSystemInstances = GetSystemInstances(ENiagaraSystemInstanceState::Running);
	TArray<FNiagaraSystemInstance*>& PendingSpawnSystemInstances = GetSystemInstances(ENiagaraSystemInstanceState::PendingSpawn);

	//-OPT: On cooked builds we do not need to do this every frame
	if (SimCache->CanRead(System) == false)
	{
		for (int32 i=RunningSystemInstances.Num() - 1; i >= 0; --i)
		{
			RunningSystemInstances[i]->Complete(false);
		}

		for (int32 i=PendingSpawnSystemInstances.Num() - 1; i >= 0; --i)
		{
			PendingSpawnSystemInstances[i]->Complete(false);
		}
		return;
	}
	...
	if (GNiagaraSystemSimulationAllowASyncSimCache && SimCache->CanRunAsync())
	{
		...
		TSharedPtr<FFinalizeTickBatch> FinalizeTickBatch(new FFinalizeTickBatch());
		...
		for (FNiagaraSystemInstance* SystemInstance : RunningSystemInstances)
		{
			SystemInstance->ConcurrentTickGraphEvent = ConcurrentTickGraphEvent;
		}

		for (FNiagaraSystemInstance* SystemInstance : PendingSpawnSystemInstances)
		{
			SystemInstance->ConcurrentTickGraphEvent = ConcurrentTickGraphEvent;
		}
		...
		UE::Tasks::FTask ConcurrentTask = UE::Tasks::Launch(
			...
			[this, SimCache, DeltaSeconds, FinalizeTickBatch]()
			{
				...
				TArray<FNiagaraSystemInstance*>& RunningSystemInstances = GetSystemInstances(ENiagaraSystemInstanceState::Running);
				FinalizeTickBatch->Instances.Reserve(RunningSystemInstances.Num());
				for (FNiagaraSystemInstance* SystemInstance : RunningSystemInstances)
				{
		...
		UE::Tasks::Launch(
			...
			[this, FinalizeTickBatch]()
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(Niagara::SimCacheTick_Finalize);

				for (FNiagaraSystemInstance* Instance : FinalizeTickBatch->Instances)
				{
					if (Instance)
					{
			...
			#if DO_CHECK
				ensureMsgf(FinalizeTickBatch->DebugCounter.load() == 0, TEXT("Finalize batch is complete but counter is %d when it should be zero"), FinalizeTickBatch->DebugCounter.load());
			#endif
	...
	else
	{
		...
		for ( int32 i=RunningSystemInstances.Num()-1; i >= 0; --i)
		{
			RunningSystemInstances[i]->SimCacheTickFinalize_GameThread();
		}
	}
}
...
void FNiagaraSystemSimulation::DumpStalledInfo()
{
	...
	UE_LOGF(LogNiagara, Fatal, "NiagaraSystemSimulation(%ls) is stalled.\n%ls", *GetNameSafe(GetSystem()), Builder.ToString());
}
...
void FNiagaraSystemSimulation::SpawnSystemInstances(FNiagaraSystemSimulationTickContext& Context)
{
	...
	if (GbDumpSystemData || Context.System->bDumpDebugSystemInfo)
	{
		UE_LOGF(LogNiagara, Log, "=== Spwaned %d Systems ===", NumInstances);
...
void FNiagaraSystemSimulation::UpdateSystemInstances(FNiagaraSystemSimulationTickContext& Context)
{
	...
	if (NumInstances > 0)
	{
		...
		if (GbDumpSystemData || Context.System->bDumpDebugSystemInfo)
		{
			UE_LOGF(LogNiagara, Log, "=== Updated %d Systems ===", NumInstances);
			DestinationData.Dump(0, NumInstances, TEXT("System Data - Post Update"));
			UpdateInstanceParameterDataSet.GetCurrentDataChecked().Dump(0, NumInstances, TEXT("Update Instance Parameter Data"));
		}
		...
		if ( (SpawnNum > 0) && GNiagaraSystemSimulationUpdateOnSpawn)
		{
			...
			if (GNiagaraSystemSimulationUpdateOnSpawn == 1)
			{
			...
			if (GbDumpSystemData || Context.System->bDumpDebugSystemInfo)
			{
				UE_LOGF(LogNiagara, Log, "=== Spawn Updated %d Systems ===", SpawnNum);
				DestinationData.Dump(OrigNum, SpawnNum, TEXT("System Data - Post Update (new systems)"));
				UpdateInstanceParameterDataSet.GetCurrentDataChecked().Dump(OrigNum, SpawnNum, TEXT("Update Instance Parameter Data (new systems)"));
			}
		}
			...
			for (int32 EmitterIdx = 0; EmitterIdx < Emitters.Num(); ++EmitterIdx)
			{
				...
				if ( FNiagaraEmitterInstanceImpl* StatefulEmitter = EmitterInstance.AsStateful() )
				{
					...
					for (int32 EventIdx = 0; EventIdx < EventContexts.Num(); ++EventIdx)
					{
						...
						{
							UE_LOGF(LogNiagara, Log, "Skipping DataSetToEmitterEventParameters because EventIdx is out-of-bounds. %d of %d", EventIdx, DataSetToEmitterEventParameters[EmitterIdx].Num());
						}
					}
				}
			}
