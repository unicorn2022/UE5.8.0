...
TSharedPtr<FNiagaraCompileRequestDataBase, ESPMode::ThreadSafe> FNiagaraLazyPrecompileReference::GetPrecompileData(UNiagaraScript* ForScript)
{
	...
	{
		UE_LOGF(LogNiagara, Error, "Failed to precompile %ls.  This is due to unexpected invalid or broken data.  Additional details should be in the log.", *System->GetPathName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogNiagara, Log, "Failed to precompile %ls. This is probably because the system was modified between the original compile request and now.", *System->GetPathName());
		return nullptr;
	}
...
void FNiagaraAsyncCompileTask::MoveToState(ENiagaraCompilationState NewState)
{
	...
	UE_LOGF(LogNiagara, Verbose, "Changing state %i -> %i for for %ls!", int(CurrentState), int(NewState), *AssetPath);
...
void LogVMID(const FNiagaraVMExecutableDataId& ID, FString Name)
{
	...
	UE_LOGF(LogNiagara, Display, "FNiagaraVMExecutableDataId %ls:", *Name);
	UE_LOGF(LogNiagara, Display, "  ScriptUsageType: %i", int(ID.ScriptUsageType));
	UE_LOGF(LogNiagara, Display, "  ScriptUsageTypeID: %ls", *ID.ScriptUsageTypeID.ToString());
	UE_LOGF(LogNiagara, Display, "  CompilerVersionID: %ls", *ID.CompilerVersionID.ToString());
	UE_LOGF(LogNiagara, Display, "  BaseScriptCompileHash: %ls", *ID.BaseScriptCompileHash.ToString());
	UE_LOGF(LogNiagara, Display, "  bUsesRapidIterationParams: %i", ID.bUsesRapidIterationParams);
	UE_LOGF(LogNiagara, Display, "  AdditionalDefines:");
	for (int32 Idx = 0; Idx < ID.AdditionalDefines.Num(); Idx++)
	{
		UE_LOGF(LogNiagara, Display, "     %ls", *ID.AdditionalDefines[Idx]);
	}
	UE_LOGF(LogNiagara, Display, "  AdditionalVariables:");
	for (int32 Idx = 0; Idx < ID.AdditionalVariables.Num(); Idx++)
	{
		UE_LOGF(LogNiagara, Display, "     %ls / %ls", *ID.AdditionalVariables[Idx].GetName().ToString(), *ID.AdditionalVariables[Idx].GetType().GetName());
	}
	UE_LOGF(LogNiagara, Display, "  ReferencedCompileHashes:");
	for (int32 HashIndex = 0; HashIndex < ID.ReferencedCompileHashes.Num(); HashIndex++)
	{
		UE_LOGF(LogNiagara, Display, "     %ls", *ID.ReferencedCompileHashes[HashIndex].ToString());
	}
}
...
void FNiagaraAsyncCompileTask::PrecompileData()
{
	...
	UE_LOGF(LogNiagara, Verbose, "Getting precompile data for %ls!", *AssetPath);
	...
	{
		UE_LOGF(LogNiagara, Warning, "Unable to proceed with compilation due to changes to asset %ls.  Aborting.", *AssetPath);
		AbortTask();
		return;
	}
...
{
	UE_LOGF(LogNiagara, Verbose, "Starting compilation for %ls!", *AssetPath);
	...
	{
		UE_LOGF(LogNiagara, Verbose, "Designated compilation ID %i for compilation of %ls!", ScriptPair.PendingJobID, *AssetPath);
		bUsedShaderCompilerWorker = true;
	}
	else
	{
		UE_LOGF(LogNiagara, Warning, "For some reason we are reporting that %ls is in sync even though AreScriptAndSourceSynchronized returned false!", *ScriptPair.CompiledScript->GetPathName())
		AbortTask();
	}
}
...
{
	UE_LOGF(LogNiagara, Verbose, "Waiting on compilation to finish for %ls!", *AssetPath);
...
{
	check(ExeData);
	UE_LOGF(LogNiagara, Verbose, "Processing compilation results for %ls!", *AssetPath);
	...
	ExeData->BakedRapidIterationParameters = BakedRapidIterationParameters;
	ScriptPair.CompiledScript->ExecToBinaryData(ScriptPair.CompiledScript, DDCOutData, *ExeData);
	UE_LOGF(LogNiagara, Verbose, "Got %i bytes in ddc data for %ls", DDCOutData.Num(), *AssetPath);
}
	...
	if (DDC->PollAsynchronousCompletion(TaskHandle))
	{
		...
		if (bHasResults && DDCOutData.Num() > 0)
		{
			...
			if (ScriptPair.CompiledScript->BinaryToExecData(ScriptPair.CompiledScript, DDCOutData, *ExeData))
			{
				...
				MoveToState(ENiagaraCompilationState::Finished);
				UE_LOGF(LogNiagara, Verbose, "Compilation data for %ls could be pulled from the ddc.", *AssetPath);
			}
			else
			{
				UE_LOGF(LogNiagara, Warning, "Unable to create exec data from ddc data for script %ls, going to recompile it from scratch. DDC might be corrupted or there is a problem with script serialization.", *AssetPath);
				ExeData.Reset();
		...
		{
			MoveToState(ENiagaraCompilationState::Precompile);
			UE_LOGF(LogNiagara, Verbose, "No compilation data for %ls found in the ddc.", *AssetPath);
		}
		CompileMetrics.DDCFetchTime = (float) (FPlatformTime::Seconds() - StartTaskTime);
		UE_LOGF(LogNiagara, VeryVerbose, "Compile task for %ls took %f seconds to fetch data from ddc.", *AssetPath, CompileMetrics.DDCFetchTime);
	}
	else if ((FPlatformTime::Seconds() - StartTaskTime) > GNiagaraCompileDDCWaitTimeout)
	{
		MoveToState(ENiagaraCompilationState::Precompile);
		UE_LOGF(LogNiagara, Log, "DDC timed out after %i seconds, starting compilation for %ls.", (int)GNiagaraCompileDDCWaitTimeout, *AssetPath);
	}
}
...
void FNiagaraAsyncCompileTask::PutToDDC()
{
	if (DDCOutData.Num() > 0)
	{
		UE_LOGF(LogNiagara, Verbose, "Writing data for %ls to the ddc", *AssetPath);
...
bool FNiagaraAsyncCompileTask::CompilationIdMatchesRequest() const
{
	...
	if (!IdMatches)
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Compile ID for %ls changed during the compilation task.", *AssetPath);
...
bool FNiagaraActiveCompilationDefault::ValidateConsistentResults(const FNiagaraQueryCompilationOptions& Options) const
{
	...
	if (SpawnScriptValid && UpdateScriptValid)
	{
		...
		if (SpawnScriptPair.CompileResults->Attributes != UpdateScriptPair.CompileResults->Attributes)
		{
			...
			{
				UE_LOGF(LogNiagara, Log, "Failed to generate consistent results for System spawn and update scripts for system %ls.", *Options.System->GetFullName());
			}
		...
		for (UNiagaraScript* Script : ScriptsNeedingCompile)
		{
			if (FAsyncTaskPtr* AsyncTaskPtr = FindTask(Script))
			{
				if (ensure(*AsyncTaskPtr))
				{
					...
					UE_LOGF(LogNiagara, Verbose, "Scheduling async get task for %ls", *CompileTask.AssetPath);
...
void FNiagaraActiveCompilationDefault::ReportResults(const FNiagaraQueryCompilationOptions& Options) const
{
	...
	if (bHasCompiledJobs)
	{
		UE_LOGF(LogNiagara, Log, "Compiling System %ls took %f sec (time since issued), %f sec (combined shader worker time).",
			*Options.System->GetFullName(), ElapsedWallTime, CombinedCompileTime);
	}
	else if (bAllScriptsSynchronized == false)
	{
		UE_LOGF(LogNiagara, Verbose, "Retrieving %ls from DDC took %f sec.", *Options.System->GetFullName(), ElapsedWallTime);
	}
