...
void UNiagaraDataInterfaceSpriteRendererInfo::GetFeedback(UNiagaraSystem* InAsset, UNiagaraComponent* InComponent, TArray<FNiagaraDataInterfaceError>& OutErrors, TArray<FNiagaraDataInterfaceFeedback>& OutWarnings, TArray<FNiagaraDataInterfaceFeedback>& OutInfo)
{
	...
	else if ( !SpriteRenderer->IsContainedInEmitterRendererArray() )
	{
		OutWarnings.Emplace(
			LOCTEXT("OrphanedRendererWarning", "The selected Sprite Renderer is invalid"),
			LOCTEXT("OrphanedRendererWarningSummary", "Sprite Renderer is orphaned and no longer valid"),
			FNiagaraDataInterfaceFix::CreateLambda(
				[this]()
				{
					const FScopedTransaction Transaction(LOCTEXT("ClearSpriteRenderer", "Clear Sprite Renderer"));
					Modify();
