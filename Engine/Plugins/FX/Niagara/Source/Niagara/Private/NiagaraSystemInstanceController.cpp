	...
	USceneComponent* AttachComponent, ENiagaraTickBehavior TickBehavior, bool bPooled, int32 RandomSeedOffset, bool bForceSolo, int32 InWarmupTickCount, float InWarmupTickDelta)
{
	...
	//UE_LOGF(LogNiagara, Log, "Create System: %p | %ls\n", SystemInstance.Get(), *GetAsset()->GetFullName());
...
void FNiagaraSystemInstanceController::GetMaterialStreamingInfo(FNiagaraMaterialAndScaleArray& OutMaterialAndScales) const
{
	...
	// If renderers read from parameters that might have changed in the parent we need to ensure it's flushed
	// Note that this can be called during AddPrimitive on concurrent threads
	if ( IsInGameThread() )
	{
...
void FNiagaraSystemInstanceController::DebugDump(bool bFullDump)
{
	if (SystemInstance.IsValid())
	{
		...
		UE_LOGF(LogNiagara, Log, "\tSystem ExecutionState(%ls) RequestedExecutionState(%ls)", *ExecutionStateEnum->GetNameStringByIndex((int32)SystemInstance->GetActualExecutionState()), *ExecutionStateEnum->GetNameStringByIndex((int32)SystemInstance->GetRequestedExecutionState()));
		...
		if (!SystemInstance->IsComplete())
		{
			for (const FNiagaraEmitterInstanceRef& Emitter : SystemInstance->GetEmitters())
			{
				...
				{
					UE_LOGF(LogNiagara, Log, "\tEmitter '%ls' ExecutionState(%ls) NumParticles(%d)", *Emitter->GetEmitterHandle().GetUniqueInstanceName(), *ExecutionStateEnum->GetNameStringByIndex((int32)Emitter->GetExecutionState()), Emitter->GetNumParticles());
				}
			}
		}
		...
		if (bFullDump)
		{
			UE_LOGF(LogNiagara, Log, "=========================== Begin Full Dump ===========================");
			SystemInstance->Dump();
			UE_LOGF(LogNiagara, Log, "=========================== End Full Dump ===========================");
		}
	}
}
...
{
	check(IsInRenderingThread() || IsInGameThread() || IsAsyncLoading() || UE::IsSavingPackage()); // Same restrictions as MIDs
				...
				[&](const UNiagaraRendererProperties* Properties)
				{
					...
					for (UMaterialInterface*& ExistingMaterial : UsedMaterials)
					{
						if (ExistingMaterial)
						{
							...
							// Create a new MID
							//UE_LOGF(LogNiagara, Log, "Create Dynamic Material for component %ls", *GetPathName());
...
void FNiagaraSystemInstanceController::SetVariable_InternalUseOnly_Deferred(const FNiagaraVariableBase& Variable, const FNiagaraVariant& Variant)
{
	SetVariable_Deferred(Variable, Variant);
}

void FNiagaraSystemInstanceController::SetVariable(FNiagaraVariableBase Variable, FNiagaraVariant Variant)
{
	if (OverrideParameters && Variant.IsValid())
	{
		//TODO: Unify this modification code with the typed setters so it doesn't diverge/become a maintainance headache.
		bool bIsMaterial = Variable.GetType() == FNiagaraTypeDefinition::GetUMaterialDef();
		bool bIsStaticMesh = Variable.GetType() == FNiagaraTypeDefinition::GetUStaticMeshDef();
		bool bUpdateMaterialsIfModified = bIsMaterial || bIsStaticMesh;
		bool bMarkNeedsTickOnModified = bIsStaticMesh;
		bool bMarkNeedsRendererRecacheOnModified = bIsStaticMesh;
		bool bCheckModified = Variant.GetMode() == ENiagaraVariantMode::Object && (bUpdateMaterialsIfModified || bMarkNeedsRendererRecacheOnModified || bMarkNeedsTickOnModified);

		bool bUObjectModified = false;
		if (bCheckModified)
		{
			UObject* CurrValue = OverrideParameters->GetUObject(Variable);
			bUObjectModified = CurrValue != Variant.GetUObject();
		}

		OverrideParameters->SetParameter_InternalUseOnly(Variable, Variant, true);

		//Handle any modification updates for some uobject types.
		if (bUObjectModified)
		{
			if (bUpdateMaterialsIfModified)
			{
				bNeedsUpdateEmitterMaterials = true;
			}
			if (bMarkNeedsTickOnModified)
			{
				bNeedsOverrideParametersTicked = true;
			}
			if (bMarkNeedsRendererRecacheOnModified)
			{
