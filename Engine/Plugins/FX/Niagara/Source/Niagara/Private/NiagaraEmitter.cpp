...
static bool GNiagaraEmitterPostLoadCleanup = true;
static FAutoConsoleVariableRef CVarRollback_NiagaraEmitterPostLoadCleanup(
	TEXT("fx.Niagara.Rollback.NiagaraEmitterPostLoadCleanup"),
	GNiagaraEmitterPostLoadCleanup,
	TEXT("When true use a leaner version of post load"),
	ECVF_Default
);
...
static FAutoConsoleVariableRef CVarNiagaraEmitterComputePSOPrecacheMode(
	...
	TEXT("1 = Enabled if r.PSOPrecaching is also enabled. Emitters are not allowed to run until they complete if r.PSOPrecache.ProxyCreationStrategy is not set to AlwaysCreate\n")
...
void FNiagaraEmitterScriptProperties::InitDataSetAccess()
{
	...
	{
		//UE_LOGF(LogNiagara, Log, "InitDataSetAccess: %ls %d %d", *Script->GetPathName(), Script->ReadDataSets.Num(), Script->WriteDataSets.Num());
...
TArray<INiagaraMergeManager::FMergeEmitterResults> UNiagaraEmitter::MergeChangesFromParent()
{
	...
	for (FVersionedNiagaraEmitterData& Data : VersionData)
	{
		...
		{
			UE_LOGF(LogNiagara, Log, "Emitter %ls is merging changes from parent %ls because its Change ID was updated.", *GetPathName(),
				Data.VersionedParent.Emitter != nullptr ? *Data.VersionedParent.Emitter->GetPathName() : TEXT("(null)"));

			UE_LOGF(LogNiagara, Log, "\nEmitter %ls Id=%ls \nParentAtLastMerge %ls id=%ls \nParent %ls Id=%ls.", 
		...
		{
			UE_LOGF(LogNiagara, Warning, "Failed to merge changes for parent emitter.  Emitter: %ls  Parent Emitter: %ls  Error Message: %ls",
		...
		for (UNiagaraSimulationStageBase* Stage : Data.SimulationStages)
		{
			if (Stage)
			{
				if (Stage->Script)
				{
					...
					{
						UE_LOGF(LogNiagara, Error, "Emitter \"%ls\" has a simulation stage with a Transient script and the emitter itself isn't transient!", *GetPathName());
					}
				}
				else
				{
					UE_LOGF(LogNiagara, Error, "Emitter \"%ls\" has a simulation stage with a null Script entry!", *GetPathName());
				}

			}
			else
			{
				UE_LOGF(LogNiagara, Error, "Emitter \"%ls\" has a simulation stage with a null entry!", *GetPathName());
			}
		}
	}
...
void FVersionedNiagaraEmitterData::PostLoad(UNiagaraEmitter& Emitter, int32 NiagaraVer)
{
	...
	if (!Emitter.GetOutermost()->bIsCookedForEditor)
	{
		...
		if (GNiagaraEmitterPostLoadCleanup == false)
		{
		...
		if (IsRunningCommandlet() && Emitter.IsSynchronizedWithParent() == false)
		{
	...
	if (SpawnScriptProps.Script)
	{
		...
		if (bActualInterpolatedSpawning != bUsesInterpolatedSpawning)
		{
			...
			UE_LOGF(LogNiagara, Warning, "Disabling interpolated spawn because emitter flag and script type don't match. Did you adjust this value in the UI? Emitter may need recompile.. %ls", *Emitter.GetFullName());
		}
	}
	...
	// Gather up custom asset tags for DataInterfaces
	{
		...
		auto AddDIs = [&](UNiagaraScript* Script)
		{
			if (Script)
			{
				...
				{
					const UNiagaraDataInterface* DefaultDataInterface = Info.GetDefaultDataInterface();
					if (DefaultDataInterface != nullptr)
					{
...
bool UNiagaraEmitter::NeedsLoadForTargetPlatform(const ITargetPlatform* TargetPlatform)const
{
	...
	{
		UE_LOGF(LogNiagara, Verbose, "Pruned emitter %ls for platform %ls from system scalability", *GetFullName(), *TargetPlatform->DisplayName().ToString())
		return false;
	}
	...
	if(!bIsEnabled)
	{
		UE_LOGF(LogNiagara, Verbose, "Pruned emitter %ls for platform %ls", *GetFullName(), *TargetPlatform->DisplayName().ToString())
	}
	return bIsEnabled;
}
...
void UNiagaraEmitter::PostEditChangeVersionedProperty(FPropertyChangedEvent& PropertyChangedEvent, const FGuid& Version)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(FVersionedNiagaraEmitterData, InterpolatedSpawnMode))
	{
		...
		if (EmitterInterpolationMode != ScriptInterpolationMode)
		{
			...
			UE_LOGF(LogNiagara, Log, "Updating script usage: Script->IsInterpolatdSpawn %d Emitter->bInterpolatedSpawning %d", (int32)EmitterData->SpawnScriptProps.Script->IsInterpolatedParticleSpawnScript(), EmitterData->UsesInterpolatedSpawning());
...
void FVersionedNiagaraEmitterData::CacheFromCompiledData(const FNiagaraDataSetCompiledData* CompiledData, const UNiagaraEmitter& Emitter, const FGuid& EmitterHandleId)
{
	...
	bIsAllowedToExecute &= FNiagaraComponentSettings::IsEmitterAllowedToRun(*this, Emitter, EmitterHandleId);
}
...
void FVersionedNiagaraEmitterData::RebuildRendererBindings(const UNiagaraEmitter& Emitter)
{
	...
	{
		TArray<TPair<UNiagaraScript*,const FNiagaraScriptRuntimeData*>> TargetScriptPairs;
		const FNiagaraEmitterHandle* EmitterHandle = nullptr;
		UNiagaraSystem* SystemOwner = Emitter.GetTypedOuter<UNiagaraSystem>();
		if (SystemOwner != nullptr)
		{
			TargetScriptPairs.Add(TPair<UNiagaraScript*,const FNiagaraScriptRuntimeData*>(
				SystemOwner->GetSystemSpawnScript(), SystemOwner->GetScriptRuntimeData(nullptr, *SystemOwner->GetSystemSpawnScript()).Get()));
			TargetScriptPairs.Add(TPair<UNiagaraScript*, const FNiagaraScriptRuntimeData*>(
				SystemOwner->GetSystemUpdateScript(), SystemOwner->GetScriptRuntimeData(nullptr, *SystemOwner->GetSystemUpdateScript()).Get()));
			EmitterHandle = SystemOwner->GetEmitterHandles().FindByPredicate([this]
				(const FNiagaraEmitterHandle& EmitterHandle) { return EmitterHandle.GetInstance().GetEmitterData() == this; });
		}

		if (EmitterHandle != nullptr)
		{
			TArray<UNiagaraScript*> EmitterScripts;
			GetScripts(EmitterScripts, false, true);
			for (UNiagaraScript* EmitterScript : EmitterScripts)
			{
				TargetScriptPairs.Add(TPair<UNiagaraScript*, const FNiagaraScriptRuntimeData*>(EmitterScript, SystemOwner->GetScriptRuntimeData(EmitterHandle, *EmitterScript).Get()));
			}
		}
		...
		{
			for (TPair<UNiagaraScript*, const FNiagaraScriptRuntimeData*>& TargetScriptPair : TargetScriptPairs)
			{
				UNiagaraScript* Script = TargetScriptPair.Key;
				if (Script == nullptr)
				{
					continue;
				}
				...
				{
					const FNiagaraScriptExecutionParameterStore* ScriptParameterStore = TargetScriptPair.Value != nullptr
						? TargetScriptPair.Value->GetExecutionReadyParameterStore() : nullptr;
					if (ScriptParameterStore != nullptr)
					{
		...
		if (RendererBindings.GetUObjects().Num() > 0)
		{
			for (TPair<UNiagaraScript*, const FNiagaraScriptRuntimeData*>& TargetScriptPair : TargetScriptPairs)
			{
				const UNiagaraScript* Script = TargetScriptPair.Key;
...
void UNiagaraEmitter::UpdateEmitterAfterLoad()
{
	...
	PostLoadDefinitionsSubscriptions(
		...
		[](UObject* ObjectOwner) -> TArray<INiagaraParameterDefinitionsSubscriber*>
		{
			...
			if (UNiagaraEmitter* Owner = Cast<UNiagaraEmitter>(ObjectOwner))
			{
				OutArray.Add(Owner);
			}
			return OutArray;
	...
	if (!GetOutermost()->bIsCookedForEditor)
	{
		...
		if (GetForceCompileOnLoad())
		{
			...
			if (OuterObj == GetOutermost())
			{
				...
				{
					UE_LOGF(LogNiagara, Log, "InvalidateCachedCompileIds for %ls because GbForceNiagaraCompileOnLoad = %d", *GetPathName(), GbForceNiagaraCompileOnLoad);
				}
			}
		}
		...
		if (ChangeId.IsValid() == false)
		{
			...
			if (GEnableVerboseNiagaraChangeIdLogging)
			{
				UE_LOGF(LogNiagara, Log, "Change ID updated for emitter %ls because the ID was invalid.", *GetPathName());
			}
		}
...
void FVersionedNiagaraEmitterData::OnPostCompile(const UNiagaraEmitter& InEmitter)
{
	...
	RuntimeEstimation.SetIsEstimationDirty(false);
#if STATS
	StatDatabase.Init();
...
struct FNiagaraEmitterVersionDataAccessor final : FNiagaraVersionDataAccessor
{
	...
	virtual TSubclassOf<UNiagaraVersionUpgradeBase>& GetUpdateUtilityClass() override { return EmitterData->UpdateUtilityClass; }
...
void UNiagaraEmitter::UpdateChangeId(const FString& Reason)
{
	...
	if (GbEnableEmitterChangeIdMergeLogging)
	{
		UE_LOGF(LogNiagara, Log, "Emitter %ls change id updated. Reason: %ls OldId: %ls NewId: %ls",
...
void FVersionedNiagaraEmitterData::ClearRuntimeAllocationEstimate(uint64 ReportHandle)
{
	...
	if (ReportHandle == INDEX_NONE)
	{
		RuntimeEstimation.AllocationEstimate = 0;
		RuntimeEstimation.RuntimeAllocations.Empty();
		RuntimeEstimation.SetIsEstimationDirty(true);
	}
	else
	{
...
int32 FVersionedNiagaraEmitterData::GetMaxParticleCountEstimate()
{
	...
	if (RuntimeEstimation.GetIsEstimationDirty())
	{
		FScopeLock lock(RuntimeEstimation.GetCriticalSection());
...
FMemoryRuntimeEstimation::FMemoryRuntimeEstimation()
{
...
// Register global PSO collector so compile time and tracked in the stats
static const TCHAR* NiagaraComputePSOCollectorName = TEXT("NiagaraComputePSOCollector");
FRegisterGlobalPSOCollectorFunction RegisterNiagaraComputePSOCollector(nullptr, NiagaraComputePSOCollectorName);
...
FGraphEventArray FVersionedNiagaraEmitterData::PrecacheComputePSOs(const UNiagaraEmitter& NiagaraEmitter)
{
	...
	if (GNiagaraEmitterComputePSOPrecacheMode == 3)
	{
		...
		PSORequestParams.PrecachePriority = EPSOPrecachePriority::High;
		PSORequestParams.CollectorName = NiagaraComputePSOCollectorName;
		PSORequestParams.ResourceName = NiagaraEmitter.GetName();
		PSORequestParams.bForcePrecache = true;

		FGraphEventArray PSOPrecacheEvents;
		for (int32 i = 0; i < ShaderScript->GetNumPermutations(); ++i)
		{
			FNiagaraShaderRef ShaderRef = ShaderScript->GetShaderGameThread(i);
			if (ShaderRef.IsValid() == false)
			{
				continue;
			}

			FRHIComputeShader* ComputeShader = ShaderRef.GetComputeShader();
			FPSOPrecacheRequestResult PSOPrecacheRequestResult = PipelineStateCache::PrecacheComputePipelineState(ComputeShader, PSORequestParams);
			...
			void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
			{
				...
				for (int32 i = 0; i < ShaderScript->GetNumPermutations(); ++i)
				{
					...
					if (ShaderResult != EPSOPrecacheResult::Complete)
					{
						...
						UE_LOGF(LogNiagara, Warning, "Niagara ComputePSOPrecache Failed for Emitter(%ls) System(%ls), emitter will not run.", *GetNameSafe(VersionedEmitter.Emitter), *GetNameSafe(NiagaraSystem));
						return;
					}
				}
	...
	else if (GNiagaraEmitterComputePSOPrecacheMode == 2 || PipelineStateCache::IsPSOPrecachingEnabled() )
	{
		...
		PSOInitializers.Reserve(ShaderScript->GetNumPermutations());
		
		static const int32 NiagaraEmitterPSOCollectorIndex = FGlobalPSOCollectorManager::GetIndex(NiagaraComputePSOCollectorName);
		...
		for (int32 i=0; i < ShaderScript->GetNumPermutations(); ++i)
		{
			...
			PSOPrecacheData.ComputeInitializer = FComputePipelineStateInitializer(ShaderRef.GetComputeShader());
			PSOPrecacheData.InMemoryMode = EPSOPrecacheInMemoryMode::Uncached;
			PSOPrecacheData.Priority = EPSOPrecachePriority::High;
		#if PSO_PRECACHING_VALIDATE
			PSOPrecacheData.PSOCollectorIndex = NiagaraEmitterPSOCollectorIndex;
			PSOPrecacheData.VertexFactoryType = nullptr;
		#endif // PSO_PRECACHING_VALIDATE
			PSOInitializers.Add(MoveTemp(PSOPrecacheData));
		}

		FPSOPrecacheRequestResultArray ResultArray = RequestPrecachePSOs(EPSOPrecacheType::Global, PSOInitializers);

		EPSOPrecacheProxyCreationStrategy ProxyCreationStrategy = GetPSOPrecacheProxyCreationStrategy();
		bool bAddToPSOReadyGraphTasks = GNiagaraEmitterComputePSOPrecacheMode == 1 && ProxyCreationStrategy != EPSOPrecacheProxyCreationStrategy::AlwaysCreate;
		if (bAddToPSOReadyGraphTasks)
		{			
			for (FPSOPrecacheRequestResult PSOPrecacheRequestResult : ResultArray)
			{
