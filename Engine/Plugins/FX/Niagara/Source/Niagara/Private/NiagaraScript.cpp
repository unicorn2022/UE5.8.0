...
static bool GNiagaraCompileGpuSupportsMultiPlatformCook = false;
static FAutoConsoleVariableRef CVarSystemGpuSupportsMultiPlatformCook(
	TEXT("fx.Niagara.SystemGpuSupportsMultiPlatformCook"),
	GNiagaraCompileGpuSupportsMultiPlatformCook,
	TEXT("If false GPU shaders for Niagara scripts will not be compiled by the system when doing a multiplatform cook. \n"),
	ECVF_Default
);
...
struct FNiagaraScriptVersionDataAccessor final : FNiagaraVersionDataAccessor
{
	...
	virtual TSubclassOf<UNiagaraVersionUpgradeBase>& GetUpdateUtilityClass() override { return ScriptData->UpdateUtilityClass; }
...
void UNiagaraScript::ComputeVMCompilationId(FNiagaraVMExecutableDataId& Id, const FGuid& VersionGuid, FNiagaraScriptHashCollector* OutHashCollector) const
{
	...
	if (EmitterData != nullptr)
	{
		...
		// Warn against drift between script and emitter interpolation mode
		if (bUsesInterpolatedSpawning && Usage == ENiagaraScriptUsage::ParticleSpawnScript)
		{
			UE_LOGF(LogNiagara, Warning, "Emitter InterpolatedSpawnMode is on but spawn script Usage is ParticleSpawnScript (expected ParticleSpawnScriptInterpolated). Compile/runtime constant-buffer drift may occur. Script: %ls", *GetFullName());
		}
	...
	
	{
		...
		{
			//UE_LOGF(LogNiagara, Display, "AreScriptAndSourceSynchronized %ls ======================== ", *GetFullName());
		...
		for (int32 i = 0; i < ReferencedStaticVars.Num(); i++)
		{
			...
			{
				UE_LOGF(LogNiagara, Display, "***** Referenced Static Vars %d %ls", (int32)Usage, *GetPathName());
			}
			...
			if (const uint8* VarData = ReferencedStaticVarValues[i])
			{
				...
				{
					UE_LOGF(LogNiagara, Display, "Param %ls %ls %d", *StaticVar.GetType().GetName(), *StaticVar.GetName().ToString(), (uint32)VarData[0]);
				}
	...
	if (GNiagaraDumpKeyGen == 1 && Id != LastGeneratedVMId)
	{
		...
		UE_LOGF(LogNiagara, Display, "KeyGen %ls\n==================\n", *GetPathName());
		for (int32 i = 0; i < OutputByLines.Num(); i++)
		{
			UE_LOGF(LogNiagara, Display, "/*%04d*/\t\t%ls", i + 1, *OutputByLines[i]);
		}
	}
...
void UNiagaraScript::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsCooking())
	{
		...
		if (!bUsesRapidIterationParams)
		{
			...
			UE_LOGF(LogNiagara, Verbose, "Pruned %d/%d parameters from script %ls", NumRemoved, ParameterVariables.Num(), *GetFullName());
		}
	...
	{		
		...
		VersionedScriptAdapter.PostLoadDefinitionsSubscriptions(
			...
			[](UObject* ObjectOwner) -> TArray<INiagaraParameterDefinitionsSubscriber*>
			{
				...
				if (UNiagaraScript* Owner = Cast<UNiagaraScript>(ObjectOwner))
				{
					OutArray.Reserve(Owner->VersionedScriptAdapters.Num());
					for (FVersionedNiagaraScript& Adapter : Owner->VersionedScriptAdapters)
					{
						OutArray.Add(&Adapter);
					}
				}
				return OutArray;
			}
		...
		if (NiagaraVer < FNiagaraCustomVersion::UseHashesToIdentifyCompileStateOfTopLevelScripts && CachedScriptVMId.CompilerVersionID.IsValid())
		{
			...
			if (BaseId.IsValid() == false)
			{
				UE_LOGF(LogNiagara, Warning,
					"Invalidating compile ids for script %ls because it doesn't have a valid base id.  The owning asset will continue to compile on load until it is resaved.",
					*GetPathName());
...
#if WITH_EDITORONLY_DATA
void UNiagaraScript::DeclareConstructClasses(TArray<FTopLevelAssetPath>& OutConstructClasses, const UClass* SpecificSubclass)
{
	Super::DeclareConstructClasses(OutConstructClasses, SpecificSubclass);
	OutConstructClasses.Add(FTopLevelAssetPath(TEXT("/Script/NiagaraEditor.NiagaraHierarchyScriptParameter"))); // cannot use NiagaraHierarchyScriptParameter::StaticClass() here as this type is in an editor module
}
#endif
		...
		if (!bSynchronized && NewId.IsValid() && CachedScriptVMId.IsValid() && CachedScriptVM.IsValid())
		{
			if (NewId != LastReportedVMId)
			{
				if (GEnableVerboseNiagaraChangeIdLogging)
				{
					...
					{
						UE_LOGF(LogNiagara, Log, "AreScriptAndSourceSynchronized base script compile hashes don't match. %ls != %ls, script %ls",
					...
					{
						UE_LOGF(LogNiagara, Log, "AreScriptAndSourceSynchronized num referenced compile hashes don't match. %d != %d, script %ls",
					...
					else
					{
						for (int32 i = 0; i < NewId.ReferencedCompileHashes.Num(); i++)
						{
							...
							{
								UE_LOGF(LogNiagara, Log, "AreScriptAndSourceSynchronized referenced compile hash %d doesn't match. %ls != %ls, script %ls, source %ls",
	...
	while (Iter)
	{
...

		{
			if (nullptr != RapidIterationParameters.FindParameterOffset(RISrcVarA))
			{
				...
				UE_LOGF(LogNiagara, Log, "Converted RI variable \"%ls\" to \"%ls\" in Script \"%ls\"", *RISrcVarA.GetName().ToString(), *RIDestVarA.GetName().ToString(), *GetFullName());
			...
			else if (nullptr != RapidIterationParameters.FindParameterOffset(RISrcVarB))
			{
				...
				UE_LOGF(LogNiagara, Log, "Converted RI variable \"%ls\" to \"%ls\" in Script \"%ls\"", *RISrcVarB.GetName().ToString(), *RIDestVarB.GetName().ToString(), *GetFullName());
				bConvertedAnything = true;
			}
		}

		{
			...
			if (VarIdx != INDEX_NONE)
			{
				...
				UE_LOGF(LogNiagara, Log, "Converted exec param variable \"%ls\" to \"%ls\" in Script \"%ls\"", *RISrcVarA.GetName().ToString(), *RIDestVarA.GetName().ToString(), *GetFullName());
				bConvertedAnything = true;
			}
			...
			if (VarIdx != INDEX_NONE)
			{
				...
				UE_LOGF(LogNiagara, Log, "Converted exec param  variable \"%ls\" to \"%ls\" in Script \"%ls\"", *RISrcVarB.GetName().ToString(), *RIDestVarB.GetName().ToString(), *GetFullName());
				bConvertedAnything = true;
			}
		}
...
bool UNiagaraScript::BinaryToExecData(const UNiagaraScript* Script, const TArray<uint8>& InBinaryData, FNiagaraVMExecutableData& OutExecData)
{
	...
	{
		UE_LOGF(LogNiagara, Display, "Failed to validate FNiagaraVMExecutableData received from DDC, rejecting!  Reasons:\nDeprecated object version");
		return false;
	}
...
bool UNiagaraScript::ExecToBinaryData(const UNiagaraScript* Script, TArray<uint8>& OutBinaryData, FNiagaraVMExecutableData& InExecData)
{
	...
	{
		UE_LOGF(LogNiagara, Error, "Failed to validate FNiagaraVMExecutableData being pushed to DDC, rejecting!  Errors:\n%ls", *ValidationErrors);
		return false;
	}
...
void WriteTextFileToDisk(FString SaveDirectory, FString FileName, FString TextToSave, bool bAllowOverwriting)
{
	...
	if (PlatformFile.CreateDirectoryTree(*SaveDirectory))
	{
		...
		if (bAllowOverwriting || !PlatformFile.FileExists(*AbsoluteFilePath))
		{
			...
			{
				UE_LOGF(LogNiagara, Log, "Wrote file to %ls", *AbsoluteFilePath);
				return;
			}
...
void DumpNameMap(const TMap<FName, UNiagaraDataInterface*>& ObjectNameMap)
{
	...
	{
		UE_LOGF(LogNiagara, Log, "%ls -> %ls", *Pair.Key.ToString(), *GetPathNameSafe(Pair.Value));
	}
}
...
void UNiagaraScript::SetVMCompilationResults(const FNiagaraVMExecutableDataId& InCompileId, FNiagaraVMExecutableData& InScriptVM, const FString& EmitterUniqueName, const TMap<FName, UNiagaraDataInterface*>& ObjectNameMap, bool bApplyRapidIterationParameters)
{
	...
	for (const FNiagaraScriptDataInterfaceCompileInfo& Info : CachedScriptVM.DataInterfaceInfo)
	{
		...
		if (CachedDefaultDataInterfaces[Idx].DataInterface == nullptr)
		{
			if ( Info.Type.IsValid() )
			{
				...
				{
					UE_LOGF(LogNiagara, Display, "We somehow ended up with a data interface that we couldn't match post compile. This shouldn't happen. Creating a dummy to prevent crashes. DataInterfaceInfoName:%ls Object:%ls", *Info.Name.ToString(), *GetPathNameSafe(this));
					UE_LOGF(LogNiagara, Log, "Object to Name map contents:");
			...
			{
				UE_LOGF(LogNiagara, Log, "Data Interface has no class, was it deleted? DataInterfaceInfoName:%ls Object:%ls", *Info.Name.ToString(), *GetPathNameSafe(this));
			}
		}
	}
	...
	if (!AreGpuScriptsCompiledBySystem())
	{
		if (Usage == ENiagaraScriptUsage::ParticleGPUComputeScript)
		{
			...
			{
				UE_LOGF(LogNiagara, Warning,
					"Could not cache resource shaders for rendering for script %ls because it had an invalid cached script id. This should be fixed by force recompiling the owning asset using the 'Full Rebuild' option and then saving the asset.",
					*GetPathName());
			}
		}
	}
...
void UNiagaraScript::ProcessCompilationErrors(bool bCompileSucceeded, TConstArrayView<FShaderCompilerError> CompilationErrors, TArray<FString>& ProcessedErrors)
{
	...
	{
		UE_LOGF(LogNiagara, Display, "There were issues for compilation of script \"%ls\"", *GetFriendlyName());
		...
		if (!LogNiagara.IsSuppressed(ELogVerbosity::Verbose))
		{
			...
			UE_LOGF(LogNiagara, Verbose, "Compile output as text:");
			UE_LOGF(LogNiagara, Verbose, "===================================================================================");
			for (int32 i = 0; i < OutputByLines.Num(); i++)
			{
				UE_LOGF(LogNiagara, Verbose, "/*%04d*/\t\t%ls", i + 1, *OutputByLines[i]);
			}
			UE_LOGF(LogNiagara, Verbose, "===================================================================================");
		}
		...
		{
			UE_LOGF(LogNiagara, Warning, "%ls", *Error.GetErrorString())
		}
	}
}
...
void UNiagaraScript::RequestCompile(const FGuid& ScriptVersion, bool bForceCompile)
{
	...
	if (ScriptData && (!AreScriptAndSourceSynchronized(ScriptVersion) || bForceCompile))
	{
		...
		if (RequestData.IsValid() == false)
		{
			...
			UE_LOGF(LogNiagara, Error, "Failed to precompile %ls.  This is due to unexpected invalid or broken data.  Additional details should be in the log.", *GetPathName());
			return;
		}
		...
		{
			COOK_STAT(Timer.TrackCyclesOnly());
			UE_LOGF(LogNiagara, Error, "Failed to precompile duplicate %ls.  This is due to unexpected invalid or broken data.  Additional details should be in the log.", *GetPathName());
			return;
		}
	...
	{
		UE_LOGF(LogNiagara, Verbose, "Script '%ls' is in-sync skipping compile..", *GetFullName());
	}
}
...
bool UNiagaraScript::RequestExternallyManagedAsyncCompile(const TSharedPtr<FNiagaraCompileRequestDataBase, ESPMode::ThreadSafe>& RequestData, const TSharedPtr<FNiagaraCompileRequestDuplicateDataBase, ESPMode::ThreadSafe>& RequestDuplicateData, FNiagaraVMExecutableDataId& OutCompileId, uint32& OutAsyncHandle)
{
	...
	if (ensure(ScriptData) && !AreScriptAndSourceSynchronized(LastGeneratedVMId.ScriptVersionID))
	{
		if (IsCompilable() == false)
		{
			...
			CachedScriptVM.LastCompileStatus = ENiagaraScriptCompileStatus::NCS_Unknown;
			CachedScriptVMId = LastGeneratedVMId;
			UE_LOGF(LogNiagara, Warning, "Requested compile of script '%ls', which cannot be compiled!", *GetFullName());
			return false;
		}
		...
		UE_LOGF(LogNiagara, Verbose, "Script '%ls' is requesting compile..", *GetFullName());
		return true;
	}
	else
	{
...
{
	// nothing to be done for cooking non-gpu compute scripts; their work is handled by the owning NiagaraSystem
	if (Usage != ENiagaraScriptUsage::ParticleGPUComputeScript)
	{
		return;
	}
	...
	if (!ShaderFormatsForCooking.IsEmpty())
	{
		...
		{
			UE_LOGF(LogNiagara, Warning,
				"Could not cache cooked shader for script %ls because it had an invalid cached script id.  This should be fixed by running the console command fx.PreventSystemRecompile with the owning system asset path as the argument and then resaving the assets.",
				*GetPathName());
			return;
		}
...
{
	if (!GNiagaraGpuScriptsCompiledBySystem)
	{
		return false;
	}
	...
	if (!GNiagaraCompileGpuSupportsMultiPlatformCook && IsRunningCookCommandlet())
	{
		if (GetTargetPlatformManagerRef().GetActiveTargetPlatforms().Num() > 1)
		{
			return false;
		}
	}
...
void UNiagaraScript::CacheShadersForResources(FNiagaraShaderScript* ResourceToCache, bool bApplyCompletedShaderMapForRendering, bool bForceRecompile, bool bCooking, const ITargetPlatform* TargetPlatform)
{
	...
	if (CanBeRunOnGpu())
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogNiagara, Warning, "Failed to compile Niagara shader %ls for platform %ls.",
				*GetPathName(),
...
void UNiagaraScript::SyncAliases(const FNiagaraAliasContext& ResolveAliasesContext)
{
	...
	for (int32 i = 0; i < GetVMExecutableData().Parameters.Parameters.Num(); i++)
	{
		if (GetVMExecutableData().Parameters.Parameters[i].IsValid() == false)
		{
			...
			UE_LOGF(LogNiagara, Error, "Invalid parameter found while syncing script aliases.  Script: %ls Parameter Name: %ls Parameter Type: %ls",
...
{
	UE_LOGF(LogNiagara, Verbose, "InvalidateCompileResults Script:%ls Reason:%ls", *GetPathName(), *Reason);
	CachedScriptVM.Reset();
	ScriptResource->Invalidate();
...
bool UNiagaraScript::CanBeRunOnGpu()const
{
	...
	for (const FNiagaraScriptDataInterfaceCompileInfo& InterfaceInfo : CachedScriptVM.DataInterfaceInfo)
	{
		const UNiagaraDataInterface* DefaultDataInterface = InterfaceInfo.GetDefaultDataInterface();
		if (DefaultDataInterface == nullptr || DefaultDataInterface->CanExecuteOnTarget(ENiagaraSimTarget::GPUComputeSim) == false)
		{
			return false;
		}
	}
