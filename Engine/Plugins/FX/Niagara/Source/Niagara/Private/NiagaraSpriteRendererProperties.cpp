...
void UNiagaraSpriteRendererProperties::InitCDOPropertiesAfterModuleStartup()
{
	...
	{
		if (UNiagaraSpriteRendererProperties* RendererProperties = WeakSpriteRendererProperties.Get())
		{
...
TArray<FNiagaraVariable> UNiagaraSpriteRendererProperties::GetBoundAttributes() const
{
	...
	BoundAttributes.Reserve(BoundAttributes.Num() + MaterialParameters.AttributeBindings.Num() + 2);
	...
	if (MaterialUserParamBinding.Parameter.IsValid())
	{
		BoundAttributes.AddUnique(MaterialUserParamBinding.Parameter);
	}
...
void UNiagaraSpriteRendererProperties::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (bUseMaterialCutoutTexture || CutoutTexture || DerivedData.BoundingGeometry.Num())
	{
		const bool bUpdateCutoutDDC =
			PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, bUseMaterialCutoutTexture) ||
			PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, bCutoutIgnoreSubUV) ||
			PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, CutoutTexture) ||
			PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, BoundingMode) ||
			PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, OpacitySourceMode) ||
			PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, AlphaThreshold) ||
			MemberPropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, SubImageSize) ||
			(bUseMaterialCutoutTexture && PropertyName == GET_MEMBER_NAME_CHECKED(UNiagaraSpriteRendererProperties, Material));
...
void UNiagaraSpriteRendererProperties::CacheDerivedData()
{
	if (CutoutTexture)
	{
		const FIntVector2 DerivedSubImageSize = bCutoutIgnoreSubUV ? FIntVector2(1, 1) : FIntVector2((int32)SubImageSize.X, (int32)SubImageSize.Y);

		const FString KeyString = FSubUVDerivedData::GetDDCKeyString(CutoutTexture->Source.GetId(), DerivedSubImageSize.X, DerivedSubImageSize.Y, (int32)BoundingMode, AlphaThreshold, (int32)OpacitySourceMode);
		TArray<uint8> Data;
		...
		{
			DerivedData.Build(CutoutTexture, DerivedSubImageSize.X, DerivedSubImageSize.Y, BoundingMode, AlphaThreshold, OpacitySourceMode);
		...
		if (bCutoutIgnoreSubUV)
		{
			const int32 NumFrames = (int32)SubImageSize.X * (int32)SubImageSize.Y;
			const int32 NumVerts = DerivedData.BoundingGeometry.Num();

			DerivedData.BoundingGeometry.SetNumUninitialized(NumVerts * NumFrames);
			for ( int32 i=1; i < NumFrames; ++i )
			{
				FMemory::Memcpy(&DerivedData.BoundingGeometry[NumVerts * i], &DerivedData.BoundingGeometry[0], sizeof(FVector2f) * NumVerts);
			}
		}
	}
	else
