...
void FNiagaraSystemUpdateContext::CommitUpdate()
{
	...
	{
		if (NiagaraSystem && NiagaraSystem->IsReadyToRun())
		{
...
FNiagaraScriptDataKey::FNiagaraScriptDataKey(const FNiagaraEmitterHandle* EmitterHandle, const UNiagaraScript& Script)
	: EmitterHandleId(EmitterHandle != nullptr ? EmitterHandle->GetId() : FGuid())
	, ScriptUsage(Script.GetUsage())
	, ScriptUsageId(Script.GetUsageId())
{
}
...
{
	UE_LOGF(LogNiagara, Log, "PostLoad for FNiagaraVariableAttributeBinding....");
	UE_LOGF(LogNiagara, Log, "ParamMapVariable: %ls %ls", *ParamMapVariable.GetName().ToString(), *ParamMapVariable.GetType().GetName());
	UE_LOGF(LogNiagara, Log, "DataSetName: %ls", *DataSetName.ToString());
	UE_LOGF(LogNiagara, Log, "RootVariable: %ls", *RootName.ToString());
#if WITH_EDITORONLY_DATA
	UE_LOGF(LogNiagara, Log, "BoundVariable: %ls %ls", *BoundVariable.GetName().ToString(), *BoundVariable.GetType().GetName());
	UE_LOGF(LogNiagara, Log, "CachedDisplayName: %ls", *CachedDisplayName.ToString());
#endif
	UE_LOGF(LogNiagara, Log, "BindingSourceMode: %d     bBindingExistsOnSource: %d     bIsCachedParticleValue: %d", (int32)BindingSourceMode.GetValue(),
...
bool FNiagaraScriptResolvedDataInterfaceInfo::NeedsPerInstanceBinding() const
{
	...
	return NameBuilder.ToView().StartsWith(PARAM_MAP_USER_STR) 
		|| (ResolvedDataInterface != nullptr && ResolvedDataInterface->PerInstanceDataSize() > 0);
}
...
bool FNiagaraScriptDataInterfaceCompileInfo::CanExecuteOnTarget(ENiagaraSimTarget SimTarget) const
{
	...
	UE_LOGF(LogNiagara, Error, "Failed to call CanExecuteOnTarget for DataInterface \"%ls\". Perhaps missing a plugin for your project?", *Name.ToString());
	return false;
}
...
UNiagaraDataInterface* FNiagaraScriptDataInterfaceCompileInfo::GetDefaultDataInterface() const
{
	...
	if (Type.IsDataInterface())
	{
		...
		if (TargetClass)
		{
			...
			UE_LOGF(LogNiagara, Error, "Failed to create default object for class \"%ls\". Perhaps missing a plugin for your project?", *TargetClass->GetName());
			return nullptr;
		}

	}
	UE_LOGF(LogNiagara, Error, "Failed to create default object for compiled variable \"%ls\". Perhaps missing a plugin for your project?", *this->Name.ToString());
	return nullptr;
}
...
{
	UE_LOGF(LogNiagara, Display, "Compile output as text: %ls", *DebugName);
...
TOptional<EPixelFormat> FNiagaraUtilities::BufferFormatToPixelFormat(ENiagaraGpuBufferFormat NiagaraFormat, EPixelFormatCapabilities RequiredCapabilities, int NumberOfChannels, UObject* RequesterForDebugging)
{
	if (!ensure(NumberOfChannels >= 1 && NumberOfChannels <= 4))
	{
		UE_LOGF(LogNiagara, Error, "Invalid number of channels %d", NumberOfChannels);
	...
	UE_LOGF(LogNiagara, Error, "Could not find a valid EPixelFormat for NiagaraFormat(%ls) Channels(%d) Caps(%08x) for (%ls)", *StaticEnum<ENiagaraGpuBufferFormat>()->GetNameStringByValue(int64(NiagaraFormat)), NumberOfChannels, int(RequiredCapabilities), *GetFullNameSafe(RequesterForDebugging));
	return TOptional<EPixelFormat>();
}

TOptional<ETextureRenderTargetFormat> FNiagaraUtilities::BufferFormatToRenderTargetFormat(ENiagaraGpuBufferFormat NiagaraFormat, EPixelFormatCapabilities RequiredCapabilities, UObject* RequesterForDebugging)
{
	switch (NiagaraFormat)
	{
		...
		default:
			UE_LOGF(LogNiagara, Error, "Could not find a valid ETextureRenderTargetFormat for NiagaraFormat(%ls) Caps(%08x) for (%ls)", *StaticEnum<ENiagaraGpuBufferFormat>()->GetNameStringByValue(int64(NiagaraFormat)), int(RequiredCapabilities), *GetFullNameSafe(RequesterForDebugging));
...
{
	return BufferFormatToPixelFormat(NiagaraFormat, RequiredCapabilities, NumberOfChannels, nullptr);
}
...
{
	return BufferFormatToRenderTargetFormat(NiagaraFormat, RequiredCapabilities, nullptr);
}
