...
namespace NSMLightAttributesPrivate
{
	...
	static void ParticleSimulate(const NiagaraStateless::FParticleSimulationContext& ParticleSimulationContext)
	{
		...
		if (const FNiagaraStatelessSystemScaleBuildData* SystemScaleBuildData = ParticleSimulationContext.ReadSharedBuiltData<FNiagaraStatelessSystemScaleBuildData>())
		{
			if (SystemScaleBuildData->bScaleInitialLightRadius)
			{
				const FVector3f LocalToWorldScale3D = ParticleSimulationContext.GetLocalToWorldScale();
				const float LocalToWorldScale = SystemScaleBuildData->bScaleInitialLightRadiusBySingleAxis ? LocalToWorldScale3D.Component(int32(SystemScaleBuildData->ScaleInitialLightRadiusScalingAxis)) : LocalToWorldScale3D.GetMax();
				RadiusScale = FMath::Lerp(1.0f, LocalToWorldScale, SystemScaleBuildData->ScaleInitialLightRadiusScaleAmount);
			}
		}
		...
		for (uint32 i=0; i < ParticleSimulationContext.GetNumInstances(); ++i)
		{
			for ( int a=0; a < int(EModuleAttribute::Num); ++a )
			{
				...
				{
					const float Scale = a == int(EModuleAttribute::Radius) ? RadiusScale : 1.0f;
					...
					ParticleSimulationContext.WriteParticleVariable(ModuleBuiltData->AttributeOffset[a], i, Value * Scale);
				}
			}
		}
	}
