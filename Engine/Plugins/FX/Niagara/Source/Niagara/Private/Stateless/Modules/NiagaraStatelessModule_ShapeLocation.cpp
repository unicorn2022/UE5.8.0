...
namespace NSMShapeLocationPrivate
{
	struct FModuleBuiltData
	{
		...
		FUintVector3					TransformOffset = FUintVector3::ZeroValue;
		FNiagaraStatelessRangeVector3	TransformOffsetScale;
	...
	static void ParticleSimulate(const NiagaraStateless::FParticleSimulationContext& ParticleSimulationContext)
	{
		...
		const float* ParticlesAge = ParticleSimulationContext.GetParticleNormalizedAge();
		const float* ParticlesPreviousAge = ParticleSimulationContext.GetParticlePreviousNormalizedAge();
		...
		for (uint32 i = 0; i < ParticleSimulationContext.GetNumInstances(); ++i)
		{
			...
			FVector3f PositionOffsets[2];
			ParticleSimulationContext.SampleDistributionValues(ShaderParameters->ShapeLocation_TransformOffset, i, 1, ParticlesAge[i], ParticlesPreviousAge[i], PositionOffsets[0], PositionOffsets[1]);

			ParticleSimulationContext.WriteParticleVariable(ModuleBuiltData->PositionVariableOffset, i, Position + (PositionOffsets[0] * ShaderParameters->ShapeLocation_TransformOffsetScale) + ShapeLocation);
			ParticleSimulationContext.WriteParticleVariable(ModuleBuiltData->PreviousPositionVariableOffset, i, PreviousPosition + (PositionOffsets[1] * ShaderParameters->ShapeLocation_TransformOffsetScale) + ShapeLocation);
		}
	}
}
...
void UNiagaraStatelessModule_ShapeLocation::BuildEmitterData(const FNiagaraStatelessEmitterDataBuildContext& BuildContext) const
{
	...
	BuiltData->TransformOffset		= BuildContext.AddDistribution(TransformOffset);
	BuiltData->TransformOffsetScale	= BuildContext.ConvertDistributionToRange(TransformOffsetScale, FVector3f::OneVector);
...
void UNiagaraStatelessModule_ShapeLocation::SetShaderParameters(const FNiagaraStatelessSetShaderParameterContext& SetShaderParameterContext) const
{
	...
	Parameters->ShapeLocation_TransformOffset		= ModuleBuiltData->TransformOffset;
	Parameters->ShapeLocation_TransformOffsetScale	= SetShaderParameterContext.ConvertRangeToValue(ModuleBuiltData->TransformOffsetScale);
