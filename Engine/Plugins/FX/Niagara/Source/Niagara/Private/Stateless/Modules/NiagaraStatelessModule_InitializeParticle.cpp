...
void UNiagaraStatelessModule_InitializeParticle::SetShaderParameters(const FNiagaraStatelessSetShaderParameterContext& SetShaderParameterContext) const
{
	...
	{
		if (SystemScaleBuildData->bScaleInitialSpriteSize)
		{
			const FVector3f TransformScale = SetShaderParameterContext.GetSpaceTransforms().GetLocalToWorld().GetScale3D();
			float Scale = 0.0f;
			if (SystemScaleBuildData->bScaleInitialSpriteSizeBySingleAxis)
			{
				Scale = TransformScale.Component(int32(SystemScaleBuildData->ScaleInitialSpriteSizeAxis));
			}
			else
			{
				Scale = TransformScale.GetMax();
			}
			Scale = FMath::Lerp(1.0f, Scale, SystemScaleBuildData->ScaleInitialSpriteSizeScaleAmount);

			Parameters->InitializeParticle_SpriteSizeScale *= Scale;
			Parameters->InitializeParticle_SpriteSizeBias *= Scale;
		}

		//if (SystemScaleBuildData->bScaleInitialMeshSize)
		//{
		//	const FVector3f TransformScale = SetShaderParameterContext.GetSpaceTransforms().GetLocalToWorld().GetScale3D();
		//	const FVector3f MeshSpaceScale = SystemScaleBuildData->bScaleInitialMeshSizeAbsoluteScale ? TransformScale.GetAbs() : TransformScale;
		//	const FVector3f FinalScale = FMath::Lerp(FVector3f::OneVector, MeshSpaceScale, SystemScaleBuildData->ScaleInitialMeshSizeScaleAmount);
		//	Parameters->InitializeParticle_MeshScaleScale *= FinalScale;
		//	Parameters->InitializeParticle_MeshScaleBias *= FinalScale;
		//}

		if (SystemScaleBuildData->bScaleInitialRibbonWidth)
		{
			const FVector3f TransformScale = SetShaderParameterContext.GetSpaceTransforms().GetLocalToWorld().GetScale3D();
			float Scale = 0.0f;
			if (SystemScaleBuildData->bScaleInitialRibbonWidthBySingleAxis)
			{
				Scale = TransformScale.Component(int32(SystemScaleBuildData->ScaleInitialRibbonWidthScalingAxis));
			}
			else
			{
				Scale = TransformScale.GetMax();
			}
			Scale = FMath::Lerp(1.0f, Scale, SystemScaleBuildData->ScaleInitialRibbonWidthScaleAmount);

			Parameters->InitializeParticle_RibbonWidthScale *= Scale;
			Parameters->InitializeParticle_RibbonWidthBias *= Scale;
		}
	}
}
