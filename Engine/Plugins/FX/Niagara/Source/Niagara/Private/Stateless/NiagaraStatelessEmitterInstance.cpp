...
namespace NiagaraStatelessEmitterInstancePrivate
{
	...
	{
		ShaderParameters->Common_LocalToWorldScale = Transforms.GetLocalToWorld().GetScale3D();
...
void FNiagaraStatelessEmitterInstance::BindParameters(bool bExternalOnly)
{
	if (!RendererBindings.IsEmpty())
	{
		if (ParentSystemInstance)
		{
			...
			ParameterCollectionResolver.BindCollections(MakeArrayView(EmitterData->BoundParameterCollections), RendererBindings);
		}
	}
}
...
void FNiagaraStatelessEmitterInstance::UnbindParameters(bool bExternalOnly)
{
	if (!RendererBindings.IsEmpty())
	{
		if (ParentSystemInstance)
		{
			...
			const FNiagaraParameterCollectionResolver ParameterCollectionResolver = ParentSystemInstance->CreateParameterCollectionResolver();
			ParameterCollectionResolver.UnbindCollections(MakeArrayView(EmitterData->BoundParameterCollections), RendererBindings);
		}
	}
}

void FNiagaraStatelessEmitterInstance::RebindParameterCollection(UNiagaraParameterCollectionInstance* OldCollection, UNiagaraParameterCollectionInstance* NewCollection)
{
	if (RendererBindings.IsEmpty() || !ParentSystemInstance)
	{
		return;
	}

	if (!EmitterData->BoundParameterCollections.Contains(OldCollection->GetParent()))
	{
		return;
	}

	OldCollection->GetParameterStore().Unbind(&RendererBindings);
	NewCollection->GetParameterStore().Bind(&RendererBindings);
}
...
{
	const bool bForceParametersUpdate = bNeedsEmitterStateInit;
	...
	TickEmitterState();
	CalculateBounds();
	UpdateSimulationData(DeltaSeconds, bForceParametersUpdate);
}
...
void FNiagaraStatelessEmitterInstance::InitEmitterState()
{
	...
	{
		// Infinite will loop after this time which works as expected, however we do start to run into numerical precision issues after running for around 1.5 days
		// We also can not use TNumericLimits<float>::Max() / FLT_MAX as it will introduce numerical precision issues resulting in nothing appearing to spawn for rate based spawning
		constexpr float LongDuration = 60.0f * 60.0f * 24.0f;
		CurrentLoopDuration	= LongDuration;
		CurrentLoopAgeEnd	= LongDuration;
	}
	else
	{
...
void FNiagaraStatelessEmitterInstance::UpdateSimulationData(float DeltaSeconds, bool bForceParametersUpdate)
{
	using namespace NiagaraStateless;

	// Update parameter data (if needed)
	bool bNeedsShaderParametersUpdate = false;
	bool bNeedsParametersUpdate = bForceParametersUpdate;
...
void FNiagaraStatelessEmitterInstance::SetExecutionStateInternal(ENiagaraExecutionState RequestedExecutionState)
{
	...
	switch (RequestedExecutionState)
	{
		case ENiagaraExecutionState::Active:
			UE_LOGF(LogNiagara, Error, "Lightweight Emitter: Was requested to go Active and we do not supoprt that.");
			break;
