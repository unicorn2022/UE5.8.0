...
namespace NSMScaleSpriteSizeBySpeedPrivate
{
	...
	static void ParticleSimulate(const NiagaraStateless::FParticleSimulationContext& ParticleSimulationContext)
	{
		...
		for (uint32 i = 0; i < ParticleSimulationContext.GetNumInstances(); ++i)
		{
			...
			FVector3f Velocity;
			if (ModuleBuiltData->PreviousPositionVariableOffset != INDEX_NONE)
			{
				const FVector3f PreviousPosition = ParticleSimulationContext.ReadParticleVariable(ModuleBuiltData->PreviousPositionVariableOffset, i, Position);
				Velocity = (Position - PreviousPosition) * ParticleSimulationContext.GetInvDeltaTime();
			}
			else
			{
				Velocity = ParticleSimulationContext.ReadParticleVariable(ModuleBuiltData->VelocityVariableOffset, i, FVector3f::ZeroVector);
			}
...
void UNiagaraStatelessModule_ScaleSpriteSizeBySpeed::BuildEmitterData(const FNiagaraStatelessEmitterDataBuildContext& BuildContext) const
{
	...
	if (BuiltData->PreviousPositionVariableOffset != INDEX_NONE || BuiltData->VelocityVariableOffset != INDEX_NONE)
	{
