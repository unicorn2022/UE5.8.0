...
namespace NSMDecalAttributesPrivate
{
	...
	static void ParticleSimulate(const NiagaraStateless::FParticleSimulationContext& ParticleSimulationContext)
	{
		...
		FVector3f SizeScale = FVector3f::OneVector;
		if (ModuleBuiltData->AttributeOffset[int(EModuleAttribute::Size)] != INDEX_NONE)
		{
			const FNiagaraStatelessSystemScaleBuildData* SystemScaleBuildData = ParticleSimulationContext.ReadSharedBuiltData<FNiagaraStatelessSystemScaleBuildData>();
			if (SystemScaleBuildData && SystemScaleBuildData->bScaleInitialDecalSize)
			{
				const FVector3f LocalToWorldScale = ParticleSimulationContext.GetLocalToWorldScale();
				SizeScale = FMath::Lerp(FVector3f::OneVector, LocalToWorldScale, SystemScaleBuildData->ScaleInitialDecalSizeScaleAmount);
			}
		}
		...
		for (uint32 i = 0; i < ParticleSimulationContext.GetNumInstances(); ++i)
		{
			...
			if (ModuleBuiltData->AttributeOffset[int(EModuleAttribute::Size)] != INDEX_NONE)
			{
				...
				ParticleSimulationContext.WriteParticleVariable(ModuleBuiltData->AttributeOffset[int(EModuleAttribute::Size)], i, Value * SizeScale);
			}
