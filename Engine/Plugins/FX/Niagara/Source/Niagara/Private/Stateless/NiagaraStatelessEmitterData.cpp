...
uint32 FNiagaraStatelessEmitterData::CalculateActiveParticles(int32 InRandomSeed, TConstArrayView<FNiagaraStatelessRuntimeSpawnInfo> RuntimeSpawnInfos, TOptional<float> Age, NiagaraStateless::FSpawnInfoShaderParameters* SpawnParameters) const
{
	...
	if (AmountOverLimit > 0)
	{
		GEngine->AddOnScreenDebugMessage(
			...
			FColor::White,
			*FString::Printf(
				TEXT("Stateless Simulation (%s:%s) has overflowed spawn infos (%d of %d), this can result in flickering / missing particles.  Reduce the number of spawn infos / loop time / particle age / increase code limit to fix."),
				*DebugSimulationName.ToString(), *DebugEmitterName.ToString(),
				NiagaraStateless::MaxGpuSpawnInfos + AmountOverLimit,
				NiagaraStateless::MaxGpuSpawnInfos
			)
		);
	}
#endif
