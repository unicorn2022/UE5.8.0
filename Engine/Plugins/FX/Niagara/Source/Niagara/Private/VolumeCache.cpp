...
#else
			UE_LOGF(LogVolumeCache, Warning, "OpenVDB not supported on the platform");
#endif

			break;
		}
		default:
			UE_LOGF(LogVolumeCache, Warning, "Invalid Volume Cache type");
	}
...
bool FOpenVDBCacheData::LoadFile(FString Path, int frame)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogVolumeCache, Warning, "Cache File not Found: %ls", *FullPath);
			return false;
		}
		...
		try
		{
			...
			{
				UE_LOGF(LogVolumeCache, Warning, "Grids are not the same size: %ls", *FullPath);
				return false;
			}
			...
			if (UseFloatGrids)
			{
				UE_LOGF(LogVolumeCache, Warning, "Cache has float grids, which are not used for cache reading: %ls", *FullPath);
				return false;
			}
			else
			{
				...
				{
					UE_LOGF(LogVolumeCache, Warning, "Cache has invalid grids: %ls", *FullPath);
					return false;
				}
		...
		{
			UE_LOGF(LogVolumeCache, Warning, "No cache metadata found: %ls", *FullPath);
			return false;
		}
...
bool FOpenVDBCacheData::Fill3DTexture_RenderThread(int frame, FTextureRHIRef TextureToFill, FRHICommandListImmediate& RHICmdList)
{
	if (OpenVDBGrids.Contains(frame) && OpenVDBGrids[frame] != nullptr)
	{
		...
		{
			UE_LOGF(LogVolumeCache, Warning, "Target texture resolution %ls doesn't match OpenVDB grid resolution %ls.  Regenerate your cache", *TextureToFill->GetSizeXYZ().ToString(), *DenseResolution.ToString());
		}
		else
		{
