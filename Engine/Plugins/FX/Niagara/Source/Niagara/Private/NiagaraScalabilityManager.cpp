	...
	if (GScalabilityMaxUpdatesPerFrame > 0 && EffectType->UpdateFrequency != ENiagaraScalabilityUpdateFrequency::Continuous)
	{
		...
		if (UpdateCount > GScalabilityMaxUpdatesPerFrame)
		{
			...
			if (FNiagaraUtilities::LogVerboseWarnings())
			{
				...
				if (!AlreadyAdded)
				{
					UE_LOGF(LogNiagara, Verbose, "NiagaraScalabilityManager needs to process %d updates (will be clamped to %d) for EffectType - %ls - (%d items, %f period (s), %f delta (s)",
						UpdateCount,
...
void FNiagaraScalabilityManager::PreGarbageCollectBeginDestroy()
{
	...
	while (--CompIdx >= 0)
	{
		...
		if (Comp == nullptr)
		{
			//UE_LOGF(LogNiagara, Warning, "Unregister from PreGCBeginDestroy @%d/%d - %ls", CompIdx, ManagedComponents.Num(), *EffectType->GetName());
...
void FNiagaraScalabilityManager::Register(UNiagaraComponent* Component)
{
	...
	//UE_LOGF(LogNiagara, Warning, "Registered Component %p at index %d", Component, Component->ScalabilityManagerHandle);
}
...
{
	//UE_LOGF(LogNiagara, Warning, "Unregistering Component %p at index %d (Replaced with %p)", ManagedComponents[IndexToRemove], IndexToRemove, ManagedComponents.Num() > 1 ? ManagedComponents.Last() : nullptr);
	...
	{
		UE_LOGF(LogNiagara, Warning, "Attempting to unregister an invalid index from the Scalability Manager. Index: %d - Num: %d", IndexToRemove, ManagedComponents.Num());
	}
...
bool FNiagaraScalabilityManager::EvaluateCullState(FComponentIterationContext& Context, int32 ComponentIndex, int32& UpdateCounter)
{
	...
	if (UpdateScalability)
	{
		...
		if (System == nullptr)
		{
			UE_LOGF(LogNiagara, Warning, "Niagara System has been destroyed with components still registered to the scalability manager. Unregistering this component.\nComponent: 0x%p - %ls\nEffectType: 0x%p - %ls",
	...
	if (UNiagaraComponent* Component = ManagedComponents[ComponentIndex])
	{
		...
		else
		{
			...
			if(bAllow)
			{
				...
				{
					UE_LOGF(LogNiagara, Error, "Niagara Component is incorrectly still registered with the scalability manager. %d - %ls ", (int32)CullReaction, *Component->GetAsset()->GetFullName());
				}
...
void FNiagaraScalabilityManager::Dump()
{
	...
	UE_LOGF(LogNiagara, Display, "Effect Type: %ls", *EffectType->GetPathName());
	UE_LOGF(LogNiagara, Display, "-------------------------------------------------------------------------------");
	UE_LOGF(LogNiagara, Display, "| Summary for managed systems of this effect type. Does NOT inclued all possible Niagara FX in scene. |");
	UE_LOGF(LogNiagara, Display, "| Num Managed Components: %d |", ManagedComponents.Num());
	UE_LOGF(LogNiagara, Display, "| Num Active: %d |", ManagedComponents.Num() - Summary.NumCulled);
	UE_LOGF(LogNiagara, Display, "| Num Culled: %d |", Summary.NumCulled);
	UE_LOGF(LogNiagara, Display, "| Num Culled By Distance: %d |", Summary.NumCulledByDistance);
	UE_LOGF(LogNiagara, Display, "| Num Culled By Instance Count: %d |", Summary.NumCulledByInstanceCount);
	UE_LOGF(LogNiagara, Display, "| Num Culled By Visibility: %d |", Summary.NumCulledByVisibility);
	UE_LOGF(LogNiagara, Display, "-------------------------------------------------------------------------------");
	UE_LOGF(LogNiagara, Display, "| Details |");
	UE_LOGF(LogNiagara, Display, "-------------------------------------------------------------------------------\n%ls", *DetailString);
}
