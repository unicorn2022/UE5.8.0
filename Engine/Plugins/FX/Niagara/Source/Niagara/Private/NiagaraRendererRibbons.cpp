...
namespace NiagaraRendererRibbonsPrivate
{
	// max absolute error 9.0x10^-3
	// Eberly's polynomial degree 1 - respect bounds
	// input [-1, 1] and output [0, PI]
	static float AcosFast(float InX)
	{
	...
	static uint32 CalculateBitsForRange(uint32 Range)
	{
	...
	static uint32 CalculateBitMask(uint32 NumBits)
	{
		return static_cast<uint32>(static_cast<uint64>(0xFFFFFFFF) >> (32 - NumBits));
	}

	static ENiagaraGpuTessellationMode::Type GetGpuTessellationMode(const FNiagaraRibbonGenerationConfig& GenerationConfig)
	{
		if (GenerationConfig.WantsAutomaticTessellation())
		{
			return ENiagaraGpuTessellationMode::Automatic;
		}
		else if (GenerationConfig.WantsConstantTessellation())
		{
			return ENiagaraGpuTessellationMode::Constant;
		}
		return ENiagaraGpuTessellationMode::Disabled;
	}
} //namespace NiagaraRendererRibbonsPrivate
...
struct FNiagaraRibbonIndexBuffer final : FIndexBuffer
{
	...
	void Initialize(FRHICommandListBase& RHICmdList, const uint32 NumElements)
	{
		...
		const FRHIBufferCreateDesc CreateDesc =
			...
			.SetInitialState(ERHIAccess::SRVMask | ERHIAccess::VertexOrIndexBuffer);
...
{	
	...
	if (bIsUsingGPUInit)
	{
		...
		bJustCreatedCommandBuffer |= GPUComputeCommandBuffer.Allocate(RHICmdList, VERTEX_GEN_OUTPUT_DATA_AUTO_TESSELLATION_NUM_ELEMENTS, VERTEX_GEN_OUTPUT_DATA_AUTO_TESSELLATION_NUM_ELEMENTS, InitialBufferAccessFlags | ERHIAccess::IndirectArgs, false, EBufferUsageFlags::DrawIndirect);
	}
	else
	{		
...
struct FNiagaraRibbonGPUInitComputeBuffers
{
	...
	{
		using namespace NiagaraRendererRibbonsPrivate;
		...
		const uint32 TessellationBufferSize = NeededSize * (bWantsTessellation ? (bWantsTessellationTwist ? NIAGARA_RIBBON_TESSELLATION_FULL_STATS_NUM_ELEMENTS : NIAGARA_RIBBON_TESSELLATION_NO_TWIST_STATS_NUM_ELEMENTS) : 0);
...
void FNiagaraRendererRibbons::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector, const FNiagaraSceneProxy *SceneProxy) const
{
	...
	// Material should never be nullptr, it will point to the Default Material if it was null from the game thread
	check(DynamicData->Material);
	const FMaterial& Material = DynamicData->Material->GetIncompleteMaterialWithFallback(FeatureLevel);

	const bool bTranslucentMaterial = IsTranslucentBlendMode(Material);
	const bool bCanCastTranslucentShadows = SceneProxy->CastsVolumetricTranslucentShadow() || Material.GetCastDynamicShadowAsMasked();
	if (bTranslucentMaterial && AreViewsRenderingOpaqueOnly(Views, VisibilityMap, bCanCastTranslucentShadows))
	{
		return;
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			if (bTranslucentMaterial && IsViewRenderingOpaqueOnly(View, bCanCastTranslucentShadows))
			{
				continue;
			}
...
FNiagaraDynamicDataBase* FNiagaraRendererRibbons::GenerateDynamicData(const FNiagaraSceneProxy* Proxy, const UNiagaraRendererProperties* InProperties, const FNiagaraEmitterInstance* Emitter)const
{
	...
	if (Properties)
	{
		...
		{		
			...
			{
				DynamicData->MaxAllocatedCountEstimate = FMath::Min<uint32>(EmitterData->GetMaxParticleCountEstimate(), DynamicData->MaxAllocationCount);
			}
...
void FNiagaraRendererRibbons::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector, const FNiagaraSceneProxy* SceneProxy)
{
	...
	// Pick the visible view that demands the highest tessellation since the BLAS is shared by all views.
	// Only meaningful on the CPU-init path; the GPU-init path uses a view-independent fixed tessellation factor (see CalculateIndexBufferConfiguration).
	int32 BLASViewIndex = FMath::CountTrailingZeros(VisibilityMap);
	if (!bDataGeneratedOnGpu && Views.Num() > 1)
	{
		int32 HighestTessellation = 1;
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
			...
			const FSceneView* CandidateView = Views[ViewIndex];
			const int32 Tess = CalculateTessellationFactor(SceneProxy, CandidateView, CandidateView->ViewMatrices.GetViewOrigin());
			if (Tess > HighestTessellation)
			{
				HighestTessellation = Tess;
				BLASViewIndex = ViewIndex;
			}
		}
	}

	checkf(Views.IsValidIndex(BLASViewIndex), TEXT("There should be at least one active view when calling GetDynamicRayTracingInstances(...)."));
	const FSceneView* BLASView = Views[BLASViewIndex];
	const FVector BLASViewOrigin = BLASView->ViewMatrices.GetViewOrigin();
	...
	FNiagaraRibbonRenderingFrameViewResources* RenderingViewResources = RenderingResources.RibbonResources->ViewResources.Add_GetRef(MakeShared<FNiagaraRibbonRenderingFrameViewResources>()).Get();
	RenderingViewResources->IndexGenerationSettings = CalculateIndexBufferConfiguration(DynamicDataRibbon->GenerationOutput, SourceParticleData, SceneProxy, BLASView, BLASViewOrigin, DynamicDataRibbon->bUseGPUInit, DynamicDataRibbon->bIsGPUSystem);
	...
	GenerateIndexBufferForView(RHICmdList, GpuRibbonDataManager, Collector, RenderingViewResources->IndexGenerationSettings, DynamicDataRibbon, RenderingViewResources, BLASView, BLASViewOrigin);

	SetupPerViewUniformBuffer(RenderingViewResources->IndexGenerationSettings, BLASView, *BLASView->Family, SceneProxy, RenderingViewResources->UniformBuffer);
	...
	SetupMeshBatchAndCollectorResourceForView(RHICmdList, RenderingViewResources->IndexGenerationSettings, DynamicDataRibbon, SourceParticleData, BLASView, *BLASView->Family, SceneProxy, RenderingResources.RibbonResources, RenderingViewResources, MeshBatch, DynamicDataRibbon->bUseGPUInit);
	...
	// A single ray tracing geometry is reused by all views
	// TODO: Support per-view ray tracing geometry when using ENiagaraRibbonFacingMode::Screen
	{
		FRayTracingDynamicGeometryUpdateParams UpdateParams;
		UpdateParams.MeshBatchesView = CollectorMeshBatches;
		UpdateParams.bUsingIndirectDraw = bDataGeneratedOnGpu;
		UpdateParams.NumVertices = VertexCount;
		UpdateParams.VertexBufferSize = VertexCount * static_cast<uint32>(sizeof(FVector3f));
		UpdateParams.NumTriangles = TriangleCount;
		UpdateParams.Geometry = &RayTracingGeometry;
		UpdateParams.Buffer = VertexBuffer;
		UpdateParams.bApplyWorldPositionOffset = true;

		Collector.AddRayTracingGeometryUpdate(BLASViewIndex, MoveTemp(UpdateParams));
	}
...
void FNiagaraRendererRibbons::GenerateShapeStateMultiPlane(FNiagaraRibbonShapeGeometryData& State, int32 MultiPlaneCount, int32 WidthSegmentationCount, bool bEnableAccurateGeometry, bool bUseMaterialBackfaceCulling)
{
	...
	const float NormalizeVertexId = float(WidthSegmentationCount > 0 ? WidthSegmentationCount : 1);
	for (int32 PlaneIndex = 0; PlaneIndex < MultiPlaneCount; PlaneIndex++)
	{
		...
		{
			const FVector2f Position = FVector2f((static_cast<float>(VertexId) / NormalizeVertexId) - 0.5f, 0).GetRotated(RotationAngle);
			const FVector2f Normal = FVector2f(0, 1).GetRotated(RotationAngle);
			const float TextureV = static_cast<float>(VertexId) / NormalizeVertexId;
...
void FNiagaraRendererRibbons::GenerateShapeStateTube(FNiagaraRibbonShapeGeometryData& State, int32 TubeSubdivisions, bool bUseMaterialBackfaceCulling)
{
	...
	const float NormalizeVertexId = float(TubeSubdivisions > 0 ? TubeSubdivisions : 1);
	for (int32 VertexId = 0; VertexId <= TubeSubdivisions; VertexId++)
	{
		const float RotationAngle = (static_cast<float>(VertexId) / NormalizeVertexId) * -360.0f;
...
void FNiagaraRendererRibbons::GenerateShapeStatePlane(FNiagaraRibbonShapeGeometryData& State, int32 WidthSegmentationCount, bool bUseMaterialBackfaceCulling)
{
	...
	for (int32 VertexId = 0; VertexId <= WidthSegmentationCount; VertexId++)
	{
		const FVector2f Position = FVector2f((static_cast<float>(VertexId) / NormalizeVertexId) - 0.5f, 0);
...
void FNiagaraRendererRibbons::InitializeShape(const UNiagaraRibbonRendererProperties* Properties)
{
	...
	{
		GenerateShapeStateMultiPlane(ShapeState, Properties->MultiPlaneCount, Properties->GetWidthSegmentationCount(), Properties->bEnableAccurateGeometry, Properties->bUseMaterialBackfaceCulling);
	}
	else
	{
		GenerateShapeStatePlane(ShapeState, Properties->GetWidthSegmentationCount(), Properties->bUseMaterialBackfaceCulling);
	}	
}
...
{	
	...
	PerViewUniformParameters.TwistDataOffset = GenerationConfig.HasTwist() ? VFVariables[ENiagaraRibbonVFLayout::Twist].GetGPUOffset() : -1;
    ...
    FMeshBatch& OutMeshBatch, bool bShouldUseGPUInitIndices) const
{
	...
	if (bShouldUseGPUInitIndices)
	{
		...
		MeshElement.IndirectArgsOffset = View->IsInstancedStereo() ? FNiagaraRibbonIndirectDrawBufferLayout::StereoDrawIndirectParametersByteOffset : FNiagaraRibbonIndirectDrawBufferLayout::DrawIndirectParametersByteOffset;
	}
	else
	{
...
void FNiagaraRendererRibbons::InitializeViewIndexBuffersGPU(FRHICommandListImmediate& RHICmdList, FNiagaraGpuComputeDispatchInterface* ComputeDispatchInterface, const FNiagaraRibbonGPUInitParameters& GpuInitParameters, const FNiagaraRibbonRenderingFrameViewResources* RenderingViewResources) const
{
	...
	{
		TShaderMapRef<FNiagaraRibbonCreateIndexBufferParamsCS> ComputeShader(GetGlobalShaderMap(GMaxRHIFeatureLevel));
		...
		Params.RibbonTessellationMode = GetGpuTessellationMode(GenerationConfig);
		...
		
	{
		...
		RHICmdList.Transition(FRHITransitionInfo(RenderingViewResources->IndexBuffer->UAV, ERHIAccess::SRVMask | ERHIAccess::VertexOrIndexBuffer, ERHIAccess::UAVCompute));
		...
		RHICmdList.Transition(FRHITransitionInfo(RenderingViewResources->IndexBuffer->UAV, ERHIAccess::UAVCompute, ERHIAccess::SRVMask | ERHIAccess::VertexOrIndexBuffer));
	}
}
...
FRibbonComputeUniformParameters FNiagaraRendererRibbons::SetupComputeVertexGenParams(FNiagaraGpuComputeDispatchInterface* ComputeDispatchInterface, const TSharedPtr<FNiagaraRibbonRenderingFrameResources>& RenderingResources, const FNiagaraRibbonGPUInitParameters& GpuInitParameters) const
{
	...
	CommonParams.RibbonLinkOrderIsFloat = bGpuRibbonLinkIsFloat;
	CommonParams.RibbonLinkOrderDataOffset = GpuRibbonLinkOrderOffset;

	CommonParams.RibbonTessellationMode = GetGpuTessellationMode(GenerationConfig);
...
{	
	...
	
	{
		...
		// apply the aggregate data onto the output buffers
		{
			...
			if (ApplyParams.OutputMultiRibbonIndices)
			{
				Transitions.Emplace(ApplyParams.OutputMultiRibbonIndices, ERHIAccess::SRVMask, ERHIAccess::UAVMask);
			}
			else
			{
				ApplyParams.OutputMultiRibbonIndices = ComputeDispatchInterface->GetEmptyUAVFromPool(RHICmdList, VertexBuffers.MultiRibbonIndicesBuffer.PixelFormat, ENiagaraEmptyUAVType::Buffer);
			}
			ApplyParams.OutputTessellationStats = TempBuffers.TransientTessellationStats.UAV;
			if (ApplyParams.OutputTessellationStats)
			{
				Transitions.Emplace(ApplyParams.OutputTessellationStats, ERHIAccess::SRVMask, ERHIAccess::UAVMask);
			}
			else
			{
				ApplyParams.OutputTessellationStats = ComputeDispatchInterface->GetEmptyUAVFromPool(RHICmdList, EPixelFormat::PF_R32_FLOAT, ENiagaraEmptyUAVType::Buffer);
			}
			ApplyParams.OutputSegments = VertexBuffers.SegmentsBuffer.UAV;

			Transitions.Emplace(ApplyParams.OutputTangentsAndDistances, ERHIAccess::SRVMask, ERHIAccess::UAVMask);
			Transitions.Emplace(ApplyParams.OutputSegments, ERHIAccess::SRVMask, ERHIAccess::UAVMask);
		...
		
		{
			...
			FinalizationParams.MultiRibbonIndices = FNiagaraRenderer::GetSrvOrDefaultUInt(VertexBuffers.MultiRibbonIndicesBuffer.SRV);
			FinalizationParams.TessellationStats = FNiagaraRenderer::GetSrvOrDefaultFloat(TempBuffers.TransientTessellationStats.SRV);
		...
		
		{
	...
	
			{
				...
				FComputeShaderUtils::DispatchIndirect(RHICmdList, ComputeShader, FinalizeParams, VertexBuffers.GPUComputeCommandBuffer.Buffer, CommandBufferOffset * VERTEX_GEN_OUTPUT_DATA_AUTO_TESSELLATION_NUM_ELEMENTS);
