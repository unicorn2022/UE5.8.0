	...
	if (BindingMode == ENiagaraDataInterfaceEmitterBindingMode::Self)
	{
		...
		else
		{
			...
			for (UNiagaraScript* NiagaraScript : { NiagaraSystem->GetSystemUpdateScript(), NiagaraSystem->GetSystemSpawnScript() })
			{
				...
				{
					const FNiagaraScriptRuntimeData* ScriptRuntimeData = NiagaraSystem->GetScriptRuntimeData(nullptr, *NiagaraScript).Get();
					if (ScriptRuntimeData == nullptr)
					{
						continue;
					}
					const FNiagaraScriptResolvedDataInterfaceInfo* MatchingResolvedDataInterface = ScriptRuntimeData->GetResolvedDataInterfaces().FindByPredicate(
		...
		{
			UE_LOGF(LogNiagara, Warning, "EmitterBinding failed to find self emitter");
		}
	}
	else
	{
		if ( EmitterName.IsNone() )
		{
			if ( FNiagaraUtilities::LogVerboseWarnings() )
			{
				UE_LOGF(LogNiagara, Warning, "EmitterName has not been set but we are in Other mode");
			}
			return nullptr;
		}
		...
		if (FNiagaraUtilities::LogVerboseWarnings())
		{
			UE_LOGF(LogNiagara, Warning, "EmitterBinding failed to find emitter '%ls' it might not exist or has been cooked out", *EmitterNameString);
		}
	}
	return nullptr;
}
...
const FNiagaraEmitterHandle* FNiagaraDataInterfaceEmitterBinding::ResolveHandle(const UNiagaraDataInterface* DataInterface) const
{
	...
	if (BindingMode == ENiagaraDataInterfaceEmitterBindingMode::Self)
	{
		...
		else
		{
			...
			for (const FNiagaraEmitterHandle& EmitterHandle : OwnerSystem->GetEmitterHandles())
			{
				...
				if (EmitterHandle.GetIsEnabled() && EmitterData)
				{
					...
					EmitterData->ForEachScript(
						[OwnerSystem, &EmitterHandle, DataInterface, &bFoundDI, &bIsEmitterNamespace](const UNiagaraScript* Script)
						{
							const FNiagaraScriptRuntimeData* ScriptRuntimeData = OwnerSystem->GetScriptRuntimeData(&EmitterHandle, *Script).Get();
							if (ScriptRuntimeData == nullptr)
							{
								return;
							}
			...
			{
				const FNiagaraScriptRuntimeData* ScriptRuntimeData = OwnerSystem->GetScriptRuntimeData(nullptr, *SystemScript).Get();
				if (ScriptRuntimeData == nullptr)
				{
					continue;
				}
