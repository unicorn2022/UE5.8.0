...
namespace NiagaraRendererReadback
{
	...
	struct FRendererReadbackRequest
	{
		...
		bool													bCaptureAllPrimitives = false;
	...
	void ExportMeshBatch(FRHICommandList& RHICmdList, FScene* Scene, ERHIFeatureLevel::Type FeatureLevel, const FReadbackMeshBatch& ReadbackMeshBatch, bool bApplyWPO, bool bWorldSpace, const FNiagaraRendererReadbackResult& VertexLayout, FRDGBufferUAV* VertexBufferUAV)
	{
		...
		for (const FMaterialRenderProxy* MaterialRenderProxy=MeshBatch.MaterialRenderProxy; MaterialRenderProxy; MaterialRenderProxy=MaterialRenderProxy->GetFallback(FeatureLevel))
		{
			...
			SingleShaderBindings.Add(Shader->bWorldSpace,				bWorldSpace ? 1 : 0);
			...
			SingleShaderBindings.Add(Shader->VertexTexCoordBindings,	VertexLayout.VertexTexCoordBindings);
	...
	public:
				...
				// Capture data
				{
					...
					GraphBuilder.AddPass(
						...
						[Request, PassParameters, Scene, FeatureLevel, SceneView](/*FRDGAsyncTask, */FRHICommandListImmediate& RHICmdList)
						{
							...
							{
								ExportMeshBatch(RHICmdList, Scene, FeatureLevel, ReadbackMeshBatch, Request->bApplyWPO, Request->bWorldSpace, Request->Result, PassParameters->RWVertexBuffer);
							}
						}
					);
				}
		...
		void AddMeshBatch(const FSceneView* View, const FPrimitiveSceneProxy* SceneProxy, const FMeshBatch& MeshBatch, uint32 NumInstances, uint32 NumVerticesPerInstance)
		{
			for (const FRendererReadbackRequestPtr& Request : ReadbackRequests)
			{
				if (Request->bCaptureAllPrimitives == false && !Request->PrimitivesToCapture.Contains(SceneProxy->GetPrimitiveComponentId()))
				{
					continue;
				}
	...
	template<typename T>
	T GetVertexValue(const FNiagaraRendererReadbackResult& Result, uint32 Vertex, uint32 ComponentOffset, uint32 ElementOffset, const T& DefaultValue)
	{
		T Value = DefaultValue;
		if (Vertex < Result.NumVertices && ComponentOffset != INDEX_NONE)
		{
			const uint32 Offset = (Result.VertexStride * Vertex) + ComponentOffset + ElementOffset;
	...
	{
		return GetVertexValue(Result, Vertex, ComponentOffset, 0, DefaultValue);
	}

	void InternalEnqueueReadback(FNiagaraGpuComputeDispatchInterface* ComputeInterface, FRendererReadbackRequestPtr ReadbackRequest, const FNiagaraRendererReadbackParameters& Parameters)
	{
		...
		ReadbackRequest->bWorldSpace		= Parameters.CoordinateSpace == ENiagaraRendererReadbackCoordinateSpace::World;
		...
		// Build Vertex Output
		{
			...
			if (Parameters.ExportNumTexCoords > 0)
			{
				...
				for (int32 i=0; i < FMath::Min<int32>(Parameters.ExportTexCoordBindings.Num(), UE_ARRAY_COUNT(Result.VertexTexCoordBindings)); ++i )
				{
					Result.VertexTexCoordBindings[i] = uint32(Parameters.ExportTexCoordBindings[i]);
				}
			}
		}
	...
	void EnqueueReadbackForComponents(TConstArrayView<UNiagaraComponent*> Components, FNiagaraRendererReadbackComplete Callback, const FNiagaraRendererReadbackParameters& Parameters)
	{
		...
		ReadbackRequest->bCaptureAllPrimitives = false;
		...
		InternalEnqueueReadback(ComputeInterface, ReadbackRequest, Parameters);
	}

	void EnqueueReadbackForWorld(UWorld* World, FNiagaraRendererReadbackComplete Callback, const FNiagaraRendererReadbackParameters& Parameters)
	{
		FRendererReadbackRequestPtr ReadbackRequest = MakeShared<FRendererReadbackRequest>();
		ReadbackRequest->CompleteCallback = MoveTemp(Callback);
		ReadbackRequest->bCaptureAllPrimitives = true;

		FNiagaraGpuComputeDispatchInterface* ComputeInterface = FNiagaraGpuComputeDispatchInterface::Get(World);
		if (ComputeInterface == nullptr)
		{
			ReadbackRequest->Result.Errors.AddUnique(TEXT("Request failed due to no compute interface."));
			ExecuteCompleteCallback(ReadbackRequest);
			return;
...
{
	EnqueueReadbackForComponents(Components, MoveTemp(Callback), Parameters);
}
...
{
	EnqueueReadbackForComponents(MakeArrayView(&Component, 1), MoveTemp(Callback), Parameters);
}

void NiagaraRendererReadback::EnqueueReadback(UWorld* World, FNiagaraRendererReadbackComplete Callback, const FNiagaraRendererReadbackParameters& Parameters)
{
	EnqueueReadbackForWorld(World, MoveTemp(Callback), Parameters);
}
