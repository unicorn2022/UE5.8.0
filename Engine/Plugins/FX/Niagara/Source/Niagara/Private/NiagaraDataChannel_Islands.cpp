...
namespace NDCIslands_Local
{
	...
	static FAutoConsoleVariableRef CVarAllowAsyncLoad(TEXT("fx.Niagara.DataChannels.AllowAsyncLoad"), AllowAsyncLoad, TEXT("True if we should attempt to load systems etc asynchronously."), ECVF_Default);
...
void UNiagaraDataChannelHandler_Islands::Tick(float DeltaTime, ETickingGroup TickGroup)
{
	...
	//UE_LOGF(LogNiagara,Warning, "UNiagaraDataChannelHandler_Islands::Tick - %ls", *TGEnum->GetDisplayNameTextByValue((int32)TickGroup).ToString());
	const UNiagaraDataChannel_Islands* IslandChannel = GetChannelTyped<const UNiagaraDataChannel_Islands>();
	for (auto It = ActiveIslands.CreateIterator(); It; ++It)
	{
		FNDCIsland& Island = IslandPool[*It];
...
FNDCIsland* UNiagaraDataChannelHandler_Islands::FindOrCreateIsland(const FNiagaraDataChannelSearchParameters& SearchParams, ENiagaraResourceAccess AccessType)
{
	...
	//Failing that, get and init a new island from the pool.
...
int32 UNiagaraDataChannelHandler_Islands::ActivateNewIsland(FVector Location)
{
	...
	{
		UE_LOGF(LogNiagara, Error, "Failed to allocate new island for Island Data Channel %ls", *GetDataChannel()->GetName());
	}
...
void FNDCIsland::OnAcquired(FVector Location)
{
	...
	for (const TObjectPtr<UNiagaraSystem>& Sys : Channel->GetSystems())
	{
		if(Sys)
		{
			...
			SpawnParams.bAutoActivate = false;//We must activate AFTER adding this component to our handler system array.
...
void FNDCIsland::Tick(const ETickingGroup& TickGroup)
{
	...
	if (AddedData > 0)
	{
		for (UNiagaraComponent* Comp : SpawnedComponents)
		{
			if (::IsValid(Comp) && Comp->IsComplete())
			{
				Comp->Activate();
			}
		}
...
{
	const bool bSystemsIncomplete = SpawnedComponents.ContainsByPredicate([](const TObjectPtr<UNiagaraComponent>& Comp) { return IsValid(Comp) && !Comp->IsComplete(); });
	return Data.IsValid() && (!Data.IsUnique() || bSystemsIncomplete);
}
...
bool FNDCIsland::TryGrow(FVector Point, FVector PerElementExtents, FVector MaxIslandExtents)
{
	...
	//If new bounds are valid, grow our bounds and update the system bounds
	FBox HandlerSystemBounds(-Bounds.BoxExtent, Bounds.BoxExtent);
