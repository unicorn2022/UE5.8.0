...
struct FNiagaraGrid2DLegacyTiled2DInfo
{
	...
	void CopyTo2D(FRDGBuilder& GraphBuilder, FRDGTexture* SourceTexture, FRHITexture* DestinationTextureRHI, const TCHAR* NameIfNotRegistered) const
	{
		...
		if (ensureMsgf(DestTextureDesc.Dimension != ETextureDimension::Texture2D, TEXT("Destination for Grid2D copy is invalid type (%d), copy will fail"), EnumToUnderlyingType(DestTextureDesc.Dimension)))
		{
			return;
		}
...
bool UNiagaraDataInterfaceGrid2DCollection::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	{
		UE_LOGF(LogNiagara, Log, "%ls: Unnamed attributes should not be used with named.  This is a deprecated workflow", *Proxy->GetSourceDIName().ToString());
	}
	...
	InstanceData->PixelFormat = FNiagaraUtilities::BufferFormatToPixelFormat(BufferFormat, GNDIGrid2DFomatCaps, 1, this);
...
void UNiagaraDataInterfaceGrid2DCollection::CollectAttributesForScript(UNiagaraScript* Script, const FNiagaraScriptRuntimeData& ScriptRuntimeData, const UNiagaraDataInterface* DataInterface, TArray<FNiagaraVariableBase>& OutVariables, TArray<uint32>& OutVariableOffsets, int32& TotalAttributes, TArray<FText>* OutWarnings)
{
	if (const FNiagaraScriptExecutionParameterStore* ParameterStore = ScriptRuntimeData.GetExecutionReadyParameterStore())
	{
		...
		if (IndexOfDataInterface != INDEX_NONE && IndexOfDataInterface < ParamInfoArray.Num())
		{
...
bool UNiagaraDataInterfaceGrid2DCollection::PerInstanceTickPostSimulate(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (InstanceData->NeedsRealloc && InstanceData->NumCells.X > 0 && InstanceData->NumCells.Y > 0)
	{
		...
		if (InstanceData->TargetTexture)
		{
			...
			bNeedsReset = InstanceData->UpdateTargetTexture(BufferFormat, this);
		}
...
bool FGrid2DCollectionRWInstanceData_GameThread::UpdateTargetTexture(ENiagaraGpuBufferFormat BufferFormat, UObject* Owner)
{
	...
	if (UObject* UserParamObject = RTUserParamBinding.GetValue())
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "RenderTarget UserParam is a '%ls' but is expected to be a UTextureRenderTarget2DArray or UTextureRenderTarget2D", *GetNameSafe(UserParamObject->GetClass()));
		}
	}
	...
	if (TargetTexture != nullptr)
	{
		...
		{
			const TOptional<EPixelFormat> RenderTargetFormat = FNiagaraUtilities::BufferFormatToPixelFormat(BufferFormat, GNDIGrid2DFomatCaps, 1, Owner);
		...
		else if (UTextureRenderTarget2D* TargetTexture2D = Cast<UTextureRenderTarget2D>(TargetTexture))
		{
			...
			else
			{
				...
				const TOptional<ETextureRenderTargetFormat> RenderTargetFormat  = FNiagaraUtilities::BufferFormatToRenderTargetFormat(BufferFormat, GNDIGrid2DFomatCaps, Owner);
				if ( RenderTargetFormat.IsSet() )
				{
...
void FNiagaraDataInterfaceProxyGrid2DCollectionProxy::PostSimulate(const FNDIGpuComputePostSimulateContext& Context)
{
	...
	if (ProxyData->bPreviewGrid && ProxyData->CurrentData && ProxyData->CurrentData->IsValid())
	{
		...
		{
			GpuComputeDebugInterface.AddAttributeTexture(GraphBuilder, Context.GetSystemInstanceID(), GetSourceDIName(), ProxyData->CurrentData->GetOrCreateTexture(GraphBuilder), FIntPoint::ZeroValue, ProxyData->PreviewAttribute);
		}
		else
		{
