...
const FNiagaraSystemScalabilitySettings& UNiagaraEffectType::GetActiveSystemScalabilitySettings()const
{
	...
	//UE_LOGF(LogNiagara, Warning, "Could not find active system scalability settings for EffectType %ls", *GetFullName());
...
const FNiagaraEmitterScalabilitySettings& UNiagaraEffectType::GetActiveEmitterScalabilitySettings()const
{
	...
	//UE_LOGF(LogNiagara, Warning, "Could not find active emitter scalability settings for EffectType %ls", *GetFullName());

	static FNiagaraEmitterScalabilitySettings Dummy;
	return Dummy;
}

const FNiagaraReadinessSettingsProfile* UNiagaraEffectType::FindReadinessSettingsProfile(FGameplayTag Profile)const
{
	FGameplayTag ProfileToUse = Profile.IsValid() ? Profile : ReadinessProfile;
	if (ReadinessSettings)
	{
		return ReadinessSettings->Find(ProfileToUse);
	}
