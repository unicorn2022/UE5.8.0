...
bool UNiagaraDataInterfaceCurveBase::CompareLUTS(const TArray<float>& OtherLUT) const
{
	if (ShaderLUT.Num() == OtherLUT.Num())
	{
		...
		for (int32 i = 0; i < ShaderLUT.Num(); i++)
		{
			...
			{
				bMatched = false;
				UE_LOGF(LogNiagara, Log, "First LUT mismatch found on comparison - LUT[%d] = %.9f  Other = %.9f \t%.9f", i, ShaderLUT[i], OtherLUT[i], fabsf(ShaderLUT[i] - OtherLUT[i]));
				break;
			}
		}
		return bMatched;
	...
	{
		UE_LOGF(LogNiagara, Log, "Table sizes don't match");
		return false;
	}
}
...
void UNiagaraDataInterfaceCurveBase::CacheStaticBuffers(struct FNiagaraSystemStaticBuffers& StaticBuffers, bool bIsExternalDataInterface, bool bUsedByCPU, bool bUsedByGPU)
{
	LUTOffset = INDEX_NONE;
	if (bUsedByGPU && bIsExternalDataInterface == false)
	{
