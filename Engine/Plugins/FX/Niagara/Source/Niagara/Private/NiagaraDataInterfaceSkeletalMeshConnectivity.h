...
struct FSkeletalMeshConnectivity
{
	...
	FSkeletalMeshConnectivity(TWeakObjectPtr<USkinnedAsset> InMeshObject, int32 InLodIndex);
	...
	bool CanBeUsed(const TWeakObjectPtr<USkinnedAsset>& InMeshObject, int32 InLodIndex) const;
	const FSkeletalMeshConnectivityProxy* GetProxy() const;

	static bool IsValidMeshObject(TWeakObjectPtr<USkinnedAsset>& MeshObject, int32 InLodIndex);
...
private:
	static const FSkeletalMeshLODRenderData* GetLodRenderData(const USkinnedAsset& Mesh, int32 LodIndex);
