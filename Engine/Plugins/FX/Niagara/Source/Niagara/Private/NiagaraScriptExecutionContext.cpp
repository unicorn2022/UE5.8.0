...
bool FNiagaraScriptExecutionContextBase::Init(FNiagaraSystemInstance* ParentSystemInstance, UNiagaraScript* InScript, TSharedPtr<const FNiagaraScriptRuntimeData> InScriptRuntimeData)
{
	if (InScript == nullptr || InScriptRuntimeData.IsValid() == false)
	{
		return false;
	}

	Script = InScript;
	ScriptRuntimeData = InScriptRuntimeData;

	Parameters.InitFromOwningContext(Script, ScriptRuntimeData.Get(), true);
	...
	if(ParentSystemInstance == nullptr)
	{
		ENiagaraScriptUsage Usage = Script->GetUsage();
		for (const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDIInfo : ScriptRuntimeData->GetResolvedDataInterfaces())
		{
...
{
	ENiagaraScriptUsage Usage = Script->GetUsage();
	for (auto& ResolvedDIInfo : ScriptRuntimeData->GetResolvedDataInterfaces())
	{
		if (ResolvedDIInfo.ResolvedDataInterface)
		{
			bHasDIsWithPreStageTick |= (ResolvedDIInfo.ResolvedDataInterface->HasPreStageTick(Usage));
	...
	{
		DIStageTickHandler.Init(Usage, ScriptRuntimeData.Get(), Instance);
	}
}
...
bool FNiagaraScriptExecutionContext::Tick(FNiagaraSystemInstance* ParentSystemInstance, ENiagaraSimTarget SimTarget)
{
	...
	if (Parameters.GetInterfacesDirty())
	{
		...
		if (Script && Script->IsReadyToRun(ENiagaraSimTarget::CPUSim) && SimTarget == ENiagaraSimTarget::CPUSim && ScriptRuntimeData.IsValid())//TODO: Remove. Script can only be null for system instances that currently don't have their script exec context set up correctly.
		{
			...
			// UE_LOGF(LogNiagara, Log, "Updating data interfaces for script %ls", *Script->GetFullName());

			// We must make sure that the data interfaces match up between the original script values and our overrides...
			TConstArrayView<FNiagaraScriptResolvedDataInterfaceInfo> ScriptDataInterfaceInfos = ScriptRuntimeData->GetResolvedDataInterfaces();
			if (ScriptDataInterfaceInfos.Num() != DataInterfaces.Num())
			{
				UE_LOGF(LogNiagara, Warning, "Mismatch between Niagara Exectuion Context data interfaces and those in it's script!");
				return false;
			}
			...
			if (ParentSystemInstance)
			{
				...
				for (int32 i = 0; i < DataInterfaces.Num(); i++)
				{
					...
					int32 UserPtrIdx = ScriptDataInterfaceInfos[i].UserPtrIdx;
					if (UserPtrIdx != INDEX_NONE)
					{
						...
						{
							UE_LOGF(LogNiagara, Warning, "Failed to resolve User Pointer for UserPtrTable[%d] looking for DI: %ls for system: %ls",
								UserPtrIdx,
								*Interface->GetName(),
			...
			const FNiagaraScriptExecutionParameterStore* ScriptParameterStore = ScriptRuntimeData->GetExecutionReadyParameterStore();
			check(ScriptParameterStore != nullptr);
			const auto& ScriptDataInterfaces = ScriptParameterStore->GetDataInterfaces();
			TConstArrayView<FVMExternalFunction> ScriptCalledVMExternalFunctionBindings = ScriptRuntimeData->GetCalledVMExternalFunctionBindings();
			...
			for (int32 FunctionIt = 0; FunctionIt < FunctionCount; ++FunctionIt)
			{
				...
				for (int32 i = 0; i < ScriptDataInterfaceInfos.Num(); i++)
				{
					const FNiagaraScriptResolvedDataInterfaceInfo& ScriptInfo = ScriptDataInterfaceInfos[i];
					UNiagaraDataInterface* ExternalInterface = DataInterfaces[i];
					if (ScriptInfo.CompileName == BindingInfo.OwnerName)
					{
						// first check to see if we should just use the one from the script
						if (ScriptCalledVMExternalFunctionBindings.IsValidIndex(FunctionIt)
							&& ScriptDataInterfaces.IsValidIndex(i)
							&& ExternalInterface == ScriptDataInterfaces[i])
						{
							const FVMExternalFunction& ScriptFuncBind = ScriptCalledVMExternalFunctionBindings[FunctionIt];
						...
						{
							UE_LOGF(LogNiagara, Error, "Could not Get VMExternalFunction '%ls'.. emitter will not run!", *BindingInfo.Name.ToString());
			...
			for (int32 i = 0; i < FunctionTable.Num(); i++)
			{
				...
				{
					UE_LOGF(LogNiagara, Warning, "Invalid Function Table Entry! %ls", *ScriptExecutableData.CalledVMExternalFunctions[i].Name.ToString());
				}
			}
			...
			{
				UE_LOGF(LogNiagara, Warning, "Error building data interface function table!");
				FunctionTable.Empty();
				return false;
			}
		}
...
bool FNiagaraSystemScriptExecutionContext::Init(FNiagaraSystemInstance* ParentSystemInstance, UNiagaraScript* InScript, TSharedPtr<const FNiagaraScriptRuntimeData> InScriptRuntimeData)
{
	...
	return FNiagaraScriptExecutionContextBase::Init(ParentSystemInstance, InScript, InScriptRuntimeData);
}
...
bool FNiagaraSystemScriptExecutionContext::Tick(class FNiagaraSystemInstance* Instance, ENiagaraSimTarget SimTarget)
{
	...
	if (Parameters.GetInterfacesDirty())
	{
		...
		if (Script && Script->IsReadyToRun(ENiagaraSimTarget::CPUSim))//TODO: Remove. Script can only be null for system instances that currently don't have their script exec context set up correctly.
		{
			...
			check(ScriptParameterStore != nullptr);
			const auto& ScriptDataInterfaces = ScriptParameterStore->GetDataInterfaces();
			if (ScriptDataInterfaceInfos.Num() != ScriptDataInterfaces.Num())
			{
				UE_LOGF(LogNiagara, Warning, "Error building data interface function table for system script! %ls", *Instance->GetSystem()->GetPathName());
				FunctionTable.Empty();
				return false;
			}
			...
			for (int32 FunctionIndex = 0; FunctionIndex < FunctionCount; ++FunctionIndex)
			{
				...
				for (int32 i = 0; i < ScriptDataInterfaceInfos.Num(); i++)
				{
					const FNiagaraScriptResolvedDataInterfaceInfo& ScriptDIInfo = ScriptDataInterfaceInfos[i];
					UNiagaraDataInterface* ScriptInterface = ScriptDataInterfaces[i];
					UNiagaraDataInterface* ExternalInterface = DataInterfaces[i];

					if (ScriptDIInfo.CompileName == BindingInfo.OwnerName)
					{
						...
						{
							// first check to see if we should just use the one from the script
							if (ScriptCalledVMExternalFunctionBindings.IsValidIndex(FunctionIndex)
								&& ScriptInterface
								&& ExternalInterface == ScriptDataInterfaces[i])
							{
								const FVMExternalFunction& ScriptFuncBind = ScriptCalledVMExternalFunctionBindings[FunctionIndex];
								if (ScriptFuncBind.IsBound())
								{
				...
				{
					UE_LOGF(LogNiagara, Warning, "Error building data interface function table for system script!");
					FunctionTable.Empty();
					return false;
				}
			}
...
bool FNiagaraSystemScriptExecutionContext::GeneratePerInstanceDIFunctionTable(FNiagaraSystemInstance* Inst, TArray<FNiagaraPerInstanceDIFuncInfo>& OutFunctions)
{
	...
	const FNiagaraVMExecutableData& ScriptExecutableData = Script->GetVMExecutableData();
	TConstArrayView<FNiagaraScriptResolvedDataInterfaceInfo> ScriptDataInterfaceInfos = ScriptRuntimeData->GetResolvedDataInterfaces();
	const TArray<UNiagaraDataInterface*>& ContextDataInterfaces = GetDataInterfaces();
	if (ScriptDataInterfaceInfos.Num() != ContextDataInterfaces.Num())
	{
		return false;
	}
	...
	for (int32 FunctionIndex = 0; FunctionIndex < ScriptExecutableData.CalledVMExternalFunctions.Num(); ++FunctionIndex)
	{
		...
		{
			const FNiagaraScriptResolvedDataInterfaceInfo& ScriptDIInfo = ScriptDataInterfaceInfos[i];
			UNiagaraDataInterface* ExternalInterface = ContextDataInterfaces[i];

			if (ScriptDIInfo.CompileName == BindingInfo.OwnerName && (ScriptDIInfo.NeedsPerInstanceBinding()))
			{
				...
				if (const int32* UserDIIndex = Inst->GetInstanceParameters().FindParameterOffset(ScriptDIInfo.ResolvedVariable))
				{
					//If this is a User DI we bind to the user DI and find instance data with it.
					if (UNiagaraDataInterface* UserInterface = Inst->GetInstanceParameters().GetDataInterface(*UserDIIndex))
					{
