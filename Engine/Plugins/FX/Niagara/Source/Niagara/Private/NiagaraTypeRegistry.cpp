...
namespace NiagaraTypeRegistryLocal
{
	...
	static std::atomic<int32> GSuspendPluginUnmountCounter = 0;
	static FCriticalSection GPluginUnmountSync;
	static TArray<FString> GPluginMountPointsToUnmount;

	void PluginUnmounted(IPlugin& Plugin)
	{
		const FString MountedPath = Plugin.GetMountedAssetPath();

		if (GSuspendPluginUnmountCounter.load(std::memory_order_acquire) > 0)
		{
			FScopeLock Guard(&NiagaraTypeRegistryLocal::GPluginUnmountSync);
			NiagaraTypeRegistryLocal::GPluginMountPointsToUnmount.Add(MountedPath);
		}
		else
		{
...
void FNiagaraTypeRegistry::SuspendPluginUnmount()
{
	NiagaraTypeRegistryLocal::GSuspendPluginUnmountCounter.fetch_add(1, std::memory_order_acq_rel);
}

void FNiagaraTypeRegistry::ResumePluginUnmount()
{
	if (NiagaraTypeRegistryLocal::GSuspendPluginUnmountCounter.fetch_sub(1, std::memory_order_acq_rel) == 1)
	{
		TArray<FString> LocalListOfPaths;
		{
			FScopeLock Guard(&NiagaraTypeRegistryLocal::GPluginUnmountSync);
			LocalListOfPaths = MoveTemp(NiagaraTypeRegistryLocal::GPluginMountPointsToUnmount);
		}
		InvalidateTypesByPaths(LocalListOfPaths);
	}
}

void FNiagaraTypeRegistry::InvalidateTypesByPath(const FString& AssetPath)
{
	InvalidateTypesByPaths({AssetPath});
}

void FNiagaraTypeRegistry::InvalidateTypesByPaths(TConstArrayView<FString> AssetPaths)
{
	...
	TRACE_CPUPROFILER_EVENT_SCOPE(NiagaraTypeRegistry_InvalidateTypesByPaths);
...

	{
		...
		FNameBuilder RegistryTypeNameStr;
		const int32 TypeCount = Registry.RegisteredSoftObjectPaths.Num();
		for (int32 TypeIt = 0; TypeIt < TypeCount; ++TypeIt)
		{
			Registry.RegisteredSoftObjectPaths[TypeIt].GetLongPackageFName().ToString(RegistryTypeNameStr);
			const FStringView RegistryTypeName = RegistryTypeNameStr.ToView();
			for (const FString& AssetPath : AssetPaths)
			{
				if (RegistryTypeName.StartsWith(AssetPath))
				{
...
int32 FNiagaraTypeRegistry::RegisterTypeInternal(const FNiagaraTypeDefinition& NewType)
{
	...
	if (NewType.RegisteredTypeDefIndex != INDEX_NONE)
	{
		if (NewType.IsValid())
		{
...

			{
				...
				{
					const FNiagaraTypeDefinition& OldType = *RegisteredTypes[NewType.RegisteredTypeDefIndex]; 
					...
					checkf(bNeedsRegistration || (NewType == OldType),
						...
						bNeedsRegistration, 
						*NewType.GetNameText().ToString(),
						*OldType.GetNameText().ToString(),
						TypeHash,
						GetTypeHash(OldType)
					);
				}
	...
	{
		// We may need to create a new entry for a registered type
		// Between the read and the write it could have been added so we need to do an addition search to make sure we don't double register
		{
			UE::TWriteScopeLock Lock(RegisteredTypesLock);
			NewType.RegisteredTypeDefIndex = RegisteredTypes.IndexOfByPredicate([NewType](const TUniquePtr<FNiagaraTypeDefinition>& InDef) {return *InDef.Get() == NewType; });
			if (NewType.RegisteredTypeDefIndex == INDEX_NONE)
			{
...

		{
			...
			if (!RegisteredTypeIndexMap.Contains(TypeHash))
			{
