...
void UNiagaraDataInterfaceMeshRendererInfo::GetFeedback(UNiagaraSystem* InAsset, UNiagaraComponent* InComponent, TArray<FNiagaraDataInterfaceError>& OutErrors, TArray<FNiagaraDataInterfaceFeedback>& OutWarnings, TArray<FNiagaraDataInterfaceFeedback>& OutInfo)
{
	...
	else if (!MeshRenderer->GetIsEnabled())
	{
	...
	// Check for the renderer being referenced but no longer referenced by it's parent emitter as that's likely a invalid dangling reference
	else if ( !MeshRenderer->IsContainedInEmitterRendererArray() )
	{
		OutWarnings.Emplace(
			LOCTEXT("OrphanedRendererWarning", "The selected Mesh Renderer is invalid"),
			LOCTEXT("OrphanedRendererWarningSummary", "Mesh Renderer is orphaned and no longer valid"),
			FNiagaraDataInterfaceFix::CreateLambda(
				[this]()
				{
					const FScopedTransaction Transaction(LOCTEXT("ClearMeshRenderer", "Clear Mesh Renderer"));
					Modify();
