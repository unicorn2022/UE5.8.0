	...
	FConsoleCommandWithArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args)
		{
			if(Args.Num() == 0)
			{
				...
				UE_LOGF(LogNiagara, Display, "Clearing the Niagara Quality Level override and returning quality level to %ls", *FNiagaraPlatformSet::GetQualityLevelText(FNiagaraPlatformSet::GetQualityLevel()).ToString());
			}
			else if(Args.Num() == 1)
			{
				if(FCString::IsNumeric(*Args[0]))
				{
					...
					if(ensure(NewQualityLevel >= 0 && NewQualityLevel <= 4))
					{
						...
						UE_LOGF(LogNiagara, Display, "Set the global Niagara quality level to %ls", *FNiagaraPlatformSet::GetQualityLevelText(FNiagaraPlatformSet::GetQualityLevel()).ToString());
					}
				}
			}
		}
	...
	FConsoleCommandWithArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args)
		{
			...
			if (Args.Num() == 0)
			{
				...
				UE_LOGF(LogNiagara, Warning, "Niagara Clearing Override DeviceProfile");
			}
			else
			{
				...
				{
					UE_LOGF(LogNiagara, Warning, "Niagara Setting Override DeviceProfile '%ls'", *Args[0]);
				}
				else
				{
					UE_LOGF(LogNiagara, Warning, "Niagara Failed to Find Override DeviceProfile '%ls'", *Args[0]);
				}
			}
		}
	)
...
int32 FNiagaraPlatformSet::GetActiveQualityMaskForDeviceProfile(const UDeviceProfile* Profile)
{
	...
	//UE_LOGF(LogNiagara, Warning, "%ls - DeviceProfile(%d)", *Profile->GetName(), QLMask);
...
bool FNiagaraPlatformSet::ApplyRedirects()
{
	...
	for (const FNiagaraPlatformSetRedirect& Redirect : Settings->PlatformSetRedirects)
	{
		if (Redirect.Mode == ENiagaraDeviceProfileRedirectMode::CVar)
		{
			...
			if (!CVar)
			{
				UE_LOGF(LogNiagara, Warning, "Could not find CVar %ls when trying to apply PlatformSet Redirects.", *Redirect.CVarConditionEnabled.CVarName.ToString());
				bSuccess  = false;
				continue;
			}
			...
			if (!CVar)
			{
				UE_LOGF(LogNiagara, Warning, "Could not find CVar %ls when trying to apply PlatformSet Redirects.", *Redirect.CVarConditionDisabled.CVarName.ToString());
				bSuccess = false;
				continue;
			}
#endif
...
// 				{
// 					UE_LOGF(LogNiagara, Warning, "Cannot redirect entry for device profile %ls with a CVar as it is not set for all quality levels.", *DPEntry.ProfileName.ToString());
// 					continue;
// 				}
// 
...
// 				}
			...
			for (auto It = DeviceProfileStates.CreateIterator(); It; It++)
			{
				...
				if(Redirect.ProfileNames.Contains(Entry.ProfileName))
				{
					if (bRedirect)
					{
						...
						if (!bMatch)
						{
							UE_LOGF(LogNiagara, Warning, "Found conflicting entries when trying to apply DP redirect from %ls to %ls", *Entry.ProfileName.ToString(), *Redirect.RedirectProfileName.ToString());
							bSuccess = false;
							break;
						}
						It.RemoveCurrent();
		...
		{
			UE_LOGF(LogNiagara, Warning, "Bad Mode for Platform Set Redirect!");
			bSuccess = false;
		}
	}
...
void FNiagaraPlatformSet::RefreshScalability()
{
	...
	if (GNiagaraQualityLevel != CachedQualityLevel)
	{
		...
		//Always refresh our scalability settings. Can we be more targeted here?
		for (TObjectIterator<UNiagaraSettingsProfileSet> It; It; ++It)
		{
			UNiagaraSettingsProfileSet* Settings = *It;
			check(Settings);
			Settings->OnScalabilityUpdate();
		}
	}
...
bool FNiagaraPlatformSetCVarCondition::Evaluate(const UDeviceProfile* DeviceProfile, int32 QualityLevel, bool bCheckCurrentStateOnly, bool& bOutCanEverPass, bool& bOutCanEverFail)const
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "Niagara Platform Set is trying to use a CVar that doesn't exist. %ls", *CVarName.ToString());
	}
