...
namespace NiagaraDecalRendererPropertiesLocal
{
	...
	static void SetupBindings(UNiagaraDecalRendererProperties* Props)
	{
		...
		Props->InitRendererBaseBindings();
...
TArray<FNiagaraVariable> UNiagaraDecalRendererProperties::GetBoundAttributes() const
{
	...
	BoundAttributes.Reserve(BoundAttributes.Num() + MaterialParameters.AttributeBindings.Num() + 1);
	...
	if (MaterialParameterBinding.IsSetToDefault() == false)
	{
		BoundAttributes.AddUnique(MaterialParameterBinding.AliasedParameter);
	}
