...
void FNiagaraGPUInstanceCountManager::ResizeBuffers(FRHICommandListImmediate& RHICmdList, int32 ReservedInstanceCounts)
{
	...
	if (RequiredInstanceCounts > 0)
	{
		...
		if (!AllocatedInstanceCounts)
		{
			...
			//UE_LOGF(LogNiagara, Log, "FNiagaraGPUInstanceCountManager::ResizeBuffers Alloc AllocatedInstanceCounts: %d ReservedInstanceCounts: %d", AllocatedInstanceCounts, ReservedInstanceCounts);
		}
		...
		else if (RequiredInstanceCounts > AllocatedInstanceCounts)
		{
			...
			//UE_LOGF(LogNiagara, Log, "FNiagaraGPUInstanceCountManager::ResizeBuffers Resize AllocatedInstanceCounts: %d ReservedInstanceCounts: %d", AllocatedInstanceCounts, ReservedInstanceCounts);
		}
