...
void FNCPool::Cleanup()
{
	for (FNCPoolElement& Elem : FreeElements)
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "Free element in the NiagaraComponentPool was null. Someone must be keeping a reference to a NC that has been freed to the pool and then are manually destroying it.");
		}
	}
	FreeElements.Empty();
}
...
UNiagaraComponent* FNCPool::Acquire(UWorld* World, UNiagaraSystem* Template, ENCPoolMethod PoolingMethod, bool bForceNew)
{
	...
	while (FreeElements.Num() && !bForceNew)//Loop until we pop a valid free element or we're empty.
	{
		...
		if (!RetElem.Component || !IsValidChecked(RetElem.Component.Get()))
		{			
			...
			UE_LOGF(LogNiagara, Warning, "Pooled NC has been destroyed or is pending kill! Possibly via a DestroyComponent() call. You should not destroy pooled components manually. \nJust deactivate them and allow them to destroy themselves or be reclaimed by the pool. | NC: %p |\t System: %ls", RetElem.Component.Get(), *Template->GetFullName());
	...
	if(RetElem.Component == nullptr)
	{
		...
		RetElem.Component.Reset(NewObject<UNiagaraComponent>(OuterObject));
...
void FNCPool::Reclaim(UNiagaraComponent* Component, const double CurrentTimeSeconds)
{
	...
	if (GbEnableNiagaraSystemPooling != 0 && FreeElements.Num() < (int32)Component->GetAsset()->MaxPoolSize && Component->GetWorld()->bIsTearingDown == false)
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Component is pending kill or unreachable when reclaimed Component(%p %ls)", Component, *Component->GetFullName());
			return;
		}
...
bool FNCPool::RemoveComponent(UNiagaraComponent* Component)
{
	...
	{
		if (FreeElements[i].Component.Get() == Component)
		{
...
{
	int32 i = 0;
	int32 PrimedSize = GNigaraAllowPrimedPools != 0 && Template ? Template->PoolPrimeSize : 0;
	while (i < FreeElements.Num() && FreeElements.Num() > PrimedSize)//Don't free below the primed size
	{
		...
		{
			if (UNiagaraComponent* Component = FreeElements[i].Component.Get())
			{
				Component->PoolingMethod = ENCPoolMethod::None; // Reset so we don't trigger warnings about destroying pooled NCs.
				Component->DestroyComponent();
			}
...
void UNiagaraComponentPool::Cleanup(UWorld* World)
{
	...
	{
		FNiagaraCrashReporterScope CRScope(Pool.Key.ResolveObjectPtr());//In practice this may be null by now :(
		Pool.Value.Cleanup();
	}
...
void UNiagaraComponentPool::PrimePool(UNiagaraSystem* Template, UWorld* World)
{
	...
	if (!Template)
	{
		UE_LOGF(LogNiagara, Warning, "Attempted UNiagaraComponentPool::PrimePool() with a NULL Template!");
		return;
	}
	...
	{
		UE_LOGF(LogNiagara, Warning, "Failed to prime particle pool as we are tearing the world down.");
		return;
	}

	if (World->Scene == nullptr)
	{
		UE_LOGF(LogNiagara, Verbose, "Failed to prime particle pool as the world does not have a scene.");
		return;
	}	
	...
	{
		UE_LOGF(LogNiagara, Verbose, "Failed to prime particle pool as the Niagara System needs to compile.");
		return;
	}
#endif
	...
	{
		UE_LOGF(LogNiagara, Verbose, "Failed to prime particle pool as the Niagara System is not ready to run.");
		return;
	}
	...
	if (FXSystemInterface)
	{
		...
		{
			UE_LOGF(LogNiagara, Verbose, "Failed to prime particle pool as the world does not have a FNiagaraGpuComputeDispatchInterface.");
			return;
		}
	}
	else
	{
		UE_LOGF(LogNiagara, Verbose, "Failed to prime particle pool as the world does not have an FFXSystem.");
		return;
	}
...
UNiagaraComponent* UNiagaraComponentPool::CreateWorldParticleSystem(UNiagaraSystem* Template, UWorld* World, ENCPoolMethod PoolingMethod)
{
	...
	if (!Template)
	{
		UE_LOGF(LogNiagara, Warning, "Attempted CreateWorldParticleSystem() with a NULL Template!");
		return nullptr;
	}

	if (World->bIsTearingDown)
	{
		UE_LOGF(LogNiagara, Warning, "Failed to create pooled particle system as we are tearing the world down.");
		return nullptr;
	}
...
void UNiagaraComponentPool::ReclaimWorldParticleSystem(UNiagaraComponent* Component)
{
	...
	{
		UE_LOGF(LogNiagara, Log, "Pooled NC has been destroyed! Possibly via a DestroyComponent() call. You should not destroy components set to auto destroy manually. \nJust deactivate them and allow them to destroy themselves or be reclaimed by the pool if pooling is enabled. | NC: %p |\t System: %ls", Component, Asset ? *Asset->GetFullName() : TEXT("(nullptr)"));
		return;
	}
	...
	if (GbEnableNiagaraSystemPooling && Asset != nullptr && WorldParticleSystemPools.Num() > 0)
	{
		...
		if (CurrentTime - LastParticleSytemPoolCleanTime > GNiagaraSystemPoolingCleanTime)
		{
			...
			{
				UNiagaraSystem* Template = Pair.Key.ResolveObjectPtr();
				Pair.Value.KillUnusedComponents(CurrentTime - GNiagaraSystemPoolKillUnusedTime, Template);
			}
		}
		...
		if (!bWasInList)
		{
			UE_LOGF(LogNiagara, Error, "World Niagara System Pool is reclaiming a component that is not in it's InUse list!");
		}
#else
		...
		if (!NCPool)
		{
			...
			if (!bWasInList)
			{
				UE_LOGF(LogNiagara, Warning, "WorldNC Pool trying to reclaim a system for which it doesn't have a pool! Likely because SetAsset() has been called on this NC. | World: %p | NC: %p | Sys: %ls", Component->GetWorld(), Component, *Component->GetAsset()->GetFullName());
			}
...
void UNiagaraComponentPool::PooledComponentDestroyed(UNiagaraComponent* Component)
{
	...
	switch (Component->PoolingMethod)
	{
		...
		case ENCPoolMethod::FreeInPool:
		{
			...
			{
				if (FNCPool* NCPool = WorldParticleSystemPools.Find(NiagaraSystem))
				{
...
#if ENABLE_NC_POOL_DEBUGGING
						UE_LOGF(LogNiagara, Warning, "UNiagaraComponentPool::PooledComponentDestroyed: Component is marked as FreeInPool but does not exist");
#endif
					}
				}
			}
		...
		default:
			UE_LOGF(LogNiagara, Warning, "UNiagaraComponentPool::PooledComponentDestroyed: Invalid pooling mode?");
			break;
	}
	...
	if (GbEnableNiagaraSystemPoolValidation)
	{
		for (auto it=WorldParticleSystemPools.CreateIterator(); it; ++it)
		{
			...
			{
				UE_LOGF(LogNiagara, Warning, "UNiagaraComponentPool::PooledComponentDestroyed: Component existed in a pool that it should not be in?");
			}
		}
	}
...
void UNiagaraComponentPool::Dump()
{
	...
	for (auto& Pair : WorldParticleSystemPools)
	{
		UNiagaraSystem* System = Pair.Key.ResolveObjectPtr();
	...
	UE_LOGF(LogNiagara, Log, "*Particle System Pool Info - Total Mem = %.2fMB*", TotalMemUsage / 1024.0f / 1024.0f);
	UE_LOGF(LogNiagara, Log, "***************************************");
	UE_LOGF(LogNiagara, Log, "%ls", *DumpStr);
	UE_LOGF(LogNiagara, Log, "***************************************");
