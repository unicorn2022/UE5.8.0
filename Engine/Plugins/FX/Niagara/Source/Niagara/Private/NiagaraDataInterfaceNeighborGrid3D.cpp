...
void UNiagaraDataInterfaceNeighborGrid3D::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction &OutFunc)
{
	...
	if (BindingInfo.Name == UNiagaraDataInterfaceRWBase::WorldBBoxSizeFunctionName)
	{
		...
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceNeighborGrid3D::GetWorldBBoxSize);
	}
	else if (BindingInfo.Name == UNiagaraDataInterfaceRWBase::NumCellsFunctionName)
	{
		check(BindingInfo.GetNumInputs() == 1 && BindingInfo.GetNumOutputs() == 3);
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceNeighborGrid3D::GetNumCells);
	}
	else if (BindingInfo.Name == MaxNeighborsPerCellFunctionName)
	{
		check(BindingInfo.GetNumInputs() == 1 && BindingInfo.GetNumOutputs() == 1);
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceNeighborGrid3D::GetMaxNeighborsPerCell);
	}
...
bool UNiagaraDataInterfaceNeighborGrid3D::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	{
		UE_LOGF(LogNiagara, Error, "NiagaraDataInterfaceNeighborGrid3D - Invalid NumCells(%dx%dx%dx%d) with total = %u exceeds the maximum of %d for '%ls'.  Please adjust your NeighborGrid3D or increase fx.MaxNiagaraNeighborGridCells", RT_NumCells.X, RT_NumCells.Y, RT_NumCells.Z, RT_MaxNeighborsPerCell, NumTotalCells, GMaxNiagaraNeighborGridCells, *GetFullNameSafe(this));
		return false;
	}
	else if (NumTotalCells > MaxViewDimensionForTypedBuffer)
	{
		UE_LOGF(LogNiagara, Error, "NiagaraDataInterfaceNeighborGrid3D - Invalid NumCells(%dx%dx%dx%d) with total = %u exceeds the platform maximum of %llu for '%ls'.  Please reduce the memory usage.", RT_NumCells.X, RT_NumCells.Y, RT_NumCells.Z, RT_MaxNeighborsPerCell, NumTotalCells, MaxViewDimensionForTypedBuffer, *GetFullNameSafe(this));
		return false;
	}
...
void UNiagaraDataInterfaceNeighborGrid3D::SetNumCells(FVectorVMExternalFunctionContext& Context)
{
	...
	for (int32 InstanceIdx = 0; InstanceIdx < Context.GetNumInstances(); ++InstanceIdx)
	{
		...
		if (NumTotalCells == 0 || NumTotalCells > uint64(GMaxNiagaraNeighborGridCells))
		{
			UE_LOGF(LogNiagara, Error, "NiagaraDataInterfaceNeighborGrid3D - Invalid NumCells(%dx%dx%dx%d) with total = %u exceeds the maximum of %d for '%ls'.  Please adjust your NeighborGrid3D or increase fx.MaxNiagaraNeighborGridCells", NewNumCellsX, NewNumCellsY, NewNumCellsZ, NewMaxNeighborsPerCell, NumTotalCells, GMaxNiagaraNeighborGridCells, *GetFullNameSafe(this));
			bSuccess = false;
		}
		else if (NumTotalCells > MaxViewDimensionForTypedBuffer)
		{
			UE_LOGF(LogNiagara, Error, "NiagaraDataInterfaceNeighborGrid3D - Invalid NumCells(%dx%dx%dx%d) with total = %u exceeds the platform maximum of %lld for '%ls'.  Please reduce the memory usage.", NewNumCellsX, NewNumCellsY, NewNumCellsZ, NewMaxNeighborsPerCell, NumTotalCells, MaxViewDimensionForTypedBuffer, *GetFullNameSafe(this));
			bSuccess = false;
		}
