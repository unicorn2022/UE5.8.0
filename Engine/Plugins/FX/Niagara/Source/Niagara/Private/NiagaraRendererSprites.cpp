...
void FNiagaraRendererSprites::PrepareParticleSpriteRenderData(FRHICommandListBase& RHICmdList, FParticleSpriteRenderData& ParticleSpriteRenderData, const FSceneViewFamily& ViewFamily, FNiagaraDynamicDataBase* InDynamicData, const FNiagaraSceneProxy* SceneProxy, ENiagaraGpuComputeTickStage::Type GpuReadyTickStage) const
{
	...
	ParticleSpriteRenderData.bCanCastTranslucentShadows = SceneProxy->CastsVolumetricTranslucentShadow() || Material.GetCastDynamicShadowAsMasked();
	...
	bool bLowLatencyTranslucencyEnabled =
		...
		!ParticleSpriteRenderData.bCanCastTranslucentShadows &&
...
void FNiagaraRendererSprites::GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector, const FNiagaraSceneProxy *SceneProxy) const
{
	...
	if (ParticleSpriteRenderData.bHasTranslucentMaterials && AreViewsRenderingOpaqueOnly(Views, VisibilityMap, ParticleSpriteRenderData.bCanCastTranslucentShadows))
	{
		return;
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		if (VisibilityMap & (1 << ViewIndex))
		{
			...
			if (ParticleSpriteRenderData.bHasTranslucentMaterials && IsViewRenderingOpaqueOnly(View, ParticleSpriteRenderData.bCanCastTranslucentShadows))
			{
				continue;
			}
...
void FNiagaraRendererSprites::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector, const FNiagaraSceneProxy* SceneProxy)
{
	...
	if (NumInstances > 0)
	{
...

		{
			...
			UpdateParams.bUsingIndirectDraw = MeshBatch.Elements[0].NumPrimitives == 0;
			UpdateParams.NumVertices = NumVerticesPerInstance * NumInstances;
			UpdateParams.VertexBufferSize = NumVerticesPerInstance * NumInstances * (uint32)sizeof(FVector3f);
			UpdateParams.NumTriangles = NumTrianglesPerInstance * NumInstances;
