...
{
	//Rollback gate for culling more proxyless systems
	bool GAggressiveProxylessCulling = true;
	static FAutoConsoleVariableRef CVarRollback_AggressiveProxylessCulling(
		TEXT("fx.Niagara.Rollback.AggressiveProxylessCulling"),
		GAggressiveProxylessCulling,
		TEXT("When true components without proxies will be more aggresively culled."),
		ECVF_Default
	);
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args, UWorld* World)
		{
			if ( FNiagaraWorldManager* WorldManager = FNiagaraWorldManager::Get(World) )
			{
				if (Args.Num() != 1)
				{
					UE_LOGF(LogNiagara, Log, "fx.Niagara.Debug.PlaybackMode %d", (int32)WorldManager->GetDebugPlaybackMode());
				}
				else
				{
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args, UWorld* World)
		{
			if ( FNiagaraWorldManager* WorldManager = FNiagaraWorldManager::Get(World) )
			{
				if (Args.Num() != 1)
				{
					UE_LOGF(LogNiagara, Log, "fx.Niagara.Debug.PlaybackRate %5.2f", WorldManager->GetDebugPlaybackRate());
				}
				else
				{
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args, UWorld* World)
		{
			if (FNiagaraWorldManager* WorldManager = FNiagaraWorldManager::Get(World))
			{
				...
				{
					UE_LOGF(LogNiagara, Log, "fx.Niagara.ScalabililityCullingMode %d", (int32)WorldManager->GetScalabilityCullingMode());
					UE_LOGF(LogNiagara, Log, "Set scalability culling mode");
					UE_LOGF(LogNiagara, Log, "0 - Enabled. Culling is enabled as normal.");
					UE_LOGF(LogNiagara, Log, "1 - Paused. No culling will occur but FX will still be tracked internally so culling can be resumed correctly later.");
					UE_LOGF(LogNiagara, Log, "2 - Disabled. No culling will occur and no FX will be tracked. Culling may not work correctly for some FX if enabled again after this.");
				}
				else
				{
			...
			{
				UE_LOGF(LogNiagara, Warning, "Cannot set Niagara Scalability Culling Mode on a null world.");
			}
		}
	)
);
...
namespace NiagaraWorldManagerInternal
{
	...
	ETickingGroup GetTickingGroupFromNiagara(int ActualTickGroup)
	{
		const int32 TickGroup = FMath::Clamp(ActualTickGroup + NiagaraFirstTickGroup, 0, TG_MAX - 1);
		return ETickingGroup(TickGroup);
	}
...
FNiagaraWorldManager* FNiagaraWorldManager::Get(const UWorld* World)
{
	...
	if (OutWorld == nullptr)
	{
		UE_LOGF(LogNiagara, Warning, "Calling FNiagaraWorldManager::Get \"%ls\", but Niagara has never encountered this world before. "
...
void FNiagaraWorldManager::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	Collector.AddReferencedObjects(DataManagers);
}
...
FNiagaraSystemSimulationRef FNiagaraWorldManager::GetSystemSimulation(ETickingGroup TickGroup, UNiagaraSystem* System)
{
	...
	if (ActiveNiagaraTickGroup == ActualTickGroup)
	{
		...
		// Note we must change TickGroup to the new tick group in case we need to create a new simulation, otherwise it will have the wrong TickGroup assigned to it
		TickGroup = NiagaraWorldManagerInternal::GetTickingGroupFromNiagara(ActualTickGroup);
	}
	...
	Sim->Init(System, World, false, TickGroup, FNiagaraParameterCollectionResolver(nullptr, System, this));
...
void FNiagaraWorldManager::PostReachabilityAnalysis()
{
	...
	for (TObjectIterator<UNiagaraComponent> ComponentIt; ComponentIt; ++ComponentIt)
	{
		...
		if (IsValid(Component) && !Component->HasAnyFlags(EObjectFlags::RF_ClassDefaultObject))
		{
			FNiagaraUserRedirectionParameterStore& ParameterStore = Component->GetOverrideParameters();
			if (ParameterStore.GetUObjects().Num() > 0)
			{
				ParameterStore.MarkUObjectsDirty();
...
void FNiagaraWorldManager::TickStart(float DeltaSeconds)
{
	...
	{
		UNiagaraDataManager* Manager = Pair.Value.Get();
		if(Manager)
		{
			Manager->BeginFrame();
		}
...
void FNiagaraWorldManager::ExecutePreTickWork(ETickingGroup TickGroup, float DeltaSeconds)
{
	...
	for(auto& Pair : DataManagers)
	{
		...
		if(Manager)
		{
			Manager->PreTick(TickGroup, DeltaSeconds);
		}
	}
...
void FNiagaraWorldManager::CalculateScalabilityState(UNiagaraSystem* System, const FNiagaraSystemScalabilitySettings& ScalabilitySettings, UNiagaraEffectType* EffectType, UNiagaraComponent* Component, bool bIsPreCull, float WorstGlobalBudgetUse, FNiagaraScalabilityState& OutState)
{
	...
	if (GEnableNiagaraVisCulling)
	{
		...
		// If we don't have a scene proxy we can not cull against last render time
		// We also make sure to update the last render time so when the proxy becomes visible we don't get a potential invalid frame before we get data back from the render thread
		float TimeSinceRendered = 0.0f;
		if (Component->GetSceneProxy() || (NiagaraRollbackCVars::GAggressiveProxylessCulling && !Component->ShouldComponentAddToScene()))
		{
			TimeSinceRendered = float(FMath::Max(0.0f, GetWorld()->LastRenderTime - Component->GetLastRenderTime() - World->GetDeltaSeconds() - RenderThreadWorldTimeLag - GLastRenderTimeSafetyBias));
		}
		else
		{
			Component->SetLastRenderTime(GetWorld()->LastRenderTime);
		}
...
{
	UE_LOGF(LogNiagara, Display, "========================================================================");
	UE_LOGF(LogNiagara, Display, "Niagara World Manager Scalability State. 0x%p - %ls", World, *World->GetPathName());
	UE_LOGF(LogNiagara, Display, "========================================================================");
