	...
	for (int AttrNameIdx = 0; AttrNameIdx < ShaderStorage.AttributeNames.Num(); ++AttrNameIdx)
	{
		...
		for (int VarIdx = 0; VarIdx < SourceEmitterVariables.Num(); ++VarIdx)
		{
			...
			if (Var.GetName() == AttrName)
			{
				...
				{
					UE_LOGF(LogNiagara, Error, "Variable '%ls' in emitter '%ls' has type '%ls', but particle read DI tried to access it as '%ls'.",
		...
		if (!FoundVariable)
		{
			UE_LOGF(LogNiagara, Error, "Particle read DI is trying to access inexistent variable '%ls' in emitter '%ls'.", *AttrName.ToString(), *InstanceData->DebugSourceName);
			InstanceData->AttributeIndices[AttrNameIdx] = -1;
			InstanceData->AttributeCompressed[AttrNameIdx] = 0;
		}
	}
...
bool UNiagaraDataInterfaceParticleRead::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	if (PIData->EmitterInstance == nullptr)
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Source emitter '%ls' not found. System: %ls", *EmitterBinding.ResolveUniqueName(this), *GetFullNameSafe(SystemInstance->GetSystem()));
		}
	}
...
void UNiagaraDataInterfaceParticleRead::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction& OutFunc)
{
	...
	{	
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceParticleRead::VMGetLocalSpace);
		return;
	}
	...
	{
		UE_LOGF(LogNiagara, Error, "VMExternalFunction '%ls' does not have a function specifier 'attribute'!", *BindingInfo.Name.ToString());
		return;
	}
	...
	if (!bBindSuccessful)
	{
		...
		UE_LOGF(LogNiagara, Warning, "ParticleRead: Failed to '%ls' attribute '%ls' System '%ls' Emitter '%ls'! Check that the attribute is named correctly.", *BindingInfo.Name.ToString(), *AttributeToRead.ToString(), *GetNameSafe(NiagaraSystem), *EmitterBinding.ResolveUniqueName(this));
	}
}
	...
	if (ComponentType != ENiagaraParticleDataComponentType::Float)
	{
		...
		switch (ComponentType)
		{
			...
			case ENiagaraParticleDataComponentType::ID:
				...
				break;
			default:
				UE_LOGF(LogNiagara, Error, "Unknown component type %d while generating function %ls", int(ComponentType), *FunctionInfo.InstanceName);
				return false;
		}
...
void UNiagaraDataInterfaceParticleRead::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	...
	if (InstanceData->SourceEmitterGPUContext == nullptr)
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "GPU particle read could not find GPU context for '%ls', emitter either doesn't exist or is not a GPU simulation.", *InstanceData->DebugSourceName);
	...
	if (Context.GetComputeInstanceData().Context == InstanceData->SourceEmitterGPUContext)
	{
		...
		if (const FSimulationStageMetaData* StageMetaData = Context.GetSimStageData().StageMetaData)
		{
			if (StageMetaData->bPartialParticleUpdate)
			{
				...
				{
					UE_LOGF(LogNiagara, Error, "Particle read DI reading self '%ls' on stage '%ls' is unsafe, please disable partial writes on the stage.", *InstanceData->DebugSourceName, *StageMetaData->SimulationStageName.ToString());
				}
				SetErrorParams(true);
				return;
			}
	...
	if (InstanceData->bWarnFailedToFindAcquireTag && (InstanceData->AcquireTagRegisterIndex == -1) && Context.IsParameterBound(&ShaderParameters->AcquireTagRegisterIndex) )
	{
		...
		UE_LOGF(LogNiagara, Error, "Particle read DI cannot find ID variable in emitter '%ls'.", *InstanceData->DebugSourceName);
	}
}
