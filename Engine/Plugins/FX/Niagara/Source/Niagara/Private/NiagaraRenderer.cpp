...
bool FNiagaraRenderer::IsRendererEnabled(const UNiagaraRendererProperties* InProperties, const FNiagaraEmitterInstance* Emitter) const
{
	...
	if (Emitter->IsWaitingForPSOCaching())
	{
		return false;
	}
	return true;
...
bool FNiagaraRenderer::IsViewRenderingOpaqueOnly(const FSceneView* View, bool bCanCastTranslucentShadows)
{
	const bool bShadowView = View->GetDynamicMeshElementsShadowCullFrustum() != nullptr;
	return View->CustomRenderPass || (bShadowView && !bCanCastTranslucentShadows);
}

bool FNiagaraRenderer::AreViewsRenderingOpaqueOnly(const TArray<const FSceneView*>& Views, int32 ViewVisibilityMask, bool bCanCastTranslucentShadows)
{
	for ( int32 i=0; i < Views.Num(); ++i )
	{
		if (((ViewVisibilityMask & (1 << i)) != 0) && !IsViewRenderingOpaqueOnly(Views[i], bCanCastTranslucentShadows))
		{
			return false;
		}
	}
...
FVector4f FNiagaraRenderer::CalcMacroUVParameters(const FSceneView& View, FVector MacroUVPosition, float MacroUVRadius)
{
	...
	{
		const FMatrix& ViewProjMatrix = View.ViewMatrices.GetWorldToClip();
