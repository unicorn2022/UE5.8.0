...
struct FNiagaraSimCacheHelper
{
	...
	void ReadDataBufferGPU(float InFrameFraction, float InFrameDeltaSeconds, float InSimDeltaSeconds, const FTransform& InRebaseATransform, const FTransform& InRebaseBTransform, FNiagaraEmitterInstance& EmitterInstance, const FNiagaraSimCacheDataBuffersLayout& InCacheLayout, const FNiagaraSimCacheDataBuffers& InCacheBufferA, const FNiagaraSimCacheDataBuffers& InCacheBufferB, FNiagaraDataSet& InDataSet, std::atomic<int32>& InPendingCommandsCounter)
	{
		...
		GPUExecContext->ParticleCountReadFence.fetch_add(1, std::memory_order_relaxed);
		GPUExecContext->bResetPending_GT = false;

		ENQUEUE_RENDER_COMMAND(NiagaraSimCacheGpuReadFrame)(
			[DispathInterface, GPUExecContext, ParticleCountFence=GPUExecContext->ParticleCountReadFence.load(std::memory_order_relaxed), FrameFraction = InFrameFraction, FrameDeltaSeconds = InFrameDeltaSeconds, SimDeltaSeconds = InSimDeltaSeconds, RebaseATransform = InRebaseATransform, RebaseBTransform = InRebaseBTransform, CacheLayout = &InCacheLayout, CacheBufferA = &InCacheBufferA, CacheBufferB = &InCacheBufferB, DataSet = &InDataSet, PendingCommandsCounter = &InPendingCommandsCounter](FRHICommandListImmediate& RHICmdList)
			{
				const uint32 NumInstances = FMath::Min(CacheBufferA->NumInstances, DataSet->GetMaxAllocationCount());
				...
				FNiagaraDataBuffer& DataBuffer = DataSet->GetCurrentDataChecked();
				{
					...
					GPUExecContext->SetCurrentNumInstances(NumInstances);
					GPUExecContext->ParticleCountWriteFence.store(ParticleCountFence, std::memory_order_relaxed);
