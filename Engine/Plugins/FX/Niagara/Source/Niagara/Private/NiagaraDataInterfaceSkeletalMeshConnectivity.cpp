...
FSkeletalMeshConnectivity::FSkeletalMeshConnectivity(TWeakObjectPtr<USkinnedAsset> InMeshObject, int32 InLodIndex)
	: LodIndex(InLodIndex)
...
bool FSkeletalMeshConnectivity::CanBeUsed(const TWeakObjectPtr<USkinnedAsset>& InMeshObject, int32 InLodIndex) const
{
	return IsUsed() && LodIndex == InLodIndex && MeshObject == InMeshObject;
}

bool FSkeletalMeshConnectivity::IsValidMeshObject(TWeakObjectPtr<USkinnedAsset>& MeshObject, int32 InLodIndex)
{
	USkinnedAsset* Mesh = MeshObject.Get();
...
const FSkeletalMeshLODRenderData* FSkeletalMeshConnectivity::GetLodRenderData(const USkinnedAsset& Mesh, int32 LodIndex)
{
...
{
	if (const USkinnedAsset* Mesh = MeshObject.Get())
	{
	...
	for (TriangleIndexType TriangleIt = 0; TriangleIt < TriangleCount; ++TriangleIt)
	{
		...
		for (int32 i = 0; i < 3; ++i)
		{
			...
			while (InsertionPoint < MaxAdjacencyCount)
			{
				...
				if (TestTriangle == TriangleIndexType(INDEX_NONE))
				{
					...
					|| (!SortBySize && TriangleIt < TestTriangle))
				{
					...
					while (AdjacentTriangles[ShiftIt - 1] == TriangleIndexType(INDEX_NONE))
					{
						--ShiftIt;
					}
...
FSkeletalMeshConnectivityProxy::Initialize(const FSkeletalMeshConnectivity& Connectivity)
{
	if (const FSkeletalMeshLODRenderData* LodRenderData = Connectivity.GetLodRenderData())
	{
		...
		if (!AdjacencySuccess)
		{
			UE_LOGF(LogNiagara, Warning, "Failed to build adjacency for %ls.  Check project settings for NDISkelMesh_AdjacencyTriangleIndexFormat.  Currently using %ls.",
		...
		{
			UE_LOGF(LogNiagara, Warning, "Max adjacency limit of %d exceeded (up to %d found) when processing %ls.  Some connections will be ignored.",
