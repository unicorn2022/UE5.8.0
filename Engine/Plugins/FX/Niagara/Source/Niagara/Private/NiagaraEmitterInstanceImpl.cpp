...
FNiagaraEmitterInstanceImpl::~FNiagaraEmitterInstanceImpl()
{
	...
	//UE_LOGF(LogNiagara, Warning, "~Simulator %p", this);
	CachedBounds.Init();
...
void FNiagaraEmitterInstanceImpl::Dump()const
{
	...
	UE_LOGF(LogNiagara, Log, "==  %ls ========", *VersionedEmitter.Emitter->GetUniqueEmitterName());
	UE_LOGF(LogNiagara, Log, ".................Spawn.................");
	SpawnExecContext.Parameters.DumpParameters(true);
	UE_LOGF(LogNiagara, Log, ".................Update.................");
	...
	{
		UE_LOGF(LogNiagara, Log, "................. %ls Combined Parameters .................", TEXT("GPU Script"));
...
void FNiagaraEmitterInstanceImpl::Init(int32 InEmitterIndex)
{
...

	{
		...
		if (bVerboseAttributeLogging)
		{
			for (const FNiagaraVariableBase& Attr : EmitterData->UpdateScriptProps.Script->GetVMExecutableData().Attributes)
			{
				...
				{
					UE_LOGF(LogNiagara, Warning, "Attribute %ls is used in the Update script for %ls but it is not initialised in the Spawn script!", *Attr.GetName().ToString(), *EmitterHandle.GetName().ToString());
				}
				for (int32 i = 0; i < EmitterData->GetEventHandlers().Num(); i++)
				{
					...
					{
						UE_LOGF(LogNiagara, Warning, "Attribute %ls is used in the event handler script for %ls but it is not initialised in the Spawn script!", *Attr.GetName().ToString(), *EmitterHandle.GetName().ToString());
					}
				}
			}
		}
	...
	UNiagaraSystem* ParentSystem = ParentSystemInstance->GetSystem();

	{
		...
		if (UpdateEventGeneratorCount || SpawnEventGeneratorCount || NumEvents)
		{
			...
			for (int32 i = 0; i < NumEvents; i++)
			{
				...
				EventInstanceData->EventExecContexts[i].Init(ParentSystemInstance, EventScript, ParentSystem->GetScriptRuntimeData(&EmitterHandle, *EventScript));
	...
	{
		SpawnExecContext.Init(ParentSystemInstance, EmitterData->SpawnScriptProps.Script, ParentSystem->GetScriptRuntimeData(&EmitterHandle, *EmitterData->SpawnScriptProps.Script));
		UpdateExecContext.Init(ParentSystemInstance, EmitterData->UpdateScriptProps.Script, ParentSystem->GetScriptRuntimeData(&EmitterHandle, *EmitterData->UpdateScriptProps.Script));
		...
		if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim && EmitterData->GetGPUComputeScript() != nullptr)
		{
			GPUExecContext = new FNiagaraComputeExecutionContext();
			GPUExecContext->InitParams(EmitterData->GetGPUComputeScript(), ParentSystem->GetScriptRuntimeData(&EmitterHandle, *EmitterData->GetGPUComputeScript()), EmitterData->GetSimStageExcecutionData());
...
void FNiagaraEmitterInstanceImpl::ResetSimulation(bool bKillExisting /*= true*/)
{
	...
	if (MinOverallocation > 100 && GbNiagaraShowAllocationWarnings)
	{
		...
		UE_LOGF(LogNiagara, Warning, "The emitter %ls over-allocated %i particles during its runtime. If this happens frequently, consider setting the emitter's AllocationMode property to 'manual' to improve runtime performance.", *FullName, MinOverallocation);
	}
...
void FNiagaraEmitterInstanceImpl::CheckForErrors()
{
	...
	if (!EmitterData->UpdateScriptProps.Script || !EmitterData->SpawnScriptProps.Script )
	{
		...
		UE_LOGF(LogNiagara, Error, "Emitter %ls cannot be enabled because it's doesn't have both an update and spawn script.", *VersionedEmitter.Emitter->GetFullName());
	...
	{
		//TODO - Arbitrary named scripts. Would need some base functionality for Spawn/Udpate to be called that can be overriden in BPs for emitters with custom scripts.
		UE_LOGF(LogNiagara, Error, "Emitter %ls cannot be enabled because it's doesn't have both an update and spawn script ready to run CPU scripts.", *VersionedEmitter.Emitter->GetFullName());
		SetExecutionState(ENiagaraExecutionState::Disabled);
		return;
	}
	...
	{
		UE_LOGF(LogNiagara, Error, "%ls reads attribute data and so cannot be used as a spawn script. The data being read would be invalid.", *EmitterData->SpawnScriptProps.Script->GetName());
		SetExecutionState(ENiagaraExecutionState::Disabled);
		return;
	}
	...
	{
		UE_LOGF(LogNiagara, Error, "This emitter cannot be enabled because its spawn or update script doesn't have any attributes..");
		SetExecutionState(ENiagaraExecutionState::Disabled);
		return;
	}
	...
	if (EmitterData->SimTarget == ENiagaraSimTarget::CPUSim)
	{
		...
		{
			bFailed = true;
			UE_LOGF(LogNiagara, Error, "This emitter cannot be enabled because its CPU Spawn script failed to compile.");
		}

		if (!EmitterData->UpdateScriptProps.Script->DidScriptCompilationSucceed(false))
		{
			bFailed = true;
			UE_LOGF(LogNiagara, Error, "This emitter cannot be enabled because its CPU Update script failed to compile.");
		}
		...
		if (EmitterData->GetEventHandlers().Num() != 0)
		{
			for (int32 i = 0; i < EmitterData->GetEventHandlers().Num(); i++)
			{
				...
				{
					bFailed = true;
					UE_LOGF(LogNiagara, Error, "This emitter cannot be enabled because one of its CPU Event scripts failed to compile.");
				}
			}
		}
	...
	if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim)
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "This emitter cannot be enabled because its GPU script hasn't been compiled..");
			SetExecutionState(ENiagaraExecutionState::Disabled);
			return;
		}
		if (!EmitterData->GetGPUComputeScript()->DidScriptCompilationSucceed(true))
		{
			UE_LOGF(LogNiagara, Error, "This emitter cannot be enabled because its GPU script failed to compile.");
			SetExecutionState(ENiagaraExecutionState::Disabled);
			return;
		}
	}
...
void FNiagaraEmitterInstanceImpl::UpdatePSOStatus()
{
	bWaitingForPSO = false;
	...
	if (FVersionedNiagaraEmitterData* EmitterData = EmitterHandle.GetEmitterData())
	{
		...
		EmitterHandle.ForEachEnabledRendererWithIndex([&PSOPrecacheList, this](const UNiagaraRendererProperties* Properties, int32)
		{
			Properties->CollectPSOPrecacheData(this, PSOPrecacheList);
		});
		for (FMaterialInterfacePSOPrecacheParams& PSOPrecacheParams : PSOPrecacheList)
		{
			if (ParentSystem->IsLoadingPSOCache(PSOPrecacheParams.MaterialInterface))
			{
				bWaitingForPSO = true;
				break;
			}
...
void FNiagaraEmitterInstanceImpl::UnbindParameters(bool bExternalOnly)
{
	...
	{
		const FNiagaraParameterCollectionResolver ParameterCollectionResolver = ParentSystemInstance->CreateParameterCollectionResolver();
		ParameterCollectionResolver.UnbindCollections(SpawnExecContext.Script, SpawnExecContext.Parameters);
		ParameterCollectionResolver.UnbindCollections(UpdateExecContext.Script, UpdateExecContext.Parameters);

		for (FNiagaraScriptExecutionContext& EventContext : GetEventExecutionContexts())
		{
			ParameterCollectionResolver.UnbindCollections(EventContext.Script, EventContext.Parameters);
		}
		...
		if (FVersionedNiagaraEmitterData* EmitterData = VersionedEmitter.GetEmitterData())
		{
			...
			for (const UNiagaraSimulationStageBase* SimStage : EmitterData->GetSimulationStages())
			{
				...
				ParameterCollectionResolver.UnbindCollections(SimStage->Script, TargetParamStore);
			}
		}
...
void FNiagaraEmitterInstanceImpl::BindParameters(bool bExternalOnly)
{
	...
	const FNiagaraParameterCollectionResolver ParameterCollectionResolver = ParentSystemInstance->CreateParameterCollectionResolver();
	ParameterCollectionResolver.BindCollections(SpawnExecContext.Script, SpawnExecContext.Parameters);
	ParameterCollectionResolver.BindCollections(UpdateExecContext.Script, UpdateExecContext.Parameters);

	FVersionedNiagaraEmitterData* EmitterData = VersionedEmitter.GetEmitterData();
	if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim)
	{
		ParameterCollectionResolver.BindCollections(SpawnExecContext.Script, GPUExecContext->CombinedParamStore);
		ParameterCollectionResolver.BindCollections(UpdateExecContext.Script, GPUExecContext->CombinedParamStore);
	}

	for (FNiagaraScriptExecutionContext& EventContext : GetEventExecutionContexts())
	{
		ParameterCollectionResolver.BindCollections(EventContext.Script, EventContext.Parameters);
	}
	...
	for (const UNiagaraSimulationStageBase* SimStage : EmitterData->GetSimulationStages())
	{
		...
		ParameterCollectionResolver.BindCollections(SimStage->Script, TargetParamStore);
	}
...
void FNiagaraEmitterInstanceImpl::RebindParameterCollection(UNiagaraParameterCollectionInstance* OldCollection, UNiagaraParameterCollectionInstance* NewCollection)
{
	if (IsDisabled())
	{
		return;
	...
	const auto RebindToParameterStore =
		[CollectionParent=OldCollection->GetParent(), OldCollection, NewCollection](UNiagaraScript* Script, FNiagaraParameterStore* ParameterStore)
		{
			if (Script && Script->GetCachedParameterCollectionReferences().Contains(CollectionParent))
			{
				OldCollection->GetParameterStore().Unbind(ParameterStore);
				NewCollection->GetParameterStore().Bind(ParameterStore);
			}
		};

	RebindToParameterStore(SpawnExecContext.Script, &SpawnExecContext.Parameters);
	RebindToParameterStore(UpdateExecContext.Script, &UpdateExecContext.Parameters);

	if (SimTarget == ENiagaraSimTarget::GPUComputeSim)
	{
		RebindToParameterStore(SpawnExecContext.Script, &GPUExecContext->CombinedParamStore);
		RebindToParameterStore(UpdateExecContext.Script, &GPUExecContext->CombinedParamStore);
	}

	for (FNiagaraScriptExecutionContext& EventContext : GetEventExecutionContexts())
	{
		RebindToParameterStore(EventContext.Script, &EventContext.Parameters);
	}

	FVersionedNiagaraEmitterData* EmitterData = VersionedEmitter.GetEmitterData();
	FNiagaraScriptInstanceParameterStore& TargetParamStore = SimTarget == ENiagaraSimTarget::GPUComputeSim ? GPUExecContext->CombinedParamStore : UpdateExecContext.Parameters;
	for (const UNiagaraSimulationStageBase* SimStage : EmitterData->GetSimulationStages())
	{
		if (SimStage->bEnabled == false)
		{
		...
		RebindToParameterStore(SimStage->Script, &TargetParamStore);
	}
}
...
bool FNiagaraEmitterInstanceImpl::WaitForDebugInfo()
{
	...
	if (VersionedEmitter.GetEmitterData()->SimTarget == ENiagaraSimTarget::GPUComputeSim && DebugContext)
	{
		...
		{
			UE_LOGF(LogNiagara, Fatal, "Compute interface is invalid and we have a GPU simulation, this may result in a softlock.");
		}
...
void FNiagaraEmitterInstanceImpl::Tick(float DeltaSeconds)
{
	...
	//UE_LOGF(LogNiagara, Warning, "Emitter Tick %f", EmitterAge);
	...
	if (GbDumpParticleData || System->bDumpDebugEmitterInfo)
	{
		...
		UE_LOGF(LogNiagara, Log, "|=== FNiagaraEmitterInstanceImpl::Tick [ %ls ] ===============|", *VersionedEmitter.Emitter->GetPathName());
	}
	...
	if (AllocationSize > MaxAllocationCount)
	{
		...
		if (GbNiagaraShowAllocationWarnings && Estimations >= 5 && ReallocationCount == 3)
		{
			...
			FString FullName = SystemName + "::" + this->GetEmitterHandle().GetName().ToString();
			UE_LOGF(LogNiagara, Warning, "The emitter %ls required many memory reallocation due to changing particle counts. Consider setting the emitter's AllocationMode property to 'manual' to improve runtime performance.", *FullName);
		}
	}
	...
	if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim && GPUExecContext != nullptr)
	{
				...
				for (int32 SpawnInfoIdx = 0; SpawnInfoIdx < SpawnInfos.Num(); SpawnInfoIdx++)
				{
					...
					if (Info.Count > 0 && (NumSpawnInfos < NIAGARA_MAX_GPU_SPAWN_INFOS))
					{
						...
						if (NumSpawnedOnGPUThisFrame > MaxParticlesSpawnedPerFrame)
						{
							...
							UE_LOGF(LogNiagara, Warning, "%ls", *DebugMsg);
		...
		if (GbDumpParticleData || System->bDumpDebugEmitterInfo)
		{
			...
			UE_LOGF(LogNiagara, Log, ".................Update.................");
			UpdateExecContext.Parameters.DumpParameters(true);
			UE_LOGF(LogNiagara, Log, "................. %ls Combined Parameters (%d Spawned ).................", TEXT("GPU Script"), SpawnTotal);
			GPUExecContext->CombinedParamStore.DumpParameters();
		}
	...
	{
		UE_LOGF(LogNiagara, Error, "NiagaraEmitterInstance: DestinationData is nullptr.");
		UE_LOGF(LogNiagara, Error, "ExecutionState(%d) TickCount(%d) TotalSpawnedParticles(%d) ParentSystemInstance(%p)", int(ExecutionState), TickCount, TotalSpawnedParticles, ParentSystemInstance);
		if (ParentSystemInstance)
		{
			...
			UE_LOGF(LogNiagara, Error, "SystemInstanceState(%d) AttachComponent(%p=%ls) NiagaraSystem(%p=%ls)",
	...
	if (SpawnTotal + EventSpawnTotal > 0)
	{
		...
		auto SpawnParticles = [&](int32 Num, const TCHAR* DumpLabel)
		{
			...
			if (EventInstanceData.IsValid())
			{
				//UE_LOGF(LogNiagara, Log, "SpawnScriptEventDataSets: %d", SpawnScriptEventDataSets.Num());
				int32 EventDataSetIdx = 1;
				for (FNiagaraDataSet* EventDataSet : EventInstanceData->SpawnScriptEventDataSets)
				{
					//UE_LOGF(LogNiagara, Log, "SpawnScriptEventDataSets.. %d", EventDataSet->GetNumVariables());
			...
			if (GbDumpParticleData || System->bDumpDebugEmitterInfo)
			{
				...
				//UE_LOGF(LogNiagara, Log, "=== %ls Spawn Parameters ===", *DumpLabel);
	...
	{
		UE_LOGF(LogNiagara, Log, "Skipping spawning due to execution state! %d", (uint32)ExecutionState)
	}*/
				...
				for (int32 i = 0; i < Info.SpawnCounts.Num(); i++)
				{
					...
					if (EventNumToSpawn > 0)
					{
						...
						if (GbDumpParticleData || System->bDumpDebugEmitterInfo)
						{
							...
							//UE_LOGF(LogNiagara, Log, "=== Event %d Parameters ===", EventScriptIdx);
...
// 						UE_LOGF(LogNiagara, Log, "=== Event %d Src Parameters ===", EventScriptIdx);
// 						EventHandlerProps.Script->RapidIterationParameters.Dump();
// 						UE_LOGF(LogNiagara, Log, "=== Event %d Context Parameters ===", EventScriptIdx);
// 						EventExecContexts[EventScriptIdx].Parameters.Dump();
// 						UE_LOGF(LogNiagara, Log, "=== Event %d Particles (%d index written, %d total) ===", EventScriptIdx, Index, Data.GetNumInstances());
...
// 		}
	...
	if (GbDumpParticleData || System->bDumpDebugEmitterInfo)
	{
		UE_LOGF(LogNiagara, Log, "|=== END OF FNiagaraEmitterInstanceImpl::Tick [ %ls ] ===============|", *VersionedEmitter.Emitter->GetPathName());
		UE_LOGF(LogNiagara, Log, "|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
	}
...
void FNiagaraEmitterInstanceImpl::SetExecutionState(ENiagaraExecutionState InState)
{
	/*if (InState != ExecutionState)
	{
		...
		UE_LOGF(LogNiagara, Log, "Emitter \"%ls\" change state: %ls to %ls", *GetEmitterHandle().GetName().ToString(), *EnumPtr->GetNameStringByValue((int64)ExecutionState),
	...
	{
		UE_LOGF(LogNiagara, Log, "Emitter \"%ls\" change state N O O O O O ", *GetEmitterHandle().GetName().ToString());
	}*/
