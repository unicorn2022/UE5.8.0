...
void FNiagaraDataChannelDebugUtilities::LogWrite(const FNiagaraDataChannelPublishRequest& WriteRequest, const UNiagaraDataChannel* DataChannel, const ETickingGroup& TickGroup)
{
	...
	if (NDCCVars::LogWritesToOutputLog > 0)
	{
		...
		UE_LOGF(LogNiagara, Log, "Frame %llu, TG %ls, NDC write by %ls (BP[%ls]/CPU[%ls]/GPU[%ls]): %i entries to data channel %ls %ls%ls"
			, GFrameCounter
...
void FNiagaraDataChannelDebugUtilities::DumpAllWritesToLog()
{
	...
	{
		UE_LOGF(LogNiagara, Log, "No writes are currently stored in the log. fx.Niagara.DataChannels.FrameDataToCapture = %i", NDCCVars::FrameDataToCapture);
		return;
	}
	UE_LOGF(LogNiagara, Log, "Current Frame is %llu, logging data from oldest to newest:", GFrameCounter);
	for (const FFrameDebugData& FrameData : Debugger.FrameData)
	{
		UE_LOGF(LogNiagara, Log, "Frame %llu: %i entries", FrameData.FrameNumber, FrameData.WriteRequests.Num());
		for (const FChannelWriteRequest& Request : FrameData.WriteRequests)
		{
			FString DataString = ToJson(Request.Data.Get());
			UE_LOGF(LogNiagara, Log, "Write by %ls (BP[%ls]/CPU[%ls]/GPU[%ls], TG %ls): %i entries to data channel %ls \n%ls"
			, *Request.DebugSource
