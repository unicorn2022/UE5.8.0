...
void FParticlePerfStatsListener_NiagaraPerformanceReporter::ReportToLog()
{
	...
	auto PrintHeader = [](FString HeaderText, bool bWithStatHeadings=true)
	{
		...
		UE_LOGF(LogNiagara, Log, "| %ls |", *HeaderText);
		UE_LOGF(LogNiagara, Log, "|----------------------------------------------------------|");
	};

	UE_LOGF(LogNiagara, Log, "|================================================================|");
	UE_LOGF(LogNiagara, Log, "| Niagara Performance Report |");
	UE_LOGF(LogNiagara, Log, "| Stats for each system are gathered in short tests of %u frames. |", GPerfTestFrames);
	UE_LOGF(LogNiagara, Log, "| Total Tests: %u |", TotalTests);
	UE_LOGF(LogNiagara, Log, "|================================================================|");
	...
	if(BadPerfSystems.Num() > 0)
	{
		...
		{
			UE_LOGF(LogNiagara, Log, "| Test %u | Start Time: %g | End Time: %g ", TestInfo.TestIndex, TestInfo.StartTime, TestInfo.EndTime); 
			...
			{			
				...
				UE_LOGF(LogNiagara, Log, "| Game Thread | Num: %4llu | Avg: %6u | Max: %6u | AvgRatio: %g | Max Ratio: %g |", GT.AccumulatedStats.NumInstances, (uint32)GT.GetPerInstanceAvg(), (uint32)GT.GetPerInstanceMax(GTMaxIndex), Ratio.PerInstanceAvg_GT, Ratio.PerInstanceMax_GT);
				UE_LOGF(LogNiagara, Log, "| | Tick            | Total: %5u | Avg: %5llu |", TickGameThreadTime, TickGameThreadTime / GT.AccumulatedStats.NumInstances);
				UE_LOGF(LogNiagara, Log, "| | Tick Concurrent | Total: %5u | Avg: %5llu |", TickConcurrentTime, TickConcurrentTime / GT.AccumulatedStats.NumInstances);
				UE_LOGF(LogNiagara, Log, "| | Finalize        | Total: %5u | Avg: %5llu |", FinalizeTime, FinalizeTime / GT.AccumulatedStats.NumInstances);
				UE_LOGF(LogNiagara, Log, "| | EOF Update      | Total: %5u | Avg: %5llu |", EndOfFrameTime, EndOfFrameTime / GT.AccumulatedStats.NumInstances);
			}
			...
			{		
				...
				UE_LOGF(LogNiagara, Log, "| Render Thread | Num: %4llu | Avg: %6u | Max: %6u | AvgRatio: %g | Max Ratio: %g |", RT.AccumulatedStats.NumInstances, RTAvg, RTMax, Ratio.PerInstanceAvg_RT, Ratio.PerInstanceMax_RT);
				UE_LOGF(LogNiagara, Log, "| | Update | Total: %5u | Avg: %5llu |", RenderUpdateTime, RenderUpdateTime / RT.AccumulatedStats.NumInstances);
				UE_LOGF(LogNiagara, Log, "| | GDME   | Total: %5u | Avg: %5llu |", GDMETime, GDMETime / RT.AccumulatedStats.NumInstances);
			}
		};
		...
		{
			UE_LOGF(LogNiagara, Log, "|================================================================|");
			UE_LOGF(LogNiagara, Log, "| System: %ls | Total Bad Tests: %u", *Item.System->GetName(), Item.Stats->BadTestHistory.Num());
			...
			UE_LOGF(LogNiagara, Log, "| Baseline: %ls | GT Avg: %5u | GT Max: %5u | RT Avg: %5u | RT Max: %5u |", *Controller->GetSystem()->GetName(), (uint32)Baseline.PerInstanceAvg_GT, (uint32)Baseline.PerInstanceMax_GT, (uint32)Baseline.PerInstanceAvg_RT, (uint32)Baseline.PerInstanceMax_RT);
			UE_LOGF(LogNiagara, Log, "|----------------------------------------------------------------|");
	...
	if (InvalidBaselineSystems.Num() > 0)
	{
		...
		{
			UE_LOGF(LogNiagara, Log, "| System: %ls | FXType: %ls |", *Item.System->GetName(), *Item.System->GetEffectType()->GetName());
		}
	}
	...
	if (NoFXTypeSystems.Num() > 0)
	{
		...
		{
			UE_LOGF(LogNiagara, Log, "| System: %ls |", *Item.System->GetName());
		}
	}
	UE_LOGF(LogNiagara, Log, "|================================================================|");
}
			...
			if (UNiagaraEffectType* FXType = System->GetEffectType())
			{
				if (FXType->IsPerfBaselineValid())
				{
					...
					if (ComparisonResult == FNiagaraPerfBaselineStats::EComparisonResult::Poor || ComparisonResult == FNiagaraPerfBaselineStats::EComparisonResult::Bad)
					{
						...
						UE_LOGF(LogNiagara, Warning, "%ls", *Message);
