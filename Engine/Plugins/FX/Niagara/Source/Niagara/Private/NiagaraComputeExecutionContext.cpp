...
void FNiagaraComputeExecutionContext::InitParams(UNiagaraScript* InGPUComputeScript, TSharedPtr<const FNiagaraScriptRuntimeData> InScriptRuntimeData, const FNiagaraSimStageExecutionDataPtr& InSimStageExecData)
{
	if (InGPUComputeScript == nullptr || InScriptRuntimeData.IsValid() == false)
	{
		return;
	}
	...
	CombinedParamStore.InitFromOwningContext(InGPUComputeScript, ScriptRuntimeData.Get(), true);
...
{
	if (DIProxy && !DIProxy->GetSourceDIName().IsNone())
	{		
		return SimStageExecData->SimStageMetaData[SimulationStageIndex].OutputDestinations.Contains(DIProxy->GetSourceDIName());
	}
	return false;
}
...
bool FNiagaraComputeExecutionContext::IsInputStage(FNiagaraDataInterfaceProxy* DIProxy, uint32 SimulationStageIndex) const
{
	if (DIProxy && !DIProxy->GetSourceDIName().IsNone())
	{
		return SimStageExecData->SimStageMetaData[SimulationStageIndex].InputDataInterfaces.Contains(DIProxy->GetSourceDIName());
	}
	return false;
}
...
bool FNiagaraComputeExecutionContext::IsIterationStage(FNiagaraDataInterfaceProxy* DIProxy, uint32 SimulationStageIndex) const
{
	if (DIProxy && !DIProxy->GetSourceDIName().IsNone())
	{
		return SimStageExecData->SimStageMetaData[SimulationStageIndex].IterationSourceType == ENiagaraIterationSource::DataInterface && (SimStageExecData->SimStageMetaData[SimulationStageIndex].IterationDataInterface == DIProxy->GetSourceDIName());
	}
	return false;
}
...
FNiagaraDataInterfaceProxyRW* FNiagaraComputeExecutionContext::FindIterationInterface(const TArray<FNiagaraDataInterfaceProxyRW*>& InProxies, uint32 SimulationStageIndex) const
{
	...
	{
		if (Proxy->GetSourceDIName() == SimStageExecData->SimStageMetaData[SimulationStageIndex].IterationDataInterface)
		{
			return Proxy;
		}
	}

	UE_LOGF(LogNiagara, Verbose, "FNiagaraComputeExecutionContext::FindIterationInterface could not find IterationInterface %ls", *SimStageExecData->SimStageMetaData[SimulationStageIndex].IterationDataInterface.ToString());
...
bool FNiagaraComputeExecutionContext::Tick(FNiagaraSystemInstance* ParentSystemInstance)
{
	...
	if (CombinedParamStore.GetInterfacesDirty())
	{
		...
		if (bHasMismatchedDIs)
		{
			UE_LOGF(LogNiagara, Error, "Niagara GPU Execution Context Mismatch with DataInterfaces. System (%ls) will not run", *GetNameSafe(ParentSystemInstance->GetSystem()));
			...
			for (int32 i = 0; i < MaxDataInterfaces; ++i)
			{
				...
				UE_LOGF(LogNiagara, Error, " - DI(%d) SourceType(%ls) ExpectedType(%ls) %ls", i, *SourceType.ToString(), *ExpectedType.ToString(), ErrorType);
			}
...
{
	if (GPUScript && ScriptRuntimeData.IsValid())
	{
		...
		if (VMData.IsValid() && VMData.bNeedsGPUContextInit)
		{
			...
			for (int32 i = 0; i < DataInterfaces.Num(); i++)
			{
				...
				if (UserPtrIdx != INDEX_NONE)
				{
					...
					{
						Interface->GPUContextInit(ScriptDataInterfaceInfos[i], VMData.CalledVMExternalFunctions, InstData, ParentSystemInstance);
					}
				}
			}
		}
