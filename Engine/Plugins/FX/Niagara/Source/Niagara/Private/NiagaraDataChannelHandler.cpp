...
void UNiagaraDataChannelHandler::Init(const UNiagaraDataChannel* InChannel)
{
	...
	CachedLayoutInfo = DataChannel->GetLayoutInfo();
}
...
void UNiagaraDataChannelHandler::OnDataChannelLayoutChanged()
{
	if(const UNiagaraDataChannel* NDC = DataChannel.Get())
	{
		CachedLayoutInfo = NDC->GetLayoutInfo();
	}
	else
	{
		CachedLayoutInfo.Reset();
	}
}
