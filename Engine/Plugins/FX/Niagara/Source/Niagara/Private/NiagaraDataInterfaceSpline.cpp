...
void UNiagaraDataInterfaceSpline::DrawDebugHud(FNDIDrawDebugHudContext& DebugHudContext) const
{
	const FNDISpline_InstanceData* InstanceData_GT = DebugHudContext.GetSystemInstance()->FindTypedDataInterfaceInstanceData<FNDISpline_InstanceData>(this);
	if (InstanceData_GT == nullptr)
	{
		return;
	}

	USplineComponent* SplineComponent = InstanceData_GT->Component.Get();
	AActor* SceneActor = SplineComponent ? SplineComponent->GetAttachmentRootActor() : nullptr;
	DebugHudContext.GetOutputString().Appendf(TEXT("SplineComponent(%s:%s)"), SceneActor ? *SceneActor->GetActorNameOrLabel() : TEXT("None"), *GetNameSafe(SplineComponent));

	if (!DebugHudContext.IsVerbose() || SplineComponent == nullptr)
	{
		return;
	}
	...
	auto DrawDebugSpline =
		[Canvas](int32 NumPoints, TFunction<FVector(int)> GetPoint, const FColor& Color)
		{
			TOptional<FVector> LastScreenPosition;
			for (int32 i = 0; i < NumPoints; ++i)
			{
				const FVector WorldPosition = GetPoint(i);
				const FVector ScreenPosition = Canvas->Project(WorldPosition, false);
				if (ScreenPosition.Z > 0.0f)
				{
					if (LastScreenPosition.IsSet())
					{
						FCanvasLineItem LineItem(FVector2D(ScreenPosition), FVector2D(LastScreenPosition.GetValue()));
						LineItem.SetColor(Color);
						Canvas->Canvas->DrawItem(LineItem);
					}

					Canvas->Canvas->DrawNGon(FVector2D(ScreenPosition), Color, 8, 4.0f);
					LastScreenPosition.Emplace(ScreenPosition);
				}
				else
				{
					LastScreenPosition.Reset();
				}
			}
		};
	...
	DrawDebugSpline(
		...
		[SplineComponent](int32 PointIndex)
		{
			const FSplinePoint SplinePoint = SplineComponent->GetSplinePointAt(PointIndex, ESplineCoordinateSpace::World);
	...
	{
		DrawDebugSpline(
			InstanceData_GT->SplineLUT.Positions.Num(),
			[InstanceData_GT, Transform=SplineComponent->GetComponentTransform()](int32 PointIndex)
			{
				const FVector Position = InstanceData_GT->SplineLUT.Positions[PointIndex];
...
bool UNiagaraDataInterfaceSpline::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	InstData->UserParamBinding.Init(SystemInstance->GetInstanceParameters(), SplineUserParameter.Parameter);
...
USplineComponent* UNiagaraDataInterfaceSpline::ResolveSplineComponent(FNiagaraSystemInstance* SystemInstance, FNDISpline_InstanceData* InstanceData) const
{
	const bool bTryParameterBinding = SourceMode == ENDISpline_SourceMode::Default || SourceMode == ENDISpline_SourceMode::ParameterBindingOnly;
	const bool bTrySourceActor = SourceMode == ENDISpline_SourceMode::Default;
	const bool bTryAttachParent = SourceMode == ENDISpline_SourceMode::Default || SourceMode == ENDISpline_SourceMode::AttachParentOnly;

	// Parameter binding
	UObject* ParameterBindingValue = InstanceData->UserParamBinding.GetValue();
	if (bTryParameterBinding && ParameterBindingValue)
	{
		if (USplineComponent* BoundComponent = Cast<USplineComponent>(ParameterBindingValue))
		{
		...
		else if (AActor* Actor = Cast<AActor>(ParameterBindingValue))
		{
			if (USplineComponent* ActorComponent = Actor->FindComponentByClass<USplineComponent>())
			{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Spline data interface has an invalid object parameter type. Only USplineComponent or AActor are supported.");
			UE_LOGF(LogNiagara, Warning, "Invalid Parameter : %ls", *ParameterBindingValue->GetFullName());
			UE_LOGF(LogNiagara, Warning, "Niagara Component : %ls", *GetFullNameSafe(SystemInstance ? Cast<UNiagaraComponent>(SystemInstance->GetAttachComponent()) : nullptr));
			UE_LOGF(LogNiagara, Warning, "System : %ls", *GetFullNameSafe(SystemInstance ? SystemInstance->GetSystem() : nullptr));
		}
	}
	...
	if (bTrySourceActor)
	{
		if (AActor* SourceActor = SoftSourceActor.Get())
		{
			return SourceActor->FindComponentByClass<USplineComponent>();
		}
	}

	// Attach parent
	if (bTryAttachParent && SystemInstance)
	{
		if (USceneComponent* AttachComp = SystemInstance->GetAttachComponent())
		{
...
{
	check(SystemInstance && PerInstanceData);
	FNDISpline_InstanceData* InstData = static_cast<FNDISpline_InstanceData*>(PerInstanceData);

	// Reolve the spline parameter
	USplineComponent* SplineComponent = nullptr;
	if (SourceMode == ENDISpline_SourceMode::Default)
	{
		if (SplineUserParameter.Parameter.IsValid())
		{
			if (InstData->CachedUserParam && InstData->UserParamBinding.GetValue() != InstData->CachedUserParam)
			{
				return true;
			}
		}

		SplineComponent = InstData->Component.Get();
		if (SplineComponent == nullptr)
		{
			SplineComponent = ResolveSplineComponent(SystemInstance, InstData);
	...
	// Handle the parameter changing
	if (SplineComponent != InstData->Component.Get())
	{
		InstData->Component = SplineComponent;
		InstData->SplineCurvesVersion.Reset();
	}
	...
	if (SplineComponent != nullptr)
	{
		...
		if (!InstData->SplineCurvesVersion.IsSet() || (InstData->SplineCurvesVersion != SplineComponent->GetVersion()))
		{
...
void FNiagaraDataInterfaceSplineLUT::BuildLUT(const FSplineCurves& SplineCurves, int32 NumSteps)
{
	...
	InvSplineDistanceStep = SplineDistanceStep > 0.0f ? 1.0f / SplineDistanceStep : 0.0f;
...
float FNDISpline_InstanceData::EvaluateFindNearestPosition<TIntegralConstant<bool, true>>(FVector InPosition) const
{
	...
	const int32 NumPointsMinusOne = SplineLUT.Positions.Num() - 1;
	return KeyToNearest > 0 && NumPointsMinusOne > 0 ? float(KeyToNearest) / float(NumPointsMinusOne) : 0.0f;
}
...
void UNiagaraDataInterfaceSpline::FindClosestUnitDistanceFromPositionWS(FVectorVMExternalFunctionContext& Context)
{
	...
	if (InstData->IsValid())
	{
		...
		for (int32 i = 0; i < Context.GetNumInstances(); ++i)
		{
			...
			const float UnitDistance = FinalKeyTime > 0.0f ? KeyTime / FinalKeyTime : 0.0f;
