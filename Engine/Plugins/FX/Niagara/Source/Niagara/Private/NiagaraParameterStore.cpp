...
bool FNiagaraParameterStoreBinding::VerifyBinding(const FNiagaraParameterStore* DestStore, const FNiagaraParameterStore* SrcStore)const
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "Invalid ParameterStore Binding: Parameters missing from binding between %ls and %ls. Stores should have been rebound when one changed!", *SrcStore->DebugName, *DestStore->DebugName);
		for (FName MissingParam : MissingParameterNames)
		{
			UE_LOGF(LogNiagara, Warning, "%ls", *MissingParam.ToString());
		}
	}
#endif
...
void FNiagaraParameterStoreBinding::Dump(const FNiagaraParameterStore* DestStore, const FNiagaraParameterStore* SrcStore)const
{
	...
	for (const FParameterBinding& Binding : ParameterBindings)
	{
		...
		if (ensure(bFound))
		{
			...
			{
				UE_LOGF(LogNiagara, Log, "Parameter in dest store has incorrect data!\n");
			}
		}
		else
		{
			UE_LOGF(LogNiagara, Log, "Failed to find matching param in bound store!\n");
		}
	}
	...
	for (const FInterfaceBinding& Binding : InterfaceBindings)
	{
		...
		if (ensure(bFound))
		{
			...
			{
				UE_LOGF(LogNiagara, Log, "Data interface parameter in dest store is incorrect!\n");
			}
		}
		else
		{
			UE_LOGF(LogNiagara, Log, "Failed to find matching data interface param in bound store!\n");
		}
	}
	...
	for (const FUObjectBinding& Binding : UObjectBindings)
	{
		...
		if (ensure(bFound))
		{
			UE_LOGF(LogNiagara, Log, "| UObject | %ls %ls: Src:%d - Dest:%d\n", *Param.GetType().GetName(), *Param.GetName().ToString(), Binding.SrcOffset, Binding.DestOffset);
...
bool FNiagaraParameterStore::VerifyBinding(const FNiagaraParameterStore* DestStore)const
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "Invalid ParameterStore Binding: %ls was not bound to %ls.", *DebugName, *DestStore->DebugName);
	}
...
{
	//UE_LOGF(LogNiagara, Log, "FNiagaraParameterStore::UnUnbindFromSourceStoresbind() - Src: 0x%p - SrcName: %ls", this, *DebugName);
...
void FNiagaraParameterStore::DumpParameters(bool bDumpBindings) const
{
	for (const FNiagaraVariableWithOffset& VariableBase : ReadParameterVariables())
	{
		...
		UE_LOGF(LogNiagara, Log, "Param: %ls Offset: %d Type : %ls", *Var.ToString(), IndexOf(Var), *Var.GetType().GetName());
	}
...
bool FNiagaraParameterStore::AddParameter(const FNiagaraVariable& Param, bool bInitInterfaces, bool bTriggerRebind, int32* OutOffset)
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "AddParameter: ParameterOffsets.Num() should be 0 is %d, please investigate for %ls.", ParameterOffsets.Num() , *GetPathNameSafe(Owner.Get()));
	}
...
bool FNiagaraParameterStore::RemoveParameter(const FNiagaraVariableBase& ToRemove)
{
	...
	if (ParameterOffsets.Num())
	{
		UE_LOGF(LogNiagara, Warning, "RemoveParameter: ParameterOffsets.Num() should be 0 is %d, please investigate for %ls", ParameterOffsets.Num(), *GetPathNameSafe(Owner.Get()));
	}
	//check(!ParameterOffsets.Num()); // Migration to SortedParameterOffsets
#endif
	...
	if (IndexOf(ToRemove) != INDEX_NONE)
	{
		...
		for (const FNiagaraVariableWithOffset& Existing : ReadParameterVariables())
		{
			...
			if (static_cast<const FNiagaraVariableBase&>(ExistingVar) != ToRemove)
			{
				...
				{
					UE_LOGF(LogNiagara, Warning, "RemoveParameter: Encountered invalid variable in parameter store (%ls) -> variable: %ls", *GetPathNameSafe(Owner.Get()), *ExistingVar.GetName().ToString());
				}
			}
		}
...
void FNiagaraParameterStore::RenameParameter(const FNiagaraVariableBase& Param, FName NewName)
{
	...
	if (ParameterOffsets.Num())
	{
		UE_LOGF(LogNiagara, Warning, "RenameParameter: ParameterOffsets.Num() should be 0 is %d, please investigate for %ls", ParameterOffsets.Num(), *GetPathNameSafe(Owner.Get()));
	}
	//check(!ParameterOffsets.Num()); // Migration to SortedParameterOffsets
#endif
	...
	if(Idx != INDEX_NONE)
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Ignored attempt to rename a parameter overtop of an existing parameter!  Old name: %ls, New name: %ls", *Param.GetName().ToString(), *NewName.ToString());
		}
	}
}
...
void FNiagaraParameterStore::SanityCheckData(bool bInitInterfaces)
{
		...
		if (Parameter.IsValid())
		{
			if (Parameter.IsDataInterface())
			{
				if (DataInterfaces.Num() <= SrcIndex)
				{
					...
					UE_LOGF(LogNiagara, Verbose, "Missing data interfaces! Had to add %d data interface entries to ParameterStore on %ls", NewNum , Owner != nullptr ? *Owner->GetPathName() : TEXT("Unknown owner"));
				...
				if (DataInterfaces[SrcIndex] == nullptr && bInitInterfaces && Owner.IsValid())
				{
					...
					UE_LOGF(LogNiagara, Verbose, "Had to initialize data interface! %ls on %ls", *Parameter.GetName().ToString(), Owner != nullptr ? *Owner->GetPathName() : TEXT("Unknown owner"));
			...
			else if (Parameter.IsUObject())
			{
				if (UObjects.Num() <= SrcIndex)
				{
					...
					UE_LOGF(LogNiagara, Verbose, "Missing UObject interfaces! Had to add %d UObject entries for %ls on %ls", NewNum , *Parameter.GetName().ToString(), Owner != nullptr ? *Owner->GetPathName() : TEXT("Unknown owner"));
			...
			else
			{
				...
				{
					UE_LOGF(LogNiagara, Verbose, "Missing parameter data! %ls on %ls", *Parameter.GetName().ToString(), Owner != nullptr ? *Owner->GetPathName() : TEXT("Unknown owner"));
	...
	{
		UE_LOGF(LogNiagara, Verbose, "%ls needs to be resaved to prevent above warnings due to the parameter state being stale.", *Owner->GetFullName());
	}
}
...
void FNiagaraParameterStore::CopyParametersTo(FNiagaraParameterStore& DestStore, bool bOnlyAdd, EDataInterfaceCopyMethod DataInterfaceCopyMethod) const
{
	for (const FNiagaraVariableWithOffset& Parameter : ReadParameterVariables())
	{
		...
		if (Parameter.IsValid() == false)
		{
			...
			UE_LOGF(LogNiagara, Error, "Invalid parameter found while attempting to copy parameters from one parameter store to another.  Parameter Store: %ls Parameter Name: %ls Parameter Type: %ls", 
...
void FNiagaraParameterStore::InitFromSource(const FNiagaraParameterStore* SrcStore, bool bNotifyAsDirty)
{
	...
	if (ParameterOffsets.Num())
	{
		UE_LOGF(LogNiagara, Warning, "InitFromSource: ParameterOffsets.Num() should be 0 is %d, please investigate for %ls", ParameterOffsets.Num(), *GetPathNameSafe(Owner.Get()));
	}
#endif // WITH_EDITORONLY_DATA
...
void FNiagaraParameterStore::SetParameter_InternalUseOnly(const FNiagaraVariableBase& Key, const FNiagaraVariant& Value, bool bAdd)
{	
	if (Key.IsDataInterface())
	{
		UE_LOGF(LogNiagara, Error, "We do not support setting data interfaces in SetParameter_InternalUseOnly() currently.");
		return;
	}
	...
	if (bAdd)
	{
		bRebind = AddParameter(Key, false, false);
	}

	if (Key.IsUObject())
	{
		SetUObject(Value.GetUObject(), Key);
	}
	else if (Key.GetType() == FNiagaraTypeDefinition::GetPositionDef())
	{
		FVector WorldPos = FVector::ZeroVector;
		if (Value.IsValid(FNiagaraTypeHelper::GetVectorDef()))
		{			
			WorldPos = Value.GetBytesValue<FVector>(FNiagaraTypeHelper::GetVectorDef());
		}
		else if(Value.IsValid(FNiagaraTypeDefinition::GetVec3Def()))
		{
			WorldPos = FVector(Value.GetBytesValue<FVector3f>(FNiagaraTypeDefinition::GetVec3Def()));
		}
		else
		{
			UE_LOGF(LogNiagara, Error, "Malformed Variant for setting position data. Parameter was set to default/origin.");
		}

		SetPositionParameterValue(WorldPos, Key.GetName(), bAdd);
	}
	else if(Value.IsValid(Key.GetType()))
	{
		SetParameterData(Value.GetBytes(), Key, bAdd);
	}
...
void FNiagaraParameterStore::GetParameter_InternalUseOnly(const FNiagaraVariableBase& Key, FNiagaraVariant& Value)const
{
	Value.Reset();
	if (Key.IsDataInterface())
	{
		...
		if (Index != INDEX_NONE)
		{
			Value.SetDataInterface(GetDataInterface(Index));
		}
	}
	else if (Key.IsUObject())
	{
		...
		if (Index != INDEX_NONE)
		{
			Value.SetUObject(GetUObject(Index));
		}
	}
	else if (Key.GetType() == FNiagaraTypeDefinition::GetPositionDef())
	{
		FVector PosValue(FVector::ZeroVector);
		if (const FVector* PosData = GetPositionParameterValue(Key.GetName()))
		{
			PosValue = *PosData;
			Value.SetBytesValue(FNiagaraTypeHelper::GetVectorDef(), PosValue);
		}
	}
	else
	{
		if (const uint8* Data = GetParameterData(Key))
		{
			Value.SetBytes(Data, Key.GetSizeInBytes());
		}
	}
}
	...
	if (DestIndex != INDEX_NONE && SrcIndex != INDEX_NONE)
	{
		if (SourceParameter.IsDataInterface())
		{
			...
			if (DestDataInterface == nullptr)
			{
				...
				{
					UE_LOGF(LogNiagara, Warning, "While trying to copy parameter data the destination data interface was null, creating a new one.  Parameter: %ls Destination Store Owner: %ls", *SourceParameter.GetName().ToString(), *GetPathNameSafe(DestStore.Owner.Get()));
...
const int32* FNiagaraParameterStore::FindParameterOffset(const FNiagaraVariableBase& Parameter, bool IgnoreType) const
{
	...
	if (ParameterOffsets.Num())
	{
		UE_LOGF(LogNiagara, Warning, "FindParameterOffset: ParameterOffsets.Num() should be 0 is %d, please investigate for %ls", ParameterOffsets.Num(), *GetPathNameSafe(Owner.Get()));
	}
	//check(!ParameterOffsets.Num()); // Migration to SortedParameterOffsets
#endif
