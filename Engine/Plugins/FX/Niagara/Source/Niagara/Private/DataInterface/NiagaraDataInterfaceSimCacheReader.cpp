...
namespace NDISimCacheReaderLocal
{
	...
	static const FName GetInstanceIndexFromID("GetInstanceIndexFromID");
	...
	static FNiagaraTypeDefinition FunctionNameToTypeDef(FName FunctionName)
	{
		...
		if (FunctionName == GetInstanceIndexFromID)
		{
			return FNiagaraTypeDefinition::GetIDDef();
		}
...
void UNiagaraDataInterfaceSimCacheReader::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
	...
	{
		FNiagaraFunctionSignature& Sig = OutFunctions.Add_GetRef(DefaultSignature);
		Sig.Name = GetInstanceIndexFromID;
		Sig.Inputs.Emplace(FNiagaraTypeDefinition::GetIntDef(), TEXT("FrameIndex"));
		Sig.Inputs.Emplace(FNiagaraTypeDefinition::GetIDDef(), TEXT("ID"));
		Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetBoolDef(), TEXT("Valid"));
		Sig.Outputs.Emplace(FNiagaraTypeDefinition::GetIntDef(), TEXT("InstanceIndex"));
		Sig.FunctionSpecifiers.Emplace(NAME_Attribute);
		Sig.bExperimental = true;
	#if WITH_EDITORONLY_DATA
		Sig.ExperimentalMessage = LOCTEXT("GetInstanceIndexFromIDExperimental", "This functions is experimental and not optimal as it may perform a linear search.");
	#endif
		Sig.SetDescription(LOCTEXT("GetInstanceIndexFromIDDesc", "Returns the Instance Index if the ID was found otherwise -1."));
	}
}
#endif
...
void UNiagaraDataInterfaceSimCacheReader::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* PerInstanceData, FVMExternalFunction& OutFunc)
{
	...
	{
		const int32 AttributeIndex = AttributeIndexFromVariable(InstanceSimCache, BindingInfo.Name, BindingInfo.FunctionSpecifiers[0].Value, InstanceData->EmitterIndex);
		OutFunc = FVMExternalFunction::CreateUObject(this, &UNiagaraDataInterfaceSimCacheReader::VMGetInstanceID, AttributeIndex);
	}
	else if (BindingInfo.Name == GetInstanceIndexFromID)
	{
		const int32 AttributeIndex = AttributeIndexFromVariable(InstanceSimCache, BindingInfo.Name, BindingInfo.FunctionSpecifiers[0].Value, InstanceData->EmitterIndex);
		OutFunc = FVMExternalFunction::CreateUObject(this, &UNiagaraDataInterfaceSimCacheReader::VMGetInstanceIndexFromID, AttributeIndex);
	}
...
bool UNiagaraDataInterfaceSimCacheReader::GetFunctionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, FString& OutHLSL)
{
	...
	if (FunctionInfo.DefinitionName == GetInstanceIndexFromID)
	{
		OutHLSL.Appendf(TEXT("void %s(int FrameIndex, NiagaraID ID, out bool bValid, out int Value)\n"), *FunctionInfo.InstanceName);
		OutHLSL.Append(TEXT("{\n"));
		OutHLSL.Appendf(TEXT("	int AttributeIndex = %s_ComponentOffsets[%d][%d];\n"), *ParamInfo.DataInterfaceHLSLSymbol, Float4Index, ElementIndex);
		OutHLSL.Appendf(TEXT("	GetInstanceIndexFromID_%s(FrameIndex, %s_EmitterIndex, AttributeIndex, ID, bValid, Value);\n"), *ParamInfo.DataInterfaceHLSLSymbol, *ParamInfo.DataInterfaceHLSLSymbol);
		OutHLSL.Append(TEXT("}\n"));
		return true;
	}
...
void UNiagaraDataInterfaceSimCacheReader::VMGetInstanceID(FVectorVMExternalFunctionContext& Context, int AttributeIndex)
{
	using namespace NDISimCacheReaderLocal;
...
void UNiagaraDataInterfaceSimCacheReader::VMGetInstanceIndexFromID(FVectorVMExternalFunctionContext& Context, int AttributeIndex)
{
	using namespace NDISimCacheReaderLocal;

	VectorVM::FUserPtrHandler<FInstanceData_GameThread> InstanceData_GT(Context);
	FNDIInputParam<int32>		InFrameIndex(Context);
	FNDIInputParam<FNiagaraID>	InID(Context);
	FNDIOutputParam<bool>		OutValid(Context);
	...
	if (UNiagaraSimCache* InstanceSimCache = InstanceData_GT->WeakSimCache.Get())
	{
		...
		{
			const int32 FrameIndex = InFrameIndex.GetAndAdvance();
			const FNiagaraID ID = InID.GetAndAdvance();
			...
			{
				for (int32 j = 0; j < AttributeReader.GetNumInstances(); ++j)
				{
					if (AttributeReader.ReadID(j) == ID)
					{
						InstanceIndex = j;
						break;
					}
			...
			OutValid.SetAndAdvance(InstanceIndex != INDEX_NONE);
			OutValue.SetAndAdvance(InstanceIndex);
		}
	}
	else
	{
		...
		{
			OutValid.SetAndAdvance(false);
			OutValue.SetAndAdvance(INDEX_NONE);
		}
	}
}
