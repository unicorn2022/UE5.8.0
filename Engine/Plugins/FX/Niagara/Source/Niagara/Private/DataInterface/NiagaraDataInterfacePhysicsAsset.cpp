	...
	FNDIPhysicsAssetArrays* OutAssetArrays, const FTransform& InWorldTransform, const FTransform& LocalTransform)
{
	if (OutAssetArrays != nullptr)
	{
		...
		{
			UE_LOGF(LogPhysicsAsset, Warning, "Number of physics asset primitives is higher than the niagara %d limit", PHYSICS_ASSET_MAX_PRIMITIVES);
		}
	}
}
...
void FNDIPhysicsAssetProxy::ConsumePerInstanceDataFromGameThread(void* PerInstanceData, const FNiagaraSystemInstanceID& Instance)
{
	...
	{
		UE_LOGF(LogPhysicsAsset, Log, "ConsumePerInstanceDataFromGameThread() ... could not find %lld", Instance);
	}
...
void UNiagaraDataInterfacePhysicsAsset::ExtractSourceComponent(FNiagaraSystemInstance* SystemInstance, FTransform& BoneTransform)
{
	...
	if (MeshUserParameter.Parameter.IsValid())
	{
		...
		if (UserParamObject)
		{
			...
			else
			{
				...
				UE_LOGF(LogPhysicsAsset, Warning, "SkeletalMesh data interface using object parameter with invalid type. Skeletal Mesh Data Interfaces can only get a valid mesh from SkeletalMeshComponents, SkeletalMeshActors or Actors.");
				UE_LOGF(LogPhysicsAsset, Warning, "Invalid Parameter : %ls", *UserParamObject->GetFullName());
				UE_LOGF(LogPhysicsAsset, Warning, "System : %ls", *GetFullNameSafe(SystemInstance->GetSystem()));
			}
		}
		else
		{
