...
namespace NDIStaticMeshLocal
{
	...
	struct FInstanceData_RenderThread
	{
		...
		void Init(FRHICommandListBase& RHICmdList, FGpuInitializeData& GpuInitializeData)
		{
			...
			if (GpuInitializeData.LODResource)
			{
				...
				{
#if DO_CHECK
					UE_LOGF(LogNiagara, Log, "NiagaraStaticMeshDataInterface used by GPU emitter but does not have SRV access on this platform.  Enable CPU access to fix this issue. System: %ls, Mesh: %ls", *GpuInitializeData.SystemFName.ToString(), *GpuInitializeData.StaticMeshFName.ToString());
#endif
	...
	struct FInstanceData_GameThread
	{
		...
		bool CalculateLODIndex(UNiagaraDataInterfaceStaticMesh* Interface, FNiagaraSystemInstance* Instance, UStaticMesh* StaticMesh, int32& OutLOD, int32& OutMinLOD)
		{
			...
			OutLOD = FMath::Clamp(OutLOD, MinSampleLOD, MaxSampleLOD);
		...
		bool Init(UNiagaraDataInterfaceStaticMesh* Interface, FNiagaraSystemInstance* SystemInstance)
		{
			...
			{
				UE_LOGF(LogNiagara, Log, "StaticMesh data interface aborting because \"fx.Niagara.FailStaticMeshDataInterface 1\". Failed InitPerInstanceData - %ls", *Interface->GetFullName());
				return false;
			}
			...
			{
				UE_LOGF(LogNiagara, Log, "StaticMesh data interface has no valid mesh - %ls", *Interface->GetFullName());
			}
			else if (StaticMesh->HasValidNaniteData() && !StaticMesh->HasNaniteFallbackMesh(ShaderPlatform))
			{
				UE_LOGF(LogNiagara, Warning, "NiagaraStaticMeshDataInterface does not have a fallback mesh. Enable Nanite fallback mesh to fix this issue. System: %ls, Mesh: %ls", *GetFullNameSafe(SystemInstance->GetSystem()), *GetFullNameSafe(StaticMesh));
				StaticMesh = nullptr;
			}
			else if (!StaticMesh->bAllowCPUAccess)
			{
				...
				if (Interface->IsUsedWithGPUScript() && !Interface->IsUsedWithCPUScript())
				{
					...
					{
						UE_LOGF(LogNiagara, Log, "NiagaraStaticMeshDataInterface used by GPU emitter but does not have SRV access on this platform.  Enable CPU access to fix this issue. System: %ls, Mesh: %ls", *GetFullNameSafe(SystemInstance->GetSystem()), *GetFullNameSafe(StaticMesh));
						StaticMesh = nullptr;
					}
				}
				else
				{
					...
					if (bNeedsCpuAccess)
					{
						...
						{
							UE_LOGF(LogNiagara, Log, "NiagaraStaticMeshDataInterface used by CPU emitter and does not allow CPU access. System: %ls, Mesh: %ls", *GetFullNameSafe(SystemInstance->GetSystem()), *GetFullNameSafe(StaticMesh));
						}
						StaticMesh = nullptr;
					}
				}
			...
			if (StaticMesh != nullptr)
			{
				...
				if (NumMeshSockets > 0)
				{
					...
					{
						UE_LOGF(LogNiagara, Warning, "Static Mesh Data Interface '%ls' requires more sockets '%d' than we currently support '%d' Mesh '%ls' Component '%ls'", *GetFullNameSafe(Interface), NumMeshSockets, TNumericLimits<uint16>::Max(), *GetFullNameSafe(StaticMesh), *GetFullNameSafe(Component));
						return false;
					}
					...
					if (Interface->FilteredSockets.Num() > 0)
					{
						...
						for (int32 i = 0; i < Interface->FilteredSockets.Num(); ++i)
						{
							...
							{
								UE_LOGF(LogNiagara, Warning, "Static Mesh Data Interface '%ls' could not find socket '%ls' Mesh '%ls' Component '%ls'", *GetFullNameSafe(Interface), *FilteredSocketName.ToString(), *GetFullNameSafe(StaticMesh), *GetFullNameSafe(Component));
								continue;
							}
							++NumFilteredSockets;
...
void UNiagaraDataInterfaceStaticMesh::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction& OutFunc)
{
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMIsValidVertex);
	}
	else if (BindingInfo.Name == RandomVertexName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomVertex);
	}
	else if (BindingInfo.Name == GetVertexCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertexCount);
	}
	//else if (BindingInfo.Name == IsValidFilteredVertexName)
	//{
	//	OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMIsValidFilteredVertex);
	//}
	//else if (BindingInfo.Name == RandomFilteredVertexName)
	//{
	//	OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomFilteredVertexAt);
	//}
	//else if (BindingInfo.Name == GetFilteredVertexCountName)
	//{
	//	OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredVertexCount);
	//}
	//else if (BindingInfo.Name == GetFilteredVertexAtName)
	//{
	//	OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredVertexAt);
	//}
	else if (BindingInfo.Name == GetVertexName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertex<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == GetVertexWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertex<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetVertexWSInterpolatedName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertexInterpolated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetVertexColorName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertexColor);
	}
	else if (BindingInfo.Name == GetVertexUVName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertexUV);
	}
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMIsValidTriangle);
	}
	else if (BindingInfo.Name == RandomTriangleName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomTriangle<FNDIRandomHelper>);
	}
	else if (BindingInfo.Name == GetTriangleCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleCount);
	}
	else if (BindingInfo.Name == IsValidFilteredTriangleName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMIsValidFilteredTriangle);
	}
	else if (BindingInfo.Name == RandomFilteredTriangleName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomFilteredTriangle);
	}
	else if (BindingInfo.Name == GetFilteredTriangleCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredTriangleCount);
	}
	else if (BindingInfo.Name == GetFilteredTriangleAtName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredTriangleAt);
	}
	else if (BindingInfo.Name == IsValidUnfilteredTriangleName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMIsValidUnfilteredTriangle);
	}
	else if (BindingInfo.Name == RandomUnfilteredTriangleName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomUnfilteredTriangle);
	}
	else if (BindingInfo.Name == GetUnfilteredTriangleCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredTriangleCount);
	}
	else if (BindingInfo.Name == GetUnfilteredTriangleAtName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredTriangleAt);
	}
	else if (BindingInfo.Name == GetTriangleName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangle<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == GetTriangleWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangle<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetTriangleWSInterpolatedName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleInterpolated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetTriangleColorName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleColor);
	}
	else if (BindingInfo.Name == GetTriangleUVName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleUV);
	}
	else if (BindingInfo.Name == GetTriangleIndicesName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleIndices);
	}
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetSocketCount);
	}
	else if (BindingInfo.Name == GetFilteredSocketCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredSocketCount);
	}
	else if (BindingInfo.Name == GetUnfilteredSocketCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredSocketCount);
	}
	else if (BindingInfo.Name == RandomSocketName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomSocket);
	}
	else if (BindingInfo.Name == RandomFilteredSocketName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomFilteredSocket);
	}
	else if (BindingInfo.Name == RandomUnfilteredSocketName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomUnfilteredSocket);
	}
	else if (BindingInfo.Name == GetSocketTransformName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetSocketTransform<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == GetSocketTransformWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetSocketTransform<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetSocketTransformWSInterpolatedName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetSocketTransformInterpolated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetFilteredSocketTransformName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredSocketTransform<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == GetFilteredSocketTransformWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredSocketTransform<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetFilteredSocketTransformWSInterpolatedName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredSocketTransformInterpolated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetUnfilteredSocketTransformName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredSocketTransform<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == GetUnfilteredSocketTransformWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredSocketTransform<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetUnfilteredSocketTransformWSInterpolatedName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredSocketTransformInterpolated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == GetFilteredSocketName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredSocket);
	}
	else if (BindingInfo.Name == GetUnfilteredSocketName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredSocket);
	}
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMIsValidSection);
	}
	else if (BindingInfo.Name == GetSectionTriangleCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetSectionTriangleCount);
	}
	else if (BindingInfo.Name == RandomSectionTriangleName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomSectionTriangle<FNDIRandomHelper>);
	}
	else if (BindingInfo.Name == GetSectionTriangleAtName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetSectionTriangleAt);
	}
	else if (BindingInfo.Name == GetFilteredSectionAtName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredSectionAt);
	}
	else if (BindingInfo.Name == GetUnfilteredSectionAtName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredSectionAt);
	}
	else if (BindingInfo.Name == GetSectionCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetSectionCount);
	}
	else if (BindingInfo.Name == GetFilteredSectionCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetFilteredSectionCount);
	}
	else if (BindingInfo.Name == GetUnfilteredSectionCountName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetUnfilteredSectionCount);
	}
	else if (BindingInfo.Name == RandomSectionName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomSection<FNDIRandomHelper>);
	}
	else if (BindingInfo.Name == RandomFilteredSectionName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomFilteredSection<FNDIRandomHelper>);
	}
	else if (BindingInfo.Name == RandomUnfilteredSectionName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomUnfilteredSection);
	}
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMIsValid);
	}
	else if (BindingInfo.Name == GetPreSkinnedLocalBoundsName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetPreSkinnedLocalBounds);
	}
	else if (BindingInfo.Name == GetMeshBoundsName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetMeshBounds<true>);
	}
	else if (BindingInfo.Name == GetMeshBoundsWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetMeshBounds<false>);
	}
	else if (BindingInfo.Name == GetLocalToWorldName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetLocalToWorld);
	}
	else if (BindingInfo.Name == GetLocalToWorldInverseTransposedName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetLocalToWorldInverseTransposed);
	}
	else if (BindingInfo.Name == GetWorldVelocityName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetWorldVelocity);
	}
	else if (BindingInfo.Name == GetInstanceIndexName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetInstanceIndex);
	}
	else if (BindingInfo.Name == SetInstanceIndexName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMSetInstanceIndex);
	}
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleCoordAtUV);
	}
	else if (BindingInfo.Name == GetTriangleCoordInAabbName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleCoordInAabb);
	}
	else if (BindingInfo.Name == BuildUvMappingName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMBuildUvMapping);
	}
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomFilteredSection<FNDIRandomHelperFromStream>);
	}
	else if (BindingInfo.Name == Deprecated_RandomTriCoordName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomTriangle<FNDIRandomHelperFromStream>);
	}
	else if (BindingInfo.Name == Deprecated_RandomTriCoordOnSectionName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMRandomSectionTriangle<FNDIRandomHelperFromStream>);
	}
	else if (BindingInfo.Name == Deprecated_GetVertexPositionName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertexPosition_Deprecated<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == Deprecated_GetVertexPositionWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetVertexPosition_Deprecated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == Deprecated_GetTriPositionName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriPosition_Deprecated<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == Deprecated_GetTriPositionWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriPosition_Deprecated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == Deprecated_GetTriTangentsName)
	{	
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleTangentBasis_Deprecated<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == Deprecated_GetTriTangentsWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleTangentBasis_Deprecated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == Deprecated_GetTriNormalName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleNormal_Deprecated<FNDITransformHandlerNoop>);
	}
	else if (BindingInfo.Name == Deprecated_GetTriNormalWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriangleNormal_Deprecated<FNDITransformHandler>);
	}
	else if (BindingInfo.Name == Deprecated_GetTriPositionAndVelocityWSName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceStaticMesh::VMGetTriPositionAndVelocity_Deprecated<FNDITransformHandler>);
	}
}
...
void UNiagaraDataInterfaceStaticMesh::GetFeedback(UNiagaraSystem* Asset, UNiagaraComponent* Component, TArray<FNiagaraDataInterfaceError>& OutErrors, TArray<FNiagaraDataInterfaceFeedback>& OutWarnings, TArray<FNiagaraDataInterfaceFeedback>& OutInfo)
{
	...
	if (SourceMode == ENDIStaticMesh_SourceMode::Default && DefaultMesh != nullptr)
	{
		OutWarnings.Emplace(
...
void UNiagaraDataInterfaceStaticMesh::VMGetMeshBounds(FVectorVMExternalFunctionContext& Context)
{
	...
	const FVector3f HalfExtents = bLocalSpace ? FVector3f(MeshBounds.GetExtent()) : FVector3f(WorldBounds.GetExtent());
	const FVector3f Extents = HalfExtents * 2.0f;
	const FVector3f ExtentsMin = Center - HalfExtents;
	const FVector3f ExtentsMax = Center + HalfExtents;
