...
struct FNDIDebugDrawInstanceData_GameThread
{
	...
	{
		if (NumHeightSegments == 0 || NumRadiusSegments == 0)
		{
			return;
		}
	...
	struct FDebugPrim_PersistentShape
	{
		...
		void TransformVector(bool bVectorWasSet, FVector3f& Vector, const TOptional<ENiagaraCoordinateSpace>& SourceSpace, ENiagaraCoordinateSpace DestSpace, const FNiagaraSystemInstance* SystemInstance)
		{
			...
			{
				UE_LOGF(LogNiagara, Warning, "Unknown space!");
			}
		}
...
bool UNiagaraDataInterfaceDebugDraw::GPUContextInit(const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDataInterfaceData, const TArray<FVMExternalFunctionBindingInfo>& CalledFunctions, void* PerInstanceData, FNiagaraSystemInstance* SystemInstance) const
{
	...
	for (const FVMExternalFunctionBindingInfo& Sig : CalledFunctions)
	{
		if (Sig.OwnerName == ResolvedDataInterfaceData.CompileName && Sig.FunctionSpecifiers.Num() > 0)
		{
			// The HLSL translator adds this function specifier in so that we have a unqiue key during compilation.
			const FVMFunctionSpecifier* SpecifierPtr = Sig.FindSpecifier(NDIDebugDrawLocal::CompileTagKey);
			const FName* Specifier = SpecifierPtr != nullptr ? &SpecifierPtr->Value : nullptr;
