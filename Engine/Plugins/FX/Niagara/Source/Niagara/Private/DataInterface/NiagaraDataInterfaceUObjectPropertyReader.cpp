				...
				#define NDI_PROPERTY_TYPE(TYPE) \
					...
					{ \
						...
						{ \
							...
							UE_LOGF(LogNiagara, Warning, "Could convert return property for function '%ls' type '%ls' into expected Niagara type '%ls'", *PropertyGetter.Variable.GetName().ToString(), *PropertyType, *PropertyGetter.Variable.GetType().GetName()); \
						} \
					} \
			...
			#define NDI_PROPERTY_TYPE(TYPE) \
				...
				{ \
					...
					{ \
						...
						UE_LOGF(LogNiagara, Warning, "Could not copy property '%ls' type '%ls' into expected Niagara type '%ls'", *Property->GetName(), *PropertyType, *PropertyGetter.Variable.GetType().GetName()); \
					} \
				} \
	...
	if ( InstanceData_GT->WeakUObject.Get() != ObjectBinding )
	{
		...
		for (FNDIPropertyGetter& PropertyGetter : InstanceData_GT->PropertyGetters)
		{
			...
			if ( ObjectBinding != nullptr )
			{
				...
				if (PropertyGetter.PropertyCopyFunction == nullptr && FNiagaraUtilities::LogVerboseWarnings())
				{
					UE_LOGF(LogNiagara, Warning, "Could not find property '%ls' inside object '%ls' or component '%ls'", *PropertyGetter.Variable.GetName().ToString(), *GetNameSafe(ObjectBinding), *GetNameSafe(ActorComponent));
				}
			}
		}
	}
...
void UNiagaraDataInterfaceUObjectPropertyReader::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* PerInstanceData, FVMExternalFunction& OutFunc)
{
	...
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceUObjectPropertyReader::VMGetComponentTransform);
	}
	else if (BindingInfo.Name == GetComponentInvTransformName)
	{
		OutFunc.BindUObject(this, &UNiagaraDataInterfaceUObjectPropertyReader::VMGetComponentInvTransform);
	}
	...
	#define NDI_PROPERTY_TYPE(TYPE) \
		...
		{ \
			...
			OutFunc.BindStatic(&FTypeHelper<TYPE>::VMFunction, GetterIndex); \
		} \
