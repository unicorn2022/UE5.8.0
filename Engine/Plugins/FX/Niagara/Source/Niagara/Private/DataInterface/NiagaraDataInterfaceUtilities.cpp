...
void ForEachGpuFunctionImpl(class UNiagaraDataInterface* RuntimeResolvedDataInterface, const class UNiagaraSystem* NiagaraSystem, const FNiagaraParameterStore& ParameterStore, TFunction<bool(const UNiagaraScript*, const FNiagaraDataInterfaceGeneratedFunction&)> Action)
{
	...
	NiagaraSystem->ForEachScriptWithRuntimeData(
		[&](const UNiagaraScript* Script, const FNiagaraScriptRuntimeData* ScriptRuntimeData)
		{
			...
			TConstArrayView<FNiagaraScriptResolvedDataInterfaceInfo> ResolvedDIs = ScriptRuntimeData->GetResolvedDataInterfaces();
...
void FNiagaraDataInterfaceUtilities::ForEachDataInterface(const UNiagaraSystem* NiagaraSystem, TFunction<bool(const FDataInterfaceUsageContext&)> Action)
{
	...
	NiagaraSystem->ForEachScriptWithOwningContext([&Action](const UNiagaraSystem* OwningSystem, const FNiagaraEmitterHandle* OwningEmitterHandle, UNiagaraScript* Script)
	{
		if (Script == nullptr)
		{
			return;
		...
		{
			EmitterHandleId = OwningEmitterHandle->GetId();
			UsageContext.OwnerObject = OwningEmitterHandle->GetInstance().Emitter;
		}
		else
		{
			EmitterHandleId = FGuid();
			UsageContext.OwnerObject = OwningSystem;
		}
		...
		ForEachDataInterface(UsageContext, *ScriptRuntimeData->GetExecutionReadyParameterStore(), Action);
	});
}