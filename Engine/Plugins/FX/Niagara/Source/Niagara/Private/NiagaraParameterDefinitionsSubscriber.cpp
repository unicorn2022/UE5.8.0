...
namespace NiagaraParameterDefinitionsSubscriberPrivate
{
	FDelegateHandle OnAssetRegistryFilesLoaded;
	TArray<TPair<TWeakObjectPtr<UObject>, INiagaraParameterDefinitionsSubscriber::FDeferredSyncFunction>> DeferredSyncFunctions;
	...
	void OnFilesLoaded()
	{
		...
		for (const auto& DeferredSync : DeferredSyncFunctions)
		{
			if (UObject* Owner = DeferredSync.Key.Get())
			{
				for (INiagaraParameterDefinitionsSubscriber* Subscriber : DeferredSync.Value(Owner))
				{
					Subscriber->SynchronizeWithParameterDefinitions(Args);
				}
			}
		}
		DeferredSyncFunctions.Empty();

		FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
		AssetRegistryModule.Get().OnFilesLoaded().Remove(OnAssetRegistryFilesLoaded);
		OnAssetRegistryFilesLoaded.Reset();
	}

	void AddDeferredSync(TWeakObjectPtr<UObject> WeakOwner, INiagaraParameterDefinitionsSubscriber::FDeferredSyncFunction Function)
	{
		check(IsInGameThread());
		if (OnAssetRegistryFilesLoaded.IsValid() == false)
		{
			FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
			OnAssetRegistryFilesLoaded = AssetRegistryModule.Get().OnFilesLoaded().AddStatic(&OnFilesLoaded);
		}

		for (const auto& DeferredSync : DeferredSyncFunctions)
		{
			if (DeferredSync.Key == WeakOwner)
			{
				return;
			}
		}

		DeferredSyncFunctions.Emplace(WeakOwner, Function);
	}
}

void INiagaraParameterDefinitionsSubscriber::PostLoadDefinitionsSubscriptions(TWeakObjectPtr<UObject> WeakOwner, TFunction<TArray<INiagaraParameterDefinitionsSubscriber*>(UObject*)> DeferredSync)
{
	...
	if (bAllParameterDefinitionsDiscovered == false)
	{
		...
		NiagaraParameterDefinitionsSubscriberPrivate::AddDeferredSync(WeakOwner, DeferredSync);
	}
...
void INiagaraParameterDefinitionsSubscriber::SynchronizeWithParameterDefinitions(const FSynchronizeWithParameterDefinitionsArgs Args /*= FSynchronizeWithParameterDefinitionsArgs()*/)
{
	...
	if (AssetRegistryModule.Get().IsLoadingAssets() == false)
	{
		...
		for (const FAssetData& ParameterDefinitionsAssetDatum : ParameterDefinitionsAssetData)
		{
			...
			{
				UE_LOGF(LogNiagara, Warning, "Failed to load parameter definition %ls from asset registry!", *ParameterDefinitionsAssetDatum.GetSoftObjectPath().ToString());
				continue;
			}
