...
bool FNiagaraSimCacheJson::DumpToFile(const UNiagaraSimCache& SimCache, const FString& FullPath, EExportType ExportType)
{
	if (ExportType == EExportType::SingleJsonFile)
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Writing file to %ls", *FullPath);
...
bool FNiagaraSimCacheJson::DumpFramesToFolder(const UNiagaraSimCache& SimCache, const FString& TargetFolder)
{
	...
	{
		UE_LOGF(LogNiagaraSimCacheJson, Warning, "Unable to save to folder %ls", *TargetFolder);
		return false;
	}
...

	{
		...
		{
			UE_LOGF(LogNiagaraSimCacheJson, Warning, "Unable to export system data for cache %ls", *SimCache.GetName());
			return false;
		}
	...
	for (int FrameIndex = 0; FrameIndex < SimCache.GetNumFrames(); FrameIndex++)
	{
		...
		{
			UE_LOGF(LogNiagaraSimCacheJson, Warning, "Export cancelled by user");
			return false;
		}
		...
		for (int EmitterIndex = 0; EmitterIndex < EmitterCount; EmitterIndex++)
		{
			...
			{
				UE_LOGF(LogNiagaraSimCacheJson, Warning, "Unable to export data for emitter %ls", *SimCache.GetEmitterName(EmitterIndex).ToString());
				return false;
			}
