...
const FName UNiagaraDataInterfaceGrid3DCollection::GetOutputGridFloatValueFunctionName("GetOutputGridFloatValue");
const FName UNiagaraDataInterfaceGrid3DCollection::GetOutputGridVector2ValueFunctionName("GetOutputGridVector2Value");
const FName UNiagaraDataInterfaceGrid3DCollection::GetOutputGridVectorValueFunctionName("GetOutputGridVectorValue");
const FName UNiagaraDataInterfaceGrid3DCollection::GetOutputGridVector4ValueFunctionName("GetOutputGridVector4Value");
...
bool UNiagaraDataInterfaceGrid3DCollection::CanCreateVarFromFuncName(const FName& FuncName)
{
	if (
		...
		FuncName == UNiagaraDataInterfaceGrid3DCollection::GetOutputGridVector4ValueFunctionName)
		...
		FuncName == UNiagaraDataInterfaceGrid3DCollection::GetOutputGridVectorValueFunctionName)
		return true;
	else if (
		...
		FuncName == UNiagaraDataInterfaceGrid3DCollection::GetOutputGridVector2ValueFunctionName)
		return true;
	else if (
		...
		FuncName == UNiagaraDataInterfaceGrid3DCollection::GetOutputGridFloatValueFunctionName)
		return true;
	return false;
}
...
void UNiagaraDataInterfaceGrid3DCollection::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
	...
	// GetOutputGrid* functions - read from OutputGrid UAV (destination buffer) for in-place iterative solvers (e.g. RBGS-SOR).
	// NOTE: bWriteFunction is set to true even though these functions only READ from the OutputGrid. This is intentional:
	// bWriteFunction adds the DI to OutputDestinations, which makes IsOutputStage()=true so PreStage/PostStage fire.
	// Crucially, bReadFunction is NOT set, so the DI is NOT added to InputDataInterfaces, meaning IsInputStage()=false.
	// This causes BeginSimulate to receive RequiresBuffering=false, which sets DestinationData=CurrentData (single buffer).
	// If we used bReadFunction instead, it would force double-buffering, defeating the purpose of these functions.
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetOutputGridFloatValueFunctionName;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Grid")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexX")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexY")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexZ")));
		...
		Sig.bSupportsGPU = true;
		Sig.bWriteFunction = true;
		Sig.Description = NSLOCTEXT("Niagara", "NiagaraDataInterfaceGridColl3D_GetOutputGridFloat", "Gets a float value from the current OutputGrid (destination buffer) by Attribute name. Use for in-place iterative solvers like Red-Black Gauss-Seidel.");
		OutFunctions.Add(Sig);
	}

	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetOutputGridVector2ValueFunctionName;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Grid")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexX")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexY")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexZ")));
		...
		Sig.bSupportsGPU = true;
		Sig.bWriteFunction = true;
		Sig.Description = NSLOCTEXT("Niagara", "NiagaraDataInterfaceGridColl3D_GetOutputGridVector2", "Gets a Vector2 value from the current OutputGrid (destination buffer) by Attribute name. Use for in-place iterative solvers like Red-Black Gauss-Seidel.");
		OutFunctions.Add(Sig);
	}

	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetOutputGridVectorValueFunctionName;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Grid")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexX")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexY")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexZ")));
		...
		Sig.bSupportsGPU = true;
		Sig.bWriteFunction = true;
		Sig.Description = NSLOCTEXT("Niagara", "NiagaraDataInterfaceGridColl3D_GetOutputGridVector", "Gets a Vector3 value from the current OutputGrid (destination buffer) by Attribute name. Use for in-place iterative solvers like Red-Black Gauss-Seidel.");
		OutFunctions.Add(Sig);
	}

	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetOutputGridVector4ValueFunctionName;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Grid")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexX")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexY")));
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("IndexZ")));
		...
		Sig.bSupportsGPU = true;
		Sig.bWriteFunction = true;
		Sig.Description = NSLOCTEXT("Niagara", "NiagaraDataInterfaceGridColl3D_GetOutputGridVector4", "Gets a Vector4 value from the current OutputGrid (destination buffer) by Attribute name. Use for in-place iterative solvers like Red-Black Gauss-Seidel.");
		OutFunctions.Add(Sig);
	}
...
void UNiagaraDataInterfaceGrid3DCollection::WriteGetOutputGridHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, int32 InNumChannels, FString& OutHLSL)
{
	// Like WriteGetHLSL but reads from OutputGrid UAV instead of Grid SRV
	FString FormatBounds = TEXT(R"(
			void {FunctionName}(int In_IndexX, int In_IndexY, int In_IndexZ, out float{NumChannelsVariableSuffix} Out_Val)
			{
				int In_AttributeIndex = {AttributeIndicesName}[{AttributeIndexGroup}]{AttributeIndexComponent};
			    ...
			    for (int i = 0; i < {NumChannels}; i++)
				{
					int CurAttributeIndex = In_AttributeIndex + i;
					int3 TileOffset = {PerAttributeDataName}[CurAttributeIndex].xyz;
					float Val = {OutputGrid}[int3(In_IndexX + TileOffset.x, In_IndexY + TileOffset.y, In_IndexZ + TileOffset.z)];
					)");
	if (InNumChannels == 1)
	{
		FormatBounds += TEXT("					Out_Val = Val;\n");
...
bool UNiagaraDataInterfaceGrid3DCollection::GetFunctionHLSL(const FNiagaraDataInterfaceGPUParamInfo& ParamInfo, const FNiagaraDataInterfaceGeneratedFunction& FunctionInfo, int FunctionInstanceIndex, FString& OutHLSL)
{
	...
	*/
	...
	else if (FunctionInfo.DefinitionName == GetOutputGridVector4ValueFunctionName)
	{
		if (AttributeWriter.UseRGBAGrid())
		{
			AttributeWriter.WriteGetOutputGridHLSL(EGridAttributeRetrievalMode::RGBAGrid, FunctionInfo, OutHLSL);
		}
		else
		{
			WriteGetOutputGridHLSL(ParamInfo, FunctionInfo, FunctionInstanceIndex, 4, OutHLSL);
		}
		return true;
	}
	else if (FunctionInfo.DefinitionName == GetOutputGridVectorValueFunctionName)
	{
		if (AttributeWriter.UseRGBAGrid())
		{
			AttributeWriter.WriteGetOutputGridHLSL(EGridAttributeRetrievalMode::RGBAGrid, FunctionInfo, OutHLSL);
		...
		{
			WriteGetOutputGridHLSL(ParamInfo, FunctionInfo, FunctionInstanceIndex, 3, OutHLSL);
		}
		return true;
	}
	else if (FunctionInfo.DefinitionName == GetOutputGridVector2ValueFunctionName)
	{
		if (AttributeWriter.UseRGBAGrid())
		{
			AttributeWriter.WriteGetOutputGridHLSL(EGridAttributeRetrievalMode::RGBAGrid, FunctionInfo, OutHLSL);
		...
		{
			WriteGetOutputGridHLSL(ParamInfo, FunctionInfo, FunctionInstanceIndex, 2, OutHLSL);
		}
		return true;
	}
	else if (FunctionInfo.DefinitionName == GetOutputGridFloatValueFunctionName)
	{
		if (AttributeWriter.UseRGBAGrid())
		{
			AttributeWriter.WriteGetOutputGridHLSL(EGridAttributeRetrievalMode::RGBAGrid, FunctionInfo, OutHLSL);
		...
		{
			WriteGetOutputGridHLSL(ParamInfo, FunctionInfo, FunctionInstanceIndex, 1, OutHLSL);
		}
		return true;
	}
	...
	else if (FunctionInfo.DefinitionName == CubicSampleGridFunctionName || FunctionInfo.DefinitionName == CubicSamplePreviousGridAtIndexFunctionName)
	{
		if (AttributeWriter.UseRGBAGrid())
		{
			...
			UE_LOGF(LogNiagara, Error, "RGBA grids are not supported for this function");
			return false;
		}
		else
		{
...
bool UNiagaraDataInterfaceGrid3DCollection::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "%ls: Unnamed attributes should not be used with named.  This is a deprecated workflow", *Proxy->GetSourceDIName().ToString());
	}
	...
	{
		UE_LOGF(LogNiagara, Error, "Zero grid resolution defined on %ls", *Proxy->GetSourceDIName().ToString());
		return false;
	}
	...
	if (NumAttribChannelsFound == 0)
	{
		if (!Proxy->GetSourceDIName().IsNone())
		{
			UE_LOGF(LogNiagara, Log, "Zero attributes defined on %ls", *Proxy->GetSourceDIName().ToString());
		}
	...
	if (UseRGBATexture)
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "%ls: Unnamed attributes do not work with rgba textures", *Proxy->GetSourceDIName().ToString());
			return false;
		}
		...
		InstanceData->PixelFormat = FNiagaraUtilities::BufferFormatToPixelFormat(BufferFormat, GNDIGrid3DFomatCaps, NumAttribChannelsFound, this);
	...
	else
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "Resolution is too high on %ls... max is 2048, defined as %d, %d %d", *Proxy->GetSourceDIName().ToString(), InstanceData->NumCells.X, InstanceData->NumCells.Y, InstanceData->NumCells.Z);
			return false;
		}

		if (NumAttribChannelsFound > MaxAttributes && MaxAttributes > 0)
		{
			UE_LOGF(LogNiagara, Error, "Invalid number of attributes defined on %ls... max is %d, num defined is %d", *Proxy->GetSourceDIName().ToString(), MaxAttributes, NumAttribChannelsFound);
			return false;
		}
		...
		InstanceData->PixelFormat = FNiagaraUtilities::BufferFormatToPixelFormat(BufferFormat, GNDIGrid3DFomatCaps, 1, this);
	}
	...
	InstanceData->UpdateTargetTexture(BufferFormat, this);
	...
	if (bPreviewGrid && !PreviewAttribute.IsNone())
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Failed to map PreviewAttribute %ls to a grid index", *PreviewAttribute.ToString());
		}
	}
#endif
	...
	ENQUEUE_RENDER_COMMAND(FUpdateData)(
		[RT_Proxy, RT_Resource = InstanceData->TargetTexture ? InstanceData->TargetTexture->GetResource() : nullptr, InstanceID = SystemInstance->GetId(), RT_InstanceData = *InstanceData](FRHICommandListImmediate& RHICmdList)
		{
			...
			TargetData->SourceDIName = RT_Proxy->GetSourceDIName();
...
bool UNiagaraDataInterfaceGrid3DCollection::PerInstanceTick(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	bool NeedsReset = InstanceData->UpdateTargetTexture(BufferFormat, this);
...
bool UNiagaraDataInterfaceGrid3DCollection::PerInstanceTickPostSimulate(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (InstanceData && InstanceData->NeedsRealloc && InstanceData->NumCells.X > 0 && InstanceData->NumCells.Y > 0 && InstanceData->NumCells.Z > 0)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogNiagara, Error, "Invalid number of attributes defined on %ls... max is %d, num defined is %d", *Proxy->GetSourceDIName().ToString(), MaxAttributes, InstanceData->TotalNumAttributes);
				return false;
			}

			if (InstanceData->TotalNumAttributes <= 0)
			{
				UE_LOGF(LogNiagara, Warning, "Invalid number of attributes defined on %ls... max is %d, num defined is %d", *Proxy->GetSourceDIName().ToString(), MaxAttributes, InstanceData->TotalNumAttributes);
				return false;
			}
...
bool FGrid3DCollectionRWInstanceData_GameThread::UpdateTargetTexture(ENiagaraGpuBufferFormat BufferFormat, UObject* Owner)
{
	...
	if (UObject* UserParamObject = RTUserParamBinding.GetValue())
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "RenderTarget UserParam is a '%ls' but is expected to be a UTextureRenderTargetVolume", *GetNameSafe(UserParamObject->GetClass()));
		}
	}
	...
	if (TargetTexture != nullptr)
	{
		...
		const TOptional<EPixelFormat> RenderTargetFormat = FNiagaraUtilities::BufferFormatToPixelFormat(BufferFormat, GNDIGrid3DFomatCaps, 1, Owner);
...
void FNiagaraDataInterfaceProxyGrid3DCollectionProxy::PreStage(const FNDIGpuComputePreStageContext& Context)
{
	...
	if (Context.IsOutputStage())
	{
		...
		{
			// When the grid is in single-buffer mode (DestinationData == CurrentData), clearing
			// the destination would destroy the only copy of the data. This happens when the grid
			// is an output but not an input for a stage (e.g. only GetOutputGrid* + Set* are used,
			// no read functions), so BeginSimulate receives RequiresBuffering=false and reuses the
			// same buffer for both read and write. Skip the clear in this case.
			// #todo(dmp): surface this as a per-instance editor warning in GetFeedback so users
			// know ClearBeforeNonIterationStage has no effect when the grid is in single-buffer mode.
			if (ProxyData->DestinationData != ProxyData->CurrentData)
			{
...
void FNiagaraDataInterfaceProxyGrid3DCollectionProxy::PostSimulate(const FNDIGpuComputePostSimulateContext& Context)
{
	...
	if (ProxyData->bPreviewGrid && ProxyData->CurrentData && ProxyData->CurrentData->IsValid())
	{
		...
		if (ProxyData->PreviewAttribute[0] != INDEX_NONE)
		{
			...
			GpuComputeDebugInterface.AddAttributeTexture(GraphBuilder, Context.GetSystemInstanceID(), GetSourceDIName(), ProxyData->CurrentData->GetOrCreateTexture(GraphBuilder), TotalNumAttributeVector, ProxyData->PreviewAttribute);
		}
		else
		{
			GpuComputeDebugInterface.AddTexture(GraphBuilder, Context.GetSystemInstanceID(), GetSourceDIName(), ProxyData->CurrentData->GetOrCreateTexture(GraphBuilder));
		}
	}
#endif
...
{
	{
		...
		for (const FNiagaraEmitterHandle& EmitterHandle : OwnerSystem->GetEmitterHandles())
		{
			...
			{
				TSharedPtr<const FNiagaraScriptRuntimeData> ScriptRuntimeData = OwnerSystem->GetScriptRuntimeData(EmitterHandle.GetId(), ENiagaraScriptUsage::ParticleGPUComputeScript, FGuid());
				if (ScriptRuntimeData.IsValid())
				{
					CollectAttributesForScript(EmitterData->GetGPUComputeScript(), *ScriptRuntimeData, this, OutVariables, OutVariableOffsets, TotalAttributes, OutWarnings);
				}
			}
		}
...
void UNiagaraDataInterfaceGrid3DCollection::CollectAttributesForScript(UNiagaraScript* Script, const FNiagaraScriptRuntimeData& ScriptRuntimeData, const UNiagaraDataInterface* DataInterface, TArray<FNiagaraVariableBase>& OutVariables, TArray<uint32>& OutVariableOffsets, int32& TotalAttributes, TArray<FText>* OutWarnings)
{
	if (const FNiagaraScriptExecutionParameterStore* ParameterStore = ScriptRuntimeData.GetExecutionReadyParameterStore())
	{
