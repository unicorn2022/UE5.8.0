...
bool FNDCGameplayBurstAttachmentSettings::UseAttachedPathForComponent(const USceneComponent* Component, bool bForce)const
{
	...
	{
		const UPrimitiveComponent* ConstPrimComp = Cast<const UPrimitiveComponent>(Component);
		UPrimitiveComponent* PrimComp = const_cast<UPrimitiveComponent*>(ConstPrimComp);
		const FMovementBaseInterfaceData MovementBaseData(PrimComp);
		...
		if (MovementBaseUtility::IsDynamicBase(&MovementBaseData))
		{
			FVector BaseVel = MovementBaseUtility::GetMovementBaseVelocity(&MovementBaseData, NAME_None);
		...
		if (AttachParent)
		{
			...
			if (UPrimitiveComponent* PrimitiveComponentBase = Cast<UPrimitiveComponent>(AttachParent->GetRootComponent()))
			{
				const FMovementBaseInterfaceData AttachParentMovementBaseData(PrimitiveComponentBase);
				FVector BaseVel = MovementBaseUtility::GetMovementBaseVelocity(&AttachParentMovementBaseData, NAME_None);
				if (BaseVel.SquaredLength() >= (SpeedThreshold * SpeedThreshold))
				{
					return true;
				}
...
void UNiagaraDataChannelHandler_GameplayBurst::GenerateKey(FNDCAccessContextInst& AccessContext, FNDCMapKeyWriter& KeyWriter)const
{
	...
	if (Comp)
	{
		...
		if (Comp->ComponentHasTag(AttachedComponentTag))
		{
			...
			// Comp can potentially be nullptr now as we moved to the attach parent, update bUseAttachedPath accordingly to avoid a null deref
			bUseAttachedPath = Comp != nullptr;
		}
		else
		{
