...
bool UNiagaraDataInterfaceRasterizationGrid3D::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "Dimensions are too big! Please adjust! %d x %d x %d > %d for ==> %ls", RT_NumCells.X, RT_NumCells.Y, RT_NumCells.Z, GMaxNiagaraRasterizationGridCells , *GetFullNameSafe(this))
			return false;
	}
	...
	if (NumAttribChannelsFound == 0 && !Proxy->GetSourceDIName().IsNone())
	{
