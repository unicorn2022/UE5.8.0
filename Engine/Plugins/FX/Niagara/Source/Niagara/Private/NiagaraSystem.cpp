...
void UNiagaraSystem::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	PreSaveScriptRuntimeData(ObjectSaveContext);
#endif
}
...
{
	//UE_LOGF(LogNiagara, Display, "UNiagaraSystem::BeginCacheForCookedPlatformData %ls %ls", *GetFullName(), UE::IsSavingPackage() ? TEXT("Saving...") : TEXT("Not Saving..."));
...
bool UNiagaraSystem::IsCachedCookedPlatformDataLoaded(const ITargetPlatform* TargetPlatform)
{
	...
	{
		const bool bWasCompilationComplete = PollForCompilationComplete(false);

		if (bWasCompilationComplete)
		{
			// in order to handle the possibility that the requested compile failed in such a way that a new compile is being requested
			// then issue that compile now and continue to wait.  This can happen in the case of system scripts being mismatched (for
			// a currently unknown reason) and a force-compile is required to address the issue.
			if (NeedsRequestCompile())
			{
				RequestCompile(false, nullptr, TargetPlatform);
				return false;
...
UNiagaraScript* UNiagaraSystem::GetTargetCompiledScript(const FNiagaraScriptDataKey& ScriptDataKey) const
{
	switch(ScriptDataKey.ScriptUsage)
	{
	...
	default:
		if (ScriptDataKey.EmitterHandleId.IsValid())
		{
			const FNiagaraEmitterHandle* EmitterHandle = EmitterHandles.FindByPredicate([&ScriptDataKey]
				(const FNiagaraEmitterHandle& EmitterHandle) { return EmitterHandle.GetId() == ScriptDataKey.EmitterHandleId; });
			if (EmitterHandle != nullptr)
			{
				...
				case ENiagaraScriptUsage::ParticleSimulationStageScript:
					return EmitterData->GetGPUComputeScript();
				case ENiagaraScriptUsage::ParticleEventScript:
					return EmitterData->GetScript(ScriptDataKey.ScriptUsage, ScriptDataKey.ScriptUsageId);
				}
			}
		}
	}
...
TSharedPtr<const FNiagaraScriptRuntimeData> UNiagaraSystem::GetScriptRuntimeData(const FNiagaraScriptDataKey& ScriptDataKey) const
{
	const TSharedPtr<FNiagaraScriptRuntimeData>* ScriptRuntimeDataPtr = ScriptRuntimeDataMap.Find(ScriptDataKey);
	if (ScriptRuntimeDataPtr != nullptr)
	{
		return *ScriptRuntimeDataPtr;
	}

#if WITH_EDITORONLY_DATA
	TSharedPtr<FNiagaraScriptRuntimeData> ScriptRuntimeData;

	TSharedPtr<const FNiagaraScriptRuntimeCompiledDataHandle> ScriptRuntimeCompiledDataHandle = CreateScriptRuntimeCompiledDataHandle(ScriptDataKey);
	if (ScriptRuntimeCompiledDataHandle.IsValid())
	{
		UNiagaraScript* TargetCompiledScript = GetTargetCompiledScript(ScriptDataKey);
		if (TargetCompiledScript != nullptr)
		{
			if (TargetCompiledScript->GetVMExecutableData().DataInterfaceInfo.Num() == ScriptRuntimeCompiledDataHandle->Instance->ResolvedDataInterfaces.Num())
			{
				TSharedPtr<const FNiagaraScriptRuntimeTransientData> ScriptRuntimeTransientData = MakeShared<FNiagaraScriptRuntimeTransientData>(*TargetCompiledScript, *ScriptRuntimeCompiledDataHandle->Instance);
				ScriptRuntimeData = MakeShared<FNiagaraScriptRuntimeData>(ScriptRuntimeCompiledDataHandle.ToSharedRef(), ScriptRuntimeTransientData.ToSharedRef());
			}
			else
			{
				UE_LOGF(LogNiagara, Log, "Could not create script runtime data for script %ls due to compiled DI count %i and resolved DI count %i difference.",
					*TargetCompiledScript->GetPathName(), TargetCompiledScript->GetVMExecutableData().DataInterfaceInfo.Num(), ScriptRuntimeCompiledDataHandle->Instance->ResolvedDataInterfaces.Num());
			}
		}
	}

	ScriptRuntimeDataMap.Add(ScriptDataKey, ScriptRuntimeData);
	return ScriptRuntimeData;
#else
	return TSharedPtr<FNiagaraScriptRuntimeData>();
#endif
}

#if WITH_EDITORONLY_DATA
UNiagaraSystem::FEditorRuntimeCompiledDataHandleDeleter::FEditorRuntimeCompiledDataHandleDeleter(const UNiagaraSystem* System)
	: SystemWeak(TWeakObjectPtr<const UNiagaraSystem>(System))
{
}

void UNiagaraSystem::FEditorRuntimeCompiledDataHandleDeleter::operator()(FNiagaraScriptRuntimeCompiledDataHandle* ObjectToDelete) const
{
	FObjectKey OwnerEditorReferenceKey = ObjectToDelete->OwnerEditorReferenceKey;
	FNiagaraScriptDataKey OwnerScriptDataKey = ObjectToDelete->OwnerScriptDataKey;

	delete ObjectToDelete;

	const UNiagaraSystem* System = SystemWeak.Get();
	if (System != nullptr)
	{
		System->NotifyScriptRuntimeCompiledDataHandleDeleted(OwnerEditorReferenceKey, OwnerScriptDataKey);
	}
}

void UNiagaraSystem::NotifyScriptRuntimeCompiledDataHandleDeleted(const FObjectKey& OwnerEditorReferenceKey, const FNiagaraScriptDataKey& OwnerScriptDataKey) const
{
	TArray<FNiagaraScriptDataKey>* StaleRuntimeCompiledDataHandleUsage = StaleRuntimeCompiledDataUsageMap.Find(OwnerEditorReferenceKey);
	if (StaleRuntimeCompiledDataHandleUsage != nullptr)
	{
		StaleRuntimeCompiledDataHandleUsage->Remove(OwnerScriptDataKey);
		if (StaleRuntimeCompiledDataHandleUsage->Num() == 0)
		{
			StaleRuntimeCompiledDataUsageMap.Remove(OwnerEditorReferenceKey);

			int32 StaleDataIndex = StaleRuntimeCompiledDataForEditor.IndexOfByPredicate([&OwnerEditorReferenceKey]
				(const UNiagaraScriptRuntimeCompiledDataEditorReference* StaleData) { return FObjectKey(StaleData) == OwnerEditorReferenceKey; });
			if (StaleDataIndex != INDEX_NONE)
			{
				StaleRuntimeCompiledDataForEditor[StaleDataIndex]->ScriptRuntimeCompiledDataMap.Empty();
				StaleRuntimeCompiledDataForEditor.RemoveAt(StaleDataIndex);
			}
		}
	}
}
...
TSharedPtr<const FNiagaraScriptRuntimeCompiledDataHandle> UNiagaraSystem::CreateScriptRuntimeCompiledDataHandle(const FNiagaraScriptDataKey& ScriptDataKey) const
{
	if (ScriptRuntimeCookedDataMap.Num() > 0)
	{
		const FNiagaraScriptRuntimeCookedData* ScriptRuntimeCookedData = ScriptRuntimeCookedDataMap.Find(ScriptDataKey);
		return ScriptRuntimeCookedData != nullptr
			? MakeShared<FNiagaraScriptRuntimeCompiledDataHandle>(ScriptRuntimeCookedData->CookedScriptRuntimeCompiledData)
			: TSharedPtr<const FNiagaraScriptRuntimeCompiledDataHandle>();
	}

#if WITH_EDITORONLY_DATA
	if (ScriptRuntimeCompiledDataForEditor != nullptr)
	{
		const FNiagaraScriptRuntimeCompiledData* ScriptRuntimeCompiledData = ScriptRuntimeCompiledDataForEditor->ScriptRuntimeCompiledDataMap.Find(ScriptDataKey);
		if (ScriptRuntimeCompiledData != nullptr)
		{
			FNiagaraScriptRuntimeCompiledDataHandle* ScriptRuntimeCompiledDataHandle = 
				new FNiagaraScriptRuntimeCompiledDataHandle(FObjectKey(ScriptRuntimeCompiledDataForEditor), ScriptDataKey, *ScriptRuntimeCompiledData);
			return MakeShareable(ScriptRuntimeCompiledDataHandle, FEditorRuntimeCompiledDataHandleDeleter(this));
		}
	}
#endif

	return TSharedPtr<const FNiagaraScriptRuntimeCompiledDataHandle>();
}
...
namespace FNiagaraSystemParameterUtilities
{
	void DumpParameterStateToLog(FString Message) const
	{
		...
		{
			UE_LOGF(LogNiagara, Log, "Emitter Handle: %ls", *EmitterHandle.GetUniqueInstanceName());
...
void UNiagaraSystem::UpdateSystemAfterLoad()
{
	...
	if (!GetOutermost()->bIsCookedForEditor)
	{
		...
		PostLoadDefinitionsSubscriptions(
			...
			[](UObject* ObjectOwner) -> TArray<INiagaraParameterDefinitionsSubscriber*>
			{
				TArray<INiagaraParameterDefinitionsSubscriber*> OutArray;
				if (UNiagaraSystem* Owner = Cast<UNiagaraSystem>(ObjectOwner))
				{
					OutArray.Add(Owner);
				}
				return OutArray;
		...
		if (UNiagaraEmitter::GetForceCompileOnLoad())
		{
			...
			UE_LOGF(LogNiagara, Log, "System %ls being rebuilt because UNiagaraEmitter::GetForceCompileOnLoad() == true.", *GetPathName());
		}
	...
	if (GetOutermost()->HasAnyPackageFlags(EPackageFlags::PKG_Cooked) == false)
	{
		...
		{
			UE_LOGF(LogNiagara, Log, "System %ls being compiled because there were changes to a system script Change ID.", *GetPathName());
		}

		if (bEmitterScriptsAreSynchronized == false && GEnableVerboseNiagaraChangeIdLogging)
		{
			UE_LOGF(LogNiagara, Log, "System %ls being compiled because there were changes to an emitter script Change ID.", *GetPathName());
		}
...
void UNiagaraSystem::PostLoad()
{
	...
	if (GNiagaraObjectNeedsLoadMode > 0)
	{
		...
		for (UObject* Dependency : ObjectReferences)
		{
			...
			{
				UE_LOGF(LogNiagara, Log, "NiagaraSystem::PostLoad() - SubObject(%ls) RF_NeedLoad", *GetFullNameSafe(Dependency));
			}
	...
	const FNiagaraReadinessSettings& CurrentReadinessSettings = GetReadinessSettings();
	if (CurrentReadinessSettings.PSOPrecacheSettings.bCacheOnLoad)
	{
		PrecacheAssetPSOs();
	}
}
...
void UNiagaraSystem::PrecacheAssetPSOs()
{
	...
	{
		for (FMaterialInterfacePSOPrecacheParams& PrecacheParams : MaterialInterfacePSOPrecacheParamsList)
		{
			PrecacheParams.PSOPrecacheParams.bRenderCustomDepth = bRenderCustomDepth;
		}
	}

	LaunchPSOPrecachingInternal(MaterialInterfacePSOPrecacheParamsList);
}
...
bool UNiagaraSystem::IsReadyToRunInternal() const
{
	...
	if (!SystemSpawnScript || !SystemUpdateScript)
	{
		if (FPlatformProperties::RequiresCookedData())
		{
			UE_LOGF(LogNiagara, Warning, "%ls IsReadyToRunInternal() failed due to missing SystemScript.  Spawn[%ls] Update[%ls]",
				*GetFullName(),
	...
	for (int32 EmitterIt = 0; EmitterIt < EmitterCount; ++EmitterIt)
	{
		...
		if (EmitterData && !EmitterData->IsReadyToRun())
		{
			if (FPlatformProperties::RequiresCookedData())
			{
				UE_LOGF(LogNiagara, Warning, "%ls IsReadyToRunInternal() failed due to Emitter not being ready to run.  Emitter #%d - %ls",
					*GetFullName(),
					EmitterIt,
	...
	if (SystemSpawnScript->GetVMExecutableData().Attributes != SystemUpdateScript->GetVMExecutableData().Attributes)
	{
		if (FPlatformProperties::RequiresCookedData())
		{
			UE_LOGF(LogNiagara, Warning, "%ls IsReadyToRunInternal() failed due to mismatch between System spawn and update script attributes.", *GetFullName());
		}
...
{
	const FNiagaraReadinessSettings& CurrentReadinessSettings = GetReadinessSettings();
	if (CurrentReadinessSettings.PSOPrecacheSettings.DelayMode == ENiagaraSystemPSOPrecachingDelayMode::DelayActivation)
	{
...
void UNiagaraSystem::FindDataInterfaceDependencies(const FGuid& EmitterHandleId, const UNiagaraScript* Script, TArray<FVersionedNiagaraEmitter>& Dependencies)
{
	const FNiagaraScriptRuntimeData* ScriptRuntimeData = GetScriptRuntimeData(FNiagaraScriptDataKey(EmitterHandleId, Script->GetUsage(), Script->GetUsageId())).Get();
	if (ScriptRuntimeData == nullptr)
	{
		return;
	}

	for (UNiagaraDataInterface* DataInterface : ScriptRuntimeData->GetExecutionReadyParameterStore()->GetDataInterfaces())
	{
	...
	for (int32 EmitterIdx = 0; EmitterIdx < NumEmitters; ++EmitterIdx)
	{
		...
		if (EmitterData)
		{
			...
			if (EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim && EmitterData->GetGPUComputeScript())
			{
				...
				FindDataInterfaceDependencies(EmitterHandle.GetId(), EmitterData->GetGPUComputeScript(), EmitterDependencies);
			}
			else
			{
				...
				FindDataInterfaceDependencies(EmitterHandle.GetId(), EmitterData->SpawnScriptProps.Script, EmitterDependencies);
				FindDataInterfaceDependencies(EmitterHandle.GetId(), EmitterData->UpdateScriptProps.Script, EmitterDependencies);
			...
			for (int32 DepIdx = 0; DepIdx < EmitterDependencies.Num(); ++DepIdx)
			{
				for (int32 OtherEmitterIdx = 0; OtherEmitterIdx < NumEmitters; ++OtherEmitterIdx)
				{
					if (EmitterDependencies[DepIdx] == EmitterHandles[OtherEmitterIdx].GetInstance())
					{
						...
						{
							UE_LOGF(LogNiagara, Error, "Emitter[%ls] depends on Emitter[%ls] which is not available (has scalability removed it during a cook?).",
		...
		for (int32 EmitterIdx = 0; EmitterIdx < NumEmitters; ++EmitterIdx)
		{
			if (EmitterPriorities[EmitterIdx] < 0)
			{
				if (!ComputeEmitterPriority(EmitterIdx, EmitterPriorities, EmitterDependencyGraph))
				{
					...
					{
						UE_LOGF(LogNiagara, Log, "%ls", *ErrorMessage.ToString());
					}
				#endif
					break;
				}
...
void UNiagaraSystem::CacheFromCompiledData()
{
...
#if WITH_EDITORONLY_DATA
	// Update our system state data (needed for rapid iteration parameter changes)
	if (FNiagaraSystemStateData* SystemStateData = SystemStateDataStruct.GetMutablePtr<FNiagaraSystemStateData>())
	{
		if (SystemStateData->ScriptResolver)
		{
			SystemStateData->ScriptResolver->ResolveData(*SystemStateData);
		}
	}
#endif
	...
	for (int32 i=0; i < EmitterHandles.Num(); ++i)
	{
		...
		if (EmitterHandle.GetEmitterMode() == ENiagaraEmitterMode::Standard)
		{
			if (FVersionedNiagaraEmitterData* EmitterData = EmitterHandle.GetEmitterData())
			{
				...
				EmitterData->CacheFromCompiledData(DataSetCompiledData, *EmitterHandle.GetInstance().Emitter, EmitterHandle.GetId());
	...
	// Collect the static buffer flags for all data interfaces in the system.
	TMap<UNiagaraDataInterface*, FStaticBuffersCachingFlags> DataInterfaceToStaticCachingFlagsMap;
	ForEachScriptWithOwningContext([this, &DataInterfaceToStaticCachingFlagsMap](const UNiagaraSystem* OwningSystem, const FNiagaraEmitterHandle* OwningEmitterHandle, UNiagaraScript* Script)
	{
		if (Script == nullptr)
		{
			return;
		...
		FVersionedNiagaraEmitterData* EmitterData = OwningEmitterHandle != nullptr ? OwningEmitterHandle->GetEmitterData() : nullptr;
		FGuid EmitterHandleId = OwningEmitterHandle != nullptr ? OwningEmitterHandle->GetId() : FGuid();
		bool bUsedByCPU = EmitterData == nullptr || EmitterData->SimTarget == ENiagaraSimTarget::CPUSim;
		bool bUsedByGPU = EmitterData != nullptr && EmitterData->SimTarget == ENiagaraSimTarget::GPUComputeSim;
		...
		for (const FNiagaraScriptResolvedDataInterfaceInfo& DataInterfaceData : ScriptRuntimeData->GetResolvedDataInterfaces())
		{
			if (UNiagaraDataInterface* DataInterface = DataInterfaceData.ResolvedDataInterface)
			{
				FStaticBuffersCachingFlags& CachingFlagsForDataInterface = DataInterfaceToStaticCachingFlagsMap.FindOrAdd(DataInterface);
				CachingFlagsForDataInterface.bIsUsedByCpu |= bUsedByCPU;
				CachingFlagsForDataInterface.bIsUsedByGpu |= bUsedByGPU;
				CachingFlagsForDataInterface.bIsExternalDataInterface |= DataInterfaceData.ResolvedVariable.IsInExternalNamespace();
			}
		}
	});

	for (const TPair<UNiagaraDataInterface*, FStaticBuffersCachingFlags>& DataInterfaceStaticCachingFlagsPair : DataInterfaceToStaticCachingFlagsMap)
	{
		UNiagaraDataInterface* DataInterface = DataInterfaceStaticCachingFlagsPair.Key;
		const FStaticBuffersCachingFlags& StaticCachingFlags = DataInterfaceStaticCachingFlagsPair.Value;
		DataInterface->CacheStaticBuffers(*StaticBuffers.Get(), StaticCachingFlags.bIsExternalDataInterface, StaticCachingFlags.bIsUsedByCpu, StaticCachingFlags.bIsUsedByGpu);
	}
...
void UNiagaraSystem::InitScriptCompiledData()
{
	UNiagaraScriptRuntimeCompiledDataEditorReference* OldScriptRuntimeCompiledData = ScriptRuntimeCompiledDataForEditor;
	if (OldScriptRuntimeCompiledData != nullptr)
	{
		TArray<FNiagaraScriptDataKey> ScriptsInUse;
		for (TPair<FNiagaraScriptDataKey, TSharedPtr<FNiagaraScriptRuntimeData>>& ScriptKeyRuntimeDataPair : ScriptRuntimeDataMap)
		{
			if (ScriptKeyRuntimeDataPair.Value.IsValid())
			{
				ScriptsInUse.Add(ScriptKeyRuntimeDataPair.Key);
			}
		}

		if (ScriptsInUse.Num() > 0)
		{
			StaleRuntimeCompiledDataForEditor.Add(OldScriptRuntimeCompiledData);
			StaleRuntimeCompiledDataUsageMap.Add(FObjectKey(OldScriptRuntimeCompiledData)).Append(ScriptsInUse);
		}

		ScriptRuntimeCompiledDataForEditor = nullptr;
		ScriptRuntimeDataMap.Empty();
	}

	ScriptRuntimeCompiledDataForEditor = NewObject<UNiagaraScriptRuntimeCompiledDataEditorReference>(this);

	TArray<FText> ErrorMessages;
	FNiagaraResolveDIHelpers::ResolveDIs(this, ScriptRuntimeCompiledDataForEditor->ScriptRuntimeCompiledDataMap, ErrorMessages);
	if (ErrorMessages.Num() > 0)
	{
		...
		{
			UE_LOGF(LogNiagara, Warning, "Issues reported while resolving data interfaces for system %ls", *this->GetPathName());
			for (const FText& ErrorMessage : ErrorMessages)
			{
				UE_LOGF(LogNiagara, Warning, "%ls", *ErrorMessage.ToString());
			}
		}
	}
	else
	...
	{
		ForEachScriptWithOwningContext(
			[&](const UNiagaraSystem* OwningSystem, const FNiagaraEmitterHandle* OwningEmitterHandle, UNiagaraScript* Script)
			{
				if (Script == nullptr || Script->GetCachedDefaultUObjects().Num() == 0)
				{
					return;
				}
				...
				FGuid EmitterHandleId = OwningEmitterHandle != nullptr ? OwningEmitterHandle->GetId() : FGuid();
				FNiagaraScriptDataKey ScriptDataKey = FNiagaraScriptDataKey(EmitterHandleId, Script->GetUsage(), Script->GetUsageId());
				FNiagaraScriptRuntimeCompiledData& ScriptRuntimeCompiledData = ScriptRuntimeCompiledDataForEditor->ScriptRuntimeCompiledDataMap.FindOrAdd(ScriptDataKey);
				ScriptRuntimeCompiledData.ResolvedUObjects.Empty(Script->GetCachedDefaultUObjects().Num());
				...
				for (const FNiagaraScriptUObjectCompileInfo& DefaultInfo : Script->GetCachedDefaultUObjects())
				{
					...
					{
						FNiagaraResolvedUObjectInfo& ResolvedInfo = ScriptRuntimeCompiledData.ResolvedUObjects.AddDefaulted_GetRef();
				...
				ScriptRuntimeCompiledData.ResolvedUObjects.Shrink();
			});
	}
}

void UNiagaraSystem::InvalidateScriptRuntimeData()
{
	ScriptRuntimeDataMap.Empty();
}

void UNiagaraSystem::PreSaveScriptRuntimeData(FObjectPreSaveContext ObjectSaveContext)
{
	const ITargetPlatform* TargetPlatform = ObjectSaveContext.GetTargetPlatform();
	ForEachScriptWithOwningContext([this, &ObjectSaveContext, &TargetPlatform](const UNiagaraSystem* OwningSystem, const FNiagaraEmitterHandle* OwningEmitterHandle, UNiagaraScript* Script)
	{
		if (Script == nullptr)
		{
			return;
		...
		FGuid EmitterHandleId = OwningEmitterHandle != nullptr ? OwningEmitterHandle->GetId() : FGuid();
		FNiagaraScriptDataKey ScriptDataKey = FNiagaraScriptDataKey(EmitterHandleId, Script->GetUsage(), Script->GetUsageId());
		const FNiagaraScriptRuntimeCompiledData* ScriptRuntimeCompiledData = ScriptRuntimeCompiledDataForEditor != nullptr 
			? ScriptRuntimeCompiledDataForEditor->ScriptRuntimeCompiledDataMap.Find(ScriptDataKey) : nullptr;
		if (ScriptRuntimeCompiledData == nullptr)
		{
			return;
		}
		...
		if (ScriptRuntimeCompiledData->ValidateWithScript(*Script) == false)
		{
			Script->InvalidateCompileResults(TEXT("Failed to validate script compiled data during presave."));
			return;
		}

		if (TargetPlatform != nullptr && TargetPlatform->RequiresCookedData())
		{
			FNiagaraScriptRuntimeCookedData& ScriptCookedRuntimeData = ScriptRuntimeCookedDataMap.FindOrAdd(ScriptDataKey);
			ScriptCookedRuntimeData.Init(*Script, *ScriptRuntimeCompiledData);
		}
	});


	if (TargetPlatform != nullptr && TargetPlatform->RequiresCookedData() == false)
	{
		ScriptRuntimeCookedDataMap.Empty();
	}
}
...
void UNiagaraSystem::UpdatePostCompileDIInfo()
{
	...
	TSet<UNiagaraDataInterface*> ResolvedDataInterfaces;
	if (ScriptRuntimeCookedDataMap.Num() > 0)
	{
		for (const TPair<FNiagaraScriptDataKey, FNiagaraScriptRuntimeCookedData>& ScriptRuntimeCookedDataPair : ScriptRuntimeCookedDataMap)
		{
			const FNiagaraScriptRuntimeCookedData& ScriptRuntimeCookedData = ScriptRuntimeCookedDataPair.Value;
			for (const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDIInfo : ScriptRuntimeCookedData.CookedScriptRuntimeCompiledData.ResolvedDataInterfaces)
			{
				if (ResolvedDIInfo.ResolvedDataInterface)
				{
					ResolvedDataInterfaces.Add(ResolvedDIInfo.ResolvedDataInterface);
				}
			}
		}
	}
#if WITH_EDITORONLY_DATA
	else if (ScriptRuntimeCompiledDataForEditor != nullptr)
	{
		for (const TPair<FNiagaraScriptDataKey, FNiagaraScriptRuntimeCompiledData>& ScriptRuntimeCompiledDataPair : ScriptRuntimeCompiledDataForEditor->ScriptRuntimeCompiledDataMap)
		{
			const FNiagaraScriptRuntimeCompiledData& ScriptRuntimeCompiledData = ScriptRuntimeCompiledDataPair.Value;
			for (const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDIInfo : ScriptRuntimeCompiledData.ResolvedDataInterfaces)
			{
				if(ResolvedDIInfo.ResolvedDataInterface)
				{
					ResolvedDataInterfaces.Add(ResolvedDIInfo.ResolvedDataInterface);
	...
	for (UNiagaraDataInterface* ResolvedDataInterface : ResolvedDataInterfaces)
	{
		ResolvedDataInterface->PostCompile(*this);
	}
}
...
void UNiagaraSystem::UpdateDITickFlags()
{
	...
	ForEachScriptWithRuntimeData([&](const UNiagaraScript* Script, const FNiagaraScriptRuntimeData* ScriptRuntimeData)
	{
		for (const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDataInterfaceData : ScriptRuntimeData->GetResolvedDataInterfaces())
		{
			if(const UNiagaraDataInterface* ResolvedDataInterface = ResolvedDataInterfaceData.ResolvedDataInterface)
			{
				if (ResolvedDataInterface->HasPostSimulateTick())
				{
					bHasDIsWithPostSimulateTick |= true;
					bAllDIsPostSimulateCanOverlapFrames &= ResolvedDataInterface->PostSimulateCanOverlapFrames();
				}
				bAllDIsPostStageCanOverlapTickGroups &= ResolvedDataInterface->PostStageCanOverlapTickGroups();
			}				
		}
	});
}
...
void UNiagaraSystem::PostLoadScriptRuntimeData()
{
#if WITH_EDITORONLY_DATA
	ScriptRuntimeCookedDataMap.Empty();
#else
	ScriptRuntimeDataMap.Empty();
	ForEachScriptWithOwningContext([this](const UNiagaraSystem* OwningSystem, const FNiagaraEmitterHandle* OwningEmitterHandle, UNiagaraScript* Script)
	{
		if (Script == nullptr)
		{
			return;
		...
		FNiagaraScriptDataKey ScriptDataKey = FNiagaraScriptDataKey(OwningEmitterHandle, *Script);
		FNiagaraScriptRuntimeCookedData* ScriptCookedRuntimeData = ScriptRuntimeCookedDataMap.Find(ScriptDataKey);
		if (ScriptCookedRuntimeData == nullptr)
		{
			return;
		}

		Script->ConditionalPostLoad();
		ScriptCookedRuntimeData->CookedExecutionParameterStore.PostLoad(this);

		if (ScriptCookedRuntimeData->ValidateWithScript(*Script) == false)
		{
			Script->GetVMExecutableData().Reset();
			return;
		}

		Script->RapidIterationParameters.Bind(&ScriptCookedRuntimeData->CookedExecutionParameterStore, &ScriptCookedRuntimeData->CookedRapidIterationBindings);
		ScriptCookedRuntimeData->CookedExecutionParameterStore.bInitialized = true;

		TSharedRef<FNiagaraScriptRuntimeCompiledDataHandle> RuntimeCompiledDataHandle = MakeShared<FNiagaraScriptRuntimeCompiledDataHandle>(ScriptCookedRuntimeData->CookedScriptRuntimeCompiledData);
		TSharedRef<FNiagaraScriptRuntimeTransientData> RuntimeTransientData = MakeShared<FNiagaraScriptRuntimeTransientData>(*Script, ScriptCookedRuntimeData->CookedScriptRuntimeCompiledData, ScriptCookedRuntimeData->CookedExecutionParameterStore);
		ScriptRuntimeDataMap.Add(ScriptDataKey, MakeShared<FNiagaraScriptRuntimeData>(RuntimeCompiledDataHandle, RuntimeTransientData)); 
	});
#endif
}
	...
	while (!ActiveCompilations.IsEmpty())
	{
		...
		if (GIsAutomationTesting)
		{
			...
			if (DeltaTimeSinceLastLog > TimeLogThreshold)
			{
				...
				UE_LOGF(LogNiagara, Log, "Waiting for %f seconds > %ls", (float)DeltaTimeSinceLastLog, *GetPathNameSafe(this));
				NumLogIterations--;
			}

			if (NumLogIterations == 0)
			{
				UE_LOGF(LogNiagara, Warning, "Timed out for compiling > %ls", *GetPathNameSafe(this));
				break;
			}
		}
	}
...
const FNiagaraReadinessSettings& UNiagaraSystem::GetReadinessSettings() const
{
	if (UNiagaraEffectType* FXType = GetEffectType())
	{
		if (const FNiagaraReadinessSettingsProfile* SettingsProfile = FXType->FindReadinessSettingsProfile())
		{
			return SettingsProfile->Get();
		}
	}
	
	static FNiagaraReadinessSettings Fallback;
	Fallback.PSOPrecacheSettings = GetDefault<UNiagaraSettings>()->DefaultPSOPrecacheSettings;
	return Fallback;
}
...
void UNiagaraSystem::ResolveScalabilitySettings()
{
	...
	//resolve emitter importance
	MaxEmitterImportance = ENiagaraEmitterImportance::Low;
	for (const FNiagaraEmitterHandle& EmitterHandle : GetEmitterHandles())
	{
		if (EmitterHandle.GetIsEnabled())
		{
			if (FVersionedNiagaraEmitterData* Data = EmitterHandle.GetEmitterData())
			{
				MaxEmitterImportance = MaxEmitterImportance < Data->Importance ? MaxEmitterImportance : Data->Importance;
			}
		}
	}
...
void UNiagaraSystem::FixupPositionUserParameters()
{
	...
	for (const FNiagaraVariable& LinkedParameter : LinkedPositionInputs)
	{
		...
		UE_LOGF(LogNiagara, Log, "Converted parameter %ls from vec3 to position type in asset %ls", *LinkedParameter.GetName().ToString(), *GetPathName());
	}
}
