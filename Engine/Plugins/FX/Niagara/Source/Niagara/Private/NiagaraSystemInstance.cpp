...
FNiagaraSystemInstance::FNiagaraSystemInstance(UWorld& InWorld, UNiagaraSystem& InSystem, FNiagaraUserRedirectionParameterStore* InOverrideParameters,
	...
	: TickBehavior(InTickBehavior)
	  ...
	  , WeakAttachComponent(InAttachComponent)
	  , Age(0.0f)
	  , LastRenderTime(0.0f)
	  , TickCount(0)
	  , RandomSeed(0)
...
void FNiagaraSystemInstance::SetEmitterEnable(FName EmitterName, bool bNewEnableState)
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "SetEmitterEnable: Failed to find Emitter(%ls) System(%ls) Component(%ls)", *EmitterName.ToString(), *GetNameSafe(System), *GetFullNameSafe(GetAttachComponent()));
	}
}
...
void FNiagaraSystemInstance::SetRequestedExecutionState(ENiagaraExecutionState InState)
{
	...
	if (RequestedExecutionState != InState && RequestedExecutionState != ENiagaraExecutionState::Disabled)
	{
		...
		UE_LOGF(LogNiagara, Log, "Component \"%ls\" System \"%ls\" requested change state: %ls to %ls, actual %ls", *GetComponent()->GetName(), *GetSystem()->GetName(), *EnumPtr->GetNameStringByValue((int64)RequestedExecutionState),
...
void FNiagaraSystemInstance::SetActualExecutionState(ENiagaraExecutionState InState)
{
	...
	{
		/*const UEnum* EnumPtr = FNiagaraTypeDefinition::GetExecutionStateEnum();
		UE_LOGF(LogNiagara, Log, "Component \"%ls\" System \"%ls\" actual change state: %ls to %ls", *GetComponent()->GetName(), *GetSystem()->GetName(), *EnumPtr->GetNameStringByValue((int64)ActualExecutionState),
...
bool FNiagaraSystemInstance::RequestCapture(const FGuid& RequestId)
{
	...
	UE_LOGF(LogNiagara, Warning, "Capture requested!");
...
void FNiagaraSystemInstance::SetSolo(bool bInSolo)
{
	...
	if ( SystemSimulation.IsValid() )
	{
		if (bInSolo)
		{
			...
			NewSoloSim->Init(System, World, true, TG_MAX, FNiagaraParameterCollectionResolver(Cast<UNiagaraComponent>(GetAttachComponent()), System, World));
...
void FNiagaraSystemInstance::SetGpuComputeDebug(bool bEnableDebug)
{
	...
	if (bEnableDebug)
	{
		...
		if (USceneComponent* Owner = GetAttachComponent())
		{
...
{
	UActorComponent* PrereqComponent = GetAttachComponent();
...
void FNiagaraSystemInstance::Complete(bool bExternalCompletion)
{
	...
	//UE_LOGF(LogNiagara, Log, "FNiagaraSystemInstance::Complete { %p", this);
...
void FNiagaraSystemInstance::Reset(EResetMode Mode)
{
	...
	// Update LWC Tile & WorldTransform as it may be used when data interaces create instance data
	...
	{
		//UE_LOGF(LogNiagara, Log, "FNiagaraSystemInstance::Reset false");
		ResetInternal(false);
	}
	else if (Mode == EResetMode::ResetAll)
	{
		//UE_LOGF(LogNiagara, Log, "FNiagaraSystemInstance::Reset true");
		ResetInternal(true);
		bBindParams = !IsDisabled();
	}
	else if (Mode == EResetMode::ReInit)
	{
		//UE_LOGF(LogNiagara, Log, "FNiagaraSystemInstance::ReInit");
		ReInitInternal();
		bBindParams = !IsDisabled();
	}
...
void FNiagaraSystemInstance::ResetInternal(bool bResetSimulations)
{
	...
	{
		SetRequestedExecutionState(ENiagaraExecutionState::Disabled);
		UE_LOGF(LogNiagara, Warning, "Failed to activate Niagara System due to invalid asset! System(%ls) Component(%ls)", *System->GetName(), *GetFullNameSafe(GetAttachComponent()));
		return;
	}
...
#if WITH_EDITOR
	//UE_LOGF(LogNiagara, Log, "OnResetInternal %p", this);
	OnResetDelegate.Broadcast();
#endif
}

const FNiagaraParameterCollectionResolver FNiagaraSystemInstance::CreateParameterCollectionResolver() const
{
	return FNiagaraParameterCollectionResolver(Cast<UNiagaraComponent>(GetAttachComponent()), System, World);
}
...
void FNiagaraSystemInstance::ReInitInternal()
{
	...
	if (bSolo)
	{
		if (!SystemSimulation.IsValid())
		{
			...
			SystemSimulation->Init(System, World, true, TG_MAX, FNiagaraParameterCollectionResolver(Cast<UNiagaraComponent>(GetAttachComponent()), System, World));
		}
	}
	else
	{
...
{
	//UE_LOGF(LogNiagara, Log, "~FNiagaraSystemInstance %p", this);
...
void FNiagaraSystemInstance::BindParameters()
{
	...
	{
		UE_LOGF(LogNiagara, Warning, "OverrideParameters is null.  Component(%ls) System(%ls)", *GetFullNameSafe(GetAttachComponent()), *GetFullNameSafe(GetSystem()));
	}
...
void FNiagaraSystemInstance::RebindParameterCollection(UNiagaraParameterCollectionInstance* OldCollection, UNiagaraParameterCollectionInstance* NewCollection)
{
	check(OldCollection && NewCollection);
	check(OldCollection->GetParent() == NewCollection->GetParent());

	WaitForConcurrentTickDoNotFinalize(true);

	// Rebind the system simulation parameter collection
	if (ensure(IsSolo()))
	{
		UNiagaraParameterCollection* CollectionParent = OldCollection->GetParent();
		for (FNiagaraScriptExecutionContextBase* ScriptExecutionContext : { SystemSimulation->GetSpawnExecutionContext() , SystemSimulation->GetUpdateExecutionContext() })
		{
			if (ScriptExecutionContext->Script->GetCachedParameterCollectionReferences().Contains(CollectionParent))
			{
				OldCollection->GetParameterStore().Unbind(&ScriptExecutionContext->Parameters);
				NewCollection->GetParameterStore().Bind(&ScriptExecutionContext->Parameters);
			}
		}
	}
 	...
 	{
 		Simulation->RebindParameterCollection(OldCollection, NewCollection);
 	}

	// If we have data interfaces inside the collections we may need to initialize them again
	//-OPT: We could look to see if any of them are actually bound, this plays is safe
	if (NewCollection->GetParameterStore().GetDataInterfaces().Num() > 0)
	{
		InitDataInterfaces();
	}
}
...
void FNiagaraSystemInstance::UpdateLWCTile(const FVector3f& InLWCTile, FNiagaraDataSet& EmitterDataSet)
{
	...
	if ( bResetSimulation )
	{
		if (TickCount > 1)
		{
			UE_LOGF(LogNiagara, Warning, "NiagaraComponent(%ls - %ls) required LWC tile recache and was reset.", *GetFullNameSafe(GetAttachComponent()), *GetFullNameSafe(System));
		}
...
void FNiagaraSystemInstance::InitDataInterfaces()
{
			...
			if (Var.IsDataInterface())
			{
				...
				if (Interface)
				{
					if (int32 Size = Interface->PerInstanceDataSize())
					{
						...
						{
							//UE_LOGF(LogNiagara, Log, "Adding DI %p %ls %ls", Interface, *Interface->GetClass()->GetName(), *Interface->GetPathName());
					...
					if (bIsGPUScript)
					{
						...
						{
							Proxy->SetSourceDIName(Var.GetName());
						}
					}
	...
	for (int32 i=0; i < DataInterfaceInstanceDataOffsets.Num(); ++i)
	{
		...
		if (UNiagaraDataInterface* Interface = Pair.Key.Get())
		{
			...
			if (!bResult)
			{
				UE_LOGF(LogNiagara, Error, "Error initializing data interface \"%ls\" for system. %ls", *Interface->GetPathName(), *GetNameSafe(System));
			}
		}
		else
		{
			UE_LOGF(LogNiagara, Error, "A data interface currently in use by '%ls' has been destroyed.", *GetNameSafe(System));
			bDataInterfacesInitialized = false;
		}
	}
	...
	if (!bDataInterfacesInitialized && (!IsComplete() && !IsPendingSpawn()))
	{
		...
		UE_LOGF(LogNiagara, Error, "Error initializing data interfaces. Completing system. %ls", *GetNameSafe(System));
		Complete(true);
		return;
	}
	...
	SystemSpawnDIStageTickHandler.Init(ENiagaraScriptUsage::SystemSpawnScript, System->GetScriptRuntimeData(FGuid(), ENiagaraScriptUsage::SystemSpawnScript, FGuid()).Get(), this);
	SystemUpdateDIStageTickHandler.Init(ENiagaraScriptUsage::SystemUpdateScript, System->GetScriptRuntimeData(FGuid(), ENiagaraScriptUsage::SystemUpdateScript, FGuid()).Get(), this);
	for (const FNiagaraEmitterInstanceRef& EmitterRef : Emitters)
	{
		//-TODO:Stateless: Avoid Casting
		FNiagaraEmitterInstanceImpl* Emitter = EmitterRef->AsStateful();
...
void FNiagaraSystemInstance::ResolveUserDataInterfaceBindings()
{
	...
	auto ResolveUserDIs = [&InstanceParameterDataInterfaces](FNiagaraParameterStore& TargetParameterStore, const FNiagaraScriptRuntimeData* ScriptRuntimeData)
	{
		...
		for (const FNiagaraResolvedUserDataInterfaceBinding& ResolvedUserDIBinding : ScriptRuntimeData->GetResolvedUserDataInterfaceBindings())
		{
	...
	// NOTE: We do not resolve the system/emitter script user parameters here because they are handled differently.  There
	// is only one execution parameter store for all system instances, and each system instance can have different user
	// data interfaces, so the derived system script execution context handles these bindings by parameter name, rather than
	...
	for (const FNiagaraEmitterInstanceRef& EmitterRef : Emitters)
	{
		...
		ResolveUserDIs(Emitter->GetSpawnExecutionContext().Parameters, Emitter->GetSpawnExecutionContext().GetScriptRuntimeData());
		ResolveUserDIs(Emitter->GetUpdateExecutionContext().Parameters, Emitter->GetUpdateExecutionContext().GetScriptRuntimeData());
		if (Emitter->GetGPUContext() != nullptr)
		{
			ResolveUserDIs(Emitter->GetGPUContext()->CombinedParamStore, Emitter->GetGPUContext()->GetScriptRuntimeData());
		}
		for (FNiagaraScriptExecutionContext& EventContext : Emitter->GetEventExecutionContexts())
		{
			ResolveUserDIs(EventContext.Parameters, EventContext.GetScriptRuntimeData());
		}
	}
}
...
void FNiagaraSystemInstance::TickDataInterfaces(float DeltaSeconds, bool bPostSimulate)
{
	...
	if (bRebindVMFuncs)
	{
		...
		// Rebind funcs for system scripts
		{
			...
			{
				// Some error initializing the per instance function tables.
				UE_LOGF(LogNiagara, Error, "Error rebinding VM functions after re-initializing data interface(s). Completing system. %ls", *GetNameSafe(System));
				Complete(true);
			}
		}
		}
...
ETickingGroup FNiagaraSystemInstance::CalculateTickGroup() const
{
	...
	switch ( TickBehavior )
	{
		...
		case ENiagaraTickBehavior::UsePrereqs:
			...
			if(NewMinTickGroup > NewMaxTickGroup)
			{
				...
				UE_LOGF(LogNiagara, Warning, "Niagara Component has DIs with conflicting Tick Group Dependencies. This may lead to some incorrect behavior.\nSystem:%ls\nMinTickGroup:%ls\nMaxTickGroup:%ls"
				, *System->GetName()
	...
	//UE_LOGF(LogNiagara, Log, "TickGroup: %ls %d %d", *Component->GetPathName(), (int32)TickBehavior, (int32)NewTickGroup);
...
void FNiagaraSystemInstance::DumpStalledInfo()
{
	...
	Builder.Appendf(TEXT("SystemInstanceState (%d)\n"), uint32{EnumToUnderlyingType(SystemInstanceState)});

	UE_LOGF(LogNiagara, Fatal, "FNiagaraSystemInstance is stalled.\n%ls", Builder.ToString());
}
...
void FNiagaraSystemInstance::WaitForConcurrentTickDoNotFinalize(bool bEnsureComplete)
{
	...
	if (bDidWait)
	{
		...
		if ((GWaitForAsyncStallWarnThresholdMS > 0.0f) && (StallTimeMS > GWaitForAsyncStallWarnThresholdMS))
		{
			...
			UE_LOGF(LogNiagara, Log, "Niagara Effect stalled GT for %g ms. Component(%ls) System(%ls)", StallTimeMS, *GetFullNameSafe(GetAttachComponent()), *GetFullNameSafe(GetSystem()));
		}
	}
...
bool FNiagaraSystemInstance::HandleCompletion()
{
	...
	{
		//UE_LOGF(LogNiagara, Log, "Completion Achieved");
		Complete(false);
		return true;
	}
...
void FNiagaraSystemInstance::FinalizeTick_GameThread(bool bEnqueueGPUTickIfNeeded)
{
	...
	if (!HandleCompletion())
	{
		...
		checkf(Sim != nullptr,
			...
			*GetFullNameSafe(System), *GetFullNameSafe(GetAttachComponent()), EnumToUnderlyingType(ActualExecutionState), SystemInstanceIndex,
...
void FNiagaraSystemInstance::InitGPUTick(FNiagaraGPUSystemTick& OutTick)
{
	...
	//	//UE_LOGF(LogNiagara, Log, "GT Testing (dipacket) %p (baseptr) %p", GPUTick.DIInstanceData, BasePointer);
	//	for (auto& Pair : GPUTick.DIInstanceData->InterfaceProxiesToOffsets)
	//	{
	//		FNiagaraDataInterfaceProxy* Proxy = Pair.Key;
	//		UE_LOGF(LogNiagara, Log, "\tGT (proxy) %p (size) %u", Proxy, Proxy->PerInstanceDataPassedToRenderThreadSize());
	//	}
	//}
}
...
void FNiagaraSystemInstance::SetEmitterFixedBounds(FName EmitterName, const FBox& InLocalBounds)
{
	...
	UE_LOGF(LogNiagara, Warning, "SetEmitterFixedBounds: Failed to find Emitter(%ls) System(%ls) Component(%ls)", *EmitterName.ToString(), *GetNameSafe(System), *GetFullNameSafe(GetAttachComponent()));
}
...
FBox FNiagaraSystemInstance::GetEmitterFixedBounds(FName EmitterName) const
{
	...
	// Failed to find emitter
	UE_LOGF(LogNiagara, Warning, "GetEmitterFixedBounds: Failed to find Emitter(%ls) System(%ls) Component(%ls)", *EmitterName.ToString(), *GetNameSafe(System), *GetFullNameSafe(GetAttachComponent()));
	return FBox(ForceInit);
}
...
const FString& FNiagaraSystemInstance::GetCrashReporterTag()const
{
	...
	{
		UNiagaraComponent* AttachComponent = Cast<UNiagaraComponent>(GetAttachComponent());
		USceneComponent* AttachParent = AttachComponent ? AttachComponent->GetAttachParent() : nullptr;

		CrashReporterTag = FString::Printf(
			TEXT("SystemInstance | System: %s | bSolo: %s | Component: %s | AttachedTo: %s |"),
			*GetFullNameSafe(GetSystem()),
			IsSolo() ? TEXT("true") : TEXT("false"),
			*GetFullNameSafe(AttachComponent),
			*GetFullNameSafe(AttachParent)
		);
	}
