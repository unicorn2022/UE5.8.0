...
static int GNiagaraPSOPrecacheReverseCulling = 0;
...
TArray<FNiagaraVariable> UNiagaraRendererProperties::GetBoundAttributes() const
{
	...
	BoundAttributes.Reserve(AttributeBindings.Num() + 1);
	...
	if (RendererEnabledBinding.IsValid())
	{
		BoundAttributes.AddUnique(RendererEnabledBinding.GetParamMapBindableVariable());
	}
...
void UNiagaraRendererProperties::UpdateMaterialParametersMIC(const FNiagaraRendererMaterialParameters& MaterialParameters, TObjectPtr<UMaterialInterface>& InOutMaterial, TObjectPtr<UMaterialInstanceConstant>& InOutMIC, FMaterialUpdateContext* OptionalMaterialUpdateContext)
{
	TArray<UMaterialInterface*> LocalMaterials = { InOutMaterial };
	TArray<TObjectPtr<UMaterialInstanceConstant>> LocalMICs = { InOutMIC };

	UpdateMaterialParametersMIC(MaterialParameters, LocalMaterials, LocalMICs, OptionalMaterialUpdateContext);

	InOutMIC = LocalMICs.IsEmpty() ? nullptr : LocalMICs[0];
}

void UNiagaraRendererProperties::UpdateMaterialParametersMIC(const FNiagaraRendererMaterialParameters& MaterialParameters, TArrayView<UMaterialInterface*> Materials, TArray<TObjectPtr<UMaterialInstanceConstant>>& InOutMICs, FMaterialUpdateContext* OptionalMaterialUpdateContext)
{
	...
	FMaterialUpdateContext DefaultMaterialUpdateContext(FMaterialUpdateContext::EOptions::SkipRecacheStaticPermutations | FMaterialUpdateContext::EOptions::SkipUpdateStaticDrawLists);
		...
		if (BuildMaterialStaticParameterSet(MaterialParameters, Material, MaterialParameterSet))
		{
			...
			if (MIC == nullptr)
			{
				if (UObject* ExistingObject = StaticFindObject(UMaterialInstanceConstant::StaticClass(), this, *NameBuilder))
				{
					...
					UE_LOGF(LogNiagara, Log, "While trying to allocate %ls UNiagaraRendererProperties::UpdateMaterialParametersMIC() found a pre-existing object.  This will result in a re-allocation!  FullName: %ls | InternalFlags: %x | Flags: %x",
						*NameBuilder, *FullName, EnumToUnderlyingType(InternalFlags), EnumToUnderlyingType(Flags));
			...
			MIC->UpdateStaticPermutation(MaterialParameterSet, OptionalMaterialUpdateContext ? OptionalMaterialUpdateContext : &DefaultMaterialUpdateContext);
			MIC->PostEditChange();
...
void UNiagaraRendererProperties::GetMaterialUsageFeedback(EMaterialUsage Usage, TConstArrayView<EMaterialDomain> InvalidMaterialDomains, TArray<FNiagaraRendererFeedback>& OutFeedback) const
{
	...
	for (UMaterialInterface* Material : Materials)
	{
		...
		Material->NeedsSetMaterialUsage_Concurrent(bOutHasUsage, Usage);
...
{
	// LWE don't support emitter mode
	// Doing the reverse test does not work as the outer might be the clipboard
	return GetTypedOuter<UNiagaraStatelessEmitter>() == nullptr;
}
...
#if WITH_EDITORONLY_DATA
bool UNiagaraRendererProperties::IsContainedInEmitterRendererArray() const
{
	FVersionedNiagaraEmitterBase VersionedEmitter = GetOuterEmitterBase();
	if (VersionedEmitter.Emitter)
	{
		if (UNiagaraEmitter* Emitter = Cast<UNiagaraEmitter>(VersionedEmitter.Emitter))
		{
			if (FVersionedNiagaraEmitterData* EmitterData = Emitter->GetEmitterData(VersionedEmitter.Version))
			{
				return EmitterData->GetRenderers().Contains(this);
			}
		}
		else if (UNiagaraStatelessEmitter* StatelessEmitter = Cast<UNiagaraStatelessEmitter>(VersionedEmitter.Emitter))
		{
			return StatelessEmitter->GetRenderers().Contains(this);
		}
	}
	return false;
}
...
void UNiagaraRendererProperties::InitRendererBaseBindings()
{
	if (!RendererEnabledBinding.IsValid())
	{
