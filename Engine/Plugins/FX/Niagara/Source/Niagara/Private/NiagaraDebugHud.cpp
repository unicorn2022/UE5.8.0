...
namespace NiagaraDebugLocal
{
	...
	static TTuple<const TCHAR*, const TCHAR*, TFunction<void(FString)>> GDebugConsoleCommands[] =
	{
		...
		MakeTuple(TEXT("FiltersEnabled="), TEXT("Enable or disable the filters."), [](FString Arg) {Settings.bFiltersEnabled = FCString::Atoi(*Arg) != 0; }),
		...
		MakeTuple(TEXT("ShowRegisteredComponents="), TEXT("Should we show registered components in the overview."), [](FString Arg) {Settings.bShowRegisteredComponents = FCString::Atoi(*Arg) != 0; }),
	...
	static FAutoConsoleCommandWithWorldAndArgs CmdDebugHud(
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& Args, UWorld*)
			{
				if ( Args.Num() == 0 )
				{
					UE_LOGF(LogNiagara, Log, "fx.Niagara.Debug.Hud - CommandList");
					for ( const auto& Command : GDebugConsoleCommands )
					{
						UE_LOGF(LogNiagara, Log, " \"%ls\" %ls", Command.Get<0>(), Command.Get<1>());
					}
					return;
				}
				...
				for ( FString Arg : Args )
				{
					...
					if ( !bFound )
					{
						UE_LOGF(LogNiagara, Warning, "Command '%ls' not found", *Arg);
					}
				}
			}
		)
	...
	static FAutoConsoleCommandWithWorldAndArgs CmdSpawnComponent(
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& Args, UWorld* World)
			{
				...
				if ( Args.Num() == 0 )
				{
					UE_LOGF(LogNiagara, Log, "fx.Niagara.Debug.SpawnSystem <AssetPath>");
					return;
				}
				...
				{
					UE_LOGF(LogNiagara, Warning, "Failed to load NiagaraSystem '%ls'", *Args[0]);
					return;
				}
	...
	void FindScriptVariablesByWildcard(UNiagaraScript* NiagaraScript, const FNiagaraScriptRuntimeData* ScriptRuntimeData, const TArray<FNiagaraDebugHUDVariable>& DebugVariables, FCachedVariables* CachedVariables, EParameterStoreLocation StoreLocation, int32 EmitterIndex = INDEX_NONE, ENiagaraSimTarget SimTarget = ENiagaraSimTarget::CPUSim)
	{
		if (NiagaraScript && ScriptRuntimeData)
		{
			FindParameterStoreVariablesByWildcard(ScriptRuntimeData->GetExecutionReadyParameterStore(), DebugVariables, CachedVariables, StoreLocation, EmitterIndex, SimTarget);
	...
	void FindSystemVariablesByWildcard(UNiagaraSystem* NiagaraSystem, const TArray<FNiagaraDebugHUDVariable>& DebugVariables, FCachedVariables* CachedVariables)
	{
		...
		FindScriptVariablesByWildcard(NiagaraSystem->GetSystemSpawnScript(), NiagaraSystem->GetScriptRuntimeData(nullptr, *NiagaraSystem->GetSystemSpawnScript()).Get(),
			DebugVariables, CachedVariables, EParameterStoreLocation::SystemSpawn);
		FindScriptVariablesByWildcard(NiagaraSystem->GetSystemUpdateScript(), NiagaraSystem->GetScriptRuntimeData(nullptr, *NiagaraSystem->GetSystemUpdateScript()).Get(),
			DebugVariables, CachedVariables, EParameterStoreLocation::SystemUpdate);
		...
		for (const FNiagaraEmitterHandle& EmitterHandle : NiagaraSystem->GetEmitterHandles())
		{
			...
			{
				FindScriptVariablesByWildcard(EmitterData->SpawnScriptProps.Script, NiagaraSystem->GetScriptRuntimeData(&EmitterHandle, *EmitterData->SpawnScriptProps.Script).Get(),
					DebugVariables, CachedVariables, EParameterStoreLocation::EmitterSpawn, EmitterIndex, EmitterData->SimTarget);
				FindScriptVariablesByWildcard(EmitterData->UpdateScriptProps.Script, NiagaraSystem->GetScriptRuntimeData(&EmitterHandle, *EmitterData->UpdateScriptProps.Script).Get(),
					DebugVariables, CachedVariables, EParameterStoreLocation::EmitterUpdate, EmitterIndex, EmitterData->SimTarget);
	...
	const FCachedVariables& GetCachedVariables(UNiagaraSystem* NiagaraSystem)
	{
		...
		if (CachedVariables == nullptr)
		{
			...
			if (Settings.bShowParticleVariables && Settings.ParticlesVariables.Num() > 0)
			{
				...
				TArray<FNiagaraDebugHUDVariable> LocalParticlesVariables;
				LocalParticlesVariables.Reserve(Settings.ParticlesVariables.Num());
				Algo::TransformIf(
					Settings.ParticlesVariables,
					LocalParticlesVariables,
					[](const FNiagaraDebugHUDVariable& Variable) { return Variable.bEnabled; },
					[](const FNiagaraDebugHUDVariable& Variable)
					{
						FNiagaraDebugHUDVariable NewVariable = Variable;
						NewVariable.Name.RemoveFromStart(FNiagaraConstants::ParticleAttributeNamespaceString);
						return NewVariable;
					}
				);				
	...
	void DrawSystemLocation(UCanvas* Canvas, bool bIsActive, const FVector& ScreenLocation, const FRotator& Rotation)
	{
		...
		{
			const FMatrix& ViewMatrix = SceneView->ViewMatrices.GetWorldToView();
				...
				EmitterInstance->ForEachEnabledRenderer(
					[&](const UNiagaraRendererProperties* RenderProperties)
					{
						...
						if (bSupportsThisFrameData)
						{
							...
							{
								if (Material && (!IsTranslucentBlendMode(*Material) || Material->GetCastDynamicShadowAsMasked()))
								{
...
void FNiagaraDebugHud::GatherSystemInfo()
{
	...
	for (TObjectIterator<UFXSystemComponent> It; It; ++It)
	{
		...
		UNiagaraComponent* NiagaraComponent = Settings.bIncludeNiagara ? Cast<UNiagaraComponent>(FXComponent) : nullptr;
		UParticleSystemComponent* CascadeComponent = Settings.bIncludeCascade ? Cast<UParticleSystemComponent>(FXComponent) : nullptr;
		...
		const bool bHasScalability = NiagaraComponent ? NiagaraComponent->IsRegisteredWithScalabilityManager() : (CascadeComponent ? CascadeComponent->bIsManagingSignificance : false);
		...
		SystemDebugInfo.bPassesSystemFilter = !(Settings.bFiltersEnabled && Settings.bSystemFilterEnabled) || Settings.SystemFilter.IsEmpty() || SystemDebugInfo.SystemName.MatchesWildcard(Settings.SystemFilter);
		...
		if (bCanShowInWorld)
		{
			...
			// Filter by actor
			if (Settings.bFiltersEnabled && Settings.bActorFilterEnabled && !Settings.ActorFilter.IsEmpty())
			{
			...
			// Filter by component
			if ( bIsMatch && Settings.bFiltersEnabled && Settings.bComponentFilterEnabled && !Settings.ComponentFilter.IsEmpty())
			{
...
const FNiagaraDataSet* FNiagaraDebugHud::GetParticleDataSet(FNiagaraSystemInstance* SystemInstance, FNiagaraEmitterInstance* EmitterInstance, int32 iEmitter)
{
	...
	if (EmitterInstance == nullptr || SystemInstance == nullptr)
	{
		return nullptr;
	}
	...
	if (EmitterInstance->GetSimTarget() == ENiagaraSimTarget::GPUComputeSim)
	{
		...
		else if (!GpuCachedData->PendingEmitterData.IsValidIndex(iEmitter))
		{
			// In case we didn't get the change notification make sure we update the arrays
			GpuCachedData->CurrentEmitterData.SetNum(iEmitter + 1);
			GpuCachedData->PendingEmitterData.SetNum(iEmitter + 1);
		}
...
void FNiagaraDebugHud::Draw(FNiagaraWorldManager* WorldManager, UCanvas* Canvas, APlayerController* PC)
{
	...
	if (Settings.bHudRenderingEnabled && Canvas && Canvas->Canvas)
	{
...
void FNiagaraDebugHud::DrawValidation(class FNiagaraWorldManager* WorldManager, class FCanvas* DrawCanvas, FVector2f& TextLocation)
{
	...
	if ( ValidationErrors.Num() > 0 )
	{
		...
		if (ErrorString.Len() > 0)
		{
			...
			{
				UE_LOGF(LogNiagara, Warning, "Validation Errors - %ls", ErrorString.ToString());
			}
		}
	}
}
