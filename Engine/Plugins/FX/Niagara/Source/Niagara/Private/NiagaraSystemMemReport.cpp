	...
	static FAutoConsoleCommandWithWorldAndArgs CmdReportSystemMem(
		...
		FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
			[](const TArray<FString>& Args, UWorld*)
			{
				...
				UE_LOGF(LogNiagara, Log, "============ Niagara MemReport");
				for (TObjectIterator<UNiagaraSystem> SystemIt; SystemIt; ++SystemIt)
				{
					...
					{
						UE_LOGF(LogNiagara, Log, "%*ls = %ls %ls (%u, %u)", Node.Depth, TEXT(""), *Node.ObjectClass.ToString(), *Node.ObjectName.ToString(), uint32(Node.ExclusiveSizeBytes), uint32(Node.InclusiveSizeBytes));
					}
...
void FNiagaraSystemMemReport::GenerateReport(EReportType ReportType, UNiagaraSystem* System)
{
	...
	GatherResourceMemory(ReportType, System, System, 0);
}

uint64 FNiagaraSystemMemReport::GatherResourceMemory(EReportType ReportType, UNiagaraSystem* OwningSystem, UObject* Object, uint32 Depth)
{
	...
	if (ReportType == EReportType::Verbose)
	{
		if (UNiagaraScript* NiagaraScript = Cast<UNiagaraScript>(Object))
		{
			...
			if (const FVersionedNiagaraEmitterData* EmitterData = VersionedNiagaraEmitter.GetEmitterData())
			{
				...
				const FNiagaraEmitterHandle* EmitterHandle = OwningSystem->GetEmitterHandles().FindByPredicate([&EmitterData]
					(const FNiagaraEmitterHandle& EmitterHandle) { return EmitterHandle.GetEmitterData() == EmitterData; });
				EmitterHandleId = EmitterHandle != nullptr ? EmitterHandle->GetId() : FGuid();
			}

			const FNiagaraScriptRuntimeData* ScriptRuntimeData = OwningSystem->GetScriptRuntimeData(EmitterHandleId, NiagaraScript->GetUsage(), NiagaraScript->GetUsageId()).Get();
			if (const FNiagaraScriptExecutionParameterStore* ExecParameterStore = ScriptRuntimeData != nullptr ? ScriptRuntimeData->GetExecutionReadyParameterStore() : nullptr)
			{
	...
	GetObjectsWithOuter(Object, SubObjects, EGetObjectsFlags::None);
	if (SubObjects.Num() > 0)
	{
	#if WITH_EDITORONLY_DATA
		...
		{
			Nodes[NodeIndex].InclusiveSizeBytes += GatherResourceMemory(ReportType, OwningSystem, SubObject, Depth + 1);
		}
	}
