...
void FNiagaraScriptExecutionParameterStore::InitFromOwningScript(UNiagaraScript* Script, const FNiagaraScriptRuntimeCompiledData* ScriptRuntimeCompiledData, bool bNotifyAsDirty)
{
	//TEMPORARTY:
	...
	if (Script != nullptr && ScriptRuntimeCompiledData != nullptr)
	{
		AddScriptParams(Script, ScriptRuntimeCompiledData);
...
void FNiagaraScriptExecutionParameterStore::AddScriptParams(UNiagaraScript* Script, const FNiagaraScriptRuntimeCompiledData* ScriptRuntimeCompiledData)
{
	if (Script == nullptr || ScriptRuntimeCompiledData == nullptr)
	{
		return;
	...
	if (ScriptRuntimeCompiledData->SimTarget != ENiagaraSimTarget::GPUComputeSim)
	{
		ExecutableData.BakeScriptLiterals(CachedScriptLiterals);
	}

	check(Script->GetVMExecutableData().DataInterfaceInfo.Num() == ScriptRuntimeCompiledData->ResolvedDataInterfaces.Num());
	for (const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDataInterface : ScriptRuntimeCompiledData->ResolvedDataInterfaces)
	{
	...
	for (const FNiagaraResolvedUObjectInfo& ResolvedUObject : ScriptRuntimeCompiledData->ResolvedUObjects)
	{
		int32 VarOffset = INDEX_NONE;
		bAdded |= AddParameter(ResolvedUObject.ResolvedVariable, false, false, &VarOffset);
		if (GetUObjects().IsValidIndex(VarOffset))
		{
			SetUObject(ResolvedUObject.Object, VarOffset);
		}
	}
}
#endif
...
void FNiagaraScriptInstanceParameterStore::InitFromOwningContext(UNiagaraScript* Script, const FNiagaraScriptRuntimeData* InScriptRuntimeData, bool bNotifyAsDirty)
{
	const FNiagaraScriptExecutionParameterStore* SrcStore = InScriptRuntimeData != nullptr 
		? InScriptRuntimeData->GetExecutionReadyParameterStore() : nullptr;
