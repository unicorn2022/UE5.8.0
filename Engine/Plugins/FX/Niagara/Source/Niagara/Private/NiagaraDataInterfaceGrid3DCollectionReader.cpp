...
bool UNiagaraDataInterfaceGrid3DCollectionReader::InitPerInstanceData(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance)
{
	...
	if (EmitterInstanceToUse != nullptr)
	{
		...
		if (ExecContext != nullptr && ExecContext->GPUScript != nullptr && ExecContext->GetScriptRuntimeData() != nullptr)
		{
			TConstArrayView<FNiagaraScriptResolvedDataInterfaceInfo> DataInterfaceInfo = ExecContext->GetScriptRuntimeData()->GetResolvedDataInterfaces();
			...
			{
				if (DataInterfaceInfo[Index].CompileName.GetPlainNameString() == FullName)
				{
		...
		if (GridInterface && ProxyToUse)
		{
			...
			{
				UE_LOGF(LogNiagara, Error, "RGBA grids and Grid3DCollectionReaders are not compatible with Emitter %ls and DataInterface %ls.  Turn off RGBA grids with fx.Niagara.Grid3D.UseRGBAGrid 0, or split up your grid collections so the one passed to this reader has only one attribute.", *this->EmitterName, *this->DIName);
				return false;
			}
		...
		{
			UE_LOGF(LogNiagara, Error, "Emitter %ls doesn't have Grid3D Collection DI named %ls for DI: %ls", *this->EmitterName, *this->DIName, *Proxy->GetSourceDIName().ToString());
		}
	}
	else
	{
		UE_LOGF(LogNiagara, Error, "Emitter %ls doesn't exist for DI: %ls", *this->EmitterName, *Proxy->GetSourceDIName().ToString());
	}
...
bool UNiagaraDataInterfaceGrid3DCollectionReader::PerInstanceTickPostSimulate(void* PerInstanceData, FNiagaraSystemInstance* SystemInstance, float DeltaSeconds)
{
	...
	if (InstanceData->OtherInstanceData == nullptr && InstanceData->OtherDI)
	{
		...
		{
			UE_LOGF(LogNiagara, Error, "%ls: The DI for Emitter %ls and DataInterface %ls is not initialized.", *Proxy->GetSourceDIName().ToString(), *this->EmitterName, *this->DIName);
			return false;
		}
