...
public:
	...
	*/
	...
	NIAGARA_API void SetParameter_InternalUseOnly(const FNiagaraVariableBase& Key, const FNiagaraVariant& Value, bool bAdd=false);
	NIAGARA_API void GetParameter_InternalUseOnly(const FNiagaraVariableBase& Key, FNiagaraVariant& Value)const;
...
inline void FNiagaraParameterStoreBinding::Empty(FNiagaraParameterStore* DestStore, FNiagaraParameterStore* SrcStore)
{
	if (DestStore)
	{
		//UE_LOGF(LogNiagara, Log, "Remove Src Binding: Src: 0x%p - Dst: 0x%p", SrcStore, DestStore);
...
inline bool FNiagaraParameterStoreBinding::Initialize(FNiagaraParameterStore* DestStore, FNiagaraParameterStore* SrcStore, const FNiagaraBoundParameterArray* BoundParameters)
{
	...
	{
		//UE_LOGF(LogNiagara, Log, "Add Src Binding: Src: 0x%p - Dst: 0x%p", SrcStore, DestStore);
...
struct FNiagaraParameterDirectBinding<UObject*>
{
	...
	TObjectPtr<UObject> Init(FNiagaraParameterStore& InStore, const FNiagaraVariable& DestVariable)
	{
		if (DestVariable.IsValid())
		{
			...
			const FNiagaraVariableWithOffset* VariableWithOffset = BoundStore->FindParameterVariable(DestVariable, true);
			UObjectOffset = VariableWithOffset && VariableWithOffset->IsUObject() && !VariableWithOffset->IsDataInterface() ? VariableWithOffset->Offset : INDEX_NONE;
