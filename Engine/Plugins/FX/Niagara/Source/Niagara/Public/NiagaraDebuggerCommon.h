...
struct FNiagaraOutlinerEmitterInstanceData
{
	...
	UPROPERTY(VisibleAnywhere, Category = "Emitter")
	uint32 bRequiresPersistentIDs : 1 = false;

	UPROPERTY(VisibleAnywhere, Category = "Emitter")
	uint32 bIsLightweight : 1 = false;
...
struct FNiagaraDebugHUDSettingsData
{
	...
	/** When enabled the overview display will include Niagara FX. */
	UPROPERTY(EditAnywhere, Category = "Debug Overview", AdvancedDisplay)
	...
	/** Master control for enabling and disabling filtering by asset / actor / component */
	UPROPERTY(EditAnywhere, Category = "Debug Filter")
