...
#define NIAGARA_COMPUTEDEBUG_ENABLED_IN_EDITOR	1
#define NIAGARA_COMPUTEDEBUG_ENABLED_IN_COOKED	0
#define NIAGARA_COMPUTEDEBUG_ENABLED ((WITH_EDITOR && NIAGARA_COMPUTEDEBUG_ENABLED_IN_EDITOR) || (!WITH_EDITOR && NIAGARA_COMPUTEDEBUG_ENABLED_IN_COOKED))
...
struct FNiagaraFunctionSignature
{
	...
	* This is useful if the default value for an input makes no sense and would lead to a failed function call at runtime.
	...
	FNiagaraFunctionSignature(FName InName, TConstArrayView<FNiagaraVariable> InInputs, TConstArrayView<FNiagaraVariableBase> InOutputs, bool bInRequiresContext, bool bInMemberFunction)
		: Name(InName)
		, Inputs(InInputs)
		, Outputs(InOutputs)
...
USTRUCT()
struct FNiagaraScriptDataKey
{
	GENERATED_BODY();

	FNiagaraScriptDataKey()
		: ScriptUsage(ENiagaraScriptUsage::Function)
	{
	}

	FNiagaraScriptDataKey(const FGuid& InEmitterHandleId, ENiagaraScriptUsage InScriptUsage, const FGuid& InScriptUsageId)
	...
	NIAGARA_API FNiagaraScriptDataKey(const FNiagaraEmitterHandle* EmitterHandle, const UNiagaraScript& Script);

	FORCEINLINE bool operator==(const FNiagaraScriptDataKey& Other) const
	{
		return EmitterHandleId == Other.EmitterHandleId && 
			ScriptUsage == Other.ScriptUsage &&
			ScriptUsageId == Other.ScriptUsageId;
	}

	friend uint32 GetTypeHash(const FNiagaraScriptDataKey& Ref)
	{
		uint32 Hash = GetTypeHash(Ref.EmitterHandleId);
		Hash = HashCombine(Hash, GetTypeHash(Ref.ScriptUsage));
		Hash = HashCombine(Hash, GetTypeHash(Ref.ScriptUsageId));
		return Hash;
	}
...
namespace FNiagaraUtilities
{
	...
	*/
	NIAGARA_API TOptional<EPixelFormat> BufferFormatToPixelFormat(ENiagaraGpuBufferFormat NiagaraFormat, EPixelFormatCapabilities RequiredCapabilities, int NumberOfChannels, UObject* RequesterForDebugging);
	...
	If the return value optional is invalid that means no format was found.
	*/
	NIAGARA_API TOptional<ETextureRenderTargetFormat> BufferFormatToRenderTargetFormat(ENiagaraGpuBufferFormat NiagaraFormat, EPixelFormatCapabilities RequiredCapabilities, UObject* RequesterForDebugging);
	...
	UE_DEPRECATED(5.8, "Please update your code to include the caller information.")
...
struct FNiagaraUserParameterBinding
{
	...
	/** Link to a User Parameter to control a value within a Niagara System. */
