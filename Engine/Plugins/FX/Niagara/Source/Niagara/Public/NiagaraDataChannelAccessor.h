...
public:
	...
	bool IsInitialized(UNiagaraDataChannel* NDC)const { return CachedLayout.IsValid() && !CachedLayout.IsUnique() && NDC && CachedLayout == NDC->GetLayoutInfo(); }
...
struct FNDCPackedData
{
	int32 GetData() { return static_cast<int32>(PackedData); }//convert to int32 for passing into Niagara.

protected:
	uint32 PackedData = 0;

	template <typename T, uint32 Shift, uint32 NumBits>
	inline void Set(T Value)
	{
		static_assert(std::is_integral_v<T>, "Must use integral types for NDC packed data.");
		static_assert(!std::is_signed_v<T>, "Signed values not supported currently but could be.");
		static_assert(Shift < 32, "Shift cannot exceeded size of packed uint32.");
		static_assert(NumBits > 0, "Cannot pack zero bits.");
		static_assert(NumBits < 32, "Cannot pack more bits that the underlying uint32.");		
		static_assert(Shift + NumBits < 32, "Cannot pack more bits that the underlying uint32.");

		uint32 V;
		if constexpr(std::is_same_v<T,bool>)
		{
			static_assert(NumBits == 1, "Bools should be packed into one bit.");
			V = !!Value;
		}
		else
		{
		...
		PackedData &= ~(Mask << Shift);
		PackedData |= (V & Mask) << Shift; 
	}
	...
	inline T Get()
	{
		...
		uint32 Mask = (1u << NumBits) - 1u;
		uint32 V = (PackedData >> Shift) & Mask;
		if constexpr(std::is_same_v<T,bool>)
		{
			static_assert(NumBits == 1, "Bools should be packed into one bit.");
			return V != 0;
...
#define NDCVarPackedValue(Name, Type, Shift, NumBits) \
void Set_##Name(Type Value){ Set<Type, Shift, NumBits>(Value); }\
Type Get_##Name(){ return Get<Type, Shift, NumBits>(); }
