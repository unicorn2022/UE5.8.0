...
public:
	...
	NIAGARA_API virtual void GetUsedMeshes(const FNiagaraEmitterInstance* InEmitter, TArray<UObject*>& OutMeshes) const override;
...
#if WITH_EDITOR
	// Old function should not be called by user code but is in some instances
	NIAGARA_API void OnMeshChanged();
	// Internal API call for handling mesh changes (reimport / modification / etc)
	NIAGARA_API static void OnMeshChanged(UObject* MeshObject);
	// Internal API to remove all of the mesh changed handlers
	NIAGARA_API static void RemoveMeshChangedHandlers();
#endif
