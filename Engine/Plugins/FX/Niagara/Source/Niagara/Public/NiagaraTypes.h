...
private:
	UPROPERTY(EditAnywhere, Category = Parameters)// Must be either FNiagaraBool::True(-1) or FNiagaraBool::False(0).
	int32 Value = False;
};
...
struct FNiagaraStructConversionStep
{
	...
	void CopyInternal(uint8* DestinationData, int32 DestStride, int32 DestOffset, const uint8* SourceData, int32 SourceStride, int32 SourceOffset, int32 Count)const
	{
		...
		{
			TSrcType SrcValue;
			FMemory::Memcpy(&SrcValue, (SourceData + i * SourceStride) + SourceOffset, sizeof(TSrcType));

			TDstType DstValue = TDstType(SrcValue);
			FMemory::Memcpy((DestinationData + i * DestStride) + DestOffset, &DstValue, sizeof(TDstType));
		}
	}
};
...
private:
	...
	friend inline uint32 GetPersistentTypeHash(const FNiagaraTypeDefinition& Type)
	{
		const FString PathName = GetPathNameSafe(Type.ClassStructOrEnum);
		return HashCombine(HashCombine(GetTypeHash(PathName), GetTypeHash(Type.UnderlyingType)), GetTypeHash(Type.GetFlags()));
	}
};
...
protected:
	...
	// note that this implementation uses the hash of the variable's FName which is not consistent from run to run.  Use GetPersistentTypeHash()
	// if the hash is being used in serialized data
	...
	friend inline uint32 GetPersistentTypeHash(const FNiagaraVariableBase& Var)
	{
		return HashCombine(GetPersistentTypeHash(Var.GetType()), GetTypeHash(Var.GetName().ToString()));
	}
};
