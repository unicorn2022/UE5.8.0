...
struct FNiagaraUserParameterInfo
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Niagara)
	TObjectPtr<UStruct> ParameterType;
...
USTRUCT(MinimalAPI, BlueprintType)
struct FNiagaraMinimalEmitterInfo
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Niagara)
	TArray<TObjectPtr<UMaterialInterface>> UsedRendererMaterials;

	// A list of all meshes referenced by renderers in this emitter. Could actually be less on the cooked target platform, depending on scalability settings.
	// This list does NOT include meshes from user parameters or data interfaces, which are resolved at runtime. Also, this covers only the meshes reported by
	// renderers, the emitter itself might use more meshes in other places, for example for skeletal mesh sampling.
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Niagara)
	TArray<TObjectPtr<UObject>> UsedRendererMeshes;
};
...
public:
	...
	*/
	...
	static NIAGARA_API void SetSceneCapture2DDataInterfaceManagedMode(UNiagaraComponent* NiagaraSystem, const FName& DIName,
		...
		float ManagedLODDistanceFactor = 1.0f);
	...
	/** Returns basic info on all user parameters of a niagara system.*/
	UFUNCTION(BlueprintCallable, Category = Niagara)
	static NIAGARA_API TArray<FNiagaraUserParameterInfo> GetAllUserParameters(UNiagaraSystem* NiagaraSystem);

	/** Returns basic info on all emitters of a niagara system.*/
	UFUNCTION(BlueprintCallable, Category = Niagara)
	static NIAGARA_API TArray<FNiagaraMinimalEmitterInfo> GetAllEmitters(UNiagaraSystem* NiagaraSystem);
...
// 	static void SetUpdateScriptConstant(UNiagaraComponent* Component, FName EmitterName, FName ConstantName, FVector Value);
	...
	UFUNCTION(BlueprintCallable, Category = Niagara, meta = (Keywords = "niagara parameter collection", WorldContext = "WorldContextObject", UnsafeDuringActorConstruction = "true"))
	...
	/**
	Create a parameter collection instance for the provided parameter collection.
	This can return null if not in a game world or your passed in a null collection.
	*/
	UFUNCTION(BlueprintCallable, Category = Niagara, meta = (Keywords = "niagara parameter collection", WorldContext = "WorldContextObject", UnsafeDuringActorConstruction = "true"))
	static NIAGARA_API UNiagaraParameterCollectionInstance* CreateNiagaraParameterCollectionInstance(UObject* WorldContextObject, UNiagaraParameterCollection* ParentCollection);
