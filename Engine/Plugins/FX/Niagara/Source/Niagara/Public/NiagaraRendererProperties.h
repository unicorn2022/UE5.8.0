...
public:
	...
	/** Returns a list of meshes used by this renderer. What type of object meets the definition of a mesh is up to the renderer to decide, so this might return more than just static meshes (e.g. geometry caches or skel meshes).
	 * If the emitter instance is not given then the values returned might not be representative of the meshes used at runtime. */
	virtual void GetUsedMeshes(const FNiagaraEmitterInstance* InEmitter, TArray<UObject*>& OutMeshes) const {}
	...
	*/
	...
	NIAGARA_API void GetMaterialUsageFeedback(EMaterialUsage Usage, TConstArrayView<EMaterialDomain> InvalidMaterialDomains, TArray<FNiagaraRendererFeedback>& OutFeedback) const;
	NIAGARA_API void GetMaterialUsageFeedback(EMaterialUsage Usage, TArray<FNiagaraRendererFeedback>& OutFeedback) const;
...
#if WITH_EDITORONLY_DATA
	// Checks to see if th renderer is contained with it's owning emitters renderer array
	// If not this is likely an orphaned or invalid renderer
	[[nodiscard]] bool IsContainedInEmitterRendererArray() const;
#endif

	NIAGARA_API void InitRendererBaseBindings();
	...
	UPROPERTY(EditAnywhere, Category = "Rendering")
	ENiagaraRendererMotionVectorSetting MotionVectorSetting = ENiagaraRendererMotionVectorSetting::AutoDetect;
...
protected:
	...
	If your renderer uses static draw lists and you are unsure if the data could still be inside the renderer make sure to pass a valid material update context.
	The default is to skip updating anything as Niagara system should be recreated as part of the caller to update the MICs.
	*/
	NIAGARA_API void UpdateMaterialParametersMIC(const FNiagaraRendererMaterialParameters& MaterialParameters, TObjectPtr<UMaterialInterface>& InOutMaterial, TObjectPtr<UMaterialInstanceConstant>& InOutMIC, FMaterialUpdateContext* OptionalMaterialUpdateContext = nullptr);
	NIAGARA_API void UpdateMaterialParametersMIC(const FNiagaraRendererMaterialParameters& MaterialParameters, TArrayView<UMaterialInterface*> Materials, TArray<TObjectPtr<UMaterialInstanceConstant>>& InOutMICs, FMaterialUpdateContext* OptionalMaterialUpdateContext = nullptr);
