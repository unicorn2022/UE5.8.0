...
public:
	...
	void RebindParameterCollection(UNiagaraParameterCollectionInstance* OldCollection, UNiagaraParameterCollectionInstance* NewCollection);
...
//	FVector GetOwnerVelocity() const { return OwnerVelocityParam.GetValue(); }
	...
	inline USceneComponent* GetAttachComponent() const { return WeakAttachComponent.Get(); }
	...
	FORCEINLINE int32 GetPerInstanceDIFunctionNum(ENiagaraSystemSimulationScript ScriptType) const { return PerInstanceDIFunctions[(int32)ScriptType].Num(); }
#endif
	...
	const FNiagaraParameterCollectionResolver CreateParameterCollectionResolver() const;
...
private:
	...
	TWeakObjectPtr<USceneComponent> WeakAttachComponent;
