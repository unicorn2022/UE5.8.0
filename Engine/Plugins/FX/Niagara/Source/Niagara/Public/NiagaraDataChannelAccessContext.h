...
struct FNDCAccessContext : public FNDCAccessContextBase
{
	...
	/** If true, SpawnedSystems will be filled with both existing and newly spawned systems. If false, SpawnedSystems will contain only newly spawned systems. */
	UPROPERTY(BlueprintReadWrite, Category = "Input", meta = (NDCAccessContextInput))
	uint32 bReturnExistingSystems : 1 = false;
	...
	If bReturnExistingSystems is set, this will contain both new and existing systems matching this access context.
	*/
	...
	void ForEachSpawnedSystem(TAction Func) const;

	template<typename TContainerType>
	void GetSpawnedSystems(TContainerType& Container) const;
};
...
struct TNDCAccessContextType
{
	...
	[[nodiscard]] explicit TNDCAccessContextType(UScriptStruct* InStruct)
		: AccessContextStruct(InStruct && InStruct->IsChildOf(FNDCAccessContextBase::StaticStruct()) ? InStruct : nullptr)
	{
