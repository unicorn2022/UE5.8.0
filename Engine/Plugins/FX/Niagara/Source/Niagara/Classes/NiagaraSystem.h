...
class UNiagaraScriptRuntimeCompiledDataEditorReference;
...
public:
	...
	/** Performs the passed action for all scripts in this system and provides the script runtime data.
		Include NiagaraSystemImpl.h to be able to call this function */
	template<typename TAction>
	inline void ForEachScriptWithRuntimeData(TAction Func) const;

	/** Performs the passed action for all scripts in this system and provides the owning system and emitter handle.
		Include NiagaraSystemImpl.h to be able to call this function */
	template<typename TAction>
	inline void ForEachScriptWithOwningContext(TAction Func) const;
	...
	NIAGARA_API virtual void PrecacheAssetPSOs() override;
...
public:
	...
	UPROPERTY()
	TObjectPtr<UObject> TypeContainer;
	...
	const UNiagaraScript* GetTargetCompiledScript(const FGuid& EmitterHandleId, ENiagaraScriptUsage ScriptUsage, const FGuid& ScriptUsageId) const
	{
		return GetTargetCompiledScript(FNiagaraScriptDataKey(EmitterHandleId, ScriptUsage, ScriptUsageId));
	}

	/** Gets the script that will have the compiled data for the supplied script data key. */
	UNiagaraScript* GetTargetCompiledScript(const FNiagaraScriptDataKey& ScriptDataKey) const;

	const FNiagaraSystemCompiledData& GetSystemCompiledData() const { return SystemCompiledData; };

	TSharedPtr<const FNiagaraScriptRuntimeData> GetScriptRuntimeData(const FNiagaraEmitterHandle* EmitterHandle, const UNiagaraScript& Script) const
	{
		return GetScriptRuntimeData(FNiagaraScriptDataKey(EmitterHandle, Script));
	}

	TSharedPtr<const FNiagaraScriptRuntimeData> GetScriptRuntimeData(const FGuid& EmitterHandleId, ENiagaraScriptUsage ScriptUsage, const FGuid& ScriptUsageId) const
	{
		return GetScriptRuntimeData(FNiagaraScriptDataKey(EmitterHandleId, ScriptUsage, ScriptUsageId));
	}

	NIAGARA_API TSharedPtr<const FNiagaraScriptRuntimeData> GetScriptRuntimeData(const FNiagaraScriptDataKey& ScriptDataKey) const;

private:
#if WITH_EDITORONLY_DATA
	struct FEditorRuntimeCompiledDataHandleDeleter
	{
		FEditorRuntimeCompiledDataHandleDeleter(const UNiagaraSystem* System);

		void operator()(FNiagaraScriptRuntimeCompiledDataHandle* ObjectToDelete) const;

		TWeakObjectPtr<const UNiagaraSystem> SystemWeak;
	};

	void NotifyScriptRuntimeCompiledDataHandleDeleted(const FObjectKey& OwnerEditorReferenceKey, const FNiagaraScriptDataKey& OwnerScriptDataKey) const;
#endif

	TSharedPtr<const FNiagaraScriptRuntimeCompiledDataHandle> CreateScriptRuntimeCompiledDataHandle(const FNiagaraScriptDataKey& ScriptDataKey) const;
...
public:
	...
	*/
	...
	NIAGARA_API void FindDataInterfaceDependencies(const FGuid& EmitterHandleId, const UNiagaraScript* Script, TArray<FVersionedNiagaraEmitter>& Dependencies);
...
public:
	...
	const FNiagaraReadinessSettings& GetReadinessSettings() const;
	ENiagaraEmitterImportance GetHighestEmitterImportance() const { return MaxEmitterImportance; }
	...
	NIAGARA_API const FNiagaraSystemStateData& GetSystemStateData() const;
...
private:
	...
	*/
	...
	/** Helper for filling in precomputed variable names per emitter. Converts an emitter parameter "Emitter.XXXX" into it's real parameter name. */
	...
	NIAGARA_API void InitScriptCompiledData();

	void InvalidateScriptRuntimeData();

	void PreSaveScriptRuntimeData(FObjectPreSaveContext ObjectSaveContext);
#endif
	...
	void PostLoadScriptRuntimeData();
...
protected:
...
#if WITH_EDITORONLY_DATA
	UPROPERTY()
	TObjectPtr<UNiagaraScriptRuntimeCompiledDataEditorReference> ScriptRuntimeCompiledDataForEditor;

	UPROPERTY(transient)
	mutable TArray<TObjectPtr<UNiagaraScriptRuntimeCompiledDataEditorReference>> StaleRuntimeCompiledDataForEditor;

	mutable TMap<FObjectKey, TArray<FNiagaraScriptDataKey>> StaleRuntimeCompiledDataUsageMap;

#endif
	mutable TMap<FNiagaraScriptDataKey, TSharedPtr<FNiagaraScriptRuntimeData>> ScriptRuntimeDataMap;

	UPROPERTY()
	TMap<FNiagaraScriptDataKey, FNiagaraScriptRuntimeCookedData> ScriptRuntimeCookedDataMap;
	...
	/**
	Initial streaming bounds are only included when the streaming system asks for the initial bounds,
	...
	*/
	...
	UPROPERTY()
	ENiagaraEmitterImportance MaxEmitterImportance = ENiagaraEmitterImportance::Low;
...
protected:
	...
	struct FStaticBuffersCachingFlags
	{
		bool bIsExternalDataInterface = false;
		bool bIsUsedByCpu = false;
		bool bIsUsedByGpu = false;
	};
