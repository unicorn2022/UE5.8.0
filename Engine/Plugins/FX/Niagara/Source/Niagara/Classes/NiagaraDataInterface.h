...
{\
	...
	static void Bind(UNiagaraDataInterface* Interface, const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction &OutFunc)\
	{\
		OutFunc.BindUObject(static_cast<ClassName*>(Interface), &ClassName::FuncName<ParamTypes...>);\
	}\
};
...
{\
	...
	{\
		OutFunc.BindUObject(static_cast<ClassName*>(Interface), &ClassName::FuncName);\
	}\
};
...
struct FNiagaraDataInterfaceProxy
{
	...
	inline FName GetSourceDIName() const
	{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return IsInGameThread() || IsInParallelGameThread() ? SourceDIName : SourceDIName_RT;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	void SetSourceDIName(FName InName);
	...
	UE_DEPRECATED(5.8, "Please update your API to use the get API to read SourceDIName")
	FName SourceDIName;

private:
	FName SourceDIName_RT;
};
...
struct FNDIStageTickHandler
{
	...
	void Init(ENiagaraScriptUsage ScriptUsage, const FNiagaraScriptRuntimeData* ScriptRuntimeData, FNiagaraSystemInstance* Instance);
...
public:
	...
	virtual void CacheStaticBuffers(struct FNiagaraSystemStaticBuffers& StaticBuffers, bool bIsExternalDataInterface, bool bUsedByCPU, bool bUsedByGPU) {}

	virtual bool NeedsGPUContextInit() const { return false; }
	virtual bool GPUContextInit(const FNiagaraScriptResolvedDataInterfaceInfo& ResolvedDataInterfaceData, const TArray<FVMExternalFunctionBindingInfo>& CalledFunctions, void* PerInstanceData, FNiagaraSystemInstance* SystemInstance) const { return false; }
