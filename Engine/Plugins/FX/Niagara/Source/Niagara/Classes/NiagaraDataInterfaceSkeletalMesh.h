...
struct FSkeletalMeshSkinningData
{
	explicit FSkeletalMeshSkinningData(TWeakObjectPtr<USkinnedMeshComponent> InWeakMeshComponent)
		: WeakMeshComponent(InWeakMeshComponent)
		, DeltaSeconds(.0333f)
		, CurrIndex(0)
		, BoneMatrixUsers(0)
	...
	TWeakObjectPtr<USkinnedMeshComponent> GetWeakMeshComponent() const { return WeakMeshComponent; }
...
private:
	...
	TWeakObjectPtr<USkinnedMeshComponent> WeakMeshComponent;
...
class FNDI_SkeletalMesh_GeneratedData : public FNDI_GeneratedData
{
	...
	TMap<TWeakObjectPtr<USkinnedMeshComponent>, TSharedPtr<FSkeletalMeshSkinningData> > CachedSkinningData;
...
public:
	NIAGARA_API FSkeletalMeshSkinningDataHandle GetCachedSkinningData(TWeakObjectPtr<USkinnedMeshComponent> InComponent, FSkeletalMeshSkinningDataUsage Usage, bool bNeedsDataImmediately);
	NIAGARA_API FSkeletalMeshUvMappingHandle GetCachedUvMapping(TWeakObjectPtr<USkinnedAsset> InMeshObject, int32 InLodIndex, int32 InUvSetIndex, FMeshUvMappingUsage Usage, bool bNeedsDataImmediately);
	NIAGARA_API FSkeletalMeshConnectivityHandle GetCachedConnectivity(TWeakObjectPtr<USkinnedAsset> InMeshObject, int32 InLodIndex, FSkeletalMeshConnectivityUsage Usage, bool bNeedsDataImmediately);
...
enum class ENDISkeletalMesh_SourceMode : uint8
{
	...
	*/
	...
	/**
	Search for the component as a child of the Niagara component.
	It does not recurse the hierarchy, only looking for direct children.
	*/
	ChildrenOnly,
};
...
public:
	...
	USceneComponent* GetSceneComponent() const { return WeakSceneComponent.Get(); }
...
protected:
	TWeakObjectPtr<USceneComponent> WeakSceneComponent = nullptr;
...
public:
	...
	FSkeletalMeshGpuDynamicBufferProxy() = default;
	virtual ~FSkeletalMeshGpuDynamicBufferProxy() override = default;
	...
	uint32 GetSectionBufferNumElements() const { return SectionBoneCount; }
	uint32 GetSamplingBufferNumElements() const { return SamplingBoneCount + SamplingSocketCount; }
...
struct FNDISkeletalMesh_InstanceData
{
	/** Cached ptr to Scene Component we sample from, when found. Otherwise, the scene component to use to transform the PreviewMesh */
	...
	TWeakObjectPtr<USceneComponent> WeakCachedAttachParent;

	UObject* CachedUserParam;

	TWeakObjectPtr<USkinnedAsset> WeakSkinnedMesh;
	...
	{
		USkinnedMeshComponent* SkinnedComponent = Cast<USkinnedMeshComponent>(WeakSceneComponent.Get());
		if (SkinnedComponent != nullptr && SkinnedComponent->GetSkinnedAsset() != nullptr)
		{
			return SkinnedComponent->GetSkinWeightBuffer(CachedLODIdx);
		}
...
protected:
	...
	/** Class type we should look for when attaching defaults to USkeletalMeshComponent if unset */
	UPROPERTY(EditAnywhere, Category = "Mesh")
	TSubclassOf<USkinnedMeshComponent> OptionalComponentClass;
	...
	UPROPERTY(Transient)
	TObjectPtr<USkinnedMeshComponent> SourceComponent;
...
public:
	...
	*/
	...
	NIAGARA_API USkinnedAsset* GetSkinnedMesh(FNiagaraSystemInstance* SystemInstance, USceneComponent* AttachComponent, TWeakObjectPtr<USceneComponent>& SceneComponent, USkinnedMeshComponent*& FoundSkinnedComponent, FNDISkeletalMesh_InstanceData* InstData);
	/** Finds the skeletal mesh based on settings of the DI and the hierarchy of the object provided */
	NIAGARA_API USkinnedAsset* GetSkinnedMesh(UNiagaraComponent* Component);

	USkinnedMeshComponent* GetSourceComponent() const { return SourceComponent; }
	AActor* GetSourceActor() const { return SoftSourceActor.Get(); }

	NIAGARA_API int32 CalculateLODIndexAndSamplingRegions(USkinnedAsset* SkinnedMesh, TArray<int32>& OutSamplingRegionIndices, bool& OutAllRegionsAreAreaWeighting) const;
...
public:
	...
	NIAGARA_API void SetSourceComponentFromBlueprints(USkinnedMeshComponent* ComponentToUse);
