...
class FNiagaraScriptComputeRuntimeData;
...
struct FNiagaraComputeExecutionContext : public INiagaraComputeDataBufferInterface
{
	...
	void InitParams(UNiagaraScript* InGPUComputeScript, TSharedPtr<const FNiagaraScriptRuntimeData> InScriptRuntimeData, const FNiagaraSimStageExecutionDataPtr& InSimStageExecData);
	...
	const FNiagaraScriptRuntimeData* GetScriptRuntimeData() const { return ScriptRuntimeData.Get(); }
...
public:
	...
	// Particle count read / write fence used when to avoid racing with the render thread
	// Read count is used on the game thread / game concurrent threads and allows us to read the particle count when <= the write fence
	// When the fence has not been passed we fallback to the potentially spawned particles
	std::atomic<uint32> ParticleCountReadFence = 1;
	std::atomic<uint32> ParticleCountWriteFence = 0;
	...
	UE_DEPRECATED(5.8, "Direct access of CurrentNumInstances_RT is deprecated please use the get / set methods")
	std::atomic<uint32> CurrentNumInstances_RT = 0;
	...
	void SetCurrentNumInstances(uint32 NumInstances)
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		CurrentNumInstances_RT.store(NumInstances, std::memory_order_relaxed);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	uint32 GetCurrentNumInstances() const
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return CurrentNumInstances_RT.load(std::memory_order_relaxed);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
