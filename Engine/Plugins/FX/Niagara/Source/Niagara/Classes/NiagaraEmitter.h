...
struct FMemoryRuntimeEstimation
{
	...
	UE_DEPRECATED(5.8, "Direct use of IsEstimationDirty is deprecated please use the get / set methods")
	std::atomic<bool> IsEstimationDirty = false;
	int32 AllocationEstimate = 0;

	FMemoryRuntimeEstimation();
	FMemoryRuntimeEstimation(const FMemoryRuntimeEstimation& Other) { *this = Other; }

	FMemoryRuntimeEstimation& operator=(const FMemoryRuntimeEstimation& Other)
	{
		EstimationCriticalSection = Other.EstimationCriticalSection;
		RuntimeAllocations = Other.RuntimeAllocations;
		SetIsEstimationDirty(Other.GetIsEstimationDirty());
		AllocationEstimate = Other.AllocationEstimate;
		return *this;
	}

	void SetIsEstimationDirty(bool bIsDirty)
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		IsEstimationDirty.store(bIsDirty, std::memory_order_relaxed);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

	bool GetIsEstimationDirty() const
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return IsEstimationDirty.load(std::memory_order_relaxed);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
...
struct FVersionedNiagaraEmitterData
{
	...
	UPROPERTY()
	TSubclassOf<UNiagaraVersionUpgradeBase> UpdateUtilityClass;
	...
	*/
	...
	// Determines how important this emitter is for the runtime and rendering of the system.
	// An emitter with low priority does not block system execution or other emitters from rendering if its own materials are not yet done compiling.
	// A critical emitter can block both activation and rendering of the system until it's ready.
	// See also: readiness settings in the effect type.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Emitter")
	ENiagaraEmitterImportance Importance = ENiagaraEmitterImportance::Normal;
	...
	**/
	...
	UNiagaraRendererProperties* GetRenderer(int32 RendererIndex)const { return RendererProperties.IsValidIndex(RendererIndex) ? RendererProperties[RendererIndex] : nullptr; }
	...
	NIAGARA_API void CacheFromCompiledData(const FNiagaraDataSetCompiledData* CompiledData, const UNiagaraEmitter& Emitter, const FGuid& EmitterHandleId);
	...
	/** Performs the passed action for all platform sets in this emitter.
	Include NiagaraEmitterImpl.h to be able to call this function */
	template<typename TAction>
	inline void ForEachPlatformSet(const UNiagaraSystem& OwningSystem, const FGuid& EmitterHandleId, TAction Func);
