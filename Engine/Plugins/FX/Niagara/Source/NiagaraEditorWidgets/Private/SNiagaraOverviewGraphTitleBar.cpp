...
void SNiagaraOverviewGraphTitleBar::RebuildWidget()
{
	...
	// Clear information about LWE & performance
	if (SystemViewModel.IsValid() && SystemViewModel.Pin()->GetEditMode() == ENiagaraSystemViewModelEditMode::SystemAsset)
	{
		ContainerWidget->AddSlot()
		.HAlign(HAlign_Fill)
		...
		[
			SNew(SBorder)
			.BorderImage(FAppStyle::GetBrush(TEXT("Graph.TitleBackground")))
			.ColorAndOpacity(this, &SNiagaraOverviewGraphTitleBar::GetStatelessInfoHeaderColor)
			.Visibility(this, &SNiagaraOverviewGraphTitleBar::GetStatelessInfoVisibility)
			.HAlign(HAlign_Fill)
			[
				SNew(STextBlock)
				.Text(this, &SNiagaraOverviewGraphTitleBar::GetStatelessInfoText)
				.TextStyle(FAppStyle::Get(), TEXT("GraphBreadcrumbButtonText"))
				.Justification(ETextJustify::Center)
			]
		];
...
EVisibility SNiagaraOverviewGraphTitleBar::GetSystemCompileModeHeaderVisibility() const
{
	if (SystemViewModel.IsValid())
	{
		...
		return NiagaraSystem.GetCompileForEdit() ? EVisibility::Collapsed : EVisibility::Visible;
	}
...
FLinearColor SNiagaraOverviewGraphTitleBar::GetStatelessInfoHeaderColor() const
{
	if (SystemViewModel.IsValid())
	{
		const UNiagaraSystem& NiagaraSystem = SystemViewModel.Pin()->GetSystem();
		const FNiagaraSystemStateData& SystemStateData = NiagaraSystem.GetSystemStateData();
		if (SystemStateData.bRunUpdateScript)
		{
			return FLinearColor::Red;
		}
		else if (SystemStateData.bRunSpawnScript)
		{
...
EVisibility SNiagaraOverviewGraphTitleBar::GetStatelessInfoVisibility() const
{
	if (SystemViewModel.IsValid())
	{
		const UNiagaraSystem& NiagaraSystem = SystemViewModel.Pin()->GetSystem();
		if (NiagaraSystem.GetEmitterHandles().Num() > 0)
		{
			bool bHasEnabledEmitters = false;
			for (const FNiagaraEmitterHandle& EmitterHandle : NiagaraSystem.GetEmitterHandles())
			{
				...
				bHasEnabledEmitters = true;
				if (EmitterHandle.GetEmitterMode() != ENiagaraEmitterMode::Stateless)
				{
					return EVisibility::Collapsed;
				}
			}
			return bHasEnabledEmitters ? EVisibility::Visible : EVisibility::Collapsed;
		}
	}
	return EVisibility::Collapsed;
}

FText SNiagaraOverviewGraphTitleBar::GetStatelessInfoText() const
{
	if (SystemViewModel.IsValid())
	{
		const UNiagaraSystem& NiagaraSystem = SystemViewModel.Pin()->GetSystem();
		...
		if (SystemStateData.bRunUpdateScript)
		{
			return LOCTEXT("LWEInfo_WorstPerf", "Lightweight Performance: (Normal) Spawn & update scripts are running");
		}
		else if (SystemStateData.bRunSpawnScript)
		{
			return LOCTEXT("LWEInfo_GoodPerf", "Lightweight Performance: (Good) Spawn script is running");
		}
		return LOCTEXT("LWEInfo_BestPerf", "Lightweight Performance: (Best) No scripts are running");
	}
