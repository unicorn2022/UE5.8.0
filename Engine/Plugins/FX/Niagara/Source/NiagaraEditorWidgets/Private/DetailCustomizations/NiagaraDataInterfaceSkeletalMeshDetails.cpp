...
void FNiagaraDataInterfaceSkeletalMeshDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	SkinnedMeshObject = Interface->GetSkinnedMesh(Cast<UNiagaraComponent>(Interface->GetOuter()));
	SkeletalMeshObject = Cast<USkeletalMesh>(SkinnedMeshObject);
	if (SkeletalMeshObject.IsValid())
	{
		SkeletalMeshObject->GetOnMeshChanged().RemoveAll(this);
		SkeletalMeshObject->GetOnMeshChanged().AddSP(this, &FNiagaraDataInterfaceSkeletalMeshDetails::OnDataChanged);
	}
...
void FNiagaraDataInterfaceSkeletalMeshDetails::GenerateRegionsArray(TArray<TSharedPtr<FName>>& SourceArray)
{
	...
	{
		if (USkeletalMesh* Mesh = Cast<USkeletalMesh>(Interface->GetSkinnedMesh(Cast<UNiagaraComponent>(Interface->GetOuter()))))
		{
 ...
 void FNiagaraDataInterfaceSkeletalMeshDetails::GenerateBonesArray(TArray<TSharedPtr<FName>>& SourceArray)
 {
	...
	if (UNiagaraDataInterfaceSkeletalMesh* Interface = MeshInterface.Get())
	{
		if (USkinnedAsset* Mesh = Interface->GetSkinnedMesh(Cast<UNiagaraComponent>(Interface->GetOuter())))
		{
