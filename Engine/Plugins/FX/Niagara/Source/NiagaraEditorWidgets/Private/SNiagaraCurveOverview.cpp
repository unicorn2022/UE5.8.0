...
#include "Misc/CurveEditorIntegrationUtils.h"
...
struct FNiagaraCurveOverviewTreeItem : public ICurveEditorTreeItem
{
	...
	virtual void CreateCurveModels(TArray<TUniquePtr<FCurveModel>>& OutCurveModels) override
	{
		if(CurveSelectionTreeNode->GetCurve() != nullptr && CurveSelectionTreeNode->GetCurveDataInterface().IsValid())
		{
			...
			NewCurve->SetChannelName(CurveSelectionTreeNode->GetCurveName());
	...
	static bool CompareTreeItems(const FCurveEditorTreeItem& ItemA, const FCurveEditorTreeItem& ItemB)
	{
		const FNiagaraCurveOverviewTreeItem& NiagaraItemA = static_cast<const FNiagaraCurveOverviewTreeItem&>(*ItemA.GetItem().Get());
		const FNiagaraCurveOverviewTreeItem& NiagaraItemB = static_cast<const FNiagaraCurveOverviewTreeItem&>(*ItemB.GetItem().Get());
...
void SNiagaraCurveOverview::Construct(const FArguments& InArgs, TSharedRef<FNiagaraSystemViewModel> InSystemViewModel)
{
	...
	InitParams.ResolveCurveModelDelegate.BindStatic(&UE::NiagaraEditorWidgets::ResolveNiagaraCurveModel); // Needed for undo / redo.
	CurveEditor->InitCurveEditor(InitParams);
	CurveEditor->GridLineLabelFormatXAttribute = LOCTEXT("GridXLabelFormat", "{0}");
	CurveEditor->GetTree()->SetSortPredicate(FCurveEditorTree::FCurveTreeItemSortPredicate::CreateStatic(&FNiagaraCurveOverviewTreeItem::CompareTreeItems));
