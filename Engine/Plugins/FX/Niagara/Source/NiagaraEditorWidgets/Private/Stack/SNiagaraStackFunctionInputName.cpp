...
void SNiagaraStackFunctionInputName::FillRowContextMenu(class FMenuBuilder& MenuBuilder)
{
	...
	{
		TArray<FNiagaraStackFunctionInputActions> CustomActions = FunctionInput->GetCustomActions();
		if (CustomActions.Num() > 0)
		{
			MenuBuilder.BeginSection("InputActions", LOCTEXT("InputActions", "Input Actions"));
			for (const FNiagaraStackFunctionInputActions& Action : CustomActions)
			{
				MenuBuilder.AddMenuEntry(Action.Label, Action.ToolTip, FSlateIcon(), FUIAction(Action.ExecuteAction));
			}
			MenuBuilder.EndSection();
		}
	}
