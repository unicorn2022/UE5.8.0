...
void FNiagaraStackPropertyRowUtilities::OnFillPropertyRowContextMenu(FMenuBuilder& MenuBuilder, TWeakObjectPtr<UNiagaraStackPropertyRow> PropertyRowWeak, FNodeWidgetActions PropertyNodeWidgetActions)
{
	...
	// Allow the property customization to contribute context menu actions regardless of copy/paste support.
	if (TSharedPtr<const FNiagaraStackObjectPropertyCustomization> Customization = PropertyRow->GetCustomization())
	{
		Customization->GenerateContextMenuActions(PropertyRow, MenuBuilder);
	}
