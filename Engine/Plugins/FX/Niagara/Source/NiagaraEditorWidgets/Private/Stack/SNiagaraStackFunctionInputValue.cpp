...
class SNiagaraStackFunctionLinkedSharedButton : public SComboButton
{
public:
	SLATE_BEGIN_ARGS(SNiagaraStackFunctionLinkedSharedButton) { }
	SLATE_END_ARGS()

	void Construct(const FArguments& InArgs, UNiagaraStackFunctionInput* InFunctionInput)
	{
		UNiagaraStackFunctionInput* LinkedFunctionInput = InFunctionInput->GetSharedLinkedValueInput();
		if (LinkedFunctionInput == nullptr)
		{
			return;
		}

		SComboButton::Construct(SComboButton::FArguments()
			.ComboButtonStyle(&FNiagaraEditorWidgetsStyle::Get().GetWidgetStyle<FComboButtonStyle>("NiagaraEditor.Stack.CompactComboButtonWithBorder"))
			.OnGetMenuContent(this, &SNiagaraStackFunctionLinkedSharedButton::GetLinkedSharedActionMenu)
			.ButtonContent()
			[
				SNew(SImage)
				.ColorAndOpacity(FLinearColor(0.7f, .3f, 0.7f, 1.0f))
				.Image(FNiagaraEditorStyle::Get().GetBrush("NiagaraEditor.LinkedSharedIcon"))
			]);

		SetToolTipText(FText::Format(LOCTEXT("LinkedSharedButtonToolTipFormat",
			"This input is is linked to a shared parameter {0}.\n"
			"Edits made here will modify that linked parameter."),
			FText::FromName(LinkedFunctionInput->GetInputParameterHandle().GetName())));

		FunctionInputWeak = InFunctionInput;
		LinkedFunctionInputWeak = LinkedFunctionInput;
	}

private:
	TSharedRef<SWidget> GetLinkedSharedActionMenu()
	{
		FMenuBuilder MenuBuilder(true, nullptr);
		UNiagaraStackFunctionInput* LinkedFunctionInput = LinkedFunctionInputWeak.Get();
		if (LinkedFunctionInput != nullptr && LinkedFunctionInput->IsFinalized() == false)
		{
			MenuBuilder.BeginSection("LinkedSharedParameterActions", LOCTEXT("LinkedSharedSourceSection", "Linked Shared Parameter Actions"));
			{
				MenuBuilder.AddMenuEntry(
					LOCTEXT("GoToLinkedSharedSource", "Go to source parameter"),
					LOCTEXT("GoToLinkedSharedSourceToolTip", "Selects the set parameters node that last set the shared source parameter."),
					FSlateIcon(),
					FUIAction(FExecuteAction::CreateSP(this, &SNiagaraStackFunctionLinkedSharedButton::SelectLinkedSharedSourceModule)));
				MenuBuilder.AddMenuEntry(
					LOCTEXT("UnlinkFromSharedSource", "Unlink from source parameter."),
					LOCTEXT("UnlinkFromSharedSourceToolTip", "Unlinks this input from the shared source parameter while preserving the value."),
					FSlateIcon(),
					FUIAction(FExecuteAction::CreateSP(this, &SNiagaraStackFunctionLinkedSharedButton::UnlinkFromSharedParameter)));
			}
		}
		return MenuBuilder.MakeWidget();
	}

	void SelectLinkedSharedSourceModule()
	{
		UNiagaraStackFunctionInput* LinkedFunctionInput = LinkedFunctionInputWeak.Get();
		if (LinkedFunctionInput != nullptr && LinkedFunctionInput->IsFinalized() == false)
		{
			LinkedFunctionInput->GetSystemViewModel()->GetSelectionViewModel()->EmptySelection();
			LinkedFunctionInput->GetSystemViewModel()->GetSelectionViewModel()->AddEntryToSelectionByDisplayedObjectDeferred(&LinkedFunctionInput->GetInputFunctionCallNode());
		}
	}

	void UnlinkFromSharedParameter()
	{
		UNiagaraStackFunctionInput* FunctionInput = FunctionInputWeak.Get();
		if (FunctionInput != nullptr && FunctionInput->IsFinalized() == false)
		{
			FunctionInput->UnlinkFromSharedParameter();
		}
	}

private:
	TWeakObjectPtr<UNiagaraStackFunctionInput> FunctionInputWeak;
	TWeakObjectPtr<UNiagaraStackFunctionInput> LinkedFunctionInputWeak;
};
...
void SNiagaraStackFunctionInputValue::Construct(const FArguments& InArgs, UNiagaraStackFunctionInput* InFunctionInput)
{
	...
	bool bAddIndent = LayoutMode == ELayoutMode::FullRow;
	bool bAddValue = LayoutMode != ELayoutMode::EditDropDownOnly;
	bool bAddEditDropDown = LayoutMode == ELayoutMode::FullRow || LayoutMode == ELayoutMode::LinkedInline;
	bool bAddCompactEditDropdown = LayoutMode == ELayoutMode::CompactInline || LayoutMode == ELayoutMode::EditDropDownOnly;
	bool bAddResetButtons = LayoutMode == ELayoutMode::FullRow;
	...
	if (bAddValue)
	{
			...
			if (InArgs._ValueDecorator.Widget != SNullWidget::NullWidget)
			{
				ChildrenBox->AddSlot()
					.VAlign(VAlign_Center)
					.Padding(0)
	...
	// Compact edit drop down
	if (bAddCompactEditDropdown)
	{
		ChildrenBox->AddSlot()
		.AutoWidth()
		.VAlign(VAlign_Center)
...
TSharedRef<SWidget> SNiagaraStackFunctionInputValue::ConstructValueWidgets()
{
	...
	case UNiagaraStackFunctionInput::EValueMode::LinkedShared:
		ConstructLinkedSharedValueWidgets(ValueBox);
		bShowTypeIcon = false;
		break;
...
void SNiagaraStackFunctionInputValue::ConstructLinkedSharedValueWidgets(TSharedRef<SHorizontalBox>& ValueBox)
{
	ValueBox->AddSlot()
		.VAlign(VAlign_Center)
		.Padding(0)
		[
			SNew(SBorder)
				...
				.BorderBackgroundColor(FLinearColor(0.5f, 0, 0.5f, 0.2f))
				.Padding(0)
				[
					SNew(SNiagaraStackFunctionInputValue, FunctionInput->GetSharedLinkedValueInput())
						.LayoutMode(ELayoutMode::LinkedInline)
						.ValueDecorator()
						[
							SNew(SNiagaraStackFunctionLinkedSharedButton, FunctionInput)
								.Visibility(this, &SNiagaraStackFunctionInputValue::GetVisibilityForHoverOnlyWidgets)
						]
				]
		];
}
...
{
	if (FunctionInput->GetValueMode() == UNiagaraStackFunctionInput::EValueMode::LinkedShared)
	{
...
EVisibility SNiagaraStackFunctionInputValue::GetVisibilityForHoverOnlyWidgets() const
{
...
TArray<TSharedPtr<FNiagaraMenuAction_Generic>> SNiagaraStackFunctionInputValue::CollectActions()
{
	...
	TArray<ENiagaraScriptUsage> ScriptUsagesForSharedParameters;
	FunctionInput->GetAvailableScriptUsagesForSharedParameters(ScriptUsagesForSharedParameters);
	if (ScriptUsagesForSharedParameters.Num() > 0)
	{
		UEnum* ScriptUsageEnum = StaticEnum<ENiagaraScriptUsage>();
		for (ENiagaraScriptUsage TargetUsage : ScriptUsagesForSharedParameters)
		{
			FText TargetUsageDisplayName = ScriptUsageEnum->GetDisplayNameTextByValue((int64)TargetUsage);
			const FText DisplayName = FText::Format(LOCTEXT("MoveToSharedParameterFormat", "Shared {0} parameter."), TargetUsageDisplayName);
			const FText ToolTip = FText::Format(LOCTEXT("MoveToSharedParameterToolTipFormat", "Move the value of this input to a shared {0} parameter and show that parameter value inline here."), TargetUsageDisplayName);

			TSharedPtr<FNiagaraMenuAction_Generic> ShareInTargetUsageAction(new FNiagaraMenuAction_Generic(
				FNiagaraMenuAction_Generic::FOnExecuteAction::CreateUObject(FunctionInput, &UNiagaraStackFunctionInput::MoveValueToLinkedSharedParameter, TOptional<ENiagaraScriptUsage>(TargetUsage)),
				DisplayName, { TEXT("Share Input Value") }, {}, ToolTip, FText()));

			ShareInTargetUsageAction->SourceData = NiagaraSourceData;
			OutAllActions.Add(ShareInTargetUsageAction);
		}

		const FText TransientDisplayName = LOCTEXT("MoveToSharedTransientParameterFormat", "Shared transient parameter.");
		const FText TransientToolTip = LOCTEXT("MoveToSharedTransientParameterToolTipFormat", "Move the value of this input to a shared transient parameter and show that parameter value inline here.");

		TSharedPtr<FNiagaraMenuAction_Generic> ShareInTransientAction(new FNiagaraMenuAction_Generic(
			FNiagaraMenuAction_Generic::FOnExecuteAction::CreateUObject(FunctionInput, &UNiagaraStackFunctionInput::MoveValueToLinkedSharedParameter, TOptional<ENiagaraScriptUsage>()),
			TransientDisplayName, { TEXT("Share Input Value") }, {}, TransientToolTip, FText()));

		ShareInTransientAction->SourceData = NiagaraSourceData;

		OutAllActions.Add(ShareInTransientAction);
	}
