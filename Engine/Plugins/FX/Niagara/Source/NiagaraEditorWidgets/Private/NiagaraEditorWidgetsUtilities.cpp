...
FText FNiagaraStackEditorWidgetsUtilities::GetIconToolTipForInputMode(UNiagaraStackFunctionInput::EValueMode InputValueMode)
{
	...
	case UNiagaraStackFunctionInput::EValueMode::LinkedShared:
		return LOCTEXT("LinkSharedInputIconToolTip", "Linked Shared Value");
	case UNiagaraStackFunctionInput::EValueMode::Data:
...
bool FNiagaraStackEditorWidgetsUtilities::AddStackModuleItemContextMenuActions(FMenuBuilder& MenuBuilder, UNiagaraStackModuleItem& StackModuleItem, TSharedRef<SWidget> TargetWidget)
{
	...
	{
		// Insert Above/Below require valid group add utilities. These are not available when the module
		// item is displayed in the Emitter Summary context (utilities are null), so skip those entries.
		if (StackModuleItem.GetGroupAddUtilities() != nullptr)
		{
			MenuBuilder.AddMenuEntry(
