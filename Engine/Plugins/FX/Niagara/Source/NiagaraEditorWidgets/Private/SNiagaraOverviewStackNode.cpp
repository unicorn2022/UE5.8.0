...
ECheckBoxState SNiagaraOverviewStackNode::IsScalabilityModeActive() const
{
	...
	{
		return NiagaraSystemViewModel->GetWorkflowMode().IsEqual(FNiagaraSystemToolkitModeBase::ScalabilityModeName) ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
	}
...
void SNiagaraOverviewStackNode::OnScalabilityModeStateChanged(ECheckBoxState CheckBoxState)
{
	...
	if(NiagaraSystemViewModel.IsValid())
	{
		if(NiagaraSystemViewModel->GetWorkflowMode().IsEqual(FNiagaraSystemToolkitModeBase::ScalabilityModeName))
		{
			NiagaraSystemViewModel->SetWorkflowMode(FNiagaraSystemToolkitModeBase::DefaultModeName);
		}
		else
		{
			NiagaraSystemViewModel->SetWorkflowMode(FNiagaraSystemToolkitModeBase::ScalabilityModeName);
		}
	}
}
...
TSharedRef<SButton> SNiagaraOverviewStackNode::CreateBoundsButton()
{
	return SNew(SButton)
	.ButtonStyle(FAppStyle::Get(), "HoverHintOnly")
	.HAlign(HAlign_Center)
	.ContentPadding(1)
	.ToolTipText(this, &SNiagaraOverviewStackNode::GetToggleBoundsToolTip)
	.OnClicked(this, &SNiagaraOverviewStackNode::OnToggleBoundsButtonClicked)
	.Visibility(this, &SNiagaraOverviewStackNode::GetToggleBoundsVisibility)
	.IsFocusable(false)
	.Content()
	[
		SNew(SImage)
		.Image(this, &SNiagaraOverviewStackNode::GetToggleBoundsImage)
		.ColorAndOpacity(this, &SNiagaraOverviewStackNode::GetToggleBoundsImageColor)
	];
}
...
FReply SNiagaraOverviewStackNode::OnToggleIsolateButtonClicked()
{
	...
	if (EmitterHandleViewModel.IsValid())
	{
		...
		{
			// Shift+click: toggle isolation only on this emitter, leaving other emitters' isolation state untouched.
			EmitterHandleViewModel->ToggleIsIsolated();
		}
		else
		{
...
FText SNiagaraOverviewStackNode::GetToggleIsolateToolTip() const
{
	...
	return EmitterHandleViewModel.IsValid() && EmitterHandleViewModel->GetIsIsolated()
		? LOCTEXT("TurnOffEmitterIsolation", "Disable emitter isolation.\nShift+click: toggle only this emitter without affecting other emitters' isolation state.")
...
FText SNiagaraOverviewStackNode::GetToggleBoundsToolTip() const
{
	return LOCTEXT("ToggleBoundsTooltip", "Toggle debug emitter bounds visibility on / off");
}

FReply SNiagaraOverviewStackNode::OnToggleBoundsButtonClicked()
{
	TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel = EmitterHandleViewModelWeak.Pin();
...
EVisibility SNiagaraOverviewStackNode::GetToggleBoundsVisibility() const
{
	TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel = EmitterHandleViewModelWeak.Pin();
	return EmitterHandleViewModel.IsValid() &&
		EmitterHandleViewModel->GetOwningSystemEditMode() == ENiagaraSystemViewModelEditMode::SystemAsset
...
const FSlateBrush* SNiagaraOverviewStackNode::GetToggleBoundsImage() const
{
	return FAppStyle::Get().GetBrush("AssetEditor.ToggleShowBounds");
}

FSlateColor SNiagaraOverviewStackNode::GetToggleBoundsImageColor() const
{
	TSharedPtr<FNiagaraEmitterHandleViewModel> EmitterHandleViewModel = EmitterHandleViewModelWeak.Pin();
	FNiagaraEmitterHandle* EmitterHandle = EmitterHandleViewModel.IsValid() ? EmitterHandleViewModel->GetEmitterHandle() : nullptr;
	return EmitterHandle && EmitterHandle->GetDebugShowBounds() ? FAppStyle::GetSlateColor("SelectionColor") : FLinearColor::Gray;
}
