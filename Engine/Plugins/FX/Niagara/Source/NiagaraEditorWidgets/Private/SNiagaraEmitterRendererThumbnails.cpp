...
{	
	...
	for (int32 StackEntryIndex = 0; StackEntryIndex < RendererPreviewStackEntries.Num(); StackEntryIndex++)
	{
		...
		if (TooltipWidgets.Num() < Widgets.Num())
		{
			UE_LOGF(LogNiagaraEditor, Warning, "Renderer(%ls) has a mismatch of widgets(%d) to tooltips(%d)", *GetFullNameSafe(RendererProperties), Widgets.Num(), TooltipWidgets.Num());
		}
		...
		while ( TooltipWidgets.Num() < Widgets.Num() )
		{
