...
void FNiagaraShaderScript::CancelCompilation()
{
	...
	if (bWasPending)
	{
		UE_LOGF(LogShaders, Log, "CancelCompilation %p.", this);
	}
...
void FNiagaraShaderScript::RemoveOutstandingCompileId(const int32 OldOutstandingCompileShaderMapId)
{
	...
	{
		//UE_LOGF(LogShaders, Log, "RemoveOutstandingCompileId %p %d", this, OldOutstandingCompileShaderMapId);
	}
}
#endif // WITH_EDITOR
...
void FNiagaraShaderScript::SerializeShaderMap(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogShaders, Fatal, "This platform requires cooked packages, and shaders were not cooked into this Niagara script %ls.", *GetFriendlyName());
	}
	...
	if (bCooked)
	{
		if (Ar.IsCooking())
		{
			...
			//{
			//	FString Name;
			//	UE_LOGF(LogShaders, Error, "Failed to compile Niagara shader %ls.", *GetFriendlyName());
			//}
#endif
		}
		else
...
void FNiagaraShaderScript::UpdateCachedData_PostCompile(bool bCalledFromSerialize)
{
	...
	{		
		...
		PSOInitializers.Reserve(CachedData.NumPermutations);

		static const int32 PSOCollectorIndex = FGlobalPSOCollectorManager::GetIndex(TEXT("NiagaraComputePSOCollector"));
		...
		for (int32 iPermutation = 0; iPermutation < CachedData.NumPermutations; ++iPermutation)
		{
			...
			if (GMaxRHIShaderPlatform == GameThreadShaderMap->GetShaderPlatform())
			{
				...
				else if (IsResourcePSOPrecachingEnabled() || IsComponentPSOPrecachingEnabled())
				{
					...
					PSOPrecacheData.ComputeInitializer = FComputePipelineStateInitializer(RHIComputeShader);
		...
		if (!PSOInitializers.IsEmpty())
		{
			RequestPrecachePSOs(EPSOPrecacheType::Global, PSOInitializers);
		}
	}
	else
	{
...
bool FNiagaraShaderScript::CacheShaders(const FNiagaraShaderMapId& ShaderMapId, bool bApplyCompletedShaderMapForRendering, bool bForceRecompile, bool bSynchronous)
{
...

	{
		...
		if (!bForceRecompile && !GameThreadShaderMap && !FPlatformProperties::RequiresCookedData())
		{
			...
			if (GameThreadShaderMap && GameThreadShaderMap->IsValid())
			{
				UE_LOGF(LogShaders, Verbose, "Loaded shader for Niagara script %ls from DDC", *GetFriendlyName());
			}
			else
			{
				UE_LOGF(LogShaders, Display, "Loading shader for Niagara script %ls from DDC failed. Shader needs recompile.", *GetFriendlyName());
			}
		}
	}
	...
	if (GameThreadShaderMap && GameThreadShaderMap->TryToAddToExistingCompilationTask(this))
	{
		...
		UE_LOGF(LogShaders, Log, "CacheShaders AddUniqueExisting %p %d", this, GameThreadShaderMap->GetCompilingId());
	...
	else if (bForceRecompile || !GameThreadShaderMap || !(bAssumeShaderMapIsComplete || GameThreadShaderMap->IsComplete(this, false)))
	{
		...
		{
			UE_LOGF(LogShaders, Log, "Can't compile %ls with cooked content!", *GetFriendlyName());
			// Reset the shader map so we fall back to CPU sim
			GameThreadShaderMap = nullptr;
		}
		else
		{
			UE_LOGF(LogShaders, Log, "%ls cached shader map for script %ls, compiling.", GameThreadShaderMap? TEXT("Incomplete") : TEXT("Missing"), *GetFriendlyName());
...
void FNiagaraShaderScript::FinishCompilation()
{
	...
	if (ShaderMapIdsToFinish.Num() > 0)
	{
		...
		{
			UE_LOGF(LogShaders, Log, "FinishCompilation()[%d] %ls id %d!", i, *GetFriendlyName(), ShaderMapIdsToFinish[i]);
		}
		...
		{
			UE_LOGF(LogShaders, Warning, "Skipped multiple Niagara shader maps for compilation! May be indicative of no support for a given platform. Count: %d", ShaderMapIdsToFinish2.Num());
		}
	}
}
	...
	bool bSynchronous)
{
	...
	UE_LOGF(LogShaders, Log, "BeginCompileShaderMap AddUnique %p %d", this, NewShaderMap->GetCompilingId());
