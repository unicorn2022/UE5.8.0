...
UNiagaraDataInterfaceBase* INiagaraShaderModule::RequestDefaultDataInterface(const FString& DIClassName)
{
	...
	{
		UE_LOGF(LogTemp, Log, "NiagaraShader requires data interface '%ls' and is serialized before Niagara module startup, attempting to load Niagara module.", *DIClassName);
		...
		{
			UE_LOGF(LogTemp, Warning, "Module manager failed to load Niagara module LoadResult(%d).", int(LoadResult));
		}

		if ( !OnRequestDefaultDataInterface.IsBound() )
		{
			UE_LOGF(LogTemp, Fatal, "Failed to start Niagara module for serialization of shader that requires data interface '%ls'.  This is a load order issue that can not be resolved, content dependencies will need fixing.", *DIClassName);
		}
	}
