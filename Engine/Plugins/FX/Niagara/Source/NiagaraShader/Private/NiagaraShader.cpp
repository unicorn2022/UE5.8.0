...
uint64 FNiagaraShaderMapId::GetScriptHash() const
{
	...
	{
		HashState.Update(*AdditionalDefines[Index], AdditionalDefines[Index].Len() * sizeof(FMemoryImageString::ElementType));
	}

	for (int32 Index = 0; Index < AdditionalVariables.Num(); Index++)
	{
		HashState.Update(*AdditionalVariables[Index], AdditionalVariables[Index].Len() * sizeof(FMemoryImageString::ElementType));
	}
...
{
	FShaderCompileJob* NewJob = GShaderCompilingManager->PrepareShaderCompileJob(ShaderMapId, FShaderCompileJobKey(this, Platform, nullptr, PermutationId), EShaderCompileJobPriority::Normal);
	if (!NewJob)
	{
		return;
	}
	...
	UE_LOGF(LogShaders, Verbose, "			%ls", GetName());
	...
	bool bApplyCompletedShaderMapForRendering)
{
	...
	{
		UE_LOGF(LogShaders, Fatal, "Trying to compile Niagara shader %ls at run-time, which is not supported on consoles!", *Script->GetFriendlyName() );
	}
	else
	{
		...
		else
		{
			...
			for(TLinkedList<FShaderType*>::TIterator ShaderTypeIt(FShaderType::GetTypeList());ShaderTypeIt;ShaderTypeIt.Next())
			{
				...
				{
					UE_LOGF(LogShaders, Display, "Skipping compilation of %ls as it isn't supported on this target type.", *Script->SourceName);
					Script->RemoveOutstandingCompileId(CompilingId);
...
FShader* FNiagaraShaderMap::ProcessCompilationResultsForSingleJob(const TRefCountPtr<class FShaderCommonCompileJob>& SingleJob, uint64 ShaderMapHash)
{
...
{
	uint64 ShaderMapHash = GetContent()->ShaderMapId.GetScriptHash();
...
bool FNiagaraShaderMap::TryToAddToExistingCompilationTask(FNiagaraShaderScript* Script)
{
	...
	if (CorrespondingScripts)
	{
		...
		UE_LOGF(LogShaders, Log, "TryToAddToExistingCompilationTask %p %d", Script, GetCompilingId());
	...
	if (ShouldCacheNiagaraShader(ShaderType, GetShaderPlatform(), Script))
	{
		for (int32 PermutationId = 0; PermutationId < Script->GetNumPermutations(); ++PermutationId)
		{
			if (!GetContent()->HasShader((FShaderType*)ShaderType, PermutationId))
			{
				if (!bSilent)
				{
					UE_LOGF(LogShaders, Warning, "Incomplete shader %ls, missing FNiagaraShader %ls.", *Script->GetFriendlyName(), ShaderType->GetName());
				}
				return false;
			}
		}
