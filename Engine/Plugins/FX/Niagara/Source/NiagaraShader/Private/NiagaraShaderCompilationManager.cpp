...
//		ShaderMapInfo.bRecreateComponentRenderStateOnCompletion = bRecreateComponentRenderStateOnCompletion;
		...
		UE_LOGF(LogNiagaraShaderCompiler, Verbose, "Adding niagara gpu shader compile job... %ls", *CurrentJob->Input.DebugGroupName);
...
void FNiagaraShaderCompilationManager::ProcessAsyncResults()
{
	...
	for (int32 JobIndex = JobQueue.Num() - 1; JobIndex >= 0; JobIndex--)
	{
		...
		if (CurrentJob->GetStatus() != EShaderCompileJobStatus::Released)
		{
			continue;
		}
		...
		{
			UE_LOGF(LogNiagaraShaderCompiler, Verbose, "GPU shader compile succeeded. Id %d", CurrentJob->Id);
		}
		else
		{
			UE_LOGF(LogNiagaraShaderCompiler, Warning, "GPU shader compile failed! Id: %d Name: %ls", CurrentJob->Id, *CurrentJob->Input.DebugGroupName);
		}
		...
		if (ShaderMap && Scripts)
		{
			...
			for (int32 JobIndex = 0; JobIndex < ResultArray.Num(); JobIndex++)
			{
				...
				if (GShowNiagaraShaderWarnings || !CurrentJob->bSucceeded)
				{
					...
					{
						UE_LOGF(LogNiagaraShaderCompiler, Display, "There were issues for job \"%ls\"", *CurrentJob->Input.DebugGroupName);
						for (const FShaderCompilerError& Error : CurrentJob->Output.Errors)
						{
							UE_LOGF(LogNiagaraShaderCompiler, Warning, "%ls", *Error.GetErrorString())
						}
					}
				}
				else
				{
					UE_LOGF(LogNiagaraShaderCompiler, Verbose, "Shader compile job \"%ls\" completed.", *CurrentJob->Input.DebugGroupName);
				}
			}
				...
				for (FNiagaraShaderScript* Script : ScriptArray)
				{
					...
					if (Script->IsSame(CompletedShaderMap->GetShaderMapId()))
					{
						if (Errors.Num() != 0)
						{
							...
							UE_LOGF(LogNiagaraShaderCompiler, Verbose, "Compile output as text:");
							UE_LOGF(LogNiagaraShaderCompiler, Verbose, "===================================================================================");
							TArray<FString> OutputByLines;
							SourceCode.ParseIntoArrayLines(OutputByLines, false);
							for (int32 i = 0; i < OutputByLines.Num(); i++)
							{
								UE_LOGF(LogNiagaraShaderCompiler, Verbose, "/*%04d*/\t\t%ls", i + 1, *OutputByLines[i]);
							}
							UE_LOGF(LogNiagaraShaderCompiler, Verbose, "===================================================================================");
						}
						...
						if (!bSuccess)
						{
							...
							for (int32 ErrorIndex = 0; ErrorIndex < Errors.Num(); ErrorIndex++)
							{
								...
								UE_LOGF(LogNiagaraShaderCompiler, Warning, "	%ls", *ErrorMessage);
							}
						}
						else
						{
							...
							{
								UE_LOGF(LogNiagaraShaderCompiler, Warning, "Warnings while compiling Niagara Script %ls for platform %ls:",
									*Script->GetFriendlyName(),
									*LegacyShaderPlatformToShaderFormat(ShaderMap->GetShaderPlatform()).ToString());
								for (int32 ErrorIndex = 0; ErrorIndex < Errors.Num(); ErrorIndex++)
								{
									UE_LOGF(LogNiagaraShaderCompiler, Warning, "	%ls", *Errors[ErrorIndex]);
								}
							}
						}
