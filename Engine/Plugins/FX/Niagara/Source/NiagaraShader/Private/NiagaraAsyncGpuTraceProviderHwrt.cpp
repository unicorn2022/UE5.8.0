...
bool PlatformRequiresInlineSBTForAsyncGpuTraceHwrtInline(EShaderPlatform Platform)
{
	// PC can only fetch normals if InlineRayTracingBindingDataBuffer is valid. Otherwise we will fetch garbage
	// Other platforms can query the normal by pulling verts directly from the RT BVH
...
bool FNiagaraAsyncGpuTraceProviderHwrt::IsAvailable() const
{
	...
	if (PlatformRequiresInlineSBTForAsyncGpuTraceHwrtInline(GMaxRHIShaderPlatform))
	{
		FSceneInterface* Scene = Dispatcher->GetSceneInterface();
		if (UE::FXRenderingUtils::RayTracing::GetInlineRayTracingBindingDataBuffer(*Scene) == nullptr)
		{
			return false;
		}
	}
...
void FNiagaraAsyncGpuTraceProviderHwrt::PostRenderOpaque(FRHICommandList& RHICmdList, TConstStridedView<FSceneView> Views, TUniformBufferRef<FSceneUniformParameters> SceneUniformBufferRHI, FCollisionGroupHashMap* CollisionGroupHash)
{
	...
	if (UE::FXRenderingUtils::RayTracing::HasRayTracingScene(*Scene))
	{
		...
		InlineRayTracingBindingDataBufferSRV = UE::FXRenderingUtils::RayTracing::GetInlineRayTracingBindingDataBuffer(*Scene);
...
void FNiagaraAsyncGpuTraceProviderHwrt::IssueTraces(FRHICommandList& RHICmdList, const FDispatchRequest& Request, TUniformBufferRef<FSceneUniformParameters> SceneUniformBufferRHI, FCollisionGroupHashMap* CollisionGroupHash)
{
	...
	bool bValidInlineRaytracingBindingDataBufferFound = InlineRayTracingBindingDataBufferSRV != nullptr;
	Params.RayTracingSceneMetadata = InlineRayTracingBindingDataBufferSRV ? InlineRayTracingBindingDataBufferSRV : NullSRVBinding;
	
	if (SupportsNiagaraAsyncGpuTraceHwrtInline())
	{
		check(bValidInlineRaytracingBindingDataBufferFound || !PlatformRequiresInlineSBTForAsyncGpuTraceHwrtInline(GMaxRHIShaderPlatform));
...
void FNiagaraAsyncGpuTraceProviderHwrt::Reset()
{
	...
	InlineRayTracingBindingDataBufferSRV = nullptr;
}
