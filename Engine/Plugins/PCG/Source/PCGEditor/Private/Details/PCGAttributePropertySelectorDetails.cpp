...
namespace PCGAttributePropertySelectorDetails
{
	...
	void SetValue(const FPCGAttributePropertySelector* InSelector, TSharedPtr<IPropertyHandle>& InPropertyHandle, T&& InValue, Func InCallback)
	{
		if (!InSelector)
		{
			return;
		...
		FPCGAttributePropertySelector TempSelector = *InSelector;

		if (InCallback(&TempSelector, std::forward<T>(InValue)))
		{
			// Need to export text and import text to go through the normal flow (like update Blueprint instances)
			FString SelectorAsText{};
			FPCGAttributePropertySelector::StaticStruct()->ExportText(SelectorAsText, &TempSelector, /*Defaults=*/nullptr, nullptr, PPF_None, /*ExportRootScope=*/nullptr);
			if (!SelectorAsText.IsEmpty())
			{
				FScopedTransaction Transaction(SetAttributePropertyNameTransaction);

				for (uint32 i = 0; i < InPropertyHandle->GetNumOuterObjects(); ++i)
				{
					InPropertyHandle->SetPerObjectValue(i, SelectorAsText);
				}
			}
		}
	}
...
void FPCGAttributePropertySelectorDetails::CustomizeHeader(TSharedRef<IPropertyHandle> InPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
	...
	{
		const TTuple<FPropertyAccess::Result, const FPCGAttributePropertySelector*> Result = GetStruct();
		return (Result.Get<0>() == FPropertyAccess::Success && Result.Get<1>() && !Result.Get<1>()->IsValid()) ? FStyleColors::AccentRed : FSlateColor::UseForeground();
	};
...
TSharedRef<SWidget> FPCGAttributePropertySelectorDetails::GenerateExtraMenu()
{
	...
	// Can't add an extractor if we have different values
	if (GetStruct().Get<0>() == FPropertyAccess::Success)
	{
...
TTuple<FPropertyAccess::Result, const FPCGAttributePropertySelector*> FPCGAttributePropertySelectorDetails::GetStruct() const
{
	void* Data = nullptr;
	const FPropertyAccess::Result Result = PropertyHandle->GetValueData(Data);
	const FPCGAttributePropertySelector* OutStruct = nullptr;
	...
	case FPropertyAccess::MultipleValues:
		{
			...
			OutStruct = !RawData.IsEmpty() ? static_cast<const FPCGAttributePropertySelector*>(RawData[0]) : nullptr;
		}
		break;
	case FPropertyAccess::Success:
		OutStruct = static_cast<const FPCGAttributePropertySelector*>(Data);
		break;
	...
	return {Result, OutStruct};
}

FText FPCGAttributePropertySelectorDetails::GetText() const
{
	TTuple<FPropertyAccess::Result, const FPCGAttributePropertySelector*> Result = GetStruct();
	...
	case FPropertyAccess::Success:
		check(Result.Get<1>());
		return Result.Get<1>()->GetDisplayText();
	case FPropertyAccess::Fail: // fall-through
	default:
		return FText{};
	}
...
void FPCGAttributePropertySelectorDetails::SetText(const FText& NewText, ETextCommit::Type CommitInfo)
{
	...
	PCGAttributePropertySelectorDetails::SetValue(GetStruct().Get<1>(), PropertyHandle, NewText.ToString(), [](FPCGAttributePropertySelector* InStruct, FString InValue) -> bool { return InStruct && InStruct->Update(std::move(InValue)); });
}

void FPCGAttributePropertySelectorDetails::SetPointProperty(EPCGPointProperties EnumValue)
{
	PCGAttributePropertySelectorDetails::SetValue(GetStruct().Get<1>(), PropertyHandle, EnumValue, [](FPCGAttributePropertySelector* InStruct, EPCGPointProperties InValue) -> bool { return InStruct && InStruct->SetPointProperty(InValue); });
}

void FPCGAttributePropertySelectorDetails::SetAttributeName(FName NewName)
{
	PCGAttributePropertySelectorDetails::SetValue(GetStruct().Get<1>(), PropertyHandle, NewName, [](FPCGAttributePropertySelector* InStruct, FName InValue) -> bool { return InStruct && InStruct->SetAttributeName(InValue); });
}
...
void FPCGAttributePropertySelectorDetails::SetExtraProperty(EPCGExtraProperties EnumValue)
{
	...
	PCGAttributePropertySelectorDetails::SetValue(GetStruct().Get<1>(), PropertyHandle, EnumValue, [DomainName](FPCGAttributePropertySelector* InStruct, EPCGExtraProperties InValue) -> bool
		{
			if (!InStruct)
			{
				return false;
...
{
	PCGAttributePropertySelectorDetails::SetValue(GetStruct().Get<1>(), PropertyHandle, InSelector, [](FPCGAttributePropertySelector* InStruct, FPCGAttributePropertySelector InValue) -> bool
	{
		if (!InStruct)
		{
			return false;
...
{
	PCGAttributePropertySelectorDetails::SetValue(GetStruct().Get<1>(), PropertyHandle, InExtractor, [](FPCGAttributePropertySelector* InStruct, FName InValue) -> bool 
	{
		if (!InStruct)
		{
			return false;
