...
void FPCGOverrideInstancedPropertyBagDetails::CustomizeChildren(TSharedRef<IPropertyHandle> InPropertyHandle, IDetailChildrenBuilder& InChildrenBuilder, IPropertyTypeCustomizationUtils& InCustomizationUtils)
{
	...
	// We forward the schema metadata
	if (PropertyHandle.IsValid() && PropertyHandle->HasMetaData(UE::StructUtils::Metadata::PropertyBagSchemaClassName) && InstancedPropertyBagHandle.IsValid())
	{
		InstancedPropertyBagHandle->SetInstanceMetaData(UE::StructUtils::Metadata::PropertyBagSchemaClassName, *PropertyHandle->GetMetaData(UE::StructUtils::Metadata::PropertyBagSchemaClassName));
	}
	
	const TSharedRef<FPCGOverrideInstancedPropertyBagDataDetails> InstanceDetails = MakeShared<FPCGOverrideInstancedPropertyBagDataDetails>(InstancedPropertyBagHandle, InCustomizationUtils.GetPropertyUtilities());
...
FPCGOverrideInstancedPropertyBagDataDetails::FPCGOverrideInstancedPropertyBagDataDetails(TSharedPtr<IPropertyHandle> InStructProperty, const TSharedPtr<IPropertyUtilities>& InPropUtils)
	: FPropertyBagInstanceDataDetails(
		FConstructParams{
			.BagStructProperty = InStructProperty.IsValid() ? InStructProperty : nullptr,
