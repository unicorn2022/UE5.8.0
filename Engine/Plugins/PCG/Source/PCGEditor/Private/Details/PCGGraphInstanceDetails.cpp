...
void FPCGGraphInstanceDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (SelectedGraphInstances.Num() == 1 && SelectedGraphInstances[0]->CanSaveAsAsset())
	{
		NewRow.ValueContent()
...
FReply FPCGGraphInstanceDetails::OnSaveInstanceClicked()
{
	...
	{
		UE_LOGF(LogPCGEditor, Warning, "Graph instance asset creation was aborted or failed, abort.");
		return FReply::Handled();
	}
