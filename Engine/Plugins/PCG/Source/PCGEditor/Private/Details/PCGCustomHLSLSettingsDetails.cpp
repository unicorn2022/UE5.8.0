...
void FPCGCustomHLSLSettingsDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (!ObjectsBeingCustomized.IsEmpty() && ObjectsBeingCustomized[0].IsValid())
	{
		...
		if (PCGEditorSharedPtr.IsValid() && !PCGEditorSharedPtr->IsPanelCurrentlyOpen(PCGEditorTabs::CodeEditorID))
		{
			SettingsCategory.AddCustomRow(FText::GetEmpty())
				...
				[PCGEditorWeakPtr]
				{
					...
					return (SharedPtr.IsValid() && SharedPtr->IsPanelCurrentlyOpen(PCGEditorTabs::CodeEditorID)) ? EVisibility::Collapsed : EVisibility::Visible;
				}))
				.ValueContent()
				...
				[
					...
					[
						...
						.OnClicked_Lambda([PCGEditorWeakPtr]() -> FReply
						{
							...
							{
								SharedPtr->BringFocusToPanel(PCGEditorTabs::CodeEditorID);
							}
