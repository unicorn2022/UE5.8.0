...
TSharedPtr<SGraphPin> SPCGEditorGraphNodeCompact::CreatePinWidget(UEdGraphPin* Pin) const
{
	// Compact nodes never show pin labels — bake ShowLabel=false at construction so no dynamic attribute is needed.
	// Note: compact nodes do not use default value widgets (no UPCGSettingsDefaultValueProvider), so the parent
	// CreatePinWidget logic for specialised pin types is intentionally bypassed here.
	return SNew(SPCGEditorGraphNodePin, Pin).ShowLabel(false);
}
