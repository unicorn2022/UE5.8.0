...
{
	// Never reformat the node name, as we don't want to have nodes that seems the same but ar not (example, "As_A" and "As A" would appear the same)
	return PCGNode ? FText::FromName(PCGNode->NodeTitle) : FText{};
}
