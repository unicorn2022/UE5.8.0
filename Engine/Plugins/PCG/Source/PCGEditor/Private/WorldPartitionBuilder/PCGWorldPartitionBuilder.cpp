...
struct FPCGWorldPartitionBuilder
{
	...
	static FBox GetPCGBuilderVolumeBounds(UWorld* InWorld);
private:
...
private:
	void HandleDeletedActor(AActor* InActor)
	{
		if (InActor && InActor->IsPackageExternal())
		{
			...
			{
				UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Actor '%ls' deleted, package '%ls' added to delete list.",
					*InActor->GetName(),
...
bool FPCGWorldPartitionBuilderArgs::InitializeFrom(const FPCGWorldPartitionCommandlineArgs& CommandlineArgs, FPCGWorldPartitionBuilderArgs& OutArgs)
{
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Initialize Builder Args");
	...
	if (SettingsPath.IsValid() || SettingsPath.IsPending())
	{
		if (UPCGBuilderSettings* BuilderSettings = Cast<UPCGBuilderSettings>(SettingsPath.LoadSynchronous()))
		{
			...
			UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Loaded PCGBuilderSettings '%ls'", *SettingsPath.ToString());
			bUsingBuilderSettings = true;
		}
		else
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Error, "Failed to load PCGBuilderSettings '%ls'", *SettingsPath.ToString());
			return false;
		}
	}
	...
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Not using any PCGBuilderSettings");
	}
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "bGenerateEditingModeLoadAsPreviewComponents : '%d' (%ls)", 
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "bGenerateEditingModeNormalComponents : '%d' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "bGenerateEditingModePreviewComponents : '%d' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "bOneComponentAtATime : '%d' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "bIgnoreGenerationErrors : '%d' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "IncludeActorIDs : '%ls' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "IncludeGraphNames : '%ls' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "IncludeGraphs '%ls' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "bIterativeCellLoading : '%d' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "IterativeCellSize : '%d' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "IncludedDataLayers '%ls' (%ls)",
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "ExcludedDataLayers '%ls' (%ls)",
		DataLayersToString.IsEmpty() ? TEXT("None") : *FString::Join(DataLayersToString, TEXT(",")),
		FromDefault);

	// Log Revert parameters
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "bRevertUnchangedActors '%d' (%ls)",
		BuilderArgs.bRevertUnchangedActors ? 1 : 0,
		FromDefault);

	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "OutputsToIgnoreForRevert '%ls' (%ls)",
	...
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		if (GetParamValue("IterativeCellSize=", ParamIterativeCellSize))
		{
			...
			{
				UE_LOGF(LogPCGWorldPartitionBuilder, Warning, "Minimum value for 'IterativeCellSize' is %d", MinIterativeCellSize);
			}
		}
	}
}
...
{
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Added Asset to Submit Allow List: '%ls'", *InPath.ToString());
...
bool UPCGWorldPartitionBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	{
		FBox PCGBuilderVolumeBounds = FPCGWorldPartitionBuilder::GetPCGBuilderVolumeBounds(World);
		if (PCGBuilderVolumeBounds.IsValid)
		{
			IterativeWorldBounds = PCGBuilderVolumeBounds;
		}
...
bool UPCGWorldPartitionBuilder::RunEntireWorld(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (!bGeneratedAnyComponent)
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "No components found to generate. No packages will be saved.");
	}
	else if (DirtyPackages.IsEmpty())
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "At least one component was generated but no additional packages were dirtied.");
	}
	else
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Generation complete, %d packages dirtied.", DirtyPackages.Num());
	}

	if (!bGeneratedAnyComponent)
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Dirty package detection and save skipped due to trivial run");
...
bool UPCGWorldPartitionBuilder::SaveEntireWorldPackages(UWorld* World, FPackageSourceControlHelper& PackageHelper, TArray<UPackage*> DirtyPackages, TSet<FString> DeletedActorPackages)
{
	...
	if (bErrorOccurredWhileGenerating)
	{
		...
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Dirty package detection and save skipped due to errors during generation.");

			return !bErrorOccurredWhileGenerating;
		}
		else
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Generation errors ignored, dirty packages will be saved.");
		}
	}
...
bool UPCGWorldPartitionBuilder::RunPerCell(UWorld* InWorld, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	if (bErrorsOccurred && !Args.bIgnoreGenerationErrors)
	{
		if (bCellHasChanges)
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Save skipped due to errors during generation.");
		}
		return false;
	}
...
bool UPCGWorldPartitionBuilder::PostRun(UWorld* World, FPackageSourceControlHelper& PackageHelper, const bool bInRunSuccess)
{
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "PostRun: Final package changes: %d new, %d modified, %d deleted, %d unchanged.", NumNewPackages, NumModifiedPackages , NumDeletedPackages, RevertedActors.Num());
	...
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Error, "Failed to initialize source control provider.");
		return false;
	}
...
bool UPCGWorldPartitionBuilder::SavePerCellPackages(UWorld* InWorld, FPackageSourceControlHelper& PackageHelper, TSet<FString> CellDeletedPackages)
{
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Cell package changes: %d modified, %d new, %d deleted.", CellModifiedPackageCount, CellNewPackageCount, CellDeletedPackageCount);
	return true;
}
	...
	bool& bOutGenerationErrors)
{
	...
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Completing generation on PCG component on actor '%ls' label '%ls', graph '%ls'",
	...
	if (!InArgs.bOneComponentAtATime)
	{
		for (const auto& KeyValuePair : GeneratedComponents)
		{
			...
			{
				UE_LOGF(LogPCGWorldPartitionBuilder, Warning, "GeneratedComponent reference lost, abandoning.");
			}
		}
	}
...
FPCGTaskId FPCGWorldPartitionBuilder::GenerateComponent(UPCGComponent* InComponent, UWorld* InWorld, const FPCGWorldPartitionBuilderArgs& InArgs, FPCGTaskId InTaskDependency)
{
	...
	{
		GenerateTask = InComponent->GenerateLocalGetTaskId(EPCGComponentGenerationTrigger::GenerateOnDemand, /*bForce=*/true, PCGHiGenGrid::UnboundedGridSize());
	}
	else if(InTaskDependency != InvalidPCGTaskId)
	{
		GenerateTask = InComponent->GenerateLocalGetTaskId(EPCGComponentGenerationTrigger::GenerateOnDemand, /*bForce=*/true, PCGHiGenGrid::UninitializedGridSize(), { InTaskDependency });
	}
	else
	{
	...
	if (GenerateTask == InvalidPCGTaskId)
	{
		...
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Warning, "Scheduling generate task failed for PCG Component on actor '%ls' label '%ls' graph '%ls'.",
				*InComponent->GetOwner()->GetName(),
				*InComponent->GetOwner()->GetActorNameOrLabel(),
				*InComponent->GetGraph()->GetName());
		}
		...
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Warning, "Scheduling generate task failed for PCG Component with no owner. Component '%ls' graph '%ls'.",
				*InComponent->GetName(),
				*InComponent->GetGraph()->GetName());
		}
	}
...
TUniquePtr<IWorldPartitionActorLoaderInterface::ILoaderAdapter> FPCGWorldPartitionBuilder::ExpandLoadedBoundsIfNeeded(UWorld* InWorld, const FBox& InCurrentBounds, const FBox& InNewBounds, const FString& InLoaderName)
{
	...
	if (!InNewBounds.Equals(InCurrentBounds))
	{
		...
		UE_LOGF(LogPCGWorldPartitionBuilder, Verbose, "Expanding Cell Bounds: Min %ls, Max %ls", *InNewBounds.Min.ToString(), *InNewBounds.Max.ToString());
	}
...
bool FPCGWorldPartitionBuilder::IsActorRevertable(AActor* InActor, const FPCGWorldPartitionBuilderArgs& InArgs, FPCGCrc& OutCrc)
{
	...
	UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' : begin", *InActor->GetActorLabel());
	...
	InActor->ForEachComponent(/*bIncludeFromChildActors=*/false, [bLocalComponents, &bCanRevertActor, &PCGComponents](const UActorComponent* InComponent)
	{
		...
		else if (!InComponent->IsDefaultSubobject())
		{
			...
			UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' : Can't revert actor because of non default component '%ls'", *InComponent->GetOwner()->GetActorLabel(), *InComponent->GetName());
		}
	});

	if (!bCanRevertActor)
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' : end", *InActor->GetActorLabel());
		return false;
	}
	...
	for (const UPCGComponent* PCGComponent : PCGComponents)
	{
		...
		PCGComponent->ForEachConstManagedResource([&Crc, &bCanRevertActor, InActor](const UPCGManagedResource* ManagedResource)
		{
			...
			{
				bCanRevertActor = false;
				UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' Can't revert actor because of invalid crc for resource : '%ls'", *InActor->GetActorLabel(), *ManagedResource->GetName());
			}
			else
			{
				Crc.Combine(ManagedResource->GetCrc());
				UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' : resource '%ls' : crc '%d'", *InActor->GetActorLabel(), *ManagedResource->GetName(), ResourceCrc.GetValue());
			}
		});
		...
		for (const FPCGTaggedData& Output : PCGComponent->GetGeneratedGraphOutput().TaggedData)
		{
			...
			{
				UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' : output pin ignored: '%ls'", *InActor->GetActorLabel(), *Output.Pin.ToString());
				continue;
			}

			FPCGCrc OutputCrc = Output.ComputeCrc(/*bFullDataCrc=*/true);
			UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' : output pin: '%ls' : crc '%d'", *InActor->GetActorLabel() , *Output.Pin.ToString(), OutputCrc.GetValue());
	...
	OutCrc = Crc;
	UE_LOGF(LogPCGWorldPartitionBuilder, VeryVerbose, "IsActorRevertable : '%ls' : crc : '%d' : end", *InActor->GetActorLabel(), OutCrc.GetValue());
	return true;
}
...
void FPCGWorldPartitionBuilder::LogRevertedActors(const TSet<TSoftObjectPtr<AActor>>& InRevertedActors)
{
	...
	for (TSoftObjectPtr<AActor> RevertedActorPtr : InRevertedActors)
	{
		...
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Unchanged actor '%ls' (name '%ls' - package '%ls')", *RevertedActor->GetActorLabel(), *RevertedActor->GetName(), *RevertedActor->GetPackage()->GetName());
		}
	}
}
...
void FPCGWorldPartitionBuilder::LogModifiedAndNewActorsFromDirtyPackages(const TArray<UPackage*>& InDirtyPackages)
{
	...
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Modified actor '%ls' (name '%ls' - package '%ls')", *ModifiedActor->GetActorLabel(), *ModifiedActor->GetName(), *ModifiedActor->GetPackage()->GetName());
	}

	for (AActor* NewActor : NewActors)
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Added actor '%ls' (name '%ls' - package '%ls')", *NewActor->GetActorLabel(), *NewActor->GetName(), *NewActor->GetPackage()->GetName());
	}
}
...
void FPCGWorldPartitionBuilder::UpdateEditingMode(UPCGComponent* InComponent, bool bForce)
{
	...
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Setting PCG editing mode to Load As Preview on actor '%ls' label '%ls' graph '%ls'.",
			*InComponent->GetOwner()->GetName(),
			*InComponent->GetOwner()->GetActorNameOrLabel(),
			*InComponent->GetGraph()->GetName());
...
FBox FPCGWorldPartitionBuilder::GetPCGBuilderVolumeBounds(UWorld* InWorld)
{
	FBox PCGBuilderVolumeBounds(ForceInit);

	check(InWorld);

	// Override the builder bounds if pcg builder volumes exists
	for (TActorIterator<APCGBuilderVolume> It(InWorld); It; ++It)
	{
		if (APCGBuilderVolume* BuilderVolume = *It)
		{
			if (FBox Bounds = BuilderVolume->GetBounds().GetBox(); Bounds.IsValid)
			{
				PCGBuilderVolumeBounds += Bounds;
				UE_LOGF(LogPCGWorldPartitionBuilder, Display, "Found PCGBuilderVolume actor '%ls' label '%ls' bounds : %ls",
					*BuilderVolume->GetName(),
					*BuilderVolume->GetActorNameOrLabel(),
					*Bounds.ToString());
			}
			else
			{
				UE_LOGF(LogPCGWorldPartitionBuilder, Warning, "Found PCGBuilderVolume actor '%ls' label '%ls' with invalid bounds (ignored)",
					*BuilderVolume->GetName(),
					*BuilderVolume->GetActorNameOrLabel());
			}
		}
	...
	if (PCGBuilderVolumeBounds.IsValid)
	{
		UE_LOGF(LogPCGWorldPartitionBuilder, Display, "PCG builder bounds '%ls'", *PCGBuilderVolumeBounds.ToString());
	}

	return PCGBuilderVolumeBounds;
}
						...
						if (ActorPath.IsValid())
						{
							if (const FWorldPartitionActorDescInstance* ActorDescInstance = WorldPartition->GetActorDescInstanceByPath(ActorPath))
							{
								...
								if (AActor* LoadedActor = ActorReference.GetActor())
								{
									...
									UE_LOGF(LogPCGWorldPartitionBuilder, Verbose, "Loaded Original PCG Component Actor '%ls' label '%ls'.",
										*LoadedActor->GetName(),
			...
			for (const TSoftObjectPtr<AActor>& ManagedActorPath : ManagedActorPaths)
			{
				if (!ManagedActorPath.IsNull() && !ManagedActorPath.IsValid())
				{
					if (const FWorldPartitionActorDescInstance* ActorDescInstance = WorldPartition->GetActorDescInstanceByPath(ManagedActorPath.ToSoftObjectPath()))
					{
						...
						{
							OutLoadedActorDependencies.Add(ActorReference);
							UE_LOGF(LogPCGWorldPartitionBuilder, Verbose, "Loaded Managed Actor '%ls' label '%ls'.",
								*LoadedActor->GetName(),
								*LoadedActor->GetActorNameOrLabel());
						}
					}
...
void FPCGWorldPartitionBuilder::CollectComponentsToGenerate(UWorld* InWorld, const FPCGWorldPartitionBuilderArgs& InArgs, TFunctionRef<bool(const UPCGComponent*)> ComponentFilter, TArray<TWeakObjectPtr<UPCGComponent>>& OutComponents)
{
	...
	for (UObject* ComponentObject : AllComponents)
	{
		...
		if (!Graph)
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Warning, "PCG component on actor '%ls' label '%ls' has no graph assigned, skipping.",
		...
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Display, "'Activated' toggle was set false on PCG component on actor '%ls' label '%ls' graph '%ls'. Component skipped.",
				*Component->GetOwner()->GetName(),
				*Component->GetOwner()->GetActorNameOrLabel(),
		...
		{
			UE_LOGF(LogPCGWorldPartitionBuilder, Display, "PCG component generation trigger is set to run-time generation on actor '%ls' label '%ls' graph '%ls'. Component skipped.",
				*Component->GetOwner()->GetName(),
				*Component->GetOwner()->GetActorNameOrLabel(),
				*Component->GetGraph()->GetName());
			continue;
