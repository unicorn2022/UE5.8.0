...
public:
	...
	UPCGGraph* GetPCGGraph() const { return WeakPCGGraph.Get(); }
	...
	UPCGEditorGraphNodeBase* GetEditorNodeFromPCGNode(const UPCGNode* InPCGNode);

	/** Returns whether the edge is used by the downstream node's execution. Cached per graph change. */
	bool IsEdgeUsedByExecution(const UPCGEdge* InEdge) const;
...
private:
	UPROPERTY()
	TWeakObjectPtr<UPCGGraph> WeakPCGGraph;

	TWeakPtr<FPCGEditor> PCGEditor = nullptr;

	mutable TMap<TObjectKey<UPCGEdge>, bool> CachedEdgeUsedByExecution;
};
