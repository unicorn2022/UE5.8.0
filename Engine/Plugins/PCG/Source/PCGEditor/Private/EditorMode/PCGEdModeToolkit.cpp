...
#include "Tools/Query/PCGQueryTool.h"
...
namespace PCGEdModeToolkit
{
	static TAutoConsoleVariable<bool> CVarEnableManualEditPalette(
		TEXT("pcg.Editor.EnableManualEditPalette"),
		true,
		TEXT("Whether the Manual Edit tool palette is shown in the PCG Editor Mode toolkit."));
}
...
void FPCGEditorModeToolkit::RegisterPalettes()
{
		...
		
	{
		...
		TSharedPtr<FPCGToolPalette> DrawSplineToolSection = MakeShared<FPCGToolPalette>(PaletteCommands.LoadSplineContextPalette, ToolCommands.GetLineContextToolCommands());
		ToolkitBuilder->AddPalette(MoveTemp(DrawSplineToolSection));
	}

	{
		FString ToolIdentifier = UPCGInteractiveToolSettings_PaintTool::StaticGetToolTag().ToString();
		TSharedPtr<FPCGToolPalette> PaintToolPalette = MakeShared<FPCGToolPalette>(PaletteCommands.LoadPaintContextPalette, ToolCommands.GetPaintContextToolCommands());
		ToolkitBuilder->AddPalette(MoveTemp(PaintToolPalette));
	}

	{
		FString ToolIdentifier = UPCGInteractiveToolSettings_Volume::StaticGetToolTag().ToString();
		TSharedPtr<FPCGToolPalette> VolumeToolPalette = MakeShared<FPCGToolPalette>(PaletteCommands.LoadVolumeContextPalette, ToolCommands.GetVolumeContextToolCommands());
		ToolkitBuilder->AddPalette(MoveTemp(VolumeToolPalette));
	}

	{
		FString ToolIdentifier = UPCGQueryTool::StaticGetToolTag().ToString();
		TSharedPtr<FPCGToolPalette> QueryToolPalette = MakeShared<FPCGToolPalette>(PaletteCommands.LoadQueryContextPalette, ToolCommands.GetQueryContextToolCommands());
		ToolkitBuilder->AddPalette(MoveTemp(QueryToolPalette));
	}

	if (PCGEdModeToolkit::CVarEnableManualEditPalette.GetValueOnAnyThread())
	{
		FString ToolIdentifier = UPCGInteractiveToolSettings_ManualEdit::StaticGetToolTag().ToString();
		TSharedPtr<FPCGToolPalette> ManualEditPalette = MakeShared<FPCGToolPalette>(PaletteCommands.LoadManualContextPalette, ToolCommands.GetManualEditContextToolCommands());
		ToolkitBuilder->AddPalette(MoveTemp(ManualEditPalette));
	}
