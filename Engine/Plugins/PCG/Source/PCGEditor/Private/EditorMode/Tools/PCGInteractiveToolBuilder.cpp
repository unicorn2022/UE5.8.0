...
bool UE::PCG::EditorMode::Tool::BuildTool(UInteractiveTool* InTool)
{
	...
	if ((Cast<UPCGInteractiveTool>(InTool) && !Cast<UPCGInteractiveTool>(InTool)->RequiresSettings()))
	{
		return true;
	}
	...
	for(TPropertyValueIterator<FObjectProperty> PropertyIt(InTool->GetClass(), InTool); PropertyIt; ++PropertyIt)
	{
		if(PropertyIt.Key()->IsA(FObjectProperty::StaticClass()))
		{
			...
			if(ObjectProperty->PropertyClass->IsChildOf(UPCGInteractiveToolBaseSettings::StaticClass()) && ObjectProperty->PropertyClass->HasAnyClassFlags(EClassFlags::CLASS_Abstract) == false)
			{
	...
	{
		FText Message = FText::FormatOrdered(LOCTEXT("PCGSettingsPropertyMissingMessage", "Settings {0} of tool {1} does not have a valid PCGInteractiveToolBaseSettings property. Did you forget to mark it as UPROPERTY?"), SettingsClass ? SettingsClass->GetClass()->GetDisplayNameText() : FText::GetEmpty(), InTool->GetClass()->GetDisplayNameText());
		InTool->GetToolManager()->DisplayMessage(Message, EToolMessageLevel::UserError);
		return false;
	}
	
	UPCGInteractiveToolBaseSettings* Settings = NewObject<UPCGInteractiveToolBaseSettings>(InTool, SettingsClass);
