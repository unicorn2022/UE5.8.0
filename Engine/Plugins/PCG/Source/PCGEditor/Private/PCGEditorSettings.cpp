	...
	: Super(ObjectInitializer)
{
	...
	EmbeddedSubgraphNodeColor = FLinearColor(0.225f, 0.f, 0.f);
	...
	DataViewPinColor = FLinearColor(0.69f, 0.92f, 0.23f);
	...
	RawBufferPinColor = FLinearColor(0.6f, 0.7f, 1.0f);
...
FLinearColor UPCGEditorSettings::GetColor(UPCGSettings* Settings) const
{
	...
	switch (Settings->GetType())
	{
		...
		case EPCGSettingsType::Subgraph:
		{
			...
			{
				if (UPCGGraph* Subgraph = SubGraphSettings->GetSubgraph(); Subgraph && Subgraph->IsEmbeddedSubgraph())
				{
					return EmbeddedSubgraphNodeColor;
				}
			}
