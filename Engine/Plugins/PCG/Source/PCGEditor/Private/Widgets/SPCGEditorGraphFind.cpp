...
FReply FPCGEditorGraphFindResult::OnClick(TWeakPtr<FPCGEditor> InPCGEditorPtr)
{
	...
	if (ResolvedPin && ResolvedPin->GetOwningNode()->GetGraph() == Editor->GetFocusedEditorGraph())
	{
...
FReply FPCGEditorGraphFindResult::OnDoubleClick(TWeakPtr<FPCGEditor> InPCGEditorPtr)
{
	...
	if (ParentGraph)
	{
		Editor->OpenEditorForGraph(ParentGraph->GetPCGGraph());
...
void SPCGEditorGraphFind::MatchTokens(const TArray<FString>& InTokens)
{
	...
	UPCGEditorGraph* PCGEditorGraph = PCGEditor ? PCGEditor->GetMainEditorGraph() : nullptr;
	...
	auto VisitAllNodes = [&AllGraphs, PCGEditor](UPCGEditorGraph* EditorGraph, PCGEditorGraphFind::GraphTree& Parent, auto&& RecursiveCall)
	{
		if (!EditorGraph)
		{
			return;
				...
				if (const UPCGBaseSubgraphNode* SubgraphNode = Cast<UPCGBaseSubgraphNode>(PCGEditorNode->GetPCGNode()))
				{
					if (UPCGGraph* Subgraph = SubgraphNode->GetSubgraph())
					{
						...
						if (!Parent.HasVisitedGraph(Subgraph))
						{
							...
							{
								RecursiveCall(PCGEditor->GetOrCreatePCGEditorGraph(Subgraph), *Child, RecursiveCall);
							}
						}
					}
				}
		...
		for (const FPCGStackSharedPtr& SubstackPtr : Substacks)
		{
			...
			while (FrameIndex < Substack.GetStackFrames().Num() && Current)
			{
				...
				if (bProcessPair)
				{
					...
					CurrentEditorGraph = Current && Subgraph ? PCGEditor->GetOrCreatePCGEditorGraph(Subgraph) : nullptr;
	...
	{
		auto RecurseInGraphTree = [this, &InTokens, PCGEditor](const PCGEditorGraphFind::GraphTree& Node, TFunctionRef<FPCGEditorGraphFindResultPtr()> GetParentFunc, auto&& RecurseCall) -> void
		{
			check(Node.Graph);
			...
			auto GetOrCreateNodeResult = [this, &Node, &NodeString, &ThisResult, &GetParentFunc, PCGEditor]()
			{
				...
				if(!ThisResult.IsValid() && Node.Node)
				{
					...
					ThisResult->ParentGraph = PCGEditor->GetOrCreatePCGEditorGraph(Node.Graph);
			...
			MatchTokensInternal(InTokens, PCGEditor->GetOrCreatePCGEditorGraph(Node.Graph), GetOrCreateNodeResult/*, MatchedTokens*/);
	...
	else if (FindMode == EPCGGraphFindMode::ShowFlatList)
	{
		for(UPCGGraph* Graph : AllGraphs)
		{
			...
			auto CreateGraphNode = [Graph, &GraphString, &GraphNodePtr, &GraphRoot, PCGEditor]()
			{
				if (!GraphNodePtr.IsValid())
				{
					...
					GraphNodePtr->ParentGraph = PCGEditor->GetOrCreatePCGEditorGraph(Graph);
			...
			MatchTokensInternal(InTokens,  PCGEditor->GetOrCreatePCGEditorGraph(Graph), CreateGraphNode/*, Results*/);
