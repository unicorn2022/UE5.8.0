...
#include "SPCGMetadataAttributeGenericViewer.h"
...
namespace PCGEditorGraphAttributeListView
{
	...
	struct FPCGListViewItemRowTextInfo
	{
	...
	FPCGListViewItemRowTextInfo ExtractText(FPCGColumnData* PCGColumnData, int32 Index, const FName& ColumnId)
	{
		FPCGListViewItemRowTextInfo Result{};
		
		if (Index == -1 && !PCGColumnData->DataAccessor->IsAttribute())
		{
			Result.RowText = ColumnId == PCGDataVisualizationConstants::NAME_Index ? LOCTEXT("DefaultIndex", "Default") : FText{};
			return Result;
		}
		
		const FPCGMetadataAttributeDesc& Desc = PCGColumnData->DataAccessor->GetUnderlyingDesc();
		
		// If the descriptor is an array, print the first few values.
		if (Desc.IsArray())
		{
			// Always show some text for the array to know that it is an array.
			Result.bShowText = true;

			auto CallbackArray = [&PCGColumnData, Index, &Result]<typename ValueType>(ValueType)
			{
				PCG::TPCGArrayAccessorWrapper<ValueType> Value{};
				...
				if (!PCGColumnData->DataAccessor->Get(Value, Index == -1 ? 0 : Index, DataKeys))
				{
					return false;
				}

				if (PCG::Private::IsOfTypes<ValueType, FString, FName, FSoftClassPath, FSoftObjectPath>())
				{
					Result.bShouldComputeCellWidth = true;
				}
				
				Result.OverflowPolicy = GetDefault<UPCGEditorSettings>()->AttributeListViewSettings.StringOverflowPolicy;

				constexpr int32 MaxElements = 3;
				const int32 NumElements = Value.GetView().Num();
				
				auto Projection = [](const ValueType& Item)
				{
					// To be consistent with FText::AsNumber
					if constexpr (FTextAsNumberIsValid<ValueType>::value && !std::is_same_v<ValueType, bool>)
					{
						static const FDecimalNumberFormattingRules DefaultFormattingRules{};
						static const FNumberFormattingOptions DefaultFormattingOptions{};
						return FastDecimalFormat::NumberToString(Item, DefaultFormattingRules, DefaultFormattingOptions);
					}
					else
					{
						return PCG::Private::MetadataTraits<ValueType>::ToString(Item);
					}
				};
				
				TStringBuilder<256> Builder;
				Builder += TEXT("{ ");
				Builder += FString::JoinBy(MakeConstArrayView(Value.GetView().GetData(), FMath::Min(MaxElements, NumElements)), TEXT(", "), Projection);
				...
				Result.RowText = FText::FromStringView(Builder.ToView());
				return true;
			};
			
			if (!PCGMetadataAttribute::CallbackWithRightType(static_cast<uint16>(Desc.ValueType), CallbackArray))
			{
				Result.RowText = INVTEXT("{ (...) }");
			}

			return Result;
		}
		
		auto Callback = [&PCGColumnData, Index, &Result]<typename ValueType>(ValueType)
		{
			...
			if (PCGColumnData->DataAccessor->Get<ValueType>(Value, Index == -1 ? 0 : Index, DataKeys))
			{
				Result.bShowText = true;

				if constexpr (PCG::Private::IsOfTypes<ValueType, bool>())
				{
					Result.RowText = FText::FromString(LexToString(Value));
				}
				else if constexpr (PCG::Private::IsOfTypes<ValueType, FString>())
				{
					Result.RowText = FText::FromString(Value);
					Result.bShouldComputeCellWidth = true;
					Result.OverflowPolicy = GetDefault<UPCGEditorSettings>()->AttributeListViewSettings.StringOverflowPolicy;
				}
				else if constexpr (PCG::Private::IsOfTypes<ValueType, FName>())
				{
					Result.RowText = FText::FromName(Value);
					Result.bShouldComputeCellWidth = true;
					Result.OverflowPolicy = GetDefault<UPCGEditorSettings>()->AttributeListViewSettings.NameOverflowPolicy;
				}
				else if constexpr (FTextAsNumberIsValid<ValueType>::value)
				{
					Result.RowText = FText::AsNumber(Value);
				}
				else if constexpr (PCG::Private::IsOfTypes<ValueType, FSoftObjectPath, FSoftClassPath>())
				{
					Result.RowText = FText::FromString(Value.ToString());
					Result.bShouldComputeCellWidth = true;
					Result.bIsClickable = true;
					Result.OverflowPolicy = GetDefault<UPCGEditorSettings>()->AttributeListViewSettings.HyperLinkOverflowPolicy;
				}
				else
				{
					ensureMsgf(false, TEXT("Unsupported Data Type"));
					Result.RowText = LOCTEXT("UnsupportedDataTypeError", "Unsupported Data Type");
				}
			}
		};
...
{
	PCGEditorGraphAttributeListView::FPCGListViewItemRowTextInfo RowTextInfo{};
	...
	TOptional<TTuple<const FPCGMetadataAttributeBase*, PCGMetadataEntryKey>> AttrAndKey{};
	
	const int32 Index = InternalItem->Index;

	if (FPCGColumnData* PCGColumnData = SharedAttributeListView->PCGColumnData.Find(ColumnId))
	{
		if (PCGColumnData->DataKeys.IsValid() && PCGColumnData->UnderlyingAttribute)
		{
			const PCGMetadataEntryKey* Key = nullptr;
			if (PCGColumnData->DataKeys->GetKey<PCGMetadataEntryKey>(Index, Key))
			{
				AttrAndKey = MakeTuple(PCGColumnData->UnderlyingAttribute, *Key);
			}
		}
		...
		{
			RowTextInfo = PCGEditorGraphAttributeListView::ExtractText(PCGColumnData, Index, ColumnId);
		}
	}

	if (RowTextInfo.bShouldComputeCellWidth)
	{
		float& CurrentColumnWidth = SharedAttributeListView->ColumnsMaxWidthMapping[ColumnId];
		const float TextWidth = PCGEditorGraphAttributeListView::CalculateColumnWidth(RowTextInfo.RowText, /*bClampToMaxColumnWidth=*/false);
	...
	if (RowTextInfo.bIsClickable)
	{
		TSharedPtr<SHyperlink> Hyperlink;
		SAssignNew(Hyperlink, SHyperlink)
			.Text(RowTextInfo.RowText)
			.ToolTipText(LOCTEXT("Hyperlink", "Ctrl + Click to jump to the asset."))
			.Style(FAppStyle::Get(), "HoverOnlyHyperlink")
			.TextStyle(TextStyle)
			.Padding(Margin)
			.OverflowPolicy(RowTextInfo.OverflowPolicy)
			.OnNavigate(FSimpleDelegate::CreateLambda([Text = MoveTemp(RowTextInfo.RowText)](){ SPCGListViewItemRow::OnSoftObjectPathHyperlinkClicked(Text); }));

		return Hyperlink.ToSharedRef();
	}
	else if (!AttrAndKey.IsSet())
	{
		return SNew(STextBlock)
			.Text(MoveTemp(RowTextInfo.RowText))
			.OverflowPolicy(RowTextInfo.OverflowPolicy)
			.Margin(Margin)
			.TextStyle(TextStyle);
	}
	else if (AttrAndKey.IsSet() && AttrAndKey->Get<0>() && SharedAttributeListView->DataPtr)
	{
		auto CreateDetails = [AttrAndKey, WeakData = TWeakObjectPtr<const UPCGData>(SharedAttributeListView->DataPtr)]()
		{
			TSharedRef<SWindow> ModalWindow = SNew(SWindow)
				.Title(INVTEXT("Viewer"))
				.ClientSize(FVector2D(700, 700));

			TSharedRef<SPCGMetadataAttributeGenericViewer> Viewer = SNew(SPCGMetadataAttributeGenericViewer);
			Viewer->Setup(WeakData, AttrAndKey->Get<0>(), AttrAndKey->Get<1>());

			ModalWindow->SetContent(Viewer);
		...
		TSharedPtr<SHyperlink> Hyperlink;
		SAssignNew(Hyperlink, SHyperlink)
			.Text(RowTextInfo.bShowText ? RowTextInfo.RowText : INVTEXT("(...)"))
			.ToolTipText(LOCTEXT("HyperlinkGeneric", "Click to open a details view for this complex value."))
			.Style(FAppStyle::Get(), "HoverOnlyHyperlink")
			.TextStyle(TextStyle)
			.Padding(Margin)
			.OverflowPolicy(RowTextInfo.OverflowPolicy)
			.OnNavigate(FSimpleDelegate::CreateLambda(CreateDetails));
...
void SPCGEditorGraphAttributeListView::OnTick()
{
	if (bNeedsRefresh)
	{
		...
		// Read back any GPU data after the pin selection is set.
		if (!ReadbackGPUData())
		{
			return;
		}
		...
		RefreshDataComboBox(bKeepSelection || bKeepDataComboBoxSelection);
		RefreshAttributeList();

		bKeepDataComboBoxSelection = false;
...
bool SPCGEditorGraphAttributeListView::ReadbackGPUData() const
{
	bool bAllDataReady = true;

	if (const FPCGDataCollection* InspectionData = GetInspectionData())
	{
		for (int32 DataIndex = 0; DataIndex < InspectionData->TaggedData.Num(); ++DataIndex)
		{
			if (const UPCGProxyForGPUData* Proxy = Cast<UPCGProxyForGPUData>(InspectionData->TaggedData[DataIndex].Data))
			{
				// Poll all readbacks to progress them (don't early out if one is not ready).
				bAllDataReady &= Proxy->GetCPUData(/*InContext=*/nullptr).bComplete;
			}
			else if (const UPCGTexture2DSingleBaseData* TextureData = Cast<UPCGTexture2DSingleBaseData>(InspectionData->TaggedData[DataIndex].Data))
			{
				bAllDataReady &= TextureData->RequestCPUReadback();
			}
		}
	}

	return bAllDataReady;
}
...
void SPCGEditorGraphAttributeListView::RequestRefresh()
{
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		if (!bTickForced)
		{
			// Force tick on next game thread tick to ensure refresh happens.
			FTSTicker::GetCoreTicker().AddTicker(
				FTickerDelegate::CreateLambda([WeakThis=AsWeak()](float DeltaTime)
				{
...
void SPCGEditorGraphAttributeListView::SetNodeBeingInspected(UPCGEditorGraphNodeBase* InPCGEditorGraphNode)
{
	...
	{
		ViewportWidget->OnNewNodeInspected();
	}

	RequestRefresh();
}

void SPCGEditorGraphAttributeListView::SetViewportWidget(TSharedPtr<SPCGEditorViewport> InViewportWidget, FName InViewportEditorPanel)
{
...
TSharedPtr<FPCGInspectionData> SPCGEditorGraphAttributeListView::GetInspectionDataPtr(const TSharedPtr<FPinComboBoxItem>& EditorPin) const
{
	if (!EditorPin)
	{
		return nullptr;
	...
	if (TSharedPtr<FPCGEditor> PCGEditor = PCGEditorPtr.Pin())
	{
		...
		return PCGEditor->GetInspectionDataManager().GetInspectionDataPtr(Params);
	}
	else
	{
		return nullptr;
...
bool SPCGEditorGraphAttributeListView::InspectData(const TSharedPtr<FPinComboBoxItem>& EditorPin, TFunctionRef<void(const FPCGDataCollection&)> InspectFunc) const
{
	if (!EditorPin)
	{
		return false;
	...
	{
		const FPCGEditorInspectionDataEntrySetupParams Params{ WidgetEntryNumber, PCGEditorGraphNode.Get(), EditorPin->PinIndex, EditorPin->bIsOutputPin };
		return PCGEditor->GetInspectionDataManager().InspectData(Params, InspectFunc);
	}
	else
	{
		return false;
...
void SPCGEditorGraphAttributeListView::RefreshAttributeList()
{
	...
	else if (const UPCGTexture2DSingleBaseData* TextureData = Cast<UPCGTexture2DSingleBaseData>(PCGData))
	{
...
void SPCGEditorGraphAttributeListView::RefreshDataComboBox(bool bKeepSelection)
{
	...
	for (int32 TaggedDataIndex = 0; TaggedDataIndex < InspectionData->TaggedData.Num(); ++TaggedDataIndex)
	{
		...
		if (const UPCGProxyForGPUData* ProxyData = Cast<UPCGProxyForGPUData>(Data))
		{
			UPCGProxyForGPUData::FReadbackResult Readback = ProxyData->GetCPUData(nullptr);
			if (ensure(Readback.bComplete)) // Readbacks should have been performed at beginning of refresh
			{
				Data = Readback.TaggedData.Data;
			}
		}

		FString ItemName = FString::Format(
			TEXT("[{0}] {1}"),
			{ FText::AsNumber(TaggedDataIndex).ToString(), (Data ? Data->GetClass()->GetDisplayNameText().ToString(): TEXT("No Data")) });
...
void SPCGEditorGraphAttributeListView::RefreshDomainComboBox(bool bKeepSelection)
{
	...
	if (const UPCGProxyForGPUData* ProxyData = Cast<UPCGProxyForGPUData>(PCGData))
	{
		UPCGProxyForGPUData::FReadbackResult Readback = ProxyData->GetCPUData(nullptr);
		if (ensure(Readback.bComplete)) // Readbacks should have been performed at beginning of refresh
		{
			PCGData = Readback.TaggedData.Data;
		}
	}
...
void SPCGEditorGraphAttributeListView::RefreshViewport()
{
	...
	else if (const UPCGTexture2DSingleBaseData* TextureData = Cast<UPCGTexture2DSingleBaseData>(Data))
	{
		if (!TextureData->RequestCPUReadback())
		{
			bViewportNeedsRefresh = true;
...
bool SPCGEditorGraphAttributeListView::BuildDataCollectionForSave(bool bUsePinComboIndex, bool bUseDataComboIndex, FPCGDataCollection* OutSaveData) const
{
	// Return if we found any data to save, when OutSaveData is nullptr this method can be used to test without copying any data
	bool bDataToSave = false;
	...
	if (!bUsePinComboIndex)
	{
		...
		{
			InspectData(PinComboBoxItem, [&bDataToSave, OutSaveData](const FPCGDataCollection& InspectionData)
			{
				...
				if (OutSaveData)
				{
					OutSaveData->TaggedData.Append(InspectionData.TaggedData);
				}
			});
		}
	}
	else
	{
		if (const FPCGDataCollection* PinInspectionData = GetInspectionData())
		{
			...
			{
				if (PinInspectionData->TaggedData.Num() > 0)
				{
					bDataToSave = true;
				}
			...
			else if(int SelectedIndex = GetSelectedDataIndex(); PinInspectionData->TaggedData.IsValidIndex(SelectedIndex))
			{
				...
				if (OutSaveData)
				{
					OutSaveData->TaggedData.Add(PinInspectionData->TaggedData[SelectedIndex]);
				}
			}
		}
	}
...
{
	// FPCGInspectionData will make sure data is visible to GC while we save
	FPCGInspectionData SaveData;
	SaveData.Data = MakeShared<FPCGDataCollection>();
	if (!BuildDataCollectionForSave(bUsePinComboIndex, bUseDataComboIndex, SaveData.Data.Get()))
	{
		return;
	}

	UPCGDataCollectionExporter* Exporter = NewObject<UPCGDataCollectionExporter>();
	Exporter->Data = *SaveData.Data;
...
{
	// Query only
	return BuildDataCollectionForSave(bUsePinComboIndex, bUseDataComboIndex, nullptr);
}
...
void SPCGEditorGraphAttributeListView::OnSelectionChangedPin(TSharedPtr<FPinComboBoxItem> InItem, ESelectInfo::Type InSelectInfo)
{
	...
	bKeepDataComboBoxSelection = InSelectInfo == ESelectInfo::Direct;
	RequestRefresh();
}
...
void SPCGEditorGraphAttributeListView::AddColumn(const FPCGTableVisualizerColumnInfo& InColumnInfo)
{
	...
	ColumnData.UnderlyingAttribute = InColumnInfo.UnderlyingAttribute;
...
{
	return PCGEditorPtr.IsValid() && (WidgetEntryNumber >= 0) && (WidgetEntryNumber < PCGEditorTabs::NumAttributeTabs) && PCGEditorPtr.Pin()->IsPanelCurrentlyOpen(PCGEditorTabs::AttributesID[WidgetEntryNumber]);
}
