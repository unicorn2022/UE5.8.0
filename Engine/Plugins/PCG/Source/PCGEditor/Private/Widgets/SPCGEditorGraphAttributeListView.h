...
public:
	...
	void SetViewportWidget(TSharedPtr<SPCGEditorViewport> InViewportWidget, FName InViewportEditorPanel);
...
private:
	...
	/** Read back any GPU data in the inspection data. Returns true when read backs complete. */
	bool ReadbackGPUData() const;
	...
	TSharedPtr<FPCGInspectionData> GetInspectionDataPtr(const TSharedPtr<FPinComboBoxItem>& EditorPin) const;
	bool InspectData(const TSharedPtr<FPinComboBoxItem>& EditorPin, TFunctionRef<void(const FPCGDataCollection&)> InspectFunc) const;
	...
	bool BuildDataCollectionForSave(bool bUsePinComboIndex, bool bUseDataComboIndex, FPCGDataCollection* OutData) const;
	...
	/** Data visualization viewport editor panel id. */
	FName ViewportEditorPanel;
	...
	bool bTickForced : 1 = false;

	bool bKeepDataComboBoxSelection = true;
