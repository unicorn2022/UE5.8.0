...
void SPCGEditorGraphNodePalette::CollectAllActions(FGraphActionListBuilderBase& OutAllActions)
{
	...
	check(Editor);
	FPCGActionsFilter ActionFilter(Editor->GetMainEditorGraph(), ElementType & PCGSchema->GetElementTypeFiltering(), PCGSchema->GetGraphEditorCustomization(Editor->GetMainEditorGraph()));
