...
FPCGCodeEditorTextBoxCommands::FPCGCodeEditorTextBoxCommands() 
	: TCommands<FPCGCodeEditorTextBoxCommands>(
...
void FPCGCodeEditorTextBoxCommands::RegisterCommands()
{
...
SPCGCodeEditorTextBox::SPCGCodeEditorTextBox()
...
void SPCGCodeEditorTextBox::Construct(const FArguments& InArgs)
{
	RegisterCommands();
	...
	const FPCGCodeEditorTextBoxCommands& Commands = FPCGCodeEditorTextBoxCommands::Get();
	...
	Text = SNew(SMultiLineEditableText)
			...
			.OnKeyCharHandler(this, &SPCGCodeEditorTextBox::OnTextKeyChar)
			.OnKeyDownHandler(this, &SPCGCodeEditorTextBox::OnTextKeyDown)
			...
			.IsReadOnly(this, &SPCGCodeEditorTextBox::IsReadOnly)
			.HScrollBar(HScrollBar)
			.VScrollBar(VScrollBar);

	SearchBar = SNew(SPCGShaderTextSearchWidget)
		.OnTextChanged(this, &SPCGCodeEditorTextBox::OnSearchTextChanged)
		.OnTextCommitted(this, &SPCGCodeEditorTextBox::OnSearchTextCommitted)
		.SearchResultData(this, &SPCGCodeEditorTextBox::GetSearchResultData)
		.OnResultNavigationButtonClicked(this, &SPCGCodeEditorTextBox::OnSearchResultNavigationButtonClicked);
	...
	[
		...
		[
			...
			.BorderBackgroundColor(this, &SPCGCodeEditorTextBox::GetBorderBackgroundColor)
			[
				SNew(SGridPanel)
				.FillColumn(0,1.0f)
				.FillRow(0,1.0f)
...
void SPCGCodeEditorTextBox::SetTextProviderObject(UObject* InProviderObject)
{
...
void SPCGCodeEditorTextBox::RegisterCommands()
{
	const FPCGCodeEditorTextBoxCommands& Commands = FPCGCodeEditorTextBoxCommands::Get();
	
	TopLevelCommandList->MapAction(Commands.Search, FExecuteAction::CreateSP(this, &SPCGCodeEditorTextBox::OnTriggerSearch));
	TopLevelCommandList->MapAction(Commands.NextOccurrence, FExecuteAction::CreateSP(this, &SPCGCodeEditorTextBox::OnGoToNextOccurrence));
	TopLevelCommandList->MapAction(Commands.PreviousOccurrence, FExecuteAction::CreateSP(this, &SPCGCodeEditorTextBox::OnGoToPreviousOccurrence));
	TextCommandList->MapAction(Commands.ApplyChanges, FExecuteAction::CreateSP(this, &SPCGCodeEditorTextBox::OnApplyChanges));
	TextCommandList->MapAction(Commands.ToggleComment, FExecuteAction::CreateSP(this, &SPCGCodeEditorTextBox::OnToggleComment));
}

FReply SPCGCodeEditorTextBox::OnPreviewKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
bool SPCGCodeEditorTextBox::HandleEscape()
{
	if (HideSearchBar())
	{
		return true;
...
void SPCGCodeEditorTextBox::ShowSearchBar()
{
...
bool SPCGCodeEditorTextBox::HideSearchBar() 
{
	if (!bIsSearchBarHidden)
	{
		bIsSearchBarHidden = true;
...
void SPCGCodeEditorTextBox::OnTriggerSearch()
{
	ShowSearchBar();
...
void SPCGCodeEditorTextBox::Refresh() const
{
	check(IsInGameThread());
	Text->Refresh();
}

void SPCGCodeEditorTextBox::OnSearchTextChanged(const FText& InTextToSearch)
{
...
void SPCGCodeEditorTextBox::OnSearchTextCommitted(const FText& InTextToSearch, ETextCommit::Type InCommitType)
{
...
TOptional<SSearchBox::FSearchResultData> SPCGCodeEditorTextBox::GetSearchResultData() const
{
...
void SPCGCodeEditorTextBox::OnSearchResultNavigationButtonClicked(SSearchBox::SearchDirection InDirection)
{
	Text->AdvanceSearch(InDirection == SSearchBox::SearchDirection::Previous);
}

void SPCGCodeEditorTextBox::OnGoToNextOccurrence()
{
	OnSearchResultNavigationButtonClicked(SSearchBox::SearchDirection::Next);
}

void SPCGCodeEditorTextBox::OnGoToPreviousOccurrence()
{
	OnSearchResultNavigationButtonClicked(SSearchBox::SearchDirection::Previous);
}

FReply SPCGCodeEditorTextBox::OnTextKeyDown(
...
void SPCGCodeEditorTextBox::OnApplyChanges() const
{
	OnTextChangesAppliedCallback.ExecuteIfBound();
}

void SPCGCodeEditorTextBox::OnToggleComment() const
{
	...
	if (bShouldGoToNewLine)
	{
		...
		if (LineIndex < const_cast<const SMultiLineEditableText*>(Text.Get())->GetTextLineCount())
		{
			FString Line;
...
FReply SPCGCodeEditorTextBox::OnTextKeyChar(
	const FGeometry& MyGeometry,
...
void SPCGCodeEditorTextBox::HandleAutoIndent() const
{
	const FTextLocation CursorLocation = Text->GetCursorLocation();
...
bool SPCGCodeEditorTextBox::IsReadOnly() const
{
	if (IPCGCodeEditorTextProvider* Provider = Cast<IPCGCodeEditorTextProvider>(ShaderTextProviderObject))
	{
...
FSlateColor SPCGCodeEditorTextBox::GetBorderBackgroundColor() const
{
	// Hand tweaked values.
	return IsReadOnly() ? FLinearColor(0.72f, 0.72f, 0.72f, 1.0f) : FLinearColor(0.4f, 0.4f, 0.4f, 1.0f);
}
