...
void SPCGEditorGraphUserParametersView::Construct(const FArguments& InArgs, const TSharedPtr<FPCGEditor>& InPCGEditor)
{
	...
	UPCGGraph* PCGGraph = InPCGEditor.IsValid() ? InPCGEditor->GetFocusedGraph() : nullptr;
	...
	FDetailsViewArgs DetailsViewArgs;
	{
		...
		DetailsViewArgs.ShouldForceHideProperty.BindLambda(
			[](const TSharedRef<IPropertyHandle>& PropertyNode)
			{
				...
				if (const FProperty* Property = PropertyNode->GetProperty())
				{
					...
					for (TSharedPtr<IPropertyHandle> ParentNode = PropertyNode->GetParentHandle(); ParentNode != nullptr; ParentNode = ParentNode->GetParentHandle())
					{
...
void SPCGEditorGraphUserParametersView::SetGraph(UPCGGraph* InGraph)
{
	if (DetailsView)
	{
