...
FPCGEditorStyle::FPCGEditorStyle() : FSlateStyleSet("PCGEditorStyle")
{
	...
	Set(PCGEditorStyleConstants::NodeFilter, new IMAGE_BRUSH_SVG("Style/PCG_Graph_Filter", Icon16x16));
	Set(PCGEditorStyleConstants::NodeConvert, new IMAGE_BRUSH_SVG("Style/PCG_Graph_To", Icon16x16));

	Set(PCGEditorStyleConstants::Node_ToolStart, new IMAGE_BRUSH_SVG("Style/PCG_Node_ViewportToolStart", Icon20x20));
	Set(PCGEditorStyleConstants::Node_DataOverride, new IMAGE_BRUSH_SVG("Style/PCG_Node_DataOverride", Icon16x16));
	Set(PCGEditorStyleConstants::Node_ManualEdit, new IMAGE_BRUSH_SVG("Style/PCG_Node_ManualEdit", Icon16x16));
	...
	// Orphaned override row style for the Data Overrides panel.
	{
		FTableRowStyle OrphanedOverrideRowStyle = FAppStyle::Get().GetWidgetStyle<FTableRowStyle>("DataTableEditor.CellListViewRow");

		const FSlateColorBrush OrphanedOverrideEvenBackground(FLinearColor(0.135f, 0.105f, 0.090f, 1.0f));
		const FSlateColorBrush OrphanedOverrideEvenBackgroundHovered(FLinearColor(0.180f, 0.140f, 0.120f, 1.0f));
		const FSlateColorBrush OrphanedOverrideOddBackground(FLinearColor(0.075f, 0.055f, 0.045f, 1.0f));
		const FSlateColorBrush OrphanedOverrideOddBackgroundHovered(FLinearColor(0.110f, 0.080f, 0.070f, 1.0f));
		const FSlateColor OrphanedOverrideTextColor(FLinearColor(1.0f, 0.78f, 0.74f, 1.0f));

		OrphanedOverrideRowStyle.SetEvenRowBackgroundBrush(OrphanedOverrideEvenBackground);
		OrphanedOverrideRowStyle.SetEvenRowBackgroundHoveredBrush(OrphanedOverrideEvenBackgroundHovered);
		OrphanedOverrideRowStyle.SetOddRowBackgroundBrush(OrphanedOverrideOddBackground);
		OrphanedOverrideRowStyle.SetOddRowBackgroundHoveredBrush(OrphanedOverrideOddBackgroundHovered);
		OrphanedOverrideRowStyle.SetTextColor(OrphanedOverrideTextColor);
		OrphanedOverrideRowStyle.SetSelectedTextColor(OrphanedOverrideTextColor);

		Set("PCG.OrphanedOverrideRow", OrphanedOverrideRowStyle);
	}
	...
	Set("PCG.Command.EditGraph", new IMAGE_BRUSH_SVG("Style/PCG_Command_EditGraph", Icon20x20));
	...
	// Viewport overlay styles
	{
		FLinearColor ViewportOverlayBackgroundColor = FStyleColors::Foreground.GetSpecifiedColor();
		ViewportOverlayBackgroundColor.A = 0.4f;
		Set("PCG.OverlayBackgroundBrush", new FSlateRoundedBoxBrush(ViewportOverlayBackgroundColor, 8.0f, FStyleColors::Dropdown, 1.0f));

		FLinearColor InnerPanelColor = FStyleColors::Panel.GetSpecifiedColor();
		InnerPanelColor.A = 0.85f;
		Set("PCG.OverlayInnerPanelBrush", new FSlateRoundedBoxBrush(InnerPanelColor, 6.0f));
	}
