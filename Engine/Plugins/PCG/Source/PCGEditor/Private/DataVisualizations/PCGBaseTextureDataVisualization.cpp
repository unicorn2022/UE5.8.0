...
{
	return [this, WeakData=TWeakObjectPtr<const UPCGTexture2DSingleBaseData>(Cast<UPCGTexture2DSingleBaseData>(Data))](FPCGSceneSetupParams& InOutParams)
	{
		...
		else if (WeakData->GetTextureResourceType() == EPCGTextureResourceType::ExportedTexture)
		{
			...
			VisParams.TextureRHI = WeakData->GetTextureRHI();
			VisParams.Resolution = WeakData->GetResolution();
			VisParams.SliceIndex = WeakData->GetTextureSlice();
		...
		{
			UE_LOGF(LogPCG, Error, "Texture data uses an unsupported resource type for data viewport visualization.");
			return;
		}
