...
public:
	...
	PCGEDITOR_API const FPCGDataCollection* GetInspectionData(const FPCGEditorInspectionDataEntrySetupParams& InParams) const { return nullptr; }

	PCGEDITOR_API const TSharedPtr<FPCGInspectionData> GetInspectionDataPtr(const FPCGEditorInspectionDataEntrySetupParams& InParams) const;
	PCGEDITOR_API bool InspectData(const FPCGEditorInspectionDataEntrySetupParams& InParams, TFunctionRef<void(const FPCGDataCollection&)> InspectFunc) const;
...
private:
	void ConstructEntry(const FPCGEditorInspectionDataEntrySetupParams& InParams, FPCGEditorInspectionDataEntry& OutEntry) const;

	bool GetInspectionDataParams(const FPCGEditorInspectionDataEntrySetupParams& InParams, FPCGStack& OutStack, UPCGPin const*& OutPin) const;
