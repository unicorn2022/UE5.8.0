...
public:
	...
	/** Call when a new node is selected. The next ResetScene will restore the viewport type to default and SetupScene will auto-focus. */
	PCGEDITOR_API void OnNewNodeInspected();
...
protected:
	...
	/** Slot in the viewport toolbar populated by the active visualization with its ViewportToolbarWidget. */
	TSharedPtr<SBox> DynamicToolbarSlot;

private:
	/** Set by OnNewNodeInspected(); consumed by ResetScene() to restore viewport type and trigger auto-focus. */
	bool bPendingViewportReset = false;
};
