...
UENUM()
enum class EPCGTreatViewportAsGenerationSource : uint8
{
	PCGWorldActor UMETA(DisplayName = "Use PCG World Actor flag"),
	Always,
	Never,
};
...
public:
	...
	/** Actor classes to ignore for the PCG Level to Asset export */
	UPROPERTY(EditAnywhere, config, Category = "PCG Level to Asset", meta=(DisplayName = "Excluded Actor Classes"))
	TArray<TSubclassOf<AActor>> PCGLevelToAssetExcludedActorClasses;

	/** Deprecated. Replaced by per-graph UPCGGraph::bAutoRefreshOnRenderStateRefresh (and bAutoRefreshOnQualitySettingsChanged for quality events). */
	UE_DEPRECATED(5.8, "Use UPCGGraph::bAutoRefreshOnRenderStateRefresh / bAutoRefreshOnQualitySettingsChanged on each graph instead.")
	UPROPERTY(Config)
...
public:
	...
	/** Color used for embedded subgraph-like operations */
	UPROPERTY(EditAnywhere, config, Category = Node, meta = (HideAlphaChannel))
	FLinearColor EmbeddedSubgraphNodeColor;
	...
	/** Color used for data pins of type Data View */
	UPROPERTY(EditAnywhere, config, Category = Node, meta = (HideAlphaChannel))
	FLinearColor DataViewPinColor;
	...
	/** Color used for data pins of type Raw Buffer */
	UPROPERTY(EditAnywhere, config, Category = Node, meta = (HideAlphaChannel))
	FLinearColor RawBufferPinColor;
	...
	/** Controls if we want to treat the viewport as a runtime generation source */
	UPROPERTY(EditAnywhere, config, Category = "Editor Performance")
	EPCGTreatViewportAsGenerationSource TreatViewportAsGenerationSource = EPCGTreatViewportAsGenerationSource::PCGWorldActor;
	...
	/** Font size used in the HLSL Code Editor. */
	UPROPERTY(EditAnywhere, config, Category = "Code Editor", meta = (ClampMin = 6, ClampMax = 32))
	int32 CodeEditorFontSize = 9;
