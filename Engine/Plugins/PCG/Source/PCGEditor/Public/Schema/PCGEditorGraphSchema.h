...
namespace PCGActionsHelpers
{
	...
	ENUM_CLASS_FLAGS(ECompatibilityMask);
}
...
public:
	...
	// Returns the settings class to use when creating a subgraph node. Override in derived schemas to provide a custom subgraph settings class.
	PCGEDITOR_API virtual TSubclassOf<UPCGBaseSubgraphSettings> GetSubgraphSettingsClass() const;
	...
	PCGEDITOR_API virtual void DroppedAssetsOnNode(const TArray<struct FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const override;
	...
	PCGEDITOR_API virtual void GetAssetsNodeHoverMessage(const TArray<struct FAssetData>& Assets, const UEdGraphNode* HoverNode, FString& OutTooltipText, bool& OutOkIcon) const override;
	...
	PCGEDITOR_API virtual void GetGraphDisplayInformation(const UEdGraph& Graph, /*out*/ FGraphDisplayInfo& DisplayInfo) const override;
	...
	virtual bool ShouldAddViewportEditBrushOnNode() const { return true; }
...
protected:
	...
	PCGEDITOR_API virtual bool DroppedAssetsOnNodeInternal(const TArray<struct FAssetData>& Assets, const FVector2f& GraphPosition, const UEdGraphNode* Node, bool bValidate, FString& OutTooltipText) const;
