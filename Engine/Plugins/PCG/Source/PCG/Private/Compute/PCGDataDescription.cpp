...
bool FPCGKernelAttributeKey::UpdateIdentifierFromSelector()
{
	...
	{
		UE_LOGF(LogPCG, Error, "While updating FPCGKernelAttributeKey with %ls, the selector was not targeting an attribute. Discarded.", *Name.ToString());
	...
	{
		//UE_LOGF(LogPCG, Error, "While updating FPCGKernelAttributeKey with %ls, the selector was targeting a domain that is not the @Data domain which is unsupported at the moment. Discarded.", *Name.ToString());
		UE_LOGF(LogPCG, Error, "While updating FPCGKernelAttributeKey with %ls, the selector was targeting a domain which is unsupported at the moment. Discarded.", *Name.ToString());
		Identifier.MetadataDomain = PCGMetadataDomainID::Invalid;
		return true;
	}
...
int32 FPCGKernelAttributeTable::AddAttributeAssumeUnique(const FPCGKernelAttributeKey& Key)
{
	if (AttributeTable.Num() < PCGDataCollectionPackingConstants::MAX_NUM_CUSTOM_ATTRS)
	{
		return AttributeTable.Add(Key);
	}
	else
	{
		return INDEX_NONE;
...
FPCGKernelAttributeTable::FAttributeKeyToIndexMap FPCGKernelAttributeTable::GetAttributeKeyToIndex() const
{
	FAttributeKeyToIndexMap AttributeToIndex;
	AttributeToIndex.Reserve(AttributeTable.Num());

	for (int Index = 0; Index < AttributeTable.Num(); ++Index)
	{
		AttributeToIndex.Add(AttributeTable[Index], Index);
	}
...
void FPCGKernelAttributeTable::DebugLog() const
{
	...
	{
		UE_LOGF(LogPCG, Display, "\tName: %ls\t\tID: %d\t\tType: %ls",
...
FPCGDataDesc::FPCGDataDesc(const FPCGTaggedData& InTaggedData, const UPCGDataBinding* InBinding)
{
	...
	// @todo_pcg: Implement a way for a UPCGData to generate its data description.
	if (const UPCGTexture2DBaseData* TextureData = Cast<UPCGTexture2DBaseData>(InTaggedData.Data))
	{
		SetTextureArraySize(TextureData->GetArraySize());
		SetRenderTargetFormat(PCGComputeHelpers::GetPCGRenderTargetFormatFromPixelFormat(TextureData->GetFormat()));
		SetTextureFilter(TextureData->GetFilter());
		SetTextureTransform(TextureData->GetTransform());
	}
...
void FPCGDataDesc::DeallocateProperties(EPCGPointNativeProperties InProperties)
{
	// Never deallocate Density - it is used to remove points.
	AllocatedPointProperties &= ~(InProperties & ~EPCGPointNativeProperties::Density);
}
...
bool FPCGDataDesc::IsAttributeAllocated(int32 InAttributeId) const
{
	...
	if (Type == FPCGDataTypeInfoPoint::AsId() && InAttributeId < PCGDataCollectionPackingConstants::NUM_RESERVED_ATTRS)
	{
...
void FPCGDataDesc::InitializeAttributeDescs(const UPCGData* InData, const UPCGDataBinding* InBinding)
{
	...
	if (Type == FPCGDataTypeInfoPoint::AsId())
	{
	...
	if (InBinding && Metadata)
	{
		...
		for (int CustomAttributeIndex = 0; CustomAttributeIndex < AttributeIdentifiers.Num(); ++CustomAttributeIndex)
		{
			...
			if (AttributeType == EPCGKernelAttributeType::Invalid)
			{
				...
				UE_LOGF(LogPCG, Warning, "Skipping attribute '%ls'. '%ls' type attributes are not supported on GPU.",
					*Selector.ToString(),
			...
			if (CustomAttributeIndex >= PCGDataCollectionPackingConstants::MAX_NUM_CUSTOM_ATTRS)
			{
				...
				UE_LOGF(LogPCG, Warning, "Attempted to exceed max number of custom attributes (%d). Additional attributes will be ignored.", PCGDataCollectionPackingConstants::MAX_NUM_CUSTOM_ATTRS);
				break;
			}
...
void FPCGDataCollectionDesc::DeallocatePropertiesForAllData(EPCGPointNativeProperties InProperties)
{
	for (FPCGDataDesc& DataDesc : DataDescs)
	{
		DataDesc.DeallocateProperties(InProperties);
	}
}

EPCGPointNativeProperties FPCGDataCollectionDesc::GetAllocatedPropertiesUnion() const
{
	EPCGPointNativeProperties Result = EPCGPointNativeProperties::None;
	for (const FPCGDataDesc& DataDesc : DataDescs)
	{
		Result |= DataDesc.GetAllocatedProperties();
	}
...
void FPCGDataCollectionDesc::SetRenderTargetFormatForAllData(EPCGRenderTargetFormat InRenderTargetFormat)
{
	for (FPCGDataDesc& DataDesc : DataDescs)
	{
		DataDesc.SetRenderTargetFormat(InRenderTargetFormat);
	}
}

void FPCGDataCollectionDesc::SetTextureFilterForAllData(EPCGTextureFilter InTextureFilter)
{
	for (FPCGDataDesc& DataDesc : DataDescs)
	{
		DataDesc.SetTextureFilter(InTextureFilter);
	}
}

void FPCGDataCollectionDesc::SetTextureArraySizeForAllData(uint16 InTextureArraySize)
{
	for (FPCGDataDesc& DataDesc : DataDescs)
	{
		DataDesc.SetTextureArraySize(InTextureArraySize);
	}
}

void FPCGDataCollectionDesc::SetTextureTransformForAllData(const FTransform& InTransform)
{
	for (FPCGDataDesc& DataDesc : DataDescs)
	{
		DataDesc.SetTextureTransform(InTransform);
	}
}
