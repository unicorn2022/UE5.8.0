...
public:
	...
	/** Get the virtual label associated with a kernel pin. This only exists if the kernel pin is an input pin on the border of the compute graph. */
	const FName* GetVirtualPinLabelFromKernelInputPin(const FPCGKernelPin& InKernelPin) const { return KernelPinToVirtualPinLabel.Find(InKernelPin); }
	...
	bool IsGraphDataValid(const UPCGDataBinding* InDataBinding, FPCGContext* InContext = nullptr) const;
	...
	/** Returns the execution group required by this compute graph based on its kernels' requirements. */
	FName GetRequiredExecutionGroup() const;
...
protected:
	...
	/** Kernel input pin to virtual pin label, populated for pins that receive data from external output pins. */
	UPROPERTY()
	TMap<FPCGKernelPin, /*VirtualPinLabel*/FName> KernelPinToVirtualPinLabel;
