...
namespace PCGRawBufferDataInterface
{
	...
	/** Adds function definitions common to both SRV (input) and UAV (output) views. */
...
void UPCGRawBufferDataInterface::GetSupportedInputs(TArray<FShaderFunctionDefinition>& OutFunctions) const
{
#if WITH_EDITOR
	PCGRawBufferDataInterface::AddCommonFunctions(OutFunctions);

	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetSRVFunction(TEXT("Load")))
		.AddParam(EShaderFundamentalType::Uint) // InElementIndex
		.AddReturnType(EShaderFundamentalType::Uint);

	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetSRVFunction(TEXT("Load4")))
...
void UPCGRawBufferDataInterface::GetSupportedOutputs(TArray<FShaderFunctionDefinition>& OutFunctions) const
{
	...
	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetUAVFunction(TEXT("Load")))
		.AddParam(EShaderFundamentalType::Uint) // InElementIndex
		.AddReturnType(EShaderFundamentalType::Uint);

	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetUAVFunction(TEXT("Load4")))
		.AddParam(EShaderFundamentalType::Uint) // InFirstElementIndex
		.AddReturnType(EShaderFundamentalType::Uint, 4);

	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetUAVFunction(TEXT("Store")))
		.AddParam(EShaderFundamentalType::Uint) // InElementIndex
		.AddParam(EShaderFundamentalType::Uint); // InValue

	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetUAVFunction(TEXT("Store4")))
		.AddParam(EShaderFundamentalType::Uint) // InFirstElementIndex
		.AddParam(EShaderFundamentalType::Uint, 4); // InValue

	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetUAVFunction(TEXT("AtomicAdd")))
		.AddParam(EShaderFundamentalType::Uint) // InElementIndex
		.AddParam(EShaderFundamentalType::Uint) // InValueToAdd
		.AddReturnType(EShaderFundamentalType::Uint); // Value before add

	OutFunctions.AddDefaulted_GetRef()
		.SetName(PCGComputeHelpers::GetUAVFunction(TEXT("AtomicMax")))
		.AddParam(EShaderFundamentalType::Uint) // InElementIndex
		.AddParam(EShaderFundamentalType::Uint) // InValueToMax
		.AddReturnType(EShaderFundamentalType::Uint); // Value before max
#endif // WITH_EDITOR
}

BEGIN_SHADER_PARAMETER_STRUCT(FPCGRawBufferDataInterfaceParameters,)
	SHADER_PARAMETER_RDG_BUFFER_SRV(ByteAddressBuffer, DataSRV)
	SHADER_PARAMETER_RDG_BUFFER_UAV(RWByteAddressBuffer, DataUAV)
...
void UPCGRawBufferDataInterface::GetHLSL(FString& OutHLSL, FString const& InDataInterfaceName) const
{
	...
	// Expand the {{BeginDuplicateForSRVUAV}} / {{EndDuplicateForSRVUAV}} region into parallel SRV and UAV variants.
...
FComputeDataProviderRenderProxy* UPCGRawBufferDataProvider::GetRenderProxy()
{
	...
	Params.AsyncReadbackCallback_RenderThread = [ReadbackState=ReadbackState, DataProviderWeakPtr=ThisWeak, GenerationCount=GetGenerationCounter(), ExportMode=GetExportMode(), OutputPinLabel=GetOutputPinLabel(), OutputPinLabelAlias = GetOutputPinLabelAlias(), BindingWeakPtr=TWeakObjectPtr<UPCGDataBinding>(GetDataBinding()), PinDesc](const void* InData, int InNumBytes)
	{
		...
		ExecuteOnGameThread(UE_SOURCE_LOCATION, [ReadbackState, DataProviderWeakPtr, GenerationCount, ExportMode, OutputPinLabel, OutputPinLabelAlias, BindingWeakPtr, PinDesc]()
		{
			LLM_SCOPE_BYTAG(PCG);
			...
			if (!DataProvider)
			{
				UE_LOGF(LogPCG, Error, "Could not resolve UPCGRawBufferDataProvider pointer to pass back buffer handle.");
				return;
			}
			...
			Binding->ReceiveDataFromGPU_GameThread(RawBufferData, DataProvider->GetProducerSettings(), ExportMode, OutputPinLabel, OutputPinLabelAlias, PinDesc->GetDataDescriptions()[0]);
...
bool FPCGRawBufferDataProviderProxy::IsValid(FValidationData const& InValidationData) const
{
	...
	{
		UE_LOGF(LogPCG, Warning, "FPCGRawBufferDataProviderProxy not valid due to invalid size: %d bytes.", SizeBytes);
		return false;
	}
...
void FPCGRawBufferDataProviderProxy::AllocateResources(FRDGBuilder& GraphBuilder, FAllocationData const& InAllocationData)
{
	...
	DataSRV = GraphBuilder.CreateSRV(Data);
...
void FPCGRawBufferDataProviderProxy::GatherDispatchData(FDispatchData const& InDispatchData)
{
	...
	for (int32 InvocationIndex = 0; InvocationIndex < ParameterArray.Num(); ++InvocationIndex)
	{
		...
		// Both views target the same physical buffer. ValidateShaderParameters requires every slot filled; shader
		// DCE drops whichever view the shader doesn't reference, so the unused side consumes no GPU register.
		Parameters.DataSRV = DataSRV;
		Parameters.DataUAV = DataUAV;
