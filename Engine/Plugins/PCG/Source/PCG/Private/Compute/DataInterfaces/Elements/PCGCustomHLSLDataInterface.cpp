...
BEGIN_SHADER_PARAMETER_STRUCT(FPCGCustomHLSLDataInterfaceParameters,)
	...
	SHADER_PARAMETER(int32, NumElementsX)
	SHADER_PARAMETER(int32, NumElementsY)
	SHADER_PARAMETER(int32, NumElementsZ)
...
void UPCGCustomHLSLDataInterface::GetShaderHash(FString& InOutKey) const
{
	// Required because the shader code didn't change when we added this param to the shader struct.
	InOutKey += TEXT("PCGCustomHLSL_DI_v2_AddNumElementsZToShaderParamStruct");
}
...
FComputeDataProviderRenderProxy* UPCGCustomHLSLDataProvider::GetRenderProxy()
{
	...
	if (ensure(KernelParams))
	{
		...
		ProxyData.NumElementsX = KernelParams->GetValueInt(GET_MEMBER_NAME_CHECKED(UPCGCustomHLSLSettings, NumElementsX));
		ProxyData.NumElementsY = KernelParams->GetValueInt(GET_MEMBER_NAME_CHECKED(UPCGCustomHLSLSettings, NumElementsY));
		ProxyData.NumElementsZ = KernelParams->GetValueInt(GET_MEMBER_NAME_CHECKED(UPCGCustomHLSLSettings, NumElementsZ));
...
void FPCGCustomHLSLDataProviderProxy::GatherDispatchData(FDispatchData const& InDispatchData)
{
	...
	for (int32 InvocationIndex = 0; InvocationIndex < ParameterArray.Num(); ++InvocationIndex)
	{
		...
		Parameters.NumElementsX = Data.NumElementsX;
		Parameters.NumElementsY = Data.NumElementsY;
		Parameters.NumElementsZ = Data.NumElementsZ;
