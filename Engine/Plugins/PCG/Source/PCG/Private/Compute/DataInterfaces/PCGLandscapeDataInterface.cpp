	...
	: ResourceKey(InKey)
{
	...
	if (!LandscapeInfo)
	{
		UE_LOGF(LogPCG, Warning, "FPCGLandscapeResource: Could not retrieve landscape info to setup landscape height data.");
		return;
	}
...
BEGIN_SHADER_PARAMETER_STRUCT(FPCGLandscapeDataInterfaceParameters,)
	...
	SHADER_PARAMETER(FUintVector2,				NormalVirtualTexturePageTableLayers)
...
void UPCGLandscapeDataProvider::InitFromLandscape(UPCGDataBinding* InBinding, ALandscape* InLandscape, const FBox& Bounds, bool bAllowSampleVirtualTexture, bool bAllowSampleVirtualTextureNormal)
{
	...
	{
		UE_CLOGF(bAllowSampleVirtualTexture, LogPCG, Verbose, "UPCGLandscapeDataProvider: No runtime virtual texture found to use for landscape heights, falling back to uploading data from landscape collision components.");
...
void UPCGLandscapeDataProvider::InitializeFromLandscapeCollision(UPCGDataBinding* InBinding, ALandscape* InLandscape, const FBox& Bounds)
{
	...
	// Rescale based on the quad size.
	...
	// Clamp between Min/Max int32.
	const FVector MinIntValue = FVector(double(TNumericLimits<int32>::Min()));
	SystemBoundsInLandscape.Min = SystemBoundsInLandscape.Min.ComponentMax(MinIntValue);
	SystemBoundsInLandscape.Max = SystemBoundsInLandscape.Max.ComponentMax(MinIntValue);
...
void FPCGLandscapeDataProviderProxy::AllocateResources(FRDGBuilder& GraphBuilder, FAllocationData const& InAllocationData)
{
	...
	if (TStrongObjectPtr<const URuntimeVirtualTexture> BaseColorVT = BaseColorVirtualTexture.Pin())
	{
		...
		BaseColorVirtualPage.Initialize(BaseColorVT.Get());
	}
	...
	if (TStrongObjectPtr<const URuntimeVirtualTexture> HeightVT = HeightVirtualTexture.Pin())
	{
		...
		HeightVirtualPage.Initialize(HeightVT.Get());
	}
	...
	if (TStrongObjectPtr<const URuntimeVirtualTexture> NormalVT = NormalVirtualTexture.Pin())
	{
		...
		NormalVirtualPage.Initialize(NormalVTPtr);
		...
		case ERuntimeVirtualTextureMaterialType::BaseColor_Normal_Roughness:
			...
			NormalPageTableLayers = NormalVirtualPage.bSinglePhysicalSpace ? FUintVector2(0, 0) : FUintVector2(0, 1);
			break;
		case ERuntimeVirtualTextureMaterialType::BaseColor_Normal_Specular:
			NormalVirtualLayer0.Initialize(NormalVTPtr, /*PageTableIndex=*/0, /*bSRGB=*/false);
			NormalVirtualLayer1.Initialize(NormalVTPtr, /*PageTableIndex=*/1, /*bSRGB=*/false);
		...
		case ERuntimeVirtualTextureMaterialType::BaseColor_Normal_Specular_Mask_YCoCg:
			...
			NormalPageTableLayers = NormalVirtualPage.bSinglePhysicalSpace ? FUintVector2(0, 0) : FUintVector2(1, 2);
			break;
		default:
			...
			NormalPageTableLayers = FUintVector2(0, 0);
			break;
		}
	}
...
void FPCGLandscapeDataProviderProxy::SetShaderParameters(FParameters& OutShaderParameters)
{
	...
	{
		// Can be hit frequently in editor in unloaded areas for spatially loaded landscapes.
		UE_LOGF(LogPCG, Verbose, "PCGLandscapeDataInterface: No height texture initialized. If you are trying to use a virtual texture, make sure the RVT volumes are created. This may also occur when generating in areas where the landscape is not loaded.");
	}
...
bool FPCGLandscapeDataProviderProxy::SetNormalVirtualTextureParameters(FParameters& OutShaderParameters) const
{
	...
	OutShaderParameters.NormalVirtualTexturePageTableLayers = NormalPageTableLayers;
	return true;
}
...
void FPCGLandscapeDataProviderProxy::SetNormalVirtualTextureParameters_Default(FParameters& OutShaderParameters)
{
	...
	OutShaderParameters.NormalVirtualTexturePageTableLayers = FUintVector2(0, 0);
