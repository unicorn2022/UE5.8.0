...
public:
	...
	/** Make GetSupportedInputs return the union of read and write functions, to not break legacy Custom HLSL nodes. */
	UPROPERTY()
	bool bLegacyAllowSettersOnInputs = false;
...
public:
	struct FSetupParams
	{
		...
		uint32 ElementCountMultiplier = 0;
		uint32 AllocatedNativePropertiesMask = 0;
...
protected:
	...
	/** Union of EPCGPointNativeProperties bits across all output data items: which native properties are N-slot (fully allocated) vs CVR. Set from the pin description at proxy creation time. */
	uint32 AllocatedNativePropertiesMask = 0;
	...
	FRDGBufferSRVRef BufferSRV = nullptr;
