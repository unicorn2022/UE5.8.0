...
void UPCGSkinnedMeshSpawnerDataInterface::GetHLSL(FString& OutHLSL, FString const& InDataInterfaceName) const
{
	...
	OutHLSL += FString::Format(TEXT(
		...
		"}\n"
		"\n"
		"uint SMSpawner_GetPrimitiveIndexFromPrimitiveId_{DataInterfaceName}(int InMeshPathStringKey)\n"
		"{\n"
		...
		"}\n"
		"\n"
		// todo_pcg: Culling cell functions are not implemented for the skinned mesh spawner. Stubs return the
		// 'no culling' defaults so that the if (NumCullingCellsTotal > 1) block in the USF is skipped.
...
bool UPCGSkinnedMeshSpawnerDataProvider::PerformPreExecuteReadbacks_GameThread(UPCGDataBinding* InBinding)
{
	...
	const FPCGMetadataAttributeBase* AnalysisValueAttributeBase = AnalysisMetadata ? AnalysisMetadata->GetConstAttribute(PCGAttributeAnalysisKernelConstants::ValueAttributeName) : nullptr;
	const FPCGMetadataAttributeBase* AnalysisCountAttributeBase = AnalysisMetadata ? AnalysisMetadata->GetConstAttribute(PCGAttributeAnalysisKernelConstants::ValueCountAttributeName) : nullptr;
...
bool UPCGSkinnedMeshSpawnerDataProvider::PrepareForExecute_GameThread(UPCGDataBinding* InBinding)
{
	...
	if (!bPrimitiveDescriptorsCreated)
	{
		...
		{
			UE_LOGF(LogPCG, Verbose, "Request '%ls' to load.", *PrimitiveDescriptor.SkinnedAsset.ToString());
	...
	if (!bStaticMeshesLoaded)
	{
		for (const FPCGSoftSkinnedMeshComponentDescriptor& PrimitiveDescriptor : PrimitiveDescriptors)
		{
			...
			{
				UE_LOGF(LogPCG, Verbose, "Waiting for '%ls' to load.", *PrimitiveDescriptor.SkinnedAsset.ToString());
...
void UPCGSkinnedMeshSpawnerDataProvider::CreatePrimitiveDescriptors(FPCGContext* InContext, UPCGDataBinding* InBinding)
{
	...
	AActor* TargetActor = Settings->TargetActor.Get() ? Settings->TargetActor.Get() : InContext->GetTypedExecutionTarget<AActor>();
