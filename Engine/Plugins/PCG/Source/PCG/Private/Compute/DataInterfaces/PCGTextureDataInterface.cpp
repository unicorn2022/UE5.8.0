...
BEGIN_SHADER_PARAMETER_STRUCT(FPCGTextureDataInterfaceParameters,)
	...
	SHADER_PARAMETER_ARRAY(FMatrix44f, TextureTransforms, [PCGTextureDataInterfaceConstants::MAX_NUM_SRV_BINDINGS])
	SHADER_PARAMETER_ARRAY(FMatrix44f, TextureInverseTransforms, [PCGTextureDataInterfaceConstants::MAX_NUM_SRV_BINDINGS])
	SHADER_PARAMETER(uint32, NumTextureInfos)
	SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<FUintVector4>, TextureInfos) // todo_pcg: Could be simple uniform array?
...
void UPCGTextureDataInterface::GetHLSL(FString& OutHLSL, FString const& InDataInterfaceName) const
{
	TMap<FString, FStringFormatArg> TemplateArgs =
	{
		...
		{ TEXT("EnableMultipleTextureObjects"), bSingleData ? 0 : 1 },
	};
...
void UPCGTextureDataInterface::GetDefines(FComputeKernelDefinitionSet& OutDefinitionSet) const
{
	...
	OutDefinitionSet.Defines.Add(FComputeKernelDefinition(TEXT("PCG_ENABLE_MULTIPLE_TEXTURE_OBJECTS"), FString::FromInt(bSingleData ? 0 : 1)));
}
...
void UPCGTextureDataProvider::Initialize(const UComputeDataInterface* InDataInterface, UObject* InBinding, uint64 InInputMask, uint64 InOutputMask)
{
	...
	bSingleData = DataInterface->GetSingleData();
}
...
bool UPCGTextureDataProvider::PrepareForExecute_GameThread(UPCGDataBinding* InBinding)
{
	...
	// Validate that all binding infos have the same sampler - otherwise warn that only the first will be used.
	if (!BindingInfos.IsEmpty())
	{
		const EPCGTextureFilter Filter = BindingInfos[0].Filter;

		for (int BindingIndex = 1; BindingIndex < BindingInfos.Num(); ++BindingIndex)
		{
			if (BindingInfos[BindingIndex].Filter != Filter)
			{
				UE_LOGF(LogPCG, Warning, "Encountered multiple unique texture filters on texture data interface: '%ls'. Only the first will be used.", *GetOutputPinLabel().ToString());
			}
		}
	}
...
void UPCGTextureDataProvider::BuildInfosFromDataCollection(UPCGDataBinding* InBinding)
{
	...
	const int32 SRVCapacity = bSingleData ? 1 : PCGTextureDataInterfaceConstants::MAX_NUM_SRV_BINDINGS;
	...
	for (int I = 0; I < InputTaggedData.Num(); ++I)
	{
		...
		{
			UE_LOGF(LogPCG, Error, "Unsupported data type encountered by texture data interface: '%ls'", TaggedData.Data ? *TaggedData.Data->GetName() : TEXT("NULL"));
			continue;
		}
		...
		{
			if (BindingInfos.Num() < SRVCapacity)
			{
			...
			else
			{
				if (!bWarnedOverflow)
				{
					...
					UE_LOGF(LogPCG, Warning, "Texture data interface on pin '%ls' received %d textures but binding capacity is %d; remaining textures ignored.",
...
void UPCGTextureDataProvider::BuildInfosFromDataDescription(UPCGDataBinding* InBinding)
{
	...
	{
		FPCGTextureBindingInfo BindingInfo(DataDesc);
		int32 BindingIndex = BindingInfos.Find(BindingInfo);
		...
		if (BindingIndex == INDEX_NONE)
		{
			...
			else
			{
				...
				{
					bWarnedOverflow = true;
					UE_LOGF(LogPCG, Warning, "Texture data interface on pin '%ls' received %d texture UAVs but binding capacity is %u; remaining UAVs ignored.",
						*GetOutputPinLabel().ToString(),
						PinDesc->GetDataDescriptions().Num(),
...
FPCGTextureDataProviderProxy::FPCGTextureDataProviderProxy(TWeakObjectPtr<UPCGTextureDataProvider> InDataProvider)
{
	...
	bSingleData = InDataProvider->GetSingleData();
...
bool FPCGTextureDataProviderProxy::IsValid(FValidationData const& InValidationData) const
{
	...
	{
		if (!BindingInfo.IsValid(ReasonPtr))
		{
			UE_LOGF(LogPCG, Warning, "FPCGTextureDataProviderProxy invalid due to invalid texture binding.");
			if (ReasonPtr)
			{
				UE_LOGF(LogPCG, Warning, "FPCGTextureDataProviderProxy: %ls.", **ReasonPtr);
			}
			return false;
		}
	}
	...
	for (const FPCGTextureInfo& TextureInfo : TextureInfos)
	{
		...
		{
			UE_LOGF(LogPCG, Warning, "FPCGTextureDataProviderProxy invalid due to invalid binding indices.");
			return false;
		}
	}
...
void FPCGTextureDataProviderProxy::GatherDispatchData(FDispatchData const& InDispatchData)
{
	...
	for (int32 InvocationIndex = 0; InvocationIndex < ParameterArray.Num(); ++InvocationIndex)
	{
		...
		Parameters.Sampler = (!BindingInfos.IsEmpty() && BindingInfos[0].Filter == EPCGTextureFilter::Point)
		...
		for (int BindingIndex = 0; BindingIndex < PCGTextureDataInterfaceConstants::MAX_NUM_SRV_BINDINGS; ++BindingIndex)
		{
			...
			{
				const FTransform& Transform = BindingInfos[BindingIndex].Transform;
				Parameters.TextureTransforms[BindingIndex] = FMatrix44f(Transform.ToMatrixWithScale()).GetTransposed();
				Parameters.TextureInverseTransforms[BindingIndex] = !FMath::IsNearlyZero(Transform.GetDeterminant()) ? FMatrix44f(Transform.ToInverseMatrixWithScale()).GetTransposed() : FMatrix44f::Identity;
			}
			else
			{
				Parameters.TextureTransforms[BindingIndex] = FMatrix44f::Identity;
				Parameters.TextureInverseTransforms[BindingIndex] = FMatrix44f::Identity;
			}
		}
...
void FPCGTextureDataProviderProxy::CreateDefaultTextures(FRDGBuilder& GraphBuilder)
{
	...
	// Always fill every slot - ValidateShaderParameters checks the full C++ struct regardless of
	// which slots the HLSL actually uses (e.g. when PCG_ENABLE_MULTIPLE_TEXTURE_OBJECTS == 0).
	for (int BindingIndex = 0; BindingIndex < PCGTextureDataInterfaceConstants::MAX_NUM_SRV_BINDINGS; ++BindingIndex)
	{
		...
		else if (BindingIndex < PCGTextureDataInterfaceConstants::MAX_NUM_UAV_BINDINGS)
		{
			Texture = GraphBuilder.CreateTexture(
				FRDGTextureDesc::Create2D(
					...
					BindingInfos[BindingIndex].Format,
...
void FPCGTextureDataProviderProxy::ExportTextureUAVs(const TArray<TRefCountPtr<IPooledRenderTarget>>& ExportedTextures)
{
	...
	ExecuteOnGameThread(UE_SOURCE_LOCATION, [ExportedTextures=ExportedTextures, DataProviderWeakPtr=DataProviderWeakPtr_GT, PinDesc=PinDesc, OutputPinLabel=OutputPinLabel, ExportMode=ExportMode, OutputPinLabelAlias=OutputPinLabelAlias, GenerationCount=OriginatingGenerationCount]()
	{
		...
		if (!DataProvider)
		{
			UE_LOGF(LogPCG, Error, "Could not resolve UPCGTextureDataProvider object to pass back buffer handle.");
			return;
		}
		...
		if (UPCGDataBinding* Binding = DataProvider->GetDataBinding())
		{
			...
			for (int TextureDataIndex = 0; TextureDataIndex < TextureInfos_GT.Num(); ++TextureDataIndex)
			{
				...
				ExportedData->Initialize(ExportedTextures[TextureDataIndex], TextureInfo.SliceIndex, TextureTransform);

				// @todo_pcg: Binding is doing a lot of work. Could store a context handle in the data provider instead?
				Binding->ReceiveDataFromGPU_GameThread(ExportedData, DataProvider->GetProducerSettings(), ExportMode, OutputPinLabel, OutputPinLabelAlias, PinDesc->GetDataDescriptions()[TextureDataIndex]);
			}
		}
...
void FPCGTextureDataProviderProxy::PackTextureInfos(FRDGBuilder& GraphBuilder)
{
	...
	//   - uint32 - ResolutionX
	//   - uint32 - ResolutionY
	//   - float  - TexelSizeWorldX (cm)
	//   - float  - TexelSizeWorldY (cm)
	// - TextureInfo1
	// - TextureInfo2
	// - ...
	...
	if (!TextureInfos.IsEmpty())
	{
		...
		for (int TextureInfoIndex = 0; TextureInfoIndex < TextureInfos.Num(); ++TextureInfoIndex)
		{
			...
			const float TexelWorldSizeX = BindingInfo.Size.X != 0 ? (TextureWorldScale.X * 2.0) / BindingInfo.Size.X : 0.0f;
			const float TexelWorldSizeY = BindingInfo.Size.Y != 0 ? (TextureWorldScale.Y * 2.0) / BindingInfo.Size.Y : 0.0f;

			TextureInfosPackedData[TextureInfoIndex * 2 + 0] = FUintVector4(TextureInfo.BindingIndex, TextureInfo.SliceIndex, static_cast<uint32>(BindingInfo.Dimension), BindingInfo.NumMips);
			TextureInfosPackedData[TextureInfoIndex * 2 + 1] = FUintVector4(BindingInfo.Size.X, BindingInfo.Size.Y, FMath::AsUInt(TexelWorldSizeX), FMath::AsUInt(TexelWorldSizeY));
		}
