...
bool UPCGMetaDataPartitionDataProvider::PerformPreExecuteReadbacks_GameThread(UPCGDataBinding* InBinding)
{
	...
	if (AnalysisDataIndices.IsEmpty())
	{
		...
		if (AnalysisDataIndices.IsEmpty())
		{
			UE_LOGF(LogPCG, Verbose, "UPCGMetaDataPartitionDataProvider: No analysis data found from count kernel.");
			return true;
		}
...
bool UPCGMetaDataPartitionDataProvider::PrepareForExecute_GameThread(UPCGDataBinding* InBinding)
{
	...
	if (!InputDesc)
	{
		UE_LOGF(LogPCG, Error, "UPCGMetaDataPartitionDataProvider: Could not retrieve input data description.");
		return true;
	}

	if (AnalysisDataIndices.Num() != InputDesc->GetDataDescriptions().Num())
	{
		UE_LOGF(LogPCG, Error, "UPCGMetaDataPartitionDataProvider: Number of received analysis data items (%d) does not match input data count (%d).",
	...
	for (int InputDataIndex = 0; InputDataIndex < AnalysisDataIndices.Num(); ++InputDataIndex)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogPCG, Error, "Analysis results attributes missing from read back data.");
			}
...
bool FPCGMetaDataPartitionProviderProxy::IsValid(FValidationData const& InValidationData) const
{
	for (const TArray<int32>& UniqueStringKeyValues : UniqueStringKeyValuesPerInputData)
	{
		...
		{
			UE_LOGF(LogPCG, Warning, "Metadata Partition will not execute, proxy invalid due to missing UniqueStringKeyValues.");
			return false;
		}
	}
	...
	if (NumPartitions <= 0)
	{
		UE_LOGF(LogPCG, Warning, "Metadata Partition will not execute, no partitions to generate.");
		return false;
	}

	if (PartitionAttributeId < 0)
	{
		UE_LOGF(LogPCG, Warning, "Metadata Partition will not execute, partition attribute ID not set.");
		return false;
	}
