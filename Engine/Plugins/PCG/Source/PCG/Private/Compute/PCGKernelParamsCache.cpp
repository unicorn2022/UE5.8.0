...
namespace PCGKernelParamsHelpers
{
	void OverrideKernelParamValue(FPCGContext* InContext, const UPCGSettings* InSettings, FInstancedPropertyBag& InOutKernelParams, const UStruct* KernelParamsStructType, FName InParamName, const FProperty* OverrideProperty, void* OverrideValuePtr, const UPCGData* InData)
	{
		...
		if (const FNumericProperty* NumericProperty = CastField<FNumericProperty>(OverrideProperty))
		{
			if (NumericProperty->IsFloatingPoint())
			{
				...
				case PCG::Private::MetadataTypes<float>::Id:
					Value = static_cast<const FPCGMetadataAttribute<float>*>(AttributeBase)->GetValueFromItemKey(PCGFirstEntryKey);
					break;
				case PCG::Private::MetadataTypes<double>::Id:
					Value = static_cast<const FPCGMetadataAttribute<double>*>(AttributeBase)->GetValueFromItemKey(PCGFirstEntryKey);
					break;
				default:
		...
		else if (const FEnumProperty* EnumPropety = CastField<FEnumProperty>(OverrideProperty))
		{
			const FNumericProperty* UnderlyingNumericProperty = EnumPropety->GetUnderlyingProperty();

			if (!ensure(UnderlyingNumericProperty))
			{
				PCG_KERNEL_VALIDATION_ERR(
					InContext,
					InSettings,
					FText::Format(LOCTEXT("MissingUnderlyingNumericProperty", "Tried to override pin '{0}', but enum property '{1}' does not have a valid underlying numeric property."),
						FText::FromName(InParamName),
						FText::FromString(OverrideProperty->GetClass()->GetName())));
				return;
			}
			...
			UnderlyingNumericProperty->SetIntPropertyValue(OverrideValuePtr, Value);
		}
		...
		else if (const FStructProperty* StructProperty = CastField<FStructProperty>(OverrideProperty))
		{
...
#define PCG_OVERRIDE_STRUCT_TYPE(Type) \
			if (Struct == TBaseStructure<Type>::Get()) \
			{ \
				if (AttributeBase->GetTypeId() == PCG::Private::MetadataTypes<Type>::Id) \
				{ \
					*static_cast<Type*>(OverrideValuePtr) = static_cast<const FPCGMetadataAttribute<Type>*>(AttributeBase)->GetValueFromItemKey(PCGFirstEntryKey); \
					bHandled = true; \
				} \
				else \
				{ \
			...
			PCG_OVERRIDE_STRUCT_TYPE(FVector2D)
			else PCG_OVERRIDE_STRUCT_TYPE(FVector)
			else PCG_OVERRIDE_STRUCT_TYPE(FVector4)
			else PCG_OVERRIDE_STRUCT_TYPE(FQuat)
			else PCG_OVERRIDE_STRUCT_TYPE(FRotator)
			else PCG_OVERRIDE_STRUCT_TYPE(FTransform)

#undef PCG_OVERRIDE_STRUCT_TYPE
			...
			if (!bHandled)
			{
				PCG_KERNEL_VALIDATION_ERR(
					InContext,
					InSettings,
					FText::Format(LOCTEXT("UnsupportedStructType", "Tried to override pin '{0}', but struct type '{1}' is not supported."),
						FText::FromName(InParamName),
...
{
	// Use FNumericProperty so int8/16/32/64 (and unsigned variants) all work.
	const FNumericProperty* Property = nullptr;
	const void* ValuePtr = nullptr;

	if (const FPropertyAndAddress* PropertyAndAddress = OverriddenProperties.Find(InParamName))
	{
		Property = CastField<FNumericProperty>(PropertyAndAddress->Key);
		ValuePtr = PropertyAndAddress->Value;
	}
	else if (const UPCGSettings* Settings = WeakSettings.Get())
	{
		Property = CastField<FNumericProperty>(Settings->GetClass()->FindPropertyByName(InParamName));
		ValuePtr = Property ? Property->ContainerPtrToValuePtr<void>(Settings) : nullptr;
	}

	return ensure(Property && ValuePtr && Property->IsInteger()) ? static_cast<int>(Property->GetSignedIntPropertyValue(ValuePtr)) : 0;
}
...
float FPCGKernelParams::GetValueFloat(FName InParamName) const
{
	return static_cast<float>(GetValueDouble(InParamName));
}

double FPCGKernelParams::GetValueDouble(FName InParamName) const
{
	// Use FNumericProperty so both FFloatProperty and FDoubleProperty work.
	const FNumericProperty* Property = nullptr;
	const void* ValuePtr = nullptr;
	...
	return ensure(Property && ValuePtr && Property->IsFloatingPoint()) ? Property->GetFloatingPointPropertyValue(ValuePtr) : 0.0;
}

FVector2D FPCGKernelParams::GetValueVector2D(FName InParamName) const
{
	return GetStructValue<FVector2D>(InParamName);
}

FVector FPCGKernelParams::GetValueVector(FName InParamName) const
{
	return GetStructValue<FVector>(InParamName);
}

FVector4 FPCGKernelParams::GetValueVector4(FName InParamName) const
{
	return GetStructValue<FVector4>(InParamName);
}

FQuat FPCGKernelParams::GetValueQuat(FName InParamName) const
{
	return GetStructValue<FQuat>(InParamName);
}

FRotator FPCGKernelParams::GetValueRotator(FName InParamName) const
{
	return GetStructValue<FRotator>(InParamName);
}

FTransform FPCGKernelParams::GetValueTransform(FName InParamName) const
{
	return GetStructValue<FTransform>(InParamName);
}

uint32 FPCGKernelParams::GetValueEnumAsUint(FName InParamName) const
{
	...
	return static_cast<uint32>(ensure(NumericProperty && ValuePtr) ? NumericProperty->GetUnsignedIntPropertyValue(ValuePtr) : 0);
}
...
{
	KernelParams.Empty();
	OverridableParamDataIndices.Empty();
}
