...
void UPCGProxyForGPUData::ReleaseTransientResources(const TCHAR* InReason)
{
	...
	UE_LOGF(LogPCG, Warning, "Releasing GPU data for '%ls' due to %ls", *GetName(), InReason ? InReason : TEXT("NOREASON"));
#endif
...
FIntVector4 UPCGProxyForGPUData::GetNumElements() const
{
	if (TSharedPtr<const FPCGProxyForGPUDataCollection> DataCollectionGPU = GetGPUInfo())
	{
		if (TSharedPtr<const FPCGDataCollectionDesc> Desc = DataCollectionGPU->GetDescription())
		{
			if (Desc->GetDataDescriptions().IsValidIndex(DataIndexInCollection))
			{
				return Desc->GetDataDescriptions()[DataIndexInCollection].GetElementCount();
			}
		}
	}
...
UPCGProxyForGPUData::FReadbackResult UPCGProxyForGPUData::GetCPUData(FPCGContext* InContext) const
{
	...
	if (!DataOnGPU)
	{
		UE_LOGF(LogPCG, Error, "Data collection lost! Enabling the define PCG_DATA_USAGE_LOGGING may help to identify when resource was released.");
...
{
	PCG::TScopeLock Lock(ReadbackLock);
