...
/** Helper struct for serializing per-pin created attribute key lists. */
USTRUCT()
struct FPCGKernelAttributeKeyList
{
    GENERATED_BODY()
...
public:
	...
	void SetOutputPinRequiresZeroInitialization(FName InOutputPinLabel) { PinsRequiringZeroInitialization.AddUnique(InOutputPinLabel); }
	virtual bool DoesOutputPinRequireZeroInitialization(FName InOutputPinLabel) const override { return PinsRequiringZeroInitialization.Contains(InOutputPinLabel); }
...
protected:
	...
	bool AreAttributesValid(const UPCGDataBinding* InDataBinding, FPCGContext* InContext, FText* OutErrorText) const;
...
protected:
	...
	/** Per-output-pin created attribute keys, combining manually authored CreatedKernelAttributeKeys and
	 *  keys inferred from HLSL Set/AtomicAdd calls. Populated during editor init, serialized for runtime use. */
	UPROPERTY()
	TMap<FName, FPCGKernelAttributeKeyList> PinCreatedAttributeKeys;
	...
	UPROPERTY()
	TMap<FName, /*EPCGPointNativeProperties*/int32> PinToPropertiesWithSetters;

	/** Properties written exclusively with constant literal expressions per pin. These are stripped from any inherited input allocation so the output uses a single-slot constant value range (CVR) instead of a
	 *  per-element buffer. Disjoint with PinToPropertiesWithSetters: a non-constant setter always wins. */
	UPROPERTY()
	TMap<FName, /*EPCGPointNativeProperties*/int32> PinToPropertiesWithSingleConstantSetters;

	/** Per-output-pin fine-grained TRS setter tracking for smart init. Bit 0 = SetPosition was called, Bit 1 = SetRotation was called, Bit 2 = SetScale was called. SetPointTransform sets all three bits.
	 *  Separate from PinToPropertiesWithSetters which uses the combined Transform bit; allows smart-init to handle e.g. rotation/scale independently when only position is written. */
	UPROPERTY()
	TMap<FName, int32> PinWrittenTransformSetters;

	/** Pins for which the output buffer should be statically initialized to 0 before execution. */
	UPROPERTY()
	TArray<FName> PinsRequiringZeroInitialization;
};
