...
bool FPCGMoveResourceParams::Accepts(const UObject* InObject) const
{
	if (!InObject) // Never accept nullptrs
	{
		return false;
	}
	else if (RequiredTags.IsEmpty() && ExcludedTags.IsEmpty()) // Always accept if there are no requirements
	{
		return true;
	}
	else if (const AActor* Actor = Cast<AActor>(InObject))
	{
		auto TestActorTags = [Actor](FName Tag) { return Actor->Tags.Contains(Tag); };

		return (RequiredTags.IsEmpty() || Algo::AllOf(RequiredTags, TestActorTags)) &&
			(ExcludedTags.IsEmpty() || Algo::NoneOf(ExcludedTags, TestActorTags));
	}
	else if (const UActorComponent* Component = Cast<UActorComponent>(InObject))
	{
		auto TestComponentTags = [Component](FName Tag) { return Component->ComponentHasTag(Tag); };

		return (RequiredTags.IsEmpty() || Algo::AllOf(RequiredTags, TestComponentTags)) &&
			(ExcludedTags.IsEmpty() || Algo::NoneOf(ExcludedTags, TestComponentTags));
	}
	else
	{
		// Unknown object type, assume we accept it.
		return true;
	}
}

bool FPCGMoveResourceParams::Accepts(const FPCGTaggedData& InData) const
{
	if (!InData.Data)
	{
		return false;
	}
	else if (RequiredTags.IsEmpty() && ExcludedTags.IsEmpty())
	{
		return true;
	}
	else
	{
		auto TestDataTags = [&InData](FName Tag) { return InData.Tags.Contains(Tag.ToString()); };

		return (RequiredTags.IsEmpty() || Algo::AllOf(RequiredTags, TestDataTags)) &&
			(ExcludedTags.IsEmpty() || Algo::NoneOf(ExcludedTags, TestDataTags));
	}
}
...
void UPCGManagedResource::MarkAsUsed()
{
	ensure(CanBeUsed()); 
...
bool UPCGManagedResource::MoveResourceToNewActor(AActor* NewActor, const AActor* ExpectedPreviousOwner)
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return MoveResourceToNewActor(NewActor);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}

bool UPCGManagedResource::MoveResource(const FPCGMoveResourceParams& Params)
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return MoveResourceToNewActor(Params.GetTarget<AActor>(), Params.GetExpectedPreviousOwner<AActor>());
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}

FPCGManagedActorLoadingScope::FPCGManagedActorLoadingScope()
{
#if WITH_EDITOR
...
FPCGManagedActorLoadingScope::FPCGManagedActorLoadingScope(bool bInShouldLoadActors)
{
#if WITH_EDITOR
...
void FPCGManagedActorLoadingScope::AddResources(IPCGGraphExecutionSource* InExecutionSource, const TArray<UPCGManagedResource*>& InManagedResources)
{
#if WITH_EDITOR
	...
	if (!InExecutionSource || !InExecutionSource->GetExecutionState().GetWorld() || InExecutionSource->GetExecutionState().GetWorld()->IsGameWorld())
	{
		return;
	}

	// @todo_pcg: check that this is valid way of getting WP here
	UWorldPartition* WorldPartition = FWorldPartitionHelpers::GetWorldPartition(InExecutionSource->GetExecutionState().GetWorld());
	if (!WorldPartition)
	{
		return;
	}
...
bool UPCGManagedActors::MoveResourceToNewActor(AActor* NewActor)
{
	...
	return MoveResource(Params);
}

bool UPCGManagedActors::MoveResource(const FPCGMoveResourceParams& Params)
{
	AActor* TargetActor = Params.GetTarget<AActor>();
	check(TargetActor);
	bool bMovedResource = false;

	for(int ActorIndex = GeneratedActorsArray.Num() - 1; ActorIndex >=0; --ActorIndex)
	{
		TSoftObjectPtr<AActor> Actor = GeneratedActorsArray[ActorIndex];

		if (!Actor.IsValid() || !Params.Accepts(Actor.Get()))
		{
			continue;
		}
		...
		if (bWasAttached)
		{
			...
			Actor->AttachToActor(TargetActor, FAttachmentTransformRules::KeepWorldTransform);
		}

		GeneratedActorsArray.RemoveAtSwap(ActorIndex);
	}

	return bMovedResource;
}
...
void UPCGManagedComponent::HideComponent()
{
	if (GeneratedComponent.IsValid())
	{
		...
		// If transient on load, hide for PIE
		if (bMarkedTransientOnLoad)
		{
			if (USceneComponent* SceneComponent = Cast<USceneComponent>(GeneratedComponent.Get()))
			{
...
void UPCGManagedComponentList::HideComponent(int32 ComponentIndex)
{
	if (GeneratedComponents[ComponentIndex].IsValid())
	{
		...
		if (bMarkedTransientOnLoad)
		{
			if (USceneComponent* SceneComponent = Cast<USceneComponent>(GeneratedComponents[ComponentIndex].Get()))
			{
				SceneComponent->bHiddenInGame = true;
			}
		}
...
bool UPCGManagedComponentBase::MoveResource(const FPCGMoveResourceParams& Params)
{
	AActor* TargetActor = Params.GetTarget<AActor>();
	check(TargetActor);
	...
	for (int32 ComponentIndex = GeneratedComponents.Num() - 1; ComponentIndex >= 0; --ComponentIndex)
	{
		...
		if (!GeneratedComponent.IsValid() || !Params.Accepts(GeneratedComponent.Get()))
		{
			continue;
		}
		...
		// Prevent moving of components on external (or spawned) actors, but still transfer resource
		if (!Params.ExpectedPreviousOwner || OldOwner == Params.ExpectedPreviousOwner)
		{
			...
			GeneratedComponent->Rename(nullptr, TargetActor);
			...
			if (TObjectPtr<USceneComponent> GeneratedSceneComponent = Cast<USceneComponent>(GeneratedComponent.Get()))
			{
				...
				bDetached = true;
				bAttached = GeneratedSceneComponent->AttachToComponent(TargetActor->GetRootComponent(), FAttachmentTransformRules::KeepWorldTransform);
			}
			...
			if (!bAttached)
			{
				...
				TargetActor->AddInstanceComponent(GeneratedComponent.Get());
			}
...
void UPCGManagedISMComponent::MarkAsUsed()
{
	...
	DataCrc = FPCGCrc();
	DataCrcReuseCount = INDEX_NONE;
	CrcReuseCount = INDEX_NONE;
...
void UPCGManagedSplineMeshComponent::PostLoad()
{
	Super::PostLoad();

	// Cache raw ptr
	CacheAndGetComponent();
}

void UPCGManagedSplineMeshComponent::ForgetComponent()
{
	Super::ForgetComponent();
	CachedGeneratedComponent.Reset();
}

USplineMeshComponent* UPCGManagedSplineMeshComponent::GetComponent() const
{
	// Read first, if it is cached, return it.
	{
		PCG::TSharedScopeLock ReadLock(CachedLock);
		if (USplineMeshComponent* GeneratedComponentPtr = CachedGeneratedComponent.Get())
		{
			return GeneratedComponentPtr;
		}
	}
	
	// Otherwise, lock on write, check again and cache it if needed.
	return CacheAndGetComponent();
}

void UPCGManagedSplineMeshComponent::SetComponent(USplineMeshComponent* InComponent)
{
	GeneratedComponent = InComponent;
	CachedGeneratedComponent = InComponent;
}

USplineMeshComponent* UPCGManagedSplineMeshComponent::CacheAndGetComponent() const
{
	PCG::TUniqueScopeLock WriteLock(CachedLock);
	USplineMeshComponent* GeneratedComponentPtr = CachedGeneratedComponent.Get();
	if (!GeneratedComponentPtr)
	{
		GeneratedComponentPtr = Cast<USplineMeshComponent>(GeneratedComponent.Get());
		CachedGeneratedComponent = GeneratedComponentPtr;
	}
