...
void UPCGData::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
...
void UPCGData::MarkUsage(EPCGDataUsage InUsage) const
{
	...
	UE_LOGF(LogPCG, Warning, "%ls: Marked usage flag %ls, flags %d -> %d",
		*GetName(),
...
void UPCGData::ClearUsage(EPCGDataUsage InUsage) const
{
#ifdef PCG_DATA_USAGE_LOGGING
	const UEnum* PCGDataUsageEnum = StaticEnum<EPCGDataUsage>();
	UE_LOGF(LogPCG, Warning, "%ls: Unmarked usage flag %ls, flags %d -> %d",
		*GetName(),
		*PCGDataUsageEnum->GetNameStringByValue(static_cast<__underlying_type(EPCGDataUsage)>(InUsage)),
		Usage.load(),
		Usage.load() & ~static_cast<__underlying_type(EPCGDataUsage)>(InUsage));
	...
	{
#ifdef PCG_DATA_USAGE_LOGGING
		UE_LOGF(LogPCG, Warning, "%ls: !! ReleaseTransientResources after usage unmarked !!", *GetName());
#endif
	...
	if (Node)
	{
		for (const UPCGPin* InputPin : Node->GetInputPins())
		{
			...
			for (const FPCGTaggedData& TaggedDatum : GetParamsByPin(InputPin->Properties.Label))
			{
				...
				{
					UE_LOGF(LogPCG, Warning, "[%ls] Found an Attribute Set data on an input pin that should not accept attributes. Make sure to re-wire it to the Overrides pin if it is used for overrides.", *Node->GetNodeTitle(EPCGNodeTitleType::ListView).ToString());
