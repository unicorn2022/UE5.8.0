...
namespace PCGGraphCompilerGPU
{
	...
	static void ValidateNodeKernels(const UPCGSettings* InNodeSettings, const TArray<UPCGComputeKernel*>& InNodeKernels, const TArray<FPCGKernelEdge>& InNodeKernelEdges)
	{
		UE_CLOGF(InNodeKernels.IsEmpty(), LogPCG, Error, "PCG GPU compiler: Settings '%ls' did not emit any kernels, check implementation of UPCGSettings::CreateKernels().", *InNodeSettings->GetName());
		UE_CLOGF(InNodeKernels.Contains(nullptr), LogPCG, Error, "PCG GPU compiler: Settings '%ls' emitted one or more null kernels, check implementation of UPCGSettings::CreateKernels().", *InNodeSettings->GetName());

		auto ValidateKernelPin = [InNodeSettings](const FPCGPinReference& Pin, bool bIsInputPin)
		{
			if (Pin.Kernel)
			{
				...
				if (bIsInputPin)
				{
					...
					Pin.Kernel->GetInputPins(Pins);
					bPinFound = Pins.ContainsByPredicate([InNodeSettings, &Pin](const FPCGPinProperties& Other) { return Other.Label == Pin.Label; });
				}
				else
				{
					TArray<FPCGPinPropertiesGPU> Pins;
					Pin.Kernel->GetOutputPins(Pins);
					bPinFound = Pins.ContainsByPredicate([InNodeSettings, &Pin](const FPCGPinPropertiesGPU& Other) { return Other.Label == Pin.Label; });
				}

				if (!bPinFound)
				{
					UE_LOGF(LogPCG, Error, "PCG GPU compiler: Settings '%ls' emitted a kernel edge connecting to %s pin '%ls' of kernel '%ls' which does not exist, check the kernel is returning this pin from Get%sPins().",
						*InNodeSettings->GetName(),
						bIsInputPin ? "input" : "output",
						*Pin.Label.ToString(),
						*Pin.Kernel->GetName(),
						bIsInputPin ? "Input" : "Output");
				}
			...
			{
				TArray<FPCGPinProperties> Pins = bIsInputPin ? InNodeSettings->InputPinProperties() : InNodeSettings->OutputPinProperties();

				if (!Pins.ContainsByPredicate([InNodeSettings, &Pin](const FPCGPinProperties& Other) { return Other.Label == Pin.Label; }))
				{
					UE_LOGF(LogPCG, Error, "PCG GPU compiler: Settings '%ls' emitted a kernel edge connecting to external %s pin '%ls' of the node which does not exist, check the node really has this pin.",
						*InNodeSettings->GetName(),
						bIsInputPin ? "input" : "output",
						*Pin.Label.ToString());
				}
		...
		for (const FPCGKernelEdge& Edge : InNodeKernelEdges)
		{
			ValidateKernelPin(Edge.UpstreamPin, /*bIsInputPin=*/!Edge.GetUpstreamKernel()); // Either wired to an output pin of an upstream kernel, or a node input pin.
			ValidateKernelPin(Edge.DownstreamPin, /*bIsInputPin=*/!!Edge.GetDownstreamKernel()); // Either wired to an input pin of a downstream kernel, or a node output pin.
		}
	}
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphCompilerGPU::ExpandGPUNodeKernelsToTasks);
	...
	for (FPCGTaskId TaskId : InOutGPUCompatibleTaskIds)
	{
		...
		Settings->CreateKernels(InOutContext, GetTransientPackage(), InOutCompiledTasks[TaskId].Node, NodeKernels, NodeKernelEdges);

		PCGGraphCompilerGPU::ValidateNodeKernels(Settings, NodeKernels, NodeKernelEdges);

		NodeKernels.Remove(nullptr);
		...
		while (!NodeKernels.IsEmpty())
		{
			...
			for (const FPCGPinProperties& KernelInputProps : KernelInputs)
			{
				...
				else
				{
					...
					if (OriginalTaskInput)
					{
						...
						// todo_pcg: Possibly the graph compiler should resolve dynamic pins during compilation.
			...
			// Preserve dependency-only inputs (e.g. execution dependency pins) from the original task. These are PCG control-flow inputs
			// (bProvideData=false) that have no kernel equivalent but are required for dynamic culling to work correctly at execution time.
			for (const FPCGGraphTaskInput& OriginalInput : InOutCompiledTasks[TaskId].Inputs)
			{
				if (!OriginalInput.bProvideData && OriginalInput.DownstreamPin.IsSet())
				{
					NewKernelTask.Inputs.Add(OriginalInput);
				}
			}
	...
	FOriginalToVirtualPin& OutOriginalToVirtualPin)
{
	...
	for (FPCGTaskId GPUTaskId : InCollapsedTasks)
	{
		...
		if (const UPCGComputeKernel* const* FoundKernel = InTaskIdToKernel.Find(GPUTaskId); ensure(FoundKernel && *FoundKernel))
		{
			TArray<FPCGPinPropertiesGPU> OutputPins;
			(*FoundKernel)->GetOutputPins(OutputPins);
	...
	const FOriginalToVirtualPin& InOriginalToVirtualPin)
{
	...
	for (FPCGTaskId GPUTaskId : InCollapsedTasks)
	{
		...
		for (FPCGTaskId Successor : InTaskSuccessors[GPUTaskId])
		{
			...
			// Build a new inputs array that retains the existing kernel-task entry (so CullTasks can still find and remove it later) and injects the
			// corresponding compute-graph-task edge immediately after it. Injecting immediately after each match preserves the relative position of
			// every entry, so the final order after CullTasks matches edge-addition order even when multiple separate GPU islands all feed the same downstream CPU node.
			TArray<FPCGGraphTaskInput> NewInputs;
			for (FPCGGraphTaskInput& ExistingInput : DownstreamCPUNode.Inputs)
			{
				NewInputs.Add(MoveTemp(ExistingInput));

				// Move on if this input is not associated with the current task.
				if (ExistingInput.TaskId != GPUTaskId)
				{
					continue;
				}

				// Inject new compute-graph-task edge immediately after the original kernel-task edge. Start by duplicating the input.
				FPCGGraphTaskInput NewInput = NewInputs.Last();
				NewInput.TaskId = InGPUGraphTaskId;

				if (NewInput.UpstreamPin.IsSet())
				{
					// Get the compute graph virtual pin corresponding to this pin, if one was created.
					const FName* VirtualPinLabel = InOriginalToVirtualPin.Find({ GPUTaskId, NewInput.UpstreamPin->Label, /*Pin is input*/false });

					if (ensure(VirtualPinLabel)) // Ensure should never fire - compute graph outputs are currently created for all kernel output pins.
					{
						NewInput.UpstreamPin->Label = *VirtualPinLabel;
					}
				}

				NewInputs.Add(MoveTemp(NewInput));
			}

			DownstreamCPUNode.Inputs = MoveTemp(NewInputs);
		}
	}
}
	...
	for (FPCGTaskId TaskId : InCollapsedTasks)
	{
		if (const UPCGComputeKernel* Kernel = InTaskIdToKernel[TaskId]; ensure(Kernel))
		{
			...
			for (const FPCGKernelAttributeKey& Key : AttributeKeys)
			{
				...
				{
					UE_LOGFMT(LogPCG, Error, "FPCGGraphCompilerGPU: Static attribute table exceeded maximum size ({Maximum}), use the 'Dump Data Descriptions' setting on the GPU node(s) to list attributes that are present.",
						PCGDataCollectionPackingConstants::MAX_NUM_CUSTOM_ATTRS);
					break;
				}
			}
	...
	FPinToDataInterface& InOutPinToDataInterface)
{
	...
	for (FPCGTaskId TaskId : InCollapsedTasks)
	{
		...
		const UPCGComputeKernel* TaskKernel = nullptr;
		if (UPCGComputeKernel* const* FoundKernel = InTaskIdToKernel.Find(TaskId))
		{
			TaskKernel = *FoundKernel;
		}
		...
		if (TaskKernel)
		{
			...
			TArray<FPCGPinPropertiesGPU> OutputPins;
			TaskKernel->GetOutputPins(OutputPins);
		...
		for (const FPCGPinProperties& OutputPinProperties : OutputPinsProperties) 
		{
			...
			if (const TArray<FPCGTaskId>* Successors = InTaskSuccessors.Find(TaskId))
			{
				for (FPCGTaskId Successor : *Successors)
				{
					...
					{
						if (Input.TaskId == TaskId && Input.UpstreamPin && Input.UpstreamPin->Label == OutputPinProperties.Label && !InCollapsedTasks.Contains(Successor))
						{
		...
		for (const FPCGGraphTaskInput& Input : InCompiledTasks[TaskId].Inputs)
		{
			...
			if (TaskKernel && !TaskKernel->UploadInputPinDataToGPU(Input.DownstreamPin->Label))
			{
				UE_LOGF(LogPCG, Verbose, "Kernel '%ls' input pin '%ls' does not require data on GPU, skipped creating data interface.", *TaskKernel->GetName(), *Input.DownstreamPin->Label.ToString());
				continue;
			}
	...
	// Resolve bLegacyAllowSettersOnInputs on each UPCGDataCollectionDataInterface: a DI is legacy iff ANY of its consumer kernels is a UPCGCustomHLSLKernel
	// with bUseLegacyDataCollectionAPI=true. Only consumer-side legacy matters because the flag controls what GetSupportedInputs exposes - i.e. what functions
	// consumers see on their input pins.
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(ResolveDataInterfaceLegacy);

		auto IsKernelLegacy = [&InTaskIdToKernel](FPCGTaskId TaskId) -> bool
		{
			const UPCGComputeKernel* const* FoundKernel = InTaskIdToKernel.Find(TaskId);
			if (!FoundKernel || !*FoundKernel)
			{
				return false;
			}
			const UPCGCustomHLSLSettings* CustomHLSLSettings = Cast<UPCGCustomHLSLSettings>((*FoundKernel)->GetSettings());
			return CustomHLSLSettings && CustomHLSLSettings->GetUseLegacyDataCollectionAPI();
		};

		for (const TPair<TPair<FPCGTaskId, FName>, UPCGComputeDataInterface*>& PinAndDataInterface : InOutPinToDataInterface)
		{
			UPCGDataCollectionDataInterface* DataCollectionDI = Cast<UPCGDataCollectionDataInterface>(PinAndDataInterface.Value);
			if (!DataCollectionDI)
			{
				continue;
			}

			const FPCGTaskId ProducerTaskId = PinAndDataInterface.Key.Key;
			const FName ProducerPinLabel = PinAndDataInterface.Key.Value;

			bool bAnyConsumerLegacy = false;

			// Walk all consumers (any task with an input edge from this producer pin).
			for (FPCGTaskId TaskId = 0; !bAnyConsumerLegacy && TaskId < (FPCGTaskId)InCompiledTasks.Num(); ++TaskId)
			{
				for (const FPCGGraphTaskInput& TaskInput : InCompiledTasks[TaskId].Inputs)
				{
					if (TaskInput.TaskId == ProducerTaskId
						&& TaskInput.UpstreamPin.IsSet()
						&& TaskInput.UpstreamPin.GetValue().Label == ProducerPinLabel
						&& IsKernelLegacy(TaskId))
					{
						bAnyConsumerLegacy = true;
						break;
					}
				}
			}

			DataCollectionDI->bLegacyAllowSettersOnInputs = bAnyConsumerLegacy;
		}
	}
}
	...
	UPCGComputeGraph* InOutComputeGraph)
{
	...
	TMap<const UComputeDataInterface*, TArray<FShaderFunctionDefinition>> DataInterfaceToInputs;
	TMap<const UComputeDataInterface*, TArray<FShaderFunctionDefinition>> DataInterfaceToOutputs;
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(CacheDataInterfaceInputOutputs);
		for (const TPair<TPair</*Node task*/FPCGTaskId, /*Pin label*/FName>, UPCGComputeDataInterface*>& PinAndDataInterface : InOutputPinToDataInterface)
		{
			// FindOrAdd is safe because the data interfaces in InOutputPinToDataInterface will be unique by definition, because every output pin maps to its own DI.
			TArray<FShaderFunctionDefinition>& InputDefinitions = DataInterfaceToInputs.FindOrAdd(PinAndDataInterface.Value);
			InputDefinitions.Reserve(128);
			PinAndDataInterface.Value->GetSupportedInputs(InputDefinitions);

			TArray<FShaderFunctionDefinition>& OutputDefinitions = DataInterfaceToOutputs.FindOrAdd(PinAndDataInterface.Value);
			OutputDefinitions.Reserve(128);
			PinAndDataInterface.Value->GetSupportedOutputs(OutputDefinitions);
		}
	}
	...
	while (!ReadyTasks.IsEmpty())
	{
		...
		TArray<FPCGPinPropertiesGPU> KernelOutputPinProperties;
		KernelWithBindings.Kernel->GetOutputPins(KernelOutputPinProperties);
		...
		{
			auto GetDataLabels = [Kernel=KernelWithBindings.Kernel, InOutComputeGraph](const auto& PinProperties)
			{
			...
			KernelWithBindings.Kernel->GetInputPins(InputPinProperties);
			GetDataLabels(InputPinProperties);

			GetDataLabels(KernelOutputPinProperties);
		}
		...
		for (const FPCGGraphTaskInput& Input : InCompiledTasks[TaskId].Inputs)
		{
			...
			// Setup maps related to virtual pins.
			if (const FName* VirtualPin = InOriginalToVirtualPin.Find(FNodePin(TaskId, Input.DownstreamPin->Label, /*IsInput*/true)))
			{
				InOutComputeGraph->KernelPinToVirtualPinLabel.Add(FPCGKernelPin(KernelIndex, Input.DownstreamPin->Label, /*bInIsInput=*/true), *VirtualPin);
			...
			if (!FoundDI || !*FoundDI)
			{
				UE_LOGFMT(LogPCG, Verbose, "No data interface associated with pin '{Label}', expected if the pin opted out of creating one.", Input.UpstreamPin->Label);
				continue;
			}
		...
		for (const FPCGPinPropertiesGPU& OutputPinProperties : KernelOutputPinProperties)
		{
			if (OutputPinProperties.Label == NAME_None)
			{
				continue;
			...
			if (!FoundDI || !*FoundDI)
			{
				UE_LOGFMT(LogPCG, Verbose, "No data interface associated with pin '{Label}', expected if the pin opted out of creating one.", OutputPinProperties.Label);
				continue;
			}
		...
		// Additional DIs created by kernels.
		{
			...
			KernelWithBindings.Kernel->CreateKernelParamsDataInterface(InOutContext, InOutComputeGraph, AdditionalInputDIs);
		...
		auto SetupAllInputBindings = [&KernelWithBindings, InOutComputeGraph, &DataInterfaceToInputs](const FDataInterfaceInfo& InDataInterfaceInfo)
		{
			const UComputeDataInterface* DataInterface = InOutComputeGraph->DataInterfaces[InDataInterfaceInfo.Index];
			check(DataInterface);

			TArray<FShaderFunctionDefinition>* FunctionsFromCache = DataInterfaceToInputs.Find(DataInterface);
			TArray<FShaderFunctionDefinition> RetrievedFunctions; // Used as local storage if we need to retrieve the functions here.
			if (!FunctionsFromCache)
			{
				RetrievedFunctions.Reserve(128);
				DataInterface->GetSupportedInputs(RetrievedFunctions);
			}

			TArray<FShaderFunctionDefinition>& Functions = FunctionsFromCache ? *FunctionsFromCache : RetrievedFunctions;
			...
			if (!FunctionsFromCache)
			{
				// Augment the DI function cache with any additional data interfaces created for the current task.
				DataInterfaceToInputs.Add(DataInterface, MoveTemp(RetrievedFunctions));
			}
		};

		auto SetupAllOutputBindings = [&KernelWithBindings, InOutComputeGraph, &DataInterfaceToOutputs](const FDataInterfaceInfo& InDataInterfaceInfo)
		{
			const UComputeDataInterface* DataInterface = InOutComputeGraph->DataInterfaces[InDataInterfaceInfo.Index];
			check(DataInterface);

			TArray<FShaderFunctionDefinition>* FunctionsFromCache = DataInterfaceToOutputs.Find(DataInterface);
			TArray<FShaderFunctionDefinition> RetrievedFunctions; // Used as local storage if we need to retrieve the functions here.
			if (!FunctionsFromCache)
			{
				RetrievedFunctions.Reserve(128);
				DataInterface->GetSupportedOutputs(RetrievedFunctions);
			}
			...
			{
				// Augment the DI function cache with any additional data interfaces created for the current task.
				DataInterfaceToOutputs.Add(DataInterface, MoveTemp(RetrievedFunctions));
			}
		};
...

		{
			...
			for (const FPCGSettingsOverridableParam& OverridableParam : Settings->OverridableParams())
			{
				if (OverridableParam.bSupportsGPU)
				{
					...
					const FString ParamLabel = OverridableParam.Label.ToString();
					...
					if (!OverridableParam.bRequiresGPUReadback && Settings->IsPropertyOverriddenByPin(OverridableParam.PropertiesNames))
					{
						Source += FString::Format(TEXT(
							"{1} {0}_GetOverridableValue()\n"
							"{\n"
							"	return {0}_GetFirstAttributeAs{2}();\n"
							"}\n"
							"\n"), { ParamLabel, TypeInfo.HLSLType, TypeInfo.AccessorSuffix });
					}
					else
					{
						Source += FString::Format(TEXT(
			...
			{
				TArray<FShaderFunctionDefinition>& Functions = DataInterfaceToInputs.FindChecked(Binding.DataInterface);
			...
			{
				TArray<FShaderFunctionDefinition>& Functions = DataInterfaceToOutputs.FindChecked(Binding.DataInterface);
				check(Functions.IsValidIndex(Binding.DataInterfaceBindingIndex));
		...
		auto AddAllEdgesForKernel = [&KernelWithBindings, InOutComputeGraph, &OriginalToVirtualInputPin, &DataInterfaceToInputs, &DataInterfaceToOutputs](int32 InKernelIndex, bool bInEdgesAreInputs)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(AddAllEdgesForKernel);
			...
			InOutComputeGraph->GraphEdges.Reserve(InOutComputeGraph->GraphEdges.Num() + Bindings.Num());
			...
			for (int KernelBindingIndex = 0; KernelBindingIndex < Bindings.Num(); ++KernelBindingIndex)
			{
				...
				{
					TArray<FShaderFunctionDefinition>& DataInterfaceFunctions = bInEdgesAreInputs ? DataInterfaceToInputs.FindChecked(DataInterface) : DataInterfaceToOutputs.FindChecked(DataInterface);

					// Strip the "_PCG_SRV" / "_PCG_UAV" suffix from the function name so the user-facing wrap name is direction-agnostic (e.g. In_Load / Out_Load, regardless of which underlying body is bound).
					// The full suffixed name is still used by the shim to resolve the underlying body in the .ush.
					FString PublicFuncName = DataInterfaceFunctions[Edge.DataInterfaceBindingIndex].Name;
					PCGComputeHelpers::StripSRVUAVSuffix(PublicFuncName);

					Edge.BindingFunctionNameOverride = FString::Format(
						TEXT("{0}_{1}"),
						{ PCGComputeHelpers::SanitizePinLabelForHLSL(Binding.Prefix), PublicFuncName }
					);
				}
		...
		if (DownstreamEdge.bKernelInput && bFirstEdgeOfDI)
		{
			...
			for (int32 UpstreamEdgeIndex = 0; UpstreamEdgeIndex < InOutComputeGraph->GraphEdges.Num(); ++UpstreamEdgeIndex)
			{
				...
				if (bFirstEdgeOfUpstreamDI && UpstreamEdge.KernelIndex != DownstreamEdge.KernelIndex && UpstreamEdge.DataInterfaceIndex == DownstreamEdge.DataInterfaceIndex && !UpstreamEdge.bKernelInput)
				{
					...
					UE_LOGF(LogPCG, Warning, "Connecting edge %d %ls (KBD: %d) to edge %d %ls (KBD: %d)",
					...
					UE_LOGF(LogPCG, Warning, "\tUpstream is kernel %d, DI '%ls', edge index %d ('%ls')",
						UpstreamEdge.KernelIndex,
					...
					UE_LOGF(LogPCG, Warning, "\tDownstream is kernel %d, DI '%ls', edge index %d ('%ls')",
						DownstreamEdge.KernelIndex,
...
void FPCGGraphCompilerGPU::CreateGPUNodes(FPCGGraphCompiler& InOutCompiler, UPCGGraph* InGraph, uint32 InGridSize, TArray<FPCGGraphTask>& InOutCompiledTasks)
{
	...
	{
		SetupVirtualPins(NodeSubsetsToConvertToCFGraph[ComputeGraphIndex], InOutCompiledTasks, TaskSuccessors, TaskIdToKernel, OriginalToVirtualPin[ComputeGraphIndex]);
	}
	...
	{
		PCG::TUniqueScopeLock WriteLock(InOutCompiler.GetCache().GraphToTaskMapLock);
