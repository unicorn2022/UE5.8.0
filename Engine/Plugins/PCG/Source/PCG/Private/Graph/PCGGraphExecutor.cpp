...
{
	TAutoConsoleVariable<int32> CVarRuntimeMaxNumProcessingTasks(
		TEXT("pcg.Runtime.MaxProcessingTasks"),
		4096,
		TEXT("Runtime: Maximum number of concurrent tasks for PCG Async processing"));

	TAutoConsoleVariable<float> CVarRuntimeMaxPercentageOfThreadsToUse(
		TEXT("pcg.Runtime.MaxPercentageOfThreadsToUse"),
		0.9f,
		TEXT("Runtime: Maximum percentage of number of threads for all concurrent PCG processing/execution"));

	TAutoConsoleVariable<float> CVarRuntimeMaxPercentageOfExecutingThreads(
		TEXT("pcg.Runtime.MaxPercentageOfExecutingThreads"),
		0.3f,
		TEXT("Runtime: Maximum percentage of number of threads for processing scheduled pcg elements (percentage of pcg.Runtime.MaxPercentageOfThreadsToUse)"));

	TAutoConsoleVariable<int32> CVarRuntimeMaxExecutingThreads(
		TEXT("pcg.Runtime.MaxExecutingThreads"),
		8,
		TEXT("Runtime: Hard cap on number of threads for processing scheduled pcg elements calculated from pcg.Runtime.MaxPercentageOfExecutingThreads"));

	// Disabled by default because on lower end platforms it is a performance choice (adding a dedicated thread considering lower number of cores might cause less scheduling contention which this consumer thread tries to alleviate)
	TAutoConsoleVariable<bool> CVarRuntimeEnablePostExecuteConsumerThread(
		TEXT("pcg.Runtime.EnablePostExecuteConsumerThread"),
		false,
		TEXT("Runtime: When enabled, a single thread will process all post executes and scheduling, instead of doing it directly in the async the executing threads"));

	TAutoConsoleVariable<float> CVarRuntimePostExecuteConsumerThreadTime(
		TEXT("pcg.Runtime.PostExecuteConsumerThreadTime"),
		5.0f,
		TEXT("Runtime: Allocated time the consumer thread will use before yielding (when pcg.Runtime.EnablePostExecuteConsumerThread is enabled)"));
...
#if WITH_EDITOR
	TAutoConsoleVariable<int32> CVarEditorMaxNumProcessingTasks(
		TEXT("pcg.Editor.MaxProcessingTasks"),
		4096,
		TEXT("Editor: Maximum number of concurrent tasks for PCG Async processing"));

	TAutoConsoleVariable<float> CVarEditorMaxPercentageOfThreadsToUse(
		TEXT("pcg.Editor.MaxPercentageOfThreadsToUse"),
		0.9f,
		TEXT("Editor: Maximum percentage of number of threads for all concurrent PCG processing/execution"));

	TAutoConsoleVariable<float> CVarEditorMaxPercentageOfExecutingThreads(
		TEXT("pcg.Editor.MaxPercentageOfExecutingThreads"),
		0.3f,
		TEXT("Editor: Maximum percentage of number of threads for processing scheduled pcg elements (percentage of pcg.Editor.MaxPercentageOfThreadsToUse)"));

	TAutoConsoleVariable<int32> CVarEditorMaxExecutingThreads(
		TEXT("pcg.Editor.MaxExecutingThreads"),
		8,
		TEXT("Editor: Hard cap on number of threads for processing scheduled pcg elements calculated from pcg.Editor.MaxPercentageOfExecutingThreads"));

	TAutoConsoleVariable<bool> CVarEditorEnablePostExecuteConsumerThread(
		TEXT("pcg.Editor.EnablePostExecuteConsumerThread"),
		false,
		TEXT("Editor: When enabled, a single thread will process all post executes and scheduling, instead of doing it directly in the async the executing threads"));

	TAutoConsoleVariable<float> CVarEditorPostExecuteConsumerThreadTime(
		TEXT("pcg.Editor.PostExecuteConsumerThreadTime"),
		15.0f,
		TEXT("Editor: Allocated time the consumer thread will use before yielding (when pcg.Editor.EnablePostExecuteConsumerThread is enabled)"));
	...
	TAutoConsoleVariable<int32> CVarMaxNumTasks_DEPRECATED(
		TEXT("pcg.MaxConcurrentTasks"),
		4096,
		TEXT("DEPRECATED(5.8): use pcg.Editor.MaxProcessingTasks or pcg.Runtime.MaxProcessingTasks instead."),
		FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable)
		{
			...
			UE_LOGF(LogPCG, Warning, "pcg.MaxConcurrentTasks is deprecated in 5.8. use pcg.Editor.MaxProcessingTasks or pcg.Runtime.MaxProcessingTasks instead.");
			check(InVariable);
			int32 NewValue = 0;
			InVariable->GetValue(NewValue);

			CVarRuntimeMaxNumProcessingTasks->Set(NewValue, ECVF_SetByCode);
#if WITH_EDITOR
			CVarEditorMaxNumProcessingTasks->Set(NewValue, ECVF_SetByCode);
#endif
		}));

	TAutoConsoleVariable<float> CVarMaxPercentageOfThreadsToUse_DEPRECATED(
		TEXT("pcg.MaxPercentageOfThreadsToUse"),
		0.9f,
		TEXT("DEPRECATED(5.8) : use pcg.Editor.MaxPercentageOfThreadsToUse or pcg.Runtime.MaxPercentageOfThreadsToUse instead."),
		FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable)
		{
			// Called when user sets cvar from console, or when this cvar is set when launching standalone build. Pushes value to new cvars.
			UE_LOGF(LogPCG, Warning, "pcg.MaxPercentageOfThreadsToUse is deprecated in 5.8. use pcg.Editor.MaxPercentageOfThreadsToUse or pcg.Runtime.MaxPercentageOfThreadsToUse instead.");
			check(InVariable);
			float NewValue = 0.f;
			InVariable->GetValue(NewValue);

			CVarRuntimeMaxPercentageOfThreadsToUse->Set(NewValue, ECVF_SetByCode);
#if WITH_EDITOR
			CVarEditorMaxPercentageOfThreadsToUse->Set(NewValue, ECVF_SetByCode);
#endif
			}));

	TAutoConsoleVariable<float> CVarMaxPercentageOfExecutingThreads_DEPRECATED(
		TEXT("pcg.MaxPercentageOfExecutingThreads"),
		0.3f,
		TEXT("DEPRECATED(5.8) : use pcg.Editor.MaxPercentageOfExecutingThreads or pcg.Runtime.MaxPercentageOfExecutingThreads instead."),
		FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable)
		{
			// Called when user sets cvar from console, or when this cvar is set when launching standalone build. Pushes value to new cvars.
			UE_LOGF(LogPCG, Warning, "pcg.MaxPercentageOfExecutingThreads is deprecated in 5.8. use pcg.Editor.MaxPercentageOfExecutingThreads or pcg.Runtime.MaxPercentageOfExecutingThreads instead.");
			check(InVariable);
			float NewValue = 0.f;
			InVariable->GetValue(NewValue);

			CVarRuntimeMaxPercentageOfExecutingThreads->Set(NewValue, ECVF_SetByCode);
#if WITH_EDITOR
			CVarEditorMaxPercentageOfExecutingThreads->Set(NewValue, ECVF_SetByCode);
#endif
		}));
	...
	static void MigrateCVarsIfNeeded()
	{
		...
		{
			const uint32 MaxNumTasks_Flags = CVarMaxNumTasks_DEPRECATED->GetFlags();
			const uint32 MaxNumTasks_SetBy = (MaxNumTasks_Flags & ECVF_SetByMask);

			if (SetFromIni(MaxNumTasks_Flags, MaxNumTasks_SetBy))
			{
				const int32 Value = CVarMaxNumTasks_DEPRECATED->GetInt();

				// Copy with the same SetBy priority so behavior matches the user's source of truth.
				CVarRuntimeMaxNumProcessingTasks->Set(Value, (EConsoleVariableFlags)MaxNumTasks_SetBy);
				CVarEditorMaxNumProcessingTasks->Set(Value, (EConsoleVariableFlags)MaxNumTasks_SetBy);

				UE_LOGF(LogPCG, Warning, "Cvar 'pcg.MaxConcurrentTasks' was set from ini but is deprecated in 5.8. Migrated value to pcg.Runtime.MaxProcessingTasks=%d, pcg.Editor.MaxProcessingTasks=%d (SetBy=0x%x).", Value, Value, MaxNumTasks_SetBy);
			}
		}

		{
			const uint32 MaxPercentage_Flags = CVarMaxPercentageOfThreadsToUse_DEPRECATED->GetFlags();
			const uint32 MaxPercentage_SetBy = (MaxPercentage_Flags & ECVF_SetByMask);

			if (SetFromIni(MaxPercentage_Flags, MaxPercentage_SetBy))
			{
				const float Value = CVarMaxPercentageOfThreadsToUse_DEPRECATED->GetFloat();

				// Copy with the same SetBy priority so behavior matches the user's source of truth.
				CVarRuntimeMaxPercentageOfThreadsToUse->Set(Value, (EConsoleVariableFlags)MaxPercentage_SetBy);
				CVarEditorMaxPercentageOfThreadsToUse->Set(Value, (EConsoleVariableFlags)MaxPercentage_SetBy);

				UE_LOGF(LogPCG, Warning, "Cvar 'pcg.MaxPercentageOfThreadsToUse' was set from ini but is deprecated in 5.8. Migrated value to pcg.Runtime.MaxPercentageOfThreadsToUse=%f, pcg.Editor.MaxPercentageOfThreadsToUse=%f (SetBy=0x%x).", Value, Value, MaxPercentage_SetBy);
			}
		}

		{
			const uint32 MaxExecutePercentage_Flags = CVarMaxPercentageOfExecutingThreads_DEPRECATED->GetFlags();
			const uint32 MaxExecutePercentage_SetBy = (MaxExecutePercentage_Flags & ECVF_SetByMask);

			if (SetFromIni(MaxExecutePercentage_Flags, MaxExecutePercentage_SetBy))
			{
				const float Value = CVarMaxPercentageOfExecutingThreads_DEPRECATED->GetFloat();

				// Copy with the same SetBy priority so behavior matches the user's source of truth.
				CVarRuntimeMaxPercentageOfExecutingThreads->Set(Value, (EConsoleVariableFlags)MaxExecutePercentage_SetBy);
				CVarEditorMaxPercentageOfExecutingThreads->Set(Value, (EConsoleVariableFlags)MaxExecutePercentage_SetBy);

				UE_LOGF(LogPCG, Warning, "Cvar 'pcg.MaxPercentageOfExecutingThreads' was set from ini but is deprecated in 5.8. Migrated value to pcg.Runtime.MaxPercentageOfExecutingThreads=%f, pcg.Editor.MaxPercentageOfExecutingThreads=%f (SetBy=0x%x).", Value, Value, MaxExecutePercentage_SetBy);
			}
		}
	}
	...
	template<class T>
	T GetCVarValueForWorld(const UWorld* World, const TAutoConsoleVariable<T>& RuntimeValue, const TAutoConsoleVariable<T>& EditorValue)
	{
#if WITH_EDITOR
		if (!World || !World->IsGameWorld())
		{
			return EditorValue.GetValueOnAnyThread();
		}
#endif

		return RuntimeValue.GetValueOnAnyThread();
	}
}
...
FPCGGraphExecutor::~FPCGGraphExecutor()
{
	...
	// Shutdown thread
	PostTaskExecuteConsumerThread.Reset();
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphExecutor::ScheduleGraph);
	...
	{
		const IPCGGraphExecutionState& ExecutionState = InParams.ExecutionSource->GetExecutionState();

		if (ExecutionState.IsLocalSource() || ExecutionState.IsPartitioned())
		{
			GenerationGridSize = ExecutionState.GetGenerationGridSize();
		}
	}
	...
	{ 
		...
		PCG::TScopeLock ScopeLock(ScheduleLock);
		...
		else if (InParams.ExecutionSource->GetExecutionState().GetGraph() != InParams.Graph)
		{
			const UPCGGraphInterface* OverrideGraph = InParams.ExecutionSource->GetExecutionState().GetGraphInstance();
			if (!OverrideGraph)
			{
				OverrideGraph = InParams.ExecutionSource->GetExecutionState().GetGraph();
			}

			if (OverrideGraph)
			{
				PreGraphTask.Element = MakeShared<FPCGPreGraphElement>(OverrideGraph);
			}
		}

		if (!PreGraphTask.Element)
		{
			PreGraphTask.Element = GetPreGraphElement();
		}
...
TSet<IPCGGraphExecutionSource*> FPCGGraphExecutor::Cancel(TFunctionRef<bool(TWeakInterfacePtr<IPCGGraphExecutionSource>)> CancelFilter)
{
	...
	{
		// Visit ready tasks
		PCG::TScopeLock ScopeLock(LiveTasksLock);
	...
	while (!bStableCancellationSet)
	{
		...
		// Mark as cancelled in the active tasks - needed to make sure we're not breaking the current execution (if any)
		{
			...
			for (TTuple<TSharedPtr<FPCGGraphActiveTask>, UE::Tasks::TTask<bool>>& CancelledActiveTask : CancelledActiveTasks)
			{
				...
				// Even if we've set SchedulingDisabled = true, it is possible some tasks slipped into the queue and will get PostTaskExecute called through consumer thread
				// if this is the case the bIsExecutingTask would be true and we can wait for the consumer thread to process.
				while (PostTaskExecuteConsumerThread.IsValid() && ActiveTask->bIsExecutingTask)
				{
		...
		// Remove from paused tasks
		{
			...
			{
				PCG::TScopeLock ScopeLock(LiveTasksLock);
				PCG::TScopeLock PauseScopeLock(PausedTaskSuccessorsLock);
...
void FPCGGraphExecutor::MarkInputResults(TArrayView<const FPCGTaskId> InInputResults)
{
	...
	{
		PCG::TUniqueScopeLock WriteLock(TaskOutputsLock);
...
bool FPCGGraphExecutor::GetOutputData(FPCGTaskId TaskId, FPCGDataCollection& OutData)
{
	...
	PCG::TSharedScopeLock ReadLock(TaskOutputsLock);
...
void FPCGGraphExecutor::CheckState(bool bIsInGameThread)
{
	...
	PCG::TScopeLock ScheduleScopeLock(ScheduleLock, /*bTryLock=*/ ElapsedTimeSinceLastCheck < 3.0);
	...
	PCG::TScopeLock ScopeLock(TasksLock);
	PCG::TScopeLock ChildScopeLock(LiveTasksLock);
	PCG::TScopeLock PausedLock(PausedTaskSuccessorsLock);
	PCG::TScopeLock CachingResultsScopeLock(CachingResultsLock);
	...
	{
		UE_LOGF(LogPCG, Error, "PCG Graph executor error: tasks are in a deadlocked state. Will drop all tasks.");
		ClearAllTasks();
	}
}
...
bool FPCGGraphExecutor::ProcessScheduledTasks(double EndTime)
{
	...
	FCachedResults CachedResults;

	{
		// Process any newly scheduled graphs to execute
		PCG::TScopeLock ScopeLock(ScheduleLock, !bIsInGameThread);
		if (ScopeLock.IsLocked())
		{
			if (ScheduledTasks.Num() > 0)
			{
				...
				while (ScheduledTaskIndex >= 0)
				{
					...
					{
						PCG::TScopeLock ChildScopeLock(TasksLock);
						PCG::TSharedScopeLock ReadLock(TaskOutputsLock);
					...
					// Process ready tasks
					if (NewReadyTasks.Num() > 0)
					{
						OnTaskInputsReady(NewReadyTasks, CachedResults, bIsInGameThread);
						NewReadyTaskCount -= NewReadyTasks.Num();
					}
...
void FPCGGraphExecutor::ExecuteTasksEnded()
{
	...
	if (GetNonScheduledRemainingTaskCount() == 0)
	{
		...
		{
			{
				PCG::TScopeLock ScopeChildLock(TasksLock);
...
void FPCGGraphExecutor::PostTaskExecute(TSharedPtr<FPCGGraphActiveTask> ActiveTaskPtr, bool bIsDone)
{
	...
	if (!bIsDone || !ActiveTask.Context->DynamicDependencies.IsEmpty())
	{
		...
		if (!ActiveTaskPtr->bWasCancelled)
		{
			...
			if (ActiveTask.Context->bIsPaused)
			{
				...
				if (bHasPausedDependencies)
				{
					PCG::TScopeLock PauseLock(PausedTaskSuccessorsLock);
					PCG::TSharedScopeLock ReadLock(TaskOutputsLock);
	...
	if (bTaskWasBypassed || bTaskFullyExecuted)
	{
		...
		if (ActiveTaskSettingsInterface && ActiveTaskSettingsInterface->bDebug)
		{
			...
			{
				PCG::TScopeLock ScopeLock(CollectGCReferenceTasksLock);
		...
		// Outside of editor, only collect this information if profiling is enabled. In editor inspection data is always collected.
#if !WITH_EDITOR
		...
		const bool bNeedsManualClear = (ActiveTask.Element == GraphCompiler.GetSharedTrivialPostGraphElement());

		// Store output in data map.
		FTaskResult NewResult{ ActiveTask.NodeId, &ActiveTask.Context->OutputData, bNeedsManualClear };
		StoreResults(MakeArrayView(&NewResult, 1));

		// Book-keeping
		QueueNextTasks(MakeArrayView(&ActiveTask.NodeId, 1));
	}
...
void FPCGGraphExecutor::OnTaskInputsReady(TArrayView<FPCGGraphTask> InNewReadyTasks, FCachedResults& OutCachedResults, bool bIsInGameThread)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphExecutor::OnTaskInputsReady);
	FTaskIds ResultsToMarkAsRead;
	TArray<FPCGGraphTask, TInlineAllocator<16>> UncachedReadyTasks;

	for (FPCGGraphTask& Task : InNewReadyTasks)
	{
		// LocalResultsToMarkAsRead does not contain duplicates
		FTaskIds LocalResultsToMarkAsRead;
		if (SetupTask(Task, LocalResultsToMarkAsRead))
		{
			check(Task.Element);
			...
			{
				UncachedReadyTasks.Emplace(MoveTemp(Task));
			}
		}

		// ResultsToMarkAsRead can contain duplicates as we want to mark the same input as read multiple times if multiple tasks depend on it
		ResultsToMarkAsRead.Append(LocalResultsToMarkAsRead);
	}

	if (UncachedReadyTasks.Num() > 0)
	{
		PCG::TScopeLock ChildScopeLock(LiveTasksLock);
		ReadyTasks.Append(MoveTemp(UncachedReadyTasks));
	}

	if (ResultsToMarkAsRead.Num() > 0)
	{
		MarkInputResults(ResultsToMarkAsRead);
	}
}

bool FPCGGraphExecutor::SetupTask(FPCGGraphTask& Task, FTaskIds& ResultsToMarkAsRead)
{
...
void FPCGGraphExecutor::PrepareForExecute(FPCGGraphTask& Task, FCachedResult*& OutCachedResult, bool bLiveTasksLockAlreadyLocked)
{
	...
	FPCGGetFromCacheParams CacheParam;
	...
	// This section requires the lock on the caching results to prevent interaction between different tasks and the GC so we avoid locking if cache is disabled.
	if(GraphCache.IsEnabled() && (!ExecutionSource || ExecutionSource->GetExecutionState().IsCacheEnabled()))
	{
		// Build cache param outside of lock
		if (Task.Element->BuildCacheParams(&GraphCache, Task.Node, Task.TaskInput, ExecutionSource, CacheParam))
		{
			// Check that we have a valid cache param to avoid locking for nothing
			if (CacheParam.Crc.IsValid())
			{
				PCG::TScopeLock ScopeLock(CachingResultsLock);
				FCachedResult LocalCachedResult;
				CacheStatus = Task.Element->RetrieveResultsFromCache(&GraphCache, CacheParam, LocalCachedResult.Output);
	...
	if (!bNeedsToCreateActiveTask)
	{
		...
		if (PCGGraphExecutor::CVarDynamicTaskCulling.GetValueOnAnyThread() && TaskSettings && TaskSettings->OutputPinsCanBeDeactivated() && OutCachedResult->Output.InactiveOutputPinBitmask != 0)
		{
			...
			OutCachedResult->Stack = Task.GetStackHandle();
	...
	{
		Task.Context = Task.Element->Initialize(FPCGInitializeElementParams(&Task.TaskInput, ExecutionSource, Task.Node));
		...
		Task.Context->DependenciesCrc = CacheParam.Crc;
...
void FPCGGraphExecutor::DebugDisplayTask(TWeakPtr<FPCGGraphExecutor::FGameThreadHandler> WeakHandler, TSharedPtr<FPCGGraphActiveTask> ActiveTaskPtr)
{
	...
	PCG::TScopeLock ScopeLock(Handler->GetExecutor()->CollectGCReferenceTasksLock);
...
{
#if WITH_EDITOR
	return PCGGraphExecutor::GetCVarValueForWorld(World, PCGGraphExecutor::CVarTimePerFrame, PCGGraphExecutor::CVarEditorTimePerFrame) / 1000.0;
#else
	return PCGGraphExecutor::CVarTimePerFrame.GetValueOnAnyThread() / 1000.0;
#endif
}

double FPCGGraphExecutor::GetPostExecuteConsumerThreadBudgetInSeconds() const
{
#if WITH_EDITOR
	return PCGGraphExecutor::GetCVarValueForWorld(World, PCGGraphExecutor::CVarRuntimePostExecuteConsumerThreadTime, PCGGraphExecutor::CVarEditorPostExecuteConsumerThreadTime) / 1000.0;
#else
	return PCGGraphExecutor::CVarRuntimePostExecuteConsumerThreadTime.GetValueOnAnyThread() / 1000.0;
#endif
}

int32 FPCGGraphExecutor::GetMaxNumProcessingTasks() const
{
#if WITH_EDITOR
	return PCGGraphExecutor::GetCVarValueForWorld(World, PCGGraphExecutor::CVarRuntimeMaxNumProcessingTasks, PCGGraphExecutor::CVarEditorMaxNumProcessingTasks);
#else
	return PCGGraphExecutor::CVarRuntimeMaxNumProcessingTasks.GetValueOnAnyThread();
#endif
}

float FPCGGraphExecutor::GetMaxPercentageOfThreadsToUse() const
{
#if WITH_EDITOR
	return PCGGraphExecutor::GetCVarValueForWorld(World, PCGGraphExecutor::CVarRuntimeMaxPercentageOfThreadsToUse, PCGGraphExecutor::CVarEditorMaxPercentageOfThreadsToUse);
#else
	return PCGGraphExecutor::CVarRuntimeMaxPercentageOfThreadsToUse.GetValueOnAnyThread();
#endif
}

float FPCGGraphExecutor::GetMaxPercentageOfExecutingThreads() const
{
#if WITH_EDITOR
	return PCGGraphExecutor::GetCVarValueForWorld(World, PCGGraphExecutor::CVarRuntimeMaxPercentageOfExecutingThreads, PCGGraphExecutor::CVarEditorMaxPercentageOfExecutingThreads);
#else
	return PCGGraphExecutor::CVarRuntimeMaxPercentageOfExecutingThreads.GetValueOnAnyThread();
#endif
}

int32 FPCGGraphExecutor::GetMaxExecutingThreads() const
{
#if WITH_EDITOR
	return PCGGraphExecutor::GetCVarValueForWorld(World, PCGGraphExecutor::CVarRuntimeMaxExecutingThreads, PCGGraphExecutor::CVarEditorMaxExecutingThreads);
#else
	return PCGGraphExecutor::CVarRuntimeMaxExecutingThreads.GetValueOnAnyThread();
#endif
}

bool FPCGGraphExecutor::IsPostExecuteConsumerThreadEnabled() const
{
...
#if WITH_EDITOR
	return PCGGraphExecutor::GetCVarValueForWorld(World, PCGGraphExecutor::CVarRuntimeEnablePostExecuteConsumerThread, PCGGraphExecutor::CVarEditorEnablePostExecuteConsumerThread);
#else
	return PCGGraphExecutor::CVarRuntimeEnablePostExecuteConsumerThread.GetValueOnAnyThread();
#endif
}

void FPCGGraphExecutor::ProcessAsyncExecuteResult(TSharedPtr<FPCGGraphActiveTask> Task, bool bIsDone, bool bIsPaused, double EndTime)
{
	PostTaskExecute(Task, bIsDone);
...
uint32 FPCGGraphExecutor::FPostTaskExecuteConsumerThread::Run()
{
	check(Owner);
	while (Event->Wait() && !bQuit)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphExecutor::FPostTaskExecuteConsumerThread::Run);
		const double EndTime = FPlatformTime::Seconds() + Owner->GetPostExecuteConsumerThreadBudgetInSeconds();

		FPostTaskExecuteItem Item;
		while (Queue.Dequeue(Item))
		{
			Owner->ProcessAsyncExecuteResult(Item.Task, Item.bIsDone, Item.bIsPaused, EndTime);

			// Yield everytime we've exhausted our budget
			if (FPlatformTime::Seconds() > EndTime)
			{
				FPlatformProcess::YieldThread();
			}
...
FPCGGraphExecutor::FPostTaskExecuteConsumerThread::FPostTaskExecuteConsumerThread(FPCGGraphExecutor* InOwner)
	: Owner(InOwner)
{
	Thread = FRunnableThread::Create(this, TEXT("FPostTaskExecuteConsumerThread"));
}

FPCGGraphExecutor::FPostTaskExecuteConsumerThread::~FPostTaskExecuteConsumerThread()
{
	bQuit = true;
	Event->Trigger();
	Thread->Kill(true);
...
void FPCGGraphExecutor::StartPostExecuteConsumerThread()
{
	if (PostTaskExecuteConsumerThread.IsValid() || !IsPostExecuteConsumerThreadEnabled())
	{
		return;
	}

	PostTaskExecuteConsumerThread = MakeUnique<FPostTaskExecuteConsumerThread>(this);
}
...
bool FPCGGraphExecutor::ExecuteScheduling(double EndTime, TSharedPtr<FPCGGraphActiveTask>* OutMainThreadTask, bool bForceCheckPausedTasks)
{
	...
	const float MaxPercentageOfThreadsToUse = FMath::Clamp(GetMaxPercentageOfThreadsToUse(), 0.0f, 1.0f);

	// Clamp to 0.8 to never allow starving of PCGAsync Tasks
	const float MaxPercentageOfExecutingThreads = FMath::Clamp(GetMaxPercentageOfExecutingThreads(), 0.0f, 0.8f);
	...
	const int32 MaxExecutingThreadsPriorHardCap = PCGGraphExecutor::CVarGraphMultithreading.GetValueOnAnyThread() ? FMath::Max(0, (int32)(MaxNumThreads * MaxPercentageOfExecutingThreads)) : 0;

	// Final number of threads to use for PCG Element execution (outside of main thread tick)
	const int32 MaxExecutingThreads = FMath::Min(MaxExecutingThreadsPriorHardCap, FMath::Max(0, GetMaxExecutingThreads()));

	// Number of threads to use for PCG Async operations 
	const int32 MaxPCGAsyncThreads = FMath::Min(MaxNumThreads - MaxExecutingThreads, GetMaxNumProcessingTasks());
	...
	FCachedResults CachedResults;
	{
		...
		PCG::TScopeLock ScopeLock(LiveTasksLock, bTryLock);
		if (ScopeLock.IsLocked())
		{
	...
	// Dispatch Tasks
	{
		PCG::TScopeLock ScopeLock(LiveTasksLock, !bIsInGameThread);
		if (ScopeLock.IsLocked())
		{
				...
				for (int32 ExecutionIndex = 0; ExecutionIndex < ActiveTasks.Num(); ++ExecutionIndex)
				{
...
#endif
					{
						...
						ActiveTask->ExecutingTask = UE::Tasks::Launch(UE_SOURCE_LOCATION, [this, ActiveTask, SchedulingBudgetInSeconds]() -> bool
						{
							...
							// If Scheduling is disabled it means we are cancelling tasks so in that case avoid using the consumer thread so that the Wait calls in Cancel
							// can rely on the task having been fully executed (PostTaskExecute included)
							if (!SchedulingDisabled && PostTaskExecuteConsumerThread.IsValid())
							{
								PostTaskExecuteConsumerThread->Queue.Enqueue(ActiveTask, bIsDone, bIsPaused);
								PostTaskExecuteConsumerThread->Event->Trigger();
							}
							else
							{
								const double EndTime = FPlatformTime::Seconds() + SchedulingBudgetInSeconds;
								ProcessAsyncExecuteResult(ActiveTask, bIsDone, bIsPaused, EndTime);
							}
							return bIsDone;
						});
						bStateChanged = true;
...
void FPCGGraphExecutor::Execute(double& InOutEndTime)
{
	...
	StartPostExecuteConsumerThread();
...
void FPCGGraphExecutor::QueueNextTasksInternal(TConstArrayView<FPCGTaskId> FinishedTasks, FCachedResults& OutCachedResults)
{
	...
	TArray<FPCGGraphTask, TInlineAllocator<16>> NewReadyTasks;
	{
		PCG::TScopeLock ScopeLock(TasksLock);
		PCG::TSharedScopeLock ReadLock(TaskOutputsLock);
		for (FPCGTaskId FinishedTask : FinishedTasks)
		{
	...
	if (NewReadyTasks.Num() > 0)
	{
		OnTaskInputsReady(NewReadyTasks, OutCachedResults, bIsInGameThread);
		NewReadyTaskCount -= NewReadyTasks.Num();
	}
...
void FPCGGraphExecutor::QueueNextTasks(TConstArrayView<FPCGTaskId> FinishedTasks)
{
	FCachedResults CachedResults;
	QueueNextTasksInternal(FinishedTasks, CachedResults);
...
void FPCGGraphExecutor::ProcessCachedResults(TConstArrayView<FCachedResult*> CachedResults)
{
	...
	FTaskIds NextTasks;
	ProcessCachedResultsInternal(CachedResults, NextTasks);
	
	FCachedResults NewCachedResults;
	QueueNextTasksInternal(NextTasks, NewCachedResults);

	while (NewCachedResults.Num() > 0)
	{
		NextTasks.Reset();
		ProcessCachedResultsInternal(NewCachedResults, NextTasks);
		
		NewCachedResults.Reset();
		QueueNextTasksInternal(NextTasks, NewCachedResults);
	}
}

void FPCGGraphExecutor::ProcessCachedResultsInternal(TConstArrayView<FCachedResult*> CachedResults, FTaskIds& OutNextTasks)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphExecutor::ProcessCachedResults);
	
	TArray<FTaskResult, TInlineAllocator<16>> NewResults;
...
#if WITH_EDITOR
			SendInactivePinNotification(CachedResult->Node, CachedResult->Stack.GetStack(), CachedResult->Output.InactiveOutputPinBitmask);
#endif
		}

		
		NewResults.Add({ CachedResult->TaskId, &CachedResult->Output, CachedResult->bIsPostGraphTask });
		OutNextTasks.Add(CachedResult->TaskId);
	}
...
void FPCGGraphExecutor::BuildTaskInput(FPCGGraphTask& Task, FTaskIds& ResultsToMarkAsRead)
{
	...
	auto LogDiscardedData = [&Task](const TOptional<FPCGPinProperties>& DownstreamPin)
	{
		...
		UE_LOGF(LogPCG, Warning, "%ls", *Message);
#endif
	};
		...
		for (const FPCGGraphTaskInput& Input : Task.Inputs)
		{
			...
			if (Input.UpstreamPin.IsSet())
			{
				...
				if (!InputDataOnPin.IsEmpty())
				{
					...
					check(InputDataCrcsOnPin.IsEmpty() || InputDataOnPin.Num() == InputDataCrcsOnPin.Num());
					...
					Task.TaskInput.AddDataForPin(
						...
						MakeArrayView(InputDataCrcsOnPin.GetData(), FMath::Min(NumberDataItemsToTake, InputDataCrcsOnPin.Num())),
						InputPinLabelCrc);
...
void FPCGGraphExecutor::StoreResults(TConstArrayView<FTaskResult> InResults)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphExecutor::StoreResults);

	TArray<TPair<FPCGTaskId, FOutputDataInfo>, TInlineAllocator<16>> NewOutputs;

	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphExecutor::StoreResults::Successors);
		PCG::TScopeLock ScopeLock(TasksLock);
		for (const FTaskResult& Result : InResults)
		{
			FOutputDataInfo OutputDataInfo;
			check(Result.TaskOutput);
			OutputDataInfo.DataCollection = *Result.TaskOutput;
			OutputDataInfo.bNeedsManualClear = Result.bNeedsManualClear;
			OutputDataInfo.DataCollection.MarkUsage(EPCGDataUsage::GraphExecutorTaskOutput);

			if (TSet<FPCGTaskId>* Successors = TaskSuccessors.Find(Result.TaskId))
			{
				OutputDataInfo.RemainingSuccessorCount = Successors->Num();
			}

			NewOutputs.Add({ Result.TaskId, MoveTemp(OutputDataInfo) });
		}
	}
	...
	for (auto& Pair : NewOutputs)
	{
		TaskOutputs.Add(Pair.Key, MoveTemp(Pair.Value));
	}
}
...
{
	InOutPinIds.Reserve(InOutPinIds.Num() + FMath::CountBits64(InactiveOutputPinBitmask));
...
void FPCGGraphExecutor::CullInactiveDownstreamNodes(FPCGTaskId InCompletedTaskId, uint64 InInactiveOutputPinBitmask)
{
		...
		while (!PinIdsToDeactivate.IsEmpty())
		{
			...
			for (const FPCGTaskId RemovedTaskId : TasksToRemove)
			{
				// Scope in which RemovedTask reference is valid.
				{
					...
					// Dynamically culled grid linkage tasks must do special processing to prevent downstream tasks from never finishing execution
					if (RemovedTask.Element && RemovedTask.Element->IsGridLinkage())
					{
						static_cast<PCGGraphExecutor::FPCGGridLinkageElement*>(RemovedTask.Element.Get())->OnUpstreamLinkCulled(RemovedTask.ExecutionSource.Get());
					}
	...
	if (AllRemovedTasks.Num() > 0)
	{
		...
		QueueNextTasks(AllRemovedTasks.Array());
	}
}
...
{
	PCG::TScopeLock ScopeLock(ReleasedContextLock);
...
{
	PCG::TScopeLock Lock(DefaultElementsLock);
...
void FPCGGraphExecutor::LogTaskStateNoLock() const
{
	...
	if (PCGGraphExecutionLogging::CullingLogEnabled())
	{
		UE_LOGF(LogPCG, Log, "\tDORMANT (FPCGGraphExecutor::Tasks):");
...
FPCGTaskId FPCGGraphExecutor::ScheduleDebugWithTaskCallback(UPCGComponent* InComponent, TFunction<void(FPCGTaskId/* TaskId*/, const UPCGNode*/* Node*/, const FPCGDataCollection&/* TaskOutput*/)> TaskCompleteCallback)
{
	...
	for (const FPCGGraphTask& CompiledTask : CompiledTasks)
	{
		...
		{
			// If the task was dynamically culled it produced no output — skip the callback.
			{
				PCG::TSharedScopeLock ReadLock(TaskOutputsLock);
				const FOutputDataInfo* OutputDataInfo = TaskOutputs.Find(CompiledTask.NodeId);
				if (OutputDataInfo && OutputDataInfo->bCulled)
				{
					return true;
				}
			}
...
bool FPCGPreGraphElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	{
		PCG_EXECUTION_CACHE_VALIDATION_CREATE_SCOPE(ExecutionSource);
		ExecutionSource->GetExecutionState().ExecutePreGraph(Context);
	}
	
	// Also emplace in the output a copy of the user parameters. Check in priority: Graph override, graph instance on the execution source and graph on the execution source.
	const UPCGGraphInterface* Graph = GraphOverride.Get();

	if (!Graph)
	{
		Graph = ExecutionSource->GetExecutionState().GetGraphInstance();
	}
	
	if (!Graph)
	{
		Graph = ExecutionSource->GetExecutionState().GetGraph();
	}
	
	if (const UPCGUserParametersData* UserParametersData = Graph ? Graph->ConstructUserParametersData(Context) : nullptr)
	{
		FPCGTaggedData& TaggedData = Context->OutputData.TaggedData.Emplace_GetRef();
		TaggedData.Data = UserParametersData;
		TaggedData.Tags.Add(PCG::Private::UserParameterTagData);
...
FPCGPreGraphElement::FPCGPreGraphElement(const UPCGGraphInterface* InGraphOverride)
	: GraphOverride(InGraphOverride)
{
}
...
namespace PCGGraphExecutor
{
	...
	bool FPCGGridLinkageElement::ExecuteInternal(FPCGContext* InContext) const
	{
		check(InContext);
		FPCGGridLinkageContext* Context = static_cast<FPCGGridLinkageContext*>(InContext);

		return ExecuteGridLinkage(
			GenerationGrid,
			FromGrid,
			ToGrid,
			ResourceKey,
		...
		FPCGGridLinkageContext* InContext)
	{
		...
		if (!ExecutionState.GetGraph()->IsHierarchicalGenerationEnabled()
			|| !ensure(PCGHiGenGrid::IsValidGridSize(InFromGrid) || InFromGrid == PCGHiGenGrid::UnboundedGridSize()))
		{
		...
		if (InFromGrid < InToGrid)
		{
			...
			if (IPCGEditorModule* PCGEditorModule = IPCGEditorModule::Get())
			{
				...
				if (InToGrid == PCGHiGenGrid::UnboundedGridSize())
				{
		...
		if (InFromGrid == InGenerationGrid && InFromGrid != InToGrid)
		{
			...
			PCGGraphExecutionLogging::LogGridLinkageTaskExecuteStore(InContext, InGenerationGrid, InFromGrid, InToGrid, InResourceKey, Data.TaggedData.Num());
			
			ExecutionState.StoreOutputDataForPin(InResourceKey, Data);
		}
		else if (InToGrid == InGenerationGrid && InFromGrid != InToGrid)
		{
			PCGGraphExecutionLogging::LogGridLinkageTaskExecuteRetrieve(InContext, InGenerationGrid, InFromGrid, InToGrid, InResourceKey);

			IPCGGraphExecutionSource* ExecSourceWithData = nullptr;

			if (InFromGrid == PCGHiGenGrid::UnboundedGridSize())
			{
			...
			else
			{
				...
				const FPCGGridDescriptor GridDescriptor = OriginalSource->GetExecutionState().GetGridDescriptor(InFromGrid);

				// Get grid coords using the parent grid (InFromGrid).
			...
			// If we need data from a higher execution source but it's still generating, then we'll wait for it.
			if (ExecSourceWithData->GetExecutionState().IsGenerating())
			{
			...
			if (!InContext->bScheduledGraph)
			{
				...
				const FPCGTaskId GraphTaskId = ExecSourceWithData->GetExecutionState().Generate(Params);
	...
	void FPCGGridLinkageElement::OnUpstreamLinkCulled(IPCGGraphExecutionSource* InExecutionSource) const
	{
		...
		const IPCGGraphExecutionState& ExecutionState = InExecutionSource->GetExecutionState();

		// Non-hierarchical generation - no linkage required - data should just pass through.
		if (!ExecutionState.GetGraph()->IsHierarchicalGenerationEnabled()
			|| !ensure(PCGHiGenGrid::IsValidGridSize(FromGrid) || FromGrid == PCGHiGenGrid::UnboundedGridSize()))
		{
			return;
		}

		if (FromGrid == GenerationGrid && FromGrid != ToGrid)
		{
			FPCGDataCollection EmptyData;
			PCGGraphExecutionLogging::LogGridLinkageTaskStoreNullDataOnCulled(InExecutionSource, GenerationGrid, FromGrid, ToGrid, ResourceKey);

			ExecutionState.StoreOutputDataForPin(ResourceKey, EmptyData);
		}
	}
}
...
FPCGElementPtr UPCGGridLinkageSettings::CreateElement() const
{
	...
	return MakeShared<PCGGraphExecutor::FPCGGridLinkageElement>(FromGrid, ToGrid, GenerationGrid, ResourceKey, ResolvedUpstreamPin);
}
