...
UPCGComputeGraph* FPCGGraphCompilerCache::GetCompiledComputeGraph(const UPCGGraph* InGraph, uint32 GridSize, uint32 ComputeGraphIndex)
{
#if WITH_EDITOR
	{
		PCG::TSharedScopeLock ReadLock(GraphToTaskMapLock);
	...
	const UPCGGraphCompilationData* CookedData = InGraph ? InGraph->GetCookedCompilationData().Get() : nullptr;
...
{
	UE_LOGF(LogPCG, Verbose, "FPCGGraphCompilerCache::RemoveFromCache '%ls'", *InGraph->GetName());
...
{
	{
		PCG::TUniqueScopeLock WriteLock(GraphToTaskMapLock);
	...
	{
		PCG::TScopeLock Lock(GraphDependenciesLock);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphCompiler::CompileGraph);
	if (!InGraph)
	{
	...
	while (NodeQueue.Num() > 0)
	{
		...
		if(Subgraph && !bIsDynamic && bIsNotDisabled && !bIsRecursiveGraph)
		{
			...
			for (const UPCGPin* InputPin : Node->InputPins)
			{
				...
				for (const UPCGEdge* InboundEdge : InputPin->Edges)
				{
					...
					{
						UE_LOGFMT(LogPCG, Warning, "Invalid inbound edge on subgraph");
					}
				}
			}
		...
		else
		{
			...
			for (const UPCGPin* InputPin : Node->InputPins)
			{
				...
				for (const UPCGEdge* InboundEdge : InputPin->Edges)
				{
					if (!InboundEdge->IsValid())
					{
						UE_LOGFMT(LogPCG, Warning, "Unbound inbound edge");
						continue;
					}
					...
					{
						UE_LOGFMT(LogPCG, Error, "Inconsistent node linkage on node '{Node}'", FSoftObjectPath(Node));
						return TArray<FPCGGraphTask>();
					}
				}
			}
		...
		for (const UPCGPin* OutPin : Node->OutputPins)
		{
			for (const UPCGEdge* OutboundEdge : OutPin->Edges)
			{
				...
				{
					UE_LOGFMT(LogPCG, Warning, "Unbound outbound edge");
					continue;
				}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphCompiler::Compile);

	bool bAlreadyCached = false;
	{
		PCG::TSharedScopeLock ReadLock(Cache.GraphToTaskMapLock);
	...
	{
		PCG::TUniqueScopeLock WriteLock(Cache.GraphToTaskMapLock);
	...
	bool bIsCooking)
{
	...
	{
		const uint32 GraphGenerationGrid = InOutTaskGenerationGrid[TaskId];
		...
		for (int32 InputIndex = 0; InputIndex < InOutCompiledTasks[TaskId].Inputs.Num(); ++InputIndex)
		{
			...
			const uint32 InputGraphGenerationGrid = InOutTaskGenerationGrid[TaskInput.TaskId];
			...
			if (InputGraphGenerationGrid != PCGHiGenGrid::UninitializedGridSize() && InputGraphGenerationGrid > GraphGenerationGrid)
			{
				...
				const uint32 FromGrid = InOutTaskGenerationGrid[TaskInput.TaskId];
				const uint32 ToGrid = InOutTaskGenerationGrid[TaskId];
...
#endif
				{
					LinkTask.Element = MakeShared<PCGGraphExecutor::FPCGGridLinkageElement>(FromGrid, ToGrid, InGenerationGrid, ResourceKey, UpstreamPin);
				}
				...
				// The link executes at both FROM grid size (store) and TO grid size (retrieve). Just store from grid size here - the task culling lambda checks for
				// grind links and checks both grid sizes.
				InOutTaskGenerationGrid.Add(FromGrid);
			}
		}
	}
}

uint32 FPCGGraphCompiler::CalculateGridRecursive(
	FPCGTaskId InTaskId,
	uint32 GenerationDefaultGrid,
	const FPCGStackContext& InStackContext,
	...
	TArray<uint32>& InOutTaskGenerationGrid)
{
	if (InOutTaskGenerationGrid[InTaskId] != PCGHiGenGrid::UninitializedGridSize())
	{
	...
	{
		GridSize = FMath::Min(GenerationDefaultGrid, GridSizeSettings->GetScaledGridSize());
	}
	else
	{
		if (InCompiledTasks[InTaskId].Inputs.IsEmpty())
		{
			if (bTopLevelGraph)
			{
				...
				GridSize = GenerationDefaultGrid;
			}
			else
			{
				...
				GridSize = CalculateGridRecursive(InCompiledTasks[InTaskId].ParentId, GenerationDefaultGrid, InStackContext, InCompiledTasks, InOutTaskGenerationGrid);
			}
		}
		else
		{
			...
			{
				const uint32 InputGridSize = CalculateGridRecursive(InputTask.TaskId, GenerationDefaultGrid, InStackContext, InCompiledTasks, InOutTaskGenerationGrid);
				if (PCGHiGenGrid::IsValidGridSize(InputGridSize))
				{
					GridSize = FMath::Min(InputGridSize, GridSize);
				}
			}
		}
	}

	ensure(GridSize != PCGHiGenGrid::UninitializedGridSize());

	InOutTaskGenerationGrid[InTaskId] = GridSize;
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphCompiler::CalculateDynamicActivePinDependencies);
	...
	{
		UE_LOGFMT(LogPCG, Warning, "Number of output pins ({OutputPinCount}) on node {NodeName} exceeded maximum limit ({Limit}) for dynamic node culling. "
	...
	for (const FPCGGraphTaskInput& Input : InOutCompiledTasks[InTaskId].Inputs)
	{
		...
		{
			// For ComputeGraph tasks (Node=nullptr), the node's settings are unavailable so we cannot call
			// IsInputPinRequiredByExecution. Fall back to the downstream pin's own PinStatus: pins explicitly
			// marked Required (e.g. exec dep pins with bExecutionDependencyRequired=true) are treated as
			// required by execution. Normal/Advanced pins fall through to the disjunction pass below.
			if (!Input.DownstreamPin.GetValue().IsRequiredPin())
			{
				continue;
			}
		}
	...
	else
	{
		...
		for (const FPCGGraphTaskInput& Input : InOutCompiledTasks[InTaskId].Inputs)
		{
			...
			if (Input.UpstreamPin.IsSet())
			{
				...
				if (!UpstreamNode)
				{
					// Upstream is a ComputeGraph task (Node=nullptr). We cannot resolve a pin index from its virtual output pins, so fall back
					// to a pin-less dependency. This ensures that if the ComputeGraph task is dynamically culled, the culling propagates to
					// this task as well (CullInactiveDownstreamNodes adds NodeIdToPinId for culled tasks).
				...
				else
				{
					...
					const int PinIndex = UpstreamNode->GetOutputPins().IndexOfByPredicate([&Input](const UPCGPin* InPin)
					{
						return InPin->Properties.Label == Input.UpstreamPin.GetValue().Label;
					});
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphCompiler::GetCompiledTasks);
	UE_LOG_CONTEXT("PCGGraph", FSoftObjectPath(InGraph));
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGGraphCompiler::CompileTopGraph);
	bool bAlreadyCached = false;
	...
	UE_LOGF(LogPCG, Verbose, "FPCGGraphCompiler::CompileTopGraph '%ls' grid: %u", *InGraph->GetName(), GenerationGridSize);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(TaskStaticCulling);

		// Remove reroute nodes before execution grid setup, as grid linkages need final nodes to connect from/to.
		// Note that reroutes nodes won't be culled if they have no inbound edges or are marked for debug.
		auto RerouteCullTasks = [](const FPCGGraphTask& InTask) -> bool
		{
			if (const UPCGRerouteSettings* Settings = InTask.Node ? Cast<UPCGRerouteSettings>(InTask.Node->GetSettings()) : nullptr)
			{
				return InTask.Node->HasInboundEdges() && !Settings->bDebug;
			}
			else
			{
				return false;
		...
		CullTasks(CompiledTasks, /*bAddPassthroughWires=*/true, RerouteCullTasks);
		...
		{
			const uint32 DefaultGrid = InGraph->GetDefaultGridSize();
			...
			ResolveGridSizes(GenerationGridSize, CompiledTasks, StackContext, DefaultGrid, TaskGenerationGrid);

			// Create linkage tasks for edges that cross from large grid to small grid tasks.
			CreateGridLinkages(InGraph, GenerationGridSize, TaskGenerationGrid, CompiledTasks, StackContext, bIsCooking);

			// Cull any task that should not execute on the current grid.
			CullTasks(CompiledTasks, /*bAddPassthroughWires=*/false, [GenerationGridSize, &TaskGenerationGrid](const FPCGGraphTask& InTask)
			{
				const uint32 TaskGridSize = TaskGenerationGrid[InTask.NodeId];
				if (TaskGridSize == PCGHiGenGrid::UninitializedGridSize())
				{
					// Don't cull if no grid size set.
					return false;
				}

				if (InTask.Element && InTask.Element->IsGridLinkage()) // Non-cooking case
				{
					PCGGraphExecutor::FPCGGridLinkageElement* LinkElement = static_cast<PCGGraphExecutor::FPCGGridLinkageElement*>(InTask.Element.Get());
					return LinkElement->GetFromGridSize() != GenerationGridSize && LinkElement->GetToGridSize() != GenerationGridSize;
				}
				else if (const UPCGGridLinkageSettings* GridLinkSettings = Cast<UPCGGridLinkageSettings>(InTask.CookedSettings)) // Cooking case
				{
					return GridLinkSettings->FromGrid != GenerationGridSize && GridLinkSettings->ToGrid != GenerationGridSize;
				}
				else
				{
					return TaskGridSize != GenerationGridSize;
				}
			});
		}
	}
...
#endif
	{
		...
		{
			UE_LOGFMT(LogPCG, Warning, "One or more GPU nodes were culled from the graph!");
		}
	}
...
{
	{
		PCG::TSharedScopeLock ReadLock(SharedTrivialElementLock);
	...
	PCG::TUniqueScopeLock WriteLock(SharedTrivialElementLock);
...
{
	{
		PCG::TSharedScopeLock ReadLock(SharedGatherElementLock);
	...
	PCG::TUniqueScopeLock WriteLock(SharedGatherElementLock);
...
{
	{
		PCG::TSharedScopeLock ReadLock(SharedTrivialPostGraphElementLock);
	...
	PCG::TUniqueScopeLock WriteLock(SharedTrivialPostGraphElementLock);
