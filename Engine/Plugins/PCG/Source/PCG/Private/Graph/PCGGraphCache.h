...
struct FCachedMemoryRecord
{
	...
	/** Allows retrieval of data ptr for logging purposes. */
	FObjectKey Data;
};
...
{
	extern thread_local bool bIsGatheringCacheSize;
	extern TAutoConsoleVariable<bool> CVarEnableTLSGatherCacheSize;
}
...
public:
	...
	/** Returns whether a GetResourceSizeEx call is done for cache sizing (or pcg memory profiling) scope vs. the system-level resource gathering. */
	static bool IsGatheringCacheSize();

	/** Memory details required in the GetResourceSizeEx calls. Will always be 0 (e.g. full) when not gathering for the cache. */
	static int CacheSizeDetailLevel();
...
private:
	...
	// Store Get/Store stats
	struct FPCGCacheStats
	{
		int64 GetCount = 0;
		int64 StoreCount = 0;
		int64 HitCount = 0;
