...
void UPCGBlueprintElement::PointLoop(FPCGContext& InContext, const UPCGPointData* InData, UPCGPointData*& OutData, UPCGPointData* OptionalOutData, bool bGameThread) const
{
	if (!InData)
	{
	...
	if (bGameThread && !IsInGameThread())
	{
		ThrowRequiresGameThreadException(&InContext, TEXT("PointLoop"));
		return;
	}
	...
	TGuardValue<int32> NumTaskOverride(CurrentContext->AsyncState.NumAvailableTasks, bGameThread ? 1 : CurrentContext->AsyncState.NumAvailableTasks);
...
void UPCGBlueprintElement::VariableLoop(FPCGContext& InContext, const UPCGPointData* InData, UPCGPointData*& OutData, UPCGPointData* OptionalOutData, bool bGameThread) const
{
	if (!InData)
	{
	...
	if (bGameThread && !IsInGameThread())
	{
		ThrowRequiresGameThreadException(&InContext, TEXT("VariableLoop"));
		return;
	}
...
void UPCGBlueprintElement::NestedLoop(FPCGContext& InContext, const UPCGPointData* InOuterData, const UPCGPointData* InInnerData, UPCGPointData*& OutData, UPCGPointData* OptionalOutData, bool bGameThread) const
{
	...
	if (bGameThread && !IsInGameThread())
	{
		ThrowRequiresGameThreadException(&InContext, TEXT("NestedLoop"));
		return;
	}
...
void UPCGBlueprintElement::IterationLoop(FPCGContext& InContext, int64 NumIterations, UPCGPointData*& OutData, const UPCGSpatialData* InA, const UPCGSpatialData* InB, UPCGPointData* OptionalOutData, bool bGameThread) const
{
	if (NumIterations < 0)
	{
	...
	if (bGameThread && !IsInGameThread())
	{
		ThrowRequiresGameThreadException(&InContext, TEXT("IterationLoop"));
		return;
	}
