...
void APCGWorldActor::OnPartitionGridSizeChanged()
{
	...
	if (!bAllSafeToDelete)
	{
		UE_LOGF(LogPCG, Error, "Trying to change the partition grid size while there are partitioned PCGComponents that are refreshing. We cannot stop the refresh for now, so we abort there. You should delete your partition actors manually and regenerate when the refresh is done");
		return;
	}
	...
	for (IPCGGraphExecutionSource* ExecutionSource : PCGSubsystem->GetAllRegisteredPartitionedExecutionSources())
	{
