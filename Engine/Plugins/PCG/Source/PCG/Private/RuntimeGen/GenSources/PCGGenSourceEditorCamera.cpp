...
void UPCGGenSourceEditorCamera::Tick(const FPCGRuntimeGenContext& InContext)
{
#if WITH_EDITOR
	ViewFrustum = TOptional<FConvexVolume>();

	if (InContext.bAnySourcesUseFrustumCulling && EditorViewportClient)
	{
		...
		if (World && Viewport)
		{
			...
			if (FSceneView* SceneView = EditorViewportClient->CalcSceneView(&ViewFamily))
			{
				...
				GetViewFrustumBounds(ConvexVolume, SceneView->ViewMatrices.GetWorldToClip(), /*bUseNearPlane=*/true, /*bUseFarPlane=*/true);
