...
namespace PCGRuntimeGenChangeDetection
{
	...
	void FDetector::FlushCachedState(const IPCGGraphExecutionSource* InOptionalOriginalExecutionSource)
	{
		if (InOptionalOriginalExecutionSource)
		{
			TArray<FDetectionInputs, TInlineAllocator<32>> KeysToRemove;

			for (const TPair<FDetectionInputs, FCachedState>& Entry : StateCache)
			{
				const IPCGGraphExecutionSource* ExecutionSource = Entry.Key.GetOriginalExecutionSource();
				if (!ExecutionSource || ExecutionSource == InOptionalOriginalExecutionSource)
				{
