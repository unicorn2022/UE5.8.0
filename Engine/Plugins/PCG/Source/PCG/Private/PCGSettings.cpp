...
#if WITH_EDITOR
FPCGNodeThumbnailProxy FPCGNodeThumbnailProxy::FromAssetPath(const FSoftObjectPath& InAssetPath)
{
	return FPCGNodeThumbnailProxy{FTextureType{TInPlaceType<FSoftObjectPath>(), InAssetPath}};
}

FPCGNodeThumbnailProxy FPCGNodeThumbnailProxy::FromTexture(const UTexture* InTexture)
{
	return FPCGNodeThumbnailProxy{FTextureType{TInPlaceType<const UTexture*>(), InTexture}};
}
#endif // WITH_EDITOR

FString FPCGSettingsOverridableParam::GetPropertyPath() const
{
	return FString::JoinBy(PropertiesNames, PCGSettings::PropertyPathSeparator, [](const FName InName) { return InName.ToString(); });
}
...
bool UPCGSettingsInterface::SetDebugged(bool bInDebugged)
{
	if (bDebug == bInDebugged)
	{
		return false;
	}
	...
	if (UPCGSettings* Settings = GetSettings())
	{
		const EPCGChangeType ChangeType = Settings->GetChangeTypeForProperty(GET_MEMBER_NAME_CHECKED(UPCGSettingsInterface, bDebug));
		OnSettingsChangedDelegate.Broadcast(Settings, ChangeType);
	}
#endif
...
#if WITH_EDITOR
void UPCGSettingsInterface::SetTemporaryManualEditingEnabled(const bool bInEnabled)
{
	bTemporaryManualEditingEnabled = bInEnabled;
}

void UPCGSettingsInterface::SetMarkedForManualEditing(const bool bInMarked)
{
	bMarkedForManualEditing = bInMarked;
}
#endif // WITH_EDITOR

uint32 UPCGSettings::GetTypeNameHash() const
{
	return GetTypeHash(GetClass()->GetName());
}
...
FPCGAssetCachedPins UPCGSettings::BuildCachedPins() const
{
	FPCGAssetCachedPins Pins;
	Pins.bHasCachedPins = true;
	Pins.InputPins = InputPinProperties();
	Pins.OutputPins = OutputPinProperties();

	return Pins;
}

void UPCGSettings::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	Super::PreSave(ObjectSaveContext);

	// Update cached pins - note that we need to cache pins only for asset-level settings. Otherwise, make them always empty.
	CachedPins = IsAsset() ? BuildCachedPins() : FPCGAssetCachedPins();
}
#endif // WITH_EDITOR
...
void UPCGSettings::PostLoad()
{
	...
	if (FPCGModule::IsCPUOnlyExecution())
	{
		bExecuteOnGPU = false;
	}
#endif
...
#if WITH_EDITOR
	InitializeCachedOverridableParams();
	OnSettingsChangedDelegate.AddUObject(this, &UPCGSettings::OnSettingsChanged);
#endif //WITH_EDITOR
...
void UPCGSettings::FillOverridableParamsPins(TArray<FPCGPinProperties>& OutPins) const
{
	...
	if (PinType)
	{
		if (*PinType != EPCGDataType::Param)
		{
			...
			UE_LOGF(LogPCG, Error, "[%ls-%ls] While adding pin '%ls', another pin '%ls' was found with invalid type (type must be Attribute Set). "
...
#if WITH_EDITOR
		ParamPin.Tooltip = LOCTEXT("GlobalParamPinTooltipV2", "Attribute Set containing multiple parameters to override. Names must match perfectly.");
#endif // WITH_EDITOR
	}
	...
	for (const FPCGSettingsOverridableParam& OverridableParam : OverridableParams())
	{
		if (InputPinsLabelsAndTypes.Contains(OverridableParam.Label))
		{
			if (PCGSettings::CVarWarnOverridePinsCollideWithInputPins.GetValueOnAnyThread())
			{
				...
				UE_LOGF(LogPCG, Warning, "[%ls-%ls] While automatically adding override pins, an existing pin was found with conflicting name '%ls'. "
					"Rename or remove this pin to allow the automatic override pin to be added. Automatic override pin '%ls' skipped.",
					*GraphName, *NodeName, *ParamsName, *ParamsName);
			}
			continue;
		}
		...
		if (!OverridableParam.Properties.IsEmpty())
		{
			...
			const FText ContainerInfo = OverridableParam.NumContainers > 0 ? LOCTEXT("ContainerTooltip", " This is an array.") : FText{};

			ParamPin.Tooltip = FText::Format(LOCTEXT("OverridableParamPinTooltip", "{0}Attribute type is \"{1}\" and its exact name is \"{2}\".{3}"),
...
FPCGElementPtr UPCGSettings::GetElement() const
{
	if (!CachedElement)
	{
		PCG::TScopeLock ScopeLock(CacheLock);
...
bool UPCGSettings::CanEditChange(const FEditPropertyChain& InPropertyChain) const
{
	...
	const bool bIsOverridable = MemberProperty->HasMetaData(PCGObjectMetadata::Overridable)
		|| MemberProperty->HasMetaData(PCGObjectMetadata::OverridableCPUAndGPU)
		|| MemberProperty->HasMetaData(PCGObjectMetadata::OverridableCPUAndGPUWithReadback);

	if (!bIsOverridable || Property->HasMetaData(PCGObjectMetadata::NotOverridable))
	{
		return true;
	}
...
bool UPCGSettings::CanUserEditTitle() const
{
	return !ShouldDrawNodeCompact();
}
...
void UPCGSettings::ApplyStructuralDeprecation(UPCGNode* InOutNode)
{
	...
	if (DataVersion < FPCGCustomVersion::GetPCGComponentDataMustOverlapSourceComponentByDefault && !TagsAppliedOnOutput_DEPRECATED.IsEmpty())
	{
		...
		{
			UE_LOGF(LogPCG, Log, "Tags applied to output detected. One or more 'Add Tags' node was created automatically to repliecate the previous behavior.");
		}
	}
}
#endif // WITH_EDITOR
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	{
		UE_LOGF(LogPCG, Warning, "Class '%ls' implements `GetCurrentPinTypes` and not `GetCurrentPinTypesID`. Needs to be updated.", *GetClass()->GetName());
		return LegacyResult;
	}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	// @todo_pcg: investigate the nodes that still rely on this fallback (i.e. CustomHLSL kernels) and migrate them to IPCGDynamicPinsProvider.
	// IPCGDynamicPinsProvider implementers store explicit pin types in their containers, so use the declared type directly.
	// Other HasDynamicPins() nodes need their output type inferred from the first input pin's incident edges.
	if (HasDynamicPins() && InPin->IsOutputPin() && !GetClass()->ImplementsInterface(UPCGDynamicPinsProvider::StaticClass()))
	{
...
TArray<FPCGSettingsOverridableParam> UPCGSettings::GatherOverridableParams() const
{
	...
	Config.ShouldKeepPropertyFunc = [](const FProperty* InProperty, int32 Depth) -> bool
	{
		check(InProperty);
		...
		{
			// All properties explicitly marked "Overridable" are overridable
			return true;
		}
		else if (InProperty->HasMetaData(PCGObjectMetadata::NotOverridable))
		{
			// All properties explicitly marked "Not Overridable" are not overridable
			return false;
		}
		else if (!InProperty->HasAnyPropertyFlags(CPF_Edit))
		{
			// Then for recursive structs, make sure to not take properties that are not editable.
			return false;
		}
		else
		{
			// If we have none of that, we only allow overrides for nested properties.
			return Depth > 0;
		}
	};
...
void UPCGSettings::CreateKernels(FPCGGPUCompilationContext& InOutContext, UObject* InObjectOuter, TArray<UPCGComputeKernel*>& OutKernels, TArray<FPCGKernelEdge>& OutEdges) const
{
	CreateKernels(InOutContext, InObjectOuter, GetTypedOuter<UPCGNode>(), OutKernels, OutEdges);
}

bool UPCGSettings::DisplayExecuteOnGPUSettingCondition() const
{
	if (FPCGModule::IsCPUOnlyExecution())
	{
		return false;
	...
	return DisplayExecuteOnGPUSetting();
}

bool UPCGSettings::IsGPUFriendly(const FPCGPreConfiguredSettingsInfo* PreconfiguredInfo) const
{
	return DisplayExecuteOnGPUSetting() || GetType() == EPCGSettingsType::GPU;
}
#endif // WITH_EDITOR

bool UPCGSettings::ShouldExecuteOnGPU() const
{
#if WITH_EDITOR
	if (FPCGModule::IsCPUOnlyExecution())
	{
...
void UPCGSettings::InitializeCachedOverridableParams(bool bReset)
{
	...
	if (CachedOverridableParams.IsEmpty() || bReset)
	{
		...
		// Also we keep a list of previously serialized labels. If we have an alias that is the same as one of those labels, it will also mark this label up for deprecation to rename and not breaking pins.
		TMap<FString, FName> PropertyPathToLabel;
		TSet<FName> PreUpdatedLabels;
		if (!CachedOverridableParams.IsEmpty())
		{
			for (const FPCGSettingsOverridableParam& Param : CachedOverridableParams)
			{
				...
				PropertyPathToLabel.Emplace(Param.GetPropertyPath(), Param.Label);
				PreUpdatedLabels.Add(Param.Label);
			}
		}
		...
		if (!PropertyPathToLabel.IsEmpty())
		{
			for (FPCGSettingsOverridableParam& Param : CachedOverridableParams)
			{
				...
				const FString Path = Param.GetPropertyPath();
				...
				for (const FName Alias : Param.GenerateAllPossibleAliases())
				{
					if (PreUpdatedLabels.Contains(Alias))
					{
						Param.MapOfAliases.FindOrAdd(PCGSettings::DeprecationAliasIndex).Aliases.Emplace(Alias);
						break;
					}
				}
			}
	...
	for (int32 i = 0; i < CachedOverridableParams.Num(); ++i)
	{
		...
		if (Param.PropertiesNames.IsEmpty())
		{
			UE_LOGF(LogPCG, Error, "[InitializeCachedOverridableParams] %ls one param is missing its property names, we cannot use this one as overridable param", *GetName());
		...
		if (!Param.PropertyClass)
		{
			...
			if (!Param.PropertyClass)
			{
				UE_LOGF(LogPCG, Error, "[InitializeCachedOverridableParams] %ls one param is missing its property class, we cannot use this one as overridable param", *GetName());
				CachedOverridableParams.RemoveAt(i--);
				continue;
			}
		}
		...
		// Do not support multi containers
		if (Param.NumContainers > 1)
		{
			CachedOverridableParams.RemoveAt(i--);
			continue;
		}
		...
		const FProperty* CurrentProperty = PCGPropertyHelpers::FindPropertyByName(Param.PropertyClass, Param.PropertiesNames[0]);
		if (!CurrentProperty)
		{
			CachedOverridableParams.RemoveAt(i--);
			continue;
		...
		for (int32 j = 0; j < Param.PropertiesNames.Num(); ++j)
		{
			// If we have multiple depth properties, it can be containers. In that case, we have 2 properties for a single name (the container property and inner property)
			...
			// Otherwise it can be objects/structs.
			...
			if (UnderlyingStruct && j < Param.PropertiesNames.Num() - 1)
			{
				Param.Properties.Add(CurrentProperty);
				CurrentProperty = PCGPropertyHelpers::FindPropertyByName(UnderlyingStruct, Param.PropertiesNames[j + 1]);
				check(CurrentProperty);
			}
			else if (j == Param.PropertiesNames.Num() - 1)
			{
				// If we are at the last property we're fine.
				Param.Properties.Add(CurrentProperty);
			}
			else
			{
				// Otherwise we're ill-formed, reset the properties.
...
#if WITH_EDITOR
void UPCGSettings::OnSettingsChanged(UPCGSettings* InSettings, EPCGChangeType ChangeType)
{
	if (ensure(InSettings == this) && EnumHasAnyFlags(ChangeType, EPCGChangeType::Settings))
	{
		// Clear the cached crc if an external call has settings changed externally.
		CachedCrc = FPCGCrc();
	}
}
#endif
