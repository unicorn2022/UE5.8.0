...
UPackage* UPCGAssetExporter::Update(const FAssetData& PCGAsset)
{
	...
	{
		UE_LOGF(LogPCG, Warning, "Updating the '%ls' PCG asset failed.", *PCGAsset.AssetName.ToString())
		return nullptr;
	}
#else
	UE_LOGF(LogPCG, Error, "PCG Asset update is not supported in non-editor builds.");
	return nullptr;
#endif
}
...
#else
	UE_LOGF(LogPCG, Error, "PCG Asset export should not be used in non-editor builds.");
	return false;
#endif
}
