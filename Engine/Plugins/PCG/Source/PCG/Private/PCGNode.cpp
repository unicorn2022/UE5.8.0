...
void UPCGNode::ApplyDeprecation()
{
	...
	for (const TObjectPtr<UPCGNode>& OutboundNode : OutboundNodes_DEPRECATED)
	{
		...
		{
			UE_LOGF(LogPCG, Error, "Unable to apply deprecation on outbound nodes");
		}
	}
	...
	for (UPCGEdge* OutboundEdge : OutboundEdges_DEPRECATED)
	{
		...
		if (!OutputPin)
		{
			UE_LOGF(LogPCG, Error, "Unable to apply deprecation on outbound edge on node '%ls' - can't find output pin '%ls'", *GetFName().ToString(), *OutboundEdge->InboundLabel_DEPRECATED.ToString());
			continue;
		}
		...
		if (!OtherNode)
		{
			UE_LOGF(LogPCG, Error, "Unable to apply deprecation on outbound edge on node '%ls' - can't find other node", *GetFName().ToString());
			continue;
		}
		...
		{
			UE_LOGF(LogPCG, Error, "Unable to apply deprecation on outbound edge on node %ls output pin %ls - can't find node %ls input pin %ls", *GetFName().ToString(), *OutboundEdge->InboundLabel_DEPRECATED.ToString(), *OtherNode->GetFName().ToString(), *OutboundEdge->OutboundLabel_DEPRECATED.ToString());
		}
	}
...
void UPCGNode::RebuildAfterPaste()
{
	...
	GetObjectsWithOuter(this, Objects, EGetObjectsFlags::None, RF_NoFlags, EInternalObjectFlags::Garbage);
...
EPCGChangeType UPCGNode::UpdatePins(bool bQuiet)
{
	return UpdatePins([](UPCGNode* Node){ return NewObject<UPCGPin>(Node); }, bQuiet);
}
...
EPCGChangeType UPCGNode::UpdatePins(TFunctionRef<UPCGPin*(UPCGNode*)> PinAllocator, bool bQuiet)
{
	...
	UE_LOG_CONTEXT("PCGNode", FSoftObjectPath(Settings));

	const FName SettingsName = Settings->GetFName();
	const FString GraphPath = GetGraph() ?  FSoftObjectPath(GetGraph()).ToString() : TEXT("Unknown");
	...
	auto RemoveDuplicates = [this, &GraphPath](TArray<FPCGPinProperties>& Properties)
	{
		for (int32 i = Properties.Num() - 2; i >= 0; --i)
		{
			for (int32 j = i + 1; j < Properties.Num(); ++j)
			{
				...
				{
					UE_LOGF(LogPCG, Warning, "UpdatePins: Pin properties from the settings on node '%ls' in graph '%ls' contained a duplicate pin '%ls', removing this pin properties.",
						*GetName(), *GraphPath, *Properties[i].Label.ToString());
	...
	auto UpdatePins = [this, &PinAllocator, &TouchedNodes, &SettingsName, bQuiet, &GraphPath](TArray<UPCGPin*>& Pins, const TArray<FPCGPinProperties>& PinProperties)
	{
		...
		for (UPCGPin* Pin : Pins)
		{
			if (const FPCGPinProperties* MatchingProperties = PinProperties.FindByPredicate([Pin](const FPCGPinProperties& Prop) { return Prop.Label == Pin->Properties.Label; }))
			{
				if (!(Pin->Properties == *MatchingProperties))
				{
					...
					if (Pin->BreakAllIncompatibleEdges(&TouchedNodes, !bQuiet ? &Reason : nullptr))
					{
						bAppliedEdgeChanges = true;
						if (!bQuiet)
						{
							UE_LOGF(LogPCG, Warning, "[%ls - %ls] UpdatePins: %ls", *GraphPath, *SettingsName.ToString(), *Reason);	
						}
					}
		...
		{
			const FName PreviousName = UnmatchedPins[0]->Properties.Label;
			const FName NewName = UnmatchedProperties[0].Label;
			...
			if (UnmatchedPins[0]->BreakAllIncompatibleEdges(&TouchedNodes, !bQuiet ? &Reason : nullptr))
			{
				bAppliedEdgeChanges = true;
				if (!bQuiet)
				{
					UE_LOGF(LogPCG, Warning, "[%ls-%ls] UpdatePins: Pin %ls was matched and renamed to %ls\n%ls", *GraphPath, *SettingsName.ToString(), *PreviousName.ToString(), *NewName.ToString(), *Reason);	
				}
			}
		...
		else
		{
			...
			auto RemovePins = [&Pins, &AllPinNames, &bAppliedEdgeChanges, &TouchedNodes, bQuiet, &SettingsName, &GraphPath](TArray<UPCGPin*>& PinsToRemove, bool bRemoveFromAllNames, bool bWarn)
			{
				for (int32 RemovedPinIndex = PinsToRemove.Num() - 1; RemovedPinIndex >= 0; --RemovedPinIndex)
				{
					...
					if (PinIndex >= 0)
					{
						...
						TSet<UPCGPin*> OtherPinsTouched{};

						if (Pins[PinIndex]->BreakAllEdges(&TouchedNodes, !bQuiet ? &OtherPinsTouched : nullptr))
						{
							bAppliedEdgeChanges = true;

							if (bWarn && !bQuiet)
							{
								TStringBuilder<512> WarnMessage;
								WarnMessage += FString::Printf(TEXT("[%s-%s] UpdatePins: Pin %s does not exist anymore.\n"), *GraphPath, *SettingsName.ToString(), *Pins[PinIndex]->Properties.Label.ToString());
								WarnMessage += TEXT("List of broken edges:\n");
								for (const UPCGPin* OtherPin : OtherPinsTouched)
								{
									if (!OtherPin)
									{
										continue;
									...
									const FName OtherPinLabel = OtherPin->Properties.Label;
									const UPCGSettings* OtherSettings = OtherPin->Node ? OtherPin->Node->GetSettings() : nullptr;
									const FName OtherSettingsName = OtherSettings ? OtherSettings->GetFName() : "Unknown";

									WarnMessage += FString::Printf(TEXT(" - %s:%s"), *OtherSettingsName.ToString(), *OtherPinLabel.ToString());
								}

								UE_LOGF(LogPCG, Warning, "%ls", WarnMessage.ToString());
							}
						}
			...
			RemovePins(UnmatchedPins, /*bRemoveFromAllNames=*/true, /*bWarn=*/true);
			RemovePins(DuplicatedNamePins, /*bRemoveFromAllNames=*/false, /*bWarn=*/false);
