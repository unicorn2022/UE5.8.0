...
bool UPCGPin::BreakAllEdges(TSet<UPCGNode*>* InTouchedNodes, TSet<UPCGPin*>* OptionalOtherPinsTouched)
{
	bool bChanged = false;
	...
	for (UPCGEdge* Edge : Edges)
	{
		if (UPCGPin* OtherPin = Edge->GetOtherPin(this))
		{
			...
			if (OptionalOtherPinsTouched)
			{
				OptionalOtherPinsTouched->Add(OtherPin);
			}
		}
	}
...
bool UPCGPin::BreakAllIncompatibleEdges(TSet<UPCGNode*>* InTouchedNodes/*= nullptr*/, FString* OptionalReason)
{
	bool bChanged = false;
	bool bHasAValidEdge = false;

	auto AppendReason = [OptionalReason](FString InReason)
	{
		if (!OptionalReason)
		{
			return;
		...
		{
			*OptionalReason = MoveTemp(InReason);
		}
		else
		{
			*OptionalReason = FString::Printf(TEXT("%s\n%s"), **OptionalReason, *InReason);
		}
	};

	const FName ThisPinName = Properties.Label;
	...
	for (int32 EdgeIndex = Edges.Num() - 1; EdgeIndex >= 0; --EdgeIndex)
	{
		...
		const UPCGNode* OtherNode = OtherPin ? Cast<const UPCGNode>(OtherPin->GetOuter()) : nullptr;
		const FName OtherNodeName = OtherNode ? OtherNode->GetFName() : FName("Unknown");
		const FName OtherPinName = OtherPin ? OtherPin->Properties.Label : FName("Unknown");

		bool bRemoveEdge = (!AllowsMultipleConnections() && bHasAValidEdge);
		if (bRemoveEdge && OptionalReason)
		{
			AppendReason(FString::Printf(TEXT("Breaking edge between %s on node %s and %s because the pin does not support multi connection."),
				*OtherPinName.ToString(),
				*OtherNodeName.ToString(),
		...
		if (!PCGDataTypeCompatibilityResult::IsValid(Compatibility) && Compatibility != EPCGDataTypeCompatibilityResult::TypeCompatibleSubtypeNotCompatible)
		{
			if (OptionalReason)
			{
				AppendReason(FString::Printf(TEXT("Breaking edge between %s on node %s and %s because the pin types are not compatible."),
					*OtherPinName.ToString(),
					*OtherNodeName.ToString(),
					*ThisPinName.ToString()));
			}
