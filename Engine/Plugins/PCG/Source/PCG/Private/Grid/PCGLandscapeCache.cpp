...
bool FPCGLandscapeCacheEntry::TouchAndLoad(int32 InTouch) const
{
	...
	if (!bDataLoaded)
	{
		PCG::TScopeLock ScopeDataLock(DataLock);
		if (!bDataLoaded)
		{
...
bool FPCGLandscapeCacheEntry::SerializeFromBulkData() const
{
	...
	if (!Data)
	{
		UE_LOGF(LogPCG, Error, "Unable to load Landscape Cache Entry bulk data");
		return false;
	}
...
TSharedPtr<const FPCGLandscapeCacheEntry> UPCGLandscapeCache::GetOrCreateCacheEntryInternal(const ULandscapeInfo* InLandscapeInfo, const FIntPoint& InComponentKey, bool bInAllowLoadOrCreate, bool& bOutIsValidEntry)
{
	...
	PCG::TUniqueScopeLock WriteLock(CacheLock);
...
TSharedPtr<const FPCGLandscapeCacheEntry> UPCGLandscapeCache::GetCacheEntryInternal(AActor* HintActor, const FGuid& LandscapeGuid, const FIntPoint& ComponentCoordinate, bool bInAllowLoad)
{
	...
	{
#if WITH_EDITOR
		PCG::TSharedScopeLock ReadLock(CacheLock);
#else
		if (!bLoggedNoCacheError && CachedData.IsEmpty())
		{
			UE_LOGF(LogPCG, Error, "PCG Landscape cache is empty; it probably was never serialized. Check the landscape cache object on the PCG World Actor.");
			bLoggedNoCacheError = true;
		}
#endif
		if (const TSharedPtr<FPCGLandscapeCacheEntry>* FoundEntry = CachedData.Find(ComponentKey))
...
void UPCGLandscapeCache::SampleMetadataOnPoint(ALandscapeProxy* Landscape, const FTransform& InTransform, int64& OutMetadataEntry, UPCGMetadata* OutMetadata)
{
	...
	if (!LandscapeInfo)
	{
		UE_LOGF(LogPCG, Warning, "Unable to get landscape layer weights because the landscape info is not available (landscape not registered yet?");
		return;
	}
