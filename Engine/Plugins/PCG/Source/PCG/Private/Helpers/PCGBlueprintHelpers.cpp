...
#else
	UE_LOGF(LogPCG, Error, "GenerateGraphAsync not supported in non editor builds.")
...
{
	PCGLog::LogErrorOnGraph(ErrorMessage);
...
const UPCGSettings* UPCGBlueprintHelpers::GetSettingsWithContext(const FPCGBlueprintContextHandle& ContextHandle)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::GetSettingsWithContext : Invalid context handle");
	return nullptr;
}
...
UPCGData* UPCGBlueprintHelpers::GetActorDataWithContext(const FPCGBlueprintContextHandle& ContextHandle)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::GetActorDataWithContext : Invalid context handle");
	return nullptr;
}
...
UPCGData* UPCGBlueprintHelpers::GetInputDataWithContext(const FPCGBlueprintContextHandle& ContextHandle)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::GetInputDataWithContext : Invalid context handle");
	return nullptr;
}
...
UPCGComponent* UPCGBlueprintHelpers::GetComponentWithContext(const FPCGBlueprintContextHandle& ContextHandle)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::GetComponentWithContext : Invalid context handle");
	return nullptr;
}
...
UPCGComponent* UPCGBlueprintHelpers::GetOriginalComponentWithContext(const FPCGBlueprintContextHandle& ContextHandle)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::GetOriginalComponentWithContext : Invalid context handle");
	return nullptr;
}
...
AActor* UPCGBlueprintHelpers::GetTargetActorWithContext(const FPCGBlueprintContextHandle& ContextHandle, UPCGSpatialData* SpatialData)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::GetTargetActorWithContext : Invalid context handle");
	return nullptr;
}
...
TArray<FPCGLandscapeLayerWeight> UPCGBlueprintHelpers::GetInterpolatedPCGLandscapeLayerWeights(UObject* WorldContextObject, const FVector& Location)
{
	...
	{
		UE_LOGF(LogPCG, Warning, "[%ls] %ls", *WorldContextObject->GetName(), *FailureReason);
	}
...
int64 UPCGBlueprintHelpers::GetTaskIdWithContext(const FPCGBlueprintContextHandle& ContextHandle)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::GetTaskIdWithContext : Invalid context handle");
...
void UPCGBlueprintHelpers::RefreshPCGRuntimeComponent(UPCGComponent* InComponent, const bool bFlushCache)
{
	if (UPCGSubsystem* PCGSubsystem = UPCGSubsystem::GetSubsystemForCurrentWorld())
	{
		...
		PCGSubsystem->RefreshRuntimeGenExecutionSource(InComponent, EPCGChangeType::GenerationGrid);
	}
}
...
UPCGData* UPCGBlueprintHelpers::DuplicateDataWithContext(const UPCGData* InData, const FPCGBlueprintContextHandle& ContextHandle, bool bInitializeMetadata)
{
	...
	UE_LOGF(LogPCG, Error, "UPCGBlueprintHelpers::DuplicateDataWithContext : Invalid context handle");
	return nullptr;
}

DEFINE_FUNCTION(UPCGBlueprintHelpers::execExtractStructureAsAttributeSet)
{
	Stack.StepCompiledIn<FStructProperty>(nullptr);
	void* StructPtr = Stack.MostRecentPropertyAddress;
	FStructProperty* StructProp = CastField<FStructProperty>(Stack.MostRecentProperty);

	P_GET_PROPERTY(FNameProperty, PropertyPath);
	P_GET_PROPERTY(FNameProperty, OutputAttributeName);
	P_GET_PROPERTY(FBoolProperty, bExtract);
	P_GET_PROPERTY(FBoolProperty, bStrictlySanitizeNames);

	P_FINISH;

	P_NATIVE_BEGIN;

	UPCGParamData* Result = nullptr;

	const bool bHasValidStructProp = StructProp && StructProp->Struct && StructPtr;

	FPCGAttributePropertySelector InputSelector = FPCGAttributePropertySelector::CreateSelectorFromString<FPCGAttributePropertySelector>(PropertyPath.ToString());

	if (bHasValidStructProp)
	{
		PCGPropertyHelpers::FExtractorParameters Params(StructPtr, StructProp->Struct, InputSelector, OutputAttributeName, /*bInPropertyNeedsToBeVisible=*/true);
		if (bExtract)
		{
		...
		Params.bStrictSanitizeOutputAttributeNames = bStrictlySanitizeNames;
		Result = PCGPropertyHelpers::ExtractPropertyAsAttributeSet(Params);
		if (!Result)
		{
			ThrowBlueprintException(NSLOCTEXT("PCGPropertyHelpers", "FailedToExtract", "Failed to extract the struct as a ParamData. Check the log for more information."));
		}
	}
	else
	{
		ThrowBlueprintException(NSLOCTEXT("PCGPropertyHelpers", "InvalidStruct", "Parameter passed into \"Structure To Convert\" is not a Structure."));
	}

	*static_cast<UPCGParamData**>(RESULT_PARAM) = Result;
