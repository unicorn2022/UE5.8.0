...
namespace PCGSubgraphHelpersExtra
{
	...
	*/
	struct FCollapsingInformation
	{
		...
		static FCollapsingInformation CreateAndInitialize(const TArray<UPCGNode*>& InNodesToCollapse, const FInstancedPropertyBag& InGraphParameters, UPCGGraph* InOriginalGraph, TSubclassOf<UPCGBaseSubgraphSettings> InSubgraphSettingsClass);
		bool ValidateCollapseIsNotIntroducingCycle();

		void SetNewGraphAndUpdatePositions(UPCGGraph* NewGraph);

		void CopyAndRemapEmbeddedSubgraphs();
	...
	private:
		...
		// List used embedded subgraphs
		TArray<UPCGNode*> EmbeddedSubgraphNodes;
		...
		TSubclassOf<UPCGBaseSubgraphSettings> SubgraphSettingsClass = UPCGSubgraphSettings::StaticClass();

		TMap<UPCGGraph*, UPCGGraph*> DuplicatedEmbeddedSubgraphs;
	...
	FCollapsingInformation FCollapsingInformation::CreateAndInitialize(const TArray<UPCGNode*>& InNodesToCollapse, const FInstancedPropertyBag& InGraphParameters, UPCGGraph* InOriginalGraph, TSubclassOf<UPCGBaseSubgraphSettings> InSubgraphSettingsClass)
	{
		...
		if (InSubgraphSettingsClass)
		{
			CollapseInfo.SubgraphSettingsClass = InSubgraphSettingsClass;
		}
		...
		for (UPCGNode* PCGNode : InNodesToCollapse)
		{
			...
			if (const UPCGBaseSubgraphSettings* SubgraphSettings = Cast<UPCGBaseSubgraphSettings>(NodeSettings))
			{
				if (SubgraphSettings->GetSubgraph() && SubgraphSettings->GetSubgraph()->IsEmbeddedSubgraph())
				{
					CollapseInfo.EmbeddedSubgraphNodes.Add(PCGNode);
				}
			}
	...
	void FCollapsingInformation::CopyAndRemapEmbeddedSubgraphs()
	{
		check(OriginalGraph && NewGraph);

		// Not needed if we are creating a new embedded graph (and not an asset)
		if(NewGraph->GetEmbeddedParentGraph() == OriginalGraph)
		{
			return;
		}

		TArray<UPCGGraph*> EmbeddedSubgraphsToCopy;
		for (UPCGNode* EmbeddedSubgraphNode : EmbeddedSubgraphNodes)
		{
			UPCGBaseSubgraphSettings* SubgraphSettings = CastChecked<UPCGBaseSubgraphSettings>(EmbeddedSubgraphNode->GetSettings());
			UPCGGraph* EmbeddedSubgraph = SubgraphSettings->GetSubgraph();
			check(EmbeddedSubgraph && EmbeddedSubgraph->IsEmbeddedSubgraph());
			EmbeddedSubgraphsToCopy.AddUnique(EmbeddedSubgraph);
		}

		DuplicatedEmbeddedSubgraphs.Empty();
		NewGraph->DuplicateEmbeddedSubgraphs(EmbeddedSubgraphsToCopy, DuplicatedEmbeddedSubgraphs);

		for (UPCGNode* EmbeddedSubgraphNode : EmbeddedSubgraphNodes)
		{
			check(OriginalToNewNodesMapping.Contains(EmbeddedSubgraphNode));
			UPCGNode* NewEmbeddedSubgraphNode = OriginalToNewNodesMapping[EmbeddedSubgraphNode];
			check(NewEmbeddedSubgraphNode);

			UPCGBaseSubgraphSettings* SubgraphSettings = CastChecked<UPCGBaseSubgraphSettings>(NewEmbeddedSubgraphNode->GetSettings());
			UPCGGraph* DuplicatedEmbeddedSubgraph = DuplicatedEmbeddedSubgraphs.FindChecked(SubgraphSettings->GetSubgraph());

			SubgraphSettings->SetSubgraph(DuplicatedEmbeddedSubgraph);
		}
	}
	...
	void FCollapsingInformation::CreateSubgraphNodeAndConnectRemainingPins()
	{
		...
		UPCGNode* SubgraphNode = OriginalGraph->AddNodeOfType(SubgraphSettingsClass, DefaultNodeSettings);
		UPCGBaseSubgraphSettings* DefaultSubgraphSettings = CastChecked<UPCGBaseSubgraphSettings>(DefaultNodeSettings);
...
UPCGGraph* FPCGSubgraphHelpers::CollapseIntoSubgraph(UPCGGraph* InOriginalGraph, const TArray<UPCGNode*>& InNodesToCollapse, const TArray<UObject*>& InExtraNodesToCollapse, UPCGGraph* OptionalPreAllocatedGraph)
{
	...
	if (!OutSubgraph)
	{
		UE_LOGF(LogPCG, Warning, "%ls", *OutFailReason.ToString());
	}

	return OutSubgraph;
}

UPCGGraph* FPCGSubgraphHelpers::CollapseIntoSubgraphWithReason(UPCGGraph* InOriginalGraph, const TArray<UPCGNode*>& InNodesToCollapse, const TArray<UObject*>& InExtraNodesToCollapse, FText& OutFailReason, UPCGGraph* OptionalPreAllocatedGraph, TSubclassOf<UPCGBaseSubgraphSettings> InSubgraphSettingsClass)
{
	...
	PCGSubgraphHelpersExtra::FCollapsingInformation CollapseInfo = PCGSubgraphHelpersExtra::FCollapsingInformation::CreateAndInitialize(InNodesToCollapse, *GraphParameters, InOriginalGraph, InSubgraphSettingsClass);
	...
	CollapseInfo.CopyAndRemapEmbeddedSubgraphs();
