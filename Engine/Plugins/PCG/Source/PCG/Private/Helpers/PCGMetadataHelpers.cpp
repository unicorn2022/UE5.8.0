	...
	bool CopyAttributes(UPCGData* TargetData, const UPCGData* SourceData, const TArray<TTuple<FPCGAttributePropertyInputSelector, FPCGAttributePropertyOutputSelector, EPCGMetadataTypes>>& AttributeSelectorsWithOutputType, bool bSameOrigin, FPCGContext* OptionalContext)
	{
		...
		for (const auto& [InputSource, OutputTarget, RequestedOutputType] : AttributeSelectorsWithOutputType)
		{
			...
			if (!bNeedAccessors)
			{
				...
				// Copy entries (and keep the parent) only if they come from the same data and they are on the same domain.
				const bool bCopyEntriesAndKeepParent = bSameOrigin && SourceAttribute->GetMetadataDomain()->GetDomainID() == TargetMetadataDomain->GetDomainID();
				...
				if (FPCGMetadataAttributeBase* NewAttr = TargetMetadataDomain->CopyAttribute(SourceAttribute, LocalDestinationAttribute.Name, /*bKeepParent=*/bCopyEntriesAndKeepParent, /*bCopyEntries=*/bCopyEntriesAndKeepParent, /*bCopyValues=*/true))
				{
