...
AActor* FPCGContext::GetTargetActor(const UPCGSpatialData* InSpatialData) const
{
	...
	{
		return GetTypedExecutionTarget<AActor>();
	}
}

UObject* FPCGContext::GetExecutionTarget() const
{
	if (IPCGGraphExecutionSource* ExecutionSourcePtr = ExecutionSource.Get())
	{
		return ExecutionSourcePtr->GetExecutionState().GetTarget();
	}
	else
	{
		return nullptr;
			...
			if (bHasParamConnected)
			{
				...
				if (NodeSettings->HasAnyOverridableHardReferences())
				{
					for (const FPCGSettingsOverridableParam& Param : OverridableParams)
					{
						...
						// If we have a container on the param, we have to get all the paths
						TArray<FSoftObjectPath> ObjectPaths;
						if (ensure(Param.NumContainers <= 1))
						{
							if (Param.NumContainers == 1 && ensure(AccessorResult.Keys.IsValid()))
							{
								ObjectPaths.SetNum(AccessorResult.Keys->GetNum());
								if (!AttributeAccessor->GetRange<FSoftObjectPath>(ObjectPaths, 0, *AccessorResult.Keys, EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible))
								{
									continue;
								}
							}
							...
							{
								if (!PCGAttributeAccessorHelpers::GetOverrideParamValue(*AttributeAccessor, ObjectPaths.Emplace_GetRef()))
								{
									continue;
								}
							}
							...
							for (const FSoftObjectPath& ObjectPath : ObjectPaths)
							{
								if (!ObjectPath.ResolveObject())
								{
									...
									RequestToRunOnMainThread();
									break;
								}
							}
						}

						if (CanExecuteOnlyOnMainThread())
						{
							break;
						}
					}
...
void FPCGContext::OverrideSettings()
{
	...
	check(!bMustRunOnMainThread || IsInGameThread());
	bMustRunOnMainThread = false;
	...
	for (const FPCGSettingsOverridableParam& Param : OriginalSettings->OverridableParams())
	{
		...
		if (!SettingsWithOverride->GetClass()->IsChildOf(Param.PropertyClass))
		{
			...
			{
				FStructView StructView = GetExternalStructContainerForOverridableParam(Param);
				if (StructView.IsValid() && StructView.GetScriptStruct() == Param.Properties[0]->GetOwnerStruct())
				{
					Container = StructView.GetMemory();
				}
				else if (!StructView.IsValid())
				{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		if (AccessorResult.bHasMultipleAttributeSetsOnOverridePin || AccessorResult.bHasMultipleDataInAttributeSet)
		{
			...
			if (AccessorResult.bHasMultipleDataInAttributeSet && Param.NumContainers == 0)
			{
		...
		{
			PCGLog::LogWarningOnGraph(FText::Format(LOCTEXT("TooManyContainers", "Property {0} is within multiple containers (i.e. array of array) which is not supported."), FText::FromName(Param.Label)), this);
			continue;
		}
		...
		const FPCGMetadataAttributeDesc PropertyDesc = FPCGMetadataAttributeDesc::CreateFromProperty(Param.Properties.Last());
		const bool bDescriptorsMatch = PropertyDesc.IsSameType(AttributeAccessor->GetUnderlyingDesc());
		const bool bUseGenericAccessors = PropertyDesc.IsValid() && (!AttributeAccessor->GetUnderlyingDesc().IsSingleValue() || bDescriptorsMatch);
		
		TUniquePtr<IPCGAttributeAccessor> PropertyAccessor;
		
		if (PCGAttributeAccessorHelpers::IsPropertyAccessorSupported(Param.Properties.Last(), bUseGenericAccessors))
		{
			PropertyAccessor = Param.NumContainers == 1
				? PCGContainerAccessorHelpers::MakeContainerAccessor(Param.Properties, bUseGenericAccessors)
				: PCGAttributeAccessorHelpers::CreatePropertyChainAccessor(TArray<const FProperty*>(Param.Properties), bUseGenericAccessors);
		}
		
		if (!PropertyAccessor.IsValid() || (Param.NumContainers == 1 && !AccessorResult.Keys))
		{
			continue;
		}
		
		FPCGAttributeAccessorKeysEntries FirstEntry(PCGFirstEntryKey);
		// If we are converting single value attribute to array attribute we need to use the keys, otherwise it is the first entry and a count of 1
		const bool bSingleValueToArrayConversion = AttributeAccessor->GetUnderlyingDesc().IsSingleValue() && Param.NumContainers == 1 && AccessorResult.Keys;
		const int32 Count = bSingleValueToArrayConversion ? AccessorResult.Keys->GetNum() : 1;
		const IPCGAttributeAccessorKeys& AttributeKeys = bSingleValueToArrayConversion ? *AccessorResult.Keys : FirstEntry;
		
		const bool bParamOverridden = AttributeAccessor->CopyTo(AttributeKeys, *PropertyAccessor, PropertyObjectKey, /*Index=*/ 0, /*Count=*/ Count, EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible | EPCGAttributeAccessorFlags::AllowRangeOfValuesIntoSingleContainer);
		if (!bParamOverridden)
		{
			PCGE_LOG_C(Warning, GraphAndLog, this, FText::Format(LOCTEXT("ConversionFailed", "Parameter '{0}' ({1}) cannot be converted from attribute '{2}' ({3})"),
				...
				PropertyAccessor->GetUnderlyingDesc().GetTypeText(),
				FText::FromName(AttributeName),
				AttributeAccessor->GetUnderlyingDesc().GetTypeText()));
		}
		else
		{
...
bool FPCGContext::IsExecutingGraphInspected() const
{
	...
	return Graph && Graph->IsSameInspectedStack(*StackPtr);
}

AActor* FPCGContext::GetDebugDisplayActor() const
{
	if (AActor* Actor = GetTypedExecutionTarget<AActor>())
	{
		return Actor;
	}
	else if (IPCGGraphExecutionSource* ExecutionSourcePtr = ExecutionSource.Get())
	{
		if (UPCGSubsystem* PCGSubsystem = Cast<UPCGSubsystem>(ExecutionSource->GetExecutionState().GetSubsystem()))
		{
			return PCGSubsystem->GetOrCreateDebugActor();
		}
	}
...
bool FPCGContext::IsCacheEnabled()
{
	IPCGGraphExecutionSource* Source = ExecutionSource.IsValid() ? ExecutionSource.Get() : nullptr;
	if (Source && !Source->GetExecutionState().IsCacheEnabled())
	{
		return false;
	}

	TSharedPtr<FPCGGraphExecutor> Executor = GraphExecutor.Pin();
	return Executor && Executor->GetCache().IsEnabled();
}
...
namespace PCGContext
{
	thread_local FPCGContext* ThisThreadContext = nullptr;
}

FPCGContext* FPCGContext::GetContextForThread()
{
	return PCGContext::ThisThreadContext;
}

FPCGSourceDataContainer* FPCGContext::GetSourceDataContainer()
{
	IPCGGraphExecutionSource* Source = ExecutionSource.IsValid() ? ExecutionSource.Get() : nullptr;
	return Source ? Source->GetExecutionState().GetSourceDataContainer() : nullptr;
}

const FPCGSourceDataContainer* FPCGContext::GetSourceDataContainer() const
{
	const IPCGGraphExecutionSource* Source = ExecutionSource.IsValid() ? ExecutionSource.Get() : nullptr;
	return Source ? Source->GetExecutionState().GetSourceDataContainer() : nullptr;
}

FPCGContextScope::FPCGContextScope(FPCGContext* InContext)
{
	PreviousContext = PCGContext::ThisThreadContext;
	ThisContext = InContext;
	PCGContext::ThisThreadContext = InContext;
}

FPCGContextScope::~FPCGContextScope()
{
	checkf(PCGContext::ThisThreadContext == ThisContext, TEXT("Multiple scope are destroyed in the wrong order"));
	PCGContext::ThisThreadContext = PreviousContext;
}
