...
{
#if !WITH_EDITOR
	// Capture only required for profiling data outside of editor.
	...
	case EPCGExecutionPhase::PostExecute:
		...
		{
			ensure(!Timer.OutputCPUMemorySize.IsSet() && !Timer.OutputGPUMemorySize.IsSet());
			// Count memory here
			...
			uint64 CPUMemorySize = 0;
			uint64 GPUMemorySize = 0;
			...
			for (const UPCGData* Data : NewData)
			{
				...
				CPUMemorySize += ResSize.GetDedicatedSystemMemoryBytes();
				GPUMemorySize += ResSize.GetDedicatedVideoMemoryBytes();
			}
			Timer.OutputCPUMemorySize = CPUMemorySize;
			Timer.OutputGPUMemorySize = GPUMemorySize;
		}
...
#if WITH_EDITOR
	PCG::TScopeLock ScopedLock(Lock);
...
namespace PCGUtils
{
	void BuildTreeInfo(FCallTreeInfo& Info, const TMap<const UPCGNode*, const UPCGGraph*>& SubgraphNodeToGraphMap)
	{
		...
		for (FCallTreeInfo& Child : Info.Children)
		{
			...
			if (Child.CallTime.OutputCPUMemorySize.IsSet())
			{
				Info.CallTime.OutputCPUMemorySize = Info.CallTime.OutputCPUMemorySize.Get(0) + Child.CallTime.OutputCPUMemorySize.GetValue();
			}

			if (Child.CallTime.OutputGPUMemorySize.IsSet())
			{
				Info.CallTime.OutputGPUMemorySize = Info.CallTime.OutputGPUMemorySize.Get(0) + Child.CallTime.OutputGPUMemorySize.GetValue();
			}

			Info.NumberOfTasks += Child.NumberOfTasks;
		}
	}
}
...
PCGUtils::FScopedCall::FScopedCall(const IPCGElement* InOwner, FPCGContext* InContext, FScopedCallOutputDevice& InOutputDevice)
	: Owner(InOwner)
	, Context(InContext)
	, Phase(InContext ? InContext->CurrentPhase : EPCGExecutionPhase::NotExecuted)
