...
namespace PCGSubdivideSplineHelpers
{
	struct FParameters
	{
		...
		const UPCGSubdivideSplineSettings* Settings = nullptr;
	...
	static void Process(FParameters& InOutParameters, const FString& InGrammar)
	{
		...
		const double HalfModuleHeight = InOutParameters.ModuleHeight / 2;
		...
		{
			if (!InOutParameters.Settings->bSilenceGrammarDoesNotFitErrors)
			{
				PCGLog::LogErrorOnGraph(LOCTEXT("GrammarDoesNotFit", "Grammar does not fit for this spline."), InOutParameters.Context);
			}
		...
		for (int32 ModuleInstanceIndex = 0; ModuleInstanceIndex < NumIterations; ModuleInstanceIndex++)
		{
			...
			const double OffsetZ = InOutParameters.Settings->bCenterBounds ? 0.0 : HalfModuleHeight;
			OutPoint.SetLocalBounds(FBox(FVector(-HalfSubmoduleSize, 0, -HalfModuleHeight + OffsetZ), FVector(HalfSubmoduleSize, 1, HalfModuleHeight + OffsetZ)));
...
void UPCGSubdivideSplineSettings::PostLoad()
{
	Super::PostLoad();
