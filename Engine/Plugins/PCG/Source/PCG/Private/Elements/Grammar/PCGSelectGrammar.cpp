...
TArray<FPCGPinProperties> UPCGSelectGrammarSettings::InputPinProperties() const
{
	...
	FPCGPinProperties& InputPin = PinProperties.Emplace_GetRef(PCGPinConstants::DefaultInputLabel, FPCGDataTypeIdentifier::Construct(FPCGDataTypeInfoPoint::AsId(), FPCGDataTypeInfoSpline::AsId()));
...
{
	TArray<FPCGPinProperties> PinProperties;
	PinProperties.Emplace(PCGPinConstants::DefaultOutputLabel, FPCGDataTypeIdentifier::Construct(FPCGDataTypeInfoPoint::AsId(), FPCGDataTypeInfoSpline::AsId()));
...
bool FPCGSelectGrammarElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	for (const FPCGTaggedData& Input : Inputs)
	{
		...
		if (!InputData || (!InputData->IsA<UPCGBasePointData>() && !InputData->IsA<UPCGSplineData>()))
		{
			continue;
		}

		FPCGAttributePropertyInputSelector KeySelector = Settings->KeyAttribute.CopyAndFixLast(InputData);
		TUniquePtr<const IPCGAttributeAccessor> InputKeyAccessor;
		TUniquePtr<const IPCGAttributeAccessorKeys> InputKeys;

		if (Settings->bKeyAsAttribute)
		{
			InputKeyAccessor = PCGAttributeAccessorHelpers::CreateConstAccessor(InputData, KeySelector);
			InputKeys = PCGAttributeAccessorHelpers::CreateConstKeys(InputData, KeySelector);
		}
		else
		{
			InputKeyAccessor = MakeUnique<const FPCGConstantValueAccessor<FName>>(Settings->Key);
			// Dummy single key
			InputKeys = MakeUnique<FPCGAttributeAccessorKeysSingleObjectPtr<void>>();
		}
		...
		UPCGData* OutputData = InputData->DuplicateData(InContext);
		Outputs.Emplace_GetRef(Input).Data = OutputData;

		FPCGAttributePropertyOutputSelector OutputGrammarSelector = Settings->OutputGrammarAttribute.CopyAndFixSource(&KeySelector);
		FPCGMetadataDomain* MetadataDomain = OutputData->MutableMetadata()->GetMetadataDomainFromSelector(OutputGrammarSelector);
		FPCGMetadataAttribute<FString>* OutputAttribute = MetadataDomain->FindOrCreateAttribute(OutputGrammarSelector.GetName(), FString{}, /*bAllowsInterpolation=*/false);
		TUniquePtr<IPCGAttributeAccessor> WriteGrammarAccessor = PCGAttributeAccessorHelpers::CreateAccessor(OutputAttribute, MetadataDomain);
		TUniquePtr<IPCGAttributeAccessorKeys> OutputKeys = PCGAttributeAccessorHelpers::CreateKeys(OutputData, OutputGrammarSelector);
		...
		{
			FPCGAttributePropertyInputSelector ComparedValueSelector = Settings->ComparedValueAttribute.CopyAndFixLast(InputData);
			TUniquePtr<const IPCGAttributeAccessor> ComparedValuesAccessor = PCGAttributeAccessorHelpers::CreateConstAccessor(InputData, ComparedValueSelector);
			TUniquePtr<const IPCGAttributeAccessorKeys> ComparedValuesKeys = PCGAttributeAccessorHelpers::CreateConstKeys(InputData, ComparedValueSelector);
			...
			TStaticArray<const IPCGAttributeAccessor*, 2> Accessors = {InputKeyAccessor.Get(), ComparedValuesAccessor.Get()};
			TStaticArray<const IPCGAttributeAccessorKeys*, 2> Keys = {InputKeys.Get(), ComparedValuesKeys.Get()};

			// Technically we can mix Data and Elements domains, but if the output grammar goes into the data domain, we don't need to process more than one. (Effectively evaluating only the first element).
			const int32 Count = OutputKeys->GetNum();
			PCGMetadataElementCommon::ApplyOnMultiAccessorsRange<FName, double>(Keys, Accessors, ProcessWithCriteria, EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible, PCGMetadataElementCommon::DefaultChunkSize, Count);
		}
	}
