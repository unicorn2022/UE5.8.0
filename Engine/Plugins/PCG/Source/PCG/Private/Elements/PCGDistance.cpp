...
bool FPCGDistanceElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	for (int32 SourceIndex = 0; SourceIndex < Sources.Num(); ++SourceIndex) 
	{
		...
		auto ProcessDistanceFunc = [SourceShape, TargetShape, &TargetPointDatas, MaximumDistance, MaximumDistanceRecip, bSetDensity, bOutputDistanceVector, OutputData, SourcePointData, &ResultCache, bWriteToAttribute = Accessor.IsValid(), Settings, SourceIndex](int32 StartReadIndex, int32 StartWriteIndex, int32 Count)
		{
			...
			struct FPointDataInfo
			{
				explicit FPointDataInfo(const UPCGBasePointData* InPointData)
				...
				const TConstPCGValueRange<FVector> BoundsMinRange;
				const TConstPCGValueRange<FVector> BoundsMaxRange;
			};

			const FPointDataInfo SourceDataInfo{SourcePointData};
			...
			{
				const FTransform& SourceTransform = SourceDataInfo.TransformRange[Index];
				const FVector& SourceBoundsMin = SourceDataInfo.BoundsMinRange[Index];
				const FVector& SourceBoundsMax = SourceDataInfo.BoundsMaxRange[Index];
				...
				auto CalculateSDF = [Index, &MinDistanceSquared, &MinDistanceVector, SourcePointData, &SourceTransform, &SourceBoundsMin, &SourceBoundsMax, SourceCenter, SourceShape, TargetShape](const FPointDataInfo& TargetDataInfo, const int32 TargetPointIndex, const FBoxSphereBounds& Bounds)
				{
					// If the source pointer and target pointer are the same, ignore distance to the exact same point
					if (Index == TargetPointIndex && SourcePointData == TargetDataInfo.PointData)
					{
						return;
					}
					...
					const FVector TargetShapePos = PCGDistance::CalcPosition(TargetShape, TargetDataInfo.TransformRange[TargetPointIndex], TargetDataInfo.BoundsMinRange[TargetPointIndex], TargetDataInfo.BoundsMaxRange[TargetPointIndex], TargetCenter, SourceCenter);
				...
				auto CheckAgainstTargetPointData = [&SourceQueryBounds, &CalculateSDF](const UPCGBasePointData* TargetPointData)
				{
					...
					Octree.FindElementsWithBoundsTest(FBoxCenterAndExtent(SourceQueryBounds.Origin, SourceQueryBounds.BoxExtent),
						[TargetDataInfo = FPointDataInfo{TargetPointData}, &CalculateSDF](const PCGPointOctree::FPointRef& InPointRef)
						{
							CalculateSDF(TargetDataInfo, InPointRef.Index, InPointRef.Bounds);
						});
				};
