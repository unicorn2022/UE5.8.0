...
namespace PCGWorldQuery
{
	...
	void ExtractObjectFiltersIfNeeded(const FPCGWorldCommonQueryParams& QueryParams, FPCGContext* Context, WorldQueryDataType* WorldQueryData)
	{
		...
		if (QueryParams.ActorFilterFromInput != EPCGWorldQueryFilter::None)
		{
			...
			if (ActorFilterData && !QueryParams.ExtractObjectFiltersIfNeeded(ActorFilterData, WorldQueryData->GetMutableFilteredObjects(), Context))
			{
...
bool FPCGWorldVolumetricQueryElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	if (!Context->ExecutionSource->GetExecutionState().GetWorld())
	{
		PCGLog::LogErrorOnGraph(LOCTEXT("NoWorld_VolumetricQuery", "Execution source does not have a valid world."), Context);
		return true;
	}

	UPCGWorldVolumetricData* Data = FPCGContext::NewObject_AnyThread<UPCGWorldVolumetricData>(Context);
	Data->Initialize(Context->ExecutionSource.Get());
	Data->QueryParams = QueryParams;
	Data->QueryParams.Initialize();
	Data->SetOriginatingSource(Context->ExecutionSource.Get());

	PCGWorldQuery::ExtractObjectFiltersIfNeeded(Settings->QueryParams, Context, Data);
...
bool FPCGWorldRayHitQueryElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	if (!World)
	{
		PCGLog::LogErrorOnGraph(LOCTEXT("NoWorld_RayHitQuery", "Execution source does not have a valid world."), Context);
		return true;
	}
	...
	Data->Initialize(Context->ExecutionSource.Get(), Transform, /*InBounds=*/FBox(EForceInit::ForceInit), LocalBounds);

	if (QueryParams.bApplyMetadataFromLandscape && Data->Metadata)
	{
		UPCGSubsystem* PCGSubsystem = Cast<UPCGSubsystem>(ExecutionSource->GetExecutionState().GetSubsystem());
		...
		if (LandscapeCache)
		{
			...
			{
				BoundsCheck = [Data, ExecutionSource](const AActor* OtherActor) -> bool
				{
			...
			TArray<AActor*> LandscapeActors = PCGActorSelector::FindActors(ActorSelector, ExecutionSource, BoundsCheck, SelfIgnoreCheck);
