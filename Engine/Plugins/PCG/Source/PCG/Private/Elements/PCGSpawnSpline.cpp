...
bool FPCGSpawnSplineElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (!*Settings->SplineComponent || !ExecutionSource || !ManagedResourcesContainerHelper.IsValid())
	{
		return true;
	}
	...
	for (const FPCGTaggedData& InputData : Inputs)
	{
...
#if WITH_EDITOR
		FScopedTransaction Transaction(LOCTEXT("AddSplineComponent", "Adding spline component to actor from PCG"), ExecutionSource->GetExecutionState().UseTransactions());
#endif
		...
		UPCGManagedComponent* ManagedComponent = NewObject<UPCGManagedComponent>(ExecutionSourceObject);
		...
		// Add tags, both the default ones (source name + PCG Generated) and the ones from the override pin.
		// We do this after the override in the case that the override was writing the tags.
		if (ExecutionSourceObject)
		{
			SplineComponent->ComponentTags.Add(ExecutionSourceObject->GetFName());
		}
