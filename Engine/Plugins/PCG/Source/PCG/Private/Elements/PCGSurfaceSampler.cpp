...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			{
				if (!GeneratingShape->IsBounded() && !ExecState.BoundingShapeBounds.IsValid)
				{
...
void UPCGSurfaceSamplerSettings::ApplyDeprecation(UPCGNode* InOutNode)
{
	...
	{
		UE_LOGF(LogPCG, Log, "Surface Sampler node migrated from an older version. Review edges on the input pins and then save this graph to upgrade the data.");
