...
bool UPCGCopyPointsKernel::IsKernelDataValid(const UPCGDataBinding* InDataBinding, FPCGContext* InContext) const
{
	...
	return PCGCopyPointsKernel::IsKernelDataValid(this, InDataBinding, InContext);
}
...
TSharedPtr<const FPCGDataCollectionDesc> UPCGCopyPointsKernel::ComputeOutputBindingDataDesc(FName InOutputPinLabel, UPCGDataBinding* InBinding) const
{
	...
	if (bMatchBasedOnAttribute)
	{
		...
		if (AnalysisDataIndex != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogPCG, Warning, "No analysis data received by copy points kernel, no points will be copied.");
