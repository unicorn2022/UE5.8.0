...
bool FPCGCreatePointsElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	if (Settings->CoordinateSpace == EPCGCoordinateSpace::LocalComponent)
	{
		LocalTransform = Context->ExecutionSource->GetExecutionState().GetTransform();
	}
	else if (Settings->CoordinateSpace == EPCGCoordinateSpace::OriginalComponent)
	{
		LocalTransform = Context->ExecutionSource->GetExecutionState().GetOriginalTransform();
	}
...
namespace PCGCreatePointsHelper
{
	UPCGData* GetDependenciesData(IPCGGraphExecutionSource* InExecutionSource, EPCGCoordinateSpace InCoordinateSpace, bool bInCullPointsOutsideVolume)
	{
		if (InExecutionSource)
		{
			...
			{
				if (IPCGGraphExecutionSource* OriginalExecutionSource = InExecutionSource ? InExecutionSource->GetExecutionState().GetOriginalSource() : nullptr)
				{
					return OriginalExecutionSource->GetExecutionState().GetSelfData();
				}
				else
				{
					return InExecutionSource->GetExecutionState().GetSelfData();
