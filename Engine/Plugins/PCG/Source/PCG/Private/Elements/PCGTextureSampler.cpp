...
EPCGChangeType UPCGTextureSamplerSettings::GetChangeTypeForProperty(const FName& InPropertyName) const
{
	EPCGChangeType ChangeType = Super::GetChangeTypeForProperty(InPropertyName);

	if (InPropertyName == GET_MEMBER_NAME_CHECKED(UPCGTextureSamplerSettings, Texture))
	{
...
TOptional<FPCGNodeThumbnailProxy> UPCGTextureSamplerSettings::GetNodeThumbnail() const
{
	if (Texture.IsNull())
	{
		return {};
	...
	return FPCGNodeThumbnailProxy::FromAssetPath(Texture.ToSoftObjectPath());
}
#endif // WITH_EDITOR
...
TArray<FPCGPinProperties> UPCGTextureSamplerSettings::OutputPinProperties() const
{
	...
	FPCGPinProperties& Pin = Properties.Emplace_GetRef(PCGPinConstants::DefaultOutputLabel, EPCGDataType::BaseTexture);
...
bool FPCGTextureSamplerElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (!Context->bTransformInitialized)
	{
		...
		{
			const FTransform Transform = Context->ExecutionSource->GetExecutionState().GetTransform();
			Context->Transform = Settings->Transform * Transform;

			const FBox Bounds = Context->ExecutionSource->GetExecutionState().GetLocalSpaceBounds();
			Context->Transform.SetScale3D(Context->Transform.GetScale3D() * Bounds.GetExtent());
		}
	...
	UPCGTexture2DSingleBaseData* BaseTextureData = nullptr;
	...
	if (UTextureRenderTarget2D* RenderTarget = Cast<UTextureRenderTarget2D>(Texture))
	{
		...
		RenderTargetData->Initialize(RenderTarget, Context->Transform);
	...
	BaseTextureData->bKeepZeroDensityPoints = Settings->bKeepZeroDensityPoints;
