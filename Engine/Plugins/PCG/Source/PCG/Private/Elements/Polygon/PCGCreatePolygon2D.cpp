...
bool FPCGCreatePolygonElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	for (const FPCGTaggedData& Input : Inputs)
	{
		...
		FVector DataNormal = Settings->DefaultPolygonNormal.GetSafeNormal(UE_SMALL_NUMBER, FVector::ZAxisVector);
		...
		if (const UPCGSplineData* SplineData = Cast<UPCGSplineData>(Input.Data))
		{
			...
			DataNormal = SplineData->GetTransform().GetUnitAxis(EAxis::Z);
		...
		double PolygonArea = PolygonTriangulation::ComputePolygonPlane(Positions, PlaneNormal, PlaneCentroid);

		// Compute bounds for the input points so we have a comparison basis vs the polygon area
		FBox PolygonBounds(EForceInit::ForceInit);

		for (const FVector& Position : Positions)
		{
			PolygonBounds += Position;
		}

		const double LargestDimension = PolygonBounds.GetSize().GetMax();
		const bool bDegeneratePolygonCase = PolygonArea <= UE_SMALL_NUMBER * FMath::Square(LargestDimension);
		...
		// If the polygon is a point/line or otherwise has a very small area, always use the provided default up axis.
		// Alternatively, use the default polygon normal based on the normal behavior and the polygon type.
		if (bDegeneratePolygonCase || 
			(Settings->DefaultPolygonNormalBehavior == EPCGCreatePolygonDefaultNormalBehavior::OnlyForOpenPolygons && bCreateOpenPolygon) ||
			Settings->DefaultPolygonNormalBehavior == EPCGCreatePolygonDefaultNormalBehavior::Always)
		{
			PlaneNormal = Settings->DefaultPolygonNormal.GetSafeNormal(UE_SMALL_NUMBER, FVector::ZAxisVector);
		}

		// Ensure normal is aligned to expectation otherwise it will break sampling, conversion to points, etc.
		if ((PlaneNormal | DataNormal) < 0)
		{
		...
		if (bCreateOpenPolygon)
		{
			...
			const UE::Geometry::EPolygonOffsetEndType EndType = PCGPolygon2DUtils::GetEndType(Settings->EndType);
			const UE::Geometry::EPolygonOffsetJoinType JoinType = PCGPolygon2DUtils::GetJoinType(Settings->JoinType);
