...
bool FPCGClipPathsElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	for (const FPCGTaggedData& Input : InputPaths)
	{
		...
		TArray<const UPCGSpatialData*> PathData;
		...
		for (const TArray<FVector2D>& ResultPath : Result)
		{
			...
			const bool bTestForEquality = Result.Num() == 1 && ResultPath[0].Equals(Positions2D[0]) && ResultPath.Last().Equals(Positions2D.Last());
			auto TestPointEquality = [&ResultPath, &Positions2D]()
			{
				for (int PointIndex = 1; PointIndex < ResultPath.Num() - 1; ++PointIndex)
				{
					if (!ResultPath[PointIndex].Equals(Positions2D[PointIndex]))
					{
						return false;
					}
				}
			...
			const bool bIsSameAsInput = bTestForEquality && TestPointEquality();

			// Special case: if the result is, in fact, the full path, return the data as-is. It'll be more efficient and will not cause issue with loop splines
			if(bIsSameAsInput)
			{
				PathData.Add(Cast<UPCGSpatialData>(Input.Data));
			}
			else if (const UPCGSplineData* InputSpline = Cast<UPCGSplineData>(Input.Data))
			{
				...
				// Another detail here: while we could jiggle the start & end, considering we already know we have an actual cut happening
				// it will not yield a loop in the end. 
				// If we discover there are some edge cases to this, we can jiggle the path start (by doing result path 0 + epsilon result path 1) + same for end,
				// which will be sufficient. We could also then test if the "whole loop" is covered too.
				...
				if (PCGSplitSpline::SplitSpline(InputSpline, SplitSplineData, StartKey, EndKey) == PCGSplitSpline::EStatus::Split)
				{
		...
		for (const UPCGSpatialData* PathDatum : PathData)
		{
