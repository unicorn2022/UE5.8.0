...
TArray<FPCGPreConfiguredSettingsInfo> UPCGPolygon2DOperationSettings::GetPreconfiguredInfo() const
{
	return FPCGPreConfiguredSettingsInfo::PopulateFromEnum<EPCGPolygonOperation>({}, FTextFormat(LOCTEXT("PreconfigureInfo", "Polygon {0}")), FTextFormat(LOCTEXT("TooltipTextFormat", "{0}")));
}
...
void UPCGPolygon2DOperationSettings::ApplyPreconfiguredSettings(const FPCGPreConfiguredSettingsInfo& PreconfiguredInfo)
{
	if (const UEnum* EnumPtr = StaticEnum<EPCGPolygonOperation>())
	{
		if (EnumPtr->IsValidEnumValue(PreconfiguredInfo.PreconfiguredIndex))
		{
			Operation = EPCGPolygonOperation(PreconfiguredInfo.PreconfiguredIndex);
		}
	}
}
