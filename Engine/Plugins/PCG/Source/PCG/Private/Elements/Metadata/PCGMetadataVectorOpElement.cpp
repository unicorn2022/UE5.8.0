...
bool UPCGMetadataVectorSettings::IsSupportedInputType(uint16 TypeId, uint32 InputIndex, bool& bHasSpecialRequirement) const
{
	...
	if (Operation == EPCGMetadataVectorOperation::EulerBetweenVectors || Operation == EPCGMetadataVectorOperation::RotatorBetweenVectors)
	{
		return PCG::Private::IsOfTypes<FVector, FVector2D>(TypeId);
	}
	else if (Operation == EPCGMetadataVectorOperation::AngleBetweenVector2D)
	{
		return PCG::Private::IsOfTypes<FVector2D>(TypeId);
	}
	else if (Operation == EPCGMetadataVectorOperation::RotateAroundAxis && InputIndex == 2)
	{
...
uint16 UPCGMetadataVectorSettings::GetOutputType(uint16 InputTypeId) const
{
	...
	if (Operation == EPCGMetadataVectorOperation::Dot ||
		...
		Operation == EPCGMetadataVectorOperation::AngleBetweenVector2D ||
	...
	else if (Operation == EPCGMetadataVectorOperation::EulerBetweenVectors)
	{
		return (uint16)EPCGMetadataTypes::Vector;
	}
	else if (Operation == EPCGMetadataVectorOperation::RotatorBetweenVectors)
	{
	...
	auto VectorFunc = [this, Operation = Settings->Operation, &OperationData]<typename AttributeType>(AttributeType) -> bool
	{
		...
		else
		{
			...
			else if (PCG::Private::IsOfTypes<double>(OperationData.OutputType))
			{
				...
				{
					return DoBinaryOp<FVector2D, FVector2D>(OperationData, [Operation](const FVector2D& Value1, const FVector2D& Value2) -> double {
							return FMath::RadiansToDegrees(FGenericPlatformMath::Atan2(Value1 ^ Value2, Value1 | Value2 ));
						});
				}
				else
				{
			...
			{
				return DoBinaryOp<FVector, FVector>(OperationData, [Operation](const FVector& Value1, const FVector& Value2) -> FVector {
						return FQuat::FindBetweenVectors(Value1, Value2).Euler();
					});
			}
			else if (Operation == EPCGMetadataVectorOperation::RotatorBetweenVectors)
			{
				return DoBinaryOp<FVector, FVector>(OperationData, [Operation](const FVector& Value1, const FVector& Value2) -> FRotator {
						return FQuat::FindBetweenVectors(Value1, Value2).Rotator();
					});
			}
			else
			{
