...
bool UPCGMetadataPartitionKernel::IsKernelDataValid(const UPCGDataBinding* InDataBinding, FPCGContext* InContext) const
{
	...
	if (InDataBinding)
	{
		...
		const TSharedPtr<const FPCGDataCollectionDesc> InputDesc = InDataBinding->GetCachedKernelPinDataDesc(this, PCGPinConstants::DefaultInputLabel, /*bIsInput=*/true);
...
TSharedPtr<const FPCGDataCollectionDesc> UPCGMetadataPartitionKernel::ComputeOutputBindingDataDesc(FName InOutputPinLabel, UPCGDataBinding* InBinding) const
{
	...
	{
		UE_LOGF(LogPCG, Verbose, "UPCGMetadataPartitionKernel: No analysis data found from count kernel.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogPCG, Error, "UPCGMetadataPartitionKernel: Number of received analysis data items (%d) does not match input data count (%d).",
	...
	for (int32 DataIndex = 0; DataIndex < AnalysisDataIndices.Num(); ++DataIndex)
	{
		...
		{
			UE_LOGF(LogPCG, Warning, "UPCGMetadataPartitionKernel: No analysis data received.");
