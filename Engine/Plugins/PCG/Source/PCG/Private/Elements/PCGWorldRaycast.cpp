...
#if WITH_EDITOR
bool UPCGWorldRaycastElementSettings::IsGPUFriendly(const FPCGPreConfiguredSettingsInfo* PreconfiguredInfo) const
{
	// Only the Line Trace preconfigured variant has a GPU-friendly backend currently. When no preconfig is provided
	// (e.g. the non-preconfigured palette entry) we report friendly, since the user may configure the instance as a line trace.
	if (!PreconfiguredInfo || PreconfiguredInfo->PreconfiguredIndex == INDEX_NONE)
	{
		return true;
	}

	return PreconfiguredInfo->PreconfiguredIndex == PCGWorldRaycastElement::Constants::Preconfiguration::LineTraceIndex;
}
...
void UPCGWorldRaycastElementSettings::CreateKernels(FPCGGPUCompilationContext& InOutContext, UObject* InObjectOuter, const UPCGNode* InNode, TArray<UPCGComputeKernel*>& OutKernels, TArray<FPCGKernelEdge>& OutEdges) const
{
	PCGKernelHelpers::FCreateKernelParams CreateParams(InObjectOuter, this, InNode);

	// Wiring is complex enough to just wire it out manually below (and node input label is not "In").
	CreateParams.NodeInputPinsToWire.Empty();
	CreateParams.NodeOutputPinsToWire.Empty();

	UPCGPreRayTraceKernel* PreKernel = PCGKernelHelpers::CreateKernel<UPCGPreRayTraceKernel>(InOutContext, CreateParams, OutKernels, OutEdges);
	UPCGPostRayTraceKernel* PostKernel = PCGKernelHelpers::CreateKernel<UPCGPostRayTraceKernel>(InOutContext, CreateParams, OutKernels, OutEdges);

	// Node input to pre-kernel which exports data collection buffer to raw buffer.
	OutEdges.Emplace(FPCGPinReference(PCGWorldRaycastElement::Constants::Labels::OriginsInput), FPCGPinReference(PreKernel, PCGPinConstants::DefaultInputLabel));

	if (RaycastMode == EPCGWorldRaycastMode::Segments)
	{
		OutEdges.Emplace(FPCGPinReference(PCGWorldRaycastElement::Constants::Labels::EndPointsInput), FPCGPinReference(PreKernel, PCGWorldRaycastElement::Constants::Labels::EndPointsInput));
		PreKernel->SetIncludeEndPointsInputPin(true);
	}

	// Node input to post-kernel so it can flow the data through (and modify positions based on RT results).
	OutEdges.Emplace(FPCGPinReference(PCGWorldRaycastElement::Constants::Labels::OriginsInput), FPCGPinReference(PostKernel, PCGPinConstants::DefaultInputLabel));

	// Ray trace results to post-kernel.
	OutEdges.Emplace(FPCGPinReference(FPCGPinReference(PreKernel, PCGPinConstants::DefaultOutputLabel)), FPCGPinReference(PostKernel, PCGPostRayTraceKernel::RayTraceResultsLabel));

	// Post kernel to node output.
	OutEdges.Emplace(FPCGPinReference(FPCGPinReference(PostKernel, PCGPinConstants::DefaultOutputLabel)), FPCGPinReference(PCGPinConstants::DefaultOutputLabel));
}
#endif // WITH_EDITOR

bool UPCGWorldRaycastElementSettings::IsPinUsedByNodeExecution(const UPCGPin* InPin) const
{
	if (InPin && ShouldExecuteOnGPU() && InPin->Properties.Label == PCGWorldRaycastElement::Constants::Labels::BoundingShape)
	{
		return false;
	}
...
TArray<FPCGPinProperties> UPCGWorldRaycastElementSettings::InputPinProperties() const
{
	...
	using namespace PCGWorldRaycastElement::Constants;
	FPCGPinProperties& OriginPointsInputPin = PinProperties.Emplace_GetRef(Labels::OriginsInput, EPCGDataType::PointOrParam, bAllowMultipleConnections);
	...
	if (RaycastMode == EPCGWorldRaycastMode::Segments)
	{
		FPCGPinProperties& EndPointsInputPin = PinProperties.Emplace_GetRef(Labels::EndPointsInput, EPCGDataType::PointOrParam, bAllowMultipleConnections);
#if WITH_EDITOR
...
bool FPCGWorldRaycastElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	Context->InitializePerIterationStates(OriginsInputData.Num(), [Context, Settings, &OriginsInputData, &EndPointsInputData, &FilterActorInputData](IterStateType& OutState, const ExecStateType&, int32 Index)
	{
		...
		if (!Settings->WorldQueryParams.ExtractLoadedObjectFiltersIfNeeded(FilterActorsData, OutState.CachedFilterObjects, Context))
		{
			return EPCGTimeSliceInitResult::NoOperation;
		}
...
bool FPCGWorldRaycastElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	return ExecuteSlice(Context, [World](ContextType* Context, const ExecStateType& ExecState, IterStateType& IterState, const uint32 IterIndex)
	{
		...
		for (int64 PointIndex = 0; PointIndex < PointCount; ++PointIndex)
		{
			...
			TOptional<FHitResult> HitResult = PCGWorldQueryHelpers::FilterRayHitResults(WorldQueryParams, Context->ExecutionSource.Get(), Hits, IterState.CachedFilterObjects);
