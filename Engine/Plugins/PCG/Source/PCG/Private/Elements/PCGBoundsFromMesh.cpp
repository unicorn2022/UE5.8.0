...
bool FPCGBoundsFromMeshElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	if (!Context->bInputsGathered)
	{
		Context->CachedInputs = Context->InputData.GetInputsByPin(PCGPinConstants::DefaultInputLabel);
		Context->bInputsGathered = true;
	}
	...
	if (!Context->bPrepareDone)
	{
		...
		else
		{
			...
			const TArray<FPCGTaggedData>& Inputs = Context->CachedInputs;
