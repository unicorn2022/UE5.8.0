...
namespace PCGDataFromActorConstants
{
	...
	static const FName BoundingShapeLabel = TEXT("BoundingShape");
...
namespace PCGDataFromActorHelpers
{
		...
		const FBox& OverlappingBounds = FBox())
	{
		...
		if (bGetLocalComponents)
		{
			...
			{
				const EPCGHiGenGrid UnscaledGrid = LocalComponent->GetUnscaledGenerationGrid();
				if (bGetAllGrids || (AllowedGrids & (int32)UnscaledGrid))
				{
...
{
	// If the input pin is connected, tracking is dynamic.
	...
	// When bounds come from the pin we can't statically restrict tracking to actor bounds, so we'll consider it fully dynamic
	const bool bBoundingShapePinConnected = Node && Node->IsInputPinConnected(PCGDataFromActorConstants::BoundingShapeLabel);
	if (bTrackActorsOnlyWithinBounds && bBoundingShapePinConnected)
	{
		return;
	}
...
TArray<FPCGPinProperties> UPCGDataFromActorSettings::InputPinProperties() const
{
	...
	FPCGPinProperties& BoundingShapePin = PinProperties.Emplace_GetRef(
		PCGDataFromActorConstants::BoundingShapeLabel,
		EPCGDataType::Spatial,
		/*bAllowMultipleConnections=*/false,
		/*bAllowMultipleData=*/false
#if WITH_EDITOR
		, LOCTEXT("BoundingShapePinTooltip", "Optional bounding shape to filter found actors by. When connected, overrides the implicit PCG actor bounds used by 'Must Overlap Self'.")
#endif
	);
	BoundingShapePin.SetAdvancedPin();
...
bool FPCGDataFromActorElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (!Context->bPerformedQuery)
	{
		...
		TArray<FPCGTaggedData> BoundingShapeInputData = Context->InputData.GetInputsByPin(PCGDataFromActorConstants::BoundingShapeLabel);
		const UPCGSpatialData* BoundingShape = !BoundingShapeInputData.IsEmpty() ? Cast<UPCGSpatialData>(BoundingShapeInputData[0].Data) : nullptr;
		const FBox Bounds = BoundingShape ? BoundingShape->GetBounds() : (Settings->ActorSelector.bMustOverlapSelf ? ExecutionSource->GetExecutionState().GetBounds() : FBox(EForceInit::ForceInit));

		TFunction<bool(const AActor*)> BoundsCheck = [](const AActor*) -> bool { return true; };
		if (BoundingShape || Settings->ActorSelector.bMustOverlapSelf)
		{
			// Capture Bounds by value because it goes out of scope
			BoundsCheck = [Bounds, ExecutionSource](const AActor* OtherActor) -> bool
			{
				const FBox OtherActorBounds = OtherActor ? PCGHelpers::GetGridBounds(OtherActor, ExecutionSource) : FBox(EForceInit::ForceInit);
				return OtherActorBounds.IsValid && Bounds.IsValid && Bounds.Intersect(OtherActorBounds);
			};
		}
		...
		{
			// @todo_pcg: RG source should support self ignore check
		...
		if (Settings->Mode == EPCGGetDataFromActorMode::GetDataFromPCGComponent && Settings->ActorSelector.ActorFilter == EPCGActorFilter::AllWorldActors)
		{
			if (UPCGSubsystem* Subsystem = Cast<UPCGSubsystem>(ExecutionSource->GetExecutionState().GetSubsystem()))
			{
				...
				if (BoundingShape || Settings->ActorSelector.bMustOverlapSelf)
				{
					for (IPCGGraphExecutionSource* CurrentExecutionSource : Subsystem->GetAllIntersectingExecutionSources(Bounds))
					{
						if (UPCGComponent* Component = Cast<UPCGComponent>(CurrentExecutionSource))
						{
				...
				{
					for (IPCGGraphExecutionSource* CurrentExecutionSource : Subsystem->GetAllRegisteredExecutionSources())
					{
						if (UPCGComponent* Component = Cast<UPCGComponent>(CurrentExecutionSource))
						{
							if (AActor* Actor = Component->GetOwner())
		...
		if (!Context->bPerformedQuery)
		{
			...
			Context->FoundActors = PCGActorSelector::FindActors(&Settings->ActorSelector, &Context->ComponentSelector, ExecutionSource, BoundsCheck, SelfIgnoreCheck, ActorsFromInput);
			Context->bPerformedQuery = true;
		}
		...
		if (Settings->ActorSelector.ActorFilter == EPCGActorFilter::FromInput)
		{
			...
			for (const TTuple<FSoftObjectPath, int32, int32>& Path : Context->PathsToObjectsAndDataIndex)
			{
				...
				DynamicTracking.AddToTracking(MoveTemp(Key), /*bIsCulled=*/Settings->bTrackActorsOnlyWithinBounds);
			}

			DynamicTracking.Finalize(Context);
		}
		else if (BoundingShape || Context->IsValueOverriden(GET_MEMBER_NAME_CHECKED(UPCGDataFromActorSettings, ActorSelector)))
		{
			...
			// If we were using a custom bounding shape for retrieval, pass it to the selection key so it's properly preserved and tested against.
			if (BoundingShape)
			{
				Key.OptionalBounds.Add(Bounds);
			}

			//@todo_pcg: if the bounding shape bounds are strictly smaller than the execution source bounds, then we technically could cull them.
			// however this wouldn't be super robust to the execution source moving either.
			const bool bIsCulled = !BoundingShape && Settings->ActorSelector.bMustOverlapSelf;
			FPCGDynamicTrackingHelper::AddSingleDynamicTrackingKey(Context, MoveTemp(Key), bIsCulled);
		}

		// Any isolated actors should also be dynamically tracked.
		if (Settings->ActorSelector.IncludeIsolatedActors != EPCGIsolatedActorIncludeFlag::Ignore)
		{
			for (AActor* FoundActor : Context->FoundActors)
			{
				if (APCGIsolatedActor* IsolatedActor = Cast<APCGIsolatedActor>(FoundActor))
				{
					FPCGDynamicTrackingHelper::AddSingleDynamicTrackingKey(Context, FPCGSelectionKey::CreateFromObjectPtr(IsolatedActor), /*bIsCulled=*/false);
				}
			}
		}
#endif // WITH_EDITOR
	...
	{
#if WITH_EDITOR
		UPCGComponent* PCGComponent = Cast<UPCGComponent>(ExecutionSource);
...
bool ShouldIgnorePCGComponent(FPCGDataFromActorContext* Context, const UPCGDataFromActorSettings* Settings, UPCGComponent* Component, const IPCGGraphExecutionSource* DownstreamOriginalSource)
{
	...
	const UObject* DownstreamOriginalOwner = DownstreamOriginalSource ? DownstreamOriginalSource->GetExecutionState().GetTarget() : nullptr;

	return !OriginalComponent
		|| OriginalComponent == DownstreamOriginalSource
		|| (Settings->ActorSelector.bIgnoreSelfAndChildren && OriginalComponent->GetExecutionState().GetTarget() == DownstreamOriginalOwner);
}
...
void FPCGDataFromActorElement::GatherWaitTasks(AActor* FoundActor, FPCGContext* InContext, TArray<FPCGTaskId>& OutWaitTasks) const
{
	...
	IPCGGraphExecutionSource* OriginalSource = Source ? Source->GetExecutionState().GetOriginalSource() : nullptr;
	...
	TInlineComponentArray<UPCGComponent*, 1> UpstreamComponents = PCGDataFromActorHelpers::GetPCGComponentsFromActor(
		FoundActor,
		Cast<UPCGSubsystem>(Source->GetExecutionState().GetSubsystem()),
		/*bGetLocalComponents=*/true,
		Settings->bGetDataOnAllGrids,
		Settings->AllowedGrids,
		Settings->bComponentsMustOverlapSelf,
		Settings->bComponentsMustOverlapSelf ? Source->GetExecutionState().GetBounds() : FBox());

	for (UPCGComponent* UpstreamComponent : UpstreamComponents)
	{
		// Avoid waiting on our own execution (including local components).
		if (ShouldIgnorePCGComponent(Context, Settings, UpstreamComponent, OriginalSource))
		{
			continue;
		}

		if (UpstreamComponent->IsGenerating())
		{
			OutWaitTasks.Add(UpstreamComponent->GetGenerationTaskId());
		}
		else if (!UpstreamComponent->bGenerated && UpstreamComponent->bActivated && (UpstreamComponent->GetSerializedEditingMode() == EPCGEditorDirtyMode::Preview) && UpstreamComponent->GetOwner())
		{
#if WITH_EDITOR
			...
			// Note: Uses owner because FPCGTrackingManager::OnPCGGraphGeneratedOrCleaned reports change on owner.
			if (UPCGComponent* OriginalComponent = Cast<UPCGComponent>(OriginalSource))
			{
				OriginalComponent->StartIgnoringChangeOriginDuringGeneration(UpstreamComponent->GetOwner());
			}
			Context->IgnoredChangeOrigins.Add(UpstreamComponent->GetOwner());
#endif

			const FPCGTaskId GenerateTask = UpstreamComponent->GenerateLocalGetTaskId(EPCGComponentGenerationTrigger::GenerateOnDemand, /*bForce=*/false);
			if (GenerateTask != InvalidPCGTaskId)
			{
				PCGGraphExecutionLogging::LogGraphScheduleDependency(UpstreamComponent, Context->GetStack());

				OutWaitTasks.Add(GenerateTask);
			}
			else
			{
				PCGGraphExecutionLogging::LogGraphScheduleDependencyFailed(UpstreamComponent, Context->GetStack());
			}
		}
	}
}
...
void FPCGDataFromActorElement::ProcessActor(FPCGContext* InContext, const UPCGDataFromActorSettings* Settings, AActor* FoundActor) const
{
	...
	if (!FoundActor || !IsValid(FoundActor) || !OriginalSource)
	{
		return;
	}

	const UObject* SourceOwner = OriginalSource->GetExecutionState().GetTarget();
	TInlineComponentArray<UPCGComponent*, 1> UpstreamComponents;
	bool bHasGeneratedPCGData = false;
	FProperty* FoundProperty = nullptr;

	if (Settings->Mode == EPCGGetDataFromActorMode::GetDataFromPCGComponent || Settings->Mode == EPCGGetDataFromActorMode::GetDataFromPCGComponentOrParseComponents)
	{
		UpstreamComponents = PCGDataFromActorHelpers::GetPCGComponentsFromActor(
			FoundActor,
			Cast<UPCGSubsystem>(Source->GetExecutionState().GetSubsystem()),
			/*bGetLocalComponents=*/true,
			Settings->bGetDataOnAllGrids,
		...
		UpstreamComponents.RemoveAllSwap([Context, Settings, OriginalSource](UPCGComponent* UpstreamComponent)
		{
			return ShouldIgnorePCGComponent(Context, Settings, UpstreamComponent, OriginalSource);
		});

		for (UPCGComponent* UpstreamComponent : UpstreamComponents)
		{
			bHasGeneratedPCGData |= !UpstreamComponent->GetGeneratedGraphOutput().TaggedData.IsEmpty();
		}
		if (APCGIsolatedActor* IsolatedActor = Cast<APCGIsolatedActor>(FoundActor))
		{
			bHasGeneratedPCGData |= !IsolatedActor->Data.GetAllInputs().IsEmpty();
		}
	}
	...
	auto GetActorDataCollection = [Source, Context, Settings](AActor* Actor, EPCGDataType DataType, bool bParseActor) -> FPCGDataCollection
	{
		FPCGGetDataFunctionRegistryParams GDFRParams;
		GDFRParams.Source = Source;
	...
	{
		for (UPCGComponent* Component : UpstreamComponents)
		{
			for (const FPCGTaggedData& TaggedData : Component->GetGeneratedGraphOutput().TaggedData)
			{
				if (ensure(TaggedData.Data))
				{
					...
					// Use stable/canonical grid size for tags.
					const EPCGHiGenGrid UnscaledGrid = Component->GetUnscaledGenerationGrid();
					DuplicatedTaggedData.Tags.Add(PCGDataFromActorConstants::PCGComponentDataGridSizeTagPrefix + FString::FromInt(PCGHiGenGrid::GridToGridSize(UnscaledGrid)));
					...
					{
						PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("InvalidComponentOutputPinPCGComponentData", "Component '{0}' on actor '{1}' has data from an unexpected output pin: '{2}'.\nMaybe the data is outdated, try regenerate this component to refresh its output data"),
		...
		// If the Actor is an Isolated actor, we also want its own data.
		if (APCGIsolatedActor* IsolatedActor = Cast<APCGIsolatedActor>(FoundActor))
		{
			for (const FPCGTaggedData& TaggedData : IsolatedActor->Data.GetAllInputs())
			{
				if (ensure(TaggedData.Data))
				{
					FPCGTaggedData& DuplicatedTaggedData = Outputs.Add_GetRef(TaggedData);
					...
					// In cases where the owner of the data (component -> owner) isn't in the main world, we MUST duplicate the data, otherwise we'll cause reference leaks in the cache.
#if WITH_EDITOR
					if (!SourceOwner || (FoundActor && FoundActor->GetLevel() != SourceOwner->GetWorld()->PersistentLevel))
					{
						DuplicatedTaggedData.Data = Cast<UPCGData>(StaticDuplicateObject(TaggedData.Data, GetTransientPackage()));
					}
#endif
					
					//todo_pcg Should we add the grid size tags from the owner'component?
					...
					if (DuplicatedTaggedData.Pin != PCGPinConstants::DefaultOutputLabel && !Settings->ExpectedPins.IsEmpty() && !Settings->ExpectedPins.Contains(TaggedData.Pin))
					{
						PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("InvalidIsolatedActorOutputPinPCGComponentData", 
							"Isolated actor '{0}' has data from an unexpected output pin: '{1}'.\nMaybe the data is outdated, try regenerate this component to refresh its output data"),
							FText::FromName(FoundActor->GetFName()),
							FText::FromName(TaggedData.Pin)), Context);
					}
				}
			}
	...
	else
	{
		...
		EPCGDataType TargetType = (Settings->Mode == EPCGGetDataFromActorMode::GetPointData ? EPCGDataType::Point : Settings->GetDataFilter());

		FPCGDataCollection ActorDataCollection = GetActorDataCollection(FoundActor, TargetType, bParseActor);
...
void FPCGDataFromActorElement::GetDependenciesCrc(const FPCGGetDependenciesCrcParams& InParams, FPCGCrc& OutCrc) const
{
	...
	if (const UPCGDataFromActorSettings* Settings = Cast<const UPCGDataFromActorSettings>(InParams.Settings))
	{
		...
		// When the bounding shape pin is connected its data is already CRC'd by the base class;
		// suppress the self-bounds CRC contribution to avoid stale invalidation when the PCG actor moves.
		const bool bBoundingShapeConnected = InParams.InputData && InParams.InputData->GetInputsByPin(PCGDataFromActorConstants::BoundingShapeLabel).Num() > 0;
		...
		const bool bDependsOnSelfBounds = Settings->ActorSelector.bMustOverlapSelf && !bBoundingShapeConnected;

		if (InParams.ExecutionSource && (bDependsOnSelfOrHierarchy || bDependsOnSelfBounds))
		{
			IPCGGraphExecutionSource* ExecutionSourceToCheck = (Settings->ActorSelector.ActorFilter == EPCGActorFilter::Original) ? InParams.ExecutionSource->GetExecutionState().GetOriginalSource() : InParams.ExecutionSource;
			const UPCGData* SelfData = ExecutionSourceToCheck ? ExecutionSourceToCheck->GetExecutionState().GetSelfData() : nullptr;
			...
			if (SelfData)
			{
				...
				if (bDependsOnSelfOrHierarchy && ExecutionSourceToCheck)
				{
					if (const AActor* Owner = ExecutionSourceToCheck->GetExecutionState().GetTypedTarget<AActor>())
					{
			...
			// Include isolated actors when relevant.
			// @todo_pcg : is this a good idea if this isn't on the GT?
			if (Settings->ActorSelector.IncludeIsolatedActors != EPCGIsolatedActorIncludeFlag::Ignore && ExecutionSourceToCheck)
			{
				TArray<APCGIsolatedActor*> IsolatedActors = UPCGActorHelpers::GetIsolatedActorsOriginatingFrom(ExecutionSourceToCheck->GetExecutionState().GetTypedTarget<AActor>());
				for (APCGIsolatedActor* IsolatedActor : IsolatedActors)
				{
					Crc.Combine(GetTypeHash(IsolatedActor->GetPathName()));
				}
			}
		}
		...
		const bool bDependsOnLocalComponentBounds = (Settings->bComponentsMustOverlapSelf || !Settings->bGetDataOnAllGrids) && !bBoundingShapeConnected;

		if (InParams.ExecutionSource && bDependsOnComponentData && bDependsOnLocalComponentBounds)
		{
			if (const UPCGData* LocalActorData = InParams.ExecutionSource->GetExecutionState().GetSelfData())
			{
