...
bool FPCGGetActorPropertyElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (Settings->ActorSelector.ActorFilter == EPCGActorFilter::FromInput)
	{
		...
		for (const TTuple<FSoftObjectPath, int32, int32>& Path : Context->PathsToObjectsAndDataIndex)
		{
			...
			DynamicTracking.AddToTracking(FPCGSelectionKey::CreateFromPath(Path.Get<0>()), /*bIsCulled=*/Settings->bTrackActorsOnlyWithinBounds);
		}
	...
	for (AActor* FoundActor : FoundActors)
	{
		...
		for (UObject* ObjectToInspect : ObjectsToInspect)
		{
			...
			PCGPropertyHelpers::FExtractorParameters Parameters(ObjectToInspect, ObjectToInspect->GetClass(), Settings->PropertyName.ToString(), OutputAttributeName, /*bPropertyNeedsToBeVisible=*/true);
			Parameters.StructExtractorBehavior = Settings->StructExtractorBehavior;
			Parameters.ContainerExtractorBehavior = Settings->ContainerExtractorBehavior;
			Parameters.ObjectExtractorBehavior = Settings->ObjectExtractorBehavior;
			...
			if (!ObjectTraversed.IsEmpty())
			{
				...
				{
					DynamicTracking.AddToTracking(FPCGSelectionKey::CreateFromPath(Path), /*bCulled=*/Settings->bTrackActorsOnlyWithinBounds);
				}
