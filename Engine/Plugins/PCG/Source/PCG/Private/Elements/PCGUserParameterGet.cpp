	...
	namespace Settings
	{
		...
		*/
		TArray<FConstStructView, TInlineAllocator<16>> GetValidLayouts(FPCGContext& InContext)
		{
			...
			TArray<FPCGTaggedData> UserParameterData = InContext.InputData.GetTaggedTypedInputs<UPCGUserParametersData>(PCG::Private::UserParameterTagData);
			...
			// We gather the outer graph of this node, to make sure it matches our interface. It must happen on the game thread
			if (!ensure(IsInGameThread()))
			{
				PCGE_LOG_C(Error, LogOnly, &InContext, LOCTEXT("NoUserParameterData", "Internal error, PCG User Parameters Data is absent and we are not executing on the game thread"));
				return {};
			}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	if (bForceObjectAndStructExtraction)
	{
		bForceObjectAndStructExtraction = false;
		StructExtractorBehavior = EPCGStructExtractorBehavior::Extract;
		ObjectExtractorBehavior = EPCGObjectExtractorBehavior::Extract;
	}
...
void UPCGUserParameterGetSettings::ApplyPreconfiguredSettings(const FPCGPreConfiguredSettingsInfo& PreconfigureInfo)
{
	...
	if (UPCGGraph* Graph = Node ? Node->GetGraph() : nullptr)
	{
		...
		{
			UE_LOGF(LogPCG, Error, "Could not create a unique graph parameter with name '%ls'. Check if the graph is valid and the name has not reached its maximum.", *PropertyName.ToString());
			return;
		}
		...
		if (FInstancedPropertyBag* PropertyBag = Graph->GetMutableUserParametersStruct_Unsafe())
		{
			...
			{
				UE_LOGF(LogPCG, Error, "Could not create a unique graph parameter with name '%ls'.", *PropertyName.ToString());
				ensure(false);
			}
		}
	}
...
bool FPCGUserParameterGetElement::CanExecuteOnlyOnMainThread(FPCGContext* Context) const
{
	// If we have no user parameter data as input, we'll have to access the parameters on the component/graph directly, which is not thread safe
	// If there is no context, we can't check the inputs, so do the assumption it is possible to run on another thread, and we'll ask to come back to
	// the main thread on execute in the unlikely possibility that the parameters are not there. 
	return Context ? Algo::NoneOf(Context->InputData.TaggedData, [](const FPCGTaggedData& InData) { return InData.Data.IsA<UPCGUserParametersData>(); }) : false;
}
...
bool FPCGUserParameterGetElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	check(Context);
	// If we have no user parameter data as input, we'll have to access the parameters on the component/graph directly, which is not thread safe
	if (CanExecuteOnlyOnMainThread(Context) && !IsInGameThread())
	{
		PCGLog::LogWarningOnGraph(LOCTEXT("ForcedBackOnMainThread", "Get Graph Parameter node is forced back on the main thread because it doesn't have UPCGUserParametersData as (implicit) input.\n"
			"If the graph was scheduled manually through the Subsystem, make sure that you provide no PreTask element, or compute the UserParameterData in it."), Context);
		Context->RequestToRunOnMainThread();
		return false;
	}
	...
	for (int CurrentLayoutIndex = 0; CurrentLayoutIndex < ValidLayouts.Num(); ++CurrentLayoutIndex)
	{
		...
		if (Settings)
		{
			...
			ExtractorParameters.StructExtractorBehavior = Settings->StructExtractorBehavior;
			ExtractorParameters.ObjectExtractorBehavior = Settings->ObjectExtractorBehavior;
			ExtractorParameters.ContainerExtractorBehavior = Settings->ContainerExtractorBehavior;
		...
		else if (GenericSettings)
		{
			...
			ExtractorParameters.StructExtractorBehavior = GenericSettings->StructExtractorBehavior;
			ExtractorParameters.ObjectExtractorBehavior = GenericSettings->ObjectExtractorBehavior;
			ExtractorParameters.ContainerExtractorBehavior = GenericSettings->ContainerExtractorBehavior;
