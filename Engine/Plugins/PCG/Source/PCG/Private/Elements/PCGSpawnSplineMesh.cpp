...
bool FPCGSpawnSplineMeshElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	if (!Context->WasLoadRequested())
	{
		...
		{
			OutState.TargetActor = Settings->TargetActor.Get() ? Settings->TargetActor.Get() : Context->GetTypedExecutionTarget<AActor>();
...
bool FPCGSpawnSplineMeshElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	return ExecuteSlice(Context, [this, Settings](ContextType* Context, const ExecStateType& ExecState, IterStateType& IterState, const uint32 IterIndex)
	{
		...
		while (IterState.ElementIndex < NumSegments)
		{
			...
			// Need to apply the transform to the tangents too, as we are working in world space (and tangents are in local space)
			const FTransform SplineTransform = SplineData->GetTransform();
			LeaveTangent = SplineTransform.TransformVector(LeaveTangent);
			ArriveTangent = SplineTransform.TransformVector(ArriveTangent);
			...
			IPCGGraphExecutionSource* ExecutionSource = Context->ExecutionSource.Get();
			check(ExecutionSource); // not likely to happen since it was tested at the beginning of the execute.

#if WITH_EDITOR
			FScopedTransaction Transaction(LOCTEXT("AddSplineMeshComponent", "Adding spline mesh component to actor from PCG"), ExecutionSource->GetExecutionState().UseTransactions());
#endif

			if (USplineMeshComponent* SplineMeshComponent = UPCGActorHelpers::GetOrCreateSplineMeshComponent(ExecState.TargetActor, ExecutionSource, IterState.SMCBuilderParams, Context))
			{
				...
				if (IterState.ComponentOverrides.IsValid())
				{
					...
					// Since the component overrides can override the component tags, we need to ensure that the default tags are still present
					if (UObject* ExecutionSourceObject = Cast<UObject>(ExecutionSource))
					{
						SplineMeshComponent->ComponentTags.AddUnique(ExecutionSourceObject->GetFName());
					}
					
					SplineMeshComponent->ComponentTags.AddUnique(PCGHelpers::DefaultPCGTag);
				}

				// Apply tags on the Spline Mesh Component
				for (const FName& Tag : Settings->TagsToAddOnComponents)
				{
					SplineMeshComponent->ComponentTags.AddUnique(Tag);
				}
