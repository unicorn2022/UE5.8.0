...
namespace PCGAttributeFilterHelpers
{
	struct ThresholdInfo
	{
		...
		bool bNeedDifferentAccessorsPerData = true;
		TArray<FPCGTaggedData> FilterData;
	...
	bool InitialPrepareThresholdInfo(FPCGContext* InContext, const TArray<FPCGTaggedData>& DataToFilter, TArray<FPCGTaggedData> FilterData, const FPCGAttributeFilterThresholdSettings& ThresholdSettings, const FName ThresholdPinLabel, ThresholdInfo& OutThresholdInfo)
	{
		if (ThresholdSettings.bUseConstantThreshold)
		{
			auto ConstantThreshold = [&OutThresholdInfo]<typename T>(const T& Value)
			{
				OutThresholdInfo.ThresholdAccessor = MakeUnique<FPCGConstantValueAccessor<T>>(Value);
				// Dummy keys
			...
			OutThresholdInfo.bNeedDifferentAccessorsPerData = false;
		}
		else
		{
			// If we have more than 1 filter data, it is a N:N operation, we need to have as many data to filter than filter data.
			// We won't create thresholds accessors as we'll need one for each data.
			if (FilterData.Num() > 1 && ThresholdSettings.bFilterEachInputWithEachFilterDataRespectively)
			{
				if (DataToFilter.Num() != FilterData.Num())
				{
					PCGLog::InputOutput::LogInvalidCardinalityError(PCGPinConstants::DefaultInputLabel, ThresholdPinLabel, InContext);
					return false;
				}
				
				OutThresholdInfo.FilterData = MoveTemp(FilterData);
				OutThresholdInfo.bUseSpatialQuery = ThresholdSettings.bUseSpatialQuery;
			}
			else if (FilterData.IsEmpty())
			{
				// We'll use the input data as filter
	...
	bool PrepareThresholdInfoFromInput(FPCGContext* InContext, const int32 DataIndex, const UPCGData* InputData, const int32 NumInput, const FPCGAttributeFilterThresholdSettings& ThresholdSettings, ThresholdInfo& InOutThresholdInfo, int16 TargetType, bool bCheckCompare, bool bCheckStringSearch, bool bWarnOnDataMissingAttribute, const ThresholdInfo* OtherInfo = nullptr)
	{
		check(InContext && InputData);

		auto CreateAccessor = [&ThresholdSettings, &InOutThresholdInfo](const UPCGData* InData)
		{
			if (InData)
			{
				const FPCGAttributePropertyInputSelector ThresholdSelector = ThresholdSettings.ThresholdAttribute.CopyAndFixLast(InData);
				InOutThresholdInfo.ThresholdAccessor = PCGAttributeAccessorHelpers::CreateConstAccessor(InData, ThresholdSelector);
				InOutThresholdInfo.ThresholdKeys = PCGAttributeAccessorHelpers::CreateConstKeys(InData, ThresholdSelector);
			}
		};
		...
		if (InOutThresholdInfo.bUseInputDataForThreshold)
		{
			...
			CreateAccessor(InputData);
		}
		else if (InOutThresholdInfo.bNeedDifferentAccessorsPerData)
		{
			check(InOutThresholdInfo.ThresholdSpatialData != nullptr || !InOutThresholdInfo.FilterData.IsEmpty());
			const UPCGData* FilterData = InOutThresholdInfo.FilterData.IsEmpty() ? InOutThresholdInfo.ThresholdSpatialData : InOutThresholdInfo.FilterData[DataIndex].Data;
			
			if (const UPCGSpatialData* SpatialData = Cast<const UPCGSpatialData>(FilterData); SpatialData && (!SpatialData->IsA<UPCGBasePointData>() || InOutThresholdInfo.bUseSpatialQuery))
			{
				InOutThresholdInfo.ThresholdSpatialData = SpatialData;
				...
				CreateAccessor(InOutThresholdInfo.ThresholdPointData);
			}
			else
			{
				// At that point, filter data can't be empty, as it would have been caught by earlier ifs
				check(InOutThresholdInfo.FilterData.IsValidIndex(DataIndex));
				CreateAccessor(InOutThresholdInfo.FilterData[DataIndex].Data);
			}
		}
...
bool FPCGAttributeFilterElementBase::DoFiltering(FPCGContext* Context, EPCGAttributeFilterOperator InOperation, const FPCGAttributePropertyInputSelector& InTargetAttribute, bool bHasSpatialToPointDeprecation, bool bWarnOnDataMissingAttribute, bool bAlwaysGenerateOutputData, const FPCGAttributeFilterThresholdSettings& FirstThreshold, const FPCGAttributeFilterThresholdSettings* SecondThreshold) const
{
	...
	const FName FirstThresholdPinLabel = SecondThreshold ? PCGAttributeFilterConstants::FilterMinLabel : PCGAttributeFilterConstants::FilterLabel;
	const FName SecondThresholdPinLabel = PCGAttributeFilterConstants::FilterMaxLabel;

	TArray<FPCGTaggedData> FirstFilterData = Context->InputData.GetInputsByPin(FirstThresholdPinLabel);
	TArray<FPCGTaggedData> SecondFilterData = SecondThreshold ? Context->InputData.GetInputsByPin(SecondThresholdPinLabel) : TArray<FPCGTaggedData>{};
	...
	if (!PCGAttributeFilterHelpers::InitialPrepareThresholdInfo(Context, DataToFilter, MoveTemp(FirstFilterData), FirstThreshold, FirstThresholdPinLabel, FirstThresholdInfo))
	{
		return true;
	}

	if (SecondThreshold && !PCGAttributeFilterHelpers::InitialPrepareThresholdInfo(Context, DataToFilter, MoveTemp(SecondFilterData), *SecondThreshold, SecondThresholdPinLabel, SecondThresholdInfo))
	{
		return true;
	}

	for (int32 i = 0; i < DataToFilter.Num(); ++i)
	{
		const FPCGTaggedData& Input = DataToFilter[i];
		...
		if (!PCGAttributeFilterHelpers::PrepareThresholdInfoFromInput(Context, i, OriginalData, NumInput, FirstThreshold, FirstThresholdInfo, TargetType, bCheckCompare, bCheckStringSearch, bWarnOnDataMissingAttribute))
		{
			ForwardInputToInFilterPin();
			continue;
		}

		if (SecondThreshold && !PCGAttributeFilterHelpers::PrepareThresholdInfoFromInput(Context, i, OriginalData, NumInput , *SecondThreshold, SecondThresholdInfo, TargetType, bCheckCompare, bCheckStringSearch, bWarnOnDataMissingAttribute, &FirstThresholdInfo))
		{
			ForwardInputToInFilterPin();
			continue;
		}
...
bool FPCGAttributeFilterElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	ThresholdSettings.bFilterEachInputWithEachFilterDataRespectively = Settings->bFilterEachInputWithEachFilterDataRespectively;
