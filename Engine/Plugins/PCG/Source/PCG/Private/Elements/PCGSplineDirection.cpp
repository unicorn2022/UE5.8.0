...
namespace PCGSplineDirection
{
	...
	UPCGSplineData* Reverse(const UPCGSplineData* InputSplineData, FPCGContext* Context)
	{
		...
		const float FirstKeyValue = ControlPointsPosition.Points.IsEmpty() ? 0.0f : ControlPointsPosition.Points[0].InVal;
		const float LastKeyValue = ControlPointsPosition.Points.IsEmpty() ? 0.0f : ControlPointsPosition.Points.Last().InVal;
		...
		for (int i = ControlPointsPosition.Points.Num() - 1; i >= 0; --i)
		{
			 ...
			 */
			const int PreviousPointIndex = (i > 0 ? i - 1 : (InputSplineData->IsClosed() ? ControlPointsPosition.Points.Num() - 1 : 0));
			ESplinePointType::Type NewControlPointType = ConvertInterpCurveModeToSplinePointType(ControlPointsPosition.Points[PreviousPointIndex].InterpMode);

			if ((NewControlPointType == ESplinePointType::Curve || NewControlPointType == ESplinePointType::CurveClamped || NewControlPointType == ESplinePointType::CurveCustomTangent))
			{
				NewControlPointType = ESplinePointType::CurveCustomTangent;
			}

			NewControlPoints.Emplace(FirstKeyValue + LastKeyValue - ControlPointsPosition.Points[i].InVal,
		...
		// Set the key offset too based on the original spline.
		if (InputSplineData->IsClosed())
		{
			NewSplineData->SplineStruct.SetLoopKeyOffset(InputSplineData->GetLoopKeyOffset());
		}
