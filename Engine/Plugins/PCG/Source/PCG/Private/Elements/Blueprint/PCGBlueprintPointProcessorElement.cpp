...
void UPCGBlueprintPointProcessorElement::PointLoop(const UPCGBasePointData* InputPointData, UPCGBasePointData*& OutputPointData, bool bGameThread)
{
	if (bGameThread && !IsInGameThread())
	{
		ThrowRequiresGameThreadException(CurrentContext, TEXT("PointLoop"));
		return;
	}
...
void UPCGBlueprintPointProcessorElement::IterationLoop(int32 NumIterations, const UPCGBasePointData* InputPointDataA, const UPCGBasePointData* InputPointDataB, UPCGBasePointData*& OutputPointData, bool bGameThread)
{
	if (bGameThread && !IsInGameThread())
	{
		ThrowRequiresGameThreadException(CurrentContext, TEXT("IterationLoop"));
		return;
	}
