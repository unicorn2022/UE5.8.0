...
#if WITH_EDITOR
FPCGAssetCachedPins UPCGBlueprintBaseElement::BuildCachedPins() const
{
	FPCGAssetCachedPins Pins;
	Pins.bHasCachedPins = true;
	Pins.InputPins = GetInputPins();
	Pins.OutputPins = GetOutputPins();

	return Pins;
}

void UPCGBlueprintBaseElement::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
...
void UPCGBlueprintBaseElement::ThrowRequiresGameThreadException(FPCGContext* InContext, const FString& InFunctionName) const
{
	const FText FormatText = FText::Format(LOCTEXT("FunctionRequiresGameThread", "Function '{0}' requires game thread and is being called outside of game thread. Make sure to enable 'bRequiresGameThread' on your Blueprint."), FText::FromString(InFunctionName));
	PCGE_LOG_C(Error, GraphAndLog, InContext, FormatText);
	UPCGBlueprintHelpers::ThrowBlueprintException(FormatText);
}
