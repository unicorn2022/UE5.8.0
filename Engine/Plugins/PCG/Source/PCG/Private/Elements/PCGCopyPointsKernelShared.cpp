...
bool PCGCopyPointsKernel::IsKernelDataValid(const UPCGComputeKernel* InKernel, const UPCGDataBinding* InDataBinding, FPCGContext* InContext)
{
	check(InKernel);

	if (ensure(InDataBinding))
	{
		const TSharedPtr<const FPCGDataCollectionDesc> SourcePinDesc = InDataBinding->GetCachedKernelPinDataDesc(InKernel, PCGCopyPointsConstants::SourcePointsLabel, /*bIsInputPin=*/true);
		...
		const TSharedPtr<const FPCGDataCollectionDesc> TargetPinDesc = InDataBinding->GetCachedKernelPinDataDesc(InKernel, PCGCopyPointsConstants::TargetPointsLabel, /*bIsInputPin=*/true);
		...
		const FPCGKernelParams* KernelParams = InDataBinding->GetCachedKernelParams(InKernel);
