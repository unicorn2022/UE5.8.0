...
void UPCGBaseSubgraphSettings::GetStaticTrackedKeys(FPCGSelectionKeyToSettingsMap& OutKeysToSettings, TArray<TObjectPtr<const UPCGGraph>>& VisitedGraphs) const
{
	...
	{
		Subgraph->GetTrackedSelectionKeysToSettings(OutKeysToSettings, VisitedGraphs);
	}
}
...
TArray<FPCGSettingsOverridableParam> UPCGBaseSubgraphSettings::GatherOverridableParams() const
{
	...
	if (PCGGraph)
	{
		if (const FInstancedPropertyBag* UserParameterStruct = PCGGraph->GetUserParametersStruct())
		{
			if (const UPropertyBag* ScriptStruct = UserParameterStruct->GetPropertyBagStruct())
			{
				PCGSettingsHelpers::FPCGGetAllOverridableParamsConfig Config;
				Config.bExcludeSuperProperties = true;
				Config.ShouldKeepPropertyFunc = [](const FProperty* InProperty, int32) { return InProperty && !InProperty->HasAnyPropertyFlags(CPF_DisableEditOnInstance); };
				Config.bExtractArrays = true;
				Config.MaxContainersNum = 1; // Do not support more than 1 container deep.

				TArray<FPCGSettingsOverridableParam> ParamsFromGraph = PCGSettingsHelpers::GetAllOverridableParams(ScriptStruct, Config);

				// Gather order of properties from the hierarchy root, if any.
				if (UHierarchyRoot* HierarchyRoot = Cast<UHierarchyRoot>(PCGGraph->GetUserParameterHierarchyRoot()); HierarchyRoot && !ParamsFromGraph.IsEmpty())
				{
					TArray<FHierarchyElementIdentity> PropsIds;
					PropsIds.Reserve(ParamsFromGraph.Num());

					for(const FPCGSettingsOverridableParam& Param : ParamsFromGraph)
					{
						const FPropertyBagPropertyDesc* PropDesc = ScriptStruct->FindPropertyDescByProperty(Param.Properties.Last());
						PropsIds.Add(PropDesc ? FHierarchyElementIdentity({ PropDesc->ID }, {}) : FHierarchyElementIdentity());
					}

					// Push overridable params in order of the hierarchy
					TArray<UHierarchyItem*> AllItems;
					HierarchyRoot->GetChildrenOfType<UHierarchyItem>(AllItems, /*bRecursive=*/true);

					for (UHierarchyItem* Item : AllItems)
					{
						check(Item);
						FHierarchyElementIdentity ItemIdentity = Item->GetPersistentIdentity();
						const int32 PropIndex = PropsIds.IndexOfByKey(ItemIdentity);
						if (PropIndex != INDEX_NONE)
						{
							OverridableParams.Add(MoveTemp(ParamsFromGraph[PropIndex]));
							ParamsFromGraph.RemoveAtSwap(PropIndex);
							PropsIds.RemoveAtSwap(PropIndex);
						}
					}
				}

				// Any remaining properties that weren't found from the hierarchy, push them in the current order
				OverridableParams.Append(ParamsFromGraph);
			}
		}
	}
...
FString UPCGSubgraphSettings::GetAdditionalTitleInformation() const
{
	...
	{
		return TargetSubgraph->GetDisplayName().ToString();
	}
	else
	{
...
{
	TObjectPtr<UPCGBaseSubgraphSettings> Settings = Cast<UPCGBaseSubgraphSettings>(GetSettings());
	return Settings ? Settings->GetSubgraphInterface() : nullptr;
}

FStructView FPCGSubgraphContext::GetExternalStructContainerForOverridableParam(const FPCGSettingsOverridableParam& InParam)
{
	...
	{
		return GraphInstanceParametersOverride;
	}
	else
	{
		return nullptr;
...
void FPCGSubgraphContext::UpdateOverridesWithOverriddenGraph()
{
	...
	{
		// In the case of subgraph override, and the subgraph override is different from the one setup on the node, the settings might not have 
		// all the overrides. If the "Overrides" pin is also connected, first make a pass on it to setup the graph overrides.
		// This will be super-seeded by pin overrides connected. 
		const UPCGSubgraphSettings* SubgraphSettings = GetInputSettings<UPCGSubgraphSettings>();
		const bool bHasSubgraphOverride = SubgraphSettings && SubgraphSettings->SubgraphOverride;
		const UPCGGraph* RootSubgraph = SubgraphSettings && SubgraphSettings->SubgraphInstance ? SubgraphSettings->SubgraphInstance->GetGraph() : nullptr;
		const bool bHasDifferentSubgraphOverride = bHasSubgraphOverride && SubgraphSettings->SubgraphOverride->GetGraph() != RootSubgraph;
		
		if (bHasDifferentSubgraphOverride && Node && Node->IsInputPinConnected(PCGPinConstants::DefaultParamsLabel))
		{
			const TArray<FPCGTaggedData> OverrideData = InputData.GetParamsByPin(PCGPinConstants::DefaultParamsLabel);
			if (OverrideData.Num() > 1)
			{
				PCGLog::InputOutput::LogFirstInputOnlyWarning(PCGPinConstants::DefaultParamsLabel, this);
			}
			
			if (!OverrideData.IsEmpty())
			{
				void* ParametersPtr = GraphInstanceParametersOverride.GetMutableMemory();
				PCGPropertyHelpers::ExtractAttributeSetToContainers(
					CastChecked<UPCGParamData>(OverrideData[0].Data),
					GraphInstanceParametersOverride.GetScriptStruct(),
					MakeArrayView<void*>(&ParametersPtr, 1),
					/*OptionalNameMapping=*/nullptr,
					this);
			}
		}
...
void FPCGSubgraphElement::PrepareSubgraphData(const UPCGBaseSubgraphSettings* Settings, FPCGSubgraphContext* Context, const FPCGDataCollection& InputData, FPCGDataCollection& OutputData) const
{
...
void FPCGSubgraphElement::PrepareSubgraphUserParameters(const UPCGBaseSubgraphSettings* Settings, FPCGSubgraphContext* Context, FPCGDataCollection& OutputData) const
{
	...
	if (const UPCGGraphInterface* SubgraphInterface = Settings->GetSubgraphInterface())
	{
		...
		TArray<FPCGTaggedData> UpstreamUserParameterData = Context->InputData.GetTaggedTypedInputs<UPCGUserParametersData>(PCG::Private::UserParameterTagData);
...
{
	const UPCGBaseSubgraphSettings* Settings = Cast<UPCGBaseSubgraphSettings>(InSettings);
...
bool FPCGSubgraphElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	if (bIsDynamic && !Context->bScheduledSubgraph && Settings->GetSubgraphOverride())
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FPCGSubgraphElement::ExecuteInternal::DynamicTrackingAndOverrides);

#if WITH_EDITOR
		FPCGDynamicTrackingHelper::AddSingleDynamicTrackingKey(Context, FPCGSelectionKey::CreateFromPath(Settings->GetSubgraphOverride()), /*bIsCulled=*/false);
		// Also need to add statically tracked keys
		if (InContext && InContext->ExecutionSource.IsValid())
		{
			if (const UPCGGraph* Graph = Settings->GetSubgraphOverride()->GetGraph())
			{
				InContext->ExecutionSource->GetExecutionState().RegisterDynamicTracking(Graph->GetTrackedSelectionKeysToSettings());
			}
		}
#endif // WITH_EDITOR
