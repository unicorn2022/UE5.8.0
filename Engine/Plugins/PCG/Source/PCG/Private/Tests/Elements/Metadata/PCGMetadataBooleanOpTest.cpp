...
struct FPCGMetadataBooleanOpTestState
{
...
protected:
	static FPCGMetadataBooleanOpTestState Init()
	{
		FPCGMetadataBooleanOpTestState State;

		State.ParamData1 = PCGTestsCommon::CreateEmptyParamData();
		State.ParamData2 = PCGTestsCommon::CreateEmptyParamData();

		constexpr bool bAllowInterpolation = false;
		constexpr bool bOverrideParent = false;

		State.ParamData1->Metadata->CreateAttribute<bool>(TEXT("True"), /*DefaultValue=*/ true, bAllowInterpolation, bOverrideParent);
		State.ParamData1->Metadata->CreateAttribute<bool>(TEXT("False"), /*DefaultValue=*/ false, bAllowInterpolation, bOverrideParent);
		State.ParamData1->Metadata->CreateAttribute<int64>(TEXT("Int0"), /*DefaultValue=*/ 0, bAllowInterpolation, bOverrideParent);
		State.ParamData1->Metadata->CreateAttribute<int64>(TEXT("Int1"), /*DefaultValue=*/ 1, bAllowInterpolation, bOverrideParent);
		State.ParamData1->Metadata->CreateAttribute<FVector>(TEXT("Invalid"), /*DefaultValue=*/ FVector::ZeroVector, bAllowInterpolation, bOverrideParent);
		State.ParamData1->Metadata->AddEntry();

		State.ParamData2->Metadata->CreateAttribute<bool>(TEXT("True"), /*DefaultValue=*/ true, bAllowInterpolation, bOverrideParent);
		State.ParamData2->Metadata->CreateAttribute<bool>(TEXT("False"), /*DefaultValue=*/ false, bAllowInterpolation, bOverrideParent);
		State.ParamData2->Metadata->CreateAttribute<int64>(TEXT("Int0"), /*DefaultValue=*/ 0, bAllowInterpolation, bOverrideParent);
		State.ParamData2->Metadata->CreateAttribute<int64>(TEXT("Int1"), /*DefaultValue=*/ 1, bAllowInterpolation, bOverrideParent);
		State.ParamData2->Metadata->CreateAttribute<FVector>(TEXT("Invalid"), /*DefaultValue=*/ FVector::ZeroVector, bAllowInterpolation, bOverrideParent);
		State.ParamData2->Metadata->AddEntry();

		State.Settings = NewObject<UPCGMetadataBooleanSettings>();
	...
	{
		FPCGMetadataBooleanOpTestState State = Init();

		PCGTestsCommon::FTestData TestData;
		TestData.Reset(State.Settings);
		...
		ParamTaggedData1.Data = State.ParamData1;
		...
		ParamTaggedData2.Data = State.ParamData2;
		ParamTaggedData2.Pin = PCGMetadataSettingsBaseConstants::DoubleInputSecondLabel;

		State.Settings->Operation = Op;
		State.Settings->InputSource1.SetAttributeName(Attr1);
		State.Settings->InputSource2.SetAttributeName(Attr2);
		State.Settings->OutputTarget.SetAttributeName(OutputAttributeName);
		State.Settings->ForceOutputConnections[0] = true;
		...
		const FPCGMetadataAttribute<bool>* OutAttribute = OutMetadata->GetConstTypedAttribute<bool>(State.Settings->OutputTarget.GetName());
	...
	bool Run(const EPCGMetadataBooleanOperation Op, const FName Attr1, const FName Attr2, const bool bExpectedResult, bool bIsValid = true)
	{
		if (!RunInternal(Op, Attr1, Attr2, bExpectedResult, bIsValid))
		{
			...
			UE_LOGF(LogPCG, Error, "%ls %ls %ls == %ls failed", *Attr1.ToString(), *OpName.ToString(), *Attr2.ToString(), bExpectedResult ? TEXT("true") : TEXT("false"));
			return false;
		}
		else
		{
