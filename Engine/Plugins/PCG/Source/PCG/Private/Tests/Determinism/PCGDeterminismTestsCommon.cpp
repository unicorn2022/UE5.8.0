...
namespace PCGDeterminismTests
{
	...
	{
		UE_LOGF(LogPCG, Warning, "Attempting to run an invalid determinism test");
	...
	void RunDeterminismTest(const UPCGGraph* InPCGGraph, const IPCGGraphExecutionSource* InSource, FDeterminismTestResult& OutResult)
	{
		...
		for (const TTuple<FPCGTaskId, TTuple<const UPCGNode*, const FPCGDataCollection>>& TaskOutputMapping : FirstIntermediateOutputArray)
		{
			...
			UE_LOGF(LogPCG, Log, "[%ls] Evaluating Node [%ls]...", *InPCGGraph->GetName(), *NodeNameString);
			...
			if (HighestDeterminismLevel == EDeterminismLevel::OrderIndependent)
			{
				if (!DataCollectionsAreIdentical(FirstOutput, SecondOutput))
				{
					...
					UE_LOGF(LogPCG, Log, "[%ls] Node failed \"independent\" test. Downgrading to \"consistent\"", *NodeNameString);
			...
			if (HighestDeterminismLevel == EDeterminismLevel::OrderConsistent)
			{
				if (!DataCollectionsAreConsistent(FirstOutput, SecondOutput, FirstOutput.TaggedData.Num()))
				{
					...
					UE_LOGF(LogPCG, Log, "[%ls] Node failed \"consistent\" test. Downgrading to \"orthogonal\"", *NodeNameString);
			...
			if (HighestDeterminismLevel == EDeterminismLevel::OrderOrthogonal)
			{
				if (!DataCollectionsContainSameData(FirstOutput, SecondOutput))
				{
					...
					UE_LOGF(LogPCG, Log, "[%ls] Node failed \"orthogonal\" test. Test failed.", *NodeNameString);
	...
	void AddRandomizedInputData(PCGTestsCommon::FTestData& TestData, FPCGDataTypeIdentifier DataType, const FName& PinName)
	{
		...
		{
			UE_LOGF(LogPCG, Error, "Specific type required to add to test data");
		}
	}
	...
	bool SurfaceDataIsIdentical(const UPCGData* FirstData, const UPCGData* SecondData)
	{
		...
		UE_LOGF(LogPCG, Warning, "Surface comparison not fully implemented.");
		...
		for (const FName& AttributeName : FirstAttributeNames)
		{
			...
			for (PCGMetadataValueKey ValueKey = 0; ValueKey < FirstNumValueKeys; ++ValueKey)
			{
				...
				case PCG::Private::MetadataTypes<FTransform>::Id:
					...
					break;
				default:
					UE_LOGF(LogPCG, Warning, "Invalid type to compare for metadata being identical: %d", FirstAttributeBase->GetTypeId());
					break;
				}
	...
	{
		// Comparable data types (Param and Spatial)
		const FPCGDataTypeIdentifier ValidTypes = FPCGDataTypeIdentifier::Construct(FPCGDataTypeInfoParam::AsId(), FPCGDataTypeInfoSpatial::AsId());
		if (DataType.Intersects(ValidTypes))
		{
			return true;
		}

		UE_LOGF(LogPCG, Warning, "Unknown determinism data comparison type: %ls", *DataType.ToString());
		return false;
	}
	...
	TArray<FPCGDataTypeIdentifier> FilterTestableDataTypes(FPCGDataTypeIdentifier AllowedTypes, int32 NumMultipleInputs)
	{
		...
		if (!(AllowedTypes & EPCGDataType::Spatial))
		{
			...
			UE_LOGF(LogPCG, Error, "Only spatial data types are currently supported");
	...
	TFunction<bool(const UPCGData*, const UPCGData*)> GetDataCompareFunction(FPCGDataTypeIdentifier DataType, EDeterminismLevel DeterminismLevel)
	{
		if (!DataTypeIsComparable(DataType))
		{
			...
			UE_LOGF(LogPCG, Warning, "Attempting to compare incomparable data.");
			return ComparisonIsUnimplemented;
		}

		if (DataType.IsSameType(FPCGDataTypeInfoParam::AsId()))
		{
			return ParamDataIsIdentical;
		}

		if (DataType.IsChildOf(FPCGDataTypeInfoSpatial::AsId()))
		{
			switch (DeterminismLevel)
			{
				...
				default:
					UE_LOGF(LogPCG, Warning, "Comparable PCG DataType has no 'data comparison function'.");
					return ComparisonIsUnimplemented;
			}
		}

		UE_LOGF(LogPCG, Warning, "Can not compare non-spatial or non-param data: %ls", *DataType.ToString());
		return ComparisonIsUnimplemented;
	}
	...
	TFunction<bool(const UPCGData*, const UPCGData*)> GetDataCollectionCompareFunction(FPCGDataTypeIdentifier DataType)
	{
		...
		if (!DataTypeIsComparable(DataType) || !DataType.IsChildOf(FPCGDataTypeInfoSpatial::AsId()))
		{
			// Should never reach here
			UE_LOGF(LogPCG, Warning, "Attempting to compare incomparable data.");
			return ComparisonIsUnimplemented;
