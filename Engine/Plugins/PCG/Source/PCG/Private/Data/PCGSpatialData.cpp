...
const UPCGBasePointData* UPCGSpatialData::ToBasePointDataWithContext(const FPCGBlueprintContextHandle& ContextHandle) const
{
	...
	UE_LOGF(LogPCG, Error, "UPCGSpatialData::ToBasePointDataWithContext:: Invalid context handle");
	return nullptr;
}
...
const UPCGBasePointData* FPCGPointDataCache::ToBasePointDataInternal(FPCGContext* Context, const FBox& InBounds, bool bSupportsBoundedPointData, PCG::FLock& InCacheLock, TFunctionRef<const UPCGBasePointData*(FPCGContext*, const FBox&)> CreatePointDataFunc)
{
	if (InBounds.IsValid && bSupportsBoundedPointData)
	{
		const UPCGBasePointData* BoundedPointData = nullptr;
		{
			PCG::TScopeLock ScopeLock(InCacheLock);
...
UPCGSpatialData* UPCGSpatialData::ProjectOn(FPCGContext* InContext, const UPCGSpatialData* InOther, const FPCGProjectionParams& InParams) const
{
	...
	if (!InOther)
	{
		UE_LOGF(LogPCG, Warning, "No projection target specified, no projection will occur");
		return DuplicateData(InContext);
	}

	if (GetDimension() > InOther->GetDimension())
	{
		UE_LOGF(LogPCG, Error, "Dimension of projection source (%d) must be less than or equal to that of the projection target (%d)", GetDimension(), InOther->GetDimension());
		return DuplicateData(InContext);
	}
	...
	if (!ConcreteTarget)
	{
		UE_LOGF(LogPCG, Error, "Could not find a concrete shape in the target data to project onto.");
		return DuplicateData(InContext);
	}
...
UPCGMetadata* UPCGSpatialData::CreateEmptyMetadata()
{
	if (Metadata)
	{
		UE_LOGF(LogPCG, Warning, "Spatial data already had metadata");
	}
...
void UPCGSpatialData::InitializeFromData(const UPCGSpatialData* InSource, const UPCGMetadata* InMetadataParentOverride, bool bInheritMetadata, bool bInheritAttributes)
{
	...
	if (InSource)
	{
		if (InMetadataParentOverride)
		{
			...
			UE_LOGF(LogPCG, Warning, "MetadataParentOverride is deprecated and should not be used anymore. The source data is now responsible to initialize it.");
