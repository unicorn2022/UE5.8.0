	...
	TUniquePtr<IPCGAttributeAccessor> CreatePropertyAccessor(EPCGPolygon2DProperties Property, bool bQuiet, bool bConst)
	{
		if (bConst)
		{
			...
			default:
				if (!bQuiet)
				{
					UE_LOGF(LogPCG, Error, "EPCGPolygon2DProperties value '%d' does not exist.", EnumToUnderlyingType(Property));
				}
		...
		else
		{
			...
			case EPCGPolygon2DProperties::LocalRotation:
				if (!bQuiet)
				{
					UE_LOGF(LogPCG, Error, "EPCGPolygon2DProperties value '%d' is read-only.", EnumToUnderlyingType(Property));
				}
				return TUniquePtr<IPCGAttributeAccessor>();
			default:
				if (!bQuiet)
	...
	TUniquePtr<IPCGAttributeAccessor> CreateDataPropertyAccessor(EPCGPolygon2DDataProperties DataProperty, bool bQuiet, bool bConst)
	{
		if (bConst)
		{
			...
			default:
				if (!bQuiet)
				{
					UE_LOGF(LogPCG, Error, "EPCGPolygon2DDataProperties value '%d' does not exist.", EnumToUnderlyingType(DataProperty));
				}
				return TUniquePtr<IPCGAttributeAccessor>();
			}
		}
...
void UPCGPolygon2DData::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	Super::GetResourceSizeEx(CumulativeResourceSize);

	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(
		sizeof(UE::Geometry::FGeneralPolygon2d) +
		Polygon.GetAllocatedSize() +
		sizeof(Transform) +
		VertexEntryKeys.GetAllocatedSize() +
		SegmentIndexToSegmentAndHoleIndices.GetAllocatedSize() +
		SegmentAndHoleIndicesToSegmentIndex.GetAllocatedSize() +
		sizeof(int));
}
