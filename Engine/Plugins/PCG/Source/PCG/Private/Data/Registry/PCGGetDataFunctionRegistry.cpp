...
#define LOCTEXT_NAMESPACE "PCGGetDataFunctionRegistry"
...
int FPCGGetDataFunctionRegistry::DefaultDataFromActor(FPCGContext* InContext, const FPCGGetDataFunctionRegistryParams& InParams, AActor* Actor, FPCGGetDataFunctionRegistryOutput& Output) const
{
	...
	const IPCGGraphExecutionSource* Source = InParams.Source;
	...
	else if (InDataFilter == EPCGDataType::Point)
	{
		// Implementation note: when we're asking explicitly for point data, this behaves like the 'level to asset' workflow
		// but operates on all primitive components (not just SMCs).
		...
		FPCGMetadataAttribute<FSoftObjectPath>* MeshAttribute = PointMetadata->CreateAttribute<FSoftObjectPath>(PCGLevelToAssetConstants::MeshAttributeName, FSoftObjectPath(), /*bAllowsInterpolation=*/false, /*bOverrideParent=*/true);
		FPCGMetadataAttribute<FSoftObjectPath>* SkeletalMeshAttribute = PointMetadata->CreateAttribute<FSoftObjectPath>(PCGLevelToAssetConstants::SkeletalMeshAttributeName, FSoftObjectPath(), /*bAllowsInterpolation=*/false, /*bOverrideParent=*/true);
		FPCGMetadataAttribute<FSoftObjectPath>* MaterialAttribute = PointMetadata->CreateAttribute<FSoftObjectPath>(PCGLevelToAssetConstants::MaterialAttributeName, FSoftObjectPath(), /*bAllowsInterpolation=*/false, /*bOverrideParent=*/true);
		FPCGMetadataAttribute<FSoftObjectPath>* ActorReferenceAttribute = PointMetadata->CreateAttribute<FSoftObjectPath>(PCGAttributeNameConstants::ActorReferenceAttribute, FSoftObjectPath(), /*bAllowsInterpolation=*/false, /*bOverrideParent=*/true);
		FPCGMetadataAttribute<FSoftObjectPath>* ComponentReferenceAttribute = PointMetadata->CreateAttribute<FSoftObjectPath>(PCGAttributeNameConstants::ComponentReferenceAttribute, FSoftObjectPath(), /*bAllowsInterpolation=*/false, /*bOverrideParent=*/true);
		...
		Context.ActorReferenceAttribute = ActorReferenceAttribute;
		Context.ComponentReferenceAttribute = ComponentReferenceAttribute;
		Context.TagToAttributeMap = &TagToAttributeMap;
		Context.SanitizedAttributeNames = &SanitizedAttributeNames;
		Context.bOnlyMeshComponents = false;
		Context.bIgnorePCGCreatedComponents = InParams.bIgnorePCGGeneratedComponents;

		if (Context.ParseActorComponents(Actor) == 0)
		{
			// Fallback on standard single point per actor path
			PointData->AddSinglePointFromActor(Actor, &Output.bSanitizedTagAttributeNames);
		}
	...
	{
		// If we have a Partition Actor, the execution source must be a PCG Component.
		const UPCGComponent* Component = Cast<UPCGComponent>(Source);

		// Invalid processing otherwise because of the this usage
		if (!Component || Component->GetOwner() != Actor)
		{
			PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("InvalidComponent", "Could not get data from actor {0}, execution source is invalid."), FText::FromString(PartitionActor->GetActorNameOrLabel())), InContext);
			return 0;
		}
	...
	else if (LandscapeActor)
	{
		...
		const UPCGGraph* PCGGraph = Source ? Source->GetExecutionState().GetGraph() : nullptr;
		...
		Data->Initialize({ LandscapeActor }, PCGHelpers::GetGridBounds(Actor, Source), LandscapeDataProps);
