...
void UPCGSplineData::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	Super::GetResourceSizeEx(CumulativeResourceSize);

	// Lightweight - skip
	if (FPCGGraphCache::CacheSizeDetailLevel() == 2)
	{
		return;
	}

	FArchiveCountMem Archive(this);
	FPCGSplineStruct::StaticStruct()->SerializeItem(FStructuredArchiveFromArchive(Archive).GetSlot(), &SplineStruct, /*Defaults*/nullptr);
	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(Archive.GetNum() + sizeof(FBox));
}
...
float UPCGSplineData::GetInputKeyAtAlpha(float InAlpha) const
{
	// Alpha is a measure of progression along the spline, where segments have an equal portion.
	const float Alpha = FMath::Clamp(InAlpha, 0.0f, 1.0f);
	const int NumSegments = GetNumSegments();

	const float UnnormalizedAlpha = Alpha * NumSegments;
	const int SegmentIndex = static_cast<int>(UnnormalizedAlpha);
	const float Remainder = UnnormalizedAlpha - static_cast<float>(SegmentIndex);

	return GetInputKeyAtDistance(SegmentIndex, Remainder * GetSegmentLength(SegmentIndex));
}
...
float UPCGSplineData::GetLoopKeyOffset() const
{
	return SplineStruct.GetLoopKeyOffset();
}
...
TUniquePtr<IPCGAttributeAccessor> UPCGSplineData::CreateStaticAccessor(const FPCGAttributePropertySelector& InSelector, bool bQuiet)
{
	...
	else if (InSelector.GetSelection() == EPCGAttributePropertySelection::Property && DomainName == PCGDataConstants::DataDomainName)
	{
		...
		{
			return MakeUnique<FPCGSplineAccessorV2<FTransform, EPCGSplineDataProperties::SplineTransform>>(PCGSplineStructProperty);
		}
		else if (PropertyName == TEXT("IsClosed"))
		{
			return MakeUnique<FPCGSplineAccessorV2<bool, EPCGSplineDataProperties::IsClosed>>(PCGSplineStructProperty);
		}
		else if (PropertyName == TEXT("Length"))
		{
			return MakeUnique<FPCGSplineAccessorV2<double, EPCGSplineDataProperties::Length>>(PCGSplineStructProperty);
		}
	}
...
const UPCGBasePointData* UPCGSplineData::CreateBasePointData(FPCGContext* Context, TSubclassOf<UPCGBasePointData> PointDataClass) const
{
	...
	UE_LOGF(LogPCG, Verbose, "Spline generated %d points", Data->GetNumPoints());
...
TArray<FSplinePoint> UPCGSplineData::GetSplinePoints() const
{
	...
	{
		ControlPoints.Emplace(ControlPointsPosition.Points[i].InVal,
