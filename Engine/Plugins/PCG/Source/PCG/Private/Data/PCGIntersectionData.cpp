...
UPCGBasePointData* UPCGIntersectionData::CreateAndFilterPointData(FPCGContext* Context, const UPCGSpatialData* X, const UPCGSpatialData* Y, TSubclassOf<UPCGBasePointData> PointDataClass) const
{
	...
	if (!SourcePointData)
	{
		UE_LOGF(LogPCG, Error, "Intersection unable to get source points");
		return nullptr;
	}
	...
	UE_LOGF(LogPCG, Verbose, "Intersection generated %d points from %d source points", OutputData->GetNumPoints(), SourcePointData->GetNumPoints());
