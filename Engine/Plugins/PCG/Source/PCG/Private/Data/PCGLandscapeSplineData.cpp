...
bool UPCGLandscapeSplineData::CheckSpline() const
{
	if (!Spline.IsValid())
	{
		...
		{
			UE_LOGF(LogPCG, Warning, "Invalid landscape spline component");
...
const UPCGBasePointData* UPCGLandscapeSplineData::CreateBasePointData(FPCGContext* Context, TSubclassOf<UPCGBasePointData> PointDataClass) const
{
	...
	if (CheckSpline())
	{
		UE_LOGF(LogPCG, Verbose, "Landscape spline %ls generated %d points", *Spline->GetFName().ToString(), Data->GetNumPoints());
	}
