...
namespace PCGBasePointData
{
	TUniquePtr<IPCGAttributeAccessor> CreatePropertyAccessor(UPCGBasePointData* PointData, EPCGPointNativeProperties NativeProperty, bool bQuiet)
	{
		...
		default:
			UE_LOGF(LogPCG, Error, "EPCGPointNativeProperty value '%d' does not exist.", EnumToUnderlyingType(NativeProperty));
	...
	TArray<typename TPCGPointNativeProperty<PointProperty>::Type> GetInputRangeValues(const FPCGPointInputRange& Range)
	{
		if (!Range.PointData)
		{
			UE_LOGF(LogPCG, Error, "GetInputRangeValues '%ls' called with invalid point data.", *StaticEnum<EPCGPointNativeProperties>()->GetNameStringByValue((int64)PointProperty));
			return {};
		}
		...
		{
			UE_LOGF(LogPCG, Error, "GetInputRangeValues '%ls' called with input range [%d, %d[ when valid range is [0, %d].", *StaticEnum<EPCGPointNativeProperties>()->GetNameStringByValue((int64)PointProperty), Range.RangeStartIndex, Range.RangeSize, Range.PointData->GetNumPoints()-1);
			return {};
		}
	...
	void SetOutputRangeValues(FPCGPointOutputRange& Range, const TArray<typename TPCGPointNativeProperty<PointProperty>::Type>& Values)
	{
		...
		{
			UE_LOGF(LogPCG, Error, "SetOutputRangeValues '%ls' called with invalid point data.", *StaticEnum<EPCGPointNativeProperties>()->GetNameStringByValue((int64)PointProperty));
			return;
		}

		if (!EnumHasAnyFlags(Range.PointData->GetAllocatedProperties(/*bWithInheritance=*/false), PointProperty))
		{
			UE_LOGF(LogPCG, Error, "SetOutputRangeValues '%ls' called on non-allocated property.", *StaticEnum<EPCGPointNativeProperties>()->GetNameStringByValue((int64)PointProperty));
			return;
		}
		...
		if (!PCGBasePointData::IsValidRange<PointProperty>(Range, ValueRange))
		{
			UE_LOGF(LogPCG, Error, "SetOutputRangeValues '%ls' called with output range [%d, %d[ when valid range is [0, %d].", *StaticEnum<EPCGPointNativeProperties>()->GetNameStringByValue((int64)PointProperty), Range.RangeStartIndex, Range.RangeSize, Range.PointData->GetNumPoints()-1);
			return;
		}

		if (Values.Num() > Range.RangeSize)
		{
			UE_LOGF(LogPCG, Error, "SetOutputRangeValues '%ls' called with invalid values array size '%d' when max array size '%d'.", *StaticEnum<EPCGPointNativeProperties>()->GetNameStringByValue((int64)PointProperty), Values.Num(), Range.RangeSize);
			return;
		}
...
{
	PCG::TScopeLock Lock(CachedDataLock);
	if (!bBoundsAreDirty)
	{
		return;
	}
...
{
	PCG::TScopeLock Lock(CachedDataLock);
	if (!bOctreeIsDirty && PCGPointOctree.IsValid())
	{
		return;
	}
	...
	const bool bHasInvalidBounds = PointBounds.ContainsNaN();

	if (bHasInvalidBounds)
	{
		UE_LOGF(LogPCG, Error, "Invalid bounds in the point data makes it impossible to create a valid Octree.");
		PointBounds = FBox(EForceInit::ForceInit);
	}
	...
	PCGPointOctree = MakeUnique<PCGPointOctree::FPointOctree>(MoveTemp(NewOctree));
...
void UPCGBasePointData::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	...
	const SIZE_T OctreeSize = PCGPointOctree.IsValid() ? sizeof(*PCGPointOctree) + PCGPointOctree->GetSizeBytes() : 0;

	CumulativeResourceSize.AddDedicatedSystemMemoryBytes(OctreeSize + sizeof(Bounds));
}
...
bool UPCGBasePointData::ProjectPoint(const FTransform& InTransform, const FBox& InBounds, const FPCGProjectionParams& InParams, FPCGPoint& OutPoint, UPCGMetadata* OutMetadata, bool bUseBounds) const
{
	...
	// Can only compute the metadata if the output metadata supports elements.
	if (FPCGMetadataDomain* OutDomain = OutMetadata ? OutMetadata->GetMetadataDomain(PCGMetadataDomainID::Elements) : nullptr)
	{
		//TRACE_CPUPROFILER_EVENT_SCOPE(UPCGBasePointData::SamplePoint::SetupMetadata);
		// Initialize metadata entry for this temporary point
		const FPCGMetadataDomain* ThisDomain = ConstMetadata() ? ConstMetadata()->GetConstMetadataDomain(PCGMetadataDomainID::Elements) : nullptr;
		OutPoint.MetadataEntry = OutDomain->HasParent(ThisDomain) ? OutDomain->AddEntry(MetadataEntryRange[MaxContributor]) : OutDomain->AddEntry();
		
		if (ContributionsForMetadata.Num() > 1)
		{
			OutDomain->ComputeWeightedAttribute(OutPoint.MetadataEntry, MakeArrayView(ContributionsForMetadata), ThisDomain);
		}
	}
	else
	{
...
FPCGPoint UPCGBasePointData::GetPointFromRange(const FPCGPointInputRange& InputRange, int32 Index)
{
	...
	{
		UE_LOGF(LogPCG, Error, "GetPointFromRange called with invalid InputRange RangeStartIndex '%d'.", InputRange.RangeStartIndex);
		return FPCGPoint();
	}

	if (Index < 0 || Index >= InputRange.RangeSize)
	{
		UE_LOGF(LogPCG, Error, "GetPointFromRange called with invalid Index '%d' when valid InputRange is [0, %d].", Index, InputRange.RangeSize - 1);
		return FPCGPoint();
	}

	if (!InputRange.PointData)
	{
		UE_LOGF(LogPCG, Error, "GetPointFromRange called with invalid InputRange PointData.");
		return FPCGPoint();
	}
	...
	{
		UE_LOGF(LogPCG, Error, "GetPointFromRange called with invalid index '%d' when valid range is [0, %d].", FinalIndex, InputRange.PointData->GetNumPoints()-1);
		return FPCGPoint();
	}
...
void UPCGBasePointData::SetPointOnRange(UPARAM(ref) FPCGPointOutputRange& OutputRange, int32 Index, const FPCGPoint& Point)
{
	...
	{
		UE_LOGF(LogPCG, Error, "SetPointOnRange called with invalid OutputRange RangeStartIndex '%d'.", OutputRange.RangeStartIndex);
		return;
	}

	if (Index < 0 || Index >= OutputRange.RangeSize)
	{
		UE_LOGF(LogPCG, Error, "SetPointOnRange called with invalid Index '%d' when valid OutputRange is [0, %d].", Index, OutputRange.RangeSize-1);
		return;
	}

	if (!OutputRange.PointData)
	{
		UE_LOGF(LogPCG, Error, "SetPointOnRange called with invalid OutputRange PointData.");
		return;
	}
	...
	{
		UE_LOGF(LogPCG, Error, "SetPointOnRange called with invalid index '%d' when valid range is [0, %d].", FinalIndex, OutputRange.PointData->GetNumPoints()-1);
		return;
	}
