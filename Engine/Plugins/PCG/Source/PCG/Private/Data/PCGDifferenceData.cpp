...
const UPCGBasePointData* UPCGDifferenceData::CreateBasePointData(FPCGContext* Context, const UPCGBasePointData* SourcePointData, TSubclassOf<UPCGBasePointData> PointDataClass) const
{
	if (!SourcePointData)
	{
		UE_LOGF(LogPCG, Error, "Difference unable to get source points");
	...
	if (!DifferenceData)
	{
		UE_LOGF(LogPCG, Verbose, "Difference is trivial");
		return SourcePointData;
	}
	...
	UE_LOGF(LogPCG, Verbose, "Difference generated %d points from %d source points", TargetPointData->GetNumPoints(), SourcePointData->GetNumPoints());
