...
namespace PCGObjectHash
{
	...
	FAutoConsoleCommand CalculatePCGObjectHashCommand(
		*GCalculatePCGObjectHashCommandName,
		...
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& InArgs)
		{
			if (InArgs.Num() < 1)
			{
				UE_LOGF(LogPCG, Warning, "%ls : Requires at least 1 argument. First argument should be the object path.", *GCalculatePCGObjectHashCommandName);
				return;
			}
			...
			if (!ResolvedObject)
			{
				UE_LOGF(LogPCG, Warning, "%ls : Could not find object: '%ls'", *GCalculatePCGObjectHashCommandName, *SoftObjectPath.ToString());
				return;
			}
			...
			{
				UE_LOGF(LogPCG, Warning, "%ls : Unsupported object: '%ls'", *GCalculatePCGObjectHashCommandName, *SoftObjectPath.ToString());
			}
		}));
	...
	protected:
		...
		virtual bool ShouldSkipProperty(const FProperty* InProperty) const override
		{
			...
			{
				UE_LOGF(LogPCG, Log, "[PCGHash] %ls  Property skipped: '%ls' (Type: '%ls' - Object : '%ls')", FCString::Tab(Level), *InProperty->GetName(), *CurrentObject->GetClass()->GetName(), *CurrentObject->GetPathName());
			}
					...
					*this << VisitingIndex;
					...
					else
					{
						...
						*this << ObjectReferencePath;
						...
						{
							UE_LOGF(LogPCG, Log, "[PCGHash] %ls  Object skipped: '%ls' (Type: '%ls')", FCString::Tab(Level), *InObjectReference->GetPathName(), *InObjectReference->GetClass()->GetName());
						}
					}
				}
				else
...
FPCGObjectHash::FHash FPCGObjectHash::GetHashInternal(bool bInVerbose, uint32 InLevel) const
{
	...
	if (bShouldLog)
	{
		UE_LOGF(LogPCG, Log, "[PCGHash] %lsBegin Hash: Object: '%ls' (Type: '%ls')", FCString::Tab(InLevel), *Context->GetObject()->GetPathName(), *Context->GetObject()->GetClass()->GetName());
	}
	...
	if (bShouldLog)
	{
		...
		UE_LOGF(LogPCG, Log, "[PCGHash] %lsEnd Hash: Object: '%ls' -> Hash: '%ls' took '%ls' seconds", FCString::Tab(InLevel), *Context->GetObject()->GetPathName(), *LexToString(FinalHash.Hash), *FText::AsNumber(HashTime).ToString());
	}
