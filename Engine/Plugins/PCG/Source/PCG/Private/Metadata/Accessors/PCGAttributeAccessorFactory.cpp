	...
	TUniquePtr<IPCGAttributeAccessor> CreateDefaultAccessor(UPCGData* InData, const FPCGAttributePropertySelector& InSelector, const bool bQuiet)
	{
		...
		if (InSelector.GetSelection() == EPCGAttributePropertySelection::ExtraProperty)
		{
			...
			if (!Accessor.IsValid())
			{
				if (!bQuiet)
				{
					UE_LOGF(LogPCG, Error, "[FPCGAttributeAccessorFactory::DefaultAccessor] Expected to select an extra property '%ls' but the data doesn't support it or is only constant.", *InSelector.GetName().ToString());
				}
		...
		if (InSelector.GetSelection() == EPCGAttributePropertySelection::ExtraProperty)
		{
			if (InSelector.GetExtraProperty() == EPCGExtraProperties::NumElements)
			{
				if (InSelector.GetDomainName() != PCGDataConstants::DataDomainName || !InData->IsSupportedMetadataDomainID(PCGMetadataDomainID::Elements))
				{
					if (!bQuiet)
					{
						UE_LOGF(LogPCG, Error, "[FPCGAttributeAccessorFactory::DefaultConstAccessor] Num elements is only supported on @Data domain and for data that have elements.");
					}
			...
			if (!Accessor.IsValid())
			{
				if (!bQuiet)
				{
					UE_LOGF(LogPCG, Error, "[FPCGAttributeAccessorFactory::DefaultAccessor] Expected to select an extra property '%ls' but the data doesn't support it.", *InSelector.GetName().ToString());
				}
	...
	const FPCGAttributePropertySelector& InSelector, const bool bQuiet) const
{
	if (!InData)
	{
		if (!bQuiet)
		{
			UE_LOGF(LogPCG, Error, "[FPCGAttributeAccessorFactory::CreateSimpleKeys] Can't create keys with no input data.");
		}
	...
	const FPCGAttributePropertySelector& InSelector, const bool bQuiet) const
{
	if (!InData)
	{
		if (!bQuiet)
		{
			UE_LOGF(LogPCG, Error, "[FPCGAttributeAccessorFactory::CreateSimpleConstKeys] Can't create keys with no input data.");
		}
...
void FPCGAttributeAccessorFactory::RegisterMethods(TSubclassOf<UPCGData> PCGDataClass, FPCGAttributeAccessorMethods&& InAccessorMethods)
{
	...
	{
		UE_LOGF(LogPCG, Error, "Trying to register %ls accessor methods multiple times, will be ignored.", *PCGDataClass->GetName());
		return;
	}
