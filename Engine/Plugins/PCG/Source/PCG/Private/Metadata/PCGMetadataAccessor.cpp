...
namespace PCGMetadataAccessorHelpers
{
	...
	static const FText InvalidValue = LOCTEXT("InvalidValue", "[PCG] Value is invalid.");
	static const FTextFormat InvalidAttribute = LOCTEXT("InvalidAttribute", "[PCG] Could not find attribute {0} in metadata");
	static const FText InvalidValueType = LOCTEXT("InvalidValueType", "[PCG] Value is not of a valid type.");
	...
	bool SetAttributeValue(PCGMetadataEntryKey& OutKey, FPCGMetadataDomain* Metadata, const FName AttributeName, const FProperty* ValueProperty, const void* ValuePtr)
	{
		if (!Metadata)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(LOCTEXT("InvalidMetadataOrConst", "[PCG] Metadata is invalid or const."));
			return false;
		}
	
		if (!ValueProperty || !ValuePtr)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(InvalidValue);
			return false;
		}
		...
		if (!BaseAttribute)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(FText::Format(InvalidAttribute, FText::FromName(AttributeName)));
			return false;
		}
	
		TUniquePtr<const IPCGAttributeAccessor> PropertyAccessor = PCGAttributeAccessorHelpers::CreatePropertyAccessor(ValueProperty);
		if (!PropertyAccessor)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(InvalidValueType);
			return false;
		}
	
		TUniquePtr<IPCGAttributeAccessor> AttributeAccessor = PCGAttributeAccessorHelpers::CreateAccessor(BaseAttribute, Metadata);
		check(AttributeAccessor);
	
		FPCGAttributeAccessorKeysEntries AttributeKeys(MakeArrayView(&OutKey, 1));
		const FPCGAttributeAccessorKeysGenericPtrs PropertyKeys(MakeArrayView<const void*>(&ValuePtr, 1));

		const bool bSuccess = PropertyAccessor->CopyTo(PropertyKeys, *AttributeAccessor, AttributeKeys, /*Index=*/0, /*Count=*/1, EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible);
		if (!bSuccess)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(FText::Format(LOCTEXT("InvalidOpTypeWrite", "[PCG] Cannot write value ({0}) into the attribute {1} ({2})"), 
				FText::FromString(PropertyAccessor->GetUnderlyingDesc().GetTypeString()), FText::FromName(AttributeName), FText::FromString(AttributeAccessor->GetUnderlyingDesc().GetTypeString())));
		}
		
		return bSuccess;
	}
	
	bool GetAttributeValue(PCGMetadataEntryKey InKey, const FPCGMetadataDomain* Metadata, const FName AttributeName, const FProperty* ValueProperty, void* ValuePtr)
	{
		if (!Metadata)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(LOCTEXT("InvalidMetadata", "[PCG] Metadata is invalid"));
			return false;
		}
		...
		const FPCGMetadataAttributeBase* BaseAttribute = Metadata->GetConstAttribute(AttributeName);
		if (!BaseAttribute)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(FText::Format(InvalidAttribute, FText::FromName(AttributeName)));
			return false;
		...
		TUniquePtr<IPCGAttributeAccessor> PropertyAccessor = PCGAttributeAccessorHelpers::CreatePropertyAccessor(ValueProperty);
		if (!PropertyAccessor)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(InvalidValueType);
			return false;
		...
		TUniquePtr<const IPCGAttributeAccessor> AttributeAccessor = PCGAttributeAccessorHelpers::CreateConstAccessor(BaseAttribute, Metadata);
		check(AttributeAccessor);
	
		const FPCGAttributeAccessorKeysEntries AttributeKeys(MakeArrayView(&InKey, 1));
		FPCGAttributeAccessorKeysGenericPtrs PropertyKeys(MakeArrayView<void*>(&ValuePtr, 1));

		const bool bSuccess = AttributeAccessor->CopyTo(AttributeKeys, *PropertyAccessor, PropertyKeys, /*Index=*/0, /*Count=*/1, EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible);
		if (!bSuccess)
		{
			UPCGBlueprintHelpers::ThrowBlueprintException(FText::Format(LOCTEXT("InvalidOpTypeRead", "[PCG] Cannot read value ({0}) from the attribute {1} ({2})"), 
				FText::FromString(PropertyAccessor->GetUnderlyingDesc().GetTypeString()), FText::FromName(AttributeName), FText::FromString(AttributeAccessor->GetUnderlyingDesc().GetTypeString())));
		}
...
namespace PCGMetadataAccessorHelpers
{
	// Since the code differ only from the key and metadata, factorize all the code in this template function
	// KeyType will be either be PCGMetadataEntryKey& (Set) or PCGMetadataEntryKey (Get)
	// MetadataType will be either be FPCGMetadataDomain (Set) or const FPCGMetadataDomain (Get)
	template <typename KeyType, typename MetadataType>
	void GetSetAttributeValue(KeyType Key, MetadataType* Metadata, UObject* Context, FFrame& Stack, RESULT_DECL)
	{
		...
		static_assert(std::is_const_v<MetadataType> != std::is_reference_v<KeyType>);
		
		if constexpr (std::is_const_v<MetadataType>)
		{
			*static_cast<bool*>(RESULT_PARAM) = PCGMetadataAccessorHelpers::GetAttributeValue(Key, Metadata, Name, TargetProperty, TargetPtr);
		}
		else
		{
			*static_cast<bool*>(RESULT_PARAM) = PCGMetadataAccessorHelpers::SetAttributeValue(Key, Metadata, Name, TargetProperty, TargetPtr);
		}
...
DEFINE_FUNCTION(UPCGMetadataAccessorHelpers::execSetAttributeValueByMetadataKey)
{
	P_GET_OBJECT(UPCGMetadata, Metadata)
	P_GET_PROPERTY_REF(FInt64Property, Key)

	PCGMetadataAccessorHelpers::GetSetAttributeValue<PCGMetadataEntryKey&, FPCGMetadataDomain>(Key, Metadata ? Metadata->GetDefaultMetadataDomain() : nullptr, Context, Stack, RESULT_PARAM);
}

DEFINE_FUNCTION(UPCGMetadataAccessorHelpers::execGetAttributeValueByMetadataKey)
{
	P_GET_OBJECT(const UPCGMetadata, Metadata)
	P_GET_PROPERTY(FInt64Property, Key)
	
	PCGMetadataAccessorHelpers::GetSetAttributeValue<PCGMetadataEntryKey, const FPCGMetadataDomain>(Key, Metadata ? Metadata->GetConstDefaultMetadataDomain() : nullptr, Context, Stack, RESULT_PARAM);
}

DEFINE_FUNCTION(UPCGMetadataAccessorHelpers::execSetAttributeValue)
{
	P_GET_OBJECT(UPCGMetadata, Metadata)
	P_GET_STRUCT_REF(FPCGPoint, Point)
	
	PCGMetadataAccessorHelpers::GetSetAttributeValue<PCGMetadataEntryKey&, FPCGMetadataDomain>(Point.MetadataEntry, Metadata ? Metadata->GetDefaultMetadataDomain() : nullptr, Context, Stack, RESULT_PARAM);
}

DEFINE_FUNCTION(UPCGMetadataAccessorHelpers::execGetAttributeValue)
{
	P_GET_OBJECT(const UPCGMetadata, Metadata)
	P_GET_STRUCT_REF(FPCGPoint, Point)
	
	PCGMetadataAccessorHelpers::GetSetAttributeValue<PCGMetadataEntryKey, const FPCGMetadataDomain>(Point.MetadataEntry, Metadata ? Metadata->GetConstDefaultMetadataDomain() : nullptr, Context, Stack, RESULT_PARAM);
}

DEFINE_FUNCTION(UPCGMetadataAccessorHelpers::execSetAttributeValueByMetadataKeyOnDomain)
{
	P_GET_STRUCT(FPCGMetadataDomainHandle, MetadataDomain)
	P_GET_PROPERTY_REF(FInt64Property, Key)
	
	PCGMetadataAccessorHelpers::GetSetAttributeValue<PCGMetadataEntryKey&, FPCGMetadataDomain>(Key, MetadataDomain.GetMutableDomain(), Context, Stack, RESULT_PARAM);
}

DEFINE_FUNCTION(UPCGMetadataAccessorHelpers::execGetAttributeValueByMetadataKeyOnDomain)
{
	P_GET_STRUCT(FPCGMetadataDomainHandle, MetadataDomain)
	P_GET_PROPERTY(FInt64Property, Key)

	PCGMetadataAccessorHelpers::GetSetAttributeValue<PCGMetadataEntryKey, const FPCGMetadataDomain>(Key, MetadataDomain.GetConstDomain(), Context, Stack, RESULT_PARAM);
}
