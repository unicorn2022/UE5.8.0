...
decltype(auto) UPCGMetadata::WithMetadataDomain(const FPCGMetadataDomainID& InMetadataDomainID, Func InFunc, Args&& ...InArgs)
{
	...
	{
		UE_LOGF(LogPCG, Error, "Failed to find MetadataDomain with id %ls", *InMetadataDomainID.DebugName.ToString());
...
decltype(auto) UPCGMetadata::WithMetadataDomain_Lambda(const FPCGMetadataDomainID& InMetadataDomainID, Func InFunc)
{
	...
	{
		UE_LOGF(LogPCG, Error, "Failed to find domain with id %ls", *InMetadataDomainID.DebugName.ToString());
...
FPCGMetadataDomain* UPCGMetadata::FindOrCreateMetadataDomain(const FPCGMetadataDomainID& InMetadataDomainID)
{
	...
	{
		PCG::TScopeLock Lock(MetadataDomainsLock);
...
void UPCGMetadata::SetupDomain(FPCGMetadataDomainID DomainID, bool bIsDefault)
{
	...
	if (bIsDefault && !DomainID.IsDefault())
	{
		...
		// Make sure to update the cached value for allow for multiple entries, as it might not have been setup correctly.
		DefaultIDLayer->bSupportMultiEntries = MetadataDomainSupportsMultiEntries(DomainID);
	}
	else
	{
...
void UPCGMetadata::Serialize(FArchive& InArchive)
{
	...
	if (InArchive.IsLoading())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogPCG, Warning, "Mismatch between default metadata domains while loading. "
								 "You should make sure to update the serialized metadata with the new default. "
								 "Serialized domain: %ls ; Current domain: %ls",
...
void UPCGMetadata::GetResourceSizeEx(FResourceSizeEx& CumulativeResourceSize)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UPCGMetadata::GetResourceSizeEx);
	Super::GetResourceSizeEx(CumulativeResourceSize);

	// Note: we will not gather memory from the parent, but only "local" memory here
	// This might report as not quite all the memory we need (in the context of the cache, for example)
	// but it will not also grossly over-evalute the memory needed.
	// Gather from domains
	ForEachValidUniqueConstMetadataDomain([&CumulativeResourceSize](FPCGMetadataDomainID DomainID, const FPCGMetadataDomain& Domain)
	{
		Domain.GetResourceSizeEx(CumulativeResourceSize);
	});
}

FPCGMetadataAttributeBase* UPCGMetadata::CreateAttribute(FPCGAttributeIdentifier AttributeName, const FPCGMetadataAttributeDesc& AttributeDesc, bool bAllowsInterpolation, bool bOverrideParent)
{
	FPCGMetadataDomain* FoundSubMetadata = FindOrCreateMetadataDomain(AttributeName.MetadataDomain);
	return FoundSubMetadata ? FoundSubMetadata->CreateAttribute(AttributeDesc, bAllowsInterpolation, bOverrideParent) : nullptr;
}
...
void UPCGMetadata::InitializeAsCopy(const FPCGMetadataInitializeParams& InParams)
{
	...
	if (Parent)
	{
		UE_LOGF(LogPCG, Error, "Metadata has already been initialized.");
		return;
	}
	...
	InParams.Parent->ForEachValidUniqueConstMetadataDomain([this, &bShouldParent, &InParams](const FPCGMetadataDomainID& DomainID, const FPCGMetadataDomain& OtherMetadataDomain)
	{
		auto InitializeFunc = [&bShouldParent, &InParams](FPCGMetadataDomain& CurrentMetadataDomain, const FPCGMetadataDomainInitializeParams& DomainParams)
		{
			...
			{
				CurrentMetadataDomain.Parent = nullptr;
			}
		};
...
FPCGMetadataAttributeBase* UPCGMetadata::CopyAttribute(const FPCGMetadataAttributeBase* OriginalAttribute, FPCGAttributeIdentifier NewAttributeName, bool bKeepParent, bool bCopyEntries, bool bCopyValues)
{
	...
	checkSlow((RootMetadata && RootMetadata->MetadataDomains.Contains(OriginalMetadataDomain->DomainID) && RootMetadata->MetadataDomains[OriginalMetadataDomain->DomainID].Get() == OriginalMetadataDomain->GetRoot()) || !bKeepParent);
#endif
	...
	FPCGMetadataAttributeBase* Attribute = WithMetadataDomain_Lambda(NewAttributeName.MetadataDomain, [&](FPCGMetadataDomain* MetadataDomain) -> FPCGMetadataAttributeBase*
	{
		...
		{
			// UE_LOGF(LogPCG, Error, "[Metadata - CopyAttribute] Can't copy across domains.");
			// return nullptr;
		}
	...
	if (!Attribute)
	{
		UE_LOGF(LogPCG, Error, "[Metadata - CopyAttribute] Metadata domain does not exist in current metadata or copy failed.");
	}
...
bool UPCGMetadata::ChangeAttributeType(FPCGAttributeIdentifier AttributeName, const FPCGMetadataAttributeDesc& AttributeNewDesc)
{
	// Need to differentiate overloads.
	return WithMetadataDomain(AttributeName.MetadataDomain, static_cast<bool(FPCGMetadataDomain::*)(FName, const FPCGMetadataAttributeDesc&)>(&FPCGMetadataDomain::ChangeAttributeType), AttributeName.Name, AttributeNewDesc);
}
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	// Need to differentiate overloads.
	return WithMetadataDomain(AttributeName.MetadataDomain, static_cast<bool(FPCGMetadataDomain::*)(FName, int16)>(&FPCGMetadataDomain::ChangeAttributeType), AttributeName.Name, AttributeNewType);
...
FPCGMetadataDomainHandle UPCGMetadata::BP_GetMetadataDomain(const FPCGMetadataDomainID& InMetadataDomainID)
{
	FPCGMetadataDomainHandle Result{};
	if (FindOrCreateMetadataDomain(InMetadataDomainID) != nullptr)
	{
		Result.bConst = false;
		Result.DomainHandle = MetadataDomains[InMetadataDomainID].ToWeakPtr();
	}
	
	return Result;
}

FPCGMetadataDomainHandle UPCGMetadata::BP_GetConstMetadataDomain(const FPCGMetadataDomainID& InMetadataDomainID) const
{
	FPCGMetadataDomainHandle Result{};
	if (const TSharedPtr<FPCGMetadataDomain>* Domain = MetadataDomains.Find(InMetadataDomainID))
	{
		Result.bConst = true;
		Result.DomainHandle = Domain->ToWeakPtr();
	}
