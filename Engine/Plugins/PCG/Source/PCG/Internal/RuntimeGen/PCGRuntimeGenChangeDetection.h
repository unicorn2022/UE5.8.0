...
{
	using FDetectionInputsBase = TTuple</*OriginalExecutionSource*/TObjectKey<UObject>, const IPCGGenSourceBase*, FPCGRuntimeGenerationRadii, /*RadiusMultiplier*/float, /*UseGenSourceDirection*/bool, /*MinGridSize*/uint32>;

	/** The key used for the state cache. */
	struct FDetectionInputs : FDetectionInputsBase
	{
		FDetectionInputs(const IPCGGraphExecutionSource* InOriginalExecutionSource, const IPCGGenSourceBase* InGenSource, const FPCGRuntimeGenerationRadii& InGenerationRadii, float InRadiusMultiplier, bool bInUseGenSourceDirection, uint32 InMinGridSize)
			: FDetectionInputsBase(Cast<UObject>(InOriginalExecutionSource), InGenSource, InGenerationRadii, InRadiusMultiplier, bInUseGenSourceDirection, InMinGridSize)
		{}

		const IPCGGraphExecutionSource* GetOriginalExecutionSource() const { return Cast<IPCGGraphExecutionSource>(Get<0>().ResolveObjectPtr()); }
	...
	struct FDetector
	{
		...
		void FlushCachedState(const IPCGGraphExecutionSource* InOptionalOriginalExecutionSource = nullptr);
