...
void UPCGOctreeQueries::ForEachPointInsideSphere(const UPCGBasePointData* InPointData, const FVector& InCenter, const double InRadius, Func Callback)
{
	...
	const FPCGPointTransform::ConstValueRange TransformValueRange = InPointData->GetConstTransformValueRange();
		
	InPointData->GetPointOctree().FindElementsWithBoundsTest(SearchBounds, [InPointData, Points, &TransformValueRange, &InCenter, SquaredRadius, Callback = std::move(Callback)](const PCGPointOctree::FPointRef& PointRef)
	{
		...
		const double SquaredDistance = FVector::DistSquared(InCenter, TransformValueRange[PointRef.Index].GetLocation());
