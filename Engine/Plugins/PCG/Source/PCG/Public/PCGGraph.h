...
struct FPCGGraphDocumentInfo
{
	...
	/** Saved view position */
	UPROPERTY()
	FVector2f SavedViewOffset = FVector2f::ZeroVector;

	/** Saved zoom amount */
	UPROPERTY()
	float SavedZoomAmount = -1.0f;

	FPCGGraphDocumentInfo() = default;

	FPCGGraphDocumentInfo(UObject* InEditedObject, const FVector2f& InSavedViewOffset, float InSavedZoomAmount)
	...
	friend bool operator==(const FPCGGraphDocumentInfo& LHS, const FPCGGraphDocumentInfo& RHS)
	{
...
public:
	...
	/** Construct a data that will contain a copy of the UserParameters, in order to freeze them while executing a graph. */
	UE_API UPCGUserParametersData* ConstructUserParametersData(FPCGContext* InContext) const;
	...
	UE_API static EPCGGraphUsage GetGraphUsage(const FAssetData& InAssetData);

	UFUNCTION()
	UE_API bool IsStandaloneGraph() const;

	UE_API EPCGGraphUsage GetGraphUsage() const;

	UFUNCTION()
	UE_API virtual UObject* GetUserParameterHierarchyRoot() const; 
protected:
...
public:
	...
	/** Set of documents that were being edited in this pcg graph, so we can open them right away */
	UPROPERTY()
	TArray<FPCGGraphDocumentInfo> LastEditedDocuments;
#endif
...
public:
	...
	virtual TSubclassOf<UPCGGraph> GetEmbeddedSubgraphClass() const { return UPCGGraph::StaticClass(); }

	UE_API UPCGGraph* AddNewEmbeddedSubgraph();
	UE_API void DeleteEmbeddedSubgraph(UPCGGraph* InEmbeddedSubgraph);
#endif
	UE_API bool ContainsEmbeddedSubgraph(UPCGGraph* InEmbeddedSubgraph) const;
	UE_API void DuplicateEmbeddedSubgraphs(const TArray<UPCGGraph*>& InEmbeddedSubgraphs, TMap<UPCGGraph*, UPCGGraph*>& OutDuplicatedGraphs);

	virtual bool SupportsEmbeddedSubgraphs() const { return !IsEmbeddedSubgraph(); }
	UPCGGraph* GetEmbeddedParentGraph() const { return GetTypedOuter<UPCGGraph>(); }

	UFUNCTION()
	bool IsEmbeddedSubgraph() const { return !!GetEmbeddedParentGraph(); }
	
	const TArray<TObjectPtr<UPCGGraph>> GetEmbeddedSubgraphs() { return EmbeddedSubgraphs; }
...
#if WITH_EDITOR
	virtual UObject* GetUserParameterHierarchyRoot() const override { return UserParameterHierarchyRoot; }
#endif
	/** ~End UPCGGraphInterface interface */

	/** Default grid for generation. For hierarchical generation, nodes outside of grid size graph ranges will generate on this grid. */
	UE_API EPCGHiGenGrid GetDefaultGrid() const;
	...
	UE_API bool IsHierarchicalGenerationEnabled() const;
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = AssetInfo, AssetRegistrySearchable, meta=(Multiline, PCGNoHash))
	FText Description;

	/** Marks the graph to be not refreshed automatically when the landscape changes, even if it is used. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Settings|Advanced", meta=(PCGNoHash, EditCondition="!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = AssetInfo, AssetRegistrySearchable, meta=(PCGNoHash, EditCondition="ShowToolData()", EditConditionHides))
	FPCGGraphToolData ToolData;

	/** Controls whether a graph can be executed outside of the context of a PCG actor/entity, e.g. run on for asset or level workflows. */
	...
	EPCGGraphUsage GraphUsageContext = EPCGGraphUsage::Standard;

	/** When enabled, this graph can be executed outside of the world using an editor execution source. */
	UE_DEPRECATED(5.8, "Use GraphUsageContext instead.")
	UPROPERTY(BlueprintReadWrite, Category = AssetInfo, meta = (PCGNoHash, DeprecatedProperty, DeprecationMessage = "Deprecated in favor of GraphUsageContext"))
	bool bIsStandaloneGraph = false;
#endif

	UPROPERTY(EditAnywhere, Category = "Settings|Advanced", meta=(EditCondition="!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	...
	/** Returns the default graph input pins (from the input node), which exclude the parameter pins, execution dependency pin, etc. */
	UE_API TArray<FPCGPinProperties> DefaultInputPinProperties() const;

	/** Returns the default graph output pins (from the input node), which exclude the parameter pins, execution dependency pin, etc. */
	UE_API TArray<FPCGPinProperties> DefaultOutputPinProperties() const;

	/** Returns all the edges of the graph */
	UFUNCTION(BlueprintCallable, Category = Graph)
	UE_API TArray<UPCGEdge*> GetAllEdges() const;
	...
	/** Returns multiplier on grid size. */
	double GetGridSizeMultiplier() const { return PCGHiGenGrid::SanitizeGridSizeMultiplier(HiGenGridSizeMultiplier); }

	/** Gets generation radius from grid, considering grid size multiplier. */
	UE_API double GetGridGenerationRadiusFromGrid(uint32 Grid) const;

	/** Gets cleanup radius from grid, considering grid size multiplier. */
	UE_API double GetGridCleanupRadiusFromGrid(uint32 Grid) const;

	/** Allows executing runtime PCG graphs without partition actors and local PCG components. */
	bool UseActorComponentlessGeneration() const;

#if WITH_EDITOR
	UPCGEditorGraph* GetEditorGraph() const { return PCGEditorGraph; }
	...
	UE_API void EnableInspection(const FPCGStack& InInspectedStack);
	...
	UE_API bool IsSameInspectedStack(const FPCGStack& InOtherStack) const;

	UE_DEPRECATED(5.8, "Compare using the IsSameInspectedStack method.")
	UE_API FPCGStack GetInspectedStack() const;
	...
	UE_API const TArray<TSubclassOf<UObject>>& GetChangeTrackingIgnoredClasses() const;
#endif

#if WITH_EDITOR
	UE_API FPCGSelectionKeyToSettingsMap GetTrackedSelectionKeysToSettings() const;
	UE_API void GetTrackedSelectionKeysToSettings(FPCGSelectionKeyToSettingsMap& OutKeysToSettings, TArray<TObjectPtr<const UPCGGraph>>& OutVisitedGraphs) const;
	UE_API bool UpdateTrackedSelectionKeys(FPCGSelectionKeyToSettingsMap& InExistingKeys, FPCGSelectionKeyToSettingsMap& InOutUpdatedKeys, TArray<FPCGSelectionKey>* OptionalChangedKeys) const;

	UE_DEPRECATED(5.8, "Use GetTrackedSelectionKeysToSettings instead")
	FPCGSelectionKeyToSettingsMap GetTrackedActorKeysToSettings() const { return GetTrackedSelectionKeysToSettings(); }

	UE_DEPRECATED(5.8, "Use GetTrackedSelectionKeysToSettings instead")
	void GetTrackedActorKeysToSettings(FPCGSelectionKeyToSettingsMap& OutKeysToSettings, TArray<TObjectPtr<const UPCGGraph>>& OutVisitedGraphs) const { GetTrackedSelectionKeysToSettings(OutKeysToSettings, OutVisitedGraphs); }
#endif
	...
	/** Per-target cook decision for this graph. Embedded subgraphs inherit from their parent. */
	UE_API bool ShouldCookForPlatform(const ITargetPlatform* TargetPlatform) const;

protected:
#if WITH_EDITOR
	UFUNCTION()
	virtual bool CanToggleStandaloneGraph() const { return !IsEmbeddedSubgraph(); }

	UFUNCTION()
	virtual bool SupportHierarchicalGeneration() const { return !IsStandaloneGraph(); }

	UFUNCTION()
	virtual bool CanEditHierarchicalGeneration() const { return SupportHierarchicalGeneration() && !IsEmbeddedSubgraph(); }

	UFUNCTION()
	virtual bool ShowGraphCustomization() const { return !IsEmbeddedSubgraph(); }

	UFUNCTION()
	virtual bool ShowToolData() const { return !IsEmbeddedSubgraph(); }
	...
	UPROPERTY(meta=(PCGNoHash))
	TObjectPtr<UObject> UserParameterHierarchyRoot;

	UE_API void OnUserParameterHierarchyModified();
#endif // WITH_EDITORONLY_DATA

	// Parameters
	UPROPERTY(EditAnywhere, Category = "Graph Parameters", DisplayName = "Parameters", meta = (NoResetToDefault, DefaultType = "EPropertyBagPropertyType::Double", IsPinTypeAccepted = "UserParametersIsPinTypeAccepted", CanRemoveProperty = "UserParametersCanRemoveProperty", ChildRowFeatures = "Extended", PropertyBagSchemaClass="/Script/PCGEditor.PCGGraphParametersSchema"))
	FInstancedPropertyBag UserParameters;

	UPROPERTY()
	TArray<TObjectPtr<UPCGGraph>> EmbeddedSubgraphs;
	...
	UPROPERTY(EditAnywhere, Category = Settings, meta=(EditCondition="CanEditHierarchicalGeneration()", EditConditionHides))
	bool bUseHierarchicalGeneration = false;

	UPROPERTY(EditAnywhere, Category = Settings, meta = (DisplayName = "HiGen Default Grid Size", EditCondition = "CanEditHierarchicalGeneration() && bUseHierarchicalGeneration", EditConditionHides))
	EPCGHiGenGrid HiGenGridSize = EPCGHiGenGrid::Grid256;

	/** Scales the grid sizes allowing to use larger or smaller grids than those provided out of the box. Can be useful to align generation to other grids in the world. */
	UPROPERTY(EditAnywhere, Category = Settings, meta = (DisplayName = "HiGen Grid Size Multiplier", EditCondition = "SupportHierarchicalGeneration && bUseHierarchicalGeneration", EditConditionHides))
	double HiGenGridSizeMultiplier = 1.0;

	UPROPERTY(EditAnywhere, Category = Settings, meta = (DisplayName = "2D Grid", EditCondition = "CanEditHierarchicalGeneration() && bUseHierarchicalGeneration", EditConditionHides))
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta=(EditCondition="!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	...
	mutable PCG::FSharedLock NodeToGridSizeLock;
	...
	mutable PCG::FLock CachedGridInfoLock;

	/**
	 * Controls whether this PCG graph is included when cooking for each platform.
	 * If disabled, the graph and all of its compiled data (compute graphs, kernel sources, pins, nodes, edges)
	 * will be stripped from cooked builds. Any PCG components or graph instances that reference this graph
	 * will also be excluded from cooking.
	 *
	 * Not exposed on embedded subgraphs: the cook decision is inherited from the parent graph.
	 */
	UPROPERTY(EditAnywhere, Category = Cooking, meta=(EditCondition="!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	FPerPlatformBool ShouldCook = true;
	...
	*/
	UPROPERTY(EditAnywhere, Category = Debug, meta=(EditCondition="!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	bool bDebugFlagAppliesToIndividualComponents = true;

	UPROPERTY(EditAnywhere, Category = ChangeTracking, DisplayName = "Ignored Classes", meta = (EditCondition = "!IsEmbeddedSubgraph()", EditConditionHides))
	TArray<TSubclassOf<UObject>> ChangeTrackingIgnoredClasses;
#endif // WITH_EDITORONLY_DATA
...
private:
#if WITH_EDITORONLY_DATA
	/** Cached pin data to get type information without loading. */
	UPROPERTY(AssetRegistrySearchable)
	FPCGAssetCachedPins CachedPins;

	/** Builds and returns list of pins as visible for graph creation purposes. */
	FPCGAssetCachedPins BuildCachedPins() const;
#endif // WITH_EDITORONLY_DATA
...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Runtime Generation", meta=(EditCondition = "!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	FPCGRuntimeGenerationRadii GenerationRadii;

	// @todo_pcg: This could be on by default/hidden and graphs which need actors/actor-components could create a target actor on demand when necessary.
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Runtime Generation|Advanced", meta=(EditCondition = "!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	bool bUseActorComponentlessGeneration = false;

	/** Console variable names that trigger a runtime PCG refresh of this graph when any of their values change. Disabled in shipping game builds. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Runtime Generation", meta=(EditCondition = "!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	TArray<FString> ConsoleVariablesTriggeringRefresh;

	/** Auto-refresh on render-state events: viewport resize, component re-register, scene material change, preview platform change, fast-geo recreate.
	 *  Disable for slow CPU-generated graphs that should survive incidental render-state churn. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Runtime Generation", meta=(EditCondition = "!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	bool bAutoRefreshOnRenderStateRefresh = true;

	/** Auto-refresh when engine scalability settings or the pcg.Quality CVar change. Disable for slow CPU-generated graphs that shouldn't tear down on quality toggles. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Runtime Generation", meta=(EditCondition = "!IsStandaloneGraph() && !IsEmbeddedSubgraph()", EditConditionHides))
	bool bAutoRefreshOnQualitySettingsChanged = true;
...
private:
	...
	mutable PCG::FSharedLock InspectionStateLock;
#endif // WITH_EDITOR

	// Deprecated section
public:
	UE_DEPRECATED(5.8, "Use version that takes uint32 grid size instead")
	double GetGridGenerationRadiusFromGrid(EPCGHiGenGrid Grid) const { return GetGridGenerationRadiusFromGrid(PCGHiGenGrid::GridToGridSize(Grid)); }
	UE_DEPRECATED(5.8, "Use version that takes uint32 grid size instead")
	double GetGridCleanupRadiusFromGrid(EPCGHiGenGrid Grid) const { return GetGridCleanupRadiusFromGrid(PCGHiGenGrid::GridToGridSize(Grid)); }
	UE_DEPRECATED(5.8, "Use GetGridSizeMultiplier instead")
	uint32 GetGridExponential() const { return FMath::RoundToInt(FMath::Log2(FMath::Max(HiGenGridSizeMultiplier, UE_KINDA_SMALL_NUMBER))); }
	UE_DEPRECATED(5.8, "Use ShouldCook instead")
	UPROPERTY()
	bool bIsEditorOnly = false;
protected:
	UE_DEPRECATED(5.8, "Use HiGenGridSizeMultiplier instead")
	UPROPERTY()
...
public:
	...
	UE_API virtual TOptional<FPCGGraphToolData> GetGraphToolData() const override;
#endif
...
public:
	...
	UE_API virtual UObject* GetUserParameterHierarchyRoot() const override;

	UFUNCTION()
	virtual bool ShouldShowGraphProperty() const { return true; }
	virtual bool CanSaveAsAsset() const { return true; }
#endif // WITH_EDITOR
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Instance, AssetRegistrySearchable, meta = (GetAssetFilter = "GraphAssetFilter", EditCondition = "ShouldShowGraphProperty", EditConditionHides))
	TObjectPtr<UPCGGraphInterface> Graph;

	UPROPERTY(EditAnywhere, Category = Instance, meta = (NoResetToDefault, PropertyBagSchemaClass="/Script/PCGEditor.PCGGraphParametersSchema"))
