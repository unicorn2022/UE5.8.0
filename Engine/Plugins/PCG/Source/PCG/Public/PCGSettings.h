...
#if WITH_EDITOR
struct FPCGNodeThumbnailProxy
{
	using FTextureType = TVariant<const UTexture*, FSoftObjectPath>;

	PCG_API static FPCGNodeThumbnailProxy FromAssetPath(const FSoftObjectPath& InAssetPath);
	PCG_API static FPCGNodeThumbnailProxy FromTexture(const UTexture* InTexture);

	FTextureType TextureSource;
};
...
struct FPCGSettingsOverridableParam
{
	...
	/** Indicates the number of containers in the property chain. */
	UPROPERTY()
...
public:
	...
	/**
	 * Dedicated method to change debug state because some nodes have more complex behavior on enable/disable (such as reroutes).
	 * Returns true if the flag actually changed.
	 */
	UE_API bool SetDebugged(bool bInDebugged);

#if WITH_EDITOR
	/** Whether this node has manual edit access in the viewport. */
	bool IsTemporaryManualEditingEnabled() const { return bTemporaryManualEditingEnabled; }

	/** Whether this node is persistently marked for manual editing. */
	bool IsMarkedForManualEditing() const { return bMarkedForManualEditing; }

	/** Marks the node for a one-shot pass of manual edit in the viewport. */
	UE_API void SetTemporaryManualEditingEnabled(bool bInEnabled);

	/** Marks the node for permanent access to manual edit in the viewport. */
	UE_API void SetMarkedForManualEditing(bool bInMarked);
#endif // WITH_EDITOR
	...
	/** Whether the respective viewport tool (if any) is actively running in the viewport. */
	bool bNodeToolActive = false;
#endif

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Debug, meta = (EditCondition = "bDisplayDebuggingProperties && CanBeDisabled()", EditConditionHides, HideEditConditionToggle))
	bool bEnabled = true;
...
private:
	/** Set while the user is actively viewport-editing this node. */
	UPROPERTY(Transient, DuplicateTransient, meta = (PCGNoHash))
	bool bTemporaryManualEditingEnabled = false;

	/** Saved to the graph asset so viewport editing markers survive across sessions. */
	UPROPERTY(meta = (PCGNoHash))
	bool bMarkedForManualEditing = false;
...
public:
...
#if WITH_EDITOR
	bool IsNodeToolActive() const { return bNodeToolActive; }
	void SetNodeToolActive(bool InActive) { bNodeToolActive = InActive; }

	virtual EPCGNodeToolStartBehavior GetNodeToolStartBehaviour() const { return EPCGNodeToolStartBehavior::OnToolStartButton; }
#endif // WITH_EDITOR
	...
	/** If the node has any dynamic pins that can change type based on input types or settings. If true the default behavior 
	* for the output pin types will be to take the union of types incident on primary input pin. */
	...
	/** Whether to show/hide the title text/icon on the compact node. */
	virtual bool ShouldShowCompactNodeTitle() const { return true; }
	...
	UE_API virtual bool CanUserEditTitle() const;
	...
	virtual TOptional<FPCGNodeThumbnailProxy> GetNodeThumbnail() const { return {}; }
...
public:
	...
	UE_API virtual bool ShouldExecuteOnGPU() const;

#if WITH_EDITOR
	/** Set whether this node should be executed on the GPU. */
	void SetExecuteOnGPU(bool bValue) { bExecuteOnGPU = bValue; }

	/** Create the GPU kernels and kernel edges that should execute for this node. */
	UE_DEPRECATED(5.8, "Override the overload that takes a UPCGNode.")
	UE_API virtual void CreateKernels(FPCGGPUCompilationContext& InOutContext, UObject* InObjectOuter, TArray<UPCGComputeKernel*>& OutKernels, TArray<FPCGKernelEdge>& OutEdges) const;

	/** Create the GPU kernels and kernel edges that should execute for this node. */
	virtual void CreateKernels(FPCGGPUCompilationContext& InOutContext, UObject* InObjectOuter, const UPCGNode* InNode, TArray<UPCGComputeKernel*>& OutKernels, TArray<FPCGKernelEdge>& OutEdges) const {}

	/** Whether to display GPU execution option in node settings UI. */
	virtual bool DisplayExecuteOnGPUSetting() const { return false; }

	/** Whether this node is GPU-friendly: either it has a GPU execution backend, or it runs on CPU but does not force GPU data readbacks or uploads. */
	UE_API virtual bool IsGPUFriendly(const FPCGPreConfiguredSettingsInfo* PreconfiguredInfo = nullptr) const;
#endif
...
#if WITH_EDITOR
	UFUNCTION()
	UE_API bool DisplayExecuteOnGPUSettingCondition() const;
#endif
...
protected:
	/** Whether this node should be executed on the GPU. */
	UPROPERTY(EditAnywhere, Category = "GPU", meta = (EditCondition = "DisplayExecuteOnGPUSettingCondition()", EditConditionHides, HideEditConditionToggle))
...
private:
...
#if WITH_EDITOR
	/** Self-subscribing handler that invalidates CachedCrc whenever OnSettingsChangedDelegate fires. */
	...
	/** Cached pins when settings are used as assets. Will never be used otherwise. */
...
public:
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = Instance, AssetRegistrySearchable, meta = (EditInline))
