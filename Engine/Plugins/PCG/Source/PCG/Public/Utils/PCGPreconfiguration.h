...
struct FPCGPreconfiguredInfo
{
	...
	*/
	...
	static TArray<SubclassType> PopulateFromEnum(const TSet<EnumType>& InValuesToSkip = {}, const FTextFormat& InOptionalFormat = FTextFormat(), const FTextFormat& InOptionalTooltipFormat = FTextFormat(), const TFunction<bool(SubclassType& InOutInfo, const UEnum* EnumPtr, int32 ValueIndex)>& ProcessFunc = nullptr)
	{
		...
		const bool bHasTooltipFormat = !InOptionalTooltipFormat.GetSourceText().IsEmpty();
		...
		if (const UEnum* EnumPtr = StaticEnum<EnumType>())
		{
			...
			for (int32 i = 0; i < EnumPtr->NumEnums(); ++i)
			{
				...
				if (Value != EnumPtr->GetMaxEnumValue() && !InValuesToSkip.Contains(EnumType(Value)))
				{
					...
					{
						SubclassType Info(Value, FText::Format(Format, MoveTemp(DisplayName)));

#if WITH_EDITOR
						if (bHasTooltipFormat)
						{
							Info.Tooltip = FText::Format(InOptionalTooltipFormat, EnumPtr->GetToolTipTextByIndex(i));
						}
#endif // WITH_EDITOR

						if (!ProcessFunc || ProcessFunc(Info, EnumPtr, i))
						{
							PreconfiguredInfo.Emplace(MoveTemp(Info));
						}
					}
				}
			}
	...
	{
		return PopulateFromEnum(InValuesToSkip, InOptionalFormat, {}, ProcessFunc);
	}
...
struct FPCGPreConfiguredSettingsInfo : public FPCGPreconfiguredInfo
{
	...
	FPCGPreConfiguredSettingsInfo(const int32 InIndex, const FText& InLabel, const FText& InTooltip, const FText& InSearchHints = {}, TSharedPtr<IPCGActionIconDescriptorBase> InActionIconDescriptor = nullptr)
		...
		, ActionIconDescriptor(MoveTemp(InActionIconDescriptor)) {}
#endif // WITH_EDITOR

	template <typename EnumType>
	static TArray<FPCGPreConfiguredSettingsInfo> PopulateFromEnum(const TSet<EnumType>& InValuesToSkip = {}, const FTextFormat& InOptionalFormat = FTextFormat(), const FTextFormat& InOptionalTooltipFormat = FTextFormat())
	{
		static_assert(std::is_enum_v<EnumType>, "Must use an enum to populate.");

#if WITH_EDITOR
		return FPCGPreconfiguredInfo::PopulateFromEnum<EnumType, FPCGPreConfiguredSettingsInfo>(InValuesToSkip, InOptionalFormat, InOptionalTooltipFormat, [](FPCGPreConfiguredSettingsInfo& InOutInfo, const UEnum* EnumPtr, const int32 ValueIndex) -> bool
		{
			if (EnumPtr)
			{
				...
				else if (EnumPtr->HasMetaData(ActionIconMetadataKey, ValueIndex))
				{
					...
					InOutInfo.ActionIconDescriptor = MakeShared<FPCGActionIconByLabelDescriptor>(MoveTemp(IconDescriptor));
				}
			}
...
#else
		return FPCGPreconfiguredInfo::PopulateFromEnum<EnumType, FPCGPreConfiguredSettingsInfo>(InValuesToSkip, InOptionalFormat, InOptionalTooltipFormat);
#endif // WITH_EDITOR
	}
