...
struct FPCGKernelAttributeTable
{
	...
	/** Caller must not insert the same key twice. */
	int32 AddAttributeAssumeUnique(const FPCGKernelAttributeKey& Key);

	int32 Num() const { return AttributeTable.Num(); }

	void Empty() { AttributeTable.Empty(); }

	void Reserve(int32 InCapacity) { AttributeTable.Reserve(InCapacity); }
	void Shrink() { AttributeTable.Shrink(); }

	using FAttributeKeyToIndexMap = TMap<FPCGKernelAttributeKey, int32, TInlineSetAllocator<32>>;

	/** Export a map of keys to attribute indices. */
	FAttributeKeyToIndexMap GetAttributeKeyToIndex() const;
...
public:
	...
	PCG_API void DeallocateProperties(EPCGPointNativeProperties InProperties);
	...
	uint16 GetTextureArraySize() const { return TextureArraySize; }
	void SetTextureArraySize(uint16 InTextureArraySize) { TextureArraySize = InTextureArraySize; }

	const FTransform& GetTextureTransform() const { return TextureTransform; }
	void SetTextureTransform(const FTransform& InTransform) { TextureTransform = InTransform; }

	// There's no store shader function to write to particular mips so we always generate textures with 1 mip for now.
	const int32 GetTextureNumMips() const { return 1; }
	...
	EPCGRenderTargetFormat GetRenderTargetFormat() const { return RenderTargetFormat; }
	void SetRenderTargetFormat(EPCGRenderTargetFormat InRenderTargetFormat) { RenderTargetFormat = InRenderTargetFormat; }

	EPCGTextureFilter GetTextureFilter() const { return TextureFilter; }
	void SetTextureFilter(EPCGTextureFilter InTextureFilter) { TextureFilter = InTextureFilter; }
...
private:
	...
	// Texture settings
	/** Texture format used to create a render target UAV from this data. If this description represents a CPU texture data, then this format will be a narrowed conversion from its EPixelFormat. */
	EPCGRenderTargetFormat RenderTargetFormat = EPCGRenderTargetFormat::RGBA16f;
...
struct FPCGDataCollectionDesc
{
	...
	* with conflicting names are present. */
	...
	PCG_API void DeallocatePropertiesForAllData(EPCGPointNativeProperties InProperties);
	PCG_API EPCGPointNativeProperties GetAllocatedPropertiesUnion() const;
	...
	PCG_API void SetRenderTargetFormatForAllData(EPCGRenderTargetFormat InRenderTargetFormat);

	PCG_API void SetTextureFilterForAllData(EPCGTextureFilter InTextureFilter);

	PCG_API void SetTextureArraySizeForAllData(uint16 InTextureArraySize);

	PCG_API void SetTextureTransformForAllData(const FTransform& InTransform);
