...
struct FPCGDataToDebug
{
	...
	FPCGDataToDebug() = default;
	FPCGDataToDebug(const FPCGDataToDebug& Other) = default;
	FPCGDataToDebug& operator=(const FPCGDataToDebug& Other) = default;
	...
	UE_DEPRECATED(5.8, "Use Tags field instead")
	UPROPERTY()
...
public:
	...
	/** Flushes computed data descriptions. Subsequent attempts to retrieve from the cache will trigger errors. */
	void FlushDataDescriptionCache();
	...
	PCG_API void ReceiveDataFromGPU_GameThread(UPCGData* InData, const UPCGSettings* InProducerSettings, EPCGExportMode InExportMode, FName InPinLabel, FName InPinLabelAlias, const FPCGDataDesc& InDataDesc);
	...
	*/
	...
	/** Compute how many culling cells are needed to cover the current generation volume. */
	FIntVector GetMaxNumCullingCells(int32 InCullingCellExtent) const;
...
private:
	...
	// Deprecated section
public:
	UE_DEPRECATED(5.8, "Use the version that takes a data description instead.")
