...
struct FPCGKernelOverridableParam
{
	...
	/** The underlying metadata type, used for type-aware codegen and generic shader parameter building. */
	UPROPERTY()
	EPCGMetadataTypes UnderlyingType = EPCGMetadataTypes::Integer32;
};
...
public:
	...
	PCG_API const UPCGNode* GetNode() const;
	...
	PCG_API virtual bool IsKernelDataValid(const UPCGDataBinding* InDataBinding, FPCGContext* InContext) const;
	...
	/** Create a UPCGKernelParamsDataInterface if this kernel has overridable params and one was not already provided by a subclass in CreateAdditionalInputDataInterfaces. */
	void CreateKernelParamsDataInterface(FPCGGPUCompilationContext& InOutContext, UObject* InObjectOuter, TArray<TObjectPtr<UComputeDataInterface>>& InOutDataInterfaces) const;
	...
	/** Whether the data on the input pin is used on the GPU (and thus requires a data interface) or not. */
	virtual bool UploadInputPinDataToGPU(FName InPinLabel) const { return true; }
#endif
	...
	*/
	...
	PCG_API virtual bool RequiresPostTLASBuildExecutionGroup() const;
...
private:
	...
	UPROPERTY()
	TSoftObjectPtr<const UPCGNode> NodePath;
	...
	mutable TObjectPtr<const UPCGNode> ResolvedNode = nullptr;

	// Deprecated section
public:
	UE_DEPRECATED(5.8, "Override IsKernelDataValid(const UPCGDataBinding*, FPCGContext*) instead.")
