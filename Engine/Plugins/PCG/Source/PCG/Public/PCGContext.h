...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/** Tracks whether the input conversion step has been done (inside of the PrepareData phase). */
	bool bConvertInputsIfNeededDone : 1 = false;
	...
	bool CanExecuteOnlyOnMainThread() const { return bMustRunOnMainThread; }
	void RequestToRunOnMainThread() { bMustRunOnMainThread = true; }
	...
	// Return the mutable settings casted in the wanted type.
	// If there is no override, it returns null.
	template<typename SettingsType>
	SettingsType* GetMutableInputSettings()
	{
		return SettingsWithOverride ? Cast<SettingsType>(SettingsWithOverride) : nullptr;
	}
	...
	// Will eventually be deprecated in favor of GetExecutionTarget/GetTypedExecutionTarget
	UE_API AActor* GetTargetActor(const UPCGSpatialData* InSpatialData) const;

	UE_API UObject* GetExecutionTarget() const;

	template<class TargetType>
	TargetType* GetTypedExecutionTarget() const
	{
		return Cast<TargetType>(GetExecutionTarget());
	}
	...
	/** Returns the proper actor to do debug visualization. */
	UE_API AActor* GetDebugDisplayActor() const;
#endif // WITH_EDITOR
	...
	/**
	 * Get the context setup for this thread, can be null. It is set within an element execution, but it is the responsibility of the
	 * implementer to set this context (with FPCGContextScope) when starting new async tasks.
	 */
	UE_API static FPCGContext* GetContextForThread();

	UE_API FPCGSourceDataContainer* GetSourceDataContainer();
	UE_API const FPCGSourceDataContainer* GetSourceDataContainer() const;

protected:
	virtual UObject* GetExternalContainerForOverridableParam(const FPCGSettingsOverridableParam& InParam) { return nullptr; }
	virtual FStructView GetExternalStructContainerForOverridableParam(const FPCGSettingsOverridableParam& InParam) { return {}; }

	UE_DEPRECATED(5.8, "You should use GetExternalStructContainerForOverridableParam for structs overrides.")
	...
	TWeakPtr<FPCGGraphExecutor> GetGraphExecutor() { return GraphExecutor; }
...
private:
	...
	// If the element must run on the main thread (like if the settings need to be overridden on the main thread, because we have to load objects))
	bool bMustRunOnMainThread = false;
...
/**
 * Allow to set the current context for the current thread. The context can then be queryable with "FPCGContext::GetContextForThread".
 * Scopes can be nested, as we store the previous context to restore it on scope destruction.
 * Be very careful in the scope nesting, scopes need to be destroyed in order (ScopeA ctor -> ScopeB ctor -> ScopeB dtor -> ScopeA dtor).
 */
struct FPCGContextScope
{
	PCG_API FPCGContextScope(FPCGContext* InContext);
	PCG_API ~FPCGContextScope();

	// Non-copyable, non-moveable
	FPCGContextScope(const FPCGContextScope&) = delete;
	FPCGContextScope(FPCGContextScope&&) = delete;
	FPCGContextScope& operator=(const FPCGContextScope&) = delete;
	FPCGContextScope& operator=(FPCGContextScope&&) = delete;
	
	// Keep this context to have some verification check that scopes are not destroyed in the wrong order.
	FPCGContext* ThisContext = nullptr;
	FPCGContext* PreviousContext = nullptr;
};
