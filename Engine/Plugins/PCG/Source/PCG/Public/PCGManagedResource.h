...
{
	/** Class of the object that needs to be created as part of moving the resources. */
	UClass* TemplateTargetClass = nullptr;
	/** Target object to move resources to. Can be null, in which case we'll create one from the TemplateTargetClass. */
	UObject* Target = nullptr;
	/** Additional validation object owner. */
	const UObject* ExpectedPreviousOwner = nullptr;
	/** Conjunction of tags required to move a resource to the target. */
	TArray<FName> RequiredTags;
	/** Disjunction of tags that would disqualify moving a resource to the target. */
	TArray<FName> ExcludedTags;
	/** If there is generated data on the source objects, it will be moved using the same requirements, into the target collection. */
	FPCGDataCollection* TargetDataCollection = nullptr;
	/** Controls whether the created target should be 'attached' to the original object. */
	bool bAttachToParent = false;
	/** Name Override, can be left empty. */
	FName DefaultName = NAME_None;

	template<typename ObjectType>
	TSubclassOf<ObjectType> GetTemplateClass() const
	{
		if (TemplateTargetClass && TemplateTargetClass->IsChildOf(ObjectType::StaticClass()))
		{
			return TSubclassOf<ObjectType>(TemplateTargetClass);
		}
		else
		{
			return nullptr;
	...
	{
		return Cast<ObjectType>(Target);
	}

	template<typename ObjectType>
	const ObjectType* GetExpectedPreviousOwner() const
	{
		return Cast<ObjectType>(ExpectedPreviousOwner);
	}

	bool Accepts(const UObject* InObject) const;
	bool Accepts(const FPCGTaggedData& InData) const;
};
...
public:
	...
	UE_API virtual void MarkAsUsed();
	...
	UE_API virtual bool MoveResource(const FPCGMoveResourceParams& Params);
...
protected:
	...
	// Deprecated calls section
public:
	/** Move the given resource to a new actor. Return true if it has succeeded */
	UE_DEPRECATED(5.8, "Implement or use MoveResource instead.")
	virtual bool MoveResourceToNewActor(AActor* NewActor) { return false; };

	UE_DEPRECATED(5.8, "Implement or use MoveResource instead.")
	UE_API virtual bool MoveResourceToNewActor(AActor* NewActor, const AActor* ExpectedPreviousOwner);
};
...
struct FPCGManagedActorLoadingScope
{
	...
	virtual ~FPCGManagedActorLoadingScope() = default;

	void AddResources(IPCGGraphExecutionSource* InExecutionSource, const TArray<UPCGManagedResource*>& InManagedResources);
...
public:
	...
	UE_API virtual bool MoveResource(const FPCGMoveResourceParams& Params) override;
	...
	UE_DEPRECATED(5.8, "Use MoveResource instead.")
...
public:
	...
	void SetCrcReuseCount(int32 InCount) { CrcReuseCount = InCount; }
	int32 GetCrcReuseCount() const { return CrcReuseCount; }

	UE_API void SetDataCrcReuseCount(int32 InCount) { DataCrcReuseCount = InCount; }
	UE_API int32 GetDataCrcReuseCount() const { return DataCrcReuseCount; }
...
protected:
	...
	UPROPERTY()
	int32 CrcReuseCount = INDEX_NONE;

	UPROPERTY()
	int32 DataCrcReuseCount = INDEX_NONE;
};
...
public:
	...
	//~Begin UPCGManagedComponents interface
	UE_API virtual void ForgetComponent() override;
...
private:
	USplineMeshComponent* CacheAndGetComponent() const;
	
	UPROPERTY(Transient)
	mutable TWeakObjectPtr<USplineMeshComponent> CachedGeneratedComponent;

	mutable PCG::FSharedLock CachedLock;
};
