...
public:
	...
	*/
	...
	/** Overrides the default label that would be set on the spawned actors (class based) by a user-specified one. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Option!=EPCGSpawnActorOption::CollapseActors", EditConditionHides, PCG_Overridable))
	FString ActorLabel;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Instanced, Category = Settings, meta = (ShowInnerProperties, EditCondition = "GetAllowTemplateActorEditing()", EditConditionHides))
	...
	*/
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "bSpawnByAttribute", PCG_Overridable))
...
protected:
	...
	/** Can modify the template actor for the expected class. When spawning by attribute, it is mandatory for that template actor to be a parent class of ALL actors that will be spawned. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditConditionHides, EditCondition = "Option != EPCGSpawnActorOption::CollapseActors"))
...
public:
	...
	UFUNCTION()
	PCG_API bool GetAllowTemplateActorEditing() const;
...
private:
	...
	void SpawnActors(FPCGSubgraphContext* Context, UObject* TargetWorldObject, TSubclassOf<AActor> TemplateActorClass, AActor* TemplateActor, FPCGTaggedData& Output, const UPCGBasePointData* PointData, UPCGBasePointData* OutPointData) const;
};
