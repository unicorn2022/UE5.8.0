...
namespace PCGExtractAttribute
{
	struct FExtractAttributeParams
	{
		...
		/** 
		* For deprecation, old behavior was discarding the output domain and was always writing to the Elements domain.
		* Since ExtractAttribute function was not exported, external users could not use it, so default to false in the general case.
		*/
		bool bForceOutputAttributeToBeInElementsDomain = false;
	};
...
public:
	...
	/** For deprecation, old node was discarding the output domain and was always writing to the Elements domain. This will be false for new nodes. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Settings|Deprecation", meta = (EditCondition="bForceOutputAttributeToBeInElementsDomain", EditConditionHides))
	bool bForceOutputAttributeToBeInElementsDomain = true;
};
