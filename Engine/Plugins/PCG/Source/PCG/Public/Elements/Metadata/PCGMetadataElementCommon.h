...
namespace PCGMetadataElementCommon
{
	...
	FPCGMetadataAttribute<T>* ClearOrCreateAttribute(FPCGMetadataDomain* Metadata, const FName& DestinationAttribute, T DefaultValue = PCG::Private::MetadataTraits<T>::ZeroValue())
	{
		...
		{
			UE_LOGF(LogPCG, Verbose, "Attribute %ls already exists and will be overwritten", *DestinationAttribute.ToString());
	...
	*/
	...
	bool ApplyOnMultiAccessorsRange(const TConstArrayView<IPCGAttributeAccessorKeys const*> MultiKeys, const TConstArrayView<IPCGAttributeAccessor const*> Accessors, Func&& InCallback, EPCGAttributeAccessorFlags Flags = EPCGAttributeAccessorFlags::StrictType, const int32 ChunkSize = DefaultChunkSize, const int32 Count = -1)
	{
		...
		// Making sure we can accommodate our big types that have different alignment like FTransform or FVector4
		constexpr int32 MaxAlignment = FMath::Max(alignof(T)...);
		constexpr int32 AlignedBlockSize = MaxAlignment;
		constexpr int32 MaxInlineMem = DefaultChunkSize * sizeof(FVector) / AlignedBlockSize;
		static_assert(MaxInlineMem > 0, "MaxInlineMem must be greater than 0");

		using RawBuffer = TArray<TAlignedBytes<AlignedBlockSize, MaxAlignment>, TInlineAllocator<MaxInlineMem>>;
		...
		([&]
		{
			const int32 AllocationSize = FMath::DivideAndRoundUp<int32>(sizeof(T) * ChunkSize, AlignedBlockSize);
			RawBuffers[i].SetNumUninitialized(AllocationSize);
			if constexpr (!std::is_trivially_copyable_v<T>)
			{
