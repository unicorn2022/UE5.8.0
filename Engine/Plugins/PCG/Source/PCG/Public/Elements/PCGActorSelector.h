...
UENUM()
enum class EPCGIsolatedActorIncludeFlag : uint8
{
...
struct FPCGSelectionKey
{
	...
	UE_API bool IsMatching(const FPCGSelectionKey& InSelectionKey, const IPCGGraphExecutionSource* InExecutionSource) const;
	UE_API bool IsMatching(const FPCGSelectionKey& InSelectionKey, const TSet<FName>& InRemovedTags, const TSet<IPCGGraphExecutionSource*>& InExecutionSources, TSet<IPCGGraphExecutionSource*>* OptionalExecutionSources = nullptr) const;
	...
	/** If both keys matching have optional bounds, then the IsMatching will test that there is an overlap between any pair of bounds. */
	UPROPERTY()
	TArray<FBox> OptionalBounds;

	// Deprecated methods
	UE_DEPRECATED(5.8, "Replaced by the version using IPCGGraphExecutionSource.")
...
struct FPCGActorSelectorSettings
{
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Actor Selector Settings")
	EPCGIsolatedActorIncludeFlag IncludeIsolatedActors = EPCGIsolatedActorIncludeFlag::Ignore;
...
namespace PCGActorSelector
{
	...
	PCG_API TArray<AActor*> FindActors(const FPCGActorSelectorSettings& Settings, const IPCGGraphExecutionSource* InExecutionSource, const TFunction<bool(const AActor*)>& BoundsCheck, const TFunction<bool(const AActor*)>& SelfIgnoreCheck, TArrayView<AActor*> InputActors = TArrayView<AActor*>());
	PCG_API AActor* FindActor(const FPCGActorSelectorSettings& InSettings, const IPCGGraphExecutionSource* InExecutionSource, const TFunction<bool(const AActor*)>& BoundsCheck, const TFunction<bool(const AActor*)>& SelfIgnoreCheck, TArrayView<AActor*> InputActors = TArrayView<AActor*>());

	// Actor + Component filtering
	PCG_API TArray<AActor*> FindActors(const FPCGActorSelectorSettings* ActorSettings, const FPCGComponentSelectorSettings* ComponentSettings, const IPCGGraphExecutionSource* InExecutionSource, const TFunction<bool(const AActor*)>& BoundsCheck, const TFunction<bool(const AActor*)>& SelfIgnoreCheck, TArrayView<AActor*> InputActors = TArrayView<AActor*>());
