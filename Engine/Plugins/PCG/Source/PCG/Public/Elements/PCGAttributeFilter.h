...
struct FPCGAttributeFilterThresholdSettings
{
	...
	/** If there are multiple filtering data, do a N:N filtering (Input 0 with filter 0, Input 1 with filter 1, etc...). Otherwise, only use the first data. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "!bUseConstantThreshold", EditConditionHides))
	bool bFilterEachInputWithEachFilterDataRespectively = false;
...
public:
	...
	/** If there is multiple filtering data, do a N:N filtering (Input 0 with filter 0, Input 1 with filter 1, etc...). Otherwise, only use the first data. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "!bUseConstantThreshold", EditConditionHides))
	bool bFilterEachInputWithEachFilterDataRespectively = false;
