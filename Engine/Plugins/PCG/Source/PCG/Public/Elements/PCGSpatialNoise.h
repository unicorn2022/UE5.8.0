...
public:
	...
	// How many times the fractal method recurses. A higher number will mean more detail
	...
	// If true, will generate results that tile along the bounding box size of the PCG source.
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable))
	bool bTiling = false;
	...
	// The less random this is, the more it returns to being a grid (at zero).
	...
	// Will make the falloff harsher or softer
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (EditCondition = "Mode == PCGSpatialNoiseMode::EdgeMask2D", EditConditionHides, ClampMin = "0", PCG_Overridable))
