...
UENUM()
enum class EPCGSplitSplineDataBehavior
{
	KeepAllSplines,
	KeepCutSplines,
...
namespace PCGSplitSpline
{
	...
	EStatus SplitSpline(const UPCGSplineData* Spline, UPCGSplineData* SplitSpline, double StartKey, double EndKey);
	EStatus SplitSpline(const UPCGSplineData* Spline, UPCGSplineData* SplitSpline, TConstArrayView<FSplinePoint> SplinePoints, TConstArrayView<PCGMetadataEntryKey> SplinePointsEntryKeys, double StartKey, double EndKey);
}
...
public:
	...
	/** Behavior of whether we'll keep uncut, cut on one side or only interior splines. Note that for closed splines, all cut splines are interior splines. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings)
	EPCGSplitSplineDataBehavior CutDataBehavior = EPCGSplitSplineDataBehavior::KeepAllSplines;
