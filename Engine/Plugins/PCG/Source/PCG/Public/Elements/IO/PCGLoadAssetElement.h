...
public:
	PCG_API UPCGLoadDataAssetSettings();
	...
	PCG_API virtual FPCGDataTypeIdentifier GetCurrentPinTypesID(const UPCGPin* InPin) const;

protected:
	PCG_API virtual FPCGElementPtr CreateElement() const override;
#if WITH_EDITOR
	PCG_API virtual EPCGChangeType GetChangeTypeForProperty(const FName& InPropertyName) const override;
#endif
...
public:
	PCG_API virtual void SetFromAsset(const FAssetData& InAsset);
	PCG_API virtual void UpdateFromData(bool bShouldLoadAsset = true);

#if WITH_EDITOR
	PCG_API void OnPreObjectSaved(UObject* Object, FObjectPreSaveContext SaveContext);
#endif // WITH_EDITOR
...
protected:
	PCG_API virtual bool PrepareDataInternal(FPCGContext* Context) const override;
	PCG_API virtual bool ExecuteInternal(FPCGContext* Context) const override;
};

// Extra class to handle when a PCG Data Asset is changed to update all loaded Load PCG Data Asset nodes
// Will be registered by the PCG Editor.
#if WITH_EDITOR
class FPCGLoadDataAssetChangeTracker
{
public:
	PCG_API FPCGLoadDataAssetChangeTracker();
	PCG_API ~FPCGLoadDataAssetChangeTracker();

	PCG_API static void OnPreObjectSaved(UObject* InObject, FObjectPreSaveContext InSaveContext);
private:
