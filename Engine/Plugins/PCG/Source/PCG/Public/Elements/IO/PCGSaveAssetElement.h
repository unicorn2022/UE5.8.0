...
protected:
	...
	PCG_API virtual bool ExportAsset(const FString& PackageName, UPCGDataAsset* Asset);
	PCG_API virtual UPackage* UpdateAsset(const FAssetData& PCGAsset);
	//~End UPCGAssetExporter interface

public:
	// @todo_pcg: Should it be a base class function? 
	PCG_API bool ShouldSave(const UPCGDataAsset* Asset);
...
public:
	PCG_API UPCGSaveDataAssetSettings();
