...
public:
	/** If something is connected in the In pin, it will look for this attribute values to load, representing the object reference. */
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable, DisplayName="Tags to add on actors/components"))
	...
	/**
	 * In case of container override, allow for each element in input, to use the full override data in the Property Overrides pin.
	 * In that case it is expected to have as many data in the Property Overrides pin as there are elements in the In pin.
	 * (it's also allowed to have just a single data in Property Overrides, in that case, this data will be used for all the elements in the In pin)
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings)
	bool bUseOneOverrideDataPerElement = false;
