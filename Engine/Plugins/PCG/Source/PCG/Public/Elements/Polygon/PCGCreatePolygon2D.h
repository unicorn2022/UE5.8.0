...
UENUM()
enum class EPCGCreatePolygonDefaultNormalBehavior : uint8
{
	OnlyForDegenerateCases UMETA(Tooltip="Uses the default normal only for polygons that are degenerate (collinear or too small)."),
	OnlyForOpenPolygons UMETA(Tooltip="Uses the default normal for all open polygons (including forced open polygons) and degenerate polygons."),
	Always UMETA(Tooltip="Uses the default normal for all polygons.")
};
...
public:
	...
	/** Default axis to use when the input has an ambiguous shape (lines, collinear polygons, etc.) */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, AdvancedDisplay, meta = (PCG_Overridable))
	FVector DefaultPolygonNormal = FVector::ZAxisVector;

	/** Controls whether we will use the default polygon normal : always/only for open/only for degenerate data */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, AdvancedDisplay, meta = (PCG_Overridable))
	EPCGCreatePolygonDefaultNormalBehavior DefaultPolygonNormalBehavior = EPCGCreatePolygonDefaultNormalBehavior::OnlyForDegenerateCases;

	/** Controls behavior of how joining points are resolved. See GeometryScript for more details. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, AdvancedDisplay, meta = (EditCondition = "InputType != EPCGCreatePolygonInputType::ForceClosed", PCG_Overridable))
	EPCGPolygonJoinType JoinType = EPCGPolygonJoinType::Square;

	/** Controls behavior of how end points are resolved. See GeometryScript for more details. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, AdvancedDisplay, meta = (EditCondition = "InputType != EPCGCreatePolygonInputType::ForceClosed", PCG_Overridable))
	EPCGPolygonEndType EndType = EPCGPolygonEndType::Square;
};