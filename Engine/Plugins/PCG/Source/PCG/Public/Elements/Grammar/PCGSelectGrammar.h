...
public:
	...
	/** Each criterion has an associated key, this node will only evaluate (in order) the criteria that match this value. */
	...
	/** Attribute selector to read a key attribute from the input data. Each criterion has an associated key, this node will only evaluate (in order) the criteria that match the value of this attribute for each element. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable, EditCondition = "bKeyAsAttribute", EditConditionHides))
