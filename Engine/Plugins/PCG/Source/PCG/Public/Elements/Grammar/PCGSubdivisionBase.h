...
namespace PCGSubdivisionBase
{
	...
	template <typename T>
	bool Subdivide(const T& Root, double Length, TArray<TModuleInstance<T>>& OutModuleInstances, double& RemainingLength, FPCGContext* InOptionalContext = nullptr, int32 InOptionalAdditionalSeed = 0, const bool bQuiet = false)
	{
