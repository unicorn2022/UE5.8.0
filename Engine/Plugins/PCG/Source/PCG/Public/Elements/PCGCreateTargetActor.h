...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Settings, DisplayName = "Comma Separated Actor Tags (Deprecated)", meta = (PCG_Overridable))
	...
	/** 
	 * Override the template actor class to spawn. If there is an actor template, it will only be valid if the override class is a child class (or the exact class) of the template actor.
	 * If it is not, the node will fail.
	 */
	UPROPERTY(meta = (PCG_Overridable))
	TSubclassOf<AActor> OverrideTemplateActorClass;
