...
enum class EPCGGetDataFromActorMode : uint8
{
	...
	GetPointData UMETA(Tooltip = "Produces a point data containing a point per component, except for ISMs that will have a point per instance. Similar behavior to the PCG Data Asset format.")
};
...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Data Retrieval Settings", meta = (EditCondition = "Mode == EPCGGetDataFromActorMode::ParseActorComponents || Mode == EPCGGetDataFromActorMode::GetPointData || Mode == EPCGGetDataFromActorMode::GetComponentsReference || Mode == EPCGGetDataFromActorMode::GetDataFromPCGComponentOrParseComponents", EditConditionHides))
	...
	/** Select which grid sizes to consider when collecting data from partitioned PCG components (without grid scaling applied). */
