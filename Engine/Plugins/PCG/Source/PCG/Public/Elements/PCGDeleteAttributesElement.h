...
public:
	// Implementation note: the default operation has been changed to DeleteSelected for new objects
	/** Controls if only the attributes that are selected in this node will be kept or deleted from the input data. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable))
	EPCGAttributeFilterOperation Operation = EPCGAttributeFilterOperation::KeepSelectedAttributes;

	/** Criteria to match the attribute name against the selected attributes. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable))
	...
	/** When deleting attributes, only the specified domain will be visited. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable))
