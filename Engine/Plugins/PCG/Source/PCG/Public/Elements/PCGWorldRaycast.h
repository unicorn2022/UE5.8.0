...
namespace PCGWorldRaycastElement::Constants::Labels
{
	extern const FName EndPointsInput;
}
...
#if WITH_EDITOR
	virtual bool IsGPUFriendly(const FPCGPreConfiguredSettingsInfo* PreconfiguredInfo = nullptr) const override;
#endif
...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable, ShowOnlyInnerProperties, EditCondition = "!bExecuteOnGPU", EditConditionHides))
	...
	/** [EXPERIMENTAL] Create a Vector2 attribute ('ImpactUVCoords') for UV Coordinates of the surface hit. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable, EditCondition = "bExecuteOnGPU", EditConditionHides))
	bool bGetTextureCoordinates = false;

	/** [EXPERIMENTAL] Which channel to sample UV Coordinates from. If this channel does not exist, UV coordinates will be (0, 0). */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable, EditCondition = "bExecuteOnGPU && bGetTextureCoordinates", EditConditionHides))
	int TextureCoordinatesChannelIndex = 0;

	/** [EXPERIMENTAL] Create a Vector attribute ('ImpactNormal') for normals of the surface hit. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable, EditCondition = "bExecuteOnGPU", EditConditionHides))
	bool bGetNormals = false;
};
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		FIterationState() = default;
		FIterationState(const FIterationState&) = default;
		FIterationState(FIterationState&&) = default;
		FIterationState& operator=(const FIterationState&) = default;
		FIterationState& operator=(FIterationState&&) = default;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		UE_DEPRECATED(5.8, "Use CachedFilterObjects instead")
