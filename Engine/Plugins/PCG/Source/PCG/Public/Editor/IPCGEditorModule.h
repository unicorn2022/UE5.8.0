...
public:
	...
	virtual void SetPermissionMode(EPCGEditorPermissionMode InPermissionMode) = 0;
	virtual EPCGEditorPermissionMode GetPermissionMode() const = 0;
	virtual void TransferPropertyBagMetadataIntoHierarchy(UPCGGraph* Graph) const = 0;
	virtual UObject* CreatePropertyBagHierarchyRootObject(UPCGGraph* Graph) const = 0;
	virtual void ConnectPropertyBagHierarchyRootObjectModifyDelegate(UPCGGraph* Graph) const = 0;
	virtual void GraphIsBeingDestroyed(UPCGGraph* Graph) const = 0;

	DECLARE_EVENT_OneParam(IPCGEditorModule, FPermissionModeChanged, EPCGEditorPermissionMode);
	virtual FPermissionModeChanged& OnPermissionModeChanged() = 0;

	static bool ShouldTreatViewportAsGenerationSource(const APCGWorldActor* InPCGWorldActor);
#endif
...
#if WITH_EDITOR
	virtual bool ShouldTreatViewportAsGenerationSourceInternal(const APCGWorldActor* InPCGWorldActor) const = 0;
#endif
