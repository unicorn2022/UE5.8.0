...
namespace PCGHelpers
{
	...
	/** Returns the bounds of InActor, intersected with the execution source if InActor is a partition actor */
	PCG_API FBox GetGridBounds(const AActor* InActor, const IPCGGraphExecutionSource* InSource);
	...
	*/
	...
	/**
	 * Create keys for a hypothetical attribute in the provided domain ('Elements' by default), get its count, and create
	 * a const value accessor for retrieving it. Only available if the data supports the provided domain.
	 */
	PCG_API int32 GetNumberOfElements(const UPCGData* InData, const FPCGMetadataDomainID& InDomainID = PCGMetadataDomainID::Elements);
...
public:
	static UFunction* GetPrototypeWithNoParams() { FGCScopeGuard Guard; return FindObject<UFunction>(StaticClass(), TEXT("PrototypeWithNoParams")); }
	static UFunction* GetPrototypeWithPointAndMetadata() { FGCScopeGuard Guard; return FindObject<UFunction>(StaticClass(), TEXT("PrototypeWithPointAndMetadata")); }
