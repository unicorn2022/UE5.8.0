...
struct FPCGISMComponentBuilderParams
{
	...
	int32 NumCustomDataFloats = 0;
	TConstArrayView<float> CustomPrimitiveData;
	FPCGCrc SettingsCrc;
	FPCGCrc DataCrc;
	bool bAllowDescriptorChanges = true;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		uint32 Result = GetTypeHash(Key.Descriptor);
		Result = HashCombine(Result, 1 + Key.NumCustomDataFloats);
		Result = HashCombine(Result, (Key.bAllowDescriptorChanges ? 2 : 1));
		Result = GetArrayHash(Key.CustomPrimitiveData.GetData(), Key.CustomPrimitiveData.Num(), Result);

		return Result;
	}

	inline bool operator==(const FPCGISMComponentBuilderParams& Other) const
	{
		return Descriptor == Other.Descriptor
			&& NumCustomDataFloats == Other.NumCustomDataFloats
			&& bAllowDescriptorChanges == Other.bAllowDescriptorChanges
			&& Algo::Compare(CustomPrimitiveData, Other.CustomPrimitiveData);
	}
};
...
public:
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	static PCG_API UInstancedStaticMeshComponent* GetOrCreateISMC(AActor* InTargetActor, UPCGComponent* SourceComponent, uint64 SettingsUID, const FPCGISMCBuilderParameters& Params);
	static PCG_API UPCGManagedISMComponent* GetOrCreateManagedISMC(AActor* InTargetActor, UPCGComponent* SourceComponent, uint64 SettingsUID, const FPCGISMCBuilderParameters& Params);
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	UE_DEPRECATED(5.6, "Use signature with no SettingsUID. Make sure to fill Params.SettingsCRC to enable component reuse.")
	static PCG_API UInstancedStaticMeshComponent* GetOrCreateISMC(AActor* InTargetActor, UPCGComponent* SourceComponent, uint64 SettingsUID, const FPCGISMComponentBuilderParams& Params);
	UE_DEPRECATED(5.6, "Use signature with no SettingsUID. Make sure to fill Params.SettingsCRC to enable component reuse.")	
	static PCG_API UPCGManagedISMComponent* GetOrCreateManagedISMC(AActor* InTargetActor, UPCGComponent* SourceComponent, uint64 SettingsUID, const FPCGISMComponentBuilderParams& Params);
	UE_DEPRECATED(5.6, "Use signature with no SettingsUID. Make sure to fill Params.SettingsCRC to enable component reuse.")
	static PCG_API USplineMeshComponent* GetOrCreateSplineMeshComponent(AActor* InTargetActor, UPCGComponent* SourceComponent, uint64 SettingsUID, const FPCGSplineMeshComponentBuilderParameters& Params);
	UE_DEPRECATED(5.6, "Use signature with no SettingsUID. Make sure to fill Params.SettingsCRC to enable component reuse.")
	static PCG_API UPCGManagedSplineMeshComponent* GetOrCreateManagedSplineMeshComponent(AActor* InTargetActor, UPCGComponent* SourceComponent, uint64 SettingsUID, const FPCGSplineMeshComponentBuilderParameters& Params);
	
	static PCG_API UInstancedStaticMeshComponent* GetOrCreateISMC(AActor* InTargetActor, IPCGGraphExecutionSource* ExecutionSource, const FPCGISMComponentBuilderParams& Params, FPCGContext* OptionalContext = nullptr);
	static PCG_API UPCGManagedISMComponent* GetOrCreateManagedISMC(AActor* InTargetActor, IPCGGraphExecutionSource* ExecutionSource, const FPCGISMComponentBuilderParams& Params, FPCGContext* OptionalContext = nullptr);
	static PCG_API UPCGManagedISKMComponent* GetOrCreateManagedABMC(AActor* InTargetActor, IPCGGraphExecutionSource* ExecutionSource, const FPCGSkinnedMeshComponentBuilderParams& Params, FPCGContext* OptionalContext = nullptr);
	static PCG_API USplineMeshComponent* GetOrCreateSplineMeshComponent(AActor* InTargetActor, IPCGGraphExecutionSource* ExecutionSource, const FPCGSplineMeshComponentBuilderParameters& Params, FPCGContext* OptionalContext = nullptr);
	static PCG_API UPCGManagedSplineMeshComponent* GetOrCreateManagedSplineMeshComponent(AActor* InTargetActor, IPCGGraphExecutionSource* ExecutionSource, const FPCGSplineMeshComponentBuilderParameters& Params, FPCGContext* OptionalContext = nullptr);

	static PCG_API bool DeleteActors(UWorld* World, const TArray<TSoftObjectPtr<AActor>>& ActorsToDelete);
	...
	*/
	static PCG_API void ForEachActorInLevel(ULevel* Level, TSubclassOf<AActor> ActorClass, TFunctionRef<bool(AActor*)> Callback);
	...
	* @param Callback Function to call with the found actor. Needs to return a bool, to indicate if it needs to continue (true = yes)
	*/
	static PCG_API void ForEachActorInWorld(UWorld* World, TSubclassOf<AActor> ActorClass, TFunctionRef<bool(AActor*)> Callback);
	...
	* @param Parent Optional parent to attach to.
	*/
	static PCG_API AActor* SpawnDefaultActor(UWorld* World, ULevel* Level, TSubclassOf<AActor> ActorClass, FName BaseName, const FTransform& Transform, AActor* Parent = nullptr);
	...
	* @param Parent Optional parent to attach to.
	*/
	static PCG_API AActor* SpawnDefaultActor(UWorld* World, ULevel* Level, TSubclassOf<AActor> ActorClass, const FTransform& Transform, const FActorSpawnParameters& SpawnParams, AActor* Parent = nullptr);
	...
	*/
	static PCG_API AActor* SpawnDefaultActor(const FSpawnDefaultActorParams& Params);
	
	/**
	 * Return the grid cell coordinates on the PCG partition grid given a position and the grid size.
	 */
	static PCG_API FIntVector GetCellCoord(FVector InPosition, uint32 InGridSize, bool bUse2DGrid);

	/**
	 * Return the center of the PCG partition grid cell given a position and the grid size.
	 */
	static PCG_API FVector GetCellCenter(const FVector& InPosition, uint32 InGridSize, bool bUse2DGrid);
	...
	static PCG_API int ComputeHashFromActorTagsAndReference(const AActor* InActor, const bool bIncludeTags, const bool bIncludeActorReference);

#if WITH_EDITOR
	static UPackage* CreatePreviewPackage(ULevel* InLevel, const FString& InActorName);
#endif

	/** Returns all isolated actors that originate from the given actor. */
	static TArray<APCGIsolatedActor*> GetIsolatedActorsOriginatingFrom(const AActor* InActor);
	
	/** Extracts (like Clear PCG Link) content from the actor, assuming it has a PCG component, into a PCG Isolated Actor. */
	static PCG_API AActor* IsolateFromActor(AActor* Actor, const TArray<FName>& RequiredTags);

	/** Extracts (like Clear PCG Link) content from the actor, assuming it has a PCG component, into a provided target actor or a PCG Isolated Actor created by this call. */
	static PCG_API AActor* IsolateFromActor(AActor* Actor, const FPCGMoveResourceParams& Params);

	/** Finds all execution sources using the passed in param as context for search. */
	static PCG_API TArray<IPCGGraphExecutionSource*> GetExecutionSourcesFromSelectionKey(const FPCGGetExecutionSourcesFromSelectionKeyParams& InParams);
};
