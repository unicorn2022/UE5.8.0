...
public:
	...
	/** Build a query structure to call RetrieveResultsFromCache */
	UE_API bool BuildCacheParams(IPCGGraphCache* Cache, const UPCGNode* Node, const FPCGDataCollection& Input, IPCGGraphExecutionSource* ExecutionSource, FPCGGetFromCacheParams& OutCacheParams) const;

	/** Gather input data (pre-context creation) and tries to retrieve matching data from the cache if the element is cacheable. */
	UE_API EPCGCachingStatus RetrieveResultsFromCache(IPCGGraphCache* Cache, const FPCGGetFromCacheParams& InCacheParams, FPCGDataCollection& Output) const;

	UE_DEPRECATED(5.8, "Use BuildCacheParam/RetrieveResultsFromCache combination instead")
	...
	*/
	...
	/** Which phase to apply the data overrides to the internal data. */
	UE_API virtual EPCGDataOverridePhase GetDataOverridePhase() const { return EPCGDataOverridePhase::PostExecute; }
