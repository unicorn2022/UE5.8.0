...
struct FPCGPinDefaultValueInfo
{
	...
	struct FInitParams
	{
		...
		EPCGMetadataTypes InitialType = EPCGMetadataTypes::Double;
		FString InitialValue = PCG::Private::MetadataTraits<double>::ZeroValueString();
		TArray<EPCGMetadataTypes> AvailableTypes;
	};

	FPCGPinDefaultValueInfo() = default;

	explicit FPCGPinDefaultValueInfo(FInitParams InParams)
		: CurrentType(InParams.InitialType)
		, AvailableTypes(MoveTemp(InParams.AvailableTypes))
		, InitialType(InParams.InitialType)
		, InitialValue(MoveTemp(InParams.InitialValue))
	{}
#endif // WITH_EDITOR

	UPROPERTY()
	EPCGMetadataTypes CurrentType = EPCGMetadataTypes::Unknown;

#if WITH_EDITORONLY_DATA
	/** Metadata types this pin accepts. Empty = all types are supported. */
	UPROPERTY()
	TArray<EPCGMetadataTypes> AvailableTypes;

	UPROPERTY()
	EPCGMetadataTypes InitialType = EPCGMetadataTypes::Unknown;

	UPROPERTY()
	FString InitialValue;

	UPROPERTY()
	EPCGSettingDefaultValueExtraFlags ExtraFlags = EPCGSettingDefaultValueExtraFlags::None;
...
public:
	...
	/** Whether the default value can be toggled on/off by the user. */
	virtual bool CanDefaultValueBeToggled() const { return true; }
...
protected:
	// @todo_pcg: GetPinInitialDefaultValueType should likely be Editor Only
