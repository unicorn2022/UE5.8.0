...
namespace PCGObjectPropertyOverrideHelpers
{
	...
	struct FApplyOverrideParams
	{
		TConstArrayView<FPCGObjectPropertyOverrideDescription> ObjectPropertyOverrideDescriptions;
		TConstArrayView<TPair<UObject*, int32>> TargetObjectToOverrideIndices;
		TConstArrayView<const UPCGData*> OverridesData;
		FPCGContext* OptionalContext = nullptr;
		bool bPropagateEditChangeEvent = false;
		bool bOneDataSetPerElement = false;
	};

	PCG_API void ApplyOverrides(const FApplyOverrideParams& InParams);

	// @todo_pcg in another pass
	// UE_DEPRECATED(5.8, "Use the Params version")
	PCG_API void ApplyOverrides(const TArray<FPCGObjectPropertyOverrideDescription>& InObjectPropertyOverrideDescriptions, const TArray<TPair<UObject*, int32>>& TargetObjectToOverrideIndices, FName OverridesPinLabel, int32 InputDataIndex, FPCGContext* Context, bool bPropagateEditChangeEvent = false);
}
...
struct FPCGObjectSingleOverride
{
	...
	{
		const FPCGAttributePropertySelector* InputSelector = nullptr;
		FStringView OutputProperty;
		const UStruct* TemplateClass = nullptr;
		const UPCGData* SourceData = nullptr;
		FPCGContext* OptionalContext = nullptr;
		bool bComputePropertyEditChain = false;
		bool bAllowedToUseAllSourceDataForContainers = false;
	};
	...
	PCG_API void Initialize(const FInitializeParams& InParams);

	// @todo_pcg in another pass
	//UE_DEPRECATED(5.8, "Use Params version")
...
struct FPCGObjectOverrides
{
	...
	struct FInitializeParams
	{
		TConstArrayView<FPCGObjectPropertyOverrideDescription> OverrideDescriptions;
		T* TemplateObject = nullptr;
		const UPCGData* SourceData;
		FPCGContext* OptionalContext = nullptr;
		bool bInPropagateEditChangeEvent = false;
		bool bAllowedToUseAllSourceDataForContainers = false;
	};
	...
	void Initialize(const TArray<FPCGObjectPropertyOverrideDescription>& OverrideDescriptions, T* TemplateObject, const UPCGData* SourceData, FPCGContext* Context, bool bInPropagateEditChangeEvent = false)
	{
		...
		{
			.OverrideDescriptions = OverrideDescriptions,
			.TemplateObject = TemplateObject,
			.SourceData = SourceData,
			.OptionalContext = Context,
			.bInPropagateEditChangeEvent = bInPropagateEditChangeEvent
		};
	...
	void Initialize(const FInitializeParams& InParams)
	{
		bInitialized = false;

		if (!InParams.TemplateObject)
		{
			PCGLog::LogErrorOnGraph(NSLOCTEXT("PCGObjectPropertyOverride", "InitializeOverrideFailedNoObject", "Failed to initialize property overrides. No template object was provided."), InParams.OptionalContext);
			return;
		}

		CachedOutputObject = nullptr; 
		OutputKey = FPCGAttributeAccessorKeysSingleObjectPtr<T>(InParams.TemplateObject);
		bPropagateEditChangeEvent = InParams.bInPropagateEditChangeEvent;

		const UStruct* ClassObject = nullptr;
		if constexpr (TIsDerivedFrom<T, UObject>::Value)
		{
			ClassObject = InParams.TemplateObject->GetClass();
			CachedOutputObject = bPropagateEditChangeEvent ? InParams.TemplateObject : nullptr;
		}
		else
		{
			ClassObject = InParams.TemplateObject->StaticStruct();
		}

		ObjectSingleOverrides.Empty(InParams.OverrideDescriptions.Num());

		for (int32 i = 0; i < InParams.OverrideDescriptions.Num(); ++i)
		{
			const FPCGAttributePropertyInputSelector InputSelector = InParams.OverrideDescriptions[i].InputSource.CopyAndFixLast(InParams.SourceData);
			const FString& OutputProperty = InParams.OverrideDescriptions[i].PropertyTarget;
			...
			FPCGObjectSingleOverride::FInitializeParams Params =
			{
				...
				.TemplateClass = ClassObject,
				.SourceData = InParams.SourceData,
				.OptionalContext = InParams.OptionalContext,
				.bComputePropertyEditChain = bPropagateEditChangeEvent,
			...
			{
				PCGLog::LogErrorOnGraph(FText::Format(NSLOCTEXT("PCGObjectPropertyOverride", "InitializeOverrideFailed", "Failed to initialize override '{0}' for property {1} on object '{2}'."), InputSelector.GetDisplayText(), FText::FromString(OutputProperty), FText::FromName(ClassObject->GetFName())), InParams.OptionalContext);
			}
		}
