...
public:
	...
	UE_API bool Update(const FStringView NewValue);

	// For deprecation of Update(const FString&)
	// @todo_pcg in another pass
	// UE_DEPRECATED(5.8, "Make sure to use the string view version")
	UE_API bool Update(const char* NewValue);
	...
	template <typename T = FPCGAttributePropertySelector>
	static T CreateSelectorFromString(const FStringView String)
	{
		static_assert(std::is_base_of_v<FPCGAttributePropertySelector, T>, "The type must be of base class 'FPCGAttributePropertySelector'");
		T Selector;
