...
* There is an assumption that no vertex will be set by 2 different threads at the same time and that the polygon itself should not be modified (by adding/removing vertices)
* while the accessor is getting/setting values.
		...
		else
		{
			...
			for (int i = 0; i < InValues.Num(); ++i)
			{
				...
				if constexpr (Target == EPCGPolygon2DProperties::Position)
				{
					...
					Data->UpdateVertexInPlace(FVector2d(LocalPosition.X, LocalPosition.Y), HoleIndex, SegmentIndex);
				}
				else if constexpr (Target == EPCGPolygon2DProperties::LocalPosition)
				{
					static_assert(std::is_same_v<Type, FVector2d>);
					Data->UpdateVertexInPlace(InValues[i], HoleIndex, SegmentIndex);
				}
				else
				{
					// Pitfall static assert
