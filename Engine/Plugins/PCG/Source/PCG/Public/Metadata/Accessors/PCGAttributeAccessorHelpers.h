...
{
	/** Returns true if the property is supported by PCG types and has a conversion to a Metadata type. For deprecation, the default behavior will be to return the old accessors. For the new ones, set bUseGenericAccessor to true. */
	PCG_API bool IsPropertyAccessorSupported(const FProperty* InProperty, bool bUseGenericAccessor = false);

	/** Returns true if the property is supported by PCG types and has a conversion to a Metadata type.  For deprecation, the default behavior will be to return the old accessors. For the new ones, set bUseGenericAccessor to true. */
	PCG_API bool IsPropertyAccessorSupported(const FName InPropertyName, const UStruct* InStruct, bool bUseGenericAccessor = false);
	...
	PCG_API bool IsPropertyAccessorChainSupported(const TArray<FName>& InPropertyNames, const UStruct* InStruct, bool bUseGenericAccessor = false);
	...
	PCG_API EPCGMetadataTypes GetMetadataTypeForProperty(const FProperty* InProperty, bool bUseGenericAccessor = false);
	...
	PCG_API TUniquePtr<IPCGAttributeAccessor> CreatePropertyAccessor(const FProperty* InProperty, bool bUseGenericAccessor = false);
	...
	PCG_API TUniquePtr<IPCGAttributeAccessor> CreatePropertyAccessor(const FName InPropertyName, const UStruct* InStruct, bool bUseGenericAccessor = false);

	/**
	 * Create a chain accessor for the given properties. Last property needs to be supported by PCG (cf. IsPropertyAccessorSupported). For deprecation, the default behavior will be to return the old accessors. For the new ones, set bUseGenericAccessor to true.
	 * USE WITH CAUTION: There is no validation that the properties are related and may corrupt memory if used incorrectly.
	 */
	PCG_API TUniquePtr<IPCGAttributeAccessor> CreatePropertyChainAccessor(TArray<const FProperty*>&& InProperties, bool bUseGenericAccessor = false);
	...
	PCG_API TUniquePtr<IPCGAttributeAccessor> CreatePropertyChainAccessor(const TArray<FName>& InPropertyNames, const UStruct* InStruct, bool bUseGenericAccessor = false);
	...
	struct AccessorParamResult
	{
		...
		bool bHasNoEntry = false;

		// If the param is in a container, we'll need the keys
	...
	* or the types doesn't match with the Matching Accessor or provided type, create the attribute using the Matching Accessor. The default value of this new attribute will either
	* be the first value get from the provided keys (if provided), the default value of the matching accessor (if it is an attribute) or the zero value (if no matching accessor or not supporting default values).
	* Note that InData must not be null if the selector points to an attribute,
	* but in the case of properties, it either has to be the appropriate type or null.
	* Make sure to update your selector before-hand if you want to support "@Source". Otherwise the creation will fail.
	*/
	...
	{
		UPCGData* InData = nullptr;
		const FPCGAttributePropertySelector* InSelector = nullptr;

		const IPCGAttributeAccessor* InMatchingAccessor = nullptr;
		const IPCGAttributeAccessorKeys* InMatchingKeysForDefaultValue = nullptr;
		// Discarded if InMatchingAccessor is not null.
		FPCGMetadataAttributeDesc InExpectedDesc;

		EPCGAttributeAccessorFlags InTypeMatching = EPCGAttributeAccessorFlags::AllowBroadcastAndConstructible;
		bool bQuiet = false;
	};

	PCG_API TUniquePtr<IPCGAttributeAccessor> CreateAccessorWithAttributeCreation(const FPCGCreateAccessorWithAttributeCreationParams& InParams);

	UE_DEPRECATED(5.8, "Use the version with the params")
	...
	*/
	...
	PCG_API TUniquePtr<const IPCGAttributeAccessorKeys> CreateConstKeys(const FPCGMetadataAttributeBase* InAttribute);
	PCG_API TUniquePtr<IPCGAttributeAccessorKeys> CreateKeys(FPCGMetadataAttributeBase* InAttribute);
