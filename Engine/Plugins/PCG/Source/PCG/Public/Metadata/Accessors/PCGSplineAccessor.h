...
/**
* Templated accessor for global spline data. Note that only the spline transform value is writable.
* Keys supported: PCGSplineData, FPCGSplineStruct
*/
template <typename T, EPCGSplineDataProperties Target>
class FPCGSplineAccessorV2 : public IPCGAttributeAccessorT<FPCGSplineAccessorV2<T, Target>>, IPCGPropertyChain
{
public:
	// The underlying type is a quat if we target the rotation, otherwise a vector
	using Type = T;
	using Super = IPCGAttributeAccessorT<FPCGSplineAccessorV2<T, Target>>;
	
	FPCGSplineAccessorV2(const FStructProperty* InProperty, TArray<const FProperty*>&& ExtraProperties = {})
		: Super(/*bInReadOnly=*/ Target != EPCGSplineDataProperties::SplineTransform)
		, IPCGPropertyChain(InProperty, std::move(ExtraProperties))
	{
		static_assert(PCG::Private::IsPCGType<Type>());		
		check(InProperty && InProperty->Struct->IsChildOf<FPCGSplineStruct>());
	...
	bool GetRangeImpl(TArrayView<Type> OutValues, int32 Index, const IPCGAttributeAccessorKeys& Keys) const
	{
		...
		for (int32 i = 0; i < OutValues.Num(); ++i)
		{
			if constexpr (Target == EPCGSplineDataProperties::SplineTransform)
			{
				using SplineTransformType = decltype(SplineStruct.GetTransform());
				static_assert(std::is_same_v<Type, SplineTransformType>);
				OutValues[i] = SplineStruct.GetTransform();
			}
			else if constexpr (Target == EPCGSplineDataProperties::IsClosed)
			{
				using SplineIsClosedType = decltype(SplineStruct.IsClosedLoop());
				static_assert(std::is_same_v<Type, SplineIsClosedType>);
				OutValues[i] = SplineStruct.IsClosedLoop();
			}
			else if constexpr (Target == EPCGSplineDataProperties::Length)
			{
				using SplineLengthType = decltype(SplineStruct.GetSplineLength());
				static_assert(std::is_same_v<Type, SplineLengthType>);
				OutValues[i] = SplineStruct.GetSplineLength();
			}
			else
			{
				// Pitfall static assert
	...
	bool SetRangeImpl(TArrayView<const Type> InValues, int32 Index, IPCGAttributeAccessorKeys& Keys, EPCGAttributeAccessorFlags)
	{
		...
		else
		{
			...
			if constexpr (std::is_same_v<Type, decltype(SplineStruct.Transform)>)
			{
				for (int32 i = 0; i < InValues.Num(); ++i)
				{
...
enum class UE_DEPRECATED(5.8, "Not used anymore. Replaced by EPCGSplineDataProperties") EPCGSplineAccessorTarget
{
	Transform,
	ClosedLoop
};
...
// Consteval function to convert the old enum to the new one, to be used in a template (hence the consteval)
template <EPCGSplineAccessorTarget Target>
consteval EPCGSplineDataProperties DeprecationSplineAccessorTarget()
{
	if constexpr (Target == EPCGSplineAccessorTarget::Transform)
	{
		return EPCGSplineDataProperties::SplineTransform;
	}
	else if constexpr (Target == EPCGSplineAccessorTarget::ClosedLoop)
	{
		return EPCGSplineDataProperties::IsClosed;
	}
	else
	{
		// Pitfall assert
		static_assert(!std::is_same_v<decltype(Target), EPCGSplineAccessorTarget>);
		return {};
	}
}

template <typename T, EPCGSplineAccessorTarget Target>
class UE_DEPRECATED(5.8, "Use FPCGSplineAccessorV2") FPCGSplineAccessor : public FPCGSplineAccessorV2<T, DeprecationSplineAccessorTarget<Target>()>
{
	using FPCGSplineAccessorV2<T, DeprecationSplineAccessorTarget<Target>()>::FPCGSplineAccessorV2;
};
