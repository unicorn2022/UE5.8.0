...
void FFastBuildJobProcessor::MonitorFastBuildProcess()
{
	...
	if (FPlatformProcess::IsProcRunning(BuildProcessHandle))
	{
		...
		if (STDOutput.Len() > 0)
		{
			...
			{
				UE_LOGF(LogFastBuildJobProcessor, Verbose, "%ls", *Line);
			}
		}
	...
	{	
		...
		switch (ReturnCode)
		{
			...
			case FBUILD_ERROR_LOADING_BFF:
				UE_LOGF(LogFastBuildJobProcessor, Fatal, "There was an error parsing FASTBuild script. This might be due to platform dependencies having files with the same names (Code %d).", ReturnCode);
				break;
	        ...
	        case FBUILD_WRAPPER_CRASHED:
	            ...
	            UE_LOGF(LogFastBuildJobProcessor, Fatal, "An error occurred during an FASTBuild shader compilation job. One or more of the shader compile worker processes exited unexpectedly (Code %d).", ReturnCode);
				break;
			default:
				UE_LOGF(LogFastBuildJobProcessor, Display, "An unknown error occurred during an FASTBuild shader compilation job (Code %d). Incomplete shader jobs will be redispatched in another FASTBuild build.", ReturnCode);
				break;
			case FBUILD_ALREADY_RUNNING:
				UE_LOGF(LogFastBuildJobProcessor, Display, "FASTBuild is already running. Incomplete shader jobs will be redispatched in another FASTBuild build.");
				break;
		}
...
void FFastBuildJobProcessor::SubmitPendingJobs()
{
	...
	if (!BuildProcessHandle.IsValid())
	{
		UE_LOGF(LogFastBuildJobProcessor, Fatal, "Failed to launch %ls during shader compilation.", *FastBuildUtilities::GetFastBuildExecutablePath());
	}
