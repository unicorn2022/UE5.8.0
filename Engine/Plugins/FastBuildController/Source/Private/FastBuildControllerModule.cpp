...
bool FFastBuildControllerModule::IsSupported()
{
	...
	if (FASTBuildControllerVariables::Enabled == 1)
	{
		...
		{
			UE_LOGF(LogFastBuildController, Warning, "Cannot use FASTBuild Shader Compiler as FASTBuild is not found: %ls",
		...
		if (bCopyMetalCompilerToIntermediateDir)
		{
			...
			if (SrcDir.Len() == 0)
			{
				UE_LOGF(LogFastBuildController, Warning, "Cannot use FASTBuild Shader Compiler as Metal shader compiler could not be found");
				FASTBuildControllerVariables::Enabled = 0;
			}
			else
			{
				...
				if (!bCopied)
				{
					UE_LOGF(LogFastBuildController, Warning, "Cannot use FASTBuild Shader Compiler as Metal shader compiler could not be copied to the intermediate folder: %ls -> %ls",
						*SrcDir, *DestDir);
					FASTBuildControllerVariables::Enabled = 0;
				}
			}
		...
		if (FASTBuildControllerVariables::Enabled == 1)
		{
			UE_LOGF(LogFastBuildController, Warning, "FASTBuild Shader Compiler is temporarily disabled on Mac until problems with locating Autogen shader folder are sorted out");
			FASTBuildControllerVariables::Enabled = 0;
		}
#endif // PLATFORM_MAC
	}
...
void FFastBuildControllerModule::CleanWorkingDirectory()
{
	...
	if (!WorkingDirectory.IsEmpty())
	{
		...
		{
			UE_LOGF(LogFastBuildController, Log, "%ls => Failed to delete current working Directory => %ls", ANSI_TO_TCHAR(__FUNCTION__), *RootWorkingDirectory);
		}
	}
	...
	if (!IntermediateShadersDirectory.IsEmpty())
	{
		...
		{
			UE_LOGF(LogFastBuildController, Log, "%ls => Failed to delete directory => %ls", ANSI_TO_TCHAR(__FUNCTION__), *IntermediateShadersDirectory);
		}	
	}
}
