...
namespace Detail
{
		...
		TUniquePtr<Ort::Session>& Session, FString& TempDirForModelWithExternalData)
	{
		...
		{
			UE_LOGF(LogNNERuntimeORT, Error, "Cannot create ORT session: Input model data is empty.");
			return false;
		}
		...
		if (Descriptor.AdditionalDataDescriptors.Num() > 0)
		{
			...
			if (TempDirForModelWithExternalData.IsEmpty())
			{
				...
				if (!FFileHelper::SaveArrayToFile(ModelBuffer, *Filepath))
				{
					...
					UE_LOGF(LogNNERuntimeORT, Error, "Large models are an experimental feature at the moment. Could not write model to disk at %ls.", *Filepath);
					return false;
				}
				...
				for (const FOnnxAdditionalDataDescriptor& AdditionalDataDescriptor : Descriptor.AdditionalDataDescriptors)
				{
					...
					{
						IFileManager::Get().DeleteDirectory(*TempDirForModelWithExternalData, false, true);
						UE_LOGF(LogNNERuntimeORT, Error, "Large models are an experimental feature at the moment. Could not write additional data to disk at %ls.", *AdditionalDataFilename);
						return false;
					}
				}
			}
	...
	bool SetInputTensorShapes(TConstArrayView<NNE::FTensorShape> InInputShapes, TConstArrayView<NNE::FTensorDesc> InputSymbolicTensors, TArray<NNE::FTensorShape> &InputTensorShapes)
	{
		...
		{
			UE_LOGF(LogNNERuntimeORT, Error, "Number of input shapes does not match number of input tensors");
			return false;
		}
		...
		for (int32 i = 0; i < InInputShapes.Num(); ++i)
		{
			...
			{
				UE_LOGF(LogNNERuntimeORT, Error, "Input shape does not match input tensor %ls of index %d", *SymbolicDesc.GetName(), i);
				return false;
			}
		}
...
FModelInstanceORTRunSync<ModelInterface, TensorBinding>::~FModelInstanceORTRunSync()
{
	...
	if (!TempDirForModelWithExternalData.IsEmpty())
	{
		...
		{
			UE_LOGF(LogNNERuntimeORT, Warning, "Large models are an experimental feature at the moment. Could not delete temp directy %ls on model instance destruction.", *TempDirForModelWithExternalData);
		}
	}
}
...
bool FModelInstanceORTRunSync<ModelInterface, TensorBinding>::Init(TConstArrayView64<uint8> ModelData)
{
...
#endif // WITH_EDITOR
	{
		...
		{
			UE_LOGF(LogNNERuntimeORT, Error, "InitializedAndConfigureMembers failed.");
			return false;
		}

		if (!Detail::CreateSession(ModelData, *SessionOptions, *Environment, Session, TempDirForModelWithExternalData))
		{
			UE_LOGF(LogNNERuntimeORT, Error, "Session creation failed.");
			return false;
		}

		if (!ConfigureTensors(true))
		{
			UE_LOGF(LogNNERuntimeORT, Error, "Failed to configure Inputs tensors.");
			return false;
		}
		if (!ConfigureTensors(false))
		{
			UE_LOGF(LogNNERuntimeORT, Error, "Failed to configure Outputs tensors.");
			return false;
		}
	}
#if WITH_EDITOR
	...
	{
		UE_LOGF(LogNNERuntimeORT, Error, "%ls", UTF8_TO_TCHAR(Exception.what()));
		return false;
	}
	catch (...)
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Unknown exception!");
		return false;
	}
#endif // WITH_EDITOR
...
typename ModelInterface::ERunSyncStatus FModelInstanceORTRunSync<ModelInterface, TensorBinding>::RunSync(TConstArrayView<TensorBinding> InInputBindings, TConstArrayView<TensorBinding> InOutputBindings)
{
	...
	if (!Session.IsValid())
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Called without a Session.");
	...
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Input shapes are not set, please call SetInputTensorShapes.");
		return ModelInterface::ERunSyncStatus::Fail;
	}
	...
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Input bindings need to match input tensor descriptor count (got %d, expected %d).", InInputBindings.Num(), InputTensors.Num());
		return ModelInterface::ERunSyncStatus::Fail;
	}
	...
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Output binding can be empty or needs to match output tensor descriptor count (got %d, expected %d).", InOutputBindings.Num(), OutputTensorNames.Num());
		return ModelInterface::ERunSyncStatus::Fail;
	}
...
public:
		...
		if (!TempDirForModelWithExternalData.IsEmpty())
	{
		if (!IFileManager::Get().DeleteDirectory(*TempDirForModelWithExternalData, false, true))
		{
			UE_LOGF(LogNNERuntimeORT, Warning, "Large models are an experimental feature at the moment. FModelInstanceORTDmlRDG could not delete temp directy %ls on model instance destruction.", *TempDirForModelWithExternalData);
		}
	}
	}
...
bool FModelInstanceORTDmlRDG::Init()
{
...
#endif // WITH_EDITOR
	{
		...
		{
			UE_LOGF(LogNNERuntimeORT, Error, "Failed to configure session options for DirectML Execution Provider.");
			return false;
		}
		...
		GetID3D12PlatformDynamicRHI()->RHIRunOnQueue(ED3D12RHIRunOnQueueType::Graphics, [Proxy = Proxy, InputTensorShapes = InputTensorShapes] (ID3D12CommandQueue* D3D12CommandQueue)
		{
			...
			if (!bHasSymbolicInputShapes)
			{
				...
				for (int32 i = 0; i < Proxy->OutputSymbolicTensors.Num(); i++)
				{
					...
					{
						UE_LOGF(LogNNERuntimeORT, Warning, "One or more output tensors contain free dimensions, but input tensors are all concrete!");
						return;
					}
				}
			...
			if (!Proxy->SessionOptions.IsValid())
			{
				UE_LOGF(LogNNERuntimeORT, Error, "Failed to recreate session options!");
				return;
			}
			...
			if (!Detail::CreateSession(Proxy->ModelData->GetView(), *Proxy->SessionOptions, *Proxy->Environment, Proxy->Session, Proxy->TempDirForModelWithExternalData))
			{
				UE_LOGF(LogNNERuntimeORT, Error, "Failed to recreate session!");
				return;
			}
			...
			if (!Proxy->ConfigureTensors(false))
			{
				UE_LOGF(LogNNERuntimeORT, Error, "Failed to configure tensors!");
				return;
			}
			...
			for (int32 i = 0; i < Proxy->OutputSymbolicTensors.Num(); i++)
			{
				...
				else
				{
					for (int32 j = 0; j < SymbolicTensorDesc.GetShape().Rank(); j++)
					{
						...
						{
							UE_LOGF(LogNNERuntimeORT, Warning, "Tensor '%s' has free dimension '%ls'.", Proxy->OutputTensorNames[i], *Proxy->OutputSymbolicDimensionNames[i][j]);
						}
					}

					UE_LOGF(LogNNERuntimeORT, Error, "One or more output tensors contain free dimensions!");
					return;
				}
			}
...
FModelInstanceORTDmlRDG::EEnqueueRDGStatus FModelInstanceORTDmlRDG::EnqueueRDG(FRDGBuilder& GraphBuilder, TConstArrayView<NNE::FTensorBindingRDG> Inputs, TConstArrayView<NNE::FTensorBindingRDG> Outputs)
{
	...
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Input bindings need to match input tensor descriptor count (got %d, expected %d).", Inputs.Num(), InitialInputSymbolicTensors.Num());
		return EEnqueueRDGStatus::Fail;
	}

	if (!Outputs.IsEmpty() && Outputs.Num() != InitialOutputSymbolicTensors.Num())
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Output binding can be empty or needs to match output tensor descriptor count (got %d, expected %d).", Outputs.Num(), InitialOutputSymbolicTensors.Num());
		return EEnqueueRDGStatus::Fail;
	}
		...
		RHICmdList.EnqueueLambda([Proxy = Proxy, InputTensors = InputTensors, InputBuffers = InputBuffers, OutputBuffers = OutputBuffers] (FRHICommandListImmediate& RHICmdList)
		{
			GetID3D12PlatformDynamicRHI()->RHIRunOnQueue(ED3D12RHIRunOnQueueType::Graphics, [Proxy = Proxy, InputTensors = InputTensors, InputBuffers = InputBuffers, OutputBuffers = OutputBuffers] (ID3D12CommandQueue* D3D12CommandQueue)
			{
...
#endif // WITH_EDITOR
				{
					...
					{
						UE_LOGF(LogNNERuntimeORT, Error, "Invalid Session, may be Init() should have been called.");
						return;
					}
				...
				{
					UE_LOGF(LogNNERuntimeORT, Error, "ORT Exception: %ls", UTF8_TO_TCHAR(Exception.what()));
				}
				catch (...)
				{
					UE_LOGF(LogNNERuntimeORT, Error, "ORT Exception: Unknown!");
				}
#endif // WITH_EDITOR
			}, false);
		});
