...
namespace UE::NNERuntimeORT::Private
{
	namespace DllHelper
	{
		bool GetDllHandle(const FString& DllPath, TArray<void*>& DllHandles)
		{
			...
			{
				UE_LOGF(LogNNERuntimeORT, Error, "Failed to find the third party library %ls.", *DllPath);
				return false;
			}
			...
			if (!DllHandle)
			{
				UE_LOGF(LogNNERuntimeORT, Error, "Failed to load the third party library %ls.", *DllPath);
				return false;
			}
...
void FNNERuntimeORTModule::StartupModule()
{
	...
	const FString OrtSharedLibPath = FPaths::Combine(PluginDir, TEXT(UE_STRINGIZE(ONNXRUNTIME_SHAREDLIB_PATH)));

	if (!DllHelper::GetDllHandle(OrtSharedLibPath, DllHandles))
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Failed to load ONNX Runtime shared library. ORT Runtimes won't be available.");
		return;
	}
	...
	const FString DirectMLSharedLibPath = FPaths::Combine(ModuleDir, TEXT(UE_STRINGIZE(DIRECTML_PATH)), TEXT("DirectML.dll"));

	bDirectMLDllLoaded = DllHelper::GetDllHandle(DirectMLSharedLibPath, DllHandles);
	if (!bDirectMLDllLoaded)
	{
		UE_LOGF(LogNNERuntimeORT, Error, "Failed to load DirectML shared library. ORT Dml Runtime won't be available.");
	}
	...
	{
		UE_LOGF(LogNNERuntimeORT, Fatal, "Failed to load ONNX Runtime shared library functions!");
		return;
	}
...
{
	UE_LOGF(LogNNERuntimeORT, Log, "Settings %ls changed: %ls", *InObject->GetName(), *InPropertyChangedEvent.GetPropertyName().ToString());
	UE_LOGF(LogNNERuntimeORT, Warning, "It is recommended to restart the Editor if settings %ls changed! Otherwise they might not be fully applied.", *InObject->GetName());
