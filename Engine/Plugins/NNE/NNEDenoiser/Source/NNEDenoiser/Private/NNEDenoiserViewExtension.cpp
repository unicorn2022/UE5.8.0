	...
	TSharedPtr<ITransferFunction> TransferFunction)
{
	...
	{
		UE_LOGF(LogNNEDenoiser, Log, "Try create model instance with runtime %ls on %ls...", *Pair.Value, *UEnum::GetValueAsString(Pair.Key));
		...
		{
			UE_LOGF(LogNNEDenoiser, Display, "Created model instance with runtime %ls on %ls", *Pair.Value, *UEnum::GetValueAsString(Pair.Key));
			break;
		}
	}
	...
	if (!ModelInstance.IsValid())
	{
		UE_LOGF(LogNNEDenoiser, Error, "Could not create denoiser!");
		return {};
	}
...
FParameters GetParametersValidated(const AssetType& DenoiserAsset)
{
	...
	{
		UE_LOGF(LogNNEDenoiser, Warning, "Tiling alignment should be at least 1!");
		Parameters.TilingConfig.Alignment = 1;
	}

	if (Parameters.TilingConfig.Overlap % Parameters.TilingConfig.Alignment != 0)
	{
		UE_LOGF(LogNNEDenoiser, Warning, "Tiling overlap should be aligned by %d!", Parameters.TilingConfig.Alignment);
	...
	{
		UE_LOGF(LogNNEDenoiser, Warning, "Minimum tile size should be at least overlap + alignment = %d!", Parameters.TilingConfig.Overlap + Parameters.TilingConfig.Alignment);
	...
	{
		UE_LOGF(LogNNEDenoiser, Warning, "Minimum tile size should be aligned by %d!", Parameters.TilingConfig.Alignment);
	...
	{
		UE_LOGF(LogNNEDenoiser, Warning, "Maximum tile size should be at least minimum tile size!");
	...
	{
		UE_LOGF(LogNNEDenoiser, Warning, "Maximum tile size should be aligned by %d!", Parameters.TilingConfig.Alignment);
...
TUniquePtr<FGenericDenoiser> CreateNNEDenoiserFromAsset(const FString& AssetName, EDenoiserRuntimeType RuntimeType, const FString& RuntimeNameOverride, int32 MaximumTileSizeOverride)
{
	...
	{
		UE_LOGF(LogNNEDenoiser, Error, "Asset name not set!");
		return {};
	}
	...
	if (!DenoiserAsset)
	{
		UE_LOGF(LogNNEDenoiser, Error, "Could not load denoiser model data asset!");
		return {};
	}
	...
	if (!ModelData)
	{
		UE_LOGF(LogNNEDenoiser, Error, "Asset does not contain model data!");
		return {};
	}
	UDataTable* InputMappingTable = DenoiserAsset->InputMapping.LoadSynchronous();
	if (InputMappingTable)
	{
		UE_LOGF(LogNNEDenoiser, Log, "Loaded input mapping from %ls", *DenoiserAsset->InputMapping.GetAssetName());
	}
	UDataTable* OutputMappingTable = DenoiserAsset->OutputMapping.LoadSynchronous();
	if (OutputMappingTable)
	{
		UE_LOGF(LogNNEDenoiser, Log, "Loaded output mapping from %ls", *DenoiserAsset->OutputMapping.GetAssetName());
	}
	...
	if (MaximumTileSizeOverride > -1)
	{
		if (MaximumTileSizeOverride < Parameters.TilingConfig.MaxSize)
		{
			...
			{
				UE_LOGF(LogNNEDenoiser, Display, "Maximum tile size override %d smaller than minimum tile size %d supported by asset!", MaximumTileSizeOverride, Parameters.TilingConfig.MinSize);
			}
			...
			{
				UE_LOGF(LogNNEDenoiser, Log, "Applied maximum tile size override %d to assets maximum tile size %d.", NewMaxSize, Parameters.TilingConfig.MaxSize);
...
void FViewExtension::BeginRenderViewFamily(FSceneViewFamily& InViewFamily)
{
	...
	if (bIsActive != bIsActiveBefore)
	{
		if (bIsActive)
		{
			UE_LOGF(LogNNEDenoiser, Log, "Activating...");
		}
		else
		{
			UE_LOGF(LogNNEDenoiser, Log, "Deactivating and free all resources...");
		}
	}
	...
	if (Denoiser.IsValid())
	{
		UE_LOGF(LogNNEDenoiser, Log, "Create denoiser from asset %ls...", *AssetName);

		DenoiserToSwap = MakeUnique<FPathTracingDenoiser>(MoveTemp(Denoiser));
	}
	else
	{
		UE_LOGF(LogNNEDenoiser, Error, "Could not create denoiser from asset %ls!", *AssetName);
	}
	...
	{
		UE_LOGF(LogNNEDenoiser, Log, "Create temporal denoiser from asset %ls...", *TemporalAssetName);

		SpatialTemporalDenoiserToSwap = MakeUnique<FPathTracingSpatialTemporalDenoiser>(MoveTemp(TemporalDenoiser));
	}
	else
	{
		UE_LOGF(LogNNEDenoiser, Error, "Could not create temporal denoiser from asset %ls!", *TemporalAssetName);
	}
}
