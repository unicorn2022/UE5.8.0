...
namespace UE::NNEDenoiser::Private
{
	...
	TUniquePtr<FModelInstanceCPU> FModelInstanceCPU::Make(UNNEModelData& ModelData, const FString& RuntimeName)
	{
		...
		if (!Runtime.IsValid())
		{
			UE_LOGF(LogNNEDenoiser, Log, "Could not create model instance. No CPU runtime '%ls' found. Valid CPU runtimes are: ", *RuntimeName);
		...
		{
			UE_LOGF(LogNNEDenoiser, Log, "%ls on CPU can not create model", *RuntimeName);
			return {};
		}
		...
		if (!Model.IsValid())
		{
			UE_LOGF(LogNNEDenoiser, Log, "Could not create model using %ls on CPU", *RuntimeName);
			return {};
		}
		...
		{
			UE_LOGF(LogNNEDenoiser, Log, "Could not create model instance using %ls on CPU", *RuntimeName);
			return {};
		}
	...
	FModelInstanceCPU::EEnqueueRDGStatus FModelInstanceCPU::EnqueueRDG(FRDGBuilder &GraphBuilder, TConstArrayView<NNE::FTensorBindingRDG> Inputs, TConstArrayView<NNE::FTensorBindingRDG> Outputs)
	{
		...
		[this, DenoiserParameters](FRHICommandListImmediate& RHICmdList)
		{
			...
			{
				ScratchInputBuffers[Idx].SetNumUninitialized(DenoiserParameters->InputBuffers[Idx].GetBuffer()->GetSize());
			...
			{
				ScratchOutputBuffers[Idx].SetNumUninitialized(DenoiserParameters->OutputBuffers[Idx].GetBuffer()->GetSize());
			...
			for (int32 Idx = 0; Idx < DenoiserParameters->InputBuffers.Num(); Idx++)
			{
				...
				const int32 Count = static_cast<int32>(DenoiserParameters->InputBuffers[Idx]->GetSize());
				CopyBufferFromGPUToCPU(RHICmdList, Buffer, Count, ScratchInputBuffers[Idx]);
			}
			...
			for (int32 Idx = 0; Idx < DenoiserParameters->OutputBuffers.Num(); Idx++)
			{
				...
				const int32 Count = static_cast<int32>(DenoiserParameters->OutputBuffers[Idx]->GetSize());
				CopyBufferFromCPUToGPU(RHICmdList, ScratchOutputBuffers[Idx], Count, Buffer);
			}

#if WITH_EDITOR
		FilterExecuteTime += FPlatformTime::Cycles64();
		const double FilterExecuteTimeMS = 1000.0 * FPlatformTime::ToSeconds64(FilterExecuteTime);
		UE_LOGF(LogNNEDenoiser, Log, "Denoised on CPU in %.2f ms", FilterExecuteTimeMS);
#endif
		});
