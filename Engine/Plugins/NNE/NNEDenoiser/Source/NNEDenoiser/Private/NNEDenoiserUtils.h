...
namespace UE::NNEDenoiser::Private
{
	...
	bool IsTensorShapeValid(TConstArrayView<IntType> ShapeData, TConstArrayView<int32> RequiredShapeData, const FString& Label)
	{
		...
		{
			UE_LOGF(LogNNEDenoiser, Error, "%ls has wrong rank (expected %d, got %d)!", *Label, RequiredShapeData.Num(), ShapeData.Num())
			return false;
		}
		...
		for (int32 I = 0; I < RequiredShapeData.Num(); I++)
		{
			...
			{
				UE_LOGF(LogNNEDenoiser, Error, "%ls does not have required shape (expected %d, got %d)!", *Label, RequiredShapeData[I], ShapeData[I])
				return false;
			}
		}
