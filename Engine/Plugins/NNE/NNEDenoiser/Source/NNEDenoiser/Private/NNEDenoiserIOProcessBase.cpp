	...
	const TArray<FChannelMapping>& ChannelMapping)
{
	...
	{
		UE_LOGF(LogNNEDenoiser, Warning, "AddReadInputPass: ChannelMapping is empty. Nothing to do!");
		return;
	}
	
	if (ChannelMapping.Num() >= FMappedCopyConstants::MAX_NUM_MAPPED_CHANNELS)
	{
		UE_LOGF(LogNNEDenoiser, Warning, "AddReadInputPass: ChannelMapping has too many entries!");
		return;
	}
...
bool FInputProcessBase::Validate(const IModelInstance& ModelInstance, FIntPoint Extent) const
{
	...
	UE_LOGF(LogNNEDenoiser, Log, "Validate model for extent %dx%d...", Extent.X, Extent.Y);
	...
	if (Extent == FIntPoint(-1, -1))
	{
		...
		{
			UE_LOGF(LogNNEDenoiser, Error, "Wrong number of inputs (expected %d, got %d)!", InputLayout.Num(), InputTensorDescs.Num())
			return false;
		}
	...
	else
	{
		...
		{
			UE_LOGF(LogNNEDenoiser, Error, "Wrong number of inputs (expected %d, got %d)!", InputLayout.Num(), InputTensorShapes.Num())
			return false;
		}
...
bool FInputProcessBase::Prepare(IModelInstance& ModelInstance, FIntPoint Extent) const
{
	...
	UE_LOGF(LogNNEDenoiser, Log, "Configure model for extent %dx%d...", Extent.X, Extent.Y);

	TConstArrayView<UE::NNE::FTensorDesc> InputTensorDescs = ModelInstance.GetInputTensorDescs();
	
	UE_LOGF(LogNNEDenoiser, Log, "Input shapes (set):");
	...
	for (int32 Idx = 0; Idx < InputTensorDescs.Num(); Idx++)
	{
		...
		UE_LOGF(LogNNEDenoiser, Log, "%d: (%d, %d, %d, %d)", Idx, ModelInputShape.X, ModelInputShape.Y, ModelInputShape.Z, ModelInputShape.W)
	}
	...
	{
		UE_LOGF(LogNNEDenoiser, Error, "Could not configure model instance (ModelInstance.SetInputTensorShapes() failed)!")
		return false;
	}
...
bool FOutputProcessBase::Validate(const IModelInstance& ModelInstance, FIntPoint Extent) const
{
	...
	{
		UE_LOGF(LogNNEDenoiser, Error, "Wrong number of outputs (expected %d, got %d)!", OutputLayout.Num(), OutputTensorDescs.Num())
		return false;
	}
	...
	{
		UE_LOGF(LogNNEDenoiser, Error, "Wrong number of output shapes!");
		return false;
	}

	if (OutputShapes.IsEmpty() && OutputLayout.Num() > 0)
	{
		UE_LOGF(LogNNEDenoiser, Log, "Output shapes not resolved yet");
		return true;
	}
	
	UE_LOGF(LogNNEDenoiser, Log, "Output shapes (resolved):");
	...
	for (int32 Idx = 0; Idx < OutputTensorDescs.Num(); Idx++)
	{
		...
		UE_LOGF(LogNNEDenoiser, Log, "%d: (%d, %d, %d, %d)", Idx, OutputShapeData[0], OutputShapeData[1], OutputShapeData[2], OutputShapeData[3]);
	}
