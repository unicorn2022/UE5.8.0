...
namespace MassInsightsUI
{
	...
	public:
		SLATE_BEGIN_ARGS(SFragmentListRow) {}
			SLATE_ARGUMENT(const UE::Mass::Trace::FFragmentInfo*, FragmentInfo)
		SLATE_END_ARGS()

		void Construct(const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView);
		TSharedRef<SWidget> GenerateWidgetForColumn(const FName& InColumnName) override;
	private:
		const UE::Mass::Trace::FFragmentInfo* FragmentInfo = nullptr;
	};
		...
		else if (InColumnName == NAME_COLUMN_FragmentType)
		{
			...
			return SNew(SButton)
					.Text_Lambda([this]()
					{
						...
						{
						case EFragmentType::Fragment: return LOCTEXT("Fragment", "Fragment");
						case EFragmentType::Tag: return LOCTEXT("Tag", "Tag");
						case EFragmentType::Shared: return LOCTEXT("Shared", "Shared");
						case EFragmentType::Sparse: return LOCTEXT("Sparse", "Sparse");
						case EFragmentType::ConstShared: return LOCTEXT("ConstShared", "Const Shared");
						case EFragmentType::SparseTag: return LOCTEXT("SparseTag", "Sparse Tag");
						case EFragmentType::Unknown: // Fallthrough
						default:
				...
				[this]()
				{
					switch (FragmentInfo->Type)
					{
						case EFragmentType::Fragment: return FColor(10, 120, 180);
						case EFragmentType::Sparse: return FColor(10, 160, 140);
						case EFragmentType::ConstShared: return FColor(140, 80, 180);
						case EFragmentType::SparseTag: return FColor(120, 140, 10);
						case EFragmentType::Tag: // Fallthrough
						case EFragmentType::Shared: // Fallthrough
						case EFragmentType::Unknown: // Fallthrough
						default:
	...
	void SArchetypeDetails::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
	{
			...
			&& Session.IsValid())
		{
			...
			const IMassTraceProvider& Provider = ReadMassTraceProvider(*Session);
			{
				...
				const FArchetypeInfo* ArchetypeInfo = Provider.FindArchetypeById(ArchetypeID);
