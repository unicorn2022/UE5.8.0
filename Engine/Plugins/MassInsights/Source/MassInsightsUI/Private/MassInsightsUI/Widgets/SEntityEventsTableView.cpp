...
namespace MassInsightsUI
{
	...
	TSharedRef<SWidget> SEntityEventsTableRow::GenerateWidgetForColumn(const FName& InColumnName)
	{
		...
		const IMassTraceProvider& Provider = ReadMassTraceProvider(*AnalysisSession);

		TValueOrError<FEntityEventRecord, void> Event = MakeError();
		{
	...
	SEntityEventsTableView::FEventNode::FEventNode(uint64 InProviderEntityIndex)
		: FBaseTreeNode(false /* bInIsGroup */)
		, ProviderEntityIndex(InProviderEntityIndex)
	{
	}

	const FText SEntityEventsTableView::FEventNode::GetDisplayName() const
	{
		return FText::FromString(FString::Printf(TEXT("event_%llu"), ProviderEntityIndex));
	}
			...
			public:
				virtual const TOptional<FTableCellValue> GetValue(const FTableColumn& Column, const FBaseTreeNode& Node) const override
				{
					...
					if (InsightsManager.IsValid())
					{
						...
						{
							const IMassTraceProvider& Provider = ReadMassTraceProvider(*CurrentSession);
			
							{
								TraceServices::FProviderReadScopeLock ProviderReadScopeLock(Provider);
								TValueOrError<FEntityEventRecord, void> EventData = Provider.GetEntityEvent(ProviderEventIndex);
	...
	void SEntityEventsTableView::IncrementalUpdate()
	{
		...
		if (Session.IsValid())
		{
			...
			// Note: Would be better if this were async instead of on Tick
			{
				...
				Provider.EnumerateEntityEvents(NextEventIndex, MaximumEventsToProcess, [this, &NewNextEventIndex](const FEntityEventRecord& Event, uint64 ProviderEventIndex)
				{
					++NewNextEventIndex;
					...
					TSharedPtr<FEventNode> EventNode = MakeShared<FEventNode>(ProviderEventIndex);
