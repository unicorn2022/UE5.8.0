...
void FPacketCaptureArchive::SerializePacket(void* PacketData, uint32& PacketSize)
{
	...
	if (IsLoading())
	{
		...
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Bad PacketSize value '%i' in loading packet capture file", PacketSize);
			SetError();
...
void FPacketCaptureArchive::AppendPacketFile(FPacketCaptureArchive& InPacketFile)
{
	...
	while (InPacketFile.Tell() < InPacketFile.TotalSize())
	{
		...
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Error reading packet capture data. Skipping rest of file.");
...
uint32 FPacketCaptureArchive::GetPacketCount()
{
	...
	else
	{
		...
		else
		{
			...
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Skipping packet file because it is malformed.");
			}
		}
	}
