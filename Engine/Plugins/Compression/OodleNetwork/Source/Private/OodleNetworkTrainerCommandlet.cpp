	...
	if (Tokens.Num() > 0)
	{
			...
			if (Tokens.Num() > 2)
			{
				...
				else
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Error,
							"Need to specify a target directory, e.g: MergePackets OutFile All Dir");
				}
			}
			else
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Error,
						"Error parsing 'MergePackets' parameters. Must specify an output file, and packet files to merge");
			}
		}
		else if (MainCmd == TEXT("GenerateDictionary"))
		{
			if (Tokens.Num() > 3)
			{
				...
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Error,
							"Need to specify a target directory, e.g: GenerateDictionary OutFile All Dir");
				}
			}
			else
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Error,
						"Error parsing 'GenerateDictionary' parameters. Must specify an output file, packet files as input, and a changelist (specify \"all\" to ignore changelist number)");
			}
		}
		...
		else if (MainCmd == TEXT("DebugDump"))
		{
			...
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Error,
						"Error parsing 'DebugDump' parameters. Must specify an output directory, and a source directory");
			}
		}
#endif
		else
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Unrecognized sub-command '%ls'", *MainCmd);
		}
	}
	else
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Error parsing main commandlet sub-command");

		// @todo #JohnB: Add a 'help' command, with documentation output
	}
#else
	UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Oodle unavailable.");
#endif
...
bool UOodleNetworkTrainerCommandlet::HandleEnable()
{
	...
	if (!bAlreadyEnabled)
	{
		...
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Initialized first-run settings for Oodle (will start in Trainer mode).");
	}
	else
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Error, "The Oodle PacketHandler is already enabled.");
	}
...
bool UOodleNetworkTrainerCommandlet::HandleMergePackets(FString OutputCapFile, const TArray<FString>& MergeList)
{
	...
	if (bSuccess)
	{
		...
		if (OutArc != nullptr)
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Merging files into output file: %ls", *OutputCapFile);
			
			{
				...
				for (TMap<FArchive*, FString>::TConstIterator It(MergeMap); It; ++It)
				{
...

					{
						...
						UE_LOGF(OodleNetworkHandlerComponentLog, Log, "    Merging: %ls", *It.Value());
				...
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Error appending packet file.");
					bSuccess = false;
				}
				...
				if (bSuccess)
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Merge packets success.");
				}
			}
		...
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Could not create output file.");
			bSuccess = false;
		}
	}
...
bool UOodleNetworkTrainerCommandlet::HandleDebugDumpPackets(FString OutputDirectory, FString SourceDirectory, const TArray<FString>& DumpList)
{
	...
	if (bSuccess)
	{
		for (TMap<FArchive*, FString>::TConstIterator It(SourceMap); It; ++It)
		{
			...
			UE_LOGF(OodleNetworkHandlerComponentLog, Log, "    Dumping: %ls", *It.Value());
			...
			if (OutputArc != nullptr)
			{
				...
				UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Successfully dumped packet data to file: %ls", *OutputFile);
	...
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Failed to get a list of capture files for directory: %ls", *SourceDirectory);
	}
...
bool UOodleNetworkTrainerCommandlet::GenerateDictionary(bool bIsInput, int32 ChangelistNumber)
{
	...
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Generating dictionaries for %ls folder", *InputOrOutput);

		FString OutDic = FPaths::Combine(*GOodleContentDir, *FString::Printf(TEXT("%s%s.udic"), FApp::GetProjectName(), *InputOrOutput));

		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Generating dictionary for folder '%ls', outputting to: %ls", *InputOrOutput, *OutDic);
		...
		if (bSuccess)
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Successfully generated dictionary.");
		}
		else
		{
			...
			if (bTriggerFail)
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Failed to generate dictionary for folder '%ls', aborting.", *InputOrOutput);

				return bSuccess;
			}
			else
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Warning,
					"Failed to generate dictionary for folder '%ls', continuing to next folder", *InputOrOutput);
			}
		}
	}
	else
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Could not find any packet captures for %ls.", *InputOrOutput);
	}
	return bSuccess;
}
...
bool UOodleNetworkTrainerCommandlet::GetMergeMapFromList(const TArray<FString>& FileList, TMap<FArchive*, FString>& OutMergeMap)
{
	...
	for (FString CurMergeFile : FileList)
	{
		...
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Could not read packet capture file: %ls", *CurMergeFile);
	...
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Could not find any packet capture files!");
...
bool UOodleNetworkTrainerCommandlet::VerifyOutputFile(FString OutputFile)
{
	...
	if (FPaths::FileExists(OutputFile))
	{
		if (FApp::IsUnattended())
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Output file '%ls' already exists. Aborting (can't overwrite in unattended).",
					*OutputFile);
...
bool FOodleNetworkDictionaryGenerator::InitGenerator()
{
	...
	if (HashTableSize < 17 || HashTableSize > 21)
	{
		...
		UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Hash table size is usually between 17-21. Clamping to: %i", HashTableSize);
	}
	...
	if (DictionarySize < (1024 * 1024) || DictionarySize > OODLENETWORK1_MAX_DICTIONARY_SIZE)
	{
		 ...
		 UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Dictionary size is usually between 1MB and %iMB. Clamping to: %iMB",
	...
	else
	{
		if (DictionaryTrials < 2)
		{
			...
			UE_LOGF(OodleNetworkHandlerComponentLog, Warning,
					"DictionaryTrials must be at least two. Clamping to: 2. Specify -NoTrials to disable.");
		}
		...
		if (TrialRandomness < 50 || TrialRandomness > 200)
		{
			...
			UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Dictionary trial randomness is usually between 50-200%%. Clamping to: %i",
					TrialRandomness);
		}
		...
		if (TrialGenerations < 1)
		{
			...
			UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Dictionary trial generations must be at least one. Clamping to: 1.");
		}
		else if (TrialGenerations > 1)
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Dictionary trial generations higher than 1, can take a long time.");
		}
	}


	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Oodle trainer parameters:");
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- HashTableSize: %i", HashTableSize);
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- DictionarySize: %i (~%iMB)", DictionarySize, (DictionarySize / (1024 * 1024)));
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- Performing Trials: %ls", (bNoTrials ? TEXT("No") : TEXT("Yes")));

	if (!bNoTrials)
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- DictionaryTrials: %i", DictionaryTrials);
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- TrialRandomness: %i", TrialRandomness);
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- TrialGenerations: %i", TrialGenerations);
	}

	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- WriteDictionaryVersion: %i", bWriteV5Dictionaries ? 5 : OODLE2NET_VERSION_MAJOR);
	...
	if (bSuccess && FPaths::FileExists(OutputDictionaryFile))
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Failed to delete old dictionary file: %ls", *OutputDictionaryFile);
		}
	}
...
bool FOodleNetworkDictionaryGenerator::ReadPackets(const TArray<FString>& InputCaptureFiles)
{
	...
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Error, "No capture files to process!");
	}
	...
	if (bSuccess)
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Error, "No packets gathered from any capture files!");;
		}
	}
	...
	for (TUniquePtr<FPacketCaptureArchive>& CurArc : BoundArchives)
	{
		...
		for (uint32 PacketInCurArcIndex = 0; PacketInCurArcIndex < PacketCountInCurArc; ++PacketInCurArcIndex)
		{
			...
			if (CurArc->IsError())
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Error serializing packet from packet capture archive. Skipping.");
			...
			if (!bDictionaryTestOverflow && ListIdx == 1)
			{
				...
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Log, "DictionaryTestPackets overflowed into TrainerPackets (normal).");
				}
			}
	...
	if (bSuccess)
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Error, "No packets successfully processed from capture files!");
		}
	}
...
bool FOodleNetworkDictionaryGenerator::GenerateAndWriteDictionary()
{
	...
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Beginning dictionary generation...");

	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- DictionaryPackets: %i, Size: %u (~%uMB)",
			DictionaryPackets.Num(), DictionaryPacketBytes, (DictionaryPacketBytes / (1024 * 1024)));

	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- DictionaryTestPackets: %i, Size: %u (~%uMB), Overflowed: %ls",
	...
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- TrainerPackets: %i, Size: %u (~%uMB)",
			TrainerPackets.Num(), TrainerPacketBytes, (TrainerPacketBytes / (1024 * 1024)));

	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Dictionary generation may take a very long time (up to 7 mins for 1GB of packets).");
	...
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Dictionary generation complete. Beginning Oodle state training...");
	...
	if (OutArc != nullptr)
	{
...

		{
			...
			if (bSuccess)
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Successfully processed packet captures, and wrote dictionary file.");
			}
			else
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Error writing dictionary file.");
			}
		}
	...
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Failed to create output dictionary file '%ls'", *OutputDictionaryFile);
	}


	if (bSuccess && bCompressionTest)
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Testing packet compression results...");
		...
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Compression test results:");

		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- CompressionTestPackets: %i, Size: %u (~%uMB)",
				CompressionTestPackets.Num(), CompressionTestPacketBytes, (CompressionTestPacketBytes / (1024 * 1024)));

		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- Uncompressed: %llu (~%lluMB), Compressed: %llu (~%lluMB)", TotalUncompressed,
				(TotalUncompressed / (1024 * 1024)), TotalCompressed, (TotalCompressed / (1024 * 1024)));

		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "- Total Savings: %f%%",
...
bool FOodleNetworkDictionaryGenerator::DebugDumpPackets()
{
	...
	if (OutPackets != nullptr)
	{
		...
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Successfully dumped packet data to file: %ls", *PacketsFile);
