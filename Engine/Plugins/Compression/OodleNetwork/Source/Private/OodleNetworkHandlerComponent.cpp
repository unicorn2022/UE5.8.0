...
#define UE_OODLE_LIGHTWEIGHT_TIME_GUARD_DECLARE(Name, ThresholdMS) \
	const double UE_JOIN(__TimeGuard_ThresholdMS_, Name) = ThresholdMS; \
	double UE_JOIN(__TimeGuard_MSElapsed_, Name) = 0.0;

/** To be placed inside the scope, directly before the measured code */
#define UE_OODLE_LIGHTWEIGHT_TIME_GUARD_BEGIN(Name) \
	uint64 UE_JOIN(__TimeGuard_StartCycles_, Name) = \
		(UE_JOIN(__TimeGuard_ThresholdMS_, Name) > 0.0 && UE::Oodle::GOodleTimeguardLimit > 0) ? FPlatformTime::Cycles64() : 0;

/** To be placed inside the scope, directly after the measured code */
#define UE_OODLE_LIGHTWEIGHT_TIME_GUARD_END(Name) \
	if (UE_JOIN(__TimeGuard_ThresholdMS_, Name) > 0.0 && UE::Oodle::GOodleTimeguardLimit > 0) \
	{ \
		UE_JOIN(__TimeGuard_MSElapsed_, Name) = \
			FPlatformTime::ToMilliseconds64(FPlatformTime::Cycles64() - UE_JOIN(__TimeGuard_StartCycles_, Name)); \
	}

/** To be placed outside and after the scope of the measured code */
#define UE_OODLE_LIGHTWEIGHT_TIME_GUARD_REPORT(Name) \
	if (UE_JOIN(__TimeGuard_MSElapsed_, Name) > UE_JOIN(__TimeGuard_ThresholdMS_, Name)) \
	{ \
		UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "OodleNetworkHandlerComponent '%ls' took %.2fms!", TEXT(#Name), \
				UE_JOIN(__TimeGuard_MSElapsed_, Name)); \
...
{
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Oodle Assert: File: %ls, Line: %i, Function: %ls, Message: %ls", UTF8_TO_TCHAR(File),
...
void OodleNetworkHandlerComponent::Initialize()
{
	...
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "OodleNetworkHandlerComponent::Initialize");
	#endif
	...
	#if OODLE_HANDLER_VERBOSE_LOG
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "OodleNetworkHandlerComponent::bEnableOodle = %ls", bEnableOodle ? TEXT("yes") : TEXT("no"));
	#endif

	if (!bEnableOodle && bOodleForceEnable)
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Force-enabling Oodle from commandline.");
		bEnableOodle = true;
	}
	...
	auto SetEnableModeFromStr = [&EnableModeEnum](EOodleNetworkEnableMode& OutMode, FString ModeStr) -> bool
		{
			...
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Failed to parse EOodleNetworkEnableMode value '%ls'", *ModeStr);
			}
	...
	{
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Force-enabling 'bUseDictionaryIfPresent', due to -Oodle on commandline.");
	...
	if (bEnableOodle)
	{
		...
		#if OODLE_HANDLER_VERBOSE_LOG
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "OodleNetworkHandlerComponent::bCaptureMode = %ls", bCaptureMode ? TEXT("yes") : TEXT("no"));
		#endif
		...
		if (bCaptureMode)
		{
			...
			UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Enabling Oodle capture mode: [%ls]. Random number is: %d, Capture Percentage is: %d, random number must be less than capture percentage to capture.", bShouldEnableCapture ? TEXT("TRUE") : TEXT("FALSE"), RandNum, CapturePercentage);
		...
		// in shipping build
		#if OODLE_HANDLER_VERBOSE_LOG
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "build config does not allow -OodleCapturing");
		#endif
...
void OodleNetworkHandlerComponent::InitializeDictionaries()
{
	...
	else
	{
		...
		if (bCaptureMode)
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Failed to load Oodle dictionaries. Continuing due to capture mode.");
		}
		else
#endif
		{
			//LowLevelFatalError(TEXT("Failed to load Oodle dictionaries."));
  			...
  			UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Failed to load Oodle dictionaries.");
  			bInitializedDictionaries = false;
		}
	}
}
...
void OodleNetworkHandlerComponent::InitializeDictionary(FString FilePath, TSharedPtr<FOodleNetworkDictionary>& OutDictionary)
{
	...
	if (DictionaryRef == nullptr)
	{
		...
		if (ReadArc != nullptr)
		{
			...
			if (!BoundArc.IsError())
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Log, "Loading dictionary file: %ls", *FilePath);
				...
				if ( ! UncompactOk )
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Error, "OodleNetwork1UDP_State_Uncompact failed!");
			...
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "Error loading dictionary file: %ls", *FilePath);
...
bool OodleNetworkHandlerComponent::GetDictionaryPaths(FString& OutServerDictionary, FString& OutClientDictionary, bool bFailFatal/*=true*/)
{
	...
	if (bSuccess && (ServerDictionaryPath.Len() <= 0 || ClientDictionaryPath.Len() <= 0))
	{
		...
		{
			UE_LOGF(OodleNetworkHandlerComponentLog, Warning, "%ls", Msg);
		}
														...
														bool bTestOnly/*=false*/)
{
	...
	else
	{
		...
		if (DictionaryList.Num() > 0)
		{
			...
			if (!bTestOnly)
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Log,
						"Searched for Oodle dictionary files, and selected the following non-default dictionaries:");
			}
			...
			else
			{
				...
				if (!bTestOnly)
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Log, "WARNING: Using the same dictionary for both server/client!");
				}
			}

			if (!bTestOnly)
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Log, "   Server: %ls", *OutServerDictionary);
				UE_LOGF(OodleNetworkHandlerComponentLog, Log, "   Client: %ls", *OutClientDictionary);
			}
		}
	}
...
void OodleNetworkHandlerComponent::InitializePacketLogs()
{
	...
	if (bCaptureMode && Handler->Mode == UE::Handler::Mode::Server && InPacketLog == nullptr && OutPacketLog == nullptr)
	{
		...
		#if OODLE_HANDLER_VERBOSE_LOG
		UE_LOGF(OodleNetworkHandlerComponentLog, Log, "ReadOutputLogDirectory : %ls", *ReadOutputLogDirectory );
		#endif
		...
		if (PacketBytes > 0 && PacketData[0] != 0)
		{
			...
			if (CurDict != nullptr)
			{
				...
				if (DecompressedLength >= 252)
				{
					...
					{
#if !UE_BUILD_SHIPPING
						UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Couldn't decode uncompressed length");
#endif
						return;
					}
				...
				if (DecompressedLength < MAX_OODLE_PACKET_BYTES)
				{
					...
					bool bSuccess;
					{
						...
						{
#if !UE_BUILD_SHIPPING
							UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Error decoding Oodle network data.");
#endif
					...
					if (bSuccess)
					{
						...
						if (bCaptureMode && Handler->Mode == UE::Handler::Mode::Server && InPacketLog != nullptr)
						{
							...
							UE::Net::SetDiscardTimeguardMeasurement(true);
#endif
				...
				{
#if !UE_BUILD_SHIPPING
					UE_LOGF(OodleNetworkHandlerComponentLog, Error,
							"Received packet with DecompressedLength (%i) >= MAX_OODLE_PACKET_SIZE", DecompressedLength);
#endif
	...
	if (bEnableOodle)
	{
		...
		if (CurDict != nullptr && !bSkipCompression)
		{
			...
			if (bWithinBitBounds)
			{
				...
				{
					UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Compressed packet larger than uncompressed packet! (%u vs %u)",
			...
			{
				UE_LOGF(OodleNetworkHandlerComponentLog, Error, "Failed to compress packet of size: %u bytes (%u bits)",
...
bool OodleNetworkHandlerComponent::SupportsParallelConnectionOutgoing() const
{
	//Use of NetAnalytics means that we need Multithreading support for this component to support parallel connections
#if NET_ANALYTICS_MULTITHREADING
	return true;
#else
	return false;
#endif //NET_ANALYTICS_MULTITHREADING
}
...
static bool OodleExec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	#if OODLE_HANDLER_VERBOSE_LOG
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "OodleExec" );
	#endif
...
void FOodleComponentModuleInterface::StartupModule()
{
	...
	#if OODLE_HANDLER_VERBOSE_LOG
	UE_LOGF(OodleNetworkHandlerComponentLog, Log, "FOodleComponentModuleInterface::StartupModule" );
	#endif
