...
void UInterchangeMegascansPipeline::SetupMeshLod(UInterchangeStaticMeshFactoryNode* StaticMeshFactoryNode, const UInterchangeSceneNode* SceneNode, int32 LodIndex)
{
	...
	if (const UInterchangeMeshNode* MeshNode = Cast<UInterchangeMeshNode>(BaseNodeContainer->GetNode(MeshNodeUid)))
	{
		...
		if (CommonMeshesProperties.IsValid())
		{
			bBakeMeshes = CommonMeshesProperties->bBakeMeshes;
			bBakePivotMeshes = CommonMeshesProperties->bBakePivotMeshes;
		}
		...
		FTransform CompleteSceneNodeTransform = UE::Interchange::Private::MeshHelper::CalculateCompleteSceneNodeTransform(BaseNodeContainer, SceneNode, bBakeMeshes, bBakePivotMeshes, GlobalOffsetTransform);
		StaticMeshLodDataNode->AddLODMeshData(FInterchangeLODMeshData(MeshNodeUid, CompleteSceneNodeTransform));
		StaticMeshFactoryNode->AddTargetNodeUid(MeshNodeUid);
...
void UInterchangeMegascansPipeline::SetupStaticMeshParams(const UInterchangeStaticMeshFactoryNode* StaticMeshFactoryNode, const TSharedPtr<FJsonObject>& MeshParams)
{
	if (const TSharedPtr<FJsonObject>* LodInfo; MeshParams->TryGetObjectField(TEXT("lod"), LodInfo))
	{
		...
		BaseNodeContainer->RemoveFactoryNode(StaticMeshFactoryUid);
	}
}
