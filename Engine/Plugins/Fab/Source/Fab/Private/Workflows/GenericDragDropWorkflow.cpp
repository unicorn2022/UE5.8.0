...
void FGenericDragDropWorkflow::Execute()
{
	...
	TWeakObjectPtr<UFabBrowserApi> WeakApi = FFabBrowser::GetBrowserApi();
	UFabBrowserApi* FabBrowserApi = WeakApi.Get();
	if (!FabBrowserApi)
	{
		return;
	}

	auto OnDragInfoSuccess = [this, WeakApi](const FString& InDownloadUrl, const FFabAssetMetadata& AssetMetadata)
	{
		if (AssetId == AssetMetadata.AssetId)
		{
			...
			{
				if (UFabBrowserApi* Api = WeakApi.Get())
				{
					Api->RemoveSignedUrlHandle(SignedUrlHandle);
				}
	...
	SignedUrlHandle = FabBrowserApi->AddSignedUrlCallback(OnDragInfoSuccess);
}
