...
#define UNIT_LOG_OBJ(UnitLogObj, UnitLogTypeFlags, Format, ...) \
	...
	{ \
		...
		UE_LOGF(LogUnitTest, Error, "%ls: %ls", (UnitTestRef != nullptr ? *(UnitTestRef->GetUnitTestName()) : TEXT("nullptr")), \
				*FString::Printf(Format, ##__VA_ARGS__)); \
	} \
	else if ((OptionalFlags(UnitLogTypeFlags) & ELogType::StatusWarning) == ELogType::StatusWarning) \
	{ \
		UUnitTest* UnitTestRef = UnitLogObj != nullptr ? UnitLogObj->GetUnitTest() : nullptr; \
		UE_LOGF(LogUnitTest, Warning, "%ls: %ls", (UnitTestRef != nullptr ? *(UnitTestRef->GetUnitTestName()) : TEXT("nullptr")), \
				*FString::Printf(Format, ##__VA_ARGS__)); \
	} \
	UNIT_LOG_VOID_END();
		...
		auto StatusLogWrap = [&](FUnitLogInterface* InUnitLogObj) \
			{ \
				...
				{ \
					...
					{ \
						UE_LOGF(LogUnitTest, Error, "%ls: %ls", *SourceUnitTest->GetUnitTestName(), \
								*FString::Printf(Format, ##__VA_ARGS__)); \
					} \
					else \
					{ \
				...
				{ \
					if (SourceUnitTest != nullptr) \
					{ \
						UE_LOGF(LogUnitTest, Warning, "%ls: %ls", *SourceUnitTest->GetUnitTestName(), \
								*FString::Printf(Format, ##__VA_ARGS__)); \
					} \
					else \
					{ \
