...
#define UNIT_ASSERT(Condition) \
	if (!(Condition)) \
	{ \
		UE_LOG(LogUnitTest, Error, TEXT(UE_STRINGIZE(__FILE__)) TEXT(": Line: ") TEXT(UE_STRINGIZE(__LINE__)) \
				TEXT(":")); \
		UE_LOG(LogUnitTest, Error, TEXT("Condition '(") TEXT(UE_STRINGIZE(Condition)) TEXT(")' failed")); \
