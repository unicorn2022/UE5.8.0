...
void UUnitTestManager::Initialize()
{
	...
	if (MaxUnitTestCount == 0)
	{
		...
		UE_LOGF(LogUnitTest, Log, "Creating initial unit test config file");
				...
				if (PurgeLogsDays != INDEX_NONE || MaxLogFilesOnDisk != INDEX_NONE)
				{
					...
					if (PurgeLogsDays != INDEX_NONE)
					{
						for (TMap<FString, FDateTime>::TIterator It(DirList); It; ++It)
						{
							...
							{
								UE_LOGF(LogUnitTest, Log, "Deleting old unit test log directory: %ls", *It.Key());
...
void UUnitTestManager::DumpStatus(bool bForce/*=false*/)
{
	...
	if (bForce || !bLastDumpWasBlank || !bCurDumpIsBlank)
	{
		...
		for (UUnitTest* CurUnitTest : ActiveUnitTests)
		{
			...
			STATUS_LOG(, TEXT("     - (%s) %s (Memory usage: %lluMB)"), *CurUnitTest->GetUnitTestType(), *CurUnitTest->GetUnitTestName(),
					(CurUnitTest->CurrentMemoryUsage / 1048576));
		}

		STATUS_LOG(, TEXT("- Total unit test memory usage: %zuMB"), (TotalMemoryUsage / 1048576));
...
bool UUnitTestManager::Exec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	else if (UnitTestName == TEXT("detector"))
	{
		...
		if (DetectorClass == TEXT("FFrameProfiler"))
		{
			...
			{
				UE_LOGF(LogUnitTest, Log, "TargetEvent (%ls) must be set and FramePercentThreshold (%u) must be non-zero",
						*TargetEvent, FramePercentThreshold);
			}
#else
			UE_LOGF(LogUnitTest, Log, "Can't use FFrameProfiler, stats not enable during compile.");
#endif
		}
		else
		{
			UE_LOGF(LogUnitTest, Log, "Unknown detector class '%ls'", *DetectorClass);
		}
	...
	else if (UnitTestName == TEXT("abort"))
	{
		...
		{
			UE_LOGF(LogUnitTest, Log, "Unit test abort command, must be called within a specific unit test window.");
		}
	...
	else if (UnitTestName == TEXT("restart"))
	{
		...
		if (RestartUnitTest != nullptr)
		{
			...
			{
				UE_LOGF(LogUnitTest, Log, "Unit test does not support resetting, can't restart unit test.");
			}
		}
		else
		{
			UE_LOGF(LogUnitTest, Log, "Unit test restart command, must be called within a specific unit test window.");
		}
	...
	else if (UnitTestName == TEXT("debug"))
	{
		...
		{
			UE_LOGF(LogUnitTest, Log, "Unit test 'debug' command, must be called from within a specific unit test window.");
		}
	...
	else if (UnitTestName == TEXT("all"))
	{
		...
		{
			UE_LOGF(LogUnitTest, Warning, "DO NOT USE -FORCELOGFLUSH ALONGSIDE 'UnitTest All' - IT KILLS PERFORMANCE.")
		}
