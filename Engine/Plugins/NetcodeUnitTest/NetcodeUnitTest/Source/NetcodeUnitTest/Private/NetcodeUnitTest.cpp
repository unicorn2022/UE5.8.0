...
public:
	...
	virtual void StartupModule() override
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("LogCommand="), TraceStr) && TraceStr.Len() > 0)
		{
			...
			{
				UE_LOGF(LogUnitTest, Warning, "LogCommand commandline parameter requires the format: -LogCommand=\"LogLine=Command\"");
			}
		}
	}
	...
	static void OnWorldCreated(UWorld* UnrealWorld, const UWorld::InitializationValues IVS)
	{
		...
		if (GEngine != nullptr)
		{
			...
			if (!bNUTActorPresent)
			{
				UE_LOGF(LogUnitTest, Log, "NUTActor not present in RuntimeServerActors - adding this");
	...
	static void DumpRPC(AActor* Actor, UFunction* Function, void* Parameters, bool& bBlockRPC)
	{
		...
		if (bContainsMatch)
		{
			...
			UE_LOGF(LogUnitTest, Log, "Received RPC '%ls' for actor '%ls'", *FuncName, *Actor->GetFullName());

			if (FuncParms.Len() > 0)
			{
				UE_LOGF(LogUnitTest, Log, "     '%ls' parameters: %ls", *FuncName, *FuncParms);
			}
		}
	}
};
