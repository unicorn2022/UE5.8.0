...
void UUnitTestChannel::Init(UNetConnection* InConnection, int32 InChIndex, EChannelCreateFlags CreateFlags)
{
	...
	if (ChName == NAME_None)
	{
		if (InChIndex != 0)
		{
			UE_LOGF(LogUnitTest, Warning, "Unit test channel type was NAME_None, for non-control channel");
		}
	...
	if (!OpenAcked)
	{
		...
		for (FOutBunch* Out=OutRec; Out; Out=Out->Next)
		{
			if (!Out->ReceivedAck)
			{
				...
				if (Wait > 1.0)
				{
					UE_LOGF(LogUnitTest, Log, "UnitTestChannel %i ack timeout); resending %i...", ChIndex, Out->ChSequence);
