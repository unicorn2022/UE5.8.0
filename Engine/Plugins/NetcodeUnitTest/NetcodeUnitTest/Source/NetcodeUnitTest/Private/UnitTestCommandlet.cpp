...
int32 UUnitTestCommandlet::Main(const FString& Params)
{
...
#if IS_MONOLITHIC
	UE_LOGF(LogUnitTest, Log, "NetcodeUnitTest commandlet not currently supported in static/monolithic builds. "
		"You must disable the externs in SlateOpenGLExtensions.cpp to hack-unblock monolithic builds.");
	...
	if (!IsEngineExitRequested())
	{
		...
		TSharedPtr<SViewport> SlateViewport;
		TSharedPtr<FSceneViewport> SlateViewportInterface;
		...
		if (GEngine->GameViewport == nullptr)
		{
			...
			{
				// Create viewport ecosystem so that unit tests which test these don't encounter nullptrs. 
				...
				SlateViewportInterface = FSceneViewport::Create(NewViewport, SAssignNew(SlateViewport, SViewport));
