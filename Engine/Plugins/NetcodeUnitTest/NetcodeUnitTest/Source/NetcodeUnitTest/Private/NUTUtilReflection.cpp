...
{
	UE_LOGF(LogUnitTest, Log, "FVMReflection Dump:");

	UE_LOGF(LogUnitTest, Log, "     - BaseAddress: %ls", (BaseAddress != NULL ? TEXT("Valid") : TEXT("NULL")));

	UE_LOGF(LogUnitTest, Log, "     - FieldInstance: %ls",
			(FieldInstance.IsValid() ? *FieldInstance.GetFullName() : TEXT("NULL")));
	UE_LOGF(LogUnitTest, Log, "     - FieldAddress: %ls", (FieldAddress != NULL ? TEXT("Valid") : TEXT("NULL")));
	UE_LOGF(LogUnitTest, Log, "     - bVerifiedFieldType: %i", (int32)bVerifiedFieldType);
	UE_LOGF(LogUnitTest, Log, "     - bSkipFieldVerification: %i", (int32)bSkipFieldVerification);
	UE_LOGF(LogUnitTest, Log, "     - bSetArrayElement: %i", (int32)bSetArrayElement);

	UE_LOGF(LogUnitTest, Log, "     - bNextActionMustBeCast: %i", (int32)!!bNextActionMustBeCast);
	UE_LOGF(LogUnitTest, Log, "     - NextActionError: %ls", *NextActionError);

	UE_LOGF(LogUnitTest, Log, "     - bIsError: %i", (int32)!!bIsError);
	UE_LOGF(LogUnitTest, Log, "     - bOutError: %ls", (bOutError != NULL ? TEXT("Valid") : TEXT("NULL")));
	...
	UE_LOGF(LogUnitTest, Log, "     - History: %ls", *FullHistory);
	UE_LOGF(LogUnitTest, Log, "     - OutHistoryPtr: %ls", (OutHistoryPtr != NULL ? TEXT("Valid") : TEXT("NULL")));

	UE_LOGF(LogUnitTest, Log, "     - WarnLevel: %i (%ls)",
...
void FVMReflection::SetError(FString InError, bool bCastError/*=false*/)
{
	if (!bIsError)
	{
		...
		if (WarnLevel == EVMRefWarning::Warn)
		{
			...
			UE_LOGF(LogUnitTest, Log, "Reflection Err: History dump: %ls", *CurHistory);
		}
	}
}
