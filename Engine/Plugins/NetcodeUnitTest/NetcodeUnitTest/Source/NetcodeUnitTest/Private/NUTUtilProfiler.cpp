...
void FFrameProfiler::OnNewFrame(int64 Frame)
{
	...
	if (TotalFrameTime > 0)
	{
...
#if 0
		UE_LOGF(LogUnitTest, Log, "Targeted stat found: Duration: %u, FrameTime: %f",
				TotalDuration,
				FramePercent);

		UE_LOGF(LogUnitTest, Log, "TotalFrametime: %llu, Duration: %u", TotalFrameTime, TotalDuration);
#endif
		...
		if ((FramePercent - (float)FramePercentThreshold) > 0.f)
		{
			...
			UE_LOGF(LogUnitTest, Log, "Detected event '%ls' breaching FramePercentThreshold (%u).", *TargetEvent.ToString(),
