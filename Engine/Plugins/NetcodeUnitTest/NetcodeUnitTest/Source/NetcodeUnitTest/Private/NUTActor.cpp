...
bool ANUTActor::NotifyControlMessage(UNetConnection* Connection, uint8 MessageType, FInBunch& Bunch)
{
	...
	if (MessageType == NMT_NUTControl)
	{
		...
		if (FNetControlMessage<NMT_NUTControl>::Receive(Bunch, CmdType, Command))
		{
			...
			{
				UE_LOGF(LogUnitTest, Log, "NMT_NUTControl: Executing command: %ls", *Command);
				...
				UE_LOGF(LogUnitTest, Log, "NMT_NUTControl: Command result: %ls", *CmdResult);
			...
			else if (CmdType == ENUTControlCommand::CommandResult_Failed || CmdType == ENUTControlCommand::CommandResult_Success)
			{
				...
				if (bCmdSuccess)
				{
					UE_LOGF(LogUnitTest, Log, "NMT_NUTControl: Got command result:");
					UE_LOGF(LogUnitTest, Log, "%ls", *Command);
				}
				else
				{
					UE_LOGF(LogUnitTest, Log, "NMT_NUTControl: Failed to execute command");
				}
			}
			// Ping request
			...
			else if (CmdType == ENUTControlCommand::Summon)
			{
				...
				if (SpawnClass != nullptr)
				{
					...
					if (NewActor != nullptr)
					{
						UE_LOGF(LogUnitTest, Log, "Successfully summoned actor of class '%ls'", *SpawnClassName);

						if (bForceBeginPlay && !NewActor->HasActorBegunPlay())
						{
							UE_LOGF(LogUnitTest, Log, "Forcing call to 'BeginPlay' on newly spawned actor.");
						...
						if (bGameplayDebuggerHack)
						{
							...
							{
								UE_LOGF(LogUnitTest, Log, "WARNING: Failed to find 'LocalPlayerOwner'. Unit test broken.");
							}
					...
					{
						UE_LOGF(LogUnitTest, Log, "SpawnActor failed for class '%ls'", *Command);
					}
				}
				else
				{
					UE_LOGF(LogUnitTest, Log, "Could not find actor class '%ls'", *Command);
				}
			}
			...
			{
#if PLATFORM_WINDOWS
				UE_LOGF(LogUnitTest, Log, "Suspend start.");
				...
				if (bPipeCreated)
				{
					...
					{
						UE_LOGF(LogUnitTest, Log, "WARNING: Failed to open pipe connection.");
					}
				}
				else
				{
					UE_LOGF(LogUnitTest, Log, "WARNING: Failed to create resume pipe.");
				}
				...
				while (true)
				{
					if (bPipeCreated && ResumePipe.IsReadyForRW())
					{
						...
						{
							UE_LOGF(LogUnitTest, Log, "Got resume request.");
							break;
						}
					}
				...
				UE_LOGF(LogUnitTest, Log, "Suspend end.");
#else
				UE_LOGF(LogUnitTest, Log, "Suspend/Resume is only supported in Windows.");
#endif
			}
		}
	...
	if (CurWorld != nullptr)
	{
		...
		if (bMonitorForBeacon)
		{
			...
			if (BeaconDriver != nullptr && BeaconDriver->ClientConnections.Num() > 0)
			{
				...
				UE_LOGF(LogUnitTest, Log, "Hooked beacon net driver");
...
void ANUTActor::UpdateOwner()
{
	...
	if (GameState != NULL && GEngine != NULL && CurNetMode != NM_Client)
	{
		...
		for (int i=0; i<GameState->PlayerArray.Num(); i++)
		{
			...
			{
				UE_LOGF(LogUnitTest, Log, "Setting NUTActor owner to: %ls (%ls)", *PC->GetName(),
...
void ANUTActor::HookNetDriver(UNetDriver* TargetNetDriver)
{
	if (TargetNetDriver != NULL)
	{
		...
		{
			UE_LOGF(LogUnitTest, Log, "Setting %ls InitialConnectTimeout/ConnectionTimeout to '%i'", *TargetNetDriver->GetFullName(),
					CustomTimeout);
...
{
	UE_LOGF(LogUnitTest, Log, "Executing command: %ls", *Command);
...
void ANUTActor::UnitSeamlessTravel(FString Dest/*=TEXT(" ")*/)
{
	...
	if (GameMode != NULL)
	{
		UE_LOGF(LogUnitTest, Log, "Executing seamless travel");
...
void ANUTActor::UnitTravel(FString Dest/*=TEXT(" ")*/)
{
	...
	if (GameMode != NULL)
	{
		UE_LOGF(LogUnitTest, Log, "Executing normal travel");
...
void ANUTActor::NetFlush()
{
	...
	if (CurNetDriver != nullptr)
	{
		...
		{
			UE_LOGF(LogUnitTest, Log, "Flushing ServerConnection");
			ServerConn->FlushNet();
		}
		else
		{
			UE_LOGF(LogUnitTest, Log, "Flushing ClientConnections");
...
void ANUTActor::Wait(uint16 Seconds)
{
	if (Seconds > 0)
	{
		UE_LOGF(LogUnitTest, Log, "Sleeping for '%i' seconds", Seconds);

		FPlatformProcess::Sleep(Seconds);
	}
	else
	{
		UE_LOGF(LogUnitTest, Log, "Bad 'Wait' command value '%i'", Seconds);
	}
}
...
{
	UE_LOGF(LogUnitTest, Log, "ServerReceiveText: InText: %ls", *InText.ToString());
}
...
void ANUTActor::NetMulticastPing_Implementation()
{
	...
	{
		UE_LOGF(LogUnitTest, Log, "Unit Test Client Ping.");
	}
}
	...
	if (InTargetObj != nullptr && InTargetFunc.Len() > 0)
	{
		...
		if (FString(TargetFuncName.ToString()).StartsWith(TEXT("UnitTestServer_"), ESearchCase::CaseSensitive))
		{
			if (TargetFuncObj != nullptr)
			{
				...
				{
					UE_LOGF(LogUnitTest, Log, "ExecuteOnServer: Only static functions can be passed to the server.");
				}
			}
			else
			{
				UE_LOGF(LogUnitTest, Log, "ExecuteOnServer: Could not locate InTarget function.");
			}
		}
		else
		{
			UE_LOGF(LogUnitTest, Log, "ExecuteOnServer: Target functions must be prefixed 'UnitTestServer_FuncName'");
		}
	}
	else
	{
		UE_LOGF(LogUnitTest, Log, "ExecuteOnServer: Target not specified");
	}
}
...
void ANUTActor::ServerExecute_Implementation(const FString& InDelegate)
{
	...
	{
		UE_LOGF(LogUnitTest, Log, "Executing serverside unit test function '%ls::%ls'",
	...
	{
		UE_LOGF(LogUnitTest, Log, "ServerExecute: Failed to find function '%ls'", InDelText);
	}
}
