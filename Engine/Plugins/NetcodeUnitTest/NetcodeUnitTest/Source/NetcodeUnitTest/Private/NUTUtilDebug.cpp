...
{
	UE_LOGF(LogUnitTest, Log, "FScopedProcessEventLog: Actor: %ls, Function: %ls",
...
void FNUTStackTrace::AddTrace(bool bLogAdd/*=false*/)
{
	if (IsTrackingEnabled())
	{
		if (bLogAdd)
		{
			UE_LOGF(LogUnitTest, Log, "Adding stack trace for TraceName '%ls'.", *TraceName);
		}
...
{
	UE_LOGF(LogUnitTest, Log, "Dumping tracked stack traces for TraceName '%ls':", *TraceName);
...
void FStackTraceManager::Disable(FString TraceName)
{
	...
	{
		UE_LOGF(LogUnitTest, Log, "Trace disable: No trace tracking found for TraceName '%ls'.", *TraceName);
	}
}
...
void FStackTraceManager::Dump(FString TraceName, bool bKeepTraceHistory/*=false*/, bool bKeepTracking/*=true*/)
{
	...
	{
		UE_LOGF(LogUnitTest, Log, "No trace tracking found for TraceName '%ls'.", *TraceName);
	}
}
...
{
	UE_LOGF(LogUnitTest, Log, "Dumping all tracked stack traces:");
...
void FStackTraceManager::TraceAndDump(FString TraceName)
{
	...
	{
		UE_LOGF(LogUnitTest, Log, "Dumping once-off stack trace for TraceName '%ls':", *TraceName);
...
void FLogTraceManager::AddLogTrace(FString LogLine, ELogTraceFlags TraceFlags)
{
	...
	{
		UE_LOGF(LogUnitTest, Warning, "AddLogTrace: Should specify either ELogTraceFlags::Full or ELogTraceFlags::Partial. Enabling 'Full'.");

		TraceFlags |= ELogTraceFlags::Full;
	}
	else if (bPartial && EnumHasAnyFlags(TraceFlags, ELogTraceFlags::Full))
	{
		UE_LOGF(LogUnitTest, Warning, "AddLogTrace: Specified both ELogTraceFlags::Full and ELogTraceFlags::Partial. Defaulting to 'Partial'.");
	...
	UE_LOGF(LogUnitTest, Log, "Adding %lslog trace for line: %ls (DumpTrace: %i, Debug: %i)",
	...
	{
		UE_LOGF(LogUnitTest, Warning, "Error adding log trace, GLogHookManager is not set.");
	}
}

void FLogTraceManager::ClearLogTrace(FString LogLine, bool bDump/*=true*/)
{
	UE_LOGF(LogUnitTest, Log, "Clearing log trace for line: %ls", ToCStr(LogLine));
...
{
	UE_LOGF(LogUnitTest, Log, "Clearing all log traces.");
	...
	if (bDump)
	{
		for (TMap<FString, FLogHookID>::TConstIterator It(HookMap); It; ++It)
		{
			...
			{
				UE_LOGF(LogUnitTest, Log, "No stack traces for log trace: %ls", ToCStr(CurLogLine));
			}
		}
	}
...
void FLogCommandManager::AddLogCommand(FString LogLine, FString Command)
{
	if (GLogHookManager != nullptr)
	{
		UE_LOGF(LogUnitTest, Log, "Adding log command '%ls' for line: %ls", ToCStr(Command), ToCStr(LogLine));
	...
	{
		UE_LOGF(LogUnitTest, Warning, "Error adding log command, GLogHookManager is not set.");
	}
}
...
{
	UE_LOGF(LogUnitTest, Log, "Clearing log trace for line: %ls", *LogLine);
...
void FLogStackTraceManager::ClearAll(bool bDump/*=false*/)
{
	...
	if (bDump)
	{
		for (const FLogTraceEntry& CurEntry : ExactMatches)
		{
			...
			{
				UE_LOGF(LogUnitTest, Log, "No stack traces for log trace: %ls", *CurEntry.LogLine);
			}
		}
		...
		for (auto CurEntry : PartialMatches)
		{
			...
			{
				UE_LOGF(LogUnitTest, Log, "No stack traces for (partial) log trace: %ls", *CurEntry.LogLine);
			}
		}
	}
