...
public:
	...
	/** Indicates whether the job processor instance is using the UBA virtual file mechanism */
	bool IsUsingVirtualFiles() const;
	
	/** Creates the threads and starts the main loop */
	void StartThread();
	void HandleUbaJobFinished(FDistributedBuildTask* CompileTask, TArray<uint8>&& OutputFileData, uint32 ExitCode);
