...
public:
	...
	virtual bool SupportsVirtualFiles() const override final;
	...
	std::atomic<uint32> PendingRequestedCompilationCount;

	// Event used to wake the FUbaJobProcessor instance when new tasks are available or a shutdown is requested (the instance will block on this event when out of work)
	FEventRef ProcessorWakeEvent;
