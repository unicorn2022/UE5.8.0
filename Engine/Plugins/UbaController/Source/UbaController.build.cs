...
public class UbaController : ModuleRules
{
	public UbaController(ReadOnlyTargetRules TargetRules) : base(TargetRules)
	{
		...
		if (Target.Platform == UnrealTargetPlatform.Win64)
		{
			...
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "UbaDetours.dll"));
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "UbaAgent.exe"));

			// todo: this only exists for x64 atm. Is not needed
			//AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "UbaWine.dll.so"));

			// Only for windows atm
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "UbaHost.toml"));
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "UbaAgent.toml"));
		}
		else if (Target.Platform == UnrealTargetPlatform.Mac)
		{
			...
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "libUbaDetours.dylib"));
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "UbaAgent"));
		}
		else if (Target.Platform == UnrealTargetPlatform.Linux)
		{
			...
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "libUbaDetours.so"));
			AdditionalPropertiesForReceipt.Add("Uba", Path.Combine(UbaBinariesPath, "UbaAgent"));
		}
	}
}
