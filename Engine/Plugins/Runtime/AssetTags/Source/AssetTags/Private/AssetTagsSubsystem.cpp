...
bool FindCollectionByName(ICollectionContainer& CollectionContainer, const FName Name, FCollectionScriptingRef& OutCollection)
{
	...
	{
		UE_LOGF(LogAssetTags, Warning, "No collection found called '%ls'", *Name.ToString());
		return false;
	}
	else if (CollectionNamesAndTypes.Num() > 1)
	{
		UE_LOGF(LogAssetTags, Warning, "%d collections found called '%ls'; ambiguous result", CollectionNamesAndTypes.Num(), *Name.ToString());
		return false;
	}
