...
bool UFusionPatchAssetFactory::CanReimport(UObject* Obj, TArray<FString>& OutFilenames)
{
	if (UFusionPatch* FusionPatch = Cast<UFusionPatch>(Obj))
	{
		...
		{
			UE_LOGF(LogFusionPatchAssetFactory, Warning, "%ls: Failed to reimport with new file. Invalid extension: %ls", *FusionPatch->GetPathName(), *FPaths::GetExtension(PreferredReimportPath));
			return false;
		}
...
EReimportResult::Type UFusionPatchAssetFactory::Reimport(UObject* Obj)
{
	...
	if (OutCanceled)
	{
		UE_LOGF(LogFusionPatchAssetFactory, Warning, "import canceled");
		return EReimportResult::Cancelled;
	}

	UE_LOGF(LogFusionPatchAssetFactory, Warning, "import failed");
	return EReimportResult::Failed;
}
...
UObject* UFusionPatchAssetFactory::FactoryCreateText(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, const TCHAR* Type, const TCHAR*& Buffer, const TCHAR* BufferEnd, FFeedbackContext* Warn)
{
	...
	if (!FDtaParser::DtaStringToJsonString(DtaString, JsonString, DtaErrorMessage))
	{
		...
		UE_LOGF(LogFusionPatchAssetFactory, Error, "Failed to import .fusion asset: %ls - %ls", *SourceFile, *DtaErrorMessage);
		return nullptr;
	}
	...
	if (JsonObj.IsValid())
	{
		...
		{
			UE_LOGF(LogFusionPatchAssetFactory, Log, "Successfully imported FusionPatch asset");
		...
		else
		{
			...
			FMessageDialog::Open(EAppMsgType::Ok, ImportErrorMessage);
			UE_LOGF(LogFusionPatchAssetFactory, Error, "Failed to import fusion patch: %ls", *ImportErrorMessage.ToString());
	...
	else
	{
		...
		FMessageDialog::Open(EAppMsgType::Ok, ImportErrorMessage);
		UE_LOGF(LogFusionPatchAssetFactory, Error, "Failed to read json: %ls", *ErrorMessage);
	}
