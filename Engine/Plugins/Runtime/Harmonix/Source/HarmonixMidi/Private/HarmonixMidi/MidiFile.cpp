	...
	bool EnsureNotFailed)
{
	...
	if (reader.IsFailed())
	{
		...
		{
			UE_LOGF(LogMIDI, Error, "%ls MIDI import failed. Midi data is malformed. Check the log for details.", *Filename);
		}
		TheMidiData.Empty();
...
void UMidiFile::QuantizeLengthToNearestPerfectSubdivision(const EMidiFileQuantizeDirection Direction)
{
	...
	if (QuantizedLength == 0)
	{
		...
		{
			UE_LOGF(LogMIDI, Warning, "QuantizeLengthToNearestPerfectSubdivision: Asked to Quantize file length DOWN, but that would result in a zero length midi file. NOT ALLOWED! Skipping quantization!");
			return;
		}
		...
		{
			UE_LOGF(LogMIDI, Error, "This is odd. Asked to quantize MIDI file length UP to the nearest subdivision, but returned length is zero! Skipping.");
			return;
		}
...
bool FMidiEventReceiver::OnText(int32 Tick, const FString& Str, uint8 Type)
{
	...
	if (Type == Harmonix::Midi::Constants::GMeta_TrackName)
	{
		...
		{
			UE_LOGF(LogMIDI, Warning, "MIDI track name (\"%ls\") event found at tick %d but none found at tick 0! The first track name event on a midi track should be on tick 0. Shifting it to tick 0.",
				*Str, Tick);
			Tick = 0;
		}
		else if (CurrentTrackHasName && Str != *Track.GetName())
		{
			UE_LOGF(LogMIDI, Warning, "2nd Track Name event found on MIDI track %ls --> %ls at tick %d",
...
void FMidiFileData::PostSerialize(const FArchive& Ar)
{
	...
	{
		UE_LOGF(LogMIDI, Warning, "Empty bar map. Rebuilding.");
...
bool FMidiFileData::ConformToLengthGivenLastEventTick(int32 NewLastEventTick)
{
	...
	{
		NewLastEventTick = 0;
		UE_LOGF(LogMIDI, Warning, "Asked to comform length such that the last event tick would be less than 0... Not possible! Conforming length to 1 tick, with last event being placed on tick 0!");
	}
	...
	UE_LOGF(LogMIDI, Log, "%ls", *MidiFileLengthConformSummary);
