...
namespace Midi
{
	FMusicTimestamp HARMONIXMIDI_API ParseMusicTimestamp(const FString& Str, EMusicTimeStringFormat Format)
	{
		...
		if (Parts.Num() > 2)
		{
			...
			{
				UE_LOGF(LogMIDI, Warning, "Unexpected bar value specified in midi duration string \"%ls\". Bar in duration string should be >= 0!",*Str);
				Result.Bar = 0;
			}
		}
		...
		if (Parts.Num() > 1 || (Parts.Num() == 1 && NumDelimiters == 0))
		{
			...
			{
				UE_LOGF(LogMIDI, Warning, "Unexpected beat value specified in midi position string %ls. Beat in position string should be >= 1!", *Str);
				Result.Beat = 1;
			}
			else if (Result.Beat < 0 && Format == EMusicTimeStringFormat::Duration)
			{
				UE_LOGF(LogMIDI, Warning, "Unexpected beat value specified in midi duration string %ls. Beat in duration string should be >= 0!", *Str);
				Result.Beat = 0;
			}
		}
		...
		if (PartIndex < Parts.Num())
		{
			...
			if (num < 0)
			{
				UE_LOGF(LogMIDI, Warning, "Unexpected negative value found in fractional beat portion of midi time specifier string %ls. Using 0 for fractional portion!", *Str);
				num = 0.0f;
			}
			Result.Beat +=  num / div;
		}
