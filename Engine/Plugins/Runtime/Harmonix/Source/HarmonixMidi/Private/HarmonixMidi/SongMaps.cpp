...
void FSongMaps::SetSongLengthTicks(int32 NewLengthTicks)
{
	if (NewLengthTicks < 1)
	{
		UE_LOGF(LogMIDI, Warning, "SetSongLengthTicks: Asked to set length less than 1. That is not possible. Setting to length 1!");
		NewLengthTicks = 1;
	}
...
void FSongMaps::StringLengthToMT(const FString& LengthString, int32& OutBars, int32& OutTicks)
{
	if (BarMap.IsEmpty())
	{
		UE_LOGF(LogMIDI, Log, "SongMaps does not contain a Bar Map.");
		return;
	}
	...
	{
		UE_LOGF(LogMIDI, Log, "No Time Signature found in SongMaps.");
		return;
	}
...
float ISongMapEvaluator::TickToMs(float Tick) const
{
	...
	if (TempoMap.IsEmpty())
	{
		UE_LOGF(LogMIDI, Log, "SongMaps does not contain a Tempo Map.");
		return 0.0f;
	}
...
const FBeatMapPoint* ISongMapEvaluator::GetBeatAtMs(float Ms) const
{
	...
	if (BeatMap.IsEmpty())
	{
		UE_LOGF(LogMIDI, Log, "SongMaps does not contain a Beat Map.");
		return nullptr;
	}
	int32 Tick = int32(MsToTick(Ms));
...
const FTimeSignature* ISongMapEvaluator::GetTimeSignatureAtBar(int32 Bar) const
{
	...
	if (Bar < 1)
	{
		UE_LOGF(LogMIDI, Warning, "Bar < 1 (%d) specified as a musical position! Bars are '1' based in musical positions. Using bar 1!", Bar);
	}
...
float ISongMapEvaluator::GetMsAtFractionalBarIncludingCountIn(float BarIncludingCountIn) const
{
	const float Tick = GetBarMap().FractionalBarIncludingCountInToTick(BarIncludingCountIn);
	return TickToMs(Tick);
}
...
float ISongMapEvaluator::FractionalBeatIncludingCountInToTick(float FractionalBeatIndex) const
{
	return GetBarMap().FractionalBeatIncludingCountInToTick(FractionalBeatIndex);
}
...
const FSongSection* ISongMapEvaluator::GetSectionWithName(const FString& Name) const
{
	...
	if (SectionMap.IsEmpty())
	{
		UE_LOGF(LogMIDI, Log, "SongMaps does not contain a Section Map.");
		return nullptr;
	}
...
const FChordMapPoint* ISongMapEvaluator::GetChordAtTick(int32 Tick) const
{
	...
	if (ChordMap.IsEmpty())
	{
		UE_LOGF(LogMIDI, Log, "SongMaps does not contain a Chord Map.");
		return nullptr;
	}
