...
void FStdMidiFileReader::ReadAllTracks()
{
	...
	UE_LOGF(LogMIDI, VeryVerbose, "READING ALL MIDI TRACKS:\n========================");
...
{
	UE_LOGF(LogMIDI, VeryVerbose, "Skipping Current Track (%d)", CurrentTrackIndex);
...
void FStdMidiFileReader::ReadNextEventImpl()
{
	...
	case EState::NewTrack:
		...
		{
			UE_LOGF(LogMIDI, VeryVerbose, "Type 0 file so rewinding to re-read track in 'non-conductor' mode.");
...
void FStdMidiFileReader::ReadFileHeader()
{
	...
	UE_LOGF(LogMIDI, VeryVerbose, "Reading file header...");
	...
	*Archive << TicksPerQuarter;
	...
	UE_LOGF(LogMIDI, VeryVerbose, "... Done reading file header. Format = %d, File PPQ = %d, NumTracks = %d", Format, TicksPerQuarter, Format == 0 ? 1 : NumTracks);
}
...
void FStdMidiFileReader::ReadTrackHeader()
{
	...
	MIDIREADER_EXPECT_WITH_ERROR((TrackHeader.GetLength() + (uint32)Archive->Tell()) <= (uint32)std::numeric_limits<int32>::max(),
		TEXT("%s: MIDI track data length exceeds maximum length!"), *Filename);
	...
	UE_LOGF(LogMIDI, VeryVerbose, "... Done reading track header. It contains %d bytes of midi events.", TrackHeader.GetLength());
}
...
void FStdMidiFileReader::ReadMidiEvent(int32 Tick, uint8 Status, uint8 Data1)
{
	...
	default:
		UE_LOGF(LogMIDI, Error, "%ls (%ls): Cannot parse event %i", *Filename, *CurrentTrackName, (Status & GMessageTypeMask));
		State = EState::Failed;
		return;
	}
	...
	if (ValidMidiEvent && TrackFilteringMode != ETrackFilteringMode::ConductorEvents)
	{
		...
		UE_LOGF(LogMIDI, VeryVerbose, "TICK %d :: %ls", Tick, *Harmonix::Midi::Constants::MakeStdMsgString(Status, Data1, Data2));
	}
}
...
void FStdMidiFileReader::ReadSystemEvent(int32 Tick, uint8 Status)
{
	...
	case GFile_Escape:
		{
			...
			UE_LOGF(LogMIDI, VeryVerbose, "Seeking %d bytes past SysEx or Escape system event.", PacketLength);
	...
	case GFile_Meta:
		...
		break;
	default:
		UE_LOGF(LogMIDI, Error, "%ls (%ls): Cannot parse system event %i", *Filename, *CurrentTrackName, Status);
		State = EState::Failed;
		break;
	}
}
...
void FStdMidiFileReader::ReadMetaEvent(int32 Tick, uint8 Type)
{
	...
	case GMeta_TrackName:
		...
		else if (CurrentTrackName.IsEmpty())
		{
			...
			UE_LOGF(LogMIDI, VeryVerbose, "TICK %d :: Got track name meta type but an empty string. Using a default track name...", Tick);
		}
		UE_LOGF(LogMIDI, VeryVerbose, "Got track name event... '%ls'", *CurrentTrackName);
	...
	case GMeta_CuePoint:
		...
		if (TrackFilteringMode != ETrackFilteringMode::NonConductorEvents)
		{
			...
			UE_LOGF(LogMIDI, VeryVerbose, "TICK %d :: Got Copyright, Marker, or CuePoint event... '%ls'", Tick, *WorkingString);
		}
		break;
	case GMeta_Text:
	case GMeta_Lyric:
		...
		if (TrackFilteringMode != ETrackFilteringMode::ConductorEvents)
		{
			...
			UE_LOGF(LogMIDI, VeryVerbose, "TICK %d :: Got Text or Lyric event... '%ls'", Tick, *WorkingString);
		}
		break;
	case GMeta_Tempo:
		{
			...
			*Archive << Byte1 << Byte2 << Byte3;
			...
			if (TrackFilteringMode != ETrackFilteringMode::NonConductorEvents)
			{
				...
				UE_LOGF(LogMIDI, VeryVerbose, "TICK %d :: Got Tempo event... %f bpm", Tick, Harmonix::Midi::Constants::MidiTempoToBPM(Tempo));
			}
		}
		break;
	...
	case GMeta_EndOfTrack:
		...
		UE_LOGF(LogMIDI, VeryVerbose, "TICK %d :: Got End Of Trackevent.", Tick);
		if (CurrentTrackIndex == NumTracks - 1)
		{
			State = EState::End;
	...
	case GMeta_TimeSig:
		{
			...
			*Archive << Numerator << DenominatorExp;
			...
			if (TrackFilteringMode != ETrackFilteringMode::NonConductorEvents)
			{
				...
				UE_LOGF(LogMIDI, VeryVerbose, "TICK %d :: Got Time Signature Event. %d/%d", Tick, Numerator, Denominator);
	...
	case GMeta_InstrumentName:
		...
		UE_LOGF(LogMIDI, Warning, "Track '%ls', TICK = %d :: Skipping unsupported MIDI event -> %d - %ls", *CurrentTrackName, Tick, Type, *Harmonix::Midi::Constants::GetMetaEventTypeName(Type));

		break;

	default:
		UE_LOGF(LogMIDI, Error, "%ls (%ls): Cannot parse meta event %i", *Filename, *CurrentTrackName, Type);
		State = EState::Failed;
		return;
	}
