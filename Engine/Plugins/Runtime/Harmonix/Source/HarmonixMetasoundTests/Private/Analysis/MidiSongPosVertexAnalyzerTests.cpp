...
namespace HarmonixMetasoundTests::MidiSongPosVertexAnalyzer
{
	...
	bool FMidiSongPosVertexAnalyzerTestBasic::RunTest(const FString&)
	{
		...
		auto LogSongPos = [](const FMidiSongPos& Pos)
			{
			#if 0
				UE_LOGF(LogMIDI, Log, "\tSeconds From Bar One: %f", Pos.SecondsFromBarOne);
				UE_LOGF(LogMIDI, Log, "\tSeconds Including Count In: %f", Pos.SecondsIncludingCountIn);
				UE_LOGF(LogMIDI, Log, "\tTime Signatre: %d/%d", Pos.TimeSigNumerator, Pos.TimeSigDenominator);
				UE_LOGF(LogMIDI, Log, "\tTempo: %f", Pos.Tempo);
				UE_LOGF(LogMIDI, Log, "\tIncluding Count In: Bars = %f, Beats = %f", Pos.BarsIncludingCountIn, Pos.BeatsIncludingCountIn);
				UE_LOGF(LogMIDI, Log, "\tBeatType: %ls", *MusicalBeatTypeToString(Pos.BeatType));
				UE_LOGF(LogMIDI, Log, "\tTimestamp: %d : %f", Pos.Timestamp.Bar, Pos.Timestamp.Beat);
			#endif
			};
		...
		for(int32 i = 0; i < NumBlocks; ++i)
		{
			...
			while (auto PosRef = ClockHistoryCursor.ConsumeNext())
			{
				...
				{
					UE_LOGF(LogMIDI, VeryVerbose, "----------------- LOOPED----------------- ");
				}
