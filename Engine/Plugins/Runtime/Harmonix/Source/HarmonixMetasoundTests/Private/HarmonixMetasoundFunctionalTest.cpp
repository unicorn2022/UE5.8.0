...
{
	UE_LOGF(LogHarmonixMetasoundTests, Log, "Adding output logger for output: %ls", *OutputName.ToString());
	...
	if (!ReturnValue)
	{
		UE_LOGF(LogHarmonixMetasoundTests, Error, "Failed to add logger for output: %ls", *OutputName.ToString());
	}
	return ReturnValue;
}
...
bool UHarmonixMetasoundFunctionalTestLibrary::ReadAudioFromFile(const FString& Filepath, Audio::FAlignedFloatBuffer& OutAudio, int32& OutSampleRate, int32& OutNumFrames, int32& OutNumChannels, uint16& OutFormatTag)
{
	...
	{
		UE_LOGF(LogHarmonixMetasoundTests, Error, "Failed to read wave file %ls: File does not exist", *Filepath);
		return false;
	}
	...
	{
		UE_LOGF(LogHarmonixMetasoundTests, Error, "Failed to read wave file %ls: Unable to convert data to array", *Filepath)
		return false;
	}
	...
	{
		UE_LOGF(LogHarmonixMetasoundTests, Error, "Failed to read wave file %ls: %ls", *Filepath, *ErrorMessage);
		return false;
	}
	...
	// {
	// 	UE_LOGF(LogHarmonixMetasoundTests, Error, "Failed to read wave file %ls: Unable to read format. Must be PCM short or Float");
	// 	return false;
	// }
	...
	if (*WaveInfo.pFormatTag == FWaveModInfo::WAVE_INFO_FORMAT_PCM)
	{
		...
		{
			UE_LOGF(LogHarmonixMetasoundTests, Error, "Failed to read wave file %ls: Unable to read format. Must be 16 bit PCM", *Filepath);
			return false;
		}
	...
	UE_LOGF(LogHarmonixMetasoundTests, Error, "Failed to read wave file %ls: Unable to read format. Must be 16 bit PCM or IEEE float", *Filepath);
	return false;
}
...
bool AHarmonixMetasoundFunctionalTest::IsReady_Implementation()
{
	...
	UE_LOGF(LogHarmonixMetasoundTests, Log, "%ls -- Is Ready: %d", *TestLabel, OutIsReady);
	return OutIsReady;
}
...
void AHarmonixMetasoundFunctionalTest::StartTest()
{
	...
	UE_LOGF(LogHarmonixMetasoundTests, Log, "%ls -- StartTest", *TestLabel);
...
void AHarmonixMetasoundFunctionalTest::PrepareTest()
{
	...
	UE_LOGF(LogHarmonixMetasoundTests, Log, "%ls -- PrepareTest", *TestLabel);
...
{
	UE_LOGF(LogHarmonixMetasoundTests, Log, "%ls -- OnTestFinished", *TestLabel);
	...
	if (!WavFilename_Expected.IsEmpty() && !WavFilename_Output.IsEmpty())
	{
		...
		UE_LOGF(LogHarmonixMetasoundTests, Log, "PSNR of files is: %.2f", PSNR);
	}
}
