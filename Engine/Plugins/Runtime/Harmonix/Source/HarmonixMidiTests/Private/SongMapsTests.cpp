...
namespace HarmonixMidiTests::SongMaps
{
	...
	void LogQuantizationDetails(const ISongMapEvaluator& Map, int32 OriginalTick, int32 QuantizedTick, EMidiClockSubdivisionQuantization ResultDivision)
	{
		...
		UE_LOGF(LogSongMapsTest, Log, "Tick -> %d: Quantized %ls to %d, Division = %ls -> %d | %d | %d", OriginalTick, (QuantizedTick < OriginalTick ? TEXT("^") : TEXT("v")), QuantizedTick, *StaticEnum<EQuartzCommandQuantization>()->GetDisplayNameTextByIndex((int32)ResultDivision).ToString(), BarIndex, BeatInBar, TickIndexInBeat);
	}
