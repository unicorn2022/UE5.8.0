...
bool FFusionVoice::AssignIDs(FFusionSampler* InSampler, const FKeyzoneSettings* InKeyZone, FMidiVoiceId InVoiceID, FRelinquishHandler Handler, TSharedPtr<IStretcherAndPitchShifter, ESPMode::ThreadSafe> Shifter /*= nullptr*/)
{
	...
	{
		check(InKeyZone->TimeStretchConfig.OriginalTempo > 0.0f);
	}
	...
	int32 NumInChannels = InKeyZone->TrackMap.Num() == 0 ? InKeyZone->SoundWaveData->GetNumChannels() : 0;
	...
	double FileSamplesPerSecond = InKeyZone->SoundWaveData->GetSampleRate();
	...
	//TRACE_BOOKMARK(TEXT("Starting Waveform: %s"), *InKeyZone->AudioSample->GetName().ToString());
	ActiveRenderer->SetSoundWaveData(InKeyZone->SoundWaveData.ToSharedRef(), Settings);

	OutputBuffer.SetNumValidChannels(InKeyZone->SoundWaveData->GetNumChannels());
...
void FFusionVoice::Attack()
{
	check(MySampler);
	checkSlow(KeyZone);
	checkSlow(KeyZone->SoundWaveData);
...
{
	check(KeyZone && KeyZone->SoundWaveData);
...
#endif
{
	//UE_LOGF(LogFusionVoice, Log, "0x%x - Attacking\n", (size_t)this);
	...
	EndOfSampleData = (KeyZone->SampleEndOffset == -1) ?
		KeyZone->SoundWaveData->GetNumFrames() :
		KeyZone->SampleEndOffset;

	bool HasLoopSection = KeyZone->SoundWaveData->GetLoopRegions().Num() > 0;
...
{
	//UE_LOGF(LogFusionVoice, Log, "0x%x - Preparing\n", (size_t)this);
...
uint32 FFusionVoice::Process(uint32 InSliceIndex, uint32 InSubsliceIndex, float** OutData, uint32 InNumChannels, uint32 InMaxNumSamples, float InSpeed, float InTempoBPM, bool MaintainPitchWhenSpeedChanges)
{
	...
	if (!IsInUse() || !MySampler || !KeyZone || !KeyZone->SoundWaveData)
	{
		return 0;
	}
	...
	// update all of the ramping data on a sample-by-sample basis.
	{
		...
		if (KeyZone->TimeStretchConfig.bMaintainTime && KeyZone->TimeStretchConfig.bSyncTempo && InTempoBPM > 0.0f)
		{
			...
			if (CurrentMidiQuarterNote > StartQuarterNote && (SamplePos > LastVsoPos || !bHasRenderedAnySamples))
			{
				...
				float ActualElapsedMs = (CurrentSampleFrame * 1000.0f) / FMath::Max(1, KeyZone->SoundWaveData->GetSampleRate());
				...
				if (!bHasRenderedAnySamples)
				{
					if (ErrorMs > 40.0f)
					{
						UE_LOGF(LogFusionVoice, Verbose, "Adjusting start time due to error of %f ms", ErrorMs);
					}

					SamplePos = ExpectedElapsedMs / 1000.0f * (float)KeyZone->SoundWaveData->GetSampleRate();
					
					if (SamplePos > 0.0)
					{
						UE_LOGF(LogFusionVoice, Verbose, "SHAVING %f ms from the beginning of %ls", ErrorMs, *KeyZone->SoundWaveData->GetFName().ToString());
					}
					...
					if (SamplePos > EndOfSampleData && KeyZone->SoundWaveData->GetLoopRegions().IsEmpty())
					{
						Kill();
						return 0;
					}
				...
				else if (ErrorMs > FusionVoice::kVsoThresholdMs)
				{
					if (ErrorMs > 20.0f)
					{
						UE_LOGF(LogFusionVoice, Verbose, "Fixing %ls offset -> %f\n", *KeyZone->SoundWaveData->GetFName().ToString(), ErrorMs);
					}
				...
				{
				   UE_LOGF(LogFusionVoice, Verbose, "offset -> %f (%f)\n", ErrorMs, CurrentVso);
		...
		int32 NumInChannels = KeyZone->SoundWaveData->GetNumChannels();
