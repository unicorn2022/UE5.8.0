...
void FGainTable::SetChannelLayout(EAudioBufferChannelLayout ChannelLayout)
{
	...
	case EAudioBufferChannelLayout::Raw:
		UE_LOGF(LogGainTable, Error, "Can't pan across 'Raw' channel ChannelLayout!");
		checkNoEntry();
		break;
	...
	case EAudioBufferChannelLayout::AmbisonicOrderOne:
		UE_LOGF(LogGainTable, Warning, "Can't currently pan across 'AmbisonicOrderOne' channel ChannelLayout. Stereo panning will be used. Direct mapped ambisonic channels WILL be preserved.");
	...
	default:
		UE_LOGF(LogGainTable, Error, "Unrecognized channel ChannelLayout!");
		checkNoEntry();
		break;
	}
}
