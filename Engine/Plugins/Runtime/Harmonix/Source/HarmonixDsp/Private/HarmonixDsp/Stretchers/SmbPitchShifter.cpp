...
{
	check(MySoundWaveData);
	SampleRate = MySoundWaveData->GetSampleRate();
	FreqPerFftBin = (double)MySoundWaveData->GetSampleRate() / (double)FftFrameSize;
	Reset();
}
...
double FSmbPitchShifter::Render(TAudioBuffer<float>& OutputData, double InPos, int32 InMaxFrame, double ResampleInc, double InPitchShift, double InSpeed, bool MaintainPitchWhenSpeedChanges, const FGainMatrix& InGain)
{
	double NewPos = MyAudioRenderer->RenderUnshifted(OutputData, InPos, InMaxFrame, ResampleInc * InSpeed, InGain);
