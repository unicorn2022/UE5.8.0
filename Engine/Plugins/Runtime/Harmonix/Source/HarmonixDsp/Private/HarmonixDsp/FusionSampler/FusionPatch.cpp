...
void UFusionPatch::SetUserData(TSharedRef<FFusionPatchUserData> UserData)
{
	FusionPatchData.UserData = UserData;
	UpdateRenderableForNonTrivialChange();
}
...
void UFusionPatch::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedChainEvent)
{
	...
	{
		UE_LOGF(LogFusionPatch, Log, "Fusion patch settings changed");
...
void UFusionPatch::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		if (Version < FFusionPatchCustomVersion::PitchShifterNameRedirects)
		{
			UE_LOGF(LogFusionPatch, Warning, "Fusion patch was loaded with an outdated version. Asset requires reimport: %ls", *GetPathName());
		}
		...
		if (Version < FFusionPatchCustomVersion::DeprecatedPresets)
		{
			...
			{
				UE_LOGF(LogFusionPatch, Warning, "Fusion patch (%ls) has more than one preset, but presets have been deprecated. Only the first (Default) preset will be loaded", *GetPathName());
			}
		}
...
void FFusionPatchData::ConnectSampler(const FFusionSampler* Sampler)
{
	if (UserData)
	{
		UserData->OnSamplerConnected(Sampler);
	}
}
...
void FFusionPatchData::DisconnectSampler(const FFusionSampler* Sampler)
{
	...
	if (UserData)
	{
		UserData->OnSamplerDisconnected(Sampler);
	}
}
