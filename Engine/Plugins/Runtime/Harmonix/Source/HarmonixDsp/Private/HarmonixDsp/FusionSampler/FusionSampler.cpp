...
namespace Defaults
{
	...
	constexpr float kMinPitchShift = 0.0001f;
...
void FFusionSampler::SetSpeed(float InSpeed, bool InMaintainPitch)
{
	...
	{
		UE_LOGF(LogFusionSampler, Warning, "Speed value cannot be less than %.8f. Clamping", Defaults::kMinRenderSpeed);
		Speed = Defaults::kMinRenderSpeed;	
	}
	else if (InSpeed > Defaults::kMaxRenderSpeed)
	{
		UE_LOGF(LogFusionSampler, Warning, "Speed value cannot greater than %.2f. Clamping", Defaults::kMaxRenderSpeed);
...
int32 FFusionSampler::StartNote(FMidiVoiceId InVoiceId, uint8 InMidiNoteNumber, uint8 InVelocity, bool IsNoteOn, int32 InEventTick, int32 InTriggerTick, float InOffsetMs)
{
	...
	{
		UE_LOGF(LogFusionSampler, Warning, "Cannot \"start note \" on a sampler with no voice pool!");
		return 0;
	}
...
bool FFusionSampler::TryKeyOnZone(FMidiVoiceId InVoiceId, uint8 InTriggeredNote, uint8 InTransposedNote, uint8 InVelocity, int32 InEventTick, int32 InTriggerTick, float InOffsetMs, const FKeyzoneSettings* InKeyzone)
{
	...
	if (Voice == nullptr)
	{
		UE_LOGF(LogFusionSampler, Warning, "Voice pool failed to create voice, can't play note!");
		return false;
	}
...
void FFusionSampler::SetPatch(FFusionPatchDataProxy::NodePtr PatchData)
{
	...
	if (FusionPatchData)
	{
		FusionPatchData->ConnectSampler(this);
		ApplyPatchSettings();
	}
}
...
void FFusionSampler::ResetPatchRelatedState()
{
	...
	// set up the default velocity modulator settings
...
void FFusionSampler::SetSubstreamMidiGain(int32 Idx, uint8 InMidiGain)
{
	...
	{
		UE_LOGF(LogFusionSampler, Error, "Substream index out of range!");
		return;
	}
