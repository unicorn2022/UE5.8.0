...
void IStretcherAndPitchShifterFactory::AddClient(IFeatureClient* Client, float SampleRate)
{
	...
	Clients.Add(Client);
	UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Adding client. Num Clients: %d", Clients.Num());
	...
	for (IStretcherAndPitchShifterFactory* Factory : GetAllRegisteredFactories())
	{
		...
		{
			UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Updating Factory: \"%ls\"", *FactoryName.ToString());
		}
...
void IStretcherAndPitchShifterFactory::RemoveClient(IFeatureClient* Client)
{
	...
	UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Removed client. Num Clients: %d", Clients.Num());
...
{
	UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Finding best factory for FactoryName: \"%ls\"", *InConfig.PitchShifter.Name.ToString());
	...
	if (FactoryName == NAME_None)
	{
		...
		UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "FactoryName is NONE, using DefaultFactoryName: \"%ls\"", *FactoryName.ToString());
	}
	...
	while (FactoryName != NAME_None)
	{
		if (IStretcherAndPitchShifterFactory* Factory = FindFactory(FactoryName))
		{
			...
			{
				UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Found available factory for FactoryName: \"%ls\"", *FactoryName.ToString());
				return Factory;
			}

			UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Factory for FactoryName: \"%ls\" has no free pitch shifters. Falling back to next in line.", *FactoryName.ToString());
		}
		...
		if (FactoryPriority.IsValidIndex(PriorityIndex))
		{
			if (FactoryPriority.IsValidIndex(++PriorityIndex))
			{
				...
				UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Fallback: Finding factory for FactoryName: \"%ls\"", *FactoryName.ToString());
			}
			else // we're at the end of the priority list
			{
				FactoryName = NAME_None;
				UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Fallback: No more factories to fall back to!");
			}
		}
		...
		{
			FactoryName = DefaultFactoryName;
			UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Fallback: Unable to determine a factory to fallback to (the factory name passed in is not in the Priority List!). Using DefaultFactoryName: \"%ls\"", *FactoryName.ToString());
		}
		else
		{
			// there's nothing left to try
			FactoryName = NAME_None;
			UE_LOGF(LogStretcherAndPitchShifterFactory, Verbose, "Fallback: Unable to determine a factory to fallback to (the factory name passed in is not in the Priority List!), and we already tried the DefaultFactory: \"%ls\", so there's nothing left to try!", *DefaultFactoryName.ToString());
		}
	}
