...
void IAudioDataRenderer::SetSoundWaveData(const TSharedRef<const FSoundWaveData>& InSoundWaveData, const FSettings& InSettings)
{
}

TSharedPtr<const FSoundWaveData> IAudioDataRenderer::GetSoundWaveData() const
{
	return {};
}

double IAudioDataRenderer::CalculateLerpData(FLerpData* LerpArray, int32 InNumPoints, uint32 InNumOutFrames, double InPos, int32 InMaxFrame, double InInc) const
{
	check(InNumPoints >= (int32)InNumOutFrames);
	FMemory::Memset(LerpArray, 0, sizeof(FLerpData) * InNumPoints);

	const TSharedPtr<const FSoundWaveData> SoundWaveData = GetSoundWaveData();
	if (!SoundWaveData)
	{
		return 0.0;
	}

	if (InMaxFrame < 0 || InMaxFrame > (int32)SoundWaveData->GetNumFrames())
	{
		InMaxFrame = (int32)SoundWaveData->GetNumFrames();
	}
	...
	if (SoundWaveData->GetLoopRegions().Num() > 0)
	{
		IsLooping = true;
