...
public:
	...
	*/
	...
	void AssertEqualWithTolerance(const TAudioBuffer<DataType>& Other, DataType InTolerance) const
	{
		...
		UE_LOGF(LogAudioBuffer, Error, "AudioBuffer<T>::AssertEqualWithTolerance failed on channel %d at frame %d\n", ch, fr);

		const static int32 sLength = 5;
		int32 framesRemaining = GetNumValidFrames() - fr;
		int32 length = FMath::Min(framesRemaining, sLength);

		UE_LOGF(LogAudioBuffer, Error, "this buffer channel %d, frame %d through %d:\n", ch, fr, fr + length - 1);
		FAudioBuffer::DebugLog(GetValidChannelData(ch) + fr, length);
		UE_LOGF(LogAudioBuffer, Error, "Other buffer channel %d, frame %d through %d:\n", ch, fr, fr + length - 1);
