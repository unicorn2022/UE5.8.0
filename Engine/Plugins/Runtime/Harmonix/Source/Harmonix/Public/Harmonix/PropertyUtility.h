...
bool CopyStructProperty(TStructType* InDest, TStructType* InSrc, const FPropertyChangedChainEvent& PropertyChangedChainEvent)
{
	...
	{
		UE_LOGF(LogPropertyUtility, Warning, "Can't copy structs. Dest and Src must both be valid pointers");
		return false;
	}
	...
	if (FStructProperty* StructProp = CastField<FStructProperty>(Head->GetValue()))
	{
		...
		{
			UE_LOGF(LogPropertyUtility, Warning, "Can't copy structs from property chain. Property Chain Head FStructType is of type: %ls, while the src struct is of type: %ls.",
	...
	else
	{
		...
		UE_LOGF(LogPropertyUtility, Warning, "Can't copy structs from property chain. Property Chain head is not a FStructType.");
		return false;
	}
	...
	while (Node)
	{
		...
		{
			UE_LOGF(LogPropertyUtility, Warning,
				"Got a nullptr copying struct: \"%ls\"\n\tProperty: \"%ls\"",
