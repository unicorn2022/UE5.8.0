...
namespace MetasoundMusicClockDriver
{
	float Fudge = 1.00f;
	float kP = 0.18f;
	float HistoricSmoothedAudioRenderLagSeconds = 0.030f; // this used to be baked-in/hardcoded into the smoothing of the audio render time. 
	...
	double SlowestCorrectionSpeed = 0.98f;
	double FastestCorrectionSpeed = 1.02f;
...
TOptional<FVector> FMetasoundMusicClockDriver::TryGetAudioSourceLocation() const
{
...
FMidiSongPos FMetasoundMusicClockDriver::CalculateSongPosAtMsForLoopingOrMonotonicClock(float AbsoluteMs, float& PositionTick, bool& SeekDetected, bool& LoopDetected) const
{
	...
	if (CurrentMapChain->LoopLengthTicks > 0)
	{
		...
		if (DrivingTick >= TickPastLoop)
		{
			...
			if (LoopDetected)
			{
				UE_LOGF(LogMetasoundMusicClockDriver, VeryVerbose, "Detected loop when calculating song pos (%f -> %f -> %f -> %d )", PositionTick, WrappedTick, PositionTick - WrappedTick, CurrentMapChain->LoopLengthTicks);
			}
			...
			if (!LoopDetected)
			{
				...
				if (SeekDetected)
				{
					UE_LOGF(LogMetasoundMusicClockDriver, VeryVerbose, "Detected seek when calculating song pos (%f -> %f)", PositionTick, WrappedTick);
				}
			}
	...
	if (SeekDetected)
	{
		UE_LOGF(LogMetasoundMusicClockDriver, VeryVerbose, "Detected seek when calculating song pos (%f -> %f)", PositionTick, NewPositionTick);
	}
...
FString FMetasoundMusicClockDriver::GetDisplayName() const
{
#if WITH_EDITORONLY_DATA
	// that method is only good WITH_EDITORONLY_DATA
	if (UAudioComponent* Component = AudioComponentToWatch.Get())
	{
		...
		{
			return FString::Printf(TEXT("Metasound: %s"), *Source->GetDisplayName().ToString());
		}
	}
#endif
...
{
	OnAttachedDelegate = OnGeneratorAttachedCallback;
...
void FMetasoundMusicClockDriver::RefreshCurrentSongPosFromHistory()
{
	...
	if (!bClockIsStopped)
	{
		...
		{
			UE_LOGF(LogMetasoundMusicClockDriver, Verbose, "======== MASSIVE ERROR (%f) - SEEKING ===========", (float)Error);
			RenderStartSampleCount = LastRenderPosSampleCount;
			RenderStartWallClockTimeSeconds = GetWallClockTime() - (double)RenderStartSampleCount / (double)ClockHistory->SampleRate;
	...
	//UE_LOGF(LogMetasoundMusicClockDriver, Log, "Local Minimum Error: %f Average: %f Speed: %f", (float)ErrorTracker.Min(), (float)ErrorTracker.Average(), (float)SyncSpeed);
	...
	if (Result != EHistoryFailureType::None)
	{
		if (LastRenderPosSampleCount >  (RenderSmoothingLagSeconds * ClockHistory->SampleRate * 2))
		{
			if (RenderSmoothingLagSeconds < 0.250f)
			{
				...
				UE_LOGF(LogMetasoundMusicClockDriver, Verbose, "(%lld) Smoothed Render Time too close to actual render time. Bumping up smoothing lag! (%f)", LastRenderPosSampleCount, RenderSmoothingLagSeconds);
			}
			else
			{
				UE_LOGF(LogMetasoundMusicClockDriver, Verbose, "(%lld) Smoothed Render Time too close to actual render time. ALREADY MAX SMOOTHING LAG! (%f)", LastRenderPosSampleCount, RenderSmoothingLagSeconds);
			}
		}
		else
		{
			UE_LOGF(LogMetasoundMusicClockDriver, Verbose, "(%lld) Smoothed Render Time too close to actual render time. WAITING!", LastRenderPosSampleCount);
		}
		return;
	}
