...
public:
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "MusicClock")
	UE_API FTimeSignature GetCurrentTimeSignatureStruct() const;
	...
	/* Convert zero indexed fractional beat to Ms */ 
	UFUNCTION(BlueprintPure, Category = "Beat")
	UE_API float BeatToMs(float Beat) const;

	/* Convert Ms to zero indexed fractional beat */
	UFUNCTION(BlueprintPure, Category = "Beat")
	UE_API float MsToBeat(float Ms) const;
	 ...
	 * @param StartTime      Optional start time. If unset, the current musical
	 *                       time (floored) is used.
	...
	FMusicTimerHandle BP_AddTimer(const FMusicTimeInterval& TimerInterval, TOptional<FMusicTimestamp> StartTime, ECalibratedMusicTimebase Timebase, bool bLooping, FOnMusicalTimerExecute TimerDelegate);
	 ...
	 * @param TimerDelegate  Delegate executed when the timer fires.
	 */
	UE_API FMusicTimerHandle AddTimerNative(const FMusicTimeInterval& TimerInterval, TOptional<FMusicTimestamp> StartTime, ECalibratedMusicTimebase Timebase, bool bLooping, FSimpleDelegate TimerDelegate);
	...
	UFUNCTION(BlueprintCallable, Category = "Music|Timer")
	FMusicTimestamp Quantize(const FMusicTimestamp& Time, EMidiClockSubdivisionQuantization QuantizationInterval, EMidiFileQuantizeDirection InDirection) const;

#if !UE_BUILD_SHIPPING
	// A short string description of the clock, for debugging, like song name or animation name or whatever is driving it.
	UE_API FString GetDisplayName() const;

	// Sets the DisplayName. Will be used by GetDisplayName if set, otherwise falls back to drivers. 
	UE_API void SetDisplayName(FString Name);
#endif
...
public:
	...
	static UE_API UMusicClockComponent* CreateMetasoundDrivenMusicClock(UObject* WorldContextObject, UAudioComponent* InAudioComponent, FName MetasoundOuputPinName = "MIDI Clock", bool Start = true, bool bPersistAcrossLevelTransition = false);

	UFUNCTION(BlueprintCallable, Category = "Audio|MusicClock", meta = (WorldContext = "WorldContextObject", DefaultToSelf = "WorldContextObject"))
	static UE_API UMusicClockComponent* CreateWallClockDrivenMusicClock(UObject* WorldContextObject, UMidiFile* WithTempoMap, bool Start = true, bool bPersistAcrossLevelTransition = false);

	static UE_API UMusicClockComponent* CreateMusicClockWithSettings(UObject* WorldContextObject, TInstancedStruct<FMusicClockSettingsBase> MusicClockSettings, bool Start = true, bool bPersistAcrossLevelTransition = false);

	template<typename MusicClockSettingsType = FMusicClockSettingsBase>
	static UMusicClockComponent* CreateMusicClockWithSettings(UObject* WorldContextObject, const MusicClockSettingsType& InMusicClockSettings, bool Start = true, bool bPersistAcrossLevelTransition = false)
	{
		return CreateMusicClockWithSettings(WorldContextObject, TInstancedStruct<FMusicClockSettingsBase>::Make(InMusicClockSettings), Start, bPersistAcrossLevelTransition);
	}
	...
	bool HasValidClockDriver() const { return ClockDriver.IsValid(); }
...
#if !UE_BUILD_SHIPPING
	// Holds a name for display, ideally shows the original source for the clock.
	FString DisplayName;
#endif
	
	UPROPERTY(Transient)
	TObjectPtr<UMusicTimerManager> MusicTimerManager;

	static TNotNull<UMusicClockComponent*> CreateMusicClockShared(UObject* WorldContextObject, bool bPersistAcrossLevelTransition);
