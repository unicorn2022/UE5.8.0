...
int32 UOpenCVBlueprintFunctionLibrary::OpenCVChessboardDetectCorners(const UTextureRenderTarget2D* InRenderTarget, const FIntPoint InPatternSize, const bool bDebugDrawCorners, UTexture2D*& OutDebugTexture, TArray<FVector2D>& OutDetectedCorners)
{
	...
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVChessboardDetectCorners: Invalid InRenderTarget");
		return 0;
	}

	if (InRenderTarget->RenderTargetFormat != ETextureRenderTargetFormat::RTF_RGBA8)
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVChessboardDetectCorners: Invalid InRenderTarget texture format - only RGBA8 is currently supported");
		return 0;
	}
	...
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVChessboardDetectCorners: Can't get render target resource from InRenderTarget");
		return 0;
	}
	...
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVChessboardDetectCorners: Failed to read pixels from RenderTarget");
		return 0;
	}
	...
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVChessboardDetectCorners: Invalid number of pixels read from RenderTarget");
		return 0;
	}
...
#else
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVChessboardDetectCorners: Requires OpenCV");
		return 0;
	}
#endif	// WITH_OPENCV
}
...
int32 UOpenCVBlueprintFunctionLibrary::OpenCVArucoDetectMarkers(const UTextureRenderTarget2D* InRenderTarget, const EOpenCVArucoDictionary InDictionary, const EOpenCVArucoDictionarySize InDictionarySize, const bool bDebugDrawMarkers, const bool bEstimatePose, const float InMarkerLengthInMeters, const FOpenCVLensDistortionParametersBase& InLensDistortionParameters, UTexture2D*& OutDebugTexture, TArray<FOpenCVArucoDetectedMarker>& OutDetectedMarkers)
{
	...
	if (InRenderTarget == nullptr)
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Invalid InRenderTarget");
		return 0;
	}
	...
	if (bEstimatePose)
	{
		...
		{
			UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: bEstimatePose specified, but InMarkerLengthInMeters is <= 0");
			return 0;
		}
	}
	...
	if (InRenderTarget->RenderTargetFormat != ETextureRenderTargetFormat::RTF_RGBA8)
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Invalid InRenderTarget texture format - only RGBA8 is currently supported");
		return 0;
	}
	...
	if (RenderTarget == nullptr)
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Can't get render target resource from InRenderTarget");
		return 0;
	}
	...
	if (!RenderTarget->ReadPixels(Pixels))
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Failed to read pixels from RenderTarget");
		return 0;
	}
	...
	if ((Size.X <= 0) || (Size.Y <= 0) || (Pixels.Num() != (Size.X * Size.Y)))
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Invalid number of pixels read from RenderTarget");
		return 0;
	}
	...
	if (NumMarkersFound > 0)
	{
		...
		{
			UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Number of detected MarkerIds and MarkerCorners don't match");
			return 0;
		}
		...
		for (const std::vector<cv::Point2f>& Corner : MarkerCorners)
		{
			...
			{
				UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Number of detected corners per marker must be exactly %d", kMaxCorners);
				return 0;
			}
		}
		...
		if (bDebugDrawMarkers)
		{
			...
			if (bEstimatePose)
			{
				static const float AxisLengthInMeters = 0.05f;
...
#else
	{
		UE_LOGF(LogOpenCVBlueprintFunctionLibrary, Error, "OpenCVArucoDetectMarkers: Requires OpenCV");
		return 0;
	}
#endif	// WITH_OPENCV
}
