...
struct FCharucoBoardConfig
{
	...
	/** ArUco dictionary to use for the markers. Smaller dictionaries tend to be easier to parse */
	EArucoDictionary Dictionary = EArucoDictionary::DICT_4X4_50;
};


/** Structure representing detected Charuco corners */
struct FCharucoCorners
{
	/** 2D image coordinates of detected corners */
	TArray<FVector2f> Corners;
	
	/** IDs of the detected corners (based on board layout) */
	TArray<int32> CornerIds;
	
	/** Detected ArUco markers in the Charuco board */
...
public:
	...
	/** Identify Charuco corners in the input image with the given board configuration, and output the detected corners and their IDs */
	static OPENCVHELPER_API bool IdentifyCharucoCorners(TArray<FColor>& Image, FIntPoint ImageSize, const FCharucoBoardConfig& BoardConfig, FCharucoCorners& OutCorners, const FIntRect& ROI = FIntRect());

	/** Draw a debug view of the input Charuco corners on top of the input texture */
	static OPENCVHELPER_API bool DrawCharucoCorners(const FCharucoCorners& Corners, UTexture2D* DebugTexture);

	/** Generate a Charuco board image with the given configuration for display or printing */
	static OPENCVHELPER_API UTexture2D* GenerateCharucoBoard(const FCharucoBoardConfig& BoardConfig, FIntPoint ImageSize, int32 MarginSize = 10);
};
