...
namespace UE::ADM::Spatialization
{
	void FModule::StartupModule()
	{
		...
		if (const UADMSpatializationSettings* Settings = GetDefault<UADMSpatializationSettings>())
		{
			...
			{
				UE_LOGF(LogADMSpatialization, Display, "Failed to parse specified default ADM Spatialization client endpoint in developer settings. Default IP not set for ADM Spatialization.");
			}
		}
		else
		{
			UE_LOGF(LogADMSpatialization, Error, "Failed to find ADM Spatialization Developer Settings. Default IP not set for ADM Spatialization.");
		}
