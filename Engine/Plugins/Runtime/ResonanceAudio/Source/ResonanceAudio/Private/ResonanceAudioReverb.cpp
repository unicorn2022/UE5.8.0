...
namespace ResonanceAudio
{
	...
	{
		ReverbPluginPreset = nullptr;
		if (!Audio::GRemoveSubmixesFromRoot && ReverbSubmixPtr)
		{
	...
	void FResonanceAudioReverb::ProcessMixedAudio(const FSoundEffectSubmixInputData& InData, FSoundEffectSubmixOutputData& OutData)
	{
		...
		{
			UE_LOGF(LogResonanceAudio, Warning, "Resonance Audio Reverb connected to 1-channel output, down-mixing spatialized audio");
	...
	void FResonanceAudioReverb::InitEffectSubmix()
	{
		...
		{
			UE_LOGF(LogResonanceAudio, Error, "Failed to create Resonance Reverb global Preset");
			ReverbPluginPreset = nullptr;
		}
	}
	...
	USoundSubmix* FResonanceAudioReverb::LoadSubmix()
	{
		...
		if (!ReverbSubmixPtr)
		{
			if (!IgnoreUserResonanceSubmixCVar)
			{
				...
				UE_LOGF(LogResonanceAudio, Error, "Couldn't load Resonance Reverb Submix from object path '%ls' in ResonanceSettings. Creating stub submix '%ls'.",
					*Settings->OutputSubmix.GetAssetPathString(),
					*DefaultSubmixName);
			}

			ReverbSubmixPtr = NewObject<USoundSubmix>();
			// This AddToRoot is redundant since we AddToRoot in the LoadSoundSubmix stack in the AudioMixerDevice.
			// LoadSoundSubmix gets called immediately after this function in FMixerDevice::LoadPluginSoundSubmixes.
			if (!Audio::GRemoveSubmixesFromRoot)
			{
			...
			if (const FAudioDeviceManager* DeviceManager = FAudioDeviceManager::Get())
			{
				if (const FAudioDevice* MainDevice = DeviceManager->GetMainAudioDeviceRaw())
				{
					...
					UE_LOGF(LogResonanceAudio, Log, "Setting Resonance's submix parent to Main Submix %ls"
		...
		if (ReverbPluginPreset)
		{
			...
			if (!bFoundPreset)
			{
				if (!IgnoreUserResonanceSubmixCVar)
				{
					...
					UE_LOGF(LogResonanceAudio, Error, "Failed to find Resonance UResonanceAudioReverbPluginPreset on default reverb submix. Creating stub '%ls'.",
						*DefaultPresetName);
				}
