...
namespace ResonanceAudio
{
	...
	void* LoadResonanceAudioDynamicLibrary()
	{
		auto LoadLibrary = [](const FString& FilePath)
		{
			...
			UE_LOGF(LogResonanceAudio, Log, "Attempting to load %ls", *DynamicLibraryToLoad);
			...
			{
				UE_LOGF(LogResonanceAudio, Log, "File does not exist. %ls", *DynamicLibraryToLoad);
			}
	
			if (!DynamicLibraryHandle)
			{
				UE_LOGF(LogResonanceAudio, Log, "Unable to load %ls.", *FPaths::ConvertRelativePathToFull(DynamicLibraryToLoad));
			}
			else
			{
				UE_LOGF(LogResonanceAudio, Log, "Loaded %ls.", *DynamicLibraryToLoad);
			}
...
#else
		UE_LOGF(LogResonanceAudio, Error, "Unsupported Platform. Supported platforms are ANDROID, IOS, LINUX, MAC and WINDOWS");
		return nullptr;
	...
	vraudio::MaterialName ConvertToResonanceMaterialName(ERaMaterialName UnrealMaterialName)
	{
		...
		default:
			UE_LOGF(LogResonanceAudio, Error, "Acoustic Material does not exist. Returning Transparent Material.")
			return vraudio::MaterialName::kTransparent;
		}
	}
