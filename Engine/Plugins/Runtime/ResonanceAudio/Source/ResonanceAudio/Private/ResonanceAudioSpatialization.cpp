...
namespace ResonanceAudio {
	...
	void FResonanceAudioSpatialization::Initialize(const FAudioPluginInitializationParams InitializationParams)
	{
		...
		UE_LOGF(LogResonanceAudio, Log, "Google Resonance Audio Spatialization is initialized.");
	}
	...
	void FResonanceAudioSpatialization::OnInitSource(const uint32 SourceId, const FName& AudioComponentUserId, USpatializationPluginSourceSettingsBase* InSettings)
	{
		...
		{
			UE_LOGF(LogResonanceAudio, Error, "No Spatialization Settings Preset added to the sound source! Please create a new preset or add an existing one.");
			return;
		}
		...
		if (SpatializationSettings[SourceId]->SpatializationMethod != ERaSpatializationMethod::STEREO_PANNING)
		{
			...
			case ERaQualityMode::STEREO_PANNING:
				...
				UE_LOGF(LogResonanceAudio, Verbose, "ResonanceAudioSpatializer::OnInitSource: STEREO_PANNING mode chosen.");
				break;

			case ERaQualityMode::BINAURAL_LOW:
				ResonanceRenderingMode = vraudio::RenderingMode::kBinauralLowQuality;
				UE_LOGF(LogResonanceAudio, Verbose, "ResonanceAudioSpatializer::OnInitSource: BINAURAL_LOW mode chosen.");
				break;

			case ERaQualityMode::BINAURAL_MEDIUM:
				ResonanceRenderingMode = vraudio::RenderingMode::kBinauralMediumQuality;
				UE_LOGF(LogResonanceAudio, Verbose, "ResonanceAudioSpatializer::OnInitSource: BINAURAL_MEDIUM mode chosen.");
				break;

			case ERaQualityMode::BINAURAL_HIGH:
				ResonanceRenderingMode = vraudio::RenderingMode::kBinauralHighQuality;
				UE_LOGF(LogResonanceAudio, Verbose, "ResonanceAudioSpatializer::OnInitSource: BINAURAL_HIGH mode chosen.");
				break;

			default:
				UE_LOGF(LogResonanceAudio, Error, "ResonanceAudioSpatializer::OnInitSource: Unknown quality mode!");
				break;
			}
		}
		...
		case ERaDistanceRolloffModel::NONE:
			...
			break;
		default:
			UE_LOGF(LogResonanceAudio, Error, "ResonanceAudioSpatializer::OnInitSource: Undefined distance roll-off model!");
			break;
		}
		...
		{
			UE_LOGF(LogResonanceAudio, Warning, "Source initialized (Our source ID: %i) (Binaural SourceId: %i).", SourceId, BinauralSource.Id);
		}
	}
	...
	void FResonanceAudioSpatialization::OnReleaseSource(const uint32 SourceId)
	{
		...
		if (ExtraResonanceLoggingCVar)
		{
			UE_LOGF(LogResonanceAudio, Warning, "Destroying Source (Our Source ID: %i)(Binaural Source ID: %i)", SourceId, BinauralSource.Id);
		}
	}
	...
	void FResonanceAudioSpatialization::ProcessAudio(const FAudioPluginSourceInputData& InputData, FAudioPluginSourceOutputData& OutputData)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogResonanceAudio, Warning, "FResonanceAudioSpatialization::ProcessAudio() has been called %i times", NumBuffersBetweenLogs);
			}
		}
	}
