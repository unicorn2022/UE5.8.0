...
void UpdateGPUUsageCache()
{
	...
	for (uint32 GPUIndex = 0; GPUIndex < GNumExplicitGPUsForRendering; GPUIndex++)
	{
		...
		if (!Result)
		{
			UE_LOGF(LogRHI, Error, "QueryStatistics failed for the entire GPU%d use", GPUIndex);
			continue;
		}
...
void InitGetGPUUsage()
{
	...
	{
		UE_LOGF(LogRHI, Display, "FExternalGPUStatistics disabled due to null RHI.");
		return;
	}

	if (IsRunningCommandlet())
	{
		UE_LOGF(LogRHI, Display, "FExternalGPUStatistics doesn't matter in any commandlets.");
		return;
	}
	...
	{
		UE_LOGF(LogRHI, Display, "FExternalGPUStatistics is not supported with software rendering.");
		return;
	}
	...
	else if (IsRHIDeviceIntel())
	{
		...
		{
			UE_LOGF(LogRHI, Error, "Failed to initialize Intel Level Zero API!");
			ShutdownIntel();
			return;
		}
		...
		for (int32 Index = 0; Index < IntelDrivers.Num(); Index++)
		{
			...
			{
				UE_LOGF(LogRHI, Error, "Failed to initialize devices for driver %u!", Index);
				ShutdownIntel();
				return;
			}
		}
		...
		{
			UE_LOGF(LogRHI, Error, "Discovered %u GPUs that can render, expected at least %u", IntelDevices.Num(), GNumExplicitGPUsForRendering);
			ShutdownIntel();
			return;
		}
		...
		for (uint32 GPUIndex = 0; GPUIndex < GNumExplicitGPUsForRendering; GPUIndex++)
		{
			...
			{
				UE_LOGF(LogRHI, Error, "Failed to setup metrics for GPU %u", GPUIndex);
				ShutdownIntel();
				return;
			}
			...
			{
				UE_LOGF(LogRHI,Error, "Could not activate metrics for GPU %u", GPUIndex);
				ShutdownIntel();
				return;
			}
			...
			{
				UE_LOGF(LogRHI, Error, "Failed to setup metrics streamers for GPU %u", GPUIndex);
				ShutdownIntel();
				return;
			}
		}
	...
	{
		UE_LOGF(LogRHI, Error, "FExternalGPUStatistics could not be initialized on unsupported vendor!");
		return;
	}
	...
	for (uint32 GPUIndex = 0; GPUIndex < GNumExplicitGPUsForRendering; GPUIndex++)
	{
		...
		if (CVarPerProcessGPUStatisticsEnable.GetValueOnAnyThread() && bIsPerProcessDataExpectedToWork)
		{
			...
			{
				UE_LOGF(LogRHI, Error, "QueryStatistics failed for the current process use of GPU%d", GPUIndex);
				return;
			}	
		}
	}
	...
	UE_LOGF(LogRHI, Display, "ExternalGPUStatistics module fully operational");
} // InitGetGPUUsage()
...
void ShutdownGetGPUUsage()
{
	...
	UE_LOGF(LogRHI, Display, "ExternalGPUStatistics module is no longer operational");
}
...
void InitExternalGPUStatisticsModule()
{
	...
	{
		UE_LOGF(LogRHI, Display, "FExternalGPUStatistics has been disabled with r.GPUStatistics=0");
	}
...
public:
	...
	{
		FCoreDelegates::GetOnPostEngineInit().AddStatic(&InitExternalGPUStatisticsModule);
	}
};
