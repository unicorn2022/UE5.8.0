...
void UMotorPhysicsSimComponent::Update(FAudioMotorSimInputContext& Input, FAudioMotorSimRuntimeContext& RuntimeInfo)
{
	...
	UE_LOGF(LogAudioMotorSim, Verbose, "[UMotorPhysicsSimComponent] NetForce: %f (CurrentEngineTorque: %f - BrakingForce: %f - WindResistance %f - GroundFrictionResistance %f - EngineResistance %f", NetForce, CurrentEngineTorque, BrakingForce, WindResistance, GroundFrictionResistance, EngineResistance)
	...
	UE_LOGF(LogAudioMotorSim, Verbose, "[UMotorPhysicsSimComponent] Curr Gear %d - RPM: %f", Gear, Rpm);
	UE_LOGF(LogAudioMotorSim, Verbose, "[UMotorPhysicsSimComponent] Prev Gear %d - RPM: %f", (Gear >= 1 ? Gear - 1 : 0), CalcRpm(GetGearRatio((Gear >= 1 ? Gear - 1 : 0), Input.bClutchEngaged), VirtualSpeedKmh));
