...
void SDataRegistryItemNameWidget::OnTypeChanged(bool bClearInvalid)
{
	...
	{
		UE_LOGF(LogDataRegistryIdCustomization, Warning, "%ls reset to None because it's not a valid data registry ID", *CachedIdValue.ToString());
...
namespace DataRegistryCustomizationPrivate
{
	template <typename T>
	T* GetPropertyData(TSharedPtr<IPropertyHandle> StructPropertyHandle)
	{
		if (StructPropertyHandle.IsValid())
		{
			void* StructAddress = nullptr;
			if (StructPropertyHandle->GetValueData(StructAddress) == FPropertyAccess::Success)
			{
				return static_cast<T*>(StructAddress);
			}
		}
...
void FDataRegistryIdCustomization::CustomizeChildren(TSharedRef<class IPropertyHandle> InStructPropertyHandle, class IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	TypePropertyHandle = InStructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FDataRegistryId, RegistryType));
	...
	{
		StructBuilder.AddProperty(TypePropertyHandle.ToSharedRef());
...
FDataRegistryId FDataRegistryIdCustomization::GetCurrentValue() const
{
	...
	const FDataRegistryType* DataRegistryType = DataRegistryCustomizationPrivate::GetPropertyData<FDataRegistryType>(TypePropertyHandle);
	if (DataRegistryType && NamePropertyHandle.IsValid() && NamePropertyHandle->IsValidHandle())
	{
		OutId.RegistryType = *DataRegistryType;
...
{
	const FDataRegistryId CurrentId = GetCurrentValue();
	...
	{
		if (FDataRegistryType* DataRegistryType = DataRegistryCustomizationPrivate::GetPropertyData<FDataRegistryType>(TypePropertyHandle))
		{
			*DataRegistryType = NewId.RegistryType;
		}
	}
}
...
FString FDataRegistryIdCustomization::OnGetTypeValueString() const
{
	...
	FDataRegistryType TempRegistry;
	if (TypePropertyHandle.IsValid() && TypePropertyHandle->GetValue(TempRegistry) == FPropertyAccess::MultipleValues)
	{
