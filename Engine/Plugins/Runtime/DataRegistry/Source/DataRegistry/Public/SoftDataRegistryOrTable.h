...
struct FSoftDataRegistryOrTable
{
	...
	void ForEachItem(const FString& ContextString, TFunctionRef<void(const FName& Name, const T& Item)> Predicate) const
	{
		if (bUseDataRegistry)
		{
			...
			if (!DataRegistry)
			{
				UE_LOGF(LogDataRegistry, Warning, "[%s] No Registry found (%ls)  Registry:%ls", __FUNCTION__, *ContextString, *RegistryType.GetName().ToString());
				return;
			}
		...
		else
		{
			if (!Table)
			{
				UE_LOGF(LogDataRegistry, Warning, "[%s] No Data Table found (%ls)", __FUNCTION__, *ContextString);
				return;
			}
	...
	void GetItemNames(TArray<FName>& ItemNames) const
	{
		if (bUseDataRegistry)
		{
			...
			if (!DataRegistry)
			{
				UE_LOGF(LogDataRegistry, Warning, "[%s] No Registry found  Registry:%ls", __FUNCTION__, *RegistryType.GetName().ToString());
				return;
			}
		...
		else
		{
			if (!Table)
			{
				UE_LOGF(LogDataRegistry, Warning, "[%s] No Data Table found", __FUNCTION__);
				return;
			}
	...
	/**
	 * Returns the script struct used by this data registry or table
	 */
...
struct FDataRegistryOrTableRow
{
	...
	const T* GetItem(const TCHAR* ContextString) const
	{
		if (bUseDataRegistryId)
		{
			...
			if (!EventRegistry)
			{
				UE_LOGF(LogDataRegistry, Warning, "[%s] No Registry found  Registry:%ls", __FUNCTION__, *DataRegistryId.RegistryType.GetName().ToString());
				return nullptr;
			}
