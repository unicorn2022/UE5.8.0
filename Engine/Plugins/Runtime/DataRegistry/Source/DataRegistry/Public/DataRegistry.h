...
public:
	...
	void ForEachCachedItem(const FString& ContextString, TFunctionRef<void(const FName& Name, const T& Item)> Predicate) const
	{
		...
		if (!Result.WasFound())
		{
			UE_LOGF(LogDataRegistry, Warning, "[%s] No Registry Data found (%ls)  Registry:%ls", __FUNCTION__, *ContextString, *RegistryType.ToString());
			return;
		}

		if (ItemTypeStruct == nullptr || !ItemTypeStruct->IsChildOf(T::StaticStruct()))
		{
			UE_LOGF(LogDataRegistry, Warning, "[%s] Registry has incorrect row type (%ls)  Registry:%ls", __FUNCTION__, *ContextString, *RegistryType.ToString());
			return;
		}
	...
	const TArray<TObjectPtr<UDataRegistrySource>>& GetDataSources() const { return DataSources; }

	/** Returns the data sources obtained during runtime. */
	const TArray<TObjectPtr<UDataRegistrySource>>& GetRuntimeSources() const { return RuntimeSources; }
