...
void UDataRegistrySource_DataTable::SetCachedTable(bool bForceLoad /*= false*/)
{
	...
	if (FoundTable)
	{
		...
		{
			UE_LOGF(LogDataRegistry, Error, "Cannot initialize DataRegistry source %ls, Table %ls or registry is invalid!", *GetPathName(), *FoundTable->GetPathName());
			bInvalidSourceTable = true;
		}
		else if (!RowStruct->IsChildOf(ItemStruct))
		{
			UE_LOGF(LogDataRegistry, Error, "Cannot initialize DataRegistry source %ls, Table %ls row type %ls does not match item type %ls", *GetPathName(), *FoundTable->GetPathName(), *RowStruct->GetName(), *ItemStruct->GetName());
			bInvalidSourceTable = true;
		}
		else
		{
