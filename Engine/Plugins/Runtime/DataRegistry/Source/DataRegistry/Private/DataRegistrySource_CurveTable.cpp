...
void UDataRegistrySource_CurveTable::SetCachedTable(bool bForceLoad /*= false*/)
{
	...
	if (!FoundTable && (bForceLoad || TableRules.bPrecacheTable))
	{
		if (FUObjectThreadContext::Get().IsRoutingPostLoad)
		{
			...
			UE_LOGF(LogDataRegistry, Log, "Failed to preload table %ls for source %ls, PreloadTable is %ls", *SourceTable.ToString(), *GetPathName(), *GetNameSafe(PreloadTable));
		}
		else 
		{
			if (TableRules.bPrecacheTable && !bForceLoad)
			{
				UE_LOGF(LogDataRegistry, Verbose, "Synchronously precaching table %ls for source %ls, PreloadTable is %ls", *SourceTable.ToString(), *GetPathName(), *GetNameSafe(PreloadTable));
			}
			...
			if (!FoundTable)
			{
				...
				{
					UE_LOGF(LogDataRegistry, Warning, "Force loading table %ls for source %ls failed! Source data is invalid and will be ignored.", *SourceTable.ToString(), *GetPathName());
				}
	...
	if (FoundTable)
	{
		...
		{
			UE_LOGF(LogDataRegistry, Error, "Cannot initialize DataRegistry source %ls, Preload table was not set, resave in editor!", *GetPathName());
			bInvalidSourceTable = true;
		}
		else if (!ItemStruct || !ItemStruct->IsChildOf(FRealCurve::StaticStruct()))
		{
			UE_LOGF(LogDataRegistry, Error, "Cannot initialize DataRegistry source %ls, Curve tables only work with curve items!", *GetPathName());
			bInvalidSourceTable = true;
		}
		else if (ItemStruct->IsChildOf(FSimpleCurve::StaticStruct()) && FoundTable->GetCurveTableMode() != ECurveTableMode::SimpleCurves)
		{
			UE_LOGF(LogDataRegistry, Error, "Cannot initialize DataRegistry source %ls, only simple curve tables are supported for SimpleCurve items, reimport with linear interpolation type or select different table", *GetPathName());
			bInvalidSourceTable = true;
		}
		else if (ItemStruct->IsChildOf(FRichCurve::StaticStruct()) && FoundTable->GetCurveTableMode() != ECurveTableMode::RichCurves)
		{
			UE_LOGF(LogDataRegistry, Error, "Cannot initialize DataRegistry source %ls, only rich curve tables are supported for RichCurve items, reimport with cubic interpolation type or select different table", *GetPathName());
			bInvalidSourceTable = true;
		}
		else
		{
...
void UDataRegistrySource_CurveTable::OnTableLoaded()
{
	...
	{
		UE_LOGF(LogDataRegistry, Warning, "Loading table %ls for source %ls failed! Source data is invalid and will be ignored.", *SourceTable.ToString(), *GetPathName());
		bInvalidSourceTable = true;
	}
