...
TSharedPtr<IAnalyticsProvider> FAnalyticsMulticast::CreateAnalyticsProvider(const FAnalyticsProviderConfigurationDelegate& GetConfigValue) const
{
	if (GetConfigValue.IsBound())
	{
		...
		{
			UE_LOGF(LogAnalytics, Warning, "CreateAnalyticsProvider delegate did not contain required parameter %ls", *Config::GetKeyNameForProviderModuleNames());
			return NULL;
		}
...
{
	UE_LOGF(LogAnalytics, Verbose, "Initializing Multicast Analytics provider");
...
{
	UE_LOGF(LogAnalytics, Verbose, "Destroying Multicast Analytics provider");
}
...
{
	if (TArray<TSharedPtr<IAnalyticsProvider> >::TConstIterator it(Providers);it)
	{
		return (*it)->GetUserID();
	}
	return FString();
