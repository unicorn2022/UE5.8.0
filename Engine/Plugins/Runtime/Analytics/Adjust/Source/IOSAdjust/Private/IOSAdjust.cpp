...
TSharedPtr<IAnalyticsProvider> FAnalyticsIOSAdjust::CreateAnalyticsProvider(const FAnalyticsProviderConfigurationDelegate& GetConfigValue) const
{
	...
	{
		UE_LOGF(LogAnalytics, Warning, "IOSAdjust::CreateAnalyticsProvider called with an unbound delegate");
	}
	return nullptr;
}
...
bool FAnalyticsProviderAdjust::StartSession(const TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	if (!bHasSessionStarted)
	{
		UE_LOGF(LogAnalytics, Display, "IOSAdjust::StartSession(%d attributes)", AttrCount);
	}
	else
	{
		UE_LOGF(LogAnalytics, Display, "IOSAdjust::RestartSession(%d attributes)", AttrCount);
	}
...
void FAnalyticsProviderAdjust::EndSession()
{
	...
	UE_LOGF(LogAnalytics, Display, "IOSAdjust::EndSession");
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
#endif
}
...
void FAnalyticsProviderAdjust::FlushEvents()
{
	...
	UE_LOGF(LogAnalytics, Display, "IOSAdjust::FlushEvents");
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
#endif
}
...
#if WITH_ADJUST
	UserId = InUserID;
	UE_LOGF(LogAnalytics, Display, "IOSAdjust::SetUserID(%ls)", *UserId);
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
#endif
}
...
{
#if WITH_ADJUST
	UE_LOGF(LogAnalytics, Display, "IOSAdjust::GetUserID - returning cached id '%ls'", *UserId);
...
FString FAnalyticsProviderAdjust::GetSessionID() const
{
	...
	UE_LOGF(LogAnalytics, Display, "IOSAdjust::GetSessionID - returning the id as '%ls'", *Id);
...
#if WITH_ADJUST
	// Ignored
	UE_LOGF(LogAnalytics, Display, "IOSAdjust::SetSessionID - ignoring call");
	return false;
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
	return false;
...
void FAnalyticsProviderAdjust::RecordEvent(const FString& EventName, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	if (EventTokenRef != nullptr)
	{
		...
		UE_LOGF(LogAnalytics, Display, "IOSAdjust::RecordEvent('%ls', %d attributes) Token=%ls", *EventName, AttrCount, *EventToken);
	}
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
#endif
...
//		[event setTransactionId:[NSString stringWithFString : ItemId]];
		...
		UE_LOGF(LogAnalytics, Display, "IOSAdjust::RecordItemPurchase('%ls', '%ls', %d, %d) Token=%ls", *ItemId, *Currency, PerItemCost, ItemQuantity, *EventToken);
	}
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
#endif
...
void FAnalyticsProviderAdjust::RecordCurrencyPurchase(const FString& GameCurrencyType, int GameCurrencyAmount, const FString& RealCurrencyType, float RealMoneyCost, const FString& PaymentProvider)
{
	...
	if (EventTokenRef != nullptr)
	{
		...
		UE_LOGF(LogAnalytics, Display, "IOSAdjust::RecordCurrencyPurchase('%ls', %d, '%ls', %.02f, %ls) Token=%ls", *GameCurrencyType, GameCurrencyAmount, *RealCurrencyType, RealMoneyCost, *PaymentProvider, *EventToken);
	}
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
#endif
...
void FAnalyticsProviderAdjust::RecordCurrencyGiven(const FString& GameCurrencyType, int GameCurrencyAmount)
{
	...
	if (EventTokenRef != nullptr)
	{
		...
		UE_LOGF(LogAnalytics, Display, "IOSAdjust::RecordCurrencyGiven('%ls', %d) Token=%ls", *GameCurrencyType, GameCurrencyAmount, *EventToken);
	}
#else
	UE_LOGF(LogAnalytics, Warning, "WITH_ADJUST=0. Are you missing the SDK?");
#endif
