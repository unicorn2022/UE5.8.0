...
bool FAnalyticsProviderFileLogging::StartSession(const TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	if (FileArchive != nullptr)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Session created file (%ls) for user (%ls)", *FileName, *UserId);
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::StartSession failed to create file to log analytics events to");
	}
...
void FAnalyticsProviderFileLogging::EndSession()
{
	if (FileArchive != nullptr)
	{
		...
		delete FileArchive;
		FileArchive = nullptr;
		UE_LOGF(LogFileLoggingAnalytics, Display, "Session ended for user (%ls) and session id (%ls)", *UserId, *SessionId);
	}
	bHasWrittenFirstEvent = false;
	bHasSessionStarted = false;
}
...
void FAnalyticsProviderFileLogging::FlushEvents()
{
	...
	{
		FileArchive->Flush();
		UE_LOGF(LogFileLoggingAnalytics, Display, "Analytics file flushed");
	}
}
...
void FAnalyticsProviderFileLogging::SetUserID(const FString& InUserID)
{
	...
	{
		UserId = InUserID;
		UE_LOGF(LogFileLoggingAnalytics, Display, "User is now (%ls)", *UserId);
	}
	else
	{
		// Log that we shouldn't switch users during a session
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::SetUserID called while a session is in progress. Ignoring.");
	}
}
...
bool FAnalyticsProviderFileLogging::SetSessionID(const FString& InSessionID)
{
	if (!bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Session is now (%ls)", *SessionId);
	}
	else
	{
		// Log that we shouldn't switch session ids during a session
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::SetSessionID called while a session is in progress. Ignoring.");
	}
	return !bHasSessionStarted;
}
...
void FAnalyticsProviderFileLogging::RecordEvent(const FString& EventName, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Analytics event (%ls) written with (%d) attributes", *EventName, Attributes.Num());
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordEvent called before StartSession. Ignoring.");
	}
}
...
void FAnalyticsProviderFileLogging::RecordItemPurchase(const FString& ItemId, const FString& Currency, int PerItemCost, int ItemQuantity)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "(%d) number of item (%ls) purchased with (%ls) at a cost of (%d) each", ItemQuantity, *ItemId, *Currency, PerItemCost);
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordItemPurchase called before StartSession. Ignoring.");
	}
}
...
void FAnalyticsProviderFileLogging::RecordCurrencyPurchase(const FString& GameCurrencyType, int GameCurrencyAmount, const FString& RealCurrencyType, float RealMoneyCost, const FString& PaymentProvider)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "(%d) amount of in game currency (%ls) purchased with (%ls) at a cost of (%f) each", GameCurrencyAmount, *GameCurrencyType, *RealCurrencyType, RealMoneyCost);
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordCurrencyPurchase called before StartSession. Ignoring.");
	}
}
...
void FAnalyticsProviderFileLogging::RecordCurrencyGiven(const FString& GameCurrencyType, int GameCurrencyAmount)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "(%d) amount of in game currency (%ls) given to user", GameCurrencyAmount, *GameCurrencyType);
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordCurrencyGiven called before StartSession. Ignoring.");
	}
}
...
void FAnalyticsProviderFileLogging::RecordError(const FString& Error, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Error is (%ls) number of attributes is (%d)", *Error, Attributes.Num());
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordError called before StartSession. Ignoring.");
	}
}
...
void FAnalyticsProviderFileLogging::RecordProgress(const FString& ProgressType, const FString& ProgressName, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Progress event is type (%ls), named (%ls), number of attributes is (%d)", *ProgressType, *ProgressName, Attributes.Num());
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordProgress called before StartSession. Ignoring.");
	}
}
...
void FAnalyticsProviderFileLogging::RecordItemPurchase(const FString& ItemId, int ItemQuantity, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Item purchase id (%ls), quantity (%d), number of attributes is (%d)", *ItemId, ItemQuantity, Attributes.Num());
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordItemPurchase called before StartSession. Ignoring.");
...
void FAnalyticsProviderFileLogging::RecordCurrencyPurchase(const FString& GameCurrencyType, int GameCurrencyAmount, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Currency purchase type (%ls), quantity (%d), number of attributes is (%d)", *GameCurrencyType, GameCurrencyAmount, Attributes.Num());
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordCurrencyPurchase called before StartSession. Ignoring.");
...
void FAnalyticsProviderFileLogging::RecordCurrencyGiven(const FString& GameCurrencyType, int GameCurrencyAmount, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	if (bHasSessionStarted)
	{
		...
		UE_LOGF(LogFileLoggingAnalytics, Display, "Currency given type (%ls), quantity (%d), number of attributes is (%d)", *GameCurrencyType, GameCurrencyAmount, Attributes.Num());
	}
	else
	{
		UE_LOGF(LogFileLoggingAnalytics, Warning, "FAnalyticsProviderFileLogging::RecordCurrencyGiven called before StartSession. Ignoring.");
