...
{
	UE_LOGF(LogModularGameplay, Display, "Dumping GameFrameworkComponentManagers...");
	for (TObjectIterator<UGameFrameworkComponentManager> ManagerIt; ManagerIt; ++ManagerIt)
	{
		...
		{
			UE_LOGF(LogModularGameplay, Display, "  Manager: %ls", *GetPathNameSafe(Manager));

#if WITH_EDITOR
			UE_LOGF(LogModularGameplay, Display, "    Receivers... (Num:%d)", Manager->AllReceivers.Num());
			for (auto SetIt = Manager->AllReceivers.CreateConstIterator(); SetIt; ++SetIt)
			{
				UE_LOGF(LogModularGameplay, Display, "      ReceiverInstance: %ls", *GetPathNameSafe(SetIt->ResolveObjectPtr()));
			}
#endif // WITH_EDITOR

			UE_LOGF(LogModularGameplay, Display, "    Components... (Num:%d)", Manager->ComponentClassToComponentInstanceMap.Num());
			for (auto MapIt = Manager->ComponentClassToComponentInstanceMap.CreateConstIterator(); MapIt; ++MapIt)
			{
				UE_LOGF(LogModularGameplay, Display, "      ComponentClass: %ls (Num:%d)", *GetPathNameSafe(MapIt.Key()), MapIt.Value().Num());
				for (const FObjectKey& ComponentInstance : MapIt.Value())
				{
					UE_LOGF(LogModularGameplay, Display, "        ComponentInstance: %ls", *GetPathNameSafe(ComponentInstance.ResolveObjectPtr()));
				}
			}
			UE_LOGF(LogModularGameplay, Display, "    Requests... (Num:%d)", Manager->ReceiverClassToComponentClassMap.Num());
			for (auto MapIt = Manager->ReceiverClassToComponentClassMap.CreateConstIterator(); MapIt; ++MapIt)
			{
				UE_LOGF(LogModularGameplay, Display, "      RequestReceiverClass: %ls (Num:%d)", *MapIt.Key().ToDebugString(), MapIt.Value().Num());
				for (const FComponentRequestInfo& ReceiverInfo : MapIt.Value())
				{
					UE_LOGF(LogModularGameplay, Display, "        RequestComponentClass: %ls	AdditionFlags: %u", *GetPathNameSafe(ReceiverInfo.Class), EnumToUnderlyingType(ReceiverInfo.AdditionFlags));
				}
			}
		}
	}
...
void UGameFrameworkComponentManager::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	FCoreUObjectDelegates::CompiledInUObjectsRemovedDelegate.AddUObject(this, &UGameFrameworkComponentManager::PruneUnboundDelegates);
		...
		if (!bUseAutoGeneratedName)
		{
			...
			if (UObject* ExistingObject = StaticFindObjectFast(nullptr, ActorInstance, NewComponentName))
			{
				if (ComponentClass->IsChildOf(ExistingObject->GetClass()))
				{
					...
					UE_LOGF(LogModularGameplay, Warning, "CreateComponentOnInstance is replacing existing object %ls with class %ls using flags 0x%08x, check for duplicate game feature actions.",
				...
				else
				{
					...
					UE_LOGF(LogModularGameplay, Error, "CreateComponentOnInstance cannot replace object %ls with incompatible class %ls using flags 0x%08x!",
						*ExistingObject->GetFullName(nullptr, EObjectFullNameFlags::IncludeClassPackage),
						*ComponentClass->GetPathName(), (int32)AdditionFlags);
...
void UGameFrameworkComponentManager::PruneUnboundDelegates(TConstArrayView<UPackage*> Packages)
{
	// With merged modules, dynamic libraries can unmount with the code backing delegate instances, including validity checks.
	// As a result, before a library unmount, we have to prune delegates registered to destroyed objects, which we detect by checking if they're still bound.
	auto PruneDelegateList = [](FActorFeatureDelegateList& DelegateList)
	{
		for (auto DelegateIter = DelegateList.CreateIterator(); DelegateIter; ++DelegateIter)
		{
			if (!(*DelegateIter)->IsBound())
			{
				UE_LOGF(LogModularGameplay, Verbose, "PruneUnboundDelegates: removing unbound delegate");
				(*DelegateIter)->bRemoved = true;
				DelegateIter.RemoveCurrent();
			}
		}
	};

	for (auto ClassFeatureChangeIter = ClassFeatureChangeDelegates.CreateIterator(); ClassFeatureChangeIter; ++ClassFeatureChangeIter)
	{
		PruneDelegateList(ClassFeatureChangeIter->Value);
	}

	for (auto ActorFeatureIter = ActorFeatureMap.CreateIterator(); ActorFeatureIter; ++ActorFeatureIter)
	{
		PruneDelegateList(ActorFeatureIter->Value.RegisteredDelegates);
	}

	for (auto ReceiverClassToEventIter = ReceiverClassToEventMap.CreateIterator(); ReceiverClassToEventIter; ++ReceiverClassToEventIter)
	{
		for (auto ExtensionHandlerEventIter = ReceiverClassToEventIter->Value.CreateIterator(); ExtensionHandlerEventIter; ++ExtensionHandlerEventIter)
		{
			if (!ExtensionHandlerEventIter->Value->Delegate.IsBound())
			{
				UE_LOGF(LogModularGameplay, Verbose, "PruneUnboundDelegates: removing unbound delegate");
				ExtensionHandlerEventIter->Value->bRemoved = true;
				ExtensionHandlerEventIter.RemoveCurrent();
			}
		}
	}
}
