...
void FNetworkPredictionProxy::Init(const FInitParams<ModelDef>& Params)
{
	...
	if (ID.IsValid() == false)
	{
		...
		if (!FNetworkPredictionDriver<ModelDef>::GetDefaultArchetype(CachedArchetype, WorldManager->PreferredDefaultTickingPolicy()))
		{
			UE_LOGF(LogNetworkPrediction, Error, "Unable to initialize DefaultArchetype. Skipping registration with NetworkPrediction");
			return;
		}
