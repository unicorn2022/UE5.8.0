...
namespace NetworkPredictionCVars
{
	...
	// Options governing fixed-tick time dilation. See CVar definitions for descriptions.
	extern UE_API TAutoConsoleVariable<bool> EnableTimeDilation;
	extern UE_API TAutoConsoleVariable<float> TimeDilationAmount;
	extern UE_API TAutoConsoleVariable<bool> AllowTimeDilationEscalation;
	extern UE_API TAutoConsoleVariable<float> TimeDilationEscalationDecay;
	extern UE_API TAutoConsoleVariable<float> TimeDilationEscalationDecayMax;
	extern UE_API TAutoConsoleVariable<float> TimeDilationMax;
	extern UE_API TAutoConsoleVariable<float> TimeDilationMin;
		...
		for (int32 Frame=StartFrame; Frame < EndFrame; ++Frame)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						UE_NP_TRACE_SYSTEM_FAULT("Recovering from input stream starvation on server. Advancing over dropped client frame %d", ServerRecvData.LastConsumedFrame + 1);
					}
				}
...
static constexpr float TimeDilationNetSerializationScale = 10000.f;	// Used when net-serializing a float into an int16
...
public:
	...
	static void NetRecv(const FNetSerializeParams& P, TClientRecvData<ModelDef>& ClientRecvState, TModelDataStore<ModelDef>* DataStore, FFixedTickState* TickState)
	{
		...
		Ar.SerializeBits(&TickState->bHasTimeDilation, 1); // 3. Whether We have Time Dilation Or Not. Most Of The time We Do Not.. We Don't Need to Send int16 over network
		if (TickState->bHasTimeDilation)
		{
			int16 NetworkTimeDilation = 1;
			Ar << NetworkTimeDilation; // 4. Time Dilation
			TickState->SuggestedTimeDilation = FMath::Clamp((float)NetworkTimeDilation / TimeDilationNetSerializationScale, NetworkPredictionCVars::TimeDilationMin->GetFloat(), NetworkPredictionCVars::TimeDilationMax->GetFloat());
		}
		else
		{
			TickState->SuggestedTimeDilation = 1.f;
		}
		...
		TCommonReplicator_AP<ModelDef>::NetRecv(P, InstanceData, ClientRecvState); // 5. Common

		InstanceData.CueDispatcher->NetRecvSavedCues(P.Ar, true, ServerFrame, 0); // 6. NetSimCues
	}
	...
	static void NetSend(const FNetSerializeParams& P, FNetworkPredictionID ID, TModelDataStore<ModelDef>* DataStore, const FFixedTickState* TickState)
	{
		...
		if (ServerRecvState)
		{
			...
			LastReceivedFrame = ServerRecvState->LastRecvFrame;
		}
		...
		int32 BufferOffset = LastConsumedFrame - LastReceivedFrame + 2;

		if (!NetworkPredictionCVars::AllowTimeDilationEscalation->GetBool())
		{
			BufferOffset = FMath::Clamp(BufferOffset, -1, 1);
		}
		...
		bool HasTimeDilation = false;

		if (BufferOffset != 0)
		{
			const float TimeDilationDecay = FMath::Clamp(1.0f - (NetworkPredictionCVars::TimeDilationEscalationDecay->GetFloat() * FMath::Abs(BufferOffset)), NetworkPredictionCVars::TimeDilationEscalationDecayMax->GetFloat(), 1.0f);
			CalculatedTimeDilation = 1.0f + ((NetworkPredictionCVars::TimeDilationAmount->GetFloat() * -BufferOffset) * TimeDilationDecay);
			CalculatedTimeDilation = FMath::Clamp(CalculatedTimeDilation, NetworkPredictionCVars::TimeDilationMin->GetFloat(), NetworkPredictionCVars::TimeDilationMax->GetFloat());
			HasTimeDilation = true;
		}
		else
		{
			CalculatedTimeDilation = 0.f;
		}

		Ar.SerializeBits(&HasTimeDilation, 1); // 3. Whether we have active time dilation. Most of the time we don't, so we can avoid sending the int16 over the network.
		
		if (HasTimeDilation)
		{
			int16 NetworkTimeDilation = FMath::RoundToInt(CalculatedTimeDilation * TimeDilationNetSerializationScale);
			Ar << NetworkTimeDilation; // 4. Time Dilation
		}

		TCommonReplicator_AP<ModelDef>::NetSend(P, *Instance, Frames->Buffer[PendingFrame]); // 5. Common

		Instance->CueDispatcher->NetSendSavedCues(P.Ar, ENetSimCueReplicationTarget::AutoProxy, true); // 6. NetSimCues
	}
};
