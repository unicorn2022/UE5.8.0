	...
	void FinalizeFrame(float DeltaTimeSeconds, const FVariableTickState* TickState) final override
	{
		...
		for (auto& It : Instances)
		{
			...
			if (FromFrame != ToFrame)
			{
				...
				{
					UE_LOGF(LogNetworkPrediction, Display, "");
					UE_LOGF(LogNetworkPrediction, Display, "Interpolation %ls. Frames: %d - %d. InterpolationTimeMS: %d. PCT: %.2f", ModelDef::GetName(), FromFrame, ToFrame, InterpolationTimeMS, PCT);

					UE_LOGF(LogNetworkPrediction, Display, "From: %d", FromTimeMS);
					FNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(FromFrameData.SyncState);

					UE_LOGF(LogNetworkPrediction, Display, "To: %d", ToTimeMS);
					FNetworkPredictionDriver<ModelDef>::template LogUserState<SyncType>(ToFromData.SyncState);

					UE_LOGF(LogNetworkPrediction, Display, "Interpolated: ");
			...
			else
			{
				...
				{
					UE_LOGF(LogNetworkPrediction, Display, "[Interpolate] Interpolation out of sync. We don't have valid frames for this time for this sim. Frame %d. TimeMS: %d. @ %d", FromFrame, ActualTimeMS, InterpolationTimeMS);
				}
