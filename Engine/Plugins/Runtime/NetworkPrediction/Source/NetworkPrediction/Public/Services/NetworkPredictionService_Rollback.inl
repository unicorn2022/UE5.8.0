	...
	int32 QueryRollback(const FFixedTickState* TickState) final override
	{
		...
		for (TConstDualSetBitIterator<FDefaultBitArrayAllocator,FDefaultBitArrayAllocator> BitIt(InstanceBitArray, DataStore->ClientRecvBitMask); BitIt; ++BitIt)
		{
			...
			if (NetworkPredictionCVars::ForceReconcile() > 0)
			{
				...
				{				
					UE_LOGF(LogNetworkPrediction, Warning, "Reconcile activated due to ForceReconcile (to RollbackFrame=%i, including %i extra rollback frames)", RollbackFrame, -(RollbackFrame-LocalFrame));
				}

				NetworkPredictionCVars::SetForceReconcile(0); // reset
			}
			else if (FNetworkPredictionDriver<ModelDef>::ShouldReconcile(InstanceData.Info.Driver, SyncAuxType(LocalFrameData.SyncState, LocalFrameData.AuxState), SyncAuxType(ClientRecvData.SyncState, ClientRecvData.AuxState)))
			{
				UE_NP_TRACE_SHOULD_RECONCILE(ClientRecvData.TraceID);
				bDoRollback = true;
				...
				if (NetworkPredictionCVars::PrintReconciles())
				{
					UE_LOGF(LogNetworkPrediction, Warning, "Reconcile required due to Sync/Aux mismatch. LocalFrame: %d. Recv Frame: %d. Offset: %d. Idx: %d", LocalFrame, ClientRecvData.ServerFrame, Offset, LocalFrame % Frames.Buffer.Capacity());

					UE_LOGF(LogNetworkPrediction, Warning, "Received:");
					FNetworkPredictionDriver<ModelDef>::LogUserStates({ClientRecvData.InputCmd, ClientRecvData.SyncState, ClientRecvData.AuxState });

					UE_LOGF(LogNetworkPrediction, Warning, "Local:");
...
public:
	...
	void Reconcile(const FVariableTickState* TickState) final override
	{
		...
		for (TConstDualSetBitIterator<FDefaultBitArrayAllocator,FDefaultBitArrayAllocator> BitIt(InstanceBitArray, DataStore->ClientRecvBitMask); BitIt; ++BitIt)
		{
			...
			if (FNetworkPredictionDriver<ModelDef>::ShouldReconcile( Instance.Info.Driver, SyncAuxType(LocalFrameData.SyncState, LocalFrameData.AuxState), SyncAuxType(ClientRecvData.SyncState, ClientRecvData.AuxState) ))
			{
				UE_NP_TRACE_SHOULD_RECONCILE(ClientRecvData.TraceID);
				if (NetworkPredictionCVars::PrintReconciles())
				{
					UE_LOGF(LogNetworkPrediction, Warning, "ShouldReconcile. Frame: %d.", LocalFrame);
