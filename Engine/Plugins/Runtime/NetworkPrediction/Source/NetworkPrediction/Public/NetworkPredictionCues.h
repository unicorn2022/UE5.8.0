...
{
	virtual ~FNetSimCueWrapperBase() = default;
...
public:
	...
	FORCENOINLINE static void RegisterCueType(const FString& TypeName)
	{
		...
		UE_LOGF(LogNetworkPredictionCues, Verbose, "RegisterCueType %ls @ 0x%llX. (NumPending: %d) ", *TypeName, &TCue::ID, RegisteredTypes.CueTypes.Num());
	...
	static void UnregisterCueType()
	{
		...
		UE_LOGF(LogNetworkPredictionCues, Verbose, "UnregisterCueType 0x%llX.", &TCue::ID);
	...
	void FinalizeCueTypes()
	{
		...
		{
			UE_LOGF(LogNetworkPredictionCues, Verbose, "FinalizeTypes: 0x%llX. %d Pending Types", &RegisteredTypes, RegisteredTypes.CueTypes.Num());
			...
			for (FTypeInfo& TypeInfo: RegisteredTypes.CueTypes)
			{
				...
				*TypeInfo.IDPtr = ++ID;
				...
				UE_LOGF(LogNetworkPredictionCues, Verbose, "    Cue %ls assigned ID: %d (0x%llX)", *TypeInfo.TypeName, *TypeInfo.IDPtr, TypeInfo.IDPtr);
			}
...
public:
	...
	void Dispatch(FSavedCue& SavedCue, TCueHandler& Handler, const int32& TimeSinceInvocation)
	{
		...
		else
		{
			...
			UE_LOGF(LogNetworkPredictionCues, Warning, "Could not Find NetCue %ls (ID: %d) in dispatch table (%d entries).", *FGlobalCueTypeTable::Get().GetTypeName(SavedCue.ID), SavedCue.ID, CueTypeInfoMap.Num());
			for (auto MapIt : CueTypeInfoMap)
			{
				UE_LOGF(LogNetworkPredictionCues, Warning, "  Have: %d", MapIt.Key);
			}
		}
	}
				...
				if (Context.TickContext == ESimulationTickContext::Resimulate)
				{
					...
					for (FSavedCue& ExistingCue : SavedCues)
					{
						...
						if (RollbackFrame <= ExistingCue.Frame && ExistingCue.NetIdentical(NewCue))
						{
							...
							UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Resimulated Cue %ls matched existing cue. Suppressing Invocation. Time: %d", *GetDebugName(), *FGlobalCueTypeTable::Get().GetTypeName(T::ID), Context.CurrentSimTime);
							return;
						}
					}
					...
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Invoking Cue %ls during resimulate. Context: %d. Transient: %d. bAllowRollback: %d. bNetConfirmed: %d. Time: %d", *GetDebugName(), *SavedCue.GetDebugName(), int(Context.TickContext), bTransient, bAllowRollback, bNetConfirmed, Context.CurrentSimTime);
				}
				else
				{
					...
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Invoking Cue %ls. Context: %d. Transient: %d. bAllowRollback: %d. bNetConfirmed: %d. Time: %d", *GetDebugName(), *SavedCue.GetDebugName(), int(Context.TickContext), bTransient, bAllowRollback, bNetConfirmed, Context.CurrentSimTime);
				}
			}
			else
			{
				UE_LOGF(LogNetworkPredictionCues, Log, "%ls .Suppressing Cue Invocation %ls. Mask: %d. TickContext: %d. Time: %d", *GetDebugName(), *FGlobalCueTypeTable::Get().GetTypeName(T::ID), int(TNetSimCueTraits<T>::SimTickMask()), (int32)Context.TickContext, Context.CurrentSimTime);
			}
		}
	}
...
struct TNetSimCueDispatcher : public FNetSimCueDispatcher
{
	...
	void NetRecvSavedCues(FArchive& Ar, const bool bSerializeFrameNumber, const int32 InLastRecvFrame, const int32 InLastRecvTime)
	{
		...
		for (int32 CueIdx=0; CueIdx < NumCues; ++CueIdx)
		{
			...
			{
				UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Discarding replicated NetSimCue %ls that is not intended for us. CueMask: %d. Our Mask: %d", *GetDebugName(), *SerializedCue.GetDebugName(), int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
				continue;
			}
			...
			if (SerializedCue.Frame != INDEX_NONE)
			{
				...
				{
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Discarding replicated NetSimCue %ls because it is older (%d) than confirmed frame (%d). CueMask: %d. Our Mask: %d", *GetDebugName(), *SerializedCue.GetDebugName(), SerializedCue.Frame, LastRecvFrame, int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
					continue;
				}
			}
			else if (SerializedCue.Time <= LastRecvMS)
			{
				UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Discarding replicated NetSimCue %ls because it is older (%d) than confirmed TimeMS (%d). CueMask: %d. Our Mask: %d", *GetDebugName(), *SerializedCue.GetDebugName(), SerializedCue.Frame, LastRecvFrame, int(SerializedCue.ReplicationTarget), int(RecvReplicationMask));
				continue;
			}
			...
			for (int32 ExistingIdx=0; ExistingIdx < StartingNum; ++ExistingIdx)
			{
				...
				if (SerializedCue.NetIdentical(ExistingCue))
				{
					...
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Discarding replicated NetSimCue %ls because we've already processed it. (Matched %ls)", *GetDebugName(), *SerializedCue.GetDebugName(), *ExistingCue.GetDebugName());
					bUniqueCue = false;
			...
			if (bUniqueCue)
			{
				...
				UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Received !NetIdentical Cue: %ls. (Num replicated cue sent this bunch: %d. LastRecvFrame: %d. Our Mask: %d).", *GetDebugName(), *SerializedCue.GetDebugName(), NumCues, LastRecvFrame, int(RecvReplicationMask));
			}
		}
	...
	void DispatchCueRecord(T& Handler, const int32 SimFrame, const int32 CurrentSimTime, const int32 FixedTimeStepMS)
	{
		...
		if (LastRecvFrame != INDEX_NONE)
		{
			...
			for (auto It = SavedCues.CreateIterator(); It; ++It)
			{
				...
				{
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Calling OnRollback for SavedCue NetSimCue %ls. Cue has not been matched but it <= LastRecvFrame %d.", *GetDebugName(), *SavedCue.GetDebugName(), LastRecvFrame);
		...
		if (RollbackFrame >= 0)
		{
			for (auto It = SavedCues.CreateIterator(); It; ++It)
			{
				...
				if (SavedCue.bPendingResimulateRollback)
				{
					...
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Calling OnRollback for SavedCue NetSimCue %ls. Cue was not matched during a resimulate.", *GetDebugName(), *SavedCue.GetDebugName());
					SavedCue.Callbacks.OnRollback.Broadcast();
					SavedCues.RemoveAt(It.GetIndex(), EAllowShrinking::No);
				}
			}
		...
		for (int32 SavedCueIdx = 0; SavedCueIdx < SavedCues.Num(); ++ SavedCueIdx)
		{
			...
			if (!SavedCue.bDispatched)
			{
				...
				if (bDispatchedCue)
				{
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls Dispatching NetSimCue %ls {Frame: %d, Time: %d}", *GetDebugName(), *SavedCue.GetDebugName(), SavedCue.Frame, SavedCue.Time);
					SavedCue.bDispatched = true;
					TCueDispatchTable<T>::Get().Dispatch(SavedCue, Handler, TimeSinceInvocationMS);
				}
				else
				{
					UE_LOGF(LogNetworkPredictionCues, Log, "%ls Withholding Cue %ls. %d/%d > %d/%d", *GetDebugName(), *SavedCue.GetDebugName(), SavedCue.Frame, SavedCue.Time, SimFrame, CurrentSimTime);
				}
			}
		}
		...
		{
			UE_LOGF(LogNetworkPredictionCues, Log, "Dispatching transient NetSimCue %ls.", *TransientCue.GetDebugName());
		...
		if (SavedCuePruneIdx >= 0)
		{
			...
			{
				UE_CLOGF(!SavedCues[i].bDispatched, LogNetworkPredictionCues, Warning, "Non-Dispatched Cue is about to be pruned! %ls. SavedCuePruneTimeMS: %d. SavedCuePruneFrame: %d", *SavedCues[i].GetDebugName(), SavedCuePruneTimeMS, SavedCuePruneFrame);
				UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Pruning Cue %ls. Prune: (%d/%d). Invoked: (%d/%d). Current Time: %d.", *GetDebugName(), *SavedCues[i].GetDebugName(), SavedCuePruneFrame, SavedCuePruneTimeMS,  SavedCues[i].Frame, SavedCues[i].Time, CurrentSimTime);
			}
#endif
	...
	void NotifyRollback(int32 InRollbackFrame)
	{
		...
		for (FSavedCue& SavedCue : SavedCues)
		{
			...
			if (SavedCue.bResimulates && SavedCue.Frame >= InRollbackFrame)
			{
				...
				UE_LOGF(LogNetworkPredictionCues, Log, "%ls. Marking %ls bPendingResimulateRollback. (RollbackFrame: %d/%d)", *GetDebugName(), *SavedCue.GetDebugName(), RollbackFrame, InRollbackFrame);
			}
		}
	}
