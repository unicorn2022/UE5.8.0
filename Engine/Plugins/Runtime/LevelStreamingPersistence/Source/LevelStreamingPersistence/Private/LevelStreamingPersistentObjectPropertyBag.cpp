...
const FProperty* FLevelStreamingPersistentObjectPropertyBag::CopyPropertyValueFromObject(const UObject* InObject, const FProperty* InObjectProperty)
{
	...
	{
		// Requires EditAnywhere, BlueprintReadWrite or some other explicit access specifier to adhere to PropertyAccessUtil rules
		UE_LOGF(LogLevelStreamingPersistence, Warning, "Failed to copy public property '%ls' from object '%ls'. Is the property Instance Editable?", *InObjectProperty->GetName(), *InObject->GetName());
		return nullptr;
	}
}
