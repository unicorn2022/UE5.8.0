...
// By default GDME is disabled as it causess a (begnin) race condition (in CreateMeshBatch > BuildUniformShaderParameters) when reading velocity data, as the LastFrameIndex is mutable and can be read/writen at the same time.
// This can happens when view context (e.g., main view + scene capture) are updated at the same time.
static int32 GHairStrands_UseParallelGDME = 0;
static FAutoConsoleVariableRef CVarHairStrands_UseParallelGDME(TEXT("r.HairStrands.UseParallelGDME"), GHairStrands_UseParallelGDME, TEXT("When enabled, FHairStrandsSceneProxy uses parallel GDME (using async task gathering) for collecting instances to render."));
...
#if RHI_RAYTRACING
void AllocateRaytracingResources(FHairGroupInstance* Instance, bool bNeedDynamicResources, FName OwnerName);
#endif
...
FHairGroupInstance::~FHairGroupInstance()
{
	...
	if (Strands.IsValid())
	{
		...
		InternalResourceRelease(Strands.LinearSweptSphereVertexFactory);
...
public:
		...
		, MaterialRelevance(Component->GetMaterialRelevance(GetScene().GetShaderPlatform()))
	{
		...
		bSupportsParallelGDME = GHairStrands_UseParallelGDME>0;
		...
		for (int32 GroupIt=0; GroupIt<GroupCount; GroupIt++)
		{
			...
			{
				if (GetSupportHairStrandsLinearSweptSpheres(ShaderPlatform) && IsHairStrandsLinearSweptSpheresEnabled())
				{
					if (HairInstance->Strands.LinearSweptSphereVertexFactory == nullptr)
					{
						HairInstance->Strands.LinearSweptSphereVertexFactory = new FLinearSweptSpheresHairStrandsVertexFactory(HairInstance, FeatureLevel, "FLinearSweptSphereStrandsHairSceneProxy");
					}
				}
				else 
				{
					if (HairInstance->Strands.VertexFactory == nullptr)
					{
	...
	virtual void CreateRenderThreadResources(FRHICommandListBase& RHICmdList) 
	{
		...
		// Register the data to the scene. AddHairStrands takes a raw pointer 
		// but internally registers it as TRefCountPtr which keeps the instance 
		// alive while it is registered in the scene.
		FSceneInterface& LocalScene = GetScene();
		for (TRefCountPtr<FHairGroupInstance>& Instance : HairGroupInstances)
		{
			if (Instance.IsValid() && (Instance->IsValid() || Instance->Strands.ClusterResource))
			{
	...
	virtual void DestroyRenderThreadResources() override
	{
		...
		// Unregister the data from the scene. RemoveHairStrands releases the 
		// scene's TRefCountPtr, which may destroy the instance if this was 
		// the last reference.
		FSceneInterface& LocalScene = GetScene();
		for (TRefCountPtr<FHairGroupInstance>& Instance : HairGroupInstances)
		{
	...
	virtual void GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector) override
	{
		...
		for (uint32 GroupIt = 0, GroupCount = HairGroupInstances.Num(); GroupIt < GroupCount; ++GroupIt)
		{
			...
			{
				const FRayTracingGeometryInitializer& RayTracingGeometryInitializer = RTGeometry->RayTracingGeometry.GetInitializer();

				for (const FRayTracingGeometrySegment& Segment : RayTracingGeometryInitializer.Segments)
				{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			if ((IsShadowCast(View) || IsShown(View)) && (VisibilityMap & (1 << ViewIndex)))
			{
				for (uint32 GroupIt = 0; GroupIt < GroupCount; ++GroupIt)
				{
					...
					// Render bounds
					if (ViewMode == EGroomViewMode::Bounds)
					{
						FEngineShowFlags LocalShowFlags = ViewFamily.EngineShowFlags;
						LocalShowFlags.Bounds = true;
						RenderBounds(Collector.GetPDI(ViewIndex), LocalShowFlags, GetBounds(), true);
					}
					else
					{
		...
		FMaterialRenderProxy* Debug_MaterialProxy) const
	{
		...
		else if (GeometryType == EHairGeometryType::Cards)
		{
			...
			check(VertexFactory);
			PrimitiveIdMode = Instance->Cards.LODs[IntLODIndex].GetVertexFactory()->GetPrimitiveIdMode(FeatureLevel);
		...
		{
			check(Instance->Strands.LinearSweptSphereVertexFactory != nullptr || Instance->Strands.VertexFactory != nullptr);
			VertexFactory = Instance->Strands.LinearSweptSphereVertexFactory != nullptr ? static_cast<const FVertexFactory*>(Instance->Strands.LinearSweptSphereVertexFactory) : static_cast<const FVertexFactory*>(Instance->Strands.VertexFactory);
			PrimitiveIdMode = Instance->Strands.LinearSweptSphereVertexFactory != nullptr ? Instance->Strands.LinearSweptSphereVertexFactory->GetPrimitiveIdMode(FeatureLevel) : Instance->Strands.VertexFactory->GetPrimitiveIdMode(FeatureLevel);
...
void UGroomComponent::SetForcedLOD(int32 CurrLODIndex)
{
	...
	// If not setting a valid forced LOD, just reset the state and let the auto system handle the rest	
	...
	if (GroomAsset && !GroomAsset->UsesNanite())
	{
...
void UGroomComponent::SetEnableSimulation(bool bInEnableSimulation)
{
	...
	{
		UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - To be able to enable/disable the simulation from component you must enable the overide settings boolean in the simulatin settings.", *GetPathName());
	}
}
...
FPrimitiveSceneProxy* UGroomComponent::CreateSceneProxy()
{
	...
	{
		UE_LOGF(LogHairStrands, Verbose, "Skipping CreateSceneProxy for UGroomComponent %ls (UGroomComponent PSOs are still compiling)", *GetFullName());
		return nullptr;
	}
	...
	for (const TRefCountPtr<FHairGroupInstance>& Instance : HairGroupInstances)
	{
		bIsValid |= Instance.IsValid() && Instance->IsValid();
	}
	if (!bIsValid)
	{
		return nullptr;
	...
	if (FGroomAssetRenderData* LocalRenderData = GroomAsset->GetRenderData())
	{
		if (LocalRenderData->GetMaterialAudit())
		{
			return ::new Nanite::FGroomSceneProxy(*LocalRenderData->GetMaterialAudit(), this, GroomAsset, LocalRenderData);
		}
		else
		{
			return nullptr;
...
void UGroomComponent::CheckHairStrandsUsage(int32 ElementIndex) const
{
	...
	{		
		UE_CLOGF(OverrideMaterial && !bValidUsage, LogHairStrands, Warning, "Material with missing usage flag was applied to hair strands %ls", *OverrideMaterial->GetPathName());
	}
}
	...
	const USceneComponent* Component)
{
	...
	if (GeometryCacheComponent && !bHasValidSectionCount)
	{
		...
		UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - Groom is bound to a GeometryCache which has too many sections (%d), which is higher than the maximum supported for hair binding (%d). The groom binding will be disbled on this component.", *Name, GeometryCacheComponent->GetNumMaterials(), GetHairStrandsMaxSectionCount());
	}
	...
	const USceneComponent* Component)
{
	...
	if (SkeletalMeshComponent && !bHasValidSectionCount)
	{
		...
		UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - Groom is bound to a skeletal mesh which has too many sections (%d), which is higher than the maximum supported for hair binding (%d). The groom binding will be disbled on this component.", *Name, SkeletalMeshComponent->GetNumMaterials(), GetHairStrandsMaxSectionCount());
	}
	...
	if (BindingAsset)
	{
		...
		for (int32 GroupIt = 0, GroupCount = GroomAsset->GetHairGroupsPlatformData().Num(); GroupIt < GroupCount; ++GroupIt)
		{
			for (uint32 LODIt = 0, LODCount = GroomAsset->GetLODCount(); LODIt < LODCount; ++LODIt)
			{
				...
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] Groom asset (Group:%d/%d) is set to use Skinning at LOD %d/%d while the parent skel. mesh does not support skin. cache at this LOD - Groom:%ls - Skel.Mesh:%ls",
						GroupIt,
						GroupCount,
						LODIt,
						LODCount,
...
void UGroomComponent::InitResources(bool bIsBindingReloading)
{
	...
	if (!GroomAsset || GroomAsset->GetNumHairGroups() == 0 || !GroomAsset->IsValid())
	{
		return;
	}
	...
	for (int32 GroupIt = 0, GroupCount = GroomAsset->GetHairGroupsPlatformData().Num(); GroupIt < GroupCount; ++GroupIt)
	{
		...
		if (bNeedStrandsData && GroupData.Strands.HasValidData())
		{
			...
			HairGroupInstance->Strands.RenRaytracingResourceShared = GroomAsset->GetStrandsRaytracingResourcesPtr(GroupIt);
			if (IsHairRayTracingEnabled() && bVisibleInRayTracing)
			{
				// Visible to ray tracing if the user has opted in
				// If the project has path tracing, resources for pathtracing will be lazyly allocated on demand
				...
				{
					AllocateRaytracingResources(HairGroupInstance, bNeedDynamicResources, OwnerName);
				}
			}
			#endif
...
void UGroomComponent::ReleaseResources()
{
	...
	// Insure the ticking of the Groom component always happens after the skeletalMeshComponent.
	if (RegisteredMeshComponent && !RegisteredMeshComponent->IsBeingDestroyed())
	{
...
void UGroomComponent::PostLoad()
{
	...
	{
		UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - The groom instance tries to use a guide-cache, but 'Enable Guide-Cache Support' option is not enabled on the groom asset. Because of this, the Guide-Cache won't be used.", *GetPathName());
	}
}
...
void UGroomComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	bool bHidden = !IsVisible() || (Owner && Owner->IsHidden());

#if WITH_EDITOR
	bHidden |= (Owner && Owner->IsHiddenEd());
#endif

	if (bHidden && !bWasHiddenLastTick)
	{
		// Visible -> hidden: pause the Niagara sim so it doesn't drift while off-screen.
		for (UNiagaraComponent* NC : NiagaraComponents)
		{
			if (NC) { NC->DeactivateImmediate(); }
		}
		bWasHiddenLastTick = true;
	}
	else if (!bHidden && bWasHiddenLastTick)
	{
		// Hidden -> visible: re-activate with reset so we start from rest pose.
		for (UNiagaraComponent* NC : NiagaraComponents)
		{
			if (NC) { NC->Activate(/*bReset=*/true); }
		}
		bResetSimulation   = true;
		bInitSimulation    = true;
		bWasHiddenLastTick = false;
	}
	...
	for (uint32 MaterialIt = 0, MaterialCount = GetNumMaterials(); MaterialIt < MaterialCount; ++MaterialIt)
	{
		...
		switch (Result)
		{
			case EHairMaterialCompatibility::Invalid_UsedWithHairStrands:
			{
				...
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - Groom's material needs to enable the UseHairStrands option. Groom's material will be replaced with default hair strands shader in editor.", *Name);
				}
			} break;
			case EHairMaterialCompatibility::Invalid_ShadingModel:
			{
				...
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - Groom's material needs to have Hair shading model. Groom's material will be replaced with default hair strands shader in editor.", *Name);
				}
			}break;
			case EHairMaterialCompatibility::Invalid_BlendMode:
			{
				...
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - Groom's material needs to have Opaque blend mode. Groom's material will be replaced with default hair strands shader in editor.", *Name);
				}
			}break;
			...
			case EHairMaterialCompatibility::Invalid_IsNull:
			{
				...
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] %ls - Groom's material is not set and will fallback on default hair strands shader in editor.", *Name);
				}
			}break;
			#endif
		}
...
{
	HandlePlatformPreviewChanged(GShaderPlatformForFeatureLevel[InFeatureLevel]);
}
...
void UGroomComponent::HandlePlatformPreviewChanged(EShaderPlatform InShaderPlatform)
{
	if (BindingAsset)
	{
		BindingAsset->ChangeShaderPlatform(InShaderPlatform);
	}
	if (GroomAsset)
	{
		GroomAsset->ChangeShaderPlatform(InShaderPlatform);
	}
...
void DumpLoadedGroomComponent(IConsoleVariable* InCVarPakTesterEnabled)
{
	...
	UE_LOGF(LogHairStrands, Log, "[Groom] ##### UGroomComponent #####");
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	UE_LOGF(LogHairStrands, Log, "--  No.  - LOD -          GPU (     Common|     Guides|    Strands|      Cards|     Meshes) - Asset Name ");
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	FString OutputData;
	for (TObjectIterator<UGroomComponent> ComponentIt; ComponentIt; ++ComponentIt)
	{
		if (ComponentIt)
		{
			...
			for (uint32 GroupIt = 0; GroupIt < GroupCount; ++GroupIt)
			{
				...
				if (bDetails)
				{
//					UE_LOGF(LogHairStrands, Log, "--  No.  - LOD -    GPU Total (    Common|     Guides|    Strands|      Cards|     Meshes) - Asset Name ");
//					UE_LOGF(LogHairStrands, Log, "-- 00/00 -  00 -  0000.0Mb (0000.0Mb |0000.0Mb|0000.0Mb |0000.0Mb) -  0000.0Mb (0000.0Mb|0000.0Mb|0000.0Mb|0000.0Mb) - AssetName");
					UE_LOGF(LogHairStrands, Log, "-- %2d/%2d -  %2d -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb) - %ls", 
						GroupIt,
						GroupCount,
	...
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	UE_LOGF(LogHairStrands, Log, "-- C:%3d|G:%3d -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb)",
		Total_Component,
		Total_Group,
