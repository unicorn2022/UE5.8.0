			...
			if (bIsLocked)
			{
				...
				if (IsInGameThread())
				{
					UE_LOGF(
						LogHairStrands,
						Verbose,
						"Accessing property %ls of the GroomBindingAsset while it is still being built asynchronously will force it to be compiled before continuing. "
...
void UGroomBindingAsset::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsCooking())
	{
		...
		{
			UE_LOGF(LogHairStrands, Error, "[Groom] The binding asset (%ls) has missing cooked platform data.", *GetName());
		}
	}
#else
...
void UGroomBindingAsset::ReleaseResource(bool bResetLoadedSize)
{
	...
	while (RemoveHairGroupResourcesToDelete(ResourceToDelete))
	{
		...
		TArray<FHairStrandsRestRootResource*> InCardsRootResources = ResourceToDelete.CardsRootResources;
		ENQUEUE_RENDER_COMMAND(ReleaseHairStrandsResourceCommand)(UE::RenderCommandPipe::Groom,
			[InSimRootResources, InRenRootResources, InCardsRootResources](FRHICommandList& RHICmdList)
		{
			if (InSimRootResources)
			{
				InSimRootResources->ReleaseResource();
			...
			for (FHairStrandsRestRootResource* InCardRootResource : InCardsRootResources)
			{
				if (InCardRootResource)
				{
					InCardRootResource->ReleaseResource();
					delete InCardRootResource;
				}
			}
		});
	}
...
bool UGroomBindingAsset::IsCompatible(const USkeletalMesh* InSkeletalMesh, const UGroomBindingAsset* InBinding, bool bIssueWarning)
{
	if (InBinding && InSkeletalMesh && IsHairStrandsBindingEnable())
	{
		if (InBinding->GetGroomBindingType() != EGroomBindingMeshType::SkeletalMesh)
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The binding asset (%ls) cannot be bound to a SkeletalMesh because it is not the correct binding type.", *InBinding->GetName());
			}
			return false;
		}
		...
		if (!InBinding->GetTargetSkeletalMesh())
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The binding asset (%ls) does not have a target skeletal mesh. Falling back onto non-binding version.", *InBinding->GetName());
			}
			return false;
		}
		...
		if (!InSkeletalMesh->GetName().Contains(InBinding->TargetSkeletalMesh->GetName()))
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The Groom binding (%ls) does not reference the same skeletal asset (BindingAsset's skeletal:%ls vs. Attached skeletal:%ls). The binding asset will not be used.",
					*InBinding->GetName(),
		...
		for (const FHairGroupResource& Resource : InBinding->GetHairGroupResources())
		{
			if (Resource.SimRootResources && InSkeletalMesh->GetLODNum() != Resource.SimRootResources->GetLODCount())
			{
				if (bIssueWarning)
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] The Groom binding (%ls) does not have the same have LOD count (LOD sim:%d) than the skeletal mesh (%ls, LOD:%d). The binding asset will not be used.",
						*InBinding->GetName(),
			...
			if (Resource.RenRootResources && InSkeletalMesh->GetLODNum() != Resource.RenRootResources->GetLODCount() && IsHairStrandsEnabled(EHairStrandsShaderType::Strands))
			{
				if (bIssueWarning)
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] The Groom binding (%ls) does not have the same have LOD count (LOD render:%d) than the skeletal mesh (%ls, LOD:%d). The binding asset will not be used.",
						*InBinding->GetName(),
...
bool UGroomBindingAsset::IsCompatible(const UGeometryCache* InGeometryCache, const UGroomBindingAsset* InBinding, bool bIssueWarning)
{
	if (InBinding && InGeometryCache && IsHairStrandsBindingEnable())
	{
		if (InBinding->GetGroomBindingType() != EGroomBindingMeshType::GeometryCache)
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The binding asset (%ls) cannot be bound to a GeometryCache because it is not the correct binding type.", *InBinding->GetName());
			}
			return false;
		}
		...
		if (!InBinding->GetTargetGeometryCache())
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The binding asset (%ls) does not have a target GeometryCache.", *InBinding->GetName());
			}
			return false;
		}
		...
		if (MeshesData.Num() > 1)
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] Cannot be bound to a non-flattened GeometryCache. Re-import %ls with 'Flatten Tracks' enabled.", *InGeometryCache->GetName());
			}
			return false;
		}
		else if (MeshesData.Num() == 0)
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] %ls is not a valid GeometryCache to bind to.", *InGeometryCache->GetName());
			}
			return false;
		}
	}
...
bool UGroomBindingAsset::IsCompatible(const UGroomAsset* InGroom, const UGroomBindingAsset* InBinding, bool bIssueWarning)
{
	if (InBinding && InGroom && IsHairStrandsBindingEnable())
	{
		...
		{
			UE_CLOGF(bIssueWarning, LogHairStrands, Warning, "[Groom] The binding asset (%ls) does not reference a groom. Falling back onto non-binding version.", *InBinding->GetName());
			return false;
		}

		if (InGroom->GetPrimaryAssetId() != InBinding->GetGroom()->GetPrimaryAssetId())
		{
			UE_LOGF(LogHairStrands, Warning, "[Groom] The Groom binding (%ls) does not reference the same groom asset (BindingAsset's groom:%ls vs. Groom:%ls). The binding asset will not be used.", 
				*InBinding->GetName(),
		...
		{
			UE_CLOGF(bIssueWarning, LogHairStrands, Warning, "[Groom] The GroomBinding asset (%ls) does not contains the same number of groups (%d vs. %d) than the groom (%ls). The binding asset will not be used.",
				*InBinding->GetName(),
				GroupCount,
		...
		for (uint32 GroupIt = 0; GroupIt < GroupCount; ++GroupIt)
		{
			// Guides
			{
				...
				{
					UE_CLOGF(bIssueWarning, LogHairStrands, Warning, "[Groom] The GroomBinding asset (%ls) does not contains the same guides in group %d (%d vs. %d) than the groom (%ls). The binding asset will not be used.",
						*InBinding->GetName(),
						GroupIt,
						GroomCount,
						BindingCount,
			...
			if (bNeedStrandsRoot)
			{
				...
				if (GroomCount != 0 && GroomCount != BindingCount)
				{
					UE_CLOGF(bIssueWarning, LogHairStrands, Warning, "[Groom] The GroomBinding asset (%ls) does not contains the same curves in group %d (%d vs. %d) than the groom (%ls). The binding asset will not be used.",
						*InBinding->GetName(),
						GroupIt,
						GroomCount,
						BindingCount,
...
bool UGroomBindingAsset::IsBindingAssetValid(const UGroomBindingAsset* InBinding, bool bIsBindingReloading, bool bIssueWarning)
{
	if (InBinding && IsHairStrandsBindingEnable())
	{
		if (!InBinding->IsValid())
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The binding asset (%ls) is invalid. It failed to load or build. Falling back onto non-binding version.", *InBinding->GetName());
			}
			return false;
		}
		...
		if (!InBinding->GetGroom())
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The binding asset (%ls) does not reference a groom. Falling back onto non-binding version.", *InBinding->GetName());
			}
			return false;
		}
		if (InBinding->GetGroupInfos().Num() == 0)
		{
			if (bIssueWarning)
			{
				UE_LOGF(LogHairStrands, Warning, "[Groom] The GroomBinding asset (Groom:%ls) does not contain any groups. It is invalid and can't be assigned. The binding asset will not be used.", *InBinding->GetGroom()->GetName());
			}
			return false;
		}
		...
		for (const FGoomBindingGroupInfo& Info : InBinding->GetGroupInfos())
		{
			if (Info.SimRootCount == 0)
			{
				if (bIssueWarning)
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] The GroomBinding asset (Groom:%ls) has group with 0 guides. It is invalid and can't be assigned. The binding asset will not be used.", *InBinding->GetGroom()->GetName());
				}
				return false;
			}
			...
			if (bNeedStrandsRoot && Info.RenRootCount == 0 && IsHairStrandsEnabled(EHairStrandsShaderType::Strands))
			{
				if (bIssueWarning)
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] The GroomBinding asset (Groom:%ls) has group with 0 curves. It is invalid and can't be assigned. The binding asset will not be used.", *InBinding->GetGroom()->GetName());
				}
				return false;
			}
...
{
	{
		...
		if (Data)
		{
			UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[GroomBinding/DDC] Found (GroomBinding:%ls TargetPlatform:%ls Async:%ls).", *In->GetName(), *TargetPlatform->PlatformName(), IsInGameThread() ? TEXT("No") : TEXT("Yes"));
		...
		if (!bHasDataInCache)
		{
			UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[GroomBinding/DDC] Not found (GroomBinding:%ls TargetPlatform:%ls Async:%ls).", *In->GetName(), *TargetPlatform->PlatformName(), IsInGameThread() ? TEXT("No") : TEXT("Yes"));
	...
	if (TargetPlatformData == nullptr && GetGroom() != nullptr)
	{
		auto BuildTargetPlatformData = [TargetPlatform](const TArray<FString>& InGroupDerivedDataKeys_Query, const TArray<FString>& InGroupDerivedDataKeys_Cache, UGroomBindingAsset* BindingAsset, int32 InSourceMeshLOD, int32 InTargetMeshMinLOD)
		{
			...
			for (uint32 GroupIndex = 0; GroupIndex < GroupCount; ++GroupIndex)
			{
				...
				if (!bGroupValid)
				{
					UE_LOGF(LogHairStrands, Error, "[Groom] The binding asset (%ls) couldn't be built. This binding asset won't be used.", *BindingAsset->GetName());
				}
			}
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	return GET_MEMBER_NAME_CHECKED(UGroomBindingAsset, HairGroupResources);
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
}
...
void UGroomBindingAsset::ChangeShaderPlatform(EShaderPlatform InShaderPlatform)
{
