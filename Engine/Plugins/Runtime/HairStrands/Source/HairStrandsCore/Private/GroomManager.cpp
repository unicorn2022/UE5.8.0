...
struct FHairGeometryCache
{
		...
		FRDGBufferSRVRef& OutTangent)
	{
		...
		if (UniqueSections.Num() == 0)
		{
			// Binding data is valid but has no section indices
			static bool bWarningIssued = false;
			if (!bWarningIssued)
			{
				UE_LOGF(LogHairStrands, Warning, "GetOrAllocateCachedGeometry: RootResource LOD %d has no UniqueSectionIndices. Skipping.", InLODIndex);
				bWarningIssued = true;
			}
			return;
		}
		...
		for (uint32 SectionIndex : UniqueSections)
		{
			if (!Data.RequestedSectionBits[SectionIndex])
			{
				...
				const FSkeletalMeshObject::FBoneBufferSRV CurrentBones = InMeshObject->GetSectionBoneBuffer(InLODIndex, SectionIndex, false);
				const FSkeletalMeshObject::FBoneBufferSRV PrevBones    = InMeshObject->GetSectionBoneBuffer(InLODIndex, SectionIndex, true);
				UpdateSection.BoneBuffer 			  = CurrentBones.SRV;
				UpdateSection.BonePrevBuffer 		  = PrevBones.SRV;
				UpdateSection.BoneTransformOffset	  = CurrentBones.Offset;
				UpdateSection.PrevBoneTransformOffset = PrevBones.Offset;
				...
				if (UpdateSection.BoneBuffer == nullptr)
				{
					...
					UpdateSection.BoneTransformOffset = 0;
					UpdateSection.PrevBoneTransformOffset = 0;
				}
				check(UpdateSection.BoneBuffer);

				UE_CLOGF(!UpdateSection.BoneBuffer, LogHairStrands, Error,
					"Failed to get bone buffer from mesh object for hair skin cache. MeshObject: %ls, LOD: %d, Section: %d, IsGPUSkinMesh %d, IsSectionValid %d",
	...
	uint32& OutTotalSectionCount)
{
	...
	check(RootResource);
	if (!RootResource->IsDataValid(LODIndex))
	{
		return;
	}
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	for (FHairStrandsInstance* AbstractInstance : Instances)
	{
		...
		// 2. Strands/Cards
		{
			...
			// Cards 
			// This is only needed for card geometry. Mesh geometry only uses RBF deformation, which are initialized by the guide pass.
		...
		for (FInstanceData& InstanceData : InstanceDatas)
		{
			if (InstanceData.ActiveGroomCacheType != EGroomCacheType::Strands && InstanceData.bNeedDeformation)
			{
				...
				AddHairStrandsInterpolationPass(
					...
					InstanceData.Instance->Strands.RestResource,
					InstanceData.Instance->Guides.RestResource,
					bValidGuide,
	...
	if (bHasAnyRaytracingInstances)
	{
		...
		for (FInstanceData& InstanceData : InstanceDatas)
		{
			if (InstanceData.bNeedRaytracing)
			{
				...
				{
					// The entire RT buffer needs to be updated for ensuring invalid segments marked. 
					// When building the RT geometry, the entire buffer is processed, and having partially updated 
					// buffer would cause the groom to have stale curve/points
					const uint32 ActivePointCount = InstanceData.ActivePointCount;
					const uint32 TotalPointCount = InstanceData.Instance->Strands.RestResource->GetPointCount();
					AddGenerateRaytracingGeometryPass(
						...
						ActivePointCount,
						TotalPointCount,
						InstanceData.Instance->Strands.RenRaytracingResource->RayTracingPrimitiveType,
						ProceduralSplits,
						HairRadiusRT,
						HairRootScaleRT,
						HairTipScaleRT,
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	if (GSupportsEfficientAsyncCompute && GRHIGlobals.RayTracing.SupportsAsyncRayTraceDispatch && IsHairRayTracingEnabled())
	{
		...
		if (InstanceData.bNeedDeformation)
		{
			...
			if (InstanceData.CardsSimulationType == EHairCardsSimulationType::Guide)
			{
				...
				AddHairStrandsInterpolationPass(
					...
					LOD.Guides.RestResource,
					InstanceData.Instance->Guides.RestResource,
					InstanceData.bValidGuide,
...
{
	check(Instance->Strands.RenRaytracingResource == nullptr);
	...
	{
		if (Instance->Strands.RenRaytracingResourceShared)
		{
			// (Lazy) Allocate static raytracing resources (owned by the grooom asset)
			if (*Instance->Strands.RenRaytracingResourceShared == nullptr)
			{
				FName SharedOwnerName(Instance->Debug.GroomAssetName);
				FHairResourceName ResourceName(SharedOwnerName, Instance->Debug.GroupIndex);
				*Instance->Strands.RenRaytracingResourceShared = new FHairStrandsRaytracingResource(Instance->Strands.GetData(), ResourceName, SharedOwnerName);
			}
			Instance->Strands.RenRaytracingResourceOwned = false;
			Instance->Strands.RenRaytracingResource = *Instance->Strands.RenRaytracingResourceShared;
			check(Instance->Strands.RenRaytracingResource);
		}
	}
}
...
{
	const bool bNeedDynamicResources = Instance->Strands.DeformedResource != nullptr;
	if (IsHairRayTracingEnabled() && !Instance->Strands.RenRaytracingResource )
	{
		...
		AllocateRaytracingResources(Instance, bNeedDynamicResources, OwnerName);
	}
...
static FVector2f ComputeProjectedScreenPos(const FVector& InWorldPos, const FSceneView& View)
{
	...
	const FMatrix& ViewToProj = View.ViewMatrices.GetViewToClip();
	...
	bool bHasPathTracingView)
{
	...
	if (GeometryType == EHairGeometryType::NoneGeometry && !bLODNeedsGuides)
	{
		...
		// Return true to avoid further retry. While the selected LOD is not renderable, it is 'valid' as 'hidden'
		return true;
	}
	...
	if (Instance->Guides.IsValid() && bLODNeedsGuides)
	{
		...
		{ 
			...
			if (Instance->Guides.RestResource && Instance->Guides.RestResource->bIsInitialized)
			{
	...
	else if (GeometryType == EHairGeometryType::Strands)
	{
		...
		if (Instance->Strands.LinearSweptSphereVertexFactory != nullptr)
		{
			Instance->Strands.LinearSweptSphereVertexFactory->InitResources(GraphBuilder.RHICmdList);
		}
		else
		{
			check(Instance->Strands.VertexFactory != nullptr);
	...
	FHairStrandsBookmarkParameters& Parameters)
{
	...
	else if (Bookmark == EHairStrandsBookmark::ProcessDebug)
	{
		...
		if (IsInstanceFrustumCullingEnable())
		{
			...
			DebugInstances.Sort([](const TRefCountPtr<FHairStrandsInstance>& A, const TRefCountPtr<FHairStrandsInstance>& B)
			{
