...
FHairStrandsDatas::FHash FHairStrandsCommonDatas::GetHash() const
{
	FHairStrandsDatas::FHash Out;
	Out.CurveCountHash 	= CityHash64WithSeed((const char*)StrandsCurves.CurvesCount.GetData(), StrandsCurves.CurvesCount.Num() * sizeof(uint16), Out.CurveCountHash);
	Out.CurveOffsetHash = CityHash64WithSeed((const char*)StrandsCurves.CurvesOffset.GetData(), StrandsCurves.CurvesOffset.Num() * sizeof(uint32), Out.CurveOffsetHash);
	Out.PositionHash 	= CityHash64WithSeed((const char*)StrandsPoints.PointsPosition.GetData(), StrandsPoints.PointsPosition.Num() * sizeof(FVector3f), Out.PositionHash);
	return Out;
}