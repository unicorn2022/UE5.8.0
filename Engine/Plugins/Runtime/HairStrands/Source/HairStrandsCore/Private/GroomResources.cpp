...
void FHairCardsDeformedResource::InternalAllocate(FRDGBuilder& GraphBuilder)
{
	...
	// Transition to SRV in case the deformation pass does not run in this graph (e.g. rendered only in shadows or captures).
...
void FHairMeshesDeformedResource::InternalAllocate(FRDGBuilder& GraphBuilder)
{
	...
	// Transition to SRV in case the mesh RBF deformation pass does not run in this graph (e.g. lazy allocation in a capture render).
	GraphBuilder.UseExternalAccessMode(Register(GraphBuilder, DeformedPositionBuffer[0], ERDGImportedBufferFlags::CreateSRV).Buffer, ERHIAccess::SRVMask, ERHIPipeline::All);
	GraphBuilder.UseExternalAccessMode(Register(GraphBuilder, DeformedPositionBuffer[1], ERDGImportedBufferFlags::CreateSRV).Buffer, ERHIAccess::SRVMask);
}
...
static void ConvertPointToRaytracingVertexCount(EHairStrandsRayTracingPrimitiveType RayTracingPrimitiveType, uint32 InPointCount, uint32& OutVertexCount, uint32& OutIndexCount)
{
	if (RayTracingPrimitiveType == EHairStrandsRayTracingPrimitiveType::LinearSweptSphere)
	{
		// 1 vertex at the beginning to indicate invalid hair strand
		OutVertexCount = InPointCount + 1;
		OutIndexCount  = InPointCount * 2;
	}
	...
	FHairCommonResource(EHairStrandsAllocationType::Deferred, ResourceName, InOwnerName), bOwnBuffers(true)
{
	...
	RayTracingPrimitiveType = EHairStrandsRayTracingPrimitiveType::Triangle;
	if (GetSupportHairStrandsLinearSweptSpheres(GMaxRHIShaderPlatform) && IsHairStrandsLinearSweptSpheresEnabled())
	{
		RayTracingPrimitiveType = EHairStrandsRayTracingPrimitiveType::LinearSweptSphere;
	}
	else if (GetSupportHairStrandsProceduralPrimitive(GMaxRHIShaderPlatform))
	{
		RayTracingPrimitiveType = EHairStrandsRayTracingPrimitiveType::Procedural;
	}
}
...
void FHairStrandsRaytracingResource::InternalAllocate(FRDGBuilder& GraphBuilder, uint32 InCurveCount, uint32 InPointCount)
{
	if (bOwnBuffers)
	{
		ConvertPointToRaytracingVertexCount(RayTracingPrimitiveType, InPointCount, MaxVertexCount, MaxIndexCount);
