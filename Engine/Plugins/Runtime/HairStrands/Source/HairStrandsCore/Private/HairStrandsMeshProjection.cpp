...
class FSkinUpdateCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, BoneTransformOffset)
		SHADER_PARAMETER(uint32, PrevBoneTransformOffset)
		SHADER_PARAMETER_SRV(Buffer<uint>, WeightLookup)
		SHADER_PARAMETER_COMPRESSED_BONE_TRANSFORM_SRV(BoneMatrices)
		SHADER_PARAMETER_COMPRESSED_BONE_TRANSFORM_SRV(PrevBoneMatrices)
	...
	FRDGBufferRef OutDeformedTangentBuffer)
{
	...
	// Set dummy tangent if not available and not used by the shaders
	if (!Parameters->bWriteTangent && Parameters->RestTangents == nullptr)
	{
		Parameters->RestTangents = Parameters->RestPositions;
	}
	...
	GraphBuilder.AddPass(
		...
		[ParametersMetadata, Parameters, ComputeShader, Sections, bPrevPosition](FRDGAsyncTask, FRHIComputeCommandList& RHICmdList)
		{
			for (const FSkinUpdateSection& Section : Sections)
			{
				...
				Parameters->BoneTransformOffset		= Section.BoneTransformOffset;
				check(Parameters->BoneMatrices);
				if (bPrevPosition)
				{
					Parameters->PrevBoneMatrices = Section.BonePrevBuffer;
					Parameters->PrevBoneTransformOffset = Section.PrevBoneTransformOffset;
	...
	FHairStrandsDeformedRootResource* DeformedRootResources)
{
	...
	// Take buffers out of external access mode so the copy + dispatch passes below can manage transitions.
	GraphBuilder.UseInternalAccessMode({ DeformedPositionBuffer_Curr.Buffer, DeformedPositionBuffer_Prev.Buffer });
	...
	// Return buffers to external access mode so subsequent non-RDG vertex-factory binds (raw SRVs in static UBs) see them in SRV state.
	GraphBuilder.UseExternalAccessMode({ DeformedPositionBuffer_Curr.Buffer, DeformedPositionBuffer_Prev.Buffer }, ERHIAccess::SRVMask);
}
	...
	FHairStrandsDeformedRootResource* DeformedResources)
{
	...
	const uint32 UniqueSectionIndicesCount = RestLODData.GetBulkData().Header.UniqueSectionIndices.Num();
	const uint32 EffectiveSectionCount = MeshData.Sections.Num();
	if (EffectiveSectionCount == 0 || RestLODData.SampleCount == 0 || UniqueSectionIndicesCount == 0)
	{
		return;
	}
	...
	TArray<FSectionData> SectionDatas;
	SectionDatas.Init(FSectionData(), FMath::Max(uint32(MeshData.Sections.Num()), TotalSectionCount));
