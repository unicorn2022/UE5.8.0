...
static int32 GVarHairStrands_BindingDataValidation = 0;
static FAutoConsoleVariableRef CVarVarHairStrands_BindingDataValidation(TEXT("r.HairStrands.Strands.BindingDataValidation"), GVarHairStrands_BindingDataValidation, TEXT("Add binding data validation at runtime (for debugging purpose)"));
...
bool GetHairStrandsBindingDataValidation()
{
	...
	return GVarHairStrands_BindingDataValidation > 0;
	#else
	return false;
	#endif
}
...
public:
	...
	static bool DoesPlatformSupportWaveOps(EShaderPlatform InPlatform, uint32 InPointPerCurve)
	{
	...
	// Runtime check for platforms that have varying wave size support depending on GPU.
	static bool DoesSupportWaveOps(EShaderPlatform InPlatform, uint32 InPointPerCurve)
	{
		return DoesPlatformSupportWaveOps(InPlatform, InPointPerCurve) && GRHIMinimumWaveSize <= 32 && GRHIMaximumWaveSize >= 32;
	}

	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters) 
	{ 
		FPermutationDomain PermutationVector(Parameters.PermutationId);
		if (PermutationVector.Get<FWaveOps>() && !DoesPlatformSupportWaveOps(Parameters.Platform, PermutationVector.Get<FPointPerCurve>()))
		{
			return false;
		}
	...
	{ 
		FPermutationDomain PermutationVector(Parameters.PermutationId);
		if (PermutationVector.Get<FWaveOps>() && !DoesSupportWaveOps(Parameters.Platform, PermutationVector.Get<FPointPerCurve>()))
		{
...
void AddHairStrandsInterpolationPass(
	...
	const FHairStrandsRestResource* RenRestResource,
	const FHairStrandsRestResource* SimRestResource,
	const bool bSimDataValid,
	...
	const FHairStrandsLODDeformedRootResource::EFrameType DeformedFrame)
{
	...
	const bool bSupportDynamicMesh_Ren = 
	...
	const bool bSupportDynamicMesh_Sim =
		SimRestRootResources &&
		SimDeformedRootResources && 
		SimRestRootResources->GetRootCount() > 0 && 
		MeshLODIndex >= 0 &&
		SimRestRootResources->IsValid(MeshLODIndex) &&
		SimDeformedRootResources->IsValid(MeshLODIndex);
	...
	bool bSupportGlobalInterpolation = false;
	if (bSupportDynamicMesh_Sim && (Instance->Guides.bIsSimulationEnable || Instance->Guides.bHasGlobalInterpolation || Instance->Guides.bIsDeformationEnable || Instance->Guides.bIsSimulationCacheEnable))
	{
		...
		if (bSimDataValid && GetHairStrandsBindingDataValidation())
		{
			check(SimRestResource->BulkData.Header.Hash == Sim_RestLODDatas.GetBulkData().Header.Hash);
		}
	}

	// Strands
	if (bSupportDynamicMesh_Ren)
	{
		...
		{
			check(RenRestResource->BulkData.Header.Hash == Ren_RestLODDatas.GetBulkData().Header.Hash);
		}
	}
	...
	// Enable support for dynamic mesh (skinning) only if roots data are valids
	// * If there is no need of Sim data, only rely on Ren resources
	// * If there is need of Sim data, rely on both Sim and Ren resources
	const bool bSupportDynamicMesh = bHasLocalDeformation ? (bSupportDynamicMesh_Ren && bSupportDynamicMesh_Sim) : bSupportDynamicMesh_Ren;
	...
	const bool bWaveOps = FHairInterpolationCS::DoesSupportWaveOps(InPlatform, DispatchInfo.PointPerCurve);
...
class FHairRaytracingGeometryCS : public FGlobalShader
{
	...
	class FCulling : SHADER_PERMUTATION_BOOL("PERMUTATION_CULLING");
	class FPrimitiveType : SHADER_PERMUTATION_ENUM_CLASS("PRIMITIVE_TYPE", EHairStrandsRayTracingPrimitiveType);
	using FPermutationDomain = TShaderPermutationDomain<FCulling, FPrimitiveType>;
...
public:
	...
	{ 
		...
		const EHairStrandsRayTracingPrimitiveType PrimitiveType = PermutationVector.Get<FPrimitiveType>();

		if ((PrimitiveType == EHairStrandsRayTracingPrimitiveType::LinearSweptSphere) && !GetSupportHairStrandsLinearSweptSpheres(Parameters.Platform))
		{
			return false;
		}

		if ((PrimitiveType == EHairStrandsRayTracingPrimitiveType::Procedural) && !GetSupportHairStrandsProceduralPrimitive(Parameters.Platform))
		{
			return false;
		}

		if ((PrimitiveType == EHairStrandsRayTracingPrimitiveType::Triangle) && GetSupportHairStrandsProceduralPrimitive(Parameters.Platform))
		{
			return false;
		}
...
void AddGenerateRaytracingGeometryPass(
	...
	const FShaderPrintData* ShaderPrintData,
	uint32 InstanceRegisteredIndex,
	uint32 ActivePointCount,
	uint32 TotalPointCount,
	EHairStrandsRayTracingPrimitiveType RayTracingPrimitiveType,
	...
	const FRDGBufferUAVRef& OutIndexBuffer)
{
	...
	Parameters->PointCount = ActivePointCount;
	...
	PermutationVector.Set<FHairRaytracingGeometryCS::FPrimitiveType>(RayTracingPrimitiveType);
	TShaderMapRef<FHairRaytracingGeometryCS> ComputeShader(ShaderMap, PermutationVector);

	const FIntVector DispatchCount(FMath::DivideAndRoundUp(TotalPointCount, FHairRaytracingGeometryCS::GetGroupSize()), 1, 1);
	FComputeShaderUtils::AddPass(
		GraphBuilder,
		RDG_EVENT_NAME("HairStrands::GenerateRaytracingGeometry(bCulling:%d, PrimitiveType:%d)", bCullingEnable, (int32)RayTracingPrimitiveType),
		ComputeShader,
		Parameters,
		DispatchCount);
}
...
static void UpdateHairAccelerationStructure(FRHICommandList& RHICmdList, FRayTracingGeometry* RayTracingGeometry, EAccelerationStructureBuildMode InMode)
{
	...
	Params.Segments = RayTracingGeometry->GetInitializer().Segments;
	...
	int ProceduralSplits)
{
	...
	if (RayTracingPrimitiveType == EHairStrandsRayTracingPrimitiveType::LinearSweptSphere)
	{
		Initializer.IndexBuffer = IndexBuffer;
		Initializer.IndexBufferOffset = 0;
		Initializer.GeometryType = RTGT_LinearSweptSpheres;
		Initializer.TotalPrimitiveCount = RayTracingIndexCount / 2;
	}
	else if (RayTracingPrimitiveType == EHairStrandsRayTracingPrimitiveType::Procedural)
	{
	...
	// Build inline rather than queuing through the geometry manager so the same-frame dynamic RT instance gather sees this geometry as built.
	OutRayTracingGeometry->CreateRayTracingGeometry(RHICmdList, ERTAccelerationStructureBuildPriority::Skip);
	UpdateHairAccelerationStructure(RHICmdList, OutRayTracingGeometry, EAccelerationStructureBuildMode::Build);
}
	...
	FRDGBufferRef Raytracing_IndexBuffer)
{
	...
	const EHairStrandsRayTracingPrimitiveType RayTracingPrimitiveType = Instance->Strands.RenRaytracingResource->RayTracingPrimitiveType;
	...
	GraphBuilder.AddPass(
		...
		[Instance, HairLODIndex, bNeedUpdate, RayTracingPrimitiveType, ProceduralSplits](FRHICommandListImmediate& RHICmdList)
		{
			...
			if (bLocalNeedBuild)
			{
				...
				if (RayTracingPrimitiveType == EHairStrandsRayTracingPrimitiveType::Triangle ||
				    RayTracingPrimitiveType == EHairStrandsRayTracingPrimitiveType::LinearSweptSphere)
				{
					IndexBuffer = Instance->Strands.RenRaytracingResource->IndexBuffer.Buffer->GetRHI();
				}
...
FHairGroupPublicData::FVertexFactoryInput InternalComputeHairStrandsVertexInputData(FRDGBuilder* GraphBuilder, const FHairGroupInstance* Instance, EGroomViewMode ViewMode)
{
	...
	OutVFInput.Strands.Common.RaytracingRadiusScale /= InstanceScale; // In raytracing, the BLAS is automatically scaled by the instance transform, so cancel out the InstanceScale factor above to avoid applying it twice
