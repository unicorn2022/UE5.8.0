...
{
	FString Out = TEXT("v21_102031A3EE7E4570B3093414EF3956F3");
...
void ReportSize(const TCHAR* InName, const TArray<T>& In)
{
	...
	UE_LOGF(LogGroomBuilder, Log, "Size: %10d Kbytes - Num: %8d - Name: %ls", TotalKBytes, In.Num(), InName);
#endif
}
			...
			*OffsetIterator = StrandOffset; ++OffsetIterator;
			...
			for (uint32 CurveIndex = 0; CurveIndex < Out.GetNumCurves(); ++CurveIndex, ++OffsetIterator, ++LengthIterator, ++CountIterator)
			{
				...
				*OffsetIterator = StrandOffset;
				...
				{
					const FVector3f CurrentPosition = *PositionIterator;
					Out.BoundingBox += CurrentPosition;
					Out.BoundingBox.IsValid = 1;
					if (PointIndex > 0)
					{
						StrandLength += (CurrentPosition - PreviousPosition).Size();
					}
	...
	void BuildBulkData(const FHairStrandsDatas& HairStrands, const TArray<uint8>& RandomSeeds, const TArray<GroomBuilder_Voxelization::FCoverageScale>& InCoverageScales, FHairStrandsBulkData& OutBulkData, bool bAllowTranscoding)
	{
		...
		for (uint32 CurveIndex = 0; CurveIndex < NumCurves; ++CurveIndex)
		{
			...
			// Curves
			{
				...
				UE_CLOGF(PointCount > HAIR_MAX_NUM_POINT_PER_CURVE, LogGroomBuilder, Warning, "[Groom] Curve point count is greater than the allowed limit (%d/%d)", PointCount, uint32(HAIR_MAX_NUM_POINT_PER_CURVE));
				UE_CLOGF(IndexOffset > HAIR_MAX_NUM_POINT_PER_GROUP, LogGroomBuilder, Warning, "[Groom] Curve point offset is greater than the allowed limit (%d/%d)", IndexOffset, uint32(HAIR_MAX_NUM_POINT_PER_GROUP));
		...
		OutBulkData.Header.Hash = HairStrands.GetHash();
...
namespace HairInterpolationBuilder
{
	...
	{
		UE_LOGF(LogGroomBuilder, Log, "------------------------------------------------------------------------------------------");
		UE_LOGF(LogGroomBuilder, Log, "Total Metric = %f", In.Metric);
		UE_LOGF(LogGroomBuilder, Log, "Distance     = %f (%f)", In.Distance, In.DistanceMetric);
		UE_LOGF(LogGroomBuilder, Log, "Angle        = %f (%f)", FMath::RadiansToDegrees(FMath::Acos(In.CosAngle)), In.AngularMetric);
		UE_LOGF(LogGroomBuilder, Log, "Length       = %f/%f (%f)", In.RenderLength, In.GuideLength, In.LengthMetric);
	}
...
bool FGroomBuilder::BuildHairDescriptionGroups(const FHairDescription& HairDescription, FHairDescriptionGroups& Out, const bool bAllowAddEndControlPoint)
{
	...
	{
		UE_LOGF(LogGroomBuilder, Warning, "No version number attributes found. The groom may be missing attributes even if it imports.");
	}
	#endif
	...
	{
		UE_LOGF(LogGroomBuilder, Warning, "Failed to import hair: No vertices or curves data found.");
		return false;
	}
	...
	for (int32 CurveIndex = 0; CurveIndex < NumCurves; ++CurveIndex)
	{
		...
		if (CurveNumVertices > HAIR_MAX_NUM_POINT_PER_CURVE && !bPointCountWarningIssued)
		{
			...
			//UE_LOGF(LogGroomBuilder, Warning, "[Groom] Groom contains strands with more than %d control points. Control points beyond that limit will be trimmed", uint32(HAIR_MAX_NUM_POINT_PER_CURVE));
		...
		if (!bNumCurveValid)
		{
			if (!bCurveCountWarningIssued)
			{
				...
				// * This is reported at import time
				// * This is visible in Groom editor
				//UE_LOGF(LogGroomBuilder, Warning, "[Groom] Group has more than %d curves per group. Curve beyong that limit won't be part of the groom", uint32(HAIR_MAX_NUM_CURVE_PER_GROUP));
...
struct FStrandsPositionData
{
	...
	TRefCountPtr<FHairGroupInstance> GroupInstance = nullptr;
...
void RunHairStrandsPositionQueries(FRDGBuilder& GraphBuilder, FGlobalShaderMap* ShaderMap, const struct FShaderPrintData* DebugShaderData, const FSceneView* SceneView)
{
	...
	// 2. Readback positions
	{
		...
		{
			if(Q && Q->Scene && Q->GroupInstance &&
				SceneView && SceneView->Family && (Q->Scene == SceneView->Family->Scene))
			{
				if (Q->GPUPositions->IsReady() || Q->ReadbackDelay == 0)
				{
					const TRefCountPtr<FHairGroupInstance> Instance = Q->GroupInstance;
	...
	// 1. Copy positions
	{
		...
		while (GStrandsPositionQueries.Dequeue(Q))
		{
			if(Q && Q->Scene && Q->GroupInstance &&
				SceneView && SceneView->Family && (Q->Scene == SceneView->Family->Scene))
			{
				if (const TRefCountPtr<FHairGroupInstance> Instance = Q->GroupInstance)
				{
					// Require RestResource (the original crash path dereferenced it via Guides.GetData()).
					const bool bGuidesOk  =  Q->bReadGuides && Instance->Guides.IsValid();
					const bool bStrandsOk = !Q->bReadGuides && Instance->HairGroupPublicData && Instance->Strands.IsValid();
					if (!bGuidesOk && !bStrandsOk)
					{
						--Q->Output->Status;
						check(Q->Output->Status >= 0);
						continue;
...
bool RequestStrandsPosition(const UGroomComponent* Component, TSharedPtr<FStrandsPositionOutput> Output, const bool bReadGuides)
{
	...
	for (uint32 GroupIt = 0; GroupIt < GroupCount; ++GroupIt)
	{
		...
		Data->GroupInstance = GetRefCountedHairGroupInstance(Component, GroupIt);
		Data->Scene = (Component && Component->GetWorld()) ? Component->GetWorld()->Scene : nullptr;
...
void InternalBuildRenderCurveResourceBulkData(const FHairStrandsDatas& In, FRenderCurveResourceData& Out)
{
	...
	// Finalize bulk data
	{
		...
		#if 0
		{
			...
			UE_LOGF(LogGroomBuilder, Warning, "[WRITE1] Center %f%f%f CurveCount%d PointPerCurve%d MaxLength%f MaxRadius%f.", Center.X, Center.Y, Center.Z, CurveCount, PointPerCurve0, MaxLength.GetFloat(), MaxRadius.GetFloat());
		}
		#endif
