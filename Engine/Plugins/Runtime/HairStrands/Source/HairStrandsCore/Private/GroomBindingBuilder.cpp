...
FString FGroomBindingBuilder::GetVersion()
{
	...
	return TEXT("6d_563D4DE9469C41D590AE5810A85AA71B");
}
...
public:
	FSkeletalMeshData(const USkeletalMesh* InSkeletalMesh, const FSkeletalMeshRenderData* InRenderData, int32 MeshMinLOD) 
	{
		...
		if (SkeletalMesh)
		{
			...
			{
				UE_LOGF(LogHairStrands, Warning, "Could not retrieve mesh data for SkeletalMesh %ls.", *SkeletalMesh->GetName());
			}
		}
	}
...
public:
		...
		: GeometryCache(InGeometryCache)
	{
		if (GeometryCache)
		{
			...
			if (MeshesData.Num() > 1)
			{
				UE_LOGF(LogHairStrands, Warning, "Cannot use non-flattened GeometryCache %ls as input.", *GeometryCache->GetName());
			}
			else if (MeshesData.Num() == 0)
			{
				UE_LOGF(LogHairStrands, Warning, "Could not read mesh data from the GeometryCache %ls.", *GeometryCache->GetName());
			}
			else
			{
				...
				{
					UE_LOGF(LogHairStrands, Warning, "GeometryCache %ls has no valid mesh data.", *GeometryCache->GetName());
				}
			}
		}
	}
...
namespace GroomBinding_RBFWeighting
{
		...
		const FVector3f* SourcePositions, const FVector3f* TargetPositions, const bool bLocalSpace)
	{
		...
		static constexpr float REGUL_VALUE = 1e-4f;
...
namespace GroomBinding_RootProjection
{
	struct FTriangleGrid
	{
		...
		// Reject degenerate (near-zero area) triangles via cross product to avoid NaN 
		// from division by zero in ComputeClosestPoint's barycentric computation.
		bool IsTriangleValid(const FTriangle& T) const
		{
			const FVector3f AB = T.P1 - T.P0;
			const FVector3f AC = T.P2 - T.P0;
			const FVector3f Cross = FVector3f::CrossProduct(AB, AC);
			return FVector3f::DotProduct(Cross, Cross) > UE_SMALL_NUMBER;
		}
	...
	static FTrianglePoint ComputeClosestPoint(const FTriangleGrid::FTriangle& Tri, const FVector3f& P)
	{
		...
		const float DenomSum = VA + VB + VC;

		// Guard against near-zero denominator from degenerate triangles to prevent NaN propagation.
		// Fallback with picking the closest point
		if (DenomSum <= UE_SMALL_NUMBER)
		{
			const float DistA = FVector3f::DistSquared(P, A);
			const float DistB = FVector3f::DistSquared(P, B);
			const float DistC = FVector3f::DistSquared(P, C);
		...
		const float Denom = 1.f / DenomSum;
		float V = VB * Denom;
		float W = VC * Denom;
	...
	FQueryResult QueryClosestPoint(const FTriangleGrid& InGrid, const FVector3f& InQueryP)
	{
		...
		if (Cells.Num() > 0)
		{
			...
			// Gracefully handle NaN distances to report an error via the existing bValid==false path.
		...
		GroomBinding_Mesh::IMeshData* InMeshData)
	{
		...
		FTriangleGrid* TriangleGrid = nullptr;
		{
			...
			{
				UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. MeshLODData has 0 sections.");
				return;
			}
		...
		TArray<FHairStrandsRootData>& OutRootData)
	{
		...
		for (int32 MeshLODIt = 0; MeshLODIt < MeshLODCount; ++MeshLODIt)
		{
			...
			if (!bIsGridPopulated)
			{
				UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. The target skeletal mesh could be missing UVs.");
				return false;
			}
...
namespace GroomBinding_Transfer
{
		...
		const int32 InMatchingMaterialIndex)
	{
		...
		{
			UE_LOGF(LogHairStrands, Warning, "[Groom] Binding asset will not respect the requested 'Matching material' %d. The source skeletal mesh does not have such a material. Instead 'Matching material' 0 will be used.", InMatchingMaterialIndex);
		}
		FTriangleGrid2D Grid(256);
		{
			...
			if (!bIsGridPopulated)
			{
				UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. The source skeletal mesh is missing or has invalid UVs.");
				return false;
			}
		}
		...
		for (int32 TargetLODIndex = 0; TargetLODIndex < TargetLODCount; ++TargetLODIndex)
		{
			...
			if (!TargetMeshLODData.IsValidMaterialIndex(LocalTargetMaterialIndex))
			{
				...
				{
					UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built for LOD %d. TargetMeshLODData.GetNumSections() == 0.", TargetLODIndex);
					return false;
				}

				if (GHairStrandsBindingBuilderWarningEnable > 0)
				{
					UE_LOGF(LogHairStrands, Warning, "[Groom] Binding asset will not respect the requested 'Matching material' %d for LOD %d. The target skeletal mesh does not have such a material for this LOD. Instead material 0 will be used for this given LOD.", TargetMaterialIndex, TargetLODIndex);
				}
				...
				if (!bIsGridPopulated)
				{
					UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built for LOD %d. The source skeletal mesh is missing or has invalid UVs.", TargetLODIndex);
					return false;
				}
			}
			...
			{
				UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. The target skeletal mesh is missing or has invalid UVs.");
				return false;
			}
...
static void InitHairStrandsRootData(TArray<FHairStrandsRootData>& Out, uint32 CurveCount, uint32 PointCount, uint32 LODCount, uint32 MinLOD, uint32 NumSamples, uint32 InFlags, const FHairStrandsDatas::FHash& InHash)
{
	Out.SetNum(LODCount);
	uint32 LODIndex = 0;
	...
	const FHairStrandsRootData& In)
{
...

	{
		...
		Out.Header.Flags				= FHairStrandsBulkData::ConvertDataFlagsFromHairStrandsFlags(In.Flags);
		Out.Header.Hash					= In.Hash;
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// Helper functions for binding validation code

bool GetHairStrandsBindingDataValidation();

#if WITH_EDITORONLY_DATA

static FHairStrandsCommonDatas::FHash GetHash(const FHairStrandsDatas& In)
{
	return In.GetHash();
}

static uint32 GetFlags(const FHairStrandsDatas& In)
{
	return In.Flags;
}
...
static FHairStrandsCommonDatas::FHash GetHash(const FHairStrandsBulkData& In)
{
	return FHairStrandsCommonDatas::FHash();
}

static uint32 GetFlags(const FHairStrandsBulkData& In)
{
	return FHairStrandsBulkData::ConvertDataFlagsToHairStrandsFlags(In.Header.Flags);
}
...
bool BuildHairRootGroupData(const FGroomBindingBuilder::FInput& In, uint32 InGroupIndex, const ITargetPlatform* TargetPlatform, FHairRootGroupData& OutData)
{
	...
	if (!bIsAssetValid)
	{
		if (!In.GroomAsset)							{ UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset cannot be created/rebuilt - The groom binding has no groom asset."); return false; }
		if (!In.bHasValidTarget)					{ UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset cannot be created/rebuilt - The groom binding has no valid skel./geom cache. target"); return false; }
		if (In.GroomAsset->GetNumHairGroups() == 0)	{ UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset cannot be created/rebuilt - The groom asset has no groups."); return false; }
	}
	...
	OutData = FHairRootGroupData();
	{
		...
		{
			UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. Target mesh is not valid.");
			return false;
		}
		...
		// 1.2 Init root data for guides/strands/cards
		{
			...
			InitHairStrandsRootData(OutData.SimRootDatas, GuidesCurveCount, GuidesPointCount, MeshLODCount, In.TargetMeshMinLOD, In.NumInterpolationPoints, GetFlags(GuidesDataSource), GetHash(GuidesDataSource));
			...
			if (bNeedStrandsRoot)
			{
				...
				InitHairStrandsRootData(OutData.RenRootDatas, StrandsCurveCount, StrandsPointCount, MeshLODCount, In.TargetMeshMinLOD, In.NumInterpolationPoints, GetFlags(StrandsDataSource), GetHash(StrandsDataSource));
			}
			...
			for (uint32 CardsLODIt = 0; CardsLODIt < CardsLODCount; ++CardsLODIt)
			{
				if (GroupData.Cards.IsValid(CardsLODIt))
				{
...
#else
					const FHairStrandsBulkData& LODGuidesData = GroupData.Cards.LODs[CardsLODIt].GuideBulkData;
					...
					if (bIsValid)
					{
						...
						InitHairStrandsRootData(OutData.CardsRootDatas[CardsLODIt], CardsCurveCount, CardsPointCount, MeshLODCount, In.TargetMeshMinLOD, In.NumInterpolationPoints, GetFlags(LODGuidesData), GetHash(LODGuidesData));
					}
				}
			}
		}
		...
		if (bNeedTransferPosition)
		{
			if (!GroomBinding_Transfer::Transfer(
				...
				In.MatchingSection))
			{
				UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. Positions transfer between source and target mesh failed.");
				return false;
			}
		...
		// 1.4 Build root data for guides/strands/cards
		{
			// Guides
			{
				...
				// Sanity check
				#if WITH_EDITORONLY_DATA
				if (GetHairStrandsBindingDataValidation())
				{
					for (const FHairStrandsRootData& RootData : OutData.SimRootDatas)
					{
						check(RootData.Hash == GuidesDataSource.GetHash());
					}
				}
				#endif
				...
				{
					UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. Some guide roots are not close enough to the target mesh to be projected onto it.");
					return false; 
				}
				SlowTask.EnterProgressFrame();
			}
			...
			if (bNeedStrandsRoot)
			{
				...
				if (GetHairStrandsBindingDataValidation())
				{
					for (const FHairStrandsRootData& RootData : OutData.RenRootDatas)
					{
						check(RootData.Hash == StrandsDataSource.GetHash());
					}
				}
				#endif
				...
				{
					UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. Some strand roots are not close enough to the target mesh to be projected onto it.");
					return false;
				}
				SlowTask.EnterProgressFrame();
			}
			...
			for (uint32 CardsLODIt = 0; CardsLODIt < CardsLODCount; ++CardsLODIt)
			{
				if (In.GroomAsset->GetHairGroupsPlatformData()[InGroupIndex].Cards.IsValid(CardsLODIt))
				{
					...
					if (GetHairStrandsBindingDataValidation())
					{
						const FHairStrandsCommonDatas::FHash SourceHash = LODGuidesDataSource.GetHash();
						for (const FHairStrandsRootData& RootData : OutData.CardsRootDatas[CardsLODIt])
						{
							check(RootData.Hash == SourceHash);
						}
					}
#else
					...
					if (bIsValid)
					{
						...
						{
							UE_LOGF(LogHairStrands, Error, "[Groom] Binding asset could not be built. Some cards guide roots are not close enough to the target mesh to be projected onto it.");
							return false; 
						}
					}
				}
