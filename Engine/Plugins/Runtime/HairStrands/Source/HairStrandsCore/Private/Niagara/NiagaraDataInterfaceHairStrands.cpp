...
static const FName GetTimeDilationName("GetTimeDilation");
...
bool UsesNanite(const UGroomComponent* In)
{
	return In && In->GroomAsset && In->GroomAsset->UsesNanite();
}
...
TRefCountPtr<FHairGroupInstance> GetRefCountedHairGroupInstance(const FNDIHairStrandsData* ProxyData)
{
	// When a groom asset use Nanite geometry, we don't support simulation and can't retrieve the HairInstance data.
	if (ProxyData && !ProxyData->bUsesNanite)
	{
		return GetRefCountedHairGroupInstance(ProxyData->HairGroupInstSource.Get(), ProxyData->HairGroupIndex);
	}
	return nullptr;
}
...
static FHairGroupInstanceRDG Convert(FRDGBuilder& GraphBuilder, const TRefCountPtr<FHairGroupInstance>& In)
{
	...
	if (In.IsValid() && In->Guides.IsValid())
	{
		...
		const bool bHasSkinnedInterpolation = In->Guides.DeformedRootResource && In->Guides.RestRootResource && 
			In->Guides.DeformedRootResource->IsValid(MeshLODIndex) && In->Guides.RestRootResource->IsValid(MeshLODIndex);
	...
	const float DeltaSeconds)
{
	if (Interface != nullptr && InData.IsValid())
	{
		...
		if (GroomAsset != nullptr && GroomAsset->GetHairGroupsPhysics().IsValidIndex(InData.GroupIndex) && bIsSimulationEnable)
		{
			...
			{
				if(UWorld* World = Interface->SourceComponent->GetWorld())
				{ 
...
void FNDIHairStrandsProxy::ConsumePerInstanceDataFromGameThread(void* PerInstanceData, const FNiagaraSystemInstanceID& Instance)
{
	...
	{
		UE_LOGF(LogHairStrands, Log, "ConsumePerInstanceDataFromGameThread() ... could not find %ls", *FNiagaraUtilities::SystemInstanceIDToString(Instance));
	}
	...
	for (TSharedRef<FNiagaraEmitterInstance, ESPMode::ThreadSafe> EmitterInst : SystemInstance->GetEmitters())
	{
		...
		ENQUEUE_RENDER_COMMAND(NiagaraInterpolateGroomSimCache) (
			[ProxyData, DataSet_RT=&EmitterDataSet, NodePositionComponent, RestPositionComponent](FRHICommandListImmediate& RHICmdList)
			{
				...
				const TRefCountPtr<FHairGroupInstance> HairGroupInstance = GetRefCountedHairGroupInstance(&ProxyData);
...
void UNiagaraDataInterfaceHairStrands::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
	...
	{
		FNiagaraFunctionSignature Sig;
		Sig.Name = GetTimeDilationName;
		Sig.bMemberFunction = true;
		Sig.bRequiresContext = false;
		Sig.Inputs.Add(FNiagaraVariable(FNiagaraTypeDefinition(GetClass()), TEXT("Hair Strands")));
		Sig.Outputs.Add(FNiagaraVariable(FNiagaraTypeDefinition::GetFloatDef(), TEXT("Time Dilation")));
...
DEFINE_NDI_DIRECT_FUNC_BINDER(UNiagaraDataInterfaceHairStrands, GetTimeDilation);
...
void UNiagaraDataInterfaceHairStrands::GetVMExternalFunction(const FVMExternalFunctionBindingInfo& BindingInfo, void* InstanceData, FVMExternalFunction &OutFunc)
{
	...
	else if (BindingInfo.Name == GetTimeDilationName)
	{
		check(BindingInfo.GetNumInputs() == 1 && BindingInfo.GetNumOutputs() == 1);
		NDI_FUNC_BINDER(UNiagaraDataInterfaceHairStrands, GetTimeDilation)::Bind(this, OutFunc);
	}
...
void UNiagaraDataInterfaceHairStrands::GetTimeDilation(FVectorVMExternalFunctionContext& Context)
{
	VectorVM::FUserPtrHandler<FNDIHairStrandsData> InstData(Context);
	VectorVM::FExternalFuncRegisterHandler<float> OutTimeDilation(Context);

	for (int32 InstanceIdx = 0; InstanceIdx < Context.GetNumInstances(); ++InstanceIdx)
	{
		*OutTimeDilation.GetDestAndAdvance() = InstData->TimeDilation;
	}
}
...
void UNiagaraDataInterfaceHairStrands::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	...
	if (bIsHairValid && bIsRestValid)
	{
		...
		{
			UE_LOGF(LogHairStrands, Log, "Bad bones state");
		}

		if (InstanceRDG.IsDeformedValid() &&  !InstanceRDG.IsRootValid() && InstanceRDG.BindingType == EHairBindingType::Skinning)
		{
			UE_LOGF(LogHairStrands, Log, "FNDIHairStrandsParametersCS() Groom Asset %ls from component %ls is set to use skinning interpolation but the skin resources are not valid",
