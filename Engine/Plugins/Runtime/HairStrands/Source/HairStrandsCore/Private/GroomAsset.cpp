...
static int32 GHairStrandsUsesNaniteGroom = 0;
static FAutoConsoleVariableRef CVarHairStrandsUsesNaniteGroom(TEXT("r.HairStrands.Nanite"), GHairStrandsUsesNaniteGroom, TEXT("Experimental: Use Nanite to render groom geometry. Works only with non-simulated grooms and is incompatible with groom binding."), ECVF_ReadOnly);
static bool UsesNaniteGroom()
{
	return NaniteCurvesSupported() && GHairStrandsUsesNaniteGroom > 0;
}

/////////////////////////////////////////////////////////////////////////////////////////
// Nanite
bool UGroomAsset::UsesNanite() const
{
	return GetRenderData() != nullptr;
}

FGroomAssetRenderData* UGroomAsset::GetRenderData()
{
	return RenderData.Get();
}

const FGroomAssetRenderData* UGroomAsset::GetRenderData() const
{
	return RenderData.Get();
}

void UGroomAsset::SetRenderData(TUniquePtr<class FGroomAssetRenderData>&& InRenderData)
{
	RenderData = MoveTemp(InRenderData);
}

bool UGroomAsset::HasValidNaniteData() const
{
	if (const FGroomAssetRenderData* SMRenderData = GetRenderData())
	{
...
bool UGroomAsset::HasNaniteFallbackMesh(EShaderPlatform ShaderPlatform) const
{	
	return false;
}

#if WITH_EDITOR
void UGroomAsset::ClearAllCachedCookedPlatformData()
{
	if (UsesNaniteGroom())
	{
		if (FGroomAssetRenderData* SMRenderData = GetRenderData())
		{
			SMRenderData->GetResources()->DropBulkData();
		}
	}
...
UMaterialInterface* UGroomAsset::GetNaniteAuditMaterial(int32 MaterialIndex) const
{
	UMaterialInterface* Out = nullptr;
	if (GetHairGroupsMaterials().IsValidIndex(MaterialIndex))
	{
		Out = GetHairGroupsMaterials()[MaterialIndex].Material;
	}
	return Out;
}

const Nanite::FMaterialAudit* UGroomAsset::GetNaniteMaterials() const
{
	if (const FGroomAssetRenderData* LocalRenderData = GetRenderData())
	{
		return LocalRenderData->GetMaterialAudit();
	}
	return nullptr;
}
...
void DumpLoadedGroomAssets(IConsoleVariable* InCVarPakTesterEnabled)
{
	...
	UE_LOGF(LogHairStrands, Log, "[Groom] ##### UGroomAssets #####");
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	UE_LOGF(LogHairStrands, Log, "--  No.  - LOD -    CPU Total (     Guides|    Strands|      Cards|     Meshes) -    GPU Total (     Guides|    Strands|      Cards|     Meshes) - Asset Name ");
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	FString OutputData;
...
//					UE_LOGF(LogHairStrands, Log, "-- 00/00 -  00 -  0000.0Mb (0000.0Mb |0000.0Mb|0000.0Mb |0000.0Mb) -  0000.0Mb (0000.0Mb|0000.0Mb|0000.0Mb|0000.0Mb) - AssetName");
					UE_LOGF(LogHairStrands, Log, "-- %2d/%2d -  %2d -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb) -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb) - %ls", 
						GroupIt,
						GroupCount,
						LODCount, 
	...
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	UE_LOGF(LogHairStrands, Log, "-- A:%3d|G:%3d -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb) -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb|%9.3fMb)",
		Total_Asset,
		Total_Group,
...
void DumpLoadedGroomBindingAssets(IConsoleVariable* InCVarPakTesterEnabled)
{
	...
	UE_LOGF(LogHairStrands, Log, "[Groom] ##### UGroomBindingAssets #####");
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	UE_LOGF(LogHairStrands, Log, "--  No.  - LOD -    CPU Total (     Guides|    Strands|      Cards) -    GPU Total (     Guides|    Strands|      Cards) - Asset Name ");
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	FString OutputData;
	for (TObjectIterator<UGroomBindingAsset> AssetIt; AssetIt; ++AssetIt)
	{
		...
		{			
			...
			for (uint32 GroupIt = 0; GroupIt < GroupCount; ++GroupIt)
			{
				...
				{
//					UE_LOGF(LogHairStrands, Log, "--  No.  - LOD -    CPU Total (     Guides|    Strands|      Cards) -    GPU Total (     Guides|    Strands|      Cards) - Asset Name ");
//					UE_LOGF(LogHairStrands, Log, "-- 00/00 -  00 -  0000.0Mb (0000.0Mb |0000.0Mb|0000.0Mb) -  0000.0Mb (0000.0Mb|0000.0Mb|0000.0Mb) - AssetName");
					UE_LOGF(LogHairStrands, Log, "-- %2d/%2d -  %2d -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb) -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb) - %ls", 
						GroupIt,
						GroupCount,
						SkelLODCount,
                        Group.GetTotalCPUSize() * ToMb,
	...
	UE_LOGF(LogHairStrands, Log, "----------------------------------------------------------------------------------------------------------------------------------------------------------------");
	UE_LOGF(LogHairStrands, Log, "-- A:%3d|G:%3d -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb) -  %9.3fMb (%9.3fMb|%9.3fMb|%9.3fMb)",
		Total_Asset,
		Total_Group,
		Total.GetTotalCPUSize() * ToMb,
		Total.CPU.Guides * ToMb,
...
void UGroomAsset::Serialize(FArchive& Ar)
{
	...
	else
	{
		...
		UE_LOGF(LogHairStrands, Error, "[Groom] The groom asset %ls is too old. Please reimported the groom from its original source file.", *GetName());
	}

	// Basic handling of Nanite resources
	if (UsesNaniteGroom())
	{
		#if WITH_EDITOR
		if (bIsCookedOrCooking && DoesTargetPlatformSupportNanite(Ar.CookingTarget()))
		{			
			GetRenderData()->Serialize(Ar, this, true);
		}
		#else
		if (DoesPlatformSupportNanite(GMaxRHIShaderPlatform))
		{
			check(Ar.IsLoading());
			SetRenderData(MakeUnique<FGroomAssetRenderData>());
			GetRenderData()->InitNaniteResources();
			GetRenderData()->Serialize(Ar, this, true);
			GetRenderData()->BuildMaterials(this);
		}
		#endif
	}
}
...
TArray<FHairVertexFactoryTypesPerMaterialData> UGroomAsset::CollectVertexFactoryTypesPerMaterialData(EShaderPlatform ShaderPlatform)
{
	...
	for (int32 GroupIt = 0; GroupIt < GroupCount; GroupIt++)
	{
		...
		// Strands
		{
			...
			{
				AddVFPerMaterial(MaterialIndex, EHairGeometryType::Strands, &FLinearSweptSpheresHairStrandsVertexFactory::StaticType);
			}
			else
			{
...
void UGroomAsset::InitResources()
{
	...
	if (FGroomAssetRenderData* LocalRenderData = GetRenderData())
	{
		LocalRenderData->InitResources(GetWorld() ? GetWorld()->GetFeatureLevel() : ERHIFeatureLevel::Num, this);
	}
	...
	bool bSucceed = true;
	{
		...
		if (bAsyncLoadEnabled)
		{
			...
			Async(EAsyncExecution::LargeThreadPool,
				[this]()
				{
					UE_LOGF(LogHairStrands, Log, "[Groom] %ls is fully cached. Loading it asynchronously.", *GetName());
					...
					{
						UE_LOGF(LogHairStrands, Log, "[Groom] %ls failed to load asynchronously. Trying synchronous load.", *GetName());
...
bool UGroomAsset::CacheDerivedDatas()
{
	...
	if (UsesNaniteGroom())
	{
		...
		if (DoesTargetPlatformSupportNanite(RunningPlatform))
		{
			SetRenderData(MakeUnique<FGroomAssetRenderData>());
...
bool UGroomAsset::CacheStrandsData(uint32 GroupIndex, FString& OutDerivedDataKey)
{
	...
	if (Data)
	{
		UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[Groom/DDC] Strands - Found (Groom:%ls Group6:%d).", *GetName(), GroupIndex);
	...
	if (!bSuccess)
	{
		...
		UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[Groom/DDC] Strands - Not found (Groom:%ls Group:%d).", *GetName(), GroupIndex);

		const FHairDescriptionGroups& LocalHairDescriptionGroups = GetHairDescriptionGroups();

		if (!LocalHairDescriptionGroups.IsValid())
		{
			UE_LOGF(LogHairStrands, Warning, "[Groom] The groom asset %ls does not have valid hair groups.", *GetName());
			return false;
		}
...
bool UGroomAsset::CacheCardsData(uint32 GroupIndex, const FString& StrandsKey)
{
	...
	if (Data)
	{
		UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[Groom/DDC] Cards - Found (Groom:%ls Group:%d).", *GetName(), GroupIndex);
	...
	{
		UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[Groom/DDC] Cards - Not found (Groom:%ls Group:%d).", *GetName(), GroupIndex);
...
bool UGroomAsset::CacheMeshesData(uint32 GroupIndex)
{
	...
	if (Data)
	{
		UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[Groom/DDC] Meshes - Found (Groom:%ls Group:%d).", *GetName(), GroupIndex);
	...
	{
		UE_CLOGF(IsHairStrandsDDCLogEnable(), LogHairStrands, Log, "[Groom/DDC] Meshes - Not found (Groom:%ls Group:%d).", *GetName(), GroupIndex);
	...
	FHairCardsInterpolationBulkData& OutInterpolationBulkData)
{
	...
	if (InHairDescriptionGroups.IsValid() && InHairDescriptionGroups.HairGroups.IsValidIndex(InDesc->GroupIndex))
	{
	...
	{
		// If no hair description is available, build hair card with dummy hair strands data
		FHairStrandsDatas DummyHairGuidesData;
		FHairStrandsDatas DummyHairStrandsData;
		FHairStrandsVoxelData DummyHairStrandsVoxelData;
		return FHairCardsBuilder::ImportGeometry(CardsMesh, DummyHairGuidesData, DummyHairStrandsData, DummyHairStrandsVoxelData, true /*bGenerateGuidesFromCardGeometry*/, OutBulkData, OutGuidesData, OutInterpolationBulkData);
	}
}

bool UGroomAsset::HasChanged_Cards(uint32 GroupIndex, TArray<bool>& OutIsValid) const 
{
	if (!IsHairStrandsEnabled(EHairStrandsShaderType::Cards) || (GetHairGroupsCards().Num() == 0 && CachedHairGroupsCards.Num() == 0))
	{
		return false;
	}
	...
	for (uint32 LODIt = 0; LODIt < LODCount; ++LODIt)
	{
		if (FHairGroupsCardsSourceDescription* Desc = GetSourceDescription(GetHairGroupsCards(), GroupIndex, LODIt))
		{
			...
			if (UStaticMesh* CardsMesh = Desc->ImportedMesh)
			{
				...
				if (!bSuccess)
				{
					UE_LOGF(LogHairStrands, Warning, "Failed to import cards from %ls for Group %d LOD %d.", *CardsMesh->GetName(), GroupIndex, LODIt);
					LOD.BulkData.Reset();
				}
			}
			...
			if (bSuccess)
			{
				...
				{
					// Use cards' guides as simulation guides for building interpolation.
					// but these cards' guides won't be simulated, since they are not written as main guides
					SimGuidesData = LODGuidesData;
				}
...
FHairStrandsRaytracingResource** UGroomAsset::GetStrandsRaytracingResourcesPtr(uint32 GroupIndex)
{
	if (GetHairGroupsPlatformData().IsValidIndex(GroupIndex))
	{
		FHairGroupPlatformData& GroupData = GetHairGroupsPlatformData()[GroupIndex];
...
{
	ChangeShaderPlatform(GShaderPlatformForFeatureLevel[In]);
}
...
void UGroomAsset::ChangeShaderPlatform(EShaderPlatform InShaderPlatform)
{
	// When changing platform preview, recreate resources to the correct platform settings (e.g., r.hairstrands.strands=0/1)
	if (CachedResourceShaderPlatform != InShaderPlatform)
	{
		RecreateResources();
		CachedResourceShaderPlatform = InShaderPlatform;
	}
}
#endif
