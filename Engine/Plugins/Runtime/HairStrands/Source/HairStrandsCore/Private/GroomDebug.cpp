...
static void AddShaderPrintDebugFlag(EShaderPlatform InPlatform, FShaderCompilerEnvironment& OutEnvironment)
{
	const bool bNeedDebugFlags = FDataDrivenShaderPlatformInfo::GetIsLanguageNintendo(InPlatform);
	if (bNeedDebugFlags)
	{
...
public:
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		FGlobalShader::ModifyCompilationEnvironment(Parameters, OutEnvironment);
		ShaderPrint::ModifyCompilationEnvironment(Parameters, OutEnvironment);
		AddShaderPrintDebugFlag(Parameters.Platform, OutEnvironment);
	...
	const FHairInstanceCullingResults& CullingResults)
{
	...
	for (uint32 InstanceIndex = 0; InstanceIndex < InstanceCount; ++InstanceIndex)
	{
		const FHairGroupInstance* Instance = static_cast<const FHairGroupInstance*>(Instances[InstanceIndex].GetReference());
