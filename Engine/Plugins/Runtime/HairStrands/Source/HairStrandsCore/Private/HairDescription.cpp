...
static TConstArrayView<FGuid> GetHairDescriptionCustomVersions()
{
	// Use FMeshDescriptionBulkData's versions as HairDescription uses structures and functions 
	// from FMeshDescription(BulkData) which are subject to serizalization changes
	return FMeshDescriptionBulkData::GetMeshDescriptionCustomVersions();
}
#endif
...
{
	for (const FGuid& CustomVersionGuid : GetHairDescriptionCustomVersions())
	{
		Ar.UsingCustomVersion(CustomVersionGuid);
	}
