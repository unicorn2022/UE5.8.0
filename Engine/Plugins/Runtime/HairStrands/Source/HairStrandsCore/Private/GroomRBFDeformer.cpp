		...
		if (StrandGuide > 0)
		{
			...
			for (int32 Index = 0; Index < NumVertices; ++Index)
			{
				...
				else
				{
					...
					FlattenedDeformedPositions.Add(DeformedPositions[GroupIndex].GuideStrands[GroupInfo.CurrentGuideVertexIndex-1]);
				}
			}
		}
		else
	...
	const FHairStrandsRootData& RestLODData)
{
	...
	// Raw deformation with RBF
	const uint32 MeshLODCount = OutMesh->GetNumSourceModels();
	for (uint32 MeshLODIt = 0; MeshLODIt < MeshLODCount; ++MeshLODIt)
	{
		...
		// Disable bMarkPackageDirty to make this thread safe.
		//
		// Package will be marked dirty by the caller.
		UStaticMesh::FCommitMeshDescriptionParams CommitParams;
		CommitParams.bMarkPackageDirty = false;
		OutMesh->CommitMeshDescription(MeshLODIt, CommitParams);
	}
}
...
{
	const FUintPoint Coord(FMath::Min(Resolution.X - 1, InCoord.X), FMath::Min(Resolution.Y - 1, InCoord.Y));
	const uint32 Index = ComponentIndex + Coord.X * NumComponent + Coord.Y * Resolution.X * NumComponent;
	return InData[Index];
}
...
float SampleTexture(const FVector2f& InUV, const uint8 InComponentIndex, const uint8 InNumComponent, const FUintPoint& InResolution, const uint8* InRawData, float InNormalizationScale)
{
	...
	const T T01 = SampleTexture<T>(P01, InComponentIndex, InNumComponent, InResolution, TypedData);
...
void FGroomRBFDeformer::GetRBFDeformedGroomAsset(
...
{
	// If bShouldBuildStaticMeshes is true, this function may only be called from the Game Thread
	check(!bShouldBuildStaticMeshes || IsInGameThread());
	...
	if (InGroomAsset && BindingAsset && BindingAsset->GetTargetSkeletalMesh() && BindingAsset->GetSourceSkeletalMesh())
	{
		...
		for (const FHairDescriptionGroup& Group : HairDescriptionGroups.HairGroups)
		{
			...
			const TArray<FVector3f>& MeshVertexPositionsBuffer_Target = RootDatas[GroupIndex].MeshPositions[MeshLODIndex];
			...
			{
				const FHairStrandsRootData& SimRootData = RootDatas[GroupIndex].SimRootDatas[MeshLODIndex];
			...
			// Get deformed render strands
			{
				...
				// This is transient data.
				{
					...
					const FHairStrandsRootData& SimLODData = SimRootData;
		...
		for (FHairGroupsCardsSourceDescription& Desc : OutGroomAsset->GetHairGroupsCards())
		{
			if (UStaticMesh* Mesh = Desc.ImportedMesh)
			{
				if (Desc.GroupIndex >= 0)
				{
					...
					const FHairStrandsRootData& SimRootData = RootDatas[Desc.GroupIndex].SimRootDatas[MeshLODIndex];
		...
		for (FHairGroupsMeshesSourceDescription& Desc : OutGroomAsset->GetHairGroupsMeshes())
		{
			if (UStaticMesh* Mesh = Desc.ImportedMesh)
			{
				if (Mesh->GetNumSourceModels() == 0 || Desc.GroupIndex < 0 || Desc.GroupIndex >= InGroomAsset->GetNumHairGroups() || Desc.LODIndex == -1)
				{
					continue;
				}
		...
		// Build all static meshes in one batch
		if (bShouldBuildStaticMeshes)
		{
			TArray<UStaticMesh*> MeshesToBuild;
			MeshesToBuild.Reserve(OutGroomAsset->GetHairGroupsCards().Num() + OutGroomAsset->GetHairGroupsMeshes().Num());

			for (FHairGroupsCardsSourceDescription& Desc : OutGroomAsset->GetHairGroupsCards())
			{
				if (UStaticMesh* Mesh = Desc.ImportedMesh)
				{
					MeshesToBuild.Add(Mesh);
			...
			UStaticMesh::BatchBuild(MeshesToBuild, /*bSilent*/ true);
