 ...
 * 
 * This is templated to allow different behavior for Linear Swept Spheres, which do not require an intersection shader.
 */
template<bool bSupportLinearSweptSpheres>
class THairStrandsVertexFactory : public FVertexFactory
{
	DECLARE_VERTEX_FACTORY_TYPE(THairStrandsVertexFactory);
public:
	...
	THairStrandsVertexFactory(FHairGroupInstance* Instance, ERHIFeatureLevel::Type InFeatureLevel, const char* InDebugName)
	...
	*/
	UE_API void Copy(const THairStrandsVertexFactory& Other);
...
using FLinearSweptSpheresHairStrandsVertexFactory = THairStrandsVertexFactory<true>;
using FHairStrandsVertexFactory = THairStrandsVertexFactory<false>;
