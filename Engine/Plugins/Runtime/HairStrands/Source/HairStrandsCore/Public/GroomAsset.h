...
public:
	bool UsesNanite() const;
	FGroomAssetRenderData* GetRenderData();
	const FGroomAssetRenderData* GetRenderData() const;
	...
	const Nanite::FMaterialAudit* GetNaniteMaterials() const;

private:
	void SetRenderData(TUniquePtr<class FGroomAssetRenderData>&& InRenderData);
	TUniquePtr<class FGroomAssetRenderData> RenderData;
...
//private :
	...
	UE_DEPRECATED(5.8, "Use ChangeShaderPlatform(EShaderPlatform) instead.")
	...
	UE_API void ChangeShaderPlatform(EShaderPlatform InShaderPlatform);
#endif
...
private:
	...
	FHairStrandsRaytracingResource**	GetStrandsRaytracingResourcesPtr(uint32 GroupIndex);
...
private:
	...
	EShaderPlatform								CachedResourceShaderPlatform = SP_NumPlatforms;
