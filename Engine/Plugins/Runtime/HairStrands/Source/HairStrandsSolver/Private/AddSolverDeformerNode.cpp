...
void FAddSolverDeformerDataflowNode::EvaluateSimulation(UE::Dataflow::FDataflowSimulationContext& SimulationContext, const FDataflowOutput* Output) const
{
	...
	UOptimusDeformer* OptimusDeformer = Cast<UOptimusDeformer>(MeshDeformer);
	if(!SolverProperties.IsEmpty() && OptimusDeformer)
	{
			...
			if(SolverProperty.SimulationProxy)
			{
				if(FDataflowGroomSolverProxy* GroomProxy = SolverProperty.SimulationProxy->AsType<FDataflowGroomSolverProxy>())
				{
					...
					if (!DeformerInstanceGuid.IsValid())
					{
						...
						{
							if(GroomProxy->DeformerInstance)
							{
								// Create a new GUID for the new deformer instance
								DeformerInstanceGuid = FGuid::NewGuid();

								// Build a deformer instance given a guid
								UE::Groom::Private::CreateDeformerInstance(OptimusDeformer, GroomProxy->DeformerInstance, DeformerInstanceGuid, DataflowObject);
							}
						}
					}
...
{
	if(UOptimusDeformer* OptimusDeformer = Cast<UOptimusDeformer>(MeshDeformer))
	{
		TArray<UE::Dataflow::FPin> Pins;
		for (UOptimusVariableDescription* Variable : OptimusDeformer->GetVariables())
		{
			...
			{
				UE_LOGF(LogTemp, Error, "Unsupported Dataflow variable type");
			}			
		}
		return Pins;
	}
