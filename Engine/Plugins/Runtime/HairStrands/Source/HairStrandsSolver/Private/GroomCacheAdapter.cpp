...
void FGroomCacheAdapter::SetRestState(UPrimitiveComponent* InComponent, UChaosCache* InCache, const FTransform& InRootTransform, Chaos::FReal InTime) const
{
	if(bIsLoading)
	{
		if(UGroomComponent* GroomComponent = CastChecked<UGroomComponent>(InComponent))
		{
			...
			{
				if(InTime < GroomCache->GetDuration())
				{
					// Undo the dataflow editor's +DeltaTime on the groom path: groom cache lookup is
					// floor(T / SecondsPerFrame) and the +DeltaTime over-compensates here.
					const float CacheDt = GroomCache->GetGroomAnimationInfo().SecondsPerFrame;
					const float CorrectedTime = FMath::Max(0.0f, float(InTime) - CacheDt);
					GroomComponent->TickAtThisTime(CorrectedTime, /*bInIsRunning=*/true, /*bInBackwards=*/false, /*bInIsLooping=*/true);
				}
			}
		}
	}
...
bool FGroomCacheAdapter::InitializeForRecord(UPrimitiveComponent* InComponent, FObservedComponent& InObserved)
{
	...
	if(UGroomComponent* GroomComponent = CastChecked<UGroomComponent>(InComponent))
	{
		...
		// SetGroomCache(nullptr) rebuilds the component's instances; force a fresh proxy
		// synchronously so the first Record_PostSolve doesn't read the stale one and crash.
...
FORCEINLINE void FillGuidesPositions(const FStrandsPositionOutput& PositionsBuffer, TArray<FGroomCacheInputData>& GroupsData)
{
	if(PositionsBuffer.IsValid() && PositionsBuffer.Groups.Num() == GroupsData.Num())
	{
		for(uint32 GroupIndex = 0, NumGroups = GroupsData.Num(); GroupIndex < NumGroups; ++GroupIndex)
		{
			...
			const uint32 NumPoints = GuidesPositions.Num();
			uint32 PointIndex = 0;
			for(uint32 GuideIndex = 0, NumGuides = PositionsGroup.Num(); GuideIndex < NumGuides; ++GuideIndex)
			{
				// BuildGroupsData() builds destination with bAllowAddEndControlPoint=false; write all
				// source points (the original code's -1 was dropping the tip of each guide).
				for(int32 VertexIndex = 0, NumVertices = PositionsGroup[GuideIndex].Num();
						  VertexIndex < NumVertices && PointIndex < NumPoints; ++VertexIndex, ++PointIndex)
				{
		...
		for(TPair<UPrimitiveComponent*,FGroomCacheData>& GroomCache : GroomCaches)
		{
			if(UGroomComponent* GroomComponent = CastChecked<UGroomComponent>(GroomCache.Key))
			{
				...
				if(!GroomCache.Value.PositionsBuffer.IsEmpty())
				{
					...
					GroomCache.Value.AnimInfo.SecondsPerFrame = GroomCache.Value.AnimInfo.NumFrames > 0
						? GroomCache.Value.AnimInfo.Duration / GroomCache.Value.AnimInfo.NumFrames
						: 0.0f;
