...
FReply FGroomRenderingDetails::OnGenerateCardDataUsingPlugin(int32 GroupIndex)
{
	if (GroomAsset && GroomAsset->GetHairGroupsCards().IsValidIndex(GroupIndex))
	{
		...
		{
			UE_CLOGF(HairCardGeneratorPlugins.Num() > 1, LogGroomAssetDetails, Warning, "There are more than one available hair-card generator options. Defaulting to the first one found.");
