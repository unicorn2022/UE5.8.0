...
void FGroomEditor::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FGroomEditor::OnPostEngineInit);
	...
	DataflowMenusHandle = UE::DataflowAssetDefinitionHelpers::RegisterDataflowAssetMenus(UGroomAsset::StaticClass());
}
...
void FGroomEditor::OnPostEngineInit()
{
	...
	if (GEditor)
	{
		...
		PreviewPlatformChangedHandle = EditorEngine->OnPreviewShaderPlatformChanged().AddRaw(this, &FGroomEditor::OnPreviewPlatformChanged);
	}
}
...
void FGroomEditor::OnPreviewPlatformChanged(EShaderPlatform NewShaderPlatform)
{
#if WITH_EDITOR
	for (TObjectIterator<UGroomComponent> It; It; ++It)
	{
		...
		{
			Component->HandlePlatformPreviewChanged(NewShaderPlatform);
		}
	}
#endif
}
