...
namespace ReimportGroomAssetHelpers
{
	...
	UGroomAsset* ReimportGroom(const FString& SourceFilename, FMD5Hash& FileHash, TSharedPtr<IGroomTranslator> Translator, FHairImportContext& HairImportContext, FHairDescription& HairDescription, UGroomAsset* HairAsset, const FGroomCacheImportSettings& Settings, const UGroomHairGroupsMapping* InGroupsMapping)
	{
		if (Settings.bImportGroomAsset)
		{
			...
			{
				UE_LOGF(LogReimportHairStrandsFactory, Error, "Failed to reimport groom asset.");
			}
...
EReimportResult::Type UReimportHairStrandsFactory::Reimport(UObject* Obj)
{
	...
	if (!HairAsset)
	{
		...
		if (!SourceGroomCache)
		{
			UE_LOGF(LogReimportHairStrandsFactory, Error, " Not the correct asset type to reimport.");
		...
		if (!GroomCacheImportData)
		{
			UE_LOGF(LogReimportHairStrandsFactory, Error, "Asset import data missing.");
			return EReimportResult::Failed;
		}
		...
		if (!HairAsset)
		{
			...
			UE_LOGF(LogReimportHairStrandsFactory, Error, "Associated groom asset missing.");
			return EReimportResult::Failed;
		}
	}
	...
	{
		UE_LOGF(LogReimportHairStrandsFactory, Error, "File format not supported.");
		return EReimportResult::Failed;
	}
	...
	FHairDescriptionGroups OutDescription;
	{
		...
		{
			UE_LOGF(LogReimportHairStrandsFactory, Error, "Error translating file %ls.", *CurrentFilename);
			return EReimportResult::Failed;
		}
	...
	// Import asset with or without import window
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogReimportHairStrandsFactory, Error, "Error during groom reimport (file %ls) : %ls", *CurrentFilename, *GetGroomImportStatusText(ImportStatus, false /*bAddPrefix*/).ToString());
				return EReimportResult::Failed;
			}
			else if (EnumHasAnyFlags(ImportStatus.Status, EHairDescriptionStatus::Warning))
			{
				UE_LOGF(LogReimportHairStrandsFactory, Warning, "Warning during groom reimport (file %ls) : %ls", *CurrentFilename, *GetGroomImportStatusText(ImportStatus, false /*bAddPrefix*/).ToString());
			}
		}
	}
