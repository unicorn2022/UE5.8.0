	...
	if (!bHasInitialized)
	{
		...
		if (ensureAlwaysMsgf(Env, TEXT("Invalid JNIEnv! Skipping Initialize!")))
		{
			...
			//find jmethodID for UEWorkManagerJavaInterface
			{
				...
				JavaInterface_Method_IsWorkActive = FJavaWrapper::FindStaticMethod(Env, UEWorkManagerJavaInterfaceClass, "AndroidThunkJava_IsWorkActive", "(Landroid/content/Context;Ljava/lang/String;)Z", bIsOptional);
...
TOptional<bool> FUEWorkManagerNativeWrapper::CheckIfBackgroundWorkerIsRunning(FString UniqueWorkName)
{
	TOptional<bool> bIsActive;
	...
	if (ensureAlwaysMsgf(Env, TEXT("Invalid JNIEnv!")))
	{
		...
		bIsActive = (bool)Env->CallStaticBooleanMethod(FUEWorkManagerNativeWrapper::JavaInfo.UEWorkManagerJavaInterfaceClass, JavaInfo.JavaInterface_Method_IsWorkActive, *JavaAppContext, *JavaWorkName);
	}
...
void JNICALL UE::Jni::WorkManager::FUEWorker::nativeAndroidBackgroundServicesOnWorkerStart(JNIEnv* jenv, jobject thiz, jstring WorkID)
{
	...
	UE_LOGF(LogEngine, Display, "AndroidBackgroundServices called OnWorkerStart for %ls", *UEWorkID);
...
void JNICALL UE::Jni::WorkManager::FUEWorker::nativeAndroidBackgroundServicesOnWorkerStop(JNIEnv* jenv, jobject thiz, jstring WorkID)
{
	...
	UE_LOGF(LogEngine, Display, "AndroidBackgroundServices called OnWorkerStop for %ls", *UEWorkID);
