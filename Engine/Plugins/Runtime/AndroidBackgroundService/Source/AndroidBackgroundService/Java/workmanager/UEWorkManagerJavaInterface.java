...
public class UEWorkManagerJavaInterface
{
	...
	public static boolean AndroidThunkJava_IsWorkActive(Context AppContext, String TaskID)
	{
		WorkManager workManager = WorkManager.getInstance(AppContext.getApplicationContext());
		boolean bIsActive = isWorkScheduled(workManager, TaskID);
		Log.error("AndroidThunkJava_IsWorkActive: " + TaskID + " is active: " + bIsActive);
		return bIsActive;
	}
...
//						workManager.pruneWork();
					...
					else
					{
						...
						QueueOperationResult = workManager.enqueueUniqueWork(TaskID, ExistingWorkPolicy.KEEP, newWorkRequest);
					}
				}
				else
				{
