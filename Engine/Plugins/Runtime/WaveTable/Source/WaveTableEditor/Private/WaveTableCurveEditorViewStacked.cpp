	...
	namespace Editor
	{
		...
		void SViewStacked::DrawViewGrids(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 BaseLayerId, ESlateDrawEffect DrawEffects) const
		{
			...
			for (auto It = CurveInfoByID.CreateConstIterator(); It; ++It)
			{
				...
				double DefaultValueHeight = -1.0;
				if (const FWaveTableCurveModel* WaveTableCurveModel = static_cast<const FWaveTableCurveModel*>(CurveModel))
				{
					if (WaveTableCurveModel->ShowDefaultDashedLine())
					{
						double ValueMin = 0.0;
						double ValueMax = 0.0;
						WaveTableCurveModel->GetValueRange(ValueMin, ValueMax);
						double DefaultValue = WaveTableCurveModel->GetDefaultDashedLineValue(); 
						DefaultValueHeight = FMath::GetMappedRangeValueClamped(TRange<double>(ValueMin, ValueMax), TRange<double>(0.0, 1.0), DefaultValue);
					}
				}

				// Dashed line on the default value
				if (const FWaveTableCurveModel* WaveTableCurveModel = static_cast<const FWaveTableCurveModel*>(CurveModel))
				{
					if (WaveTableCurveModel->ShowDefaultDashedLine())
					{	
						DrawViewGridLineX(OutDrawElements, DrawInfo, DrawEffects, DefaultValueHeight  /* OffsetAlpha */, true  /* bIsMajor */, true /* bDashedLine */);
					}
				}
				
				TArray<double>OffsetAlphas = { 1.0, 0.75, 0.5, 0.25, 0.0 };
				for (int OffsetAlphaIndex = 0; OffsetAlphaIndex < OffsetAlphas.Num(); ++OffsetAlphaIndex)
				{
					// Skip dashed line
					double OffsetAlpha = OffsetAlphas[OffsetAlphaIndex];
					if (FMath::IsNearlyEqual(OffsetAlpha, DefaultValueHeight))
					{
						continue;
					}
					// Even indices are major lines
					bool bIsMajor = !static_cast<bool>(OffsetAlphaIndex % 2);
					DrawViewGridLineX(OutDrawElements, DrawInfo, DrawEffects, OffsetAlphas[OffsetAlphaIndex], bIsMajor, false /* bDashedLine */);
				}
		...
		void SViewStacked::DrawViewGridLineX(FSlateWindowElementList& OutDrawElements, FGridDrawInfo& DrawInfo, ESlateDrawEffect DrawEffects, double OffsetAlpha, bool bIsMajor, bool bDashedLine) const
		{
			double ValueMin = 0.0;
			double ValueMax = 0.0;
			...
			if (bDashedLine)
			{
				// There is no FVector2d version of MakeDashedLines, so convert 
				TArray<FVector2f> FloatPoints;
				Algo::Transform(DrawInfo.LinePoints, FloatPoints, [](FVector2d DoubleIn)
					{
						return FVector2f(DoubleIn);
					});
				FSlateDrawElement::MakeDashedLines(OutDrawElements, LineLayerId, DrawInfo.PaintGeometry, MoveTemp(FloatPoints), DrawEffects, Color);
			}
			else
			{
