			...
			if (!InFilePath.IsEmpty())
			{
				if (USoundFactory* SoundWaveFactory = NewObject<USoundFactory>())
				{
					...
					if (USoundWave* SoundWave = ImportObject<USoundWave>(TempPackage, "ImportedWaveTable", RF_Public | RF_Standalone, *InFilePath, nullptr, SoundWaveFactory))
					{
						...
						{
							UE_LOGF(LogWaveTableEditor, Error, "Failed to import/reimport file '%ls': Invalid num channels '%u' imported", *InFilePath, NumChannels);
						}
					}
					else
					{
						UE_LOGF(LogWaveTableEditor, Error, "Failed to import/reimport file '%ls'", *InFilePath);
					}
				}
			}
		}
