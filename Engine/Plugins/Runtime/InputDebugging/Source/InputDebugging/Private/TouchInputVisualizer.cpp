...
void FTouchInputVisualizer::OnDebugDraw(UCanvas* Canvas)
{
	if (GInputEnableTouchDebugVisualizer)
	{
		...
		for (auto& Itr : DebugTouchPoints)
		{
			...
			FVector2D Center = TouchPoint.Center;
#if WITH_EDITOR
			if (GEngine && GEngine->GameViewport)
			{
				if (const SViewport* GameViewportWidget = GEngine->GameViewport->GetGameViewportWidget().Get())
				{
					// subtract the viewport position to make sure the debug circle is drawn at the correct position even if the viewport origin is not in the top-left corner of the screen
					Center = TouchPoint.Center - GameViewportWidget->GetCachedGeometry().GetAbsolutePosition();
				}
			}
#endif
			const float Radius = MaxRadius * (1.0f + TouchPoint.Pressure);

			FVector2D LastVertex = Center + (FVector2D::UnitX() * Radius);
			for (int32 SideIndex = 0; SideIndex < NumSides; SideIndex++)
			{
				const FVector2D Vertex = Center + (FVector2D::UnitX() * FMath::Cos(AngleDelta * (SideIndex + 1)) + FVector2D::UnitY() * FMath::Sin(AngleDelta * (SideIndex + 1))) * Radius;
