...
void FInputDeviceDebugTools::ListAllKnownHardwareDeviceIdentifier(const TArray<FString>& Args, UWorld* World)
{
	if (UInputPlatformSettings* System = UInputPlatformSettings::Get())
	{
		...
		{
			UE_LOGF(LogInputDeviceProperties, Log, "%ls::%ls", *Device.InputClassName.ToString(), *Device.HardwareDeviceIdentifier.ToString());
		}
	}
}
...
void FInputDeviceDebugTools::LogAllConnectedDevices(const TArray<FString>& Args, UWorld* World)
{
	...
	if (!System)
	{
		UE_LOGF(LogInput, Error, "[%s] 'Input.LogAllConnectedDevices' command failed! Unable to find a valid UInputDeviceSubsystem!", __func__);
		return;
	}
	...
	UE_LOGF(LogInput, Log, "[%s] Logging all connected input devices from 'Input.LogAllConnectedDevices' command... \n%ls", __func__, *LogMessage);
}
