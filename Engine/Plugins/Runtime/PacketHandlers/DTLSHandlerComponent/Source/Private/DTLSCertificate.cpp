...
bool FDTLSCertificate::ExportCertificate(const FString& CertPath)
{
	if (Certificate)
	{
		if (!CertPath.IsEmpty())
		{
			...
			{
				UE_LOGF(LogDTLSHandler, Error, "ExportCertificate: Unable to create file writer for: %ls", *CertPath);
			}
		}
		else
		{
			UE_LOGF(LogDTLSHandler, Error, "ExportCertificate: Empty cert path");
		}
	}
	else
	{
		UE_LOGF(LogDTLSHandler, Error, "ExportCertificate: Cert not valid");
	}
	...
	if (Certificate == nullptr)
	{
		if (!CertPath.IsEmpty())
		{
			...
			if (ImportAr.IsValid())
			{
				...
				{
					UE_LOGF(LogDTLSHandler, Error, "ImportCertificate: Unable to read valid x509 certifcate");
				}
			}
			else
			{
				UE_LOGF(LogDTLSHandler, Error, "ImportCertificate: Unable to create file reader for: %ls", *CertPath);
			}
		}
		else
		{
			UE_LOGF(LogDTLSHandler, Error, "ImportCertificate: Empty cert path");
		}
	}
	else
	{
		UE_LOGF(LogDTLSHandler, Error, "ImportCertificate: Cert already exists");
	}
...
bool FDTLSCertificate::GenerateCertificate(const FTimespan& Lifetime)
{
	...
	if (ResultCode == 0)
	{
		UE_LOGF(LogDTLSHandler, Error, "Failed to set serial number on cert: %d", ResultCode);
		return bSuccess;
	}
	...
	if (ResultCode == 0)
	{
		UE_LOGF(LogDTLSHandler, Error, "Failed to set issuer name");
		return bSuccess;
	}
	...
	if (ResultCode <= 0)
	{
		UE_LOGF(LogDTLSHandler, Error, "Failed to sign cert: %d", ResultCode);
		return bSuccess;
	}
...
bool FDTLSCertificate::GenerateFingerprint()
{
	...
	if (ResultCode <= 0)
	{
		UE_LOGF(LogDTLSHandler, Error, "Failed to hash cert: %d", ResultCode);
		return false;
	}
