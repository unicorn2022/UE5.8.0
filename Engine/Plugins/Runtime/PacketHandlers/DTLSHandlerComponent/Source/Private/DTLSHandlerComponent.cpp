...
void FDTLSHandlerComponent::SetEncryptionData(const FEncryptionData& EncryptionData)
{
	...
	else
	{
		...
		if (Handler->Mode == UE::Handler::Mode::Client)
		{
			...
			{
				UE_LOGF(LogDTLSHandler, Error, "SetEncryptionData: Invalid fingerprint size: got %d expected %d", EncryptionData.Fingerprint.Num(), FDTLSFingerprint::Length);
			}
		}
	}
}
...
void FDTLSHandlerComponent::EnableEncryption()
{
	...
	UE_LOGF(LogDTLSHandler, Log, "EnableEncryption: %ls", LexToString(ContextType));
	...
	if (DTLSContext.IsValid())
	{
		...
		{
			UE_LOGF(LogDTLSHandler, Error, "EnableEncryption: Failed to initialize context.");
			DTLSContext.Reset();
		}
	}
	else
	{
		UE_LOGF(LogDTLSHandler, Error, "EnableEncryption: Invalid context.");
	}
}

void FDTLSHandlerComponent::DisableEncryption()
{
	UE_LOGF(LogDTLSHandler, Log, "DisableEncryption");
...
void FDTLSHandlerComponent::Incoming(FBitReader& Packet)
{
	...
	if (Packet.ReadBit() != 0)
	{
		if (DTLSContext.IsValid())
		{
			...
			{
				UE_LOGF(LogDTLSHandler, Log, "FDTLSHandlerComponent::Incoming: invalid payload size");
				Packet.SetError();
				return;
			}
			...
			if (HandshakeBit == 1)
			{
				...
				{
					UE_LOGF(LogDTLSHandler, Warning, "Failed to write entire incoming packet to input BIO: %d / %d", BytesWritten, PayloadBytes);
					Packet.SetError();
					return;
				}
			...
			else
			{
				if (InternalState == EDTLSHandlerState::Handshaking)
				{
					UE_LOGF(LogDTLSHandler, Verbose, "Ignoring non-handshake packet while handshake is still in progress.");
				...
				else if (InternalState == EDTLSHandlerState::Encrypted)
				{
					...
					if (BytesWritten == PayloadBytes)
					{
						...
						if (BytesRead > 0)
						{
							...
							{
								UE_LOGF(LogTemp, Log, "DTLS Error");
								Packet.SetError();
							}
						}
						else 
						{
							...
							{
								UE_LOGF(LogDTLSHandler, Verbose, "Ignoring non-handshake packet due to pending handshake data.");
								Packet.SetData(nullptr, 0);
								return;
							}
							else
							{
								UE_LOGF(LogDTLSHandler, Error, "SSL_read error: %d", SSLError);
								Packet.SetError();
							}
						}
					}
					...
					{
						UE_LOGF(LogDTLSHandler, Error, "Failed to write entire incoming packet to input BIO: %d / %d", BytesWritten, PayloadBytes);
						Packet.SetError();
					}
				}
				else
				{
					UE_LOGF(LogDTLSHandler, Error, "Attempted to process packet with handler in invalid state");
					Packet.SetError();
				}
			}
		}
		...
		{
			UE_LOGF(LogDTLSHandler, Verbose, "Ignoring encrypted packet before handler has been enabled.");
			Packet.SetData(nullptr, 0);
		}
	}
}
...
void FDTLSHandlerComponent::DoHandshake()
{
	...
	else if (HandshakeResult < 0)
	{
		...
		{
			UE_LOGF(LogDTLSHandler, Error, "DoHandshake:  Handshaking failed with result: %d error: %d", HandshakeResult, SSLError);
			DTLSContext.Reset();
			return;
		}
	}
	...
	{
		UE_LOGF(LogDTLSHandler, VeryVerbose, "DoHandshake:  Timeout: %ld / %ld", TimeLeft.sec, TimeLeft.usec);

		if (TimeLeft.sec <= 0 && TimeLeft.usec <= 0)
		{
			UE_LOGF(LogDTLSHandler, Log, "DoHandshake:  Handshaking timeout occurred, retrying.");
	...
	while (Pending > 0)
	{
		if (Pending <= sizeof(TempBuffer))
		{
			...
			if (BytesRead > 0)
			{
				if (BytesRead == Pending)
				{
					...
					UE_LOGF(LogDTLSHandler, Verbose, "DoHandshake:  Sending handshake packet: %d bytes", BytesRead);
				...
				{
					DTLSContext.Reset();
					UE_LOGF(LogDTLSHandler, Error, "Unexpected value from BIO_read: Read: %d Pending: %d", BytesRead, Pending);
					return;
				}
			}
			else
			{
				UE_LOGF(LogDTLSHandler, Error, "BIO_read error: %d", BytesRead);
				return;
			}
		}
		else
		{
			DTLSContext.Reset();
			UE_LOGF(LogDTLSHandler, Error, "BIO_ctrl_pending returned value out of range: %d", Pending);
			return;
		}
...
void FDTLSHandlerComponent::TickHandshake()
{
	...
	if ((InternalState == EDTLSHandlerState::Handshaking) || bPendingHandshakeData)
	{
		...
		if (DTLSContext.IsValid() && DTLSContext->IsHandshakeComplete() && (InternalState == EDTLSHandlerState::Handshaking))
		{
			...
			UE_LOGF(LogDTLSHandler, Log, "UpdateHandshake:  Handshaking completed");
		}
	...
	if (bEncryptionEnabled)
	{
		if (DTLSContext.IsValid())
		{
			if (InternalState == EDTLSHandlerState::Encrypted)
			{
				...
				{
					UE_LOGF(LogDTLSHandler, Warning, "Failed to write entire outgoing packet to SSL: %d / %lld", BytesWritten, Packet.GetNumBytes());
					Packet.SetError();
					return;
				}
				...
				{
					UE_LOGF(LogDTLSHandler, Error, "BIO_ctrl_pending error: %d", Pending);
					Packet.SetError();
				}
			}
			else if (InternalState == EDTLSHandlerState::Handshaking)
			{
				UE_LOGF(LogDTLSHandler, Verbose, "Attempted to send packet during handshaking, dropping.");
				Packet.Reset();
				return;
			}
			else
			{
				UE_LOGF(LogDTLSHandler, Log, "Attempted to send packet while handler was in invalid state");
				Packet.SetError();
			}
		}
		else
		{
			UE_LOGF(LogDTLSHandler, Verbose, "Outgoing: Invalid DTLS context.");
			Packet.SetError();
		}
	}
	else
...
void FDTLSHandlerComponent::LogError(const TCHAR* Context, int32 Result)
{
	if (DTLSContext.IsValid())
	{
		...
		UE_LOGF(LogDTLSHandler, Error, "%ls: SSL_get_error: %d", Context, SSLErrorCode);
		...
		UE_LOGF(LogDTLSHandler, Error, "%ls: ERR_print_errors: %ls", Context, *ErrorString);
	}
}
