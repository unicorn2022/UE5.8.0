...
void FAESHandlerComponent::SetEncryptionData(const FEncryptionData& EncryptionData)
{
	...
	{
		UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::SetEncryptionKey. NewKey is not %d bytes long, ignoring.", KeySizeInBytes);
		return;
	}
...
void FAESHandlerComponent::Incoming(FBitReader& Packet)
{
	...
	if (IsValid() && Packet.GetNumBytes() > 0)
	{
		...
		if (Packet.ReadBit() != 0)
		{
			...
			if (Key.Num() == 0)
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Incoming: received encrypted packet before key was set, ignoring.");
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Incoming: missing ciphertext");
				Packet.SetError();
				return;
			}
			...
			UE_LOGF(PacketHandlerLog, VeryVerbose, "AES packet handler received %ld bytes before decryption.", Ciphertext.Num());
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Incoming: failed to decrypt packet.");
				Packet.SetError();
				return;
			}
			...
			if (LastByte != 0)
			{
				...
				UE_LOGF(PacketHandlerLog, VeryVerbose, "  Have %d bits after decryption.", BitSize);

				Packet.SetData(MoveTemp(Plaintext), BitSize);
			}
			else
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Incoming: malformed packet, last byte was 0.");
				Packet.SetError();
			}
		}
	}
...
void FAESHandlerComponent::Outgoing(FBitWriter& Packet, FOutPacketTraits& Traits)
{
	...
	if (IsValid() && Packet.GetNumBytes() > 0)
	{
		...
		{
			UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Outgoing: failed to write encryption bit.");
			Packet.SetError();
			return;
		}
		...
		{
			UE_LOGF(PacketHandlerLog, VeryVerbose, "AES packet handler sending %lld bits before encryption.", Packet.GetNumBits());
			...
			if (Packet.IsError())
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Outgoing: failed to write termination bit.");
				return;
			}
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Outgoing: failed to encrypt packet.");
				Packet.SetError();
				return;
			}
			else
			{
				...
				if (NewPacket.IsError())
				{
					UE_LOGF(PacketHandlerLog, Log, "FAESHandlerComponent::Outgoing: failed to write ciphertext to packet.");
					Packet.SetError();
					return;
				}

				UE_LOGF(PacketHandlerLog, VeryVerbose, "  AES packet handler sending %lld bytes after encryption.", NewPacket.GetNumBytes());
			}
		}
		else
		{
