...
void FAESGCMHandlerComponent::SetEncryptionData(const FEncryptionData& EncryptionData)
{
	...
	{
		UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::SetEncryptionData. NewKey is not %d bytes long, ignoring.", KeySizeInBytes);
		return;
	}
	...
	do
	{
		...
		{
			UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::SetEncryptionData: failed to generate IV.");
			return;
		}
	}
	while (OutIV[0] == 0);
...
void FAESGCMHandlerComponent::Incoming(FIncomingPacketRef PacketRef)
{
	...
	if (IsValid() && PacketBytes > 0)
	{
		...
		if (PacketData[0] != 0)
		{
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::Incoming: received encrypted packet before key was set, ignoring.");
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::Incoming: missing IV");
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::Incoming: missing auth tag");
			...
			if (PacketBytes == 0)
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::Incoming: missing ciphertext");
			...
			UE_LOGF(PacketHandlerLog, VeryVerbose, "AESGCM packet handler received %d bytes before decryption.", CipherText.Num());
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::Incoming: failed to decrypt packet.");
...
void FAESGCMHandlerComponent::Outgoing(FBitWriter& Packet, FOutPacketTraits& Traits)
{
	...
	if (IsValid() && Packet.GetNumBytes() > 0)
	{
		...
		{
			UE_LOGF(PacketHandlerLog, VeryVerbose, "AESGCM packet handler sending %lld bits before encryption.", Packet.GetNumBits());
			...
			{
				UE_LOGF(PacketHandlerLog, Log, "FAESGCMHandlerComponent::Outgoing: failed to encrypt packet.");
				Packet.SetError();
				return;
			}
...
bool FAESGCMHandlerComponent::SupportsParallelConnectionOutgoing() const
{
	return true;
}
