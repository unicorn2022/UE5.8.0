...
#if WITH_STATETREE_TRACE_DEBUGGER
	class FStateTreeDebugInfoProvider : public IStateTreeDebugInfoProvider
	{
...
UE::Trace::FStoreClient* FStateTreeModule::GetStoreClient()
{
	...
	{
		StoreClient = UE::Trace::FStoreClient::Create(TEXT("localhost"));
	}
	return StoreClient.Get();
}

TNotNull<const IStateTreeDebugInfoProvider*> FStateTreeModule::GetDebugInfoProvider()
{
	if (!DebugInfoProvider)
	{
		DebugInfoProvider = MakeShared<UE::StateTree::Debug::Private::FStateTreeDebugInfoProvider>();
	}

	return DebugInfoProvider.Get();
}

void FStateTreeModule::SetDebugInfoProvider(TSharedPtr<IStateTreeDebugInfoProvider> InDebugInfoProvider)
{
	DebugInfoProvider = InDebugInfoProvider;
}
...
bool FStateTreeModule::StartTraces(int32& OutTraceId)
{
	...
	{
		UE_LOGF(LogStateTree, Log, "StateTree traces enabled");
		UE::StateTree::Delegates::OnTracingStateChanged.Broadcast(EStateTreeTraceStatus::TracesStarted);
	}

	// Enable channels after starting the trace and notifying listeners so events can't
	// be traced (on other threads) before this point.
	// This might not be required most of the time in this case (i.e., legacy StateTreeDebugger)
	// which is happening from the game thread but this is to be consistent with the RewindDebugger extension
	// that could enable channels from a worker thread.
...
void FStateTreeModule::StopTraces()
{
	...
	if (UE::StateTree::Delegates::OnTracingStateChanged.IsBound())
	{
		UE_LOGF(LogStateTree, Log, "Stopping StateTree traces...");
	...
	if (UE::StateTree::Delegates::OnTracingStateChanged.IsBound())
	{
		UE_LOGF(LogStateTree, Log, "StateTree traces stopped");
