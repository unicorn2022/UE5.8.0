...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::SendEvent(const FGameplayTag Tag, const FConstStructView Payload, const FName Origin) const requires bWithWriteAccess
{
...
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::RequestTransition(FStateTreeStateHandle TargetState,
	EStateTreeTransitionPriority Priority, EStateTreeSelectionFallback Fallback) const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::BroadcastDelegate(const FStateTreeDelegateDispatcher& Dispatcher) const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::BindDelegate(const FStateTreeDelegateListener& Listener, FSimpleDelegate Delegate) const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::UnbindDelegate(const FStateTreeDelegateListener& Listener) const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::FinishTask(EStateTreeFinishTaskType FinishType) const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
	...
	if (ActivePath.IsValid())
	{
		...
		if (ensure(FrameStateTree))
		{
			...
			auto BroadcastDelegateHelper = [Self=this, FrameStateTree, AssetNodeIndex](const FTasksCompletionStatus::FSetStatusResult StatusResult)
				{
					// Broadcast task completion dispatcher only if status actually changed
					if (StatusResult.bChanged)
					{
						check(FrameStateTree->GetNodes().IsValidIndex(AssetNodeIndex));
						const FStateTreeTaskBase* TaskPtr = FrameStateTree->GetNode(AssetNodeIndex).GetPtr<const FStateTreeTaskBase>();
						check(TaskPtr);
						if (TaskPtr->bHasTaskCompletionDelegateDispatcher)
						{
							const FTaskCompletionDelegateDispatcherContainer Dispatchers = GetTaskCompletionDispatcher(FrameStateTree, AssetNodeIndex, StatusResult.CurrentStatus);
							for (const FStateTreeDelegateDispatcher& Dispatcher : Dispatchers)
							{
								Self->BroadcastDelegate(Dispatcher);
							}
						}
					}
				};
			...
			if (ActivePath.Frame->bIsGlobalFrame && bIsGlobalTask)
			{
				...
				const FTasksCompletionStatus::FSetStatusResult GlobalStatusResult = GlobalTasksStatus.SetTaskStatusWithPriority(FrameTaskIndex, TaskStatus);
				bCompleted = GlobalTasksStatus.IsCompleted();
				BroadcastDelegateHelper(GlobalStatusResult);
				bSucceed = true;
			}
			else if (ensure(ActivePath.StateHandle.IsValid()))
			{
				...
				if (bIsStateTask)
				{
					...
					BroadcastDelegateHelper(StateStatusResult);
					bSucceed = true;
				}
			}
...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::UpdateScheduledTickRequest(UE::StateTree::FScheduledTickHandle Handle, FStateTreeScheduledTick ScheduledTick) const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::CopyInputBindings() const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
	using namespace UE::StateTree;
	using namespace UE::StateTree::InstanceData;

	const Async::FActivePathInfo ActivePath = GetActivePathInfo();
	if (!ActivePath.IsValid())
	{
		return false;
	}
	...
	const FStateTreeDataView InstanceDataView = GetDataViewOrTemporary(*Storage, nullptr, *ActivePath.Frame, Node.InstanceDataHandle);
	if (!InstanceDataView.IsValid())
	{
		return false;
	}

	return CopyBindingBatchForAsync(ActivePath.Frame, InstanceDataView, Node.BindingsBatch);
}

template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::CopyOutputBindings() const requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
	using namespace UE::StateTree;
	using namespace UE::StateTree::InstanceData;
	...
	return CopyBindingBatchForAsync(ActivePath.Frame, InstanceDataView, Node.OutputBindingsBatch);
}
...
template <bool bWithWriteAccess>
bool TStateTreeStrongExecutionContext<bWithWriteAccess>::CopyBindingBatchForAsync(const FStateTreeExecutionFrame* CurrentFrame, const FStateTreeDataView TargetView, const FStateTreeIndex16 BindingsBatch) const requires bWithWriteAccess
{
	using namespace UE::StateTree::InstanceData;
	...
	if (CurrentFrame == nullptr || CurrentFrame->StateTree == nullptr)
	{
		return false;
	}

	const FStateTreePropertyBindings& Bindings = CurrentFrame->StateTree->GetPropertyBindings();
	const FPropertyBindingCopyInfoBatch& Batch = Bindings.GetBatch(BindingsBatch);
	check(TargetView.GetStruct() == Batch.TargetStruct.Get().Struct);

	// Can't have property function because the context doesn't exist to call FStateTreePropertyFunctionBase::Execute(FStateTreeExecutionContext& Context) const.
	if (Batch.PropertyFunctionsBegin != Batch.PropertyFunctionsEnd)
	{
		UE_LOGF(LogStateTree, Verbose, "Property functions cannot be evaluated in async context. %ls", *CurrentFrame->StateTree->GetPathName());
		return false;
	}
	...
	bSucceed = true;
	for (const FPropertyBindingCopyInfo& Copy : Bindings.GetBatchCopies(Batch))
	{
		const FStateTreeDataHandle SourceHandle = Copy.SourceDataHandle.Get<FStateTreeDataHandle>();

		// Some data source are not available.
		if (DoesRequireInstanceStorage(SourceHandle.GetSource()))
		{
			FStateTreeInstanceStorage* StoragePtr = Storage.Get();
			const FStateTreeDataView SourceView = GetDataViewOrTemporary(*StoragePtr, SharedInstanceDataStorage, *CurrentFrame, SourceHandle);
			bSucceed &= Bindings.CopyProperty(Copy, SourceView, TargetView);
		}
		else
		{
			UE_LOGF(LogStateTree, Verbose, "%ls cannot be evaluated in async context.", *SourceHandle.Describe());
			bSucceed = false;
		}
	}
...
FStateTreeDataView TStateTreeStrongExecutionContext<bWithWriteAccess>::GetInstanceDataPtrInternal() const
{
	...
	if (ActivePath.IsValid())
	{
		return InstanceData::GetDataViewOrTemporary(
			*Storage,
...
template <bool bWithWriteAccess>
FStateTreeDataView TStateTreeStrongExecutionContext<bWithWriteAccess>::GetExecutionRuntimeDataPtrInternal() const
{
	using namespace UE::StateTree;
	...
	if (ActivePath.IsValid())
	{
		const FStateTreeDataHandle DataHandle = FStateTreeDataHandle(EStateTreeDataSourceType::ExecutionRuntimeDataAny, ActivePath.GetNode().ExecutionRuntimeTemplateIndex.Get());
		return InstanceData::GetDataViewOrTemporary(
			*Storage,
			/* SharedInstanceStorage */ nullptr,
			*ActivePath.Frame,
...
template <bool bWithWriteAccess>
void TStateTreeStrongExecutionContext<bWithWriteAccess>::ScheduleNextTick(TNotNull<UObject*> Owner, TNotNull<const UStateTree*> RootStateTree, FStateTreeInstanceStorage& Storage, UE::StateTree::ETickReason Reason) requires bWithWriteAccess
{
	static_assert(bWithWriteAccess);
