...
#if WITH_EDITOR
// 0 default
// 1 with DefaultGlobalParameters
const int32 UStateTree::LatestCustomAssetSavedVersion = 1;
#endif
...
bool UStateTree::IsReadyToRun() const
{
	...
	{
		// The asset can be in the compiler queue. Compile the asset sync.
		const_cast<UStateTree*>(this)->CompileIfChanged();
	}
#endif

	// Valid tree must have at least one state and valid instance data.
	return States.Num() > 0 && CompileStatus == ECompileStatus::Executable && PropertyBindings.IsValid();
}
...
FStateTreeStateHandle UStateTree::GetStateHandleFromGameplayTag(const FGameplayTag Tag, EStateGameplayTagQueryMethod QueryMethod) const
{
	TRingBuffer<uint16, TInlineAllocator<32>> HandleBFSQueue;

	auto AddRootStatesIndicesToQueue = [Self = this, &HandleBFSQueue]()
	{
		uint16 RootStateIndex = 0;
		while (RootStateIndex < Self->States.Num())
		{
			HandleBFSQueue.Add(RootStateIndex);
			RootStateIndex = Self->States[RootStateIndex].GetNextSibling();
		}
	};

	auto TagMatches = [TagToQuery = Tag, QueryMethod](const FGameplayTag StateTag)
		{
			if (QueryMethod == EStateGameplayTagQueryMethod::Includes)
			{
				return StateTag.MatchesTag(TagToQuery);
			}

			if (QueryMethod == EStateGameplayTagQueryMethod::MatchesExact)
			{
				return StateTag.MatchesTagExact(TagToQuery);
			}

			return false;
		};

	AddRootStatesIndicesToQueue();

	while (!HandleBFSQueue.IsEmpty())
	{
		const uint16 CurrentStateHandleIndex = HandleBFSQueue.PopFrontValue();
		const FCompactStateTreeState& CurrentState = States[CurrentStateHandleIndex];

		if (TagMatches(CurrentState.Tag))
		{
			return FStateTreeStateHandle(CurrentStateHandleIndex);
		}
		
		uint16 CurrentStateNextLevelChildIndex = CurrentState.ChildrenBegin;
		while (CurrentStateNextLevelChildIndex != CurrentState.ChildrenEnd)
		{
			HandleBFSQueue.Add(CurrentStateNextLevelChildIndex);
			CurrentStateNextLevelChildIndex = States[CurrentStateNextLevelChildIndex].GetNextSibling();
		}
	}
...
{
	if (SharedInstanceData.Num() == 0)
	{
		return TSharedPtr<FStateTreeInstanceData>();
	}
...
{
	void RenameObjectToTransientPackage(TNotNull<UObject*> ObjectToRename, FStringView Name)
	{
		...
		const FName UniqueName = MakeUniqueObjectName(GetTransientPackage(), ObjectToRename->GetClass(), FName(Name));
		...
		ObjectToRename->Rename(*UniqueName.ToString(), GetTransientPackage(), RenFlags);
	}
}
...
void UStateTree::ResetCompiled()
{
	...
	TaskCompletionDispatchers.Reset();
	Extensions.Reset();
	Parameters.Reset();
	...
	CompileStatus = ECompileStatus::Public;
	EditorDataDirtyStatus = EDirtyStatus::Public;
	LastCompiledEditorDataHash = 0;
...
bool UStateTree::IsEditorDataDirty() const
{
	return EditorDataDirtyStatus != EDirtyStatus::None;
}

void UStateTree::MarkAsModified(const bool bPubliclyModified)
{
	const EDirtyStatus PreviousDirtyStatus = EditorDataDirtyStatus;
	if (bPubliclyModified)
	{
		EditorDataDirtyStatus = EDirtyStatus::Public;
	}
	else if (EditorDataDirtyStatus != EDirtyStatus::Public)
	{
		EditorDataDirtyStatus = EDirtyStatus::Internal;
	}

	if (PreviousDirtyStatus != EditorDataDirtyStatus)
	{
		UE::StateTree::Delegates::Private::OnStateTreeMarkedAsModified.ExecuteIfBound(this);
	}
}

void UStateTree::PreSave(FObjectPreSaveContext Context)
{
	const bool bIsUserObject = !HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject);
	const bool bIsEditorAutoSave = Context.IsFromAutoSave();
	if (bIsUserObject && !bIsEditorAutoSave)
	{
		CustomAssetSavedVersion = LatestCustomAssetSavedVersion;
	}
...
{
	UE::StateTree::Delegates::Private::OnRequestAssetRegistryTags.ExecuteIfBound(this, Context);
	Super::GetAssetRegistryTags(Context);
}

void UStateTree::BeginCacheForCookedPlatformData(const ITargetPlatform* TargetPlatform)
{
	UE::StateTree::Delegates::Private::OnPreCookStateTreeAsset.ExecuteIfBound(this);
...
void UStateTree::PostLoad()
{
	...
	// Request a link
	CompileStatus = ECompileStatus::Link;
	...
	// Request a compilation (can be async or sync).
	UE::StateTree::Delegates::Private::OnStateTreeAssetLoaded.ExecuteIfBound(this);

	// The OnStateTreeAssetLoaded can trigger a compilation. That would also trigger a Link()
	// Only Link() if no compilation occurs.
	// LastCompiledEditorDataHash is only set if a compilation is valid. A valid compilation set CompileStatus to Executable.
	if (CompileStatus == ECompileStatus::Link && LastCompiledEditorDataHash != 0)
	{
		if (!Link())
		{
			UE_CLOGF(CompileStatus == ECompileStatus::Link, LogStateTree, Warning, "%ls failed to link. Asset will not be usable at runtime.", *GetPathName());
		}
	}
}
...
void UStateTree::AppendToClassSchema(FAppendToClassSchemaContext& Context)
{
	Super::AppendToClassSchema(Context);
	UE::StateTree::Delegates::Private::OnAppendToClassSchema.ExecuteIfBound(Context);
}
#endif
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (Record.GetUnderlyingArchive().IsModifyingWeakAndStrongReferences())
	{
		...
		{
			UE_LOGF(LogStateTree, Log, "%ls failed to link. Asset will not be usable at runtime.", *GetName());	
		}
	}
}
...
{
	if (CompileStatus == ECompileStatus::Executable)
	{
		CompileStatus = ECompileStatus::Link;
	}
#if WITH_EDITOR
	if (EditorDataDirtyStatus == EDirtyStatus::None)
	{
		EditorDataDirtyStatus = EDirtyStatus::Link;
	}
#endif
			...
			auto TestInstanceData = [this, Node, Index, &bResult](const UStruct* CurrentInstanceDataType, const UStruct* DesiredInstanceDataType)
				{
					...
					{
						UE_LOGF(LogStateTree, Error, "%ls: node (%d) '%ls' with name '%ls' failed. Missing instance value, possibly due to Blueprint class or C++ class/struct template deletion."
							, *GetPathName()
							, Index
					...
					if (HasNewerVersionExists(CurrentInstanceDataType))
					{
						...
						if (bLogError)
						{
							UE_LOGF(LogStateTree, Error, "%ls: node '%ls' failed. The source Instance Data type '%ls' has a newer version.", *GetPathName(), *WriteToString<64>(Node->StaticStruct()->GetFName()), *WriteToString<64>(CurrentInstanceDataType->GetFName()));
						}
...

					{
							...
							&& !bDesiredHasNewerVersion)
						{
							...
							if (bLogError)
							{
								UE_LOGF(LogStateTree, Error, "%ls: node '%ls' failed. The source Instance Data type '%ls' does not match '%ls'", *GetPathName(), *WriteToString<64>(Node->StaticStruct()->GetFName()), *GetNameSafe(CurrentInstanceDataType), *GetNameSafe(DesiredInstanceDataType));
							}
							bResult = false;
						}
					}
...
{
	// Make sure the StateTreeCompilationManager is aware that the asset fails to compile, so that it can retry compilation when necessary (e.g. when PIE starts).
	auto Failure = [this]()
		{
#if WITH_EDITOR
			constexpr bool bPubliclyModified = false;
			MarkAsModified(bPubliclyModified);
#endif //WITH_EDITOR
			return false;
		};

	if (CompileStatus == ECompileStatus::Public || CompileStatus == ECompileStatus::Internal)
	{
		return Failure();
	}
	...
	if (States.Num() > 0 && Nodes.Num() > 0)
	{
		...
		for (FConstStructView Node : Nodes)
		{
			if (!Node.IsValid())
			{
				UE_LOGF(LogStateTree, Error, "%ls: State Tree asset was not properly loaded (missing node). See log for loading failures, or recompile the StateTree asset.", *GetPathName());
				return Failure();
			}
		}
	}
	...
	// Resolves nodes references to other StateTree data
	{
		...
		for (int32 Index = 0; Index < Nodes.Num(); Index++)
		{
			...
			if (ensure(NodePtr))
			{
				...
				{
					UE_LOGF(LogStateTree, Error, "%ls: node '%ls' failed to resolve its references.", *GetPathName(), *NodePtr->StaticStruct()->GetName());
					return Failure();
				}
			}
		}
		...
		if (Schema)
		{
			...
			{
				UE_LOGF(LogStateTree, Error, "%ls: schema failed to resolve its references.", *GetPathName());
				return Failure();
			}
		}
		...
		for (UStateTreeExtension* Extension : Extensions)
		{
			if (Extension)
			{
				...
				if (!bLinkSucceeded || Linker.GetStatus() == EStateTreeLinkerStatus::Failed)
				{
					UE_LOGF(LogStateTree, Error, "%ls: extension failed to resolve its references.", *GetPathName());
					return Failure();
				}
			}
		}
	...
	{
		UE_LOGF(LogStateTree, Error, "%ls: State Tree asset was not properly loaded (missing instance data). See log for loading failures, or recompile the StateTree asset.", *GetPathName());
		return Failure();
	}

	if (!SharedInstanceData.AreAllInstancesValid())
	{
		UE_LOGF(LogStateTree, Error, "%ls: State Tree asset was not properly loaded (missing shared instance data). See log for loading failures, or recompile the StateTree asset.", *GetPathName());
		return Failure();
	}

	if (!DefaultEvaluationScopeInstanceData.AreAllInstancesValid())
	{
		UE_LOGF(LogStateTree, Error, "%ls: State Tree asset was not properly loaded (missing evaluation scope instance data). See log for loading failures, or recompile the StateTree asset.", *GetPathName());
		return Failure();
	}

	if (!GetDefaultExecutionRuntimeData().AreAllInstancesValid())
	{
		UE_LOGF(LogStateTree, Error, "%ls: State Tree asset was not properly loaded (missing execution runtime data). See log for loading failures, or recompile the StateTree asset.", *GetPathName());
		return Failure();
	}
	...
	CompileStatus = ECompileStatus::Executable;
#if WITH_EDITOR
	if (EditorDataDirtyStatus == EDirtyStatus::Link)
	{
		EditorDataDirtyStatus = EDirtyStatus::None;
	}
#endif
...
bool UStateTree::PatchBindings()
{
	...
	for (const FCompactStateTreeState& State : States)
	{
			...
			|| State.Type == EStateTreeStateType::LinkedAsset)
		{
			...
			{
				UE_LOGF(LogStateTree, Error, "%ls: Data for state '%ls' is malformed. Please recompile the StateTree asset.", *GetPathName(), *State.Name.ToString());
				return false;
			}
		}
	...
	for (const FCompactStateTreeState& State : States)
	{
		if (State.Type == EStateTreeStateType::Linked && State.LinkedState.IsValid())
		{
			...
			if (LinkedStateParams.GetValue().GetScriptStruct() != Params.GetValue().GetScriptStruct())
			{
				UE_LOGF(LogStateTree, Error, "%ls: The parameters on state '%ls' does not match the linked state parameters in state '%ls'. Please recompile the StateTree asset.", *GetPathName(), *State.Name.ToString(), *LinkedState.Name.ToString());
				return false;
			}
		}
		else if (State.Type == EStateTreeStateType::LinkedAsset && State.LinkedAsset)
		{
			...
			const FCompactStateTreeParameters& Params = DefaultInstanceData.GetMutableStruct(State.ParameterTemplateIndex.Get()).Get<FCompactStateTreeParameters>();
			const bool bIsValid = TargetTreeParameters.GetPropertyBagStruct() == Params.GetValue().GetScriptStruct();
			if (!bIsValid)
			{
				UE_LOGF(LogStateTree, Error, "%ls: The parameters on state '%ls' does not match the linked asset parameters '%ls'. Please recompile the StateTree asset.",
	...
	for (FCompactStateTreeState& State : States)
	{
		if (State.ParameterDataHandle.IsValid())
		{
			...
			DataViews.Add(State.ParameterDataHandle, Params.GetMutableValue());
			if (State.ParameterBindingsBatch.IsValid())
			{
				BindingBatchDataView.Add(State.ParameterBindingsBatch, Params.GetMutableValue());
			}
		}
	}
	...
	for (int32 BatchIndex = 0; BatchIndex < CopyBatches.Num(); ++BatchIndex)
	{
		...
		{
			UE_LOGF(LogStateTree, Error, "%s: '%ls' Invalid target struct when trying to bind to '%ls'"
				, __FUNCTION__
				, *GetPathName()
		...
		for (int32 Index = Batch.BindingsBegin.Get(); Index != Batch.BindingsEnd.Get(); Index++)
		{
			...
			if(!bIsSourceEvent)
			{
				...
				{
					UE_LOGF(LogStateTree, Error, "%s: '%ls' Failed to update source instance structs for property binding '%ls'. Reason: %ls"
						, __FUNCTION__
						, *GetPathName()
			...
			{
				UE_LOGF(LogStateTree, Error, "%s: '%ls' Failed to update target instance structs for property binding '%ls'. Reason: %ls"
					, __FUNCTION__
					, *GetPathName()
					, *Binding.GetTargetPath().ToString()
					, *ErrorMsg);
...
{
	UE::StateTree::Delegates::Private::OnCompileIfChanged.ExecuteIfBound(this);
}
#endif //WITH_EDITOR
...
FString UStateTree::DebugInternalLayoutAsString() const
{
	...
	DebugString.Appendf(TEXT("\nTask Completion Dispatchers(%d)\n"), TaskCompletionDispatchers.Num());
	if (TaskCompletionDispatchers.Num())
	{
		DebugString.Appendf(TEXT("  [ (Idx) | %15s | %-30s ]\n"),
			TEXT("Task Node Idx"), TEXT("Condition"));
		for (int32 Index = 0; Index < TaskCompletionDispatchers.Num(); ++Index)
		{
			const UE::StateTree::FTaskCompletionDispatcher& Dispatcher = TaskCompletionDispatchers[Index];
			DebugString.Appendf(TEXT("  | (%3d) | %15d | %-30s |\n"),
				Index,
				Dispatcher.TaskNodeIndex.Get(),
				*UEnum::GetDisplayValueAsText(Dispatcher.Condition).ToString());
		}
	}
	...
	if (EvaluatorsNum)
	{
		DebugString.Appendf(TEXT("  [ (Idx) | %-30s | %12s | %12s | %14s ]\n"),
			TEXT("Name"), TEXT("In Bindings"), TEXT("Out Binding"), TEXT("Struct Idx"));
		for (int32 EvalIndex = EvaluatorsBegin; EvalIndex < (EvaluatorsBegin + EvaluatorsNum); EvalIndex++)
		{
			const FStateTreeEvaluatorBase& Eval = Nodes[EvalIndex].Get<const FStateTreeEvaluatorBase>();
			DebugString.Appendf(TEXT("  | (%3d) | %-30s | %12d | %12d | %14s |\n"),
				EvalIndex,
				*Eval.Name.ToString(),
				Eval.BindingsBatch.Get(),
				Eval.OutputBindingsBatch.Get(),
	...
	DebugString.Appendf(TEXT("\nTasks(%d)\n  [ (Idx) | %-30s | %-30s | %12s | %12s | %14s ]\n"),
		NumberOfTasks, TEXT("State"), TEXT("Name"), TEXT("In Bindings"), TEXT("Out Bindings"), TEXT("Struct Idx"));
	for (const FCompactStateTreeState& State : States)
	{
		if (State.TasksNum)
		{
			...
			{
				const FStateTreeTaskBase& Task = Nodes[TaskIndex].Get<const FStateTreeTaskBase>();
				DebugString.Appendf(TEXT("  | (%3d) | %-30s | %-30s | %12d | %12d | %14s |\n"),
					TaskIndex,
					*State.Name.ToString(),
					*Task.Name.ToString(),
					Task.BindingsBatch.Get(),
					Task.OutputBindingsBatch.Get(),
	...
	{
		const FStateTreeTaskBase& Task = Nodes[TaskIndex].Get<const FStateTreeTaskBase>();
		DebugString.Appendf(TEXT("  | (%3d) | %-30s | %-30s | %8d | %12d | %14s |\n"),
			TaskIndex,
			TEXT("Global"),
			*Task.Name.ToString(),
			Task.BindingsBatch.Get(),
