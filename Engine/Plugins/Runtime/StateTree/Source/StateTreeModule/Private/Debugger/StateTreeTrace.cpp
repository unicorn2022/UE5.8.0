...
UE_TRACE_CHANNEL_DEFINE(StateTreeDebugChannel, "Traces StateTree state machine execution for debugging including world timestamps, asset IDs, instance \
events, frame events, phase transitions, log messages, state changes, task execution, evaluator execution, transition \
conditions, condition evaluation, and active state tracking with deferred buffering support.")
...
struct FBufferedDataList
{
	...
	/** Initializes recording world time to 0 if not initialized. */
	void InitializeWorldTimeIfNeeded_AnyThread();
	...
	{
		checkf(IsInGameThread(), TEXT("Expecting to only be called on the main thread before worker threads trace events."));
		{
...
private:
	...
	std::atomic<double> RecordingWorldTime = UNINITIALIZED_WORLD_TIME;
...
{
	InitializeWorldTimeIfNeeded_AnyThread();
	...
	if (TracedRecordingWorldTime.compare_exchange_strong(Expected, RecordingWorldTime))
	{
		UE_TRACE_LOG(StateTreeDebugger, WorldTimestampEvent, StateTreeDebugChannel)
			<< WorldTimestampEvent.WorldTime(RecordingWorldTime.load());
	}
}
...
void FBufferedDataList::InitializeWorldTimeIfNeeded_AnyThread()
{
	constexpr double InitialValue = 0;
	double Expected = UNINITIALIZED_WORLD_TIME;
	RecordingWorldTime.compare_exchange_strong(Expected, InitialValue);
}
...
void RegisterGlobalDelegates()
{
	...
	GTracingStateChangedDelegateHandle = UE::StateTree::Delegates::OnTracingStateChanged.AddLambda([](const EStateTreeTraceStatus TraceStatus)
	{
		...
		{
			GBufferedEvents.Reset_GameThread();
		}
	});
}
	...
	const EStateTreeTraceEventType EventType)
{
	...
	if (UE_TRACE_CHANNELEXPR_IS_ENABLED(StateTreeDebugChannel))
	{
		...
		else if (EventType == EStateTreeTraceEventType::Pop)
		{
			...
			// Emit the StateTree-specific InstanceEvent so the trace provider
			// sets Lifetime upper bound (making IsInstanceActive() return false).
			UE_TRACE_LOG(StateTreeDebugger, InstanceEvent, StateTreeDebugChannel)
				<< InstanceEvent.Cycle(FPlatformTime::Cycles64())
				<< InstanceEvent.InstanceId(InstanceId.Id)
				<< InstanceEvent.InstanceSerial(InstanceId.SerialNumber)
				<< InstanceEvent.EventType(static_cast<std::underlying_type_t<EStateTreeTraceEventType>>(EStateTreeTraceEventType::Pop));

			TRACE_INSTANCE_LIFETIME_END(Owner, InstanceId.ToUint64());
		}
	}
}
