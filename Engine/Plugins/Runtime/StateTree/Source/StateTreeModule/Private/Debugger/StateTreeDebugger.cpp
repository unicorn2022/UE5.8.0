...
void FStateTreeDebugger::Tick(const float DeltaTime)
{
	...
	// QOL: try to select an instance by default
	if (GetSelectedInstanceId().IsInvalid() && IsAnalysisSessionActive())
	{
		TArray<const TSharedRef<const UE::StateTreeDebugger::FInstanceDescriptor>> InstanceDescriptors;
		GetSessionInstanceDescriptors(InstanceDescriptors);

		for (const TSharedRef<const UE::StateTreeDebugger::FInstanceDescriptor>& InstanceDescriptor : InstanceDescriptors)
		{
			const UE::StateTreeDebugger::FInstanceDescriptor& Descriptor = InstanceDescriptor.Get();
			// Add instance to the list if the asset associated to the view is the instance's main asset, or one of its linked assets.
			if (Descriptor.StateTree != GetAsset()
				&& !Descriptor.LinkedStateTrees.Contains(GetAsset()))
			{
				continue;
			}
			SelectInstance(Descriptor.Id);
		}
	}
...
void FStateTreeDebugger::StopSessionAnalysis()
{
	...
	if (IsAnalysisSessionActive())
	{
		...
		// We are not responsible of the external analysis so we simply release it, otherwise we stop it.
		if (bIsExternalSessionAnalysis)
		{
	...
	CurrentFrameStateTreeAsset.Reset();
	bIsExternalSessionAnalysis = false;
...
void FStateTreeDebugger::SelectInstance(const FStateTreeInstanceDebugId InstanceId)
{
	if (SelectedInstanceId != InstanceId)
	{
		...
		// The debugger can be associated to a StateTree asset used as a linked StateTree in active instances.
		// Those active instances are available for the user to select and in that case we need to fetch the main
		// StateTree (i.e., associated to the runtime instance) to read the proper event timeline.
		InstanceRootAsset = StateTreeAsset;
		if (const TSharedPtr<const UE::StateTreeDebugger::FInstanceDescriptor> FoundDescriptor = GetDescriptor(SelectedInstanceId))
		{
			if (const UStateTree* FoundStateTree = FoundDescriptor->StateTree.Get())
			{
				InstanceRootAsset = FoundStateTree;
			}
		}
...
{
	// RewindDebugger manages its session analysis without using the unreal insights module
	//  so we need to use its analysis session first.
	if (const IRewindDebugger* RewindDebugger = IRewindDebugger::Instance())
	{
		if (const TSharedPtr<const TraceServices::IAnalysisSession> ExternalSession = RewindDebugger->GetAnalysisSessionAsShared())
		{
			AnalysisSession = ExternalSession;
			bIsExternalSessionAnalysis = true;
	...
	UE_CLOGF(RetryLoadNextLiveSessionTimer > 0, LogStateTree, Log, "Unable to start analysis for the most recent live session.");
...
void FStateTreeDebugger::ReadTrace(const uint64 FrameIndex)
{
	if (const TraceServices::IAnalysisSession* Session = GetAnalysisSession())
	{
		// UE::StateTreeDebugger::ReadTrace also uses the FrameProvider so we scope the lock for the whole operation
		TraceServices::FAnalysisSessionReadScope SessionReadScope(*Session);
		...
		if (const TraceServices::FFrame* TargetFrame = FrameProvider.GetFrame(TraceFrameType_Game, FrameIndex))
		{
			UE::StateTreeDebugger::ReadTrace(*Session
				...
				, *TargetFrame
				, /*ITraceReader*/this
				, UE::StateTreeDebugger::FTraceFilter(InstanceRootAsset.Get())
				, LastTraceReadTime);
		}
	}
...
{
	// Keep track of the StateTree associated to the last frame update so breakpoints are tested against the right asset.
	if (const FStateTreeTraceInstanceFrameEvent* FrameEvent = Event.TryGet<FStateTreeTraceInstanceFrameEvent>())
	{
		CurrentFrameStateTreeAsset = FrameEvent->WeakStateTree;
	}

	if (StateTreeAsset == nullptr // asset is required to properly match state handles
		|| CurrentFrameStateTreeAsset != StateTreeAsset // ignore events from a different asset (required for linked statetree)
	...
	TNotNull<ITraceReader*> TraceReader)
{
	...
	TimelineData.EnumerateEvents(InStartTime, InEndTime,
		[&InFrameProvider, &Frame, EventCollection, TraceReader, &Session](const double EventStartTime, const double EventEndTime, uint32 InDepth, const FStateTreeTraceEventVariantType& Event)
		{
...
void ReadTrace(
	...
	double& OutLastTraceReadTime)
{
	bool bValidLastReadFrame;
	TraceServices::FFrame LastReadFrame;
	{
		TraceServices::FAnalysisSessionReadScope SessionReadScope(Session);
		bValidLastReadFrame = FrameProvider.GetFrameFromTime(TraceFrameType_Game, OutLastTraceReadTime, LastReadFrame);
	}

	// Last time is invalid (first time processing the frame, or the buffer was cleared), and it's a different frame (scrubbing forward).
	if (!bValidLastReadFrame || Frame.Index > LastReadFrame.Index)
	{
		if (const IStateTreeTraceProvider* Provider = Session.ReadProvider<IStateTreeTraceProvider>(FStateTreeTraceProvider::ProviderName))
		{
			TraceServices::FProviderReadScopeLock ProviderReadScope(*Provider);
			if (Filter.Instance.IsValid())
			{
				Provider->ReadTimelines(Filter.Instance,
					[StartTime = OutLastTraceReadTime, EndTime = Frame.EndTime, &FrameProvider, TraceReader, &Session](const FStateTreeInstanceDebugId InstanceId, const IStateTreeTraceProvider::FEventsTimeline& TimelineData)
					{
						AddEvents(StartTime, EndTime, Session, FrameProvider, InstanceId, TimelineData, TraceReader);
					});
			}
