	...
	case RouteId_AssetDebugId:
		{
			...
			if (const UStateTree* StateTree = WeakStateTree.Get())
			{
				...
				{
					UE_LOGF(LogStateTree, Warning, "Traces are not using the same StateTree asset version as the current asset.");
				}
			}
			else
			{
				UE_LOGF(LogStateTree, Warning, "Unable to find StateTree asset: %ls : %ls", *ObjectPathName, *ObjectName);
			}
			break;
		}
	...
	case RouteId_InstanceFrame:
		{
			...
			{
				UE_LOGF(LogStateTree, Error, "Instance frame event refers to an asset Id that wasn't added previously.");
			}
