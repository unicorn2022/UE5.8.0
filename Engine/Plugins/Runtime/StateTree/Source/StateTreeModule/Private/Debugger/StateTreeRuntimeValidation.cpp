...
FRuntimeValidationInstanceData::~FRuntimeValidationInstanceData()
{
	if (Private::bRuntimeValidationEnterExitState && UObjectInitialized() && !IsEngineExitRequested())
	{
		if (NodeStates.Num() > 0)
		{
			const FNodeStatePair& Pair = NodeStates[0];
...
void FRuntimeValidationInstanceData::SetContext(TNotNull<const UObject*> InNewOwner, TNotNull<const UStateTree*> InNewStateTree, const bool bInInstanceDataWriteAccessAcquired)
{
	...
	if (Private::bRuntimeValidationContext)
	{
		if (StateTree.IsValid() && StateTree != NewStateTree && bInstanceDataWriteAccessAcquired)
		{
			ensureAlwaysMsgf(false, TEXT("StateTree runtime check failed: The StateTree '%s' is different from the previously set '%s'.\n"
				...
				, *InNewStateTree->GetFullName(), *StateTree.Get()->GetFullName());
		...
		if (Owner.IsValid() && Owner != NewOwner)
		{
			ensureAlwaysMsgf(false, TEXT("StateTree runtime check failed: The owner '%s' is different from the previously set '%s'.\n"
				...
				"Auto deactivate Runtime check StateTree.RuntimeValidation.Context to prevent reporting the same error multiple times.")
				, *InNewOwner->GetFullName(), *Owner.Get()->GetFullName());
			Private::bRuntimeValidationContext = false;
		}
	}
	...
	if (!bMaintainNodeStates.IsSet())
	{
		bMaintainNodeStates = Private::bRuntimeValidationEnterExitState;
	}
}

void FRuntimeValidationInstanceData::NodeEnterState(const FGuid& NodeID, FActiveFrameID FrameID)
{
	if (bMaintainNodeStates.Get(false))
	{
		...
		if (Found)
		{
			if (Private::bRuntimeValidationEnterExitState)
			{
		...
		{
			NodeStates.Add(FNodeStatePair{ .NodeID = NodeID, .FrameID = FrameID });
		}
	}
}

void FRuntimeValidationInstanceData::NodeExitState(const FGuid& NodeID, FActiveFrameID FrameID)
{
	if (bMaintainNodeStates.Get(false))
	{
		const int32 FoundIndex = NodeStates.IndexOfByPredicate([&NodeID, &FrameID](const FNodeStatePair& Other)
			{
				return Other.NodeID == NodeID && Other.FrameID == FrameID;
			});
		if (FoundIndex != INDEX_NONE)
		{
			NodeStates.RemoveAtSwap(FoundIndex);
		}
...
void FRuntimeValidationInstanceData::ValidateTreeNodes(TNotNull<const UStateTree*> InNewStateTree) const
{
	if (Private::bRuntimeValidationDoesNewerVersionExists)
	{
		if (InNewStateTree->IsReadyToRun())
		{
...
void FRuntimeValidationInstanceData::ValidateInstanceData(TNotNull<const UStateTree*> NewStateTree)
{
...
void FRuntimeValidation::SetContext(TNotNull<const UObject*> Owner, TNotNull<const UStateTree*> StateTree, bool bInstanceDataWriteAccessAcquired) const
{
...
void FRuntimeValidation::SetContext(TNotNull<const UObject*>, TNotNull<const UStateTree*>, bool) const
{
}
