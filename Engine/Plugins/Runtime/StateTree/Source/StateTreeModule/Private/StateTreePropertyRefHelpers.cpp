...
namespace UE::StateTree::PropertyRefHelpers
{
	...
	static const FLazyName NameName = "FName";
	static const FLazyName StringName = "FString";
	static const FLazyName TextName = "FText";
	...
	bool IsNativePropertyRefCompatibleWithProperty(const FProperty& RefProperty, const FProperty& SourceProperty)
	{
		...
		for (const FString& TargetTypeNameStr : TargetTypes)
		{
			...
			else
			{
				...
				{
					// Only referencing struct of the same exact type should be allowed. Otherwise one could e.g assign FVector to FMyVector:FVector property through reference missing some properties.
					if (SourceStructProperty->Struct == TargetRefField)
					{
						return true;
					}
				}
				...
				if (const FObjectProperty* ObjectProperty = CastField<FObjectProperty>(TestProperty))
				{
					...
					if(ObjectProperty->PropertyClass == TargetRefField)
					{
						return true;
				...
				else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(TestProperty))
				{
					if (ByteProperty->Enum == TargetRefField)
					{
						return true;
					}
				}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	FEdGraphPinType GetBlueprintPropertyRefInternalTypeAsPin(const FStateTreeBlueprintPropertyRef& PropertyRef)
	{
		...
		case EStateTreePropertyRefType::Enum:
			...
			{
				UE_LOGF(LogStateTree, Warning, "The property ref of type enum has an invalid enum. %ls", *GetFullNameSafe(PinType.PinSubCategoryObject.Get()));
			}
			break;
