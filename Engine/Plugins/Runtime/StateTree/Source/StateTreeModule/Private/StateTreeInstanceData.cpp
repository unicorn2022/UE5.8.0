...
namespace UE::StateTree::InstanceData::Private
{
	...
	TNotNull<UObject*> CopyNodeInstance(TNotNull<UObject*> Instance, TNotNull<UObject*> InOwner, bool bDuplicate)
	{
		...
		if (InstanceClass->HasAnyClassFlags(CLASS_NewerVersionExists))
		{
			...
			UE_LOGF(LogStateTree, Display, "FStateTreeInstanceData: Duplicating '%ls' with old class '%ls' as '%ls', potential data loss. Please resave State Tree asset %ls.",
...
namespace UE::StateTree::InstanceData::Private
{
		...
		const FStateTreeDataHandle& Handle)
	{
		...
		case EStateTreeDataSourceType::ExecutionRuntimeDataAny:
		{
			const int32 ActiveIndex = CurrentFrame.ExecutionRuntimeIndexBase.Get() + Handle.GetIndex();
			return Handle.HasValidIndex()
				&& CurrentFrame.ExecutionRuntimeIndexBase.IsValid() 
				&& InstanceStorage.GetExecutionRuntimeData().IsValidIndex(ActiveIndex);
		}

		case EStateTreeDataSourceType::GlobalParameterData:
		{
			// Defined as State Parameter in one of its parent frame
			if (CurrentFrame.GlobalParameterDataHandle.HasValidIndex())
			{
				check(CurrentFrame.GlobalParameterDataHandle.GetSource() == EStateTreeDataSourceType::StateParameterData);

				if (const FStateTreeExecutionFrame* ActiveGlobalParameterFrame = InstanceStorage.GetExecutionState().FindActiveFrame(CurrentFrame.GlobalParameterDataFrameID))
				{
					check(ActiveGlobalParameterFrame->ActiveInstanceIndexBase == CurrentFrame.GlobalParameterIndexBase);

					return IsActiveInstanceHandleSourceValid(InstanceStorage, *ActiveGlobalParameterFrame, CurrentFrame.GlobalParameterDataHandle);
				}

				// Parent Frame is temporary, the state parameter can't be active.
				return false;
			}

			// Root Global Parameter should always be valid.
			check(CurrentFrame.GlobalParameterDataHandle.GetSource() == EStateTreeDataSourceType::GlobalParameterData);
			return true;
		}

		case EStateTreeDataSourceType::SubtreeParameterData:
		{
			if (const FStateTreeExecutionFrame* ActiveParameterFrame = InstanceStorage.GetExecutionState().FindActiveFrame(CurrentFrame.StateParameterDataFrameID))
			{
				check(ActiveParameterFrame->ActiveInstanceIndexBase == CurrentFrame.StateParameterIndexBase);

				return IsActiveInstanceHandleSourceValid(InstanceStorage, *ActiveParameterFrame, CurrentFrame.StateParameterDataHandle);
			}

			// Frame hosting the parameter is not active
			return false;
		}
		...
		const FStateTreeDataHandle& Handle)
	{
		...
		{
		case EStateTreeDataSourceType::GlobalInstanceData:
			return InstanceStorage.GetMutableTemporaryStruct(CurrentFrame.GlobalInstanceDataFrameID, Handle);
		case EStateTreeDataSourceType::ActiveInstanceData:
			return InstanceStorage.GetMutableTemporaryStruct(CurrentFrame.FrameID, Handle);
			
		case EStateTreeDataSourceType::GlobalInstanceDataObject:
			return InstanceStorage.GetMutableTemporaryObject(CurrentFrame.GlobalInstanceDataFrameID, Handle);
		case EStateTreeDataSourceType::ActiveInstanceDataObject:
			return InstanceStorage.GetMutableTemporaryObject(CurrentFrame.FrameID, Handle);
		...
		case EStateTreeDataSourceType::GlobalParameterData:
		{
			...
			if (CurrentFrame.GlobalParameterDataHandle.HasValidIndex())
			{
				...
				if (FCompactStateTreeParameters* Params = InstanceStorage.GetMutableTemporaryStruct(CurrentFrame.GlobalParameterDataFrameID, CurrentFrame.GlobalParameterDataHandle).GetPtr<FCompactStateTreeParameters>())
				{
					return Params->GetMutableValue();
				}
			}

			// Root global parameter is never stored in temporary
			break;
		}

		case EStateTreeDataSourceType::SubtreeParameterData:
		{
			check(Handle == CurrentFrame.StateTree->GetStateFromHandle(CurrentFrame.RootState)->ParameterDataHandle);

			// A subtree could be either linked or standalone.
			check((CurrentFrame.StateParameterDataHandle.GetSource() == EStateTreeDataSourceType::SubtreeParameterData 
					&& CurrentFrame.FrameID == CurrentFrame.StateParameterDataFrameID
					&& CurrentFrame.StateParameterDataHandle == Handle)
				|| (CurrentFrame.StateParameterDataHandle.GetSource() == EStateTreeDataSourceType::StateParameterData 
					&& CurrentFrame.FrameID != CurrentFrame.StateParameterDataFrameID));

			if (FCompactStateTreeParameters* Params = InstanceStorage.GetMutableTemporaryStruct(CurrentFrame.StateParameterDataFrameID, CurrentFrame.StateParameterDataHandle).GetPtr<FCompactStateTreeParameters>())
			{
				return Params->GetMutableValue();
			}
		...
		case EStateTreeDataSourceType::StateParameterData:
		{
			if (FCompactStateTreeParameters* Params = InstanceStorage.GetMutableTemporaryStruct(CurrentFrame.FrameID, Handle).GetPtr<FCompactStateTreeParameters>())
			{
				return Params->GetMutableValue();
			}
...
namespace UE::StateTree::InstanceData
{
		...
		const FStateTreeDataHandle& Handle)
	{
		...
		{
			const int32 ActiveIndex = CurrentFrame.ExecutionRuntimeIndexBase.Get() + Handle.GetIndex();
			return InstanceStorage.GetExecutionRuntimeData().GetMutableStruct(ActiveIndex);
		}
		case EStateTreeDataSourceType::ExecutionRuntimeDataObject:
		{
			const int32 ActiveIndex = CurrentFrame.ExecutionRuntimeIndexBase.Get() + Handle.GetIndex();
			return InstanceStorage.GetExecutionRuntimeData().GetMutableObject(ActiveIndex);
		}
		case EStateTreeDataSourceType::ExecutionRuntimeDataAny:
		{
			const int32 ActiveIndex = CurrentFrame.ExecutionRuntimeIndexBase.Get() + Handle.GetIndex();
			UE::StateTree::InstanceData::FInstanceContainer& Container = InstanceStorage.GetExecutionRuntimeData();
			...
			{
				return Container.GetMutableStruct(ActiveIndex);
			}
		}
		...
		case EStateTreeDataSourceType::GlobalParameterData:
		{
			if (CurrentFrame.GlobalParameterDataHandle.HasValidIndex())
			{
				// Defined as State Parameter in one of its parent frame, we must have a valid cached Index base for active instances
				check(CurrentFrame.GlobalParameterIndexBase.IsValid());

				FCompactStateTreeParameters& StateParams = InstanceStorage.GetMutableStruct(CurrentFrame.GlobalParameterIndexBase.Get() + CurrentFrame.GlobalParameterDataHandle.GetIndex()).Get<FCompactStateTreeParameters>();
				return StateParams.GetMutableValue();
			}
		...
		case EStateTreeDataSourceType::SubtreeParameterData:
		{
			check(CurrentFrame.StateParameterIndexBase.IsValid() && CurrentFrame.StateParameterDataHandle.HasValidIndex());
			...
			FCompactStateTreeParameters& StateParams = InstanceStorage.GetMutableStruct(CurrentFrame.StateParameterIndexBase.Get() + CurrentFrame.StateParameterDataHandle.GetIndex()).Get<FCompactStateTreeParameters>();
			return StateParams.GetMutableValue();
		}
	...
	FStateTreeDataView GetDataViewOrTemporary(
		...
		const FStateTreeDataHandle& Handle)
	{
		if (Private::IsHandleSourceValid(InstanceStorage, CurrentFrame, Handle))
		{
			return GetDataView(InstanceStorage, SharedInstanceStorage, CurrentFrame, Handle);
		}

		return Private::GetTemporaryDataView(InstanceStorage, CurrentFrame, Handle);
	}
...
TArray<FStateTreeDelegateDispatcher> FStateTreeInstanceStorage::StealBroadcastedDelegates()
{
	return MoveTemp(BroadcastedDelegates);
}
...
void FStateTreeInstanceStorage::RemoveTemporaryInstance(const UE::StateTree::FActiveFrameID FrameID, const FStateTreeIndex16 OwnerNodeIndex, const FStateTreeDataHandle DataHandle)
{
	const int32 Index = TemporaryInstances.IndexOfByPredicate([&FrameID, &OwnerNodeIndex, &DataHandle](const FStateTreeTemporaryInstanceData& TempInstance)
				{
	...
	{
		TemporaryInstances.RemoveAtSwap(Index);
	}
}

FStructView FStateTreeInstanceStorage::GetMutableTemporaryStruct(UE::StateTree::FActiveFrameID FrameID, const FStateTreeDataHandle DataHandle)
{
	FStateTreeTemporaryInstanceData* ExistingInstance = TemporaryInstances.FindByPredicate([FrameID, &DataHandle](const FStateTreeTemporaryInstanceData& TempInstance)
		{
			return TempInstance.FrameID == FrameID
				&& TempInstance.DataHandle == DataHandle;
		});
...
UObject* FStateTreeInstanceStorage::GetMutableTemporaryObject(UE::StateTree::FActiveFrameID FrameID, const FStateTreeDataHandle DataHandle)
{
	FStateTreeTemporaryInstanceData* ExistingInstance = TemporaryInstances.FindByPredicate([FrameID, &DataHandle](const FStateTreeTemporaryInstanceData& TempInstance)
		{
			return TempInstance.FrameID == FrameID
...
{
	SetGlobalParameters(Parameters.GetValue());
}

void FStateTreeInstanceStorage::SetGlobalParameters(const FConstStructView Parameters)
{
...
uint32 FStateTreeInstanceStorage::GenerateUniqueId()
{
	...
	UE_LOGF(LogStateTree, Error, "The unique id overflow. Id:%d Serial:%d", ExecutionState.InstanceDebugId.Id, ExecutionState.InstanceDebugId.SerialNumber);
#else
	UE_LOGF(LogStateTree, Error, "The unique id overflow.");
#endif
...
void FStateTreeInstanceData::Append(UObject& InOwner, TConstArrayView<FConstStructView> InStructs, TConstArrayView<TOptional<FInstancedStruct*>> InInstancesToMove, FAddArgs Args)
{
	check(InStructs.Num() == InInstancesToMove.Num());
	...
	const int32 StartIndex = Storage.InstanceStructs.Num();
	//The Append will not duplicate the UObject.
	Storage.InstanceStructs.Append(InStructs);

	for (int32 Index = StartIndex; Index < Storage.InstanceStructs.Num(); Index++)
	{
		FStructView Struct = Storage.InstanceStructs[Index];
		const TOptional<FInstancedStruct*>& OptionalSource = InInstancesToMove[Index - StartIndex];

		// The source is used to move temporary instance data into instance data. Not all entries may have it.
		// The instance struct can be empty, in which case the temporary instance is ignored.
		// If the source is specified, move it to the instance data.
		// We assume that if the source is object wrapper, it is already the instance we want.
		if (Struct.IsValid() && OptionalSource.IsSet())
		{
			FInstancedStruct* Source = OptionalSource.GetValue();
			if (Source && Source->IsValid())
			{
				check(Struct.GetScriptStruct() == Source->GetScriptStruct());
			...
			{
				// The source is explicitly set to nullptr. Duplicate the UObject.
				if (Wrapper->InstanceObject)
				{
					const bool bDuplicate = Args.bDuplicateWrappedObject || &InOwner != Wrapper->InstanceObject->GetOuter();
					Wrapper->InstanceObject = UE::StateTree::InstanceData::Private::CopyNodeInstance(Wrapper->InstanceObject, &InOwner, bDuplicate);
