...
USTRUCT()
struct FGameplayTagSingleMatchConditionInstanceData
{
	GENERATED_BODY()
 ...
 *	Does Tag "A" match Tag "A.1" ?
 *		always returns false
 */
USTRUCT(DisplayName="Matches Tag", Category="Gameplay Tags")
struct FGameplayTagSingleMatchCondition : public FStateTreeConditionCommonBase
{
	GENERATED_BODY()

	using FInstanceDataType = FGameplayTagSingleMatchConditionInstanceData;

	FGameplayTagSingleMatchCondition() = default;
...
#endif // WITH_EDITOR
	/** If true, the tag has to match exactly, if false then the tag may match its parent tags */
