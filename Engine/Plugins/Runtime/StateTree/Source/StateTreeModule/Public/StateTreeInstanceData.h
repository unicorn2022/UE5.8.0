...
namespace UE::StateTree::InstanceData
{
	...
	UE_DEPRECATED(5.8, "Use the version without Parent Frame instead")
	[[nodiscard]] inline FStateTreeDataView GetDataViewOrTemporary(
		FStateTreeInstanceStorage& InstanceStorage,
		FStateTreeInstanceStorage* SharedInstanceStorage,
		const FStateTreeExecutionFrame* ParentFrame,
		const FStateTreeExecutionFrame& CurrentFrame,
		const FStateTreeDataHandle& Handle)
	{
		return GetDataViewOrTemporary(InstanceStorage, SharedInstanceStorage, CurrentFrame, Handle);
	}

	/**
	 * Check if a data source type requires FStateTreeExecutionContext to access.
	...
	/**
	 * Check if a data source type can be access from the UE::StateTree::InstanceData::GetDataView function.
	 * @param SourceType The data source type to check.
...
struct FStateTreeInstanceStorage
{
	...
	*/
	...
	/** Resets the list of broadcasted delegates and returns it's copy. */
	...
	/** Removes a temporary instance data associated with specified frame and data handle. */
	UE_API void RemoveTemporaryInstance(const UE::StateTree::FActiveFrameID FrameID, const FStateTreeIndex16 OwnerNodeIndex, const FStateTreeDataHandle DataHandle);

	UE_DEPRECATED(5.7, "Use the version with FrameID instead.")
	FStructView GetMutableTemporaryStruct(const FStateTreeExecutionFrame& Frame, const FStateTreeDataHandle DataHandle) { return GetMutableTemporaryStruct(Frame.FrameID, DataHandle); }
	...
	UE_DEPRECATED(5.7, "Use the version with FrameID instead.")
	UObject* GetMutableTemporaryObject(const FStateTreeExecutionFrame& Frame, const FStateTreeDataHandle DataHandle) { return GetMutableTemporaryObject(Frame.FrameID, DataHandle); }
...
struct FStateTreeInstanceData
{
	 ...
	 * If the struct is of type FStateTreeInstanceObjectWrapper then this will duplicate the UObject instance.
	 */
	...
	/**
	 * Appends new items to the instance, and moves existing data into the allocated instances, if provided.
	 * If the struct is of type FStateTreeInstanceObjectWrapper then this will duplicate the UObject instance only if the InstanceToMove is set.
	 */
	...
	*/
	...
	{
		return GetMutableStorage().GetMutableTemporaryStruct(Frame.FrameID, DataHandle);
	}

	/** @returns mutable pointer to the specified instance data object, or invalid view if not found. Will check() if called on non-object data. */
	UObject* GetMutableTemporaryObject(const FStateTreeExecutionFrame& Frame, const FStateTreeDataHandle DataHandle)
	{
		return GetMutableStorage().GetMutableTemporaryObject(Frame.FrameID, DataHandle);
	}
...
{
	TStateTreeInstanceDataStructRef(TWeakPtr<FStateTreeInstanceStorage> InWeakStorage, const FStateTreeExecutionFrame& CurrentFrame, const FStateTreeDataHandle InDataHandle)
	...
	TStateTreeInstanceDataStructRef(FStateTreeInstanceData& InInstanceData, const FStateTreeExecutionFrame& CurrentFrame, const FStateTreeDataHandle InDataHandle)
		: TStateTreeInstanceDataStructRef(InInstanceData.GetWeakMutableStorage(), CurrentFrame, InDataHandle)
	{
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	T* GetPtr() const
	{
		...
		else
		{
			...
			if (ExistingInstance)
			{
				if (const FStateTreeInstanceObjectWrapper* ObjectWrapper = ExistingInstance->Instance.GetPtr<FStateTreeInstanceObjectWrapper>())
				{
					DataView = FStateTreeDataView(ObjectWrapper->InstanceObject);
				}
				else
				{
					DataView = FStateTreeDataView(ExistingInstance->Instance);
				}
			}
		}
...
protected:
	FStateTreeDataView GetDataView(FStateTreeInstanceStorage& Storage, const FStateTreeExecutionFrame& CurrentFrame, const FStateTreeDataHandle Handle) const
	{
