...
private:
	...
	/**
	 * The StateTree asset associated to the selected instance.
	 * This can be different from 'StateTreeAsset' if the debugger is associated to a linked asset.
	 */
	TWeakObjectPtr<const UStateTree> InstanceRootAsset;

	/** The StateTree asset associated to the last frame event processed by the debugger */
	TWeakObjectPtr<const UStateTree> CurrentFrameStateTreeAsset;
	...
	/** Indicates whether the analysis session is controlled by an external system, and we don't control its lifetime. */
	bool bIsExternalSessionAnalysis = false;
