...
#define UE_STATETREE_DEBUG_STATE_UTILITY_EVALUATED(InCtx, InStateHandled, Score) ::UE::StateTree::Debug::StateUtilityEvaluated(*(InCtx), InStateHandled, Score)
...
#define UE_STATETREE_ID_NAME UE_JOIN(InstanceId,__LINE__) \
...
namespace UE::StateTree::Debug
{
	...
	DECLARE_TS_MULTICAST_DELEGATE_ThreeParams(FStateUtilityEvaluatedDelegate, const FStateTreeExecutionContext&, FStateTreeStateHandle, float);
	...
	void StateUtilityEvaluated(const FStateTreeExecutionContext& ExecutionContext, FStateTreeStateHandle StateHandle, float Score);
	...
	/**
	 * Debugging callback for when an utility score is calculated for a state.
	 * @note The callback executes inside the StateTree logic.
	 * @note The StateTree can execute on any thread.
	 */
	STATETREEMODULE_API extern FStateUtilityEvaluatedDelegate OnStateUtilityEvaluated_AnyThread;
} //UE::StateTree::Debug
...
#define UE_STATETREE_DEBUG_STATE_UTILITY_EVALUATED(...)
