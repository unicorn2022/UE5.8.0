...
namespace UE::StateTree
{
	...
	public:
		...
		struct FSetStatusResult
		{
			ETaskCompletionStatus CurrentStatus = ETaskCompletionStatus::Running;
			bool bChanged = false;
		};
		 ...
		 * @return Result containing current status and whether it changed.
		 */
		FSetStatusResult SetTaskStatusWithPriority(int32 StateTaskIndex, ETaskCompletionStatus NewStatus)
		{
			StateTaskIndex += BitIndex;
			check(StateTaskIndex < MaxNumTasks);
			const uint8 OldStatus = GetStatusInternal(StateTaskIndex);
			uint8 CurrentStatus = OldStatus;
			bool bChanged = false;
			if (static_cast<uint8>(NewStatus) > OldStatus)
			{
				SetStatusInternal(StateTaskIndex, NewStatus);
				CurrentStatus = static_cast<uint8>(NewStatus);
				bChanged = true;
			}
			return FSetStatusResult{ static_cast<ETaskCompletionStatus>(CurrentStatus), bChanged };
		}
...
// FStateTreeTasksCompletionStatus is not trivially relocatable. Flag their use in containers and other contexts.
template <UE::CDerivedFrom<FStateTreeTasksCompletionStatus> T>
