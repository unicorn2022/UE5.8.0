...
struct TStateTreeStrongExecutionContext
{
	 ...
	 */
	UE_API bool SendEvent(const FGameplayTag Tag, const FConstStructView Payload = FConstStructView(), const FName Origin = FName()) const requires bWithWriteAccess;
	 ...
	 */
	UE_API bool RequestTransition(FStateTreeStateHandle TargetState, EStateTreeTransitionPriority Priority = EStateTreeTransitionPriority::Normal, EStateTreeSelectionFallback Fallback = EStateTreeSelectionFallback::None) const requires bWithWriteAccess;
	 ...
	 */
	UE_API bool BroadcastDelegate(const FStateTreeDelegateDispatcher& Dispatcher) const requires bWithWriteAccess;
	 ...
	 */
	UE_API bool BindDelegate(const FStateTreeDelegateListener& Listener, FSimpleDelegate Delegate) const requires bWithWriteAccess;

	/**
	 * Unregisters the callback bound to the listener.
	 * @return false if the context is not valid or the unbind failed.
	 */
	UE_API bool UnbindDelegate(const FStateTreeDelegateListener& Listener) const requires bWithWriteAccess;
	 ...
	 */
	UE_API bool FinishTask(EStateTreeFinishTaskType FinishType) const requires bWithWriteAccess;

	/**
	 * Updates the scheduled tick of a previous request.
	 * @return false if the context is not valid.
	 */
	UE_API bool UpdateScheduledTickRequest(UE::StateTree::FScheduledTickHandle Handle, FStateTreeScheduledTick ScheduledTick) const requires bWithWriteAccess;
	...
	/**
	 * Get the Execution Runtime Data of recorded node. Only callable on lvalue because it would have lost the access track.
	 * @return nullptr if the frame or state containing the node is no longer active.
	 */
	template<typename T>
	std::conditional_t<bWithWriteAccess, T*, const T*> GetExecutionRuntimeDataPtr() const&
	{
		FStateTreeDataView DataView = GetExecutionRuntimeDataPtrInternal();
		if (DataView.IsValid() && ensure(DataView.GetStruct() == T::StaticStruct()))
		{
			return static_cast<T*>(DataView.GetMutableMemory());
		}
	...
	template<typename T>
	std::conditional_t<bWithWriteAccess, T*, const T*> GetExecutionRuntimeDataPtr() const&& = delete;
	 ...
	 */
	UE_API bool CopyInputBindings() const requires bWithWriteAccess;
	 ...
	 * @return true if all bindings are copied.
	 */
	UE_API bool CopyOutputBindings() const requires bWithWriteAccess;
...
private:
	...
	UE_API FStateTreeDataView GetExecutionRuntimeDataPtrInternal() const;
	bool CopyBindingBatchForAsync(const FStateTreeExecutionFrame* CurrentFrame, const FStateTreeDataView TargetView, const FStateTreeIndex16 BindingsBatch) const requires bWithWriteAccess;

	UE_API static void ScheduleNextTick(TNotNull<UObject*> Owner, TNotNull<const UStateTree*> RootStateTree, FStateTreeInstanceStorage& Storage, UE::StateTree::ETickReason Reason) requires bWithWriteAccess;
