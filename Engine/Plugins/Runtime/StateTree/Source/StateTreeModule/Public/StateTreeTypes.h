...
enum class EStateTreeDataSourceType : uint8
{
	...
	/** Node instance data for the duration of the execution. Discover at runtime if it's a struct or an object. */
	ExecutionRuntimeDataAny,
...
struct FStateTreeDataHandle
{
	...
	STATETREEMODULE_API FStateTreeDataHandle ToStructSource() const;
...
/**
 * Additional rules for if state was changed or sustained per transition. 
 * This is exposed to designers as "Reactivation" w/ "Override" & "Denied" options.
 */
UENUM()
...
struct FCompactStateTransition
{
	...
	/** @return flags for state selection based on this transition's configuration */
	EStateTreeStateSelectionRules GetStateSelectionRulesForTransition(EStateTreeStateSelectionRules ModifiedStateSelectionRules) const
	{
		...
		{
		case EStateTreeTransitionChangeTypeRules::ForceChanged:
			return ModifiedStateSelectionRules | EStateTreeStateSelectionRules::ReselectedStateCreatesNewStates;
		case EStateTreeTransitionChangeTypeRules::ForceSustained:
			return ModifiedStateSelectionRules & ~EStateTreeStateSelectionRules::ReselectedStateCreatesNewStates;
		case EStateTreeTransitionChangeTypeRules::Default:
		default:
	...
	/** Additional rules for state changed or sustain. Their instance data will be reinstantiated. */
	UPROPERTY()
...
struct FCompactStateTreeState
{
	...
	/** Whether the linked state supports Linked Asset override. */
	UPROPERTY()
	...
	/**
	 * If true, state parameter bindings are copied on each task tick.
	 * Parameters bindings are always copied on state enter and state exit.
	 */
	UPROPERTY()
...
struct FCompactStateTreeParameters
{
	...
	FCompactStateTreeParameters(const FCompactStateTreeParameters&) = default;
	FCompactStateTreeParameters(FCompactStateTreeParameters&&) = default;
	FCompactStateTreeParameters& operator=(const FCompactStateTreeParameters&) = default;
	FCompactStateTreeParameters& operator=(FCompactStateTreeParameters&&) = default;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	explicit FCompactStateTreeParameters(FInstancedPropertyBag InParameters)
		: Parameters(MoveTemp(InParameters))
	{
	}

	explicit FCompactStateTreeParameters(FInstancedStruct InParameters)
		: InstancedParameters(MoveTemp(InParameters))
	{
	}
...
public:
	...
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return Parameters.IsValid() ? Parameters.GetValue() : FConstStructView(InstancedParameters);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	...
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return Parameters.IsValid() ? Parameters.GetMutableValue() : FStructView(InstancedParameters);
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}

private:
	UE_DEPRECATED_FORGAME(5.8, "Direct access to Parameter is not valid. Use GetValue or GetMutableValue.")
	/** To hold a PropertyBag for internal state parameters. */
	UPROPERTY()
	FInstancedPropertyBag Parameters;

	/** To hold a UScriptStruct for a linked state tree asset. */
	UPROPERTY()
	FInstancedStruct InstancedParameters;
};
...
/**
 * Helper macro to define instance data as a trivial type.
 * The type can be safely copied using memcpy and don't require special copy construction or destruction. It requires a default constructor.
 * The type defined default values.
 * ex: struct { int32 A = 3; };
 * This is not appropriate for element types that require non-trivial copy constructor/assignment  or non-trivial move constructor/assignment.
 * ex: struct { TObjectPtr<UObject> A; }; // No. TObjectPtr has a non-trivial copy constructor when UE_OBJECT_PTR_GC_BARRIER is enabled.
 */
#define UE_STATETREE_CONSTRUCTED_TRIVIALLY_COPIED_NO_DESTRUCTOR_INSTANCEDATA(Type) \
...
/**
 * Helper macro to define instance data as a trivial type.
 * The type can be safely copied using memcpy and don't require special construction or destruction.
 * The memory will be memset to 0.
 * ex: struct { int32 A = 0; };
 * This is not appropriate for element types that require construction.
 * ex: struct { FFrameRate A; }; // No. FFrameRate has a constructor.
 */
#define UE_STATETREE_ZEROED_TRIVIALLY_COPIED_NO_DESTRUCTOR_INSTANCEDATA(Type) \
template <> struct TIsPODType<Type> { enum { Value = true }; }; \
template<> \
struct TStructOpsTypeTraits<Type> : public TStructOpsTypeTraitsBase2<Type> \
{ \
...
#define STATETREE_POD_INSTANCEDATA(Type) UE_DEPRECATED_MACRO(5.8, "Use UE_STATETREE_CONSTRUCTED_TRIVIALLY_COPIED_NO_DESTRUCTOR_INSTANCEDATA or UE_STATETREE_ZEROED_TRIVIALLY_COPIED_NO_DESTRUCTOR_INSTANCEDATA."); UE_STATETREE_ZEROED_TRIVIALLY_COPIED_NO_DESTRUCTOR_INSTANCEDATA(Type)
