...
namespace UE::StateTree::ExecutionContext
{
	...
	struct ITemporaryStorage
	{
		...
		virtual TConstArrayView<FStateTreeExecutionFrame> GetTemporaryFrames() const = 0;
	};

	UE_API const FStateTreeExecutionFrame* FindExecutionFrame(FActiveFrameID InFrameID, TConstArrayView<FStateTreeExecutionFrame> InActiveFrames, TConstArrayView<FStateTreeExecutionFrame> InTemporaryFrames);
