...
public:
	...
	/** @return const references to the instance data in use */
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	struct FStartParameters
	{
		...
		{
			/** Optional override of tag of start state to select. */
			FGameplayTag StateTag;
			
			/** Does the tag needs to match exactly when searching the state. */
		...
		UE_DEPRECATED(5.8, "Use InitialGlobalParameters instead.")
		/** Optional override of global parameters initial values. */
	...
	/**
	 * Start executing.
	 * @param InitialGlobalParameters Optional override of parameters initial values
	 * @return Tree execution status after the start.
	 */
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		return UE::StateTree::InstanceData::GetDataView(InstanceDataStorage, CurrentlyProcessedSharedInstanceStorage, CurrentFrame, Handle);
	}

	/** Looks for a frame in provided list of frames. */
	UE_DEPRECATED(5.8, "Use UE::StateTree::ExecutionContext::FindExecutionFrame instead.")
...
protected:
	static UE_API FStateTreeExecutionFrame* FindMutableExecutionFrame(UE::StateTree::FActiveFrameID InFrameID, TArrayView<FStateTreeExecutionFrame> ActiveFrames, TArrayView<FStateTreeExecutionFrame> InTemporaryFrames);
...
protected:
	...
	UE_API TSharedPtr<FSelectStateResult> GetCurrentlyProcessedStateSelection() const;

private:
	/** Move the temporary data for each evaluators and tasks to instance data. */
	void MoveGlobalTemporariesToInstances(FStateTreeExecutionFrame& Frame);
...
protected:
	...
	/** Broadcasts task completion dispatchers when a task completes. */
	void BroadcastTaskCompletionDispatchers(TNotNull<const UStateTree*> StateTree, int32 NodeIndex, UE::StateTree::ETaskCompletionStatus TaskStatus);
...
private:
	...
	enum class EConditionFunction
	{
		EnterState,
		ExitState,
		StateCompleted,
	};
	template<bool bOnActiveInstances, EConditionFunction ConditionFunction>
	void RunAllConditionsInternal(const FStateTreeExecutionFrame* CurrentParentFrame, const FStateTreeExecutionFrame& CurrentFrame, const FCompactStateTreeState& CurrentState, const FStateTreeTransitionResult& CurrentTransition);
...
protected:
	 ...
	 */
	struct FTriggerTransitionsInternalArgs
	{
		TOptional<int32> CompletedSubtreeParentFrameIndex;
	};

	UE_API bool TriggerTransitions(const FTriggerTransitionsInternalArgs Args = FTriggerTransitionsInternalArgs());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/**
	 * Move the data from the temporary buffer to the allocated instance data relative to given frame.
	 * @return instanced data of the specified handle relative to given frame.
	 */
	UE_API FStateTreeDataView MoveTemporaryToInstance(const FStateTreeExecutionFrame& Frame, const FStateTreeIndex16 NodeIndex, const FStateTreeDataHandle Handle);
...
protected:
	 ...
	 */
	UE_API bool SetGlobalParameters(FConstStructView Parameters);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	/** Current broadcasted delegates we're processing. */
	TArray<FStateTreeDelegateDispatcher> CurrentlyProcessedBroadcastedDelegates;
