...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	TStateTreeEditorNode<T>& AddConditionWithOuter(TNotNull<UStateTreeState*> InState, TArgs&&... InArgs)
	{
		FStateTreeEditorNode& CondNode = Conditions.AddDefaulted_GetRef();
		CondNode.InitializeAs<T>(InState, Forward<TArgs>(InArgs)...);
		OnNodeAdded(InState, CondNode);
		return static_cast<TStateTreeEditorNode<T>&>(CondNode);
	}

	template<typename T, typename... TArgs>
	UE_DEPRECATED(5.8, "Use AddConditionWithOuter that takes a TNotNull<UStateTreeState*> Outer.")
	...
	bool operator==(const FStateTreeTransition& Other) const
	{
		return ID == Other.ID;
	}
...
private:
	UE_API void OnNodeAdded(TNotNull<UStateTreeState*> InState, FStateTreeEditorNode& EditorNode);
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Transition", meta=(DisplayName="Transition To"))
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Transition", meta = (BaseStruct = "/Script/StateTreeModule.StateTreeConditionBase", BaseClass = "/Script/StateTreeModule.StateTreeConditionBlueprintBase"))
	 ...
	 */
	UPROPERTY(EditDefaultsOnly, Category = "Transition", meta = (HideEditConditionToggle))
	EStateTreeTransitionChangeTypeRules ReactivateTargetState = EStateTreeTransitionChangeTypeRules::Default;
...
public:
	...
	UE_API UStateTreeState* GetNextSiblingState() const;
	UE_API UStateTreeState* GetNextSelectableSiblingState() const;
	...
	UE_API UStateTreeState& AddChildState(const FName ChildName, const EStateTreeStateType StateType = EStateTreeStateType::State);
	...
	TStateTreeEditorNode<T>& AddEnterCondition(TArgs&&... InArgs)
	{
		...
		CondNode.InitializeAs<T>(this, Forward<TArgs>(InArgs)...);
		OnNodeAdded(CondNode);
		return static_cast<TStateTreeEditorNode<T>&>(CondNode);
	}

	/**
	 * Adds enter utility consideration of specified type.
	 * @return reference to the new consideration.
	 */
	template<typename T, typename... TArgs>
	TStateTreeEditorNode<T>& AddConsideration(TArgs&&... InArgs)
	{
		FStateTreeEditorNode& ConsiderationNode = Considerations.AddDefaulted_GetRef();
		ConsiderationNode.InitializeAs<T>(this, Forward<TArgs>(InArgs)...);
		OnNodeAdded(ConsiderationNode);
		return static_cast<TStateTreeEditorNode<T>&>(ConsiderationNode);
	}
	...
	TStateTreeEditorNode<T>& AddTask(TArgs&&... InArgs)
	{
		...
		TaskItem.InitializeAs<T>(this, Forward<TArgs>(InArgs)...);
		OnNodeAdded(TaskItem);
...
public:
	 ...
	 */
	UE_API FStateTreeTransition& AddTransition(const EStateTreeTransitionTrigger InTrigger, const EStateTreeTransitionType InType, const UStateTreeState* InState = nullptr);
	/**
	 * Adds Transition.
	 * @return reference to the new Transition.
	 */
	UE_API FStateTreeTransition& AddTransition(const EStateTreeTransitionTrigger InTrigger, const FGameplayTag InEventTag, const EStateTreeTransitionType InType, const UStateTreeState* InState = nullptr);

private:
	void AddTransitionImpl(FStateTreeTransition& Transition);

public:

	const FStateTreeTransition* GetTransitionByID(const FGuid TransitionID) const
	{
		return Transitions.FindByPredicate([TransitionID](const FStateTreeTransition& Other)
			{
				return Other.ID == TransitionID;
			});
	}
	
	const FStateTreeEditorNode* GetNodeByID(const FGuid NodeID) const
	{
		for (const FStateTreeEditorNode& EnterCondition : EnterConditions)
		{
			if (EnterCondition.ID == NodeID)
			{
				return &EnterCondition;
			}
		}
		for (const FStateTreeEditorNode& Task : Tasks)
		{
			if (Task.ID == NodeID)
			{
				return &Task;
		...
		for (const FStateTreeEditorNode& Consideration : Considerations)
		{
			if (Consideration.ID == NodeID)
			{
				return &Consideration;
			}
		}
		
		if (SingleTask.ID == NodeID)
		{
			return &SingleTask;
		}
	...
	/**
	 * If true, state parameter bindings are copied each task tick, not only when the state is entered or exited.
	 * Has no effect on ScheduledTick.
	 */
	UPROPERTY(EditDefaultsOnly, Category = "State")
	bool bCopyParameterBindingsOnTick = false;
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Enter Conditions", meta = (BaseStruct = "/Script/StateTreeModule.StateTreeConditionBase", BaseClass = "/Script/StateTreeModule.StateTreeConditionBlueprintBase"))
	TArray<FStateTreeEditorNode> EnterConditions;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Tasks", meta = (BaseStruct = "/Script/StateTreeModule.StateTreeTaskBase", BaseClass = "/Script/StateTreeModule.StateTreeTaskBlueprintBase"))
	TArray<FStateTreeEditorNode> Tasks;

	/** Expression used to calculate a selection utility score for state weighing. */
...
private:
	...
	/** Transaction temporary. Transitions we captured in PreEdit for remove / move comparision */
	TArray<FStateTreeTransition> PreEditTransitions;
	/** Handle for the UE::StateTree::Delegates::OnPostCompile delegate */
