...
struct FStateTreePropertyBindingCompiler
{
	...
	[[nodiscard]] UE_API bool CompileDelegateDispatchers(const FStateTreeBindableStructDesc& SourceStruct, TConstArrayView<FStateTreeEditorDelegateDispatcherCompiledBinding> PreviousDistpatchers, TConstArrayView<FStateTreePropertyPathBinding> DelegateSourceBindings, FStateTreeDataView InstanceDataView);
	 ...
	 /**
	  * Compiles delegates dispatcher for selected struct.
	  * @param SourceStruct - Description of the structs which is the delegate dispatchers when it completes.
	  * @param AllBindings - Array of all the bindings.
	  * @return True on success, false on failure.
	 */
	[[nodiscard]] UE_API bool CompileTaskCompletionDelegateDispatchers(const FStateTreeBindableStructDesc& SourceStruct, TConstArrayView<FStateTreePropertyPathBinding> AllBindings);
	...
	/** @return the task completion delegate dispatcher given task node ID and condition. */
	UE_API FStateTreeDelegateDispatcher GetTaskCompletionDispatcher(FGuid TaskNodeID, UE::StateTree::ETaskCompletionCondition Condition) const;

	/** @return the delegate dispatcher (can be a task completion) given the binding. */
	UE_API FStateTreeDelegateDispatcher GetDispatcherFromBinding(const FStateTreePropertyPathBinding& Binding) const;

	/** @return the list of compiled delegate dispatcher. */
	UE_API TConstArrayView<FStateTreeEditorDelegateDispatcherCompiledBinding> GetCompiledDelegateDispatchers() const;

	struct FTaskCompletionDispatcher
	{
		FGuid TaskNodeID;
		FStateTreeDelegateDispatcher Dispatcher;
		UE::StateTree::ETaskCompletionCondition Condition = UE::StateTree::ETaskCompletionCondition::Completes;
	};

	/** @return the list of compiled task completion delegate dispatcher. */
	UE_API TConstArrayView<FTaskCompletionDispatcher> GetCompiledTaskCompletionDispatchers() const;
...
protected:
	...
	TArray<FTaskCompletionDispatcher> CompiledTaskCompletionDelegateDispatchers;
