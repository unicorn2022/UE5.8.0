...
{
	auto CompareDataView = [](FStateTreeDataView ViewA, FStateTreeDataView ViewB)
		{
			bool bInstanceEqual = false;
			if (ViewA.GetStruct() == ViewB.GetStruct())
			{
				if (Cast<UClass>(ViewA.GetStruct()))
				{
					bInstanceEqual = AreObjectsEqual(static_cast<const UObject*>(ViewA.GetMemory()), static_cast<const UObject*>(ViewB.GetMemory()));
				}
				else if (const UScriptStruct* ScriptStruct = Cast<const UScriptStruct>(ViewA.GetStruct()))
				{
					bInstanceEqual = ScriptStruct->CompareScriptStruct(ViewA.GetMemory(), ViewB.GetMemory(), PPF_DeepComparison | PPF_ForDiff);
				}
			}
			return bInstanceEqual;
		};
	auto CompareStructView = [](FStructView ViewA, FStructView ViewB)
		{
			bool bEqual = false;
			if (ViewA.GetScriptStruct() == ViewB.GetScriptStruct())
			{
				bEqual = ViewA.GetScriptStruct() == nullptr || ViewA.GetScriptStruct()->CompareScriptStruct(ViewA.GetMemory(), ViewB.GetMemory(), PPF_DeepComparison | PPF_ForDiff);
			}
			return bEqual;
		};

	return CompareDataView(NodeA.GetInstance(), NodeB.GetInstance())
		&& CompareDataView(NodeA.GetExecutionRuntimeData(), NodeB.GetExecutionRuntimeData())
		&& CompareStructView(NodeA.GetNode(), NodeB.GetNode())
	...
	auto CheckNodes = [&StateTreePropertyPath, &Path](const TArray<FStateTreeEditorNode>& List, FName PathSegmentName)
	{
		for (int i = 0; i < List.Num(); i++)
		{
			if (List[i].ID == StateTreePropertyPath.GetStructID())
			{
				...
				const FStateTreeDataView Instance = List[i].GetInstance();
				if (Cast<UClass>(Instance.GetStruct()))
				{
