...
namespace Internal
{
	...
	{
		if (FStateTreeNodeBase* StateTreeNode = InEditorNode.GetNode().GetPtr<FStateTreeNodeBase>())
		{
			...
			if (const TDoubleLinkedList<FProperty*>::TDoubleLinkedListNode* EditorNodeMemberPropNode = InEditorNodeInChain->GetNextNode())
			{
				...
				if (const TDoubleLinkedList<FProperty*>::TDoubleLinkedListNode* ActiveMemberPropNode = EditorNodeMemberPropNode->GetNextNode()) 
				{
					...
					if (EditorNodeChildMember->GetFName() == GET_MEMBER_NAME_CHECKED(FStateTreeEditorNode, Node))
					{
						...
						|| EditorNodeChildMember->GetFName() == GET_MEMBER_NAME_CHECKED(FStateTreeEditorNode, InstanceObject))
					{
						if (InEditorNode.GetInstance().IsValid())
						{
							StateTreeNode->PostEditInstanceDataChangeChainProperty(PropertyChangedEvent, InEditorNode.GetInstance());
						}
					}
					else if (EditorNodeChildMember->GetFName() == GET_MEMBER_NAME_CHECKED(FStateTreeEditorNode, ExecutionRuntimeData)
						|| EditorNodeChildMember->GetFName() == GET_MEMBER_NAME_CHECKED(FStateTreeEditorNode, ExecutionRuntimeDataObject))
					{
						if (InEditorNode.GetExecutionRuntimeData().IsValid())
						{
							StateTreeNode->PostEditInstanceDataChangeChainProperty(PropertyChangedEvent, InEditorNode.GetExecutionRuntimeData());
						}
					}
				}
			}
...
void DispatchPostEditToNodes(UObject& Owner, FPropertyChangedChainEvent& InPropertyChangedEvent, UStateTreeEditorData& EditorData)
{
	...
	while (CurrentPropNode)
	{
		...
		while (const FArrayProperty* ArrayProperty = CastField<FArrayProperty>(CurrentProperty))
		{
			...
			{
				// It's possible that a property function node is bound directly to an array property
				// In which case the there is no index for the array property while the property chain is still going.
				// We will instead continue traversing the property chain and fetch the property function node property
				break;
			}

			if (TargetPath.GetStructID().IsValid())
			{
				// Skip adding segment for array property itself if we are not modifying its element,
				// As we will not indirect to the inner property and will add segment for array property at the end of the loop
				TargetPath.AddPathSegment(ArrayProperty->GetFName(), Index);
			}

			// Only indirect ArrayProperty to Inner if we are modifying array element
	...
	EPropertyChangeType::Type ChangeType)
{
	...
	// Set ObjectIteratorIndex to index we used above as FPropertyChangedEvent::GetArrayIndex uses that to read the array indexes.
	if (ArrayIndex != INDEX_NONE)
	{
