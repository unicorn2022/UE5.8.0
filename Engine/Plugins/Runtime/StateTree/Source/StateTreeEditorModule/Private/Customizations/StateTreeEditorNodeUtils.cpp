...
EStateTreeConditionEvaluationMode GetConditionEvaluationMode(const TSharedPtr<IPropertyHandle>& StructProperty)
{
	...
	{
		if (const FStateTreeConditionBase* ConditionBase = Node->GetNode().GetPtr<FStateTreeConditionBase>())
		{
...
{
	if (const FStateTreeTaskBase* TaskBase = EditorNode.GetNode().GetPtr<FStateTreeTaskBase>())
	{
...
bool IsTaskConsideredForCompletion(const FStateTreeEditorNode& EditorNode)
{
	...
	if (const FStateTreeBlueprintTaskWrapper* TaskWrapper = EditorNode.GetNode().GetPtr<FStateTreeBlueprintTaskWrapper>())
	{
		if (const UStateTreeTaskBlueprintBase* BPTaskBase = EditorNode.GetInstance().GetPtr<const UStateTreeTaskBlueprintBase>())
		{
			return BPTaskBase->bConsideredForCompletion;
		}
	}
	else if (const FStateTreeTaskBase* TaskBase = EditorNode.GetNode().GetPtr<FStateTreeTaskBase>())
	{
...
void SetTaskConsideredForCompletion(FStateTreeEditorNode& EditorNode, bool bIsConsidered)
{
	if (const FStateTreeBlueprintTaskWrapper* TaskWrapper = EditorNode.GetNode().GetPtr<FStateTreeBlueprintTaskWrapper>())
	{
		if (UStateTreeTaskBlueprintBase* BPTaskBase = EditorNode.GetInstance().GetMutablePtr<UStateTreeTaskBlueprintBase>())
		{
			BPTaskBase->bConsideredForCompletion = bIsConsidered;
		}
	}
	else if (FStateTreeTaskBase* TaskBase = EditorNode.GetNode().GetPtr<FStateTreeTaskBase>())
	{
...
bool CanEditTaskConsideredForCompletion(const FStateTreeEditorNode& EditorNode)
{
	if (const FStateTreeBlueprintTaskWrapper* TaskWrapper = EditorNode.GetNode().GetPtr<FStateTreeBlueprintTaskWrapper>())
	{
		if (const UStateTreeTaskBlueprintBase* BPTaskBase = EditorNode.GetInstance().GetPtr<UStateTreeTaskBlueprintBase>())
		{
...
EVisibility IsConditionVisible(const TSharedPtr<IPropertyHandle>& StructProperty)
{
	...
	if (const FStateTreeEditorNode* Node = GetCommonNode(StructProperty))
	{
		ScriptStruct = Node->GetNode().GetScriptStruct();
	}
...
FNodeRetainPropertyData GetNodeData(FStateTreeEditorNode& EditorNode)
{
	...
	if (FStateTreeNodeBase* NodeBase = EditorNode.GetNode().GetPtr<FStateTreeNodeBase>())
	{
		Data.Node.Struct = EditorNode.GetNode().GetScriptStruct();
		Data.Node.Memory = NodeBase;

		if (const UStruct* InstanceDataType = NodeBase->GetInstanceDataType())
		{
			Data.InstanceData.Struct = EditorNode.GetInstance().GetStruct();
			Data.InstanceData.Memory = EditorNode.GetInstance().GetMutableMemory();
		}
		if (const UStruct* ExecutionRuntimeInstanceDataType = NodeBase->GetExecutionRuntimeDataType())
		{
			Data.ExecutionRuntimeData.Struct = EditorNode.GetExecutionRuntimeData().GetStruct();
			Data.ExecutionRuntimeData.Memory = EditorNode.GetExecutionRuntimeData().GetMutableMemory();
		}
	}
	...
	for (int32 Index = 0; Index < RawNodeData.Num(); Index++)
	{
		if (UObject* Outer = OuterObjects[Index])
		{
			if (FStateTreeEditorNode* Node = static_cast<FStateTreeEditorNode*>(RawNodeData[Index]))
			{
				...
				if (InStruct)
				{
					Node->InitializeAs(Outer, InStruct);
		...
		if (UObject* Outer = OuterObjects[Index])
		{
			if (FStateTreeEditorNode* Node = static_cast<FStateTreeEditorNode*>(RawNodeData[Index]))
			{
				...
				if (InClass)
				{
					...
					{
						FStateTreeBlueprintTaskWrapper TaskWrapper;
						TaskWrapper.TaskClass = const_cast<UClass*>(InClass);
						Node->InitializeAs<FStateTreeBlueprintTaskWrapper>(Outer, MoveTemp(TaskWrapper));
					}
					else if (InClass->IsChildOf(UStateTreeEvaluatorBlueprintBase::StaticClass()))
					{
						FStateTreeBlueprintEvaluatorWrapper EvaluatorWrapper;
						EvaluatorWrapper.EvaluatorClass = const_cast<UClass*>(InClass);
						Node->InitializeAs<FStateTreeBlueprintEvaluatorWrapper>(Outer, MoveTemp(EvaluatorWrapper));
					}
					else if (InClass->IsChildOf(UStateTreeConditionBlueprintBase::StaticClass()))
					{
						FStateTreeBlueprintConditionWrapper ConditionWrapper;
						ConditionWrapper.ConditionClass = const_cast<UClass*>(InClass);
						Node->InitializeAs<FStateTreeBlueprintConditionWrapper>(Outer, MoveTemp(ConditionWrapper));
					}
					else if (InClass->IsChildOf(UStateTreeConsiderationBlueprintBase::StaticClass()))
					{
						FStateTreeBlueprintConsiderationWrapper ConsiderationWrapper;
						ConsiderationWrapper.ConsiderationClass = const_cast<UClass*>(InClass);
						Node->InitializeAs<FStateTreeBlueprintConsiderationWrapper>(Outer, MoveTemp(ConsiderationWrapper));
					}
					else
					{
...
{
	const FStateTreeNodeBase* Node = EditorNode.GetNode().GetPtr<FStateTreeNodeBase>();
	if (!Node)
	{
		return;
	}
	...
	EditorNode.ReallocInstanceData(&InstanceOuter);
	...
	const int32 SortOrder)
{
	...
	TSharedRef<SWidget> RemoveAllWidget = PropertyCustomizationHelpers::MakeEmptyButton(
		FSimpleDelegate::CreateLambda([ArrayPropertyHandle]()
			{
				if (ArrayPropertyHandle && ArrayPropertyHandle->IsValidHandle())
				{
					if (const TSharedPtr<IPropertyHandleArray> ArrayHandle = ArrayPropertyHandle->AsArray())
