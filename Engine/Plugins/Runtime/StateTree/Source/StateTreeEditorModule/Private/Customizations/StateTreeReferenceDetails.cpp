...
void FStateTreeReferenceDetails::CustomizeHeader(const TSharedRef<IPropertyHandle> InStructPropertyHandle, FDetailWidgetRow& InHeaderRow, IPropertyTypeCustomizationUtils& InCustomizationUtils)
{
	...
	.NameContent()
	[
		StateTreeProperty->CreatePropertyNameWidget(StructPropertyHandle->GetPropertyDisplayName())
	]
	.ValueContent()
	[
	...
	StateTreeProperty->SetOnPropertyValueChanged(FSimpleDelegate::CreateSPLambda(AsShared(), [this]()
		{
			SyncParameters();
		}));
