...
void FStateTreeEditorDataDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (Schema != nullptr)
	{
		for (const FStateTreeExternalDataDesc& ContextData : Schema->GetContextDataDescs())
		{
			...
			const UEdGraphSchema_StateTree* EdGraphSchema = GetDefault<UEdGraphSchema_StateTree>();
	...
	{
		uint32 GlobalNumParameters = EditorData ? EditorData->GetRootParametersPropertyBag().GetNumPropertiesInBag() : 0;
		...
		const FText ParametersDisplayName = EnumHasAllFlags(GetDefault<UStateTreeEditorUserSettings>()->GetDetailsViewDisplayCount(), EStateTreeEditorUserSettingsNodeType::Parameter)
			? FText::Format(LOCTEXT("EditorDataDetailsParametersCounted", "Parameters ({0})"), GlobalNumParameters)
			: LOCTEXT("EditorDataDetailsParameters", "Parameters"); 
		...
		ParametersCategory.SetColor(UE::StateTree::Colors::Blue);
		{
			...
			TSharedPtr<IPropertyHandle> PropertyBagParametersProperty = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UStateTreeEditorData, RootParameterPropertyBag));
			...
			if (EditorSchema)
			{
				...
				{
					PropertyBagParametersProperty->SetInstanceMetaData(UE::StructUtils::Metadata::PropertyBagSchemaClassName, SchemaMetaData.GetValue());
				}
			}
	...
	// Evaluators category
	int32 GlobalNumEvaluators = EditorData ? EditorData->Evaluators.Num() : 0;
	...
	if (Schema && Schema->AllowEvaluators())
	{
		UE::StateTreeEditor::EditorNodeUtils::MakeArrayCategory(
			...
			EvaluatorsProperty,
			EvalCategoryName,
			EnumHasAllFlags(GetDefault<UStateTreeEditorUserSettings>()->GetDetailsViewDisplayCount(), EStateTreeEditorUserSettingsNodeType::Evaluator)
				? FText::Format(LOCTEXT("EditorDataDetailsEvaluatorsCounted", "Evaluators ({0})"), GlobalNumEvaluators)
	...
	int32 GlobalNumTasks = EditorData ? EditorData->GlobalTasks.Num() : 0;
	...
	IDetailCategoryBuilder& GlobalTasksCategory = UE::StateTreeEditor::EditorNodeUtils::MakeArrayCategoryHeader(
		...
		EnumHasAllFlags(GetDefault<UStateTreeEditorUserSettings>()->GetDetailsViewDisplayCount(), EStateTreeEditorUserSettingsNodeType::Task)
			? FText::Format(LOCTEXT("EditorDataDetailsGlobalTasksCounted", "Global Tasks ({0})"), GlobalNumTasks)
