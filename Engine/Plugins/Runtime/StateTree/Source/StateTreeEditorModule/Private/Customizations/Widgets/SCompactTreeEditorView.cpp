...
FSlateColor CompactTreeView::FStateItemLinkData::GetBorderColor() const
{
...
void SCompactTreeEditorView::CacheState(TSharedPtr<FStateItem> ParentNode, const UStateTreeState* State)
{
	...
	if (bShouldAdd)
	{
		...
		CompactTreeView::FStateItemLinkData& CustomData = StateItem->CustomData.GetMutable<CompactTreeView::FStateItemLinkData>();
...
TSharedRef<SCompactTreeView::FStateItem> SCompactTreeEditorView::CreateStateItemInternal() const
{
	...
	StateItem->CustomData = TInstancedStruct<CompactTreeView::FStateItemLinkData>::Make({});
	return StateItem;
}
...
{
	using namespace CompactTreeView;
