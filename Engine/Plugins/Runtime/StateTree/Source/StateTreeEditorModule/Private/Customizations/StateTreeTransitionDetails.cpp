...
void FStateTreeTransitionDetails::CustomizeHeader(TSharedRef<IPropertyHandle> StructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	TransitionReactivateProperty = StructProperty->GetChildHandle(GET_MEMBER_NAME_CHECKED(FStateTreeTransition, ReactivateTargetState));
...
EStateTreeVisitor VisitNestedHierachyIncludingLinks_Internal(const TMap<UStateTreeState*, TArray<UStateTreeState*>>& LinkedByStateMap
	, TSet<UStateTreeState*>& VisitedStates
	, UStateTreeState& NestedStateLeaf
	, TFunctionRef<EStateTreeVisitor(UStateTreeState& State)> InFunc)
{
	bool bContinue = InFunc(NestedStateLeaf) == EStateTreeVisitor::Continue;
	VisitedStates.Add(&NestedStateLeaf);
	if (bContinue)
	{
		const TArray<UStateTreeState*>* LinkedByStates = LinkedByStateMap.Find(&NestedStateLeaf);
		UStateTreeState* ParentState = NestedStateLeaf.Parent;

		if (LinkedByStates)
		{
			for (UStateTreeState* LinkedByState : *LinkedByStates)
			{
				if (!VisitedStates.Contains(LinkedByState) && ensure(LinkedByState))
				{
					bContinue = VisitNestedHierachyIncludingLinks_Internal(LinkedByStateMap, VisitedStates, *LinkedByState, InFunc) == EStateTreeVisitor::Continue;
					if (!bContinue)
					{
		...
		if (!VisitedStates.Contains(ParentState) && ParentState)
		{
			bContinue = VisitNestedHierachyIncludingLinks_Internal(LinkedByStateMap, VisitedStates, *ParentState, InFunc) == EStateTreeVisitor::Continue;
			if (!bContinue)
			{
				return EStateTreeVisitor::Break;
			}
...
void VisitNestedHierachyIncludingLinks(UStateTreeEditorData* EditorData, UStateTreeState& NestedStateLeaf, TFunctionRef<EStateTreeVisitor(UStateTreeState& State)> InFunc)
{
	// We need to visit our nested state, considering links. Suppose we have:
	// 
	// Root
	// - a -> A
	// - b -> B
	...
	// X 
	// - I
	// -- P -> Root (Infinite loop, this won't compile, but we are editor time so still need consider)
	// -- Q
	// Y 
	// - J
	// 
	// First we construct a LinkedBy list
	// A: {a}
	// B: {b}
	// C: {c} 
	// X: {A, C}
	...
	// Root: {P}
	// 
	// Then we visit Nested & Linked State Parents. 
	// Algo:
	// - Visit Current, add to Visited
	// - For each node in union (LinkedBy[Current], Parent):
	// -- If not in Visited, recurse Algo as new current.
	
	// Note: Ptr's not const since we don't require the callback to be const
	TMap<UStateTreeState*, TArray<UStateTreeState*>> LinkedByStateMap;
	EditorData->VisitHierarchy([&LinkedByStateMap, EditorData](UStateTreeState& State, UStateTreeState* ParentState)
	{
		if (State.Type == EStateTreeStateType::Linked)
		{
			UStateTreeState* LinkedState = EditorData->GetMutableStateByID(State.LinkedSubtree.ID);
			if (ensure(LinkedState))
			{
				LinkedByStateMap.FindOrAdd(LinkedState).Add(&State);
			}
		}
		return EStateTreeVisitor::Continue;
	});
	...
	TSet<UStateTreeState*> VisitedStates;
	VisitNestedHierachyIncludingLinks_Internal(LinkedByStateMap, VisitedStates, NestedStateLeaf, InFunc);
}
...
void FStateTreeTransitionDetails::CustomizeChildren(TSharedRef<class IPropertyHandle> StructPropertyHandle, class IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	check(TransitionReactivateProperty);
	...
	UStateTreeState* OwningState = GetOwningStateTreeState();
	TWeakObjectPtr<UStateTreeState> WeakOwningState = OwningState;
	UStateTreeEditorData* EditorData = GetEditorData();
	...
	auto IsInNestedState = [WeakSelf, WeakOwningState, WeakEditorData]()
	{
		const TSharedPtr<FStateTreeTransitionDetails> Self = WeakSelf.Pin();
		TStrongObjectPtr<UStateTreeState> OwningState = WeakOwningState.Pin();
		TStrongObjectPtr<UStateTreeEditorData> EditorData = WeakEditorData.Pin();

		if (!Self || !OwningState || !EditorData)
		{
			return false;
		}

		// Try to return cached value if it's correct
		FStateTreeStateLink TargetStateLink = Self->GetTargetState();
		if (Self->CachedLastTargetState.ID == TargetStateLink.ID)
		{
			return Self->bCachedIsTargetStateInNestedState;
		}
		else
		{
			Self->CachedLastTargetState = TargetStateLink;
		}

		UStateTree* StateTree = EditorData->GetOuterUStateTree();
		UStateTreeState* CurrentState = OwningState.Get();
		FStateTreeStateLink CurrentStateLink = CurrentState->GetLinkToState();

		bool bIsTargetStateInNestedState = false;
		auto IsTargetState = [TargetStateLink, &bIsTargetStateInNestedState](UStateTreeState& InCurrentState)
		{
			if (TargetStateLink.ID != InCurrentState.GetLinkToState().ID)
			{
				return EStateTreeVisitor::Continue;
			}
			else
			{
				bIsTargetStateInNestedState = true;
				return EStateTreeVisitor::Break;
			}
		};
		VisitNestedHierachyIncludingLinks(EditorData.Get(), *CurrentState, IsTargetState);

		Self->bCachedIsTargetStateInNestedState = bIsTargetStateInNestedState;
		return bIsTargetStateInNestedState;
	};
	...
	StructBuilder.AddProperty(RequiredEventProperty.ToSharedRef())
		.Visibility(MakeAttributeLambda([WeakSelf]()
		{
			if (const TSharedPtr<FStateTreeTransitionDetails> Self = WeakSelf.Pin())
			{
	...
	StructBuilder.AddProperty(PriorityProperty.ToSharedRef())
		.Visibility(MakeAttributeLambda(IsNotCompletionTransition));
	...
	auto IsTransitionReactiviationSchemaEditable = [WeakEditorData]()
	{
		bool bAllowed = false;
		if (TStrongObjectPtr<UStateTreeEditorData> EditorData = WeakEditorData.Pin())
		{
			UStateTree* StateTree = EditorData->GetOuterUStateTree();
			bAllowed = EnumHasAnyFlags(EditorData->EditorSchema->GetTransitionEditingRules(EditorData.Get()), EStateTreeTransitionEditingRules::AllowReactivation);
		}

		return bAllowed ? EVisibility::Visible : EVisibility::Collapsed;
	};
	StructBuilder.AddProperty(TransitionReactivateProperty.ToSharedRef())
		.EditCondition(MakeAttributeLambda(IsInNestedState), FOnBooleanValueChanged())
		.Visibility(MakeAttributeLambda(IsTransitionReactiviationSchemaEditable));
...
FStateTreeStateLink FStateTreeTransitionDetails::GetTargetState() const
{
	FStateTreeStateLink StateValue;
	FStateTreeStateLink* StateData;
	if (ensure(StateProperty.IsValid()))
	{
		// Note: This can fail. IE: If we are deleting the current transition
		if (StateProperty->GetValueData((void*&)StateData) == FPropertyAccess::Success)
		{
...
EStateTreeTransitionTrigger FStateTreeTransitionDetails::GetTrigger() const
{
	...
	if (ensure(TriggerProperty.IsValid()))
	{
...
bool FStateTreeTransitionDetails::GetDelayTransition() const
{
	...
	if (ensure(DelayTransitionProperty.IsValid()))
	{
...
UStateTreeState* FStateTreeTransitionDetails::GetOwningStateTreeState() const
{
	TArray<UObject*> OuterObjects;
	StructProperty->GetOuterObjects(OuterObjects);
	for (UObject* Outer : OuterObjects)
	{
		if (UStateTreeState* OuterStateTreeState = Cast<UStateTreeState>(Outer))
		{
			return OuterStateTreeState;
		}
	}
