...
FText GetStateLinkDesc(const UStateTreeEditorData* EditorData, const FStateTreeStateLink& Link, EStateTreeNodeFormatting Formatting, bool bShowStatePath)
{
	...
	if (Link.LinkType == EStateTreeTransitionType::Parent)
	{
		return Formatting == EStateTreeNodeFormatting::RichText
			? LOCTEXT("TransitionParentRich", "<i>Parent</>")
			: LOCTEXT("TransitionParent", "Parent");
	}
	...
	if (Link.LinkType == EStateTreeTransitionType::NextParent)
	{
		return Formatting == EStateTreeNodeFormatting::RichText
			? LOCTEXT("TransitionNextParentRich", "<i>Next Parent</>")
			: LOCTEXT("TransitionNextParent", "Next Parent");
	}
	if (Link.LinkType == EStateTreeTransitionType::NextSelectableParent)
	{
		return Formatting == EStateTreeNodeFormatting::RichText
			? LOCTEXT("TransitionNextSelectableParentRich", "<i>Next Selectable Parent</>")
			: LOCTEXT("TransitionNextSelectableParent", "Next Selectable Parent");
	}
...
FText GetTransitionDesc(const UStateTreeEditorData* EditorData, const FStateTreeTransition& Transition, EStateTreeNodeFormatting Formatting, bool bShowStatePath)
{
	...
	FText ReactivateText = Formatting == EStateTreeNodeFormatting::RichText
		? LOCTEXT("ReselectionTextRich", "<s>(Re-Activate)</>")
		: LOCTEXT("ReselectionText", "(Re-Activate)");

	if (Transition.ReactivateTargetState != EStateTreeTransitionChangeTypeRules::ForceChanged)
	{
		ReactivateText = FText::GetEmpty();
	}

	return FText::Format(LOCTEXT("TransitionDesc", "{0} {1} {2} {3}"), TriggerText, ActionText, GetStateLinkDesc(EditorData, Transition.State, Formatting, bShowStatePath), ReactivateText);
}
