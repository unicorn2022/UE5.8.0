...
// @TODO: Dependency onto state centric to enable view toggling. 
// Long term plan is to have this in a separate tab.
...
void SStateTreeView::Construct(const FArguments& InArgs, TSharedRef<FStateTreeViewModel> InStateTreeViewModel, const TSharedRef<FUICommandList>& InCommandList)
{
	...
	TSharedRef<SWidget> StateTreeView = SNew(SVerticalBox)
		+ SVerticalBox::Slot()
	...
	if (!UStateCentricViewSettings::Get().IsStateCentricViewEnabled())
	{
		// No state centric view, only tree view
		ChildSlot
		[
			StateTreeView
		];
	...
	{
		// @TODO: Wrap main state centric view with something that accepts a null state
		UStateTreeState* StateCentricViewState = StateTreeViewModel->GetViewState();
		ensure(StateCentricViewState);
		ChildSlot
		[
			SNew(SSplitter)
			+ SSplitter::Slot()
			...
			[
				...
				[
					SNew(UE::StateTree::Editor::StateCentricView::SStateCentricToolbar, InStateTreeViewModel)
				]
				+ SSplitter::Slot()
				.Value(1.0f)
				[
					...
					[
						...
						[
							SNew(UE::StateTree::Editor::StateCentricView::SStateCentricViewMainStateNode, InStateTreeViewModel, StateCentricViewState)
						]
					]
				]
			]
	...
	DetailsView->RegisterInstancedCustomPropertyLayout(UStateTreeEditorUserSettings::StaticClass(), FOnGetDetailCustomizationInstance::CreateLambda([]()
		{
			...
			public:
				virtual void CustomizeDetails(IDetailLayoutBuilder& DetailLayout) override
				{
					...
					{
						auto FindProperty = [](TArrayView<TSharedRef<IPropertyHandle>> AllProperties, FName PropertyToFind)
							{
								return AllProperties.FindByPredicate(
									[PropertyToFind](TSharedRef<IPropertyHandle>& Other)
									{
...

						{
							...
							TSharedRef<IPropertyHandle>* FoundProperty = FindProperty(AllProperties, "StatesViewDisplayNodeType");
						...
						{
							IDetailCategoryBuilder& CategoryBuilder = DetailLayout.EditCategory("Details View");
							TArray<TSharedRef<IPropertyHandle>> AllProperties;
							CategoryBuilder.GetDefaultProperties(AllProperties);

							TSharedRef<IPropertyHandle>* FoundProperty = FindProperty(AllProperties, "DetailsViewDisplayCount");
							if (ensure(FoundProperty))
							{
								CategoryBuilder.AddProperty(*FoundProperty).CustomWidget()
									.NameContent()
									...
									[
										SNew(SEnumComboBox, StaticEnum<EStateTreeEditorUserSettingsNodeType>())
											.OnEnumSelectionChanged(SEnumComboBox::FOnEnumSelectionChanged::CreateLambda([](int32 NewValue, ESelectInfo::Type)
												{
													GetMutableDefault<UStateTreeEditorUserSettings>()->SetDetailsViewDisplayCount((EStateTreeEditorUserSettingsNodeType)NewValue);
												}))
											.CurrentValue(MakeAttributeLambda([]() { return (int32)GetDefault<UStateTreeEditorUserSettings>()->GetDetailsViewDisplayCount(); }))
											.Font(IDetailLayoutBuilder::GetDetailFont())
									];
							}
						}
