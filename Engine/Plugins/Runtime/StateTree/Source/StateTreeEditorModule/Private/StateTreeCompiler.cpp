...
{
	// Version used by AppendToClassSchema. Bump this version number when a recompile of all assets is needed.
	//Only bump when only the compiler changes.
	//You do not need to bump the version when the memory layout of UStateTree has changed.
	static constexpr int32 ClassSchemaVersion = 6;
	...
	bool bEnableConditionWithEvaluationScopeInstanceData = true;
	...
	bool bEnableUtilityConsiderationWithEvaluationScopeInstanceData = true;
	...
	bool bEnableCheckOutersOnCompilationSucceeded = false;
	FAutoConsoleVariableRef CVarEnableCheckOutersOnCompilationSucceeded(
		TEXT("StateTree.Compiler.bEnableCheckOutersOnCompilationSucceeded"),
		bEnableCheckOutersOnCompilationSucceeded,
		TEXT("After a StateTree successfully compiles, test if there are invalid references in the compiled data.")
	);

	bool bLogResultOnCompilationCompleted = false;
	FAutoConsoleVariableRef CVarLogCompiledStateTree(
		TEXT("StateTree.Compiler.LogResultOnCompilationCompleted"),
		bLogResultOnCompilationCompleted,
	...
	class FCheckOutersArchive : public FArchiveUObject
	{
		...
		TNotNull<const UStateTreeEditorData*> EditorData;
		FStateTreeCompilerLog& Log;
	public:

		FCheckOutersArchive(TNotNull<const UStateTree*> InStateTree, TNotNull<const UStateTreeEditorData*> InEditorData, FStateTreeCompilerLog& InLog)
	...
	bool ValidateNoLevelActorReferences(FStateTreeCompilerLog& Log, const FStateTreeBindableStructDesc& NodeDesc, const FStateTreeDataView NodeView, const FStateTreeDataView InstanceView)
	{
		...
		{
			TStringBuilder<256> AllActorsString;
	...
	FCompletionTasksMaskResult MakeCompletionTasksMask(int32 FullStartBitIndex, TConstArrayView<FStateTreeEditorNode> AllTasks, TConstArrayView<int32> ValidTasks)
	{
		...
		TBitArray<> ConsideredForCompletion;
		bool bHasTaskConsideredForCompletion = false;
		for (int32 Index = 0; Index < NumberOfTasks; ++Index)
		{
			const int32 TaskIndex = ValidTasks[Index];
			const bool bIsTaskConsideredForCompletion = UE::StateTreeEditor::EditorNodeUtils::IsTaskEnabled(AllTasks[TaskIndex])
				&& UE::StateTreeEditor::EditorNodeUtils::IsTaskConsideredForCompletion(AllTasks[TaskIndex]);

			ConsideredForCompletion.Add(bIsTaskConsideredForCompletion);
			bHasTaskConsideredForCompletion |= bIsTaskConsideredForCompletion;
		}

		NumberOfBitsNeeded = NumberOfTasks;

		//Each state will take at least 1 bit.
		//No task are marked as completed, we still need a flag to mark the state itself completes (ie. for linked state).
		//Linked state can't have tasks but keep that logic in case it is needed later.
		//A mask of 0, will cause issue with the GetCompletionsStatus.
		if (!bHasTaskConsideredForCompletion)
		{
			Mask |= 1;
			++NumberOfBitsNeeded;
		}
		...
		{
			Mask <<= 1;
			if (ConsideredForCompletion[Index])
			{
				Mask |= 1;
			}
		}
...
void FStateTreeCompiler::AppendToStateTreeClassSchema(FAppendToClassSchemaContext& Context)
{
	Context.Update(&UE::StateTree::Compiler::ClassSchemaVersion, sizeof(UE::StateTree::Compiler::ClassSchemaVersion));

	auto UpdateContext = [&Context](const FStringView Name)
		{
			const IConsoleVariable* Variable = IConsoleManager::Get().FindConsoleVariable(Name.GetData());
			ensure(Variable != nullptr);
			const bool Value = Variable == nullptr || !Variable->GetBool();
			Context.Update(&Value, sizeof(bool));
		};
	UpdateContext(TEXT("StateTree.Compiler.EnableParameterDelegateDispatcherBinding"));
	UpdateContext(TEXT("StateTree.Compiler.EnablePropertyFunctionWithEvaluationScopeInstanceData"));
	UpdateContext(TEXT("StateTree.Compiler.EnableConditionWithEvaluationScopeInstanceData"));
	UpdateContext(TEXT("StateTree.Compiler.EnableUtilityConsiderationWithEvaluationScopeInstanceData"));
}


bool FStateTreeCompiler::FailCompilation(UStateTree::ECompileStatus Status) const
{
	StateTree->ResetCompiled();
	StateTree->CompileStatus = Status;
	// The editor data doesn't match the state tree data, but another compilation would create the same result.
	//Mark it as not dirty and use the CompileStatus to mark the runtime data as invalid.
	//Keeping it dirty would trigger multiple compilations that are not needed.
	StateTree->EditorDataDirtyStatus = UStateTree::EDirtyStatus::None;
	StateTree->LastCompiledEditorDataHash = 0;
	return false;
}

bool FStateTreeCompiler::PreCompile(TNotNull<UStateTree*> InStateTree)
{
	if (bCompiled)
	{
	...
	{
		BindingsByTargetStructID.Add(Binding.GetTargetPath().GetStructID(), &Binding);
	}
	EditorBindingsNum = EditorData->EditorBindings.GetBindings().Num();

	return true;
}

bool FStateTreeCompiler::CompilePublic(TNotNull<UStateTree*> InStateTree)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FStateTreeCompiler::CompilePublic)
	if (!PreCompile(InStateTree))
	{
		return FailCompilation(UStateTree::ECompileStatus::Public);
	}
	...
	// Public succeeded, the next step is dirty
	StateTree->CompileStatus = UStateTree::ECompileStatus::Internal;
	StateTree->EditorDataDirtyStatus = UStateTree::EDirtyStatus::Internal;

	return true;
}

bool FStateTreeCompiler::CompileInternalImpl()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FStateTreeCompiler::CompileInternal)
		...
		for (FStateTreeExternalDataDesc& Desc : StateTree->ContextDataDescs)
		{
			if (Desc.Struct)
			{
				...
				if (const auto Validation = UE::StateTree::Compiler::IsValidIndex16(ContextDataIndex); Validation.DidFail())
				{
					...
					return FailCompilation(UStateTree::ECompileStatus::Internal);
				}
	...
	if (!CreateTaskCompletionDispatchers())
	{
		return FailCompilation(UStateTree::ECompileStatus::Internal);
	}
	...
	StateTree->IDToStateMappings.Reserve(IDToState.Num());
	...
	StateTree->IDToTransitionMappings.Reserve(IDToTransition.Num());
	...
	if (UE::StateTree::Compiler::bEnableCheckOutersOnCompilationSucceeded)
	{
		CheckCompiledStateTreeOuters(StateTree, Log);
	}

	if (UE::StateTree::Compiler::bLogResultOnCompilationCompleted)
	{
		UE_LOGF(LogStateTreeEditor, Log, "%ls", *StateTree->DebugInternalLayoutAsString());
	}

	return true;
}

bool FStateTreeCompiler::CompileInternal(TNotNull<UStateTree*> InStateTree)
{
	// Compile both step to have the public up to date.
	if (!CompilePublic(InStateTree))
	{
		return FailCompilation(UStateTree::ECompileStatus::Public);
	}
	return CompileInternalImpl();
}

bool FStateTreeCompiler::Compile(TNotNull<UStateTree*> InStateTree)
{
	return CompileInternal(InStateTree);
}
...
void FStateTreeCompiler::CheckCompiledStateTreeOuters(TNotNull<const UStateTree*> InStateTree, FStateTreeCompilerLog& InLog)
{
	const UStateTreeEditorData* EditorData = Cast<UStateTreeEditorData>(InStateTree->EditorData);
	if (!EditorData)
	{
		return;
	}

	// Test if there are invalid references in the compiled data.
	// Serialize is non-const but does not modify the StateTree when used with a saving archive.
	UE::StateTree::Compiler::FCheckOutersArchive CheckOuters(InStateTree, EditorData, InLog);
	const_cast<UStateTree*>(static_cast<const UStateTree*>(InStateTree))->Serialize(CheckOuters);
}
...
void FStateTreeCompiler::GatherBindingSources()
{
	check(EditorData);

	// Build the list of all the binding sources. Allow the compiler to know if a struct is used when compiling it by a child state (not yet fully compiled).
	for (const FPropertyBindingBinding& BindingIt : EditorData->EditorBindings.GetBindings())
	{
		const FStateTreePropertyPathBinding& Binding = static_cast<const FStateTreePropertyPathBinding&>(BindingIt);
		const FGuid SourceStructID = Binding.GetSourcePath().GetStructID();
		if (Binding.IsTaskCompletionBinding())
		{
			TaskCompletionDispatcherIDs.Add(SourceStructID);
		}
	}
}
...
bool FStateTreeCompiler::CreateParameters()
{
	...
	if (StateTree->ParameterDataType == EStateTreeParameterDataType::GlobalParameterData)
	{
		StateTree->Parameters = EditorData->GetRootParametersPropertyBag();
	}
	else
	{
		ensure(StateTree->ParameterDataType == EStateTreeParameterDataType::ExternalGlobalParameterData);
		StateTree->Parameters = EditorData->GetRootParametersPropertyBag();
	}
...
bool FStateTreeCompiler::CreateStateRecursive(UStateTreeState& State, const FStateTreeStateHandle Parent)
{
	...
	if (const auto Validation = UE::StateTree::Compiler::IsValidCount16(StateTree->States.Num()); Validation.DidFail())
	{
		Validation.Log(Log, TEXT("States"));
		return false;
	}
	const int32 StateIdx = StateTree->States.AddDefaulted();
	const FStateTreeStateHandle StateHandle = FStateTreeStateHandle(StateIdx);
	...
	CompactState.bCopyParameterBindingsOnTick = State.bCopyParameterBindingsOnTick;
	...
	if (!Parent.IsValid())
	{
		const int32 NewFrameIndex = StateTree->Frames.Num();
		...
		StateHandleToFrameIndex.Add(StateHandle, NewFrameIndex);
	}
	else
	{
		const int32* ParentFrameIndexPtr = StateHandleToFrameIndex.Find(Parent);
		if (ensure(ParentFrameIndexPtr != nullptr))
		{
			const int32 ParentFrameIndex = *ParentFrameIndexPtr;
			StateHandleToFrameIndex.Add(StateHandle, ParentFrameIndex);
		}
	}
	...
	if (State.Type == EStateTreeStateType::LinkedAsset && State.Tag.IsValid())
	{
		const EStateTreeVisitor VisitResult = EditorData->VisitStateBindings(&State, [](TNotNull<const UStateTreeState*> State, const FStateTreePropertyPathBinding& Binding, const FStateTreeBindableStructDesc& BindingTargetNodeDesc, const FStateTreeDataView BindingTargetNodeValue)
			{
				if (Binding.GetSourcePath().GetStructID() == State->Parameters.ID || Binding.GetTargetPath().GetStructID() == State->Parameters.ID)
				{
		...
		if (VisitResult == EStateTreeVisitor::Break)
		{
			CompactState.bCanOverrideLinkedAssetAtRuntime = false;
		}
	}

	SourceStates.Add(&State);
	StatePaths.Add(State.GetPath());
	...
	for (UStateTreeState* Child : State.Children)
	{
		...
		{
			if (!CreateStateRecursive(*Child, StateHandle))
			{
				return false;
			}
		}
...
bool FStateTreeCompiler::CreateStateTasksAndParameters()
{
	...
	for (int32 StateIndex = 0; StateIndex < StateTree->States.Num(); ++StateIndex)
	{
		...
		const FString& StatePath = GetStatePath(StateIndex);
		check(State != nullptr);
		...
		if (!UE::StateTree::Compiler::ValidateNoLevelActorReferences(Log, LinkedParamsDesc, FStateTreeDataView(), FStateTreeDataView(CompactStateTreeParameters.GetMutableValue())))
		{
			bSucceeded = false;
			continue;
		}
		...
		if (State->Type != EStateTreeStateType::Subtree)
		{
			...
			if (!CreateBindingsForStruct(LinkedParamsDesc, FStateTreeDataView(CompactStateTreeParameters.GetMutableValue()), PropertyFunctionsBegin, PropertyFunctionsEnd, CompactState.ParameterBindingsBatch, OutputBindingsBatch))
			{
				bSucceeded = false;
				continue;
			}
		...
		else if (State->SingleTask.GetNode().IsValid())
		{
		...
		for (int32 TaskIndex = 0; TaskIndex < Tasks.Num(); ++TaskIndex)
		{
			...
			FStateTreeTaskBase& Task = TaskNode.GetNode().Get<FStateTreeTaskBase>();
		...
		// Create completion mask
		{
			...
			UE::StateTree::Compiler::FCompletionTasksMaskResult MaskResult = UE::StateTree::Compiler::MakeCompletionTasksMask(StartBitIndex, Tasks, ValidTaskNodeIndex);
			...
			{
				const int32* FrameIndexPtr = StateHandleToFrameIndex.Find(CompactStateHandle);
				if (FrameIndexPtr == nullptr)
				{
				...
				if (!StateTree->Frames.IsValidIndex(*FrameIndexPtr))
				{
					Log.Reportf(EMessageSeverity::Error, LinkedParamsDesc, TEXT("The parent frame is invalid"));
					bSucceeded = false;
					continue;
				}

				FCompactStateTreeFrame& Frame = StateTree->Frames[*FrameIndexPtr];
				Frame.NumberOfTasksStatusMasks = FMath::Max(Frame.NumberOfTasksStatusMasks, static_cast<uint8>(CompactState.CompletionTasksMaskBufferIndex+1));
			}
		}
...
bool FStateTreeCompiler::CreateStateEnterConditions(int32 StateIndex)
{
	FCompactStateTreeState& CompactState = StateTree->States[StateIndex];
	UStateTreeState* SourceState = SourceStates[StateIndex];
	...
	for (int32 ConditionIndex = EnterConditionsBegin; ConditionIndex < EnterConditionsBegin + EnterConditionsNum; ConditionIndex++)
	{
...
bool FStateTreeCompiler::CreateStateTransitions()
{
	...
	for (int32 StateIndex = 0; StateIndex < StateTree->States.Num(); ++StateIndex)
	{
		...
		if (!CreateStateEnterConditions(StateIndex))
		{
			bSucceeded = false;
			continue;
		}
		...
		for (FStateTreeTransition& Transition : SourceState->Transitions)
		{
			...
			if (Transition.Trigger == EStateTreeTransitionTrigger::OnDelegate)
			{
				...
				const FStateTreePropertyPathBinding* Binding = static_cast<const FStateTreePropertyPathBinding*>(EditorData->EditorBindings.FindBinding(DelegateBindingPath));
				...
				CompactTransition.RequiredDelegateDispatcher = BindingsCompiler.GetDispatcherFromBinding(*Binding);
			...
			CompactTransition.ChangeTypeTargetStateRule = Transition.ReactivateTargetState;
...
bool FStateTreeCompiler::CreateStateConsideration(int32 StateIndex)
{
	FCompactStateTreeState& CompactState = StateTree->States[StateIndex];
	UStateTreeState* SourceState = SourceStates[StateIndex];
	check(SourceState != nullptr);
...
bool FStateTreeCompiler::CreateStateConsiderations()
{
	...
	for (int32 StateIndex = 0; StateIndex < StateTree->States.Num(); ++StateIndex)
	{
		...
		if (!CreateStateConsideration(StateIndex))
		{
			bSucceeded = false;
			continue;
		}
...
bool FStateTreeCompiler::CreateBindingsForNodes(TConstArrayView<FStateTreeEditorNode> EditorNodes, FStateTreeIndex16 NodesBegin, TArray<FInstancedStruct>& Instances)
{
	...
	for (const FStateTreeEditorNode& EditorNode : EditorNodes)
	{
		...
		{
			check(EditorNode.GetInstance().GetStruct() == ObjectWrapper->InstanceObject->GetClass());
			InstanceView = FStateTreeDataView(ObjectWrapper->InstanceObject);
		}
		else
		{
			check(EditorNode.GetInstance().GetStruct() == Instance.GetScriptStruct());
...

		{
			...
			if (const FStateTreeBindableStructDesc* BindableNodeStruct = BindingsCompiler.GetSourceStructDescByID(EditorNode.GetNodeID()))
			{
				...
				// CreateBindingsForStruct Compile the regular FStateTreeDelegateDispatcher. Task completion delegate dispatchers need to be processed separately.
				if (TaskCompletionDispatcherIDs.Contains(EditorNode.GetNodeID()))
				{
					const FStateTreeBindableStructDesc& TargetStruct = *BindableNodeStruct;
					if (!BindingsCompiler.CompileTaskCompletionDelegateDispatchers(TargetStruct, EditorData->EditorBindings.GetBindings()))
					{
						return false;
					}
				}
...
{
	check(EditorBindingsNum == EditorData->EditorBindings.GetBindings().Num());
	for (TMultiMap<FGuid, const FPropertyBindingBinding*>::TKeyIterator It = BindingsByTargetStructID.CreateKeyIterator(StructID); It; ++It)
	{
		const FPropertyBindingBinding& Binding = *(It.Value());
		if (!ensure(Binding.GetTargetPath().GetStructID() == StructID))
		{
			continue;
		}
...
bool FStateTreeCompiler::CreatePropertyFunction(const FStateTreeEditorNode& FuncEditorNode)
{
	...
	if (FStateTreeInstanceObjectWrapper* ObjectWrapper = Instance.GetMutablePtr<FStateTreeInstanceObjectWrapper>())
	{
		check(FuncEditorNode.GetInstance().GetStruct() == ObjectWrapper->InstanceObject->GetClass());
		InstanceView = FStateTreeDataView(ObjectWrapper->InstanceObject);
	}
	else
	{
		check(FuncEditorNode.GetInstance().GetStruct() == Instance.GetScriptStruct());
		InstanceView = FStateTreeDataView(Instance);
	}
...
FInstancedStruct* FStateTreeCompiler::CreateNode(UStateTreeState* State, const FStateTreeEditorNode& EditorNode, FStateTreeBindableStructDesc& InstanceDesc, const FStateTreeDataHandle DataHandle, TArray<FInstancedStruct>& InstancedStructContainer)
{
	...
	if (!EditorNode.GetInstance().IsValid())
	{
	...
	FInstancedStruct& RawNode = Nodes.Add_GetRef(FInstancedStruct(EditorNode.GetNode()));
...
TOptional<FStateTreeDataView> FStateTreeCompiler::CreateNodeInstanceData(const FStateTreeEditorNode& EditorNode, FStateTreeNodeBase& Node, FStateTreeBindableStructDesc& StructDesc, const FStateTreeDataHandle DataHandle, TArray<FInstancedStruct>& InstancedStructContainer)
{
	...
	{
		const FStateTreeDataView InstanceView = EditorNode.GetInstance();
		if (InstanceView.IsValid())
		{
			if (InstanceView.GetStruct() == Node.GetInstanceDataType())
			{
				if (const UScriptStruct* InstanceStruct = Cast<const UScriptStruct>(InstanceView.GetStruct()))
				{
					// Struct Instance
					FInstancedStruct InstancedStruct;
					InstancedStruct.InitializeAs(InstanceStruct, static_cast<const uint8*>(InstanceView.GetMemory()));
					const int32 InstanceIndex = InstancedStructContainer.Add(MoveTemp(InstancedStruct));
					InstantiateStructSubobjects(InstancedStructContainer[InstanceIndex]);

					// Create binding source struct FInstancedStruct descriptor.
					StructDesc.Struct = InstanceStruct;
				...
				else if (const UClass* InstanceClass = Cast<const UClass>(InstanceView.GetStruct()))
				{
					const UObject* InstanceObject = InstanceView.GetPtr<const UObject>();
					UObject* Instance = UE::StateTree::Compiler::DuplicateInstanceObject(Log, StructDesc, EditorNode.ID, InstanceObject, StateTree);
					...
					StructDesc.Struct = InstanceClass;
				...
				{
					check(false); // Unsupported instance data type. It should be either UScriptStruct or UClass.
					return {};
				}
			}
			else
	...
	{
		const FStateTreeDataView ExecutionRuntimeView = EditorNode.GetExecutionRuntimeData();
		if (ExecutionRuntimeView.IsValid())
		{
			if (ExecutionRuntimeView.GetStruct() == Node.GetExecutionRuntimeDataType())
			{
				if (const UScriptStruct* ExecutionRuntimeStruct = Cast<const UScriptStruct>(ExecutionRuntimeView.GetStruct()))
				{
					// Struct Instance
					FInstancedStruct InstancedStruct;
					InstancedStruct.InitializeAs(ExecutionRuntimeStruct, static_cast<const uint8*>(ExecutionRuntimeView.GetMemory()));
					const int32 InstanceIndex = ExecutionRuntimeStructs.Add(MoveTemp(InstancedStruct));
				...
				else if (const UClass* ExecutionRuntimeClass = Cast<const UClass>(ExecutionRuntimeView.GetStruct()))
				{
					// Object Instance
					const UObject* ExecutionRuntimeObject = ExecutionRuntimeView.GetPtr<const UObject>();
					UObject* Instance = UE::StateTree::Compiler::DuplicateInstanceObject(Log, StructDesc, EditorNode.ID, ExecutionRuntimeObject, StateTree);
...
bool FStateTreeCompiler::CreateTaskCompletionDispatchers()
{
	// Transform the compiled version into the asset version.
	bool bTransitionSucceeded = true;
	for (const FStateTreePropertyBindingCompiler::FTaskCompletionDispatcher& Info : BindingsCompiler.GetCompiledTaskCompletionDispatchers())
	{
		if (const int32* NodeIndexPtr = IDToNode.Find(Info.TaskNodeID))
		{
			const int32 TaskIndex = *NodeIndexPtr;
			if (!StateTree->Nodes.IsValidIndex(TaskIndex))
			{
				Log.Reportf(EMessageSeverity::Error, TEXT("Internal error. A task completion delegate dispatcher index is invalid. (%d)"), TaskIndex);
				bTransitionSucceeded = false;
				continue;
			}

			FStateTreeTaskBase* Task = StateTree->Nodes[TaskIndex].GetPtr<FStateTreeTaskBase>();
			if (Task == nullptr)
			{
				Log.Reportf(EMessageSeverity::Error, TEXT("Internal error. A task completion delegate dispatcher index is not a task. (%d)"), TaskIndex);
				bTransitionSucceeded = false;
				continue;
			}
			Task->bHasTaskCompletionDelegateDispatcher = true;

			if (!Info.Dispatcher.IsValid())
			{
				Log.Reportf(EMessageSeverity::Error, TEXT("Internal error. A task completion delegate dispatcher is invalid. (%d)"), TaskIndex);
				bTransitionSucceeded = false;
				continue;
			}

			UE::StateTree::FTaskCompletionDispatcher CompiledDispatcher;
			CompiledDispatcher.TaskNodeIndex = FStateTreeIndex16(TaskIndex);
			CompiledDispatcher.Condition = Info.Condition;
			CompiledDispatcher.Dispatcher = Info.Dispatcher;
			StateTree->TaskCompletionDispatchers.Add(CompiledDispatcher);
		}
		else
		{
			Log.Reportf(EMessageSeverity::Error, TEXT("Internal error. Failed to resolve task node GUID for task completion dispatcher."));
			bTransitionSucceeded = false;
		}
	}
	return bTransitionSucceeded;
}
...
bool FStateTreeCompiler::ResolveTransitionStateAndFallback(const UStateTreeState* SourceState, const FStateTreeStateLink& Link, FStateTreeStateHandle& OutTransitionHandle, EStateTreeSelectionFallback& OutFallback) const 
{
	...
	else if (Link.LinkType == EStateTreeTransitionType::Parent)
	{
		// Find next parent.
		const UStateTreeState* Parent = SourceState && SourceState->Parent ? SourceState->Parent.Get() : nullptr;
		if (Parent == nullptr)
		{
			Log.Reportf(EMessageSeverity::Error,
				TEXT("Failed to resolve transition, there's no selectable parent state."));
			return false;
		}
		OutTransitionHandle = GetStateHandle(Parent->ID);
		if (!OutTransitionHandle.IsValid())
		{
			Log.Reportf(EMessageSeverity::Error,
				TEXT("Failed to resolve transition parent state, no handle found for '%s'."),
				*Parent->Name.ToString());
			return false;
		}
	}
	else if (Link.LinkType == EStateTreeTransitionType::NextState
		|| Link.LinkType == EStateTreeTransitionType::NextSelectableState)
	{
		// Find next state.
	...
	else if (Link.LinkType == EStateTreeTransitionType::NextParent
		|| Link.LinkType == EStateTreeTransitionType::NextSelectableParent)
	{
		// Find next parent.
		const UStateTreeState* NextParent = SourceState && SourceState->Parent ? SourceState->Parent->GetNextSelectableSiblingState() : nullptr;
		if (NextParent == nullptr)
		{
			Log.Reportf(EMessageSeverity::Error,
				TEXT("Failed to resolve transition, there's no selectable next state for the parent state."));
			return false;
		}
		OutTransitionHandle = GetStateHandle(NextParent->ID);
		if (!OutTransitionHandle.IsValid())
		{
			Log.Reportf(EMessageSeverity::Error,
				TEXT("Failed to resolve transition next state, no handle found for '%s'."),
				*NextParent->Name.ToString());
			return false;
		}
	}
...
bool FStateTreeCompiler::CreateTask(UStateTreeState* State, const FStateTreeEditorNode& TaskNode, const FStateTreeDataHandle TaskDataHandle)
{
	...
	if (!TaskNode.GetInstance().IsValid())
	{
		Log.Reportf(EMessageSeverity::Error, InstanceDesc,
	...
	// Copy the task
	const int32 NewNodeIndex = Nodes.Num();
	IDToNode.Add(TaskNode.ID, NewNodeIndex);	// for the instance data
	if (TaskCompletionDispatcherIDs.Contains(TaskNode.GetNodeID()))
	{
		IDToNode.Add(TaskNode.GetNodeID(), NewNodeIndex); // for the template data
	}
	FInstancedStruct& Node = Nodes.Add_GetRef(FInstancedStruct(TaskNode.GetNode()));
...
bool FStateTreeCompiler::CreateEvaluator(const FStateTreeEditorNode& EvalNode, const FStateTreeDataHandle EvalDataHandle)
{
	...
	// Check that node has valid instance initialized.
	if (!EvalNode.GetInstance().IsValid())
	{
		Log.Reportf(EMessageSeverity::Error, InstanceDesc,
	...
	FInstancedStruct& Node = Nodes.Add_GetRef(FInstancedStruct(EvalNode.GetNode()));
	InstantiateStructSubobjects(Node);
...
void FStateTreeCompiler::CreateBindingSourceStructsForNode(const FStateTreeEditorNode& EditorNode, const FStateTreeBindableStructDesc& InstanceDesc)
{
	...
	check(EditorNode.GetNode().IsValid());

	BindingsCompiler.AddSourceStruct(InstanceDesc);

	const bool bIsTaskCompletionNode = TaskCompletionDispatcherIDs.Contains(EditorNode.GetNodeID())
		&& EditorNode.GetNode().GetScriptStruct()->IsChildOf(FStateTreeTaskBase::StaticStruct());
	if (bIsTaskCompletionNode || IsNodeStructEligibleForBinding(EditorNode.GetNode()))
	{
		FStateTreeBindableStructDesc NodeDesc = InstanceDesc;
		NodeDesc.ID = EditorNode.GetNodeID();
		NodeDesc.Struct = EditorNode.GetNode().GetScriptStruct();
...
const FString& FStateTreeCompiler::GetStatePath(int32 StateIndex) const
{
	if (ensure(StatePaths.IsValidIndex(StateIndex)))
	{
