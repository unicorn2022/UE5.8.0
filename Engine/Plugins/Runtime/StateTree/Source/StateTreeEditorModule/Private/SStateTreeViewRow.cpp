...
// @TODO: Dependency onto state centric to enable hierarchy driven colors. 
// Should instead be driven by view model.
...
TSharedRef<SWidget> SStateTreeViewRow::MakeTasksWidget(const TSharedPtr<SScrollBox>& ViewBox)
{
	...
	for (int32 TaskIndex = 0; TaskIndex < NumTasks; TaskIndex++)
	{
		...
		if (const FStateTreeTaskBase* Task = TaskNode.GetNode().GetPtr<FStateTreeTaskBase>())
		{
			...
			auto IsTaskEnabledFunc = [WeakState = WeakState, TaskIndex]
				{
					...
					{
						if (const FStateTreeTaskBase* Task = State->Tasks[TaskIndex].GetNode().GetPtr<FStateTreeTaskBase>())
						{
...
TSharedRef<SWidget> SStateTreeViewRow::MakeConditionsWidget(const TSharedPtr<SScrollBox>& ViewBox)
{
	...
	if (!State->EnterConditions.IsEmpty())
	{
		...
		for (int32 ConditionIndex = 0; ConditionIndex < NumConditions; ConditionIndex++)
		{
			...
			if (const FStateTreeConditionBase* Condition = ConditionNode.GetNode().GetPtr<FStateTreeConditionBase>())
			{
				...
				auto IsForcedConditionVisibleFunc = [WeakState = WeakState, ConditionIndex]()
					{
						...
						{
							if (const FStateTreeConditionBase* Condition = State->EnterConditions[ConditionIndex].GetNode().GetPtr<FStateTreeConditionBase>())
							{
...
TSharedRef<SWidget> SStateTreeViewRow::MakeTransitionWidgetInternal(const EStateTreeTransitionTrigger Trigger, const FTransitionDescFilterOptions FilterOptions)
{
	...
	for (const FStateTreeTransition& Transition : State->Transitions)
	{
		...
		if (bMatch)
		{
			...
			if (Transition.ReactivateTargetState == EStateTreeTransitionChangeTypeRules::ForceChanged)
			{
				FItem ReselectionNote;
				ReselectionNote.Desc = LOCTEXT("ReactivateDesc", "(Re-Activate)");
				ReselectionNote.Tooltip = LOCTEXT("ReactivateDescToolTip", "The target state and all the following states will be reactivated. Their instance data will be reinstantiated and will receive EnterState.");
				ReselectionNote.bSkipComma = true;
				DescItems.Emplace(ReselectionNote);
			}
		}
	}
	...
	if (EnumHasAnyFlags(Trigger, EStateTreeTransitionTrigger::OnTick | EStateTreeTransitionTrigger::OnEvent | EStateTreeTransitionTrigger::OnDelegate))
	{
		...
		{
			AddLinksFromStruct(FStateTreeDataView(Task.GetNode().GetScriptStruct(), const_cast<uint8*>(Task.GetNode().GetMemory())), Task.ID);
			AddLinksFromStruct(Task.GetInstance(), Task.ID);
		}

		AddLinksFromStruct(FStateTreeDataView(State->SingleTask.GetNode().GetScriptStruct(), const_cast<uint8*>(State->SingleTask.GetNode().GetMemory())), State->SingleTask.ID);
	...
	if (bDisplayFlags && State && StateTree)
	{
		if (const FCompactStateTreeState* RuntimeState = StateTree->GetStateFromHandle(StateTree->GetStateHandleFromId(State->ID)))
		{
			...
			if (RuntimeState->bHasCustomTickRate
				|| RuntimeState->DoesRequestTickTasks(bHasEvents)
				|| RuntimeState->ShouldTickTransitions(bHasEvents, bHasBroadcastedDelegates))
			{
				bool bShow = true;
				...
				else if (RuntimeState->bCachedRequestTick)
				{
					Box->AddSlot()
					.AutoWidth()
					.VAlign(VAlign_Center)
				...
				//The task can tick, but it may not affect the scheduled system (see DelayTask as an example).
				else if (RuntimeState->bCachedRequestTickOnlyOnEvents)
				{
					Box->AddSlot()
					.AutoWidth()
					.VAlign(VAlign_Center)
				...
				else if (RuntimeState->bHasTransitionTasks)
				{
					Box->AddSlot()
					.AutoWidth()
					.VAlign(VAlign_Center)
					[
						...
						.Image(FStateTreeEditorStyle::Get().GetBrush("StateTreeEditor.Flags.Tick"))
						.ColorAndOpacity(IconTint)
						.ToolTipText(LOCTEXT("StateNodeTickTransitionTask", "The state contains at least one task that ticks at runtime when evaluating transitions."))
					];
				}
				else if (RuntimeState->bHasTickTriggerTransitions)
				{
					Box->AddSlot()
					.AutoWidth()
					.VAlign(VAlign_Center)
					[
						...
						.Image(FStateTreeEditorStyle::Get().GetBrush("StateTreeEditor.Flags.Tick"))
						.ColorAndOpacity(IconTint)
						.ToolTipText(LOCTEXT("StateNodeTickTransition", "The state contains at least one transition that ticks at runtime."))
					];
				}
				else if (RuntimeState->bHasEventTriggerTransitions)
				{
					Box->AddSlot()
					.AutoWidth()
					.VAlign(VAlign_Center)
					[
						...
						.Image(FStateTreeEditorStyle::Get().GetBrush("StateTreeEditor.Flags.TickOnEvent"))
						.ColorAndOpacity(IconTint)
						.ToolTipText(LOCTEXT("StateNodeTickTransitionEvent", "The state contains at least one transition that ticks at runtime when there's an event."))
					];
				}
				else
				{
					// Don't show when bHasDelegateTriggerTransitions. It is not considered ticking.
					// Don't show when the task ticked but not considered for scheduling.
					bShow = false;
				}
...
FSlateColor SStateTreeViewRow::GetTitleColor(const float Alpha, const float Lighten) const
{
	...
	if (State && StateTreeViewModel)
	{
		Color = StateTreeViewModel->GetStateColor(State);

		// @TODO: Dependency onto state centric to enable hierarchy driven colors. 
		// Should instead be driven by view model.
		if (UStateCentricViewSettings::IsStateCentricViewEnabled() && UStateCentricViewSettings::GetSchemaSettingsDefaultFallback(StateTreeViewModel.Get()).bEnableHierarchyDrivenColors)
		{
			Color = StateTreeViewModel->GetHierarchyBasedBackgroundColor(State);
		}

		if (IsRootState() || State->Type == EStateTreeStateType::Subtree)
		{
			Color = UE::StateTree::Editor::LerpColorSRGB(Color, FColor::Black, 0.25f);
		}
	}
...
FSlateColor SStateTreeViewRow::GetActiveStateColor() const
{
	if (const UStateTreeState* State = WeakState.Get())
	{
		...
		{
			return FStateTreeEditorStyle::Get().GetSlateColor("Colors.StateSelection");
		}
	}
...
EVisibility SStateTreeViewRow::GetTaskIconVisibility(FGuid TaskID) const
{
	...
	{
		if (const FStateTreeNodeBase* BaseNode = TaskNode->GetNode().GetPtr<const FStateTreeNodeBase>())
		{
...
EVisibility SStateTreeViewRow::GetConditionIconVisibility(FGuid ConditionID) const
{
	...
	{
		if (const FStateTreeNodeBase* BaseNode = Node->GetNode().GetPtr<const FStateTreeNodeBase>())
		{
			bHasIcon = !BaseNode->GetIconName().IsNone();
		}
	}
...
FText SStateTreeViewRow::GetTransitionsDesc(const EStateTreeTransitionTrigger Trigger, const FTransitionDescFilterOptions FilterOptions) const
{
	...
	if (EnumHasAnyFlags(Trigger, EStateTreeTransitionTrigger::OnTick | EStateTreeTransitionTrigger::OnEvent| EStateTreeTransitionTrigger::OnDelegate))
	{
		...
		for (const FStateTreeEditorNode& Task : State->Tasks)
		{
			AddLinksFromStruct(FStateTreeDataView(Task.GetNode().GetScriptStruct(), const_cast<uint8*>(Task.GetNode().GetMemory())));
			AddLinksFromStruct(Task.GetInstance());
		}

		AddLinksFromStruct(FStateTreeDataView(State->SingleTask.GetNode().GetScriptStruct(), const_cast<uint8*>(State->SingleTask.GetNode().GetMemory())));
...
*/
	...
	if (FMath::CountBits64(static_cast<uint64>(IconType)) > 1)
	{
...
EVisibility SStateTreeViewRow::GetTransitionsVisibility(const EStateTreeTransitionTrigger Trigger) const
{
	...
	if (EnumHasAnyFlags(Trigger, EStateTreeTransitionTrigger::OnTick | EStateTreeTransitionTrigger::OnEvent| EStateTreeTransitionTrigger::OnDelegate))
	{
		...
		for (const FStateTreeEditorNode& Task : State->Tasks)
		{
			if (HasAnyLinksInStruct(Task.GetNode())
				|| HasAnyLinksInStruct(Task.GetInstance())
				|| HasAnyLinksInStruct(Task.GetExecutionRuntimeData()))
			{
				return EVisibility::Visible;
			}
		}

		if (HasAnyLinksInStruct(State->SingleTask.GetNode())
			|| HasAnyLinksInStruct(State->SingleTask.GetInstance())
			|| HasAnyLinksInStruct(State->SingleTask.GetExecutionRuntimeData()))
		{
			return EVisibility::Visible;
		}
	}
...
TOptional<EItemDropZone> SStateTreeViewRow::HandleCanAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TWeakObjectPtr<UStateTreeState> TargetState) const
{
	...
	if (DragDropOperation.IsValid())
	{
		...
		if (DragDropOperation->IsDraggingNode())
		{
			const bool bIsSameState = DragDropOperation->OriginState == TargetState;
			DragDropOperation->SetCanDrop(!bIsSameState);
			if (bIsSameState || DropZone != EItemDropZone::OntoItem)
			{
...
FReply SStateTreeViewRow::HandleAcceptDrop(const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TWeakObjectPtr<UStateTreeState> TargetState) const
{
	...
	if (DragDropOperation.IsValid())
	{
		if (StateTreeViewModel)
		{
			if (DragDropOperation->IsDraggingNode())
			{
				if (DragDropOperation->bIsControlKeyDown)
				{
					StateTreeViewModel->CopyNode(DragDropOperation->OriginState.Get(), DragDropOperation->NodeID);
					StateTreeViewModel->PasteNodesToStates(MakeConstArrayView({TargetState.Get()}));
				}
				else
				{
					StateTreeViewModel->MoveNode(DragDropOperation->OriginState, TargetState, DragDropOperation->NodeID);
				}
			}
			else
			{
