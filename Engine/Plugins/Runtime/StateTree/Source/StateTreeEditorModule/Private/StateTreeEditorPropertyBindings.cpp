...
FPropertyBindingPath FStateTreeEditorPropertyBindings::AddFunctionBinding(const UScriptStruct* InPropertyFunctionNodeStruct, TConstArrayView<FPropertyBindingPathSegment> InSourcePathSegments, const FPropertyBindingPath& TargetPath)
{
	...
	UObject* OwnerObject = GetBindingsOwnerObject();
	check(OwnerObject);

	FStateTreeEditorNode PropertyFunction;
	PropertyFunction.InitializeAs(OwnerObject, InPropertyFunctionNodeStruct);

	Super::RemoveBindings(TargetPath, ESearchMode::Exact);

	FConstStructView PropertyFunctionView = FConstStructView::Make(PropertyFunction);
	FPropertyBindingPath SourcePath = FPropertyBindingPath(PropertyFunction.GetInstanceDataID(), InSourcePathSegments);
	PropertyBindings.Emplace(PropertyFunctionView, SourcePath, TargetPath);

	UpdateSegmentsForNewlyAddedBinding(PropertyBindings.Last());

	if (UStateTree* StateTree = OwnerObject->GetTypedOuter<UStateTree>())
	{
...
const FStateTreePropertyPathBinding* FStateTreeEditorPropertyBindings::AddTaskCompletionBinding(const FGuid& InSourceTask
	, const FPropertyBindingPath& InTargetPath
	, const UE::StateTree::ETaskCompletionCondition Completion)
{
	if (FStateTreePropertyPathBinding* AddedBinding = static_cast<FStateTreePropertyPathBinding*>(AddBindingInternal(FPropertyBindingPath(InSourceTask), InTargetPath)))
	{
		AddedBinding->SetCompletionBinding(Completion);
...
bool FStateTreeEditorPropertyBindings::IsDependentOn(TConstArrayView<UStruct*> InStructsDependedOn) const
{
	for (const UStruct* Struct : InStructsDependedOn)
	{
		if (CachedDependencies.Contains(Struct))
		{
			return true;
		}
	}
...
TArray<TNotNull<const UStruct*>> FStateTreeEditorPropertyBindings::GatherDependencies() const
{
...
void FStateTreeEditorPropertyBindings::CacheDependency(const TConstArrayView<TNotNull<const UStruct*>> InDependentStructs)
{
	CachedDependencies.Reset();
	CachedDependencies.Reserve(InDependentStructs.Num());

	Algo::Copy(InDependentStructs, CachedDependencies, Algo::NoRef);
}
		...
		CopyBindingsImplementation(FromStructID, ToStructID, [ToStructID, &FunctionBindingsToCopy](const FPropertyBindingBinding& Binding)
			{
				...
				if (bIsFunctionBinding)
				{
					...
					{
						FunctionBindingsToCopy.Emplace(EditorNode->GetNode().GetScriptStruct(), Binding.GetSourcePath(), FPropertyBindingPath(ToStructID, Binding.GetTargetPath().GetSegments()));
					}
				}
...
void FStateTreeEditorPropertyBindings::VisitBindings(FPropertyBindingIndex16 InBegin, FPropertyBindingIndex16 InEnd,
	TFunctionRef<EVisitResult(const FPropertyBindingBinding& Binding, const int32 BindingIndex)> InFunction) const
{
	...
	for (int32 BindingIndex = InBegin.Get(); BindingIndex < InEnd.Get(); ++BindingIndex)
	{
		if (InFunction(PropertyBindings[BindingIndex], BindingIndex) == EVisitResult::Break)
		{
			break;
		}
	}
...
FText FStateTreeBindingLookup::GetPropertyPathDisplayName(const FPropertyBindingPath& InPath, EStateTreeNodeFormatting Formatting) const
{
	...
	// If path's struct is a PropertyFunction, let it override a display name.
	{
		...
		if (BindingToPath && BindingToPath->GetPropertyFunctionNode().IsValid())
		{
			...
			const FStateTreeNodeBase& Node = EditorNode.GetNode().Get<FStateTreeNodeBase>();
