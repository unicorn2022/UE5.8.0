...
FLinearColor SDiffWidget::GetRowHighlightColor(const TUniquePtr<FAsyncDetailViewDiff::DiffNodeType>& DiffNode)
{
	...
	case ETreeDiffResult::MissingFromTree1:
		return FStateTreeEditorStyle::Get().GetSlateColor("Colors.Diff.MissingFromTree1").GetSpecifiedColor();
	case ETreeDiffResult::MissingFromTree2:
		return FStateTreeEditorStyle::Get().GetSlateColor("Colors.Diff.MissingFromTree2").GetSpecifiedColor();
	default:
		return FStateTreeEditorStyle::Get().GetSlateColor("Colors.Diff.Changed").GetSpecifiedColor();
	}
}
