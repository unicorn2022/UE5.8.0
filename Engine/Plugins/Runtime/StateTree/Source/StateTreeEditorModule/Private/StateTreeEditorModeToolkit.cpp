	...
	if (SpawnedTab.TabID == UE::StateTreeEditor::FWorkspaceTabHost::BindingTabId)
	{
		if (TSharedPtr<SDockTab> DockTab = SpawnedTab.DockTab.Pin())
		{
			...
			DockTab->SetContent(
				...
				.OnBindingClicked_Lambda([WeakEditorHost = EditorHost.ToWeakPtr()](bool bInIsStruct, const FPropertyBindingPath& InBindingPath)
					{
						if (const TSharedPtr<const IStateTreeEditorHost> EditorHost = WeakEditorHost.Pin())
						{
							if (UStateTree* StateTree = EditorHost->GetStateTree())
							{
								if (UStateTreeEditingSubsystem* StateTreeEditingSubsystem = GEditor->GetEditorSubsystem<UStateTreeEditingSubsystem>())
								{
									// Make an empty binding path only containing struct ID if it is a struct
									const FPropertyBindingPath& BindingPath = bInIsStruct ? FPropertyBindingPath(InBindingPath.GetStructID()) : InBindingPath;

									const TSharedRef<FStateTreeViewModel> ViewModel = StateTreeEditingSubsystem->FindOrAddViewModel(StateTree);
									ViewModel->BringBindingPathToFocus(BindingPath);
								}
							}
						}
					})
...
FSlateIcon FStateTreeEditorModeToolkit::GetCompileStatusImage() const
{
	...
	if (UStateTreeEditorMode* EditorMode = WeakEditorMode.Get())
	{
		...
		if (StateTree)
		{
			...
			else if (!StateTree->IsReadyToRun())
			{
...
UStateTree* FStateTreeEditorModeToolkit::GetStateTree() const
{
	return EditorHost ? EditorHost->GetStateTree() : nullptr;
}
