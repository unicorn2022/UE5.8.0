...
void SStateTreeDebuggerView::ConstructView(const TSharedRef<FUICommandList>& InCommandList)
{
	...
	LeftToolbar.BeginSection(TEXT("Debugging"));
	{
		...
		LeftToolbar.SetLabelVisibility(EVisibility::Collapsed);
		LeftToolbar.AddToolBarButton(DebuggerCommands.ResumeDebuggerAnalysis);
	}
	LeftToolbar.EndSection();
		...
		.OnGetMenuContent_Lambda([Debugger = Debugger]
			{
				...
				for (const TSharedRef<const UE::StateTreeDebugger::FInstanceDescriptor>& InstanceDescriptor : InstanceDescriptors)
				{
					...
					if (InstanceDescriptor.Get().StateTree != Debugger->GetAsset()
						&& !InstanceDescriptor.Get().LinkedStateTrees.Contains(Debugger->GetAsset()))
					{
						continue;
					}
					...
					FUIAction ItemAction(FExecuteAction::CreateSPLambda(Debugger.ToSharedRef(), [Debugger, InstanceId = InstanceDescriptor.Get().Id]
						{
							...
							{
								UE_LOGF(LogStateTree, Error, "Unable to access or create instance of RewindDebugger");
								return;
							}
	...
	// Deprecation warning message. Only visible when bDisplayMigrationToRewindDebuggerWarning.
	TSharedRef<SWidget> MigrationToRewindDebugger = SNullWidget::NullWidget;
	bDisplayMigrationToRewindDebuggerWarning = GetDefault<UStateTreeEditorUserSettings>()->GetDisplayMigrationToRewindDebuggerWarning();
	if (bDisplayMigrationToRewindDebuggerWarning)
	{
		const FButtonStyle* const CloseButtonStyle = &FAppStyle::Get().GetWidgetStyle<FDockTabStyle>("Docking.Tab").CloseButtonStyle;
		MigrationToRewindDebugger = SNew(SWarningOrErrorBox)
			...
			.Visibility(MakeAttributeSPLambda(AsShared(), [Self = this]() -> EVisibility
				{
					return Self->bDisplayMigrationToRewindDebuggerWarning ? EVisibility::Visible : EVisibility::Collapsed;
				}))
			.ContentVAlign(EVerticalAlignment::VAlign_Top)
			[
				SNew(SButton)
					.ButtonStyle(CloseButtonStyle)
					.ContentPadding(FMargin(0.0f, 1.5f, 0.0f, 0.0f))
					.ToolTipText(LOCTEXT("MigrationToRewindDebuggerCloseTooltip", "Close the warning box."))
					.OnClicked(FOnClicked::CreateSPLambda(AsShared(), [Self = this]() -> FReply
						{
							Self->bDisplayMigrationToRewindDebuggerWarning = false;
							GetMutableDefault<UStateTreeEditorUserSettings>()->SetDisplayMigrationToRewindDebuggerWarning(false);
...
void SStateTreeDebuggerView::OnBreakpointHit(const FStateTreeInstanceDebugId InstanceId, const FStateTreeDebuggerBreakpoint Breakpoint, const TSharedRef<FUICommandList> ActionList) const
{
	...
	// Bring attention to the Editor associated to the breakpoint
	if (const TSharedPtr<IToolkit> FoundAssetEditor = FToolkitManager::Get().FindEditorForAsset(StateTree.Get()))
	{
