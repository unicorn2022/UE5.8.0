...
bool FRewindDebuggerTrackCreator::HasDebugInfoInternal(const RewindDebugger::FObjectId& InObjectId) const
{
	if (const IRewindDebugger* RewindDebugger = IRewindDebugger::Instance())
	{
		if (const TraceServices::IAnalysisSession* Session = RewindDebugger->GetAnalysisSession())
		{
			...
			{
				TraceServices::FProviderReadScopeLock ProviderReadScope(*StateTreeTraceProvider);
...
{
	return FStateTreeInstanceData::StaticStruct()->GetFName();
}
	...
	, ObjectId(InObjectId)
{
	...
	SAssignNew(CompactTreeView, UE::StateTree::Editor::SCompactTreeDebuggerView, Descriptor->StateTree.Get())
...
FText FRewindDebuggerTrack::GetDisplayNameInternal() const
{
	...
	{
		if (UStateTreeEditorSettings::Get().DebuggerTrackNameVerbosity == EStateTreeDebuggerTrackNameVerbosity::Verbose)
		{
			return FText::FromString(FString::Printf(TEXT("%s StateTree (%s)")
				, *Descriptor->Name
				, *GetNameSafe(Descriptor->StateTree.Get())));
		}
		return FText::FromString(GetNameSafe(Descriptor->StateTree.Get()));
	}
...
TOptional<double> FRewindDebuggerTrack::GetStepFrameTimeInternal(const RewindDebugger::EStepMode StepMode, const FScrubTimeInformation& ScrubTimeInformation) const
{
	FScrubState ScrubState(&EventCollection);
