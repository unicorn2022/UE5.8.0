...
{
	DumpToLog(nullptr, Category);
}

void FStateTreeCompilerLog::DumpToLog(const UStateTree* Asset, const FLogCategoryBase& Category) const
{
	if (Messages.Num() > 0 && Asset)
	{
		UE_LOG_REF(Category, Log, TEXT("Compile asset '%s' "), *Asset->GetPathName());
	}
