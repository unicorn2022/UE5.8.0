...
struct FTestStateTreeExecutionContext : public FStateTreeExecutionContext
{
	FTestStateTreeExecutionContext(UObject& InOwner, const UStateTree& InStateTree, FStateTreeInstanceData& InInstanceData, const FOnCollectStateTreeExternalData& CollectExternalDataCallback = {}, const EStateTreeRecordTransitions RecordTransitions = EStateTreeRecordTransitions::No)
		: FTestStateTreeExecutionContext(&InOwner, &InStateTree, InInstanceData, CollectExternalDataCallback, RecordTransitions)
	{
	}

	FTestStateTreeExecutionContext(TNotNull<UObject*> InOwner, TNotNull<const UStateTree*> InStateTree, FStateTreeInstanceData& InInstanceData, const FOnCollectStateTreeExternalData& CollectExternalDataCallback = {}, const EStateTreeRecordTransitions RecordTransitions = EStateTreeRecordTransitions::No)
...
namespace UE::StateTree::Test
{
	UENUM()
	enum class ECppClassEnumByte : uint8
	{
		A = 0,
		B = 0x11,
		C = 0x22,
	...
	UENUM()
	enum class ECppClassEnumInt : uint32
	{
		A = 0,
		B = 0xF333F333,
		C = 0xF444F444,
	...
	struct FStructWithEnum
	{
		...
		UPROPERTY()
		ECppClassEnumByte CppClassEnumByte = ECppClassEnumByte::A;
		UPROPERTY()
		ECppClassEnumInt CppClassEnumInt = ECppClassEnumInt::A;
		UPROPERTY()
		TEnumAsByte<ENamespaceEnumByte::Type> NamespaceEnumByte = ENamespaceEnumByte::A;
		UPROPERTY()
		TEnumAsByte<EEnumByte> EnumByte = EEnumByte_A;
	};

	USTRUCT()
	struct FStructWithEnumRef
	{
		GENERATED_BODY()

		UPROPERTY()
		TStateTreePropertyRef<ECppClassEnumByte> CppClassEnumByte;
		UPROPERTY()
		TStateTreePropertyRef<ECppClassEnumInt> CppClassEnumInt;
		UPROPERTY()
		TStateTreePropertyRef<TEnumAsByte<ENamespaceEnumByte::Type>> NamespaceEnumByte;
		UPROPERTY()
		TStateTreePropertyRef<TEnumAsByte<EEnumByte>> EnumByte;
	};
}
...
struct FTestEval_A : public FStateTreeEvaluatorBase
{
	...
	virtual ~FTestEval_A() override = default;
...
struct FTestTask_B : public FStateTreeTaskBase
{
	...
	virtual ~FTestTask_B() override = default;
...
struct FTestTask_PrintValue : public FStateTreeTaskBase
{
	...
	virtual ~FTestTask_PrintValue() override = default;
...
USTRUCT(meta = (Hidden))
struct FTestCondition_PrintValue : public FStateTreeConditionCommonBase
{
	GENERATED_BODY()
	...
	FTestCondition_PrintValue() = default;
	FTestCondition_PrintValue(const FName InName)
	{
		Name = InName;
	}
	virtual ~FTestCondition_PrintValue() override {}
	...
	virtual bool TestCondition(FStateTreeExecutionContext& Context) const override
	{
		FStateTreeTestLog& TestLog = Context.GetExternalData(LogHandle);
		const FInstanceDataType& InstanceData = Context.GetInstanceData(*this);
		TestLog.Log(Name, FString::Printf(TEXT("TestCondition%d"), InstanceData.Value));
		TestLog.Log(Name, FString::Printf(TEXT("TestCondition:%s"), *InstanceData.GetArrayDescription()));

		if (CustomTestConditionFunc)
		{
			return CustomTestConditionFunc(Context, this);
		}

		return true;
	}

	TFunction<bool(FStateTreeExecutionContext&, const FTestCondition_PrintValue*)> CustomTestConditionFunc;
	TStateTreeExternalDataHandle<FStateTreeTestLog> LogHandle;
};
...
struct FTestTask_StopTree : public FStateTreeTaskBase
{
	...
	virtual ~FTestTask_StopTree() override = default;
...
struct FTestTask_Stand : public FStateTreeTaskBase
{
	...
	virtual ~FTestTask_Stand() = default;
	...
	virtual EStateTreeRunStatus Tick(FStateTreeExecutionContext& Context, const float DeltaTime) const override
	{
		...
		TestLog.Log(Name, FString::Printf(TEXT("CurrentTick=%u"), InstanceData.CurrentTick));
...
USTRUCT()
struct FTestTask_StandNoStateChangeOnReselect : public FTestTask_Stand
{
	GENERATED_BODY()

	FTestTask_StandNoStateChangeOnReselect()
	{
		bShouldStateChangeOnReselect = false;
	}

	FTestTask_StandNoStateChangeOnReselect(const FName InName)
		: FTestTask_Stand(InName)
	{
		bShouldStateChangeOnReselect = false;
	}
...
struct FTestTask_PropertyRefOnNodeAndInstance_InstanceData
{
	...
	UPROPERTY(EditAnywhere, Category = Parameter, meta = (RefType = "int32", Optional))
...
USTRUCT()
struct FTestCondition_PropertyRefOnNodeAndInstance : public FStateTreeConditionBase
{
	GENERATED_BODY()

	using FInstanceDataType = FTestTask_PropertyRefOnNodeAndInstance_InstanceData;

	FTestCondition_PropertyRefOnNodeAndInstance() = default;
	FTestCondition_PropertyRefOnNodeAndInstance(FName InName)
	{
		Name = InName;
	}
	...
	virtual bool TestCondition(FStateTreeExecutionContext& Context) const override
	{
		if (CustomTestConditionFunc)
		{
			CustomTestConditionFunc(Context, *this);
		}
	...
	TFunction<void(FStateTreeExecutionContext&, const FTestCondition_PropertyRefOnNodeAndInstance&)> CustomTestConditionFunc;
};
...
USTRUCT()
struct FStateTreeTest_PropertyStructObjectPtr
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, Category = "")
	TObjectPtr<UObject> A;

	UPROPERTY(EditAnywhere, Category = "")
	TWeakObjectPtr<UObject> B;

	UPROPERTY(EditAnywhere, Category = "")
	TSoftObjectPtr<UObject> C;

	UPROPERTY(EditAnywhere, Category = "")
	TArray<TObjectPtr<UObject>> ArrayA;

	UPROPERTY(EditAnywhere, Category = "")
	TArray<TWeakObjectPtr<UObject>> ArrayB;

	UPROPERTY(EditAnywhere, Category = "")
	TArray<TSoftObjectPtr<UObject>> ArrayC;
};
...
UE_STATETREE_ZEROED_TRIVIALLY_COPIED_NO_DESTRUCTOR_INSTANCEDATA(FTestPropertyFunction_InstanceData);
...
UE_STATETREE_CONSTRUCTED_TRIVIALLY_COPIED_NO_DESTRUCTOR_INSTANCEDATA(FStateTreeTestBooleanConditionInstanceData);
...
struct FTestTask_BroadcastDelegate : public FStateTreeTaskBase
{
	...
	UPROPERTY(EditAnywhere, Category = Parameter)
	EStateTreeRunStatus TickCompletionResult = EStateTreeRunStatus::Running;
};
