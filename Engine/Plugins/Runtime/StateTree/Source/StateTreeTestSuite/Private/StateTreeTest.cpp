...
struct FStateTreeTest_MakeAndBakeStateTree : FStateTreeTestBase
{
	virtual bool InstantTest() override
	{
		...
		auto& IntCond = StateA.AddEnterCondition<FStateTreeCompareIntCondition>(UE::StateTree::EComparisonOperator::Less);
		...
		auto& TransFloatCond = Trans.AddConditionWithOuter<FStateTreeCompareFloatCondition>(&StateB, UE::StateTree::EComparisonOperator::Less);
...
struct FStateTreeTest_SubTree_Condition : FStateTreeTestBase
{
	virtual bool InstantTest() override
	{
		...
		*/
		...
		TStateTreeEditorNode<FStateTreeCompareIntCondition>& IntCond1 = State1.AddEnterCondition<FStateTreeCompareIntCondition>(UE::StateTree::EComparisonOperator::Greater);
...
struct FStateTreeTest_OverrideStartState : FStateTreeTestBase
{
	virtual bool InstantTest() override
	{
		//Tree
		...
		//  Root2
		//	  Child4
		//  Root3 SelectionBehavior: Utility
		//    Child5 score 0.5
		//    Child6 score 1
		UStateTree& StateTree = NewStateTree();
		{
			UStateTreeEditorData& EditorData = *Cast<UStateTreeEditorData>(StateTree.EditorData);
			UStateTreeState& Root1 = EditorData.AddSubTree(TEXT("Root1"));

			UStateTreeState& Child1 = Root1.AddChildState(TEXT("Child1"));

			UStateTreeState& Child2 = Root1.AddChildState(TEXT("Child2"));
			Child2.Tag = GetTestTag1();
			
			UStateTreeState& Child3 = Child2.AddChildState(TEXT("Child3"));
			Child3.Tag = GetTestTag2();

			UStateTreeState& Subtree1 = Root1.AddChildState(TEXT("Subtree1"));
			Subtree1.Type = EStateTreeStateType::Subtree;
			Subtree1.Tag = GetTestTag3();

			UStateTreeState& Root2 = EditorData.AddSubTree(TEXT("Root2"));
			Root2.Tag = GetTestTag4();
			UStateTreeState& Child4 = Root2.AddChildState(TEXT("Child4"));

			UStateTreeState& Root3 = EditorData.AddSubTree(TEXT("Root4"));
			Root3.SelectionBehavior = EStateTreeStateSelectionBehavior::TrySelectChildrenWithHighestUtility;
			{
				UStateTreeState& Child5 = Root3.AddChildState(TEXT("Child6"));
				Child5.Tag = GetTestTag5();
				TStateTreeEditorNode<FStateTreeConstantConsideration>& Child5ConsiderationEditorNode = Child5.AddConsideration<FStateTreeConstantConsideration>();
				Child5ConsiderationEditorNode.GetInstanceData().Constant = 0.5f;
			}
			{
				UStateTreeState& Child6 = Root3.AddChildState(TEXT("Child7"));
				TStateTreeEditorNode<FStateTreeConstantConsideration>& Child6ConsiderationEditorNode = Child6.AddConsideration<FStateTreeConstantConsideration>();
				Child6ConsiderationEditorNode.GetInstanceData().Constant = 1.f;
			}
		}
...

		{
			...
			// No start state override
			{
				EStateTreeRunStatus Status = Exec.Start();
				AITEST_TRUE("StateTree Active States should be in Root1/Child1",
					Exec.ExpectInActiveStates("Root1", "Child1"));
				AITEST_TRUE("StateTree should be running", Status == EStateTreeRunStatus::Running);
			...
			// Start state Child2
			{
				FStateTreeExecutionContext::FStartParameters::FStateToSelectOverrideArgs SelectStateOverrideArgs =
				{ GetTestTag1(), UStateTree::EStateGameplayTagQueryMethod::MatchesExact };

				EStateTreeRunStatus Status = Exec.Start(FStateTreeExecutionContext::FStartParameters{ .SelectStateOverrideArgs = SelectStateOverrideArgs });

				AITEST_TRUE("StateTree Active States should be in Root1/Child2/Child3 with Child2 override",
					Exec.ExpectInActiveStates("Root1", "Child2", "Child3"));
				AITEST_TRUE("StateTree should be running", Status == EStateTreeRunStatus::Running);
				Exec.LogClear();
			}
			...
			{
				FStateTreeExecutionContext::FStartParameters::FStateToSelectOverrideArgs SelectStateOverrideArgs =
				{ GetTestTag2(), UStateTree::EStateGameplayTagQueryMethod::MatchesExact };

				EStateTreeRunStatus Status = Exec.Start(FStateTreeExecutionContext::FStartParameters{ .SelectStateOverrideArgs = SelectStateOverrideArgs });

				AITEST_TRUE("StateTree Active States should be in Root1/Child2/Child3 with Child3 override",
					Exec.ExpectInActiveStates("Root1", "Child2", "Child3"));
				AITEST_TRUE("StateTree should be running", Status == EStateTreeRunStatus::Running);
				Exec.LogClear();
			}

			// Start state Subtree1
			{
				FStateTreeExecutionContext::FStartParameters::FStateToSelectOverrideArgs SelectStateOverrideArgs =
				{ GetTestTag3(), UStateTree::EStateGameplayTagQueryMethod::MatchesExact };
				
				EStateTreeRunStatus Status = Exec.Start(FStateTreeExecutionContext::FStartParameters{ .SelectStateOverrideArgs = SelectStateOverrideArgs });

				AITEST_TRUE("StateTree Active States should be in Subtree1 with Subtree1 override",
					Exec.ExpectInActiveStates("Subtree1"));
				AITEST_TRUE("StateTree should be running", Status == EStateTreeRunStatus::Running);
				Exec.LogClear();
			}
			...
			{
				FStateTreeExecutionContext::FStartParameters::FStateToSelectOverrideArgs SelectStateOverrideArgs =
				{ GetTestTag4(), UStateTree::EStateGameplayTagQueryMethod::MatchesExact };
				
				EStateTreeRunStatus Status = Exec.Start(FStateTreeExecutionContext::FStartParameters{ .SelectStateOverrideArgs = SelectStateOverrideArgs });

				AITEST_TRUE("StateTree Active States should be in Root2/Child4 with Root2 override",
					Exec.ExpectInActiveStates("Root2", "Child4"));
				AITEST_TRUE("StateTree should be running", Status == EStateTreeRunStatus::Running);
				Exec.LogClear();
			}
			...
			{
				FStateTreeExecutionContext::FStartParameters::FStateToSelectOverrideArgs SelectStateOverrideArgs =
				{ GetTestTag5(), UStateTree::EStateGameplayTagQueryMethod::MatchesExact };
				
				EStateTreeRunStatus Status = Exec.Start(FStateTreeExecutionContext::FStartParameters{ .SelectStateOverrideArgs = SelectStateOverrideArgs });

				AITEST_TRUE("StateTree Active States should be in Root4/Child6 with Child6 override",
					Exec.ExpectInActiveStates("Root4", "Child6"));
				AITEST_TRUE("StateTree should be running", Status == EStateTreeRunStatus::Running);
				Exec.LogClear();
			}
...
IMPLEMENT_STATE_TREE_INSTANT_TEST(FStateTreeTest_OverrideStartState, "System.StateTree.OverrideStartState");
