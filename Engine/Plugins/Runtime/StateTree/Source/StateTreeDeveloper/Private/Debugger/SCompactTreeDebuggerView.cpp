	...
	auto AddStateItem = [&InParentItem, State, StateTree, InStateIdx](const TSharedRef<FStateItem>& StateItem, const bool bIsActiveState)
		{
			...
			StateItem->CustomData.GetMutable<CompactTreeView::FStateItemDebuggerData>().bIsActive = bIsActiveState;
...
TSharedRef<SCompactTreeView::FStateItem> SCompactTreeDebuggerView::CreateStateItemInternal() const
{
	...
	StateItem->CustomData = TInstancedStruct<CompactTreeView::FStateItemDebuggerData>::Make({});
	return StateItem;
}
		...
		[
			...
			.BorderBackgroundColor_Lambda(
				[Item]
				{
					...
					{
						if (StateItem->CustomData.Get<CompactTreeView::FStateItemDebuggerData>().bIsActive)
						{
