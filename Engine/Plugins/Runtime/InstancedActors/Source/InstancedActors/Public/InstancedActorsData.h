...
public:
	...
	// Note: does NOT nullify the shared fragment, that is done in Deinitialize(), which is only called on true teardown
	// and not on subsystem switch, allowing the fragment to be reused by an immediate respawn.
	UE_API void DespawnEntities();

	UE_DEPRECATED(5.8, "Using DespawnEntities with a parameter is deprecated.")
	UE_API void DespawnEntities(bool bIsSwitchingSubsystems);

	// Called from the parent's AInstancedActorsManager::EndPlay. Nullifies the shared fragment and can release
	// the entity template and exemplar Actor from memory.
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category=InstancedActors)
...
#if WITH_EDITOR
	// Called from GetOrCreateActorInstanceData after EditorPreviewISMComponents are first created,
	// allowing subclasses to customize them (e.g. apply material overrides).
	virtual void OnEditorPreviewISMCsCreated()
	{
	}
#endif
...
public:
	...
	/**
	 * Moves the current list of entities pending destruction (added via QueueEntitiesForDestruction) to InOutEntities
	 * This 'consumes' these pending entities with the assumption they will get destroyed by next command flush.
	 * @see UInstancedActorsRuntimeRemovalProcessor, RuntimeRemoveInstances
	 */
	void PopAllEntitiesPendingDestruction(TArray<FMassEntityHandle>& InOutEntities);
...
private:
	void QueueEntitiesForDestruction(TArray<FMassEntityHandle>&& EntitiesToDestroy);

	mutable FCriticalSection PendingDestructionCS;

	// These will get destroyed next time commands are flushed.
	// @see QueueEntitiesForDestruction, PopAllEntitiesPendingDestruction
