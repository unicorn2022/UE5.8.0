...
void UInstancedActorsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		UE_CLOGF(LODSubsystem->IsUsingPlayerPawnLocationInsteadOfCamera() == false
			, LogInstancedActors, Log, "Using Player's camera location for instanced actors LOD calculations - this can skew the LOD calculations in non-FPP games.");
	}
...
bool UInstancedActorsSubsystem::ExecutePendingDeferredSpawnEntitiesRequests(double StopAfterSeconds)
{
	...
	UE_CLOGF(!bExecutedAllPending, LogInstancedActors, Verbose, "UInstancedActorsSubsystem deferring %d remaining spawn entities requests to next frame", PendingManagersToSpawnEntities.Num());
...
FSharedStruct UInstancedActorsSubsystem::CompileSettingsForActorClass(TSubclassOf<AActor> ActorClass) const
{
	...
	if (!bFoundClassSettings && InstancedActorsCVars::RuntimeEnforceActorClassSettingsPresence > 0 && GetWorldRef().IsGameWorld())
	{
		...
		{
			UE_LOGF(LogInstancedActors, Error, "No instanced InstancedActorsClassSettings entry found in ActorClassSettingsRegistry for %ls or it's super classes", *ActorClass->GetPathName());
		}
	}
...
void UInstancedActorsSubsystem::MarkIADDirtyWithEntitiesPendingDestruction(UInstancedActorsData& InstancedActorsData)
{
	FScopeLock Lock(&DirtyIadWithEntitiesPendingDestructionCS);
	DirtyIadWithEntitiesPendingDestruction.AddUnique(&InstancedActorsData);
}

void UInstancedActorsSubsystem::PopAllDirtyIADWithEntitiesPendingDestruction(TArray<TWeakObjectPtr<UInstancedActorsData>>& OutInstancedActorsData)
{
	FScopeLock Lock(&DirtyIadWithEntitiesPendingDestructionCS);
	OutInstancedActorsData.Append(MoveTemp(DirtyIadWithEntitiesPendingDestruction));
}
