...
namespace UE::InstancedActors
{
	namespace CVars
	{
		...
		bool bDeferRuntimeRemoveInstancesEntityDestruction = true;
		FAutoConsoleVariableRef CVarbDeferRuntimeRemoveInstancesEntityDestruction(
			TEXT("IA.DeferRuntimeRemoveInstancesEntityDestruction"),
			bDeferRuntimeRemoveInstancesEntityDestruction,
			TEXT("If enabled, instanced actors entity destruction is deferred instead of being synchronous when removing the instance at runtime."),
			ECVF_Default);
	} // CVars
...
{
	// Nullify the shared fragment now that this IAD is truly going away. This is intentionally not done in
	// DespawnEntities() so that on subsystem switch (where Deinitialize is not called), the fragment remains
	// intact and can be reused by the immediately following respawn.
	...
	if (FInstancedActorsDataSharedFragment* AsSharedManagerFragment = SharedFragmentInstance.GetPtr<FInstancedActorsDataSharedFragment>())
	{
		if (ensure(AsSharedManagerFragment->InstanceData == this))
		{
...
void UInstancedActorsData::SpawnEntities()
{
	...
	// Spawn entities
	UE_LOGF(LogInstancedActors, Verbose, "\t%ls spawning %u entities", *GetDebugName(/*bCompact*/ true), NumValidInstances);
...
void UInstancedActorsData::DespawnEntities(bool /*bIsSwitchingSubsystems*/)
{
	DespawnEntities();
}
...
FInstancedActorsInstanceHandle UInstancedActorsData::AddInstance(const FTransform& Transform, const bool bWorldSpace)
{
	...
	// Add a new instance
	else if (!ensureMsgf(InstanceTransforms.Num() < FInstancedActorsInstanceIndex::INSTANCED_INDEX_MAX, TEXT("Instance Actors do not support more than 65534 instances per cell. Make the cells smaller or lower the density of actors.")))
	{
		return FInstancedActorsInstanceHandle();
	}
	else
...
void UInstancedActorsData::PopAllEntitiesPendingDestruction(TArray<FMassEntityHandle>& InOutEntities)
{
    FScopeLock Lock(&PendingDestructionCS);
    InOutEntities.Append(MoveTemp(EntitiesPendingDestruction));
}
...
void UInstancedActorsData::ApplyInstanceDeltas()
{
	...
	UE_LOGF(LogInstancedActors, Verbose, "Applying %d instance deltas (D: %u, L: %u, LT: %u) to %ls", Deltas.Num(), InstanceDeltas.GetNumDestroyedInstanceDeltas(), InstanceDeltas.GetNumLifecyclePhaseDeltas(), InstanceDeltas.GetNumLifecyclePhaseTimeElapsedDeltas(), *GetDebugName());
#endif
...
{
#if WITH_INSTANCEDACTORS_DEBUG
	UE_LOGF(LogInstancedActors, Verbose, "Applying %d instance deltas to %ls", InstanceDeltaIndices.Num(), *GetDebugName());
#endif
    ...
    {
        if (!ensureMsgf(Deltas.IsValidIndex(InstanceDeltaIndex), TEXT("Invalid InstanceDelta index %d (Num=%d) in ApplyInstanceDeltas"), InstanceDeltaIndex, Deltas.Num()))
        {
            continue;
        }
...
{
#if WITH_INSTANCEDACTORS_DEBUG
	UE_LOGF(LogInstancedActors, Verbose, "Rolling back %d instance deltas on %ls", InstanceDeltaIndices.Num(), *GetDebugName());
#endif
...
void UInstancedActorsData::RuntimeRemoveInstances(TConstArrayView<FInstancedActorsInstanceIndex> InstancesToRemove)
{
	...
	if (HasSpawnedEntities())
	{
		...
		// for these entities.
		if (UE::InstancedActors::CVars::bDeferRuntimeRemoveInstancesEntityDestruction)
		{
			QueueEntitiesForDestruction(MoveTemp(EntitiesToDestroy));
			if (UInstancedActorsSubsystem* IASubsystem = GetManagerChecked().GetInstancedActorSubsystem())
			{
				IASubsystem->MarkIADDirtyWithEntitiesPendingDestruction(*this);
			}
		}
		else
		{
...
void UInstancedActorsData::RuntimeRemoveAllInstances()
{
	...
	if (HasSpawnedEntities())
	{
		if (UE::InstancedActors::CVars::bDeferRuntimeRemoveInstancesEntityDestruction)
		{
			QueueEntitiesForDestruction(MoveTemp(Entities));
			if (UInstancedActorsSubsystem* IASubsystem = GetManagerChecked().GetInstancedActorSubsystem())
			{
				IASubsystem->MarkIADDirtyWithEntitiesPendingDestruction(*this);
			}
...
{
	UE_LOGF(LogInstancedActors, Verbose, "Ejecting instanced actor: %ls", *ActorToEject.GetPathName());
...
void UInstancedActorsData::QueueEntitiesForDestruction(TArray<FMassEntityHandle>&& EntitiesToDestroy)
{
	...
	{
		FScopeLock Lock(&PendingDestructionCS);
		EntitiesPendingDestruction.Append(MoveTemp(EntitiesToDestroy));
	}
}
