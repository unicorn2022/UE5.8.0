...
void AInstancedActorsManager::BeginPlay()
{
	...
	UE_LOGF(LogInstancedActors, Verbose, "%ls (%d instances) BeginPlay", *GetPathName(), GetNumValidInstances());
	UMassEntitySubsystem* EntitySubsystem = World->GetSubsystem<UMassEntitySubsystem>();
	if (!ensure(EntitySubsystem))
	{
		UE_LOGF(LogInstancedActors, Error, "%ls BeginPlay fail due to missing UMassEntitySubsystem", *GetPathName());
		return;
	}
...
void AInstancedActorsManager::Serialize(FStructuredArchive::FRecord Record)
{
	...
	if (UnderlyingArchive.IsSaveGame())
	{
		...
		UE_CLOGF(UnderlyingArchive.IsLoading(), LogInstancedActors, Verbose, "%ls loading persistent data", *GetPathName());
		UE_CLOGF(!UnderlyingArchive.IsLoading(), LogInstancedActors, Verbose, "%ls saving persistent data", *GetPathName());
		...
		for (int32 InstanceDataIndex = 0; InstanceDataIndex < NumInstanceDatas; ++InstanceDataIndex)
		{
			...
			if (UnderlyingArchive.IsLoading())
			{
				...
				{
					UE_LOGF(LogInstancedActors, Warning, "%ls - no IAD found with ID %u to restore persistent data. Data will be ignored and expunged on re-save", *GetPathName(), InstanceDataID);
				}
			}
			else
			{
...
void AInstancedActorsManager::SerializeInstancePersistenceData(FStructuredArchive::FRecord Record, UInstancedActorsData* InstanceData, int64 TimeDelta) const
{
	...
	int32 NumDestroyedInstances = InstanceData ? InstanceData->InstanceDeltas.GetNumDestroyedInstanceDeltas() : 0;
	FStructuredArchive::FArray DestroyedInstancesArray = Record.EnterArray(TEXT("DestroyedInstances"), NumDestroyedInstances);

	if (UnderlyingArchive.IsLoading())
	{
		for (int32 ArrayIndex = 0; ArrayIndex < NumDestroyedInstances; ++ArrayIndex)
		{
			const int32 NumInstances = InstanceData ? InstanceData->GetNumInstances() : 0;
			if (!ensureMsgf(InstanceData && ArrayIndex < NumInstances,
				TEXT("Attempting to destroy more instances (%d) than actually exist (%d). Aborting corrupted persistence archive read. Persistence data may be lost as a result."),
				NumDestroyedInstances, NumInstances))
			{
	...
	else
	{
		...
		int32 NumDestroyedInstancesWritten = 0;
	...
	if (UnderlyingArchive.IsLoading())
	{
		for (int32 IACIndex = 0; IACIndex < NumPersistedIACs; ++IACIndex)
		{
			...
			else
			{
				...
				UE_CLOGF(!InstanceData, LogInstancedActors, Error, "No UInstancedActorComponent with PersistenceID %u found due to no IAD found to get exemplar for! Skipping this IAC data block which will be lost on resave!", IACPersistenceID);
				UE_CLOGF(InstanceData, LogInstancedActors, Error, "No UInstancedActorComponent with PersistenceID %u found in %ls ExemplarActor. Skipping this IAC data block which will be lost on resave!", IACPersistenceID, *InstanceData->ActorClass->GetPathName());
...
UInstancedActorsData& AInstancedActorsManager::GetOrCreateActorInstanceData(TSubclassOf<AActor> ActorClass, const FInstancedActorsTagSet& AdditionalInstanceTags, bool bCreateEditorPreviewISMCs)
{
	...
	if (bCreateEditorPreviewISMCs)
	{
		...
		// Allow subclasses to customize the freshly created preview ISMCs (e.g. apply material overrides).
		NewInstanceData->OnEditorPreviewISMCsCreated();
	}
...
void AInstancedActorsManager::DespawnAllEntities(bool /*bIsSwitchingSubsystems*/)
{
	DespawnAllEntities();
}
...
#if WITH_EDITOR
	Ar.Logf(TEXT("\tRuntime grid: %s"), *GetRuntimeGrid().ToString());
#endif
...
void AInstancedActorsManager::AddModifierVolume(UInstancedActorsModifierVolumeComponent& ModifierVolume)
{
	...
	UE_LOGF(LogInstancedActors, Verbose, "%ls @ %ls adding modifier volume %ls @ %ls", *GetPathName(), *GetActorLocation().ToString(), *ModifierVolume.GetReadableName(), *ModifierVolume.GetComponentLocation().ToString());
#endif
...
#if WITH_INSTANCEDACTORS_DEBUG
	UE_LOGF(LogInstancedActors, Verbose, "%ls @ %ls removing modifier volume %ls @ %ls", *GetPathName(), *GetActorLocation().ToString(), *ModifierVolume.GetReadableName(), *ModifierVolume.GetComponentLocation().ToString());
#endif
...
{
	const bool bEnableInstanceCollision = (GetNetMode() == NM_Client) 
		? UE::InstancedActors::CVars::bInstanceCollisionsOnClient
		: UE::InstancedActors::CVars::bInstanceCollisionsOnServer;
...
FInstancedActorsInstanceHandle AInstancedActorsManager::ActorInstanceHandleFromFSMInstanceId(const FSMInstanceId& InstanceId) const
{
	...
	UE_LOGF(LogInstancedActors, Error, "%ls - no Instanced Actor found for %ls index %d.", *GetPathName(), *InstanceId.ISMComponent->GetName(), InstanceId.InstanceIndex);
