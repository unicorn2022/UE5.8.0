...
ESpawnRequestStatus UServerInstancedActorsSpawnerSubsystem::SpawnActor(const FSpawnArgs& Args, FActorSpawnParameters& InOutSpawnParameters) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UServerInstancedActorsSpawnerSubsystem::SpawnActor);

	UWorld* World = Args.World;
	const FMassActorSpawnRequest& SpawnRequest = Args.Request;
	TObjectPtr<AActor>& OutSpawnedActor = Args.Request.SpawnedActor;
	...
	if (ensureMsgf(OutSpawnedActor, TEXT("Failed to spawn actor of class %s"), *GetNameSafe(SpawnRequest.Template.Get())))
	{
		...
		// InOutSpawnParameters.CustomPreSpawnInitialization delegate
