...
void UInstancedActorsRepresentationActorManagement::SetActorEnabled(const EMassActorEnabledType EnabledType, AActor& Actor, const int32 EntityIdx, FMassCommandBuffer& CommandBuffer) const
{
	...
	if (EnabledType != EMassActorEnabledType::Disabled)
	{
		if (Actor.GetActorEnableCollision())
		{
			...
			CommandBuffer.PushCommand<FMassDeferredSetCommand>([WeakActor = TWeakObjectPtr<AActor>(&Actor)](FMassEntityManager&)
				{
...
AActor* UInstancedActorsRepresentationActorManagement::FindOrInstantlySpawnActor(UMassRepresentationSubsystem& RepresentationSubsystem, FMassEntityManager& EntityManager, FMassEntityView& EntityView)
{
	...
	if (SpawnedActor)
	{
		...
		// @todo Duplicated Code could be solved with a "trait" pattern - collect all relevant classes in one declaration per mesh-type and use that. 
		...
		{
			FMassInstancedSkinnedMeshInfoArrayView SkinnedMeshInfosView = RepresentationSubsystem.GetMutableInstancedSkinnedMeshInfos();

			const int32 SkinnedMeshInfoIndex = Representation.SkinnedMeshDescHandle.ToIndex();
			if (ensureMsgf(SkinnedMeshInfosView.IsValidIndex(SkinnedMeshInfoIndex), TEXT("Invalid handle index %u for SkinnedMeshInfosView"), SkinnedMeshInfoIndex))
			{
				FMassInstancedSkinnedMeshInfo& SkinnedMeshInfo = SkinnedMeshInfosView[SkinnedMeshInfoIndex];

				SkinnedMeshInfo.RemoveInstance(EntityView.GetEntity(), Representation.PrevLODSignificance);
				SkinnedMeshInfo.RemoveInstance(EntityView.GetEntity(), RepresentationLOD.LODSignificance);
			}
		}
