...
AGameplayCueNotify_Actor* UGameplayCueManager::FindExistingCueOnActor(const AActor& TargetActor, const TSubclassOf<AGameplayCueNotify_Actor>& CueClass, const FGameplayCueParameters& Parameters) const
{
	for (AActor* Child : TargetActor.Children)
	{
		if (IsValid(Child) && Child->IsA(CueClass))
		{
			...
			{
				UE_LOGF(LogAbilitySystem, Verbose, "FindExistingCueActor considered %ls, but it was pending remove", *GetNameSafe(ChildNotify));
				continue;
			}
...
AGameplayCueNotify_Actor* UGameplayCueManager::GetInstancedCueActor(AActor* TargetActor, UClass* GameplayCueNotifyActorClass, const FGameplayCueParameters& Parameters)
{
	...
	{
		UE_LOGF(LogAbilitySystem, Error, "GetInstancedCueActor called with invalid parameters (TargetActor = %ls, CueClass = %ls)", *GetNameSafe(TargetActor), *GetNameSafe(GameplayCueNotifyActorClass));
		return nullptr;
	}

	if (TargetActor && TargetActor->GetActorTransform().ContainsNaN())
	{
		UE_LOGF(LogAbilitySystem, Error, "GetInstancedCueActor called with invalid target actor transform (TargetActor = %ls)", *GetNameSafe(TargetActor));
		return nullptr;
	}
	...
	UE_CLOGF(WITH_EDITOR && TargetActor->HasAnyFlags(RF_ClassDefaultObject), LogAbilitySystem, Warning, "Adding %ls to CDO %ls. This used to be explicitly disallowed.", *GetNameSafe(CueClass), *GetNameSafe(TargetActor));

	UWorld* World = TargetActor->GetWorld();
	if (!World)
	{
		UE_LOGF(LogAbilitySystem, Warning, "GetInstancedCueActor called on TargetActor %ls which did not belong to a world (is it a CDO or being destroyed?)", *GetNameSafe(TargetActor));
		return nullptr;
	}
	UE_CLOGF(CurrentWorld != World, LogAbilitySystem, Error, "GetInstancedCueActor had CurrentWorld set to %ls but TargetActor is in World %ls", *GetNameSafe(CurrentWorld), *GetNameSafe(World));
	...
	if (bUseActorRecycling)
	{
		if (AGameplayCueNotify_Actor* RecycledCue = FindRecycledCue(CueClass, *World))
		{
			...
			UE_CLOGF((GameplayCueActorRecycleDebug > 0), LogAbilitySystem, Display, "GetInstancedCueActor reusing Recycled CueActor: %ls", *GetNameSafe(RecycledCue));
	...
	UE_CLOGF(LogGameplayCueActorSpawning > 0, LogAbilitySystem, Warning, "Spawned Gameplay Cue Notify Actor: %ls (instance %ls)", *CueClass->GetName(), *GetNameSafe(SpawnedCue));
	return SpawnedCue;
}
...
void UGameplayCueManager::NotifyGameplayCueActorFinished(AGameplayCueNotify_Actor* Actor)
{
	...
	if (bUseActorRecycling)
	{
		...
		UE_CLOGF((GameplayCueActorRecycleDebug > 0), LogAbilitySystem, Warning, "Recycling CueActor %ls", *GetNameSafe(Actor));
		...
		UE_CLOGF((GameplayCueActorRecycleDebug > 0), LogAbilitySystem, Error, "Could not Recycle CueActor %ls, Destroying", *GetNameSafe(Actor));
	}	
...
void UGameplayCueManager::AddGameplayCueNotifyPath(const FString& InPath, const bool bShouldRescanCueAssets /* = true */)
{
	...
	const bool bAlreadyExists = RuntimeGameplayCueObjectLibrary.Paths.Find(InPath) != INDEX_NONE;
	if (!bAlreadyExists)
	{
		RuntimeGameplayCueObjectLibrary.Paths.Add(InPath);

		if(bShouldRescanCueAssets)
		{
...
TSharedPtr<FStreamableHandle> UGameplayCueManager::InitObjectLibrary(FGameplayCueObjectLibrary& Lib)
{
	...
	// When running in editor, query in-memory assets so any unsaved assets are discovered. When cooking, only query on-disk assets to avoid indeterministic results
	if (!Lib.StaticObjectLibrary)
	{
		...
		if (GIsEditor)
		{
			Lib.StaticObjectLibrary->bIncludeOnlyOnDiskAssets = IsRunningCookCommandlet();
		}
	}
	if (!Lib.ActorObjectLibrary)
	{
		...
		if (GIsEditor)
		{
			Lib.ActorObjectLibrary->bIncludeOnlyOnDiskAssets = IsRunningCookCommandlet();
		}
	}	
...
void UGameplayCueManager::BuildCuesToAddToGlobalSet(TConstArrayView<FAssetData> AssetDataList, FName TagPropertyName, TArray<FGameplayCueReferencePair>& OutCuesToAdd, TArray<FSoftObjectPath>& OutAssetsToLoad, FShouldLoadGCNotifyDelegate ShouldLoad)
{
		...
		if (const UClass* ThisClass = It.Key)
		{
			if (const AGameplayCueNotify_Actor* CDO = ThisClass->GetDefaultObject<AGameplayCueNotify_Actor>())
			{
				...
				if (bWarnOnActiveActors)
				{
					...
					UE_CLOGF(StillActive > 0, LogAbilitySystem, Error, "  GameplayCueNotify Class '%ls' had %d instances still active.  This shouldn't happen.", *ThisClass->GetName(), StillActive);
				}
			}
		}
	}
