...
static TAutoConsoleVariable<bool> CVarAllowSimulatedMontageRestart(TEXT("AbilitySystem.AllowSimulatedMontageRestart"), true, TEXT("Due to bad lag times, montages may time out on Simulated clients. This variable allows simulated montages to be restarted in such scenarios."));

/**
 * Mark the passed-in GA (and any SubObjects) as Garbage
 */
void MarkAbilityAsGarbage(UGameplayAbility* GameplayAbility)
{
	if (GameplayAbility)
	{
		GameplayAbility->MarkAsGarbage();

		// We must also mark all SubObjects as garbage, gather any ones not already marked.
		TArray<UObject*> GASubObjects;
		constexpr EObjectFlags ExclusionFlags = EObjectFlags::RF_MirroredGarbage;
		GetObjectsWithOuter(GameplayAbility, GASubObjects, EGetObjectsFlags::IncludeNestedObjects, ExclusionFlags);

		for (UObject* GASubObject : GASubObjects)
		{
			GASubObject->MarkAsGarbage();
		}
	}
}
...
void UAbilitySystemComponent::InitializeComponent()
{
	...
	GetObjectsWithOuter(Owner, ChildObjects, EGetObjectsFlags::None, RF_NoFlags, EInternalObjectFlags::Garbage);
...
void UAbilitySystemComponent::OnGiveAbility(FGameplayAbilitySpec& Spec)
{
	...
	if (Spec.GameplayEffectHandle.IsValid())
	{
		...
		UE_CLOGF(!SourceASC, LogAbilitySystem, Error, "OnGiveAbility Spec '%ls' GameplayEffectHandle had invalid Owning Ability System Component", *Spec.GetDebugString());
		if (SourceASC)
		{
			FActiveGameplayEffect* SourceActiveGE = SourceASC->ActiveGameplayEffects.GetActiveGameplayEffect(Spec.GameplayEffectHandle);
			UE_CLOGF(!SourceActiveGE, LogAbilitySystem, Error, "OnGiveAbility Spec '%ls' GameplayEffectHandle was not active on Owning Ability System Component '%ls'", *Spec.GetDebugString(), *SourceASC->GetName());
			if (SourceActiveGE)
			{
	...
	RegisterAbilityTriggers(Spec, Spec.Ability->AbilityTriggers);
	RegisterAbilityTriggers(Spec, Spec.DynamicAbilityTriggers);
...
void UAbilitySystemComponent::RegisterAbilityTriggers(const FGameplayAbilitySpec& Spec, TConstArrayView<FAbilityTriggerData> AbilityTriggers)
{
	for (const FAbilityTriggerData& TriggerData : AbilityTriggers)
	{
		FGameplayTag EventTag = TriggerData.TriggerTag;

		TMap<FGameplayTag, TArray<FGameplayAbilitySpecHandle>>& TriggeredAbilityMap = (TriggerData.TriggerSource == EGameplayAbilityTriggerSource::GameplayEvent) ? GameplayEventTriggeredAbilities : OwnedTagTriggeredAbilities;
...
void UAbilitySystemComponent::OnRemoveAbility(FGameplayAbilitySpec& Spec)
{
	...
	UE_LOGF(LogAbilitySystem, Log, "%ls: Removing Ability [%ls] %ls Level: %d", *GetNameSafe(GetOwner()), *Spec.Handle.ToString(), *GetNameSafe(Spec.Ability), Spec.Level);
	...
	UnregisterAbilityTriggers(Spec, Spec.Ability->AbilityTriggers);
	UnregisterAbilityTriggers(Spec, Spec.DynamicAbilityTriggers);
	...
	if (PrimaryInstance)
	{
		...
		MarkAbilityAsGarbage(PrimaryInstance);
	}
	else
	{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (Spec.GameplayEffectHandle.IsValid()) // This can only be true on the network authority
	{
		...
		{
			FActiveGameplayEffect* SourceActiveGE = SourceASC->ActiveGameplayEffects.GetActiveGameplayEffect(Spec.GameplayEffectHandle);
			if (ensureMsgf(SourceActiveGE, TEXT("FActiveGameplayEffectHandle::GetOwningAbilitySystemComponent returned an ASC which did not contain the handle")))
			{
...
void UAbilitySystemComponent::UnregisterAbilityTriggers(const FGameplayAbilitySpec& Spec, TConstArrayView<FAbilityTriggerData> AbilityTriggers)
{
	for (const FAbilityTriggerData& TriggerData : AbilityTriggers)
	{
		FGameplayTag EventTag = TriggerData.TriggerTag;
	...
	if (!bDestroyActiveStateInitiated && ((GetFlags() & RF_BeginDestroyed) == 0))
	{
		...
		else
		{
			...
			for (FGameplayAbilitySpec& Spec : ActivatableAbilities.Items)
			{
				...
				for (UGameplayAbility* InstanceAbility : AbilitiesToCancel)
				{
					MarkAbilityAsGarbage(InstanceAbility);
				}
...
void UAbilitySystemComponent::OnRep_ActivateAbilities()
{
	...
	for (const FPendingAbilityInfo& PendingAbilityInfo : PendingCopy)
	{
		...
		if (!PendingServerActivatedAbilities.IsEmpty())
		{
			...
			UE_CLOGF(bIsSame, LogAbilitySystem, Warning, "Failed to execute Pending Ability %ls (Handle %ls) because it was not replicated in time for second-chance activation (it will be ignored)", *PendingAbilityInfo.PredictionKey.ToString(), *PendingAbilityInfo.Handle.ToString());
			UE_CLOGF(bHasNewItem, LogAbilitySystem, Warning, "New Pending Ability added during existing execution of %ls (Handle %ls). New ability will be ignored.", *PendingAbilityInfo.PredictionKey.ToString(), *PendingAbilityInfo.Handle.ToString());
...
bool UAbilitySystemComponent::InternalTryActivateAbility(FGameplayAbilitySpecHandle Handle, FPredictionKey InPredictionKey, UGameplayAbility** OutInstancedAbility, FOnGameplayAbilityEnded::FDelegate* OnGameplayAbilityEndedDelegate, const FGameplayEventData* TriggerEventData)
{
	...
	if (!bIsLocal)
	{
		if (Ability->GetNetExecutionPolicy() == EGameplayAbilityNetExecutionPolicy::LocalOnly || (Ability->GetNetExecutionPolicy() == EGameplayAbilityNetExecutionPolicy::LocalPredicted && !InPredictionKey.IsValidKey()))
		{
			...
			UE_LOGF(LogAbilitySystem, Warning, "%ls: Can't activate %ls ability %ls when not local", *GetNameSafe(GetOwner()), *UEnum::GetValueAsString<EGameplayAbilityNetExecutionPolicy::Type>(Ability->GetNetExecutionPolicy()), *Ability->GetName());
	...
	if (NetMode != ROLE_Authority && (Ability->GetNetExecutionPolicy() == EGameplayAbilityNetExecutionPolicy::ServerOnly || Ability->GetNetExecutionPolicy() == EGameplayAbilityNetExecutionPolicy::ServerInitiated))
	{
		UE_LOGF(LogAbilitySystem, Warning, "%ls: Can't activate %ls ability %ls when not the server", *GetNameSafe(GetOwner()), *UEnum::GetValueAsString<EGameplayAbilityNetExecutionPolicy::Type>(Ability->GetNetExecutionPolicy()), *Ability->GetName());
...
void UAbilitySystemComponent::PrePredictionActivation(const UGameplayAbility* Ability)
{
	...
	if (Ability && Ability->GetNetExecutionPolicy() == EGameplayAbilityNetExecutionPolicy::LocalPredicted)
    {
		const FGameplayAbilityActorInfo* ActorInfo = AbilityActorInfo.Get();
		if (ActorInfo && !ActorInfo->IsNetAuthority())
		{
			...
			{
				if (UCharacterMovementComponent* AvatarCharMoveComp = Cast<UCharacterMovementComponent>(AvatarCharacter->GetMovementComponent()))
				{
...
void UAbilitySystemComponent::RemoteEndOrCancelAbility(FGameplayAbilitySpecHandle AbilityToEnd, FGameplayAbilityActivationInfo ActivationInfo, bool bWasCanceled)
{
	...
	if (AbilitySpec && AbilitySpec->Ability && AbilitySpec->IsActive())
	{
		...
		else
		{
			...
			for (auto Instance : Instances)
			{
				UE_CLOGF(Instance == nullptr, LogAbilitySystem, Fatal, "UAbilitySystemComponent::RemoteEndOrCancelAbility null instance for %ls", *GetNameSafe(AbilitySpec->Ability));
...
void UAbilitySystemComponent::ClientActivateAbilitySucceedWithEventData_Implementation(FGameplayAbilitySpecHandle Handle, FPredictionKey PredictionKey, FGameplayEventData TriggerEventData)
{
	...
	if (!AbilityActorInfo->OwnerActor.IsValid())
	{
		...
		UE_LOGF(LogAbilitySystem, Verbose, "%ls: server confirmed [%ls] %ls (Prediction key %ls), but failed with invalid OwnerActor", *GetNameSafe(GetOwner()), *Handle.ToString(), *GetNameSafe(AbilityToActivate), *PredictionKey.ToString());
		return;
	}

	UE_LOGF(LogAbilitySystem, Verbose, "%ls: Server Confirmed [%ls] %ls. PredictionKey: %ls", *GetNameSafe(GetOwner()), *Handle.ToString(), *GetNameSafe(AbilityToActivate), *PredictionKey.ToString());
	...
	if (OwnedTagTriggeredAbilities.Contains(Tag))
	{
		...
		for (auto AbilityHandle : TriggeredAbilityHandles)
		{
			...
			if (Spec->Ability)
			{
				...
				AbilityTriggers.Append(Spec->DynamicAbilityTriggers);
				for (const FAbilityTriggerData& TriggerData : AbilityTriggers)
				{
					FGameplayTag EventTag = TriggerData.TriggerTag;
...
float UAbilitySystemComponent::PlayMontageInternal(UGameplayAbility* InAnimatingAbility, FGameplayAbilityActivationInfo ActivationInfo, UAnimMontage* NewAnimMontage, float InPlayRate, FName StartSectionName, float StartTimeSeconds, TFunctionRef<void(FGameplayAbilityRepAnimMontage&)> MutateRepAnimMontageFunction)
{
	...
	if (AnimInstance && NewAnimMontage)
	{
		...
		if (Duration > 0.f)
		{
			...
			{
				UE_LOGF(LogRootMotion, Log, "UAbilitySystemComponent::PlayMontage %ls, Role: %ls"
...
void UAbilitySystemComponent::OnRep_ReplicatedAnimMontage()
{
	...
	if (!AbilityActorInfo->IsLocallyControlled())
	{
		...
		if(ConstRepAnimMontageInfo.Animation)
		{
			...
			//Animations on sim clients may time out due to poor network conditions; so if the client triggering an ability continues to do so and the montage is stopped for sim clients, we restart the montage on sim clients.
			const bool bSimulatedMontageRequiresRestart = LocalAnimMontageInfo.AnimMontage && !ConstRepAnimMontageInfo.IsStopped && AnimInstance->Montage_GetIsStopped(LocalAnimMontageInfo.AnimMontage) 
				&& CVarAllowSimulatedMontageRestart.GetValueOnGameThread();
			...
			if ((LocalAnimation != ConstRepAnimMontageInfo.Animation) ||
				...
				bSimulatedMontageRequiresRestart)
			{
				LocalAnimMontageInfo.PlayInstanceId = ConstRepAnimMontageInfo.PlayInstanceId;
			...
			else if (!ConstRepAnimMontageInfo.SkipPositionCorrection)
			{
				...
				if( RepSectionID != INDEX_NONE )
				{
					...
					if( NextSectionID != RepNextSectionID )
					{
						...
						ABILITY_LOG(Verbose, TEXT("OnRep_ReplicatedAnimMontage: Setting NextSection of %s to %s"), *GetNameSafe(LocalAnimMontageInfo.AnimMontage), *LocalAnimMontageInfo.AnimMontage->GetSectionName(RepNextSectionID).ToString()); 
					}
					...
					if ((CurrentSectionID != RepSectionID) && (CurrentSectionID != RepNextSectionID))
					{
						...
						ABILITY_LOG(Verbose, TEXT("OnRep_ReplicatedAnimMontage: Setting Section Start Position of %s to %.3f"), *GetNameSafe(LocalAnimMontageInfo.AnimMontage), SectionStartTime); 
					}
				}
				...
				if ((CurrentSectionID == RepSectionID) && (FMath::Abs(DeltaPosition) > MONTAGE_REP_POS_ERR_THRESH) && (ConstRepAnimMontageInfo.IsStopped == 0))
				{
					...
					if (FAnimMontageInstance* MontageInstance = AnimInstance->GetActiveInstanceForMontage(LocalAnimMontageInfo.AnimMontage))
					{
						...
						if (DeltaTime >= 0.f)
						{
							...
							// @todo: Not sure how we haven't noticed missed notifies, since we are queueing AND firing notifies from the UAnimInstance we need to manually handle the unfiltered notifies.
							
							// To keep the anim graph notify states alive, we force to only trigger montage notifies.
							AnimInstance->TriggerAnimNotifies(DeltaTime, UAnimInstance::ENotifyTriggerMode::ForceMontageOnly);
						}
					}
					// @todo We should probably instead introduce the delta offset into the anim instance and have it trickle it out over several frames,
					// as it computes deltas to maintain smoothness, and to handle anim notifies automatically.
					// Forcing the position here is not a good way to do replication syncing for animations. -jwf
					AnimInstance->Montage_SetPosition(LocalAnimMontageInfo.AnimMontage, ConstRepAnimMontageInfo.Position);
					ABILITY_LOG(Verbose, TEXT("OnRep_ReplicatedAnimMontage: Setting Position of %s to %.3f"), *GetNameSafe(LocalAnimMontageInfo.AnimMontage), ConstRepAnimMontageInfo.Position); 
				}
			}
			...
			else
			{
				...
				if (RepSectionIdToPlay != INDEX_NONE && RepSectionIdToPlay != CurrentSectionID )
				{
					...
					{
						ABILITY_LOG(Verbose, TEXT("OnRep_ReplicatedAnimMontage: %s jumping to section %s"), *GetNameSafe(LocalAnimMontageInfo.AnimMontage), *CurrentSectionName.ToString());
...
FName UAbilitySystemComponent::CurrentMontageGetNextSectionName() const
{
	UAnimInstance* AnimInstance = AbilityActorInfo.IsValid() ? AbilityActorInfo->GetAnimInstance() : nullptr;
	if (LocalAnimMontageInfo.AnimMontage && AnimInstance)
	{
		...
		if (MontageInstance)
		{
			return MontageInstance->GetNextSection();
		}
	}
...
{
	UE_LOGF(LogAbilitySystem, Verbose, "SetMontageRepAnimPositionMethod %ls to %ls", *GetNameSafe(GetOwner()), *UEnum::GetValueAsString(InMethod));
...
{
	UE_CLOGF(AbilitySystemLogServerRPCBatching, LogAbilitySystem, Display, "::ServerAbilityRPCBatch_Implementation %ls %ls", *BatchInfo.AbilitySpecHandle.ToString(), *BatchInfo.PredictionKey.ToString());
...
{
	UE_CLOGF(AbilitySystemLogServerRPCBatching, LogAbilitySystem, Display, "::BeginServerAbilityRPCBatch %ls", *AbilityHandle.ToString());
...
void UAbilitySystemComponent::EndServerAbilityRPCBatch(FGameplayAbilitySpecHandle AbilityHandle)
{
	...
	if (idx != INDEX_NONE)
	{
		UE_CLOGF(AbilitySystemLogServerRPCBatching, LogAbilitySystem, Display, "::EndServerAbilityRPCBatch. Calling ServerAbilityRPCBatch. Handle: %ls. PredictionKey: %ls.", *LocalServerAbilityRPCBatchData[idx].AbilitySpecHandle.ToString(), *LocalServerAbilityRPCBatchData[idx].PredictionKey.ToString());
...
{
	UE_CLOGF(AbilitySystemLogServerRPCBatching, LogAbilitySystem, Display, "::CallServerSetReplicatedTargetData %ls %ls %ls %ls %ls", 
	...
	if (FServerAbilityRPCBatch* ExistingBatchData = LocalServerAbilityRPCBatchData.FindByKey(AbilityHandle))
	{
		if (!ExistingBatchData->Started)
		{
			...
			FGameplayAbilitySpec* Spec = FindAbilitySpecFromHandle(AbilityHandle);
			UE_CLOGF(AbilitySystemLogServerRPCBatching, LogAbilitySystem, Display, "::CallServerSetReplicatedTargetData called for ability (%ls) when CallServerTryActivateAbility has not been called", Spec ? *GetNameSafe(Spec->Ability) : TEXT("INVALID"));
...
{
	UE_CLOGF(AbilitySystemLogServerRPCBatching, LogAbilitySystem, Display, "::CallServerEndAbility %ls (%d %ls) %ls", *AbilityHandle.ToString(), ActivationInfo.bCanBeEndedByOtherInstance, *ActivationInfo.GetActivationPredictionKey().ToString(), *PredictionKey.ToString());
	...
	if (FServerAbilityRPCBatch* ExistingBatchData = LocalServerAbilityRPCBatchData.FindByKey(AbilityHandle))
	{
		if (!ExistingBatchData->Started)
		{
			// A batch window was setup but we didn't see the normal try activate -> target data -> end path. So let this unbatched rpc through.
			FGameplayAbilitySpec* Spec = FindAbilitySpecFromHandle(AbilityHandle);
			UE_CLOGF(AbilitySystemLogServerRPCBatching, LogAbilitySystem, Display, "::CallServerEndAbility called for ability (%ls) when CallServerTryActivateAbility has not been called", Spec ? *GetNameSafe(Spec->Ability) : TEXT("INVALID"));
