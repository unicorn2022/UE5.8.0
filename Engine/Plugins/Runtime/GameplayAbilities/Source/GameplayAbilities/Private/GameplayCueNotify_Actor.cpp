...
void AGameplayCueNotify_Actor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (UGameplayCueManager::IsGameplayCueRecylingEnabled() && (EndPlayReason == EEndPlayReason::Destroyed))
	{
		...
		{
			UE_LOGF(LogGameplayCueNotify, Error, "GameplayCueNotify [%ls] is calling EndPlay(). This should not happen since they are recycled through the GameplayCueManager.", *GetName());
		}
	}
...
void AGameplayCueNotify_Actor::K2_DestroyActor()
{
	...
	{
		UE_LOGF(LogGameplayCueNotify, Warning, "GameplayCueNotify [%ls] is calling DesotryActor(). This is not necessary as GCs will be cleaned up and recycled automatically.", *GetName());
	}
	else
	{
...
void AGameplayCueNotify_Actor::Destroyed()
{
	if (UGameplayCueManager::IsGameplayCueRecylingEnabled())
	{
		...
		if (World && !World->IsPlayingReplay() && (World->WorldType == EWorldType::Game))
		{
			UE_LOGF(LogGameplayCueNotify, Error, "GameplayCueNotify [%ls] is calling Destroyed(). This should not happen since they are recycled through the GameplayCueManager.", *GetName());
		}
	}
