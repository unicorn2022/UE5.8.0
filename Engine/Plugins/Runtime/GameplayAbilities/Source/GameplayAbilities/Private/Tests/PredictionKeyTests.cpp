...
private:
	...
	void UnbindDelegates(bool bWarnIfNotTriggered = true)
	{
		if (Key.Current > 0)
		{
			...
			UE_CLOGF(bWarnIfNotTriggered && NumRemoved > 0, LogPredictionKeyTests, Warning, "%s FPredictionKeyDelegate still had entry for %d on destruction", __func__, Key.Current);
		}
	}
