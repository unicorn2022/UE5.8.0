...
void UAdditionalEffectsGameplayEffectComponent::OnActiveGameplayEffectRemoved(const FGameplayEffectRemovalInfo& RemovalInfo, FActiveGameplayEffectsContainer* ActiveGEContainer) const
{
	...
	if (!ensure(ActiveGE))
	{
		UE_LOGF(LogGameplayEffects, Error, "FGameplayEffectRemovalInfo::ActiveEffect was not populated in OnActiveGameplayEffectRemoved");
		return;
	}
	...
	{
		UE_LOGF(LogGameplayEffects, Error, "ActiveGEContainer was invalid in OnActiveGameplayEffectRemoved");
		return;
	}
	...
	// Remove any on-application effects that were configured with RemoveGrantedEffectOnEnd
	for (const FConditionalGameplayEffect& ConditionalEffect : OnApplicationGameplayEffects)
	{
		if (ConditionalEffect.RemovalPolicy == EGameplayEffectGrantedEffectRemovalPolicy::RemoveGrantedEffectOnEnd
			&& ConditionalEffect.EffectClass != nullptr)
		{
			FGameplayEffectQuery RemoveQuery;
			RemoveQuery.EffectDefinition = ConditionalEffect.EffectClass;
			ActiveGEContainer->RemoveActiveEffects(RemoveQuery, ConditionalEffect.StackCountToRemove);
		}
	}
}
...
EDataValidationResult UAdditionalEffectsGameplayEffectComponent::IsDataValid(FDataValidationContext& Context) const
{
	...
	const bool bOwnerIsInstant = (GetOwner()->DurationPolicy == EGameplayEffectDurationType::Instant);
	...
	for (const FConditionalGameplayEffect& ConditionalEffect : OnApplicationGameplayEffects)
	{
		...
		if (bOwnerIsInstant)
		{
			Context.AddError(LOCTEXT("RemoveGrantedEffectOnEndWithInstantOwner",
				"RemoveGrantedEffectOnEnd cannot be used on an Instant owning GE. Instant effects are never added to the ActiveGE container, so the removal callback will never fire."));
			Result = EDataValidationResult::Invalid;
		}

		// The granted GE must be non-Instant: instant effects are applied and immediately gone, there is nothing to remove later
		if (ConditionalEffect.EffectClass != nullptr)
		{
			if (const UGameplayEffect* GrantedCDO = ConditionalEffect.EffectClass.GetDefaultObject())
			{
				if (GrantedCDO->DurationPolicy == EGameplayEffectDurationType::Instant)
				{
					Context.AddError(LOCTEXT("RemoveGrantedEffectOnEndWithInstantChild",
							"RemoveGrantedEffectOnEnd cannot be used with Instant granted effect. Instant effects are applied and immediately removed; there is nothing to clean up when the owning GE ends."));
					Result = EDataValidationResult::Invalid;
				}
			}
		}
