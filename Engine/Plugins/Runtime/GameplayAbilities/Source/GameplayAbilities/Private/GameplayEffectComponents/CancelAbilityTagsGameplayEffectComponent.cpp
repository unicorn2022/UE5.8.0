...
void UCancelAbilityTagsGameplayEffectComponent::CancelOwnerAbilities(const FActiveGameplayEffectsContainer& ActiveGEContainer) const
{
	...
	// Empty container will fail the HasAny check, so pass nullptr instead
	const FGameplayTagContainer* WithTags = !InheritableCancelAbilitiesWithTagsContainer.CombinedTags.IsEmpty() ? &InheritableCancelAbilitiesWithTagsContainer.CombinedTags : nullptr;
	const FGameplayTagContainer* WithoutTags = !InheritableCancelAbilitiesWithoutTagsContainer.CombinedTags.IsEmpty() ? &InheritableCancelAbilitiesWithoutTagsContainer.CombinedTags : nullptr;
	ASC->CancelAbilities(WithTags, WithoutTags);
}
