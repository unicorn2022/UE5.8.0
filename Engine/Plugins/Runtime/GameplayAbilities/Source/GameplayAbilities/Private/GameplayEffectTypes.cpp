...
TAutoConsoleVariable<bool> CVarReplicateTagCountContainerWithIris(
		...
		false,
		TEXT("Default: False.  When true will use Iris serialization for Gameplay Tag Count Container enabling tag rep state promotion with iris."),
		ECVF_Default);

TAutoConsoleVariable<bool> CVarGESimOnlyTags(
		TEXT("AbilitySystem.Fix.GESimOnlyTags"),
		true,
		TEXT("Default: True.  When active GE's will use the SimOnly tag replication state so that owning client lets the GEs manage their own tags"),
		ECVF_Default);
...
bool FGameplayTagCountContainer::NetDeltaSerialize(FNetDeltaSerializeInfo & DeltaParams)
{
	...
	// Determine if we are replicating as a NetOwner
	bool bNetOwner = false;
	if (DeltaParams.Connection)
	{
		AActor* OwnerActor = nullptr;
		if (UActorComponent* ParentComponent = Cast<UActorComponent>(DeltaParams.Object))
		{
			OwnerActor = ParentComponent->GetOwner();
		}
		else
		{
			OwnerActor = Cast<AActor>(DeltaParams.Object);
		}

		bNetOwner = OwnerActor && (OwnerActor->GetNetConnection() == DeltaParams.Connection);
	}
	...
	{		
		...
		*DeltaParams.NewState = TSharedPtr<INetDeltaBaseState>(NewState);
		...
		for (const FGameplayTagCountItem& CountItem : Items)
		{
			...
			UE_CLOGF(OldItem && OldItem->ReplicationState > CountItem.ReplicationState, LogAbilitySystem, Warning,
				"Existing tag: %ls with \"%ls\" replication state now serializing with state \"%ls\"", 
			...
			// if the state is SimulatedOnly we want to skip sending this tag to the owner
			if (CVarGESimOnlyTags.GetValueOnAnyThread() && CountItem.ReplicationState == EGameplayTagReplicationState::SimulatedTagOnly && bNetOwner)
			{
				continue;
			}
			...
			bPackCount = CountItem.ReplicationState >= (bNetOwner ? EGameplayTagReplicationState::CountToOwner : EGameplayTagReplicationState::TagAndCountToAll);
		...
		for (int idx = 0; idx < Removals.Num(); idx++)
		{
			// if the state is SimulatedOnly we want to skip sending this tag removal to the owner
			if (CVarGESimOnlyTags.GetValueOnAnyThread() && Removals[idx].ReplicationState == EGameplayTagReplicationState::SimulatedTagOnly && bNetOwner)
			{
				Removals.RemoveAtSwap(idx, EAllowShrinking::No);
				--idx;
			}
		}
		...
		for (int32& Idx : ChangeIdx)
		{
			...
			bPackCount = NewItem.ReplicationState >= (bNetOwner ? EGameplayTagReplicationState::CountToOwner : EGameplayTagReplicationState::TagAndCountToAll);
	...
	if (DeltaParams.Reader)
	{
		...
		if (NumChanges > 0)
		{
			...
			for (; NumChanges > 0; NumChanges--)
			{
				...
				// We need to reconstruct the ReplicationState because of the MultiServerProxy which needs to pass this
				// data along (and thus it needs to know what to Serialize). We start by assuming it's TagOnly.
				EGameplayTagReplicationState InferredReplicationState = EGameplayTagReplicationState::TagOnly;

				Reader.SerializeBits(&bPackCount, 1);
				if (bPackCount)
				{
					FGameplayTagCountItem* ExistingItem = Items.FindByPredicate(
						[&UpdatedItem](const FGameplayTagCountItem& ExistingItem)
						{
							return ExistingItem.Tag == UpdatedItem.Tag;
						});

					// If we're receiving a count, then we are at least CountToOwner, and possibly TagAndCountToAll if we ever receive this on a non-owning connection.
					InferredReplicationState = bNetOwner ? EGameplayTagReplicationState::CountToOwner : EGameplayTagReplicationState::TagAndCountToAll;
					InferredReplicationState = ExistingItem ? FMath::Max(ExistingItem->ReplicationState, InferredReplicationState) : InferredReplicationState;

					Reader.SerializeIntPacked(NewCount);
				}

				SetTagCount(UpdatedItem.Tag, NewCount, InferredReplicationState);
			}
		}
	}
	return true;
...
bool FGameplayTagCountContainer::UpdateExplicitTags(const FGameplayTag& Tag, const int32 CountDelta, const bool bDeferParentTagsOnRemove, EGameplayTagReplicationState TagRepState)
{
	...
	{
		ABILITY_LOG(Verbose, TEXT("Attempted to remove tag: %s from tag count container, but it is not explicitly in the container!"), *Tag.ToString());
		return false;
	}
...
void FMinimalReplicationTagCountMap::UpdateOwnerTagMap()
{
	...
	if (Owner)
	{
		// We need to copy the TagMap since changing a tag count can cause side effects such as adding or removing other tags
		TMap<FGameplayTag,int32> TagMapCopy = TagMap;
		for (auto It = TagMapCopy.CreateConstIterator(); It; ++It)
		{
			const FGameplayTag Tag = It->Key;
			const int32 NewValue = It->Value;

			Owner->SetTagMapCount(Tag, NewValue);
			UE_CLOGF((NewValue != It->Value), LogAbilitySystem, Warning, "Callback for SetTagMapCount(%ls, %d) caused its own TagCount to change to %d (likely an unwanted recursion in your registered TagChanged Listeners)", *Tag.ToString(), NewValue, It->Value);
		}
