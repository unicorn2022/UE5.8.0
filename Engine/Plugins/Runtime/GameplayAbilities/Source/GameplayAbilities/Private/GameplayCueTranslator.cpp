...
FGameplayCueTranslatorNodeIndex FGameplayCueTranslationManager::GetTranslationIndexForName(FName Name, bool CreateIfInvalid)
{
	...
	{
		UE_LOGF(LogGameplayCueTranslator, Log, ".....");
	}
#endif
				...
				for (int32 ToNameIdx=0; ToNameIdx < SwapRule.ToNames.Num() && TagIdx < SplitNames.Num(); ++ToNameIdx)
				{
					...
					if (SwapRule.ToNames[ToNameIdx] == SplitNames[TagIdx])
					{
						...
						if (ToNameIdx == SwapRule.ToNames.Num()-1)
						{
							...
							UE_LOGF(LogGameplayCueTranslator, Log, "Found possible expanded tag. Original Child Tag: %ls. Possible Parent Tag: %ls", *TagName.ToString(), *ComposedString);
							...
							// Look for this tag - is it an actual real tag? If not, continue on
							{
								...
								{
									UE_LOGF(LogGameplayCueTranslator, Log, "   No tag match found, recursing...");
							...
							if (HasValidRootTag)
							{
								...
								UE_LOGF(LogGameplayCueTranslator, Log, "   Matches real tags! Adding to translation tree");
							...
							{
								UE_LOGF(LogGameplayCueTranslator, Log, "   No tag match found after recursing. Dead end.");
							}
		...
		for (int32 SwapRuleIdx=0; SwapRuleIdx < NameSwapData.NameSwaps.Num(); ++SwapRuleIdx)
		{
			...
			for (int32 TagIdx=0; TagIdx < SplitNames.Num(); ++TagIdx)
			{
				if (SplitNames[TagIdx] == SwapRule.FromName)
				{
					...
					UE_LOGF(LogGameplayCueTranslator, Log, "Found possible new expanded tag. Original: %ls. Parent: %ls", *TagName.ToString(), *ComposedString);
					FName ComposedName = FName(*ComposedString);
...
bool FGameplayCueTranslationManager::TranslateTag_Internal(FGameplayCueTranslatorNode& Node, FGameplayTag& OutTag, const FName& TagName, AActor* TargetActor, const FGameplayCueParameters& Parameters)
{
	for (FGameplayCueTranslationLink& Link : Node.Links)
	{
		...
		if (TranslationIndex != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogGameplayCueTranslator, Error, "FGameplayCueTranslationManager::TranslateTag_Internal %ls invalid index %d was returned from GameplayCueToTranslationIndex. NodeLookup.Num=%d. Tag %ls", *GetNameSafe(Link.RulesCDO), TranslationIndex, Link.NodeLookup.Num(), *TagName.ToString());
				continue;
			}
			...
			if (NodeIndex.IsValid())
			{
				...
				{
					UE_LOGF(LogGameplayCueTranslator, Error, "FGameplayCueTranslationManager::TranslateTag_Internal %ls invalid index %d was returned from NodeLookup. NodeLookup.Num=%d. Tag %ls", *GetNameSafe(Link.RulesCDO), NodeIndex.Index, TranslationLUT.Num(), *TagName.ToString());
					continue;
				}
				...
				UE_LOGF(LogGameplayCueTranslator, Verbose, "Translating %ls --> %ls (via %ls)", *TagName.ToString(), *InnerNode.CachedGameplayTagName.ToString(), *GetNameSafe(Link.RulesCDO));
...
{
	UE_LOGF(LogGameplayCueTranslator, Display, "Printing GameplayCue Translation Table. * means tag is not created but could be.");
	...
	UE_LOGF(LogGameplayCueTranslator, Display, "");
	UE_LOGF(LogGameplayCueTranslator, Display, "Total Number of Translations with valid tags: %d", TotalNumTranslations);
	UE_LOGF(LogGameplayCueTranslator, Display, "Total Number of Translations without  valid tags: %d (theoretical translations)", TotalNumTheoreticalTranslations);
}
...
void FGameplayCueTranslationManager::PrintTranslationTable_r(FGameplayCueTranslatorNode& Node, FString IdentStr)
{
	if (Node.Links.Num() > 0)
	{
		if (IdentStr.IsEmpty())
		{
			UE_LOGF(LogGameplayCueTranslator, Display, "%ls %ls", *Node.CachedGameplayTagName.ToString(), Node.CachedGameplayTag.IsValid() ? TEXT("") : TEXT("*"));
		}
		...
		for (FGameplayCueTranslationLink& Link : Node.Links)
		{
			for (FGameplayCueTranslatorNodeIndex& Index : Link.NodeLookup)
			{
				if (Index.IsValid())
				{
					...
					{
						UE_LOGF(LogGameplayCueTranslator, Display, "%ls -> %ls [%ls]", *IdentStr, *InnerNode.CachedGameplayTag.ToString(), *GetNameSafe(Link.RulesCDO) );
						TotalNumTranslations++;
					}
					else
					{
						UE_LOGF(LogGameplayCueTranslator, Display, "%ls -> %ls [%ls] *", *IdentStr, *InnerNode.CachedGameplayTagName.ToString(), *GetNameSafe(Link.RulesCDO) );
			...
			for (FGameplayCueTranslationLink& Link : TranslationLUT[idx].Links)
			{
				for (FGameplayCueTranslatorNodeIndex& LinkIdx : Link.NodeLookup)
				{
					if (LinkIdx.Index == FindIndex)
					{
						...
						//UE_LOGF(LogGameplayCueTranslator, Display, "%ls links to %ls", *Node.CachedGameplayTag.ToString(), *TranslationLUT[FindIndex].CachedGameplayTag.ToString());
