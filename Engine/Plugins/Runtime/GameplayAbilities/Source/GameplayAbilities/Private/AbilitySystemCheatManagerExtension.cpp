...
void UAbilitySystemCheatManagerExtension::AbilityGrant(const FString& AssetSearchString) const
{
	...
	if (!ASC)
	{
		UE_LOGF(LogConsoleResponse, Log, "%ls did not have AbilitySystemComponent", *GetNameSafe(PC));
		return;
	}
	...
	{
		UE_LOGF(LogConsoleResponse, Log, "Could not find a valid Gameplay Ability based on Search String '%ls'", *AssetSearchString);
		if (PC->GetWorld()->IsPlayInEditor())
		{
			UE_LOGF(LogConsoleResponse, Log, "Reminder: If it's a non-native class, make sure it's loaded in the Editor.");
		}
	...
	{
		UE_LOGF(LogConsoleResponse, Log, "Existing Ability Spec '%ls' on Player '%ls' (It is already granted).", *GetNameSafe(*ActivateAbilityClass), *GetNameSafe(PC));
		return;
	}
	...
	if (!ASC->IsOwnerActorAuthoritative())
	{
		...
		UE_LOGF(LogConsoleResponse, Log, "Sent Command 'ServerExec %ls' from Player '%ls' to Server because only the authority can grant abilities.", *ServerCommand, *GetNameSafe(PC));
		return;
	}
	...
	{
		UE_LOGF(LogConsoleResponse, Log, "Successfully Granted '%ls' on Player '%ls'.", *GetNameSafe(ActivateAbilityClass.Get()), *GetNameSafe(PC));
	}
	else
	{
		UE_LOGF(LogConsoleResponse, Log, "Failed to Grant '%ls' on Player '%ls'.", *GetNameSafe(ActivateAbilityClass.Get()), *GetNameSafe(PC));
	}
}
...
void UAbilitySystemCheatManagerExtension::AbilityListGranted() const
{
	...
	UE_LOGF(LogConsoleResponse, Log, "Granted abilities to %ls (ASC: '%ls'):", *PC->GetName(), *ASC->GetFullName());
	...
	for (FGameplayAbilitySpec& Activatable : ASC->GetActivatableAbilities())
	{
		...
		UE_LOGF(LogConsoleResponse, Log, "   %ls (%ls - %ls) %ls", *Activatable.Ability->GetName(), *ExecutionEnumPtr->GetDisplayNameTextByIndex(Activatable.Ability->GetNetExecutionPolicy()).ToString(), *SecurityEnumPtr->GetDisplayNameTextByIndex(Activatable.Ability->GetNetSecurityPolicy()).ToString(), ActiveText);
	}
}
...
void UAbilitySystemCheatManagerExtension::AbilityActivate(const FString& PartialName) const
{
	...
	if (SearchName != NAME_None)
	{
		...
		if (GameplayTag.IsValid())
		{
			...
			if (MatchingSpecs.Num() > 0)
			{
				...
				if (bSuccess)
				{
					UE_LOGF(LogConsoleResponse, Log, "Requested Tag '%ls' successfully executed one of: %ls.", *SearchName.ToString(), *MatchingAbilitiesString);
				}
				else
				{
					UE_LOGF(LogConsoleResponse, Log, "Requested Tag '%ls' was expected to execute one of: %ls. But it failed to do so due to tag requirements.", *SearchName.ToString(), *MatchingAbilitiesString);
				}
			}
			else
			{
				UE_LOGF(LogConsoleResponse, Log, "Requested Tag '%ls' matched no given Gameplay Abilities to %ls.", *SearchName.ToString(), *GetNameSafe(PC));
			}
	...
	if (!ActivateAbilityClass)
	{
		UE_LOGF(LogConsoleResponse, Log, "Could not find a valid Gameplay Ability based on Search String '%ls'", *PartialName);
		return;
	}
	...
	if (!ASC->IsOwnerActorAuthoritative())
	{
		...
		{
			UE_LOGF(LogConsoleResponse, Log, "Cannot Activate '%ls' on '%ls' as the ability is not granted.  Since its NetExecutionPolicy must activate locally, you must first grant it (use the AbilityGrant command).", *GetNameSafe(ActivateAbilityClass.Get()), *GetNameSafe(PC));
			return;
		}
		...
		if (bSendToServer)
		{
			...
			UE_LOGF(LogConsoleResponse, Log, "Sent Command '%ls' from Player '%ls' to Server (Reason: Net Execution Policy).", *ServerCommand, *GetNameSafe(PC));
			return;
		}
	}
	...
	if (FGameplayAbilitySpec* GrantedAbilitySpec = ASC->FindAbilitySpecFromClass(ActivateAbilityClass))
	{
		...
		if (bSuccess)
		{
			UE_LOGF(LogConsoleResponse, Log, "Successfully Activated previously Granted Ability '%ls' on Player '%ls'.", *GetNameSafe(ActivateAbilityClass.Get()), *GetNameSafe(PC));
		}
		else
		{
			UE_LOGF(LogConsoleResponse, Log, "Failed to Activate previously Granted Ability '%ls' on Player '%ls'. Logs:", *GetNameSafe(ActivateAbilityClass.Get()), *GetNameSafe(PC));
	...
	if (SpecHandle.IsValid())
	{
		UE_LOGF(LogConsoleResponse, Log, "Successfully Granted, then Activated '%ls' on Player '%ls'.", *GetNameSafe(ActivateAbilityClass.Get()), *GetNameSafe(PC));
	}
	else
	{
		UE_LOGF(LogConsoleResponse, Log, "Failed to Grant & Activate '%ls' on Player '%ls'. Logs:", *GetNameSafe(ActivateAbilityClass.Get()), *GetNameSafe(PC));
...
void UAbilitySystemCheatManagerExtension::AbilityCancel(const FString& PartialName) const
{
	...
	if (!ASC)
	{
		UE_LOGF(LogConsoleResponse, Log, "PlayerController %ls did not have an AbilitySystemComponent", *GetNameSafe(PC));
		return;
	}
		...
		if (GASpec.IsActive())
		{
			...
			if (ActiveAbilities.Num() > 0)
			{
				for (UGameplayAbility* Instance : ActiveAbilities)
				{
					if (Instance)
					{
						UE_LOGF(LogConsoleResponse, Log, "%ls (%ls): Cancelling (instanced) %ls", *PC->GetName(), *ASC->GetName(), *Instance->GetName());
			...
			{
				UE_LOGF(LogConsoleResponse, Log, "%ls (%ls): Cancelling (non-instanced) %ls", *PC->GetName(), *ASC->GetName(), *GASpec.Ability->GetName());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (!bFound)
	{
		UE_LOGF(LogConsoleResponse, Log, "Found no Active Gameplay Abilities on %ls (%ls) that matched '%ls'", *PC->GetName(), *ASC->GetName(), *SearchString);
	}
}
...
void UAbilitySystemCheatManagerExtension::EffectListActive() const
{
	...
	UE_LOGF(LogConsoleResponse, Log, "Active Gameplay Effects on %ls (%ls):", *PC->GetName(), *ASC->GetName());
	...
	for (const FActiveGameplayEffectHandle& ActiveGEHandle : ActiveGEContainer.GetAllActiveEffectHandles())
	{
		if (const FActiveGameplayEffect* ActiveGE = ASC->GetActiveGameplayEffect(ActiveGEHandle))
		{
			...
			UE_LOGF(LogConsoleResponse, Log, " [%u] %ls: %ls", GetTypeHash(ActiveGEHandle), *GetNameSafe(GESpecDef), *ActiveState);
		}
	}
}
...
void UAbilitySystemCheatManagerExtension::EffectApply(const FString& PartialName, float EffectLevel /*= FGameplayEffectConstants::INVALID_LEVEL*/) const
{
	...
	{
		UE_LOGF(LogConsoleResponse, Log, "Could not find a valid Gameplay Effect based on Search String '%ls'", *PartialName);
		return;
	}
	...
	{
		UE_LOGF(LogConsoleResponse, Log, "Successfully Applied (Non-Instant) Gameplay Effect '%ls' on Player '%ls'.", *GameplayEffectClass->GetName(), *PC->GetName());
	}
	else if (ActiveGEHandle.WasSuccessfullyApplied())
	{
		UE_LOGF(LogConsoleResponse, Log, "Executed (Instant) Gameplay Effect '%ls' on Player '%ls'.", *GameplayEffectClass->GetName(), *PC->GetName());
	}
	else
	{
		UE_LOGF(LogConsoleResponse, Log, "Failed to Apply Gameplay Effect '%ls' on Player '%ls'.", *GameplayEffectClass->GetName(), *PC->GetName());
	}
}
...
void UAbilitySystemCheatManagerExtension::EffectRemove(const FString& NameOrHandle) const
{
	...
	for (const FActiveGameplayEffectHandle& ActiveGEHandle : ActiveGEContainer.GetAllActiveEffectHandles())
	{
		if (const FActiveGameplayEffect* ActiveGE = ASC->GetActiveGameplayEffect(ActiveGEHandle))
		{
			...
			if (bMatch)
			{
				UE_LOGF(LogConsoleResponse, Log, "Removing Active GE Handle %u (%ls)", ActiveGEHandleValue, *GESpecDef->GetName());
	...
	if (!bAnyMatch)
	{
		UE_LOGF(LogConsoleResponse, Log, "Found no Active Gameplay Effects on %ls (%ls) that matched '%ls'", *PC->GetName(), *ASC->GetName(), *NameOrHandle);
	}
}
	...
	FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World, FOutputDevice& OutputDevice)
		{
			...
			if (bExecuteOnServer)
			{
				...
				if (bArgIsHandle)
				{
					UE_LOGF(LogConsoleResponse, Log, "Error: Search by Handle Value is not permitted with -server (because ActiveGE Handles are not replicated to the same handle value)");
					return;
				}
			}
