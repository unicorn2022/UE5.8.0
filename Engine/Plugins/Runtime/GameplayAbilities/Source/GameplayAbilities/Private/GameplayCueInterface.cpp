...
DEFINE_LOG_CATEGORY_STATIC(LogGameplayCues, Log, All);
	...
	#define CANTREPLICATEPARAMS_SEVERITY Warning
#else
	#define CANTREPLICATEPARAMS_SEVERITY Verbose
#endif
...
void IGameplayCueInterface::HandleGameplayCue(UObject* Self, FGameplayTag GameplayCueTag, EGameplayCueEvent::Type EventType, const FGameplayCueParameters& Parameters)
{
	...
	{
		UE_LOGF(LogGameplayCues, Verbose, "%ls::%s %ls %ls", *GetNameSafe(Self), __func__, *GameplayCueTag.ToString(), *UEnum::GetValueAsString(EventType));
...
void FActiveGameplayCue::PreReplicatedRemove(const struct FActiveGameplayCueContainer &InArray)
{
	...
	if (UE_LOG_ACTIVE(LogGameplayCues, VeryVerbose))
	{
		...
		UE_LOGF(LogGameplayCues, VeryVerbose, "%ls.%ls::%s %ls %ls", *GetNameSafe(PropertyOwner), *PropertyName, __func__, *GameplayCueTag.ToString(), *PredictionKey.ToString());
...
void FActiveGameplayCueContainer::AddCue(const FGameplayTag& Tag, const FPredictionKey& PredictionKey, const FGameplayCueParameters& Parameters)
{
	...
	if (UE_LOG_ACTIVE(LogGameplayCues, VeryVerbose))
	{
		...
		UE_LOGF(LogGameplayCues, VeryVerbose, "%ls.%ls::%s %ls %ls", *GetNameSafe(PropertyOwner), *PropertyName, __func__, *Tag.ToString(), *PredictionKey.ToString());
...
void FActiveGameplayCueContainer::RemoveCue(const FGameplayTag& Tag)
{
	...
	if (UE_LOG_ACTIVE(LogGameplayCues, VeryVerbose))
	{
		const UObject* PropertyOwner = GameplayCueInterfacePrivate::GetPropertyOwner(*this);
		const FString PropertyName = GameplayCueInterfacePrivate::FindNameOfProperty(this, PropertyOwner);
		UE_LOGF(LogGameplayCues, VeryVerbose, "%ls.%ls::%s %ls", *GetNameSafe(PropertyOwner), *PropertyName, __func__, *Tag.ToString());
...
void FMinimalGameplayCueReplicationProxy::SetOwner(UAbilitySystemComponent* ASC)
{
	...
	{
		const bool bLocationsValid = ensure(ReplicatedLocations.Num() >= ReplicatedTags.Num());
		...
		for (int32 idx=0; idx < ReplicatedTags.Num(); ++idx)
		{
			if (bLocationsValid)
			{
				Parameters.Location = ReplicatedLocations[idx].IsZero() ? OriginalLocationParameter : ReplicatedLocations[idx];
			}
			Owner->SetTagMapCount(ReplicatedTags[idx], 1);
			Owner->InvokeGameplayCueEvent(ReplicatedTags[idx], EGameplayCueEvent::WhileActive, Parameters);
		}
	}
}
...
void FMinimalGameplayCueReplicationProxy::PreReplication(const FActiveGameplayCueContainer& SourceContainer)
{
	if (LastSourceArrayReplicationKey != SourceContainer.ArrayReplicationKey)
	{
		...
		if (UE_LOG_ACTIVE(LogGameplayCues, CANTREPLICATEPARAMS_SEVERITY))
		{
			// Check if any new cue has parameters that we won't be able to replicate and warn the user
			FGameplayCueParameters ParamsThatCanReplicate;
			for (const FActiveGameplayCue& Cue : SourceContainer.GameplayCues)
			{
				// Look for new tags
				if (!ReplicatedTags.Contains(Cue.GameplayCueTag))
				{
					ParamsThatCanReplicate.bReplicateLocationWhenUsingMinimalRepProxy = Cue.Parameters.bReplicateLocationWhenUsingMinimalRepProxy;
					ParamsThatCanReplicate.Location = ParamsThatCanReplicate.bReplicateLocationWhenUsingMinimalRepProxy ? Cue.Parameters.Location : FVector::ZeroVector;

					// Those don't replicate
					ParamsThatCanReplicate.MatchedTagName = Cue.Parameters.MatchedTagName;
					ParamsThatCanReplicate.OriginalTag = Cue.Parameters.OriginalTag;

					// Effect context is default constructed at some places so do not warn about it
					ParamsThatCanReplicate.EffectContext = Cue.Parameters.EffectContext;

					if (Cue.Parameters != ParamsThatCanReplicate)
					{
						TPropertyValueIterator<const FProperty> CueParams(FGameplayCueParameters::StaticStruct(), &Cue.Parameters);
						TPropertyValueIterator<const FProperty> ReplicatedParams(FGameplayCueParameters::StaticStruct(), &ParamsThatCanReplicate);

						for (; CueParams && ReplicatedParams; ++CueParams, ++ReplicatedParams)
						{
							if (!ensure(CueParams->Key != nullptr))
							{
								continue;
							}

							if (!CueParams->Key->Identical(CueParams->Value, ReplicatedParams->Value))
							{
								UE_LOGF(LogGameplayCues, CANTREPLICATEPARAMS_SEVERITY, "%s: Parameter %ls on gameplay cue %ls is set but cannot be replicated", __FUNCTION__, *CueParams->Key->GetName(), *Cue.GameplayCueTag.ToString());
							}
						}
					}
				}
...
#undef CANTREPLICATEPARAMS_SEVERITY