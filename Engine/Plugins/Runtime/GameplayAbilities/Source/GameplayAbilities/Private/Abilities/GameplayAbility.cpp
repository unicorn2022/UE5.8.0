...
bool UGameplayAbility::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, OUT FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (AbilitySystemComponent->IsAbilityInputBlocked(Spec->InputID))
	{
		if (FScopedCanActivateAbilityLogEnabler::IsLoggingEnabled())
		{
			UE_LOGF(LogAbilitySystem, Verbose, "%ls: %ls could not be activated due to blocked input ID %d", *GetNameSafe(ActorInfo->OwnerActor.Get()), *GetNameSafe(Spec->Ability), Spec->InputID);
...
void UGameplayAbility::CancelAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateCancelAbility)
{
	if (CanBeCanceled())
	{
		if (ScopeLockCount > 0)
		{
			UE_LOGF(LogAbilitySystem, Verbose, "Attempting to cancel Ability %ls but ScopeLockCount was greater than 0, adding cancel to the WaitingToExecute Array", *GetName());
...
AActor* UGameplayAbility::GetOwningActorFromActorInfo() const
{
	if (!ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetOwningActorFromActorInfo() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this)))
	{
		return nullptr;
	}
...
AActor* UGameplayAbility::GetAvatarActorFromActorInfo() const
{
	ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetAvatarActorFromActorInfo() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this));
...
{
	ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetOwningComponentFromActorInfo() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this));
...
{
	ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetAbilitySystemComponentFromActorInfo() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this));
...
{
	if (!ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetCurrentActorInfo() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this)))
	{
		return nullptr;
	}
...
{
	if (!ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetCurrentActivationInfo() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this)))
	{
...
{
	if (!ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetCurrentAbilitySpecHandle() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this)))
	{
...
{
	if (ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("SetCurrentMontage() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this)))
	{
...
{
	ensureMsgf(UE::AbilitySystem::Private::CVarAllowNonInstancedGAsValue || IsInstantiated(),
		TEXT("GetCurrentMontage() called on the CDO for %s.  NonInstanced abilities are deprecated, thus we always expect this to be called on an instanced object."), *GetNameSafe(this));
...
void UGameplayAbility::OnGameplayTaskActivated(UGameplayTask& Task)
{
	...
	UE_LOGF(LogGameplayTasks, VeryVerbose, "GameplayAbility Task Started %ls", *Task.GetName());
...
void UGameplayAbility::OnGameplayTaskDeactivated(UGameplayTask& Task)
{
	...
	UE_LOGF(LogGameplayTasks, VeryVerbose, "GameplayAbility Task Ended %ls", *Task.GetName());

	ActiveTasks.Remove(&Task);

	if constexpr (ENABLE_ABILITYTASK_DEBUGMSG)
	{
...
bool UGameplayAbility::IsTriggered() const
{
	...
	if (AbilityTriggers.IsEmpty())
	{
		...
		return AbilitySpec != nullptr && !AbilitySpec->DynamicAbilityTriggers.IsEmpty();
	}
...
FActiveGameplayEffectHandle UGameplayAbility::ApplyGameplayEffectSpecToOwner(const FGameplayAbilitySpecHandle AbilityHandle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEffectSpecHandle SpecHandle) const
{
	...
	if (SpecHandle.IsValid() && ActorInfo && (HasAuthorityOrPredictionKey(ActorInfo, &ActivationInfo)))
	{
		UAbilitySystemComponent* const AbilitySystemComponent = ActorInfo->AbilitySystemComponent.Get();
		...
		if (ReturnHandle.IsValid() && ActorInfo->OwnerActor.IsValid() && ActorInfo->OwnerActor->HasAuthority())
		{
...
void UGameplayAbility::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);

	if (Ar.IsMigratingRemoteObjects())
	{
		Ar << bHasBlueprintShouldAbilityRespondToEvent;
		Ar << bHasBlueprintCanUse;
		Ar << bHasBlueprintActivate;
		Ar << bHasBlueprintActivateFromEvent;
		Ar << CurrentSpecHandle;
		Ar << TrackedGameplayCues;
	}
}

#if UE_WITH_REMOTE_OBJECT_HANDLE
void UGameplayAbility::PostMigrate(const FUObjectMigrationContext& MigrationContext)
{
	Super::PostMigrate(MigrationContext);

	if (MigrationContext.MigrationSide == EObjectMigrationSide::Receive)
	{
		// Setup our CurrentActorInfo
		PostNetInit();
	}
}
#endif
