...
namespace MenuExtension_GameplayAbilityBlueprintAudit
{
	...
	static UDataTable* CreateAssetForDataTable()
	{
		...
		if (!NewTable)
		{
			UE_LOGF(LogGameplayAbilityAudit, Error, "Could not create %ls/%ls", *PackageName, *Name);
		}
	...
	void ExecuteActionGameplayAbilityAudit(UScriptStruct& RowStruct, const FToolMenuContext& InContext)
	{
		...
		for (UBlueprint* LoadedInstance : Context->LoadSelectedObjects<UBlueprint>())
		{
			if (!LoadedInstance)
			{
				UE_LOGF(LogGameplayAbilityAudit, Error, "LoadSelectedObject failed on a Selected UBlueprint Instance.  This should not be possible.");
				UE_DEBUG_BREAK();
				continue;
			}
	...
	UScriptStruct& FindBestAuditRowStruct()
	{
		...
		UE_LOGF(LogGameplayAbilityAudit, Log, "Selected %ls as the best Gameplay Ability Audit Functionality", *GetNameSafe(BestAuditRowStruct));
