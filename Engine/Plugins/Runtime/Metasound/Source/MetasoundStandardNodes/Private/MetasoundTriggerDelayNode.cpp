...
namespace Metasound
{
	...
	void FTriggerDelayOperator::Execute()
	{
		...
		TriggerIn->ExecuteBlock(
			...
			{
				// Static cast to int32 to ensure that values are cast to int32 
				// before going into FMath::Max(...). This protects us from overflow issues 
				// when converting between int64->int32.  On some platforms, int64->int32 
				// can result in surprising outcomes when the int64 is out of range
				// of the int32. 
				const int32 FrameToTrigger = FMath::Max(static_cast<int32>(0), static_cast<int32>(FMath::RoundToInt(DelayTime->GetSeconds() * SampleRate))) + StartFrame;
