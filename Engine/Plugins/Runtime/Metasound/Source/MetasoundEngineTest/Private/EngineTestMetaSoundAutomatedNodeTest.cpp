...
namespace Metasound::EngineTest
{
	...
	private:
		const DataType* GetDataOrLogError(const FAnyDataReference& InDataRef) const
		{
			...
			if (nullptr == Value)	
			{
				...
				UE_LOGF(LogMetaSound, Error, "Failed to get data type value of type %ls", *GetMetasoundDataTypeString<DataType>());
			}
			return Value;
		}
	};
	...
	bool FOutputVertexDataTestController::AreAllAnalyzableOutputsValid() const
	{
		...
		for (const FAnalyzableOutput& AnalyzableOutput : AnalyzableOutputs)
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Invalid output encountered %ls %ls", *AnalyzableOutput.VertexName.ToString(), *AnalyzableOutput.DataReferenceToString());
				bAllAreValid = false;
			}
		}
		return bAllAreValid;
	...
	bool FOutputVertexDataTestController::AreAllOutputValuesEqualToCapturedValues() const
	{
		...
		for (const FAnalyzableOutput& AnalyzableOutput : AnalyzableOutputs)
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Unequal output encountered %ls found: %ls expected: %ls", *AnalyzableOutput.VertexName.ToString(), *AnalyzableOutput.DataReferenceToString(), *AnalyzableOutput.CapturedValueToString());
				bAllAreEqual = false;
			}
		}
		return bAllAreEqual;
	...
	void FInputVertexDataTestController::SetMutableInputsToState(const FInterfaceState& InState) 
	{
		...
		UE_LOGF(LogMetaSound, Verbose, "Setting operator input values:%ls%ls", LINE_TERMINATOR, *FString::Join(GetInputValueStrings(), LINE_TERMINATOR));
	}
	...
	TUniquePtr<INode> CreateNodeFromRegistry(const Frontend::FNodeRegistryKey& InNodeRegistryKey)
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Failed to find registered class with registry key %ls", *InNodeRegistryKey.ToString());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void GetAllRegisteredNodes(TArray<FString>& OutBeautifiedNames, TArray <FString>& OutTestCommands, bool bInIncludeDeprecatedNodes)
	{
		...
		ISearchEngine& NodeSearchEngine = ISearchEngine::Get();
		TArray<FMetaSoundClassInfo> AllClasses = NodeSearchEngine.FindAllClasses(ISearchEngine::EResultVersion::All, bIncludeUnloadedAssets);

		for (const FMetaSoundClassInfo& NodeClass : AllClasses)
		{
			// Exclude template classes because they cannot be created directly from the node registry
			if (NodeClass.ClassType == EMetasoundFrontendClassType::Template)
			{
				continue;
			}
			else if (!bInIncludeDeprecatedNodes && EnumHasAnyFlags(NodeClass.AccessFlags, EMetasoundFrontendClassAccessFlags::Deprecated))
			{
				// Optionally skip deprecated classes. 
				continue;
			}

			OutBeautifiedNames.Add(FString::Printf(TEXT("%s %s"), *NodeClass.ClassName.ToString(), *NodeClass.Version.ToString()));

			// Test commands are node registry keys
			OutTestCommands.Add(NodeClass.ToRegistryKey().ToString());
		}
	}
	...
	void GetAllRegisteredNativeNodes(TArray<FString>& OutBeautifiedNames, TArray <FString>& OutTestCommands, bool bInIncludeDeprecatedNodes)
	{
		...
		// Get all the classes that have been registered
		constexpr bool bIncludeUnloadedAssets = false;
		ISearchEngine& NodeSearchEngine = Frontend::ISearchEngine::Get();
		TArray<FMetaSoundClassInfo> AllClasses = NodeSearchEngine.FindAllClasses(Frontend::ISearchEngine::EResultVersion::All, bIncludeUnloadedAssets);

		FMetasoundFrontendRegistryContainer* NodeRegistry = FMetasoundFrontendRegistryContainer::Get();
		check(nullptr != NodeRegistry);

		IMetaSoundAssetManager* AssetManager = IMetaSoundAssetManager::Get();
		for (const FMetaSoundClassInfo& NodeClass : AllClasses)
		{
			// Exclude template classes because they cannot be created directly from the node registry
			if (NodeClass.ClassType == EMetasoundFrontendClassType::Template)
			...
			const FNodeRegistryKey NodeRegistryKey = NodeClass.ToRegistryKey();
