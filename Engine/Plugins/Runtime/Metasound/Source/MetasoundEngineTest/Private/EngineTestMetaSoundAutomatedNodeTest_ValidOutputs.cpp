...
void FMetasoundAutomatedNodeTest_ValidOutputs::GetTests(TArray<FString>& OutBeautifiedNames, TArray<FString>& OutTestCommands) const
{
	...
	for (const FString& ComplexTestName : OutBeautifiedNames)
	{
		bool Registered = TestFramework.RegisterComplexAutomationTestTags(this, ComplexTestName, TEXT("[MustPass]"));
		if (!Registered)
		{
			UE_LOGF(LogMetaSound, Warning, "Failed to register automation test tags for test %ls", *ComplexTestName);
		}
	}

	UE_LOGF(LogMetaSound, Verbose, "Found %d metasound nodes to test", OutTestCommands.Num());
}
