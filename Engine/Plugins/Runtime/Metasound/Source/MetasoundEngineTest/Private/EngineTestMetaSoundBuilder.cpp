...
bool FMetaSoundSourceBuilderSetLiteralLatentCommand::Update()
{
	...
	if (Builder)
	{
		...
		Test.AddErrorIfFalse(Result == EMetaSoundBuilderResult::Succeeded, TEXT("Failed to set MetaSound node's input default"));
	}
...
bool FMetaSoundSourceBuilderRemoveNodeDefaultLiteralLatentCommand::Update()
{
	...
	if (Builder)
	{
		...
		Test.AddErrorIfFalse(Result == EMetaSoundBuilderResult::Succeeded, TEXT("Failed to remove MetaSound node's input default"));
	}
...
REGISTER_SIMPLE_AUTOMATION_TEST_TAGS(FAudioMetaSoundBuilderInterfaceBindingConnectAndDisconnect, "Audio.Metasound.Builder.InterfaceBindingConnectAndDisconnect", "[MustPass]")
...
REGISTER_SIMPLE_AUTOMATION_TEST_TAGS(FAudioMetaSoundNodeClassQueryFunctions, "Audio.Metasound.Builder.NodeClassQueryFunctions", "[MustPass]")
