...
namespace Metasound
{
	namespace Frontend
	{
		...
		const FMetasoundFrontendGraphClassPresetOptions& IGraphController::GetGraphPresetOptions() const
		{
		...
		void IGraphController::SetGraphPresetOptions(const FMetasoundFrontendGraphClassPresetOptions& InPresetOptions)
		{
		}

		const TSet<FName>& IGraphController::GetInputsInheritingDefault() const
		{
			return Invalid::GetInvalidNameSet();
		}

		bool IGraphController::SetInputInheritsDefault(FName InName, bool bDefaultIsInherited)
		{
			return false;
		}

		void IGraphController::SetInputsInheritingDefault(TSet<FName>&& InNames)
		{
		}
		...
		FDocumentHandle IDocumentController::CreateDocumentHandle(FDocumentAccessPtr InDocument, bool bReloadBuilder)
		{
			// Create using standard document controller.
			return FDocumentController::CreateDocumentHandle(InDocument, bReloadBuilder);
		}
