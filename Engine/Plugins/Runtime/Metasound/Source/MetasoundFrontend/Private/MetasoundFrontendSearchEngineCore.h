...
namespace Metasound
{
	namespace Frontend
	{
		namespace SearchEngineQuerySteps
		{
			...
			class FMapInterfaceRegistryTransactionsToInterfaceRegistryVersions : public IFrontendQueryMapStep
			{
			public:
				virtual FFrontendQueryKey Map(const FFrontendQueryEntry& InEntry) const override;
			};
			...
			class FMapVersionToNameAndMajorVersion : public IFrontendQueryMapStep
			{
			public:
			...
			class FReduceToHighestVersion : public IFrontendQueryReduceStep
			{
			public:
				virtual void Reduce(const FFrontendQueryKey& InKey, FFrontendQueryPartition& InOutEntries) const override;
			};
			...
			class FMapVersionToName : public IFrontendQueryMapStep
			{
			public:
				virtual FFrontendQueryKey Map(const FFrontendQueryEntry& InEntry) const override;
			};
		...
		//		{
		//			FMetaSoundClassInfo MetaSoundClass;
		...
		struct FFindHighestVersionOfInterfaceQueryPolicy
		{
			using ResultType = FMetasoundFrontendVersion;

			static FFrontendQuery CreateQuery();
			static FMetasoundFrontendVersion BuildResult(const FFrontendQueryPartition& InPartition);
		};
		...
		public:
			...
			virtual bool FindRegisteredClass(const FMetasoundFrontendClassName& InName, int32 InMajorVersion, FMetaSoundClassInfo& OutClass) override;
			...
			virtual bool FindHighestInterfaceVersion(FName InInterfaceName, FMetasoundFrontendVersion& OutVersion) override;
		...
		private:
			...
			TSearchEngineQuery<FFindHighestVersionOfInterfaceQueryPolicy> FindHighestVersionOfInterfaceQueryPolicy;
		};
	}
}
