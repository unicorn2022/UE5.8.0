...
namespace Metasound::Frontend
{
	...
	: FGraphNode(FNodeData(InNodeInitData.InstanceName, InNodeInitData.InstanceID, InGraphToWrap->GetVertexInterface()), MoveTemp(InGraphToWrap))
	{
	}
	...
	void FGraphNode::SetDefaultInput(const FVertexName& InVertexName, const FLiteral& InLiteral)
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Could not set default input. Could not find input vertex %ls on node %ls of node class %ls", *InVertexName.ToString(), *GetInstanceName().ToString(), *GetMetadata().ClassName.ToString());
		}
	}
