...
namespace Metasound::Frontend
{
	...
	{
		static const FLazyName AccessFlags = "AccessFlags";

#if WITH_EDITORONLY_DATA
		static const FLazyName AssetFlags_Deprecated = "AssetFlags";
...
FMetaSoundClassInterfaceInfo::FMetaSoundClassInterfaceInfo(const FMetasoundFrontendClass& InClass)
{
#if WITH_EDITORONLY_DATA
	using namespace Metasound::Frontend;
	...
	if (!IsRunningCookCommandlet())
	{
		SearchInfo = FMetaSoundClassSearchInfo(InClass.Metadata);

		{
			...
			Algo::Transform(InClass.GetDefaultInterface().Inputs, Inputs, CopyVertexInfo);
			Algo::Transform(InClass.GetDefaultInterface().Outputs, Outputs, CopyVertexInfo);
		}
	}
#endif // WITH_EDITORONLY_DATA
}
...
namespace Metasound
{
	namespace Frontend
	{
		FMetaSoundClassInfo::FMetaSoundClassInfo(EMetasoundFrontendClassType InClassType)
			...
			, bIsValid(0)
		{
...

			{
				...
				bool bFoundFlags = InAssetData.GetTagValue(ClassTagsPrivate::AccessFlags, AccessFlagsValue);

#if WITH_EDITORONLY_DATA
				if (!bFoundFlags)
				{
					bFoundFlags = InAssetData.GetTagValue(ClassTagsPrivate::AssetFlags_Deprecated, AccessFlagsValue);
				}
#endif // WITH_EDITORONLY_DATA
		...
		FMetaSoundClassInfo::FMetaSoundClassInfo(const FMetasoundFrontendClass& InClass)
			: ClassName(InClass.Metadata.GetClassName())
			, ClassType(InClass.Metadata.GetType())
			, Version(InClass.Metadata.GetVersion())
			, bIsValid(InClass.Metadata.GetType() != EMetasoundFrontendClassType::Invalid)
		{
#if WITH_EDITORONLY_DATA
			InterfaceInfo = FMetaSoundClassInterfaceInfo(InClass);
#endif // WITH_EDITORONLY_DATA

			AccessFlags = InClass.Metadata.GetAccessFlags();
		}
