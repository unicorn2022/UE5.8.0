...
namespace Metasound
{
	...
	namespace ArrayRandomNodePrivate
	{
		const FLazyName ArrayRandomOperatorName{"Random Get"};
		constexpr int32 ArrayRandomNodeMajorVersion = 1;
		constexpr int32 ArrayRandomNodeMinorVersion = 1;

		Frontend::FNodeClassRegistryKey CreateArrayRandomNodeClassRegistryKey(const FName& InArrayDataTypeName)
		{
			return Frontend::FNodeClassRegistryKey(EMetasoundFrontendClassType::External, MetasoundArrayNodesPrivate::CreateArrayNodeClassName(ArrayRandomOperatorName, InArrayDataTypeName), ArrayRandomNodeMajorVersion, ArrayRandomNodeMinorVersion);
		}

		FVertexInterface CreateArrayRandomNodeInterface(const FName& InArrayDataTypeName, const FName& InElementDataTypeName)
		{
			...
			return FVertexInterface {
				FInputVertexInterface{
					...
					FInputDataVertex(METASOUND_GET_PARAM_NAME(InputRandomArray), InArrayDataTypeName, METASOUND_GET_PARAM_METADATA(InputRandomArray)),
					TInputDataVertex<TArray<float>>(METASOUND_GET_PARAM_NAME_AND_METADATA(InputWeights)),
				...
				FOutputVertexInterface{
					...
					FOutputDataVertex(METASOUND_GET_PARAM_NAME(ShuffleOutputValue), InElementDataTypeName, METASOUND_GET_PARAM_METADATA(ShuffleOutputValue), EVertexAccessType::Reference),
					TOutputDataVertex<int32>(METASOUND_GET_PARAM_NAME_AND_METADATA(OutputIndex))
				}
			};
		}

		FNodeClassMetadata CreateArrayRandomNodeClassMetadata(const FName& InArrayDataTypeName, const FName& InElementDataTypeName, const FText& InArrayDataTypeDisplayText)
		{
			const FText NodeDisplayName = METASOUND_LOCTEXT_FORMAT("RandomArrayGetNode_OpDisplayNamePattern", "Random Get ({0})", InArrayDataTypeDisplayText);

			const FText NodeDescription = METASOUND_LOCTEXT("RandomArrayGetNode_Description", "Randomly retrieve data from input array using the supplied weights.");

			return MetasoundArrayNodesPrivate::CreateArrayNodeClassMetadata(InArrayDataTypeName, ArrayRandomOperatorName, NodeDisplayName, NodeDescription, CreateArrayRandomNodeInterface(InArrayDataTypeName, InElementDataTypeName), ArrayRandomNodeMajorVersion, ArrayRandomNodeMinorVersion);
		}
	}
