...
bool FMetasoundCommentNodeIntVector::SerializeFromMismatchedTag(const struct FPropertyTag& Tag, FStructuredArchive::FSlot Slot)
{
	...
	if (const UScriptStruct* DeprecatedSlateType = FDeprecateSlateVector2D::StaticStruct())
	{
		if (Tag.GetType().IsStruct(DeprecatedSlateType->GetFName()))
		{
			...
			UE_LOGF(LogMetaSound, VeryVerbose, "FMetasoundCommentNodeIntVector::SerializeFromMismatchedTag - DeprecateSlateVector2D Type found: Resolving Mismatch");
			return true;
		}
	}
#endif // WITH_EDITORONLY_DATA
	...
	if (Tag.GetType().IsStruct("DeprecateSlateVector2D"))
	{
		...
		UE_LOGF(LogMetaSound, VeryVerbose, "FMetasoundCommentNodeIntVector::SerializeFromMismatchedTag - DeprecateSlateVector2D Type not loaded: Ignoring Mismatch");
	...
	{
		UE_LOGF(LogMetaSound, VeryVerbose, "FMetasoundCommentNodeIntVector::SerializeFromMismatchedTag - Did not resolve. Ignoring value (cooking content, value not necessary).");
	}
	else
	{
		UE_LOGF(LogMetaSound, VeryVerbose, "FMetasoundCommentNodeIntVector::SerializeFromMismatchedTag - Did not resolve. Returning failure state.");
	}
...
bool FMetasoundFrontendNodeInterface::Update(const FMetasoundFrontendClassInterface& InClassInterface, TFunctionRef<void(const FMetasoundFrontendVertex&)> OnPreRemoveInput, TFunctionRef<void(const FMetasoundFrontendVertex&)> OnPreRemoveOutput)
{
	...
	TArray<FMetasoundFrontendVertex> UnmatchedVertices;
	...
	for (const FMetasoundFrontendClassInput& ClassInput : InClassInterface.Inputs)
	{
		if (const int32 Index = UnmatchedVertices.IndexOfByPredicate(FFindMatchingVertex{ClassInput}); Index != INDEX_NONE)
		{
			...
			UnmatchedVertices.RemoveAt(Index, 1, EAllowShrinking::No);
		}
		else
		{
	...
	for (const FMetasoundFrontendVertex& UnmatchedInput : UnmatchedVertices)
	{
		...
		if (const int32 Index = Inputs.IndexOfByPredicate([UnmatchedInput](const FMetasoundFrontendVertex& i) { return i.Name == UnmatchedInput.Name && i.TypeName == UnmatchedInput.TypeName; } ); Index != INDEX_NONE)
		{
			// Update the node vertex with anything new from the class vertex
			Inputs.RemoveAt(Index, 1, EAllowShrinking::No);
		}	
		bInterfaceUpdated |= true;
	}
	...
	// Remove any outputs that did not exist in the class interface
	for (const FMetasoundFrontendVertex& UnmatchedOutput : UnmatchedVertices)
	{
		// Allow outside systems to react before removing the unmatched node outputs
		OnPreRemoveOutput(UnmatchedOutput);

		if (const int32 Index = Outputs.IndexOfByPredicate([&UnmatchedOutput](const FMetasoundFrontendVertex& i) { return i.Name == UnmatchedOutput.Name && i.TypeName == UnmatchedOutput.TypeName; } ); Index != INDEX_NONE)
		{
			// Update the node vertex with anything new from the class vertex
			Outputs.RemoveAt(Index, 1, EAllowShrinking::No);
		}	
		bInterfaceUpdated |= true;
	}

	// If the interface was updated, make sure the node's interface is sorted
	// the same as the class's interface
	if (bInterfaceUpdated)
	{
		// Sort the input interface
		check(InClassInterface.Inputs.Num() == Inputs.Num());
		for (int32 ClassInputIndex = 0; ClassInputIndex < InClassInterface.Inputs.Num(); ClassInputIndex++)
		{
			int32 NodeInputIndex = Inputs.IndexOfByPredicate(FFindMatchingVertex{InClassInterface.Inputs[ClassInputIndex]});
			check(NodeInputIndex != INDEX_NONE); // The matching node inputs should exist by now.
			if (NodeInputIndex > ClassInputIndex)
			{
				Inputs.Swap(NodeInputIndex, ClassInputIndex);
			}
			else
			{
				// This sort algorithm should never have NodeInputIndex < ClassInputIndex
				check(NodeInputIndex == ClassInputIndex);
			}
		}

		// Sort the output interface
		check(InClassInterface.Outputs.Num() == Outputs.Num());
		for (int32 ClassOutputIndex = 0; ClassOutputIndex < InClassInterface.Outputs.Num(); ClassOutputIndex++)
		{
			int32 NodeOutputIndex = Outputs.IndexOfByPredicate(FFindMatchingVertex{InClassInterface.Outputs[ClassOutputIndex]});
			check(NodeOutputIndex != INDEX_NONE); // The matching node inputs should exist by now.
			if (NodeOutputIndex > ClassOutputIndex)
			{
				Outputs.Swap(NodeOutputIndex, ClassOutputIndex);
			}
			else
			{
				// This sort algorithm should never have NodeOutputIndex < ClassOutputIndex
				check(NodeOutputIndex == ClassOutputIndex);
			}
		}
	}
...
#if WITH_EDITORONLY_DATA
	Metadata.Description = InInterface->GetDescription();
#endif
...
const FMetasoundFrontendClassInterface& FMetasoundFrontendClass::GetInterfaceForNode(const FMetasoundFrontendNode& InNode) const
{
	if (const FMetasoundFrontendClassInterface* InterfaceOverride = InNode.ClassInterfaceOverride.GetPtr())
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Found class interface override on class %ls. Class interface overrides are intended to be used on external nodes", *Metadata.GetClassName().ToString());
		}
...
FMetasoundFrontendClassStyle FMetasoundFrontendClassStyle::GenerateClassStyle(const Metasound::FNodeDisplayStyle& InNodeDisplayStyle)
{
	...
	Style.Display.StyleSet = InNodeDisplayStyle.StyleSet;
	Style.Display.Icon = InNodeDisplayStyle.Icon;
	Style.Display.Color = InNodeDisplayStyle.Color;
...
#if WITH_EDITORONLY_DATA
bool FMetasoundFrontendDocument::MigrateProperties()
{
	...
	for (TPair<FGuid, TObjectPtr<UMetaSoundFrontendMemberMetadata>>& Pair : Metadata.MemberMetadata)
	{
		if (UMetaSoundFrontendMemberMetadata* MemberMetadata = Pair.Value.Get())
		{
			const bool bMigratedMetadata = MemberMetadata->MigrateProperties();
			if (bMigratedMetadata)
			{
				bMigrated = true;
			}
		}
...
{
	using namespace Metasound::Frontend;
	return FVersioningManager::GetMaxDocumentVersion();
}
#endif // WITH_EDITORONLY_DATA
