...
namespace Metasound::Frontend
{
	namespace DocumentBuilderPrivate
	{
		...
		bool NameContainsInterfaceNamespace(FName VertexName, FMetasoundFrontendInterface* OutInterface)
		{
			...
			if (!InterfaceNamespace.IsNone())
			{
				...
				if (ISearchEngine::Get().FindHighestInterfaceVersion(InterfaceNamespace, InterfaceVersion))
				{
					if (OutInterface)
					{
						FMetasoundFrontendInterface FoundInterface;
						const bool bInterfaceFound = IInterfaceRegistry::Get().FindInterface(InterfaceVersion, FoundInterface);
						checkf(bInterfaceFound, TEXT("MetaSound Interface found via search engine must exist in registry"))
		...
		void SetNodeAndVertexNames(FMetasoundFrontendNode& InOutNode, const FMetasoundFrontendClassVertex& InVertex)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Node associated with graph vertex of type '%ls' does not contain input vertex of matching type.", *InVertex.TypeName.ToString());
			}
			...
			{
				UE_LOGF(LogMetaSound, Error, "Node associated with graph vertex of type '%ls' does not contain output vertex of matching type.", *InVertex.TypeName.ToString());
			}
		}
		...
		void SetDefaultLiteralOnInputNode(FMetasoundFrontendNode& InOutNode, const FMetasoundFrontendClassInput& InClassInput, const FGuid& PageID)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Input node associated with graph input vertex of name '%ls' does not contain input vertex with matching name.", *InClassInput.Name.ToString());
			}
		}
		...
		private:
			...
			bool RemoveUnsupportedVertices(FMetaSoundFrontendDocumentBuilder& OutBuilder) const
			{
				...
				for (const TPair<FMetasoundFrontendClassInput, const FMetasoundFrontendInterface*>& Pair : InputsToAdd)
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Removed existing targeted input '%ls' to avoid name collision/member data descrepancies while modifying interface(s). Desired edges may have been removed as a result.", *Pair.Key.Name.ToString());
						bDidEdit = true;
					}
				}
				...
				for (const TPair<FMetasoundFrontendClassOutput, const FMetasoundFrontendInterface*>& Pair : OutputsToAdd)
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Removed existing targeted output '%ls' to avoid name collision/member data descrepancies while modifying interface(s). Desired edges may have been removed as a result.", *Pair.Key.Name.ToString());
						bDidEdit = true;
					}
				}
				...
				if (!InputsToRemove.IsEmpty() || !OutputsToRemove.IsEmpty())
				{
					...
					for (const FMetasoundFrontendClassVertex& InputToRemove : InputsToRemove)
					{
						...
						{
							UE_LOGF(LogMetaSound, Warning, "Failed to remove existing input '%ls', which was an expected member of a removed interface.", *InputToRemove.Name.ToString());
						}
					}
					...
					for (const FMetasoundFrontendClassVertex& OutputToRemove : OutputsToRemove)
					{
						...
						{
							UE_LOGF(LogMetaSound, Warning, "Failed to remove existing output '%ls', which was an expected member of a removed interface.", *OutputToRemove.Name.ToString());
						}
					}
	...
	FModifyInterfaceOptions::FModifyInterfaceOptions(const TArray<FMetasoundFrontendVersion>& InInterfaceVersionsToRemove, const TArray<FMetasoundFrontendVersion>& InInterfaceVersionsToAdd, const FMetaSoundFrontendDocumentBuilder* InReferencedBuilder)
	{
		Algo::Transform(InInterfaceVersionsToRemove, InterfacesToRemove, [](const FMetasoundFrontendVersion& Version)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Failed to find interface '%ls' to remove", *Version.ToString());
			}
			return Interface;
		});
		...
		Algo::Transform(InInterfaceVersionsToAdd, InterfacesToAdd, [](const FMetasoundFrontendVersion& Version)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Failed to find interface '%ls' to add", *Version.ToString());
			}
			return Interface;
		});
		ReferencedBuilder = InReferencedBuilder;
...
#if WITH_EDITOR
TStructView<FMetaSoundFrontendDocumentTemplate> FMetaSoundFrontendDocumentBuilder::GetDocumentTemplate() const
{
	TInstancedStruct<FMetaSoundFrontendDocumentTemplate>& Config = GetDocumentChecked().Template;
	return Config;
}

TStructView<FMetaSoundFrontendDocumentTemplate> FMetaSoundFrontendDocumentBuilder::GetDocumentConfiguration() const
{
	return GetDocumentTemplate();
}
#endif // WITH_EDITOR

TConstStructView<FMetaSoundFrontendDocumentTemplate> FMetaSoundFrontendDocumentBuilder::GetConstDocumentTemplate() const
{
	const TInstancedStruct<FMetaSoundFrontendDocumentTemplate>& Template = GetConstDocumentChecked().Template;
	return Template;
}
...
const FMetasoundFrontendClass* FMetaSoundFrontendDocumentBuilder::ReplaceDependencyInternal(const FMetasoundFrontendClass& OldDependency, FMetasoundFrontendClass NewDependency)
{
	...
	NewDependency.ID = OldDependency.ID;
	const int32* IdxPtr = DocumentCache->FindDependencyIndex(OldDependency.ID);
	checkf(IdxPtr, TEXT("Dependency '%s' to replace not found."), *OldDependency.Metadata.GetClassName().ToString());
	const int32 Idx = *IdxPtr;
	Document.Dependencies.RemoveAt(Idx);
	Dependency = &Document.Dependencies.EmplaceAt_GetRef(Idx, MoveTemp(NewDependency));
...
bool FMetaSoundFrontendDocumentBuilder::AddNamedEdges(const TSet<Metasound::Frontend::FNamedEdge>& EdgesToMake, TArray<const FMetasoundFrontendEdge*>* OutNewEdges, bool bReplaceExistingConnections, const FGuid* InPageID)
{
	...
	for (const FNamedEdge& Edge : EdgesToMake)
	{
		...
		if (OutputVertex && InputVertex)
		{
			...
			{
				bSuccess = false;
				UE_LOGF(LogMetaSound, Error, "Failed to add connections between MetaSound output '%ls' and input '%ls': '%ls'.", *Edge.OutputName.ToString(), *Edge.InputName.ToString(), *LexToString(InvalidEdgeReason));
			}
		}
	}
	...
	for (FNewEdgeData& EdgeToAdd : EdgesToAdd)
	{
		if (bReplaceExistingConnections)
		{
			...
			if (bRemovedEdge)
			{
				...
				UE_LOGF(LogMetaSound, Verbose, "Removed connection from node output '%ls:%ls' to node '%ls:%ls' in order to connect to node output '%ls:%ls'",
		...
		else
		{
			...
			checkf(Output, TEXT("Prior loop to check edge validity should protect against missing output vertex"));

			UE_LOGF(LogMetaSound, Warning, "Connection between MetaSound output '%ls' and input '%ls' not added: Input already connected to '%ls'.", *Output->Name.ToString(), *Input->Name.ToString(), *Output->Name.ToString());
#endif // !NO_LOGGING
		}
	}
...
bool FMetaSoundFrontendDocumentBuilder::AddEdgesFromMatchingInterfaceNodeOutputsToGraphOutputs(const FGuid& InNodeID, TArray<const FMetasoundFrontendEdge*>& OutEdgesCreated, bool bReplaceExistingConnections, const FGuid* InPageID)
{
	...
	for (const FMetasoundFrontendVersion& Version : CommonInterfaces)
	{
		...
		if (bFound)
		{
			Algo::Transform(Interface.Outputs, EdgesToMake, [this, &NodeCache, &InterfaceCache, &PageID, InNodeID](const FMetasoundFrontendClassOutput& Output)
			{
...
bool FMetaSoundFrontendDocumentBuilder::AddEdgesFromMatchingInterfaceNodeInputsToGraphInputs(const FGuid& InNodeID, TArray<const FMetasoundFrontendEdge*>& OutEdgesCreated, bool bReplaceExistingConnections, const FGuid* InPageID)
{
	...
	for (const FMetasoundFrontendVersion& Version : CommonInterfaces)
	{
		...
		if (bFound)
		{
			Algo::Transform(Interface.Inputs, EdgesToMake, [this, &Graph, &NodeCache, &InterfaceCache, InNodeID](const FMetasoundFrontendClassInput& Input)
			{
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddGraphInput(FMetasoundFrontendClassInput ClassInput, const FGuid* InPageID)
{
	...
	{
		UE_LOGF(LogMetaSound, Error, "TypeName unset when attempting to add class input '%ls'", *ClassInput.Name.ToString());
		return nullptr;
	}
	else if (const FMetasoundFrontendClassInput* Input = DocumentCache->GetInterfaceCache().FindInput(ClassInput.Name))
	{
		UE_LOGF(LogMetaSound, Error, "Attempting to add MetaSound graph input '%ls' when input with name already exists", *ClassInput.Name.ToString());
	...
	{
		UE_LOGF(LogMetaSound, Error, "Cannot add MetaSound graph input '%ls' with unregistered TypeName '%ls'", *ClassInput.Name.ToString(), *ClassInput.TypeName.ToString());
		return nullptr;
	}
...
#if WITH_EDITORONLY_DATA
		if (FDocumentVertexTemplate* DocTemplate = Document.Template.GetMutablePtr<FDocumentVertexTemplate>())
		{
PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
			DocTemplate->AddVertexMetadata(*this, NewInput);
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddGraphOutput(FMetasoundFrontendClassOutput ClassOutput, const FGuid* InPageID)
{
	...
	{
		UE_LOGF(LogMetaSound, Error, "TypeName unset when attempting to add class output '%ls'", *ClassOutput.Name.ToString());
		return nullptr;
	}
	else if (const FMetasoundFrontendClassOutput* Output = DocumentCache->GetInterfaceCache().FindOutput(ClassOutput.Name))
	{
		UE_LOGF(LogMetaSound, Error, "Attempting to add MetaSound graph output '%ls' when output with name already exists", *ClassOutput.Name.ToString());
	...
	{
		UE_LOGF(LogMetaSound, Error, "Cannot add MetaSound graph output '%ls' with unregistered TypeName '%ls'", *ClassOutput.Name.ToString(), *ClassOutput.TypeName.ToString());
		return nullptr;
	}
	...
	Document.RootGraph.IterateGraphPages([&](FMetasoundFrontendGraph& Graph)
	{
		...
		if (!NewNode)
		{
			UE_LOGF(LogMetaSound, Error, "Failed to add MetaSound graph output '%ls' node to page ID '%ls'", *ClassOutput.Name.ToString(), *PageID.ToString());
			bAddedNodes = false;
		}
	});
	...
	if (bAddedNodes)
	{
		...
		{
PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
			DocTemplate->AddVertexMetadata(*this, NewOutput);
PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
		}
...
const FMetasoundFrontendVariable* FMetaSoundFrontendDocumentBuilder::AddGraphVariable(FName VariableName, FName DataType, const FMetasoundFrontendLiteral* Literal, const FText* DisplayName, const FText* Description, const FGuid* InPageID)
{
	...
	const FGuid& PageID = InPageID ? *InPageID : BuildPageID;
	if (const FMetasoundFrontendVariable* ExistingVariable = FindGraphVariable(VariableName, &PageID))
	{
		UE_LOGF(LogMetaSound, Warning, "AddGraphVariable Failed: Variable already exists with name '%ls' (existing DataType '%ls', requested DataType '%ls')",
	...
	{
		UE_LOGF(LogMetaSound, Error, "AddGraphVariable Failed: Attempted creation of variable '%ls' with unregistered DataType '%ls'", *VariableName.ToString(), *DataType.ToString());
		return nullptr;
	}
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddGraphVariableAccessorNode(FName VariableName, FGuid InNodeID, const FGuid* InPageID)
{
	...
	if (!Variable)
	{
		UE_LOGF(LogMetaSound, Error, "AddGraphVariableAccessorNode Failed: Variable does not exists with name '%ls'", *VariableName.ToString());
		return nullptr;
	} 
	...
	FNodeRegistryKey VariableNodeClassKey;
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Could not find registered \"get variable\" node class for data type \"%ls\"", *Variable->TypeName.ToString());
			return nullptr;
		}
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddGraphVariableDeferredAccessorNode(FName VariableName, FGuid InNodeID, const FGuid* InPageID)
{
	...
	if (!Variable)
	{
		UE_LOGF(LogMetaSound, Error, "AddGraphVariableGetDelayedNode Failed: Variable does not exists with name '%ls'", *VariableName.ToString());
		return nullptr;
	}
	...
	FNodeRegistryKey ClassKey;
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "AddGraphVariableGetDelayedNode Failed: Could not find registered \"get variable\" node class for data type \"%ls\"", *Variable->TypeName.ToString());
			return nullptr;
		}
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddGraphVariableMutatorNode(FName VariableName, FGuid InNodeID, const FGuid* InPageID)
{
	...
	if (!Variable)
	{
		UE_LOGF(LogMetaSound, Error, "AddGraphVariableMutatorNode Failed: Variable does not exists with name '%ls'", *VariableName.ToString());
		return nullptr;
	}

	if (const FMetasoundFrontendNode* ExistingMutatorNode = FindNode(Variable->MutatorNodeID, InPageID))
	{
		UE_LOGF(LogMetaSound, Error, "Cannot add mutator node as one already exists for variable '%ls'.", *VariableName.ToString());
		return nullptr;
	}
	...
	FNodeRegistryKey ClassKey;
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Could not find registered \"set variable\" node class for data type \"%ls\"", *Variable->TypeName.ToString());
			return nullptr;
		}
...
bool FMetaSoundFrontendDocumentBuilder::AddInterfaceInternal(FName InterfaceName, bool bAddUserModifiableInterfaceOnly, const FMetaSoundFrontendDocumentBuilder* ReferenceBuilder)
{
	...
	FMetasoundFrontendVersion InterfaceVersion;
	if (ISearchEngine::Get().FindHighestInterfaceVersion(InterfaceName, InterfaceVersion))
	{
		if (GetDocumentChecked().Interfaces.Contains(InterfaceVersion))
		{
			UE_LOGF(LogMetaSound, VeryVerbose, "MetaSound interface '%ls' already found on document. MetaSoundBuilder skipping add request.", *InterfaceName.ToString());
			return true;
		}

		FMetasoundFrontendInterface Interface;
		const bool bFoundInterface = IInterfaceRegistry::Get().FindInterface(InterfaceVersion, Interface);
		check(bFoundInterface);
		...
		for (const FMetasoundFrontendVersion& PreviousVersion : PreviousVersions)
		{
			...
			if (IInterfaceRegistry::Get().FindInterface(PreviousVersion, PreviousVersionInterface))
			{
		...
		const FMetasoundFrontendInterfaceUClassOptions* ClassOptions = Interface.Metadata.UClassOptions.FindByPredicate(FindClassOptionsPredicate);
		if (ClassOptions && bAddUserModifiableInterfaceOnly && !ClassOptions->bIsModifiable)
		{
			UE_LOGF(LogMetaSound, Error, "DocumentBuilder failed to add MetaSound Interface '%ls' to document: is not set to be user modifiable for given UClass '%ls'", *InterfaceName.ToString(), *BuilderClassPath.ToString());
			return false;
		}

		// Remove old versions and add new interfaces simultaneously to support node reconnection
		FModifyInterfaceOptions Options(MoveTemp(PreviousVersionInterfaces), { MoveTemp(Interface) }, ReferenceBuilder);
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddNodeByClassInternal(FMetasoundFrontendClass&& InClass, FGuid InNodeID, const FGuid* InPageID)
{
	...
	{
		UE_LOGF(LogMetaSound, Warning, "Failed to add new node by class name '%ls': Class is restricted type '%ls' that cannot be added via this function.",
	...
	{
		Dependency = ReplaceDependencyInternal(*Dependency, MoveTemp(InClass));
	}
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddNodeByClassName(const FMetasoundFrontendClassName& InClassName, int32 InMajorVersion, FGuid InNodeID, const FGuid* InPageID)
{
	...
	if (!ISearchEngine::Get().FindRegisteredClass(InClassName, InMajorVersion, RegistryClassInfo))
	{
		UE_LOGF(LogMetaSound, Error, "Failed to add new node by class name '%ls' and major version '%d': Class info not registered", *InClassName.ToString(), InMajorVersion);
		return nullptr;
	}

	FMetasoundFrontendClass RegisteredClass;
	if (!INodeClassRegistry::GetChecked().FindFrontendClassFromRegistered(RegistryClassInfo.ToRegistryKey(), RegisteredClass))
	{
		UE_LOGF(LogMetaSound, Error, "Failed to add new node by class name '%ls' and major version '%d': Class not found", *InClassName.ToString(), InMajorVersion);
		return nullptr;
	}
...
const FMetasoundFrontendNode* FMetaSoundFrontendDocumentBuilder::AddNodeByClassName(const FMetasoundFrontendClassName& InClassName, int32 InMajorVersion, int32 InMinorVersion, FGuid InNodeID, const FGuid* InPageID)
{
	...
	{
		UE_LOGF(LogMetaSound, Error, "Failed to add new node by class name '%ls' and major version '%d' and minor version '%d': Class not found", *InClassName.ToString(), InMajorVersion, InMinorVersion);
		return nullptr;
	}
	return AddNodeByClassInternal(MoveTemp(RegisteredClass), InNodeID, InPageID);
}
...
void FMetaSoundFrontendDocumentBuilder::CacheRegistryMetadata() const
{
	...
	for (const FMetasoundFrontendVersion& Version : InterfaceVersions)
	{
		...
		UE_CLOGF(!bFound, LogMetaSound, Error,
			"Failed to find interface (%ls) when caching registry data for %ls. "
			"MetaSound inputs and outputs for asset may not function correctly.",
			*Version.ToString(), *GetDebugName());

		if (bFound)
		{
			for (const FMetasoundFrontendClassInput& InterfaceInput : Interface.Inputs)
			{
			...
			for (const FMetasoundFrontendClassOutput& InterfaceOutput : Interface.Outputs)
			{
	...
	for (FMetasoundFrontendClass& Dependency : Document.Dependencies)
	{
		if (!FMetasoundFrontendClass::CacheGraphDependencyMetadataFromRegistry(Dependency))
		{
			UE_LOGF(LogMetaSound, Warning,
				"'%ls' failed to cache dependency registry data: Registry missing class with key '%ls'",
				*GetDebugName(),
				*Dependency.Metadata.GetClassName().ToString());
			UE_LOGF(LogMetaSound, Warning,
				"Asset '%ls' may fail to build runtime graph unless re-registered after dependency with given key is loaded.",
				*GetDebugName());
		}
	}
}
...
#if WITH_EDITORONLY_DATA
bool FMetaSoundFrontendDocumentBuilder::CanAutoUpdate(const FGuid& InNodeID, const FGuid* InPageID) const
{
	using namespace Metasound::Frontend;
	const FGuid& PageID = InPageID ? *InPageID : BuildPageID;
	return FAutoUpdateRootGraph::CanAutoUpdate(*this, InNodeID, PageID);
}
#endif // WITH_EDITORONLY_DATA
...
bool FMetaSoundFrontendDocumentBuilder::ConformObjectToDocument()
{
	return GetDocumentInterfaceChecked().ConformObjectToDocument();
}

bool FMetaSoundFrontendDocumentBuilder::ConfigureDocument()
{
	using namespace Metasound::Frontend;

	if (bInTemplateScope)
	{
		UE_LOGF(LogMetaSound, Error, "Cannot call Configure Document: Configure already called higher in stack");
		return false;
	}

	bInTemplateScope = true;

	FMetasoundFrontendDocument& Document = GetDocumentChecked();
	if (FMetaSoundFrontendDocumentTemplate* DocTemplate = Document.Template.GetMutablePtr())
	{
		if (DocTemplate->ConfigureDocument(*this))
		{
			GetDocumentInterfaceChecked().ConformObjectToDocument();

#if WITH_EDITORONLY_DATA
			FInputNodeTemplate::GetChecked().Inject(*this);
#endif // WITH_EDITORONLY_DATA

			bInTemplateScope = false;
			return true;
		}

		TSharedRef<FDocumentModifyDelegates> ModifyDelegates = MakeShared<FDocumentModifyDelegates>(*DocumentDelegates);
...
bool FMetaSoundFrontendDocumentBuilder::ConvertFromPreset()
{
	...
	if (IsPreset())
	{
		const bool bConfigChanged = GetDocumentChecked().Template.IsValid();

		// Don't configure after nulling the template in order to "freeze" the current document/MetaSound UObject state (i.e. not clear it).
		GetDocumentChecked().Template.Reset();
		...
		if (bConfigChanged)
		{
			DocumentDelegates->OnDocumentTemplateChanged.Broadcast(FDocumentTemplateChangedArgs { .Type = EDocumentTemplateChangeType::Struct });
		}
...
bool FMetaSoundFrontendDocumentBuilder::ConvertToPreset(const FMetaSoundFrontendDocumentBuilder& InReferencedDocumentBuilder, TSharedPtr<Metasound::Frontend::FDocumentModifyDelegates> ModifyDelegates)
{
	...
	FPresetConfigStruct NewConfig = FPresetConfigStruct::Make<FMetaSoundFrontendPresetTemplate>();
	NewConfig.GetMutable().Parent = &InReferencedDocumentBuilder.CastDocumentObjectChecked<UObject>();
	SetTemplate(MoveTemp(NewConfig));
	ConfigureDocument();
...
#if WITH_EDITORONLY_DATA
bool FMetaSoundFrontendDocumentBuilder::NodeInterfaceMatchesClassInterface(const FGuid& InNodeID, const FGuid& InPageID) const
{
	using namespace Metasound;
	using namespace Metasound::Frontend;
	...
	const FMetasoundFrontendNodeInterface& NodeInterface = Node->Interface;

	if ((NodeClassInterface.Inputs.Num() != NodeInterface.Inputs.Num()) ||
		(NodeClassInterface.Outputs.Num() != NodeInterface.Outputs.Num()))
	{
		return false;
	}
	...
	{
		auto IsEquivalent = [&Input](const FMetasoundFrontendVertex& RegistryVertex)
		{
			return FMetasoundFrontendVertex::IsFunctionalEquivalent(Input, RegistryVertex);
		};

		const int32 Index = NodeInterface.Inputs.FindLastByPredicate(IsEquivalent);
		if (Index == INDEX_NONE)
		{
			return false;
		}
	...
	{
		auto IsEquivalent = [&Output](const FMetasoundFrontendVertex& RegistryVertex)
		{
			return FMetasoundFrontendVertex::IsFunctionalEquivalent(Output, RegistryVertex);
		};

		const int32 Index = NodeInterface.Outputs.FindLastByPredicate(IsEquivalent);
		if (Index == INDEX_NONE)
		{
			return false;
		}
...
bool FMetaSoundFrontendDocumentBuilder::DiffAgainstRegistryInterface(const FGuid& InNodeID, const FGuid& InPageID, bool bInUseHighestMinorVersion, Metasound::Frontend::FClassInterfaceUpdates& OutInterfaceUpdates, bool bInForceRegenerateClassInterfaceOverride) const
{
	...
	FNodeClassName NodeClassName = NodeClassMetadata.GetClassName().ToNodeClassName();
	...
	if (bInUseHighestMinorVersion)
	{
		...
		if (!ISearchEngine::Get().FindRegisteredClass(NodeClassName, NodeClassMetadata.GetVersion().Major, ClassInfo))
		{
			ClearInterface();
			return false;
		}

		bool bFoundRegisteredClass = INodeClassRegistry::GetChecked().FindFrontendClassFromRegistered(ClassInfo.ToRegistryKey(), OutInterfaceUpdates.RegistryClass);
		if (!ensureMsgf(bFoundRegisteredClass, TEXT("SearchEngine returned registered class info for class that is not registered")))
		{
			ClearInterface();
			return false;
		}
	...
	else
	{
		...
		bool bFoundRegisteredClass = INodeClassRegistry::GetChecked().FindFrontendClassFromRegistered(FNodeRegistryKey(NodeClassMetadata), OutInterfaceUpdates.RegistryClass);
		if (!bFoundRegisteredClass)
		{
			// If the class was not found, mark all inputs and outputs as removed.
			UE_LOGF(LogMetaSound, Warning, "Could not find registered version of interface. %ls %ls is not registered.", *NodeClassMetadata.GetClassName().ToString(), *NodeClassMetadata.GetVersion().ToString());
			ClearInterface();
			return false;
		}
	}
...
const FMetasoundFrontendClassInput* FMetaSoundFrontendDocumentBuilder::DuplicateGraphInput(FName ExistingName, FName NewName)
{
	...
	if (!ExistingInput)
	{
		UE_LOGF(LogMetaSound, Warning, "Failed to duplicate graph input '%ls': input does not exist", *ExistingName.ToString());
		return nullptr;
	}

	if (FindGraphInput(NewName))
	{
		UE_LOGF(LogMetaSound, Warning, "Failed to duplicate graph input '%ls': input with name '%ls' already exists", *ExistingName.ToString(), *NewName.ToString());
		return nullptr;
	}
...
const FMetasoundFrontendClassOutput* FMetaSoundFrontendDocumentBuilder::DuplicateGraphOutput(FName ExistingName, FName NewName)
{
	...
	if (!ExistingOutput)
	{
		UE_LOGF(LogMetaSound, Warning, "Failed to duplicate graph output '%ls', output does not exist", *ExistingName.ToString());
		return nullptr;
	...
	{
		UE_LOGF(LogMetaSound, Warning, "Failed to duplicate graph output '%ls', output with name '%ls' already exists", *ExistingName.ToString(), *NewName.ToString());
		return nullptr;
	}
...
const FMetasoundFrontendVariable* FMetaSoundFrontendDocumentBuilder::DuplicateGraphVariable(FName ExistingName, FName NewName, const FGuid* InPageID)
{
	...
	{
		UE_LOGF(LogMetaSound, Warning, "Failed to duplicate graph variable '%ls': variable with name '%ls' already exists", *ExistingName.ToString(), *NewName.ToString());
		return nullptr;
	}
	...
	{
		const FGuid& PageID = InPageID ? *InPageID : BuildPageID;
		UE_LOGF(LogMetaSound, Warning, "Failed to duplicate graph variable '%ls' on page '%ls': variable does not exist", *ExistingName.ToString(), *PageID.ToString());
	}
...
bool FMetaSoundFrontendDocumentBuilder::FindInterfaceInputNodes(FName InterfaceName, TArray<const FMetasoundFrontendNode*>& OutInputs, const FGuid* InPageID) const
{
	...
	if (ISearchEngine::Get().FindHighestInterfaceVersion(InterfaceName, InterfaceVersion))
	{
		if (Interfaces.Contains(InterfaceVersion))
		{
			FMetasoundFrontendInterface Interface;
			const bool bFoundInterface = IInterfaceRegistry::Get().FindInterface(InterfaceVersion, Interface);
			check(bFoundInterface);
...
bool FMetaSoundFrontendDocumentBuilder::FindInterfaceOutputNodes(FName InterfaceName, TArray<const FMetasoundFrontendNode*>& OutOutputs, const FGuid* InPageID) const
{
	...
	if (ISearchEngine::Get().FindHighestInterfaceVersion(InterfaceName, InterfaceVersion))
	{
		...
		{
			FMetasoundFrontendInterface Interface;
			if (IInterfaceRegistry::Get().FindInterface(InterfaceVersion, Interface))
			{
				const IDocumentGraphNodeCache& NodeCache = DocumentCache->GetNodeCache(PageID);
				const IDocumentGraphInterfaceCache& InterfaceCache = DocumentCache->GetInterfaceCache();
...
int32 FMetaSoundFrontendDocumentBuilder::FindGraphPageIndex(const FGuid& InPageID) const
{
	const FMetasoundFrontendDocument& Document = GetDocumentChecked();
...
FName FMetaSoundFrontendDocumentBuilder::GenerateUniqueInputName(const FString& InBaseName) const
{
	using namespace Metasound::Frontend;

	TSet<FName> ExistingNames;
	auto GetName = [&](const FMetasoundFrontendClass&, const FMetasoundFrontendNode& Node) { ExistingNames.Add(Node.Name); };
	IterateNodesByClassType(GetName, EMetasoundFrontendClassType::Input);
	return DocumentBuilderPrivate::GenerateUniqueName(ExistingNames, InBaseName);
}

FName FMetaSoundFrontendDocumentBuilder::GenerateUniqueOutputName(const FString& InBaseName) const
{
	using namespace Metasound::Frontend;

	TSet<FName> ExistingNames;
	auto GetName = [&](const FMetasoundFrontendClass&, const FMetasoundFrontendNode& Node) { ExistingNames.Add(Node.Name); };
	IterateNodesByClassType(GetName, EMetasoundFrontendClassType::Output);
	return DocumentBuilderPrivate::GenerateUniqueName(ExistingNames, InBaseName);
}

FName FMetaSoundFrontendDocumentBuilder::GenerateUniqueVariableName(const FString& InBaseName) const
{
	using namespace Metasound::Frontend;

	TSet<FName> ExistingVariableNames;
	Algo::Transform(FindConstBuildGraphChecked().Variables, ExistingVariableNames, [](const FMetasoundFrontendVariable& Var) { return Var.Name; });
	return DocumentBuilderPrivate::GenerateUniqueName(ExistingVariableNames, InBaseName);
}
...
FMetasoundAssetBase* FMetaSoundFrontendDocumentBuilder::GetReferencedPresetAsset() const
{
	...
	if (const FMetaSoundFrontendPresetTemplate* PresetTemplate = GetConstTemplateAs<FMetaSoundFrontendPresetTemplate>())
	{
		if (UObject* Parent = PresetTemplate->Parent.GetObject())
		{
			return IMetaSoundAssetManager::GetChecked().GetAsAsset(*Parent);
		}
	}
...
#if WITH_EDITORONLY_DATA
bool FMetaSoundFrontendDocumentBuilder::GetIsGraphEditable(const FGuid* PageID) const
{
	bool bIsEditable = true;
	const FMetasoundFrontendDocument& Document = GetConstDocumentChecked();
	Document.RootGraph.IterateGraphPages([&bIsEditable, &PageID](const FMetasoundFrontendGraph& PageGraph)
	{
		if (!PageID || PageGraph.PageID == *PageID)
		{
			bIsEditable &= PageGraph.Style.bIsGraphEditable;
		}
	});
	return bIsEditable;
}
...
FText FMetaSoundFrontendDocumentBuilder::GetNodeInputDisplayName(const FGuid& InNodeID, const FName VertexName, const FGuid* InPageID) const
{
	...
	switch (ClassType)
	{
		...
		case EMetasoundFrontendClassType::External:
		{
			...
			const FMetasoundFrontendClassVertex* ClassVertex = Class->GetInterfaceForNode(*Node).Inputs.FindByPredicate(MatchesName);
			if (ClassVertex)
			{
...
FText FMetaSoundFrontendDocumentBuilder::GetNodeOutputDisplayName(const FGuid& InNodeID, const FName VertexName, const FGuid* InPageID) const
{
	...
	switch (ClassType)
	{
		...
		case EMetasoundFrontendClassType::External:
		{
			...
			const FMetasoundFrontendClassVertex* ClassVertex = Class->GetInterfaceForNode(*Node).Outputs.FindByPredicate(PinMatchesClassVertex);
			if (ClassVertex)
			{
				FName Namespace, ParamName;
				ClassVertex->SplitName(Namespace, ParamName);
...
#if WITH_EDITORONLY_DATA
		// Set default doc version Metadata
		{
			FMetasoundFrontendDocumentMetadata& DocMetadata = Document.Metadata;
			DocMetadata.Version.Number = FVersioningManager::GetMaxDocumentVersion();
		}
#endif // WITH_EDITORONLY_DATA
...
{
	FMetasoundFrontendClassMetadata& ThisMetadata = GetDocumentChecked().RootGraph.Metadata;

#if DO_CHECK
	using namespace Metasound::Frontend;
	if (IDocumentBuilderRegistry* BuilderRegistry = IDocumentBuilderRegistry::Get())
	{
		TArray<FMetaSoundFrontendDocumentBuilder*> RegisteredBuilders = BuilderRegistry->FindBuilders(ThisMetadata.GetClassName());
		auto IsThis = [this](const FMetaSoundFrontendDocumentBuilder* Builder)
		{
			checkf(Builder, TEXT("FindBuilders should always return valid pointers to builders"));
			return this == Builder;
		};
		checkf(!Algo::AnyOf(RegisteredBuilders, IsThis),
			TEXT("Renaming graph class will result in duplicate builder entries in the builder registry. "
				"Builder(s) '%s' must be unregistered prior to updating class name"),
			*FString::JoinBy(RegisteredBuilders, TEXT(", "), [](const FMetaSoundFrontendDocumentBuilder* RegisteredBuilder)
			{
				return RegisteredBuilder->GetDebugName();
			}));
	}
#endif // DO_CHECK

	InitGraphClassMetadata(ThisMetadata, bResetVersion, NewClassName);
}
...
bool FMetaSoundFrontendDocumentBuilder::IsConfiguringDocument() const
{
	return bInTemplateScope;
}
...
bool FMetaSoundFrontendDocumentBuilder::IsInterfaceDeclared(FName InInterfaceName) const
{
	...
	FMetasoundFrontendVersion InterfaceVersion;
	if (ISearchEngine::Get().FindHighestInterfaceVersion(InInterfaceName, InterfaceVersion))
	{
		return IsInterfaceDeclared(InterfaceVersion);
	}
...
bool FMetaSoundFrontendDocumentBuilder::IsPreset() const
{
	...
	return IsDocumentConfiguredBy<FPresetTemplate>();
}
...
void FMetaSoundFrontendDocumentBuilder::BeginBuilding(TSharedPtr<Metasound::Frontend::FDocumentModifyDelegates> Delegates, bool bPrimeCache)
{
	...
	if (DocumentInterface)
	{
		...
		if (UE_LOG_ACTIVE(LogMetaSound, VeryVerbose))
		{
			...
			{
				UE_LOGF(LogMetaSound, VeryVerbose, "MetaSoundFrontendDocumentBuilder::BeginBuilding for asset '%ls': BuildPageID initialized to '%ls'", *DebugPath.ToString(), *BuildPageID.ToString());
			}
		}
	}
...
bool FMetaSoundFrontendDocumentBuilder::RemoveNamedEdges(const TSet<Metasound::Frontend::FNamedEdge>& InNamedEdgesToRemove, TArray<FMetasoundFrontendEdge>* OutRemovedEdges, const FGuid* InPageID)
{
	...
	for (const FNamedEdge& NamedEdge : InNamedEdgesToRemove)
	{
		...
		if (OutputVertex && InputVertex)
		{
			...
			{
				bSuccess = false;
				UE_LOGF(LogMetaSound, Warning, "Failed to remove connection between MetaSound node output '%ls' and input '%ls': No connection found.", *NamedEdge.OutputName.ToString(), *NamedEdge.InputName.ToString());
			}
		}
	}
...
{
	return RemoveGraphInputDefault(InputName, InPageID);
}
...
bool FMetaSoundFrontendDocumentBuilder::RemoveGraphInputDefault(FName InputName, const FGuid& InPageID)
{
	...
	// Default page input default should not be removed at editor/runtime but can be cooked out depending on platform/cook settings. Use StripGraphInputDefault for that case instead.
	if (InPageID == DefaultPageID)
	{
		UE_LOGF(LogMetaSound, Error, "Document Builder failed to remove graph input default '%ls': Cannot remove default page input default except in cook. If this happens during cook, use StripGraphInputDefault instead.", *InputName.ToString());
		return false;
	}
	
	return RemoveGraphInputDefaultInternal(InputName, InPageID);
}

bool FMetaSoundFrontendDocumentBuilder::StripGraphInputDefault(FName InputName, const FGuid& InPageID)
{
	return RemoveGraphInputDefaultInternal(InputName, InPageID);	
}

bool FMetaSoundFrontendDocumentBuilder::RemoveGraphInputDefaultInternal(FName InputName, const FGuid& InPageID)
{
	using namespace Metasound;
	using namespace Metasound::Frontend;
	...
	if (Index != INDEX_NONE)
	{
		...
		if (bRemovedDefault)
		{
			DocumentDelegates->InterfaceDelegates.OnInputDefaultChanged.Broadcast(
				FDocumentArrayPagedInputArgs
				{
					.Index = Index,
					.PageID = InPageID,
					.ChangeType = EMetaSoundLiteralChangeType::Remove
				});
			return true;
		}
	}
...
bool FMetaSoundFrontendDocumentBuilder::RemoveGraphInput(FName InputName, bool bRemoveTemplateInputNodes)
{
	...
	if (const int32* IndexPtr = DocumentCache->GetInterfaceCache().FindInputIndex(InputName))
	{
		...
		if (bNodesRemoved)
		{
			...
			if (FDocumentVertexTemplate* DocTemplate = Document.Template.GetMutablePtr<FDocumentVertexTemplate>())
			{
				DocTemplate->RemoveVertexMetadata(NodeID);
			}
#endif // WITH_EDITORONLY_DATA
...
bool FMetaSoundFrontendDocumentBuilder::RemoveInterface(FName InterfaceName)
{
	...
	if (ISearchEngine::Get().FindHighestInterfaceVersion(InterfaceName, InterfaceVersion))
	{
		if (!GetDocumentChecked().Interfaces.Contains(InterfaceVersion))
		{
			UE_LOGF(LogMetaSound, VeryVerbose, "MetaSound interface '%ls' not found on document. MetaSoundBuilder skipping remove request.", *InterfaceName.ToString());
			return true;
		}
		...
		if (bFoundInterface)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "DocumentBuilder failed to remove MetaSound Interface '%ls' to document: is not set to be modifiable for given UClass '%ls'", *InterfaceName.ToString(), *BuilderClassPath.ToString());
				return false;
			}

			FModifyInterfaceOptions Options({ MoveTemp(Interface) }, { });
			return ModifyInterfaces(MoveTemp(Options));
		}
	}
...
bool FMetaSoundFrontendDocumentBuilder::ReplaceDependency(const Metasound::Frontend::FNodeClassRegistryKey& OldNodeClassKey, const Metasound::Frontend::FNodeClassRegistryKey& NewNodeClassKey,TArray<Metasound::Frontend::FVertexNameAndType>* OutDisconnectedInputs, TArray<Metasound::Frontend::FVertexNameAndType>* OutDisconnectedOutputs)
{
	using namespace Metasound::Frontend;
	using namespace Metasound::Frontend::NodeReplacement;
	...
	struct FNodeReplacementData
	{
		const FGuid NodeID;
		const FGuid PageID;
#if WITH_EDITORONLY_DATA
		FNodeInstanceReplacementData NodeInstanceReplacementData;
#endif // WITH_EDITORONLY_DATA
	};
	TArray<FNodeReplacementData> NodeDataToReplace;
	auto CaptureNodeData = [&](const FMetasoundFrontendClass&, const FMetasoundFrontendNode& Node, const FGuid& PageID)
	{
#if WITH_EDITORONLY_DATA
		FNodeInstanceReplacementData NodeInstanceData = CaptureNodeInstanceReplacementData(Node.GetID(), &PageID);
		NodeDataToReplace.Emplace(FNodeReplacementData { Node.GetID(), PageID, MoveTemp(NodeInstanceData) });
#else
		NodeDataToReplace.Emplace(FNodeReplacementData { Node.GetID(), PageID });
#endif // WITH_EDITORONLY_DATA
	};
	...
	for (const FNodeReplacementData& NodeReplacementData : NodeDataToReplace)
	{
		const FMetasoundFrontendNode* NewNode = AddNodeByClassName(NewNodeClassKey.ClassName, NewNodeClassKey.Version.Major, NewNodeClassKey.Version.Minor, NodeReplacementData.NodeID, &NodeReplacementData.PageID);
	...
	for (FNodeReplacementData& NodeReplacementData : NodeDataToReplace)
	{
		const FGuid& NodeID = NodeReplacementData.NodeID;
		const FGuid& PageID = NodeReplacementData.PageID;
		
		FMetasoundFrontendNode* NewNode = FindNodeInternal(NodeID, &PageID);
		if (NewNode)
		{
			const FMetasoundFrontendClass* Class = FindDependency(NewNode->ClassID);
			if (Class)
			{
				NodeReplacementData.NodeInstanceReplacementData.Style.bMessageNodeUpdated = bNodeVersionUpdated;
				ApplyNodeInstanceReplacementData(MoveTemp(NodeReplacementData.NodeInstanceReplacementData), NodeID, &PageID, OutDisconnectedInputs, OutDisconnectedOutputs);
			}
		}
	}
#endif // WITH_EDITORONLY_DATA
...
#if WITH_EDITORONLY_DATA
void FMetaSoundFrontendDocumentBuilder::ApplyNodeInstanceReplacementData(Metasound::Frontend::NodeReplacement::FNodeInstanceReplacementData&& InInstanceData, const FGuid& InNodeID, const FGuid* InPageID, TArray<Metasound::Frontend::FVertexNameAndType>* OutDisconnectedInputs, TArray<Metasound::Frontend::FVertexNameAndType>* OutDisconnectedOutputs)
{
	using namespace Metasound::Frontend;
	...
	FMetasoundFrontendNode* ReplacementNode = FindNodeInternal(InNodeID, InPageID);
	check(ReplacementNode);
	ReplacementNode->Style = MoveTemp(InInstanceData.Style);
	...
	// the first place.
	if (!ShouldReplaceExistingNodeConfig(ReplacementNode->Configuration, InInstanceData.Configuration))
	{
		// We should have kept the original node configuration. Set back to the original
		SetNodeConfiguration(InNodeID, MoveTemp(InInstanceData.Configuration), InPageID);
	}
	
	// Set literal and reconnect input connections if possible
	for (FMetasoundFrontendVertex& InputVertex : ReplacementNode->Interface.Inputs)
	{
		...
		if (FInputConnectionInfo* ConnectionInfo = InInstanceData.InputConnections.Find(ConnectionKey))
		{
			...
			{
				// Only apply literal if needs to be overwritten (an overwritten value exists, or it is not equal to class input default)
				if (const FMetasoundFrontendVertexLiteral* CurrentLiteral = FindNodeInputDefault(InNodeID, InputVertex.VertexID, InPageID))
				{
					SetNodeInputDefault(InNodeID, InputVertex.VertexID, ConnectionInfo->DefaultValue, InPageID);
				}
				else if (const TArray<FMetasoundFrontendClassInputDefault>* NodeClassDefaults = FindNodeClassInputDefaults(InNodeID, InputVertex.Name, InPageID))
				{
					...
					if (!NodeClassDefault.Literal.IsEqual(ConnectionInfo->DefaultValue))
					{
						SetNodeInputDefault(InNodeID, InputVertex.VertexID, ConnectionInfo->DefaultValue, InPageID);
					}
				}
			...
			if (ConnectionInfo->ConnectedOutput.IsSet())
			{
				...
				const bool bIsValidEdge = IsValidEdge(NewEdge, InPageID) == Metasound::Frontend::EInvalidEdgeReason::None;
				...
				if (bIsValidEdge && !ContainsEdge(NewEdge, InPageID))
				{
					AddEdge(MoveTemp(NewEdge), InPageID);
				}
	...
	for (FMetasoundFrontendVertex& OutputVertex : ReplacementNode->Interface.Outputs)
	{
...
Metasound::Frontend::NodeReplacement::FNodeInstanceReplacementData FMetaSoundFrontendDocumentBuilder::CaptureNodeInstanceReplacementData(const FGuid& InNodeID, const FGuid* InPageID) const
{
	using namespace Metasound::Frontend;
	using namespace NodeReplacement;
	const FGuid& PageID = InPageID ? *InPageID : BuildPageID;
	...
	const FMetasoundFrontendNode* ReplacementNode = FindNode(InNodeID, InPageID);
	check(ReplacementNode);
	ReplacementData.Style = ReplacementNode->Style;

	// Move any configuration and override data living on the node.
	ReplacementData.Configuration = ReplacementNode->Configuration;
	ReplacementData.ClassInterfaceOverride = ReplacementNode->ClassInterfaceOverride;

	// Cache input/output connections by name to try so they can be
	// hooked back up after swapping to the new class version.
	for (const FMetasoundFrontendVertex& InputVertex : ReplacementNode->Interface.Inputs)
	{
		...
		if (const FMetasoundFrontendVertexLiteral* VertexLiteral = ReplacementNode->InputLiterals.FindByPredicate(VertexLiteralMatchesID))
		{
		...
		const FMetasoundFrontendVertex* ConnectedOutputVertex = FindNodeOutputConnectedToNodeInput(ReplacementNode->GetID(), InputVertex.VertexID, &ConnectedOutputNode, &PageID);
	...
	for (const FMetasoundFrontendVertex& OutputVertex : ReplacementNode->Interface.Outputs)
	{
		const FVertexNameAndType ConnectionKey(OutputVertex.Name, OutputVertex.TypeName);
		TArray<const FMetasoundFrontendNode*> ConnectedInputNodes;
		TArray<const FMetasoundFrontendVertex*> ConnectedInputVertices = FindNodeInputsConnectedToNodeOutput(ReplacementNode->GetID(), OutputVertex.VertexID, &ConnectedInputNodes, &PageID);
...
bool FMetaSoundFrontendDocumentBuilder::ResetGraphInputDefault(FName InputName)
{
	...
	{
		FMetasoundFrontendClassInput& Input = Inputs[Index];
		TArray<const FGuid> PageIDs;
		Input.IterateDefaults([&](const FGuid& PageID, FMetasoundFrontendLiteral&)
		{
			PageIDs.Add(PageID);
		});
		Input.ResetDefaults();

		for (const FGuid& PageID : PageIDs)
		{
			DocumentDelegates->InterfaceDelegates.OnInputDefaultChanged.Broadcast(
			FDocumentArrayPagedInputArgs
			{
				.Index = Index,
				.PageID = PageID,
				.ChangeType = EMetaSoundLiteralChangeType::Reset
			});
		}
		
		if (FDocumentVertexTemplate* Config = Document.Template.GetMutablePtr<FDocumentVertexTemplate>())
		{
			if (FMetaSoundFrontendDocumentTemplateVertexMetadata* VertexMetadata = Config->FindVertexMetadata(Input.NodeID))
			{
				VertexMetadata->OnDefaultReset(*this, Input);
			}
		}
...
void FMetaSoundFrontendDocumentBuilder::SetTemplate(TInstancedStruct<FMetaSoundFrontendDocumentTemplate> InConfig)
{
	using namespace Metasound::Frontend;

	GetDocumentChecked().Template = MoveTemp(InConfig);
	DocumentDelegates->OnDocumentTemplateChanged.Broadcast({ EDocumentTemplateChangeType::Struct });
}

void FMetaSoundFrontendDocumentBuilder::SetTemplateProperties(TFunctionRef<void(TStructView<FMetaSoundFrontendDocumentTemplate>)> SetPropertyFunction)
{
	using namespace Metasound::Frontend;
	SetPropertyFunction(GetDocumentChecked().Template);
	DocumentDelegates->OnDocumentTemplateChanged.Broadcast({ EDocumentTemplateChangeType::Property });
}
...
void FMetaSoundFrontendDocumentBuilder::SetIsGraphEditable(bool bIsEditable, const FGuid* PageID) const
{
	FMetasoundFrontendDocument& Document = GetDocumentChecked();
	Document.RootGraph.IterateGraphPages([bIsEditable, &PageID](FMetasoundFrontendGraph& PageGraph)
	{
		if (!PageID || PageGraph.PageID == *PageID)
		{
			PageGraph.Style.bIsGraphEditable = bIsEditable;
		}
	});
}
#endif // WITH_EDITORONLY_DATA
...
bool FMetaSoundFrontendDocumentBuilder::SetGraphInputDataType(FName InputName, FName DataType)
{
	...
	if (IDataTypeRegistry::Get().IsRegistered(DataType))
	{
		...
		{
			// If input is currently a constructor (value) type, but is being set to a data type 
			// that does not support constructor types, set it back to reference
			if (GraphInput.AccessType == EMetasoundFrontendVertexAccessType::Value)
			{
				FDataTypeRegistryInfo Info;
				if (IDataTypeRegistry::Get().GetDataTypeInfo(DataType, Info))
				{
					...
					{
						GraphInput.AccessType = EMetasoundFrontendVertexAccessType::Reference;
					}
				}
			}
			GraphInput.TypeName = DataType;
			TArray<const FGuid> PageIDsModified;
			GraphInput.IterateDefaults([&](const FGuid& PageID, FMetasoundFrontendLiteral&)
			{
				PageIDsModified.Add(PageID);
			});
			...
			for (const FGuid& PageID : PageIDsModified)
			{
				DocumentDelegates->InterfaceDelegates.OnInputDefaultChanged.Broadcast(
				FDocumentArrayPagedInputArgs
				{
...
bool FMetaSoundFrontendDocumentBuilder::SetGraphInputDefault(FName InputName, FMetasoundFrontendLiteral InDefaultLiteral, const FGuid* InPageID, EMetaSoundLiteralChangeType InChangeType)
{
	using namespace Metasound;
	using namespace Metasound::Frontend;
	...
	if (Index != INDEX_NONE)
	{
		...
		if (Frontend::IDataTypeRegistry::Get().IsLiteralTypeSupported(Input.TypeName, InDefaultLiteral.GetType()))
		{
			...
			if (const FTemplateVertexMetadata* ConfigVertexMetadata = FindConstVertexMetadata<FTemplateVertexMetadata>(Input.NodeID))
			{
				const_cast<FMetaSoundFrontendDocumentTemplateVertexMetadata*>(ConfigVertexMetadata)->OnDefaultUpdated(*this, Input);
			}

			return true;
		}
		UE_LOGF(LogMetaSound, Error, "Attempting to set graph input of type '%ls' with unsupported literal type", *Input.TypeName.ToString());
	}

	return false;
}

bool FMetaSoundFrontendDocumentBuilder::SetGraphInputDefaults(FName InputName, TArray<FMetasoundFrontendClassInputDefault> Defaults, EMetaSoundLiteralChangeType InChangeType)
{
	using namespace Metasound;
	using namespace Metasound::Frontend;
	...
	{
		FMetasoundFrontendClassInput& Input = Inputs[Index];
		TArray<FGuid> PageIDsModified;

		// If defaults are being removed or reset, compare with existing ones to report the correct page id being modified
		if (InChangeType == EMetaSoundLiteralChangeType::Remove)
		{
			Input.IterateDefaults([&](const FGuid& PageID, const FMetasoundFrontendLiteral& Literal)
			{
				PageIDsModified.Emplace(PageID);
			});
			for (const FMetasoundFrontendClassInputDefault& Default : Defaults)
			{
				PageIDsModified.Remove(Default.PageID);
			}
		}
		else if (InChangeType == EMetaSoundLiteralChangeType::Set || InChangeType == EMetaSoundLiteralChangeType::Reset)
		{
			Input.IterateDefaults([&](const FGuid& PageID, const FMetasoundFrontendLiteral& Literal)
			{
				PageIDsModified.Emplace(PageID);
			...
			for (const FMetasoundFrontendClassInputDefault& Default : Defaults)
			{
				if (const FMetasoundFrontendLiteral* ExistingLiteral = Input.FindConstDefault(Default.PageID))
				{
					if (Default.Literal.IsEqual(*ExistingLiteral))
					{
						PageIDsModified.Remove(Default.PageID);
					}
				}
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Possible unhandled EMetaSoundLiteralChangeType in SetGraphInputDefaults");
		}
		...
		if (bAllSupported)
		{
			...
			if (FDocumentVertexTemplate* DocTemplate = Document.Template.GetMutablePtr<FDocumentVertexTemplate>())
			{
				if (FTemplateVertexMetadata* VertexMetadata = DocTemplate->FindVertexMetadata(Input.NodeID))
				{
					VertexMetadata->OnDefaultUpdated(*this, Input);
				}
			}

			return true;
		}
		UE_LOGF(LogMetaSound, Error, "Attempting to set graph input of type '%ls' with unsupported literal type(s)", *Input.TypeName.ToString());
	}
...
bool FMetaSoundFrontendDocumentBuilder::SetGraphOutputDataType(FName OutputName, FName DataType)
{
	...
	{
		// If output is currently a constructor (value) type, but is being set to a data type 
		// that does not support constructor types, set it back to reference
		if (GraphOutput.AccessType == EMetasoundFrontendVertexAccessType::Value)
		{
			FDataTypeRegistryInfo Info;
			if (IDataTypeRegistry::Get().GetDataTypeInfo(DataType, Info))
			{
				if (!Info.bIsConstructorType)
				{
					GraphOutput.AccessType = EMetasoundFrontendVertexAccessType::Reference;
				}
			}
		}
	...
	if (const int32* NodeIndex = NodeCache.FindNodeIndex(InNodeID))
	{
		...
		if (const FMetasoundFrontendClass* Class = FindDependency(Node.ClassID))
		{
			...
			if (!INodeClassRegistry::Get()->IsCompatibleNodeConfiguration(NodeClassKey, InNodeConfiguration))
			{
				...
				UE_LOGF(LogMetaSound, Warning, "Cannot set node configuration to type %ls, expected registered type %ls", *GetNameSafe(InNodeConfiguration.GetScriptStruct()), *GetNameSafe(RegisteredConfiguration.GetScriptStruct()));
#endif // !NO_LOGGING
				return false;
			}
	...
	if (FMetasoundFrontendNode* Node = FindNodeInternal(InNodeID, InPageID))
	{
		...
		if (InLocationGuid)
		{
			if (InLocationGuid->IsValid())
			{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Multiple locations per node no longer supported, removing other display locations for node ID '%ls'", *InNodeID.ToString());
			...
			UE_LOGF(LogMetaSound, Display, "Invalid Location Guid no longer supported, resetting display location for node with ID '%ls'", *InNodeID.ToString());
		}
