...
namespace Metasound
{
	namespace Frontend
	{
			...
			if (FMetasoundFrontendDocument* Document = DocumentPtr.Get())
			{
				...
				for (FMetasoundFrontendNode& Node : SubgraphCopy.GetDefaultGraph().Nodes)
				{
					...
					if (const FMetasoundFrontendClass* OriginalNodeClass = InOtherDocument.Dependencies.FindByPredicate(IsClassWithClassID))
					{
						...
						{
							UE_LOGF(LogMetaSound, Error, "Failed to add subgraph dependency [Class:%ls]", *OriginalNodeClass->Metadata.GetClassName().ToString());
							bSuccess = false;
						}
					}
					...
					{
						bSuccess = false;
						UE_LOGF(LogMetaSound, Error, "Failed to copy subgraph. Subgraph document is missing dependency info for node [Node:%ls, NodeID:%ls]", *Node.Name.ToString(), *Node.GetID().ToString());
					}
				}
		...
		FGraphHandle FDocumentController::AddDuplicateSubgraph(const IGraphController& InGraph)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Cannot add subgraph from invalid document");
			...
			{
				UE_LOGF(LogMetaSound, Error, "Cannot add invalid subgraph to document");
				return IGraphController::GetInvalidHandle();
			}
			...
			if (bNoMatchingClassFoundInDocument)
			{
				...
				if (FMetasoundFrontendDocument* Document = DocumentPtr.Get())
				{
					switch (InMetadata.GetType())
					{
						...
						case EMetasoundFrontendClassType::Output:
						{
							...
							else
							{
								...
								UE_LOGF(LogMetaSound, Error,
									"Cannot add external dependency. No Metasound class found with matching registry key [Key:%ls, Name:%ls, Version:%ls]. Suggested solution \"%ls\" by %ls.",
									*Key.ToString(),
...
#else
								UE_LOGF(LogMetaSound, Error,
									"Cannot add external dependency. No Metasound class found with matching registry key [Key:%ls, Name:%ls, Version:%ls].",
									*Key.ToString(),
									*InMetadata.GetClassName().ToString(),
									*InMetadata.GetVersion().ToString());
#endif // !WITH_EDITOR
						...
						default:
						{
							UE_LOGF(LogMetaSound, Error, "Unsupported metasound class type for node: \"%ls\" (%ls).",
								*InMetadata.GetClassName().ToString(),
								*InMetadata.GetVersion().ToString());
							checkNoEntry();
						}
		...
		bool FDocumentController::ExportToJSONAsset(const FString& InAbsolutePath) const
		{
			if (const FMetasoundFrontendDocument* Document = DocumentPtr.Get())
			{
				...
				{
					UE_LOGF(LogMetaSound, Error, "Failed to export Metasound json asset. Could not write to path \"%ls\".", *InAbsolutePath);
				}
			}
