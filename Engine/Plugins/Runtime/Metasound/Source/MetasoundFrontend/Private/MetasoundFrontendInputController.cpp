...
namespace Metasound
{
	namespace Frontend
	{
		...
		bool FBaseInputController::Connect(IOutputController& InController)
		{
			...
			if (FMetasoundFrontendGraph* Graph = GraphPtr.Get())
			{
				if (IsCastable(OtherDataType, DataType))
				{
					...
					{
						UE_LOGF(LogMetaSound, Error, "Cannot connect incompatible vertex access types (Input)%ls and (Output)%ls.", LexToString(GetVertexAccessType()), LexToString(InController.GetVertexAccessType()));
					}
				}
				else
				{
					UE_LOGF(LogMetaSound, Error, "Cannot connect incompatible data types %ls and %ls.", *DataType.ToString(), *OtherDataType.ToString());
				}
			}
		...
		bool FBaseInputController::ConnectWithConverterNode(IOutputController& InController, const FConverterNodeInfo& InConverterInfo)
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Converter node [Name: %ls] does not support preferred input vertex [Vertex: %ls]", *ConverterNode->GetNodeName().ToString(), *InConverterInfo.PreferredConverterInputPin.ToString());
				return false;
			}

			if (!ConverterOutput->IsValid())
			{
				UE_LOGF(LogMetaSound, Warning, "Converter node [Name: %ls] does not support preferred output vertex [Vertex: %ls]", *ConverterNode->GetNodeName().ToString(), *InConverterInfo.PreferredConverterOutputPin.ToString());
				return false;
			}
