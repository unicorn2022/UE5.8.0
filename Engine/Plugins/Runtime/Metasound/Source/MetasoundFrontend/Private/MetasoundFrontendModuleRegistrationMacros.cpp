...
namespace Metasound::Frontend::RegistrationPrivate
{
...
#if WITH_EDITORONLY_DATA
		UE_LOGF(LogMetaSound, Verbose, "Running registration actions for module %ls in plugin %ls", *InOwningModuleInfo.ModuleName.ToString(), *InOwningModuleInfo.PluginName.ToString());
#endif // if WITH_EDITORONLY_DATA
		for (FRegistrationAction* Action = Head; Action != nullptr; Action = Action->Next)
		{
#if WITH_EDITORONLY_DATA
			UE_LOGF(LogMetaSound, VeryVerbose, "Running Register:%ls for module %ls in plugin %ls", Action->Name, *InOwningModuleInfo.ModuleName.ToString(), *InOwningModuleInfo.PluginName.ToString());
#endif
			if (Action->Register)
			{
...
#if WITH_EDITORONLY_DATA
		UE_LOGF(LogMetaSound, Verbose, "Running unregistration actions for module %ls in plugin %ls", *InOwningModuleInfo.ModuleName.ToString(), *InOwningModuleInfo.PluginName.ToString());
#endif // if WITH_EDITORONLY_DATA
		for (FRegistrationAction* Action = Head; Action != nullptr; Action = Action->Next)
		{
#if WITH_EDITORONLY_DATA
			UE_LOGF(LogMetaSound, VeryVerbose, "Running Unregister:%ls for module %ls in plugin %ls", Action->Name, *InOwningModuleInfo.ModuleName.ToString(), *InOwningModuleInfo.PluginName.ToString());
#endif
	...
	void FGlobalRegistrationList::RegisterAll(const FModuleInfo& InOwningModuleInfo, FRegistrationAction* Head)
	{
...
#if WITH_EDITORONLY_DATA
		UE_LOGF(LogMetaSound, Verbose, "Running pending registration actions for global registration list");
#endif // if WITH_EDITORONLY_DATA
		...
		{
#if WITH_EDITORONLY_DATA
			UE_LOGF(LogMetaSound, VeryVerbose, "Running Register:%ls for global registration list", Action->Name);
#endif
			if (Action->Register)
			{
				Action->Register(InOwningModuleInfo);
