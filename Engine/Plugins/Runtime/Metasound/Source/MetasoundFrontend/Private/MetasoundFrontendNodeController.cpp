	...
	namespace Frontend
	{
		...
		, OwningGraph(InParams.OwningGraph)
		{
			if (FMetasoundFrontendNode* Node = NodePtr.Get())
			{
				if (const FMetasoundFrontendClass* Class = ClassPtr.Get())
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Changing node's class id from [ClassID:%ls] to [ClassID:%ls]", *Node->ClassID.ToString(), *Class->ID.ToString());
		...
		FNodeHandle FBaseNodeController::ReplaceWithVersion(const FMetasoundFrontendVersionNumber& InNewVersion, TArray<FVertexNameAndType>* OutDisconnectedInputs, TArray<FVertexNameAndType>* OutDisconnectedOutputs)
		{
			...
			if (!bFoundNewClass)
			{
				...
				UE_LOGF(LogMetaSound, Error, "Failed to change class version from %ls to %ls for class %ls. %ls %ls is not registered.", *Metadata.GetVersion().ToString(), *NewVersionString, *ClassNameString, *ClassNameString, *NewVersionString);
		...
		bool FBaseNodeController::DiffAgainstRegistryInterface(FClassInterfaceUpdates& OutInterfaceUpdates, bool bInUseHighestMinorVersion, bool bInForceRegenerateClassInterfaceOverride) const
		{
			...
			if (bInUseHighestMinorVersion)
			{
				...
				const bool bIsRegistered = ISearchEngine::Get().FindRegisteredClass(NodeClassMetadata.GetClassName().ToNodeClassName(), NodeClassMetadata.GetVersion().Major, ClassInfo);
				if (bIsRegistered)
				{
					RegistryKey = ClassInfo.ToRegistryKey();
				}
			}
			else
			{
				RegistryKey = FNodeClassRegistryKey(NodeClassMetadata);
			}

			const bool bDidFindClass = INodeClassRegistry::GetChecked().FindFrontendClassFromRegistered(RegistryKey, OutInterfaceUpdates.RegistryClass);
			if (!bDidFindClass)
			{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			if (RegistryClass.Metadata.GetVersion() == NodeClassMetadata.GetVersion())
			{
				if (FVersioningManager::ChangeIDComparisonEnabledInAutoUpdate())
				{
		...
		FNodeHandle FNodeController::CreateNodeHandle(const FNodeController::FInitParams& InParams)
		{
			if (const FMetasoundFrontendNode* Node = InParams.NodePtr.Get())
			{
				if (const FMetasoundFrontendClass* Class = InParams.ClassPtr.Get())
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Frontend Node [NodeID:%ls, ClassID:%ls] is not of expected class class [ClassID:%ls]", *Node->GetID().ToString(), *Node->ClassID.ToString(), *Class->ID.ToString());
					}
				}
			}
		...
		FNodeHandle FOutputNodeController::CreateOutputNodeHandle(const FOutputNodeController::FInitParams& InParams)
		{
			if (const FMetasoundFrontendNode* Node = InParams.NodePtr.Get())
			{
				if (const FMetasoundFrontendClass* Class = InParams.ClassPtr.Get())
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Frontend Class of incorrect class type [ClassID:%ls] while creating output node.. Must be EMetasoundFrontendClassType::Output.", *Class->ID.ToString());
					}
				}
			}
			...
			if (bIsNodeOnRootGraph)
			{
				if (const FMetasoundFrontendNode* Node = NodePtr.Get())
				{
					if (ensure(Node->Interface.Outputs.Num() == 1))
					{
						...
						{
							if (IInterfaceRegistry::Get().ContainsOutput(InterfaceVersion, Output))
							{
		...
		FNodeHandle FInputNodeController::CreateInputNodeHandle(const FInputNodeController::FInitParams& InParams)
		{
			if (const FMetasoundFrontendNode* Node = InParams.NodePtr.Get())
			{
				if (const FMetasoundFrontendClass* Class = InParams.ClassPtr.Get())
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Frontend Class of incorrect class type [ClassID:%ls] while creating input node. Must be EMetasoundFrontendClassType::Input.", *Class->ID.ToString());
					}
				}
			}
				...
				if (const FMetasoundFrontendNode* Node = NodePtr.Get())
				{
					if (ensure(Node->Interface.Inputs.Num() == 1))
					{
						...
						for (const FMetasoundFrontendVersion& InterfaceVersion : InterfaceVersions)
						{
							...
							if (IInterfaceRegistry::Get().ContainsInput(InterfaceVersion, Input))
							{
								return InterfaceVersion;
							}
						}
