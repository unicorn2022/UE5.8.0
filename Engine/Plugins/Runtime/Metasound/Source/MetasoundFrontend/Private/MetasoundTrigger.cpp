...
namespace Metasound
{
	...
	{
		if (ensureMsgf(InFrameToTrigger >= 0, TEXT("Do not trigger negative frames [%d]"), InFrameToTrigger))
		{
...
const TCHAR* LexToString(const Metasound::FTrigger& Trigger)
{
	if (Trigger)
	{
		return TEXT("Metasound::FTrigger_True");
	}

	return TEXT("Metasound::FTrigger_False");
}
