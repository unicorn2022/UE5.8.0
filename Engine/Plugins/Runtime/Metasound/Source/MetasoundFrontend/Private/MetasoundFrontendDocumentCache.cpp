...
namespace Metasound::Frontend
{
	...
	void FDocumentGraphInterfaceCache::OnInputNameChanged(FName OldName, FName NewName)
	{
		...
		const bool bRemoved = InputNameToIndex.RemoveAndCopyValue(OldName, Index);
		check(bRemoved);
	...
	void FDocumentGraphInterfaceCache::OnInputDefaultChanged(const FDocumentArrayPagedInputArgs& Args)
	{
		++TransactionCount;
	}
	...
	{
		int32 Index = INDEX_NONE;
		const bool bRemoved = OutputNameToIndex.RemoveAndCopyValue(OldName, Index);
		check(bRemoved);
	...
	FDocumentCache::FDocumentDependencyCache::FDocumentDependencyCache(const FMetasoundFrontendDocument& InDocument)
	{
		...
		for (int32 Index = 0; Index < InDocument.Dependencies.Num(); ++Index)
		{
			...
			{
				UE_LOGF(LogMetaSound, Display, "MetaSound document with class name %ls found containing multiple dependencies with the same ID while initializing builder cache. Document may be currently versioning or require versioning past version 1.15", *InDocument.RootGraph.Metadata.GetClassName().ToString());
			}
#endif // !NO_LOGGING
