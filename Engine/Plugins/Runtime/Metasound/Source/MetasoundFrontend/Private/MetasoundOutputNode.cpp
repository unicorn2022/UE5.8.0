...
namespace Metasound
{
	namespace OutputNodePrivate
	{
		...
		TUniquePtr<IOperator> FOutputOperatorFactory::CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutResults)
		{
			...
			{
				checkf(IsCastable(Ref->GetDataTypeName(), Vertex.DataTypeName), TEXT("Mismatched data type names on output node (%s). Expected (%s), received (%s)."), *VertexName.ToString(), *Vertex.DataTypeName.ToString(), *(Ref->GetDataTypeName().ToString()));
