...
FMetasoundFrontendLiteral::FMetasoundFrontendLiteral(const Metasound::FLiteral& InLiteral)
{
	this->SetFromLiteral(InLiteral);
}
...
void FMetasoundFrontendLiteral::Set(FDefaultArray InValue)
{
	AsBoolean.Empty();
	AsInteger.Empty();
	AsFloat.Empty();
...
void FMetasoundFrontendLiteral::Set(TArray<bool> InValue)
{
	AsNumDefault = 0;
	AsInteger.Empty();
	AsFloat.Empty();
...
void FMetasoundFrontendLiteral::Set(TArray<int32> InValue)
{
	AsNumDefault = 0;
	AsBoolean.Empty();
	AsFloat.Empty();
...
void FMetasoundFrontendLiteral::Set(TArray<float> InValue)
{
	AsNumDefault = 0;
	AsBoolean.Empty();
	AsInteger.Empty();
	...
	AsFloat = MoveTemp(InValue);
}

void FMetasoundFrontendLiteral::Set(FString InValue)
{
	AsNumDefault = 0;
	AsBoolean.Empty();
	AsInteger.Empty();
	...
	AsString = { MoveTemp(InValue) };
}

void FMetasoundFrontendLiteral::Set(TArray<FString> InValue)
{
	AsNumDefault = 0;
	AsBoolean.Empty();
	AsInteger.Empty();
...
void FMetasoundFrontendLiteral::Set(TArray<UObject*> InValue)
{
	AsNumDefault = 0;
	AsBoolean.Empty();
	AsInteger.Empty();
...
void FMetasoundFrontendLiteral::SetFromLiteral(const Metasound::FLiteral& InLiteral)
{
	...
	switch (InLiteral.GetType())
	{
		...
		case ELiteralType::UObjectProxy:
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Cannot set UObjectProxy from Metasound::FLiteral");
			}
...
Metasound::FLiteral FMetasoundFrontendLiteral::ToLiteralNoProxy() const
{
	...
	{
		UE_LOGF(LogMetaSound, Error, "Cannot convert FMetasoundFrontendLiteral to Metasound::Literal without data type name [Literal:%ls]", *ToString());
