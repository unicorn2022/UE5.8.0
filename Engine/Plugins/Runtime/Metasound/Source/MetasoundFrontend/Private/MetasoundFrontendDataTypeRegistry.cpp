		...
		namespace MetasoundFrontendDataTypeRegistryPrivate
		{
			...
			public:
				...
				virtual FVertexInterface GetDefaultVertexInterface() const override
				{
					...
					{
						return FVertexInterface(NodeClassMetadata->DefaultInterface);
					}
					return FVertexInterface();
				}
			...
			public:
				...
				virtual bool UnregisterDataType(const FName& InDataTypeName) override;
				...
				virtual bool GetDataTypeInfo(UClass* InClass, FDataTypeRegistryInfo& OutInfo) const override;
			...
			bool FDataTypeRegistry::RegisterDataType(TUniquePtr<IDataTypeRegistryEntry>&& InEntry)
			{
				...
				if (InEntry.IsValid())
				{
					...
					INodeClassRegistry* NodeRegistry = INodeClassRegistry::Get();
					if (ensure(nullptr != NodeRegistry))
					{
						if (Entry->GetDataTypeInfo().bIsParsable)
						{
							const bool bIsAbstract = Entry->GetDataTypeInfo().bIsAbstract;;
					...
					UE_LOGF(LogMetaSound, Verbose, "Registered Metasound Datatype [Name:%ls].", *Name.ToString());
					return true;
				}
			...
			bool FDataTypeRegistry::UnregisterDataType(const FName& InDataTypeName)
			{
				METASOUND_LLM_SCOPE;

				bool bSuccess = false;
				if (const IDataTypeRegistryEntry* Entry = FindDataTypeRegistryEntry(InDataTypeName))
				{
					bSuccess = true;
					const FDataTypeRegistryInfo& DataTypeInfo = Entry->GetDataTypeInfo();

					// Unregister nodes associated with data type.
					INodeClassRegistry& NodeRegistry = INodeClassRegistry::GetChecked();
					if (DataTypeInfo.bIsParsable)
					{
						NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendInputClass().Metadata});
						NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendOutputClass().Metadata});
						NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendLiteralClass().Metadata});
						NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendVariableClass().Metadata});
						NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendVariableMutatorClass().Metadata});
						NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendVariableAccessorClass().Metadata});
						NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendVariableDeferredAccessorClass().Metadata});

						if (DataTypeInfo.bIsConstructorType)
						{
							NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendConstructorInputClass().Metadata});
							NodeRegistry.UnregisterNode(FNodeClassRegistryKey{Entry->GetFrontendConstructorOutputClass().Metadata});
						}
					}
					
					{
						UE::TScopeLock ObjectRegistryLock(RegistryObjectMapMutex);
						if (const UClass* Class = DataTypeInfo.ProxyGeneratorClass)
						{
							bSuccess = bSuccess && RegisteredObjectClasses.Remove(Class);
						}
					}
					
					{
						UE::TScopeLock RegistryLock(RegistryMapMutex);
						bSuccess = bSuccess && RegisteredDataTypes.Remove(InDataTypeName);
					}

				}

				UE_CLOGF(bSuccess, LogMetaSound, Verbose, "Unregistered Metasound Datatype [Name:%ls].", *InDataTypeName.ToString());
				UE_CLOGF(!bSuccess, LogMetaSound, Error, "Failed to unregistered Metasound Datatype [Name:%ls].", *InDataTypeName.ToString());
			...
			bool FDataTypeRegistry::GetDataTypeInfo(UClass* InClass, FDataTypeRegistryInfo& OutInfo) const
			{
				if (InClass)
				{
					UE::TScopeLock Lock(RegistryObjectMapMutex);
					for (UClass* Class = InClass; Class != UObject::StaticClass(); Class = Class->GetSuperClass())
					{
						if (const FName* DataTypeName = RegisteredObjectClasses.Find(Class))
						{
							if (const IDataTypeRegistryEntry* Entry = FindDataTypeEntry(*DataTypeName))
							{
								const FDataTypeRegistryInfo& Info = Entry->GetDataTypeInfo();
								if (Info.bIsExplicit && Class != InClass)
								{
									return false;
								}
								OutInfo = Info;
			...
			TSharedPtr<Audio::IProxyData> FDataTypeRegistry::CreateProxyFromUObject(const FName& InDataType, UObject* InObject) const
			{
				...
				if (const IDataTypeRegistryEntry* Entry = FindDataTypeEntry(InDataType))
				{
					...
					if (ProxyPtr)
					{
						UE_LOGF(LogMetaSound, VeryVerbose, "Created UObject proxy for '%ls'.", *InObject->GetName());
					}
					else
					{
						if (InObject && FApp::CanEverRenderAudio())
						{
							UE_LOGF(LogMetaSound, Error, "Failed to create a valid proxy from UObject '%ls'.", *InObject->GetName());
						}
					}
				}
			...
			FLiteral FDataTypeRegistry::CreateLiteralFromUObjectArray(const FName& InDataType, const TArray<UObject*>& InObjectArray) const
			{
				...
				if (!DataTypeEntry)
				{
					UE_LOGF(LogMetaSound, Error, "Failed to create a valid proxy from Array DataType '%ls': Type is not registered.", *InDataType.ToString());
				...
				{
					UE_LOGF(LogMetaSound, Error, "Failed to create a valid proxy from DataType '%ls': Type is not proxy parseable.", *InDataType.ToString());
					return Metasound::FLiteral(MoveTemp(ProxyArray));
				}
				...
				if (!ElementEntry)
				{
					UE_LOGF(LogMetaSound, Error, "Failed to create a valid proxy from DataType '%ls': ElementType '%ls' is not registered.", *InDataType.ToString(), *ElementDataType.ToString());
					return Metasound::FLiteral(MoveTemp(ProxyArray));
				}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			const IDataTypeRegistryEntry* FDataTypeRegistry::FindDataTypeEntry(const FName& InDataTypeName) const
			{
				UE::TScopeLock Lock(RegistryMapMutex);
				{
					...
					{
						UE_LOGF(LogMetaSound, Error, "Data type is not registered [Name:%ls]", *InDataTypeName.ToString());
					}
				}
		...
		void FPolymorphicDataTypeBase::ResolveParent(const IDataTypeRegistry& InRegistry) const
		{
			...
			if (!ParentDataType.IsValid() && !ParentTypeName.IsNone() && ParentTypeName != VoidString)
			{
				...
				UE_CLOGF(!ParentDataType.IsValid(), LogMetaSound, Warning, "Failed to resolve Polymorphic Type '%ls' Parent Type '%ls'",
		...
		{
			UE_LOGF(LogMetaSound, Verbose, "PolymorphicInterface Type=%ls, Base=%ls, bIsAbstract=%d",
				...
				if (const IDataTypeRegistryEntry* Entry = DataTypeRegistry.FindDataTypeRegistryEntry(InputVertex.DataTypeName))
				{
					...
					else
					{
						...
						if (DataTypeInfo.bIsParsable)
						{
							...
							UE_LOGF(LogMetaSound, Warning, "Failed to create default data reference for vertex %ls of data type %ls using constructor argument %ls", *InputVertex.VertexName.ToString(), *InputVertex.DataTypeName.ToString(), *::LexToString(InputVertex.GetDefaultLiteral()));
						}
					}
				}
				else
				{
					UE_LOGF(LogMetaSound, Warning, "Failed to create default data reference for vertex %ls of data type %ls because data type is not registered. Please ensure that the plugin which registers the data type is loaded.", *InputVertex.VertexName.ToString(), *InputVertex.DataTypeName.ToString());
				}
			}
		}
