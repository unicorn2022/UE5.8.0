...
namespace MetasoundArrayHashPrivate
{
	...
	FGuid GetArrayContentHashGuid(const TArray<ElementType>& InArray)
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Array Random Get: Please implement \"uint32 GetTypeHash(const T&)\" for type %ls to use Same Data for Shared State Behavior.", *Metasound::GetMetasoundDataTypeString<ElementType>());
			return FGuid();
		}
	}
}
...
namespace Metasound
{
	...
	{
		METASOUNDFRONTEND_API Frontend::FNodeClassRegistryKey CreateArrayRandomNodeClassRegistryKey(const FName& InArrayDataTypeName);
		METASOUNDFRONTEND_API FVertexInterface CreateArrayRandomNodeInterface(const FName& InArrayDataTypeName, const FName& InElementDataTypeName);
		METASOUNDFRONTEND_API FNodeClassMetadata CreateArrayRandomNodeClassMetadata(const FName& InArrayDataTypeName, const FName& InElementDataTypeName, const FText& InArrayDataTypeDisplayText);
	}
	...
	public:
		...
		static const FVertexInterface& GetDefaultInterface()
		{
			...
			using namespace ArrayRandomNodePrivate;

			static const FVertexInterface DefaultInterface = CreateArrayRandomNodeInterface(GetMetasoundDataTypeName<ArrayType>,GetMetasoundDataTypeName<ElementType>());
		...
		{
			using namespace ArrayRandomNodePrivate;
			static const FNodeClassMetadata Metadata = CreateArrayRandomNodeClassMetadata(GetMetasoundDataTypeName<ArrayType>(), GetMetasoundDataTypeName<ElementType>(), GetMetasoundDataTypeDisplayText<ArrayType>());
		...
		void Reset(const IOperator::FResetParams& InParams)
		{
			...
			*OutIndex = INDEX_NONE;
			...
			else
			{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Array Random Get: Graph Hierarchy environment variable needed for Same Node or Same Node in Composition shared state id not found (Graph '%ls')", *GraphName);
		...
		void Execute()
		{
			...
			if (InputArrayRef.Num() == 0)
			{
				...
				{
					UE_LOGF(LogMetaSound, Verbose, "Array Random Get: empty array input (Graph '%ls')", *GraphName);
