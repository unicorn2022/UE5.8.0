...
struct FMetasoundFrontendLiteral
{
	...
	UE_API FMetasoundFrontendLiteral(const Metasound::FLiteral& InLiteral);
...
public:
	...
	UE_API void Set(FDefaultArray InValue);
	UE_API void Set(bool InValue);
	UE_API void Set(TArray<bool> InValue);
	UE_API void Set(int32 InValue);
	UE_API void Set(TArray<int32> InValue);
	UE_API void Set(float InValue);
	UE_API void Set(TArray<float> InValue);
	UE_API void Set(FString InValue);
	UE_API void Set(TArray<FString> InValue);
	UE_API void Set(UObject* InValue);
	UE_API void Set(TArray<UObject*> InValue);
...
namespace Metasound::Frontend
{
	...
	UE_API EMetasoundFrontendLiteralType GetMetasoundFrontendLiteralType(ELiteralType InLiteralType);

	// Convenience function to convert EMetasoundFrontendLiteralType to Metasound::ELiteralType.
	UE_API ELiteralType GetMetasoundLiteralType(EMetasoundFrontendLiteralType InLiteralType);

	template<typename ... ArgTypes>
	FMetasoundFrontendLiteral MakeFrontendLiteral(ArgTypes&& ... InArgs)
	{
		FMetasoundFrontendLiteral Value;
		if constexpr (sizeof...(InArgs) == 0)
		{
			Value.Set(FMetasoundFrontendLiteral::FDefault{ });
		}
		else if constexpr (sizeof...(InArgs) == 1)
		{
			Value.Set(Forward<ArgTypes>(InArgs)...);
		}
		else
		{
			static_assert(sizeof...(InArgs) <= 1, "MakeFrontendLiteral: Invalid num arguments");
		}
	...
	FMetasoundFrontendLiteral MakeFrontendLiteral(TConstArrayView<TObjectPtr<ObjectType>> ObjectPtrs)
	{
		TArray<UObject*> Objects;
		auto ObjPtrToPtr = [](const TObjectPtr<ObjectType> Obj) { return Obj.Get(); };
		Algo::Transform(ObjectPtrs, Objects, ObjPtrToPtr);

		FMetasoundFrontendLiteral Value;
		Value.Set(MoveTemp(Objects));
		return Value;
	}
}
