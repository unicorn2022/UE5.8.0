...
struct FMetasoundFrontendClassStyleDisplay
{
	...
	,	StyleSet(InDisplayStyle.StyleSet)
	,	Icon(InDisplayStyle.Icon)
	,	Color(InDisplayStyle.Color)
	{
	}
...
struct FMetasoundFrontendClassInput : public FMetasoundFrontendClassVertex
{
	...
	FMetasoundFrontendLiteral DefaultLiteral = FMetasoundFrontendLiteral::GetInvalid();
#endif // WITH_EDITORONLY_DATA
...
#if WITH_EDITORONLY_DATA
	// Developer-supplied description of the interface
...
private:
	...
	uint16 AccessFlags = static_cast<uint16>(EMetasoundFrontendClassAccessFlags::Referenceable);
...
#if WITH_EDITORONLY_DATA
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "5.8 - Preset options are now serialized in the Preset MetaSound Template. See 'FMetaSoundFrontendPresetTemplate'"))
...
public:
#if WITH_EDITORONLY_DATA
	UPROPERTY(meta = (DeprecationMessage = "5.8 - PresetOptions is now characterized as configuration vertex metadata on MetaSound Document."))
...
public:
	...
	// Enables migrating properties during asynchronous versioning safely. Favor using this
	// over versioning properties on 'PostLoad' to avoid race conditions.
	virtual bool MigrateProperties() { return false; }
#endif // WITH_EDITORONLY_DATA
};
...
public:
#if WITH_EDITORONLY_DATA
	UE_DEPRECATED(5.8, "MaxVersion is now externally accessible via FVersioningManager::GetMaxDocumentVersion()")
...
public:
#if WITH_EDITORONLY_DATA
	UE_DEPRECATED(5.8, "Moved to internal implementation. See 'MigrateProperties'")
	...
	// Migrates data from deprecated properties to new properties
	UE_INTERNAL bool MigrateProperties();
#endif // WITH_EDITORONLY_DATA
};
