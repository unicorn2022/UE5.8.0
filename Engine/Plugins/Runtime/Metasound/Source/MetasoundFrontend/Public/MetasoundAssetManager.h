...
namespace Metasound::Frontend
{
	...
	public:
		...
		// Returns all class info associated with the given asset key (may be more than one due to errors in duplication
		...
		virtual TArray<Frontend::FMetaSoundClassInfo> FindRegisteredClassInfo(const FMetaSoundAssetKey& InKey) const = 0;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		// Returns default registration options for MetaSound assets
		UE_INTERNAL virtual Frontend::FMetaSoundAssetRegistrationOptions GetRegistrationOptions() const = 0;

		// Returns whether or not the given ClassMetadata is both valid asset ClassMetadata and is registered.
...
#if WITH_EDITORONLY_DATA
		UE_DEPRECATED(5.8, "This is a deprecated feature, and exists to port old soft object references to hard references.")
		virtual void RequestAsyncLoadReferencedAssets(FMetasoundAssetBase& InAssetBase, const TSet<FSoftObjectPath>* Paths = nullptr) { }
#endif // WITH_EDITORONLY_DATA

		UE_DEPRECATED(5.8, "Use 'ReregisterLoadedAssets' instead.")
		virtual void ReloadMetaSoundAssets() const { ReregisterLoadedAssets(); }
		...
		// Finds all currently loaded assets and re-registers them with the MetasoundUObjectRegistry. Ensures all paged data is targeted
		// appropriately and can be expensive.
		virtual void ReregisterLoadedAssets() const = 0;
		...
		virtual void WaitUntilAsyncLoadReferencedAssetsComplete(const FMetasoundAssetBase& InAssetBase) = 0;
#endif // WITH_EDITORONLY_DATA
	};

	class FMetaSoundAssetManagerBase : public IMetaSoundAssetManager
	{
	public:
		UE_API FMetaSoundAssetManagerBase();
		virtual ~FMetaSoundAssetManagerBase() = default;

		UE_API IMetaSoundAssetTransactor& GetTransactor();

	private:
		TUniquePtr<IMetaSoundAssetTransactor> Transactor;
	};
