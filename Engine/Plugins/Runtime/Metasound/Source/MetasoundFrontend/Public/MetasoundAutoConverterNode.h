...
namespace Metasound
{
	namespace AutoConverterNodePrivate
	{
		...
		METASOUNDFRONTEND_API FNodeClassName CreateAutoConverterNodeClassName(const FName& InFromDataType, const FName& InToDataType);
		METASOUNDFRONTEND_API Frontend::FNodeClassRegistryKey CreateAutoConverterNodeClassRegistryKey(const FName& InFromDataType, const FName& InToDataType);
		METASOUNDFRONTEND_API FNodeClassMetadata CreateAutoConverterNodeMetadata(const FConvertDataTypeInfo& InInfo);

		template<typename FromDataType, typename ToDataType>
		FConvertDataTypeInfo MakeConvertDataTypeInfo()
		{
			return FConvertDataTypeInfo
			{
	...
	public:
		...
		static FVertexInterface DeclareVertexInterface()
		{
			...
			return CreateVertexInterface(MakeConvertDataTypeInfo<FromDataType, ToDataType>());
		}

		static const FNodeClassMetadata& GetAutoConverterNodeMetadata()
		{
			using namespace AutoConverterNodePrivate;
			static const FNodeClassMetadata Info = CreateAutoConverterNodeMetadata(MakeConvertDataTypeInfo<FromDataType, ToDataType>());
	...
	private:
		...
		public:
			...
			virtual ~FConverterOperator() = default;
			...
			void Execute()
			{
				...
				else if constexpr (TIsAutoConvertible<FromDataType, ToDataType>::bIsIntToEnumConversion)
				{
					...
					if (!EnumName.IsSet())
					{
						...
						{
							UE_LOGF(LogMetaSound, Warning, "Cannot convert int32 value '%d' to enum type '%ls'. No valid corresponding enum value exists, so returning enum default value instead.", FromInt, *GetMetasoundDataTypeDisplayText<ToDataType>().ToString());
