...
namespace Metasound::Frontend
{
	...
	namespace MetasoundDataTypeRegistrationPrivate
	{
		...
		// Register the send and receive nodes of a datatype if it is supported
		template<typename TDataType>
		void RegisterSendAndReceiveNodes(const FModuleInfo& InModuleInfo)
		{
			if constexpr (TIsTransmittable<TDataType>::Value && TEnableTransmissionNodeRegistration<TDataType>::Value)
			{
		...
		// Unregister the send and receive nodes related to a datatype.
		UE_INTERNAL
		UE_API void UnregisterSendAndReceiveNodes(const FName& InDataTypeName, const FModuleInfo& InModuleInfo);

		// Register an autoconverter node for a specific data type conversion. Only registers the node if it is supported and not already registered. 
		template<typename TFromDataType, typename TToDataType>
		void RegisterAutoConverterNode(INodeClassRegistry& InRegistry, const FModuleInfo& InModuleInfo)
		{
			if constexpr (TIsAutoConvertible<TFromDataType, TToDataType>::Value && TEnableAutoConverterNodeRegistration<TFromDataType, TToDataType>::Value && !std::is_same<TFromDataType, TToDataType>::value)
			{
				using namespace AutoConverterNodePrivate;

				const FName& FromDataTypeName = GetMetasoundDataTypeName<TFromDataType>();
				const FName& ToDataTypeName = GetMetasoundDataTypeName<TToDataType>();

				FConvertDataTypeInfo ConvertInfo = MakeConvertDataTypeInfo<TFromDataType, TToDataType>();
				const FNodeClassMetadata Metadata = CreateAutoConverterNodeMetadata(ConvertInfo);
				const Metasound::Frontend::FNodeClassRegistryKey Key{Metadata};

				if (!InRegistry.IsNodeRegistered(Key))
				{
					...
					{
						FConverterNode::GetInputName(), 
						FConverterNode::GetOutputName(), 
						FNodeClassRegistryKey{Metadata}
					};
					InRegistry.RegisterConversionNode(FConverterNodeClassRegistryKey{FromDataTypeName, ToDataTypeName}, ConverterNodeInfo);
				}
			}
		}

		// Unregister any autoconverter node that is registered for going converting between specific data types
		UE_INTERNAL
		UE_API void UnregisterAutoConverterNode(INodeClassRegistry& InRegistry, const FName& InFromDataTypeName, const FName& InToDataTypeName, const FModuleInfo& InModuleInfo);

		// Register a slew of basic converter nodes for basic types
		template<typename TDataType>
		void RegisterAutoConverterNodes(const FModuleInfo& InModuleInfo)
		{
			INodeClassRegistry& Registry = INodeClassRegistry::GetChecked();
			// Conversions to this data type:
			RegisterAutoConverterNode<bool, TDataType>(Registry, InModuleInfo);
			RegisterAutoConverterNode<int32, TDataType>(Registry, InModuleInfo);
			RegisterAutoConverterNode<float, TDataType>(Registry, InModuleInfo);
			RegisterAutoConverterNode<FString, TDataType>(Registry, InModuleInfo);

			// Conversions from this data type:
			RegisterAutoConverterNode<TDataType, bool>(Registry, InModuleInfo);
			RegisterAutoConverterNode<TDataType, int32>(Registry, InModuleInfo);
			RegisterAutoConverterNode<TDataType, float>(Registry, InModuleInfo);
			RegisterAutoConverterNode<TDataType, FString>(Registry, InModuleInfo);
		}

		// Unregister a slew of basic converter nodes for basic types.
		UE_INTERNAL
		UE_API void UnregisterAutoConverterNodes(const FName& InDataTypeName, const FModuleInfo& InModuleInfo);
		...
		bool RegisterDataTypeWithFrontendInternal(const FModuleInfo& InModuleInfo)
		{
			...
			class FDataTypeRegistryEntry : public FDataTypeRegistryEntryBase
			{
				...
				{
					if constexpr (std::is_copy_assignable_v<TDataType> && bIsParsable && !std::is_abstract<TDataType>::value)
					{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
				...
				{
					if constexpr (bIsParsable && !std::is_abstract<TDataType>::value)
					{
						switch (InAccessType)
						{
			...
			bool bSucceeded = IDataTypeRegistry::Get().RegisterDataType(MakeUnique<FDataTypeRegistryEntry>());

			if (bSucceeded)
			{
				RegisterAutoConverterNodes<TDataType>(InModuleInfo);
				RegisterSendAndReceiveNodes<TDataType>(InModuleInfo);
			}
			else
			{
				UE_LOGF(LogMetaSound, Error, "Failed to register MetaSound DataType %ls", *GetMetasoundDataTypeName<TDataType>().ToString());
			}
			
			return bSucceeded;
		}

		// Unregister a datatype as well as some autogenerated nodes. This should unregister all nodes that were registered
		// during RegisterDataTypeWIthFrontendInternal<>(...)
		UE_INTERNAL
		UE_API bool UnregisterDataTypeWithFrontendInternal(const FName& InDataTypeName, const FModuleInfo& InModuleInfo);
		...
		bool RegisterDataTypeArrayWithFrontend(const FModuleInfo& InModuleInfo) 
		{
			...
			if constexpr (TEnableAutoArrayTypeRegistration<TDataType>::Value && !CIsPolymorphicAndAbstract<TDataType>)
			{
		...
		template<typename TDataType>
		bool UnregisterDataTypeArrayWithFrontend(const FModuleInfo& InModuleInfo)
		{
			using namespace MetasoundDataTypeRegistrationPrivate;
			...
			if constexpr (TEnableAutoArrayTypeRegistration<TDataType>::Value && !CIsPolymorphicAndAbstract<TDataType>)
			{
				...
				const FName& DataTypeName = GetMetasoundDataTypeName<TArrayType>();

				// Unregistration methods are called in reverse order of registration 
				// functions.
PRAGMA_DISABLE_INTERNAL_WARNINGS
				bool bSuccess = UnregisterDataTypeWithFrontendInternal(GetMetasoundDataTypeName<TVariable<TArrayType>>(), InModuleInfo);
				bSuccess = bSuccess && UnregisterArrayNodes(DataTypeName, InModuleInfo);
				bSuccess = bSuccess && UnregisterDataTypeWithFrontendInternal(DataTypeName, InModuleInfo);
PRAGMA_ENABLE_INTERNAL_WARNINGS
	...
	/** Unregisters a data type with the MetaSound Frontend. 
	 *
	 * @tparam TDataType - The data type to register.
	 *
	 * @return True on success, false on failure.
	 */
	template<typename TDataType>
	bool UnregisterDataType(const FModuleInfo& InModuleInfo)
	{
		using namespace MetasoundDataTypeRegistrationPrivate;

PRAGMA_DISABLE_INTERNAL_WARNINGS
		// Unregistration is done in reverse order of registration
		bool bSuccess = UnregisterDataTypeArrayWithFrontend<TDataType>(InModuleInfo);
		bSuccess = bSuccess && UnregisterDataTypeWithFrontendInternal(GetMetasoundDataTypeName<TVariable<TDataType>>(), InModuleInfo);
		bSuccess = bSuccess && UnregisterDataTypeWithFrontendInternal(GetMetasoundDataTypeName<TDataType>(), InModuleInfo);
PRAGMA_ENABLE_INTERNAL_WARNINGS
...
#define ENQUEUE_METASOUND_DATATYPE_REGISTRATION_COMMAND(DataType, DataTypeName, ...) \
	...
	METASOUND_IMPLEMENT_REGISTRATION_ACTION(UE_JOIN(_DataTypeRegistration,__COUNTER__), (::Metasound::Frontend::RegisterDataType<DataType, ##__VA_ARGS__>), (::Metasound::Frontend::UnregisterDataType<DataType>))
...
// Helper macro for disabling auto array registration
#define DISABLE_METASOUND_ARRAYS_FOR_TYPE(TYPENAME)\
	template<>\
	struct TEnableAutoArrayTypeRegistration<TYPENAME>\
	{\
		static constexpr bool Value = false;\
	};
