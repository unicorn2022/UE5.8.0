	...
	public:
		...
		bool PushLiteral(const FLiteral& InLiteral) override
		{
			if (!TLiteralTraits<TDataType>::IsParsable(InLiteral))
			{
				...
				{
					UE_LOGF(LogMetaSound, Error, "Failed to send data. Data type [TypeName:%ls] cannot be parsed from literal [Literal:%ls].", *GetMetasoundDataTypeString<TDataType>(), *LexToString(InLiteral));
