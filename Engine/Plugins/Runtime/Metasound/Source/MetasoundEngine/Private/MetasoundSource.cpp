...
// Clamp a CAT format's channel count to the nearest mixer-compatible value (1, 2, 4, 6, 8).
// The AudioMixer only supports these counts for buffer allocation and interleaving.
// The actual CAT format identity is carried separately via SourceFormatName for transcoding.
static int32 ClampToMixerChannelCount(int32 InChannels)
{
	if (InChannels <= 1) return 1;
	if (InChannels <= 2) return 2;
	if (InChannels <= 4) return 4;
	if (InChannels <= 6) return 6;
	return 8;
}
...
namespace Metasound
{
	...
	namespace SourcePrivate
	{
		...
		FAudioParameter MakeAudioParameter(const Frontend::IDataTypeRegistry& InRegistry, FName InParamName, FName InTypeName, const FMetasoundFrontendLiteral& InLiteral, bool bCreateUObjectProxies) 
		{
			...
			const EMetasoundFrontendLiteralType InLiteralType = InLiteral.GetType(); 

			if (Params.TypeName == TriggerName)
			{
				Params.ParamType = EAudioParameterType::Trigger;
				check(InLiteralType == EMetasoundFrontendLiteralType::Boolean
					|| InLiteralType == EMetasoundFrontendLiteralType::None);
			}
...
public:
	...
	void CollectPresetOverrides(const FMetasoundFrontendDocument& Document, const Metasound::TSortedVertexNameMap<FRuntimeInput>& InInputMap)
	{
		using namespace Metasound;
		using namespace Metasound::Frontend;

		const FPresetTemplate* PresetTemplate = Document.Template.GetPtr<FPresetTemplate>();
		if (!PresetTemplate)
		{
			return;
		}

		if (InInputMap.IsEmpty())
		{
			return;
		}

		TArray<const FMetasoundFrontendClassInput*> ClassInputs;
		Algo::Transform(Document.RootGraph.GetDefaultInterface().Inputs, ClassInputs, [](const FMetasoundFrontendClassInput& Input) { return &Input; });
		...
		for (const TPair<FVertexName, FRuntimeInput>& Pair : InInputMap)
		{
			...
			{
				auto IsInput = [&Pair](const FMetasoundFrontendClassInput* Input) { return Input->Name == Pair.Key; };
				const int32 ClassIndex = ClassInputs.IndexOfByPredicate(IsInput);
				if (ClassIndex != INDEX_NONE)
				{
					// Avoid adding parameters which are NOT overriding a default.
					const FGuid& NodeID = ClassInputs[ClassIndex]->NodeID;
					if (const FPresetVertexMetadata* VertexMetadata = PresetTemplate->FindConstVertexMetadata<FPresetVertexMetadata>(NodeID))
					{
					...
					ClassInputs.RemoveAtSwap(ClassIndex, EAllowShrinking::No);
				}
			}
		}
	}
	...
	, FMetasoundAssetBase()
{
	...
	// Note: Can't call FDiscreteChannelAgnosticType::GetDefaultCatFormat() here as the CDO
	// constructor runs during module load before the channel registry is initialized.
	static const FName DefaultDiscreteFormat(TEXT("Cat:Mono1Dot0"));
	ChannelAgnosticCustomFormat = DefaultDiscreteFormat;
}
...
void UMetaSoundSource::PostEditUndo()
{
	...
	Metasound::Engine::FAssetHelper::PostEditUndo(DocInterface);
}
...
void UMetaSoundSource::PostEditChangeProperty(FPropertyChangedEvent& InEvent)
{
	...
	// When the CAT custom format or format chooser changes, resolve NumChannels immediately
	// so the mixer allocates the correct buffer size on the next play. (UE-370811)
	// NumChannels must be mixer-compatible (1, 2, 4, 6, 8) — the CAT format identity is
	// carried separately via SourceFormatName, and transcoding handles the conversion.
	if (InEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UMetaSoundSource, ChannelAgnosticCustomFormat) ||
		InEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UMetaSoundSource, ChannelAgnosticFormatChooser))
	{
		if (OutputFormat == EMetaSoundOutputAudioFormat::ChannelAgnostic &&
			ChannelAgnosticFormatChooser == EMetasoundChannelAgnosticSourceFormatChooser::Custom &&
			!ChannelAgnosticCustomFormat.IsNone())
		{
			PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
			if (const Audio::FChannelTypeFamily* Found = Audio::GetChannelRegistry().FindChannel(ChannelAgnosticCustomFormat))
			{
				NumChannels = ClampToMixerChannelCount(Found->NumChannels());
			}
...
bool UMetaSoundSource::CanEditChange(const FProperty* InProperty) const
{
	...
	if (InProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UMetaSoundSource, QualitySetting))
	{
		...
		const float DefaultSampleRate = SampleRateOverride.GetDefault();
...
bool UMetaSoundSource::ConformObjectToDocument()
{
	...
	for (const FOutputAudioFormatInfoPair& Pair : FormatInfo)
	{
		...
		{
			// Channel Agnostic (CAT) output: resolve NumChannels from the custom format
			// if one is selected, otherwise default to 1 (mono). NumChannels here is the
			// mixer-facing channel count, clamped to supported values (1,2,4,6,8).
			if (Pair.Key == EMetaSoundOutputAudioFormat::ChannelAgnostic)
			{
				// Resolve NumChannels now if a custom format is selected and the registry is available.
				// Previously this was hardcoded to 1 as a placeholder, causing a two-save sync bug (UE-370811).
				int32 ResolvedChannels = 1;
				if (ChannelAgnosticFormatChooser == EMetasoundChannelAgnosticSourceFormatChooser::Custom && !ChannelAgnosticCustomFormat.IsNone())
				{
					PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
					if (const Audio::FChannelTypeFamily* Found = Audio::GetChannelRegistry().FindChannel(ChannelAgnosticCustomFormat))
					{
						ResolvedChannels = ClampToMixerChannelCount(Found->NumChannels());
					}
					PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
				}

				// Only mark altered if something actually changed (avoids unnecessary asset dirtying).
				if (OutputFormat != Pair.Key || NumChannels != ResolvedChannels)
				{
					OutputFormat = Pair.Key;
					NumChannels = ResolvedChannels;
					bDidAlterObjectData = true;
				}
			}
			else if ((OutputFormat != Pair.Key) || (NumChannels != Pair.Value.OutputVertexChannelOrder.Num()))
			{
				OutputFormat = Pair.Key;
	...
	const FName InPlatformName, Metasound::SourcePrivate::FCookedQualitySettings& OutQualitySettings) const
{
	...
	if (const UMetaSoundSettings* Settings = GetDefault<UMetaSoundSettings>())
	{
		if (const FMetaSoundQualitySettings* Found = Settings->GetQualitySettings().FindByPredicate([&QT = QualitySetting](const FMetaSoundQualitySettings& Q) -> bool { return Q.Name == QT; }))
		{
			...
			{
				UE_LOGF(LogMetaSound, VeryVerbose, "Metasound [%ls] using Quality '%ls', BlockRate=%3.3f", *GetName(), *QualitySetting.ToString(), Value);
				OutQualitySettings.BlockRate = Value;
			}
			if (const int32 Value = Found->SampleRate.GetValueForPlatform(InPlatformName); Value > 0)
			{
				UE_LOGF(LogMetaSound, VeryVerbose, "Metasound [%ls] using Quality '%ls', SampleRate=%d", *GetName(), *QualitySetting.ToString(), Value);
	...
	{
		UE_LOGF(LogMetaSound, VeryVerbose, "Metasound [%ls] BlockRate Override: %3.3f", *GetName(), SerializedBlockRate);
	...
	{
		UE_LOGF(LogMetaSound, VeryVerbose, "Metasound [%ls] SampleRate Override: %d", *GetName(), SerializedSampleRate);
		OutQualitySettings.SampleRate = SerializedSampleRate;
	}
	
	// Success.
	UE_LOGF(LogMetaSound, Verbose, "Metasound [%ls] using SampleRate=%d, BlockRate=%2.3f (not-cooked)",
	...
	if (CookedQualitySettings)
	{
		...
		{
			UE_LOGF(LogMetaSound, VeryVerbose, "Metasound [%ls] BlockRate: %3.3f", *GetName(), SerializedBlockRate);
			OutQualitySettings.BlockRate = SerializedBlockRate;
		}
		if (const int32 SerializedSampleRate = CookedQualitySettings->SampleRate.GetValue(); SerializedSampleRate > 0)
		{
			UE_LOGF(LogMetaSound, VeryVerbose, "Metasound [%ls] SampleRate: %d", *GetName(), SerializedSampleRate);
			OutQualitySettings.SampleRate = SerializedSampleRate;
		}
	
		UE_LOGF(LogMetaSound, Verbose, "Metasound [%ls] using SampleRate=%d, BlockRate=%2.3f (cooked)",
			*GetName(), OutQualitySettings.SampleRate.GetValue(), OutQualitySettings.BlockRate.GetValue()) ;
		return true;
	}
...
static TSharedPtr<const Audio::FDiscreteChannelTypeFamily> NumChannelsToFormat(const int32 NumChannels)
{
	return Audio::FChannelAgnosticUtils::FindDiscreteFormatFromNumChannels(NumChannels);
}

TSharedPtr<const Audio::FChannelTypeFamily> UMetaSoundSource::CalculateAutoSourceFormat(const TArray<FAudioParameter>& InParams) const
{
	using namespace Audio;

	// This only applies if we're set to Channel Agnostic Output.
	if (OutputFormat == EMetaSoundOutputAudioFormat::ChannelAgnostic)
	{
		// Default behavior for automatic channel format determination is to find
		// the greatest channel count. In the future there may be additional controls
		// to influence how this channel count is determined.
		int32 GreatestNumChannels = 1;

		checkf(RuntimeInputData.bIsValid.load(), TEXT("RuntimeInputData must be cached before calling CalculateAutoSourceFormat"));

		// Iterate through all inputs 
		for (const auto& Pair : RuntimeInputData.InputMap)
		{
			const FRuntimeInput& RuntimeInput = Pair.Value;
			const FAudioParameter* Parameter = &RuntimeInput.DefaultParameter;
			// Check for overrides from input parameters
			if (const FAudioParameter* Found = InParams.FindByPredicate([&RuntimeInput](const FAudioParameter& Param) -> bool { return Param.ParamName == RuntimeInput.Name; }))
			{
				Parameter = Found;
			}

			// Update greatest channel count
			if ((EAudioParameterType::Object == Parameter->ParamType) || (EAudioParameterType::ObjectArray == Parameter->ParamType))
			{
				for (const TSharedPtr<IProxyData>& ObjProxy : Parameter->ObjectProxies)
				{
					if (const ISoundWaveContainer* Container = QueryInterface<ISoundWaveContainer>(ObjProxy.Get()))
					{
						for (const FSoundWaveProxyPtr& SoundProxyPtr : Container->GetContainedWaveProxies())
						{
							if (SoundProxyPtr.IsValid())
							{
								GreatestNumChannels = FMath::Max(GreatestNumChannels, (int32)SoundProxyPtr->GetSoundWaveDataRef()->GetNumChannels());

							}
						}
					}
					else if (const FSoundWaveProxy* SoundProxy = QueryInterface<FSoundWaveProxy>(ObjProxy.Get()))
					{
						GreatestNumChannels = FMath::Max(GreatestNumChannels, (int32)SoundProxy->GetSoundWaveDataRef()->GetNumChannels());
					}
				}
		...
		if (const TSharedPtr<const Audio::FDiscreteChannelTypeFamily> SP = NumChannelsToFormat(GreatestNumChannels))
		{
			return SP;	
		}
	}
...
TArray<FMetasoundAssetBase*> UMetaSoundSource::GetReferencedAssets() const
{
...
#if WITH_EDITORONLY_DATA
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	Metasound::Engine::FAssetHelper::PostLoad(*this, ReferenceAssetClassCache);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
#endif // WITH_EDITORONLY_DATA
...
void UMetaSoundSource::ResolveQualitySettings(const UMetaSoundSettings* Settings)
{
	...
	if (!Resolved && QualitySettingGuid.IsValid())
	{
		...
		Resolved = Settings->GetQualitySettings().FindByPredicate(FindByGuid);
	}
	...
	if (!Resolved)
	{
		...
		//UE_LOGF(LogMetaSound, Warning, "Failed to resolve Quality '%ls', resetting to the default.", *QualitySetting.ToString());
...
void UMetaSoundSource::InitParameters(TArray<FAudioParameter>& ParametersToInit, FName InFeatureName)
{
	...
	else
	{
		...
		else
		{
			...
			UE_CLOGF(HasNotBeenLoggedForThisObject(*this, __LINE__), LogMetaSound, Warning, "Initializing parameters on uninitialized UMetaSoundSource %ls will result in slower performance. UMetaSoundSource::InitResources should finish executing on the game thread before attempting to call UMetaSoundSource::InitParameters(...)", *GetOwningAssetName());
...
void UMetaSoundSource::InitResources()
{
	...
	else
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Attempt to call UMetaSoundSource::InitResources() on %ls from thread which may not provide garbage collection safety of the UMetaSoundSource", *GetOwningAssetName());
		}
...
Metasound::Frontend::FDocumentAccessPtr UMetaSoundSource::GetDocumentAccessPtr(bool bReloadBuilder)
{
	using namespace Metasound::Frontend;
...
UMetaSoundSource::FResolvedSourceFormat UMetaSoundSource::ResolveSourceFormatAndNumChannels(const FSoundGeneratorInitParams& InParams, const TArray<FAudioParameter>& InDefaultParameters, Metasound::FMetasoundEnvironment& InOutEnvironment)
{
	using namespace Metasound::Frontend;

	FResolvedSourceFormat Result;
	if (OutputFormat == EMetaSoundOutputAudioFormat::ChannelAgnostic)
	{
		// Priority: 1) user-specified custom format, 2) auto-detect from wave proxy assets,
		// 3) return error condition for undetermined type
		if (ChannelAgnosticFormatChooser == EMetasoundChannelAgnosticSourceFormatChooser::Custom && !ChannelAgnosticCustomFormat.IsNone())
		{
			PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
			if (const Audio::FChannelTypeFamily* Found = Audio::GetChannelRegistry().FindChannel(ChannelAgnosticCustomFormat))
			{
				Result.SourceFormatName = Found->GetName();
				InOutEnvironment.SetValue<FName>(SourceInterface::Environment::SourceFormatName, Result.SourceFormatName);
				Result.NumChannels = ClampToMixerChannelCount(Found->NumChannels());
			}
			PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
			else
			{
				UE_LOGF(LogMetaSound, Warning, "Custom format '%ls' not found in channel registry", *ChannelAgnosticCustomFormat.ToString());
				Result.NumChannels = ClampToMixerChannelCount(InParams.NumChannels);
			}
		}
		else if (TSharedPtr<const Audio::FChannelTypeFamily> SP = CalculateAutoSourceFormat(InDefaultParameters); SP.IsValid())
		{
			Result.SourceFormatName = SP->GetName();
			InOutEnvironment.SetValue<FName>(SourceInterface::Environment::SourceFormatName, Result.SourceFormatName);
			Result.NumChannels = ClampToMixerChannelCount(SP->NumChannels());
		}
		else
		{
			UE_LOGF(LogMetaSound, Error, "Failed to determine concrete format for ChannelAgnotistic output on source \"%ls\"", *GetName()) ;
		}
	}
	else
	{
		Result.NumChannels = GetOutputAudioChannelOrder().Num();
	}
...
ISoundGeneratorPtr UMetaSoundSource::CreateSoundGenerator(const FSoundGeneratorInitParams& InParams, TArray<FAudioParameter>&& InDefaultParameters)
{
	...
	const UMetaSoundSource::FResolvedSourceFormat ResolvedFormat = ResolveSourceFormatAndNumChannels(InParams, InDefaultParameters, Environment);
	if (ResolvedFormat.NumChannels == INDEX_NONE)
	{
		// Failed to resolve number of channels. This should not happen, but if it does,
		// do not attempt to render audio.
		return nullptr;
	}
	NumChannels = ResolvedFormat.NumChannels;
	...
	if (InParams.StartTime > 0.0f)
	{
		...
		else if(!bHasWarnedAboutStartTime)
		{
			const FString AssetName = GetName();
			UE_LOGF(LogMetaSound, Warning, "A non-zero StartTime (%.2f) was used in asset '%ls' that doesn't implement the Start Time interface. StartTime will be ignored.", InParams.StartTime, *AssetName);
			bHasWarnedAboutStartTime = true;
		}
	}
	...
	if (bIsDynamic)
	{
		...
		FMetasoundDynamicGraphGeneratorInitParams InitParams
		{
			FGeneratorInitParams
			{
				...
				.SourceFormatName = ResolvedFormat.SourceFormatName
			},
...
TArray<UMetaSoundSource::FRuntimeInput> UMetaSoundSource::GatherVerseParameters() const
{
	using namespace Metasound;
	using namespace Metasound::Frontend;
	using namespace Metasound::Engine;
	...
	TArray<FRuntimeInput> OutParameters;
	OutParameters.Reserve(InputMap.Num());
	for(const TPair<FVertexName, FRuntimeInput>& Pair : InputMap)
	{
		OutParameters.Add(Pair.Value);
	}
...
bool UMetaSoundSource::GetAllDefaultParameters(TArray<FAudioParameter>& OutParameters) const
{
	...
	if (Metasound::CanEverExecuteGraph())
	{
		...
		{
			UE_CLOGF(SourcePrivate::HasNotBeenLoggedForThisObject(*this, __LINE__), LogMetaSound, Warning, "Default parameters will be ommitted. Accessing invalid runtime data on MetaSound %ls. Ensure that UMetaSoundSource::InitResources() is executed on the game thread before calling UMetaSoundSource::GetAllDefaultParameters(...)", *GetOwningAssetName());
			return false;
		}
...
void UMetaSoundSource::InitParametersInternal(const Metasound::TSortedVertexNameMap<FRuntimeInput>& InInputMap, TArray<FAudioParameter>& ParametersToInit, FName InFeatureName) const
{
	...
	for (int32 i = ParametersToInit.Num() - 1; i >= 0; --i)
	{
		...
		if (!bIsParameterValid)
		{
			...
			{
				const FString AssetName = GetName();
				UE_LOGF(LogMetaSound, Warning, "Failed to set parameter '%ls' in asset '%ls': No name specified, no transmittable input found, or type mismatch.", *Parameter.ParamName.ToString(), *AssetName);
			}
#endif // !NO_LOGGING
		}
	}
...
TSharedPtr<Audio::IParameterTransmitter> UMetaSoundSource::CreateParameterTransmitter(Audio::FParameterTransmitterInitParams&& InParams) const
{
	...
	if (bCreateInputMapOnTheFly)
	{
		if (!bIsBuilderActive)
		{
			...
			UE_LOGF(LogMetaSound, Warning, "Creating a Parameter Transmiiter on uninitialized UMetaSoundSource %ls will result in slower performance. UMetaSoundSource::InitResources should finish executing on the game thread before attempting to call UMetaSoundSource::CreateParameterTransmitter(...)", *GetOwningAssetName());
		}
...
{	
	...
	UE_CLOGF(!bFoundQualitySettings && IsAsset(), LogMetaSound, Error, "Could not retrieve quality settings for asset %ls", *GetOwningAssetName());
	...
	UE_LOGF(LogMetaSound, Verbose, "Metasound [%ls] GetOperatorSettings: SampleRate: %d, BlockRate: %3.3f",
...
void UMetaSoundSource::OnBeginActiveBuilder()
{
	...
	if (bIsBuilderActive)
	{
		UE_LOGF(LogMetaSound, Error, "OnBeginActiveBuilder() call while prior builder is still active. This may indicate that multiple builders are attempting to modify the MetaSound %ls concurrently.", *GetOwningAssetName())
	}
	...
	if (bInIsEnabled)
	{
		if (!DynamicTransactor.IsValid())
		{
			...
			if (IsRegistered())
			{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Failed to get existing graph for dynamic metasound %ls. Initializing to empty graph.", *GetOwningAssetName());
...
UMetaSoundSource::FRuntimeInput UMetaSoundSource::CreateRuntimeInput(const Metasound::Frontend::IDataTypeRegistry& Registry, const FMetasoundFrontendClassInput& Input, bool bCreateUObjectProxies, TArrayView<const FGuid> InPageOrder)
{
	...
	const TArray<FMetasoundFrontendClassInputDefault>& InputDefualts = Input.GetDefaults();
	const bool bIsPaged = InputDefualts.Num() != 1 || InputDefualts.Last().PageID != Frontend::DefaultPageID;

	const IDataTypeRegistryEntry* RegistryEntry = Registry.FindDataTypeRegistryEntry(Input.TypeName);
	const FMetasoundFrontendClassInputDefault* Default = FindPreferredPage(InputDefualts, InPageOrder);

	UE_CLOGF(!RegistryEntry, LogMetaSound, Warning, "Failed to find data type '%ls' in registry. Assuming data type is not transmittable", *Input.TypeName.ToString());
	UE_CLOGF(!Default, LogMetaSound, Error, "Failed to find default literal for parameter %ls", *Input.Name.ToString());

	const bool bIsTransmittable = RegistryEntry? RegistryEntry->GetDataTypeInfo().bIsTransmittable : false;

	const FAudioParameter DefaultParameter = SourcePrivate::MakeAudioParameter
	(
		  Registry
		, Input.Name
		, Input.TypeName
		, (Default)? Default->Literal : FMetasoundFrontendLiteral(Registry.CreateDefaultLiteral(Input.TypeName))
		, bCreateUObjectProxies
	);

	return FRuntimeInput
	{
		  Input.Name
		, Input.TypeName
		, Input.AccessType
...
Metasound::TSortedVertexNameMap<UMetaSoundSource::FRuntimeInput> UMetaSoundSource::CreateRuntimeInputMap(bool bCreateUObjectProxies, TArrayView<const FGuid> InPageOrder) const
{
	...
	const IInterfaceRegistry& InterfaceRegistry = IInterfaceRegistry::Get();
	...
	for (const FMetasoundFrontendVersion& InterfaceVersion : Doc.Interfaces)
	{
		FName RouterName;
		if (IInterfaceRegistry::Get().FindInterfaceRouter(InterfaceVersion, RouterName))
		{
			if (RouterName != Audio::IParameterTransmitter::RouterName)
			{
				FMetasoundFrontendInterface Interface;
				InterfaceRegistry.FindInterface(InterfaceVersion, Interface);
	...
	for (const FMetasoundFrontendClassInput& Input : Doc.RootGraph.GetDefaultInterface().Inputs)
	{
		...
		{
			PublicInputs.Add(Input.Name, CreateRuntimeInput(DataTypeRegistry, Input, bCreateUObjectProxies, InPageOrder));
		}
	}
	...
	FAudioParameter ParameterPackDefaultParameter = SourcePrivate::MakeAudioParameter(DataTypeRegistry, ParameterPackInput.Name, ParameterPackInput.TypeName, DefaultLiteral, bCreateUObjectProxies) ;
...
void UMetaSoundSource::CacheRuntimeInputData(TArrayView<const FGuid> InPageOrder)
{
	...
	if (const FPresetTemplate* PresetTemplate = RootMetasoundDocument.Template.GetPtr<FPresetTemplate>())
	{
		auto IsOverriddenConstructorInput = [this, &PresetTemplate](const FMetasoundFrontendClassInput& Input)
		{
			if (const FRuntimeInput* RuntimeInput = RuntimeInputData.InputMap.Find(Input.Name))
			{
				if (const FPresetVertexMetadata* VertexMetadata = PresetTemplate->FindConstVertexMetadata<FPresetVertexMetadata>(Input.NodeID))
				{
					return VertexMetadata->bOverrideInheritedDefault && RuntimeInput->AccessType == EMetasoundFrontendVertexAccessType::Value;
				}
			}
			return false;
		};

		const TArray<FMetasoundFrontendClassInput>& Inputs = RootMetasoundDocument.RootGraph.GetDefaultInterface().Inputs;
		bIsPresetGraphInflationSupported = !Algo::AnyOf(Inputs, IsOverriddenConstructorInput);
	}
...
const UMetaSoundSource& UMetaSoundSource::FindFirstNoninflatableSourceInternal(TArray<FGuid>& OutHierarchy, TFunctionRef<void(const UMetaSoundSource&)> OnTraversal) const
{
	...
	if (!bIsDynamic && ConsoleVariables::bEnableExperimentalRuntimePresetGraphInflation && bIsPresetGraphInflationSupported)
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Attempt to reference parent of metasound preset failed due to unexpected number of reference asses (%d) from MetaSound Preset %ls", ReferencedAssetClassObjects.Num(), *GetOwningAssetName());
		}
	}
...
TSharedPtr<const Metasound::IGraph> UMetaSoundSource::FindFirstNoninflatableGraph(UMetaSoundSource::FAudioParameterCollector& InOutParameterCollector, Metasound::FMetasoundEnvironment& InOutEnvironment) const
{
	...
	auto OnGraphInflation = [&InOutParameterCollector](const UMetaSoundSource& InInflatedSource)
	{
		...
		InOutParameterCollector.CollectPresetOverrides(InInflatedSource.RootMetasoundDocument, InInflatedSource.RuntimeInputData.InputMap);
	};
