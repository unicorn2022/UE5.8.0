...
FMetaSoundBuilderNodeOutputHandle UMetaSoundBuilderBase::AddGraphInputNode(FName Name, FName DataType, FMetasoundFrontendLiteral DefaultValue, EMetaSoundBuilderResult& OutResult, bool bIsConstructorInput)
{
	...
	{
		UE_LOGF(LogMetaSound, Error, "AddGraphInputNode Failed on builder '%ls' when attempting to add '%ls': '%ls' is not a registered DataType", *GetName(), *Name.ToString(), *DataType.ToString());
	}
	else
	{
		...
		if (Node)
		{
			UE_LOGF(LogMetaSound, Warning, "AddGraphInputNode Failed: Input Node already exists with name '%ls'; returning handle to existing node which may or may not match requested DataType '%ls'", *Name.ToString(), *DataType.ToString());
		}
		else
		{
...
FMetaSoundBuilderNodeInputHandle UMetaSoundBuilderBase::AddGraphOutputNode(FName Name, FName DataType, FMetasoundFrontendLiteral DefaultValue, EMetaSoundBuilderResult& OutResult, bool bIsConstructorOutput)
{
	...
	{
		UE_LOGF(LogMetaSound, Error, "AddGraphOutputNode Failed on builder '%ls' when attempting to add '%ls': '%ls' is not a registered DataType", *GetName(), *Name.ToString(), *DataType.ToString());
	}
	else
	{
		...
		if (Node)
		{
			UE_LOGF(LogMetaSound, Warning, "AddGraphOutputNode Failed: Output Node already exists with name '%ls'; returning handle to existing node which may or may not match requested DataType '%ls'", *Name.ToString(), *DataType.ToString());
		}
		else
		{
...
void UMetaSoundBuilderBase::AddGraphPage(FName PageName, bool bDuplicateLastGraph, bool bSetAsBuildGraph, EMetaSoundBuilderResult& OutResult)
{
	if (const UMetaSoundSettings* Settings = GetDefault<UMetaSoundSettings>())
	{
		...
		{
			if (Builder.FindGraphPageIndex(PageSettings->UniqueId) == INDEX_NONE)
			{
...
FDelegateHandle UMetaSoundBuilderBase::AddTransactionListener(TSharedRef<Metasound::Frontend::IDocumentBuilderTransactionListener> BuilderListener)
{
	BuilderListener->OnBuilderReloaded(GetBuilderDelegates());
	return BuilderReloadDelegate.AddSP(BuilderListener, &Metasound::Frontend::IDocumentBuilderTransactionListener::OnBuilderReloaded);
}
	...
	if (NodeClass)
	{
		...
		if (bIsInvalidReference)
		{
			...
			UE_LOGF(LogMetaSound, Warning,
				"Failed to add node of transient asset '%ls' to serialized asset '%ls': "
		...
		else
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Failed to add new node of graph class '%ls': Class access flag '%ls' not set.",
...
FMetaSoundNodeHandle UMetaSoundBuilderBase::AddNodeByClassName(const FMetasoundFrontendClassName& InClassName, EMetaSoundBuilderResult& OutResult, int32 MajorVersion)
{
	...
	if (!ISearchEngine::Get().FindRegisteredClass(InClassName, MajorVersion, ClassInfo))
	{
		UE_LOGF(LogMetaSound, Error, "Failed to add new node by class name '%ls' and major version '%d': Class not found", *InClassName.ToString(), MajorVersion);
		OutResult = EMetaSoundBuilderResult::Failed;
		return NewHandle;
	}
...
void UMetaSoundBuilderBase::BuildAndOverwriteMetaSound(TScriptInterface<IMetaSoundDocumentInterface> ExistingMetaSound, bool bForceUniqueClassName)
{
	...
	if (!MetaSoundPtr)
	{
		UE_LOGF(LogMetaSound, Error, "Failed to build and overwrite MetaSound: No existing MetaSound supplied.");
		return;
	}

	if (MetaSoundPtr->IsAsset())
	{
		UE_LOGF(LogMetaSound, Error, "Failed to build and overwrite MetaSound: Cannot overwrite serialized asset "
...
{
	return Builder.ConformObjectToDocument();
}
...
void UMetaSoundBuilderBase::ConnectNodes(const FMetaSoundBuilderNodeOutputHandle& NodeOutputHandle, const FMetaSoundBuilderNodeInputHandle& NodeInputHandle, EMetaSoundBuilderResult& OutResult)
{
	...
	{
		UE_LOGF(LogMetaSound, Warning, "Builder '%ls' 'ConnectNodes' failed: '%ls'", *GetName(), *LexToString(InvalidEdgeReason));
	}
}
...
TArray<FMetaSoundBuilderNodeInputHandle> UMetaSoundBuilderBase::ConnectNodeOutputsToMatchingGraphInterfaceOutputs(const FMetaSoundNodeHandle& NodeHandle, EMetaSoundBuilderResult& OutResult)
{
	...
	Algo::Transform(NewEdges, ConnectedVertices, [this](const FMetasoundFrontendEdge* NewEdge)
	{
#if DO_CHECK
		{
			...
			checkf(Vertex, TEXT("Edge node output to interface output connection reported success but vertex not found."));
		}
#endif // DO_CHECK

		return FMetaSoundBuilderNodeInputHandle(NewEdge->ToNodeID, NewEdge->ToVertexID);
	});
...
TArray<FMetaSoundBuilderNodeOutputHandle> UMetaSoundBuilderBase::ConnectNodeInputsToMatchingGraphInterfaceInputs(const FMetaSoundNodeHandle& NodeHandle, EMetaSoundBuilderResult& OutResult)
{
	...
	Algo::Transform(NewEdges, ConnectedVertices, [this](const FMetasoundFrontendEdge* NewEdge)
	{
#if DO_CHECK
		{
			...
			checkf(Vertex, TEXT("Edge node input to interface input connection reported success but vertex not found."));
		}
#endif // DO_CHECK

		return FMetaSoundBuilderNodeOutputHandle(NewEdge->FromNodeID, NewEdge->FromVertexID);
	});
...
void UMetaSoundBuilderBase::ConnectNodeOutputToGraphOutput(FName GraphOutputName, const FMetaSoundBuilderNodeOutputHandle& NodeOutputHandle, EMetaSoundBuilderResult& OutResult)
{
	...
	if (const FMetasoundFrontendNode* GraphOutputNode = Builder.FindGraphOutputNode(GraphOutputName))
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Builder '%ls' 'ConnectNodeOutputToGraphOutput' failed: '%ls'", *GetName(), *LexToString(InvalidEdgeReason));
		}
	}
}
...
void UMetaSoundBuilderBase::ConnectNodeInputToGraphInput(FName GraphInputName, const FMetaSoundBuilderNodeInputHandle& NodeInputHandle, EMetaSoundBuilderResult& OutResult)
{
	...
	if (const FMetasoundFrontendNode* GraphInputNode = Builder.FindGraphInputNode(GraphInputName))
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Builder '%ls' 'ConnectNodeInputToGraphInput' failed: '%ls'", *GetName(), *LexToString(InvalidEdgeReason));
		}
	}
}
...
{
	using namespace Metasound::Frontend;
	if (Builder.IsDocumentConfiguredBy<FPresetTemplate>())
	{
		Builder.SetTemplate({ });
		Builder.ConfigureDocument();

#if WITH_EDITORONLY_DATA
		Builder.SetIsGraphEditable(true);
#endif // WITH_EDITORONLY_DATA
...
void UMetaSoundBuilderBase::ConvertToPreset(const TScriptInterface<IMetaSoundDocumentInterface>& ParentNodeClass, EMetaSoundBuilderResult& OutResult)
{
	using namespace Metasound::Frontend;

	const IMetaSoundDocumentInterface* ParentInterface = ParentNodeClass.GetInterface();
	if (!ParentInterface)
	{
		OutResult = EMetaSoundBuilderResult::Failed;
		return;
	...
	if (Cast<UMetaSoundBuilderDocument>(ParentInterface))
	{
		UE_LOGF(LogMetaSound, Warning, "Transient document builders cannot be referenced when converting builder '%ls' to a preset. Build the parent node class as an asset first or use an existing asset instead", *GetName());
		OutResult = EMetaSoundBuilderResult::Failed;
		return;
	}

	// Ensure the parent node class is a matching object type
	const UClass& BaseMetaSoundClass = ParentInterface->GetBaseMetaSoundUClass();
	UObject* ParentObject = ParentNodeClass.GetObject();
	if (!ParentObject || (ParentObject && !ParentObject->IsA(&BaseMetaSoundClass)))
	{
		UE_LOGF(LogMetaSound, Warning, "The referenced node type must match the base MetaSound class when converting builder '%ls' to a preset (ex. source preset must reference another source)", *GetName());
		OutResult = EMetaSoundBuilderResult::Failed;
		return;
	}

	// Ensure the parent node class is registered
	if (FMetasoundAssetBase* ParentMetaSoundAsset = Metasound::IMetasoundUObjectRegistry::Get().GetObjectAsAssetBase(ParentObject))
	{
		Metasound::Frontend::FMetaSoundAssetRegistrationOptions RegOptions;
		RegOptions.PageOrder = UMetaSoundSettings::GetPageOrder();
		ParentMetaSoundAsset->UpdateAndRegisterForExecution(RegOptions);
	}

	TSharedRef<FDocumentModifyDelegates> DocumentDelegates = MakeShared<FDocumentModifyDelegates>(ParentInterface->GetConstDocument());
	InitDelegates(*DocumentDelegates);

	TInstancedStruct<FMetaSoundFrontendPresetTemplate> NewPresetConfig = TInstancedStruct<FMetaSoundFrontendPresetTemplate>::Make();
	NewPresetConfig.GetMutable().Parent = ParentObject;

	Builder.SetTemplate(MoveTemp(NewPresetConfig));
	OutResult = Builder.ConfigureDocument() ? EMetaSoundBuilderResult::Succeeded : EMetaSoundBuilderResult::Failed;
}
...
FMetaSoundBuilderNodeInputHandle UMetaSoundBuilderBase::FindNodeInputByName(const FMetaSoundNodeHandle& NodeHandle, FName InputName, EMetaSoundBuilderResult& OutResult)
{
	if (const FMetasoundFrontendNode* Node = Builder.FindNode(NodeHandle.NodeID))
	{
		...
		UE_LOGF(LogMetaSound, Display, "Builder '%ls' failed to find node input '%ls': Node class '%ls' contains no such input", *GetName(), *InputName.ToString(), *NodeClassName);
	}
	else
	{
		UE_LOGF(LogMetaSound, Display, "Builder '%ls' failed to find node input '%ls': Node with ID '%ls' not found", *GetName(), *InputName.ToString(), *NodeHandle.NodeID.ToString());
	}
...
TArray<FMetaSoundBuilderNodeInputHandle> UMetaSoundBuilderBase::FindNodeInputsByDataType(const FMetaSoundNodeHandle& NodeHandle, EMetaSoundBuilderResult& OutResult, FName DataType)
{
	...
	{
		UE_LOGF(LogMetaSound, Display, "Failed to find node inputs by data type with builder '%ls'. Node of with ID '%ls' not found", *GetName(), *NodeHandle.NodeID.ToString());
		OutResult = EMetaSoundBuilderResult::Failed;
	}
...
FMetaSoundBuilderNodeOutputHandle UMetaSoundBuilderBase::FindNodeOutputByName(const FMetaSoundNodeHandle& NodeHandle, FName OutputName, EMetaSoundBuilderResult& OutResult)
{
	if (const FMetasoundFrontendNode* Node = Builder.FindNode(NodeHandle.NodeID))
	{
		...
		UE_LOGF(LogMetaSound, Display, "Builder '%ls' failed to find node output '%ls': Node class '%ls' contains no such output", *GetName(), *OutputName.ToString(), *NodeClassName);
	}
	else
	{
		UE_LOGF(LogMetaSound, Display, "Builder '%ls' failed to find node output '%ls': Node with ID '%ls' not found", *GetName(), *OutputName.ToString(), *NodeHandle.NodeID.ToString());
	}
...
TArray<FMetaSoundBuilderNodeOutputHandle> UMetaSoundBuilderBase::FindNodeOutputsByDataType(const FMetaSoundNodeHandle& NodeHandle, EMetaSoundBuilderResult& OutResult, FName DataType)
{
	...
	{
		UE_LOGF(LogMetaSound, Display, "Failed to find node outputs by data type with builder '%ls'. Node of with ID '%ls' not found", *GetName(), *NodeHandle.NodeID.ToString());
		OutResult = EMetaSoundBuilderResult::Failed;
	}
...
TArray<FMetaSoundNodeHandle> UMetaSoundBuilderBase::FindInterfaceInputNodes(FName InterfaceName, EMetaSoundBuilderResult& OutResult)
{
	...
	{
		UE_LOGF(LogMetaSound, Display, "'%ls' interface not found on builder '%ls'. No input nodes returned", *InterfaceName.ToString(), *GetName());
		OutResult = EMetaSoundBuilderResult::Failed;
	}
...
FMetaSoundNodeHandle UMetaSoundBuilderBase::FindGraphInputNode(FName InputName, FName& DataTypeName, FMetaSoundBuilderNodeOutputHandle& NodeOutputHandle, EMetaSoundBuilderResult& OutResult)
{
	...
	UE_LOGF(LogMetaSound, Display, "Failed to find graph input by name '%ls' with builder '%ls'", *InputName.ToString(), *GetName());
	OutResult = EMetaSoundBuilderResult::Failed;
	return { };
}
...
FMetaSoundNodeHandle UMetaSoundBuilderBase::FindGraphOutputNode(FName OutputName, FName& DataTypeName, FMetaSoundBuilderNodeInputHandle& NodeInputHandle, EMetaSoundBuilderResult& OutResult)
{
	...
	UE_LOGF(LogMetaSound, Display, "Failed to find graph output by name '%ls' with builder '%ls'", *OutputName.ToString(), *GetName());
	OutResult = EMetaSoundBuilderResult::Failed;
	return { };
}
...
FMetasoundFrontendLiteral UMetaSoundBuilderBase::GetGraphInputDefault(FName InputName, EMetaSoundBuilderResult& OutResult, FName PageName) const
{
	const FGuid* PageID = UMetaSoundSettings::GetPageID(PageName);
	if (const FMetasoundFrontendLiteral* Default = Builder.GetGraphInputDefault(InputName, PageID))
	{
		OutResult = EMetaSoundBuilderResult::Succeeded;
		return *Default;
	}
...
{
	using namespace Metasound::Frontend;
	const FPresetTemplate* PresetTemplate = GetConstBuilder()
		.GetConstTemplateAs<FPresetTemplate>();
	if (PresetTemplate)
	{
		return PresetTemplate->Parent.GetObject();
	}
...
{
	return Builder.IsDocumentConfiguredBy<FMetaSoundFrontendPresetTemplate>();
}
...
void UMetaSoundBuilderBase::ResetGraphInputDefaults(FName Name, EMetaSoundBuilderResult& OutResult)
{
	const bool bReset = Builder.ResetGraphInputDefault(Name);
	OutResult = bReset ? EMetaSoundBuilderResult::Succeeded : EMetaSoundBuilderResult::Failed;
}
...
void UMetaSoundBuilderBase::RemoveGraphInputDefault(FName Name, EMetaSoundBuilderResult& OutResult, FName PageName)
{
	const FGuid* PageID = UMetaSoundSettings::GetPageID(PageName);
	const bool bRemoved = Builder.RemoveGraphInputDefault(Name, PageID ? *PageID : Builder.GetBuildPageID());
	OutResult = bRemoved ? EMetaSoundBuilderResult::Succeeded : EMetaSoundBuilderResult::Failed;
}
...
void UMetaSoundBuilderBase::RemoveGraphVariable(FName Name, EMetaSoundBuilderResult& OutResult)
{
	...
	{
		UE_LOGF(LogMetaSound, Warning, "RemoveGraphVariable Failed: Variable not found with name '%ls'", *Name.ToString());
		OutResult = EMetaSoundBuilderResult::Failed;
	}
}
...
void UMetaSoundBuilderBase::RemoveNode(const FMetaSoundNodeHandle& NodeHandle, EMetaSoundBuilderResult& OutResult, bool bRemoveUnusedDependencies)
{
	...
	if (const FMetasoundFrontendNode* Node = Builder.FindNode(NodeHandle.NodeID))
	{
		if (const FMetasoundFrontendClass* Class = Builder.FindDependency(Node->ClassID))
		{
			...
			// Special case for inputs/outputs so template nodes and interface members are cleaned up
			if (ClassType == EMetasoundFrontendClassType::Input)
			{
				bRemoved = Builder.RemoveGraphInput(Node->Name);
			}
			else if (ClassType == EMetasoundFrontendClassType::Output)
			{
				bRemoved = Builder.RemoveGraphOutput(Node->Name);
			}
			else
			{
				bRemoved = Builder.RemoveNode(NodeHandle.NodeID);
			}
		}
	}

	if (bRemoved && bRemoveUnusedDependencies)
	{
		Builder.RemoveUnusedDependencies();
	}
	OutResult = bRemoved ? EMetaSoundBuilderResult::Succeeded : EMetaSoundBuilderResult::Failed;
...
void UMetaSoundBuilderBase::SetGraphInputDefault(FName InputName, const FMetasoundFrontendLiteral& Literal, EMetaSoundBuilderResult& OutResult, FName PageName)
{
	using namespace Metasound::Frontend;
	const FGuid* PageID = UMetaSoundSettings::GetPageID(PageName);
	const bool bSet = Builder.SetGraphInputDefault(InputName, Literal, PageID);
	OutResult = bSet ? EMetaSoundBuilderResult::Succeeded : EMetaSoundBuilderResult::Failed;
}
...
void UMetaSoundBuilderBase::SetTemplate(TInstancedStruct<FMetaSoundFrontendDocumentTemplate> Configuration)
{
	Builder.SetTemplate(MoveTemp(Configuration));
...
void UMetaSoundBuilderBase::SetTemplateProperties(TFunctionRef<void(TStructView<FMetaSoundFrontendDocumentTemplate>)> SetPropertyFunction)
{
	Builder.SetTemplateProperties(SetPropertyFunction);
...
TInstancedStruct<FMetaSoundFrontendDocumentTemplate> UMetaSoundBuilderBase::GetDocumentTemplate(EMetaSoundBuilderResult& OutResult) const
{
	using namespace Metasound::Frontend;
	...
	if (TConstStructView<FMetaSoundTemplate> DocTemplate = Builder.GetConstDocumentTemplate(); DocTemplate.IsValid())
	{
		const UScriptStruct* ScriptStruct = DocTemplate.GetScriptStruct();
		check(ScriptStruct);

		NewInstance.InitializeAsScriptStruct(ScriptStruct, DocTemplate.GetMemory());
		OutResult = EMetaSoundBuilderResult::Succeeded;
	}
...
TScriptInterface<IMetaSoundDocumentInterface> UMetaSoundBuilderBase::GetMetaSound() const
{
	return TScriptInterface<IMetaSoundDocumentInterface>(&Builder.CastDocumentObjectChecked<UObject>());
}
