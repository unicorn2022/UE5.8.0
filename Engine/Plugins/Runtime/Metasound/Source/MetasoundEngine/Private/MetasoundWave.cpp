...
namespace Metasound
{
	FWaveAsset::FWaveAsset(const TSharedPtr<const Audio::IProxyData>& InInitData)
	{
		if (InInitData.IsValid())
		{
			if (InInitData->CheckTypeCast<FSoundWaveProxy>())
			{
				// todo: remove const cast once deprecated getters can be removed and SoundWaveProxy is const.
				SoundWaveProxy = ConstCastSharedPtr<FSoundWaveProxy>(InInitData->GetAs<const FSoundWaveProxy>().AsShared().ToSharedPtr());

				if (ensureAlways(SoundWaveProxy.IsValid()))
				{
					TSharedRef<const FSoundWaveData> SoundWaveData = SoundWaveProxy->GetSoundWaveDataRef();
						...
						if (SoundWaveData->IsStreaming())
						{
							if (SoundWaveData->GetNumChunks() > 1)
							{
								if (FStreamingManagerCollection* StreamingMgr = IStreamingManager::Get_Concurrent())
								{
									...
									AudioStreamingMgr.RequestChunk(SoundWaveData->AsShared(), 1, [](EAudioChunkLoadResult) {});
								}
							}
						}
					}
