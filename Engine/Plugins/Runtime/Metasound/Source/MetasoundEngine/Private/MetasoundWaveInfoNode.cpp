...
namespace Metasound
{
	...
	{
		TSharedPtr<const FSoundWaveProxy> ProxyPtr = (*WaveAsset).GetWaveProxy();
		TSharedPtr<const FSoundWaveData> DataPtr;
		if (ProxyPtr)
		{
			DataPtr = ProxyPtr->GetSoundWaveDataRef();
		}

		if (SoundWaveData != DataPtr)
		{
			SoundWaveData = DataPtr;

			if (SoundWaveData.IsValid() && (*WaveAsset).IsSoundWaveValid())
			{
				*DurationSeconds = FTime::FromSeconds(SoundWaveData->GetDuration());

				FName AssetName = SoundWaveData->GetFName();
				*NameOutput = AssetName.ToString();

				FString FullPath;
				SoundWaveData->GetPackageName().AppendString(FullPath);
				if (!AssetName.IsNone())
				{
					FullPath += TEXT(".");
