...
namespace Metasound::Engine
{
	FDocumentBuilderRegistry::~FDocumentBuilderRegistry()
	{
		...
		UE_CLOGF(!BuildersToFinish.IsEmpty(), LogMetaSound, Display, "BuilderRegistry is shutting down with the following %i active builder entries. Forcefully shutting down:", BuildersToFinish.Num());
		int32 NumStale = 0;
		for (const TPair<FMetasoundFrontendClassName, TWeakObjectPtr<UMetaSoundBuilderBase>>& Pair : BuildersToFinish)
		{
			...
			if (bIsValid)
			{
				UE_CLOGF(bIsValid, LogMetaSound, Display, "- %ls", *Pair.Value->GetFullName());
		...
		UE_CLOGF(NumStale > 0, LogMetaSound, Display, "BuilderRegistry is shutting down with %i stale entries", NumStale);
	}

#if WITH_EDITORONLY_DATA
	void FDocumentBuilderRegistry::WaitUntilVersioningComplete(UObject& InMetaSoundObject) const
	{
		using namespace Frontend;
		FVersioningManager::Get().WaitUntilVersioningComplete(InMetaSoundObject);
	}
...
// 				{
// 					UE_LOGF(LogMetaSound, Error, "- %ls", *BuilderPtr->GetConstBuilder().CastDocumentObjectChecked<UObject>().GetPathName());
// 				}
// 			}
// 		}
// #endif // !NO_LOGGING
	...
	bool FDocumentBuilderRegistry::CookPages(FName PlatformName, FMetaSoundFrontendDocumentBuilder& Builder) const
	{
			...
			const FString& ItemType)
		{
			...
			for (const FGuid& PageID : PageIdsToResolve)
			{
				...
				if (bRemovedPageItem)
				{
					UE_LOGF(LogMetaSound, Display, "%ls: Removed %ls %ls w/PageID '%ls'",
						*DebugName,
		...
		{ // Strip graphs
			...
			{
				UE_LOGF(LogMetaSound, Display, "Cook removed %i graph page(s) from '%ls'", NumInitGraphs - NumRemainingGraphs, *Builder.GetDebugName());
			}
		}
		...
		{ // Strip default input values
			for (const FMetasoundFrontendClassInput& GraphInput : Document.RootGraph.GetDefaultInterface().Inputs)
			{
				...
				{
					return Builder.StripGraphInputDefault(GraphInput.Name, InPageID);
				};
				...
				{
					UE_LOGF(LogMetaSound, Display, "Cook removed %i default input page value(s) from input '%ls'", NumInitDefaults - NumRemainingDefaults, *GraphInput.Name.ToString());
				}
			}
		}
	...
	TArray<FMetaSoundFrontendDocumentBuilder*> FDocumentBuilderRegistry::FindBuilders(const FMetasoundFrontendClassName& ClassName) const
	{
		TArray<TWeakObjectPtr<UMetaSoundBuilderBase>> Entries;
		...
		TArray<FMetaSoundFrontendDocumentBuilder*> ToReturn;
		Algo::TransformIf(Entries, ToReturn,
			[](const TWeakObjectPtr<UMetaSoundBuilderBase>& WeakBuilder)
			{
				return WeakBuilder.IsValid();
			},
			[](const TWeakObjectPtr<UMetaSoundBuilderBase>& WeakBuilder)
			{
				return &WeakBuilder.Get()->GetBuilder();
			});
	...
	FGuid FDocumentBuilderRegistry::ResolveTargetPageIDInternal(const UMetaSoundSettings& Settings, const TArray<FGuid>& InPageIDsToResolve, const FGuid& TargetPageID, FName PlatformName) const
	{
		...
		if (!bResolved)
		{
			...
			UE_LOGF(LogMetaSound, Error,
				"Failed to resolve PageID for Target '%ls': Setting to arbitrary Page '%ls' (Target likely overridden by page not set as 'CanTarget/Targetable' for the current platform)",
