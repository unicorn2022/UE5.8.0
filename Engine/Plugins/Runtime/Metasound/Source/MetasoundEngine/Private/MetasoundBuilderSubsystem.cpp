...
void UMetaSoundSourceBuilder::Audition(UObject* Parent, UAudioComponent* AudioComponent, FOnCreateAuditionGeneratorHandleDelegate CreateGenerator, bool bLiveUpdatesEnabled)
{
	...
	if (!AudioComponent)
	{
		UE_LOGF(LogMetaSound, Error, "Failed to audition MetaSoundBuilder '%ls': No AudioComponent supplied", *GetFullName());
		return;
	}
...
void UMetaSoundSourceBuilder::InitDelegates(Metasound::Frontend::FDocumentModifyDelegates& OutDocumentDelegates)
{
	...
	{
		OutDocumentDelegates.OnDocumentTemplateChanged.AddUObject(this, &UMetaSoundSourceBuilder::OnDocumentTemplateChanged);
		...
		OutDocumentDelegates.InterfaceDelegates.OnInputDataTypeChanged.AddUObject(this, &UMetaSoundSourceBuilder::OnInputDataTypeChanged);
		OutDocumentDelegates.InterfaceDelegates.OnInputDefaultChanged.AddUObject(this, &UMetaSoundSourceBuilder::OnInputDefaultChanged);
		OutDocumentDelegates.InterfaceDelegates.OnInputNameChanged.AddUObject(this, &UMetaSoundSourceBuilder::OnInputNameChanged);
...
void UMetaSoundSourceBuilder::OnDocumentTemplateChanged(const Metasound::Frontend::FDocumentTemplateChangedArgs&) const
{
	using namespace Metasound::Frontend;
		...
		for (uint64 AudioComponentID : LiveComponentIDs)
		{
			if (UAudioComponent* AudioComponent = UAudioComponent::GetAudioComponentFromID(AudioComponentID))
			{
				if (FAudioDevice* AudioDevice = AudioComponent->GetAudioDevice())
				{
					...
					{
						if (FMetaSoundParameterTransmitter* Transmitter = static_cast<FMetaSoundParameterTransmitter*>(ActiveSound.GetTransmitter()))
						{
							Transmitter->AddAvailableParameter(NewInputName);
						}
					});
				}
			}
...
void UMetaSoundSourceBuilder::OnInputDataTypeChanged(int32 InputIndex)
{
	OnRemovingInput(InputIndex);
	OnInputAdded(InputIndex);
}

void UMetaSoundSourceBuilder::OnInputDefaultChanged(const Metasound::Frontend::FDocumentArrayPagedInputArgs& Args) const
{
	using namespace Metasound::DynamicGraph;
	using namespace Metasound::Frontend;
	...
	const FMetasoundFrontendDocument& Doc = Builder.GetConstDocumentChecked();
	const FMetasoundFrontendGraphClass& GraphClass = Doc.RootGraph;
	const FMetasoundFrontendClassInput& InputWithNewDefault = GraphClass.GetDefaultInterface().Inputs[Args.Index];

	UMetaSoundSource& Source = GetMetaSoundSource();
	Source.RuntimeInputData.InputMap.Remove(InputWithNewDefault.Name);

	if (const FMetasoundFrontendClassInput* Input = GetConstBuilder().FindGraphInput(InputWithNewDefault.Name))
	{
		constexpr bool bCreateUObjectProxies = true;
		Source.RuntimeInputData.InputMap.Add(Input->Name, UMetaSoundSource::CreateRuntimeInput(IDataTypeRegistry::Get(), *Input, bCreateUObjectProxies, UMetaSoundSettings::GetPageOrder()));
	}

	// default value changes to added/removed inputs with dynamic graphs is currently unsupported
}

void UMetaSoundSourceBuilder::OnInputNameChanged(FName OldName, FName NewName)
{
	using namespace Metasound::DynamicGraph;
	using namespace Metasound::Frontend;

	UMetaSoundSource& Source = GetMetaSoundSource();
	Source.RuntimeInputData.InputMap.Remove(OldName);

	if (const FMetasoundFrontendClassInput* Input = GetConstBuilder().FindGraphInput(NewName))
	{
		constexpr bool bCreateUObjectProxies = true;
		Source.RuntimeInputData.InputMap.Add(Input->Name, UMetaSoundSource::CreateRuntimeInput(IDataTypeRegistry::Get(), *Input, bCreateUObjectProxies, UMetaSoundSettings::GetPageOrder()));

		ExecuteAuditionableTransaction([this, OldName, &Input](FDynamicOperatorTransactor& Transactor)
		{
			using namespace Metasound;
			using namespace Metasound::Frontend;

			for (const uint64& AudioComponentID : LiveComponentIDs)
			{
				if (UAudioComponent* AudioComponent = UAudioComponent::GetAudioComponentFromID(AudioComponentID))
				{
					if (FAudioDevice* AudioDevice = AudioComponent->GetAudioDevice())
					{
						AudioDevice->SendCommandToActiveSounds(AudioComponentID, [OldName, NewInputName = Input->Name](FActiveSound& ActiveSound)
						{
							if (FMetaSoundParameterTransmitter* Transmitter = static_cast<FMetaSoundParameterTransmitter*>(ActiveSound.GetTransmitter()))
							{
								Transmitter->RemoveAvailableParameter(OldName);
								Transmitter->AddAvailableParameter(NewInputName);
							}
						});
					}
			...
			Transactor.RemoveInputDataDestination(OldName);
			const FLiteral NewInputLiteral = Input->FindConstDefaultChecked(Frontend::DefaultPageID).ToLiteral(Input->TypeName);
			Transactor.AddInputDataDestination(Input->NodeID, Input->Name, NewInputLiteral, &FMetaSoundFrontendDocumentBuilder::CreateDataReference);
			return true;
		});
	}
}
...
void UMetaSoundSourceBuilder::OnNodeAdded(int32 NodeIndex) const
{
	...
	ExecuteAuditionableTransaction([this, NodeIndex](FDynamicOperatorTransactor& Transactor)
	{
		...
		if (!NewNode.IsValid())
		{
			UE_LOGF(LogMetaSound, Error, "Builder '%ls' failed to create and forward added node '%ls' to live update transactor.", *Builder.GetDebugName(), *AddedNode.Name.ToString());
			return false;
		}
	...
	ExecuteAuditionableTransaction([this, NodeIndex](FDynamicOperatorTransactor& Transactor)
	{
		...
		if (!ConfiguredNode.IsValid())
		{
			UE_LOGF(LogMetaSound, Error,
				"Builder '%ls' failed to create and forward reconfigured node '%ls' to live update transactor.",
			...
			if (UAudioComponent* AudioComponent = UAudioComponent::GetAudioComponentFromID(AudioComponentID))
			{
				if (FAudioDevice* AudioDevice = AudioComponent->GetAudioDevice())
				{
					AudioDevice->SendCommandToActiveSounds(AudioComponentID, [InputRemoved = InputBeingRemoved.Name](FActiveSound& ActiveSound)
					{
						if (FMetaSoundParameterTransmitter* Transmitter = static_cast<FMetaSoundParameterTransmitter*>(ActiveSound.GetTransmitter()))
						{
							Transmitter->RemoveAvailableParameter(InputRemoved);
						}
					});
				}
			}
...
void UMetaSoundSourceBuilder::SetFormat(EMetaSoundOutputAudioFormat OutputFormat, EMetaSoundBuilderResult& OutResult)
{
	...
	// Clear template since interface data is being altered
	Builder.SetTemplate({ });
	...
	bool bIsOneShot)
{
	...
	if (OutResult == EMetaSoundBuilderResult::Succeeded)
	{
		...
		if (OutResult == EMetaSoundBuilderResult::Succeeded)
		{
			Algo::Transform(AudioOutputNodes, AudioOutNodeInputs, [&NewBuilder, &BuilderName](const FMetaSoundNodeHandle& AudioOutputNode) -> FMetaSoundBuilderNodeInputHandle
			{
				...
				UE_LOGF(LogMetaSound, Error, "Builder '%ls' Creation Error: Failed to find expected audio output node input vertex. Returned vertices set may be incomplete.", *BuilderName.ToString());
				return { };
			});
		}
		else
		{
			UE_LOGF(LogMetaSound, Error, "Builder '%ls' Creation Error: Failed to find expected audio output format and/or associated output nodes.", *BuilderName.ToString());
			return nullptr;
		}
	}
	else
	{
		UE_LOGF(LogMetaSound, Error, "Builder '%ls' Creation Error: Failed to set output format when initializing.", *BuilderName.ToString());
		return nullptr;
	}

	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Builder '%ls' Creation Error: Failed to add required interface '%ls' when attempting to create MetaSound Source Builder", *BuilderName.ToString(), * SourceInterface::GetVersion().ToString());
			return nullptr;
		}
		...
		if (OutResult == EMetaSoundBuilderResult::Failed)
		{
			UE_LOGF(LogMetaSound, Error, "Builder '%ls' Creation Error: Failed to find output vertex for 'OnPlay' input node when attempting to create MetaSound Source Builder", *BuilderName.ToString());
			return nullptr;
		}
	...
	if (bIsOneShot)
	{
		...
		if (OutResult == EMetaSoundBuilderResult::Failed)
		{
			UE_LOGF(LogMetaSound, Error, "Builder '%ls' Creation Error: Failed to add '%ls' interface; interface definition may not be registered.", *BuilderName.ToString(), *SourceOneShotInterface::GetVersion().ToString());
		}

		TArray<FMetaSoundBuilderNodeInputHandle> Inputs = NewBuilder.FindNodeInputs(OnFinishedNode, OutResult);
		if (OutResult == EMetaSoundBuilderResult::Failed)
		{
			UE_LOGF(LogMetaSound, Error, "Builder '%ls' Creation Error: Failed to find input vertex for 'OnFinished' output node when attempting to create MetaSound Source Builder", *BuilderName.ToString());
			return nullptr;
		}
...
UMetaSoundBuilderBase* UMetaSoundBuilderSubsystem::FindParentBuilderOfPreset(const TScriptInterface<const IMetaSoundDocumentInterface> InMetaSoundPreset, const bool bFollowPresetChain)
{
	...
	{
		UE_LOGF(LogMetaSound, Error, "FindParentBuilderOfPreset called with object that is not a MetaSound asset");
		return nullptr;
	}
	...
	if (!Builder)
	{
		UE_LOGF(LogMetaSound, Error, "Could not find builder for MetaSound '%ls'.", *GetNameSafe(InMetaSoundPreset.GetObject()));
		return nullptr;
	}

	const FMetaSoundFrontendPresetTemplate* PresetTemplate = Builder->GetConstBuilder().GetConstTemplateAs<FMetaSoundFrontendPresetTemplate>();
	if (!PresetTemplate)
	{
		UE_LOGF(LogMetaSound, Error, "MetaSound '%ls' is not a preset.", *GetNameSafe(InMetaSoundPreset.GetObject()));
		return nullptr;
	}

	TScriptInterface<IMetaSoundDocumentInterface> ParentAsset = PresetTemplate->Parent;
	if (!ParentAsset)
	{
		UE_LOGF(LogMetaSound, Error, "MetaSound preset '%ls' contains null reference.", *GetNameSafe(InMetaSoundPreset.GetObject()));
		return nullptr;
	}
	...
	while (ParentBuilder)
	{
		...
		CheckedAssets.Add(ParentAsset.GetObject(), &bIsCircular);
		if (bIsCircular)
		{
			UE_LOGF(LogMetaSound, Error, "MetaSound preset '%ls' contains circular reference.", *GetNameSafe(ParentAsset.GetObject()));
			return nullptr;
		}

		PresetTemplate = ParentBuilder->GetConstBuilder().GetConstTemplateAs<FMetaSoundFrontendPresetTemplate>();
		if (!PresetTemplate)
		{
			break;
		}

		ParentAsset = PresetTemplate->Parent;
		if (!ParentAsset)
		{
			UE_LOGF(LogMetaSound, Warning, "MetaSound preset '%ls' contains null reference.", *GetNameSafe(ParentAsset.GetObject()));
			return nullptr;
		}
...
bool UMetaSoundBuilderSubsystem::IsInterfaceRegistered(FName InInterfaceName) const
{
	...
	return ISearchEngine::Get().FindHighestInterfaceVersion(InInterfaceName, InterfaceVersion);
}
...
bool UMetaSoundBuilderSubsystem::SetTargetPage(FName PageName)
{
	...
	if (UMetaSoundSettings* Settings = GetMutableDefault<UMetaSoundSettings>())
	{
		...
		if (bTargetChanged)
		{
			IMetaSoundAssetManager::GetChecked().ReregisterLoadedAssets();
		}
