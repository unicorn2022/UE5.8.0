...
namespace Metasound::Engine
{
	namespace OutputFormatPrivate
	{
		...
		Audio::FParameterInterface::FOutput GetChannelAgnosticOutput(const FName& InVertexName)
		{
			return
			{
				LOCTEXT("OutputFormatSurroundInterface_GeneratedChannelAgnosticDisplayName", "Out Channel Agnostic"),
				LOCTEXT("OutputFormatSurroundInterface_GeneratedChannelAgnosticDescription", "The resulting channel agnostic output audio."),
...
#define AUDIO_PARAMETER_INTERFACE_NAMESPACE "UE.OutputFormat.Cat"
	namespace OutputFormatChannelAgnosticInterface
	{
		const FMetasoundFrontendVersion& GetVersion()
		{
			static const FMetasoundFrontendVersion Version = { AUDIO_PARAMETER_INTERFACE_NAMESPACE, { 1, 0 } };
		...
		namespace Outputs
		{
			const FName CatOut = AUDIO_PARAMETER_INTERFACE_MEMBER_DEFINE("Audio:0");
		}
		...
		Audio::FParameterInterfacePtr CreateInterface()
		{
			...
			{
				FInterface()
					: FParameterInterface(OutputFormatChannelAgnosticInterface::GetVersion().Name, OutputFormatChannelAgnosticInterface::GetVersion().Number.ToInterfaceVersion())
				{
					using namespace OutputFormatPrivate;
					...
					Outputs =
					{
						GetChannelAgnosticOutput(Outputs::CatOut),
					};
				}
			};
...
#undef AUDIO_PARAMETER_INTERFACE_NAMESPACE
	...
	const FOutputAudioFormatInfoMap& GetOutputAudioFormatInfo()
	{
		auto CreateFormatInfoMap = []()
		{
			...
			// Only include CAT output format when MetasoundExperimental plugin is enabled.
			if (IPluginManager::Get().FindEnabledPlugin(TEXT("MetasoundExperimental")))
			{
				Map.Add(EMetaSoundOutputAudioFormat::ChannelAgnostic,
				{
					OutputFormatChannelAgnosticInterface::GetVersion(),
					{
						OutputFormatChannelAgnosticInterface::Outputs::CatOut,
					}
				});
			}
