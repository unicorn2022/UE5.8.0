...
namespace Metasound::OperatorCachePrivate
{
	...
	TOptional<FGeneratorInitParams> CreateInitParams(UMetaSoundSource& InMetaSound, const FSoundGeneratorInitParams& InParams)
	{
		...
		// Resolve format and channel count via the centralized method on UMetaSoundSource.
		// Empty default parameters — precaching has no runtime parameter overrides.
		const UMetaSoundSource::FResolvedSourceFormat ResolvedFormat = InMetaSound.ResolveSourceFormatAndNumChannels(InParams, {}, Environment);
...
void UMetaSoundCacheSubsystem::PrecacheMetaSoundInternal(UMetaSoundSource* InMetaSound, int32 InNumInstances, bool bTouchExisting)
{
	...
	{
		UE_LOGF(LogMetaSound, Log, "Ignoring PrecacheMetaSound request since au.MetaSound.OperatorCache.EnablePrecache is false.");
		return;
	}
	...
	if (!InMetaSound)
	{
		UE_LOGF(LogMetaSound, Error, "PrecacheMetaSound called without being provided a MetaSound, ignoring request");
		return;
	}

	if (InNumInstances < 1)
	{
		UE_LOGF(LogMetaSound, Error, "PrecacheMetaSound called with invalid NumInstances %i, ignoring request", InNumInstances);
		return;
	}
	...
	{
		UE_LOGF(LogMetaSound, Warning, "Failed to retrieve MetaSoundClassName when precaching operator for MetaSound '%ls'", *InMetaSound->GetPathName());
		return;
	}
...
void UMetaSoundCacheSubsystem::RemoveCachedOperatorsForMetaSound(UMetaSoundSource* InMetaSound)
{
	...
	if (!InMetaSound)
	{
		UE_LOGF(LogMetaSound, Warning, "Remove Cached Operators called without being provided a MetaSound, ignoring request");
		return;
	}
	...
	if (TSharedPtr<FOperatorPool> OperatorPool = Module->GetOperatorPool())
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Failed to retrieve MetaSoundClassName when removing cached operator for MetaSound '%ls'", *InMetaSound->GetPathName());
		}
	}
}
