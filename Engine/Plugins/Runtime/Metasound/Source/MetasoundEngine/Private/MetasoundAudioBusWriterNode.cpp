...
namespace Metasound
{
	...
	public:
		...
		static TUniquePtr<IOperator> CreateOperator(const FBuildOperatorParams& InParams, FBuildResults& OutResults)
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Audio bus writer node requires audio device ID '%ls', audio mixer num output frames '%ls' and transmitter id '%ls' environment variables"
		...
		void CreatePatchInput()
		{
			...
			if (AudioBusProxy.IsValid())
			{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Using a audio bus writer node is inefficient if the MetaSound sample rate %f does not match the AudioMixer sample rate %f. Please update MetaSound SampleRate to match the AudioMixer's SampleRate", SampleRate, AudioMixerSampleRate);
		...
		void Reset(const IOperator::FResetParams& InParams)
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Audio bus writer node may not render correctly without the audio mixer sample rate '%ls' environment variable", *SourceInterface::Environment::AudioMixerSampleRate.ToString());
		...
		void Execute()
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Underrun detected in audio bus writer node.");
