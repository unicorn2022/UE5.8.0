...
public:
	...
	UE_API FDelegateHandle AddTransactionListener(TSharedRef<Metasound::Frontend::IDocumentBuilderTransactionListener> BuilderListener);
	...
	// Returns copy of the builder's template. Returns success if the builder has a template set, fails if not.
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (ExpandEnumAsExecs = "OutResult"))
	UE_API TInstancedStruct<FMetaSoundFrontendDocumentTemplate> GetDocumentTemplate(EMetaSoundBuilderResult& OutResult) const;

	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (DeprecatedFunction, DeprecationMessage = "5.8 - Use GetDocumentTemplate instead", ExpandEnumAsExecs = "OutResult"))
	TInstancedStruct<FMetaSoundFrontendDocumentTemplate> GetConfiguration(EMetaSoundBuilderResult& OutResult) const { return GetDocumentTemplate(OutResult); }
	...
	//  @param PageName	If the page name is not specified or not found, the builder's current build page will be used (which can be set in the MetaSound editor).
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (ExpandEnumAsExecs = "OutResult", AdvancedDisplay = "PageName"))
	UE_API UPARAM(DisplayName = "Literal Value") FMetasoundFrontendLiteral GetGraphInputDefault(FName InputName, EMetaSoundBuilderResult& OutResult, FName PageName = FName()) const;
	...
	// Returns the interface to the MetaSound the builder is acting on.
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder")
	UE_API UPARAM(DisplayName = "MetaSound") TScriptInterface<IMetaSoundDocumentInterface> GetMetaSound() const;
	...
	// Convert this builder to a MetaSound preset using the given MetaSoundDocumentInterface as its parent.
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (ExpandEnumAsExecs = "OutResult"))
	UE_API void ConvertToPreset(UPARAM(DisplayName = "Parent Node Class") const TScriptInterface<IMetaSoundDocumentInterface>& ReferencedNodeClass, EMetaSoundBuilderResult& OutResult);

	// Removes input literal defaults for all pages if set, reverting the value to the class defaults
	// Returns success if values reset, false if not (i.e. wasn't set to begin with).
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (ExpandEnumAsExecs = "OutResult"))
	UE_API void ResetGraphInputDefaults(FName Name, EMetaSoundBuilderResult& OutResult);
	...
	// Removes graph input literal default if set, reverting the value to the class default
	// @param PageName If the page name is not specified or not found, the builder's current build page will be used (which can be set in the MetaSound editor).
	// Returns success if value was removed, false if not removed (i.e. wasn't set to begin with).
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (ExpandEnumAsExecs = "OutResult", AdvancedDisplay = "PageName"))
	UE_API void RemoveGraphInputDefault(FName Name, EMetaSoundBuilderResult& OutResult, FName PageName = FName());
	...
	// Sets the given builder's template and immediately configures the MetaSound.
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder")
	UE_API void SetTemplate(TInstancedStruct<FMetaSoundFrontendDocumentTemplate> Configuration);

	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (DeprecatedFunction, DeprecationMessage = "5.8 - Use SetTemplate instead"))
	void SetConfiguration(TInstancedStruct<FMetaSoundFrontendDocumentTemplate> Configuration) { SetTemplate(MoveTemp(Configuration)); }
	...
	UE_API void SetTemplateProperties(TFunctionRef<void(TStructView<FMetaSoundFrontendDocumentTemplate>)> SetPropertiesFunction);

	UE_DEPRECATED(5.8, "Use SetTemplateProperties() instead")
	void SetConfigurationProperties(TFunctionRef<void(TStructView<FMetaSoundFrontendDocumentTemplate>)> SetPropertiesFunction) { SetTemplateProperties(SetPropertiesFunction); }

	template <typename TConfig>
	bool SetTemplateProperties(TFunctionRef<void(TConfig&)> SetPropertyFunction)
	{
		const bool bConfigUpdated = GetBuilder().SetTemplateProperties(SetPropertyFunction);
		if (bConfigUpdated)
		{
			PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
			GetBuilder().ConfigureDocument();
			PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
		}
		return bConfigUpdated;
	}
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Audio|MetaSound|Builder", meta = (ExpandEnumAsExecs = "OutResult", AdvancedDisplay = "PageName"))
	UE_API void SetGraphInputDefault(FName InputName, const FMetasoundFrontendLiteral& Literal, EMetaSoundBuilderResult& OutResult, FName PageName = FName());	
...
protected:
	...
	UClassType& BuildInternal(UObject* Parent, const FMetaSoundBuilderOptions& BuilderOptions) const
	{
		...
		UE_LOGF(LogMetaSound, VeryVerbose, "MetaSound '%ls' built from '%ls'", *BuilderOptions.Name.ToString(), *GetFullName());
		return *MetaSound;
	}
