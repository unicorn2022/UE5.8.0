...
public:
	/** Result of ResolveSourceFormatAndNumChannels. */
	struct FResolvedSourceFormat
	{
		/** The resolved source format name (NAME_None for discrete/non-CAT sources). */
		FName SourceFormatName;

		/** The resolved channel count, or INDEX_NONE on failure. */
		int32 NumChannels = INDEX_NONE;
	};

	// FRuntimeInput represents an input to a MetaSound which can be manipulated.
	struct UE_INTERNAL FRuntimeInput
	{
		...
		// True if the data / default value deviates from the default page (graph)
		bool bIsPaged = false;
...
#if WITH_EDITORONLY_DATA
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "Soft reference cache is no longer supported"))
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Metasound, meta = (DisplayName = "Source Format (exp)", EditConditionHides, EditCondition = "OutputFormat == EMetaSoundOutputAudioFormat::ChannelAgnostic"))
	EMetasoundChannelAgnosticSourceFormatChooser ChannelAgnosticFormatChooser = EMetasoundChannelAgnosticSourceFormatChooser::Auto;

	// The custom channel-agnostic format name (e.g. "Cat:Discrete:Stereo"). Only used when ChannelAgnosticFormatChooser == Custom.
	// Restricted to discrete (mixer-compatible) formats only — soundfield/ambisonics are not supported by AudioMixer.
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Metasound, meta = (DisplayName = "Custom Format Name", GetOptions="MetasoundCatCastingOptionsHelper.GetCastingOptions_AudioMixerOnly", EditConditionHides, EditCondition = "OutputFormat == EMetaSoundOutputAudioFormat::ChannelAgnostic && ChannelAgnosticFormatChooser == EMetasoundChannelAgnosticSourceFormatChooser::Custom"))
	FName ChannelAgnosticCustomFormat;
...
public:
	...
	UE_API virtual TArray<FMetasoundAssetBase*> GetReferencedAssets() const override;

#if WITH_EDITORONLY_DATA
	UE_DEPRECATED(5.8, "This is deprecated behavior and will become UE_INTERNAL API in a future release for back compat.")
	...
	PRAGMA_DISABLE_INTERNAL_WARNINGS
	UE_INTERNAL UE_API TArray<FRuntimeInput> GatherVerseParameters() const;
	PRAGMA_ENABLE_INTERNAL_WARNINGS
...
protected:
	...
	UE_API Metasound::Frontend::FDocumentAccessPtr GetDocumentAccessPtr(bool bReloadBuilder = true) override;
...
public:
	...
	/**
	 * Determines the "Auto" Channel Agnostic Format for this Source.
	 * Consults the document layer and makes a best guess at the appropriate format
	 * from wave proxy channel counts. Can be called before graph evaluation.
	 * @param InParams - Audio parameters to inspect for wave proxies. Pass empty array to use defaults.
	 * @return Shared pointer to the resolved channel type family, or nullptr if no format could be determined.
	 */
	UE_INTERNAL
	UE_API TSharedPtr<const Audio::FChannelTypeFamily> CalculateAutoSourceFormat(const TArray<FAudioParameter>& InParams) const;

	/**
	 * Resolves SourceFormatName and NumChannels based on the current OutputFormat.
	 * For ChannelAgnostic sources, determines format via Custom > Auto 
	 * priority and populates the Environment. For discrete sources, derives
	 * NumChannels from GetOutputAudioChannelOrder().
	 *
	 * Does NOT modify USoundWave::NumChannels -- the caller is responsible for
	 * assigning the returned value where appropriate.
	 *
	 * @param InParams - Sound generator init params (used for fallback channel count).
	 * @param InDefaultParameters - Audio parameters for auto-format detection (pass empty for precache).
	 * @param InOutEnvironment - Environment to populate with SourceFormatName.
	 * @return FResolvedSourceFormat containing the format name and channel count.
	 */
	UE_INTERNAL
	UE_API FResolvedSourceFormat ResolveSourceFormatAndNumChannels(const FSoundGeneratorInitParams& InParams, const TArray<FAudioParameter>& InDefaultParameters, Metasound::FMetasoundEnvironment& InOutEnvironment);
...
private:
	...
	bool bHasWarnedAboutStartTime = false;
