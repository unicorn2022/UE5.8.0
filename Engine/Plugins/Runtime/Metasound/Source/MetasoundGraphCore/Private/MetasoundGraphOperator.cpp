...
namespace Metasound
{
		...
		bool IsSupportedVertexData(const InterfaceDataType& InCurrentData, const InterfaceDataType& InNewData)
		{
			...
			while (CurrentIter && NewIter)
			{
				if (NewIter->VertexName == CurrentIter->VertexName)
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Cannot bind to FGraphOperator because vertex %ls has mismatched data", *(NewIter->VertexName.ToString()));
						return false;
					}
					else
					{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Cannot bind to FGraphOperator because vertex %ls does not exist in current vertex data", *(NewIter->VertexName.ToString()));
					return false;
				}
				else 
				{
	...
	void FGraphOperator::BindInputs(FInputVertexInterfaceData& InInputVertexData)
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "FGraphOperator does not support rebinding with new data");
		}
