...
namespace Metasound
{
	...
	{
		// FGraph is a graph boundary, not a configurable node — it must not
		// have sub-interfaces or variants on its DefaultInterface.
		ensure(!Metadata.DefaultInterface.ContainsSubInterfaces() && !Metadata.DefaultInterface.ContainsVariants());
	}
	...
	{
		GraphInterface.GetInputInterface().Add(InDestination.Vertex);
		InputDestinations.Add(MakeDestinationDataVertexKey(InDestination), InDestination);
		Metadata.DefaultInterface = GraphInterface;
	}

	bool FGraph::RemoveInputDataDestination(const FVertexName& InVertexName)
	{
		bool bSuccessRemoveFromInterface = GraphInterface.GetInputInterface().Remove(InVertexName);
		Metadata.DefaultInterface = GraphInterface;
	...
	{
		GraphInterface.GetOutputInterface().Add(InSource.Vertex);
	...
	{
		bool bSuccessRemoveFromInterface = GraphInterface.GetOutputInterface().Remove(InVertexName);
		Metadata.DefaultInterface = GraphInterface;
	...
	bool FGraph::AddDataEdge(const INode& FromNode, const FVertexName& FromKey, const INode& ToNode, const FVertexName& ToKey)
	{
		...
		Edges.Add(FDataEdge{FOutputDataSource(FromNode, FromVertex), FInputDataDestination(ToNode, ToVertex)});
	...
	void FGraph::SetNodeDefaultInput(const FGuid& InNodeID, const FVertexName& InVertexName, const FLiteral& InLiteral)
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Could not set input vertex literal on vertex %ls. No node with ID %ls found in graph %ls", *InVertexName.ToString(), *InNodeID.ToString(), *GetInstanceName().ToString())
		}
	}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void FGraph::SetDefaultInput(const FVertexName& InVertexName, const FLiteral& InLiteral)
	{
		...
		if (FInputDataVertex* InputVertex = GraphInterface.GetInputInterface().Find(InVertexName))
		{
