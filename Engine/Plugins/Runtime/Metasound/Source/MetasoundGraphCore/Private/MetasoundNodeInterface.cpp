...
namespace Metasound
{
	...
	FNodeClassMetadata::FNodeClassMetadata(
		...
		FClassInterface InDefaultInterface,
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void INodeBase::SetDefaultInput(const FVertexName& InVertexName, const FLiteral& InLiteral)
	{
		...
		if (!bDidWarn)
		{
			UE_LOGF(LogMetaSound, Warning, "Ignoring input default for vertex %ls. Please implement INodeInterface::SetDefaultInput(...) for the class representing node %ls. This method will become pure virtual in future releases. Define UE_METASOUND_PURE_VIRTUAL_SET_DEFAULT_INPUT in order to build with this method as a pure virtual on the interface.", *InVertexName.ToString(), *GetInstanceName().ToString());
			bDidWarn = true;
		}
	}
#endif
	...
	TSharedPtr<const IOperatorData> INodeBase::GetOperatorData() const
	{
		...
		if (!bDidWarn)
		{
			UE_LOGF(LogMetaSound, Warning, "Please implement INodeInterface::GetOperatorData(...) for the class representing node %ls. This method will become pure virtual in future releases. Define UE_METASOUND_PURE_VIRTUAL_GET_OPERATOR_DATA in order to build with this method as a pure virtual on the interface.", *GetInstanceName().ToString());
			bDidWarn = true;
		}
		return {};
	}
