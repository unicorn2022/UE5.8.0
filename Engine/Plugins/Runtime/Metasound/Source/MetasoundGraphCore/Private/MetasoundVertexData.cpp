...
namespace Metasound
{
	namespace VertexDataPrivate
	{
		...
		{
			check(IsCastable(InAnyDataReference.GetDataTypeName(),Vertex.DataTypeName));
		...
		{
			check(IsCastable(InBinding.GetVertex().DataTypeName,Vertex.DataTypeName));
		...
		void SetVertex(bool bIsVertexInterfaceFrozen, TArray<BindingType>& InBindings, const FVertexName& InVertexName, FAnyDataReference&& InDataReference)
		{
			...
			{
				if (IsCastable(
					InDataReference.GetDataTypeName(),						// Incoming ref. (dynamic reference).
					Binding->GetVertex().DataTypeName						// Type of this pin. (must be derived from this or equal).
					))
				{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Failed bind vertex with name '%ls'. Supplied data type (%ls) does not match vertex data type (%ls)", *InVertexName.ToString(), *InDataReference.GetDataTypeName().ToString(), *Binding->GetVertex().DataTypeName.ToString());
				}
			}
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Failed find vertex with name '%ls'. Failed to bind vertex data", *InVertexName.ToString());
			}
		}
		...
		void Bind(bool bIsVertexInterfaceFrozen, TArray<BindingType>& InThisBindings, TArray<BindingType>& InOtherBindings)
		{
			for (BindingType& OtherBinding : InOtherBindings)
			{
				...
				if (BindingType* ThisBinding = Find(InThisBindings, OtherVertexName))
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Failed bind vertex with name '%ls'. Supplied data type (%ls) does not match vertex data type (%ls)", *OtherVertexName.ToString(), *OtherDataTypeName.ToString(), *ThisBinding->GetVertex().DataTypeName.ToString());
					}
				}
				else if (!bIsVertexInterfaceFrozen)
				{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Failed find vertex with name '%ls'. Failed to bind vertex data", *OtherVertexName.ToString());
				}
			}
		}
		...
		void ForEachSubInterfaceBindingImpl(const VertexPrivate::FSubInterfaceLayout& InSubInterfaceLayout, const FVertexName& InVertexName, uint32 InExpectedNumSubInterfaceInstances, TFunctionRef<void (uint32, BindingType&)> InApplyFunc, TArrayView<BindingType> InOutBindings)
		{
			...
			{
				UE_LOGF(LogMetaSound, Verbose, "Number of instances of sub interface '%ls' is %d, expected %d. This may result in unbound vertex data.", *InSubInterfaceLayout.SubInterfaceName.ToString(), InSubInterfaceLayout.Instances.Num(), InExpectedNumSubInterfaceInstances);
			}
			...
			if (InSubInterfaceLayout.Instances.Num() > 0)
			{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Failed to find vertex with name '%ls' in sub interface with name '%ls'.", *InVertexName.ToString(), *InSubInterfaceLayout.SubInterfaceName.ToString());
				}
			}
		}
	}
	...
	void FInputVertexInterfaceData::AddVertex(const FInputDataVertex& InVertex)
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Cannot add vertex with name '%ls'. Vertex interface is frozen", *InVertex.VertexName.ToString());
		}
		else
		{
	...
	void FInputVertexInterfaceData::RemoveVertex(const FVertexName& InVertexName)
	{
		if (IsVertexInterfaceFrozen())
		{
			UE_LOGF(LogMetaSound, Warning, "Cannot remove vertex with name '%ls'. Vertex interface is frozen", *InVertexName.ToString());
		}
		else
		{
	...
	void FInputVertexInterfaceData::SetDefaultLiteral(const FVertexName& InVertexName, const FLiteral& InLiteral)
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Cannot set default literal. Failed to find input vertex with name '%ls'.", *InVertexName.ToString());
		}
	}
	...
	void FInputVertexInterfaceData::Apply(const FVertexName& InVertexName, TFunctionRef<FInputBinding ()> InCreateFunc, TFunctionRef<void (FInputBinding&)> InBindFunc)
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Failed to find input vertex with name '%ls'.", *InVertexName.ToString());
		}
		else
		{
	...
	void FInputVertexInterfaceData::ForEachSubInterfaceBinding(const FName& InSubInterfaceName, const FVertexName& InVertexName, uint32 InExpectedNumSubInterfaceInstances, TFunctionRef<void (uint32, FInputBinding&)> InApplyFunc)
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Failed to find sub interface with name '%ls'.", *InSubInterfaceName.ToString());
		}
	}
	...
	void FOutputVertexInterfaceData::Apply(const FVertexName& InVertexName, TFunctionRef<FOutputBinding ()> InCreateFunc, TFunctionRef<void (FOutputBinding&)> InBindFunc)
	{
		...
		{
			UE_LOGF(LogMetaSound, Warning, "Failed to find output vertex with name '%ls'.", *InVertexName.ToString());
		}
	}
