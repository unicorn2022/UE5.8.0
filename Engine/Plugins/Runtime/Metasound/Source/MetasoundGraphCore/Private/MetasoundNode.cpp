...
namespace Metasound
{
	...
	, Info(InInfo)
	, VertexInterface(InInfo.DefaultInterface)
	...
	{
		if (FInputDataVertex* Vertex = VertexInterface.GetInputInterface().Find(InVertexName))
		{
			Vertex->SetDefaultLiteral(InLiteral);
		}
		else
		{
			UE_LOGF(LogMetaSound, Error, "Cannot set default input. Vertex %ls does not exist on node %ls", *InVertexName.ToString(), *InstanceName.ToString());
		}
	}
