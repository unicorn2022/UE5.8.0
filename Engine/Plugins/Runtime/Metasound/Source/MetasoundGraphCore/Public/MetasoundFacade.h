...
namespace Metasound
{
	namespace MetasoundFacadePrivate
	{
		...
		class TIsFactoryMethodDeclared
		{
			private:
				...
				// Check for "static TUniquePtr<IOperator> U::CreateOperator(const FBuildOperatorParams& Inparams, FBuildResults& OutErrors)"
				template<typename T>
	...
	struct TFacadeOperatorClass
	{
		...
		static_assert(MetasoundFacadePrivate::TIsFactoryMethodDeclared<OperatorType>::Value, "To use the FNodeFacade constructor, the OperatorType must have the static function \"static TUniquePtr<IOperator> OperatorType::CreateOperator(const FBuildOperatorParams&, TArray<TUniquePtr<IOperatorBuildError>>&)\"");
	...
	private:
		...
		: FBasicNode(FNodeData(InInstanceName, InInstanceID, InNodeInfo.DefaultInterface.CreateVertexInterface()), MakeShared<FNodeClassMetadata>(InNodeInfo))
