...
namespace Metasound
{
	namespace VertexDataPrivate
	{
		...
		public:
			...
			void Set(const DataReferenceType& InDataReference)
			{
				...
				check(Metasound::IsCastable(InDataReference.GetDataTypeName(), Vertex.DataTypeName));
		...
		public:
			...
			{
				PRAGMA_DISABLE_INTERNAL_WARNINGS
				checkf(Metasound::IsCastable(InDataReference.GetDataTypeName(), Vertex.DataTypeName),
					TEXT("Ref=%s, Vertex=%s"), *InDataReference.GetDataTypeName().ToString(), *Vertex.DataTypeName.ToString());
				PRAGMA_ENABLE_INTERNAL_WARNINGS				
				Data.Emplace(InDataReference);
				CheckAccessTypeCompatibility(Vertex, *Data);
			}
	...
	public:
		...
		TDataReadReference<DataType> GetOrCreateDefaultDataReadReference(const FVertexName& InVertexName, const FOperatorSettings& InSettings) const
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Failed find vertex with name '%ls'. Cannot check for existing bound data.", *InVertexName.ToString());
			}
