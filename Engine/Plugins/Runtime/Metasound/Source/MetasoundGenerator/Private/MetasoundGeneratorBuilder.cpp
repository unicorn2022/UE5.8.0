...
namespace Metasound
{
	...
	void FAsyncMetaSoundBuilderBase::SetGraphOnGenerator(FOperatorAndInputs&& InOpAndInputs, FBuildResults&& InBuildResults)
	{
		...
		{
			UE_LOGF(LogMetaSound, Error, "Failed to build Metasound operator from graph in MetasoundSource [%ls]", *InitParams.AssetPath.ToString());
	...
	{
		TOptional<FChannelAgnosticTypeReadRef> FindOutputChannelAgnostic(const TArray<FVertexName>& InAudioVertexNames, const FVertexInterfaceData& InVertexData, const FOperatorSettings& InOperatorSettings, const FTopLevelAssetPath& InAssetPath)
		{
			...
			for (const FVertexName& AudioOutputName : InAudioVertexNames)
			{
				...
				{
					if (const FAnyDataReference* Ref = OutputVertexData.FindDataReference(AudioOutputName))
					{
						if (IsCastable(Ref->GetDataTypeName(), GetMetasoundDataTypeName<FChannelAgnosticType>()))
						{
							return Ref->GetDataReadReference<FChannelAgnosticType>();
						}
					}
				}
			}
		...
		TArray<FAudioBufferReadRef> FindOutputAudioBuffers(const TArray<FVertexName>& InAudioVertexNames, const FVertexInterfaceData& InVertexData, const FOperatorSettings& InOperatorSettings, const FTopLevelAssetPath& InAssetPath)
		{
			...
			for (const FVertexName& AudioOutputName : InAudioVertexNames)
			{
				if (const FAnyDataReference* Ref = OutputVertexData.FindDataReference(AudioOutputName))
				{
					if (Ref->GetDataTypeName() == Metasound::GetMetasoundDataTypeName<FAudioBuffer>())
					{
		...
		void LogBuildErrors(const FTopLevelAssetPath& InAssetPath, const FBuildResults& InBuildResults)
		{
			...
			for (const IOperatorBuilder::FBuildErrorPtr& Error : InBuildResults.Errors)
			{
				if (Error.IsValid())
				{
					UE_LOGF(LogMetaSound, Warning, "MetasoundSource [%ls] build error [%ls] \"%ls\"", *InAssetPath.ToString(), *(Error->GetErrorType().ToString()), *(Error->GetErrorDescription().ToString()));
				}
			}
		}
		...
		FInputVertexInterfaceData BuildGraphOperatorInputs(const FOperatorSettings& InOperatorSettings, FGeneratorInitParams& InInitParams)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Unable to build graph operator inputs for null graph in MetaSoundSource [%ls]", *InInitParams.AssetPath.ToString());
			...
			for (FAudioParameter& Parameter : InInitParams.DefaultParameters)
			{
				...
				if (const FInputDataVertex* InputVertex = InputInterface.Find(ParamName))
				{
					...
					{
						UE_LOGF(LogMetaSound, Warning, "Failed to create initial input data reference from parameter %ls of type %ls on graph in MetaSoundSource [%ls]", *ParamName.ToString(), *InputVertex->DataTypeName.ToString(), *InInitParams.AssetPath.ToString());
					}
				}
				else if(MetaSoundParameterEnableWarningOnIgnoredParameterCVar)
				{
					UE_LOGF(LogMetaSound, Warning, "Failed to set initial input parameter %ls on graph in MetaSoundSource [%ls]", *ParamName.ToString(), *InInitParams.AssetPath.ToString());
				}
			}
		...
		FOperatorAndInputs BuildGraphOperator(const FOperatorSettings& InOperatorSettings, FGeneratorInitParams& InInitParams, FBuildResults& OutBuildResults)
		{
			...
			if (!InInitParams.Graph)
			{
				UE_LOGF(LogMetaSound, Error, "Unable to build graph operator for null graph in MetaSoundSource [%ls]", *InInitParams.AssetPath.ToString());
				return OpAndInputs;
			}
		...
		MetasoundGeneratorPrivate::FMetasoundGeneratorData BuildGeneratorData(const FOperatorSettings& InOperatorSettings, const FGeneratorInitParams& InInitParams, FOperatorAndInputs&& InGraphOperatorAndInputs, TUniquePtr<Frontend::FGraphAnalyzer> InAnalyzer)
		{
			...
			// Get outputs
			TArray<FAudioBufferReadRef> OutputAudioBuffers = FindOutputAudioBuffers(InInitParams.AudioOutputNames, VertexData, InOperatorSettings, InInitParams.AssetPath);
			TOptional<FChannelAgnosticTypeReadRef> OutputCat = FindOutputChannelAgnostic(InInitParams.AudioOutputNames, VertexData, InOperatorSettings, InInitParams.AssetPath);
