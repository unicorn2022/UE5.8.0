			...
			if (ConsoleVariables::bEnableMetaSoundGeneratorNonFiniteLogging || ConsoleVariables::bEnableMetaSoundGeneratorInvalidSampleValueLogging)
			{
				...
				for (int32 ChannelIndex = 0; ChannelIndex < NumChannels; ChannelIndex++)
				{
					...
					for (int32 i = 0; i < Num; i++)
					{
						...
						{
							UE_LOGF(LogMetaSound, Error, "Found non-finite sample (%f) in channel %d of MetaSound %ls", Data[i], ChannelIndex, *InAssetPath.ToString());
							break;
						}
					}
					...
					if (ConsoleVariables::bEnableMetaSoundGeneratorInvalidSampleValueLogging)
					{
						...
						{
							UE_LOGF(LogMetaSound, Warning, "Found sample (absolute value: %f) exceeding threshold (%f) in channel %d of MetaSound %ls", MaxAbsValue, Threshold, ChannelIndex, *InAssetPath.ToString());
						}
					}
				}
			}
...
#define ENABLE_METASOUND_CONSTANT_OUTPUT_VERTEX_ERROR_LOG DO_CHECK
		...
		void LogErrorIfIsAudioVertex(const FVertexName& InVertexName, const TCHAR* Message)
		{
			...
			{
				UE_LOGF(LogMetaSound, Error, "Cannot modify vertex %ls. %ls", *InVertexName.ToString(), Message);
			}
		};
#else
		void LogErrorIfIsAudioVertex(const FVertexName& InVertexName, const TCHAR* Message)
...
namespace Metasound
{
	...
	void FGeneratorInitParams::Reset(FGeneratorInitParams& InParams)
	{
		...
		InParams.SourceFormatName = NAME_None;
	}
	...
	// Clamp a channel count to the nearest mixer-compatible value (1, 2, 4, 6, 8).
	...
	// Resolve the mixer-compatible channel count from a CAT SourceFormatName.
	static int32 ResolveNumChannelsFromFormat(FName InSourceFormatName)
	{
		PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
		if (const Audio::FChannelTypeFamily* Found = Audio::GetChannelRegistry().FindChannel(InSourceFormatName))
		{
			return ClampToMixerChannelCount(Found->NumChannels());
		}
		PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
		UE_LOGF(LogMetaSound, Warning, "CAT format '%ls' not found in channel registry; defaulting to stereo.", *InSourceFormatName.ToString());
		return 2;
	}
	...
	void FMetasoundGenerator::InitBase(FGeneratorInitParams& InInitParams)
	{
		...
		SourceFormatName = InInitParams.SourceFormatName;

		if (!SourceFormatName.IsNone())
		{
			// CAT path: derive mixer-compatible channel count from format name.
			NumChannels = ResolveNumChannelsFromFormat(SourceFormatName);
		}
		else
		{
			// Discrete path: channel count equals number of audio output vertices.
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void FMetasoundGenerator::SetGraph(TUniquePtr<MetasoundGeneratorPrivate::FMetasoundGeneratorData>&& InData, bool bTriggerGraph)
	{
		...
		GraphCatOutput.Reset();

		if (InData->OutputChannelAgnosticRef.IsSet())
		{
			// The MixerOutput must be a format AudioMixer supports (1/2/4/6/8 channels).
			// SourceFormatName carries the CAT format identity for the graph side (via Environment),
			// but the mixer-facing output is always determined from NumChannels (already clamped to
			// mixer-compatible values). This is intentionally lossy — the transcoder handles the
			// conversion from whatever the graph produces to the mixer's discrete format.
			//
			// IMPORTANT: Do NOT early-return from this block. SetGraph must complete operator
			// installation below regardless of CAT setup success. Early returns after buffer
			// resets and MoveTemp leave the generator in a stranded partial-init state with
			// no retry path (the pending graph has already been consumed).
			const TSharedPtr<const Audio::FDiscreteChannelTypeFamily> MixerFormat =
				Audio::FChannelAgnosticUtils::FindDiscreteFormatFromNumChannels(NumChannels);
			const Audio::FChannelTypeFamily* MixerChannel = nullptr;
			if (MixerFormat.IsValid())
			{
				MixerChannel = Audio::GetChannelRegistry().FindConcreteChannel(MixerFormat->GetName());
			}

			if (ensureMsgf(MixerChannel, TEXT("Failed to resolve mixer channel format for %d channels. CAT output will be silent."), NumChannels))
			{
				// Check if the graph output format already matches the mixer format.
				// If so, skip the intermediate buffer and transcoder entirely — just interleave
				// directly from the graph output. This avoids allocating a redundant MixerOutput
				// buffer and a no-op memcpy transcode on every block. (Addresses Phil Popp's
				// memory bloat concern: most CAT sources output a mixer-compatible format.)
				const Audio::FChannelTypeFamily& GraphOutputType = InData->OutputChannelAgnosticRef->Get()->GetType();
				const bool bIdentity = (&GraphOutputType == MixerChannel) || (GraphOutputType.GetName() == MixerChannel->GetName());

				if (bIdentity)
				{
					// Identity: graph output IS the mixer format. No transcode, no extra buffer.
					GraphCatOutput.Emplace(FChannelAgnosticState{
						*InData->OutputChannelAgnosticRef,	// GraphOutput
						{},					// MixerOutput — not allocated
						{},					// Transcoder — not needed
						true					// bIdentityTranscode
					});
				}
				else
				{
					// Non-identity: allocate mixer buffer and resolve transcoder.
					Audio::FChannelAgnosticType MixOut(*MixerChannel, NumFramesPerExecute);
					Audio::ChannelAgnosticTranscoder::FTranscoder Transcoder = Audio::FTranscoderResolver::Resolve(
						GraphOutputType,
						{
							.ToType = MixOut.GetType(),
							.TranscodeMethod = Audio::EChannelTranscodeMethod::MixUpOrDown,
							.MixMethod = Audio::EChannelMapMonoUpmixMethod::EqualPower,
						});
					if (ensureMsgf(Transcoder.IsSet(), TEXT("Failed to resolve CAT transcoder from '%s' to '%s'. CAT output will be silent."),
						*GraphOutputType.GetName().ToString(), *MixerChannel->GetName().ToString()))
					{
						GraphCatOutput.Emplace(FChannelAgnosticState{
							*InData->OutputChannelAgnosticRef,	// GraphOutput
							MoveTemp(MixOut),			// MixerOutput
							MoveTemp(Transcoder),			// Transcoder
							false					// bIdentityTranscode
						});
					}
					// On transcoder failure: GraphCatOutput stays empty → InterleaveGeneratedAudio
					// skips CAT path → silence for this source, but generator completes init normally.
				}
			}
			// On MixerChannel failure: GraphCatOutput stays empty → silence, but generator init completes.
		}
		else if (InData->OutputBuffers.Num() == NumChannels)
		{
		...
		else
		{
			...
			UE_LOGF(LogMetaSound, Warning, "Metasound generator expected %d number of channels, found %d", NumChannels, FoundNumChannels);
		...
		// Validate that either discrete output buffers or CAT output is configured.
		// If CAT setup failed (ensureMsgf above), both may be empty — that's a valid
		// degraded state (silence), not a crash. Only assert if we have neither AND
		// NumChannels expects output (which would cause OOB in InterleaveGeneratedAudio).
		ensureMsgf(NumChannels == GraphOutputAudio.Num() || GraphCatOutput.IsSet() || NumChannels == 0,
			TEXT("MetaSound generator has %d expected channels but no audio output configured (GraphOutputAudio=%d, CatOutput=%s)"),
			NumChannels, GraphOutputAudio.Num(), GraphCatOutput.IsSet() ? TEXT("set") : TEXT("unset"));
	...
	int32 FMetasoundGenerator::OnGenerateAudio(float* OutAudio, int32 NumSamplesRemaining)
	{
		...
		while (NumSamplesRemaining > 0 && bIsFinishTriggered == 0)
		{
			...
			// Create a scoped timed section for the bulk of the processing...
			{
				...
				{
					UE_LOGF(LogMetasoundGenerator, Warning, "Terminating OnGenerateAudio() loop for MetaSound %ls -- FillWithBuffer() returned %i", *AssetPath.ToString(), ThisLoopNumSamplesWritten);
					break;
				}
			}
	...
	void FMetasoundGenerator::InterleaveGeneratedAudio()
	{
		...
		if (GraphCatOutput.IsSet())
		{
			// Pointer to the multi-mono data we'll interleave from.
			const Audio::FChannelAgnosticType* SourceData = nullptr;

			if (GraphCatOutput->bIdentityTranscode)
			{
				// Fast path: graph output format matches mixer format — no transcode needed,
				// no intermediate buffer allocated. Read directly from graph output.
				SourceData = GraphCatOutput->GraphOutput.operator->();
			}
			else
			{
				// Transcode path: graph output format differs from mixer format.
				// Convert into the pre-allocated MixerOutput buffer, then interleave from that.
				check(GraphCatOutput->MixerOutput.IsSet());
				Audio::TStackArrayOfPointers<const float> Src = Audio::MakeMultiMonoPointersFromView(GraphCatOutput->GraphOutput->GetRawMultiMono(), NumFramesPerExecute, GraphCatOutput->GraphOutput->NumChannels());
				Audio::TStackArrayOfPointers<float> Dst = Audio::MakeMultiMonoPointersFromView(GraphCatOutput->MixerOutput->GetRawMultiMono(), NumFramesPerExecute, GraphCatOutput->MixerOutput->NumChannels());
				GraphCatOutput->Transcoder(Src, Dst, NumFramesPerExecute);
				SourceData = &GraphCatOutput->MixerOutput.GetValue();
			}

			// Interleave from SourceData into the output buffer.
			for (int32 ChannelIndex = 0; ChannelIndex < NumChannels; ChannelIndex++)
			{
				const TArrayView<const float> InputBuffer = SourceData->GetChannel(ChannelIndex);
	...
	bool FMetasoundConstGraphGenerator::TryUseCachedOperator(FGeneratorInitParams& InInitParams, bool bInTriggerGraph)
	{
		...
		if (OperatorPool.IsValid())
		{
			...
			{
				UE_LOGF(LogMetasoundGenerator, VeryVerbose, "Using cached operator %ls for MetaSound %ls", *OperatorPoolID->ToString(), *InInitParams.AssetPath.ToString());
	...
	void FMetasoundConstGraphGenerator::ReleaseOperatorToCache()
	{
		...
		if (OperatorPoolID.IsSet())
		{
			...
			if (OperatorPool.IsValid() && GraphOperator.IsValid())
			{
				...
				UE_LOGF(LogMetasoundGenerator, VeryVerbose, "Caching operator %ls", *OperatorPoolID->ToString());
	...
	void FMetasoundDynamicGraphGenerator::OnOutputUpdated(const FVertexName& InVertexName, const FOutputVertexInterfaceData& InGraphOutputs)
	{
		...
		{
			// For CAT sources, the audio output vertex carries FChannelAgnosticType instead of
			// FAudioBuffer. GraphOutputAudio is empty in the CAT path, so update GraphCatOutput
			// instead. MixerOutput buffer and Transcoder are format-dependent (not reference-
			// dependent) and remain valid across reference updates.
			if (GraphCatOutput.IsSet())
			{
				GraphCatOutput->GraphOutput = InGraphOutputs.GetDataReadReference<FChannelAgnosticType>(InVertexName);
			}
			else
			{
