...
namespace Metasound
{
...
#endif
			UE_LOGF(LogMetasoundGenerator, VeryVerbose, "Attempt to claim operator with ID %ls from operator pool resulted in %ls", *InOperatorID.ToString(), *::LexToString(bCacheHit));
		}
	...
	void FOperatorPool::AddOperatorInternal(const FOperatorPoolEntryID& InOperatorID, FOperatorAndInputs && OperatorAndInputs)
	{
...

		{
			...
			UE_LOGF(LogMetasoundGenerator, VeryVerbose, "Adding operator with ID %ls to operator pool", *InOperatorID.ToString());
		...
		// Call Trim() outside the lock so any evicted operators are destroyed
		// after CritSec is released (see Trim() for deadlock details).
		// Note: another thread may claim the just-added operator between the
		// lock release and Trim(); Trim() re-reads Stack.Num() under its own
		// lock, so it will simply find nothing to trim in that case.
		Trim();
	}
	...
	void FOperatorPool::StopAsyncTasks()
	{
		...
		{
			UE_LOGF(LogMetasoundGenerator, Display, "Cancelling active MetaSound Cache Pool Operator build requests...");
	...
	void FOperatorPool::BuildAndAddOperator(TUniquePtr<FOperatorBuildData> InBuildData)
	{
		...
		if (bMetasoundPoolSyncGraphRetrieval)
		{
			...
			if (!Graph.IsValid())
			{
				UE_LOGF(LogMetasoundGenerator, Error, "Failed to retrieve graph '%ls' synchronously when attempting to BuildAndAddOperator to pool", *InBuildData->RegistryKey.ToString());
				return;
			}
		}
		...
		ExecuteTaskAsync([Graph, PreCacheData = MoveTemp(InBuildData)](FTaskId, TWeakPtr<FOperatorPool> WeakPoolPtr)
		{
			...
			{
				UE_LOGF(LogMetasoundGenerator, Error, "Failed to retrieve graph '%ls' async when attempting to BuildAndAddOperator to pool", *PreCacheData->RegistryKey.ToString());
				return;
			}
	...
	void FOperatorPool::TouchOperators(const FOperatorPoolEntryID& InOperatorID, int32 NumToTouch)
	{
		if (!IsStopping())
		{
			...
			UE_LOGF(LogMetasoundGenerator, VeryVerbose, "Touching operator %d operators with ID %ls in operator pool", NumToMove, *InOperatorID.ToString());
			if (!NumToMove)
			{
				return;
			}
	...
	void FOperatorPool::RemoveOperatorsWithID(const FOperatorPoolEntryID& InOperatorID)
	{
		...
		ExecuteTaskAsync([OperatorID = InOperatorID](FTaskId, TWeakPtr<FOperatorPool> WeakPoolPtr)
		{
			if (TSharedPtr<FOperatorPool> Pool = WeakPoolPtr.Pin())
			{
				UE_LOGF(LogMetasoundGenerator, VeryVerbose, "Removing operators with ID %ls from operator pool", *OperatorID.ToString());

				// Move operators out of the map while holding CritSec, then destroy
				// them after the lock is released. Operator dtors (e.g.
				// FEnvelopeFollowerGenerator::~) call OnAudioDeviceDestroyed.Remove(),
				// which acquires the TS delegate write lock. Destroying under CritSec
				// while the broadcast thread holds the read lock causes an A->B / B->A
				// deadlock. By moving first, CritSec is released before destruction.
				TArray<FOperatorAndInputs> OperatorsToDestroy;
				int32 NumRemoved = 0;
				{
					FScopeLock Lock(&Pool->CriticalSection);
					if (TArray<FOperatorAndInputs>* Found = Pool->Operators.Find(OperatorID))
					{
						OperatorsToDestroy = MoveTemp(*Found);
					}
					Pool->Operators.Remove(OperatorID);
					NumRemoved = Pool->Stack.Remove(OperatorID);
	...
	{
		// Collect evicted operators outside the lock scope, then destroy them
		// after CritSec is released. Operator dtors (e.g.
		// FEnvelopeFollowerGenerator::~) call OnAudioDeviceDestroyed.Remove(),
		// which acquires the TS delegate write lock. Destroying under CritSec
		// while the broadcast thread holds the read lock can cause deadlock
		TArray<FOperatorAndInputs> TrimmedOperators;
		{
			FScopeLock Lock(&CriticalSection);
			int32 NumToTrim = Stack.Num() - Settings.MaxNumOperators;
			if (NumToTrim > 0)
			{
				TrimmedOperators.Reserve(NumToTrim);
				UE_LOGF(LogMetasoundGenerator, Verbose, "Trimming %d operators", NumToTrim);
				for (int32 i = 0; i < NumToTrim; i++)
				{
					UE_LOGF(LogMetasoundGenerator, VeryVerbose, "Trimming operator with ID %ls", *Stack[i].ToString());
					TArray<FOperatorAndInputs>* OperatorArray = Operators.Find(Stack[i]);
					if (ensure(OperatorArray))
					{
						if (ensure(OperatorArray->Num() > 0))
						{
							TrimmedOperators.Add(OperatorArray->Pop(EAllowShrinking::No));
		...
		// TrimmedOperators destructs here — CritSec is NOT held
	}
} // namespace Metasound
