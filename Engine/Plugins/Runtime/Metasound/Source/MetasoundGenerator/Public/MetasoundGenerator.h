...
namespace Metasound
{
	...
	namespace MetasoundGeneratorPrivate
	{
		...
		struct FMetasoundGeneratorData
		{
			...
			TOptional<FChannelAgnosticTypeReadRef> OutputChannelAgnosticRef;
		};
	}
	...
	struct FGeneratorInitParams
	{
		...
		// CAT format identity (e.g. "Cat:Mono1Dot0"). Empty for non-CAT sources.
		// The generator derives the mixer-compatible channel count internally:
		//   - CAT path:     from SourceFormatName via channel registry lookup + clamp.
		//   - Discrete path: from AudioOutputNames.Num().
	...
	public:
		...
		UE_API virtual int32 GetNumChannels() const override;
	...
	protected:
		...
		struct FChannelAgnosticState
		{
			FChannelAgnosticTypeReadRef GraphOutput;
			TOptional<Audio::FChannelAgnosticType> MixerOutput;		// Only allocated when transcode is needed (graph format != mixer format).
			Audio::ChannelAgnosticTranscoder::FTranscoder Transcoder;	// Empty when identity — no transcode needed.
			bool bIdentityTranscode = false;				// True when graph output format matches mixer format (skip transcode + extra buffer).
		};
		TOptional<FChannelAgnosticState> GraphCatOutput;			// This is the CAT reference from the graph.
	...
	private:
		...
		int32 NumChannels = 0;
		FName SourceFormatName; // Graph-side CAT format identity. Set in Environment upstream (MetasoundSource); held here for reference only. Mixer output uses NumChannels separately.
