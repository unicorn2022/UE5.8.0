...
namespace Metasound::Editor
{
	namespace GraphPrivate
	{
		...
		void OnLiteralChanged(UMetasoundEditorGraphMember& InMember, const FGuid* InPageID, EPropertyChangeType::Type InChangeType)
		{
			...
			EMetaSoundLiteralChangeType LiteralChangeType = EMetaSoundLiteralChangeType::Set;
			if (InChangeType & EPropertyChangeType::ResetToDefault)
			{
				LiteralChangeType = EMetaSoundLiteralChangeType::Reset;
			}
			InMember.UpdateFrontendDefaultLiteral(bPostTransaction, InPageID, LiteralChangeType);
...
bool UMetasoundEditorGraphVertex::SetIsAdvancedDisplay(const bool IsAdvancedDisplay)
{
	...
	const FText TransactionLabel = FText::Format(LOCTEXT("SetGraphVertexIsAdvancedDisplayState", "Set Metasound {0} IsAdvancedDisplay"), GetGraphMemberLabel());
...
bool UMetasoundEditorGraphVertex::NameContainsInterfaceNamespace(FMetasoundFrontendInterface* OutInterface) const
{
	...
	if (!InterfaceNamespace.IsNone())
	{
		...
		if (bFoundVersion)
		{
			if (OutInterface)
			{
				const bool bIsValid = IInterfaceRegistry::Get().FindInterface(InterfaceVersion, *OutInterface);
				checkf(bIsValid, TEXT("Version found in search engine must be registered with interface registry"));
				return bIsValid;
			}
			return true;
		}
...
{
	FMetaSoundFrontendDocumentBuilder& Builder = GetFrontendBuilderChecked();
	return Builder.GetIsGraphEditable() && !IsInterfaceMember();
}
...
bool UMetasoundEditorGraphVertex::CanRename(const FText& InNewText, FText& OutError) const
{
	...
	const FMetaSoundFrontendDocumentBuilder& Builder = GetFrontendBuilderChecked();
	if (!Builder.GetIsGraphEditable())
	{
		OutError = FText::Format(LOCTEXT("GraphVertexRenameInvalid_Preset", "{0} is a vertex in a read-only graph and cannot be renamed."), InNewText);
		return false;
	}
...
void UMetasoundEditorGraphInput::UpdateFrontendDefaultLiteral(bool bPostTransaction, const FGuid* InPageID, EMetaSoundLiteralChangeType InChangeType)
{
	using namespace Metasound::Editor;
	...
	if (Literal)
	{
		...
		if (InPageID)
		{
			...
			if (Literal->TryFindDefault(Default, InPageID))
			{
				...
				{
					Builder.SetGraphInputDefault(MemberName, Default, InPageID, InChangeType);
				}
			}
		}
		else
		{
			...
			{
				Builder.SetGraphInputDefaults(MemberName, MoveTemp(NewDefaults), InChangeType);
			}
		}
	}
}
...
void UMetasoundEditorGraphOutput::UpdateFrontendDefaultLiteral(bool bPostTransaction, const FGuid* InPageID, EMetaSoundLiteralChangeType InChangeType)
{
	using namespace Metasound;
	using namespace Metasound::Editor;
	using namespace Metasound::Frontend;
...
void UMetasoundEditorGraphVariable::UpdateFrontendDefaultLiteral(bool bPostTransaction, const FGuid* InPageID, EMetaSoundLiteralChangeType InChangeType)
{
	using namespace Metasound;
	using namespace Metasound::Editor;
	using namespace Metasound::Frontend;
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		if (OutBuilder.GetConstDocumentChecked().RootGraph.PresetOptions.bIsPreset)
		{
			return;
		}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		const FMetasoundFrontendNode* InputNode = OutBuilder.FindNode(Input.NodeID);
		...
		const FName InputName = InputNode->Name;
		if (InputNode->Interface.Outputs.IsEmpty())
		{
			UE_LOGF(LogMetaSound, Error, "Failed to migrate input node '%ls': Input has no outputs assigned on interface.", *InputName.ToString());
			return;
		}
					...
					for (const UEdGraphPin* Linked : Pin->LinkedTo)
					{
						if (Linked)
						{
							...
							if (const FMetasoundFrontendVertex* ConnectedInput = OutBuilder.FindNodeInput(ConnectedNodeID, Linked->GetFName()))
							{
								...
								if (!bEdgeRemoved)
								{
									UE_LOGF(LogMetaSound, Display, "Editor graph '%ls' migration failed to remove exact node '%ls' connection to class output '%ls': Removing any existing connections.",
								...
								{
									UE_LOGF(LogMetaSound, Display, "Editor graph '%ls' migration failed to remove connected for node '%ls' class output '%ls': Ignoring connection upgrade from input '%ls' ",
							...
							{
								UE_LOGF(LogMetaSound, Display, "Editor graph '%ls' migration failed to find node '%ls' class output '%ls': Ignoring connection upgrade from input '%ls'",
									*OutBuilder.GetDebugName(),
									*ConnectedNode->GetDisplayName().ToString(),
									*Linked->GetName(),
									*InputName.ToString());
