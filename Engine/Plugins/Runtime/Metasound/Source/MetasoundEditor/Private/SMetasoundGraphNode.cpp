	...
	namespace Editor
	{
			...
			return TAttribute<EVisibility>::CreateSPLambda(AsShared(), [this]()
			{
				...
				if (const UMetasoundEditorGraphMemberNode* Node = Cast<UMetasoundEditorGraphMemberNode>(&GetMetaSoundNode()))
				{
					if (const UMetasoundEditorGraphVertex* Vertex = Cast<UMetasoundEditorGraphVertex>(Node->GetMember()))
					{
						...
						FName RouterName;
						if (IInterfaceRegistry::Get().FindInterfaceRouter(Vertex->GetInterfaceVersion(), RouterName)
							&& RouterName != Audio::IParameterTransmitter::RouterName)
						{
							return EVisibility::Hidden;
						}
		...
		TSharedPtr<SGraphPin> SMetaSoundGraphNode::CreatePinWidget(UEdGraphPin* InPin) const
		{
			...
			if (const UMetasoundEditorGraphSchema* GraphSchema = Cast<const UMetasoundEditorGraphSchema>(InPin->GetSchema()))
			{
				...
				{
					PinWidget = FMetaSoundGraphPanelPinFactory::GetChecked()->CreatePin(InPin);
				}
			}
			...
			if (FMetaSoundGraphPanelPinFactory::GetChecked()->GetCustomPinIcons(InPin, PinConnectedIcon, PinDisconnectedIcon))
			{
		...
		TSharedRef<SWidget> SMetaSoundGraphNode::CreateTitleWidget(TSharedPtr<SNodeTitle> NodeTitle)
		{
			...
			const FMetasoundFrontendClass* FrontendClass = Node.GetFrontendClass();
			if (!FrontendClass || !FrontendClass->Style.Display.bShowName)
			{
				return SNullWidget::NullWidget;
			}

			const TSharedPtr<SHorizontalBox> TitleBoxWidget =
				SNew(SHorizontalBox)
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.VAlign(VAlign_Center)
		...
		{
			const UMetasoundEditorGraphNode& Node = GetMetaSoundNode();
			FLinearColor TitleColor = GraphNode->IsDeprecated() ? FLinearColor::Red : Node.GetNodeTitleTextColor();
			if (ClassType != EMetasoundFrontendClassType::External)
			{
				TitleColor.A = FadeCurve.GetLerp();
			}
		...
		void SMetaSoundGraphNode::SetDefaultTitleAreaWidget(TSharedRef<SOverlay> DefaultTitleAreaWidget)
		{
			...
			FMetasoundFrontendClassStyleDisplay StyleDisplay;
			if (const FMetasoundFrontendClass* FrontendClass = Node.GetFrontendClass())
			{
				StyleDisplay = FrontendClass->Style.Display;
			}

			if (!StyleDisplay.bShowName)
			{
			...
			const bool bShowTitleIcon = Node.ShouldShowTitleIcon();
			const TSharedPtr<SNodeTitle> NodeTitle = SNew(SNodeTitle, GraphNode);

			const TSharedRef<SWidget> TitleTextBlock =
				SNew(SVerticalBox)
				+ SVerticalBox::Slot()
				.AutoHeight()
				.HAlign(bShowTitleIcon ? HAlign_Left : HAlign_Center)
				[
			...
			TSharedRef<SWidget> TitleContent =
				SNew(SHorizontalBox)
				+ SHorizontalBox::Slot()
				.AutoWidth()
				.VAlign(VAlign_Center)
			...
			const TSharedRef<SWidget> MainTitleArea =
				SNew(SBorder)
				.BorderImage(FAppStyle::GetBrush("Graph.Node.ColorSpill"))
				.Padding(FMargin(10.f, 5.f, 10.f, 3.f))
				.BorderBackgroundColor(Node.GetNodeTitleColor())
				.HAlign(HAlign_Fill)
				.VAlign(VAlign_Center)
				[
					TitleContent
		...
		TSharedRef<SWidget> SMetaSoundGraphNode::CreateTitleIcon() const
		{
			const UMetasoundEditorGraphNode& Node = GetMetaSoundNode();
			if (!Node.ShouldShowTitleIcon())
			{
				return SNullWidget::NullWidget;
			}

			const FText& TooltipText = Node.GetNodeTitleIconTooltipText();
			const bool bShowTooltip = !TooltipText.IsEmpty();
			...
			return SNew(SBox)
				...
				[
					...
					.BorderImage_Lambda([]()
					{
						static const FSlateRoundedBoxBrush RoundedBoxBrush(FLinearColor::White, 3.f);
						return &RoundedBoxBrush;
					})
					.BorderBackgroundColor(FLinearColor(0, 0, 0, 0.33f))
					[
						SNew(SBox)
						.WidthOverride(16.f)
						.HeightOverride(16.f)
						...
						[
							SNew(SImage)
							.Image(Node.GetNodeTitleIconBrush())
		...
		void SMetaSoundGraphNode::FinalizeValueTransaction(TWeakObjectPtr<UMetasoundEditorGraphMember> GraphMemberPtr, TFunctionRef<void(const FGuid&, UMetasoundEditorGraphMember&, bool)> SetValue)
		{
			...
			if (!bIsInputWidgetTransacting)
			{
				...
				UE_LOGF(LogMetaSound, Warning, "Unmatched MetaSound editor widget transaction.");
			}
