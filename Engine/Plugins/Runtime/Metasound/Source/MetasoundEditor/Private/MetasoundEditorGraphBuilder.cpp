...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			if (OutputPin)
			{
				if (UMetasoundEditorGraphExternalNode* ExternalNode = Cast<UMetasoundEditorGraphExternalNode>(OutputPin->GetOwningNode()))
				{
					if (ExternalNode->GetBreadcrumb().ClassName == FRerouteNodeTemplate::ClassName)
					{
						...
						if (UEdGraphPin** RerouteInput = ExternalNode->Pins.FindByPredicate(IsInput); RerouteInput && *RerouteInput)
						{
			...
			if (OutputPin)
			{
				if (UMetasoundEditorGraphExternalNode* ExternalNode = Cast<UMetasoundEditorGraphExternalNode>(OutputPin->GetOwningNode()))
				{
					if (ExternalNode->GetBreadcrumb().ClassName == FRerouteNodeTemplate::ClassName)
					{
						...
						if (const UEdGraphPin* const* RerouteInput = ExternalNode->Pins.FindByPredicate(IsInput))
						{
		...
		FMetasoundFrontendClassInput FGraphBuilder::CreateUniqueClassInput(UObject& InMetaSound, const FCreateNodeVertexParams& InParams, const TArray<FMetasoundFrontendClassInputDefault>& InDefaultLiterals, const FName* InNameBase)
		{
			...
			ClassInput.Name = DocBuilder.GenerateUniqueInputName(InNameBase ? InNameBase->ToString() : TEXT("Input"));
			ClassInput.TypeName = InParams.DataType;
			ClassInput.VertexID = FDocumentIDGenerator::Get().CreateVertexID(Document);
			ClassInput.NodeID = FDocumentIDGenerator::Get().CreateNodeID(Document);
		...
		FMetasoundFrontendClassOutput FGraphBuilder::CreateUniqueClassOutput(UObject& InMetaSound, const FCreateNodeVertexParams& InParams, const FName* InNameBase)
		{
			...
			UMetaSoundBuilderBase& Builder = FDocumentBuilderRegistry::GetChecked().FindOrBeginBuilding(InMetaSound);
			ClassOutput.Name = Builder.GetConstBuilder().GenerateUniqueOutputName(InNameBase ? InNameBase->ToString() : TEXT("Output"));
...
// 				ensure(Result == EMetaSoundBuilderResult::Succeeded);
			...
			EEdGraphPinDirection Direction)
		{
			...
			if (const FEdGraphPinType* RegisteredPinType = FMetaSoundGraphPanelPinFactory::GetChecked()->FindPinType(InVertex.TypeName))
			{
		...
		{
			FEdGraphPinType PinType;
			if (const FEdGraphPinType* RegisteredPinType = FMetaSoundGraphPanelPinFactory::GetChecked()->FindPinType(InDataType))
			{
				PinType = *RegisteredPinType;
			}
		...
		bool FGraphBuilder::SynchronizeConnections(const FMetaSoundFrontendDocumentBuilder& InBuilder, UMetasoundEditorGraph& OutGraph)
		{
			...
			for (UMetasoundEditorGraphNode* EditorNode : EditorNodes)
			{
				...
				{
					UE_LOGF(LogMetasoundEditor, Error, "Multiple editor nodes associated with FrontendDocument node with ID '%ls'", *NodeID.ToString());
				}
				else
				{
					...
					if (const FMetasoundFrontendVertex* Output = InBuilder.FindNodeOutputConnectedToNodeInput(EditorNode->GetNodeID(), NodeInput->VertexID, &ConnectedOutputNode))
					{
						...
						if (bAddLink)
						{
							...
							if (OutputEditorNode)
							{
								...
								UE_LOGF(LogMetasoundEditor, Verbose, "Synchronizing Node '%ls' Connection: Linking Pin '%ls' to '%ls'", *OwningNodeName.ToString(), *MatchingPin->GetName(), *OutputPin->GetName());
							...
							{
								UE_LOGF(LogMetasoundEditor, Warning, "Failed to synchronize Frontend Node '%ls' Connection: Pin '%ls'", *OwningNodeName.ToString(), *MatchingPin->GetName());
							}
						}
					}
					else
					...
					for (UEdGraphPin* Pin : Node->Pins)
					{
						...
						if (const FMetasoundFrontendClassOutput* Output = ClassOutputPtr.Get())
						{
							...
							if (NodeHandle->GetID() != Output->NodeID)
							{
								...
								UE_LOGF(LogMetasoundEditor, Verbose, "Editor Output Node '%ls' interface versioned", *Node->GetDisplayName().ToString());
			...
			for (const FNodeClassPair& Pair : FrontendNodeAndClass)
			{
				...
				for (const TPair<FGuid, FVector2D>& Location : CurrentStyle.Display.Locations)
				{
					...
					switch (ClassMetadata.GetType())
					{
						...
						{
							NewGraphNode = Cast<UMetasoundEditorGraphNode>(AddExternalNode(MetaSoundObject, NodeID, ClassMetadata, false));
						}
						break;
						...
						case EMetasoundFrontendClassType::Template:
						{
							...
							{
								NewGraphNode = Cast<UMetasoundEditorGraphNode>(AddInputNode(MetaSoundObject, NodeID, false));
							}
							else
							{
								NewGraphNode = Cast<UMetasoundEditorGraphNode>(AddTemplateNode(MetaSoundObject, NodeID, ClassMetadata, false));
							}
						}
						break;
						...
						case EMetasoundFrontendClassType::Output:
						{
							NewGraphNode = Cast<UMetasoundEditorGraphNode>(AddOutputNode(MetaSoundObject, NodeID, false));
						}
						break;
						...
						case EMetasoundFrontendClassType::Variable:
						{
							NewGraphNode = Cast<UMetasoundEditorGraphNode>(AddVariableNode(MetaSoundObject, NodeID, false));
						}
						break;
					...
					{
						// Establish the editor node's identity GUID directly from the frontend Display.Locations
						// key being iterated. SyncLocationFromFrontendNode (called via GraphBuilderPrivate::AddNode)
						// picks the first map entry, which is order-sensitive across duplicated page graphs and can
						// diverge from the outer iterator's current Location.Key.
						if (Location.Key.IsValid())
						{
							NewGraphNode->NodeGuid = Location.Key;
						}
						else
						{
							// Legacy assets occasionally carry an invalid GUID key from older interface
							// migrations. Leave the freshly-finalized editor NodeGuid in place; the node
							// will reposition the next time the user moves it (which routes through
							// SetNodeLocation and rewrites the entry with a valid GUID).
							UE_LOGF(LogMetasoundEditor, Display, "Frontend node %ls has an invalid Display.Locations key; editor node will use a fresh GUID until next user move.", *NodeID.ToString());
						}
