...
// 				}
		...
		void SMetasoundPinValueInspector::PopulateTreeView()
		{
			...
			if (GraphPinObj->PinType.PinCategory == FMetaSoundGraphPanelPinFactory::PinCategoryFloat)
			{
			...
			else if (GraphPinObj->PinType.PinCategory == FMetaSoundGraphPanelPinFactory::PinCategoryInt32)
			{
				NewLine = MakeShared<FMetasoundNumericDebugLineItem>(GraphPinObj, [&](const FGraphConnectionManager& ConnectionManager, const FGuid& NodeID, const FName OutputName)
				{
					int32 Value = 0;
			...
			else if (GraphPinObj->PinType.PinCategory == FMetaSoundGraphPanelPinFactory::PinCategoryString)
			{
				NewLine = MakeShared<FMetasoundNumericDebugLineItem>(GraphPinObj, [&](const FGraphConnectionManager& ConnectionManager, const FGuid& NodeID, const FName OutputName)
				{
			...
			else if (GraphPinObj->PinType.PinCategory == FMetaSoundGraphPanelPinFactory::PinCategoryBoolean)
			{
				NewLine = MakeShared<FMetasoundNumericDebugLineItem>(GraphPinObj, [&](const FGraphConnectionManager& ConnectionManager, const FGuid& NodeID, const FName OutputName)
				{
					bool Value = false;
