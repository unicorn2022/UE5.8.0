	...
	if (const FMetasoundFrontendNode* Node = GetFrontendNode())
	{
		if (!Node->Style.Display.Locations.IsEmpty())
		{
			if (bUpdateEditorNodeID)
			{
				...
				{
					UE_LOGF(LogMetaSound, Warning, "Style location map for node %ls should only contain one location (legacy support for multiple location values should be migrated by this point)", *Node->GetID().ToString());
				}
...
const FMetasoundFrontendClass* UMetasoundEditorGraphNode::GetNodeFrontendClass() const
{
	using namespace Metasound::Frontend;

	UObject& MetaSound = GetMetasoundChecked();
	const FMetaSoundFrontendDocumentBuilder& DocBuilder = IDocumentBuilderRegistry::GetChecked().FindOrBeginBuilding(&MetaSound);

	const FMetasoundFrontendNode* Node = DocBuilder.FindNode(GetNodeID());
	if (!Node)
	{
		return nullptr;
	}

	return DocBuilder.FindDependency(Node->ClassID);
}

const UClass* UMetasoundEditorGraphNode::GetNodeAssetClass() const
{
	using namespace Metasound::Frontend;

	const FMetasoundFrontendClass* NodeFrontendClass = GetNodeFrontendClass();
	if (!NodeFrontendClass || !IMetaSoundAssetManager::GetChecked().IsAssetClass(NodeFrontendClass->Metadata))
	{
		return nullptr;
	}

	FMetasoundAssetBase* Asset = IMetaSoundAssetManager::GetChecked().FindAsset(NodeFrontendClass->Metadata);
	if (!Asset)
	{
		return nullptr;
	}

	const IMetaSoundDocumentInterface* DocumentInterface = Cast<IMetaSoundDocumentInterface>(Asset->GetOwningAsset());
	return DocumentInterface ? &DocumentInterface->GetBaseMetaSoundUClass() : nullptr;
}
...
const FSlateBrush* UMetasoundEditorGraphNode::GetNodeTitleIconBrush() const
{
	using namespace Metasound::Frontend;

	const FSlateIcon DefaultIcon = FSlateIcon("MetaSoundStyle", "MetasoundEditor.Metasound.Node.Icon");

	const FMetasoundFrontendClass* NodeFrontendClass = GetNodeFrontendClass();
	if (!NodeFrontendClass)
	{
		return DefaultIcon.GetIcon();
	}

	EMetasoundFrontendClassType ClassType = NodeFrontendClass->Metadata.GetType();
	if (ClassType == EMetasoundFrontendClassType::Graph || ClassType == EMetasoundFrontendClassType::External)
	{
		const FMetaSoundAssetKey AssetKey(NodeFrontendClass->Metadata);
...
void UMetasoundEditorGraphNode::GetNodeContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	...
	if (Context->Node)
	{
		if (GetBuilderChecked().GetConstBuilder().GetIsGraphEditable())
		{
			{
...
FLinearColor UMetasoundEditorGraphMemberNode::GetNodeTitleColor() const
{
	return FLinearColor::Transparent;
}

FLinearColor UMetasoundEditorGraphMemberNode::GetNodeTitleIconColor() const
{
	return GetNodeTitleTextColor();
}
...
FLinearColor UMetasoundEditorGraphOutputNode::GetNodeTitleTextColor() const
{
...
const FSlateBrush* UMetasoundEditorGraphOutputNode::GetNodeTitleIconBrush() const
{
	FSlateIcon OutputIcon = FSlateIcon("MetaSoundStyle", "MetasoundEditor.Graph.Node.Class.Output");
	return OutputIcon.GetIcon();
}
...
bool UMetasoundEditorGraphExternalNode::CanAutoUpdate() const
{
	...
	FMetaSoundFrontendDocumentBuilder& DocBuilder = GetBuilderChecked().GetBuilder();
	return DocBuilder.CanAutoUpdate(GetNodeID());
}
...
FLinearColor UMetasoundEditorGraphExternalNode::GetNodeTitleColor() const
{
	...
	// Return the asset type color if the node is an asset type (e.g. Source, Patch)
	if (const UClass* NodeAssetClass = GetNodeAssetClass())
	{
		if (const UAssetDefinition* AssetDefinition = UAssetDefinitionRegistry::Get()->GetAssetDefinitionForClass(NodeAssetClass))
		{
			return AssetDefinition->GetAssetColor();
		}
	}

	// Use the style color of native nodes, if specified in the node class.
	UObject& MetaSound = GetMetasoundChecked();
	const FMetaSoundFrontendDocumentBuilder& DocBuilder = IDocumentBuilderRegistry::GetChecked().FindOrBeginBuilding(&MetaSound);
	if (const FMetasoundFrontendNode* Node = DocBuilder.FindNode(GetNodeID()))
	{
		const FMetasoundFrontendClass* FrontendClass = DocBuilder.FindDependency(Node->ClassID);
		...
		if (!StyleDisplay.StyleSet.IsNone() && !StyleDisplay.Color.IsNone())
		{
			...
			{
				return Style->GetSlateColor(StyleDisplay.Color, nullptr, FLinearColor::Transparent).GetSpecifiedColor();
			}
		}
	}

	// Fallback color for outdated nodes, etc.
	if (const UMetasoundEditorSettings* EditorSettings = GetDefault<UMetasoundEditorSettings>())
	{
...
const FSlateBrush* UMetasoundEditorGraphExternalNode::GetNodeTitleIconBrush() const
{
	if (const FMetasoundFrontendClass* FrontendClass = GetFrontendClass())
	{
		const FMetasoundFrontendClassStyleDisplay StyleDisplay = FrontendClass->Style.Display;
		...
		{
			return FSlateIcon(StyleDisplay.StyleSet, StyleDisplay.Icon).GetIcon();
		}
	}

	return Super::GetNodeTitleIconBrush();
}

FText UMetasoundEditorGraphExternalNode::GetNodeTitleIconTooltipText() const
{
	FText TooltipText;
	if (!bIsClassNative)
	{
		...
		{
			const FMetasoundFrontendClassMetadata& Metadata = FrontendClass->Metadata;
			const FString MetadataAuthor = Metadata.GetAuthor();

			if (!MetadataAuthor.IsEmpty())
			{
				TooltipText = FText::Format(LOCTEXT("MetaSoundNodeAuthorFormat", "Author: {0}"), FText::FromString(MetadataAuthor));
			}
		}
	}
...
const FSlateBrush* UMetasoundEditorGraphVariableNode::GetNodeTitleIconBrush() const
{
