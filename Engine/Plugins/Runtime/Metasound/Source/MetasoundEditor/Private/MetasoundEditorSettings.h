...
public:
	...
	UE_DEPRECATED(5.8, "Custom pin colors are now managed by FMetaSoundGraphPanelPinFactory. Use IMetaSoundGraphPanelPinFactory::GetPinColor instead.")
	...
	UE_DEPRECATED(5.8, "Asset nodes now use their corresponding asset type color.")
	...
	/** MaxRecentEntries can go up to this number (limit must be known, for ordering Common actions in graph action menu above Recent ones) */
	static constexpr int32 MaxRecentEntriesEver = 200;

	/** The maximum number of Recent entries kept in the Metasound Editor action menu (ie., graph context menu). Older entries are overwritten by
	  * newer entries, and reusing one put it back at the top.
	  */
	UPROPERTY(EditAnywhere, config, Category = General, meta = (ClampMin = 1))
	int32 MaxRecentEntries = 20;

private:
	/** Higher index means more recent */
	UPROPERTY(config)
	TArray<FString> RecentMetaSoundsGraphActions;
	
	UPROPERTY(config)
	TSet<FString> FavoriteMetaSoundsGraphActions; // TODO: Implement profiles, like for blueprint favorites

public:
	void AddRecentlyUsedMetaSoundsGraphAction(FString&& ActionKey);
	int32 FindRecentlyUsedMetaSoundsGraphAction(const FString& ActionKey) const;
	void AddFavoriteMetaSoundsGraphAction(FString&& ActionKey);
	void RemoveFavoriteMetaSoundsGraphAction(const FString& ActionKey);
	bool IsFavoriteMetaSoundsGraphAction(const FString& ActionKey) const;
