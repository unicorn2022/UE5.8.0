...
namespace Metasound::Editor
{
			...
			auto CreateVersioningExecutionLambda = [Context](bool bRecursePaths) -> FExecuteAction
			{
				return FExecuteAction::CreateLambda([FolderPaths = Context->GetSelectedPackagePaths(), bRecursePaths]()
				{
					...
					if (!Results.DocumentsFoundInPackages())
					{
						...
						UE_LOGF(LogMetasoundEditor, Display, "No MetaSound(s) Versioned: Folder/child folder(s) contain(s) no MetaSound asset(s).");
					}
					else if (Results.PackagesToReserialize.IsEmpty())
					{
						...
						UE_LOGF(LogMetasoundEditor, Display, "No MetaSound(s) Versioned: Folder/child folder(s) contain(s) no MetaSound asset(s) requiring versioning.");
					}
					else if (!Results.FailedPackages.IsEmpty())
					{
						...
						UE_LOGF(LogMetasoundEditor, Display, "%i MetaSound(s) Reserialized but '%i' failed: See output log for details.", Results.PackagesToReserialize.Num(), Results.FailedPackages.Num());
					...
					else
					{
						...
						UE_LOGF(LogMetasoundEditor, Display, "%i MetaSound(s) Successfully Reserialized.", Results.PackagesToReserialize.Num());
						FEditorFileUtils::PromptToCheckoutPackages(false, Results.PackagesToReserialize);
					}
	...
	public:
			...
			: FSlateStyleSet("MetaSoundStyle")
		{
			...
			// Metasound Editor
			{
				...
				// Analyzers
				Set("MetasoundEditor.Analyzers.BackgroundColor", FLinearColor(0.0075f, 0.0075f, 0.0075f, 1.0f));
				...
				Set("MetasoundEditor.InterfacesInOut.Background", new FSlateBoxBrush(FAppStyle::Get().GetContentRootDir() / TEXT("Common/RoundedSelection_16x.png"), 4.0f / 16.0f, FLinearColor(1.f, 1.f, 1.f, 0.1f)));
				...
				Set("MetasoundEditor.Metasound.Node.Icon", new IMAGE_BRUSH_SVG(TEXT("Icons/metasound"), Icon20x20));
	...
	class FModule : public IMetasoundEditorModule
	{
		...
		virtual void RegisterPinType(FName InDataTypeName, FName InPinCategory, FName InPinSubCategory, const FSlateBrush* InPinConnectedIcon = nullptr, const FSlateBrush* InPinDisconnectedIcon = nullptr) override
		{
			...
			Params.PinCategory = InPinCategory;
			Params.PinSubCategory = InPinSubCategory;
			Params.PinConnectedIcon = InPinConnectedIcon;
			Params.PinDisconnectedIcon = InPinDisconnectedIcon;
			GraphPanelPinFactory->RegisterPin(InDataTypeName, Params);
		}
		...
		{
			return GraphPanelPinFactory->GetCustomPinIcons(InPin, PinConnectedIcon, PinDisconnectedIcon);
		}
		...
		{
			return GraphPanelPinFactory->GetCustomPinIcons(InDataType, PinConnectedIcon, PinDisconnectedIcon);
		}

		virtual const FEdGraphPinType* FindPinType(FName InDataTypeName) const override
		{
			return GraphPanelPinFactory->FindPinType(InDataTypeName);
		}

		virtual TSharedRef<IMetaSoundGraphPanelPinFactory> GetGraphPanelPinFactory() const override
		{
			return StaticCastSharedRef<IMetaSoundGraphPanelPinFactory>(GraphPanelPinFactory.ToSharedRef());
		}
		...
		virtual void StartupModule() override
		{
			...
			RegisterCustomNodeConfigurationDetailsCustomization(
				FMetaSoundDynamicInterfaceNodeConfiguration::StaticStruct()->GetFName(),
				[](TSharedPtr<IPropertyHandle> InStructProperty, TWeakObjectPtr<UMetasoundEditorGraphNode> InNode)
				{
					return MakeShareable(new FDynamicInterfaceNodeConfigurationCustomization(InStructProperty, InNode));
				});
			RegisteredCustomNodeConfigurationDetails.Add(FMetaSoundDynamicInterfaceNodeConfiguration::StaticStruct()->GetFName());
			...
			// Register MetaSound Input Browser nomad tab
			FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
				SMetasoundInputBrowser::TabId,
				FOnSpawnTab::CreateLambda([](const FSpawnTabArgs&) -> TSharedRef<SDockTab>
				{
					TSharedRef<SMetasoundInputBrowser> Browser = SNew(SMetasoundInputBrowser);
					return SNew(SDockTab)
						.TabRole(ETabRole::NomadTab)
						.Label(LOCTEXT("MetaSoundInputBrowserTabTitle", "MetaSound Input Browser"))
						[
							Browser
						];
				}))
				.SetDisplayName(LOCTEXT("MetaSoundInputBrowserDisplayName", "MetaSound Input Browser"))
				.SetTooltipText(LOCTEXT("MetaSoundInputBrowserTooltip", "Bulk view and edit MetaSound input values across multiple assets"))
		...
		virtual void ShutdownModule() override
		{
			...
			FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(SMetasoundInputBrowser::TabId);
			...
			for (const FName& NodeConfigStructName : RegisteredCustomNodeConfigurationDetails)
			{
				UnregisterCustomNodeConfigurationDetailsCustomization(NodeConfigStructName);
			}
		...
		// Captures the node configuration details registered from this module in 
		// order to track unregistration at module shutdown. It does not capture 
		// all custom registrations;
		TSet<FName> RegisteredCustomNodeConfigurationDetails;
