...
FLinearColor UMetasoundEditorGraphInputNode::GetNodeTitleTextColor() const
{
...
const FSlateBrush* UMetasoundEditorGraphInputNode::GetNodeTitleIconBrush() const
{
	const FSlateIcon InputIcon = FSlateIcon("MetaSoundStyle", "MetasoundEditor.Graph.Node.Class.Input");
	return InputIcon.GetIcon();
}
...
void UMetasoundEditorGraphInputNode::Validate(const FMetaSoundFrontendDocumentBuilder& InBuilder, Metasound::Editor::FGraphNodeValidationResult& OutResult) const
{
	...
	if (const UMetasoundEditorGraphVertex* Vertex = Cast<UMetasoundEditorGraphVertex>(GetMember()))
	{
		...
		{
			FText RequiredText;
			// This function checks there's a node output connected to the inspected Input, so it calls IsMemberInputRequired and considers what Outputs are connected.
			if (InterfaceToValidate.IsMemberInputRequired(Vertex->GetMemberName(), RequiredText))
			{
