...
namespace Metasound
{
	namespace Editor
	{
		...
		void FGraphConnectionDrawingPolicy::DetermineWiringStyle(UEdGraphPin* InPinA, UEdGraphPin* InPinB, FConnectionParams& OutParams)
		{
			...
			{
				OutParams.WireColor = FMetaSoundGraphPanelPinFactory::GetChecked()->GetPinColor(AnyPin->PinType);
			}
					...
					if (const FMetaSoundFrontendDocumentBuilder* Builder = EditorPtr->GetFrontendBuilder())
					{
						if (ensure(Builder->IsValid()))
						{
							...
							if (const FMetasoundFrontendVertex* OutputVertex = FGraphBuilder::GetPinVertex(*Builder, ReroutePin, &OutputNode))
							{
								...
								if (AnyPin->PinType.PinCategory == FMetaSoundGraphPanelPinFactory::PinCategoryTrigger)
								{
								...
								else if (AnyPin->PinType.PinCategory == FMetaSoundGraphPanelPinFactory::PinCategoryAudio)
								{
									FGraphConnectionManager& ConnectionManager = EditorPtr->GetConnectionManager();
		...
		void FGraphConnectionDrawingPolicy::DrawConnection(int32 LayerId, const FVector2f& Start, const FVector2f& End, const FConnectionParams& Params)
		{
			...
			const FDrawConnectionData ConnectionData(Start, End, *EditorPtr, FGraphConnectionDrawingPolicy::ComputeSplineTangent(Start, End), Params, *Settings, AbsoluteMousePosition);
