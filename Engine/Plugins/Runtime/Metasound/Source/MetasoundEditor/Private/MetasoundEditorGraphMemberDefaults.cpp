						...
						if (const FMetaSoundPageSettings* PageSettings = PageSettingsByID.FindRef(Default.PageID))
						{
							if (const FMetasoundFrontendLiteral* FrontendLiteral = FrontendDefaults.FindRef(Default.PageID))
							{
								...
								if (!TestLiteral.IsEqual(Value))
								{
									...
									{
										UE_LOGF(LogMetasoundEditor, Verbose, "Synchronizing Page Default: Setting member '%ls' (type '%ls') to literal value '%ls'. Type has changed or literal could not be set and will be set to type's default constructed value.",
...
bool UMetasoundEditorGraphMemberDefaultFloat::MigrateProperties()
{
	using namespace Metasound::Editor;
	...
	// Property migration occurs before document versioning, so check if either is true to
	// ensure widget type is cleared irrespective of document version.
	bool bIsPreset = Document.RootGraph.PresetOptions.bIsPreset;
	if (!bIsPreset)
	{
		const UStruct* Struct = Document.Template.GetScriptStruct();
		bIsPreset = Struct && Struct->IsChildOf(FPresetTemplate::StaticStruct());
	}
