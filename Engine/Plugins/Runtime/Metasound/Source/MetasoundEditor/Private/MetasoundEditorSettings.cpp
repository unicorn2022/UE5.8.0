...
void UMetasoundEditorSettings::AddRecentlyUsedMetaSoundsGraphAction(FString&& ActionKey)
{
	const int32 ExistingIndex = RecentMetaSoundsGraphActions.Find(ActionKey);
	if (ExistingIndex != INDEX_NONE)
	{
		RecentMetaSoundsGraphActions.RemoveAt(ExistingIndex, EAllowShrinking::No);
	}
	else if (RecentMetaSoundsGraphActions.Num() >= MaxRecentEntries)
	{
		RecentMetaSoundsGraphActions.RemoveAt(0, RecentMetaSoundsGraphActions.Num() - MaxRecentEntries + 1, EAllowShrinking::No);
	}
	RecentMetaSoundsGraphActions.Emplace(ActionKey);

	SaveConfig();
}

int32 UMetasoundEditorSettings::FindRecentlyUsedMetaSoundsGraphAction(const FString& ActionKey) const
{
	return RecentMetaSoundsGraphActions.Find(ActionKey);
}

void UMetasoundEditorSettings::AddFavoriteMetaSoundsGraphAction(FString&& ActionKey)
{
	bool bAlreadyFavorite = false;
	FavoriteMetaSoundsGraphActions.Emplace(ActionKey, &bAlreadyFavorite);
	if (!bAlreadyFavorite)
	{
		SaveConfig();
	}
...
void UMetasoundEditorSettings::RemoveFavoriteMetaSoundsGraphAction(const FString& ActionKey)
{
	if (FavoriteMetaSoundsGraphActions.Remove(ActionKey) > 0)
	{
		SaveConfig();
	}
}

bool UMetasoundEditorSettings::IsFavoriteMetaSoundsGraphAction(const FString& ActionKey) const
{
	return FavoriteMetaSoundsGraphActions.Contains(ActionKey);
}
...
void UMetasoundEditorSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(UMetasoundEditorSettings, MaxRecentEntries))
	{
		MaxRecentEntries = FMath::Min(MaxRecentEntries, MaxRecentEntriesEver);
	}
}
