...
namespace Metasound::Editor
{
	...
	enum class EPrimaryContextGroup : uint8
	{
		...
		// This MUST be the last one before Common, because the space between them is reserved for Recent actions ordered by most recent.
		Recents, 
		Common = 255
	};
	...
	public:
		...
		virtual FMetasoundFrontendClassMetadata GenerateClassMetadata() const = 0;
	...
	{
		UEdGraphNode* PromoteToInput(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const UE::Slate::FDeprecateVector2DParameter& InLocation, bool bSelectNewNode, TOptional<FMetasoundFrontendLiteral> Literal = {});
		...
		const FName MetasoundGraphSchemaActionType("FMetasoundGraphSchemaAction");
	} // namespace SchemaUtils
} // namespace Metasound::Editor
...
struct FMetasoundGraphSchemaAction : public FEdGraphSchemaAction
{
	...
	static FName StaticGetTypeId() { return Metasound::Editor::SchemaUtils::MetasoundGraphSchemaActionType; }
	...
	// Used to store & match Recent and Favorite MetaSounds graph actions in the MetaSounds editor
	virtual FString GetUniqueStableFavoriteKey() const
	{
		// Unlikely: If the user changes the editor locale, favorites and recents could technically be invalidated if their display names are localized.
		return GetMenuDescription().ToString();
	}
};
...
/** Action to add an input node of an existing graph input to the graph */
USTRUCT()
struct FMetasoundGraphSchemaAction_NewInput : public FMetasoundGraphSchemaAction_NodeWithMultipleOutputs
{
	...
	virtual FString GetUniqueStableFavoriteKey() const override
	{
...
struct FMetasoundGraphSchemaAction_NewGraphMember : public FMetasoundGraphSchemaAction_NodeWithMultipleOutputs
{
	...
	UE_API FMetasoundGraphSchemaAction_NewGraphMember(FName InDataType, FMetasoundFrontendLiteral InLiteral, FText InNodeCategory, FText InMenuDesc, FText InToolTip, Metasound::Editor::EPrimaryContextGroup InGroup)
		: FMetasoundGraphSchemaAction_NodeWithMultipleOutputs(InNodeCategory, InMenuDesc, InToolTip, InGroup)
		, DataType(InDataType)
		, DefaultLiteral(InLiteral)
	{}

	virtual ~FMetasoundGraphSchemaAction_NewGraphMember() = default;
...
/** Action to add a new graph input and input node to the graph, with the given default literal */
USTRUCT()
struct FMetasoundGraphSchemaAction_NewGraphInput : public FMetasoundGraphSchemaAction_NewGraphMember
{
	GENERATED_BODY();
	...
	UE_API FMetasoundGraphSchemaAction_NewGraphInput(FName DataType, FMetasoundFrontendLiteral InLiteral);

	virtual ~FMetasoundGraphSchemaAction_NewGraphInput() = default;
...
/** Action to add a new graph output and output node to the graph, with the given default literal */
USTRUCT()
struct FMetasoundGraphSchemaAction_NewGraphOutput : public FMetasoundGraphSchemaAction_NewGraphMember
{
	GENERATED_BODY();
	...
	UE_API FMetasoundGraphSchemaAction_NewGraphOutput(FName DataType, FMetasoundFrontendLiteral InLiteral);

	virtual ~FMetasoundGraphSchemaAction_NewGraphOutput() = default;
...
/** Action to add a new graph variable and getter node to the graph, with the given default literal */
USTRUCT()
struct FMetasoundGraphSchemaAction_NewGraphVariable : public FMetasoundGraphSchemaAction_NewGraphMember
{
	GENERATED_BODY();
	...
	UE_API FMetasoundGraphSchemaAction_NewGraphVariable(FName DataType, FMetasoundFrontendLiteral InLiteral);

	virtual ~FMetasoundGraphSchemaAction_NewGraphVariable() = default;
...
struct FMetasoundGraphSchemaAction_NewNode : public FMetasoundGraphSchemaAction_NodeWithMultipleOutputs
{
	...
	FMetasoundGraphSchemaAction_NewNode(const FText& InNodeCategory, const FText& InMenuDesc, const FText& InToolTip, Metasound::Editor::EPrimaryContextGroup InGroup, FText InKeywords = FText::GetEmpty(), TOptional<Metasound::Editor::FDroppedObjectsInfo> InDroppedObjectsInfo = {})
		: FMetasoundGraphSchemaAction_NodeWithMultipleOutputs(InNodeCategory, InMenuDesc, InToolTip, InGroup, InKeywords)
		, DroppedObjectsInfo(InDroppedObjectsInfo)
	{}
	...
	UE_API virtual FString GetUniqueStableFavoriteKey() const override;
	//~ End FMetasoundGraphSchemaAction Interface
	...
	//~ End FEdGraphSchemaAction Interface
private: 
	TOptional<Metasound::Editor::FDroppedObjectsInfo> DroppedObjectsInfo;
};
...
private:
	void QueryNodeClasses(TFunctionRef<void(TUniquePtr<Metasound::Editor::ISchemaQueryResult>&&)> OnClassFound) const;
	...
	void GetFunctionActions(FGraphActionMenuBuilder& ActionMenuBuilder, Metasound::Editor::FActionVertexFilters InFilters = Metasound::Editor::FActionVertexFilters(), bool bShowSelectedActions = true, Metasound::Frontend::FConstGraphHandle InGraphHandle = Metasound::Frontend::IGraphController::GetInvalidHandle(), const UEdGraph* CurrentGraph = nullptr) const;
	...
	// Process dropped assets
	void DroppedMetaSoundAssetsOnGraph(const TArray<FMetasoundAssetBase*>& MetaSoundAssets, const FVector2f& GraphPosition, UMetasoundEditorGraph* MetaSoundGraph) const;
	void DroppedNonMetaSoundAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UMetasoundEditorGraph* MetaSoundGraph) const;

	// Validate dropped assets, returning true if valid
	bool ValidateDroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const UEdGraph* HoverGraph, FString* OutTooltipText = nullptr) const;
	bool ValidateDroppedAssetsOnPin(const TArray<FAssetData>& Assets, const UEdGraphPin* Pin, FString* OutTooltipText = nullptr) const;
