...
public:
		...
		, TileMaterial(Component->TileMaterial)
	{
			...
			[this] (FRHICommandList& RHICmdList)
		{
			...
			{
				CreateStaticRayTracingGeometry(RHICmdList);
	...
	void SetDynamicData_RenderThread(FRHICommandListBase& RHICmdList, FCableDynamicData* NewDynamicData)
	{
		if (NewDynamicData != nullptr)
		{
			...
			if (StaticRayTracingGeometry.IsInitialized())
			{
				...
				{
					StaticRayTracingGeometry.CreateRayTracingGeometry(RHICmdList, ERTAccelerationStructureBuildPriority::Normal);
				}
				else if(!StaticRayTracingGeometry.HasPendingBuildRequest())
				{
					StaticRayTracingGeometry.SetRequiresBuild(true);
					StaticRayTracingGeometry.RequestBuildIfNeeded(RHICmdList, ERTAccelerationStructureBuildPriority::Normal);
				}
	...
	virtual void GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector) override
	{
		...
		if (!bEvaluateWPO)
		{
			...
			if (StaticRayTracingGeometry.IsEvicted())
			{
				Collector.AddReferencedGeometry(StaticRayTracingGeometry.GetGeometryHandle());
				return;
			}
		}
		...
		if (Geometry.GetInitializer().TotalPrimitiveCount <= 0)
		{
			return;
		}
		...
		if (bEvaluateWPO && VertexFactory.GetType()->SupportsRayTracingDynamicGeometry())
		{
...

			{
				...
				UpdateParams.NumVertices = (uint32)VertexCount;
				UpdateParams.VertexBufferSize = uint32((SIZE_T)VertexCount * sizeof(FVector3f));
		...
		checkf(RayTracingInstance.Geometry->GetInitializer().Segments.Num() == CachedRayTracingMaterials.Num(), TEXT("Segments/Materials mismatch. Number of segments: %d. Number of Materials: %d."),
	...
	void CreateStaticRayTracingGeometry(FRHICommandListBase& RHICmdList)
	{
		...
		StaticRayTracingGeometry.SetInitializer(Initializer);
		StaticRayTracingGeometry.InitResource(RHICmdList);
	}
#endif
...
private:
	...
	{
		FRayTracingGeometryInitializer Initializer = StaticRayTracingGeometry.GetInitializer();
...
void UCableComponent::CollectPSOPrecacheData(const FPSOPrecacheParams& BasePrecachePSOParams, FMaterialInterfacePSOPrecacheParamsList& OutParams)
{
	FMaterialInterfacePSOPrecacheParams Params;
	Params.PSOPrecacheParams = BasePrecachePSOParams;
	Params.PSOPrecacheParams.bReverseCulling |= GetRenderMatrix().Determinant() < 0.f;
	//Matches the logic in FCableSceneProxy
	Params.VertexFactoryDataList.Add(&FLocalVertexFactory::StaticType);
	Params.MaterialInterface = GetMaterial(0);
	if (!Params.MaterialInterface)
	{
		Params.MaterialInterface = UMaterial::GetDefaultMaterial(MD_Surface);
	}
