...
void UCommonInputSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	if (FSlateApplication::IsInitialized())
	{
		if (GenericApplication* GenericApplication = FSlateApplication::Get().GetPlatformApplication().Get())
		{
			if (!GenericApplication->OnVirtualKeyboardShown().IsBoundToObject(this))
			{
				GenericApplication->OnVirtualKeyboardShown().AddUObject(this, &ThisClass::HandleOnVirtualKeyboardShown);
			}

			if (!GenericApplication->OnVirtualKeyboardHidden().IsBoundToObject(this))
			{
				GenericApplication->OnVirtualKeyboardHidden().AddUObject(this, &ThisClass::HandleOnVirtualKeyboardHidden);
			}
		}
	}
}
...
void UCommonInputSubsystem::Deinitialize()
{
	...
	if (FSlateApplication::IsInitialized())
	{
		if (GenericApplication* GenericApplication = FSlateApplication::Get().GetPlatformApplication().Get())
		{
			GenericApplication->OnVirtualKeyboardShown().RemoveAll(this);
			GenericApplication->OnVirtualKeyboardHidden().RemoveAll(this);
		}
	}
}
...
void UCommonInputSubsystem::AddOrRemoveInputTypeLock(FName InReason, ECommonInputType InInputType, bool bAddLock)
{
	...
	if (bAddLock && PlatformSupportsInputType(InInputType))
	{
		...
		UE_LOGF(LogCommonInput, Log, "Adding Input Method Lock: %ls - %d", *InReason.ToString(), (int32)InInputType);
	}
	else
	{
		CurrentInputLocks.Remove(InReason);

		UE_LOGF(LogCommonInput, Log, "Removing Input Method Lock: %ls - %d", *InReason.ToString(), (int32)InInputType);
	}
...
void UCommonInputSubsystem::SetIsGamepadSimulatedClick(bool bNewIsGamepadSimulatedClick)
{
	if (bIsGamepadSimulatedClick != bNewIsGamepadSimulatedClick)
	{
		...
		UE_CLOGF(bIsGamepadSimulatedClick, LogCommonInput, VeryVerbose, "UCommonInputSubsystem::SetIsGamepadSimulatedClick(): Click is being simulated");
	}
}
