...
private:
	...
	/** Additional keys that also trigger this action (the primary Key is still used for display) */
	UPROPERTY(EditAnywhere, Category = "CommonInput")
...
public:
	...
	/** Get all keys (primary + additional) that trigger this action */
	UE_API TArray<FKey> GetAllKeys() const;

	/** Returns true if the given key matches the primary key or any additional key */
	UE_API bool IsKeyBound(const FKey& InKey) const;
