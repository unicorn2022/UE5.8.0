...
#include "Input/CommonAnalogCursorTypes.h"
...
DECLARE_EVENT_OneParam(FCommonAnalogCursor, FVirtualPointerEnabledChanged, bool);
...
public:
	...
	UE_API TWeakPtr<SWidget> GetLastCursorTarget() const;

	UE_API bool IsUsingFakeTouch() const;
	UE_API bool IsVirtualPointerEnabled() const;
	UE_API bool IsUsingVirtualPointer() const;
	UE_API void SetVirtualPointerVisibility(bool bVisible);

	UE_API FVirtualPointerEnabledChanged& GetOnVirtualPointerEnabledChanged();

	virtual void AddReferencedObjects(FReferenceCollector& Collector) override
	{
		Collector.AddReferencedObject(DefaultVirtualPointerWidget);
		Collector.AddReferencedObject(VirtualPointerWidget);
	}
	virtual FString GetReferencerName() const override { return TEXT("FCommonAnalogCursor"); }
...
protected:
	...
	bool bEnableCommonUIVirtualPointer = false;
	bool bIsAnalogMovementEnabled = false;
	
	bool bShouldHandleRightAnalog = true;

	FVirtualPointerEnabledChanged OnVirtualPointerEnabledChanged;

private:

	UE_API void HandleOnEnableVirtualPointerChanged(IConsoleVariable* ConsoleVariable);
	UE_API void UpdateInputPreProcessor();
	UE_API TObjectPtr<UUserWidget> GetOrLoadDefaultVirtualPointer();
	UE_API void OnDefaultVirtualPointerClassLoaded();
	UE_API void UpdateVirtualPointerVisualState();
	...
	TObjectPtr<UUserWidget> DefaultVirtualPointerWidget = nullptr;
	TObjectPtr<UUserWidget> VirtualPointerWidget = nullptr;

	/** In-flight async load handle for the default virtual pointer class. Reset once load completes or is cancelled. */
	TSharedPtr<FStreamableHandle> DefaultVirtualPointerLoadHandle;
	/** Set when the configured class fails to resolve so we don't keep firing async loads every frame. */
	bool bDefaultVirtualPointerLoadFailed = false;

	float TimeUntilScrollUpdate = 0.f;

	/** UE-380247: was the last cursor input a gamepad stick? */
	bool bLastCursorInputWasGamepad = false;
	/** Swallow the synthetic mouse-move that follows our SetCursorPosition. */
	bool bIgnoreNextMouseMove = false;

	bool bShowVirtualPointer = false;
	bool bLastBroadcastVirtualPointerEnabled = false;

	/** Skips per-tick UUserWidget::TakeWidget() rebuilds in RefreshCursorVisibility when unchanged. */
	TWeakObjectPtr<UUserWidget> LastAppliedSoftwareCursorWidget;
	ECursorVisualState CurrentVirtualPointerVisualState = ECursorVisualState::Default;
	bool bVirtualAcceptDown = false;
	float HoldElapsedTime = 0.f;
