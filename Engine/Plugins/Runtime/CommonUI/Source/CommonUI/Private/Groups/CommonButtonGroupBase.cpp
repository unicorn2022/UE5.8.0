...
void UCommonButtonGroupBase::SelectNextButton(bool bAllowWrap /*= true*/, const bool bAllowSound /*= true*/)
{
	...
	std::function<void(int32, bool)> SelectNextButtonRecursive = [&](int32 SelectionIndex, bool bAllowWrap)
	{
		...
		if (Buttons.IsValidIndex(SelectionIndex))
		{
			...
			{
				SelectButtonAtIndex(SelectionIndex, bAllowSound);
			}
			else
			{
...
void UCommonButtonGroupBase::SelectPreviousButton(bool bAllowWrap /*= true*/, const bool bAllowSound /*= true*/)
{
	const int32 NumButtons = Buttons.Num();
	int32 NumRecursiveSelections = 0;
...
{
	UCommonButtonBase* PreviouslySelectedButton = GetButtonBaseAtIndex(SelectedButtonIndex);

	if (bIsSelected)
	{
		// Reset index to avoid stale leakage
		SelectedButtonIndex = INDEX_NONE;
		...
		// If the button isn't in the group, do not broadcast a stale state
		if (!bFoundButton)
		{
			return;
		}
		...
		OnSelectedButtonLostSelection.Broadcast(PreviouslySelectedButton);
	}
	else
	{
		...
		PreviouslySelectedButton = GetButtonBaseAtIndex(SelectedButtonIndex);
		OnSelectedButtonLostSelection.Broadcast(PreviouslySelectedButton);
