...
void UCommonActivatableWidgetContainerBase::RemoveWidget(UCommonActivatableWidget& WidgetToRemove)
{
	UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase::RemoveWidget() WidgetToRemove: %ls", *WidgetToRemove.GetName());
...
{
	UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase::SetSwitcherIndex() TargetIndex: %d, bInstantTransition: %d", TargetIndex, bInstantTransition);
...
{
	UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase::RegisterInstanceInternal() NewWidget: %ls", *NewWidget.GetName());
...
{
	UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase::HandleSwitcherIsTransitioningChanged() bIsTransitioning: %d", bIsTransitioning);
...
{
	UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase::HandleActiveWidgetDeactivated() DeactivatedWidget: %ls", *DeactivatedWidget->GetName());
...
{
	UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase::ReleaseWidget()");
	
	if (UCommonActivatableWidget* ActivatableWidget = ActivatableWidgetFromSlate(WidgetToRelease))
	{
		UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase WidgetToRelease: %ls", *ActivatableWidget->GetName());
	...
	{
		UE_LOGF(LogCommonUI, Warning, "UCommonActivatableWidgetContainerBase::ReleaseWidget() No matching Activatable Widget found.");
	}

	const int32 RemovedIndex = MySwitcher->RemoveSlot(WidgetToRelease);
	if (RemovedIndex != INDEX_NONE)
	{
		UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase Widget removed from slot %d", RemovedIndex);
...
{
	UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetContainerBase::HandleActiveIndexChanged() ActiveWidgetIndex: %d", ActiveWidgetIndex);
...
void UCommonActivatableWidgetQueue::OnWidgetAddedToList(UCommonActivatableWidget& AddedWidget)
{
	if (MySwitcher)
	{
		UE_LOGF(LogCommonUI, VeryVerbose, "UCommonActivatableWidgetQueue::OnWidgetAddedToList() PriorNumWidgets: %d", MySwitcher->GetNumWidgets() - 1);
