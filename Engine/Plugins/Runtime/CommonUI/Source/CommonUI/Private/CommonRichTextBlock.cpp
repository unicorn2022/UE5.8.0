...
public:
	...
	virtual TSharedRef<ISlateRun> Create(const TSharedRef<class FTextLayout>& TextLayout, const FTextRunParseResults& RunParseResult, const FString& OriginalText, const TSharedRef<FString>& InOutModelText, const ISlateStyle* Style) override final
	{
		...
		if (CommonUIUtils::ShouldScaleText() && UE::CommonUI::CommonRichTextBlock::bApplyMobileScaleOnTextRuns)
		{
			float Scale = 1.0f;
			if (CommonUIUtils::ShouldDisplayMobileUISizes())
			{
				Scale = Owner->GetMobileTextBlockScale();
			}
			else if (CommonUIUtils::UISmallTextScaling > 0.0f)
			{
				if (TextStyle.Font.Size <= CommonUIUtils::UISmallTextScalingThresholdMax)
				{
					Scale = CommonUIUtils::UISmallTextScaling;
				}
			}
			else
			{
			...
			TextStyle.SetFontSize(TextStyle.Font.Size * Scale);
		}
...
private:
	TSharedRef<SWidget> CreateIconWidget(const FInlineIconEntry& Entry, const FTextBlockStyle& TextStyle, float IconScale, bool bTintIcon)
	{
		...
		if (Entry.ResourceObject.IsNull())
		{
...
void UCommonRichTextBlock::SetText(const FText& InText)
{
	...
	{
		if (CommonUIUtils::ShouldScaleText())
		{
			ApplyTextBlockScale();
		}
...
void UCommonRichTextBlock::SynchronizeProperties()
{
	...
	if (MyRichTextBlock.IsValid())
	{
		if (CommonUIUtils::ShouldScaleText() && DefaultTextStyleOverrideClass.GetDefaultObject() == nullptr)
		{
			ApplyTextBlockScale();
		}
	}
...
void UCommonRichTextBlock::ApplyTextBlockScale() const
{
	if (MyRichTextBlock.IsValid())
	{
		...
		if (CommonUIUtils::ShouldDisplayMobileUISizes())
		{
			Scale = GetMobileTextBlockScale();
		}
		else if (CommonUIUtils::UISmallTextScaling > 0.0f)
		{
			if (GetCurrentDefaultTextStyle().Font.Size <= CommonUIUtils::UISmallTextScalingThresholdMax)
			{
				Scale = CommonUIUtils::UISmallTextScaling;
			}
		}
		...
		MyRichTextBlock->SetTextBlockScale(Scale);
	}
}
