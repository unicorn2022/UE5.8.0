...
static const TAutoConsoleVariable<bool> CVarSyncCursorToFocusedWidgetOnInputMethodChange(
	TEXT("CommonUI.SyncCursorToFocusedWidgetOnInputMethodChange"),
	true,
	TEXT("Whether or not to move the cursor to the currently focused widget when input method changes to gamepad (fix for losing focus when switching between touch and gamepad inputs)."),
	ECVF_Default);

static const TAutoConsoleVariable<bool> CVarCheckKeyboardFocusAndParentage(
	TEXT("CommonUI.CheckKeyboardFocusAndParentage"),
	false,
	TEXT("If enabled, we will check for keyboard focus and parentage before setting focus"));

static const TAutoConsoleVariable<bool> CVarFallbackToDesiredOnAutoRestoreFailure(
	TEXT("CommonUI.FallbackToDesiredOnAutoRestoreFailure"),
	false,
	TEXT("If enabled, we set focus to Desired Widget if Auto Restore Focus Fails"));

static const TAutoConsoleVariable<bool> CVarDisallowUserFocusedWidgetForPendingFocusRecipient(
	TEXT("CommonUI.DisallowUserFocusedWidgetForPendingFocusRecipient"),
	false,
	TEXT("If enabled, we only check the currently focused widget when there is no pending focus recipient"));

static bool bEnhancedInputDynamicallyResolveHoldKeysForInputAction = false;
static FAutoConsoleVariableRef CVarEnhancedInputDynamicallyResolveHoldKeysForInputAction(
	TEXT("CommonUI.bEnhancedInputDynamicallyResolveHoldKeysForInputAction"),
	bEnhancedInputDynamicallyResolveHoldKeysForInputAction,
	TEXT("If enabled, triggering keys assigned to InputAction for hold triggers will be resolved dynamically during key press processing (this allows remapped keys)."));
	
static bool bAlwaysWaitForHoldActionToComplete = false;
static FAutoConsoleVariableRef CVarAlwaysWaitForHoldActionToComplete(
	TEXT("CommonUI.AlwaysWaitForHoldActionToComplete"),
	bAlwaysWaitForHoldActionToComplete,
	TEXT("If enabled, always wait for input hold time to reach triggering threshold before generating a press event (otherwise a press event will be triggered before threshold is reached if press and quick release key)."));
	...
	, InputAction(BindArgs.InputAction)
{
	...
	else
	{
		...
		// Register additional keys for each input type
		auto RegisterAdditionalKeysForTypeInfo = [&](const FCommonInputTypeInfo& TypeInfo, float ForceHoldTime)
		{
			const float HoldTime = TypeInfo.bActionRequiresHold ? TypeInfo.HoldTime : ForceHoldTime;
			const float RollbackTime = TypeInfo.bActionRequiresHold ? TypeInfo.HoldRollbackTime : 0.f;
			for (const FKey& AdditionalKey : TypeInfo.GetAllKeys())
			{
				if (AdditionalKey != TypeInfo.GetKey())
				{
					RegisterKeyMappingFunc(FUIActionKeyMapping(AdditionalKey, HoldTime, RollbackTime));
				}
			}
		};

		float ForceHoldTime_KBM = 0.f;
		float ForceHoldTime_Touch = 0.f;
		float ForceHoldTime_Gamepad = 0.f;
		if (BindArgs.bForceHold && ICommonInputModule::GetSettings().GetDefaultHoldData())
		{
			UCommonUIHoldData* CommonUIHoldBehaviorValues = ICommonInputModule::GetSettings().GetDefaultHoldData().GetDefaultObject();
			ForceHoldTime_KBM = CommonUIHoldBehaviorValues->KeyboardAndMouse.HoldTime;
			ForceHoldTime_Touch = CommonUIHoldBehaviorValues->Touch.HoldTime;
			ForceHoldTime_Gamepad = CommonUIHoldBehaviorValues->Gamepad.HoldTime;
		}

		RegisterAdditionalKeysForTypeInfo(LegacyMapping_KBM, ForceHoldTime_KBM);
		RegisterAdditionalKeysForTypeInfo(LegacyMapping_Touch, ForceHoldTime_Touch);
		RegisterAdditionalKeysForTypeInfo(LegacyMapping_Gamepad, ForceHoldTime_Gamepad);
	}
...
FUIActionBindingHandle FUIActionBinding::TryCreate(const UWidget& InBoundWidget, const FBindUIActionArgs& BindArgs, int32 UserIndex)
{
	...
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot create action binding for widget [%ls] - no action provided.", *InBoundWidget.GetName());
		return FUIActionBindingHandle();
	}
	else if (!BindArgs.OnExecuteAction.IsBound())
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot bind widget [%ls] to action [%ls] - empty handler delegate.", *InBoundWidget.GetName(), *BindActionName.ToString());
		return FUIActionBindingHandle();
	}
	else if (BindArgs.ActionTag.IsValid() && !UCommonUIInputSettings::Get().FindAction(BindArgs.ActionTag))
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot bind widget [%ls] to action [%ls] - provided tag does not map to an existing UI input action. It can be added under Project Settings->UI Input.", *InBoundWidget.GetName(), *BindArgs.GetActionName().ToString());
		return FUIActionBindingHandle();
	}
	else if (!BindArgs.LegacyActionTableRow.IsNull() && !BindArgs.LegacyActionTableRow.GetRow<FCommonInputActionDataBase>(TEXT("")))
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot bind widget [%ls] to action [%ls] - provided legacy data table row does not resolve to valid data.", *InBoundWidget.GetName(), *BindActionName.ToString());
		return FUIActionBindingHandle();
	}
	...
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot bind widget [%ls] to action [%ls] - provided input action is invalid.", *InBoundWidget.GetName(), *BindActionName.ToString());
		return FUIActionBindingHandle();
	}
	else if (UserIndex == INDEX_NONE)
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot bind widget [%ls] to action [%ls] [%ls] - invalid UserIndex.", *BindActionName.ToString(), InputEventToString(BindArgs.KeyEvent), *InBoundWidget.GetName());
		return FUIActionBindingHandle();
	}
	...
	for (const TPair<FUIActionBindingHandle, TSharedPtr<FUIActionBinding>>& HandleBindingPair : AllRegistrationsByHandle)
	{
		...
		{
			UE_LOGF(LogUIActionRouter, Error, "Widget [%ls] is already bound to action [%ls] [%ls] for user [%d]! A given widget can only bind the same action once per user. Unregister existing binding first if you wish to change any aspect of the binding", 
...
void FUIActionBinding::CleanRegistrations()
{
	...
	if (NumRemoved > 0)
	{
		UE_LOGF(LogUIActionRouter, Log, "Cleaned out [%d] inactive UI action bindings", NumRemoved);
	}
}
...
void FUIActionBinding::BeginHold()
{
	...
	UE_LOGF(LogUIActionRouter, VeryVerbose, "Hold pressed: %ls", *ActionName.ToString());
...
bool FUIActionBinding::UpdateHold(float TargetHoldTime)
{
	...
	UE_LOGF(LogUIActionRouter, VeryVerbose, "Hold repeating: %ls for %f", *ActionName.ToString(), CurrentHoldSecond);
	...
	if (HeldPercent >= 1.f)
	{
		...
		UE_LOGF(LogUIActionRouter, VeryVerbose, "Hold repeating: Fired completed action! %ls after %f", *ActionName.ToString(), CurrentHoldSecond);
		return true;
	}
	return false;
}
...
EProcessHoldActionResult FActionRouterBindingCollection::ProcessHoldInput(ECommonInputMode ActiveInputMode, FKey Key, EInputEvent InputEvent, int32 UserIndex) const
{
	for (FUIActionBindingHandle BindingHandle : ActionBindings)
	{
		if (TSharedPtr<FUIActionBinding> Binding = FUIActionBinding::FindBinding(BindingHandle))
		{
			...
			{
				// Create dynamic key mappings. This also ensures that remapped keys are properly processed.
				if (CommonUI::IsEnhancedInputSupportEnabled() && bEnhancedInputDynamicallyResolveHoldKeysForInputAction && Binding->InputAction.IsValid())
				{
					Binding->HoldMappings.Empty();
					
					if (const UInputAction* InputAction = Binding->InputAction.Get())
					{
						// find hold trigger and corresponding keys for this action if a hold trigger is found
						for (const TObjectPtr<UInputTrigger>& Trigger : InputAction->Triggers)
						{
							...
							{
								if (HoldTrigger->HoldTimeThreshold <= 0.0f)
								{
									continue;
								}
								...
								{
									if (InputActionKey.IsSameResolvedKey(Key))
									{
										Binding->HoldMappings.Emplace(FUIActionKeyMapping(InputActionKey, HoldTrigger->HoldTimeThreshold, 0.f));
										break;
									}
								}
				...
				for (const FUIActionKeyMapping& HoldMapping : Binding->HoldMappings)
				{
					...
					if (HoldMapping.Key.IsSameResolvedKey(Key) && (bIsDisplayedPersistentAction || IsWidgetReachableForInput(Binding->BoundWidget.Get())))
					{
						...
						else if (Binding->IsHoldActive())
						{
							...
							else if (InputEvent == IE_Released)
							{
								...
								UE_LOGF(LogUIActionRouter, VeryVerbose, "Hold released: %ls after %f", *Binding->ActionName.ToString(), SecondsHeld);
								...
								if (!bAlwaysWaitForHoldActionToComplete)
								{
		...
		if (TSharedPtr<FUIActionBinding> Binding = FUIActionBinding::FindBinding(BindingHandle))
		{
			if (Binding->UserIndex == UserIndex && (ActiveInputMode == ECommonInputMode::All || ActiveInputMode == Binding->InputMode))
			{
				auto TryConsumeInput = [&](const FKey& InKey, const UInputAction* InInputAction)
				{
					// A persistent displayed action skips the normal rules for reachability, since it'll always appear in a bound action bar
					const bool bIsDisplayedPersistentAction = Binding->bIsPersistent && Binding->bDisplayInActionBar;
					if (InKey.IsSameResolvedKey(Key) && Binding->InputEvent == InputEvent && (bIsDisplayedPersistentAction || IsWidgetReachableForInput(Binding->BoundWidget.Get())))
					{
						UE_LOGF(LogUIActionRouter, VeryVerbose, "Process normal input: %ls (key: %ls)", *Binding->ActionName.ToString(), *Key.ToString());
...
{
#if !UE_BUILD_SHIPPING
	UE_CLOGF(!RepresentedWidget.IsValid(), LogUIActionRouter, Warning, 
		"Represented Widget not Valid: %ls - %ls", 
		*DebugWidgetName, 
...
void FActivatableTreeNode::CacheFocusRestorationTarget()
{
	...
	{
		UE_LOGF(LogUIActionRouter, VeryVerbose, "FocusRestorationTarget for [%ls] changed from [%ls] to [%ls]", *GetNameSafe(GetWidget()), *FReflectionMetaData::GetWidgetDebugInfo(FocusRestorationTarget.Pin().Get()), *FReflectionMetaData::GetWidgetDebugInfo(FocusedWidget.Get()));
...
void FActivatableTreeNode::DebugDumpRecursive(FString& OutputStr, int32 Depth, bool bIncludeActions, bool bIncludeChildren, bool bIncludeInactive) const
{
	...
	if (RepresentedWidget.IsValid())
	{
		...
		const int32 LayerId = CachedWidget.IsValid()
			? CachedWidget->GetPersistentState().LayerId
			: INDEX_NONE;

		OutputStr.Appendf(
			TEXT("\n%s%s: IsActivated? [%s]. LayerId=[%d]. [%d] Normal Bindings. SupportsActivationFocus [%s]"),
			FCString::Spc(DepthSpacing),
			*RepresentedWidget->GetName(),
			*LexToString(RepresentedWidget->IsActivated()),
			LayerId,
			ActionBindings.Num(),
			*LexToString(RepresentedWidget->SupportsActivationFocus())
		);
	}
	else
	{
...
bool FActivatableTreeRoot::UpdateLeafmostActiveNode(FActivatableTreeNodePtr BaseCandidateNode, bool bInApplyConfig)
{
	...
	if (!bValidBaseCandidate)
	{
		...
		UE_LOGF(LogUIActionRouter, Log,
			"[FActivatableTreeRoot::UpdateLeafmostActiveNode] Node for %ls is our base candidate, but isn't receiving input. This probably means its widget was Deactivated in response to the events in its NativeOnActivated: harmless if intentional.",
	...
	{
		TSharedPtr<SWidget> OldLeafmostSlateWidget;
		...
		if (LeafmostActiveNode.IsValid())
		{
			...
			{
				OldLeafmostSlateWidget = LeafmostActivatableWidget->GetCachedWidget();
		...
		if (LeafmostActiveNode.IsValid())
		{
			...
			{
				const TSharedPtr<SWidget> NewLeafmostSlateWidget = NewLeafmostWidget->GetCachedWidget();
				...
				UE_LOGF(
					LogUIActionRouter,
					VeryVerbose,
					"Leafmost activatable widget has changed from [%ls] to [%ls]",
					OldLeafmostSlateWidget.IsValid()
					? *FReflectionMetaData::GetWidgetDebugInfo(OldLeafmostSlateWidget.Get())
					: TEXT("None"),
					NewLeafmostSlateWidget.IsValid()
					? *FReflectionMetaData::GetWidgetDebugInfo(NewLeafmostSlateWidget.Get())
					: TEXT("None")
				);
			}
		}
...
void FActivatableTreeRoot::HandleInputMethodChanged(ECommonInputType InputMethod)
{
	if (IsReceivingInput() && LeafmostActiveNode.IsValid() && ensure(LeafmostActiveNode.Pin()->IsReceivingInput()))
	{
		...
		if (InputMethod == ECommonInputType::Gamepad && CVarSyncCursorToFocusedWidgetOnInputMethodChange.GetValueOnAnyThread())
		{
			...
			if (SlateUser)
			{
				...
				// ApplyLeafmostNodeConfig may update the currently focused widget, but it doesn't change the cursor position.
				// It means that next time the user presses the confirmation button on the controller, the mouse click event
				// will be sent to the last touched position, and not the focused widget. It may make us lose focus and semi-lock inputs.
				// To avoid this, we update the cursor position to match the focused widget.
				if (FocusedWidget)
				{
					const FGeometry& Geometry = FocusedWidget->GetCachedGeometry();
					const FVector2D AbsoluteWidgetCenter = Geometry.GetAbsolutePositionAtCoordinates(FVector2D(0.5f, 0.5f));
...
void FActivatableTreeRoot::ApplyLeafmostNodeConfig(bool bAttemptRetainFocus)
{
	...
	{
		UE_LOGF(LogUIActionRouter, Log, "Didn't apply input config for leaf-most node of root node [%ls] because it's not the active action domain node", *GetNameSafe(GetWidget()));
		return;
	}
	...
	if (FActivatableTreeNodePtr PinnedLeafmostNode = LeafmostActiveNode.Pin())
	{
		...
		{
			UE_LOGF(LogUIActionRouter, Display, "Applying input config for leaf-most node [%ls]", *PinnedLeafmostNode->GetWidget()->GetName());
		...
		{
			UE_LOGF(LogUIActionRouter, Log, "Didn't apply input config for leaf-most node [%ls] because it's not receiving input right now", *PinnedLeafmostNode->GetWidget()->GetName());
		}
	}
}
...
bool FActivatableTreeRoot::IsWidgetInLeafmostNodeHierarchy(const UWidget& Widget) const
{
	if (FActivatableTreeNodePtr PinnedLeaf = LeafmostActiveNode.Pin())
	{
		// If your leaf node has an SWidget-level helper, use it; otherwise adapt to UWidget
		TSharedPtr<SWidget> Cached = Widget.GetCachedWidget();
		if (Cached.IsValid())
		{
			return PinnedLeaf->IsExclusiveParentOfWidget(Cached);
		}
	}
	return false;
}

UCommonActivatableWidget* FActivatableTreeRoot::GetLeafmostActivatableWidget() const
{
	if (FActivatableTreeNodePtr PinnedLeaf = LeafmostActiveNode.Pin())
	{
		return PinnedLeaf->GetWidget();
	}
...
void FActivatableTreeRoot::FocusLeafmostNode()
{
	if (!CanSetInputConfigAndFocus())
	{
		UE_LOGF(LogUIActionRouter, Log, "%ls cannot focus its leaf most node because another node has priority", *GetNameSafe(GetWidget()));
		return;
	}

	if (!ensure(LeafmostActiveNode.IsValid()))
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot focus leaf most node - invalid LeafmostActiveNode");
		return;
	}
	...
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot focus leaf most node - invalid PinnedLeafmostNode");
		return;
	}
	...
	{
		UE_LOGF(LogUIActionRouter, Error, "Cannot focus leaf most node - invalid LeafWidget");
		return;
	}
	...
	TSharedPtr<SWidget> AutoRestoreTarget = LeafWidget->AutoRestoresFocus() ? PinnedLeafmostNode->GetFocusFallbackTarget() : nullptr;
	UWidget* DesiredTarget = LeafWidget->GetDesiredFocusTarget();

	const bool bCheckForKeyboardAndParentage = CVarCheckKeyboardFocusAndParentage.GetValueOnAnyThread();
	if (bCheckForKeyboardAndParentage)
	{
		if (AutoRestoreTarget.IsValid())
		{
			const bool bSupportsKeyboard = AutoRestoreTarget->SupportsKeyboardFocus();
			const bool bParentOfLeaf = PinnedLeafmostNode->IsExclusiveParentOfWidget(AutoRestoreTarget);
			if (!bSupportsKeyboard || !bParentOfLeaf)
			{
				AutoRestoreTarget.Reset();
			}
		}

		if (DesiredTarget)
		{
			const bool bSupportsKeyboard = DesiredTarget->SupportsKeyboardFocus();
			const bool bParentOfLeaf = PinnedLeafmostNode->IsExclusiveParentOfWidget(DesiredTarget->GetCachedWidget());
			if (!bSupportsKeyboard || !bParentOfLeaf)
			{
	...
	const bool bSuccessfullyAutoRestoreTarget = AutoRestoreTarget.IsValid() && FSlateApplication::Get().SetUserFocus(OwnerSlateId, AutoRestoreTarget);
	bool bShouldCancelDelayedFocusOperation = bSuccessfullyAutoRestoreTarget;
	if (bSuccessfullyAutoRestoreTarget)
	{
		UE_LOGF(LogUIActionRouter, Display, "[User %d] Focused AutoRestoreTarget %ls", OwnerSlateId, *FReflectionMetaData::GetWidgetDebugInfo(AutoRestoreTarget.Get()));	
	}
		
	const bool bFallbackToDesiredOnAutoRestoreFailure = CVarFallbackToDesiredOnAutoRestoreFailure.GetValueOnAnyThread();
	const bool bAttemptToFocusDesiredTarget = DesiredTarget && (
			(!bFallbackToDesiredOnAutoRestoreFailure && !AutoRestoreTarget.IsValid()) || // Legacy Behavior
			(bFallbackToDesiredOnAutoRestoreFailure && !bSuccessfullyAutoRestoreTarget) // New Behavior
			);

	if (bAttemptToFocusDesiredTarget)
	{
		UE_LOGF(LogUIActionRouter, Display, "[User %d] Focused desired target %ls", OwnerSlateId, *DesiredTarget->GetName());
		DesiredTarget->SetFocus();
	}
	
	if (!bSuccessfullyAutoRestoreTarget && !bAttemptToFocusDesiredTarget)
	{
		UE_LOGF(LogUIActionRouter, Display, "[User %d] Failed to Auto Restore or Focus Desired Target for %ls", GetActionRouter().GetLocalPlayerIndex(), *LeafWidget->GetName());
		...
		const bool bIgnoreFocusedWidget = CVarDisallowUserFocusedWidgetForPendingFocusRecipient.GetValueOnAnyThread();
		const bool bCheckParentOfFocusedWidget = !PendingFocusRecipient.IsValid() || !bIgnoreFocusedWidget;

		const bool bIsParentOfPendingFocusRecipient = PinnedLeafmostNode->IsExclusiveParentOfWidget(PendingFocusRecipient);
		const bool bIsParentOfFocusedWidget = PinnedLeafmostNode->IsExclusiveParentOfWidget(FocusedWidget);

		if (bIsParentOfPendingFocusRecipient || (bCheckParentOfFocusedWidget && bIsParentOfFocusedWidget))
		{
			if (bIgnoreFocusedWidget)
			{
				UE_LOGF(LogUIActionRouter, Display, "[User %d] Leaf-most node [%ls] did not set focus through `GetDesiredFocusTarget`. Doing nothing. this widget should be updated.", GetActionRouter().GetLocalPlayerIndex(),  *LeafWidget->GetName());
			}
			else
		{
			UE_LOGF(LogUIActionRouter, Display, "[User %d] Leaf-most node [%ls] did not set focus through desired methods, but the currently focused widget is acceptable. Doing nothing, but this widget should be updated.", GetActionRouter().GetLocalPlayerIndex(),  *LeafWidget->GetName());
		}
		}
		else
		{
			...
			{
				UE_LOGF(LogUIActionRouter, Display, "[User %d] No focus target for leaf-most node [%ls] - setting focus directly to the widget as a last resort.", OwnerSlateId, *LeafWidget->GetName());
				LeafWidget->SetFocus();
			}
			else if (UGameViewportClient* ViewportClient = GetActionRouter().GetLocalPlayerChecked()->ViewportClient)
			{
				UE_LOGF(LogUIActionRouter, Display, "[User %d] No focus target for leaf-most node [%ls], and the widget isn't focusable - focusing the game viewport.", OwnerSlateId, *LeafWidget->GetName());
