...
void UCommonUIActionRouterBase::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	if (FSlateApplication::IsInitialized())
	{
		if (ensure(InputSubsystem))
		{
			...
			AnalogCursor->GetOnVirtualPointerEnabledChanged().AddUObject(this, &UCommonUIActionRouterBase::HandleVirtualPointerEnabled);

			if (bCheckGameViewportClientValid && !GEngine->GameViewportClientClass->IsChildOf<UCommonGameViewportClient>())
			{
				UE_LOGF(LogUIActionRouter, Error,
		...
		{
			UE_LOGF(LogUIActionRouter, Warning, "Input system not initialized before action router!");
		}
...
void UCommonUIActionRouterBase::Deinitialize()
{
	...
	if (FSlateApplication::IsInitialized())
	{
		...
		{
			AnalogCursor->GetOnVirtualPointerEnabledChanged().RemoveAll(this);
			AnalogCursor->Deinitialize();
			AnalogCursor.Reset();
		}
...
bool UCommonUIActionRouterBase::ShouldCreateSubsystem(UObject* Outer) const
{
	...
	UE_LOGF(LogUIActionRouter, Display, "Found %i derived classes when attemping to create action router (%ls)", ChildClasses.Num(), *GetClass()->GetName());
...
void UCommonUIActionRouterBase::RegisterScrollRecipient(const UWidget& ScrollableWidget, UWidget* OwningNodeWidget)
{
	if (OwningNodeWidget)
	{
		if (FActivatableTreeNodePtr OwnerNode = FindOwningNode(*OwningNodeWidget))
		{
			OwnerNode->AddScrollRecipient(ScrollableWidget);
		}
	}
	else if (FActivatableTreeNodePtr OwnerNode = FindOwningNode(ScrollableWidget))
	{
		OwnerNode->AddScrollRecipient(ScrollableWidget);
	}
	else
...
void UCommonUIActionRouterBase::UnregisterScrollRecipient(const UWidget& ScrollableWidget, UWidget* OwningNodeWidget)
{
	if (OwningNodeWidget)
	{
		if (FActivatableTreeNodePtr OwnerNode = FindOwningNode(*OwningNodeWidget))
...
bool UCommonUIActionRouterBase::IsWidgetInLeafmostNodeHierarchy(const UWidget& Widget) const
{
	FActivatableTreeRootPtr RootNode = ActiveRootNode ? ActiveRootNode : FindActiveActionDomainRootNode();
	if (!RootNode)
	{
	...
	return RootNode->IsWidgetInLeafmostNodeHierarchy(Widget);
}

UCommonActivatableWidget* UCommonUIActionRouterBase::GetLeafmostActivatableWidget() const
{
	FActivatableTreeRootPtr RootNode = ActiveRootNode ? ActiveRootNode : FindActiveActionDomainRootNode();
	if (!RootNode)
	{
	...
	return RootNode->GetLeafmostActivatableWidget();
}
...
bool UCommonUIActionRouterBase::ShouldAlwaysShowCursor() const
{
	...
	bool bVirtualPointerEnabled = AnalogCursor.IsValid() ? AnalogCursor->IsVirtualPointerEnabled() : false;

	return bAlwaysShowCursor || bUsingMouseForTouch || bVirtualPointerEnabled;
}
...
{
	EMouseCaptureMode Mode = ActiveInputConfig.IsSet() ? ActiveInputConfig.GetValue().GetMouseCaptureMode() : DefaultMouseCapture;
	
	const bool bVirtualPointerEnabled = AnalogCursor.IsValid() ? AnalogCursor->IsVirtualPointerEnabled() : false;
	if (bVirtualPointerEnabled && Mode == EMouseCaptureMode::CapturePermanently)
	{
		// While VP is enabled, mouse-up releases capture each click, so plain CapturePermanently drops
		// every subsequent mouse-down in FSceneViewport (HasMouseCapture()=false on entry).
...
void UCommonUIActionRouterBase::HandleRootWidgetSlateReleased(TWeakPtr<FActivatableTreeRoot> WeakRoot)
{
	...
	if (RootNodes.Contains(RootNode))
	{
		...
		if (RootNodes.Num() == 0)
		{
			// Clear any input locks before clearing our active config so they don't leak
...
void UCommonUIActionRouterBase::HandleRootNodeDeactivated(TWeakPtr<FActivatableTreeRoot> WeakDeactivatedRoot)
{
	...
	if (bIsActivatableTreeEnabled)
	{
		...
		{
			UE_LOGF(LogUIActionRouter, Warning, "All widgets deactivated. Existing input config set: %ls",
...
void UCommonUIActionRouterBase::HandleVirtualPointerEnabled(bool bEnabled)
{
...
void UCommonUIActionRouterBase::DebugDumpActionDomainRootNodes(int32 UserIndex, int32 ControllerId, bool bIncludeActions, bool bIncludeChildren, bool bIncludeInactive) const
{
	...
	UE_LOGF(LogUIActionRouter, Display, "Dumping ActionDomainRootNodes for LocalPlayer [User %d, ControllerId %d]:\n%ls\n", UserIndex, ControllerId, *ActionDomainsOutputStr);
}
...
void UCommonUIActionRouterBase::UpdateLeafNodeAndConfig(FActivatableTreeRootPtr DesiredRoot, FActivatableTreeNodePtr DesiredLeafNode)
{
	if (DesiredRoot.IsValid())
	{
		if (DesiredRoot == ActiveRootNode)
		{
			...
			{
				UE_LOGF(LogUIActionRouter, Warning, "LeafmostActiveNode not updated.");
			}
		}
		else
		{
...
void UCommonUIActionRouterBase::SetActiveUIInputConfig(const FUIInputConfig& NewConfig, const UObject* InConfigSource)
{
	...
	if (bTraceInputConfig && InConfigSource && (!ActiveInputConfig.IsSet() || NewConfig != ActiveInputConfig.GetValue()))
	{
		...
		UE_LOGF(LogUIActionRouter, Log, "%ls", *InputConfigTrace);
...
void UCommonUIActionRouterBase::ApplyUIInputConfig(const FUIInputConfig& NewConfig, bool bForceRefresh)
{
	...
	{
		UE_LOGF(LogUIActionRouter, Display, "UIInputConfig being changed. bForceRefresh: %d, UserIndex: %d", bForceRefresh ? 1 : 0, GetLocalPlayerIndex());
		UE_LOGF(LogUIActionRouter, Display, "\tInputMode: Previous (%ls), New (%ls)",
		...
		if (UGameViewportClient* GameViewportClient = LocalPlayer.ViewportClient)
		{
			if (TSharedPtr<SViewport> ViewportWidget = GameViewportClient->GetGameViewportWidget())
			{
				if (APlayerController* PC = LocalPlayer.GetPlayerController(GetWorld()))
				{
					// Ensure we don't ever call SetIgnoreMoveInput(false) unless we previously called SetIgnoreMoveInput(true)
					...
					// Ensure we don't ever call SetIgnoreLookInput(false) unless we previously called SetIgnoreLookInput(true)
					...
					GameViewportClient->SetMouseCaptureMode(GetActiveMouseCaptureMode()); // ActiveInputConfig has already been updated with NewConfig
					...
					if (!bForceRefresh && bWasCursorHidden && PC->ShouldShowMouseCursor())
					{
						...
						if (bCenterCursor)
						{
							...
							if (ensure(SlateUser) && ensure(GameLayerManager))
							{
								...
								UE_LOGF(LogUIActionRouter, Verbose, "Moving the cursor to the viewport center.");
							}
						}
					}
				}
				...
				{
					UE_LOGF(LogUIActionRouter, Warning, "\tFailed to commit change! Local player controller was null.");
				}
			}
			else
			{
				UE_LOGF(LogUIActionRouter, Warning, "\tFailed to commit change! ViewportWidget was null.");
			}
		}
		else
		{
			UE_LOGF(LogUIActionRouter, Warning, "\tFailed to commit change! GameViewportClient was null.");
		}
...
public:
	static void HandleDebugDumpTree(const TArray<FString>& Args, UWorld* World)
	{
		if (World == nullptr)
		{
			UE_LOGF(LogUIActionRouter, Error, "No World, unable to run CommonUI.DumpActivatableTree");
			return;
		}
		...
		{
			UE_LOGF(LogUIActionRouter, Error, "No GameInstance, unable to run CommonUI.DumpActivatableTree");
			return;
		}
		...
		for (int32 CurrIdx = 0; CurrIdx < LocalPlayers.Num(); ++CurrIdx)
		{
			...
			if (UCommonUIActionRouterBase* ActionRouter = LocalPlayer->GetSubsystem<UCommonUIActionRouterBase>())
			{
				...
				UE_LOGF(LogUIActionRouter, Display, "Dumping ActivatableWidgetTree for LocalPlayer [User %d, ControllerId %d]:\n\n%ls\n\n", CurrIdx, LocalPlayer->GetControllerId(), *TreeOutputStr);
	...
	static void HandleDumpCurrentInputConfig(UWorld* World)
	{
		if (World == nullptr)
		{
			UE_LOGF(LogUIActionRouter, Error, "No World, unable to run CommonUI.DumpInputConfig");
			return;
		}
		...
		if (!GameInstance)
		{
			UE_LOGF(LogUIActionRouter, Error, "No GameInstance, unable to run CommonUI.DumpInputConfig");
			return;
		}
		...
		UE_LOGF(LogUIActionRouter, Display, "Dumping all Input configs:\n%ls", *OutStr);
	}
};
