...
namespace UE::CommonUI::Private
{
	...
	bool bDisableDimensionValueSetInDesignTime = false;
	FAutoConsoleVariableRef CVarDisableDimensionValueSetInDesignTime(
		TEXT("CommonButton.DisableDimensionValueSetInDesignTime"),
		bDisableDimensionValueSetInDesignTime,
		TEXT("Block SetMin/MaxDimension to set directly the dimension variable (MinHeight, MaxHeight,MinWidth, MaxWidth) during design time to prevent possible data corruptoion from Mobile value"),
		ECVF_Default);
}
...
void UCommonButtonBase::SetHideInputActionWithKeyboard(bool bInHideInputActionWithKeyboard)
{
	bHideInputActionWithKeyboard = bInHideInputActionWithKeyboard;
...
{
	RefreshDimensions(MinWidth, MinHeight, MaxWidth, MaxHeight);
}

void UCommonButtonBase::RefreshDimensions(const int32 InMinWidth, const int32 InMinHeight, const int32 InMaxWidth, const int32 InMaxHeight)
{
	if (RootButton.IsValid())
	{
		const UCommonButtonStyle* const StyleCDO = GetStyleCDO();
		RootButton->SetMinDesiredWidth(FMath::Max(InMinWidth, StyleCDO ? StyleCDO->MinWidth : 0));
		RootButton->SetMinDesiredHeight(FMath::Max(InMinHeight, StyleCDO ? StyleCDO->MinHeight : 0));
		
		if (!StyleCDO)
		{
			RootButton->SetMaxDesiredWidth(InMaxWidth);
			RootButton->SetMaxDesiredHeight(InMaxHeight);
		}
		else
		{
			if (InMaxWidth > 0 && StyleCDO->MaxWidth > 0)
			{
				RootButton->SetMaxDesiredWidth(FMath::Min(InMaxWidth, StyleCDO->MaxWidth));
			}
			else
			{
				RootButton->SetMaxDesiredWidth(FMath::Max(InMaxWidth, StyleCDO->MaxWidth));
			}

			if (InMaxHeight > 0 && StyleCDO->MaxHeight > 0)
			{
				RootButton->SetMaxDesiredHeight(FMath::Min(InMaxHeight, StyleCDO->MaxHeight));
			}
			else
			{
				RootButton->SetMaxDesiredHeight(FMath::Max(InMaxHeight, StyleCDO->MaxHeight));
			}
		}
	}
}
...
{
	InMinWidth = FMath::Max(InMinWidth, 0);
	InMinHeight = FMath::Max(InMinHeight, 0);

	if (UE::CommonUI::Private::bDisableDimensionValueSetInDesignTime && IsDesignTime())
	{
		if (RootButton.IsValid())
		{
			RefreshDimensions(InMinWidth, InMinHeight, RootButton->GetMaxWidth(), RootButton->GetMaxHeight());
		}
	}
	else
	{
...
{
	InMaxWidth = FMath::Max(InMaxWidth, RootButton.IsValid() ? RootButton->GetMinWidth() : 0);
	InMaxHeight = FMath::Max(InMaxHeight, RootButton.IsValid() ? RootButton->GetMinHeight() : 0);

	if (UE::CommonUI::Private::bDisableDimensionValueSetInDesignTime && IsDesignTime())
	{
		if (RootButton.IsValid())
		{
			RefreshDimensions(RootButton->GetMinWidth(), RootButton->GetMinHeight(), InMaxWidth, InMaxHeight);
		}
	}
	else
	{
...
void UCommonButtonBase::SetActionDisplayName(const FText& InDisplayName)
{
	if (!TriggeringBindingHandle.IsValid())
	{
		return;
	...
	if (!TriggeringBindingHandle.GetDisplayName().EqualTo(InDisplayName))
	{
		TriggeringBindingHandle.SetDisplayName(InDisplayName);
	}
}
