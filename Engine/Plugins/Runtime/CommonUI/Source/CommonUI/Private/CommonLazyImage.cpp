		...
		RequestAsyncLoad(LazyObject,
			[WeakThis, LazyObject, bMatchTextureSize]() {
				if (UImage* StrongThis = WeakThis.Get())
				{
					...
					else if (TScriptInterface<ISlateTextureAtlasInterface> AsAtlasInterface = LazyObject.Get())
					{
						StrongThis->SetBrushFromAtlasInterface(AsAtlasInterface, bMatchTextureSize);
					}
