...
bool UCommonTabListWidgetBase::RegisterTab(FName TabNameID, TSubclassOf<UCommonButtonBase> ButtonWidgetType, UWidget* ContentWidget, const int32 TabIndex /*= INDEX_NONE*/)
{
	...
	if (RegisteredTabsByID.Contains(TabNameID))
	{
		...
		UE_LOGF(LogCommonUI, Warning, "RegisteredTabsByID already contains a tab called [%ls]", *TabNameID.ToString());
	}
	...
	{
		bAreParametersValid = false;
		UE_LOGF(LogCommonUI, Warning, "RegisteredTabsByID missing ButtonWidgetType [%ls]", *TabNameID.ToString());
	}
	...
	{
		bAreParametersValid = false;
		UE_LOGF(LogCommonUI, Warning, "RegisteredTabsByID missing TabButtonGroup [%ls]", *TabNameID.ToString());
	}
