...
void UCommonVisibilitySwitcher::SetActiveWidgetIndex(int32 Index)
{
	...
	{
		UE_LOGF(LogCommonUI, Verbose, "%ls [%ls] - [%d] is already the active index, doing nothing", ANSI_TO_TCHAR(__FUNCTION__), *GetName(), ActiveWidgetIndex);
		return;
	}
...
void UCommonVisibilitySwitcher::SetActiveWidget(const UWidget* Widget)
{
	...
	{
		UE_LOGF(LogCommonUI, Verbose, "%ls [%ls] - [%ls] is not a child of the switcher, doing nothing", ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *Widget->GetName());
		return;
	}
...
void UCommonVisibilitySwitcher::ActivateVisibleSlot()
{
	...
	{
		UE_LOGF(LogCommonUI, Verbose, "%ls [%ls] - Activating [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *ActivatableWidget->GetName());
...
void UCommonVisibilitySwitcher::DeactivateVisibleSlot()
{
	if (UCommonActivatableWidget* ActivatableWidget = Cast<UCommonActivatableWidget>(GetActiveWidget()))
	{
		UE_LOGF(LogCommonUI, Verbose, "%ls [%ls] - Deactivating [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *ActivatableWidget->GetName());
...
void UCommonVisibilitySwitcher::SetActiveWidgetIndex_Internal(int32 Index, bool bBroadcastChange /*= true*/)
{
	...
	if (Slots.IsValidIndex(ActiveWidgetIndex))
	{
		if (UCommonVisibilitySwitcherSlot* OldActiveSlot = Cast<UCommonVisibilitySwitcherSlot>(Slots[ActiveWidgetIndex]))
		{
			...
			UE_LOGF(LogCommonUI, Verbose, "%ls [%ls] - Setting visibility of slot containing [%ls] to Collapsed", ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *OldActiveSlot->GetName());
	...
	if (Slots.IsValidIndex(ActiveWidgetIndex))
	{
		if (UCommonVisibilitySwitcherSlot* NewActiveSlot = Cast<UCommonVisibilitySwitcherSlot>(Slots[ActiveWidgetIndex]))
		{
			...
			UE_LOGF(LogCommonUI, Verbose, "%ls [%ls] - Setting visibility of slot containing [%ls] to [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *GetName(), *NewActiveSlot->GetName(), *StaticEnum<ESlateVisibility>()->GetDisplayValueAsText(ShownVisibility).ToString());
