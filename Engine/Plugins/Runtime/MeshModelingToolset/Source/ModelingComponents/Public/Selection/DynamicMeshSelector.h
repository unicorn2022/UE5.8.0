...
public:
	...
	UE_API virtual void UpdateSoftSelection(FGeometrySelection& Selection, const FUpdateSoftSelectionConfig& InSoftSelectionConfig) override;
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS

	UE_API virtual bool ConvertSelection(const FGeometrySelection& FromSelectionIn, FGeometrySelection& ToSelectionOut, const UE::Geometry::EEnumerateSelectionConversionParams ConversionParams) override;
...
protected:
	...
	//
	// PathFinder will be built on-demand if surface-based soft selection is enabled
	//
	TPimplPtr<UE::Geometry::TMeshDijkstra<FDynamicMesh3>> PathFinder;

	//
	// CachedVolumeDistanceToSelection caches the Euclidean distance from each mesh vertex
	// to its nearest seed for the current selection. Index by VertexID; 
	//
	TArray<double> CachedVolumeDistanceToSelection;

	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	UE_EXPERIMENTAL(5.8, "Soft Selection is experimental while the API is under active development.")
	UE_API virtual void UpdateSoftSelection_Surface(FGeometrySelection& Selection, const FUpdateSoftSelectionConfig& InSoftSelectionConfig);
	UE_EXPERIMENTAL(5.8, "Soft Selection is experimental while the API is under active development.")
	UE_API virtual void UpdateSoftSelection_Volume(FGeometrySelection& Selection, const FUpdateSoftSelectionConfig& InSoftSelectionConfig);
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
...
public:
	...
	UE_API virtual void UpdateTransform( TFunctionRef<FVector3d(int32 VertexID, double Weight, const FVector3d& InitialPosition, const FTransform& WorldTransform)> PositionTransformFunc ) override;
...
protected:
	...
	TArray<double> SoftSelectedVertexWeights;
	
	// The number of vertices in MeshVertices = NumSelectedMeshVertices + SoftSelectedVertexWeights.Num()
	int32 NumSelectedMeshVertices = 0;
