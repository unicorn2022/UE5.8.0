...
public:
	...
	/** Grow current selection. If selection is empty, nothing happens */
	UFUNCTION(CallInEditor, Category = SelectionActions)
	UE_API void GrowSelection();

	/** Shrink current selection. If selection is empty, nothing happens */
	UFUNCTION(CallInEditor, Category = SelectionActions)
	UE_API void ShrinkSelection();
...
public:
	...
	UE_API virtual UMeshTopologySelectionMechanicProperties* CreateProperties() override;
	...
	*  perform the operation a bit more efficiently by using the mesh topology directly.
	*/
	UE_API void InvertSelection(bool bAsTriangleTopology);
	 ...
	 */
	UE_API void SetSelection_AsGroupTopology(const UE::Geometry::FGeometrySelection& Selection, const bool bBroadcastSelectionChanged = false);

	/**
	 * Initialize the Active Selection based on the provided Triangle-topology FGeometrySelection
	 */
	UE_API void SetSelection_AsTriangleTopology(const UE::Geometry::FGeometrySelection& Selection, const bool bBroadcastSelectionChanged = false);
