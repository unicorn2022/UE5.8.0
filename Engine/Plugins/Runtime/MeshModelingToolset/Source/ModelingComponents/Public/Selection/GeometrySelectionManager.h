...
enum class EEnumerateRenderCachesDirtyFlags: uint8
{
	...
	SoftSelectionCachesDirty = 1 << 3,

	Default = SelectionCachesDirty | UnselectedCachesDirty | PreviewCachesDirty | SoftSelectionCachesDirty,
};
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "Use the UpdateTransformation function that takes a lambda that includes per-vertex weight instead")
	 ...
	 * See IGeometrySelectionTransformer::UpdateTransform for details on this callback
	 */
	UE_EXPERIMENTAL(5.8, "Per-vertex weighted transformation is experiemental while the API is under active development.")
	UE_API virtual void UpdateTransformation( TFunctionRef<FVector3d(int32 VertexID, double Weight, const FVector3d&, const FTransform&)> PositionTransformFunc );
	 ...
	 */
	UE_EXPERIMENTAL(5.8, "This function is experiemental while the API is under active development.")
	UE_API void UpdateTransformation(
...
public:
	 ...
	 * Unselected elements, Hover over selection, Hover over non-selection, Selected elements, and Soft selected element
	 */
	UE_API void SetSelectionColors(FLinearColor UnselectedCol, FLinearColor HoverOverSelectedCol, FLinearColor HoverOverUnselectedCol, FLinearColor GeometrySelectedCol, FLinearColor SoftSelectedCol);
...
public:
	...
	UE_API void SetSoftSelectionEnabled(bool bEnable);

	UE_EXPERIMENTAL(5.8, "Soft Selection is experimental while the API is under active development.")
	bool IsSoftSelectionEnabled() const { return bEnableSoftSelection; };

	UE_EXPERIMENTAL(5.8, "Soft Selection is experimental while the API is under active development.")
	UE_API void SetSoftSelectionRadius(double Radius);

	UE_EXPERIMENTAL(5.8, "Soft Selection is experimental while the API is under active development.")
	double GetSoftSelectionRadius() const {return SoftSelectionRadius; }

	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	UE_EXPERIMENTAL(5.8, "Soft Selection is experimental while the API is under active development.")
	UE_API void SetSoftSelectionDistanceMode(IGeometrySelector::ESoftSelectionDistanceMode InMode);

	UE_EXPERIMENTAL(5.8, "Soft Selection is experimental while the API is under active development.")
	IGeometrySelector::ESoftSelectionDistanceMode GetSoftSelectionDistanceMode() const { return SoftSelectionDistanceMode; }
...
protected:
	...
	inline static TStaticArray<FString, 3> SoftSelectedSetIds = {"SoftSelected_Vertices", "SoftSelected_Lines", "SoftSelected_Triangles"};
	...
	UPROPERTY()
	FMeshElementSelectionParams SoftSelectedParams {SoftSelectedSetIds, 5.5f, 2.f, 10.f};
...
private:
	...
	TArray<FGeometrySelectionElements> CachedSoftSelectionRenderElements;
	UE_API void RebuildSoftSelectionRenderCache();
	...
	void UpdateSoftSelectionOnSelectionModified();
	void UpdateSoftSelectionOnConfigModified(bool bRecomputeDistances);

	// Soft Selection
	bool bEnableSoftSelection = false;

	double SoftSelectionRadius = 20.0;

	PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	IGeometrySelector::ESoftSelectionDistanceMode SoftSelectionDistanceMode = IGeometrySelector::ESoftSelectionDistanceMode::Surface;
	PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
};
