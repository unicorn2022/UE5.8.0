...
*/
UENUM()
enum class EMeshTopologySelectionMechanicDragTool : uint8
{
	None,
	Marquee,
	Lasso
...
public:
	...
	UPROPERTY(EditAnywhere, Category = AdditionalSelectionOptions, AdvancedDisplay)
	EMeshTopologySelectionMechanicDragTool SelectionDragTool = EMeshTopologySelectionMechanicDragTool::Marquee;

	UPROPERTY(EditAnywhere, Category = AdditionalSelectionOptions, meta = (EditCondition = "SelectionDragTool != EMeshTopologySelectionMechanicDragTool::None", EditConditionHides))
	bool bIgnoreOcclusionForDragTools = true;

#if WITH_EDITORONLY_DATA
	UE_DEPRECATED(5.8, "bEnableMarquee has been deprecated, use SelectionDragTool instead")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "bEnableMarquee has been deprecated, use SelectionDragTool instead"))
	...
	UE_DEPRECATED(5.8, "bMarqueeIgnoreOcclusion has been deprecated, use bIgnoreOcclusionForDragTools instead")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "bMarqueeIgnoreOcclusion has been deprecated, use bIgnoreOcclusionForDragTools instead"))
	...
	// When true display toggles buttons to allow switching between various selection drag tools (in the detail customization)
	bool bDisplaySelectionDragToolsControls = false;
...
UENUM()
enum class UE_DEPRECATED(5.0, "EMarqueeSelectionUpdateType is deprecated, use ESelectionDragToolUpdateType instead") EMarqueeSelectionUpdateType
{
	OnDrag,
	OnTickAndRelease,
	OnRelease
...
/*
 * Selection update type that defines when to update the selection when using a drag selection tool
 */
UENUM()
enum class ESelectionDragToolUpdateType: uint8
{
	OnDrag,				// will update the selection for every mouse input events ( can be multiple time per tick )
	OnTickAndRelease,	// will update the selection only once per tick and on the tool release
	OnRelease			// will only update the selection on ethe tool release
};
...
public:
	...
	UE_API virtual UMeshTopologySelectionMechanicProperties* CreateProperties();
	...
	UE_DEPRECATED(5.8, "deprecated, use SetSelectionDragToolUpdateType instead")
	 ...
	 * Sets how/when the selection updates are handled.
	 */
	UE_API void SetSelectionDragToolUpdateType(ESelectionDragToolUpdateType InType);
...
protected:
	// common handling of the selection tools (marquee/lasso) 
	void OnSelectionToolDragStarted();
	void OnSelectionToolDragFinished();
	...
	// These get bound to lasso mechanic delegates.
	void OnPolyLassoStarted();
	void OnPolyLassoChanged(const FCameraPolyLasso& Lasso);
	...
	UE_API virtual void UpdateLassoEnabled();
...
protected:
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_DEPRECATED(5.8, "MarqueeSelectionUpdateType is deprecated, use SelectionDragToolUpdateType (see also SetSelectionDragToolUpdateType)")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "MarqueeSelectionUpdateType is deprecated, use SelectionDragToolUpdateType (see also SetSelectionDragToolUpdateType)"))
	...
	*/
	UPROPERTY()
	ESelectionDragToolUpdateType SelectionDragToolUpdateType = ESelectionDragToolUpdateType::OnDrag;
	...
	void HandleLassoChanged(const FCameraPolyLasso& InLasso);
