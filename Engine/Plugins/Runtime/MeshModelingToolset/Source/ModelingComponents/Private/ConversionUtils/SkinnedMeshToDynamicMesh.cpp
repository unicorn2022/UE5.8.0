...
public:
	...
	, SkeletalMeshRenderData(SkinnedMeshComponent.MeshObject->GetSkeletalMeshRenderData())
	{
		...
		if (LOD != RequestedLOD)
		{
			UE_LOGF(LogGeometry, Warning, "Requesting LOD %d within [%d, %d], clamping to %d", RequestedLOD, MinLODIndex, MaxLODIndex, LOD);
		}
		...
		{
			UE_LOGF(LogGeometry, Display, "LOD vertex mismatch with [%ls -> %ls]", *SkinnedMeshComponent.GetPathName(), *SkinnedMeshComponent.GetSkinnedAsset()->GetPathName());
			UE_LOGF(LogGeometry, Display, " - Total Vertex Count: %d", SkinnedVertices.Num());
			UE_LOGF(LogGeometry, Display, " - Render Data Vertex Count: %d", LODData.StaticVertexBuffers.PositionVertexBuffer.GetNumVertices());
			UE_LOGF(LogGeometry, Display, " - Total Index/Triangle Count: %d / %d ", IndexBuffer->Num(), IndexBuffer->Num() / 3);
		 
			for (int32 SectionIndex = 0; SectionIndex < LODData.RenderSections.Num(); SectionIndex++)
			{
				const FSkelMeshRenderSection& Section = LODData.RenderSections[SectionIndex];
				UE_LOGF(LogGeometry, Display, " - Section[%d]: Skip? %ls", SectionIndex, SkipSection(Section) ? TEXT("Yes") : TEXT("No"));
				UE_LOGF(LogGeometry, Display, " - - Vertex Base: %d - Vertex Count: %d", Section.BaseVertexIndex, Section.NumVertices);
				UE_LOGF(LogGeometry, Display, " - - Index Base: %d - Triangle Base: %d - Triangle Count: %d",
