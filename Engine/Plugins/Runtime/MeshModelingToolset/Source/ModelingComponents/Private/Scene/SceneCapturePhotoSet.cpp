		...
		const FText& TaskMessage)
	{
		if (PhotoSetStatus[CaptureType] == ECaptureTypeStatus::Pending)
		{
			...
			for (const FSpatialPhotoParams& Params : PhotoSetParams)
			{
				...
				{
					UE_LOGF(LogGeometry, Display, "FSceneCapturePhotoSet: The pending '%ls' step was cancelled", *TaskMessage.ToString());
					bWasCancelled = true;
					return;
				}
				...
				if (bVisible)
				{
					...
					if (ValidSampleDepthThreshold > 0)
					{
						...
						if (DeviceZ > 0)
						{
							...
							FVector4d PixelPositionWorld = ViewMatrices.GetClipToWorld().TransformPosition(PixelPositionDevice);
		...
		for (int64 PixelLinearIndex = 0; PixelLinearIndex < Params.Dimensions.Num(); ++PixelLinearIndex)
		{
			...
			if (bValidDeviceZ) // Skip samples on the near/far plane
			{
				...
				if (bComputePointWorld)
				{
					...
					const FVector4d PointWorld4 = ViewMatrices.GetClipToWorld().TransformPosition(PointDevice);
