...
static TUniquePtr<IMeshSpatialWrapper> SpatialWrapperFactory( const FMeshTypeContainer& MeshContainer, const TArray<FActorChildMesh*>& ParentMeshes, const FMeshSceneAdapterBuildOptions& BuildOptions )
{
	if (MeshContainer.MeshType == ESceneMeshType::StaticMeshAsset)
	{
		...
		if (ensure(StaticMesh != nullptr))
		{
			...
			bUseRenderMeshes &= StaticMesh->GetRenderData() && StaticMesh->GetRenderData()->GetCurrentFirstLOD(0);
#else
...
void FMeshSceneAdapter::Build(const FMeshSceneAdapterBuildOptions& BuildOptions)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogGeometry, Warning, "[FMeshSceneAdapter] decomposed %d source meshes into %d unique meshes containing %d triangles", DecomposedSourceMeshCount.load(), DecomposedMeshesCount.load(), AddedTrisCount)
		}
...
void FMeshSceneAdapter::Build_FullDecompose(const FMeshSceneAdapterBuildOptions& BuildOptions)
{
	...
	if (BuildOptions.bPrintDebugMessages)
	{
		...
		for (int32 k = 0; k < FMath::Min(Stats.Num(), 20); ++k)
		{
			...
			UE_LOGF(LogTemp, Warning, "  %ls : SourceTris %d Inst %d ResultTris %d Inst %d | UniqueScales %d Components %d | KeptTris %d NewUniqueTris %d | (Tri,Inst) %ls",
...
void FMeshSceneAdapter::BuildSpatialEvaluationCache()
{
	...
	ForEachSpatialWrapper([](TSharedPtr<FSpatialWrapperInfo> Adapter)
	{
		...
		for (FActorChildMesh* MeshInstance : WrapperInfo.ParentMeshes)
		{
			...
			{
				UE_LOGF(LogGeometry, Warning, "FMeshSceneAdapter::BuildSpatialEvaluationCache: broken MeshSpatial link found!");
			}
		}
	});
...
void FMeshSceneAdapter::GenerateBaseClosingMesh(double BaseHeight, double ExtrudeHeight)
{
	...
	if (bOK == false)
	{
		UE_LOGF(LogTemp, Warning, "[FMeshSceneAdapter::GenerateBaseClosingMesh] failed to solve convex hull");
		return;
	}
	...
	{
		UE_LOGF(LogTemp, Warning, "[FMeshSceneAdapter::GenerateBaseClosingMesh] convex hull is degenerate");
		return;
	}
