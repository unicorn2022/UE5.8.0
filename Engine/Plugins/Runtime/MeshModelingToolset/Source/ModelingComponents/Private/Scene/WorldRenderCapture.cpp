...
UTextureRenderTarget2D* FWorldRenderCapture::GetRenderTexture()
{
	...
	{
		LinearRenderTexture->UpdateResourceImmediate(true);

		UE::Internal::FlushRHIThreadToUpdateTextureRenderTargetReference();

		return LinearRenderTexture;
	}
	...
	LinearRenderTexture = NewObject<UTextureRenderTarget2D>();
	if (ensure(LinearRenderTexture))
	{
		LinearRenderTexture->ClearColor = FLinearColor::Transparent;
		LinearRenderTexture->TargetGamma = 1.0f;
		LinearRenderTexture->InitCustomFormat(Width, Height, PF_FloatRGBA, false);
		LinearRenderTexture->UpdateResourceImmediate(true);
...
static const TCHAR* MRSPostProcessMaterialAssetPath = TEXT("/MeshModelingToolsetExp/Materials/PostProcess_PackedMRSA.PostProcess_PackedMRSA");

bool FWorldRenderCapture::CaptureUsingPostProcessMaterialPipe(
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_TEXT(*FString::Printf(TEXT("FWorldRenderCapture_%s"), DebugImageName));
	...
	UTextureRenderTarget2D* RenderTargetTexture = GetRenderTexture();
	...
	ShowFlags.SetLighting(bEnableLighting);
	if (bEnableLighting)
	{
		// Emissive capture requires the post-processing pipeline to be active.
	...
	ImagePixelPipe->bIsExpecting32BitPixelData = true;
	...
	if (bWriteDebugImage)
	{
		WriteDebugImage(ResultImageOut, DebugImageName);
	}
	...
	const FRenderCaptureConfig& Config)
{
	...
	return CaptureUsingPostProcessMaterialPipe(PostProcessMaterial, ViewFrame, HorzFOVDegrees, NearPlaneDist, ResultImageOut, Config, bEnableLighting, TEXT("CombinedMRS"));
}
	...
	const FRenderCaptureConfig& Config)
{
	...
	UE::Internal::CopyCaptureDataToOutputImageFormat(CaptureData, ResultImageOut, [](const FLinearColor& Color)
	{
		...
		float Depth = FVector3f::DotProduct(EncodedDepth, FVector3f(1.0, 1 / 255.0f, 1 / 65025.0f));
...
bool FWorldRenderCapture::CaptureBufferVisualizationFromPosition(
	...
	const FRenderCaptureConfig& Config)
{
	if (!GetBufferVisualizationData().IsInitialized())
	{
		return false;
	}

	UMaterialInterface* VisMaterial = GetBufferVisualizationData().GetMaterial(VisualizationMode);
	if (!ensure(VisMaterial))
	{
		return false;
	}

	const bool bEnableLighting = (VisualizationMode == Internal::GetBufferVisualizationModeName(ERenderCaptureType::Emissive));
	return CaptureUsingPostProcessMaterialPipe(VisMaterial, Frame, HorzFOVDegrees, NearPlaneDist, ResultImageOut, Config, bEnableLighting, *VisualizationMode.ToString());
}
	...
	const FRenderCaptureConfig& Config)
{
	...
	{
		return CaptureDeviceDepthFromPosition(ViewFrame, HorzFOVDegrees, NearPlaneDist, ResultImageOut, Config);
	}
	...
	{
		TSoftObjectPtr<UMaterialInterface> PostProcessMaterialPtr = TSoftObjectPtr<UMaterialInterface>(FSoftObjectPath(MRSPostProcessMaterialAssetPath));
		Material = PostProcessMaterialPtr.LoadSynchronous();
		DebugName = TEXT("CombinedMRS");
	}
	else
	{
		...
		if (GetBufferVisualizationData().IsInitialized())
		{
			Material = GetBufferVisualizationData().GetMaterial(BufferVisualizationMode);
		}
		DebugName = BufferVisualizationMode.ToString();
	}
	...
	const bool bEnableLighting = (CaptureType == ERenderCaptureType::Emissive);
	return CaptureUsingPostProcessMaterialPipe(Material, ViewFrame, HorzFOVDegrees, NearPlaneDist, ResultImageOut, Config, bEnableLighting, *DebugName);
}
