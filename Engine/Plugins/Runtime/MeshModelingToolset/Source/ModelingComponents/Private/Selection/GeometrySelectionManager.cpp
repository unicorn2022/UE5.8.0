...
void UGeometrySelectionManager::Initialize( UInteractiveToolsContext* ToolsContextIn, IToolsContextTransactionsAPI* TransactionsAPIIn )
{
	...
	OnSelectionModified.AddUObject(this, &UGeometrySelectionManager::UpdateSoftSelectionOnSelectionModified);
}
...
void UGeometrySelectionManager::SetSelectionElementTypeInternal(EGeometryElementType NewElementType)
{
	if (SelectionElementType != NewElementType)
	{
		...
		constexpr bool bMarkSelectableDirty = true;
		MarkRenderCachesDirty(bMarkSelectableDirty);
		ClearActivePreview();
	}
}
...
void UGeometrySelectionManager::UpdateSoftSelectionOnSelectionModified()
{
	if (!HasActiveTargets())
	{
		return;
	...
	IGeometrySelector::FUpdateSoftSelectionConfig SoftSelectionConfig;
	SoftSelectionConfig.bRecomputeDistances = true;
	SoftSelectionConfig.bEnabled = bEnableSoftSelection;
	SoftSelectionConfig.Radius = SoftSelectionRadius;
	SoftSelectionConfig.DistanceMode = SoftSelectionDistanceMode;
	Target->Selector->UpdateSoftSelection(Target->Selection, SoftSelectionConfig);
...
PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
void UGeometrySelectionManager::UpdateSoftSelectionOnConfigModified(bool bRecomputeDistances)
{
	if (!HasActiveTargets())
	{
		return;
	...
	IGeometrySelector::FUpdateSoftSelectionConfig Config;
	Config.bRecomputeDistances = bRecomputeDistances;
	Config.bEnabled = bEnableSoftSelection;
	Config.Radius = SoftSelectionRadius;
	Config.DistanceMode = SoftSelectionDistanceMode;
	Target->Selector->UpdateSoftSelection(Target->Selection, Config);

	MarkRenderCachesDirty(false);
}
PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS

void UGeometrySelectionManager::SetSoftSelectionEnabled(bool bEnable)
{
	if (bEnableSoftSelection != bEnable)
	{
		bEnableSoftSelection = bEnable;
		constexpr bool bRecomputeDistances = false;
		UpdateSoftSelectionOnConfigModified(bRecomputeDistances);
	}
}

void UGeometrySelectionManager::SetSoftSelectionRadius(double Radius)
{
	double ClampedRadius = FMath::Max(Radius, 0);
	if (SoftSelectionRadius != ClampedRadius)
	{
		SoftSelectionRadius = ClampedRadius;
		constexpr bool bRecomputeDistances = false;
		UpdateSoftSelectionOnConfigModified(bRecomputeDistances);
	}
}
...
PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
void UGeometrySelectionManager::SetSoftSelectionDistanceMode(IGeometrySelector::ESoftSelectionDistanceMode InMode)
{
	if (SoftSelectionDistanceMode != InMode)
	{
		SoftSelectionDistanceMode = InMode;
		constexpr bool bRecomputeDistances = true;
		UpdateSoftSelectionOnConfigModified(bRecomputeDistances);
	}
}
PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
...
{
	auto PositionTransformFuncWithUnusedWeight = [&PositionTransformFunc](int32 VertexID, double Weight, const FVector3d& Position, const FTransform& Transform)
		{
			return PositionTransformFunc(VertexID, Position, Transform);
		};
	UpdateTransformation(PositionTransformFuncWithUnusedWeight);
}

void UGeometrySelectionManager::UpdateTransformation( 
	TFunctionRef<FVector3d(int32 VertexID, double Weight, const FVector3d&, const FTransform&)> PositionTransformFunc )
{
	...
	RenderCachesDirtyFlags |= EEnumerateRenderCachesDirtyFlags::SoftSelectionCachesDirty;
}
...
void UGeometrySelectionManager::EndTransformation()
{
	...
	// Take ownership of ActiveTransformations locally before iterating. If EndTransform synchronously pumps
	// messages (e.g. a slow-task dialog shown during PostEditChange triggers focus loss, which routes through
	// UInputRouter::ForceTerminateAll back into OnEndGizmoTransform), any reentrant call to EndTransformation
	// will see IsInActiveTransformation() return false and bail at the ensure above.
	TArray<IGeometrySelectionTransformer*> LocalTransformations = MoveTemp(ActiveTransformations);

	GetTransactionsAPI()->BeginUndoTransaction(LOCTEXT("EndTransformation", "Transform Selection"));

	for (int32 k = 0; k < ActiveTargetReferences.Num(); ++k)
	{
		if (LocalTransformations[k] != nullptr)
		{
			LocalTransformations[k]->EndTransform(GetTransactionsAPI());

			ActiveTargetReferences[k]->Selector->ShutdownTransformation(LocalTransformations[k]);
		}
	}
	...
	)
{
	if (bWorldSpaceScale)
	{
		...
		UpdateTransformation([&StartTransform, &TranslationToApply, &RotationToApply, &ScaleToApply](int32 VertexID, double Weight, const FVector3d& InitialPosition, const FTransform3d& WorldTransform)
			{
				FVector3d PosLocal = InitialPosition;
				FVector3d PosWorld = WorldTransform.TransformPosition(PosLocal);
				FVector3d PosWorldGizmo = PosWorld - StartTransform.GetLocation();

				FVector3d NewPosWorld = FQuat::Slerp(FQuat::Identity, RotationToApply, Weight) * (((ScaleToApply * Weight ) + 1.0f) * PosWorldGizmo) + (StartTransform.GetLocation() + TranslationToApply * Weight);
		...
		UpdateTransformation([&StartTransform, &TranslationToApply, &RotationToApply, &ScaleToApply](int32 VertexID, double Weight, const FVector3d& InitialPosition, const FTransform3d& WorldTransform)
			{
				...
				FVector3d PosLocal = InitialPosition;
				FVector3d PosWorld = WorldTransform.TransformPosition(PosLocal);
				FVector3d PosGizmo = StartTransform.InverseTransformPosition(PosWorld);
				PosGizmo = ((ScaleToApply * Weight ) + 1.0f) * PosGizmo;
				FVector3d NewPosWorld = FQuat::Slerp(FQuat::Identity, RotationToApply, Weight) * StartTransform.GetRotation() * PosGizmo + (StartTransform.GetLocation() + TranslationToApply * Weight);
				FVector3d NewPosLocal = WorldTransform.InverseTransformPosition(NewPosWorld);
				return NewPosLocal;
			});
	}	
...
void UGeometrySelectionManager::OnTargetGeometryModified(IGeometrySelector* Selector)
{
	...
	CachedSoftSelectionRenderElements.Reset();
	...
	CachedSoftSelectionRenderElements.SetNum(ActiveTargetReferences.Num());

	RemoveSets(UnselectedParams.Identifiers);
	RemoveSets(SelectedParams.Identifiers);
	RemoveSets(SoftSelectedParams.Identifiers);
...
void UGeometrySelectionManager::MarkRenderCachesDirty(bool bMarkSelectableDirty)
{
	...
	RenderCachesDirtyFlags |= EEnumerateRenderCachesDirtyFlags::PreviewCachesDirty | EEnumerateRenderCachesDirtyFlags::SelectionCachesDirty | EEnumerateRenderCachesDirtyFlags::SoftSelectionCachesDirty;
}
...
void UGeometrySelectionManager::RebuildSelectionRenderCaches()
{
	...
	RebuildSoftSelectionRenderCache();
}
...
void UGeometrySelectionManager::RebuildSelectableRenderCache()
{
	...
	check(ActiveTargetReferences.Num() == CachedSelectableRenderElements.Num());
	for (int32 k = 0; k < ActiveTargetReferences.Num(); ++k)
	{
		TSharedPtr<FGeometrySelectionTarget> Target = ActiveTargetReferences[k];
...
void UGeometrySelectionManager::RebuildSoftSelectionRenderCache()
{
	if ((RenderCachesDirtyFlags & EEnumerateRenderCachesDirtyFlags::SoftSelectionCachesDirty) == EEnumerateRenderCachesDirtyFlags::None)
	{
		return;
	}

	check(ActiveTargetReferences.Num() == CachedSoftSelectionRenderElements.Num());
	for (int32 k = 0; k < ActiveTargetReferences.Num(); ++k)
	{
		TSharedPtr<FGeometrySelectionTarget> Target = ActiveTargetReferences[k];

		FGeometrySelection SoftSelectionEdges;
		SoftSelectionEdges.TopologyType = GetSelectionTopologyType();
		SoftSelectionEdges.ElementType = EGeometryElementType::Edge;
		
		Target->Selector->ConvertSelection(Target->Selection.GetSoftSelectedVertices(), SoftSelectionEdges, EEnumerateSelectionConversionParams::ExpandSelection);
		
		FGeometrySelectionElements& SoftSelectionElements = CachedSoftSelectionRenderElements[k];
		SoftSelectionElements.Reset();

		Target->Selector->AccumulateSelectionElements(SoftSelectionEdges, SoftSelectionElements, true, EEnumerateSelectionMapping::Default);

		CreateOrUpdateAllSets(SoftSelectionElements, SoftSelectedParams);
	}
...
void UGeometrySelectionManager::DebugPrintSelection()
{
	if (ActiveTargetReferences.Num() == 0)
	{
		UE_LOGF(LogGeometry, Warning, "[SelectionManager] No Active Selection");
		return;
	}
	...
	UE_LOGF(LogGeometry, Warning, "[SelectionManager] %d selected items in %d active targets", NumSelected, ActiveTargetReferences.Num());
}
...
void UGeometrySelectionManager::SetSelectionColors(
	...
	FLinearColor SoftSelectedCol)
{
	...
	SoftSelectedParams.Color = SoftSelectedCol.ToFColor(true);
	...
	if (UnselectedParams.SelectionFillColor == nullptr) // if one is null, they all will be
	{
		...
		SetMaterial(SoftSelectedParams, SoftSelectedCol);
	...
	else
	{
		...
		SetColorAndOpacity(SoftSelectedParams, SoftSelectedCol);
	}
	...
	UpdateAllSetsColor(SoftSelectedParams);
}
