...
#include "Mechanics/PolyLassoMarqueeMechanic.h" // FCameraPolyLasso
...
bool FMeshTopologySelector::FindSelectedElement(const FSelectionSettings& Settings,
	const FCameraPolyLasso& CameraLasso, FTransform3d TargetTransform, FGroupTopologySelection& ResultOut,
	TMap<int32, bool>* TriIsOccludedCache)
{
	// One minor easy thing we can do to speed up the below is to detect cases where transform does not have a non-uniform
	// scale, and in those cases we transform the rectangle once rather than transforming all the query points/curves. 
	...
	// Needed for occlusion test, which happens in local space.
	const FVector3d LocalCameraOrigin = TargetTransform.InverseTransformPosition((FVector3d)CameraLasso.CameraOrigin);

	auto IsPointInsidePolyLine = [](const TArray<FVector2D>& InPolyLine, const FVector2D& Vertex2DPos)
		{
			const int32 NumPolyPoints = InPolyLine.Num();
			if (NumPolyPoints > 0)
			{
				double WindingSum = 0;
				FVector2D Prev = (InPolyLine[0] - Vertex2DPos);
				FVector2D Curr = FVector2D::Zero();
				for (int32 PointIndex = 1; PointIndex < NumPolyPoints; ++PointIndex)
				{
					Curr = (InPolyLine[PointIndex] - Vertex2DPos);
					WindingSum += (double)FMathf::Atan2(Prev.X * Curr.Y - Prev.Y * Curr.X, Prev.X * Curr.X + Prev.Y * Curr.Y);
					Prev = Curr;
				}
				WindingSum /= FMathd::TwoPi;
				return FMathd::Abs(WindingSum) > 0.3;
			}
			return false;
		};

	auto VertexFilter = [&CameraLasso, &TargetTransform, &Settings, &LocalCameraOrigin, Spatial, &IsPointInsidePolyLine]
	(const FVector3d& PointPosition)

		{
			const FVector TransformedPoint = TargetTransform.TransformPosition(PointPosition);
			const FVector2D Vertex2DPos = CameraLasso.GetProjectedPoint(TransformedPoint);
			const bool bIsInside = IsPointInsidePolyLine(CameraLasso.Polyline, Vertex2DPos);
			return bIsInside
				&& (Settings.bIgnoreOcclusion || !IsOccluded(PointPosition, LocalCameraOrigin,
					Spatial, Settings.bHitBackFaces));
		};
	...
	if (Settings.bEnableCornerHits)
	{
		GeometrySet.ParallelFindAllPointsSatisfying(VertexFilter, ResultOut.SelectedCornerIDs);
	}

	auto SegmentIntersectPolyLine = [IsPointInsidePolyLine](const TArray<FVector2D>& InPolyLine, const FSegment2d& Segment2D, bool TestPoints = true)
		{
			if (IsPointInsidePolyLine(InPolyLine, Segment2D.StartPoint()))
			{
				return true;
			}
			if (IsPointInsidePolyLine(InPolyLine, Segment2D.EndPoint()))
			{
				return true;
			}
			...
			if (NumPolyPoints > 1)
			{
				for (int32 PointIndex = 1; PointIndex < NumPolyPoints; ++PointIndex)
				{
					const FSegment2d PolyLineSegment(InPolyLine[PointIndex - 1], InPolyLine[PointIndex]);
					if (PolyLineSegment.Intersects(Segment2D))
					{
						return true;
					}
				}
	...
	auto CurveFilter = [&CameraLasso, &TargetTransform, &Settings, &LocalCameraOrigin, Spatial, SegmentIntersectPolyLine](const FPolyline3d& Curve)
		{
			// Testing occlusion properly seems like it would be a pain, so we'll just consider something occluded
			// if one of the endpoints is occluded. This will handle the common case of not wanting to select a
			// hidden edge that is connected to a visible corner.
			...
			// Check whether any of the component segments intersect the rectangle
			const TArray<FVector3d>& Verts = Curve.GetVertices();
			FVector2D CurrentVert = CameraLasso.GetProjectedPoint(TargetTransform.TransformPosition(Verts[0]));
			for (int32 VertIndex = 1; VertIndex < Verts.Num(); ++VertIndex)
			{
				const FVector2D NextVert = CameraLasso.GetProjectedPoint(TargetTransform.TransformPosition(Verts[VertIndex]));
				const FSegment2d Segment(CurrentVert, NextVert);
				if (SegmentIntersectPolyLine(CameraLasso.Polyline, Segment))
				{
					// We consider an edge non occluded if at least one point is not
					// This is not perfect but it is a good approximation most most use case and still remain easy to evaluate
					if (Settings.bIgnoreOcclusion ||
						!IsOccluded(Verts[VertIndex - 1], LocalCameraOrigin, Spatial, Settings.bHitBackFaces) ||
						!IsOccluded(Verts[VertIndex], LocalCameraOrigin, Spatial, Settings.bHitBackFaces)
						)
					{
						return true;
					}
	...
	if (ResultOut.SelectedCornerIDs.IsEmpty() && Settings.bEnableEdgeHits)
	{
		GeometrySet.ParallelFindAllCurvesSatisfying(CurveFilter, ResultOut.SelectedEdgeIDs);
	}

	if (Settings.bEnableFaceHits && ResultOut.IsEmpty() && Mesh && TopologyProvider)
	{
		// check if the edges of triangle intersect with the lasso or if any points are inside of it 
		auto TriangleIntersectPolyLine = [IsPointInsidePolyLine](const TArray<FVector2D>& InPolyLine, const FVector2D& Vtx0, const FVector2D& Vtx1, const FVector2D& Vtx2)
			{
				const int32 NumPolyPoints = InPolyLine.Num();
				if (NumPolyPoints > 1)
				{
					const FSegment2d Segment0(Vtx0, Vtx1);
					const FSegment2d Segment1(Vtx1, Vtx2);
					const FSegment2d Segment2(Vtx2, Vtx0);

					FVector WindingSums{ 0,0,0 };
					for (int32 PointIndex = 1; PointIndex < NumPolyPoints; ++PointIndex)
					{
						const FVector2d PrevVert = InPolyLine[PointIndex - 1];
						const FVector2d CurrVert = InPolyLine[PointIndex + 0];
						const FSegment2d PolyLineSegment(PrevVert, CurrVert);
						if (PolyLineSegment.Intersects(Segment0) ||
							PolyLineSegment.Intersects(Segment1) ||
							PolyLineSegment.Intersects(Segment2) )
						{
							return true;
						}
						const FVector2d Prev0 = PrevVert - Vtx0;
						const FVector2d Curr0 = CurrVert - Vtx0;
						WindingSums[0] += (double)FMathf::Atan2(Prev0.X * Curr0.Y - Prev0.Y * Curr0.X, Prev0.X * Curr0.X + Prev0.Y * Curr0.Y);
						const FVector2d Prev1 = PrevVert - Vtx1;
						const FVector2d Curr1 = CurrVert - Vtx1;
						WindingSums[1] += (double)FMathf::Atan2(Prev1.X * Curr1.Y - Prev1.Y * Curr1.X, Prev1.X * Curr1.X + Prev1.Y * Curr1.Y);
						const FVector2d Prev2 = PrevVert - Vtx2;
						const FVector2d Curr2 = CurrVert - Vtx2;
						WindingSums[2] += (double)FMathf::Atan2(Prev2.X * Curr2.Y - Prev2.Y * Curr2.X, Prev2.X * Curr2.X + Prev2.Y * Curr2.Y);
					}
					WindingSums /= FMathd::TwoPi;

					return(
						FMathd::Abs(WindingSums[0]) > 0.3 ||
						FMathd::Abs(WindingSums[1]) > 0.3 ||
						FMathd::Abs(WindingSums[2]) > 0.3
						);
				}
				return false;
			};
		...
		TArray<bool> SelectedTriangles;
		SelectedTriangles.Init(false, MaxTriangleID);

		ParallelFor(Mesh->MaxTriangleID(), 
			[this, &SelectedTriangles/*, &VerticesInLasso*/, TriangleIntersectPolyLine, &CameraLasso, &TargetTransform](int32 TriId)
			{
				if (Mesh->IsTriangle(TriId))
				{
					const FIndex3i TriIndices = Mesh->GetTriangle(TriId);
					const FVector2D Vtx0 = CameraLasso.GetProjectedPoint(TargetTransform.TransformPosition(Mesh->GetVertex(TriIndices[0])));
					const FVector2D Vtx1 = CameraLasso.GetProjectedPoint(TargetTransform.TransformPosition(Mesh->GetVertex(TriIndices[1])));
					const FVector2D Vtx2 = CameraLasso.GetProjectedPoint(TargetTransform.TransformPosition(Mesh->GetVertex(TriIndices[2])));
					if (TriangleIntersectPolyLine(CameraLasso.Polyline, Vtx0, Vtx1, Vtx2))
					{
						SelectedTriangles[TriId] = true;
					}
				}
			});

		// Now add to the selection array 
		for (int32 TriId = 0; TriId < MaxTriangleID; ++TriId)
		{
			if (SelectedTriangles[TriId])
			{
				ResultOut.SelectedGroupIDs.Add(TopologyProvider->GetGroupIDForTriangle(TriId));
			}
		}
	}
