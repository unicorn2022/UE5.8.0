...
void UPolygonSelectionMechanicProperties::GrowSelection()
{
	if (UPolygonSelectionMechanic* PolygonMechanic = Cast<UPolygonSelectionMechanic>(Mechanic.Get()))
	{
		PolygonMechanic->GrowSelection(/*bAsTriangleTopology*/ true);
	}
}

void UPolygonSelectionMechanicProperties::ShrinkSelection()
{
	if (UPolygonSelectionMechanic* PolygonMechanic = Cast<UPolygonSelectionMechanic>(Mechanic.Get()))
	{
		PolygonMechanic->ShrinkSelection(/*bAsTriangleTopology*/ true);
	}
}

void UPolygonSelectionMechanicProperties::FloodSelection()
{
	if (UPolygonSelectionMechanic* PolygonMechanic = Cast<UPolygonSelectionMechanic>(Mechanic.Get()))
	{
		PolygonMechanic->FloodSelection();
	}
}

void UPolygonSelectionMechanicProperties::ConvertToBorder()
{
	if (UPolygonSelectionMechanic* PolygonMechanic = Cast<UPolygonSelectionMechanic>(Mechanic.Get()))
	{
		PolygonMechanic->ConvertSelectionToBorderVertices(/*bAsTriangleTopology*/ true);
	}
}
...
UMeshTopologySelectionMechanicProperties* UPolygonSelectionMechanic::CreateProperties()
{
	return NewObject<UPolygonSelectionMechanicProperties>(this);
}
...
void UPolygonSelectionMechanic::SetSelection_AsGroupTopology(const UE::Geometry::FGeometrySelection& Selection, const bool bBroadcastSelectionChanged)
{
	...
	TOptional<FGroupTopologySelection> PreviousSelection;
	if (bBroadcastSelectionChanged)
	{
		// Only remember the previous selection when broadcasting the change
	...
	if (bBroadcastSelectionChanged &&
		ensure(PreviousSelection.IsSet()) &&
		PersistentSelection != PreviousSelection.GetValue())
	{
		SelectionTimestamp++;
...
void UPolygonSelectionMechanic::SetSelection_AsTriangleTopology(const UE::Geometry::FGeometrySelection& Selection, const bool bBroadcastSelectionChanged)
{
...
void UPolygonSelectionMechanic::InvertSelection(bool bAsTriangleTopology)
{
	using namespace UE::Geometry;

	ExecuteActionThroughGeometrySelection(bAsTriangleTopology, LOCTEXT("InvertSelectionChange", "Invert Selection"),
		[this](FGeometrySelection& GeometrySelection)
		{
			FGeometrySelection NewSelection;
			NewSelection.InitializeTypes(GeometrySelection);

			bool bSuccess = MakeSelectAllSelection(
				*Topology->GetMesh(), Topology,
				[](FGeoSelectionID) { return true; },
				NewSelection);

			bSuccess = bSuccess && CombineSelectionInPlace(NewSelection, GeometrySelection,
				UE::Geometry::EGeometrySelectionCombineModes::Subtract);

			GeometrySelection = MoveTemp(NewSelection);
