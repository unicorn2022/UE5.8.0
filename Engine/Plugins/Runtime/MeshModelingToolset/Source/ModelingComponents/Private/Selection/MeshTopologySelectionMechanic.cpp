...
UMeshTopologySelectionMechanicProperties* UMeshTopologySelectionMechanic::CreateProperties()
{
	return NewObject<UMeshTopologySelectionMechanicProperties>(this);
}
...
void UMeshTopologySelectionMechanic::Setup(UInteractiveTool* ParentToolIn)
{
	...
	PolyLassoMechanic->Setup(ParentToolIn);
	PolyLassoMechanic->SetIsEnabled(false);
	PolyLassoMechanic->SpacingTolerance = 10.0f;
	PolyLassoMechanic->OnDrawPolyLassoStarted.AddUObject(this, &UMeshTopologySelectionMechanic::OnPolyLassoStarted);
	PolyLassoMechanic->OnDrawPolyLassoChanged.AddUObject(this, &UMeshTopologySelectionMechanic::OnPolyLassoChanged);
	PolyLassoMechanic->OnDrawPolyLassoFinished.AddUObject(this, &UMeshTopologySelectionMechanic::OnPolyLassoFinished);
	PolyLassoMechanic->SetBasePriority(BasePriority.MakeLower());
	...
	Properties->WatchProperty(Properties->SelectionDragTool, [this](EMeshTopologySelectionMechanicDragTool DragTool) {
		UpdateSelectorEnabled();
	});
...
void UMeshTopologySelectionMechanic::DisableBehaviors(UInteractiveTool* ParentToolIn)
{
	...
	ParentToolIn->RemoveInputBehaviorsBySource(PolyLassoMechanic);
...
void UMeshTopologySelectionMechanic::SetMarqueeSelectionUpdateType(EMarqueeSelectionUpdateType InType)
{
	...
	switch (MarqueeSelectionUpdateType)
	{
	case EMarqueeSelectionUpdateType::OnDrag:
		SetSelectionDragToolUpdateType(ESelectionDragToolUpdateType::OnDrag);
		break;
	case EMarqueeSelectionUpdateType::OnRelease:
		SetSelectionDragToolUpdateType(ESelectionDragToolUpdateType::OnRelease);
		break;
	case EMarqueeSelectionUpdateType::OnTickAndRelease:
		SetSelectionDragToolUpdateType(ESelectionDragToolUpdateType::OnTickAndRelease);
		break;
	}
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UMeshTopologySelectionMechanic::SetSelectionDragToolUpdateType(ESelectionDragToolUpdateType InType)
{
	SelectionDragToolUpdateType = InType;
}
...
void UMeshTopologySelectionMechanic::SetBasePriority(const FInputCapturePriority &Priority)
{
	...
	const FInputCapturePriority MarqueeAndLassoPriority = Priority.MakeLower();
	if (MarqueeMechanic)
	{
		MarqueeMechanic->SetBasePriority(MarqueeAndLassoPriority);
	}
	if (PolyLassoMechanic)
	{
		PolyLassoMechanic->SetBasePriority(MarqueeAndLassoPriority);
	}
}
...
void UMeshTopologySelectionMechanic::Render(IToolsContextRenderAPI* RenderAPI)
{
	...
	if (bIsEnabled)
	{
		...
		PolyLassoMechanic->Render(RenderAPI);
	}
...
void UMeshTopologySelectionMechanic::Tick(float DeltaTime)
{
	...
	if (SelectionDragToolUpdateType == ESelectionDragToolUpdateType::OnTickAndRelease && PendingSelectionFunction)
	{
...
void UMeshTopologySelectionMechanic::OnSelectionToolDragStarted()
{
	...
	PreDragTopoSelectorSettings.bIgnoreOcclusion = Properties->bIgnoreOcclusionForDragTools;
}

void UMeshTopologySelectionMechanic::OnSelectionToolDragFinished()
{
...
{
	OnSelectionToolDragFinished();
}

//////////////////////////////////////////////////////////////////////////////////////////////////////

void UMeshTopologySelectionMechanic::OnPolyLassoStarted()
{
	OnSelectionToolDragStarted();
}

void UMeshTopologySelectionMechanic::OnPolyLassoChanged(const FCameraPolyLasso& InLasso)
{
	if (SelectionDragToolUpdateType == ESelectionDragToolUpdateType::OnDrag)
	{
		HandleLassoChanged(InLasso);
	}
	else
	{
		// Defer the selection on tick or release  
		PendingSelectionFunction = [this, InLasso]()
			{
				HandleLassoChanged(InLasso);
			};
	}
...
namespace MeshTopologySelectionMechanic::Private
{

	static FCameraPolyLasso MakeClosedLasso(const FCameraPolyLasso& InLasso)
	{
		FCameraPolyLasso ClosedLasso{ InLasso };

		if (ClosedLasso.Polyline.Num() > 0)
		{
			const FVector2D FirstPoint = ClosedLasso.Polyline[0];
			if (FirstPoint != ClosedLasso.Polyline.Last())
			{
				ClosedLasso.Polyline.Add(FirstPoint);
			}
		}
		return ClosedLasso;
	}
...
void UMeshTopologySelectionMechanic::OnPolyLassoFinished(const FCameraPolyLasso& InLasso, bool bCanceled)
{
	OnSelectionToolDragFinished();
}

void UMeshTopologySelectionMechanic::HandleLassoChanged(const FCameraPolyLasso& InLasso)
{
	if (!bIsEnabled)
	{
		return;
	...
	FCameraPolyLasso ClosedLasso = MeshTopologySelectionMechanic::Private::MakeClosedLasso(InLasso);
	if (ClosedLasso.Polyline.Num() < 3)
	{
		// nothing to select really
		return;
	}

	FGroupTopologySelection RectangleSelection;

	TopoSelector->FindSelectedElement(PreDragTopoSelectorSettings, ClosedLasso, TargetTransform,
		RectangleSelection, &TriIsOccludedCache);
...
void UMeshTopologySelectionMechanic::UpdateSelectorEnabled()
{
...
{
	const bool bIsMarqueeEnabled = bIsEnabled
		&& Properties->SelectionDragTool == EMeshTopologySelectionMechanicDragTool::Marquee
	...
	if (bIsMarqueeEnabled != MarqueeMechanic->IsEnabled())
	{
		MarqueeMechanic->SetIsEnabled(bIsMarqueeEnabled);
		if (bIsMarqueeEnabled)
		{
			ClickOrDragBehavior->SetDragTarget(MarqueeMechanic.Get());
		}
	}
}

void UMeshTopologySelectionMechanic::UpdateLassoEnabled()
{
	const bool bIsPolyLassoEnabled = bIsEnabled
		&& Properties->SelectionDragTool == EMeshTopologySelectionMechanicDragTool::Lasso
		&& (Properties->bSelectVertices || Properties->bSelectEdges || Properties->bSelectFaces
			|| Properties->bSelectEdgeLoops || Properties->bSelectEdgeRings);
	if (bIsPolyLassoEnabled != PolyLassoMechanic->IsEnabled())
	{
		PolyLassoMechanic->SetIsEnabled(bIsPolyLassoEnabled);
		if (bIsPolyLassoEnabled)
		{
			ClickOrDragBehavior->SetDragTarget(PolyLassoMechanic.Get());
		}
	}
}
