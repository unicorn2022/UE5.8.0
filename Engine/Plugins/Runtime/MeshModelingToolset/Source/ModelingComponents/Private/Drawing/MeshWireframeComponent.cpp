...
public:
		...
		VertexFactory(GetScene().GetFeatureLevel(), "FPointSetSceneProxy")
	{
		...
		const bool bEnableGroupBoundaries = Component->bEnableGroupBoundaries;
		...
		ParallelFor(NumBatches, [&](int32 BatchIndex)
		{
			...
			for (int32 EdgeIndex = BatchIndex * BatchSize, BatchEnd = BatchIndex * BatchSize + CurrentBatchSize; EdgeIndex < BatchEnd; ++EdgeIndex)
			{
				...
				if (bEnableWireframe ||
					...
					(bEnableGroupBoundaries   && (static_cast<int>(EdgeType) & static_cast<int>(IMeshWireframeSource::EMeshEdgeType::GroupBoundary)) != 0) ||
		...
		const FColor GroupBoundaryColor = FLinearColor::FromSRGBColor(Component->GroupBoundaryColor).ToFColor(false);
		const float GroupBoundaryThickness = Component->ThicknessScale * Component->GroupBoundaryThickness;
		...
		ParallelFor(NumBatches, [&](int32 BatchIndex)
		{
			...
			for (int32 VisibleEdgeIndex = 0, Num = VisibleEdgeBatch.Num(); VisibleEdgeIndex < Num; ++VisibleEdgeIndex)
			{
				...
				if (EdgeType != IMeshWireframeSource::EMeshEdgeType::Regular)
				{
					...
					else if (bEnableGroupBoundaries && (static_cast<int>(EdgeType) & static_cast<int>(IMeshWireframeSource::EMeshEdgeType::GroupBoundary)) != 0)
					{
						UseThickness = GroupBoundaryThickness;
						UseColor = GroupBoundaryColor;
					}
