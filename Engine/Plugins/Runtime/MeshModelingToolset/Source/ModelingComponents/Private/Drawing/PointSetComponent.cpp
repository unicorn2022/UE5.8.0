...
int32 UPointSetComponent::AddPoints(const TArray<FRenderablePoint>& InPoints)
{
	const int NumPoints = InPoints.Num();
	if (NumPoints == 0)
	{
		return 0;
	}
...
FBoxSphereBounds UPointSetComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	...
	FBoxSphereBounds TransformBounds = Bounds.TransformBy(LocalToWorld);
	...
	if (Points.Num() > 0)
	{
		TransformBounds = TransformBounds.ExpandBy(Points[0].Size);
	}
