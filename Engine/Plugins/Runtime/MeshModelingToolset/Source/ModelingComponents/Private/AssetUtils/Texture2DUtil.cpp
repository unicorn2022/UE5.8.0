	...
	TImageBuilder<FVector4f>& DestImage)
{
	...
	else if (SourceFormat == TSF_R16F)
	{
		check(BytesPerPixel == sizeof(FFloat16));
		for (int64 i = 0; i < Num; ++i)
		{
			const uint8* PixelPtr = SourceDataPtr + (i * BytesPerPixel);
			const float Value = reinterpret_cast<const FFloat16*>(PixelPtr)->GetFloat();
			DestImage.SetPixel(i, FVector4f(Value, 0.0f, 0.0f, 1.f));
		}
		return true;
	}
