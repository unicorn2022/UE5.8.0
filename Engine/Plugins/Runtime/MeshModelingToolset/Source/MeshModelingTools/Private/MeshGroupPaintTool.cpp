...
void UMeshGroupPaintTool::Setup()
{
	...
	TFuture<void> OctreeFuture = Async(GroupPaintToolAsyncExecTarget, [&]()
	{
		...
		//UE_LOGF(LogTemp, Warning, "Octree Stats: %ls", *Stats.ToString());
	});
	...
	// Base class default is 0.01 which is too coarse for triangle painting at typical mesh scales.
	...
	PolygroupLayerProperties->bSupportLabels = true;
	...
	FilterProperties->SetGroup = IsPaintingLabel() ? LabelAdapter->GetMaxGroupID() : ActiveGroupSet->MaxGroupID;
	...
	{
		return GetColorForGroup(GetGroup(TriangleID));
	});
...
void UMeshGroupPaintTool::UpdateROI(const FSculptBrushStamp& BrushStamp)
{
	...
	const int32 SetGroupID = GetInEraseStroke() ? GetGroupToErase() : GetGroupToSet();
	...
	else if (bGroupFill)
	{
		...
		for (int32 tid : TriangleROI)
		{
			const int32 Group = GetGroup(tid);
			if (Group != SetGroupID)
			{
				StartROI.Add(tid);
				FillGroups.Add(Group);
			}
		}
		FMeshConnectedComponents::GrowToConnectedTriangles(Mesh, StartROI, TriangleROI, &TempROIBuffer,
			[&](int t1, int t2)
		{
			return FillGroups.Contains(GetGroup(t2));
		});
	}
...
bool UMeshGroupPaintTool::SyncMeshWithGroupBuffer(FDynamicMesh3* Mesh)
{
	...
	for ( int32 k = 0; k < NumT; ++k)
	{
		...
		const int32 CurGroupID = GetGroup(TriIdx);
		...
		if (ROIGroupBuffer[k] != CurGroupID)
		{
			if (IsPaintingLabel())
			{
				ActiveLabelEditBuilder->SaveTriangle(TriIdx,
					LabelAdapter->GetValueFromGroup(CurGroupID),
					LabelAdapter->GetValueFromGroup(ROIGroupBuffer[k]));
			}
			else
			{
			...
			SetGroup(TriIdx, ROIGroupBuffer[k], *Mesh);
...
void UMeshGroupPaintTool::SetTrianglesToGroupID(const TSet<int32>& Triangles, int32 ToGroupID, bool bIsErase)
{
	...
	for (int32 tid : Triangles)
	{
		...
		if (bIsErase && FilterProperties->bOnlyEraseCurrent && CurGroupID != GetGroupToSet())
		{
			continue;
		}
	...
	{
		SetGroup(tid, ToGroupID, *GetSculptMesh());
	}
	SaveTriangleEdits();
...
void UMeshGroupPaintTool::DrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI)
{
	...
	auto GetGroupString = [GroupIdLayer = ActiveGroupSet.Get(), GroupLabelAdapter = LabelAdapter.Get()](int CursorHitTID)
	{
		if (GroupLabelAdapter)
		{
			return FString::Printf(TEXT("('%s')"), *GroupLabelAdapter->GetValue(CursorHitTID).ToString());
		}
		return FString::Printf(TEXT("%d"), GroupIdLayer->GetTriangleGroup(CursorHitTID));
	};
	...
	if (FilterProperties->bShowHitGroup)
	{
		...
		if (Mesh->IsTriangle(CursorHitTID))
		{
			...
			const FString CursorString = GetGroupString(CursorHitTID);
	...
	if (FilterProperties->bShowAllGroups)
	{
		if (bDrawGroupsDataValid == false)
		{
			if (IsPaintingLabel())
			{
				GroupVisualizationCache.UpdateGroupInfo_ConnectedComponents(*Mesh, *LabelAdapter);
			}
			else
			{
...
void UMeshGroupPaintTool::OnTick(float DeltaTime)
{
	...
	if (bPendingPickGroup || bPendingToggleFreezeGroup)
	{
		if (GetBrushTriangleID() >= 0 && IsStampPending() == false )
		{
			...
			{
				const int32 HitGroupID = GetGroup(GetBrushTriangleID());
				if (bPendingPickGroup)
				{
					FilterProperties->SetGroup = HitGroupID;
					if (IsPaintingLabel())
					{
						FilterProperties->SetLabel = LabelAdapter->GetValueFromGroup(HitGroupID);
					}
					NotifyOfPropertyChangeByTool(FilterProperties);
				}
...
void UMeshGroupPaintTool::FreezeOtherGroups(int32 KeepGroupID)
{
	...
	for (int32 tid : Mesh->TriangleIndicesItr())
	{
		int32 GroupID = GetGroup(tid);
		if ( GroupID != 0 && GroupID != KeepGroupID)
		{
			FrozenGroups.AddUnique(GetGroup(tid));
			TempROIBuffer.Add(tid);
		}
	}
...
void UMeshGroupPaintTool::ClearAllFrozenGroups()
{
	...
	for (int32 tid : Mesh->TriangleIndicesItr())
	{
		if ( FrozenGroups.Contains(GetGroup(tid)) )
		{
			TempROIBuffer.Add(tid);
		}
	}
...
void UMeshGroupPaintTool::GrowCurrentGroupAction()
{
	...
	const int32 CurrentGroupID = GetGroupToSet();
	const FDynamicMesh3* Mesh = DynamicMeshComponent->GetMesh();
	FMeshFaceSelection InitialSelection(Mesh);
	InitialSelection.Select([&](int32 tid) { return GetGroup(tid) == CurrentGroupID; });
	FMeshFaceSelection ExpandSelection(InitialSelection);
	ExpandSelection.ExpandToOneRingNeighbours([&](int32 tid) { return FrozenGroups.Contains(GetGroup(tid)) == false; });
	TempROIBuffer.SetNum(0, EAllowShrinking::No);
	...
	for (int32 tid : TempROIBuffer)
	{
		SetGroup(tid, CurrentGroupID, *GetSculptMesh());
	}
	SaveTriangleEdits();
...
void UMeshGroupPaintTool::ClearCurrentGroupAction()
{
	...
	for (int32 tid : Mesh->TriangleIndicesItr())
	{
		if (GetGroup(tid) == CurrentGroupID)
		{
			TempROIBuffer.Add(tid);
		}
	}
...
void UMeshGroupPaintTool::FloodFillCurrentGroupAction()
{
	...
	const int32 SetGroupID = GetGroupToSet();
	const FDynamicMesh3* Mesh = DynamicMeshComponent->GetMesh();
	TempROIBuffer.SetNum(0, EAllowShrinking::No);
	for (int32 tid : Mesh->TriangleIndicesItr())
	{
		const int32 GroupID = GetGroup(tid);
	...
	for (int32 tid : TempROIBuffer)
	{
		SetGroup(tid, SetGroupID, *GetSculptMesh());
	}
	SaveTriangleEdits();
...
{
	check(ActiveGroupEditBuilder == nullptr && ActiveLabelEditBuilder == nullptr);
	
	if (IsPaintingLabel())
	{
		ActiveLabelEditBuilder = MakeUnique<FDynamicMeshTriangleLabelEditBuilder>(LabelAdapter.Get());
	}
	else
	{
...
{
	check(ActiveGroupEditBuilder || ActiveLabelEditBuilder);

	static const FText Description = LOCTEXT("GroupPaintChange", "Group Stroke");
	if (ActiveLabelEditBuilder)
	{
		TUniquePtr<FDynamicMeshTriangleLabelEdit> EditResult = ActiveLabelEditBuilder->ExtractResult();
		ActiveLabelEditBuilder = nullptr;

		TUniquePtr<TWrappedToolCommandChange<FMeshTriangleLabelChange>> NewChange = MakeUnique<TWrappedToolCommandChange<FMeshTriangleLabelChange>>();
		NewChange->WrappedChange = MakeUnique<FMeshTriangleLabelChange>(MoveTemp(EditResult));
		NewChange->BeforeModify = [this](bool bRevert)
		{
			this->WaitForPendingUndoRedoUpdate();
		};

		GetToolManager()->EmitObjectChange(DynamicMeshComponent, MoveTemp(NewChange), Description);
	}
	else
	{
...
void UMeshGroupPaintTool::SaveTriangleEdits() const
{
	if (IsPaintingLabel())
	{
		if (ensure(ActiveLabelEditBuilder))
		{
			ActiveLabelEditBuilder->SaveTriangles(TempROIBuffer);
		}
	}
	else
	{
		if (ensure(ActiveGroupEditBuilder))
		{
		...
		if (const FDynamicMeshAttributeSet* Attributes = Mesh->Attributes())
		{
			...
			if (Normals || UVs)
			{
				ParallelFor(Mesh->MaxEdgeID(), [&](int32 eid)
				{
					if (Mesh->IsEdge(eid))
					{
						UVSeamEdges[eid] = UVs ? UVs->IsSeamEdge(eid) : 0;
						NormalSeamEdges[eid] = Normals ? Normals->IsSeamEdge(eid) : 0;
					}
				});
			}
		}
...
int32 UMeshGroupPaintTool::GetGroup(const int32 TriangleID) const
{
	return IsPaintingLabel() ? LabelAdapter->GetGroup(TriangleID) : ActiveGroupSet->GetGroup(TriangleID);
}

void UMeshGroupPaintTool::SetGroup(const int32 TriangleID, const int32 GroupID, FDynamicMesh3& Mesh) const
{
	return IsPaintingLabel() ? LabelAdapter->SetGroup(TriangleID, GroupID, Mesh) : ActiveGroupSet->SetGroup(TriangleID, GroupID, Mesh);
}

int32 UMeshGroupPaintTool::GetGroupToSet() const
{
	return IsPaintingLabel() ? LabelAdapter->GetGroupFromValue(FilterProperties->SetLabel) : FilterProperties->SetGroup;
}

int32 UMeshGroupPaintTool::GetGroupToErase() const
{
	return IsPaintingLabel() ? LabelAdapter->GetGroupFromValue(FilterProperties->EraseLabel) : FilterProperties->EraseGroup;
}
...
{
	ActiveGroupSet.Reset();
	LabelAdapter.Reset();
	FilterProperties->bPaintingLabels = false;
	
	FDynamicMesh3* Mesh = GetSculptMesh();
	
	if (PolygroupLayerProperties->HasSelectedPolygroup() == false)
	{
		ActiveGroupSet = MakeUnique<FPolygroupSet>(Mesh);
	}
	else
	{
		FName SelectedName = PolygroupLayerProperties->ActiveGroupLayer;
		FDynamicMeshPolygroupAttribute* FoundAttrib = FindPolygroupLayerByName(*Mesh, SelectedName);
		if (!FoundAttrib)
		{
			if (FDynamicMeshTriangleLabelAttribute* Labels = FindTriangleLabelLayerByName(*Mesh, SelectedName))
			{
				LabelAdapter = MakeShared<FTriangleLabelAdapter>(Mesh, Labels);
				FilterProperties->bPaintingLabels = true;
			}
		}
		
		if (!LabelAdapter.IsValid())
		{
			ensureMsgf(FoundAttrib, TEXT("Selected Attribute Not Found! Falling back to Default group layer."));
			ActiveGroupSet = MakeUnique<FPolygroupSet>(Mesh, FoundAttrib);
		}
	}
	
	FilterProperties->bPaintingLabels = IsPaintingLabel();
...
void UMeshGroupPaintTool::ApplyAction(EMeshGroupPaintToolActions ActionType)
{
	...
	case EMeshGroupPaintToolActions::FreezeCurrent:
		ToggleFrozenGroup(GetGroupToSet());
		break;

	case EMeshGroupPaintToolActions::FreezeOthers:
		FreezeOtherGroups(GetGroupToSet());
		break;
