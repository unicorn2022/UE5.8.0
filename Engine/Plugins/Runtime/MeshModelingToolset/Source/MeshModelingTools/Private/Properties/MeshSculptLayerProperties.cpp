...
bool UMeshSculptLayerProperties::ReadSculptLayers(TFunctionRef<void(const UE::Geometry::FDynamicMesh3& Mesh, const UE::Geometry::FDynamicMeshSculptLayers* SculptLayers)> ReadFn)
{
	bool bFoundLayers = false;
	if (Tool)
	{
		Tool->ProcessToolMeshes([this, &ReadFn, &bFoundLayers](const UE::Geometry::FDynamicMesh3& Mesh, int32 Idx) -> void
			{
				// Sculpt layer UI only supports a single mesh for now
				if (Idx > 0)
				{
				...
				if (const UE::Geometry::FDynamicMeshAttributeSet* Attributes = Mesh.Attributes())
				{
					if (const UE::Geometry::FDynamicMeshSculptLayers* SculptLayers = Attributes->GetSculptLayers())
					{
						ReadFn(Mesh, SculptLayers);
						bFoundLayers = true;
					}
				}
			}
...
bool UMeshSculptLayerProperties::RemoveLayerAtIndex(const int32 IndexOfLayerToRemove)
{
	...
	{
		UE_LOGF(LogSculptLayerProps, Warning, "Sculpt Layer not removed. Invalid index: %d.", IndexOfLayerToRemove);
		return false;
	}
...
FName UMeshSculptLayerProperties::GetLayerName(const int32 InIndex)
{
	...
	{
		UE_LOGF(LogSculptLayerProps, Warning, "Sculpt Layer not found. Invalid index: %d.", InIndex);
		return NAME_None;
	}
	
	FName LayerName;
	ReadSculptLayers([this, &InIndex, &LayerName](const UE::Geometry::FDynamicMesh3& Mesh, const UE::Geometry::FDynamicMeshSculptLayers* SculptLayers)->void
		{
			if (const UE::Geometry::FDynamicMeshSculptLayerAttribute* Layer = SculptLayers->GetLayer(InIndex + NumLockedBaseLayers))
			{
...
void UMeshSculptLayerProperties::RefreshFromMesh()
{
	ReadSculptLayers([this](const UE::Geometry::FDynamicMesh3& Mesh, const UE::Geometry::FDynamicMeshSculptLayers* SculptLayers)->void
	{
		if (SculptLayers)
		{
			UpdateSettingsFromMesh(SculptLayers);
...
void UMeshSculptLayerProperties::MoveLayer(const int32 InInitIndex, int32 InTargetIndex)
{
	...
	{
		UE_LOGF(LogSculptLayerProps, Warning, "Sculpt Layer not moved. Invalid source index, %d.", InInitIndex);
		return;
	}

	if (!LayerWeights.IsValidIndex(InTargetIndex))
	{
		UE_LOGF(LogSculptLayerProps, Warning, "Sculpt Layer not moved. Invalid target index, %d.", InTargetIndex);
		return;
	}

	if (InInitIndex == InTargetIndex)
	{
		UE_LOGF(LogSculptLayerProps, Warning, "Sculpt Layer not moved. Source and target index cannot be the same.");
		return;
	}
