...
void UModifyGeometrySelectionCommand::ExecuteCommandForSelection(UGeometrySelectionEditCommandArguments* SelectionArgs, UInteractiveCommandResult** Result)
{
	...
	{
		UE_LOGF(LogGeometry, Warning, "UModifyGeometrySelectionCommand: Selection-Modify commands are no-ops unless Result is provided and used");
		return;
	}
	if (!ensure(SelectionArgs->SelectionHandle.Selector != nullptr))
	{
		UE_LOGF(LogGeometry, Warning, "UModifyGeometrySelectionCommand: Selection-Modify commands require Selector be provided in Selection Arguments");
		return;
	}
