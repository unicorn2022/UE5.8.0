...
namespace UE::PolygroupUtilLocals
{
	const TCHAR* NewLayer = TEXT("Create New...");
	const TCHAR* DefaultLayer = TEXT("Default");
}
...
public:
	...
	TSharedPtr<ITriangleGroupSetAdapter, ESPMode::ThreadSafe> SourcePolyGroups;
	...
	virtual void CalculateResult(FProgressCancel* Progress) override
	{
		...
		if (ConversionMode == EConvertToPolygonsMode::CopyFromLayer)
		{
			...
			{
				ResultMesh->SetTriangleGroup(tid, SourcePolyGroups->GetGroup(tid));
			}
			return;
		}
...
void UConvertToPolygonsTool::Setup()
{
	...
	CopyFromLayerProperties->bSupportLabels = true;
	...
	Settings->OptionsList.Add(UE::PolygroupUtilLocals::DefaultLayer);		// always have standard group
	if (FDynamicMeshAttributeSet* Attributes = OriginalDynamicMesh->Attributes())
	{
		for (int32 k = 0; k < Attributes->NumPolygroupLayers(); k++)
		{
			FName Name = Attributes->GetPolygroupLayer(k)->GetName();
			Settings->OptionsList.Add(Name.ToString());
		}
		for (const auto&[Name, LayerAttr]: Attributes->GetTriangleLabelAttributes())
		{
			Settings->OptionsList.Add(Name.ToString());
		}
	}
	
	Settings->OptionsList.Add(UE::PolygroupUtilLocals::NewLayer);
	
	Settings->WatchProperty(Settings->GroupLayer, [&](FName NewName)
	{
		Settings->bShowNewLayerName = (NewName == UE::PolygroupUtilLocals::NewLayer);
		
		if (NewName == UE::PolygroupUtilLocals::DefaultLayer)
		{
			Settings->LayerType = EPerTriangleAttributeType::Integer;
		}
		else if (NewName != UE::PolygroupUtilLocals::NewLayer)
		{
			if (FDynamicMeshAttributeSet* Attributes = OriginalDynamicMesh->Attributes())
			{
				bool bPolygroupFound = false;
				for (int32 k = 0; k < Attributes->NumPolygroupLayers(); k++)
				{
					if (Attributes->GetPolygroupLayer(k)->GetName() == NewName)
					{
						Settings->LayerType = EPerTriangleAttributeType::Integer;
						bPolygroupFound = true;
						break;
					}
				}
				...
				if (!bPolygroupFound)
				{
					if (Attributes->FindTriangleLabelAttribute(NewName))
					{
						Settings->LayerType = EPerTriangleAttributeType::Label;
					}
				}
			}
		}
...
void UConvertToPolygonsTool::UpdateOpParameters(FConvertToPolygonsOp& ConvertToPolygonsOp) const
{
	...
	if (ActiveFromGroupSetAdapter.IsValid())
	{
		ConvertToPolygonsOp.SourcePolyGroups = ActiveFromGroupSetAdapter;
		if (bUsingSelection)
		{
			ConvertToPolygonsOp.InitialGroupID = ActiveFromGroupSetAdapter->GetMaxGroupID();
		}
	}
...
void UConvertToPolygonsTool::OnShutdown(EToolShutdownType ShutdownType)
{
	...
	if (PreviewCompute)
	{
		...
		if (ShutdownType == EToolShutdownType::Accept)
		{
			...
			{
				if (Settings->GroupLayer != UE::PolygroupUtilLocals::DefaultLayer)
				{
					DoUpdateMeshAttribute(*DynamicMeshResult);
				}
				else
				{
...
void UConvertToPolygonsTool::UpdateFromGroupLayer()
{
	ComputeOperatorSharedMesh->AccessSharedObject([&](const FDynamicMesh3& ReadMesh)
	{
		...
		{
			ActiveFromGroupSetAdapter = MakeShared<FPolygroupLayerAdapter, ESPMode::ThreadSafe>(&ReadMesh, nullptr);
		}
		else
		{
			const FName SelectedName = CopyFromLayerProperties->ActiveGroupLayer;
			const FDynamicMeshPolygroupAttribute* FoundAttrib = FindPolygroupLayerByName(ReadMesh, SelectedName);
			if (!FoundAttrib)
			{
				if (const FDynamicMeshTriangleLabelAttribute* Labels = FindTriangleLabelLayerByName(ReadMesh, SelectedName))
				{
					ActiveFromGroupSetAdapter = MakeShared<FTriangleReadOnlyLabelAdapter>(&ReadMesh, Labels);
					return;
				}
			}
			
			ensureMsgf(FoundAttrib, TEXT("Selected Attribute Not Found! Falling back to Default group layer."));
			ActiveFromGroupSetAdapter = MakeShared<FPolygroupLayerAdapter, ESPMode::ThreadSafe>(&ReadMesh, FoundAttrib);
		}
	});
}
...
void UConvertToPolygonsTool::DoUpdateMeshAttribute(const FDynamicMesh3& InMesh) const
{
	if (Settings->GroupLayer == UE::PolygroupUtilLocals::DefaultLayer)
	{
		return;
	...
	{
	case EPerTriangleAttributeType::Integer:
	{
		...
		if (Settings->GroupLayer == UE::PolygroupUtilLocals::NewLayer)
		{
			...
			FString UseUniqueName = MakeUniqueGroupLayerName(UseResultMesh, Settings->NewLayerName);
			UseAttribLayer->SetName( FName(UseUniqueName) );
		}
		else
		{
			UseAttribLayer = FindPolygroupLayerByName(UseResultMesh, Settings->GroupLayer);
		}
		...
		if (UseAttribLayer)
		{
			...
			if (bUsingSelection)
			{
				for (int32 tid : *SelectionTriangleROI)
				{
					int32 GroupID = InMesh.GetTriangleGroup(OriginalSubmesh->MapTriangleToSubmesh(tid));
			...
			else
			{
				...
				{
					UseAttribLayer->SetValue(tid, InMesh.GetTriangleGroup(tid));
				}
			}
	...
	case EPerTriangleAttributeType::Label:
	{
		// if we want to write to any layer other than default, we have to find or create it
		FDynamicMeshTriangleLabelAttribute* LabelLayer = [&UseResultMesh, LayerName = Settings->GroupLayer, NewName = Settings->NewLayerName]
		{
			if (LayerName == UE::PolygroupUtilLocals::NewLayer)
			{
				FName NewAttrName(NewName);
				int32 NumberCounter = 0;	
				while (UseResultMesh.Attributes()->FindTriangleLabelAttribute(NewAttrName))
				{
					NewAttrName.SetNumber(NumberCounter++);
				}
				FDynamicMeshTriangleLabelAttribute* NewAttr = new FDynamicMeshTriangleLabelAttribute(&UseResultMesh);
				UseResultMesh.Attributes()->AttachTriangleLabelAttribute(NewAttrName, NewAttr);
				return NewAttr;
			}
			
			return UseResultMesh.Attributes()->FindTriangleLabelAttribute(LayerName);
		}();

		if (LabelLayer)
		{
			FTriangleLabelAdapter LabelAdapter(&UseResultMesh, LabelLayer);
			...
			if (bUsingSelection)
			{
				...
				{
					int32 GroupID = InMesh.GetTriangleGroup(OriginalSubmesh->MapTriangleToSubmesh(tid));
					LabelAdapter.SetGroup(tid, GroupID);
				}
			}
			else
			{
				for (int32 tid : UseResultMesh.TriangleIndicesItr())
				{
					LabelAdapter.SetGroup(tid, InMesh.GetTriangleGroup(tid));
				}
			}
			UE::ToolTarget::CommitDynamicMeshUpdate(Target, UseResultMesh, true);
		}
