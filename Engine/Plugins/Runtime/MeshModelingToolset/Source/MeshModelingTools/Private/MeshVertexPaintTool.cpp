...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarVertexPaintSuppressPostEditChangeCapture(
	TEXT("modeling.VertexPaint.SuppressPostEditChangeCapture"),
	true,
	TEXT("Suppress PostEditChange capture inside the Paint Vertex Colors commit transaction."));
#endif
...
void UMeshVertexPaintTool::Shutdown(EToolShutdownType ShutdownType)
{
	...
	if (ShutdownType == EToolShutdownType::Accept)
	{
		...
		{
			FSplitAttributeWelder::WeldSplitColors(vid, *ColorOverlay, static_cast<float>(FMathd::ZeroTolerance));
		}
...
#if WITH_EDITOR
	// Keep PostEditChange side effects out of the commit transaction.
	TOptional<UE::ToolTarget::Internal::FScopedSuppressPostEditChangeCapture> SuppressCapture;
	if (CVarVertexPaintSuppressPostEditChangeCapture.GetValueOnGameThread())
	{
		SuppressCapture.Emplace();
	}
#endif
