...
public:
	...
	void SetActiveHandle(const TArray<int>& FaceGroupIDs, const TArray<int>& TopologyEdgeIDs, const TArray<int>& TopologyCornerIDs) override;
...
void FConstrainedMeshDeformerTask::DoWork()
{
	...
	{
		//UE_LOGF(LogTemp, Warning, "Laplacian deformation failed");
	}
}
...
void FGroupTopologyLaplacianDeformer::SetActiveHandle(const TArray<int>& FaceGroupIDs, const TArray<int>& TopologyEdgeIDs,
	const TArray<int>& TopologyCornerIDs)
{
	// Currently mixing different types of handles is not yet supported.
	
	if (!FaceGroupIDs.IsEmpty())
	{
		check(TopologyEdgeIDs.IsEmpty() && TopologyCornerIDs.IsEmpty());
		SetActiveHandleFaces(FaceGroupIDs);
		return;
	}
	
	if (!TopologyEdgeIDs.IsEmpty())
	{
		check(FaceGroupIDs.IsEmpty() && TopologyCornerIDs.IsEmpty());
		SetActiveHandleEdges(TopologyEdgeIDs);
		return;
	}

	if (!TopologyCornerIDs.IsEmpty())
	{
		check(FaceGroupIDs.IsEmpty() && TopologyEdgeIDs.IsEmpty());
		SetActiveHandleCorners(TopologyCornerIDs);
		return;
	}
}
