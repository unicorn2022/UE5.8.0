...
#if WITH_EDITOR
	static EAsyncExecution AttributePaintToolAsyncExecTarget = EAsyncExecution::LargeThreadPool;
#else
	static EAsyncExecution AttributePaintToolAsyncExecTarget = EAsyncExecution::ThreadPool;
#endif
}
...
void UMeshAttributePaintTool::Setup()
{
	...
	BrushProperties->RestoreProperties(this, PropertyCacheId);

	AttribProps = NewObject<UMeshAttributePaintToolProperties>(this);
	AttribProps->RestoreProperties(this, PropertyCacheId);
	...
	PreviewMesh->EditMesh([this](FDynamicMesh3& Mesh)
	{
		...
		Mesh.Attributes()->PrimaryColors()->CreatePerVertex(0.f, true);
	...
	// assign materials
	if (IMaterialProvider* MaterialProvider = Cast<IMaterialProvider>(Target))
	{
	...
	// Enable render mesh decomposition for efficient partial updates during painting
...
void UMeshAttributePaintTool::OnEndDrag(const FRay& Ray)
{
	...
	{
		if (TUniquePtr<FMeshAttributePaintChange> Change = EndChange())
		{
...
void UMeshAttributePaintTool::CalculateVertexROI(const FBrushStampData& Stamp, TArray<int>& VertexROI)
{
	...
	if (IsFilteringTrianglesBySelection() || IsFilteringTrianglesByFrontFacing())
	{
		...
		VerticesOctree.ParallelRangeQuery(QueryBox,
	...
	if (CurrentAttributeIndex >= 0)
	{
		...
		PreviewMesh->EditMesh([&](FDynamicMesh3& Mesh)
		{
			...
			for (int32 vid : Mesh.VertexIndicesItr())
			{
				...
				// Note: This tool specifically constructs the color overlay element IDs to exactly match vertex IDs
				checkSlow(ColorOverlay->GetParentVertex(vid) == vid);
				ColorOverlay->SetElement(vid,  Color4f);
			}
		});
...
void UMeshAttributePaintTool::ApplyStamp(const FBrushStampData& Stamp)
{
	...
	// track changes
	TFuture<void> ChangeBuilderCompute;
	if (ActiveChangeBuilder)
	{
		ChangeBuilderCompute = Async(AttributePaintToolAsyncExecTarget, [this, &ActionData]()
		{
	...
	PreviewMesh->DeferredEditMesh([&](FDynamicMesh3& Mesh)
	{
		...
		for (int32 k = 0; k < NumVertices; ++k)
		{
			...
			// Note: This tool specifically constructs the color overlay element IDs to exactly match vertex IDs
			checkSlow(ColorOverlay->GetParentVertex(vid) == vid);
			ColorOverlay->SetElement(vid, NewColor);
			AttribData.Attribute->SetValue(vid, AttribData.CurrentValues[srcvid]);
			ElIDs.Reset();
		}
	}, false);

	// Updating the vertex colors is only required when the paint data is being displayed via the vertex color data
	if (ViewProperties->MaterialMode == EMeshAttributePaintMaterialMode::Shaded
		|| ViewProperties->MaterialMode == EMeshAttributePaintMaterialMode::ColorOnly)
	{
		PreviewMesh->NotifyVertexRegionDeferredEditCompleted(ActionData.ROIVertices, EMeshRenderAttributeFlags::VertexColors);
	}

	if (ActiveChangeBuilder)
	{
		ChangeBuilderCompute.Wait();
	}
}
...
void UMeshAttributePaintTool::UpdateMaterialMode(EMeshAttributePaintMaterialMode MaterialMode)
{
	...
	else if (MaterialMode == EMeshAttributePaintMaterialMode::ExistingMaterial)
	{
		ActiveOverrideMaterial = nullptr;
...
{
	const FString PropertyCacheId = GetPropertyCacheIdentifier();
	BrushProperties->SaveProperties(this, PropertyCacheId);
	BrushActionProps->SaveProperties(this, PropertyCacheId);
	ViewProperties->SaveProperties(this, PropertyCacheId);
...
FString UMeshAttributePaintTool::GetPropertyCacheIdentifier() const
{
	return TEXT("UMeshAttributePaintTool");
}
...
{
	if (!ensure(ActiveChangeBuilder)) // should be validated before calling EndChange
	{
		return nullptr;
	}
	...
	if (AttribIndex == CurrentAttributeIndex)
	{
		PreviewMesh->EditMesh([&](FDynamicMesh3& Mesh)
		{
			...
			for (int32 vid : VertexIndices)
			{
				...
				checkSlow(ColorOverlay->GetParentVertex(vid) == vid);
				ColorOverlay->SetElement(vid, NewColor);
				AttribData.Attribute->SetValue(vid, AttribData.CurrentValues[vid]);
			}
		});
	}
}
