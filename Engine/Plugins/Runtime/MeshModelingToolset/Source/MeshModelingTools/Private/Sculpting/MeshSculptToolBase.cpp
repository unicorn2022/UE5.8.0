...
void FBrushToolRadius::InitializeWorldSizeRange(TInterval<float> Range, bool bValidateWorldRadius)
{
	...
	{
		WorldRadius = WorldSizeRange.Interpolate(0.2f);
	}
	else if (WorldRadius > WorldSizeRange.Max)
	{
		WorldRadius = WorldSizeRange.Interpolate(0.8f);
	}
}
...
void UMeshSculptToolBase::Setup()
{
	...
	GizmoProperties->RestoreProperties(this, PropertyCacheId);
...
void UMeshSculptToolBase::Shutdown(EToolShutdownType ShutdownType)
{
	...
	{
		UE_LOGF(LogTemp, Error, "Tool Target has become Invalid (possibly it has been Force Deleted). Aborting Tool.");
	...
	if (GizmoProperties)
	{
		GizmoProperties->SaveProperties(this, PropertyCacheId);
	}
...
FString UMeshSculptToolBase::GetPropertyCacheIdentifier() const
{
	return TEXT("UMeshSculptToolBase");
}
...
void UMeshSculptToolBase::SaveAllBrushTypeProperties(UInteractiveTool* SaveFromTool)
{
	...
	for (auto Pair : BrushOpPropSets)
	{
		Pair.Value->SaveProperties(SaveFromTool, PropertyCacheId);
	}
...
void UMeshSculptToolBase::RestoreAllBrushTypeProperties(UInteractiveTool* RestoreToTool)
{
	...
	for (auto Pair : BrushOpPropSets)
	{
		Pair.Value->RestoreProperties(RestoreToTool, PropertyCacheId);
	}
	for (auto Pair : SecondaryBrushOpPropSets)
	{
		Pair.Value->RestoreProperties(RestoreToTool, PropertyCacheId);
...
{
	StrokePlane = ComputeStampRegionPlane(StampFrame, TriangleROI, false, bViewAligned);
}
...
void UMeshSculptToolBase::InitializeBrushSizeRange(const FAxisAlignedBox3d& TargetBounds)
{
	...
	BrushRelativeSizeRange = FInterval1d(MaxDimension * MinimumBrushAdaptiveSizeRatio, MaxDimension);
...
{
	constexpr EToolActionKeyRepeatMode KeyRepeatOnBrushAdjust = EToolActionKeyRepeatMode::Enabled;
