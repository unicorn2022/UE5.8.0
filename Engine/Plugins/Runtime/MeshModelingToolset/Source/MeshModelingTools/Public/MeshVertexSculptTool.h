...
public:
	...
	/** Whether to prevent sculpt brushes from moving mesh boundary vertices */
	UPROPERTY(EditAnywhere, Category = Sculpting)
	bool bLockBoundaries = false;

	/** Scale the rendered opacity of the on-surface brush stamp indicator */
	UPROPERTY(EditAnywhere, Category = Sculpting, AdvancedDisplay, meta = (ClampMin = "0.0", ClampMax = "1.0"))
	float StampOpacity = 1.0f;
...
private:
	...
	// Call to sync sculpt layer properties to the current mesh state
	void RefreshSculptLayerProperties();
...
protected:
	...
	UE_API virtual void SaveActiveStrokeModifiers() override;
...
protected:
	...
	/**
	 * Fetch the BrushFrame in World space with considerations for symmetry. When symmetry is enabled,
	 * the frame will always be in the positive side of the symmetry plane to simplify the mirror ROI
	 * computation. This should be used wherever we use the BrushFrame to compute an ROI to properly
	 * support symmetry.
	 * @return The brush frame in world space.
	 */
	FFrame3d GetSymmetricBrushFrameWorld() const;

	/**
	 * Fetch the BrushFrame in Local space with considerations for symmetry. When symmetry is enabled,
	 * the frame will always be in the positive side of the symmetry plane to simplify the mirror ROI
	 * computation. This should be used wherever we use the BrushFrame to compute an ROI to properly
	 * support symmetry.
	 * 
	 * @return The brush frame in local space.
	 */
	FFrame3d GetSymmetricBrushFrameLocal() const;
protected:
	...
	UE_DEPRECATED(5.8, "bUndoUpdatePending is no longer used")
	bool bUndoUpdatePending = false;
	UE_DEPRECATED(5.8, "UndoNormalsFuture is no longer used")
...
private:
	UE_API void ComputeVertexAndTriangleROI(const FFrame3d& LocalFrame);
...
protected:
	...
	// triangle octree used for raycast queries; for ROI queries we use the VertexOctree (in private scope, below)
...
protected:
	virtual bool ShowWorkPlane() const override { return GizmoProperties->bShowPlane && SculptProperties->PrimaryBrushID == (int32)EMeshVertexSculptBrushType::FixedPlane; }
...
private:
	...
	// internal implementation of OnDynamicMeshComponentChanged, exposing option to update component rendering as well
	void OnDynamicMeshComponentChanged_Internal(UDynamicMeshComponent* Component, const FMeshRegionChangeBase* Change, bool bRevert, bool bUpdateRender = false);

	double InitialBoundsMaxDim = 0;
	// Cached mesh connectivity for faster iteration
	TArray<TArray<int32, TInlineAllocator<8>>> CachedVtxTriConnectivity, CachedVtxVtxConnectivity;
	// Re-used index set for looking up the initial ROI during connected component growth
	UE::Geometry::FUniqueIndexSet VertexROILookup;
	// Octree used for vertex queries
	UE::Geometry::TDynamicVerticesOctree3<FDynamicMesh3> VertexOctree;
};
