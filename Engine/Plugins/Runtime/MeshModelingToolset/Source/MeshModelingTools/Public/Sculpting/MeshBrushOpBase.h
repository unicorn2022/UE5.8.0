...
public:
	...
	// Determines which vertices should be moved by the stroke
	enum class EROIUpdateType
	{
		// ROI is initialized from the first stamp and remains constant
		UseInitial,
		// ROI grows as new stamps overlap with more vertices
		UpdatePerStamp
	};

	// Determines which mesh should be used by ApplyStamp
	enum class EStampInputMeshType
	{
		// The "live" mesh that is updated by each stamp
		SculptMesh,
		// The "base" mesh that is typically updated after every stroke
	...
	virtual ~FMeshSculptBrushOp() = default;
	...
	virtual EROIUpdateType GetROIUpdateType() const
	{
		return EROIUpdateType::UpdatePerStamp;
	}

	virtual EStampInputMeshType GetStampInputMeshType() const
	{
		return EStampInputMeshType::SculptMesh;
	}
};
...
{
public:
	virtual ~FMeshSculptBrushOpFactory() = default;
