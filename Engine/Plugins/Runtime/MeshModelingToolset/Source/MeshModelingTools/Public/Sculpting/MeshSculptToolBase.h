...
public:
	...
	UPROPERTY(EditAnywhere, Category = TargetPlane, meta = (HideEditConditionToggle, EditCondition = "bPropertySetEnabled == true", ModelingQuickSettings))
	bool bShowGizmo = true;

	/** Toggle whether Work Plane is visible */
	UPROPERTY(EditAnywhere, Category = TargetPlane)
	bool bShowPlane = true;
...
protected:
	...
	// Should be overriden by subclasses so that settings are not shared across this tool and subclasses.
	 ...
	 */
	UE_API virtual void InitializeSculptMeshComponent(UBaseDynamicMeshComponent* Component, AActor* Actor);
...
protected:
	...
	double MinimumBrushAdaptiveSizeRatio = 0.01;
