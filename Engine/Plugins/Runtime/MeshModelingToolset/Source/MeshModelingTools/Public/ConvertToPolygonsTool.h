...
namespace UE::Geometry
{
	...
	class ITriangleGroupSetAdapter;
}
...
UENUM()
enum class EPerTriangleAttributeType
{
	/** Create per-triangle integer attribute. */
	Integer = 0 UMETA(DisplayName = "Int32"),
	/** Create per-triangle label attribute. */
	Label = 1 UMETA(DisplayName = "FName")
};
...
public:
	...
	/** Type of the new Group Layer */
	UPROPERTY(EditAnywhere, Category = Output, meta = (TransientToolProperty,
		EditCondition = "bShowNewLayerName", HideEditConditionToggle, NoResetToDefault))
	EPerTriangleAttributeType LayerType = EPerTriangleAttributeType::Integer;
};
...
protected:
	...
	TSharedPtr<UE::Geometry::ITriangleGroupSetAdapter, ESPMode::ThreadSafe> ActiveFromGroupSetAdapter;
	UE_API void OnSelectedFromGroupLayerChanged();
	UE_API void UpdateFromGroupLayer();
	
	// updates the group layer data to the mesh 
	void DoUpdateMeshAttribute(const FDynamicMesh3& InMesh) const;
};
