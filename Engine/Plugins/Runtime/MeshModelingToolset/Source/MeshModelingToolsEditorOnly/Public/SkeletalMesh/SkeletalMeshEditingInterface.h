...
class SWidget;
namespace UE::Geometry { class FMeshPlanarSymmetry; }
...
public:
	...
	UE_API virtual bool CanModifySkeleton() const;
	UE_API virtual TSharedPtr<SWidget> GetCustomSkeletonTreeWidget(TSharedPtr<ISkeletalMeshEditorBinding> EditorBinding);
...
private:
	...
	friend class FSkeletalMeshToolNotifier;
};
...
public:
	...
	// Base-mesh symmetry for the editing asset (ref pose, deltas excluded), or nullptr if unavailable.
	// Owned by the host; may be invalidated by edits that mutate base geometry.
	virtual const UE::Geometry::FMeshPlanarSymmetry* GetBaseMeshSymmetry() { return nullptr; }

	virtual const TArray<int32>& GetIsolatedTriangles() const { static const TArray<int32> Empty; return Empty; }
