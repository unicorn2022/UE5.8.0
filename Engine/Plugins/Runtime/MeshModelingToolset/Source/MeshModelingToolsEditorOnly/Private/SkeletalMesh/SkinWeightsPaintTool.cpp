...
#include "SkeletalMesh/SkinWeightsPaintToolPropertiesCustomVersion.h"
...
namespace UE::MeshModelingToolsEditorOnly::SelectionUtils
{
	/** Returns the geometry type that corresponds to a selection mode */
	UE::Geometry::EGeometryElementType GetGeometryElementTypeFromSelectionMode(const EComponentSelectionMode SelectionMode)
	{
		using namespace UE::Geometry;
		if (SelectionMode == EComponentSelectionMode::Vertices)
		{
			return EGeometryElementType::Vertex;
		}
		else if (SelectionMode == EComponentSelectionMode::Edges)
		{
			return EGeometryElementType::Edge;
		}
		else if (SelectionMode == EComponentSelectionMode::Faces)
		{
			return EGeometryElementType::Face;
		}
		else
		{
			ensureMsgf(0, TEXT("Unhandled selection mode"));
			return EGeometryElementType::Vertex;
		}
	}

	/** Util to convert selection from one selection mode to another */
	struct FWeightToolScopedConvertSelection
		: FNoncopyable
	{
		FWeightToolScopedConvertSelection(
			USkinWeightsPaintTool& WeightTool,
			const UE::Geometry::FDynamicMesh3& Mesh,
			const EComponentSelectionMode TargetSelectionMode)
		{
			using namespace UE::Geometry;

			UToolMeshSelector* MeshSelector = WeightTool.GetMainMeshSelector();
			SelectionMechanic = MeshSelector ? MeshSelector->GetSelectionMechanic() : nullptr;
			const USkinWeightsPaintToolProperties* ToolProperties = WeightTool.GetWeightToolProperties();

			if (ensure(ToolProperties && SelectionMechanic))
			{
				const EComponentSelectionMode SourceSelectionMode = ToolProperties->ComponentSelectionMode;
				if (SourceSelectionMode == TargetSelectionMode)
				{
					bPerformConversion = false;
					return;
				}

				FGeometrySelection SourceSelection;
				SourceSelection.TopologyType = EGeometryTopologyType::Triangle;
				SourceSelection.ElementType = GetGeometryElementTypeFromSelectionMode(SourceSelectionMode);
				
				SelectionMechanic->GetSelection_AsTriangleTopology(SourceSelection);
				
				const auto VerticesToEdges = [&Mesh](const FGeometrySelection& InVertexSelection)
					{
						FGeometrySelection Result;						
						Result.ElementType = EGeometryElementType::Edge;
						Result.TopologyType = EGeometryTopologyType::Triangle;

						for (const int EdgeID : Mesh.EdgeIndicesItr())
						{
							const FDynamicMesh3::FEdge Edge = Mesh.GetEdge(EdgeID);
							if (InVertexSelection.Selection.Contains(Edge.Vert.A) &&
								InVertexSelection.Selection.Contains(Edge.Vert.B))
							{
								const FMeshTriEdgeID TriEdgeID = Mesh.GetTriEdgeIDFromEdgeID(EdgeID);
								Result.Selection.Add(TriEdgeID.Encoded());
							}
						}
				...
				const auto VerticesToFaces = [&Mesh](const FGeometrySelection& InVertexSelection)
					{
						FGeometrySelection Result;
						Result.ElementType = EGeometryElementType::Face;
						Result.TopologyType = EGeometryTopologyType::Triangle;

						for (const int32& TriangleID : Mesh.TriangleIndicesItr())
						{
							const FIndex3i Triangle = Mesh.GetTriangle(TriangleID);
							if (InVertexSelection.Selection.Contains(Triangle.A) &&
								InVertexSelection.Selection.Contains(Triangle.B) &&
								InVertexSelection.Selection.Contains(Triangle.C))
							{
								Result.Selection.Add(TriangleID);
							}
						}
				...
				const auto EdgesToVertices = [this, &Mesh](const FGeometrySelection& InVertexSelection)
					{
						FGeometrySelection Result;
						Result.ElementType = EGeometryElementType::Vertex;
						Result.TopologyType = EGeometryTopologyType::Triangle;

						for (const uint64 EdgeID : InVertexSelection.Selection)
						{
							const FMeshTriEdgeID TriEdgeID(EdgeID);	
							const int TriEdge = Mesh.GetTriEdge(TriEdgeID.TriangleID, TriEdgeID.TriEdgeIndex);
							const FDynamicMesh3::FEdge Edge = Mesh.GetEdge(TriEdge);
							
							Result.Selection.Add(Edge.Vert.A);
							Result.Selection.Add(Edge.Vert.B);
						}
						
						return Result;
					};
					
				const auto FacesToVertices = [this, &Mesh](const FGeometrySelection& InVertexSelection)
					{
						FGeometrySelection Result;
						Result.ElementType = EGeometryElementType::Vertex;
						Result.TopologyType = EGeometryTopologyType::Triangle;

						for (const uint64 TriangleID : InVertexSelection.Selection)
						{
							const FIndex3i Triangle = Mesh.GetTriangle(TriangleID);
							Result.Selection.Add(Triangle.A);
							Result.Selection.Add(Triangle.B);
							Result.Selection.Add(Triangle.C);
						}
						
						return Result;
					};
				
				if (SourceSelectionMode == EComponentSelectionMode::Vertices &&
					TargetSelectionMode == EComponentSelectionMode::Edges)
				{	
					TargetSelection = VerticesToEdges(SourceSelection);
				}
				else if (
					SourceSelectionMode == EComponentSelectionMode::Vertices &&
					TargetSelectionMode == EComponentSelectionMode::Faces)
				{	
					TargetSelection = VerticesToFaces(SourceSelection);
				}
				else if (
					SourceSelectionMode == EComponentSelectionMode::Edges &&
					TargetSelectionMode == EComponentSelectionMode::Vertices)
				{	
					TargetSelection = EdgesToVertices(SourceSelection);
				}
				else if (
					SourceSelectionMode == EComponentSelectionMode::Edges &&
					TargetSelectionMode == EComponentSelectionMode::Faces)
				{
					TargetSelection = EdgesToVertices(SourceSelection);
					TargetSelection = VerticesToFaces(TargetSelection);
				}
				else if (
					SourceSelectionMode == EComponentSelectionMode::Faces &&
					TargetSelectionMode == EComponentSelectionMode::Vertices)
				{	
					TargetSelection = FacesToVertices(SourceSelection);
				}
				else if (
					SourceSelectionMode == EComponentSelectionMode::Faces &&
					TargetSelectionMode == EComponentSelectionMode::Edges)
				{
					TargetSelection = FacesToVertices(SourceSelection);
					TargetSelection = VerticesToEdges(TargetSelection);
				}
				else
				{
					ensureMsgf(0, TEXT("Unhandled selection mode"));
		...
		~FWeightToolScopedConvertSelection()
		{
			if (bPerformConversion &&
				SelectionMechanic)
			{					
				// Restore selection
				constexpr bool bBroadcastSelectionChanged = true;
				SelectionMechanic->SetSelection_AsTriangleTopology(TargetSelection, bBroadcastSelectionChanged);
			}
		}
		
	private:
		bool bPerformConversion = true;
		
		UE::Geometry::FGeometrySelection TargetSelection;

		UPolygonSelectionMechanic* SelectionMechanic = nullptr;
	};
	
	enum class ESelectionRemapDirection : uint8
	{
		ToBaseMesh,
		ToSubmesh
	};

	/** Remap a selection using a submesh mapping. */
	static void RemapSelection(
		ESelectionRemapDirection Direction,
		const UE::Geometry::FGeometrySelection& InSelection,
		UE::Geometry::FGeometrySelection& OutSelection,
		const UE::Geometry::FDynamicSubmesh3& Submesh)
	{
		using namespace UE::Geometry;
		
		OutSelection.TopologyType = InSelection.TopologyType;
		OutSelection.ElementType = InSelection.ElementType;

		if (InSelection.ElementType == EGeometryElementType::Vertex)
		{
			Algo::Transform(InSelection.Selection, OutSelection.Selection,
				[&Submesh, Direction](const uint64 ElementID)
				{
					return Direction == ESelectionRemapDirection::ToBaseMesh
						? Submesh.MapVertexToBaseMesh(ElementID)
						: Submesh.MapVertexToSubmesh(ElementID);
				});
		}
		else if (InSelection.ElementType == EGeometryElementType::Edge)
		{
			for (const uint64 EdgeID : InSelection.Selection)
			{
				FMeshTriEdgeID TriEdgeID(EdgeID);
				TriEdgeID.TriangleID = Direction == ESelectionRemapDirection::ToBaseMesh
					? Submesh.MapTriangleToBaseMesh(TriEdgeID.TriangleID)
					: Submesh.MapTriangleToSubmesh(TriEdgeID.TriangleID);
				OutSelection.Selection.Add(TriEdgeID.Encoded());
			}
		}
		else if (InSelection.ElementType == EGeometryElementType::Face)
		{
			Algo::Transform(InSelection.Selection, OutSelection.Selection,
				[&Submesh, Direction](const uint64 Index)
				{
					return Direction == ESelectionRemapDirection::ToBaseMesh
						? Submesh.MapTriangleToBaseMesh(Index)
						: Submesh.MapTriangleToSubmesh(Index);
				});
		}
	}

	/**
	 * Retains the selection across isolation changes.
	 * Constructor: captures current selection and remaps to full mesh space.
	 * Destructor: remaps from full mesh space to whatever space the isolator is in at destruction time.
	 */
	struct FWeightToolScopedRetainSelection
		: FNoncopyable
	{
		FWeightToolScopedRetainSelection(
			UWeightToolSelectionIsolator& InIsolator)
			: Isolator(InIsolator)
		{
			using namespace UE::Geometry;

			USkinWeightsPaintTool& WeightTool = InIsolator.GetWeightTool();
			UToolMeshSelector* MeshSelector = WeightTool.GetMainMeshSelector();
			SelectionMechanic = MeshSelector ? MeshSelector->GetSelectionMechanic() : nullptr;
			const USkinWeightsPaintToolProperties* ToolProperties = WeightTool.GetWeightToolProperties();

			if (ToolProperties && SelectionMechanic)
			{
				const EComponentSelectionMode SelectionMode = ToolProperties->ComponentSelectionMode;
				const EGeometryElementType ElementType = GetGeometryElementTypeFromSelectionMode(SelectionMode);

				FGeometrySelection CurrentSelection;
				CurrentSelection.TopologyType = EGeometryTopologyType::Triangle;
				CurrentSelection.ElementType = ElementType;
				SelectionMechanic->GetSelection_AsTriangleTopology(CurrentSelection);

				// Remap to full mesh space if currently isolated
				if (Isolator.IsMeshIsolated())
				{
					RemapSelection(ESelectionRemapDirection::ToBaseMesh, CurrentSelection, FullMeshSelection, Isolator.GetPartialSubmesh());
				}
				else
				{
					FullMeshSelection = MoveTemp(CurrentSelection);
				}
			}
		}
		...
		~FWeightToolScopedRetainSelection()
		{
			...
			{
				// Remap from full mesh space to current space
				if (Isolator.IsMeshIsolated())
				{
					UE::Geometry::FGeometrySelection RestoredSelection;
					RemapSelection(ESelectionRemapDirection::ToSubmesh, FullMeshSelection, RestoredSelection, Isolator.GetPartialSubmesh());
					SelectionMechanic->SetSelection_AsTriangleTopology(RestoredSelection);
				}
				else
				{
					SelectionMechanic->SetSelection_AsTriangleTopology(FullMeshSelection);
				}
			}
		}

	private:
		UE::Geometry::FGeometrySelection FullMeshSelection;
		UWeightToolSelectionIsolator& Isolator;
		UPolygonSelectionMechanic* SelectionMechanic = nullptr;
	};
}
...
static void ShowEditorMessage(ELogVerbosity::Type InMessageType, const FText& InMessage)
{
	...
	case ELogVerbosity::Warning:
		UE_LOGF(LogMeshModelingToolsEditor, Warning, "%ls", *InMessage.ToString());
		break;
	case ELogVerbosity::Error:
		State = SNotificationItem::CS_Fail;
		UE_LOGF(LogMeshModelingToolsEditor, Error, "%ls", *InMessage.ToString());
		break;
	default:
...
const TCHAR* NewProfilePlaceHolderName()
{
	static const TCHAR* Name = TEXT("SkinWeightsPaintTool_NewProfilePlaceHolderName");
	return Name;
}
...
public:
	...
	enum class EWeightSource : uint8 { PreChange, Current };

	struct FWeightSourceScope
	{
		FWeightSourceScope(FPaintToolWeightsDataSource& InDataSource, EWeightSource InTransientSource)
			: DataSource(InDataSource), PreviousSource(InDataSource.Source)
		{
			DataSource.Source = InTransientSource;
		}
		~FWeightSourceScope()
		{
			DataSource.Source = PreviousSource;
		}
		FWeightSourceScope(const FWeightSourceScope&) = delete;
		FWeightSourceScope& operator=(const FWeightSourceScope&) = delete;

	private:
		FPaintToolWeightsDataSource& DataSource;
		EWeightSource PreviousSource;
	};
	...
	{
		return GetBuffer()[VertexID].Num();
	}

	virtual int32 GetBoneIndex(const int32 VertexID, const int32 Index) override
	{
		return GetBuffer()[VertexID][Index].BoneID;
	}

	virtual float GetBoneWeight(const int32 VertexID, const int32 Index) override
	{
		return GetBuffer()[VertexID][Index].Weight;
	}

	virtual float GetWeightOfBoneOnVertex(const int32 VertexID, const int32 BoneIndex) override
	{
		return Weights->GetWeightOfBoneOnVertex(BoneIndex, VertexID, GetBuffer());
	}

private:

	const TArray<VertexWeights>& GetBuffer() const
	{
		return Source == EWeightSource::Current ? Weights->CurrentWeights : Weights->PreChangeWeights;
	}

	const FSkinToolWeights* Weights = nullptr;
	EWeightSource Source = EWeightSource::PreChange;
};
...
USkinWeightsPaintToolProperties::USkinWeightsPaintToolProperties()
	: CustomVersion(ESkinWeightsPaintToolPropertiesCustomVersion::NoVersion)
{
	...
	// Multiply's slider value is the literal multiplier (1.0 = identity); default to a
	// gentle shrink so the brush has a visible effect out of the box.
	BrushConfigMultiply.Strength = 0.9f;

	LoadConfig();

	if (LinearColorRamp.IsEmpty())
	{
		const TArray<FLinearColor> DefaultColorRamp
		{
			FLinearColor(0.8f, 0.4f, 0.8f),	// Purple
			FLinearColor(0.f, 0.f, 0.5f),	// Dark Blue
		...
		float Time = 0.f;
		const float Delta = 1.f / (DefaultColorRamp.Num() - 1);
		for (const FLinearColor& LinearColor : DefaultColorRamp)
		{
			constexpr bool bOnlyRGB = true;
			LinearColorRamp.SetColorAtTime(Time, LinearColor, bOnlyRGB);
...
void USkinWeightsPaintToolProperties::RestoreProperties(UInteractiveTool* RestoreToTool, const FString& CacheIdentifier)
{
	...
	if (CustomVersion < ESkinWeightsPaintToolPropertiesCustomVersion::SkinWeightPaintToolUseLinearColorRamp)
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS		
		if (!ColorRamp.IsEmpty())
		{
			LinearColorRamp.Reset();
		
			constexpr bool bOnlyRGB = true;
			LinearColorRamp.SetColorAtTime(0.f, ColorRamp[0], bOnlyRGB);
			
			const float DeltaTime = ColorRamp.Num() > 1 ? 1.f / (ColorRamp.Num() - 1) : 0.f;
			for (int32 RampStepIndex = 1; RampStepIndex < ColorRamp.Num() - 1; RampStepIndex++)
			{
				if (!FMath::Lerp(ColorRamp[RampStepIndex - 1], ColorRamp[RampStepIndex + 1], .5f).Equals(ColorRamp[RampStepIndex]))
				{
					// SetColorAtTime doesn't adopt alpha 1, hence force it here
					const FLinearColor Color = FLinearColor(ColorRamp[RampStepIndex]).CopyWithNewOpacity(1.f);
					LinearColorRamp.SetColorAtTime(DeltaTime * RampStepIndex, Color, bOnlyRGB);
				}
			}

			LinearColorRamp.SetColorAtTime(1.f, FLinearColor(ColorRamp.Last()).CopyWithNewOpacity(1.f), bOnlyRGB);
		}
		PRAGMA_ENABLE_DEPRECATION_WARNINGS

		CustomVersion = ESkinWeightsPaintToolPropertiesCustomVersion::SkinWeightPaintToolUseLinearColorRamp;
		SaveProperties(RestoreToTool, CacheIdentifier);
		SaveConfig();
	}

	// Clamp restored per-mode Strength values to the slider's [0,1] range. Older configs
	// could hold values outside this range (e.g. BrushConfigRelax.Strength = 5 from the
	// pre-fix 0-10 slider) and would otherwise leave the slider stuck at its new max
	// until the user touched it.
	if (CustomVersion < ESkinWeightsPaintToolPropertiesCustomVersion::SkinWeightPaintToolClampStrengthToUnit)
	{
		BrushConfigAdd.Strength = FMath::Clamp(BrushConfigAdd.Strength, 0.f, 1.f);
		BrushConfigReplace.Strength = FMath::Clamp(BrushConfigReplace.Strength, 0.f, 1.f);
		BrushConfigRelax.Strength = FMath::Clamp(BrushConfigRelax.Strength, 0.f, 1.f);
		BrushConfigMultiply.Strength = FMath::Clamp(BrushConfigMultiply.Strength, 0.f, 1.f);

		CustomVersion = ESkinWeightsPaintToolPropertiesCustomVersion::SkinWeightPaintToolClampStrengthToUnit;
		SaveProperties(RestoreToTool, CacheIdentifier);
		SaveConfig();
	}

	// FalloffMode is always shared across every per-mode config. Radius is shared only when
	// the user has the Sync toggle on; with it off, each mode's stored radius is honoured.
	const FSkinWeightBrushConfig& Active = GetBrushConfig();
	if (bSyncBrushRadiusAcrossModes)
	{
		SetSharedRadius(Active.Radius);
	}
	SetSharedFalloffMode(Active.FalloffMode);
}
...
void UWeightToolTransferManager::SetSourceMesh(USkeletalMesh* InSkeletalMesh)
{
	...
	SkeletalMeshToolsHelper::SetupPreviewTangentMode(Cast<UDynamicMeshComponent>(SourcePreviewMesh->GetRootComponent()));
...
void UWeightToolTransferManager::TransferWeights()
{
	...
	const bool bSameProfile = WeightTool->GetActiveSkinWeightProfileName() == ToolProperties->SourceSkinWeightProfile;
...
void UWeightToolTransferManager::TransferWeightsFromOtherMeshOrSubset()
{
	...
	const FName TargetProfile = WeightTool->GetActiveSkinWeightProfileName();
...
{
	if (!WeightTool->ShouldProceedWithAllBonesUnlocked())
	{
		return;
	}
	...
	const bool bIsMeshIsolated = Isolator->IsMeshIsolated();
	...
	const FDynamicMesh3& TargetMesh = bIsMeshIsolated ? Isolator->GetPartialMesh() : InTargetMesh;
	...
	for (int32 VertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex)
	{
		...
		const int32 FullVertexID = bIsMeshIsolated ? Isolator->PartialToFullMeshVertexIndex(VertexID) : VertexID;
...
FName USkinWeightsPaintToolProperties::GetDesiredSkinWeightProfileForTool() const
{
	return bShowNewProfileName ? NewProfilePlaceHolderName() : SkinWeightProfileOption;
}
	...
	const TArray<VertexWeights>& InPreChangeWeights)
{
	...
	// record a modification of this vertex weight. Pruned influences are recorded
	// only in VerticesRemovedFrom below — keeping them out of NewWeights/OldWeights
	// stops ApplyEditsToCurrentWeights' scratchpad fold from re-adding the bone at
	// weight 0 after the strip-pruned pass has just removed it.
	...
	const TArray<FTransform>& PoseComponentSpace)
{
	...
	// Refresh polygroup-edges wireframe so its scene proxy follows the new vertex positions.
	if (Tool->MeshElementsDisplay)
	{
		Tool->MeshElementsDisplay->NotifyMeshChanged();
	}
	...
	)
{
	...
	if (FDynamicMeshVertexSkinWeightsAttribute* VertexSkinWeights = Mesh.Attributes()->GetSkinWeightsAttribute(Profile))
	{
		for (int32 VertexIndex : Mesh.VertexIndicesItr())
		{
			...
			for (TTuple<BoneIndex, float>& SingleBoneWeight : NormalizedWeights)
			{
				...
				{
					UE_LOGF(LogMeshModelingToolsEditor, Warning, "InitializeSkinWeights: Invalid bone index provided (%d); falling back to 0 (root) as bone index.", BoneIndex);
					BoneIndex = 0;
				}
				const FVector& RefPoseVertexPosition = Deformer.RefPoseVertexPositions[VertexIndex];
	...
	{
		UE_LOGF(LogMeshModelingToolsEditor, Warning, "FSkinToolWeights::InitializeSkinWeights : failed to find skin weights on the target mesh, all initial weights will be empty")
	}
	...
	// Weight-edit scratch buffers used by ApplyEditsToCurrentWeights; sized to vertex count
	// and reused between calls (the pad and the tracker are cleared in place per stamp).
	PerWeightEditScratchPad.SetNum(MaxVertexIndex);
	PerWeightEditVertexChangeTracker.Init(MaxVertexIndex);
...
bool FSkinToolWeights::IsInitialized() const
{
	return Tool.IsValid();
}
...
void FSkinToolWeights::CreateWeightEditForVertex(
	...
	FMultiBoneWeightEdits& WeightEdits)
{
	const TSet<int32>& LockedBoneIndices = Tool->GetFinalLockedBoneIndices();
	// this operation should never prune weights
	constexpr bool bPruneInfluence = false;

	if (LockedBoneIndices.Contains(BoneToHoldIndex))
	{
		return;
	}
	...
	for (const FVertexBoneWeight& VertexBoneData : VertexData)
	{
		...
		if (!LockedBoneIndices.Contains(VertexBoneData.BoneID))
		{
		...
		{
			TotalLockedWeights += VertexBoneData.Weight;
		}
	}

	float MaxNewWeight = 1.f - TotalLockedWeights;
	// clamp new weight
	NewWeightValue = FMath::Clamp(NewWeightValue, 0.0f, MaxNewWeight);
	
	// assigning full weight to this vertex?
	if (FMath::IsNearlyEqual(NewWeightValue, MaxNewWeight))
	{
		// in this case normalization is trivial, just assign the full weight directly and zero all others
		const float FullWeight = MaxNewWeight;
	...
	if (bVertexHasNoOtherWeightedInfluences)
	{
		...
		if (!RecordedBonesOnVertex.IsEmpty())
		{
			...
			const float WeightToDistribute = (MaxNewWeight - NewWeightValue) / RecordedBonesOnVertex.Num();
			for (int32 i=0; i<ValuesToNormalize.Num(); ++i)
			{
				const int32 BoneIndex = RecordedBonesOnVertex[i];
		...
		else
		{
			...
			if (ParentBoneIndex == BoneToHoldIndex || Tool->GetFinalLockedBoneIndices().Contains(ParentBoneIndex))
			{
			...
			else
			{
				...
				const float NewParentWeight = MaxNewWeight - NewWeightValue;
	...
	// and split the remainder amongst the recorded influences
	{
		...
		const float AvailableTotal = MaxNewWeight - NewWeightValue;

		// normalize weights into available space not set by current bone
		for (int32 i=0; i<ValuesToNormalize.Num(); ++i)
		{
			float NormalizedValue = 0.f;
			if (AvailableTotal > MinimumWeightThreshold)
			{
...
void FSkinToolWeights::ApplyEditsToCurrentWeights(const FMultiBoneWeightEdits& Edits, FVertexChangeTracker& InOutVertexChangeTracker)
{
	// Strip pruned influences first so the scratch pad seeds from a post-removal CurrentWeights.
	...
	// Pass 1: fold each (bone, vertex, weight) edit into the per-vertex scratchpad.
	for (const TTuple<BoneIndex, FSingleBoneWeightEdits>& BoneEdits : Edits.PerBoneWeightEdits)
	{
		const FSingleBoneWeightEdits& WeightEdits = BoneEdits.Value;
		const int32 EditedBoneIndex = WeightEdits.BoneIndex;
		check(EditedBoneIndex != INDEX_NONE);
		...
		for (const TTuple<int32, float>& NewWeight : WeightEdits.NewWeights)
		{
			...
			UnlimitedVertexWeights& VertexWeights = PerWeightEditScratchPad[VertexID];
			if (PerWeightEditVertexChangeTracker.Mark(VertexID))
			{
				// First touch this stamp: seed from CurrentWeights so subsequent edits
				// merge against the post-strip baseline.
				VertexWeights.Reserve(CurrentWeights[VertexID].Num());
				for (const FVertexBoneWeight& BW : CurrentWeights[VertexID])
				{
					VertexWeights.Emplace(BW.BoneID, BW.Weight);
				}
			}

			bool bFoundExisting = false;
			for (BoneWeight& Pair : VertexWeights)
			{
				if (Pair.Key == EditedBoneIndex)
				{
					Pair.Value = Weight;
					bFoundExisting = true;
					break;
			...
			if (!bFoundExisting)
			{
				VertexWeights.Emplace(EditedBoneIndex, Weight);
			}
		}
	}

	// Pass 2: sort+truncate (conditional), renormalize (conditional), patch back into CurrentWeights.
	constexpr float NormalizationTolerance = 1.e-4f;
	for (const int32 VertexID : PerWeightEditVertexChangeTracker.Vertices)
	{
		UnlimitedVertexWeights& VertexWeights = PerWeightEditScratchPad[VertexID];

		const bool bWasTruncated = VertexWeights.Num() > UE::AnimationCore::MaxInlineBoneWeightCount;
		if (bWasTruncated)
		{
			// Descending sort so SetNum drops the smallest weights from the tail; zero-weight
			// influences sink first.
			VertexWeights.Sort([](const BoneWeight& A, const BoneWeight& B)
			{
				return A.Value > B.Value;
			});
			VertexWeights.SetNum(UE::AnimationCore::MaxInlineBoneWeightCount, EAllowShrinking::No);
		}

		// Truncation above is the only path that legitimately denormalizes VertexWeights —
		// sort + SetNum drops the smallest weights from the tail. Every other entry point
		// either comes from CreateWeightEditForVertex or PruneWeights, both of which produce
		// NewWeights summing to 1 - TotalLockedWeights; the strip-pruned-influences pass +
		// survivor fold preserves that invariant.
		float Total = 0.f;
		for (const BoneWeight& Pair : VertexWeights)
		{
			Total += Pair.Value;
		}

		auto Renormalize = [&VertexWeights, Total]()
		{
			if (Total > KINDA_SMALL_NUMBER)
			{
				for (BoneWeight& Pair : VertexWeights)
		...
		if (bWasTruncated)
		{
			// Truncation dropped weight off the tail — renormalize the survivors.
			Renormalize();
		}
		else if (!ensure(FMath::IsNearlyEqual(Total, 1.0f, NormalizationTolerance)))
		{
			// Upstream edit producer emitted denormalized weights. Surface via ensure
			// (catches the bug in dev), then recover so users don't see broken weights.
			Renormalize();
		}

		// Patch in place so surviving bones keep their cached VertexInBoneSpace.
		SkinPaintTool::VertexWeights& Dest = CurrentWeights[VertexID];
		for (int32 i = Dest.Num() - 1; i >= 0; --i)
		{
			const BoneIndex Bone = Dest[i].BoneID;
			const bool bStillPresent = VertexWeights.ContainsByPredicate(
				[Bone](const BoneWeight& Pair) { return Pair.Key == Bone; });
			if (!bStillPresent)
			{
				Dest.RemoveAt(i);
			}
		...
		for (const BoneWeight& Pair : VertexWeights)
		{
			const int32 ExistingIdx = Dest.IndexOfByPredicate(
				[&Pair](const FVertexBoneWeight& BW) { return BW.BoneID == Pair.Key; });
			if (ExistingIdx != INDEX_NONE)
			{
				Dest[ExistingIdx].Weight = Pair.Value;
			}
			else
			{
				const FVector PosLocalToBone = Deformer.InvCSRefPoseTransforms[Pair.Key].TransformPosition(
					Deformer.RefPoseVertexPositions[VertexID]);
				Dest.Emplace(Pair.Key, PosLocalToBone, Pair.Value);
			}
		}
		Deformer.SetVertexNeedsUpdated(VertexID);

		VertexWeights.Reset();
		InOutVertexChangeTracker.Mark(VertexID);
	}

	PerWeightEditVertexChangeTracker.Clear();

	for (const TTuple<BoneIndex, FSingleBoneWeightEdits>& BoneWeightEdits : Edits.PerBoneWeightEdits)
	{
		UpdateIsBoneWeighted(BoneWeightEdits.Key);
	}
}
...
void FMeshSkinWeightsChange::RestoreVertexFromSnapshot(
	USkinWeightsPaintTool* Tool,
	const int32 VertexID,
	const VertexWeights& Snapshot)
{
	FSkinToolWeights& Weights = Tool->Weights;
	Weights.CurrentWeights[VertexID] = Snapshot;
	Weights.PreChangeWeights[VertexID] = Snapshot;
	Weights.Deformer.SetVertexNeedsUpdated(VertexID);
	Tool->PerColorUpdateVertexChangeTracker.Mark(VertexID);
}
...
void FMeshSkinWeightsChange::Apply(UObject* Object)
{
	...
	const TArray<int32>& ChangedVertices = PerTransactionVertexChangeTracker.Vertices;
	check(ChangedVertices.Num() == AfterStates.Num());
	for (int32 i = 0; i < ChangedVertices.Num(); ++i)
	{
		RestoreVertexFromSnapshot(Tool, ChangedVertices[i], AfterStates[i]);
	}

	for (const BoneIndex Bone : ChangedBones)
	{
		Tool->Weights.UpdateIsBoneWeighted(Bone);
	}
...
void FMeshSkinWeightsChange::Revert(UObject* Object)
{
	...
	const TArray<int32>& ChangedVertices = PerTransactionVertexChangeTracker.Vertices;
	check(ChangedVertices.Num() == BeforeStates.Num());
	for (int32 i = 0; i < ChangedVertices.Num(); ++i)
	{
		RestoreVertexFromSnapshot(Tool, ChangedVertices[i], BeforeStates[i]);
	}
...
void FMeshSkinWeightsChange::StoreMultipleWeightEdits(const FMultiBoneWeightEdits& WeightEdits)
{
	for (const TTuple<BoneIndex, FSingleBoneWeightEdits>& Pair : WeightEdits.PerBoneWeightEdits)
	{
		ChangedBones.Add(Pair.Key);
		for (const TTuple<int32, float>& NewWeight : Pair.Value.NewWeights)
		{
			PerTransactionVertexChangeTracker.Mark(NewWeight.Key);
		}
		for (const int32 VertexID : Pair.Value.VerticesAddedTo)
		{
			PerTransactionVertexChangeTracker.Mark(VertexID);
		}
		for (const int32 VertexID : Pair.Value.VerticesRemovedFrom)
		{
			PerTransactionVertexChangeTracker.Mark(VertexID);
		}
	}
...
void FMeshSkinWeightsChange::Close(USkinWeightsPaintTool* Tool)
{
	const FSkinToolWeights& Weights = Tool->Weights;
	const TArray<int32>& ChangedVertices = PerTransactionVertexChangeTracker.Vertices;
	BeforeStates.Reset(ChangedVertices.Num());
	AfterStates.Reset(ChangedVertices.Num());
	for (const int32 VertexID : ChangedVertices)
	{
		BeforeStates.Add(Weights.PreChangeWeights[VertexID]);
		AfterStates.Add(Weights.CurrentWeights[VertexID]);
	}
	// Mask is no longer needed once Close has frozen the Vertices list — Apply/Revert
	// only walk Vertices. Free the mask memory; keep Vertices for replay.
	PerTransactionVertexChangeTracker.Mask.Empty();
}

FSkinWeightsPaintToolCommands::FSkinWeightsPaintToolCommands()
	: TCommands<FSkinWeightsPaintToolCommands>
	(
		TEXT("SkinWeightsPaintTool"),
		NSLOCTEXT("Contexts", "SkinWeightsPaintToolCommands", "Skin Weight Paint Tool Commands"),
		NAME_None,
...
void FSkinWeightsPaintToolCommands::RegisterCommands()
{
	UI_COMMAND(IsolateSelectedBones, "Isolate Selected (Unlock)", "Isolate selected bones by locking all other bones, Skin weights for locked bones cannot be changed", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ToggleSelectedBonesLockState, "Toggle Selected Lock State", "Toggle the lock state for selected bones, Skin weights for locked bones cannot be changed", EUserInterfaceActionType::Button, FInputChord(EKeys::L));
	
	UI_COMMAND(LockSelectedBones, "Lock Selected", "Lock selected bones, Skin weights for locked bones cannot be changed", EUserInterfaceActionType::Button, FInputChord());	
	UI_COMMAND(UnlockSelectedBones, "Unlock Selected", "Unlock selected bones, Skin weights for locked bones cannot be changed", EUserInterfaceActionType::Button, FInputChord());
	
	UI_COMMAND(LockAllBones, "Lock All", "Lock all bones, Skin weights for locked bones cannot be changed", EUserInterfaceActionType::Button, FInputChord());	
	UI_COMMAND(UnlockAllBones, "Unlock All", "Unlock all bones, Skin weights for locked bones cannot be changed", EUserInterfaceActionType::Button, FInputChord());
	
	UI_COMMAND(BypassLocks, "Bypass Locks", "Disable skin weight locking, existing locks are preserved but disabled", EUserInterfaceActionType::ToggleButton, FInputChord(EModifierKey::Alt, EKeys::L));
...
void USkinWeightsPaintTool::Setup()
{
	...
	// Per-mesh cache key — mirrors UMorphTargetMaskTool (MorphTargetMaskTool.cpp:74-85).
	{
		FString AssetPath = TEXT("UnknownAsset");
		if (const ISkeletalMeshBackedTarget* SkeletalMeshBackedTarget = Cast<ISkeletalMeshBackedTarget>(Target))
		{
			if (const USkeletalMesh* SkeletalMesh = SkeletalMeshBackedTarget->GetSkeletalMesh())
			{
				AssetPath = SkeletalMesh->GetPathName();
			}
		}
		WeightToolProperties->PropertySetCacheIdentifier = MoveTemp(AssetPath);
	}
	WeightToolProperties->RestoreProperties(this, WeightToolProperties->PropertySetCacheIdentifier);
	WeightToolProperties->WeightTool = this;
	WeightToolProperties->bSpecifyRadius = true;
	// watch for skin weight layer changes
	WeightToolProperties->SkinWeightProfileOption = FSkeletalMeshAttributesShared::DefaultSkinWeightProfileName;
	int32 WatcherIndex = WeightToolProperties->WatchProperty(WeightToolProperties->SkinWeightProfileOption, [this](FName) { OnActiveSkinWeightProfileChanged(); });
	WeightToolProperties->SilentUpdateWatcherAtIndex(WatcherIndex);
	
	WeightToolProperties->NewSkinWeightProfile = GetValidNewProfileName(WeightToolProperties->NewSkinWeightProfile);
	...
	ActiveSkinWeightProfileName = WeightToolProperties->GetDesiredSkinWeightProfileForTool();
	
	// listen to linear color ramp changes
	WeightToolProperties->LinearColorRamp.OnColorCurveChangedDelegate.AddUObject(this, &USkinWeightsPaintTool::OnLinearColorRampChanged);
	...
	SkeletonReader->SetReferenceSkeleton(RefSkeleton);
	SkeletonReader->SetReadOnly(true);

	// Retained-mode visualization dedicated to polygroup boundary edges.
	// Created before the isolation init below so UpdateCurrentlyEditedMesh's NotifyMeshChanged
	// hook has a valid visualizer to talk to from the first call.
	MeshElementsDisplay = NewObject<UMeshElementsVisualizer>(this);
	MeshElementsDisplay->CreateInWorld(TargetWorld.Get(), PreviewMesh->GetTransform());
	if (ensure(MeshElementsDisplay->Settings))
	{
		// Only polygroup-boundary edges are rendered; the tool's "show polygroup edges" config
		// toggle drives the visualizer's overall bVisible.
		MeshElementsDisplay->Settings->ShowAllElements(false);
		MeshElementsDisplay->Settings->bShowGroupBoundaries = true;

		MeshElementsDisplay->Settings->bVisible           = WeightToolProperties->bShowGroupBoundaries;
		MeshElementsDisplay->Settings->GroupBoundaryColor = WeightToolProperties->GroupBoundaryColor;
		MeshElementsDisplay->Settings->ThicknessScale     = WeightToolProperties->GroupBoundaryThickness;
	}
	MeshElementsDisplay->SetMeshAccessFunction([this](UMeshElementsVisualizer::ProcessDynamicMeshFunc ProcessFunc)
	{
		if (PreviewMesh) { PreviewMesh->ProcessMesh(ProcessFunc); }
	});
	
	// run all initialization for mesh/weights.
	
	// seed the isolator and initialize on the submesh if default isolation is provided
	const TArray<int32>& DefaultIsolationTriangles = GetDefaultIsolationTriangles();
	if (!DefaultIsolationTriangles.IsEmpty())
	{
		// Then isolate to external boundary
		SelectionIsolator->SetTrianglesToIsolate(DefaultIsolationTriangles);
	...
	// Best-effort recovery of the per-mesh lock cache against the now-initialized
	// BoneNameToIndexMap. Names that no longer resolve (renamed/removed bones) drop
	// silently — under-locking beats mis-locking a name collision.
	LockedBoneIndices.Reset();
	for (const FName& BoneName : WeightToolProperties->LockedBoneNames)
	{
		const BoneIndex Index = GetBoneIndexFromName(BoneName);
		if (Index != INDEX_NONE)
		{
			LockedBoneIndices.Add(Index);
		}
	}
...
FName USkinWeightsPaintTool::GetActiveSkinWeightProfileName() const
{
	return ActiveSkinWeightProfileName;
}
...
void USkinWeightsPaintTool::SetupBrushStampIndicator()
{
	if (!BrushStampIndicator)
	{
		...
		if (UVolumetricBrushStampIndicator* VolumetricBrushIndicator = Cast<UVolumetricBrushStampIndicator>(BrushStampIndicator))
		{
			...
			// Keep the volumetric brush rendered with its normal material when the viewport is in a
			// wireframe view mode (Wireframe / LitWireframe / etc.) - otherwise the brush sphere
			// renders as wireframe lines and obscures the brush footprint while painting.
			if (UPreviewMesh* BrushIndicatorPreviewMesh = VolumetricBrushIndicator->GetPreviewMesh())
			{
				BrushIndicatorPreviewMesh->SetOverrideWireframeRenderMaterial(
					ToolSetupUtil::GetDefaultBrushVolumeMaterial(GetToolManager()));
			}
		}
	}
}
...
void USkinWeightsPaintTool::OnTick(float DeltaTime)
{
	...
	{
		// Manually tick watchers for since it is not a registered property set
		MeshElementsDisplay->Settings->CheckAndUpdateWatched();
	...
	if (bConfigDirty)
	{
		if (WeightToolProperties)
		{
			WeightToolProperties->SaveConfig();
		}
		bConfigDirty = false;
	}

	if (!PerColorUpdateVertexChangeTracker.IsEmpty())
	{
		UpdateVertexColorForSubsetOfVertices();
		PerColorUpdateVertexChangeTracker.Clear();
	}
...
void USkinWeightsPaintTool::OnLinearColorRampChanged(TArray<FRichCurve*> ChangedCurves)
{
...
{
	// Should always have a valid profile before initialization
	check(IsProfileValid(GetActiveSkinWeightProfileName()))
	
	// update the preview mesh in the viewport
	PreviewMesh->ReplaceMesh(InDynamicMesh);

	// Rebuild the element-visualizer wireframe against the new mesh topology.
	if (MeshElementsDisplay)
	{
	...
	Weights.Profile = GetActiveSkinWeightProfileName();
	Weights.InitializeSkinWeights(this, InDynamicMesh);
	// Size the per-tick color tracker to the new mesh; the previous mesh's tracker
	// may have been smaller (or larger, with isolation).
	PerColorUpdateVertexChangeTracker.Init(InDynamicMesh.MaxVertexID());
	bVertexColorsNeedUpdated = true;
	
	// update smooth operator (this must be done after PreviewMesh & Weights have been updated)
	InitializeSmoothWeightsOperator(InDynamicMesh);
...
{
	// DEPRECATED 5.8
	constexpr bool bConvertSelection = false;
	SetComponentSelectionMode(InComponentMode, bConvertSelection);
}

void USkinWeightsPaintToolProperties::SetComponentSelectionMode(const EComponentSelectionMode InComponentMode, const bool bInConvertSelection)
{
	using namespace UE::MeshModelingToolsEditorOnly::SelectionUtils;
	...
	TSharedPtr<FWeightToolScopedConvertSelection> ConvertSelection;
	if (bInConvertSelection)
	{
		const UPreviewMesh* PreviewMesh = WeightTool->GetPreviewMesh();
		const FDynamicMesh3* DynamicMesh = PreviewMesh ? PreviewMesh->GetMesh() : nullptr;
		if (DynamicMesh)
		{
			ConvertSelection = MakeShared<FWeightToolScopedConvertSelection>(*WeightTool, *DynamicMesh, InComponentMode);
		}
	}
	else
	{
		UToolMeshSelector* MeshSelector = WeightTool->GetMainMeshSelector();
		UPolygonSelectionMechanic* SelectionMechanic = MeshSelector ? MeshSelector->GetSelectionMechanic() : nullptr;
		if (SelectionMechanic)
		{	
			SelectionMechanic->ClearSelection();
		}
...
{
	SetSharedFalloffMode(InFalloffMode);
	SaveConfig();
...
void USkinWeightsPaintToolProperties::SetSharedRadius(const float InRadius)
{
	// Radius behaves as a shared brush field — propagate to every per-mode config so a
	// later SetBrushMode pull sees the same value regardless of which mode is active.
	BrushConfigAdd.Radius = InRadius;
	BrushConfigReplace.Radius = InRadius;
	BrushConfigMultiply.Radius = InRadius;
	BrushConfigRelax.Radius = InRadius;
}

void USkinWeightsPaintToolProperties::SetSharedFalloffMode(const EWeightBrushFalloffMode InFalloffMode)
{
	// FalloffMode behaves as a shared brush field — propagate to every per-mode config.
	BrushConfigAdd.FalloffMode = InFalloffMode;
	BrushConfigReplace.FalloffMode = InFalloffMode;
	BrushConfigMultiply.FalloffMode = InFalloffMode;
	BrushConfigRelax.FalloffMode = InFalloffMode;
}
...
void USkinWeightsPaintToolProperties::SetBrushMode(EWeightEditOperation InBrushMode)
{
	...
	// (Radius and FalloffMode are stored per-mode for config compat but kept in lockstep
	// across all modes via SetSharedRadius / SetSharedFalloffMode, so this pull is consistent)
...
void USkinWeightsPaintTool::OnBeginDrag(const FRay& WorldRay)
{
	...
	bStampPending = true;
	LastStampWorldPosition.Reset();
	LastStampWorldRadius = 0.0;
	...
	TArray<float>& OutVertexFalloffs)
{
	...
	// Accumulating Relax bypasses the per-stroke max-falloff clamp so stamps compound;
	// other modes keep the clamp so lower-falloff re-stamps can't dial back a higher one.
	const bool bIsAccumulatingRelax =
		WeightToolProperties->BrushMode == EWeightEditOperation::Relax
		&& WeightToolProperties->RelaxBrushAdvancedConfig.bAccumulate;

	auto DistanceToFalloff = [this, bIsAccumulatingRelax](int32 InVertexID, float InDistanceSq) -> float
	{
		const float CurrentFalloff = CalculateBrushFalloff(FMath::Sqrt(InDistanceSq));
		return bIsAccumulatingRelax
			? CurrentFalloff
			: Weights.SetCurrentFalloffAndGetMaxFalloffThisStroke(InVertexID, CurrentFalloff);
	};
	...
	if (WeightToolProperties->GetBrushConfig().FalloffMode == EWeightBrushFalloffMode::Surface)
	{
		...
		// Planar projection collapses opposite sides of a sharp fold onto the same UV magnitude, which
		// lets the brush pick up vertices on the far side of the fold (e.g. back of a tilted-up neck).
		// ExponentialMapUpwindAvg propagates UVs along the surface so magnitude tracks geodesic distance.
	...
	case EWeightColorMode::Ramp:
		{
			...
			return WeightToolProperties->LinearColorRamp.GetLinearColorValue(Value);
		}
	...
	PreviewMesh->DeferredEditMesh([this](FDynamicMesh3& Mesh)
		{
			...
			UE::Geometry::FDynamicMeshColorOverlay* ColorOverlay = Mesh.Attributes()->PrimaryColors();
			const int32 CurrentBoneIndex = GetBoneIndexFromName(CurrentBone);
			for (const int32 VertexID : PerColorUpdateVertexChangeTracker.Vertices)
			{
...
bool USkinWeightsPaintTool::IsInputIsolationValidOnOutput() const
{
	return true;
}

bool USkinWeightsPaintTool::IsInputSelectionValidOnOutput()
{
	return true;
}
...
namespace SkinWeightsPaintTool::Private
{
	namespace Commands
	{
		constexpr const TCHAR* ToggleSelectedLockState = TEXT("Toggle Selected Lock State");
		constexpr const TCHAR* ToggleBypassLocks       = TEXT("Toggle Bypass Locks");
	}

	constexpr const TCHAR* const HotkeyHintCommands[] = {
		Commands::ToggleSelectedLockState,
		Commands::ToggleBypassLocks,
	};
}

void USkinWeightsPaintTool::GetHotkeyHints(TArray<FHotkeyHint>& OutHints) const
{
	for (const TCHAR* CommandName : SkinWeightsPaintTool::Private::HotkeyHintCommands)
	{
		IHotkeyHintProvider::TryAppendCommandHint(OutHints, ActionCommandsContextName, CommandName);
	}

	// B+drag is owned by BrushEditBehavior (UBaseBrushTool::BrushEditBehavior), not a FUICommandInfo.
	...
	OutHints.Add({ LOCTEXT("HintSmoothBrush", "Smooth Brush"), LOCTEXT("HintSmoothBrushChord", "Shift") });
	OutHints.Add({ LOCTEXT("HintInvertBrush", "Invert Brush"), LOCTEXT("HintInvertBrushChord", "Ctrl") });
}

void USkinWeightsPaintTool::RegisterActions(FInteractiveToolActionSet& ActionSet)
{
	using namespace SkinWeightsPaintTool::Private;
	...
	ActionSet.RegisterAction(this, GetActionId(), Commands::ToggleSelectedLockState,
		LOCTEXT("ToggleSelectedLockState", "Toggle Selected Lock State"),
		LOCTEXT("ToggleSelectedLockStateDesc", "Toggle the lock state for selected bone(s)"),
		EModifierKey::None, EKeys::L,
		[this]()
		{
			ToggleSelectedBonesLockState();
			GetToolManager()->DisplayMessage(LOCTEXT("ToggleLockMessage", "Toggled the lock state for selected bone(s) "), EToolMessageLevel::UserNotification);
		});

	ActionSet.RegisterAction(this, GetActionId(), Commands::ToggleBypassLocks,
		LOCTEXT("ToggleBypassLock", "Toggle Bypass Locks"),
		LOCTEXT("ToggleBypassLockDesc", "Toggle Bypass Locks"),
		EModifierKey::Alt, EKeys::L,
		[this]()
		{
			ToggleBypassLocks();
			GetToolManager()->DisplayMessage(LOCTEXT("ToggleBypassLockMessage", "Toggled Lock Bypass Mode"), EToolMessageLevel::UserNotification);
		});
}
...
void USkinWeightsPaintTool::ApplyStamp(const FBrushStampData& Stamp)
{
	...
	// Dwell throttle for accumulating Relax. Mirrors UMeshSculptToolBase::UpdateStampPendingState
	// (5% of combined radii in world space).
	if (bIsInRelaxMode && WeightToolProperties->RelaxBrushAdvancedConfig.bAccumulate)
	{
		const FVector3d CurrentWorldPos(Stamp.WorldPosition);
		const double CurrentBrushWorldRadius = CurrentBrushRadius;
		if (LastStampWorldPosition.IsSet())
		{
			constexpr double SpacingFraction = 0.05;
			const double Spacing = SpacingFraction * (LastStampWorldRadius + CurrentBrushWorldRadius);
			if (FVector3d::Distance(CurrentWorldPos, LastStampWorldPosition.GetValue()) < Spacing)
			{
				return; // too close to the last accepted stamp — skip
			}
		}
		LastStampWorldPosition = CurrentWorldPos;
		LastStampWorldRadius = CurrentBrushWorldRadius;
	}
	...
	FMultiBoneWeightEdits WeightEditsFromStamp;
	{
		...
		{
			// One smoothing pass per stamp; wiggling supplies the iteration count.
			const float UseStrength = CalculateBrushStrengthToUse(EWeightEditOperation::Relax);
			const bool bUsePreChangeWeight = !WeightToolProperties->RelaxBrushAdvancedConfig.bAccumulate;
			CreateWeightEditsToRelaxVertices(VerticesInStamp, VertexFalloffs, UseStrength, bUsePreChangeWeight, WeightEditsFromStamp);
		}
		else
		{
...
float USkinWeightsPaintTool::CalculateBrushStrengthToUse(EWeightEditOperation EditMode) const
{
	...
	case EWeightEditOperation::Multiply:
		{
			// Ctrl shifts the multiplier from 'x' (raw attenuate, slider in [0,1])
			// to '1 + x' (amplify, effective range [1,2]).
			UseStrength = bInvertStroke ? (1.0f + UseStrength) : UseStrength;
			break;
		}
	case EWeightEditOperation::Relax:
		{
			// Invert is intentionally not supported for Relax — sharpening skin weights
			// would leave the [0,1] simplex and isn't a useful brush operation. The previous
			// formula (1 - UseStrength) also produced negative lerp factors when Strength > 1,
			// causing unbounded oscillation across strokes.
			break;
		}
	default:
		checkNoEntry();
...
void USkinWeightsPaintTool::CreateWeightEditsToRelaxVertices(
	...
	FMultiBoneWeightEdits& InOutWeightEdits)
{
	if (!ensure(SmoothWeightsOp) || !ensure(SmoothWeightsDataSource))
	{
		return;
	}

	FPaintToolWeightsDataSource* DataSource = static_cast<FPaintToolWeightsDataSource*>(SmoothWeightsDataSource.Get());
	const FPaintToolWeightsDataSource::EWeightSource TransientSource = bUsePreChangeWeight
		? FPaintToolWeightsDataSource::EWeightSource::PreChange
		: FPaintToolWeightsDataSource::EWeightSource::Current;
	const FPaintToolWeightsDataSource::FWeightSourceScope SourceScope(*DataSource, TransientSource);

	for (int32 VertexIndex = 0; VertexIndex < VertexIndices.Num(); ++VertexIndex)
	{
		const int32 VertexID = VertexIndices[VertexIndex];
		const float UseFalloff = VertexFalloffs.IsValidIndex(VertexIndex)
			? VertexFalloffs[VertexIndex] * Strength
			: Strength;

		TMap<int32, float> FinalWeights;
		const bool bSmoothSuccess = SmoothWeightsOp->SmoothWeightsAtVertex(VertexID, UseFalloff, GetFinalLockedBoneIndices(), FinalWeights);
		...
		for (const TTuple<BoneIndex, float>& FinalWeight : FinalWeights)
		{
			...
			const float RawWeight = FinalWeight.Value;
			// The smoothing operator is expected to keep results on the [0,1] simplex.
			// Ensure here so out-of-range values are surfaced during development; clamp
			// after so a one-off stale value (e.g. left over in PreChangeWeights from an
			// earlier buggy stroke) doesn't corrupt CurrentWeights at runtime.
			ensureMsgf(RawWeight >= 0.f && RawWeight <= 1.f,
				TEXT("Smoothing produced out-of-range weight %f for bone %d on vertex %d."),
				RawWeight, BoneIndex, VertexID);
			const float NewWeight = FMath::Clamp(RawWeight, 0.f, 1.f);
			constexpr bool bPruneInfluence = false;
...
void USkinWeightsPaintTool::InitializeSmoothWeightsOperator(const FDynamicMesh3& RestPoseMesh)
{
	...
	SmoothWeightsOp = MakeUnique<UE::Geometry::TSmoothBoneWeights<int32, float>>(&RestPoseMesh, SmoothWeightsDataSource.Get());
	SmoothWeightsOp->MinimumWeightThreshold = MinimumWeightThreshold;
	SmoothWeightsOp->EdgeWeightMethod = UE::Geometry::ESmoothBoneWeightsEdgeWeightMethod::CotanWeights;
}

void USkinWeightsPaintTool::ApplyWeightEditsWithoutTransaction(const FMultiBoneWeightEdits& WeightEdits)
{
	Weights.ApplyEditsToCurrentWeights(WeightEdits, PerColorUpdateVertexChangeTracker);
	ActiveChange->StoreMultipleWeightEdits(WeightEdits);
}
...
bool USkinWeightsPaintTool::HasSelectedBone() const
{
	return !SelectedBoneIndices.IsEmpty();
}
...
{
	// apply changes to tool target
	if (ShutdownType == EToolShutdownType::Accept)
	{
		SelectionIsolator->ApplyWeightsToFullMesh();

		bool bAskToSaveNewProfile = false;
		if (IsProfileValid(NewProfilePlaceHolderName()))
		{
			// Rename the new profile to user-supplied name, if a new profile was created 
			RenameSkinWeightsAttribute(EditedMesh, NewProfilePlaceHolderName(), WeightToolProperties->NewSkinWeightProfile);
			if (ActiveSkinWeightProfileName == NewProfilePlaceHolderName())
			{
				ActiveSkinWeightProfileName = WeightToolProperties->NewSkinWeightProfile;	
			}
			else
			{
				// User is not editing the new profile when clicking accept, there is a chance that the profile was created unintentionally
				bAskToSaveNewProfile = true;
			}
		}
		
		if (bAskToSaveNewProfile)
		{
			EAppReturnType::Type Reply = FMessageDialog::Open(EAppMsgType::YesNo,
					FText::Format(
						LOCTEXT("Prompt_SkinWeightsPaintTool_SaveNewSkinWeightProfile",
							"Save new Skin Weight Profile \"{0}\"? \nSelect No to discard it."),
						FText::FromName(WeightToolProperties->NewSkinWeightProfile)));

			if (Reply == EAppReturnType::No)
			{
				EditedMesh.Attributes()->RemoveSkinWeightsAttribute(WeightToolProperties->NewSkinWeightProfile);
			}
		}
		...
		// Refresh the skeleton tree
		GetNotifier()->Notify({}, ESkeletalMeshNotifyType::HierarchyChanged);
	}
	
	SkeletonTreeNotifierBindScope.Reset();
	
	if (ensure(WeightToolProperties))
	{
		WeightToolProperties->LinearColorRamp.OnColorCurveChangedDelegate.RemoveAll(this);
	}

	// Snapshot the lock set into the persisted name list on non-Cancel. On Cancel,
	// leave LockedBoneNames as the values restored at Setup so the cache rolls back.
	if (ShutdownType != EToolShutdownType::Cancel)
	{
		WeightToolProperties->LockedBoneNames.Reset(LockedBoneIndices.Num());
		for (const BoneIndex Index : LockedBoneIndices)
		{
			const FName Name = GetBoneNameFromIndex(Index);
			if (Name != NAME_None)
			{
				WeightToolProperties->LockedBoneNames.Add(Name);
			}
		}
	}

	WeightToolProperties->SaveProperties(this, WeightToolProperties->PropertySetCacheIdentifier);
...
const TArray<int32>& USkinWeightsPaintTool::GetDefaultIsolationTriangles() const
{
	static const TArray<int32> Empty;
	USkeletalMeshEditorContextObjectBase* Context = GetToolManager()->GetContextObjectStore()->FindContext<USkeletalMeshEditorContextObjectBase>();
	if (Context)
	{
		return Context->GetIsolatedTriangles();
	}
	return Empty;
}

const TSet<int32>& USkinWeightsPaintTool::GetFinalLockedBoneIndices()
{
	if (IsBypassingAllLocks())
	{
		static TSet<int32> EmptySet;
		return EmptySet;
	}
	
	return GetRawLockedBoneIndices();	
}

const TSet<int32>& USkinWeightsPaintTool::GetRawLockedBoneIndices()
{
	return LockedBoneIndices;
}

bool USkinWeightsPaintTool::IsBypassingAllLocks() const
{
	return bBypassLocks;
}
...
{
	const FName SkinProfile = GetActiveSkinWeightProfileName();
	ActiveChange = MakeUnique<FMeshSkinWeightsChange>(SkinProfile, Weights.PreChangeWeights.Num());
}

void USkinWeightsPaintTool::EndChange(const FText& TransactionLabel)
{
	// Close before Sync — Sync overwrites PreChangeWeights and Close needs it for Before snapshots.
	ActiveChange->Close(this);
...
void USkinWeightsPaintTool::ExternalUpdateSkinWeightLayer(const FName InSkinWeightProfile)
{
	...
	// Trigger skin weight profile switch to show the correct profile during undo/redo
	if (InSkinWeightProfile != GetActiveSkinWeightProfileName())
	{
		if (InSkinWeightProfile == NewProfilePlaceHolderName())
		{
			WeightToolProperties->SkinWeightProfileOption = CreateNewName();
			WeightToolProperties->bShowNewProfileName = true;
		}
		else
		{
			WeightToolProperties->SkinWeightProfileOption = InSkinWeightProfile;
	...
	EMirrorDirection InMirrorDirection)
{
	...
	for (FName BoneName : BoneNames)
	{
		...
		//UE_LOGF(LogTemp, Log, "Bone    : %ls", *BoneName.ToString());
		//UE_LOGF(LogTemp, Log, "Mirrored: %ls", *MirroredBoneName.ToString());
		//UE_LOGF(LogTemp, Log, "-------");
	}
...
{
	if (!ShouldProceedWithAllBonesUnlocked())
	{
		return;
	}
	...
	{
		UE_LOGF(LogMeshModelingToolsEditor, Log, "Mirror Skin Weights: %d vertex weights were not mirrored because a vertex was not found close enough to the mirrored location.", MissingVertices.Num());
	}
}
	...
	const bool bShouldTransact)
{
	...
	const TArray<float> VertexFalloffs = {}; // no falloff in mesh-selection mode
	...
	{
		// Mesh-mode Relax always compounds: each iteration reads the prior iteration's
		// output via CurrentWeights, independent of the brush's bAccumulate setting.
		constexpr bool bUsePreChangeWeight = false;
		for (int32 i = 0; i < Iterations; ++i)
		{
			FMultiBoneWeightEdits IterEdits;
			CreateWeightEditsToRelaxVertices(
				...
				bUsePreChangeWeight,
				IterEdits);
			ApplyWeightEditsWithoutTransaction(IterEdits);
		}
	}
	else
	{
...
void USkinWeightsPaintTool::PruneWeights(float Threshold, const TArray<BoneIndex>& BonesToPrune)
{
	...
	for (const VertexIndex VertexID : VerticesToPrune)
	{
		...
		// Track which bones are getting pruned so the renormalize pass below can skip them.
		TArray<BoneIndex, TInlineAllocator<UE::AnimationCore::MaxInlineBoneWeightCount>> PrunedBones;
		for (const FVertexBoneWeight& BoneWeight : VertexWeights)
		{
			if (BoneWeight.Weight < Threshold || BonesToPrune.Contains(BoneWeight.BoneID))
			{
				PrunedBones.Add(BoneWeight.BoneID);
				constexpr float NewWeight = 0.f;
				constexpr bool bPruneInfluence = true;
		...
		const int32 NumSurviving = VertexWeights.Num() - PrunedBones.Num();
		if (NumSurviving == 0)
		{
			// All influences pruned — dump full weight on root so the vertex stays normalized.
			constexpr BoneIndex RootBoneIndex = 0;
			constexpr bool bPruneInfluence = false;
		...
		else
		{
			...
			for (const FVertexBoneWeight& BoneWeight : VertexWeights)
			{
				if (!PrunedBones.Contains(BoneWeight.BoneID))
				{
					TotalWeight += BoneWeight.Weight;
				}
			}

			// All survivors at zero → split evenly so the vertex has well-defined weights.
			const bool bNoOtherWeights = FMath::IsNearlyEqual(TotalWeight, 0.f);
			const float EvenlySplitWeight = 1.0f / NumSurviving;
...
void USkinWeightsPaintTool::AverageWeights(const float Strength)
{
	...
	TMap<BoneIndex, float> AccumulatedWeights;	
	AccumulateWeights(Weights.PreChangeWeights, VerticesToAverage, GetFinalLockedBoneIndices(), AccumulatedWeights);
	...
	{
		float TotalLockedWeight = 0.0f;
		int32 NumLockedBones = 0;
		GetLockedWeight(Weights.PreChangeWeights, VertexID, GetFinalLockedBoneIndices(), TotalLockedWeight, NumLockedBones);
			
		TMap<BoneIndex, float> AveragedWeights = AccumulatedWeights;	
		TruncateWeightMap(AveragedWeights, NumLockedBones);
		NormalizeWeightMap(AveragedWeights, 1.f - TotalLockedWeight);	
		...
		if (FMath::IsNearlyEqual(Strength, 1.0f))
		{
			...
			{
				if (!AveragedWeights.Contains(BoneWeight.BoneID) && !GetFinalLockedBoneIndices().Contains(BoneWeight.BoneID))
				{
					constexpr bool bPruneInfluence = false;
					constexpr float NewWeight = 0.f;
		...
		else
		{
			...
			for (const FVertexBoneWeight& BoneWeight : Weights.PreChangeWeights[VertexID])
			{
				if (!GetFinalLockedBoneIndices().Contains(BoneWeight.BoneID))
				{
			...
			TruncateWeightMap(BlendedWeights, NumLockedBones);
			NormalizeWeightMap(BlendedWeights, 1.0f - TotalLockedWeight);
...
void USkinWeightsPaintTool::CopyWeights()
{
	...
	for (const TPair<BoneIndex, float>& BoneWeight : WeightsToCopy)
	{
		...
		WeightJSON->SetStringField(TEXT("BoneName"), GetBoneNameFromIndex(BoneWeight.Key).ToString());
		WeightJSON->SetNumberField(TEXT("Weight"), BoneWeight.Value);
	...
	if (FJsonSerializer::Deserialize(Reader, JsonParsed) && JsonParsed.IsValid())
	{
		...
		for (const TSharedPtr<FJsonValue>& JsonValue : JsonArray)
		{
			...
			if (IndexOfBone == INDEX_NONE)
			{
				...
				UE_LOGF(LogMeshModelingToolsEditor, Warning, "Pasted weights referenced a missing bone: %ls", *NameString);
				continue;
			}
...
void USkinWeightsPaintTool::TruncateWeightMap(TMap<BoneIndex, float>& InOutWeights, int32 NumLockedBones)
{
	...
	{
		if (Index >= MAX_TOTAL_INFLUENCES - NumLockedBones)
		{
			It.RemoveCurrent();
		}
		else
...
void USkinWeightsPaintTool::NormalizeWeightMap(TMap<BoneIndex, float>& InOutWeights, float ExpectedSum)
{
	// normalize remaining influences
	float TotalWeight = 0.f;
	...
	TMap<BoneIndex, float>& OutAccumulatedWeights)
{
	for (const VertexIndex VertexID : VerticesToAccumulate)
	{
		...
		{
			if (!ExcludedBones.Contains(BoneWeight.BoneID))
			{
				float& AccumulatedWeight = OutAccumulatedWeights.FindOrAdd(BoneWeight.BoneID);
...
void USkinWeightsPaintTool::GetLockedWeight(
	const TArray<VertexWeights>& AllWeights,
	VertexIndex InVertexID,
	const TSet<BoneIndex>& LockedBones,
	float& OutTotalLockedWeight,
	int32& OutNumLockedBones)
{
	OutTotalLockedWeight = 0;
	OutNumLockedBones = 0;
	
	for (const FVertexBoneWeight& BoneWeight : AllWeights[InVertexID])
	{
		if (LockedBones.Contains(BoneWeight.BoneID))
		{
			OutTotalLockedWeight += BoneWeight.Weight;
			OutNumLockedBones++;
		}
	}
}
...
TSharedPtr<SWidget> USkinWeightsPaintTool::GetCustomSkeletonTreeWidget(TSharedPtr<ISkeletalMeshEditorBinding> EditorBinding)
{
	if (!SkeletonReader)
	{
		return nullptr;
	...
	Delegates.OnGetRowWidget.BindUObject(this,&USkinWeightsPaintTool::OnGetLockWidget);
	Delegates.OnExtendCommandList.BindLambda([this]( TSharedPtr<FUICommandList> InOutCommandList)
		{
			FSkinWeightsPaintToolCommands Commands = FSkinWeightsPaintToolCommands::Get();
			InOutCommandList->MapAction( Commands.IsolateSelectedBones,
				FExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::IsolateSelectedBones),
				FCanExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::HasSelectedBone));
			
			InOutCommandList->MapAction( Commands.ToggleSelectedBonesLockState,
				FExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::ToggleSelectedBonesLockState),
				FCanExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::HasSelectedBone));
			
			InOutCommandList->MapAction( Commands.LockSelectedBones,
				FExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::SetLockSelectedBones, true),
				FCanExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::HasSelectedBone));
			InOutCommandList->MapAction( Commands.UnlockSelectedBones,
				FExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::SetLockSelectedBones, false),
				FCanExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::HasSelectedBone));
			
			InOutCommandList->MapAction( Commands.LockAllBones, FExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::SetLockAllBones, true));
			InOutCommandList->MapAction( Commands.UnlockAllBones, FExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::SetLockAllBones, false));
			
			InOutCommandList->MapAction( Commands.BypassLocks,
				FExecuteAction::CreateUObject(this, &USkinWeightsPaintTool::ToggleBypassLocks),
				FCanExecuteAction(),
				FIsActionChecked::CreateUObject(this, &USkinWeightsPaintTool::IsBypassingAllLocks));
		});
	Delegates.OnExtendContextMenu.BindLambda([](FMenuBuilder& MenuBuilder)
		{
			FSkinWeightsPaintToolCommands Commands = FSkinWeightsPaintToolCommands::Get();
			...
			{
				MenuBuilder.AddMenuEntry(Commands.IsolateSelectedBones);
				MenuBuilder.AddMenuEntry(Commands.ToggleSelectedBonesLockState);
			}
			MenuBuilder.EndSection();
			
			MenuBuilder.BeginSection("Lock", LOCTEXT("LockSection", "Lock"));
			{
				MenuBuilder.AddMenuEntry(Commands.LockSelectedBones);
				MenuBuilder.AddMenuEntry(Commands.LockAllBones);
			}
			MenuBuilder.EndSection();
			
			MenuBuilder.BeginSection("Unlock", LOCTEXT("UnlockSection", "Unlock"));
			{
				MenuBuilder.AddMenuEntry(Commands.UnlockSelectedBones);
				MenuBuilder.AddMenuEntry(Commands.UnlockAllBones);
			}
			MenuBuilder.EndSection();
			
			MenuBuilder.BeginSection("Bypass", LOCTEXT("BypassSection", "Bypass"));
			{
				MenuBuilder.AddMenuEntry(Commands.BypassLocks);
			}
			MenuBuilder.EndSection();
	...
	ToolSkeletonTree =
		SNew(SReferenceSkeletonTree)
		.Modifier(SkeletonReader)
		.Delegates(Delegates);

	SkeletonTreeNotifierBindScope.Reset(
		new FSkeletalMeshNotifierBindScope(
			EditorBinding->GetNotifier(),
			ToolSkeletonTree->GetNotifier()));	
		
	TArray<FName> SelectedBones = EditorBinding->GetSelectedBones();
	ToolSkeletonTree->GetNotifier()->HandleNotification(SelectedBones, ESkeletalMeshNotifyType::BonesSelected);		
	
	return ToolSkeletonTree;
}

TSharedRef<SWidget> USkinWeightsPaintTool::OnGetLockWidget(FName BoneName)
{
	int32 BoneIndex = GetBoneIndexFromName(BoneName);

	auto GetColorAndOpacity = [this, BoneIndex] ()
		{
			const bool bIsLockByUser = GetRawLockedBoneIndices().Contains(BoneIndex);
			const bool bIsHovered = LockButtons[BoneIndex]->IsHovered();
	...
	TSharedPtr<SButton> LockButton = SNew(SButton)
		...
		.ButtonColorAndOpacity_Lambda(GetColorAndOpacity)
		.OnClicked_Lambda([this, BoneIndex]()
		{
			if (LockedBoneIndices.Contains(BoneIndex))
			{
				SetLockBone(BoneIndex, false);
			}
			else
			{
				SetLockBone(BoneIndex, true);
			}
			return FReply::Handled();
		})
		.ContentPadding(1.f)
		.ToolTipText(LOCTEXT("BoneWeightLockToolTip", "Skin weights for locked bones cannot be changed"))
		[
			SNew(SImage)
			.ColorAndOpacity_Lambda(GetColorAndOpacity)
			.Image_Lambda([this]()
				{
					if (bBypassLocks)
					{
	...
	LockButtons.Emplace(BoneIndex, LockButton);
	
	return LockButton.ToSharedRef();
}

void USkinWeightsPaintTool::SetLockAllBones(bool bLock)
{
	ISkeletonProvider* SkeletonProvider = CastChecked<ISkeletonProvider>(Target);
	const FReferenceSkeleton& RefSkeleton = SkeletonProvider->GetSkeleton();
	...
	{
		SetLockBone(BoneIndex, bLock);
	}
}

void USkinWeightsPaintTool::SetLockSelectedBones(bool bLock)
{
...
void USkinWeightsPaintTool::IsolateSelectedBones()
{
	SetLockAllBones(true);
	SetLockSelectedBones(false);
}

void USkinWeightsPaintTool::ToggleSelectedBonesLockState()
{
	for (int32 BoneIndex : SelectedBoneIndices)
	{
		if (GetRawLockedBoneIndices().Contains(BoneIndex))
		{
			SetLockBone(BoneIndex,false);
		}
		else
...
void USkinWeightsPaintTool::ToggleBypassLocks()
{
	bBypassLocks = !bBypassLocks;
}

void USkinWeightsPaintTool::SetLockBone(BoneIndex InBoneIndex, bool bLock)
{
	if (bLock)
	{
		LockedBoneIndices.Add(InBoneIndex);
	}
	else
	{
		LockedBoneIndices.Remove(InBoneIndex);
	}	
}

bool USkinWeightsPaintTool::ShouldProceedWithAllBonesUnlocked()
{
	if (GetFinalLockedBoneIndices().IsEmpty())
	{
		return true;
	}

	FText Message = LOCTEXT("UnlockAllBonesDialog", "The current operation does not support skin weight locking, unlock weights for all bones and continue?");
	EAppReturnType::Type Response = FMessageDialog::Open(EAppMsgType::OkCancel, Message);

	if (Response == EAppReturnType::Ok)
	{
		SetLockAllBones(false);	
	}
...
{
	FScopedSuspendIsolation RetainIsolationScope(this);

	WeightToolProperties->bShowNewProfileName = WeightToolProperties->SkinWeightProfileOption == CreateNewName();

	FName DesiredSkinWeightProfileName = WeightToolProperties->GetDesiredSkinWeightProfileForTool();

	GetOrCreateSkinWeightsAttribute(EditedMesh, DesiredSkinWeightProfileName);
	
	check(IsProfileValid(DesiredSkinWeightProfileName));
	
	if ( DesiredSkinWeightProfileName == ActiveSkinWeightProfileName)
	{
		return;
	}

	ActiveSkinWeightProfileName = DesiredSkinWeightProfileName;

	// Re-init Weights with new skin profile
	Weights = FSkinToolWeights();
	Weights.Profile = ActiveSkinWeightProfileName;
...
{
	// We use NewProfilePlaceHolderName() to address the new profile while the tool is running
	// and only apply user-supplied new profile name to the mesh during shutdown. 
	// It is done this way because profile rename does not support yet support undo, while weight edits
	// can undo across profiles. This is not the most intuitive and could be refactored in the future.
	
	// Given this design, here we only need to make sure the new profile name is unique w.r.t 
	// existing profiles + our new profile placeholder name
	
	// Note: it is a bit silly that this might be called twice since we are changing the property
	// being watched.
	
	if (ensure(WeightToolProperties->bShowNewProfileName))
	{
		WeightToolProperties->NewSkinWeightProfile = GetValidNewProfileName(WeightToolProperties->NewSkinWeightProfile);
	}
}
...
USkinWeightsPaintTool::FScopedSuspendIsolation::FScopedSuspendIsolation(USkinWeightsPaintTool* InTool)
	: Tool(InTool)
{
	// Restore to full mesh to add the new profile and reisolate later to default isolation state
	if (!Tool->GetSelectionIsolator()->IsMeshIsolated())
	{
		// apply previous changes
		Tool->GetWeights().ApplyCurrentWeightsToMesh(Tool->GetMesh());
	}
	else
	{
		SavedIsolatedTriangles = Tool->GetSelectionIsolator()->GetIsolatedTriangles();
		// Restore also applies weights
		Tool->GetSelectionIsolator()->RestoreFullMesh();
	}
}

USkinWeightsPaintTool::FScopedSuspendIsolation::~FScopedSuspendIsolation()
{
	if (!SavedIsolatedTriangles.IsEmpty())
	{
		// re-isolate to default isolation if present
		Tool->GetSelectionIsolator()->SetTrianglesToIsolate(SavedIsolatedTriangles);
		Tool->GetSelectionIsolator()->UpdateIsolatedSelection();
	}
}

FName USkinWeightsPaintTool::GetValidNewProfileName(const FName& InProposedName) const
{
	FName ValidName = InProposedName;
	TArray<FName> Profiles;
	EditedMesh.Attributes()->GetSkinWeightsAttributes().GenerateKeyArray(Profiles);
		
	while (Profiles.Contains(ValidName))
	{
		ValidName.SetNumber(ValidName.GetNumber() + 1);
	}	
...
bool UWeightToolSelectionIsolator::CanIsolate() const
{
	// Currently only support isolation when the mesh in default isolation state (either full mesh, or isolated to a set externally provided via context)
	// It also means it is possible for the isolator to start an isolation while already in a isolated state if the default is not full mesh
	return CurrentlyIsolatedTriangles.Num() == WeightTool->GetDefaultIsolationTriangles().Num();
}

bool UWeightToolSelectionIsolator::IsMeshIsolated() const
{
...
void UWeightToolSelectionIsolator::IsolateSelectionAsTransaction()
{
	...
	// Selected triangles are in preview mesh space. If currently isolated, remap to EditedMesh (full mesh) space.
	if (IsMeshIsolated())
	{
		for (int32& TriID : ActiveChange->IsolatedTrianglesAfter)
		{
			TriID = PartialSubMesh.MapTriangleToBaseMesh(TriID);
		}
	}
...
void UWeightToolSelectionIsolator::UnIsolateSelectionAsTransaction()
{
	...
	TUniquePtr<FIsolateSelectionChange> ActiveChange = MakeUnique<FIsolateSelectionChange>();
	ActiveChange->IsolatedTrianglesBefore = GetIsolatedTriangles();
	// Restore to default isolation, which is either a valid set or empty(full mesh)
	ActiveChange->IsolatedTrianglesAfter = WeightTool->GetDefaultIsolationTriangles();
...
void UWeightToolSelectionIsolator::RestoreFullMesh()
{
	...
	ApplyWeightsToFullMesh();

	// retain selection (a convenience for the user that allows for easily adjusting the isolation by going back/forth as needed)
	const FWeightToolScopedRetainSelection RetainSelectionScope(*this);
...
void UWeightToolSelectionIsolator::ApplyWeightsToFullMesh() const
{
	// get the weights
	FSkinToolWeights& Weights = WeightTool->GetWeights();
	FDynamicMesh3& FullMesh = WeightTool->GetMesh();
	...
	if (IsMeshIsolated())
	{
		auto PartialToFull = [this](int32 Idx) { return PartialSubMesh.MapVertexToBaseMesh(Idx); };
		Weights.ApplyCurrentWeightsToMesh(FullMesh, PartialToFull);
	}
	else
	{
...
const FDynamicMesh3& UWeightToolSelectionIsolator::GetPartialMesh() const
{
	...
	return IsMeshIsolated() ? PartialSubMesh.GetSubmesh() : Dummy;
}
...
void UWeightToolSelectionIsolator::CreatePartialMesh()
{
	...
	// retain selection — captures in full mesh space, restores in new submesh space
	const FWeightToolScopedRetainSelection RetainSelectionScope(*this);

	FDynamicMesh3& FullMesh = WeightTool->GetMesh();
	// create a partial sub-mesh from a subset of triangles on the current mesh
...
void USkinWeightsPaintTool::SelectAffected() const
{
	...
	const bool bIsShiftDown = FSlateApplication::IsInitialized() ? FSlateApplication::Get().GetModifierKeys().IsShiftDown() : false;
	const bool bIsControlDown = FSlateApplication::IsInitialized() ? FSlateApplication::Get().GetModifierKeys().IsControlDown() : false;
	if (bIsShiftDown)
	{
		// ADD to selection
	...
	{		
		// REMOVE from selection if all affected vertices are contained in the current selection
		// ADD to the selection otherwise
		const bool bRemoveFromSelection = Algo::AllOf(AffectedVertices, [&CurrentSelection](const int32 VertexIndex)
			{
				return CurrentSelection.SelectedCornerIDs.Contains(VertexIndex);
			});
...
void USkinWeightsPaintTool::OnPropertyModified(UObject* ModifiedObject, FProperty* ModifiedProperty)
{
	...
	if (ModifiedProperty)
	{
		...
		else if (ModifiedProperty->GetName() == GET_MEMBER_NAME_STRING_CHECKED(USkinWeightsPaintToolProperties, BrushRadius))
		{
			// Honour the user's Sync toggle: lockstep across modes when on, per-mode save when off.
			if (WeightToolProperties->bSyncBrushRadiusAcrossModes)
			{
				WeightToolProperties->SetSharedRadius(WeightToolProperties->BrushRadius);
			}
			else
			{
		...
		else if (ModifiedProperty->GetName() == GET_MEMBER_NAME_STRING_CHECKED(USkinWeightsPaintToolProperties, BrushFalloffAmount))
		{
			WeightToolProperties->GetBrushConfig().Falloff = WeightToolProperties->BrushFalloffAmount;
		}
		else if (ModifiedProperty->GetName() == GET_MEMBER_NAME_STRING_CHECKED(USkinWeightsPaintToolProperties, ColorMode))
		{
			bVertexColorsNeedUpdated = true;
		}
