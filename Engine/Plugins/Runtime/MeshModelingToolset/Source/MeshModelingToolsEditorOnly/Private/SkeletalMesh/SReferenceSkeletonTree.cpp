...
/** The actual tree view for this widget */
class SReferenceSkeletonTreeView 
	: public STreeView<TSharedPtr<FBoneElement>>
{
public:
	/** Stores sparse item infos */
	void StoreSparseItemInfos()
	{
		CachedSparseItemInfos = STreeView<TSharedPtr<FBoneElement>>::SparseItemInfos;
	}
	
	/** Restores sparse item infos from last known infos */
	void RestoreSparseItemInfos(const TArray<TSharedPtr<FBoneElement>> TreeItems)
	{
		for (auto It = STreeView<TSharedPtr<FBoneElement>>::SparseItemInfos.CreateIterator(); It; ++It)
		{
			if (!TreeItems.Contains(It->Key))
			{
				It.RemoveCurrent();
			}
		}
		
		for (auto It = CachedSparseItemInfos.CreateIterator(); It; ++It)
		{
			const TSharedPtr<FBoneElement>* TreeItemPtr = TreeItems.FindByPredicate(
				[It](const TSharedPtr<FBoneElement>& TreeItem)
				{
					return It->Key->BoneName == TreeItem->BoneName;
				});
				
			if (TreeItemPtr)
			{
				STreeView<TSharedPtr<FBoneElement>>::SparseItemInfos.Add(*TreeItemPtr, It->Value);
			}
		}
	}
	
private:
	/** Holds the last known sparse item infos */
	STreeView<TSharedPtr<FBoneElement>>::TSparseItemMap CachedSparseItemInfos;
};
}
...
void SBoneItem::Construct(const FArguments& InArgs)
{
	...
	OnBoneElementNameCommitted = InArgs._Delegates.OnBoneElementNameCommitted;
    ...
    [
		...
		.OnTextCommitted(this, &SBoneItem::OnTextCommitted)
		.MultiLine(false)
...
void SBoneItem::OnTextCommitted(const FText& Text, ETextCommit::Type TextCommitType)
{
	if (WeakTreeElement.IsValid())
	{
		OnBoneElementNameCommitted.ExecuteIfBound(Text, TextCommitType, WeakTreeElement.Pin().ToSharedRef());
	}
}
...
TSharedRef<SWidget> SReferenceSkeletonRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	...
	if (Delegates.OnGetRowWidget.IsBound())
	{
		Widget->InsertSlot(0)
		.AutoWidth()
		[
			Delegates.OnGetRowWidget.Execute(WeakTreeElement.Pin()->BoneName)	
		];
	}
...
{
	checkf(InArgs._Modifier.IsValid(), TEXT("Modifier is a required argument for SReferenceSkeletonTree"));
	Modifier = InArgs._Modifier;
	bIsReadOnly = Modifier->IsReadOnly();

	Delegates = InArgs._Delegates;
	Delegates.OnCanAcceptDrop = !Delegates.OnCanAcceptDrop.IsBound() ? FOnRefSkeletonTreeCanAcceptDrop::CreateSP(this, &SReferenceSkeletonTree::OnCanAcceptDrop) : Delegates.OnCanAcceptDrop;
	Delegates.OnAcceptDrop = !Delegates.OnAcceptDrop.IsBound() ? FOnRefSkeletonTreeAcceptDrop::CreateSP(this, &SReferenceSkeletonTree::OnAcceptDrop) : Delegates.OnAcceptDrop;
	Delegates.OnDragDetected = !Delegates.OnDragDetected.IsBound() ? FOnDragDetected::CreateSP(this, &SReferenceSkeletonTree::OnDragDetected) : Delegates.OnDragDetected;
	Delegates.OnBoneRenamed = !Delegates.OnBoneRenamed.IsBound() ? FOnBoneRenamed::CreateSP(this, &SReferenceSkeletonTree::OnBoneRenamed) : Delegates.OnBoneRenamed;	
	if (!Delegates.OnBoneElementNameCommitted.IsBound())
	{
		Delegates.OnBoneElementNameCommitted = FRefSkeletonTreeDelegates::FOnBoneElementNameCommitted::CreateSP(this, &SReferenceSkeletonTree::OnBoneElementNameCommitted);
	}
	...
	[
		...
		[
			...
			[
				...
				[
					SAssignNew(TreeView, SReferenceSkeletonTreeView)
...
void SReferenceSkeletonTree::BindCommands()
{
	if (!bIsReadOnly)
	{
		...
		CommandList->MapAction(Commands.PasteBonesLocal,
			FExecuteAction::CreateSP(this, &SReferenceSkeletonTree::HandlePasteBones, !bPasteUsingGlobalTransforms),
			FCanExecuteAction::CreateSP(this, &SReferenceSkeletonTree::CanPasteBones));
		
		CommandList->MapAction(Commands.PasteBonesGlobal,
			FExecuteAction::CreateSP(this, &SReferenceSkeletonTree::HandlePasteBones, bPasteUsingGlobalTransforms),
			FCanExecuteAction::CreateSP(this, &SReferenceSkeletonTree::CanPasteBones));
	...
	if (Delegates.OnExtendCommandList.IsBound())
	{
		Delegates.OnExtendCommandList.Execute(CommandList);
	}
}
...
void SReferenceSkeletonTree::HandleCopyBones() const
{
	...
	GetSelectedBoneNames(SelectedBones);

	if (!SelectedBones.IsEmpty())
	{
		SkeletonClipboard::CopyBonesToClipboard(*Modifier.Get(), SelectedBones);
	}
}
...
void SReferenceSkeletonTree::HandlePasteBones(const bool bUsingGlobalTransforms)
{
	if (!ensure(Modifier.IsValid()))
	{
		return;
...
bool SReferenceSkeletonTree::CanPasteBones() const
{
	...
	return Modifier.IsValid() && SkeletonClipboard::IsBoneClipboardValid();
}
...
void SReferenceSkeletonTree::HandleDuplicateBones()
{
	...
	const TArray<FName> NewBones = SkeletonClipboard::ClipboardDuplicateBones(*Modifier.Get(), SelectedBones);
	if (NewBones.IsEmpty())
	{
		CancelChange();
		return;
...
void SReferenceSkeletonTree::OnBoneElementNameCommitted(const FText& InText, ETextCommit::Type InCommitType, TSharedRef<FBoneElement> BoneElement)
{
	...
	// If the selection does not contain the bone name, the user committed by changing the selection.
	// Handle the case by reanming the element which invoked this commit and not the selection.
	if (!BoneNames.Contains(BoneElement->BoneName))
	{
		BoneNames = { BoneElement->BoneName };
	}
...
void SReferenceSkeletonTree::RefreshTreeView(bool IsInitialSetup /*=false*/)
{
	...
	TArray<FName> PreviouslySelectedBones;
	GetSelectedBoneNames(PreviouslySelectedBones);

	TreeView->StoreSparseItemInfos();
	...
	TreeView->RestoreSparseItemInfos(AllElements);

	if (!PreviouslySelectedBones.IsEmpty())
	{
		SelectItemFromNames(PreviouslySelectedBones);
	}
}
...
TSharedPtr<SWidget> SReferenceSkeletonTree::CreateContextMenu()
{
	...
	bool bHasValidMenuOptions = false;
	if (!bIsReadOnly)
	{
		const FSkeletonEditingToolCommands& Commands = FSkeletonEditingToolCommands::Get();
		bHasValidMenuOptions = true;
		if (SelectedItems.IsEmpty())
		{
			...
			MenuBuilder.AddMenuEntry(Commands.PasteBonesLocal);
			MenuBuilder.AddMenuEntry(Commands.PasteBonesGlobal);
	...
	if (Delegates.OnExtendContextMenu.IsBound())
	{
		if (Delegates.OnExtendContextMenu.Execute( MenuBuilder))
		{
			bHasValidMenuOptions = true;
		}
	}
	
	if (bHasValidMenuOptions)
	{
		return MenuBuilder.MakeWidget();
	}
