...
#include "Operations/HoleFillUtil.h"
...
bool FHoleFillOp::FillSingleTriangleHole(const FEdgeLoop& Loop, int32& NewGroupID)
{
	...
	// Pick the overlay elements to use based on neighbors, to minimize added seams. Returns false when neighbors don't share elements along the hole.
	auto GetOverlayElementsFromNeighbors = [this, &Vertices, &ExistingTriangles, &ExistingEdges]<typename OverlayType>(OverlayType* Overlay, FIndex3i& NewTriangleElements) -> bool
	{
		// We're going to try to assign the overlay elements such that the number of overlay seam edges is minimized. If at least 
		// two of our neighboring triangles have a shared overlay element at one of our vertices, then using the elements
		...
		for (int NeighborIndex = 0; NeighborIndex < 3; ++NeighborIndex)
		{
			...
			FIndex3i NeighborTriElements = Overlay->GetTriangle(ExistingTriangles[NeighborIndex]);
			...
			VertElements[VertIndex1].B = NeighborTriElements[IndexUtil::FindTriIndex(Vertices[VertIndex1], NeighborTriVids)];
			VertElements[VertIndex2].A = NeighborTriElements[IndexUtil::FindTriIndex(Vertices[VertIndex2], NeighborTriVids)];
		}
		 ...
		 *    B   A
		 */
		if (VertElements[0].A == VertElements[0].B)
		{
			NewTriangleElements = FIndex3i(VertElements[0].A, VertElements[1].A, VertElements[2].B);
		}
		else if (VertElements[1].A == VertElements[1].B)
		{
			NewTriangleElements = FIndex3i(VertElements[0].B, VertElements[1].A, VertElements[2].A);
		}
		else if (VertElements[2].A == VertElements[2].B)
		{
			NewTriangleElements = FIndex3i(VertElements[0].A, VertElements[1].B, VertElements[2].A);
		}
		else
		{
			return false; // failed to find a clean match, fall back to overlay-type-specific logic
		}
		return true;
	};
	...
	if (ResultMesh->HasAttributes() && ResultMesh->Attributes()->PrimaryUV())
	{
		...
		FIndex3i NewTriangleElements = FIndex3i::Invalid();

		if (!GetOverlayElementsFromNeighbors(UVOverlay, NewTriangleElements))
		{
			// Since we didn't have a matching element ID above, at least one of ExistingTriangles[0] or [1] must have set UVs --
			//  i.e., if they were both unset, then their element IDs would all be -1 and one of the tests for matching element IDs would pass
			...
			SetUVTriangleFromAdjacentTriangle(*UVOverlay, NeighborTidToUse, NewTriangleID, NewTriangleElements);
		}
	...
	if (ResultMesh->HasAttributes() && ResultMesh->Attributes()->HasPrimaryColors())
	{
		FDynamicMeshColorOverlay* ColorOverlay = ResultMesh->Attributes()->PrimaryColors();
		FIndex3i NewTriangleElements = FIndex3i::Invalid();

		// try to fill the color overlay seamlessly from local info
		if (GetOverlayElementsFromNeighbors(ColorOverlay, NewTriangleElements))
		{
			if (NewTriangleElements.A != IndexConstants::InvalidID)
			{
				EMeshResult Result = ColorOverlay->SetTriangle(NewTriangleID, NewTriangleElements);
				ensure(Result == EMeshResult::Ok); // should always succeed, but if it doesn't we will just have some unset colors
			}
		}
		else
		{
			// fall back to a standard color overlay fill method, which should at least try to avoid a seam along one edge
			// (since it stomps IDs to match for each it finds with valid overlay elements, so should avoid seams at least on the last edge it processes)
			TStaticArray<int32, 3> ExistingEdgesArr{ ExistingEdges.A, ExistingEdges.B, ExistingEdges.C };
			HoleFillUtil::FillColorOverlay(*ResultMesh, ExistingEdgesArr, TArrayView<int32>(&NewTriangleID, 1));
		}
	}
