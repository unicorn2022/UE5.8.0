...
void FRemeshMeshOp::CalculateResult(FProgressCancel* Progress)
{
	...
	bool bHasDiscardedAttributes = bDiscardAttributes && !bPreserveSharpEdges && !bNeedsWeightMaps;
	ResultMesh->Copy(*OriginalMesh, true, true, true, !bHasDiscardedAttributes); // if we'll never need attributes, copying w/out them is fastest

	// helper to discard only attributes that we don't need
	auto SafeDiscardAttributes = [this, &bHasDiscardedAttributes](FDynamicMesh3& Mesh)
	{
		if (bHasDiscardedAttributes || !Mesh.HasAttributes())
		{
			return;
		}
		bHasDiscardedAttributes = true;
		if (bNeedsWeightMaps)
		{
			// manually turn off attributes aside from the weights
			Mesh.Attributes()->SetNumNormalLayers(0);
			Mesh.Attributes()->SetNumPolygroupLayers(0);
			Mesh.Attributes()->SetNumUVLayers(0);
			Mesh.Attributes()->DisableBones();
			Mesh.Attributes()->DiscardSculptLayers();
			Mesh.Attributes()->DisablePrimaryColors();
			Mesh.Attributes()->DisableMaterialID();
		}
		else
		{
			// discard the entire attribute set
			Mesh.DiscardAttributes();
		}
	};

	// if we copied w/ attributes but we just needed the weight maps, discard all else
	if (bDiscardAttributes && bNeedsWeightMaps && !bPreserveSharpEdges)
	{
		SafeDiscardAttributes(*ResultMesh);
	}
	...
	if (bDiscardAttributes && !bHasDiscardedAttributes)
	{
		SafeDiscardAttributes(*TargetMesh);
	}
	...
	if (bHasDiscardedAttributes && TargetMesh->HasAttributes())
	{
		// return the default single UV and normal layers, if we discarded to 0 above,
		// since we typically expect PrimaryUV and PrimaryNormals to be valid
		TargetMesh->Attributes()->SetNumUVLayers(1);
		TargetMesh->Attributes()->SetNumNormalLayers(1);
	}

	if ((bHasDiscardedAttributes || !TargetMesh->HasAttributes()) && bResultMustHaveAttributesEnabled)
	{
