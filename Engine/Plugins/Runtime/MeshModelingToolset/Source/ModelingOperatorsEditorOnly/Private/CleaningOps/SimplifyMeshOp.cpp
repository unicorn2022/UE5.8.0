...
void ComputeSimplify(FDynamicMesh3* TargetMesh, const bool bReproject,
					 ...
					 const float TargetPercentage, const int TargetCount, const float TargetEdgeLength, const float TargetQuadricError,
					 const float AngleThreshold,
	                 typename SimplificationType::ESimplificationCollapseModes CollapseMode,
					 bool bUseQuadricMemory,
					 float GeometricTolerance,
					 float RegularizeWeight,
	...
	TFunction<double(const FDynamicMesh3&, int Vertex)> CustomQuadricErrorScaleF)
{
	...
	Reducer.CustomGeometricErrorScaleF = CustomGeometricErrorScaleF;
	Reducer.CustomQuadricErrorScaleF = CustomQuadricErrorScaleF;
	Reducer.CustomEdgeLengthScaleF = CustomEdgeLengthScaleF;

	Reducer.RegularizeWeight = RegularizeWeight;
	...
	else if (TargetMode == ESimplifyTargetType::QuadricError)
	{
		Reducer.SimplifyToMaxError(TargetQuadricError);
	}
}
							...
							float PercentReduction, bool bTriBasedReduction, bool bDiscardAttributes, FProgressCancel* Progress)
{
	...
	// Spot-check the mesh description attributes, to catch mesh descriptions w/out expected static mesh attributes
	{
		const FStaticMeshConstAttributes SrcAttrs(SrcMeshDescription);
		if (!SrcAttrs.GetVertexInstanceUVs().IsValid())
		{
			UE_LOGF(LogGeometry, Error,
				"FSimplifyMeshOp::ComputeStandardSimplifier -- mesh description is missing standard static mesh attributes expected by the simplifier");
			return false;
		}
	}
	...
	FStaticMeshOperations::FindOverlappingCorners(OverlappingCorners, SrcMeshDescription, 1.e-5f);
...
void FSimplifyMeshOp::CalculateResult(FProgressCancel* Progress)
{
	...
	if (SimplifierType == ESimplifyType::QEM)
	{
		...
		ComputeSimplify<FQEMSimplification>(TargetMesh, bReproject, OriginalTriCount, *OriginalMesh, *OriginalMeshSpatial,
											...
											TargetMode, TargetPercentage, TargetCount, TargetEdgeLength, TargetQuadricError, MinimalPlanarAngleThresh,
											FQEMSimplification::ESimplificationCollapseModes::MinimalQuadricPositionError, bUseQuadricMemory, 
											UseGeometricTolerance,
											RegularizeWeight,
											CustomGeometricErrorScaleF,
											CustomEdgeLengthScaleF,
											CustomQuadricErrorScaleF);
	}
	...
	else if (SimplifierType == ESimplifyType::MinimalPlanar)
	{
		...
		ComputeSimplify<FQEMSimplification>(TargetMesh, bReproject, OriginalTriCount, *OriginalMesh, *OriginalMeshSpatial,
			...
			MaterialBoundaryConstraint,
			bPreserveSharpEdges, bAllowSeamCollapse, bPreventNormalFlips, bPreventTinyTriangles,
			ESimplifyTargetType::MinimalPlanar, TargetPercentage, TargetCount, TargetEdgeLength, TargetQuadricError, MinimalPlanarAngleThresh,
			FQEMSimplification::ESimplificationCollapseModes::MinimalQuadricPositionError, bUseQuadricMemory,
			UseGeometricTolerance,
			RegularizeWeight,
			CustomGeometricErrorScaleF,
	...
	else if (SimplifierType == ESimplifyType::ClusterBased)
	{
		...
		SimplifyOptions.OptionalTargetEdgeLengthScale = ClusterTargetEdgeLengthScaleF;
	...
	else // SimplifierType == ESimplifyType::UEStandard
	{
		...
		else if (TargetMode == ESimplifyTargetType::TriangleCount)
		{
			...
			PercentReduction = NumTris > 0 ? (float)TargetCount / (float)NumTris : 0.f;
		}
		else if (TargetMode == ESimplifyTargetType::VertexCount)
		{
			int32 NumVerts = SrcMeshDescription->Vertices().Num();
			PercentReduction = NumVerts > 0 ? (float)TargetCount / (float)NumVerts : 0.f;
		}
