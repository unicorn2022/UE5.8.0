...
enum class ESimplifyTargetType : uint8
{
	...
	/** Quadric error threshold */
	QuadricError = 5 UMETA(DisplayName = "Quadric Error"),
...
{
public:
	virtual ~FSimplifyMeshOp() = default;
	...
	// Weight of optional regularization term, for QEM-based simplifiers
	float RegularizeWeight = 0.f;

	// QEM simplifier custom scales to apply to errors and edge lengths. Note this is in effect the inverse of scaling the tolerances / target lengths.
	...
	// Cluster simplifier custom scales apply to the *target* edge lengths, and so is conceptually the inverse of the above scales.
	TFunction<double(int32)> ClusterTargetEdgeLengthScaleF = nullptr;
