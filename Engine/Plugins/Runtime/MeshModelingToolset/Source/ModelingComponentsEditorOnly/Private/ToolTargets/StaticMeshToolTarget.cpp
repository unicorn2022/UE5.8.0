	...
	const FComponentMaterialSet& MaterialSet, bool bApplyToAsset)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "CANNOT MODIFY BUILT-IN ENGINE ASSET %ls", *StaticMeshIn->GetPathName());
		return false;
	}
...
{
	// Note: Static init is thread-safe, as long as it's done entirely in this assignment; do not change the EmptyMeshDescription object after.
	static const FMeshDescription EmptyMeshDescription = []
	{
		FMeshDescription MD;
...
void UStaticMeshToolTarget::CommitMeshDescription(UStaticMesh* StaticMeshIn, const FCommitter& Committer, EMeshLODIdentifier EditingLODIn)
{
	...
	{
		UE_LOGF(LogGeometry, Warning, "UStaticMeshToolTarget::CommitMeshDescription: invalid Target LOD, must specify explicit LOD");
		return;
	}
	if (StaticMeshIn->GetPathName().StartsWith(TEXT("/Engine/")))
	{
