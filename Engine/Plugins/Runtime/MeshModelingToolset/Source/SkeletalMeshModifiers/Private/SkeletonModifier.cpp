	...
	public:
		...
		bool DoesParentChainMatch(const int32 InStartBoneIndex, const FReferenceSkeleton& InRefSkeleton) const
		{
			...
			do
			{
				...
				{
					UE_LOGF(LogAnimation, Warning, "%ls : Hierarchy does not match %ls - %ls.", *ReferenceSkeleton.GetBoneName(SkeletonBoneIndex).ToString(), *ReferenceSkeleton.GetBoneName(ParentSkeletonBoneIndex).ToString(), *InRefSkeleton.GetBoneName(ParentOtherBoneIndex).ToString());
					return false;
				}
		...
		bool IsCompatibleReferenceSkeleton(const FReferenceSkeleton& InRefSkeleton, bool bDoParentChainCheck=true) const
		{
			...
			for (int32 OtherBoneIndex=0; OtherBoneIndex<OtherNumBones; OtherBoneIndex++)
			{
				...
				if( SkeletonBoneIndex != INDEX_NONE )
				{
					...
					{
						UE_LOGF(LogAnimation, Warning, "%ls : Hierarchy does not match.", *OtherBoneName.ToString());
						return false;
					}
				}
				else
				{
					...
					if( SkeletonBoneIndex == INDEX_NONE )
					{
						UE_LOGF(LogAnimation, Warning, "%ls : Missing joint on skeleton. Make sure to assign to the skeleton.", *OtherBoneName.ToString());
						return false;
					}
...
bool USkeletonModifier::SetSkeletalMesh(USkeletalMesh* InSkeletalMesh)
{
	...
	if (!InSkeletalMesh)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier: No skeletal mesh supplied to load.");
		return false;
	}
	...
	if (!Skeleton)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier: Skeletal Mesh supplied has no skeleton.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier: Cannot modify built-in engine asset.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier: mesh description is emtpy.");
		return false;
	}

	// store reference skeleton to edit
	SetReferenceSkeleton(SkeletalMesh->GetRefSkeleton());
...
bool USkeletonModifier::SetDynamicMesh(UDynamicMesh* InDynamicMesh)
{
	...
	FReferenceSkeleton MeshSkeleton;
	{
		const FDynamicMesh3& Mesh = InDynamicMesh->GetMeshRef();
		
		const int32 NumBones = Mesh.Attributes()->GetNumBones();
		FReferenceSkeletonModifier RefSkeletonModifier(MeshSkeleton, nullptr);
	...
	SetReferenceSkeleton(MeshSkeleton);
...
bool USkeletonModifier::SetReferenceSkeleton(const FReferenceSkeleton& InReferenceSkeleton)
{
	ReferenceSkeleton.Reset();
	TransformComposer.Reset();
	BoneIndexTracker.Reset();

	// store reference skeleton to read from
...
void USkeletonModifier::SetReadOnly(bool bInReadOnly)
{
	bReadOnly = bInReadOnly;
}

bool USkeletonModifier::IsReadOnly() const
{
	return bReadOnly;
}
			...
			for (int32 VertexID : Mesh.VertexIndicesItr())
			{
				...
				if (const int32 NumBoneWeights = BoneWeights.Num())
				{
					...
					for (int32 Idx = 0; Idx < NumBoneWeights; ++Idx)
					{
						...
						{
							UE_LOGF(LogAnimation, Warning, "Skeleton Modifier - Commit: Invalid bone index provided (%d); falling back to 0 as bone index.", BoneIndex);
						}

						if (NewBoneIndex == INDEX_NONE)
						{
							// Bone was deleted: fall back to root so the vertex stays visible instead of collapsing to the origin.
							// Note: ideally we should dump the weights of the deleted bone to its parent at the same time the bone
							// is deleted
							NewBoneIndex = 0;
						}

						const uint16 AddedRawWeight = OldBoneWeight.GetRawWeight();
						if (FBoneWeight* Existing = NewWeights.FindByPredicate(
							[NewBoneIndex](const FBoneWeight& W){ return W.GetBoneIndex() == NewBoneIndex; }))
						{
							// Multiple source weights collapsed to the same target bone (e.g. deleted bone + existing root weight on the same vert): sum, don't duplicate.
							Existing->SetRawWeight(static_cast<uint16>(
								FMath::Min<uint32>(uint32(Existing->GetRawWeight()) + uint32(AddedRawWeight), FBoneWeight::GetMaxRawWeight())));
						}
						else
						{
							NewWeights.Add(FBoneWeight(NewBoneIndex, AddedRawWeight));
						}
					}
...
bool USkeletonModifier::IsReferenceSkeletonValid(const bool bLog) const
{
	if (!ReferenceSkeleton.IsValid())
	{
		if(bLog)
		{
			UE_LOGF(LogAnimation, Error, "Skeleton Modifier: No valid reference skeleton provided.");
		}
		return false;		
	}
	return true;
		...
		else
		{
			for (int32 NewBoneIndex = 0; NewBoneIndex < NumNewBones; NewBoneIndex++)
			{
				...
				else
				{
					...
					// case-sensitive change only
					if (OtherBoneInfo.Name == NewBoneName && !OtherBoneInfo.Name.IsEqual(NewBoneName, ENameCase::CaseSensitive))
					{
						EnumAddFlags(Modifications, ESkeletalMeshModificationType::BonesRenamed);
					}
				}
...
bool USkeletonModifier::CommitSkeletonToSkeletalMesh()
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier: No mesh loaded. Cannot apply skeleton edits.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Warning, "Skeleton Modifier: No modification needed.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Warning, "Skeleton Modifier: Skeleton can't be modified.");
		return false;
	}
	...
	if (SkeletonModifications == ESkeletonModificationType::DuplicateAndMerge)
	{
		...
		if (!NewSkeleton)
		{
			UE_LOGF(LogAnimation, Warning, "Skeleton Modifier: Skeleton can't be duplicated.");
			return false;
		}
...
void USkeletonModifier::EnsureSingleRoot()
{
	...
	if (!BoneInfos.IsEmpty())
	{
		...
		if (!BonesToParent.IsEmpty())
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "Skeleton Modifier: %ls will be parented to the root bone before commiting.", *BoneName.ToString());
			}
...
void USkeletonModifier::NotifyFromSkeletonChanges() const
{
	...
	if (bDebug)
	{
		...
		for (const FAssetData& AssetData: Assets)
		{
			...
			{
				UE_LOGF(LogAnimation, Warning, "%ls references that skeleton.", *AssetData.GetExportTextName());
			}
		}
	}
}
...
bool USkeletonModifier::AddBone(const FName InBoneName, const FName InParentName, const FTransform& InTransform)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Add: Cannot add bone with no name.");
		return false;
	}
	...
	const TArray<FName>& InBoneNames, const TArray<FName>& InParentNames, const TArray<FTransform>& InTransforms)
{
	...
	if (NumBonesToAdd == 0)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Add: The provided bone names array is empty.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Add: None of the provided names is avalable to be added.");
		return false;
	}
...
bool USkeletonModifier::MirrorBone(const FName InBoneName, const FMirrorOptions& InOptions)
{
	if (InBoneName == NAME_None)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Mirror: Cannot mirror bone with no name.");
		return false;
	}
...
bool USkeletonModifier::MirrorBones(const TArray<FName>& InBonesName, const FMirrorOptions& InOptions)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Mirror: None of the provided names has been found.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Mirror: Couldn't find mirrored bones.");
		return false;
	}
...
{	
	// Similar to UMirrorDataTable::GetMirrorName but doing a search from the end if the find string is contained
	...
	const FName LeftFindRegex = *FString::Printf(TEXT("([^}]*)%s([^}]*)"), *InOptions.LeftString);
	const FName RightReplaceRegex = *FString::Printf(TEXT("$1%s$2"), *InOptions.RightString);

	const FName RightFindRegex = *FString::Printf(TEXT("([^}]*)%s([^}]*)"), *InOptions.RightString);
	const FName LeftReplaceRegex = *FString::Printf(TEXT("$1%s$2"), *InOptions.LeftString);

	const TArray MirrorFindReplaceExpressions({
		FMirrorFindReplaceExpression(LeftFindRegex, RightReplaceRegex, EMirrorFindReplaceMethod::RegularExpression),
		FMirrorFindReplaceExpression(RightFindRegex, LeftReplaceRegex, EMirrorFindReplaceMethod::RegularExpression)
	});
		
	const TArray<FMeshBoneInfo>& BoneInfos = ReferenceSkeleton->GetRawRefBoneInfo();
	
	const auto FindReplaceFromEnd = [](const FName& InBoneName, const FString& InFind, const FString& InReplace, FName& OutResult)
		{
			if (InBoneName.IsNone() ||
				InFind.IsEmpty() ||
				InReplace.IsEmpty())
			{
				// Since replacing can not occur, result to NAME_None implying a new name should be generated
				OutResult = NAME_None;
				return true;
			}
			...
			const int32 ReplaceIndex = BoneNameString.Find(InFind, ESearchCase::CaseSensitive, ESearchDir::FromEnd);
			if (ReplaceIndex != INDEX_NONE)
			{
				FString ReplaceBoneName = BoneNameString;
				ReplaceBoneName.RemoveAt(ReplaceIndex, InFind.Len());
				ReplaceBoneName.InsertAt(ReplaceIndex, InReplace);

				OutResult = *ReplaceBoneName;
				return true;
			}
	...
	Algo::Transform(InBonesToMirror, OutBonesName, [&](const int32 BoneIndex)
		{
			...
			const FName MirrorName = [&BoneName, &InOptions, &FindReplaceFromEnd, &MirrorFindReplaceExpressions]()
			{
				// Try to replace with an exact match from end, and if successful prefer it over 
				// UMirrorDataTable::GetMirrorName that does look ups from the front.
				// Still use UMirrorDataTable::GetMirrorName to support regular expressions.
				FName MirrorName;
				if (!FindReplaceFromEnd(BoneName, InOptions.LeftString, InOptions.RightString, MirrorName) &&
					!FindReplaceFromEnd(BoneName, InOptions.RightString, InOptions.LeftString, MirrorName))
				{
					MirrorName = UMirrorDataTable::GetMirrorName(BoneName, MirrorFindReplaceExpressions);
				}
	...
	const FMirrorOptions& InOptions, TArray<FTransform>& OutMirroredTransforms) const
{
	...
	TArray<int32> BonesAndBonesToMirror;
	BonesAndBonesToMirror.Append(InBonesToMirror);
	BonesAndBonesToMirror.Append(InMirroredBones);

	// compute global mirrored transforms
	TArray<FTransform> MirroredGlobal; 
	MirroredGlobal.Reserve(NumBonesToMirror);
	for (int32 Index = 0; Index < NumBonesToMirror; Index++)
	{
		// bone global
		const int32 RefBoneIndex = InBonesToMirror[Index];
		const FTransform GlobalTransform = TransformComposer->GetGlobalTransform(RefBoneIndex);

		// mirror
		const FTransform MirroredGlobalTransform = InOptions.MirrorTransform(GlobalTransform);

		MirroredGlobal.Add(MirroredGlobalTransform);
	}
...
bool USkeletonModifier::SetBoneTransform( const FName InBoneName, const FTransform& InNewTransform, const bool bMoveChildren)
{
	if (InBoneName == NAME_None)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Move: Cannot move bone with no name.");
		return false;
	}
	...
	const TArray<FName>& InBoneNames, const TArray<FTransform>& InNewTransforms, const bool bMoveChildren)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Move: Discrepancy between bones and transforms (%d / %d).", NumBonesToMove, InNewTransforms.Num());
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Move: None of the provided bones has been found.");
		return false;
	}
...
bool USkeletonModifier::RemoveBone(const FName InBoneName, const bool bRemoveChildren)
{
	if (InBoneName == NAME_None)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Remove: Cannot remove bone with no name.");
		return false;
	}
...
bool USkeletonModifier::RemoveBones(const TArray<FName>& InBoneNames, const bool bRemoveChildren)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Remove: No bone provided.");
		return false;
	}
	...
	{ // no bone has been removed
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Remove: No bone has been removed.");
		return false;
	}
...
bool USkeletonModifier::RenameBone(const FName InOldBoneName, const FName InNewBoneName)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Rename: cannot rename %ls with %ls.", *InOldBoneName.ToString(), *InNewBoneName.ToString());
		return false;
	}
...
bool USkeletonModifier::RenameBones(const TArray<FName>& InOldBoneNames, const TArray<FName>& InNewBoneNames)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Rename: Discrepancy between old and new names (%d / %d).", InOldBoneNames.Num(), InNewBoneNames.Num());
		return false;
	}
...
bool USkeletonModifier::ParentBone(const FName InBoneName, const FName InParentName)
{
	if (InBoneName == NAME_None)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Parent: Cannot parent a bone with no name.");
		return false;
	}
...
bool USkeletonModifier::ParentBones(const TArray<FName>& InBoneNames, const TArray<FName>& InParentNames)
{
	...
	if (InBoneNames.IsEmpty())
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Parent: No bone provided.");
		return false;
	}
...
bool USkeletonModifier::OrientBone(const FName InBoneName, const FOrientOptions& InOptions)
{
	if (InBoneName == NAME_None)
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Orient: Cannot orient a bone with no name.");
		return false;
	}
...
bool USkeletonModifier::OrientBones(const TArray<FName>& InBoneNames, const FOrientOptions& InOptions)
{
	...
	if (InBoneNames.IsEmpty())
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Orient: No bone provided.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Orient: None of the provided names has been found.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skeleton Modifier - Orient: No bone to orient.");
		return false;
	}
