...
bool USkinWeightModifier::SetSkeletalMesh(USkeletalMesh* InMesh)
{
	...
	if (!InMesh)
	{
		UE_LOGF(LogAnimation, Error, "Skin Weight Modifier: No skeletal mesh supplied to load.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skin Weight Modifier: Cannot modify built-in engine asset.");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimation, Error, "Skin Weight Modifier: mesh description is empty.");
		return false;
	}
	...
	for (int32 VertexIndex = 0; VertexIndex < NumVertices; VertexIndex++)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: Supplied skeletal mesh has a vertex with too many influences.");
		}
	}
	return true;
...
bool USkinWeightModifier::CommitWeightsToSkeletalMesh()
{
	if (!Mesh)
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: No mesh loaded. Cannot apply weights to skeletal mesh.");
		return false;
	}
...
TMap<FName, float> USkinWeightModifier::GetVertexWeights(int32 VertexID)
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: Supplied vertex index is not valid for this mesh.");
	...
	const bool bReplaceAll)
{
	if (!Mesh)
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: No mesh loaded. Cannot set weights.");
		return false;
	}

	if (!Weights.IsValidIndex(VertexID))
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: Supplied vertex index is not valid on this mesh. Cannot set weights.");
		return false;
	}
...
bool USkinWeightModifier::NormalizeVertexWeights(const int32 VertexID)
{
	if (!Mesh)
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: No mesh loaded. Cannot normalize weights.");
		return false;
	}

	if (!Weights.IsValidIndex(VertexID))
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: Supplied vertex ID was invalid. Cannot normalize vertex weights.");
		return false;
	}
...
bool USkinWeightModifier::EnforceMaxInfluences(const int32 MaxInfluences)
{
	if (!MeshDescription)
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: No mesh loaded. Cannot enforce max influences.");
		return false;
	}
...
bool USkinWeightModifier::PruneVertexWeights(const int32 VertexID, const float WeightThreshold)
{
	if (!Mesh)
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: No mesh loaded. Cannot prune weights.");
		return false;
	}

	if (!Weights.IsValidIndex(VertexID))
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: Supplied vertex ID was invalid. Cannot prune weights.");
		return false;
	}
...
int32 USkinWeightModifier::GetNumVertices()
{
	if (!MeshDescription)
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: No mesh loaded. Cannot get vertex count.");
		return INDEX_NONE;
	}
...
TArray<FName> USkinWeightModifier::GetAllBoneNames()
{
	if (!Mesh)
	{
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: No mesh loaded. Cannot get bones.");
		return TArray<FName>();
	}
...
bool USkinWeightModifier::NormalizeVertexInternal(const int32 VertexID)
{
	...
	if (VertexWeights.IsEmpty())
	{
		...
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: Supplied vertex did not have any bone influences. Applied all weight to root bone.");
		return true;
	}
	...
	if (TotalWeight <= KINDA_SMALL_NUMBER)
	{
		...
		UE_LOGF(LogAnimation, Warning, "Skin Weight Modifier: Supplied vertex had near zero total weight.");
		return true;
	}
