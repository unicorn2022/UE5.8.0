...
public:
	...
	FXRSwapChainPtr CreateSwapchain(XrSession InSession, FRHITexture* Template, ETextureCreateFlags CreateFlags)
	{
		...
		return CreateSwapchain(InSession,
			...
			Template->GetSizeX(),
			Template->GetSizeY(),
			Template->GetDesc().Dimension == ETextureDimension::Texture3D ? 1 : Template->GetSizeZ(),
			Template->GetNumMips(),
	...
	virtual bool PresentOnSubmissionThread(int32& InOutSyncInterval) override;
	...
	virtual void HMDOnFinishRendering_SubmissionThread();
