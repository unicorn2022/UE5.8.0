...
public:
	...
	struct FPipelinedLayerState
	{
		...
		TArray<FXrCompositionLayerUnion> NativeUnderlays; // Underlays will be rendered below the projection layers
		...
		TArray<FXRSwapChainPtr> NativeLayerSwapchains;	// This is currently only used as a reference to keep the swapchains from being destroyed
...
protected:
	...
	void DrawAllEmulatedLayers_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& InView);
	...
	[[nodiscard]] int32 GetNumHeaders() const;
...
public:
	...
	void OnFinishRendering(IRHICommandContext* RHICmdContext);
	void OnBackBufferReadyToPresent(class SWindow& Window, ISlateViewportProvider& ViewportProvider);
...
private:
	...
	class FEmulatedLayersPass* SetupEmulatedLayersRenderPass(
	...
	[[nodiscard]] FEmulatedLayersPass* SetupEmulatedLayerRenderPass(
		FRDGBuilder& GraphBuilder,
		const FSceneView& InView,
	...
	TArray<FDefaultStereoLayers::FStereoLayerToRender> EmulatedLayers;
