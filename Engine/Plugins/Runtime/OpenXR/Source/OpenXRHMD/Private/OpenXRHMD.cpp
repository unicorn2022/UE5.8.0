...
namespace UE::OpenXR
{
	static bool bStandAloneStereoOnlyDevice = false;
	static FAutoConsoleVariableRef CVarStandAloneStereoOnlyDevice(
		TEXT("XR.StandAloneStereoOnlyDevice"),
		bStandAloneStereoOnlyDevice,
		TEXT("If true, the current device will be considered stereo only, meaning it is a dedicated VR headset.  This is very often set in BaseDeviceProfiles.ini\n"),
		ECVF_Default | ECVF_ReadOnly);

	// This approach, albeit OpenXR compliant, uses one pass per stereo layer. Therefore, performance might be affected,
	// specially as the number of layers increases. This could / should potentially be improved in the future with a single ordered pass with
	// a custom depth test to apply depth to only layers that have it
	static bool bRenderEmulatedLayerDepth = true;
	static FAutoConsoleVariableRef CVarRenderEmulatedLayerDepth(
		TEXT("XR.StereoLayers.RenderEmulatedLayerDepth"),
		bRenderEmulatedLayerDepth,
		TEXT("If true, emulated stereo layers will be rendered to be fully compliant with OpenXR and support depth.\n"),
		ECVF_Default);
}
...
static TAutoConsoleVariable<int32> CVarOpenXRProjectionLayerPriority(
	TEXT("xr.OpenXRProjectionLayerPriority"),
	0,
	TEXT("Render priority for the projection layer (main UE environment). Stereo layers with priorities greater than or equal to this value will render above the projection layer. Layers with priorities less than this value will render below the projection layer."),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<bool> CVarOpenXRSortFaceLockedLayersAboveOtherLayers(
	TEXT("xr.OpenXRSortFaceLockedLayersAboveOtherLayers"),
	false,
	TEXT("Sort face locked stereo layers above all other layers, including the projection layer, even if the face locked layer has a lower priority. This was the standard behavior before UE 5.8."),
	ECVF_RenderThreadSafe);
...
bool FOpenXRHMD::GetCurrentInteractionProfile(const EControllerHand Hand, FString& InteractionProfile)
{
	...
	{
		UE_LOGF(LogHMD, Warning, "GetCurrentInteractionProfile failed because that EControllerHandValue %i does not map to a user path!", int(Hand));
		return false;
	}
	...
	if (Session)
	{
		...
		else
		{
			...
			UE_LOGF(LogHMD, Warning, "GetCurrentInteractionProfile for %i (%ls) failed because xrGetCurrentInteractionProfile failed with result %ls.", int(Hand), *PathStr, OpenXRResultToString(Result));
			return false;
		}
	}
	else
	{
		UE_LOGF(LogHMD, Warning, "GetCurrentInteractionProfile for %i failed because session is null!", int(Hand));
		return false;
	}
...
FVector2D FOpenXRHMD::GetPlayAreaBounds(EHMDTrackingOrigin::Type Origin) const
{
	...
	case EHMDTrackingOrigin::CustomOpenXR:
		...
		{
			UE_LOGF(LogHMD, Warning, "GetPlayAreaBounds(EHMDTrackingOrigin::CustomOpenXR), but we are not using a custom reference space now. Returning zero vector.");
	...
	{
		UE_LOGF(LogHMD, Warning, "GetPlayAreaBounds xrGetReferenceSpaceBoundsRect with reference space %ls failed with result %ls. Returning zero vector.", OpenXRReferenceSpaceTypeToString(Space), OpenXRResultToString(Result));
		return FVector2D::ZeroVector;
	}
...
void FOpenXRHMD::SetTrackingOrigin(EHMDTrackingOrigin::Type NewOrigin)
{
	...
	{
		UE_LOGF(LogHMD, Warning, "SetTrackingOrigin(EHMDTrackingOrigin::View) called, which is invalid (We allow getting the view transform as a tracking space, but we do not allow setting the tracking space origin to the View).  We are setting the tracking space to Local, to maintain legacy behavior, however ideally the blueprint calling this would be fixed to use Local space.");
	...
	if (NewOrigin == EHMDTrackingOrigin::CustomOpenXR)
	{
		if (!bUseCustomReferenceSpace)
		{
			UE_LOGF(LogHMD, Warning, "SetTrackingOrigin(EHMDTrackingOrigin::CustomOpenXR) called when bUseCustomReferenceSpace is false.  This call is being ignored.  Reference space will remain %ls.", OpenXRReferenceSpaceTypeToString(TrackingSpaceType));
			return;
		}
	...
	if (bUseCustomReferenceSpace)
	{
		UE_LOGF(LogHMD, Warning, "SetTrackingOrigin(%i) called when bUseCustomReferenceSpace is true.  This call is being ignored.  Reference space will remain custom %ls.", NewOrigin, OpenXRReferenceSpaceTypeToString(TrackingSpaceType));
		return;
	}
...
EHMDTrackingOrigin::Type FOpenXRHMD::GetTrackingOrigin() const
{
	...
	default:
		...
		{
			UE_LOGF(LogHMD, Warning, "GetTrackingOrigin() called when unexpected tracking space %ls is in use.  Returning EHMDTrackingOrigin::Local because it gives the fewest guarantees, but this value is not correct!  Perhaps this function needs to support more TrackingSpaceTypes?", OpenXRReferenceSpaceTypeToString(TrackingSpaceType));
			check(false);
			return EHMDTrackingOrigin::Local;
		}
	}
	...
	, RecommendedMotionVectorTextureSize(0, 0)
{
	...
	bHiddenAreaMaskSupported = IsExtensionEnabled(XR_KHR_VISIBILITY_MASK_EXTENSION_NAME) && !FCStringAnsi::Strstr(InstanceProperties.runtimeName, "Oculus");
	...
	bLocalFloorSpaceSupported = IsOpenXRAPIVersionMet(EOpenXRAPIVersion::V_1_1) || IsExtensionEnabled(XR_EXT_LOCAL_FLOOR_EXTENSION_NAME);
 	...
 	bIsStandaloneStereoOnlyDevice |= UE::OpenXR::bStandAloneStereoOnlyDevice;
	if (bIsStandaloneStereoOnlyDevice)
	{
		UE_LOGF(LogHMD, Log, "IsStandaloneStereoOnlyDevice %i because of XR.StandAloneStereoOnlyDevice", bIsStandaloneStereoOnlyDevice)
	}
	else
	{
	for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
	{
		if (Module->IsStandaloneStereoOnlyDevice())
		{
			bIsStandaloneStereoOnlyDevice = true;
				UE_LOGF(LogHMD, Log, "IsStandaloneStereoOnlyDevice %i because of Plugin %ls", bIsStandaloneStereoOnlyDevice, *Module->GetDisplayName())
				break;
			}
		}
	}
	...
	{
		FSlateApplication::Get().GetRenderer()->OnBackBufferReadyToPresent().AddRaw(this, &FOpenXRHMD::OnBackBufferReadyToPresent);
	}
}
...
bool FOpenXRHMD::ReconfigureForShaderPlatform(EShaderPlatform NewShaderPlatform)
{
	...
	UE_LOGF(LogHMD, Log, "HMD configured for shader platform %ls, bIsMobileMultiViewEnabled=%d, bProjectionLayerAlphaEnabled=%d",
...
void FOpenXRHMD::EnumerateViews(FPipelinedFrameState& PipelineState)
{
	...
	if (IsExtensionEnabled(XR_EXT_FRAME_SYNTHESIS_EXTENSION_NAME) && PipelineState.ViewConfigs.Num())
	{
		...
		ensure(RecommendedMotionVectorTextureSize.X && RecommendedMotionVectorTextureSize.Y);
	}
}
...
void FOpenXRHMD::BuildOcclusionMeshes()
{
	...
	{
		UE_LOGF(LogHMD, Error, "Failed to create all visibility mask meshes for device/views. Abandoning visibility mask.");
...
bool FOpenXRHMD::BuildOcclusionMesh(XrVisibilityMaskTypeKHR Type, int View, FHMDViewMesh& Mesh)
{
	...
	{
		UE_LOGF(LogHMD, Error, "Visibility Mask Mesh returned from runtime is invalid.");
		return false;
	}
...
int32 FOpenXRHMD::GetNumHeaders() const
{
	int32 NumHeaders = 0;
	for (const FOpenXRLayer& Layer : NativeLayers)
	{
		NumHeaders += Layer.GetNumHeaders();
	}
	return NumHeaders;
}
...
bool FOpenXRHMD::OnStereoStartup()
{
	...
	if (!AcquireSystemIdAndProperties())
	{
		UE_LOGF(LogHMD, Error, "Failed to get an OpenXR system, please check that you have a VR headset connected.");
		return false;
	}
	...
	{
		UE_LOGF(LogHMD, Error, "No compatible view configuration type found, falling back to runtime preferred type.");
	...
	if (RenderBridge.IsValid())
	{
		...
		if (!SessionInfo.next)
		{
			UE_LOGF(LogHMD, Warning, "Failed to get an OpenXR graphics binding, editor restart required.");
#if WITH_EDITOR
	...
	{
		UE_LOGF(LogHMD, Warning, "xrCreateSession failed.")
		return false;
	}

	UE_LOGF(LogHMD, Verbose, "xrCreateSession created %llu", Session);
	...
	if (!bUseExtensionSpectatorScreenController && !bIsStandaloneStereoOnlyDevice)
	{
		...
		UE_LOGF(LogHMD, Verbose, "OpenXR using base spectator screen.");
	}
	else
	{
		if (SpectatorScreenController == nullptr)
		{
			UE_LOGF(LogHMD, Verbose, "OpenXR disabling spectator screen.");
		}
		else
		{
			UE_LOGF(LogHMD, Verbose, "OpenXR using extension spectator screen.");
		}
	}
...
bool FOpenXRHMD::OnStereoTeardown()
{
	XrResult Result = XR_ERROR_SESSION_NOT_RUNNING;
	{
		...
		{
			UE_LOGF(LogHMD, Verbose, "FOpenXRHMD::OnStereoTeardown() calling xrRequestExitSession");
...
void FOpenXRHMD::DestroySession()
{
	...
	if (Session != XR_NULL_HANDLE)
	{
		...
		PipelinedLayerStateRendering.NativeLayerSwapchains.Reset();
		...
		PipelinedLayerStateRHI.NativeLayerSwapchains.Reset();
		...
		Session = XR_NULL_HANDLE;
		CurrentSessionState = XR_SESSION_STATE_UNKNOWN;
		UE_LOGF(LogHMD, Verbose, "Session state switched to XR_SESSION_STATE_UNKNOWN by DestroySession()");
		bStereoEnabled = false;
		bIsReady = false;
		bIsRunning = false;
		bIsRendering = false;
...
bool FOpenXRHMD::AllocateSwapchainTextures_RenderThread(const FOpenXRSwapchainProperties& InSwapchainProperties, FXRSwapChainPtr& InOutSwapchain, uint8& OutActualFormat)
{
		...
		SwapchainTexture->GetFlags() != InSwapchainProperties.CreateFlags)
	{
		...
		if (InOutSwapchain)
		{
			UE_LOGF(LogHMD, Verbose, "Allocated %.*ls OpenXR Swapchain of Format: %u, SizeX: %u, SizeY: %u, ArraySize: %u, NumMips: %u, NumSamples: %u, Flags: %llu",
				...
				InSwapchainProperties.NumMips,
				InSwapchainProperties.NumSamples,
				EnumToUnderlyingType(InSwapchainProperties.CreateFlags));
...
void FOpenXRHMD::SetEnvironmentBlendMode(XrEnvironmentBlendMode NewBlendMode) 
{
	...
	{
		UE_LOGF(LogHMD, Error, "Environment Blend Mode can't be set to XR_ENVIRONMENT_BLEND_MODE_MAX_ENUM.");
		return;
	}
	...
	if (BlendModes.Contains(NewBlendMode))
	{
		...
		UE_LOGF(LogHMD, Log, "Environment Blend Mode set to: %d.", SelectedEnvironmentBlendMode);
	}
	else
	{
		UE_LOGF(LogHMD, Error, "Environment Blend Mode %d is not supported. Environment Blend Mode remains %d.", NewBlendMode, SelectedEnvironmentBlendMode);
	}
}
...
bool FOpenXRHMD::AllocateRenderTargetTextures(FRHICommandListBase& RHICmdList, uint32 SizeX, uint32 SizeY, uint8 Format, uint32 NumMips, ETextureCreateFlags Flags, ETextureCreateFlags TargetableTextureFlags, TArray<FTextureRHIRef>& OutTargetableTextures, TArray<FTextureRHIRef>& OutShaderResourceTextures, uint32 NumSamples)
{
	check(IsInRenderingThread());
	...
	// Allow extension plugins to override the color swapchain format.
	uint8 PluginFormat = PF_Unknown;
	FString PluginFormatExtensionName;
	for (IOpenXRExtensionPlugin* Plugin : ExtensionPlugins)
	{
		const uint8 CurrentPluginFormat = Plugin->GetPreferredSwapchainFormat(Format);

		if (CurrentPluginFormat != PF_Unknown && CurrentPluginFormat < PF_MAX)
		{
			if (PluginFormat == PF_Unknown)
			{
				PluginFormat = CurrentPluginFormat;
				PluginFormatExtensionName = Plugin->GetDisplayName();
			}
			else if (CurrentPluginFormat != PluginFormat)
			{
				UE_LOG(LogHMD, Warning, TEXT("Extension %s requested swapchain format %s via GetPreferredSwapchainFormat, but we are already using format %s requested by extension %s."), 
					*Plugin->GetDisplayName(),
					GetPixelFormatString(static_cast<EPixelFormat>(CurrentPluginFormat)),
					GetPixelFormatString(static_cast<EPixelFormat>(PluginFormat)),
					*PluginFormatExtensionName);
			}
		}

		else if (CurrentPluginFormat >= PF_MAX)
		{
			UE_LOG(LogHMD, Warning, TEXT("Extension %s requested invalid swapchain format %d via GetPreferredSwapchainFormat."), *Plugin->GetDisplayName(), CurrentPluginFormat);
		}
	}
	...
	{
		UE_LOGF(LogHMD, Warning, "Requesting 10 bit swapchain, but not supported: fall back to 8bpc.");
	...
	if (!bIsAcquireOnAnyThreadSupported)
	{
		RHICmdList.EnqueueLambda([this, Swapchain](FRHICommandListBase&)
		{
	...
	if (IsEmulatingStereoLayers() && (SystemProperties.graphicsProperties.maxLayerCount > 1))
	{
		...
		if (!bIsAcquireOnAnyThreadSupported && EmulationSwapchain.IsValid())
		{
			RHICmdList.EnqueueLambda([this, EmulationSwapchain](FRHICommandListBase&)
			{
	...
	// Only create motion vector + motion vector depth swapchains if XR_EXT_frame_synthesis or XR_FB_space_warp are supported and r.Velocity.DirectlyRenderOpenXRMotionVectors=1
	FIntPoint MotionVectorSwapchainSize = FIntPoint();
	if (GetRecommendedMotionVectorTextureSize(MotionVectorSwapchainSize) && PlatformSupportsOpenXRMotionVectors(GetConfiguredShaderPlatform()))
	{
		...
		if (!bIsAcquireOnAnyThreadSupported && MotionVectorSwapchain.IsValid() && MotionVectorDepthSwapchain.IsValid())
		{
			RHICmdList.EnqueueLambda([this, MotionVectorSwapchain, MotionVectorDepthSwapchain](FRHICommandListBase&)
			{
...
void FOpenXRLayer::FPerEyeTextureData::ConfigureSwapchain(XrSession Session, FOpenXRRenderBridge* RenderBridge, FTextureRHIRef InTexture, bool bInStaticSwapchain)
{
	...
	if (Texture.IsValid())
	{
			...
			(bStaticSwapchain && bNewTexture))
		{
			...
			const bool bIsCubemap = Texture->GetDesc().Dimension == ETextureDimension::TextureCube || Texture->GetDesc().Dimension == ETextureDimension::TextureCubeArray;
			DynamicFlag |= bIsCubemap ? TexCreate_TargetArraySlicesIndependently : TexCreate_None;
			uint8 UnusedActualFormat = 0;
			
			Swapchain = RenderBridge->CreateSwapchain(
				Session,
				IStereoRenderTargetManager::GetStereoLayerPixelFormat(),
				UnusedActualFormat,
				Texture->GetSizeX(),
				Texture->GetSizeY(),
				Texture->GetDesc().Dimension == ETextureDimension::Texture3D ? 1 : Texture->GetSizeZ(),
				Texture->GetNumMips(),
...
void FOpenXRHMD::SetupFrameLayers_GameThread()
{
	if (GetStereoLayersDirty())
	{
			...
			if (!(Desc.Flags & LAYER_FLAG_HIDDEN))
			{
				...
				if (Desc.HasValidTexture())
				{
...
#if WITH_EDITOR
							LayersToRender.Last().Name = Desc.Name;
#endif
						}
						else
						{
							UE_LOGF(LogHMD, Warning, "Layer type not supported!");
						}
					...
					else
					{
						...
						{
							if (Desc.HasShape<FCylinderLayer>() && !bCylinderLayersSupported)
							{
								UE_LOGF(LogHMD, Warning, "Cylinder layers not supported!");
							}
							else if (Desc.HasShape<FEquirectLayer>() && !bEquirectLayersSupported)
							{
								UE_LOGF(LogHMD, Warning, "Equirect layers not supported!");
							}
							else if (Desc.HasShape<FCubemapLayer>() && !bCubemapLayersSupported)
							{
								UE_LOGF(LogHMD, Warning, "Cubemap layers not supported!");
							}
						}
					}
				}
		...
		const bool bUseLegacyFaceLockedLayersSorting = CVarOpenXRSortFaceLockedLayersAboveOtherLayers.GetValueOnAnyThread();

		// Sort from lowest priority to highest priority. Layers with higher priority will render on top of layers with lower priority.
		auto LayerCompare =
			[bUseLegacyFaceLockedLayersSorting]
			(const auto& DescA, const auto& DescB) -> bool
			{
				if (bUseLegacyFaceLockedLayersSorting && (DescA.PositionType == IStereoLayers::FaceLocked) != (DescB.PositionType ==
					IStereoLayers::FaceLocked))
				{
					return DescB.PositionType == IStereoLayers::FaceLocked;
				}
				else if (DescA.Priority != DescB.Priority)
				{
		...
		ENQUEUE_RENDER_COMMAND(OpenXRHMD_SetupFrameLayers)(
			[this,
				SwapchainUpdates = MoveTemp(SwapchainUpdates),
				LayersToRender = MoveTemp(LayersToRender),
				XVisibleLayerIds = VisibleLayerIds]
			(FRHICommandListImmediate& RHICmdList) mutable
			{
				...
				for (const FLayerToUpdateSwapchain& Update : SwapchainUpdates)
				{
					...
					int32 OldLayerIndex = NativeLayersBackup.IndexOfByPredicate(
						[LayerId = Layer.Desc.Id](const FOpenXRLayer& Layer)
						{
				...
				BackgroundCompositedEmulatedLayers.Reset();
				EmulatedFaceLockedLayers.Reset();
				EmulatedLayers.Reset();

				if (UE::OpenXR::bRenderEmulatedLayerDepth)
				{
					EmulatedLayers.Append(MoveTemp(LayersToRender));
					Algo::Sort(EmulatedLayers);
				}
				else
				{
...
{
	auto GetTexture =
		[](FTextureResource* Resource, ETextureDimension Dimension) -> FTextureRHIRef
		{
			FTextureRHIRef Tex = Resource ? Resource->GetTextureRHI() : nullptr;
			if (Tex && Tex->GetDesc().Dimension != Dimension)
			{
				UE_LOGF(LogHMD, Error, "Texture dimensions do not match");
				return nullptr;
			}
			return Tex;
		};
	...
	FTextureRHIRef Texture = GetTexture(Update.TextureResource, TargetDimension);
	FTextureRHIRef LeftTexture = GetTexture(Update.LeftTextureResource, TargetDimension);
...
void FOpenXRHMD::SetupFrameLayers_RenderThread(FRDGBuilder& GraphBuilder)
{
	...
	const int32 NumHeaders = GetNumHeaders();
	PipelinedLayerStateRendering.NativeOverlays.Reset(NumHeaders);
	PipelinedLayerStateRendering.NativeUnderlays.Reset(NumHeaders);
	PipelinedLayerStateRendering.NativeLayerSwapchains.Reset(NumHeaders);
	PipelinedLayerStateRendering.CompositionDepthTestLayers.Reset(NumHeaders);

	const bool bUseLegacyFaceLockedLayersSorting = CVarOpenXRSortFaceLockedLayersAboveOtherLayers.GetValueOnAnyThread();
	const int32 ProjectionLayerPriority = CVarOpenXRProjectionLayerPriority.GetValueOnAnyThread();
	...
	for (FOpenXRLayer& Layer : NativeLayers)
	{
		...
		const bool bIsFaceLocked = Layer.Desc.PositionType == ELayerType::FaceLocked;
		XrSpace Space = bIsFaceLocked ? DeviceSpaces[HMDDeviceId].Space : PipelinedFrameStateRendering.TrackingSpace->Handle;
		...
		if (Layer.Desc.Priority >= ProjectionLayerPriority || (bUseLegacyFaceLockedLayersSorting && bIsFaceLocked))
		{
			PipelinedLayerStateRendering.NativeOverlays.Append(Headers);
		}
		else
		{
			PipelinedLayerStateRendering.NativeUnderlays.Append(Headers);
		}
...
void FOpenXRHMD::UpdateLayerSwapchainTexture(FOpenXRLayer& Layer, FRDGBuilder& GraphBuilder)
{
	...
	if (Layer.RightEye.Swapchain.IsValid() && Layer.RightEye.Texture.IsValid())
	{
		...
		PipelinedLayerStateRendering.NativeLayerSwapchains.Add(Layer.RightEye.Swapchain);
	}
	if (Layer.LeftEye.Swapchain.IsValid() && Layer.LeftEye.Texture.IsValid())
	{
		...
		PipelinedLayerStateRendering.NativeLayerSwapchains.Add(Layer.LeftEye.Swapchain);
	}
}
...
{	
	...
	if (UE::OpenXR::bRenderEmulatedLayerDepth && PipelinedLayerStateRendering.DepthSwapchain)
	{
		DrawAllEmulatedLayers_RenderThread(GraphBuilder, InView);
	}
	else
	{
...
void FOpenXRHMD::DrawEmulatedFaceLockedLayers_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& InView)
{
	...
	GraphBuilder.AddPass(
		RDG_EVENT_NAME("OpenXREmulatedFaceLockedLayerRender"),
		PassInfo,
		ERDGPassFlags::Raster,
		[this, RenderParams](FRHICommandListImmediate& RHICmdList)
		{
			RHICmdList.SetViewport(
				(float)RenderParams.Viewport.Min.X,
				(float)RenderParams.Viewport.Min.Y,
				0.0f,
				(float)RenderParams.Viewport.Max.X,
				(float)RenderParams.Viewport.Max.Y,
				1.0f);
...
void FOpenXRHMD::DrawAllEmulatedLayers_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& InView)
{
	FRDGTextureRef RDGDepthTexture = RegisterExternalTexture(
		GraphBuilder,
		PipelinedLayerStateRendering.DepthSwapchain->GetTexture(),
		TEXT("OpenXR_Depth"));
	...
	auto RenderLayer =
		[this, RDGDepthTexture, RDGRenderTarget, &GraphBuilder, &InView](const FDefaultStereoLayers::FStereoLayerToRender& Layer)
		{
			FDefaultStereoLayers_LayerRenderParams RenderParams;
			FEmulatedLayersPass* PassInfo = SetupEmulatedLayerRenderPass(GraphBuilder, InView, Layer, RDGRenderTarget, RDGDepthTexture, RenderParams);
		
			GraphBuilder.AddPass(
#if WITH_EDITOR
			RDG_EVENT_NAME("OpenXREmulatedStereoLayerRender_%s", *Layer.Name),
#else
			RDG_EVENT_NAME("OpenXREmulatedStereoLayerRender"),
#endif
			PassInfo,
			ERDGPassFlags::Raster,
			[this, RenderParams, Layer](FRHICommandListImmediate& RHICmdList)
			{
				FDefaultStereoLayers::StereoLayerRender(RHICmdList, Layer, RenderParams);
			});
		};
	
	TArray<const FDefaultStereoLayers::FStereoLayerToRender*> NonWorldLocked;
	
	for (const FDefaultStereoLayers::FStereoLayerToRender& Layer : EmulatedLayers)
	{
		if (Layer.PositionType != ELayerType::WorldLocked)
		{
			NonWorldLocked.Add(&Layer);
			continue;
		}
	...
	for (const FDefaultStereoLayers::FStereoLayerToRender* Layer : NonWorldLocked)
	{
		RenderLayer(*Layer);
	}
}
...
void FOpenXRHMD::DrawBackgroundCompositedEmulatedLayers_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& InView)
{
	...
	GraphBuilder.AddPass(RDG_EVENT_NAME("OpenXREmulatedLayerRender"), PassInfo, ERDGPassFlags::Raster,
		[this, RenderParams](FRHICommandListImmediate& RHICmdList)
		{
			...
			if (!EnumHasAnyFlags(PipelinedLayerStateRendering.LayerStateFlags, EOpenXRLayerStateFlags::BackgroundLayerVisible))
			{
...
FEmulatedLayersPass* FOpenXRHMD::SetupEmulatedLayersRenderPass(
	FRDGBuilder& GraphBuilder,
	const FSceneView& InView,
	TArray<FDefaultStereoLayers::FStereoLayerToRender>& Layers,
	FRDGTextureRef RenderTarget,
	FDefaultStereoLayers_LayerRenderParams& OutRenderParams)
{
...
class FEmulatedLayersPass* FOpenXRHMD::SetupEmulatedLayerRenderPass(
	FRDGBuilder& GraphBuilder,
	const FSceneView& InView,
	const FDefaultStereoLayers::FStereoLayerToRender& Layer,
	FRDGTextureRef RenderTarget,
...
FDefaultStereoLayers_LayerRenderParams FOpenXRHMD::CalculateEmulatedLayerRenderParams(const FSceneView& InView)
{
	...
	const FMatrix& ProjectionMatrix = ModifiedViewMatrices.GetViewToClip();
	const FMatrix& ViewProjectionMatrix = ModifiedViewMatrices.GetWorldToClip();
...
void FOpenXRHMD::OnBeginRendering_RenderThread(FRDGBuilder& GraphBuilder, FSceneViewFamily& ViewFamily)
{
	...
	if (bIsRunning && PipelinedFrameStateRendering.bXrFrameStateUpdated)
	{
		...
		UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i EnqueueLambda OnBeginRendering_RHIThread", HMDThreadString(), PipelinedFrameStateRendering.WaitCount);
...
void FOpenXRHMD::OnBeginRendering_GameThread(FSceneViewFamily& InViewFamily)
{
	...
	ENQUEUE_RENDER_COMMAND(TransferFrameStateToRenderingThread)(
		...
		{
			UE_CLOGF(PipelinedFrameStateRendering.FrameState.predictedDisplayTime >= GameFrameState.FrameState.predictedDisplayTime,
				LogHMD, VeryVerbose, "Predicted display time went backwards from %lld to %lld", PipelinedFrameStateRendering.FrameState.predictedDisplayTime, GameFrameState.FrameState.predictedDisplayTime);

			UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i FOpenXRHMD TransferFrameStateToRenderingThread", HMDThreadString(), GameFrameState.WaitCount);
...
void FOpenXRHMD::OnBeginSimulation_GameThread()
{
	...
	++WaitCount;
	UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i xrWaitFrame Calling...", HMDThreadString(), WaitCount);
	{
	...
	UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i xrWaitFrame Complete", HMDThreadString(), WaitCount);
...
bool FOpenXRHMD::OnStartGameFrame(FWorldContext& WorldContext)
{
	...
	while (ReadNextEvent(&event))
	{
		...
		case XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED:
		{
			...
			UE_LOGF(LogHMD, Verbose, "Session state switching to %ls", OpenXRSessionStateToString(CurrentSessionState));
...
{
	UE_LOGF(LogHMD, Log, "FOpenXRHMD is requesting app exit.  CurrentSessionState: %ls", OpenXRSessionStateToString(CurrentSessionState));
...
void FOpenXRHMD::OnBeginRendering_RHIThread(IRHICommandContext& RHICmdContext, const FPipelinedFrameState& InFrameState, FXRSwapChainPtr ColorSwapchain, FXRSwapChainPtr DepthSwapchain, FXRSwapChainPtr MotionVectorSwapchain, FXRSwapChainPtr MotionVectorDepthSwapchain, FXRSwapChainPtr EmulationSwapchain)
{
	...
	UE_CLOGF(bIsRendering, LogHMD, Verbose, "Discarded previous frame and started rendering a new frame.");
	...
	{
		UE_LOGF(LogHMD, Verbose, "FOpenXRHMD::OnBeginRendering_RHIThread returning before xrBeginFrame because xrWaitFrame %i is already consumed.  This is expected twice during LoadMap and may also happen during other 'extra' render pumps.", InFrameState.WaitCount);
		return;
	}
	...
	UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i xrBeginFrame BeginCount: %i", HMDThreadString(), PipelinedFrameStateRHI.WaitCount, PipelinedFrameStateRHI.BeginCount);
	...
	if (XR_SUCCEEDED(Result))
	{
		...
		UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i Rendering frame predicted to be displayed at %lld", 
	...
	else
	{
		...
		{
			UE_LOGF(LogHMD, Error, "Unexpected error on xrBeginFrame. Error code was %ls.", OpenXRResultToString(Result));
			bLoggedBeginFrameFailure = true;
		}
	}
}

void FOpenXRHMD::OnBackBufferReadyToPresent(SWindow& Window, ISlateViewportProvider& ViewportProvider)
{
	// Hook the present for this window
	if (Window.GetViewport() && Window.GetViewport()->IsStereoscopic3D())
	{
		ViewportProvider.SetCustomPresent(RenderBridge);
	}
}

void FOpenXRHMD::OnFinishRendering(IRHICommandContext* RHICmdContext)
{
	...
	UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i FOpenXRHMD::OnFinishRendering releasing swapchain images now.", HMDThreadString(), PipelinedFrameStateRHI.WaitCount);
	...
	if (bIsRunning)
	{
		...
		// Layers are rendered in the order they're submitted to the compositor, so earlier layers in the Headers list will appear below later layers.
		// So we add underlays, then projection layers, then overlays. Underlays will appear below the projection layers, and overlays will appear above.
		for (FXrCompositionLayerUnion& NativeUnderlay : PipelinedLayerStateRHI.NativeUnderlays)
		{
			Headers.Add(reinterpret_cast<XrCompositionLayerBaseHeader*>(&NativeUnderlay.Header));
		}

		XrCompositionLayerProjection BackgroundLayer = {};
		...
		{
			BackgroundLayer.type = XR_TYPE_COMPOSITION_LAYER_PROJECTION;
			BackgroundLayer.next = nullptr;
			BackgroundLayer.layerFlags = bProjectionLayerAlphaEnabled ? XR_COMPOSITION_LAYER_BLEND_TEXTURE_SOURCE_ALPHA_BIT : 0;
			BackgroundLayer.space = PipelinedFrameStateRHI.TrackingSpace->Handle;
			BackgroundLayer.viewCount = PipelinedLayerStateRHI.ProjectionLayers.Num();
			BackgroundLayer.views = PipelinedLayerStateRHI.ProjectionLayers.GetData();

			Headers.Add(reinterpret_cast<XrCompositionLayerBaseHeader*>(&BackgroundLayer));
			...
			if (CVarOpenXRInvertAlpha.GetValueOnRenderThread())
			{
				...
				{
					BackgroundLayer.layerFlags |= XR_COMPOSITION_LAYER_INVERTED_ALPHA_BIT_EXT;
				}
				else if (IsExtensionEnabled(XR_FB_COMPOSITION_LAYER_ALPHA_BLEND_EXTENSION_NAME))
				{
					LayerAlphaBlend.next = const_cast<void*>(BackgroundLayer.next);
					...
					BackgroundLayer.next = &LayerAlphaBlend;
				}
			}

			if (bCompositionLayerColorScaleBiasSupported)
			{
				ColorScaleBias.next = const_cast<void*>(BackgroundLayer.next);
				...
				BackgroundLayer.next = &ColorScaleBias;
			}
			...
			{
				LayerDepthTest.next = const_cast<void*>(BackgroundLayer.next);
				LayerDepthTest.depthMask = true;
				LayerDepthTest.compareOp = XR_COMPARE_OP_LESS_FB;
				BackgroundLayer.next = &LayerDepthTest;
			}

			for (IOpenXRExtensionPlugin* Module : ExtensionPlugins)
			{
				BackgroundLayer.next = Module->OnEndProjectionLayer_RHIThread(Session, 0, BackgroundLayer.next, BackgroundLayer.layerFlags);
			}
		}
		...
		for (FXrCompositionLayerUnion& NativeOverlay : PipelinedLayerStateRHI.NativeOverlays)
		{
			Headers.Add(reinterpret_cast<XrCompositionLayerBaseHeader*>(&NativeOverlay.Header));
		}
		...
		UE_LOGF(LogHMD, VeryVerbose, "Presenting frame predicted to be displayed at %lld", PipelinedFrameStateRHI.FrameState.predictedDisplayTime);
		...
		UE_LOGF(LogHMD, VeryVerbose, "%ls WF_%i xrEndFrame WaitCount: %i BeginCount: %i EndCount: %i", HMDThreadString(), PipelinedFrameStateRHI.WaitCount, PipelinedFrameStateRHI.WaitCount, PipelinedFrameStateRHI.BeginCount, PipelinedFrameStateRHI.EndCount);
	...
	FIntRect DstRect, bool bClearBlack, EXRCopyTextureBlendModifier SrcTextureCopyModifier, FStaticFeatureLevel FeatureLevel, FStaticShaderPlatform ShaderPlatform) const
{
	...
	AddPass(GraphBuilder, RDG_EVENT_NAME("OpenXRHMD_AcquireLayerSwapchain"), [this, bAcquireOnRender, DstSwapChain](FRHICommandListImmediate& RHICmdList)
		{
		RHICmdList.EnqueueLambda([this, bAcquireOnRender, DstSwapChain](FRHICommandListImmediate& InRHICmdList)
		{
			RenderBridge->RunOnRHISubmissionThread([bAcquireOnRender, DstSwapChain]()
			{
				if (!bAcquireOnRender)
				{
					DstSwapChain->IncrementSwapChainIndex_RHIThread();
...
void FOpenXRHMD::RenderTexture_RenderThread(FRDGBuilder& GraphBuilder, FRDGTextureRef BackBuffer, FRDGTextureRef SrcTexture, FVector2f WindowSize) const
{
	if (SpectatorScreenController)
	{
		const bool bShouldPassLayersTexture =
			EnumHasAnyFlags(PipelinedLayerStateRendering.LayerStateFlags, EOpenXRLayerStateFlags::SubmitEmulatedFaceLockedLayer) &&
			!CVarOpenXRDoNotCopyEmulatedLayersToSpectatorScreen.GetValueOnRenderThread();
		
		const FRDGTextureRef LayersTexture =
			bShouldPassLayersTexture ?
			RegisterExternalTexture(GraphBuilder, PipelinedLayerStateRendering.EmulatedLayerState.EmulationSwapchain->GetTextureRef(), TEXT("OpenXRLayersTexture")) :
			nullptr;
