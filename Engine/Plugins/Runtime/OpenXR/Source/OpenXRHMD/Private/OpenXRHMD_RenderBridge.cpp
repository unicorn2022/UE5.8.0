...
bool FOpenXRRenderBridge::Present(FRHIViewport* Viewport, IRHICommandContext& RHICmdContext, int32& InOutSyncInterval)
{
...
bool FOpenXRRenderBridge::PresentOnSubmissionThread(int32& InOutSyncInterval)
{
	bool bNeedsNativePresent = true;

	if (OpenXRHMD)
	{
		HMDOnFinishRendering_SubmissionThread();
		bNeedsNativePresent = !OpenXRHMD->IsStandaloneStereoOnlyDevice();
	}
...
void FOpenXRRenderBridge::HMDOnFinishRendering_RHIThread(IRHICommandContext& RHICmdContext)
{
	if (OpenXRHMD)
	{
		...
		OpenXRHMD->OnFinishRendering(&RHICmdContext);
	}
}

void FOpenXRRenderBridge::HMDOnFinishRendering_SubmissionThread()
{
	if (OpenXRHMD)
	{
		check(!IsInRenderingThread() && !IsInRHIThread());
		check(OpenXRHMD->RuntimeRequiresRHIContext() == false);
		
		IRHICommandContext* RHIContext = nullptr;
		OpenXRHMD->OnFinishRendering(RHIContext);
	}
}
...
public:
	...
	virtual void* GetGraphicsBinding(XrSystemId InSystem) override
	{
		LUID AdapterLuid = GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU()->GetAdapterLuid();
		...
		Binding.next = nullptr;
		Binding.device = GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU();
...
public:
	...
	virtual void* GetGraphicsBinding(XrSystemId InSystem) override
	{
		...
		{
			UE_LOGF(LogHMD, Error, "The OpenGL API version does not meet the minimum version required by the OpenXR runtime");
			return nullptr;
		}

		if (RHIVersion > Requirements.maxApiVersionSupported)
		{
			UE_LOGF(LogHMD, Warning, "The OpenGL API version has not been tested with the OpenXR runtime");
		}
...
public:
	...
	virtual void* GetGraphicsBinding(XrSystemId InSystem) override
	{
		...
		{
			UE_LOGF(LogHMD, Error, "The OpenGLES API version does not meet the minimum version required by the OpenXR runtime");
			return nullptr;
		}

		if (RHIVersion > Requirements.maxApiVersionSupported)
		{
			UE_LOGF(LogHMD, Warning, "The OpenGLES API version has not been tested with the OpenXR runtime");
		}
...
public:
	...
	virtual void* GetGraphicsBinding(XrSystemId InSystem) override
	{
		...
		if (RHIVersion < Requirements.minApiVersionSupported) //-V547
		{
			UE_LOGF(LogHMD, Fatal, "The Vulkan API version does not meet the minimum version required by the OpenXR runtime");
		}

		if (RHIVersion > Requirements.maxApiVersionSupported)
		{
			UE_LOGF(LogHMD, Warning, "The Vulkan API version has not been tested with the OpenXR runtime");
		}
