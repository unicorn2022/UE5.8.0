...
{
	const bool bUseDepthTest = 
		EnumHasAllFlags((IStereoLayers::ELayerFlags)Desc.Flags, IStereoLayers::LAYER_FLAG_SUPPORT_DEPTH) &&
...
int32 FOpenXRLayer::GetNumHeaders() const
{
	const bool bHasRightEye = RightEye.Swapchain.IsValid(); 
	const bool bHasLeftEye = LeftEye.Swapchain.IsValid(); 
	return bHasRightEye && bHasLeftEye ? 2 : bHasRightEye || bHasLeftEye ? 1 : 0;
}
...
void FOpenXRLayer::CreateOpenXRCylinderLayer(bool bIsStereo, bool bNoAlpha, FTransform PositionTransform, float WorldToMeters, XrSpace Space, TArray<FXrCompositionLayerUnion>& Headers) const
{
	...
	Cylinder.layerFlags = bNoAlpha ? 0 : XR_COMPOSITION_LAYER_UNPREMULTIPLIED_ALPHA_BIT | XR_COMPOSITION_LAYER_BLEND_TEXTURE_SOURCE_ALPHA_BIT;
...
void FOpenXRLayer::CreateOpenXREquirect2Layer(bool bIsStereo, bool bNoAlpha, FTransform PositionTransform, float WorldToMeters, XrSpace Space, TArray<FXrCompositionLayerUnion>& Headers) const
{
	...
	Equirect2.layerFlags = 
		bNoAlpha ? 
		0 :
		XR_COMPOSITION_LAYER_UNPREMULTIPLIED_ALPHA_BIT | XR_COMPOSITION_LAYER_BLEND_TEXTURE_SOURCE_ALPHA_BIT;
	
	Equirect2.space = Space;
	Equirect2.subImage.imageArrayIndex = 0;
	Equirect2.radius = FMath::Abs(EquirectProps.Radius / WorldToMeters);
	FXrCompositionLayerUnion LayerUnion;
...
void FOpenXRLayer::CreateOpenXRCubemapLayer(bool bIsStereo, bool bNoAlpha, FTransform PositionTransform, float WorldToMeters, XrSpace Space, TArray<FXrCompositionLayerUnion>& Headers) const
{
	...
	Cube.layerFlags = bNoAlpha ? 0 : XR_COMPOSITION_LAYER_UNPREMULTIPLIED_ALPHA_BIT | XR_COMPOSITION_LAYER_BLEND_TEXTURE_SOURCE_ALPHA_BIT;
	Cube.space = Space;
