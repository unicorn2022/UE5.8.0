...
void FOpenXRSwapchain::IncrementSwapChainIndex_RHIThread()
{
	...
	if (WasAcquired)
	{
		UE_LOGF(LogHMD, Verbose, "Attempted to redundantly acquire image %d in swapchain %p", SwapChainIndex_RHIThread.load(), reinterpret_cast<const void*>(Handle));
		return;
	}
	...
	UE_LOGF(LogHMD, VeryVerbose, "FOpenXRSwapchain::IncrementSwapChainIndex_RHIThread() Acquired image %d in swapchain %p metal texture: 0x%llx", SwapChainIndex, reinterpret_cast<const void*>(Handle), RHITexture.GetReference()->GetNativeResource());
}
...
void FOpenXRSwapchain::WaitCurrentImage_RHIThread(int64 Timeout)
{
	...
	if (!WasAcquired)
	{
		UE_LOGF(LogHMD, Warning, "Attempted to wait on unacquired image %d in swapchain %p", SwapChainIndex_RHIThread.load(), reinterpret_cast<const void*>(Handle));
		return;
	}
	...
	if (WasReady)
	{
		UE_LOGF(LogHMD, Verbose, "Attempted to redundantly wait on image %d in swapchain %p", SwapChainIndex_RHIThread.load(), reinterpret_cast<const void*>(Handle));
		return;
	}
	...
	do
	{
		...
		{
			UE_LOGF(LogHMD, Warning, "Timed out waiting on swapchain image %u! Attempts remaining %d.", SwapChainIndex_RHIThread.load(), RetryCount);
		}
	...
	if (WaitResult != XR_SUCCESS) //-V547
	{
		...
		UE_LOGF(LogHMD, Fatal, "Failed to wait on acquired swapchain image. This usually indicates a problem with the OpenXR runtime.");
	}
	
	UE_LOGF(LogHMD, VeryVerbose, "FOpenXRSwapchain::WaitCurrentImage_RHIThread() Waited on image swapchain %p", reinterpret_cast<const void*>(Handle));
}
...
void FOpenXRSwapchain::ReleaseCurrentImage_RHIThread(IRHICommandContext* RHICmdContext)
{
	...
	if (!WasReady)
	{
		UE_LOGF(LogHMD, Warning, "Attempted to release image %d in swapchain %p that wasn't ready for being written to.", SwapChainIndex_RHIThread.load(), reinterpret_cast<const void*>(Handle));
		return;
	}
	...
	UE_LOGF(LogHMD, VeryVerbose, "FOpenXRSwapchain::ReleaseCurrentImage_RHIThread() Released on image in swapchain %p", reinterpret_cast<const void*>(Handle));
}
...
uint8 FOpenXRSwapchain::GetNearestSupportedSwapchainFormat(XrSession InSession, uint8 RequestedFormat, TFunction<uint32(uint8)> ToPlatformFormat /*= nullptr*/)
{
	...
	if (!FallbackFormat)
	{
		UE_LOGF(LogHMD, Warning, "No compatible swapchain format found!");
		return PF_Unknown;
	}

	UE_LOGF(LogHMD, Warning, "Swapchain format not supported (%d), falling back to runtime preferred format (%d).", PlatformFormat, FallbackPlatformFormat);
...
XrSwapchain FOpenXRSwapchain::CreateSwapchain(XrSession InSession, uint32 PlatformFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, void* Next /*= nullptr*/)
{
	...
	if (EnumHasAnyFlags(CreateFlags, TexCreate_RenderTargetable | TexCreate_DepthStencilTargetable))
	{
...
TArray<T> EnumerateImages(XrSwapchain InSwapchain, XrStructureType InType, void* Next = nullptr)
{
	...
	if (!XR_ENSURE(xrEnumerateSwapchainImages(InSwapchain, 0, &ChainCount, nullptr)) || ChainCount == 0)
	{
		return Images;
	}
	...
	if (!XR_ENSURE(xrEnumerateSwapchainImages(InSwapchain, ChainCount, &ChainCount, reinterpret_cast<XrSwapchainImageBaseHeader*>(Images.GetData()))))
	{
		Images.Empty();
	}
	return Images;
...
FXRSwapChainPtr CreateSwapchain_D3D11(XrSession InSession, uint8 Format, uint8& OutActualFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, const FClearValueBinding& ClearValueBinding, ETextureCreateFlags AuxiliaryCreateFlags)
{
	...
	if (Images.IsEmpty())
	{
		UE_LOGF(LogHMD, Error, "Failed to enumerate D3D11 swapchain images");
	...
	for (const auto& Image : Images)
	{
		...
		case ETextureDimension::TextureCube:
			...
			break;
		default:
			UE_LOGF(LogHMD, Error, "Unsupported texture dimension");
			return nullptr;
		}
	}
...
FXRSwapChainPtr CreateSwapchain_D3D12(XrSession InSession, uint8 Format, uint8& OutActualFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, const FClearValueBinding& ClearValueBinding, ETextureCreateFlags AuxiliaryCreateFlags)
{
	...
	if (Images.IsEmpty())
	{
		UE_LOGF(LogHMD, Error, "Failed to enumerate D3D12 swapchain images");
		xrDestroySwapchain(Swapchain);
		return nullptr;
	}
	ETextureCreateFlags SanitizedCreateFlags = SanitizeTextureCreateFlags(CreateFlags);
...
FXRSwapChainPtr CreateSwapchain_OpenGL(XrSession InSession, uint8 Format, uint8& OutActualFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, const FClearValueBinding& ClearValueBinding, ETextureCreateFlags AuxiliaryCreateFlags)
{
	...
	if (Images.IsEmpty())
	{
		UE_LOGF(LogHMD, Error, "Failed to enumerate OpenGL swapchain images");
		xrDestroySwapchain(Swapchain);
		return nullptr;
	}
	ETextureCreateFlags SanitizedCreateFlags = SanitizeTextureCreateFlags(CreateFlags);
...
FXRSwapChainPtr CreateSwapchain_OpenGLES(XrSession InSession, uint8 Format, uint8& OutActualFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, const FClearValueBinding& ClearValueBinding, ETextureCreateFlags AuxiliaryCreateFlags)
{
	...
	if (Images.IsEmpty())
	{
		UE_LOGF(LogHMD, Error, "Failed to enumerate OpenGL ES swapchain images");
		xrDestroySwapchain(Swapchain);
		return nullptr;
	}
	ETextureCreateFlags SanitizedCreateFlags = SanitizeTextureCreateFlags(CreateFlags);
...
FXRSwapChainPtr CreateSwapchain_Vulkan(XrSession InSession, uint8 Format, uint8& OutActualFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, const FClearValueBinding& ClearValueBinding, ETextureCreateFlags AuxiliaryCreateFlags)
{
	...
	XrSwapchainCreateInfoFoveationFB FoveationCreateInfo { XR_TYPE_SWAPCHAIN_CREATE_INFO_FOVEATION_FB };
	XrVulkanSwapchainCreateInfoMETA VulkanMetaCreateInfo { XR_TYPE_VULKAN_SWAPCHAIN_CREATE_INFO_META };
	if (EnumHasAllFlags(AuxiliaryCreateFlags, TexCreate_Foveation))
	{
		FoveationCreateInfo.next = Next;
		...
		{
			VulkanMetaCreateInfo.next = &FoveationCreateInfo;
			VulkanMetaCreateInfo.additionalUsageFlags = 0;
			VulkanMetaCreateInfo.additionalCreateFlags = VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_EXT;

			Next = &VulkanMetaCreateInfo;
		}
	}
	...
	if (Images.IsEmpty())
	{
		UE_LOGF(LogHMD, Error, "Failed to enumerate Vulkan swapchain images");
		xrDestroySwapchain(Swapchain);
		return nullptr;
	}
	ETextureCreateFlags SanitizedCreateFlags = SanitizeTextureCreateFlags(CreateFlags);
