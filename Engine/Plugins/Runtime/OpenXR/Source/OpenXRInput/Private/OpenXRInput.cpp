...
bool FOpenXRInputPlugin::FOpenXRInput::BuildActions(XrSession Session)
{
		...
		auto AddProfile = [&Profiles](const char* const Profile, const char* const PathChars, bool HasHaptics)
			{
				...
				{
					UE_LOGF(LogHMD, Warning, "BuildActions tried to AddProfile: '%ls' Path: '%ls', but it looks like the path creation failed. Profile not added. Look above for path related error messages.", ANSI_TO_TCHAR(Profile), ANSI_TO_TCHAR(PathChars));
					return;
				}
	...
	if (InputSettings && InputSettings->bEnableDefaultMappingContexts)
	{
		for (const auto& Context : InputSettings->DefaultMappingContexts)
		{
			...
			{
				UE_LOGF(LogHMD, Warning, "Default Mapping Contexts contains an Input Mapping Context set to \"None\", ignoring while building OpenXR actions.");
			}
		}
	}
	...
	{
		UE_LOGF(LogHMD, Warning, "No mapping context provided in the OpenXR Input project settings, action bindings will not be visible to the OpenXR runtime. Action/Axis mappings from the Input configuration are deprecated for XR in favor of Enhanced Input.");
...
void FOpenXRInputPlugin::FOpenXRInput::BuildEnhancedActions(TMap<FString, FInteractionProfile>& Profiles)
{
	...
	for (const auto& MappingContext : InputMappingContextToPriorityMap)
	{
		...
		// Handle any description string duplicates
		{
			...
			{
				UE_LOGF(LogHMD, Warning, "Input Mapping Context %ls has a Description, \"%ls\", which exactly matches the Description already used by Input Mapping Context %ls.  Identical localized descriptions are not allowed by OpenXR.  The FName of this mapping context %ls, which is unique, will replace the duplicated localized string so the Input Mapping Context functions but it will not localize correctly."
		...
		for (const FEnhancedActionKeyMapping& Mapping : MappingContext.Key->GetMappings())
		{
			...
			if (ActionIndex == INDEX_NONE)
			{
				...
				if (!ensureMsgf(!NewActionName.GetDisplayNameEntry()->IsWide(), TEXT("Input Action %s has wide characters in its name, This name will be passed to OpenXR which does not support wide characters in path names (see spec).  Ignoring this action, it will not function."), *NewActionName.ToString()))
				{
					continue;
				}
				...
				FString NewActionDescription = Mapping.Action->ActionDescription.ToString();
				{
					...
					{
						UE_LOGF(LogHMD, Warning, "Input Action %ls has a Description, \"%ls\", which exactly matches the Description already used by Action %ls.  Identical localized descriptions are not allowed by OpenXR.  The FName of this action %ls, which is unique, will replace the localized string so that the input Action functions but it will not localize correctly."
...
bool FOpenXRInputPlugin::FOpenXRInput::SuggestBindingForKey(TMap<FString, FInteractionProfile>& Profiles, FOpenXRAction& Action, const FKey& InFKey, const TArray<UInputModifier*>& Modifiers, const TArray<UInputTrigger*>& Triggers)
{
	...
	else if(Identifier.StartsWith(TEXT("dpad")))
	{
		if(bDirectionalBindingSupported)
		{
			// dpad inputs are specified as <ProfileName>_<Hand>_DPad<Direction>_<action>.  Normally, we'd have a _ between
			// "dpad" and the direction of the button, but in order to match the hard NUM_XR_KEY_TOKENS
			// formatting requirement, we detect if the third Token starts with "dpad," and extract the direction in order
			// to form the longer path required.
			Identifier = "dpad_" + Identifier.Mid(4);
		}
		else
		{
			return false;
...
bool FOpenXRInputPlugin::FOpenXRInput::SetEnhancedInputUserSettings(TObjectPtr<class UEnhancedInputUserSettings> InputSettings)
{
	if (bActionsAttached)
	{
		UE_LOGF(LogHMD, Error, "Attempted to attach a set of enhanced user input settings when actions are already attached for the current session.");
...
bool FOpenXRInputPlugin::FOpenXRInput::AttachInputMappingContexts(const TSet<TObjectPtr<UInputMappingContext>>& MappingContexts)
{
	if (bActionsAttached)
	{
		UE_LOGF(LogHMD, Error, "Attempted to attach input mapping contexts when action sets are already attached for the current session.");
