...
namespace UE::MultiServerProxy::Private
{
	...
	struct MULTISERVERREPLICATION_API FPendingBroadcastRPC
	{
		TObjectKey<const UFunction> Function;

		TArray<uint8> Parms;

		TObjectKey<const UObject> SubObject;

		FPendingBroadcastRPC() = default;
		FPendingBroadcastRPC(FPendingBroadcastRPC&& Other) = default;
		FPendingBroadcastRPC& operator=(FPendingBroadcastRPC&& Other) = default;

		FPendingBroadcastRPC(const UFunction* InFunction, const void* InParms, const UObject* InSubObject);
		FPendingBroadcastRPC(const FPendingBroadcastRPC& Other);
		FPendingBroadcastRPC& operator=(const FPendingBroadcastRPC& Other);
		~FPendingBroadcastRPC();
...
struct FMultiServerProxyInternalConnectionRoute
{
	...
	/** Queue to hold RPCs that will be broadcast to all game servers using BroadcastToAllClientConnectedGameServers(). */
	TArray<UE::MultiServerProxy::Private::FPendingBroadcastRPC> PendingBroadcastRPCs;
...
protected:
	...
	virtual void RemoveLocalPlayer(UGameInstance* GameInstance, ULocalPlayer* LocalPlayer) override;
};
...
private:
	...
	void OnIrisRootObjectPostInit(UE::Net::FNetRefHandle RootHandle, UObject* RootObject, UE::Net::FNetObjectFactoryId FactoryId);
	void OnIrisRootObjectDetached(UE::Net::FNetRefHandle RootHandle, UObject* RootObject, UE::Net::EDetachReason Reason);
	void OnIrisSubObjectPostInit(UE::Net::FNetRefHandle SubObjectHandle, UObject* SubObject, UE::Net::FNetRefHandle RootObjectHandle, UObject* RootObject, UE::Net::FNetObjectFactoryId FactoryId);
	void OnIrisSubObjectDetached(UE::Net::FNetRefHandle SubObjectHandle, UObject* SubObject, UE::Net::FNetRefHandle RootObjectHandle, UObject* RootObject, UE::Net::EDetachReason Reason);

	UPROPERTY()
	TObjectPtr<UProxyNetDriver> ProxyNetDriver;

	FDelegateHandle OnIrisRootObjectPostInitHandle;
	FDelegateHandle OnIrisRootObjectDetachedHandle;
	FDelegateHandle OnIrisSubObjectPostInitHandle;
	FDelegateHandle OnIrisSubObjectDetachedHandle;
};
...
public:
	...
	/** Cycle through game servers as primary game servers each time a client connects to the proxy. */
	void SetCyclePrimaryGameServer(bool bEnable);
	...
	/** Forward an RPC to all primary and non-primary game servers for a proxy client connection using PlayerToProxyConnection. */
	void BroadcastToAllClientConnectedGameServers(const ULocalPlayer* Player, const UFunction* Function, const void* Parms, const UObject* SubObject);

	/** Return the proxy connection for a local player, or nullptr if not found. */
	UNetConnection* GetProxyConnectionForPlayer(const ULocalPlayer* Player) const;
	...
	/** Flush any broadcast RPCs that were queued while a route was not yet connected. */
	void FlushPendingBroadcastRPCs(FMultiServerProxyInternalConnectionRoute* Route);
...
private:
	...
	friend class FNetTestProxyAllowListExecOnProxy;
	friend class FNetTestProxyNonAllowListedSkipsProxy;
	friend class FNetTestProxyNonBroadcastPrimaryOnly;
	friend class FNetTestProxyBroadcastReachesAllServers;
	friend class FNetTestProxyGCCleansPendingBroadcastRPCs;
	...
	/** Clean up any internal data structures that hold weak object references that are now invalid. */
	void PreGarbageCollect();
	void PostGarbageCollect();

	/** 
	 * Send an RPC correct proxy connection based on the actor's owner and local player.
	 *
	 * This function won't perform logic to send the RPC; that logic is expected to be called as part of the function
	 * argument RPCSendingFunction. RPCSendingFunction will be provided the derived proxy connection, if one can be found, as an argument.
	 */
	void RedirectRPCToProxyConnection(AActor* Actor, UFunction* Function, TFunction<void(UNetConnection*)> RPCSendingFunction);

	/** (Iris only) Configure the connection to own this particular actor (required for replication conditionals and internal filtering logic). */
	void SetProxyConnectionAsOwner(UNetConnection* ProxyConnection, const AActor* Actor);
	...
	/** Map a local player to it's proxy net connection. Multiple players will point to the same proxy net connection. */
	TMap<TObjectKey<class ULocalPlayer>, TObjectPtr<UNetConnection>> PlayerToProxyConnection;

	FDelegateHandle ProxyPostGarbageCollectHandle;
	FDelegateHandle ProxyPreGarbageCollectHandle;
	...
	TSet<FSoftObjectPath> BroadcastToAllGameServersFunctions;

	/** RPCs that are allow-listed to be executed on the proxy when forwarding (both client->server and server->client). */
	TSet<FSoftObjectPath> AllowListedProxyFunctions;
};
