	...
	if(URL.Valid)
	{
		if (InitBase() && NetDriver)
		{
			...
			else
			{
				...
				UE_LOGF(LogBeacon, Log, "AMultiServerBeaconHost::InitHost failed");
				OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_multiserverhost_init_host_failure"));
			}
		}
	}
	...
	if (NetDriver->ServerConnection == nullptr)
	{
		...
		case NMT_BeaconJoin:
			...
			if (AtCapacity())
			{
				...
				UE_LOGF(LogBeacon, Log, "%ls: %ls", *Connection->GetName(), *ErrorMsg);
