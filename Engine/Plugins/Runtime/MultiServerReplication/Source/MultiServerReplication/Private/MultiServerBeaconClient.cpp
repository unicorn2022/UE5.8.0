...
void AMultiServerBeaconClient::OnConnected()
{
	...
	UE_LOGF(LogMultiServerBeacon, Log, "MultiServer beacon connection established.");
...
void AMultiServerBeaconClient::ConnectToServer(const FString& ConnectInfo)
{
	...
	Params.URL = FURL(nullptr, *ConnectInfo, TRAVEL_Absolute);
	Params.bReplicateTransactionally = false;

	if (InitClient(Params))
	{
		bSuccess = true;
	}
	else
	{
		UE_LOGF(LogMultiServerBeacon, Warning, "ConnectToRelay: Failure to init client beacon with %ls.", *Params.URL.ToString());
	}

	if (!bSuccess)
	{
		AOnlineBeacon::OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_multiserverclient_init_beacon_failure"));
	}
}
...
void AMultiServerBeaconClient::ClientPeerConnected_Implementation(const FString& NewRemotePeerId, AMultiServerBeaconClient* Beacon)
{
	...
	if (OwningNode)
	{
		OwningNode->NotifyMultiServerConnected(OwningNode->GetLocalPeerId(), NewRemotePeerId, this);
	}
}
...
bool AMultiServerBeaconClient::ServerUpdateLevelVisibility_Validate(const FUpdateLevelVisibilityLevelInfo& LevelVisibility)
{
	...
	{
		UE_LOGF(LogMultiServerBeacon, Warning, "ServerUpdateLevelVisibility() Invalid package name: %ls (%ls)", *LevelVisibility.PackageName.ToString(), *Reason.ToString());
		return false;
	}
...
{
	UE_LOGF(LogMultiServerBeacon, Log, "ServerSetRemotePeerId: %ls %ls", *GetNameSafe(this), *NewRemoteId);
	RemotePeerId = NewRemoteId;

	if (OwningNode)
	{
		OwningNode->NotifyMultiServerConnected(OwningNode->GetLocalPeerId(), NewRemoteId, this);
	}
}
