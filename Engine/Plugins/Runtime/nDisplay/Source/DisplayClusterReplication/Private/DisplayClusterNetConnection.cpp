...
void UDisplayClusterNetConnection::SetClientLoginState(const EClientLoginState::Type NewState)
{
	...
	if (ClientLoginState == EClientLoginState::Type::ReceivedJoin)
	{
		...
		{
			UE_LOGF(LogDisplayClusterNetDriver, Verbose, "Cluster connection Join has failed: URI arguments are invalid");
			return;
		}
		...
		if (PrimaryNodeId == NodeName)
		{
			...
			UE_LOGF(LogDisplayClusterNetDriver, Verbose, "Cluster connection joined: Client [%u]; Primary node [%ls]; RemoteAddr: %ls:%i", ClientId, *NodeName, *NodeAddress, NodePort);
		}
		else
		{
			UE_LOGF(LogDisplayClusterNetDriver, Verbose, "Cluster connection joined: Client [%u]; Node [%ls]; RemoteAddr: %ls", ClientId, *NodeName, *NodeAddress);
		}
...
void UDisplayClusterNetConnection::ReceivedPacket(FBitReader& Reader, bool bIsReinjectedPacket, bool bDispatchPacket)
{
	...
	if (bSynchronousMode && !DisplayClusterNetDriver->IsServer())
	{
		...
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Packet %i received and accumulated", InPacketId);
	}
	else
	{
		...
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Packet %i received and processed", InPacketId);
	}
}
...
void UDisplayClusterNetConnection::ProcessPacket(int32 PacketId)
{
	...
	if (DisplayClusterNetDriver == nullptr)
	{
		UE_LOGF(LogDisplayClusterNetDriver, Error, "ProcessPacket: Can't cast NetDriver to DisplayClusterNetDriver");
		return;
	}

	if (!InPackets.Contains(PacketId))
	{
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Packet %i skipped!", PacketId);
	...
	for (int32 CurrentPacketId : PacketIDs)
	{
		...
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Packet %i processed", CurrentPacketId);
