...
bool UDisplayClusterNetDriver::InitListen(FNetworkNotify* InNotify, FURL& ListenURL, bool bReuseAddressAndPort, FString& Error)
{
	...
	if (World->GetNetMode() == NM_ListenServer)
	{
		...
		{
			UE_LOGF(LogDisplayClusterNetDriver, Warning, "InitListen:: Can't get required URL arguments");
		...
		if (ClusterNetworkDriverHelper->RegisterClusterEventsBinaryClient(ClusterId, NodeAddress, NodePort))
		{
			...
			UE_LOGF(LogDisplayClusterNetDriver, Verbose, "Registered primary node for cluster [%u] node [%ls:%u]", ClusterId, *NodeAddress, NodePort);
		}
	}
...
void UDisplayClusterNetDriver::TickDispatch(float DeltaTime)
{
	...
	{
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Primary Node connections = [%u]", PrimaryNodeConnections.Num());
		...
		for (UDisplayClusterNetConnection* PrimaryNodeConnectionIt : PrimaryNodeConnections)
		{
			...
			{
				UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "PrimaryNodeConnectionIt->ClusterNodesNum = [%u], ClusterNodeConnections.Num() = [%u]", PrimaryNodeConnectionIt->ClusterNodesNum, ClusterNodeConnections.Num());
				continue;
			}
			...
			{
				UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Registered primary node for cluster [%u] node [%ls]", PrimaryNodeConnectionIt->ClusterId, *PrimaryNodeConnectionIt->NodeName);
			}
...
void UDisplayClusterNetDriver::RemoveNodeConnection(UDisplayClusterNetConnection* NetConnection)
{
	if (NetConnection->bNodeIsPrimary)
	{
		...
		{
			UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Removed primary node for cluster [%u] node [%ls]", NetConnection->ClusterId, *NetConnection->NodeName);
		}
...
void UDisplayClusterNetDriver::HandleEvent(FDisplayClusterClusterEventBinary const& InEvent)
{
	...
	{
		UE_LOGF(LogDisplayClusterNetDriver, Error, "HandleEvent: Can't cast NetConnection to DriverClusterNetConnection");
		return;
	}

	if (InEvent.EventId == NodeSyncEvent)
	{
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "[StartSynchronousMode] command received");
		...
		{
			UE_LOGF(LogDisplayClusterNetDriver, Error, "Can't reset internal idle mode, GEngine is not UDisplayClusterGameEngine instance");
		}

		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Client node self-introducing: %u", ClusterServerConnection->ClientId);
	}
	...
	if (InEvent.EventId == PacketSyncEvent)
	{
		...
		while (ReadOffset < InEvent.EventData.Num())
		{
			...
			if (CommandClientId == ClusterServerConnection->ClientId)
			{
				...
				UE_LOGF(LogDisplayClusterNetDriver, Verbose, "[ProcessPacket %i] command received", CommandPacketId);
...
void UDisplayClusterNetDriver::TickFlush(float DeltaSeconds)
{
	if (IsServer())
	{
		...
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "bSkipServerReplicateActors %u; ClusterNumNodes: %d, SyncConnections.Num(): %d", bSkipServerReplicateActors,
	...
	if (bCanFormSlice)
	{
		...
		for (UDisplayClusterNetConnection* SyncConnectionIt : SyncConnections)
		{
			...
			UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Client %u; Packet %i sent, %i acked", ConnectionUniqueId, LastOutPacketId, SyncConnectionIt->OutAckPacketId);
		}
	}
	...
	if (bPacketsReadyForSync)
	{
		...
		for (UDisplayClusterNetConnection* SyncConnection : SyncConnections)
		{
			...
			UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Client %u; [ProcessPacket %i] command sent", SyncConnection->ClientId, ReadyForProcessingPacket);
		}
...
void UDisplayClusterNetDriver::PreListUpdate(ConsiderListUpdateParams const& UpdateParams, int& OutUpdated, const TArray<FNetworkObjectInfo*>& ConsiderList)
{
	...
	if (World->GetNetMode() == NM_ListenServer)
	{
		if (UDisplayClusterNetConnection** ClusterConnection = NodeConnections.FindArbitraryElement())
		{
...
bool UDisplayClusterNetDriver::NotifyClusterAsReadyForSync(int32 ClusterId)
{
	...
	if (bEventWasSent)
	{
		...
		UE_LOGF(LogDisplayClusterNetDriver, VeryVerbose, "Setting synchronous mode in cluster [%u]", ClusterId);
