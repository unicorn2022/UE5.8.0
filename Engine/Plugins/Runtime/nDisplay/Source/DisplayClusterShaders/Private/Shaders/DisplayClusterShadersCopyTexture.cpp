	...
	*/
	...
	public:
			...
			FColorEncodingCopyRectPSPermutation::FCommonPSDomain& OutPermutationVector)
		{
			...
			{
				UE_LOGF(LogDisplayClusterShaders, Warning, "Invalid permutation vector %d for shader `FColorEncodingCopyRectPS`", OutPermutationVector.ToDimensionValueId());
...
#if UE_BUILD_DEBUG
			// Dest should be RTT (debug purpose)
			check(EnumHasAnyFlags(DestTexture->GetDesc().Flags, ETextureCreateFlags::RenderTargetable));
#endif
