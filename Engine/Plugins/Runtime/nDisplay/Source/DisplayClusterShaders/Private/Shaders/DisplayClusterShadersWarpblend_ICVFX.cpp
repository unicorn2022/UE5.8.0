...
public:
	...
	bool RenderCurentPass(FRHICommandListImmediate& RHICmdList, FIcvfxRenderPassData& RenderPassData)
	{
		...
		{
			UE_LOGF(LogDisplayClusterShaders, Warning, "Invalid permutation vector %d for shader FIcvfxWarpPS", RenderPassData.PSPermutationVector.ToDimensionValueId());
		...
		{
			UE_LOGF(LogDisplayClusterShaders, Warning, "Invalid permutation vector %d for shader FIcvfxWarpVS", RenderPassData.VSPermutationVector.ToDimensionValueId());
		...
		if (!VertexShader.IsValid() || !PixelShader.IsValid())
		{
			UE_LOGF(LogDisplayClusterShaders, Warning, "ICVFX shaders are not initialized properly.");
