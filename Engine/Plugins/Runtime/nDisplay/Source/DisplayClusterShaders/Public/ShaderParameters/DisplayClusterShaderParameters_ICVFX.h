...
public:
	...
	void SetParameters(const FDisplayClusterShaderParameters_ICVFX& InParameters)
	{
		...
		EnforcedCameraOrder = InParameters.EnforcedCameraOrder;
	}

	inline void SortCamerasRenderOrder(const bool bReverseOrder = false)
	{
		// Default sorting: by RenderOrder, then alphabetically by ViewportId
		auto DefaultCameraSort = [](const FCameraSettings& It1, const FCameraSettings& It2)
		{
		...
		// If we have an enforced camera order, apply it first
		if (!EnforcedCameraOrder.IsEmpty())
		{
			TArray<FCameraSettings> EnforcedCameras;
			TArray<FCameraSettings> RemainingCameras;

			// Separate cameras into enforced and remaining groups
			for (const FCameraSettings& CameraIt : Cameras)
			{
				const int32 EnforcedIndex = EnforcedCameraOrder.IndexOfByKey(CameraIt.Resource.ViewportId);
				if (EnforcedIndex != INDEX_NONE)
				{
					EnforcedCameras.Add(CameraIt);
				}
				else
				{
					RemainingCameras.Add(CameraIt);
				}
			}

			// Sort remaining cameras using default logic
			RemainingCameras.Sort(DefaultCameraSort);

			// Sort enforced cameras by their position in the EnforcedCameraOrder array
			// Index 0 in EnforcedCameraOrder composes on top, and conveniently appears at top in the details panel.
			// Note: The final order will be reversed if the bReverseOrder option is used.
			EnforcedCameras.Sort([this](const FCameraSettings& It1, const FCameraSettings& It2)
			{
				const int32 Index1 = EnforcedCameraOrder.IndexOfByKey(It1.Resource.ViewportId);
				const int32 Index2 = EnforcedCameraOrder.IndexOfByKey(It2.Resource.ViewportId);
				return Index1 > Index2; // Reverse: lower index renders later (on top)
			});

			// Combine EnforcedCameras with RemainingCameras. EnforcedCameras fill the top of the stack,
			// while the remaining ones are appended and preserve their original relative order.
			Cameras = RemainingCameras;
			Cameras.Append(EnforcedCameras);
		}
		else
		{
			// No enforced order, use default sorting
			Cameras.Sort(DefaultCameraSort);
		}

		// Apply reverse order as the final step
		if (bReverseOrder)
		{
			Algo::Reverse(Cameras);
		}
	}
...
public:
	...
	public:
		...
		/**
		 * Camera render order priority. Lower values render first (appear underneath), higher values render last (appear on top).
		 * When multiple cameras have the same RenderOrder, they are sorted alphabetically by viewport ID.
		 * Can be overridden per-viewport using EnforcedCameraOrder array.
		 */
		int32 RenderOrder = -1;
	};
	...
	float LightCardGamma = 2.2f;

	// Enforced camera order for custom sorting. Contains the camera viewport ids, where index 0 should be composed on top, index 1 below it, and so on. 
	TArray<FString> EnforcedCameraOrder;
};
