...
bool FDisplayClusterWarpBlendMath_Frustum::ImplCalcFrustumProjection_WarpMap(const FMatrix& InGeometryToFrustumMatrix)
{
	...
	{
		const FIntPoint WarpMapTextureSize(WarpMapTexture->GetWidth(), WarpMapTexture->GetHeight());
		if (WarpMapTextureSize.GetMin() < 2)
		{
			// Minimal texture size should be at least 2x2
			return false;
		}

		bool bAllPointsInFrustum = true;

		const int32 PointsAmount = WarpMapTextureSize.X * WarpMapTextureSize.Y;
