...
{
	UE_LOGF(LogDisplayClusterMedia, Verbose, "MediaInput '%ls': importing texture on RT frame '%llu'...", *GetMediaId(), GFrameCounterRenderThread);
	...
	{
		UE_LOGF(LogDisplayClusterMedia, Warning, "MediaInput '%ls': wrong texture on RT frame '%llu'...", *GetMediaId(), GFrameCounterRenderThread);
		return;
	}
...
void FDisplayClusterMediaInputBase::OnMediaEvent(EMediaEvent MediaEvent)
{
	...
	case EMediaEvent::MediaConnecting:
		UE_LOGF(LogDisplayClusterMedia, Log, "Media event for '%ls': Connection", *GetMediaId());
		break;

	/** A new media source has been opened. */
	case EMediaEvent::MediaOpened:
		UE_LOGF(LogDisplayClusterMedia, Log, "Media event for '%ls': Opened", *GetMediaId());
		break;

	/** The current media source has been closed. */
	case EMediaEvent::MediaClosed:
		UE_LOGF(LogDisplayClusterMedia, Log, "Media event for '%ls': Closed", *GetMediaId());
		OnPlayerClosed();
		break;
		
	/** A media source failed to open. */
	case EMediaEvent::MediaOpenFailed:
		UE_LOGF(LogDisplayClusterMedia, Log, "Media event for '%ls': OpenFailed", *GetMediaId());
		break;

	default:
		UE_LOGF(LogDisplayClusterMedia, Log, "Media event for '%ls': %d", *GetMediaId(), static_cast<int32>(MediaEvent));
		break;
	}
}
...
bool FDisplayClusterMediaInputBase::StartPlayer()
{
	...
	if (bIsPlaying)
	{
		UE_LOGF(LogDisplayClusterMedia, Log, "Started playing media: %ls", *GetMediaId());
	}
	else
	{
		UE_LOGF(LogDisplayClusterMedia, Warning, "Couldn't start playing media: %ls", *GetMediaId());
	}
...
void FDisplayClusterMediaInputBase::OnPlayerClosed()
{
	if (MediaPlayer && bWasPlayerStarted)
	{
		...
		{
			UE_LOGF(LogDisplayClusterMedia, Log, "MediaPlayer '%ls' is in error, restarting it.", *GetMediaId());
