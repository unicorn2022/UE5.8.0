...
FIntPoint FDisplayClusterMediaCaptureNodeTile::GetCaptureSize() const
{
	...
	{
		UE_LOGF(LogDisplayClusterMedia, Warning, "'%ls' couldn't get viewport size", *GetMediaId());
		return { };
	}
	...
	UE_LOGF(LogDisplayClusterMedia, Log, "'%ls' capture size is [%d, %d]", *GetMediaId(), ThisTileWidth, ThisTileHeight);
...
void FDisplayClusterMediaCaptureNodeTile::ProcessPostBackbufferUpdated_RenderThread(FRHICommandListImmediate& RHICmdList, FViewport* Viewport)
{
	if (!Viewport)
	{
		UE_LOGF(LogDisplayClusterMedia, Warning, "'%ls' couldn't capture backbuffer tile [%d:%d], nullptr viewport", *GetMediaId(), TilePosition.X, TilePosition.Y);
		return;
	}

	if (!bValidTileSettings)
	{
		UE_LOGF(LogDisplayClusterMedia, Warning, "'%ls' couldn't capture backbuffer tile [%d:%d], layout [%d:%d] - invalid tile settings", *GetMediaId(), TilePosition.X, TilePosition.Y, TileLayout.X, TileLayout.Y);
		return;
	}
	...
	if (FRHITexture* const BackbufferTexture = Viewport->GetRenderTargetTexture())
	{
		...
		UE_LOGF(LogDisplayClusterMedia, VeryVerbose, "'%ls' capturing backbuffer tile [%d:%d], region [%d:%d - %d:%d] of size [%dx%d]",
