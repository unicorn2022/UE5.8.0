...
FIntPoint FDisplayClusterMediaCaptureViewportBase::GetViewportSize() const
{
	...
	{
		UE_LOGF(LogDisplayClusterMedia, Verbose, "'%ls' acquired capture size from game proxy [%d, %d]", *GetMediaId(), CaptureSize.X, CaptureSize.Y);
	}
	else if (GetCaptureSizeFromConfig(CaptureSize))
	{
		UE_LOGF(LogDisplayClusterMedia, Verbose, "'%ls' acquired capture size from config [%d, %d]", *GetMediaId(), CaptureSize.X, CaptureSize.Y);
	}
	else
	{
		UE_LOGF(LogDisplayClusterMedia, Verbose, "'%ls' couldn't acquire capture size", *GetMediaId());
	}
