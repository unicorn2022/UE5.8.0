...
{
	UE_LOGF(LogDisplayClusterStageMonitoringDWM, Log, "DWM Sync watchdog active");
...
void FDWMSyncWatchdog::OnPresentationPreSynchronization_RHIThread(FRHIViewport* Viewport)
{
	...
	if (IDXGISwapChain* const SwapChain = static_cast<IDXGISwapChain*>(Viewport->GetNativeSwapChain()))
	{
		...
		if (SwapChain->GetFrameStatistics(&Stats) == S_OK)
		{
			...
			{
				UE_LOGF(LogDisplayClusterStageMonitoringDWM, VeryVerbose, "PreSync(%d): PresentCount: %u, LastPresentCount: %u, PresentRefreshCount: %u, SyncRefreshCount: %u"
...
void FDWMSyncWatchdog::OnPresentationPostSynchronization_RHIThread(FRHIViewport* Viewport)
{
	...
	if (IDXGISwapChain* const SwapChain = static_cast<IDXGISwapChain*>(Viewport->GetNativeSwapChain()))
	{
		...
		if (SwapChain->GetFrameStatistics(&Stats) == S_OK)
		{
			...
			if (SwapChain->GetLastPresentCount(&LastPresentCount) == S_OK)
			{
				...
				UE_LOGF(LogDisplayClusterStageMonitoringDWM, VeryVerbose, "PostSync(%d): PresentCount: %u, LastPresentCount: %u, PresentRefreshCount: %u, SyncRefreshCount: %u"
					, GFrameNumberRenderThread
					, Stats.PresentCount
					, LastPresentCount
					, Stats.PresentRefreshCount
				...
				if (PreviousFrameCount.IsSet())
				{
					...
					if (DeltaFrame > 1)
					{
						...
						UE_LOGF(LogDisplayClusterStageMonitoringDWM, Warning, "'%d' VBlanks were missed.", MissedFrames);
