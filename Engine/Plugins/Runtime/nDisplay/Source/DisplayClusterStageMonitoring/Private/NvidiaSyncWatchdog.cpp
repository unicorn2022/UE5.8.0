...
void FNvidiaSyncWatchdog::OnCustomPresentCreated()
{
	...
	{
	case ERHIInterfaceType::D3D11: D3DDevice = GetID3D11DynamicRHI()->RHIGetDevice(); break;
	case ERHIInterfaceType::D3D12: D3DDevice = GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU(); break;
	}

	if (D3DDevice)
	{
		UE_LOGF(LogDisplayClusterStageMonitoring, VeryVerbose, "Nvidia Sync watchdog active");
...
void FNvidiaSyncWatchdog::OnPresentationPreSynchronization_RHIThread(FRHIViewport* Viewport)
{
...
void FNvidiaSyncWatchdog::OnPresentationPostSynchronization_RHIThread(FRHIViewport* Viewport)
{
#if WITH_NVAPI
	...
	if (AsyncFrameCountFuture.IsValid())
	{
		...
		if (AsyncFrameCountFuture.IsReady())
		{
			...
			{
				UE_LOGF(LogDisplayClusterStageMonitoring, Verbose, "Couldn't query frame count from nvapi, error code 0x%x", FutureResult.QueryResult);
			...
			else
			{
				...
				if (PreviousFrameCount.IsSet())
				{
					...
					if (DeltaFrame != 1)
					{
						...
						UE_LOGF(LogDisplayClusterStageMonitoring, Verbose, "Missed '%d' frames. Last frame took '%0.5f' ms and waited '%0.5f' for the hardware sync", MissedFrames, LastFrameTimeMS, SynchronizationTimeMS);
		...
		{
			UE_LOGF(LogDisplayClusterStageMonitoring, Warning, "Could not completed query of NvAPI frame counter inside a frame");
		}
	}
	...
	AsyncFrameCountFuture = Async(EAsyncExecution::ThreadPool, [AsyncD3DDevice = D3DDevice, PreAsyncTaskCycles]()
	{
		...
		{
			UE_LOGF(LogDisplayClusterStageMonitoring, VeryVerbose, "New NvAPI FrameCount = '%d'. Took %0.6fms to complete", Result.FrameCount, KickTaskToCompletionMilliseconds);
		}
