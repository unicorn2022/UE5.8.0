...
{	
	...
	UE_LOGF(LogDisplayClusterFillDerivedDataCache, Display, "Running commandlet: %ls %ls", *CurrentExecutableName, *Arguments);
...
void FDisplayClusterFillDerivedDataCacheWorker::CompleteCommandletAndShowNotification()
{
	if (bWasCancelled)
	{
		UE_LOGF(LogDisplayClusterFillDerivedDataCache, Display, "#### Commandlet Process Cancelled ####");
	...
	{
		UE_LOGF(LogDisplayClusterFillDerivedDataCache, Error, "#### Commandlet Finished With Errors ####");
		UE_LOGF(LogDisplayClusterFillDerivedDataCache, Error, "%ls %ls", *CurrentExecutableName, *Arguments);
		UE_LOGF(LogDisplayClusterFillDerivedDataCache, Error, "Return Code: %i", ResultCode);
	...
	{
		UE_LOGF(LogDisplayClusterFillDerivedDataCache, Display, "#### Commandlet Finished Without Error ####");
	}
...
void FDisplayClusterFillDerivedDataCacheWorker::ReadCommandletOutputAndUpdateEditorNotification()
{
	while (FPlatformProcess::IsProcRunning(ProcessHandle))
	{
		...
		{
			UE_LOGF(LogDisplayClusterFillDerivedDataCache, Display, "----------CANCEL BUTTON PRESSED----------");
			CancelTask();
			break;
		}
		...
		for (const FString& Line : EachLineInLogBlock)
		{
			...
			UE_LOGF(LogDisplayClusterFillDerivedDataCache, Display, "Commandlet Output: %ls", *Line)
	...
	UE_LOGF(LogDisplayClusterFillDerivedDataCache, Display, "----------PROC NOT RUNNING----------");
	CompleteCommandletAndShowNotification();
}
