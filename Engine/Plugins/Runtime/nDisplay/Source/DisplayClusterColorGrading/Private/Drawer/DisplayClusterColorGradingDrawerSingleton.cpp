...
void FDisplayClusterColorGradingDrawerSingleton::RefreshColorGradingDrawers()
{
	...
	// Update any top-level color grading drawers open in the level editor
	FLevelEditorModule& LevelEditor = FModuleManager::Get().LoadModuleChecked<FLevelEditorModule>("LevelEditor");
	if (TSharedPtr<SDockTab> ExistingTab = LevelEditor.GetLevelEditorTabManager()->FindExistingLiveTab(IColorGradingEditor::Get().GetColorGradingTabSpawnerId()))
	{
		TSharedRef<SColorGradingPanel> ColorGradingPanel = StaticCastSharedRef<SColorGradingPanel>(ExistingTab->GetContent());
		ColorGradingPanel->Refresh();
	}
}
