...
bool FDisplayClusterProjectionMPCDIPolicy::HandleStartScene(IDisplayClusterViewport* InViewport)
{
	...
	if (WarpBlendInterface.IsValid() == false && !CreateWarpBlendFromConfig(InViewport))
	{
		...
		{
			UE_LOGF(LogDisplayClusterProjectionMPCDI, Warning, "Couldn't load MPCDI config for viewport '%ls'", *InViewport->GetId());
		}
...
bool FDisplayClusterProjectionMPCDIPolicy::CalculateView(IDisplayClusterViewport* InViewport, const uint32 InContextNum, FVector& InOutViewLocation, FRotator& InOutViewRotation, const FVector& InViewOffset, const float WorldToMeters, const float NCP, const float FCP)
{
	...
	if (WarpBlendInterface.IsValid() == false || WarpBlendContexts.Num() == 0)
	{
		if (!IsEditorOperationMode(InViewport))
		{
			UE_LOGF(LogDisplayClusterProjectionMPCDI, Warning, "Invalid warp data for viewport '%ls'", *InViewport->GetId());
		}
...
bool FDisplayClusterProjectionMPCDIPolicy::IsFrustumRotatedToFitContextSize_RenderThread(const IDisplayClusterViewportProxy* InViewportProxy, const uint32 InContextNum)
{
	check(IsInRenderingThread());

	if (WarpBlendContexts_Proxy.IsValidIndex(InContextNum))
	{
		return WarpBlendContexts_Proxy[InContextNum].bFrustumRotated;
	}
...
void FDisplayClusterProjectionMPCDIPolicy::ApplyWarpBlend_RenderThread(FRHICommandListImmediate& RHICmdList, const IDisplayClusterViewportProxy* InViewportProxy)
{
	...
	// ICVFX use custom rendering logic.
	const bool bRenderICVFX = EnumHasAllFlags(SettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::Target)
							&& InViewportProxy->GetConfigurationProxy().IsICVFXEnabled_RenderThread();
	if (bRenderICVFX)
	{
		...
		for (int32 ContextNum = 0; ContextNum < InputTextures.Num(); ContextNum++)
		{
			...
			if (!GetShadersAPI().RenderWarpBlend_ICVFX(RHICmdList, WarpBlendParameters, ShaderICVFX))
			{
				...
				{
					UE_LOGF(LogDisplayClusterProjectionMPCDI, Warning, "Couldn't apply icvfx warp&blend");
				}
	...
	for (int32 ContextNum = 0; ContextNum < InputTextures.Num(); ContextNum++)
	{
		...
		if (!GetShadersAPI().RenderWarpBlend_MPCDI(RHICmdList, WarpBlendParameters))
		{
			if (!IsEditorOperationMode_RenderThread(InViewportProxy))
			{
				UE_LOGF(LogDisplayClusterProjectionMPCDI, Warning, "Couldn't apply mpcdi warp&blend");
			}
	...
	if (CfgData.IsValid())
	{
		...
		if (CfgData.PFMFile.IsEmpty())
		{
			...
			if (CfgData.MPCDIFileName.IsEmpty())
			{
				if (!IsEditorOperationMode(InViewport))
				{
					UE_LOGF(LogDisplayClusterProjectionMPCDI, Warning, "File not found: Empty");
				}
			...
			{
				UE_LOGF(LogDisplayClusterProjectionMPCDI, Warning, "File not found: %ls", *CfgData.MPCDIFileName);
				return false;
			}
			...
			if (!WarpBlendInterface.IsValid())
			{
				UE_LOGF(LogDisplayClusterProjectionMPCDI, Warning, "Couldn't load MPCDI file: %ls", *CfgData.MPCDIFileName);
				return false;
			}

			UE_LOGF(LogDisplayClusterProjectionMPCDI, Verbose, "MPCDI policy has been initialized [%ls:%ls in %ls]", *CfgData.BufferId, *CfgData.RegionId, *CfgData.MPCDIFileName);
			return true;
		}
		...
		if (!WarpBlendInterface.IsValid())
		{
			UE_LOGF(LogDisplayClusterProjectionMPCDI, Error, "Failed to load PFM from file: %ls", *CfgData.PFMFile);
			return false;
		}

		UE_LOGF(LogDisplayClusterProjectionMPCDI, Verbose, "MPCDI policy has been initialized from PFM '%ls", *CfgData.PFMFile);
		return true;
	}
