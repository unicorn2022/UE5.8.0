...
TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe> FDisplayClusterProjectionMPCDIPolicyFactory::Create(const FString& ProjectionPolicyId, const FDisplayClusterConfigurationProjection* InConfigurationProjectionPolicy)
{
	if(!InConfigurationProjectionPolicy)
	{
		...
		{
			UE_LOGF(LogDisplayClusterProjectionMPCDI, Error, "MPCDI policy '%ls': null configuration", *ProjectionPolicyId);
		}
		return nullptr;
	}

	UE_LOGF(LogDisplayClusterProjectionMPCDI, Verbose, "Instantiating projection policy <%ls>...id='%ls'", *InConfigurationProjectionPolicy->Type, *ProjectionPolicyId);
