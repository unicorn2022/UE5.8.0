...
class FDisplayClusterProjectionVIOSOPolicy
	...
	, public TSharedFromThis<FDisplayClusterProjectionVIOSOPolicy, ESPMode::ThreadSafe>
{
public:
...
protected:
	...
	// View data owned and updated on the game thread
	TArray<TSharedRef<FDisplayClusterProjectionVIOSOPolicyViewData, ESPMode::ThreadSafe>> PolicyViewData;

	// Snapshot of PolicyViewData pushed to the render thread each frame via UpdateProxyData()
	TArray<TSharedRef<FDisplayClusterProjectionVIOSOPolicyViewData, ESPMode::ThreadSafe>> PolicyViewDataProxy;
