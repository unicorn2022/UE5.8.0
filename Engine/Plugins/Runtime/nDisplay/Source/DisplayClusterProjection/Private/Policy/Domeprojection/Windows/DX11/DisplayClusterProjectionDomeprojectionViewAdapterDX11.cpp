...
bool FDisplayClusterProjectionDomeprojectionViewAdapterDX11::ImplApplyWarpBlend_RenderThread(FRHICommandListImmediate& RHICmdList, uint32 InContextNum, const uint32 Channel, FRHITexture* InputTextures, FRHITexture* OutputTextures)
{
	...
	if (D3D11Device && DeviceContext)
	{
		...
		// perform warp/blend
		{
			...
			if (Result != dpNoError)
			{
				UE_LOGF(LogDisplayClusterProjectionDomeprojection, Error, "Domeprojection couldn't perform rendering operation");
				return false;
			}
	...
	UE_LOGF(LogDisplayClusterProjectionDomeprojection, Error, "UE couldn't perform domeprojection rendering on current render device");
	return false;
}
...
bool FDisplayClusterProjectionDomeprojectionViewAdapterDX11::FViewData::Initialize(IDisplayClusterViewport* InViewport, const FString& InFile, FCriticalSection& InDllAccessCS)
{
	...
	if (!DisplayClusterProjectionDomeprojectionLibraryDX11::Initialize())
	{
		...
		{
			UE_LOGF(LogDisplayClusterProjectionDomeprojection, Error, "Couldn't link to the Domeprojection DLL");
		}
	...
	if (InFile.IsEmpty())
	{
		if (!FDisplayClusterProjectionPolicyBase::IsEditorOperationMode(InViewport))
		{
			UE_LOGF(LogDisplayClusterProjectionDomeprojection, Error, "File name is empty");
		}
	...
	if (Result != dpNoError)
	{
		UE_LOGF(LogDisplayClusterProjectionDomeprojection, Error, "Couldn't initialize Domeprojection context");
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterProjectionDomeprojection, Error, "File '%ls' not found", *InFile);
		return false;
	}
	...
	if (Result != dpNoError)
	{
		UE_LOGF(LogDisplayClusterProjectionDomeprojection, Error, "Loading configuration from %ls failed with %d", *InFile, Result);
		return false;
	}
