...
public:
	...
	FDisplayClusterProjectionVIOSOPolicyViewData(const FDisplayClusterProjectionVIOSOPolicyViewData& In)
		: ViewLocation(In.ViewLocation)
		, ViewRotation(In.ViewRotation)
		, ProjectionMatrix(In.ProjectionMatrix)
#if WITH_VIOSO_LIBRARY
		, WarperInterface(In.WarperInterface)
#endif
		, RenderDevice(In.RenderDevice)
		, bInitialized(In.bInitialized)
		, UsedRenderTargetTexture(In.UsedRenderTargetTexture)
		, ClippingPlanes(In.ClippingPlanes)
	{ }
