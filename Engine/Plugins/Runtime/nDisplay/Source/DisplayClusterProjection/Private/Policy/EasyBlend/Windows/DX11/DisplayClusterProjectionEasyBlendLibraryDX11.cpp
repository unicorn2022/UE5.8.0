...
#define ImportWarpBlendFunc(DLLFunc)\
	...
	if(EasyBlend1##DLLFunc==nullptr)\
		{\
			UE_LOGF(LogDisplayClusterProjectionEasyBlend, Error, "Couldn't initialize EasyBlend DX11 API func '%ls'.", TEXT("EasyBlend"#DLLFunc));\
			return false;\
		}
...
FDisplayClusterProjectionEasyBlendLibraryDX11::FDisplayClusterProjectionEasyBlendLibraryDX11()
{
	...
	if (EasyBlend_DLL_Handler)
	{
		if (InitializeDLL())
		{
			UE_LOGF(LogDisplayClusterProjectionEasyBlend, Log, "EasyBlend DX11 API was initialized from file '%ls'.", *DllPath);

			return;
		}

		UE_LOGF(LogDisplayClusterProjectionEasyBlend, Error, "EasyBlend DX11 API could not be initialized. Maybe wrong DLL-file '%ls'.", *DllPath);
	}
	else
	{
		UE_LOGF(LogDisplayClusterProjectionEasyBlend, Error, "Could not find EasyBlend DX11 API '%ls' file.", *DllPath);
	}
...
FDisplayClusterProjectionEasyBlendLibraryDX11::~FDisplayClusterProjectionEasyBlendLibraryDX11()
{
	UE_LOGF(LogDisplayClusterProjectionEasyBlend, Log, "EasyBlend DX11 API released.");
	ReleaseDLL();
}
