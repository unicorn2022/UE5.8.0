...
#endif
{
	...
	{
		UE_LOGF(LogDisplayClusterProjectionVIOSO, Error, "VIOSO warp projection not supported by '%ls' rhi", GDynamicRHI->GetName());
	}
}
...
bool FDisplayClusterProjectionVIOSOPolicyViewData::InitializeVIOSO_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture* RenderTargetTexture)
{
	...
	case ERenderDevice::D3D11:
	{
		...
		{
			if (ID3D11Device* D3D11Device = D3D11DynamicRHI->RHIGetDevice())
			{
				FD3D11ContextHelper D3D11ContextHelper;
	...
	default:
		UE_LOGF(LogDisplayClusterProjectionVIOSO, Error, "Unsupported render device for VIOSO");
		break;
	}

	UE_LOGF(LogDisplayClusterProjectionVIOSO, Error, "Couldn't initialize VIOSO internals");
#endif 
