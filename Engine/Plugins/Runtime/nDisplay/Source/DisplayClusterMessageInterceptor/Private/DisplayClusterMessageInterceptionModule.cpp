...
private:
	...
	void SetupForMultiUser()
	{
		...
		{
			UE_LOGF(LogDisplayClusterInterception, Display, "No multi-user detected. Not intercepting initial activity sync.");
		}
#else
		UE_LOGF(LogDisplayClusterInterception, Display, "No multi-user available.");
#endif
	}
	void OnNewSceneEvent()
	{
		...
		UE_LOGF(LogDisplayClusterInterception, Display, "New scene event");
	}
	...
	void PackageSyncEvent(int64 ActivityId)
	{
		...
		UE_LOGF(LogDisplayClusterInterception, Display, "Sending package sync event.");
		IDisplayClusterClusterManager* ClusterManager = IDisplayCluster::Get().GetClusterMgr();
	...
	void WorkspaceSyncEvent()
	{
		...
		UE_LOGF(LogDisplayClusterInterception, Display, "Sending activity sync event.");
		IDisplayClusterClusterManager* ClusterManager = IDisplayCluster::Get().GetClusterMgr();
		check(ClusterManager != nullptr);
	...
	void OnSessionConnectionChanged(IConcertClientSession& InSession, EConcertConnectionStatus ConnectionStatus)
	{
		...
		else if (ConnectionStatus == EConcertConnectionStatus::Disconnected)
		{
			...
			UE_LOGF(LogDisplayClusterInterception, Display, "Disconnecting from session.");
			Workspace->RemoveWorkspaceFinalizeDelegate(TEXT("nDisplay Interceptor"));
	...
	void HandleClusterPreTick()
	{
		...
		if (bStartInterceptionRequested)
		{
			...
			if (Interceptor && ClusterManager)
			{
				...
				{
					UE_LOGF(LogDisplayClusterInterception, Display, "Sync received! Starting interception!");
	...
	{
		UE_LOGF(LogDisplayClusterInterception, Log, "Handling '%ls' node failure...", *NodeId);
	...
	{
		UE_LOGF(LogDisplayClusterInterception, Display, "Handle multi-user package event sync with id %ls -> %ls.", *InEvent.Type, *InEvent.Name);
		...
		{
			UE_LOGF(LogDisplayClusterInterception, Display, "All nodes have received package event %ls.", *InEvent.Type);
			PackageActivities.Remove(InEvent.Type);
			if (PackageActivities.Num() == 0)
			{
				UE_LOGF(LogDisplayClusterInterception, Display, "Releasing the package lock.");
				bAllowPackages = true;
			}
		}
	}
	...
	void HandlePackageNodeFailure(const FString& NodeId)
	{
		...
		for (TPair<FString, TSet<FString>>& PackageActivity : PackageActivities)
		{
			if (PackageActivity.Value.Remove(NodeId) > 0)
			{
				...
				{
					UE_LOGF(LogDisplayClusterInterception, Display, "All nodes have received package event %ls.", *PackageActivity.Key);
	...
	{
		UE_LOGF(LogDisplayClusterInterception, Display, "Handle multi-user workspace sync -> %ls.", *InEvent.Name);
		...
		{
			UE_LOGF(LogDisplayClusterInterception, Display, "Allowing multi-user workspace sync on next pre-tick.");
			CanFinalizeWorkspace = true;
		}
	}
	...
	void HandleMessageInterceptorSetupEvent(const FDisplayClusterClusterEventJson& InEvent)
	{
		if (Interceptor)
		{
			...
			if (ClusterManager)
			{
				...
				UE_LOGF(LogDisplayClusterInterception, Display, "Node '%ls' received synchronization settings event", *ClusterManager->GetNodeId());
			}
		}
	}
...
private:
	...
	{
		UE_LOGF(LogDisplayClusterInterception, Display, "Temporarily disabling multi-user workspace sync.");
