...
void FDisplayClusterMessageInterceptor::Start(TSharedPtr<IMessageBus, ESPMode::ThreadSafe> InBus)
{
	...
	{
		UE_LOGF(LogDisplayClusterInterception, Display, "Starting interception of bus messages");
		...
		for (const FInterceptedMessageDescriptor& Descriptor : InterceptedMessages)
		{
			...
			for (const FTopLevelAssetPath& MessageType : Descriptor.MessageTypes)
			{
				...
				UE_LOGF(LogDisplayClusterInterception, Display, "Intercepting message type: '%ls' with annotation '%ls'", *MessageType.ToString(), *AnnotationString);
			}
...
void FDisplayClusterMessageInterceptor::Stop()
{
	if (bIsIntercepting && InterceptedBus)
	{
		...
		bIsIntercepting = false;
		Purge();
		UE_LOGF(LogDisplayClusterInterception, Display, "Stopping interception of bus messages.");
	}
...
void FDisplayClusterMessageInterceptor::SyncMessages()
{
	if (ClusterManager)
	{
		...
		for (const FString& MessageId : MessageIds)
		{
			...
			UE_LOGF(LogDisplayClusterInterception, VeryVerbose, "Emitting cluster event for message %ls on frame %lld", *MessageId, GFrameCounter);
		}
	}
	...
	TArray<TSharedPtr<IMessageContext, ESPMode::ThreadSafe>> ContextToForward;
	{
		...
		for (auto It = ContextMap.CreateIterator(); It; ++It)
		{
			if (It.Value().ContextPtr->GetTimeForwarded() + MessageTimeoutSpan <= UtcNow)
			{
				...
				{
					UE_LOGF(LogDisplayClusterInterception, VeryVerbose, "Discarding unreliable message '%ls' left intercepted for more than %0.1f seconds", *It.Key(), InterceptionSettings.TimeoutSeconds);
					It.RemoveCurrent();
				}
				else
				{
					UE_LOGF(LogDisplayClusterInterception, Warning, "Forcing dispatching of reliable message '%ls' left intercepted for more than %0.1f seconds", *It.Key(), InterceptionSettings.TimeoutSeconds);
...
void FDisplayClusterMessageInterceptor::HandleClusterEvent(const FDisplayClusterClusterEventJson& InEvent)
{
	...
	if (InEvent.Category == TEXT("nDCI") && ClusterManager)
	{
		...
		if (FContextSync* ContextSync = ContextMap.Find(InEvent.Type))
		{
			...
			{
				UE_LOGF(LogDisplayClusterInterception, VeryVerbose, "Fowarding message for message id %ls on frame %lld", *InEvent.Type, GFrameCounter);
...
void FDisplayClusterMessageInterceptor::HandleClusterNodeFailure(const FString& NodeId)
{
...

	{
		...
		for (TPair<FString, FContextSync>& MessageContext : ContextMap)
		{
			...
			{
				UE_LOGF(LogDisplayClusterInterception, VeryVerbose, "Fowarding message for message id %ls on frame %lld", *MessageContext.Key, GFrameCounter);
...
bool FDisplayClusterMessageInterceptor::InterceptMessage(const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	if (Context->GetForwarder() != Address)
	{
		...
		if (Descriptor)
		{
			...
			if (MessageId)
			{
				UE_LOGF(LogDisplayClusterInterception, VeryVerbose, "Intercepted message '%ls'", **MessageId);
