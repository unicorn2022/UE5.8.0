...
{
	UE_LOGF(LogDisplayClusterEditorEngine, VeryVerbose, "UDisplayClusterEditorEngine::Init");
	...
	{
		UE_LOGF(LogDisplayClusterEditorEngine, Error, "Couldn't initialize DisplayCluster module");
		return;
	}
	...
	if (!bResult)
	{
		UE_LOGF(LogDisplayClusterEditorEngine, Error, "An error occured during DisplayCluster initialization");
		return;
	}

	UE_LOGF(LogDisplayClusterEditorEngine, Log, "DisplayCluster module has been initialized");
...
{
	UE_LOGF(LogDisplayClusterEditorEngine, VeryVerbose, "UDisplayClusterEditorEngine::PreExit");
	...
	if (InWorld && InWorld->PersistentLevel)
	{
		for (AActor* const Actor : InWorld->PersistentLevel->Actors)
		{
			if (IsValid(Actor))
			{
				...
				if (RootActor)
				{
					UE_LOGF(LogDisplayClusterEditorEngine, Log, "Found root actor - %ls", *RootActor->GetName());
					return RootActor;
				}
			}
		}
...
{
	UE_LOGF(LogDisplayClusterEditorEngine, VeryVerbose, "UDisplayClusterEditorEngine::StartPlayInEditorSession");
		...
		if (RootActor)
		{
			...
			if (ConfigData)
			{
				...
				else
				{
					...
					UE_LOGF(LogDisplayClusterEditorEngine, Error, "An error occurred during DisplayCluster session start");
				}
			}
			else
			{
				UE_LOGF(LogDisplayClusterEditorEngine, Error, "Couldn't load config data");
			}
		}
	}
...
{
	UE_LOGF(LogDisplayClusterEditorEngine, VeryVerbose, "UDisplayClusterEditorEngine::OnBeginPIE");
...
{
	UE_LOGF(LogDisplayClusterEditorEngine, VeryVerbose, "UDisplayClusterEditorEngine::OnEndPIE");
