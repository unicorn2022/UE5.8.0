...
void FDisplayClusterLightCardEditorViewportClient::Tick(float DeltaSeconds)
{
	...
	{
		EditorWidget->Tick(this, DeltaSeconds);
	}
}
...
void FDisplayClusterLightCardEditorViewportClient::Draw(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
		...
		for (const TWeakObjectPtr<UBillboardComponent>& BillboardComponent : BillboardComponentProxies)
		{
			...
			if (const FTexture* TextureResource = SpriteProxy.Sprite ? SpriteProxy.Sprite->GetResource() : nullptr)
			{
				...
				if (Settings->bDisplayIcons)
				{
					...
					if (SpriteProxy.bIsScreenSizeScaled && (View->ViewMatrices.GetViewToClip().M[3][3] != 1.0f))
					{
	...
	if (LastSelectedActor.IsValid() && LastSelectedActor->IsUVActor() == bIsUVProjection)
	{
		...
		EditorWidget->SetProjectionTransform(FDisplayClusterMeshProjectionTransform(ProjectionMode, View->ViewMatrices.GetWorldToView()));
	...
	{
		const FDisplayClusterMeshProjectionTransform ProjectionTransform(ProjectionMode, View->ViewMatrices.GetWorldToView());
		...
		auto DrawLine = [&](const FIntPoint& MousePosA, const FIntPoint& MousePosB, const FLinearColor& LineColor)
		{
			...
			FVector4 ViewPosA = View->ViewMatrices.GetClipToView().TransformFVector4(ScreenPosA);
			FVector4 ViewPosB = View->ViewMatrices.GetClipToView().TransformFVector4(ScreenPosB);

			constexpr float NearPlaneSafetyFactor = 1.01f;
			...
			const FVector4 WorldPosA = View->ViewMatrices.GetViewToWorld().TransformFVector4(ViewPosA);
			const FVector4 WorldPosB = View->ViewMatrices.GetViewToWorld().TransformFVector4(ViewPosB);
...
void FDisplayClusterLightCardEditorViewportClient::TrackingStopped()
{
	if (InputMode == EInputMode::DraggingActor)
	{
		...
		{
			// Update pre-capture mouse pos to avoid it resetting back to the starting position
...
void FDisplayClusterLightCardEditorViewportClient::CreateDrawnLightCard(const TArray<FIntPoint>& MousePositions)
{
...

	{
		...
		{
			UE_LOGF(DisplayClusterLightCardEditorLog, Warning, "Could not determine a LightCardDirection for the given polygon");
			return;
		}
	}
	...
	{
		UE_LOGF(DisplayClusterLightCardEditorLog, Warning, "Could not create new light card because one or more of its dimensions was zero.");
		return;
	}
	...
	{
		UE_LOGF(DisplayClusterLightCardEditorLog, Warning, "Could not create new light card because LightCardEditorPtr was not valid.");
		return;
	}
	...
	if (!LightCard)
	{
		UE_LOGF(DisplayClusterLightCardEditorLog, Warning, "Could not create new light card because AddNewLightCard failed.");
		return;
	}
	...
	if (RootActorProxy.IsValid())
	{
		...
		for (const TPair<FString, TObjectPtr<UDisplayClusterConfigurationClusterNode>>& NodePair : Config->Cluster->Nodes)
		{
			...
			for (const TPair<FString, TObjectPtr<UDisplayClusterConfigurationViewport>>& ViewportPair : Node->Viewports)
			{
				...
				if (CfgViewport->ProjectionPolicy.Type.Equals(DisplayClusterProjectionStrings::projection::Mesh, ESearchCase::IgnoreCase)
...
FVector2D FDisplayClusterLightCardEditorViewportClient::GetActorScaleDelta(FViewport* InViewport, const FDisplayClusterWeakStageActorPtr& InActor, EAxisList::Type CurrentAxis)
{
	...
	const double ScreenSize = RenderViewportType == LVT_Perspective 
		...
		: SizeToScale.Length() * View->ViewMatrices.GetViewToClip().M[0][0] * View->UnscaledViewRect.Width();
...
{
	FDisplayClusterMeshProjectionTransform Transform(ProjectionMode, ViewMatrices.GetWorldToView());
