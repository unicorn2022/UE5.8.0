...
public:
	...
	/** Called from the game thread, allows the widget to perform any game thread specific actions */
	void Tick(const FDisplayClusterLightCardEditorViewportClient* ViewportClient, float InDeltaSeconds);
...
private:
	...
	/** Cached DPI scale used to scale the widget's rendered size */
	float DPIScale = 1.f;
};