...
bool FSharedMemoryMediaPlayer::Open(const FString& Url, const IMediaOptions* Options)
{
	...
	if (!PlatformData.IsValid())
	{
		...
		if (!PlatformData.IsValid())
		{
			UE_LOGF(LogSharedMemoryMedia, Error, "Unfortunately, SharedMemoryMedia doesn't support the current RHI type '%ls'",
	...
	// Grab unique name
	{
		...
		if (!UniqueName.Len())
		{
			UE_LOGF(LogSharedMemoryMedia, Error, "SharedMemoryMediaSource must have a UniqueName that is not empty, and should match the MediaOutput's UniqueName");
			Close();
			return true;
		}
	}
...
void FSharedMemoryMediaPlayer::TickFetch(FTimespan DeltaTime, FTimespan Timecode)
{
	...
	for (int32 MemIdx = 0; MemIdx < NUMSHAREDMEM; ++MemIdx)
	{
		if (!SharedMemory[MemIdx])
		{
			...
			if (SharedMemory[MemIdx])
			{
				UE_LOGF(LogSharedMemoryMedia, Verbose, "Opened SharedMemory named '%ls'", *SharedMemoryRegionName);
			...
			{
				UE_LOGF(LogSharedMemoryMedia, Warning, "Could not open SharedMemory named '%ls'", *SharedMemoryRegionName);
			}
		}
	...
	for (uint32 MemIdx = 0; MemIdx < NUMSHAREDMEM; MemIdx++)
	{
		...
		{
			UE_LOGF(LogSharedMemoryMedia, Warning, "Invalid zero guid");
			continue;
		}
...
bool FSharedMemoryMediaPlayer::DetermineNextSourceFrameGenlockMode(uint64 FrameNumber, uint32& OutExpectedFrameNumber, uint32& OutSharedMemoryIdx)
{
	...
	{
		UE_LOGF(LogSharedMemoryMedia, Warning, "SharedMemoryMedia: In genlock mode, the sender's frame count seems to have reset to earlier than %u",
...
bool FSharedMemoryMediaPlayer::DetermineNextSourceFrameFreerunMode(uint64 FrameNumber, uint32& OutExpectedFrameNumber, uint32& OutSharedMemoryIdx)
{
	...
	else if (State.LastSourceFrameNumberPicked > HighestFrameNumber)
	{
		...
		UE_LOGF(LogSharedMemoryMedia, Warning, "SharedMemoryMedia: In freerun mode, the sender's frame count seems to have reset from %u to %u.",
...
void FSharedMemoryMediaPlayer::JustInTimeSampleRender(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		UE_LOGF(LogSharedMemoryMedia, Verbose, 
			"FSharedMemoryMediaPlayer '%ls' JustInTimeSampleRender called more than once in GFrameCounterRenderThread %llu",
	...
	UE_LOGF(LogSharedMemoryMedia, VeryVerbose, "FSharedMemoryMediaPlayer '%ls' JustInTimeSampleRender called for GFrameCounterRenderThread %llu", 
		*UniqueName, GFrameCounterRenderThread
	);
				...
				while (true)
				{
					...
					if (SenderMetadata.Magic == FSharedMemoryMediaFrameMetadata::MAGIC)
					{
						if (SenderMetadata.FrameNumber >= ExpectedFrameNumber)
						{
							if (SenderMetadata.FrameNumber > ExpectedFrameNumber)
							{
								UE_LOGF(LogSharedMemoryMedia, Warning, "Using too recent frame. Expected %u and used %u", 
					...
					{
						UE_LOGF(LogSharedMemoryMedia, Warning, "FSharedMemoryMediaPlayer timed out waiting for ExpectedFrameNumber %u for frame %llu, only saw up to frame %u",
	...
	{
		UE_LOGF(LogSharedMemoryMedia, Warning, "bFrameAckFenceBusy[%d] was busy for ExpectedFrameNumber %d",
	...
	{
		UE_LOGF(LogSharedMemoryMedia, Error, "SharedCrossGpuTextures[%d] was unexpectedly invalid", SharedMemoryIdx);
	}
