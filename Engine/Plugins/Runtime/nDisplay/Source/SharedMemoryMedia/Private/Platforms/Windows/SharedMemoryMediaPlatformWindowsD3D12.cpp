...
FTextureRHIRef FSharedMemoryMediaPlatformWindowsD3D12::CreateSharedTexture(EPixelFormat Format, bool bSrgb, int32 Width, int32 Height, const FGuid& Guid, uint32 BufferIdx, bool bCrossGpu)
{
	...
	ID3D12Device* D3D12Device = GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU();
	check(D3D12Device);
	...
	if (FAILED(HResult))
	{
		UE_LOGF(LogSharedMemoryMedia, Error, "D3D12Device->CreateCommittedResource failed when creating a cross GPU texture:\n"
			"0x%lX - %ls. Texture was %dx%d, EPixelFormat %d and DXGI type %d. Use -d3ddebug for more information. ",
...
FTextureRHIRef FSharedMemoryMediaPlatformWindowsD3D12::OpenSharedTextureByGuid(const FGuid& Guid, FSharedMemoryMediaTextureDescription& OutTextureDescription)
{
	...
	if (FAILED(HResult))
	{
		UE_LOGF(LogSharedMemoryMedia, Error, "D3D12Device->OpenSharedHandleByName(%ls) failed: 0x%lX - %ls",
	...
	UE_CLOGF(FAILED(HResult), LogSharedMemoryMedia, Error, "D3D12Device->OpenSharedHandle(0x%p) failed: 0x%lX - %ls",
	...
	// Query texture dimensions and use them to describe downstream resources
	{
		...
		if (OutTextureDescription.Format == EPixelFormat::PF_Unknown)
		{
			UE_LOGF(LogSharedMemoryMedia, Error, "Could not find a known pixel format for SharedCrossGpuTexture DXGI_FORMAT %d.",
