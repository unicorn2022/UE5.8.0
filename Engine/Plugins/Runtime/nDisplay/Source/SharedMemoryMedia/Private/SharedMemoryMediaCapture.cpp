...
bool USharedMemoryMediaCapture::InitializeCapture()
{
	...
	{
		UE_LOGF(LogSharedMemoryMedia, Error, "Invalid MediaOutput for '%ls', cannot InitializeCapture", *GetName());
		return false;
	}
	...
	for (int32 BufferIdx = 0; BufferIdx < NUMBUFFERS; ++BufferIdx)
	{
		...
		if (!SharedMemoryRegion)
		{
			...
			if (SharedMemoryRegion)
			{
				...
				UE_LOGF(LogSharedMemoryMedia, Verbose, "Created SharedMemoryRegion[%d] = %ls for UniqueName '%ls'", 
	...
	FVector2D CropV)
{
	...
	for (uint32 Idx = 0; Idx < NUMBUFFERS; Idx++)
	{
		if (!SharedCrossGpuTextures[Idx].IsValid())
		{
			...
			if (!SharedCrossGpuTextures[Idx].IsValid())
			{
				UE_LOGF(LogSharedMemoryMedia, Error, "Unable to create cross GPU texture of the requested type for Unique Name '%ls'", 
			...
			UE_LOGF(LogSharedMemoryMedia, Verbose, "Created SharedGpuTextureGuid[%d] = %ls for UniqueName '%ls'", 
		...
		{
			UE_LOGF(LogSharedMemoryMedia, Warning,
				"Cross GPU texture is out of date for Unique Name '%ls'. It's size was (%d,%d) but OutputTexture was (%d,%d)",
	...
	// Sanity check the copy regions are contained inside the input and output texture sizes.
	{
		...
		{
			UE_LOGF(LogSharedMemoryMedia, Error, 
				"Invalid source CopyInfo passed SharedMemoryMedia with Unique Name '%ls'. (%d,%d,%d,%d) not contained in (%d,%d)",
	...
	}
	{
		...
		{
			UE_LOGF(LogSharedMemoryMedia, Error, 
				"Invalid destination CopyInfo passed SharedMemoryMedia with Unique Name '%ls'. [(%d,%d),(%d,%d)] not contained in (%d,%d)",
				*SharedMemoryMediaOutput->UniqueName, Rect.Min.X, Rect.Min.Y, Rect.Max.X, Rect.Max.Y, Size.X, Size.Y
			);
	...
	GraphBuilder.AddPass(
		...
		[InSourceTexture, SharedTextureIdx, CopyInfo, this](FRHICommandList& RHICmdList)
		{
			...
			if (bTextureReadyFenceBusy[SharedTextureIdx])
			{
				...
				UE_LOGF(LogSharedMemoryMedia, Verbose, "bTextureReadyFenceBusy[%d] for frame %lld was busy, so we wait",
			...
			UE::Tasks::Launch(UE_SOURCE_LOCATION, [FrameNumber = GFrameCounterRenderThread, SharedTextureIdx, this]()
			{
				...
				const FString CopyThreadName = FString::Printf(TEXT("SharedMemMediaOutputGpuTextureInTransitForFrame_%lld"), FrameNumber % 100);
				...
				// all active receivers have acked a frame number equal or greater than FrameNumber.
				{
					...
					while (!SharedMemoryData->AllReceiversAckedFrameNumber(FrameNumber))
					{
						...
						{
							UE_LOGF(LogSharedMemoryMedia, Warning, 
								"FSharedMemoryMediaCapture timed out waiting for its receiver to ack frame %lld", FrameNumber
							);
