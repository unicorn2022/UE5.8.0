...
public:
	...
	*/
	...
	/** Get viewport proxy object. */
	virtual TSharedRef<class IDisplayClusterViewportProxy, ESPMode::ThreadSafe> GetViewportProxy() const = 0;
	...
	*/
	...
	/** Get viewport contexts captured during the previous frame. */
	virtual const TArray<FDisplayClusterViewport_Context>& GetPrevFrameContexts() const = 0;

	/** Get the frame data snapshot captured for the current frame. */
	virtual const FDisplayClusterViewportFrameData& GetFrameData() const = 0;

	/** Get the frame data snapshot captured during the previous frame. */
	virtual const FDisplayClusterViewportFrameData& GetPrevFrameData() const = 0;

	/**
	 * Returns the ViewState for the specified viewport context, creating it if necessary.
	 *
	 * @param InContextNum Eye context index for this viewport.
	 *
	 * @return Shared pointer to the corresponding ViewState, or nullptr if ViewState usage is not allowed for this viewport.
	 */
	virtual TSharedPtr<FSceneViewStateReference, ESPMode::ThreadSafe> GetOrCreateViewState(uint32 InContextNum) = 0;
	...
	*
	* @param InWorld                    - (in)       In this world, an active game camera will be searched for.
	* @param bUseCameraPostprocess      - (in)       if false, then InOutViewInfo.PostProcessBlendWeight is set to 0.
	* @param InOutViewInfo              - (in,out)   ViewPoint data
	* @param OutCustomNearClippingPlane - (opt, out) Custom NCP, or a value less than zero if not defined.
	*
	* @return false if the viewpoint data cannot be retrieved.
	*/
	static bool GetPlayerCameraView(UWorld* InWorld, const bool bUseCameraPostprocess, FMinimalViewInfo& InOutViewInfo, float* OutCustomNearClippingPlane = nullptr);
