...
public:
	...
	*/
	...
	/**
	 * Returns the camera component driving this projection policy, or nullptr if the policy does not use one.
	 */
	...
	*/
	...
	/** Returns true if the frustum has been rotated to fit the context size.
	*
	* @param InViewportProxy - The owning viewport proxy
	* @param InContextNum - Index of view that is being processed for this viewport
	*
	* @return True if succeeded.
	*/
	virtual bool IsFrustumRotatedToFitContextSize_RenderThread(const IDisplayClusterViewportProxy* InViewportProxy, const uint32 InContextNum)
	{
		return false;
	}
