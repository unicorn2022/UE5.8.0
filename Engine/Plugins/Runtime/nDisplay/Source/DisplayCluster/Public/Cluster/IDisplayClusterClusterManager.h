...
public:
	...
	/**
	 * Registers custom state for auto-replication
	 * 
	 * @param UniqueStateName - Unique name to distinguish different states
	 * @param StateVariable   - A variable that references the state
	 * 
	 * @return True if registered successfully, otherwise false (has been registered already)
	 */
	virtual bool RegisterCustomState(const FName& UniqueStateName, TSharedPtr<IDisplayClusterCustomState>& StateVariable) = 0;

	/**
	 * Unregisters specific custom state
	 *
	 * @param UniqueStateName - Unique name to distinguish different states
	 *
	 * @return True if unregistered successfully, otherwise false (not found)
	 */
	virtual bool UnregisterCustomState(const FName& UniqueStateName) = 0;

	/**
	 * Returns the names of all registered states
	 *
	 * @return Set of custom state names currently registered
	 */
	virtual TSet<FName> GetRegisteredCustomStateNames() const = 0;

	/**
	 * Checks whether a custom state with the specified name has already been registered
	 *
	 * @return True if such state exists
	 */
	virtual bool IsCustomStateRegistered(const FName& UniqueStateName) const = 0;
