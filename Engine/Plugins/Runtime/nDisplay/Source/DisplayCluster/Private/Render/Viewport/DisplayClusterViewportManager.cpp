...
FDisplayClusterViewportManager::FDisplayClusterViewportManager()
	...
	, AutoExposure(MakeShared<FDisplayClusterAutoExposure>(Configuration))
{
...
void FDisplayClusterViewportManager::Release()
{
	...
	// Remove exclusive locks on release.
	Configuration->SetExclusiveLockOnRootActors(false);
...
FDisplayClusterViewportManager::~FDisplayClusterViewportManager()
{
	if (!bReleaseFunctionCalledBeforeDestructor)
	{
		...
		UE_LOGF(LogDisplayClusterViewport, Error,
			"~FDisplayClusterViewportManager() is called at an inappropriate time. The Release() function must be called before this.");
	}
...
void FDisplayClusterViewportManager::ReleaseAllResources()
{
	for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ViewportIt : ImplGetEntireClusterViewports())
	{
		...
		{
			ViewportIt->ReleaseAllResources();
		}
	}
	...
	{
		// Remove viewport manager proxy on render_thread
		ENQUEUE_RENDER_COMMAND(DisplayClusterViewportManagerProxy_ReleaseAllResources)(
			[ViewportManagerProxy = ViewportManagerProxy](FRHICommandListImmediate& RHICmdList)
			{
				ViewportManagerProxy->ReleaseAllResources_RenderThread();
			});
	}
	...
	// Release AutoExposure inner data
...
void FDisplayClusterViewportManager::HandleStartScene()
{
	...
	UE_LOGF(LogDisplayClusterViewport, Log, "DCRA '%ls': Handling StartScene().", *Configuration->GetRootActorName());
...
void FDisplayClusterViewportManager::HandleEndScene()
{
	...
	UE_LOGF(LogDisplayClusterViewport, Log, "DCRA '%ls': Handling EndScene().", *Configuration->GetRootActorName());
...
int32 FDisplayClusterViewportManager::GetViewportStereoViewIndexBase(const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& InViewport) const
{
	...
	if (InViewport.IsValid() && ViewPerViewportAmount > 0)
	{
		...
		{
			// Calculate the unique StereoViewIndex base for the viewport.
			const int32 ViewportStereoViewIndexBase =
				// Each StereoViewIndexBase has a distance to others equal to ViewPerViewportAmount.
				(ViewportIndex * ViewPerViewportAmount)
				//  Shifts to the nDisplay range; engine indices are below this.
				+ FDisplayClusterViewportStereoscopicPass::StereoViewIndexBase;

			return ViewportStereoViewIndexBase;
		}
	}
...
bool FDisplayClusterViewportManager::BeginNewFrame(FViewport* InViewport, FDisplayClusterRenderFrame& OutRenderFrame)
{
	...
	for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ViewportIt : ImplGetCurrentRenderFrameViewports())
	{
		...
		const int32 ViewportStereoViewIndexBase = GetViewportStereoViewIndexBase(ViewportIt);
		if (ViewportStereoViewIndexBase != INDEX_NONE)
		{
			...
			ViewportIt->UpdateFrameContexts(ViewportStereoViewIndexBase);
...
void FDisplayClusterViewportManager::InitializeNewFrame()
{
	...
	// Update AutoExposure data.
	AutoExposure->SyncAndComputeAutoExposure();
}
...
void FDisplayClusterViewportManager::OnEntireClusterPreviewRendered()
{
	// Forward the "entire cluster preview rendered" event so AutoExposure can refresh its data.
	AutoExposure->OnEntireClusterPreviewRendered();
}
...
FDisplayClusterViewport* FDisplayClusterViewportManager::CreateViewport(const FString& InViewportBaseId, const class UDisplayClusterConfigurationViewport& ConfigurationViewport)
{
	check(IsInGameThread());
	...
	{
		UE_LOGF(LogDisplayClusterViewport, Warning, "Wrong viewport ID");
		return nullptr;
	}

	// ID must be unique
	if (FindViewport(InViewportBaseId) != nullptr)
	{
		UE_LOGF(LogDisplayClusterViewport, Warning, "Viewport '%ls' already exists", *InViewportBaseId);

		return nullptr;
	}

	// Create projection policy for viewport
	const TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe> NewProjectionPolicy = CreateProjectionPolicy(InViewportBaseId, &ConfigurationViewport.ProjectionPolicy);
	if (NewProjectionPolicy.IsValid())
	{
		// Create viewport for new projection policy
		TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> NewViewport = ImplCreateViewport(InViewportBaseId, NewProjectionPolicy);
	...
	UE_LOGF(LogDisplayClusterViewport, Error, "Viewports '%ls' not created.", *InViewportBaseId);
...
TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> FDisplayClusterViewportManager::ImplFindViewport(
	...
	const FIntPoint* InOptTilePos) const
{
	...
	// Generate unique viewport name
	const FString ViewportId = Configuration->BuildViewportName(InViewportBaseId, InViewportFlags, InOptTilePos);

	// Ok, we have a request for a particular viewport. Let's find it.
	TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> const* DesiredViewport = ImplGetEntireClusterViewports().FindByPredicate(
		[ViewportId](const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ItemViewport)
	{
...
FDisplayClusterViewport* FDisplayClusterViewportManager::CreateViewport(const FString& InViewportBaseId, const TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe>& InProjectionPolicy)
{
	check(IsInGameThread());

	TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> ExistViewport = ImplFindViewport(InViewportBaseId);
	...
	TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> NewViewport = ImplCreateViewport(InViewportBaseId, InProjectionPolicy);
...
TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> FDisplayClusterViewportManager::ImplCreateViewport(
	const FString& InViewportBaseId,
	...
	const FIntPoint* InOptTilePos)
{
	...
	TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> NewViewport = MakeShared<FDisplayClusterViewport>(
		Configuration, InViewportBaseId, InViewportFlags, InProjectionPolicy, InOptTilePos);
	if (NewViewport.IsValid())
	{
...
FDisplayClusterViewport* FDisplayClusterViewportManager::ImplFindViewport(const int32 StereoViewIndex, uint32* OutContextNum) const
{
	check(IsInGameThread());
	...
	for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ViewportIt : ImplGetEntireClusterViewports())
	{
		if (ViewportIt.IsValid() && ViewportIt->FindContext(StereoViewIndex, OutContextNum))
		{
...
TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe> FDisplayClusterViewportManager::CreateProjectionPolicy(const FString& InViewportId, const FDisplayClusterConfigurationProjection* InConfigurationProjectionPolicy)
{
	...
	const FString ProjectionPolicyId = FString::Printf(TEXT("prj_%s"), *InViewportId);
	...
	if (ProjPolicyFactory.IsValid())
	{
		...
		{
			UE_LOGF(LogDisplayClusterViewport, Warning, "Invalid projection policy: type '%ls', RHI '%ls', viewport '%ls'", *InConfigurationProjectionPolicy->Type, GDynamicRHI->GetName(), *ProjectionPolicyId);
		}
	}
	else
	{
		UE_LOGF(LogDisplayClusterViewport, Warning, "No projection factory found for projection type '%ls'", *InConfigurationProjectionPolicy->Type);
	}
