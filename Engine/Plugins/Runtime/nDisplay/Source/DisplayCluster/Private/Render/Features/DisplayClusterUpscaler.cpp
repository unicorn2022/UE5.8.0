...
namespace UE::DisplayClusterRender::Features::Upscaler
{
	...
	static float GetSecondaryViewFraction(const float InDPIScale)
	{
		...
		if (CVarCustomSecondaryScreenPercentage)
		{
			...
			if (GIsEditor && !GIsHighResScreenshot && InDPIScale > 0.0)
			{
	...
	static bool CanUseMSAA(const FStaticFeatureLevel InFeatureLevel)
	{
		...
		{
			// MSAA requires forward shading renderer
			return false;
		}
...
{
	using namespace UE::DisplayClusterRender::Features::Upscaler;
	...
	const TArray<FSceneView*>& InOutViews)
{
	...
	// Validating that InOutViews is non-empty.
	if (InOutViews.IsEmpty() || InOutViews[0] == nullptr)
	{
		return NAME_None;
	}
