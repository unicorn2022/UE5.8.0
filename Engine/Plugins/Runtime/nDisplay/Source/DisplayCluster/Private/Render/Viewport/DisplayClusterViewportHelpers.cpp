	...
	if (InRect != OutRect)
	{
		if (LogMessageStatePtr == nullptr || *LogMessageStatePtr == false)
		{
			UE_LOGF(LogDisplayClusterViewport, Error,
				"The '%ls' %ls has been clamped %dx%d->%dx%d (max texture dimensions is %d)",
				*InViewportId,
...
{
	// Floor-scale each axis and clamp to at least 1x1.
	return FIntPoint(
		FMath::Max(1, FMath::FloorToInt(InSize.X * InMult)),
		FMath::Max(1, FMath::FloorToInt(InSize.Y * InMult)));
}

FIntRect FDisplayClusterViewportHelpers::ScaleViewportRect(const FIntRect& InRect, const FVector2D& InPerAxisMult)
{
	FIntRect OutRect(
		FIntPoint(FMath::FloorToInt(InRect.Min.X * InPerAxisMult.X),
				  FMath::FloorToInt(InRect.Min.Y * InPerAxisMult.Y)),
		FIntPoint(FMath::FloorToInt(InRect.Max.X * InPerAxisMult.X),
				  FMath::FloorToInt(InRect.Max.Y * InPerAxisMult.Y))
		);
	
	// Ensure the rect is at least 1x1 after scaling.
	OutRect.Max.X = FMath::Max(OutRect.Max.X, OutRect.Min.X + 1);
	OutRect.Max.Y = FMath::Max(OutRect.Max.Y, OutRect.Min.Y + 1);
