...
public:
	...
	*
	* Note: This function has been renamed; its previous name was 'ReleaseTextures'.
	*/
...
public:
	...
	{
		return ImplFindViewport(ViewIndex, OutContextNum);
	}
	...
	// internal use only
	FDisplayClusterViewport* CreateViewport(const FString& InViewportBaseId, const class UDisplayClusterConfigurationViewport& ConfigurationViewport);
	FDisplayClusterViewport* CreateViewport(const FString& InViewportBaseId, const TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe>& InProjectionPolicy);
	bool                     DeleteViewport(const FString& ViewportId);

	FDisplayClusterViewport* ImplFindViewport(const int32 ViewIndex, uint32* OutContextNum = nullptr) const;
	/**
	* Creates and initializes a new nDisplay viewport instance.
	*
	...
	*
	* @return A thread-safe shared pointer to the created FDisplayClusterViewport.
	*/
	TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> ImplCreateViewport(
	...
	* @param InOptTilePos     Optional parameter for tile viewport position
	*
	* @return A thread-safe shared pointer to the matching FDisplayClusterViewport,
	*         or nullptr if no viewport matches the given criteria.
	*/
	TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe> ImplFindViewport(
		const FString& InViewportBaseId,
		const EDisplayClusterViewportFlags InViewportFlags = EDisplayClusterViewportFlags::None,
	...
	static TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe> CreateProjectionPolicy(
	...
	/** 
	* Gets the unique StereoViewIndex base for the viewport.
	* Each StereoViewIndexBase has a distance to others equal to ViewPerViewportAmount.
	* 
	* Note: This function has been renamed. Its previous name was FindFirstViewportStereoViewIndex().
	*
	* @param InViewport - viewport asked for a StereoViewIndex base
	* @return StereoViewIndex base or INDEX_NONE, if the viewport cannot be rendered.
	*/
	int32 GetViewportStereoViewIndexBase(const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& InViewport) const;
...
public:
	...
	// nDisplay multi-viewport AutoExposure feature.
	const TSharedRef<class FDisplayClusterAutoExposure> AutoExposure;
