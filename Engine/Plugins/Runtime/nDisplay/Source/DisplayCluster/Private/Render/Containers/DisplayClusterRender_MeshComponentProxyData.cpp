	...
	: SourceGeometryName(InSourceGeometryName)
{
	if (InUVs[FDisplayClusterMeshUVs::ERemapTarget::Base] > 3)
	{
		...
		UE_LOGF(LogDisplayClusterRender, Warning, "MeshComponentProxyData('%ls'): Base UVIndex value '%d' is invalid - ProceduralMesh support range 0..3", *SourceGeometryName, InUVs[FDisplayClusterMeshUVs::ERemapTarget::Base]);
	}

	if (InUVs[FDisplayClusterMeshUVs::ERemapTarget::Chromakey] > 3)
	{
		// Show warning for values >3
		UE_LOGF(LogDisplayClusterRender, Warning, "MeshComponentProxyData('%ls'): Chromakey UVIndex value '%d' is invalid - ProceduralMesh support range 0..3", *SourceGeometryName, InUVs[FDisplayClusterMeshUVs::ERemapTarget::Chromakey]);
	}
...
void FDisplayClusterRender_MeshComponentProxyData::UpdateData(const EDisplayClusterRender_MeshComponentProxyDataFunc InDataFunc)
{
	...
	if (!IsValid())
	{
		UE_LOGF(LogDisplayClusterRender, Error, "MeshComponentProxyData::UpdateData('%ls') Invalid mesh - ignored", *SourceGeometryName);
	}
}
...
void FDisplayClusterRender_MeshComponentProxyData::ImplNormalizeToScreenSpace()
{
	...
	{
		UE_LOGF(LogDisplayClusterRender, Error, "MeshComponentProxyData::NormalizeToScreenSpace('%ls'): The Y axis is used in screen space as 'x' and the distance cannot be zero.", *SourceGeometryName);
		bIsValidMeshAxis = false;
	}

	if (FMath::Abs(Size.Z) <= KINDA_SMALL_NUMBER)
	{
		UE_LOGF(LogDisplayClusterRender, Error, "MeshComponentProxyData::NormalizeToScreenSpace('%ls'): The Z axis is used in screen space as 'y' and the distance cannot be zero.", *SourceGeometryName);
		bIsValidMeshAxis = false;
	}
	...
	if (bIsValidMeshAxis)
	{
		...
		if (AspectRatio > 10)
		{
			...
			UE_LOGF(LogDisplayClusterRender, Warning, "MeshComponentProxyData::NormalizeToScreenSpace('%ls'): Aspect ratio is to big '%f'- <%f,%f>", *SourceGeometryName, AspectRatio, ScreenSize.X, ScreenSize.Y);
		}
	}
	...
	if (!bIsValidMeshAxis)
	{
		...
		UE_LOGF(LogDisplayClusterRender, Error, "MeshComponentProxyData::NormalizeToScreenSpace('%ls') Invalid mesh. Expected mesh axis: Z=UP(screen Y), Y=RIGHT(screen X), X=NotUsed", *SourceGeometryName);
		return;
	}
...
void FDisplayClusterRender_MeshComponentProxyData::ImplRemoveInvisibleFaces()
{
	...
	if (RemovedFacesNum)
	{
		UE_LOGF(LogDisplayClusterRender, Error, "MeshComponentProxyData::RemoveInvisibleFaces('%ls') Removed %d/%d faces", *SourceGeometryName, RemovedFacesNum, FacesNum);
	}
}
