...
FDisplayClusterViewportManagerProxy::FDisplayClusterViewportManagerProxy(const FDisplayClusterViewportManager& InViewportManager)
	...
	, AutoExposure(InViewportManager.AutoExposure)
...
void FDisplayClusterViewportManagerProxy::ImplRenderFrame_GameThread(FViewport* InViewport)
{
		...
		[InViewportManagerProxy = SharedThis(this)](FRHICommandListImmediate& RHICmdList)
	{
		...
		// Collects AutoExposure (Eye Adaptation) data from the viewports
		ViewportManagerProxy->AutoExposure->GatherMeterViewportsAutoExposureDataAndSync_RenderThread(
			ViewportManagerProxy->ImplGetCurrentRenderFrameViewportProxies_RenderThread());
	});
...
bool FDisplayClusterViewportManagerProxy::ImplResolveFrameTargetToBackBuffer_RenderThread(FRDGBuilder& GraphBuilder, const uint32 InContextNum, const int32 DestArrayIndex, FRDGTextureRef DestTexture, FVector2f WindowSize) const
{
	...
	if (DestArrayIndex > 0)
	{
		...
		if (!bStereoTexture)
		{
			...
			{
				UE_LOGF(LogDisplayClusterRender, Error, "Backbuffer does not support stereo rendering.");
...
void FDisplayClusterViewportManagerProxy::ReleaseAllResources_RenderThread()
{
	for (const TSharedPtr<FDisplayClusterViewportProxy, ESPMode::ThreadSafe>& ViewportProxyIt : ImplGetEntireClusterViewportProxies_RenderThread())
	{
		if (ViewportProxyIt.IsValid())
		{
			ViewportProxyIt->ReleaseAllResources_RenderThread();
		}
	}
}
