...
void FDisplayClusterViewportConfiguration_ICVFX::Update()
{
	...
	for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& TargetIt : TargetViewports)
	{
		if (TargetIt.IsValid())
		{
			...
			// Clear enforced camera order each frame to avoid stale data
			InOutRenderSettingsICVFX.ICVFX.EnforcedCameraOrder.Empty();

			// Build enforced camera order by resolving camera names to viewport IDs
			if (const UDisplayClusterConfigurationViewport* ViewportCfg = TargetIt->GetViewportConfigurationData())
			{

				if (!ViewportCfg->ICVFX.EnforcedCameraOrder.IsEmpty())
				{
					// Build full viewport IDs from camera names using helper
					for (const FString& CameraName : ViewportCfg->ICVFX.EnforcedCameraOrder)
					{
						if (!CameraName.IsEmpty())
						{
							const FString InCameraViewportId = Configuration.BuildViewportName(CameraName, EDisplayClusterViewportFlags::ICVFX_InnerFrustum);
							InOutRenderSettingsICVFX.ICVFX.EnforcedCameraOrder.Add(InCameraViewportId);
						}
					}
				}
			}
...
void FDisplayClusterViewportConfiguration_ICVFX::ImplBeginReallocateViewports()
{
	if (FDisplayClusterViewportManager* ViewportManager = Configuration.GetViewportManagerImpl())
	{
		for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ViewportIt : ViewportManager->ImplGetCurrentRenderFrameViewports())
		{
			...
			if (ViewportIt.IsValid()
				// Exclude tile viewports: they handled in FDisplayClusterViewportConfiguration_Tile::Update()
				&& !EnumHasAnyFlags(ViewportIt->GetViewportFlags(), EDisplayClusterViewportFlags::TileViewport)
				// Process only ICVFX viewports
				&& EnumHasAnyFlags(ViewportIt->GetViewportFlags(), EDisplayClusterViewportFlags::ICVFX_AnyRole))
			{
				// Gain direct access to internal settings of the viewport:
				FDisplayClusterViewport_RenderSettingsICVFX& InOutRenderSettingsICVFX = ViewportIt->GetRenderSettingsICVFXImpl();

				// Mark this ICVFX viewport as unused. This flag is cleared later if the viewport is actually used.
				// Otherwise, this viewport will be removed in ImplFinishReallocateViewports().
...
{
	// Clear stage cameras unconditionally to prevent stale data
	StageCameras.Empty();
