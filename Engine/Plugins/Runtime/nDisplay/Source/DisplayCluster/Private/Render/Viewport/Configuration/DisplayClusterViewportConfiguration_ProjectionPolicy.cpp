...
bool FDisplayClusterViewportConfiguration_ProjectionPolicy::UpdateCameraPolicy(FDisplayClusterViewport& DstViewport)
{
	...
	if (CameraComponentId.IsEmpty())
	{
		...
		{
			UE_LOGF(LogDisplayClusterViewport, Verbose, "Viewport '%ls': referenced camera '' (empty name).", *DstViewport.GetId());
		}
	...
	{
		UE_LOGF(LogDisplayClusterViewport, Error, "Viewport '%ls': referenced camera '%ls' not found.", *DstViewport.GetId(), *CameraComponentId);
	}
