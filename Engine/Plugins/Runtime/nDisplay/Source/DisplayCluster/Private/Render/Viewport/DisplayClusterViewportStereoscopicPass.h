...
{
	// Base of the reserved StereoViewIndex range for nDisplay; engine indices are below this.
	static constexpr int32 StereoViewIndexBase = 100;

	/**
	* True if this view index falls in the nDisplay-reserved range.
	*/
	inline static bool IsValidStereoViewIndex(const int32 ViewIndex)
	{
		return ViewIndex >= StereoViewIndexBase;
	}
