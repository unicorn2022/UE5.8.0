...
void FDisplayClusterViewportConfigurationHelpers_Visibility::UpdateShowOnlyList_ICVFX(
	FDisplayClusterViewport& DstViewport,
	const FDisplayClusterConfigurationICVFX_VisibilityList& InVisibilityList,
	const FDisplayClusterConfigurationICVFX_CameraSettings* InCameraSettings)
{
	...
	// Create show-only list for 3D-LC/CK viewports
	...
	ADisplayClusterRootActor* ConfigurationRootActor = DstViewport.Configuration->GetRootActor(EDisplayClusterRootActorType::Configuration);
	if (!StageSettings || !ConfigurationRootActor)
	{
		return;
	}

	// Exclude entire cluster HideList
	TSet<FPrimitiveComponentId> EntireClusterHiddenComponentsList;
	UE::DisplayCluster::Configuration::VisibilityHelpers::ImplCollectComponentsFromVisibilityList(
		DstViewport.Configuration.Get(), StageSettings->HideList, EntireClusterHiddenComponentsList);
	DstViewport.GetVisibilitySettingsImpl().AddExcludedComponents(EntireClusterHiddenComponentsList);

	// Update exclude list for 3D-LC viewports
	const FString& ParentViewportId = DstViewport.GetRenderSettings().GetParentViewportId();
	const bool bLightcardViewport = EnumHasAnyFlags(DstViewport.GetRenderSettingsICVFX().RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::Lightcard);
	if (bLightcardViewport && !ParentViewportId.IsEmpty())
	{
		// Exclude Outer viewport HideList
		TSet<FPrimitiveComponentId> OuterViewportHiddenComponentsList;
		UE::DisplayCluster::Configuration::VisibilityHelpers::ImplCollectComponentsFromVisibilityList(
			DstViewport.Configuration.Get(), StageSettings->OuterViewportHideList, OuterViewportHiddenComponentsList);
		DstViewport.GetVisibilitySettingsImpl().AddExcludedComponents(OuterViewportHiddenComponentsList);

		// Exclude Per-viewport HideList
		const UDisplayClusterConfigurationData* ConfigData = ConfigurationRootActor->GetConfigData();
		if (const UDisplayClusterConfigurationViewport* ViewportCfg = ConfigData
			? ConfigData->GetViewport(DstViewport.ClusterNodeId, ParentViewportId) : nullptr)
		{
			TSet<FPrimitiveComponentId> PerViewportHiddenComponentsList;
			UE::DisplayCluster::Configuration::VisibilityHelpers::ImplCollectComponentsFromVisibilityList(
				DstViewport.Configuration.Get(), ViewportCfg->RenderSettings.HiddenContent, PerViewportHiddenComponentsList);

			DstViewport.GetVisibilitySettingsImpl().AddExcludedComponents(PerViewportHiddenComponentsList);
		}
	}

	// Update exclude list for CK viewports
	const bool bChromakeyViewport = EnumHasAnyFlags(DstViewport.GetRenderSettingsICVFX().RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::Chromakey);
	if (bChromakeyViewport && InCameraSettings)
	{
		// Exclude camera HideList
		TSet<FPrimitiveComponentId> CameraHiddenComponentsList;
		UE::DisplayCluster::Configuration::VisibilityHelpers::ImplCollectComponentsFromVisibilityList(
			DstViewport.Configuration.Get(), InCameraSettings->CameraHideList, CameraHiddenComponentsList);
		DstViewport.GetVisibilitySettingsImpl().AddExcludedComponents(CameraHiddenComponentsList);
	}
}
