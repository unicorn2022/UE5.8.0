...
public:
	/**
	* Constructs a new nDisplay viewport.
	*
...
public:
	...
	virtual TSharedRef<IDisplayClusterViewportProxy, ESPMode::ThreadSafe> GetViewportProxy() const override;
	...
	virtual const TArray<FDisplayClusterViewport_Context>& GetPrevFrameContexts() const override
	{
		check(IsInGameThread());
		return PreviousFrameData.Contexts;
	}

	virtual const FDisplayClusterViewportFrameData& GetFrameData() const override
	{
		check(IsInGameThread());
		return CurrentFrameData;
	}

	virtual const FDisplayClusterViewportFrameData& GetPrevFrameData() const override
	{
		check(IsInGameThread());
		return PreviousFrameData;
	}

	virtual TSharedPtr<FSceneViewStateReference, ESPMode::ThreadSafe> GetOrCreateViewState(uint32 InContextNum) override;
	...
	/** Release all viewport resources: textures and ViewState. */
	...
	/** Returns the raw ViewInfo from the camera, before stereo or warp-blend modification. */
	bool GetCameraViewPoint(FMinimalViewInfo& InOutViewInfo);
...
public:
	...
	// Usage flags of the viewport (e.g. InCamera, Chromakey, LightCard, Tile).
