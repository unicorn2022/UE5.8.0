...
////////////////////////////////////////////////////////////////////////////////
void FDisplayClusterViewportConfigurationHelpers_ICVFX::UpdateLegacyRuntimeICVFXFlagsFromViewportFlags(FDisplayClusterViewport& InOutViewport)
{
	// TODO: Deprecate and remove the duplicate flags from EDisplayClusterViewportRuntimeICVFXFlags.
	// They have been moved to EDisplayClusterViewportFlags.

	// Temporary bridge: mirror legacy ICVFX flags from EDisplayClusterViewportFlags into
	// EDisplayClusterViewportRuntimeICVFXFlags until the old flags are removed.
	FDisplayClusterViewport_RenderSettingsICVFX& RenderSettingsICVFX = InOutViewport.GetRenderSettingsICVFXImpl();
	if (EnumHasAnyFlags(InOutViewport.ViewportFlags, EDisplayClusterViewportFlags::ICVFX_InnerFrustum))
	{
		// Add viewport ICVFX usage as Incamera
		EnumAddFlags(RenderSettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::InCamera);
	}
	else if (EnumHasAnyFlags(InOutViewport.ViewportFlags, EDisplayClusterViewportFlags::ICVFX_ChromaKey))
	{
		// Add viewport ICVFX usage as Chromakey
		EnumAddFlags(RenderSettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::Chromakey);
	}
	else if (EnumHasAnyFlags(InOutViewport.ViewportFlags, EDisplayClusterViewportFlags::ICVFX_LightCard))
	{
		// Add viewport ICVFX usage as Lightcard
		EnumAddFlags(RenderSettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::Lightcard);

		// Add LC viewport position flag
		if (EnumHasAnyFlags(InOutViewport.ViewportFlags, EDisplayClusterViewportFlags::ICVFX_Modifier_RenderOverInnerFrustum))
		{
			EnumAddFlags(RenderSettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::OverInFrustum);
		}
		else if (EnumHasAnyFlags(InOutViewport.ViewportFlags, EDisplayClusterViewportFlags::ICVFX_Modifier_RenderUnderInnerFrustum))
		{
			EnumAddFlags(RenderSettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::UnderInFrustum);
		}
		else
		{
			check(false); // invalid flag settings
		}
	}
	else if (EnumHasAnyFlags(InOutViewport.ViewportFlags, EDisplayClusterViewportFlags::ICVFX_UVLightCard))
	{
		// Add viewport ICVFX usage as UVLightcard
		EnumAddFlags(RenderSettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::UVLightcard);
...
TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe> FDisplayClusterViewportConfigurationHelpers_ICVFX::CreateProjectionPolicyCameraICVFX(FDisplayClusterViewportConfiguration& InConfiguration, UDisplayClusterICVFXCameraComponent& InCameraComponent, const FDisplayClusterConfigurationICVFX_CameraSettings& InCameraSettings)
{
	TSharedPtr<IDisplayClusterProjectionPolicy, ESPMode::ThreadSafe> OutProjPolicy =
		InConfiguration.CreateProjectionPolicy_ICVFXViewport(InCameraComponent.GetCameraUniqueId(), EDisplayClusterViewportFlags::ICVFX_InnerFrustum);

	if (OutProjPolicy.IsValid() && UpdateCameraProjectionSettingsICVFX(InConfiguration, InCameraComponent, InCameraSettings, OutProjPolicy))
	{
		return OutProjPolicy;
	}
...
{
	FDisplayClusterViewport* CameraViewport = InConfiguration.FindViewport(InCameraComponent.GetCameraUniqueId(), EDisplayClusterViewportFlags::ICVFX_InnerFrustum);
	if (!CameraViewport)
	{
		// Create viewport instance
		CameraViewport = InConfiguration.CreateViewport(
			InCameraComponent.GetCameraUniqueId(),
			CreateProjectionPolicyCameraICVFX(InConfiguration, InCameraComponent, InCameraSettings),
			EDisplayClusterViewportFlags::ICVFX_InnerFrustum);
		if(!CameraViewport)
		{
			return nullptr;
		}
...
TArray<TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>> FDisplayClusterViewportConfigurationHelpers_ICVFX::PreviewGetRenderedInCameraViewports(FDisplayClusterViewportConfiguration& InConfiguration, const FString& InICVFXCameraId, bool bGetChromakey)
{
	...
	// Filter viewports by usage flags
	const EDisplayClusterViewportFlags ExpectedViewportFlags = bGetChromakey
	...
	if (FDisplayClusterViewportManager* ViewportManager = InConfiguration.GetViewportManagerImpl())
	{
		for (const TSharedPtr<FDisplayClusterViewport, ESPMode::ThreadSafe>& ViewportIt : ViewportManager->ImplGetEntireClusterViewports())
		{
			if (ViewportIt.IsValid()
				&& EnumHasAnyFlags(ViewportIt->ViewportFlags, ExpectedViewportFlags)
			...
			{
				// This is InCamera based viewport. Check by the base name.
				if (ViewportIt->ViewportBaseId == InICVFXCameraId)
				{
...
FDisplayClusterViewport* FDisplayClusterViewportConfigurationHelpers_ICVFX::GetOrCreateChromakeyViewport(FDisplayClusterViewportConfiguration& InConfiguration, UDisplayClusterICVFXCameraComponent& InCameraComponent, const FDisplayClusterConfigurationICVFX_CameraSettings& InCameraSettings)
{
	...
	FDisplayClusterViewport* ChromakeyViewport = InConfiguration.FindViewport(InICVFXCameraId, EDisplayClusterViewportFlags::ICVFX_ChromaKey);
	if (ChromakeyViewport == nullptr)
	{
		ChromakeyViewport = InConfiguration.CreateViewport(
			InICVFXCameraId,
			InConfiguration.CreateProjectionPolicy_ICVFXViewport(InICVFXCameraId, EDisplayClusterViewportFlags::ICVFX_ChromaKey),
			EDisplayClusterViewportFlags::ICVFX_ChromaKey);
		if (ChromakeyViewport == nullptr)
		{
			return nullptr;
...
FDisplayClusterViewport* FDisplayClusterViewportConfigurationHelpers_ICVFX::GetOrCreateLightcardViewport(FDisplayClusterViewport& BaseViewport, const bool bOverInFrustum)
{
	...
	const EDisplayClusterViewportFlags ViewportFlags= bOverInFrustum
		? EDisplayClusterViewportFlags::ICVFX_LightCard | EDisplayClusterViewportFlags::ICVFX_Modifier_RenderOverInnerFrustum
		: EDisplayClusterViewportFlags::ICVFX_LightCard | EDisplayClusterViewportFlags::ICVFX_Modifier_RenderUnderInnerFrustum;

	FDisplayClusterViewport* LightcardViewport = BaseViewport.Configuration->FindViewport(BaseViewport.GetId(), ViewportFlags);
	if (LightcardViewport == nullptr)
	{
		LightcardViewport = BaseViewport.Configuration->CreateViewport(
			BaseViewport.GetId(),
			BaseViewport.Configuration->CreateProjectionPolicy_ICVFXViewport(BaseViewport.GetId(), ViewportFlags),
			ViewportFlags);
...
{
	// Create new UV lightcard viewport
	const EDisplayClusterViewportFlags ViewportFlags = bOverInFrustum
		? EDisplayClusterViewportFlags::ICVFX_UVLightCard | EDisplayClusterViewportFlags::ICVFX_Modifier_RenderOverInnerFrustum
		: EDisplayClusterViewportFlags::ICVFX_UVLightCard | EDisplayClusterViewportFlags::ICVFX_Modifier_RenderUnderInnerFrustum;

	FDisplayClusterViewport* UVLightcardViewport = BaseViewport.Configuration->FindViewport(BaseViewport.GetId(), ViewportFlags);
	if (UVLightcardViewport == nullptr)
	{
		UVLightcardViewport = BaseViewport.Configuration->CreateViewport(
			BaseViewport.GetId(),
			BaseViewport.Configuration->CreateProjectionPolicy_ICVFXViewport(BaseViewport.GetId(), ViewportFlags),
			ViewportFlags);
...
void FDisplayClusterViewportConfigurationHelpers_ICVFX::UpdateChromakeyViewportSettings(FDisplayClusterViewport& DstViewport, FDisplayClusterViewport& InCameraViewport, const FDisplayClusterConfigurationICVFX_CameraSettings& InCameraSettings)
{
	...
	if (const FDisplayClusterConfigurationICVFX_ChromakeyRenderSettings* InChromakeyRenderSettings = InChromakeySettings.GetChromakeyRenderSettings(*StageSettings))
	{
		...
		if (!DstViewport.GetPostRenderSettings().Replace.IsEnabled())
		{
			...
			FDisplayClusterViewportConfigurationHelpers_Visibility::UpdateShowOnlyList_ICVFX(DstViewport, InChromakeyRenderSettings->ShowOnlyList, &InCameraSettings);
		}
		...
		if (ChromakeyTextureSize.GetMin() <= 0)
		{
			...
			{
				UE_LOGF(LogDisplayClusterViewport, Error, "ICVFX Chromakey Viewport '%ls': Has zero size. (ChromakeySizeMult=%.2f)", *DstViewport.GetId(), InChromakeyRenderSettings->ChromakeySizeMult);
			}
			else
			{
