...
{
	// If set, use this value instead of GFrameNumber.
	TOptional<uint32> FrameNumberOverride;
	...
	// When set, nDisplay's own overscan settings are ignored and this external value is used instead.
	uint8 bUseExternalOverscan : 1 = 0;

	// When external overscan is active, controls whether viewports render at the overscan resolution (true)
	// or at the configured resolution with post-render scaling (false).
	uint8 bExternalOverscanOversize : 1 = 1;

	// Overrides the camera overscan value when bUseExternalOverscan is true.
	// When not set, the overscan is read from FMinimalViewInfo.GetOverscan().
	TOptional<float> ExternalOverscanOverride;
...
public:
	...
	// Set via IDisplayClusterViewportConfiguration::SetRenderResolutionScale().
...
public:
	...
	*/
	...
	{
		if (FrameNumberOverride.IsSet())
		{
			return *FrameNumberOverride;
		}
