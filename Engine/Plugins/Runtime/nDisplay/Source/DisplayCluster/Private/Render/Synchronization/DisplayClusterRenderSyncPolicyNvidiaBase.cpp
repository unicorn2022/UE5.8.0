	...
	, bCfgPostPresentAlignment(CVarNvidiaSyncPostPresentAlignment.GetValueOnAnyThread())
{
	...
	{
		UE_LOGF(LogDisplayClusterRenderSync, Error, "NvAPI_Initialize() failed, error 0x%x", NvApiResult);
	}
#endif // WITH_NVAPI

	UE_LOGF(LogDisplayClusterRenderSync, Log,
		"NVIDIA sync configuration: PreInitAlignment=%d, PostBarrierJoinSleep=%f, PrePresentAlignmentLimit=%d, FrameCompletionLimit=%d, PostPresentAlignment=%d",
