...
#include "Render/Features/DisplayClusterUpscaler.h"
...
FDisplayClusterViewport::FDisplayClusterViewport(
	...
	, ViewportId(InConfiguration->BuildViewportName(InViewportBaseId, InViewportFlags, InOptTilePos))
	, ClusterNodeId(InConfiguration->GetClusterNodeId())
	, ViewportProxy(MakeShared<FDisplayClusterViewportProxy, ESPMode::ThreadSafe>(
		InConfiguration, ViewportBaseId, InViewportFlags, ViewportId, InProjectionPolicy))
	, ViewportPreview(MakeShared< FDisplayClusterViewportPreview, ESPMode::ThreadSafe>(InConfiguration, ViewportId))
	, UninitializedProjectionPolicy(InProjectionPolicy)
	, Contexts(CurrentFrameData.Contexts)
{
	check(!ClusterNodeId.IsEmpty());
	check(!ViewportBaseId.IsEmpty());
	check(UninitializedProjectionPolicy.IsValid());

	// Initialize ICVFX viewport
	if (EnumHasAnyFlags(InViewportFlags, EDisplayClusterViewportFlags::ICVFX_AnyRole))
	{
		// ICVFX viewports are not directly visible on the final frame texture (back buffer).
		// They are composed using ICVFX shaders.
		RenderSettings.bVisible = false;

		// All ICVFX viewports should have a flag to manage it properly
		// Mark as internal resource
		EnumAddFlags(RenderSettingsICVFX.RuntimeFlags, EDisplayClusterViewportRuntimeICVFXFlags::InternalResource);

		// Backward-compatibility bridge: derive legacy EDisplayClusterViewportRuntimeICVFXFlags
		// from the new EDisplayClusterViewportFlags until the legacy runtime enum is removed.
		FDisplayClusterViewportConfigurationHelpers_ICVFX::UpdateLegacyRuntimeICVFXFlagsFromViewportFlags(*this);
	}
...
TSharedRef<IDisplayClusterViewportProxy, ESPMode::ThreadSafe> FDisplayClusterViewport::GetViewportProxy() const
{
...
FDisplayClusterViewportProxyData* FDisplayClusterViewport::CreateViewportProxyData()
{
	...
	OutViewportProxyData->CurrentFrameData = CurrentFrameData;
...
void FDisplayClusterViewport::ReleaseAllResources()
{
	// Release viewport-owned render resources (textures, RTT).
	Resources.ReleaseAllResources();

	// Release material/view-state references on the game thread.
	CleanupViewState();

	// Drop game-thread view state references held by this viewport.
	// The viewport proxy keeps its own copy of these references.
	ViewStates.Reset();
}
...
void FDisplayClusterViewport::OnHandleStartScene()
{
	...
	else
	{
		...
		if (!ProjectionPolicy.IsValid())
		{
			...
			{
				UE_LOGF(LogDisplayClusterViewport, Error, "No projection policy assigned for Viewports '%ls'.", *GetId());
			}
		}
	}
}
...
void FDisplayClusterViewport::SetupSceneView(uint32 ContextNum, class UWorld* World, FSceneViewFamily& InOutViewFamily, FSceneView& InOutView) const
{
	...
	if(RenderSettings.CaptureMode == EDisplayClusterViewportCaptureMode::MoviePipeline)
	{
		...
		VisibilitySettings.SetupSceneView(*this, InOutView);
	...
	// Handle AutoExposure for this viewport
	if (FDisplayClusterViewportManager* ViewportManager = Configuration->GetViewportManagerImpl())
	{
		ViewportManager->AutoExposure->ConfigureAutoExposureForSceneView(*this, ContextNum, InOutView);
	}
...
void FDisplayClusterViewport::ResetFrameContexts()
{
	...
	// Reset means this viewport is no longer rendering. Both current and previous frame data are cleared
	// because if rendering resumes in a future frame it has no continuity with the old frame history
	// (e.g. temporal effects such as motion blur must not bleed across the gap).
	CurrentFrameData.Reset();
	PreviousFrameData.Reset();
}
...
bool FDisplayClusterViewport::UpdateFrameContexts(const uint32 InStereoViewIndex)
{
	...
	// Scale desired frame rect
	const FVector2D DesiredFrameMult = InFrameSettings.GetDesiredFrameMult() * InFrameSettings.RenderResolutionScale;
	FIntRect DesiredFrameTargetRect = FDisplayClusterViewportHelpers::ScaleViewportRect(RenderSettings.Rect, DesiredFrameMult);
	...
	// Snapshot the current contexts as the previous frame's contexts once per frame.
	// The frame number guard prevents redundant copies when this function is called multiple times per frame.
	const uint32 CurrentFrameNumber = Configuration->GetRenderFrameSettings().GetFrameNumber();
	if (CurrentFrameData.FrameNumber != CurrentFrameNumber)
	{
		// Cache previous frame data.
		PreviousFrameData = CurrentFrameData;
	}

	// Initialize new frame data
	CurrentFrameData.Reset();
	CurrentFrameData.FrameNumber = CurrentFrameNumber;
	...
	if (FrameTargetRect.Size().GetMin() <= 0)
	{
		...
		{
			UE_LOGF(LogDisplayClusterViewport, Error, "The viewport '%ls' FrameTarget rect has zero size %dx%d: Disabled", *GetId(), FrameTargetRect.Size().X, FrameTargetRect.Size().Y);
		}

		return false;
	}

	// When external overscan is enabled, bypass nDisplay's own overscan configuration
	// and apply a symmetric overscan sourced from either the explicit override value or the camera view info.
	if (Configuration->GetRenderFrameSettings().bUseExternalOverscan)
	{
		// External overscan applies to Outer and InnerFrustum viewports only.
		// ChromaKey, LightCard, and UVLightCard are excluded.
		const bool bViewportCanUseExternalOverscan = !EnumHasAnyFlags(GetViewportFlags(),
			EDisplayClusterViewportFlags::ICVFX_ChromaKey |
			EDisplayClusterViewportFlags::ICVFX_LightCard  |
			EDisplayClusterViewportFlags::ICVFX_UVLightCard);

		if (bViewportCanUseExternalOverscan)
		{
			float CameraOverscan = 0.f;
			if (Configuration->GetRenderFrameSettings().ExternalOverscanOverride.IsSet())
			{
				CameraOverscan = Configuration->GetRenderFrameSettings().ExternalOverscanOverride.GetValue();
			}
			else
			{
				FMinimalViewInfo CameraViewInfo;
				if (GetCameraViewPoint(CameraViewInfo))
				{
					CameraOverscan = CameraViewInfo.GetOverscan();
				}
			}

			// CameraOverscan is a per-side fraction of the half-frustum (UE camera convention).
			// nDisplay margin values are fractions of the full frustum extent, so divide by 2 to convert.
			const float FrustumMargin = CameraOverscan * 0.5f;

			// Apply overscan and enlarge the render target to match.
			RenderSettings.OverscanSettings.SetSymmetricOverscan(FrustumMargin, Configuration->GetRenderFrameSettings().bExternalOverscanOversize);
		}
	}
	...
	if (DesiredContextSize.GetMin() <= 0)
	{
		...
		{
			UE_LOGF(LogDisplayClusterViewport, Error, "The viewport '%ls' RenderTarget rect has zero size %dx%d: Disabled", *GetId(), DesiredContextSize.X, DesiredContextSize.Y);
		}
	...
	if (bUseTileRendering && ContextSize != TileContextSize)
	{
		...
		{
			UE_LOGF(LogDisplayClusterViewport, Error, "The viewport '%ls' context size [%dx%d] should be equal with tile size [%dx%d]: Disabled", *GetId(), ContextSize.X, ContextSize.Y, TileContextSize.X, TileContextSize.Y);
		}
	...
	for (uint32 ContextIt = 0; ContextIt < ViewportContextAmount; ++ContextIt)
	{
		...
		{
			Context.OverscanInnerRenderTargetRect = OverscanRuntimeSettings.OverscanPixels.GetInnerRect(RenderTargetRect);
		}
		else
		{
			Context.OverscanInnerRenderTargetRect = RenderTargetRect;
		}
		...
		// Update internal caches for current frame
		FMinimalViewInfo DummyViewInfo;
		SetupViewPoint(ContextIt, DummyViewInfo);
	}
