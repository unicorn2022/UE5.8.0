	...
	*/
	...
	while(!PreviewRenderFrame->RenderTargets.IsEmpty())
	{
		...
		while(!PreviewRenderFrame->RenderTargets[0].ViewFamilies.IsEmpty())
		{
			...
			// ViewState references that must stay alive until rendering is complete.
			// They are passed to the render thread to prevent premature destruction.
			TArray<TSharedPtr<FSceneViewStateReference, ESPMode::ThreadSafe>> ViewStatesToKeepAlive;
			...
			for(const FDisplayClusterRenderFrameTargetView& ViewIt : ViewFamiliesIt.Views)
			{
				...
				TSharedPtr<FSceneViewStateReference, ESPMode::ThreadSafe> ReferencedViewState;
				FSceneView* View = ViewportPtr->ViewportPreview->CalcSceneView(ViewFamily, ViewIt.ContextNum, ReferencedViewState);
				...
				if (View)
				{
					...
					// Keep the ViewState alive until rendering is complete.
					if (ReferencedViewState.IsValid())
					{
						ViewStatesToKeepAlive.Add(ReferencedViewState);
					}
				}
			}
			...
			if (bShouldRenderViewport)
			{
				...
				// Pass ViewState references to the render thread to keep them alive until rendering is complete.
				ENQUEUE_RENDER_COMMAND(FDisplayClusterViewportManagerPreview_KeepViewStatesAlive)(
					[ViewStatesToKeepAlive = MoveTemp(ViewStatesToKeepAlive)](FRHICommandListImmediate& RHICmdList) { });
