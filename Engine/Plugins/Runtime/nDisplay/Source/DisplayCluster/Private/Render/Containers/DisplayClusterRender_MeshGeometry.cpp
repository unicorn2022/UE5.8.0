	...
	if (FPaths::FileExists(FullPathFileName))
	{
		...
		if (FFileHelper::LoadANSITextFileToStrings(*FullPathFileName, nullptr, data) == true)
		{
			...
			for (FString Line : data)
			{
				...
				if (!ParseLine(Line))
				{
					UE_LOGF(LogDisplayClusterRender, Error, "MeshGeometryLoaderOBJ: Invalid line %d: '%ls'", LineIdx , *Line);
					bResult = false;
				}
			}
			...
			if (!bResult)
			{
				UE_LOGF(LogDisplayClusterRender, Error, "MeshGeometryLoaderOBJ: Can't load mesh geometry from file '%ls'", *FullPathFileName);
			}
...
int32 FDisplayCluster_MeshGeometryLoaderOBJ::ExtractFaceVertex(const FString& Line)
{
	...
	if (Line.ParseIntoArray(Data, MeshGeometryStrings::delims::Face) > 1)
	{
		...
		{
			UE_LOGF(LogDisplayClusterRender, Error, "MeshGeometryLoaderOBJ: broken vertex index. Line: '%ls'", *Line);
		...
		{
			UE_LOGF(LogDisplayClusterRender, Error, "MeshGeometryLoaderOBJ: broken uv index. Line: '%ls'", *Line);
			OutFaceIdx.UVIdx = -1;
		}
		else
		{
		...
		if (bImportVertexNormal && Data.Num() > 2)
		{
			...
			{
				UE_LOGF(LogDisplayClusterRender, Error, "MeshGeometryLoaderOBJ: broken vertex normal  index. Line: '%ls'", *Line);
