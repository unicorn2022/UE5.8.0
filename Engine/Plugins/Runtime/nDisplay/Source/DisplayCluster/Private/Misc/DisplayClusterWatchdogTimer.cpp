...
{
	UE_LOGF(LogDisplayClusterNetwork, Log, "Watchdog %ls: working thread started", *TimerName);
	...
	while (!bThreadExitRequested)
	{
		...
		UE_LOGF(LogDisplayClusterNetwork, Verbose, "Watchdog %ls: timer set - %u ms", *TimerName, Timeout);
...

		{
			...
			if (!bTimerWasReset)
			{
				UE_LOGF(LogDisplayClusterNetwork, Log, "Watchdog %ls: timeout!", *TimerName);
				OnWatchdogTimeOut().Broadcast();
			}
			else
			{
				UE_LOGF(LogDisplayClusterNetwork, Verbose, "Watchdog %ls: timer reset", *TimerName);
			}
	...
	UE_LOGF(LogDisplayClusterNetwork, Log, "Watchdog %ls: working thread finished", *TimerName);
