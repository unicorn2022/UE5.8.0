...
bool FDisplayClusterFailoverNodeCtrlMain::Initialize(const UDisplayClusterConfigurationData* ConfigData)
{
	...
	if (!bConfigSetup)
	{
		UE_LOGF(LogDisplayClusterFailover, Warning, "Couldn't setup failover configuration");
		return false;
	}
...
bool FDisplayClusterFailoverNodeCtrlMain::HandleFailure(const FString& FailedNodeId)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Log, "Reported node '%ls' failure.", *FailedNodeId);

	// If failover is disabled, always terminate
	if (!FailoverSettings->bEnabled)
	{
		bTerminateTransactionProcessingLoop = true;
		UE_LOGF(LogDisplayClusterFailover, Log, "Failover subsystem is disabled. No failures allowed.");
		return false;
	}
	...
	{
		bTerminateTransactionProcessingLoop = true;
		UE_LOGF(LogDisplayClusterFailover, Log, "This node just failed. Terminating...");
		return false;
	}
	...
	{
		bTerminateTransactionProcessingLoop = true;
		UE_LOGF(LogDisplayClusterFailover, Log, "Critical node '%ls' has failed. Cluster termination is required.", *FailedNodeId);
		return false;
	}
	...
	while (!NodeProcessing.IsEmpty())
	{
		...
		{
			bTerminateTransactionProcessingLoop = true;
			UE_LOGF(LogDisplayClusterFailover, Log, "Critical node '%ls' has failed. Cluster termination is required.", *NodeProcessing);
			return false;
		}
		...
		{
			bTerminateTransactionProcessingLoop = true;
			UE_LOGF(LogDisplayClusterFailover, Log, "No P-node candidates left.");
			return false;
		}

		UE_LOGF(LogDisplayClusterFailover, Log, "Elected new P-node: '%ls'.", *NewPrimaryId);
		...
		if (!ProcessRecovery())
		{
			...
			UE_LOGF(LogDisplayClusterFailover, Warning, "Couldn't switch to the new P-node '%ls'. Restarting recovery cycle.", *NewPrimaryId);
		}
		else
		{
...
EDisplayClusterCommResult FDisplayClusterFailoverNodeCtrlMain::GetObjectsData(EDisplayClusterSyncGroup InSyncGroup, TMap<FString, TArray<uint8>>& OutObjectsData)
{
...
EDisplayClusterCommResult FDisplayClusterFailoverNodeCtrlMain::GetNativeInputData(TMap<FString, TArray<uint8>>& OutNativeInputData)
{
...
EDisplayClusterCommResult FDisplayClusterFailoverNodeCtrlMain::PropagateStatesData(const TArray<uint8>& InLocalStatesData, TArray<uint8>& OutClusterStatesData)
{
	static const FName TxnName     = TEXT("PropagateStatesData");
	static const FName BarrierName = TEXT("SyncStatesBarrier");
	...
	const EDisplayClusterCommResult CommResult = ProcessTransactionSYNC(
		...
		[&]() -> EDisplayClusterCommResult // OpSendReq
		{
			return GetNodeController()->PropagateStatesData(InLocalStatesData, OutClusterStatesData);
		});
...
bool FDisplayClusterFailoverNodeCtrlMain::SetupFailoverConfiguration(const UDisplayClusterConfigurationData* ConfigData)
{
	...
	{
		UE_LOGF(LogDisplayClusterFailover, Error, "Invalid configuration data");
		return false;
	}
...
{
	UE_LOGF(LogDisplayClusterFailover, Log, "Failover config: Failover Enabled = %d", FailoverCfg.bEnabled ? 1 : 0);

	const int32 PrioritizedBackupsNum = FailoverCfg.PrimaryBackups.ItemNames.Num();
	UE_LOGF(LogDisplayClusterFailover, Log, "Failover config: Prioritized primary backups (%d items):", PrioritizedBackupsNum);
	for (int32 Idx = 0; Idx < PrioritizedBackupsNum; ++Idx)
	{
		UE_LOGF(LogDisplayClusterFailover, Log, "Failover config: Prio-backup [%2d]: %ls", Idx, *FailoverCfg.PrimaryBackups.ItemNames[Idx]);
	}

	const int32 CriticalNodesNum = FailoverCfg.CriticalNodes.ItemNames.Num();
	UE_LOGF(LogDisplayClusterFailover, Log, "Failover config: Critical nodes (%d items):", CriticalNodesNum);
	for (int32 Idx = 0; Idx < CriticalNodesNum; ++Idx)
	{
		UE_LOGF(LogDisplayClusterFailover, Log, "Failover config: Critical node [%2d]: %ls", Idx, *FailoverCfg.CriticalNodes.ItemNames[Idx]);
	}
}
...
void FDisplayClusterFailoverNodeCtrlMain::SetupInternals()
{
	...
	{
		UE_LOGF(LogDisplayClusterFailover, Log, "Found invalid backup node '%ls'. Removing it from the backup list.", *InvalidBackupNode);
	}
...
bool FDisplayClusterFailoverNodeCtrlMain::ProcessRecovery()
{
	...
	{
		UE_LOGF(LogDisplayClusterFailover, Warning, "Failed to process recovery syncrhonization");
		return false;
	}
	...
	const FOpSendReq&      OpSendReq)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn GET beg: %ls [%llu]",
	...
	while (!bTerminateTransactionProcessingLoop && !IsEngineExitRequested())
	{
...

		{
			...
			{
				UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn GET@%ls: %ls [%llu] - using cached data",
			...
			else
			{
				...
				UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn GET@%ls: %ls [%llu] - sending request",
					*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
		...
		UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn GET@%ls: %ls [%llu] - request failed",
			*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn GET end: %ls [%llu]", *TransactionName.ToString(), CurrentTransactionNum);
	...
	const FOpSendReq& OpSendReq)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn PUSH beg: %ls [%llu]",
		*TransactionName.ToString(), CurrentTransactionNum);
	...
	while (!bTerminateTransactionProcessingLoop && !IsEngineExitRequested())
	{
...

		{
			...
			UE_LOGF(LogDisplayClusterFailover, VeryVerbose, "Txn PUSH@%ls: %ls [%llu] - sending request",
				*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
		...
		UE_LOGF(LogDisplayClusterFailover, Log, "Txn PUSH@%ls: %ls [%llu] - request failed",
			*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn PUSH end: %ls [%llu]", *TransactionName.ToString(), CurrentTransactionNum);
	...
	const FOpSendReq& OpSendReq)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn SYNC beg: %ls [%llu]",
		*TransactionName.ToString(), CurrentTransactionNum);
	...
	while (!bTerminateTransactionProcessingLoop && !IsEngineExitRequested())
	{
...

		{
			...
			{
				UE_LOGF(LogDisplayClusterFailover, VeryVerbose, "Txn SYNC@%ls: %ls [%llu] - using cached data",
					*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
			...
			else
			{
				...
				UE_LOGF(LogDisplayClusterFailover, VeryVerbose, "Txn SYNC@%ls: %ls [%llu] - sending request",
					*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
		...
		UE_LOGF(LogDisplayClusterFailover, Log, "Txn SYNC@%ls: %ls [%llu] - request failed",
			*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn SYNC end: %ls [%llu]", *TransactionName.ToString(), CurrentTransactionNum);
	...
	const TSet<FString>& TargetNodes)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn MCAST beg: %ls [%llu]",
		*TransactionName.ToString(), CurrentTransactionNum);
	...
	for (const FString& TxnTargetNode : TargetNodes)
	{
...

		{
			...
			UE_LOGF(LogDisplayClusterFailover, VeryVerbose, "Txn MCAST@%ls: %ls [%llu] - sending request",
				*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
		...
		UE_LOGF(LogDisplayClusterFailover, Log, "Txn MCAST@%ls: %ls [%llu] - request failed",
			*TxnTargetNode, *TransactionName.ToString(), CurrentTransactionNum);
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Txn MCAST end: %ls [%llu]", *TransactionName.ToString(), CurrentTransactionNum);
	...
	const FOpSendReq& OpSendReq)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "RECOVERY beg: %ls [%llu] - requesting data from '%ls'",
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "RECOVERY end: %ls [%llu]",
		*TransactionName.ToString(), CurrentTransactionNum);
