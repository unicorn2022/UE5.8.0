...
{
	GetObjectsData_OpLoad = [this](const EDisplayClusterSyncGroup InSyncGroup, TMap<FString, TArray<uint8>>& OutObjectsData)
		{
			OpLoadImpl<FCache_GetObjectsData>(GetObjectsDataSlotName(InSyncGroup), OutObjectsData);
		};

	GetObjectsData_OpSave = [this](const EDisplayClusterSyncGroup InSyncGroup, TMap<FString, TArray<uint8>>& InObjectsData)
		{
...
{
	GetNativeInputData_OpLoad = [this](TMap<FString, TArray<uint8>>& OutNativeInputData)
		{
			OpLoadImpl<FCache_GetNativeInputData>(UE::nDisplay::Failover::Private::Name_SlotNativeInput, OutNativeInputData);
		};

	GetNativeInputData_OpSave = [this](TMap<FString, TArray<uint8>>& InNativeInputData)
		{
...
bool FDisplayClusterCommDataCache::OpIsCachedImpl(const FName& SlotName) const
{
	// Check local cache first as the most used
	{
		...
		{
			UE_LOGF(LogDisplayClusterFailover, Warning, "Local cache doesn't have a '%ls' slot", *SlotName.ToString());
			checkNoEntry();
		}
	}
...
void FDisplayClusterCommDataCache::OpLoadImpl(const FName& SlotName, Args&... SlotData)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Warning, "No cached data found for '%ls'", *SlotName.ToString());
	checkNoEntry();
}
...
void FDisplayClusterCommDataCache::SubscribeToCallbacks()
{
	...
	// Set up a post-failure negotiation delegate
	{
		...
		{
			UE_LOGF(LogDisplayClusterFailover, Warning, "Couldn't set up a post-failure negotiation delegate");
		}
	}
}
...
{
	UE_LOGF(LogDisplayClusterFailover, Log, "Post-failure recovery. Building actual sync state...");
...
void FDisplayClusterCommDataCache::GenerateNodeSyncState(TArray<uint8>& OutNodeSyncState)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Post-failure recovery. Local sync state:\n%ls", *LocalDataCache.ToLogString());
...
void FDisplayClusterCommDataCache::UpdateClusterSyncState(const TArray<uint8>& InClusterSyncState)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Verbose, "Post-failure recovery. Got cluster sync state:\n%ls", *ClusterDataCache.ToLogString());
}
...
void FDisplayClusterCommDataCache::BuildClusterSyncState(const TMap<FString, TArray<uint8>>& RequestData, TMap<FString, TArray<uint8>>& ResponseData)
{
	...
	UE_LOGF(LogDisplayClusterFailover, Log, "\n Post-failure synchronization \n\nInput states: \n");
	...
	for (const TPair<FString, TArray<uint8>>& NodeRequest : RequestData)
	{
		...
		UE_LOGF(LogDisplayClusterFailover, Log, " * %ls: %ls\n", *NodeRequest.Key, *NodeSyncState.ToLogString());
	}
	...
	UE_LOGF(LogDisplayClusterFailover, Log, "\nOutput state: \n [%ls]", *ClusterSyncData.ToLogString());
...
FString FDisplayClusterCommDataCache::FCache_GetObjectsData::ToLogString() const
{
	...
	int32 Idx = 0;
	for (const TPair<FString, TArray<uint8>>& DataIt : ObjData)
	{
		Output += FString::Printf(TEXT("<%d : %s=[%d bytes]> "), Idx++, *DataIt.Key, DataIt.Value.Num());
	}
...
FString FDisplayClusterCommDataCache::FCache_GetNativeInputData::ToLogString() const
{
	...
	int32 Idx = 0;
	for (const TPair<FString, TArray<uint8>>& DataIt : NativeInputData)
	{
		Output += FString::Printf(TEXT("<%d : %s -> %d bytes> "), Idx++, *DataIt.Key, DataIt.Value.Num());
	}
