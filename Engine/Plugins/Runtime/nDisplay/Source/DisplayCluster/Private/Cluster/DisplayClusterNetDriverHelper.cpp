...
{	
	...
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "Couldn't not register event binary client for %ls:%d. It already exists.", *ClientAddress, ClientPort);
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "Couldn't not connect to %ls:%d", *ClientAddress, ClientPort);
...
bool FDisplayClusterNetDriverHelper::SendCommandToCluster(uint32 ClusterId, const FDisplayClusterClusterEventBinary& Event)
{
	...
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "Event binary client not found for ClusterId %d", ClusterId);
		return false;
	}
