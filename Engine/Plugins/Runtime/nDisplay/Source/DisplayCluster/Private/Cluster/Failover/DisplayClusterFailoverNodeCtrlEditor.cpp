...
EDisplayClusterCommResult FDisplayClusterFailoverNodeCtrlEditor::GetObjectsData(EDisplayClusterSyncGroup InSyncGroup, TMap<FString, TArray<uint8>>& OutObjectsData)
{
	// `Editor` failover controller doesn't perform any validations, just forward the call
...
EDisplayClusterCommResult FDisplayClusterFailoverNodeCtrlEditor::GetNativeInputData(TMap<FString, TArray<uint8>>& OutNativeInputData)
{
	// `Editor` failover controller doesn't perform any validations, just forward the call
	return GetNodeController()->GetNativeInputData(OutNativeInputData);
}

EDisplayClusterCommResult FDisplayClusterFailoverNodeCtrlEditor::PropagateStatesData(const TArray<uint8>& InLocalStatesData, TArray<uint8>& OutClusterStatesData)
{
	// `Editor` failover controller doesn't perform any validations, just forward the call
