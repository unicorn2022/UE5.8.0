...
bool FDisplayClusterClusterNodeCtrlEditor::Initialize()
{
	...
	if (!bBaseInitialized)
	{
		UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - Couldn't initialize base controller internals", *GetControllerName());
		// Don't return false as it's not critical for PIE
	}

	if (!InitializeServers())
	{
		UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - Couldn't initialize internal servers", *GetControllerName());
		// Don't return false as it's not critical for PIE
	}

	if (!StartServers())
	{
		UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - An error occurred while starting internal servers", *GetControllerName());
		// Don't return false as it's not critical for PIE
	}
...
EDisplayClusterCommResult FDisplayClusterClusterNodeCtrlEditor::GetTimeData(double& OutDeltaTime, double& OutGameTime, TOptional<FQualifiedFrameTime>& OutFrameTime)
{
	...
	if (!ClusterMgr)
	{
		UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - wrong cluster manager object", *GetControllerName());
...
EDisplayClusterCommResult FDisplayClusterClusterNodeCtrlEditor::GetObjectsData(EDisplayClusterSyncGroup InSyncGroup, TMap<FString, TArray<uint8>>& OutObjectsData)
{
	// PIE stub
	return EDisplayClusterCommResult::Ok;
}
...
EDisplayClusterCommResult FDisplayClusterClusterNodeCtrlEditor::GetNativeInputData(TMap<FString, TArray<uint8>>& OutNativeInputData)
{
	// PIE stub
	return EDisplayClusterCommResult::Ok;
}

EDisplayClusterCommResult FDisplayClusterClusterNodeCtrlEditor::PropagateStatesData(const TArray<uint8>& InLocalStatesData, TArray<uint8>& OutClusterStatesData)
{
	// PIE stub
	return EDisplayClusterCommResult::Ok;
}
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Initializing servers...", *GetControllerName());
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Starting servers...", *GetControllerName());

	// Get config data
	const UDisplayClusterConfigurationData* ConfigData = GDisplayCluster->GetPrivateConfigMgr()->GetConfig();
	if (!ConfigData)
	{
		UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - Couldn't get configuration data", *GetControllerName());
		return false;
	}
	...
	if (!PrimaryNodeCfg)
	{
		UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - No P-node configuration was found", *GetControllerName());
		return false;
	}
	...
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Listening at `%ls` to port_cej=%u, port_ceb=%u",
