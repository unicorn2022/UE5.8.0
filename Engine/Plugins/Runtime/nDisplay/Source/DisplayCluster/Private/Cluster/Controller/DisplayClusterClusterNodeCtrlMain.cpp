...
bool FDisplayClusterClusterNodeCtrlMain::Initialize()
{
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't initialize base controller internals", *GetControllerName());
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't initialize node servers", *GetControllerName());
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't initialize node clients", *GetControllerName());
		return false;
	}
	...
	if (!ConfigMgr)
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't get configuration manager", *GetControllerName());
		return false;
	}
	...
	if (!ConfigData)
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't get configuration data", *GetControllerName());
		return false;
	}
	...
	if (NodeId.IsEmpty())
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Invalid cluster node ID", *GetControllerName());
		return false;
	}
	...
	if (!ConfigNode)
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't obtain any configuration for node '%ls'", *GetControllerName(), *NodeId);
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - An error occurred while starting internal servers", *GetControllerName());
		return false;
	}
	...
	if (bIsPrimary)
	{
		...
		{
			UE_LOGF(LogDisplayClusterCluster, Error, "%ls - An error occurred while starting external servers", *GetControllerName());
			return false;
		}
	}
	...
	if (!bClientsStarted)
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - An error occurred during clients start", *GetControllerName());
		return false;
	}
	...
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - In-cluster connections have been established", *GetControllerName());
...
int32 FDisplayClusterClusterNodeCtrlMain::InitializeGeneralPurposeBarrierClients()
{
	...
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Allocated GPB clients set '%d'", *GetControllerName(), NewClientSetId);
...
void FDisplayClusterClusterNodeCtrlMain::ReleaseGeneralPurposeBarrierClients(int32 ClientSetId)
{
	...
	{
		FScopeLock Lock(&GPBClientsCS);
		UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Releasing GPB clients set '%d'", *GetControllerName(), ClientSetId);
...
bool FDisplayClusterClusterNodeCtrlMain::DropClusterNode(const FString& NodeId)
{
	...
	for (const FName& ServiceName : InternalServiceNames)
	{
		...
		{
			UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - Service '%ls' is not running, couldn't find the server", *GetControllerName(), *ServiceName.ToString());
		}
	}
...
EDisplayClusterCommResult FDisplayClusterClusterNodeCtrlMain::GetObjectsData(EDisplayClusterSyncGroup InSyncGroup, TMap<FString, TArray<uint8>>& OutObjectsData)
{
	IPDisplayClusterClusterManager* const ClusterMgr = GDisplayCluster->GetPrivateClusterMgr();
...
EDisplayClusterCommResult FDisplayClusterClusterNodeCtrlMain::GetNativeInputData(TMap<FString, TArray<uint8>>& OutNativeInputData)
{
	IPDisplayClusterClusterManager* const ClusterMgr = GDisplayCluster->GetPrivateClusterMgr();
...
EDisplayClusterCommResult FDisplayClusterClusterNodeCtrlMain::PropagateStatesData(const TArray<uint8>& InLocalStatesData, TArray<uint8>& OutClusterStatesData)
{
	if (const TSharedPtr<FNodeClientSet>& ClientSet = GetActiveClientSet())
	{
		return ClientSet->ClusterSyncClient->PropagateStatesData(InLocalStatesData, OutClusterStatesData);
	}
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Initializing local servers...", *GetControllerName());

	// CS - ClusterSync
	{
		const FName& ServerName = UE::nDisplay::Network::Configuration::ClusterSyncServerName;
		UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Initializing %ls...", *GetControllerName(), *ServerName.ToString());
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Starting internal servers...", *GetControllerName());
	...
	for (const FName& ServiceName : InternalServiceNames)
	{
		...
		{
			UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - Couldn't find server '%ls'", *GetControllerName(), *ServiceName.ToString());
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Internal TCP listener was not able to start at [%ls:%u]", *GetControllerName(), *InConfigNode->Host, RequestedPortNum);
	}
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Starting external servers...", *GetControllerName());
	...
	if (TSharedPtr<FDisplayClusterService> Service = GetService(UE::nDisplay::Network::Configuration::JsonEventsExternalServerName).Pin())
	{
		...
		UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Public JSON events server starting at [%ls:%u]", *GetControllerName(), *InConfigNode->Host, PortNum);
	...
	if (TSharedPtr<FDisplayClusterService> Service = GetService(UE::nDisplay::Network::Configuration::BinaryEventsExternalServerName).Pin())
	{
		...
		UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Public binary events server starting at [%ls:%u]", *GetControllerName(), *InConfigNode->Host, PortNum);
		bAllExternalServersStarted &= StartServerWithLogs(Service, *InConfigNode->Host, PortNum);
	}
			...
			while (NodeCtrlWeakPtr.IsValid() && !IsEngineExitRequested())
			{
				if (TSharedPtr<FDisplayClusterClusterNodeCtrlMain> NodeCtrl = NodeCtrlWeakPtr.Pin())
				{
					...
					if (!bExternalServersRunning)
					{
						...
						UE_LOGF(LogDisplayClusterCluster, Warning, "%ls - (Delayed re-start) Couldn't start external servers. Retry in %d seconds.", *GetControllerName(), RetryDelay);
					...
					else
					{
						...
						UE_LOGF(LogDisplayClusterCluster, Log, "%ls - (Delayed re-start) External servers started.", *GetControllerName());
						break;
					}
				}
			}
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Stopping internal servers...", *GetControllerName());
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Stopping external servers...", *GetControllerName());
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Initializing internal clients...", *GetControllerName());

	// Cluster manager
	const IPDisplayClusterClusterManager* const ClusterMgr = GDisplayCluster->GetPrivateClusterMgr();
	if (!ClusterMgr)
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't access cluster manager", *GetControllerName());
		return false;
	}
	...
	if (NodeIds.IsEmpty())
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - No cluster nodes found", *GetControllerName());
		return false;
	}
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Starting internal clients...", *GetControllerName());
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't connect to the primary node", *GetControllerName());
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't connect to the remaining nodes", *GetControllerName());
		return false;
	}
...
bool FDisplayClusterClusterNodeCtrlMain::StartClientsConnectPrimary(const UDisplayClusterConfigurationData* const InConfigData)
{
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't find a set of '%ls' P-node clients", *GetControllerName(), *InfoPrimaryNode.Id);
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't find a configuration data of '%ls' P-node", *GetControllerName(), *InfoPrimaryNode.Id);
		return false;
	}
...
bool FDisplayClusterClusterNodeCtrlMain::StartClientsConnectRemaining(const UDisplayClusterConfigurationData* const InConfigData)
{
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't obtain '%ls' P-node client set", *GetControllerName(), *InConfigData->Cluster->PrimaryNode.Id);
		return false;
	}
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't gather any service hosting info", *GetControllerName());
		return false;
	}

	// Log connection information
	{
		UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Exported connection info: port_cs=%u, port_eb=%u, port_ej=%u",
		...
		{
			UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Imported connection info: node=%ls port_cs=%u, port_eb=%u, port_ej=%u",
	...
	for (const TPair<FName, TSharedRef<FNodeClientSet>>& NodeClients : Clients)
	{
		...
		if (!NodeServices)
		{
			UE_LOGF(LogDisplayClusterCluster, Error, "%ls - The cluster hosting information doesn't contain node '%ls'", *GetControllerName(), *NodeClients.Key.ToString());
			return false;
		}
		...
		{
			UE_LOGF(LogDisplayClusterCluster, Error, "%ls - Couldn't obtain configuration data for node '%ls'", *GetControllerName(), *ConnectingToNodeId);
			return false;
		}
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Stopping internal clients...", *GetControllerName());
...
{
	UE_LOGF(LogDisplayClusterCluster, Log, "%ls - Shutting down...", *GetControllerName());
		...
		if (const IPDisplayClusterConfigManager* const ConfigMgr = GDisplayCluster->GetPrivateConfigMgr())
		{
			if (const UDisplayClusterConfigurationData* const ConfigData = ConfigMgr->GetConfig())
			{
				for (const FString& NodeId : ActiveNodes)
				{
					...
					if (!IsValid(NodeCfg))
					{
						UE_LOGF(LogDisplayClusterCluster, Warning, "Couldn't initialize GPB client for node '%ls'. No corresponding config data found.", *NodeId);
						return false;
					}
					...
					{
						UE_LOGF(LogDisplayClusterCluster, Warning, "Couldn't establish connection for GPB client '%ls'.", *NewClient->GetName());
