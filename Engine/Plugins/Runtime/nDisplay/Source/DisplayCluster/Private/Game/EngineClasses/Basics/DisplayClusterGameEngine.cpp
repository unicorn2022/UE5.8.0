...
{
	UE_LOGF(LogDisplayClusterEngine, Log, "UDisplayClusterGameEngine::Init");

	// Detect requested operation mode
	OperationMode = DetectOperationMode();

	// Modify any required CVars before keep initialization
	ModifyCVars();
	...
	if (OperationMode == EDisplayClusterOperationMode::Cluster)
	{
		...
		{
			UE_LOGF(LogDisplayClusterEngine, Log, "Node ID is not specified. Trying to resolve from host address...");
			...
			UE_LOGF(LogDisplayClusterEngine, Log, "Node ID has been successfully resolved: %ls", *NodeId);
		}
...
EDisplayClusterOperationMode UDisplayClusterGameEngine::DetectOperationMode() const
{
	...
	UE_LOGF(LogDisplayClusterEngine, Log, "Detected operation mode: %ls", *DisplayClusterTypesConverter::template ToString(OpMode));
...
bool UDisplayClusterGameEngine::InitializeInternals()
{
	...
	if (!DCAPI)
	{
		UE_LOGF(LogDisplayClusterEngine, Warning, "Could not initialize DC game engine, invalid GDisplayCluster");
		return false;
	}
	...
	UE_LOGF(LogDisplayClusterEngine, Log, "Configuring sound enabled: %ls", *DisplayClusterTypesConverter::template ToString(bSoundEnabled));
	if (!bSoundEnabled)
	{
...
bool UDisplayClusterGameEngine::GetResolvedNodeId(const UDisplayClusterConfigurationData* ConfigData, FString& NodeId) const
{
	...
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "Couldn't get local addresses list. Cannot find node ID by its address.");
		return false;
	}

	if (LocalAddresses.Num() < 1)
	{
		UE_LOGF(LogDisplayClusterCluster, Error, "No local addresses found");
		return false;
	}
	...
	for (const auto& it : ConfigData->Cluster->Nodes)
	{
		for (const auto& LocalAddress : LocalAddresses)
		{
			...
			UE_LOGF(LogDisplayClusterCluster, Log, "Comparing addresses: %ls - %ls", *epaddr, *it.Value->Host);
...
bool UDisplayClusterGameEngine::ValidateConfigFile(const FString& FilePath)
{
	...
	switch (ConfigVersion)
	{
		case EDisplayClusterConfigurationVersion::Version_426:
			...
			UE_LOGF(LogDisplayClusterEngine, Error, "Detected old (.ndisplay 4.26 or .ndisplay 4.27p1) config format. Please upgrade to the actual version.");
			return true;

		case EDisplayClusterConfigurationVersion::Version_427:
			// Ok, it's one of the actual config formats
			UE_LOGF(LogDisplayClusterEngine, Log, "Detected (.ndisplay 4.27) config format");
			return true;

		case EDisplayClusterConfigurationVersion::Version_500:
			// Ok, it's one of the actual config formats
			UE_LOGF(LogDisplayClusterEngine, Log, "Detected (.ndisplay 5.00) config format");
			return true;

		case EDisplayClusterConfigurationVersion::Unknown:
		default:
			// Something unexpected came here
			UE_LOGF(LogDisplayClusterEngine, Error, "Unknown or unsupported config format");
			break;
	}
...
{
	UE_LOGF(LogDisplayClusterEngine, Log, "UDisplayClusterGameEngine::PreExitImpl");
...
{
	UE_LOGF(LogDisplayClusterEngine, Log, "UDisplayClusterGameEngine::LoadMap, URL=%ls", *URL.ToString());
	...
	if (OperationMode == EDisplayClusterOperationMode::Cluster)
	{
		...
		if(BarrierAvoidanceOn() && RunningMode != EDisplayClusterRunningMode::Startup)
		{
			...
			UE_LOGF(LogDisplayClusterEngine, Display, "LoadMap occurred after startup for Level %ls", *WaitForGameEvent.Type);
		}
		else
		{
...
{
	UE_LOGF(LogDisplayClusterEngine, Verbose, "UDisplayClusterGameEngine::Tick, Delta=%f, Idle=%d", DeltaSeconds, bIdleMode ? 1 : 0);
	...
	if (CanTick())
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////
		// Frame start barrier
		UE_LOGF(LogDisplayClusterEngine, Verbose, "Sync frame start");
		...
		UE_LOGF(LogDisplayClusterEngine, Verbose, "DisplayCluster delta seconds: %f", DeltaSeconds);

		// Perform PreTick for DisplayCluster module
		UE_LOGF(LogDisplayClusterEngine, Verbose, "Perform PreTick()");
		GDisplayCluster->PreTick(DeltaSeconds);

		// Perform UGameEngine::Tick() calls for scene actors
		UE_LOGF(LogDisplayClusterEngine, Verbose, "Perform UGameEngine::Tick()");
		Super::Tick(DeltaSeconds, bIdleMode || bForcedTickIdleMode);

		// Perform PostTick for DisplayCluster module
		UE_LOGF(LogDisplayClusterEngine, Verbose, "Perform PostTick()");
		...
		if (Diagnostics.bSimulateLag)
		{
			...
			UE_LOGF(LogDisplayClusterEngine, Log, "Simulating lag: %f seconds", LagTime);
		...
		UE_LOGF(LogDisplayClusterEngine, Verbose, "Sync frame end");
	}
	else
	{
		UE_LOGF(LogDisplayClusterEngine, Verbose, "UDisplayClusterGameEngine::Tick, Tick() is not allowed");
...
{
	UE_LOGF(LogDisplayClusterEngine, Verbose, "UDisplayClusterGameEngine::UpdateTimeAndHandleMaxTickRate");
...
{
	UE_LOGF(LogDisplayClusterEngine,Display, "GameSyncChange event received.");
	...
	for (const TTuple<FString, TSet<FString>>& SyncObj : SyncMap)
	{
		...
		UE_LOGF(LogDisplayClusterEngine,Display, "    %ls -> %ls", *SyncObj.Key, *Join);
	}
}
...
void UDisplayClusterGameEngine::CheckGameStartBarrier()
{
	...
	else
	{
		if (!OutOfSync())
		{
			UE_LOGF(LogDisplayClusterEngine, Display, "CheckGameStartBarrier - we are no longer out of sync. Restoring Play.");
		...
		{
			UE_LOGF(LogDisplayClusterEngine, Display, "CheckGameStartBarrier - we are out of sync. Pausing Play.");
...
void UDisplayClusterGameEngine::ModifyCVars()
{
	// Always call Present() from the RHI thread to prevent the submission thread from deadlocking.
	// Some synchronization policies wait for frame completion. In other words, they use GPU fences
	// for synchronization during Present() call. This can't be done on the submission thread as
	// it would deadlock itself.
	if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.D3D12.PresentOnSubmissionThread")))
	{
		CVar->Set(false);
	}
}
