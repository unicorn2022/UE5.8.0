...
bool UDisplayClusterStageGeometryComponent::GetStageDistanceAndNormal(const FVector& InDirection, float& OutDistance, FVector& OutNormal)
{
	if (bGeometryMapLoaded)
	{
		...
		const FVector ViewPos = GeometryMap.ViewMatrices.GetWorldToView().TransformVector(InDirection);
		...
		const FVector4 ScreenPos = GeometryMap.ViewMatrices.GetViewToClip().TransformFVector4(FVector4(ProjectedViewPos, 1));
		...
		if (ScreenPos.W != 0.0)
		{
			...
			FVector4 DepthPos = GeometryMap.ViewMatrices.GetClipToView().TransformFVector4(FVector4(ScreenPos.X * InvW, ScreenPos.Y * InvW, Depth, 1.0f));
			DepthPos /= DepthPos.W;
