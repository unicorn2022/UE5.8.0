...
IDisplayClusterViewportManager* ADisplayClusterRootActor::GetOrCreateViewportManager()
{
	...
	{
		UE_LOGF(LogDisplayClusterGame, Log, "DCRA '%ls': Create viewport manager.", *GetName());
...
void ADisplayClusterRootActor::RemoveViewportManager()
{
	if (ViewportManagerPtr.IsValid())
	{
		UE_LOGF(LogDisplayClusterGame, Log, "DCRA '%ls': Remove viewport manager.", *GetName());
...
void ADisplayClusterRootActor::OverrideFromConfig(UDisplayClusterConfigurationData* ConfigData)
{
	...
	if (CurrentConfigData->Cluster)
	{
		...
		for (auto NewNodeIt = ConfigData->Cluster->Nodes.CreateConstIterator(); NewNodeIt; ++NewNodeIt)
		{
			...
			if (!CurrentNodePtr)
			{
				CurrentConfigData->Cluster->Nodes.Emplace(
					NewNodeIt->Key,
...
void ADisplayClusterRootActor::Tick(float DeltaSeconds)
{
	...
	if (OperationMode != EDisplayClusterOperationMode::Disabled && bIsPrimaryRootActor)
	{
		...
		if (InstancedStereoCVar)
		{
			...
			{
				UE_LOGF(LogDisplayClusterGame, Error, "Instanced stereo was requested. nDisplay doesn't support instanced stereo so far.");
...
void ADisplayClusterRootActor::TickPreviewRenderer()
{
	...
	if (IDisplayClusterViewportManager* ViewportManager = bEnablePreviewInScene ? GetOrCreateViewportManager() : GetViewportManager())
	{
		...
		if (ViewportManager->GetConfiguration().IsRootActorExclusivelyLocked())
		{
...
UDisplayClusterDisplayDeviceBaseComponent* ADisplayClusterRootActor::GetDefaultDisplayDevice() const
{
	...
	if (!DefaultDisplayDeviceName.IsNone() && DefaultDisplayDeviceName != GetInternalDisplayDeviceName())
	{
		...
		if (!DefaultDisplayDeviceComponent)
		{
			UE_LOGF(LogDisplayClusterGame, Warning, "Invalid default display device. Using internal nDisplay default device.");
		}
	}
...
bool ADisplayClusterRootActor::SetReplaceTextureFlagForAllViewports(bool bReplace)
{
	...
	if (!ConfigData)
	{
		UE_LOGF(LogDisplayClusterGame, Warning, "RootActor's ConfigData was null");
		return false;
	}

	if (!ConfigData->Cluster)
	{
		UE_LOGF(LogDisplayClusterGame, Warning, "ConfigData's Cluster was null");
		return false;
	}
	...
	else
	{
		...
		if (!Node)
		{
			UE_LOGF(LogDisplayClusterGame, Warning, "NodeId '%ls' not found in ConfigData", *NodeId);
			return false;
		}
...
bool ADisplayClusterRootActor::SetFreezeOuterViewports(bool bEnable)
{
	...
	if (!ConfigData)
	{
		UE_LOGF(LogDisplayClusterGame, Warning, "ADisplayClusterRootActor::SetFreezeOuterViewports failed because ConfigData was null");
		return false;
	}
...
void ADisplayClusterRootActor::RemoveRootActorExpiredLockOwners()
{
	for (auto LockOwnerIt = RootActorExclusiveLockOwners.CreateIterator(); LockOwnerIt; ++LockOwnerIt)
	{
		if (!LockOwnerIt->IsValid()
		 || !LockOwnerIt->Pin().IsValid()
		 || !LockOwnerIt->Pin().Get()->GetConfiguration().IsRootActorLockedByThis(this))
		{
			LockOwnerIt.RemoveCurrent();
		}
	}
}

void ADisplayClusterRootActor::AddRootActorExclusiveLockOwner(const TSharedRef<IDisplayClusterViewportManager, ESPMode::ThreadSafe> InOwner)
{
	FScopeLock Lock(&RootActorExclusiveLockOwnersMutex);
	RootActorExclusiveLockOwners.Add(InOwner);
	RemoveRootActorExpiredLockOwners();
}

void ADisplayClusterRootActor::RemoveRootActorExclusiveLockOwner(const TSharedRef<IDisplayClusterViewportManager, ESPMode::ThreadSafe> InOwner)
{
	FScopeLock Lock(&RootActorExclusiveLockOwnersMutex);
	RootActorExclusiveLockOwners.Remove(InOwner);
	RemoveRootActorExpiredLockOwners();
}

bool ADisplayClusterRootActor::IsRootActorExclusivelyLocked() const
{
	FScopeLock Lock(&RootActorExclusiveLockOwnersMutex);

	if (RootActorExclusiveLockOwners.IsEmpty())
	{
		return false;
	}

	for(const TWeakPtr<IDisplayClusterViewportManager, ESPMode::ThreadSafe>& LockOwnerIt : RootActorExclusiveLockOwners)
	{
		if (LockOwnerIt.IsValid()
		 && LockOwnerIt.Pin().IsValid()
		 && LockOwnerIt.Pin().Get()->GetConfiguration().IsRootActorLockedByThis(this))
