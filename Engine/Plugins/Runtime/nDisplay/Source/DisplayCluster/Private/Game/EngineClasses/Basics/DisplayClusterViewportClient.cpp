...
{
public:
	FDisplayClusterSceneViewport(const TSharedPtr<SViewport>& InViewportWidget)
		: FSceneViewport(InViewportWidget)
	{}

	static TSharedRef<FDisplayClusterSceneViewport> Create(UDisplayClusterViewportClient* InViewportClient, const TSharedPtr<SViewport>& InViewportWidget)
	{
		return FSceneViewport::Create<FDisplayClusterSceneViewport>(InViewportClient, InViewportWidget);
	}
...
TSharedRef<FSceneViewport> UDisplayClusterViewportClient::CreateViewport(TSharedPtr<SViewport> InViewportWidget)
{
	return FDisplayClusterSceneViewport::Create(this, InViewportWidget);
}
...
void UDisplayClusterViewportClient::Draw(FViewport* InViewport, FCanvas* SceneCanvas)
{
	...
	if (DebugCanvas)
	{
		...
		const bool bGuiPropagationActive = FDisplayClusterGuiLayerController::Get().IsEnabled();
		...
		for (FDisplayClusterRenderFrameTargetViewFamily& DCViewFamily : DCRenderTarget.ViewFamilies)
		{
			...
			for (FDisplayClusterRenderFrameTargetView& DCView : DCViewFamily.Views)
			{
				...
				if (View)
				{
					...
					IStreamingManager::Get().AddViewInformation(View->ViewMatrices.GetViewOrigin(), View->UnscaledViewRect.Width(), View->UnscaledViewRect.Width() * View->ViewMatrices.GetViewToClip().M[0][0], StreamingScale);
