		...
		auto AddKeyWithInstancedObject = [&](const FString* Key, UObject* ArchetypeToUse)
			{
				...
				else if (GIsTransacting)
				{
					...
					if (UDisplayClusterConfigurationViewport* InstanceViewport = Cast<UDisplayClusterConfigurationViewport>(ObjectToAdd))
					{
						...
						{
							UE_LOGF(LogDisplayClusterBlueprint, Warning, "Projection Policy mismatch from archetype on viewport, correcting. Instance: %ls, Archetype: %ls.",
...
#endif
						UE_LOGF(LogDisplayClusterBlueprint, Warning, "Archetype mismatch on nDisplay config %ls. Make sure the config is compiled and saved. Property: %ls, Instance: %ls, Archetype: %ls, Default: %ls.",
...
void ADisplayClusterRootActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	bool bResetInnerFrustumPriority = false;
	bool bResetEntireClusterPreviewRendering = false;
	...
	{
		bResetEntireClusterPreviewRendering = true;

		bReinitializeActor = false;
	}
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(ADisplayClusterRootActor, InnerFrustumPriority))
	{
		bResetInnerFrustumPriority = true;
	}

	// Changes inside CurrentConfigData never alter DCRA component topology, so we always skip construction scripts for them,
	// independent of the Blueprint's bRunConstructionScriptOnInteractiveChange flag. We still need to rebuild the nDisplay
	// hierarchy afterwards (handled below via bReinitializeActor).
	const bool bChangeInCurrentConfigData = PropertyName == GET_MEMBER_NAME_CHECKED(ADisplayClusterRootActor, CurrentConfigData);

	// For non-config interactive edits (e.g. dragging a slider), skipping construction scripts is opt-in per Blueprint.
	bool bCanSkipForInteractive = false;
	if (PropertyChangedEvent.ChangeType == EPropertyChangeType::Interactive)
	{
		...
		{
			bCanSkipForInteractive = !Blueprint->bRunConstructionScriptOnInteractiveChange;
		}
	}

	if (!CurrentTransactionAnnotation.IsValid() && (bChangeInCurrentConfigData || bCanSkipForInteractive))
	{
	...
	else
	{
		...
		// InitializeRootActor() already called in PostEditChangeProperty()->RerunConstructionScripts()->RerunConstructionScripts_Editor()
		bReinitializeActor = false;
	}

	if (bResetEntireClusterPreviewRendering)
	{
		ResetEntireClusterPreviewRendering();
	}

	if (bResetInnerFrustumPriority)
	{
		ResetInnerFrustumPriority();
	}
