	...
	if (GIsEditor)
	{
		...
		for (UClass* ChildClass : ChildClasses)
		{
			...
			if (!ChildDCBP->ConfigExport.Equals(OriginalChildConfigExport))
			{
				...
				{
					UE_LOGF(LogDisplayClusterBlueprint, Display,
						"ConfigExport of the child nDisplay blueprint actor '%ls' is not up to date in the asset, so the package was marked as dirty and should be re-saved.",
...
void UDisplayClusterBlueprint::PostLoad()
{
	...
	if (GIsEditor)
	{
		...
		if (!ConfigExport.Equals(LoadedConfigExport))
		{
			...
			{			
				UE_LOGF(LogDisplayClusterBlueprint, Display,
					"ConfigExport of the nDisplay actor '%ls' was not up to date in the asset, so the package was marked as dirty and should be re-saved.",
