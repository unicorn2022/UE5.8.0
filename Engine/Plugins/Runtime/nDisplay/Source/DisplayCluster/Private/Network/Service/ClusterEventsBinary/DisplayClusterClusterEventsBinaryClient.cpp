...
EDisplayClusterCommResult FDisplayClusterClusterEventsBinaryClient::EmitClusterEventBinary(const FDisplayClusterClusterEventBinary& Event)
{
	...
	if (!Request)
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "Couldn't convert binary cluster event data to net packet");
	...
	if (!bResult)
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "Couldn't send binary cluster event");
