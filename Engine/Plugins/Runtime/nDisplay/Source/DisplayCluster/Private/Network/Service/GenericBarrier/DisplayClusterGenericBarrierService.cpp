...
{
	return DisplayClusterGenericBarrierStrings::ProtocolName;
}
...
TSharedPtr<FDisplayClusterPacketInternal> FDisplayClusterGenericBarrierService::ProcessPacket(const TSharedPtr<FDisplayClusterPacketInternal>& Request, const FDisplayClusterSessionInfo& SessionInfo)
{
	...
	if (!Request->GetProtocol().Equals(DisplayClusterGenericBarrierStrings::ProtocolName, ESearchCase::IgnoreCase) ||
		!Request->GetType().Equals(DisplayClusterGenericBarrierStrings::TypeRequest, ESearchCase::IgnoreCase))
	{
...
EDisplayClusterCommResult FDisplayClusterGenericBarrierService::CreateBarrier(const FString& BarrierId, const TMap<FString, TSet<FString>>& NodeToSyncCallers, uint32 Timeout, EBarrierControlResult& Result)
{
	...
	{
		UE_LOGF(LogDisplayClusterBarrierGP, Warning, "%ls - CreateBarrier - invalid request data", *GetName());
...

	{
		...
		{
			UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - CreateBarrier - Barrier '%ls' already exists", *GetName(), *BarrierId);
		...
		UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - CreateBarrier - Barrier '%ls' created successfully", *GetName(), *BarrierId);
...
EDisplayClusterCommResult FDisplayClusterGenericBarrierService::WaitUntilBarrierIsCreated(const FString& BarrierId, EBarrierControlResult& Result)
{
	...
	{
		UE_LOGF(LogDisplayClusterBarrierGP, Warning, "%ls - WaitUntilBarrierIsCreated - invalid request data", *GetName());
		return EDisplayClusterCommResult::WrongRequestData;
	}
...

	{
		...
		if (Barriers.Contains(BarrierId))
		{
			UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - WaitUntilBarrierIsCreated - Barrier '%ls' already exists", *GetName(), *BarrierId);
			Result = EBarrierControlResult::AlreadyExists;
			return EDisplayClusterCommResult::Ok;
		}
		...
		{
			UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - WaitUntilBarrierIsCreated - Barrier '%ls' not found. Waiting until it's created.", *GetName(), *BarrierId);
...
EDisplayClusterCommResult FDisplayClusterGenericBarrierService::IsBarrierAvailable(const FString& BarrierId, EBarrierControlResult& Result)
{
	...
	if (BarrierId.IsEmpty())
	{
		UE_LOGF(LogDisplayClusterBarrierGP, Warning, "%ls - IsBarrierAvailable - invalid request data", *GetName());
		return EDisplayClusterCommResult::WrongRequestData;
	}

	{
		...
		UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - IsBarrierAvailable - Barrier '%ls' is %ls", *GetName(), *BarrierId, Barriers.Contains(BarrierId) ? TEXT("available") : TEXT("not available"));
	}
...
EDisplayClusterCommResult FDisplayClusterGenericBarrierService::ReleaseBarrier(const FString& BarrierId, EBarrierControlResult& Result)
{
	...
	if (BarrierId.IsEmpty())
	{
		UE_LOGF(LogDisplayClusterBarrierGP, Warning, "%ls - ReleaseBarrier - invalid request data", *GetName());
		return EDisplayClusterCommResult::WrongRequestData;
	}

	{
		...
		if (TSharedPtr<IDisplayClusterBarrier, ESPMode::ThreadSafe>* Barrier = Barriers.Find(BarrierId))
		{
			...
			UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - ReleaseBarrier - Barrier '%ls' has been released", *GetName(), *BarrierId);
		...
		{
			// Not exists
			UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - ReleaseBarrier - Barrier '%ls' not found", *GetName(), *BarrierId);
...
EDisplayClusterCommResult FDisplayClusterGenericBarrierService::SyncOnBarrier(const FString& BarrierId, const FString& CallerId, EBarrierControlResult& Result)
{
	...
	{
		UE_LOGF(LogDisplayClusterBarrierGP, Warning, "%ls - SyncOnBarrier - invalid request data", *GetName());
		Result = EBarrierControlResult::UnknownError;
		return EDisplayClusterCommResult::WrongRequestData;
	}
	...
	// Sync on the barrier
	UE_LOGF(LogDisplayClusterBarrierGP, VeryVerbose, "%ls - SyncOnBarrier - Barrier '%ls' wait start", *GetName(), *BarrierId);
	(*Barrier)->Wait(CallerId);
	UE_LOGF(LogDisplayClusterBarrierGP, VeryVerbose, "%ls - SyncOnBarrier - Barrier '%ls' wait end", *GetName(), *BarrierId);
...
EDisplayClusterCommResult FDisplayClusterGenericBarrierService::SyncOnBarrierWithData(const FString& BarrierId, const FString& CallerId, const TArray<uint8>& RequestData, TArray<uint8>& OutResponseData, EBarrierControlResult& Result)
{
	...
	{
		// Barrier not found
		UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - SyncOnBarrierWithData - Barrier '%ls' not found", *GetName(), *BarrierId);
		Result = EBarrierControlResult::NotFound;
		return EDisplayClusterCommResult::WrongRequestData;
	}
	else if (!(*Barrier)->IsActivated())
	{
		UE_LOGF(LogDisplayClusterBarrierGP, Verbose, "%ls - SyncOnBarrierWithData - Barrier '%ls' is not active", *GetName(), *BarrierId);
		Result = EBarrierControlResult::UnknownError;
		return EDisplayClusterCommResult::NotAllowed;
	}

	// Sync on the barrier
	UE_LOGF(LogDisplayClusterBarrierGP, VeryVerbose, "%ls - SyncOnBarrierWithData - Barrier '%ls' wait start", *GetName(), *BarrierId);
	(*Barrier)->WaitWithData(CallerId, RequestData, OutResponseData);
	UE_LOGF(LogDisplayClusterBarrierGP, VeryVerbose, "%ls - SyncOnBarrierWithData - Barrier '%ls' wait end", *GetName(), *BarrierId);
