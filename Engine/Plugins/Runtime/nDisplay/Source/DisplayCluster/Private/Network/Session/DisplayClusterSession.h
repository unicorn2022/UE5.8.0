...
protected:
	...
	{
		UE_LOGF(LogDisplayClusterNetwork, Log, "Session thread %ls has started", *SessionInfo.SessionName);
		...
		while (FDisplayClusterSocketOperations::IsOpen())
		{
			...
			if (!Request)
			{
				UE_LOGF(LogDisplayClusterNetwork, Warning, "Session %ls: couldn't receive a request packet", *SessionInfo.SessionName);
				break;
			}
			...
			if (!bResult)
			{
				UE_LOGF(LogDisplayClusterNetwork, Warning, "Session %ls: couldn't send a response packet", *SessionInfo.SessionName);
				break;
			}

			UE_LOGF(LogDisplayClusterNetwork, Verbose, "Session %ls has processed a packet", *SessionInfo.SessionName);
		}

		// Set session end time
		SessionInfo.TimeEnd = FPlatformTime::Seconds();

		UE_LOGF(LogDisplayClusterNetwork, Log, "Session thread %ls has finished", *SessionInfo.SessionName);
