...
bool FDisplayClusterServer::Start(const FString& Address, const uint16 Port)
{
	...
	UE_LOGF(LogDisplayClusterNetwork, Log, "%ls starting...", *InstanceName);
	...
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "%ls couldn't start connection listener [%ls:%d]", *InstanceName, *Address, Port);
		return false;
	}
...
bool FDisplayClusterServer::ConnectionHandler(FDisplayClusterSessionInfo& SessionInfo)
{
	...
	{
		UE_LOGF(LogDisplayClusterNetwork, Verbose, "Connection %ls is allowed %ls",
	...
	{
		UE_LOGF(LogDisplayClusterNetwork, Log, "Connection request %ls has been rejected", *SessionInfo.ToString());
	}
...
void FDisplayClusterServer::ProcessSessionOpened(const FDisplayClusterSessionInfo& SessionInfo)
{
	...
	if (IsRunning())
	{
		UE_LOGF(LogDisplayClusterNetwork, Verbose, "Session start: %ls", *SessionInfo.SessionName);
...
void FDisplayClusterServer::ProcessSessionClosed(const FDisplayClusterSessionInfo& SessionInfo)
{
	...
	if (IsRunning())
	{
		UE_LOGF(LogDisplayClusterNetwork, Verbose, "Session end: %ls", *SessionInfo.SessionName);
...
void FDisplayClusterServer::ShutdownImpl()
{
...

	{
		...
		UE_LOGF(LogDisplayClusterNetwork, Log, "%ls stopping...", *InstanceName);
...
void FDisplayClusterServer::CleanPendingKillSessions()
{
	...
	for (auto SessionIt = PendingKillSessions.CreateIterator(); SessionIt; ++SessionIt)
	{
		...
		{
			UE_LOGF(LogDisplayClusterNetwork, Verbose, "%ls - cleaning session resources, SessionId=%llu...", *InstanceName, SessionIt->Key);
