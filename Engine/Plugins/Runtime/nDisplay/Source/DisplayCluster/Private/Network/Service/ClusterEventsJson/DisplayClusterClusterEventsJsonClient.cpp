...
EDisplayClusterCommResult FDisplayClusterClusterEventsJsonClient::EmitClusterEventJson(const FDisplayClusterClusterEventJson& Event)
{
	...
	if (!Request)
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "Couldn't convert json cluster event data to net packet");
	...
	if (!bResult)
	{
		UE_LOGF(LogDisplayClusterNetwork, Error, "Couldn't send json cluster event");
