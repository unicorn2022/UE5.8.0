...
{
	UE_LOGF(LogDisplayClusterBarrier, Log, "Initialized barrier '%ls' with timeout %u ms and threads limit: %u", *Name, Timeout, CallersAllowed.Num());

	int32 Idx = 0;
	for (const FString& CallerId : CallersAllowed)
	{
		UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': client (%d): '%ls'", *Name, Idx++, *CallerId);
	}
...
bool FDisplayClusterBarrier::Activate()
{
	...
	UE_LOGF(LogDisplayClusterBarrier, Log, "Barrier '%ls': activating...", *Name);
...
void FDisplayClusterBarrier::Deactivate()
{
	...
	if (bActive)
	{
		UE_LOGF(LogDisplayClusterBarrier, Log, "Barrier '%ls': deactivating...", *Name);
...
{
	UE_LOGF(LogDisplayClusterBarrier, VeryVerbose, "Barrier '%ls': caller arrived '%ls'", *Name, *CallerId);
...

	{
...

		{
			...
			{
				UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': caller '%ls' not allowed to join, it has been timed out previously", *Name, *CallerId);
			...
			if (!bActive)
			{
				UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': not active", *Name);
			...
			{
				UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': caller '%ls' not allowed to join, no permission", *Name, *CallerId);
			...
			if (bActive)
			{
				...
				{
					UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': sync start, cycle %llu", *Name, SyncCycleCounter);
				...
				UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': awaiting threads amount - %d", *Name, CallersAwaiting.Num());
				...
				if (CallersAwaiting.Num() == CallersAllowed.Num())
				{
					...
					UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': sync end, cycle %llu, barrier wait time %f", *Name, SyncCycleCounter, BarrierWaitTimeOverall);
	...
	{
		UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': caller '%ls' is leaving the barrier", *Name, *CallerId);
	...
	UE_LOGF(LogDisplayClusterBarrier, VeryVerbose, "Barrier '%ls': caller left '%ls'", *Name, *CallerId);
...
{
	UE_LOGF(LogDisplayClusterBarrier, Log, "Barrier '%ls': unregistering caller '%ls'...", *Name, *CallerId);
	...
	if (CallersAwaiting.Num() == CallersAllowed.Num())
	{
		...
		UE_LOGF(LogDisplayClusterBarrier, Verbose, "Barrier '%ls': sync end, barrier wait time %f", *Name, BarrierWaitTimeOverall);
...
void FDisplayClusterBarrier::HandleBarrierTimeout()
{
	...
	UE_LOGF(LogDisplayClusterBarrier, Log, "Barrier '%ls': Time out! %d callers missing", *Name, CallersAllowed.Num() - CallersAwaiting.Num());
	...
	{
		UE_LOGF(LogDisplayClusterBarrier, Log, "Barrier '%ls': caller '%ls' was moved to the 'TimedOut' list", *Name, *CallerId);
	}
	...
	UE_LOGF(LogDisplayClusterBarrier, Log, "Barrier '%ls': new threads limit %d", *Name, CallersAllowed.Num());
