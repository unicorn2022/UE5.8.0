	...
	ADisplayClusterRootActor* RootActor, const FName& BlueprintName, UObject* Package)
{
	...
	if (UObject* ExistingObject = FindObject<UObject>(Package, *BlueprintNameStr))
	{
		...
		UE_LOGF(DisplayClusterConfiguratorLog, Error, "Cannot create a new nDisplay blueprint with the name '%ls'. The file already exists. To re-import right click on the file and use the context option instead.",
			*BlueprintNameStr);
