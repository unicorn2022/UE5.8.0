...
TSharedPtr<IPropertyHandle> UE::DisplayClusterConfiguratorPropertyUtils::AddKeyValueToMap(uint8* MapOwner, TSharedPtr<IPropertyHandle> PropertyHandle, const FString& Key, const FString& Value, EPropertyValueSetFlags::Type SetFlags)
{
	...
	// Original approach — currently crashes consistently during RemoveKeyFromMap().
	// The root cause is most likely within the compact hashmap implementation,
	// but that is for another discussion.
	//
	// The algorithm was structured as follows:
	// 1. Remove the existing 'Key' entry (it will be recreated later anyway).
	// 2. Remove any existing empty key, since a new empty key will be introduced.
	// 3. Add a new key–value pair, which initially has an empty key
	//    (hence the need for step [2]).
	// 4. Locate the newly created entry from step [3].
	// ... set up key/value data for the entry
#if 0
...
#else
	// Revised approach - avoids calling RemoveKeyFromMap(), eliminating the crash.
	//
	// This version achieves the same result via a different flow:
	// 1. Look up the existing 'Key' entry.
	// 2. If it does not exist, create a new entry with an empty key.
	// 3. Locate the resulting entry (either the existing one or the newly created one)
	// ... set up key/value data for the entry
	void* MapContainer = MapProperty->ContainerPtrToValuePtr<void*>(MapOwner);
	FScriptMapHelper MapHelper(MapProperty, MapContainer);
	
	int32 IndexToModify = FindMapHandleIndexFromKey(MapHandle, MapHelper, Key);
	if (IndexToModify == INDEX_NONE)
	{
		const FString DefaultKey = TEXT("");
		IndexToModify = FindMapHandleIndexFromKey(MapHandle, MapHelper, DefaultKey);
		if (IndexToModify == INDEX_NONE)
		{
			const FPropertyAccess::Result Result = MapHandle->AddItem();
			check(Result == FPropertyAccess::Success);
