...
void FDisplayClusterConfiguratorProjectionCustomization::BuildParametersForPolicy(const FString& Policy, IDetailChildrenBuilder& InChildBuilder)
{
	...
	{
		UE_LOGF(DisplayClusterConfiguratorLog, Warning, "Details policy selection blueprint invalid.");
		return;
	}
	 ...
	 * Camera
	 */
	if (PolicyLower == DisplayClusterProjectionStrings::projection::Camera)
	{
...
{
	TSharedPtr<FPolicyParameterInfoViewportCombo> ViewportCombo = MakeShared<FPolicyParameterInfoViewportCombo>(
		"Source Viewport",
		DisplayClusterProjectionStrings::cfg::reference::ViewportId,
		Blueprint,
		ConfigurationViewports);

	ViewportCombo->SetParameterTooltip(LOCTEXT("ReferencePolicySourceViewportTooltip",
		"The viewport to reference for this projection. Only viewports within the same cluster node can be referenced. "
		"This option is disabled when editing viewports from different cluster nodes."));

	CustomPolicyParameters.Add(ViewportCombo);
}
