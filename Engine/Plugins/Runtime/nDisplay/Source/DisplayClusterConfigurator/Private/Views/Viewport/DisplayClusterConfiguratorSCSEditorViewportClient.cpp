...
FDisplayClusterConfiguratorSCSEditorViewportClient::FDisplayClusterConfiguratorSCSEditorViewportClient(
	TWeakPtr<FBlueprintEditor>& InBlueprintEditorPtr,
	FPreviewScene* InPreviewScene,
	const TSharedRef<SDisplayClusterConfiguratorSCSEditorViewport>& InSCSEditorViewport
)
	: FEditorViewportClient(
		InBlueprintEditorPtr.Pin() ? &StaticCastSharedPtr<FDisplayClusterConfiguratorBlueprintEditor>(InBlueprintEditorPtr.Pin())->GetModeTools().Get() : nullptr,
		InPreviewScene,
		StaticCastSharedRef<SEditorViewport>(InSCSEditorViewport))
...
bool FDisplayClusterConfiguratorSCSEditorViewportClient::OnTransformCanceled(const FGizmoState& InState)
{
	return EndTransform(InState);
}
...
{
	//UE_LOGF(LogSCSEditorViewport, Log, "FDisplayClusterConfiguratorSCSEditorViewportClient::BeginTransaction() pre: %ls %08x", SessionName, *((uint32*)&ScopedTransaction));
	...
	//UE_LOGF(LogSCSEditorViewport, Log, "FDisplayClusterConfiguratorSCSEditorViewportClient::BeginTransaction() post: %ls %08x", SessionName, *((uint32*)&ScopedTransaction));
}

void FDisplayClusterConfiguratorSCSEditorViewportClient::EndTransaction()
{
	//UE_LOGF(LogSCSEditorViewport, Log, "FDisplayClusterConfiguratorSCSEditorViewportClient::EndTransaction(): %08x", *((uint32*)&ScopedTransaction));
