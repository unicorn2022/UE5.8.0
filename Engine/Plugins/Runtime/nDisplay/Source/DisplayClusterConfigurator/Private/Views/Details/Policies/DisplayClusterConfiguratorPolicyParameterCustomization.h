...
/**
 * Policy info for combo box picker for viewports (for Reference projection policy).
 */
class FPolicyParameterInfoViewportCombo final : public FPolicyParameterInfoCombo
{
public:
	FPolicyParameterInfoViewportCombo(
		const FString& InDisplayName,
		const FString& InKey,
		UDisplayClusterBlueprint* InBlueprint,
		const TArray<TWeakObjectPtr<UDisplayClusterConfigurationViewport>>& InConfigurationViewports);
	...
	/** Populate CustomParameterOptions with all viewports except the currently edited ones. */
	void CreateParameterValues();

private:
	/** Check if all edited viewports belong to the same cluster node. */
	bool AreAllViewportsInSameClusterNode() const;
};
