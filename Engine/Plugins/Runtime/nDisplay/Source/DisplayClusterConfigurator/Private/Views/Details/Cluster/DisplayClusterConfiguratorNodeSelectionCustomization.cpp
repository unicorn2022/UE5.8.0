...
void FDisplayClusterConfiguratorNodeSelection::ResetOptions()
{
	...
	if (OperationMode == ICVFXCameras)
	{
		// Get ICVFX camera components from the root actor
		if (ADisplayClusterRootActor* RootActor = GetRootActor())
		{
			TArray<UActorComponent*> CameraComponents;
			RootActor->GetComponents(UDisplayClusterICVFXCameraComponent::StaticClass(), CameraComponents);

			for (UActorComponent* Component : CameraComponents)
			{
				if (Component->IsA<UDisplayClusterICVFXCameraComponent>())
				{
					Options.Add(MakeShared<FString>(Component->GetName()));
				}
			}
		}
	}
	else if (UDisplayClusterConfigurationData* ConfigData = GetConfigData())
	{
