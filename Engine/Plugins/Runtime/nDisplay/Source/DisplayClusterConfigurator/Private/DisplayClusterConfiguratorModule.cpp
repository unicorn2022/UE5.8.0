...
#include "Views/Details/Cluster/DisplayClusterConfiguratorCameraOrderCustomization.h"
...
{
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FDisplayClusterConfiguratorModule::OnPostEngineInit);
...
void FDisplayClusterConfiguratorModule::UnregisterSettings()
{
	if (ISettingsModule* SettingsModule = FModuleManager::GetModulePtr<ISettingsModule>("Settings"))
	{
		SettingsModule->UnregisterSettings("Editor", "Plugins", "nDisplayEditor");
	}
}
...
void FDisplayClusterConfiguratorModule::RegisterSectionMappings()
{
	...
	{
		const TSharedRef<FPropertySection> Section = PropertyModule.FindOrCreateSection(ADisplayClusterRootActor::StaticClass()->GetFName(),
			DisplayClusterConfigurationStrings::categories::ExposureCategory, LOCTEXT("Exposure", "Exposure"));
		Section->AddCategory(DisplayClusterConfigurationStrings::categories::ExposureCategory);
	}
	{
		const TSharedRef<FPropertySection> Section = PropertyModule.FindOrCreateSection(ADisplayClusterRootActor::StaticClass()->GetFName(),
...
void FDisplayClusterConfiguratorModule::UnregisterSectionMappings()
{
	if (FModuleManager::Get().IsModuleLoaded("PropertyEditor") && FSlateApplication::IsInitialized())
	{
		...
		PropertyModule.RemoveSection(ADisplayClusterRootActor::StaticClass()->GetFName(), DisplayClusterConfigurationStrings::categories::ExposureCategory);
