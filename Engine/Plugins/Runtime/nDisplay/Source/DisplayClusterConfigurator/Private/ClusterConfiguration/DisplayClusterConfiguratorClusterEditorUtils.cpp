...
void UE::DisplayClusterConfiguratorClusterEditorUtils::ShowNewClusterItemDialogWindow(TSharedRef<SDisplayClusterConfiguratorNewClusterItemDialog> DialogContent, TSharedPtr<SWidget> ParentElement, FText WindowTitle, FVector2D WindowSize)
{
	...
	// UE-378282
	// There appears to be an issue with modal dialog behavior on Linux.
	// The dialog may appear in the background and not behave as expected.
	// As a workaround, use a simpler window initialization path that does
	// not explicitly request window position or size.
#if 1
