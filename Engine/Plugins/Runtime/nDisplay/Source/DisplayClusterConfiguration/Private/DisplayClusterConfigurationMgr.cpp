...
UDisplayClusterConfigurationData* FDisplayClusterConfigurationMgr::LoadConfig(const FString& FilePath, UObject* Owner)
{
	...
	{
		UE_LOGF(LogDisplayClusterConfiguration, Error, "File not found: %ls", *ConfigFile);
		return nullptr;
	}
	...
	default:
		UE_LOGF(LogDisplayClusterConfiguration, Error, "No parser implemented for file: %ls", *ConfigFile);
		return nullptr;
	}

	// Load configuration data
	UDisplayClusterConfigurationData* Config = Parser->LoadData(ConfigFile);

	// Apply any upgrades if necessary
	if (IsValid(Config))
	{
		FDisplayClusterConfigurationUtils::UpdateToLatest(Config);
	}
