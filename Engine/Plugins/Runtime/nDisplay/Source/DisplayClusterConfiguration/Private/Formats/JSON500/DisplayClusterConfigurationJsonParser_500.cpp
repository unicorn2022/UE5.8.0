...
namespace JSON500
{
	UDisplayClusterConfigurationData* FDisplayClusterConfigurationJsonParser::LoadData(const FString& FilePath, UObject* Owner)
	{
		...
		UE_LOGF(LogDisplayClusterConfiguration, Log, "nDisplay json configuration: %ls", *JsonText);
	...
	bool FDisplayClusterConfigurationJsonParser::SaveData(const UDisplayClusterConfigurationData* ConfigData, const FString& FilePath)
	{
		...
		{
			UE_LOGF(LogDisplayClusterConfiguration, Error, "Couldn't save data to file: %ls", *FilePath);
			return false;
		}

		UE_LOGF(LogDisplayClusterConfiguration, Log, "Configuration data has been successfully saved to file: %ls", *FilePath);
	...
	bool FDisplayClusterConfigurationJsonParser::AsString(const UDisplayClusterConfigurationData* ConfigData, FString& OutString)
	{
		...
		{
			UE_LOGF(LogDisplayClusterConfiguration, Error, "Couldn't convert data to json data types");
			return false;
		}
		...
		{
			UE_LOGF(LogDisplayClusterConfiguration, Error, "Couldn't serialize data to json");
			return false;
		}
	...
	bool FDisplayClusterConfigurationJsonParser::ConvertDataToExternalTypes(const UDisplayClusterConfigurationData* Config)
	{
		...
		{
			UE_LOGF(LogDisplayClusterConfiguration, Error, "nullptr detected in the configuration data");
			return false;
		}
