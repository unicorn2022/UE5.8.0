...
FSceneView* UDisplayClusterMoviePipelineViewportPassBase::GetSceneViewForSampleState(FSceneViewFamily* ViewFamily, FMoviePipelineRenderPassMetrics& InOutSampleState, IViewCalcPayload* OptPayload)
{
	...
	if (DCRootActor && GetViewportId(InViewportIndex, ViewportId))
	{
		...
		if (HighResSettings->TileCount > 1 && bFrameWarpBlend)
		{
			...
			UE_LOGF(LogMovieRenderPipeline, Warning, "DCRA WarpBlend isn't supported when using tiling! (Viewport '%ls')", *ViewportId);
		}
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "DCRA render failed for viewport '%ls')", *ViewportId);
		}
	}
	else
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "DCRA render failed: undefined viewport index, skipped");
	}
...
void UDisplayClusterMoviePipelineViewportPassBase::ReleaseDisplayCluster()
{
	...
	if (DCRootActor)
	{
		if (IDisplayClusterViewportManager* ViewportManager = DCRootActor->GetViewportManager())
		{
			// Re-enable preview/PIE rendering for this DCRA.
			ViewportManager->GetConfiguration().SetExclusiveLockOnRootActors(false);
		}

		// Delete all used rendering resources
		// Note: FDisplayClusterViewportManager::Release() remove exclusive locks on release.
...
bool UDisplayClusterMoviePipelineViewportPassBase::InitializeDisplayCluster()
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Failed to find current world for nDisplay render");
	...
	if (UMoviePipelineExecutorShot* CurrentShot = GetPipeline()->GetActiveShotList()[GetPipeline()->GetCurrentShotIndex()])
	{
		if (UDisplayClusterMoviePipelineSettings* DCMoviePipelineSettings = GetPipeline()->FindOrAddSettingForShot<UDisplayClusterMoviePipelineSettings>(CurrentShot))
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Error, "Failed to find nDisplay RootActor");
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Error, "Failed to find nDisplay viewports");
			...
			{
				// Disable preview/PIE rendering for this DCRA.
				ViewportManager->GetConfiguration().SetExclusiveLockOnRootActors(true);
	...
	UE_LOGF(LogMovieRenderPipeline, Error, "Failed to initialize nDisplay render");
	ReleaseDisplayCluster();
...
void UDisplayClusterMoviePipelineViewportPass_PathTracer::SetupImpl(const MoviePipeline::FMoviePipelineRenderPassInitSettings& InPassInitSettings)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Cannot render a nDisplay Path Tracer pass, Path Tracer is not enabled by this project.");
