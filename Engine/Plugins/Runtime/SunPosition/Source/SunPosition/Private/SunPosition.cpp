...
//		TestCalculator();
	...
	{
		UE_LOGF(LogSunPosition, Warning, "invalid date");
		return;
	}
...
void TestExpectedValues(float Latitude, float Longitude, float TimeZone, bool bIsDaylightSavingTime, int32 Year, int32 Month, int32 Day, int32 Hours, int32 Minutes, int32 Seconds, double ExpectedAzimuth, double ExpectedElevation, double ExpectedCorrectedElevation)
{
	...
	{
		UE_LOGF(LogSunPosition, Warning, "Elevation error above threshold %0.2f%%", ElevError);
	}
	...
	{
		UE_LOGF(LogSunPosition, Warning, "Corrected elevation error above threshold %0.2f%%", CorrElevError);
	}
	...
	{
		UE_LOGF(LogSunPosition, Warning, "Azimuth error above threshold %0.2f%%", AzError);
	}
}
