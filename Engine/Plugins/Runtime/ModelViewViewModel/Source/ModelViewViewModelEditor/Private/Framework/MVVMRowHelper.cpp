...
{
struct FMVVMBindingClipboardData
{
	...
	/* 
	* When copy/pasting the following import objects are the target outer chain when importing condition and event objects.
	*/
	UWidgetBlueprint* ImportBlueprint = nullptr;
	UMVVMBlueprintView* ImportView = nullptr;
	UMVVMWidgetBlueprintExtension_View* ImportExtensionView = nullptr;
};
...
bool ExportClipboardData(UMVVMBlueprintView* BlueprintView, const TArray<TSharedPtr<FBindingEntry>>& Entries, FMVVMBindingClipboardData& OutClipboardData)
{
	...
	while (!EntriesToExport.IsEmpty())
	{
		...
		case FBindingEntry::ERowType::Binding:
		{
			...
			{
				FMVVMBindingClipboardData::Type Item;
...
bool ImportClipboardData(UMVVMBlueprintView* BlueprintView, FMVVMBindingClipboardData& OutClipboardData)
{
	class FClipboardObjectFactory : public FCustomizableTextObjectFactory
	{
	public:
		FClipboardObjectFactory(FMVVMBindingClipboardData& ClipboardData)
	...
	private:
		FMVVMBindingClipboardData& ClipboardData;
	};
	...
	OutClipboardData.ImportBlueprint = NewObject<UWidgetBlueprint>(); 
	OutClipboardData.ImportExtensionView = NewObject<UMVVMWidgetBlueprintExtension_View>(OutClipboardData.ImportBlueprint); 
	OutClipboardData.ImportView = NewObject<UMVVMBlueprintView>(OutClipboardData.ImportExtensionView ); 
	for (const FString& ItemText : Items)
	{
		...
		else
		{
			...
			Factory.ProcessBuffer(OutClipboardData.ImportView, RF_NoFlags, ItemText);
		}
	}
...
void FRowHelper::CopyEntries(UWidgetBlueprint* WidgetBlueprint, UMVVMBlueprintView* BlueprintView, const TArray<TSharedPtr<FBindingEntry>>& Entries)
{
	...
	for (FMVVMBindingClipboardData::Type Item : ClipboardData.Items)
	{
		bool bFailed = false;
...
void FRowHelper::PasteEntries(UWidgetBlueprint* WidgetBlueprint, UMVVMBlueprintView* BlueprintView, const TArray<TSharedPtr<FBindingEntry>>& Entries)
{
	...
	if (Entries.IsEmpty())
	{
		...
		for (const FMVVMBindingClipboardData::Type& Item : ClipboardData.Items)
		{
			if (Item.IsType<FMVVMBlueprintViewBinding>())
			{
				const FMVVMBlueprintViewBinding& Binding = Item.Get<FMVVMBlueprintViewBinding>();
	...
	const FMVVMBindingClipboardData::Type& ClipboardItem = ClipboardData.Items.Last();
	...
	if (WidgetBlueprint && View && Entries.Num() > 0)
	{
		...
		const UMVVMEditorSubsystem* EditorSubsystem = GEditor->GetEditorSubsystem<UMVVMEditorSubsystem>();
		{
			...
			if (bCanPasteEntry && Entries.Num() == 1) // Check paste type
			{
				...
				{
					const TSharedPtr<FBindingEntry> Entry = Entries.Last();
					const FMVVMBindingClipboardData::Type& Item = ClipboardData.Items.Last();
