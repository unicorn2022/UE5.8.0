			...
			if (ensure(CreateSetterFunctionHandle))
			{
				ChildBuilder.AddProperty(CreateSetterFunctionHandle.ToSharedRef())
					.IsEnabled(MakeAttributeLambda([bCanEdit, ContextPtr]()
						{
							bool bResult = ContextPtr->CreationType != EMVVMBlueprintViewModelContextCreationType::Manual && ContextPtr->CreationType != EMVVMBlueprintViewModelContextCreationType::Context;
...
void FBlueprintViewModelContextDetailCustomization::HandleCreationTypeChanged()
{
	...
	if (CreationTypeHandle->GetValue(NewValue) == FPropertyAccess::Success)
	{
		if (FMVVMBlueprintViewModelContext* ContextPtr = Private::GetViewModelContext(ContextHandle.ToSharedRef()))
		{
			...
			const bool bIsAlwaysOptional = CreationType == EMVVMBlueprintViewModelContextCreationType::Manual || CreationType == EMVVMBlueprintViewModelContextCreationType::Context;
			if (ContextPtr->bOptional != bIsAlwaysOptional)
			{
				OptionalHandle->SetValue(bIsAlwaysOptional);
			}
