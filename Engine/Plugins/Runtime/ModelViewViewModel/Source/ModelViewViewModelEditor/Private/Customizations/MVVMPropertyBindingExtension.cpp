...
struct FCreatePropertyBindingParams
{
	TWeakObjectPtr<const UWidgetBlueprint> WidgetBlueprint;
	TWeakObjectPtr<UMVVMBlueprintView> MVVMBlueprintView;
	TWeakObjectPtr<UWidget> Widget;
	TSharedPtr<IPropertyHandle> PropertyHandle;
};

void CreateBindingFromProperty(const FCreatePropertyBindingParams& Params, FMVVMLinkedPinValue LinkedValue, const FMVVMFieldConversion& Conversion)
{
	UWidgetBlueprint* WidgetBlueprint = const_cast<UWidgetBlueprint*>(Params.WidgetBlueprint.Get());
	UMVVMBlueprintView* MVVMBlueprintView = Params.MVVMBlueprintView.Get();
	UMVVMEditorSubsystem* Subsystem = GEditor->GetEditorSubsystem<UMVVMEditorSubsystem>();
	UWidget* Widget = Params.Widget.Get();
	if (WidgetBlueprint == nullptr || MVVMBlueprintView == nullptr || Widget == nullptr || Subsystem == nullptr)
	{
		return;
	}
	...
	FMVVMBlueprintViewBinding* Binding = MVVMBlueprintView->FindBinding(Widget, Params.PropertyHandle.Get()->GetProperty());
	if (Binding == nullptr)
	{
	...
	{
		const FMVVMBlueprintPropertyPath& SourcePath = LinkedValue.GetPropertyPath();
		Binding->SourcePath = SourcePath;

		// Generate the destination path from the widget property that we are dropping on.
		FCachedPropertyPath CachedPropertyPath(Params.PropertyHandle->GeneratePathToProperty());
		...
		{
			Subsystem->SetSourceToDestinationConversionFunction(WidgetBlueprint, *Binding, Conversion.ToBlueprintFunctionReference(WidgetBlueprint));
			Subsystem->SetPathForConversionFunctionArgument(WidgetBlueprint, *Binding, Conversion.ParameterId, SourcePath, true);
		}
	}
	else
	{
...
TSharedRef<SWidget> FMVVMPropertyBindingExtension::OnGenerateMenu(const UWidgetBlueprint* WidgetBlueprint, UWidget* Widget, TSharedPtr<IPropertyHandle> PropertyHandle, TWeakPtr<IDetailsView> DetailsView)
{
	...
	Private::FCreatePropertyBindingParams CreateBindingParams;
	CreateBindingParams.WidgetBlueprint = TWeakObjectPtr<const UWidgetBlueprint>(WidgetBlueprint);
	CreateBindingParams.MVVMBlueprintView = TWeakObjectPtr<UMVVMBlueprintView>(MVVMBlueprintView);
	CreateBindingParams.Widget = TWeakObjectPtr<UWidget>(Widget);
	CreateBindingParams.PropertyHandle = PropertyHandle;

	SFieldSelectorMenu::FOnLinkedValueConversionSelected HandleFieldConversionSelectionChanged = SFieldSelectorMenu::FOnLinkedValueConversionSelected::CreateLambda(
		[CreateBindingParams, DetailsView]
		(FMVVMLinkedPinValue LinkedValue, const FMVVMFieldConversion& Conversion, SFieldSelectorMenu::ESelectionType SelectionType)
		{
			FSlateApplication::Get().DismissAllMenus();

			Private::CreateBindingFromProperty(CreateBindingParams, LinkedValue, Conversion);
	...
	SFieldSelectorMenu::FOnLinkedValueSelected HandleFieldSelectionChanged = SFieldSelectorMenu::FOnLinkedValueSelected::CreateLambda(
		[HandleFieldConversionSelectionChanged]
		(FMVVMLinkedPinValue LinkedValue, const SFieldSelectorMenu::ESelectionType SelectionType)
		{
			HandleFieldConversionSelectionChanged.ExecuteIfBound(LinkedValue, FMVVMFieldConversion(), SelectionType);
		});
	...
	TSharedRef<SFieldSelectorMenu> Menu = SNew(SFieldSelectorMenu, WidgetBlueprint)
		...
		.OnSelectedConversion(HandleFieldConversionSelectionChanged)
