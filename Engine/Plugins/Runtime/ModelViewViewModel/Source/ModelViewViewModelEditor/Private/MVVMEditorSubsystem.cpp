...
FGuid UMVVMEditorSubsystem::AddViewModel(UWidgetBlueprint* WidgetBlueprint, const UClass* ViewModelClass)
{
	...
	if (ViewModelClass && ViewModelClass->ImplementsInterface(UNotifyFieldValueChanged::StaticClass()))
	{
		if (UMVVMBlueprintView* View = GetView(WidgetBlueprint))
		{
			...
			if (Context.IsValid())
			{
				Context.Resolver = Context.CreateDefaultResolver(WidgetBlueprint);
...
void UMVVMEditorSubsystem::SetSourceToDestinationConversionFunction(UWidgetBlueprint* WidgetBlueprint, FMVVMBlueprintViewBinding& Binding, FMVVMBlueprintFunctionReference NewConversionFunction)
{
	if (UMVVMBlueprintView* View = GetView(WidgetBlueprint))
	{
		if (NewConversionFunction.GetType() == EMVVMBlueprintFunctionReferenceType::Function)
		{
			...
			// The new source path will be the conversion function, only validate whether the conversion's return type matches the destination
			if (!IsValidConversionFunction(WidgetBlueprint, NewFunction, FMVVMBlueprintPropertyPath(), Binding.DestinationPath))
			{
...
void UMVVMEditorSubsystem::SetDestinationToSourceConversionFunction(UWidgetBlueprint* WidgetBlueprint, FMVVMBlueprintViewBinding& Binding, FMVVMBlueprintFunctionReference NewConversionFunction)
{
	if (UMVVMBlueprintView* View = GetView(WidgetBlueprint))
	{
		...
		{
			const UFunction* NewFunction = NewConversionFunction.GetFunction(WidgetBlueprint);
			// The new destination path will be the conversion function, only validate whether the conversion's return type matches the destination
			if (!IsValidConversionFunction(WidgetBlueprint, NewFunction, Binding.SourcePath, FMVVMBlueprintPropertyPath()))
			{
				NewConversionFunction = FMVVMBlueprintFunctionReference();
			}
		}
...
TArray<FMVVMBlueprintPinId> UMVVMEditorSubsystem::GetConversionFunctionPins(const UWidgetBlueprint* WidgetBlueprint, const UE::MVVM::FConversionFunctionValue& Function, const FProperty* ExpectedArgumentType) const
{
	TArray<FMVVMBlueprintPinId> OutPins;
	if (Function.IsFunction())
	{
		TValueOrError<TArray<const FProperty*>, FText> ArgumentsResult = UE::MVVM::BindingHelper::TryGetArgumentsForConversionFunction(Function.GetFunction());
		if (ArgumentsResult.HasError())
		{
			return OutPins;
		}
