...
public:
	...
	DECLARE_DELEGATE_ThreeParams(FOnLinkedValueConversionSelected, FMVVMLinkedPinValue, const FMVVMFieldConversion&, ESelectionType);
	...
	SLATE_BEGIN_ARGS(SFieldSelectorMenu){}
		...
		SLATE_EVENT(FOnLinkedValueConversionSelected, OnSelectedConversion)
		...
		SLATE_ARGUMENT_DEFAULT(bool, ShowTooltipText) { false };

		/**
		* If CanConvertField is set to true
		* - Menu will display Fields that can be converted to the Selected Property.
		* - Selections will be return via OnSelectedConversion
		*/
		SLATE_ARGUMENT_DEFAULT(bool, CanConvertField) { false };
	SLATE_END_ARGS()
...
private:
	...
	void SetSelectedValue(const TOptional<FMVVMLinkedPinValue>& NewSelection);

	FText GetTooltipText() const;
	FText GetAdditionalTooltipText() const;
	FMVVMBlueprintPropertyPath GetTooltipPropertyPath() const;
...
private:
	...
	TArray<FMVVMFieldConversion> AvailableFieldConversions;

	TOptional<FMVVMLinkedPinValue> CurrentSelectedValue;
	TOptional<FMVVMFieldConversion> CurrentSelectedConversionValue;

	bool bIsMenuInitialized = false;
	bool bCanCreateEvent = false;	
	bool bCanConvertField = false;
	bool bIsSelectingConversion = false;
};
