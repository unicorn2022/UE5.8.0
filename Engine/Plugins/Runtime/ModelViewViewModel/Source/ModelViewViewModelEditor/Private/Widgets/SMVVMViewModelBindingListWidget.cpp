...
namespace Private
{
	struct FPassFilterInputs
	{
		FPassFilterInputs(
			const UWidgetBlueprint* WidgetBlueprint,
			const FMVVMAvailableBinding Binding,
			const UStruct* Struct,
			EFieldVisibility FieldVisibilityFlags,
			const FProperty* AssignableTo,
			bool bDoObjectProperty,
			bool bAllowComponentProperty,
			bool bCanConvert)
			...
			, bDoObjectProperty(bDoObjectProperty)
			, bAllowComponentProperty(bAllowComponentProperty)
			, bCanConvert(bCanConvert)
		{}
	...
	TOptional<FFieldVariant> PassFilter(const FPassFilterInputs& Inputs, const FMVVMFieldVariant& FieldVariant);
	TOptional<FFieldVariant> PassFilter(const FPassFilterInputs& Inputs);
	bool PassFilter_ConversionFunction(const FPassFilterInputs& Inputs, const FProperty* Source);

	TArray<FMVVMFieldConversion> GetCompatibleSourceConversions(const UWidgetBlueprint* WidgetBlueprint, const FProperty* Source, const FProperty* Destination, bool bReturnFirst);

	TOptional<FFieldVariant> PassFilter(const FPassFilterInputs& Inputs)
	{
		if (EnumHasAllFlags(Inputs.FieldVisibilityFlags, EFieldVisibility::Readable) && !Inputs.Binding.IsReadable())
		{
			return TOptional<FFieldVariant>();
		}

		if (EnumHasAllFlags(Inputs.FieldVisibilityFlags, EFieldVisibility::Notify | EFieldVisibility::Readable) && !Inputs.Binding.HasNotify())
		{
			return TOptional<FFieldVariant>();
		}

		FMVVMFieldVariant FieldVariant = BindingHelper::FindFieldByName(Inputs.Struct, Inputs.Binding.GetBindingName());
		return PassFilter(Inputs, FieldVariant);
	}
	...
	TOptional<FFieldVariant> PassFilter(const FPassFilterInputs& Inputs, const FMVVMFieldVariant& FieldVariant)
	{
		...
		if (ensure(!FieldVariant.IsEmpty()))
		{
			if (FieldVariant.IsFunction())
			{
				...
				if (!GetDefault<UMVVMDeveloperProjectSettings>()->IsFunctionAllowed(Inputs.WidgetBlueprint, Cast<const UClass>(Inputs.Struct), Function))
				{
					return TOptional<FFieldVariant>();
				}

				const FProperty* ReturnProperty = BindingHelper::GetReturnProperty(Function);
				bool bDoCompatibleTest = ReturnProperty == nullptr || Inputs.AssignableTo != nullptr;
				bool bDoWritableTest = true;
				// Do we allow walking up the tree
				if (CastField<FObjectPropertyBase>(ReturnProperty))
				{
					bDoCompatibleTest = bDoCompatibleTest && Inputs.bDoObjectProperty;
					bDoWritableTest = Inputs.bDoObjectProperty;
				}

				if (bDoWritableTest)
				{
					if (EnumHasAllFlags(Inputs.FieldVisibilityFlags, EFieldVisibility::Writable) && !Inputs.Binding.IsWritable())
					{
						return TOptional<FFieldVariant>();
					}
				}
				...
				{
					if (EnumHasAllFlags(Inputs.FieldVisibilityFlags, EFieldVisibility::Writable))
					{
						if (Inputs.AssignableTo && !BindingHelper::ArePropertiesCompatible(Inputs.AssignableTo, BindingHelper::GetFirstArgumentProperty(Function)))
						{
							if (Inputs.bCanConvert && PassFilter_ConversionFunction(Inputs, ReturnProperty))
							{
								return FFieldVariant(ReturnProperty);
							}
						}
					}
					if (EnumHasAllFlags(Inputs.FieldVisibilityFlags, EFieldVisibility::Readable))
					{
						if (Inputs.AssignableTo && !BindingHelper::ArePropertiesCompatible(ReturnProperty, Inputs.AssignableTo))
						{
							return TOptional<FFieldVariant>();
						}
					}
			...
			else if (FieldVariant.IsProperty())
			{
				...
				if (!GetDefault<UMVVMDeveloperProjectSettings>()->IsPropertyAllowed(Inputs.WidgetBlueprint, Inputs.Struct, Property))
				{
					return TOptional<FFieldVariant>();
				}

				bool bDoCompatibleTest = Inputs.AssignableTo != nullptr;
				bool bDoWritableTest = true;
				// Do we allow walking up the tree
				...
				if (bDoCompatibleTest)
				{
					if (EnumHasAllFlags(Inputs.FieldVisibilityFlags, EFieldVisibility::Writable))
					{
						if (!BindingHelper::ArePropertiesCompatible(Inputs.AssignableTo, Property))
						{
							if (Inputs.bCanConvert && PassFilter_ConversionFunction(Inputs, Property))
							{
					...
					if (EnumHasAllFlags(Inputs.FieldVisibilityFlags, EFieldVisibility::Readable))
					{
						if (!BindingHelper::ArePropertiesCompatible(Property, Inputs.AssignableTo))
						{
							return TOptional<FFieldVariant>();
						}
					}
				...
				if (Property->HasMetaData(UUIComponentWidgetBlueprintExtension::MD_ComponentVariable) && !Inputs.bAllowComponentProperty)
				{
					return TOptional<FFieldVariant>();
				}
	...
	bool PassFilter_ConversionFunction(const FPassFilterInputs& Inputs, const FProperty* Source)
	{
		const TArray<FMVVMFieldConversion>& Conversions = GetCompatibleSourceConversions(Inputs.WidgetBlueprint, Source, Inputs.AssignableTo, true);
		return Conversions.Num() > 0;
	}

	TArray<FMVVMFieldConversion> GetCompatibleSourceConversions(const UWidgetBlueprint* WidgetBlueprint, const FProperty* Source, const FProperty* Destination, bool bReturnFirst)
	{
		TArray<FMVVMFieldConversion> OutConversions;

		// No conversion necessary
		if (BindingHelper::ArePropertiesCompatible(Source, Destination))
		{
			return OutConversions;
		}

		if (UMVVMEditorSubsystem* Subsystem = GEditor->GetEditorSubsystem<UMVVMEditorSubsystem>())
		{
			TArray<FConversionFunctionValue> ConversionFunctions = Subsystem->GetConversionFunctions(WidgetBlueprint, Source, Destination);
			for (const FConversionFunctionValue& ConversionFunction : ConversionFunctions)
			{
				// Returns function pins that support the Source property. If there are multiple, just pick the first. 
				// Currently no method to determine which parameter is the conversion value
				TArray<FMVVMBlueprintPinId> ArgumentPins = Subsystem->GetConversionFunctionPins(WidgetBlueprint, ConversionFunction, Source);
				if (!ArgumentPins.IsEmpty()) 
				{
					FMVVMFieldConversion Conversion(ConversionFunction, ArgumentPins[0]);
					OutConversions.Add(Conversion);
					if (bReturnFirst)
					{
						break;
					}
...
FFieldIterator_Bindable::FFieldIterator_Bindable(const UWidgetBlueprint* InWidgetBlueprint, EFieldVisibility InVisibilityFlags, const FProperty* InAssignableTo, const bool InIsBindingToEvent, const bool bEnableConversion) :
	WidgetBlueprint(InWidgetBlueprint),
	FieldVisibilityFlags(InVisibilityFlags),
	AssignableTo(InAssignableTo),
	bIsBindingToEvent(InIsBindingToEvent),
	bEnableConversion(bEnableConversion)
{
}
...
TArray<FFieldVariant> FFieldIterator_Bindable::GetFields(const UStruct* Struct, const FName FieldName, const UStruct* Container) const
{
	...
	auto AddResult = [this, &Result](const TArray<FMVVMAvailableBinding>& AvailableBindingsList, const UStruct* Struct, const bool bAllowComponentProperty)
	{
		...
		for (const FMVVMAvailableBinding& Value : AvailableBindingsList)
		{
			...
			{
				const bool bDoObjectProperty = AssignableTo != nullptr;
				const Private::FPassFilterInputs Params(WidgetBlueprint.Get(), Value, Struct, FieldVisibilityFlags, AssignableTo, bDoObjectProperty, bAllowComponentProperty, bEnableConversion);
				PassResult = Private::PassFilter(Params);
			}
			else
			{
				const Private::FPassFilterInputs Params(WidgetBlueprint.Get(), Value, Struct, EFieldVisibility::None, nullptr, false, bAllowComponentProperty, bEnableConversion);
				PassResult = Private::PassFilter(Params);
			}
			if (PassResult.IsSet())
			{
	...
	TSubclassOf<UObject> AccessorClass = nullptr;
	if (WidgetBlueprintPtr)
	{
		AccessorClass = WidgetBlueprintPtr->SkeletonGeneratedClass ? WidgetBlueprintPtr->SkeletonGeneratedClass : WidgetBlueprintPtr->GeneratedClass;
	}
...
TSharedRef<SWidget> ConstructFieldPreSlot(const UWidgetBlueprint* WidgetBlueprint, UE::PropertyViewer::SPropertyViewer::FHandle Handle, const FFieldVariant FieldPath, const bool bIsForEvent)
{
	...
	const UClass* AccessorClass = nullptr;
	if (WidgetBlueprint != nullptr)
	{
		AccessorClass = WidgetBlueprint->SkeletonGeneratedClass ? WidgetBlueprint->SkeletonGeneratedClass : WidgetBlueprint->GeneratedClass;
	}
...
FMVVMFieldConversion::FMVVMFieldConversion(FConversionFunctionValue Function, FMVVMBlueprintPinId ParameterId)
	: Function(Function), ParameterId(ParameterId)
{
}

bool FMVVMFieldConversion::IsValid() const
{
	return Function.IsValid() && ParameterId.IsValid(); 
}

FName FMVVMFieldConversion::GetFName() const
{ 
	return Function.GetFName(); 
}

FMVVMBlueprintFunctionReference FMVVMFieldConversion::ToBlueprintFunctionReference(UBlueprint* Blueprint) const
{
	if (Function.IsFunction())
	{
		return FMVVMBlueprintFunctionReference(Blueprint, Function.GetFunction());
	}
	else if (Function.IsNode())
	{
		return FMVVMBlueprintFunctionReference(Function.GetNode());
	}
	return FMVVMBlueprintFunctionReference();
}
...
{
	WidgetBlueprint = InWidgetBlueprint;
	FieldIterator = MakeUnique<FFieldIterator_Bindable>(InWidgetBlueprint, InArgs._FieldVisibilityFlags, InArgs._AssignableTo, InArgs._IsBindingToEvent, InArgs._bEnableConversion);
...
FMVVMBlueprintPropertyPath SSourceBindingList::CreateBlueprintPropertyPath(SPropertyViewer::FHandle Handle, TArrayView<const FFieldVariant> FieldPath) const
{
		...
		for (const FFieldVariant& FieldVariant : FieldPath)
		{
			...
			if (const UClass* OwnerClass = Cast<const UClass>(CurrentContainer))
			{
				...
				if (Binding.IsValid())
				{
					...
					if (FilterFlags == EFilterFlag::All)
					{
						const Private::FPassFilterInputs Params(WidgetBlueprint.Get(), Binding, OwnerClass, FieldIterator->GetFieldVisibilityFlags(), FieldIterator->GetAssignableTo(), true, false, IsConversionEnabled());
						bPassFilter = Private::PassFilter(Params).IsSet();
					}
					else
					{
						const Private::FPassFilterInputs Params(WidgetBlueprint.Get(), Binding, OwnerClass, EFieldVisibility::None, nullptr, true, false, IsConversionEnabled());
						bPassFilter = Private::PassFilter(Params).IsSet();
					}
				}
				break;
				...
				if (UUIComponentWidgetBlueprintExtension* ComponentWidgetBlueprintExtension = UWidgetBlueprintExtension::GetExtension<UUIComponentWidgetBlueprintExtension>(WidgetBlueprintPtr))
				{
					if (UWidget* Widget = WidgetBlueprint->WidgetTree->FindWidget(Source->Key.GetWidgetName()))
					{
						...
						for (UUIComponent* Component : ComponentsList)
						{
							...
							{
								FName ComponentPropertyName = UUIComponentContainer::GetPropertyNameForComponent(Component, Widget->GetFName());
								if (FProperty* Property = FindFProperty<FObjectPropertyBase>(Cast<UWidgetBlueprintGeneratedClass>(AccessorClass), ComponentPropertyName))
								{
	...
	else if(AccessorClass)
	{
		...
		if (Binding.IsValid())
		{
			...
			if (FilterFlags == EFilterFlag::All)
			{
				const Private::FPassFilterInputs Params(WidgetBlueprint.Get(), Binding, AccessorClass, FieldIterator->GetFieldVisibilityFlags(), FieldIterator->GetAssignableTo(), true, false, IsConversionEnabled());
				bPassFilter = Private::PassFilter(Params).IsSet();
			}
			else
			{
				const Private::FPassFilterInputs Params(WidgetBlueprint.Get(), Binding, AccessorClass, EFieldVisibility::None, nullptr, true, false, IsConversionEnabled());
				bPassFilter = Private::PassFilter(Params).IsSet();
			}
			if (bPassFilter)
			{
...
void SSourceBindingList::EnableConversion(bool InEnabled)
{
	if (FieldIterator)
	{
		FieldIterator->EnableConversion(InEnabled);
	}

	Invalidate(EInvalidateWidgetReason::Layout);
}

bool SSourceBindingList::IsConversionEnabled() const
{
	if (FieldIterator)
	{
		return FieldIterator->IsConversionEnabled();
	}
...
TArray<FMVVMFieldConversion> SSourceBindingList::GetSelectedConversions() const
{
	TArray<FMVVMFieldConversion> SelectedConversions;
	...
	TArray<SPropertyViewer::FSelectedItem> SelectedItems = PropertyViewer->GetSelectedItems();
	for (const SPropertyViewer::FSelectedItem& SelectedItem : SelectedItems)
	{
		if (SelectedItem.Fields.IsEmpty())
		{
			continue;
		}

		for(const TArray<FFieldVariant>& Fields : SelectedItem.Fields)
		{
			if (Fields.IsEmpty())
			{
				continue;
			...
			if (FProperty* Property = CastField<FProperty>(Fields.Last().ToField()))
			{
				TArray<FMVVMFieldConversion> Conversions = Private::GetCompatibleSourceConversions(WidgetBlueprintPtr, Property, FieldIterator->GetAssignableTo(), false);
				SelectedConversions.Append(MoveTemp(Conversions));
			}
		}
	}
	return SelectedConversions;
