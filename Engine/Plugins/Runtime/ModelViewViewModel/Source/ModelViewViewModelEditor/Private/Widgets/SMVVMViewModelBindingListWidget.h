...
namespace UE::MVVM
{
	...
	{
	public:
		FFieldIterator_Bindable(const UWidgetBlueprint* WidgetBlueprint, EFieldVisibility InVisibilityFlags, const FProperty* InAssignableTo = nullptr, const bool InIsBindingToEvent = false, const bool bEnableConversion = false);
		...
		{
			return bEnableConversion;
		}
		void EnableConversion(bool InEnabled)
		{
			bEnableConversion = InEnabled;
		}
	private:
	...
	struct FMVVMFieldConversion : TSharedFromThis<FMVVMFieldConversion>
	{
		FMVVMFieldConversion() = default;
		FMVVMFieldConversion(FConversionFunctionValue Function, FMVVMBlueprintPinId ParameterId);

		FConversionFunctionValue Function;
		FMVVMBlueprintPinId ParameterId;

		bool IsValid() const; 
		FName GetFName() const;
		FMVVMBlueprintFunctionReference ToBlueprintFunctionReference(UBlueprint* Blueprint) const;
	};
	...
	public:
		...
		SLATE_BEGIN_ARGS(SSourceBindingList) {}
			...
			/**
			* If enabled, will show properties that can be converted to AssignableTo. 
			* The compatible conversion functions will be accessible via GetSelectedConversions
			*/
			SLATE_ARGUMENT_DEFAULT(bool, bEnableConversion) = false;
		...
		TArray<FMVVMFieldConversion> GetSelectedConversions() const;
