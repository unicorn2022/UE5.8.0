...
void SFieldSelectorMenu::Construct(const FArguments& InArgs, const UWidgetBlueprint* InWidgetBlueprint)
{
	...
	OnSelectedConversion = InArgs._OnSelectedConversion;
	...
	bCanConvertField = InArgs._CanConvertField;
	...
	if (bShowConversionFunctions || bCanConvertField)
	{
	...
	if (InArgs._ShowTooltipText)
	{
		VBox->AddSlot()
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(this, &SFieldSelectorMenu::GetTooltipText)
				]
				+ SHorizontalBox::Slot()
				.HAlign(HAlign_Left)
				.VAlign(VAlign_Center)
				...
				[
					...
					.ShowOnlyLastPath(true)
					.OnGetPropertyPath(this, &SFieldSelectorMenu::GetTooltipPropertyPath)
				]
				+ SHorizontalBox::Slot()
				.HAlign(HAlign_Left)
				.VAlign(VAlign_Center)
				...
				[
					SNew(STextBlock)
					.Text(this, &SFieldSelectorMenu::GetAdditionalTooltipText)
				]
			];
	}
...
void SFieldSelectorMenu::UpdateSelection()
{
	...
	if(!bIsSelectingConversion && ConversionFunctionCategoryTree.IsValid())
	{
		// If users enabled auto-conversion, do not show the conversion function selection tree. Users will select conversion function when auto-converting
		ConversionFunctionCategoryTree->SetVisibility(BindingList->IsConversionEnabled() ? EVisibility::Collapsed : EVisibility::Visible);
	}

	if (bIsSelectingConversion)
	{
		if (BindingList.IsValid())
		{
			BindingList->SetEnabled(false);
			BindingList->ClearSources();
		}

		if (ViewModelList.IsValid())
		{
			ViewModelList->SetEnabled(false);
...
void SFieldSelectorMenu::SetSelectedValue(const TOptional<FMVVMLinkedPinValue>& NewSelection)
{
	if (bIsSelectingConversion)
	{
		if (NewSelection.IsSet())
		{
			// Find the available conversion
			for (const FMVVMFieldConversion& AvailableConversion : AvailableFieldConversions)
			{
				if (NewSelection->IsConversionFunction() && AvailableConversion.Function.IsFunction()
					&& AvailableConversion.Function.GetFunction()->GetFName() == NewSelection->GetConversionFunction()->GetFName())
				{
					CurrentSelectedConversionValue = AvailableConversion;
					break;
				}
				if (NewSelection->IsConversionNode() && AvailableConversion.Function.IsNode()
					&& AvailableConversion.Function.GetNode()->GetFName() == NewSelection->GetConversionNode()->GetFName())
				{
					CurrentSelectedConversionValue = AvailableConversion;
					break;
				}
...
FText SFieldSelectorMenu::GetTooltipText() const
{
	if (CurrentSelectedValue.IsSet())
	{
		return LOCTEXT("FieldSelector_TooltipBindTo", "Bind to ");
	}
	return LOCTEXT("FieldSelector_Tooltip", "Select a property to bind to:");
}

FText SFieldSelectorMenu::GetAdditionalTooltipText() const
{
	if (bIsSelectingConversion)
	{
		return LOCTEXT("FieldSelector_TooltipConversion", " with conversion function:");
	}
	return FText::GetEmpty();
}

FMVVMBlueprintPropertyPath SFieldSelectorMenu::GetTooltipPropertyPath() const
{
	return CurrentSelectedValue.Get(FMVVMLinkedPinValue()).GetPropertyPath();
}
...
bool SFieldSelectorMenu::IsSelectEnabled() const
{
	if (bIsSelectingConversion)
	{
		return CurrentSelectedConversionValue.IsSet();
	}
...
FReply SFieldSelectorMenu::HandleSelectClicked()
{
	if (bCanConvertField)
	{
		return PromptConversionSelection();
	}

	OnSelected.ExecuteIfBound(CurrentSelectedValue.Get(FMVVMLinkedPinValue()), ESelectionType::Binding);
...
{
	OnSelected.ExecuteIfBound(CurrentSelectedValue.Get(FMVVMLinkedPinValue()), ESelectionType::Event);
...
FReply SFieldSelectorMenu::PromptConversionSelection()
{
	if (!bIsSelectingConversion)
	{
		check(BindingList);
		AvailableFieldConversions = BindingList->GetSelectedConversions();

		if (AvailableFieldConversions.Num() == 0)
		{
			OnSelectedConversion.ExecuteIfBound(CurrentSelectedValue.Get(FMVVMLinkedPinValue()), FMVVMFieldConversion(), ESelectionType::Binding);
		}
		else if (AvailableFieldConversions.Num() == 1)
		{
			CurrentSelectedConversionValue = AvailableFieldConversions[0];
			OnSelectedConversion.ExecuteIfBound(CurrentSelectedValue.Get(FMVVMLinkedPinValue()), CurrentSelectedConversionValue.Get(FMVVMFieldConversion()), ESelectionType::Binding);
		}
		else if (AvailableFieldConversions.Num() > 1)
		{
			// If multiple are available, display conversion function list and wait for users to select.
			bIsSelectingConversion = true;
			...
			if (ConversionFunctionList.IsValid())
			{
				ConversionFunctionList->SetVisibility(EVisibility::Visible);
	...
	// If we are selecting conversion, the select button is disabled until a valid conversion is selected
	bIsSelectingConversion = false;
	AvailableFieldConversions.Empty();
	OnSelectedConversion.ExecuteIfBound(CurrentSelectedValue.Get(FMVVMLinkedPinValue()), CurrentSelectedConversionValue.Get(FMVVMFieldConversion()), ESelectionType::Binding);
...
{
	SetSelectedValue(FMVVMLinkedPinValue(SelectedPath));
...
{
	SetSelectedValue(FMVVMLinkedPinValue(SelectedFunction));
	if (IsEventSelectEnabled())
	{
		HandleEventSelectClicked();
	}
...
TSharedRef<SWidget> SFieldSelectorMenu::CreateBindingContextPanel(const FArguments& InArgs)
{
	...
	if (SelectionContext.bAllowConversionFunctions)
	{
		...
		// bCanConvertField handles visibility of this
		if (!SelectionContext.bAllowConversionFunctions)
		{
			ConversionFunctionCategoryTree->SetVisibility(EVisibility::Collapsed);
		}
...
TSharedRef<SWidget> SFieldSelectorMenu::CreateBindingListPanel(const FArguments& InArgs, const FProperty* AssignableToProperty)
{
	BindingList = SNew(SSourceBindingList, WidgetBlueprint.Get())
		...
		.bEnableConversion(bCanConvertField);
	...
	if (SelectionContext.bAllowConversionFunctions || bCanConvertField)
	{
		...
		if (bCanConvertField)
		{
			ConversionFunctionList->SetVisibility(EVisibility::Collapsed);
		}
	}
...
void SFieldSelectorMenu::HandleEnabledContextToggleChanged(ECheckBoxState CheckState)
{
	...
	BindingList->EnableConversion(bCanConvertField && FilterSettings.FilterFlags == EFilterFlag::All);
...
void SFieldSelectorMenu::FilterConversionFunctions()
{
	if (bIsSelectingConversion)
	{
		...
		for (const FMVVMFieldConversion& AvailableConversion : AvailableFieldConversions)
		{
			if (AvailableConversion.IsValid())
			{
				FilteredConversionFunctions.Add(AvailableConversion.Function);
			}
		}
		return;
	}
