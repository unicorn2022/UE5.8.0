...
EVisibility SBindingRow::GetErrorButtonVisibility() const
{
	...
	{
		UMVVMBlueprintView* BlueprintView = GetBlueprintView();
		const bool HasBindingMessage = BlueprintView->HasBindingMessage(ViewModelBinding->BindingId);
		return HasBindingMessage ? EVisibility::Visible : EVisibility::Hidden;
	}
...
FText SBindingRow::GetErrorButtonToolTip() const
{
	...
	if (const FMVVMBlueprintViewBinding* ViewModelBinding = GetThisViewBinding())
	{
		...
		TArray<FText> BindingInfoList = BlueprintView->GetBindingMessages(ViewModelBinding->BindingId, EBindingMessageType::Info);
		BindingErrorList.Append(BindingWarningList);
		BindingErrorList.Append(BindingInfoList);

		static const FText NewLineText = FText::FromString(TEXT("\n"));
		FText HintText = LOCTEXT("ErrorButtonText", "Messages: (Click to show in a separate window)");
...
FReply SBindingRow::OnErrorButtonClicked()
{
	...
	if (WidgetBlueprint && View && ViewBinding)
	{
		...
		for (const FText& InfoText : View->GetBindingMessages(ViewBinding->BindingId, EBindingMessageType::Info))
		{
			ErrorItems.Add(MakeShared<FText>(InfoText));
		}
		...
		TSharedRef<SCustomDialog> ErrorDialog = SNew(SCustomDialog)
			.Title(FText::Format(LOCTEXT("Compilation Messages", "Compilation messages for {0}"), BindingDisplayName))
			.Buttons({
