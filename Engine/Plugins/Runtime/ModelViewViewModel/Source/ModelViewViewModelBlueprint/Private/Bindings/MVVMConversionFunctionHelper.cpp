	...
	TValueOrError<TArray<UEdGraphPin*>, void> BuildPropertyPath(const UBlueprint* Blueprint, UEdGraph* FunctionGraph, const FMVVMBlueprintPropertyPath& PropertyPath, int32 NumberOfFieldExcludingThePropertyPathSource, FVector2f EndLocation)
	{
		...
		for (int32 Index = 0; Index < NumberOfFieldExcludingThePropertyPathSource; ++Index)
		{
			...
			if (Field.IsProperty())
			{
				...
				if (const FStructProperty* PreviousStructProperty = CastField<FStructProperty>(PreviousProperty))
				{
					...
					if (MetaData.Len() > 0)
					{
						...
						{
							UE_LOGFMT(LogMVVM, Error, "BuildPropertyPath: a function in path couldn't be resolved. Blueprint '{Blueprint}' Function '{Function}'.",
				...
				{
					UE_LOGFMT(LogMVVM, Error, "BuildPropertyPath: cannot create getter node — no previous class context for property '{Property}' in blueprint '{Blueprint}'.",
						("Property", Property ? Property->GetFName() : NAME_None),
						("Blueprint", GetNameSafe(Blueprint)));
					return MakeError();
				}
			}
			else if (Field.IsFunction())
			{
				...
				{
					UE_LOGFMT(LogMVVM, Error, "BuildPropertyPath: cannot call a function on struct property '{Property}' in blueprint '{Blueprint}'.",
						("Property", PreviousProperty ? PreviousProperty->GetFName() : NAME_None),
						("Blueprint", GetNameSafe(Blueprint)));
					return MakeError();
				}
			...
			{
				UE_LOGFMT(LogMVVM, Error, "BuildPropertyPath: invalid path, empty field in path for blueprint '{Blueprint}'.",
					("Blueprint", GetNameSafe(Blueprint)));
				return MakeError();
			}
			...
			{
				UE_LOGFMT(LogMVVM, Error, "BuildPropertyPath: a node in path doesn't have a return value. Node '{Node}'.",
					("Node", GetPathNameSafe(NewNode)));
				return MakeError();
			}
