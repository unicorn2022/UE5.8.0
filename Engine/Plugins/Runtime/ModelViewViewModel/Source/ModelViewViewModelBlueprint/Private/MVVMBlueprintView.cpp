...
bool UMVVMBlueprintView::HasBindingMessage(FGuid Id) const
{
	return BindingMessages.Find(Id) != nullptr;
}
...
void UMVVMBlueprintView::PostLoad()
{
	...
	UWidgetBlueprint* ThisWidgetBlueprint = GetOuterUMVVMWidgetBlueprintExtension_View()->GetWidgetBlueprint();
	...
	for (FMVVMBlueprintViewModelContext& ViewModelContext : AvailableViewModels)
	{
		if (UMVVMViewModelContextResolver* ResolverObject = ViewModelContext.Resolver)
		{
			if (!ResolverObject->HasAnyFlags(RF_Public))
			{
				ResolverObject->SetFlags(RF_Public);
			}
			if (ResolverObject->GetOuter() == ThisWidgetBlueprint->GetPackage())
			{
				ResolverObject->Rename(nullptr, ThisWidgetBlueprint, REN_DontCreateRedirectors | REN_NonTransactional);
			}
			else
			{
				// We need to explicitly clear up resolver redirectors, they were created when the resolver was moved to the correct package. 
				// Now REN_DontCreateRedirectors flag is added to ensure these redirectors are not created. But we still need to clear the existing ones.
				// Redirectors are not needed here because the resolver was not RF_Public before, it's only being made public at the same time as the package change.
				UObjectRedirector* FoundResolverRedirector = nullptr;
				ForEachObjectWithPackage(ThisWidgetBlueprint->GetPackage(), [ResolverObject, &FoundResolverRedirector](UObject* Object)
					{
						if (UObjectRedirector* Redirector = Cast<UObjectRedirector>(Object))
						{
							if (Redirector->DestinationObject == ResolverObject)
							{
...
{
	// Cleanup Event and Conditions to prevent BP compilation issues
	for (int32 EventIndex = Conditions.Num() - 1; EventIndex >= 0; --EventIndex)
	{
		UMVVMBlueprintViewCondition* Condition = Conditions[EventIndex];
		if (Condition == nullptr)
		{
			Conditions.RemoveAt(EventIndex);
		}
	}

	for (int32 ConditionIndex = Events.Num() - 1; ConditionIndex >= 0; --ConditionIndex)
	{
		UMVVMBlueprintViewEvent* Event = Events[ConditionIndex];
		if (Event == nullptr)
		{
			Events.RemoveAt(ConditionIndex);
		}
	}
