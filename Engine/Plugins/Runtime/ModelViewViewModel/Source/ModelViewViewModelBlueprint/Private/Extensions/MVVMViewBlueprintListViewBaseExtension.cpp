	...
	if (UWidget* const* FoundList = Compiler->GetWidgetNameToWidgetPointerMap().Find(WidgetName))
	{
		if (UListViewBase* ListViewBase = Cast<UListViewBase>(*FoundList))
		{
			...
			if (EntryWidgetClass.Get())
			{
				...
				if (const UWidgetBlueprint* EntryBlueprint = Cast<UWidgetBlueprint>(EntryWidgetClass->ClassGeneratedBy))
				{
					if (const UMVVMBlueprintView* EntryBPView = GetEntryWidgetBlueprintView(EntryBlueprint))
					{
						...
						else
						{
							...
							Compiler->AddMessage(FText::Format(LOCTEXT("PreCompileMVVMWidgetExtensionEntryVMDeleted", "No viewmodel selected for entry widget {0}. Please select a viewmodel for it via the details panel or remove the Viewmodel extension on the containing widget {1}."), EntryWidgetClass->GetDisplayNameText(), FText::FromName(WidgetName)), UE::MVVM::Compiler::EMessageType::Error);
					...
					else
					{
						...
						EntryViewModelId = FGuid();
						Compiler->AddMessage(FText::Format(LOCTEXT("PreCompileMVVMWidgetExtensionEntryHasNoView", "Entry widget {0} doesn't have a View. Consider adding a binding to it or remove the MVVM extension on the containing widget {1}."), EntryWidgetClass->GetDisplayNameText(), FText::FromName(WidgetName)), UE::MVVM::Compiler::EMessageType::Error);
					}
				}
				else
				{
					Compiler->AddMessage(FText::Format(LOCTEXT("PreCompileMVVMWidgetExtensionEntryWidgetBlueprintNotFound", "No widget blueprint was found for entry widget {0}. The MVVM extension on the containing widget {1} won't work."), EntryWidgetClass->GetDisplayNameText(), FText::FromName(WidgetName)), UE::MVVM::Compiler::EMessageType::Error);
				}
			}
		}
		else
...
const UMVVMBlueprintView* UMVVMBlueprintViewExtension_ListViewBase::GetEntryWidgetBlueprintView(const UWidgetBlueprint* EntryWidgetBlueprint) const
{
	if (EntryWidgetBlueprint)
	{
		if (const UMVVMWidgetBlueprintExtension_View* EntryWidgetExtension = UMVVMWidgetBlueprintExtension_View::GetExtension<UMVVMWidgetBlueprintExtension_View>(EntryWidgetBlueprint))
		{
