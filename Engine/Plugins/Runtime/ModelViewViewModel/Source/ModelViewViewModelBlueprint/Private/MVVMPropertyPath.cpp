...
TArray<UE::MVVM::FMVVMConstFieldVariant> FMVVMBlueprintPropertyPath::GetComponentFields(const UClass* InSelfContext) const
{
	...
	for (const FMVVMBlueprintFieldPath& Path : Paths)
	{
		const UE::MVVM::FMVVMConstFieldVariant Field = Path.GetField(InSelfContext);
		if (!Field.IsProperty())
		{
			// Pass functions through directly
			Result.Add(Field);
			continue;
		}
		if (const FProperty* PropertyPtr = Field.GetProperty())
		{
	...
	if (InContext != nullptr)
	{
		...
		for (FMVVMBlueprintFieldPath& Path : Paths)
		{
			if (bFoundRenamedField && !bIsComponent)
			{
				...
				{
					FName FieldName = Path.GetFieldName(nullptr);
					// We need to update the stored parent in the path in case the parent changed type
					FProperty* UpdatedProperty = OwnerStruct->FindPropertyByName(FieldName);
					if (UpdatedProperty)
					{
						Path = { InContext, UE::MVVM::FMVVMConstFieldVariant(UpdatedProperty) };
					}
					else if (const UClass* OwnerClass = Cast<UClass>(OwnerStruct))
					{
						if (UFunction* UpdatedFunction = OwnerClass->FindFunctionByName(FieldName))
						{
							Path = { InContext, UE::MVVM::FMVVMConstFieldVariant(UpdatedFunction) };
						}
					}
					else
					{
						UE_LOGFMT(LogMVVM, Error, "OnFieldRenamed: Field name '{Field}' does not exist in class '{Struct}'. Binding won't compile.",
							("Field", FieldName.ToString()),
							("Struct", OwnerStruct->GetName()));
					}
				}
