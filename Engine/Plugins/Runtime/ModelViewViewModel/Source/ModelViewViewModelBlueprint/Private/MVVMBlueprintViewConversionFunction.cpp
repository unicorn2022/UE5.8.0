...
UEdGraph* UMVVMBlueprintViewConversionFunction::CreateWrapperGraphInternal(UBlueprint* Blueprint)
{
	...
	if (bIsUbergraphPage)
	{
		...
		// Create the NodeGuid deterministically. Use this->GetPathName(), for the guid instead of LatentEventNodeUUID->GetPathName(), 
		// to avoid possible indeterminism in the LatentEventNodeUUID's objectname.
		LatentEventNodeUUID->NodeGuid = FGuid::NewDeterministicGuid(this->GetPathName(), 1);
	}
