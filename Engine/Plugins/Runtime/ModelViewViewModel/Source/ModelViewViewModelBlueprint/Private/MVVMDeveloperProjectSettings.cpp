...
UMVVMDeveloperProjectSettings::UMVVMDeveloperProjectSettings()
{
	...
	AllowedContextCreationType.Add(EMVVMBlueprintViewModelContextCreationType::Context);
...
bool UMVVMDeveloperProjectSettings::IsPropertyAllowed(const UBlueprint* GeneratingFor, const UStruct* ObjectStruct, const FProperty* Property) const
{
	...
	{
		if (FPropertyEditorPermissionList::Get().IsEnabledForPackage(GeneratingFor->GetPackage()) && !FPropertyEditorPermissionList::Get().DoesPropertyPassFilter(AuthoritativeClass, Property->GetFName()))
		{
			return false;
		}
	}
