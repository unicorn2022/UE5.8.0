...
void UMVVMView::InitializeSources()
{
	...
	for (int32 Index = 0; Index < NumberOfSources; ++Index)
	{
		...
		InitializeSourceParentBinding(FMVVMView_SourceKey(Index));
	}
...
void UMVVMView::UninitializeSources()
{
	...
	for (int32 Index = 0; Index < NumberOfSources; ++Index)
	{
		...
		UninitializeSourceParentBinding(FMVVMView_SourceKey(Index));
	}
}
...
void UMVVMView::InitializeSourceInternal(UObject* NewSource, FMVVMViewClass_SourceKey SourceKey, const FMVVMViewClass_Source& ClassSource, FMVVMView_Source& ViewSource)
{
	...
	// See comment in UMVVMView::InitializeSourceParentBinding for why we do this
	GetUserWidget()->BroadcastFieldValueChanged(UE::FieldNotification::FFieldId(ClassSource.GetName(), 0));
}
...
void UMVVMView::InitializeSourceParentBinding(FMVVMView_SourceKey SourceKey)
{
	FMVVMView_Source& ViewSource = Sources[SourceKey.GetIndex()];
	const FMVVMViewClass_Source& ClassSource = GeneratedViewClass->GetSource(ViewSource.ClassKey);

	if (ClassSource.IsParentContextReference())
	{
		FMVVMViewClass_Source::FParentSourceInfo ParentInfo = ClassSource.GetParentSupplyingSourceClass(GetUserWidget());
		if (ParentInfo.ParentView && ParentInfo.ParentUserWidget)
		{
			const FMVVMView_SourceKey ParentSourceKey = ParentInfo.ParentSourceKey;
			const FMVVMViewClass_Source& ParentClassSource = ParentInfo.ParentView->GetViewClass()->GetSource(ParentInfo.ParentSourceKey.GetIndex());

			TWeakObjectPtr<const UMVVMView> WeakView = ParentInfo.ParentView;
			UE::FieldNotification::FFieldMulticastDelegate::FDelegate Delegate = UE::FieldNotification::FFieldMulticastDelegate::FDelegate::CreateWeakLambda(this, [this, SourceKey, ParentSourceKey, WeakView](UObject*, UE::FieldNotification::FFieldId)
				{
					if (const UMVVMView* ParentView = WeakView.Get())
					{
						SetSourceInternal(FMVVMViewClass_SourceKey(SourceKey.GetIndex()), ParentView->GetSource(ParentSourceKey).Source, true);
					}
				});

			ParentInfo.ParentUserWidget->GetFieldNotificationDescriptor();

			// This is a big of a hack. The VM variable is not marked as field notify and we don't want it to be, but we also don't want to add bespoke
			// notification logic. We always know that there will be at least one field notify in a user widget (UWidget has a few) and field notifies
			// use the index to enable callbacks then the names to filter them. Adding this will enable all calbacks bound to index 0 (which may be multiple)
			// and get called for any callbacks that share the name, regardless of index. If there happens to be a duplicate name in the delegates, then
			// it likely just means that some callbacks are firing off more than they should, i.e. being notified of a change when there wasn't any,
			// so there should be very low risk to this.
			//
			// If any of this changes make sure to also update UMVVMView::InitializeSourceInternal and UMVVMView::UninitializeSourceParentBinding
			ParentInfo.ParentUserWidget->AddFieldValueChangedDelegate(UE::FieldNotification::FFieldId(ParentClassSource.GetName(), 0), MoveTemp(Delegate));
		}
	}
}

void UMVVMView::UninitializeSourceParentBinding(FMVVMView_SourceKey SourceKey)
{
	FMVVMView_Source& ViewSource = Sources[SourceKey.GetIndex()];
	const FMVVMViewClass_Source& ClassSource = GeneratedViewClass->GetSource(ViewSource.ClassKey);
	...
	if (ClassSource.IsParentContextReference())
	{
		...
		if (ParentInfo.ParentView && ParentInfo.ParentUserWidget)
		{
			ParentInfo.ParentUserWidget->RemoveAllFieldValueChangedDelegates(this);
		}
	}
}
			...
			if (NotifyFieldValueChanged.GetInterface() != nullptr)
			{
				...
				for (const FMVVMViewClass_FieldId& FieldId : ClassSource.GetFieldIds())
				{
					if (ensureMsgf(FieldId.IsValid(), TEXT("Invalid field. It failed somewhere in the compiler.")))
					{
						...
						FDelegateHandle Handle = NotifyFieldValueChanged->AddFieldValueChangedDelegate(FieldId.GetFieldId(), MoveTemp(Delegate));
						ensure(Handle.IsValid());
						RegisteredBindings.Add( { SourceKey.GetIndex(), HandleIndex }, Handle);
...
void UMVVMView::UninitializeSourceBindings(FMVVMViewClass_SourceKey SourceKey, const FMVVMViewClass_Source& ClassSource, FMVVMView_Source& ViewSource)
{
	...
	if (ViewSource.bBindingsInitialized && ViewSource.RegisteredCount > 0)
	{
		if (ViewSource.Source)
		{
			...
			int32 HandleIndex = 0;
			int32 HandlesRemoved = 0;
			for (const FMVVMViewClass_FieldId& FieldId : ClassSource.GetFieldIds())
			{
				if (FieldId.IsValid())
				{
					FDelegateHandle Handle;
					ensure(RegisteredBindings.RemoveAndCopyValue({ SourceKey.GetIndex(), HandleIndex }, Handle));
					ensure(SourceAsInterface->RemoveFieldValueChangedDelegate(FieldId.GetFieldId(), Handle));
					++HandlesRemoved;
				}

				++HandleIndex;
			}
			ensure(HandlesRemoved == ViewSource.RegisteredCount);
...
bool UMVVMView::SetSourceInternal(FMVVMViewClass_SourceKey ClassSourceKey, TScriptInterface<INotifyFieldValueChanged> NewValue, bool bForDynamicSource)
{
		...
		&& !NewValue.GetObject()->GetClass()->IsChildOf(ClassSource.GetSourceClass()))
	{
		...
		UE_LOGF(LogMVVM, Error, "The viewmodel name '%ls' is invalid for the view '%ls'", *ClassSource.GetName().ToString(), *GetFullName());
		return false;
	}
