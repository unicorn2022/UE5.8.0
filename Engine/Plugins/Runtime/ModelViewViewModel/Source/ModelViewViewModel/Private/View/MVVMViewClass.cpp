...
#include "Extensions/ContextDataWidgetExtension.h"
...
UObject* FMVVMViewClass_Source::GetOrCreateInstance(const UMVVMViewClass* InViewClass, UMVVMView* InView, UUserWidget* InUserWidget) const
{
	...
	else if (IsParentContextReference())
	{
		UE::MVVM::FMessageLog Log(InUserWidget);

		if (ExpectedSourceType.Get() != nullptr)
		{
			const FParentSourceInfo ParentInfo = GetParentSupplyingSourceClass(InUserWidget);
			if (ParentInfo.ParentView)
			{
				Result = ParentInfo.Source;

				Log.Info(FText::Format(LOCTEXT("CreateInstanceContextParentFound", "The source '{0}' was found on view from parent widget '{1}'"), FText::FromName(PropertyName), FText::FromString(GetNameSafe(ParentInfo.ParentUserWidget))));
			}
			else if (ParentInfo.Source)
			{
				Result = ParentInfo.Source;

				Log.Info(FText::Format(LOCTEXT("CreateInstanceContextDataFound", "The source '{0}' was found on context data from widget '{1}'"), FText::FromName(PropertyName), FText::FromString(GetNameSafe(ParentInfo.ParentUserWidget))));
			}
			else
			{
				Log.Error(FText::Format(LOCTEXT("CreateInstanceContextParentMissing", "The source '{0}' could not be created. No parent widget found that provides an instance of '{1}'"), FText::FromName(PropertyName), FText::FromString(GetNameSafe(ExpectedSourceType))));
			}		
		}
		else if (!bOptional)
		{
			Log.Error(FText::Format(LOCTEXT("CreateInstanceContextClassMissing", "The source '{0}' could not be created. The class is not loaded."), FText::FromName(PropertyName)));
		}
	}
...
void FMVVMViewClass_Source::ReleaseInstance(UObject* ViewModel, const UMVVMView* View) const
{
	if (Resolver)
	{
...
FMVVMViewClass_Source::FParentSourceInfo FMVVMViewClass_Source::GetParentSupplyingSourceClass(UUserWidget* UserWidget) const
{
	// Find an existing instance from a parent source and/or bind to when that paren't source changes
	for (UUserWidget* ContextWidget = UserWidget; ContextWidget; ContextWidget = ContextWidget->GetTypedOuter<UUserWidget>())
	{
		if (const UContextDataWidgetExtension* ContextDataExtension = ContextWidget->GetExtension<UContextDataWidgetExtension>())
		{
			if (UObject* ContextData = ContextDataExtension->FindData(ExpectedSourceType))
			{
				FParentSourceInfo Result;
				Result.ParentUserWidget = ContextWidget;
				Result.Source = ContextData;
				return Result;
			}
		}

		if (ContextWidget != UserWidget)
		{
			if (const UMVVMView* ParentView = UMVVMSubsystem::GetViewFromUserWidget(ContextWidget); ParentView && ParentView->GetViewClass())
			{
				const int32 FoundIndex = ParentView->GetViewClass()->GetSources().IndexOfByPredicate([this](const FMVVMViewClass_Source& Other)
					{
						return Other.IsViewModel() && Other.GetSourceClass()->IsChildOf(ExpectedSourceType) &&
							(GetName().IsNone() ? Other.GetName().IsNone() : GetName() == Other.GetName());
					});

				if (ParentView->GetSources().IsValidIndex(FoundIndex))
				{
					FParentSourceInfo Result;
					Result.ParentView = ParentView;
					Result.ParentSourceKey = FoundIndex;
					Result.ParentUserWidget = ContextWidget;
					Result.Source = ParentView->GetSource(Result.ParentSourceKey).Source;
					return Result;
				}
			}
		}
