...
void FModelViewViewModelModule::StartupModule()
{
	...
	NumericConverter = MakeShared<FMVVMNumericImplicitConverter>();
	RegisterImplicitConverter(NumericConverter.ToSharedRef());

#if WITH_EDITOR
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FModelViewViewModelModule::OnPostEngineInit);
#endif
}
...
void FModelViewViewModelModule::ShutdownModule()
{
	...
	if (NumericConverter.IsValid())
	{
		UnregisterImplicitConverter(NumericConverter.ToSharedRef());
	}
...
void FModelViewViewModelModule::RegisterImplicitConverter(const TSharedRef<FMVVMImplicitConverter>& Converter)
{
	ImplicitConverters.AddUnique(Converter);
}

void FModelViewViewModelModule::UnregisterImplicitConverter(const TSharedRef<FMVVMImplicitConverter>& Converter)
{
	ImplicitConverters.Remove(Converter);
}
