...
namespace UE::MVVM::BindingHelper
{
	...
	namespace Private
	{
		...
		bool CanConvertImplicitly(const FProperty* Source, const FProperty* Destination)
		{
			const FModelViewViewModelModule& MVVMModule = FModuleManager::GetModuleChecked<FModelViewViewModelModule>("ModelViewViewModel");
			for (const TSharedPtr<FMVVMImplicitConverter>& Converter : MVVMModule.GetImplicitConverters())
			{
				if (Converter.IsValid() && Converter->CanConvertImplicitly(Source, Destination))
				{
					return true;
				}
			}
		...
		void ConvertImplicitly(const FProperty* Source, const FProperty* Destination, void* Data)
		{
			check(Source);
			check(Destination);
			check(Data);
			...
			for (const TSharedPtr<FMVVMImplicitConverter>& Converter : MVVMModule.GetImplicitConverters())
			{
				if (Converter.IsValid() && Converter->CanConvertImplicitly(Source, Destination))
				{
					Converter->ConvertImplicitly(Source, Destination, Data);
					break;
				}
			}
		}
	...
	bool ArePropertiesCompatible(const FProperty* Source, const FProperty* Destination)
	{
		...
		return Source->SameType(Destination)
				|| Private::CanConvertImplicitly(Source, Destination)
	...
	void ExecuteBinding_NoCheck(const FFieldContext& Source, const FFieldContext& Destination)
	{
		...
		Private::ConvertImplicitly(GetterType, SetterType, DataPtr);
	...
	void ExecuteBinding_NoCheck(const FFieldContext& Destination, const FFunctionContext& ConversionFunction)
	{
		...
		if (const FProperty* ReturnConversionProperty = Private::GetRuntimeReturnProperty(ConversionFunction.GetFunction()))
		{
			...
			if (Private::CanConvertImplicitly(ReturnConversionProperty, SetterType))
			{
				...
				Private::ConvertImplicitly(ReturnConversionProperty, SetterType, SetterDataPtr);
	...
	void ExecuteBinding_NoCheck(const FFieldContext& Source, const FFieldContext& Destination, const FFunctionContext& ConversionFunction)
	{
		...
		// Get the value
		{
			...
			if (Private::CanConvertImplicitly(GetterType, ArgumentConversionProperty))
			{
				// we need to do a copy because we may destroy the ReturnConversionProperty (imagine the return type is a TArray)
				...
				Private::ConvertImplicitly(GetterType, ArgumentConversionProperty, GetterDataPtr);
