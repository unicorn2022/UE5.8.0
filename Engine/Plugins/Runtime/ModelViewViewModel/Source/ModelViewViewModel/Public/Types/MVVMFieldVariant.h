...
namespace UE::MVVM
{
	...
	public:
		...
		UE_DEPRECATED(5.8, "TMVVMFieldVariant::HasAnyFlags is deprecated, it is only relevant for functions. Consider using GetFunction()->HasAnyFlags() after checking IsFunction() instead.")
		[[nodiscard]] bool HasAnyFlags(EObjectFlags InFlags) const
		{
			if (Type == EVariantType::Function)
			{
