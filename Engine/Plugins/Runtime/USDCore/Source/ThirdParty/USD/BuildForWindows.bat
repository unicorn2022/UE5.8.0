...
set TBB_LOCATION=%UE_SOURCE_THIRD_PARTY_LOCATION%\Intel\TBB\Deploy\oneTBB-2022.3.0
set TBB_LIB_LOCATION=%TBB_LOCATION%\%COMPILER_VERSION_NAME%\%ARCH_NAME%\lib
set TBB_CMAKE_LOCATION=%TBB_LIB_LOCATION%\cmake\TBB
set IMATH_LOCATION=%UE_SOURCE_THIRD_PARTY_LOCATION%\Imath\Deploy\Imath-3.2.2
...
set OPENSUBDIV_LOCATION=%UE_SOURCE_THIRD_PARTY_LOCATION%\OpenSubdiv\Deploy\OpenSubdiv-3.7.0
set OPENSUBDIV_LIB_LOCATION=%OPENSUBDIV_LOCATION%\%COMPILER_VERSION_NAME%\%ARCH_NAME%\lib
set OPENSUBDIV_CMAKE_LOCATION=%OPENSUBDIV_LIB_LOCATION%\cmake\OpenSubdiv
set ALEMBIC_LOCATION=%UE_SOURCE_THIRD_PARTY_LOCATION%\Alembic\Deploy\alembic-1.8.9
...
set MATERIALX_LOCATION=%UE_SOURCE_THIRD_PARTY_LOCATION%\MaterialX\Deploy\MaterialX-1.39.4
...
rem For maximum compatibility, we build the binaries for Windows using the
rem engine's minimum supported version of msvc. This is done by specifying
rem the toolchain version using cmake's -T option. Ensure this version has
rem been installed using Visual Studio Installer.
rem Check "MinimumVisualCppVersion" in the config file here for updates:
rem     Engine/Config/Windows/Windows_SDK.json
...
cmake -G "Visual Studio 17 2022" %OPENUSD_SOURCE_LOCATION%^
    ...
    -DCMAKE_PREFIX_PATH="%TBB_CMAKE_LOCATION%;%IMATH_CMAKE_LOCATION%;%OPENSUBDIV_CMAKE_LOCATION%;%ALEMBIC_CMAKE_LOCATION%;%MATERIALX_CMAKE_LOCATION%"^
    ...
    -DPXR_ENABLE_RUNTIME_MEMORY_OVERLOADS=ON^
...
move "%INSTALL_PLUGIN_USD_LOCATION%\hioOpenEXR" "%INSTALL_RESOURCES_PLUGINS_LOCATION%"
