...
namespace UsdToUnreal
{
	...
	struct FUsdMeshConversionOptions
	{
		...
		// Whether to always check the deprecated "unrealMaterial" string attribute on prims regardless of the configured RenderContext.
		// When false (the default), the unrealMaterial attribute is only checked when the RenderContext is "unreal".
		// This is mainly used by Interchange: The render context is resolved only later (and we always provide 'universal'), so we need
		// this additional parameter to not ignore the string attribute completely
		bool bForceCheckUnrealMaterialAttribute;

		// If true, a bound material is only used if it has a surface shader output for the configured RenderContext.
		// If it doesn't, the binding is rejected and processing falls through to the DisplayColor fallback.
		// Legacy USD importers set this to true; Interchange sets it to false because the render context is resolved at a later stage.
	...
	/**
	 * Converts an UsdGeomCapsule(_1), UsdGeomCone, UsdGeomCube, UsdGeomCylinder(_1), UsdGeomPlane or UsdGeomSphere and appends
	...
	/**
	 * Extracts a "primitive transform" from the provided UsdGeomCapsule(_1), UsdGeomCone, UsdGeomCube, UsdGeomCylinder(_1),
...
namespace UsdUtils
{
	 ...
	 * @param bForceCheckUnrealMaterialAttribute - Whether to always check the "unrealMaterial" string attribute regardless of render context
	 * @param bRequireMaterialsHaveProvidedRenderContext - If this is enabled, we'll only consider a material binding if it has a surface output
	 * for the provided render context. If this is false, we'll use the bound material regardless
	...
	USDUTILITIES_API FUsdPrimMaterialAssignmentInfo GetPrimMaterialAssignments(
		...
		bool bForceCheckUnrealMaterialAttribute = false,
		bool bRequireMaterialsHaveProvidedRenderContext = true
	);
