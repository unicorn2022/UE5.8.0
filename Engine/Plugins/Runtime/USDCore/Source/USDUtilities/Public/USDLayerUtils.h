...
namespace UsdUtils
{
	...
	enum class EUSDZDecompressResult
	{
		/** Extraction failed due to invalid, unsafe, or unsupported archive contents. */
		Failed,

		/** Extraction succeeded, but some entries were skipped. */
		SuccessWithSkippedEntries,

		/** Extraction succeeded with all entries processed. */
		Success
	};
	...
	/**
	* Returns true if the given path is a zip archive, by checking the first four 'magic' bytes. This prevents a usd zip archive with a 
	* usda or usdc suffix from eliding our unzip handling, which is required for correctness and zip safety/exploit reasons.
	*/

	USDUTILITIES_API bool IsZipArchive(const FString& InUSDZFilePath);
	/**
	...
	/**
	 * If InUSDZFilePath points at an actual USDZ file, this function will decompress the archive and put its contents inside InOutputDir,
	 * returning Success or SuccessWithSkippedEntries if succeeding. In that case OutDecompressedRootLayer will also be set to the
	 * path of the root layer of the decompressed USDZ archive. Note that for safety reasons, InOutputDir must not exist prior to 
	 * calling this function. The directory will be created once the zip file contents have been validated, and will be removed if
	 * an error is encountered during decompression.
	 */
	USDUTILITIES_API EUSDZDecompressResult DecompressUSDZFile(const FString& InUSDZFilePath, const FString& InOutputDir, FString* OutDecompressedRootLayer = nullptr);
