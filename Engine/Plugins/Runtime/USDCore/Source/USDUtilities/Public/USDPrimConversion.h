...
namespace UnrealToUsd
{
	 ...
	 *
	 * If bCheckAssetUserData is enabled, this function will check the asset's USD Asset User Data in order to determine the mapping between
...
namespace UsdUtils
{
	...
	/**
	 * Checks if the given property requires frame-by-frame baking due to non-linear attribute
	 * interactions (e.g. light intensity = inputs:intensity * 2^inputs:exposure).
	 * If so, densifies InOutTimeSamples to include every integer time code between the first and last
	 * existing sample, ensuring the non-linear combination is properly captured as linear keyframes.
	 */
	USDUTILITIES_API void DensifyTimeSamplesIfNeeded(
		const UE::FUsdPrim& Prim,
