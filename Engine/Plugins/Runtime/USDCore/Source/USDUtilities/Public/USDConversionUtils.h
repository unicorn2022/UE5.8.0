...
namespace UsdUtils
{
	...
	/**
	 * Adds or removes a case sensitive hash suffix to the PrimPath string.
	 *
	 * We mainly use this to make sure that prim paths that differ only by case don't cause collisions inside
	 * case insensitive TMaps and TSets. We can sometimes use the case sensitive versions of those containers
	 * (see USDSetUtils.h), but sometimes that is not possible, due to backwards compatibility or due to the
	 * container being a UPROPERTY
	 */
	USDUTILITIES_API FString AddHashToPrimPath(const FString& PrimPath);
	USDUTILITIES_API FString RemoveHashFromPrimPath(const FString& HashedPrimPath);
	...
	/** Returns a list of all the registered property names for a provided APISchema name. Also works for codeless schemas */
	USDUTILITIES_API TArray<FString> GetAppliedAPISchemaPropertyNames(const FString& SchemaName);
	...
	/** Checks the prim kind and the collapsing schema to determine whether the provided prim should collapse or not */
	USDUTILITIES_API bool PrimCollapses(
		const pxr::UsdPrim& Prim,
	...
	/** Returns the filepaths that this prim is referencing or using as a payload, in case it has any external reference or payload authored */
	USDUTILITIES_API TArray<FString> GetReferenceOrPayloadFilePaths(const UE::FUsdPrim& Prim);
	...
	/** Collects how many times each schema shows up on the provided stage and appends those attributes to the provided OutAttributes */
	USDUTILITIES_API void CollectSchemaAnalytics(const UE::FUsdStage& Stage, TArray<FAnalyticsEventAttribute>& OutAttributes);
