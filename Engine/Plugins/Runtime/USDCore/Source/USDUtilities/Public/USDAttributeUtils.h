...
namespace UsdUtils
{
	...
	/** Returns true only if the attribute has an authored default opinion. If the attribute only has timeSamples, splines, or no opinion at all, this will return false */
	USDUTILITIES_API bool HasAuthoredDefaultOpinion(const UE::FUsdAttribute& Attribute);

	/** Returns true only if the attribute has an authored animated (timeSample or splines) opinion. If the attribute only has a default opinion (or no opinion at all) this will return false */
	USDUTILITIES_API bool HasAuthoredAnimatedOpinion(const UE::FUsdAttribute& Attribute);
