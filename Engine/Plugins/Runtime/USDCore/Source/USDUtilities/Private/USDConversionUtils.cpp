...
UClass* UsdUtils::GetActorTypeForPrim(const pxr::UsdPrim& Prim)
{
	...
	else if (Prim.IsA<pxr::UsdLuxDomeLight>() || Prim.IsA<pxr::UsdLuxDomeLight_1>())
	{
...
)
{
	...
	if (Component == Owner->GetRootComponent())
	{
		...
		else
		{
			...
			GetObjectsWithOuter(OwnerOuter, SiblingActors, EGetObjectsFlags::None);
...
TArray<FString> UsdUtils::GetAppliedAPISchemaPropertyNames(const FString& SchemaName)
{
	const pxr::UsdSchemaRegistry& Registry = pxr::UsdSchemaRegistry::GetInstance();

	const pxr::UsdPrimDefinition* Definition = Registry.FindAppliedAPIPrimDefinition(UnrealToUsd::ConvertToken(*SchemaName).Get());
	if (!Definition)
	{
		return {};
	}

	const TUsdStore<std::vector<pxr::TfToken>> PxrPropertyNames = Definition->GetPropertyNames();

	TArray<FString> PropertyNames;
	PropertyNames.Reserve(PxrPropertyNames->size());

	for (const pxr::TfToken& PxrPropertyName : *PxrPropertyNames)
	{
		PropertyNames.Add(UsdToUnreal::ConvertToken(PxrPropertyName));
	}
...
bool UsdUtils::PrimCollapses(
	const pxr::UsdPrim& Prim,
	EUsdDefaultKind KindsToCollapse,
	bool bUsePrimKindsForCollapsing,
	bool bUseSchemaForCollapsing,
	bool bCanCollapseByDefault
)
{
	// Prioritize checking for the schema if we have one
	if (bUseSchemaForCollapsing)
	{
		...
		switch (Preference)
		{
			...
			case UsdUtils::ECollapsingPreference::Never:
			{
				return false;
			}
			case UsdUtils::ECollapsingPreference::Default:	  // Fallthrough
			default:
			{
				break;
			}
...
FString UsdUtils::AddHashToPrimPath(const FString& PrimPath)
{
	// Pad to exactly 10 digits, so it's easy to strip
	// Adding the underscore makes it so that we can use this even on the pseudoroot path "/", without
	// making an invalid SdfPath in the process (as something like /6154761547 is not a valid prim path)
	return FString::Printf(TEXT("%s_%010u"), *PrimPath, UE::HashStringFNV1a32(MakeStringView(PrimPath)));
}

FString UsdUtils::RemoveHashFromPrimPath(const FString& HashedPrimPath)
{
	// Make sure this matches the number of digits on the Printf format above (+ underscore)!
	return HashedPrimPath.LeftChop(11);
}
...
namespace UE::UsdConversionUtils::Private
{
	...
	)
	{
		...
		if (RelativePath.IsEmpty() && !TargetPrimPath.IsPrimPath())
		{
			// Nothing to target
			return;
		}
...
TArray<FString> UsdUtils::GetReferenceOrPayloadFilePaths(const UE::FUsdPrim& Prim)
{
...
FString UsdUtils::SanitizeUsdIdentifier(const TCHAR* InIdentifier)
{
	...
	// We used to use pxr::TfMakeValidIdentifier() here, but it seems that that sanitizes away UTF8
	// characters, which are otherwise supported.
	//
	// Now we implement something based on _IsValidIdentifier() from USD's sdf\path.cpp instead,
	// as we need to actually produce a valid identifier and not only check if it's valid or not

	std::string Identifier = UnrealToUsd::ConvertString(InIdentifier).Get();
	if (Identifier.empty())
	{
		Identifier = "_";
	}
	else
	{
		...
		pxr::TfUtf8CodePointIterator Iter{std::cbegin(View), std::cend(View)};

		// First codepoint has a different rule
		const pxr::TfUtf8CodePoint& FirstCodePoint = *Iter;
		if (FirstCodePoint == pxr::TfUtf8CodePointFromAscii('_') || pxr::TfIsUtf8CodePointXidStart(FirstCodePoint))
		{
			Stream << FirstCodePoint;
		}
		else
		{
			Stream << '_';
		...
		// Sanitize remaining codepoints
		for (++Iter; Iter != pxr::TfUtf8CodePointIterator::PastTheEndSentinel{}; ++Iter)
		{
			const pxr::TfUtf8CodePoint& CodePoint = *Iter;
			if (TfIsUtf8CodePointXidContinue(CodePoint))
			{
				Stream << CodePoint;
			}
			else
	...
	return UsdToUnreal::ConvertString(Identifier);
#else
	return InIdentifier;
#endif	  // USE_USD_SDK
}
...
TArray<UE::FUsdPrim> UsdUtils::GetVisibleChildren(const UE::FUsdPrim& Prim, EUsdPurpose AllowedPurposes)
{
	...
	RecursivelyCollectVisibleMeshes = [&RecursivelyCollectVisibleMeshes, &VisiblePrims, AllowedPurposes](const pxr::UsdPrim& Prim)
	{
		...
		// We want to only collect meshes from visible purposes, but unlike for the visibility attribute itself,
		// we don't want to stop traversal due to a hidden purpose, as a child prim may have a different purpose
		// to its parent
		if (EnumHasAllFlags(AllowedPurposes, IUsdPrim::GetPurpose(Prim)))
		{
...
void UsdUtils::CollectSchemaAnalytics(const UE::FUsdStage& Stage, TArray<FAnalyticsEventAttribute>& OutAttributes)
{
#if USE_USD_SDK
	if (!Stage)
	{
	...
	const static TSet<FString> NativeSchemaNames{
		...
		"Capsule_1",
		"ChaosFabricAPI",
		"ChaosSolverPropertiesAPI",
		"ClipsAPI",
		"CloFabricAPI",
		"CloSolverPropertiesAPI",
		"ClothRootAPI",
		"ClothSolverAPI",
		"CollapsingAPI",
		"CollectionAPI",
	...
	TMap<FString, int32> InterestingNativeSchemaCounts{
		...
		{"MeshLightAPI", 0},
		{"MotionAPI", 0},
		{"NaniteAssemblyExternalRefAPI", 0},
		{"NaniteAssemblyRootAPI", 0},
		{"NaniteAssemblySkelBindingAPI", 0},
		{"NurbsCurves", 0},
		{"NurbsPatch", 0},
		{"OpenVDBAsset", 0},
	...
	{
		OutAttributes.Emplace(TEXT("CustomSchemas"), CustomSchemaCount);
	}
	...
	for (const TPair<FString, int32>& Pair : InterestingNativeSchemaCounts)
	{
		...
		if (Count > 0)
		{
			OutAttributes.Emplace(InterestingNativeSchemaName, Count);
		}
	}
#endif	  // USE_USD_SDK
}
