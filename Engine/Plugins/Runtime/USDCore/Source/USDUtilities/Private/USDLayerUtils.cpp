...
bool UsdUtils::IsZipArchive(const FString& InUSDZFilePath)
{
	if (TUniquePtr<FArchive> Ar = TUniquePtr<FArchive>(IFileManager::Get().CreateFileReader(*InUSDZFilePath)))
	{
		uint32 Magic = 0;
		if (Ar->TotalSize() >= 4)
		{
			Ar->Serialize(&Magic, sizeof(Magic));
			// ZIP local file header: 0x04034B50 (Little Endian for "PK\x03\x04")
...
UsdUtils::EUSDZDecompressResult UsdUtils::DecompressUSDZFile(const FString& InUSDZFilePath, const FString& InOutputDir, FString* OutDecompressedRootLayer)
{
	if (!IsZipArchive(InUSDZFilePath))
	{
		return EUSDZDecompressResult::Failed;
	}

	FScopedUsdAllocs Allocs;

	pxr::SdfZipFile File = pxr::SdfZipFile::Open(UnrealToUsd::ConvertString(*InUSDZFilePath).Get());
	if (!File)
	{
		return EUSDZDecompressResult::Failed;
	}

	auto TryMakeSafeExtractionPath = [](const FString& InExtractionRoot, const FString& InArchiveEntryPath, FString& OutSafeAbsolutePath) -> bool
	{
		ensure(!InArchiveEntryPath.IsEmpty());

		// Normalize path separators first so we can consistently reason about segments
		FString ArchiveEntryPath = InArchiveEntryPath;
		FPaths::NormalizeFilename(ArchiveEntryPath);

		// Reject rooted / absolute / drive-qualified / UNC-like paths. Note we don't
		// automatically reject relative '..' path components here, but instead wait
		// for the normalized absolute path to escape the extraction directory.
		if (ArchiveEntryPath.StartsWith(TEXT("/")) ||
			ArchiveEntryPath.StartsWith(TEXT("\\")) ||
			ArchiveEntryPath.Contains(TEXT(":")))
		{
			return false;
		}

		// Build canonical absolute extraction root
		FString ExtractionRoot = FPaths::ConvertRelativePathToFull(InExtractionRoot);
		FPaths::NormalizeDirectoryName(ExtractionRoot);

		// Build canonical absolute candidate output path
		FString CandidatePath = FPaths::Combine(ExtractionRoot, ArchiveEntryPath);
		CandidatePath = FPaths::ConvertRelativePathToFull(CandidatePath);
		FPaths::NormalizeFilename(CandidatePath);

		// Check that CandidatePath remains under ExtractionRoot
		FString ExtractionRootPrefix = ExtractionRoot;
		FPaths::NormalizeFilename(ExtractionRootPrefix);
		if (!ExtractionRootPrefix.EndsWith(TEXT("/")))
		{
			ExtractionRootPrefix += TEXT("/");
		}
		...
		if (!CandidatePath.StartsWith(ExtractionRootPrefix, PathSearchCase))
		{
			return false;
		}

		OutSafeAbsolutePath = CandidatePath;
		return true;
	};

	TMap<int32, FString> SafeTargetPathsByArchiveIndex;
	TSet<FString> UnsafeArchivePaths;

	bool bSawFirstFile = false;
	FString ValidatedRootLayerPath;

	// Pass 1: Validate the entire archive and compute final extraction paths.
	int32 ArchiveIndex = 0;
	for (pxr::SdfZipFile::Iterator Iter = File.begin(), End = File.end(); Iter != End; ++Iter, ++ArchiveIndex)
	{
		const pxr::SdfZipFile::FileInfo Info = Iter.GetFileInfo();
		const FString UEInnerFilePath = UsdToUnreal::ConvertString(*Iter);

		if (UEInnerFilePath.IsEmpty())
		{
			continue;
		}

		FString SafeTargetPath;
		if (!TryMakeSafeExtractionPath(InOutputDir, UEInnerFilePath, SafeTargetPath))
		{
			UnsafeArchivePaths.Add(UEInnerFilePath);

			USD_LOG_USERERROR(FText::Format(
				LOCTEXT("UnsafeUSDZEntryPath", "Cannot decompress USDZ file '{0}': Archive entry '{1}' attempts to write outside the extraction directory."),
		...
		// According to USDZ spec, the first file must be the root USD layer
		if (!bSawFirstFile)
		{
			bSawFirstFile = true;

			const bool bIncludeDot = false;
			const FString Extension = FPaths::GetExtension(UEInnerFilePath, bIncludeDot).ToLower();
			if (Extension.StartsWith(TEXT("usd")))
			{
				ValidatedRootLayerPath = SafeTargetPath;
			}
			else
			{
				USD_LOG_USERWARNING(FText::Format(
					LOCTEXT("FailedToDecompress", "Failed to decompress {0}: First file '{1}' should always be a USD file!"),
					FText::FromString(InUSDZFilePath),
					FText::FromString(UEInnerFilePath)
				));
				return EUSDZDecompressResult::Failed;
		...
		SafeTargetPathsByArchiveIndex.Add(ArchiveIndex, SafeTargetPath);
	}

	if (!bSawFirstFile || ValidatedRootLayerPath.IsEmpty() || !UnsafeArchivePaths.IsEmpty())
	{
		return EUSDZDecompressResult::Failed;
	}
	...
	// For now we don't support extracting to a directory that exists. Symlinks and junctions 
	// inside an existing directory can lead to unsafe extractions without additional checks.
	if (FileManager.DirectoryExists(*InOutputDir))
	{
		USD_LOG_USERERROR(FText::Format(
			LOCTEXT("USDZDirectoryExists", "Cannot decompress USDZ file '{0}': Target directory {1} already exists."),
			FText::FromString(InUSDZFilePath),
			FText::FromString(InOutputDir)
		));
		return EUSDZDecompressResult::Failed;
	}

	const bool bTree = true;
	if (!FileManager.MakeDirectory(*InOutputDir, bTree))
	{
		USD_LOG_USERERROR(FText::Format(
			LOCTEXT("USDZFailedToCreateDirectory", "Cannot decompress USDZ file '{0}': Failed to create target directory {1}."),
			FText::FromString(InUSDZFilePath),
			FText::FromString(InOutputDir)
		));
		return EUSDZDecompressResult::Failed;
	...
	EUSDZDecompressResult Result = SafeTargetPathsByArchiveIndex.Num() == ArchiveIndex
		? EUSDZDecompressResult::Success
		: EUSDZDecompressResult::SuccessWithSkippedEntries;

	// Pass 2: Extract using only pre-validated target paths.
	ArchiveIndex = 0;
	for (pxr::SdfZipFile::Iterator Iter = File.begin(), End = File.end(); Iter != End; ++Iter, ++ArchiveIndex)
	{
		const FString* SafeTargetPath = SafeTargetPathsByArchiveIndex.Find(ArchiveIndex);
		if (SafeTargetPath == nullptr)
		{
			continue;
		}
		...
		const std::size_t TargetFileSizeBytes = Info.size;

		// Create any nested folder (usually a folder named "0")
		if (!IFileManager::Get().MakeDirectory(*FPaths::GetPath(*SafeTargetPath), bTree))
		{
			Result = EUSDZDecompressResult::Failed;
			break;
		}

		TUniquePtr<FArchive> Ar{ IFileManager::Get().CreateFileWriter(**SafeTargetPath) };
		if (!Ar)
		{
			Result = EUSDZDecompressResult::Failed;
			break;
		...
		Ar->Serialize((void*)FileStart, TargetFileSizeBytes);
		if(!Ar->Close())
		{
			Result = EUSDZDecompressResult::Failed;
			break;
	...
	if (Result != EUSDZDecompressResult::Failed)
	{
		if (OutDecompressedRootLayer != nullptr)
		{
			*OutDecompressedRootLayer = ValidatedRootLayerPath;
		}
	}
	else
	{ 
		if (!FileManager.DeleteDirectory(*InOutputDir, /*RequireExists=*/ true, /*Tree=*/ true))
		{
			USD_LOG_USERWARNING(FText::Format(
				LOCTEXT("USDZFailedToCleanup", "Failed to clean up directory {0} while decompressing USDZ file '{1}'."),
				FText::FromString(InOutputDir),
				FText::FromString(InUSDZFilePath)
			));
		}
...
UE::FSdfLayer UsdUtils::GetUEPersistentStateSublayer(const UE::FUsdStage& Stage, bool bCreateIfNeeded)
{
	...
	if (!StateLayer && bCreateIfNeeded)
	{
		StateLayer = pxr::SdfLayer::New(
			pxr::SdfFileFormat::FindById(pxr::SdfUsdaFileFormatTokens->Id),
