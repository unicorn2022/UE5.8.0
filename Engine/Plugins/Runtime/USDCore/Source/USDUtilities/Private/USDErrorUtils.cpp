...
namespace UE::USDErrorUtils::Private
{
	...
	{
		// This FName constructor takes an int32. Unfortunately, there are now some new validation 
		// on FNames that asserts if we try using negative integers, so here we mask off the top bit
		// to ensure the int32 becomes positive.
		//
		// See also FName::IsValidInternalNumber and NAME_NO_NUMBER_INTERNAL
		return FName(TEXT("USD_LOG_ID"), Number & 0x7FFFFFFFu);
	}
	...
	void SendMessageToOutputLog(const FTokenizedMessage& Message)
	{
		...
		{
			UE_LOGF(LogUsd, Error, "%ls", *(Message.ToText().ToString()));
		}
		else if (Message.GetSeverity() == EMessageSeverity::Warning || Message.GetSeverity() == EMessageSeverity::PerformanceWarning)
		{
			UE_LOGF(LogUsd, Warning, "%ls", *(Message.ToText().ToString()));
		}
		else
		{
			UE_LOGF(LogUsd, Log, "%ls", *(Message.ToText().ToString()));
		}
	}
	...
	public:
		...
		FUsdCombinedMessages(FUsdLogManager::FMessageDisplayCallback InCallback)
			: MessageDisplayCallback(InCallback)
		{
		...
		void DisplayMessages()
		{
			...
			if (MessageDisplayCallback)
			{
				MessageDisplayCallback(TokenizedMessages);
			}
#endif	  // WITH_EDITOR
		}
	...
	private:
		...
		FUsdLogManager::FMessageDisplayCallback MessageDisplayCallback;
	};
...
void FUsdLogManager::DisplayMessagesOnMessageLog(const TArray<TSharedRef<FTokenizedMessage>>& Messages)
{
#if WITH_EDITOR
	if (Messages.Num() == 0)
	{
	...
	LogListing->AddMessages(Messages, bMirrorToOutputLog);
...
FScopedUsdMessageLog::FScopedUsdMessageLog(FUsdLogManager::FMessageDisplayCallback InCallback)
{
#if USE_USD_SDK
	using namespace UE::USDErrorUtils::Private;
	...
	{
		CombinedMessages = MakeUnique<FUsdCombinedMessages>(InCallback);
		PushErrorMark();
	}
