...
namespace UsdToUnreal::NaniteAssemblyUtils
{
	...
	{
		UE_LOGF(LogUsd, Log, "// -----------------------------------------------------------------------------------");
		UE_LOGF(LogUsd, Log, "// FMeshEntry ");
		UE_LOGF(LogUsd, Log, "// .. catg: %d", EnumToUnderlyingType(Category));
		UE_LOGF(LogUsd, Log, "// .. path: %ls", *PrimPath.GetString());
		UE_LOGF(LogUsd, Log, "// ..  uid: %ls", *NodeUid);
		for (int32 Index = 0; Index < TransformStack.Num(); ++Index)
		{
			...
			UE_LOGF(LogUsd, Log, "// ..   xf: [%d]  T: %ls  R: %ls S: %ls", Index, *T.ToString(), *R.ToString(), *S.ToString());
		}
		for (int32 Index = 0; Index < PrototypeStack.Num(); ++Index)
		{
			const FPrimPrototypeEntry& Proto = PrototypeStack[Index];
			UE_LOGF(LogUsd, Log, "// ..   pi: [%d] %ls", Index, *Proto.PointInstancerPrimPath.GetString());
			UE_LOGF(LogUsd, Log, "//           `-> %ls", *Proto.PrototypePrimPath.GetString());
		}
	}
		...
		const TMap<FString, int32>* JointIndexByJointNameTable)
	{
		...
		int32 ElementSize = INDEX_NONE;
		{
			...
			if (BindJoints.empty())
			{
				...
				{
					USD_LOG_WARNING(
						TEXT("Prim '%s' with schema '%s' has empty primvar '%s' which does not match the number of instances (%zu).")
						, *UsdToUnreal::ConvertPath(Prim.GetPath())
						, *UsdToUnreal::ConvertToken(UnrealIdentifiers::NaniteAssemblySkelBindingAPI)
						, *UsdToUnreal::ConvertToken(UnrealIdentifiers::PrimvarsUnrealNaniteAssemblyBindJoints)
						, NumInstances
					...
					else
					{
						USD_LOG_WARNING(
							TEXT("Element size (%d) for Nanite assembly pointinstancer primvar '%s.%s' is invalid for the "
							     "number of bindings (%d) compared to the number of point instances (%zu)")
							, ElementSize
							, *UsdToUnreal::ConvertPath(Prim.GetPath())
							, *UsdToUnreal::ConvertToken(UnrealIdentifiers::PrimvarsUnrealNaniteAssemblyBindJoints)
							, NumBindings
		...
		int32 PartIndexStartOffset)
	{
		...
		{
			UE_LOGF(LogUsd, Warning
				, "Invalid Nanite assembly pointinstancer '%ls' with different transforms and "
		...
		if (NumInvalidPrototypeIndices > 0)
		{
			...
			{
				UE_LOGF(LogUsd, Warning
					, "Ignoring Nanite assembly pointinstancer '%ls' - all (%d) instance points referred to non-existant prototype paths."
					, *PointInstancerPrim.GetPrimPath().GetString()
					, NumInvalidPrototypeIndices
				);
				return {};
			...
			{
				UE_LOGF(LogUsd, Warning
					, "(%d) point(s) in Nanite assembly pointinstancer '%ls' refer to one or more invalid prototypes - some instances will be missing."
					, NumInvalidPrototypeIndices
					, *PointInstancerPrim.GetPrimPath().GetString()
				);
			}
