...
bool UsdUtils::HasAuthoredDefaultOpinion(const UE::FUsdAttribute& Attribute)
{
#if USE_USD_SDK
	pxr::UsdAttribute PxrAttr{Attribute};

	if (!PxrAttr.HasAuthoredValue())
	{
		return false;
	}

	pxr::UsdResolveInfo Info = PxrAttr.GetResolveInfo(pxr::UsdTimeCode::Default());
...
bool UsdUtils::HasAuthoredAnimatedOpinion(const UE::FUsdAttribute& Attribute)
{
	return Attribute.HasAuthoredValue() && Attribute.ValueMightBeTimeVarying();
}
