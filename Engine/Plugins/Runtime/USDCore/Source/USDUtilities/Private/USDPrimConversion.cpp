...
namespace UE::USDPrimConversion::Private
{
	...
	pxr::UsdGeomXformOp ForceMatrixXform(pxr::UsdGeomXformable& Xformable)
	{
		...
		// If we have other xformOps, let's convert them to matrix xforms as well.
		// This is important as we can end up in edge cases like UE-221328, where we may have some timeSamples for different
		// xformOps that are fully lost when we author a single default opinion for a matrix op, as we'll also force the
		// xformOpOrder to just use our matrix op
		bool bResetsXformStack = false;
		std::vector<pxr::UsdGeomXformOp> ExistingOps = Xformable.GetOrderedXformOps(&bResetsXformStack);
		if (ExistingOps.size() > 0)
		{
			// If there's exactly one op and it's already xformOp:transform, then we don't need to do any conversion
			bool bNeedConversion = false;
			if (ExistingOps.size() > 1)
			{
				bNeedConversion = true;
			}
			else if (ExistingOps[0].GetOpType() != pxr::UsdGeomXformOp::TypeTransform || ExistingOps[0].IsInverseOp())
			{
				bNeedConversion = true;
			}
			...
			if (bNeedConversion)
			{
				// Convert default opinion if any of the Ops have one
				bool bHasDefault = false;
				for (const pxr::UsdGeomXformOp& Op : ExistingOps)
				{
					const pxr::UsdAttribute& OpAttr = Op;
					bHasDefault = OpAttr.GetResolveInfo(pxr::UsdTimeCode::Default()).HasAuthoredValue();
					if (bHasDefault)
					{
						break;
					}
				...
				if (bHasDefault)
				{
					// TODO: Proper handling of bResetsXformStack
					pxr::GfMatrix4d Transform;
					bool bResetsXformStackDefault = false;
					if (Xformable.GetLocalTransformation(&Transform, &bResetsXformStackDefault, pxr::UsdTimeCode::Default()))
					{
						TransformAttr.Set(Transform, pxr::UsdTimeCode::Default());
					}
				}

				// Convert timeSamples
				std::vector<double> TimeSamples;
				if (Xformable.GetTimeSamples(&TimeSamples) && TimeSamples.size() > 0)
				{
					for (double TimeSample : TimeSamples)
					{
						pxr::GfMatrix4d Transform;
						bool bResetsXformStackTimeSample = false;
						if (Xformable.GetLocalTransformation(&Transform, &bResetsXformStackTimeSample, TimeSample))
						{
							TransformAttr.Set(Transform, TimeSample);
						}
					}
				}
			}
...
bool UsdToUnreal::ConvertGeomCamera(const UE::FUsdPrim& Prim, UCineCameraComponent& CameraComponent, double UsdTimeCode)
{
	...
	{
		float UsdFocusDistance = UsdToUnreal::ConvertDistance(
			StageInfo,
			UsdUtils::GetUsdValue<float>(GeomCamera.GetFocusDistanceAttr(), UsdTimeCode)
		);

		CameraComponent.FocusSettings.ManualFocusDistance = UsdFocusDistance;

		if (!FMath::IsNearlyZero(UsdFocusDistance))
		{
			bNeedsManualFocus = true;
		}
	}

	// Regular 'fStop' is now meant to be used for DoF effects according to the latest schema
	if (pxr::UsdAttribute Attr = GeomCamera.GetFStopAttr(); Attr.HasAuthoredValue())
	{
		...
		CameraComponent.PostProcessSettings.DepthOfFieldFstop = UsdFStop;
		bNeedsManualFocus = true;
	...
	// 'exposure:fStop' is the the value that is meant to affect exposure
	if (pxr::UsdAttribute Attr = GeomCamera.GetExposureFStopAttr(); Attr.HasAuthoredValue())
	{
		const float UsdFStop = UsdUtils::GetUsdValue<float>(Attr, UsdTimeCode);
	...
	if (pxr::UsdAttribute Attr = GeomCamera.GetExposureAttr(); Attr.HasAuthoredValue())
	{
		...
		// We don't necessarily want manual exposure just because we have this: It still works with auto exposure
	}
	...
	{
		CameraComponent.PostProcessSettings.bOverride_CameraISO = true;
		CameraComponent.PostProcessSettings.CameraISO = UsdUtils::GetUsdValue<float>(Attr, UsdTimeCode);
		bNeedsManualExposure = true;
	}

	if (pxr::UsdAttribute Attr = GeomCamera.GetExposureTimeAttr(); Attr.HasAuthoredValue())
	{
		float UsdTimeSeconds = UsdUtils::GetUsdValue<float>(Attr, UsdTimeCode);
		if (!FMath::IsNearlyZero(UsdTimeSeconds))
		{
			CameraComponent.PostProcessSettings.bOverride_CameraShutterSpeed = true;
			CameraComponent.PostProcessSettings.CameraShutterSpeed = 1.0f / UsdTimeSeconds;
		}
		bNeedsManualExposure = true;
	}
...
)
{
	...
	if (pxr::UsdGeomCamera Camera{UsdPrim})
	{
		...
		else if (PropertyPath == UnrealIdentifiers::CurrentAperturePropertyName)
		{
			...
			Attr = Camera.GetExposureFStopAttr();
		}
		...
		else if (PropertyPath == UnrealIdentifiers::DepthOfFieldFstopPropertyName)
		{
			bConvertDistance = false;
			Attr = Camera.GetFStopAttr();
		}
		else if (PropertyPath == UnrealIdentifiers::CameraISOPropertyName)
		{
			bConvertDistance = false;
			Attr = Camera.GetExposureIsoAttr();
		}
		else if (PropertyPath == UnrealIdentifiers::CameraShutterSpeedPropertyName)
		{
			bInvertValue = true;
			bConvertDistance = false;
			Attr = Camera.GetExposureTimeAttr();
		}
			...
			else
			{
				...
				if (bInvertValue)
				{
					Reader.FloatReader = [Attr, Default](double UsdTimeCode)
					{
						...
						return Result != 0.0f ? (1.0f / Result) : 0.0f;
					};
				}
				else
				{
	...
	else if (const pxr::UsdLuxLightAPI LightAPI{Prim})
	{
			...
			else if (UsdPrim.HasAPI<pxr::UsdLuxShapingAPI>())
			{
				...
				if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
				{
					...
					if (IntensityAttr && ExposureAttr)
					{
						...
						float Default = UsdToUnreal::ConvertLightIntensity(DefaultUsdIntensity, DefaultUsdExposure);
						...
						Reader.FloatReader = [IntensityAttr, ExposureAttr, Default, StageInfo](double UsdTimeCode)
						{
							...
							{
								Result = UsdToUnreal::ConvertLightIntensity(UsdIntensity, UsdExposure);
							}
				...
				else if (PropertyPath == UnrealIdentifiers::InnerConeAnglePropertyName)
				{
					...
					if (ConeAngleAttr && ConeSoftnessAttr)
					{
						...
						float DefaultInnerConeAngle = 0.0f;
						float DefaultOuterConeAngle = 0.0f;
						UsdToUnreal::ConvertSpotLightConeAngles(DefaultUsdConeAngle, DefaultUsdConeSoftness, DefaultOuterConeAngle, DefaultInnerConeAngle);

						Reader.FloatReader = [ConeAngleAttr, ConeSoftnessAttr, DefaultInnerConeAngle](double UsdTimeCode)
						{
							float Result = DefaultInnerConeAngle;
							...
							if (ConeAngleAttr.Get<float>(&UsdConeAngle, UsdTimeCode) && ConeSoftnessAttr.Get<float>(&UsdConeSoftness, UsdTimeCode))
							{
								...
								UsdToUnreal::ConvertSpotLightConeAngles(UsdConeAngle, UsdConeSoftness, OuterConeAngle, Result);
							}
		...
		else if (pxr::UsdLuxDiskLight DiskLight{UsdPrim})
		{
			if (PropertyPath == UnrealIdentifiers::SourceWidthPropertyName || PropertyPath == UnrealIdentifiers::SourceHeightPropertyName)
			{
				...
				{
					// The convention is that we fake a disk light with a rect light. For area lights the most important thing
					// is to preserve the area though, so here we compute the side of the rect light that would produce the same
					// total area as the disk
					float Default = 0.0;
					Attr.Get<float>(&Default);
					float UsdArea = PI * Default * Default;
					float UsdSquareSide = FMath::Sqrt(UsdArea);
					Default = UsdToUnreal::ConvertDistance(StageInfo, UsdSquareSide);
					...
					Reader.FloatReader = [Attr, Default, StageInfo](double UsdTimeCode)
					{
						...
						if (Attr.Get<float>(&Result, UsdTimeCode))
						{
							float UsdArea = PI * Result * Result;
							float UsdSquareSide = FMath::Sqrt(UsdArea);
							Result = UsdToUnreal::ConvertDistance(StageInfo, UsdSquareSide);
						}
		...
		else if (pxr::UsdLuxDistantLight DistantLight{UsdPrim})
		{
			...
			{
				pxr::UsdAttribute IntensityAttr = DistantLight.GetIntensityAttr();
				pxr::UsdAttribute ExposureAttr = DistantLight.GetExposureAttr();
		...
		else if (UsdPrim.IsA<pxr::UsdLuxDomeLight>() || UsdPrim.IsA<pxr::UsdLuxDomeLight_1>())
		{
			pxr::UsdLuxNonboundableLightBase LightBase{UsdPrim};

			if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
			{
				pxr::UsdAttribute IntensityAttr = LightBase.GetIntensityAttr();
				pxr::UsdAttribute ExposureAttr = LightBase.GetExposureAttr();
...
bool UnrealToUsd::ConvertCameraComponent(const UCineCameraComponent& CameraComponent, pxr::UsdPrim& Prim, double UsdTimeCode)
{
	...
	{
		if (CameraComponent.PostProcessSettings.bOverride_DepthOfFieldFstop)
		{
			if (pxr::UsdAttribute Attr = GeomCamera.CreateFStopAttr())
			{
				Attr.Set<float>(CameraComponent.PostProcessSettings.DepthOfFieldFstop, UsdTimeCode);
				UsdUtils::NotifyIfOverriddenOpinion(Attr);
			}
		}
	...
	{
		if (pxr::UsdAttribute Attr = GeomCamera.CreateExposureAttr())
		{
	...
	if (CameraComponent.PostProcessSettings.bOverride_AutoExposureMethod && CameraComponent.PostProcessSettings.AutoExposureMethod == EAutoExposureMethod::AEM_Manual)
	{
		if (pxr::UsdAttribute Attr = GeomCamera.CreateExposureFStopAttr())
		{
			Attr.Set<float>(CameraComponent.CurrentAperture, UsdTimeCode);
			UsdUtils::NotifyIfOverriddenOpinion(Attr);
		}

		if (CameraComponent.PostProcessSettings.bOverride_CameraISO)
		{
			if (pxr::UsdAttribute Attr = GeomCamera.CreateExposureIsoAttr())
			{
				Attr.Set<float>(CameraComponent.PostProcessSettings.CameraISO, UsdTimeCode);
				UsdUtils::NotifyIfOverriddenOpinion(Attr);
			}
		}

		if (CameraComponent.PostProcessSettings.bOverride_CameraShutterSpeed)
		{
			if (pxr::UsdAttribute Attr = GeomCamera.CreateExposureTimeAttr())
			{
				float UsdValue = CameraComponent.PostProcessSettings.CameraShutterSpeed != 0.0f ? (1.0f / CameraComponent.PostProcessSettings.CameraShutterSpeed) : 0.0f;
				Attr.Set<float>(UsdValue, UsdTimeCode);
				UsdUtils::NotifyIfOverriddenOpinion(Attr);
			}
		}
	}
...
)
{
	...
	if (const UGeometryCache* GeometryCache = Cast<const UGeometryCache>(MeshAsset))
	{
		for (int32 MatIndex = 0; MatIndex < MaterialOverrides.Num(); ++MatIndex)
		{
			...
			const UUsdMeshAssetUserData* UserData = bCheckAssetUserData
	...
	else if (const UStaticMesh* StaticMesh = Cast<const UStaticMesh>(MeshAsset))
	{
		...
		const UUsdMeshAssetUserData* UserData = bCheckAssetUserData
			? const_cast<UStaticMesh*>(StaticMesh)->GetAssetUserData<UUsdMeshAssetUserData>() 
			: nullptr;
...
)
{
	...
	TFunction<float(float)> InvertConversion = [StageInfo](float UEValue)
	{
		return UEValue != 0.0f ? (1.0f / UEValue) : 0.0f;
	};
	...
	if (const UCineCameraComponent* CameraComponent = Cast<UCineCameraComponent>(&Component))
	{
		...
		if (RelevantProperties.Contains(PropertyPath))
		{
			...
			pxr::UsdAttribute ExposureFStopAttr = GeomCamera.CreateExposureFStopAttr();
			pxr::UsdAttribute ExposureISOAttr = GeomCamera.CreateExposureIsoAttr();
			pxr::UsdAttribute ExposureTimeAttr = GeomCamera.CreateExposureTimeAttr();
			...
			TFunction<void(float, double)> ExposureFStopSetter = CreateCachedAttrSetter(ExposureFStopAttr, NoFloatConversion);
			TFunction<void(float, double)> ExposureISOSetter = CreateCachedAttrSetter(ExposureISOAttr, NoFloatConversion);
			TFunction<void(float, double)> ExposureTimeSetter = CreateCachedAttrSetter(ExposureTimeAttr, InvertConversion);
			...
			BakerFunction = [DefaultUEFarPlane,
							 ...
							 ExposureFStopSetter,
							 ExposureISOSetter,
							 ExposureTimeSetter](double UsdTimeCode) mutable
			{
				...
				if (CameraComponent->PostProcessSettings.bOverride_DepthOfFieldFstop)
				{
					FStopSetter(CameraComponent->PostProcessSettings.DepthOfFieldFstop, UsdTimeCode);
				}
				...
				ExposureFStopSetter(CameraComponent->CurrentAperture, UsdTimeCode);

				if (CameraComponent->PostProcessSettings.bOverride_CameraISO)
				{
					ExposureISOSetter(CameraComponent->PostProcessSettings.CameraISO, UsdTimeCode);
				}

				if (CameraComponent->PostProcessSettings.bOverride_CameraShutterSpeed)
				{
					ExposureTimeSetter(CameraComponent->PostProcessSettings.CameraShutterSpeed, UsdTimeCode);
				}
			};
		}
	}
	else if (const ULightComponentBase* LightComponentBase = Cast<ULightComponentBase>(&Component))
	{
		...
		static IConsoleVariable* MinLightSourceSizeCvar = IConsoleManager::Get().FindConsoleVariable(TEXT("USD.LightExport.MinLightSourceSize"));
		const float MinLightSourceSize = MinLightSourceSizeCvar ? MinLightSourceSizeCvar->GetFloat() : 1.0f;
		...
		if (const URectLightComponent* RectLightComponent = Cast<URectLightComponent>(LightComponentBase))
		{
			...
			if (RelevantProperties.Contains(PropertyPath))
			{
				...
				TFunction<void(float, double)> HeightSetter = CreateCachedAttrSetter(HeightAttr, DistanceConversion);
				...
				BakerFunction = [ComponentGetter,
								 ...
								 WidthSetter,
								 HeightSetter,
								 MinLightSourceSize](double UsdTimeCode) mutable
				{
					...
					const float SourceWidthUE = FMath::Max(RectLightComponent->SourceWidth, MinLightSourceSize);
					const float SourceHeightUE = FMath::Max(RectLightComponent->SourceHeight, MinLightSourceSize);
					const float SolidAngle = UsdUtils::GetRectLightSolidAngle();
					const float AreaInSqMeters = UsdUtils::GetRectLightArea(SourceWidthUE / 100.0f, SourceHeightUE / 100.0f);
					const float FinalIntensityNits = UsdUtils::ConvertIntensityToNits(
						RectLightComponent->Intensity,
						SolidAngle,
						AreaInSqMeters,
					...
					WidthSetter(SourceWidthUE, UsdTimeCode);
					HeightSetter(SourceHeightUE, UsdTimeCode);
		...
		else if (const USpotLightComponent* SpotLightComponent = Cast<USpotLightComponent>(LightComponentBase))
		{
			...
			if (RelevantProperties.Contains(PropertyPath))
			{
								 ...
								 MinLightSourceSize](double UsdTimeCode) mutable
				{
					...
					const float SourceRadiusUE = FMath::Max(SpotLightComponent->SourceRadius, MinLightSourceSize);
					const float SolidAngle = UsdUtils::GetSpotLightSolidAngle(SpotLightComponent->OuterConeAngle, SpotLightComponent->InnerConeAngle);
					const float AreaInSqMeters = UsdUtils::GetSpotLightArea(SourceRadiusUE / 100.0f);
					const float FinalIntensityNits = UsdUtils::ConvertIntensityToNits(
					...
					RadiusSetter(SourceRadiusUE, UsdTimeCode);
					
					float UsdConeAngle = 90.0;
					float UsdConeSoftness = 0.0f;
					UnrealToUsd::ConvertSpotLightConeAngles(SpotLightComponent->OuterConeAngle, SpotLightComponent->InnerConeAngle, UsdConeAngle, UsdConeSoftness);
					AngleSetter(UsdConeAngle, UsdTimeCode);
					SoftnessSetter(UsdConeSoftness, UsdTimeCode);
		...
		else if (const UPointLightComponent* PointLightComponent = Cast<UPointLightComponent>(LightComponentBase))
		{
			...
			if (RelevantProperties.Contains(PropertyPath))
			{
								 ...
								 MinLightSourceSize](double UsdTimeCode) mutable
				{
					...
					const float SourceRadiusUE = FMath::Max(PointLightComponent->SourceRadius, MinLightSourceSize);
					const float SolidAngle = UsdUtils::GetPointLightSolidAngle();
					const float AreaInSqMeters = UsdUtils::GetPointLightArea(SourceRadiusUE / 100.0f);
					const float FinalIntensityNits = UsdUtils::ConvertIntensityToNits(
...
)
{
	...
	pxr::UsdAttribute& Attr = Attrs[0];
	{
		...
		if (pxr::UsdGeomCamera Camera{UsdPrim})
		{
			...
			{
				bConvertDistance = false;
				Attr = Camera.CreateExposureFStopAttr();
			}
			else if (PropertyPath == UnrealIdentifiers::SensorWidthPropertyName)
			{
			...
			{
				bConvertDistance = false;
				Attr = Camera.CreateExposureIsoAttr();
			}
			else if (PropertyPath == UnrealIdentifiers::CameraShutterSpeedPropertyName)
			{
				bConvertDistance = false;
				bInvertValue = true;
				Attr = Camera.CreateExposureTimeAttr();
			}
			...
			if (Attr)
			{
				...
				else if (bInvertValue)
				{
					Result.FloatWriter = [Attr](float UEValue, double UsdTimeCode) mutable
					{
						Attr.Set(UEValue != 0.0f ? (1.0f / UEValue) : 0.0f, UsdTimeCode);
					};
				}
				else
				{
		...
		else if (pxr::UsdLuxLightAPI LightAPI{Prim})
		{
			...
			else if (pxr::UsdLuxSphereLight SphereLight{UsdPrim})
			{
				...
				else if (UsdPrim.HasAPI<pxr::UsdLuxShapingAPI>())
				{
					...
					if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
					{
						...
						ELightUnits Units = ELightUnits::Nits;
						...
						{
							Result.FloatWriter =
								[Attr, RadiusAttr, ConeAngleAttr, ConeSoftnessAttr, StageInfo, Units](float UEIntensity, double UsdTimeCode)
							{
								...
								const float RadiusMeters = UsdToUnreal::ConvertDistance(StageInfo, UsdRadius) / 100.0f;
								const float AreaSqMeters = UsdUtils::GetSpotLightArea(RadiusMeters);
								const float SolidAngle = UsdUtils::GetSpotLightSolidAngle(OuterConeAngle, InnerConeAngle);
								const float UsdIntensity = UsdUtils::ConvertIntensityToNits(UEIntensity, SolidAngle, AreaSqMeters, Units);

								Attr.Set(UsdIntensity, UsdTimeCode);
							};
						}
					}
					else if (PropertyPath == UnrealIdentifiers::OuterConeAnglePropertyName)
					...
					else if (PropertyPath == UnrealIdentifiers::InnerConeAnglePropertyName)
					{
						...
						{
							Result.FloatWriter = [Attr, ConeAngleAttr](float UEInnerConeAngle, double UsdTimeCode)
							{
								// Awkwardly, to write the cone softness we need not only the inner cone angle, but
								// the outer cone angle too. We know that the outer cone angle matches the USD cone angle,
								// so here we just read the angle from USD
								float UsdConeAngle = UsdUtils::GetUsdValue<float>(ConeAngleAttr, UsdTimeCode);
								float OuterConeAngle = UsdConeAngle;

								float UsdConeSoftness = 0.0f;
								UnrealToUsd::ConvertSpotLightConeAngles(OuterConeAngle, UEInnerConeAngle, UsdConeAngle, UsdConeSoftness);

								Attr.Set(UsdConeSoftness, UsdTimeCode);
							};
						}
					}
				}
				...
				else
				{
					if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
					{
						...
						if (Attr && RadiusAttr)
						{
							Result.FloatWriter = [Attr, RadiusAttr, StageInfo, Units](float UEIntensity, double UsdTimeCode)
							{
								const float UsdRadius = UsdUtils::GetUsdValue<float>(RadiusAttr, UsdTimeCode);

								const float RadiusMeters = UsdToUnreal::ConvertDistance(StageInfo, UsdRadius) / 100.0f;
								const float SolidAngle = UsdUtils::GetPointLightSolidAngle();
								const float AreaSqMeters = UsdUtils::GetPointLightArea(RadiusMeters);
								const float UsdIntensity = UsdUtils::ConvertIntensityToNits(UEIntensity, SolidAngle, AreaSqMeters, Units);
			...
			else if (pxr::UsdLuxRectLight RectLight{UsdPrim})
			{
				...
				else if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
				{
					...
					{
						Result.FloatWriter = [Attr, WidthAttr, HeightAttr, StageInfo, Units](float UEIntensity, double UsdTimeCode)
						{
							const float UsdWidth = UsdUtils::GetUsdValue<float>(WidthAttr, UsdTimeCode);
							const float UsdHeight = UsdUtils::GetUsdValue<float>(HeightAttr, UsdTimeCode);

							const float WidthMeters = UsdToUnreal::ConvertDistance(StageInfo, UsdWidth) / 100.0f;
							const float HeightMeters = UsdToUnreal::ConvertDistance(StageInfo, UsdHeight) / 100.0f;
							const float SolidAngle = UsdUtils::GetRectLightSolidAngle();
			...
			else if (pxr::UsdLuxDiskLight DiskLight{UsdPrim})
			{
				if (PropertyPath == UnrealIdentifiers::SourceWidthPropertyName || PropertyPath == UnrealIdentifiers::SourceHeightPropertyName)
				{
					...
					if (Attr)
					{
						...
						Result.FloatWriter = [Attr, StageInfo](float UEValue, double UsdTimeCode)
						{
							float UEArea = UEValue * UEValue;
							float UERadius = FMath::Sqrt(UEArea / PI);
							Attr.Set(UnrealToUsd::ConvertDistance(StageInfo, UERadius), UsdTimeCode);
						};
					}
				}
				else if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
				{
					Attr = DiskLight.GetIntensityAttr();
					pxr::UsdAttribute RadiusAttr = DiskLight.GetRadiusAttr();

					// Always clear exposure because we'll put all of our "light intensity" on the intensity attr and assume exposure
					// is zero, as we can't manipulate something like that exposure directly from UE anyway
					if (pxr::UsdAttribute ExposureAttr = DiskLight.GetExposureAttr())
					{
						ExposureAttr.Clear();
					}
					...
					if (Attr && RadiusAttr)
					{
						Result.FloatWriter = [Attr, RadiusAttr, StageInfo, Units](float UEIntensity, double UsdTimeCode)
						{
							...
							const float RadiusMeters = UsdToUnreal::ConvertDistance(StageInfo, UsdRadius) / 100.0f;
							const float SolidAngle = UsdUtils::GetDiskLightSolidAngle();
							const float AreaSqMeters = UsdUtils::GetDiskLightArea(RadiusMeters);
							const float UsdIntensity = UsdUtils::ConvertIntensityToNits(UEIntensity, SolidAngle, AreaSqMeters, Units);
			...
			else if (pxr::UsdLuxDistantLight DistantLight{UsdPrim})
			{
				...
				else if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
				{
					...
					// Always clear exposure because we'll put all of our "light intensity" on the intensity attr and assume exposure
					// is zero, as we can't manipulate something like that exposure directly from UE anyway
					if (pxr::UsdAttribute ExposureAttr = DistantLight.GetExposureAttr())
					{
						ExposureAttr.Clear();
					}
					...
					if (Attr)
					{
						Result.FloatWriter = [Attr](float UEIntensity, double UsdTimeCode)
						{
							// Both UE and USD exclusively use lux for directional light intensity
							Attr.Set(UEIntensity, UsdTimeCode);
						};
					}
				}
			}
			else if (pxr::UsdLuxDomeLight DomeLight{UsdPrim})
			{
				if (PropertyPath == UnrealIdentifiers::IntensityPropertyName)
				{
					Attr = DomeLight.GetIntensityAttr();

					// Always clear exposure because we'll put all of our "light intensity" on the intensity attr and assume exposure
					// is zero, as we can't manipulate something like that exposure directly from UE anyway
					if (pxr::UsdAttribute ExposureAttr = DomeLight.GetExposureAttr())
					{
						ExposureAttr.Clear();
					}
					...
					if (Attr)
					{
						Result.FloatWriter = [Attr](float UEIntensity, double UsdTimeCode)
						{
							// Both UE and USD use a flat multiplier for dome/skylight intensity
							Attr.Set(UEIntensity, UsdTimeCode);
						};
					}
				}
...
TArray<UE::FUsdAttribute> UsdUtils::GetAttributesForProperty(const UE::FUsdPrim& Prim, const FName& PropertyPath)
{
	...
	if (PropertyPath == TransformPropertyName)
	{
		...
		if (pxr::UsdGeomCapsule_1 Capsule_1{UsdPrim})
		{
			Attrs.Emplace(Capsule_1.GetHeightAttr());
			Attrs.Emplace(Capsule_1.GetRadiusTopAttr());
		}
		...
		else if (pxr::UsdGeomCylinder_1 Cylinder_1{UsdPrim})
		{
			Attrs.Emplace(Cylinder_1.GetHeightAttr());
			Attrs.Emplace(Cylinder_1.GetRadiusTopAttr());
		}
	...
	{
		return {UE::FUsdAttribute{UsdPrim.GetAttribute(pxr::UsdGeomTokens->exposureFStop)}};
	}
	...
	else if (PropertyPath == DepthOfFieldFstopPropertyName)
	{
		return {UE::FUsdAttribute{UsdPrim.GetAttribute(pxr::UsdGeomTokens->fStop)}};
	}
	else if (PropertyPath == CameraISOPropertyName)
	{
		return {UE::FUsdAttribute{UsdPrim.GetAttribute(pxr::UsdGeomTokens->exposureIso)}};
	}
	else if (PropertyPath == CameraShutterSpeedPropertyName)
	{
		return {UE::FUsdAttribute{UsdPrim.GetAttribute(pxr::UsdGeomTokens->exposureTime)}};
	}
...
TArray<FName> UsdUtils::GetPropertiesForAttribute(const UE::FUsdPrim& Prim, const FString& AttrName)
{
	...
	const static std::unordered_map<pxr::TfToken, TArray<FName>, pxr::TfHash> StandardMapping = {
		...
		{pxr::UsdGeomTokens->fStop, 						{DepthOfFieldFstopPropertyName}},
		...
		{pxr::UsdGeomTokens->exposureFStop, 				{CurrentAperturePropertyName}},
		{pxr::UsdGeomTokens->exposureIso, 					{CameraISOPropertyName}},
		{pxr::UsdGeomTokens->exposureTime, 					{CameraShutterSpeedPropertyName}},
		...
		{pxr::UsdLuxTokens->inputsWidth, 					{SourceWidthPropertyName}},
		{pxr::UsdLuxTokens->inputsHeight, 					{SourceHeightPropertyName}},
		{pxr::UsdLuxTokens->inputsAngle, 					{LightSourceAnglePropertyName}},
		{pxr::UsdLuxTokens->inputsShapingConeAngle, 		{OuterConeAnglePropertyName}},
		{pxr::UsdLuxTokens->inputsShapingConeSoftness, 		{InnerConeAnglePropertyName}},
	...
	if (AttrToken == pxr::UsdLuxTokens->inputsRadius)
	{
		...
		{
			return {SourceRadiusPropertyName};
		}
		else if (UsdPrim.IsA<pxr::UsdLuxDiskLight>())
		{
			return {SourceWidthPropertyName, SourceHeightPropertyName};
		}
	}
...
void UsdUtils::DensifyTimeSamplesIfNeeded(
	const UE::FUsdPrim& Prim,
	const FName& PropertyPath,
	TArray<double>& InOutTimeSamples
)
{
	using namespace UnrealIdentifiers;

	// For now, only light intensity needs densification, because the final USD intensity
	// is intensity * 2^exposure, which is non-linear in exposure. If we only sample at
	// authored keyframe times, linearly interpolating the resulting UE keyframes won't
	// match the non-linear USD interpolation of the separate attributes.
	if (PropertyPath != IntensityPropertyName)
	{
		return;
	}

	if (InOutTimeSamples.Num() < 2)
	{
		return;
	}
	...
	pxr::UsdAttribute ExposureAttr = UsdPrim.GetAttribute(pxr::UsdLuxTokens->inputsExposure);
	if (!ExposureAttr || !ExposureAttr.ValueMightBeTimeVarying())
	{
		return;
	}

	// Check if all exposure time samples are zero (effectively no exposure animation)
	std::vector<double> ExposureSamples;
	ExposureAttr.GetTimeSamples(&ExposureSamples);
	if (ExposureSamples.empty())
	{
		return;
	}

	bool bAllZero = true;
	for (double SampleTime : ExposureSamples)
	{
		float ExposureValue = 0.0f;
		if (ExposureAttr.Get<float>(&ExposureValue, SampleTime) && !FMath::IsNearlyZero(ExposureValue))
		{
			bAllZero = false;
			break;
		}
	...
	// Densify: generate every integer time code between the first and last sample
	USD_LOG_INFO(
		TEXT("Baking light intensity animation frame-by-frame for prim '%s' because it has animated exposure, "
			 "which combines non-linearly with intensity (intensity * 2^exposure)"),
		*Prim.GetPrimPath().GetString()
	);

	const double First = FMath::FloorToDouble(InOutTimeSamples[0]);
	const double Last = FMath::CeilToDouble(InOutTimeSamples.Last());

	TSet<double> TimeCodeSet;
	TimeCodeSet.Reserve(static_cast<int32>(Last - First) + 1 + InOutTimeSamples.Num());

	// Preserve original samples (they may not be on integer boundaries)
	for (double Sample : InOutTimeSamples)
	{
		TimeCodeSet.Add(Sample);
	}

	// Add every integer time code in the range
	for (double TimeCode = First; TimeCode <= Last; TimeCode += 1.0)
	{
		TimeCodeSet.Add(TimeCode);
	}

	InOutTimeSamples = TimeCodeSet.Array();
	InOutTimeSamples.Sort();
}
