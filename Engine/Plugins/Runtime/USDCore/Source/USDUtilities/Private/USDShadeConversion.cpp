...
namespace UE::UsdShadeConversion::Private
{
	...
	)
	{
		...
		if (pxr::UsdShadeConnectableAPI::GetConnectedSource(Input.GetAttr(), &Source, &SourceName, &AttributeType))
		{
			// Recurse because the attribute may just be pointing at some other attribute that has the data
			// (e.g. when shader input is just "hoisted" and connected to the parent material input)
			return GetIntParameterValue(Source, SourceName, DefaultValue, OutValue);
		}
		// No other node connected, so we must have some value
		else
		{
			int32 InputValue = DefaultValue;
			Input.Get<int32>(&InputValue);

			OutValue.Set<int32>(InputValue);
		}

		return true;
	}

	// No recursion for token value
	bool GetTokenAsFloatParameterValue(
		pxr::UsdShadeConnectableAPI& Connectable,
		const pxr::TfToken& InputName,
		const char* DefaultValue,
		const TMap<FString, float>& Values,
		UsdToUnreal::FParameterValue& OutValue
	)
	{
		FScopedUsdAllocs Allocs;
		...
		pxr::TfToken TokenInputValue{ DefaultValue };
		Input.Get<pxr::TfToken>(&TokenInputValue);
		float InputValue = 0;
		if (const float* FoundValue = Values.Find(UsdToUnreal::ConvertToken(TokenInputValue)))
		{
	...
	struct FGetExpressionForValueVisitor
	{
		...
		UMaterialExpression* operator()(const UsdToUnreal::FTextureParameterValue& TextureValue) const
		{
			...
			Expression->SamplerType = MaterialExpressionUtils::GetSamplerTypeForTexture(TextureValue.Texture);
	...
	struct FSetParameterValueVisitor
	{
		...
		// we handle int as scalar parameters
		void operator()(const int32 Int32Value) const
		{
			UsdUtils::SetScalarParameterValue(Material, ParameterName, Int32Value);
		}
	...
	)
	{
		...
		// Analogously, the default refraction value in USD is 1.5, which is very distorted, while it's 1.0 in UE.
		// So if we have no other opinion here, let's author a 1.0 refraction value so that it looks like it would in UE.
		// We only care about that if we're also baking opacity though (so that we're in some kind of translucent mode). 
		// If we're fully opaque then we don't really care to author this
		if (!PropertiesToProcess.Contains(MP_Refraction) && (PropertiesToProcess.Contains(MP_Opacity) || PropertiesToProcess.Contains(MP_OpacityMask)))
		{
			PropertiesToProcess.Add(MP_Refraction);
		}
		...
		for (const EMaterialProperty& Property : PropertiesToProcess)
		{
			...
			if (Property == MP_Refraction && NumSamples == 0 && !UserConstantValue)
			{
				UserConstantValue = &One;
				bPropertyValueIsConstant = true;
			}
			...
			if (!UserConstantValue && NumSamples > 0 && SampleSize)
			{
				...
				{
					USD_LOG_WARNING(
						TEXT("Skipping material property %d as it has an unexpected number of samples (%lld instead of %d)."),
						Property,
						NumSamples,
			...
			switch (Property)
			{
				...
				case MP_OpacityMask:
					...
					break;
				case MP_Refraction:
					InputToken = UnrealIdentifiers::Refraction;
					InputType = pxr::SdfValueTypeNames->Float;
					ConstantValue = UserConstantValue ? *UserConstantValue : ConstantLinearValue[0];
					FallbackValue = pxr::GfVec4f{1.0, 1.0, 1.0, 1.0f};
					break;
...
)
{
	...
	// SpecularColor
	if (GetVec3ParameterValue(Connectable, UnrealIdentifiers::SpecularColor, FLinearColor::White, ParameterValue))
	{
		OutMaterialData.Parameters.Add(TEXT("SpecularColor"), ParameterValue);
	}

	// Use Specular Workflow
	if (GetIntParameterValue(Connectable, UnrealIdentifiers::UseSpecularWorkflow, 0, ParameterValue))
	{
		OutMaterialData.Parameters.Add(TEXT("UseSpecularWorkflow"), ParameterValue);
	}

	// ClearCoat
	if (GetFloatParameterValue(Connectable, UnrealIdentifiers::ClearCoat, 0, ParameterValue))
	{
		OutMaterialData.Parameters.Add(TEXT("ClearCoat"), ParameterValue);
	}

	// ClearCoat Roughness
	if (GetFloatParameterValue(Connectable, UnrealIdentifiers::ClearCoatRoughness, 0.1f, ParameterValue))
	{
		OutMaterialData.Parameters.Add(TEXT("ClearCoatRoughness"), ParameterValue);
	}
	...
	// Opacity Mode
	static TMap<FString, float> OpacityModeMap{ {"transparent", 0.f}, {"presence", 1.f} };
	if (GetTokenAsFloatParameterValue(Connectable, UnrealIdentifiers::OpacityMode, "transparent", OpacityModeMap, ParameterValue))
	{
		OutMaterialData.Parameters.Add(TEXT("OpacityMode"), ParameterValue);
	}

	// Opacity Threshold
	if (GetFloatParameterValue(Connectable, UnrealIdentifiers::OpacityThreshold, 0, ParameterValue))
	{
		OutMaterialData.Parameters.Add(TEXT("OpacityThreshold"), ParameterValue);
	}
	...
	// Displacement
	if (GetFloatParameterValue(Connectable, UnrealIdentifiers::Displacement, 0.0f, ParameterValue))
	{
		OutMaterialData.Parameters.Add(TEXT("Displacement"), ParameterValue);
	}

	// UseWorldSpaceNormal
	// This is not part of the UsdPreviewSurface schema, but it's something we export for landscape materials.
	// Unlike the other attributes this is written directly to the Material prim, and not on the surface shader.
	if (pxr::UsdAttribute Attr = UsdShadeMaterialPrim.GetAttribute(UnrealIdentifiers::WorldSpaceNormals))
	{
		bool bValue = false;
		if (Attr.Get<bool>(&bValue) && bValue)
		{
			// Use an int parameter here as actual bool parameters are used for static switches it seems
			UsdToUnreal::FParameterValue IntParameterValue{TInPlaceType<int32>(), static_cast<int32>(bValue)};
			OutMaterialData.Parameters.Add(TEXT("UseWorldSpaceNormals"), IntParameterValue);
		}
	}
...
void UsdUtils::AuthorUnrealMaterialBinding(pxr::UsdPrim& MeshOrGeomSubsetPrim, const FString& UnrealMaterialPathName, bool bStrongerThanDescendants)
{
	...
	if (pxr::UsdPrim MaterialPrim = ChildMaterial.GetPrim())
	{
		...
		BindingAPI.Bind(
			ChildMaterial,
			bStrongerThanDescendants ? pxr::UsdShadeTokens->strongerThanDescendants : pxr::UsdShadeTokens->fallbackStrength
		);
	}
}
...
bool UsdUtils::HasSurfaceOutput(const pxr::UsdPrim& MaterialPrim, const TArray<FString>& RenderContexts)
{
	if (!MaterialPrim)
	{
		return false;
	...
	{
		pxr::UsdShadeOutput MaterialOutput = ShadeMaterial.GetSurfaceOutput(UnrealToUsd::ConvertToken(*RenderContext).Get());
		if (MaterialOutput)
		{
			return true;
		}
...
bool UsdUtils::HasDisplacementOutput(const pxr::UsdPrim& MaterialPrim, const TArray<FString>& RenderContexts)
{
	if (!MaterialPrim)
	{
		return false;
	...
	for (const FString& RenderContext : RenderContexts)
	{
		pxr::UsdShadeOutput MaterialOutput = ShadeMaterial.GetDisplacementOutput(UnrealToUsd::ConvertToken(*RenderContext).Get());
		if (MaterialOutput)
		{
			return true;
		}
...
bool UsdUtils::HasVolumeOutput(const pxr::UsdPrim& MaterialPrim, const TArray<FString>& RenderContexts)
{
	if (!MaterialPrim)
	{
		return false;
	...
	for (const FString& RenderContext : RenderContexts)
	{
		pxr::UsdShadeOutput MaterialOutput = ShadeMaterial.GetVolumeOutput(UnrealToUsd::ConvertToken(*RenderContext).Get());
		if (MaterialOutput)
		{
			return true;
		}
