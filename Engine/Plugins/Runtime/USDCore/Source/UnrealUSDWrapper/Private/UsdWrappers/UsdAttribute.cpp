...
namespace UsdUtils
{
	...
	template<typename ValueType>
	bool GetAuthoredAttributeValue(const UE::FUsdPrim& InPrim, FStringView InAttributeName, ValueType& OutValue, TOptional<double> InTime)
	{
		if (!InPrim)
		{
			return false;
		...
		UE::FUsdAttribute Attribute = InPrim.GetAttribute(InAttributeName.GetData());
		if (!Attribute)
		{
			return false;
		}

		const bool bGotValue = Attribute.Get(OutValue, InTime);
		return bGotValue && Attribute.HasAuthoredValue();
	}
}	 // namespace UsdUtils
...
#define SPECIALIZE(X) \
	...
	template UNREALUSDWRAPPER_API bool UsdUtils::GetAuthoredAttributeValue(const UE::FUsdPrim& InPrim, FStringView InAttributeName, X& OutValue, TOptional<double> InTime);
