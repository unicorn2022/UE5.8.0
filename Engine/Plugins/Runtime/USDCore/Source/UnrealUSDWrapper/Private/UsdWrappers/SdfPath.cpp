...
namespace UE
{
	...
	bool FSdfPath::IsValidPathString(const TCHAR* PathString, FString* OutErrorMessage)
	{
#if USE_USD_SDK
		FScopedUsdAllocs Allocs;

		std::string ErrorMessage;
		const bool bIsValid = pxr::SdfPath::IsValidPathString(
			TCHAR_TO_UTF8(PathString),
			OutErrorMessage ? &ErrorMessage : nullptr
		);

		if (OutErrorMessage)
		{
			*OutErrorMessage = FString(UTF8_TO_TCHAR(ErrorMessage.c_str()));
		}
	...
	FSdfPath FSdfPath::ReplacePrefix(const UE::FSdfPath& OldPrefix,
								     const UE::FSdfPath& NewPrefix,
			                         bool FixTargetPaths) const
	{
#if USE_USD_SDK
		return FSdfPath(Impl->PxrSdfPath.Get().ReplacePrefix(OldPrefix, NewPrefix, FixTargetPaths));
#else
		return FSdfPath();
#endif	  // #if USE_USD_SDK
	}
	...
	int32 FSdfPath::GetPathElementCount() const
	{
#if USE_USD_SDK
		FScopedUsdAllocs Allocs;

		return static_cast<int32>(Impl->PxrSdfPath.Get().GetPathElementCount());
#else
		return -1;
#endif	  // #if USE_USD_SDK
	}
