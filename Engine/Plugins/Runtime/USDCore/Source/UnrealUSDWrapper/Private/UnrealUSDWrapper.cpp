...
namespace UnrealIdentifiers
{
	...
	const FString StaticMeshBuildSettingsAPI(TEXT("StaticMeshBuildSettingsAPI"));
	const FString NaniteBuildSettingsAPI(TEXT("NaniteBuildSettingsAPI"));
	...
	const TfToken OpacityMode = TfToken("opacityMode");
	const TfToken OpacityThreshold = TfToken("opacityThreshold");
	const TfToken Normal = TfToken("normal");
	const TfToken Specular = TfToken("specular");
	const TfToken SpecularColor = TfToken("specularColor");
	const TfToken UseSpecularWorkflow = TfToken("useSpecularWorkflow");
	const TfToken ClearCoat = TfToken("clearcoat");
	const TfToken ClearCoatRoughness = TfToken("clearcoatRoughness");
	...
	const TfToken Displacement = TfToken("displacement");
...
bool IUsdPrim::SetActiveLODIndex(const pxr::UsdPrim& Prim, int LODIndex)
{
	...
	if (Prim.HasVariantSets())
	{
		...
		if (LODVariantSet.IsValid())
		{
			...
			{
				return LODVariantSet.SetVariantSelection(VariantNames[LODIndex]);
			}
		}
	}
...
FName UnrealIdentifiers::AspectRatioPropertyName = GET_MEMBER_NAME_CHECKED(UCameraComponent, AspectRatio);
FName UnrealIdentifiers::OrthoWidthPropertyName = GET_MEMBER_NAME_CHECKED(UCameraComponent, OrthoWidth);
FName UnrealIdentifiers::OrthoFarClipPlanePropertyName = GET_MEMBER_NAME_CHECKED(UCameraComponent, OrthoFarClipPlane);
FName UnrealIdentifiers::OrthoNearClipPlanePropertyName = GET_MEMBER_NAME_CHECKED(UCameraComponent, OrthoNearClipPlane);
...
FName UnrealIdentifiers::DepthOfFieldFstopPropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, PostProcessSettings.DepthOfFieldFstop);
FName UnrealIdentifiers::CameraISOPropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, PostProcessSettings.CameraISO);
FName UnrealIdentifiers::CameraShutterSpeedPropertyName = GET_MEMBER_NAME_CHECKED(UCineCameraComponent, PostProcessSettings.CameraShutterSpeed);
...
namespace UsdWrapperUtils
{
	void CheckIfForceDisabled()
	{
		...
		UE_LOGF(
			LogUsd,
			Error,
			"The USD SDK is disabled because the executable is not forcing the ansi C allocator (you need to set 'FORCE_ANSI_ALLOCATOR=1' as a "
...
namespace UE::UnrealUSDWrapper::Private
{
	void EnsureModuleIsLoaded()
	{
		...
		if (!Manager.IsModuleLoaded(ModuleName))
		{
			...
			if (!LoadedModule)
			{
				UE_LOGF(LogUsd, Warning, "Failed to load the UnrealUSDWrapper module!");
			}
		}
	}
};
...
TArray<FString> UnrealUSDWrapper::GetAllSupportedFileFormats() TSAN_SAFE
{
	...
	for (const std::string& Ext : Extensions)
	{
		...
		pxr::SdfFileFormatConstPtr Format = pxr::SdfFileFormat::FindByExtension(Ext, pxr::SdfUsdFileFormatTokens->Target);
		if (Format == nullptr)
		{
			continue;
		}
...
TArray<FString> UnrealUSDWrapper::GetNativeFileFormats() TSAN_SAFE
{
	...
	const pxr::TfTokenVector NativeFormatIds{
		pxr::SdfUsdFileFormatTokens->Id,
		pxr::SdfUsdaFileFormatTokens->Id,
		pxr::SdfUsdcFileFormatTokens->Id,
		pxr::SdfUsdzFileFormatTokens->Id
	};
...
void UnrealUSDWrapper::GetNativeFileFormats(TArray<FString>& OutTextFormats, TArray<FString>& OutPossiblyBinaryFormats) TSAN_SAFE
{
	...
	const pxr::SdfFileFormatConstPtr TextFormat = pxr::SdfFileFormat::FindById(pxr::SdfUsdaFileFormatTokens->Id);
	if (TextFormat)
	{
...
#if PLATFORM_WINDOWS
#if !USE_USD_MEMORY_MANAGER
		// Only Windows need to register the memory overloads when enabled in the USD SDK
