...
namespace UsdUtils
{
	...
	// Outputs the authored attribute value for the given prim and attribute name, and returns true.
	// If the attribute doesn't exist, returns false and leaves OutValue unassigned.
	// If the attribute does exist but has no authored opinion, returns false but assigns OutValue with the attribute's fallback value, if possible.
	template<typename ValueType>
	UNREALUSDWRAPPER_API bool GetAuthoredAttributeValue(const UE::FUsdPrim& InPrim, FStringView InAttributeName, ValueType& OutValue, TOptional<double> InTime = {});
}