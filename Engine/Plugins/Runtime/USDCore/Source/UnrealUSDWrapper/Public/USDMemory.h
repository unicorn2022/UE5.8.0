...
// Default to 0 in case the USD SDK is not used
// Also, on Linux/Mac, the USD SDK doesn't need the memory overloads to support UE allocators so the USD memory manager is not needed
...
// See UnrealUSDWrapper.build.cs for the definition of this macro
#if ENABLE_USD_MEMORY_OVERLOADS

#ifndef PXR_RUNTIME_MEMORY_OVERLOADS_ENABLED
// The USD SDK was compiled without the support for UE memory overloads so the USD memory manager has to be enabled
...
// USD memory overloads not enabled so the USD memory manager has to be enabled on Windows
#if PLATFORM_WINDOWS
#undef USE_USD_MEMORY_MANAGER
#define USE_USD_MEMORY_MANAGER 1

// On Windows the USD SDK is compiled with PXR_ENABLE_RUNTIME_MEMORY_OVERLOADS=ON, so this is just a sanity check
#ifdef PXR_RUNTIME_MEMORY_OVERLOADS_ENABLED
COMPILE_WARNING("The USD SDK was compiled with the memory overloads enabled but the UnrealUSDWrapper module was not. Make sure the memory overloads match between UE and the USD SDK.")
#endif // PXR_RUNTIME_MEMORY_OVERLOADS_ENABLED
