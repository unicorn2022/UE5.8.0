...
namespace UnrealBuildTool.Rules
{
	public class UnrealUSDWrapper : ModuleRules
	{
		...
		{
			// Must be set to true if the USD SDK was compiled with PXR_ENABLE_RUNTIME_MEMORY_OVERLOADS=ON
			// Only relevant on Windows since on Linux/Mac, the UE allocators will always be used
			bool bEnableUEAllocatorsInUSD = true;
			bool bIsUsdSdkEnabled = EnableUsdSdk(Target, Module);
			
			// Only Windows require memory overloads in the USD SDK
			bool bPlatformNeedsUsdMemoryOverloads = Target.Platform == UnrealTargetPlatform.Win64;
			bool bEnableUsdMemoryOverloads = bIsUsdSdkEnabled && bEnableUEAllocatorsInUSD && bPlatformNeedsUsdMemoryOverloads;
			...
			if (!Target.bMergeModules && !bEnableUsdMemoryOverloads && bPlatformNeedsUsdMemoryOverloads)
			{
