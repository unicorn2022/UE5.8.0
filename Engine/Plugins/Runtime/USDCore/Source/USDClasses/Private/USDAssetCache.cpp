...
void UUsdAssetCache::CacheAsset(const FString& Hash, UObject* Asset, const FString& PrimPath /*= FString() */)
{
	if (!Asset)
	{
		UE_LOGF(LogUsd, Warning, "Attempted to add a null asset to USD Asset Cache with hash '%ls' and PrimPath '%ls'!", *Hash, *PrimPath);
		return;
	}
		...
		if (UObject* ExistingAsset = TransientStorage.FindRef(Hash))
		{
			if (ExistingAsset != Asset)
			{
				UE_LOGF(
					LogUsd,
					Log,
					"Overwriting asset '%ls' with '%ls' (prim path '%ls') in the asset cache's transient storage",
		...
		if (UObject* ExistingAsset = PersistentStorage.FindRef(Hash))
		{
			if (ExistingAsset != Asset)
			{
				UE_LOGF(
					LogUsd,
					Log,
					"Overwriting asset '%ls' with '%ls' (prim path '%ls') in the asset cache's persistent storage",
					*ExistingAsset->GetPathName(),
					*Asset->GetPathName(),
					*PrimPath
				);
...
void UUsdAssetCache::LinkAssetToPrim(const FString& PrimPath, UObject* Asset)
{
	...
	if (!OwnedAssets.Contains(Asset))
	{
		UE_LOGF(
			LogUsd,
			Warning,
			"Tried to set prim path '%ls' to asset '%ls', but it is not currently owned by the USD stage cache!",
			*PrimPath,
			*Asset->GetName()
		);
		return;
