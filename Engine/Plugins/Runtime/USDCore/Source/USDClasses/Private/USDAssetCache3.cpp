	...
	void SafeDeleteObjects(const TSet<UObject*>& ObjectsToDelete, const UUsdAssetCache3& AssetCache)
	{
		...
		// for us to track all the UObjects ourselves for our ExternalReferencers step below, and for runtime parity
		{
			...
			for (UObject* Object : ObjectsToDelete)
			{
				...
				GetObjectsWithOuter(Object, ExtraObjects, EGetObjectsFlags::IncludeNestedObjects);
				...
				if (TSet<UObject*>* ReferencedObjects = ReferencerToReferenced.Find(Referencer))
				{
					for (UObject* ReferencedObject : *ReferencedObjects)
					{
						...
						UE_LOGF(
							LogUsd,
							Verbose,
							"Not trying to clean up '%ls' because it is referenced by '%ls'",
		...
		// Reference: ObjectTools::DeleteObjects
		{
			...
			if (!CanDeleteResult.Get())
			{
				UE_LOGF(
					LogUsd,
					Warning,
					"Cancelling the deletion of '%d' assets as the deletion operation was blocked by an engine event",
			...
			FOnPreDestructiveAssetActionArgs PreDestructiveArgs(DeletableAssetsArray, EDestructiveAssetActions::AssetDelete);
			FEditorDelegates::OnPreDestructiveAssetActionWithArgs.Broadcast(PreDestructiveArgs);
			if (!PreDestructiveArgs.Result.bSuccess)
			{
				UE_LOGF(LogUsd, Warning, "Cancelling the deletion of '%d' assets as the deletion operation was blocked by an editor event", DeletableAssetsArray.Num());
				UE_LOGF(LogUsd, Warning, "%ls", *PreDestructiveArgs.Result.ErrorMessage);
				return;
			}
		...
		// Finally actually delete the assets that we can
		{
			...
			{
				UE_LOGF(LogUsd, Verbose, "Deleting '%ls'", *DeletableAsset->GetPathName());
				...
				{
					UE_LOGF(LogUsd, Warning, "Failed to delete asset '%ls'", *DeletableAsset->GetPathName());
				}
			}
#else
			for (UObject* DeletableAsset : DeletableAssets)
		...
		if (DeletableAssets.Num() > 0)
		{
			...
			UE_LOGF(
				LogUsd,
				Verbose,
				"Deleted %d out of %d assets in %.3f s (including GC and transaction reset)",
				DeletableAssets.Num(),
				ObjectsToDelete.Num(),
				ElapsedSeconds
			);
	...
	if (Asset)
	{
		if (!Asset->IsA(Class))
		{
			UE_LOGF(
				LogUsd,
				Warning,
				"Asset cache '%ls' stopped tracking asset '%ls' for hash '%ls' as its class ('%ls') differs from the requested class '%ls'. A new asset of the requested class will be instantiated for that hash.",
				*GetPathName(),
				*Asset->GetPathName(),
				*Hash,
		...
		if (!bIsTransientAsset)
		{
			...
			if (AssetName != PackageName)
			{
				UE_LOGF(
					LogUsd,
					Warning,
					"Asset '%ls' has a different name than it's package short name '%ls' (full name '%ls'), which could cause issues",
					*AssetName,
					*PackageName,
	...
	if (FString* OldHash = AssetPathToHashes.Find(Asset))
	{
		...
		if (*OldHash != Hash)
		{
			UE_LOGF(
				LogUsd,
				Warning,
				"An asset can only be associated with a single hash! Discarding old hash '%ls' mapped to recently cached asset '%ls' (new hash '%ls')",
				**OldHash,
				*Asset->GetPathName(),
				*Hash
			);
		...
		for (const TPair<FString, FSoftObjectPath>& Pair : HashToAssetPaths)
		{
			...
			if (!Package || Package->GetFileSize() > 0)
			{
				UE_LOGF(
					LogUsd,
					Verbose,
					"Not trying to clean up '%ls' because the path doesn't resolve, or resolve to a saved asset",
					*AssetPathString
				);
				continue;
			}
			...
			if (!LoadedObject || !DeletableAssetKeys.Contains(FObjectKey{LoadedObject}))
			{
				...
				UE_LOGF(LogUsd, Verbose, "Not trying to clean up '%ls' because it hasn't been set as deletable", *AssetPathString);
				continue;
			}
			...
			if (TArray<FObjectKey>* FoundReferencers = HashToReferencer.Find(Hash))
			{
				...
				{
					UE_LOGF(LogUsd, Verbose, "Not trying to clean up '%ls' because it has object referencers", *AssetPathString);
					continue;
				}
			}
...
FSoftObjectPath UUsdAssetCache3::StopTrackingAssetInternal(const FString& Hash)
{
	...
	if (LoadedObject)
	{
		UE_LOGF(LogUsd, Verbose, "Setting '%ls' as undeletable", *LoadedObject->GetPathName());
...
void UUsdAssetCache3::TryCachingAssetFromAssetUserData(const FAssetData& ExistingAssetData)
{
	...
	if (!ExistingAssetData.IsAssetLoaded())
	{
		UE_LOGF(
			LogUsd,
			Verbose,
			"Loading existing asset '%ls' to check if it can be automatically added to the asset cache '%ls'",
		...
		if (UUsdAssetUserData* UserData = UsdUnreal::ObjectUtils::GetAssetUserData(LoadedAsset))
		{
			if (!UserData->OriginalHash.IsEmpty())
			{
				UE_LOGF(
					LogUsd,
					Verbose,
					"Automatically caching asset '%ls' into asset cache '%ls' with hash '%ls'",
					*ExistingAssetPathStr,
					*GetPathName(),
	...
	if (!bCurrentIsValid)
	{
		...
		if (bEmitWarning)
		{
			UE_LOGF(
				LogUsd,
				Log,
				"Resetting %ls's AssetDirectory to '%ls', as '%ls' is not a valid content folder path for this asset cache",
				*GetPathName(),
				*NewPath,
				*AssetDirectory.Path
			);
