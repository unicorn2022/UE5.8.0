...
namespace UE::USDClasses::Private
{
	TSharedPtr<FJsonObject> ParseJSON(const FString& FileContents)
	{
		...
		{
			UE_LOGF(LogUsd, Warning, "Failed to parse plugInfo.json file: '%ls'", *Reader->GetErrorMessage());
		}
	...
	for (const FString& JsonFilePath : JsonPaths)
	{
		...
		for (const TSharedPtr<FJsonValue>& PluginEntry : *PluginEntriesPtr)
		{
			...
			if (!FPaths::MakePathRelativeTo(PathToDll, *RootPath))
			{
				...
				{
					UE_LOGF(LogUsd, Warning, "Failed to handle Current DLL Location '%ls' when trying to update plugInfo.json paths", *PathToDll);
					continue;
				}
			}
		...
		if (bUpdatedFile)
		{
			...
			{
				UE_LOGF(LogUsd, Log, "Updated LibraryPaths for USD plugInfo.json file '%ls'", *JsonFilePath);
			}
			else
			{
				UE_LOGF(LogUsd, Warning, "Failed to update LibraryPath for USD plugInfo.json file '%ls'", *JsonFilePath);
			}
		}
	}
}
...
TSet<UObject*> IUsdClassesModule::GetAssetDependencies(UObject* Asset)
{
	...
	{
		UE_LOGF(LogUsd, Warning, "Unknown asset '%ls' encountered when collecting dependencies.", Asset ? *Asset->GetName() : TEXT("nullptr"));
	}
...
UUsdAssetCache3* IUsdClassesModule::GetAssetCacheForProject()
{
	...
	if (DefaultCache && ProjectSettings && GIsEditor)
	{
		...
		UE_LOGF(LogUsd, Log, "%ls", *SubText.ToString().Replace(TEXT("\n\n"), TEXT(" ")));
