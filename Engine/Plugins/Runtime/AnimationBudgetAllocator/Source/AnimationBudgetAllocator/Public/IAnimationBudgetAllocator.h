...
public:
	...
	/**
	* Add external time spent on animation that should come out of the Animation Budget but is not accounted for in the ABA update.
	* TimeSpent is accumulated and spent each frame, it is reset to 0 each time it is used.  
	*/
	virtual void AddExternalAnimationTimeSpentThisFrame(float TimeSpent) = 0;
};