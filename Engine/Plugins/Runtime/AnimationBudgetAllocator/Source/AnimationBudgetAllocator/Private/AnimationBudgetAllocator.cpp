...
FAnimBudgetAllocatorComponentData::FAnimBudgetAllocatorComponentData(USkeletalMeshComponentBudgeted* InComponent, float InGameThreadLastTickTimeMs, int32 InStateChangeThrottle)
	...
	, PrevFrameGameThreadLastCompletionTimeMs(0.0f)
#endif
	, FrameOffset(0)
	, DesiredTickRate(1)
	, TickRate(1)
...
void FAnimationBudgetAllocator::QueueSortedComponentIndices(float InDeltaSeconds)
{
	...
	auto QueueComponentTick = [InDeltaSeconds, this, bUseSignificanceDelegate](FAnimBudgetAllocatorComponentData& InComponentData, int32 InComponentIndex, bool bInOnScreen)
	{
		...
		InComponentData.bWasOffScreenPrevTick = (bInOnScreen && !InComponentData.bOnScreen);
	...
	{
		ComponentData.bReducedWorkDueToOffscreen = false;
		...
		if(ComponentData.bTickEnabled)
		{
			...
			if (Component && Component->IsRegistered())
			{
				...
				if(bShouldTick)
				{
					...
					// Skip offscreen components with a significance of zero or less (or less than zero if GAnimationBudgetComponentsWithZeroSigAreNonRendered is enabled).
					if((Component->bUseScreenRenderStateForUpdate ? Component->GetLastRenderTimeOnScreen() : Component->GetLastRenderTime()) <= WorldTime && 
						ComponentData.bTickEvenIfNotRendered && 
						((GAnimationBudgetComponentsWithZeroSigAreNonRendered && ComponentData.Significance >= 0.0f) || (!GAnimationBudgetComponentsWithZeroSigAreNonRendered && ComponentData.Significance > 0.0f)))
					{
...
#if ENABLE_DRAW_DEBUG
					NonRenderedNotSignificantComponentDataForDebugDraw.Add(ComponentIndex);
		...
		{
			DisabledTickingComponentDataForDebugDraw.Add(ComponentIndex);
		}
#endif // #if ENABLE_DRAW_DEBUG
	...
	if(MaxOffscreenComponents > 0)
	{
		...
		{
			if(InComponentData.bAllowReducedWork
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
				&& !GAnimationBudgetDebugForce
#endif
			)
			{
				InComponentData.bReducedWorkDueToOffscreen = true;

				if (!InComponentData.bReducedWork && InComponentData.Component->OnReduceWork().IsBound())
				{
#if WITH_TICK_DEBUG
					UE_LOGF(LogTemp, Warning, "Force-decreasing offscreen component work (mesh %ls) (actor %llx)", InComponentData.Component->SkeletalMesh ? *InComponentData.Component->SkeletalMesh->GetName() : TEXT("null"), (uint64)InComponentData.Component->GetOwner());
#endif
		...
		if (GAnimationBudgetMaintainReduceWorkWhenOffScreen)
		{
			// if component ticks were disabled either here or through reduce work, they should be taken out of the sorted component list
			for (int32 SortedCompIdx = 0 ; SortedCompIdx < AllSortedComponentData.Num() ; ++SortedCompIdx)
			{
				FAnimBudgetAllocatorComponentData& SortedComponentData = AllComponentData[AllSortedComponentData[SortedCompIdx]];
				if (!SortedComponentData.bTickEnabled)
				{
					AllSortedComponentData.RemoveAt(SortedCompIdx);
					--SortedCompIdx;
				}
			}
		}
...
int32 FAnimationBudgetAllocator::CalculateWorkDistributionAndQueue(float InDeltaSeconds, float& OutAverageTickRate)
{
...
#if ENABLE_DRAW_DEBUG
	bEmergencyReducedWorkActive = false;
#endif
	...
	auto QueueForTick = [&NumTicked, this](FAnimBudgetAllocatorComponentData& InComponentData, int32 InStateChangeThrottleInFrames)
	{
		...
		if (GAnimationBudgetForceTickWhenComponentExitsOffScreen != 0)
		{
			if (InComponentData.bWasOffScreenPrevTick && !bTickThisFrame)
			{
				// force tick this frame and fix up frameoffset so we maintain the same tick rate
				bTickThisFrame = true;
				InComponentData.FrameOffset += (InComponentData.TickRate - ((GFrameCounter + InComponentData.FrameOffset) % InComponentData.TickRate));
			}
		}
...
#if ENABLE_DRAW_DEBUG
		InComponentData.PrevFrameGameThreadLastCompletionTimeMs = InComponentData.GameThreadLastCompletionTimeMs;
#endif
	...
	if(GAnimationBudgetDebugForce)
	{
		for(int32 SortedComponentIndex : AllSortedComponentData)
		{
			...
			if (GAnimationBudgetDebugForceExcludeAlwaysTicked == 0 || !ComponentData.bAlwaysTick)
			{
...
#endif
	{
		...
		if(TotalIdealWorkUnits > 0)
		{
			...
			const float AvailableBudget = Parameters.BudgetInMs - ExternalAnimationTimeSpent;
			ExternalAnimationTimeSpent = 0.0f;

			const float WorkUnitBudget = FMath::Max(AvailableBudget / AverageWorkUnitTimeMs, (float)TotalIdealWorkUnits * Parameters.MinQuality);
			...
			const bool bEmergencyReducedWork = (SmoothedBudgetPressure >= Parameters.BudgetPressureBeforeEmergencyReducedWork) && (GAnimationBudgetEmergencyReduceWorkDisabled == 0);

#if ENABLE_DRAW_DEBUG
			bEmergencyReducedWorkActive = bEmergencyReducedWork;
#endif
			...
			for(int32 DisallowedReducedWorkComponentIndex : DisallowedReducedWorkComponentData)
			{
				...
				{
#if WITH_TICK_DEBUG
					UE_LOGF(LogTemp, Warning, "Force-increasing component work (mesh %ls) (actor %llx)", ComponentData.Component->SkeletalMesh ? *ComponentData.Component->SkeletalMesh->GetName() : TEXT("null"), (uint64)ComponentData.Component->GetOwner());
#endif
					ComponentData.Component->OnReduceWork().Execute(ComponentData.Component, false);
					ComponentData.bReducedWork = false;
				}
			...
			if(--ReducedComponentWorkCounter <= 0)
			{
				...
				if (ReducedWorkComponentData.Num() > 0 && SmoothedBudgetPressure < Parameters.BudgetFactorBeforeReducedWork - Parameters.BudgetFactorBeforeReducedWorkEpsilon)
				{
					for(int32 ReducedWorkComponentIndex : ReducedWorkComponentData)
					{
						...
						// check if ReducedWorkDueToOffscreen is true from this component or the root component. If so, don't remove reduce work due to budget
						bool bReducedWorkDueToOffscreen = false;
						
						if (GAnimationBudgetMaintainReduceWorkWhenOffScreen)
						{
							bReducedWorkDueToOffscreen = ComponentData.bReducedWorkDueToOffscreen;
							if (ComponentData.RootPrerequisite != nullptr)
							{
								const int32 PrerequisiteHandle = ComponentData.RootPrerequisite->GetAnimationBudgetHandle();
								if (PrerequisiteHandle != INDEX_NONE)
								{
									FAnimBudgetAllocatorComponentData& RootPrerequisiteComponentData = AllComponentData[PrerequisiteHandle];
									bReducedWorkDueToOffscreen |= RootPrerequisiteComponentData.bReducedWorkDueToOffscreen;
								}
							}
						}

						if(ComponentData.bReducedWork && !bReducedWorkDueToOffscreen && ComponentData.Component->OnReduceWork().IsBound())
						{
#if WITH_TICK_DEBUG
							UE_LOGF(LogTemp, Warning, "Increasing component work (mesh %ls) (actor %llx)", ComponentData.Component->SkeletalMesh ? *ComponentData.Component->SkeletalMesh->GetName() : TEXT("null"), (uint64)ComponentData.Component->GetOwner());
#endif
							ComponentData.Component->OnReduceWork().Execute(ComponentData.Component, false);
							ComponentData.bReducedWork = false;
				...
				else if(SmoothedBudgetPressure > Parameters.BudgetFactorBeforeReducedWork)
				{
					...
					for (SortedComponentIndex = TotalIdealWorkUnits - 1; SortedComponentIndex >= FullIndexEnd; --SortedComponentIndex)
					{
						...
						{
#if WITH_TICK_DEBUG
							UE_LOGF(LogTemp, Warning, "Reducing component work (mesh %ls) (actor %llx)", ComponentData.Component->SkeletalMesh ? *ComponentData.Component->SkeletalMesh->GetName() : TEXT("null"), (uint64)ComponentData.Component->GetOwner());
#endif
							ComponentData.Component->OnReduceWork().Execute(ComponentData.Component, true);
							ComponentData.bReducedWork = true;
...
void FAnimationBudgetAllocator::Update(float DeltaSeconds)
{
	...
	if (bCurrentlyUpdating)
	{
...
#if ENABLE_DRAW_DEBUG
		DisabledTickingComponentDataForDebugDraw.Reset();
		NonRenderedNotSignificantComponentDataForDebugDraw.Reset();
#endif
		...
		if(GAnimationBudgetDebugEnabled != 0)
		{
			...
			if (GAnimationBudgetDebugVerboseActive)
			{
				const float WorldTime = World->TimeSeconds - 1.0f;

				if (GAnimationBudgetDebugVerboseShowNonTicking)
				{
					for (int32 DisabledTickingComponentDataIndex : DisabledTickingComponentDataForDebugDraw)
					{
						FAnimBudgetAllocatorComponentData& ComponentData = AllComponentData[DisabledTickingComponentDataIndex];
						TArray<int32>& ComponentIndexArray = ActorMap.FindOrAdd(ComponentData.Component->GetOwner());
						ComponentIndexArray.Add(DisabledTickingComponentDataIndex);
					}

					for (int32 NonRenderedComponentDataIndex : NonRenderedNotSignificantComponentDataForDebugDraw)
					{
						FAnimBudgetAllocatorComponentData& ComponentData = AllComponentData[NonRenderedComponentDataIndex];
						TArray<int32>& ComponentIndexArray = ActorMap.FindOrAdd(ComponentData.Component->GetOwner());
						ComponentIndexArray.Add(NonRenderedComponentDataIndex);
					}
				}

				for(TPair<AActor*, TArray<int32>>& ActorIndicesPair : ActorMap)
				{
					ActorIndicesPair.Value.Sort();

					FString DebugString;
					bool bActorOffscreen = false;
				
					for(int32 ComponentDataIndex : ActorIndicesPair.Value)
					{
						FAnimBudgetAllocatorComponentData& ComponentData = AllComponentData[ComponentDataIndex];

						if (!ComponentData.bOnScreen)
						{
							bActorOffscreen = true;
						}

						// component could be not ticking due to tick being disabled externally (bTickEnabled is false) or due to being not visible, etc
						const bool bTicking = ComponentData.Component->PrimaryComponentTick.IsTickFunctionEnabled();

						if((bTicking && ComponentData.bOnScreen) || GAnimationBudgetDebugVerboseShowNonTicking)
						{
							FString StatusStr = FString::Printf(TEXT("%s %s %.2f "),
								ComponentData.Component->bHiddenInGame ? TEXT("HID") : TEXT("NOTHID"),
								ComponentData.Component->IsVisible() ? TEXT("VIS") : TEXT("NOTVIS"), 
								ComponentData.Component->GetComponentScale().X);

							if (!ComponentData.bTickEnabled)
							{
								StatusStr += TEXT(" DISABLED ");
							}
							else if (NonRenderedNotSignificantComponentDataForDebugDraw.Contains(ComponentDataIndex))
							{
								StatusStr += TEXT(" OFF (NOT SIG) ");
							}
							else if (!ComponentData.bOnScreen)
							{
								StatusStr += TEXT(" OFF (SIG) ");
							}

							FString TickStr = TEXT("   ");

							if (bTicking)
							{
								// if offscreen 
								if (ComponentData.Component->GetLastRenderTime() <= WorldTime)
								{
									switch (ComponentData.Component->VisibilityBasedAnimTickOption)
									{
							...
							DebugString += FString::Printf(TEXT("%s%s%s%.03f %d %s %s%s - T:%.3f E:%.3f A:%.3f\n"), *StatusStr,
								(GAnimationBudgetDebugShowAddresses) ? *FString::Printf(TEXT("0x%" UPTRINT_x_FMT " "), (UPTRINT)ComponentData.Component) : TEXT(""),
								(GAnimationBudgetDebugVerboseShowNames) ? *FString::Printf(TEXT("%s%s "), (GAnimationBudgetDebugVerboseShowActorNames) ? *FString::Printf(TEXT("%s - "), *ComponentData.Component->GetOwner()->GetName()) : TEXT(""), *ComponentData.Component->GetName()) : TEXT(""),
								ComponentData.Significance, ComponentData.TickRate, ComponentData.bInterpolate ? TEXT("I") : TEXT(" "), ComponentData.bReducedWork ? TEXT("Lo") : TEXT("Hi"), *TickStr,
								ComponentData.GameThreadLastTickTimeMs, ComponentData.PrevFrameGameThreadLastCompletionTimeMs, ComponentData.GameThreadLastTickTimeMs + ComponentData.PrevFrameGameThreadLastCompletionTimeMs);
						}
					}
				
					if (GAnimationBudgetDebugVerboseOnCanvas != 0)
					{
				...
				if (bEmergencyReducedWorkActive)
				{
					GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::Red, TEXT("Emergency Reduce Work Enabled"));
				}

				if (AllSortedComponentData.Num() > 0)
				{
					FString Prefix;
					if (World->WorldType == EWorldType::PIE && World->GetNetMode() == NM_Client)
					{
						Prefix = FString::Printf(TEXT("Client %d: "), UE::GetPlayInEditorID());
					}

					GEngine->AddOnScreenDebugMessage(-1, 0.0f, FColor::White, *FString::Printf(TEXT("%s Frame: %d Frame Budget: %.4f"), *Prefix, GFrameNumber, Parameters.BudgetInMs));
				}
			}
			else  // !GAnimationBudgetDebugVerboseActive
			{
...
{
	if(FAnimationBudgetAllocator::bCachedEnabled && bEnabled && GAnimationBudgetDebugEnabled != 0 && GAnimationBudgetDebugDisableGraph == 0)
	{
...
void FAnimationBudgetAllocator::AddExternalAnimationTimeSpentThisFrame(float TimeSpent)
{
	ExternalAnimationTimeSpent += TimeSpent;
}
