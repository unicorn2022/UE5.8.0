...
public:
	...
	RIGVM_API virtual FName GetDisplayNameForArgument(const FName& InArgumentName, const FRigVMTemplateTypeMap& InTypes) const;
	...
	bool IsControlFlowDispatch(const FRigVMDispatchContext& InContext, const FRigVMTemplateTypeMap& InTypeMap) const { return !GetControlFlowBlocks(InContext, InTypeMap).IsEmpty(); }

	// returns the control flow blocks of this dispatch
	RIGVM_API const TArray<FName>& GetControlFlowBlocks(const FRigVMDispatchContext& InContext, const FRigVMTemplateTypeMap& InTypeMap) const;
#endif
...
protected:
	...
	bool CheckArgumentType(bool bCondition, const FName& InArgumentName) const
	{
		if(!bCondition)
		{
			UE_LOGF(LogRigVM, Error, "Fatal: '%ls' Argument '%ls' has incorrect type.", *GetFactoryName().ToString(), *InArgumentName.ToString())
		}
		return bCondition;
	}
#endif
...
#if WITH_EDITORONLY_DATA
	RIGVM_API virtual const TArray<FName>& GetControlFlowBlocks_Impl(const FRigVMDispatchContext& InContext, const FRigVMTemplateTypeMap& InTypeMap) const;
#endif
