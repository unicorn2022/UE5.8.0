...
struct FRigVMCallableArgument
{
	...
	UPROPERTY()
	FRigVMOperand ForwardedOperand;

	UPROPERTY()
	ERigVMPinDirection Direction;

	RIGVM_API void Serialize(FArchive& Ar);
	friend FArchive& operator<<(FArchive& Ar, FRigVMCallableArgument& P)
	{
		P.Serialize(Ar);
		return Ar;
	}
	...
	bool IsInput() const
	{
		return Direction == ERigVMPinDirection::Input || Direction == ERigVMPinDirection::IO; 
	}

	bool IsOutput() const
	{
		return Direction == ERigVMPinDirection::Output || Direction == ERigVMPinDirection::IO; 
	}

	friend uint32 GetTypeHash(const FRigVMCallableArgument& InArgument)
	{
		return HashCombine(
			GetTypeHash(InArgument.Name.ToString()),
			GetTypeHash(InArgument.TypeString),
			GetTypeHash(InArgument.InterfaceOperand),
			GetTypeHash(InArgument.ForwardedOperand),
			GetTypeHash(InArgument.Direction));
	}

};

// A callable section of the rigvm bytecode. This is used to represent functions
// and events with arguments.
USTRUCT()
struct FRigVMCallableInfo
{
	...
	FRigVMCallableInfo(int32 InIndex, const FName& InName, uint32 InHash, int32 InFirstInstruction, int32 InLastInstruction)
	: Index(InIndex)
	, Name(InName)
	, FunctionHash(InHash)
	, FirstInstruction(InFirstInstruction)
	, LastInstruction(InLastInstruction)
	{
	}
	...
	UPROPERTY()
	TArray<FRigVMCallableArgument> Arguments;
	...
	RIGVM_API int32 GetArgumentIndex(const FName& InName) const;
	RIGVM_API const FRigVMCallableArgument* FindArgument(const FName& InName) const;
	int32 GetNumInstructions() const
	{
		return FMath::Max(0, LastInstruction - FirstInstruction + 1);
	}

	RIGVM_API void Serialize(FArchive& Ar);
	friend FArchive& operator<<(FArchive& Ar, FRigVMCallableInfo& P)
	{
		P.Serialize(Ar);
		return Ar;
	}

	friend uint32 GetTypeHash(const FRigVMCallableInfo& InCallableInfo)
	{
		uint32 Hash = HashCombine(
			GetTypeHash(InCallableInfo.Index),
			GetTypeHash(InCallableInfo.Name.ToString()),
			GetTypeHash(InCallableInfo.FirstInstruction),
			GetTypeHash(InCallableInfo.LastInstruction)
		);
		for (const FRigVMCallableArgument& Argument : InCallableInfo.Arguments)
		{
...
struct FRigVMInvokeCallableOp : public FRigVMBaseOp
{
	...
	FRigVMInvokeCallableOp(uint16 InCallableIndex, uint16 InArgumentCount)
	: FRigVMBaseOp(ERigVMOpCode::InvokeCallable)
	, CallableIndex(InCallableIndex)
	, ArgumentCount(InArgumentCount)
	{
	}

	FRigVMInvokeCallableOp(ERigVMOpCode InOpCode, uint16 InCallableIndex, uint16 InArgumentCount)
	...
	uint16 CallableIndex;
	uint16 ArgumentCount;

	friend uint32 GetTypeHash(const FRigVMInvokeCallableOp& Op)
	{
		return HashCombine(
			GetTypeHash((const FRigVMBaseOp&)Op),
			HashCombine(
				GetTypeHash(Op.CallableIndex),
	...
	friend FArchive& operator<<(FArchive& Ar, FRigVMInvokeCallableOp& P)
	{
		P.Serialize(Ar);
		return Ar;
	}
	...
	static void ZeroPaddedMemoryIfNeeded(FRigVMBaseOp* InMemory)
	{
		FRigVMInvokeCallableOp* This = reinterpret_cast<FRigVMInvokeCallableOp*>(InMemory);
		RigVM::ZeroPaddedMemory(&This->OpCode, &This->CallableIndex);
	}
};
...
struct FRigVMExecuteOp : public FRigVMInvokeCallableOp
{
	...
	: FRigVMInvokeCallableOp(ERigVMOpCode::Execute, InFunctionIndex, InArgumentCount)
	, FirstPredicateIndex(INDEX_NONE)
	, PredicateCount(0)
	{
	}
...
struct FRigVMJumpOp : public FRigVMBaseOp
{
	...
	static int32 GetTargetInstruction(ERigVMOpCode InOpCode, const int32 InCurrentInstruction, const int32 InOpJumpIndex)
	{
		check(InOpCode == ERigVMOpCode::JumpAbsolute ||
			InOpCode == ERigVMOpCode::JumpForward ||
			InOpCode == ERigVMOpCode::JumpBackward ||
			InOpCode == ERigVMOpCode::JumpAbsoluteIf ||
			InOpCode == ERigVMOpCode::JumpForwardIf ||
			InOpCode == ERigVMOpCode::JumpBackwardIf);

		if (InOpCode == ERigVMOpCode::JumpAbsolute ||
			InOpCode == ERigVMOpCode::JumpAbsoluteIf)
		{
			return InOpJumpIndex;
		}

		if (InOpCode == ERigVMOpCode::JumpForward ||
			InOpCode == ERigVMOpCode::JumpForwardIf)
		{
			return InCurrentInstruction + InOpJumpIndex;
		}
...
public:
	...
	const FRigVMInstruction* GetData() const { return Instructions.GetData(); }
...
public:
	...
	// shrinks the container and reduces the memory
	UE_API void Shrink();
	...
	// returns the number of callables
	UE_API int32 NumCallables() const;

	// returns true if the provided callable index is valid
	UE_API bool IsValidCallableIndex(int32 InCallableIndex) const;
	UE_API bool IsValidCallableIndex(uint16 InCallableIndex) const;

	// returns the callable info with a given index
	UE_API const FRigVMCallableInfo* GetCallable(int32 InCallableIndex) const;
	UE_API FRigVMCallableInfo* GetCallable(int32 InCallableIndex);
	UE_API const FRigVMCallableInfo* GetCallable(uint16 InCallableIndex) const;
	UE_API FRigVMCallableInfo* GetCallable(uint16 InCallableIndex);

	// returns the callable's index' with a given its hash
	UE_API int32 GetCallableIndex(const uint32& InHash) const;

	// returns a callable given its hash
	UE_API const FRigVMCallableInfo* FindCallable(const uint32& InHash) const;

	// returns a callable given its name
	UE_API const FRigVMCallableInfo* FindCallable(const FName& InName) const;

	// returns a callable given an instruction within the callable
	UE_API const FRigVMCallableInfo* FindCallableForInstruction(int32 InInstructionIndex) const;
	...
	// adds an execute operator given its function index operands
	UE_API int32 InlineFunction(const FRigVMByteCode* FunctionByteCode, int32 InFirstIndex, int32 InLastIndex);
	...
	// adds an invoke callable operator
	UE_API int32 AddInvokeCallableOp(const uint32& InCallableHash, const FRigVMOperandArray& InOperands);
	UE_API int32 AddInvokeCallableOpByIndex(const uint16& InCallableIndex, const FRigVMOperandArray& InOperands);
	...
	// returns the number of branches within this bytecode
	int32 NumBranches() const { return BranchInfos.Num(); }

	// returns the branch given the branch index
	const FRigVMBranchInfo& GetBranch(int32 InIndex) const
	{
		return BranchInfos[InIndex];
	}

	// returns the branch for a given argument index
	UE_API const FRigVMBranchInfo* FindBranch(int32 InInstructionIndex, int32 InArgumentIndex) const; 

	// returns the branch for a given label
	UE_API const FRigVMBranchInfo* FindBranch(int32 InInstructionIndex, const FName& InLabel) const; 

	// returns the number of predicate branches within this bytecode
	int32 NumPredicateBranches() const { return PredicateBranches.Num(); }

	// returns the predicate branch given the branch index
	const FRigVMPredicateBranch& GetPredicateBranch(int32 InIndex) const
	{
		return PredicateBranches[InIndex];
	}
	...
	// returns true if the provided opcode refers to a callable
	static bool IsCallableOpCode(const ERigVMOpCode InOpCode)
	{
		return InOpCode == ERigVMOpCode::InvokeCallable || InOpCode == ERigVMOpCode::Execute;
	}

	// returns the size of a callable op
	static int32 GetCallableOpSize(const ERigVMOpCode InOpCode)
	{
		return InOpCode == ERigVMOpCode::Execute ? sizeof(FRigVMExecuteOp) : sizeof(FRigVMInvokeCallableOp);
	}
	...
	UE_DEPRECATED(5.8, "Please use GetOperandsForCallableOp instead.")
	...
	FRigVMOperandArray GetOperandsForCallableOp(const FRigVMInstruction& InInstruction) const
	{
		int32 ByteCodeIndex = InInstruction.ByteCodeIndex;
		const FRigVMInvokeCallableOp& CallableOp = GetOpAt<FRigVMInvokeCallableOp>(ByteCodeIndex);
		...
		if (IsCallableOpCode(CallableOp.OpCode))
		{
			ByteCodeIndex += GetCallableOpSize(CallableOp.OpCode) + InInstruction.OperandAlignment;
		}
		return GetOperandsAt(ByteCodeIndex, CallableOp.GetOperandCount());
	}
...
private:
	...
	// a list of stored callables
	UPROPERTY()
	TArray<FRigVMCallableInfo> CallableInfos;
