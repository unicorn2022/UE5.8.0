...
public:
	...
	/** Meta tag for input pins that are mutable (passed by non-const reference). Must be combined with Input. */
	static inline const FLazyName ExternalVariableMetaName = FLazyName(TEXT("ExternalVariable"));
	...
	static inline const FLazyName NoOpMetaName = FLazyName(TEXT("NoOp"));
	...
	static inline const FLazyName PureMetaName = FLazyName(TEXT("Pure")); // The node will not modify the context
...
#if WITH_EDITOR
	/**
	 * Checks if any struct in the hierarchy has the specified metadata.
	 * UScriptStruct metadata is not inherited automatically, so this walks
	 * up the hierarchy to check each struct in the chain.
	 */
	static bool HasMetaData(const UScriptStruct* InStruct, const FName& InMetaDataKey)
	{
		for (const UScriptStruct* Current = InStruct; Current; Current = Cast<UScriptStruct>(Current->GetSuperStruct()))
		{
			if (Current->HasMetaData(InMetaDataKey))
			{
				return true;
			}
		}
...
protected:
	...
	RIGVM_API static TMap<FName, FString> GetDefaultValues(const void* InValue, UScriptStruct* InScriptStruct);
...
struct FRigVMStructMutable : public FRigVMStruct
{
	...
	virtual ~FRigVMStructMutable() = default;
