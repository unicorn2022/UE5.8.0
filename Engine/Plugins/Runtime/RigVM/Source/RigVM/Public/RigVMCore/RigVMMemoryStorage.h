...
struct FRigVMBranchInfo
{
	...
	{
		uint32 Hash = GetTypeHash(InBranchInfo.Label.ToString());
	...
	{
		return RigVMStringUtils::JoinStrings(InArrayName, InElementName.Replace(TEXT("_"), TEXT("")), TEXT("_"));
	}
...
public:
	...
	FRigVMLazyBranch(int32 InBranchIndex, const TFunction<ERigVMExecuteResult()>& InFunctionPtr)
		: VM(nullptr)
		, BranchInfo()
		, FunctionPtr(InFunctionPtr)
	{
		BranchInfo.Index = InBranchIndex;
	}
...
/**
 * The FRigVMForwardedMemoryHandle is used to forward memory from a call site
 * to a callable. The compiler compiled callables arguments to this structure.
 * FRigVMMemoryHandles pointing to this structure will use the pointer to
 * perform indirection on the memory.
 */
USTRUCT()
...
public:
		...
		, bIsForwarded(false)
	{
		...
		{
			bIsForwarded = StructProperty->Struct == FRigVMForwardedMemoryHandle::StaticStruct();
		}
	}
	...
	const FProperty* GetProperty() const
	{
		if (bIsForwarded)
		{
			const FRigVMForwardedMemoryHandle* ForwardedHandle = (const FRigVMForwardedMemoryHandle*)Ptr;
			checkf(ForwardedHandle->Property, TEXT("If this is null the handle is being interacted with outside of a Invoke Callable instruction."));
			return ForwardedHandle->Property;
		}
		return Property;
	}
	...
	const FProperty* GetResolvedProperty(bool bIsHiddenArgument = false) const
	{
		if(bIsHiddenArgument)
		{
			...
			return GetArrayElementPropertyChecked(GetProperty());
		}
...
private:
	...
	uint8* GetData_Internal(bool bFollowPropertyPath, int32 InSliceIndex) const
	{
		...
		if (bIsForwarded)
		{
			FRigVMForwardedMemoryHandle* ForwardedHandle = (FRigVMForwardedMemoryHandle*)Memory;
			checkf(ForwardedHandle->Memory, TEXT("If this is null the handle is being interacted with outside of a Invoke Callable instruction."));
			Memory = ForwardedHandle->Memory;
		}
		...
		if(InSliceIndex != INDEX_NONE)
		{
			...
			const FArrayProperty* ArrayProperty = CastFieldChecked<FArrayProperty>(GetProperty());
					...
					if (FirstAddedIndex > 0)
					{
						if (const uint8* SourceElementMemory = ArrayHelper.GetRawPtr(FirstAddedIndex - 1))
						{
							...
							for (int32 i = FirstAddedIndex; i < ArrayHelper.Num(); ++i)
							{
								...
								UE_LOGF(LogRigVM, Display, "Adding slice %d for Property '%ls', defaulting to '%ls'.",
									InSliceIndex,
			...
			// 	nullptr);
			//
			// UE_LOGF(LogRigVM, Display, "Getting slice %d for Property '%ls', currently '%ls'.",
			// 	InSliceIndex,
			// 	*ArrayProperty->GetName(),
			// 	*DefaultValue
			// );
		...
		{
			return PropertyPath->GetData<uint8>(Memory, GetProperty());
		}
		return Memory;
	}
	...
	// If this is true the handle points to a FRigVMForwardedMemoryHandle
	bool bIsForwarded;
