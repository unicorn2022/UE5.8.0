...
struct FRigVMDrawInstruction
{
	...
	// Only used when PrimitiveType is Text. The string is rendered on the 2D canvas by the edit
	// mode via FCanvasTextItem, anchored at Positions[0] (additional entries in Positions are
	// ignored).
	UPROPERTY(EditAnywhere, Category = "DrawInstruction")
	FString Text;
	...
	bool IsValid() const
	{
		...
		if (PrimitiveType == ERigVMDrawSettings::Text)
		{
			return Positions.Num() != 0 && !Text.IsEmpty();
		}
