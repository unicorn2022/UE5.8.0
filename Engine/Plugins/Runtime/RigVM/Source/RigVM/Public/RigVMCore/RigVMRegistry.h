...
public:
	...
	UE_API void Serialize_NoLock(FArchive& Ar, const UObject* Owner = nullptr);
	...
	// Returns true if the given type is specifically allowed 
	UE_API virtual bool IsSpecificallyAllowedType_NoLock(const UObject* InObject) const;

	// Sets a given type as specifically allowed 
	UE_API virtual void AddSpecificallyAllowedType_NoLock(const UObject* InObject);
	...
	UE_API virtual const FRigVMTemplate* FindTemplate_NoLock(const FName& InNotation, bool bIncludeDeprecated = false, bool bRemapToCoreDispatch = true) const;
...
protected:
	...
	public:
			...
			, DesiredLockType(InLockType)
		{
			...
			{
				// Only attempt to lock if we're not already holding a write lock.
			...
			else if (DesiredLockType == LockType_Write)
			{
				...
				UE_AUTORTFM_OPEN
				{
					Registry->WriteLockOwnerThread.store(FPlatformTLS::GetCurrentThreadId());
				};
			}
		...
		~FConditionalScopeLock()
		{
			...
			if (DesiredLockType == LockType_Read)
			{
				...
				ensure (Registry->WriteLockOwnerThread != FPlatformTLS::GetCurrentThreadId());
			...
			else if (DesiredLockType == LockType_Write)
			{
				...
				UE_AUTORTFM_OPEN
				{
					Registry->WriteLockOwnerThread.store(static_cast<uint32>(-1));
				};
...
public:
	...
	public:
		...
		UE_NONCOPYABLE(FConditionalReadScopeLock);
	};
	...
	public:
		...
		UE_NONCOPYABLE(FConditionalWriteScopeLock);
	};

	class FSuspendPluginUnmounting
	{
	public:
		UE_NODISCARD_CTOR FSuspendPluginUnmounting() { SuspendGlobalUnmount(); }
		~FSuspendPluginUnmounting() { ResumeGlobalUnmount(); }
	};
	...
	// Returns true if the given type is specifically allowed 
	bool IsSpecificallyAllowedType(const UObject* InObject) const
	{
		FConditionalReadScopeLock _(*this);
		return Super::IsSpecificallyAllowedType_NoLock(InObject);
	}

	// Sets a given type as specifically allowed 
	void AddSpecificallyAllowedType(const UObject* InObject)
	{
		FConditionalWriteScopeLock _(*this);
		Super::AddSpecificallyAllowedType_NoLock(InObject);
	}
	...
	// Returns a template pointer given its notation (or nullptr)
	const FRigVMTemplate* FindTemplate(const FName& InNotation, bool bIncludeDeprecated = false, bool bRemapToCoreDispatch = true) const
	{
		FConditionalWriteScopeLock _(*this);
		return Super::FindTemplate_NoLock(InNotation, bIncludeDeprecated, bRemapToCoreDispatch);
	}
...
protected:
	...
	// Returns true if the given type is specifically allowed 
	// Note: Only call NoLock methods on the FRigVMRegistry_NoLock
	virtual bool IsSpecificallyAllowedType_NoLock(const UObject* InObject) const override
	{
		return Super::IsSpecificallyAllowedType_NoLock(InObject);
	}
	...
	// Sets a given type as specifically allowed 
	// Note: Only call NoLock methods on the FRigVMRegistry_NoLock
	virtual void AddSpecificallyAllowedType_NoLock(const UObject* InObject) override
	{
		Super::AddSpecificallyAllowedType_NoLock(InObject);
	}
	...
	// Returns a template pointer given its notation (or nullptr)
	// Note: Only call NoLock methods on the FRigVMRegistry_NoLock
	virtual const FRigVMTemplate* FindTemplate_NoLock(const FName& InNotation, bool bIncludeDeprecated = false, bool bRemapToCoreDispatch = true) const override
	{
		return Super::FindTemplate_NoLock(InNotation, bIncludeDeprecated, bRemapToCoreDispatch);
	}
...
private:
	...
	UE_API static void SuspendGlobalUnmount();
	UE_API static void ResumeGlobalUnmount();
