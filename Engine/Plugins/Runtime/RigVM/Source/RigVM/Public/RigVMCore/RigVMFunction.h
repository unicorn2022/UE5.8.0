...
struct FRigVMFunction
{
	...
	RIGVM_API int32 GetArgumentIndex(const FName& InArgumentName) const;
	...
	RIGVM_API void Serialize_NoLock(FArchive& Ar, FRigVMRegistryHandle& InRegistry, const UObject* Owner = nullptr);

#if WITH_EDITOR
	RIGVM_API bool IsFixedSizeArray(const FName& InArgumentName) const;
	RIGVM_API bool IsLazyValue(const FName& InArgumentName) const;
#endif
};
