...
struct FRigVMTemplateArgumentType
{
	...
	RIGVM_API FRigVMTemplateArgumentType(const FName& InCPPType, UObject* InCPPTypeObject = nullptr, bool bIsSerializing = false);
...
private:
	...
	private:
		TArray<TArrayView<const TRigVMTypeIndex>, TInlineAllocator<(uint32)ETypeCategory::ETypeCategory_Count>> Types;
	};
};
...
public:
	...
	RIGVM_API const FRigVMExecuteArgument* FindExecuteArgument_NoLock(const FName& InArgumentName, const FRigVMDispatchContext& InContext, const FRigVMRegistryHandle& InRegistry) const;
	...
	RIGVM_API FRigVMTemplate(UScriptStruct* InStruct, const FString& InTemplateName, int32 InFunctionIndex, FRigVMRegistryHandle& InRegistry);

	// Constructor from a template name and argument infos
	RIGVM_API FRigVMTemplate(const FName& InTemplateName, const TArray<FRigVMTemplateArgumentInfo>& InInfos, const FRigVMRegistryHandle& InRegistry);
