...
FRigVMFunction_GetAbsoluteTime_Execute()
{
	Result = ExecuteContext.GetAbsoluteTime<float>();
}

#if WITH_DEV_AUTOMATION_TESTS

IMPLEMENT_RIGVMSTRUCT_AUTOMATION_TEST(FRigVMFunction_GetAbsoluteTime)
{
	ExecuteContext.SetAbsoluteTime(7.0f);
	Execute();
	AddErrorIfFalse(FMath::IsNearlyEqual(Unit.Result, 7.0f), TEXT("unexpected absolute time"));
	return true;
}
#endif