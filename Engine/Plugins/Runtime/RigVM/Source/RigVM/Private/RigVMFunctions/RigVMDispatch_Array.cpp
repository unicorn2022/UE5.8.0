...
FRigVMTemplateArgumentInfo FRigVMDispatch_ArrayBase::CreateArgumentInfo(const FName& InName, ERigVMPinDirection InDirection)
{
	...
	if(InName == SuccessName || InName == IsEmptyName)
	{
...
FName FRigVMDispatch_ArrayIsEmpty::GetArgumentNameForOperandIndex(int32 InOperandIndex, int32 InTotalOperands, FRigVMRegistryHandle& InRegistry) const
{
	static const FLazyName ArgumentNames[] = {
		ArrayName,
		IsEmptyName
...
const TArray<FRigVMTemplateArgumentInfo>& FRigVMDispatch_ArrayIsEmpty::GetArgumentInfos(FRigVMRegistryHandle& InRegistry) const
{
	if(CachedArgumentInfos.IsEmpty())
	{
		CachedArgumentInfos.Emplace( CreateArgumentInfo(ArrayName, ERigVMPinDirection::Input) );
		CachedArgumentInfos.Emplace( CreateArgumentInfo(IsEmptyName, ERigVMPinDirection::Output) );
	}
	return CachedArgumentInfos;
}

FRigVMTemplateTypeMap FRigVMDispatch_ArrayIsEmpty::OnNewArgumentType(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex, FRigVMRegistryHandle& InRegistry) const
{
	check(InArgumentName == ArrayName);
	return {
		{ArrayName, InTypeIndex},
		{IsEmptyName, RigVMTypeUtils::TypeIndex::Bool}
	};
}

void FRigVMDispatch_ArrayIsEmpty::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	const FArrayProperty* ArrayProperty = CastFieldChecked<FArrayProperty>(Handles[0].GetResolvedProperty());
	const FScriptArrayHelper ArrayHelper(ArrayProperty, Handles[0].GetInputData());
	bool* IsEmpty = (bool*)Handles[1].GetOutputData();
	*IsEmpty = ArrayHelper.Num() == 0;
}

#if WITH_EDITOR

FText FRigVMDispatch_ArrayIsEmpty::GetNodeTooltip(const FRigVMTemplateTypeMap& InTypes) const
{
    return LOCTEXT("ArrayIsEmptyToolTip", "Returns true if the array is empty");
}

FText FRigVMDispatch_ArrayIsEmpty::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if(InArgumentName == ArrayName)
	{
		return LOCTEXT("ArrayIsEmpty_ArrayArgumentToolTip", "The array to check for emptiness.");
	}
	if(InArgumentName == IsEmptyName)
	{
		return LOCTEXT("ArrayIsEmpty_NumArgumentToolTip", "True if the array is empty.");
	}
	return FText();
}
...
FText FRigVMDispatch_ArrayReverse::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if(InArgumentName == ArrayName)
	{
		return LOCTEXT("ArrayReverse_ArrayArgumentToolTip", "The array to reverse.");
	}
	return FText();
}
...
{
	return LOCTEXT("ArrayFindToolTip", "Searches a potential element in an array and returns its index.");
}
...
{
	return LOCTEXT("ArrayAppendToolTip", "Appends another array to the main one.\nModifies the input array.");
}
...
const TArray<FName>& FRigVMDispatch_ArrayIterator::GetControlFlowBlocks_Impl(
	const FRigVMDispatchContext& InContext, const FRigVMTemplateTypeMap& InTypeMap) const
{
...
void FRigVMDispatch_ArrayIterator::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	// Do not rely on Count, the array might have changed inside the loop
	const int32 Num = ArrayHelper.Num();
	if (Num != Count)
	{
		static constexpr TCHAR InvalidSizeMessage[] = TEXT("ArrayIterator: The size of the array has changed inside the loop (initially %d, current %d).");
		InContext.GetPublicData<>().Logf(EMessageSeverity::Warning, InvalidSizeMessage, Count, Num);
	}

	Index++;
	if(Num > 1)
	{
		Ratio = float(Index) / float(Num - 1);
	}
	else
	{
		Ratio = 0.f;
