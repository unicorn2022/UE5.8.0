...
{
	return LOCTEXT("CastEnumToIntToolTip", "Casts from enum to int");
}

FText FRigVMDispatch_CastEnumToInt::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if (InArgumentName == ValueName)
	{
		return LOCTEXT("CastEnumToIntValueToolTip", "The enum value to cast to an integer");
	}
	if (InArgumentName == ResultName)
	{
		return LOCTEXT("CastEnumToIntResultToolTip", "The resulting integer value");
	}
...
FText FRigVMDispatch_CastIntToEnum::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if (InArgumentName == ValueName)
	{
		return LOCTEXT("CastIntToEnumValueToolTip", "The integer value to cast to an enum value");
	}
	if (InArgumentName == ResultName)
	{
		return LOCTEXT("CastIntToEnumResultToolTip", "The resulting enum value");
	}
	return FRigVMDispatchFactory::GetArgumentTooltip(InArgumentName, InTypeIndex);
}
