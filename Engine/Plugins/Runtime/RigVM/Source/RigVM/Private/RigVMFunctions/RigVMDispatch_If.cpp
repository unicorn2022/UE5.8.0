...
#define LOCTEXT_NAMESPACE "RigVMDispatch_If"
...
FText FRigVMDispatch_If::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if (InArgumentName == ConditionName)
	{
		return LOCTEXT("ConditionToolTip", "The condition on which we'll decide between the True and False values");
	}
	if (InArgumentName == TrueName)
	{
		return LOCTEXT("TrueToolTip", "The value to use if the condition is True");
	}
	if (InArgumentName == FalseName)
	{
		return LOCTEXT("FalseToolTip", "The value to use if the condition is False");
	}
	if (InArgumentName == ResultName)
	{
		return LOCTEXT("ResultToolTip", "The resulting value - either True or False based on the condition");
	}
