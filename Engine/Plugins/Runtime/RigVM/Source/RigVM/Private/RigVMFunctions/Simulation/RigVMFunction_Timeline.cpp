...
FRigVMFunction_TimerValue_Execute()
{
	if (!bIsInitialized || Reset)
	{
		AccumulatedValue = 0.f;
		bIsInitialized = true;
		IsRunning = true;
	}

	const float AbsoluteDuration = FMath::Abs(Duration);
	if(!IsRunning || AbsoluteDuration < SMALL_NUMBER)
	{
		JustStarted = JustFinished = false;
	}
	else if(IsRunning)
	{
		JustStarted = AccumulatedValue < SMALL_NUMBER;
		IsRunning = AccumulatedValue < AbsoluteDuration + SMALL_NUMBER;

		AccumulatedValue = AccumulatedValue + ExecuteContext.GetDeltaTime<float>() * Speed;

		JustFinished = AccumulatedValue > AbsoluteDuration - SMALL_NUMBER;
		if(JustFinished)
		{
			IsRunning = false;
		}
	...
	Time = FMath::Clamp(AccumulatedValue, 0.f, AbsoluteDuration);
	if (AbsoluteDuration < SMALL_NUMBER)
	{
		Ratio = 0.f;
	}
	else
	{
		Ratio = FMath::Clamp(Time / AbsoluteDuration, 0.f, 1.f);
	}
