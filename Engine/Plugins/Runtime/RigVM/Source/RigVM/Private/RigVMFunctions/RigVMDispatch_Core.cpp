...
#define LOCTEXT_NAMESPACE "RigVMDispatch_Core"
...
FText FRigVMDispatch_CoreEquals::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if (InArgumentName == AName)
	{
		return LOCTEXT("CoreEqualsAToolTip", "The first value to compare");
	}
	if (InArgumentName == BName)
	{
		return LOCTEXT("CoreEqualsBToolTip", "The second value to compare");
	}
	if (InArgumentName == ResultName)
	{
		return LOCTEXT("CoreEqualsResultToolTip", "The boolean result of the comparison");
	}
...
void FRigVMDispatch_CoreEquals::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray Predicates)
{
	...
	check(PropertyA);
	check(PropertyB);
	check(RigVMTypeUtils::AreCompatible(PropertyA, PropertyB));
