...
IMPLEMENT_RIGVMSTRUCT_AUTOMATION_TEST(FRigVMFunction_MathFloatWrap)
{
	Unit.Minimum = 1.f;
	Unit.Maximum = 3.f;
	Unit.Value = 2.f;
	...
	Unit.Value = -0.5f;
	Execute();
	AddErrorIfFalse(FMath::IsNearlyEqual(Unit.Result, 1.5f), TEXT("unexpected result"));
	...
	Unit.Value = -1.0f;
	Execute();
	AddErrorIfFalse(FMath::IsNearlyEqual(Unit.Result, 1.0f), TEXT("unexpected result"));

	Unit.Value = 5.0f;
	Execute();
	AddErrorIfFalse(FMath::IsNearlyEqual(Unit.Result, 3.0f), TEXT("unexpected result"));
