...
#define LOCTEXT_NAMESPACE "RigVMDispatch_Select"
...
FText FRigVMDispatch_SelectInt32::GetArgumentTooltip(const FName& InArgumentName, TRigVMTypeIndex InTypeIndex) const
{
	if (InArgumentName == IndexName)
	{
		return LOCTEXT("IndexToolTip", "The index of the value to select");
	}
	if (InArgumentName == ValuesName)
	{
		return LOCTEXT("ValuesToolTip", "The fixed array of values to select from");
	}
	if (InArgumentName == ResultName)
	{
		return LOCTEXT("ResultToolTip", "The resulting selected value from the values array based on the index");
	}
