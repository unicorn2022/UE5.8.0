	...
	while(!WorkPath.IsEmpty())
	{
		...
		if(const FStructProperty* StructProperty = CastField<FStructProperty>(Property))
		{
			...
			else
			{
				Segments.Empty();
#if UE_BUILD_DEBUG
				UE_LOGF(LogRigVM, Warning, "Unable to resolve property path '%ls' for property '%ls'.", *InSegmentPath, *InProperty->GetPathName())
#endif
				return;
			}
		}
		...
		else if(CastField<FSetProperty>(Property))
		{
			Segments.Empty();
#if UE_BUILD_DEBUG
			UE_LOGF(LogRigVM, Warning, "Unable to resolve property path '%ls' for property '%ls'. FSetProperty is not supported.", *InSegmentPath, *InProperty->GetPathName())
#endif
			return;
		}
		else
...
FString FRigVMPropertyPath::GetName() const
{
	TArray<FString> Parts;
	Parts.Reset(Segments.Num() + 2);
	Parts.Add(TEXT("PropertyPath"));
	Parts.Add(GetHeadCPPType().Replace(TEXT("<"), TEXT("_")).Replace(TEXT(">"), TEXT("_")));
	for(const FRigVMPropertyPathSegment& Segment : Segments)
	{
		if (Segment.Type == ERigVMPropertyPathSegmentType::ArrayElement)
		{
			Parts.Add(FString::FromInt(Segment.Index));
		}
		else
		{
			Parts.Add(Segment.Name.ToString());
		}
	}
	return RigVMStringUtils::JoinStrings(Parts, TEXT("_"));
}

FString FRigVMPropertyPath::GetHeadCPPType() const
{
	if(Segments.IsEmpty())
	{
		return FString();
	...
	const FRigVMPropertyPathSegment& FirstSegment = Segments[0];
	switch(FirstSegment.Type)
	{
		case ERigVMPropertyPathSegmentType::ArrayElement:
		case ERigVMPropertyPathSegmentType::MapValue:
		{
			// for array elements map values the segment refers to the array itself
			FString ExtendedType;
			FString BaseType = FirstSegment.Property->GetCPPType(&ExtendedType);  
			return BaseType + ExtendedType;
		}
		case ERigVMPropertyPathSegmentType::StructMember:
		{
			UScriptStruct* OwnerStruct = Cast<UScriptStruct>(FirstSegment.Property->GetOwnerStruct());
			check(OwnerStruct);
			return OwnerStruct->GetStructCPPName();
		}
		default:
		{
			checkNoEntry();
