...
void FRigVMMemoryStorageStruct::InitializeFromNativeStruct(const UScriptStruct* InStruct)
{
	Value.InitializeAs(InStruct);
...
void FRigVMMemoryStorageStruct::GetRequiredPlugins(TArray<FString>& OutPlugins) const
{
...
void FRigVMMemoryStorageStruct::ForEachObjectDependency(const TFunction<void(const UObject*)>& InCallBack) const
{
	if (!InCallBack)
	{
		return;
...
void FRigVMMemoryStorageStruct::AddProperties(const TArray<FRigVMPropertyDescription>& InPropertyDescriptions, const TArray<FRigVMPropertyPathDescription>& InPropertyPathDescriptions)
{
	...
	FInstancedPropertyBag::AddProperties(BagDescriptors);
...
FString FRigVMMemoryStorageStruct::GetDataAsString(int32 InPropertyIndex, int32 PortFlags) const
{
...
FString FRigVMMemoryStorageStruct::GetDataAsString(const FRigVMOperand& InOperand, int32 PortFlags) const
{
...
FString FRigVMMemoryStorageStruct::GetDataAsStringSafe(int32 InPropertyIndex, int32 PortFlags) const
{
...
FString FRigVMMemoryStorageStruct::GetDataAsStringSafe(const FRigVMOperand& InOperand, int32 PortFlags) const
{
	const int32 PropertyIndex = InOperand.GetRegisterIndex();
...
const UStruct* FRigVMMemoryStorageStruct::GetStruct() const
{
	return Value.IsValid() ? Value.GetScriptStruct() : nullptr;
}
...
void FRigVMMemoryStorageStruct::RefreshLinkedProperties()
{
	...
	if (const UStruct* Struct = GetStruct())
	{
		const FProperty* Property = CastField<FProperty>(Struct->ChildProperties);
		while (Property)
		{
...
void FRigVMMemoryStorageStruct::Refresh()
{
	RefreshLinkedProperties();
	RefreshPropertyPaths();
}
...
#if WITH_EDITOR
			if (!PropertyPaths[PropertyPathIndex].IsValid())
			{
#if UE_BUILD_DEBUG
				UE_LOGF(LogRigVM, Warning, "Unable to resolve property path '%ls' for property '%ls'.", *PropertyPathDescriptions[PropertyPathIndex].SegmentPath, *LinkedProperties[PropertyIndex]->GetPathName());
#endif
			}
#endif
		}
