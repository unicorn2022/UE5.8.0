...
public:
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogRigVM, Display, "Skipping Importing To MemoryContainer: %ls", V);
#else
		UE_LOGF(LogRigVM, Error, "Error Importing To MemoryContainer: %ls", V);
#endif
		NumErrors++;
	}
};
...
void FRigVMMemoryContainer::Load(FArchive& Ar)
{
	...
	for(const FRigVMRegisterOffset& RegisterOffset : RegisterOffsets)
	{
		if(RegisterOffset.GetType() == ERigVMRegisterType::Invalid)
		{
			...
			UE_LOGF(LogRigVM, Error, "RegisterOffset '%ls' cannot be found. Asset '%ls' no longer functional.", *RegisterOffset.GetCachedSegmentPath(), *PackagePath);
	...
	for (const FString& ScriptStructPath : ScriptStructPaths)
	{
		...
		{
			FString PackagePath = Ar.GetArchiveName();
			UE_LOGF(LogRigVM, Error, "Struct '%ls' cannot be found. Asset '%ls' no longer functional.", *ScriptStructPath, *PackagePath);
			bEncounteredErrorDuringLoad = true;
		}
