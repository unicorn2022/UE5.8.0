...
FName FRigVMUnitNodeCreatedContext::FindFirstVariableOfType(FName InCPPTypeName) const
{
	...
	for (FRigVMExternalVariable ExternalVariable : ExternalVariables)
	{
		if (ExternalVariable.GetExtendedCPPType() == InCPPTypeName)
		{
			return ExternalVariable.GetName();
		}
	}
	return NAME_None;
}
...
FName FRigVMUnitNodeCreatedContext::FindFirstVariableOfType(UObject* InCPPTypeObject) const
{
	...
	for (FRigVMExternalVariable ExternalVariable : ExternalVariables)
	{
		if (ExternalVariable.GetCPPTypeObject() == InCPPTypeObject)
		{
			return ExternalVariable.GetName();
		}
	}
...
FRigVMStructUpgradeInfo FRigVMStructUpgradeInfo::MakeFromStructToFactory(UScriptStruct* InRigVMStruct, UScriptStruct* InFactoryStruct, const TMap<FName,FName>& InArgumentNameMap)
{
	...
	const FName FactoryName = FRigVMDispatchFactory::GetFactoryName(InFactoryStruct);
	const FRigVMDispatchFactory* Factory = Registry.FindDispatchFactory(FactoryName);
	if (!Factory)
	{
		Factory = Registry.FindOrAddDispatchFactory(InFactoryStruct);
		check(Factory);
	}
			...
			for(const FRigVMTemplateArgumentInfo& Info : Infos)
			{
				...
				else
				{
					UE_LOGF(
						LogRigVM,
						Error, 
						"Cannot upgrade node using '%ls' to dispatch '%ls' - type for argument '%ls' cannot be determined.\nYou may need to provide the InArgumentNameMap.",
...
void FRigVMStructUpgradeInfo::SetDefaultValues(const void* InNewStructMemory)
{
	check(NewStruct);
	check(InNewStructMemory);
	DefaultValues = FRigVMStruct::GetDefaultValues(InNewStructMemory, NewStruct);
}
...
{
	UE_LOGF(LogRigVM, Warning, "FRigVMStruct::IsForLoop is deprecated. Please call FRigVMStruct::IsControlFlowNode instead.");
...
{
	const bool bIsInput = InProperty->HasMetaData(InputMetaName);
	const bool bIsOutput = InProperty->HasMetaData(OutputMetaName);
	const bool bIsIO = InProperty->HasMetaData(IOMetaName);
	const bool bIsVisible = InProperty->HasMetaData(VisibleMetaName);
	const bool bIsExternalVariable = InProperty->HasMetaData(ExternalVariableMetaName);

	// Validate ExternalVariable usage: only valid as (Input, ExternalVariable) — no other combinations.
	if (bIsExternalVariable)
	{
		if (bIsIO)
		{
			// IO already implies mutability; ExternalVariable is redundant and confusing.
			UE_LOGF(LogRigVM, Warning,
				"Property '%ls' has ExternalVariable meta combined with IO — IO already implies mutability. Treating as Hidden.",
				*InProperty->GetName());
			return ERigVMPinDirection::Hidden;
		}
		if (bIsOutput)
		{
			// Output (or Input+Output) is incompatible with ExternalVariable.
			UE_LOGF(LogRigVM, Warning,
				"Property '%ls' has ExternalVariable meta combined with Output — ExternalVariable is not compatible with Output. Treating as Hidden.",
				*InProperty->GetName());
			return ERigVMPinDirection::Hidden;
		}
		if (!bIsInput)
		{
			// ExternalVariable requires Input — standalone ExternalVariable is invalid.
			UE_LOGF(LogRigVM, Warning,
				"Property '%ls' has ExternalVariable meta without Input — ExternalVariable requires Input. Treating as Hidden.",
				*InProperty->GetName());
			return ERigVMPinDirection::Hidden;
		}
		if (InProperty->HasMetaData(ConstantMetaName))
		{
			// Constant forces immutability, which contradicts ExternalVariable's mutable-reference purpose.
			UE_LOGF(LogRigVM, Warning,
				"Property '%ls' has ExternalVariable meta combined with Constant — these are contradictory. Treating as Hidden.",
				*InProperty->GetName());
			return ERigVMPinDirection::Hidden;
		}

		// Valid case: (Input, ExternalVariable) — mutable input passed by non-const reference.
...
{
	return GetDefaultValues(this, InScriptStruct);
}

TMap<FName, FString> FRigVMStruct::GetDefaultValues(const void* InValue, UScriptStruct* InScriptStruct)
{
	check(InScriptStruct);
	...
	if((InScriptStruct->StructFlags & (STRUCT_ImportTextItemNative | STRUCT_ExportTextItemNative)) == 0)
	{
		for (TFieldIterator<FProperty> It(InScriptStruct); It; ++It)
		{
			...
			const FName PropertyName = It->GetFName();
			const uint8* StructMemberMemoryPtr = It->ContainerPtrToValuePtr<uint8>(InValue);
...
public:
	...
	virtual void Serialize(const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category) override
	{
		...
		{
			UE_LOGF(LogRigVM, Warning, "Error when applying upgrade data from %ls to %ls, property %ls: %ls",
		...
		{
			UE_LOGF(LogRigVM, Warning, "Warning when applying upgrade data from %ls to %ls, property %ls: %ls",
				*OldStruct->GetStructCPPName(),
				*NewStruct->GetStructCPPName(),
				*PropertyName.ToString(),
				V);
