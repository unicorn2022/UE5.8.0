...
void FRigVMCallableArgument::Serialize(FArchive& Ar)
{
	Ar << Name;
	Ar << TypeString;
	Ar << InterfaceOperand;
...
int32 FRigVMCallableInfo::GetArgumentIndex(const FName& InName) const
{
	return Arguments.IndexOfByPredicate([InName](const FRigVMCallableArgument& Argument) -> bool
	{
		return Argument.Name == InName;
	});
}

const FRigVMCallableArgument* FRigVMCallableInfo::FindArgument(const FName& InName) const
{
	return Arguments.FindByPredicate([InName](const FRigVMCallableArgument& Argument) -> bool
	{
		return Argument.Name == InName;
	});
}

void FRigVMCallableInfo::Serialize(FArchive& Ar)
{
	Ar << Index;
	Ar << Name;
	Ar << FunctionHash;
...
void FRigVMInvokeCallableOp::Serialize(FArchive& Ar)
{
	Ar << OpCode;
	Ar << CallableIndex;
	...
	if(Ar.IsLoading())
	{
		...
		{
			check(FRigVMByteCode::IsCallableOpCode(OpCode));
			Ar << ArgumentCount;
		}
	}
	else
...
{
	FRigVMInvokeCallableOp::Serialize(Ar);
...
FRigVMInstructionArray::FRigVMInstructionArray(const FRigVMByteCode& InByteCode, bool bByteCodeIsAligned)
{
	...
	while (ByteIndex < InByteCode.Num())
	{
		...
		if (bByteCodeIsAligned)
		{
			...
			if (FRigVMByteCode::IsCallableOpCode(OpCode))
			{
				const int32 OperandByteIndex = ByteIndex + FRigVMByteCode::GetCallableOpSize(OpCode);
...
void FRigVMByteCode::Save(FArchive& Ar)
{
	...
	for (int32 InstructionIndex = 0; InstructionIndex < InstructionCount; InstructionIndex++)
	{
		...
		switch (OpCode)
		{
			case ERigVMOpCode::Execute:
			{
				...
				FRigVMOperandArray Operands = GetOperandsForCallableOp(Instruction);
			...
			{
				FRigVMInvokeCallableOp Op = GetOpAt<FRigVMInvokeCallableOp>(Instruction.ByteCodeIndex);
				Ar << Op;
	...
	Ar << CallableInfos;	
	UE_RIGVM_ARCHIVETRACE_ENTRY(Ar, TEXT("CallableInfos"));
...
void FRigVMByteCode::Load(FArchive& Ar)
{
	...
	for (int32 InstructionIndex = 0; InstructionIndex < InstructionCount; InstructionIndex++)
	{
		...
		switch (OpCode)
		{
			case ERigVMOpCode::Execute:
			{
				...
				AddExecuteOp(Op.CallableIndex, Operands, Op.FirstPredicateIndex, Op.PredicateCount);
				break;
			}
			case ERigVMOpCode::InvokeCallable:
			{
				FRigVMInvokeCallableOp Op;
				Ar << Op;
				...
				AddInvokeCallableOpByIndex(Op.CallableIndex, Operands);
				break;
			}
			case ERigVMOpCode::Copy:
			{
	...
	if (Ar.CustomVer(FRigVMObjectVersion::GUID) >= FRigVMObjectVersion::RigVMCallables)
	{
		Ar << CallableInfos;
	}
	else
...
void FRigVMByteCode::Shrink()
{
	ByteCode.Shrink();
	Entries.Shrink();
	BranchInfos.Shrink();
	BranchInfoLookup.Shrink();
	CallableInfos.Shrink();

#if WITH_EDITORONLY_DATA
	SubjectPerInstruction.Shrink();
	SubjectToInstructions.Shrink();
	CallPathPerInstruction.Shrink();
	CallPathToInstructions.Shrink();
	CallstackPerInstruction.Shrink();
	CallstackHashToInstructions.Shrink();
	CallstackHashPerInstruction.Shrink();
	InputOperandsPerInstruction.Shrink();
	OutputOperandsPerInstruction.Shrink();
#endif
}
...
uint32 FRigVMByteCode::GetByteCodeHash() const
{
	...
	for(const FRigVMCallableInfo& CallableInfo : CallableInfos)
	{
		Hash = HashCombine(Hash, GetTypeHash(CallableInfo));
	}
...
uint32 FRigVMByteCode::GetOperatorHash(const FRigVMInstruction& InInstruction) const
{
	switch (GetOpCodeAt(InInstruction.ByteCodeIndex))
	{
		case ERigVMOpCode::Execute:
		{
			...
			for(const FRigVMOperand& Operand : GetOperandsForCallableOp(InInstruction))
			{
		...
		case ERigVMOpCode::InvokeCallable:
		{
			const FRigVMInvokeCallableOp& Op = GetOpAt<FRigVMInvokeCallableOp>(InInstruction);
			uint32 Hash = GetTypeHash(Op);
			for(const FRigVMOperand& Operand : GetOperandsForCallableOp(InInstruction))
			{
				Hash = HashCombine(Hash, GetTypeHash(Operand));
...
int32 FRigVMByteCode::NumCallables() const
{
	return CallableInfos.Num();
}

bool FRigVMByteCode::IsValidCallableIndex(int32 InCallableIndex) const
{
	return CallableInfos.IsValidIndex(InCallableIndex);
}

bool FRigVMByteCode::IsValidCallableIndex(uint16 InCallableIndex) const
{
	return IsValidCallableIndex(IntCastChecked<int32, uint16>(InCallableIndex));
}

const FRigVMCallableInfo* FRigVMByteCode::GetCallable(int32 InCallableIndex) const
{
	if (ensure(CallableInfos.IsValidIndex(InCallableIndex)))
	{
		ensure(CallableInfos[InCallableIndex].Index == InCallableIndex);
		return &CallableInfos[InCallableIndex];
	}
	return nullptr;
}

FRigVMCallableInfo* FRigVMByteCode::GetCallable(int32 InCallableIndex)
{
	if (ensure(CallableInfos.IsValidIndex(InCallableIndex)))
	{
		ensure(CallableInfos[InCallableIndex].Index == InCallableIndex);
...
const FRigVMCallableInfo* FRigVMByteCode::GetCallable(uint16 InCallableIndex) const
{
	return GetCallable(IntCastChecked<int32, uint16>(InCallableIndex));
}

FRigVMCallableInfo* FRigVMByteCode::GetCallable(uint16 InCallableIndex)
{
	return GetCallable(IntCastChecked<int32, uint16>(InCallableIndex));
}

int32 FRigVMByteCode::GetCallableIndex(const uint32& InHash) const
{
	return CallableInfos.IndexOfByPredicate([InHash](const FRigVMCallableInfo& InCallableInfo) -> bool
	{
		return InCallableInfo.FunctionHash == InHash;
	});
}

const FRigVMCallableInfo* FRigVMByteCode::FindCallable(const uint32& InHash) const
{
	return CallableInfos.FindByPredicate([InHash](const FRigVMCallableInfo& InCallableInfo) -> const FRigVMCallableInfo*
	{
		if (InCallableInfo.FunctionHash == InHash)
		{
			return &InCallableInfo;
		}
		return nullptr;
	});
}

const FRigVMCallableInfo* FRigVMByteCode::FindCallable(const FName& InName) const
{
	return CallableInfos.FindByPredicate([InName](const FRigVMCallableInfo& InCallableInfo) -> const FRigVMCallableInfo*
	{
		if (InCallableInfo.Name == InName)
		{
			return &InCallableInfo;
		}
		return nullptr;
...
const FRigVMCallableInfo* FRigVMByteCode::FindCallableForInstruction(int32 InInstructionIndex) const
{
	return CallableInfos.FindByPredicate([InInstructionIndex](const FRigVMCallableInfo& InCallableInfo) -> const FRigVMCallableInfo*
	{
		if (FMath::IsWithinInclusive(InInstructionIndex, InCallableInfo.FirstInstruction, InCallableInfo.LastInstruction))
		{
			return &InCallableInfo;
		}
		return nullptr;
...
{
	const ERigVMOpCode OpCode = GetOpCodeAt(InByteCodeIndex);
	switch (OpCode)
	{
		...
		case ERigVMOpCode::InvokeCallable:
		{
			const int32 OpSize = GetCallableOpSize(OpCode); 
			int32 NumBytes = OpSize;
			if(bIncludeOperands)
			{
				FRigVMInvokeCallableOp CallableOp;
				uint8* CallableOpPtr = reinterpret_cast<uint8*>(&CallableOp);
				for (int32 ByteIndex = 0; ByteIndex < sizeof(FRigVMInvokeCallableOp); ByteIndex++)
				{
					CallableOpPtr[ByteIndex] = ByteCode[InByteCodeIndex + ByteIndex];
				}
				...
				NumBytes += CallableOp.GetOperandCount() * sizeof(FRigVMOperand);
			}
			return NumBytes;
		}
		case ERigVMOpCode::Copy:
...
#if WITH_EDITOR
	for (const FRigVMOperand& Operand : InOperands)
	{
		verify(Operand.IsValid());
	}
#endif
...
int32 FRigVMByteCode::InlineFunction(const FRigVMByteCode* FunctionByteCode, int32 InFirstIndex, int32 InLastIndex)
{
	check(FunctionByteCode);
	check(!FunctionByteCode->bByteCodeIsAligned);
	const int32 OpByteIndex = ByteCode.Num();
	if (FunctionByteCode->Num() == 0)
	{
		return OpByteIndex;
	}

	if (InFirstIndex == INDEX_NONE && InLastIndex == INDEX_NONE)
	{
	...
	else
	{
		...
		check(FunctionInstructions.IsValidIndex(InFirstIndex) && FunctionInstructions.IsValidIndex(InLastIndex));

		const TArrayView<const uint8> BytesView(FunctionByteCode->ByteCode);
		const int32 NumInstructionsToAdd = InLastIndex - InFirstIndex + 1;
		const int32 FirstByteIndex = FunctionInstructions[InFirstIndex].ByteCodeIndex;
		const int32 LastByteIndex =
			InLastIndex == FunctionInstructions.Num() - 1 ?
			FunctionByteCode->Num() - 1 :
			FunctionInstructions[InLastIndex + 1].ByteCodeIndex - 1;
		const int32 NumBytesToAdd = LastByteIndex - FirstByteIndex + 1;
		
		const TArrayView<const uint8> BytesToAdd(BytesView.Slice(FirstByteIndex, NumBytesToAdd));
		ByteCode.Append(BytesToAdd);
		NumInstructions += NumInstructionsToAdd;
	}
	...
	for (int32 InstructionIndex = 0; InstructionIndex < Instructions.Num(); InstructionIndex++)
	{
		...
		switch (OpCode)
		{
			case ERigVMOpCode::Execute:
			{
				...
				Line += FString::Printf(TEXT(", FunctionIndex %u"), Op.CallableIndex);
			...
			case ERigVMOpCode::InvokeCallable:
			{
				const FRigVMInvokeCallableOp& Op = GetOpAt<FRigVMInvokeCallableOp>(Instruction.ByteCodeIndex);
				const FRigVMCallableInfo* CallableInfo = GetCallable(Op.CallableIndex);
				check(CallableInfo);
				Line += FString::Printf(TEXT(", CallableIndex %u"), Op.CallableIndex);
				Line += FString::Printf(TEXT(", Function '%s'"), *CallableInfo->Name.ToString());
...
int32 FRigVMByteCode::AddInvokeCallableOp(const uint32& InCallableHash, const FRigVMOperandArray& InOperands)
{
	const FRigVMCallableInfo* CallableInfo = FindCallable(InCallableHash);
	check(CallableInfo);
	return AddInvokeCallableOpByIndex(IntCastChecked<uint16>(CallableInfo->Index), InOperands);
}

int32 FRigVMByteCode::AddInvokeCallableOpByIndex(const uint16& InCallableIndex, const FRigVMOperandArray& InOperands)
{
	FRigVMInvokeCallableOp Op(InCallableIndex, IntCastChecked<uint16>(InOperands.Num()));
	int32 OpByteIndex = AddOp(Op);
...
const FRigVMBranchInfo* FRigVMByteCode::FindBranch(int32 InInstructionIndex, int32 InArgumentIndex) const
{
	for(const FRigVMBranchInfo& BranchInfo : BranchInfos)
	{
		if (BranchInfo.InstructionIndex == InInstructionIndex && BranchInfo.ArgumentIndex == InArgumentIndex)
		{
			return &BranchInfo;
		}
	}
...
const FRigVMBranchInfo* FRigVMByteCode::FindBranch(int32 InInstructionIndex, const FName& InLabel) const
{
	for(const FRigVMBranchInfo& BranchInfo : BranchInfos)
	{
		if (BranchInfo.InstructionIndex == InInstructionIndex && BranchInfo.Label == InLabel)
		{
			return &BranchInfo;
		}
	}
...
FRigVMOperandArray FRigVMByteCode::GetOperandsForOp(const FRigVMInstruction& InInstruction) const
{
	switch(InInstruction.OpCode)
	{
		...
		case ERigVMOpCode::InvokeCallable:
		{
			return GetOperandsForCallableOp(InInstruction);
		}
		case ERigVMOpCode::Copy:
		{
...
{
	if (IsCallableOpCode(InInstruction.OpCode))
	{
		const int32 OpSize = GetCallableOpSize(InInstruction.OpCode);
		...
		return ByteCodeIndex + OpSize + InInstruction.OperandAlignment;
	}
...
int32 FRigVMByteCode::GetOpAlignment(ERigVMOpCode InOpCode)
{
	switch (InOpCode)
	{
		...
		case ERigVMOpCode::InvokeCallable:
		{
			static const int32 Alignment = FRigVMInvokeCallableOp::StaticStruct()->GetCppStructOps()->GetAlignment();
			return Alignment;
		}
		case ERigVMOpCode::Copy:
		{
...
void FRigVMByteCode::AlignByteCode()
{
	...
	for (int32 InstructionIndex = 0; InstructionIndex < Instructions.Num(); InstructionIndex++)
	{
		...
		if (IsCallableOpCode(Instruction.OpCode))
		{
	...
	for (int32 InstructionIndex = 0; InstructionIndex < Instructions.Num(); InstructionIndex++)
	{
		...
		if (IsCallableOpCode(Instruction.OpCode))
		{
			...
			const int32 OperandCount = GetOpAt<FRigVMInvokeCallableOp>(OriginalByteCodeIndex).GetOperandCount();
			OriginalByteCodeIndex += NumBytesToCopy;
			NumBytesToCopy = sizeof(FRigVMOperand) * OperandCount;
