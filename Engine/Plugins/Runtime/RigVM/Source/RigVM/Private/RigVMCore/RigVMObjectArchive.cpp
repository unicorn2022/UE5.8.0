...
bool FRigVMObjectArchive::Decompress()
{
	if(!IsCompressed())
	{
		return true;
...
FString FRigVMObjectArchive::ToString()
{
	if (IsEmpty())
	{
		return FString();
	...
	for (const TPair<FGuid,int32>& Pair : Versions)
	{
		VersionStrings.Add(FString::Printf(TEXT("%s=%d"), *Pair.Key.ToString(EGuidFormats::Base36Encoded), Pair.Value));
	}
	const FString VersionsString = RigVMStringUtils::JoinStrings(VersionStrings, TEXT("|"));
	const FString Base64Header = FString::Printf(TEXT("%d,%d,%s,Base64:"), CompressedSize, UncompressedSize, *VersionsString);
	return Base64Header + FBase64::Encode(Buffer.GetData(), Buffer.Num());
}

bool FRigVMObjectArchive::SetFromString(const FString& InString)
{
	Reset();

	check(!InString.IsEmpty());
	check(InString.Contains(TEXT(",Base64:"), ESearchCase::CaseSensitive));

	FStringView CompleteView(InString);
	int32 Separator = INDEX_NONE;
	if(!CompleteView.FindChar(TEXT(':'), Separator))
	{
		Reset();
		return false;
	}
	FStringView Header = CompleteView.Left(Separator);
	FStringView Data = CompleteView.Mid(Separator + 1);

	if(!Header.FindChar(TEXT(','), Separator))
	{
		Reset();
		return false;
	}
	CompressedSize = FCString::Atoi(*FString(Header.Left(Separator)));
	Header = Header.Mid(Separator + 1);
	if(!Header.FindChar(TEXT(','), Separator))
	{
		Reset();
		return false;
	}
	UncompressedSize = FCString::Atoi(*FString(Header.Left(Separator)));
	Header = Header.Mid(Separator + 1);
	if(!Header.FindChar(TEXT(','), Separator))
	{
		Reset();
	...
	const FString VersionsString = FString(Header.Left(Separator));
	TArray<FString> VersionStrings;
	RigVMStringUtils::SplitString(VersionsString, TEXT("|"), VersionStrings);
	for (const FString& VersionString: VersionStrings)
	{
		FString Guid,Value;
		if(!VersionString.Split(TEXT("="), &Guid, &Value))
		{
			Reset();
			return false;
		}
		Versions.Add(FGuid(Guid), FCString::Atoi(*Value));
	}
	Header = Header.Mid(Separator + 1);
	if(Header != TEXT("Base64"))
	{
		Reset();
		return false;		
	}

	if (!FBase64::Decode(FString(Data), Buffer))
	{
		Reset();
		return false;
	}

	bIsCompressed = CompressedSize != INDEX_NONE && CompressedSize < UncompressedSize;
	return true;
}

void FRigVMObjectArchive::SetVersionsFromArchive(const FArchive& Ar)
{
	const FCustomVersionContainer& VersionContainer = Ar.GetCustomVersions();
	const FCustomVersionArray& CustomVersions = VersionContainer.GetAllVersions();

	Versions.Reset();
	Versions.Reserve(CustomVersions.Num());
	for (const FCustomVersion& CustomVersion : CustomVersions)
	{
		Versions.FindOrAdd(CustomVersion.Key, 0) = CustomVersion.Version;
	}
}
...
FRigVMObjectArchiveWriter::~FRigVMObjectArchiveWriter()
{
	Archive.SetVersionsFromArchive(*this);
}
...
void FRigVMObjectArchiveReader::ReadVersions()
{
	if (Archive.Versions.IsEmpty())
	{
		return;
	}

	FCustomVersionContainer VersionContainer = GetCustomVersions();
	for (const TPair<FGuid,int32>& Pair : Archive.Versions)
	{
		VersionContainer.SetVersion(Pair.Key, Pair.Value, VersionContainer.GetFriendlyName(Pair.Key));
	}
...
{
	if (!Ar.IsLoading() || Ar.CustomVer(FRigVMObjectVersion::GUID) >= FRigVMObjectVersion::ObjectArchiveVersionContainerSerialization)
	{
		Ar << Data.Versions;
	}
	else
	{
		Data.SetVersionsFromArchive(Ar);
	}
	Ar << Data.Buffer;
