...
, bEnabled(true)
{
	...
	if(bEnabled)
	{
		...
		UE_LOGF(LogRigVM, Display, "%ls %ls%ls %ls%ls", *ArchiveName, *ArchiveOffset, *EntrySize, *ArchiveWhiteSpace, *InScope);
	}
}
...
void FRigVMArchiveTraceBracket::AddEntry(FArchive& InArchive, const FString& InScope)
{
	if(bEnabled)
	{
		...
		UE_LOGF(LogRigVM, Display, "%ls %ls,%ls %ls %ls", *ArchiveName, *ArchiveOffset, *EntrySize, *ArchivePrefix, *InScope);
