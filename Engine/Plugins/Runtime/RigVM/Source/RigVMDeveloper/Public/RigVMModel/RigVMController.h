...
struct FRigVMPinInfo
{
	...
	{
		return bIsInputVariable && Direction == ERigVMPinDirection::Input;
	}
};
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = RigVMController, meta = (DeprecatedFunction, DeprecationMessage = "Use RefreshVariableNode that takes a FGuid"))
	...
	// Refreshes the variable node with the new data
	UE_API void RefreshVariableNode(const FName& InNodeName, const FGuid& InVariableGuid, const FName& InVariableName, const FString& InCPPType, UObject* InCPPTypeObject, bool bSetupUndoRedo, bool bSetupOrphanPins = true);

	// Removes all nodes related to a given variable
	UE_API void OnExternalVariableRemoved(const FGuid& InVarGuid, bool bSetupUndoRedo);

	// Renames the variable name in all relevant nodes
	UE_API bool OnExternalVariableRenamed(const FGuid& InVarGuid, const FName& InNewVarName, bool bSetupUndoRedo);

	// Changes the data type of all nodes matching a given variable name
	UE_DEPRECATED(5.8, "Use OnExternalVariableTypeChanged that receives the variable Guid")
	...
	UE_API void OnExternalVariableTypeChanged(const FGuid& InVarGuid, const FString& InCPPType, UObject* InCPPTypeObject, bool bSetupUndoRedo);
	UE_DEPRECATED(5.8, "Use OnExternalVariableTypeChangedFromObjectPath that receives the variable Guid")
	...
	UE_API void OnExternalVariableTypeChangedFromObjectPath(const FGuid& InVarGuid, const FString& InCPPType, const FString& InCPPTypeObjectPath, bool bSetupUndoRedo);
	...
	UE_API bool RenameVariable(const FGuid& InGuid, const FName& InNewName, bool bSetupUndoRedo = true);
	...
	// Promotes a pin to a local variable
	UFUNCTION(BlueprintCallable, Category = RigVMController)
	UE_API bool PromotePinToLocalVariable(const FString& InPinPath, bool bCreateVariableNode, const FVector2D& InNodePosition = FVector2D::ZeroVector, bool bSetupUndoRedo = true, bool bPrintPythonCommand = false);
	...
	UFUNCTION(BlueprintCallable, Category = RigVMController)
	UE_API FName AddExposedPin(const FName& InPinName, ERigVMPinDirection InDirection, const FString& InCPPType, const FName& InCPPTypeObjectPath, const FString& InDefaultValue, bool bSetupUndoRedo = true, bool bPrintPythonCommand = false, bool bIsInputVariable = false);
	...
	// Marks an exposed pin as defined as an input variable
	UFUNCTION(BlueprintCallable, Category = RigVMController)
	UE_API bool MarkExposedPinAsInputVariable(const FName& InPinName, bool bDefinedAsExternalVariable, bool bSetupUndoRedo = true, bool bPrintPythonCommand = false);
	...
	UE_DEPRECATED(5.8, "Please use BindPinToVariable instead.")
	UFUNCTION(BlueprintCallable, Category = RigVMController, meta=(DeprecatedFunction, DeprecationMessage="Function has been deprecated, please use BindPinToVariable instead."))
	...
	// Sets whether a function is pure (no execute pins) or mutable (has execute pins).
	// Call this on the function library controller.
	// Returns true if the function's purity was successfully changed.
	UFUNCTION(BlueprintCallable, Category = RigVMController)
	UE_API bool SetFunctionIsPure(const FName& InFunctionName, bool bIsPure, bool bSetupUndoRedo = true, bool bPrintPythonCommand = false);
	...
	// Reorder a local variable to a new index (UFUNCTION wrapper - converts name to GUID)
	UFUNCTION(BlueprintCallable, Category = RigVMController)
	UE_API bool SetLocalVariableIndex(const FName& InVariableName, int32 InNewIndex, bool bSetupUndoRedo = true, bool bPrintPythonCommand = false);

	// Reorder a local variable to a new index (primary GUID-based implementation)
	UE_API bool SetLocalVariableIndex(const FGuid& InVariableGuid, int32 InNewIndex, bool bSetupUndoRedo = true, bool bPrintPythonCommand = false);
	...
	// bInRecurseIntoCollapseNodes: when true (default), if InNode is a collapse node the function recurses
	// into its contained graph and resolves a controller for that sub-graph via GetControllerForGraph. Once
	// the recursion starts, all nested collapse nodes are walked (the flag is not propagated as a per-level
	// switch — it gates the top-level entry only). Callers that only need the node's own pin info should
	// pass false to avoid the recursion (and the side-effect of creating a cached sub-graph controller in
	// FRigVMClient::Controllers).
	UE_API void GenerateRepopulatePinsNodeData(TArray<FRepopulatePinsNodeData>& NodesPinData, URigVMNode* InNode, bool bInFollowCoreRedirectors = true, bool bInSetupOrphanedPins = false, bool bInRecreateLinks = false, bool bInRecurseIntoCollapseNodes = true);
...
private:
	...
	UE_API FName AddExposedPin_Internal(const FName& InPinName, ERigVMPinDirection InDirection, const FString& InCPPType, const FName& InCPPTypeObjectPath, const FString& InDefaultValue, bool bSetupUndoRedo, bool bPrintPythonCommand, bool bIsInputVariable, bool bAllowExecutePinManagement);
	UE_API bool RemoveExposedPin_Internal(const FName& InPinName, bool bSetupUndoRedo, bool bPrintPythonCommand, bool bAllowExecutePinManagement);
	UE_API bool ChangeExposedPinType_Internal(const FName& InPinName, const FString& InCPPType, const FName& InCPPTypeObjectPath, bool& bSetupUndoRedo, bool bSetupOrphanPins, bool bPrintPythonCommand, bool bAllowExecutePinManagement);
	...
	UE_API bool PromotePinToVariable(URigVMPin* InPin, bool bCreateLocalVariable, bool bCreateVariableNode, const FVector2D& InNodePosition, bool bSetupUndoRedo);
...
private:
	...
	UE_API void ResolveTemplateNodeMetaData(URigVMTemplateNode* InNode, const TArray<int32>& InFilteredPermutationIndices, bool bSetupUndoRedo);
	...
	UE_API bool MarkPackageDirty(const UObject* InObjectToDirty) const;
	...
	// Mirror of the cross-graph pin layout propagation in ChangeExposedPinType_Internal —
	// not on the action stack on its own, so FRigVMChangePinTypeAction::Undo/Redo replays it.
	UE_API void PropagateExposedPinTypeChangeToReferencesAndInterface(const URigVMLibraryNode* InFunctionDefinition, bool bSetupOrphanPins);

public:
	
	UE_API FRigVMExternalVariable GetVariableByProperty(const FProperty* InExternalVariableProperty) const;
	...
	UE_API FRigVMExternalVariable GetVariableByGuid(const FGuid& InExternalVariableGuid, const bool bIncludeInputArguments = false) const;
	...
	UE_API URigVMActionStack* GetActionStack_NoLock() const;

	UFUNCTION(BlueprintCallable, Category = RigVMController)
	UE_API void SetActionStack(URigVMActionStack* InActionStack);
	UE_API void SetActionStack_NoLock(URigVMActionStack* InActionStack);
...
protected:
	...
	UE_API FRigVMClientPatchResult PatchConstantPins();
	...
	UE_API FRigVMClientPatchResult PatchPinGuidInCollapseNodes();
	UE_API FRigVMClientPatchResult PatchExternalVariablesToArguments(bool bInSetupUndoRedo=false);
...
private: 
	...
	bool bSuspendNotifications;
	bool bSuspendMarkingPackageDirty;
	...
	mutable FTransactionallySafeCriticalSection ActionStackLock;
...
private:
	...
	friend class FRigVMControllerASTLinkCheckGuard;
	friend struct FRigVMClient;
...
class FRigVMControllerASTLinkCheckGuard
{
public:

	explicit FRigVMControllerASTLinkCheckGuard(URigVMController* InController, bool bInSuspendASTLinkChecks = true)
		: Controller(InController)
	{
		bPreviousSuspendASTLinkChecks = Controller->bSuspendASTLinkChecks;
		Controller->bSuspendASTLinkChecks = bInSuspendASTLinkChecks;
	}

	~FRigVMControllerASTLinkCheckGuard()
	{
		Controller->bSuspendASTLinkChecks = bPreviousSuspendASTLinkChecks;
	}

private:

	URigVMController* Controller;
	bool bPreviousSuspendASTLinkChecks;
};
