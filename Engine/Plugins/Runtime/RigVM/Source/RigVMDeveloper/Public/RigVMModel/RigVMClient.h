...
public:
	...
	virtual TArray<FString> GeneratePythonCommands()  = 0;
...
public:
	...
	UE_API void RefreshVariableNodes(const TArray<FRigVMExternalVariable> InPublicVariables);
	...
	UE_API void SetGetFunctionHostObjectPathDelegate(URigVMFunctionLibrary* FunctionLibrary);
...
private:
	...
	struct FRigVMClientAction
	{
		...
		// For ERigVMClientAction_RenameModel only: the index in Models[] the renamed graph
		// occupied at action time. Used by PostTransacted to rebind the renamed graph back
		// into the exact slot that property restoration nulled out, which avoids order
		// corruption when more than one Models[] slot is nullptr simultaneously
		// (e.g. two root-graph renames inside the same outer transaction during redo).
		// INDEX_NONE for non-rename actions or when the graph wasn't tracked in Models.
	...
	// Self-healing cache: GetController() can evict stale entries (orphaned controllers whose graph was
	// replaced by an undo/redo transaction); declared mutable so the const observer path can do that.
	UPROPERTY(transient)
	mutable TMap<FSoftObjectPath, TObjectPtr<URigVMController>> Controllers;

	// Evicts the slot at Key from Controllers and tears down the supplied controller if it is alive.
	// Caller MUST hold ControllersLock. The slot is removed first so any reentrant lookup during teardown
	// misses cleanly. StaleController may be nullptr or already-garbage — both are handled here so callers
	// can pass any controller pointer without pre-checking. When non-null and not garbage, the controller's
	// OnModified delegate is cleared, its action stack is detached, and it is renamed into the transient
	// package, removed from root, and marked garbage (matching the teardown done by RemoveController).
	void PurgeStaleController(const FSoftObjectPath& Key, URigVMController* StaleController) const;
