...
public:
	...
	static UE_API bool SplitPinPathAtStart(FStringView InPinPath, FStringView& LeftMost, FStringView& Right);
	...
	UE_API bool NameEquals(FStringView InName, bool bFollowCoreRedirectors = false) const;
	...
	// Returns true if the pin is defined as an input variable
	UFUNCTION(BlueprintCallable, Category = RigVMPin)
	UE_API bool IsDefinedAsInputVariable() const;

	// Returns the description of the input variable
	UE_API FRigVMGraphVariableDescription GetInputVariableDescription() const; 
	...
	// Returns the default value object if this pin represents a UObject
	UE_API UObject* GetDefaultValueObject() const;
	...
	UE_API URigVMPin* FindSubPinView(FStringView InPinPath) const;
...
private:
	...
	UE_API void ClearPinPathCache() const;
	...
	UPROPERTY()
	TObjectPtr<UObject> DefaultValueObject;
