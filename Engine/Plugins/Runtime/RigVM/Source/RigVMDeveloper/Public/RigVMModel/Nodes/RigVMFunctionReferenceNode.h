...
public:
	...
	// Variable remapping
	UE_API bool ReferencesFunctionInSamePackage() const;
	UE_API bool RequiresVariableRemapping() const;
	UE_API bool IsFullyRemapped() const;
	UE_API const TMap<FName, FName> GetVariableMap() const;
	UE_API FName GetOuterVariableName(const FName& InInnerVariableName) const;
	UE_API FString GetOuterVariablePath(const FName& InInnerVariableName) const;
	...
	UE_API bool SupportsCallable() const;

	UE_API TArray<FRigVMTag> GetVariantTags() const;
	
	UE_API FRigVMReferenceNodeData GetReferenceNodeData() const;

protected:

	UE_API virtual FString GetOriginalDefaultValueForRootPin(const URigVMPin* InRootPin) const override;
	UE_API TArray<FRigVMExternalVariable> GetAssetExternalVariables() const;
...
private:
	...
	UPROPERTY()
	TMap<FName, FName> VariableMap_DEPRECATED;
