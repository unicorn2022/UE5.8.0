...
public:
	...
	// Looks for all function references and public functions (in RigVMClientHost metadata) and initializes the URigVMBuildData
	UE_API void InitializeIfNeeded();

	static UE_API void RegisterReferencesFromAsset(const FAssetData& AssetData);
	static UE_API void RegisterPublicFunctionsFromAsset(const FAssetData& AssetData);
	...
	*/
	...
	// Register a new public function with its header
	UE_API void RegisterPublicFunction(const FRigVMGraphFunctionIdentifier& InFunction, const FRigVMGraphFunctionHeader& InHeader);
	
	// Unregisters a public function
	UE_API void UnregisterPublicFunction(const FRigVMGraphFunctionIdentifier& InFunction);
	
	// Updates the header of a public function
	UE_API void UpdatePublicFunctionHeader(const FRigVMGraphFunctionIdentifier& InFunction, const FRigVMGraphFunctionHeader& InHeader);
	
	// Get header from function identifier
	UE_API const FRigVMGraphFunctionHeader& FindFunctionHeader(const FRigVMGraphFunctionIdentifier& InFunction);
	...
	// Remove all data, and mark that initialization is needed
	UE_API void Reset();
...
private:
	...
	UE_API static void TearDownRigVMGraphFunctionPointers();
	...
	UPROPERTY(VisibleAnywhere, Category = "BuildData")
	TMap<FRigVMGraphFunctionIdentifier, FRigVMGraphFunctionHeader> PublicGraphFunctions;
