...
public:
	...
	// Returns the input variables of this function
	UFUNCTION(BlueprintCallable, Category = RigVMGraph)
	UE_API TArray<FRigVMGraphVariableDescription> GetInputVariables() const;
	...
	UE_API TSharedPtr<FScribbleGraphData> SetupScribbleGraph(bool bCreateIfMissing = false);
...
private:
	...
	UE_API bool IsNameAvailable(FName InName) const;

	UPROPERTY()
	FScribbleGraph ScribbleGraph;
	...
	friend class FRigVMFunctionArgumentReorderDragDropHandler;
	friend class FRigVMGraphDetailCustomizationImpl;
