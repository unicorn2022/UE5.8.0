...
enum class ERigVMGraphNotifType : uint8
{
	...
	VariableRemappingChanged, // (DEPRECATED) A function reference node's remapping has changed (Subject == URigVMFunctionReferenceNode)
	...
	LocalVariableIndexChanged, // A local variable's index has changed (Subject == URigVMLibraryNode)
	Invalid // The max for this enum (used for guarding)
};

// A delegate for subscribing / reacting to graph modifications.
DECLARE_TS_MULTICAST_DELEGATE_ThreeParams(FRigVMGraphModifiedEvent, ERigVMGraphNotifType /* type */, URigVMGraph* /* graph */, UObject* /* subject */);
