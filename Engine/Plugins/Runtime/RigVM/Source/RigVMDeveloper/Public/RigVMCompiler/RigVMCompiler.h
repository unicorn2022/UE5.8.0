...
enum ERigVMCompilerPhase : uint8
{
	ERigVMCompilerPhase_SetupMemory,
	ERigVMCompilerPhase_BuildInstructions,
};

struct FRigVMCompilerWorkData
{
public:
	URigVMCompiler* Compiler = nullptr;
	FRigVMCompileSettings Settings;
	ERigVMCompilerPhase CompilerPhase = ERigVMCompilerPhase_SetupMemory;
	URigVM* VM = nullptr;
	TArray<URigVMGraph*> Graphs;
	const URigVMLibraryNode* CurrentlyCompilingFunctionNode = nullptr;
	FRigVMFunctionCompilationData* FunctionCompilationData = nullptr;
	TMap<uint32,TMap<FRigVMOperand,FRigVMOperand>> OperandMapPerFunction;
	...
	struct FFunctionRegisterData
	{
		...
		{
			uint32 Result = HashCombine(GetTypeHash(Data.ObjectId), GetTypeHash(Data.MemoryType));
			return HashCombine(Result, GetTypeHash(Data.RegisterIndex));
		}

		bool operator==(const FFunctionRegisterData& Other) const
		{
			return ObjectId == Other.ObjectId && MemoryType == Other.MemoryType && RegisterIndex == Other.RegisterIndex;
		}
	};
	...
	// literal register indices that belong to interactive nodes (e.g. nodes with DM active).
	// the FindOrAddProperty sharing scan skips these so that non-interactive nodes cannot
	// be mapped to an interactive node's register, which would cause DM writes to bleed
	// into nodes that are not being manipulated.
	TSet<int32> InteractiveLiteralIndices;
	...
	UE_API FRigVMOperand FindOrAddProperty(const ERigVMMemoryType InMemoryType, const FRigVMFunctionCompilationPropertyDescription& InProperty, const FString& InNewName, const bool bMakeUniqueProperty);
	UE_API FRigVMOperand FindOrAddProperty(ERigVMMemoryType InMemoryType, const FName& InName, const FString& InCPPType, UObject* InCPPTypeObject, const FString& InDefaultValue, const bool bMakeUniqueProperty);
	...
	UE_API bool AddPropertiesForFunction(
		bool InIsCallable,
		const uint32& InHash,
		...
		const URigVMFunctionReferenceNode* InFunctionReferenceNode = nullptr);
	UE_API void SetupPropertyPathsForFoldedCopies(const FRigVMNodeExprAST* InExpr);

	UE_API bool RemapInstructionAt(
	...
	/**
	 * Tracks a JumpForward op emitted for a Break or Continue node that needs
	 * its relative offset patched once the enclosing loop's EndBlock is emitted.
	 */
	struct FPendingLoopJump
	{
		int32 ByteOffset;       // Byte offset of the JumpForward op in the bytecode stream (returned by AddJumpOp)
		int32 InstructionIndex; // Instruction index of the JumpForward op (for relative offset calculation)
	};

	/**
	 * Maps each enclosing loop's CallExtern expression to the list of Break/Continue
	 * JumpForward ops that must be patched to jump to that loop's EndBlock.
	 * Entries are consumed (removed) when the loop's EndBlock is emitted.
	 */
	TMap<const FRigVMCallExternExprAST*, TArray<FPendingLoopJump>> PendingLoopJumps;
...
public:
	...
	UE_API bool CompileCallable(FRigVMCompilerWorkData& WorkData, URigVM* OutVM, FRigVMExtendedExecuteContext& OutVMContext, const FRigVMCallableInfo* InCallable, const FRigVMGraphFunctionHeader& InHeader, const FRigVMFunctionCompilationData* InFunctionCompilationData, const URigVMFunctionReferenceNode* InFunctionReferenceNode = nullptr);

	FRigVMCompiler_GetFunctionCompilationData GetFunctionCompilationData;

	struct FCompiledFunctionData
	{
		const URigVMFunctionReferenceNode* FunctionReferenceNode;
		const FRigVMFunctionCompilationData* CompilationData;
	};
	
	TMap<FString, FCompiledFunctionData> CompiledFunctions;

	static UE_API UScriptStruct* GetScriptStructForCPPType(const FString& InCPPType);
	static UE_API FString GetPinHash(const URigVMPin* InPin, const FRigVMVarExprAST* InVarExpr, const URigVMLibraryNode* FunctionCompiling = nullptr, const FRigVMASTProxy& InPinProxy = FRigVMASTProxy());
	...
	UE_API void MarkDebugWatch(FRigVMExtendedExecuteContext& Context, bool bRequired, URigVMPin* InPin, URigVM* OutVM, TMap<FString, FRigVMOperand>* OutOperands, TSharedPtr<FRigVMParserAST> InRuntimeAST);
...
private:
	...
	static UE_API FString GetPinHashImpl(const URigVMPin* InPin, const FRigVMVarExprAST* InVarExpr, const URigVMLibraryNode* FunctionCompiling = nullptr, const FRigVMASTProxy& InPinProxy = FRigVMASTProxy());
	...
	UE_API bool TraverseCallableFunction(const FRigVMInlineFunctionExprAST* InExpr, const uint32 InCallableHash, FRigVMCompilerWorkData& WorkData);
	...
	UE_API FRigVMOperand FindOrAddRegister(const FRigVMVarExprAST* InVarExpr, FRigVMCompilerWorkData& WorkData);
