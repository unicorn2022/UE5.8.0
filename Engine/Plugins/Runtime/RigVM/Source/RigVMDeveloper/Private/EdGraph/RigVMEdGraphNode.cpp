	...
	if(NodeTitle.IsEmpty())
	{
		...
		if(URigVMNode* ModelNode = GetModelNode())
		{
			...
			else if(URigVMVariableNode* VariableNode = Cast<URigVMVariableNode>(ModelNode))
			{
				...
				if (SubTitle.Len() == 0)
				{
					if(FRigVMEditorAssetInterfacePtr Blueprint = GetAsset())
					{
						...
						for(const FRigVMGraphVariableDescription& NewVariable : Blueprint->GetAssetVariables())
						{
							if(NewVariable.Name == VariableName)
							{
								...
								{
									const FEdGraphPinType PinType = RigVMTypeUtils::PinTypeFromRigVMVariableDescription(NewVariable);
...
void URigVMEdGraphNode::ReconstructNode_Internal(bool bForce)
{
	...
	{
		FRigVMEditorAssetInterfacePtr RigVMBlueprint = GetAsset();
...
void URigVMEdGraphNode::DestroyPinList(TArray<UEdGraphPin*>& InPins)
{
	...
	const FRigVMEditorAssetInterfacePtr Blueprint = GetAsset();
	bool bNotify = false;
	...
	auto SyncGraphPinLambda = [this](const URigVMPin* InModelPin, bool bAsInput) -> bool
	{
		if(UEdGraphPin* GraphPin = FindGraphPinFromModelPin(InModelPin, bAsInput))
		{
			...
			if(OldPinName != NewPinName)
			{
				...
				GraphPin->bOrphanedPin = InModelPin->IsOrphanPin() ? 1 : 0;
	...
	UEdGraphPin* InParentPin)
{
	...
	// obey to the schema. certain pins may be hidden as per schema contract.
	if (const URigVMEdGraphSchema* Schema = Cast<URigVMEdGraphSchema>(GetSchema()))
	{
		if (Schema->ShouldHidePinOnNode(InModelPin))
		{
			return false;
		}
	}
	...
	auto CreatePinLambda = [this](const URigVMPin* InModelPin, EEdGraphPinDirection InDirection, UEdGraphPin* InParentPin) -> UEdGraphPin*
	{
		...
		const bool bShouldDisplayAsMutableRef = InModelPin->IsWildCard() || InModelPin->IsDefinedAsInputVariable();

		UEdGraphPin* GraphPin = nullptr;
		UEdGraphPin** ExistingEdGraphPinPtr = LastEdGraphPins.FindByPredicate([PinPath, InDirection, PinType, bShouldDisplayAsMutableRef](const UEdGraphPin* ExistingPin) -> bool
		{
			return !ExistingPin->bWasTrashed &&
				ExistingPin->GetName() == PinPath &&
				ExistingPin->Direction == InDirection &&
				ExistingPin->PinType == PinType &&
				ExistingPin->bDisplayAsMutableRef == bShouldDisplayAsMutableRef;
		});
...
float URigVMEdGraphNode::GetFadedOutState() const
{
	...
	if (URigVMNode* ModelNode = GetModelNode())
	{
		if (URigVMVariableNode* VariableNode = Cast<URigVMVariableNode>(ModelNode))
		{
			...
			{
				if (URigVMEdGraph* Graph = Cast<URigVMEdGraph>(GetGraph()))
				{
					TArray<URigVMNode*> TargetNodes = VariableNode->GetLinkedTargetNodes();
					for (URigVMNode* TargetNode : TargetNodes)
					{
						if (URigVMEdGraphNode* TargetEdNode = Cast<URigVMEdGraphNode>(Graph->FindNodeForModelNodeName(TargetNode->GetFName())))
						{
							if (!TargetEdNode->IsFadedOut())
							{
								bIsVariableUsed = true;
								break;
							}
						}
					}
	...
	if (IsExcludedFromPreview())
	{
		...
		{
			if(IRigVMEditorAssetInterface* Blueprint = GetImplementingOuter<IRigVMEditorAssetInterface>())
			{
...
void URigVMEdGraphNode::AllocateDefaultPins()
{
	...
	if (URigVMFunctionReferenceNode* FunctionReferenceNode = Cast<URigVMFunctionReferenceNode>(GetModelNode()))
	{
		...
		{
			TArray<FRigVMExternalVariable> CurrentExternalVariables = FunctionReferenceNode->GetExternalVariables();
...
void URigVMEdGraphNode::GetNodeContextMenuActions(class UToolMenu* Menu, class UGraphNodeContextMenuContext* Context) const
{
	...
	if(const FRigVMEditorAssetInterfacePtr Blueprint = GetAsset())
	{
...
void URigVMEdGraphNode::DestroyNode()
{
	...
	if(URigVMEdGraph* Graph = Cast<URigVMEdGraph>(GetOuter()))
	{
		...
		if(IRigVMEditorAssetInterface* RigVMBlueprint = Graph->GetImplementingOuter<IRigVMEditorAssetInterface>())
		{
...
FRigVMEditorAssetInterfacePtr URigVMEdGraphNode::GetAsset() const
{
	if(const URigVMEdGraph* Graph = Cast<URigVMEdGraph>(GetOuter()))
	{
...
FEdGraphPinType URigVMEdGraphNode::GetPinTypeForModelPin(const URigVMPin* InModelPin)
{
	...
	PinType.bIsReference = InModelPin->IsDefinedAsInputVariable();
	return PinType;
}
...
void URigVMEdGraphNode::ConfigurePin(UEdGraphPin* EdGraphPin, const URigVMPin* ModelPin) const
{
	...
	EdGraphPin->bDisplayAsMutableRef = ModelPin->IsWildCard() || ModelPin->IsDefinedAsInputVariable();
	if (!EdGraphPin->bDisplayAsMutableRef)
	{
		if(ModelPin->GetDirection() == ERigVMPinDirection::Output && ModelPin->IsRootPin())
		{
			if (const URigVMVariableNode* VariableNode = Cast<URigVMVariableNode>(ModelPin->GetNode()))
			{
				if (!VariableNode->IsInputArgument())
				{
					if (VariableNode->GetValuePin() == ModelPin)
					{
						EdGraphPin->bDisplayAsMutableRef = true;
					}
				}
			}
		}
...
void URigVMEdGraphNode::UpdateVisualSettings()
{
	...
	{
		if(const FRigVMEditorAssetInterfacePtr RigBlueprint = RigGraph->GetAsset())
		{
