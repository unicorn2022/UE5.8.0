...
void URigVMEdGraph::HandleRigVMGraphRenamed(const FString& InOldNodePath, const FString& InNewNodePath)
{
	...
	if(ModelNodePath == InOldNodePath)
	{
		...
		Rename(*GraphName, nullptr, REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
	}
...
const FRigVMEditorAssetInterfacePtr URigVMEdGraph::GetAssetDefaultObject() const
{
...
void URigVMEdGraph::SetBlueprintClass(const UClass* InClass)
{
	...
	if(const FRigVMEditorAssetInterfacePtr Blueprint = GetAssetDefaultObject())
	{
		Schema = Blueprint->GetRigVMClientHost()->GetRigVMEdGraphSchemaClass();
	}
}

void URigVMEdGraph::InitializeFromAsset(FRigVMEditorAssetInterfacePtr InBlueprint)
{
	...
	if (URigVMGraph* RigVMGraph = GetModel())
	{
		...
		for (const FRigVMGraphVariableDescription& LocalVariable : LocalVariables)
		{
			if (LocalVariable.Name.IsEqual(InVariableName))
			{
				...
				const FEdGraphPinType Type = RigVMTypeUtils::PinTypeFromRigVMVariableDescription(LocalVariable);
...
FRigVMEditorAssetInterfacePtr URigVMEdGraph::GetAsset() const
{
...
URigVMGraph* URigVMEdGraph::GetModel() const
{
	...
	{
		if (URigVMGraph* Model = Client->GetModel(this))
		{
...
URigVMController* URigVMEdGraph::GetController() const
{
	...
	{
		if (URigVMController* Controller = Client->GetOrCreateController(GetModel()))
		{
			return Controller;
		}
	}
