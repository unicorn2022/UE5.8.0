	...
	if(IsVar())
	{
		...
		if(const URigVMPin* Pin = VarExpr->GetPin())
		{
			if(const URigVMNode* Node = Pin->GetNode())
			{
				...
				// Entry node output pins may be shared across many blocks,
...
FRigVMExprAST* FRigVMParserAST::TraverseMutableNode(const FRigVMASTProxy& InNodeProxy, FRigVMExprAST* InParentExpr)
{
	...
	if (NodeExpr)
	{
		...
		if (NodeExpr->IsNode())
		{
			TraverseNodeExpressionForRequiredVariableClone(NodeExpr->To<FRigVMNodeExprAST>());
		}
...
FRigVMExprAST* FRigVMParserAST::CreateExpressionForNode(const FRigVMASTProxy& InNodeProxy, FRigVMExprAST* InParentExpr)
{
	...
	FName NodeName = Node->GetEventName();
...
FRigVMExprAST* FRigVMParserAST::TraversePin(const FRigVMASTProxy& InPinProxy, FRigVMExprAST* InParentExpr)
{
	...
	else if (IsNoOpUnitNode(Pin->GetNode()))
	{
		PinExpr = MakeExpr<FRigVMLiteralExprAST>(InPinProxy);
	}
	else
...
FRigVMExprAST* FRigVMParserAST::TraverseLink(int32 InLinkIndex, FRigVMExprAST* InParentExpr)
{
	...
	if (NodeExpr)
	{
			...
			SourceRootPin->GetTypedOuter<URigVMLibraryNode>() == LibraryNodeBeingCompiled)
		{
			...
			FRigVMVarExprAST* SourcePinExpr = nullptr;
			if (NodeExpr->IsNode())
			{
				SourcePinExpr = const_cast<FRigVMVarExprAST*>(NodeExpr->To<FRigVMNodeExprAST>()->FindVarWithPinName(SourceRootPin->GetFName()));

				if (SourcePinExpr)
				{
					if (const FRigVMVarExprAST* ClonedSourcePinExpr = SourcePinExpr->GetVariableCloneExpr())
					{
						SourcePinExpr = const_cast<FRigVMVarExprAST*>(ClonedSourcePinExpr);
					}
				}
			}
			...
			if (!SourcePinExpr)
			{
				SourcePinExpr = MakeExpr<FRigVMVarExprAST>(FRigVMExprAST::EType::Var, SourceRootProxy);
			}
...
bool FRigVMParserAST::TraverseNodeExpressionForRequiredVariableClone(FRigVMNodeExprAST* InNodeExpr)
{
	const URigVMNode* Node = InNodeExpr->GetNode();
	if (!Node)
	{
		return false;
	}
	
	const URigVMVariableNode* SourceVariableNode = Cast<URigVMVariableNode>(Node);
	const URigVMFunctionEntryNode* SourceEntryNode = Cast<URigVMFunctionEntryNode>(Node);
	if (SourceVariableNode || SourceEntryNode)
	{
		TArray<TTuple<URigVMPin*, FName>> PinsToCheck;
		if (SourceVariableNode && SourceVariableNode->IsGetter())
		{
			PinsToCheck.Emplace(SourceVariableNode->GetValuePin(), SourceVariableNode->GetVariableName());
		}
		else if (SourceEntryNode)
		{
			for (URigVMPin* Pin : SourceEntryNode->GetPins())
			{
				if (Pin->GetDirection() != ERigVMPinDirection::Output)
				{
					continue;
				...
				PinsToCheck.Emplace(Pin, Pin->GetFName());
			}
		}

		for (int32 PinToCheckIndex = 0; PinToCheckIndex < PinsToCheck.Num(); PinToCheckIndex++)
		{
			URigVMPin* PinToCheck = PinsToCheck[PinToCheckIndex].Get<0>();
			const FName& VariableName = PinsToCheck[PinToCheckIndex].Get<1>();

			bool bRequiresCloningVariableValue = false;
			FRigVMASTProxy PinProxyToCheck = InNodeExpr->GetProxy().GetSibling(PinToCheck);

			// for variable / entry nodes we need to deal with a specific case where a sub pin
			// of the pin is wired to a variable setter node with the same variable
			// on a different sub pin. so for example Get MyVector.X -> Set MyVector.Y.
			// In that case we need to inject a clone of the variable in the getter.
			// Since arrays don't have sub pins on the getter this can only happen to non-arrays.
			const TArray<int32>& LinkIndices = GetTargetLinkIndices(PinProxyToCheck, true);
			for(const int32 LinkIndex : LinkIndices)
			{
				const FRigVMASTLinkDescription& Link = Links[LinkIndex];
				const URigVMPin* SourcePin = Link.SourceProxy.GetSubjectChecked<URigVMPin>();
				const URigVMPin* TargetPin = Link.TargetProxy.GetSubjectChecked<URigVMPin>();
				if (TargetPin && !Link.SegmentPath.IsEmpty())
				{
					TargetPin = TargetPin->FindSubPin(Link.SegmentPath);
				}
				...
				const URigVMVariableNode* TargetVariableNode = Cast<URigVMVariableNode>(TargetPin->GetNode());
				if (!TargetVariableNode)
				{
					continue;
				}
				if (VariableName != TargetVariableNode->GetVariableName())
				{
					continue;
				}

				if (SourcePin->GetSegmentPath() != TargetPin->GetSegmentPath())
				{
					bRequiresCloningVariableValue = true;
					break;
				}
			}

			if (bRequiresCloningVariableValue)
			{
				if (const FRigVMVarExprAST* ConstVariableVarExpr = InNodeExpr->FindVarWithPinName(PinToCheck->GetFName()))
				{
					FRigVMVarExprAST* VariableVarExpr = const_cast<FRigVMVarExprAST*>(ConstVariableVarExpr);
			
					FRigVMASTProxy PinProxy = VariableVarExpr->GetProxy();
					FRigVMVarExprAST* ClonedVariableVarExpr = MakeExpr<FRigVMVarExprAST>(FRigVMExprAST::EType::Var, PinProxy);
					ClonedVariableVarExpr->Name = *FString::Printf(TEXT("%s::Clone"), *VariableVarExpr->Name.ToString());
					const FRigVMASTLinkDescription CloneLink(PinProxy, PinProxy, FString());
					FRigVMExprAST* PinCopyExpr = MakeExpr<FRigVMCopyExprAST>(CloneLink);
					PinCopyExpr->Name = *FString::Printf(TEXT("Cloning Variable %s"), *VariableVarExpr->Name.ToString());
					PinCopyExpr->AddParent(ClonedVariableVarExpr);
					VariableVarExpr->AddParent(PinCopyExpr);
					VariableVarExpr->ClonedVarExpr = ClonedVariableVarExpr;
					InNodeExpr->ReplaceChild(VariableVarExpr, ClonedVariableVarExpr);
				}
			}
		}
	}
...
void FRigVMParserAST::FoldAssignments()
{
	...
	for (FRigVMExprAST* Expression : Expressions)
	{
		...
		// If the assign is between external variable pins, don't fold it
		if (TargetPin->GetRootPin()->IsDefinedAsInputVariable())
		{
			continue;
		}
...
void FRigVMParserAST::FoldLiterals()
{
	...
	// Returns true if the literal's owning node is marked as interactive (e.g. has DM active).
	// Interactive nodes must not share literal registers with other nodes.
	auto IsInteractive = [this](const FRigVMLiteralExprAST* InLiteral) -> bool
	{
		if (Settings.InteractiveNodes.IsEmpty())
		{
			return false;
		}
		if (const URigVMPin* Pin = InLiteral->GetPin())
		{
			if (const URigVMNode* Node = Pin->GetNode())
			{
				return Settings.InteractiveNodes.Contains(FObjectKey(Node));
			}
		}
		return false;
	};
	...
	for (int32 ExpressionIndex = 0; ExpressionIndex < Expressions.Num(); ExpressionIndex++)
	{
		...
		if (Expression->GetType() == FRigVMExprAST::EType::Literal)
		{
			...
			if (MappedExpr)
			{
				// Skip folding if either this literal or the canonical belongs to an
				// interactive node — each interactive node must keep its own register.
				if (!IsInteractive(LiteralExpr) && !IsInteractive(*MappedExpr))
				{
					TArray<FRigVMExprAST*> Parents = Expression->Parents;
					for (FRigVMExprAST* Parent : Parents)
					{
			...
			{
				// Don't use an interactive node's literal as the canonical for a value —
				// doing so would allow non-interactive nodes to share its register.
				if (!IsInteractive(LiteralExpr))
				{
...
bool FRigVMParserAST::CanLink(URigVMPin* InSourcePin, URigVMPin* InTargetPin, FString* OutFailureReason)
{
	...
	if (bNeedsCycleChecking)
	{
		...
		if (const URigVMVariableNode* TargetVariableNode = Cast<URigVMVariableNode>(TargetNode))
		{
			if (TargetVariableNode->GetValuePin() == InTargetPin->GetRootPin())
			{
				return true;
			}
		}
		...
		int32 VisitExpr(const FRigVMExprAST* InExpr, int32 InSubGraphIndex, FVisualGraph& OutGraph)
		{
			...
			switch (InExpr->GetType())
			{
				...
				case FRigVMExprAST::EType::Var:
				{
					...
					{
						const URigVMPin* Pin = InExpr->To<FRigVMVarExprAST>()->GetPin();
						check(Pin);
						if (Pin->IsDefinedAsInputVariable())
						{
							if (InExpr->Name.ToString().EndsWith(TEXT("::Clone")))
							{
								Label = FString::Printf(TEXT("Cloned Input Variable %s"), *Pin->GetName());
							}
							else
							{
								Label = FString::Printf(TEXT("Input Variable %s"), *Pin->GetName());
							}
						}
						else
						{
							Label = Pin->GetPinPath(true);
						}
					}
...
bool FRigVMParserAST::IsNoOpUnitNode(const URigVMNode* InNode)
{
#if WITH_EDITOR
	if(const URigVMUnitNode* UnitNode = Cast<URigVMUnitNode>(InNode))
	{
		...
		{
			if (ScriptStruct->HasMetaDataHierarchical(FRigVMStruct::NoOpMetaName))
			{
				return true;
			}
		}
