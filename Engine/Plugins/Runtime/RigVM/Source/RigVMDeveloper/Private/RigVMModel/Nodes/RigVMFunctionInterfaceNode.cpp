...
bool URigVMFunctionInterfaceNode::IsInterfacePinUsed(const FName& InInterfacePinName) const
{
	...
	if (const URigVMGraph* Graph = RootPin->GetGraph())
	{
		for (const URigVMNode* Node : Graph->GetNodes())
		{
			...
			// we found a variable node that sets the interface pin value
...
const URigVMCollapseNode* URigVMFunctionInterfaceNode::FindReferencedNode() const
{
	return Cast<URigVMCollapseNode>(GetGraph()->GetOuter());
}
...
{
	if(const URigVMCollapseNode* OuterNode = FindReferencedNode())
	{
