...
void URigVMPin::Serialize(FArchive& Ar)
{
...
void URigVMPin::PostLoad()
{
	Super::PostLoad();

	if (GetLinkerCustomVersion(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::RigVMPinDefaultValueObjectPtr)
	{
		if (IsUObject() && DefaultValueObject == nullptr)
		{
			// Patch object default value - value should already be hashed (if not fully loaded) as it is also referenced via the edgraph node
			FSoftObjectPath SoftObjectPath(DefaultValue.TrimQuotes());
			DefaultValueObject = SoftObjectPath.ResolveObject();
			if (!SoftObjectPath.IsNull())
			{
				// Log if we couldnt resolve the object reference - this is usually fine as it means the reference didnt exist anyways
				UE_CLOGF(DefaultValueObject == nullptr, LogRigVMDeveloper, Display, "Could not resolve legacy text pin reference to '%ls'", *SoftObjectPath.ToString());
			}
			DefaultValue.Empty();
		}
	}
...
bool URigVMPin::SplitPinPathAtStart(FStringView InPinPath, FStringView& LeftMost, FStringView& Right)
{
	return RigVMStringUtils::SplitPinPathAtStart(InPinPath, LeftMost, Right);
}
...
FString URigVMPin::GetDefaultValueForArray(TConstArrayView<FString> DefaultValues)
{
	...
	if (DefaultValues.Num())
	{
		...
		for (const FString& CurrentDefaultValue : DefaultValues.Slice(1, DefaultValues.Num() - 1))
		{
			Builder << TCHAR(',') << CurrentDefaultValue;
		}
	}
	Builder << TCHAR(')');
...
bool URigVMPin::NameEquals(FStringView InName, bool bFollowCoreRedirectors) const
{
	...
	if(bFollowCoreRedirectors)
	{
		...
		if(Struct)
		{
			...
			const FCoreRedirectObjectName OldObjectName(FName(InName), Struct->GetFName(), *Struct->GetOutermost()->GetPathName());
...
void URigVMPin::ClearPinPathCache() const
{
	for (URigVMPin* SubPin : GetSubPins())
	{
		SubPin->ClearPinPathCache();
	}
	CachedPinPath.ResetCachedValue();
	CachedPinPathWithNodePath.ResetCachedValue();
}
...
bool URigVMPin::IsDefinedAsInputVariable() const
{
	if (!IsRootPin())
	{
		return false;
	...
	return bIsInputVariable;
}

FRigVMGraphVariableDescription URigVMPin::GetInputVariableDescription() const
{
	FRigVMGraphVariableDescription Description;
	if (IsDefinedAsInputVariable())
	{
		const URigVMPin* PinToQueryGuid = this;
		const URigVMCollapseNode* CollapseNode = Cast<URigVMCollapseNode>(GetNode());
		if (!CollapseNode)
		{
			if (URigVMFunctionInterfaceNode* InterfaceNode = Cast<URigVMFunctionInterfaceNode>(GetNode()))
			{
				CollapseNode = InterfaceNode->FindReferencedNode();
				PinToQueryGuid = InterfaceNode->FindReferencedPin(this);
			}
		}
		
		if (CollapseNode && PinToQueryGuid)
		{
			Description.Guid = CollapseNode->FindPinGuid(PinToQueryGuid);
		}
		
		Description.Name = GetFName();
		Description.CPPType = GetCPPType();
		Description.CPPTypeObject = GetCPPTypeObject();
...
bool URigVMPin::ShouldHideSubPins() const
{
	...
	if(IsRootPin())
	{
		...
		if (IsDefinedAsInputVariable() && GetDirection() == ERigVMPinDirection::Input)
		{
			return true;
		}
	}
...
FString URigVMPin::GetDefaultValue(const URigVMPin::FPinOverride& InOverride, bool bAdaptValueForPinType) const
{
	...
	else if (IsStruct())
	{
		...
		if((GetScriptStruct()->StructFlags & (STRUCT_ImportTextItemNative | STRUCT_ExportTextItemNative)) == (STRUCT_ImportTextItemNative | STRUCT_ExportTextItemNative))
		{
	...
	else if (IsArrayElement() && ((!IsUObject() && DefaultValue.IsEmpty()) || (IsUObject() && DefaultValueObject == nullptr)))
	{
	...
	{
		FSoftObjectPath Object(DefaultValueObject);
...
UObject* URigVMPin::GetDefaultValueObject() const
{
...
URigVMVariableNode* URigVMPin::GetBoundVariableNode() const
{
	...
	if (IsDefinedAsInputVariable() && Links.Num() == 1)
	{
		if (Links[0]->GetTargetPin() == this)
		{
			if (const URigVMPin* SourcePin = Links[0]->GetSourcePin())
			{
				if (URigVMVariableNode* VariableNode = Cast<URigVMVariableNode>(SourcePin->GetNode()))
				{
					if (VariableNode->GetValuePin() == SourcePin->GetRootPin())
					{
						return VariableNode;
					}
				}
...
const FString URigVMPin::GetBoundVariablePath(const URigVMPin::FPinOverride& InOverride) const
{
	...
	for (const TObjectPtr<URigVMInjectionInfo>& InjectionInfo : InjectionInfos)
	{
		if (URigVMVariableNode* VariableNode = Cast<URigVMVariableNode>(InjectionInfo->Node))
		{
			if (URigVMPin* Pin = InjectionInfo->GetPin())
			{
		...
		if (Links[0]->GetTargetPin() == this)
		{
			if (const URigVMPin* SourcePin = Links[0]->GetSourcePin())
			{
				if (URigVMVariableNode* VariableNode = Cast<URigVMVariableNode>(SourcePin->GetNode()))
				{
					if (VariableNode->GetValuePin() == SourcePin->GetRootPin())
					{
						const FString SegmentPath = SourcePin->GetSegmentPath(false);
						if (SegmentPath.IsEmpty())
						{
							return VariableNode->GetVariableName().ToString();
						}
				...
				if (SourcePin->IsDefinedAsInputVariable() && Cast<URigVMFunctionEntryNode>(SourcePin->GetNode()))
				{
					const FString SegmentPath = SourcePin->GetSegmentPath(false);
					if (SegmentPath.IsEmpty())
					{
						return SourcePin->GetName();
					}
					return SourcePin->GetName() + TEXT(".") + SegmentPath;
				}
			}
		}
	}
...
bool URigVMPin::IsBoundToInputArgument() const
{
	...
	{
		if (const URigVMPin* InterfacePin = EntryNode->FindPin(VariableName))
		{
			return !InterfacePin->IsDefinedAsInputVariable();
		}
	}
...
bool URigVMPin::CanBeBoundToVariable(const FRigVMExternalVariable& InExternalVariable, const FString& InSegmentPath) const
{
	...
	if (bIsInputVariable && InExternalVariable.IsReadOnly())
	{
		return false;
	}
	...
	if (IsArray() != InExternalVariable.IsArray())
	{
		return false;
	}

	FName ExternalCPPType = InExternalVariable.GetExtendedCPPType();
	UObject* ExternalCPPTypeObject = const_cast<UObject*>(InExternalVariable.GetCPPTypeObject());

	if(!InSegmentPath.IsEmpty())
	{
		check(InExternalVariable.GetProperty());

		const FProperty* Property = InExternalVariable.GetProperty();
	...
	return RigVMTypeUtils::AreCompatible(ExternalCPPType, ExternalCPPTypeObject, *CPPBaseType, GetCPPTypeObject());
}
...
FRigVMExternalVariable URigVMPin::ToExternalVariable() const
{
	...
	ExternalVariable = RigVMTypeUtils::ExternalVariableFromCPPType(FGuid(), *VariableName, CPPType, GetCPPTypeObject(), false, false);
...
URigVMPin* URigVMPin::FindSubPinView(FStringView InPinPath) const
{
	FStringView Left, Right;
	...
	for (URigVMPin* Pin : SubPins)
	{
		if (Pin->NameEquals(Left, true))
		{
			...
			return Pin->FindSubPinView(Right);
		}
	}
	return nullptr;
}
...
{
	return FindSubPinView(InPinPath);
}
...
bool URigVMPin::CanLink(const URigVMPin* InSourcePin, const URigVMPin* InTargetPin, FString* OutFailureReason, const FRigVMByteCode* InByteCode, ERigVMPinDirection InUserLinkDirection, bool bInAllowNonArgumentPins, bool bEnableTypeCasting)
{
	...
	if (InTargetPin->IsDefinedAsInputVariable())
	{
		if (!InSourcePin->IsDefinedAsInputVariable())
		{
			if (!InSourcePin->IsRootPin())
			{
				if (OutFailureReason)
				{
					*OutFailureReason = TEXT("Variable / Ref pins can only be connected to other root variable pins.");
				}
				return false;
			}

			const URigVMVariableNode* SourceVariableNode = Cast<URigVMVariableNode>(SourceNode);
			...
			if (SourceVariableNode->GetValuePin() != InSourcePin)
			{
				if (OutFailureReason)
				{
					*OutFailureReason = TEXT("Variable / Ref pins can only be connected to other root variable pins.");
			...
			if (SourceVariableNode->IsInputArgument())
			{
				if (OutFailureReason)
				{
					*OutFailureReason = TEXT("Variable / Ref pins cannot be connected to input arguments.");
				}
				return false;
			}
		}
	...
	// In pure function graphs, certain sources cannot connect to IO pins.
	// Specifically: non-local variables and input parameters are rejected, as they would
	// allow the pure function to mutate external state. Local variables, constants, and
	// other function outputs are allowed since they are either function-scoped or immutable.
	if (const URigVMCollapseNode* OwningFunction = Cast<URigVMCollapseNode>(SourceNode->FindFunctionForNode()))
	{
		if (OwningFunction->IsPure() && !InSourcePin->IsExecuteContext() && !InTargetPin->IsExecuteContext())
		{
			const URigVMPin* SourcePin = InSourcePin;
			while (SourcePin->GetNode()->IsA<URigVMRerouteNode>())
			{
				if (URigVMRerouteNode* RereouteNode = Cast<URigVMRerouteNode>(SourcePin->GetNode()))
				{
					TArray<URigVMPin*> SourcePins = RereouteNode->FindPin(URigVMRerouteNode::ValueName)->GetLinkedSourcePins();
					if (SourcePins.Num() == 1)
					{
			...
			SourceNode = SourcePin->GetNode();
			
			TArray<const URigVMPin*> PinsToVisit = {InTargetPin};
			TArray<const URigVMPin*> TargetPins;
			while (!PinsToVisit.IsEmpty())
			{
				const URigVMPin* PinToVisit = PinsToVisit.Pop();
				if (URigVMRerouteNode* RereouteNode = Cast<URigVMRerouteNode>(PinToVisit->GetNode()))
				{
					PinsToVisit.Append(RereouteNode->FindPin(URigVMRerouteNode::ValueName)->GetLinkedTargetPins());
				}
				else
				{
					TargetPins.Add(PinToVisit);
				}
			}
			
			for (const URigVMPin* TargetPin : TargetPins)
			{
				TargetNode = TargetPin->GetNode();

				// If the target pin is const, the connection is fine.
				// However, whether this target is an IO pin or an input variable pin — both represent
				// mutable references that a pure function must not be allowed to write through.
				const bool bIsIOPin = (TargetPin->GetDirection() == ERigVMPinDirection::IO);
				const bool bIsInputVariablePin = TargetPin->IsDefinedAsInputVariable();
				const bool bIsConstPin = TargetPin->IsDefinedAsConstant();
				...
				if (bIsIOPin || bIsInputVariablePin)
				{
					// Build a human-readable label for the target pin kind used in failure messages
					const TCHAR* TargetPinKind = bIsInputVariablePin ? TEXT("input variable") : TEXT("IO");

					// Local variable getters are allowed — they're function-scoped and
					// do not expose external state, so mutation is safe.
					if (const URigVMVariableNode* VarNode = Cast<URigVMVariableNode>(SourcePin->GetNode()))
					{
						if (VarNode->IsGetter() && !VarNode->IsLocalVariable())
						{
							...
							{
								*OutFailureReason = FString::Printf(
									TEXT("Only local variables can connect to %s pin '%s' in a pure function %s. Non-local data cannot be modified."),
									TargetPinKind, *TargetPin->GetPinPath(), *OwningFunction->GetName());
							}
					...
					// Entry node pins (input parameters) connected to IO / input variable pins
					// would allow the pure function to mutate caller-owned data.
					if (SourceNode->IsA<URigVMFunctionEntryNode>())
					{
						if (OutFailureReason)
						{
							*OutFailureReason = FString::Printf(
								TEXT("Input parameter '%s' cannot connect to %s pin '%s' in a pure function %s. This would allow modification of the input."),
								*SourcePin->GetName(), TargetPinKind, *TargetPin->GetPinPath(), *OwningFunction->GetName());
						}
						return false;
					}
				}
