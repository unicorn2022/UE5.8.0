...
bool URigVMDispatchNode::IsPure() const
{
	if (const UScriptStruct* Struct = GetFactoryStruct())
	{
...
const TArray<FName>& URigVMDispatchNode::GetControlFlowBlocks() const
{
	if (const FRigVMDispatchFactory* Factory = GetFactory())
	{
		return Factory->GetControlFlowBlocks(GetDispatchContext(), GetTemplatePinTypeMap());
	}
...
bool URigVMDispatchNode::IsPinConstant(const URigVMPin* InPin) const
{
	const URigVMPin* RootPin = InPin->GetRootPin();
	check(RootPin->GetNode() == this);
	...
	if(const FRigVMDispatchFactory* Factory = GetFactory())
	{
		return Factory->HasArgumentMetaData(RootPin->GetFName(), FRigVMStruct::ConstantMetaName); 
	}
...
FName URigVMDispatchNode::GetDisplayNameForPin(const URigVMPin* InPin) const
{
	if(const FRigVMDispatchFactory* Factory = GetFactory())
	{
		return Factory->GetDisplayNameForArgument(*InPin->GetSubPinPath(InPin->GetRootPin(), true), GetTemplatePinTypeMap());
	}
