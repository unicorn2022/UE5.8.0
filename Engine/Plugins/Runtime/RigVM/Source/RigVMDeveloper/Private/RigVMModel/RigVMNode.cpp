...
bool URigVMNode::IsLoopNode() const
{
	if(IsControlFlowNode())
	{
		...
		if(Blocks.Num() == 2 && Blocks[1] == FRigVMStruct::ForLoopCompletedPinName)
		{
			// The body block is named "ExecuteContext" for FRigVMStruct-based loops
			// and "ExecutePin" for FRigVMStructMutable-based loops (e.g. ForLoopCount).
			return Blocks[0] == FRigVMStruct::ExecuteContextName || Blocks[0] == FRigVMStruct::ExecutePinName;
		}
	}
