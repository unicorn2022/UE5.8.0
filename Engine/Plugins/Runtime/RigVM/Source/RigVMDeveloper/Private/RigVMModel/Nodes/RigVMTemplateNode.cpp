...
FString URigVMTemplateNode::GetInitialDefaultValueForPin(const FName& InRootPinName, const TArray<int32>& InPermutationIndices) const
{
	...
	if (Argument)
	{
		for(const int32 PermutationIndex : PermutationIndices)
		{
			...
			const TRigVMTypeIndex TypeIndex = Argument->GetTypeIndex_NoLock(PermutationIndex, RegistryReadLock);
			...
			else if(const FRigVMFunction* Permutation = Template->GetPermutation_NoLock(PermutationIndex, RegistryReadLock))
			{
				const TSharedPtr<FStructOnScope> StructOnScope = MakeShareable(new FStructOnScope(Permutation->Struct));
			...
			{
				if (RegistryReadLock->IsArrayType_NoLock(TypeIndex))
				{
					NewDefaultValue = TEXT("()");
				}
				else
				{
					const FRigVMTemplateArgumentType& Type = RegistryReadLock->GetType_NoLock(TypeIndex);
...
FName URigVMTemplateNode::GetDisplayNameForPin(const URigVMPin* InPin, const TArray<int32>& InPermutationIndices) const
{
#if WITH_EDITOR
	if(const FRigVMTemplate* Template = GetTemplate())
	{
		...
		if(InPin->IsRootPin())
		{
			...
			const FText DisplayNameText = Template->GetDisplayNameForArgument(RootPinName, InPermutationIndices.Num() > 0 ? InPermutationIndices : GetResolvedPermutationIndices(false));
...
{
	return GetDisplayNameForPin(InPin, GetResolvedPermutationIndices(true));
}
...
{
	return TryReduceTypesToSingle_NoLock(InTypes, FRigVMRegistryReadLock(), PreferredType);
}

TRigVMTypeIndex URigVMTemplateNode::TryReduceTypesToSingle_NoLock(const TArray<TRigVMTypeIndex>& InTypes, const FRigVMRegistryHandle& InRegistry, const TRigVMTypeIndex PreferredType) const
{
	if (InTypes.IsEmpty())
	{
		return INDEX_NONE;
	...
	{
		if (!InRegistry->CanMatchTypes_NoLock(InTypes[0], InTypes[i], true))
		{
			return INDEX_NONE;
		}
	}
