...
{
	return !IsLocalVariable() && !IsInputArgument() && !IsInputVariable();
}
...
bool URigVMVariableNode::IsInputArgument() const
{
	...
	if(URigVMGraph* Graph = GetGraph())
	{
		...
		{
			if (const URigVMPin* Pin = EntryNode->FindRootPinByName(CurrentVariableName))
			{
...
bool URigVMVariableNode::IsInputVariable() const
{
	const FName CurrentVariableName = GetVariableName();
...
bool URigVMVariableNode::IsPure() const
{
	if (IsGetter())
	{
		return true;
	...
	// If its a local variable return true
	if (IsLocalVariable())
	{
		return true;
	}
