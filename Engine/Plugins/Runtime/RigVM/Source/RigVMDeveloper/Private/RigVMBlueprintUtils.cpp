...
FName FRigVMBlueprintUtils::ValidateName(FRigVMEditorAssetInterfacePtr InBlueprint, const FString& InName)
{
...
UEdGraphNode* FRigVMBlueprintUtils::GetNodeByGUID(const FRigVMEditorAssetInterfacePtr InBlueprint, const FGuid& InNodeGuid)
{
...
FRigVMEditorAssetInterfacePtr FRigVMBlueprintUtils::FindAssetForGraph(const UEdGraph* Graph)
{
	for (UObject* TestOuter = Graph ? Graph->GetOuter() : nullptr; TestOuter; TestOuter = TestOuter->GetOuter())
	{
		if (TestOuter->Implements<URigVMEditorAssetInterface>())
		{
			return TestOuter;
		}
	}
...
FRigVMEditorAssetInterfacePtr FRigVMBlueprintUtils::FindAssetForNode(const UEdGraphNode* Node)
{
	if (Node)
	{
...
FName FRigVMBlueprintUtils::FindUniqueVariableName(const IRigVMEditorAssetInterface* InBlueprint, const FString& InBaseName)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
{
	if (FRigVMEditorAssetInterfacePtr Interface = InBlueprint)
	{
		HandleReconstructAllBlueprintNodes(Interface);
	}
}

void FRigVMBlueprintUtils::HandleReconstructAllBlueprintNodes(FRigVMEditorAssetInterfacePtr InBlueprint)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
void FRigVMBlueprintUtils::HandleRefreshAllNodes(FRigVMEditorAssetInterfacePtr InBlueprint)
{
...
{
	if (InAssetData.GetClass() && InAssetData.GetClass()->ImplementsInterface(URigVMEditorAssetInterface::StaticClass()))
	{
...
void FRigVMBlueprintUtils::RemoveMemberVariableIfNotUsed(FRigVMEditorAssetInterfacePtr Blueprint, const FName VarName)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
