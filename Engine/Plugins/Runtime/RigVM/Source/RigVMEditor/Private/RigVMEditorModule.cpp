...
#include "ToolMenus.h"
#include "Editor/RigVMPythonLogDetails.h"
...
TAutoConsoleVariable<bool> CVarRigVMBlueprintIndependentAssets(TEXT("RigVM.BlueprintIndependentAssets"), true, TEXT("Create assets that do not inherit from blueprint."));
...
FRewindDebuggerRigVMTrackCreator GRewindDebuggerRigVMTrackCreator;
#endif
				...
				Section->AddDynamicEntry("CreateVariant", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& InSection)
				{
					...
					if (Context)
					{
						if(CVarRigVMEnableVariants.GetValueOnAnyThread())
						{
							...
							for (FAssetData SelectedAssetData : SelectedAssets)
							{
								if (!SelectedAssetData.GetClass()->ImplementsInterface(URigVMAssetInterface::StaticClass()) &&
									!SelectedAssetData.GetClass()->ImplementsInterface(URigVMRuntimeAssetInterface::StaticClass()))
								{
							...
							InSection.AddMenuEntry("CreateVariant", LOCTEXT("CreateVariant", "Create variant"), LOCTEXT("CreateVariant_ToolTip", "Create a variant for this asset"), FSlateIcon(FRigVMEditorStyle::Get().GetStyleSetName(), "RigVM", "RigVM.Unit"), FExecuteAction::CreateLambda([SelectedAssets]()
							{
								for (FAssetData SelectedAssetData : SelectedAssets)
								{
									...
									if (IRigVMEditorAssetInterface* DuplicateBlueprint = Cast<IRigVMEditorAssetInterface>(DuplicateAsset))
									{
										DuplicateBlueprint->GetAssetVariant() = Cast<IRigVMEditorAssetInterface>(SelectedObject)->GetAssetVariant();
									}
									else if (IRigVMRuntimeAssetInterface* DuplicateRuntimeBlueprint = Cast<IRigVMRuntimeAssetInterface>(DuplicateAsset))
									{
										DuplicateRuntimeBlueprint->GetAssetVariant() = Cast<IRigVMRuntimeAssetInterface>(SelectedObject)->GetAssetVariant();
									}
								}
							}));
...
#if RIGVM_TRACE_ENABLED
		IModularFeatures::Get().RegisterModularFeature( IRewindDebuggerExtension::ModularFeatureName, &RewindDebuggerForRigVM);
		IModularFeatures::Get().RegisterModularFeature(TraceServices::ModuleFeatureName, &RigVMTraceModule);
		IModularFeatures::Get().RegisterModularFeature(RewindDebugger::IRewindDebuggerTrackCreator::ModularFeatureName, &GRewindDebuggerRigVMTrackCreator);
#endif

		// Register asset type actions for EditorAsset redirection
		IAssetTools& AssetTools = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools").Get();
		TSharedRef<IAssetTypeActions> EditorAssetActions = MakeShareable(new FRigVMEditorAssetActions());
		RegisteredAssetTypeActions.Add(EditorAssetActions);
		AssetTools.RegisterAssetTypeActions(EditorAssetActions);
	}
...
void FRigVMEditorModule::StartupModuleCommon()
{
...
#if WITH_RIGVMLEGACYEDITOR
	const UClass* BlueprintClass = GetRigVMLegacyBlueprintClass();
	FBlueprintEditorModule& BlueprintEditorModule = FModuleManager::LoadModuleChecked<FBlueprintEditorModule>("Kismet");
	...
	PropertyEditorModule.RegisterCustomPropertyTypeLayout(PropertiesToUnregisterOnShutdown.Last(), FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FRigVMPythonLogDetails::MakeInstance));

	AssetsPreDeleteDelegate = FEditorDelegates::OnAssetsPreDelete.AddStatic(&FRigVMEditorModule::HandleAssetsPreDelete);
}
...
#if RIGVM_TRACE_ENABLED
			IModularFeatures::Get().UnregisterModularFeature( IRewindDebuggerExtension::ModularFeatureName, &RewindDebuggerForRigVM);
			IModularFeatures::Get().UnregisterModularFeature(TraceServices::ModuleFeatureName, &RigVMTraceModule);
			IModularFeatures::Get().UnregisterModularFeature(RewindDebugger::IRewindDebuggerTrackCreator::ModularFeatureName, &GRewindDebuggerRigVMTrackCreator);
#endif
...
void FRigVMEditorModule::ShutdownModuleCommon()
{
	...
	// Clear all instances in the class pools of the details wrapper objects
	URigVMDetailsViewWrapperObject::ClearAllClassPools();

	// Unregister to fixup newly created BPs
	FKismetEditorUtilities::UnregisterAutoBlueprintNodeCreation(this);
	
	FEditorDelegates::OnAssetsPreDelete.Remove(AssetsPreDeleteDelegate);
}

void FRigVMEditorModule::GetTypeActions(FRigVMEditorAssetInterfacePtr RigVMBlueprint, FBlueprintActionDatabaseRegistrar& ActionRegistrar)
{
	...
	TArray<TSharedRef<URigVMEdGraphNodeSpawner>> NodeSpawners;
	GetTypeActions(RigVMBlueprint, NodeSpawners);

	for (TSharedRef<URigVMEdGraphNodeSpawner>& NodeSpawner : NodeSpawners)
	{
...
void FRigVMEditorModule::GetTypeActions(FRigVMEditorAssetInterfacePtr RigVMBlueprint, TArray<TSharedRef<URigVMEdGraphNodeSpawner>>& OutGraphNodeSpawners)
{
	// Only register actions for ourselves
	UClass* InClass = RigVMBlueprint.GetObject()->GetClass();
	if (TScriptInterface<IRigVMRuntimeAssetInterface> RuntimeAsset = RigVMBlueprint->GetRuntimeAssetInterface())
	{
		InClass = RuntimeAsset.GetObject()->GetClass();
	}
	UClass* BlueprintClass = GetRigVMAssetClass();
	if (RigVMBlueprint->IsBlueprintAsset() && CVarRigVMBlueprintIndependentAssets->GetBool())
	{
		BlueprintClass = GetRigVMLegacyBlueprintClass();
	}
	...
	for (const FRigVMTemplate& Template : Registry.GetTemplates())
	{
		...
		ensure (!Template.GetNotation().ToString().Contains(TEXT("AnimNext")));
		...
		OutGraphNodeSpawners.Add(NodeSpawner);
	};
	...
	if(PublicGraphFunctionsProperty || PublicFunctionsProperty)
	{
		...
		TMap<FRigVMGraphFunctionIdentifier, FAssetData> ControlRigAssetDatas;
		const TArray<FRigVMGraphFunctionIdentifier> FunctionIds = URigVMBuildData::Get()->GetAllFunctionIdentifiers();
		for (const FRigVMGraphFunctionIdentifier& FunctionId : FunctionIds)
		{
			// Starting from a library node path
			const FString LibraryNodePath = FunctionId.GetLibraryNodePath();

			FAssetData AssetData = FunctionId.GetAssetData();
			if (AssetData.IsValid())  // Found in registry = exists on disk
			{
				ControlRigAssetDatas.FindOrAdd(FunctionId) = AssetData;
			}
		}
		...
		auto AddClassToFilter = [&ControlRigAssetFilter](const UClass* InClass)
		{
			...
			while(Class)
			{
				...
				if (Class == URigVMBlueprint::StaticClass())
				{
					ControlRigAssetFilter.ClassPaths.Add(URigVMRuntimeAsset::StaticClass()->GetClassPathName());
				}
				else if (Class == URigVMRuntimeAsset::StaticClass())
				{
					ControlRigAssetFilter.ClassPaths.Add(URigVMBlueprint::StaticClass()->GetClassPathName());
				}
				else if (Class->GetClassPathName() == FTopLevelAssetPath(TEXT("/Script/ControlRig"), TEXT("ControlRigRuntimeAsset")))
				{
					ControlRigAssetFilter.ClassPaths.Add(FTopLevelAssetPath(TEXT("/Script/ControlRigDeveloper"), TEXT("ControlRigBlueprint")));
				}
				else if (Class->GetClassPathName() == FTopLevelAssetPath(TEXT("/Script/ControlRigDeveloper"), TEXT("ControlRigBlueprint")))
				{
					ControlRigAssetFilter.ClassPaths.Add(FTopLevelAssetPath(TEXT("/Script/ControlRig"), TEXT("ControlRigRuntimeAsset")));
				}
		...
		AddClassToFilter(RigVMBlueprint->GetAssetObjectForEditor()->GetClass());
		AddClassToFilter(RigVMBlueprint->GetRigVMGeneratedClassPrototype());

		TArray<FRigVMGraphFunctionHeader> PublicFunctions;
		for(TPair<FRigVMGraphFunctionIdentifier, FAssetData> Pair : ControlRigAssetDatas)
		{
			if (!ControlRigAssetFilter.ClassPaths.Contains(Pair.Value.AssetClassPath))
			{
				continue;
			}

			if (!AssetsPublicFunctionsAllowed(Pair.Value))
			{
				continue;
			}

			const FRigVMGraphFunctionHeader& Header = URigVMBuildData::Get()->FindFunctionHeader(Pair.Key);
			if (Header.IsValid())
			{
				PublicFunctions.Add(Header);
			}
		}
		for(FRigVMGraphFunctionHeader& PublicFunction : PublicFunctions)
		{
			...
			{
				TSharedRef<URigVMEdGraphFunctionRefNodeSpawner> NodeSpawner = MakeShared<URigVMEdGraphFunctionRefNodeSpawner>(ControlRigAssetDatas.FindChecked(PublicFunction.LibraryPointer), PublicFunction);
				NodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
				OutGraphNodeSpawners.Add(NodeSpawner);
			}
		}
...
void FRigVMEditorModule::GetInstanceActions(FRigVMEditorAssetInterfacePtr RigVMBlueprint, FBlueprintActionDatabaseRegistrar& ActionRegistrar)
{
	if (UClass* GeneratedClass = Cast<UClass>(RigVMBlueprint->GetRuntimeAssetInterface().GetObject()))
	{
		...
		TArray<TSharedRef<URigVMEdGraphNodeSpawner>> NodeSpawners;
		GetInstanceActions(RigVMBlueprint, NodeSpawners);

		for (TSharedRef<URigVMEdGraphNodeSpawner>& NodeSpawner : NodeSpawners)
		{
			URigVMEdGraphNodeBlueprintSpawner* BlueprintNodeSpawner = URigVMEdGraphNodeBlueprintSpawner::CreateFromRigVMSpawner(NodeSpawner);
			ActionRegistrar.AddBlueprintAction(GeneratedClass, BlueprintNodeSpawner);
		}
	}
}

void FRigVMEditorModule::GetInstanceActions(FRigVMEditorAssetInterfacePtr RigVMBlueprint, TArray<TSharedRef<URigVMEdGraphNodeSpawner>>& OutGraphNodeSpawners)
{
	// Only register actions for ourselves
	UClass* InClass = RigVMBlueprint.GetObject()->GetClass();
	if (!RigVMBlueprint->IsBlueprintAsset())
	{
		if (TScriptInterface<IRigVMRuntimeAssetInterface> RuntimeAsset = RigVMBlueprint->GetRuntimeAssetInterface())
		{
			InClass = RuntimeAsset.GetObject()->GetClass();
	...
	for (const FRigVMGraphVariableDescription& Variable : Variables)
	{
		...
		FText MenuDesc = FText::FromName(ExternalVariable.GetName());
		FText ToolTip = FText::FromString(FString::Printf(TEXT("Get the value of variable %s"), *ExternalVariable.GetName().ToString()));
		TSharedRef<URigVMEdGraphVariableNodeSpawner> GetNodeSpawner = MakeShared<URigVMEdGraphVariableNodeSpawner>(RigVMBlueprint, ExternalVariable, true, MenuDesc, NodeCategory, ToolTip);
		GetNodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
		OutGraphNodeSpawners.Add(GetNodeSpawner);

		ToolTip = FText::FromString(FString::Printf(TEXT("Set the value of variable %s"), *ExternalVariable.GetName().ToString()));
		TSharedRef<URigVMEdGraphVariableNodeSpawner> SetNodeSpawner = MakeShared<URigVMEdGraphVariableNodeSpawner>(RigVMBlueprint, ExternalVariable, false, MenuDesc, NodeCategory, ToolTip);
		SetNodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
		OutGraphNodeSpawners.Add(SetNodeSpawner);
	}
	...
	if (URigVMFunctionLibrary* LocalFunctionLibrary = RigVMBlueprint->GetLocalFunctionLibrary())
	{
		...
		for (URigVMLibraryNode* Function : Functions)
		{
			...
			TSharedRef<URigVMEdGraphFunctionRefNodeSpawner> NodeSpawner = MakeShared<URigVMEdGraphFunctionRefNodeSpawner>(Function);
			NodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
			OutGraphNodeSpawners.Add(NodeSpawner);
		}
		...
		for (URigVMLibraryNode* Function : Functions)
		{
			for (const FRigVMGraphVariableDescription& LocalVariable : Function->GetContainedGraph()->GetLocalVariables())
			{
				...
				TSharedRef<URigVMEdGraphVariableNodeSpawner> GetNodeSpawner = MakeShared<URigVMEdGraphVariableNodeSpawner>(RigVMBlueprint, Function->GetContainedGraph(), LocalVariable, true, MenuDesc, LocalNodeCategory, ToolTip);
				GetNodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
				OutGraphNodeSpawners.Add(GetNodeSpawner);

				ToolTip = FText::FromString(FString::Printf(TEXT("Set the value of variable %s"), *LocalVariable.Name.ToString()));
				TSharedRef<URigVMEdGraphVariableNodeSpawner> SetNodeSpawner = MakeShared<URigVMEdGraphVariableNodeSpawner>(RigVMBlueprint, Function->GetContainedGraph(), LocalVariable, false, MenuDesc, LocalNodeCategory, ToolTip);
				SetNodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
				OutGraphNodeSpawners.Add(SetNodeSpawner);
			}
		}
	...
	for (URigVMGraph* Graph : RigVMBlueprint->GetAllModels())
	{
		if (Graph->GetEntryNode())
		{
			...
			for (const FRigVMGraphVariableDescription& InputArgument : Graph->GetInputArguments())
			{
				...
				TSharedRef<URigVMEdGraphVariableNodeSpawner> NodeSpawner = MakeShared<URigVMEdGraphVariableNodeSpawner>(RigVMBlueprint, Graph, InputArgument, true, MenuDesc, InputNodeCategory, ToolTip);
				NodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
				OutGraphNodeSpawners.Add(NodeSpawner);
			}
			
			static const FText InputVariableCategory = LOCTEXT("InputVariables", "Input Variables");
			for (const FRigVMGraphVariableDescription& InputVariable : Graph->GetInputVariables())
			{
				FText MenuDesc = FText::FromName(InputVariable.Name);
				FText ToolTip = FText::FromString(FString::Printf(TEXT("Get the value of input variable %s"), *InputVariable.Name.ToString()));
				TSharedRef<URigVMEdGraphVariableNodeSpawner> NodeSpawner = MakeShared<URigVMEdGraphVariableNodeSpawner>(RigVMBlueprint, Graph, InputVariable, true, MenuDesc, InputVariableCategory, ToolTip);
				NodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
				OutGraphNodeSpawners.Add(NodeSpawner);

				ToolTip = FText::FromString(FString::Printf(TEXT("Set the value of input variable %s"), *InputVariable.Name.ToString()));
				NodeSpawner = MakeShared<URigVMEdGraphVariableNodeSpawner>(RigVMBlueprint, Graph, InputVariable, false, MenuDesc, InputVariableCategory, ToolTip);
				NodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
				OutGraphNodeSpawners.Add(NodeSpawner);
			}			
		}
	...
	if(!EntryNames.IsEmpty())
	{
		...
		for (const FName& EntryName : EntryNames)
		{
			...
			TSharedRef<URigVMEdGraphInvokeEntryNodeSpawner> NodeSpawner = MakeShared<URigVMEdGraphInvokeEntryNodeSpawner>(RigVMBlueprint, EntryName, MenuDesc, EventNodeCategory, ToolTip);
			NodeSpawner->SetRelatedBlueprintClass(BlueprintClass);
			OutGraphNodeSpawners.Add(NodeSpawner);
		}
	}
...
void FRigVMEditorModule::GetNodeContextMenuActions(TScriptInterface<IRigVMClientHost> RigVMClientHost, const URigVMEdGraphNode* EdGraphNode, URigVMNode* ModelNode, UToolMenu* Menu) const
{
	...
	if(FRigVMEditorAssetInterfacePtr RigVMBlueprint = RigVMClientHost.GetObject())
	{
		UClass* BPClass = RigVMBlueprint->GetObject()->GetClass();
		UClass* BaseClass = GetRigVMAssetClass();
		if (RigVMBlueprint->IsBlueprintAsset() && CVarRigVMBlueprintIndependentAssets->GetBool())
		{
			BaseClass = GetRigVMLegacyBlueprintClass();
		}
		else if (BaseClass->IsChildOf(URigVMRuntimeAsset::StaticClass()))
		{
			BPClass = RigVMBlueprint->GetRuntimeAssetInterface().GetObject()->GetClass();
		}
		if(BPClass != BaseClass && !BPClass->IsChildOf(BaseClass))
		{
			return;
		}
	}
...
{
	const TScriptInterface<IRigVMEditorAssetInterface> RigVMAsset = RigVMClientHost.GetObject();
	if (!EdGraphNode || 
		!ModelNode || 
		!Menu || 
		!RigVMAsset)
	...
	const TWeakInterfacePtr<IRigVMEditorAssetInterface> WeakRigVMAsset = RigVMAsset;
	...
	NodeActionsSection.AddMenuEntry(FGenericCommands::Get().Paste);
...
{
	FRigVMEditorAssetInterfacePtr RigVMBlueprint = RigVMClientHost.GetObject();
...
{
	if(FRigVMEditorAssetInterfacePtr RigVMAsset = RigVMClientHost.GetObject())
	{
			...
			if(!FunctionReferenceNode->IsFullyRemapped() && FunctionReferenceNode->GetReferencedFunctionHeader().LibraryPointer.GetNodeSoftPath().ResolveObject())
			{
				...
				VariablesSection.AddMenuEntry(
					...
					FUIAction(FExecuteAction::CreateLambda([Controller, FunctionReferenceNode, RigVMBlueprint]() {
						...
						const TArray<FRigVMExternalVariable> ExternalVariables = FunctionReferenceNode->GetExternalVariables();
						if(!ExternalVariables.IsEmpty())
						{
							...
							if (URigVMLibraryNode* LibraryNode = FunctionReferenceNode->LoadReferencedNode())
							{
							   ...
							   // ReferencedBlueprint != RigVMBlueprint - since only FunctionReferenceNodes from other assets have the potential to be unmapped
								...
								{
									URigVMPin* InterfacePin = FunctionReferenceNode->FindRootPinByName(ExternalVariable.GetName());
									if (!InterfacePin)
									{
										continue;
									}
									...
									if(ReferencedBlueprint)
									{
										...
										{
											if(NewVariable.Name == ExternalVariable.GetName())
											{
									...
									{
										Controller->BindPinToVariable(InterfacePin->GetPinPath(), NewVariableName.ToString());
									}
								}
							}
	...
	if(bCanNodeBeUpgraded || bIsFunctionReference)
	{
		...
		if(bIsFunctionReference)
		{
			VariantSection.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(FExecuteAction::CreateLambda([Model, RigVMClientHost]() {
					...
					if(!FunctionReferenceNodes.IsEmpty())
					{
						...
						WidgetArgs
							...
							.Assets({FAssetData(RigVMClientHost->GetRigVMGraphFunctionHost().GetObject())})
...
{
	FRigVMEditorAssetInterfacePtr Blueprint = RigVMClientHost.GetObject();
...
void FRigVMEditorModule::GetPinContextMenuActions(TScriptInterface<IRigVMClientHost> RigVMClientHost, const UEdGraphPin* EdGraphPin, URigVMPin* ModelPin, UToolMenu* Menu) const
{
	...
	GetPinSequenceContextMenuActions(RigVMClientHost, EdGraphPin, ModelPin, Menu);
...
{
	if (URigVMDispatchNode* DispatchNode = Cast<URigVMDispatchNode>(ModelPin->GetNode()))
	{
...
void FRigVMEditorModule::GetPinSequenceContextMenuActions(TScriptInterface<IRigVMClientHost> RigVMClientHost, const UEdGraphPin* EdGraphPin, URigVMPin* ModelPin, UToolMenu* Menu) const
{
	if (ModelPin->GetDirection() == ERigVMPinDirection::Output)
	{
		URigVMController* Controller = RigVMClientHost->GetRigVMClient()->GetController(ModelPin->GetGraph());
		if (URigVMAggregateNode* AggregateNode = Cast<URigVMAggregateNode>(ModelPin->GetNode()))
		{
			if (URigVMUnitNode* UnitNode = Cast<URigVMUnitNode>(AggregateNode->GetFirstInnerNode()))
			{
				if (UnitNode->GetScriptStruct() == FRigVMFunction_Sequence::StaticStruct())
				{
					FToolMenuSection& Section = Menu->AddSection("RigVMEditorContextMenuSequencePin", LOCTEXT("SequencePin", "Sequence"));
					Section.AddMenuEntry(
						"InsertSequencePin",
						LOCTEXT("InsertSequencePin", "Insert Pin Above"),
						LOCTEXT("InsertSequencePin_Tooltip", "Inserts a new output pin"),
						FSlateIcon(),
						FUIAction(FExecuteAction::CreateLambda([Controller, ModelPin, AggregateNode]() {

							Controller->GetActionStack()->OpenUndoBracket(Controller, TEXT("Insert Sequence Pin"));
							if (!Controller->AddAggregatePin(AggregateNode, FString(), FString(), true, true).IsEmpty())
							{
								TArray<URigVMPin*> Pins = AggregateNode->GetPins();
								for (int32 i=Pins.Num()-1; i > 0; i--)
								{
									...
									if (Pins.IsValidIndex(i-1))
									{
										TArray<URigVMPin*> TargetPins = Pins[i-1]->GetLinkedTargetPins();
										
										Controller->BreakAllLinks(Pins[i-1], false);
										for (URigVMPin* TargetPin : TargetPins)
										{
											Controller->AddLink(Pins[i], TargetPin);
										}
									}
								}
							}
							...
							{
								Controller->GetActionStack()->CancelUndoBracket(Controller);
							}
							Controller->GetActionStack()->CloseUndoBracket(Controller);
						})
					));
				}
			}
...
void FRigVMEditorModule::PostChange(const UUserDefinedStruct* Changed, FStructureEditorUtils::EStructureEditorChangeInfo ChangedType)
{
	...
	TArray<FRigVMEditorAssetInterfacePtr> BlueprintsToRefresh;
	...
	for (TObjectIterator<URigVMPin> It(RF_Transient | RF_ClassDefaultObject, /** bIncludeDerivedClasses */ true, /** InternalExcludeFlags */ EInternalObjectFlags::Garbage); It; ++It)
	{
		...
		{
			if (FRigVMEditorAssetInterfacePtr RigVMBlueprint = IRigVMEditorAssetInterface::GetInterfaceOuter(Pin))
			{
				...
				if (Pin->GetGraph() == RigVMBlueprint->GetLocalFunctionLibrary())
				{
					...
					for (const TSoftObjectPtr<URigVMFunctionReferenceNode>& Reference : References)
					{
						...
						if (FRigVMEditorAssetInterfacePtr FunctionUserBlueprint = IRigVMEditorAssetInterface::GetInterfaceOuter(RefNode))
						{
	...
	for (FRigVMEditorAssetInterfacePtr RigVMBlueprint : BlueprintsToRefresh)
	{
		RigVMBlueprint->OnRigVMRegistryChanged();
		(void)RigVMBlueprint.GetObject()->MarkPackageDirty();
	}
...
void FRigVMEditorModule::CreateRootGraphIfRequired(FRigVMEditorAssetInterfacePtr InBlueprint) const
{
...
{
	CreateRootGraphIfRequired(FRigVMEditorAssetInterfacePtr(InBlueprint));
}
...
bool FRigVMEditorModule::IsRigVMEditorModuleBase() const
{
	...
	if (Class == URigVMEditorAssetInterface::StaticClass() || Class->ImplementsInterface(URigVMEditorAssetInterface::StaticClass()))
	{
		return true;
	}
	return false;
...
void FRigVMEditorModule::HandleAssetsPreDelete(const TArray<UObject*>& InAssetsToDelete)
{
	for (UObject* AssetToDelete : InAssetsToDelete)
	{
		if (URigVMRuntimeAsset* Asset = Cast<URigVMRuntimeAsset>(AssetToDelete))
		{
			Asset->MarkInstancesAsGarbage();
		}
	}
}
