	...
	if(!RemainingTasks.IsEmpty())
	{
		...
		ExecuteOnGameThread(UE_SOURCE_LOCATION, [this]()
		{
			...
			if(Task.IsValid())
			{
				...
				if(FRigVMEditorAssetInterfacePtr Blueprint = Task->GetRigVMAsset(Phase))
				{
							...
							if(InSubject)
							{
								...
								if(InMessage.Split(TEXT("@@"), &Left, &Right))
								{
									...
									SubjectToken->OnMessageTokenActivated(FOnMessageTokenActivated::CreateLambda([WeakSubject](const TSharedRef<IMessageToken>&)
									{
										...
										{
											if (FRigVMEditorAssetInterfacePtr EditorAsset = IRigVMEditorAssetInterface::GetInterfaceOuter(WeakSubject.Get()))
											{
