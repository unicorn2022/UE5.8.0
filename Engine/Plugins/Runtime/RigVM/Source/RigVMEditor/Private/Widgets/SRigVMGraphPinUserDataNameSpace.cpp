...
TArray<TSharedPtr<FString>>& SRigVMGraphPinUserDataNameSpace::GetNameSpaces()
{
	...
	if (FRigVMEditorAssetInterfacePtr EditorAsset = IRigVMEditorAssetInterface::GetInterfaceOuter(GraphPinObj->GetOwningNode()))
	{
		if(const UObject* DebuggedObject = EditorAsset->GetObjectBeingDebugged())
		{
