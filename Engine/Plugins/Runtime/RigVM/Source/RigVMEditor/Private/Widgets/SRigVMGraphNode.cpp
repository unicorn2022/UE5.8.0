...
void SRigVMGraphNode::Construct( const FArguments& InArgs )
{
	...
	SetEnabled(InArgs._Enabled);
	PinDragDropEnabledAttribute = InArgs._PinDragDropEnabled;
...
TOptional<FSlateColor> SRigVMGraphNode::GetPinTextColor(const SGraphPin* InGraphPin) const
{
	...
	const URigVMFunctionReferenceNode* FunctionReferenceNode = Cast<URigVMFunctionReferenceNode>(ModelNode);
	if (!FunctionInterfaceNode && !FunctionReferenceNode)
	{
		return Result;
	}
	...
	{
		if (FunctionInterfaceNode && !ModelPin->IsLinked() &&
		...
		if (FunctionReferenceNode && 
			...
			ModelPin->IsDefinedAsInputVariable() &&
			ModelPin->GetBoundVariableName().IsEmpty())
		{
...
void SRigVMGraphNode::CreateAddPinButton()
{
	if (LeftNodeBox.IsValid() && ModelNode.IsValid())
	{
		...
		if (URigVMDispatchNode* DispatchNode = Cast<URigVMDispatchNode>(ModelNode.Get()))
		{
...
void SRigVMGraphNode::AddPin(const TSharedRef<SGraphPin>& PinToAdd) 
{
	if(ModelNode.IsValid())
	{
		...
		const bool bPinDragDropEnabled = PinDragDropEnabledAttribute.Get(true);
		PinToAdd->EnableDragAndDrop(bPinDragDropEnabled);
	}
}
...
void SRigVMGraphNode::CreateStandardPinWidget(UEdGraphPin* CurPin)
{
	...
	if(const URigVMEdGraphNode* RigGraphNode = Cast<URigVMEdGraphNode>(GraphNode))
	{
		if(const URigVMPin* ModelPin = RigGraphNode->FindModelPinFromGraphPin(CurPin))
		{
			...
			bIsInputVariable = ModelPin->IsDefinedAsInputVariable();
		}
	}
	...
	if (bShowPin)
	{
		...
		if (RecycledPinPtr)
		{
			...
			if(PinInfoMetaData.IsValid())
			{
				if(PinInfoMetaData->CPPType == CPPType &&
					...
					PinInfoMetaData->bIsInputVariable == bIsInputVariable)
				{
					NewPin = RecycledPin;
				}
			}
		...
		if(!NewPin.IsValid())
		{
			...
			NewPin->AddMetadata(MakeShared<FPinInfoMetaData>(CPPType, BoundVariableName, bIsInputVariable));
...
EVisibility SRigVMGraphNode::GetArrayPlusButtonVisibility(URigVMPin* InModelPin) const
{
	if(InModelPin)
	{
		if (InModelPin->IsDefinedAsInputVariable())
		{
...
FText SRigVMGraphNode::GetVariableLabelTooltipText(TWeakInterfacePtr<IRigVMEditorAssetInterface> InBlueprint,
...
void SRigVMGraphNode::GetNodeInfoPopups(FNodeInfoContext* Context, TArray<FGraphInformationPopupInfo>& Popups) const
{
	...
	FRigVMEditorAssetInterfacePtr RigAsset = K2Context->SourceBlueprint;

	if (!RigAsset && RigGraphNode)
	{
		RigAsset = RigGraphNode->GetAsset();
	}

	if (!ActiveObject && RigAsset)
	{
		ActiveObject = RigAsset->GetDebuggedRigVMHost();
	}

	// Display any pending latent actions
	if (ActiveObject && RigAsset && RigGraphNode)
	{
		// Display pinned watches
		if (RigAsset->NodeContainsWatchedPins(GraphNode))
		{
			...
			for (int32 PinIndex = 0; PinIndex < GraphNode->Pins.Num(); ++PinIndex)
			{
				...
				if (RigAsset->IsPinBeingWatched(WatchPin))
				{
					if (URigVMPin* ModelPin = RigGraphNode->GetModel()->FindPin(WatchPin->GetName()))
					{
						...
						if(DefaultValues.IsEmpty() && ModelNode.IsValid())
						{
							// calling on the impl here directly to avoid the ensure / exception around functions
							const FString PinHash = URigVMCompiler::GetPinHashImpl(ModelPin, nullptr);
							
							if (const FRigVMOperand* PinOperand = RigAsset->PinToOperandMap.Find(PinHash))
							{
								if (const FRigVMOperand* DebugOperand = ActiveObject->GetDebugOperandForOperand(*PinOperand))
								{
									...
									const FString DebugValue = Memory->GetDataAsStringSafe(DebugOperand->GetRegisterIndex(), PPF_ExternalEditor | STRUCT_ExportTextItemNative);
...
void SRigVMGraphNode::UpdatePinTreeView()
{
	...
	(const URigVMPin* InModelPin, const FString& InCategory)
	{
		...
		if(bIsModelPin)
		{
			...
			PinInfo.bIsInputVariable = InModelPin->IsDefinedAsInputVariable();
		...
		else
		{
			...
			PinInfo.bIsInputVariable = false;
		...
		if(bIsModelPin)
		{
			...
			PinInfo.bHideInputWidget = (!InModelPin->IsBoundToVariable()) && PinInfo.bIsContainer && !PinInfo.bIsInputVariable;
			if (!PinInfo.bHideInputWidget && !PinInfo.bIsInputVariable)
			{
