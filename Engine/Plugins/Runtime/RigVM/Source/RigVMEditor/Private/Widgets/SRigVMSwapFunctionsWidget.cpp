...
TArray<TSharedRef<FRigVMTreeNode>> FRigVMTreeFunctionRefAssetNode::GetChildrenImpl(const TSharedRef<FRigVMTreeContext>& InContext) const
{
	if(LoadedGraphNodes.IsEmpty())
	{
		...
		{
			const IRigVMClientHost* ClientHost = Cast<IRigVMClientHost>(AssetObject);
			if (!ClientHost)
			{
				if (TScriptInterface<const IRigVMRuntimeAssetInterface> RuntimeAsset = AssetObject)
				{
					ClientHost = Cast<IRigVMClientHost>(RuntimeAsset->GetEditorOnlyData());
				}
			}
			if(ClientHost)
			{
...
{
	const FRigVMGraphFunctionHeader Header = URigVMBuildData::Get()->FindFunctionHeader(Identifier);
	if(Header.IsValid())
	{
...
bool FRigVMSwapFunctionTask::Execute(const TSharedRef<FRigVMTreePhase>& InPhase)
{
	...
	ReferenceNodeToken->OnMessageTokenActivated(FOnMessageTokenActivated::CreateLambda([WeakRefNode](const TSharedRef<IMessageToken>&)
	{
		...
		{
			if (FRigVMEditorAssetInterfacePtr EditorAsset = IRigVMEditorAssetInterface::GetInterfaceOuter(WeakRefNode.Get()))
			{
...
FRigVMEditorAssetInterfacePtr FRigVMSwapFunctionTask::GetRigVMAsset(const TSharedRef<FRigVMTreePhase>& InPhase) const
{
	URigVMFunctionReferenceNode* ReferenceNode = GetReferenceNode(InPhase);
	if(ReferenceNode == nullptr)
	{
	...
	return IRigVMEditorAssetInterface::GetInterfaceOuter(ReferenceNode);
}
...
TArray<TSharedRef<FRigVMTreeNode>> SRigVMSwapFunctionsWidget::GetFunctionIdentifierNodes(const FArguments& InArgs)
{
	...
	TArray<FRigVMGraphFunctionIdentifier> Identifiers = URigVMBuildData::Get()->GetAllFunctionIdentifiers();
	
	// Add private functions from the given assets
	for (const FAssetData& AssetData : InArgs._Assets)
	{
		if (AssetData.IsAssetLoaded())
		{
			UObject* AssetObj = AssetData.FastGetAsset();
			TScriptInterface<IRigVMGraphFunctionHost> FunctionHost = AssetObj;
			if (!FunctionHost)
			{
				if (TScriptInterface<IRigVMClientHost> ClientHost = AssetObj)
				{
			...
			if (FunctionHost)
			{
				for (const FRigVMGraphFunctionData& FuncData : FunctionHost->GetRigVMGraphFunctionStore()->PrivateFunctions)
				{
					Identifiers.Add(FuncData.Header.LibraryPointer);
				}
			}
		}
	}
	...
	{
		FString PackagePath = Identifier.HostObject.GetLongPackageName();
		TSharedPtr<FRigVMTreeFunctionIdentifierAssetNode> PackageCategory;
		if(!VisitedAssets.Contains(PackagePath))
		{
			FAssetData AssetData = FRigVMTreeContext::FindAssetFromAnyPath(PackagePath, false);
			if (!AssetData.IsValid())
			{
				AssetData = FRigVMTreeContext::FindAssetFromAnyPath( Identifier.HostObject.GetAssetPath().ToString(), false);
			}
...
FReply SRigVMSwapFunctionsWidget::OnNodeDoubleClicked(TSharedRef<FRigVMTreeNode> Node)
{
	if(Node->IsA<FRigVMTreeFunctionIdentifierNode>() || Node->IsA<FRigVMTreeFunctionRefNode>())
	{
		...
		if(AssetData.IsValid())
		{
			...
			{
				if(TopLevelObject->GetClass()->ImplementsInterface(URigVMEditorAssetInterface::StaticClass()) || 
					TopLevelObject->GetClass()->ImplementsInterface(URigVMRuntimeAssetInterface::StaticClass()))
				{
					if(const UObject* ObjectReference = StaticLoadObject(UObject::StaticClass(), nullptr, *Node->GetPath(), nullptr))
					{
						UObject* EditorObject = TopLevelObject;
						if (TScriptInterface<IRigVMRuntimeAssetInterface> RuntimeAsset = TopLevelObject)
						{
							EditorObject = RuntimeAsset->GetEditorOnlyData();
						}
						GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(EditorObject);
