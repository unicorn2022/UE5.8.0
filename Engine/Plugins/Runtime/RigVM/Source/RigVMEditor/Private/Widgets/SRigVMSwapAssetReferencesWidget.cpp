...
FReply SRigVMSwapAssetReferencesWidget::OnNodeDoubleClicked(TSharedRef<FRigVMTreeNode> Node)
{
	if(Node->IsA<FRigVMTreePackageNode>())
	{
		...
		if(AssetData.IsValid())
		{
			...
			{
				const bool bValidAsset = TopLevelObject->GetClass()->ImplementsInterface(URigVMEditorAssetInterface::StaticClass()) ||
				...
				if(bValidAsset)
				{
					if(const UObject* ObjectReference = StaticLoadObject(UObject::StaticClass(), nullptr, *Node->GetPath(), nullptr))
					{
						...
						if(IAssetEditorInstance* Editor = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->FindEditorForAsset(EditorObject, true))
						{
