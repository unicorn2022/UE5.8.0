...
URigVMEdGraphVariableNodeSpawner::URigVMEdGraphVariableNodeSpawner(FRigVMEditorAssetInterfacePtr InBlueprint, const FRigVMExternalVariable& InExternalVariable, bool bInIsGetter, const FText& InMenuDesc, const FText& InCategory, const FText& InTooltip)
{
...
URigVMEdGraphVariableNodeSpawner::URigVMEdGraphVariableNodeSpawner(
	FRigVMEditorAssetInterfacePtr InBlueprint, URigVMGraph* InGraphOwner, const FRigVMGraphVariableDescription& InLocalVariable,
...
{
	return FString("ExternalVariable=" + ExternalVariable.GetName().ToString());
}
...
URigVMEdGraphNode* URigVMEdGraphVariableNodeSpawner::Invoke(URigVMEdGraph* ParentGraph, FVector2D const Location) const
{
	...
	FRigVMEditorAssetInterfacePtr RigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(ParentGraph);
	check(RigBlueprint);
	...
	{
		ObjectPath = ExternalVariable.GetCPPTypeObject()->GetPathName();
	}
	...
	if(bIsTemplateNode)
	{
		...
		NodeName = FString::Printf(VariableNodeNameFormat, bIsGetter ? *GetterPrefix : *SetterPrefix, *ExternalVariable.GetName().ToString());
		...
		Pins.Emplace(
			...
			ExternalVariable.GetExtendedCPPType(),
			const_cast<UObject*>(ExternalVariable.GetCPPTypeObject()));
	...
	if (URigVMNode* ModelNode = Controller->AddVariableNodeFromObjectPath(ExternalVariable.GetName(), TypeName, ObjectPath, bIsGetter, FString(), Location, NodeName, true, true))
	{
