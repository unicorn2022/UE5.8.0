...
URigVMEdGraphFunctionRefNodeSpawner::URigVMEdGraphFunctionRefNodeSpawner(const FAssetData& InAssetData, const FRigVMOldPublicFunctionData& InPublicFunction)
{
	...
	if(const FRigVMEditorAssetInterfacePtr ReferencedBlueprint = InAssetData.FastGetAsset(false))
	{
...
URigVMEdGraphNode* URigVMEdGraphFunctionRefNodeSpawner::Invoke(URigVMEdGraph* ParentGraph, FVector2D const Location) const
{
	...
	if(!FBlueprintNodeTemplateCache::IsTemplateOuter(ParentGraph))
	{
		if (GetResolvedAssetPath().IsValid())
		{
			...
			{
				if (BlueprintObj->Implements<URigVMEditorAssetInterface>())
				{
					FRigVMEditorAssetInterfacePtr Blueprint = BlueprintObj;
...
URigVMEdGraphNode* URigVMEdGraphFunctionRefNodeSpawner::SpawnNode(UEdGraph* ParentGraph, FRigVMEditorAssetInterfacePtr Blueprint, FRigVMGraphFunctionHeader& InFunction, FVector2D const Location)
{
	URigVMEdGraphNode* NewNode = nullptr;
...
bool URigVMEdGraphFunctionRefNodeSpawner::IsTemplateNodeFilteredOut(TArray<UObject*>& InAssets, TArray<UEdGraph*> InGraphs, TArray<UEdGraphPin*> InPins) const
{
	...
	if(bIsLocalFunction)
	{
		if(ReferencedPublicFunctionHeader.IsValid())
		{
			...
			{
				if (IRigVMEditorAssetInterface* Asset = Cast<IRigVMEditorAssetInterface>(Blueprint))
				{
					if (Asset->GetRigVMClientHost()->GetRigVMGraphFunctionHost() != ReferencedPublicFunctionHeader.GetFunctionHostObject())
					{
						return true;
					}
				}
