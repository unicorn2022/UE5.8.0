...
void FRigVMLegacyEditor::CommonInitialization(const TArray<FRigVMEditorAssetInterfacePtr>& InitBlueprints, bool bShouldOpenInDefaultsMode)
{
	TArray<UBlueprint*> Blueprints;
	Algo::Transform(InitBlueprints, Blueprints, [](FRigVMEditorAssetInterfacePtr In)
	{
...
TSharedRef<SGraphEditor> FRigVMLegacyEditor::CreateGraphEditorWidget(TSharedRef<FTabInfo> InTabInfo, UEdGraph* InGraph)
{
	...
	{
		if (URigVMGraph* Model = WeakGraph->GetModel())
		{
			if (CVarScribbleEnabled.GetValueOnAnyThread())
			{
				TSharedRef<SWidget> GraphPanelWidget = GraphEditor->GetGraphPanel()->AsShared();
				if (TSharedPtr<SWidget> ParentWidget = GraphPanelWidget->GetParentWidget())
				{
					if (ParentWidget->GetWidgetClass().GetWidgetType() == SOverlay::StaticWidgetClass().GetWidgetType())
					{
						TSharedPtr<SOverlay> Overlay = StaticCastSharedPtr<SOverlay>(ParentWidget);
						if (Overlay)
						{
							Overlay->AddSlot()
							.Padding(0.f)
							...
							[
								SNew(SScribbleGraph)
								.GraphData(Model->SetupScribbleGraph(true))
								.AllowNavigation(true)
								.ShouldDrawBackground(false)
								.ShouldDrawSurroundingShadow(false)
								.ToolbarPadding(FMargin(8,44,0,0))
								.PanelToSync(StaticCastWeakPtr<SGraphPanel,SWidget>(GraphPanelWidget.ToWeakPtr()))
							];
						}
					}
				}
