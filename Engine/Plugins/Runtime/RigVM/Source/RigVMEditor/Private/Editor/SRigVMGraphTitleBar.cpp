...
void SRigVMGraphTitleBar::Construct( const FArguments& InArgs )
{
	...
	FRigVMEditorAssetInterfacePtr BlueprintObj = FRigVMBlueprintUtils::FindAssetForGraph(this->EdGraphObj);
	if (BlueprintObj)
	{
...
void SRigVMGraphTitleBar::Refresh()
{
	...
	if (EdGraphObj)
	{
		if (FRigVMEditorAssetInterfacePtr BlueprintObj = FRigVMBlueprintUtils::FindAssetForGraph(this->EdGraphObj))
		{
			BlueprintTitle = FText::FromString(BlueprintObj->GetObject()->GetName());
			RebuildBreadcrumbTrail();
		}
