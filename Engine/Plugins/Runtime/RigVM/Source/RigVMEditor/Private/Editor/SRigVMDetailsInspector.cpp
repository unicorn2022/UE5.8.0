...
void SRigVMDetailsInspector::OnEditorClose(const IRigVMEditor* RigVMEditorBase, FRigVMEditorAssetInterfacePtr RigVMBlueprint)
{
	...
	PropertyView->UnregisterInstancedCustomPropertyLayout(URigVMRuntimeAsset::StaticClass());
...
void SRigVMDetailsInspector::Construct(const FArguments& InArgs)
{
	...
	FOnGetDetailCustomizationInstance LayoutClassSettingsDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FRigVMClassSettingsDetailCustomization::MakeInstance, Editor);
	PropertyView->RegisterInstancedCustomPropertyLayout(URigVMRuntimeAsset::StaticClass(), LayoutClassSettingsDetails);
...
bool SRigVMDetailsInspector::IsPropertyVisible( const FPropertyAndParent& PropertyAndParent ) const
{
	...
	if (bIsEditingTemplate)
	{
		...
		TSharedPtr<IRigVMEditor> Editor = WeakEditor.Pin();
		FRigVMEditorAssetInterfacePtr Blueprint = Editor.IsValid() ? Editor->GetRigVMAssetInterface() : nullptr;
