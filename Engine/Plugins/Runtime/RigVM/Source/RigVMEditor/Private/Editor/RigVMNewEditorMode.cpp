	...
	: FApplicationMode(FRigVMEditorModes::RigVMEditorMode, FRigVMEditorModes::GetLocalizedMode)
{
	...
	TabLayout = FTabManager::NewLayout("Standalone_RigVMEditMode_Layout_v1.7")
		->AddArea
		(
			...
			(
				...
				(
					...
					(
						...
						->AddTab(FRigVMGraphEditorTabSummoner::TabID(), ETabState::ClosedTab)
					)
					->Split
					(
...
void FRigVMNewEditorMode::PreDeactivateMode()
{
	FApplicationMode::PreDeactivateMode();

	// Capture which documents are open so the asset's LastEditedDocuments reflects the
	// current session; without this, tab state from the editing session is lost on close.
	// SaveEditedObjectState itself early-returns when IsEditingSingleBlueprint() is false,
	// so we don't gate it here (that method is protected on FRigVMEditorBase anyway).
	if (TSharedPtr<FRigVMNewEditor> BP = Editor.Pin())
	{
