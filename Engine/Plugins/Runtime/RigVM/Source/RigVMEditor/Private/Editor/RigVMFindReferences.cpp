...
void RigVMFindReferencesHelpers::ExpandAllChildren(FRigVMFindResultPtr InTreeNode, TSharedPtr<STreeView<FRigVMFindResultPtr>> InTreeView)
{
...
FRigVMFindResult::FRigVMFindResult(TWeakInterfacePtr<IRigVMEditorAssetInterface> InBlueprint)
	: WeakBlueprint(InBlueprint)
{
}

FRigVMFindResult::FRigVMFindResult(TWeakInterfacePtr<IRigVMEditorAssetInterface> InBlueprint, const FText& InDisplayText )
...
UObject* FRigVMFindResult::GetObject(FRigVMEditorAssetInterfacePtr InBlueprint) const
{
...
FRigVMEditorAssetInterfacePtr FRigVMFindResult::GetRigVMAssetInterface()
{
	if (TStrongObjectPtr<UObject> Blueprint = WeakBlueprint.GetWeakObjectPtr().Pin())
	{
		return FRigVMEditorAssetInterfacePtr(Blueprint.Get());
	}
	return nullptr;
}

const FRigVMEditorAssetInterfacePtr FRigVMFindResult::GetRigVMAssetInterface() const
{
...
FRigVMFindReferencesGraphNode::FRigVMFindReferencesGraphNode(TWeakObjectPtr<URigVMBlueprint> InBlueprint)
	: FRigVMFindResult(FRigVMEditorAssetInterfacePtr(InBlueprint.Get())), Glyph(FAppStyle::GetAppStyleSetName(), "")
	, Class(nullptr)
{
}

FRigVMFindReferencesGraphNode::FRigVMFindReferencesGraphNode(FRigVMEditorAssetInterfacePtr InBlueprint)
...
UObject* FRigVMFindReferencesGraphNode::GetObject(FRigVMEditorAssetInterfacePtr InBlueprint) const
{
...
FRigVMFindReferencesPin::FRigVMFindReferencesPin(TWeakObjectPtr<URigVMBlueprint> InBlueprint, FString InSchemaName)
	: FRigVMFindResult(FRigVMEditorAssetInterfacePtr(InBlueprint.Get()))
	, SchemaName(InSchemaName)
	, IconColor(FSlateColor::UseForeground())
{
}

FRigVMFindReferencesPin::FRigVMFindReferencesPin(FRigVMEditorAssetInterfacePtr InBlueprint, FString InSchemaName)
...
FRigVMFindReferencesVariable::FRigVMFindReferencesVariable(FRigVMEditorAssetInterfacePtr InBlueprint)
	: FRigVMFindResult(InBlueprint)
{
}
...
FRigVMFindReferencesGraph::FRigVMFindReferencesGraph(TWeakObjectPtr<URigVMBlueprint> InBlueprint, EGraphType InGraphType)
	: FRigVMFindResult(FRigVMEditorAssetInterfacePtr(InBlueprint.Get())), GraphType(InGraphType)
{
}
FRigVMFindReferencesGraph::FRigVMFindReferencesGraph(FRigVMEditorAssetInterfacePtr InBlueprint, EGraphType InGraphType)
	...
	[
		...
		[
			...
			[
				...
				[
					SAssignNew(TreeView, SRigVMFindResultTreeViewType)
...
void SRigVMFindReferences::FindReferences(const FString& SearchTerms)
{
	...
	{
		FRigVMFindResultPtr GraphResult;
		if (FRigVMFindResultPtr* Found = ElementHashToResult.Find(GetTypeHash(EdGraph)))
		{
			GraphResult = *Found;
		}
		else
		{
			GraphResult = ItemsFound.Add_GetRef(FRigVMFindResultPtr(new FRigVMFindReferencesGraph(Blueprint, Schema->GetGraphType(EdGraph))));
	...
	{
		FRigVMFindResultPtr NodeResult;
		if (FRigVMFindResultPtr* Found = ElementHashToResult.Find(GetTypeHash(EdNode)))
		{
			NodeResult = *Found;
		}
		else
		{
			FRigVMFindResultPtr GraphResult = FindOrAddGraphResult(EdNode->GetGraph());
			NodeResult = FRigVMFindResultPtr(new FRigVMFindReferencesGraphNode(Blueprint));
	...
	{
		FRigVMFindResultPtr PinResult;
		if (FRigVMFindResultPtr* Found = ElementHashToResult.Find(GetTypeHash(EdPin)))
		{
			PinResult = *Found;
		}
		else
		{
			...
			FRigVMFindResultPtr NodeResult = FindOrAddNodeResult(EdPin->GetOwningNode(), NodeMetaData);
			PinResult = FRigVMFindResultPtr(new FRigVMFindReferencesPin(Blueprint, SchemaClassName));
	...
	{
		FRigVMFindResultPtr VariableResult;

		const uint32 Hash = HashCombine(GetTypeHash(VariableName), GetTypeHash(Graph));
		if (FRigVMFindResultPtr* Found = ElementHashToResult.Find(Hash))
		{
		...
		else
		{
			...
			VariableResult = FRigVMFindResultPtr(new FRigVMFindReferencesVariable(Blueprint));
	...
	for (const URigVMGraph* Graph : Graphs)
	{
		if (URigVMEdGraph* EdGraph = Cast<URigVMEdGraph>(Blueprint->GetEdGraph(Graph)))
		{
			for (TObjectPtr<UEdGraphNode> EdNode : EdGraph->Nodes)
			{
				...
				if (MatchedPair)
				{
					FRigVMFindResultPtr Result = FindOrAddNodeResult(EdNode, NodeMetadata);

					const FText DisplayText = FText::Format(LOCTEXT("RigVMFindReferencesValues", "{0}: {1}"), MatchedPair->Key, MatchedPair->Value);
					FRigVMFindResultPtr DetailResult = FRigVMFindResultPtr(new FRigVMFindResult(Blueprint, DisplayText));
				...
				for (UEdGraphPin* Pin : EdNode->GetAllPins())
				{
					...
					if (MatchedPinPair)
					{
						FRigVMFindResultPtr Result = FindOrAddPinResult(Pin, PinMetaData, NodeMetadata);
			...
			for (const FRigVMGraphVariableDescription& Variable : LocalVariables)
			{
				...
				{
					FRigVMFindResultPtr Result = FindOrAddVariableResult(Variable.Name, EdGraph, LocalVariableMetadata);
	...
	{
		if (FRigVMEditorAssetInterfacePtr BP = Blueprint)
		{
			...
			if (MatchedVariablePair)
			{
				FRigVMFindResultPtr Result = FindOrAddVariableResult(Variable.Name, nullptr, BPVariableMetadata);
	...
	for (FRigVMFindResultPtr Item : ItemsFound)
	{
...
TSharedRef<ITableRow> SRigVMFindReferences::OnGenerateRow( FRigVMFindResultPtr InItem, const TSharedRef<STableViewBase>& OwnerTable )
{
...
void SRigVMFindReferences::OnGetChildren( FRigVMFindResultPtr InItem, TArray< FRigVMFindResultPtr >& OutChildren )
{
	OutChildren += InItem->Children;
}

void SRigVMFindReferences::OnTreeSelectionDoubleClicked( FRigVMFindResultPtr Item )
{
	if(Item.IsValid())
	{
		Item->OnClick();
...
void SRigVMFindReferences::SelectAllItemsHelper(FRigVMFindResultPtr InItemToSelect)
{
