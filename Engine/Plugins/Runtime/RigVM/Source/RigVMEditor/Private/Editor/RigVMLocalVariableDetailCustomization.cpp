...
void FRigVMLocalVariableDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (ObjectsBeingCustomized.Num() > 0 && ObjectsBeingCustomized[0].IsValid())
	{
		...
		{
			BlueprintBeingCustomized = IRigVMEditorAssetInterface::GetInterfaceOuter(GraphBeingCustomized);
		}
	...
	if (GraphBeingCustomized)
	{
		...
		const FName SanitizedName = FInstancedPropertyBag::SanitizePropertyName(SourcePath);
		FProperty* Property = Memory->FindPropertyByName(SanitizedName);
		if (Property)
		{
			...
			if (IDetailPropertyRow* Row = DefaultValueCategory.AddExternalStructureProperty(MakeShared<FInstancePropertyBagStructureDataProvider>(*Memory), SanitizedName))
			{
				...
				{
					const int32 PropertyIndex = MemoryStorage->GetPropertyIndex(Property);
					if (MemoryStorage->IsValidIndex(PropertyIndex))
					{
						VariableDescription.DefaultValue = MemoryStorage->GetDataAsStringSafe(PropertyIndex);
...
void FRigVMLocalVariableDetailCustomization::HandlePinInfoChanged(const FEdGraphPinType& PinType)
{
	...
	RigVMTypeUtils::CPPTypeFromPinType(PinType, CPPType, &CPPTypeObject);
	VariableDescription.ChangeType(CPPType, CPPTypeObject);
