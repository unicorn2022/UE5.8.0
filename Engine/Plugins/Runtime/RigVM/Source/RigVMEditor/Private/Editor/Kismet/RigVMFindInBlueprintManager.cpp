...
uint32 FRigVMStreamSearch::Run()
{
	...
	UE_LOGF(LogRigVMFindInBlueprint, Log, "Search completed in %0.2f seconds.", FPlatformTime::Seconds() - StartTime);
...
bool FRigVMFindInBlueprintSearchManager::ProcessEncodedValueForUnloadedBlueprint(FRigVMSearchData& SearchData)
{
	...
	if (!SearchData.AssetKeyForValue.IsNone() && SearchData.Value.IsEmpty())
	{
		...
		if (TempEncodedString.Len() == 0)
		{
			...
			UE_LOGF(LogRigVMFindInBlueprint, Warning, "%ls AssetData loaded with %ls has an invalid FiB tag!", *AssetData.GetSoftObjectPath().ToString(), *AssetPath);
		...
		while (SearchQuery->DeferredAssetPaths.Dequeue(AssetPath))
		{
			...
			if (SearchData.IsValid() && !SearchData.IsMarkedForDeletion())
			{
				...
				if (!SearchData.IsIndexingCompleted())
				{
					...
					{
						UE_LOGF(LogRigVMFindInBlueprint, Warning, "%ls has been unexpectedly excluded from indexing and will not be included in the search.", *AssetPath.ToString());
					}
...
void FRigVMFindInBlueprintSearchManager::ExportOutdatedAssetList()
{
	...
	if (FArchive* Ar = IFileManager::Get().CreateFileWriter(*FullPath))
	{
		...
		UE_LOGF(LogRigVMFindInBlueprint, Log, "%ls", *ExportConfirmationText.ToString());
	...
	else
	{
		...
		UE_LOGF(LogRigVMFindInBlueprint, Log, "%ls", *ExportFailureText.ToString());
	...
	virtual bool Exec_Editor(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
	{
		if (FParse::Command(&Cmd, TEXT("DUMPFIBINDEXCACHE")))
		{
			...
			if (DumpFile)
			{
				...
				UE_LOGF(LogRigVMFindInBlueprint, Log, "Wrote full index cache to %ls", *FullPath);
			}
			return true;
		}
		return false;
