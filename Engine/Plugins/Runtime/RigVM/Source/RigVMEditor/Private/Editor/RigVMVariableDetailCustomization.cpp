...
TSharedRef<IDetailCustomization> FRigVMVariableDetailCustomization::MakeInstance(TSharedPtr<IRigVMEditor> InEditor)
{
	...
	if (Objects && Objects->Num() == 1)
	{
		...
		else if ((*Objects)[0]->Implements<URigVMRuntimeAssetInterface>())
		{
			...
			return MakeShareable(new FRigVMVariableDetailCustomization(InEditor, RuntimeAsset->GetEditorOnlyData()));
		}
	}

	return MakeShareable(new FRigVMVariableDetailCustomization((TSharedPtr<IRigVMEditor>)nullptr, FRigVMEditorAssetInterfacePtr(nullptr)));
}

FRigVMVariableDetailCustomization::FRigVMVariableDetailCustomization(TSharedPtr<IRigVMEditor> InEditor, FRigVMEditorAssetInterfacePtr Blueprint)
		: EditorPtr(InEditor)
...
void FRigVMVariableDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	if (Editor->GetRigVMInspector())
	{
		...
		TSharedPtr<INameValidatorInterface> NameValidator = nullptr;
		{
			...
			{
				FRigVMEditorAssetInterfacePtr AssetPtr = BlueprintPtr.GetObject();
		...
		TSharedPtr<SToolTip> PublicTooltip = IDocumentation::Get()->CreateToolTip(LOCTEXT("VariablePublic_Tooltip", "Should this variable be public?"), NULL, "", TEXT("Public"));

		Category.AddCustomRow(LOCTEXT("VariablePublic", "Public"))
		.NameContent()
		[
			SNew(STextBlock)
			.ToolTip(PublicTooltip)
			.Text( LOCTEXT("VariablePublic", "Public") )
			.Font( DetailFontInfo )
		]
		.ValueContent()
		[
			SNew(SCheckBox)
			.IsChecked( this, &FRigVMVariableDetailCustomization::OnGetPublicCheckboxState )
			.OnCheckStateChanged( this, &FRigVMVariableDetailCustomization::OnPublicChanged )
			.ToolTip(PublicTooltip)
		];
		...
		{
			FRigVMEditorAssetInterfacePtr Asset = StrongObj.Get();
			
			// Add in default value editing for properties that can be edited, local properties cannot be edited
			if (Asset->IsBlueprintAsset() && Asset->GetRuntimeAssetInterface() != nullptr)
			{
				URigVMBlueprint* Blueprint = Cast<URigVMBlueprint>(Asset.GetObject());
			...
			{
				VariableProperty = Asset->FindGeneratedPropertyByName(CachedVariableName);
			}
			...
			if (VariableProperty == nullptr)
			{
				if (Asset->GetAssetStatus() != RVMA_UpToDate)
				{
			...
			else 
			{
				...
				if (Asset->IsBlueprintAsset())
				{
					// Things are in order, show the property and allow it to be edited
					TArray<UObject*> ObjectList;
					if (TScriptInterface<IRigVMRuntimeAssetInterface> RuntimeInterface = Asset->GetRuntimeAssetInterface())
					{
						if (UClass* GeneratedClass = Cast<UClass>(RuntimeInterface.GetObject()))
						{
							ObjectList.Add(GeneratedClass->GetDefaultObject());
						}
					}
				...
				else
				{
					...
					if (FRigVMPropertyBag* InstancedPropertyBag = Asset->GetVariablesPropertyBag())
					{
						...
						if (StructView.IsValid())
						{
							...
							// Don't rely on the CachedVariableName because it is not sanitized (it allows spaces)
							DefaultValueCategory.AddExternalStructureProperty(StructOnScope, VariableProperty->GetFName(), EPropertyLocation::Default, Params);
						}
					}
				}
...
void FRigVMVariableDetailCustomization::PopulateCategories()
{
	...
	FRigVMEditorAssetInterfacePtr Interface = Blueprint.Get();
...
void FRigVMVariableDetailCustomization::OnFinishedChangingVariable(const FPropertyChangedEvent& PropertyChangedEvent)
{
	TSharedPtr<IRigVMEditor> Editor = EditorPtr.Pin();
	if (!Editor)
	{
	...
	if (FRigVMEditorAssetInterfacePtr Asset = Blueprint.Get())
	{
		Asset->MarkAssetAsModified();
		Asset->SetAssetStatus(ERigVMAssetStatus::RVMA_Dirty);

		// If the changed property is the root property, autocompile
		// Otherwise wait until the user forces a compile, we do this in order to allow multi-parameter editing of structs like FVector (using tab)
...
FEdGraphPinType FRigVMVariableDetailCustomization::OnGetVarType() const
{
	...
	if (SelectionVarType == ERigVMExplorerElementType::Variable)
	{
		for (FRigVMGraphVariableDescription& Var : Interface->GetAssetVariables())
		{
			...
			{
				return RigVMTypeUtils::PinTypeFromRigVMVariableDescription(Var);
			}
		}
		return Type;
	}
...
{
	const FProperty* Property = CachedVariableProperty.Get();
	if (Property)
	{
...
ECheckBoxState FRigVMVariableDetailCustomization::OnGetPublicCheckboxState() const
{
	const FProperty* Property = CachedVariableProperty.Get();
	if (Property)
	{
		return (Property && (Property->PropertyFlags & CPF_DisableEditOnInstance) == 0) ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
	}
	return ECheckBoxState::Unchecked;
}

void FRigVMVariableDetailCustomization::OnPublicChanged(ECheckBoxState InNewState)
{
	TStrongObjectPtr<UObject> Blueprint = BlueprintPtr.GetWeakObjectPtr().Pin();
	if (!Blueprint)
	{
	...
	if (CachedVariableName != NAME_None)
	{
		const bool bPublic = (InNewState == ECheckBoxState::Checked);
		Interface->SetVariablePublic(CachedVariableName, bPublic);
	}
}
