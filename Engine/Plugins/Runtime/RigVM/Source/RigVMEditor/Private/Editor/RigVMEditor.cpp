...
{
	FRigVMEditorAssetInterfacePtr RigVMBlueprint = GetRigVMAssetInterface();
	RigVMEditorClosedDelegate.Broadcast(this, RigVMBlueprint);

	GraphExplorerWidget = nullptr;
	ClearDetailObject();
	...
	if (RigVMBlueprint)
	{
		...
		IRigVMEditorAssetInterface::sCurrentlyOpenedRigVMBlueprints.Remove(RigVMBlueprint);
	...
	if (bRequestedReopen)
	{
		...
		FSoftObjectPath AssetToReopen = RigVMBlueprint->GetAssetObjectForEditor();
...
void FRigVMEditorBase::InitRigVMEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, FRigVMEditorAssetInterfacePtr InRigVMBlueprint)
{
...
#if WITH_RIGVMLEGACYEDITOR
	if (InRigVMBlueprint->IsBlueprintAsset())
	{
	...
	if (UModeManagerInteractiveToolsContext* ToolsContext = GetToolkitEditorModeManager().GetInteractiveToolsContext())
	{
		ToolsContext->SetViewportInteractionsEnabled(true);
	}

	TArray<FRigVMEditorAssetInterfacePtr> Blueprints;
	...
	IRigVMEditorAssetInterface::sCurrentlyOpenedRigVMBlueprints.AddUnique(InRigVMBlueprint);
	...
	if (Blueprints.Num() > 0)
	{
		if(Blueprints[0]->GetAssetStatus() == RVMA_Error || 
			Blueprints[0]->GetAssetStatus() == RVMA_Dirty || 
			Blueprints[0]->GetAssetStatus() == RVMA_Unknown)
		{
			Compile();
		}
	}
	...
	ExecuteOnGameThread(UE_SOURCE_LOCATION, [ThisWeakPtr, InRigVMBlueprint]()
	{
		if (TSharedPtr<IRigVMEditor> Editor = ThisWeakPtr.Pin())
		{
		   ...
		   if (!IRigVMEditorAssetInterface::sCurrentlyOpenedRigVMBlueprints.Contains(InRigVMBlueprint))
		   {
			   return;		
		   }
	...
	InRigVMBlueprint->ClearPinWatches();
...
FRigVMEditorAssetInterfacePtr FRigVMEditorBase::GetRigVMAssetInterface() const
{
	const TArray<UObject*>& EditingObjs = GetEditingBlueprints();
	for (UObject* Obj : EditingObjs)
	{
		if (Obj->Implements<URigVMEditorAssetInterface>()) 
		{
			return Obj;
		}
	}
...
{
	if(const FRigVMEditorAssetInterfacePtr RigVMBlueprint = GetRigVMAssetInterface())
	{
...
{
	// tick the  rigvm host
	if (const FRigVMEditorAssetInterfacePtr Blueprint = GetRigVMAssetInterface())
	{
...
{
	if (FRigVMEditorAssetInterfacePtr RigVMBlueprint = GetRigVMAssetInterface())
	{
...
void FRigVMEditorBase::OnAddNewLocalVariable()
{
	...
	RigVMTypeUtils::CPPTypeFromPinType(GetLastPinTypeUsed(), CPPType, &CPPTypeObject);
	LastTypeVar.ChangeType(CPPType, CPPTypeObject);
...
void FRigVMEditorBase::OnPasteNewLocalVariable(const FBPVariableDescription& VariableDescription)
{
	...
	FString CPPType;
	UObject* CPPTypeObject = nullptr;
	RigVMTypeUtils::CPPTypeFromPinType(VariableDescription.VarType, CPPType, &CPPTypeObject);
	TypeVar.ChangeType(CPPType, CPPTypeObject);
	...
	if(!InChord.HasAnyModifierKeys())
	{
		if(URigVMEdGraph* RigVMEdGraph = Cast<URigVMEdGraph>(InGraph))
		{
			if(URigVMController* Controller = RigVMEdGraph->GetController())
			{
				...
				else if (InChord.Key == EKeys::F)
				{
					URigVMTemplateNode* NewNode = Controller->AddTemplateNode(FRigVMDispatch_ArrayIterator().GetTemplateNotation(), FDeprecateSlateVector2D(InPosition), FString(), true, true);
				}
			}
		}
	}
...
void FRigVMEditorBase::PostTransaction(bool bSuccess, const FTransaction* Transaction, bool bIsRedo)
{
	...
	const FRigVMEditorAssetInterfacePtr Blueprint = GetRigVMAssetInterface();
...
{
	if (FRigVMEditorAssetInterfacePtr EditorAsset = GetRigVMAssetInterface())
	{
		if (EditorAsset->IsBlueprintAsset())
		{
			return EditorAsset.GetObject();
		}
		
		return EditorAsset->GetRuntimeAssetInterface().GetObject();
	}
	return nullptr;
}
...
bool FRigVMEditorBase::SelectVariable(const FName& VariableName)
{
	if (FRigVMEditorAssetInterfacePtr Asset = GetRigVMAssetInterface())
	{
		const FProperty* Prop = Asset->FindGeneratedPropertyByName(VariableName);
		UPropertyWrapper* PropWrap = (Prop ? const_cast<FProperty*>(Prop)->GetUPropertyWrapper() : nullptr);
		if (TSharedPtr<SRigVMDetailsInspector> RigVMInspector = GetRigVMInspector())
		{
			SRigVMDetailsInspector::FShowDetailsOptions Options(FText::FromName(VariableName));
...
{
	{
		...
		// Disable single frame evaluation mode
		SetRunGraphOnce(false);
		...
		if (bAnyVariableValueChanged)
		{
			...
			for (const FRigVMCustomDebugObject& DebugObject : DebugList)
			{
				...
				{
					if (TScriptInterface<IRigVMRuntimeAssetInterface> RuntimeAsset = DebuggedHost->GetGeneratedByAsset())
					{
						RuntimeAsset->InitializeInstance(DebuggedHost);
					}
					else
					{
...
bool FRigVMEditorBase::IsEditable(UEdGraph* InGraph) const
{
	...
	if(FRigVMEditorAssetInterfacePtr RigVMBlueprint = GetRigVMAssetInterface())
	{
		...
		const bool bIsReadOnly = RigVMBlueprint->IsEditingLocked();
		const bool bIsEditable = !bIsReadOnly;
		InGraph->bEditable = bIsEditable;
		return bIsEditable;
	}
...
void FRigVMEditorBase::OnBlueprintChangedImpl(IRigVMEditorAssetInterface* InBlueprint, bool bIsJustBeingCompiled)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
void FRigVMEditorBase::BindCommands()
{
	...
	ToolkitCommands->MapAction(
		EditorCommands.LockGraph,
		FExecuteAction::CreateSP(ThisRef, &FRigVMEditorBase::ToggleLockGraph), 
		FIsActionChecked::CreateSP(ThisRef, &FRigVMEditorBase::CanLockGraph),
		FIsActionChecked::CreateSP(ThisRef, &FRigVMEditorBase::IsLockGraphOn));
	...
	ToolkitCommands->MapAction(
		EditorCommands.RunGraphOnce,
		FExecuteAction::CreateSP(ThisRef, &FRigVMEditorBase::ToggleRunGraphOnce), 
		FIsActionChecked::CreateSP(ThisRef, &FRigVMEditorBase::CanRunGraphOnce),
		FIsActionChecked::CreateSP(ThisRef, &FRigVMEditorBase::IsRunGraphOnceOn));
	...
	ToolkitCommands->MapAction(
		...
		{
			if (const FRigVMEditorAssetInterfacePtr RigVMAsset = ThisRef->GetRigVMAssetInterface())
			{
...
void FRigVMEditorBase::UnbindCommands()
{
	...
	ToolkitCommands->UnmapAction(EditorCommands.LockGraph);
...
void FRigVMEditorBase::ToggleLockGraph()
{
	if (FRigVMEditorAssetInterfacePtr AssetInterface = GetRigVMAssetInterface())
	{
		AssetInterface->SetEditingLocked(!AssetInterface->IsEditingLocked());
		AssetInterface->RequestAutoVMRecompilation();
	}
}

bool FRigVMEditorBase::IsLockGraphOn() const
{
	if (FRigVMEditorAssetInterfacePtr AssetInterface = GetRigVMAssetInterface())
	{
		return AssetInterface->IsEditingLocked();
	}
	return false;
}

bool FRigVMEditorBase::CanLockGraph() const
{
	if (FRigVMEditorAssetInterfacePtr AssetInterface = GetRigVMAssetInterface())
	{
		return AssetInterface->CanSetEditingLocked();
	}
	return false;
}
...
void FRigVMEditorBase::ToggleRunGraphOnce()
{
	SetRunGraphOnce(!bRunGraphOnce);
}

void FRigVMEditorBase::SetRunGraphOnce(bool InRunGraphOnceEnabled)
{
	if (bRunGraphOnce == InRunGraphOnceEnabled)
	{
		return;
	}
	bRunGraphOnce = InRunGraphOnceEnabled;
	OnRunGraphOnceToggled(bRunGraphOnce);
}

bool FRigVMEditorBase::IsRunGraphOnceOn() const
{
	if (CanRunGraphOnce())
	{
		return bRunGraphOnce;
	}
	return false;
}
...
FGraphAppearanceInfo FRigVMEditorBase::GetGraphAppearance(UEdGraph* InGraph) const
{
	...
	if (const FRigVMEditorAssetInterfacePtr RigVMBlueprint = GetRigVMAssetInterface())
	{
		...
		if (RigVMBlueprint->IsEditingLocked())
		{
			AppearanceInfo.InstructionFade = 1;
			AppearanceInfo.InstructionText = FText::FromString(TEXT("This graph is locked."));
		}

		if(URigVMHost* RigVMHost = GetRigVMHost())
		{
#if RIGVM_TRACE_ENABLED
			if (RigVMHost->IsPlayingRewindDebugTrace())
			{
				AppearanceInfo.InstructionFade = 1;
				AppearanceInfo.InstructionText = FText::FromString(TEXT("Rewind Debug in Progress."));
			} else
#endif
			if(RigVMHost->GetVM() && RigVMHost->GetVM()->IsNativized())
			{
				...
				AppearanceInfo.InstructionText = FText::FromString(
					FString::Printf(TEXT("Running a nativized VM (U%s)."), *RigVMHost->GetVM()->GetClass()->GetName())
				);
			}
...
{
	if (InCompiledObject->Implements<URigVMEditorAssetInterface>())
	{
		FRigVMEditorAssetInterfacePtr RigVMBlueprint = InCompiledObject;
		if (TSharedPtr<class IMessageLogListing> CompilerListing = GetCompilerResultsListing())
		{
			CompilerListing->ClearMessages();
			CompilerListing->AddMessages(RigVMBlueprint->GetCompileLog().Messages);
		}
		RigVMBlueprint->GetCompileLog().Messages.Reset();
...
FRigVMController_BulkEditResult FRigVMEditorBase::OnRequestBulkEditDialog(FRigVMEditorAssetInterfacePtr InBlueprint,
...
{
	FRigVMEditorAssetInterfacePtr RigBlueprint = GetRigVMAssetInterface();
	...
	{
		if(IRigVMEditorAssetInterface* OtherAsset = NodeToJumpTo->GetImplementingOuter<IRigVMEditorAssetInterface>())
		{
			if(OtherAsset != RigBlueprint.GetInterface())
			{
				UObject* OtherAssetObject = OtherAsset->GetAssetObjectForEditor();
				if (GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(OtherAssetObject))
				{
...
void FRigVMEditorBase::UpdateRigVMHost()
{
...

	{
		...
		if (URigVMHost* CurrentHost = GetRigVMHost())
		{
			...
			if (!Blueprint->GetRuntimeAssetInterface()->IsObjectInstanceOfAsset(CurrentHost))
			{
				SetHost(nullptr);
			}
		}
		...
		RigVMHost->ResetEvaluationsLeft();
		...
		{
			TGuardValue<bool> UICompilationGuard(bIsCompilingThroughUI, true);
				...
				if (FRigVMEditorAssetInterfacePtr Blueprint = GetRigVMAssetInterface())
				{
					if (URigVMController* Controller = Blueprint->GetController(RigVMEdGraph))
					{
						...
						else
						{
							...
							FVector2D Location = GraphEditor->GetPasteLocation2f();
							if (GraphEditor->GetBoundsForSelectedNodes(Bounds, 50.f))
							{
								Location = Bounds.GetTopLeft();
								Size = Bounds.GetSize();
							}
							Controller->AddCommentNode(TEXT("Comment"), Location, Size, GetDefault<UGraphEditorSettings>()->DefaultCommentNodeTitleColor, FString(), true, true);
						}
...
void FRigVMEditorBase::SetDetailViewForVariable()
{
	FName VariableName;
	if (TSharedPtr<SRigVMDetailsInspector> RigVMInspector = GetRigVMInspector())
	{
		TArray< TWeakObjectPtr<UObject> > SelectedObjects = RigVMInspector->GetSelectedObjects();
		for (TWeakObjectPtr<UObject> SelectedObject : SelectedObjects)
		{
			if (SelectedObject.IsValid())
			{
				if(UPropertyWrapper* WrapperObject = Cast<UPropertyWrapper>(SelectedObject.Get()))
				{
					VariableName = WrapperObject->GetProperty()->GetFName();
					break;
				}
			}
		}
...
void FRigVMEditorBase::RefreshDetailView()
{
	...
	if(DetailViewShowsLocalVariable())
	{
		SetDetailViewForLocalVariable();
	}
	else if(DetailViewShowsVariable())
	{
		SetDetailViewForVariable();
	}
...
bool FRigVMEditorBase::DetailViewShowsVariable() const
{
	if (TSharedPtr<SRigVMDetailsInspector> RigVMInspector = GetRigVMInspector())
	{
		TArray< TWeakObjectPtr<UObject> > SelectedObjects = RigVMInspector->GetSelectedObjects();
...
void FRigVMEditorBase::ClearDetailsViewWrapperObjects()
{
	for(const TStrongObjectPtr<URigVMDetailsViewWrapperObject>& WrapperObjectPtr : WrapperObjects)
	{
		if(WrapperObjectPtr.IsValid())
		{
			...
			URigVMDetailsViewWrapperObject::ReturnToPool(WrapperObjectPtr.Get());
		}
	}
...
void FRigVMEditorBase::FillToolbar(FToolBarBuilder& ToolbarBuilder, bool bEndSection)
{
	ToolbarBuilder.BeginSection("Toolbar");
	{
		...
		AddLockEditingWidget(ToolbarBuilder);
		...
		AddRunOnceWidget(ToolbarBuilder);
...
void FRigVMEditorBase::AddLockEditingWidget(FToolBarBuilder& ToolbarBuilder)
{
	ToolbarBuilder.AddToolBarButton(
		FRigVMEditorCommands::Get().LockGraph,
		NAME_None,
		TAttribute<FText>::Create([this]() -> FText
		{
			if (FRigVMEditorAssetInterfacePtr EditorAssetInterface = GetRigVMAssetInterface())
			{
				return EditorAssetInterface->GetEditingLockedLabel();
			}
			return FText();
		}),
		TAttribute<FText>::Create([this]() -> FText
		{
			if (FRigVMEditorAssetInterfacePtr EditorAssetInterface = GetRigVMAssetInterface())
			{
				return EditorAssetInterface->GetEditingLockedTooltip();
			}
			return FText();
		}),
		...
		TAttribute<EVisibility>::CreateLambda([this]() -> EVisibility
		{
			if (FRigVMEditorAssetInterfacePtr EditorAssetInterface = GetRigVMAssetInterface())
			{
				if (EditorAssetInterface->IsEditingLockedVisible())
				{
...
void FRigVMEditorBase::HandleRefreshEditorFromBlueprint(FRigVMEditorAssetInterfacePtr InBlueprint)
{
	Compile();
}
...
void FRigVMEditorBase::HandleVariableDroppedFromBlueprint(UObject* InSubject, FProperty* InVariableToDrop, const FVector2D& InDropPosition, const FVector2D& InScreenPosition)
{
	...
	FRigVMExternalVariable ExternalVariable = Controller->GetVariableByProperty(InVariableToDrop);
	if (!ExternalVariable.IsValid(true /* allow null ptr */))
	{
		// In some cases the variable property does not match because it comes from the SkeletalClass, not the GeneratedBlueprintClass (in the case of legacy)
		// Let's fall back to looking by name
		ExternalVariable = Controller->GetVariableByName(InVariableToDrop->GetFName());
	}
	...
	const FText SectionText = FText::FromString(FString::Printf(TEXT("Variable %s"), *ExternalVariable.GetName().ToString()));

	MenuBuilder.BeginSection("VariableDropped", SectionText);

	MenuBuilder.AddMenuEntry(
		FText::FromString(FString::Printf(TEXT("Get %s"), *ExternalVariable.GetName().ToString())),
		FText::FromString(FString::Printf(TEXT("Adds a getter node for variable %s"), *ExternalVariable.GetName().ToString())),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda([ExternalVariable, Controller, InDropPosition] {

				Controller->AddVariableNode(ExternalVariable.GetName(), ExternalVariable.GetExtendedCPPType().ToString(), const_cast<UObject*>(ExternalVariable.GetCPPTypeObject()), true, FString(), InDropPosition, FString(), true, true);
	...
	MenuBuilder.AddMenuEntry(
		FText::FromString(FString::Printf(TEXT("Set %s"), *ExternalVariable.GetName().ToString())),
		FText::FromString(FString::Printf(TEXT("Adds a setter node for variable %s"), *ExternalVariable.GetName().ToString())),
		FSlateIcon(),
		FUIAction(
			FExecuteAction::CreateLambda([ExternalVariable, Controller, InDropPosition] {

				Controller->AddVariableNode(ExternalVariable.GetName(), ExternalVariable.GetExtendedCPPType().ToString(), const_cast<UObject*>(ExternalVariable.GetCPPTypeObject()), false, FString(), InDropPosition, FString(), true, true);
...
void FRigVMEditorBase::OnNodeDoubleClicked(FRigVMEditorAssetInterfacePtr InBlueprint, URigVMNode* InNode)
{
	...
	if (URigVMLibraryNode* LibraryNode = Cast<URigVMLibraryNode>(InNode))
	{
		...
		if(URigVMGraph* ContainedGraph = LibraryNode->GetContainedGraph())
		{
			...
			{
				UE_LOGF(LogRigVMEditor, Warning, "Could not open graph (%ls)", *LibraryNode->GetFunctionIdentifier().GetLibraryNodePath());
			}
		}
	}
}
...
{
	TScriptInterface<IRigVMEditorAssetInterface> Asset = GetRigVMAssetInterface();
...
{
	const FRigVMEditorAssetInterfacePtr RigVMAsset = GetRigVMAssetInterface();
	...
	if (InDragDropOp->IsOfType<FRigVMGraphExplorerDragDropOp>())
	{
		...
		if (GetFocusedGraphEditor().IsValid())
		{
			...
			else if (ExplorerOp->GetElement().Type == ERigVMExplorerElementType::Variable)
			{
				if (FRigVMEditorAssetInterfacePtr TargetRigBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(InGraph))
				{
					const FProperty* Property = TargetRigBlueprint->FindGeneratedPropertyByName(*ExplorerOp->GetElement().Name);
					Schema->RequestVariableDropOnPanel(InGraph, const_cast<FProperty*>(Property), InNodePosition, InScreenPosition);
				}
			}
...
void FRigVMEditorBase::AddRunOnceWidget(FToolBarBuilder& ToolbarBuilder)
{
	TWeakPtr<IRigVMEditor> WeakThis = SharedRef().ToWeakPtr();

	ToolbarBuilder.AddToolBarButton(
		FRigVMEditorCommands::Get().RunGraphOnce,
		NAME_None,
		FText(),
		TAttribute<FText>(),
		TAttribute<FSlateIcon>::CreateLambda(
			[WeakThis]() -> FSlateIcon
			{
				static const FSlateIcon RunningContinuously(FAppStyle::GetAppStyleSetName(), "PlayWorld.PlaySimulate");
				static const FSlateIcon RunningOnce(FAppStyle::GetAppStyleSetName(), "PlayWorld.SingleFrameAdvance.Small");
				if (WeakThis.IsValid())
				{
					if (TSharedPtr<IRigVMEditor> StrongThis = WeakThis.Pin())
					{
						const FRigVMEditorBase* Editor = static_cast<const FRigVMEditorBase*>(StrongThis.Get());
						if (Editor->IsRunGraphOnceOn())
						{
							return RunningOnce;
						}
					}
...
void FRigVMEditorBase::SetGraphExplorerWidget(TSharedPtr<SRigVMEditorGraphExplorer> InWidget)
{
	GraphExplorerWidget = InWidget;
	GetRigVMAssetInterface()->OnModified().AddSP(GraphExplorerWidget.ToSharedRef(), &SRigVMEditorGraphExplorer::HandleGraphModifiedEvent);
}
...
void FRigVMEditorBase::OnRequestFindNodeReferences(const UE::RigVMEditor::FRigVMEditorFindNodeReferencesParams Params)
{
	...
	TScriptInterface<IRigVMRuntimeAssetInterface> GeneratedClass = Params.WeakRigVMAsset.Get()->GetRuntimeAssetInterface();
