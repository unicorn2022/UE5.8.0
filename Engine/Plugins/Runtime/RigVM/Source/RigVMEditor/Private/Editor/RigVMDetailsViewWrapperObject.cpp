...
TMap<UClass*, TArray<URigVMDetailsViewWrapperObject*>> URigVMDetailsViewWrapperObject::InstancePool;
...
URigVMDetailsViewWrapperObject* URigVMDetailsViewWrapperObject::MakeInstance(UClass* InWrapperObjectClass, UObject* InOuter, UScriptStruct* InStruct, uint8* InStructMemory, UObject* InSubject)
{
	...
	URigVMDetailsViewWrapperObject* Instance = GetOrCreateNewInstance(WrapperClass, InOuter);
...
UClass* URigVMDetailsViewWrapperObject::GetClassForNodes(TArray<URigVMNode*> InNodes, bool bCreateIfNeeded) const
{
	...
	for(URigVMPin* Pin : PinsToInspect)
	{
		...
		if(RigVMTypeUtils::IsArrayType(CPPType))
		{
			...
			FArrayProperty* ArrayProperty = new FArrayProperty(PropertyOwner, Pin->GetFName());
			...
			if (RigVMTypeUtils::IsArrayType(BaseCPPType))
			{
				...
				// create an array property as a container for the tail
				FArrayProperty* InnerArrayProperty = new FArrayProperty(PropertyOwner, Pin->GetFName());
		...
		{
			(*ElementProperty) = new FBoolProperty(PropertyOwner, Pin->GetFName());
		}
		...
		{
			(*ElementProperty) = new FIntProperty(PropertyOwner, Pin->GetFName());
		}
		else if(BaseCPPType.Equals(RigVMTypeUtils::FloatType, ESearchCase::IgnoreCase))
		{
			(*ElementProperty) = new FFloatProperty(PropertyOwner, Pin->GetFName());
		}
		else if(BaseCPPType.Equals(RigVMTypeUtils::DoubleType, ESearchCase::IgnoreCase))
		{
			(*ElementProperty) = new FDoubleProperty(PropertyOwner, Pin->GetFName());
		}
		else if(BaseCPPType.Equals(RigVMTypeUtils::FStringType, ESearchCase::IgnoreCase))
		{
			(*ElementProperty) = new FStrProperty(PropertyOwner, Pin->GetFName());
		}
		else if(BaseCPPType.Equals(RigVMTypeUtils::FNameType, ESearchCase::IgnoreCase))
		{
			(*ElementProperty) = new FNameProperty(PropertyOwner, Pin->GetFName());
		}
		else if(Pin->GetCPPTypeObject())
		{
			if(UEnum* Enum = Cast<UEnum>(Pin->GetCPPTypeObject()))
			{
				FByteProperty* ByteProperty = new FByteProperty(PropertyOwner, Pin->GetFName());
			...
			else if(UClass* Class = Cast<UClass>(Pin->GetCPPTypeObject()))
			{
				...
				if(bIsClass)
				{
					FClassProperty* ClassProperty = new FClassProperty(PropertyOwner, Pin->GetFName());
					ClassProperty->MetaClass = Class; 
					(*ElementProperty) = ClassProperty;
				}
				else
				{
					FObjectProperty* ObjectProperty = new FObjectProperty(PropertyOwner, Pin->GetFName());
			...
			{
				FStructProperty* StructProperty = new FStructProperty(PropertyOwner, Pin->GetFName());
...
bool URigVMDetailsViewWrapperObject::IsActive() const
{
	return SubjectPtr.IsValid() && IsValidClass(GetClass());
}

URigVMDetailsViewWrapperObject* URigVMDetailsViewWrapperObject::GetOrCreateNewInstance(const UClass* InWrapperObjectClass, UObject* InOuter)
{
	URigVMDetailsViewWrapperObject* Instance = nullptr;

	// Try to get from pool first
	if (TArray<URigVMDetailsViewWrapperObject*>* Pool = InstancePool.Find(InWrapperObjectClass))
	{
		// Filter out any invalid instances
		Pool->RemoveAll([](URigVMDetailsViewWrapperObject* Candidate)
		{
		...
		if (Pool->Num() > 0)
		{
			...
			// Rename to new outer if needed
			if (Instance->GetOuter() != InOuter)
			{
				Instance->Rename(nullptr, InOuter, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional);
			}
		}
	}
	...
	{
		Instance = NewObject<URigVMDetailsViewWrapperObject>(InOuter, InWrapperObjectClass, NAME_None, RF_Public | RF_Transient | RF_TextExportTransient | RF_DuplicateTransient);
	}
	
	return Instance;
}

void URigVMDetailsViewWrapperObject::ReturnToPool(URigVMDetailsViewWrapperObject* InInstance)
{
	if (!InInstance || !IsValid(InInstance))
		return;

	UClass* InstanceClass = InInstance->GetClass();

	// Don't pool instances of outdated classes
	if (OutdatedClassToRecreate.Contains(InstanceClass))
	{
		InInstance->RemoveFromRoot();
		InInstance->MarkAsGarbage();
		return;
	}

	TArray<URigVMDetailsViewWrapperObject*>& Pool = InstancePool.FindOrAdd(InstanceClass);

	// Don't exceed max pool size
	if (Pool.Num() >= MaxPoolSizePerClass)
	{
		InInstance->RemoveFromRoot();
		InInstance->MarkAsGarbage();
		return;
	...
	// Reset state and add to pool
	InInstance->ResetForReuse();
	Pool.Add(InInstance);
}

void URigVMDetailsViewWrapperObject::ClearClassPool(UClass* InClass)
{
	TArray<URigVMDetailsViewWrapperObject*> Pool;
	if (InstancePool.RemoveAndCopyValue(InClass, Pool))
	{
		for (URigVMDetailsViewWrapperObject* Instance : Pool)
		{
			if (IsValid(Instance))
			{
...
void URigVMDetailsViewWrapperObject::ClearAllClassPools()
{
	// During shutdown, we don't need to remove instances individually
	if (!IsEngineExitRequested())
	{
		TArray<UClass*> Keys;
		InstancePool.GetKeys(Keys);
...
void URigVMDetailsViewWrapperObject::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	do
	{
		...
		if (PropertyChangedEvent.ChangeType == EPropertyChangeType::ToggleEditable && PropertyNode->GetValue() == PropertyChangedEvent.Property)
		{
			bool bNegate;
			if (FProperty* EditProperty = PropertyCustomizationHelpers::GetEditConditionProperty(PropertyChangedEvent.Property, bNegate))
			{
				PropertyName = EditProperty->GetName();
			}
		}
...
void URigVMDetailsViewWrapperObject::ResetForReuse()
{
	// Unbind from graph events if this was a node wrapper
	if (URigVMNode* Node = Cast<URigVMNode>(GetSubject()))
	{
	...
	// Clear the subject reference
	SubjectPtr.Reset();

	// Clear the event delegate bindings
	WrappedPropertyChangedChainEvent.Clear();

	// Reset the value setting guard
	bIsSettingValue = false;

	// Reset all UPROPERTY values to defaults from CDO
	UClass* MyClass = GetClass();
	UObject* CDO = MyClass->GetDefaultObject();
	for (TFieldIterator<FProperty> It(MyClass); It; ++It)
	{
		FProperty* Property = *It;
		// Skip properties from parent class (URigVMDetailsViewWrapperObject itself)
		if (Property->GetOwnerClass() != MyClass)
			continue;

		uint8* DestPtr = Property->ContainerPtrToValuePtr<uint8>(this);
		const uint8* SrcPtr = Property->ContainerPtrToValuePtr<uint8>(CDO);
		Property->CopyCompleteValue(DestPtr, SrcPtr);
	}
	
	// Move to transient package so we don't hold references to the old outer
	Rename(nullptr, GetTransientPackage(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional);
	AddToRoot();  // Ensure pooled instances won't be GC'd
}
...
void URigVMDetailsViewWrapperObject::SetContentForPin(URigVMPin* InPin)
{
	...
	public:
		...
		{
			UE_LOGF(LogRigVMDeveloper, Error, "Error Importing To Hierarchy: %ls", V);
			NumErrors++;
		}
	};
