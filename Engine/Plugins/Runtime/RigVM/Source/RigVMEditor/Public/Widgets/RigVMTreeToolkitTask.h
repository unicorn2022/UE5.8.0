...
public:
	...
	UE_DEPRECATED(5.7, "Please use FRigVMEditorAssetInterfacePtr GetRigVMAsset(const TSharedRef<FRigVMTreePhase>& InPhase) const")
	virtual URigVMBlueprint* GetBlueprint(const TSharedRef<FRigVMTreePhase>& InPhase) const
	{
		return nullptr;
	}

	virtual FRigVMEditorAssetInterfacePtr GetRigVMAsset(const TSharedRef<FRigVMTreePhase>& InPhase) const
	{
		return nullptr;
	}
