...
public:
	...
	virtual ~FRigVMFunctionArgumentReorderDragDropHandler() override = default;
...
public:
	...
	FRigVMFunctionArgumentLayout(
		...
		, NameValidator((const FRigVMEditorAssetInterfacePtr)nullptr, InGraph.Get(), InPin->GetFName()) // TODO check if missing Blueprint affects name validation
	{}
...
private:
	...
	typedef TTuple<EPinContainerType, bool, FText, FText, const FSlateBrush*> TContainerType;
	...
	UE_API const TArray<TSharedPtr<TContainerType>> GetContainerTypes() const;
	UE_API bool CanChangeContainerType() const;
	UE_API TSharedRef<SWidget> GetContainerTypeMenuContent();
	UE_API const FSlateBrush* GetContainerTypeIconImage() const;
	UE_API FSlateColor GetContainerTypeIconColor() const;
	UE_API void OnContainerTypeSelectionChanged(TSharedPtr<TContainerType> InContainerType);
...
private:
	...
	/** The ed graph schema that this is using */
	mutable TWeakObjectPtr<URigVMEdGraphSchema> EdGraphSchemaPtr;
	...
	const FSlateBrush* CachedImg_Pin_Disconnected = nullptr;
	const FSlateBrush* CachedImg_RefPin_Disconnected = nullptr;
};
...
private:
	...
	UE_API void HandleModifiedEvent(ERigVMGraphNotifType InNotifType, URigVMGraph* InGraph, UObject* InSubject, bool bFromTargetToSource);

	TWeakObjectPtr<URigVMGraph> GraphPtr;
	TWeakObjectPtr<URigVMEdGraph> EdGraphOuterPtr;
	TWeakInterfacePtr<IRigVMClientHost> WeakRigVMClientHost;
	FSimpleDelegate OnRebuildChildren;
	TSharedPtr<SRigVMNodePreviewWidget> PreviewWidget;
	FDelegateHandle GraphChangedDelegateHandle;
	TSharedPtr<FRigVMMinimalEnvironment> MinimalEnvironment;
	bool bIsHandlingModifiedEvent;
};
...
public:
	...
	UE_API FRigVMGraphDetailCustomization(TSharedPtr<IRigVMEditor> RigVMigEditor, FRigVMEditorAssetInterfacePtr RigVMBlueprint);
	...
	UE_API FRigVMGraphDetailCustomization(TSharedPtr<IBlueprintEditor> RigVMigEditor, FRigVMEditorAssetInterfacePtr RigVMBlueprint);
#endif
...
private:
	...
	TWeakInterfacePtr<IRigVMEditorAssetInterface> RigVMBlueprintPtr;
...
private:
	...
	UE_API ECheckBoxState GetFunctionPureCheckState() const;
	UE_API void OnFunctionPureCheckStateChanged(ECheckBoxState NewState);
...
protected:
	...
	TWeakInterfacePtr<IRigVMEditorAssetInterface> BlueprintBeingCustomized;
