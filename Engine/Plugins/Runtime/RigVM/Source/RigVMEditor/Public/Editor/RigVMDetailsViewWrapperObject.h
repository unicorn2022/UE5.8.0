...
public:
	...
	static UE_API URigVMDetailsViewWrapperObject* GetOrCreateNewInstance(const UClass* InWrapperObjectClass, UObject* InOuter);
	
	// Return an instance to the pool for reuse
	static UE_API void ReturnToPool(URigVMDetailsViewWrapperObject* InInstance);

	// Clear pooled instance of a certain class
	static UE_API void ClearClassPool(UClass* InClass);
	
	// Clear all pooled instances (call on module shutdown)
	static UE_API void ClearAllClassPools();
...
private:
	...
	// Pool of available wrapper objects per class
	static TMap<UClass*, TArray<URigVMDetailsViewWrapperObject*>> InstancePool;

	// Maximum pool size per class (prevents unbounded growth)
	static constexpr int32 MaxPoolSizePerClass = 32;
