...
struct FRigVMAction_PromoteVariable : public FEdGraphSchemaAction
{
	...
	virtual UEdGraphNode* PerformAction( class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode = true) override
	{
		...
		{
			if (FRigVMEditorAssetInterfacePtr RigVMBlueprint = FRigVMBlueprintUtils::FindAssetForGraph(ParentGraph))
			{
...
UEdGraphNode* FRigVMGraphSchemaAction_NewNode::PerformAction(class UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
	if (URigVMEdGraph* RigVMEdGraph = Cast<URigVMEdGraph>(ParentGraph))
	{
		if (URigVMEdGraphNode* NewNode = NodeSpawner->Invoke(RigVMEdGraph, FVector2D(Location)))
		{
			...
			FRigVMEditorAssetInterfacePtr Blueprint = FRigVMBlueprintUtils::FindAssetForGraph(ParentGraph);
			URigVMController* Controller = Blueprint->GetController(ParentGraph);
			if (FromPin)
			{
				RigVMEdGraph->GetRigVMEdGraphSchema()->AutowireNewNode(NewNode, FromPin);
			}

			if (bSelectNewNode)
			{
				Controller->SelectNode(NewNode->GetModelNode(), true);
			}
		}
	}
			...
			[
				...
				[
					SAssignNew(GraphActionMenu, SGraphActionMenu)
						...
						.OnActionDoubleClicked(this, &SRigVMActionMenu::OnActionSelected, ESelectInfo::OnMouseClick)
...
TSharedRef<FGraphActionListBuilderBase> SRigVMActionMenu::OnGetActionList()
{
	...
	FRigVMEditorAssetInterfacePtr Asset = BlueprintEditor->GetRigVMAssetInterface();
	if (IRigVMEditorModule* RigVMEditorModule = Asset->GetEditorModule())
	{
		TArray<TSharedRef<URigVMEdGraphNodeSpawner>> NodeSpawners;
		RigVMEditorModule->GetTypeActions(Asset, NodeSpawners);
		RigVMEditorModule->GetInstanceActions(Asset, NodeSpawners);
		
		for (TSharedRef<URigVMEdGraphNodeSpawner>& NodeSpawner : NodeSpawners)
		{
			TSharedPtr<FRigVMGraphSchemaAction_NewNode> NewNodeAction(new FRigVMGraphSchemaAction_NewNode(
					NodeSpawner->GetMenuCategory(),
					NodeSpawner->GetMenuName(),
					NodeSpawner->GetMenuTooltip(),
					0
				));
			NewNodeAction->NodeSpawner = NodeSpawner;
	...
	if (const URigVMEdGraphSchema* Schema = Cast<const URigVMEdGraphSchema>(GraphObj->GetSchema()))
	{
		Schema->InsertAdditionalActions({Asset}, FilterContext.Graphs, FilterContext.Pins, *ContextMenuBuilder);
	}
...
TSharedRef<SWidget> SRigVMActionMenu::OnCreateWidgetForAction(FCreateWidgetForActionData* const InCreateData)
{
	...
	// Get Palette Item Icon
	{
		...
		else if (GraphAction->GetTypeId() == FRigVMGraphSchemaAction_NewNode::StaticGetTypeId())
		{
			FRigVMGraphSchemaAction_NewNode* NodeSpawnerAction = (FRigVMGraphSchemaAction_NewNode*)GraphAction.Get();
			IconBrush = NodeSpawnerAction->NodeSpawner->GetMenuIcon().GetOptionalIcon();
		}
	}
