...
protected:
	...
	UE_API virtual void CommonInitialization(const TArray<FRigVMEditorAssetInterfacePtr>& InitBlueprints, bool bShouldOpenInDefaultsMode) override;
...
public:
	...
	TSharedRef<SWidget> GetCompilerResults() const;
...
protected:
	...
	UE_API virtual void AnalyticsTrackNodeEvent(IRigVMEditorAssetInterface* Blueprint, UEdGraphNode* GraphNode, bool bNodeDelete) const override;
	...
	UE_API virtual void OnBlueprintChangedInnerImpl(IRigVMEditorAssetInterface* InBlueprint, bool bIsJustBeingCompiled) override;
	...
	/** Called when the graph canvas was double clicked */
	UE_API void HandleGraphCanvasDoubleClicked(UEdGraph* InGraph);
	UE_API void OnClosedActionMenu(UEdGraph* InEdGraph) const;
	...
	UE_API virtual bool IsDetailsPanelEditingGlobalOptions() const override;
	UE_API virtual bool IsDetailsPanelEditingClassDefaults() const override;
...
public:
	...
	UE_API virtual void AddFindWidget(FToolBarBuilder& ToolbarBuilder) override;
...
public:
	UE_DEPRECATED(5.7, "Please use void CreateDefaultTabContents(const TArray<FRigVMEditorAssetInterfacePtr> InBlueprints)")
	void CreateDefaultTabContents(const TArray<UBlueprint*> InBlueprints){}
	UE_API void CreateDefaultTabContents(const TArray<FRigVMEditorAssetInterfacePtr> InBlueprints);

	UE_API TSharedRef<SWidget> GenerateCompileOptionsMenu();
	UE_API void MakeSaveOnCompileSubMenu(FMenuBuilder& InMenu);
	UE_API void SetSaveOnCompileSetting(ERigVMSaveOnCompile NewSetting);
	UE_API bool IsSaveOnCompileOptionSet(ERigVMSaveOnCompile Option);
...
protected:
	...
	FName ElementInDetailsPanel = NAME_None;

	/** Last frame in which the graph was double clicked. Useful to filter excess double clicks in a single frame */
	uint64 LastDoubleClickFrame = TNumericLimits<uint64>::Max();

	UE_API void HandlePreVariablesChanged(FName InName);
	UE_API void HandlePostVariablesChanged(FName InName);
};
