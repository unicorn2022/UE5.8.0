...
namespace UELocal
{
		...
		FString CallerString)
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "%ls: Property Set is null", *CallerString);
			return nullptr;
		}
		...
		for (TFieldIterator<FProperty> PropIt(PropertySet->GetClass(), EFieldIterationFlags::None); PropIt; ++PropIt)
		{
			...
			if (Property->GetName() == PropertyName)
			{
				...
				{
					UE_LOGF(LogTemp, Warning, "%ls: Property %ls does not have a compatible Type", *CallerString, *PropertyName);
					return nullptr;
				}
			}
		}

		UE_LOGF(LogTemp, Warning, "%ls: Could not find a Property named %ls in the Property Set %ls", *CallerString, *PropertyName,
			*PropertySet->GetName());
		return nullptr;
	}
...
UScriptableInteractiveToolPropertySet* UScriptableInteractiveToolPropertySet::MakeTransactional()
{
...
UScriptableTool_RenderAPI* UScriptableTool_RenderAPI::DrawLine(FVector Start, FVector End, FLinearColor Color, float Thickness, float DepthBias, bool bDepthTested)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "[ScriptableInteractiveTool::DrawLine] DrawLine can only be called during OnScriptRender event callback");
		return this;
	}
...
UScriptableTool_RenderAPI* UScriptableTool_RenderAPI::DrawRectWidthHeightXY(FTransform Transform, double Width, double Height, FLinearColor Color, float LineThickness, float DepthBias, bool bDepthTested, bool bOriginIsCenter)
{
	if (ActiveVisualizer == nullptr)
	{
		UE_LOGF(LogTemp, Warning, "[ScriptableInteractiveTool::DrawRectWidthHeight] DrawRectWidthHeight can only be called during OnScriptRender event callback");
		return this;
	}
...
UScriptableTool_HUDAPI* UScriptableTool_HUDAPI::DrawTextAtLocation(FVector Location, FString String, FLinearColor Color, bool bCentered, float ShiftRowsY)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "[ScriptableInteractiveTool::DrawTextAtLocation] DrawTextAtLocation can only be called during OnScriptDrawHUD event callback");
		return this;
	}
...
UScriptableTool_HUDAPI* UScriptableTool_HUDAPI::GetCanvasLocation(FVector Location, FVector2D& CanvasLocation)
{
	if (ActiveCanvas == nullptr || ActiveSceneView == nullptr)
	{
		UE_LOGF(LogTemp, Warning, "[ScriptableInteractiveTool::GetCanvasLocation] DrawTextAtLocation can only be called during OnScriptDrawHUD event callback");
	...
	EToolsFrameworkOutcomePins& Outcome)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "[AddPropertySetOfType] PropertySet Identifier %ls is invalid or already in use!", *Identifier);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogTemp, Warning, "[AddPropertySetOfType] Class Type %ls is not a UScriptableInteractiveToolPropertySet Subclass!", *Class->GetAuthoredName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogTemp, Warning, "[AddPropertySetOfType] failed to create instance of Class Type %ls", *Class->GetAuthoredName());
		return nullptr;
	}
	...
	const FToolFloatPropertyModifiedDelegate& OnModified)
{
			...
			PropertySet, PropertyName, TEXT("WatchFloatProperty")))
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "WatchFloatProperty: Property %ls is not Float type", *PropertyName);
			return PropertySet;
		}
	...
	const FToolIntPropertyModifiedDelegate& OnModified)
{
			...
			PropertySet, PropertyName, TEXT("WatchIntProperty")))
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "WatchIntProperty: Property %ls is not Integer type", *PropertyName);
			return PropertySet;
		}
	...
	const FToolPropertyModifiedDelegate& OnModified)
{
	...
	if (FNumericProperty* NumericProp = CastField<FNumericProperty>(Property))
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "WatchProperty: Property %ls is an unsupported type", *PropertyName);
		}
	}
	...
	{
		UE_LOGF(LogTemp, Warning, "WatchProperty: Property Type for Property %ls is not currently supported", *PropertyName)
	...
	EToolsFrameworkOutcomePins& Outcome)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "[CreateTRSGizmo] Gizmo Identifier %ls is invalid or already in use!", *Identifier);
		return;
	}
	...
	{
		UE_LOGF(LogTemp, Warning, "[CreateTRSGizmo] Cannot create a Transform Gizmo thath as no sub-widgets");
		return;
	}
	...
	if (NewGizmo == nullptr)
	{
		UE_LOGF(LogTemp, Warning, "[CreateTRSGizmo] Gizmo creation failed");
		return;
	}
	...
	EToolsFrameworkOutcomePins& Outcome)
{
	...
	if ( Found == nullptr )
	{
		UE_LOGF(LogTemp, Warning, "[DestroyTRSGizmo] Gizmo Identifier %ls could not be found", *Identifier);
		return;
	}
	...
	bool bVisible)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "[SetGizmoVisible] Gizmo Identifier %ls could not be found", *Identifier);
	}
}
	...
	bool bUndoable)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "[SetGizmoTransform] Gizmo Identifier %ls could not be found", *Identifier);
	}
}
	...
	FString Identifier)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "[GetGizmoTransform] Gizmo Identifier %ls could not be found", *Identifier);
...
void UScriptableInteractiveTool::AddLogMessage(FText Message, bool bHighlighted)
{
	if (UInteractiveToolManager* ToolManager = GetToolManager())
	{
		if (bHighlighted)
		{
			UE_LOGF(LogScriptableInteractiveTool, Warning, "%ls", *Message.ToString() );
		}
		else
		{
			UE_LOGF(LogScriptableInteractiveTool, Display, "%ls", *Message.ToString() );
		}
	}
}
...
void UScriptableInteractiveTool::SetOverlayWidget(UUserWidget* Widget, bool bMakeDraggable )
{
	...
	if (ViewportWidgetAPI && Widget)
	{
		if (bMakeDraggable)
		{
			if (ViewportWidgetAPI->MakeDraggableWidgetFunc)
			{
				TSharedRef<SWidget> WrappedWidget = Widget->TakeDerivedWidget<ScriptableInteractiveToolLocals::SScriptableToolViewportOverlayDragContainer>([&ViewportWidgetAPI](UUserWidget* Widget, TSharedRef<SWidget> Content) {
				return SNew(ScriptableInteractiveToolLocals::SScriptableToolViewportOverlayDragContainer, Widget)
					[
						ViewportWidgetAPI->MakeDraggableWidgetFunc(Content)
					];
				});
