...
namespace Audio
{
	...
	,	SlidingBuffer(1, 1)
	{
		...
		{
			UE_LOGF(LogAudioSynesthesiaCore, Log, "The analysis window size [%d] is being reduced to the FFT size [%d]", AnalysisWindowNumSamples, FFTSize);
			AnalysisWindowNumSamples = FFTSize;
		}

		UE_CLOGF(AnalysisHopNumSamples > AnalysisWindowNumSamples, LogAudioSynesthesiaCore, Log, "Some samples will not be anlayzed because the hop size [%d] is larger than the analysis window [%d]", AnalysisHopNumSamples, AnalysisWindowNumSamples);
