...
namespace GameFeatureData
{
#if WITH_EDITOR
	class FScopeGlobalCoreRedirectAdded
	{
	public:
		FScopeGlobalCoreRedirectAdded(const FString& PluginName, const FString& PluginConfigFilename)
		{
			...
			FCoreRedirects::FGlobalCoreRedirectAddedEvent* Event = FCoreRedirects::GetOnGlobalCoreRedirectAdded();
			if (!Event)
			{
				return;
			}

			OnGlobalCoreRedirectAddedHandle = Event->AddLambda([&PluginName, &PluginConfigFilename](const FCoreRedirect& CoreRedirect)
				{
					UE_LOGF(LogGameFeatures, Warning, "Plugin %ls added global core redirect defined in file %ls (%ls -> %ls). This will trigger useless recooks.",
						*PluginName, *PluginConfigFilename, *CoreRedirect.OldName.ToString(), *CoreRedirect.NewName.ToString());
				});
		}

		~FScopeGlobalCoreRedirectAdded()
		{
			if (OnGlobalCoreRedirectAddedHandle.IsValid())
			{
				FCoreRedirects::GetOnGlobalCoreRedirectAdded()->Remove(OnGlobalCoreRedirectAddedHandle);
			}
		}

	private:
		FDelegateHandle OnGlobalCoreRedirectAddedHandle;
	};
#else // #if WITH_EDITOR
	class FScopeGlobalCoreRedirectAdded
	{
...
static void ProcessDeprecation(const FString& File)
{
	if (!IsInGameThread())
	{
		AsyncTask(ENamedThreads::GameThread, [File]() { ProcessDeprecation(File); });
		return;
	}

	FFormatNamedArguments Arguments;
	Arguments.Add(TEXT("File"), FText::FromString(File));
	FText Msg = FText::Format(LOCTEXT("DeprecatedGFPConfig", "Found a deprecated, old-style, GFP config file '{File}'. Rename if to the new Default<PluginName>.ini style, and MAKE SURE to add + tokens before array properties (like other ini files would have). In the future, this file will not be loaded."), Arguments);
...
void UGameFeatureData::InitializeBasePluginIniFile(const FString& PluginInstalledFilename)
{
	...
	// UE_DEPRECATED(5.8, "Handle old-style named GFP .ini files - keeping for a bit to tell licensees, as well as any straggling merges of streams into Main")
	// (Using the UE_DEPRECATED tag above just so we can find this later to delete the whole block)
	// This old path will be removed. See ProcessDeprecation() has the error messaging
	{
		...
		{
			ProcessDeprecation(PluginConfigFilename);
			...
			GameFeatureData::FScopeGlobalCoreRedirectAdded ScopeWarnOnDlcAddingGlobalCoreRedirect(PluginName, PluginConfigFilename);
		...
		// remove the branch so we don't dirty it up with empty branches
	...
	if (bUseNewDynamicLayers)
	{
		...
		UE_LOGF(LogGameFeatures, Verbose, "Loading GameFeature base plugin hierarchy for %ls", *PluginName);
...
void UGameFeatureData::InitializeHierarchicalPluginIniFiles(const FString& PluginInstalledFilename)
{
	...
	if (bUseNewDynamicLayers)
	{
		...
		UE_LOGF(LogGameFeatures, Verbose, "Loading GameFeature config modification for %ls", *PluginName.ToString());
		...
		UE::DynamicConfig::PerformDynamicConfig(PluginName, [PluginName](FConfigModificationTracker* ChangeTracker)
			{
				...
				// track the loaded files so we can log potential filenames below in the ensure
				...
				{
					ensureAlwaysMsgf(!Pair.Value.Contains(TEXT("ConsoleVariables")) && !Pair.Value.Contains(TEXT("ConsoleVariables_HighPriority")),
						TEXT("GameFeaturePlugins .ini files should no longer use [ConsoleVariables] or [ConsoleVariables_HighPriority] sections. Instead, use [PluginCVars_LowPriority] and [PluginCVars_HighPriority]. The old naming is in one or more of: '%s'."),
						*FString::Join(ChangeTracker->LoadedFiles, TEXT(", ")));
				}
#endif
			});
		UE::DynamicConfig::PerformDynamicConfig(PluginName, [PluginName](FConfigModificationTracker* ChangeTracker)
			{
				{
					// Hotfixes still use [ConsoleVariables], not PluginCVars_Low/HighPriority - since these are set at Hotfix priority, 
					// we use the HF style naming
...
#undef HOTFIX_BRANCH
	...
	auto InsertRuntimeDeviceProfilesIntoConfig = [&](FConfigFile& PluginConfig, const FConfigFile& ExistingConfig)
	{
		...
		for (auto& Section : AsConst(PluginConfig))
		{
			...
			if (Section.Key.Split(TEXT(" "), &RuleName, &ParentClass))
			{
				...
				{
					UE_LOGF(LogGameFeatures, Log, "Game feature '%ls' found runtime device profile rule %ls", *PluginName, *RuleName);
					...
					if (ParentProfileName && ProfileSuffix)
					{
						...
						for (const UDeviceProfile* Profile : DeviceProfileManager.Profiles)
						{
							...
							if (bProfileHasCompatibleParent && !Profile->GetName().EndsWith(ProfileSuffix->GetValue()))
							{
								...
								{
									UE_LOGF(LogGameFeatures, Log, "Ignoring profile %ls that has already been overriden as %ls", *Profile->GetName(), *FinalProfileName);
									continue;
								}
								...
								UE_LOGF(LogGameFeatures, Log, "Creating override for base profile %ls", *Profile->GetName());
								...
								if (Profile->GetName().Contains("MatchedFragments"))
								{
									...
									UE_LOGF(LogGameFeatures, Log, "Found %d fragment matching rules", MatchingRulesArray.Num());
								...
								for (const auto& CVar : PluginCVars)
								{
									...
									{
										UE_LOGF(LogGameFeatures, Log, " Found CVar: %ls=%ls", *CVarKey, *CVarValue);
								...
								for (const auto& CVar : HotfixCVars)
								{
									...
									{
										UE_LOGF(LogGameFeatures, Log, " Added CVar: %ls=%ls", *CVarKey, *CVarValue);
					...
					{
						UE_LOGF(LogGameFeatures, Warning, "Game feature '%ls' has invalid runtime device profile with parent %ls, suffix %ls, %d CVars, %d fragments",
				...
				{
					UE_LOGF(LogGameFeatures, Log, "Game feature '%ls' found device profile fragment %ls", *PluginName, *RuleName);
					...
					if (HotfixCVars.Num() > 0)
					{
						...
						for (const auto& CVar : PluginCVars)
						{
							...
							{
								UE_LOGF(LogGameFeatures, Log, " Removed CVar: %ls", *CVar.Value.GetValue());
								PluginConfig.RemoveFromSection(*Section.Key, CVar.Key, CVar.Value.GetValue());
							}
							else
							{
								UE_LOGF(LogGameFeatures, Log, " Kept CVar: %ls=%ls", *PluginCVarKey, *PluginCVarValue);
							}
						}
						...
						for (const auto& CVar : HotfixCVars)
						{
							...
							{
								UE_LOGF(LogGameFeatures, Log, " Added CVar: %ls=%ls", *HotfixCVarKey, *HotfixCVarValue);
	...
	auto LoadDeviceProfilesFromConfig = [&](const FConfigFile& Config)
	{
		for (TPair<const FString&, const FConfigSection&> Section : Config)
		{
			...
			if (Section.Key.Split(TEXT(" "), &ProfileName, &ParentClass) && ParentClass == UDeviceProfile::StaticClass()->GetName())
			{
				...
				if (DeviceType)
				{
					UE_LOGF(LogGameFeatures, Log, "Game feature '%ls' adding new device profile %ls", *PluginName, *ProfileName);
	...
	for (const FIniLoadingParams& Ini : IniFilesToLoad)
	{
		...
		{
			UE_LOGF(LogGameFeatures, Log, "Game feature '%ls' loaded config file %ls", *PluginName, *PluginIniName);
...
void UGameFeatureData::InitializeHierarchicalPluginIniFiles(const TArrayView<FString>& PluginInstalledFilenames)
{
	...
	if (bUseNewDynamicLayers)
	{
		...
		if (UE_GET_LOG_VERBOSITY(LogGameFeatures) >= ELogVerbosity::Verbose)
		{
			...
			UE_LOGF(LogGameFeatures, Verbose, "Loading GameFeature config modification for %ls", *PluginList);
		}
#endif
...
void UGameFeatureData::ReloadConfigs(FConfigFile& PluginConfig)
{
	...
	for (const auto& ConfigEntry : AsConst(PluginConfig))
	{
		...
		if (PerObjConfigDelimIdx != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogGameFeatures, Warning, "Couldn't find PerObjectConfig class %ls for %ls while processing %ls, config changes won't be reloaded.", *ClassName, *ObjectName, *PluginConfig.Name.ToString());
			}
		}
