...
void UGameFeatureAction_AddActorFactory::AddActorFactory()
{
	...
	{
		UE_LOGF(LogAddActorFactory, Warning, "ActorFactory is null. Unable to add factory");
		return;
	}
	if (UClass* FactoryClass = ActorFactory.LoadSynchronous())
	{
		...
		{
			UE_LOGF(LogAddActorFactory, Error, "ActorFactory (%ls) was not an ActorFactory class", *FactoryClass->GetName());
			FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("AddActorFactory_BadSubclass", "Selected class was not an ActorFactory class."));
			ActorFactory.Reset();
			return;
		}
		UE_LOGF(LogAddActorFactory, Verbose, "Adding actor factory %ls", *FactoryClass->GetName());
...
void UGameFeatureAction_AddActorFactory::RemoveActorFactory()
{
	...
	{
		UE_LOGF(LogAddActorFactory, Verbose, "Removing actor factory %ls", *FactoryToRemove->GetName());
