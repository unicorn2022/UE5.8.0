	...
	if (ShouldPreloadAtRegistration())
	{
		...
		if (ensure(DataRegistrySubsystem))
		{
			for (const FDataRegistrySourceToAdd& RegistrySource : SourcesToAdd)
			{
				...
				{
					UE_LOGF(LogGameFeatures, Log, "OnGameFeatureRegistering %ls: Preloading DataRegistrySource %ls for editor preview", *GetPathName(), *RegistrySource.DataTableToAdd.ToString())
						AssetList.Add(RegistrySource.DataTableToAdd.ToSoftObjectPath());
				}

				if (!RegistrySource.CurveTableToAdd.IsNull())
				{
					UE_LOGF(LogGameFeatures, Log, "OnGameFeatureRegistering %ls: Preloading DataRegistrySource %ls for editor preview", *GetPathName(), *RegistrySource.CurveTableToAdd.ToString())
...
void UGameFeatureAction_DataRegistrySource::RegisterRuntimeSources()
{
	if (ShouldPreloadAtRegistration())
	{
		...
		for (const FDataRegistrySourceToAdd& RegistrySource : SourcesToAdd)
		{
			...
			if (bShouldAdd)
			{
				...
				if (DataRegistrySubsystem->AreRegistriesInitialized())
				{
					...
					{
						UE_LOGF(LogGameFeatures, Log, "RegisterRuntimeSources %ls: DataRegistry source asset %ls was not loaded before activation, this may cause a long hitch", *GetPathName(), *RegistrySource.DataTableToAdd.ToString())
					}
...
void UGameFeatureAction_DataRegistrySource::OnGameFeatureActivating(FGameFeatureActivatingContext& Context)
{
	Super::OnGameFeatureActivating(Context);

	RegisterRuntimeSources();
}
	
void UGameFeatureAction_DataRegistrySource::UnregisterSources()
{
	UDataRegistrySubsystem* DataRegistrySubsystem = UDataRegistrySubsystem::Get();
	if (ensure(DataRegistrySubsystem))
	{
		for (const FDataRegistrySourceToAdd& RegistrySource : SourcesToAdd)
		{
			if (!RegistrySource.DataTableToAdd.IsNull())
			{
				...
				{
					UE_LOGF(LogGameFeatures, Log, "UnregisterSources %ls: DataRegistry data table %ls failed to unregister", *GetPathName(), *RegistrySource.DataTableToAdd.ToString())
				}
				else
				{
					UE_LOGF(LogGameFeatures, Log, "UnregisterSources %ls: Temporarily disabling data table %ls", *GetPathName(), *RegistrySource.DataTableToAdd.ToString())
				}
			}
			...
			if (!RegistrySource.CurveTableToAdd.IsNull())
			{
				...
				{
					UE_LOGF(LogGameFeatures, Log, "UnregisterSources %ls: DataRegistry curve table %ls failed to unregister", *GetPathName(), *RegistrySource.CurveTableToAdd.ToString())
				}
				else
				{
					UE_LOGF(LogGameFeatures, Log, "UnregisterSources %ls: Temporarily disabling curve table %ls", *GetPathName(), *RegistrySource.CurveTableToAdd.ToString())
				}
			}
		}
	}
