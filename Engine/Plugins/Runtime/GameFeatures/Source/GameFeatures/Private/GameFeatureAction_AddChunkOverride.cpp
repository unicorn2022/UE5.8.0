...
void UGameFeatureAction_AddChunkOverride::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UGameFeatureAction_AddChunkOverride, bShouldOverrideChunk))
	{
		...
		{
			UE_LOGF(LogAddChunkOverride, Log, "Detected invalid ChunkId autogenerating new ID based on PluginName");
...
void UGameFeatureAction_AddChunkOverride::AddChunkIdOverride()
{
	...
	if (ChunkId < 0)
	{
		UE_LOGF(LogAddChunkOverride, Error, "ChunkId is negative. Unable to override to a negative chunk");
		return;
	}
	...
	if (UGameFeatureData* GameFeatureData = GetTypedOuter<UGameFeatureData>())
	{
		...
		UE_LOGF(LogAddChunkOverride, Log, "Plugin(%ls) will cook assets into chunk(%d)", *PluginName, ChunkId);
...
void UGameFeatureAction_AddChunkOverride::RemoveChunkIdOverride()
{
	...
	if (UGameFeatureData* GameFeatureData = GetTypedOuter<UGameFeatureData>())
	{
		...
		{
			UE_LOGF(LogAddChunkOverride, Verbose, "No chunk override found for (%ls) Skipping override removal", *PluginName);
			return;
		}
		...
		UE_LOGF(LogAddChunkOverride, Log, "Removing ChunkId override (%d) for Plugin (%ls)", ChunkIdOverride, *PluginName);
...
int32 UGameFeatureAction_AddChunkOverride::GenerateUniqueChunkId() const
{
	...
	{
		UE_LOGF(LogAddChunkOverride, Warning, "Autogenerated ChunkId(%d) is lower than the config specified LowestAllowedChunkIndexForAutoGeneration(%d)", NewChunkId, LowestAllowedChunkIndexForAutoGeneration);
	...
	{
		UE_LOGF(LogAddChunkOverride, Warning, "ChunkId(%d) is in use by %ls. Unable to autogenerate unique id. Lowest allowed ChunkId(%d)", NewChunkId, *FString::Join(GameFeatureAction_AddChunkOverride::ChunkIdToPluginMap[ChunkId], TEXT(",")), LowestAllowedChunkIndexForAutoGeneration);
