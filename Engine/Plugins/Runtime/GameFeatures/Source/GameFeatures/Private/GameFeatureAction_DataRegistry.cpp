		...
		if (ensure(DataRegistrySubsystem))
		{
			for (const TSoftObjectPtr<UDataRegistry>& RegistryToAdd : RegistriesToAdd)
			{
				if (!RegistryToAdd.IsNull())
				{
					...
					UE_LOGF(LogGameFeatures, Log, "OnGameFeatureRegistering %ls: Preloading DataRegistry %ls for editor preview", *GetPathName(), *RegistryPath.ToString())
		...
		for (const TSoftObjectPtr<UDataRegistry>& RegistryToAdd : RegistriesToAdd)
		{
			if (!RegistryToAdd.IsNull())
			{
				...
				if (DataRegistrySubsystem->AreRegistriesInitialized())
				{
					...
					if (!LoadedRegistry)
					{
						UE_LOGF(LogGameFeatures, Log, "OnGameFeatureActivating %ls: DataRegistry %ls was not loaded before activation, this may cause a long hitch", *GetPathName(), *RegistryPath.ToString())
					}
					else
					{
						...
						{
							UE_LOGF(LogGameFeatures, Log, "OnGameFeatureActivating %ls: DataRegistry %ls is already enabled from another source! This can cause problems on deactivation", *GetPathName(), *RegistryPath.ToString())
						}
					}
				}
#endif
		...
		if (ensure(DataRegistrySubsystem))
		{
			for (const TSoftObjectPtr<UDataRegistry>& RegistryToAdd : RegistriesToAdd)
			{
				if (!RegistryToAdd.IsNull())
				{
					...
					UE_LOGF(LogGameFeatures, Log, "OnGameFeatureUnregistering %ls: Temporarily disabling preloaded DataRegistry %ls", *GetPathName(), *RegistryPath.ToString())
