...
#define GFP_STATE_LOG(VERBOSITY, MSG, ...) UE_LOGFMT(LogGameFeatures, VERBOSITY, \
	"{Plugin}[{State}] " MSG, StateProperties.PluginIdentifier.GetPluginName(), State __VA_OPT__(,) __VA_ARGS__);
...
namespace UE::GameFeatures
{
	...
	bool bUseNewExecutor = true;
	static FAutoConsoleVariableRef CVarUseNewExecutor(TEXT("GameFeaturePlugin.UseNewExecutor"),
		bUseNewExecutor,
		TEXT("Should control of state machine updates be handled by the GameFeaturesSubsystem (new behavior) to all more batch & parallel processing."),
	ECVF_ReadOnly | ECVF_SaveForNextBoot);

	// Options for how to defer execution of state updates and how to order those updates
	struct FExecutionOptions
	{
		// Group by current state before executing
		bool bGrouped = true;
		// Execute the update requests (potentially within each group) in a random order
		bool bShuffle = false;
		// Reverse the order of update requests so that deferred request are executed in LIFO order
		bool bReverse = false;
	} ExecutionOptions;

	static FAutoConsoleVariableRef CVarExecutionGrouped(TEXT("GameFeaturePlugin.Execution.Grouped"),
		ExecutionOptions.bGrouped,
		TEXT("Group deferred updates by current state before executing. This is the intended future default path."),
		ECVF_Default);
	static FAutoConsoleVariableRef CVarExecutionShuffle(TEXT("GameFeaturePlugin.Execution.Shuffle"),
		ExecutionOptions.bShuffle,
		TEXT("Shuffle deferred updates before execution (within a group if .Grouped is set). For testing implicit dependencies."),
		ECVF_Default);
	static FAutoConsoleVariableRef CVarExecutionReverse(TEXT("GameFeaturePlugin.Execution.Reverse"),
		ExecutionOptions.bReverse,
		TEXT("Reverse deferred updates before execution to execute in LIFO order. For testing deferred dependencies."),
		ECVF_Default);

	bool bLoadBuiltin_DeferExecution = true;
	static FAutoConsoleVariableRef CVarLoadBuiltin_DeferExecution (TEXT("GameFeaturePlugin.LoadBuiltins.DeferExecution"),
		bLoadBuiltin_DeferExecution ,
		TEXT("When GameFeaturePlugin.UseNewExecutor is true, should execution of state machine updates in LoadBuiltInGameFeaturePlugins be deferred until all state transitions have been queued?"),
	ECVF_ReadOnly);
	...
	static TAutoConsoleVariable<bool> CVarForceGFPIADDownload(TEXT("GameFeaturePlugin.ForceGFPIADDownload"),
		false,
		TEXT("Enable batch processing when processing multiple plugins and specified in protocol options."));

#if !UE_BUILD_SHIPPING
	FString DebugGFPNameMatch = "";
	static FAutoConsoleVariableRef CVarDebugGFPNameMatch(TEXT("GameFeaturePlugin.DebugGFPName"), DebugGFPNameMatch, TEXT("Set cvar and add breakpoint to \"ShouldDebugGFPNameMatch()\" To debug"));

	bool ShouldDebugGFP(const FString& GFPName)
	{
		static FString CachedDebugName;
		static TUniquePtr<FRegexPattern> Pattern;
		if (CachedDebugName != DebugGFPNameMatch)
		{
			CachedDebugName = DebugGFPNameMatch;
			if (DebugGFPNameMatch.IsEmpty())
			{
				Pattern = nullptr;
			}
			else
			{
				Pattern.Reset(new FRegexPattern(DebugGFPNameMatch));
			}
		}

		if (Pattern.IsValid())
		{
			FRegexMatcher Matcher(*Pattern, GFPName);
			if (Matcher.FindNext())
			{
				return true;
			}
	...
	void MountLocalizationData(UGameFeaturePluginStateMachine* CurrentMachine, FGameFeaturePluginStateMachineProperties& StateProperties)
	{
		...
		check(CurrentMachine && &CurrentMachine->StateProperties == &StateProperties);
		...
		IPluginManager::Get().MountExplicitlyLoadedPluginLocalizationData(StateProperties.PluginName, [WeakMachine = TWeakObjectPtr<UGameFeaturePluginStateMachine>(CurrentMachine), &StateProperties, bAllowAsyncLoading = StateProperties.AllowAsyncLoading()](bool bLoadedLocalization, const FString& PluginName)
		{
			if (UGameFeaturePluginStateMachine* StateMachine = WeakMachine.Get();
				StateMachine && ensureAlways(&StateMachine->StateProperties == &StateProperties))
			{
				if (IsInGameThread())
				{
				...
				else if (bAllowAsyncLoading)
				{
					ExecuteOnGameThread(UE_SOURCE_LOCATION, [WeakMachine, &StateProperties]()
					{
						...
						{
							StateProperties.bIsLoadingLocalizationData = false;
							StateProperties.UpdateStateMachineImmediate();
						}
					});
				}
			}
	...
	constexpr double RealtimeMaxFrameTime = 0.033; // 30fps
	constexpr double RealtimeAllottedTime = RealtimeMaxFrameTime / 2;
	...
	public:
		~FRealtimeMode()
		{
			...
			int64 UpdateRequest = 0;
			UGameFeaturesSubsystem& Subsystem = UGameFeaturesSubsystem::Get();
			while (UpdateRequests.Dequeue(UpdateRequest))
			{
				Subsystem.TryUpdateStateMachineImmediate(UpdateRequest);
			}
		}

		void AddUpdateRequest(FGameFeaturePluginStateMachineProperties StateProperties)
		{
			UpdateRequests.Enqueue(StateProperties.Id);
			EnableTick();
		}
		...
		bool Tick(float DeltaTime)
		{
...

			{
				...
				while (UpdateRequests.Dequeue(UpdateRequest))
				{
					...
					if ((ElapsedTime > RealtimeAllottedTime) || ((DeltaTime + ElapsedTime) > RealtimeMaxFrameTime))
					{
						break;
					}
				}
	...
	static FAutoConsoleVariableRef CVarRealtimeMode(TEXT("GameFeaturePlugin.RealtimeMode"),
		...
		FConsoleVariableDelegate::CreateLambda(
			[](IConsoleVariable* Var)
			{
				if (bUseNewExecutor)
				{
					// new executor reads bRealtimeMode directly and enforces timings
					return;
				}
				if (Var->GetBool())
				{
...
#if WITH_EDITOR
	TMap<FString, int64> PluginsToUnloadAssets;
	...
	bool TickUnloadPluginAssets(float /*DeltaTime*/)
	{
		...
		TArray<int64> UpdateStateMachineIds;
		{
			PluginNames.Reserve(PluginsToUnloadAssets.Num());
			UpdateStateMachineIds.Reserve(PluginsToUnloadAssets.Num());
			for (TPair<FString, int64>& PluginsToUnloadAsset : PluginsToUnloadAssets)
			{
				PluginNames.Add(PluginsToUnloadAsset.Key);
				UpdateStateMachineIds.Add(PluginsToUnloadAsset.Value);
			}
		...
		UGameFeaturesSubsystem& Subsystem = UGameFeaturesSubsystem::Get();
		for (int64 Id : UpdateStateMachineIds)
		{
			Subsystem.TryUpdateStateMachineImmediate(Id);
		}

		return false;
	}

	void ScheduleUnloadPluginAssets(const FString& PluginName, const FGameFeaturePluginStateMachineProperties& StateProperties)
	{
		check(IsInGameThread());
		ensure(!PluginsToUnloadAssets.Contains(PluginName));
		PluginsToUnloadAssets.Add(PluginName, StateProperties.Id);
	...
	struct FPluginInstallBundleReferencers
	{
		...
		TMap<FString, EGFPInstallLevel> GFPs;
	};
			...
			const FStringView PluginName, const EGFPInstallLevel InLevel, const TConstArrayView<FName> Bundles)
		{
			...
			for (const FName Bundle : Bundles)
			{
				...
				if (PluginRefs)
				{
					...
					for (TPair<FString, EGFPInstallLevel>& GFPRef : PluginRefs->GFPs)
					{
	...
	// Default next state to transition to when transitioning down towards Terminal
	static constexpr EGameFeaturePluginState NextStateTowardsTerminal[EGameFeaturePluginState::MAX] = 
	{
		/* Uninitialized */ 				EGameFeaturePluginState::Uninitialized,
		/* Terminal */ 						EGameFeaturePluginState::Terminal,
		/* UnknownStatus */ 				EGameFeaturePluginState::Terminal,
		/* Uninstalled */ 					EGameFeaturePluginState::Terminal,
		/* Uninstalling */					EGameFeaturePluginState::Uninstalled,
		/* ErrorUninstalling */ 			EGameFeaturePluginState::Uninstalling, // NOTE: May not need to uninstall
		/* CheckingStatus */ 				EGameFeaturePluginState::StatusKnown, // NOTE: No down transition
		/* ErrorCheckingStatus */ 			EGameFeaturePluginState::Terminal,
		/* ErrorUnavailable */ 				EGameFeaturePluginState::Terminal,
		/* StatusKnown */ 					EGameFeaturePluginState::Uninstalling, // NOTE: May not need to uninstall
		/* Releasing */ 					EGameFeaturePluginState::StatusKnown,
		/* ErrorManagingData */ 			EGameFeaturePluginState::Releasing, // TODO: Confirm
		/* Downloading */ 					EGameFeaturePluginState::Installed, // NOTE: No down transition
		/* Installed */ 					EGameFeaturePluginState::Releasing,
		/* ErrorMounting */ 				EGameFeaturePluginState::Unmounting,
		/* ErrorWaitingForDependencies */	EGameFeaturePluginState::Unmounting,
		/* ErrorRegistering */		 		EGameFeaturePluginState::Unregistering,
		/* WaitingForDependencies */ 		EGameFeaturePluginState::AssetDependencyStreaming, // NOTE: No down transition
		/* AssetDependencyStreamOut */	 	EGameFeaturePluginState::Unmounting,
		/* ErrorAssetDependencyStreaming */ EGameFeaturePluginState::AssetDependencyStreamOut,
		/* AssetDependencyStreaming */ 		EGameFeaturePluginState::Registering, // NOTE: No down transition
		/* Unmounting */ 					EGameFeaturePluginState::Installed,
		/* Mounting */ 						EGameFeaturePluginState::WaitingForDependencies, // NOTE: No down transition
		/* Unregistering */ 				EGameFeaturePluginState::AssetDependencyStreamOut,
		/* Registering */ 					EGameFeaturePluginState::Registered, // NOTE: No down transition
		/* Registered */ 					EGameFeaturePluginState::Unregistering,
		/* ErrorLoading */					EGameFeaturePluginState::Unloading,
		/* Unloading */ 					EGameFeaturePluginState::Registered,
		/* Loading */ 						EGameFeaturePluginState::Loaded, // NOTE: No down transitoin
		/* Loaded */ 						EGameFeaturePluginState::Unloading,
		/* ErrorActivatingDependencies */	EGameFeaturePluginState::Unloading,
		/* ActivatingDependencies */ 		EGameFeaturePluginState::Activating, // NOTE: No down transition
		/* Deactivating */ 					EGameFeaturePluginState::Loaded,
		/* ErrorDeactivatingDependencies */	EGameFeaturePluginState::DeactivatingDependencies,
		/* DeactivatingDependencies */ 		EGameFeaturePluginState::Deactivating,
		/* Activating */ 					EGameFeaturePluginState::Active, // NOTE: No down transition
		/* Active */ 						EGameFeaturePluginState::DeactivatingDependencies,
	};
	// Default next state to transition to when transitioning up towards Active
	static constexpr EGameFeaturePluginState NextStateTowardsActive[EGameFeaturePluginState::MAX] = 
	{
		/* Uninitialized */ 				EGameFeaturePluginState::UnknownStatus,
		/* Terminal */ 						EGameFeaturePluginState::Terminal, // No up transition, must be recreated
		/* UnknownStatus */ 				EGameFeaturePluginState::CheckingStatus,
		/* Uninstalled */ 					EGameFeaturePluginState::CheckingStatus,
		/* Uninstalling */					EGameFeaturePluginState::Uninstalled, // NOTE: No up transition
		/* ErrorUninstalling */ 			EGameFeaturePluginState::StatusKnown,
		/* CheckingStatus */ 				EGameFeaturePluginState::StatusKnown,
		/* ErrorCheckingStatus */ 			EGameFeaturePluginState::CheckingStatus,
		/* ErrorUnavailable */ 				EGameFeaturePluginState::CheckingStatus,
		/* StatusKnown */ 					EGameFeaturePluginState::Downloading, // NOTE: May go straight to Installed
		/* Releasing */ 					EGameFeaturePluginState::StatusKnown, // NOTE: No up transition
		/* ErrorManagingData */ 			EGameFeaturePluginState::Downloading,
		/* Downloading */ 					EGameFeaturePluginState::Installed,
		/* Installed */ 					EGameFeaturePluginState::Mounting,
		/* ErrorMounting */ 				EGameFeaturePluginState::Mounting,
		/* ErrorWaitingForDependencies */	EGameFeaturePluginState::WaitingForDependencies,
		/* ErrorRegistering */		 		EGameFeaturePluginState::Registering,
		/* WaitingForDependencies */ 		EGameFeaturePluginState::AssetDependencyStreaming,
		/* AssetDependencyStreamOut */	 	EGameFeaturePluginState::Unmounting, // NOTE: No up transition
		/* ErrorAssetDependencyStreaming */ EGameFeaturePluginState::AssetDependencyStreaming,
		/* AssetDependencyStreaming */ 		EGameFeaturePluginState::Registering,
		/* Unmounting */ 					EGameFeaturePluginState::Installed, // NOTE: No up transition
		/* Mounting */ 						EGameFeaturePluginState::WaitingForDependencies,
		/* Unregistering */ 				EGameFeaturePluginState::AssetDependencyStreamOut, // NOTE: No up transition
		/* Registering */ 					EGameFeaturePluginState::Registered,
		/* Registered */ 					EGameFeaturePluginState::Loading,
		/* ErrorLoading */					EGameFeaturePluginState::Loading,
		/* Unloading */ 					EGameFeaturePluginState::Registered, // NOTE: No up transition
		/* Loading */ 						EGameFeaturePluginState::Loaded,
		/* Loaded */ 						EGameFeaturePluginState::ActivatingDependencies,
		/* ErrorActivatingDependencies */	EGameFeaturePluginState::ActivatingDependencies,
		/* ActivatingDependencies */ 		EGameFeaturePluginState::Activating,
		/* Deactivating */ 					EGameFeaturePluginState::Loaded, // NOTE: No up transition
		/* ErrorDeactivatingDependencies */	EGameFeaturePluginState::DeactivatingDependencies,
		/* DeactivatingDependencies */ 		EGameFeaturePluginState::Deactivating, // NOTE: No up transition
		/* Activating */ 					EGameFeaturePluginState::Active,
		/* Active */ 						EGameFeaturePluginState::Active
	};
	// Default next state to transition to when encountering an error
	static constexpr EGameFeaturePluginState NextStateOnError[EGameFeaturePluginState::MAX] = 
	{
		/* Uninitialized */ 				EGameFeaturePluginState::Uninitialized,// No errors handled
		/* Terminal */ 						EGameFeaturePluginState::Terminal, // No errors handled
		/* UnknownStatus */ 				EGameFeaturePluginState::UnknownStatus, // No errors handled
		/* Uninstalled */ 					EGameFeaturePluginState::Uninstalled, // No errors handled
		/* Uninstalling */					EGameFeaturePluginState::ErrorUninstalling, 
		/* ErrorUninstalling */ 			EGameFeaturePluginState::ErrorUninstalling, // Already an error state
		/* CheckingStatus */ 				EGameFeaturePluginState::ErrorCheckingStatus,
		/* ErrorCheckingStatus */ 			EGameFeaturePluginState::ErrorCheckingStatus, // Already an error state
		/* ErrorUnavailable */ 				EGameFeaturePluginState::ErrorUnavailable, // Already an error state
		/* StatusKnown */ 					EGameFeaturePluginState::StatusKnown, // No errors handled
		/* Releasing */ 					EGameFeaturePluginState::ErrorManagingData, 
		/* ErrorManagingData */ 			EGameFeaturePluginState::ErrorManagingData, // Already an error state
		/* Downloading */ 					EGameFeaturePluginState::ErrorManagingData,
		/* Installed */ 					EGameFeaturePluginState::Installed, // No errors handled
		/* ErrorMounting */ 				EGameFeaturePluginState::ErrorMounting, // Already an error state
		/* ErrorWaitingForDependencies */	EGameFeaturePluginState::ErrorWaitingForDependencies, // Already an error state
		/* ErrorRegistering */		 		EGameFeaturePluginState::ErrorRegistering, // Already an error state
		/* WaitingForDependencies */ 		EGameFeaturePluginState::ErrorWaitingForDependencies,
		/* AssetDependencyStreamOut */	 	EGameFeaturePluginState::ErrorAssetDependencyStreaming, 
		/* ErrorAssetDependencyStreaming */ EGameFeaturePluginState::ErrorAssetDependencyStreaming, // Already an error state
		/* AssetDependencyStreaming */ 		EGameFeaturePluginState::ErrorAssetDependencyStreaming,
		/* Unmounting */ 					EGameFeaturePluginState::ErrorMounting,
		/* Mounting */ 						EGameFeaturePluginState::ErrorMounting,
		/* Unregistering */ 				EGameFeaturePluginState::AssetDependencyStreamOut, // NOTE: No up transition
		/* Registering */ 					EGameFeaturePluginState::ErrorRegistering,
		/* Registered */ 					EGameFeaturePluginState::Registered, // No errors handled
		/* ErrorLoading */					EGameFeaturePluginState::Loading,
		/* Unloading */ 					EGameFeaturePluginState::ErrorLoading, 
		/* Loading */ 						EGameFeaturePluginState::ErrorLoading,
		/* Loaded */ 						EGameFeaturePluginState::Loaded, // No errors handled
		/* ErrorActivatingDependencies */	EGameFeaturePluginState::ErrorActivatingDependencies, // Already an error state
		/* ActivatingDependencies */ 		EGameFeaturePluginState::ErrorActivatingDependencies,
		/* Deactivating */ 					EGameFeaturePluginState::Deactivating, // No errors handled
		/* ErrorDeactivatingDependencies */	EGameFeaturePluginState::ErrorActivatingDependencies, // Already an error state
		/* DeactivatingDependencies */ 		EGameFeaturePluginState::ErrorDeactivatingDependencies, 
		/* Activating */ 					EGameFeaturePluginState::Activating, // No errors handled
		/* Active */ 						EGameFeaturePluginState::Active // No errors handled
	};
	static constexpr EGameFeaturePluginStateType StateType[EGameFeaturePluginState::MAX] = 
	{
		/* Uninitialized */ 				EGameFeaturePluginStateType::Transition,
		/* Terminal */ 						EGameFeaturePluginStateType::Destination,
		/* UnknownStatus */ 				EGameFeaturePluginStateType::Destination,
		/* Uninstalled */ 					EGameFeaturePluginStateType::Destination,
		/* Uninstalling */					EGameFeaturePluginStateType::Transition,
		/* ErrorUninstalling */ 			EGameFeaturePluginStateType::Error,
		/* CheckingStatus */ 				EGameFeaturePluginStateType::Transition,
		/* ErrorCheckingStatus */ 			EGameFeaturePluginStateType::Error,
		/* ErrorUnavailable */ 				EGameFeaturePluginStateType::Error,
		/* StatusKnown */ 					EGameFeaturePluginStateType::Destination,
		/* Releasing */ 					EGameFeaturePluginStateType::Transition,
		/* ErrorManagingData */ 			EGameFeaturePluginStateType::Error,
		/* Downloading */ 					EGameFeaturePluginStateType::Transition,
		/* Installed */ 					EGameFeaturePluginStateType::Destination,
		/* ErrorMounting */ 				EGameFeaturePluginStateType::Error,
		/* ErrorWaitingForDependencies */	EGameFeaturePluginStateType::Error,
		/* ErrorRegistering */		 		EGameFeaturePluginStateType::Error,
		/* WaitingForDependencies */ 		EGameFeaturePluginStateType::Transition,
		/* AssetDependencyStreamOut */	 	EGameFeaturePluginStateType::Transition,
		/* ErrorAssetDependencyStreaming */ EGameFeaturePluginStateType::Error,
		/* AssetDependencyStreaming */ 		EGameFeaturePluginStateType::Transition,
		/* Unmounting */ 					EGameFeaturePluginStateType::Transition,
		/* Mounting */ 						EGameFeaturePluginStateType::Transition,
		/* Unregistering */ 				EGameFeaturePluginStateType::Transition,
		/* Registering */ 					EGameFeaturePluginStateType::Transition,
		/* Registered */ 					EGameFeaturePluginStateType::Destination,
		/* ErrorLoading */					EGameFeaturePluginStateType::Error,
		/* Unloading */ 					EGameFeaturePluginStateType::Transition,
		/* Loading */ 						EGameFeaturePluginStateType::Transition,
		/* Loaded */ 						EGameFeaturePluginStateType::Destination,
		/* ErrorActivatingDependencies */	EGameFeaturePluginStateType::Error,
		/* ActivatingDependencies */ 		EGameFeaturePluginStateType::Transition,
		/* Deactivating */ 					EGameFeaturePluginStateType::Transition,
		/* ErrorDeactivatingDependencies */	EGameFeaturePluginStateType::Error,
		/* DeactivatingDependencies */ 		EGameFeaturePluginStateType::Transition,
		/* Activating */ 					EGameFeaturePluginStateType::Transition,
		/* Active */ 						EGameFeaturePluginStateType::Destination,
	};
}
...
{
	Writer.BeginObject();
	Writer.AddString(ANSITEXTVIEW("$type"), ANSITEXTVIEW("GameFeaturePluginStateRange"));
	Writer.AddString(ANSITEXTVIEW("$format"), ANSITEXTVIEW("({MinState}, {MaxState})"));
	Writer << ANSITEXTVIEW("MinState") << InRange.MinState;
	Writer << ANSITEXTVIEW("MaxState") <<  InRange.MaxState;
	Writer.EndObject();
}
...
void SerializeForLog(FCbWriter& Writer, EGameFeaturePluginState State)
{
#define STATE_TO_STRING(NAME, LOCNAME) case EGameFeaturePluginState::NAME: Writer.AddString(TEXT(#NAME)); break;
	switch (State)
	{
GAME_FEATURE_PLUGIN_STATE_LIST(STATE_TO_STRING)
	}
...
{
	// If the shared executor is enabled, don't create an individual callback for this state
	if (StateProperties.UpdateStateMachineDeferred(Delay))
	{
		return;
	}
	CleanupDeferredUpdateCallbacks();

	GFP_STATE_LOG(VeryVerbose, "queueing deferred update");
...
{
	StateProperties.StateProgress = Progress; 
}
...
void FDestinationGameFeaturePluginState::TryCancelState()
{
}

void FDestinationGameFeaturePluginState::EndState()
{
}
...
struct FTransitionDependenciesGameFeaturePluginState : public FGameFeaturePluginState
{
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		if (!bCheckedRealtimeMode)
		{
			...
			{
				UE::GameFeatures::RealtimeMode->AddUpdateRequest(StateProperties);
				return;
			}
		}
		...
		if (!bRequestedDependencies)
		{
			...
			UE_CLOGFMT(Dependencies.Num() > 0, LogGameFeatures, Verbose, "Found {Count} dependencies for {PluginName} : {Dependencies}", 
				Dependencies.Num(), StateProperties.PluginName,
				[&](){
					TArray<FString> DependencyNames;
					for (UGameFeaturePluginStateMachine* SM : Dependencies)
					{
						DependencyNames.Add(SM->StateProperties.PluginName);
					}
			...
			{
				ensureMsgf(bAllowAsyncLoading || !GameFeaturesSubsystem.AllowAsyncLoading(Dependency), 
					TEXT("FGameFeaturePluginState::AllowAsyncLoading is false for %s but true for dependency being waited on %s"), 
					*StateProperties.PluginName, *GameFeaturesSubsystem.GetPluginURL(Dependency));

				if (TransitionPolicy::ShouldWaitForDependencies() && 
					GameFeaturesSubsystem.IsInErrorState(Dependency) &&
					GameFeaturesSubsystem.IsErrorStateUnrecoverable(Dependency))
				{
					const EGameFeaturePluginState DependencyState = GameFeaturesSubsystem.GetCurrentState(Dependency);
					const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
					const FStringView DepShortUrl = GameFeaturesSubsystem.GetPluginIdentifier(Dependency).GetIdentifyingString();
					UE_LOGF(LogGameFeatures, Error, "Plugin (%.*ls) dependency (%.*ls) failed and is in state %ls",
					...
					{
						UE_LOGF(LogGameFeatures, Error, "Setting plugin (%.*ls) to be in unrecoverable error because dependency (%.*ls) is in unrecoverable error", 
							ShortUrl.Len(), ShortUrl.GetData(),
							DepShortUrl.Len(), DepShortUrl.GetData());
						GameFeaturesSubsystem.SetUnrecoverableError(CurrentMachine);
					}
					return;
				}
				else
		...
		for (FDepResultPair& Pair : RemainingDependencies)
		{
			...
			{
				const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
				const FStringView DepShortUrl = GameFeaturesSubsystem.GetPluginIdentifier(RemainingDependency).GetIdentifyingString();
				UE_LOGF(LogGameFeatures, Error, "Plugin (%.*ls) dependency (%.*ls) failed to transition with error %ls",
					ShortUrl.Len(), ShortUrl.GetData(),
					DepShortUrl.Len(), DepShortUrl.GetData(),
					*Pair.Value.GetError());
				StateStatus.SetTransitionError(TransitionPolicy::GetErrorState(), GetErrorResult(TEXT("Failed_Dependency_Transition")));
				if (GameFeaturesSubsystem.IsErrorStateUnrecoverable(RemainingDependency))
				{
					if (UGameFeaturePluginStateMachine* CurrentMachine = GameFeaturesSubsystem.FindGameFeaturePluginStateMachine(StateProperties.PluginIdentifier))
					{
						UE_LOGF(LogGameFeatures, Error, "Setting plugin (%.*ls) to be in unrecoverable error because dependency (%.*ls) is in unrecoverable error",
	...
	void TransitionDependency(UGameFeaturePluginStateMachine* Dependency)
	{
		...
		{
			GameFeaturesSubsystem.ExcludeFromBatchProcessing(Dependency);
		}
		
		if (TransitionPolicy::ShouldWaitForDependencies())
		{
			bSetDestination = GameFeaturesSubsystem.SetDestination(Dependency, TransitionPolicy::GetDependencyStateRange(),
		...
		if (!bSetDestination)
		{
			const bool bCancelPending = GameFeaturesSubsystem.TryCancel(Dependency,
	...
	void OnDependencyTransitionCanceled(UGameFeaturePluginStateMachine* Dependency)
	{
		...
		if (GameFeaturesSubsystem.GetCurrentState(Dependency) == EGameFeaturePluginState::Terminal)
		{
			// Inherit dep protocol options if possible
			FGameFeatureProtocolOptions DepProtocolOptions;
			EGameFeaturePluginProtocol DepProtocol = GameFeaturesSubsystem.GetPluginIdentifier(Dependency).GetPluginProtocol();
			...
			UGameFeaturePluginStateMachine* NewMachine = UGameFeaturesSubsystem::Get().FindOrCreateGameFeaturePluginStateMachine(GameFeaturesSubsystem.GetPluginURL(Dependency), DepProtocolOptions);
			checkf(NewMachine != Dependency, TEXT("Game Feature Plugin %s should have already been removed from subsystem!"), *GameFeaturesSubsystem.GetPluginURL(Dependency));
			...
			GameFeaturesSubsystem.RemovePendingTransitionCallback(Dependency, this);
			GameFeaturesSubsystem.RemovePendingCancelCallback(Dependency, this);
		...
		if (Result.HasError())
		{
			if (Result.GetError() == UE::GameFeatures::CanceledResult.GetError())
			{
				UE_LOGFMT(LogGameFeatures, Warning, "Dependency {Dep} failed to transition because it was cancelled by another request {Error}",
					("Dep", GameFeaturesSubsystem.GetPluginIdentifier(Dependency).GetIdentifyingString()),
	...
	void ClearDependencies()
	{
		if (RemainingDependencies.Num() > 0)
		{
			...
			for (FDepResultPair& Pair : RemainingDependencies)
			{
				...
				if (RemainingDependency)
				{
					GameFeaturesSubsystem.RemovePendingTransitionCallback(RemainingDependency, this);
					GameFeaturesSubsystem.RemovePendingCancelCallback(RemainingDependency, this);
				}
			}
...
struct FGameFeaturePluginState_UnknownStatus : public FDestinationGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::UnknownStatus] == EGameFeaturePluginState::Terminal);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::UnknownStatus] == EGameFeaturePluginState::CheckingStatus);
};
...
{
	using FErrorGameFeaturePluginState::FErrorGameFeaturePluginState;

	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorCheckingStatus] == EGameFeaturePluginState::Terminal);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorCheckingStatus] == EGameFeaturePluginState::CheckingStatus);
};
...
struct FGameFeaturePluginState_ErrorUnavailable : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorUnavailable] == EGameFeaturePluginState::Terminal);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorUnavailable] == EGameFeaturePluginState::CheckingStatus);
};
...
struct FGameFeaturePluginState_StatusKnown : public FDestinationGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::StatusKnown] == EGameFeaturePluginState::Uninstalling);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::StatusKnown] == EGameFeaturePluginState::Downloading);
};

struct FGameFeaturePluginState_ErrorManagingData : public FErrorGameFeaturePluginState
{
	using FErrorGameFeaturePluginState::FErrorGameFeaturePluginState;
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorManagingData] == EGameFeaturePluginState::Releasing);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorManagingData] == EGameFeaturePluginState::Downloading);
};
...
struct FGameFeaturePluginState_ErrorUninstalling : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorUninstalling] == EGameFeaturePluginState::Uninstalling);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorUninstalling] == EGameFeaturePluginState::StatusKnown);
};
...
struct FBaseDataReleaseGameFeaturePluginState : public FGameFeaturePluginState
{
	...
	void BeginRemoveRequest()
	{
		...
		TValueOrError<FInstallBundleReleaseRequestInfo, EInstallBundleResult> MaybeRequestInfo = BundleManager->RequestReleaseContent(
			InstallBundlesToRelease, ReleaseFlags, TArrayView<const FName>(), ELogVerbosity::Verbose);
...
struct FGameFeaturePluginState_Uninstalled : public FDestinationGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Uninstalled] == EGameFeaturePluginState::Terminal);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Uninstalled] == EGameFeaturePluginState::CheckingStatus);
};

struct FGameFeaturePluginState_Uninstalling : public FBaseDataReleaseGameFeaturePluginState
{
	using FBaseDataReleaseGameFeaturePluginState::FBaseDataReleaseGameFeaturePluginState;
...
struct FGameFeaturePluginState_Downloading : public FGameFeaturePluginState
{
	...
	void InstallIoStoreOnDemandContent()
	{
...
#if !UE_BUILD_SHIPPING
		static bool bBreakOnInstallBundlesWithAssetDependencies = true;
		if (bBreakOnInstallBundlesWithAssetDependencies && MetaData.InstallBundlesWithAssetDependencies.IsEmpty())
		{
			static int i = 0;
			++i;
		}
		...
		for (const FName InstallBundle : MetaData.InstallBundlesWithAssetDependencies)
		{
			...
			if (Options.bDoNotDownload && !UE::GameFeatures::CVarForceGFPIADDownload.GetValueOnGameThread())
			{
			...
			InstallArgs.DebugName = StateProperties.PluginName;
	...
	bool OnUpdateProgress(float dts)
	{
		if (ProgressTracker)
		{
			...
			const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
			UE_LOGF(LogGameFeatures, VeryVerbose, "Download Progress: %f for PluginURL(%.*ls)", Progress, ShortUrl.Len(), ShortUrl.GetData());
		}
	...
	virtual void BeginState() override
	{
		...
		if ((Options.bDoNotDownload && !UE::GameFeatures::CVarForceGFPIADDownload.GetValueOnGameThread()) && !bAllUpToDate)
		{
	...
	EInstallBundleRequestFlags GetRequestFlags() const
	{
		...
		InstallFlags |= EInstallBundleRequestFlags::ExplicitUpdateList; // avoid chasing depends for the install bundles
		return InstallFlags;
	}
...
struct FGameFeaturePluginState_Installed : public FDestinationGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Installed] == EGameFeaturePluginState::Mounting);
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Installed] == EGameFeaturePluginState::Releasing);
};
...
struct FGameFeaturePluginState_ErrorMounting : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorMounting] == EGameFeaturePluginState::Unmounting);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorMounting] == EGameFeaturePluginState::Mounting);
};
...
struct FGameFeaturePluginState_ErrorWaitingForDependencies : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorWaitingForDependencies] == EGameFeaturePluginState::Unmounting);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorWaitingForDependencies] == EGameFeaturePluginState::WaitingForDependencies);
...
struct FGameFeaturePluginState_ErrorRegistering : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorRegistering] == EGameFeaturePluginState::Unregistering);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorRegistering] == EGameFeaturePluginState::Registering);
};
...
struct FGameFeaturePluginState_Unmounting : public FGameFeaturePluginState
{
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		...
		static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Unmounting] == EGameFeaturePluginState::Installed);
		static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Unmounting] == EGameFeaturePluginState::Installed);
...
struct FGameFeaturePluginState_Mounting : public FGameFeaturePluginState
{
	...
	void OnPakFileMounted(const IPakFile& PakFile)
	{
		...
		{
			const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
			UE_LOGF(LogGameFeatures, Display, "Mounted Pak File for (%.*ls) with following files:", ShortUrl.Len(), ShortUrl.GetData());
			TArray<FString> OutFileList;
			Pak->GetPrunedFilenames(OutFileList);
			for (const FString& FileName : OutFileList)
			{
				UE_LOGF(LogGameFeatures, Display, "(%ls)", *FileName);
			}
		}
	}
	...
	void OnPostMountPauserCompleted(FStringView InPauserTag)
	{
		...
		UE_LOGF(LogGameFeatures, Display, "Post-mount of %ls resumed by %.*ls", *StateProperties.PluginName, InPauserTag.Len(), InPauserTag.GetData());
	...
	virtual void BeginState() override
	{
		...
		InstallFlags |= EInstallBundleRequestFlags::ExplicitUpdateList; // skip over collecting all the depends here, as we do this in CheckingStatus state
	...
	void UpdateState_LoadAssetRegistry()
	{
		...
		if (!UseAsyncLoading())
		{
			...
			if (AssetRegistry.ActiveMountsRegisterAndLoadFilePathSynchronous(PluginAssetRegistry))
			{
		...
		{
			// ActiveMountsRegisterAndLoadFilePath does not support bForceSyncAssetRegistryAppend; we have to load the AssetRegistry file
			// ourselves into an FAssetRegistryState to be able to append it on the game thread.
			UE::Tasks::Launch(UE_SOURCE_LOCATION, [this, PluginAssetRegistry=MoveTemp(PluginAssetRegistry), RefreshPackageLocalizationCacheForPlugin]
			{
				TSharedPtr<FAssetRegistryState> PluginAssetRegistryState = MakeShared<FAssetRegistryState>();
				bool bSuccess = FAssetRegistryState::LoadFromDisk(*PluginAssetRegistry, FAssetRegistryLoadOptions(),
					*PluginAssetRegistryState);

				ExecuteOnGameThread(UE_SOURCE_LOCATION, [this, PluginAssetRegistryState, bSuccess, RefreshPackageLocalizationCacheForPlugin]
				{
		...
		{
			IAssetRegistry& AssetRegistry = UAssetManager::Get().GetAssetRegistry();
			AssetRegistry.ActiveMountsRegisterAndLoadFilePathAsync(PluginAssetRegistry,
				[this, RefreshPackageLocalizationCacheForPlugin](bool bSuccess)
				{
					if (bSuccess)
					{
						RefreshPackageLocalizationCacheForPlugin();
	...
	void TransitionOut(FGameFeaturePluginStateStatus& StateStatus)
	{
		...
		{
			static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Mounting] == EGameFeaturePluginState::WaitingForDependencies);
			static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Mounting] == EGameFeaturePluginState::WaitingForDependencies);
...
{
	using Super = FTransitionDependenciesGameFeaturePluginState<FWaitingForDependenciesTransitionPolicy>;
	using Super::Super;
};
...
struct FGameFeaturePluginState_AssetDependencyStreamOut : public FGameFeaturePluginState
{
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::AssetDependencyStreamOut] == EGameFeaturePluginState::Unmounting);
		static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::AssetDependencyStreamOut] == EGameFeaturePluginState::Unmounting);
...
struct FGameFeaturePluginState_ErrorAssetDependencyStreaming : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorAssetDependencyStreaming] == EGameFeaturePluginState::AssetDependencyStreamOut);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorAssetDependencyStreaming] == EGameFeaturePluginState::AssetDependencyStreaming);
};
...
struct FGameFeaturePluginState_AssetDependencyStreaming : public FGameFeaturePluginState
{
	...
	virtual void BeginState() override
	{
		Cleanup();
#if !UE_BUILD_SHIPPING
		if (UE::GameFeatures::ShouldDebugGFP(StateProperties.PluginName))
		{
			static int32 I = 0; 
			++I;
		}
		...
		for (int i = 0; const FName InstallBundle : MetaData.InstallBundlesWithAssetDependencies)
		{
			...
			if (InstallMode == EStreamingAssetInstallMode::GfpRequiredOnly)
			{
				...
				InstallArgs.TagSets.Emplace(StateProperties.PluginName);
			}
			InstallArgs.Options |= UE::IoStore::EOnDemandInstallOptions::InstallSoftReferences;
			InstallArgs.Options |= UE::IoStore::EOnDemandInstallOptions::CallbackOnGameThread;
			if (Options.bDoNotDownload && !UE::GameFeatures::CVarForceGFPIADDownload.GetValueOnGameThread())
			...
			IoStoreOnDemandContext->InstallRequests.Add(IoStore->Install(MoveTemp(InstallArgs),
				// On Complete
				[this, LambdaOnDemandContext = IoStoreOnDemandContext, DebugName = InstallArgs.DebugName](const UE::IoStore::FOnDemandInstallResult& OnDemandInstallResult)
				{
					if (!LambdaOnDemandContext->bStateValid)
					{
						// Owning state got cleaned up, bail
					...
					if (!OnDemandInstallResult.IsOk() && !Result.HasError())
					{
						...
						UE_LOGFMT(LogGameFeatures, Error, "Failed to install request {DebugName} due to error {Error}",
							("DebugName", DebugName),
							("Error", Result.GetError()));
						TryCancelState();
					}
				...
				[this, LambdaOnDemandContext = IoStoreOnDemandContext, InstallBundle](const UE::IoStore::FOnDemandInstallProgress& Progress)
				{
					...
					StateProperties.StateProgress = OverallProgress;
				}));
		}
	}
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		...
		static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::AssetDependencyStreaming] == EGameFeaturePluginState::Registering);
		static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::AssetDependencyStreaming] == EGameFeaturePluginState::Registering);
...
struct FGameFeaturePluginState_Unregistering : public FGameFeaturePluginState
{
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		if (bHasUnloaded)
		{
			static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Unregistering] == EGameFeaturePluginState::AssetDependencyStreamOut);
			static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Unregistering] == EGameFeaturePluginState::AssetDependencyStreamOut);
		...
		else
		{
			...
			UE::GameFeatures::ScheduleUnloadPluginAssets(StateProperties.PluginName, StateProperties);
		}
#else
		bHasUnloaded = true;
		UpdateStateMachineDeferred();
...
struct FGameFeaturePluginState_Registering : public FGameFeaturePluginState
{
	...
	void TryAsyncLoadGameFeatureData(int32 Attempt = 0)
	{
		...
		{
			GFP_STATE_LOG(VeryVerbose, "Ran out of game feature paths to try");
		...
		GFP_STATE_LOG(VeryVerbose, "Trying to load game feature data {GameFeatureData}", GameFeatureDataPaths[Attempt]);
		...
		{
			GFP_STATE_LOG(VeryVerbose, "Waiting for load of game feature data {GameFeatureData}", GameFeatureDataPaths[Attempt]);
			...
			GameFeatureDataHandle->BindCancelDelegate(FStreamableDelegateWithHandle::CreateWeakLambda(CurrentMachine, [this](const TSharedPtr<FStreamableHandle>& Handle)
			{
				...
				{
					GFP_STATE_LOG(VeryVerbose, "Ignoring cancel of game feature data load as we are no longer in a relevant state");
					// We're no longer in a state where we want to process this callback (maybe we already went through EndState)
					return;
				}

				const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
				GFP_STATE_LOG(Error, "Game Feature Data loading was cancelled for URL {ShortURL}", ShortUrl);
			...
			GameFeatureDataHandle->BindCompleteDelegate(FStreamableDelegateWithHandle::CreateWeakLambda(CurrentMachine, [this, Attempt](const TSharedPtr<FStreamableHandle>& Handle)
			{
				if (GameFeatureDataHandle != Handle)
				{
					GFP_STATE_LOG(VeryVerbose, "Ignoring completion of game feature data load as we are no longer in a relevant state");
					// We're no longer in a state where we want to process this callback (maybe we already went through EndState)
					return;
				}
				...
				if (StateProperties.GameFeatureData)
				{
					GFP_STATE_LOG(VeryVerbose, "Loaded GameFeatureData {GameFeatureData}", FSoftObjectPath(StateProperties.GameFeatureData));
					LoadGFDState = ELoadGFDState::Success;
				}
				else
				{
					// This will set LoadGFDState
					HandleMissingGameFeatureData();
				}
	...
	UGameFeatureData* CreateTransientGameFeatureData()
	{
		return NewObject<UGameFeatureData>(GetTransientPackage(), FName(*StateProperties.PluginName), RF_Transient);
	}

	void HandleMissingGameFeatureData()
	{
		const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
		if (UGameFeaturesSubsystem::Get().GetPolicy().ShouldAllowMissingGameFeatureData(StateProperties.PluginName))
		{
			UE_LOGF(LogGameFeatures, Log, "Game Feature Data package not found for URL %.*ls. Creating a transient GameFeatureData as a placeholder.", ShortUrl.Len(), ShortUrl.GetData());
			StateProperties.GameFeatureData = CreateTransientGameFeatureData();
		...
		else
		{
			...
			{
				if (GameFeatureDataHandle->HasError())
				{
					const TOptional<UE::UnifiedError::FError>& Error = GameFeatureDataHandle->GetError();
					UE_LOGF(LogGameFeatures, Error, "Game Feature Data loading failed with error [%ls] for URL %.*ls", *LexToString(*Error), ShortUrl.Len(), ShortUrl.GetData());
				}
				else
				{
					UE_LOGF(LogGameFeatures, Error, "Game Feature Data loading failed without an error for URL %.*ls", ShortUrl.Len(), ShortUrl.GetData());
				}
			}
			else
			{
				UE_LOGF(LogGameFeatures, Error, "Game Feature Data package not found for URL %.*ls", ShortUrl.Len(), ShortUrl.GetData());
			}
	...
	virtual void BeginState() override
	{
		...
		{
			GFP_STATE_LOG(VeryVerbose, "Creating transient GameFeatureData for non-content plugin");
			StateProperties.GameFeatureData = CreateTransientGameFeatureData();
			LoadGFDState = ELoadGFDState::Success;
			return;
		}
		...
		else
		{
			...
			{
				GFP_STATE_LOG(VeryVerbose, "Synchronously loading GameFeatureData {Path}", Path);
			...
			{
				GFP_STATE_LOG(VeryVerbose, "Failed to synchronously load GameFeatureData");
				// This will set LoadGFDState
				HandleMissingGameFeatureData();
			}
		}
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		...
		if (StateProperties.GameFeatureData)
		{
			...
			static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Registering] == EGameFeaturePluginState::Registered);
			static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Registering] == EGameFeaturePluginState::Registered);
			StateStatus.SetTransition(EGameFeaturePluginState::Registered);

			check(StateProperties.AddedPrimaryAssetTypes.Num() == 0);
			GameFeaturesSubsystem.AddGameFeatureToAssetManager(StateProperties.GameFeatureData, StateProperties.PluginName, StateProperties.AddedPrimaryAssetTypes);

			GameFeaturesSubsystem.OnGameFeatureRegistering(StateProperties.GameFeatureData, StateProperties.PluginName, StateProperties.PluginIdentifier);
		}
		else
		{
			...
			{
				const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
				UE_LOGF(LogGameFeatures, Error, "Setting %.*ls to be in unrecoverable error as GameFeatureData is missing", ShortUrl.Len(), ShortUrl.GetData());
				GameFeaturesSubsystem.SetUnrecoverableError(CurrentMachine);
			}
		}
	}
...
struct FGameFeaturePluginState_Registered : public FDestinationGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Registered] == EGameFeaturePluginState::Loading);
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Registered] == EGameFeaturePluginState::Unregistering);
};
...
struct FGameFeaturePluginState_ErrorLoading : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorLoading] == EGameFeaturePluginState::Unloading);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorLoading] == EGameFeaturePluginState::Loading);
};
...
struct FGameFeaturePluginState_Unloading : public FGameFeaturePluginState
{
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		...
		static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Unloading] == EGameFeaturePluginState::Registered);
		static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Unloading] == EGameFeaturePluginState::Registered);
		StateStatus.SetTransition(EGameFeaturePluginState::Registered);
	}
...
struct FGameFeaturePluginState_Loading : public FGameFeaturePluginState
{
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		...
		static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Loading] == EGameFeaturePluginState::Loaded);
		static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Loading] == EGameFeaturePluginState::Loaded);
	...
	void LoadGameFeatureBundles(const UGameFeatureData* GameFeatureToLoad)
	{
		...
		if (AssetIdsToLoad.Num() > 0)
		{
			...
			LoadParams.OnCancel = FStreamableDelegateWithHandle::CreateWeakLambda(CurrentMachine, [this](const TSharedPtr<FStreamableHandle>& Handle)
			{
				...
				const FStringView ShortUrl = StateProperties.PluginIdentifier.GetIdentifyingString();
				UE_LOGF(LogGameFeatures, Error, "Game Feature preloading was cancelled for URL %.*ls", ShortUrl.Len(), ShortUrl.GetData());
				UpdateStateMachineDeferred();
			});
			...
			FString PluginName;
			GameFeatureToLoad->GetPluginName(PluginName);
			BundleHandle = UAssetManager::Get().LoadPrimaryAssets(AssetIdsToLoad, Policy.GetPreloadBundleStateForPlugin(PluginName), MoveTemp(LoadParams));
		}
		else
		{
			BundleHandle.Reset();
...
struct FGameFeaturePluginState_Loaded : public FDestinationGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Loaded] == EGameFeaturePluginState::ActivatingDependencies);
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Loaded] == EGameFeaturePluginState::Unloading);
};
...
struct FGameFeaturePluginState_ErrorDeactivatingDependencies : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorDeactivatingDependencies] == EGameFeaturePluginState::DeactivatingDependencies);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorDeactivatingDependencies] == EGameFeaturePluginState::DeactivatingDependencies);
};
...
{
	using Super = FTransitionDependenciesGameFeaturePluginState<FDeactivatingDependenciesTransitionPolicy>;
	using Super::Super;
};
...
struct FGameFeaturePluginState_Deactivating : public FGameFeaturePluginState
{
	...
	virtual void BeginState() override
	{
		...
		if (bUseNewDynamicLayers)
		{
			...
			{
				// note that we are unloading the GFP so we don't call LoadObject iuuselessly
				ChangeTracker->bIsUnloadingPlugin = true;
				FConfigCacheIni::RemoveTagFromAllBranches(Tag, ChangeTracker);
				IConsoleManager::Get().UnsetAllConsoleVariablesWithTag(Tag);
				UE::DynamicConfig::RemovePluginHotfixesFromDynamicBranch.Broadcast(Tag);
			});
		}
	}
	...
	void OnPauserCompleted(FStringView InPauserTag)
	{
		...
		UE_LOGF(LogGameFeatures, Display, "Deactivation of %ls resumed by %.*ls", *StateProperties.PluginName, InPauserTag.Len(), InPauserTag.GetData());
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		if (bHasUnloaded)
		{
			...
			static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Deactivating] == EGameFeaturePluginState::Loaded);
			static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Deactivating] == EGameFeaturePluginState::Loaded);
			StateStatus.SetTransition(EGameFeaturePluginState::Loaded);
			return;
		}
		...
		{
			UE_LOGF(LogGameFeatures, Log, "Game feature %ls deactivation paused until %d observer tasks complete their deactivation", *GetPathNameSafe(StateProperties.GameFeatureData), NumExpectedPausers - NumObservedPausers);
		}
	}
};
...
struct FGameFeaturePluginState_ErrorActivatingDependencies : public FErrorGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::ErrorActivatingDependencies] == EGameFeaturePluginState::Unloading);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::ErrorActivatingDependencies] == EGameFeaturePluginState::ActivatingDependencies);
};
...
{
	using Super = FTransitionDependenciesGameFeaturePluginState<FActivatingDependenciesTransitionPolicy>;
	using Super::Super;
};
...
struct FGameFeaturePluginState_Activating : public FGameFeaturePluginState
{
	...
	virtual void UpdateState(FGameFeaturePluginStateStatus& StateStatus) override
	{
		...
		static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Activating] == EGameFeaturePluginState::Active);
		static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Activating] == EGameFeaturePluginState::Active);
	...
	static void BatchProcess(TConstArrayView<UGameFeaturePluginStateMachine*> GFPs)
	{
		...
		{
			PluginInstalledFilenames.Emplace(GFPSubSys.GetProperties(GFPSM).PluginInstalledFilename);
		}
...
struct FGameFeaturePluginState_Active : public FDestinationGameFeaturePluginState
{
	...
	static_assert(UE::GameFeatures::NextStateTowardsTerminal[EGameFeaturePluginState::Active] == EGameFeaturePluginState::DeactivatingDependencies);
	static_assert(UE::GameFeatures::NextStateTowardsActive[EGameFeaturePluginState::Active] == EGameFeaturePluginState::Active);
};
...
FDestinationGameFeaturePluginState* AsDestinationState(FGameFeaturePluginState* InState)
{
	if (InState)
	{
		return InState->AsDestinationState();
	}
	return nullptr;
}

UE::GameFeatures::FScopedPauseStateMachineExecution::FScopedPauseStateMachineExecution(UE::GameFeatures::FStateMachineExecutor* InExecutor)
: Executor(InExecutor)
{
	if (Executor)
	{
		bOldPaused = Executor->PauseExecution();	
	}
}

UE::GameFeatures::FScopedPauseStateMachineExecution::~FScopedPauseStateMachineExecution()
{
	if (Executor)
	{
		Executor->TryResumeExecution(bOldPaused);
	}
}

UE::GameFeatures::FStateMachineExecutor::FStateMachineExecutor(UGameFeaturesSubsystem* InOwningSubsystem)
	: OwningSubsystem(InOwningSubsystem)
{
}
UE::GameFeatures::FStateMachineExecutor::~FStateMachineExecutor()
{
}

bool UE::GameFeatures::FStateMachineExecutor::PauseExecution()
{
	bool OldValue = bPauseImmediateExecution;
	bPauseImmediateExecution = true;
	return OldValue;
}

void UE::GameFeatures::FStateMachineExecutor::TryResumeExecution(bool bStayPaused)
{
	bPauseImmediateExecution = bStayPaused;
	if (!bPauseImmediateExecution)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("FStateMachineExecutor::TryResumeExecution")
		// No time limit based on delta time here, just use the per-frame budget
		UpdateStateMachines(0.0f); 
	}
}

bool UE::GameFeatures::FStateMachineExecutor::UpdateStateMachines(float DeltaTime, int32 MaxIterations)
{
	const double StartTime = UE::GameFeatures::bRealtimeMode ? FPlatformTime::Seconds() : 0.0;
	...
	if (UE::GameFeatures::ExecutionOptions.bGrouped)
	{
		// New execution style, group by state and only allow state machines to make one transition at a time, hoisting the repetition to an outer loop here 
		// Pause execution of re-entrant calls but do not use TryResumeExecution which will try and execute anything queued immediately.
		TGuardValue<bool> GuardPause(bPauseImmediateExecution, true);
		// Separate queues of GFPs to update by current state
		TArray<UGameFeaturePluginStateMachine*> Queues[EGameFeaturePluginState::MAX];
		for (int32 Iteration = 0; Iteration < MaxIterations; ++Iteration)
		{ 
			if (UE::GameFeatures::ExecutionOptions.bShuffle)
			{
				Algo::RandomShuffle(OrderedUpdateQueue);
			}
			else if (UE::GameFeatures::ExecutionOptions.bReverse)
			{
				Algo::Reverse(OrderedUpdateQueue);
			}

			// On every iteration, gather newly queued updates per state so that batches are as large as possible
			for (FStateMachineId Id : OrderedUpdateQueue)
			{
				UGameFeaturePluginStateMachine* StateMachine = OwningSubsystem->GameFeaturePluginStateMachines.FindRef(Id);
				if (ensureMsgf(StateMachine, TEXT("State machine %lld wasn't present in subsystem when trying to update it"), Id))
				{
					StateMachine->StateProperties.bQueuedForUpdate = false;
					Queues[StateMachine->StateProperties.CurrentState].Add(StateMachine);
				}
			}
			OrderedUpdateQueue.Reset();

			// Find the earliest state with work to do
			EGameFeaturePluginState State = EGameFeaturePluginState::UnknownStatus;
			for (EGameFeaturePluginState It : TEnumRange<EGameFeaturePluginState>())
			{
				if (Queues[It].Num() > 0)
				{
					State = It;
			...
			if (Queues[State].IsEmpty())
			{
				// No more work to do
				break;
			}

			bool bTimeLimitReached = UpdateStateMachinesGrouped(StartTime, DeltaTime, State, MakeArrayView(Queues[State]));
			Queues[State].Reset();
			if (bTimeLimitReached)
			{
				OrderedUpdateQueue.Reserve(OrderedUpdateQueue.Num() + Algo::TransformAccumulate(Queues, UE_PROJECTION(GetNum), 0));
				// UpdateStateMachinesGrouped will already have requeued Queues[State], all other queues need to be requeued
				// They can be queued at the end because we'll sort again next time
				for (EGameFeaturePluginState It : TEnumRange<EGameFeaturePluginState>())
				{
					Algo::Transform(Queues[It], OrderedUpdateQueue, [](UGameFeaturePluginStateMachine* StateMachine)
					{
						StateMachine->StateProperties.bQueuedForUpdate = true;
						return StateMachine->StateProperties.Id;
					});
				}
				break;
			}
	...
	else 
	{
		...
		// Closer to original execution style, update all deferred state machines and allow them to make as many transitions as they want, and update other state machines immediately
		TArray<UGameFeaturePluginStateMachine*> LocalUpdateQueue;
		LocalUpdateQueue.Reserve(OrderedUpdateQueue.Num());
		for (const FStateMachineId& Id : OrderedUpdateQueue)
		{
			UGameFeaturePluginStateMachine* StateMachine = OwningSubsystem->GameFeaturePluginStateMachines.FindRef(Id);
			if (ensureMsgf(StateMachine, TEXT("State machine %lld wasn't present in subsystem when trying to update it"), Id))
			{
				LocalUpdateQueue.Add(StateMachine);
				StateMachine->StateProperties.bQueuedForUpdate = false;
			}
		}
		OrderedUpdateQueue.Reset();

		UpdateStateMachinesRealtime(StartTime, DeltaTime, LocalUpdateQueue);
	}

	// Return if this function should be called again next frame
	return OrderedUpdateQueue.Num() > 0;
}

bool UE::GameFeatures::FStateMachineExecutor::UpdateStateMachinesRealtime(double StartTime, float DeltaTime, TConstArrayView<UGameFeaturePluginStateMachine*> LocalUpdateQueue)
{
	if (bRealtimeMode)
	{
		for (int32 i=0; i < LocalUpdateQueue.Num(); ++i)
		{
			UGameFeaturePluginStateMachine* StateMachine = LocalUpdateQueue[i];
			OwningSubsystem->UpdateStateMachine(StateMachine, {});

			const double ElapsedTime = FPlatformTime::Seconds() - StartTime;
			if ((ElapsedTime > RealtimeAllottedTime) || ((DeltaTime + ElapsedTime) > RealtimeMaxFrameTime))
			{
				// Put anything we didn't finish back at the head of the queue
				//-1 because we updated the machine at index i
				TConstArrayView<UGameFeaturePluginStateMachine*> Remaining = LocalUpdateQueue.RightChop(i+1);
				OrderedUpdateQueue.InsertZeroed(0, Remaining.Num());
				for (int32 j=0; j < Remaining.Num(); ++j)
				{
					UGameFeaturePluginStateMachine* StateMachineToRequeue = Remaining[j];
					StateMachineToRequeue->StateProperties.bQueuedForUpdate = true;
					OrderedUpdateQueue[j] = StateMachineToRequeue->StateProperties.Id;
				}
	...
	{ 
		for (UGameFeaturePluginStateMachine* StateMachine : LocalUpdateQueue)
		{
			OwningSubsystem->UpdateStateMachine(StateMachine, {});
		}
	}
...
bool UE::GameFeatures::FStateMachineExecutor::UpdateStateMachinesGrouped(double StartTime, float DeltaTime, EGameFeaturePluginState State, TConstArrayView<UGameFeaturePluginStateMachine*> StateMachines)
{
	switch (State)
	{
#define GFP_STATE_OP(STATE_NAME, TXT) \
	case EGameFeaturePluginState::STATE_NAME: \
		{ \
			TRACE_CPUPROFILER_EVENT_SCOPE_STR("UpdateStateMachinesGrouped<" #STATE_NAME ">"); \
			return UpdateStateMachinesGrouped<EGameFeaturePluginState::STATE_NAME>(StartTime, DeltaTime, StateMachines);\
		} \
		break;
	GAME_FEATURE_PLUGIN_STATE_LIST(GFP_STATE_OP)
#undef GFP_STATE_OP
	}
	return false;
}
...
bool UE::GameFeatures::FStateMachineExecutor::UpdateStateMachinesGrouped(double StartTime, float DeltaTime, TConstArrayView<UGameFeaturePluginStateMachine*> StateMachines)
{
	return UpdateStateMachinesRealtime(StartTime, DeltaTime, StateMachines);
}

void UE::GameFeatures::FStateMachineExecutor::QueueUpdateStateMachine(FGameFeaturePluginStateMachineProperties& InProperties)
{
	if (!InProperties.bQueuedForUpdate)
	{ 
		ensureMsgf(OwningSubsystem->GameFeaturePluginStateMachines.FindRef(InProperties.Id), 
			TEXT("State machine %lld wasn't present in subsystem when trying to update it"), InProperties.Id);
		OwningSubsystem->EnableTick();
		OrderedUpdateQueue.Add(InProperties.Id);
		InProperties.bQueuedForUpdate = true;
	}
}

void UE::GameFeatures::FStateMachineExecutor::TryUpdateStateMachineImmediate(int64 Id)
{
	UGameFeaturePluginStateMachine* StateMachine = OwningSubsystem->GameFeaturePluginStateMachines.FindRef(Id);
	if (ensureMsgf(StateMachine, TEXT("State machine %lld wasn't present in subsystem when trying to update it immediately"), Id))
	{
		if (StateMachine->StateProperties.bQueuedForUpdate)
		{
			return;
		}
		else if (bPauseImmediateExecution)
		{
			QueueUpdateStateMachine(StateMachine->StateProperties);
		}
		else
		{
			TRACE_CPUPROFILER_EVENT_SCOPE_STR("FStateMachineExecutor Exectuting State Machine Immediately");
			OwningSubsystem->UpdateStateMachine(StateMachine);
		}
	}
}

void UE::GameFeatures::FStateMachineExecutor::TryUpdateStateMachineImmediate(FGameFeaturePluginStateMachineProperties& InProperties)
{
	if (InProperties.bQueuedForUpdate)
	{
		return;
	...
	{
		QueueUpdateStateMachine(InProperties);
	}
	else 
	{
		UGameFeaturePluginStateMachine* StateMachine = OwningSubsystem->GameFeaturePluginStateMachines.FindRef(InProperties.Id);
		if (ensureMsgf(StateMachine, TEXT("State machine %lld wasn't present in subsystem when trying to update it immediately"), InProperties.Id))
		{
			OwningSubsystem->UpdateStateMachine(StateMachine);
		}
	}
...
void UGameFeaturesSubsystem::InitStateMachine(
	UGameFeaturePluginStateMachine* This,
	int64 InId,
	FGameFeaturePluginIdentifier InPluginIdentifier,
	const FGameFeatureProtocolOptions& InProtocolOptions) 
{
	LLM_SCOPE_BYTAG(GFP);

	check(GetCurrentState(This) == EGameFeaturePluginState::Uninitialized);
	FString PluginName{InPluginIdentifier.GetPluginName()};

	FGameFeaturePluginRequestUpdateStateMachine UpdateCallback;
	if (!StateMachineExecutor)
	{
		UpdateCallback.BindWeakLambda(this, 
			[this, PluginName, WeakThis=MakeWeakObjectPtr(This)]()
			{
				if (UGameFeaturePluginStateMachine* This = WeakThis.Get())
				{
					UpdateStateMachine(This);
				}
				else
				{
					UE_LOGFMT(LogGameFeatures, Warning, "State machine pointer for {Plugin} became unexpectedly null.", PluginName);
				}
			});
	}

	constexpr EGameFeaturePluginState InitialState  = EGameFeaturePluginState::UnknownStatus;
	This->StateProperties = FGameFeaturePluginStateMachineProperties(
		InId,
		MoveTemp(InPluginIdentifier),
		InitialState,
		StateMachineExecutor.Get(),
		UpdateCallback);

	This->StateProperties.ProtocolOptions = InProtocolOptions;
	This->Executor = StateMachineExecutor.Get();

#define GAME_FEATURE_PLUGIN_STATE_MAKE_STATE(inEnum, inText) This->AllStates[EGameFeaturePluginState::inEnum] = MakeUnique<FGameFeaturePluginState_##inEnum>(EGameFeaturePluginState::inEnum, This->StateProperties);
	GAME_FEATURE_PLUGIN_STATE_LIST(GAME_FEATURE_PLUGIN_STATE_MAKE_STATE)
#undef GAME_FEATURE_PLUGIN_STATE_MAKE_STATE

	CheckAddBatchingRequestForCurrentState(This);
	This->AllStates[This->StateProperties.CurrentState]->BeginState();
}

bool UGameFeaturesSubsystem::SetDestination(UGameFeaturePluginStateMachine* This, FGameFeaturePluginStateRange InDestination, FGameFeatureStateTransitionComplete OnFeatureStateTransitionComplete, FDelegateHandle* OutCallbackHandle /*= nullptr*/)
{
	LLM_SCOPE_BYTAG(GFP);

	check(IsValidDestinationState(This, InDestination.MinState));
	check(IsValidDestinationState(This, InDestination.MaxState));
	...
	ON_SCOPE_EXIT
	{
		if (bDestinationChanged && GetPluginDebugStateEnabled(GetPluginURL(This)))
		{
			PLATFORM_BREAK();
		}
	};
	...
	else if (This->StateProperties.CurrentState == EGameFeaturePluginState::Terminal && !InDestination.Contains(EGameFeaturePluginState::Terminal))
	{
		// Can't transition away from terminal state
		return false;
	}

	// Limit destination to allowed maximum. If requested minimum is beyond that we fail, if requested maximum is beyond it we simply clamp.
	if (InDestination.MinState > This->StateProperties.MaximumState)
	{
		UE_LOGFMT(LogGameFeatures, Warning, "Ignoring request to change game feature destination to {DestinationRange} because maximum state is {MaximumState}",
			InDestination, This->StateProperties.MaximumState);
		return false;
	}

	if (InDestination.MaxState > This->StateProperties.MaximumState)
	{
		InDestination.MaxState = This->StateProperties.MaximumState;
		UE_LOGFMT(LogGameFeatures, Log, "Limited state transition for GFP {Name} to maximum state {MaximumState}", GetGameFeatureName(This), This->StateProperties.MaximumState);
	}
	...
	if (!IsRunning(This))
	{
		...
		FDestinationGameFeaturePluginState* CurrState = This->AllStates[This->StateProperties.CurrentState]->AsDestinationState();

		if (InDestination.Contains(This->StateProperties.CurrentState))
		{
			OnFeatureStateTransitionComplete.ExecuteIfBound(This, MakeValue());
		}
		else
		{
			if (This->StateProperties.CurrentState < InDestination)
			{
				FDestinationGameFeaturePluginState* MinDestState = This->AllStates[InDestination.MinState]->AsDestinationState();
			...
			else if (This->StateProperties.CurrentState > InDestination)
			{
				FDestinationGameFeaturePluginState* MaxDestState = This->AllStates[InDestination.MaxState]->AsDestinationState();
			...
			This->StateProperties.Destination = InDestination;
			This->StateProperties.UpdateStateMachineImmediate();
			bDestinationChanged = true;
		}

		bDestinationSet = true;
	}
	else if (TOptional<FGameFeaturePluginStateRange> NewDestination = This->StateProperties.Destination.Intersect(InDestination))
	{
		...
		if (This->StateProperties.CurrentState < This->StateProperties.Destination)
		{
			This->StateProperties.Destination = *NewDestination;

			if (InDestination.Contains(This->StateProperties.CurrentState))
			{
				// TODO: Consider deferring for executor 
				OnFeatureStateTransitionComplete.ExecuteIfBound(This, MakeValue());
			}
			else
			{
		...
		else if(This->StateProperties.CurrentState > This->StateProperties.Destination)
		{
			This->StateProperties.Destination = *NewDestination;
...
bool UGameFeaturesSubsystem::TryCancel(UGameFeaturePluginStateMachine* This, FGameFeatureStateTransitionCanceled OnFeatureStateTransitionCanceled, FDelegateHandle* OutCallbackHandle /*= nullptr*/) const
{
	if (!IsRunning(This))
	{
		return false;
	...
	This->StateProperties.bTryCancel = true;
	FDelegateHandle CallbackHandle = This->StateProperties.OnTransitionCanceled.Add(MoveTemp(OnFeatureStateTransitionCanceled));
	if(OutCallbackHandle)
	{
		*OutCallbackHandle = CallbackHandle;
	}

	const EGameFeaturePluginState CurrentState = GetCurrentState(This);
	This->AllStates[CurrentState]->TryCancelState();

	return true;
}

UE::GameFeatures::FResult UGameFeaturesSubsystem::TryUpdatePluginProtocolOptions(UGameFeaturePluginStateMachine* This, const FGameFeatureProtocolOptions& InOptions, bool& bOutDidUpdate) const
{
	bOutDidUpdate = false;

	if (This->StateProperties.ProtocolOptions == InOptions)
	{
		return MakeValue();
	}
	...
	const EGameFeaturePluginState CurrentState = GetCurrentState(This);
	UE::GameFeatures::FResult Result = This->AllStates[CurrentState]->TryUpdateProtocolOptions(InOptions);
	bOutDidUpdate = Result.HasValue();

	return Result;
}

void UGameFeaturesSubsystem::RemovePendingTransitionCallback(UGameFeaturePluginStateMachine* This, FDelegateHandle InHandle) const
{
	for (std::underlying_type<EGameFeaturePluginState>::type iState = 0;
		iState < EGameFeaturePluginState::MAX;
		++iState)
	{
		if (FDestinationGameFeaturePluginState* DestState = AsDestinationState(This->AllStates[iState].Get()))
		{
...
void UGameFeaturesSubsystem::RemovePendingTransitionCallback(UGameFeaturePluginStateMachine* This, FDelegateUserObject DelegateObject) const
{
	for (std::underlying_type<EGameFeaturePluginState>::type iState = 0;
		iState < EGameFeaturePluginState::MAX;
		++iState)
...
void UGameFeaturesSubsystem::RemovePendingCancelCallback(UGameFeaturePluginStateMachine* This, FDelegateHandle InHandle) const
{
	This->StateProperties.OnTransitionCanceled.Remove(InHandle);
}

void UGameFeaturesSubsystem::RemovePendingCancelCallback(UGameFeaturePluginStateMachine* This, FDelegateUserObject DelegateObject) const
{
	This->StateProperties.OnTransitionCanceled.RemoveAll(DelegateObject);
}

const FString& UGameFeaturesSubsystem::GetGameFeatureName(const UGameFeaturePluginStateMachine* This) const
{
	FString PluginFilename;
	if (!This->StateProperties.PluginName.IsEmpty())
	{
		return This->StateProperties.PluginName;
	}
	else
	{
		return This->StateProperties.PluginIdentifier.GetFullPluginURL();
	}
}

const FGameFeaturePluginIdentifier& UGameFeaturesSubsystem::GetPluginIdentifier(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.PluginIdentifier;
}

const FString& UGameFeaturesSubsystem::GetPluginURL(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.PluginIdentifier.GetFullPluginURL();
}

const FGameFeatureProtocolMetadata& UGameFeaturesSubsystem::GetProtocolMetadata(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.ProtocolMetadata;
}

const FGameFeatureProtocolOptions& UGameFeaturesSubsystem::GetProtocolOptions(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.ProtocolOptions;
}

FGameFeatureProtocolOptions UGameFeaturesSubsystem::RecycleProtocolOptions(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.RecycleProtocolOptions();
}

const FString& UGameFeaturesSubsystem::GetPluginName(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.PluginName;
}

bool UGameFeaturesSubsystem::GetPluginFilename(const UGameFeaturePluginStateMachine* This, FString& OutPluginFilename) const
{
	OutPluginFilename = This->StateProperties.PluginInstalledFilename;
	return !OutPluginFilename.IsEmpty();
}

EGameFeaturePluginState UGameFeaturesSubsystem::GetCurrentState(const UGameFeaturePluginStateMachine* This) const
{
	return GetCurrentStateInfo(This).State;
}

FGameFeaturePluginStateRange UGameFeaturesSubsystem::GetDestination(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.Destination;
}

FGameFeaturePluginStateInfo UGameFeaturesSubsystem::GetCurrentStateInfo(const UGameFeaturePluginStateMachine* This) const
{
	return FGameFeaturePluginStateInfo{ This->StateProperties.CurrentState, This->StateProperties.StateProgress };
}

bool UGameFeaturesSubsystem::IsRunning(const UGameFeaturePluginStateMachine* This) const
{
	return !This->StateProperties.Destination.Contains(This->StateProperties.CurrentState);
}

bool UGameFeaturesSubsystem::IsStatusKnown(const UGameFeaturePluginStateMachine* This) const
{
	return GetCurrentState(This) == EGameFeaturePluginState::ErrorUnavailable || 
		   GetCurrentState(This) == EGameFeaturePluginState::Uninstalling || 
		   GetCurrentState(This) == EGameFeaturePluginState::ErrorUninstalling ||
		   GetCurrentState(This) >= EGameFeaturePluginState::StatusKnown;
}

bool UGameFeaturesSubsystem::IsAvailable(const UGameFeaturePluginStateMachine* This) const
{
	ensure(IsStatusKnown(This));
	return GetCurrentState(This) >= EGameFeaturePluginState::StatusKnown;
}

bool UGameFeaturesSubsystem::IsInErrorState(const UGameFeaturePluginStateMachine* This) const
{
	return IsValidErrorState(This, GetCurrentState(This));
}

bool UGameFeaturesSubsystem::AllowAsyncLoading(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.AllowAsyncLoading();
}

bool UGameFeaturesSubsystem::HasAssetStreamingDependencies(const UGameFeaturePluginStateMachine* This) const
{
	ensure(IsStatusKnown(This));
	if (This->StateProperties.ProtocolMetadata.HasSubtype<FInstallBundlePluginProtocolMetaData>())
	{
		const FInstallBundlePluginProtocolMetaData& ProtocolData = This->StateProperties.ProtocolMetadata.GetSubtype<FInstallBundlePluginProtocolMetaData>();
		return !ProtocolData.InstallBundlesWithAssetDependencies.IsEmpty();
	}
	return false;
}

void UGameFeaturesSubsystem::SetWasLoadedAsBuiltIn(UGameFeaturePluginStateMachine* This)
{
	This->StateProperties.bWasLoadedAsBuiltInGameFeaturePlugin = true;
}

bool UGameFeaturesSubsystem::WasLoadedAsBuiltIn(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties.bWasLoadedAsBuiltInGameFeaturePlugin;
}

UGameFeatureData* UGameFeaturesSubsystem::GetGameFeatureDataForActivePlugin(UGameFeaturePluginStateMachine* This) const
{
	if (GetCurrentState(This) == EGameFeaturePluginState::Active)
	{
		return This->StateProperties.GameFeatureData;
	}

	return nullptr;
}

const UGameFeatureData* UGameFeaturesSubsystem::GetGameFeatureDataForActivePlugin(const UGameFeaturePluginStateMachine* This) const
{
	if (GetCurrentState(This) == EGameFeaturePluginState::Active)
	{
		return This->StateProperties.GameFeatureData;
...
UGameFeatureData* UGameFeaturesSubsystem::GetGameFeatureDataForRegisteredPlugin(UGameFeaturePluginStateMachine* This, bool bCheckForRegistering /*= false*/) const
{
	const EGameFeaturePluginState CurrentState = GetCurrentState(This);
...
const UGameFeatureData* UGameFeaturesSubsystem::GetGameFeatureDataForRegisteredPlugin(const UGameFeaturePluginStateMachine* This, bool bCheckForRegistering /*= false*/) const
{
	const EGameFeaturePluginState CurrentState = GetCurrentState(This);
...
const FGameFeaturePluginStateMachineProperties& UGameFeaturesSubsystem::GetProperties(const UGameFeaturePluginStateMachine* This) const
{
	return This->StateProperties;
}

bool UGameFeaturesSubsystem::IsErrorStateUnrecoverable(const UGameFeaturePluginStateMachine* This) const
{
	return This->bIsInUnrecoverableError;
}

void UGameFeaturesSubsystem::SetUnrecoverableError(UGameFeaturePluginStateMachine* This) const
{
	This->bIsInUnrecoverableError = true;
}

bool UGameFeaturesSubsystem::IsValidTransitionState(const UGameFeaturePluginStateMachine* This, EGameFeaturePluginState InState) const
{
	check(InState != EGameFeaturePluginState::MAX);
	return This->AllStates[InState]->GetStateType() == EGameFeaturePluginStateType::Transition;
}

bool UGameFeaturesSubsystem::IsValidDestinationState(const UGameFeaturePluginStateMachine* This, EGameFeaturePluginState InDestinationState) const
{
	check(InDestinationState != EGameFeaturePluginState::MAX);
	return This->AllStates[InDestinationState]->GetStateType() == EGameFeaturePluginStateType::Destination;
}

bool UGameFeaturesSubsystem::IsValidErrorState(const UGameFeaturePluginStateMachine* This, EGameFeaturePluginState InDestinationState) const
{
	check(InDestinationState != EGameFeaturePluginState::MAX);
	return This->AllStates[InDestinationState]->GetStateType() == EGameFeaturePluginStateType::Error;
}
...
UE_TRACE_EVENT_BEGIN(Cpu, GFP_UpdateStateMachine_BeginState, NoSync)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, State)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Cpu, GFP_UpdateStateMachine_UpdateState, NoSync)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, State)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN(Cpu, GFP_UpdateStateMachine_EndState, NoSync)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, State)
UE_TRACE_EVENT_END()

void UGameFeaturesSubsystem::UpdateStateMachine(UGameFeaturePluginStateMachine* This, TOptional<int32> InMaxTransitions) 
{
	LLM_SCOPE_BYTAG(GFP);

	const EGameFeaturePluginState InitialState = GetCurrentState(This);
	EGameFeaturePluginState CurrentState = InitialState;
	if (This->bInUpdateStateMachine)
	{
		UE_LOGF(LogGameFeatures, Verbose, "Game feature state machine skipping update for %ls in ::UpdateStateMachine. Current State: %ls", *GetGameFeatureName(This), *UE::GameFeatures::ToString(CurrentState));
		return;
	}

	UE_TRACE_LOG_SCOPED_T(Cpu, GFP_UpdateStateMachine, CpuChannel)
		<< GFP_UpdateStateMachine.PluginName(*GetGameFeatureName(This));

	TOptional<TGuardValue<bool>> ScopeGuard(InPlace, This->bInUpdateStateMachine, true);
	...
	auto DoCallbacks = [This](const UE::GameFeatures::FResult& Result, StateIt Begin, StateIt End)
	{
		...
		{
			if (FDestinationGameFeaturePluginState* DestState = This->AllStates[iState]->AsDestinationState())
			{
	...
	RegisterAsTransitioningStateMachine(This);

	bool bKeepProcessing = false;
	int32 NumTransitions = 0;
	const int32 MaxTransitions = InMaxTransitions.Get(10000);
	do
	{
		...
		{
			const FGameFeaturePluginStateMachineProperties& Props = This->AllStates[CurrentState]->StateProperties;
			...
			UE_TRACE_LOG_SCOPED_T(Cpu, GFP_UpdateStateMachine_UpdateState, CpuChannel)
				<< GFP_UpdateStateMachine_UpdateState.State(*UE::GameFeatures::ToString(CurrentState));
			This->AllStates[CurrentState]->UpdateState(StateStatus);
		}

		if (StateStatus.TransitionToState == CurrentState)
		{
			UE_LOGF(LogGameFeatures, Fatal, "Game feature state %ls transitioning to itself. GameFeature: %ls", *UE::GameFeatures::ToString(CurrentState), *GetGameFeatureName(This));
		}

		if (StateStatus.TransitionToState != EGameFeaturePluginState::Uninitialized)
		{
			UE_LOGF(LogGameFeatures, Verbose, "Game feature '%ls' transitioning state (%ls -> %ls)", *GetGameFeatureName(This), *UE::GameFeatures::ToString(CurrentState), *UE::GameFeatures::ToString(StateStatus.TransitionToState));
			{
				UE_TRACE_LOG_SCOPED_T(Cpu, GFP_UpdateStateMachine_EndState, CpuChannel)
					<< GFP_UpdateStateMachine_EndState.State(*UE::GameFeatures::ToString(CurrentState));
				This->AllStates[CurrentState]->EndState();
				CheckAndCancelBatchingRequestForCurrentState(This);
			}
			This->StateProperties.CurrentState = StateStatus.TransitionToState;
			This->StateProperties.StateProgress = 0.0f;
			...
			{
				UE_TRACE_LOG_SCOPED_T(Cpu, GFP_UpdateStateMachine_BeginState, CpuChannel)
					<< GFP_UpdateStateMachine_BeginState.State(*UE::GameFeatures::ToString(CurrentState));
				CheckAddBatchingRequestForCurrentState(This);
				This->AllStates[CurrentState]->BeginState();
			}
			...
			if (CurrentState == EGameFeaturePluginState::Terminal)
			{
				...
				BeginTermination(This);
			}

			if (This->StateProperties.bTryCancel && This->AllStates[CurrentState]->GetStateType() != EGameFeaturePluginStateType::Transition)
			{
				This->StateProperties.Destination = FGameFeaturePluginStateRange(CurrentState);

				This->StateProperties.bTryCancel = false;
				bKeepProcessing = false;

				// Make sure This->bInUpdateStateMachine is not set while processing callbacks if we are at our destination
				ScopeGuard.Reset();
				...
				UE::GameFeatures::FBroadcastingOnTransitionCanceled LocalOnTransitionCanceled(MoveTemp(This->StateProperties.OnTransitionCanceled));
				This->StateProperties.OnTransitionCanceled.Clear();
			...
			{
				check(IsValidErrorState(This, CurrentState));
				This->StateProperties.Destination = FGameFeaturePluginStateRange(CurrentState);
			...
			{
				bKeepProcessing = This->AllStates[CurrentState]->GetStateType() == EGameFeaturePluginStateType::Transition || !This->StateProperties.Destination.Contains(CurrentState);
				if (!bKeepProcessing)
				{
					// Make sure This->bInUpdateStateMachine is not set while processing callbacks if we are at our destination
					ScopeGuard.Reset();
			...
			if (!bKeepProcessing)
			{
				UnregisterAsTransitioningStateMachine(This);
			}
			...
			if (CurrentState == EGameFeaturePluginState::Terminal)
			{
				...
				FinishTermination(This);
				This->MarkAsGarbage();
			}
		}
		else if(!IsRunning(This))
		{
			UnregisterAsTransitioningStateMachine(This);
		}
		...
		if (!bKeepProcessing && InitialState != CurrentState)
		{
			if (!StateStatus.TransitionResult.HasValue())
			{
				...
				if (StateStatus.bSuppressErrorLog)
				{
					UE_LOG(LogGameFeatures, Display, ErrLogFmt,
						*GetGameFeatureName(This),
						*UE::GameFeatures::ToString(CurrentState),
						*UE::GameFeatures::ToString(This->StateProperties.Destination.MinState),
						*UE::GameFeatures::ToString(This->StateProperties.Destination.MaxState),
			...
			else if (This->StateProperties.Destination.Contains(CurrentState))
			{
				UE_LOGF(LogGameFeatures, Display, "Game feature '%ls' transitioned successfully. Ending state: %ls [%ls, %ls]",
					*GetGameFeatureName(This),
					*UE::GameFeatures::ToString(CurrentState),
					*UE::GameFeatures::ToString(This->StateProperties.Destination.MinState),
					*UE::GameFeatures::ToString(This->StateProperties.Destination.MaxState));
		...
		{
			UE_CLOGF(!InMaxTransitions.IsSet(), LogGameFeatures, Fatal, "Infinite loop in game feature state machine transitions. Current state %ls. GameFeature: %ls", *UE::GameFeatures::ToString(CurrentState), *GetGameFeatureName(This));
			// If we aren't attempting to do all transitions at once, make sure we queue up another UpdateStateMachine if there was still work to do
			// (e.g. we just transitioned to a new state which wants UpdateState to be called for the first time)
			if (bKeepProcessing)
			{
				This->StateProperties.UpdateStateMachineDeferred(0.0f);
			}
			break;
		}
...
void UGameFeaturesSubsystem::RegisterAsTransitioningStateMachine(UGameFeaturePluginStateMachine* This) 
{
	if (This->bRegisteredAsTransitioningGFPSM)
	{
		return;
	}

	RegisterRunningStateMachine(This);
	This->bRegisteredAsTransitioningGFPSM = true;
}

void UGameFeaturesSubsystem::UnregisterAsTransitioningStateMachine(UGameFeaturePluginStateMachine* This) 
{
	if (!This->bRegisteredAsTransitioningGFPSM)
	{
		return;
	...
	UnregisterRunningStateMachine(This);
	This->bRegisteredAsTransitioningGFPSM = false;
}

void UGameFeaturesSubsystem::CheckAddBatchingRequestForCurrentState(UGameFeaturePluginStateMachine* This) 
{
	check(!This->StateProperties.BatchProcessingHandle.IsValid());

	const bool bCanBatchProcess =
		UE::GameFeatures::BatchProcessingHelperFunctors[This->StateProperties.CurrentState].ImplementsBatchProcess() &&
		This->StateProperties.CanBatchProcess() &&
		This->AllStates[This->StateProperties.CurrentState]->CanBatchProcess();
	
	if (bCanBatchProcess)
	{
		UE_LOGF(LogGameFeatures, Verbose, "Game feature '%ls' awaiting batch processing of state (%ls)", *GetGameFeatureName(This), *UE::GameFeatures::ToString(This->StateProperties.CurrentState));
		This->StateProperties.BatchProcessingHandle = AddBatchingRequest(
			This->StateProperties.CurrentState,
			This->StateProperties.OnRequestUpdateStateMachine);
	}
}

void UGameFeaturesSubsystem::CheckAndCancelBatchingRequestForCurrentState(UGameFeaturePluginStateMachine* This) 
{
	// Reset batch processing state.
	This->StateProperties.bWasBatchProcessed = false;

	// If we are currently awaiting batch processing, cancel and update the state machine.
	if (This->StateProperties.IsWaitingForBatchProcessing())
	{
		UE_LOGF(LogGameFeatures, Verbose, "Game feature '%ls' cancelled batch processing of state (%ls)", *GetGameFeatureName(This), *UE::GameFeatures::ToString(This->StateProperties.CurrentState));
		CancelBatchingRequest(
			This->StateProperties.CurrentState,
			This->StateProperties.BatchProcessingHandle);

		This->StateProperties.BatchProcessingHandle.Reset();
		UpdateStateMachine(This);
	}
}

/* static */ void UGameFeaturesSubsystem::BatchProcess(EGameFeaturePluginState State, TConstArrayView<UGameFeaturePluginStateMachine*> GFPSMs)
{
...
void UGameFeaturesSubsystem::ExcludeFromBatchProcessing(UGameFeaturePluginStateMachine* This) 
{
	// Ensure protocol options are updated to reflect the exclusion of this state machine from batch processing.
	if (This->StateProperties.ProtocolOptions.bBatchProcess)
	{
		UE_LOGF(LogGameFeatures, Verbose, "Game feature '%ls' excluded from batch processing", *GetGameFeatureName(This));

		FGameFeatureProtocolOptions NewOptions = This->StateProperties.ProtocolOptions;
		NewOptions.bBatchProcess = false;

		bool bOutDidUpdate = false;
		TryUpdatePluginProtocolOptions(This, NewOptions, bOutDidUpdate);
		check(bOutDidUpdate);
		CheckAndCancelBatchingRequestForCurrentState(This);
	}	
}

void UGameFeaturesSubsystem::TryUpdateStateMachineImmediate(int64 Id)
{
	if (StateMachineExecutor)
	{
		StateMachineExecutor->TryUpdateStateMachineImmediate(Id);
	}
	else if (UGameFeaturePluginStateMachine* StateMachine = GameFeaturePluginStateMachines.FindRef(Id))
	{
...
FGameFeaturePluginStateMachineProperties::FGameFeaturePluginStateMachineProperties(
	...
	UE::GameFeatures::FStateMachineExecutor* InExecutor,
	...
	, Destination(FGameFeaturePluginStateRange(InitialState))
	, OnRequestUpdateStateMachine(RequestUpdateStateMachineDelegate)
	, Executor(InExecutor)
{
}

void FGameFeaturePluginStateMachineProperties::UpdateStateMachineImmediate() 
{
	if (Executor)
	{
		Executor->TryUpdateStateMachineImmediate(*this);
	}
	else
	{
		OnRequestUpdateStateMachine.ExecuteIfBound();
	}
}

bool FGameFeaturePluginStateMachineProperties::UpdateStateMachineDeferred(float Delay) 
{
	if (Executor)
	{
		Executor->QueueUpdateStateMachine(*this);
		return true;
	}
	return false;
}
...
FString FInstallBundlePluginProtocolMetaData::ToString() const
{
	...
	static_assert(FGenericPlatformMath::CountBits64(static_cast<uint64>(EGameFeatureURLOptions::All)) == 1, "Update this function to handle the newly added EGameFeatureInstallBundleProtocolOptions value!");
...
TValueOrError<FInstallBundlePluginProtocolMetaData, FString> FInstallBundlePluginProtocolMetaData::FromString(FStringView URLOptionsString)
{
	...
	{
		bParseSuccess = false;
		UE_LOGF(LogGameFeatures, Error, "Error parsing InstallBundle protocol options URL %.*ls", URLOptionsString.Len(), URLOptionsString.GetData());
...
{
	return ProtocolOptions.bBatchProcess && !UE::GameFeatures::bUseNewExecutor && UE::GameFeatures::CVarEnableBatchProcessing.GetValueOnGameThread();
}
