...
struct FGameFeatureProtocolMetadata;
...
using FMultipleGameFeaturePluginsDeactivate = FMultipleGameFeaturePluginChangeStateComplete;
using FMultipleGameFeaturePluginsUnload = FMultipleGameFeaturePluginChangeStateComplete;
...
// TODO: clean this up in 5.10 because we can't neatly deprecate it without breaking external projects
// So the Editor has to have this optimization disabled until 5.10 can enable it by default
#ifndef UE_GAME_FEATURE_PLUGIN_DETAILS_USE_UTF8_STRING
//	Do not trigger deprecated warning for Editor builds, only for clients
#	if !WITH_EDITOR
	UE_DEPRECATED(5.8, "Please set UE_GAME_FEATURE_PLUGIN_DETAILS_USE_UTF8_STRING to 1 in your project settings and migrate to UTF8 implementation")
#	endif
	typedef FString FPluginString;
#	define UE_GAME_FEATURE_PLUGIN_DETAILS_USE_UTF8_STRING 0
#endif

#if UE_GAME_FEATURE_PLUGIN_DETAILS_USE_UTF8_STRING
	typedef FUtf8String FPluginString;
#	define PLUGIN_TEXT(x) UTF8TEXT(x)
#	define TO_PLUGIN_STRING(x) FUtf8String(x)
#else
#	define PLUGIN_TEXT(x) TEXT(x)
#	define TO_PLUGIN_STRING(x) x
#endif

struct FGameFeaturePluginReferenceDetails
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FPluginString PluginName;
...
struct FGameFeaturePluginDetails
{
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	TMap<FPluginString, TSharedPtr<class FJsonValue>> AdditionalMetadata;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
struct FBuiltInGameFeaturePluginBehaviorOptions
{
	...
	/** Maximum state this plugin is allowed to reach */
	EGameFeatureTargetState MaximumState = EGameFeatureTargetState::Active;
...
{
	virtual ~FGameFeaturePluginAsyncHandle() = default;
...
public:
	UGameFeaturesSubsystem(const FObjectInitializer& ObjectInitializer);
	...
	UE_API static FSimpleMulticastDelegate OnGameFeaturePolicyPostInit;
...
public:
	...
	/** Determines if a plugin is in an error state */
	UE_API bool IsGameFeaturePluginInErrorState(const FString& PluginURL) const;
	...
	/** Deactivates the specified plugin. Transition to [Terminal, Loaded] */
	...
	UE_API void DeactivateGameFeaturePlugin(TConstArrayView<FString> PluginURLs, const FMultipleGameFeaturePluginsDeactivate& CompleteDelegate);

	/** Unloads the specified game feature plugin. Transition to bKeepRegistered = false -> [Terminal, Installed], bKeepRegistered = true -> [Terminal, Registered]*/
	...
	UE_API void UnloadGameFeaturePlugin(TConstArrayView<FString> PluginURLs, const FMultipleGameFeaturePluginsUnload& CompleteDelegate, bool bKeepRegistered = false);

	/** Releases any game data stored for this GameFeaturePlugin. Does not uninstall data and it will remain on disk. [Terminal, StatusKnown] */
	...
	/** Terminate the GameFeaturePlugin and remove all associated plugin tracking data. Transition to [Terminal, Terminal] */
	 ...
	 * THREAD SAFETY: Any thread as long as GameSpecificPolicies meets version EGameFeaturesProjectPoliciesThreadSafetyVersion::Initial
	 */
	UE_API bool GetPluginURLByName(FStringView PluginName, FString& OutPluginURL) const;
	UE_API bool GetPluginURLByName(FUtf8StringView PluginName, FString& OutPluginURL) const;
	...
	UE_API void LoadBuiltInGameFeaturePlugins_Amortized(const TArray<TSharedRef<IPlugin>>& Plugins, const FBuiltInPluginAdditionalFilters_Copyable& AdditionalFilter, int32 AmortizeRate, const FBuiltInGameFeaturePluginsLoaded& CompleteDelegate = FBuiltInGameFeaturePluginsLoaded());
private:
	TArray<TSharedRef<IPlugin>> GetInternalEnabledPlugins() const;
	void LoadBuiltInGameFeaturePluginsInternal(TArray<TSharedRef<IPlugin>> Plugins, FBuiltInPluginAdditionalFilters AdditionalFilter, const FBuiltInPluginAdditionalFilters_Copyable& AdditionalFilter_Copyable, int32 AmortizeRate, const FBuiltInGameFeaturePluginsLoaded& CompleteDelegate = FBuiltInGameFeaturePluginsLoaded());
public:
	...
	/** Returns a shared pointer to immutable cached data about the given plugin. Should only be used for built-in GFPs */
	UE_API TSharedPtr<const FGameFeaturePluginDetails> GetBuiltInGameFeaturePluginDetails(const TSharedRef<IPlugin>& Plugin) const;

	/** 
	  * Load and cache all details for all builtin game feature plugins 
	  * @param bForce If true, will try to load all details even if it has been done already. 
	  * 	Use if the GameSpecificPolicies has changed which plugins are allowed.
	  */
	UE_API void LoadAllBuiltInGameFeaturePluginDetails(bool bForce = false);
	...
	/**  Returns a shared pointer to immutable cached data about the given plugin if it's installed. */
	UE_API TSharedPtr<const FGameFeaturePluginDetails> GetGameFeaturePluginDetails(const FString& PluginURL) const;
	 ...
	 * THREAD SAFETY: Any thread
	 */
	UE_API bool IsPluginAllowed(const FString& PluginURL, const FGameFeaturePluginDetails* InDetails, FString* OutReason) const;
	 ...
	 * If bWaitForBundlesToRelease is true, OnComplete will not be called until the bundles have been released
	 */
	UE_API TSharedRef<FGameFeaturePluginPredownloadHandle> PredownloadGameFeaturePlugins(TConstArrayView<FString> PluginURLs, 
		TUniqueFunction<void(const UE::GameFeatures::FResult&)> OnComplete = nullptr, TUniqueFunction<void(float)> OnProgress = nullptr, bool bWaitForBundlesToRelease = false);
	...
	static UE_API EBuiltInAutoState DetermineBuiltInInitialFeatureState(UE::Json::FConstObject Descriptor, const FString& ErrorContext);
	...
	static UE_API const TSet<FString>& GetDlcPlugins();

	/**
	 * Returns true if plugin is a GameFeaturePlugin
	 * @param InPlugin The plugin to check
	 * @return True if plugin considered a GameFeaturePlugin
	 */
	static UE_API bool IsGameFeaturePlugin(const TSharedPtr<IPlugin>& InPlugin);

	UE_API bool GetPluginDebugStateEnabled(const FString& PluginUrl) const;
	...
	/**
	 * Returns the required bundles for this plugin based upon the current state machine.
	 * Unlike GetInstallBundleName this will return all the bundles and dependencies for the given plugin.
	 * @param - PluginName - the name of the GameFeaturePlugin we want to get the bundles for.
	 * @param - OutBundles - Output reference array for the found bundles for the plugin.
	 * @return true if an existing state machine with install bundle protocol is found.
	 */
	UE_API bool GetBundlesForPlugin(FStringView PluginName, TArray<FName>& OutBundles);
	UE_API bool GetBundlesForPluginURL(FStringView PluginURL, TArray<FName>& OutBundles);
...
private:
	...
	/** 
	  * THREAD SAFETY: Any thread as long as no other thread is operating on that state machine.
	  * e.g. from a parallel for on the game thread, as state machine updates only happen on the game thread.
	  */
	...
	UE_API TSharedPtr<const struct FGameFeaturePluginDetails> GetGameFeaturePluginDetailsInternal(const FString& PluginDescriptorFilename) const;

	/** Load details for the given plugin if they are not cached and return them asynchronously. */
	UE_API UE::Tasks::TTask<TSharedPtr<const struct FGameFeaturePluginDetails>> LoadGameFeaturePluginDetailsAsync(const FString& PluginDescriptorFilename) const;

	/** Parse FGameFeaturePluginDetails from a string. Note that parsing may modify the string in-place */
	TSharedPtr<const FGameFeaturePluginDetails> ParseGameFeaturePluginDetails(const FString& ErrorContext, TArrayView<TCHAR> JsonTxt) const;
	/** Parse FGameFeaturePluginDetails from a JSON object */
	TSharedPtr<const FGameFeaturePluginDetails> ParseGameFeaturePluginDetails(const FString& ErrorContext, TSharedRef<FJsonObject> JsonObject) const;
	/** Parse FGameFeaturePluginDetails from a RapidJSON object */
	TSharedPtr<const FGameFeaturePluginDetails> ParseGameFeaturePluginDetails(const FString& ErrorContext, UE::Json::FConstObject JsonObject) const;
	...
	enum class EBatchObserverCallback : uint8
	{
		BatchOperationStarted,
		BatchOperationComplete
	};

	UE_API void BatchCallbackObservers(EBatchObserverCallback BatchCallbackType);
	friend struct GameFeaturesSubsystem::FNotifyObservers;
	...
	friend class UE::GameFeatures::FRealtimeMode;
	friend bool UE::GameFeatures::TickUnloadPluginAssets(float DeltaTime);
	void TryUpdateStateMachineImmediate(int64 Id);

private:
	/** The list of all game feature plugin state machine objects addressed by FGameFeaturePluginStateMachineProperties::Id */
	UPROPERTY(Transient)
	TMap<int64, TObjectPtr<UGameFeaturePluginStateMachine>> GameFeaturePluginStateMachines;

	/** Lookup of game feature plugins by FGameFeaturePluginIdentifier::GetIdentifyingString */
	UPROPERTY(Transient)
	TMap<FString, TObjectPtr<UGameFeaturePluginStateMachine>> GameFeaturesByIdentifier;

	/** Private type controlling execution of state machine transitions when UE::GameFeatures::bUseNewExecutor */
	TPimplPtr<UE::GameFeatures::FStateMachineExecutor> StateMachineExecutor;
	friend class UE::GameFeatures::FStateMachineExecutor;
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	TMap<FPluginString, FPluginString> GameFeaturePluginNameToPathMap;
	mutable FTransactionallySafeRWLock GameFeaturePluginNameToPathMapLock;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

	mutable TMap<FString, TSharedRef<const FGameFeaturePluginDetails>> CachedPluginDetailsByFilename;
	...
	bool bLoadedBuiltInDetails = false;

	static UE_API TSet<FString> DlcPlugins;

	/**
	 * Next identifier to be used for a GFP state machine.
	 * Identifiers are not reused, and when a GFP is terminated and later recreated, a new identifier is assigned.
	 */
	int64 NextInstanceId = 0;

private:
	/**
	 * State machine execution functions hoisted from UGameFeaturePluginStateMachine etc as a refactor
	 * to enable more batch & parallel processing.
	 * These fields could be further factored out into another class.
	 * These methods are marked const until we make changes that hoist state out into the subsystem.
	 */
	...
	void InitStateMachine(UGameFeaturePluginStateMachine* This, int64 Id, FGameFeaturePluginIdentifier InPluginIdentifier, const FGameFeatureProtocolOptions& InProtocolOptions);
	...
	bool SetDestination(
		...
		FGameFeaturePluginStateRange InDestination,
		TDelegate<void(UGameFeaturePluginStateMachine*, const UE::GameFeatures::FResult&)> OnFeatureStateTransitionComplete,
		FDelegateHandle* OutCallbackHandle = nullptr);

	/** Cancel the current transition if possible */
	bool TryCancel(UGameFeaturePluginStateMachine* This, TDelegate<void(UGameFeaturePluginStateMachine*)> OnFeatureStateTransitionCanceled, FDelegateHandle* OutCallbackHandle = nullptr) const;
	...
	UE::GameFeatures::FResult TryUpdatePluginProtocolOptions(UGameFeaturePluginStateMachine* This, const FGameFeatureProtocolOptions& InOptions, bool& bOutDidUpdate) const;

	/** Remove any pending callback from SetDestination */
	void RemovePendingTransitionCallback(UGameFeaturePluginStateMachine* This, FDelegateHandle InHandle) const;

	/** Remove any pending callback from SetDestination */
	void RemovePendingTransitionCallback(UGameFeaturePluginStateMachine* This, FDelegateUserObject DelegateObject) const;

	/** Remove any pending callback from TryCancel */
	void RemovePendingCancelCallback(UGameFeaturePluginStateMachine* This, FDelegateHandle InHandle) const;

	/** Remove any pending callback from TryCancel */
	void RemovePendingCancelCallback(UGameFeaturePluginStateMachine* This, FDelegateUserObject DelegateObject) const;

	/** Returns the PluginIdentifier used to identify this plugin uniquely to the GameFeaturePlugin Subsystem */
	const FGameFeaturePluginIdentifier& GetPluginIdentifier(const UGameFeaturePluginStateMachine* This) const;

	/** Returns the name of the game feature. Before StatusKnown, this returns the URL. */
	const FString& GetGameFeatureName(const UGameFeaturePluginStateMachine* This) const;

	/** Returns the URL */
	const FString& GetPluginURL(const UGameFeaturePluginStateMachine* This) const;

	/** Returns protocol medata */
	const FGameFeatureProtocolMetadata& GetProtocolMetadata(const UGameFeaturePluginStateMachine* This) const;

	/** Returns any protocol options */
	const FGameFeatureProtocolOptions& GetProtocolOptions(const UGameFeaturePluginStateMachine* This) const;

	/** Returns protocol options suitable for reuse by another state machine */
	FGameFeatureProtocolOptions RecycleProtocolOptions(const UGameFeaturePluginStateMachine* This) const;

	/** Returns the plugin name if known (plugin must have been registered to know the name). */
	const FString& GetPluginName(const UGameFeaturePluginStateMachine* This) const;

	/** Returns the uplugin filename of the game feature. Before StatusKnown, this returns false. */
	bool GetPluginFilename(const UGameFeaturePluginStateMachine* This, FString& OutPluginFilename) const;

	/** Returns the enum state for this machine */
	EGameFeaturePluginState GetCurrentState(const UGameFeaturePluginStateMachine* This) const;

	/** Returns the state range this machine is trying to move to */
	FGameFeaturePluginStateRange GetDestination(const UGameFeaturePluginStateMachine* This) const;

	/** Returns information about the current state */
	FGameFeaturePluginStateInfo GetCurrentStateInfo(const UGameFeaturePluginStateMachine* This) const;

	/** Returns true if attempting to reach a new destination state */
	bool IsRunning(const UGameFeaturePluginStateMachine* This) const;

	/** Returns true if the state is at least StatusKnown so we can query info about the game feature plugin */
	bool IsStatusKnown(const UGameFeaturePluginStateMachine* This) const;

	/** Returns true if the plugin is available to download/load. Only call if IsStatusKnown is true */
	bool IsAvailable(const UGameFeaturePluginStateMachine* This) const;

	/** Returns true if the plugin is in an error state */
	bool IsInErrorState(const UGameFeaturePluginStateMachine* This) const;

	/** Whether this machine is allowed to be asynchronous */
	bool AllowAsyncLoading(const UGameFeaturePluginStateMachine* This) const;

	/** Returns true if the plugin will stream dependencies after installing. Only call if IsStatusKnown is true */
	bool HasAssetStreamingDependencies(const UGameFeaturePluginStateMachine* This) const;

	void SetWasLoadedAsBuiltIn(UGameFeaturePluginStateMachine* This);

	bool WasLoadedAsBuiltIn(const UGameFeaturePluginStateMachine* This) const;

	/** If the plugin is activated already, we will retrieve its game feature data */
	UGameFeatureData* GetGameFeatureDataForActivePlugin(UGameFeaturePluginStateMachine* This) const;
	const UGameFeatureData* GetGameFeatureDataForActivePlugin(const UGameFeaturePluginStateMachine* This) const;

	/** If the plugin is registered already, we will retrieve its game feature data */
	UGameFeatureData* GetGameFeatureDataForRegisteredPlugin(UGameFeaturePluginStateMachine* This, bool bCheckForRegistering = false) const;
	const UGameFeatureData* GetGameFeatureDataForRegisteredPlugin(const UGameFeaturePluginStateMachine* This, bool bCheckForRegistering = false) const;

	const FGameFeaturePluginStateMachineProperties& GetProperties(const UGameFeaturePluginStateMachine* This) const;

	/** Returns true if the state machine is in an unrecoverable error state. */
	bool IsErrorStateUnrecoverable(const UGameFeaturePluginStateMachine* This) const;

	/** Sets that the state machine is in an unrecoverable error. Used when the GFD is missing as that can't be recovered from. */
	void SetUnrecoverableError(UGameFeaturePluginStateMachine* This) const;
	...
	void ExcludeFromBatchProcessing(UGameFeaturePluginStateMachine* This);
...
private:
	...
	bool IsValidTransitionState(const UGameFeaturePluginStateMachine* This, EGameFeaturePluginState InState) const;

	/** Returns true if the specified state is a destination state */
	bool IsValidDestinationState(const UGameFeaturePluginStateMachine* This, EGameFeaturePluginState InDestinationState) const;

	/** Returns true if the specified state is a error state */
	bool IsValidErrorState(const UGameFeaturePluginStateMachine* This, EGameFeaturePluginState InDestinationState) const;

	/** Processes the current state and looks for state transitions */
	void UpdateStateMachine(UGameFeaturePluginStateMachine* This, TOptional<int32> MaxTransitions = {});

	/** Helper to register the state machine with the manager as a state machine in transiton */
	void RegisterAsTransitioningStateMachine(UGameFeaturePluginStateMachine* This);

	/** Helper to unregister the state machine with the manager as a state machine in transiton */
	void UnregisterAsTransitioningStateMachine(UGameFeaturePluginStateMachine* This);
	
	/** Adds a batching request for current state if state can be batched */
	void CheckAddBatchingRequestForCurrentState(UGameFeaturePluginStateMachine* This);

	/** Cancels batching re1quest for current state if one exists */
	void CheckAndCancelBatchingRequestForCurrentState(UGameFeaturePluginStateMachine* This);
	...
	bool CancellAmortizedLoads(const FString& PluginURL);
	TMap<FGuid, TWeakPtr<FAmmortizedLoadContext>> ActiveAmmoritzedLoads;
};
