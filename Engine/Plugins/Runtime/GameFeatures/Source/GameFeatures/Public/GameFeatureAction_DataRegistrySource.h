...
public:
	GAMEFEATURES_API virtual void OnGameFeatureRegistering() override;
	GAMEFEATURES_API virtual void OnGameFeatureUnregistering() override;
	GAMEFEATURES_API virtual void OnGameFeatureActivating(FGameFeatureActivatingContext& Context) override;
	GAMEFEATURES_API virtual void OnGameFeatureDeactivating(FGameFeatureDeactivatingContext& Context) override;

	/** If true, we should load the sources at registration time instead of activation time */
	GAMEFEATURES_API virtual bool ShouldPreloadAtRegistration();

#if WITH_EDITORONLY_DATA
	GAMEFEATURES_API virtual void AddAdditionalAssetBundleData(FAssetBundleData& AssetBundleData) override;
#endif

	//~UObject interface
#if WITH_EDITOR
	GAMEFEATURES_API virtual EDataValidationResult IsDataValid(class FDataValidationContext& Context) const override;
#endif
...
protected:
	GAMEFEATURES_API void RegisterRuntimeSources();
	
	GAMEFEATURES_API void UnregisterSources();
