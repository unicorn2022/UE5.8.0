...
bool UGameFeatureActionConvertContentBundleWorldPartitionBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	bDeleteHLODs = Switches.Contains(TEXT("DeleteHLODs"));
	...
	if (FString const* ContentBundlesToConvertString = CommandLineParams.Find(TEXT("ContentBundles")))
	{
		...
		{
			UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to parse ContentBundles argument '%ls'.", **ContentBundlesToConvertString);
			return false;
		}
	}
...
bool UGameFeatureActionConvertContentBundleWorldPartitionBuilder::RunInternal(UWorld* World, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	{
		UE_LOGF(LogConvertContentBundleBuilder, Error, "World %ls does not have content bundles.", *World->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogConvertContentBundleBuilder, Error, "World %ls does not have an ExternalDataLayerManager", *World->GetName());
		return false;
	}

	UE_LOGF(LogConvertContentBundleBuilder, Log, "Found %d Content bundles in World %ls", ContentBundles.Num(), *World->GetName());
	...
	if (!ContentBundlesToConvert.IsEmpty())
	{
		for (TSharedPtr<FContentBundleEditor>& ContentBundle : ContentBundles)
		{
			...
			{
				UE_LOGF(LogConvertContentBundleBuilder, Log, "Skipping %ls: Not in the list of content bundles to convert.", *ContentBundle->GetDisplayName());
				continue;
			}
	...
	for (TSharedPtr<FContentBundleEditor>& ContentBundle : ContentBundlesToProcess)
	{
		...
		{
			UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to access Client of Content Bundle %ls", *ContentBundle->GetDisplayName());
			bIsSuccess = false;
			continue;
		}
		...
		if (ContentBundleClient->GetState() == EContentBundleClientState::Registered)
		{
			if (const FContentBundleBase* CB = ContentBundleManager->GetContentBundle(World, ContentBundle->GetDescriptor()->GetGuid()))
			{
				...
				{
					UE_LOGF(LogConvertContentBundleBuilder, Log, "Requesting forced injection of Content Bundle %ls for conversion purposes.", *ContentBundle->GetDisplayName());
		...
		if (const FContentBundleBase* CB = ContentBundleManager->GetContentBundle(World, ContentBundle->GetDescriptor()->GetGuid()))
		{
			if (CB->GetStatus() != EContentBundleStatus::ContentInjected)
			{
				UE_LOGF(LogConvertContentBundleBuilder, Log, "Skipping %ls: Not injected in this world.", *ContentBundle->GetDisplayName());
				continue;
			}
		}
		...
		{
			UE_LOGF(LogConvertContentBundleBuilder, Error, "Skipping invalid Content Bundle %ls", *ContentBundle->GetDisplayName());
			bIsSuccess = false;
			continue;
		}
		...
		{
			UE_LOGF(LogConvertContentBundleBuilder, Log, "Skipping conversion : Empty Content Bundle %ls", *ContentBundle->GetDisplayName());
		...
		UE_CLOGF(ExternalDataLayerAsset, LogConvertContentBundleBuilder, Log, "Converting Content Bundle %ls using External Data Layer Asset %ls.", *ContentBundle->GetDisplayName(), *ExternalDataLayerAsset->GetPathName());
		if (!ExternalDataLayerAsset)
		{
			UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to retrieve or create External Data Layer Asset for Content Bundle %ls", *ContentBundle->GetDisplayName());
			bIsSuccess = false;
			continue;
		}
		...
		if (!AddWorldPartitionContent)
		{
			...
			if (!AddWorldPartitionContent)
			{
				...
				UE_LOGF(LogConvertContentBundleBuilder, Log, "Added new Action of type 'GameFeatureAction_AddWorldPartitionContent' to GameFeatureData %ls using External Data Layer Asset %ls while converting Content Bundle %ls.", *GameFeatureData->GetName(), *ExternalDataLayerAsset->GetPathName(), *ContentBundle->GetDisplayName());
			}
		...
		if (!ExternalDataLayerInstance)
		{
			...
			UE_CLOGF(ExternalDataLayerInstance, LogConvertContentBundleBuilder, Log, "Create External Data Layer Instance %ls while converting Content Bundle %ls.", *ExternalDataLayerAsset->GetPathName(), *ContentBundle->GetDisplayName());
			if (!ExternalDataLayerInstance)
			{
				UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to create External Data Layer Instance for External Data Layer Asset %ls while converting Content Bundle %ls", *ExternalDataLayerAsset->GetName(), *ContentBundle->GetDisplayName());
				bIsSuccess = false;
				continue;
			}
		...
		{
			UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to find a valid WorldDataLayers actor for External Data Layer Asset %ls while converting Content Bundle %ls", *ExternalDataLayerAsset->GetName(), *ContentBundle->GetDisplayName());
			bIsSuccess = false;
			continue;
		}
		...
		// In case we want to delete HLODs
		...
		for (UActorDescContainerInstance::TIterator<> It(ContentBundleContainerInstance); It; ++It)
		{
			...
			if (!ActorRef.IsValid())
			{
				UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to load actor %ls(%ls). Actor won't be converted.", *It->GetActorLabelOrName().ToString(), *It->GetActorPackage().ToString());
			...
			// Content bundles can contain HLODs, and since their names are partly based on the content bundle they were in, moving/renaming them over in an EDL is not straightforward.
			// It's easier to delete them and let the automated nightly recreate them properly.
			// Only need to use this option if a HLOD is preventing FExternalDataLayerHelper::MoveActorsToExternalDataLayer from succeeding, and you want to avoid a manual Remove HLOD step
			if (bDeleteHLODs)
			{
				if (HLODActorClasses.FindByPredicate([ActorClass = Actor->GetClass()](const UClass* HLODClass) { return ActorClass->IsChildOf(HLODClass); }))
				{
					PackagesToDelete.Add(Actor->GetExternalPackage());
					ConvertedActorGuids.Add(It->GetGuid());
					UE_LOGF(LogConvertContentBundleBuilder, Log, "Deleted HLOD-related Actor %ls.", *Actor->GetFullName());
					continue;
				}
			}
			...
			{
				UE_LOGF(LogConvertContentBundleBuilder, Error, "Can't create package for actor %ls. %ls", *Actor->GetActorNameOrLabel(), *FailureReason.ToString());
				bContentBundleActorConversionSuccess = false;
				break;
			}
			...
			UE_LOGF(LogConvertContentBundleBuilder, Log, "Converted Actor %ls(%ls) to %ls.", *Actor->GetName(), *NewPackage->GetName(), *ExternalDataLayerAsset->GetName());
		...
		{
			UE_LOGF(LogConvertContentBundleBuilder, Log, "Package to save: %ls", *PackageToSave->GetPathName());
		}

		for (UPackage* PackageToDelete : PackagesToDelete)
		{
			UE_LOGF(LogConvertContentBundleBuilder, Log, "Package to delete: %ls", *PackageToDelete->GetPathName());
		}
		...
		if (bContentBundleActorConversionSuccess && !bReportOnly)
		{
			...
			{
				UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to save packages. Conversion sanity is not guaranteed. Consult log for details.");
				bContentBundleActorConversionSuccess = false;
			}
			else if (!UWorldPartitionBuilder::DeletePackages(PackagesToDelete.Array(), PackageHelper))
			{
				UE_LOGF(LogConvertContentBundleBuilder, Error, "Failed to delete packages. Conversion sanity is not guaranteed. Consult log for details.");
				bContentBundleActorConversionSuccess = false;
			}
		}
	...
	if (FinalReport.Num())
	{
		UE_LOGF(LogConvertContentBundleBuilder, Log, "=================================================================================================");
		UE_LOGF(LogConvertContentBundleBuilder, Log, "Content Bundle Conversion : Found(%d) | Converted(%d) | Empty(%d) | Failed(%d)", ContentBundlesToProcess.Num(), ConvertedContentBundles.Num(), SkippedEmptyContentBundles.Num(), FailedContentBundles.Num());
		UE_LOGF(LogConvertContentBundleBuilder, Log, "=================================================================================================");
		for (const FString& Str : FinalReport)
		{
			UE_LOGF(LogConvertContentBundleBuilder, Log, "%ls", *Str);
		}
		UE_LOGF(LogConvertContentBundleBuilder, Log, "=================================================================================================");
	}
...
UExternalDataLayerAsset* UGameFeatureActionConvertContentBundleWorldPartitionBuilder::GetOrCreateExternalDataLayerAsset(const UContentBundleDescriptor* InContentBundleDescriptor, UExternalDataLayerFactory* InExternalDataLayerFactory, TSet<UPackage*>& OutPackagesToSave) const
{
	...
	UE_CLOGF(!NewExternalDataLayerAsset, LogConvertContentBundleBuilder, Error, "Failed to create external data layer asset for %ls.", *InContentBundleDescriptor->GetDisplayName());
