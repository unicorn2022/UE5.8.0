...
EDataValidationResult UIllegalPluginDependenciesValidator::ValidateLoadedAsset_Implementation(const FAssetData& AssetData, UObject* InAsset, FDataValidationContext& InContext)
{
	...
	for (const FString& ModifiedFile : DataValidationChangelist->ModifiedFiles)
	{
		if (ModifiedFile.EndsWith(TEXT(".uplugin")))
		{
			...
			if (Plugin.IsValid())
			{
				if (GFPs.Contains(ModifiedPlugin) || UGameFeaturesSubsystem::IsGameFeaturePlugin(Plugin))
				{
					continue;
				}
				if (!FPaths::IsUnderDirectory(Plugin->GetDescriptorFileName(), FPaths::EnginePluginsDir()) && !FPaths::IsUnderDirectory(Plugin->GetDescriptorFileName(), FPaths::ProjectPluginsDir()))
				{
					// do not validate plugins outside of this project.
					continue;
				}
