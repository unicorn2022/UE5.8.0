...
AGeoReferencingSystem* AGeoReferencingSystem::GetGeoReferencingSystem(UObject* WorldContextObject)
{
	...
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		if (NbActors == 0)
		{
			UE_LOGF(LogGeoReferencing, Error, "GeoReferencingSystem actor not found. Please add one to your world to configure your geo referencing system.");
		}
		else if (NbActors > 1)
		{
			UE_LOGF(LogGeoReferencing, Error, "Multiple GeoReferencingSystem actors found. Only one actor should be used to configure your geo referencing system");
		}
		else
		{
...
void AGeoReferencingSystem::Initialize()
{
	...
	{
		UE_LOGF(LogGeoReferencing, Display, "Enable World Bounds Checks is enabled in your World Settings. You might consider disabling it when working with large terrains, otherwise your pawns won't be able to go too far from the Origin");
	}
...
bool AGeoReferencingSystem::FGeoReferencingSystemInternals::GetEllipsoid(FString CRSString, FEllipsoid& Ellipsoid)
{
	...
	if (CRSPJ != nullptr)
	{
		...
		if (EllipsoidPJ != nullptr)
		{
			...
			else
			{
				...
				UE_LOGF(LogGeoReferencing, Error, "AGeoReferencingSystem::GetEllipsoid failed in proj_ellipsoid_get_parameters : %ls ", *ProjError);
				bSuccess = false;
			}
		...
		else
		{
			int ErrorNumber = proj_context_errno(ProjContext);
			FString ProjError = FString(proj_errno_string(ErrorNumber));
			UE_LOGF(LogGeoReferencing, Error, "AGeoReferencingSystem::GetEllipsoid failed in proj_get_ellipsoid : %ls ", *ProjError);
			bSuccess = false;
		}
	...
	else
	{
		int ErrorNumber = proj_context_errno(ProjContext);
		FString ProjError = FString(proj_errno_string(ErrorNumber));
		UE_LOGF(LogGeoReferencing, Error, "AGeoReferencingSystem::GetEllipsoid failed in proj_create : %ls ", *ProjError);
		UE_LOGF(LogGeoReferencing, Display, "CRSString was : %ls ", *CRSString);
		bSuccess = false;
	}
...
static void ProjLog(void* app_data, int level, const char* message)
{
	...
	{
	case PJ_LOG_ERROR:
		UE_LOGF(LogGeoReferencing, Error, "%ls", Message.Get());
		break;
	case PJ_LOG_DEBUG:
		UE_LOGF(LogGeoReferencing, Verbose, "%ls", Message.Get());
		break;
	case PJ_LOG_TRACE:
		UE_LOGF(LogGeoReferencing, VeryVerbose, "%ls", Message.Get());
		break;
	}
}
...
void AGeoReferencingSystem::FGeoReferencingSystemInternals::InitPROJLibrary()
{
	...
	{
		UE_LOGF(LogGeoReferencing, Error, "proj_context_create() failed");
		return;
	}
	...
	UE_LOGF(LogGeoReferencing, Display, "Setting search path in %ls ", *ProjDataPath);
	...
	if (PlatformFile.IsSandboxEnabled())
	{
		...
		UE_LOGF(LogGeoReferencing, Warning, "Running in Sandboxed mode with Proj.db expected in %ls ", *ProjDBFileSandboxPath);

		// Check if the file is missing there
		if (!IFileManager::Get().FileExists(*ProjDBFileSandboxPath))
		{
			UE_LOGF(LogGeoReferencing, Warning, "Missing PROJ data files in then Sandbox environment --> Copy them!");
	...
	if (!GIsEditor)
	{
		...
		{
			UE_LOGF(LogGeoReferencing, Error, "proj_context_set_fileapi() failed");
		}
	}
}
...
PJ* AGeoReferencingSystem::FGeoReferencingSystemInternals::GetPROJProjection(FString SourceCRS, FString DestinationCRS)
{
	...
	if (TempPJ == nullptr)
	{
		int ErrorNumber = proj_context_errno(ProjContext);
		FString ProjError = FString(proj_errno_string(ErrorNumber));
		UE_LOGF(LogGeoReferencing, Error, "AGeoReferencingSystem::BuildProjection failed in proj_create_crs_to_crs : %ls ", *ProjError);

		UE_LOGF(LogGeoReferencing, Display, "SourceCRS was : %ls ", *SourceCRS);
		UE_LOGF(LogGeoReferencing, Display, "DestinationCRS was : %ls ", *DestinationCRS);
	...
	if (P_for_GIS == nullptr)
	{
		int ErrorNumber = proj_context_errno(ProjContext);
		FString ProjError = FString(proj_errno_string(ErrorNumber));
		UE_LOGF(LogGeoReferencing, Error, "AGeoReferencingSystem::BuildProjection failed in proj_normalize_for_visualization : %ls ", *ProjError);
	}
