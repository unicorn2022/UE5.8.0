...
PROJ_FILE_HANDLE* FUFSProj::Open(PJ_CONTEXT* ctx, const char* filename, PROJ_OPEN_ACCESS access, void* user_data)
{
	...
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::open '%ls'", FilePath.Get());
...
{
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::read '%ls'", *handle->RequestedFile);
...
{
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::write '%ls'", *handle->RequestedFile);
...
{
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::seek '%ls'", *handle->RequestedFile);
	switch (whence)
	{
	case SEEK_CUR:
...
{
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::tell '%ls'", *handle->RequestedFile);
...
{
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::close '%ls'", *handle->RequestedFile);
...
{
	FUTF8ToTCHAR FilePath(filename);
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::exists '%ls'", FilePath.Get());
...
{
	FUTF8ToTCHAR FilePath(filename);
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::mkdir '%ls'", FilePath.Get());
...
{
	FUTF8ToTCHAR FilePath(filename);
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::unlink '%ls'", FilePath.Get());
...
int FUFSProj::Rename(PJ_CONTEXT* ctx, const char* oldPath, const char* newPath, void* user_data)
{
	...
	UE_CLOGF(EnableLogging, LogGeoReferencing, Log, "FUFSProj::rename '%ls' -> '%ls'", OldFilePath.Get(), NewFilePath.Get());
	return FALSE;
}
