...
namespace UE::OSC
{
	namespace ManagerPrivate
	{
		...
		const FOSCData* GetDataAtIndex(const FOSCMessage& InMessage, const int32 InIndex)
		{
			...
			UE_LOGF(LogOSC, Warning, "Index '%d' out-of-bounds.  Message argument size = '%d'", InIndex, Args.Num());
			return nullptr;
		}
		...
		{
			UE_LOGF(LogOSC, Warning, "OSC Message Parse Failed: OSCData not %ls: index '%i', OSCAddress '%ls'",
		...
		static FAutoConsoleCommand GOSCPrintServers(
			...
			FConsoleCommandDelegate::CreateStatic(
				[]()
				{
					...
					UE_LOGF(LogOSC, Display, "Local IP: %ls", *LocalAddr);

					UE_LOGF(LogOSC, Display, "OSC Servers:");
					for (TObjectIterator<UOSCServer> Itr; Itr; ++Itr)
					{
						if (UOSCServer* Server = *Itr)
						{
							...
							UE_LOGF(LogOSC, Display, "%ls", *ToPrint);

							const TArray<FOSCAddress> BoundPatterns = Server->GetBoundOSCAddressPatterns();
							if (BoundPatterns.Num() > 0)
							{
								UE_LOGF(LogOSC, Display, "    Bound Address Patterns:");
								for (const FOSCAddress& Pattern : BoundPatterns)
								{
									UE_LOGF(LogOSC, Display, "         %ls", *Pattern.GetFullPath());
								}
								UE_LOGF(LogOSC, Display, "");
							}
						}
					}
				}
		...
		static FAutoConsoleCommand GOSCServerConnect(
			...
			FConsoleCommandWithArgsDelegate::CreateStatic(
				[](const TArray<FString>& Args)
				{
					...
					UE_LOGF(LogOSC, Warning, "Server object with name '%ls' not found, (re)connect not performed.", *SrvName);
				}
			)
		);
		...
		static FAutoConsoleCommand GOSCServerConnectById(
			...
			FConsoleCommandWithArgsDelegate::CreateStatic(
				[](const TArray<FString>& Args)
				{
					...
					UE_LOGF(LogOSC, Warning, "Server object with id '%u' not found, (re)connect not performed.", SrvId);
				}
			)
		);
		...
		static FAutoConsoleCommand GOSCPrintClients(
			...
			FConsoleCommandDelegate::CreateStatic(
				[]()
				{
					...
					UE_LOGF(LogOSC, Display, "OSC Clients:");
		...
		static FAutoConsoleCommand GOSCClientConnect(
			...
			FConsoleCommandWithArgsDelegate::CreateStatic(
				[](const TArray< FString >& Args)
				{
					...
					UE_LOGF(LogOSC, Warning, "Client object with name '%ls' not found, (re)connect not performed.", *CliName);
				}
			)
		);
		...
		static FAutoConsoleCommand GOSCClientConnectById(
			...
			FConsoleCommandWithArgsDelegate::CreateStatic(
				[](const TArray< FString >& Args)
				{
					...
					UE_LOGF(LogOSC, Warning, "Client object with id '%u' not found, (re)connect not performed.", CliId);
				}
			)
		);
	} // namespace ManagerPrivate
...
UOSCServer* UOSCManager::CreateOSCServer(FString InReceiveIPAddress, int32 InPort, bool bInMulticastLoopback, bool bInStartListening, FString ServerName, UObject* Outer)
{
	...
	{
		UE_LOGF(LogOSC, Display, "OSCServer ReceiveAddress not specified. Using LocalHost IP: '%ls'", *InReceiveIPAddress);
	}
	...
	{
		UE_LOGF(LogOSC, Warning, "Outer object not set.  OSCServer may be garbage collected if not referenced.");
	...
	if (NewOSCServer)
	{
		...
		{
			UE_LOGF(LogOSC, Warning, "Failed to parse ReceiveAddress '%ls' for OSCServer.", *InReceiveIPAddress);
		}
...
UOSCClient* UOSCManager::CreateOSCClient(FString InSendIPAddress, int32 InPort, FString ClientName, UObject* Outer)
{
	...
	{
		UE_LOGF(LogOSC, Display, "OSCClient SendAddress not specified. Using LocalHost IP: '%ls'", *InSendIPAddress);
	}
	...
	{
		UE_LOGF(LogOSC, Warning, "Outer object not set.  OSCClient '%ls' may be garbage collected if not referenced.", *ClientName);
	...
	if (NewOSCClient)
	{
		...
		{
			UE_LOGF(LogOSC, Warning, "Failed to parse SendAddress '%ls' for OSCClient. Client unable to send new messages.", *InSendIPAddress);
		}
...
UObject* UOSCManager::FindObjectAtOSCAddress(const FOSCAddress& InAddress)
{
	...
	UE_LOGF(LogOSC, Verbose, "Failed to load object from OSCAddress '%ls'", *InAddress.GetFullPath());
	return nullptr;
}
