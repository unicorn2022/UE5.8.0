...
namespace UE::OSC
{
	...
	TCHAR FStream::ReadChar()
	{
		...
		if (Read(&Temp, 1) > 0)
		{
			UE_CLOGF(Temp > 0x7F, LogOSC, Warning, "Non-ANSI character '%u' written to OSCStream", Temp);
			return (TCHAR)Temp;
		}
	...
	FString FStream::ReadString()
	{
		...
		{
			UE_LOGF(LogOSC, Error, "Invalid string when reading OSCStream: Null terminator '\0' not found");
			return FString();
		}
