...
void UOSCServer::BroadcastBundle(const FOSCBundle& InBundle)
{
	...
	for (const TSharedRef<IPacket>& SubPacket : Packets)
	{
		...
		{
			UE_LOGF(LogOSC, Warning, "Failed to parse invalid received message. Invalid OSC type (packet is neither identified as message nor bundle).");
		}
	}
}
...
void UOSCServer::BroadcastMessage(const FOSCMessage& InMessage)
{
	...
	UE_LOGF(LogOSC, Verbose, "Message received from IP endpoint '%ls', OSCAddress of '%ls'.", *Endpoint.ToString(), *InMessage.GetAddress().GetFullPath());
	...
	for (const TPair<FOSCAddress, FOSCDispatchMessageEvent>& Pair : AddressPatterns)
	{
		...
		if (Pair.Key.Matches(InMessage.GetAddress()))
		{
			...
			UE_LOGF(LogOSC, Verbose, "Message dispatched from IP endpoint '%ls', OSCAddress path of '%ls' matched OSCAddress pattern '%ls'.",
				*Endpoint.ToString(),
