...
namespace UE::OSC
{
	TSharedPtr<IPacket> IPacket::CreatePacket(const uint8* InPacketType, const FIPv4Endpoint& InIPEndpoint)
	{
		...
		{
			UE_LOGF(LogOSC, Warning, "Failed to parse lead character of OSC packet. "
				"Lead identifier of '%c' not valid bundle tag ('%ls') or message ('%ls') identifier.", PacketIdentifier[0], *OSC::BundleTag, *OSC::PathSeparator);
			return nullptr;
		}
