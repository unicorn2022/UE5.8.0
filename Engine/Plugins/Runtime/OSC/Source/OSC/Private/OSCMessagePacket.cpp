...
namespace UE::OSC
{
	...
	void FMessagePacket::WriteData(FStream& Stream)
	{
		...
		{
			UE_LOGF(LogOSC, Warning, "Failed to write OSCMessagePacket. Invalid OSCAddress '%ls'", *Address.GetFullPath());
			return;
		}
	...
	void FMessagePacket::ReadData(FStream& Stream)
	{
		...
		if(TagTypes.Num() == 0)
		{
			UE_LOGF(LogOSC, Error, "Failed to read message packet with address '%ls' from stream: Invalid (Empty) Type Tag", *Address.GetFullPath());
			return;
		}
