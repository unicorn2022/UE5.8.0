...
namespace UE::OSC
{
	...
	void FServerProxy::OnPacketReceived(FConstPacketDataRef InData, const FIPv4Endpoint& InEndpoint)
	{
		...
		if (!Packet.IsValid())
		{
			UE_LOGF(LogOSC, Verbose, "Message received from endpoint '%ls' invalid OSC packet.", *InEndpoint.ToString());
			return;
		}
	...
	void FServerProxy::Listen(const FString& InServerName)
	{
		if (IsActive())
		{
			UE_LOGF(LogOSC, Error, "OSCServer currently '%ls' currently listening to endpoint '%ls'. Failed to start new service prior to calling stop.",
	...
	bool FServerProxy::SetIPEndpoint(const FIPv4Endpoint& InEndpoint)
	{
		if (IsActive())
		{
			UE_LOGF(LogOSC, Error, "Cannot set '%ls' endpoint to '%ls' while OSC server is currently active.", *GetDescription(), *InEndpoint.ToString());
			return false;
		}
	...
	bool FServerProxy::SetMulticastLoopback(bool bInMulticastLoopback)
	{
		...
		{
			UE_LOGF(LogOSC, Error, "Cannot update MulticastLoopback while OSCServer is active.");
			return false;
		}
	...
	void FServerProxy::AddClientToAllowList(const FString& InIPAddress)
	{
		...
		{
			UE_LOGF(LogOSC, Warning, "OSCServer failed to add IP Address '%ls' to allow list. Address is invalid.", *InIPAddress);
			return;
		}
	...
	void FServerProxy::RemoveClientFromAllowList(const FString& InIPAddress)
	{
		...
		{
			UE_LOGF(LogOSC, Warning, "OSCServer failed to remove IP Address '%ls' from allow list. Address is invalid.", *InIPAddress);
			return;
		}
