...
namespace UE::OSC
{
	...
	FServerReceiver::~FServerReceiver()
	{
		...
		UE_LOGF(LogOSC, Display, "Destroying OSC Socket Receiver '%ls'", *GetDescription());
		if (Thread)
		{
			const FString& ThreadName = Thread->GetThreadName();
			UE_LOGF(LogOSC, Display, "Killing process thread '%ls'...", *ThreadName);
			constexpr bool bWait = true;
			Thread->Kill(bWait);
			UE_LOGF(LogOSC, Display, "Process thread '%ls' killed successfully.", *ThreadName);
			Thread.Reset();
		}
		...
		if (Socket)
		{
			...
			UE_LOGF(LogOSC, Display, "Socket '%ls' '%ls' closed.", *SocketDesc, bSocketClosed ? TEXT("successfully") : TEXT("failed to"));
			Socket.Reset();
		}
	}
	...
	bool FServerReceiver::BindSocket(ISocketSubsystem& SocketSubsystem, const FIPv4Endpoint& InEndpoint, TSharedRef<FInternetAddr> RemoteAddr, const FString& InName)
	{
			...
			!Socket->SetRecvErr())
		{
			...
			UE_LOGF(LogOSC, Warning, "OSC Socket Receiver: Failed to configure socket %ls: Error code %d: %ls",
		...
		if (!Socket->Bind(*RemoteAddr))
		{
			...
			UE_LOGF(LogOSC, Warning, "OSC Socket Receiver: Failed to bind %ls to %ls. Error code %d: %ls",
	...
	bool FServerReceiver::InitMulticast(ISocketSubsystem& InSocketSubsystem, const FIPv4Endpoint& InEndpoint, TSharedRef<FInternetAddr> RemoteAddr, const FString& InName)
	{
		if (!Socket->SetMulticastLoopback(Options.bMulticastLoopback) || !Socket->SetMulticastTtl(1))
		{
			...
			UE_LOGF(LogOSC, Warning, "OSC Socket Receiver: Failed to configure multicast for %ls (loopback: %i). Error code %d.",
		...
		{
			const ESocketErrors SocketError = InSocketSubsystem.GetLastErrorCode();
			UE_LOGF(LogOSC, Warning, "OSC Server Receiver: Failed to subscribe %ls to multicast group %ls. Error code %d.",
	...
	bool FServerReceiver::InitSocket(const FString& InName, FIPv4Endpoint Endpoint)
	{
		...
		if (!Socket.IsValid())
		{
			UE_LOGF(LogOSC, Warning, "OSCServer '%ls' failed to create socket", *InName);
			return false;
		}
	...
	TSharedRef<FServerReceiver> FServerReceiver::Launch(const FString& InName, const FIPv4Endpoint& InEndpoint, FOptions InOptions)
	{
		...
		if (bSocketInit)
		{
			if (NewReceiver->Socket)
			{
				...
				UE_LOGF(LogOSC, Display, "OSCServer '%ls' started", *InName);
			}
		}
		else
		{
