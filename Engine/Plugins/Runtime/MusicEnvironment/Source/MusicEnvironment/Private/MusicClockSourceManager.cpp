...
TScriptInterface<IMusicEnvironmentClockSource> UMusicClockSourceManager::FindClock(const FGameplayTag& Tag, bool bExactMatch)
{
	...
	if (!bExactMatch)
	{
		...
		if (MatchingClock.IsValid())
		{
			...
			{ 
				UE_LOGF(LogMusicEnvironment, Warning, "FindClock found %d clocks that match the tag '%ls'. Returning clock tagged '%ls'.", NumberOfMatches, *Tag.ToString(), *MatchingTag.ToString());
			}
...
void UMusicClockSourceManager::AddTaggedClock(const FGameplayTag& Tag, UObject* InClock)
{
	...
	if (TaggedClocks.Contains(Tag))
	{
		...
		{
			UE_LOGF(LogMusicEnvironment, Warning, "The MusicClockSourceManager is already tracking a clock tagged '%ls'. It will be replaced by this new request.", *Tag.ToString());
