...
: FEnsureScope(
	...
	{
		UE_LOGF(LogNetworkAutomationTest, Display, "Ensure condition failed: %s\n%ls\n", Args.Expression, Args.Message);
		return true;
	}
  )
...
#define UE_TEST_MESSAGE_LOG_(Verbosity) UE_LOGF(LogNetworkAutomationTest, Verbosity, "TestCase %ls: %ls", Test.GetName(), Message.C_Str())
