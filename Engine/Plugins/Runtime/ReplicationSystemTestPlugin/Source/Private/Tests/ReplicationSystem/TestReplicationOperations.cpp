...
UE_NET_TEST_FIXTURE(FTestReplicationOperationsFixture, InitPropertyIsNotUpdatedWhenInitStateIsExcluded)
{
	...
	FInternalNetSerializationContext InternalContext;
	NetContext.SetInternalContext(&InternalContext);
...
{
	const bool bIsGarbageEliminationEnabled = UObjectBaseUtility::IsGarbageEliminationEnabled();
	UObjectBaseUtility::ForceSetGarbageEliminationEnabledForTest(true);
	ON_SCOPE_EXIT
	{
		UObjectBaseUtility::ForceSetGarbageEliminationEnabledForTest(bIsGarbageEliminationEnabled);
	};
	...
	Object1->AddComponents({.ObjectReferenceComponentCount = 1});
