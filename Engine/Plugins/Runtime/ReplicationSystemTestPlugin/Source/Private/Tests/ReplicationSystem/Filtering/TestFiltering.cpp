...
private:
	...
	void InitObjectScopeHysteresisProfiles()
	{
		...
		TArray<FObjectScopeHysteresisProfile> NewProfiles;
		{
			...
			{
				FObjectScopeHysteresisProfile& Profile = NewProfiles.Emplace_GetRef();
				Profile.FilterProfileName = "VeryManyFrames";
				Profile.HysteresisFrameCount = 255;
			}
		}
...
protected:
	UMockNetObjectFilter* MockNetObjectFilter = nullptr;
	FNetObjectFilterHandle MockFilterHandle;
	FNetObjectFilterHandle NotRoutedFilterHandle;
...
// Test that a dynamically filtered out object doesn't enter hysteresis for late joining client 
UE_NET_TEST_FIXTURE(FTestFilteringFixture, HysteresisDoesNotAffectDynamicallyFilteredOutObjectForLateJoiningClient)
{
	constexpr uint32 DefaultHysteresisFrameCount = 3;
	FScopedDefaultHysteresisFrameCount ScopedDefaultHysteresisFrameCount(DefaultHysteresisFrameCount);

	// Create object with filter which disallows replication.
	UReplicatedTestObject* ServerObject = Server->CreateObject();
	Server->ReplicationSystem->SetFilter(ServerObject->NetRefHandle, NotRoutedFilterHandle);
	...
	// Perform send update for more than the hysteresis duration and make sure the object never gets created.
	for (uint32 It = 0; It < DefaultHysteresisFrameCount + 1U; ++It)
	{
		Server->UpdateAndSend({ Client });
		UE_NET_ASSERT_EQ(Client->GetReplicationBridge()->GetReplicatedObject(ServerObject->NetRefHandle), nullptr);
	}
...
// Test that a dynamically filtered out dependent objects whose parent is not in scope doesn't enter hysteresis for late joining client 
UE_NET_TEST_FIXTURE(FTestFilteringFixture, HysteresisDoesNotAffectDynamicallyFilteredOutDependentObjectForLateJoiningClient)
{
	constexpr uint32 DefaultHysteresisFrameCount = 3;
	FScopedDefaultHysteresisFrameCount ScopedDefaultHysteresisFrameCount(DefaultHysteresisFrameCount);
	...
	// Create dependent object with filter which disallows replication.
	UReplicatedTestObject* ServerDependentObject = Server->CreateObject();
	Server->ReplicationSystem->SetFilter(ServerDependentObject->NetRefHandle, NotRoutedFilterHandle);
	...
	// Perform send update for more than the hysteresis duration and make sure the objects never get created.
	for (uint32 It = 0; It < DefaultHysteresisFrameCount + 1U; ++It)
	{
		Server->UpdateAndSend({ Client });
		UE_NET_ASSERT_EQ(Client->GetReplicationBridge()->GetReplicatedObject(ServerObject->NetRefHandle), nullptr);
...
UE_NET_TEST_FIXTURE(FTestFilteringFixture, TestDependentObjectIsFilteredOutTogetherWithExclusionFilteredOutParent)
{
	constexpr uint32 DefaultHysteresisFrameCount = 2;
	FScopedDefaultHysteresisFrameCount ScopedDefaultHysteresisFrameCount(DefaultHysteresisFrameCount);
	...
	// Verify that objects have been created on the client
	...
	// Throw in some filtering groups into the mix
	FNetObjectGroupHandle ExclusionFilterGroup = ReplicationSystem->CreateGroup(NAME_None);
	ReplicationSystem->AddExclusionFilterGroup(ExclusionFilterGroup);
	FNetObjectGroupHandle InclusionFilterGroup = ReplicationSystem->CreateGroup(NAME_None);
	ReplicationSystem->AddInclusionFilterGroup(InclusionFilterGroup);

	// Allow replication of the InclusionGroup to the client
	ReplicationSystem->SetGroupFilterStatus(InclusionFilterGroup, Client->ConnectionIdOnServer, ENetFilterStatus::Allow);

	// Add parent to groups
	ReplicationSystem->AddToGroup(ExclusionFilterGroup, ServerObject->NetRefHandle);
	ReplicationSystem->AddToGroup(InclusionFilterGroup, ServerObject->NetRefHandle);

	// Send data
	Server->UpdateAndSend({Client});

	// Allow replication of the ExclusionGroup to the client
	ReplicationSystem->SetGroupFilterStatus(ExclusionFilterGroup, Client->ConnectionIdOnServer, ENetFilterStatus::Allow);

	// Send data
	Server->UpdateAndSend({Client});

	// Verify that the objects exist on the client
	UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(ServerObject->NetRefHandle));
	UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(ServerDependentObject->NetRefHandle));

	// Disallow replication of the ExclusionGroup to the client
	ReplicationSystem->SetGroupFilterStatus(ExclusionFilterGroup, Client->ConnectionIdOnServer, ENetFilterStatus::Disallow);

	// Perform send update for the hysteresis duration
	for (uint32 It = 0; It != DefaultHysteresisFrameCount + 1; ++It)
	{
		Server->UpdateAndSend({ Client });
	}

	// Verify that the objects are no longer valid on the client
	UE_NET_ASSERT_FALSE(Client->IsValidNetRefHandle(ServerObject->NetRefHandle));
	UE_NET_ASSERT_FALSE(Client->IsValidNetRefHandle(ServerDependentObject->NetRefHandle));
}

UE_NET_TEST_FIXTURE(FTestFilteringFixture, DependentObjectAddedToExclusionFilteredOutObjectIsNotInScope)
{
	FReplicationSystemTestClient* ClientArray[] = { CreateClient(), CreateClient() };

	UReplicatedTestObject* ServerObject = Server->CreateObject();

	UReplicatedTestObject* ServerDependentObject1 = Server->CreateObject();
	Server->ReplicationSystem->SetFilter(ServerDependentObject1->NetRefHandle, NotRoutedFilterHandle, /* .FilterConfigProfile */ "VeryManyFrames");

	Server->ReplicationBridge->AddDependentObject(ServerObject->NetRefHandle, ServerDependentObject1->NetRefHandle);

	Server->UpdateAndSend(ClientArray);

	UE_NET_ASSERT_TRUE(ClientArray[0]->IsResolvableNetRefHandle(ServerObject->NetRefHandle));
	UE_NET_ASSERT_TRUE(ClientArray[0]->IsResolvableNetRefHandle(ServerDependentObject1->NetRefHandle));

	UE_NET_ASSERT_TRUE(ClientArray[1]->IsResolvableNetRefHandle(ServerObject->NetRefHandle));
	UE_NET_ASSERT_TRUE(ClientArray[1]->IsResolvableNetRefHandle(ServerDependentObject1->NetRefHandle));
	...
	Server->ReplicationSystem->SetGroupFilterStatus(GroupHandle, ENetFilterStatus::Allow);
	Server->ReplicationSystem->SetGroupFilterStatus(GroupHandle, ClientArray[1]->ConnectionIdOnServer, ENetFilterStatus::Disallow);
	...
	UE_NET_ASSERT_FALSE(ClientArray[1]->IsValidNetRefHandle(ServerObject->NetRefHandle));
	// The dependent object was in scope for a while and should be subject to hysteresis, thus would be resolvable even for the client for which the dependant was just filtered out
	UE_NET_ASSERT_TRUE(ClientArray[1]->IsValidNetRefHandle(ServerDependentObject1->NetRefHandle));

	UReplicatedTestObject* ServerDependentObject2 = Server->CreateObject();
	Server->ReplicationSystem->SetFilter(ServerDependentObject2->NetRefHandle, NotRoutedFilterHandle, /* .FilterConfigProfile */ "VeryManyFrames");

	Server->ReplicationBridge->RemoveDependentObject(ServerObject->NetRefHandle, ServerDependentObject1->NetRefHandle);
	Server->ReplicationBridge->AddDependentObject(ServerObject->NetRefHandle, ServerDependentObject2->NetRefHandle);

	Server->UpdateAndSend(ClientArray);

	UE_NET_ASSERT_TRUE(ClientArray[0]->IsResolvableNetRefHandle(ServerObject->NetRefHandle));
	UE_NET_ASSERT_TRUE(ClientArray[0]->IsResolvableNetRefHandle(ServerDependentObject1->NetRefHandle));
	UE_NET_ASSERT_TRUE(ClientArray[0]->IsResolvableNetRefHandle(ServerDependentObject2->NetRefHandle));

	UE_NET_ASSERT_FALSE(ClientArray[1]->IsValidNetRefHandle(ServerObject->NetRefHandle));
	UE_NET_ASSERT_TRUE(ClientArray[1]->IsResolvableNetRefHandle(ServerDependentObject1->NetRefHandle));
	// The ServerDependentObject2 was never in scope for ClientArray[1] and isn't expected to be now that is was just added.
	UE_NET_ASSERT_FALSE(ClientArray[1]->IsValidNetRefHandle(ServerDependentObject2->NetRefHandle));
}

UE_NET_TEST_FIXTURE(FTestFilteringFixture, AlreadyCreatedObjectAddedAsDependencyToExclusionFilteredOutObjectIsNotInScope)
{
	FReplicationSystemTestClient* ClientArray[] = { CreateClient(), CreateClient() };
...
UE_NET_TEST_FIXTURE(FTestFilteringFixture, DependentObjectWithHysteresisAddedToExclusionGroupObjectIsNotInScope)
{
	FReplicationSystemTestClient* ClientArray[] = { CreateClient(), CreateClient() };
	...
	UReplicatedTestObject* ServerDependentObject = Server->CreateObject();
	Server->ReplicationSystem->SetFilter(ServerDependentObject->NetRefHandle, NotRoutedFilterHandle, /* .FilterConfigProfile */ "VeryManyFrames");
	...
	FNetObjectGroupHandle GroupHandle = Server->ReplicationSystem->CreateGroup(NAME_None);
	Server->ReplicationSystem->AddToGroup(GroupHandle, ServerDependentObject->NetRefHandle);
	Server->ReplicationSystem->AddExclusionFilterGroup(GroupHandle);
	...
	UE_NET_ASSERT_TRUE(ClientArray[0]->IsResolvableNetRefHandle(ServerDependentObject->NetRefHandle));
	UE_NET_ASSERT_FALSE(ClientArray[1]->IsValidNetRefHandle(ServerDependentObject->NetRefHandle));
}

UE_NET_TEST_FIXTURE(FTestFilteringFixture, DependentObjectWithHysteresisAddedToExclusionGroupThenAllowedIsNotFilteredOutByHysteresis)
{
	FReplicationSystemTestClient* ClientArray[] = { CreateClient(), CreateClient() };
	...
	UReplicatedTestObject* ServerDependentObject = Server->CreateObject();
	Server->ReplicationSystem->SetFilter(ServerDependentObject->NetRefHandle, NotRoutedFilterHandle, /* .FilterConfigProfile */ "FiveFrames");
	...
	// Allow dependent object again
	Server->ReplicationSystem->SetGroupFilterStatus(GroupHandle, ClientArray[1]->ConnectionIdOnServer, ENetFilterStatus::Allow);

	Server->UpdateAndSend(ClientArray);

	// Dependent object should now have been replicated again
	UE_NET_ASSERT_TRUE(ClientArray[1]->IsResolvableNetRefHandle(ServerDependentObject->NetRefHandle));

	// Update for the expected hysteresis duration and make sure the dependent object never goes out of scope.
	for (unsigned FrameIt = 0; FrameIt < 6U; ++FrameIt)
	{
		Server->UpdateAndSend(ClientArray);
		UE_NET_ASSERT_TRUE(ClientArray[1]->IsResolvableNetRefHandle(ServerDependentObject->NetRefHandle));
	}
...
UE_NET_TEST_FIXTURE(FTestFilteringFixture, CreationDependentObjectRespectsParentHysteresis)
{
	UReplicatedTestObjectBridge* Bridge = Server->GetReplicationBridge();

	FReplicationSystemTestClient* Client = CreateClient();

	UReplicatedTestObject* Parent = Server->CreateObject();
	UReplicatedTestObject* Child = Server->CreateObject();
	Bridge->AddCreationDependencyLink(Parent->NetRefHandle, Child->NetRefHandle);

	// Create objects on client.
	Server->UpdateAndSend({Client});
	UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(Parent->NetRefHandle));
	UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(Child->NetRefHandle));

	// Filter out the parent, but set hysteresis to five frames.
	Server->ReplicationSystem->SetFilter(Parent->NetRefHandle, NotRoutedFilterHandle, "FiveFrames");

	// Parent and child should both stay in scope for 5 frames.
	for (unsigned FrameIt = 0; FrameIt < 5U; ++FrameIt)
	{
		Server->UpdateAndSend({Client});
		UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(Parent->NetRefHandle));
		UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(Child->NetRefHandle));
	...
	// Parent and child should both go out of scope after 5 frames.
	Server->UpdateAndSend({Client});
	UE_NET_ASSERT_FALSE(Client->IsResolvableNetRefHandle(Parent->NetRefHandle));
	UE_NET_ASSERT_FALSE(Client->IsResolvableNetRefHandle(Child->NetRefHandle));
}

UE_NET_TEST_FIXTURE(FTestFilteringFixture, CreationDependentObjectHysteresisIgnoredWhenParentIsFilteredOut)
{
	UReplicatedTestObjectBridge* Bridge = Server->GetReplicationBridge();
	...
	// Filter out parent, filter out child with hysteresis.
	Server->ReplicationSystem->SetFilter(Parent->NetRefHandle, NotRoutedFilterHandle);
	Server->ReplicationSystem->SetFilter(Child->NetRefHandle, NotRoutedFilterHandle, "FiveFrames");

	// Creation dependency should override child hysteresis, so both objects shouldn't exist on client.
	Server->UpdateAndSend({Client});
	UE_NET_ASSERT_FALSE(Client->IsResolvableNetRefHandle(Parent->NetRefHandle));
	UE_NET_ASSERT_FALSE(Client->IsResolvableNetRefHandle(Child->NetRefHandle));
}

UE_NET_TEST_FIXTURE(FTestFilteringFixture, CreationDependentObjectInHysteresisFilteredOutWhenParentHysteresisEnds)
{
	UReplicatedTestObjectBridge* Bridge = Server->GetReplicationBridge();
	...
	// Filter out parent after one frame, filter out child after five frames.
	Server->ReplicationSystem->SetFilter(Parent->NetRefHandle, NotRoutedFilterHandle, "OneFrame");
	Server->ReplicationSystem->SetFilter(Child->NetRefHandle, NotRoutedFilterHandle, "FiveFrames");

	// Both objects should still exist after one frame.
	Server->UpdateAndSend({Client});
	UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(Parent->NetRefHandle));
	UE_NET_ASSERT_TRUE(Client->IsResolvableNetRefHandle(Child->NetRefHandle));

	// Parent hysteresis ends after one frame, so child should be pulled out of hysteresis.
	// Parent and child should remain out of scope for the duration of what would have been the child's hysteresis.
	for (unsigned FrameIt = 0; FrameIt < 5U; ++FrameIt)
	{
		Server->UpdateAndSend( { Client });
		UE_NET_ASSERT_FALSE(Client->IsResolvableNetRefHandle(Parent->NetRefHandle));
...
UE_NET_TEST_FIXTURE(FTestFilteringFixture, TestIsRelevantForConnection)
{
	// Add client
	FReplicationSystemTestClient* Client = CreateClient();
	...
	UE_NET_ASSERT_TRUE(Server->ReplicationSystem->IsRelevantForConnection(ServerObject->NetRefHandle, Client->ConnectionIdOnServer));
	...
	UE_NET_ASSERT_FALSE(Server->ReplicationSystem->IsRelevantForConnection(ServerObject->NetRefHandle, Client->ConnectionIdOnServer));
	...
	// Test Subobjects
	{
		// Test a normal subobject
		UReplicatedTestObject* ServerSubObject = Server->CreateSubObject(ServerObject->NetRefHandle);

		Server->UpdateAndSend({ Client });

		UE_NET_ASSERT_TRUE(Server->ReplicationSystem->IsRelevantForConnection(ServerSubObject->NetRefHandle, Client->ConnectionIdOnServer));
	...
	// Test a subobject set to COND_Never
	{
		UReplicatedTestObject* ServerSubObjectNever = Server->CreateSubObject(ServerObject->NetRefHandle);

		Server->ReplicationBridge->SetSubObjectNetCondition(ServerSubObjectNever->NetRefHandle, COND_Never);

		Server->UpdateAndSend({ Client });

		UE_NET_ASSERT_FALSE(Server->ReplicationSystem->IsRelevantForConnection(ServerSubObjectNever->NetRefHandle, Client->ConnectionIdOnServer));

		// Object should never exist on client
		UE_NET_ASSERT_EQ(Client->GetReplicationBridge()->GetReplicatedObject(ServerSubObjectNever->NetRefHandle), nullptr);
	}
