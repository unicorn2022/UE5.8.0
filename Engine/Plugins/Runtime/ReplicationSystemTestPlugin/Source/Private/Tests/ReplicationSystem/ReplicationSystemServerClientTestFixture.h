...
public:
	...
	template<typename T>
	T* CreateStablyNamedObject(const TCHAR* ObjectName)
	{
		...
		T* CreatedObject = NewObject<T>(ReplicationSystem->GetReplicationBridge(), *FString::Printf(TEXT("%s"), ObjectName));

		if (UReplicatedTestObject* ReplicatedTestObject = Cast<UReplicatedTestObject>(CreatedObject))
		{
			ReplicatedTestObject->bForceStablyNamed = true;
			CreatedObjects.Add(TStrongObjectPtr<UObject>(CreatedObject));
	...
	template<typename T>
	T* CreateSubObject(FNetRefHandle OwnerHandle)
	{
		T* CreatedSubObject = NewObject<T>();
		if (Cast<UReplicatedTestObject>(CreatedSubObject))
		{
			CreatedObjects.Add(TStrongObjectPtr<UObject>(CreatedSubObject));

			// Add it to the bridge for replication
			ReplicationBridge->BeginReplication(OwnerHandle, CreatedSubObject);
	
			return CreatedSubObject;
		}
		return nullptr;
	}

	template<typename T>
	T* CreateSubObjectWithParams(const UObjectReplicationBridge::FSubObjectReplicationParams& SubObjectParams)
	{
		T* CreatedSubObject = NewObject<T>();
		if (Cast<UReplicatedTestObject>(CreatedSubObject))
		{
			...
			// Add it to the bridge for replication
			ReplicationBridge->BeginReplication(CreatedSubObject, SubObjectParams);
...
{
public:
	explicit FReplicationSystemTestClient(const TCHAR* Name, const FReplicationSystemTestNode::FReplicationSystemParamsOverride* ParamsOverride = nullptr);
...
public:
	...
	// If set, automatically call NetUpdate and PostSendUpdate after each deliverd packet to simulate a client tick.
	bool bUpdateClientOnDelivery = true;
};
