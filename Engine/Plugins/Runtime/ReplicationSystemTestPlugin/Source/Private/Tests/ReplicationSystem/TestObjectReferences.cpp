...
void UTestObjectReferences_TestClassWithDefaultSubObject::RegisterReplicationFragments(UE::Net::FFragmentRegistrationContext& Context, UE::Net::EFragmentRegistrationFlags RegistrationFlags)
{
	...
	if (CreatedSubObjectRef.Get())
	{
		static_cast<UTestReplicatedIrisObject*>(CreatedSubObjectRef.Get())->RegisterReplicationFragments(Context, RegistrationFlags);
	}
}
...
UTestObjectReferences_TestClassWithNestedDefaultSubObjects::UTestObjectReferences_TestClassWithNestedDefaultSubObjects()
: UReplicatedTestObject()
{
	CreatedSubObjectRef = CreateDefaultSubobject<UTestObjectReferences_TestClassWithDefaultSubObject>("TestDefaultSubObjectRef");
}

void UTestObjectReferences_TestClassWithNestedDefaultSubObjects::RegisterReplicationFragments(UE::Net::FFragmentRegistrationContext& Context, UE::Net::EFragmentRegistrationFlags RegistrationFlags)
{
	UE::Net::FReplicationFragmentUtil::CreateAndRegisterFragmentsForObject(this, Context, RegistrationFlags);
	...
	if (CreatedSubObjectRef.Get())
	{
		CreatedSubObjectRef.Get()->RegisterReplicationFragments(Context, RegistrationFlags);
	}
}

void UTestObjectReferences_TestClassWithNestedDefaultSubObjects::GetLifetimeReplicatedProps(TArray< class FLifetimeProperty > & OutLifetimeProps) const
{
	DOREPLIFETIME(UTestObjectReferences_TestClassWithNestedDefaultSubObjects, ObjectRef);
}
...
UE_NET_TEST_FIXTURE(FReplicationSystemServerClientTestFixture, TestInlinedSubObjectReference)
{
	...
	UE_NET_ASSERT_EQ(ClientObject->ObjectRef.Get(), ClientObject->CreatedSubObjectRef.Get());
}
...
UE_NET_TEST_FIXTURE(FReplicationSystemServerClientTestFixture, TestExternalSubObjectReference)
{
	...
	// Verify that object has been spawned on client
	UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object->NetRefHandle);
	UTestObjectReferences_TestClassWithDefaultSubObject* ClientExternalSubObject = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(ExternalSubObject->NetRefHandle);
	...
	// Verify that we could resolve the subobject ref and that we got the expected reference
	UE_NET_ASSERT_TRUE(ClientObject->ObjectRef != nullptr);
	UE_NET_ASSERT_EQ(static_cast<UTestObjectReferences_TestClassWithDefaultSubObject*>(ClientObject->ObjectRef.Get()), ClientExternalSubObject);
}
...
// Test that references to subobjects of stale dynamic objects works as epxected
UE_NET_TEST_FIXTURE(FReplicationSystemServerClientTestFixture, TestStaleDefaultSubObjectReferences)
{
	TGuardConsoleVariable<bool> CVarOverride(TEXT("net.iris.VerifyOuterForStaleDynamicReferences"), true);

	// Add a client
	FReplicationSystemTestClient* Client = CreateClient();

	// Spawn objects on server
	UTestObjectReferences_TestClassWithDefaultSubObject* Object1 = Server->CreateObject<UTestObjectReferences_TestClassWithDefaultSubObject>();
	UTestObjectReferences_TestClassWithDefaultSubObject* Object2 = Server->CreateObject<UTestObjectReferences_TestClassWithDefaultSubObject>();
	// Have object2 reference the default subobject of object1
	Object2->ObjectRef = Object1->CreatedSubObjectRef;

	Server->UpdateAndSend({Client});

	// Store original references
	const FNetObjectReference OriginalRefToReplicatedObject1 = Server->ReplicationBridge->GetOrCreateObjectReference(Object1);
	const FNetObjectReference OriginalRefToSubObject = Server->ReplicationBridge->GetOrCreateObjectReference(Object1->CreatedSubObjectRef);

	// Verify that objects has been spawned on client and that we can resolve the replicated subobject via ref
	{
		UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject1 = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object1->NetRefHandle);
		UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject2 = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object2->NetRefHandle);
	
		UE_NET_ASSERT_NE(ClientObject1, nullptr);
		UE_NET_ASSERT_NE(ClientObject2, nullptr);
		UE_NET_ASSERT_NE(ClientObject2->ObjectRef.Get(), nullptr);
	}

	// Stop replicating object1, Note: Object is not destroyed on server, but we tell server to destroy it on client
	Server->ReplicationBridge->EndReplication(Object1, EEndReplicationFlags::Destroy);

	Server->UpdateAndSend({Client});

	constexpr bool bPerformFullPurge = true;
	CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS, bPerformFullPurge);

	// Verify new status
	{
		// This will assign a new reference
		const FNetObjectReference NewRefToNotReplicatingObject1 = Server->ReplicationBridge->GetOrCreateObjectReference(Object1);
		UE_NET_ASSERT_FALSE(NewRefToNotReplicatingObject1 == OriginalRefToReplicatedObject1);

		// This will assign a new reference as the outer has changed.
		const FNetObjectReference NewRefToNotReplicatingSubObject = Server->ReplicationBridge->GetOrCreateObjectReference(Object1->CreatedSubObjectRef);
		UE_NET_ASSERT_FALSE(NewRefToNotReplicatingSubObject == OriginalRefToSubObject);

		UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject1 = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object1->NetRefHandle);
		UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject2 = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object2->NetRefHandle);
	
		UE_NET_ASSERT_EQ(ClientObject1, nullptr);
		UE_NET_ASSERT_NE(ClientObject2, nullptr);
		UE_NET_ASSERT_EQ(ClientObject2->ObjectRef.Get(), nullptr);
	}

	// Restart replication of object1, Note: since it is dynamic, it will get a new NetRefHandle
	Server->ReplicationBridge->BeginReplication(Object1);

	// Spawn objects on server
	UTestObjectReferences_TestClassWithDefaultSubObject* Object3 = Server->CreateObject<UTestObjectReferences_TestClassWithDefaultSubObject>();

	// Have object3 reference object1
	Object3->ObjectRef = Object1->CreatedSubObjectRef;

	Server->UpdateAndSend({Client});

	// Verify that objects has been spawned on client and that we can resolve the new replicated subobject for client object 3
	{
		UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject1 = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object1->NetRefHandle);
		UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject2 = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object2->NetRefHandle);
		UTestObjectReferences_TestClassWithDefaultSubObject* ClientObject3 = Client->GetObjectAs<UTestObjectReferences_TestClassWithDefaultSubObject>(Object3->NetRefHandle);
	
		UE_NET_ASSERT_NE(ClientObject1, nullptr);
		UE_NET_ASSERT_NE(ClientObject2, nullptr);
		// This will not resolve as it references a subobject of the old no longer existing instance of the dynamic object
		UE_NET_ASSERT_EQ(ClientObject2->ObjectRef.Get(), nullptr);
		UE_NET_ASSERT_NE(ClientObject3->ObjectRef.Get(), nullptr);
	}
}

// Test that references to nested default subobjects of stale replicated dynamic objects works as expected
UE_NET_TEST_FIXTURE(FReplicationSystemServerClientTestFixture, TestStaleNestedDefaultSubObjectReferences)
{
	TGuardConsoleVariable<bool> CVarOverride(TEXT("net.iris.VerifyOuterForStaleDynamicReferences"), true);
	...
	// Spawn objects on server
	UTestObjectReferences_TestClassWithNestedDefaultSubObjects* Object1 = Server->CreateObject<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>();
	UTestObjectReferences_TestClassWithNestedDefaultSubObjects* Object2 = Server->CreateObject<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>();

	// Have object2 reference nested default subobject of object1->
	Object2->ObjectRef = Object1->CreatedSubObjectRef->CreatedSubObjectRef;

	Server->UpdateAndSend({Client});

	// Store original references
	const FNetObjectReference OriginalRefToReplicatedObject1 = Server->ReplicationBridge->GetOrCreateObjectReference(Object1);
	const FNetObjectReference OriginalRefToSubObject = Server->ReplicationBridge->GetOrCreateObjectReference(Object1->CreatedSubObjectRef->CreatedSubObjectRef);

	// Verify that objects has been spawned on client and that we can resolve the replicated subobject via ref
	{
		UTestObjectReferences_TestClassWithNestedDefaultSubObjects* ClientObject1 = Client->GetObjectAs<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>(Object1->NetRefHandle);
		UTestObjectReferences_TestClassWithNestedDefaultSubObjects* ClientObject2 = Client->GetObjectAs<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>(Object2->NetRefHandle);
	...
	// Verify new status
	{
		...
		// This will assign a new reference as the outer has changed.
		const FNetObjectReference NewRefToNotReplicatingSubObject = Server->ReplicationBridge->GetOrCreateObjectReference(Object1->CreatedSubObjectRef->CreatedSubObjectRef);
		UE_NET_ASSERT_FALSE(NewRefToNotReplicatingSubObject == OriginalRefToSubObject);
	...
	// Spawn objects on server
	UTestObjectReferences_TestClassWithNestedDefaultSubObjects* Object3 = Server->CreateObject<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>();

	// Have object3 reference object1
	Object3->ObjectRef = Object1->CreatedSubObjectRef->CreatedSubObjectRef;
	...
	// Verify that objects has been spawned on client and that we can resolve the new replicated subobject for client object 3
	{
		UTestObjectReferences_TestClassWithNestedDefaultSubObjects* ClientObject1 = Client->GetObjectAs<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>(Object1->NetRefHandle);
		UTestObjectReferences_TestClassWithNestedDefaultSubObjects* ClientObject2 = Client->GetObjectAs<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>(Object2->NetRefHandle);
		UTestObjectReferences_TestClassWithNestedDefaultSubObjects* ClientObject3 = Client->GetObjectAs<UTestObjectReferences_TestClassWithNestedDefaultSubObjects>(Object3->NetRefHandle);
