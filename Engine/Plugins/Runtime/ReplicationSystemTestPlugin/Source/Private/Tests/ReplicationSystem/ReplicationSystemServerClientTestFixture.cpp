...
void FReplicationSystemTestNode::Setup(bool bIsServer, const TCHAR* Name, const FReplicationSystemTestNode::FReplicationSystemParamsOverride* ParamsOverride)
{
	...
	if (ParamsOverride)
	{
		...
		Params.InitialNetRefHandleIndex = ParamsOverride->InitialNetRefHandleIndex;
		Params.ProxyType = ParamsOverride->ProxyType;
	}
...
bool FReplicationSystemTestNode::SendUpdate(uint32 ConnectionId, const TCHAR* Desc)
{
	...
	if (bWroteData)
	{
		...
		UE_LOGF(LogIris, Verbose, "ReplicationSystemTestFixture: Conn: %u Send PacketId: %u %ls", ConnectionId, Packet.PacketId, *Packet.Desc);
	}
...
void FReplicationSystemTestNode::DeliverTo(FReplicationSystemTestNode& Dest, uint32 LocalConnectionId, uint32 RemoteConnectionId, bool bDeliver)
{
	...
	{
		UE_LOGF(LogIris, Verbose, "ReplicationSystemTestFixture: Conn: %u Unable to %s packet as there are no packets.", LocalConnectionId, (bDeliver ? "deliver" : "drop"));
		return;
	}
	...
	if (bDeliver)
	{
		...
		UE_LOGF(LogIris, Verbose, "ReplicationSystemTestFixture: Conn: %u Deliver PacketId: %u %ls", LocalConnectionId, Packet.PacketId, *Packet.Desc);
	...
	{
		UE_LOGF(LogIris, Verbose, "ReplicationSystemTestFixture: Conn: %u Dropped PacketId: %u %ls", LocalConnectionId, Packet.PacketId, *Packet.Desc);
...
FReplicationSystemTestClient::FReplicationSystemTestClient(const TCHAR* Name, const FReplicationSystemTestNode::FReplicationSystemParamsOverride* ParamsOverride)
: FReplicationSystemTestNode(false, Name, ParamsOverride)
...
bool FReplicationSystemTestClient::UpdateAndSend(FReplicationSystemTestServer* Server, bool bDeliver, const TCHAR* Desc)
{
	...
	NetUpdate();
	const bool bSentPacket = SendUpdate(Desc);
	PostSendUpdate();
...
bool FReplicationSystemTestServer::UpdateAndSend(const TArrayView<FReplicationSystemTestClient*const>& Clients, bool bDeliver /*= true*/, const TCHAR* Desc)
{
	...
	TArray<FReplicationSystemTestClient*> ClientsWithData;
	ClientsWithData.Reserve(Clients.Num());

	FNetBitArray SentPackets;
	SentPackets.Init(Clients.Num());

	for (FReplicationSystemTestClient* Client : Clients)
	{
		if (SendTo(Client, Desc))
		{
			ClientsWithData.Add(Client);
		}
	}
	
	PostSendUpdate();

	for (FReplicationSystemTestClient* Client : ClientsWithData)
	{
		DeliverTo(Client, bDeliver);
		bSentPackets = true;
	}
...
void FReplicationSystemServerClientTestFixture::DestroyClient(FReplicationSystemTestClient* Client)
{
	...
	{
		UE_LOGF(LogIris, Warning, "Unable to find FReplicationSystemTestClient %p for destroy. NOT destroying.", Client);
		return;
	}
