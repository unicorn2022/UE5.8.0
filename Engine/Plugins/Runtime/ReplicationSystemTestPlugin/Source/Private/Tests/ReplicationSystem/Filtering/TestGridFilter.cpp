...
UE_NET_TEST_FIXTURE(FTestGridFilterFixture, TestStartQueueRpcFlushNetDormancyStop)
{
	...
	// Filter out to ensure ServrObject is not referenced by any connection
	FNetObjectGroupHandle NotReplicatedGroupHandle = Server->GetReplicationSystem()->GetNotReplicatedNetObjectGroup();
	Server->ReplicationSystem->AddToGroup(NotReplicatedGroupHandle, ServerObject->NetRefHandle);

	Server->ReplicationBridge->NetFlushDormantObject(ServerObject->NetRefHandle);
	Server->DestroyObject(ServerObject, EEndReplicationFlags::Destroy | EEndReplicationFlags::DestroyNetHandle | EEndReplicationFlags::Flush);

	// Send update
	Server->UpdateAndSend({Client});

	// Send update. Object should be destroyed on server.
	Server->UpdateAndSend({Client});
}
