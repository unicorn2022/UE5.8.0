...
protected:
	...
	// Test-only access into the cache's private storage. The fixture is friended on
	// FPolymorphicNetSerializerScriptStructCache so this can poke private members; tests
	// (which run on a class derived from the fixture) call through this helper.
	static int32 RemoveTypeFromCache(FPolymorphicNetSerializerScriptStructCache& Cache, FName StructName)
	{
		return Cache.RegisteredTypes.Remove(StructName);
	}
...
UE_NET_TEST_FIXTURE(FTestPolymorphicArrayStructNetSerializerFixture, TestCacheLookupByName)
{
	// Two independently initialized caches must produce equivalent FName lookups for the same
	// struct, and lookups for unregistered names must return nullptr (not mis-resolve to
	// some other entry the way an array index would).
	FPolymorphicNetSerializerScriptStructCache CacheA;
	FPolymorphicNetSerializerScriptStructCache CacheB;
	CacheA.InitForType(FExamplePolymorphicStructBase::StaticStruct());
	CacheB.InitForType(FExamplePolymorphicStructBase::StaticStruct());

	const FName StructAName = FPolymorphicNetSerializerScriptStructCache::MakeStructName(FExamplePolymorphicStructA::StaticStruct());
	UE_NET_ASSERT_FALSE(StructAName.IsNone());

	const FPolymorphicNetSerializerScriptStructCache::FTypeInfo* InfoA = CacheA.GetTypeInfo(StructAName);
	const FPolymorphicNetSerializerScriptStructCache::FTypeInfo* InfoB = CacheB.GetTypeInfo(StructAName);
	UE_NET_ASSERT_NE(InfoA, static_cast<const FPolymorphicNetSerializerScriptStructCache::FTypeInfo*>(nullptr));
	UE_NET_ASSERT_NE(InfoB, static_cast<const FPolymorphicNetSerializerScriptStructCache::FTypeInfo*>(nullptr));
	UE_NET_ASSERT_TRUE(InfoA->ScriptStruct == InfoB->ScriptStruct);
	UE_NET_ASSERT_TRUE(InfoA->ScriptStruct == FExamplePolymorphicStructA::StaticStruct());

	// Lookup by ScriptStruct pointer must agree with lookup by FName.
	UE_NET_ASSERT_EQ(CacheA.GetTypeInfo(FExamplePolymorphicStructA::StaticStruct()), InfoA);

	// Unknown names resolve to nullptr (the bug we are fixing: under TypeIndex this would
	// silently land on an unrelated entry).
	UE_NET_ASSERT_EQ(CacheA.GetTypeInfo(FName()), static_cast<const FPolymorphicNetSerializerScriptStructCache::FTypeInfo*>(nullptr));
	UE_NET_ASSERT_EQ(CacheA.GetTypeInfo(FName(TEXT("/Script/UnloadedPlugin.SomeStructThatDoesNotExist"))), static_cast<const FPolymorphicNetSerializerScriptStructCache::FTypeInfo*>(nullptr));
}

UE_NET_TEST_FIXTURE(FTestPolymorphicArrayStructNetSerializerFixture, TestCacheDivergenceMatchingStruct)
{
	// Server has the full set of polymorphic structs registered. Client is missing
	// FExamplePolymorphicStructA (simulating a client build that didn't load that module).
	// Server quantize+serialize an array containing only StructB, which both sides have.
	// Client deserialize+dequantize must succeed and round-trip the value.
	FTestPolymorphicArrayStructNetSerializerConfig ServerConfig;
	FTestPolymorphicArrayStructNetSerializerConfig ClientConfig;
	ServerConfig.RegisteredTypes.InitForType(FExamplePolymorphicStructBase::StaticStruct());
	ClientConfig.RegisteredTypes.InitForType(FExamplePolymorphicStructBase::StaticStruct());

	const FName StructAName = FPolymorphicNetSerializerScriptStructCache::MakeStructName(FExamplePolymorphicStructA::StaticStruct());
	UE_NET_ASSERT_NE(RemoveTypeFromCache(ClientConfig.RegisteredTypes, StructAName), 0);

	FExamplePolymorphicArrayStruct ServerArray;
	{
		FExamplePolymorphicStructB* StructBLocal = new FExamplePolymorphicStructB;
		StructBLocal->SomeFloat = 7.0f;
		ServerArray.Add(StructBLocal);
	}

	FMemory::Memzero(QuantizedBuffer[0], sizeof(QuantizedBuffer[0]));

	{
		FNetQuantizeArgs Args = {};
		Args.Version = PolymorphicNetSerializer->Version;
		Args.NetSerializerConfig = NetSerializerConfigParam(&ServerConfig);
		Args.Target = NetSerializerValuePointer(&QuantizedBuffer[0]);
		Args.Source = NetSerializerValuePointer(&ServerArray);
	...
	Reader.InitBits(BitStreamBuffer, Writer.GetPosBits());
	{
		...
		Args.Version = PolymorphicNetSerializer->Version;
		Args.NetSerializerConfig = NetSerializerConfigParam(&ClientConfig);
		Args.Target = NetSerializerValuePointer(&QuantizedBuffer[0]);
		PolymorphicNetSerializer->Deserialize(Context, Args);
		UE_NET_ASSERT_FALSE(Context.HasError());
	}

	FExamplePolymorphicArrayStruct ClientArray;
	{
		FNetDequantizeArgs Args = {};
		Args.Version = PolymorphicNetSerializer->Version;
		Args.NetSerializerConfig = NetSerializerConfigParam(&ClientConfig);
		Args.Source = NetSerializerValuePointer(&QuantizedBuffer[0]);
		Args.Target = NetSerializerValuePointer(&ClientArray);
		PolymorphicNetSerializer->Dequantize(NetSerializationContext, Args);
	}

	UE_NET_ASSERT_EQ(ClientArray.Num(), 1);
	const FExamplePolymorphicStructBase* RoundTrippedBase = ClientArray.Get(0);
	UE_NET_ASSERT_NE(RoundTrippedBase, static_cast<const FExamplePolymorphicStructBase*>(nullptr));
	UE_NET_ASSERT_EQ(RoundTrippedBase->GetScriptStruct(), FExamplePolymorphicStructB::StaticStruct());
	const FExamplePolymorphicStructB* RoundTripped = static_cast<const FExamplePolymorphicStructB*>(RoundTrippedBase);
	UE_NET_ASSERT_EQ(RoundTripped->SomeFloat, 7.0f);
...
UE_NET_TEST_FIXTURE(FTestPolymorphicArrayStructNetSerializerFixture, TestCacheDivergenceMissingStruct)
{
	// Server sends a struct that the client's cache does not contain (the client did not load
	// that module). The deserializer must report NetError_PolymorphicStructNetSerializer_InvalidStructType
	// rather than mis-resolving to a different struct.
	FTestPolymorphicArrayStructNetSerializerConfig ServerConfig;
	FTestPolymorphicArrayStructNetSerializerConfig ClientConfig;
	ServerConfig.RegisteredTypes.InitForType(FExamplePolymorphicStructBase::StaticStruct());
	ClientConfig.RegisteredTypes.InitForType(FExamplePolymorphicStructBase::StaticStruct());
	...
	FExamplePolymorphicArrayStruct ServerArray;
	{
		FExamplePolymorphicStructA* StructALocal = new FExamplePolymorphicStructA;
		StructALocal->SomeInt = 99;
		ServerArray.Add(StructALocal);
	}
	...
	Reader.InitBits(BitStreamBuffer, Writer.GetPosBits());
	{
		...
		Args.Target = NetSerializerValuePointer(&QuantizedBuffer[0]);
		PolymorphicNetSerializer->Deserialize(Context, Args);
		UE_NET_ASSERT_TRUE(Context.HasError());
		UE_NET_ASSERT_EQ(Context.GetError(), NetError_PolymorphicStructNetSerializer_InvalidStructType);
	}
	FMemory::Memzero(QuantizedBuffer[0], sizeof(QuantizedBuffer[0]));
}
