...
static void WriteDataRecursive(FNetBitStreamWriter& Writer, FNetTraceCollector* Collector, uint32 CurrentDepth)
{
	UE_NET_TRACE_SCOPE(WrittenData, Writer, Collector, ENetTraceVerbosity::Verbose);
	Writer.WriteBits(1U, 1);
	if (CurrentDepth > 0)
	{
		WriteDataRecursive(Writer, Collector, CurrentDepth - 1U);
	}
}

UE_NET_TEST_FIXTURE(FNetBitStreamWriterTest, TraceScopeClampedNestingLevel)
{
	FNetTraceCollector* Collector = UE_NET_TRACE_CREATE_COLLECTOR(ENetTraceVerbosity::Trace);

	Writer.InitBytes(Buffer.GetBuffer(), Buffer.GetBufferCapacity());

	{
		UE_NET_TRACE_SCOPE(Payload, Writer, Collector, ENetTraceVerbosity::Verbose);
		const uint32 TestNestedDepth = FNetTraceCollector::MaxNestingLevel + 5;
	
		// Just verify we do not trigger check/crash.
		WriteDataRecursive(Writer, Collector, TestNestedDepth);
	}

	UE_NET_TRACE_DESTROY_COLLECTOR(Collector);
}
#endif
