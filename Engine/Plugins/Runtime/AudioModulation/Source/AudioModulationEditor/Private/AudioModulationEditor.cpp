...
void FAudioModulationEditorModule::StartupModule()
{
	...
	if (GIsEditor)
	{
		...
		AudioControlBusTrackCreateEditorHandle = SequencerModule.RegisterTrackEditor(FOnCreateTrackEditor::CreateStatic(&FAudioControlBusTrackEditor::CreateTrackEditor));
		AudioControlBusMixTrackCreateEditorHandle = SequencerModule.RegisterTrackEditor(FOnCreateTrackEditor::CreateStatic(&FAudioControlBusMixTrackEditor::CreateTrackEditor));
	}
#endif
...
#if WITH_AUDIOMODULATION_METASOUND_SUPPORT
	{
		...
		PinFactory->RegisterPin("Modulator");
		PinFactory->RegisterPin(CreateArrayTypeNameFromElementTypeName("Modulator"));
		PinFactory->RegisterPin("ModulationParameter");
		PinFactory->RegisterPin(CreateArrayTypeNameFromElementTypeName("ModulationParameter"));
	}
#endif // WITH_AUDIOMODULATION_METASOUND_SUPPORT
...
void FAudioModulationEditorModule::ShutdownModule()
{
	...
	if (FModuleManager::Get().IsModuleLoaded(Metasound::Editor::IMetasoundEditorModule::ModuleName))
	{
		...
		IMetasoundEditorModule& Module = FModuleManager::GetModuleChecked<IMetasoundEditorModule>(IMetasoundEditorModule::ModuleName);
		TSharedRef<IMetaSoundGraphPanelPinFactory> PinFactory = Module.GetGraphPanelPinFactory();
		PinFactory->UnregisterPin("Modulator");
		PinFactory->UnregisterPin(CreateArrayTypeNameFromElementTypeName("Modulator"));
		PinFactory->UnregisterPin("ModulationParameter");
		PinFactory->UnregisterPin(CreateArrayTypeNameFromElementTypeName("ModulationParameter"));
	}
#endif // WITH_AUDIOMODULATION_METASOUND_SUPPORT
	...
	{
		ISequencerModule& SequencerModule = FModuleManager::Get().LoadModuleChecked<ISequencerModule>("Sequencer");
		SequencerModule.UnRegisterTrackEditor(AudioControlBusTrackCreateEditorHandle);
		SequencerModule.UnRegisterTrackEditor(AudioControlBusMixTrackCreateEditorHandle);
	}
#endif
}
