			...
			[
				...
				.Text_Lambda(
					[StructPropertyHandle]()
					{
						...
						if (TSharedPtr<IPropertyHandle> BusProperty = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FSoundControlBusMixStage, Bus), bRecurse))
						{
							const UObject* BusObject = nullptr;
							if (BusProperty->GetValue(BusObject) == FPropertyAccess::Success)
							{
								return FText::AsCultureInvariant(GetNameSafe(BusObject));
							}
						}

						return FText::Format(LOCTEXT("BusMixStageHeader_Format", "Bus Stage {0}"), StructPropertyHandle->GetPropertyDisplayName());
					}
				)
				.ToolTipText_Lambda(
					[StructPropertyHandle]()
					{
						...
						if (TSharedPtr<IPropertyHandle> BusProperty = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FSoundControlBusMixStage, Bus), bRecurse))
						{
							...
							if (BusProperty->GetValue(BusObject) == FPropertyAccess::Success)
							{
								...
								{
									UPackage* Package = BusObject->GetPackage();
									return FText::AsCultureInvariant(GetPathNameSafe(Package ? (UObject*)Package : BusObject));
								}
								return FText::GetEmpty();
							}
						}

						return StructPropertyHandle->GetToolTipText();
					}
				)
			]
		];
