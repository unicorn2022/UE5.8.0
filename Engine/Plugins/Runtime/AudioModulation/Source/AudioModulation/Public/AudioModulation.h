...
namespace AudioModulation
{
	...
	public:
		...
		UE_API virtual void GetControlBusAssetPath(TArray<FTopLevelAssetPath>& OutClassNames) override;
	...
	public:
		...
		UE_API USoundControlBusMix* CreateBusMixFromValue(FName Name, const TArray<USoundControlBus*>& Buses, float Value, float AttackTime = -1.0f, float ReleaseTime = -1.0f, UObject* InOuter = nullptr);

		UE_DEPRECATED(5.4, "Deactivation of modulators in this manner is now deprecated. Use USoundModulationDestination to safety activate and track a given modulator")
		UE_API void DeactivateBus(const USoundControlBus& InBus);

		UE_API void DeactivateBusMix(const USoundControlBusMix& InBusMix);
		UE_API void DeactivateBusMixByID(const uint32 ID);
