...
struct FSoundModulationParameterSettings
{
	...
	UPROPERTY(EditAnywhere, Category = General, BlueprintReadOnly, meta = (EditCondition = "bShowClampOption"))
...
public:
	...
	/** Returns the range to use when FSoundModulationParameterSettings::bClampToNormalizedRange is set to false.
	 *  If overriden, the Unit/Normalized conversion functions must be written in a way to account for this range.
	 *  Returned range is unitless/"normalized". */
	virtual TRange<float> GetNormalizedLimits() const
	{
	...
	UE_API bool IsModulationParameterClamped() const;
};
...
public:
	/** Minimum volume of parameter (dB). Only enforced at modulation destination. */
	UPROPERTY(EditAnywhere, Category = General, BlueprintReadOnly, meta = (ClampMax = 0.0))
	float MinVolume = -100.0f;

	/** What the maximum volume (in dB) should be if "Clamp to Normalized Range" is set to false. */
	UPROPERTY(EditAnywhere, Category = General, BlueprintReadOnly)
	float VolumeLimit = 20.0f;
	...
	virtual float GetUnitMin() const override;
	virtual float GetUnitMax() const override;
	virtual TRange<float> GetNormalizedLimits() const override;
