...
namespace AudioModulation
{
	...
	public:
		...
		static TUniquePtr<Metasound::IOperator> CreateOperator(const Metasound::FBuildOperatorParams& InParams, Metasound::FBuildResults& OutResults)
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Cannot create mix modulators node when no audio device ID supplied. Expected metasound environment variable '%ls'", *Metasound::Frontend::SourceInterface::Environment::DeviceID.ToString());
				return nullptr;
			}
		}
		...
		void Reset(const IOperator::FResetParams& InParams)
		{
			...
			{
				UE_LOGF(LogMetaSound, Warning, "Missing audio device ID environment variable (%ls) required to properly configure node (Modulation.MixModulators)", *Metasound::Frontend::SourceInterface::Environment::DeviceID.ToString());
			}
			Execute();
		}
