...
namespace AudioModulation
{
	class FProfileSerializer
	{
		...
		static bool GetMixProfileConfigPath(const FString& Name, const FString& Path, const int32 InProfileIndex, bool bInCreateDirIfMissing, FString& OutMixProfileConfig)
		{
			...
			if (!GConfig)
			{
				UE_LOGF(LogAudioModulation, Error,
					"Failed to find mix profile '%ls' config file. Config cache not initialized.",
					*Path);
				return false;
			}
			...
			{
				UE_LOGF(LogAudioModulation, Error, "Failed to %ls mix profile '%ls' config directory.",
	...
	public:
		static bool Serialize(const USoundControlBusMix& InBusMix, const int32 InProfileIndex, const FString* InMixPathOverride = nullptr)
		{
			...
			UE_LOGF(LogAudioModulation, Display, "Serialized mix to '%ls'", *Path);
		...
		static bool Deserialize(int32 InProfileIndex, USoundControlBusMix& InOutBusMix, const FString* InMixPathOverride = nullptr)
		{
			...
			if (!ConfigFile)
			{
				UE_LOGF(LogAudioModulation, Warning, "Config '%ls' not found. Mix '%ls' not loaded from config profile.", *ConfigPath , *Name);
				return false;
			}
				...
				if (!SectionsProcessed.Contains(SectionName))
				{
					...
					else
					{
						UE_LOGF(LogAudioModulation, Warning,
							"Bus missing or invalid type at '%ls'. Profile '%ls' stage not added/updated for mix '%ls'",
							*SectionName,
							*ConfigPath,
							*Path);
					}
			...
			UE_LOGF(LogAudioModulation, Display, "Deserialized mix from '%ls'", *ConfigPath);
			return true;
		}
	};
} // namespace AudioModulation
