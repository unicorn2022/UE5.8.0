...
namespace AudioModulation
{
	...
	public:
		...
		void DeactivateBusMixByID(const uint32 ID);
		 ...
		 * InOuter, when supplied, is used as the new mix's outer object so its lifetime tracks the caller's scope (typically the supplied world);
		 * the transient package is used otherwise.
		 */
		USoundControlBusMix* CreateBusMixFromValue(FName Name, const TArray<USoundControlBus*>& Buses, float Value, float AttackTime = -1.0f, float ReleaseTime = -1.0f, UObject* InOuter = nullptr);
	...
	private:
		...
		/*
		* Note: The following mirrors data from Engine/Plugins/Runtime/AudioModulationInsights
		*
		...
		* We must ensure this data is kept in sync
		*
		* Intentionally not using the EModulatorType within AudioModulationSystem.cpp:
		* 1. For the reasons above
		* 2. To allow inclusion of Control Bus Mixes
		* 3. To avoid being unintentionally stomped over by future engine changes to EModulatorType
		*/

		// Mirrors EModulatorTraceType: Engine/Plugins/Runtime/AudioModulationInsights/Source/AudioModulationInsights/Private/AudioModulationDataTypes.h
		enum class EModulatorTraceType : Audio::FModulatorTypeId
		{
			ControlBus,
			ControlBusMix,
			Generator,
		...
		void UpdateAudioInsightsTraceMessages(const double InElapsed);

		void TraceActivateModulatorMessage(const Audio::FModulatorId InModulatorId,
			const EModulatorTraceType InModulatorTraceType,
			const FString& InModulatorName);

		template <typename TModulatorProxy>
		void TraceUpdateModulatorMessage(const Audio::FModulatorId InModulatorId,
		...
		void TraceDeactivateModulatorMessage(const Audio::FModulatorId InModulatorId);

		bool bIsAudioModulationInsightsPluginEnabled { false };

		float ModulatorActivePingTimeSec { 0.0f };
		static constexpr float ModulatorActivePingIntervalTimeSec { 1.0f };

		struct AudioInsightsControlBusMixData
		{
			Audio::FModulatorId ControlBusMixId;
			Audio::FModulatorId ControlBusId;
			float Value;
		};

		TArray<Audio::FModulatorId, TInlineAllocator<32>> CurrentlyActiveModulatorIds;
		TArray<Audio::FModulatorId, TInlineAllocator<32>> PreviouslyActiveModulatorIds;
		TMap<Audio::FModulatorId, FString> ActiveModulatorPaths;
		
		TArray<float, TInlineAllocator<32>> ContributingModulatorValues;
		TArray<Audio::FModulatorId, TInlineAllocator<32>> ContributingModulatorIds;
		TArray<AudioInsightsControlBusMixData, TInlineAllocator<32>> ControlBusMixContributingDataContainer;
...
namespace AudioModulation
{
	...
	public:
		...
		USoundControlBusMix* CreateBusMixFromValue(FName Name, const TArray<USoundControlBus*>& Buses, float Value, float AttackTime = -1.0f, float ReleaseTime = -1.0f, UObject* InOuter = nullptr) { return nullptr; }
