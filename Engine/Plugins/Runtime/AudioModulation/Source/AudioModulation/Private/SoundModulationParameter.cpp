...
{
	float NewNormalizedValue = ConvertUnitToNormalized(Settings.ValueUnit);
	if (IsModulationParameterClamped())
	{
		NewNormalizedValue = FMath::Clamp(NewNormalizedValue, 0.0f, 1.0f);
	}
	if (!FMath::IsNearlyEqual(NewNormalizedValue, Settings.ValueNormalized))
	{
		Settings.ValueNormalized = NewNormalizedValue;
	}
}

void USoundModulationParameter::RefreshUnitValue()
{
	float NewUnitValue = ConvertNormalizedToUnit(Settings.ValueNormalized);
	if (IsModulationParameterClamped())
	{
		NewUnitValue = FMath::Clamp(NewUnitValue, GetUnitMin(), GetUnitMax());
	}
	if (!FMath::IsNearlyEqual(NewUnitValue, Settings.ValueUnit))
	{
		Settings.ValueUnit = NewUnitValue;
	}
}
#endif // WITH_EDITOR
...
Audio::FModulationParameter USoundModulationParameter::CreateParameter() const
{
	...
	Parameter.bClampToNormalizedRange = Settings.bClampToNormalizedRange;
...
bool USoundModulationParameter::IsModulationParameterClamped() const
{
	bool bClampToNormalizedRange = Settings.bClampToNormalizedRange;
	Audio::GetModulationParameterClampValue(GetFName(), bClampToNormalizedRange);
...
USoundModulationParameterVolume::USoundModulationParameterVolume(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
#if WITH_EDITORONLY_DATA
	Settings.bShowClampOption = true;
#endif
}
...
{
	return [InUnitMin = GetUnitMin(), InUnitMax = VolumeLimit, bClampToNormalized = IsModulationParameterClamped()](float& InOutValue)
	{
		InOutValue = InOutValue > 0.0f
			? Audio::ConvertToDecibels(InOutValue)
			: InUnitMin;

		if (bClampToNormalized)
		{
			InOutValue = FMath::Clamp(InOutValue, InUnitMin, 0.0f);
		}
		else
		{
			InOutValue = FMath::Clamp(InOutValue, InUnitMin, InUnitMax);
		}
	};
}
...
Audio::FModulationNormalizedConversionFunction USoundModulationParameterVolume::GetNormalizedConversionFunction() const
{
	return [InUnitMin = GetUnitMin(), InUnitMax = VolumeLimit, bClampToNormalized = IsModulationParameterClamped()](float& InOutValue)
	{
		...
		if (bClampToNormalized)
		{
			InOutValue = FMath::Clamp(InOutValue, 0, 1.0f);
		}
		else
		{
			InOutValue = FMath::Clamp(InOutValue, 0, Audio::ConvertToLinear(InUnitMax));
		}
	};
}
...
TRange<float> USoundModulationParameterVolume::GetNormalizedLimits() const
{
	return TRange<float>(0.0f, Audio::ConvertToLinear(VolumeLimit));
}
...
#if WITH_EDITOR
void USoundModulationParameterVolume::PostEditChangeProperty(FPropertyChangedEvent& InPropertyChangedEvent)
{
	VolumeLimit = FMath::Max(VolumeLimit, MinVolume);
	...
	const Audio::FModulationParameter& GetOrRegisterParameter(const USoundModulationParameter* InParameter, const FString& InName, const FString& InClassName)
	{
		...
		if (InParameter)
		{
			...
			if (!Audio::IsModulationParameterRegistered(ParamName))
			{
				...
				UE_LOGF(LogAudioModulation, Display,
					"Parameter '%ls' not registered.  Registration forced via '%ls'.",
					*ParamName.ToString(),
					*Breadcrumb);
