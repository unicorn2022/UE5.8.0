...
void USoundControlBusMix::BeginDestroy()
{
	...
	{
		DeviceManager->IterateOverAllDevices([this](::Audio::FDeviceId DeviceId, FAudioDevice* AudioDevice)
			{
				...
				|| (Property->GetFName() == GET_MEMBER_NAME_CHECKED(FSoundControlBusMixStage, Bus)))
			{
				for (FSoundControlBusMixStage& Stage : MixStages)
				{
					if (Stage.Bus)
					{
						...
						{
							// Clamp the normalized values--either to the normalized [0,1] range or to the parameter-specified "unclamped" range.
							// The normalized value is not clamped in the ConvertNormalizedToUnit function below, so we do it here.
							if (!Parameter->IsModulationParameterClamped())
							{
								const TRange<float> Range = Parameter->GetNormalizedLimits();
								Stage.Value.TargetValue = FMath::Clamp(Stage.Value.TargetValue, Range.GetLowerBoundValue(), Range.GetUpperBoundValue());
							}
							else
							{
