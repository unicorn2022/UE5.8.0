...
void UAudioModulationSettings::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	if (PropertyChangedEvent.Property)
	{
		...
		{
			UE_LOGF(LogAudioModulation, Display, "Reloading Parameters From Modulation Developer Settings...");
			RegisterParameters();
		}
	}
}
...
void UAudioModulationSettings::RegisterParameters() const
{
	...
	for (const FSoftObjectPath& Path : Parameters)
	{
		if (USoundModulationParameter* Param = Cast<USoundModulationParameter>(Path.TryLoad()))
		{
			...
			UE_LOGF(LogAudioModulation, Display, "Initialized Audio Modulation Parameter '%ls'", *NewParam.ParameterName.ToString());
		}
		else
		{
			const FString& ParamName = Path.GetAssetName();
			UE_LOGF(LogAudioModulation, Error, "Failed to load parameter at '%ls': Missing asset or invalid type.", *ParamName);
			...
			if (ParamName == "HPFCutoffFrequency")
			{
				...
				Audio::RegisterModulationParameter(NewParam.ParameterName, MoveTemp(NewParam));
				UE_LOGF(LogAudioModulation, Error, "Created temporary HPFCutoffFrequency parameter to use instead.");
			}
			else if (ParamName == "LPFCutoffFrequency")
			{
				...
				Audio::RegisterModulationParameter(NewParam.ParameterName, MoveTemp(NewParam));
				UE_LOGF(LogAudioModulation, Error, "Created temporary LPFCutoffFrequency parameter to use instead.");
			}
			else if (ParamName == "Pitch")
			{
				...
				Audio::RegisterModulationParameter(NewParam.ParameterName, MoveTemp(NewParam));
				UE_LOGF(LogAudioModulation, Error, "Created temporary Pitch parameter to use instead.");
			}
			else if (ParamName == "Volume")
			{
				...
				Audio::RegisterModulationParameter(NewParam.ParameterName, MoveTemp(NewParam));
				UE_LOGF(LogAudioModulation, Error, "Created temporary Volume parameter to use instead.");
			}
		}
	}
}
