...
static FAutoConsoleCommand GModulationSaveMixProfile(
	...
	FConsoleCommandWithArgsDelegate::CreateStatic(
		[](const TArray<FString>& Args)
		{
			if (Args.Num() < 1)
			{
				UE_LOGF(LogAudioModulation, Error, "Failed to save mix profile: Path not provided");
				return;
			}
			...
			UE_LOGF(LogAudioModulation, Error, "Failed to save mix '%ls' to profile index '%i'", *Path, ProfileIndex);
		}
	)
);
...
static FAutoConsoleCommand GModulationLoadMixProfile(
	...
	FConsoleCommandWithArgsDelegate::CreateStatic(
		[](const TArray<FString>& Args)
		{
			if (Args.Num() < 1)
			{
				UE_LOGF(LogAudioModulation, Error, "Failed to load mix profile: Object path not provided");
				return;
			}
			...
			UE_LOGF(LogAudioModulation, Error, "Failed to load mix '%ls' from profile index '%i'", *Path, ProfileIndex);
		}
	)
);
...
USoundControlBusMix* UAudioModulationStatics::CreateBusMix(UObject* WorldContextObject, FName Name, TArray<FSoundControlBusMixStage> Stages, bool Activate, double Duration, bool bRetriggerOnActivation)
{
	...
	for (FSoundControlBusMixStage& Stage : Stages)
	{
		...
		{
			UE_LOGF(LogAudioModulation, Warning,
				"USoundControlBusMix '%ls' was created but bus provided is null. Stage not added to mix.",
				*Name.ToString());
		}
	}
...
void UAudioModulationStatics::DeactivateBusMixByID(const UObject* WorldContextObject, uint32 MixID)
{
	UWorld* World = GetAudioWorld(WorldContextObject);
	if (AudioModulation::FAudioModulationManager* ModSystem = GetModulation(World))
	{
		ModSystem->DeactivateBusMixByID(MixID);
	}
}
...
USoundControlBusMix* UAudioModulationStatics::CreateBusMixFromValue(const UObject* WorldContextObject, FName Name, const TArray<USoundControlBus*>& Buses, float Value, float AttackTime, float ReleaseTime, bool bActivate)
{
	...
	if (AudioModulation::FAudioModulationManager* ModSystem = GetModulation(World))
	{
		// Outer the new bus mix to the resolved world so its lifetime is bound to that world.
		// Without this it would land in the transient package and survive PIE/world teardown,
		// continuing to drive bus values past the point the caller expects it to deactivate.
		if (USoundControlBusMix* NewMix = ModSystem->CreateBusMixFromValue(Name, Buses, Value, AttackTime, ReleaseTime, World))
		{
			if (bActivate)
			{
