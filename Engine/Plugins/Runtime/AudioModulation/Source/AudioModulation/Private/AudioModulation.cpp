...
namespace AudioModulation
{
	...
	void FAudioModulationManager::GetControlBusAssetPath(TArray<FTopLevelAssetPath>& OutClassNames)
	{
		OutClassNames.Add(USoundControlBus::StaticClass()->GetClassPathName());
	}

	void FAudioModulationManager::ActivateBus(const USoundControlBus& InBus)
	{
		UE_LOGF(LogAudioModulation, VeryVerbose, "FAudioModulationManager::ActivateBus is deprecated. Use UAudioModulationDestination API (see SetModulator) instead to enforce bus lifetime");
	...
	{
		UE_LOGF(LogAudioModulation, VeryVerbose, "FAudioModulationManager::ActivateGenerator is deprecated. Use UAudioModulationDestination API (see SetModulator) instead to enforce generator lifetime");
	...
	USoundControlBusMix* FAudioModulationManager::CreateBusMixFromValue(FName Name, const TArray<USoundControlBus*>& Buses, float Value, float AttackTime, float ReleaseTime, UObject* InOuter)
	{
		return ModSystem->CreateBusMixFromValue(Name, Buses, Value, AttackTime, ReleaseTime, InOuter);
	}

	void FAudioModulationManager::DeactivateBus(const USoundControlBus& InBus)
	{
		UE_LOGF(LogAudioModulation, VeryVerbose, "FAudioModulationManager::DeactivateBus is deprecated. Use UAudioModulationDestination API (see ClearModulator) instead to enforce bus lifetime.");
	...
	void FAudioModulationManager::DeactivateBusMixByID(const uint32 ID)
	{
		ModSystem->DeactivateBusMixByID(ID);
	}
	...
	{
		UE_LOGF(LogAudioModulation, VeryVerbose, "FAudioModulationManager::DeactivateGenerator is deprecated. Use UAudioModulationDestination API (see ClearModulator) instead to enforce generator lifetime");
...
void FAudioModulationModule::StartupModule()
{
...
#if WITH_AUDIOMODULATION_METASOUND_SUPPORT
	UE_LOGF(LogAudioModulation, Log, "Registering Modulation MetaSound Nodes...");
	...
	UE_LOGF(LogAudioModulation, Log, "Audio Modulation Initialized");
}
...
#if WITH_AUDIOMODULATION_METASOUND_SUPPORT
	UE_LOGF(LogAudioModulation, Log, "Unregistering Modulation MetaSound Nodes...");
	METASOUND_UNREGISTER_ITEMS_IN_MODULE
#endif // WITH_AUDIOMODULATION_METASOUND_SUPPORT

	UE_LOGF(LogAudioModulation, Log, "Audio Modulation Shutdown");
}
