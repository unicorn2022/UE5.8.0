...
bool FWebMAudioBackendSDL::InitializePlatform()
{
	...
	if (Result < 0)
	{
		UE_LOGF(LogWebMMoviePlayer, Error, "SDL_InitSubSystem create failed: %d", Result);
		bSDLInitialized = false;
		return false;
	}
	else
...
bool FWebMAudioBackendSDL::StartStreaming(int32 SampleRate, int32 NumOfChannels, EStreamState StreamState)
{
	...
	if (!AudioStream)
	{
		UE_LOGF(LogWebMMoviePlayer, Error, "SDL_OpenAudioDevice failed");
		return false;
	}
	else
	{
...
bool FWebMAudioBackendSDL::SendAudio(const FTimespan& Timespan, const uint8* Buffer, size_t BufferSize)
{
	...
	if (Result < 0)
	{
		UE_LOGF(LogWebMMoviePlayer, Error, "SDL_QueueAudio failed: %d", Result);
		return false;
	}
	else
	{
