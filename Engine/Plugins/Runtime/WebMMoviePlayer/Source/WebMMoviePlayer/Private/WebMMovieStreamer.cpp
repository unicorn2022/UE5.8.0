...
bool FWebMMovieStreamer::StartNextMovie()
{
	...
	if (MovieQueue.Num() > 0)
	{
		...
		{
			UE_LOGF(LogWebMMoviePlayer, Error, "Movie '%ls' not found.", *MoviePath);

			MovieName = FString();
			return false;
		}

		UE_LOGF(LogWebMMoviePlayer, Log, "Starting '%ls'", *MoviePath);
	...
	{
		UE_LOGF(LogWebMMoviePlayer, Log, "No Movie to start.");
		return false;
	}
}
...
{
	UE_LOGF(LogWebMMoviePlayer, Log, "FWebMMovieStreamer::HandleApplicationWillEnterBackground, pausing - we were %ls", bPaused ? TEXT("paused") : TEXT("not paused"));
...
{
	UE_LOGF(LogWebMMoviePlayer, Log, "FWebMMovieStreamer::HandleApplicationHasEnteredForeground, unpausing - we were %ls", bPaused ? TEXT("paused") : TEXT("not paused"));
	...
	if (bPaused)
	{
		...
		bPaused = false;
		UE_LOGF(LogWebMMoviePlayer, Log, "Spent %f seconds in background, adjusting start time from %f to %f", TimeSpentInBackground, PrevStartTimeForLog, StartTime);
	}
}
