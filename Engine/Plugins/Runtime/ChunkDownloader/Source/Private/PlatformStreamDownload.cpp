	...
	Request->OnProcessRequestComplete().BindLambda([Callback, TargetFile, SizeOnDisk](FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSuccess) {
		...
		if (HttpResponse.IsValid())
		{
			...
			if (bIsPartialContent)
			{
				...
				{
					UE_LOGF(LogChunkDownloader, Error, "Content-Range for %ls was '%ls' but expected '%ls' prefix", *HttpRequest->GetURL(), *HeaderValue, *ExpectedHeaderPrefix);
					bHeadersOk = false;
				}
			}
			...
			if (bHeadersOk)
			{
				...
				if (ManifestFile != nullptr)
				{
					...
					if (!bSuccess)
					{
						UE_LOGF(LogChunkDownloader, Error, "Write error writing to %ls", *TargetFile);
				...
				{
					UE_LOGF(LogChunkDownloader, Error, "Unable to save file to %ls", *TargetFile);
			...
			{
				UE_LOGF(LogChunkDownloader, Error, "HTTP %d returned from '%ls'", HttpStatus, *HttpRequest->GetURL());
		...
		{
			UE_LOGF(LogChunkDownloader, Error, "HTTP connection issue downloading '%ls'", *HttpRequest->GetURL());
		}
