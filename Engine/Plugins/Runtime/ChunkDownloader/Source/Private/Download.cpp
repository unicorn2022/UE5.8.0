...
{
	check(!bHasCompleted);
	UE_LOGF(LogChunkDownloader, Warning, "Canceling download of '%ls'. result=%ls", *PakFile->Entry.FileName, bResult ? TEXT("true") : TEXT("false"));
...
bool FDownload::ValidateFile() const
{
	...
	{
		UE_LOGF(LogChunkDownloader, Error, "Size mismatch. Expected %llu, got %llu", PakFile->Entry.FileSize, PakFile->SizeOnDisk);
		return false;
	}
	...
	if (PakFile->Entry.FileVersion.StartsWith(TEXT("SHA1:")))
	{
		...
		{
			UE_LOGF(LogChunkDownloader, Error, "Checksum mismatch. Expected %ls", *PakFile->Entry.FileVersion);
			return false;
		}
	}
...
bool FDownload::HasDeviceSpaceRequired() const
{
	...
	if (FPlatformMisc::GetDiskTotalAndFreeSpace(Downloader->CacheFolder, TotalDiskSpace, TotalDiskFreeSpace))
	{
		...
		{
			// not enough space
			UE_LOGF(LogChunkDownloader, Warning, "Unable to download '%ls'. Needed %llu bytes had %llu bytes free (of %llu bytes)",
...
void FDownload::StartDownload(int TryNumber)
{
	...
	UE_LOGF(LogChunkDownloader, Log, "Downloading %ls from %ls", *PakFile->Entry.FileName, *Url);
	TWeakPtr<FDownload> WeakThisPtr = AsShared();
...
void FDownload::OnDownloadComplete(const FString& Url, int TryNumber, int32 HttpStatus)
{
	...
	if (EHttpResponseCodes::IsOk(HttpStatus))
	{
		...
		UE_LOGF(LogChunkDownloader, Error, "%ls from %ls failed validation", *TargetFile, *Url);
	...
	UE_LOGF(LogChunkDownloader, Log, "Will re-attempt to download %ls in %f seconds", *PakFile->Entry.FileName, SecondsToDelay);
	TWeakPtr<FDownload> WeakThisPtr = AsShared();
