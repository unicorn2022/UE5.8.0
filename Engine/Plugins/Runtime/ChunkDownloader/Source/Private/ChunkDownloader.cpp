...
bool FChunkDownloader::CheckFileSha1Hash(const FString& FullPathOnDisk, const FString& Sha1HashStr)
{
	...
	if (FilePtr == nullptr)
	{
		UE_LOGF(LogChunkDownloader, Error, "Unable to open %ls for hash verify.", *FullPathOnDisk);
		return false;
	}
	...
	// read in 64K chunks to prevent raising the memory high water mark too much
	{
		...
		for (int64 Pointer = 0; Pointer<FileSize;)
		{
			...
			{
				UE_LOGF(LogChunkDownloader, Error, "Read error while validating '%ls' at offset %lld.", *FullPathOnDisk, Pointer);
...
static bool WriteStringAsUtf8TextFile(const FString& FileText, const FString& FilePath)
{
	...
	if (ManifestFile != nullptr)
	{
		...
		{
			UE_LOGF(LogChunkDownloader, Log, "Wrote to %ls", *FilePath);
			bSuccess = true;
		}
		else
		{
			UE_LOGF(LogChunkDownloader, Error, "Write error writing to %ls", *FilePath);
		}
	...
	{
		UE_LOGF(LogChunkDownloader, Error, "Unable open %ls for writing.", *FilePath);
	}
	return bSuccess;
}
	...
	void DoWork()
	{
		...
		if (FCoreDelegates::MountPak.IsBound())
		{
			...
			for (const TSharedRef<FPakFile>& PakFile : PakFiles)
			{
				...
				{
					UE_LOGF(LogChunkDownloader, Error, "Unable to mount %ls from chunk %d (mount operation failed)", *FullPathOnDisk, ChunkId);
				}
			}
		}
		else
		{
			UE_LOGF(LogChunkDownloader, Error, "Unable to mount chunk %d (no FCoreDelegates::MountPak bound)", ChunkId);
		}
	}
...
static TArray<FPakFileEntry> ParseManifest(const FString& ManifestPath, TMap<FString,FString>* Properties = nullptr)
{
	...
	if (ManifestFile != nullptr)
	{
		...
		if (FileSize > 0)
		{
			UE_LOGF(LogChunkDownloader, Log, "Found manifest at %ls", *ManifestPath);
			...
			{
				UE_LOGF(LogChunkDownloader, Error, "Read error loading manifest at %ls", *ManifestPath);
			}
		}
		else
		{
			UE_LOGF(LogChunkDownloader, Log, "Empty manifest found at %ls", *ManifestPath);
		}
	...
	{
		UE_LOGF(LogChunkDownloader, Log, "No manifest found at %ls", *ManifestPath);
	}

	if (ExpectedEntries >= 0 && ExpectedEntries != Entries.Num())
	{
		UE_LOGF(LogChunkDownloader, Error, "Corrupt manifest at %ls (expected %d entries, got %d)", *ManifestPath, ExpectedEntries, Entries.Num());
		Entries.Empty();
		if (Properties != nullptr)
		{
			Properties->Empty();
...
void FChunkDownloader::Initialize(const FString& InPlatformName, int32 TargetDownloadsInFlightIn)
{
	...
	UE_LOGF(LogChunkDownloader, Display, "Initializing with platform='%ls'", *InPlatformName);
	...
	{
		UE_LOGF(LogChunkDownloader, Error, "Unable to create cache folder at '%ls'", *CacheFolder);
	}
		...
		for (const FPakFileEntry& Entry : LocalManifest)
		{
			...
			if (SizeOnDiskInt > 0)
			{
				...
				if (FileInfo->SizeOnDisk > Entry.FileSize)
				{
					...
					UE_LOGF(LogChunkDownloader, Warning, "Found '%ls' on disk with size larger than LocalManifest indicates", *LocalPath);
			...
			else
			{
				...
				UE_LOGF(LogChunkDownloader, Log, "'%ls' appears in LocalManifest but is not on disk (not necessarily a problem)", *LocalPath);
				bNeedsManifestSave = true;
			}
	...
	for (FString Orphan : StrayFiles)
	{
		...
		UE_LOGF(LogChunkDownloader, Log, "Deleting orphaned file '%ls'", *FullPathOnDisk);
		if (!ensure(FileManager.Delete(*FullPathOnDisk)))
		{
			// log an error (best we can do)
			UE_LOGF(LogChunkDownloader, Error, "Unable to delete '%ls'", *FullPathOnDisk);
		}
	}
...
void FChunkDownloader::SetContentBuildId(const FString& DeploymentName, const FString& NewContentBuildId)
{
	...
	UE_LOGF(LogChunkDownloader, Display, "Deployment = %ls, ContentBuildId = %ls", *DeploymentName, *ContentBuildId);
	...
	if (CdnBaseUrls.Num() <= 0)
	{
		...
		if (CdnBaseUrls.Num() <= 0)
		{
			UE_LOGF(LogChunkDownloader, Warning, "No CDN base URLs configured in [%ls]. Chunk downloading will only be able to use embedded cache.", *ConfigSectionName);
		}
	}
	...
	for (int32 i=0,n=CdnBaseUrls.Num();i<n;++i)
	{
		...
		UE_LOGF(LogChunkDownloader, Display, "ContentBaseUrl[%d] = %ls", i, *BuildUrl);
...
{
	UE_LOGF(LogChunkDownloader, Display, "Finalizing.");
...
void FChunkDownloader::WaitForMounts()
{
	...
	for (const auto& It : Chunks)
	{
		...
		if (Chunk->MountTask != nullptr)
		{
			if (!bWaiting)
			{
				UE_LOGF(LogChunkDownloader, Display, "Waiting for chunk mounts to complete...");
				bWaiting = true;
			}
	...
	if (bWaiting)
	{
		UE_LOGF(LogChunkDownloader, Display, "...chunk mounts finished.");
	}
}
...
void FChunkDownloader::UnmountPakFile(const TSharedRef<FPakFile>& PakFile)
{
	...
	if (PakFile->bIsMounted)
	{
		...
		if (ensure(FCoreDelegates::OnUnmountPak.IsBound()))
		{
			...
			{
				UE_LOGF(LogChunkDownloader, Error, "Unable to unmount %ls", *FullPathOnDisk);
			}
		}
		else
		{
			UE_LOGF(LogChunkDownloader, Error, "Unable to unmount %ls because no OnUnmountPak is bound", *PakFile->Entry.FileName);
		}
	}
}
...
void FChunkDownloader::DumpLoadedChunks()
{
	...
	UE_LOGF(LogChunkDownloader, Display, "Dumping loaded chunk status\n--------------------------");
	for (int32 ChunkId : ChunkIdList)
	{
		auto ChunkStatus = ChunkDownloader->GetChunkStatus(ChunkId);
		UE_LOGF(LogChunkDownloader, Display, "Chunk #%d => %ls", ChunkId, ChunkStatusToString(ChunkStatus));
	}
#endif
}
...
int FChunkDownloader::FlushCache()
{
	...
	UE_LOGF(LogChunkDownloader, Display, "Flushing chunk caches at %ls", *CacheFolder);
		...
		if (bDownloadPending)
		{
			for (const TSharedRef<FPakFile>& PakFile : Chunk->PakFiles)
			{
				if (PakFile->SizeOnDisk > 0)
				{
					...
					UE_LOGF(LogChunkDownloader, Warning, "Could not flush %ls (chunk %d) due to download in progress.", *PakFile->Entry.FileName, Chunk->ChunkId);
					++FilesSkipped;
				}
			}
		}
		else
		{
			...
			for (const TSharedRef<FPakFile>& PakFile : Chunk->PakFiles)
			{
				if (PakFile->SizeOnDisk > 0 && !PakFile->bIsEmbedded)
				{
					...
					if (ensure(FileManager.Delete(*FullPathOnDisk)))
					{
						UE_LOGF(LogChunkDownloader, Log, "Deleted %ls (chunk %d).", *FullPathOnDisk, Chunk->ChunkId);
						++FilesDeleted;
	...
	UE_LOGF(LogChunkDownloader, Display, "Chunk cache flush complete. %d files deleted. %d files skipped.", FilesDeleted, FilesSkipped);
	return FilesSkipped;
}
...
int FChunkDownloader::ValidateCache()
{
	...
	UE_LOGF(LogChunkDownloader, Display, "Starting inline chunk validation.");
	...
	for (const auto& It : PakFiles)
	{
		...
		if (PakFile->bIsCached && !PakFile->bIsEmbedded)
		{
			...
			else
			{
				...
				UE_LOGF(LogChunkDownloader, Warning, "Unable to validate %ls with version '%ls'.", *PakFile->Entry.FileName, *PakFile->Entry.FileVersion);
				++SkippedFiles;
				continue;
			}
			...
			{
				// log valid
				UE_LOGF(LogChunkDownloader, Log, "%ls matches hash '%ls'.", *PakFile->Entry.FileName, *PakFile->Entry.FileVersion);
				++ValidFiles;
			}
			else
			{
				// log invalid
				UE_LOGF(LogChunkDownloader, Warning, "%ls does NOT match hash '%ls'.", *PakFile->Entry.FileName, *PakFile->Entry.FileVersion);
				++InvalidFiles;
				...
				if (ensure(FileManager.Delete(*FullPathOnDisk)))
				{
					UE_LOGF(LogChunkDownloader, Log, "Deleted invalid pak %ls (chunk %d).", *FullPathOnDisk, PakFile->Entry.ChunkId);
					PakFile->bIsCached = false;
					PakFile->SizeOnDisk = 0;
					bNeedsManifestSave = true;
				}
	...
	UE_LOGF(LogChunkDownloader, Display, "Chunk validation complete. %d valid, %d invalid, %d skipped", ValidFiles, InvalidFiles, SkippedFiles);
	return InvalidFiles;
}
...
void FChunkDownloader::BeginLoadingMode(const FCallback& Callback)
{
	...
	if (PostLoadCallbacks.Num() > 0)
	{
		UE_LOGF(LogChunkDownloader, Log, "JoinLoadingMode");
	...
	// start loading mode
	UE_LOGF(LogChunkDownloader, Log, "BeginLoadingMode");
#if PLATFORM_ANDROID || PLATFORM_IOS
...
bool FChunkDownloader::UpdateLoadingMode()
{
		...
		LoadingModeStats.ChunksMounted >= LoadingModeStats.TotalChunksToMount)
	{
		...
		{
			// end loading mode
			UE_LOGF(LogChunkDownloader, Log, "EndLoadingMode (%d files downloaded, %d chunks mounted)", LoadingModeStats.FilesDownloaded, LoadingModeStats.ChunksMounted);
#if PLATFORM_ANDROID || PLATFORM_IOS
...
void FChunkDownloader::TryLoadBuildManifest(int TryNumber)
{
	...
	if (CachedManifestProps.FindOrAdd(BUILD_ID_KEY) != ContentBuildId)
	{
		...
		{
			UE_LOGF(LogChunkDownloader, Error, "Unable to download build manifest. No CDN urls configured.");
		...
		UE_LOGF(LogChunkDownloader, Log, "Will re-attempt manifest download in %f seconds", SecondsToDelay);
		TWeakPtr<FChunkDownloader> WeakThisPtr = AsShared();
...
void FChunkDownloader::TryDownloadBuildManifest(int TryNumber)
{
	...
	UE_LOGF(LogChunkDownloader, Log, "Downloading build manifest (attempt #%d) from %ls", TryNumber+1, *Url);
	...
	ManifestRequest->OnProcessRequestComplete().BindLambda([WeakThisPtr, TryNumber, CachedManifestFullPath](FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSuccess) {
		...
		if (bSuccess && HttpResponse.IsValid())
		{
			...
			if (EHttpResponseCodes::IsOk(HttpStatus))
			{
				...
				{
					UE_LOGF(LogChunkDownloader, Error, "Failed to write manifest to '%ls'", *CachedManifestFullPath);
					LastError = FText::Format(LOCTEXT("FailedToWriteManifest", "[Try {0}] Failed to write manifest."), FText::AsNumber(TryNumber));
				}
			}
			else
			{
				UE_LOGF(LogChunkDownloader, Error, "HTTP %d while downloading manifest from '%ls'", HttpStatus, *HttpRequest->GetURL());
		...
		{
			UE_LOGF(LogChunkDownloader, Error, "HTTP connection issue while downloading manifest '%ls'", *HttpRequest->GetURL());
		...
		if (!SharedThis.IsValid())
		{
			UE_LOGF(LogChunkDownloader, Warning, "FChunkDownloader was destroyed while downloading manifest '%ls'", *HttpRequest->GetURL());
			return;
		}
...
{
	UE_LOGF(LogChunkDownloader, Display, "Beginning manifest load.");
	...
	for (const auto& It : Manifest)
	{
		...
		UE_LOGF(LogChunkDownloader, Verbose, "Found chunk %d (%d pak files).", ChunkId, Chunk->PakFiles.Num());
		++NumChunks;
	...
	{
		const TSharedRef<FPakFile>& File = It.Value;
		UE_LOGF(LogChunkDownloader, Log, "Removing orphaned pak file %ls (was chunk %d).", *File->Entry.FileName, File->Entry.ChunkId);
		...
		if (File->SizeOnDisk > 0 && !File->bIsEmbedded)
		{
			...
			if (!ensure(FileManager.Delete(*FullPathOnDisk)))
			{
				UE_LOGF(LogChunkDownloader, Error, "Failed to delete orphaned pak %ls.", *FullPathOnDisk);
			}
		}
	}
	...
	UE_LOGF(LogChunkDownloader, Display, "Manifest load complete. %d chunks with %d pak files.", NumChunks, NumPaks);
}

void FChunkDownloader::DownloadChunkInternal(const FChunk& Chunk, const FCallback& Callback, int32 Priority)
{
	UE_LOGF(LogChunkDownloader, Log, "Chunk %d download requested.", Chunk.ChunkId);
	...
	if (BuildBaseUrls.Num() <= 0)
	{
		UE_LOGF(LogChunkDownloader, Error, "Unable to download Chunk %d (no CDN urls).", Chunk.ChunkId);
		ExecuteNextTick(Callback, false);
		return;
	}
...
void FChunkDownloader::MountChunkInternal(FChunk& Chunk, const FCallback& Callback)
{
	...
	if (bAllPaksCached)
	{
		...
		UE_LOGF(LogChunkDownloader, Log, "Chunk %d mount requested (%d pak sequence).", Chunk.ChunkId, Chunk.PakFiles.Num());
...
void FChunkDownloader::IssueDownloads()
{
	for (int32 i = 0; i < DownloadRequests.Num() && i < TargetDownloadsInFlight; ++i)
	{
		...
		UE_LOGF(LogChunkDownloader, Log, "Pak file %ls download requested (%ls).",
...
void FChunkDownloader::CompleteMountTask(FChunk& Chunk)
{
	...
	if (Chunk.bIsMounted)
	{
		UE_LOGF(LogChunkDownloader, Log, "Chunk %d mount succeeded.", Chunk.ChunkId);
	}
	else
	{
		UE_LOGF(LogChunkDownloader, Error, "Chunk %d mount failed.", Chunk.ChunkId);
	}
...
void FChunkDownloader::DownloadChunk(int32 ChunkId, const FCallback& Callback, int32 Priority)
{
	...
	if (ChunkPtr == nullptr || (*ChunkPtr)->PakFiles.Num() <= 0)
	{
		...
		UE_LOGF(LogChunkDownloader, Warning, "Ignoring download request for chunk %d (no mapped pak files).", ChunkId);
		ExecuteNextTick(Callback, true);
		return;
	}
	const FChunk& Chunk = **ChunkPtr;
...
void FChunkDownloader::MountChunk(int32 ChunkId, const FCallback& Callback)
{
	...
	if (ChunkPtr == nullptr || (*ChunkPtr)->PakFiles.Num() <= 0)
	{
		// a chunk that doesn't exist or one with no pak files are both considered "complete" for the purposes of this call
		// use GetChunkStatus to differentiate from chunks that mounted successfully
		UE_LOGF(LogChunkDownloader, Warning, "Ignoring mount request for chunk %d (no mapped pak files).", ChunkId);
		ExecuteNextTick(Callback, true);
		return;
	}
