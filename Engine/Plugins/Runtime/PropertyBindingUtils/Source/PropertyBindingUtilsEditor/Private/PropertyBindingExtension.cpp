...
#define LOCTEXT_NAMESPACE "PropertyBindingExtension"
	...
	, const FGuid InFallbackStructID)
{
	...
	auto ExtractStructIdFromPropertyHandle = [&StructID](const TSharedPtr<const IPropertyHandle>& ParentHandle)
	{
		if (const FString* IDString = ParentHandle->GetInstanceMetaData(MetaDataStructIDName))
		{
	...
	while (CurrentPropertyHandle.IsValid())
	{
		if (const FProperty* Property = CurrentPropertyHandle->GetProperty())
		{
			...
			if (ExtractStructIdFromPropertyHandle(CurrentPropertyHandle))
			{
		...
		TSharedPtr<IPropertyHandle> ParentHandle = CurrentPropertyHandle->GetParentHandle();
		if (ParentHandle)
		{
			// Let's check if a property is acting as the root of the property path
			if (ParentHandle->HasMetaData(MetaDataBindingRoot))
			{
				if (ExtractStructIdFromPropertyHandle(ParentHandle))
				{
					BindablePropertyHandle = CurrentPropertyHandle;
				}
		...
		CurrentPropertyHandle = MoveTemp(ParentHandle);
	}
...
{
	return GetPropertyTypeText(GetDefault<UEdGraphSchema_K2>(), Property);
}

FText GetPropertyTypeText(TNotNull<const UEdGraphSchema_K2*> Schema, const FProperty* Property)
{
...
TSharedRef<SWidget> MakeBindingPropertyInfoWidget(TNotNull<const UEdGraphSchema_K2*> Schema, const FText& InDisplayText, const FEdGraphPinType& InPinType)
{
	const FSlateBrush* Icon = FBlueprintEditorUtils::GetIconFromPin(InPinType, /*bIsLarge*/true);
	const FLinearColor IconColor = Schema->GetPinTypeColor(InPinType);
...
FCachedBindingData::FPinInfo FCachedBindingData::GetPinInfoForProperty(const TNotNull<const FProperty*> InProperty, FPropertyBindingDataView InTargetDataView) const
{
	FPinInfo Info;
	const UEdGraphSchema_K2* Schema = GetEdGraphSchema();
	Schema->ConvertPropertyToPinType(InProperty, Info.Type);
	Info.IconBrush = FAppStyle::GetBrush("Kismet.Tabs.Variables");
	Info.IconColor = Schema->GetPinTypeColor(Info.Type);
	return Info;
}
...
void FCachedBindingData::UpdateData()
{
	...
	FPinInfo PinInfo = GetPinInfoForProperty(Property, TargetDataView);
	...
	if (const FPropertyBindingBinding* CurrentBinding = EditorBindings->FindBinding(TargetPath))
	{
		...
		if (BindingOwner->GetBindableStructByID(SourcePath.GetStructID(), SourceDesc))
		{
			...
			if (bIsValidBinding)
			{
				...
				if (bIsPropertyReference && SourceLeafProperty)
				{
					Schema->ConvertPropertyToPinType(SourceLeafProperty, PinInfo.Type);
					PinInfo.IconColor = Schema->GetPinTypeColor(PinInfo.Type);
				}

				Image = PinInfo.IconBrush;
				Color = PinInfo.IconColor;
			...
			else
			{
				...
				{
					SourceType = GetPropertyTypeText(Schema, SourceLeafProperty);
				}
				else if (SourceStruct)
				{
					SourceType = SourceStruct->GetDisplayNameText();
				}
				FText TargetType = GetPropertyTypeText(Schema, Property);
	...
	else
	{
		// No bindings
		FormatableText = FText::GetEmpty();
		TooltipBuilder.AppendLineFormat(LOCTEXT("BindTooltip", "Bind {0} to value from another property."), UE::PropertyBinding::GetPropertyTypeText(GetEdGraphSchema(), Property));
...
void FCachedBindingData::PromoteToParameter(const FName InPropertyName, TConstStructView<FPropertyBindingBindableStructDescriptor> InStructDesc, const TSharedPtr<const FPropertyInfoOverride> InPropertyInfoOverride)
{
	...
	if (BindingsOwner->GetBindingDataViewByID(TargetPath.GetStructID(), TargetDataView) && TargetDataView.IsValid())
	{
		TArray<FPropertyBindingPathIndirection> TargetIndirections;
		FString Error;
		const bool bResolved = TargetPath.ResolveIndirectionsWithValue(TargetDataView, TargetIndirections, &Error);

		if (ensureMsgf(bResolved, TEXT("Failed to resolve indirections: %s"), *Error))
		{
...
{
	return DeterminePropertyAndContextStructCompatibleInternal(SourceStruct, TargetProperty);
}

bool FCachedBindingData::DeterminePropertyAndContextStructCompatibleInternal(const UStruct* SourceStruct, const FProperty* TargetProperty) const
{
...
TNotNull<const UEdGraphSchema_K2*> FCachedBindingData::GetEdGraphSchema() const
{
...
bool IsPropertyBindable(const FProperty& Property)
{
	...
	if (!bIsUserEditable)
	{
		UE_LOGF(LogPropertyBindingUtils, Verbose, "Property %ls is not bindable because it's not user-settable in the editor",
			*Property.GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogPropertyBindingUtils, Verbose, "Property %ls is not bindable because it's either private or protected and not private-accessible to blueprints",
			*Property.GetName());
		return false;
	}
						...
						if (InCachedBindingData->CanCreateParameter(ContextStruct, /*out*/PropertyInfoOverrides))
						{
							...
							if (PropertyInfoOverrides.IsEmpty())
							{
								InMenuBuilder.AddMenuEntry(FExecuteAction::CreateLambda(
									...
									MakeContextStructWidget(InCachedBindingData->GetEdGraphSchema(), ContextStruct));
							}
							else
							{
								InMenuBuilder.AddSubMenu(MakeContextStructWidget(InCachedBindingData->GetEdGraphSchema(), ContextStruct),
									FNewMenuDelegate::CreateLambda([InCachedBindingData, PropertyName, ContextStruct, PropertyInfoOverrides = MoveTemp(PropertyInfoOverrides), PromoteToParameterAndRefresh](FMenuBuilder& InSubMenuBuilder)
										{
											...
											for (const TSharedPtr<const FPropertyInfoOverride>& PropertyInfoOverride : PropertyInfoOverrides)
											{
												InSubMenuBuilder.AddMenuEntry(
													...
													MakeBindingPropertyInfoWidget(InCachedBindingData->GetEdGraphSchema(), PropertyInfoOverride->TypeNameText, PropertyInfoOverride->PinType));
											}
										}));
							}
						}
...
{
	WrapValueWidget(InWidgetRow, InCachedBindingData);
	CustomizeResetToDefault(InWidgetRow, InDetailBuilder, InPropertyHandle, InCachedBindingData);
}

void FPropertyBindingExtension::WrapValueWidget(FDetailWidgetRow& InWidgetRow,
	const TSharedPtr<UE::PropertyBinding::FCachedBindingData>& InCachedBindingData) const
{
	auto IsValueVisible = TAttribute<EVisibility>::Create([InCachedBindingData]() -> EVisibility
	{
		return InCachedBindingData->HasBinding(FPropertyBindingBindingCollection::ESearchMode::Exact) ? EVisibility::Collapsed : EVisibility::Visible;
	});
...
void FPropertyBindingExtension::CustomizeResetToDefault(FDetailWidgetRow& InWidgetRow,
	const IDetailLayoutBuilder& InDetailBuilder, const TSharedPtr<IPropertyHandle>& InPropertyHandle,
	const TSharedPtr<UE::PropertyBinding::FCachedBindingData>& InCachedBindingData) const
{
...
TNotNull<const UEdGraphSchema_K2*> FPropertyBindingExtension::GetEdGraphSchema() const
{
	return GetDefault<UEdGraphSchema_K2>();
}
...
{
	return MakeContextStructWidget(GetDefault<UEdGraphSchema_K2>(), InContextStruct);
}

TSharedRef<SWidget> FPropertyBindingExtension::MakeContextStructWidget(TNotNull<const UEdGraphSchema_K2*> Schema, const FPropertyBindingBindableStructDescriptor& InContextStruct)
{
	FEdGraphPinType PinType;
