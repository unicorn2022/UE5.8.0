...
void FPropertyBindingBindingCollection::RemoveInvalidBindings(const TMap<FGuid, const FPropertyBindingDataView>& InValidStructs, EInvalidBindingCriteria InCriteria)
{
	const UObject* LogOwner = GetLogOwner();
	// first pass, we remove invalid ones
	RemoveBindingsInternal([LogOwner, InValidStructs, InCriteria](const FPropertyBindingBinding& Binding)
		{
...

			{
				...
				if (EnumHasAnyFlags(InCriteria, EInvalidBindingCriteria::RemoveMissingSourceStruct) && (!SourceValue || !SourceValue->IsValid()))
				{
					return true;
				}

				if (EnumHasAnyFlags(InCriteria, EInvalidBindingCriteria::RemoveFailedToResolveSourcePath) && SourceValue && SourceValue->IsValid())
				{
					FString Error;
			...
			{
				const FPropertyBindingDataView* TargetValue = InValidStructs.Find(Binding.GetTargetPath().GetStructID());
				if (EnumHasAnyFlags(InCriteria, EInvalidBindingCriteria::RemoveMissingTargetStruct) &&(!TargetValue || !TargetValue->IsValid()))
				{
					return true;
				}

				if (EnumHasAnyFlags(InCriteria, EInvalidBindingCriteria::RemoveFailedToResolveTargetPath) && TargetValue && TargetValue->IsValid())
				{
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPropertyBindingBindingCollection::ResolvePaths)
...
bool FPropertyBindingBindingCollection::ResolvePath(const FPropertyBindingDataView DataView, const FPropertyBindingPath& Path, FPropertyBindingPropertyIndirection& OutFirstIndirection, FPropertyBindingPathIndirection& OutLeafIndirection)
{
	...
	FString Error;
	TArray<FPropertyBindingPathIndirection, TInlineAllocator<16>> PathIndirections;
...
bool FPropertyBindingBindingCollection::ResolveCopyInfoBetweenIndirections(const FPropertyBindingPathIndirection& InSourceIndirection, const FPropertyBindingPathIndirection& InTargetIndirection, FPropertyBindingCopyInfo& OutCopyInfo) const
{
	...
	const FProperty* CopySourceProperty = SourceProperty;
	const UStruct* CopySourceStruct = InSourceIndirection.GetContainerStruct();
	const FProperty* CopyTargetProperty = TargetProperty;
	const UStruct* CopyTargetStruct = InTargetIndirection.GetContainerStruct();
	if (OutCopyInfo.bCopyFromTargetToSource)
	{
		Swap(CopySourceProperty, CopyTargetProperty);
		Swap(CopySourceStruct, CopyTargetStruct);
	}
	...
	UE::PropertyBinding::EPropertyCompatibility Compatibility = UE::PropertyBinding::GetPropertyCompatibility(CopySourceProperty, CopyTargetProperty);

	if (Compatibility == UE::PropertyBinding::EPropertyCompatibility::Promotable)
	{
		const FArrayProperty* CopyTargetArrayProperty = CastField<FArrayProperty>(CopyTargetProperty);
		const FArrayProperty* CopySourceArrayProperty = CastField<FArrayProperty>(CopySourceProperty);
		if (CopyTargetArrayProperty && CopySourceArrayProperty)
		{
			OutCopyInfo.bPromoteArrayCopy = true;
			CopyTargetProperty = CopyTargetArrayProperty->Inner;
			CopySourceProperty = CopySourceArrayProperty->Inner;
			Compatibility = UE::PropertyBinding::GetPropertyCompatibility(CopySourceProperty, CopyTargetProperty);
		}
	}
	...
	if (Compatibility == UE::PropertyBinding::EPropertyCompatibility::Compatible)
	{
		...
		{
			const bool bBothAreSoftObjectProperty = CopySourceProperty->IsA<FSoftObjectProperty>() && CopyTargetProperty->IsA<FSoftObjectProperty>();
			const bool bBothAreWeakObjectProperty = CopySourceProperty->IsA<FWeakObjectProperty>() && CopyTargetProperty->IsA<FWeakObjectProperty>();
			const bool bBothAreLazyObjectProperty = CopySourceProperty->IsA<FLazyObjectProperty>() && CopyTargetProperty->IsA<FLazyObjectProperty>();
			if (bBothAreSoftObjectProperty || bBothAreWeakObjectProperty || bBothAreLazyObjectProperty)
			{
...
uint8* FPropertyBindingBindingCollection::GetAddress(FPropertyBindingDataView InStructView, const FPropertyBindingPropertyIndirection& FirstIndirection, const FProperty* LeafProperty) const
{
	...
	while (Indirection != nullptr && Address != nullptr)
	{
		...
		{
			FWeakObjectPtr& WeakObjectPtr = *reinterpret_cast<FWeakObjectPtr*>(Address + Indirection->Offset);
...
{
	const FProperty* SourceLeafProperty = Copy.SourceLeafProperty;
	const FProperty* TargetLeafProperty = Copy.TargetLeafProperty;
	if (Copy.bCopyFromTargetToSource)
	{
		Swap(SourceAddress, TargetAddress);
		Swap(SourceLeafProperty, TargetLeafProperty);
	}
	...
	auto PerformCopyInternal = [Self = this, &Copy](const FProperty* SourceLeafProperty, uint8* SourceAddress, const FProperty* TargetLeafProperty, uint8* TargetAddress)
		{
			switch (Copy.Type)
			{
			...
			{
				checkf(Self->PropertyReferenceCopyFunc, TEXT("Not expecting EPropertyCopyType::StructReference if copy functor was not provided"));
				...
				{
					Self->PropertyReferenceCopyFunc(*SourceStructProperty, SourceAddress, TargetAddress);
				}
				break;
			}
			// Bool promotions
	...
	if (!Copy.bPromoteArrayCopy)
	{
		PerformCopyInternal(SourceLeafProperty, SourceAddress, TargetLeafProperty, TargetAddress);
	}
	else
	{
		const FArrayProperty* SourceArrayProperty = CastFieldChecked<const FArrayProperty>(SourceLeafProperty);
		...
		const int32 NumberOfElements = SourceArrayHelper.Num();
		if (TargetArrayProperty->HasAnyPropertyFlags(CPF_EditFixedSize) && TargetArrayHelper.Num() != NumberOfElements)
		{
			// The target is of fixed size. The size of the source need to match. It would be bad to copy half or none of the property.
			PROPERTY_BINDINGS_LOG(GetLogOwner(), Verbose, TEXT("%hs skipped: the target array size do not match '%d'."), __FUNCTION__, NumberOfElements);
			return false;
		}

		TargetArrayHelper.Resize(NumberOfElements);
		for (int32 ElementIndex = 0; ElementIndex < NumberOfElements; ++ElementIndex)
		{
			PerformCopyInternal(SourceArrayProperty->Inner, SourceArrayHelper.GetRawPtr(ElementIndex), TargetArrayProperty->Inner, TargetArrayHelper.GetRawPtr(ElementIndex));
		}
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPropertyBindingBindingCollection::CopyProperty)
...
void FPropertyBindingBindingCollection::PerformResetObjects(const FPropertyBindingCopyInfo& Copy, uint8* TargetAddress) const
{
	...
	auto PerformResetObjectsInternal = [Self = this, Type = Copy.Type](const FProperty* TargetLeafProperty, uint8* TargetAddress)
		{
			switch (Type)
			{
			case EPropertyCopyType::CopyComplex:
				TargetLeafProperty->ClearValue(TargetAddress);
				break;
			case EPropertyCopyType::CopyStruct:
				CastFieldChecked<const FStructProperty>(TargetLeafProperty)->Struct->ClearScriptStruct(TargetAddress);
				break;
			case EPropertyCopyType::CopyObject:
				CastFieldChecked<const FObjectPropertyBase>(TargetLeafProperty)->SetObjectPropertyValue(TargetAddress, nullptr);
				break;
			case EPropertyCopyType::StructReference:
				checkf(Self->PropertyReferenceResetFunc, TEXT("Not expecting EPropertyCopyType::StructReference if reset object functor was not provided"));
				Self->PropertyReferenceResetFunc(TargetAddress);
				break;
			case EPropertyCopyType::CopyFixedArray:
			{
				// Copy into fixed sized array (EditFixedSize). Resizable arrays are copied as Complex, and regular fixed sizes arrays via the regular copies (dim specifies array size).
	...
	if (!Copy.bPromoteArrayCopy)
	{
		PerformResetObjectsInternal(Copy.TargetLeafProperty, TargetAddress);
	}
	else
	{
		const FArrayProperty* TargetArrayProperty = CastFieldChecked<const FArrayProperty>(Copy.TargetLeafProperty);
		FScriptArrayHelper TargetArrayHelper(TargetArrayProperty, TargetAddress);
		const int32 NumberOfElements = TargetArrayHelper.Num();
		for (int32 ElementIndex = 0; ElementIndex < NumberOfElements; ++ElementIndex)
		{
			PerformResetObjectsInternal(TargetArrayProperty->Inner, TargetArrayHelper.GetRawPtr(ElementIndex));
		}
...
bool FPropertyBindingBindingCollection::ContainsAnyStruct(const TSet<const UStruct*>& InStructs) const
{
	...
	// @todo: at runtime, we won't be able to check any Container Struct.
	// Ex: we are able to check Container Structs for FStateTreeEditorPropertyBindings but not FStateTreePropertyBindings 
	bool bPathContainsStruct = false;

	TArray<const UStruct*> StructsArray;
	StructsArray.Reserve(InStructs.Num());
	Algo::Copy(InStructs, StructsArray, Algo::NoRef);

#if WITH_EDITOR
	if (const IPropertyBindingBindingCollectionOwner* BindingsOwnerPtr = GetBindingsOwner())
	{
		VisitBindings([&bPathContainsStruct, &BindingsOwnerPtr, &StructsArray](const FPropertyBindingBinding& Binding)
			{
				FPropertyBindingDataView SourceDataView;
				BindingsOwnerPtr->GetBindingDataViewByID(Binding.GetSourcePath().GetStructID(), SourceDataView);
				if (Binding.GetSourcePath().ContainsAnyStructWithValue(StructsArray, SourceDataView))
				{
				...
				FPropertyBindingDataView TargetDataView;
				BindingsOwnerPtr->GetBindingDataViewByID(Binding.GetTargetPath().GetStructID(), TargetDataView);
				if (Binding.GetTargetPath().ContainsAnyStructWithValue(StructsArray, TargetDataView))
				{
					bPathContainsStruct = true;
					return EVisitResult::Break;
				}
...
#endif //WITH_EDITOR
	{
		VisitBindings([&bPathContainsStruct, &StructsArray](const FPropertyBindingBinding& Binding)
			{
				if (Binding.GetSourcePath().ContainsAnyStruct(StructsArray))
				{
					bPathContainsStruct = true;
					return EVisitResult::Break;
				}

				if (Binding.GetTargetPath().ContainsAnyStruct(StructsArray))
				{
					bPathContainsStruct = true;
					return EVisitResult::Break;
				}
...
FString FPropertyBindingBindingCollection::DebugAsString() const
{
	...
	if (CopyBatches.Num())
	{
		DebugString.Appendf(TEXT("  [ (Idx) | %-45s | %-80s | %-8s [%-5s:%-5s[ | %-8s [%-5s:%-5s[ ]\n"),
		...
		for (int32 Index = 0; Index < CopyBatches.Num(); ++Index)
		{
			...
			DebugString.Appendf(TEXT("  | (%3d) | %-45s | %-80s | %8s [%5d:%-5d[ | %8s [%5d:%-5d[ |\n"),
				Index,
