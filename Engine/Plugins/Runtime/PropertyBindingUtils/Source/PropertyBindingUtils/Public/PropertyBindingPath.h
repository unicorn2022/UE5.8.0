...
#define UE_API PROPERTYBINDINGUTILS_API
...
struct FPropertyBindingPath
{
	 ...
	 */
	UE_API bool FromString(const FStringView InPath);
	 ...
	 */
	UE_API FString ToString(const int32 HighlightedSegment = INDEX_NONE, const TCHAR* HighlightPrefix = nullptr, const TCHAR* HighlightPostfix = nullptr, const bool bOutputInstances = false, const int32 FirstSegment = 0) const;
	 ...
	 * @param bResetIndirectionsIfFailed If true, indirections resolved so far will be cleared if the path failed to resolve in the end. 
	 * @return true of path could be resolved against the base value, and instance types were updated.
	 */
	UE_API bool ResolveIndirections(const UStruct* BaseStruct, TArray<FPropertyBindingPathIndirection>& OutIndirections, FString* OutError = nullptr, bool bHandleRedirects = false, bool bResetIndirectionsIfFailed = true) const;
	UE_API bool ResolveIndirections(const UStruct* BaseStruct, TArray<FPropertyBindingPathIndirection, TInlineAllocator<16>>& OutIndirections, FString* OutError = nullptr, bool bHandleRedirects = false, bool bResetIndirectionsIfFailed = true) const;
	 ...
	 * @return true of path could be resolved against the base value, and instance types were updated.
	 */
	UE_API bool ResolveIndirectionsWithValue(const FPropertyBindingDataView BaseValueView, TArray<FPropertyBindingPathIndirection>& OutIndirections, FString* OutError = nullptr, bool bHandleRedirects = false, bool bResetIndirectionsIfFailed = true) const;
	UE_API bool ResolveIndirectionsWithValue(const FPropertyBindingDataView BaseValueView, TArray<FPropertyBindingPathIndirection, TInlineAllocator<16>>& OutIndirections, FString* OutError = nullptr, bool bHandleRedirects = false, bool bResetIndirectionsIfFailed = true) const;
	 ...
	 * @return true of path could be resolved against the base value, and instance types were updated.
	 */
	UE_API bool UpdateSegments(const UStruct* BaseStruct, FString* OutError = nullptr);
	 ...
	 * @return true of path could be resolved against the base value, and instance types were updated.
	 */
	UE_API bool UpdateSegmentsFromValue(const FPropertyBindingDataView BaseValueView, FString* OutError = nullptr);

	/**
	 * Updates property segments from resolved indirections. The path is expected to be relative to the base value.
	 * The method updates instance types, and handles renamed properties (core redirect, Blueprint, User Defined Structs and Property Bags by ID).
	 * By storing the instance types on the path, we can resolve the path without the base value later.
	 * @param Indirections completely resolved indirections
	 */
	UE_API void UpdateSegmentsFromIndirections(const TConstArrayView<FPropertyBindingPathIndirection>& Indirections);

	/**
	 * Check if the binding path contains any structs from cached instance struct type.
	 * @param InStructs any structs the binding path may contain
	 * @return true if path contains any struct
	 */
	UE_API bool ContainsAnyStruct(TConstArrayView<const UStruct*> InStructs) const;
	 ...
	 * @return true if path contains any struct
	 */
	UE_API bool ContainsAnyStruct(TConstArrayView<const UStruct*> InStructs, const UStruct* BaseStruct) const;

	/**
	 * Check if the binding path contains any structs from base value.
	 * @param InStructs any structs the binding path may contain
	 * @param BaseValueView Base value the path is relative to
	 * @return true if path contains any struct
	 */
	UE_API bool ContainsAnyStructWithValue(TConstArrayView<const UStruct*> InStructs, const FPropertyBindingDataView BaseValueView) const;

	/**
	 * Check if indirections contain any structs. 
	 * @param InStructs any structs the binding path may contain
	 * @param Indirections Indirections resolved so far to check against
	 * @return true if indirections contains any struct
	 */
	UE_API static bool IndirectionsContainAnyStruct(TConstArrayView<const UStruct*> InStructs, const TConstArrayView<FPropertyBindingPathIndirection> Indirections);
	...
	UE_API bool operator==(const FPropertyBindingPath& RHS) const;

	/**
	 * Test if this paths includes the provided path.
	 * A path includes another one when they are == but this path can be longer.
	 */
	UE_API bool Includes(const FPropertyBindingPath& Other) const;
...
private:
	...
	bool ResolveIndirectionsWithValue(const FPropertyBindingDataView BaseValueView, TArray<FPropertyBindingPathIndirection, Allocator>& OutIndirections, FString* OutError = nullptr, bool bHandleRedirects = false, bool bResetIndirectionsIfFailed = true) const;
};
...
struct FPropertyBindingCopyInfo
{
	...
	/**
	 * The source property and the target property are array properties.
	 * The inner type of the source property can be promoted to the inner type of the target property.
	 */
	UPROPERTY()
	bool bPromoteArrayCopy = false;

	/** Copy will be performed from target property to the source property. */
	UPROPERTY()
