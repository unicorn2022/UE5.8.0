...
struct FPropertyBindingBindingCollection
{
	...
	/** Get the associated bindings owner as a UObject. */
	UObject* GetBindingsOwnerObject() const
	{
		return BindingsOwner.GetObject();
	}
	...
	enum class EInvalidBindingCriteria : uint8
	{
		// Binding is removed if Binding Source Struct is invalid
		RemoveMissingSourceStruct = 1 << 0,
		// Binding is removed if Binding Target Struct is invalid
		RemoveMissingTargetStruct = 1 << 1,
		// Binding is removed if Binding Source Path failed to resolve
		RemoveFailedToResolveSourcePath = 1 << 2,
		// Binding is removed if Binding Target Path failed to resolve
		RemoveFailedToResolveTargetPath = 1 << 3,

		// Missing Source Struct is not included to be consistent with the old behavior 
		Default = RemoveMissingTargetStruct | RemoveFailedToResolveSourcePath | RemoveFailedToResolveTargetPath
	};

	/**
	 * Removes bindings which do not point to valid structs IDs.
	 * @param InValidStructs Set of struct IDs that are currently valid.
	 * @param InCriteria Rules for what bindings are regarded as invalid
	 */
	PROPERTYBINDINGUTILS_API void RemoveInvalidBindings(const TMap<FGuid, const FPropertyBindingDataView>& InValidStructs, EInvalidBindingCriteria InCriteria = EInvalidBindingCriteria::Default);
...
public:
	...
	/**
	 * Iterates all bindings between indices [Begin, End) and calls the provided function on each non-mutable binding.
	 * That function must return whether the iteration should stop or not (i.e., EVisitResult::Break / Continue).
	 * @param InBegin Index of the first binding to process
	 * @param InEnd Index to one past the last binding to process
	 * @param InFunction Function to call on each binding and that indicates whether the visit should stop
	 ...
	 * @see VisitMutableBindings, ForEachBinding, ForEachMutableBinding
	 */
	virtual void VisitBindings(FPropertyBindingIndex16 InBegin, FPropertyBindingIndex16 InEnd, TFunctionRef<EVisitResult(const FPropertyBindingBinding& Binding, const int32 BindingIndex)> InFunction) 
		const PURE_VIRTUAL(FPropertyBindingBindingCollection::VisitBindings, );
...
#if WITH_EDITOR
ENUM_CLASS_FLAGS(FPropertyBindingBindingCollection::EInvalidBindingCriteria)
#endif //WITH_EDITOR