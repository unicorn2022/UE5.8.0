...
UComputeDataProvider* UComputeDataInterfaceDispatch::CreateDataProvider() const
{
	return NewObject<UDispatchDataProvider>();
}

void UDispatchDataProvider::Initialize(int32 InDataInterfaceIndex, UComputeDataInterface const* InDataInterface, UObject* InBinding, uint64 InInputMask, uint64 InOutputMask)
{
...
FDispatchDataProviderProxy::FDispatchDataProviderProxy(FIntVector InThreadCount)
...
int32 FDispatchDataProviderProxy::GetDispatchThreadCount(TArray<FIntVector>& InOutThreadCounts) const
{
	...
	InOutThreadCounts.Add(ThreadCount);
	return 1;
}
...
void FDispatchDataProviderProxy::GatherDispatchData(FDispatchData const& InDispatchData)
{
	...
	for (int32 InvocationIndex = 0; InvocationIndex < ParameterArray.Num(); ++InvocationIndex)
	{
		...
		Parameters.NumThreads = FUintVector(ThreadCount.X, ThreadCount.Y, ThreadCount.Z);
	}
}
