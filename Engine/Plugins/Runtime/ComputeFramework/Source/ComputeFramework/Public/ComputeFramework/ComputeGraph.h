...
class UComputeGraph : public UObject
{
	...
	friend class FComputeGraphBuilder;
...
public:
	...
	/** Get the number of binding objects used by the graph. */
	int32 GetNumBindingObjects() const { return Bindings.Num(); }
...
private:
	...
	void GetDataInterfaceInputOutputMasks(int32 InDataInterfaceIndex, uint64& OutInputMask, uint64& OutOutputMask) const;
	...
	/** For cooked packages, promote inline shader maps to the render thread. Returns false if any kernel was missing an inline shader map. */
	bool CacheCookedResourceShadersForRendering();
...
private:
	...
	struct FComputeKernelResourceSet
	{
		...
		/** Cache inline shader map for rendering. Returns false if no inline shader map was available. */
		bool CacheInlineShaders();
