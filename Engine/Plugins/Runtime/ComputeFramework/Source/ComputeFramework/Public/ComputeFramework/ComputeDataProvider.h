...
public:
	...
	virtual void Initialize(UComputeDataInterface const* InDataInterface, UObject* InBinding, uint64 InInputMask, uint64 InOutputMask) {}
	virtual void Initialize(int32 InDataInterfaceIndex, UComputeDataInterface const* InDataInterface, UObject* InBinding, uint64 InInputMask, uint64 InOutputMask)
	{
		Initialize(InDataInterface, InBinding, InInputMask, InOutputMask);
	}


	struct FInputOutputMask
	{
		uint64 InputMask = 0;
		uint64 OutputMask = 0;
	};
	...
	/**
	 * Called only when the associated UComputeDataInterface::GetRequiresPerKernelInputOutputMasks() returns true.
	 * Supplies the per-kernel breakdown of input/output bindings, which can be use by the proxy for bookkeeping 
	 * data related to the currently dispatching kernel (looked up via FComputeContext::GraphKernelIndex). 
	 */
	virtual void InitializePerKernelInputOutputMasks(TMap<int32, FInputOutputMask>&& InPerKernelMasks) {}
...
{
public:
	virtual ~FComputeDataProviderRenderProxy() = default;
