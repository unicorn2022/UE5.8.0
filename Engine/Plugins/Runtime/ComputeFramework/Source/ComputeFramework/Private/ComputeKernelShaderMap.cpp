...
FCriticalSection FComputeKernelShaderMap::ShaderMapRegistryMutex;
...
uint64 FComputeKernelShaderMapId::GetComputeKernelHash() const
{
	...
	)
{
	FShaderCompileJob* NewJob = GShaderCompilingManager->PrepareShaderCompileJob(
		InShaderMapId, 
		FShaderCompileJobKey(this, InPlatform, nullptr, PermutationId),
	...
	UE_LOGF(LogComputeFramework, Verbose, "%ls", *InKernel->GetHLSLSource());
	
	AddUniformBufferIncludesToEnvironment(NewJob->Input.Environment, InPlatform);

	FShaderCompilerEnvironment& ShaderEnvironment = NewJob->Input.Environment;

	UE_LOGF(LogComputeFramework, Verbose, "			%ls", GetName());
 ...
 */
TRefCountPtr<FComputeKernelShaderMap> FComputeKernelShaderMap::FindId(const FComputeKernelShaderMapId& InShaderMapId, EShaderPlatform InPlatform)
{
	FScopeLock Lock(&ShaderMapRegistryMutex);
	TRefCountPtr<FComputeKernelShaderMap> Result = GIdToComputeKernelShaderMap[InPlatform].FindRef(InShaderMapId);
	...
	)
{
	...
	{
		UE_LOGF(LogComputeFramework, Fatal, "Trying to compile ComputeKernel shader %ls at run-time, which is not supported on consoles!", *InKernel->GetFriendlyName() );
		return;
	}
	...
	if (TArray<FComputeKernelResource*>* ExistingKernels = ComputeKernelShaderMapsBeingCompiled.Find(this))
	{
		check(!bSynchronousCompile);
		ExistingKernels->AddUnique(InKernel);
		return;
	}
	...
	UE_LOGF(LogComputeFramework, Verbose, "CompilingId = %p %d", InKernel, CompilingId);
	...
	UE_LOGF(LogComputeFramework, Verbose, "		%u Shaders", NumShaders);
...
FShader* FComputeKernelShaderMap::ProcessCompilationResultsForSingleJob(FShaderCompileJob& CurrentJob, uint64 InShaderMapHash)
{
...
bool FComputeKernelShaderMap::ProcessCompilationResults(const TArray<FShaderCommonCompileJobPtr>& InCompilationResults, int32& InOutJobIndex, float& InOutTimeBudget)
{
	...
	uint64 ShaderMapHash = GetContent()->ShaderMapId.GetComputeKernelHash();
...
bool FComputeKernelShaderMap::TryToAddToExistingCompilationTask(FComputeKernelResource* InKernel)
{
	...
	if (TArray<FComputeKernelResource*>* CorrespondingKernels = ComputeKernelShaderMapsBeingCompiled.Find(this))
	{
		CorrespondingKernels->AddUnique(InKernel);

		UE_LOGF(LogComputeFramework, Verbose, "TryToAddToExistingCompilationTask %p %d", InKernel, GetCompilingId());
	...
	if (ShouldCacheComputeKernelShader(InShaderType, GetShaderPlatform(), InKernel))
	{
		for (int32 PermutationId = 0; PermutationId < InKernel->GetNumPermutations(); ++PermutationId)
		{
			if (!GetContent()->HasShader((FShaderType*)InShaderType, PermutationId))
			{
				if (!bSilent)
				{
					UE_LOGF(LogComputeFramework, Warning, "Incomplete shader %ls, missing FComputeKernelShader %ls.", *InKernel->GetFriendlyName(), InShaderType->GetName());
				}
				return false;
			}
		}
...
#if WITH_EDITOR
	check(IsInGameThread());
	if (ComputeKernelShaderMapsBeingCompiled.Contains(this))
	{
...
{
	GetContent()->GetShaderList(*this, 0u, OutShaders);
}
...
void FComputeKernelShaderMap::Register(EShaderPlatform InShaderPlatform)
{
	...
	FComputeKernelShaderMap* Existing = GIdToComputeKernelShaderMap[GetShaderPlatform()].FindRef(GetContent()->ShaderMapId);
	checkf(Existing == nullptr || Existing == this, TEXT("ComputeKernel shader map collision for id %s"), *GetContent()->FriendlyName);
...
{
	bool bShouldBeginCleanup = false;
	{
		FScopeLock Lock(&ShaderMapRegistryMutex);
		check(NumRefs > 0);
		if (FPlatformAtomics::InterlockedDecrement(&NumRefs) == 0)
		{
			...
			bShouldBeginCleanup = true;
		}
	}

	if (bShouldBeginCleanup)
	{
		// BeginCleanup can run arbitrary deferred-cleanup work, so invoke it outside the lock.
		BeginCleanup(this);
	}
}
...
void FComputeKernelShaderMap::RemovePending(FComputeKernelResource* InKernel)
{
	...
	TArray<uint32> RemovedCompilingIds;
	for (TMap<TRefCountPtr<FComputeKernelShaderMap>, TArray<FComputeKernelResource*> >::TIterator It(ComputeKernelShaderMapsBeingCompiled); It; ++It)
	{
		...
		if (Result)
		{
			RemovedCompilingIds.Add(It.Key()->CompilingId);
		}
	}

	for (uint32 CompilingIdToRemove : RemovedCompilingIds)
	{
		InKernel->RemoveOutstandingCompileId(CompilingIdToRemove);
