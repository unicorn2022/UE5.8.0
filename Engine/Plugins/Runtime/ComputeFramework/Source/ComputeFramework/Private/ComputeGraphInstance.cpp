...
#if HAS_GPU_STATS
	// GPU profiler callbacks only fire at real frame boundaries; in automation tests none exist. Fulfill promises immediately on the game thread
	// so callers don't wait, and pass an empty array to the render command so no FComputeFrameworkGPUStat objects are created.
	if (GIsAutomationTesting)
	{
		for (TPromise<FComputeFrameworkKernelTiming>& Promise : InKernelTimingPromises)
		{
			Promise.SetValue(FComputeFrameworkKernelTiming{});
		}
		InKernelTimingPromises.Reset();
	}
#endif

	ENQUEUE_RENDER_COMMAND(ComputeFrameworkEnqueueExecutionCommand)(
		[ComputeGraphWorker, InExecutionGroupName, InOwnerName, SortPriority = InGraphSortPriorityOffset + GraphSortPriority, GraphRenderProxy, MovedDataProviderRenderProxies = MoveTemp(DataProviderRenderProxies), InFallbackDelegate, InOwnerPointer, bEnableRenderCaptures=this->bEnableRenderCaptures
#if HAS_GPU_STATS
		, MovedKernelTimingPromises = MoveTemp(InKernelTimingPromises)
#endif
		](FRHICommandListImmediate& RHICmdList) mutable
		{
			// Compute graph scheduler will take ownership of the provider proxies.
			ComputeGraphWorker->Enqueue(InExecutionGroupName, InOwnerName, SortPriority, GraphRenderProxy, MovedDataProviderRenderProxies, InFallbackDelegate, bEnableRenderCaptures
#if HAS_GPU_STATS
				, MoveTemp(MovedKernelTimingPromises)
#endif
				, InOwnerPointer);
		});
