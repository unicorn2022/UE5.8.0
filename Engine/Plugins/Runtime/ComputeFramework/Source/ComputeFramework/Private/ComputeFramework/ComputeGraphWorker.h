...
#include "ComputeFramework/ComputeFrameworkKernelTiming.h"
...
#if HAS_GPU_STATS
		/** One TPromise per kernel (parallel to graph kernels). Moved into FComputeFrameworkGPUStat objects during SubmitWork. */
		mutable TArray<TPromise<FComputeFrameworkKernelTiming>> KernelTimingPromises;

		/** Fulfills any remaining promises with a zero result so consumers don't block, then clears the array.
		 *  Called on validation failure (!bIsValid path) and explicitly from Abort() before the invocation is removed. */
		void SendNullKernelTimings() const;
#endif
