...
public:
	...
	uint64 GetComputeKernelHash() const;
...
public:
	 ...
	 */
	static TRefCountPtr<FComputeKernelShaderMap> FindId(const FComputeKernelShaderMapId& InShaderMapId, EShaderPlatform InPlatform);
...
#if WITH_EDITOR
	static TMap<TRefCountPtr<FComputeKernelShaderMap>, TArray<FComputeKernelResource*> > &GetInFlightShaderMaps()
	{
		// ComputeKernelShaderMapsBeingCompiled is game thread only.
		check(IsInGameThread());
		return ComputeKernelShaderMapsBeingCompiled;
	}
...
private:
	...
	// Protects GIdToComputeKernelShaderMap plus associated state. 
	// These are mutated from any thread that constructs/destroys/registers/unregisters a FComputeKernelShaderMap
	// including async-loading worker threads.
	static FCriticalSection ShaderMapRegistryMutex;

#if WITH_EDITOR
	// Tracks resources and their shader maps that are being compiled. 
	// Game-thread only, asserted by check(IsInGameThread()) at every access point.
	 ...
	 */
	std::atomic<bool> bCompilationFinalized;

	std::atomic<bool> bCompiledSuccessfully;
...
#if WITH_EDITOR
	FShader* ProcessCompilationResultsForSingleJob(FShaderCompileJob& InSingleJob, uint64 InShaderMapHash);
#endif
...
public:
	...
	bool CacheInlineShaders(EShaderPlatform InPlatform);
