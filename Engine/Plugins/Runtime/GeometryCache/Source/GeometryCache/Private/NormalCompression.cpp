...
void FNormalCoderSmithTest::Test()
{
	...
	for (float Theta = 0.0f; Theta < 2 * PI; Theta += Step)
	{
		for (float Phi = 0.0f; Phi < PI / 2; Phi += Step)
		{
			...
			UE_LOGF(LogGeoCaStreamingNormalCompression, Log, "Input: (%i, %i, %i), Encoded: (%i, %i), Decoded: (%i, %i, %i), Difference: (%i, %i, %i)\n",
	...
	UE_LOGF(LogGeoCaStreamingNormalCompression, Log, "Maximum difference: (%i, %i, %i)\n", MaximumDifference.X, MaximumDifference.Y, MaximumDifference.Z);
	UE_LOGF(LogGeoCaStreamingNormalCompression, Log, "Maximum difference spherical (theta, phi): (%.2f, %.2f)\n", MaximumDifferenceSpherical.X, MaximumDifferenceSpherical.Y);
}