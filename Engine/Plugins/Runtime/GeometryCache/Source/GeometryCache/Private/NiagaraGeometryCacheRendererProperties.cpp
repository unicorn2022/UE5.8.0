...
void UNiagaraGeometryCacheRendererProperties::InitCDOPropertiesAfterModuleStartup()
{
	...
	{
		if (UNiagaraGeometryCacheRendererProperties* RedererProperties = WeakGeometryCacheRendererProperties.Get())
		{
			RedererProperties->InitRendererBaseBindings();
			if (!RedererProperties->PositionBinding.IsValid())
			{
				RedererProperties->PositionBinding = FNiagaraConstants::GetAttributeDefaultBinding(SYS_PARAM_PARTICLES_POSITION);
			}
			if (!RedererProperties->ScaleBinding.IsValid())
			{
				RedererProperties->ScaleBinding = FNiagaraConstants::GetAttributeDefaultBinding(SYS_PARAM_PARTICLES_SCALE);
			}
			if (!RedererProperties->RendererVisibilityTagBinding.IsValid())
			{
				RedererProperties->RendererVisibilityTagBinding = FNiagaraConstants::GetAttributeDefaultBinding(SYS_PARAM_PARTICLES_VISIBILITY_TAG);
			}
			if (!RedererProperties->RotationBinding.IsValid())
			{
				RedererProperties->RotationBinding = CreateDefaultBinding(Particles_GeoCacheRotation, FVector3f(0, 0, 0));
			}
			if (!RedererProperties->ElapsedTimeBinding.IsValid())
			{
				RedererProperties->ElapsedTimeBinding = CreateDefaultBinding(Particles_Age, 0.0f);
			}
			if (!RedererProperties->EnabledBinding.IsValid())
			{
				RedererProperties->EnabledBinding = CreateDefaultBinding(Particles_GeoCacheIsEnabled, false);
			}
			if (!RedererProperties->ArrayIndexBinding.IsValid())
			{
				RedererProperties->ArrayIndexBinding = CreateDefaultBinding(Particles_ArrayIndex, 0);
			}
		}
	}
}
...
void UNiagaraGeometryCacheRendererProperties::GetUsedMeshes(const FNiagaraEmitterInstance* InEmitter, TArray<UObject*>& OutMeshes) const
{
	for (const FNiagaraGeometryCacheReference& Cache : GeometryCaches)
	{
		if (UGeometryCache* GeometryCache = ResolveGeometryCache(Cache, InEmitter))
		{
			OutMeshes.AddUnique(GeometryCache);
		}
	}
}
...
TArray<FNiagaraVariable> UNiagaraGeometryCacheRendererProperties::GetBoundAttributes() const
{
	...
	for (const FNiagaraGeometryCacheReference& GeometryCache : GeometryCaches)
	{
		if (GeometryCache.GeometryCacheUserParamBinding.Parameter.IsValid())
		{
			BoundAttributes.AddUnique(GeometryCache.GeometryCacheUserParamBinding.Parameter);
		}
	}
