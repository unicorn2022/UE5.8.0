...
namespace UE::GeometryCacheHelpers
{
	...
	bool FTrackWriter::WriteAndClose(TArrayView<FFrameData> FramesToMoveFrom, const TArray<FVisibilitySample>& VisibilitySamples)
	{
		...
		{
			UE_LOGF(LogGeometryCacheWriter, Error, "The number of UVs does not match the number of vertices");
			return false;
		}
		const bool bHasColor0 = !Colors.IsEmpty();
		if (bHasColor0 && Colors.Num() != NumVertices)
		{
			UE_LOGF(LogGeometryCacheWriter, Error, "The number of colors does not match the number of vertices");
			return false;
		}
		...
		{
			UE_LOGF(LogGeometryCacheWriter, Error, "The number of imported vertex numbers does not match the number of vertices");
			return false;
		}
		...
		for (int32 Frame = 0; Frame < NumFrames; ++Frame)
		{
			...
			{
				UE_LOGF(LogGeometryCacheWriter, Error, "The number of vertices at Frame %d does not match that at Frame 0. Finishing up.", Frame);
				break;
			}
			...
			if (bHasUV0)
			{
				if (FrameData.Normals.Num() == NumVertices)
				{
					...
					else
					{
						...
						{
							UE_LOGF(LogGeometryCacheWriter, Error, "The number of TangentsX at Frame %d does not match that at Frame 0.", Frame);
						}
						MeshData.VertexInfo.bHasTangentX = true;
				...
				else
				{
					...
					{
						UE_LOGF(LogGeometryCacheWriter, Error, "The number of normals at Frame %d does not match that at Frame 0", Frame);
					}
					MeshData.VertexInfo.bHasTangentZ = true;
