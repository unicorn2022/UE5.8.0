	...
	for (int32 NeededIndex : ChunksNeeded)
	{
		if (!ChunksAvailable.Contains(NeededIndex))
		{
			...
			if (!ChunksRequested.Contains(NeededIndex))
			{
				...
				{
					UE_LOGF(LogGeoCaStreaming, Error, "Geometry cache streaming read request failed.");
					return;
				}
...
void FStreamingGeometryCacheData::ProcessCompletedChunks()
{
	...
	while (CompletedChunks.Dequeue(CompletedChunk))
	{
		...
		if (!Chunk)
		{
			UE_LOGF(LogGeoCaStreaming, Error, "Got a stray async read request");
			return;
		}
		...
		if (Chunk->IORequest != nullptr)
		{
			...
			{
				UE_LOGF(LogGeoCaStreaming, Error, "Async loading request failed!");
				ChunksRequested.Remove(CompletedChunk.LoadedChunkIndex);
...
const uint8* FStreamingGeometryCacheData::MapChunk(uint32 ChunkIndex, uint32* OutChunkSize)
{
	...
	if (!ChunksAvailable.Contains(ChunkIndex))
	{
		if (!ChunksRequested.Contains(ChunkIndex))
		{
			...
			{
				UE_LOGF(LogGeoCaStreaming, Log, "Tried to map an evicted chunk: %i.", ChunkIndex);
			}
			else
			{
				UE_LOGF(LogGeoCaStreaming, Log, "Tried to map an unavailabe non-requested chunk: %i.", ChunkIndex);
			}
		}
		else
		{
			UE_LOGF(LogGeoCaStreaming, Log, "Tried to map a chunk (%i) that is still being streamed in.", ChunkIndex);
		}
		return nullptr;
	}
	else
...
void FStreamingGeometryCacheData::UnmapChunk(uint32 ChunkIndex)
{
	...
	{
		UE_LOGF(LogGeoCaStreaming, Log, "Tried to unmap an unknown chunk.");
	}
}
