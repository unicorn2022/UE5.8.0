...
*/
	...
	UE_LOGF(LogGeoCaStreamingCodecV1, Log, "Compressed %u vertices, %u bytes to %u bytes in %.2f milliseconds (%.2f ratio), quantizer precision: %.2f units.", 
...
bool FCodecV1Decoder::DecodeFrameData(FBufferReader& Reader, FGeometryCacheMeshData &OutMeshData)
{
	...
	{
		UE_LOGF(LogGeoCaStreamingCodecV1, Error, "Incompatible bitstream found");
		return false;
	}
	...
	if (CVarCodecDebug.GetValueOnAnyThread() == 1)
	{
		...
		UE_LOGF(LogGeoCaStreamingCodecV1, Log, "Decoded frame with %i vertices in %.2f milliseconds.", NumVertices, TimeFloat);
	}
