...
void UGeometryCacheComponent::JumpAnimationToTime(float Time, bool bInIsRunning, bool bInBackwards, bool bInIsLooping)
{
	...
	if (FGeometryCacheSceneProxy* CastedProxy = static_cast<FGeometryCacheSceneProxy*>(SceneProxy))
	{
		...
		FGeometryCacheSceneProxy::FAnimationUpdateParameters UpdatedValues =
		{
			Time,
			bInIsLooping,
			bInBackwards,
			...
			IsInterpolatingFrames()
		};
		ENQUEUE_RENDER_COMMAND(FGeometryCacheUpdateAnimation)(
			[InSceneProxy, UpdatedValues](FRHICommandList& RHICmdList)
			{
				InSceneProxy->UpdateAnimation(RHICmdList, UpdatedValues);
			});
	}
}
...
{	
	...
	const bool bReverseCulling = GetRenderMatrix().Determinant() < 0;

	for (UMaterialInterface* Material : GetMaterials())
	{
		if (Material == nullptr || !Material->CheckMaterialUsage_Concurrent(MATUSAGE_GeometryCache))
		{
		...
		ComponentParams.PSOPrecacheParams.bReverseCulling |= bReverseCulling;
	}

	UMaterialInterface* OverlayMaterialInterface = GetOverlayMaterial();
	if (OverlayMaterialInterface && OverlayMaterialInterface->CheckMaterialUsage_Concurrent(MATUSAGE_GeometryCache))
	{
...
FPrimitiveSceneProxy* UGeometryCacheComponent::CreateSceneProxy()
{
	...
	{
		UE_LOGF(LogGeometryCache, Verbose, "Skipping CreateSceneProxy for GeometryCacheComponent %ls (PSOs are still compiling)", *GetFullName());
		return nullptr;
	}
...
bool UGeometryCacheComponent::IsInterpolatingFrames() const
{
	return bInterpolateFrames;
}

void UGeometryCacheComponent::SetInterpolateFrames(const bool bNewInterpolating)
{
	bInterpolateFrames = bNewInterpolating;
}
