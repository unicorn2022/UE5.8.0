...
public:
	...
	virtual void GetUsedMeshes(const FNiagaraEmitterInstance* InEmitter, TArray<UObject*>& OutMeshes) const override;
	...
	* Disabling this option is faster, but a particle can get a different component each tick, which can lead to problems with for example motion blur. */
	...
	/** Determines if the geometry component will affect the engine's physics system with its collision shape. */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = "Component")
	TEnumAsByte<ECollisionEnabled::Type> EnableCollision = ECollisionEnabled::NoCollision;
	
	/** If true then the geometry component will invalidate the nav mesh on movement. */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category = "Component")
