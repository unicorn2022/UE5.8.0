...
float FMovieSceneGeometryCacheSectionTemplateParameters::MapTimeToAnimation(float ComponentDuration, FFrameTime InPosition, FFrameRate InTickResolution) const
{
	...
	const float FirstLoopSeqLength = static_cast<float>(SequenceLength - InTickResolution.AsSeconds(FirstLoopStartFrameOffset + StartFrameOffset + EndFrameOffset));
	const float SeqLength = static_cast<float>(SequenceLength - InTickResolution.AsSeconds(StartFrameOffset + EndFrameOffset));
	...
	AnimPosition += static_cast<float>(InTickResolution.AsSeconds(FirstLoopStartFrameOffset));
	if (SeqLength > 0.f && (bLooping || !FMath::IsNearlyEqual(AnimPosition, SeqLength, 1e-4f)))
	{
		AnimPosition = FMath::Fmod(AnimPosition, SeqLength);
	}
	AnimPosition += static_cast<float>(InTickResolution.AsSeconds(StartFrameOffset));
	if (bReverse)
	{
