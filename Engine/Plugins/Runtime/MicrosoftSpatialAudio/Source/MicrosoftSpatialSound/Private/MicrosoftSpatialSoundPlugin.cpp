...
static void LogMicrosoftSpatialAudioError(HRESULT Result, int32 LineNumber)
{
	...
	UE_LOGF(LogMicrosoftSpatialSound, Error, "%ls", *ErrorString);
}
...
void FMicrosoftSpatialSound::ProcessAudio(const FAudioPluginSourceInputData& InputData, FAudioPluginSourceOutputData& OutputData)
{
	...
	{
		UE_LOGF(LogMicrosoftSpatialSound, Warning, "Source circular buffers should be bigger!");
	}
...
uint32 FMicrosoftSpatialSound::Run()
{
	...
	while (!SAC->IsActive() && bIsRendering)
	{
		...
		{
			UE_LOGF(LogMicrosoftSpatialSound, Warning, "Microsoft Spatial Sound has zero MaxDynamicObjects.  No sounds can play!  You need to enable Spatial Sound (Windows Sonic for Headphones) in your PC audio settings then restart Unreal.");
	...
	while (bIsRendering)
	{
		...
		{
			//UE_LOGF(LogMicrosoftSpatialSound, Warning, "Microsoft Spatial Sound buffer completion event timed out.");
		}
...
void FMicrosoftSpatialSoundModule::StartupModule()
{
	...
	{
		UE_LOGF(LogMicrosoftSpatialSound, Warning, "Failed to load the SpatialAudioClientInterop DLL");
	}
