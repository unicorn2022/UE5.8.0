...
bool FSteamSocketsSubsystem::Init(FString& Error)
{
	...
	if (GConfig)
	{
		...
		{
			UE_CLOGF(bIsDedicated, LogSockets, Warning, "SteamSockets: Missing config value for bAllowP2PPacketRelay, will be unable to determine ping functionality");
		}
	}
	...
	{
		UE_LOGF(LogSockets, Log, "SteamSockets: Set relay setting to %d", bOverrideRelays);
	...
	if (IsUsingRelayNetwork())
	{
		...
		UE_LOGF(LogSockets, Log, "SteamSockets: Initializing Network Relay");
...
{
	UE_LOGF(LogSockets, Log, "SteamSockets: Cleaning up");
...
class FSocket* FSteamSocketsSubsystem::CreateSocket(const FName& SocketType, const FString& SocketDescription, const FName& ProtocolType)
{
	...
	{
		UE_LOGF(LogSockets, Warning, "SteamSockets: Attempted to create a socket with a protocol outside of the steam network!");
		return nullptr;
	}
...
void FSteamSocketsSubsystem::DestroySocket(FSocket* Socket)
{
	if (Socket != nullptr)
	{
		...
		UE_LOGF(LogSockets, Verbose, "SteamSockets: Destroy socket called on %u", SteamSocket->InternalHandle);
...
bool FSteamSocketsSubsystem::GetHostName(FString& HostName)
{
	...
	UE_LOGF(LogSockets, Warning, "GetHostName is not supported on SteamSockets");
	return false;
}
...
void FSteamSocketsSubsystem::DumpSocketInformationMap() const
{
	...
	{
		UE_LOGF(LogSockets, Log, "SteamSockets: Socket Information Map is empty!");
		return;
	}

	UE_LOGF(LogSockets, Log, "SteamSockets: Printing Socket Information Map:\n");
	for (SocketHandleInfoMap::TConstIterator It(SocketInformationMap); It; ++It)
	{
		const FSteamSocketInformation& SocketInfo = It.Value();
		UE_LOGF(LogSockets, Log, "# %ls", *SocketInfo.ToString());
	}
#endif
}
...
bool FSteamSocketsSubsystem::Exec_Dev(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	{
		bIsHandled = true;
		UE_LOGF(LogSockets, Log, "SteamSockets: Printing Pending Socket Info");
		for (auto& Pending : PendingListenerArray)
		{
			UE_LOGF(LogSockets, Log, "# %ls", *Pending.ToString());
		}
	}
	else if (FParse::Command(&Cmd, TEXT("ClearSteamSocketInfo")))
	{
		UE_LOGF(LogSockets, Log, "SteamSockets: Clearing all socket information!");
		bIsHandled = true;
	...
	else if (FParse::Command(&Cmd, TEXT("TogglePeekMessaging")))
	{
		...
		UE_LOGF(LogSockets, Log, "SteamSockets: Set Peek Messaging to %d", bShouldTestPeek);
	}
#endif
...
void FSteamSocketsSubsystem::FSteamSocketInformation::MarkForDeletion()
{
	...
	{
		UE_LOGF(LogSockets, Verbose, "SteamSockets: Handle %u marked for deletion. Has parent? %d NetDriver definition: %ls", 
...
FSteamSocketsSubsystem::FSteamSocketInformation* FSteamSocketsSubsystem::GetSocketInfo(SteamSocketHandles InternalSocketHandle)
{
	...
	{
		UE_LOGF(LogSockets, Warning, "SteamSockets: Cannot get information on an invalid socket handle, returning null");
		return nullptr;
	}
...
void FSteamSocketsSubsystem::AddSocket(const FInternetAddr& ForAddr, FSteamSocket* NewSocket, FSteamSocket* ParentSocket)
{
	...
	{
		UE_LOGF(LogSockets, Warning, "SteamSockets: Attempted to track invalid socket data!");
		return;
	}
	...
	{
		UE_LOGF(LogSockets, Verbose, "SteamSockets: Dropped socket tracking for socket with invalid handle.");
		return;
	}
	...
	{
		UE_LOGF(LogSockets, Log, "SteamSockets: Now tracking socket %u for addr %ls, has parent? %d", NewSocket->InternalHandle, *ForAddr.ToString(true), (ParentSocket != nullptr));
		SocketInformationMap.Add(NewSocket->InternalHandle, NewSocketInfo);
	}
	else
	{
		UE_LOGF(LogSockets, Warning, "SteamSockets: A socket with the address %ls already exists, will not add another!", *ForAddr.ToString(true));
	}
}
...
void FSteamSocketsSubsystem::RemoveSocketsForListener(FSteamSocket* ListenerSocket)
{
	...
	{
		UE_LOGF(LogSockets, Warning, "SteamSockets: Attempted to free sockets attached to an invalid listener!");
		return;
	}
	...
	UE_LOGF(LogSockets, Log, "SteamSockets: Closing all sockets attached to listener %u", ListenerSocket->InternalHandle);
	for (SocketHandleInfoMap::TIterator It(SocketInformationMap); It; ++It)
	{
		FSteamSocketInformation& SocketInfo = It.Value();
		if (SocketInfo.Parent == ListenerSocket)
		{
			UE_LOGF(LogSockets, Verbose, "SteamSockets: Removed socket %u", SocketInfo.Socket->InternalHandle);
...
void FSteamSocketsSubsystem::QueueRemoval(SteamSocketHandles RemoveHandle)
{
	...
	if (SocketInfo != nullptr)
	{
		...
		UE_LOGF(LogSockets, Verbose, "SteamSockets: Marked socket %u with address %ls for removal (pending)", RemoveHandle, *Address);
		SocketInfo->MarkForDeletion();
	}
}
...
void FSteamSocketsSubsystem::LinkNetDriver(FSocket* Socket, USteamSocketsNetDriver* NewNetDriver)
{
	...
	{
		UE_LOGF(LogSockets, Warning, "SteamSockets: Attempted to create an invalid socket/netdriver pairing!");
		return;
	}
	...
	{
		UE_LOGF(LogSockets, Verbose, "SteamSockets: Dropped netdriver link with socket that has invalid handle.");
		return;
	}
	STEAM_SDK_IGNORE_REDUNDANCY_END
...
void FSteamSocketsSubsystem::AddDelayedListener(FSteamSocket* ListenSocket, USteamSocketsNetDriver* NewNetDriver)
{
	...
	{
		UE_LOGF(LogSockets, Warning, "SteamSockets: Add delayed listener cannot listen to invalid data!");
		return;
	}
...
void FSteamSocketsSubsystem::OnServerLoginComplete(bool bWasSuccessful)
{
	...
	for (auto& DelayedListener : PendingListenerArray)
	{
		...
		UE_LOGF(LogSockets, Verbose, "SteamSockets: Processed delayed listener: %ls", *DelayedListener.ToString());
	}
...
TSharedPtr<FInternetAddr> FSteamSocketsSubsystem::GetIdentityAddress()
{
	...
	else if (!IsRunningDedicatedServer() && SteamUser())
	{
		...
		{
			UE_LOGF(LogSockets, Warning, "SteamSockets: Unable to process current user's steam id!");
		}
	}
...
void FSteamSocketsSubsystem::SteamSocketEventHandler(struct SteamNetConnectionStatusChangedCallback_t* Message)
{
		...
		(Message->m_info.m_eState == k_ESteamNetworkingConnectionState_ClosedByPeer || Message->m_info.m_eState == k_ESteamNetworkingConnectionState_ProblemDetectedLocally))
	{
		...
		if (SocketInfo != nullptr && SocketInfo->Socket != nullptr && !SocketInfo->Socket->bIsListenSocket)
		{
			...
			UE_LOGF(LogSockets, Verbose, "SteamSockets: Connection %u has disconnected. Old state: %ls Reason: %ls", Message->m_hConn,
