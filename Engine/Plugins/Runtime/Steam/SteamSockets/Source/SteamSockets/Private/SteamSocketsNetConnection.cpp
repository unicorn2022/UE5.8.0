...
void USteamSocketsNetConnection::InitLocalConnection(UNetDriver* InDriver, FSocket* InSocket, const FURL& InURL, EConnectionState InState, int32 InMaxPacket, int32 InPacketOverhead)
{
	...
	{
		UE_LOGF(LogNet, Error, "SteamSockets: InitLocalConnection: Unable to get SocketSubsystem");
		Close();
		return;
	}
	...
	{
		UE_LOGF(LogNet, Error, "SteamSockets: Cannot create connection address from NetConnection URL");
		Close();
		return;
	}
...
void USteamSocketsNetConnection::LowLevelSend(void* Data, int32 CountBits, FOutPacketTraits& Traits)
{
	...
	if (ConnectionSocket != nullptr && SocketSub)
	{
		...
		{
			UE_LOGF(LogNet, Error, "SteamSockets: LowLevelSend: Attempted to send %d bytes of data, which is over the %d maximum", 
		...
		if (BytesToSend > 0)
		{
			...
			{
				UE_LOGF(LogNet, Warning, "SteamSockets: LowLevelSend: Could not send %d bytes of data got error %d", BytesToSend, (int32)SocketSub->GetLastErrorCode());
			}
		}
	}
	else if (SocketSub)
	{
		UE_LOGF(LogNet, Error, "SteamSockets: LowLevelSend: Attempted to send data over a netconnection that has no socket!");
	...
	if (bInConnectionlessHandshake)
	{
		...
		if (SteamNetDriver && !SteamNetDriver->ArePacketHandlersDisabled() && SteamNetDriver->ConnectionlessHandler.IsValid() && SteamNetDriver->StatelessConnectComponent.IsValid())
		{
			...
			if (!RawPacket.bError && StatelessConnect->HasPassedChallenge(SharedAddress, bRestartedHandshake) && !bRestartedHandshake)
			{
				...
				UE_LOGF(LogNet, Log, "SteamSockets: Connectionless handshake complete");
