...
FString FSteamSocketsPing::GetHostPingData() const
{
	...
	if (!IsUsingP2PRelays())
	{
		UE_LOGF(LogNet, Warning, "SteamSockets: GetHostPingData was called but relays are disabled! This is invalid!");
		return TEXT("");
	}

	if (!SteamNetworkingUtils() || !SocketSub)
	{
		UE_LOGF(LogNet, Warning, "SteamSockets: Could not get host ping data as SteamNetworking is not available.");
		return TEXT("");
	}
	...
	if (PingDataAge == -1)
	{
		...
		UE_LOGF(LogNet, Log, "SteamSockets: No ping data is available at this time");
		return TEXT("");
	}
...
int32 FSteamSocketsPing::GetPingFromHostData(const FString& HostPingStr) const
{
	...
	{
		UE_LOGF(LogNet, Warning, "SteamSockets: Could not determine ping data as SteamNetworking is unavailable.");
		return -1;
	}
	...
	else
	{
		...
		UE_LOGF(LogNet, Log, "SteamSockets: Could not parse the host location in order to determine ping.");
		return -1;
	}
}
