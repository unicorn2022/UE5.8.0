...
bool UMobileInstalledContent::Mount(int32 InPakOrder, const FString& InMountPoint)
{
	...
	if (InstalledManifest.IsValid())
	{
		...
		for (const FString& FileName : InstalledFileNames)
		{
			if (FPaths::GetExtension(FileName) == TEXT("pak"))
			{
				...
				{
					UE_LOGF(LogMobilePatchingUtils, Log, "Mounted = %ls, Order = %d, MountPoint = %ls", *PakFullName, PakOrder, !MountPount ? TEXT("(null)") : MountPount);
					bMounted = true;
				}
				else
				{
					UE_LOGF(LogMobilePatchingUtils, Error, "Failed to mount pak = %ls", *PakFullName);
					bMounted = false;
					break;
				}
			}
	...
	{
		UE_LOGF(LogMobilePatchingUtils, Log, "No installed manifest, failed to mount");
	}
...
static void OnInstallComplete(const IBuildInstallerRef& Installer, UMobilePendingContent* MobilePendingContent, FOnContentInstallSucceeded OnSucceeded, FOnContentInstallFailed OnFailed)
{
	if (MobilePendingContent == nullptr)
	{
		...
		UE_LOGF(LogMobilePatchingUtils, Error, "Installation Failed. MobilePendingContent is null");
		return;
	}
	...
	if (Installer->CompletedSuccessfully())
	{
		...
		{
			UE_LOGF(LogMobilePatchingUtils, Error, "Failed to save manifest to disk. %ls", *ManifestFullFilename);
		}
	...
	else
	{
		...
		UE_LOGF(LogMobilePatchingUtils, Error, "Installation Failed. Code %d Msg: %ls", (int32)ErrorCode, *ErrorText.ToString());
...
void UMobilePendingContent::StartInstall(FOnContentInstallSucceeded OnSucceeded, FOnContentInstallFailed OnFailed)
{
	...
	UE_LOGF(LogMobilePatchingUtils, Log, "Download size = %.2f MB", DownloadSize);
	UE_LOGF(LogMobilePatchingUtils, Log, "Required disk space = %.2f MB", RequiredDiskSpace);
	UE_LOGF(LogMobilePatchingUtils, Log, "Disk free space = %.2f MB", DiskFreeSpace);
...
static void OnRemoteManifestReady(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bSucceeded, UMobilePendingContent* MobilePendingContent, FOnRequestContentSucceeded OnSucceeded, FOnRequestContentFailed OnFailed)
{
	...
	ON_SCOPE_EXIT
	{
		if (ErrorCode != ERequestContentError::NoError)
		{
			...
			UE_LOGF(LogMobilePatchingUtils, Error, "ErrorText: %ls. Code %d", *ErrorText.ToString(), (int32)ErrorCode);
			OnFailed.ExecuteIfBound(ErrorText, (int32)ErrorCode);
		}
	};
	...
	{
		UE_LOGF(LogMobilePatchingUtils, Error, "Failed to download manifest. ResponseCode = %d, ResponseMsg = %ls", ResponseCode, *Response->GetContentAsString());
