...
bool FOpenXRActionPoseManager::FilterSpaceHistory(const TArray<FOpenXRLocateSpacePacket>& RawHistory, TArray<FOpenXRLocateSpacePacket>& FilteredHistory)
{
	...
	{
		UE_LOGF(LogXRScribePoseManager, Warning, "Unable to process space history with non-stage base space");
		return false;
	}
...
void FOpenXRActionPoseManager::ProcessCapturedReferenceSpaceHistory(const TArray<FOpenXRLocateSpacePacket>& SpaceHistory)
{
	...
	{
		UE_LOGF(LogXRScribePoseManager, Warning, "Unable to process space history for non-view reference spaces");
		return;
	}
...
void FOpenXRActionPoseManager::CalculateCapturedTimeRange()
{
	...
	for (const auto& ActionStateHistory : CapturedRawBooleanActionStates)
	{
		...
		UE_LOGF(LogXRScribePoseManager, Verbose, "Bool action %p - active count: %d / %d", ActionStateHistory.Key, NumActive, ActionStateHistory.Value.Num());
	}
	for (const auto& ActionStateHistory : CapturedRawFloatActionStates)
	{
		...
		UE_LOGF(LogXRScribePoseManager, Verbose, "Float action %p - active count: %d / %d", ActionStateHistory.Key, NumActive, ActionStateHistory.Value.Num());
	}
	for (const auto& ActionStateHistory : CapturedRawVectorActionStates)
	{
		...
		UE_LOGF(LogXRScribePoseManager, Verbose, "Vector2F action %p - active count: %d / %d", ActionStateHistory.Key, NumActive, ActionStateHistory.Value.Num());
	}
...
void FOpenXRActionPoseManager::ProcessCapturedHistories()
{
	...
	for (const auto& SpaceHistoryTuple : CapturedRawSpaceHistories)
	{
		...
		{
			UE_LOGF(LogXRScribePoseManager, Warning, "Unable to process space history for captured space: %p", reinterpret_cast<const void*>(CapturedLocatedSpace));
		}
	}
...
bool FOpenXRActionPoseManager::ValidateSubpath(const TSet<FName>& ValidSubpaths, XrPath ActionSubPath)
{
	...
	if (ActionSubPath != XR_NULL_PATH)
	{
		...
		{
			bSubpathValid = false;
			UE_LOGF(LogXRScribePoseManager, Warning, "Unknown emulated path (%lld) to check against action subpaths", ActionSubPath);
		}
	}
