...
// I could also use PRAGMA_DISABLE_CAST_FUNCTION_TYPE_MISMATCH_WARNINGS
...
EXRScribeRunMode DetermineRunMode(EXRScribeRunMode FallbackRunMode, FString& EmulationLoadPath)
{
	...
	if (bEmulationCLI)
	{
		UE_LOGF(LogXRScribeAPI, Log, "xremu command-line argument detected to emulate built-in capture");
		...
		if (RequestedEmulationProfile == TEXT("mq2"))
		{
			...
			UE_LOGF(LogXRScribeAPI, Log, "mq2 built-in capture requested");
		}
		else if (RequestedEmulationProfile == TEXT("vi"))
		{
			EmulationLoadPath = ExtrasDir / TEXT("vi.xrs");
			UE_LOGF(LogXRScribeAPI, Log, "vi built-in capture requested");
		}
		else
		{
			UE_LOGF(LogXRScribeAPI, Warning, "Unknown built-in capture, deferring to config run-mode");
			bEmulationCLI = false;
		}
	}
...
public:
	...
	void SetFallbackRunMode(int32 Fallback) override
	{
		...
		{
			UE_LOGF(LogXRScribeAPI, Warning, "Invalid run mode override specified (%d), ignoring", Fallback);
		}
	}
	...
	bool SetChainedGetProcAddr(PFN_xrGetInstanceProcAddr InChainedGetProcAddr) override
	{
		...
		{
			UE_LOGF(LogXRScribeAPI, Log, "Capture layer selected");

			if (InChainedGetProcAddr != nullptr)
			{
				UE_LOGF(LogXRScribeAPI, Log, "Valid PFN_xrGetInstanceProcAddr handed off to Capture Layer");
			...
			{
				UE_LOGF(LogXRScribeAPI, Warning, "Invalid PFN_xrGetInstanceProcAddr, Capture Layer disabled");
				return false;
			}
		}
		else if (SelectedRunMode == EXRScribeRunMode::Emulate)
		{
			UE_LOGF(LogXRScribeAPI, Log, "Emulation layer selected");
			...
			{
				UE_LOGF(LogXRScribeAPI, Log, "Emulation Layer enabled");
			}
			else
			{
				UE_LOGF(LogXRScribeAPI, Log, "Emulation Layer disabled - unable to load the capture file.");
			}
