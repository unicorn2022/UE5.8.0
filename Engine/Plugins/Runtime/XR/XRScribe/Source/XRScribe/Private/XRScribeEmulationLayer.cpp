...
{
	UE_LOGF(LogXRScribeEmulate, Error, "Unimplemented OpenXR API: %d", (uint32)ApiId);
...
#if defined(XR_USE_GRAPHICS_API_D3D12)
	UE_LOGF(LogXRScribeEmulate, Log, "XRScribe emulation supports D3D12");
#endif
}
...
bool FOpenXREmulationLayer::LoadCaptureFromFile(const FString& EmulationLoadPath)
{
	...
	{
		UE_LOGF(LogXRScribeEmulate, Log, "Capture successfully loaded: %ls", *EmulationLoadPath);
		...
		{
			UE_LOGF(LogXRScribeEmulate, Error, "Failed to decode capture");
	...
	{
		UE_LOGF(LogXRScribeEmulate, Log, "Capture '%ls' failed to load.", *EmulationLoadPath);
		return false;
	}
}
...
bool FOpenXREmulationLayer::SessionHandleCheck(XrSession InputSession)
{
	...
	{
		UE_LOGF(LogXRScribeEmulate, Error, "Session handle invalid: %llx", reinterpret_cast<intptr_t>(InputSession));
		return false;
	}
...
XrResult FOpenXREmulationLayer::SetupGraphicsBinding(const XrSystemId SystemId, const void* InBinding, TUniquePtr<FOpenXREmulatedSession>& Session)
{
	...
	{
		UE_LOGF(LogXRScribeEmulate, Log, "Session requesting D3D12 graphics binding");
	}
	else
#endif
	{
...

	{
		...
		{
			UE_LOGF(LogXRScribeEmulate, Error, "GraphicsRequirement call missing");
...
EPixelFormat FOpenXREmulationLayer::ConvertPlatformFormat(int64 PlatformFormat)
{
	...
	if (BindingType == XR_TYPE_GRAPHICS_BINDING_D3D12_KHR)
	{
		...
		if (DX12PlatformToEngineFormat.Contains(DxgiFormat))
		{
			...
			UE_LOGF(LogXRScribeEmulate, Log, "DXGI_FORMAT %d selects EPixelFormat for swapchain: %d", DxgiFormat, SelectedFormat);
		}
		else
		{
			UE_LOGF(LogXRScribeEmulate, Error, "Unknown DXGI_FORMAT requested for swapchain: %d", DxgiFormat);
		}
	}
#endif
...
XrResult FOpenXREmulationLayer::XrLayerCreateReferenceSpace(XrSession session, const XrReferenceSpaceCreateInfo* createInfo, XrSpace* space)
{
	...
	{
		UE_LOGF(LogXRScribeEmulate, Error, "CreateReferenceSpace: nullptr arguments");
		return XR_ERROR_VALIDATION_FAILURE;
	}

	if (createInfo->type != XR_TYPE_REFERENCE_SPACE_CREATE_INFO)
	{
		UE_LOGF(LogXRScribeEmulate, Error, "CreateReferenceSpace: invalid XrStructureType for XrReferenceSpaceCreateInfo");
		return XR_ERROR_VALIDATION_FAILURE;
	}

	if (!CaptureDecoder.GetReferenceSpaceTypes().Contains(createInfo->referenceSpaceType))
	{
		UE_LOGF(LogXRScribeEmulate, Error, "CreateReferenceSpace: unsupported XrReferenceSpaceType");
...

	{
		...
		{
			UE_LOGF(LogXRScribeEmulate, Error, "CreateReferenceSpace: invalid reference pose");
...
XrResult FOpenXREmulationLayer::XrLayerGetReferenceSpaceBoundsRect(XrSession session, XrReferenceSpaceType referenceSpaceType, XrExtent2Df* bounds)
{
	...
	if (bounds == nullptr)
	{
		UE_LOGF(LogXRScribeEmulate, Error, "GetReferenceSpaceBoundsRect: nullptr bounds");
		return XR_ERROR_VALIDATION_FAILURE;
	}

	if (!CaptureDecoder.GetReferenceSpaceTypes().Contains(referenceSpaceType))
	{
		UE_LOGF(LogXRScribeEmulate, Error, "GetReferenceSpaceBoundsRect: unsupported XrReferenceSpaceType");
		return XR_ERROR_REFERENCE_SPACE_UNSUPPORTED;
	}
	...
	if (!BoundsMap.Contains(referenceSpaceType))
	{
		...
		UE_LOGF(LogXRScribeEmulate, Error, "GetReferenceSpaceBoundsRect: unknown bounds for supported reference space");
		return XR_ERROR_RUNTIME_FAILURE;
	}
...
XrResult FOpenXREmulationLayer::XrLayerCreateActionSpace(XrSession session, const XrActionSpaceCreateInfo* createInfo, XrSpace* space)
{
	...
	if ((createInfo == nullptr) || (space == nullptr))
	{
		UE_LOGF(LogXRScribeEmulate, Error, "CreateActionSpace: nullptr arguments");
		return XR_ERROR_VALIDATION_FAILURE;
	}

	if (createInfo->type != XR_TYPE_ACTION_SPACE_CREATE_INFO)
	{
		UE_LOGF(LogXRScribeEmulate, Error, "CreateActionSpace: invalid XrStructureType for XrActionSpaceCreateInfo");
		return XR_ERROR_VALIDATION_FAILURE;
	}

	if (!ActionHandleCheck(createInfo->action))
	{
		UE_LOGF(LogXRScribeEmulate, Error, "CreateActionSpace: unknown action");
		return XR_ERROR_VALIDATION_FAILURE;
	}
...

	{
		...
		if (!QuatCheck.IsNormalized())
		{
			UE_LOGF(LogXRScribeEmulate, Error, "CreateActionSpace: invalid pose");
			return XR_ERROR_POSE_INVALID;
		}
	}
...
XrResult FOpenXREmulationLayer::XrLayerEndFrame(XrSession session, const XrFrameEndInfo* frameEndInfo)
{
	...
	//{
	//	UE_LOGF(LogXRScribeEmulate, Error, "EndFrame: unsupported layer count");
...
XrResult FOpenXREmulationLayer::XrLayerGetActionStatePose(XrSession session, const XrActionStateGetInfo* getInfo, XrActionStatePose* state)
{
	...
	{
		UE_LOGF(LogXRScribeEmulate, Error, "GetActionStatePose: unknown action");
		return XR_ERROR_VALIDATION_FAILURE;
	}
...
XrResult FOpenXREmulationLayer::XrLayerGetD3D12GraphicsRequirementsKHR(XrInstance instance, XrSystemId systemId, XrGraphicsRequirementsD3D12KHR* graphicsRequirements)
{
	...
	{
		graphicsRequirements->adapterLuid = GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU()->GetAdapterLuid();
	}
	else
	{
