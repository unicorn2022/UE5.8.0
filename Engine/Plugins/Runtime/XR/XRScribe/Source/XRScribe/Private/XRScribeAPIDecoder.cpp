...
bool FOpenXRCaptureDecoder::DecodeDataFromMemory()
{
	...
	while (CurByteIndex < NumBytes)
	{
		...
		{
			UE_LOGF(LogXRScribeEmulate, Error, "Encountered invalid magic byte in packet %lld while decoding capture", CurPacketIndex);
			return false;
		}
		...
		{
			UE_LOGF(LogXRScribeEmulate, Error, "Encountered invalid API ID in packet %lld while decoding capture", CurPacketIndex);
			return false;
		}
		...
		if (!bValid)
		{
			UE_LOGF(LogXRScribeEmulate, Error, "Encountered invalid XrResult in packet %lld while decoding capture", CurPacketIndex);
			return false;
		}
