...
bool USoundscapeColorPointHashMap::AddColorPointToHash(const FVector& Location, const FGameplayTag ColorPoint)
{
	...
	{
		UE_LOGF(LogSoundscapeSubsystem, Warning, "Location out of Hash Bounds");
...
{
	UE_LOGF(LogSoundscapeSubsystem, Verbose, "Subsystem Initializing");
	...
	if (const USoundscapeSettings* ProjectSettings = GetDefault<USoundscapeSettings>())
	{
		if (ProjectSettings->SoundscapePaletteCollection.IsEmpty() == false)
		{
			...
			UE_LOGF(LogSoundscapeSubsystem, Verbose, "Project settings Soundscape Palette Collection is valid");

			AddPaletteCollection(FName("GlobalPaletteCollection"), PaletteCollectionToAdd);			
		}
		else
		{
			UE_LOGF(LogSoundscapeSubsystem, Verbose, "Project settings Soundscape Palette Collection is empty");
		}
	...
	{
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Invalid Project Settings");
	}
...
{
	UE_LOGF(LogSoundscapeSubsystem, Verbose, "Subsystem Deinitializing");
...
bool USoundscapeSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
	...
	{
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Subsystem will not be created based on parent rules");
	...
	if (UWorld* OuterWorld = Outer->GetWorld())
	{
		...
		{
			UE_LOGF(LogSoundscapeSubsystem, Verbose, "Audio device valid, will create subsystem");
	...
	UE_LOGF(LogSoundscapeSubsystem, Verbose, "Subsystem will not be created");
...
void USoundscapeSubsystem::SetState(FGameplayTag SoundscapeState)
{
	if (SoundscapeState.IsValid())
	{
		...
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Set State called");

		UpdateState();
	}
	else
	{
		UE_LOGF(LogSoundscapeSubsystem, Warning, "Set State called with invalid State Tag");
	}
}
...
void USoundscapeSubsystem::ClearState(FGameplayTag SoundscapeState)
{
	if (SoundscapeState.IsValid())
	{
		...
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Clear State called");

		UpdateState();
	}
	else
	{
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Clear State called with invalid State Tag");
	}
}
...
bool USoundscapeSubsystem::AddPaletteCollection(FName PaletteCollectionName, FSoundscapePaletteCollection PaletteCollection)
{
	if (UnloadedPaletteCollections.Contains(PaletteCollectionName) == false)
	{
		...
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Palette Collection %ls added", *PaletteCollectionName.ToString());

		return LoadPaletteCollection(PaletteCollectionName);
	}

	UE_LOGF(LogSoundscapeSubsystem, Verbose, "Add Palette Collection called with empty Palette Collection");
...
bool USoundscapeSubsystem::RemovePaletteCollection(FName PaletteCollectionName)
{
	if (UnloadedPaletteCollections.Find(PaletteCollectionName))
	{
		if (UnloadPaletteCollection(PaletteCollectionName))
		{
			...
			UE_LOGF(LogSoundscapeSubsystem, Verbose, "Palette Collection %ls removed", *PaletteCollectionName.ToString());
	...
	UE_LOGF(LogSoundscapeSubsystem, Verbose, "Remove Palette Collection called, but the name could not be found");
...
bool USoundscapeSubsystem::LoadPaletteCollection(FName PaletteCollectionName)
{
	...
	{
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Attempting to load Palette Collection %ls", *PaletteCollectionName.ToString());
		...
		{
			UE_LOGF(LogSoundscapeSubsystem, Verbose, "Attempted to load Palette Collection %ls but Collection Empty", *PaletteCollectionName.ToString());
	...
	UE_LOGF(LogSoundscapeSubsystem, Verbose, "Attempted to load Palette Collection %ls but failed", *PaletteCollectionName.ToString());
	...
	for (FSoftObjectPath& ObjPath : PaletteCollectionToLoad.SoundscapePaletteCollection)
	{
		if (UObject* Object = ObjPath.ResolveObject())
		{
			...
			if (SoundscapePalette)
			{
				...
				UE_LOGF(LogSoundscapeSubsystem, Verbose, "Valid Soundscape Palette %ls in Palette Collection %ls", *SoundscapePalette->GetFullName(), *PaletteCollectionName.ToString());
			}
			else
			{
				UE_LOGF(LogSoundscapeSubsystem, Verbose, "Invalid Soundscape Palette in Palette Collection %ls", *PaletteCollectionName.ToString());
			}
		}
		else
		{
			UE_LOGF(LogSoundscapeSubsystem, Verbose, "Failed to load %ls", *ObjPath.ToString());
		}
	}
...
bool USoundscapeSubsystem::UnloadPaletteCollection(FName PaletteCollectionName)
{
	if (UnloadedPaletteCollections.Find(PaletteCollectionName))
	{
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Attempting to unload Palette Collection %ls", *PaletteCollectionName.ToString());
		...
		for (USoundscapePalette* PaletteKeyToStop : PalettesToUnloadStopAndRemove)
		{
			if (PaletteKeyToStop)
			{
				if (UActiveSoundscapePalette* ActiveSoundscapePalette = ActivePalettes.FindRef(PaletteKeyToStop))
				{
					...
					UE_LOGF(LogSoundscapeSubsystem, Verbose, "Soundscape Palette %ls stopped", *ActiveSoundscapePalette->GetFullName());
				}
			}
		}
		...
		UE_LOGF(LogSoundscapeSubsystem, Verbose, "Palette Collection %ls unloaded", *PaletteCollectionName.ToString());

		return true;
	}

	UE_LOGF(LogSoundscapeSubsystem, Verbose, "Attempted to unload Palette Collection %ls but could not find matching collection", *PaletteCollectionName.ToString());
		...
		if (USoundscapePalette* SoundscapePalette = *PaletteIterator)
		{
			...
			if ((SoundscapePalette->SoundscapePalettePlaybackConditions.IsEmpty() || SoundscapePalette->SoundscapePalettePlaybackConditions.Matches(SubsystemState)) && ActivePalettes.Contains(SoundscapePalette) == false)
			{
				...
				if (World)
				{
					...
					UE_LOGF(LogSoundscapeSubsystem, Verbose, "Soundscape Palette %ls started", *ActiveSoundscapePalette->GetFullName());
