...
protected:
	...
	// GameInput polling is designed for multi-threaded access per Microsoft docs.
	virtual EInputDeviceThreadAffinity GetThreadAffinity() const override { return EInputDeviceThreadAffinity::InputThreadSafe; }
	...
	/**
	 * Returns true if the given IGameInputDevice is one which is allowed to be processed based on the Game Input Developer Settings.
	 * 
	 * This may return false for devices which have specific PID/VID's which this project does not wish GameInput to process,
	 * or if the device is within a specific family that we do not want to process.
	 */
	GAMEINPUTBASE_API virtual bool IsSupportedDevice(IGameInputDevice* Device) const;
...
private:
	...
	/**
	 * Handle haptics being notified as ready from the given device.
	 * 
	 * This should only be called internally from this device interface in response to
	 * a callback from the GameInput api
	 */
	void HandleHapticsReady(IGameInputDevice* Device, uint64 Timestamp);

	GAMEINPUTBASE_API void TeardownHapticAudio(IGameInputDevice* Device);
	
protected:
	
	/**
	 * Any platform specific haptic setup can be done here in an override.
	 * The base implementation sets up WASAPI-backed audio endpoints for haptic audio.
	 *
	 * Note: You likely want to wrap the implementation of this function in #if UE_GAME_INPUT_SUPPORTS_HAPTICS.
	 */
	GAMEINPUTBASE_API virtual void HandleHapticsReady_Impl(IGameInputDevice* Device, uint64 Timestamp, struct GameInputHapticInfo& HapticInfo);

	/**
	 * Tears down the WASAPI haptic audio device associated with the given GameInput device on disconnect.
	 * Only has effect when UE_GAME_INPUT_SUPPORTS_HAPTICS is true.
	 */
	GAMEINPUTBASE_API virtual void TeardownHapticAudio_Impl(IGameInputDevice* Device);
	...
	void CoalesceDeferredDeviceConnectionChanges();
