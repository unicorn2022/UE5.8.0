...
/**
* UPROP friendly representation of GameInput's GameInputDeviceFamily type.
*/
UENUM(meta = (BitFlags, UseEnumValuesAsMaskValuesInEditor = "true"))
enum class EGameInputDeviceFamily : uint32
{
	None 	  =  0 UMETA(Hidden),
	Virtual   =  1 << 0,
	Unknown   =  1 << 1,
	...
	I8042     =  1 << 5,
	Aggregate =  1 << 6,
	All		  =  Virtual | Unknown | XboxOne | Xbox360 | Hid | I8042 | Aggregate UMETA(Hidden),
};

ENUM_CLASS_FLAGS(EGameInputDeviceFamily)
...
enum class ERawDeviceReportTranslationBehavior : uint8
{
	...
	// Treat this raw report index as two values, reporting two different sets of buttons.
	// The lower 4 bits represent one set of buttons, and the upper 4 bits represent another set of buttons.
	TreatAsPackedAxisUint8 = 5,
};
...
struct FGameInputRawDeviceReportData
{
	 ...
	 */
	UPROPERTY(EditAnywhere, Config, Category = "Game Input Device", meta=(EditCondition="TranslationBehavior != ERawDeviceReportTranslationBehavior::TreatAsButtonBitmask && TranslationBehavior != ERawDeviceReportTranslationBehavior::TreatAsPackedAxisUint8", EditConditionHides))
	FName KeyName = NAME_None;
	...
	*/
	...
	/**
	* Array of FKey names to report when the lower bits of this raw value index are detected. Each element of this array will be reported once per-frame, known as an "input chord"
	*/
	UPROPERTY(EditAnywhere, Config, Category = "Game Input Device", meta = (EditCondition = "TranslationBehavior == ERawDeviceReportTranslationBehavior::TreatAsPackedAxisUint8", EditConditionHides))
	TArray<FName> LowerBitKeys;

	/**
	* Array of FKey names to report when the upper bits of this raw value index are detected. Each element of this array will be reported once per-frame, known as an "input chord" 
	*/
	UPROPERTY(EditAnywhere, Config, Category = "Game Input Device", meta = (EditCondition = "TranslationBehavior == ERawDeviceReportTranslationBehavior::TreatAsPackedAxisUint8", EditConditionHides))
	TArray<FName> UpperBitKeys;

	/**
	 * If true, then this packed bitmask value will be treated as an analog button.
	 * This means that it will use "OnControllerAnalog", reporting a float value, instead of "OnControllerButtonPressed" using input events. 
	 */
	UPROPERTY(EditAnywhere, Config, Category = "Game Input Device", meta = (EditCondition = "TranslationBehavior == ERawDeviceReportTranslationBehavior::TreatAsPackedAxisUint8", EditConditionHides))
	bool bTreatValuesAsAnalog = false;
...
struct FGameInputDeviceConfiguration
{
	...
	*/
	UPROPERTY(EditAnywhere, Config, Category = "Device Settings", meta = (InlineEditConditionToggle, ConfigRestartRequired=true))
	...
	*/
	UPROPERTY(EditAnywhere, Config, Category = "Device Settings", meta=(EditCondition="bOverrideHardwareDeviceIdString", ConfigRestartRequired=true))
...
public:
	...
	*/
	...
	/**
	 * If true, then we will process the GameInputKindSensors type such as Gyrometer, Orientation, and Accelerometer.
	 *
	 * Experimental.
	 */
	UE_EXPERIMENTAL(5.8, "Experimental, API may change in the future")
	UPROPERTY(Config, EditAnywhere, Category="Processing Options")
	bool bProcessSensors = false;
...
public:
	...
	*/
	...
	/**
	* Returns true if the gien device should be completely ignored by GameInput processing.
	* If a device is ignored, then it will not even fire connection event delegates.
	*/
	bool ShouldIgnoreDevice(const GameInputDeviceInfo* const Info) const; 
...
protected:
	...
	/**
	 * A list of devices for game input to completely ignore. If a device is connected that is in this list,
	 * no processors will ever be created for it at all.
	 */
	UPROPERTY(EditAnywhere, Config, Category = "Device Settings")
	TArray<FGameInputDeviceIdentifier> DevicesToIgnore;

	/**
	 * A bitmask of which GameInput device families you want to support. 
	 * If a family is not enabled as a part of this list, it will be ignored.
	 * 
	 * Default: All families are supported.
	 */
	UPROPERTY(EditAnywhere, Config, Category = "Device Settings", meta = (Bitmask, BitmaskEnum = "/Script/GameInputBase.EGameInputDeviceFamily"))
	uint32 SupportedDeviceFamilies;
...
public:
	...
	*/
	...
	/**
	 * @return True if this build target supports haptics (UE_GAME_INPUT_SUPPORTS_HAPTICS) and the project setting
	 * for enabling haptic support is true.
	 */
	bool IsHapticSupportEnabled() const;
	
protected:
	
	/**
	 * If true, then Game Input will listen for the GameInputDeviceHapticInfoReady device status on platforms
	 * which UE_GAME_INPUT_SUPPORTS_HAPTICS is true.
	 *
	 * Haptic sensor support is like a "fancy" force feedback. On supported devices, an audio endpoint is created.
	 * You can use audio files to drive sound and vibration on the devices.
	 */
	UPROPERTY(Config, EditAnywhere, Category="Experimental|Game Input")
	bool bEnableHapticSensorSupport;
