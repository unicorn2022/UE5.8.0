...
void FWorldConditionResultInvalidationHandle::InvalidateResult() const
{
	...
	{
		UE_LOGF(LogWorldCondition, Warning, "World Condition: Trying to access freed state memory while calling InvalidateResult(). "
...
FWorldConditionQueryState::~FWorldConditionQueryState()
{
	...
	if (IsInitialized() && Definition != nullptr)
	{
		...
		{
			// During GC the Owner may already have been destroyed in the same purge pass,
			// leaving a dangling pointer. IsValid() would dereference it and crash, so we
			// can only inspect the Owner when we are not running inside garbage collection.
			const bool bCanInspectOwner = !IsGarbageCollecting();
			if (bCanInspectOwner && IsValid(Owner))
			{
				// We can call Deactivate(), but we don't know the context data, so some uninitialization can potentially be incomplete.
				UE_LOGF(LogWorldCondition, Warning, "World Condition: State %p owned by %ls is still active on destructor, "
			...
			{
				// Either the owner is not valid or we are inside GC and cannot safely inspect it.
				// Fall back to Free() which only releases our own heap memory and is GC-safe.
				UE_LOGF(LogWorldCondition, Warning, "World Condition: State %p is still active on destructor (%ls), "
					"calling Free() without Deactivate(), might leak memory or resources.",
					this,
					bCanInspectOwner ? TEXT("invalid Owner") : TEXT("inside garbage collection"));
	...
	if (Memory.IsValid())
	{
		...
		UE_LOGF(LogWorldCondition, Error, "World Condition: State %p has still allocated memory in destructor, might leak memory.", Memory.Get());
		Memory = nullptr;
	}
}
...
void FWorldConditionQueryState::InitializeInternal(const UObject* InOwner, const FSharedStruct InSharedDefinition)
{
	...
	{
		UE_LOGF(LogWorldCondition, Error, "World Condition: Trying to initialize query state with invalid definition for %ls.", *GetNameSafe(Owner));
		return;
	}
...
void FWorldConditionQuerySharedDefinition::PostSerialize(const FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		if (!Link(SerializedObject))
		{
			...
			{
				TStringBuilder<256> PropertyPath;
				...
				UE_LOGF(LogWorldCondition, Error, "World Condition: Failed to link query for property '%ls' in '%ls'"
					, *PropertyPath
...
bool FWorldConditionQuerySharedDefinition::Link(const UObject* Outer)
{
	...
	if (!Schema)
	{
		UE_LOGF(LogWorldCondition, Error, "World Condition: invalid schema class %ls in a query definition for %ls."
	...
	for (int32 Index = 0; Index < Conditions.Num(); Index++)
	{
		...
		if (!Condition)
		{
			UE_LOGF(LogWorldCondition, Error, "World Condition: Encountered empty condition in a query definition for %ls. This may be caused by the query using a struct from a plugin that is not loaded, or the condition type is removed.",
				*GetNameSafe(Outer));
			return false;
		}
...
bool FWorldConditionQueryDefinition::Initialize(const UObject* Outer, const EInitializationMode InitializationMode)
{
	...
	{
		UE_LOGF(LogWorldCondition, Warning, "World Condition: Failed to initialize query for %ls due to missing schema.", *GetFullNameSafe(Outer));
		return false;
	}
	...
	for (FWorldConditionEditable& EditableCondition : EditableConditions)
	{
		if (EditableCondition.Condition.IsValid())
		{
			...
			{
				UE_LOGF(LogWorldCondition, Warning, "World Condition: Query for %ls contains condition of type %ls that is not allowed by schema %ls.",
...
bool FWorldConditionQueryDefinition::Serialize(FArchive& Ar)
{
	...
	// Lazy-build SharedDefinition BEFORE the property serialization runs.
	// Save runs Serialize twice - once for the name harvest pass and once for the actual
	// write - and both must reference the same set of FNames. If we built SharedDefinition
	// AFTER serialize, the harvest pass would see SharedDefinition.IsValid() == false and
	// skip the inner SerializeTaggedProperties; the write pass would then see it valid (we
	// just built it) and reference names like "Conditions" and "Index" that harvest never
	// recorded, tripping LinkerSave's "Name X is not mapped" critical-error ensure and
	// flipping save_assets to False.
	if (Ar.IsSaving()
		&& Ar.IsPersistent()
		&& !SharedDefinition.IsValid()
		&& EditableConditions.Num() > 0)
	{
		const FUObjectSerializeContext* LoadContext = FUObjectThreadContext::Get().GetSerializeContext();
...
#if WITH_EDITORONLY_DATA
	// Loading-side lazy build. The save-side equivalent moved to before the property
	// serialization above to keep the harvest and write passes deterministic.
	if (Ar.IsLoading()
		&& !SharedDefinition.IsValid()
		&& EditableConditions.Num() > 0)
	{
