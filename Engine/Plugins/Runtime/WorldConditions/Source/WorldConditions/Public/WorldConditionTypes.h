...
struct FWorldConditionContextDataDesc
{
	...
	bool MatchesType(const UStruct* TypeToMatch) const
	{
		if (!Struct)
		{
			return false;
		...
		if (Struct->IsChildOf(TypeToMatch))
		{
			return true;
		}

		if (const UClass* DescClass = Cast<UClass>(Struct))
		{
			return DescClass->ImplementsInterface(Cast<UClass>(TypeToMatch));
		}
