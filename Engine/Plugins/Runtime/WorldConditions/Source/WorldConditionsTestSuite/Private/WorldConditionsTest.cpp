...
// Tests that FWorldConditionQuerySharedDefinition is properly linked when FWorldConditionQueryDefinition
// is nested inside a UStruct. In that case, UObject serialize context (SerializedObject) may be null
// when PostSerialize is called, and Link() must still be invoked.
struct FWorldConditionTest_SerializationInStruct : FAITestBase
{
	virtual bool InstantTest() override
	{
		UWorldConditionOwnerClassWithNestedStruct* Owner = NewAutoDestroyObject<UWorldConditionOwnerClassWithNestedStruct>();
		const bool bInitialized = Owner->Container.Definition.Initialize(&GetWorld(), UWorldConditionTestSchema::StaticClass(),
			{
				FWorldConditionEditable(0, EWorldConditionOperator::Copy, FConstStructView::Make(FWorldConditionTest())),
				FWorldConditionEditable(0, EWorldConditionOperator::And, FConstStructView::Make(FWorldConditionTest())),
			});
		...
		AITEST_TRUE("Query definition should get initialized", bInitialized);
		AITEST_TRUE("Query definition should be valid before serialization", Owner->Container.Definition.IsValid());
		...
		// Read back into a new object WITHOUT setting SerializedObject in the context.
		// This replicates the case where FWorldConditionQueryDefinition is nested inside a UStruct,
		// where the UObject serialize context is not set up by the caller (e.g., raw struct serialization).
		Owner = NewAutoDestroyObject<UWorldConditionOwnerClassWithNestedStruct>();
		FMemoryReader Reader(Data);
		FObjectAndNameAsStringProxyArchive ReaderProxy(Reader, /*bInLoadIfFindFails*/true);
		Owner->Serialize(ReaderProxy);

		AITEST_TRUE("Query definition nested in a struct should be linked after loading", Owner->Container.Definition.IsValid());

		return true;
	}
};
IMPLEMENT_AI_INSTANT_TEST(FWorldConditionTest_SerializationInStruct, "System.AI.WorldConditions.SerializationInStruct");
