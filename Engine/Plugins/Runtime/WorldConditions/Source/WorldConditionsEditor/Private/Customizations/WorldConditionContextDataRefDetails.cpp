...
void FWorldConditionContextDataRefDetails::CacheContextData()
{
	...
	if (BaseStructs.IsEmpty())
	{
		...
		UE_LOGF(LogWorldCondition, Error, "%ls: Could not find BaseStructs '%ls' nor BaseClasses '%ls' based on the property metadata, expecting full struct name separated by commas if multiple.",
	...
	if (Schema && !BaseStructs.IsEmpty())
	{
		...
		for (const FWorldConditionContextDataDesc& Desc : Schema->GetContextDataDescs())
		{
			...
			{
				if (Desc.MatchesType(BaseStruct))
				{
