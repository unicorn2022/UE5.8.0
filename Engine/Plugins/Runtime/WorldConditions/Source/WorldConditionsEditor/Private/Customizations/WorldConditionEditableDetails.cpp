...
void FWorldConditionEditableDetails::CustomizeHeader(TSharedRef<IPropertyHandle> StructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	HeaderRow
		.CopyAction(FUIAction(FExecuteAction::CreateSP(this, &FWorldConditionEditableDetails::OnCopyCondition)))
		.PasteAction(FUIAction(
			FExecuteAction::CreateSP(this, &FWorldConditionEditableDetails::OnPasteCondition),
			FCanExecuteAction::CreateSP(this, &FWorldConditionEditableDetails::CanPasteCondition)))
		.WholeRowContent()
...
void FWorldConditionEditableDetails::OnCopyCondition() const
{
	FString Value;
...
bool FWorldConditionEditableDetails::CanPasteCondition() const
{
	// Never allowing pasting with no schema, we are not supporting anything basically.
	if (!Schema)
	{
		return false;
	}
	...
	// Try to deserialize the clipboard data as a FWorldConditionEditable.
	FWorldConditionEditable TempEditable;
	const TCHAR* Buffer = *ClipboardText;
	const TCHAR* Result = FWorldConditionEditable::StaticStruct()->ImportText(
		Buffer, &TempEditable, nullptr, PPF_Copy, nullptr, FWorldConditionEditable::StaticStruct()->GetName());
	if (!Result)
	{
		return false;
	}

	// If the condition is valid, check if the struct type is allowed.
	if (TempEditable.Condition.IsValid())
	{
		return Schema->IsStructAllowed(TempEditable.Condition.GetScriptStruct());
	}

	return true;
}

void FWorldConditionEditableDetails::OnPasteCondition() const
{
	FString Value;
	...
	if (!Value.IsEmpty())
	{
		FScopedTransaction Transaction(LOCTEXT("PasteCondition", "Paste Condition"));
		StructProperty->SetValueFromFormattedString(Value, EPropertyValueSetFlags::InstanceObjects);
	}
}
