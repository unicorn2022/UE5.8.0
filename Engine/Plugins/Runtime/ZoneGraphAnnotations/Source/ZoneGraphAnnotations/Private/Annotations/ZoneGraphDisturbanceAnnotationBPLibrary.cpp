...
void UZoneGraphDisturbanceAnnotationBPLibrary::TriggerDanger(UObject* WorldContextObject, const AActor* Instigator, const FVector Position, const float Radius, const float Duration)
{
	...
	{
		UE_LOGF(LogZoneGraphAnnotations, Error, "%ls: WorldContextObject not set.", ANSI_TO_TCHAR(__FUNCTION__));
		return;
	}
	...
	{
		UE_LOGF(LogZoneGraphAnnotations, Error, "%ls: Expecting ZoneGraphAnnotationSubsystem to be present.", ANSI_TO_TCHAR(__FUNCTION__));
		return;
	}
