...
{
    UE_LOGF(LogLocationServicesIOS, Log, "IOS locationManager didFailWithError");
...
bool ULocationServicesIOSImpl::IsLocationServiceEnabled()
{
	...
	if (!bEnabled)
	{
		UE_LOGF(LogLocationServicesIOS, Log, "ULocationServicesIOSImpl::IsServiceEnabled() - location services disabled in settings");
	}
    bool bAuthorized = LocManager.authorizationStatus == kCLAuthorizationStatusDenied;
	if (!bAuthorized)
	{
		UE_LOGF(LogLocationServicesIOS, Log, "ULocationServicesIOSImpl::IsServiceEnabled() - location services have not been authorized for use");
	}
