...
namespace
{
	...
	void AddMeshInfos_RenderThread(const FPrimitiveSceneProxy* InSceneProxy, FMeshInfoArray& OutMeshInfos, int64 MaterialMappingIndex)
	{
		...
		{ 
			...
			{
				UE_LOGF(LogRenderTrace, Warning, "Found a mesh with %d elements, only handling the first one", (int32)OutMeshElements.Num());
			}
		}
	}
		...
		if (CompletionState == ECompletionState::Pending)
		{
			...
			if (bFlush || bIsReady)
			{
				...
				if (Task.MaterialMap.IsValidIndex(Result.MaterialMapIndex))
				{
					...
					{
						UE_LOGF(LogRenderTrace, Warning, "Completed a task but had an invalid material index (%d:%d)", Result.MaterialMapIndex, Result.PhysicalMaterialIndex);
					}
				}
				else
				{
					UE_LOGF(LogRenderTrace, Warning, "Completed a task but had an invalid material map index (%d:%d)", Result.MaterialMapIndex, Result.PhysicalMaterialIndex);
				}
...
uint32 FRenderTraceQueue::AsyncRenderTraceComponents(TArrayView<const class UPrimitiveComponent*> PrimitiveComponents, FVector RayOrigin, FVector RayDirection, FRenderTraceDelegate OnComplete, int64 UserData)
{
	...
	{
		UE_LOGF(LogRenderTrace, Warning, "AsyncSampleComponents given invalid RayDirection");
	...
	UE_LOGF(LogRenderTrace, Verbose, "AsyncSampleComponents: Queueing new task %u(%p) with %d primitives", TaskRequestID, NewTask.Get(), PrimitiveComponents.Num());
...
void FRenderTraceQueue::Tick(float DeltaTime)
{
	...
	auto CheckCompletionAndRemove = [this](const TSharedPtr<FRenderTraceTask>& Task)
	{
		if (Task->CompletionState.load() == ECompletionState::Complete)
		{
			...
			UE_LOGF(LogTemp, Log, "RenderTrace completed in %f seconds (%d frames)", TaskDurationSeconds, TaskDurationFrames);
#endif
			...
			{
				UE_LOGF(LogTemp, Warning, "RenderTrace completed with an unbound delegate");
			}
			return true;
		}
