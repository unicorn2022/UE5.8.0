...
bool FSQLitePreparedStatement::Create(FSQLiteDatabase& InDatabase, const TCHAR* InStatement, const ESQLitePreparedStatementFlags InFlags)
{
	...
	{
		UE_LOGF(LogSQLitePreparedStatement, Warning, "Failed to create prepared statement from '%ls': %ls", InStatement, *InDatabase.GetLastError());
		return false;
	}
...
ESQLitePreparedStatementStepResult FSQLitePreparedStatement::Step(int32* OutErrorCode)
{
	if (OutErrorCode)
	{
		*OutErrorCode = SQLITE_OK;
	}
