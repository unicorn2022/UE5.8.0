...
bool FSQLiteDatabase::Open(const TCHAR* InFilename, const ESQLiteDatabaseOpenMode InOpenMode)
{
	...
	if (sqlite3_open_v2(TCHAR_TO_UTF8(InFilename), &Database, OpenFlags, nullptr) != SQLITE_OK)
	{
		if (Database)
		{
			UE_LOGF(LogSQLiteDatabase, Warning, "Failed to open database '%ls': %ls", InFilename, *GetLastError());
	...
	UE_LOGF(LogSQLiteDatabase, Log, "Opened database '%ls'", InFilename);
...
FString FSQLiteDatabase::GetErrorForCode(const int32 ResultCode)
{
	if (const char* ErrorStr = sqlite3_errstr(ResultCode))
	{
