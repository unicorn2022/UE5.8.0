...
public:
	 ...
	 * @param OutErrorCode Optional value to store the low-level error code of the step (@see FSQLiteDatabase::GetErrorForCode).
	 * @note See FSQLiteDatabase::Execute for a simple example of stepping a statement.
	 */
	UE_API ESQLitePreparedStatementStepResult Step(int32* OutErrorCode = nullptr);
