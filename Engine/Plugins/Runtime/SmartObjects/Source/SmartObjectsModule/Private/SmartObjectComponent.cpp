...
FBox USmartObjectComponent::GetSmartObjectBounds() const
{
	...
	if (const AActor* Owner = GetOwner())
	{
		...
		{
			BoundingBox = Definition->GetBounds().TransformBy(GetComponentTransform());
		}
	}
...
void USmartObjectComponent::SetDefinition(USmartObjectDefinition* Definition)
{
	...
	{
		UE_LOGF(LogSmartObject, Warning,
			"Changing Definition is not supported when the component is registered to the simulation."
...
{
	return K2_SetSmartObjectEnabledForReason(UE::SmartObject::EnabledReason::Gameplay, bEnable);
}

bool USmartObjectComponent::K2_SetSmartObjectEnabled(const bool bEnable) const
{
	return K2_SetSmartObjectEnabledForReason(UE::SmartObject::EnabledReason::Gameplay, bEnable);
}
...
{
	return K2_SetSmartObjectEnabledForReason(ReasonTag, bEnabled);
}

bool USmartObjectComponent::K2_SetSmartObjectEnabledForReason(FGameplayTag ReasonTag, const bool bEnabled) const
{
...
#if WITH_EDITOR
void USmartObjectComponent::GetActorDescProperties(FPropertyPairsMap& PropertyPairsMap) const
{
	Super::GetActorDescProperties(PropertyPairsMap);

	PropertyPairsMap.AddProperty(UE::SmartObject::WithSmartObjectTag);
}
