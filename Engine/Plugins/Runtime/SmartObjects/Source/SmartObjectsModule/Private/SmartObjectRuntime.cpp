...
{
	TStringBuilder<256> DisableFlagsStringBuilder;
	...
	return FString(DisableFlagsStringBuilder);
}
...
bool FSmartObjectRuntimeSlot::Release(const FSmartObjectClaimHandle& ClaimHandle, const bool bAborted)
{
	...
	{
		UE_LOGF(LogSmartObject, Error, "Expected slot state is 'Claimed' or 'Occupied' but current state is '%ls'. Slot will not be released",
			*UEnum::GetValueAsString(State));
	}
	else if (ClaimHandle.UserHandle != User)
	{
		UE_LOGF(LogSmartObject, Error, "User '%ls' is trying to release slot claimed or used by other user '%ls'. Slot will not be released",
	...
	else
	{
		if (bAborted)
		{
			...
			UE_LOGF(LogSmartObject, Verbose, "Slot invalidated callback was%lscalled for %ls", bFunctionWasExecuted ? TEXT(" ") : TEXT(" not "), *LexToString(ClaimHandle));
		}
