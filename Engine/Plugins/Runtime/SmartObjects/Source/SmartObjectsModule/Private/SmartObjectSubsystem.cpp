...
namespace UE::SmartObject
{
	...
	/** Cache for property binding layouts for BindPropertiesFromStructInternal. */
	struct FPropertyBindingCache
	{
		struct FPropertyMapping
		{
			const FProperty* Property = nullptr;
			FWorldConditionContextDataRef ContextRef;
			bool bIsStructProperty = false;
			const UScriptStruct* StructType = nullptr;
		};

		TMap<TPair<const UScriptStruct*, const UWorldConditionSchema*>, TArray<FPropertyMapping>> Cache;
	};
} // UE::SmartObject
...
void USmartObjectSubsystem::BindPropertiesFromStructInternal(FWorldConditionContextData& ContextData, const FConstStructView& UserData, UE::SmartObject::FPropertyBindingCache* PropertyCache) const
{
	const UWorldConditionSchema* Schema = ContextData.GetSchema();
	check(Schema);
	const UScriptStruct* ScriptStruct = UserData.GetScriptStruct();
	const uint8* Memory = UserData.GetMemory();

	// Check cache first
	if (PropertyCache)
	{
		if (TArray<UE::SmartObject::FPropertyBindingCache::FPropertyMapping>* CachedMappings = PropertyCache->Cache.Find(MakeTuple(ScriptStruct, Schema)))
		{
			// Use cached mappings - skip reflection
			for (const UE::SmartObject::FPropertyBindingCache::FPropertyMapping& Mapping : *CachedMappings)
			{
				if (Mapping.bIsStructProperty)
				{
					const FConstStructView StructView(Mapping.StructType, Memory + Mapping.Property->GetOffset_ForInternal());
					ContextData.SetContextData(Mapping.ContextRef, StructView);
				}
				else
				{
					const FObjectPropertyBase* ObjectProperty = CastField<FObjectPropertyBase>(Mapping.Property);
					const UObject* Object = ObjectProperty->GetObjectPropertyValue(Memory + Mapping.Property->GetOffset_ForInternal());
					ContextData.SetContextData(Mapping.ContextRef, Object);
				}
			}
	...
	TArray<UE::SmartObject::FPropertyBindingCache::FPropertyMapping> NewMappings;
	for (TFieldIterator<FProperty> It(UserData.GetScriptStruct()); It; ++It)
	{
		...
		if (const FStructProperty* StructProperty = CastField<FStructProperty>(Property))
		{
			...
			if (Ref.IsValid())
			{
				const FConstStructView StructView(StructProperty->Struct, Memory + Property->GetOffset_ForInternal());
				ContextData.SetContextData(Ref, StructView);

				if (PropertyCache)
				{
					NewMappings.Add(
						{ .Property = Property, .ContextRef = Ref, .bIsStructProperty = true, .StructType = StructProperty->Struct });
				}
			}
		}
		else if (const FObjectPropertyBase* ObjectProperty = CastField<FObjectPropertyBase>(Property))
		{
			...
			if (Ref.IsValid())
			{
				const UObject* Object = ObjectProperty->GetObjectPropertyValue(Memory + Property->GetOffset_ForInternal());
				ContextData.SetContextData(Ref, Object);

				if (PropertyCache)
				{
					NewMappings.Add(
						{ .Property = Property, .ContextRef = Ref, .bIsStructProperty = false, .StructType = nullptr });
				}
			}
		}
	}
	...
	if (PropertyCache)
	{
		PropertyCache->Cache.Add(MakeTuple(ScriptStruct, Schema), MoveTemp(NewMappings));
	}
}
...
bool USmartObjectSubsystem::ReadInstanceData(const FSmartObjectHandle& Handle, TFunctionRef<void(FConstSmartObjectView)> Function) const
{
	bool bExecuted = false;
	ExecuteOnValidatedRuntime(Handle,
		[Function, &Handle, &bExecuted](const FSmartObjectRuntime& SmartObjectRuntime)
		{
			FConstSmartObjectView ConstInstanceView(Handle, SmartObjectRuntime);
			if (ConstInstanceView.IsValid())
			{
				Function(ConstInstanceView);
				bExecuted = true;
			}
		}, __FUNCTION__);
...
void USmartObjectSubsystem::FindSlotsInternal(const FSmartObjectHandle Handle, const FSmartObjectRuntime& SmartObjectRuntime, const FSmartObjectRequestFilter& Filter, TArray<FSmartObjectSlotHandle>& OutResults, const FConstStructView UserData, UE::SmartObject::FDefinitionFilterCache* DefinitionFilterCache, UE::SmartObject::FPropertyBindingCache* PropertyCache) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("SmartObject_FilterSlots");

#if WITH_SMARTOBJECT_DEBUG
	auto RecordObjectRejection = [&Filter, Handle](ESmartObjectFilterRejection Reason)
	{
		if (Filter.DebugRejections)
		{
			FSmartObjectDebugObjectRejection& Rejection = Filter.DebugRejections->ObjectRejections.AddDefaulted_GetRef();
			Rejection.SmartObjectHandle = Handle;
			Rejection.Reason = Reason;
		}
	};

	auto RecordSlotRejection = [&Filter, Handle](int32 SlotIndex, ESmartObjectSlotFilterRejection Reason)
	{
		if (Filter.DebugRejections)
		{
			FSmartObjectDebugSlotRejection& Rejection = Filter.DebugRejections->SlotRejections.AddDefaulted_GetRef();
			Rejection.SmartObjectHandle = Handle;
			Rejection.SlotIndex = SlotIndex;
			Rejection.Reason = Reason;
		}
	};
#endif
	...
	{
#if WITH_SMARTOBJECT_DEBUG
		RecordObjectRejection(ESmartObjectFilterRejection::SmartObjectDisabled);
#endif
		return;
	}
	...
	{
#if WITH_SMARTOBJECT_DEBUG
		RecordObjectRejection(ESmartObjectFilterRejection::PredicateRejected);
#endif
		return;
	}

	// Apply definition level filtering (Tags and BehaviorDefinition)
	UE::SmartObject::FSlotIndicesArray ValidSlotIndices;
	if (DefinitionFilterCache)
	{
		if (UE::SmartObject::FSlotIndicesArray* CachedIndices = DefinitionFilterCache->Find(&Definition))
		{
			ValidSlotIndices = *CachedIndices;
		}
		else
		{
			FindMatchingSlotDefinitionIndicesInternal(Definition, Filter, ValidSlotIndices);
			DefinitionFilterCache->Add(&Definition, { ValidSlotIndices });
		}
	}
	else
	{
	...
	BindPropertiesFromStructInternal(ConditionContextData, UserData, PropertyCache);
	...
	{
#if WITH_SMARTOBJECT_DEBUG
		RecordObjectRejection(ESmartObjectFilterRejection::ObjectPreconditionsFailed);
#endif
		return;
	}
	...
	for (const int32 SlotIndex : ValidSlotIndices)
	{
		...
		{
#if WITH_SMARTOBJECT_DEBUG
			RecordSlotRejection(SlotIndex, ESmartObjectSlotFilterRejection::SlotDisabled);
#endif
			continue;
		}
				...
				&& RuntimeSlot.ClaimedPriority < Filter.ClaimPriority))
		{
			...
			{
#if WITH_SMARTOBJECT_DEBUG
				RecordSlotRejection(SlotIndex, ESmartObjectSlotFilterRejection::SlotPreconditionsFailed);
#endif
				continue;
			}
		...
		{
			RecordSlotRejection(SlotIndex, ESmartObjectSlotFilterRejection::SlotClaimedByHigherPriority);
		}
#endif
	}
}

void USmartObjectSubsystem::FindMatchingSlotDefinitionIndicesInternal(const USmartObjectDefinition& Definition, const FSmartObjectRequestFilter& Filter, UE::SmartObject::FSlotIndicesArray& OutValidIndices)
{
	const ESmartObjectTagFilteringPolicy UserTagsFilteringPolicy = Definition.GetUserTagsFilteringPolicy();

#if WITH_SMARTOBJECT_DEBUG
	auto RecordDefinitionRejection = [&Filter, &Definition](int32 SlotIndex, ESmartObjectDefinitionFilterRejection Reason)
	{
		if (Filter.DebugRejections)
		{
			FSmartObjectDebugDefinitionRejection& Rejection = Filter.DebugRejections->DefinitionRejections.AddDefaulted_GetRef();
			Rejection.Definition = &Definition;
			Rejection.SlotIndex = SlotIndex;
			Rejection.Reason = Reason;
		}
	};
		...
		&& !MatchesTagQueryFunc(Definition.GetUserTagFilter(), Filter.UserTags))
	{
		...
		if (Filter.DebugRejections)
		{
			...
			for (int32 SlotIndex = 0; SlotIndex < SlotDefinitions.Num(); ++SlotIndex)
			{
				RecordDefinitionRejection(SlotIndex, ESmartObjectDefinitionFilterRejection::CombinedUserTagMismatch);
			}
		}
#endif
		return;
	...
	for (int32 SlotIndex = 0; SlotIndex < SlotDefinitions.Num(); ++SlotIndex)
	{
		const FSmartObjectSlotDefinition& Slot = SlotDefinitions[SlotIndex];
		...
		if (!Filter.BehaviorDefinitionClasses.IsEmpty())
		{
			...
			{
				if (Definition.GetBehaviorDefinition(SlotIndex, BehaviorDefinitionClass) != nullptr)
				{
					bMatchesAny = true;
					break;
				}
			...
			{
#if WITH_SMARTOBJECT_DEBUG
				RecordDefinitionRejection(SlotIndex, ESmartObjectDefinitionFilterRejection::BehaviorDefinitionMismatch);
#endif
				continue;
			}
		}
		...
		{
#if WITH_SMARTOBJECT_DEBUG
			RecordDefinitionRejection(SlotIndex, ESmartObjectDefinitionFilterRejection::ActivityTagMismatch);
#endif
			continue;
		}
		...
		{
#if WITH_SMARTOBJECT_DEBUG
			RecordDefinitionRejection(SlotIndex, ESmartObjectDefinitionFilterRejection::UserTagMismatch);
#endif
			continue;
		}
...
{
	UE::SmartObject::FSlotIndicesArray InlineValidIndices;
	FindMatchingSlotDefinitionIndicesInternal(Definition, Filter, InlineValidIndices);

	OutValidIndices = InlineValidIndices;
}
...
bool USmartObjectSubsystem::FindSmartObjects(const FSmartObjectRequest& Request, TArray<FSmartObjectRequestResult>& OutResults, const FConstStructView UserData) const
{
	...
	UE::SmartObject::FDefinitionFilterCache DefinitionFilterCache;
	UE::SmartObject::FPropertyBindingCache PropertyBindingCache;
	for (const FSmartObjectHandle SmartObjectHandle : QueryResults)
	{
		...
		if (!Request.QueryBox.IsInside(SmartObjectRuntime->GetTransform().GetLocation()))
		{
			...
			{
				FSmartObjectDebugObjectRejection& Rejection = Filter.DebugRejections->ObjectRejections.AddDefaulted_GetRef();
				Rejection.SmartObjectHandle = SmartObjectHandle;
				Rejection.Reason = ESmartObjectFilterRejection::OutsideSpatialBounds;
			}
#endif
			continue;
		}

		FindSlotsInternal(SmartObjectHandle, *SmartObjectRuntime, Filter, SlotHandles, UserData, &DefinitionFilterCache, &PropertyBindingCache);
