...
void FSmartObjectBindingCollection::VisitBindings(FPropertyBindingIndex16 InBegin, FPropertyBindingIndex16 InEnd,
	TFunctionRef<EVisitResult(const FPropertyBindingBinding& Binding, const int32 BindingIndex)> InFunction) const
{
	checkf(InBegin.IsValid() && InEnd.IsValid(), TEXT("%hs expects valid indices."), __FUNCTION__);
	
	for (int32 BindingIdx = InBegin.Get(); BindingIdx < InEnd.Get(); ++BindingIdx)
	{
		if (InFunction(PropertyBindings[BindingIdx], BindingIdx) == EVisitResult::Break)
		{
			break;
		}
	}
	...
	, const FPropertyBindingPath& InTargetPath)
{
	...
	{
		UE_LOGF(LogPropertyBindingUtils, Verbose, "%s %d bindings", __FUNCTION__, PropertyBindings.Num()+1);
	}
