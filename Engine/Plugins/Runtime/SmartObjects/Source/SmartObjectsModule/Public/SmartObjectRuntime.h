...
USTRUCT()
struct UE_DEPRECATED(all, "Transform is moved to FSmartObjectRuntimeSlot.") SMARTOBJECTSMODULE_API FSmartObjectSlotTransform : public FSmartObjectSlotStateData
{
	GENERATED_BODY()
...
public:
	...
	/**
	 * Returns a reference to the definition of the instance's parent object.
	 * Method will fail a check if called on an invalid view.
	 * @note The definition fragment is always created and assigned when creating an entity associated to an instance
	 * so a valid view is guaranteed to be able to provide it.
	 */
	const USmartObjectDefinition& GetSmartObjectDefinition() const
	{
		checkf(IsValid(), TEXT("Definition can only be accessed through a valid SmartObjectView"));
	...
	bool IsEnabled() const
	{
		checkf(IsValid(), TEXT("Enabled can only be accessed through a valid SmartObjectView"));
		return Runtime->IsEnabled();
	}

	/** 
	 * @return runtime gameplay tags of the instance.
	 * Method will fail a check if called on an invalid view
	 */
	const FGameplayTagContainer& GetTags() const
	{
	...
	{
		checkf(IsValid(), TEXT("WorldTransform can only be accessed through a valid SmartObjectView"));
		return Runtime->GetTransform();
	}
...
public:
	...
	{
		checkf(IsValid(), TEXT("State can only be accessed through a valid SlotView"));
	...
	bool CanBeClaimed(ESmartObjectClaimPriority ClaimPriority) const
	{
		checkf(IsValid(), TEXT("Claim can only be accessed through a valid SlotView"));
	...
	bool IsEnabled() const
	{
		checkf(IsValid(), TEXT("Enabled can only be accessed through a valid SlotView"));
	...
	const FGameplayTagContainer& GetTags() const
	{
		checkf(IsValid(), TEXT("Tags can only be accessed through a valid SlotView"));
		return Slot->GetTags();
	}
	...
	/** 
	 * @return the world transform. Must be only called on valid slots. 
	 * Method will fail a check if called on an invalid SlotView
	 */
	FTransform GetWorldTransform() const
	{
