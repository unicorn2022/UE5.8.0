...
{
	using FSlotIndicesArray = TArray<int32, TInlineAllocator<16>>;
	using FDefinitionFilterCache = TMap<const USmartObjectDefinition*, FSlotIndicesArray>;
 ...
 * and UserCapsuleParams are queried via the INavAgentInterface and USmartObjectUserComponent on the actor if they are _not_ set.
 * 
 * If the user actor is not available (e.g. with Mass), then ValidationFilter and UserCapsuleParams must be defined, and if bProjectNavigationLocation is set NavigationData must be valid. 
 * 
...
struct FSmartObjectSlotEntranceLocationResult
{
	...
	// Macro needed to avoid deprecation errors with "Tag_DEPRECATED" being copied or created in the default methods
	...
	UE_DEPRECATED(all, "Use Tags instead.")
	UPROPERTY()
...
public:
	...
	/**
	 * Executes the provided function if a valid const view can be created for the provided handle.
	 * In a multithreaded scenario, the method will prevent the instance from being modified
	 * during the execution of the function.
	 * @param Handle Handle to a smart object.
	 * @param Function Function to execute if a valid view can be created
	 * @return Whether the instance was found, was valid, and the function was called
	 */
	SMARTOBJECTSMODULE_API bool ReadInstanceData(const FSmartObjectHandle& Handle, TFunctionRef<void(FConstSmartObjectView)> Function) const;
...
protected:
	 ...
	 * @param DefinitionFilterCache (optional) cache for definition filter results, keyed by Definition pointer. Avoids redundant FindMatchingSlotDefinitionIndicesInternal calls when multiple objects share the same definition.
	 * @param PropertyCache (optional) cache for property binding layouts, keyed by (UserData type, Schema). Avoids TFieldIterator reflection overhead when multiple objects use the same UserData struct.
	*/
	SMARTOBJECTSMODULE_API void FindSlotsInternal(
		...
		const FConstStructView UserData, 
		UE::SmartObject::FDefinitionFilterCache* DefinitionFilterCache = nullptr,
		UE::SmartObject::FPropertyBindingCache* PropertyCache = nullptr) const;

	/**
	 * Applies filter on provided definition and fills OutValidIndices with indices of all valid slots.
	 * Per-slot rejection reasons are recorded to Filter.DebugRejections->DefinitionRejections if set (WITH_SMARTOBJECT_DEBUG only).
	 */
	static SMARTOBJECTSMODULE_API void FindMatchingSlotDefinitionIndicesInternal(const USmartObjectDefinition& Definition, const FSmartObjectRequestFilter& Filter, UE::SmartObject::FSlotIndicesArray& OutValidIndices);

	UE_DEPRECATED(5.8, "Use the version with UE::SmartObject::FSlotIndicesArray.")
	...
	SMARTOBJECTSMODULE_API void ExecuteOnSlotFilteredBySelectionConditions(
		...
		const FConstStructView UserData,
		TFunctionRef<void(const int32 /*SlotIndex*/)> Function) const;
	...
	*/
	 ...
	 * @param UserData Additional data that could be provided to bind values in the conditions evaluation context
	 */
	SMARTOBJECTSMODULE_API void BindPropertiesFromStructInternal(FWorldConditionContextData& ContextData, const FConstStructView& UserData, UE::SmartObject::FPropertyBindingCache* PropertyCache = nullptr) const;
	...
	[[nodiscard]] SMARTOBJECTSMODULE_API bool EvaluateSlotConditionsInternal(
		...
		const FSmartObjectRuntime& SmartObjectRuntime,
		const FSmartObjectSlotHandle& SlotHandle) const;
	...
	[[nodiscard]] SMARTOBJECTSMODULE_API bool EvaluateConditionsForFilteringInternal(
		...
		const FConstStructView UserData,
		TPair<const FSmartObjectRuntime*, bool>& LastEvaluatedRuntime) const;
	...
	SMARTOBJECTSMODULE_API bool FindEntranceLocationInternal(
		...
		const FSmartObjectSlotEntranceLocationRequest& Request,
		FSmartObjectSlotEntranceLocationResult& OutResult) const;
