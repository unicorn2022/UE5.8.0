...
public:
	...
	/**
	 * Returns the number of entrance annotations on a slot.
	 * Use this to iterate entrances with GetSlotEntranceOffsetAndRotation.
	 * @param Definition The smart object definition to query
	 * @param SlotIndex Index of the slot to query
	 * @return The number of entrance annotations on the slot, or 0 if the definition or slot is invalid
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "SmartObject", meta = (DisplayName = "Get Num Slot Entrances", ReturnDisplayName = "Count", Keywords = "entrance entry count number"))
	static UE_API int32 GetNumSlotEntrances(const USmartObjectDefinition* Definition, int32 SlotIndex);

	/**
	 * Returns the entrance offset and rotation for a specific entrance annotation on a slot.
	 * @param Definition The smart object definition to query
	 * @param SlotIndex Index of the slot to query
	 * @param EntranceIndex Index of the entrance annotation within the slot (0 for first entrance)
	 * @param OutOffset The local space offset of the entrance
	 * @param OutRotation The local space rotation of the entrance
	 * @return True if the entrance annotation was found at the given indices
	 */
	UFUNCTION(BlueprintCallable, Category = "SmartObject", meta = (DisplayName = "Get Slot Entrance Offset and Rotation", ReturnDisplayName = "bFound", Keywords = "entrance entry offset rotation approach navigation"))
	static UE_API bool GetSlotEntranceOffsetAndRotation(const USmartObjectDefinition* Definition, int32 SlotIndex, int32 EntranceIndex, FVector& OutOffset, FRotator& OutRotation);

	/**
	 * Returns the entrance transform (offset and rotation combined) for a specific entrance annotation on a slot.
	 * This is equivalent to GetSlotEntranceOffsetAndRotation but returns the result as a single FTransform.
	 * @param Definition The smart object definition to query
	 * @param SlotIndex Index of the slot to query
	 * @param EntranceIndex Index of the entrance annotation within the slot (0 for first entrance)
	 * @param OutTransform The local space transform of the entrance (translation from Offset, rotation from Rotation, unit scale)
	 * @return True if the entrance annotation was found at the given indices
	 */
	UFUNCTION(BlueprintCallable, Category = "SmartObject", meta = (DisplayName = "Get Slot Entrance Transform", ReturnDisplayName = "bFound", Keywords = "entrance entry transform offset rotation approach navigation"))
	static UE_API bool GetSlotEntranceTransform(const USmartObjectDefinition* Definition, int32 SlotIndex, int32 EntranceIndex, FTransform& OutTransform);

	/**
	 * Returns all definition data entries of a specific struct type from a slot's DefinitionData.
	 * Set the output pin to the desired struct type (must inherit from FSmartObjectDefinitionData) to filter results.
	 * The returned array contains copies of all matching entries, safe for Blueprint read access.
	 * @note If the output type is a base class and the actual data is a derived type, only the base class fields
	 *       will be copied (struct slicing). Use the most specific type you need for complete data.
	 * @param Definition The smart object definition to query
	 * @param SlotIndex Index of the slot to query
	 * @param OutDefinitionData Output array of matching definition data structs (type determined by pin connection)
	 * @return True if at least one matching entry was found
	 */
	UFUNCTION(BlueprintCallable, CustomThunk, Category = "SmartObject", meta = (ArrayParm = "OutDefinitionData", DisplayName = "Get Slot Definition Data By Type", ReturnDisplayName = "bFound", Keywords = "definition data filter annotation slot struct type"))
	static UE_API bool GetSlotDefinitionDataByType(const USmartObjectDefinition* Definition, int32 SlotIndex, TArray<int32>& OutDefinitionData);
