...
bool UWorldPartitionSmartObjectCollectionBuilder::PreRun(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	{
		UE_LOGF(LogSmartObject, Error, "SmartObjectSubsystem not found.");
		return false;
	}
	...
	if (!WorldPartition)
	{
		UE_LOGF(LogSmartObject, Error, "Failed to retrieve WorldPartition.");
		return false;
	}
	...
	{
		if (ActorDescInstance->HasProperty(UE::SmartObject::WithSmartObjectTag)
...
bool UWorldPartitionSmartObjectCollectionBuilder::PostRun(UWorld* World, FPackageSourceControlHelper& PackageHelper, const bool bInRunSuccess)
{
	...
	for (int32 CollectionIndex = 0; CollectionIndex < CollectionsCount; ++CollectionIndex)
	{
		...
		if (bDeletePackage)
		{
			UE_LOGF(LogSmartObject, Log, "Deleting package %ls.", *Package->GetName());
			if (!PackageHelper.Delete(Package))
			{
				UE_LOGF(LogSmartObject, Error, "Error deleting package.");
		...
		{
			{
				...
				else
				{
					...
					if (IPlatformFile::GetPlatformPhysical().FileExists(*PackageFilename))
					{
						...
						{
							UE_LOGF(LogSmartObject, Error, "Error setting %ls writable", *PackageFilename);
							bErrorsEncountered = true;
							continue;
						}
					}
...

			{
				...
				UE_LOGF(LogSmartObject, Log, "   Saving package  %ls.", *Package->GetName());
				const FString PackageFileName = SourceControlHelpers::PackageFilename(Package);
				...
				{
					UE_LOGF(LogSmartObject, Error, "Error saving package %ls.", *Package->GetName());
					bErrorsEncountered = true;
					continue;
				}
			}

			{
				...
				UE_LOGF(LogSmartObject, Log, "Adding package to source control.");

				if (!PackageHelper.AddToSourceControl(Package))
				{
					UE_LOGF(LogSmartObject, Error, "Error adding package %ls to source control.", *Package->GetName());
					bErrorsEncountered = true;
					continue;
				}
			}
