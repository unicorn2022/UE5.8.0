...
bool UAudioCaptureComponent::Init(int32& SampleRate)
{
	...
	if (CaptureSynth.GetDefaultCaptureDeviceInfo(DeviceInfo))
	{
		...
		{
			UE_LOGF(LogAudio, Warning, "Attempted to open a capture device with the invalid SampleRate value of %i", DeviceInfo.PreferredSampleRate);
		}
		...
		{
			UE_LOGF(LogAudio, Warning, "Attempted to open a device with the invalid NumChannels value of %i", NumChannels);
		}
	...
	if (CapturedAudioDataSamples > 0 || CaptureSynth.GetNumSamplesEnqueued() > 1024)
	{
		...
		if (ReadSampleIndex + NumSamples > CaptureAudioData.Num())
		{
			...
			if (SamplesLeft > 0)
			{
				...
				{
					UE_LOGF(LogAudio, Warning, "Attempt to write past end of buffer in OnGenerateAudio, when we got more data from the synth");
					return NumSamples;
				}
		...
		if (CaptureAudioData.Num() > 0)
		{
			...
			{
				UE_LOGF(LogAudio, Warning, "Attempt to read past end of buffer in OnGenerateAudio, when we did not get more data from the synth");
				return NumSamples;
			}
