...
void FAudioRecordingManager::StartRecording(const FRecordingSettings& InSettings)
{
	...
	if (ensure(CurrentState == EAudioRecorderState::PreRecord))
	{
		...
		UE_LOGF(LogMicManager, Log, "Starting mic recording.");
		...
		{
			UE_LOGF(LogMicManager, Error, "Unable to get audio capture device.");
			return;
		}
		...
		{
			UE_LOGF(LogMicManager, Warning, "No audio channels to record.");
			return;
		}
		...
		UE_LOGF(LogMicManager, Log, "Initialized mic recording manager at %.4f hz sample rate, %d channels, and %d Recording Block Size", RecordingSampleRate, StreamParams.NumInputChannels, BufferFrames);
...
void FAudioRecordingManager::StopRecording()
{
	...
	if (ensure(CurrentState == EAudioRecorderState::Recording))
	{
		...
		if (CurrentRecordedPCMData.Num() > 0)
		{
			...
			UE_LOGF(LogMicManager, Log, "Stopping mic recording. Recorded %d frames of audio (%.4f seconds). Detected %d buffer overflows.",
				NumRecordedSamples,
...
void FAudioRecordingManager::ProcessRecordedData(const FRecordingManagerSourceSettings& InSourceSettings)
{
	...
	{
		UE_LOGF(LogMicManager, Log, "Converting sample rate from %d hz to 44100 hz.", (int32)RecordingSampleRate);
...
void FAudioRecordingManager::ApplyGain(const FRecordingManagerSourceSettings& InSourceSettings, const int32 InNumFrames)
{
	...
	if (InputGain != 1.0f)
	{
		UE_LOGF(LogMicManager, Log, "Scaling gain of recording by %.2f linear gain.", InputGain);
