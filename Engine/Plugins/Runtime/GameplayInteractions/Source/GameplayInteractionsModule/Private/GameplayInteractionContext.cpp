...
bool FGameplayInteractionContext::Activate(const UGameplayInteractionSmartObjectBehaviorDefinition& InDefinition)
{
	...
	if (Definition)
	{
		if (const UStateTree* PrevTree = Definition->GetStateTree())
		{
			...
			{
				UE_LOGF(LogGameplayInteractions, Error, "Trying to activate interaction with different root State Tree while the previous tree is still running!");
				return false;
			}
		}
	}
	...
	if (!IsValid())
	{
		UE_LOGF(LogGameplayInteractions, Error, "Failed to activate interaction. Context is not properly setup.");
		return false;
	}
	...
	{
		UE_LOGF(LogGameplayInteractions, Error, "Reentrant call to %s is not allowed!", __FUNCTION__);
		return false;
	}
