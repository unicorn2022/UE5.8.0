...
bool UStateTreeTask_PlayContextualAnim_InstanceData::StartContextualAnim(const FStateTreeExecutionContext& Context) const
{
	...
	FContextualAnimSceneBindings Bindings = FContextualAnimSceneBindings(*SceneAsset);

	// Add primary actor to the contextual anim scene bindings
	if (Bindings.BindActorToRole(*PrimaryActor, PrimaryRole, SectionIdx, AnimSetIdx) == false)
	{
	...
	if (Bindings.BindActorToRole(*SecondaryActor, SecondaryRole, SectionIdx, AnimSetIdx) == false)
	{
	...
	{
		if (Bindings.BindActorToRole(*TertiaryActor, TertiaryRole, SectionIdx, AnimSetIdx) == false)
		{
...
void UStateTreeTask_PlayContextualAnim_InstanceData::OnMontageEnded(UAnimMontage* const EndedMontage, const bool bInterrupted)
{
	...
	UE_LOGF(LogStateTree, Verbose, "%s EndedMontage: %ls bInterrupted: %d", __FUNCTION__, *GetNameSafe(EndedMontage), bInterrupted);
...
{
	UE_LOGF(LogStateTree, Verbose, "%s NotifyName: %ls Anim: %ls",	__FUNCTION__, *NotifyName.ToString(), *GetNameSafe(BranchingPointNotifyPayload.SequenceAsset));
