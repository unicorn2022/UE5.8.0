...
bool UMixedRealityCaptureComponent::SaveConfiguration(const FString& SlotName, int32 UserIndex) const
{
	...
	if (bSuccess)
	{
		UE_LOGF(LogMixedRealityCapture, Log, "UMixedRealityCaptureComponent::SaveConfiguration to slot %ls user %i Succeeded.", *LocalSlotName, LocalUserIndex);
	} 
	else
	{
		UE_LOGF(LogMixedRealityCapture, Warning, "UMixedRealityCaptureComponent::SaveConfiguration to slot %ls user %i Failed!", *LocalSlotName, LocalUserIndex);
	}
	return bSuccess;
}
...
bool UMixedRealityCaptureComponent::LoadConfiguration(const FString& SlotName, int32 UserIndex)
{
	...
	{
		UE_LOGF(LogMixedRealityCapture, Warning, "UMixedRealityCaptureComponent::LoadConfiguration from slot %ls user %i Failed!", *LocalSlotName, LocalUserIndex);
		return false;
	}

	ApplyCalibrationData(SaveGameInstance);

	UE_LOGF(LogMixedRealityCapture, Log, "UMixedRealityCaptureComponent::LoadConfiguration from slot %ls user %i Succeeded.", *LocalSlotName, LocalUserIndex);
	return true;
}
