...
bool FMinimalHoleFiller::Fill(int32 GroupID)
{
	...
	Simplefill.bAutoFillPrimaryColors = false;
	...
	auto FillColors = [this]()
	{
		if (bAutoFillPrimaryColors)
		{
			HoleFillUtil::FillColorOverlay(*Mesh, FillLoop.Edges, NewTriangles);
		}
	};

	if (FillLoop.Vertices.Num() <= 3) 
	{
		NewTriangles = MoveTemp(Simplefill.NewTriangles);
		FillColors();
		return true;
	}
