...
namespace UE::MeshClusterSimplifyLocals
{
	...
	static TArray<float> InitTargetLengths(const MeshType& InMesh, float& OutMinTargetLength, const FSimplifyOptions& SimplifyOptions)
	{
		TArray<float> TargetLengths;
		OutMinTargetLength = FMathf::MaxReal;
		if (SimplifyOptions.OptionalTargetEdgeLengthScale)
		{
			TargetLengths.SetNumUninitialized(InMesh.MaxVertexID());
			TArray<float> MinLengthContexts;
			ParallelForWithTaskContext(MinLengthContexts, InMesh.MaxVertexID(),
				[](int32, int32) -> float {return FMathf::MaxReal;},
				[&InMesh, &TargetLengths, &SimplifyOptions](float& MinLength, int32 VID)
				{
					if (!InMesh.IsVertex(VID))
					{
						return;
					}
					float TargetLength = float(SimplifyOptions.TargetEdgeLength * SimplifyOptions.OptionalTargetEdgeLengthScale(VID));
					MinLength = FMath::Min(MinLength, TargetLength);
					TargetLengths[VID] = TargetLength;
				}
			);
			OutMinTargetLength = *Algo::MinElement(MinLengthContexts);
		}
	...
	void ClusterVerticesByRegionGrowth(const MeshType& InMesh, const MeshConnType& InMeshConnectivity, TArray<int32>& Source, TArray<float>& SourceDist, const FSimplifyOptions& SimplifyOptions,
		const TArray<EElemTag>& EdgeTags, const TArray<EElemTag>& VertexTags, const TArray<float>& TargetLengths, float MinTargetLength,
		const FVertexBuckets& VertexIDBuckets)
	{
		...
		// process buckets in two phases:
		// first, starting from fixed/constrained verts, and disallowing free->(fixed/constrained) walks
		// second, starting-from and walking-to remaining unclaimed free vertices
		uint8 BucketIdx = 0;
		for (uint8 TagIdx = 1; TagIdx < (uint8)EElemTag::MAX; ++TagIdx)
		{
			for (; BucketIdx <= TagIdx; ++BucketIdx)
			{
				...
				for (int32 InBucketIdx = 0; InBucketIdx < CurBucket.Num(); ++InBucketIdx)
				{
					...
					// the vertex is fixed or unclaimed, claim it as a new source/kept vertex
					if (BucketIdx == (uint8)EElemTag::Fixed || CurSourceVID == INDEX_NONE)
					{
						CurSourceVID = GrowFromVID;
						CurSourceDist = 0.f;
					}
					// the vertex was claimed by another source, no need to process it further
					else
					{
						continue;
					}

					float CurrentMaxDist = TargetLengths.IsEmpty() ? (float)SimplifyOptions.TargetEdgeLength : TargetLengths[CurSourceVID];

					// if we're on the unconstrained (free) vertex bucket, don't need to check vertex/edge tags anymore
					bool bIsFreePass = TagIdx == (uint8)EElemTag::Free;

					// helper to add candidate verts to a heap
					HeapV.Reset();
					auto AddCandidates = [
						&HeapV, &InMesh, &InMeshConnectivity, &SourceDist, &Source, &EdgeTags, &VertexTags, TagIdx, bIsFreePass]
						(const FWalk& From, float UseMaxDist)
						{
							uint8 FromTagIdx = bIsFreePass ? (uint8)EElemTag::Free : (uint8)VertexTags[From.VID];
							// expand to one-ring
							InMeshConnectivity.EnumerateVertexEdges(From.VID,
								[&From, UseMaxDist,
								&HeapV, &InMesh, &InMeshConnectivity, &SourceDist, &Source, &EdgeTags, &VertexTags, TagIdx, FromTagIdx, bIsFreePass]
								(int32 EID)
								{
									FIndex2i EdgeV = InMeshConnectivity.GetEdgeV(EID);
									int32 ToVID = EdgeV.A == From.VID ? EdgeV.B : EdgeV.A;
									
									uint8 UseFromTagIdx = FromTagIdx;
									// in free pass we're only visiting free verts which only have free edges by construction
									// but in constrained pass, we need to also test the tag on the edge
									if (!bIsFreePass)
									{
										uint8 EdgeTagIdx = (uint8)EdgeTags[EID];
										// can't go from (fixed/constrained)->constrained through an unconstrained edge
										UseFromTagIdx = FMath::Max(EdgeTagIdx, FromTagIdx);
									}
									if (IsMoreConstrained(VertexTags[ToVID], UseFromTagIdx) || From.Dist >= SourceDist[ToVID])
									{
									...
									if (NewDist < UseMaxDist && NewDist < SourceDist[ToVID])
									{
					...
					AddCandidates(Start, CurrentMaxDist);

					// In uniform target length case, or if we're already at the min target length, can skip the MaxDist updates
					bool bNeedToUpdateMaxDist = !TargetLengths.IsEmpty() && CurrentMaxDist > MinTargetLength;
					...
					while (!HeapV.IsEmpty())
					{
						...
						// If we have smaller TargetLengths elsewhere, test if we are walking over
						// a smaller length and clamp max dist + stop walking if needed
						if (bNeedToUpdateMaxDist)
						{
							CurrentMaxDist = FMath::Min(CurrentMaxDist, TargetLengths[CurWalk.VID]);
							
							if (CurWalk.Dist >= CurrentMaxDist)
							{
								break;
							}
						}
						...
						AddCandidates(CurWalk, CurrentMaxDist);
					}
				}
			}
		}
	...
	static void PreserveCollapsedClusters(const MeshType& InMesh, const MeshConnType& InMeshConnectivity,
		...
		const TArray<EElemTag>& EdgeTags, const TArray<EElemTag>& VertexTags, TArray<float>& TargetLengths, float MinTargetLength,
		const FVertexBuckets& VertexIDBuckets)
	{
		for (int32 PassIdx = 0; PassIdx < SimplifyOptions.MaxPreserveCollapsedClusterPasses; ++PassIdx)
		{
			...
			// Re-run tagging
			ClusterVerticesByRegionGrowth(InMesh, InMeshConnectivity, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength, ReProcessBuckets);
	...
	void BuildSimplifiedMeshFromClusters(ResultMeshType& ResultMesh, TArray<int32>& FromResVID, TArray<int32>& ResultToSourceTri,
		...
		const TArray<EElemTag>& EdgeTags, TArray<EElemTag>& VertexTags, const TArray<float>& TargetLengths, float MinTargetLength,
		FVertexBuckets& ProcessBuckets)
	{
		...
		while (MeshBuildAttempts-- > 0)
		{
			...
			if (!bAllowDegenerate && SourceVIDToFix.Num() > 0)
			{
				...
				ClusterVerticesByRegionGrowth(InMesh, InMeshConnectivity, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength, ProcessBuckets);
				FindClustersInOutput(InMesh, Source, ClusterInOutput);
				continue;
			}
...
bool Simplify(const FDynamicMesh3& InMesh, FDynamicMesh3& ResultMesh, const FSimplifyOptions& SimplifyOptions)
{
	...
	TArray<float> TargetLengths = InitTargetLengths(InMesh, MinTargetLength, SimplifyOptions);
	...
	ClusterVerticesByRegionGrowth(InMesh, InMesh, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength, ProcessBuckets);
	...
	TArray<bool> ClusterInOutput;
	FindClustersInOutput(InMesh, Source, ClusterInOutput);
	PreserveCollapsedClusters(InMesh, InMesh, ClusterInOutput, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength, ProcessBuckets);
	...
	BuildSimplifiedMeshFromClusters<FDynamicMesh3, FDynamicMesh3, FDynamicMesh3>(ResultMesh, FromResVID, ResultToSourceTri, ClusterInOutput, bResultHasDuplicateVertices,
		InMesh, InMesh, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength,
		ProcessBuckets);
...
bool Simplify(const FTriangleMeshAdapterd& InMesh, FResultMeshAdapter& OutSimplifiedMesh, const FSimplifyOptions& SimplifyOptions)
{
	...
	ClusterVerticesByRegionGrowth(InMesh, MeshConnectivity, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength, ProcessBuckets);
	for (int32 Idx = 0; Idx < ProcessBuckets.Num(); ++Idx)
	{
		ProcessBuckets[Idx].Empty();
	}
	...
	TArray<bool> ClusterInOutput;
	FindClustersInOutput(InMesh, Source, ClusterInOutput);
	PreserveCollapsedClusters(InMesh, MeshConnectivity, ClusterInOutput, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength, ProcessBuckets);
	...
	BuildSimplifiedMeshFromClusters<TTriangleMeshAdapter<double>, FMeshConnectivity, FResultMeshAdapter>(OutSimplifiedMesh, FromResVID, ResultToSourceTri, ClusterInOutput,
		bResultHasDuplicateVertices,
		InMesh, MeshConnectivity, Source, SourceDist, SimplifyOptions, EdgeTags, VertexTags, TargetLengths, MinTargetLength,
		ProcessBuckets);
