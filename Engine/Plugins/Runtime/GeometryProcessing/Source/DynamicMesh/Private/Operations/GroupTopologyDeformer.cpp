...
void FGroupTopologyDeformer::SetActiveHandleFromSelection(const FGeometrySelection& Selection)
{
	...
	for (const TPair<uint64, double>& WeightedVert : Selection.SoftSelectedVertices)
	{
		int32 CornerID = FGeoSelectionID(WeightedVert.Key).TopologyID;
		if (CornerID >= 0 && CornerID < Topology->Corners.Num())
		{
			Verts.Add(CornerID);
		}
	...
	SetActiveHandle(Faces, Edges, Verts);

}


void FGroupTopologyDeformer::SetActiveHandle(
	...
	)
{
	...
	for (int GroupID : FaceGroupIDs)
	{
		...
		for (int NbrGroupID : CurGroupNbrGroups)
		{
			if (HandleGroups.Contains(NbrGroupID) == false)
			{
				NbrGroups.AddUnique(NbrGroupID);
			}
		}
	}
	...
	for (int EdgeID : TopologyEdgeIDs)
	{
		...
		{
			HandleVertices.Add(VertID);
			HandleBoundaryVertices.Add(VertID);
		}
	}

	TArray<int> EdgeNbrGroups;
	Topology->FindEdgeNbrGroups(TopologyEdgeIDs, EdgeNbrGroups);
	for (int NbrGroupID : EdgeNbrGroups)
	{
		if (HandleGroups.Contains(NbrGroupID) == false)
		{
			NbrGroups.AddUnique(NbrGroupID);
	...
	// Corner
	for (int CornerID : TopologyCornerIDs)
	{
		int VertID = Topology->GetCornerVertexID(CornerID);
		if (VertID >= 0)
		{
	...
	TArray<int> CornerNbrGroups;
	Topology->FindCornerNbrGroups(TopologyCornerIDs, CornerNbrGroups);
	for (int NbrGroupID : CornerNbrGroups)
	{
		if (HandleGroups.Contains(NbrGroupID) == false)
		{
			NbrGroups.AddUnique(NbrGroupID);
