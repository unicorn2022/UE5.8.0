...
bool FMeshConvexHull::Compute(FProgressCancel* Progress)
{
	...
	if (bPostSimplify)
	{
		bOK = SimplifyHull(ConvexHull, SimplifyOptions, Progress);
	}
...
bool FMeshConvexHull::SimplifyHull(FDynamicMesh3& HullMesh, FMeshConvexHull::FSimplifyOptions SimplifyOptions, FProgressCancel* Progress)
{
	...
	if (!ensure(SimplifyOptions.MaxTargetFaceCount > 0))
	{
		SimplifyOptions.MaxTargetFaceCount = 1;
	}
	bool bSimplified = false;

	
	bool bUseGeometricError = SimplifyOptions.GeometricToleranceAfterTarget > 0;
	FDynamicMeshAABBTree3 Spatial;
	TUniquePtr<FDynamicMesh3> RefMesh;
	if (bUseGeometricError)
	{
		RefMesh = MakeUnique<FDynamicMesh3>(HullMesh);
		Spatial.SetMesh(RefMesh.Get(), true);
	}

	if (HullMesh.TriangleCount() > SimplifyOptions.MaxTargetFaceCount)
	{
		...
		Simplifier.SimplifyToTriangleCount(SimplifyOptions.MaxTargetFaceCount);
		if (bUseGeometricError)
		{
			Simplifier.GeometricErrorConstraint = FVolPresMeshSimplification::EGeometricErrorCriteria::PredictedPointToProjectionTarget;
			Simplifier.GeometricErrorTolerance = SimplifyOptions.GeometricToleranceAfterTarget;
			UE::Geometry::FMeshProjectionTarget ProjTarget(RefMesh.Get(), &Spatial);
