	...
	const TSet<int32>& DisallowedVids, FProgressCancel* Progress)
{
	...
	else
	{
		...
		CurrentEdgeVertPlaneDistances[1] = (float)FVector::PointPlaneDist((FVector)Mesh.GetVertex(EdgeVids.B), CutPlaneOrigin, CutPlaneNormal);
			...
			OutputPath.Emplace(FMeshSurfacePoint::MakeEdgePoint(StartPoint.ElementID, StartPoint.EdgeTValue), FDynamicMesh3::InvalidID);
		}
	...
	while (!(CurrentElementID == EndPoint.ElementID && bCurrentPointIsVertex == EndPoint.bIsVertex))
	{
			...
			for (int32 Tid : Mesh.VtxTrianglesItr(CurrentElementID))
			{
				...
				if (!EndPoint.bIsVertex)
				{
					...
					for (int32 i = 0; i < 3; ++i)
					{
						...
						{
							// Got to the end
							OutputPath.Emplace(FMeshSurfacePoint::MakeEdgePoint(EndPoint.ElementID, EndPoint.EdgeTValue), FDynamicMesh3::InvalidID);
							return true;
						}
					}
				}
		...
		else
		{
			...
			else
			{
				...
				OutputPath.Emplace(FMeshSurfacePoint::MakeEdgePoint(Eid, EdgeTValue), FDynamicMesh3::InvalidID);
