...
void FMeshSimpleShapeApproximation::Generate_ConvexHulls(FSimpleShapeSet3d& ShapeSetOut, FProgressCancel* Progress)
{
	...
	ParallelFor(SourceMeshes.Num(), [&](int32 idx)
	{
		...
		Hull.SimplifyOptions.MaxTargetFaceCount = HullTargetFaceCount;
		Hull.SimplifyOptions.GeometricToleranceAfterTarget = HullGeometricToleranceAfterTarget;
...
void FMeshSimpleShapeApproximation::Generate_ConvexHullDecompositions(FSimpleShapeSet3d& ShapeSetOut, FProgressCancel* Progress)
{
	...
	ParallelFor(SourceMeshes.Num(), [&](int32 idx)
	{
		...
		for (int32 HullIdx = 0; HullIdx < Decomposition.NumHulls(); HullIdx++)
		{
			...
			FMeshConvexHull::FSimplifyOptions SimplifyOptions;
			SimplifyOptions.MaxTargetFaceCount = HullTargetFaceCount;
			SimplifyOptions.GeometricToleranceAfterTarget = HullGeometricToleranceAfterTarget;
			if (bSimplifyHulls && FMeshConvexHull::SimplifyHull(HullMesh, SimplifyOptions, Progress) == false)
			{
				return;
			}
