	...
	float CurveConstraintToleranceDistance)
{
	...
	float EdgeToleranceDistSq = CurveConstraintToleranceDistance > 0 ? CurveConstraintToleranceDistance * CurveConstraintToleranceDistance : -1.f;
	// helper to update edge tolerance distances
	auto MergeEdgeToleranceDistSq = [](float A, float B) -> float
	{
		if (A <= 0) // if A is invalid, return B
		{
			return B;
		}
		else if (B <= 0) // if B is invalid, return A
		{
			return A;
		}
		else // if both valid, return the smallest tolerance
		{
	...
	for (int32 TopologyEdgeIndex = 0; TopologyEdgeIndex < NumEdges; ++TopologyEdgeIndex)
	{
		...
		for (int32 EdgeEdgeIndex = 0; EdgeEdgeIndex < EdgeEdges.Num(); ++EdgeEdgeIndex)
		{
			...
			if (Constraints.GetEdgeConstraint(EdgeID, FoundEdgeConstraint))
			{
				...
				FoundEdgeConstraint.DistanceToleranceSq = MergeEdgeToleranceDistSq(EdgeToleranceDistSq, FoundEdgeConstraint.DistanceToleranceSq);
			...
			else
			{
				...
				GroupEdgeConstraint.DistanceToleranceSq = EdgeToleranceDistSq;
