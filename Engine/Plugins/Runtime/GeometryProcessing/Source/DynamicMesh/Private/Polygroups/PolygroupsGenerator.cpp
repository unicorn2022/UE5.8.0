	...
	int MaxSearchRounds)
{
	...
	while (!bDone && Iters++ < MaxSearchRounds)
	{
		...
		if ((MaxSearchRounds - Iters) > 0)
		{
			...
			//UE_LOGF(LogTemp, Warning, "Found %d quads, have %d tris remaining, %d isolated tris, %d isolated-adjacent quads", FoundQuads.Num(), RemainingTriangles, NumIsolated, (FoundQuads.Num() - LastRoundQuads.Num()));
		}
#endif
	}
