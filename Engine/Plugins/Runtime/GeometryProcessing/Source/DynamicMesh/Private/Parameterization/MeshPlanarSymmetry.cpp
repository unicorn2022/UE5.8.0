...
bool FMeshPlanarSymmetry::AssignMatches(const FDynamicMesh3* Mesh, const TPointHashGrid3d<int32>& VertexHash, const TArray<FVector3d>& InvariantFeatures, FFrame3d SymmetryFrameIn)
{
	...
	TArray<TArray<int32, TInlineAllocator<4>>> CandidatesPerPositiveVertex;
	CandidatesPerPositiveVertex.SetNum(PositiveVerts.Num());
	...
	ParallelFor(PositiveVerts.Num(), [&](int32 k)
		{
			...
			VertexHash.FindPointsInBall(MirrorPosition, MatchVertexTolerance, [MirrorPosition, Mesh](const int32& OtherVID)
			{
			...
			if (Candidates.IsEmpty())
			{
				...
				if (bReattemptMatchesOnFailure && Vertices[VertexID].PlaneSignedDistance >= OnPlaneTolerance)
				{
					VertexHash.FindPointsInBall(MirrorPosition, MatchVertexTolerance, [MirrorPosition, Mesh](const int32& OtherVID)
					{
						return FVector3d::DistSquared(MirrorPosition, Mesh->GetVertex(OtherVID));
			...
			// Rank all candidates first by Euclidean distance and then by feature distance
			Algo::Sort(Candidates,
				[&MirrorPosition, Mesh, &InvariantFeatures, Feature](const int32& A, const int32& B)
				{
					double DistanceA = FVector3d::DistSquared(MirrorPosition, Mesh->GetVertex(A));
					double FeatureDistanceA = FVector3d::DistSquared(InvariantFeatures[A], Feature);
				
					double DistanceB = FVector3d::DistSquared(MirrorPosition, Mesh->GetVertex(B));
					double FeatureDistanceB = FVector3d::DistSquared(InvariantFeatures[B], Feature);
					...
					if (DistanceA == DistanceB)
					{
						if (FeatureDistanceA < FeatureDistanceB)
						{
							return true;
						}
					}
			...
			CandidatesPerPositiveVertex[k] = MoveTemp(Candidates); 
		});
	
	for (int32 k = 0; k < PositiveVerts.Num(); k++)
	{
		int32 VertexID = PositiveVerts[k];
		
		for (int32 Candidate : CandidatesPerPositiveVertex[k])
		{
			// Avoid many to one matching
			if (Vertices[Candidate].PairedVertex != INDEX_NONE)
			{
				continue;
			}

			Vertices[Candidate].PairedVertex = VertexID;
			Vertices[VertexID].PairedVertex = Candidate;
			break;
		}
	}
...
{
	constexpr bool bForceComputeVertexPairs = false;
	return Initialize(Mesh, Bounds, SymmetryFrameIn, bForceComputeVertexPairs);
}

bool FMeshPlanarSymmetry::Initialize(FDynamicMesh3* Mesh, const FAxisAlignedBox3d& Bounds, FFrame3d SymmetryFrameIn, bool bForceComputeVertexPairs)
{
	this->TargetMesh = Mesh;
	if (!Validate(Mesh))
	{
...
void FMeshPlanarSymmetry::GetMirrorVertexROI(const TArray<int>& VertexROI, TArray<int>& MirrorVertexROI, bool bForceSameSizeWithGaps, TFunctionRef<bool(int32 VID)> ShouldExcludeVertex) const
{
	...
	for (int32 VertexID : VertexROI)
	{
		...
		int32 MirrorVID = INDEX_NONE;
		if (Vertex.bIsSourceVertex)
		{
			MirrorVID = Vertex.PairedVertex;
		}
		...
		if (MirrorVID == INDEX_NONE || ShouldExcludeVertex(MirrorVID))
		{
			if (bForceSameSizeWithGaps)
			{
				MirrorVertexROI.Add(INDEX_NONE);
			}
		}
		else
		{
