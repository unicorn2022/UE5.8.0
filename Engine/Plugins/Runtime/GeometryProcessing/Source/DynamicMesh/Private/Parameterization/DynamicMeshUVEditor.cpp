...
bool FDynamicMeshUVEditor::SetTriangleUVsFromExpMap(const TArray<int32>& Triangles, const FExpMapOptions& Options, FUVEditResult* Result)
{
	...
	for (int32 vid : Submesh.VertexIndicesItr())
	{
		if (Param.HasUV(vid))
		{
			// This is super suspicious. GetUV returns TNumericLimits<double> as an invalid UV, which then gets
			// cast down here as a float, losing invalids entirely. Under precise float math this causes a compilation
			// error, which goes away if you add an intermediary because it no longer gets fully inlined causing the double to get
			// placed into the float.
			// .. separately, not sure about this in the vector header where it just willy nilly casts around. Should probably have this
			// do a saturation on the input ?
			/*
			...
			*/
			FVector2d UVd = Param.GetUV(vid);
			FVector2f UV;

			// Stil doesn't handle when we're at _min_, but at least this doesn't cause a compilation error.
			double MaxFloat = TNumericLimits<float>::Max();
			UV.X = static_cast<float>(UVd.X > MaxFloat ? MaxFloat : UVd.X);
			UV.Y = static_cast<float>(UVd.Y > MaxFloat ? MaxFloat : UVd.Y);
...
bool FDynamicMeshUVEditor::AutoOrientUVArea(const TArray<int32>& Triangles)
{
	...
	{
		FVector2f A = HullPoints[j%NV], B = HullPoints[(j+1)%NV], C = HullPoints[(j+NV/2)%NV]; //-V609
		FVector2f Axis0 = B - A; 
