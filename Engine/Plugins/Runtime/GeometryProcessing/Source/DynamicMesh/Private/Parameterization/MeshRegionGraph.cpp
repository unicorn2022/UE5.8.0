	...
	TFunctionRef<bool(int32, int32)> TrisConnectedPredicate)
{
	...
	Regions.SetNum(N);
	TriangleToRegionMap.Init(INDEX_NONE, Mesh.MaxTriangleID());
	TriangleNbrTris.Init(FIndex3i(INDEX_NONE, INDEX_NONE, INDEX_NONE), Mesh.MaxTriangleID());
...
bool FMeshRegionGraph::OptimizeBorders(int32 MaxRounds)
{
	auto GetSwapNbrRegionIndex = [this](int32 tid) -> int32
	{
		...
		for (int32 j = 0; j < 3; ++j)
		{
			if (NbrTris[j] >= 0)
			{
				...
				{
					Nbrs[NbrCount++] = NbrRegionIndex;
				}
			}
		}
		if ((NbrCount == 2 && Nbrs[0] == Nbrs[1]) || (NbrCount == 3 && Nbrs[0] == Nbrs[1] && Nbrs[0] == Nbrs[2]))
		{
			return Nbrs[0];
		}
	...
	{
		PotentialSwaps.Reset();

		for (int32 tid = 0, Num = TriangleToRegionMap.Num(); tid < Num; ++tid)
		{
			if (TriangleToRegionMap[tid] != INDEX_NONE)
			{
				const int32 SwapToNbrIndex = GetSwapNbrRegionIndex(tid);
		...
		{
			if (GetSwapNbrRegionIndex(Swap.A) == Swap.B)
			{
				TriangleToRegionMap[Swap.A] = Swap.B;
				bModified = true;
			}
		}
	}
	...
	if (bModified)
	{
		...
		for (int32 tid = 0, Num = TriangleToRegionMap.Num(); tid < Num; ++tid)
		{
			if (TriangleToRegionMap[tid] != INDEX_NONE)
			{
				Regions[TriangleToRegionMap[tid]].Triangles.Add(tid);
			}
		}
	}
