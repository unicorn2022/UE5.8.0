...
class FDynamicMesh3Policy
{
  ...
  public:
	...
	UE::Geometry::FEdgeSplitInfo SplitEdge(const int32 TriIndex, const int32 TriEdgeIndex, const double SplitWeight )
	{
		const int EdgeID = Mesh.GetTriEdge(TriIndex, TriEdgeIndex);
		...
		const double Weight = (vA < vB) ? SplitWeight : (1. - SplitWeight);
