...
public:
	 ...
	 * Given a Mesh and its bounding box, and a Symmetry Plane/Frame, detect any pairs of vertices with
	 * planar/mirror-symmetry relationships, as well as "on plane" vertices
	 * @param bForceComputeVertexPairs Computes vertex pairs even if the mesh doesn't pass quick full symmetry test
	 * @return false if any non-on-plane vertex fails to find a match.
	 */
	UE_API bool Initialize(FDynamicMesh3* Mesh, const FAxisAlignedBox3d& Bounds, FFrame3d SymmetryFrameIn, bool bForceComputeVertexPairs);
	 ...
	 * for vertices in VertexROI that are source vertices, or on the symmetry plane, or excluded by ShouldExcludeVertex().
	 * Otherwise those vertices are skipped.
	 * 
	 * @param ShouldExcludeVertex Optional filter to exclude vertices: takes vertex ID, returns true if the vertex should not be added to the ROI, false otherwise
	 */
	UE_API void GetMirrorVertexROI(const TArray<int>& VertexROI, TArray<int>& MirrorVertexROIOut, bool bForceSameSizeWithGaps, TFunctionRef<bool(int32 VID)> ShouldExcludeVertex = [](int32) {return false;}) const;
