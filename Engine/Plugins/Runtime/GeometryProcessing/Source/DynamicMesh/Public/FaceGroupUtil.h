...
PREDECLARE_USE_GEOMETRY_CLASS(ITriangleGroupSetAdapter);
...
namespace FaceGroupUtil
{
	...
	*/
	UE_DEPRECATED(5.8, "Use TArray<TUniquePtr<FDynamicMesh3>> version")
	DYNAMICMESH_API void SeparateMeshByGroups(FDynamicMesh3& Mesh, TArray<FDynamicMesh3>& SplitMeshes);
	/**
	 * split input Mesh into submeshes based on group ID
	 * **does not** separate disconnected components w/ same group ID
	 */
	DYNAMICMESH_API void SeparateMeshByGroups(FDynamicMesh3& Mesh, TArray<TUniquePtr<FDynamicMesh3>>& SplitMeshes);
	...
	*/
	 ...
	 * **does not** separate disconnected components w/ same group ID
	 */
	DYNAMICMESH_API void SeparateMeshByGroups(FDynamicMesh3& Mesh, TArray<TUniquePtr<FDynamicMesh3>>& SplitMeshes, TArray<int32>& GroupIDs);
