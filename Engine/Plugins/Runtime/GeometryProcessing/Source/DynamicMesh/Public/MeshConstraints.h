...
enum class EEdgeRefineFlags 
{
	...
	NoCollapse = 4,
	/** Topology of constraints should not be changed, i.e. disconnected constraint edges should not be connected via an adjacent edge collapse */
	NoTopologyMerge = 8,
	/** Edge cannot be flipped, split, collapsed, or topologically-merged */
...
public:
	...
	/** Squared tolerance distance: if positive, edge should not move farther than this distance from Target (if set, and feature is enabled; see TMeshSimplification::bLimitConstrainedSeamMovement) */
	float DistanceToleranceSq = -1.f;
	...
	bool CanMergeTopology() const
	{
		return CanMergeTopology(RefineFlags);
	}
	...
	static inline bool CanMergeTopology(const EEdgeRefineFlags Flags)
	{
		return ((int)Flags & (int)EEdgeRefineFlags::NoTopologyMerge) == 0;
	}
...
public:
	...
	{
		int32 NearestSegmentIdx_Unused;
		double DistSq_Unused, UnitParam_Unused;
		FVector3d ClosestPoint;
		if (CurveUtil::ProjectToCurve<double, FVector3d>(Vertices, false, Point, DistSq_Unused, NearestSegmentIdx_Unused, ClosestPoint, UnitParam_Unused, nullptr))
		{
			return ClosestPoint;
		}
		else
		{
			// Just return the input if the projection fails (i.e., if the constraint curve is empty)
			return Point;
		}
	}
};
