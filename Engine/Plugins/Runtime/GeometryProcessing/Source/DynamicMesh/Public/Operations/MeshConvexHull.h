...
public:
	...
	/** If true, output convex hull is simplified down based on the SimplifyOptions */
	...
	struct FSimplifyOptions
	{
		...
		double GeometricToleranceAfterTarget = 0.;
	};

	/** IF bPostSimplify, options controlling the simplification */
	FSimplifyOptions SimplifyOptions;

	UE_DEPRECATED(5.8, "Use SimplifyOptions.MaxTargetFaceCount instead")
	int32 MaxTargetFaceCount = 0;
...
public:
	...
	{
		FSimplifyOptions Options;
		Options.MaxTargetFaceCount = MaxTargetFaceCount;
		return SimplifyHull(HullMesh, Options, Progress);
	}
	 ...
	 * @return true on success
	 */
	static UE_API bool SimplifyHull(FDynamicMesh3& HullMesh, FSimplifyOptions SimplifyOptions, FProgressCancel* Progress = nullptr);
