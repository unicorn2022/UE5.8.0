...
struct FSimplifyOptions
{
	...
	// Optionally locally scales the simplifier's TargetEdgeLength on a per-vertex basis
	// If provided, the target edge length at a vertex VID will be TargetEdgeLength * OptionalTargetEdgeLengthScale(VID)
	TFunction<double(int32)> OptionalTargetEdgeLengthScale = nullptr;
