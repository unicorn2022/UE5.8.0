...
/** Method used to compute edge weights during bone weight smoothing. */
enum class ESmoothBoneWeightsEdgeWeightMethod : uint8
{
	/** Each neighbor contributes equally (existing behavior). */
	Uniform = 0,
	/** Neighbors weighted by cotangent of opposite angles on shared edge. */
	CotanWeights = 1
};
...
public:
	...
	/** The edge weighting method used during smoothing. */
	ESmoothBoneWeightsEdgeWeightMethod EdgeWeightMethod = ESmoothBoneWeightsEdgeWeightMethod::Uniform;
...
protected:
	...
	/** Pre-computed cotangent edge weights indexed by EdgeID. Populated lazily when CotanWeights is selected. */
	TArray<double> CotangentEdgeWeights;

	/** Pre-compute edge weights if needed based on EdgeWeightMethod. */
	void ComputeEdgeWeightsIfNeeded();
...
public:
	 ...
	 * @param VertexID		  The vertex whose weights we are smoothing.
	 * @param VertexFalloff   The lerp value between the old and the new weight.
	 * @param LockedBones	  The bones to exclude from the smooth op
	 * @param OutFinalWeights The smoothed weights.
	 * 
	 * @return true if the algorithm succeeds, false if it failed or was canceled by the user.
	 */
