...
//    Scalar check_eps = mesh.params.eps_input * mesh.params.eps_input;
        ...
        for (int j = 0; j < 4; j++) {
            if (t.is_surface_fs[j] <= 0) {
                ...
                if(tree.is_out_sf_envelope(ps, mesh.params.eps_2)){
                    ...
                    UE_LOGF(LogTemp, Warning, "fTetWild mesh face is too far from input mesh: %f > %f!", d, check_eps);
                }
            }
        }
    }
    ...
    for (auto &v: mesh.tet_vertices) {
        ...
        if (d > mesh.params.eps_2) {
            UE_LOGF(LogTemp, Warning, "fTetWild mesh vertex is too far from input mesh: %f > %f!", d, check_eps);
        }
    }
}
