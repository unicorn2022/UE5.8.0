...
//    timer.start();
    ...
    if (!cut_mesh.get_intersecting_edges_and_points(points, map_edge_to_intersecting_point, subdivide_t_ids)) {
		...
		UE_LOGF(LogTemp, Warning, "fTetWild failure in get_intersecting_edges_and_points");
        return false;
    }
    ...
    if (!subdivide_tets(insert_f_id, mesh, cut_mesh, points, map_edge_to_intersecting_point, track_surface_fs,
		...
		UE_LOGF(LogTemp, Warning, "fTetWild failure in subdivide_tets");
        return false;
    }
...
//        b_edges.push_back(e);
        ...
        if (n_f_ids.empty()) {
            UE_LOGF(LogTemp, Warning, "fTetWild failure in insert_boundary_edges: n_f_ids is empty");
            continue;
        }
        ...
        if (!insert_boundary_edges_get_intersecting_edges_and_points(covered_fs_infos,
            ...
            UE_LOGF(LogTemp, Warning, "fTetWild failure in insert_boundary_edges_get_intersecting_edges_and_points");
            continue;
        }
        ...
        if (!subdivide_tets(-1, mesh, empty_cut_mesh, points, map_edge_to_intersecting_point, track_surface_fs,
            ...
            if (!is_inside_envelope) {
				...
				UE_LOGF(LogTemp, Warning, "fTetWild semi-failure in subdivide_tets");
            }
