...
void UHeadMountedDisplayFunctionLibrary::SetSpectatorScreenMode(ESpectatorScreenMode Mode)
{
	...
	{
		UE_LOGF(LogHMD, Warning, "SetSpectatorScreenMode called, but the SpectatorScreenController was not found.  Perhaps this platform does not support spectator screen?  Ignoring this call.");
	}
}
...
void UHeadMountedDisplayFunctionLibrary::SetSpectatorScreenTexture(UTexture* InTexture)
{
	...
	if (Controller)
	{
		if (!InTexture)
		{
			UE_LOGF(LogHMD, Warning, "SetSpectatorScreenTexture blueprint function called with null Texture!");
		}
