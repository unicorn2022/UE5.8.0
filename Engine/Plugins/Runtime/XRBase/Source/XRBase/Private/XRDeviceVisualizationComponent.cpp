...
void UXRDeviceVisualizationComponent::OnRegister() {
	...
	{
		UE_LOGF(LogXRDeviceVisualizationComponent, Error, "No parent UMotionControllerComponent for %ls was found in the hierarchy. This component will not render anything.", *GetName());
	}
}
...
void UXRDeviceVisualizationComponent::RefreshMesh()
{
	if (CanDeviceBeDisplayed())
	{
		...
		if (DisplayModelSource == UXRDeviceVisualizationComponent::CustomModelSourceId)
		{
			...
			{
				SetStaticMesh(nullptr);
				UE_LOGF(LogXRDeviceVisualizationComponent, Error, "Failed to set a custom mesh for %ls because no custom mesh was specified.", *GetName());
			}
		}
		else
		{
			...
			for (IXRSystemAssets* AssetSys : XRAssetSystems)
			{
				...
				if (RenderComponent != nullptr) 
				{
					...
					else 
					{
						...
						SetStaticMesh(nullptr);
						UE_LOGF(LogXRDeviceVisualizationComponent, Error, "CreateRenderComponent must return a UStaticMeshComponent. Setting a NULL StaticMesh.");
					}
				}
				else 
				{
					...
					{
						UE_LOGF(LogXRDeviceVisualizationComponent, Error, "CreateRenderComponent returned a NULL UPrimitiveComponent*. Setting a NULL StaticMesh.");
					}
				}
			}
		}
...
void UXRDeviceVisualizationComponent::OnDisplayModelLoaded(UPrimitiveComponent* InDisplayComponent)
{
	if (DisplayModelLoadState == EModelLoadStatus::Pending || DisplayModelLoadState == EModelLoadStatus::InProgress)
	{
		...
		{
			SetStaticMesh(nullptr);
			UE_LOGF(LogXRDeviceVisualizationComponent, Warning, "OnDisplayModelLoaded expects to received a UStaticMeshComponent with a valid StaticMesh. Setting a NULL StaticMesh.");
		}
	}
