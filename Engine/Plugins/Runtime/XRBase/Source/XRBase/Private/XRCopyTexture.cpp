...
void XRCopyTexture_InRenderPass(
	...
	FIntRect DstRect,
	const FXRCopyTextureOptions& Options,
	uint32 ArraySlice, uint32 MipLevel)
{
	...
	TShaderRef<FXRCubemapPS> CubemapPS;
	...
	if (bIsCubeSource)
	{
		TShaderMapRef<FXRCubemapPS> ScreenPSRef(ShaderMap);
	...
	const FXRCopyTextureOptions& Options)
{
	...
	int TotalSlices = bIsCubeCopy ? 6 : bIsArrayCopy ? FMath::Min(DstTexture->Desc.ArraySize, SrcTexture->Desc.ArraySize) : 1;
