	...
	const FXRComponentLoadComplete& OnLoadComplete)
{
	...
	if (!IsValid(Target))
	{
		UE_LOGF(LogHMD, Warning, "The target actor is invalid. Therefore you're unable to add a device render component to it.");
		return nullptr;
	}
	...
	for (IXRSystemAssets* AssetSys : XRAssetSystems)
	{
		...
		{
			UE_LOGF(LogHMD, Warning, "The specified XR device does not have an associated render model.");
		}
		break;
	}
	...
	if (NewComponent == nullptr)
	{
		UE_LOGF(LogHMD, Warning, "Failed to find an active XR system with a model for the requested device.");
	}
	...
	const FXRComponentLoadComplete& OnLoadComplete)
{
	...
	if (NewComponent == nullptr)
	{
		UE_LOGF(LogHMD, Warning, "Failed to find an active XR system with a model for the requested device: %ls.", *DeviceName.ToString());
	}
	return NewComponent;
}
