...
#define XRTHREADUTILS_SUPPRESS_DEPRECATION 1
#include "XRThreadUtils.h"
#undef XRTHREADUTILS_SUPPRESS_DEPRECATION

#include "RenderingThread.h"
#include "RHICommandList.h"

XRBASE_API void ExecuteOnRenderThread_DoNotWait(const TFunction<void()>& Function)
{
...
XRBASE_API void ExecuteOnRenderThread_DoNotWait(const TFunction<void(FRHICommandListImmediate&)>& Function)
{
	check(IsInGameThread());
...
XRBASE_API void ExecuteOnRenderThread(const TFunctionRef<void()>& Function)
{
	check(IsInGameThread());
...
XRBASE_API void ExecuteOnRenderThread(const TFunctionRef<void(FRHICommandListImmediate&)>& Function)
{
	check(IsInGameThread());
...
XRBASE_API bool ExecuteOnRHIThread_DoNotWait(const TFunction<void()>& Function)
{
	return ExecuteOnRHIThread_Impl(Function, false);
}

XRBASE_API bool ExecuteOnRHIThread_DoNotWait(const TFunction<void(FRHICommandListImmediate&)>& Function)
{
	return ExecuteOnRHIThread_Impl(Function, false);
}

XRBASE_API void ExecuteOnRHIThread(const TFunctionRef<void()>& Function)
{
	ExecuteOnRHIThread_Impl(Function, true);
}

XRBASE_API void ExecuteOnRHIThread(const TFunctionRef<void(FRHICommandListImmediate&)>& Function)
{
	ExecuteOnRHIThread_Impl(Function, true);
}
