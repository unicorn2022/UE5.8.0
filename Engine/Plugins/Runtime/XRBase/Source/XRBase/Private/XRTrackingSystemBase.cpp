...
void FXRTrackingSystemBase::CalibrateExternalTrackingSource(const FTransform& ExternalTrackingTransform)
{
	...
	UE_LOGF(LogHMD, Log, "XRTracker is now using calibrated offset to external tracking source: [%ls]", *CalibratedOffset.GetTranslation().ToCompactString());
}
...
void FXRTrackingSystemBase::UpdateExternalTrackingPosition(const FTransform& ExternalTrackingTransform)
{
	...
	//UE_LOGF(LogHMD, Warning, "---> Delta from Calibrated: [%ls] (%ls)", *DeltaOffset.GetTranslation().ToCompactString(), *DeltaOffset.GetRotation().Rotator().ToCompactString());
...
void FXRTrackingSystemBase::GetMotionControllerState(UObject* WorldContext, const EXRSpaceType XRSpaceType, const EControllerHand Hand, const EXRControllerPoseType XRControllerPoseType, FXRMotionControllerState& MotionControllerState)
{
	...
	{
		UE_LOGF(LogHMD, Warning, "GetMotionControllerData could not find matching UMotionControllerComponent, Hand: %d", int(Hand));
	}
...
{
	UE_LOGF(LogHMD, Warning, "GetCurrentInteractionProfile for %i failed because the current VR tracking system does not support it!", int(Hand));
