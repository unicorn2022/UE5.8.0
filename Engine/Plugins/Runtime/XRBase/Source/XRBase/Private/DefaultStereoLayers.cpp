...
// static
void FDefaultStereoLayers::StereoLayerRender(
	FRHICommandListImmediate& RHICmdList,
	TArrayView<const FStereoLayerToRender> LayersToRender,
	const FDefaultStereoLayers_LayerRenderParams& RenderParams)
{
	...
	{
		// We should have tested for this when building the LayersToRender array, but just in case lets check here.
		if (Layer.Texture == nullptr || (Layer.Flags & LAYER_FLAG_HIDDEN))
		{
			continue;
		}
...
void FDefaultStereoLayers::StereoLayerRender(
	FRHICommandListImmediate& RHICmdList,
	const FStereoLayerToRender& Layer,
	const FDefaultStereoLayers_LayerRenderParams& RenderParams)
{
	check(IsInRenderingThread());
	...
	GraphicsPSOInit.RasterizerState = TStaticRasterizerState<FM_Solid, CM_None, ERasterizerDepthClipMode::DepthClip, false>::GetRHI();
	GraphicsPSOInit.DepthStencilState = Layer.Flags & LAYER_FLAG_SUPPORT_DEPTH ? TStaticDepthStencilState<true, CF_DepthNear>::GetRHI() : TStaticDepthStencilState<true, CF_Always>::GetRHI();
	...
	const bool bIsOpaque = Layer.Flags & LAYER_FLAG_TEX_NO_ALPHA_CHANNEL;
	const bool bIsExternal = Layer.Flags & LAYER_FLAG_TEX_EXTERNAL;
...
bool FDefaultStereoLayers::FStereoLayerToRender::operator<(const FDefaultStereoLayers::FStereoLayerToRender& Rhs)
{
...
FDefaultStereoLayers::FStereoLayerToRender::FStereoLayerToRender(const FStereoLayerToRenderTransfer& Transfer)
	...
	, Texture(Transfer.Texture ? Transfer.Texture->GetTextureRHI() : nullptr)
#if WITH_EDITOR
	 , Name(Transfer.Name)
#endif
{
...
void FDefaultStereoLayers::BeginRenderViewFamily(FSceneViewFamily& InViewFamily)
{
	...
	bool bClearLayerBackgroundThisFrame = !IsBackgroundLayerVisible();
