...
TSharedRef<SWidget> UAudioMaterialKnob::RebuildWidget()
{
	Knob = SNew(SAudioMaterialKnob)
		...
		.OnMouseCaptureBegin(BIND_UOBJECT_DELEGATE(FSimpleDelegate, HandleOnKnobCaptureBegin))
		.OnMouseCaptureEnd(BIND_UOBJECT_DELEGATE(FSimpleDelegate, HandleOnKnobCaptureEnd));

	return Knob.ToSharedRef();
}

void UAudioMaterialKnob::HandleOnKnobCaptureBegin()
{
	OnMouseCaptureBegin.Broadcast();
}

void UAudioMaterialKnob::HandleOnKnobCaptureEnd()
{
