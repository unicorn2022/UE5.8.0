...
namespace AudioWidgets
{
		...
		const float InAnalysisPeriodMs)
	{
		if (!InAudioBus)
		{
			UE_LOGF(LogAudioWidgets, Error, "Unable to obtain audio samples for visualization without a valid audio bus.");
			return;
		}
		...
		{
			UE_LOGF(LogAudioWidgets, Error, "Unable to obtain audio samples for visualization without a valid audio device manager.");
			return;
		}
		...
		if (!MixerDevice)
		{
			UE_LOGF(LogAudioWidgets, Error, "Unable to obtain audio samples for visualization without a valid audio mixer.");
			return;
		}
