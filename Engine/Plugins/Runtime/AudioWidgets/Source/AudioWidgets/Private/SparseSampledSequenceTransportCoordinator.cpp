...
const bool FSparseSampledSequenceTransportCoordinator::IsScrolling() const
{
	return bIsScrolling;
}

void FSparseSampledSequenceTransportCoordinator::SetScrolling(const bool bInIsScrolling)
{
	bIsScrolling = bInIsScrolling;
}
...
const double FSparseSampledSequenceTransportCoordinator::GetFocusPoint() const
{
	return FocusPoint;
}
...
{	 
	...
	if (NewPlayheadPosition < 0.f)
	{
		// If the playhead is off-screen to the left (e.g. loop restarted or user scrolled away),
		// recenter the display range with the playback position on the left edge

		double NewDisplayLowBound = FMath::Clamp(ScaledPlaybackRatio, 0.0, 1.0 - ZoomRatio);
		double NewDisplayUpBound = NewDisplayLowBound + ZoomRatio;

		// Update DisplayRange first so the Playhead is placed correctly at the left edge
		UpdateDisplayRange(NewDisplayLowBound, NewDisplayUpBound);
		
		constexpr double LeftBound = 0.0;
		MoveFocusPoint(LeftBound);
	}
	else if (NewPlayheadPosition >= FocusPointLockPosition)
	{
		...
		{
			// If playhead is far off screen, the NewPlayheadPosition could be > 1.0
			constexpr double RightBound = 1.0;
			MoveFocusPoint(FMath::Min(NewPlayheadPosition, RightBound));
		}
	}
	else
	{
