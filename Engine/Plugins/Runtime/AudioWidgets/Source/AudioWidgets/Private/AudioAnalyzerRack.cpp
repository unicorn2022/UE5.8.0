...
namespace AudioWidgets
{
		...
		: TabManagerLayoutName(Params.TabManagerLayoutName)
	{
...

		{
			...
			{
				StringBuilder << '_' << RackUnitTypeName;
			}

			TabManagerLayoutName = FName(StringBuilder);
		}

		ExcludedRackUnitTypes = Params.ExcludedRackUnitTypes;
	...
	TSharedRef<SWidget> FAudioAnalyzerRack::CreateWidget(TSharedRef<SDockTab> DockTab, const FSpawnTabArgs& SpawnTabArgs)
	{
		...
		if (!ExcludedRackUnitTypes.IsEmpty())
		{
			RackUnitTypes.RemoveAll([this](const FAudioAnalyzerRackUnitTypeInfo* Type) { return ExcludedRackUnitTypes.Contains(Type->TypeName); });
		}
