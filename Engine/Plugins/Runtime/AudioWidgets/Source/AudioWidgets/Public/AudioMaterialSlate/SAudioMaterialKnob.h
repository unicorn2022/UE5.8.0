...
public:
	...
	/** Invoked when mouse capture ends (mouse release or any other capture loss). */
...
public:
	...
	// Holds a delegate that is executed when the knob's value is committed (mouse capture ends).
	...
	// Holds a delegate that is executed when mouse capture ends.
