...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnKnobMouseCaptureBeginEvent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnKnobMouseCaptureEndEvent);
...
public:
	...
	UPROPERTY(BlueprintAssignable, Category = "Widget Event")
	FOnKnobMouseCaptureBeginEvent OnMouseCaptureBegin;

	/** Called when mouse capture ends (mouse release or any other capture loss). */
	UPROPERTY(BlueprintAssignable, Category = "Widget Event")
	FOnKnobMouseCaptureEndEvent OnMouseCaptureEnd;
...
private:
	...
	void HandleOnKnobCaptureBegin();
	void HandleOnKnobCaptureEnd();
