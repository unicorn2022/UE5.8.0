...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnSliderMouseCaptureBeginEvent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnSliderMouseCaptureEndEvent);
 ...
 * * No Children
 */
UCLASS(MinimalAPI, BlueprintType, Blueprintable, CustomFieldNotify)
...
public:
	...
	UPROPERTY(BlueprintAssignable, FieldNotify, Category = "Widget Event")
	FOnSliderFloatValueChangedEvent OnValueChanged;

	/** Called when mouse capture begins. */
	UPROPERTY(BlueprintAssignable, Category = "Widget Event")
	FOnSliderMouseCaptureBeginEvent OnMouseCaptureBegin;

	/** Called when mouse capture ends (mouse release or any other capture loss). */
	UPROPERTY(BlueprintAssignable, Category = "Widget Event")
	FOnSliderMouseCaptureEndEvent OnMouseCaptureEnd;
...
private:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, FieldNotify, Setter = SetValue, Getter = GetValue, Category = "Appearance", meta = (UIMin = "0", UIMax = "1", AllowPrivateAccess))
	float Value = 1.f;
