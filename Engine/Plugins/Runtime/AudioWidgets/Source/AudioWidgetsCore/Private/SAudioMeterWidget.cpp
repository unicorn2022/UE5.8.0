...
int32 SAudioMeterWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (Orientation == Orient_Vertical)
	{
 	...
 	int32 MaxValueDb = FMath::Max(static_cast<int32>(Style->ValueRangeDb.X), static_cast<int32>(Style->ValueRangeDb.Y));
	...
	if (ValueDeltaDb == 0.0f)
	{
		// ValueRangeDb is zero, don't draw any values.
		return LayerId;
	}
	...
	for (int32 ChannelIndex = 0; ChannelIndex < NumChannels; ++ChannelIndex)
	{
		...
		const float ChannelMeterValueBarStartDb = ChannelInfos[ChannelIndex].MeterValueBarStart.Get(MinValueDb);
		const float ChannelMeterValueBarStart = FMath::Clamp((ChannelMeterValueBarStartDb - MinValueDb) / ValueDeltaDb, 0.0f, 1.0f);

		const FFloatInterval MeterValueBarInterval(
			FMath::Min(ChannelMeterValueBarStart, ChannelMeterValue),
			FMath::Max(ChannelMeterValueBarStart, ChannelMeterValue));

		const float AllottedWidthMinusPadding = AllottedWidth - 2.0f * Style->MeterPadding.X;

		const FVector2D MeterValueTopLeft(Style->MeterPadding.X + MeterValueBarInterval.Min * AllottedWidthMinusPadding,
									Style->MeterPadding.Y + ScaleOffset + Style->MeterValuePadding + ChannelIndex * (Style->MeterSize.Y + Style->MeterPadding.Y));
		
		const FVector2D MeterValueSize(MeterValueBarInterval.Size() * AllottedWidthMinusPadding,
	...
	// Draw the meter clipping value
	const FLinearColor ClippingColor = MeterClippingColor.Get().GetColor(InWidgetStyle) * InWidgetStyle.GetColorAndOpacityTint();
	if (ClippingColor.A > 0.0f)
	{
		for (int32 ChannelIndex = 0; ChannelIndex < NumChannels; ++ChannelIndex)
		{
			const float ChannelClippingValueDb = ChannelInfos[ChannelIndex].ClippingValue;
			const float ChannelClippingValue   = FMath::Clamp((ChannelClippingValueDb - MinValueDb) / ValueDeltaDb, 0.0f, 1.0f);

			const float ClippingPixel = ChannelClippingValue * (AllottedWidth - 2.0f * Style->MeterPadding.X);

			const FVector2D ClippingValueTopLeft(Style->MeterPadding.X + ClippingPixel - 0.5f * Style->ClippingValueWidth,
												 Style->MeterPadding.Y + ScaleOffset + Style->MeterValuePadding + ChannelIndex * (Style->MeterSize.Y + Style->MeterPadding.Y));

			const FVector2D ClippingValueSize(Style->ClippingValueWidth,
											  Style->MeterSize.Y - 2.0f * Style->MeterValuePadding);

			FSlateDrawElement::MakeBox(
				OutDrawElements,
				LayerId,
				MeterGeometry.ToPaintGeometry(ClippingValueSize, FSlateLayoutTransform(ClippingValueTopLeft)),
				&Style->MeterClippingImage,
				ESlateDrawEffect::None,
				Style->MeterClippingImage.GetTint(InWidgetStyle) * ClippingColor
			);
		}
	}
	++LayerId;
	...
	if (Style->bShowScale)
	{
		...
		// Round down MaxValueDb to find the first hash mark to draw:
		int32 CurrentHashValue = DecibelsPerHash * FMath::FloorToInt32(float(MaxValueDb) / float(DecibelsPerHash));
