	...
	if (bIsInteractable)
	{
		if (bRectChanged || !bWasInteractable)
		{
			if (ActionZoneRect.IsValid() && !ActionZoneRect.IsEmpty())
			{
				...
				UE_LOGF(LogGFNActionZoneProcessor, Display, "[GFNWidgetActionZone::UpdateActionZone] Updating Widget %p GFN Action Zone | ActionZoneRect : L: %f , T: %f , R: %f , B: %f", Widget, ActionZoneRect.Left, ActionZoneRect.Top, ActionZoneRect.Right, ActionZoneRect.Bottom);
				...
				if (GfnResult != GfnError::gfnSuccess)
				{
					...
					UE_LOGF(LogGFNActionZoneProcessor, Warning, "[GFNWidgetActionZone::UpdateActionZone] Failed to set Action Zone.  | Error Code : %i", GfnResult);
				}
			}
			else
			{
				UE_LOGF(LogGFNActionZoneProcessor, Display, "[GFNWidgetActionZone::UpdateActionZone] Updating Widget %p GFN Action Zone | No longer interactable", Widget);
...
void FWidgetGFNActionZone::ClearActionZone()
{
	if (bHasActionZone)
	{
		...
		{
			bHasActionZone = true;
			UE_LOGF(LogGFNActionZoneProcessor, Warning, "[GFNWidgetActionZone::ClearActionZone] Failed to Remove Action Zone. | Error Code : %i", GfnResult);
		}
	}
}
...
bool GeForceNOWActionZoneProcessor::Initialize()
{
	...
	{
		UE_LOGF(LogGFNActionZoneProcessor, Warning, "SlateWidgetTracker is not initialized. GeForceNOWActionZoneProcessor will not function.");
	}
	return false;
}
