...
/*static*/ bool GeForceNOWWrapper::IsRunningMockGFN()
{
	...
	{
		UE_LOGF(LogGFNWrapper, Warning, "[GeForceNOWWrapper::IsRunningMockGFN] MockGFN was enabled but no mockgfn file is found. Please create mockgfn.txt file before launching with mockgfn logic.");
	}
#else
...
void GeForceNOWWrapper::HandleLaunchURL(const TCHAR* URL)
{
	if (URL == nullptr)
	{
		UE_LOGF(LogGFNWrapper, Display, "[GeForceNOWWrapper::HandleLaunchURL] Failed to open URL, the provided URL string is empty");
		return;
	}
	...
	{
		UE_LOGF(LogGFNWrapper, Display, "[GeForceNOWWrapper::HandleLaunchURL] Unable to send URL request to client. Error: %d", ErrorCode);
	}
}
