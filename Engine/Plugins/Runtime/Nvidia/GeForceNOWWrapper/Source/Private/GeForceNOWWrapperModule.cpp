...
public:
	...
	virtual void StartupModule() override
	{
		...
		{
			UE_LOGF(LogGeForceNow, Log, "GeForceNow SDK initialized: %d", (int32)GfnResult);
			const bool bIsRunningInTheCloud = GeForceNOWWrapper::Get().IsRunningInCloud();
			UE_LOGF(LogGeForceNow, Log, "GeForceNow IsRunningInCloud %d", bIsRunningInTheCloud ? 1 : 0);
		...
		{
			UE_LOGF(LogGeForceNow, Log, "GeForceNow SDK initialization failed: %d", (int32)GfnResult);
		}
#endif // NV_GEFORCENOW
	}
