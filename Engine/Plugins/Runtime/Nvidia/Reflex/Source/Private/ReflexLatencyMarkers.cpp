...
void FReflexLatencyMarkers::Tick(float DeltaTime)
{
	if (DisableLatencyMarkers == 0 && bEnabled && bFeatureSupport)
	{
		...
		LatencyStatus = NvAPI_D3D_GetLatency(FReflexModule::GetD3DDevice(), &LatencyResults);
		...
		if (LatencyStatus == NVAPI_OK)
		{
			...
			if (TotalLatencyUs != 0)
			{
				...
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageTotalLatencyMs: %f", AverageTotalLatencyMs);
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageGameLatencyMs: %f", AverageGameLatencyMs);
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageRenderLatencyMs: %f", AverageRenderLatencyMs);

				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageSimulationLatencyMs: %f", AverageSimulationLatencyMs);
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageRenderSubmitLatencyMs: %f", AverageRenderSubmitLatencyMs);
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AveragePresentLatencyMs: %f", AveragePresentLatencyMs);
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageDriverLatencyMs: %f", AverageDriverLatencyMs);
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageOSRenderQueueLatencyMs: %f", AverageOSRenderQueueLatencyMs);
				UE_LOGF(LogLatencyMarkers, VeryVerbose, "AverageGPURenderLatencyMs: %f", AverageGPURenderLatencyMs);
			}
		}
	}
}
...
void FReflexLatencyMarkers::SetInputSampleLatencyMarker(uint64 FrameNumber)
{
	if (DisableLatencyMarkers == 0 && bEnabled && bFeatureSupport)
	{
		...
		status = NvAPI_D3D_SetLatencyMarker(FReflexModule::GetD3DDevice(), &params);
	}
}
...
bool FReflexLatencyMarkers::Exec_Runtime(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FParse::Value(Cmd, TEXT("ReflexLatencyMarkers="), ReflexLatencyMarkers))
	{
		if (ReflexLatencyMarkers == "0")
		{
			...
			UE_LOGF(LogLatencyMarkers, Log, "Reflex Latency Markers: Off");
		}
		else if (ReflexLatencyMarkers == "1")
		{
			...
			UE_LOGF(LogLatencyMarkers, Log, "Reflex Latency Markers: On");
		}
