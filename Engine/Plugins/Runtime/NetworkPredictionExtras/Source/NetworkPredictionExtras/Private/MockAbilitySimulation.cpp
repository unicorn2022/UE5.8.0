...
void FMockAbilitySimulation::SimulationTick(const FNetSimTimeStep& TimeStep, const TNetSimInput<TMockAbilityBufferTypes>& Input, const TNetSimOutput<TMockAbilityBufferTypes>& Output)
{
	...
	if(bBlinkActivate)
	{
		...
		UE_LOGF(LogNetworkPrediction, Warning, "Invoking FMockAbilityBlinkActivateCue from sim");
	...
	if (LocalAux.BlinkWarmupLeft > 0)
	{
		...
		if (NewBlinkWarmupLeft <= 0)
		{
			...
			if (OwningActor->TeleportTo(DestLocation, LocalSync.Rotation))
			{
				...
				//UE_LOGF(LogNetworkPrediction, Warning, "Invoking FMockAbilityBlinkCue from sim. %ls - %ls (LocalSync.Location: %ls) ", *Input.Sync->Location.ToString(), *DestLocation.ToString(), *LocalSync.Location.ToString());
		...
		if (UKismetSystemLibrary::SphereTraceMultiForObjects(UpdatedComponent, Start, End, Radius, ObjectTypes, false, ActorsToIgnore, EDrawDebugTrace::None, OutHits, true))
		{
			for (FHitResult& Hit : OutHits)
			{
				if (UPrimitiveComponent* HitComponent = Hit.Component.Get())
				{
					...
					{
						UE_LOGF(LogNetworkPrediction, Warning, "[%d] %ls. PhysicsGun Adding Impulses! ImpactPoint: %ls", TimeStep.Frame, UpdatedComponent->GetOwnerRole() == ROLE_Authority ? TEXT("Server") : TEXT("Client"), *Hit.ImpactPoint.ToString());
	...
	if (LocalCmd.bSecondaryPressed && !(MockAbilityCVars::DisablePhysicsGunServer() && UpdatedComponent->GetOwnerRole() == ROLE_Authority))
	{
		...
		if (UKismetSystemLibrary::SphereOverlapComponents(UpdatedComponent, SpherePosition, SphereRadius, ObjectTypes, nullptr, ActorsToIgnore, OutComponents))
		{
			...
			{
				UE_LOGF(LogNetworkPrediction, Warning, "Hit: %ls", *GetPathNameSafe(HitComponent));
	...
	UE_LOGF(LogNetworkPrediction, Warning, "[%d] Adding Impulses! %ls", TimeStep.Frame, OwningActor->GetLocalRole() == ROLE_Authority ? TEXT("Server") : TEXT("Client"));
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (UPrimitiveComponent* Prim = TestActor->FindComponentByClass<UPrimitiveComponent>())
		{
			...
			{
				//UE_LOGF(LogMockNetworkSim, Warning, "Adding Impulses! %ls  %f / %f", OwningActor->GetLocalRole() == ROLE_Authority ? TEXT("Server") : TEXT("Client"), Prim->GetOwner()->GetActorLocation().Z, PhysVec.Z);
...
void UMockFlyingAbilityComponent::HandleCue(const FMockAbilityBlinkActivateCue& BlinkCue, const FNetSimCueSystemParamemters& SystemParameters)
{
	...
	UE_LOGF(LogNetworkPrediction, Display, "[%ls] BlinkActivatedCue! TimeMS: %d", *RoleStr, SystemParameters.TimeSinceInvocation);
	...
	{
		UE_LOGF(LogNetworkPrediction, Display, "  System Callbacks available!");

		SystemParameters.Callbacks->OnRollback.AddLambda([RoleStr, this]()
		{
			UE_LOGF(LogNetworkPrediction, Display, "  %ls BlinkActivatedCue Rollback!", *RoleStr);
...
void UMockFlyingAbilityComponent::HandleCue(const FMockAbilityBlinkCue& BlinkCue, const FNetSimCueSystemParamemters& SystemParameters)
{
	...
	UE_LOGF(LogNetworkPrediction, Display, "[%ls] BlinkCue! : <%f, %f, %f> - <%f, %f, %f>. ElapsedTimeMS: %d. Delta: %.3f", *RoleStr, BlinkCue.StartLocation.X, BlinkCue.StartLocation.Y, BlinkCue.StartLocation.Z,
	...
	if (SystemParameters.Callbacks)
	{
		...
		SystemParameters.Callbacks->OnRollback.AddLambda([RoleStr, this]()
		{
			UE_LOGF(LogNetworkPrediction, Display, "  %ls BlinkCue Rollback!", *RoleStr);
		});
	}
}
