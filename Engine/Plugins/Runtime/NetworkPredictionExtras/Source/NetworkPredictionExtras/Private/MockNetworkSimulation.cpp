...
void FMockNetworkSimulation::SimulationTick(const FNetSimTimeStep& TimeStep, const TNetSimInput<TMockNetworkSimulationBufferTypes>& Input, const TNetSimOutput<TMockNetworkSimulationBufferTypes>& Output)
{
	...
	{
		UE_LOGF(LogMockNetworkSim, Warning, "[%d] Output.Sync->Total: %f. Input.Cmd->InputValue: %f... %d", TimeStep.Frame, Output.Sync->Total, Input.Cmd->InputValue, TimeStep.StepMS);
	}
	...
	if (ForceMispredict)
	{
		...
		UE_LOGF(LogMockNetworkSim, Warning, "ForcingMispredict via CVar. Fudge=%.2f. NewTotal: %.2f", Fudge, Output.Sync->Total);
...
{
	UE_LOGF(LogMockNetworkSim, Display, "MockCue Handled! Value: %d", MockCue.RandomData);
}
