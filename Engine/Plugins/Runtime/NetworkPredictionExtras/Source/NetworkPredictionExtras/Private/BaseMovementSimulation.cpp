...
bool FBaseMovementSimulation::ResolvePenetration(const FVector& ProposedAdjustment, const FHitResult& Hit, const FQuat& NewRotationQuat) const
{
	...
	if (!Adjustment.IsZero() && UpdatedPrimitive)
	{
		...
		UE_LOGF(LogBaseMovement, Verbose, "ResolvePenetration: %ls.%ls at location %ls inside %ls.%ls at location %ls by %.3f (netmode: %d)",
			*ActorOwner->GetName(),
		...
		if (!bEncroached)
		{
			...
			UE_LOGF(LogBaseMovement, Verbose, "ResolvePenetration:   teleport by %ls", *Adjustment.ToString());
			return true;
		}
		else
		{
			...
			UE_LOGF(LogBaseMovement, Verbose, "ResolvePenetration:   sweep by %ls (success = %d)", *Adjustment.ToString(), bMoved);
			...
			if (!bMoved && SweepOutHit.bStartPenetrating)
			{
				...
				if (SecondMTD != Adjustment && !CombinedMTD.IsZero())
				{
					...
					UE_LOGF(LogBaseMovement, Verbose, "ResolvePenetration:   sweep by %ls (MTD combo success = %d)", *CombinedMTD.ToString(), bMoved);
				}
			}
			...
			if (!bMoved)
			{
				...
				if (!MoveDelta.IsZero())
				{
					...
					UE_LOGF(LogBaseMovement, Verbose, "ResolvePenetration:   sweep by %ls (adjusted attempt success = %d)", *(Adjustment + MoveDelta).ToString(), bMoved);
				}
			}	
