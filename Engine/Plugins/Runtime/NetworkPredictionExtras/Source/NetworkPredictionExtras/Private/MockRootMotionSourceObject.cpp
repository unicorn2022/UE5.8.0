...
void UMockRootMotionSourceClassMap::BuildClassMap_Internal()
{
	...
	//UE_LOGF(LogTemp, Warning, "UMockRootMotionSourceClassMap::BuildClassMap. Found %d classes.", Classes.Num());
	...
	//UE_LOGF(LogTemp, Warning, "Scanning Asset Registry for root motion source objects");
	...
	//UE_LOGF(LogTemp, Warning, "Found %d items in AssetDataList", AssetDataList.Num());
	...
	for (auto It = SourceList.CreateIterator(); It; ++It)
	{
		...
		//UE_LOGF(LogTemp, Warning, "%ls = %d (%ls)", *Subclass.ToString(), It.GetIndex(), LoadedClass ? TEXT("Loaded") : TEXT("Unloaded"));
	}
...
FMockRootMotionReturnValue UMockRootMotionSource_Montage::Step(const FMockRootMotionStepParameters& Parameters)
{
	...
	//UE_LOGF(LogTemp, Warning, "%.4f: Delta: %ls", EndPosition, *NewTransform.GetTranslation().ToString());
	return ReturnValue;
}
