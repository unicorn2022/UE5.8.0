...
{
#if PLATFORM_ANDROID && USE_ANDROID_JNI
	UE_LOGF(LogAndroidPermission, Log, "UAndroidPermissionFunctionLibrary::CheckPermission %ls (Android)", *permission);
	JNIEnv* env = FAndroidApplication::GetJavaEnv();
	...
	return bResult;
#else
	UE_LOGF(LogAndroidPermission, Log, "UAndroidPermissionFunctionLibrary::CheckPermission (Else)");
	return false;
#endif
}
...
{
#if PLATFORM_ANDROID && USE_ANDROID_JNI
	UE_LOGF(LogAndroidPermission, Log, "UAndroidPermissionFunctionLibrary::AcquirePermissions");
	JNIEnv* env = FAndroidApplication::GetJavaEnv();
...
#else
	UE_LOGF(LogAndroidPermission, Log, "UAndroidPermissionFunctionLibrary::AcquirePermissions(%ls) (Android)", *(FString::Join(permissions, TEXT(","))));
	return UAndroidPermissionCallbackProxy::GetInstance();
#endif
}
