...
UReplicationGraphNode_AlwaysRelevant_ForConnection* UBasicReplicationGraph::GetAlwaysRelevantNodeForConnection(UNetConnection* Connection)
{
	...
	if (Connection)
	{
		if (FConnectionAlwaysRelevantNodePair* Pair = AlwaysRelevantForConnectionList.FindByKey(Connection))
		{
			...
			{
				UE_LOGF(LogNet, Warning, "AlwaysRelevantNode for connection %ls is null.", *GetNameSafe(Connection));
			}
		}
		else
		{
			UE_LOGF(LogNet, Warning, "Could not find AlwaysRelevantNode for connection %ls. This should have been created in UBasicReplicationGraph::InitConnectionGraphNodes.", *GetNameSafe(Connection));
		}
	}
	else
	{
		...
		UE_LOGF(LogNet, Warning, "Actor: bOnlyRelevantToOwner is set but does not have an owning Netconnection. It will not be replicated");
	}
