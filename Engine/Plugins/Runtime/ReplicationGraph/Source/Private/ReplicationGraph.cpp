...
void UpdateActorConnectionCounter(AActor* InActor, UNetConnection* InConnection, TMap<FActorConnectionPair, int32>& Counter)
{
	...
	if (DebugActorConnectionPair.Actor.IsValid() == false)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "Setting WARN Pair: %ls - %ls", *GetPathNameSafe(InActor), *InConnection->Describe());
...
void LogMoreInfoOnIsActorValidFailure(const FActorRepListType& In)
{
	...
	if (CVar_RepGraph_LogDebugInfoPeriod > 0 && ((CurrentTime - LastTimeLoggedMoreInfo) > double(CVar_RepGraph_LogDebugInfoPeriod)))
	{
		...
		if (Actor)
		{
			...
			UE_LOGF(LogReplicationGraph, Error, "Actor not valid for replication (BeingDestroyed:%d) (IsValid:%d) (Unreachable:%d) (TearOff:%d)! Actor = %ls",
				Actor->IsActorBeingDestroyed(), IsValid(Actor), Actor->IsUnreachable(), Actor->GetTearOff(),
				*Actor->GetFullName());
#if UE_ACTOR_REPLIST_TYPE_EXTRA_SAFETY
			UE_LOGF(LogReplicationGraph, Error, "More info about invalid actor '%ls': Owner='%ls', OuterPackage='%ls'",
		...
		{
#if UE_ACTOR_REPLIST_TYPE_EXTRA_SAFETY
			UE_LOGF(LogReplicationGraph, Error, "Invalid actor pointer detected during replication: Name='%ls', Owner='%ls', OuterPackage='%ls'",
				*In.ActorName.ToString(), *In.OwnerName.ToString(), *In.OuterPackageName.ToString()
				);
#else
			UE_LOGF(LogReplicationGraph, Error, "Invalid actor pointer detected during replication");
#endif
		}

		UE_LOGF(LogReplicationGraph, Log, "Invalid actor found at the last moment, printing (game-specific) current routing:");
...
void UReplicationGraph::BeginDestroy()
{
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "%d actors were not removed from ReplicationGraph", ActiveNetworkActors.Num());

		for (AActor* Actor : ActiveNetworkActors)
		{
			UE_LOGF(LogReplicationGraph, Log, "  Leaked actor: %ls", *GetFullNameSafe(Actor));
		}
	}
...
UNetReplicationGraphConnection* UReplicationGraph::FindConnectionManager(UNetConnection* NetConnection) const
{
	...
	if (NetConnection->GetUChildConnection() != nullptr)
	{
		...
		UE_LOGF(LogReplicationGraph, Verbose, "UReplicationGraph::FindOrAddConnectionManager was called with a child connection, redirecting to parent");
...
void UReplicationGraph::AddClientConnection(UNetConnection* NetConnection)
{
	...
	if (NetConnection->GetUChildConnection() != nullptr)
	{
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraph::AddClientConnection was called with a child connection, dropping.");
		return;
	}
...
void UReplicationGraph::RemoveClientConnection(UNetConnection* NetConnection)
{
	...
	if (NetConnection->GetUChildConnection() != nullptr)
	{
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraph::RemoveClientConnection was called with a child connection!");
		return;
	}
	...
	if (!ActiveGraphConnectionRemoved && !PendingGraphConnectionRemoved)
	{
		...
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraph::RemoveClientConnection could not find connection %ls in Connections (%d) or PendingConnections (%d) lists", *GetNameSafe(NetConnection), Connections.Num(), PendingConnections.Num());
	}
}
...
void UReplicationGraph::RemoveNetworkActor(AActor* Actor)
{
	...
	if (UNLIKELY(!IsValid(Actor)))
	{
		UE_LOGF(
			LogReplicationGraph,
			Warning,
			"Actor should still be valid during removal from networking: %ls",
	...
	if (UNLIKELY(!bWasInActiveSet))
	{
		UE_LOGF(
			LogReplicationGraph,
			Verbose,
			"Removing Actor that was not in the active set: %ls",
			*GetFullNameSafe(Actor));
	}
	else
	{
		...
		if (Actor->GetTearOff() == false)
		{
			UE_CLOGF(
				...
				LogReplicationGraph,
				Display,
				"UReplicationGraph::RemoveNetworkActor %ls",
				*GetFullNameSafe(Actor));
	...
	if (UNLIKELY(!bWasInActiveSet && bWasInReplicationInfoMap))
	{
		...
		UE_LOGF(
			LogReplicationGraph,
			Warning,
			"Actor replication info found for actor that was not in the active set: %ls",
			*GetFullNameSafe(Actor));
	}
}
...
void UReplicationGraph::FlushNetDormancy(AActor* Actor, bool bWasDormInitial)
{
	...
	{
		UE_CLOGF(CVar_RepGraph_LogActorAdd > 0, LogReplicationGraph, Display, "UReplicationGraph::FlushNetDormancy called on %ls but not fully initiailized yet. Discarding.", *Actor->GetPathName());
		return;
	}

	if (IsActorValidForReplication(Actor) == false)
	{
		UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Display, "UReplicationGraph::FlushNetDormancy called on %ls. Ignored since actor is destroyed or about to be", *Actor->GetPathName());
		return;
	}
	...
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Display, "UReplicationGraph::FlushNetDormancy %ls. Old WantsToBeDormant: %d. New WantsToBeDormant: %d", *Actor->GetPathName(), GlobalInfo.bWantsToBeDormant, bNewWantsToBeDormant);

	if (GlobalInfo.bWantsToBeDormant != bNewWantsToBeDormant)
	{
		UE_LOGF(LogReplicationGraph, Verbose, "UReplicationGraph::FlushNetDormancy %ls. WantsToBeDormant is changing (%d -> %d) from a Flush! We expect NotifyActorDormancyChange to be called first.", *Actor->GetPathName(), (bool)GlobalInfo.bWantsToBeDormant, bNewWantsToBeDormant);
		GlobalInfo.bWantsToBeDormant = Actor->NetDormancy > DORM_Awake;
	}
...
void UReplicationGraph::NotifyActorFullyDormantForConnection(AActor* Actor, UNetConnection* Connection)
{
	...
	if (Connection->GetUChildConnection() != nullptr)
	{
		...
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraph::NotifyActorFullyDormantForConnection was called for a child connection %ls on actor %ls", *Connection->GetName(), *Actor->GetName());
...
void UReplicationGraph::NotifyActorDormancyChange(AActor* Actor, ENetDormancy OldDormancyState)
{
	...
	if (!ActorRepInfo)
	{
		UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Display, "UReplicationGraph::NotifyActorDormancyChange %ls. Ignoring change since actor is not registered yet.", *Actor->GetPathName());
		return;
	}

	if (IsActorValidForReplication(Actor) == false)
	{
		UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Display, "UReplicationGraph::NotifyActorDormancyChange %ls. Ignoring change since actor is destroyed or about to be.", *Actor->GetPathName());
		return;
	}

	ENetDormancy CurrentDormancy = Actor->NetDormancy;

	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Display, "UReplicationGraph::NotifyActorDormancyChange %ls. Old WantsToBeDormant: %d. New WantsToBeDormant: %d", *Actor->GetPathName(), ActorRepInfo->bWantsToBeDormant, CurrentDormancy > DORM_Awake ? 1 : 0);
...
void UReplicationGraph::NotifyActorRenamed(AActor* Actor, UObject* PreviousOuter, FName PreviousName)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraph::NotifyActorRenamed %ls. Actor invalid for replication.", *GetFullNameSafe(Actor));
		return;
	}
...
int32 UReplicationGraph::ServerReplicateActors(float DeltaSeconds)
{
	...
	for (UNetReplicationGraphConnection* ConnectionManager: Connections)
	{
...

		{
			...
			{
				UE_LOGF(LogReplicationGraph, Verbose, "No Replication Lists were returned for connection");
				continue;
			}
...

		{
			...
			// ------------------------------------------			
			{
				...
				for (auto MapIt = ConnectionActorInfoMap.CreateChannelIterator(); MapIt; ++MapIt)
				{
					...
					if (Channel->Closing)
					{
						UE_LOGF(LogReplicationGraph, Verbose, "NET_ReplicateActors_LookForNonRelevantChannels (key) Channel %ls is closing. Skipping.", *Channel->Describe());
						continue;
					}
					else if (ConnectionActorInfo.Channel->Closing)
					{
						UE_LOGF(LogReplicationGraph, Verbose, "NET_ReplicateActors_LookForNonRelevantChannels (value) Channel %ls is closing. Skipping.", *ConnectionActorInfo.Channel->Describe());
						continue;
					}
					...
					if (ConnectionActorInfo.ActorChannelCloseFrameNum > 0 && ConnectionActorInfo.ActorChannelCloseFrameNum <= FrameNum)
					{
							...
							FrameNum, !!ConnectionActorInfo.bTearOff, !!ConnectionActorInfo.bDormantOnConnection, *(Channel->Describe()), static_cast<int32>(NetConnection->GetConnectionState())))
						{
							...
							//UE_CLOGF(DebugConnection, LogReplicationGraph, Display, "Closing Actor Channel:0x%x 0x%X0x%X, %ls %d <= %d", ConnectionActorInfo.Channel, Actor, NetConnection, *GetNameSafe(ConnectionActorInfo.Channel->Actor), ConnectionActorInfo.ActorChannelCloseFrameNum, FrameNum);
							if (RepGraphConditionalActorBreakpoint(Actor, NetConnection))
							{
								UE_LOGF(LogReplicationGraph, Display, "Closing Actor Channel due to timeout: %ls. %d <= %d (%ls)", *(ConnectionActorInfo.Channel->Describe()), ConnectionActorInfo.ActorChannelCloseFrameNum, FrameNum, *NetConnection->Describe());
							}
	...
	if (CVar_RepGraph_PrintTrackClassReplication)
	{
		...
		UE_LOGF(LogReplicationGraph, Display, "Changed Classes: %ls", *ChangeClassAccumulator.BuildString());
		UE_LOGF(LogReplicationGraph, Display, "No Change Classes: %ls", *NoChangeClassAccumulator.BuildString());
	}
#endif
...
void UReplicationGraph::ReplicateActorListsForConnections_Default(UNetReplicationGraphConnection* ConnectionManager, FGatheredReplicationActorLists& GatheredReplicationListsForConnection, FNetViewerArray& Viewers)
{
	...
	// --------------------------------------------------------------------------------------------------------------
	{
		...
		for (const FActorRepListType& Actor : Actors)
		{
			...
			if (RepGraphConditionalActorBreakpoint(Actor, NetConnection))
			{
				UE_LOGF(LogReplicationGraph, Display, "UReplicationGraph PrioritizeActor: %ls", *Actor->GetName());
			}
			...
			//UE_CLOGF(DebugConnection, LogReplicationGraph, Display, "0x%X0x%X ConnectionData.ActorChannelCloseFrameNum=%d on %d", Actor, NetConnection, ConnectionData.ActorChannelCloseFrameNum, FrameNum);
	...
	if (bDoCulledOnConnectionCount)
	{
		UE_LOGF(LogReplicationGraph, Display, "Dormant Culled classes: %ls", *DormancyClassAccumulator.BuildString());
		UE_LOGF(LogReplicationGraph, Display, "Dist Culled classes: %ls", *DistanceClassAccumulator.BuildString());
		UE_LOGF(LogReplicationGraph, Display, "Saturated Connections: %d", GNumSaturatedConnections);
		UE_LOGF(LogReplicationGraph, Display, "");

		UE_LOGF(LogReplicationGraph, Display, "Gathered Actors: %d, PrioritizedActors: %d", NumGatheredActorsOnConnection, NumPrioritizedActorsOnConnection);
		UE_LOGF(LogReplicationGraph, Display, "Connection Loaded Streaming Levels: %d", NetConnection->ClientVisibleLevelNames.Num());
	}
#endif // WITH_SERVER_CODE
}
...
void UReplicationGraph::ReplicateActorListsForConnections_FastShared(UNetReplicationGraphConnection* ConnectionManager, FGatheredReplicationActorLists& GatheredReplicationListsForConnection, FNetViewerArray& Viewers)
{
	...
	for (int32 i = 0; i < Actors.Num(); ++i)
	{
		...
		if (RepGraphConditionalActorBreakpoint(Actor, NetConnection))
		{
			UE_LOGF(LogReplicationGraph, Display, "UReplicationGraph FastShared Path Replication: %ls", *Actor->GetName());
		}
	...
	if (GlobalActorInfo.FastSharedReplicationInfo->LastAttemptBuildFrameNum < FrameNum)
	{
		...
		if (GlobalActorInfo.Settings.FastSharedReplicationFunc == nullptr)
		{
			...
			if (WarnedClasses.Contains(FObjectKey(Actor->GetClass())) == false)
			{
				...
				UE_LOGF(LogReplicationGraph, Warning, "::ReplicateSingleActor_FastShared called on %ls (%ls) when it doesn't have a FastSharedReplicationFunc defined, skipping actor. This is ineffecient.", *GetPathNameSafe(Actor), *Actor->GetClass()->GetName());
			}
#endif
			return 0;
		}
...
int64 UReplicationGraph::ReplicateSingleActor(const FActorRepListType& Actor, FConnectionReplicationActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalActorInfo, FPerConnectionActorInfoMap& ConnectionActorInfoMap, UNetReplicationGraphConnection& ConnectionManager, const uint32 FrameNum)
{
	...
	if (RepGraphConditionalActorBreakpoint(Actor, NetConnection))
	{
		UE_LOGF(LogReplicationGraph, Display, "UReplicationGraph::ReplicateSingleActor: %ls. NetConnection: %ls", *Actor->GetName(), *NetConnection->Describe());
	}
	...
	if (bOpenActorChannel)
	{
		...
		//UE_LOGF(LogReplicationGraph, Display, "Created Actor Channel:0x%x 0x%X0x%X, %d", ActorInfo.Channel, Actor, NetConnection, FrameNum);
	...
	if (ListGatherer.NumLists() > 0)
	{
		...
		for (AActor* DependentActor : DependentActors)
		{
			...
			//UE_LOGF(LogReplicationGraph, Display, "DependentActor %ls %ls. NextReplicationFrameNum: %d. FrameNum: %d. ForceNetUpdateFrame: %d. LastRepFrameNum: %d.", *DependentActor->GetPathName(), *NetConnection->GetName(), DependentActorConnectionInfo.NextReplicationFrameNum, FrameNum, DependentActorGlobalData.ForceNetUpdateFrame, DependentActorConnectionInfo.LastRepFrameNum);
...
void UReplicationGraph::UpdateActorChannelCloseFrameNum(AActor* Actor, FConnectionReplicationActorInfo& ConnectionData, const FGlobalActorReplicationInfo& GlobalData, const uint32 FrameNum, UNetConnection* NetConnection) const
{
	if (RepGraphConditionalActorBreakpoint(Actor, NetConnection))
	{
		UE_LOGF(LogReplicationGraph, Display, "UReplicationGraph::UpdateActorChannelCloseFrameNum: %ls. Channel: %ls FrameNum: %d ActorChannelFrameTimeout: %d.", *Actor->GetName(), *(ConnectionData.Channel ? ConnectionData.Channel->Describe() : FString(TEXT("None"))), FrameNum, GlobalData.Settings.ActorChannelFrameTimeout);
	}
...
bool UReplicationGraph::ProcessRemoteFunction(class AActor* Actor, UFunction* Function, void* Parameters, FOutParmRec* OutParms, FFrame* Stack, class UObject* SubObject )
{
	...
	{
		UE_LOGF(LogReplicationGraph, Display, "UReplicationGraph::ProcessRemoteFunction: %ls. Function: %ls.", *GetNameSafe(Actor), *GetNameSafe(Function));
	}

	if (IsActorValidForReplication(Actor) == false || Actor->IsActorBeingDestroyed())
	{
		UE_LOGF(LogReplicationGraph, Display, "UReplicationGraph::ProcessRemoteFunction: Actor %ls destroyed or not ready! Function: %ls.", *GetNameSafe(Actor), *GetNameSafe(Function));
		return true;
	}
	...
	if (!ClassCache)
	{
		UE_LOGF(LogReplicationGraph, Warning, "ClassNetCache empty, not calling %ls::%ls", *Actor->GetName(), *Function->GetName());
		return true;
	}
	...
	if ( !FieldCache )
	{
		UE_LOGF(LogReplicationGraph, Warning, "FieldCache empty, not calling %ls::%ls", *Actor->GetName(), *Function->GetName());
		return true;
	}
	...
	if ((Function->FunctionFlags & FUNC_NetMulticast))
	{
		...
		for (UNetReplicationGraphConnection* Manager : Connections)
		{
			...
			//UE_CLOGF(ConnectionActorInfo.Channel == nullptr, LogReplicationGraph, Display, "Null channel on %ls for %ls", *GetPathNameSafe(Actor), *GetNameSafe(Function));
			if (ConnectionActorInfo.Channel == nullptr && (RPC_Multicast_OpenChannelForClass.GetChecked(Actor->GetClass()) == true))
			{
				...
				// if (Actor->NetDormancy > DORM_Awake)
				{
					...
					if (bShouldOpenChannel)
					{
						...
						{
							UE_LOGF(LogReplicationGraph, Warning, "Multicast RPC opening channel for bOnlyRelevantToOwner actor, check RPC_Multicast_OpenChannelForClass: Actor: %ls Target: %ls Function: %ls", *GetNameSafe(Actor), *GetNameSafe(TargetObj), *GetNameSafe(Function));
	...
	{
		UE_LOGF(LogNet, Warning, "UReplicationGraph::ProcessRemoteFunction: No owning connection for actor %ls. Function %ls will not be processed.", *Actor->GetName(), *Function->GetName());
	}
#endif // WITH_SERVER_CODE
...
void UReplicationGraph::SetActorDiscoveryBudget(int32 ActorDiscoveryBudgetInKBytesPerSec)
{
	...
	{
		ActorDiscoveryMaxBitsPerFrame = 0;
		UE_LOGF(LogReplicationGraph, Display, "SetActorDiscoveryBudget disabled the ActorDiscovery budget.");
		return;
	}

	if (NetDriver == nullptr)
	{
		UE_LOGF(LogReplicationGraph, Warning, "SetActorDiscoveryBudget ignored since NetDriver was not initialized.");
		return;
	}
	...
	UE_LOGF(LogReplicationGraph, Display, "SetActorDiscoveryBudget set to %d kBps (%d bits per network tick).", ActorDiscoveryBudgetInKBytesPerSec, ActorDiscoveryMaxBitsPerFrame);
}
...
void UReplicationGraph::SetActorDestructionInfoToIgnoreDistanceCulling(AActor* DestroyedActor)
{
	...
	{
		UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Warning, "SetActorDestructionInfoToIgnoreDistanceCulling ignored for %ls. No NetGUID assigned to the actor", *GetNameSafe(DestroyedActor));
		return;
	}
...
void UNetReplicationGraphConnection::NotifyActorChannelAdded(AActor* Actor, class UActorChannel* Channel)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Display, "::NotifyActorChannelAdded. %ls. Channel: %ls. Connection: %ls", *GetPathNameSafe(Actor), *Channel->Describe(), *Channel->Connection->Describe());
	}
	...
	{
		UE_LOGF(LogReplicationGraph, Log, "::NotifyActorChannelAdded. Fixing up stale channel reference Old: %ls New: %ls", *ActorInfo.Channel->Describe(), *Channel->Describe());
...
void UNetReplicationGraphConnection::NotifyActorChannelCleanedUp(UActorChannel* Channel)
{
	if (Channel)
	{
		...
		if (ActorInfo)
		{
			...
			{
				UE_LOGF(LogReplicationGraph, Log, "::NotifyActorChannelCleanedUp. CleanUp for stale channel reference Old: %ls New: %ls", *Channel->Describe(), *DescribeSafe(ActorInfo->Channel));
			}
...
void UNetReplicationGraphConnection::NotifyAddDestructionInfo(FActorDestructionInfo* DestructInfo)
{
	...
	//UE_LOGF(LogReplicationGraph, Display, "::NotifyAddDestructionInfo. Connection: %ls. DestructInfo: %ls. NewTotal: %d", *NetConnection->Describe(), *DestructInfo->PathName, PendingDestructInfoList.Num());
#endif // WITH_SERVER_CODE
}
...
void UNetReplicationGraphConnection::NotifyAddDormantDestructionInfo(AActor* Actor)
{
	...
	if (Actor && NetConnection && NetConnection->Driver && NetConnection->Driver->GetNetGuidCache())
	{
		...
		if (Level && !Level->IsPersistentLevel())
		{
			...
			{
				UE_LOGF(LogReplicationGraph, Verbose, "NotifyAddDormantDestructionInfo skipping actor [%ls] because streaming level [%ls] is no longer visible.", *GetNameSafe(Actor), *StreamingLevelName.ToString());
				return;
			}
		}
...
void UNetReplicationGraphConnection::NotifyRemoveDestructionInfo(FActorDestructionInfo* DestructInfo)
{
	...
	while(true)
	{
		...
		if (!ensureMsgf(DuplicateIdx == INDEX_NONE, TEXT("::NotifyRemoveDestructionInfo list STILL contains DestructInfo: 0x%llX (%s)"), (int64)DestructInfo, *DestructInfo->PathName))
		{
...
bool FStreamingLevelActorListCollection::RemoveActor(const FNewReplicatedActorInfo& ActorInfo, bool bWarnIfNotFound, UObject* Outer)
{
	...
	for (FStreamingLevelActors& StreamingList : StreamingLevelLists)
	{
		if (StreamingList.StreamingLevelName == ActorInfo.StreamingLevelName)
		{
			...
			{
				UE_LOGF(LogReplicationGraph, Warning, "Attempted to remove %ls from list %ls but it was not found. (StreamingLevelName == %ls)", *GetActorRepListTypeDebugString(ActorInfo.Actor), *GetPathNameSafe(Outer), *ActorInfo.StreamingLevelName.ToString() );
			}
...
void FStreamingLevelActorListCollection::Gather(const FConnectionGatherActorListParameters& Params) const
{
	for (const FStreamingLevelActors& StreamingList : StreamingLevelLists)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Verbose, "Level Not Loaded %ls. (Client has %d levels loaded)", *StreamingList.StreamingLevelName.ToString(), Params.ClientVisibleLevelNamesRef.Num());
		}
	}
}
...
{
	UE_CLOGF(CVar_RepGraph_LogActorAdd>0, LogReplicationGraph, Display, "UReplicationGraphNode_ActorList::NotifyAddNetworkActor %ls on %ls.", *ActorInfo.Actor->GetFullName(), *GetPathName());
...
{
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_ActorList::NotifyRemoveNetworkActor %ls on %ls.", *ActorInfo.Actor->GetFullName(), *GetPathName());
	...
	if (ActorInfo.StreamingLevelName == NAME_None)
	{
		...
		UE_CLOGF(!bRemovedSomething && bWarnIfNotFound, LogReplicationGraph, Warning, "Attempted to remove %ls from list %ls but it was not found. (StreamingLevelName == NAME_None)", *GetActorRepListTypeDebugString(ActorInfo.Actor), *GetFullName());
...
{
	UE_CLOGF(CVar_RepGraph_LogActorAdd>0, LogReplicationGraph, Display, "UReplicationGraphNode_ActorListFrequencyBuckets::NotifyAddNetworkActor %ls on %ls.", *ActorInfo.Actor->GetFullName(), *GetPathName());
...
{
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_ActorListFrequencyBuckets::NotifyRemoveNetworkActor %ls on %ls.", *ActorInfo.Actor->GetFullName(), *GetPathName());
	...
	if (ActorInfo.StreamingLevelName == NAME_None)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "Attempted to remove %ls from list %ls but it was not found. (StreamingLevelName == NAME_None)", *GetActorRepListTypeDebugString(ActorInfo.Actor), *GetFullName());
		}
	}
	else
	{
...
void UReplicationGraphNode_ActorListFrequencyBuckets::CheckRebalance()
{
	...
	{
		//UE_LOGF(LogReplicationGraph, Display, "Rebalancing %ls for %d buckets (%d total actors)", *GetPathName(), DesiredNumBuckets, TotalNumNonStreamingActors);
...
void UReplicationGraphNode_DynamicSpatialFrequency::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
...

	{
		...
		for (const FDynamicSpatialFrequency_SortedItem& Item : SortedReplicationList)
		{
			...
			if (RepGraphConditionalActorBreakpoint(Actor, NetConnection))
			{
				UE_LOGF(LogReplicationGraph, Display, "UReplicationGraphNode_DynamicSpatialFrequency_Connection Replication: %ls", *Actor->GetName());
			}
			...
			{
				//UE_LOGF(LogReplicationGraph, Display, "[%d] Opportunistic Skip. %ls. OpportunisticLoadBalanceQuota: %d (%d, %d)", FrameNum, *Actor->GetName(), OpportunisticLoadBalanceQuota, NumExpectedReplicationsThisFrame, NumExpectedReplicationsNextFrame);
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "Uncapped bandwidth usage of UReplicationGraphNode_DynamicSpatialFrequency = %lld bits -> %lld bytes -> %.2f KBytes/sec", BitsWritten, (BitsWritten+7) >> 3,  ((float)((BitsWritten+7)>>3)/1024.f) * ((float)GraphGlobals->ReplicationGraph->NetDriver->GetNetServerMaxTickRate()));
		}
	}
#endif // WITH_SERVER_CODE
}
...
void UReplicationGraphNode_DynamicSpatialFrequency::CalcFrequencyForActor(AActor* Actor, UReplicationGraph* RepGraph, const UNetReplicationGraphConnection& RepGraphConnection, FGlobalActorReplicationInfo& GlobalInfo, FPerConnectionActorInfoMap& ConnectionMap, FSettings& MySettings, const FNetViewerArray& Viewers, const uint32 FrameNum, int32 ExistingItemIndex)
{
	...
	if (RepGraphConditionalActorBreakpoint(Actor, NetConnection))
	{
		UE_LOGF(LogReplicationGraph, Display, "UReplicationGraphNode_DynamicSpatialFrequency::CalcFrequencyForActor: %ls", *Actor->GetName());
	}
	...
	UE_LOGF(LogReplicationGraph, VeryVerbose, "UReplicationGraphNode_DynamicSpatialFrequency::CalcFrequencyForActor: Using viewer %ls for spatical determination for actor %ls", ((LowestDistanceViewer->Connection != nullptr) ? *LowestDistanceViewer->Connection->GetName() : TEXT("INVALID")), *Actor->GetName());
		...
		if (DotP <= ZoneInfo.MinDotProduct)
		{
			...
			// --------------------------------------------------------------------------------------------------------
			{
				...
				if (CullDistSq <= 0.f)
				{
					...
					UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_DynamicSpatialFrequency::GatherActors: %ls has cull distance of 0 (connection %f | global %f). Removing from node",
			...
			else if (FramesTillReplicate == 0)
			{
				...
				{
					//UE_LOGF(LogReplicationGraph, Display, "  THIS[%d]: %ls. Def: %d (%d) Fast: %d (%d)", FrameNum, *Actor->GetName(), ConnectionInfo.NextReplicationFrameNum, ConnectionInfo.ReplicationPeriodFrame, ConnectionInfo.FastPath_NextReplicationFrameNum, ConnectionInfo.FastPath_ReplicationPeriodFrame);
			...
			else if (FramesTillReplicate == 1)
			{
				...
				if (ReplicatesEveryFrame(ConnectionInfo, EnableFastPath) == false)
				{
					...
					//UE_LOGF(LogReplicationGraph, Display, "  NEXT[%d]: %ls. Def: %d (%d) Fast: %d (%d)", FrameNum, *Actor->GetName(), ConnectionInfo.NextReplicationFrameNum, ConnectionInfo.ReplicationPeriodFrame, ConnectionInfo.FastPath_NextReplicationFrameNum, ConnectionInfo.FastPath_ReplicationPeriodFrame);
				}
	...
	UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_DynamicSpatialFrequency::CalcFrequencyForActor: %ls was not placed in any valid zone. Viewer: %ls DotP: %.2f ", *Actor->GetName(), ((LowestDistanceViewer->Connection != nullptr) ? *LowestDistanceViewer->Connection->GetName() : TEXT("INVALID")), DotP);
	RemoveExistingItem();
#endif // WITH_SERVER_CODE
}
...
{
	UE_LOGF(LogReplicationGraph, Log, "SetNumFramesUntilObsolete setting is now %u", InNumFrames);
...
void UReplicationGraphNode_ConnectionDormancyNode::ConditionalGatherDormantActorsForConnection(FActorRepListRefView& ConnectionList, const FConnectionGatherActorListParameters& Params, FActorRepListRefView* RemovedList)
{
	...
	for (int32 idx = ConnectionList.Num()-1; idx >= 0; --idx)
	{
		...
		if (ConnectionActorInfo.bDormantOnConnection)
		{
			...
			UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Display, "GRAPH_DORMANCY: Actor %ls is Dormant on %ls. Removing from list. (%d elements left)", *Actor->GetPathName(), *GetName(), ConnectionList.Num());
...
void UReplicationGraphNode_ConnectionDormancyNode::OnClientVisibleLevelNameAdd(FName LevelName, UWorld* World)
{
	...
	if (!RemoveList)
	{
		UE_LOGF(LogReplicationGraph, Warning, ":OnClientVisibleLevelNameAdd called on %ls but there is no RemoveList. How did this get bound in the first place?. Level: %ls", *GetPathName(), *LevelName.ToString());
		return;
	}
	...
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails, LogReplicationGraph, Display, "::OnClientVisibleLevelNameadd %ls. LevelName: %ls.", *GetPathName(), *LevelName.ToString());
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails, LogReplicationGraph, Display, "    CurrentAddList: %ls", *AddList->ReplicationActorList.BuildDebugString());
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails, LogReplicationGraph, Display, "    RemoveList: %ls", *RemoveList->ReplicationActorList.BuildDebugString());
...
void UReplicationGraphNode_DormancyNode::AddDormantActor(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
	...
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0 && ConnectionNodes.Num() > 0, LogReplicationGraph, Display, "GRAPH_DORMANCY: AddDormantActor %ls on %ls. Adding to %d connection nodes.", *ActorInfo.Actor->GetPathName(), *GetName(), ConnectionNodes.Num());
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_DormancyNode already bound to dormancyflush for Actor %ls", *GetPathNameSafe(ActorInfo.GetActor()));
	}
}
...
void UReplicationGraphNode_DormancyNode::RemoveDormantActor(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo)
{
	...
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_DormancyNode::RemoveDormantActor %ls on %ls. (%d connection nodes). ChildNodes: %d", *GetNameSafe(ActorInfo.Actor), *GetPathName(), ConnectionNodes.Num(), AllChildNodes.Num());
...
UReplicationGraphNode_ConnectionDormancyNode* UReplicationGraphNode_DormancyNode::CreateConnectionNode(const FConnectionGatherActorListParameters& Params)
{
	...
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0, LogReplicationGraph, Display, "GRAPH_DORMANCY: First time seeing connection %ls in node %ls. Created ConnectionDormancyNode %ls.", *Params.ConnectionManager.GetName(), *GetName(), *ConnectionNode->GetName());
...
void UReplicationGraphNode_DormancyNode::OnActorDormancyFlush(FActorRepListType Actor, FGlobalActorReplicationInfo& GlobalInfo)
{
	...
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails > 0 && ConnectionNodes.Num() > 0, LogReplicationGraph, Display, "GRAPH_DORMANCY: Actor %ls Flushed Dormancy. %ls. Refreshing all %d connection nodes.", *Actor->GetPathName(), *GetName(), ConnectionNodes.Num());
...
{
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_Simple2DSpatializationLeaf::RemoveStaticActor %ls on %ls", *ActorInfo.Actor->GetPathName(), *GetPathName());
...
{
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_Simple2DSpatializationLeaf::RemoveDynamicActor %ls on %ls", *ActorInfo.Actor->GetPathName(), *GetPathName());
...
{
	UE_CLOGF(CVar_RepGraph_LogNetDormancyDetails>0, LogReplicationGraph, Display, "UReplicationGraphNode_GridCell::OnNetDormancyChange. %ls on %ls. Old: %d, New: %d", *Actor->GetPathName(), *GetPathName(), NewValue, OldValue);
...
{
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_GridSpatialization2D::AddActor_Dormancy %ls on %ls", *ActorInfo.Actor->GetFullName(), *GetPathName());
...
{
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_GridSpatialization2D::RemoveActor_Static %ls on %ls", *ActorInfo.Actor->GetFullName(), *GetPathName());
...
{
	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraphNode_GridSpatialization2D::RemoveActor_Dormancy %ls on %ls", *ActorInfo.Actor->GetFullName(), *GetPathName());
...
void UReplicationGraphNode_GridSpatialization2D::AddActorInternal_Dynamic(const FNewReplicatedActorInfo& ActorInfo)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "Always relevant actor being added to spatialized graph node. %ls", *GetNameSafe(ActorInfo.Actor));
		return;
	}
#endif

	UE_CLOGF(CVar_RepGraph_LogActorRemove>0, LogReplicationGraph, Display, "UReplicationGraph::AddActorInternal_Dynamic %ls", *ActorInfo.Actor->GetFullName());
...
void UReplicationGraphNode_GridSpatialization2D::AddActorInternal_Static(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo, bool bDormancyDriven)
{
	...
	if (Actor->IsActorInitialized() == false)
	{
		...
		for (int32 idx=PendingStaticSpatializedActors.Num()-1; idx >= 0; --idx)
		{
			...
			{
				UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_GridSpatialization2D::AddActorInternal_Static was called on %ls when it was already in the PendingStaticSpatializedActors list!", *Actor->GetPathName());
				return;
			}
		}
...
void UReplicationGraphNode_GridSpatialization2D::AddActorInternal_Static_Implementation(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo, bool bDormancyDriven)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Display, "UReplicationGraphNode_GridSpatialization2D::AddActorInternal_Static placing %ls into static grid at %ls", *Actor->GetPathName(), *ActorRepInfo.WorldLocation.ToString());
	}
...
void UReplicationGraphNode_GridSpatialization2D::RemoveActorInternal_Dynamic(const FNewReplicatedActorInfo& ActorInfo)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_Simple2DSpatialization::RemoveActorInternal_Dynamic attempted remove %ls from streaming dynamic list but it was not there.", *GetActorRepListTypeDebugString(ActorInfo.Actor));
		if (StaticSpatializedActors.Remove(ActorInfo.Actor) > 0)
		{
			UE_LOGF(LogReplicationGraph, Warning, "   It was in StaticSpatializedActors!");
		}
	}
}
...
void UReplicationGraphNode_GridSpatialization2D::RemoveActorInternal_Static(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& ActorRepInfo, bool bWasAddedAsDormantActor)
{
	if (StaticSpatializedActors.Remove(ActorInfo.Actor) <= 0)
	{
		...
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_Simple2DSpatialization::RemoveActorInternal_Static attempted remove %ls from static list but it was not there.", *GetActorRepListTypeDebugString(ActorInfo.Actor));
		if(DynamicSpatializedActors.Remove(ActorInfo.Actor) > 0)
		{
			UE_LOGF(LogReplicationGraph, Warning, "   It was in DynamicStreamingSpatializedActors!");
		}
	}
...
void UReplicationGraphNode_GridSpatialization2D::RenameActor_Static(const FRenamedReplicatedActorInfo& ActorInfo)
{
	...
	else
	{
		...
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_GridSpatialization2D::RenameActor_Static attempted rename %ls from static list but it was not there.", *GetActorRepListTypeDebugString(ActorInfo.NewActorInfo.Actor));
		...
		if (DynamicFoundInfo)
		{
			...
			UE_LOGF(LogReplicationGraph, Warning, "   It was in DynamicSpatializedActors!");
		}
	}
...
void UReplicationGraphNode_GridSpatialization2D::RenameActor_Dynamic(const FRenamedReplicatedActorInfo& ActorInfo)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_GridSpatialization2D::RenameActor_Dynamic attempted rename %ls from streaming dynamic list but it was not there.", *GetActorRepListTypeDebugString(ActorInfo.NewActorInfo.Actor));
...
UReplicationGraphNode_GridSpatialization2D::FActorCellInfo UReplicationGraphNode_GridSpatialization2D::GetCellInfoForActor(FActorRepListType Actor, const FVector& Location3D, float CullDistance)
{
	...
	if (CullDistance <= 0.f)
	{
		UE_LOGF(LogReplicationGraph, Warning, "::GetGridNodesForActor called on %ls when its CullDistance = %.2f. (Must be > 0)", *GetActorRepListTypeDebugString(Actor), CullDistance);
	}
#endif
		...
		Location3D.Z < -RepGraphHalfWorldMax || Location3D.Z > RepGraphHalfWorldMax)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "GetCellInfoForActor: Actor %ls is outside world bounds with a location of %ls. Clamping grid location to world bounds.", *GetFullNameSafe(Actor), *Location3D.ToString());	
...
/*
	...
	if (DebugActorNames.Num() > 0)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Display, "Adding Actor %ls. WorldLocation (Cached): %ls. WorldLocation (AActor): %ls. Buckets: %d/%d. SpatialBias: %ls", *Actor->GetName(), *ActorRepInfo.WorldLocation.ToString(), *Actor->GetActorLocation().ToString(), BucketX, BucketY, *SpatialBias.ToString());
		}
	}
#endif
*/
...
void UReplicationGraphNode_GridSpatialization2D::HandleActorOutOfSpatialBounds(AActor* Actor, const FVector& Location3D, const bool bStaticActor)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "Spatialization Rebuild caused by: %ls at %ls. New Bias: %ls. IsStatic: %d", *Actor->GetPathName(), *Location3D.ToString(), *SpatialBias.ToString(), (int32)bStaticActor);
	}
}
...
void UReplicationGraphNode_GridSpatialization2D::PrepareForReplication()
{
...
#endif
	{
		...
		for (auto& MapIt : DynamicSpatializedActors)
		{
			...
			{
				UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_GridSpatialization2D::PrepareForReplication: Dynamic Actor no longer ready for replication");
				UE_LOGF(LogReplicationGraph, Warning, "%ls", *GetNameSafe(DynamicActor));
				continue;
			}
#endif
				...
				if (PreviousCellInfo.IsValid())
				{
					...
					if (bDirty)
					{
						...
						if (CVar_RepGraph_Spatial_DebugDynamic)
						{
							...
							UE_LOGF(LogReplicationGraph, Display, "%ls moved cells. From %ls to %ls", *GetActorRepListTypeDebugString(DynamicActor), *CellInfoStr(PreviousCellInfo), *CellInfoStr(NewCellInfo));
							...
							for (int32 Y = MinY; Y <= MaxY; ++Y)
							{
								...
								for (int32 X = MinX; X <= MaxX; ++X)
								{
									...
									if (auto& Node = GetCell(GetGridX(X),Y))
									{
										...
										for (auto ActorInCell : ActorsInCell)
										{
											if (ActorInCell == DynamicActor)
											{
												if (bInCell)
												{
													UE_LOGF(LogReplicationGraph, Warning, "  Actor is in cell multiple times! [%d, %d]", X, Y);
												}
												bInCell = true;
											}
										}
									...
									{
										UE_LOGF(LogReplicationGraph, Warning, "  Bad update! Cell [%d,%d]. ShouldBeInOld: %d. ShouldBeInNew: %d. IsInCell: %d", X, Y, bShouldBeInOld, bShouldBeInNew, bInCell);
										Str += "! ";
									}
								}

								UE_LOGF(LogReplicationGraph, Display, "%ls", *Str);
							}
						}
#endif
	...
	if (bNeedsRebuild)
	{
		...
		UE_LOGF(LogReplicationGraph, Warning, "Rebuilding spatialization graph for bias %ls", *SpatialBias.ToString());
...
void UReplicationGraphNode_GridSpatialization2D::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (CellX < 0)
		{
			UE_LOGF(LogReplicationGraph, Verbose, "Net view location.X %ls is less than the spatial bias %ls for %ls", *ClampedViewLoc.ToString(), *SpatialBias.ToString(), CurViewer.Connection ? *CurViewer.Connection->Describe() : TEXT("NONE"));
			CellX = 0;
		}
		...
		if (CellY < 0)
		{
			UE_LOGF(LogReplicationGraph, Verbose, "Net view location.Y %ls is less than the spatial bias %ls for %ls", *ClampedViewLoc.ToString(), *SpatialBias.ToString(), CurViewer.Connection ? *CurViewer.Connection->Describe() : TEXT("NONE"));
			CellY = 0;
		}
	...
	if (bDestroyDormantDynamicActors && CVar_RepGraph_DormantDynamicActorsDestruction > 0 && Params.bIsSelectedForHeavyComputation)
	{
		...
		if (PrevDormantActorList.Num() > 0)
		{
			...
			UE_LOGF(LogReplicationGraph, Verbose, "UReplicationGraphNode_GridSpatialization2D::GatherActorListsForConnection: Removing %d Actors (List size: %d)", FMath::Min(NumActorsToRemove, PrevDormantActorList.Num()), PrevDormantActorList.Num());
			...
			for (int32 i = 0; i < PrevDormantActorList.Num() && NumActorsToRemove > 0; i++)
			{
				...
				if (FConnectionReplicationActorInfo* ActorInfo = Params.ConnectionManager.ActorInfoMap.Find(Actor))
				{
					...
					else if (ActorInfo->Channel == nullptr)
					{
						...
						UE_CLOGF(CVar_RepGraph_Verify, LogReplicationGraph, Warning, "UReplicationGraphNode_GridSpatialization2D::GatherActorListsForConnection: Actor with null channel pointer in Connection's PrevDormantActorList, it is preferred that we remove actors from the PrevDormantActorList when we clear their channel.");
						ActorInfo->bGridSpatilization_AlreadyDormant = false;
						PrevDormantActorList.RemoveAtSwap(i--);
					}
				}
...
void UReplicationGraphNode_GridSpatialization2D::NotifyActorCullDistChange(AActor* Actor, FGlobalActorReplicationInfo& GlobalInfo, float OldDist)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_GridSpatialization2D::NotifyActorCullDistChange. %ls Changed Cull Distance (%.2f -> %.2f) but is not in static or dynamic actor lists. %ls", *Actor->GetPathName(), OldDist, GlobalInfo.Settings.GetCullDistance(), *GetPathName() );
			...
			if (CVar_RepGraph_Verify)
			{
				...
				for (auto& InnerArray : Grid)
				{
					for (UReplicationGraphNode_GridCell* CellNode : InnerArray)
					{
						if (CellNode)
						{
							...
							{
								UE_LOGF(LogReplicationGraph, Warning, "  Its in node %ls", *CellNode->GetPathName());
								bFound = true;
							}
						}
					}
				...
				if (!bFound)
				{
					UE_LOGF(LogReplicationGraph, Warning, "  Not in the grid at all!");
				}
			}
		}
#endif
...
void UReplicationGraphNode_AlwaysRelevant::AddAlwaysRelevantClass(UClass* Class)
{
	...
	for (UClass* ExistingClass : AlwaysRelevantClasses)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "UReplicationGraphNode_AlwaysRelevant::AddAlwaysRelevantClass Adding class %ls when %ls is already in the list.", *Class->GetName(), *ExistingClass->GetName());
		}
	}
...
void UReplicationGraphNode_TearOff_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	...
	if (TearOffActors.Num() > 0)
	{
		...
		for (int32 idx=TearOffActors.Num()-1; idx >=0; --idx)
		{
			...
			//UE_LOGF(LogReplicationGraph, Display, "UReplicationGraphNode_TearOff_ForConnection::GatherActorListsForConnection. Actor: %ls. GetTearOff: %d. FrameNum: %d. TearOffFrameNum: %d", *GetNameSafe(Actor), (int32)Actor->GetTearOff(), Params.ReplicationFrameNum, TearOffFrameNum);
			...
			if (Actor && IsActorValidForReplication(Actor))
			{
				...
				{
					//UE_LOGF(LogReplicationGraph, Display, "0x%X ActorInfo->LastRepFrameNum: %d  ActorInfo->NextReplicationFrameNum: %d. (%d)", (int64)ActorInfo, ActorInfo->LastRepFrameNum, ActorInfo->NextReplicationFrameNum, (ActorInfo->NextReplicationFrameNum - Params.ReplicationFrameNum));
			...
			//UE_LOGF(LogReplicationGraph, Display, "Removing tearOffActor: %ls. GetTearOff: %d", *GetNameSafe(Actor), (int32)Actor->GetTearOff());
...
FAutoConsoleCommandWithWorldAndArgs NetRepGraphPrintChannelCounters(TEXT("Net.RepGraph.PrintActorChannelCounters"),TEXT(""),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		for (auto& MapIt: ActorChannelCreateCounter)
		{
			...
			UE_LOGF(LogReplicationGraph, Display, "%ls : %ls ----> %d", *GetNameSafe(Pair.Actor.Get()), *GetNameSafe(Pair.Connection.Get()), Count );
		}
	})
);
