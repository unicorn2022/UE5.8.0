...
private:
	...
	FPool& GetOrCreatePoolForListSize(int32 ExpectedMaxSize, bool ForPreAllocation=false)
	{
		...
		if (!Pool)
		{
			...
			if (!ForPreAllocation)
			{
				UE_LOGF(LogReplicationGraph, Warning, "No pool big enough for requested list size %d. Creating a new pool. (You may want to preallocate a pool of this size or investigate why this size is needed)", ExpectedMaxSize);
...
bool FActorRepListRefView::VerifyContents_Slow() const
{
	for (FActorRepListType Actor : *this)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "Actor %ls not valid for replication", *GetActorRepListTypeDebugString(Actor));
			return false;
		}
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "Actor %ls failed WeakObjectPtr resolve", *GetActorRepListTypeDebugString(Actor));
			return false;
		}
	}
...
void FGlobalActorReplicationInfoMap::AddDependentActor(AActor* Parent, AActor* Child, FGlobalActorReplicationInfoMap::EWarnFlag WarnFlag)
{
	...
	*/
	...
	if (bIsParentValid && bIsChildValid)
	{
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "FGlobalActorReplicationInfoMap::AddDependentActor Parent %ls not registered yet, cannot add child %ls", *GetNameSafe(Parent), *GetNameSafe(Child));
		}
		...
		{
			UE_LOGF(LogReplicationGraph, Warning, "FGlobalActorReplicationInfoMap::AddDependentActor child %ls already dependant of parent %ls", *GetNameSafe(Child), *GetNameSafe(Parent));
		}
	}
}
...
void FLevelBasedActorList::UpdateActorLevel(AActor* NetActor, FName PreviousStreamingLevelName)
{
	...
	{
		UE_LOGF(LogReplicationGraph, Warning, "FLevelBasedActorList::UpdateActorLevel did not find existing actor %ls in level lists", *GetFullNameSafe(NetActor));
	}
}