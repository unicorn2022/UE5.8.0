	...
	UGeometryScriptDebug* Debug) 
{
	...
	TargetMesh->EditMesh([&](FDynamicMesh3& EditMesh)
	{
		...
		TUniquePtr<FGroupTopology> Topo = GroupLayer.bDefaultLayer
			? MakeUnique<FGroupTopology>(&EditMesh, bAutoBuild)
			: MakeUnique<FGroupTopology>(&EditMesh, EditMesh.Attributes()->GetPolygroupLayer(GroupLayer.ExtendedLayerIndex), bAutoBuild);

		Topo->ShouldAddExtraCornerAtVert =
			[&EditMesh](const FGroupTopology& GroupTopology, int32 Vid, const FIndex2i& AttachedGroupEdgeEids)
				{
					// Help avoid creating group boundary with only 2 group edges
					return EditMesh.IsBoundaryVertex(Vid);
				};
