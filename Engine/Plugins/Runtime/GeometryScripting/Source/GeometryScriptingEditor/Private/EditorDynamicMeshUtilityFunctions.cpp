...
UDynamicMesh* UGeometryScriptLibrary_EditorDynamicMeshFunctions::BeginTrackedMeshChange(UDynamicMesh* TargetMesh, FDynamicMeshChangeContainer& ChangeContainer)
{
	...
	{
		UE_LOGF(LogGeometry, Warning, "BeginTrackedMeshChange: TargetMesh was null");
		return TargetMesh;
	}
	if (ChangeContainer.bIsActive)
	{
		UE_LOGF(LogGeometry, Warning, "BeginTrackedMeshChange: ChangeContainer was marked as active, discarding previous change");
	}
...
UDynamicMesh* UGeometryScriptLibrary_EditorDynamicMeshFunctions::EmitTrackedMeshChange(UDynamicMesh* TargetMesh, FDynamicMeshChangeContainer& ChangeContainer)
{
	...
	if (GUndo == nullptr)
	{
		UE_LOGF(LogGeometry, Warning, "EmitTrackedMeshChange: No Transaction is actively open, ignoring.");
		return TargetMesh;
	}

	if (ChangeContainer.bIsActive == false || ChangeContainer.ChangeInternals.IsValid() == false)
	{
		UE_LOGF(LogGeometry, Warning, "EmitTrackedMeshChange: ChangeContainer was not active, ignoring");
		return TargetMesh;
	}
	if (ChangeContainer.TargetMesh != TargetMesh)
	{
		UE_LOGF(LogGeometry, Warning, "EmitTrackedMeshChange: ChangeContainer was created for a different UDynamicMesh, ignoring");
		return TargetMesh;
	}
