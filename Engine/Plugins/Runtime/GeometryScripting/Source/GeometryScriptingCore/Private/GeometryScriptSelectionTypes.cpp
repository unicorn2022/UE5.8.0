...
EGeometryScriptMeshSelectionType FGeometryScriptMeshSelection::GetSelectionType() const
{
	...
	UE_LOGF(LogGeometry, Warning, "GeometryScriptMeshSelection::GetSelectionType() - GeoSelection has invalid type for FGeometryScriptMeshSelection!");
	return EGeometryScriptMeshSelectionType::Triangles;
}
...
int32 FGeometryScriptMeshSelection::GetNumUniqueSelected(const UE::Geometry::FDynamicMesh3& Mesh) const
{
	if (GeoSelection->TopologyType == EGeometryTopologyType::Polygroup)
	{
		if (GeoSelection->ElementType != EGeometryElementType::Face)
		{
			UE_LOGF(LogGeometry, Warning, "GeometryScriptMeshSelection::GetNumUniqueSelected() - GeoSelection has invalid type for FGeometryScriptMeshSelection!");
			return 0;
		}
...
void FGeometryScriptMeshSelection::DebugPrint() const
{
	...
	if (bIsPolygroups)
	{
		UE_LOGF(LogGeometry, Log, "FGeometryScriptSelectionList: Selection contains %d groups:", GeoSelection->Selection.Num());
	}
	else
	{
		UE_LOGF(LogGeometry, Log, "FGeometryScriptSelectionList: Selection contains %d %ls",
	...
	UE_LOGF(LogGeometry, Log, "Indices: %ls", *ResultString);
}
	...
	const FDynamicMesh3& Mesh, TArray<int32>& IndexListOut, EGeometryScriptIndexType ConvertToType) const
{
	...
	// indicates failure;
	UE_LOGF(LogGeometry, Warning, "FGeometryScriptSelectionListInternal: Conversion not currently supported");
