	...
	UGeometryScriptDebug* Debug)
{
	...
	TargetMesh->EditMesh([&](FDynamicMesh3& EditMesh)
	{
		...
		NewTriangleIndices.Init(INDEX_NONE, NumTriangles);
		...
		for (int32 k = 0; k < NumTriangles; ++k)
		{
			...
			const FIntVector& BufferTri = Buffers.Triangles[k];
			if (!VertexIDMap.IsValidIndex(BufferTri.X) || !VertexIDMap.IsValidIndex(BufferTri.Y) || !VertexIDMap.IsValidIndex(BufferTri.Z))
			{
				UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::InvalidInputs, LOCTEXT("AppendBuffersToMesh_Invalid", "AppendBuffersToMesh: Triangle cannot be added because it had invalid indices"));
				continue;
			}
			...
			else
			{
				...
				NewTriangleIndices[k] = NewTriangleID;
			}
		}
