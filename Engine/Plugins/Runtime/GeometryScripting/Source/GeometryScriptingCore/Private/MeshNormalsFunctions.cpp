	...
	bool bAverageSplitVertexValues)
{
	if (TargetMesh == nullptr)
	{
		UE_LOGF(LogGeometry, Warning, "GetMeshPerVertexNormals: TargetMesh is Null");
		return TargetMesh;
	}
	...
	TargetMesh->ProcessMesh([&](const FDynamicMesh3& ReadMesh)
	{
		...
		{
			FMeshNormals MeshNormals(&ReadMesh);
			MeshNormals.GetVertexNormalsFromOverlayNormals(bAverageSplitVertexValues ? FMeshNormals::ECombineSplitNormalsMethod::Average : FMeshNormals::ECombineSplitNormalsMethod::CopyAny);
			Normals = MeshNormals.MoveNormals();
	...
	bool bAverageSplitVertexValues)
{
	if (TargetMesh == nullptr)
	{
		UE_LOGF(LogGeometry, Warning, "GetMeshPerVertexTangents: TargetMesh is Null");
		return TargetMesh;
	}
	...
	bool bDeferChangeNotifications)
{
	if (TargetMesh == nullptr)
	{
		UE_LOGF(LogGeometry, Warning, "UpdateVertexNormal: TargetMesh is Null");
		return TargetMesh;
	}
	...
	TargetMesh->EditMesh([&](FDynamicMesh3& EditMesh) 
	{
		...
		{
			UE_LOGF(LogGeometry, Warning, "UpdateVertexNormal: TargetMesh does not have valid Normals attributes. Try computing Vertex Normals before using UpdateVertexNormal.");
			return;
		}
		if (bUpdateTangents && EditMesh.Attributes()->HasTangentSpace() == false)
		{
			UE_LOGF(LogGeometry, Warning, "UpdateVertexNormal: TargetMesh does not have valid Tangents attributes. Try computing Tangents before using UpdateVertexNormal.");
			return;
		}
		if (EditMesh.IsVertex(VertexID) == false)
		{
			UE_LOGF(LogGeometry, Warning, "UpdateVertexNormal: VertexID %d is not a valid vertex in TargetMesh", VertexID);
			return;
		}
		bIsValidVertex = true;
		...
		if (bUpdateNormal)
		{
			...
			{
				UE_LOGF(LogGeometry, Warning, "UpdateVertexNormal: VertexID %d has no existing normals in Normal Overlay. Try computing Vertex Normals before using UpdateVertexNormal.", VertexID);
				bIsValidVertex = false;
			}
		}
		if (bUpdateTangents)
		{
			...
			{
				UE_LOGF(LogGeometry, Warning, "UpdateVertexNormal: VertexID %d has no existing tangents in Tangent Overlay. Try computing Tangents before using UpdateVertexNormal.", VertexID);
				bIsValidVertex = false;
			}
		}
