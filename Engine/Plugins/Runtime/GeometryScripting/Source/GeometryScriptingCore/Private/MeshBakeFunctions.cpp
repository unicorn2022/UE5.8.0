...
namespace GeometryScriptBakeLocals
{
	...
	// Verify that every enabled per-triangle overlay on the supplied mesh has element-triangle
	// storage covering the parent's MaxTriangleID.
	bool ValidateOverlayTriangleStorage(
		const UDynamicMesh* Mesh,
		const FText& MeshLabel,
		const FText& DebugPrefix,
		TArray<FGeometryScriptDebugMessage>* Debug)
	{
		if (Mesh == nullptr)
		{
			return true; // null-mesh checks are handled by the callers
		}
		const FDynamicMesh3* MeshPtr = Mesh->GetMeshPtr();
		if (MeshPtr == nullptr || !MeshPtr->HasAttributes())
		{
			return true; // no overlays => nothing to validate here
		}

		const FDynamicMeshAttributeSet* Attributes = MeshPtr->Attributes();
		const int32 MaxTriangleID = MeshPtr->MaxTriangleID();

		auto ReportError = [Debug, &DebugPrefix, &MeshLabel, MaxTriangleID](const FText& OverlayLabel)
		{
			UE::Geometry::AppendError(Debug, EGeometryScriptErrorType::InvalidInputs,
				FText::Format(LOCTEXT("Bake_OverlayCoverageMismatch",
					"{0}: {1} {2} is structurally inconsistent with its parent mesh "
					"(MaxTriangleID={3}). The overlay's element-triangle storage does not cover "
					"every triangle in the parent. This usually means the mesh was constructed via "
					"a path that bypassed AppendTriangle/InsertTriangle (direct triangle-table "
					"writes, custom merges, or a deserialization mismatch). Baking this mesh would "
					"read out of bounds; aborting."),
					DebugPrefix, MeshLabel, OverlayLabel, FText::AsNumber(MaxTriangleID)));
		};
		...
		{
			const FDynamicMeshUVOverlay* UVLayer = Attributes->GetUVLayer(LayerIdx);
			if (UVLayer && !UVLayer->IsTriangleStorageValid())
			{
				ReportError(FText::Format(LOCTEXT("Bake_OverlayKindUV",
					"UV channel {0}"), FText::AsNumber(LayerIdx)));
				bAllValid = false;
			}
		}

		const int32 NumNormalLayers = Attributes->NumNormalLayers();
		for (int32 LayerIdx = 0; LayerIdx < NumNormalLayers; ++LayerIdx)
		{
			const FDynamicMeshNormalOverlay* NormalLayer = Attributes->GetNormalLayer(LayerIdx);
			if (NormalLayer && !NormalLayer->IsTriangleStorageValid())
			{
				ReportError(FText::Format(LOCTEXT("Bake_OverlayKindNormal",
					"Normal channel {0}"), FText::AsNumber(LayerIdx)));
				bAllValid = false;
			}
		}

		if (const FDynamicMeshColorOverlay* ColorLayer = Attributes->PrimaryColors())
		{
			if (!ColorLayer->IsTriangleStorageValid())
			{
				ReportError(LOCTEXT("Bake_OverlayKindColor", "Color overlay"));
				bAllValid = false;
			}
		}
		...
		TArray<FGeometryScriptDebugMessage>* Debug)
	{
		...
		const FText TargetMeshLabel = LOCTEXT("Bake_TargetMeshLabel", "TargetMesh");
		const FText SourceMeshLabel = LOCTEXT("Bake_SourceMeshLabel", "SourceMesh");
		bool bAllStorageValid = ValidateOverlayTriangleStorage(TargetMesh, TargetMeshLabel, BakeTexturePrefix, Debug);
		if (TargetMesh != SourceMesh)
		{
			bAllStorageValid &= ValidateOverlayTriangleStorage(SourceMesh, SourceMeshLabel, BakeTexturePrefix, Debug);
		}
		if (!bAllStorageValid)
		{
			return nullptr;
		...
		TArray<FGeometryScriptDebugMessage>* Debug)
	{
		...
		const FText TargetMeshLabel = LOCTEXT("Bake_TargetMeshLabel", "TargetMesh");
		const FText SourceMeshLabel = LOCTEXT("Bake_SourceMeshLabel", "SourceMesh");
		bool bAllStorageValid = ValidateOverlayTriangleStorage(TargetMesh, TargetMeshLabel, BakeVertexPrefix, Debug);
		if (TargetMesh != SourceMesh)
		{
			bAllStorageValid &= ValidateOverlayTriangleStorage(SourceMesh, SourceMeshLabel, BakeVertexPrefix, Debug);
		}
		if (!bAllStorageValid)
		{
			return nullptr;
