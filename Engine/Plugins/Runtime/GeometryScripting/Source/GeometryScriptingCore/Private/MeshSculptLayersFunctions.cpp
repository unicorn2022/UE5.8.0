...
TArray<double> UGeometryScriptLibrary_MeshSculptLayersFunctions::GetSculptLayerWeightsArray(UDynamicMesh* TargetMesh)
{
	...
	if (TargetMesh == nullptr)
	{
		UE_LOGF(LogGeometry, Error, "GetActiveSculptLayer: TargetMesh is Null");
		return Weights;
	}
	...
	TargetMesh->ProcessMesh([&Weights](const FDynamicMesh3& EditMesh)
	{
		...
		{
			UE_LOGF(LogGeometry, Error, "GetActiveSculptLayer: TargetMesh does not have sculpt layers enabled");
			return;
		}
...
int32 UGeometryScriptLibrary_MeshSculptLayersFunctions::GetNumSculptLayers(const UDynamicMesh* TargetMesh)
{
	if (TargetMesh == nullptr)
	{
		UE_LOGF(LogGeometry, Error, "GetNumSculptLayers: TargetMesh is Null");
		return -1;
	}
	...
	TargetMesh->ProcessMesh([&FoundLayerCount](const FDynamicMesh3& EditMesh)
	{
		if (!EditMesh.HasAttributes())
		{
			UE_LOGF(LogGeometry, Error, "GetNumSculptLayers: TargetMesh does not have attributes enabled");
			return;
		}
