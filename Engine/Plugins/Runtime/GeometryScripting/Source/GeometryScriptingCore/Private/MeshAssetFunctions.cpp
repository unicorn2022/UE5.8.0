	...
	UGeometryScriptDebug* Debug)
{
	...
	if (TargetLOD.bWriteHiResSource)
	{
		...
		if (Options.bCleanAssignedMaterials)
		{
			ConversionOptions.MaxValidMaterialID = ToStaticMeshAsset->GetStaticMaterials().Num() - 1;
		}
	...
	{ 
		...
		if (Options.bCleanAssignedMaterials)
		{
			...
			{
				ConversionOptions.MaxValidMaterialID = Options.NewMaterials.Num() - 1;
			}
			else
			{
				const FMeshSectionInfoMap& SectionMap = ToStaticMeshAsset->GetSectionInfoMap();
				int32 LODSectionNum = SectionMap.GetSectionNumber(UseLODIndex);
				ConversionOptions.MaxValidMaterialID = LODSectionNum - 1;
			}
		}
		...
		if (Options.bReplaceMaterials)
		{
			...
			auto SetMaterialSlotNames = [&MeshDescription](const TArray<FStaticMaterial>& NewMaterials)
			{
			...
			auto MakeNewMaterial = [&Options, bHaveSlotNames](const TArray<FStaticMaterial>& StaticMeshMaterials, int32 NewMatIdx) -> FStaticMaterial
			{
				FStaticMaterial NewMaterial;
				NewMaterial.MaterialInterface = Options.NewMaterials[NewMatIdx];
				FName UseSlotName = (bHaveSlotNames && Options.NewMaterialSlotNames[NewMatIdx] != NAME_None) ? Options.NewMaterialSlotNames[NewMatIdx] :
					UE::AssetUtils::GenerateNewMaterialSlotName(StaticMeshMaterials, NewMaterial.MaterialInterface, NewMatIdx);
			...
			if (Options.NewMaterialUsage == ECopyMeshToAssetUseNewMaterialsOptions::ReplaceAssetMaterials)
			{
				TArray<FStaticMaterial> NewMaterials;
				for (int32 k = 0; k < Options.NewMaterials.Num(); ++k)
				{
					NewMaterials.Add(MakeNewMaterial(NewMaterials, k));
				}

				ToStaticMeshAsset->SetStaticMaterials(NewMaterials);

				SetMaterialSlotNames(NewMaterials);
			...
			{
				TArray<FStaticMaterial> ToUpdateMaterials = ToStaticMeshAsset->GetStaticMaterials();
				TMap<UMaterialInterface*, int32> MatToIndex;
				for (int32 OldMatIdx = 0; OldMatIdx < ToUpdateMaterials.Num(); ++OldMatIdx)
				{
					MatToIndex.Add(ToUpdateMaterials[OldMatIdx].MaterialInterface, OldMatIdx);
				}
				// 
				for (int32 NewMatIdx = 0; NewMatIdx < Options.NewMaterials.Num(); ++NewMatIdx)
				{
					UMaterialInterface* NewMat = Options.NewMaterials[NewMatIdx];
					if (!MatToIndex.Contains(NewMat))
					{
						int32 NewIdx = ToUpdateMaterials.Add(MakeNewMaterial(ToUpdateMaterials, NewMatIdx));
						MatToIndex.Add(NewMat, NewIdx);
					}
				}

				ToStaticMeshAsset->SetStaticMaterials(ToUpdateMaterials);

				SetMaterialSlotNames(ToUpdateMaterials);

				// update material sections map for the current LOD
				auto UpdateMaterialSectionInfo = [&MatToIndex, &Options](FMeshSectionInfoMap& SectionInfoMap, int32 LODIndex)
				{
					// clear old sections for this LOD
					int32 NumSection = SectionInfoMap.GetSectionNumber(LODIndex);
					for (int32 Idx = 0; Idx < NumSection; ++Idx)
					{
						SectionInfoMap.Remove(LODIndex, Idx);
					}
					// add new section per material in the NewMaterials array
					for (int32 NewMatIdx = 0; NewMatIdx < Options.NewMaterials.Num(); ++NewMatIdx)
					{
						SectionInfoMap.Set(LODIndex, NewMatIdx, FMeshSectionInfo(MatToIndex[Options.NewMaterials[NewMatIdx]]));
					}
				};
				UpdateMaterialSectionInfo(ToStaticMeshAsset->GetSectionInfoMap(), UseLODIndex);
				UpdateMaterialSectionInfo(ToStaticMeshAsset->GetOriginalSectionInfoMap(), UseLODIndex);
			}
		}
...
TMap<FName, UMaterialInterface*> UGeometryScriptLibrary_StaticMeshFunctions::ConvertMaterialListToMaterialMap(const TArray<UMaterialInterface*>& MaterialList, const TArray<FName>& MaterialSlotNames)
{
	...
	{
		UE_LOGF(LogGeometry, Warning, "ConvertMaterialListToMaterialMap: Number of Material Slot Names does not match number of Materials");
	}
		...
		UGeometryScriptDebug* Debug)
{
	...
	TUniquePtr<TArray<int32>> MatIDInverseMap;

	// Update asset materials / set a corresponding material remap on the converter if needed
	if (Options.bReplaceMaterials)
	{
		bool bHaveSlotNames = (Options.NewMaterialSlotNames.Num() == Options.NewMaterials.Num());

		auto MakeNewMaterial = [&Options, bHaveSlotNames](TArray<FSkeletalMaterial>& SkelMaterials, int32 NewMatIdx) -> FSkeletalMaterial
		{
			FSkeletalMaterial NewMaterial;
			NewMaterial.MaterialInterface = Options.NewMaterials[NewMatIdx];
			FName UseSlotName = (bHaveSlotNames && Options.NewMaterialSlotNames[NewMatIdx] != NAME_None) ? Options.NewMaterialSlotNames[NewMatIdx] :
				UELocal::GenerateNewMaterialSlotName(SkelMaterials, NewMaterial.MaterialInterface, NewMatIdx);
		...
		{
			TArray<FSkeletalMaterial> ToUpdateMaterials = ToSkeletalMeshAsset->GetMaterials();
			TMap<UMaterialInterface*, int32> MatToIndex;
			for (int32 OldMatIdx = 0; OldMatIdx < ToUpdateMaterials.Num(); ++OldMatIdx)
			{
				MatToIndex.Add(ToUpdateMaterials[OldMatIdx].MaterialInterface, OldMatIdx);
			...
			// build material remap s.t. dynamic mesh material indices map to where materials were set in the skeletal mesh asset
			MatIDInverseMap = MakeUnique<TArray<int32>>();
			MatIDInverseMap->Init(-1, ToUpdateMaterials.Num());
			for (int32 NewMatIdx = 0; NewMatIdx < Options.NewMaterials.Num(); ++NewMatIdx)
			{
				(*MatIDInverseMap)[MatToIndex[Options.NewMaterials[NewMatIdx]]] = NewMatIdx;
			}

			if (bUpdated)
			{
				ToSkeletalMeshAsset->SetMaterials(ToUpdateMaterials);
			}
		}
	}
	...
	if (MatIDInverseMap)
	{
		Converter.SetMaterialIDMapFromInverseMap(*MatIDInverseMap);
	}
	if (Options.bCleanAssignedMaterials)
	{
		if (Options.bReplaceMaterials)
		{
			...
			else if (ensure(MatIDInverseMap))
			{
				ConversionOptions.MaxValidMaterialID = MatIDInverseMap->Num() - 1;
			}
		}
		else
		{
			FSkeletalMeshLODInfo* SkeletalLODInfo = ToSkeletalMeshAsset->GetLODInfo(TargetLOD.LODIndex);
			int32 LODNumMats = SkeletalLODInfo->LODMaterialMap.IsEmpty() ? ToSkeletalMeshAsset->GetNumMaterials() : SkeletalLODInfo->LODMaterialMap.Num();
			ConversionOptions.MaxValidMaterialID = LODNumMats - 1;
		}
	}
	...
	// Final pass of material updating
	if (Options.bReplaceMaterials)
	{
		// reset LODMaterialMap; it will be set up later with just the materials actually used
		SkeletalLODInfo->LODMaterialMap.Reset();
		
		// Set material slot names on the mesh description
		TPolygonGroupAttributesRef<FName> PolygonGroupImportedMaterialSlotNames = MeshAttributes.GetPolygonGroupMaterialSlotNames();
		const TArray<FSkeletalMaterial>& UpdatedMaterials = ToSkeletalMeshAsset->GetMaterials();
		for (int32 SlotIdx = 0; SlotIdx < UpdatedMaterials.Num(); ++SlotIdx)
		{
			if (SlotIdx < PolygonGroupImportedMaterialSlotNames.GetNumElements())
			{
				PolygonGroupImportedMaterialSlotNames.Set(SlotIdx, UpdatedMaterials[SlotIdx].ImportedMaterialSlotName);
			}
		}
	}
