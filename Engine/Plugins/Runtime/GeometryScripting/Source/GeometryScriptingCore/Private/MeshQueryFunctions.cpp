...
FGeometryScriptWeightMapHandle UGeometryScriptLibrary_MeshQueryFunctions::GetWeightMapHandle(UDynamicMesh* TargetMesh, FName Name, EGeometryScriptSearchOutcomePins& FoundWeights)
{
	FGeometryScriptWeightMapHandle Handle;
	Handle.WeightMapAttributeLayerIndex = SimpleMeshQuery<int32>(TargetMesh, (int32)-1, [&Name](const FDynamicMesh3& Mesh) -> int32
	{
		if (Mesh.HasAttributes())
		{
		...
		UE::Geometry::MakeScriptWarning(EGeometryScriptErrorType::InvalidInputs,
			FText::Format(LOCTEXT("InvalidWeightMapName", "Weight Map Layer with name \"{0}\" not found on TargetMesh"), FText::FromName(Name)));

		return -1;
	});

	FoundWeights = Handle.IsValid() ? EGeometryScriptSearchOutcomePins::Found : EGeometryScriptSearchOutcomePins::NotFound;
