...
namespace UE::VolumeTextureBakeLocal
{
		...
		EVolumeTextureFormat Format)
	{
		...
		default:
			UE_LOGF(LogGeometry, Error, "Unknown volume texture format");
			return false;
		}
	}
}
...
)
{
	...
	{
		UE_LOGF(LogGeometry, Warning, "BakeSignedDistanceToVolumeTexture: Target Mesh was null");
		return false;
	}

	if (VolumeTexture == nullptr)
	{
		UE_LOGF(LogGeometry, Warning, "BakeSignedDistanceToVolumeTexture: Volume Texture was null");
		return false;
	}
	...
	return bSuccess;
#else
	UE_LOGF(LogGeometry, Warning, "BakeSignedDistanceToVolumeTexture: Baking signed distance to volume texture is only supported in editor");
	return false;
#endif
}
