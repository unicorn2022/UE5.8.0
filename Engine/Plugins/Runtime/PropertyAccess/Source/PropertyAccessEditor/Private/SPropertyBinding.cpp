...
bool SPropertyBinding::IsClassDenied(const FPropertyBindingWidgetArgs& Args, UClass* InClass)
{
...
bool SPropertyBinding::IsFieldFromDeniedClass(const FPropertyBindingWidgetArgs& Args, FFieldVariant Field)
{
	return IsClassDenied(Args, Field.GetOwnerClass());
}

template <typename Predicate>
void SPropertyBinding::ForEachBindableFunction(const FPropertyBindingWidgetArgs& Args, UClass* FromClass, Predicate Pred)
{
	if(Args.OnCanBindFunction.IsBound())
	{
		auto CheckBindableClass = [&Args, &Pred](UClass* InBindableClass)
		{
			...
			for ( TFieldIterator<UFunction> FuncIt(InBindableClass, EFieldIteratorFlags::IncludeSuper, EFieldIteratorFlags::ExcludeDeprecated, EFieldIteratorFlags::IncludeInterfaces); FuncIt; ++FuncIt )
			{
				...
				if ( IsFieldFromDeniedClass(Args, Function) )
				{
					break;
				}
...
bool SPropertyBinding::HasCategorySomethingToDisplayRecursive(const FBindingContextStructCategory& Category) const
{
	...
	for(const int32 ContextStructIndex : Category.BindingContextStructIndices)
	{
		...
		if (HasBindableProperties(Args, Blueprint, Struct, BindingChain))
		{
			return true;
		}
	}
...
template <typename Predicate>
void SPropertyBinding::ForEachBindableProperty(const FPropertyBindingWidgetArgs& Args,  const UBlueprint* Blueprint, UStruct* InStruct, const TArray<TSharedPtr<FBindingChainElement>>& BindingChain, Predicate Pred)
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	if(InStruct && bIsOnCanBindPropertyBound)
	{
		...
		for ( TFieldIterator<FProperty> PropIt(InStruct, EFieldIteratorFlags::IncludeSuper); PropIt; ++PropIt )
		{
			...
			if ( IsFieldFromDeniedClass(Args, Property) )
			{
				break;
			}

			if (!CanAcceptPropertyOrChildren(Args, Property, BindingChain))
			{
				continue;
			}
			if (SkeletonClass)
...
bool SPropertyBinding::HasBindableProperties(const FPropertyBindingWidgetArgs& Args, const UBlueprint* Blueprint, UStruct* InStruct, TArray<TSharedPtr<FBindingChainElement>>& BindingChain)
{
	TSet<UStruct*> VisitedStructs;
	return HasBindablePropertiesRecursive(Args, Blueprint, InStruct, VisitedStructs, BindingChain);
}

bool SPropertyBinding::HasBindablePropertiesRecursive(const FPropertyBindingWidgetArgs& Args, const UBlueprint* Blueprint, UStruct* InStruct, TSet<UStruct*>& VisitedStructs, TArray<TSharedPtr<FBindingChainElement>>& BindingChain)
{
	...
	ForEachBindableProperty(Args, Blueprint, InStruct, BindingChain, [&Args, Blueprint, &BindableCount, &VisitedStructs, &BindingChain] (FProperty* Property)
	{
		...
		if(CanBindProperty(Args, Property, BindingChain))
		{
			BindableCount++;
			return;
		}
		...
		if(Args.bAllowArrayElementBindings && ArrayProperty != nullptr)
		{
			...
			if(CanBindProperty(Args, ArrayProperty->Inner, BindingChain))
			{
				BindableCount++;
				return;
			}	
		...
		if (Struct)
		{
			if (UStruct* ResolvedStruct = ResolveIndirection(Args, BindingChain))
			{
				Struct = ResolvedStruct;
			}

			// Recurse into a struct, except if it is the same type as the one we're binding.
			if (HasBindablePropertiesRecursive(Args, Blueprint, Struct, VisitedStructs, BindingChain))
			{
				if (Class)
				{
					// Ignore any subobject properties that are not bindable.
					// Also ignore any class that is explicitly on the deny list.
					if (IsClassDenied(Args, Class) || (Args.OnCanBindToSubObjectClass.IsBound() && Args.OnCanBindToSubObjectClass.Execute(Class)))
					{
						return;
					}
				}
	...
	{
		ForEachBindableFunction(Args, Class, [&Args, &Blueprint, &BindableCount, &VisitedStructs, &BindingChain](const FFunctionInfo& Info)
		{
			...
			if(!Args.bAllowUObjectFunctions || CanBindProperty(Args, ReturnProperty, BindingChain))
			{
				BindableCount++;
			}
			...
			if(Args.bGeneratePureBindings)
			{
				...
				{
					if (HasBindablePropertiesRecursive(Args, Blueprint,ObjectPropertyBase->PropertyClass, VisitedStructs, BindingChain))
					{
						if (Args.bAllowUObjectFunctions)
						{
							BindableCount++;
				...
				{
					if (HasBindablePropertiesRecursive(Args, Blueprint, StructProperty->Struct, VisitedStructs, BindingChain))
					{
						BindableCount++;
					}
				}
	...
	if (BindingContextStructs.Num() > 0)
	{
		...
		for(const FBindingContextStructCategory& Section : BindingContextStructSections)
		{
			...
			for (int32 Index : Section.BindingContextStructIndices)
			{
				...
				if (HasBindableProperties(Args, Blueprint, ContextStruct.Struct, BindingChain))
				{
...
{
	FillPropertyMenu(MenuBuilder, Args, Blueprint, PropertyName, InOwnerStruct, InBindingChain);	
}

void SPropertyBinding::FillPropertyMenu(FMenuBuilder& MenuBuilder, const FPropertyBindingWidgetArgs& Args, const UBlueprint* Blueprint, FName PropertyName,  UStruct* InOwnerStruct, TArray<TSharedPtr<FBindingChainElement>> InBindingChain)
{
	auto MakeArrayElementPropertyWidget = [&Args, PropertyName](FProperty* InProperty, const TArray<TSharedPtr<FBindingChainElement>>& InNewBindingChain)
	{
		const UEdGraphSchema_K2* Schema = GetDefault<UEdGraphSchema_K2>();
		...
		return SNew(SHorizontalBox)
			...
			[
				...
				[
					...
					.OnValueCommitted_Lambda([Args, PropertyName, InProperty, InNewBindingChain](int32 ArrayIndex, ETextCommit::Type TextCommitType)
						{
							HandleSetBindingArrayIndex(Args, PropertyName, ArrayIndex, TextCommitType, InProperty, InNewBindingChain);
						})
				]
			];
	};

	auto MakeArrayElementEntry = [&Args, PropertyName, &MenuBuilder, &MakeArrayElementPropertyWidget](FProperty* InProperty, const TArray<TSharedPtr<FBindingChainElement>>& InNewBindingChain)
	{
		MenuBuilder.AddMenuEntry(
			FUIAction(FExecuteAction::CreateLambda([Args, PropertyName, InNewBindingChain]()
				{
					HandleAddBinding(Args, PropertyName, InNewBindingChain);
				})),
			SNew(SHorizontalBox)
			+SHorizontalBox::Slot()
			.FillWidth(1.0f)
	...
	auto MakePropertyEntry = [&Args, PropertyName, &MenuBuilder, &MakePropertyWidget](FProperty* InProperty, const TArray<TSharedPtr<FBindingChainElement>>& InNewBindingChain)
	{
		MenuBuilder.AddMenuEntry(
			FUIAction(FExecuteAction::CreateLambda(
				[Args, PropertyName, InNewBindingChain]()
				{
					HandleAddBinding(Args, PropertyName, InNewBindingChain);
				})),
	...
	auto MakeFunctionWidget = [](const FFunctionInfo& Info)
	{
	...
	if (Args.bAllowFunctionBindings)
	{
		if ( UClass* OwnerClass = Cast<UClass>(InOwnerStruct) )
		{
			...
			{
				ForEachBindableFunction(Args, OwnerClass, [&Args, Blueprint, PropertyName, &InBindingChain, &MenuBuilder, &MakeFunctionWidget] (const FFunctionInfo& Info) 
				{
					...
					FProperty* ReturnProperty = Info.Function->GetReturnProperty();
					// We can get here if we accept non-leaf UObject functions, so if so we need to check the return value for compatibility
					if(!Args.bAllowUObjectFunctions || CanBindProperty(Args, ReturnProperty, InBindingChain))
					{
						MenuBuilder.AddMenuEntry(
							FUIAction(FExecuteAction::CreateLambda(
								[Args, PropertyName, NewBindingChain]()
									{
										HandleAddBinding(Args, PropertyName, NewBindingChain);
									})),
					...
					if(Args.bGeneratePureBindings)
					{
						if(FObjectPropertyBase* ObjectPropertyBase = CastField<FObjectPropertyBase>(ReturnProperty))
						{
							if (HasBindableProperties(Args, Blueprint, ObjectPropertyBase->PropertyClass,  NewBindingChain))
							{
								MenuBuilder.AddSubMenu(
									MakeFunctionWidget(Info),
									FNewMenuDelegate::CreateLambda(
										[Args, Blueprint, PropertyName, ObjectPropertyBase, NewBindingChain](FMenuBuilder& MenuBuilder)
											{
												FillPropertyMenu(MenuBuilder, Args, Blueprint, PropertyName, static_cast<UStruct*>(ObjectPropertyBase->PropertyClass), NewBindingChain);
											}));
							}
						}
						else if(FStructProperty* StructProperty = CastField<FStructProperty>(ReturnProperty))
						{
							if (HasBindableProperties(Args, Blueprint, StructProperty->Struct, NewBindingChain))
							{
								MenuBuilder.AddSubMenu(
									MakeFunctionWidget(Info),
									FNewMenuDelegate::CreateLambda([Args, Blueprint, PropertyName, StructProperty, NewBindingChain](FMenuBuilder& MenuBuilder)
								{
									FillPropertyMenu(MenuBuilder, Args, Blueprint, PropertyName, static_cast<UStruct*>(StructProperty->Struct), NewBindingChain);
								}));
							}
						}
					}
	...
	if (Args.bGeneratePureBindings && Args.bAllowPropertyBindings)
	{
		...
		if (CanBindProperty(Args, BindingProperty, InBindingChain))
		{
			...
			{
				ForEachBindableProperty(Args, Blueprint, InOwnerStruct, InBindingChain, [&Args, Blueprint, PropertyName, &InBindingChain, BindingStruct, &MenuBuilder, &MakeArrayElementPropertyWidget, &MakePropertyWidget, &MakePropertyEntry, &MakeArrayElementEntry](FProperty* Property)
					{
						TArray<TSharedPtr<FBindingChainElement>> NewBindingChain(InBindingChain);
						...
						if (CanBindProperty(Args, Property, NewBindingChain))
						{
						...
						if (bUseArrayProperty)
						{
							...
							if (CanBindProperty(Args, ArrayProperty->Inner, NewArrayElementBindingChain))
							{
						...
						if (Struct)
						{
							if (UStruct* ResolvedStruct = ResolveIndirection(Args, NewBindingChain))
							{
								Struct = ResolvedStruct;
							}
							...
							TArray<TSharedPtr<FBindingChainElement>>& BindablePropertyBindingChain = bUseArrayProperty
								...
								: NewBindingChain;
							if (HasBindableProperties(Args, Blueprint, Struct, BindablePropertyBindingChain))
							{
								if (bUseArrayProperty)
								{
									MenuBuilder.AddSubMenu(
										...
										FNewMenuDelegate::CreateLambda(
											[Args, Blueprint, PropertyName, Struct, NewArrayElementBindingChain](FMenuBuilder& MenuBuilder)
												{
													FillPropertyMenu(MenuBuilder, Args, Blueprint, PropertyName, Struct, NewArrayElementBindingChain);
												})
									);
								}
								else if (Args.bAllowStructMemberBindings)
								{
									MenuBuilder.AddSubMenu(
										...
										FNewMenuDelegate::CreateLambda(
											[Args, Blueprint, PropertyName, Struct, NewBindingChain](FMenuBuilder& MenuBuilder)
												{
													FillPropertyMenu(MenuBuilder, Args, Blueprint, PropertyName, Struct, NewBindingChain);
												})
									);
								}
							}
...
void SPropertyBinding::HandleAddBinding(const TArray<TSharedPtr<FBindingChainElement>> InBindingChain) const
{
	HandleAddBinding(Args, PropertyName, InBindingChain);
}

void SPropertyBinding::HandleAddBinding(const FPropertyBindingWidgetArgs& Args, FName PropertyName, const TArray<TSharedPtr<FBindingChainElement>>& InBindingChain)
{
...
void SPropertyBinding::HandleSetBindingArrayIndex(const FPropertyBindingWidgetArgs& Args, FName PropertyName, int32 InArrayIndex, ETextCommit::Type InCommitType, FProperty* InProperty, TArray<TSharedPtr<FBindingChainElement>> InBindingChain)
{
	InBindingChain.Last()->ArrayIndex = InArrayIndex;

	// If the user hit enter on a compatible property, assume they want to accept
	if(CanBindProperty(Args, InProperty, InBindingChain) && InCommitType == ETextCommit::OnEnter)
	{
		HandleAddBinding(Args, PropertyName, InBindingChain);
	}
}
...
void SPropertyBinding::HandleCreateAndAddBinding()
{
	...
	HandleAddBinding(Args, PropertyName, BindingPath);
...
UStruct* SPropertyBinding::ResolveIndirection(const FPropertyBindingWidgetArgs& Args, const TArray<TSharedPtr<FBindingChainElement>>& InBindingChain)
{
...
bool SPropertyBinding::CanAcceptPropertyOrChildren(const FPropertyBindingWidgetArgs& Args, FProperty* InProperty, TConstArrayView<TSharedPtr<FBindingChainElement>> InBindingChain)
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
bool SPropertyBinding::CanBindProperty(const FPropertyBindingWidgetArgs& Args, FProperty* InProperty, TConstArrayView<TSharedPtr<FBindingChainElement>> InBindingChain)
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
