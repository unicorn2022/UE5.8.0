...
namespace Audio
{
	...
	bool FAC3DownmixerFactory::GetAC3DownmixEntries(const FInputFormat& InInputFormat, const FOutputFormat& InOutputFormat, TArray<FChannelMixEntry>& OutMixEntries)
	{
		...
		{
			UE_LOGF(LogSynthesis, Error, "Unexpected channel map size of %d. Expected %d", ChannelMap.Num(), ExpectedChannelMapSize);
			return false;
		}
	...
	TUniquePtr<FSimpleRouter> FSimpleRouter::CreateSimpleRouter(const TArray<EAudioMixerChannel::Type>& InInputChannelTypes, const TArray<EAudioMixerChannel::Type>& InOutputChannelTypes, int32 InNumFramesPerCall)
	{
		...
		{
			UE_LOGF(LogSynthesis, Error, "Invalid number of frames per call [%d]", InNumFramesPerCall);
		...
		{
			UE_LOGF(LogSynthesis, Error, "Input channel type contains duplicate values.");
			return TUniquePtr<FSimpleRouter>();
		}
		...
		{
			UE_LOGF(LogSynthesis, Error, "Output channel type contains duplicate values.");
			return TUniquePtr<FSimpleRouter>();
		}
		...
		if (!bUpmixMono)
		{
			...
			for (int32 InputChannelIndex = 0; InputChannelIndex < InInputChannelTypes.Num(); InputChannelIndex++)
			{
				...
				else
				{
					...
					UE_LOGF(LogSynthesis, Warning, "Unable to match input channel [%ls] to output channel", EAudioMixerChannel::ToString(InInputChannelTypes[InputChannelIndex]));
				}
			}
	...
	TUniquePtr<FSimpleUpmixer> FSimpleUpmixer::CreateSimpleUpmixer(const TArray<EAudioMixerChannel::Type>& InInputChannelTypes, const TArray<EAudioMixerChannel::Type>& InOutputChannelTypes, int32 InNumFramesPerCall)
	{
		...
		{
			UE_LOGF(LogSynthesis, Error, "Invalid input format channel count (%d). Must be greater than zero", InInputChannelTypes.Num());
			return TUniquePtr<FSimpleUpmixer>(nullptr);
		}

		if (InOutputChannelTypes.Num() < 1) 
		{
			UE_LOGF(LogSynthesis, Error, "Invalid output format channel count (%d). Must be greater than zero", InOutputChannelTypes.Num());
			return TUniquePtr<FSimpleUpmixer>(nullptr);
		}

		if (InNumFramesPerCall < 1)
		{
			UE_LOGF(LogSynthesis, Error, "Invalid num frames per call (%d). Must be greater than zero", InNumFramesPerCall);
			return TUniquePtr<FSimpleUpmixer>(nullptr);
		}
