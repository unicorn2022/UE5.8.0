...
namespace Audio
{
	namespace ConvolutionReverbIntrinsics
	{
		...
		bool SetImpulseResponse(IConvolutionAlgorithm& InAlgo, const TArray<float>& InSamples, int32 InNumImpulseResponses, float InImpulseSampleRate, float InTargetSampleRate)
		{
			...
			if (InImpulseSampleRate != InTargetSampleRate)
			{
				...
				{
					UE_LOGF(LogSynthesis, Error, "ISampleRateConverter failed to create a sample rate converter");
					return false;
				}
	...
	TUniquePtr<FConvolutionReverb> FConvolutionReverb::CreateConvolutionReverb(const FConvolutionReverbInitData& InInitData, const FConvolutionReverbSettings& InSettings)
	{
		...
		{
			UE_LOGF(LogSynthesis, Error, "Invalid impulse sample rate: %f.", InInitData.ImpulseSampleRate);
			return TUniquePtr<FConvolutionReverb>();
		}

		if (InInitData.TargetSampleRate <= 0.f)
		{
			UE_LOGF(LogSynthesis, Error, "Invalid target sample rate: %f.", InInitData.TargetSampleRate);
			return TUniquePtr<FConvolutionReverb>();
		}
		...
		{
			UE_LOGF(LogSynthesis, Error, "Invalid num input channels: %d", AlgoSettings.NumInputChannels);
			return TUniquePtr<FConvolutionReverb>();
		}
		
		if (AlgoSettings.NumOutputChannels < 1)
		{
			UE_LOGF(LogSynthesis, Error, "Invalid num output channels: %d", AlgoSettings.NumOutputChannels);
			return TUniquePtr<FConvolutionReverb>();
		}

		if (AlgoSettings.NumImpulseResponses < 1)
		{
			UE_LOGF(LogSynthesis, Error, "Invalid num impulse responses: %d", AlgoSettings.NumImpulseResponses);
			return TUniquePtr<FConvolutionReverb>();
		}
		...
		{
			UE_LOGF(LogSynthesis, Error, "Invalid input audio format. Channel count: %d", InputAudioFormat.NumChannels);
			return TUniquePtr<FConvolutionReverb>();
		}
		...
		{
			UE_LOGF(LogSynthesis, Error, "Mismatched input format. Received %d input channels. Expected %d.", InputAudioFormat.NumChannels, AlgoSettings.NumInputChannels);
			return TUniquePtr<FConvolutionReverb>();
		}
		...
		{
			UE_LOGF(LogSynthesis, Error, "Invalid output audio format. Channel count: %d", OutputAudioFormat.NumChannels);
			return TUniquePtr<FConvolutionReverb>();
		}
		...
		{
			UE_LOGF(LogSynthesis, Warning, "Failed to create convolution algorithm for convolution reverb.");
			return TUniquePtr<FConvolutionReverb>();
		}
		...
		if (!bSuccess)
		{
			UE_LOGF(LogSynthesis, Warning, "Failed to set impulse responses on convolution algorithm.");
			return TUniquePtr<FConvolutionReverb>();
		}
		...
		if (InInitData.bMixInputChannelFormatToImpulseResponseFormat && (InputAudioFormat.NumChannels != AlgoSettings.NumInputChannels))
		{
			...
			{
				UE_LOGF(LogSynthesis, Warning, "Failed to convert input audio format to convolution reverb input format.");
				return TUniquePtr<FConvolutionReverb>();
			}
		}
		...
		else if (bUpmixReverbToOutput || bRouteReverbToOutput)
		{
			...
			{
				UE_LOGF(LogSynthesis, Warning, "Failed to handle reverb output channel count [%d]", AlgoSettings.NumOutputChannels);
				return TUniquePtr<FConvolutionReverb>();
			}
			...
			{
				UE_LOGF(LogSynthesis, Warning, "Failed to handle output audio channel count [%d]", OutputAudioFormat.NumChannels);
				return TUniquePtr<FConvolutionReverb>();
			}
	...
	void FConvolutionReverb::ProcessAudio(int32 InNumInputChannels, const float* InputAudio, int32 InNumOutputChannels, float* OutputAudio, const int32 InNumFrames)
	{
		...
		{
			UE_LOGF(LogSynthesis, Error, "Convolution num input channel mismatch. Expected %d, got %d", GetNumInputChannels(), InNumInputChannels);
			return;
		}
		else if (InNumOutputChannels != GetNumOutputChannels())
		{
			UE_LOGF(LogSynthesis, Error, "Convolution num output channel mismatch. Expected %d, got %d", GetNumOutputChannels(), InNumOutputChannels);
			return;
		}
