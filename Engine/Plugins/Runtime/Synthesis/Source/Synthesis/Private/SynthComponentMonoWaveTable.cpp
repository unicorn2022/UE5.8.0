...
void UMonoWaveTableSynthPreset::EditChangeInternal()
{
	...
	for (FRuntimeFloatCurve& Curve : WaveTable)
	{
		...
		Curve.EditorCurveData.RemoveRedundantAutoTangentKeys(SMALL_NUMBER, -0.1f, 1.1f);
...
bool USynthComponentMonoWaveTable::SetCurveValue(int32 TableIndex, int32 KeyframeIndex, const float NewValue)
{
	...
	if (!CachedPreset)
	{
		UE_LOGF(LogSynthesis, Warning, "Trying to edit a Wave Table curve value when there is no Wave Table Preset Asset, returning false");
		return false;
	}
	else if (TableIndex >= CachedPreset->WaveTable.Num())
	{
		UE_LOGF(LogSynthesis, Warning, "Table index: %i is out of bounds! (Number of Curves: %i), returning false"
		, TableIndex, CachedPreset->WaveTable.Num());
		return false;
	}
	else if (KeyframeIndex >= CachedPreset->WaveTable[TableIndex].EditorCurveData.GetNumKeys())
	{
		UE_LOGF(LogSynthesis, Warning, "Frame index: %i is out of bounds (Number of KeyFrames: %i), returning false"
...
bool USynthComponentMonoWaveTable::SetCurveInterpolationType(CurveInterpolationType InterpolationType, int32 TableIndex)
{
	...
	if (!CachedPreset)
	{
		UE_LOGF(LogSynthesis, Warning, "Trying to edit a Wave Table interpolation type when there is no Wave Table Preset Asset, returning false");
		return false;
	}
	else if (TableIndex >= CachedPreset->WaveTable.Num())
	{
...
bool USynthComponentMonoWaveTable::SetCurveTangent(int32 TableIndex, float InNewTangent)
{
	...
	if (!CachedPreset)
	{
		UE_LOGF(LogSynthesis, Warning, "Trying to edit a Wave Table curve tangent value when there is no Wave Table Preset Asset, returning false");
		return false;
	}
	else if (TableIndex >= CachedPreset->WaveTable.Num())
	{
...
float USynthComponentMonoWaveTable::GetCurveTangent(int32 TableIndex)
{
	...
	{
		// no asset
		UE_LOGF(LogSynthesis, Warning, "Trying to get Wave Table Tangent values when there is no Wave Table Curve Asset, returning 0.0");
		return 0.0f;
	}
	else if (CachedPreset->WaveTable.Num() <= TableIndex || CachedPreset->CurveBiDirTangents.Num() <= TableIndex || TableIndex <= -1.0f)
	{
		// Index out of range
		UE_LOGF(LogSynthesis, Warning, "Table index: %i is out of bounds! (Number of Curves: %i), returning 0.0"
			, TableIndex, CachedPreset->WaveTable.Num());
		return 0.0f;
	}
...
TArray<float> USynthComponentMonoWaveTable::GetKeyFrameValuesForTable(float TableIndex) const
{
	...
	{
		// no asset
		UE_LOGF(LogSynthesis, Warning, "Trying to get Wave Table curve values when there is no Wave Table Curve Asset, returning empty TArray");
		return Values;
	}
	else if (CachedPreset->WaveTable.Num() <= TableIndex || TableIndex < 0.0f)
	{
		// Index out of range
		UE_LOGF(LogSynthesis, Warning, "Table index: %f is out of bounds! (Number of Curves: %i), returning empty TArray"
		, TableIndex, CachedPreset->WaveTable.Num());
		return Values;
	}
