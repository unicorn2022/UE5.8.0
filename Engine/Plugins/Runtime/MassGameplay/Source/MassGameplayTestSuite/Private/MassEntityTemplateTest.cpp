...
struct FEntityTemplate_Composition : FEntityTemplateBase
{
	virtual bool InstantTest() override
	{
		...
		ExpectedComposition.Add<FTestFragment_Int>();
		ExpectedComposition.Add<FTestFragment_Float>();
		ExpectedComposition.Add<FTestChunkFragment_Int>();
		ExpectedComposition.Add<FTestSharedFragment_Int>();
		ExpectedComposition.Add<FTestSharedFragment_Float>();
...
struct FEntityTemplate_Modified : FEntityTemplateBase
{
	virtual bool InstantTest() override
	{
		...
		FTestFragment_Int* CopiedFragment = NewTemplateData.GetMutableFragment<FTestFragment_Int>();
		AITEST_NOT_NULL("The fragment instance is expected to be found", CopiedFragment);
		AITEST_EQUAL("The fragment instance is expected to have the same value as the original one", CopiedFragment->Value, IntFragment.Value);
		CopiedFragment->Value = FTestFragment_Int::TestIntValue + 1;
		AITEST_NOT_EQUAL("Modifying the coppied instance of the fragment doesn't affect the original", CopiedFragment->Value, IntFragment.Value);
...
struct FEntityTest_BatchSetMissingFragment : FEntityTemplateBase
{
	virtual bool InstantTest() override
	{
		...
		{
			MASS_SCOPED_ENSURE_TEST("Attempting to set value of fragment of type", 1);
