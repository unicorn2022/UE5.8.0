...
void UDebugVisLocationProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	{
		// MassDebugVisualization relies on an Actor/Component combo added to the world
			...
			EntityQuery.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context)
				{
					...
					{
						UE_LOGF(LogMassDebug, Log, "UDebugVisLocationProcessor: Trying to update InstanceStaticMeshes while none created. Check your debug visualization setup");
					}
				});
	...
	{
		UWorld* World = UE::Mass::Debug::GetDebugDrawWorld(Context);

		if (World && World->GetLineBatcher(UWorld::ELineBatcherType::World))
		{
			...
			AllLocationEntitiesQuery.ForEachEntityChunk(Context, [&LineBatcher](const FMassExecutionContext& Context)
				{
					const FColor ArchetypeColor = Context.DebugGetArchetypeColor();
