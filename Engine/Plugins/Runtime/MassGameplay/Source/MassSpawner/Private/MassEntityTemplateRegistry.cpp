...
namespace UE::Mass::Debug
{
	...
	const FName TraitOverridden(TEXT("TraitOverridden"));
...
bool FMassEntityTemplateBuildContext::BuildFromTraits(TConstArrayView<UMassEntityTraitBase*> Traits, const UWorld& World)
{
	...
	const EProcessorExecutionFlags WorldProcessingFlags = World.IsGameWorld()
		? UE::Mass::Utils::DetermineProcessorExecutionFlags(&World)
	...
	{
		check(Trait);
		if (static_cast<bool>(Trait->ValidTargetConfig & WorldProcessingFlags)
...
bool FMassEntityTemplateBuildContext::SetTraitBeingProcessed(const UMassEntityTraitBase* Trait)
{
	...
	UE_LOGF(LogMass, Warning, "Attempting to add %ls to FMassEntityTemplateBuildContext while this or another instance of the trait class has already been added."
		, *GetNameSafe(Trait));
...
#define IF_MESSAGES(_)
		...
		for (const UStruct* TypeAdded : TraitData.TypesAdded)
		{
			...
			if (SourceTrait != nullptr)
			{
				if (UE::Mass::Debug::bReportDuplicatedFragmentsAsWarnings)
				{
					...
					UE_CLOGF(SourceTrait != TraitData.Trait
						, LogMass, Warning, "%ls: Fragment %ls already added by %ls. Check the entity config for conflicting traits"
	...
	for (const FTraitData& TraitData : TraitsData)
	{
		for (const UStruct* TypeRequired : TraitData.TypesRequired)
		{
			...
			{
				UE_LOGF(LogMass, Error, "%ls: Missing required element of type %ls"
	...
	for (const UMassEntityTraitBase* Trait : OverriddenTraits)
	{
		IF_MESSAGES(
			FMassDebugger::DebugEvent(UE::Mass::Debug::TraitOverridden, FConstStructView::Make(FMassGenericDebugEvent{Trait}));
		);
		++InfosCount;
	}
