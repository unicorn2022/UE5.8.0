...
TSharedPtr<FMassEntityManager::FEntityCreationContext> UMassSpawnerSubsystem::DoSpawning(const FMassEntityTemplate& EntityTemplate, const int32 NumToSpawn, FConstStructView SpawnData, TSubclassOf<UMassProcessor> InitializerClass, TArray<FMassEntityHandle>& OutEntities)
{
	...
	if (SpawnedEntities.Num())
	{
		...
		// note: using UE::Mass::Utils::CreateEntityCollections rather than CreationContext->GetEntityCollections since the creation context could have been 
		// created outside of this function and reused in a loop to call DoSpawning, in which case CreationContext->GetEntityCollections
		// would contain more than just the freshly created entities
		TArray<FMassArchetypeEntityCollection> EntityCollections;
		UE::Mass::Utils::CreateEntityCollections(*EntityManager, SpawnedEntities, FMassArchetypeEntityCollection::NoDuplicates, EntityCollections);
		EntityManager->BatchSetEntityFragmentValues(EntityCollections, FragmentInstances);
		...
		if (SpawnDataInitializer)
		{
			...
			UE::Mass::Executor::RunProcessorsView(MakeArrayView(&SpawnDataInitializer, 1), ProcessingContext, EntityCollections);
		}
