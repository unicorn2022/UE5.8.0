...
struct FMassRepresentationFragment : public FMassFragment
{
	...
	static constexpr int32 MaxActorVisualReadinessWaitFrames = static_cast<int32>(TNumericLimits<uint8>::Max());
	...
	UPROPERTY()
    FSkinnedMeshInstanceVisualizationDescHandle SkinnedMeshDescHandle;
    ...
    UPROPERTY()
    uint8 bIsPendingDestruction : 1  = false;

private:
	/**
	 * When true, the actor has been spawned (or re-used externally) but is kept disabled while the prior mesh
	 * representation (ISM/SkinnedMesh) covers the visual gap. Cleared once
	 * UMassRepresentationActorManagement::IsActorReadyForRepresentation reports true (or after the
	 * bDoWaitForActorVisualReadiness 1-frame delay). May span multiple frames when an override delays readiness for
	 * asset streaming.
	 */
	UPROPERTY()
	uint8 bIsWaitingForActorVisualReadiness : 1  = false;

	/**
	 * Counts consecutive frames bIsWaitingForActorVisualReadiness has been set, used by UpdateRepresentation to detect runaway
	 * readiness gates (e.g. an override that never returns true) and force-enable the actor after a configurable
	 * cap. Reset to 0 whenever bIsWaitingForActorVisualReadiness transitions to false. Saturates at 255.
	 */
	UPROPERTY()
	uint8 ActorVisualReadinessWaitFrames = 0;
...
public:
	...
	bool IsWaitingForActorVisualReadiness() const
	{
		return bIsWaitingForActorVisualReadiness;
	}

	/** sets bIsWaitingForActorVisualReadiness and resets ActorVisualReadinessWaitFrames */
	void SetIsWaitingForActorVisualReadiness(const bool bNewValue)
	{
		bIsWaitingForActorVisualReadiness = bNewValue;
		ActorVisualReadinessWaitFrames = 0;
	}

	void IncreaseActorVisualReadinessWaitFrames()
	{
		ActorVisualReadinessWaitFrames = static_cast<uint8>(FMath::Min(static_cast<int32>(ActorVisualReadinessWaitFrames + 1), MaxActorVisualReadinessWaitFrames));
	}

	bool IsActorVisualRedinessTimedOut(const int32 Limit) const
	{
		return static_cast<int32>(ActorVisualReadinessWaitFrames >= Limit);
	}
};

USTRUCT()
struct FMassRepresentationAnimationFragment : public FMassFragment
{
    GENERATED_BODY()
...
struct FMassRepresentationParameters : public FMassConstSharedFragment
{
	...
	/**
	 * When switching from ISM/SkinnedMesh to Actor, keep the mesh visible while the spawned actor reports
	 * not-ready (UMassRepresentationActorManagement::IsActorReadyForRepresentation) - typically because a
	 * soft-pointered SkeletalMesh / StaticMesh is still streaming in, or downstream init has not run yet
	 * on the freshly-spawned actor. Bounded by ai.massrepresentation.MaxActorVisualReadinessWaitFrames.
	 */
	UPROPERTY(EditAnywhere, Category = "Mass|Representation", config)
	uint8 bWaitForActorVisualReadiness : 1  = false;
	...
	bool Identical(const FMassRepresentationParameters* InOtherValue, uint32 /*PortFlags*/) const
	{
		...
		return Result == 0
			...
			&& bWaitForActorVisualReadiness == InOtherValue->bWaitForActorVisualReadiness
