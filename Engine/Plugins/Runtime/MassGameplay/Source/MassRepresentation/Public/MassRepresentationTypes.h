...
namespace UE::Mass::ProcessorGroupNames
{
	...
	// Sub-group of Representation containing every UMassVisualizationProcessor subclass.
	// Consumers that depend on CurrentRepresentation being finalised should ExecuteAfter this.
	const FName VisualizationProcessing = FName(TEXT("Representation.VisualizationProcessing"));
}
...
{
	UE_API inline bool IsValidMeshRepresentation(EMassRepresentationType RepresentationType)
	{
		return RepresentationType == EMassRepresentationType::SkinnedMeshInstance || RepresentationType == EMassRepresentationType::StaticMeshInstance;
	}

	UE_API inline bool IsValidActorRepresentation(EMassRepresentationType RepresentationType)
	{
		return RepresentationType == EMassRepresentationType::HighResSpawnedActor || RepresentationType == EMassRepresentationType::LowResSpawnedActor;
	}

	/**
	 * Debug-draw color per EMassRepresentationType, shared across Mass visualization debug paths.
	 * The array is sized to cover every enum value (including None);
	 * a static_assert in MassRepresentationTypes.cpp keeps it in sync with the enum.
	 */
	extern UE_API const FColor RepresentationTypeColors[];
}
...
struct FMassStaticMeshInstanceVisualizationMeshDesc
{
	...
	UE_API FMassStaticMeshInstanceVisualizationMeshDesc();
...
public:
	...
	UE_API void AddBatchedTransform(const FMassEntityHandle EntityHandle, const FTransform& Transform, const FTransform& PrevTransform, TConstArrayView<FISMCSharedDataKey> ExcludeStaticMeshRefs);
	...
	UE_API void AddBatchedCustomDataFloats(const TArray<float>& CustomFloats, const TArray<FISMCSharedDataKey>& ExcludeStaticMeshRefs);
	...
	UE_API void WriteCustomDataFloatsAtStartIndex(int32 StaticMeshIndex, const TArrayView<float>& CustomFloats, const int32 FloatsPerInstance, const int32 StartIndex, const TArray<FISMCSharedDataKey>& ExcludeStaticMeshRefs);
...
protected:
	...
	UE_API void ClearVisualInstance(UInstancedStaticMeshComponent& ISMComponent);
...
using FMassInstancedStaticMeshInfoArrayView = FMassInstancedStaticMeshInfoArrayViewAccessDetector;

#else // ENABLE_MT_DETECTOR

#define MAKE_MASS_INSTANCED_STATIC_MESH_INFO_ARRAY_VIEW(ArrayView, AccessDetector) ArrayView

using FMassInstancedStaticMeshInfoArrayView = TArrayView<FMassInstancedStaticMeshInfo>;
