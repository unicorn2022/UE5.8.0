...
namespace UE::Mass::Representation
{
	...
	extern MASSREPRESENTATION_API int32 bAllowWaitForActorVisualReadiness;

	/**
	 * Computes whether to keep the actor visible an extra frame across an actor->ISM transition.
	 * Combines the cvar kill-switch with the per-archetype params.
	 */
	inline bool ShouldKeepActorExtraFrame(const FMassRepresentationParameters& Params)
	{
		return bAllowKeepActorExtraFrame != 0 && Params.bKeepLowResActors && Params.bKeepActorExtraFrame;
	}
}
...
public:
	UE_API UMassVisualizationProcessor();
