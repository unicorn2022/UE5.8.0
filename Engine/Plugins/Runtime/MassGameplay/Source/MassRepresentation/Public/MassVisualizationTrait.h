...
public:
	...
	/** Instanced skinned mesh information for this agent */
	UPROPERTY(EditAnywhere, Category = "Mass|Visual")
	mutable FSkinnedMeshInstanceVisualizationDesc SkinnedMeshInstanceDesc;
...
protected:
	 ...
	 */
	UE_DEPRECATED(5.8, "Please use SanitizeMeshParams version for ISKM Support")
	...
	/**
	 * Tests whether StaticMeshInstanceDesc and SkinnedMeshInstanceDesc are valid and if not cleans up InOutParams for those occurrences.
	 * @param bStaticMeshDeterminedInvalid if StaticMeshInstanceDesc has already been determined invalid then bStaticMeshDeterminedInvalid
	 *	can be set to `true` to skip the redundant check.
	 * @param bSkinnedMeshDeterminedInvalid if SkinnedMeshInstanceDesc has already been determined invalid then bSkinnedMeshDeterminedInvalid
	 *	can be set to `true` to skip the redundant check.
	 */
	UE_API virtual void SanitizeMeshParams(FMassRepresentationParameters& InOutParams, const bool bStaticMeshDeterminedInvalid = false, const bool bSkinnedMeshDeterminedInvalid = false) const;
