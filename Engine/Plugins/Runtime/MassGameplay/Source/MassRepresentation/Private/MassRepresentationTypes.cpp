...
{
const FColor RepresentationTypeColors[] =
{
	FColor::Red,     // HighResSpawnedActor
	FColor::Yellow,  // LowResSpawnedActor
	FColor::Cyan,    // SkinnedMeshInstance
	FColor::Emerald, // StaticMeshInstance
	FColor::White,   // None
};

static_assert(UE_ARRAY_COUNT(RepresentationTypeColors) == static_cast<uint8>(EMassRepresentationType::None) + 1,
	"RepresentationTypeColors must have one entry per EMassRepresentationType value. "
	"If you added a new value to the enum, append a matching color entry here and update this assertion.");
