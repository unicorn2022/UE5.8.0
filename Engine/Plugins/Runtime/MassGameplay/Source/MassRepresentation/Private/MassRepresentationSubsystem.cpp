...
FMassInstancedSkinnedMeshInfoArrayView UMassRepresentationSubsystem::GetMutableInstancedSkinnedMeshInfos()
{
	if (LIKELY(VisualizationComponent))
	{
		FMassInstancedSkinnedMeshInfoArrayView View = VisualizationComponent->GetMutableInstanceVisualInfos();
		return MoveTemp(View);
	}
	...
	// It can also happen during world teardown if this is called after actors are destroyed.
	// This is not hit during normal execution, but we still need a default return value.
	FMassInstancedSkinnedMeshInfoArrayView EmptyView = MAKE_MASS_INSTANCED_SKINNED_MESH_INFO_ARRAY_VIEW(MakeArrayView<FMassInstancedSkinnedMeshInfo>(nullptr, 0), InstancedSkinnedMeshInfosDetector);
	return MoveTemp(EmptyView);
}
...
AActor* UMassRepresentationSubsystem::GetOrRequestSpawnActorFromTemplate(const FMassEntityHandle MassAgent, const FTransform& Transform
	...
	, FMassActorPreSpawnDelegate ActorPreSpawnDelegate, FMassActorPostSpawnDelegate ActorPostSpawnDelegate)
{
	...
	{
		UE_LOGF(LogMassRepresentation, Error, "Template actor type %i is not referring to a valid type", TemplateActorIndex);
		return nullptr;
	}
...
bool UMassRepresentationSubsystem::ReleaseTemplateActorInternal(const int16 TemplateActorIndex, AActor* ActorToRelease, bool bImmediate)
{
	...
	if (!TemplateActors.IsValidIndex(TemplateActorIndex))
	{
		UE_LOGF(LogMassRepresentation, Error, "Template actor type %i is not referring a valid type", TemplateActorIndex);
		return false;
	}
...
bool UMassRepresentationSubsystem::CancelSpawningInternal(const int16 TemplateActorIndex, FMassActorSpawnRequestHandle& SpawnRequestHandle, const bool bImmediateActorRelease)
{
	...
	if (SpawnRequestHandle.IsValid() == false)
	{
		return false;
	}

	check(ActorSpawnerSubsystem);
	if (ActorSpawnerSubsystem->IsSpawnRequestHandleValid(SpawnRequestHandle) == false)
	{
		UE_VLOG_UELOG(this, LogMassRepresentation, Error
			, TEXT("Attempting to cancel spawning using a NO LONGER VALID spawn request handle [i: %d, s/n: %d]")
			, SpawnRequestHandle.GetIndex(), SpawnRequestHandle.GetSerialNumber());
		return false;
	}
...
void UMassRepresentationSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	check(SimSettings);
	RetryMovedDistanceSq = FMath::Square(SimSettings->DesiredActorFailedSpawningRetryMoveDistance);
	RetryTimeInterval = SimSettings->DesiredActorFailedSpawningRetryTimeInterval;
}
...
FSkinnedMeshInstanceVisualizationDescHandle UMassRepresentationSubsystem::FindOrAddSkinnedMeshVisualizationDesc(const FSkinnedMeshInstanceVisualizationDesc& Desc)
{
	if (LIKELY(VisualizationComponent))
	{
		return VisualizationComponent->FindOrAddSkinnedMeshInstanceVisualDesc(Desc);
	}
