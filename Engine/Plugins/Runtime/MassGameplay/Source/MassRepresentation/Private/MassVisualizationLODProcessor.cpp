	...
	if (UE::Mass::Representation::Debug::DebugRepresentationLOD == 1 || UE::Mass::Representation::Debug::DebugRepresentationLOD >= 3)
	{
		...
		if (UWorld* DebugDrawingWorld = UE::Mass::Debug::GetDebugDrawWorld(Context))
		{
			DebugEntityQuery.ForEachEntityChunk(Context, [DebugDrawingWorld](FMassExecutionContext& Context)
			{
				...
				LODSharedFragment.LODCalculator.DebugDisplaySignificantLOD(Context, RepresentationLODList, TransformList, DebugDrawingWorld, UE::Mass::Representation::Debug::DebugRepresentationLODMaxSignificance);
			});
		}
	}
	// Optional vislog
