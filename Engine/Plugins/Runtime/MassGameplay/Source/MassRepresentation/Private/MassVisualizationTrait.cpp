...
void UMassVisualizationTrait::BuildTemplate(FMassEntityTemplateBuildContext& BuildContext, const UWorld& World) const
{
	...
	{
		UE_LOGF(LogMassRepresentation, Error, "Expecting a valid class for the representation subsystem");
	...
	{
		UE_LOGF(LogMassRepresentation, Error, "Expecting a valid class for the representation actor management");
	}
	...
	bool bSkinnedMeshDescriptionValid = SkinnedMeshInstanceDesc.IsValid();
	if (bSkinnedMeshDescriptionValid)
	{
		if (!BuildContext.IsInspectingData())
		{
			RepresentationFragment.SkinnedMeshDescHandle = RepresentationSubsystem->FindOrAddSkinnedMeshVisualizationDesc(SkinnedMeshInstanceDesc);
			ensureMsgf(RepresentationFragment.SkinnedMeshDescHandle.IsValid()
				, TEXT("Expected to get a valid SkinnedMeshDescHandle since we already checked that SkinnedMeshInstanceDesc is valid"));
			// if the unexpected happens and SkinnedMeshDescHandle is not valid we're going to treat it as if SkinnedMeshInstanceDesc
			// was not valid in the first place and handle it accordingly in a moment
			bSkinnedMeshDescriptionValid = RepresentationFragment.SkinnedMeshDescHandle.IsValid();
		}
	}
	...
	if (bStaticMeshDescriptionValid && bSkinnedMeshDescriptionValid)
	{
	...
	else
	{
		...
		SanitizeMeshParams(ParamsCopy, !bStaticMeshDescriptionValid, !bSkinnedMeshDescriptionValid);
...
{
	SanitizeMeshParams(InOutParams, bStaticMeshDeterminedInvalid);
}

void UMassVisualizationTrait::SanitizeMeshParams(FMassRepresentationParameters& InOutParams, const bool bStaticMeshDeterminedInvalid /*= false*/, const bool bSkinnedMeshDeterminedInvalid /*= false*/) const
{
	if (bStaticMeshDeterminedInvalid)
	{
	...
	if (bSkinnedMeshDeterminedInvalid)
	{
		for (int32 LODIndex = 0; LODIndex < EMassLOD::Max; ++LODIndex)
		{
			if (InOutParams.LODRepresentation[LODIndex] == EMassRepresentationType::SkinnedMeshInstance)
			{
				InOutParams.LODRepresentation[LODIndex] = EMassRepresentationType::None;
			}
		}
			...
			&& StaticMeshInstanceDesc.IsValid() == false)
		{
			for (int32 LODIndex = 0; LODIndex < EMassLOD::Max; ++LODIndex)
			{
				if (Params.LODRepresentation[LODIndex] == EMassRepresentationType::StaticMeshInstance)
				{
					...
					UE_LOGF(LogMassRepresentation, Error, "Trait %ls is using StaticMeshInstance representation type for "
						"LODRepresentation[%ls] while the trait's StaticMeshInstanceDesc is not valid (has no Meshes). Entities "
