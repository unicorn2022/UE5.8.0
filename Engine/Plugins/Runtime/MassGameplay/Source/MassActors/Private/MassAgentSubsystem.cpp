...
FMassEntityTemplateID UMassAgentSubsystem::RegisterAgentComponent(UMassAgentComponent& AgentComp)
{
	...
	// This path is hit when a regular (non-puppet) actor that owns a UMassAgentComponent gets
	// unregistered (e.g. via EndPlay or RemoveFromWorld) and then registered again (e.g. re-added
	// to the world or re-activated). In that scenario the component still carries a valid
	// TemplateID from its previous registration. We intentionally bail out with a warning rather
	// than re-register, because the old Mass entity handle may still be live and double-registration
	// would create orphaned entity data. Callers that need a fresh registration should first ensure
	// the component has been fully unregistered (TemplateID invalidated) before calling this again.
...
void UMassAgentSubsystem::UnregisterAgentComponent(UMassAgentComponent& AgentComp)
{
	...
	if (AgentComp.IsPuppetReplicationPending())
	{
		// The actor was destroyed (e.g., by Iris relevancy) before its NetID was replicated
		// via the old bubble system. No Mass entity exists yet, no cleanup needed —
		// just transition to None.
		AgentComp.PuppetReplicationAborted();
	}
	else if (AgentComp.IsPuppet())
	{
