...
{
	bool bBreakOnFailedAgentCheck = true;

	namespace
	{
		FAutoConsoleVariableRef CVars[] = 
		{
			{TEXT("mass.debug.BreakOnFailedAgentCheck"), bBreakOnFailedAgentCheck, TEXT("Enabled by default. If disabled, won't cause check failures when AgentComponent is in an unexpected state.")}
		};
	}
}
...
#define MASSAGENT_CHECK( condition, Format, ... ) \
	...
	checkf(condition || !UE::Mass::Debug::bBreakOnFailedAgentCheck, Format, ##__VA_ARGS__);
...
void UMassAgentComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.MemberProperty && PropertyChangedEvent.MemberProperty->GetFName() == NAME_EntityConfig)
	{
...
void UMassAgentComponent::PuppetReplicationAborted()
{
	MASSAGENT_CHECK(State == EAgentComponentState::PuppetPendingReplication,
		TEXT("%s is expecting to be in state[PuppetPendingReplication] but is in %s"), ANSI_TO_TCHAR(__FUNCTION__), *UEnum::GetValueAsString(State));
	// No Mass entity exists yet in this state (no handle, no NetID). Just reset.
		...
		case EAgentComponentState::PuppetPaused:
		{
			...
			if (bValidAgentHandle)
			{
				if (const UMassEntitySubsystem* EntitySubsystem = UWorld::GetSubsystem<UMassEntitySubsystem>(GetWorld()))
				{
					...
					MASSAGENT_CHECK(bIsValidEntity, TEXT("Exepecting a valid entity in state %s"), *UEnum::GetValueAsString(State))
					if (bIsValidEntity)
					{
...
{
	if (ensureMsgf(IsPuppet(), TEXT("%s can only be called when the the mass agent component acts as a puppet. Current state is %s"), ANSI_TO_TCHAR(__FUNCTION__), *UEnum::GetValueAsString(State))
		&& IsPuppetPaused() != bPause)
	{
		if (UMassAgentSubsystem* AgentSubsystem = UWorld::GetSubsystem<UMassAgentSubsystem>(GetWorld()))
		{
			if (bPause)
...
{
	checkf(IsNetSimulating(), TEXT("Expecting a replicated puppet"));
	MASSAGENT_CHECK(State == EAgentComponentState::None,
		TEXT("%s is expecting to be in state[None] but is in %s"), ANSI_TO_TCHAR(__FUNCTION__), *UEnum::GetValueAsString(State));
