...
namespace UE::Mass::Replication
{
	...
	template<typename T>
	concept TIsCallable = std::is_class_v<typename TRemoveReference<T>::Type>
		&& requires(T t)
		{
			&TRemoveReference<T>::Type::operator();
		};
}; //namespace UE::Mass::Replication
...
public:
	...
	void MarkItemDirty(FFastArraySerializerItem& Item) const;
	AgentArrayItem& GetAgentItem(const FMassReplicatedAgentHandle Handle) const;
	...
	void SetBubbleData(const FMassReplicatedAgentHandle Handle, const TReplicatedDataType& NewData)
	{
		AgentArrayItem& Item = GetAgentItem(Handle);

		checkf(Item.Agent.GetNetID().IsValid(),
			TEXT("Data should not be updated on Items that have an Invalid ID! First Add the Agent!"));

		TReplicatedDataType& OldData = Item.Agent.template GetReplicatedData<TReplicatedDataType>();

		if (UE::Mass::Replication::CheckIsDirty(OldData, NewData))
		{
			OldData = NewData;
			MarkItemDirty(Item);
		}
...
protected:
	...
	/**
	 * the TEventHandlers template parameters below require the following callable members (functions, lambda, functors):
	 * {
	 *		AddRequirementsForSpawnQuery;
	 *		CacheFragmentViewsForSpawnQuery;
	 *		SetSpawnedEntityData;
	 *		SetModifiedEntityData;
	 *		GetSpawnQuery -> FMassEntityQuery;
	 * }
	 * Note:
	 *	- PostReplicatedAddEntitiesHelper doesn't call SetModifiedEntityData
	 *	- PostReplicatedChangeHelper only calls SetModifiedEntityData
	 *	- it's recommended to supply GetSpawnQuery by deriving from UE::Mass::Replication::TCachedSpawnQuery
	 */

	/** Called from TClientBubbleHandlerBase derived classes in PostReplicatedAd() */
	template<typename TEventHandlers>
	void PostReplicatedAddHelper(const TArrayView<int32> AddedIndices, TEventHandlers& EventHandlers);
	...
	template<typename TEventHandlers>
	void PostReplicatedAddEntitiesHelper(const TArrayView<int32> AddedIndices, TEventHandlers& EventHandlers);
	...
	template<typename TEventHandlers>
	requires (!UE::Mass::Replication::TIsCallable<TEventHandlers>)
	void PostReplicatedChangeHelper(const TArrayView<int32> ChangedIndices, TEventHandlers& HandlersPack);
...
template <typename AgentArrayItem>
UWorld* TClientBubbleHandlerBase<AgentArrayItem>::GetWorld() const
{
	return Serializer->GetWorld();
}
...
template <typename AgentArrayItem>
void TClientBubbleHandlerBase<AgentArrayItem>::MarkItemDirty(FFastArraySerializerItem& Item) const
{
	Serializer->MarkItemDirty(Item);
}
...
template <typename AgentArrayItem>
AgentArrayItem& TClientBubbleHandlerBase<AgentArrayItem>::GetAgentItem(const FMassReplicatedAgentHandle Handle) const
{
	check(AgentHandleManager.IsValidHandle(Handle));
        
	const int32 AgentsIdx = AgentLookupArray[Handle.GetIndex()].AgentsIdx;
	
	return (*Agents)[AgentsIdx];
}
...
template<typename AgentArrayItem>
template<typename TEventHandlers>
void TClientBubbleHandlerBase<AgentArrayItem>::PostReplicatedAddHelper(const TArrayView<int32> AddedIndices, TEventHandlers& EventHandlers)
{
	...
	for (int32 Idx : AddedIndices)
	{
		...
		case UMassReplicationSubsystem::EFindOrAddMassEntityInfo::FoundOlderReplicationID:
		{
			...
			if (EntityInfo->Entity.IsSet())
			{
				...
				EventHandlers.SetModifiedEntityData(EntityView, AddedItem.Agent);
			}
	...
	PostReplicatedAddEntitiesHelper(AgentsToAddArray, EventHandlers);
}
...
void TClientBubbleHandlerBase<AgentArrayItem>::PostReplicatedAddHelper(const TArrayView<int32> AddedIndices, FAddRequirementsForSpawnQueryFunction AddRequirementsForSpawnQuery
	, FCacheFragmentViewsForSpawnQueryFunction CacheFragmentViewsForSpawnQuery, FSetSpawnedEntityDataFunction SetSpawnedEntityData, FSetModifiedEntityDataFunction SetModifiedEntityData)
{
	struct FEventHandlers : UE::Mass::Replication::TCachedSpawnQuery<FEventHandlers>
	{
		using UE::Mass::Replication::TCachedSpawnQuery<FEventHandlers>::GetSpawnQuery;
		FAddRequirementsForSpawnQueryFunction AddRequirementsForSpawnQuery;
		FCacheFragmentViewsForSpawnQueryFunction CacheFragmentViewsForSpawnQuery;
		FSetSpawnedEntityDataFunction SetSpawnedEntityData;
		FSetModifiedEntityDataFunction SetModifiedEntityData;
	};

	FEventHandlers EventHandlers = {
		.AddRequirementsForSpawnQuery = AddRequirementsForSpawnQuery,
		.CacheFragmentViewsForSpawnQuery = CacheFragmentViewsForSpawnQuery,
		.SetSpawnedEntityData = SetSpawnedEntityData,
		.SetModifiedEntityData = SetModifiedEntityData
	};

	PostReplicatedAddHelper(AddedIndices, EventHandlers);
}

template<typename AgentArrayItem>
template<typename TEventHandlers>
void TClientBubbleHandlerBase<AgentArrayItem>::PostReplicatedAddEntitiesHelper(const TArrayView<int32> AddedIndices, TEventHandlers& EventHandlers)
{
	check(Serializer);
	...
	FMassEntityQuery& Query = EventHandlers.GetSpawnQuery(EntityManager.AsShared());
	...
	for (const TPair<FMassEntityTemplateID, TArray<typename AgentArrayItem::FReplicatedAgentType*>>& Item : AgentsSpawnMap)
	{
		...
		// if there were any initializer or observer processors it's possible that the spawned entities
		// are no longer in the archetype indicated by the template. We need to create the entity collections
		// the slower way (i.e. assuming we don't know the current archetype)
		// Note that we're not using CreationContext->GetEntityCollections (with CreationContext being
		// the return value of SpawnerSubsystem->SpawnEntities) because it might contain unrelated entities.
		...
		// Note that the way this functionality is implemented here it's not possible to parallelize the lambda's
		// execution, due to using AgentsSpawnIdx. If attempt is made at parallelizing the work this detail needs to be changed.
		int32 AgentsSpawnIdx = 0;

		Query.ForEachEntityChunkInCollections(EntityCollections, ExecContext
			, [&AgentsSpawn, &AgentsSpawnIdx, this, ReplicationSubsystem, &ExecContext, &EventHandlers](FMassExecutionContext& Context)
			{
				EventHandlers.CacheFragmentViewsForSpawnQuery(Context);
				...
				for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
				{
					...
					FMassEntityView EntityView(Context.GetEntityManagerChecked(), Entity);
					EventHandlers.SetSpawnedEntityData(EntityView, AgentSpawn, EntityIt);
	...
	, FCacheFragmentViewsForSpawnQueryFunction CacheFragmentViewsForSpawnQuery, FSetSpawnedEntityDataFunction SetSpawnedEntityData)
{
	...
	PostReplicatedAddEntitiesHelper(AddedIndices, EventHandlers);
}
#endif // UE_REPLICATION_COMPILE_CLIENT_CODE
...
template<typename TEventHandlers>
requires (!UE::Mass::Replication::TIsCallable<TEventHandlers>)
void TClientBubbleHandlerBase<AgentArrayItem>::PostReplicatedChangeHelper(const TArrayView<int32> ChangedIndices, TEventHandlers& HandlersPack)
{
	FMassEntityManager& EntityManager = Serializer->GetEntityManagerChecked();
	...
	for (int32 Idx : ChangedIndices)
	{
		...
		{
			FMassEntityView EntityView(EntityManager, EntityInfo->Entity);
			HandlersPack.SetModifiedEntityData(EntityView, ChangedItem.Agent);
		}
	}
}
#endif //UE_REPLICATION_COMPILE_CLIENT_CODE
