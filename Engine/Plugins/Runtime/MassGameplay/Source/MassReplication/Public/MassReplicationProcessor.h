...
UCLASS(MinimalAPI)
class UMassReplicationEntityDestructionObserver : public UMassObserverProcessor
{
	GENERATED_BODY()

public:
	MASSREPLICATION_API UMassReplicationEntityDestructionObserver();
...
template<typename AgentArrayItem, typename CacheViewsCallback, typename AddEntityCallback, typename ModifyEntityCallback, typename RemoveEntityCallback>
void UMassReplicatorBase::CalculateClientReplication(FMassExecutionContext& Context, FMassReplicationContext& ReplicationContext, CacheViewsCallback&& CacheViews
	, AddEntityCallback&& AddEntity, ModifyEntityCallback&& ModifyEntity, RemoveEntityCallback&& RemoveEntity)
{
	...
	for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
	{
		...
		else
		{
			...
			if (AgentData.Handle.IsValid())
			{
				if constexpr (std::is_invocable_v<RemoveEntityCallback, FMassExecutionContext&, FMassEntityHandle, FMassReplicatedAgentHandle, FMassClientHandle>)
				{
					RemoveEntity(Context, Context.GetEntity(EntityIt), AgentData.Handle, ClientHandle);
				}
				else
				{
	...
	FMassClientReplicationInfo& ClientReplicationInfo = ReplicationContext.ReplicationSubsystem.GetMutableClientReplicationInfoChecked(ClientHandle);
	for (FMassReplicationAgentDataMap::TIterator It = ClientReplicationInfo.AgentsData.CreateIterator(); It; ++It)
	{
		...
		if (AgentData.Handle.IsValid() && AgentData.bPendingDestruction)
		{
			if constexpr (std::is_invocable_v<RemoveEntityCallback, FMassExecutionContext&, FMassEntityHandle, FMassReplicatedAgentHandle, FMassClientHandle>)
			{
				RemoveEntity(Context, It.Key(), AgentData.Handle, ClientHandle);
			}
			else
			{
				RemoveEntity(Context, AgentData.Handle, ClientHandle);
