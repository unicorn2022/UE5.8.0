	...
	: EntityQuery(*this)
{
	...
	ObservedTypes.Add(FMassNetworkIDFragment::StaticStruct());
...
{
	// Setting the LODMaxCoundPerViewer as the MAXCount as the processor will be called for each client and will recalculate it per client
	LODCalculator.Initialize(Params.LODDistance, Params.BufferHysteresisOnDistancePercentage / 100.0f, Params.LODMaxCountPerViewer); 
	BubbleInfoClassHandle = ReplicationSubsystem.GetBubbleInfoClassHandle(Params.BubbleInfoClass);
	checkf(BubbleInfoClassHandle.IsValid(), TEXT("%s has not been registered as a bubble info class (yet)"), *GetNameSafe(Params.BubbleInfoClass.Get()));
