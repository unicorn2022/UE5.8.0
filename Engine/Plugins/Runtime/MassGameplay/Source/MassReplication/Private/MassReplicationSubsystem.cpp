...
FMassBubbleInfoClassHandle UMassReplicationSubsystem::RegisterBubbleInfoClass(const TSubclassOf<AMassClientBubbleInfoBase>& BubbleInfoClass)
{
	...
	{
		UE_LOGF(LogMassReplication, Log, "UMassReplicationSubsystem: Trying to RegisterBubbleInfoClass() twice with the same BubbleInfoClass, Only one BubbleInfoClass will be registered for this type");
...
FMassEntityHandle UMassReplicationSubsystem::ResetEntityIfValid(const FMassNetworkID NetworkID, int32 ReplicationID)
{
	...
	if (ensureMsgf(EntityInfo, TEXT("EntityData must have been added to EntityInfoMap!")))
	{
...
void UMassReplicationSubsystem::NotifyEntityDestroyed(FMassEntityHandle EntityHandle)
{
	for (FMassClientReplicationInfo& ClientReplicationInfo : ClientsReplicationInfo)
	{
		if (FMassReplicatedAgentData* AgentDataPtr = ClientReplicationInfo.AgentsData.Find(EntityHandle))
		{
			AgentDataPtr->bPendingDestruction = true;
		}
	}
}
