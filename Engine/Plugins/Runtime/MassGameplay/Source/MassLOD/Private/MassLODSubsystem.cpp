...
{
	const FName LODCollector = TEXT("LODCollector");
	const FName LOD = TEXT("LOD");
}
...
namespace UE::MassLOD
{
	...
	namespace Debug
	{
		...
		bool GetSubsystemAndBoolArgument(const TArray<FString>& Args, UWorld* World, UMassLODSubsystem*& OutMassLODSubsystem, bool& bOutBool)
		{
			...
			{
				UE_LOGF(LogConsoleResponse, Display, "Error: Unable to fetch MassLODSubsystem instance");
				return false;
			}
			...
			{
				UE_LOGF(LogConsoleResponse, Display, "Error: parameter must be an integer or a boolean");
				return false;
			}
...
void UMassLODSubsystem::SynchronizeViewers()
{
	...
	for (FViewerInfo& ViewerInfo : Viewers)
	{
		...
		if (APlayerController* ViewerAsPlayerController = ViewerInfo.GetPlayerController())
		{
			...
			// LocalPlayer is null on a dedicated server; leave FOV / AspectRatio at their defaults.
			const ULocalPlayer* LocalPlayer = ViewerAsPlayerController->GetLocalPlayer();
			if (LocalPlayer && LocalPlayer->ViewportClient && LocalPlayer->ViewportClient->Viewport)
			{
				...
				if (LocalPlayer->GetProjectionData(LocalPlayer->ViewportClient->Viewport, ProjectionData))
				{
					...
					// M[3][3] == 0 for perspective matrices, 1 for orthographic. FOV / aspect
					// recovery from M[0][0] / M[1][1] only makes sense for perspective.
					if (FMath::Abs(Proj.M[3][3]) < UE_KINDA_SMALL_NUMBER)
					{
						const double TanHalfH = (FMath::Abs(Proj.M[0][0]) > UE_KINDA_SMALL_NUMBER) ? 1.0 / FMath::Abs(Proj.M[0][0]) : 1.0;
						const double TanHalfV = (FMath::Abs(Proj.M[1][1]) > UE_KINDA_SMALL_NUMBER) ? 1.0 / FMath::Abs(Proj.M[1][1]) : 1.0;
						ViewerInfo.FOV = static_cast<float>(FMath::RadiansToDegrees(FMath::Atan(TanHalfH) * 2.0));
						ViewerInfo.AspectRatio = static_cast<float>(TanHalfH / TanHalfV);
					}
				}
			}
		}
...
{
	using ClientIndexType = decltype(FViewerInfo::EditorViewportClientIndex);
	if (!ensureMsgf(ClientIndex >= std::numeric_limits<ClientIndexType>::min() && ClientIndex <= std::numeric_limits<ClientIndexType>::max()
		, TEXT("Editor viewport client index %d does not fit in storage type (range %d..%d); skipping LOD viewer registration.")
		, ClientIndex, int32(std::numeric_limits<ClientIndexType>::min()), int32(std::numeric_limits<ClientIndexType>::max())))
	{
		return;
	}
