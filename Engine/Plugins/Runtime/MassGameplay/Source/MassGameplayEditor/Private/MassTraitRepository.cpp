...
#define OVERRIDABLE_SEVERITY(DefaultSeverity) UE::Mass::Debug::MassSeverityToMessageSeverity(DefaultSeverity, SeverityOverride)
	...
	if (EventName == MissingTraitMessageName)
	{
		if (const FMassMissingTraitMessage* MissingTraitMessage = Payload.GetPtr<const FMassMissingTraitMessage>())
		{
			...
			else
			{
				...
				{
					IntroMessage->AddText(
						FText::FormatOrdered(LOCTEXT("ExecutionFlags", "(ExecutionFlags: {0}) "),
						UEnum::GetDisplayValueAsText(MissingTraitMessage->RequestingTrait->ValidTargetConfig))
					);
				}
	...
	else if (EventName == UE::Mass::Debug::TraitOverridden)
	{
		if (const FMassGenericDebugEvent* GenericEvent = Payload.GetPtr<const FMassGenericDebugEvent>())
		{
			const UMassEntityTraitBase* Trait = Cast<UMassEntityTraitBase>(GenericEvent->Context);
			FMessageLog MessageLog(UE::Mass::Editor::MessageLogPageName);
			MessageLog.AddMessage(FTokenizedMessage::Create(OVERRIDABLE_SEVERITY(EMessageSeverity::Info)))
				->AddToken(FUObjectToken::Create(Trait))
				->AddToken(FTextToken::Create(LOCTEXT("TraitOverridden", "trait was overridden by another instance. Make sure that's intended.")));
		}
	}
...
void UMassTraitRepository::OnNewTraitType(UMassEntityTraitBase& Trait)
{
	...
	for (FMassFragmentBitSet::FIndexIterator It = Composition.GetElementsBitSet().GetIndexIterator(); It; ++It)
	{
		const FName StructName = Composition.GetElementsBitSet().DebugGetStructTypeName(*It);
