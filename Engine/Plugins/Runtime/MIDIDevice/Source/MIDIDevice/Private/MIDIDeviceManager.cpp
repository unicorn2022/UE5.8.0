...
void UMIDIDeviceManager::StartupMIDIDeviceManager()
{
	if (ensure(!bIsInitialized))
	{
		...
		else
		{
			...
			UE_LOGF(LogMIDIDevice, Error, "Unable to open initialize the MIDI device manager (PortMidi error: %ls).  You won't be use MIDI features in this session.", *ErrorText);
		}
	}
}
	...
	for(TObjectIterator<UMIDIDeviceControllerBase> ControllerIter; ControllerIter; ++ControllerIter)
	{
		...
		if(MIDIDeviceController != nullptr && IsValidChecked(MIDIDeviceController))
		{
			...
			if (!bStartedSuccessfully)
			{
				...
				UE_LOGF(LogMIDIDevice, Warning, "MIDI Device Controller re-initialization failed.");
			}
		}
	}
}
...
void UMIDIDeviceManager::FindMIDIDevicesInternal(TArray<FFoundMIDIDevice>& OutMIDIDevices)
{
	if(bIsInitialized)
	{
		...
		for(PmDeviceID PMDeviceID = 0; PMDeviceID < PMDeviceCount; ++PMDeviceID)
		{
			...
			{
				UE_LOGF(LogMIDIDevice, Error, "Unable to query information about MIDI device (PortMidi device ID: %i).  This device won't be available for input or output.", PMDeviceID);
			}
		}
	}
	else
	{
		UE_LOGF(LogMIDIDevice, Warning, "Find MIDI Devices cannot be used because the MIDI device manager failed to initialize.  Check earlier in the log to see why.");
	}
}
...
void UMIDIDeviceManager::GetMIDIInputDeviceIDByName(const FString DeviceName, int32& DeviceID)
{
	...
	{
		UE_LOGF(LogMIDIDevice, Warning, "MIDI device manager not initialized");
	}
}
...
UMIDIDeviceController* UMIDIDeviceManager::CreateMIDIDeviceController(const int32 DeviceID, const int32 MIDIBufferSize)
{
	...
	if (bIsInitialized)
	{
		...
		if (!bStartedSuccessfully)
		{
			...
			UE_LOGF(LogMIDIDevice, Warning, "Create MIDI Device Controller wasn't able to create the controller successfully. Returning a null reference.");
		}
	}
	else
	{
		UE_LOGF(LogMIDIDevice, Warning, "Create MIDI Device Controller isn't able to create a controller because the MIDI Device Manager failed to initialize. Look earlier in the log to see why it failed to startup.  Returning a null reference.");
	}
