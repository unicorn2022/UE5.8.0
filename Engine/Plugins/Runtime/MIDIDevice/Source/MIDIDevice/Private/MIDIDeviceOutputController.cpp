...
void UMIDIDeviceOutputController::StartupDevice(const int32 InitDeviceID, const int32 InitMIDIBufferSize, bool& bOutWasSuccessful)
{
	...
	if(PMDeviceInfo != nullptr)
	{
		...
		else
		{
			...
			UE_LOGF(LogMIDIDevice, Error, "Unable to open output connection to MIDI device ID %i (%ls) (PortMidi error: %ls).", PMDeviceID, ANSI_TO_TCHAR(PMDeviceInfo->name), *ErrorText);
		}
	}
	else
	{
...
void UMIDIDeviceOutputController::ShutdownDevice()
{
	if (this->PMMIDIStream != nullptr)
	{
		...
		{
			const FString ErrorText = MIDIDeviceInternal::ParsePmError(PMError);
			UE_LOGF(LogMIDIDevice, Error, "Encounter an error when closing the output connection to MIDI device ID %i (%ls) (PortMidi error: %ls).", this->DeviceID, *this->DeviceName, *ErrorText);
		}
