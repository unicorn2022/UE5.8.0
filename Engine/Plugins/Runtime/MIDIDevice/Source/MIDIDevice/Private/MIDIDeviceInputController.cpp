...
void UMIDIDeviceInputController::StartupDevice(const int32 InitDeviceID, const int32 InitMIDIBufferSize, bool& bOutWasSuccessful)
{
	...
	if(InitMIDIBufferSize > 0)
	{
		...
		if(PMDeviceInfo != nullptr)
		{
			...
			{
				UE_LOGF(LogMIDIDevice, Warning, "Failed to bind to MIDI device '%ls' (ID: %i): Device is already in use", ANSI_TO_TCHAR(PMDeviceInfo->name), PMDeviceID);
				return;
			}
			...
			{
				UE_LOGF(LogMIDIDevice, Warning, "Failed to bind to MIDI device '%ls' (ID: %i): Device not setup to receive MIDI", ANSI_TO_TCHAR(PMDeviceInfo->name), PMDeviceID);
				return;
			}
			...
			else
			{
				...
				UE_LOGF(LogMIDIDevice, Error, "Unable to open input connection to MIDI device ID %i (%ls) (PortMidi error: %ls).", PMDeviceID, ANSI_TO_TCHAR(PMDeviceInfo->name), *ErrorText);
			}
		}
		else
		{
			UE_LOGF(LogMIDIDevice, Error, "Unable to query information about MIDI device (PortMidi device ID: %i).", PMDeviceID);
		}
	}
	else
	{
		UE_LOGF(LogMIDIDevice, Error, "The specified MIDI Buffer Size must be greater than zero.");
	}
}
...
void UMIDIDeviceInputController::ShutdownDevice()
{
	if(this->PMMIDIStream != nullptr)
	{
		...
		{
			const FString ErrorText = MIDIDeviceInternal::ParsePmError(PMError);
			UE_LOGF(LogMIDIDevice, Error, "Encounter an error when closing the input connection to MIDI device ID %i (%ls) (PortMidi error: %ls).", this->DeviceID, *this->DeviceName, *ErrorText);
		}
...
void UMIDIDeviceInputController::ProcessIncomingMIDIEvents()
{
	if(this->PMMIDIStream != nullptr)
	{
		...
		if(PMEventCount < 0)
		{
			...
			UE_LOGF(LogMIDIDevice, Error, "Encountered an error when processing incoming MIDI events for device ID %i (%ls) (PortMidi error: %ls).", this->DeviceID, *this->DeviceName, *ErrorText);
		}
		else
		{
