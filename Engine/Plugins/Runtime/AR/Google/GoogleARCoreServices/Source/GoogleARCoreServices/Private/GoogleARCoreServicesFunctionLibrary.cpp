...
public:
	...
	virtual void UpdateOperation(FLatentResponse& Response) override
	{
		if (!bHostStarted)
		{
			UE_LOGF(LogGoogleARCoreServices, Verbose, "Creating and Hosting CloudARPin.");
		...
		else 
		{
			...
			UE_LOGF(LogGoogleARCoreServices, Verbose, "Creating and Hosting Finished with TaskResult: %d.", (int)OutHostingResult);
			UE_LOGF(LogGoogleARCoreServices, Verbose, "CloudARPin Id: %ls", *NewCloudARPin->GetCloudID());
			OutCloudARPin = NewCloudARPin;
			Response.FinishAndTriggerIf(true, ExecutionFunction, OutputLink, CallbackTarget);
		}
	}
...
void UGoogleARCoreServicesFunctionLibrary::CreateAndHostCloudARPinLatentAction(UObject* WorldContextObject, struct FLatentActionInfo LatentInfo, UARPin* ARPinToHost, int32 LifetimeInDays, EARPinCloudTaskResult& OutHostingResult, UCloudARPin*& OutCloudARPin)
{
	...
	{
		UE_LOGF(LogGoogleARCoreServices, Verbose, "Create Host CloudARPin Action. UUID: %d", LatentInfo.UUID);
		...
		{
			UE_LOGF(LogGoogleARCoreServices, Verbose, "Skipping Create Host CloudARPin latent action.");
		}
	}
}
...
void UGoogleARCoreServicesFunctionLibrary::CreateAndResolveCloudARPinLatentAction(UObject* WorldContextObject, struct FLatentActionInfo LatentInfo, FString CloudId, EARPinCloudTaskResult& OutAcquiringResult, UCloudARPin*& OutCloudARPin)
{
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			UE_LOGF(LogGoogleARCoreServices, Log, "Skipping Create Host CloudARPin latent action %d.", LatentInfo.UUID);
		}
	}
}
