...
#endif
		default:
			UE_LOGF(LogGoogleARCoreServices, Warning, "ToCloudTaskResult defaulting to EARPinCloudTaskResult::Failed for ArStatus %d", Status);
		...
		if (NewMode == EARPinCloudMode::Enabled)
		{
			if (SessionHandle == nullptr)
			{
				...
				case AR_UNAVAILABLE_DEVICE_NOT_COMPATIBLE:
					UE_LOGF(LogGoogleARCoreServices, Error, "Failed to enable CloudARPin. Device is not compatable.");
					break;
				case AR_ERROR_INVALID_ARGUMENT:
					UE_LOGF(LogGoogleARCoreServices, Error, "Failed to enable CloudARPin. Invalid API Key");
					break;
				default:
...
void FGoogleARCoreCloudARPinManager::UpdateCloudARPin(UCloudARPin* CloudARPin, ArSession* SessionHandle)
{
	...
	{
		UE_LOGF(LogGoogleARCoreServices, Verbose, "UpdateCloudARPin Future for CloudARPin %ls %ls CANCELLED", *CloudARPin->GetDebugName().ToString(), *CloudARPin->GetCloudID());
	...
	{
		UE_LOGF(LogGoogleARCoreServices, Verbose, "UpdateCloudARPin Future for CloudARPin %ls %ls DONE", *CloudARPin->GetDebugName().ToString(), *CloudARPin->GetCloudID());
		...
		case ECloudAnchorFutureType::Host:
		{
			...
			{
				UE_LOGF(LogGoogleARCoreServices, Verbose, "UpdateCloudARPin Future for Host of CloudARPin %ls (%ls) AR_CLOUD_ANCHOR_STATE_SUCCESS", *CloudARPin->GetDebugName().ToString(), *CloudARPin->GetCloudID());
			...
			{
				UE_LOGF(LogGoogleARCoreServices, Warning, "UpdateCloudARPin Future for Host of CloudARPin %ls (%ls) not success: %i %ls", *CloudARPin->GetDebugName().ToString(), *CloudARPin->GetCloudID(), (int)CloudAnchorState, ToTCHAR(CloudAnchorState));
			}
		...
		case ECloudAnchorFutureType::Resolve:
		{
			...
			if (CloudAnchorState == AR_CLOUD_ANCHOR_STATE_SUCCESS)
			{
				UE_LOGF(LogGoogleARCoreServices, Verbose, "UpdateCloudARPin Future for Resolve of CloudARPin %ls (%ls) AR_CLOUD_ANCHOR_STATE_SUCCESS", *CloudARPin->GetDebugName().ToString(), *CloudARPin->GetCloudID());
				...
				UE_LOGF(LogGoogleARCoreServices, Verbose, "UpdateCloudARPin PinTrackingTransform %ls", *PinTrackingTransform.ToString());
			...
			{
				UE_LOGF(LogGoogleARCoreServices, Warning, "UpdateCloudARPin Future for Resolve of CloudARPin %ls (%ls) not success: %i %ls", *CloudARPin->GetDebugName().ToString(), *CloudARPin->GetCloudID(), (int)CloudAnchorState, ToTCHAR(CloudAnchorState));
			}
...
UCloudARPin* FGoogleARCoreCloudARPinManager::CreateAndHostCloudARPin(UARPin* PinToHost, int32 LifetimeInDays, EARPinCloudTaskResult& OutTaskResult)
{
	...
	UE_LOGF(LogGoogleARCoreServices, Log, "ArSession_hostAndAcquireNewCloudAnchor returns TaskResult: %u, Anchor: %p", (int)OutTaskResult, NewAnchorHandle);
...
UCloudARPin* FGoogleARCoreCloudARPinManager::ResolveAndCreateCloudARPin(FString CloudID, EARPinCloudTaskResult& OutTaskResult)
{
	...
	UE_LOGF(LogGoogleARCoreServices, Log, "ArSession_resolveAndAcquireNewCloudAnchor returns TaskResult: %d, Anchor: %p, CloudID: %ls", (int)OutTaskResult, AnchorHandle, *CloudID);
	ensure(AnchorHandle != nullptr);
...
UCloudARPin* FGoogleARCoreCloudARPinManager::CreateAndHostCloudARPin(UARPin* PinToHost, int32 LifetimeInDays, EARPinCloudTaskResult& OutTaskResult)
{
	...
	UE_LOGF(LogGoogleARCoreServices, Verbose, "ArSession_hostCloudAnchorAsync returns TaskResult: %i for Anchor: %p", (int)OutTaskResult, AnchorHandle);
	...
	UE_LOGF(LogGoogleARCoreServices, Verbose, "ArSession_hostAndAcquireNewCloudAnchor returns TaskResult: %u, Anchor: %p", (int)OutTaskResult, AnchorHandle);
#endif
