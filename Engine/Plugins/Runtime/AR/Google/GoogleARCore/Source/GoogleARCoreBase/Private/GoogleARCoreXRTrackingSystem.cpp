...
{
	UE_LOGF(LogGoogleARCoreTrackingSystem, Log, "Creating GoogleARCore Tracking System.");
...
bool FGoogleARCoreXRTrackingSystem::OnAddRuntimeCandidateImage(UARSessionConfig* SessionConfig, UTexture2D* CandidateTexture, FString FriendlyName, float PhysicalWidth)
{
	...
	UE_LOGF(LogGoogleARCoreTrackingSystem, Warning, "Failed to add runtime candidate image: Unsupported texture format: %ls. ARCore only support PF_B8G8R8A8 or PF_G8 for now for adding runtime candidate image in ARCore", GetPixelFormatString(PixelFormat));
	return false;
}
...
void FGoogleARCoreXRTrackingSystem::OnTrackableAdded(UARTrackedGeometry* InTrackedGeometry)
{
	...
	{
		UE_LOGF(LogGoogleARCoreTrackingSystem, Log, "OnTrackableAdded: %ls %ls", *InTrackedGeometry->GetName(), *InTrackedGeometry->GetDebugName().ToString());
...
void FGoogleARCoreXRTrackingSystem::OnTrackableRemoved(UARTrackedGeometry* InTrackedGeometry)
{
	if (ensure(InTrackedGeometry))
	{
		UE_LOGF(LogGoogleARCoreTrackingSystem, Log, "OnTrackableRemoved: %ls %ls", *InTrackedGeometry->GetName(), *InTrackedGeometry->GetDebugName().ToString());
