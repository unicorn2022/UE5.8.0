...
void UGoogleARCoreAugmentedImageDatabase::Serialize(FArchive& Ar)
{
	...
	if (!Ar.IsLoading() && Ar.IsCooking()) {
		...
		if (Entries.Num()) {
			...
			{
				Ar.SetCriticalError();
				UE_LOGF(LogGoogleARCoreAPI, Error, "Failed to build augmented image database: %ls", *OutStderr);
			}
			else
			{
				FFileHelper::LoadFileToArray(SerializedDatabase, *PathToImageDb, 0);
				UE_LOGF(LogGoogleARCoreAPI, Log,
			...
			{
				UE_LOGF(LogGoogleARCoreAPI, Log, "Cleaning up: %ls", *CleanupList[i]);
