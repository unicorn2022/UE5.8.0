...
{
	UE_LOGF(LogGoogleARCore, Log, "Start ARCore session requested.");
...
#if PLATFORM_ANDROID
	UE_LOGF(LogGoogleARCore, Display, "ARCore session using %ls.", ShouldUseVulkan() ? TEXT("Vulkan") : TEXT("OpenGL ES"));
#endif

	if (bIsARCoreSessionRunning)
	{
		UE_LOGF(LogGoogleARCore, Log, "ARCore session is already running, set it to use the new session config.");
	...
	{
		UE_LOGF(LogGoogleARCore, Warning, "ARCore session is already starting. This will overriding the previous session config with the new one.")
	}
	else 
	{
		PauseARCoreSession();
	...
	{
		UE_LOGF(LogGoogleARCore, Warning, "Reset ARCore session due to fatal error detected.");
		ResetARCoreSession();
	}
}
...
bool FGoogleARCoreDevice::SetARCameraConfig(FGoogleARCoreCameraConfig CameraConfig)
{
	...
	{
		UE_LOGF(LogGoogleARCore, Error, "Failed to set ARCamera configuration due to the arcore session isn't paused.");
	}
	else
	{
		UE_LOGF(LogGoogleARCore, Error, "Failed to set ARCamera configuration with provided CameraConfig.");
	}

	UE_LOGF(LogGoogleARCore, Error, "You should only call the ConfigARCoreCamera function when the OnConfigCamera delegate get called, and the provided CameraConfig must be from the array that is passed by the delegate.");
	return false;
}
	...
	int ImageWidth, int ImageHeight, FString ImageName, float ImageWidthInMeter)
{
	if (!ARCoreSession.IsValid())
	{
		UE_LOGF(LogGoogleARCore, Warning, "Failed to add runtime augmented image: No valid session!");
		return -1;
	}
	...
	FString FriendlyName, float PhysicalWidth)
{
	if (!ARCoreSession.IsValid())
	{
		UE_LOGF(LogGoogleARCore, Warning, "Failed to add runtime candidate image: No valid session!");
		return false;
	}
...
void FGoogleARCoreDevice::HandleRuntimePermissionsGranted(const TArray<FString>& RuntimePermissions, const TArray<bool>& Granted)
{
	...
	for (int32 i = 0; i < RuntimePermissions.Num(); i++)
	{
		...
		{
			bGranted = false;
			UE_LOGF(LogGoogleARCore, Warning, "Android runtime permission denied: %ls", *RuntimePermissions[i]);
		}
		else
		{
			UE_LOGF(LogGoogleARCore, Log, "Android runtime permission granted: %ls", *RuntimePermissions[i]);
		}
	}
...
void FGoogleARCoreDevice::StartSession()
{
	...
	{
		UE_LOGF(LogGoogleARCore, Warning, "Start AR failed: Unsupported AR tracking type %d for GoogleARCore", static_cast<int>(RequestedConfig->GetSessionType()));
	...
	UE_LOGF(LogGoogleARCore, Log, "FGoogleARCoreDevice::StartSession() Configuring camera...");
	{
		...
		if (GoogleConfig != nullptr)
		{
			...
			{
				UE_LOGF(LogGoogleARCore, Error, "EARSessionType::Face requested, but the rear camera has been explicitly specified in the session config.  This is not supported and the session create will fail.");
			}
		}
		else
		{
		...
		UE_LOGF(LogGoogleARCore, Log, "Got %d supported camera config from ARCore:", SupportedCameraConfigs.Num());
		for (int Index = 0; Index < SupportedCameraConfigs.Num(); ++Index)
		{
			UE_LOGF(LogGoogleARCore, Log, "%d: %ls", Index + 1, *SupportedCameraConfigs[Index].ToLogString());
		}
		...
		if (bHasDesiredVideoFormatResolution)
		{
			if (bHasValidDesiredVideoFormatFPS)
			{
				...
				if (MatchCameraConfig)
				{
					...
					UE_LOGF(LogGoogleARCore, Log, "Found and applied camera config matching video format (%d x %d, %d FPS): %ls",
						DesiredVideoFormat.Width, DesiredVideoFormat.Height, DesiredVideoFormat.FPS, *MatchCameraConfig->ToLogString());
				}
				else
				{
					UE_LOGF(LogGoogleARCore, Warning, "Couldn't find any camera config matching desired video format (%d x %d, %d FPS)",
						DesiredVideoFormat.Width, DesiredVideoFormat.Height, DesiredVideoFormat.FPS);
				}
			}
			else
			{
				UE_LOGF(LogGoogleARCore, Warning, "ARCore camera only supports 30/60 FPS.  Default format will be used.  Check your ARSessionConfig.");
			}
		}
		else
		{
			UE_LOGF(LogGoogleARCore, Log, "DesiredVideoFormat width,height = %i,%i default format will be used.", DesiredVideoFormat.Width > 0, DesiredVideoFormat.Height);
		}
		...
		if (bUseFrontCamera && !MatchCameraConfig)
		{
			...
			{
				UE_LOGF(LogGoogleARCore, Error, "We want to use a front camera but did not find any front camera configs.  This device may not have a front camera.  The session may use the back camera or session create may fail.");
			}
			else
			{
				MatchCameraConfig = &(SupportedCameraConfigs[0]);
				SetARCameraConfig(*MatchCameraConfig);
				UE_LOGF(LogGoogleARCore, Log, "Found and applied a front camera config (%d x %d, %d FPS): %ls",
					DesiredVideoFormat.Width, DesiredVideoFormat.Height, DesiredVideoFormat.FPS, *MatchCameraConfig->ToLogString());
			}
		}
	}
	...
	UE_LOGF(LogGoogleARCore, Log, "FGoogleARCoreDevice::StartSession() Configuring session...");
	EGoogleARCoreAPIStatus Status = ARCoreSession->ConfigSession(*RequestedConfig);

	if (Status != EGoogleARCoreAPIStatus::AR_SUCCESS)
	{
		UE_LOGF(LogGoogleARCore, Error, "ARCore Session start failed ConfigSession with error status %d", static_cast<int>(Status));
	...
	UE_LOGF(LogGoogleARCore, Display, "SetCameraTextureIds %i textures", TextureIds.Num());
	ARCoreSession->SetCameraTextureIds(TextureIds);


	if (Status != EGoogleARCoreAPIStatus::AR_SUCCESS)
	{
		UE_LOGF(LogGoogleARCore, Error, "ARCore Session start failed Resume with error status %d", static_cast<int>(Status));
	...
	{
		UE_LOGF(LogGoogleARCore, Error, "ERROR: GoogleARCoreXRTrackingSystem is not available.");
	}
	...
	UE_LOGF(LogGoogleARCore, Log, "ARCore session started successfully.");
...
{
	UE_LOGF(LogGoogleARCore, Log, "Pausing ARCore session.");
	if (!bIsARCoreSessionRunning)
	{
		if(bStartSessionRequested)
		{
		...
		{
			UE_LOGF(LogGoogleARCore, Log, "Could not stop ARCore tracking session because there is no running tracking session!");
		}
		return;
	}
	...
	bIsARCoreSessionRunning = false;
	UE_LOGF(LogGoogleARCore, Log, "ARCore session paused");
}
...
void FGoogleARCoreDevice::ResetARCoreSession()
{
	...
	UE_LOGF(LogGoogleARCore, Log, "ARCore session reset");
}
...
void FGoogleARCoreDevice::AllocatePassthroughCameraTextures()
{
	...
	for (auto Texture : Textures)
	{
		...
		UE_LOGF(LogGoogleARCore, Log, "Created external camera texture of size (%.0f x %.0f) with texture Id (%d) and external texture Guid (%ls)",
...
void FGoogleARCoreDevice::TransformARCoordinates2D(EGoogleARCoreCoordinates2DType InputCoordinatesType, const TArray<FVector2D>& InputCoordinates, EGoogleARCoreCoordinates2DType OutputCoordinatesType, TArray<FVector2D>& OutputCoordinates) const
{
	if (!ARCoreSession.IsValid() || ARCoreSession->GetLatestFrame() == nullptr)
	{
		UE_LOGF(LogGoogleARCore, Log, "Failed to transform ARCoordinate2D due to thers is no valid ARCore session.");
		return;
	}
...
{
	UE_LOGF(LogGoogleARCore, Log, "OnPause Called: %d", bIsARCoreSessionRunning);
...
{
	UE_LOGF(LogGoogleARCore, Log, "OnResume Called: %d", bShouldSessionRestart);
