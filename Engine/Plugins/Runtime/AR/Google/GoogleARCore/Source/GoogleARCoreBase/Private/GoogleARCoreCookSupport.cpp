...
bool FGoogleARCoreSessionConfigCookSupport::SaveTextureToPNG(UTexture2D *Tex, const FString &Filename)
{
	...
	if (Tex->Source.GetMipData(MipData, 0))
	{
		if (Tex->Source.GetFormat() != TSF_BGRA8 )
		{
			UE_LOGF(
				...
				"Texture %ls is not RGBA8 or BGRA8 and cannot be used as a tracking target.",
				*Tex->GetName());
		...
		if (PngPtr)
		{
			...
			if (setjmp(png_jmpbuf(PngPtr)))
			{
				UE_LOGF(
					LogGoogleARCore, Error,
					"Error writing PNG for texture %ls.",
					*Tex->GetName());
				Ret = false;
			}
			else
	...
	else
	{
		UE_LOGF(
			LogGoogleARCore, Error,
			"Error reading mip data in texture %ls.",
			*Tex->GetName());
...
void FGoogleARCoreSessionConfigCookSupport::OnSerializeSessionConfig(UARSessionConfig* SessionConfig, FArchive& Ar, TArray<uint8>& SerializedARCandidateImageDatabase)
{
	...
	if (CandidateImageList.Num()) {
		UE_LOGF(LogGoogleARCore, Display, "Cooking ARSessionConfig for platform: %ls", *Ar.CookingTarget()->PlatformName());
		...
		{
			Ar.SetCriticalError();
			UE_LOGF(LogGoogleARCore, Error, "Failed to build augmented image database: %ls", *OutStderr);
		}
		else
		{
			FFileHelper::LoadFileToArray(SerializedARCandidateImageDatabase, *PathToImageDb, 0);
			UE_LOGF(LogGoogleARCore, Display,
				"Augmented image database created. Size: %d bytes. Tool output: %ls",
				SerializedARCandidateImageDatabase.Num(), *OutStdout);
		}

		for (int32 i = 0; i < CleanupList.Num(); i++)
		{
			UE_LOGF(LogGoogleARCore, Log, "Cleaning up: %ls", *CleanupList[i]);
