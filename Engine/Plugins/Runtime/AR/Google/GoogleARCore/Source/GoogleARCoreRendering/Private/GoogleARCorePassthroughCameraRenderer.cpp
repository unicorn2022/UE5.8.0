...
public:
	...
	FYCbCrConversionParameters GetYCbCrConversionParameters(const FYCbCrConversion& YCbCrConversion) const
	{
		...
		default:
			UE_LOGF(LogTemp, Error, "Unexpected YCbCr Model conversion: %d", EnumToUnderlyingType(YCbCrConversion.YCbCrModelConversion));
			break;
		}
		...
		default:
			UE_LOGF(LogTemp, Error, "Unexpected number of bits in YCbCr conversion: %d", YCbCrConversion.NumBits);
			break;
		}
...
void FGoogleARCorePassthroughCameraRenderer::RenderVideoOverlayWithMaterial(FRHICommandList& RHICmdList, FSceneView& InView, UMaterialInstanceDynamic* OverlayMaterialToUse, bool bRenderingOcclusion, bool bUsesCameraTexture)
{
	...
	if (FAndroidMisc::ShouldUseVulkan() && IsMobileHDR() && !RHICmdList.IsInsideRenderPass())
	{
		...
		UE_LOGF(LogTemp, Warning, "FGoogleARCorePassthroughCameraRenderer::RenderVideoOverlayWithMaterial: skipped due to not called within a render pass on Vulkan!");
		return;
	}
