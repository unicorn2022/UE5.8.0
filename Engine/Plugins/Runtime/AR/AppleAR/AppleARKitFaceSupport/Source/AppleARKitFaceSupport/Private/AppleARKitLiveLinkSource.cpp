...
TSharedPtr<IARKitBlendShapePublisher, ESPMode::ThreadSafe> FAppleARKitLiveLinkSourceFactory::CreateLiveLinkRemotePublisher(const FString& RemoteAddr)
{
	...
	{
		UE_LOGF(LogAppleARKitFace, Warning, "Failed to create LiveLink remote publisher, so no data will be sent out");
		delete Publisher;
	}
#endif
...
void FAppleARKitLiveLinkSource::InitializeRemoteListener()
{
	...
	if (!RemoteListener->InitReceiveSocket(ConnectionSettings.Port))
	{
		...
		UE_LOGF(LogAppleARKitFace, Warning, "Could not start AppleARKit remote listener on port %d", ConnectionSettings.Port);
	}
#endif
}
...
void FAppleARKitLiveLinkSource::UpdateStaticData(FName SubjectName, const FARBlendShapeMap& FaceBlendShapes, FName DeviceId /*= NAME_None*/)
{
	...
	UE_LOGF(LogAppleARKitFace, Verbose, "Pushing AppleARKit Subject '%ls' with %d blend shapes", *SubjectName.ToString(), BaseStaticData->PropertyNames.Num());
...
bool FAppleARKitLiveLinkRemotePublisher::InitSendSocket()
{
	...
	if (Addr->IsValid())
	{
		...
		UE_LOGF(LogAppleARKitFace, Log, "Sending LiveLink face AR data to address (%ls)", *Addr->ToString(true));
	}
...
void FAppleARKitLiveLinkRemotePublisher::PublishBlendShapes(FName SubjectName, const FQualifiedFrameTime& FrameTime, const FARBlendShapeMap& FaceBlendShapes, FName DeviceId)
{
	if (SendSocket != nullptr)
	{
		...
		{
			ISocketSubsystem* SocketSub = ISocketSubsystem::Get();
			UE_LOGF(LogAppleARKitFace, Verbose, "Failed to send face AR packet with error (%ls). Packet size (%d), sent (%d)", SocketSub->GetSocketError(), SourceBufferSize, AmountSent);
		}
...
bool FAppleARKitLiveLinkRemoteListener::InitReceiveSocket(int32 Port)
{
	...
	if (RecvSocket != nullptr)
	{
		...
		if (!RecvSocket->Bind(*Addr))
		{
			...
			RecvSocket = nullptr;
			UE_LOGF(LogAppleARKitFace, Warning, "Failed to bind to the listen port (%ls) for LiveLink face AR receiving with error (%ls)",
...
void FAppleARKitLiveLinkRemoteListener::Tick(float DeltaTime)
{
	...
	while (RecvSocket->HasPendingData(BytesPending))
	{
			...
			BytesRead > MIN_BLEND_SHAPE_PACKET_SIZE)
		{
			...
			{
				UE_LOGF(LogAppleARKitFace, Verbose, "Packet overflow reading the packet version for the face AR packet");
				return;
			}
			FromBuffer >> DeviceId;
			...
			{
				UE_LOGF(LogAppleARKitFace, Verbose, "Packet overflow reading the face AR packet's non-array fields");
				return;
			}
			...
			{
				UE_LOGF(LogAppleARKitFace, Verbose, "Packet overflow reading the face AR packet's array of blend shapes");
			}
		}
	}
}
