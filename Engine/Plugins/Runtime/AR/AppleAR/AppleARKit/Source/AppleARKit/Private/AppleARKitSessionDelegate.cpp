...
- (id)initWithAppleARKitSystem:(FAppleARKitSystem*)InAppleARKitSystem
{
	...
	if (self)
	{
		UE_LOGF(LogAppleARKit, Log, "Delegate created with session: %p", InAppleARKitSystem);
...
- (void)session:(ARSession *)session didUpdateFrame:(ARFrame *)frame 
{
	...
	{
		UE_LOGF(LogAppleARKit, Log, "Delegate didUpdateFrame with no valid _metalTextureCache (ignoring)");
		return;
	}
...
- (void)session:(ARSession *)session didFailWithError:(NSError *)error
{
	...
	UE_LOGF( LogAppleARKit, Warning, "Session failed with error: %ls", *FString(ErrorDescription) );
}
