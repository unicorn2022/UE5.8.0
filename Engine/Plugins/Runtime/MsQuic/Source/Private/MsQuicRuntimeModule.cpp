...
bool FMsQuicRuntimeModule::InitRuntime()
{
	...
	{
		UE_LOGF(LogMsQuicRuntime, Display,
			"[MsQuicRuntimeModule] MsQuic DLL already loaded.");
	...
	if (!LoadMsQuicDll())
	{
		UE_LOGF(LogMsQuicRuntime, Error,
			"[MsQuicRuntimeModule] Could not load MsQuic DLL.");
