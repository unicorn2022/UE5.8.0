...
FZoneGraphDataHandle UZoneGraphSubsystem::RegisterZoneGraphData(AZoneGraphData& InZoneGraphData)
{
	...
	{
		UE_LOGF(LogZoneGraph, Error, "Trying to register already registered ZoneGraphData \'%ls\'", *InZoneGraphData.GetName());
		return FZoneGraphDataHandle();
	}
	...
	{
		UE_LOGF(LogZoneGraph, Error, "ZoneGraphData \'%ls\' already exists in RegisteredZoneGraphData.", *InZoneGraphData.GetName());
		return FZoneGraphDataHandle();
	}
...
void UZoneGraphSubsystem::UnregisterZoneGraphData(AZoneGraphData& InZoneGraphData)
{
	...
	if (Index == INDEX_NONE)
	{
		UE_LOGF(LogZoneGraph, Error, "Trying to remove ZoneGraphData \'%ls\' that does not exists in RegisteredZoneGraphData.", *InZoneGraphData.GetName());
		return;
	}
...
void UZoneGraphSubsystem::UnregisterStaleZoneGraphDataInstances()
{
	...
	for (int32 Index = 0; Index < RegisteredZoneGraphData.Num(); Index++)
	{
		if (RegisteredZoneGraphData[Index].bInUse)
		{
			...
			{
				UE_LOGF(LogZoneGraph, Error, "Removing stale ZoneGraphData \'%ls\' that does not exists in RegisteredZoneGraphData.", *GetNameSafe(ZoneGraphData));
				RemoveRegisteredDataItem(Index);
				Index--;
			}
			else if (!ZoneGraphData->IsRegistered())
			{
				UE_LOGF(LogZoneGraph, Error, "Removing unregistered ZoneGraphData \'%ls\' that does not exists in RegisteredZoneGraphData.", *GetNameSafe(ZoneGraphData));
				RemoveRegisteredDataItem(Index);
				Index--;
			}
		}
