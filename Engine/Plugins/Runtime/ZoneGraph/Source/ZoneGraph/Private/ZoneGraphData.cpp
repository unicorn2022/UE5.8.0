...
void AZoneGraphData::PostLoad()
{
	...
	{
		UE_LOGF(LogZoneGraph, Warning, "\'%ls\' Is missing LaneUpVectors, generating default up vectors.", *GetName());
	...
	{
		UE_LOGF(LogZoneGraph, Warning, "\'%ls\' Is missing LaneTangentVectors, generating zero tangents.", *GetName());
	...
	{
		UE_LOGF(LogZoneGraph, Warning, "\'%ls\' Is missing ZoneBVTree, generating for %d zones...", *GetName(),ZoneStorage.Zones.Num());

		FAutoScopedDurationTimer Timer;
		Timer.Start();
		ZoneStorage.ZoneBVTree.Build(MakeStridedView(ZoneStorage.Zones, &FZoneData::Bounds));
		
		UE_LOGF(LogZoneGraph, Warning, "  - Generated in %.2fms", Timer.GetTime() * 1000.0);
	}

	const bool bSucceeded = RegisterWithSubsystem();
	UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) PostLoad - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
}
...
void AZoneGraphData::Destroyed()
{
	...
	Super::Destroyed();
	UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) Destroyed - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
}
...
void AZoneGraphData::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) EndPlay - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
}
...
void AZoneGraphData::PostActorCreated()
{
	...
	const bool bSucceeded = RegisterWithSubsystem();
	UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) PostActorCreated - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
}
...
void AZoneGraphData::PreRegisterAllComponents()
{
	...
	if (const ULevel* Level = GetLevel())
	{
		...
		{
			const bool bSucceeded = RegisterWithSubsystem();
			UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) PreRegisterAllComponents - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
		}
	}
}
...
void AZoneGraphData::PostUnregisterAllComponents()
{
	...
	if (const ULevel* Level = GetLevel())
	{
		...
		{
			const bool bSucceeded = UnregisterWithSubsystem();
			UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) PostUnregisterAllComponents - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
		}
	}
...
void AZoneGraphData::PostEditUndo()
{
	...
	{
		const bool bSucceeded = UnregisterWithSubsystem();
		UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) PostEditUndo/PendingKill - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
	}
	else
	{
		const bool bSucceeded = RegisterWithSubsystem();
		UE_LOGF(LogZoneGraph, Verbose, "\'%ls\' (0x%llx) PostEditUndo - %ls", *GetName(), UPTRINT(this), bSucceeded ? TEXT("Succeeded") : TEXT("Failed"));
	}
}
