...
void DrawZoneBoundary(const FZoneGraphStorage& ZoneStorage, int32 ZoneIndex, FPrimitiveDrawInterface* PDI, const FMatrix& LocalToWorld, const float LineThickness, const float DepthBias, const float Alpha, const FZoneDrawAnnotator& Annotator)
{
	...
	FZoneGraphTagMask VisTagMask = Annotator.VisualizedTagsOverride.Get(ZoneGraphSettings->GetVisualizedTags());
	...
	FColor Color = GetTagMaskColor(Annotator.ZoneTagsOverride.Get(Zone.Tags) & VisTagMask, TagInfo);
...
void DrawZoneLanes(const FZoneGraphStorage& ZoneStorage, int32 ZoneIndex, FPrimitiveDrawInterface* PDI, const FMatrix& LocalToWorld, const float LineThickness, const float DepthBias, const float Alpha, const bool bDrawDetails, const FLaneHighlight& LaneHighlight, const FZoneDrawAnnotator& Annotator)
{
	if (ZoneIndex < 0 || ZoneIndex >= ZoneStorage.Zones.Num())
	{
		return;
	...
	for (int32 LaneIdx = Zone.LanesBegin; LaneIdx < Zone.LanesEnd; LaneIdx++)
	{
		...
		FColor Color = GetTagMaskColor((Annotator.GetLaneTagsOverride.IsSet() ? Annotator.GetLaneTagsOverride.GetValue()(LaneIdx, Lane) : Lane.Tags) & VisTagMask, TagInfo);
