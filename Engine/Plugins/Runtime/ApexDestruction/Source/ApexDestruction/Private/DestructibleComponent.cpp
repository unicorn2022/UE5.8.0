...
void UDestructibleComponent::OnUpdateTransform(EUpdateTransformFlags UpdateTransformFlags, ETeleportType Teleport)
{
	...
	{
		UE_LOGF(LogPhysics, Log, "UDestructibleComponent::SendPhysicsTransform : Non-uniform scale factor (%ls) can cause physics to mismatch for %ls  SkelMesh: %ls", *MeshScale3D.ToString(), *GetFullName(), GetSkinnedAsset() ? *GetSkinnedAsset()->GetFullName() : TEXT("NULL"));
	}
#endif
}
...
void UDestructibleComponent::SetSkeletalMesh(USkeletalMesh* InSkelMesh, bool bReinitPose)
{
	if(InSkelMesh != NULL && !InSkelMesh->IsA(UDestructibleMesh::StaticClass()))
	{
		...
		UE_LOGF(LogPhysics, Log, "UDestructibleComponent::SetSkeletalMesh(): Passed-in USkeletalMesh (%ls) must be a UDestructibleMesh.  SkeletalMesh not set.", *InSkelMesh->GetPathName() );
		return;
	}
...
void UDestructibleComponent::BeginPlay()
{
	...
	UE_LOGF(LogDestructible, Warning, "(%ls): DestructibleComponent is deprecated. Destruction is now supported by UGeometryCollection. Please update your assets before upgrading to the next release.", *GetPathName());
#endif
}
