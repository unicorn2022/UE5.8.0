...
void UDestructibleMesh::PostLoad()
{
	...
	UE_LOGF(LogDestructible, Warning, "During PostLoad for Mesh %ls: DestructibleMesh is deprecated. Destruction is now supported by UGeometryCollection. Please update your assets before upgrading to the next release.", *GetPathName());
#endif
...
APEXDESTRUCTION_API bool UDestructibleMesh::SetupChunksFromStaticMeshes( const TArray<UStaticMesh*>& ChunkMeshes )
{
	...
	{
		UE_LOGF(LogDestructible, Warning, "Unable to import FBX as level 1 chunks if the DM was not created from a static mesh.");
		return false;
	}
