...
void UPropertyAnimatorCoreEditorStackCustomization::RemoveAnimatorAction(FOperatorStackEditorItemPtr InItem) const
{
	...
	if (InItem->IsA<UPropertyAnimatorCoreBase>())
	{
		...
		{
			UE_LOGF(LogPropertyAnimatorCoreEditorStackCustomization, Warning, "Could not remove %i animator(s)", Animators.Num())
		}
	}
	else if (InItem->IsA<UPropertyAnimatorCoreComponent>())
	{
		...
		{
			UE_LOGF(LogPropertyAnimatorCoreEditorStackCustomization, Warning, "Could not remove %i animator component(s)", Components.Num())
		}
	}
}
