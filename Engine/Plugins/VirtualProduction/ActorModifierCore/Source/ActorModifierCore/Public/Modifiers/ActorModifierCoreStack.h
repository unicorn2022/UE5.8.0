...
public:
	...
	/** Returns true if this stack is frozen */
	ACTORMODIFIERCORE_API bool IsModifierStackFrozen() const;

	/**
	 * Set whether the modifier stack is frozen
	 * @see bModifierStackFrozen
	 */
	ACTORMODIFIERCORE_API void SetModifierStackFrozen(bool bInFrozen);
...
private:
	...
	/** Called when the modifier stack frozen has changed */
	void OnModifierStackFrozenChanged();
	...
	/**
	 * EXPERIMENTAL:
	 * Freeze the stack to disable modifiers from being added/removed/moved, while also
	 * preventing restore states from occurring.
	 * The only operation allowed in a frozen state is to remove the stack as a whole.
	 * This can be used to temporarily apply a set of modifiers once, and persist the changes after removing the modifiers.
	 */
	UPROPERTY(EditInstanceOnly, Category="Modifier", meta=(DisplayName="Freeze Stack"))
	bool bModifierStackFrozen = false;
