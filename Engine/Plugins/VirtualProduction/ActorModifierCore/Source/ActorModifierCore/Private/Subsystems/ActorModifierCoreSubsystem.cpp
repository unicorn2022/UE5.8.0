...
FTSTicker UActorModifierCoreSubsystem::ModifierTicker;
...
FTSTicker& UActorModifierCoreSubsystem::GetModifierTicker()
{
	return ModifierTicker;
}
...
bool UActorModifierCoreSubsystem::RegisterModifierClass(const UClass* InModifierClass, bool bInOverrideIfExists)
{
	...
	if (UActorModifierCoreBase* CDO = InModifierClass->GetDefaultObject<UActorModifierCoreBase>())
	{
		...
		{
			UE_LOGF(LogActorModifierCoreSubsystem, Warning, "[%ls] Could not register modifier class : Modifier metadata is invalid", *CDO->GetClass()->GetName())
			return false;	
		}
		...
		{
			UE_LOGF(LogActorModifierCoreSubsystem, Warning, "[%ls] Could not register modifier class : Modifier name is invalid", *CDO->GetClass()->GetName())
			return false;
		}
...
bool UActorModifierCoreSubsystem::UnregisterModifierClass(const UClass* InModifierClass)
{
	...
	for (TMap<FName, TSharedRef<FActorModifierCoreMetadata>>::TIterator It(ModifiersMetadata); It; ++It)
	{
		...
		{
			UActorModifierCoreSubsystem::OnModifierClassUnregisteredDelegate.Broadcast(*It.Value());
			It.RemoveCurrent();
			bUnregistered = true;
		}
	}
...
ETickableTickType UActorModifierCoreSubsystem::GetTickableTickType() const
{
	return ETickableTickType::Always;
}

bool UActorModifierCoreSubsystem::IsTickableInEditor() const
{
	return true;
}

void UActorModifierCoreSubsystem::Tick(float InDeltaTime)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UActorModifierCoreSubsystem::Tick);
	ModifierTicker.Tick(InDeltaTime);
}

TStatId UActorModifierCoreSubsystem::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(UActorModifierCoreSubsystem, STATGROUP_Tickables);
}
		...
		for (UActorModifierCoreBase* Modifier : InModifiers)
		{
			if (IsValid(Modifier))
			{
				...
				else
				{
					...
					UE_LOGF(LogActorModifierCoreSubsystem, Warning, "Move modifier %ls on actor %ls failed : %ls",
		...
		for (UActorModifierCoreBase* Modifier : InModifiers)
		{
			if (IsValid(Modifier))
			{
				...
				else
				{
					...
					UE_LOGF(LogActorModifierCoreSubsystem, Warning, "Clone modifier %ls on actor %ls failed : %ls",
...
bool UActorModifierCoreSubsystem::RegisterModifierSharedProvider(AActorModifierCoreSharedActor* InSharedActor) const
{
	...
	UE_LOGF(LogActorModifierCoreSubsystem, Verbose, "Modifier shared provider registered for world %ls and level %ls", *Level->GetWorld()->GetDebugDisplayName(), *Level->GetName());
...
bool UActorModifierCoreSubsystem::UnregisterModifierSharedProvider(const AActor* InSharedActor) const
{
	...
	UE_LOGF(LogActorModifierCoreSubsystem, Verbose, "Modifier shared provider unregistered for world %ls and level %ls", *Level->GetWorld()->GetDebugDisplayName(), *Level->GetName());
