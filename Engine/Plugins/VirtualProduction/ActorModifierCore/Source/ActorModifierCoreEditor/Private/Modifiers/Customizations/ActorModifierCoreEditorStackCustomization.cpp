...
namespace UE::ActorModifierCoreEditor::Private
{

FText GetStackPropertyTooltip(FName InPropertyName)
{
	const FProperty* Property = FindFProperty<FProperty>(UActorModifierCoreStack::StaticClass(), InPropertyName);
	return Property ? Property->GetToolTipText() : FText::GetEmpty();
}

} // UE::ActorModifierCoreEditor::Private
...
void UActorModifierCoreEditorStackCustomization::CustomizeItemHeader(const FOperatorStackEditorItemPtr& InItem, const FOperatorStackEditorTree& InItemTree, FOperatorStackEditorHeaderBuilder& InHeaderBuilder)
{
	...
	if (InItem->IsA<UActorModifierCoreBase>() && InItem->HasValue(0))
	{
...
void UActorModifierCoreEditorStackCustomization::CustomizeItemBody(const FOperatorStackEditorItemPtr& InItem, const FOperatorStackEditorTree& InItemTree, FOperatorStackEditorBodyBuilder& InBodyBuilder)
{
	...
	{
		FBoolProperty* ModifierEnableProperty = FindFProperty<FBoolProperty>(UActorModifierCoreBase::StaticClass(), GET_MEMBER_NAME_CHECKED(UActorModifierCoreBase, bModifierEnabled));
		FBoolProperty* StackFrozenProperty = FindFProperty<FBoolProperty>(UActorModifierCoreStack::StaticClass(), GET_MEMBER_NAME_CHECKED(UActorModifierCoreStack, bModifierStackFrozen));
		...
		InBodyBuilder
			...
			.DisallowProperty(StackFrozenProperty)
...
{
	using namespace UE::ActorModifierCoreEditor;
	...
	// Add freeze & profiling stat toggle entry
	if (ItemContext->IsA<UActorModifierCoreStack>())
	{
		const FToolMenuEntry FreezeModifierStackAction = FToolMenuEntry::InitToolBarButton(
			TEXT("FreezeModifierStackMenuEntry")
			, FUIAction(
				FExecuteAction::CreateUObject(this, &UActorModifierCoreEditorStackCustomization::ToggleModifierStackFrozenAction, ItemContext)
				, FCanExecuteAction()
				, FIsActionChecked::CreateUObject(this, &UActorModifierCoreEditorStackCustomization::IsModifierStackFrozen, ItemContext))
			, FText::GetEmpty()
			, Private::GetStackPropertyTooltip(GET_MEMBER_NAME_CHECKED(UActorModifierCoreStack, bModifierStackFrozen))
			, FSlateIcon(FActorModifierCoreEditorStyle::Get().GetStyleSetName(), "FreezeStack")
			, EUserInterfaceActionType::ToggleButton
		);
		InToolMenu->AddMenuEntry(FreezeModifierStackAction.Name, FreezeModifierStackAction);
...
void UActorModifierCoreEditorStackCustomization::FillItemContextActionMenu(UToolMenu* InToolMenu) const
{
	...
	if (ItemContext->IsA<UActorModifierCoreStack>())
	{
		const FToolMenuEntry FreezeModifierStackAction = FToolMenuEntry::InitMenuEntry(
			TEXT("FreezeModifierStackMenuEntry")
			, LOCTEXT("FreezeModifierStack", "Freeze Modifier Stack")
			, Private::GetStackPropertyTooltip(GET_MEMBER_NAME_CHECKED(UActorModifierCoreStack, bModifierStackFrozen))
			, FSlateIcon(FActorModifierCoreEditorStyle::Get().GetStyleSetName(), "FreezeStack")
			, FUIAction(
				FExecuteAction::CreateUObject(this, &UActorModifierCoreEditorStackCustomization::ToggleModifierStackFrozenAction, ItemContext)
...
bool UActorModifierCoreEditorStackCustomization::CanRemoveModifier(FOperatorStackEditorItemPtr InItem) const
{
	...
	for (UActorModifierCoreBase* Modifier : InItem->GetAsArray<UActorModifierCoreBase>())
	{
		...
		const UActorModifierCoreStack* const ModifierStack = Modifier->GetModifierStack();
		if (ModifierStack && ModifierStack->IsModifierStackFrozen())
		{
			return false;
		}
	}
...
void UActorModifierCoreEditorStackCustomization::RemoveModifierAction(FOperatorStackEditorItemPtr InItem) const
{
	...
	if (InItem->IsA<UActorModifierCoreStack>())
	{
		...
		{
			UE_LOGF(LogActorModifierCoreEditorStackCustomization, Warning, "Could not remove modifier stacks from actors")
		}
	}
	else
	{
...
void UActorModifierCoreEditorStackCustomization::CopyModifierAction(FOperatorStackEditorItemPtr InItem) const
{
	...
	{
		UE_LOGF(LogActorModifierCoreEditorStackCustomization, Warning, "Unable to serialize the selected modifier(s) into Json format");
		return;
	}
...
void UActorModifierCoreEditorStackCustomization::PasteModifierAction(FOperatorStackEditorItemPtr InItem) const
{
	...
	{
		UE_LOGF(LogActorModifierCoreEditorStackCustomization, Warning, "Unable to set properties from %ls modifier due to target modifiers (%i) and modifiers properties (%i) count mismatch", *ModifierName.ToString(), TargetModifiers.Num(), ModifierPropertiesWrapper.Num());
		return;
	}
	...
	for (int32 Index = 0; Index < TargetModifiers.Num(); Index++)
	{
		...
		else
		{
			...
			UE_LOGF(LogActorModifierCoreEditorStackCustomization, Warning, "Unable to copy properties from %ls modifier to %ls modifier", *SourceModifierName, *DestinationModifierName);
		}
	}
}

bool UActorModifierCoreEditorStackCustomization::IsModifierStackFrozen(FOperatorStackEditorItemPtr InItem) const
{
	if (!InItem.IsValid() || !InItem->HasValue())
	{
		return false;
...
void UActorModifierCoreEditorStackCustomization::ToggleModifierStackFrozenAction(FOperatorStackEditorItemPtr InItem) const
{
	if (!InItem.IsValid() || !InItem->HasValue())
	{
		return;
	...
	const bool bIsModifierFrozen = IsModifierStackFrozen(InItem);
	for (UActorModifierCoreStack* ModifierStack : InItem->GetAsArray<UActorModifierCoreStack>())
	{
		if (ModifierStack)
		{
			ModifierStack->SetModifierStackFrozen(!bIsModifierFrozen);	
		}
	}
}
...
bool UActorModifierCoreEditorStackCustomization::GetModifierPropertiesWrapperFromClipboard(TArray<FActorModifierCoreEditorPropertiesWrapper>& OutModifierProperties) const
{
	...
	{
		UE_LOGF(LogActorModifierCoreEditorStackCustomization, Warning, "Unable to deserialize the clipboard text into Json format");
		return false;
	}

	if (JsonModifiers.IsEmpty())
	{
		UE_LOGF(LogActorModifierCoreEditorStackCustomization, Warning, "No Json modifiers data available to paste");
		return false;
	}

	if (!FJsonObjectConverter::JsonArrayToUStruct(JsonModifiers, &OutModifierProperties))
	{
		UE_LOGF(LogActorModifierCoreEditorStackCustomization, Warning, "Invalid Json modifiers properties found");
		return false;
	}
