...
UCanvasRenderTarget2D* UGeometryMaskCanvasReferenceComponentBase::GetTexture() const
{
	return nullptr;
}

UTextureRenderTarget2DArray* UGeometryMaskCanvasReferenceComponentBase::GetRenderTarget() const
{
	const UGeometryMaskCanvas* Canvas = CanvasWeak.Get();
	return Canvas ? Canvas->GetRenderTarget() : nullptr;
}
...
bool UGeometryMaskCanvasReferenceComponentBase::TryResolveNamedCanvas(FName InCanvasName)
{
	...
	{
		Canvas = Subsystem->GetNamedCanvas(GetComponentLevel(), InCanvasName, this);
...
bool UGeometryMaskCanvasReferenceComponentBase::ForEachUsedCanvasName(TFunctionRef<bool(FName)> InFunc) const
{
	return true;
}
