...
namespace UE::DataLinkOAuth::Private
{
	...
	bool FindUnusedPort(int32& OutPort)
	{
		...
		if (!SocketSubsystem)
		{
			UE_LOGF(LogDataLinkOAuth, Error, "Find Unused Port failed. Invalid Socket Subsystem");
			return false;
		}
		...
		if (!Socket)
		{
			UE_LOGF(LogDataLinkOAuth, Error, "Find Unused Port failed. Could not create Socket [%ls, %ls]"
				, SocketDescription
		...
		{
			UE_LOGF(LogDataLinkOAuth, Error, "Find Unused Port failed. Failed to Bind Socket [%ls, %ls]"
				, SocketDescription
				, *InternetAddress->ToString(/*bAppendPort*/false));
			return false;
		}
	...
	FDataLinkOAuthHandle StartListening(int32 InListenPort, const FOnAuthResponse& InOnAuthResponse)
	{
		...
		if (!Router.IsValid())
		{
			UE_LOGF(LogDataLinkOAuth, Error, "Start Listening failed. Could not get HTTP Router for port %d.", InListenPort);
...
bool UE::DataLinkOAuth::RequestAuthorization(const FAuthRequestParams& InParams)
{
	...
	if (!OAuthSubsystem)
	{
		UE_LOGF(LogDataLinkOAuth, Error, "Start Listening failed. Invalid OAuth Subsystem.");
		return false;
	}
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "Start Listening failed. Could not find unused port.");
		return false;
	}
