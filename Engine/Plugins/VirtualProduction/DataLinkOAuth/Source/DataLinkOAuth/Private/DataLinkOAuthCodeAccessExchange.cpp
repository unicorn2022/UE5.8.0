...
namespace UE::DataLinkOAuth::Private
{
	FString GetResponseString(FHttpResponsePtr InResponse, bool bInProcessedSuccessfully)
	{
		...
		{
			UE_LOGF(LogDataLinkOAuth, Error, "OAuth Code Exchange failed to be processed.");
			return FString();
		}
		...
		{
			UE_LOGF(LogDataLinkOAuth, Error, "OAuth Code Exchange failed with response code %d", ResponseCode);
			return FString();
		}
...
bool UE::DataLinkOAuth::ExchangeAuthCodeForAccess(const FExchangeAuthCodeParams& InParams)
{
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "OAuth Code Exchange Failed. Invalid Auth Code.");
		return false;
	}
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "OAuth Code Exchange Failed. Could not build request URL. (Settings: %ls)"
