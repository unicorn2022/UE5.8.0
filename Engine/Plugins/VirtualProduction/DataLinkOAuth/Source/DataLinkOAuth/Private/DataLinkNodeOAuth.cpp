...
EDataLinkExecutionReply UDataLinkNodeOAuth::OnExecute(FDataLinkExecutor& InExecutor) const
{
	...
	if (!OAuthSettings)
	{
		UE_LOGF(LogDataLinkOAuth, Error, "[%ls] Data Link OAuth - Invalid OAuth settings.", InExecutor.GetContextName().GetData());
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "[%ls] Data Link OAuth - Authorization request failed.", InExecutor.GetContextName().GetData());
		return EDataLinkExecutionReply::Unhandled;
	}
...
void UDataLinkNodeOAuth::OnAuthResponse(const FHttpServerRequest& InRequest, TWeakPtr<FDataLinkExecutor> InExecutorWeak) const
{
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "[%ls] Data Link OAuth - Invalid Authorization Code (Settings: %ls)"
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "[%ls] Data Link OAuth - Failed to Exchange Auth Code For Access (Settings: %ls)"
			, Executor->GetContextName().GetData()
			, *OAuthSettings->GetName());
...
void UDataLinkNodeOAuth::OnExchangeCodeResponse(FStringView InResponse, TWeakPtr<FDataLinkExecutor> InExecutorWeak) const
{
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "[%ls] Data Link OAuth - Failed to get a valid response with Access Token (Settings: %ls)"
			, Executor->GetContextName().GetData()
			, *OAuthSettings->GetName());
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "[%ls] Data Link OAuth - Failed to build OAuth Token. (Settings: %ls) Exchange Code Response: %ls"
			, Executor->GetContextName().GetData()
			, *OAuthSettings->GetName()
			, InResponse.GetData());
	...
	if (!OAuthSettings->AuthorizeHttpRequest(OAuthToken, OutputHttpSettings))
	{
		UE_LOGF(LogDataLinkOAuth, Error, "[%ls] Data Link OAuth - OAuth Token unexpectedly failed to authorize the Http Request. (Settings: %ls) Exchange Code Response: %ls"
			, Executor->GetContextName().GetData()
			, *OAuthSettings->GetName()
			, InResponse.GetData());
