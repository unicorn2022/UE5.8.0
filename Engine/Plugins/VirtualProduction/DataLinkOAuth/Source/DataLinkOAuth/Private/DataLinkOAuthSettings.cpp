...
bool UDataLinkOAuthSettings::BuildAuthToken(FStringView InAccessResponse, FDataLinkOAuthToken& OutAuthToken) const
{
	...
	{
		UE_LOGF(LogDataLinkOAuth, Error, "Response %ls could not be converted to a Json Object", InAccessResponse.GetData());
		return false;
	}

	if (!ResponseJson->TryGetStringField(TEXT("token_type"), OutAuthToken.TokenType))
	{
		UE_LOGF(LogDataLinkOAuth, Error, "Response %ls did not have a valid 'token_type' entry", InAccessResponse.GetData());
		return false;
	}

	if (!ResponseJson->TryGetStringField(TEXT("access_token"), OutAuthToken.AccessToken))
	{
		UE_LOGF(LogDataLinkOAuth, Error, "Response %ls did not have a valid 'access_token' entry", InAccessResponse.GetData());
		return false;
	}

	if (!ResponseJson->TryGetStringField(TEXT("refresh_token"), OutAuthToken.RefreshToken))
	{
		UE_LOGF(LogDataLinkOAuth, Warning, "Response %ls did not have a valid 'refresh_token' entry", InAccessResponse.GetData());
	}
	...
	{
		UE_LOGF(LogDataLinkOAuth, Warning, "Response %ls did not have a valid 'expires_in' entry", InAccessResponse.GetData());
	}
