...
const FName STimedDataMonitorPanel::TabName = "TimedDataSourceMonitorPanelApp";


void STimedDataMonitorPanel::RegisterNomadTabSpawner(TSharedPtr<FTabManager> TabManager, TSharedRef<FWorkspaceItem> InWorkspaceItem)
{
	if (TabManager)
	{
		TabManager->RegisterTabSpawner(Utilities::NAME_App, FOnSpawnTab::CreateStatic(&Utilities::CreateApp))
...
void STimedDataMonitorPanel::UnregisterNomadTabSpawner(TSharedPtr<FTabManager> TabManager)
{
	FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(Utilities::NAME_TimedDataMonitorBuffers);

	if (TabManager)
	{
		TabManager->UnregisterTabSpawner(Utilities::NAME_App);
	}
}
	...
	TSharedRef<SWidget> Toolbar = 
		...
		[
			...
			[
				SNew(SButton)
				.ButtonStyle(FTimedDataMonitorEditorStyle::Get(), "ToggleButton")
				.ToolTipText(LOCTEXT("SavePreset_ToolTip", "Save all monitored input settings and the Live Link preset"))
				.ContentPadding(FMargin(4, 4))
				.HAlign(HAlign_Center)
				.VAlign(VAlign_Center)
				.OnClicked(this, &STimedDataMonitorPanel::OnSavePreset)
				[
					SNew(SImage)
					.Image(this, &STimedDataMonitorPanel::GetSaveButtonBrush)
...
FLiveLinkClient* STimedDataMonitorPanel::GetLiveLinkClient() const
{
	if (IModularFeatures::Get().IsModularFeatureAvailable(ILiveLinkClient::ModularFeatureName))
	{
		return static_cast<FLiveLinkClient*>(&IModularFeatures::Get().GetModularFeature<ILiveLinkClient>(ILiveLinkClient::ModularFeatureName));
	}
	return nullptr;
}

namespace SavePresetUtils
{
	...
	static bool GetSavePresetPackageName(FString& OutName)
	{
		...
		while (!bFilenameValid)
		{
			...
			FText OutError;
			bFilenameValid = FPackageName::IsValidLongPackageName(NewPackageName, false, &OutError);
		}
...
void STimedDataMonitorPanel::CollectInputSettingsPackages(TArray<UPackage*>& OutPackages)
{
	// If the timecode provider is an asset sub-object (e.g. from a media profile), include its package.
	if (UTimecodeProvider* TimecodeProvider = GEngine->GetTimecodeProvider())
	{
		if (TimecodeProvider->GetOuter() != GEngine)
		{
			UPackage* Package = TimecodeProvider->GetPackage();
			if (Package && Package != GetTransientPackage())
			{
				TimecodeProvider->MarkPackageDirty();
				OutPackages.AddUnique(Package);
			}
		}
	}

	UTimedDataMonitorSubsystem* Subsystem = GEngine->GetEngineSubsystem<UTimedDataMonitorSubsystem>();
	...
	TArray<FTimedDataMonitorInputIdentifier> AllInputs = Subsystem->GetAllInputs();

	for (const FTimedDataMonitorInputIdentifier& InputId : AllInputs)
	{
		ITimedDataInput* Input = Subsystem->GetTimedDataInput(InputId);
		...
		UObject* SettingsObject = Input->GetSettingsObject();
		...
		UPackage* Package = SettingsObject->GetPackage();

		if (Package && Package != GetTransientPackage())
		{
			SettingsObject->MarkPackageDirty();
			OutPackages.AddUnique(Package);
		}
	}
}

const FSlateBrush* STimedDataMonitorPanel::GetSaveButtonBrush() const
{
	return FAppStyle::GetBrush(bSettingsDirty ? TEXT("Icons.SaveChanged") : TEXT("Icons.Save"));
}

FReply STimedDataMonitorPanel::OnSavePreset()
{
	FLiveLinkClient* LiveLinkClient = GetLiveLinkClient();
	const bool bHasLiveLinkSources = LiveLinkClient && !LiveLinkClient->GetSources().IsEmpty();

	// If LL sources exist but no preset has been saved yet, prompt for a name.
	if (bHasLiveLinkSources && !LiveLinkClient->GetLastSavedPreset())
	{
		OnSaveAsPreset();
		return FReply::Handled();
	}
	...
	if (bHasLiveLinkSources)
	{
		ULiveLinkPreset* CurrentPreset = LiveLinkClient->GetLastSavedPreset();
		FScopedTransaction Transaction(LOCTEXT("SavePreset", "Save Preset"));
		CurrentPreset->BuildFromClient();
		CurrentPreset->MarkPackageDirty();
		PackagesToSave.Add(CurrentPreset->GetPackage());
	}

	CollectInputSettingsPackages(PackagesToSave);
...
void STimedDataMonitorPanel::OnSaveAsPreset()
{
	FString PackageName;
	if (!SavePresetUtils::GetSavePresetPackageName(PackageName))
	{
		return;
	}
	...
	if (NewPreset)
	{
		...
		if (Result == FEditorFileUtils::PR_Success)
		{
			...
			if (LiveLinkClient)
			{
				LiveLinkClient->SetLastSavedPreset(NewPreset);
			}
		}
	}
}
