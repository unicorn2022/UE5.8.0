...
void FAvaSceneStateRCTask::OnStart(const FSceneStateExecutionContext& InContext, FStructView InTaskInstance) const
{
	...
	{
		UE_LOGF(LogAvaSceneState, Warning, "[%ls] No valid source data to copy from!", *InContext.GetExecutionContextName());
		return;
	}
	...
	for (const FPropertyBagPropertyDesc& SourcePropertyDesc : SourcePropertyBag->GetPropertyDescs())
	{
		...
		{
			UE_LOGF(LogAvaSceneState, Warning, "[%ls] Property '%ls' was not invalid in the controller values!"
		...
		{
			UE_LOGF(LogAvaSceneState, Warning, "[%ls] Property '%ls' was not found in the controller mappings!"
				, *InContext.GetExecutionContextName()
				, *SourcePropertyDesc.Name.ToString());
			continue;
		}
		...
		if (!Controller)
		{
			UE_LOGF(LogAvaSceneState, Warning, "[%ls] Controller '%ls' was not found in preset '%ls'!"
				, *InContext.GetExecutionContextName()
				, *SourcePropertyDesc.Name.ToString()
				, *Preset->GetName());
			continue;
		...
		if (!TargetProperty)
		{
			UE_LOGF(LogAvaSceneState, Warning, "[%ls] Controller '%ls' has invalid target property in preset '%ls'"
				, *InContext.GetExecutionContextName()
		...
		{
			UE_LOGF(LogAvaSceneState, Warning, "[%ls] Controller '%ls' has type incompatibility in preset '%ls'"
				, *InContext.GetExecutionContextName()
				, *Controller->DisplayName.ToString()
				, *Preset->GetName());
			continue;
...
bool FAvaSceneStateRCTask::GetControllerDataView(const FSceneStateExecutionContext& InContext, FStructView& OutControllerView, URemoteControlPreset*& OutPreset) const
{
	...
	if (!OutPreset)
	{
		UE_LOGF(LogAvaSceneState, Warning, "[%ls] Failed to find Remote Control Preset in Scene!", *InContext.GetExecutionContextName());
		return false;
	}
