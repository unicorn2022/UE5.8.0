...
void FSVGParsingUtils::PointsFromString(FString InPointsString, TArray<FVector2D>& OutPoints)
{
	...
	if (Points.Num() % 2 != 0)
	{
		UE_LOGF(SVGImporterEditorLog, Warning, "Trying to parse SVG attribute as point list, but number of resulting members is not even %i. Returned points list will be empty. %ls", Points.Num(), *InPointsString);
		return;
	}
...
TArray<TArray<FSVGPathCommand>> FSVGParsingUtils::ParseStringAsPathCommands(const FString& InString)
{
	...
	{
		UE_LOGF(SVGImporterEditorLog, Warning, "Trying to parse SVG attribute as path, but first instruction is not a moveto! Returned path commands list will be empty.");
		return SubPaths;
	}
	...
	for (FString& CommandStringElem : CommandsToParse)
	{
		...
		if (bInvalidCommand)
		{
			UE_LOGF(SVGImporterEditorLog, Warning, "Invalid svg command detected during parsing (%ls), skipping entire svg subpath (%ls)", *CommandStringElem, *InString);
			SubPaths.Empty();
			break;
		}
	}
