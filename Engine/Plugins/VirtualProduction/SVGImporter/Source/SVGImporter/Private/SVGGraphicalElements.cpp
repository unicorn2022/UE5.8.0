	...
	, Points(InPoints)
{
	...
	if (Points.IsEmpty())
	{
		UE_LOGF(LogSVGImporter, Warning, "Trying to setup SVG Polygon with empty points list.");
		return;
	}
	...
	: Cx(InCx), Cy(InCy), R(InR)
{
	...
	if (R <= 0.0f)
	{
		UE_LOGF(LogSVGImporter, Warning, "Trying to setup SVG Circle with zero or negative radius");
		return;
	}
	...
	: Cx(InCx), Cy(InCy), Rx(InRx), Ry(InRy)
{
	...
	if (Rx <= 0.0f || Ry <= 0.0f)
	{
		UE_LOGF(LogSVGImporter, Warning, "Trying to setup SVG Ellipse with zero or negative value for X and/or Y radius");
		return;
	}
