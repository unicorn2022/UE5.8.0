...
void FDataLinkExecutor::Next(const UDataLinkNode* InNode)
{
	...
	if (InputPinsToExecute.IsEmpty())
	{
		...
		{
			UE_LOGF(LogDataLink, Error, "[%ls] Node ('%ls') has no node to go to and it's not an output node!", GetContextName().GetData(), *InNode->GetName());
			BroadcastFailure();
			return;
		}
...
void FDataLinkExecutor::Fail(const UDataLinkNode* InNode)
{
	...
	UE_LOGF(LogDataLink, Log, "[%ls] Node ('%ls') has failed to execute. Data Link execution finished.", GetContextName().GetData(), *InNode->GetName());
	BroadcastFailure();
}
...
bool FDataLinkExecutor::ValidateRun()
{
	...
	{
		UE_LOGF(LogDataLink, Error, "[%.*ls] Invalid Data Link specified!"
			, GetContextName().Len()
	...
	{
		UE_LOGF(LogDataLink, Error, "[%.*ls] DataLink Graph '%ls' requires %d inputs but %d were provided"
			, GetContextName().Len()
			, GetContextName().GetData()
	...
	const bool bResult = Instance.DataLinkGraph->ForEachInputPin(
		[&PinIndex, this](FDataLinkPinReference InPinReference)->bool
		{
			...
			{
				UE_LOGF(LogDataLink, Error, "[%.*ls] Input pin (%ls': input struct '%ls', owner '%ls') is not compatible with input data view '%ls'"
					, GetContextName().Len()
					, GetContextName().GetData()
...
bool FDataLinkExecutor::ValidateGraphOutputNode()
{
	...
	if (!OutputNode)
	{
		UE_LOGF(LogDataLink, Error, "[%.*ls] Invalid Data Link output node! Graph: %ls"
			, GetContextName().Len()
			, GetContextName().GetData()
			, *Instance.DataLinkGraph->GetName());
		return false;
	...
	if (!OutputNode->GetOutputPins().Contains(OutputPinName))
	{
		...
		{
			UE_LOGF(LogDataLink, Warning, "[%.*ls] Output pin name '%ls' was not found as a valid pin name in output node '%ls' in Data Link Graph '%ls'. Recompiling graph..."
				, GetContextName().Len()
				, GetContextName().GetData()
			...
			{
				UE_LOGF(LogDataLink, Error, "[%.*ls] Graph '%ls' failed to compile. Recompile graph manually."
					, GetContextName().Len()
					, GetContextName().GetData()
					, *Instance.DataLinkGraph->GetName());
				return false;
		...
		{
			UE_LOGF(LogDataLink, Error, "[%.*ls] Output pin name '%ls' was not found as a valid pin name in output node '%ls' in Data Link Graph '%ls'. Recompile Graph manually."
				, GetContextName().Len()
				, GetContextName().GetData()
				, *OutputPinName.ToString()
				, *OutputNode->GetName()
...
bool FDataLinkExecutor::ValidateInputPins(TConstArrayView<FDataLinkPinReference> InInputPins, TConstArrayView<FConstStructView> InInputDataViews) const
{
	...
	{
		UE_LOGF(LogDataLink, Error, "[%ls] Input Pin count %d does not match Input Data View %d"
			, GetContextName().GetData()
			, InInputPins.Num()
	...
	for (int32 InputIndex = 0; InputIndex < InInputPins.Num(); ++InputIndex)
	{
		...
		{
			UE_LOGF(LogDataLink, Error, "[%ls] Input Data struct '%ls' does not match expected input pin '%ls' of type '%ls'"
				, GetContextName().GetData()
...
bool FDataLinkExecutor::ValidateNodeExecutionStatus(const UDataLinkNode* InNode)
{
	...
	{
		UE_LOGF(LogDataLink, Log, "[%.*ls] Node %ls Finished Execution."
			"\n-------------------------------------------------------------"
			"\nINPUT PINS:\n%ls"
			"\n-------------------------------------------------------------"
			"\nOUTPUT PINS:\n%ls"
