...
EDataLinkExecutionReply UDataLinkConstant::OnExecute(FDataLinkExecutor& InExecutor) const
{
	if (!Instance.IsValid())
	{
		UE_LOGF(LogDataLink, Error, "[%.*ls] DataLinkConstant failed. Instance is not valid"
	...
	{
		UE_LOGF(LogDataLink, Error, "[%.*ls] DataLinkConstant failed. %ls does not match output struct %ls"
			, InExecutor.GetContextName().Len(), InExecutor.GetContextName().GetData()
