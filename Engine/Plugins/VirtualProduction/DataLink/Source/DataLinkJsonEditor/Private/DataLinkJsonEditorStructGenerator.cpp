...
UE::DataLinkJsonEditor::FStructKey::FStructKey(FStructGenerator& InStructGenerator, const TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& InJsonEntries)
{
	Hash = 0;
	JsonTypeMap.Reserve(InJsonEntries.Num());
	for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& JsonEntry : InJsonEntries)
	{
		FEdGraphPinType Type;
		FString Key(JsonEntry.Key);
		if (InStructGenerator.FromJsonValue(JsonEntry.Value, Type, Key))
		{
...
bool UE::DataLinkJsonEditor::FStructGenerator::FromJsonValue(const TSharedPtr<FJsonValue>& InJsonValue, FEdGraphPinType& OutPinType, const FString& InNameToUse)
{
	...
	{
		UE_LOGF(LogDataLinkJsonEditor, Error, "Invalid Json Value '%ls'", *InNameToUse);
		return false;
	}
	...
	case EJson::Array:
		{
			...
			{
				UE_LOGF(LogDataLinkJsonEditor, Error, "Empty array '%ls' is not supported!", *InNameToUse);
				return false;
			}
			...
			{
				UE_LOGF(LogDataLinkJsonEditor, Error, "Nested array '%ls' is not supported!", *InNameToUse);
				return false;
			}
		...
		UE_LOGF(LogDataLinkJsonEditor, Error, "Could not create a valid pin array type for '%ls'", *InNameToUse);
		return false;
	...
	case EJson::Object:
		...
		UE_LOGF(LogDataLinkJsonEditor, Error, "Could not create a valid struct type for '%ls'", *InNameToUse);
		return false;
	}

	UE_LOGF(LogDataLinkJsonEditor, Error, "Unsupported Json type encountered at '%ls'", *InNameToUse);
	return false;
}
...
UUserDefinedStruct* UE::DataLinkJsonEditor::FStructGenerator::CreateEmptyStruct(const FString& InNameToUse)
{
	...
	{
		UE_LOGF(LogDataLinkJsonEditor, Error, "Could not create a valid struct. Empty name!");
		return nullptr;
	}
	...
	if (!FName::IsValidXName(InNameToUse, INVALID_OBJECTNAME_CHARACTERS))
	{
		...
		UE_LOGF(LogDataLinkJsonEditor, Log, "Input Struct name '%ls' is not valid. Cleaned name to '%ls'", *InNameToUse, *CleanName);
...
UUserDefinedStruct* UE::DataLinkJsonEditor::FStructGenerator::GetOrCreateStruct(const TSharedPtr<FJsonObject>& InJsonObject, const FString& InNameToUse)
{
	...
	{
		UE_LOGF(LogDataLinkJsonEditor, Error, "Could not create a valid struct. Json Object: '%ls'", *InNameToUse);
		return nullptr;
	}
	...
	for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& JsonEntry : InJsonObject->Values)
	{
		AddVariable(Struct, FString(JsonEntry.Key), JsonEntry.Value);
	}
