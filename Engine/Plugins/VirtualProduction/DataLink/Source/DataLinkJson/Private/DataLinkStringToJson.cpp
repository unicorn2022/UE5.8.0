...
EDataLinkExecutionReply UDataLinkStringToJson::OnExecute(FDataLinkExecutor& InExecutor) const
{
	...
	// Deserialize JSON
	{
		...
		{
			UE_LOGF(LogDataLinkJson, Error, "[%.*ls] Failed to deserialize JSON from string %ls"
	...
	if (JsonValue->TryGetArray(JsonArray) && ensure(JsonArray))
	{
		...
		{
			UE_LOGF(LogDataLinkJson, Error, "[%.*ls] Failed to find a valid JSON object from string %ls. Expected 1 valid json object in top level array. Found %d"
				, InExecutor.GetContextName().Len()
				, InExecutor.GetContextName().GetData()
				, *InputData.Value
				, JsonArray->Num());
	...
	{
		UE_LOGF(LogDataLinkJson, Error, "[%.*ls] Failed to find a valid JSON object from string %ls"
			, InExecutor.GetContextName().Len()
			, InExecutor.GetContextName().GetData()
			, *InputData.Value);
		return EDataLinkExecutionReply::Unhandled;
