...
void UDMMaterialStage::GenerateExpressions(const TSharedRef<FDMMaterialBuildState>& InBuildState) const
{
	...
	if (!IsValid(Source))
	{
		UE_LOGF(LogDynamicMaterialEditor, Warning, "Stage with no source attempted to generate material expressions.");
		return;
	}
...
void UDMMaterialStage::GeneratePreviewMaterial(UMaterial* InPreviewMaterial)
{
	...
	UE_LOGF(LogDynamicMaterialEditor, Display, "Building Material Designer Stage Preview (%ls)...", *GetName());
