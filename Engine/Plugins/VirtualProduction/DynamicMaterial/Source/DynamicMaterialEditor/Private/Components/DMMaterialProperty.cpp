...
void UDMMaterialProperty::GeneratePreviewMaterial(UMaterial* InPreviewMaterial)
{
	...
	UE_LOGF(LogDynamicMaterialEditor, Display, "Building Material Designer Property Preview (%ls)...", *GetName());
...
bool UDMMaterialProperty::IsValidForModel(const UDynamicMaterialModelEditorOnlyData& InEditorOnlyData) const
{
	if (UMaterial* GeneratedMaterial = InEditorOnlyData.GetGeneratedMaterial())
	{
		return GeneratedMaterial->IsPropertyActiveInEditor(FDMUtils::MaterialPropertyTypeToMaterialProperty(MaterialProperty));
	}
