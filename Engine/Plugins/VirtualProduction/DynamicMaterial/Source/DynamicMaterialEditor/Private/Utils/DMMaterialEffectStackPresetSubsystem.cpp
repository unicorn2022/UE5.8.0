...
namespace UE::DynamicMaterialEditor::Private
{
	...
	static const FJsonObject::FStringType EffectClassFieldName(TEXT("Class"));
	static const FJsonObject::FStringType EffectDataFieldName(TEXT("Data"));
	...
	bool LoadPreset(const FString& InPath, FDMMaterialEffectStackJson& OutPreset)
	{
		...
		{
			UE_LOGF(LogDynamicMaterialEditor, Error, "Material Effect Preset Subsystem: LoadPreset() - Unable to load Json file: %ls", *InPath);
			return false;
		}
		...
		{
			UE_LOGF(LogDynamicMaterialEditor, Error, "Material Effect Preset Subsystem: LoadPreset() - Unable to parse file [%ls]. Json=[%ls]", *InPath, *FileJsonString);
			return false;
		}

		if (!RootObject->HasTypedField<EJson::Array>(EffectListFieldName))
		{
			UE_LOGF(LogDynamicMaterialEditor, Error, "Material Effect Preset Subsystem: LoadPreset() - Malformed data in json [%ls]. Json=[%ls]", *InPath, *FileJsonString);
			return false;
		}
		...
		for (const TSharedPtr<FJsonValue>& JsonMaterialEffect : EffectList)
		{
			...
			if (JsonMaterialEffect->TryGetObject(JsonMaterialEffectObject))
			{
				...
				{
					UE_LOGF(LogDynamicMaterialEditor, Error, "Material Effect Preset Subsystem: LoadPreset() - Invalid class in json [%ls]. Json=[%ls]", *InPath, *FileJsonString);
					return false;
				}
			...
			{
				UE_LOGF(LogDynamicMaterialEditor, Warning, "Material Effect Preset Subsystem: LoadPreset() - Malformed data in json [%ls]. Json=[%ls]", *InPath, *FileJsonString);
				return false;
			}
		}
	...
	bool SavePreset(const FString& InPath, const FDMMaterialEffectStackJson& InPreset)
	{
		...
		{
			UE_LOGF(LogDynamicMaterialEditor, Warning, "Material Effect Preset Subsystem: SavePreset() - Unable to serialize [%ls]. Json=[%ls]", *InPath, *FileJsonString);
			return false;
		}

		if (!FFileHelper::SaveStringToFile(FileJsonString, *InPath))
		{
			UE_LOGF(LogDynamicMaterialEditor, Warning, "Material Effect Preset Subsystem: SavePreset() - Unable to save Json file: %ls", *InPath);
			return false;
		}
...
bool UDMMaterialEffectStackPresetSubsystem::SavePreset(const FString& InPresetName, const FDMMaterialEffectStackJson& InPreset) const
{
	...
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Material Effect Preset Subsystem: SavePreset() - Failed [%ls]", *InPresetName);
		return false;
	}

	UE_LOGF(LogDynamicMaterialEditor, Log, "Material Effect Preset Subsystem: SavePreset() - Success [%ls]", *InPresetName);
	return true;
}
...
bool UDMMaterialEffectStackPresetSubsystem::LoadPreset(const FString& InPresetName, FDMMaterialEffectStackJson& OutPreset) const
{
	...
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Material Effect Preset Subsystem: LoadPreset() - Failed [%ls]", *InPresetName);
		return false;
	}

	UE_LOGF(LogDynamicMaterialEditor, Log, "Material Effect Preset Subsystem: LoadPreset() - Success [%ls]", *InPresetName);
	return true;
}
