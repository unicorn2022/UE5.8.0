...
bool FDMMaterialShapshotLibrary::SnapshotMaterial(UMaterialInterface* InMaterial, const FIntPoint& InTextureSize, const FString& InSavePath)
{
	...
	if (!RenderTarget)
	{
		UE_LOGF(LogDynamicMaterialEditor, Warning, "Unable to create render target for material snapshot.");
		return false;
	}
	...
	if (!Texture)
	{
		UE_LOGF(LogDynamicMaterialEditor, Warning, "Unable to texture for material snapshot.");
		return false;
	}
	...
	if (!Package)
	{
		UE_LOGF(LogDynamicMaterialEditor, Warning, "Unable to create package for material snapshot.");
		return false;
	}
