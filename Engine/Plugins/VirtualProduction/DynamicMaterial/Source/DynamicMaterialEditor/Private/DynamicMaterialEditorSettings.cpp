...
UDynamicMaterialEditorSettings::UDynamicMaterialEditorSettings()
{
	...
	Opaque.UsageFlag |= EDMMaterialFlagsUsage::DMMFU_StaticMesh | EDMMaterialFlagsUsage::DMMFU_NiagaraMeshParticles;
	Opaque.GeneralFlags |= static_cast<uint8>(EDMMaterialFlagsGeneral::TwoSided);
	...
	Emissive.UsageFlag |= EDMMaterialFlagsUsage::DMMFU_StaticMesh | EDMMaterialFlagsUsage::DMMFU_NiagaraMeshParticles;
	Emissive.GeneralFlags |= static_cast<uint8>(EDMMaterialFlagsGeneral::TwoSided);
	...
	Translucent.UsageFlag |= EDMMaterialFlagsUsage::DMMFU_StaticMesh | EDMMaterialFlagsUsage::DMMFU_NiagaraMeshParticles;
	Translucent.GeneralFlags |= static_cast<uint8>(EDMMaterialFlagsGeneral::TwoSided);
	...
	PBR.UsageFlag |= EDMMaterialFlagsUsage::DMMFU_StaticMesh | EDMMaterialFlagsUsage::DMMFU_NiagaraMeshParticles;
	PBR.GeneralFlags |= static_cast<uint8>(EDMMaterialFlagsGeneral::TwoSided);
	...
	All.UsageFlag |= EDMMaterialFlagsUsage::DMMFU_StaticMesh | EDMMaterialFlagsUsage::DMMFU_NiagaraMeshParticles;
	All.GeneralFlags |= static_cast<uint8>(EDMMaterialFlagsGeneral::TwoSided);
...
void UDynamicMaterialEditorSettings::PostInitProperties()
{
	...
	if (!bConvertedVarsToFlags)
	{
		ConvertDeprecatedVarsToFlags();
		bConvertedVarsToFlags = true;
	}
}
...
void UDynamicMaterialEditorSettings::ResetAllLayoutSettings()
{
	...
	PreviewSplitterLocation = 0.333f;
	ThumbnailSize = 256.f;
...
void UDynamicMaterialEditorSettings::EnsureUniqueChannelPresetNames()
{
	...
	for (int32 IndexBase = 0; IndexBase < Count; ++IndexBase)
	{
		...
		for (int32 IndexCheck = 0; IndexCheck < Count; ++IndexCheck)
		{
			...
			UE_LOGF(LogDynamicMaterialEditor, Warning, "Duplicate channel list preset name detected.");
...
void UDynamicMaterialEditorSettings::ConvertDeprecatedVarsToFlags()
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS

	for (FDMMaterialChannelListPreset& Preset : MaterialChannelPresets)
	{
		if (Preset.bDefaultTwoSided_DEPRECATED)
		{
			Preset.GeneralFlags |= static_cast<uint8>(EDMMaterialFlagsGeneral::TwoSided);
		}

		if (Preset.bDefaultAnimated_DEPRECATED)
		{
			Preset.MotionFlags |= static_cast<uint8>(EDMMaterialFlagsMotion::PixelAnimation);
		}
	}
