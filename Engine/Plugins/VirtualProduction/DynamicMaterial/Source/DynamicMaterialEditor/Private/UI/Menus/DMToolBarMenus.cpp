...
void FDMToolBarMenus::SnapshotMaterial(TWeakObjectPtr<UDynamicMaterialModelBase> InMaterialModelBaseWeak, FIntPoint InTextureSize)
{
	...
	if (MaterialInstance)
	{
		...
		{
			UE_LOGF(LogDynamicMaterialEditor, Warning, "Unable to find world to find material parent.");
			return;
		}
	...
	if (!Material)
	{
		UE_LOGF(LogDynamicMaterialEditor, Warning, "Unable to find material to snapshot.");
		return;
	}
