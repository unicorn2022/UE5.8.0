...
FReply SDMToolBar::OnConvertToEditableClicked()
{
	...
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Tried to convert a null or non-dynamic model to editable.");
		return FReply::Handled();
	}
	...
	if (!ParentModel)
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Failed to find parent model.");
		return FReply::Handled();
	}
	...
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Cannot create a new asset for embedded instances without an active world subsystem.");
		return FReply::Handled();
	}
	...
	if (!NewModel)
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Failed to create new model.");
		return FReply::Handled();
	}
...
FReply SDMToolBar::OnConvertToInstanceClicked()
{
	...
	if (!CurrentModel)
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Tried to convert a null or dynamic model to an instance.");
		return FReply::Handled();
	}
	...
	if (!OldInstance)
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Unable to find material to replace.");
		return FReply::Handled();
	}
	...
	if (!bIsAsset)
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Tried to convert a non-asset material to an instance.");
		return FReply::Handled();
	}
	...
	if (ActorMaterialProperties.IsEmpty() 
		...
		|| !GetMaterialActor())
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Unable to find actor or actor property to insert the new instance.");
		return FReply::Handled();
	}
	...
	if (!NewInstance)
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Failed to create new material.");
		return FReply::Handled();
	}

	if (!UDMMaterialModelFunctionLibrary::CreateModelInstanceInMaterial(CurrentModel, NewInstance))
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Failed to create new material model instance.");
		return FReply::Handled();
	}
	...
	{
		UE_LOGF(LogDynamicMaterialEditor, Error, "Failed to assign new material to actor.");
		return FReply::Handled();
	}
