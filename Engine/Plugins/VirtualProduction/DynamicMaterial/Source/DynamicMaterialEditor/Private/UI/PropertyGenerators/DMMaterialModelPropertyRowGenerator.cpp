...
void FDMMaterialModelPropertyRowGenerator::AddMaterialModelProperties(FDMComponentPropertyRowGeneratorParams& InParams)
{
	...
	if (UDynamicMaterialModelEditorOnlyData* EditorOnlyData = UDynamicMaterialModelEditorOnlyData::Get(MaterialModelBase))
	{
		...
		AddVariable(InParams, EditorOnlyData,
			GET_MEMBER_NAME_CHECKED(UDynamicMaterialModelEditorOnlyData, UsageFlags));

		AddVariable(InParams, EditorOnlyData,
			GET_MEMBER_NAME_CHECKED(UDynamicMaterialModelEditorOnlyData, GeneralFlags));

		AddVariable(InParams, EditorOnlyData,
			GET_MEMBER_NAME_CHECKED(UDynamicMaterialModelEditorOnlyData, LightingFlags));

		AddVariable(InParams, EditorOnlyData,
			GET_MEMBER_NAME_CHECKED(UDynamicMaterialModelEditorOnlyData, TranslucencyFlags));

		AddVariable(InParams, EditorOnlyData,
			GET_MEMBER_NAME_CHECKED(UDynamicMaterialModelEditorOnlyData, OpacityMaskClipValue));

		AddVariable(InParams, EditorOnlyData,
			GET_MEMBER_NAME_CHECKED(UDynamicMaterialModelEditorOnlyData, MotionFlags));

		AddVariable(InParams, EditorOnlyData,
			GET_MEMBER_NAME_CHECKED(UDynamicMaterialModelEditorOnlyData, ForwardRendererFlags));
