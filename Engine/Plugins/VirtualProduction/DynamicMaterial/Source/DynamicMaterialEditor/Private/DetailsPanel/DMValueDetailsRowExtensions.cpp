...
void FDMValueDetailsRowExtensions::SetValueParameterName(TWeakObjectPtr<UDMMaterialValue> InValueWeak, FName InName)
{
	...
	UE_LOGF(LogDynamicMaterialEditor, Log, "Parameter renamed to: %ls", *Value->GetMaterialParameterName().ToString());
}
	...
	int32 InComponent, FName InName)
{
	...
	UE_LOGF(LogDynamicMaterialEditor, Log, "Parameter renamed to: %ls", *TextureUV->GetMaterialParameterName(InPropertyName, InComponent).ToString());
}
