...
{
	const FJsonObject::FStringType JsonKey_Class(TEXT("Class"));
	const FJsonObject::FStringType JsonKey_Data(TEXT("Data"));
}
...
TSharedPtr<FJsonValue> FDMJsonUtils::Serialize(const UScriptStruct* InScriptStruct, const void* InData)
{
	...
	{
		UE_LOGF(LogDynamicMaterial, Error, "Invalid script struct.");
		return MakeShared<FJsonValueNull>();
	}
	...
	UE_LOGF(LogDynamicMaterial, Error,"Failed to convert struct to json. [%ls]", *InScriptStruct->GetName());
	return MakeShared<FJsonValueNull>();
}
...
TSharedPtr<FJsonValue> FDMJsonUtils::Serialize(const UObject* InObject)
{
	...
	UE_LOGF(LogDynamicMaterial, Error, "Failed to convert object to json. [%ls] [%ls]", 
...
TSharedPtr<FJsonValue> FDMJsonUtils::Serialize(const TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& InMap)
{
...
bool FDMJsonUtils::DeserializeNumber(const TSharedPtr<FJsonValue>& InJsonValue, double& OutNumber)
{
	...
	UE_LOGF(LogDynamicMaterial, Error, "Failed to read number from json.");
	return false;
}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, bool& bOutValue)
{
	...
	UE_LOGF(LogDynamicMaterial, Error,"Failed to read bool from json.");
	return false;
}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, FString& OutString)
{
	...
	UE_LOGF(LogDynamicMaterial, Error, "Failed to read string from json.");
	return false;
}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, FText& OutText)
{
	...
	UE_LOGF(LogDynamicMaterial, Error, "Failed to read text from json.");
	return false;
}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, FName& OutName)
{
	...
	UE_LOGF(LogDynamicMaterial, Error, "Failed to read name from json.");
	return false;
}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, UClass*& OutClass)
{
	...
	if (Deserialize(InJsonValue, ClassString))
	{
		...
		UE_LOGF(LogDynamicMaterial, Error, "Failed to find class. [%ls]", *ClassString);
	}
	else
	{
		UE_LOGF(LogDynamicMaterial, Error, "Failed to read class path.");
	}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, const UScriptStruct* InScriptStruct, void* OutData)
{
	...
	if (InJsonValue->TryGetObject(JsonObject))
	{
		...
		UE_LOGF(LogDynamicMaterial, Error, "Failed to find deserialize struct. [%ls]", *InScriptStruct->GetName());
	}
	else
	{
		UE_LOGF(LogDynamicMaterial, Error, "Failed to read struct data. [%ls]", *InScriptStruct->GetName());
	}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, const UEnum* InEnum, int64& OutValue)
{
	if (!InEnum)
	{
		UE_LOGF(LogDynamicMaterial, Error, "Invalid enum.");
		return false;
	}
	...
	if (Deserialize(InJsonValue, Value))
	{
		...
		UE_LOGF(LogDynamicMaterial, Error, "Invalid enum value. [%ls] [%lli]", *InEnum->GetName(), Value);
	}
	else
	{
		UE_LOGF(LogDynamicMaterial, Error, "Failed to read data from json.");
	}
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, UObject*& OutObject, UObject* InOuter)
{
	...
	if (InJsonValue->TryGetString(ObjectPath))
	{
		...
		UE_LOGF(LogDynamicMaterial, Error, "Missing object. [%ls]", *ObjectPath);
		return false;
	}
	...
	do
	{
		...
		TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>> Data;
		...
		{
			UE_LOGF(LogDynamicMaterial, Error, "Missing data in json.");
			break;
		}
		...
		{
			UE_LOGF(LogDynamicMaterial, Error, "Failed to read class from json.");
			break;
		}
		...
		{
			UE_LOGF(LogDynamicMaterial, Error, "Failed to read object data from json.");
			break;
		}
		...
		if (!Object)
		{
			UE_LOGF(LogDynamicMaterial, Error, "Failed to find instantiate class. [%ls]", *Class->GetName());
			break;
		}
		...
		if (Object->Implements<UDMJsonSerializable>())
		{
			...
			{
				UE_LOGF(LogDynamicMaterial, Error, "Failed to find deserialize object.");
				break;
			}
		}
		else
...
bool FDMJsonUtils::Deserialize(const TSharedPtr<FJsonValue>& InJsonValue, TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& OutMap)
{
	const TSharedPtr<FJsonObject>* JsonObject;
	...
	UE_LOGF(LogDynamicMaterial, Error, "Failed to read object from json.");		
	return false;
}
