...
void UDMMaterialValueColorAtlas::GenerateExpression(const TSharedRef<IDMMaterialBuildStateInterface>& InBuildState) const
{
	...
	// UMaterialExpressionCurveAtlasRowParameter::Compile returns early with a compile error when Atlas or Curve are null.
	// Letting this compile error propagate through Material Designer build process causes an RHI crash at draw time,
	// so a vector4 constant is added as a placeholder until this object has valid parameters set.
	// See UE-365324.
	if (!Atlas || !Curve)
	{
		UMaterialExpressionConstant4Vector* const VectorParameter = InBuildState->GetBuildUtils().CreateExpression<UMaterialExpressionConstant4Vector>(UE_DM_NodeComment_Default, InBuildState->GetDynamicMaterial());
		InBuildState->AddValueExpressions(this, {VectorParameter});
		return;
	}
...
TSharedPtr<FJsonValue> UDMMaterialValueColorAtlas::JsonSerialize() const
{
	return FDMJsonUtils::Serialize({
		{FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(ThisClass, Value)), FDMJsonUtils::Serialize(Value)},
		{FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(ThisClass, Atlas)), FDMJsonUtils::Serialize(Atlas)},
		{FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(ThisClass, Curve)), FDMJsonUtils::Serialize(Curve)}
	});
}
...
bool UDMMaterialValueColorAtlas::JsonDeserialize(const TSharedPtr<FJsonValue>& InJsonValue)
{
	...
	if (const TSharedPtr<FJsonValue>* JsonValue = Data.Find(FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(ThisClass, Atlas))))
	{
	...
	if (const TSharedPtr<FJsonValue>* JsonValue = Data.Find(FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(ThisClass, Curve))))
	{
	...
	if (const TSharedPtr<FJsonValue>* JsonValue = Data.Find(FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(ThisClass, Value))))
	{
		float ValueJson;
