...
public:
	...
	UPROPERTY(config)
	float TreeBindingSplitRatio = 0.7f;

	/** The split widget control ratio between entity tree and details/protocol binding list. */
	UPROPERTY(config)
	float ActionPanelSplitRatio = 0.6f;
	...
	/** 
	 * Enables any remote function execution, enabling this could open you up to vulnerabilities if an outside actor has access to your server. 
	 * If true, any remote function call will be allowed.
	 * If false, only the built-in allowed functions and those specified in 'CustomAllowedRemoteFunctionCalls' will be considered.
	 */
	...
	/**
	 * Specifies built-in function calls that are allowed to be called remotely. This is only considered if `bAllowAnyRemoteFunctionCall` is false 
	 * NOTE: For console command and python execution, 'bAllowConsoleCommandRemoteExecution' and 'bEnableRemotePythonExecution' are still required to be enabled respectively.
	 */
	UPROPERTY()
	TArray<FRCAllowedRemoteFunctionCall> AllowedRemoteFunctionCalls;

	/**
	 * Specifies custom function calls that are allowed to be called remotely. This is only considered if `bAllowAnyRemoteFunctionCall` is false 
	 * NOTE: For console command and python execution, 'bAllowConsoleCommandRemoteExecution' and 'bEnableRemotePythonExecution' are still required to be enabled respectively.
	 */
	UPROPERTY(Config, EditAnywhere, Category = "Remote Control | Security", meta = (EditCondition="!bAllowAnyRemoteFunctionCall", DisplayAfter="bAllowAnyRemoteFunctionCall"))
	TArray<FRCAllowedRemoteFunctionCall> CustomAllowedRemoteFunctionCalls;
