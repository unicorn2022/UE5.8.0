...
namespace PropertyContainers
{
	...
	FProperty* CreatePropertyInternal(const FFieldVariant& InParent, const FFieldVariant& InChild, const FName& InPropertyName)
	{
		...
		if (FieldClassTypeName == NAME_Object || FieldClassTypeName == NAME_Interface || FieldClassTypeName == NAME_SoftObjectProperty)
		{
			...
			if (SubType)
			{
				...
				if (bIsInterface)
				{
					FInterfaceProperty* NewPropertyObj = new FInterfaceProperty(PropertyScope, InPropertyName);
				...
				else
				{
					...
					{
						NewPropertyObj = new FSoftObjectProperty(PropertyScope, InPropertyName);
					}
					else
					{
						NewPropertyObj = new FObjectProperty(PropertyScope, InPropertyName);
					}
		...
		else if (FieldClassTypeName == NAME_StructProperty)
		{
			if (UScriptStruct* SubType = InChild.Get<UScriptStruct>())
			{
				...
				{
					FStructProperty* NewPropertyStruct = new FStructProperty(PropertyScope, InPropertyName);
				...
				{
					UE_LOGF(LogRemoteControlCommon, Error,
						"Invalid property '%ls' structure '%ls' error: %ls",
		...
		else if (FieldClassTypeName == NAME_Class)
		{
			...
			if (SubType == nullptr)
			{
				...
				UE_LOGF(LogRemoteControlCommon, Error,
					"Invalid property '%ls' class, replaced with Object.  Please fix or remove.",
					*InPropertyName.ToString());
			}
			...
			if (SubType)
			{
				...
				{
					FSoftClassProperty* SoftClassProperty = new FSoftClassProperty(PropertyScope, InPropertyName);
					// we want to use this setter function instead of setting the 
				...
				{
					FClassProperty* NewPropertyClass = new FClassProperty(PropertyScope, InPropertyName);
					// we want to use this setter function instead of setting the 
					// MetaClass member directly, because it properly handles  
					// placeholder classes (classes that are stubbed in during load)
		...
		{
			FBoolProperty* BoolProperty = new FBoolProperty(PropertyScope, InPropertyName);
		...
		else if(FieldClassTypeName == NAME_EnumProperty)
		{
			...
			if(Enum)
			{
				FEnumProperty* EnumProp = new FEnumProperty(PropertyScope, InPropertyName);
		...
		{
			NewProperty = new FInt8Property(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if (FieldClassTypeName == NAME_ByteProperty)
		{
			...
			{
				FByteProperty* ByteProp = new FByteProperty(PropertyScope, InPropertyName);
		...
		{
			NewProperty = new FInt16Property(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if(FieldClassTypeName == NAME_UInt16Property)
		{
			NewProperty = new FUInt16Property(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if (FieldClassTypeName == NAME_IntProperty)
		{ 
			NewProperty = new FIntProperty(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if(FieldClassTypeName == NAME_UInt32Property)
		{
			NewProperty = new FUInt32Property(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if (FieldClassTypeName == NAME_Int64Property)
		{
			NewProperty = new FInt64Property(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if(FieldClassTypeName == NAME_UInt64Property)
		{
			NewProperty = new FUInt64Property(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if (FieldClassTypeName == NAME_FloatProperty)
		{
			NewProperty = new FFloatProperty(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if (FieldClassTypeName == NAME_DoubleProperty)
		{
			NewProperty = new FDoubleProperty(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		...
		{
			NewProperty = new FStrProperty(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if (FieldClassTypeName == NAME_NameProperty)
		{
			NewProperty = new FNameProperty(PropertyScope, InPropertyName);
			NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
		}
		else if (FieldClassTypeName == NAME_TextProperty)
		{
			NewProperty = new FTextProperty(PropertyScope, InPropertyName);
		}
		
		else if (InParent.IsA<FFieldPathProperty>())
		{
			FFieldPathProperty* NewFieldPathProperty = new FFieldPathProperty(PropertyScope, InPropertyName);
	...
	FProperty* CreateProperty(const FFieldVariant& InParent, const FFieldVariant& InChild, const FName& InPropertyName)
	{
		...
		{
			bIsArray = true;
			NewArrayProperty = new FArrayProperty(PropertyScope, PropertyName);
		...
		{
			bIsMap = true;
			NewMapProperty = new FMapProperty(PropertyScope, PropertyName);
		...
		{
			bIsSet = true;
			NewSetProperty = new FSetProperty(PropertyScope, PropertyName);
		...
		{
			NewProperty = CreatePropertyInternal(PropertyScope, InChild, PropertyName);
		}
		...
		else if(bIsMap)
		{
			if(NewProperty)
			{
				...
				{
					UE_LOGF(LogRemoteControlCommon, Error, "Property has key of type %ls which cannot be hashed and is therefore invalid.", TEXT("@todo"));
				}
		...
		else if(bIsSet)
		{
			if(NewProperty)
			{
				if(!NewProperty->HasAnyPropertyFlags(CPF_HasGetValueTypeHash))
				{
					UE_LOGF(LogRemoteControlCommon, Error, "Property is set of type %ls which cannot be hashed and is therefore invalid.", TEXT("@todo"));
					NewProperty->SetPropertyFlags(CPF_HasGetValueTypeHash);
				}
...
URCPropertyContainerBase* URCPropertyContainerRegistry::CreateContainer(UObject* InOwner, const FName& InValueTypeName, const FProperty* InValueSrcProperty)
{
	...
	{
		UE_LOGF(LogRemoteControlCommon, Warning, "Could not create PropertyContainer found for %ls", *SanitizedName.ToString());
		return nullptr;
	}
}
