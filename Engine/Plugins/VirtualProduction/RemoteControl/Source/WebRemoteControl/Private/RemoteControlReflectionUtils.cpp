...
UScriptStruct* UWebRCStructRegistry::GenerateStructInternal(FName StructureName, const FWebRCGenerateStructArgs& Args)
{
	...
	{
		NewStruct->AddCppProperty(new FStrProperty(NewStruct, *ParamName.ToString()));
	}

	for (const TPair<FName, UScriptStruct*>& Pair : Args.ArrayProperties)
	{
		FArrayProperty* NewArrayProperty = new FArrayProperty(NewStruct, Pair.Key);

		FName InnerPathPropName(Pair.Key.ToString() + TEXT("_Inner"));
		FStructProperty* NewInnerProp = new FStructProperty(NewArrayProperty, InnerPathPropName);
	...
	{
		check(Pair.Value);
		FStructProperty* StructProperty = new FStructProperty(NewStruct, Pair.Key);
