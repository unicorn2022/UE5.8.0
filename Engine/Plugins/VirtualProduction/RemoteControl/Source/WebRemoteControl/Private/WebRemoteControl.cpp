...
void FWebRemoteControlModule::StartupModule()
{
	...
	{
		UE_LOGF(LogRemoteControl, Display, "Web remote control is disabled by default when running outside the editor. Use the -RCWebControlEnable flag when launching in order to use it.");
		return;
	}
...
void FWebRemoteControlModule::StartHttpServer()
{
	if (!HttpRouter)
	{
		...
		if (!HttpRouter)
		{
			UE_LOGF(LogRemoteControl, Error, "Web Remote Call server couldn't be started on port %d", HttpServerPort);
			return;
		}
...
void FWebRemoteControlModule::StartWebSocketServer()
{
	if (!WebSocketServer.IsRunning())
	{
		...
		{
			UE_LOGF(LogRemoteControl, Error, "Web Remote Call WebSocket server couldn't be started on port %d", WebSocketServerPort);
#if WITH_EDITOR
		...
		UE_LOGF(LogRemoteControl, Log, "Web Remote Control WebSocket server started on port %d", WebSocketServerPort);
		...
		)
	{
		...
		if (FStructDeserializer::Deserialize((void*)FunctionArgs.GetStructMemory(), *const_cast<UStruct*>(FunctionArgs.GetStruct()), ReaderBackend, FStructDeserializerPolicies()))
		{
			...
			for (UObject* Object : RCFunction->GetBoundObjects())
			{
				...
				FString ErrorText;
				if (!WebRemoteControlInternalUtils::ValidateFunctionCall(Call, &ErrorText))
				{
