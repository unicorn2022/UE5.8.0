...
#define IMAGE_PLUGIN_BRUSH_SVG( RelativePath, ... ) FSlateVectorImageBrush(FRemoteControlPanelStyle::InContent(RelativePath, ".svg" ), __VA_ARGS__)
...
void FRemoteControlPanelStyle::Initialize()
{
	...
	StyleSet->Set("Icons.RemoteControlPanel", new IMAGE_PLUGIN_BRUSH_SVG("Icons/RemoteControlPanel_16", Icon16x16));
	...
	ExposeFunctionButtonStyle.Normal.TintColor = FLinearColor(0, 0, 0, 0.1f);
	...
	NewSelectionColor.R *= 1.8f;
	NewSelectionColor.G *= 1.8f;
	NewSelectionColor.B *= 1.8f;
	NewSelectionColor.A = 0.4f;
