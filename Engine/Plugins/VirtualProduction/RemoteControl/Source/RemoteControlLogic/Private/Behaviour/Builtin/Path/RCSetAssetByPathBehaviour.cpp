...
bool URCSetAssetByPathBehaviour::SetInternalAsset(UObject* SetterObject)
{
	...
	if (const TWeakPtr<FRemoteControlProperty>* ExposedPropertyPtr = ExposedProperties.FindByPredicate([this](const TWeakPtr<FRemoteControlProperty>& Property){ return Property.Pin()->GetLabel() == TargetEntity->GetLabel(); }))
	{
		...
		if (AssetClass == UStaticMesh::StaticClass() && SetterObject->IsA(UStaticMesh::StaticClass()))
		{
			...
			UE_LOGF(LogRemoteControl, Verbose, "Path Behaviour sets Static Mesh Asset %ls", *SetterObject->GetName());
			return true;
		}
		...
		if (AssetClass == UMaterial::StaticClass() && SetterObject->IsA(UMaterial::StaticClass()))
		{
			...
			UE_LOGF(LogRemoteControl, Verbose, "Path Behaviour sets Material Asset %ls", *SetterObject->GetName());
			return true;
		}
	...
	if (TargetEntity)
	{
		...
		if (AssetClass == UBlueprint::StaticClass() && SetterObject->IsA(UBlueprint::StaticClass()))
		{
			...
			OldActor->Rename(*OldActorReplacedNamed.ToString(), OldActor->GetOuter(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
			...
			if (AActor* NewActor = World->SpawnActor(Cast<UBlueprint>(SetterObject)->GeneratedClass, &OldLocation, &OldRotation, SpawnParams))
			{
				...
				UE_LOGF(LogRemoteControl, Verbose, "Path Behaviour sets Blueprint Asset %ls", *SetterObject->GetName());
				return true;
			}

			UE_LOGF(LogRemoteControl, Error, "Path Behaviour unable to delete old Actor %ls", *OldActor->GetName());
			OldActor->UObject::Rename(*OldActorName.ToString(), OldActor->GetOuter(), REN_DoNotDirty | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
			OldActor->RegisterAllComponents();
		}
	}
	
	UE_LOGF(LogRemoteControl, Error, "Path Behaviour fails to set Asset %ls", *SetterObject->GetName());
	return false;
}
...
void URCSetAssetByPathBehaviour::ReadFileHttpHandler(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, TSharedPtr<FRemoteControlProperty> InRCPropertyToSet)
{
	...
	{
		UE_LOGF(LogTemp, Verbose, "ReadFile request complete, Url=%ls Code=%d", *HttpRequest->GetURL(), HttpResponse->GetResponseCode());
