...
void FRemoteControlPresetRebindingManager::Rebind_NewAlgo(URemoteControlPreset* Preset)
{
	...
	auto RebindComponentBasedOnClass = [&](URemoteControlLevelDependantBinding* InBinding, AActor* InActorMatch)
	{
		if (UClass* SupportedClass = InBinding->BindingContext.SupportedClass.LoadSynchronous())
		{
			if (SupportedClass->IsChildOf(UActorComponent::StaticClass()))
			{
				...
				UE_LOGF(LogRemoteControl, Verbose, "Potential Subobject candidates:");
				for (UActorComponent* Component : Components)
				{
					UE_LOGF(LogRemoteControl, Verbose, "\t%ls", *Component->GetName());

					if (TryRebindPair(InBinding, Component->GetOwner(), Component))
					{
						UE_LOGF(LogRemoteControl, Verbose, "Found a matching subobject (%ls) with a matching class (%ls) on actor %ls", *Component->GetName(), *Component->GetClass()->GetName(), *InActorMatch->GetName());
						return true;
					}
				}
			}
		...
		UE_LOGF(LogRemoteControl, Verbose, "Could not find an object to rebind using the binding class.");
		return false;
	};
	...
	{
		UE_LOGF(LogRemoteControl, Verbose, "Attempting to find subobject %ls on object %ls.", *BindingToRebind->BindingContext.ComponentName.ToString(), *PotentialMatch->GetName());
		
		if (UObject* TargetSubObject = GetSubObjectBasedOnName(BindingToRebind, PotentialMatch))
		{
			UE_LOGF(LogRemoteControl, Verbose, "Found a matching subobject (%ls) with a matching name on actor %ls", *TargetSubObject->GetName(), *PotentialMatch->GetName());
			return TryRebindPair(BindingToRebind, TargetSubObject->GetTypedOuter<AActor>(), TargetSubObject);
		}
		else
		{
			UE_LOGF(LogRemoteControl, Verbose, "Could not find a subobject using its name, trying to find one with a matching class (%ls)", *BindingToRebind->BindingContext.OwnerActorClass->GetClass()->GetName());
	...
	auto FindSubObjectToRebind = [&](URemoteControlLevelDependantBinding* InvalidBinding, const TArray<AActor*>& ObjectsWithSupportedClass)
	{
		...
		{
			UE_LOGF(LogRemoteControl, Verbose, "Found owner with a matching Owner Actor Name. (%ls)", *Match->GetName());
		...
		else
		{
			...
			UE_LOGF(LogRemoteControl, Verbose, "Could not find an actor with the same name, defaulting to the first actor that supports the binding.");
	...
	auto Rebind = [&](URemoteControlLevelDependantBinding* InvalidBinding)
	{
		...
		{
			UE_LOGF(LogRemoteControl, Verbose, "Potential Actor Matches: ");
			for (AActor* Actor : ObjectsWithSupportedClass)
			{
#if WITH_EDITOR
				UE_LOGF(LogRemoteControl, Verbose, "\t%ls (%ls)", *Actor->GetActorLabel(), *Actor->GetName());
#else
				UE_LOGF(LogRemoteControl, Verbose, "\t%ls", *Actor->GetName());
#endif
	...
	{
		UE_LOGF(LogRemoteControl, Verbose, "Binding did not contain a full component path, checking if it's a ndisplay actor.");
		if (UBlueprintGeneratedClass* BPClass = Cast <UBlueprintGeneratedClass>(InvalidBinding->BindingContext.OwnerActorClass.Get()))
		{
			...
			{
				UE_LOGF(LogRemoteControl, Verbose, "Binding was on a ndisplay actor.");
				TArray<AActor*> ObjectsWithSupportedClass;
				if (!GetPotentialActors(InvalidBinding, ObjectsWithSupportedClass))
				{
					UE_LOGF(LogRemoteControl, Verbose, "No potential ndisplay actor to rebind to.");
					return;
				}
				...
				{
					UE_LOGF(LogRemoteControl, Verbose, "Could not find the last bound object path on the binding.");
					return;
				}
				...
				for (AActor* Actor : ObjectsWithSupportedClass)
				{
					...
					{
						UE_LOGF(LogRemoteControl, Verbose, "Found object %ls that contains subobject %ls", *Actor->GetName(), *SubObjectSubPath);
	...
	UE_LOGF(LogRemoteControl, Verbose, "============================== Rebinding All for preset %ls ===============================", *Preset->GetName());

	for (URemoteControlLevelDependantBinding* InvalidBinding : InvalidBindings)
	{
		UE_LOGF(LogRemoteControl, Verbose, "Rebinding Invalid Binding");
		UE_LOGF(LogRemoteControl, Verbose, "\tOwnerActorName: %ls", *InvalidBinding->BindingContext.OwnerActorName.ToString());
		UE_LOGF(LogRemoteControl, Verbose, "\tOwnerActorClass: %ls", *InvalidBinding->BindingContext.OwnerActorClass.ToString());
		UE_LOGF(LogRemoteControl, Verbose, "\tSupportedClass: %ls", *InvalidBinding->BindingContext.SupportedClass.ToString());
		UE_LOGF(LogRemoteControl, Verbose, "\tComponentName: %ls", *InvalidBinding->BindingContext.ComponentName.ToString());
		UE_LOGF(LogRemoteControl, Verbose, "\tSubObjectPath: %ls", *InvalidBinding->BindingContext.SubObjectPath);
		...
		UE_LOGF(LogRemoteControl, Verbose, "===================================================================");
	}
