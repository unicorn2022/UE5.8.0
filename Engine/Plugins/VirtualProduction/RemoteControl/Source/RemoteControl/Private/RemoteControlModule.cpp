...
#if WITH_EDITOR
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FRemoteControlModule::HandleEnginePostInit);
	...
	ConsoleObjects.Add(IConsoleManager::Get().RegisterConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateLambda([this](const TArray<FString>& InArgs)
		{
			...
			if (bEnable)
			{
				...
				UE_LOGF(LogRemoteControl, Log, "Local Forwarding Interception is enabled.");
			}
			else
			{
				UnregisterLocalForwardingInterceptor();
				UE_LOGF(LogRemoteControl, Log, "Local Forwarding Interception is disabled.");
			}
		}),
		ECVF_Default
		));
...
{
	UE_LOGF(LogRemoteControl, VeryVerbose, "Invoke function");
		...
		if (InInterceptPayload.Num() != 0 && CanInterceptFunction(InCall))
		{
			...
			for (int32 InterceptorIdx = 0; InterceptorIdx < InterceptorsAmount; ++InterceptorIdx)
			{
				...
				if (Interceptor)
				{
					...
					UE_LOGF(LogRemoteControl, VeryVerbose, "Invoke function - Intercepted");
...
bool FRemoteControlModule::SetObjectProperties(const FRCObjectReference& ObjectAccess, IStructDeserializerBackend& Backend, ERCPayloadType InPayloadType, const TArray<uint8>& InPayload, ERCModifyOperation Operation, const ERCModifyOperationFlags ModifyOperationFlags)
{
	...
	UE_LOGF(LogRemoteControl, VeryVerbose, "Set Object Properties");
	...
	if (InPayload.Num() != 0 && ObjectAccess.Object.IsValid())
	{
		...
		for (int32 InterceptorIdx = 0; InterceptorIdx < InterceptorsAmount; ++InterceptorIdx)
		{
			...
			{
				// Update response flag
				UE_LOGF(LogRemoteControl, VeryVerbose, "Set Object Properties - Intercepted");
...
bool FRemoteControlModule::AppendToObjectArrayProperty(const FRCObjectReference& ObjectAccess, IStructDeserializerBackend& Backend, ERCPayloadType InPayloadType, const TArray<uint8>& InPayload)
{
	...
	UE_LOGF(LogRemoteControl, VeryVerbose, "Append to Object Array Property");
...
bool FRemoteControlModule::InsertToObjectArrayProperty(int32 Index, const FRCObjectReference& ObjectAccess, IStructDeserializerBackend& Backend, ERCPayloadType InPayloadType, const TArray<uint8>& InPayload)
{
	...
	UE_LOGF(LogRemoteControl, VeryVerbose, "Insert to Object Array Property");
...
bool FRemoteControlModule::RemoveFromObjectArrayProperty(int32 Index, const FRCObjectReference& ObjectAccess)
{
	...
	UE_LOGF(LogRemoteControl, VeryVerbose, "Remove from Object Array Property");
...
bool FRemoteControlModule::SetPresetController(const FName PresetName, class URCVirtualPropertyBase* VirtualProperty, IStructDeserializerBackend& Backend, const TArray<uint8>& InPayload, const bool bAllowIntercept)
{
	...
	if (bAllowIntercept)
	{
		...
		UE_LOGF(LogRemoteControl, VeryVerbose, "SetPresetController - Num Interceptors: %d", InterceptorsAmount);
...
void FRemoteControlModule::RegisterEntityFactory(const FName InFactoryName, const TSharedRef<IRemoteControlPropertyFactory>& InFactory)
{
	...
	{
		UE_LOGF(LogRemoteControl, Error, "Factory should have a name");
		ensure(false);
	}
}
...
void FRemoteControlModule::LogOnce(ELogVerbosity::Type InVerbosity, const FString& LogDetails, const FString& FileName, int32 LineNumber) const
{
	...
	if (!LogHistory.Contains(Hash))
	{
		...
		case ELogVerbosity::Fatal:
			UE_LOGF(LogRemoteControl, Fatal, "%ls", *LogDetails);
			break;

		case ELogVerbosity::Error:
			UE_LOGF(LogRemoteControl, Error, "%ls", *LogDetails);
			break;

		case ELogVerbosity::Warning:
			UE_LOGF(LogRemoteControl, Warning, "%ls", *LogDetails);
			break;

		case ELogVerbosity::Display:
			UE_LOGF(LogRemoteControl, Display, "%ls", *LogDetails);
			break;

		case ELogVerbosity::Log:
			UE_LOGF(LogRemoteControl, Log, "%ls", *LogDetails);
			break;

		case ELogVerbosity::Verbose:
			UE_LOGF(LogRemoteControl, Verbose, "%ls", *LogDetails);
			break;

		case ELogVerbosity::VeryVerbose:
			UE_LOGF(LogRemoteControl, VeryVerbose, "%ls", *LogDetails);
			break;
