...
FArchive& operator<<(FArchive& Ar, FRemoteControlFunction& RCFunction)
{
	...
	if (Ar.IsLoading())
	{
		...
		if (UFunction* Function = RCFunction.GetFunction())
		{
			...
			{
				UE_LOGF(LogRemoteControl, Warning, "Arguments size mismatch from size serialized in asset. Expected %lld, Actual: %lld", NumSerializedBytesFromArchive, Ar.Tell() - ArgsBegin);
				Ar.SetError();
			}
		}
		else
		{
			UE_LOGF(LogRemoteControl, Warning, "%ls could not be loaded while deserialzing a Remote Control Function.", *RCFunction.FunctionPath.ToString());
