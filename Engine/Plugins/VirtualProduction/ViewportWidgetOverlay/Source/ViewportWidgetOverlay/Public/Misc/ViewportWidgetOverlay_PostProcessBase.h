...
#include "ViewportWidgetOverlay_PostProcessBase.generated.h"
...
namespace UE::VPUtilities::Private { class FVPWidgetPostProcessHitTester; }
...
struct FViewportWidgetOverlay_PostProcessBase
{
	...
	UE_API FViewportWidgetOverlay_PostProcessBase();
	UE_API virtual ~FViewportWidgetOverlay_PostProcessBase() = default;

	/** Deinits this output type */
	UE_API virtual void Hide(UWorld* World);
	
	UE_API TSharedPtr<SVirtualWindow> GetSlateWindow() const;

protected:

	UE_API bool CreateRenderer(UWorld* World, UUserWidget* Widget, TAttribute<float> InDPIScale);
	UE_API virtual void ReleaseRenderer();
	UE_API void TickRenderer(UWorld* World, float DeltaSeconds);
