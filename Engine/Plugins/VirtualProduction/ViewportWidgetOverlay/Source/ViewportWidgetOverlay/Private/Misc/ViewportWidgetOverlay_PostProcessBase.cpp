...
#include "Misc/ViewportWidgetOverlay_PostProcessBase.h"
...
FViewportWidgetOverlay_PostProcessBase::FViewportWidgetOverlay_PostProcessBase()
...
void FViewportWidgetOverlay_PostProcessBase::Hide(UWorld* World)
{
	ReleaseRenderer();
}

TSharedPtr<SVirtualWindow> FViewportWidgetOverlay_PostProcessBase::GetSlateWindow() const
{
	return SlateWindow;
}

bool FViewportWidgetOverlay_PostProcessBase::CreateRenderer(UWorld* World, UUserWidget* Widget, TAttribute<float> InDPIScale)
{
	ReleaseRenderer();
...
void FViewportWidgetOverlay_PostProcessBase::ReleaseRenderer()
{
	if (WidgetRenderer)
	{
...
void FViewportWidgetOverlay_PostProcessBase::TickRenderer(UWorld* World, float DeltaSeconds)
{
	check(World);
...
FIntPoint FViewportWidgetOverlay_PostProcessBase::CalculateWidgetDrawSize(UWorld* World)
{
	...
	if (World->WorldType == EWorldType::Game || World->WorldType == EWorldType::PIE)
	{
		...
		UE_LOGF(LogViewportWidgetOverlay, Warning, "CalculateWidgetDrawSize failed for game world.");
	...
	UE_LOGF(LogViewportWidgetOverlay, Warning, "CalculateWidgetDrawSize failed for editor world.");
#endif
	
	return FIntPoint::ZeroValue;
}

bool FViewportWidgetOverlay_PostProcessBase::IsTextureSizeValid(FIntPoint Size) const
{
...
void FViewportWidgetOverlay_PostProcessBase::RegisterHitTesterWithViewport(UWorld* World)
{
	...
	if (EngineViewportWidget && bReceiveHardwareInput)
	{
		...
		{
			UE_LOGF(LogViewportWidgetOverlay, Warning, "Can't register a hit tester for FullScreenUserWidget. There is already one defined.");
		}
		else
		{
			...
			CustomHitTestPath = MakeShared<UE::VPUtilities::Private::FVPWidgetPostProcessHitTester>(
				World,
				SlateWindow,
				TAttribute<float>::CreateRaw(this, &FViewportWidgetOverlay_PostProcess::GetDPIScaleForPostProcessHitTester, TWeakObjectPtr<UWorld>(World))
				);
			CustomHitTestPath->SetWidgetDrawSize(CurrentWidgetDrawSize);
...
void FViewportWidgetOverlay_PostProcessBase::UnRegisterHitTesterWithViewport()
{
...
TSharedPtr<SViewport> FViewportWidgetOverlay_PostProcessBase::GetViewport(UWorld* World) const
{
	if (World->WorldType == EWorldType::Game || World->WorldType == EWorldType::PIE)
	{
...
float FViewportWidgetOverlay_PostProcessBase::GetDPIScaleForPostProcessHitTester(TWeakObjectPtr<UWorld> World) const
{
