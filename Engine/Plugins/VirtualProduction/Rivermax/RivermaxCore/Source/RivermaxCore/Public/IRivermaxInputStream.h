...
namespace UE::RivermaxCore
{
	...
	*/
	...
	/**
	* 2110-40 ANC sample type interface.
	*/
	class RIVERMAXCORE_API IRivermaxAncSample : public IRivermaxSample
	{
	public:
		virtual void SetAncData(uint16 DID, uint16 SDID, const TArray<uint16>& UDWs) = 0;
		virtual uint16 GetDID() const = 0;
		virtual uint16 GetSDID() const = 0;
		virtual const TArray<uint16>& GetUserDataWords() const = 0;
	};

	/** Descriptor filled during ANC packet reception, passed to OnAncFrameRequested. */
	struct RIVERMAXCORE_API FRivermaxInputAncFrameDescriptor
	{
		uint32 Timestamp = 0;
		uint32 FrameNumber = 0;
		uint16 DID = 0;
	...
	*/
	...
	public:
		...
		/** Called when an ANC stream is ready to receive a packet. Return a valid sample to accept it. */
		virtual TSharedPtr<IRivermaxAncSample> OnAncFrameRequested(const FRivermaxInputAncFrameDescriptor& FrameInfo)
		{
			return nullptr;
		}

		/** Called when an ANC packet has been fully received and parsed into InReceivedAncFrame. */
		virtual void OnAncFrameReceived(TSharedPtr<IRivermaxAncSample> InReceivedAncFrame)
		{
		}

		/** Called when a received ANC packet could not be parsed (checksum error, truncated packet, etc.). */
		virtual void OnAncFrameReceptionError()
		{
		}
	};
