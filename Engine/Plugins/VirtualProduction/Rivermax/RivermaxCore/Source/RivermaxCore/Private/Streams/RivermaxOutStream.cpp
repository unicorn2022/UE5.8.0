...
namespace UE::RivermaxCore::Private
{
	...
	static TAutoConsoleVariable<float> CVarRivermaxOutputRandomDelay(
		TEXT("Rivermax.Output.TriggerRandomDelayMs"), 0.0,
		TEXT("Will cause a delay of selected amount of time when next frame is sent in milliseconds."),
		ECVF_Default);

	enum class ETimingIssueMode : int32
	{
		Disabled = 0,
		Immediate = 1,
		Delay = 2,
	...
	static TAutoConsoleVariable<int32> CVarRivermaxOutputTriggerTimingIssue(
		TEXT("Rivermax.Output.TriggerTimingIssue"), 0,
		TEXT("This is a debugging Cvar to introduce artificial timing issue.\n\
		0: disabled \n\
		1: set to this value to do it once, \n\
		2: set to this value to skip one frame in rougly 10 seconds.\n\
		3: set to this value to continuosly and randomly skip 10% of the frames."),
		ECVF_Default);
	...
	static TAutoConsoleVariable<float> CVarRivermaxOutputCommitChunksOffsetPercent(
		TEXT("Rivermax.Output.CommitChunksOffsetPercent"), 0.3f,
	...
	bool FRivermaxOutStream::Initialize(const FRivermaxOutputOptions& InOptions, IRivermaxOutputStreamListener& InListener)
	{
		...
		{
			UE_LOGF(LogRivermax, Warning, "Can't create Rivermax Output Stream. Library isn't initialized.");
			return false;
		}
		...
		if (IConsoleVariable* CvarDelay = CVarRivermaxOutputTriggerTimingIssue.AsVariable())
		{
			CvarDelay->OnChangedDelegate().AddRaw(this, &FRivermaxOutStream::OnCVarTriggerTimingIssueChanged);
		}
				...
				if (Status == RMX_OK)
				{
					...
					if (Status == RMX_OK)
					{
						...
						if (Status == RMX_OK)
						{
							...
							UE_LOGF(LogRivermax, Verbose, "Created stream using SDP:\n%ls", *SDPAsAString);
						...
						{
							UE_LOGF(LogRivermax, Warning, "Failed querying destination address. Output Stream won't be created. Status: %d", Status);
						}
					}
					else
					{
						UE_LOGF(LogRivermax, Warning, "Failed querying local address. Output Stream won't be created. Status: %d", Status);
					}
				}
				else
				{
					UE_LOGF(LogRivermax, Warning, "Failed to create Rivermax output stream. Status: %d", Status);
				}
			}
	...
	void FRivermaxOutStream::Uninitialize()
	{
		if (RivermaxThread != nullptr)
		{
			...
			UE_LOGF(LogRivermax, Log, "Rivermax Output stream has shutdown");
		}
	...
	void FRivermaxOutStream::DestroyStream()
	{
		...
		auto CancelUnsentChunks = [&] {
			...
			if (Status != RMX_OK)
			{
				UE_LOGF(LogRivermax, Warning, "Could not cancel unsent chunks when destroying output stream. Status: %d", Status);
			}
		};
		...
		if (Status == RMX_BUSY)
		{
			UE_LOGF(LogRivermax, Warning, "Could not destroy the stream. StreamId: %d", StreamId);
		}
	}
	...
	bool FRivermaxOutStream::WaitForNextRound()
	{
		...
		if (!bIsFrameReady)
		{
			...
			if (StreamData.bHasValidNextFrameNumber && CachedCVars.bShowOutputStats)
			{
				...
				UE_LOGF(LogRivermax, Verbose, "CurrentTime %llu. OvershootSleep: %0.9f. ExpectedWait: %0.9f. RealWait: %0.9f, Scheduling at %llu. NextAlign %llu. "
					, CurrentTimeNanosec
					, OvershootSleepSec
		...
		else
		{
			...
			{
				const uint64 AfterSleepTimeNanosec = RivermaxModule->GetRivermaxManager()->GetTime();
				UE_LOGF(LogRivermax, Verbose, "Early data available. CurrentTime %llu. Scheduling at %llu. NextAlign %llu. "
		...
		do
		{
			...
			else if (Status == RMX_NO_FREE_CHUNK)
			{
				...
				if (!bHasAddedTrace)
				{
					...
					UE_LOGF(LogRivermax, Verbose, "No free chunks to get for chunk '%u'. Waiting", CurrentFrame->ChunkNumber);
			...
			{
				UE_LOGF(LogRivermax, Error, "Invalid error happened while trying to get next chunks. Status: %d", Status);
	...
	void FRivermaxOutStream::CommitNextChunks()
	{
		...
		do
		{
			...
			{
				UE_LOGF(LogRivermax, Error, "Completion issue while trying to commit next round of chunks.");
				Listener->OnStreamError();
				Stop();
			}
			else
			{
				UE_LOGF(LogRivermax, Error, "Unhandled error (%d) while trying to commit next round of chunks.", Status);
				Listener->OnStreamError();
				Stop();
			}
		...
		{
			UE_LOGF(LogRivermax, Verbose, "Committed frame [%llu]. Scheduled for '%llu'. Aligned with '%llu'. Current time '%llu'. Was late: %d. Slack: %llu. Errorcount: %d"
	...
	void FRivermaxOutStream::PreprocessNextFrame()
	{
		...
		{
			UE_LOGF(LogRivermax, Error, "Unexpected error, no frame was available.");
			Listener->OnStreamError();
			Stop();
		}
	}
	...
	void FRivermaxOutStream::PrepareNextFrame_AlignmentPoint()
	{
		...
		else
		{
			...
			if (!StreamMemory.bUseIntermediateBuffer)
			{
				...
				UE_LOGF(LogRivermax, Verbose, "No frame to send. Reusing last frame with Frame Counter: %llu", CurrentFrame->GetFrameCounter());
	...
	void FRivermaxOutStream::InitializeStreamTimingSettings()
	{
		...
		else
		{
			...
			auto ComputeVideoTROMultiplier = [](bool bProgressive, int32 AlignedHeight) -> double
			{
				// See https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=8165971 for reference
				// Gapped PRS doesn't support non standard resolution.
				if (bProgressive)
				{
					// As defined by SMPTE 2110-21 6.3.2
					return AlignedHeight >= FullHDHeight ? (43.0 / 1125.0) : (28.0 / 750.0);
				}
				else
				{
					// As defined by SMPTE 2110-21 6.3.3
					if (AlignedHeight >= FullHDHeight) { return (22.0 / 1125.0); }
					if (AlignedHeight >= 576)          { return (26.0 / 625.0);  }
					return (20.0 / 525.0);
				}
			};
			...
			if (StreamType == ERivermaxStreamType::ST2110_20)
			{
				...
				TransmitOffsetNanosec = (uint64)(ComputeVideoTROMultiplier(bIsProgressive, VideoOptions->AlignedResolution.Y) * FrameIntervalNs);
			}
			else if (StreamType == ERivermaxStreamType::ST2110_40 || StreamType == ERivermaxStreamType::ST2110_40_TC)
			{
				// ST2110-40 ANC TRoffset matches the companion video stream (SMPTE ST 2110-40 Sec. 7).
			...
			// TODO: Add TRoffset for ST2110_30 audio.
		}
	...
	void FRivermaxOutStream::LogStreamDescriptionOnCreation() const
	{
		...
		UE_LOGF(LogRivermax, Display, "%ls", *FString(StreamDescription));
	}
	...
	void FRivermaxOutStream::ShowStats()
	{
		if (CachedCVars.bShowOutputStats)
		{
			...
			if (CurrentTime - LastStatsShownTimestamp > CachedCVars.ShowOutputStatsIntervalSeconds)
			{
				...
				UE_LOGF(LogRivermax, Log, "Stats: FrameSent: %llu. CommitImmediate: %u. CommitRetries: %u. ChunkRetries: %u. ChunkSkippingRetries: %u. Timing issues: %llu", Stats.FramesSentCounter, Stats.CommitImmediate, Stats.CommitRetries, Stats.TotalChunkRetries, Stats.ChunkSkippingRetries, Stats.TimingIssueCount);
			}
		}
	}
	...
	void FRivermaxOutStream::CalculateNextScheduleTime_AlignementPoints(uint64 CurrentClockTimeNanosec, uint64 CurrentFrameNumber)
	{
		...
		{	
			...
			if (CurrentFrameNumber == StreamData.NextAlignmentPointFrameNumber - 1)
			{
				...
				UE_LOGF(LogRivermax, Verbose, "Scheduling last frame was faster than expected. (CurrentFrame: '%llu' LastScheduled: '%llu') Scheduling for following expected one."
					, CurrentFrameNumber
			...
			else
			{
				...
				if (CurrentFrameNumber >= StreamData.NextAlignmentPointFrameNumber)
				{
					...
					{
						NextFrameNumber = StreamData.NextAlignmentPointFrameNumber + 2;
						UE_LOG(LogRivermax, Warning, TEXT("Timing issue detected during frame %llu. Skipping frame %llu to keep sync."), StreamData.NextAlignmentPointFrameNumber, StreamData.NextAlignmentPointFrameNumber + 1);
					}
					else
					{
						...
						if (DeltaFrames >= 1)
						{
							UE_LOGF(LogRivermax, Warning, "Output missed %llu frames.", DeltaFrames);
	...
	void FRivermaxOutStream::OnCVarTriggerTimingIssueChanged(IConsoleVariable* Var)
	{
		ScheduledTimeForArtificialTimingIssue = 0.0;
		ETimingIssueMode State = static_cast<ETimingIssueMode>(Var->GetInt());
		if (State == ETimingIssueMode::Delay)
		{
			ScheduledTimeForArtificialTimingIssue = FPlatformTime::Seconds() + 10.;
		}
		else if (State == ETimingIssueMode::Immediate)
		{
			ScheduledTimeForArtificialTimingIssue = FPlatformTime::Seconds();
		}
	}
		...
		do
		{
			...
			if (Status != RMX_OK)
			{
				if (Status == RMX_NO_FREE_CHUNK)
				{
					...
					if (!bHasAddedTrace)
					{
						UE_LOGF(LogRivermax, Warning, "No chunks ready to skip. Waiting");
				...
				else
				{
					...
					UE_LOGF(LogRivermax, Error, "Invalid error happened while trying to skip chunks. Status: %d.", Status);
					Listener->OnStreamError();
					Stop();
				}
			}
	...
	void FRivermaxOutStream::SendFrame()
	{
		...
		if (bTriggerRandomDelay)
		{
			...
			FPlatformProcess::SleepNoStats(CVarRivermaxOutputRandomDelay.GetValueOnRenderThread()/1000.);
		}
		...
		UE_LOGF(LogRivermax, VeryVerbose, "RmaxRX Sending frame number %u with timestamp %u.", MediaFrameNumber, CurrentFrame->MediaTimestamp);

		// Process completions of the previous frame's chunks.
		Stats.ChunkCompletionTracker->PollAndReportCompletionOfTrackedChunks();

		ETimingIssueMode TriggerTimingIssue = static_cast<ETimingIssueMode>(CVarRivermaxOutputTriggerTimingIssue.GetValueOnAnyThread());
		...
		do
		{
			...
			if (bIsActive)
			{
				if (!CurrentFrame->bCaughtTimingIssue)
				{
					...
					if ((TriggerTimingIssue == ETimingIssueMode::Immediate ||
						TriggerTimingIssue == ETimingIssueMode::Delay)
						&& (FPlatformTime::Seconds() > ScheduledTimeForArtificialTimingIssue && ScheduledTimeForArtificialTimingIssue > 0.0))
					{
						CurrentFrame->bCaughtTimingIssue = true;
						ScheduledTimeForArtificialTimingIssue = 0.0;
					}
					else if (TriggerTimingIssue == ETimingIssueMode::RandomContinuous && FMath::FRandRange(0., 1.) < .1)
					{
						CurrentFrame->bCaughtTimingIssue = true;
					}
				}
	...
	void FRivermaxChunkCompletionTracker::PollAndReportCompletionOfTrackedChunks()
	{
		...
		for (const FTrackedChunkInfo& TrackedChunk : TrackedChunks)
		{
			...
			if (!ChunkCompletion)
			{
				UE_LOGF(LogRivermax, Error, "ChunkCompletion is invalid. Rivermax stream is either shut down or chunk handle is invalid.");
				continue;
			}
			...
			UE_LOGF(LogRivermax, VeryVerbose, "Chunk \"%ls\" was completed with timestamp: %llu nanoseconds, which is %lld microseconds away from the expected alignment point.\n"
				"The chunk is %lld microseconds away from the predicted chunk completion time."
				, *HumanReadableString, TimeStamp, TimeDeltaAlignmentPoint, TimeDeltaExpectedChunkCompletionTime);

			if (TimeDeltaAlignmentPoint < 0)
			{
				UE_LOGF(LogRivermax, Error, "Chunk \"%ls\" was completed %lld microseconds before the first packet was expected to be on the wire. ", *HumanReadableString, FMath::Abs(TimeDeltaAlignmentPoint));
			}
			...
			{
				UE_LOGF(LogRivermax, Verbose, "Chunk \"%ls\" was completed %lld microseconds after it was expected to be completed. ", *HumanReadableString, FMath::Abs(TimeDeltaExpectedChunkCompletionTime));
			}
