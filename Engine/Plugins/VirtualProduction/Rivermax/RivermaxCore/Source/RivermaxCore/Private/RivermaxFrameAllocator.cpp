...
namespace UE::RivermaxCore::Private
{
	...
	bool FGPUAllocator::Allocate(int32 FrameCount, bool bAlignFrameMemory)
	{
		...
		{
			UE_LOGF(LogRivermax, Warning, "Can't allocate GPUMemory. Failed to get a Cuda device for GPU %d. Status: %d", GPUIndex, Status);
			return false;
		}
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't allocate GPUMemory. Failed to get allocation granularity. Status: %d", Status);
			return false;
		}
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't allocate GPUMemory. Failed to reserve memory for %zu bytes. Status: %d", TotalCudaAllocSize, Status);
			return false;
		}
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't allocate GPUMemory. Failed to create memory on device. Status: %d", Status);
			return false;
		}
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't allocate GPUMemory. Failed to map memory. Status: %d", Status);
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't allocate GPUMemory. Failed to release handle. Status: %d", Status);
			return false;
		}
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't initialize output to use GPUDirect. Failed to configure memory access. Status: %d", Status);
			return false;
		}
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't initialize output to use GPUDirect. Failed to create its stream. Status: %d", Status);
			return false;
		}
		...
		if (Status != CUDA_SUCCESS)
		{
			UE_LOGF(LogRivermax, Warning, "Can't initialize output to use GPUDirect. Failed to synchronize context. Status: %d", Status);
			return false;
		}
	...
	void FGPUAllocator::Deallocate()
	{
		if (CudaAllocatedMemory > 0)
		{
			...
			if (Status != CUDA_SUCCESS)
			{
				UE_LOGF(LogRivermax, Warning, "Failed to unmap cuda memory. Status: %d", Status);
			}

			Status = CudaModule.DriverAPI()->cuMemAddressFree(BaseAddress, CudaAllocatedMemory);
			if (Status != CUDA_SUCCESS)
			{
				UE_LOGF(LogRivermax, Warning, "Failed to free cuda memory. Status: %d", Status);
			}
	...
	bool FGPUAllocator::CopyData(const FCopyArgs& Args)
	{
		...
		TRACE_CPUPROFILER_EVENT_SCOPE(Rmax::CudaCopyStart);
		{
			...
			if (SourceMemory == nullptr)
			{
				...
				if (SourceMemory == nullptr)
				{
					UE_LOGF(LogRivermax, Error, "Failed to find a mapped memory address for captured buffer. Stopping capture.");
					return false;
				}
			}
			...
			{
				UE_LOGF(LogRivermax, Error, "Trying to copy data outside of allocated buffer.");
				return false;
			}

			{
				...
				{
					UE_LOGF(LogRivermax, Error, "Failed to copy captured buffer to cuda memory. Stopping capture. Error: %d", Result);
					return false;
				}
			}
