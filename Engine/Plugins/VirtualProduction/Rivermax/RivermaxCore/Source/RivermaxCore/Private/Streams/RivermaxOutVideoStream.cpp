	...
	)
	{
		...
		*VideoHeader = {};
...
#endif
		{
			...
			ensureMsgf(
				((uint32)SRD1Length + (uint32)SRD2Length) == CurrentPayloadSize,
				TEXT("SRD lengths must sum to payload size. SRD1=%u SRD2=%u Payload=%u Line=%u Offset=%u"), SRD1Length, SRD2Length, CurrentPayloadSize, OutLineNumber, OutSRDOffset
			);

			ensureMsgf(
				(SRD1Length % InFormatInfo.PixelGroupSize) == 0 && (SRD2Length % InFormatInfo.PixelGroupSize) == 0,
				TEXT("SRD lengths must be pixelgroup aligned. SRD1=%u SRD2=%u PixelGroupSize=%u"), SRD1Length, SRD2Length, InFormatInfo.PixelGroupSize
			);

			// If SRD2 is needed on the last line, something upstream is wrong (payload sizing/header sizing mismatch).
			ensureMsgf(
				!(SRD2Length && OutLineNumber == ((uint32)AlignedResolution.Y - 1)),
				TEXT("Invalid packetization. Payload crosses past last line. SRD2=%u Line=%u Res=%dx%d Offset=%u Payload=%u"),
				SRD2Length,
				OutLineNumber,
				AlignedResolution.X, AlignedResolution.Y,
				OutSRDOffset,
				CurrentPayloadSize
			);
			...
			VideoHeader->SRD1.FieldIdentification = FieldIdentification;
#endif
			...
			if (SRD2Length > 0)
			{
				...
				VideoHeader->SRD2.FieldIdentification = FieldIdentification;
#endif
			...
			// Never happened. Sanity check.
			ensureMsgf(
				OutLineNumber <= (uint32)AlignedResolution.Y,
				TEXT("LineNumber overflow. Line=%u ResY=%d"), OutLineNumber, AlignedResolution.Y
			);
		}
...
// Unit test to run when switching platform to make sure that the bit fields are interpreted correctly.
	...
	bool FRivermaxOutputStreamVideo::InitializeStreamMemoryConfig()
	{
		...
		{
			UE_LOGF(LogRivermax, Warning, "Can't create Rivermax Output Stream. Aligned horizontal resolution of %d doesn't align with pixel group coverage of %d.", StreamOptions->AlignedResolution.X, FormatInfo.PixelGroupCoverage);
			return false;
		}
		...
		if (FrameSize == 0)
		{
			UE_LOGF(LogRivermax, Warning, "Can't create Rivermax Output Stream. Frame size of 0 is invalid. Verify resolution.");
			return false;
		}
		...
		if (bFoundPayload == false)
		{
			...
			{
				UE_LOGF(LogRivermax, Log, "Due to resolution %dx%d, row data will be sent over multiple packets with varied sizes.", StreamOptions->AlignedResolution.X, StreamOptions->AlignedResolution.Y);
			...
			{
				UE_LOGF(LogRivermax, Warning, "Could not find payload size for desired resolution %dx%d for desired pixel format."
		...
		// Sanity check. By this point, frame size must be pixel group aligned.
		ensureMsgf(
			(FrameSizeInBytes % FormatInfo.PixelGroupSize) == 0,
			TEXT("FrameSizeInBytes (%llu) must be pixel-group aligned (PixelGroupSize=%u). Res=%dx%d Sampling=%s"),
			FrameSizeInBytes,
			FormatInfo.PixelGroupSize,
			StreamOptions->AlignedResolution.X,
			StreamOptions->AlignedResolution.Y,
			LexToString(FormatInfo.Sampling)
		);
		...
		for (uint32 BlockIndex = 0; BlockIndex < StreamMemory.MemoryBlockCount; ++BlockIndex)
		{
			...
			{
				UE_LOGF(LogRivermax, Warning, "Faild to get payload memory block. Output stream won't be created.");
				return false;
			}
			...
			{
				UE_LOGF(LogRivermax, Warning, "Failed to get header memory block. Output stream won't be created.");
				return false;
			}
		...
		uint32 LineSize = 0; // bytes consumed in current line (0..BytesPerRow)

		const uint32 BytesPerRowU32 = (uint32)BytesPerRow;

		for (uint32 PacketIndex = 0; PacketIndex < RealPacketsPerFrame; ++PacketIndex)
		{
			uint32 HeaderSize = 0;
			uint32 ThisPayloadSize = 0;

			if (ProcessedBytes < FrameSizeInBytes)
			{
				// 1. Decide actual payload size for this packet (full or last remainder)
				ThisPayloadSize = StreamMemory.PayloadSize;
				if ((ProcessedBytes + ThisPayloadSize) > FrameSizeInBytes)
				{
					ThisPayloadSize = (uint32)(FrameSizeInBytes - ProcessedBytes);
				}

				// Payload must always align to pixel groups and be a multiple of header defined payload size. 
				ensureMsgf(
					(ThisPayloadSize % FormatInfo.PixelGroupSize) == 0,
					TEXT("Payload size (%u) must be pixel-group aligned (PixelGroupSize=%u). Packet=%u ProcessedBytes=%llu FrameSizeInBytes=%llu"),
					ThisPayloadSize,
					FormatInfo.PixelGroupSize,
					PacketIndex,
					ProcessedBytes,
				...
				// 2. Decide SRD count using THIS payload size
				const uint32 RemainingInLine = BytesPerRowU32 - LineSize;

				if (ThisPayloadSize <= RemainingInLine)
				{
					HeaderSize = FVideoRTPHeader::OneSRDSize;

					LineSize += ThisPayloadSize;
					if (LineSize == BytesPerRowU32)
					{
						LineSize = 0;
					}
				}
				else
				{
					...
					const uint32 Spill = ThisPayloadSize - RemainingInLine;
					LineSize = Spill;

					// Would require more than two SRDs, which is unsupported
					if (LineSize > BytesPerRowU32)
					{
						UE_LOGF(LogRivermax, Warning,
							"Unsupported resolution/payload: packet crosses more than 2 lines (needs >2 SRD). Res=%dx%d Spill=%u BytesPerRow=%u Payload=%u",
							StreamOptions->AlignedResolution.X, StreamOptions->AlignedResolution.Y, LineSize, BytesPerRowU32, ThisPayloadSize);
						return false;
					}
				}
			}
			...
			{
				// Extra packet slots for chunk alignment.
				HeaderSize = 0;
				ThisPayloadSize = 0;
			}

			// All buffers configured the same
			for (uint32 BufferIndex = 0; BufferIndex < StreamMemory.FramesFieldPerMemoryBlock; ++BufferIndex)
			{
				StreamMemory.HeaderSizes[PacketIndex + (BufferIndex * RealPacketsPerFrame)] = HeaderSize;
				StreamMemory.PayloadSizes[PacketIndex + (BufferIndex * RealPacketsPerFrame)] = ThisPayloadSize;
			}

			if (CachedCVars.bPrefillRTPHeaders && HeaderSize > 0)
			{
				RTPFiller.Update(PacketIndex);
			}
	...
	bool FRivermaxOutputStreamVideo::CopyFrameData(const TSharedPtr<FRivermaxOutputFrame>& SourceFrame, uint8* DestinationBase)
	{
		...
		{
			UE_LOGF(LogRivermax, Error, "Invalid frame size detected while stream was active. Shutting down.");
	...
	bool FRivermaxOutputStreamVideo::SetupFrameManagement()
	{
		...
		if (bTryGPUDirect)
		{
			...
			{
				UE_LOGF(LogRivermax, Warning, "Can't initialize output to use GPUDirect. RHI is %d but only Dx12 is supported at the moment.", EnumToUnderlyingType(RHIType));
				bTryGPUDirect = false;
			}
		}
	...
	void FRivermaxOutputStreamVideo::SetupRTPHeadersForChunk()
	{
		...
		//UE_LOGF(LogRivermax, Log, "Packets Processed: %d, Packets per chunk: %d, ChunkNum: %d", PacketIndex, StreamMemory.PacketsPerChunk, CurrentFrame->ChunkNumber);
