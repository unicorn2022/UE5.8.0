...
#pragma pack(pop)
	...
	*/
	...
	/**
	 * Counterpart to FBigEndianHeaderPacker. Reads fields from a big-endian Most significant byte first packed
	 */
	class FBigEndianHeaderUnpacker
	{
	public:
		FBigEndianHeaderUnpacker(const uint8* InBuffer, int32 InMaxSize)
			: Data(InBuffer, InMaxSize)
		{
			check(InMaxSize > 0);
		}

		FORCEINLINE uint32 GetNextField(int32 InNumBits)
		{
			check(InNumBits > 0 && InNumBits <= 32);
			uint32 Result = 0;
			const int32 EndBit = CurrentBit + InNumBits;
			for (int32 BitIdx = CurrentBit; BitIdx < EndBit; ++BitIdx)
			{
				const int32 ByteIdx   = BitIdx / 8;
				const int32 BitInByte = 7 - (BitIdx % 8); // MSB-first within each byte
				Result = (Result << 1) | ((Data[ByteIdx] >> BitInByte) & 1u);
			}
			CurrentBit = EndBit;
			return Result;
		}

		/** Skip InNumBits without reading. */
		FORCEINLINE void SkipBits(int32 InNumBits)
		{
			check(InNumBits >= 0);
			CurrentBit += InNumBits;
		}

		/**
		 * Advance to the next InWordSizeByte boundary.
		 */
		FORCEINLINE void WordAlign(int32 InWordSizeByte)
		{
			check(InWordSizeByte > 0);
			const int32 WordBits     = InWordSizeByte * 8;
			const int32 Misalignment = CurrentBit % WordBits;
			if (Misalignment != 0)
			{
				CurrentBit += WordBits - Misalignment;
			}
		}

		/** Returns true if at least InNumBits remain in the buffer. */
		FORCEINLINE bool CanRead(int32 InNumBits) const
		{
			return CurrentBit + InNumBits <= Data.Num() * 8;
		}

		/** Current bit cursor position. */
		FORCEINLINE int32 GetBitPosition() const { return CurrentBit; }
