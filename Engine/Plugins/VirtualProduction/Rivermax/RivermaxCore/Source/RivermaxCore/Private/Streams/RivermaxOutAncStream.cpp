	...
	*/
		...
		, TSharedPtr<UE::RivermaxCore::Private::FRivermaxOutputFrame> CurrentFrame)
	{
		...
		for (size_t PayloadIndex = 0; PayloadIndex < InStreamMemory.PacketsPerChunk; ++PayloadIndex) 
		{
			...
			const uint16 Checksum = UE::RivermaxCore::Private::Utils::ComputeAncChecksum(
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FRivermaxOutputStreamAnc::CopyFrameData);

		const uint32 MediaFrameNumber = Utils::TimestampToFrameNumber(CurrentFrame->MediaTimestamp, GetFrameRate());
		TRACE_CPUPROFILER_EVENT_SCOPE_TEXT(*FRivermaxTracingUtils::RmaxOutAncSendingFrameTraceEvents[MediaFrameNumber % 10]);
