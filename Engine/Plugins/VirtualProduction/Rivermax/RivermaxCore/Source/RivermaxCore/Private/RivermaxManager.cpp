...
namespace UE::RivermaxCore
{
	...
	TMap<uint8, FString> FRivermaxTracingUtils::RmaxOutAncSendingFrameTraceEvents;
	...
	// There is a known issue in 1.71.1 Rivermax version on linux, which is the reason for the split.
#if PLATFORM_LINUX
	
	static const FString RivermaxLibraryEnvironmentVariable = TEXT("RIVERMAX_PATH_1_80_24");
	static const uint32 RivermaxVersion[3] = { 1, 80, 24 };
#else
...
namespace UE::RivermaxCore::Private
{
	...
	static TAutoConsoleVariable<int32> CVarRivermaxCoreAffinity(
		TEXT("Rivermax.CPUAffinity"), -1,
		TEXT("CPU Core to pin Rivermax output/input thread to. Negative means disabled. Core index is 0..X.\n\
			The best choice for the cpu core is one that is on the same NUMA node as the DPU/NIC used for Rivermax traffic."),
		ECVF_Default);


	static uint64_t RivermaxTimeHandler(void* Context)
	{
	...
	FRivermaxManager::~FRivermaxManager()
	{
		...
		if (bIsCleanupRequired)
		{
			...
			if (Status != RMX_OK)
			{
				UE_LOGF(LogRivermax, Warning, "Failed to cleanup Rivermax Library. Status: %d", Status);
			}
			UE_LOGF(LogRivermax, Log, "Rivermax Library has shutdown.");
		}
	...
	bool FRivermaxManager::LoadRivermaxLibrary()
	{
		...
		FString LibraryPath = TEXT(UE_STRINGIZE(RIVERMAX_LIBRARY_PLATFORM_PATH));
		...
		{
#if PLATFORM_WINDOWS
			// On Windows, fall back to locating the library via the Mellanox WinOF2 registry key.
...
#elif PLATFORM_LINUX
			// On Linux, the compile-time default path (RIVERMAX_LIBRARY_PLATFORM_PATH =
			// /usr/lib/x86_64-linux-gnu) set above is already correct. The Rivermax SDK
			// installs to a fixed system path; no additional lookup is needed.
#endif // PLATFORM_WINDOWS
		}

		const FString LibraryName = TEXT(UE_STRINGIZE(RIVERMAX_LIBRARY_NAME));
...
#if PLATFORM_LINUX
		// If the configured path doesn't contain the library (e.g. on RPM-based distros
		// where Rivermax installs to /usr/lib64 instead of /usr/lib/x86_64-linux-gnu),
		// fall back to the system library search path (LD_LIBRARY_PATH, ldconfig).
		if (LibraryHandle == nullptr)
		{
			LibraryHandle = FPlatformProcess::GetDllHandle(*LibraryName);
		}
		...
		if(!bIsLibraryValid)
		{
			UE_LOGF(LogRivermax, Log, "Failed to load Rivermax library.");
		}
		else
		{
			UE_LOGF(LogRivermax, Display, "Successfully loaded Rivermax library from '%ls'", *LibraryPath);
		}
	...
	uint64 FRivermaxManager::GetTime() const
	{
		...
		switch (GetTimeSource())
		{
			case ERivermaxTimeSource::PTP:
			{
				...
				{
					UE_LOGF(LogRivermax, Warning, "PTP time is the time source but was unavailable");
				}
				break;
			}
	...
	bool FRivermaxManager::LoadRivermaxFunctions()
	{
		...
		if (!bIsLibraryLoaded)
		{
			UE_LOGF(LogRivermax, Log, "Failed to load Rivermax library functions.");
		}
	...
	void FRivermaxManager::SetCPUAffinity(int32 CpuCore)
	{
		if (CpuCore < 0)
		{
			UE_LOGF(LogRivermax, Log, "Rivermax CPU affinity not set (CpuCore=%d)", CpuCore);
			return;
		}
		...
		{
			UE_LOGF(LogRivermax, Warning, "Rivermax CPU affinity not set: could not query core count (CpuCore=%d)", CpuCore);
			return;
		}

		if (CpuCore >= NumLogicalCores)
		{
			UE_LOGF(LogRivermax, Warning, "Rivermax CPU affinity not set: CpuCore=%d out of range [0..%d]", CpuCore, NumLogicalCores - 1);
			return;
		}

		constexpr int32 BitsPerWord = 64;
		const int32 NumBits = CpuCore + 1;
		const int32 NumWords = (NumBits + BitsPerWord - 1) / BitsPerWord;

		TArray<uint64_t> CpuMask;
		CpuMask.Init(0ull, NumWords);

		const int32 WordIndex = CpuCore / BitsPerWord;
		const int32 BitOffset = CpuCore % BitsPerWord;
		CpuMask[WordIndex] |= (1ULL << BitOffset);

		const rmx_status Status = GetApi()->rmx_set_cpu_affinity(CpuMask.GetData(), static_cast<size_t>(NumBits));

		if (Status != RMX_OK)
		{
			UE_LOGF(LogRivermax, Error, "Failed to set affinity to core %d (status=%d)", CpuCore, (int32)Status);
		}
		else
		{
			UE_LOGF(LogRivermax, Log, "Set CPU affinity to core %d", CpuCore);
		}
	...
	void FRivermaxManager::InitializeLibrary()
	{
		...
		if (bCanProceed)
		{
			...
			const int32 Core = CVarRivermaxCoreAffinity.GetValueOnGameThread();
			SetCPUAffinity(Core);
			...
			if (Status == RMX_OK)
			{
				...
				if (bIsClockConfigured)
				{
					...
					if (CurrentVersion)
					{
						...
						{
							UE_LOGF(LogRivermax, Warning, "Rivermax library version %d.%d.%d is not officially supported by this version of the engine.\n"
															"Please install version: %d.%d.%d or set the environment variable %ls to point to the lib folder of the Rivermax library.",
								CurrentVersion->major, CurrentVersion->minor, CurrentVersion->patch, 
								RivermaxVersion[0], RivermaxVersion[1], RivermaxVersion[2], 
								*RivermaxLibraryEnvironmentVariable);
							UE_LOGF(LogRivermax, Warning, "The engine will proceed with the current library version %ls However, there might be unexpected issues.", GPUDirectSupport);
						}
						else
						{
							UE_LOGF(LogRivermax, Log, "Rivermax library version %d.%d.%d successfully initialized, %ls", CurrentVersion->major, CurrentVersion->minor, CurrentVersion->patch, GPUDirectSupport);
						}
					}
					else
					{
						UE_LOGF(LogRivermax, Log, "Failed to retrieve Rivermax library version. Status: %d", Status);
					}
				}
			}
			else if (Status == RMX_LICENSE_ISSUE)
			{
				UE_LOGF(LogRivermax, Warning, "Rivermax License could not be found. Have you configured RIVERMAX_LICENSE_PATH environment variable?");
			}
			else if (Status > RMX_INVALID_PARAM_MIX && Status <= RMX_INVALID_PARAM_10)
			{
				UE_LOGF(LogRivermax, Warning, "Rivermax initialization error, installed Rivermax SDK must be version %d.%d.%d.", RMX_VERSION_MAJOR, RMX_VERSION_MINOR, RMX_VERSION_PATCH);
			}

			if (bIsLibraryInitialized == false)
			{
				UE_LOGF(LogRivermax, Error, "Rivermax library failed to initialize. Status: %d", Status);
			}
		}
		else
		{
			UE_LOGF(LogRivermax, Log, "Skipping Rivermax initialization. Library won't be usable.");
		}
	...
	void FRivermaxManager::VerifyGPUDirectCapability()
	{
		...
		{
			UE_LOGF(LogRivermax, Log, "GPUDirect won't be available for Rivermax as it's only available with D3D12 RHI.");
			return;
		}
		...
		{	
			...
			if (Context == nullptr)
			{
				UE_LOGF(LogRivermax, Log, "GPUDirect won't be available for Rivermax since Cuda context can't be fetched.");
				return;
			}
			...
			if (DeviceCount <= 0)
			{
				UE_LOGF(LogRivermax, Log, "No Cuda compatible device found. GPUDirect won't be available for Rivermax. Cuda status: %d", Result);
				return;
			}
			...
			{
				UE_LOGF(LogRivermax, Log, "Could not get a valid Cuda device. GPUDirect won't be available for Rivermax. Cuda status: %d", Result);
				return;
			}
			...
			if (SupportsVMM != 1)
			{
				UE_LOGF(LogRivermax, Log, "Cuda device doesn't support virtual memory management. GPUDirect won't be available for Rivermax. Cuda status: %d", Result);
				return;
			}
			...
			if (SupportsRDMA != 1)
			{
				UE_LOGF(LogRivermax, Log, "Cuda device doesn't support RDMA. GPUDirect won't be available for Rivermax. Cuda status: %d", Result);
				return;
			}
...
#else
		UE_LOG(LogRivermax, Log, TEXT("GPUDirect is not supported on this platform."));
#endif // PLATFORM_WINDOWS
	}
		...
		switch (DesiredTimeSource)
		{
			case ERivermaxTimeSource::PTP:
			{
				...
				if (DeviceFinder->ResolveIP(Settings->PTPInterfaceAddress, PTPAddress))
				{
					...
					{
						UE_LOGF(LogRivermax, Display, "Configure Rivermax clock for PTP using device interface %ls", *PTPAddress);
						...
						if (Status == RMX_OK)
						{
							...
							{
								UE_LOGF(LogRivermax, Warning, "PTP clock not supported on device interface %ls. Falling back to system clock.", *PTPAddress);
							}
							else
							{
								UE_LOGF(LogRivermax, Warning, "Failed to configure PTP clock on device interface %ls with error '%d'. Falling back to system clock.", *PTPAddress, Status);
							}
						}
						else
						{
							UE_LOGF(LogRivermax, Warning, "Failed to get device with IP %ls during PTP clock configuration, Status: '%d'. Falling back to system clock.", *PTPAddress, Status);
						}
				...
				UE_LOGF(LogRivermax, Warning, "Failed to configure PTP clock. Device interface %ls was invalid. Falling back to system clock.", *PTPAddress);
				return false;
			}
			case ERivermaxTimeSource::Engine:
			{
				...
				UE_LOGF(LogRivermax, Display, "Configure Rivermax clock for engine time");
				const rmx_status Status = FuncList.rmx_use_user_clock(&UserClockParameters);
				if (Status != RMX_OK)
				{
					UE_LOGF(LogRivermax, Warning, "Failed to configure clock to use engine time with error '%d'.", Status);
				}
			...
			case ERivermaxTimeSource::System:
			{
				...
				UE_LOGF(LogRivermax, Display, "Configure Rivermax clock for system time");
				return true;
			}
			default:
			{
...
#endif
			UE_LOGF(LogRivermax, Warning, "%ls", *ErrorText.ToString());
		}
	...
	bool FRivermaxManager::VerifyPrerequesites()
	{
		...
		if (bFoundKey)
		{
			...
			{
				bSuccess = true;
				UE_LOGF(LogRivermax, Verbose, "Rivermax setup registry key was found and enabled.");
			}
			else
			{
				UE_LOGF(LogRivermax, Warning, "Rivermax setup registry key was found but not enabled.");
			}
		}
		else
		{
			UE_LOGF(LogRivermax, Warning, "Could not find RivermaxSetup registry key. Verify installation and reboot your machine.");
		}
#elif PLATFORM_LINUX
		// On Linux, verify the mlx5 kernel module is loaded as a proxy for driver installation.
		// Full adapter mode configuration (mlxconfig INTERNAL_CPU_MODEL) is the user's responsibility
		// and cannot be checked programmatically without external tools.
		if (access("/sys/module/mlx5_core", F_OK) == 0)
		{
			bSuccess = true;
		}
		else
		{
			UE_LOG(LogRivermax, Warning, TEXT("mlx5_core kernel module is not loaded. Verify Mellanox/NVIDIA OFED driver installation."));
		}
#endif // PLATFORM_WINDOWS
		...
		if (bSuccess)
		{
			...
			if (!bSuccess)
			{
				UE_LOGF(LogRivermax, Warning, "Could not find any compatible Rivermax network adapters.");
			}
		}
	...
	bool FRivermaxManager::EnableDynamicHeaderSupport(const FString& Interface)
	{
		...
		{
			UE_LOGF(LogRivermax, Warning, "Failed to use inet_pton for interface '%ls'", *Interface);
			return false;
		}
		...
		if (Status != RMX_OK)
		{
			UE_LOGF(LogRivermax, Warning, "Failed to get device for interface '%ls'", *Interface);
			return false;
		}
		...
		if (Status != RMX_OK) 
		{
			UE_LOGF(LogRivermax, Warning, "Failed to query capabilities for device IP '%ls'", *Interface);
			return false;
		}
		...
		if (Status != RMX_OK)
		{
			UE_LOGF(LogRivermax, Warning, "Could not enable dynamic header split for device IP '%ls'. Error: %d", *Interface, Status);
			return false;
		}

		UE_LOGF(LogRivermax, Log, "Dynamic header split for device IP '%ls' has been enabled.", *Interface);
	...
	void FRivermaxManager::DisableDynamicHeaderSupport(const FString& Interface)
	{
		if (uint32* Users = DynamicHeaderUsers.Find(Interface))
		{
			...
			if (bCanUnsetDevice)
			{
				...
				if (Status != RMX_OK)
				{
					UE_LOGF(LogRivermax, Warning, "Could not disable dynamic header split for device IP '%ls'. Error: %d", *Interface, Status);
				}
				else
				{
					UE_LOGF(LogRivermax, Log, "Dynamic header split for device IP '%ls' has been disabled.", *Interface);
				}
			}
	...
	void FRivermaxManager::InitializeTraceMarkupStrings()
	{
		...
		for (uint8 Index = 0; Index < MarkupCount; ++Index)
		{
			...
			FRivermaxTracingUtils::RmaxOutAncSendingFrameTraceEvents.Add(Index, FString::Printf(TEXT("RmaxOut ANC Sending: %u"), Index));
		}
	}
