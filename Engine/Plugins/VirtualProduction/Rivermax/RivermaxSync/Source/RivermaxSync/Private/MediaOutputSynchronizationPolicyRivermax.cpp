...
{
	{
		...
		if (!IsRunning())
		{
			UE_LOGF(LogRivermaxSync, Warning, "'%ls': Synchronization is off", *GetMediaDeviceId());
			return;
		}
		...
		if (!BarrierClient)
		{
			UE_LOGF(LogRivermaxSync, Warning, "'%ls': Barrier client is nullptr", *GetMediaDeviceId());
			return;
		}

		UE_LOGF(LogRivermaxSync, Verbose, "'%ls': Synchronizing caller '%ls' at the barrier '%ls'", *GetMediaDeviceId(), *GetThreadMarker(), *GetBarrierId());
		...
		{
			UE_LOGF(LogRivermaxSync, Warning, "'%ls': Rivermax Capture isn't valid or not capturing", *GetMediaDeviceId());
			return;
		}
		...
		// Verify if we are safe to go inside the barrier.
		{
			...
			if (TimeLeftSeconds < MarginSeconds)
			{
				...
				UE_LOGF(LogRivermaxSync, VeryVerbose, "'%ls': TimeLeft(%lf) < Margin(%lf) --> Sleeping for %lf...",
		...
		UE_LOGF(LogRivermaxSync, VeryVerbose, "'%ls' Entering with %u", *GetMediaDeviceId(), BarrierDataStruct.LastRenderedFrameNumber[0]);
...
bool FMediaOutputSynchronizationPolicyRivermaxHandler::InitializeBarrier(const FString& SyncInstanceId)
{
	...
	{
		UE_LOGF(LogRivermaxSync, Warning, "Couldn't initialize barrier for '%ls'", *GetMediaDeviceId());
		return false;
	}
	...
	if (!BarrierClient)
	{
		UE_LOGF(LogRivermaxSync, Warning, "Couldn't access a barrier client for '%ls'", *GetMediaDeviceId());
		return false;
	}
	...
	if (!Delegate)
	{
		UE_LOGF(LogRivermaxSync, Warning, "'%ls': Couldn't access a barrier delegate for barrier '%ls'", *GetMediaDeviceId(), *GetBarrierId());
		return false;
	}
...
void FMediaOutputSynchronizationPolicyRivermaxHandler::HandleBarrierSync(FGenericBarrierSynchronizationDelegateData& BarrierSyncData)
{
	...
	if (BarrierSyncData.RequestData.Num() <= 0)
	{
		UE_LOGF(LogRivermaxSync, Verbose, "'%ls': No data was provided by nodes for sync barrier.", *GetMediaDeviceId())
		return;
	}
	...
	{
		UE_LOGF(LogRivermaxSync, Warning, "Could not find a base node for ptp mismatch comparisons");
		return;
	}
	...
	const bool bShouldReportPtpMismatches = (PtpUnsyncFramesPerReport > 0) && !(PtpBaseNodeData->LastRenderedFrameNumber[0] % PtpUnsyncFramesPerReport);
	...
	for (const TPair<FString, TArray<uint8>>& FramePresentedInfo : BarrierSyncData.RequestData)
	{
		...
		if (PtpBaseNodeData->HasConfirmedDesync(*NodeData, VsyncDelta))
		{
			...
			UE_LOGF(LogRivermaxSync, Warning,
				"Desync detected: Node '%ls' presented frames (%ls) at boundaries (%ls), but node '%ls' presented frames (%ls) at boundaries (%ls)",
				*PtpBaseNodeId,
		...
		{
			UE_LOGF(LogRivermaxSync, Warning,
				"Frames not presented at the same PTP frame boundary: Node '%ls' presented frames (%ls) at boundaries (%ls), but node '%ls' presented frames (%ls) at boundaries (%ls)",
				*PtpBaseNodeId,
				*PtpBaseNodeData->LastRenderedFrameNumbersAsString(),
				*PtpBaseNodeData->PresentedFrameBoundaryNumbersAsString(),
				**NodePresentedFrame,
	...
	else
	{
		...
		UE_LOGF(LogRivermaxSync, VeryVerbose, "'%ls': Cluster likely synchronized (no confirmed desync). ptp base node '%ls' presented frame %u at frame boundary %llu", 
			*GetMediaDeviceId(),
			*PtpBaseNodeId,
