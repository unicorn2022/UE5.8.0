...
void FRivermaxMediaInitializerFeature::InitializeMediaObjectForTile(UObject* MediaObject, const FMediaObjectOwnerInfo& OnwerInfo, const FIntPoint& TilePos)
{
	if (URivermaxMediaSource* RivermaxMediaSource = Cast<URivermaxMediaSource>(MediaObject))
	{
		...
		RivermaxMediaSource->VideoStream.bOverrideResolution = false;
		RivermaxMediaSource->VideoStream.FrameRate       = { 60, 1 };
		RivermaxMediaSource->VideoStream.PixelFormat     = ERivermaxPixelFormat::RGB_10bit;
		RivermaxMediaSource->VideoStream.InterfaceAddress = GetRivermaxInterfaceAddress();
		RivermaxMediaSource->VideoStream.StreamAddress   = GenerateStreamAddress(OnwerInfo.OwnerUniqueIdx, TilePos);
		RivermaxMediaSource->VideoStream.Port            = 50000;
		RivermaxMediaSource->VideoStream.bUseGPUDirect   = true;
	}
	else if (URivermaxMediaOutput* RivermaxMediaOutput = Cast<URivermaxMediaOutput>(MediaObject))
	{
		...
		RivermaxMediaOutput->NumberOfTextureBuffers = 2;
		...
		RivermaxMediaOutput->VideoStream.PixelFormat         = ERivermaxPixelFormat::RGB_10bit;
...
void FRivermaxMediaInitializerFeature::InitializeMediaObjectForFullFrame(UObject* MediaObject, const FMediaObjectOwnerInfo& OnwerInfo)
{
	if (URivermaxMediaSource* RivermaxMediaSource = Cast<URivermaxMediaSource>(MediaObject))
	{
		...
		RivermaxMediaSource->VideoStream.PixelFormat     = ERivermaxPixelFormat::RGB_10bit;
		RivermaxMediaSource->VideoStream.InterfaceAddress = GetRivermaxInterfaceAddress();
		RivermaxMediaSource->VideoStream.StreamAddress   = GenerateStreamAddress(OnwerInfo.ClusterNodeUniqueIdx.Get(0), OnwerInfo.OwnerUniqueIdx, OnwerInfo.OwnerType);
		RivermaxMediaSource->VideoStream.Port            = 50000;
		RivermaxMediaSource->VideoStream.bUseGPUDirect   = true;
	}
	else if (URivermaxMediaOutput* RivermaxMediaOutput = Cast<URivermaxMediaOutput>(MediaObject))
