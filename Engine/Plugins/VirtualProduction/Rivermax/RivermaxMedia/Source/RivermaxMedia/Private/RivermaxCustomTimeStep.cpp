...
bool URivermaxCustomTimeStep::WaitForSync()
{
	...
	if (!bWasWaitValid)
	{
		...
		{
			UE_LOGF(LogRivermaxMedia, Error, "Rivermax CustomTimeStep wait for sync timed out.");
		}
	...
	{
		UE_LOGF(LogRivermaxMedia, Warning,
			"The Engine couldn't run fast enough to keep up with the CustomTimeStep Sync. '%lld' frame(s) dropped.",
			NewFrameNumber - PreviousFrameNumber - ExpectedFramesPerWait);
	}
...
void URivermaxCustomTimeStep::OnRivermaxManagerInitialized()
{
	if (State == ECustomTimeStepSynchronizationState::Synchronizing)
	{
		...
		{
			UE_LOGF(LogRivermaxMedia, Warning, "Can't initialize Rivermax custom timestep. Library isn't available.");
		...
		{
			UE_LOGF(LogRivermaxMedia, Warning, "Can't initialize Rivermax custom timestep. No devices were found.")
			State = ECustomTimeStepSynchronizationState::Error;
			return;
		}
