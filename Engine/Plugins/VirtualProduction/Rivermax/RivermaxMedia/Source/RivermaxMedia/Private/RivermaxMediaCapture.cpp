...
{
	void GetOutputEncodingInfo(ERivermaxPixelFormat InPixelFormat, const FIntPoint& InSize, uint32& OutBytesPerElement, uint32& OutElementsPerFrame)
	{
		using namespace UE::RivermaxCore;

		const ESamplingType SamplingType = UE::RivermaxMediaUtils::Private::PixelFormatToRivermaxSamplingType(InPixelFormat);
...
bool URivermaxMediaCapture::ValidateMediaOutput() const
{
	...
	{
		UE_LOGF(LogRivermaxMedia, Error, "Can not start the capture. MediaOutput's class is not supported.");
		return false;
	}
	...
	for (uint8 StreamId = 0; StreamId < ERivermaxStreamType::MAX; ++StreamId)
	{
		...
		if (Options.GetStreamOptions(StreamType).IsValid())
		{
			...
			if (Streams[StreamId].IsValid())
			{
				...
				{
					UE_LOGF(LogRivermaxMedia, Error, "Couldn't initialize stream ERivermaxStreamType: %d", StreamType);
					break;
				}
			}
			else
			{
				...
				UE_LOGF(LogRivermaxMedia, Error, "Unable to create stream ERivermaxStreamType: %d", StreamType);
				break;
			}
		}
	}
...
bool URivermaxMediaCapture::ConfigureCapture() const
{
	...
	if (VideoOptions.IsValid())
	{
		...
		{
			UE_LOGF(LogRivermaxMedia, Error, "Could not find a matching interface for IP '%ls'", *VideoOptions->InterfaceAddress);
			return false;
		}
		...
		{
			UE_LOGF(LogRivermaxMedia, Warning, "Can't start capture. Invalid resolution requested: %dx%d", VideoOptions->Resolution.X, VideoOptions->Resolution.Y);
			return false;
		}
	...
	if (AncOptions.IsValid())
	{
		...
		if (bFoundDevice == false)
		{
			UE_LOGF(LogRivermaxMedia, Error, "Could not find a matching interface for IP '%ls'", *AncOptions->InterfaceAddress);
			return false;
		}
	}
	return true;
...
void URivermaxMediaCapture::OnFrameCapturedInternal_AnyThread(const FCaptureBaseData& InBaseData, TSharedPtr<FMediaCaptureUserData, ESPMode::ThreadSafe> InUserData, void* InBuffer, int32 Width, int32 Height, int32 BytesPerRow)
{
	...
	{
		UE_LOGF(LogRivermaxMedia, Verbose, "Failed to push captured frame");
	}
...
#if WITH_EDITOR
FString URivermaxMediaCapture::GetAnalyticsEventPrefix() const
{
	return TEXT("MediaFramework.Rivermax");
}

void URivermaxMediaCapture::GetAnalyticsEventAttributes(const FString& EventName, TArray<FAnalyticsEventAttribute>& InOutAttributes) const
{
	Super::GetAnalyticsEventAttributes(EventName, InOutAttributes);
	
	if (URivermaxMediaOutput* RivermaxOutput = Cast<URivermaxMediaOutput>(MediaOutput))
	{
		InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("FrameRate"), RivermaxOutput->VideoStream.FrameRate.ToPrettyText().ToString()));
	}
}
#endif
...
{
	UE_LOGF(LogRivermaxMedia, Error, "Outputstream has caught an error. Stopping capture.");
	if (GetState() != EMediaCaptureState::Stopped)
	{
