...
bool URivermaxTimecodeProvider::Initialize(class UEngine* InEngine)
{
	...
	{
		UE_LOGF(LogRivermaxMedia, Warning, "Can't initialize Rivermax TimecodeProvider, Rivermax Core Module isn't available.");
	...
	{
		UE_LOGF(LogRivermaxMedia, Warning, "Can't initialize Rivermax TimecodeProvider, library isn't initialized.");
		State = ETimecodeProviderSynchronizationState::Error;
		return false;
	}
	...
	{
		UE_LOGF(LogRivermaxMedia, Warning, "Can't initialize Rivermax TimecodeProvider, no compatible devices were found.");
		State = ETimecodeProviderSynchronizationState::Error;
		return false;
	}
	...
	{
		UE_LOGF(LogRivermaxMedia, Warning, "Can't initialize Rivermax TimecodeProvider, Rivermax clock type isn't supported.");
		State = ETimecodeProviderSynchronizationState::Error;
		return false;
	}
	...
	{
		UE_LOGF(LogRivermaxMedia, Warning, "Rivermax clock is using system time. Make sure it's synchronized to PTP to get expected results.");
	}
