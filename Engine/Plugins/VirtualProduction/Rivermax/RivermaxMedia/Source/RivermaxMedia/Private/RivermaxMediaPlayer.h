...
namespace  UE::RivermaxMedia
{
	...
	public: 
		...
		ERivermaxPixelFormat GetDesiredPixelFormat()
		{
	...
	private:
		...
		/**
		 * Listener forwarded to the ANC input stream.
		 * Keeps ANC initialization separate from the video stream state machine.
		 */
		class FAncStreamListener : public IRivermaxInputStreamListener
		{
		public:
			explicit FAncStreamListener(FRivermaxMediaPlayer& InPlayer) : Player(InPlayer) {}
			virtual ~FAncStreamListener() = default;

			virtual void OnInitializationCompleted(const FRivermaxInputInitializationResult& Result) override {}
			virtual TSharedPtr<IRivermaxVideoSample> OnVideoFrameRequested(const FRivermaxInputVideoFrameDescriptor& FrameInfo) override { return nullptr; }
			virtual void OnVideoFrameReceived(TSharedPtr<IRivermaxVideoSample> InReceivedVideoFrame) override {}
			virtual void OnStreamError() override {}
			virtual void OnVideoFormatChanged(const FRivermaxInputVideoFormatChangedInfo& NewFormatInfo) override {}

			virtual TSharedPtr<IRivermaxAncSample> OnAncFrameRequested(const FRivermaxInputAncFrameDescriptor& FrameInfo) override
			{
				return Player.HandleAncFrameRequested(FrameInfo);
			}
			virtual void OnAncFrameReceived(TSharedPtr<IRivermaxAncSample> InReceivedAncFrame) override
			{
				Player.HandleAncFrameReceived(InReceivedAncFrame);
			}
			virtual void OnAncFrameReceptionError() override
			{
				Player.HandleAncFrameReceptionError();
			}

		private:
			FRivermaxMediaPlayer& Player;
		};

		/** Called by FAncStreamListener when an ANC packet arrives. */
		TSharedPtr<IRivermaxAncSample> HandleAncFrameRequested(const FRivermaxInputAncFrameDescriptor& FrameInfo);
		void HandleAncFrameReceived(TSharedPtr<IRivermaxAncSample> InReceivedAncFrame);
		void HandleAncFrameReceptionError();
	...
	private:
		...
		/** ANC (2110-40) input streams (one per configured ANC stream). */
		TArray<TUniquePtr<IRivermaxInputStream>> AncInputStreams;

		/** Listeners forwarded to AncInputStreams to keep ANC callbacks separate from video state. */
		TArray<TUniquePtr<FAncStreamListener>> AncStreamListeners;

		/** Decoded ANC payload for one video frame. Add a TOptional field per new ANC type. */
		struct FVideoFrameAncData
		{
			// ATC LTC (see AncTimecodeDID/AncTimecodeSDID)
			TOptional<FTimecode> Timecode;
		};

		/** Keyed by RTP timestamp (shared 90 kHz PTP clock between ST2110-20 and -40). */
		TMap<uint32, FVideoFrameAncData> PendingAncData;
		FCriticalSection PendingAncDataLock;

		/** Pixel format provided by media source */
		ERivermaxPixelFormat DesiredPixelFormat = ERivermaxPixelFormat::RGB_10bit;
