...
enum class ERivermaxMediaOutputVersion : int32
{
	BeforeAncTimecode = 0,
	AncTimecode = 1,
	FrameManagementChange = 2,
	Latest = FrameManagementChange
};
...
void URivermaxMediaOutput::ExportSDP(const FString& InPath)
{
	...
	if (bSaved)
	{
		UE_LOGF(LogTemp, Log, "Saved SDP successfully to: %ls", *InPath);
	}
	else
	{
		UE_LOGF(LogTemp, Error, "Failed to save SDP file: %ls", *InPath);
	}
}
...
UE::RivermaxCore::FRivermaxOutputOptions URivermaxMediaOutput::GenerateStreamOptions() const
{
	...
	// Video configuration
	{
		...
		OutOutputOptions.NumberOfBuffers = NumberOfTextureBuffers;
		...
		VideoOptions->PixelFormat = UE::RivermaxMediaUtils::Private::PixelFormatToRivermaxSamplingType(VideoStream.PixelFormat);
...
void URivermaxMediaOutput::PostLoad()
{
	...
	if (Version < (int32)ERivermaxMediaOutputVersion::FrameManagementChange)
	{
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		NumberOfTextureBuffers = PresentationQueueSize_DEPRECATED;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
