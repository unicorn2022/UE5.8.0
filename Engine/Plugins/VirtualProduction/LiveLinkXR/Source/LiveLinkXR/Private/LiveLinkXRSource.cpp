	...
	: ConnectionSettings(InConnectionSettings)
{
	...
	{
		UE_LOGF(LogLiveLinkXR, Error, "LiveLinkXRSource: Couldn't find a valid XR System");
		return;
	}
	...
	{
		UE_LOGF(LogLiveLinkXR, Error, "LiveLinkXRSource: FLiveLinkXROpenXRExtModule not available");
		return;
	}
	...
	{
		UE_LOGF(LogLiveLinkXR, Error, "LiveLinkXRSource: Extension plugin failed to initialize");
		return;
	}
...
uint32 FLiveLinkXRSource::Run()
{
	...
	while (!bStopping)
	{
		...
		if (TimeNowSec >= (LastFrameTimeSec + FrameIntervalSec))
		{
			...
			{
				UE_LOGF(LogLiveLinkXR, Error, "Source thread couldn't pin FLiveLinkXROpenXRExtension; stopping");
				bStopping = true;
				return 1;
			}
