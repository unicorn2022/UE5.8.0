...
void USoundWaveTimecodeUtils::SetTimecodeInfo(const FTimecode& InTimecode, const FFrameRate& InFrameRate, USoundWave* OutSoundWave)
{
	...
	{
		UE_LOGF(LogSoundWaveTimecodeUtils, Error, "[%ls] Sound wave pointer is invalid", ANSI_TO_TCHAR(__FUNCTION__));
		return;
	}
	...
	check(false);
	UE_LOGF(LogSoundWaveTimecodeUtils, Error, "Function %ls is not supported in non-editor targets", ANSI_TO_TCHAR(__FUNCTION__));
#endif
}
...
bool USoundWaveTimecodeUtils::HasTimecode(const USoundWave* InSoundWave)
{
	if (!ensure(IsValid(InSoundWave)))
	{
		UE_LOGF(LogSoundWaveTimecodeUtils, Error, "[%ls] Sound wave pointer is invalid", ANSI_TO_TCHAR(__FUNCTION__));
	...
	if (!SoundWaveTimecodeInfo->TimecodeRate.IsValid() || SoundWaveTimecodeInfo->TimecodeRate.Numerator <= 0 || SoundWaveTimecodeInfo->TimecodeRate == FFrameRate())
	{
		return false;
	}

	if (SoundWaveTimecodeInfo->NumSamplesPerSecond == 0)
	{
		return false;
	}
...
bool USoundWaveTimecodeUtils::HasTimecodeRate(const USoundWave* InSoundWave)
{
	if (!ensure(IsValid(InSoundWave)))
	{
		UE_LOGF(LogSoundWaveTimecodeUtils, Error, "[%ls] Sound wave pointer is invalid", ANSI_TO_TCHAR(__FUNCTION__));
