...
bool UMeshCaptureData::IsInitialized(ECaptureDataInitializedCheck InInitializedCheck) const
{
...
void UMeshCaptureData::GetDataForConforming(const FTransform& InTransform, TArray<float>& OutVertices, TArray<int32>& OutTriangles) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogCaptureDataCore, Error, "Failed to get data for conforming as TargetMesh is a '%ls' but should be a UStaticMesh or USkeletalMesh", *TargetMesh->GetClass()->GetName());
		}
		else
		{
			UE_LOGF(LogCaptureDataCore, Error, "Failed to get data for conforming as TargetMesh is invalid");
		}
	}
#endif
}
...
bool UFootageCaptureData::IsInitialized(ECaptureDataInitializedCheck InInitializedCheck) const
{
	return !VerifyData(InInitializedCheck).HasError();
}

UFootageCaptureData::FVerifyResult UFootageCaptureData::VerifyData(ECaptureDataInitializedCheck InInitializedCheck) const
{
	CHECK_AND_RET_ERROR(ViewsContainsValidData(InInitializedCheck));

	if (EnumHasAnyFlags(InInitializedCheck, ECaptureDataInitializedCheck::Calibrations))
	{
		CHECK_AND_RET_ERROR(CalibrationContainsValidData());
	}

	if (EnumHasAnyFlags(InInitializedCheck, ECaptureDataInitializedCheck::Metadata))
	{
...
UFootageCaptureData::FVerifyResult UFootageCaptureData::ViewsContainsValidData(ECaptureDataInitializedCheck InInitializedCheck) const
{
	if (EnumHasAnyFlags(InInitializedCheck, ECaptureDataInitializedCheck::ImageSequences))
	{
		...
		for (int32 Index = 0; Index < ImageSequences.Num(); ++Index)
		{
			...
			CHECK_AND_PRINT_RET(FImageSequenceUtils::IsImageSequenceValid(ImageSequence), TEXT("Image Sequence asset is invalid"));
			CHECK_AND_PRINT_RET(ImageSequence->FrameRateOverride.IsValid(), TEXT("Image Sequence asset contains invalid frame rate"));
		}
	}

	if (EnumHasAnyFlags(InInitializedCheck, ECaptureDataInitializedCheck::DepthSequences))
	{
		...
		for (int32 Index = 0; Index < DepthSequences.Num(); ++Index)
		{
			...
			CHECK_AND_PRINT_RET(FImageSequenceUtils::IsImageSequenceValid(DepthSequence), TEXT("Depth Sequence asset is invalid"));
...
FTimecode UFootageCaptureData::GetEffectiveAudioTimecode() const
{
	...
	if (AudioTracks.Num() > 0)
	{
		...
		{
			// Checks the timecode rate as well
			if (USoundWaveTimecodeUtils::HasTimecode(SoundWave))
			{
				Timecode = USoundWaveTimecodeUtils::GetTimecode(SoundWave);
...
FFrameRate UFootageCaptureData::GetEffectiveAudioTimecodeRate() const
{
	...
	if (AudioTracks.Num() > 0)
	{
		...
		if (SoundWave != nullptr)
		{
			if (USoundWaveTimecodeUtils::HasTimecodeRate(SoundWave))
			{
				FrameRate = USoundWaveTimecodeUtils::GetFrameRate(SoundWave);
...
void UFootageCaptureData::GetDefaultTimecodeInfo(FTimecode& OutTimecode, FFrameRate& OutFrameRate) const
{
	...
	for (const TObjectPtr<class UImgMediaSource>& ImageSequence : ImageSequences)
	{
		if (ImageSequence.Get() != nullptr)
		{
			...
			if (!bTimecodeSet && UImageSequenceTimecodeUtils::IsValidTimecode(Timecode))
			{
				OutTimecode = MoveTemp(Timecode);
				bTimecodeSet = true;
			}

			if (!bFrameRateSet && UImageSequenceTimecodeUtils::IsValidFrameRate(FrameRate))
			{
	...
	if (AudioTracks.Num() > 0)
	{
		...
		{
			// Checks the timecode rate as well
			if (!bTimecodeSet && USoundWaveTimecodeUtils::HasTimecode(SoundWave))
			{
				OutTimecode = USoundWaveTimecodeUtils::GetTimecode(SoundWave);
				bTimecodeSet = true;
			}
			
			if (!bFrameRateSet)
			{
				OutFrameRate = USoundWaveTimecodeUtils::GetFrameRate(SoundWave);
				bFrameRateSet = true;
			}
		}
	}
...
static int32 FindCameraByName(const TArray<TObjectPtr<UImgMediaSource>>& InSequence, const FString& InName)
{
	for (int32 Index = 0; Index < InSequence.Num(); ++Index)
	{
		TObjectPtr<const UImgMediaSource> ImageSequence = InSequence[Index];

		UPackage* Package = ImageSequence.GetPackage();
		check(Package);

		FMetaData& MetaData = Package->GetMetaData();
		if (!MetaData.HasValue(ImageSequence, "CameraId"))
		{
			// End the loop early as we are not supporting having partial metadata
			return INDEX_NONE;
		}

		FString CameraId = MetaData.GetValue(ImageSequence, "CameraId");
		if (CameraId == InName)
		{
			return Index;
		}
...
static int32 CheckCalibrationArray(const TArray<FExtendedLensFile>& InCalibrationArray, const FString& InName, bool bIsDepth)
{
	int32 Index = 0;
...
int32 UFootageCaptureData::GetViewIndexByCameraName(const FString& InName) const
{
	...
	// Making sure that the code above returns if the CameraCalibrations array is empty
	ensure(CameraCalibrations.Num() >= 1);

	if (!CameraCalibrations[0])
	{
		return INDEX_NONE;
	}
...
#if WITH_EDITORONLY_DATA
	Index = FindCameraByName(ImageSequences, InName);
	if (Index != INDEX_NONE)
	{
		return Index;
	}

	Index = FindCameraByName(DepthSequences, InName);
	if (Index != INDEX_NONE)
	{
		return Index;
	}
	...
	Index = CheckCalibrationArray(CameraCalibrations[0]->CameraCalibrations, InName, false);
	...
	UE_LOGF(
		LogCaptureDataCore,
		Warning,
		"Camera name \"%ls\" could not be found among the calibrations of the capture data asset. Available names are %ls",
		*InName,
		*AvailableNames
	);
