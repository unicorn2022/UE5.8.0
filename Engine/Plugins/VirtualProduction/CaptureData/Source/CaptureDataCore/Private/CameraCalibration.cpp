														  ...
														  TArray<TPair<FString, FString>>& OutStereoReconstructionPairs) const
{
	...
	for (int32 Cam = 0; Cam < CameraCalibrations.Num(); Cam++)
	{
		...
		{
			UE_LOGF(LogCaptureDataCore, Warning, "Camera calibration does not contain a SphericalLensModel lens distortion.");
			return false;
		}
		...
		if (!bGotPoint)
		{
			UE_LOGF(LogCaptureDataCore, Warning, "Camera calibration does not contain a valid lens distortion.");
			return false;
		}
		...
		if (!bGotPoint)
		{
			UE_LOGF(LogCaptureDataCore, Warning, "Camera calibration does not contain a valid focal length.");
			return false;
		}
		...
		if (!bGotPoint)
		{
			UE_LOGF(LogCaptureDataCore, Warning, "Camera calibration does not contain a valid image center.");
			return false;
		}
		...
		if (!bGotPoint)
		{
			UE_LOGF(LogCaptureDataCore, Warning, "Camera calibration does not contain a valid nodal offset.");
			return false;
		}
...
bool UCameraCalibration::ConvertFromTrackerNodeCameraModels(const TArray<FCameraCalibration>& InCalibrations, const TMap<FString, FString>& InLensAssetNamesMap, bool bInUsingUnrealCoordinateSystem)
{
	for (int32 Index = 0; Index < InCalibrations.Num(); ++Index)
	{
		...
		else
		{
			...
			UE_LOGF(LogCaptureDataCore, Warning, "No lens file asset name specified. Using default lens file asset name.");
		}
...
int32 UCameraCalibration::GetCalibrationIndexByName(const FString& InName) const
{
	...
	UE_LOGF(
		LogCaptureDataCore,
		Warning,
		"Camera name \"%ls\" could not be found in the camera calibration asset. Available names are %ls",
		*InName,
		*AvailableNames
	);
