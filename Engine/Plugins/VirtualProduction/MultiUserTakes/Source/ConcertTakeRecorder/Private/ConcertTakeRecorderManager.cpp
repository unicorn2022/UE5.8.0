...
static int32 bSkipPackageFilter = 0;
static FAutoConsoleVariableRef  CVarSkipPackageFilter(TEXT("Concert.TakeRecorderSkipPackageFilter"), bSkipPackageFilter, TEXT("Indicates if we should skip running the package filter for take recorder while in a Multi-user session. This will force all packages to be use default behavior and _temp packages may not get excluded."));
...
{
	UE_LOGF(LogConcertTakeRecorder, Display, "Multi-user Take Recorder Session Startup: %ls", *InSession->GetSessionInfo().SessionName);
	WeakSession = nullptr;
...
{
	UE_LOGF(LogConcertTakeRecorder, Display, "Multi-user Take Recorder Session Shutdown: %ls", *InSession->GetSessionInfo().SessionName);
...
bool FConcertTakeRecorderManager::ShouldIconBeVisible() const
{
	...
	{
		bool bShouldSuppressBecauseOfRemoteOpen = false;
		if (TSharedPtr<IConcertSyncClient> ConcertSyncClient = IConcertSyncClientModule::Get().GetClient(TEXT("MultiUser")))
		{
			const SIZE_T Recorders = CanRecord() + RemoteRecorders();
			bShouldSuppressBecauseOfRemoteOpen = Recorders > 1 && ConcertSyncClient->GetSequencerManager()->IsSequencerRemoteOpenEnabled();
		}
		...
		if (LevelSequence)
		{
			...
			return !bShouldSuppressBecauseOfRemoteOpen && Sources && Sources->GetSources().Num() > 0;
		}
	}
...
void FConcertTakeRecorderManager::OnTakeRecorderStopped(UTakeRecorder* TakeRecorder)
{
	...
	{
		UE_LOGF(LogConcertTakeRecorder, Warning, "A transaction is pending but we are currently stopping a Take Recorder. Transactions during a Recorder Stop can cause asset locking between Multi-user clients.");
	}
#endif
}
	...
	if (IsTakeSyncEnabled() && !bIsRecording && TakeRecorderState.LastStartedTake != TakeRecorder->GetName())
	{
		if (TSharedPtr<IConcertClientSession> Session = WeakSession.Pin())
		{
			...
			if (TakeMetaData && TakePreset)
			{
				...
				UE_LOGF(LogConcertTakeRecorder, Display, "Sending custom event to initialize recording.");
...
void FConcertTakeRecorderManager::OnTakeInitializedEvent(const FConcertSessionContext&, const FConcertTakeInitializedEvent& InEvent)
{
	...
	if (IsTakeSyncEnabled() && CanRecord())
	{
		...
		if (TakePreset && TakePreset->GetLevelSequence())
		{
			...
			if (bIsRecording)
			{
				UE_LOGF(LogConcertTakeRecorder, Display, "Take initialized for recording by multi-user event.");
			}
			else
			{
				UE_LOGF(LogConcertTakeRecorder, Error, "Failed to initialize for recording by multi-user event.");
			}
...
void FConcertTakeRecorderManager::OnSessionClientChanged(IConcertClientSession& Session, EConcertClientStatus ClientStatus, const FConcertSessionClientInfo& InClientInfo)
{
	...
	case EConcertClientStatus::Connected:
	{
		...
		UE_LOGF(LogConcertTakeRecorder, Display, "Session Client Change: %ls Connected.", *InClientInfo.ClientInfo.DisplayName);
	...
	case EConcertClientStatus::Disconnected:
		UE_LOGF(LogConcertTakeRecorder, Display, "Session Client Change: %ls Disconnected.", *InClientInfo.ClientInfo.DisplayName);
		break;
...
{
	UE_LOGF(LogConcertTakeRecorder, Display, "Multi-user Take Recorder Connected to Session: %ls", *InSession.GetSessionInfo().SessionName);
...
void FConcertTakeRecorderManager::OnSessionConnectionChanged(IConcertClientSession& InSession, EConcertConnectionStatus ConnectionStatus)
{
	...
	{
		UE_LOGF(LogConcertTakeRecorder, Display, "Multi-user Take Recorder Disconnecting from Session: %ls", *InSession.GetSessionInfo().SessionName);
		DisconnectFromSession();
	}
...
{
	UE_LOGF(LogConcertTakeRecorder, Display, "Record Setting Changed");
...
void FConcertTakeRecorderManager::UpdateSessionClientList()
{
	...
	{
		UE_LOGF(LogConcertTakeRecorder, Display, "Session Client Add->%ls", *Client.ToDisplayString().ToString());
...
EPackageFilterResult FConcertTakeRecorderManager::ShouldPackageBeFiltered(const FConcertPackageInfo& InPackageInfo)
{
	...
	if (IsTakeSyncEnabled() && WeakSession.IsValid() && !CanRecord() && bSkipPackageFilter == 0)
	{
		...
		if (  (  FullName.Contains(Project.RootTakeSaveDir.Path)
			  || FullName.Contains(Project.GetResolvedRootTakeSaveDir()))
			&& FullName.Contains(UE::TakeRecorderManager::Private::TempPostfix))
		{
			bShouldFilter = true;
		}
	}
	FString YesOrNo = bShouldFilter ? TEXT("Yes") : TEXT("UseDefault");
	UE_LOGF(LogConcertTakeRecorder, Verbose, "Considering package %ls for filtering: %ls", *FullName, *YesOrNo);
	return bShouldFilter ? EPackageFilterResult::Exclude : EPackageFilterResult::UseDefault;
}
