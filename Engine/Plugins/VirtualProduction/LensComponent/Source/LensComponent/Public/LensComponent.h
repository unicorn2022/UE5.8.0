...
public:
	...
	/** Get whether the camera overscan is overriden. */
	UFUNCTION(BlueprintPure, Category = "Lens Component")
	UE_API bool GetOverrideCameraOverscan() const;

	/** Set whether the camera overscan should be overriden. */
	UFUNCTION(BlueprintCallable, Category = "Lens Component")
	UE_API void SetOverrideCameraOverscan(bool bOverride);
...
private:
	...
	/** Convenience function to restore the camera overscan defaults. */
	void RestoreCameraOverscanDefaults();
...
protected:
	 ...
	 *   Disabled for Post Process Material or pass after tonemap
	 *   Enabled for Temporal Super Resolution
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Getter = GetOverrideCameraOverscan, Setter = SetOverrideCameraOverscan, Category = "Distortion")
	...
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadWrite, Category = "Distortion", meta = (InlineEditConditionToggle))
