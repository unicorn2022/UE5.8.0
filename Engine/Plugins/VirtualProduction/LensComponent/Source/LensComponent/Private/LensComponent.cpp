	...
	if (UCineCameraComponent* const CineCameraComponent = Cast<UCineCameraComponent>(TargetCameraComponent.GetComponent(GetOwner())))
	{
		...
		if (bApplyDistortion)
		{
			if (LensDistortionHandler && bWasDistortionEvaluated)
			{
				...
				else if (ResolvedRenderingMode == EDistortionRenderingMode::TemporalSuperResolution || ResolvedRenderingMode == EDistortionRenderingMode::PassAfterTonemap)
				{
					if (UCameraCalibrationSubsystem* SubSystem = GEngine->GetEngineSubsystem<UCameraCalibrationSubsystem>())
					{
						...
						{
							SubSystem->SetLensDistortionSVEState(CameraActor, BlendState, LensDistortionHandler, ResolvedRenderingMode);
						}
					}
				...
				if (bOverrideCameraOverscan)
				{
					...
					LensDistortionHandler->SetOverscanFactor(CineCameraComponent->Overscan + 1.0f);
 					...
 					CineCameraComponent->bCropOverscan = (ResolvedRenderingMode == EDistortionRenderingMode::TemporalSuperResolution);
				}
			}
			else
			{
...
void ULensComponent::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		SetApplyDistortion(bApplyDistortion);
	}
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(ULensComponent, bOverrideCameraOverscan))
	{
		SetOverrideCameraOverscan(bOverrideCameraOverscan);
	}
...
void ULensComponent::RestoreCameraOverscanDefaults()
{
	const UCineCameraComponent* CDO = UCineCameraComponent::StaticClass()->GetDefaultObject<UCineCameraComponent>();
	UCineCameraComponent* CineCameraComponent = Cast<UCineCameraComponent>(TargetCameraComponent.GetComponent(GetOwner()));

	if (IsValid(CineCameraComponent))
	{
		CineCameraComponent->Modify();
		CineCameraComponent->Overscan = CDO->Overscan;
		CineCameraComponent->bCropOverscan = CDO->bCropOverscan;
	}
}
...
void ULensComponent::SetApplyDistortion(bool bApply)
{
	...
	if (bApplyDistortion == false)
	{
		...
		RestoreCameraOverscanDefaults();
	}
}

bool ULensComponent::GetOverrideCameraOverscan() const
{
	return bOverrideCameraOverscan;
}

void ULensComponent::SetOverrideCameraOverscan(bool bOverride)
{
	bOverrideCameraOverscan = bOverride;

	if (bOverrideCameraOverscan == false)
	{
		RestoreCameraOverscanDefaults();
	}
}
...
EDistortionRenderingMode ULensComponent::GetDistortionRenderingMode() const
{
	...
	if (LensDistortionHandler)
	{
		...
		{
			// Reverse distorting models prefer the plugin pass for accuracy, forward distorting models prefer TSR
			return LensDistortionHandler->IsForwardDistorting() 
				? EDistortionRenderingMode::TemporalSuperResolution
				: EDistortionRenderingMode::PassAfterTonemap;
		}
		
		return PreferredMode;
	}

	// Default to plugin pass if no handler is found
	return EDistortionRenderingMode::PassAfterTonemap;
}
