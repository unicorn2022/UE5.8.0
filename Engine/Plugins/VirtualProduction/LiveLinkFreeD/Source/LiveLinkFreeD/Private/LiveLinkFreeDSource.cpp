...
, CustomSubjectName(ConnectionSettings.SubjectName)
{
	...
	if ((Socket != nullptr) && (Socket->GetSocketType() == SOCKTYPE_Datagram))
	{
		...
		UE_LOGF(LogLiveLinkFreeD, Log, "LiveLinkFreeDSource: Opened UDP socket with IP address %ls", *DeviceEndpoint.ToString());
	}
	else
	{
		UE_LOGF(LogLiveLinkFreeD, Error, "LiveLinkFreeDSource: Failed to open UDP socket with IP address %ls", *DeviceEndpoint.ToString());
	}
}
...
void FLiveLinkFreeDSource::OnSettingsChanged(ULiveLinkSourceSettings* Settings, const FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (Property && MemberProperty && (PropertyChangedEvent.ChangeType != EPropertyChangeType::Interactive))
	{
		...
		{
			UE_LOGF(LogLiveLinkFreeD, Error, "LiveLinkFreeDSource: OnSettingsChanged pointers don't match - this should never happen!");
			return;
		}
...
void FLiveLinkFreeDSource::UpdateEncoderData(FFreeDEncoderData* InEncoderData)
{
	...
	{
		UE_LOGF(LogLiveLinkFreeD, Error, "LiveLinkFreeDSource: EncoderData Min/Max values can't be equal (you may need to rack your encoder) - incoming data may be invalid!");
	}
			...
			while (Socket && Socket->HasPendingData(PendingDataSize))
			{
				...
				if (Socket && Socket->Recv(ReceiveBuffer.GetData(), ReceiveBufferSize, ReceivedDataSize))
				{
					if (ReceivedDataSize > 0)
					{
						...
						{
							UE_LOGF(LogLiveLinkFreeD, Error, "LiveLinkFreeDSource: Received a packet, but we don't have a valid SavedSourceSettings!");
						}
						else if (ReceiveBuffer[FreeDPacketDefinition::PacketType] == FreeDPacketDefinition::PacketTypeD1)
						{
							...
							{
								UE_LOGF(LogLiveLinkFreeD, Warning, "LiveLinkFreeDSource: Received packet checksum error - received 0x%02x, calculated 0x%02x", ReceiveBuffer[FreeDPacketDefinition::Checksum], Checksum);
							}

							if (ReceivedDataSize != FreeDPacketDefinition::PacketSizeD1)
							{
								UE_LOGF(LogLiveLinkFreeD, Warning, "LiveLinkFreeDSource: Received packet length mismatch - received 0x%02x, calculated 0x%02x", ReceivedDataSize, FreeDPacketDefinition::PacketSizeD1);
							}
						...
						{
							UE_LOGF(LogLiveLinkFreeD, Warning, "LiveLinkFreeDSource: Unsupported FreeD message type 0x%02x", ReceiveBuffer[FreeDPacketDefinition::PacketType]);
						}
					}
				}
			}
