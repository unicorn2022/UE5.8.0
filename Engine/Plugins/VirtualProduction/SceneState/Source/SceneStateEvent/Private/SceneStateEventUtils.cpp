...
bool BroadcastEvent(const UObject* InEventContext, const FSceneStateEventSchemaHandle& InEventSchemaHandle, FInstancedStruct&& InEventData)
{
	if (!InEventContext)
	{
		UE_LOGF(LogSceneStateEvent, Error, "Broadcast event failed. Event context is invalid.");
		return false;
	}
	...
	if (!EventSubsystem)
	{
		UE_LOGF(LogSceneStateEvent, Error, "Broadcast event failed. Event subsystem not found.");
		return false;
	}
	...
	if (!EventSchema)
	{
		UE_LOGF(LogSceneStateEvent, Error, "Broadcast event failed. Event schema not found.");
		return false;
	}
