...
void FSceneStateTaskDescRegistry::CacheTaskDescs()
{
	...
	for (const UScriptStruct* Struct : TObjectRange<UScriptStruct>())
	{
		if (Struct->IsChildOf<FSceneStateTaskDesc>())
		{
			...
			if (!SupportedTask)
			{
				UE_LOGF(LogSceneStateEditor, Error, "Task Desc '%ls' cannot be registered because it does not have a valid supported task!"
			...
			{
				UE_LOGF(LogSceneStateEditor, Warning, "Existing Task Desc '%ls' will get replaced by '%ls'"
