...
void SStateMachineTaskNode::GetNodeInfoPopups(FNodeInfoContext* InContext, TArray<FGraphInformationPopupInfo>& OutPopups) const
{
	...
	const USceneStateMachineTaskNode* const Node = Cast<USceneStateMachineTaskNode>(GraphNode);
	if (!Node || !Node->GetCompiledIndex().IsSet())
	{
		return;
	}
	...
	TemplateData->ForEachTaskStatus(RootState->GetContextRegistry(), *Node->GetCompiledIndex(),
		[&OutPopups, &ResultStatus, &StatusColors = StatusColors](UE::SceneState::EExecutionStatus InTaskStatus)
		{
			// Get the lowest status to show
			if (InTaskStatus <= ResultStatus.Get(EExecutionStatus::Finished))
			{
				ResultStatus = InTaskStatus;
			}
			...
			OutPopups.Emplace(nullptr, StatusColors[InTaskStatus], Message.ToString());
		});
