...
FSceneStateMachine FStateMachineCompiler::Compile()
{
	...
	StateNodeIndexMap.Reset();

	// Upgrade the map to absolute indices before baking it to the generated class' map
	ToAbsoluteIndexMap(StateParameterIdIndexMap, StateMachine.StateRange.Index);
	TemplateData->StateParameterIdToIndex.Append(MoveTemp(StateParameterIdIndexMap));
	StateParameterIdIndexMap.Reset();
...
void FStateMachineCompiler::CompileState(const USceneStateMachineStateNode* InStateNode)
{
	...
	NewState.Parameters = InStateNode->Parameters;
	...
	StateParameterIdIndexMap.Add(InStateNode->ParametersId, StateIndex);
...
void FStateMachineCompiler::CompileSubStateMachines(FSceneState& InNewState, const USceneStateMachineStateNode* InStateNode)
{
	...
	const TArray<UEdGraph*> SubGraphs = InStateNode->GetSubGraphs();
	...
	StateMachineIdToIndex.Reserve(SubGraphs.Num());
	...
	for (UEdGraph* SubGraph : SubGraphs)
	{
		...
		{
			const uint16 StateMachineIndex = SubStateMachines.Num();
			SubStateMachines.Emplace(MoveTemp(NewStateMachine));
			StateMachineGraphToIndex.Emplace(NewStateMachineGraph, StateMachineIndex);
			StateMachineIdToIndex.Emplace(NewStateMachineGraph->ParametersId, StateMachineIndex);
		}
	}
	...
	// Upgrade the map to absolute indices before baking it to the generated class' map
	ToAbsoluteIndexMap(StateMachineIdToIndex, InNewState.StateMachineRange.Index);
	TemplateData->StateMachineIdToIndex.Append(MoveTemp(StateMachineIdToIndex));
}
