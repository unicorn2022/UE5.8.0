...
void FStateMachineConnectionDrawingPolicy::DrawArrowLine(const FVector2f& InStartPoint, const FVector2f& InEndPoint, const FConnectionParams& InParams)
{
	...
	const FVector2f ClosestPoint = FMath::ClosestPointOnSegment2D(AbsoluteMousePosition, InStartPoint, EndPoint);

	constexpr double RelinkHandleHoverRadius = 20.0;

	if ((ClosestPoint - AbsoluteMousePosition).Length() < RelinkHandleHoverRadius)
	{
		const double RelinkHoverRadiusSq = FMath::Square(RelinkHandleHoverRadius);
		const double StartMouseSizeSq = FVector2f(InStartPoint - AbsoluteMousePosition).SizeSquared();
		const double EndMouseSizeSq = FVector2f(EndPoint - AbsoluteMousePosition).SizeSquared();
