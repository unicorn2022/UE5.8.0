...
FSceneStateCachedBindingData::FPinInfo FSceneStateCachedBindingData::GetPinInfoForProperty(TNotNull<const FProperty*> InProperty, FPropertyBindingDataView InTargetDataView) const
{
	...
	if (UE::SceneState::IsPropertyReference(InProperty) && InTargetDataView.IsValid())
	{
		...
		{
			Result.Type = GetPropertyReferencePinType(InProperty, TargetIndirections.Last().GetPropertyAddress());
		}
		Result.IconBrush = FAppStyle::GetBrush("Kismet.Tabs.Variables");
	}
	return Result;
}
...
void FSceneStateCachedBindingData::AddPropertyInfoOverride(const FProperty& InProperty, TArray<TSharedPtr<const UE::PropertyBinding::FPropertyInfoOverride>>& OutPropertyInfoOverrides) const
{
	...
	if (StructProperty->Struct->IsChildOf<FSceneStatePropertyReference>())
	{
		...
		for (const FEdGraphPinType& PinType : PropertyReferenceInfo)
		{
			...
			PropertyInfoOverride->TypeNameText = UEdGraphSchema_K2::TypeToText(PinType);
			OutPropertyInfoOverrides.Emplace(MoveTemp(PropertyInfoOverride));
		}

		TSharedRef<UE::PropertyBinding::FPropertyInfoOverride> PropertyInfoOverride = MakeShared<UE::PropertyBinding::FPropertyInfoOverride>();
		PropertyInfoOverride->PinType.PinCategory = UEdGraphSchema_K2::PC_Struct;
		PropertyInfoOverride->PinType.PinSubCategoryObject = StructProperty->Struct;
		PropertyInfoOverride->TypeNameText = LOCTEXT("PromotePropertyReferenceDisplayName", "Promote as Reference");
		OutPropertyInfoOverrides.Emplace(MoveTemp(PropertyInfoOverride));
	}
}
