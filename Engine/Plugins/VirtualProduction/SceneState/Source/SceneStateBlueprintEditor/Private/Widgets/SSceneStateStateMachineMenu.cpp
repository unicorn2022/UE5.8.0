...
void SStateMachineMenu::GetGraphActionDetails(const TSharedPtr<FEdGraphSchemaAction>& InAction, UObject*& OutDetailsObject, FText& OutDetailsText) const
{
	...
	else if (GraphActionType == Graph::FStateMachineAction_Node::StaticGetTypeId())
	{
		TSharedPtr<Graph::FStateMachineAction_Node> NodeAction = StaticCastSharedPtr<Graph::FStateMachineAction_Node>(InAction);
		if (USceneStateMachineNode* Node = NodeAction->GetNode())
		{
			OutDetailsObject = Node;
			OutDetailsText = FText::FromName(Node->GetNodeName());
		}
	}
}
...
bool SStateMachineMenu::ShouldProcessNode(TNotNull<const USceneStateMachineNode*> InGraphNode) const
{
	return InGraphNode->GetBoundGraphs().ContainsByPredicate(
		[This=this](UEdGraph* InBoundGraph)
		{
			return This->ShouldProcessGraph(InBoundGraph);
		});
}

void SStateMachineMenu::CollectNodeActionsRecursive(TNotNull<USceneStateMachineNode*> InNode, FText InCategory, FGraphActionListBuilderBase& OutActions)
{
	if (!ShouldProcessNode(InNode))
	{
		return;
	}

	const Graph::FStateMachineAction_Node::FArguments Arguments
		{
			.Node = InNode,
			.Category = InCategory,
	...
	TSharedRef<Graph::FStateMachineAction_Node> NodeAction = MakeShared<Graph::FStateMachineAction_Node>(Arguments);
	OutActions.AddAction(NodeAction);

	InCategory = FText::Format(INVTEXT("{0}|{1}"), InCategory, NodeAction->GetMenuDescription());

	for (UEdGraph* BoundGraph : InNode->GetBoundGraphs())
	{
		CollectGraphActionsRecursive(BoundGraph, InCategory, EEdGraphSchemaAction_K2Graph::Type::Subgraph, OutActions);
	}
}
...
void SStateMachineMenu::CollectGraphActionsRecursive(UEdGraph* InGraph, FText InCategory, int32 InGraphType, FGraphActionListBuilderBase& OutActions)
{
	...
	{
		if (USceneStateMachineNode* Node = Cast<USceneStateMachineNode>(GraphNode))
		{
			CollectNodeActionsRecursive(Node, InCategory, OutActions);
		}
	}
}
...
void SStateMachineMenu::ExecuteGraphAction(const TSharedPtr<FEdGraphSchemaAction>& InAction)
{
	...
	const FName ActionTypeId = InAction->GetTypeId();
	if (ActionTypeId == Graph::FBlueprintAction_Graph::StaticGetTypeId())
	{
		TSharedPtr<Graph::FBlueprintAction_Graph> GraphAction = StaticCastSharedPtr<Graph::FBlueprintAction_Graph>(InAction);
		if (GraphAction->EdGraph)
		{
	...
	else if (ActionTypeId == Graph::FStateMachineAction_Node::StaticGetTypeId())
	{
		TSharedPtr<Graph::FStateMachineAction_Node> NodeAction = StaticCastSharedPtr<Graph::FStateMachineAction_Node>(InAction);
		if (USceneStateMachineNode* Node = NodeAction->GetNode())
		{
			BlueprintEditor->JumpToHyperlink(Node, /*bRequestRename*/false);
		}
	}
}
