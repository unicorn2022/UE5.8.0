...
public:
	...
	/** Get the index to the state machine this node compiled to last */
	UE_INTERNAL UE_API TOptional<uint16> GetCompiledIndex() const;

	/** Sets the index to the state machine this node compiled to last */
...
private:
	/** Index to the state machine this graph compiled to last. */
	UPROPERTY()
