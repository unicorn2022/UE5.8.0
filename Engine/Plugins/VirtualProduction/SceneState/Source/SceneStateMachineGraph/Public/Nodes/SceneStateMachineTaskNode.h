...
public:
	...
	UE_API bool SetTaskStruct(const UScriptStruct* InTaskStruct);

	UE_API void SetTaskBlueprintClass(TSubclassOf<USceneStateBlueprintableTask> InTaskClass);

	UE_API bool FindDataViewById(const FGuid& InStructId, FStructView& OutDataView);
	...
	/** Gets the index to the task this node compiled to last */
	UE_INTERNAL UE_API TOptional<uint16> GetCompiledIndex() const;

	/** Sets the index to the task this node compiled to last */
...
private:
	...
	/** Index to the task this task node compiled to last. */
	UPROPERTY()
