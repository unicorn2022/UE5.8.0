...
void FSceneStateBindingCollection::VisitBindings(FPropertyBindingIndex16 InBegin, FPropertyBindingIndex16 InEnd,
	TFunctionRef<EVisitResult(const FPropertyBindingBinding& Binding, const int32 BindingIndex)> InFunction) const
{
	...
	{
		checkf(Bindings.IsValidIndex(BindingIndex), TEXT("Index %d out of bounds! Bindings Num: %d"), BindingIndex, Bindings.Num());
		if (InFunction(Bindings[BindingIndex], BindingIndex) == EVisitResult::Break)
		{
			break;
		}
	}
