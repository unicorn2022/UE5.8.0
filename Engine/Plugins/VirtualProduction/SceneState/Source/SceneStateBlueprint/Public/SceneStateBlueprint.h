...
class USceneStateBlueprint : public UBlueprint, public ISceneStateSchemaProvider, public ISceneStateBindingCollectionOwner
{
	GENERATED_BODY()
...
public:
	...
	/** Sets the scene state schema to use for this blueprint */
	UE_API void SetSceneStateSchema(TSubclassOf<USceneStateSchema> InSchemaClass);

	//~ Begin ISceneStateSchemaProvider
	UE_API virtual TSubclassOf<USceneStateSchema> GetSceneStateSchemaClass() const override;
	//~ End ISceneStateSchemaProvider
...
private:
	...
	/** Called when a state's parameters have changed */
	void OnStateParametersChanged(USceneStateMachineStateNode* InStateNode);

	/** Called when parameters have changed. Used to fix the path to parameters that have possibly been renamed */
	void UpdateParametersBindings(FPropertyBindingPath& InPath, const FGuid& InParametersId, const FInstancedPropertyBag& InParameters);

	/** Scene State schema class to use for this blueprint */
	UPROPERTY(VisibleAnywhere, Category="Scene State", AssetRegistrySearchable)
	TSubclassOf<USceneStateSchema> SceneStateSchemaClass;
	...
	/** Handle to the delegate when a state parameters have changed */
