...
void CreateBlueprintVariables(USceneStateBlueprint* InBlueprint, TArrayView<UE::PropertyBinding::FPropertyCreationDescriptor> InPropertyCreationDescs)
{
	...
	for (UE::PropertyBinding::FPropertyCreationDescriptor& CreationDesc : InPropertyCreationDescs)
	{
		...
		{
			const FEdGraphPinType SourcePropertyType = GetPropertyDescAsPin(FPropertyBagPropertyDesc(CreationDesc.SourceProperty->GetFName(), CreationDesc.SourceProperty));
			// Only copy default if the values are the exact same
			if (SourcePropertyType == VariableType)
			{
