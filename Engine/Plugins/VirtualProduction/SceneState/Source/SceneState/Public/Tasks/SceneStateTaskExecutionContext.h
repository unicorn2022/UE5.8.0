...
{
	UE_API FTaskExecutionContext();

	UE_API explicit FTaskExecutionContext(const FSceneStateTask& InTask, const FSceneStateExecutionContext& InContext);

	/** Resolves the execution context if it still exists */
	UE_API const FSceneStateExecutionContext* GetExecutionContext() const;
	...
	UE_API FConstStructView GetTask() const;
