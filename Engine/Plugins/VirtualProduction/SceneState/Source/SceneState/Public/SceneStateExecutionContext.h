...
struct FSceneStateExecutionContext
{
	...
	/** Gets the context handle for this execution context, if any */
	UE::SceneState::FExecutionContextHandle GetContextHandle() const;
	...
	UE_API USceneStateObject* GetRootObject() const;
	...
	struct FSetupParams
	{
		...
		USceneStateObject* RootObject = nullptr;
	...
	void ForEachTask(const FSceneState& InState, TFunctionRef<UE::SceneState::EIterationResult(const FSceneStateTask&, uint16 /*RelativeIndex*/)> InCallable) const;
	...
	/** Iterates the active state instances of this context */
	void ForEachStateInstance(TFunctionRef<void(uint16, const FSceneStateInstance&)> InFunc) const;
	...
	/** Gets the task index relative to the other tasks in the state */
	uint16 FindTaskRelativeIndex(const FSceneStateTask& InTask) const;

	/** Returns the Task Instance of this context for the given task. Returns an invalid view if not found */
	UE_INTERNAL UE_API FStructView FindTaskInstance(const FSceneStateTask& InTask) const;

	/** Returns the Task Instance of this context for the given task absolute index. Returns an invalid view if not found */
	FStructView FindTaskInstance(uint16 InTaskAbsoluteIndex) const;

	/** Returns the given task's execution status, or unset if task was not found */
	UE_INTERNAL UE_API TOptional<UE::SceneState::EExecutionStatus> GetTaskStatus(const FSceneStateTask& InTask) const;

	/** Set the task status of this context for the given task */
	void SetTaskStatus(const FSceneStateTask& InTask, UE::SceneState::EExecutionStatus InStatus) const;
	...
	/** Called to set the status of the given state */
	void SetStateStatus(const FSceneState& InState, FSceneStateInstance& InStateInstance, UE::SceneState::EExecutionStatus InStatus) const;
...
private:
	...
	/** Pointer to the root object owning this execution */
	UPROPERTY()
	TObjectPtr<USceneStateObject> RootObject;
	...
	/** Context handle to this execution. */
	UE::SceneState::FExecutionContextHandle ContextHandle;
...
/** FSceneStateExecutionContext is not trivially relocatable */
template<UE::CDerivedFrom<FSceneStateExecutionContext> T>
