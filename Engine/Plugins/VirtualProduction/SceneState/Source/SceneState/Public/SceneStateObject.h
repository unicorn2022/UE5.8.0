...
#if WITH_EDITOR
	/** Gets the currently attached debugger, if any */
	UE_INTERNAL UE_API TSharedPtr<UE::SceneState::FDebugger> GetDebugger() const;

	/** Attaches a new debugger, detaching the old one if any */
	void SetDebugger(const TWeakPtr<UE::SceneState::FDebugger>& InDebuggerWeak);
#endif
...
#if WITH_EDITOR
	/** Debugger attached to this object */
	TWeakPtr<UE::SceneState::FDebugger> DebuggerWeak;
#endif
};
