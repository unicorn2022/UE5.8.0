...
FTaskExecutionContext::FTaskExecutionContext(const FSceneStateTask& InTask, const FSceneStateExecutionContext& InContext)
	: ContextHandle(InContext.GetContextHandle())
...
void FTaskExecutionContext::FinishTask() const
{
	...
	if (!ExecutionContext)
	{
		UE_LOGF(LogSceneState, Error, "[%ls] %ls Invalid Execution Context !"
			, *GetDebugString()
			, BaseErrorMessage)
		return;
	}
	...
	if (!Task)
	{
		UE_LOGF(LogSceneState, Error, "[%ls in Context '%ls'] %ls Task not found!"
			, *GetDebugString()
			, BaseErrorMessage
	...
	{
		UE_LOGF(LogSceneState, Error, "[%ls in Context '%ls'] %ls Task Instance not found!"
			, *GetDebugString()
			, *ExecutionContext->GetExecutionContextName()
			, BaseErrorMessage)
		return;
	...
	if (!Private::IsMatchingInstanceId(*ExecutionContext, StateIndex, StateInstanceId))
	{
		UE_LOGF(LogSceneState, Error, "[%ls in Context '%ls'] %ls Task Context outdated!"
			, *GetDebugString()
			, *ExecutionContext->GetExecutionContextName()
			, BaseErrorMessage)
		return;
