...
void UAvaDataLinkRCProcessor::OnProcessOutput(const FDataLinkExecutor& InExecutor, FConstStructView InOutputDataView)
{
	...
	if (!Preset)
	{
		UE_LOGF(LogAvaDataLink, Error, "[%.*ls] Data Link execution finished, but Remote Control is invalid!"
	...
	{
		UE_LOGF(LogAvaDataLink, Error, "[%.*ls] Data Link execution finished, but Remote Control '%ls' has invalid Controller Container!"
			, InExecutor.GetContextName().Len()
			, InExecutor.GetContextName().GetData()
			, *Preset->GetName());
		return;
	...
	{
		UE_LOGF(LogAvaDataLink, Error, "[%.*ls] Data Link execution finished, but Remote Control '%ls' has invalid Controller Container Data View!"
			, InExecutor.GetContextName().Len()
			, InExecutor.GetContextName().GetData()
			, *Preset->GetName());
		return;
	...
	if (const FJsonObjectWrapper* OutputJson = InOutputDataView.GetPtr<const FJsonObjectWrapper>())
	{
		...
		{
			UE_LOGF(LogAvaDataLink, Warning, "[%.*ls] Data Link output could not be applied to controllers. Json Object was not valid!"
				, InExecutor.GetContextName().Len()
				, InExecutor.GetContextName().GetData());
			return;
		}
	...
	else
	{
		ForEachResolvedController(InExecutor, Preset, TargetDataView,
			[&InOutputDataView, &ModifiedControllers, &InExecutor](const FResolvedController& InResolvedController)
			{
				...
				{
					UE_LOGF(LogAvaDataLink, Warning, "[%.*ls] Data Link output field name '%ls' could not be applied: %ls"
						, InExecutor.GetContextName().Len()
						, InExecutor.GetContextName().GetData()
				...
				{
					UE_LOGF(LogAvaDataLink, Warning, "[%.*ls] Data Link output '%ls' could not be applied to controller '%ls' as types are incompatible"
						, InExecutor.GetContextName().Len()
						, InExecutor.GetContextName().GetData()
						, *InResolvedController.Mapping->OutputFieldName
...
void UAvaDataLinkRCProcessor::ForEachResolvedController(const FDataLinkExecutor& InExecutor, URemoteControlPreset* InPreset, FStructView InTargetDataView, TFunctionRef<void(const FResolvedController&)> InFunction)
{
	for (const FAvaDataLinkControllerMapping& Mapping : ControllerMappings)
	{
		...
		{
			UE_LOGF(LogAvaDataLink, Warning, "[%.*ls] Data Link output '%ls' could not be applied to controller '%ls'. Controller was not found in preset '%ls'!"
				, InExecutor.GetContextName().Len()
				, InExecutor.GetContextName().GetData()
		...
		{
			UE_LOGF(LogAvaDataLink, Warning, "[%.*ls] Data Link output '%ls' could not be applied to controller '%ls'. Controller property '%ls' in preset '%ls' was not found!"
				, InExecutor.GetContextName().Len()
				, InExecutor.GetContextName().GetData()
				, *Mapping.OutputFieldName
				, *Mapping.TargetController.Name.ToString()
