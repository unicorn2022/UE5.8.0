...
{
	DisplayType = EViewportWidgetOverlay_DisplayType::PostProcessSceneViewExtension;
}
...
void UVCamOutputRemoteSession::CreateRemoteSession()
{
	if (!IsInitialized())
	{
		UE_LOGF(LogVCamOutputProvider, Warning, "CreateRemoteSession has been called, but has not been initialized yet");
		return;
	}
	...
	if (!RemoteSessionHost.IsValid())
	{
		if (IRemoteSessionModule* RemoteSession = FModuleManager::LoadModulePtr<IRemoteSessionModule>("RemoteSession"))
		{
			...
			{
				UE_LOGF(LogVCamOutputProvider, Error, "Unable to create a RemoteSession host on port %d. Check the port is not in use.", PortNumber);
			}			
		}
	}
	else
	{
		UE_LOGF(LogVCamOutputProvider, Warning, "CreateRemoteSession has been called, but there is already a valid RemoteSessionHost!");
	}
}
...
void UVCamOutputRemoteSession::OnImageChannelCreated(TWeakPtr<IRemoteSessionChannel> Instance)
{
	...
	if (ImageChannel)
	{
		...
		if (FinalOutputRenderTarget)
		{
			...
			UE_LOGF(LogVCamOutputProvider, Log, "ImageChannel callback - MediaCapture set with TextureRenderTarget2D");
		}
		else
		{
			...
			if (TSharedPtr<FSceneViewport> PinnedSceneViewport = SceneViewport.Pin())
			{
				...
				UE_LOGF(LogVCamOutputProvider, Log, "ImageChannel callback - MediaCapture set with viewport");
			}
		}
	}
}
...
void UVCamOutputRemoteSession::OnInputChannelCreated(TWeakPtr<IRemoteSessionChannel> Instance)
{
	...
	if (InputChannel)
	{
		...
		if (GetUMGClass() && GetUMGWidget()) 
		{
			...
			checkf(UViewportWidgetOverlay::DoesDisplayTypeUsePostProcessSettings(DisplayType), TEXT("DisplayType not set up correctly in constructor!"));
			InputWindow = GetUMGWidget()->GetPostProcessDisplayTypeSettingsFor(DisplayType)->GetSlateWindow();
			UE_LOGF(LogVCamOutputProvider, Log, "InputChannel callback - Routing input to active viewport with UMG");
		...
		else
		{
			...
			UE_LOGF(LogVCamOutputProvider, Log, "InputChannel callback - Routing input to active viewport");
		}
	}
}
