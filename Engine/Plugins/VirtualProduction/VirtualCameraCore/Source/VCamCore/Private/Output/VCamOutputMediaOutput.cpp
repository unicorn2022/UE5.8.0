...
{
	DisplayType = EViewportWidgetOverlay_DisplayType::PostProcessWithBlendMaterial;
}
	...
	if (OutputConfig)
	{
		...
		if (MediaCapture)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogVCamOutputProvider, Warning, "MediaOutput mode - failed to find valid SceneViewport");
				}
			}
		}
		else
		{
			UE_LOGF(LogVCamOutputProvider, Warning, "MediaOutput mode - failed to create MediaCapture");
		}
	}
	else
	{
		UE_LOGF(LogVCamOutputProvider, Warning, "MediaOutput mode - missing valid OutputConfig");
	}
}
