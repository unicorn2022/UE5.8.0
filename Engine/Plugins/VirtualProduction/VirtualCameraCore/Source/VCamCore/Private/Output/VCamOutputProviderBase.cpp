...
namespace UE::VCamCore
{
	...
	static bool ValidateOverlayClassAndLogErrors(const TSubclassOf<UUserWidget>& InUMGClass)
	{
		...
		UE_CLOGF(!bHasCorrectClassFlags, LogVCamOutputProvider, Warning, "Class %ls cannot be deprecated nor abstract", *InUMGClass->GetPathName());
...
void UVCamOutputProviderBase::SetActiveInternal(const bool bInActive)
{
	...
	else
	{
		...
		UE_LOGF(LogVCamOutputProvider,
			   Warning,
			   "SetActive: Owning VCamComponent is not enabled or initialized. Call SetEnabled(true) on the owning VCamComponent. Output provider bIsActive was set to true but the activation logic was skipped; it will run once you initialize the owning VCamComponent. Output provider: %ls",
			   *GetPathName()
		);
	}
}
...
void UVCamOutputProviderBase::CreateUMG()
{
	...
	if (UMGWidget)
	{
		UE_LOGF(LogVCamOutputProvider, Error, "CreateUMG widget already set - failed to create");
		return;
	}
	...
	UMGWidget = NewObject<UViewportWidgetOverlay>(this, UViewportWidgetOverlay::StaticClass());
	...
	if (UMGWidget->GetDisplayType(GetWorld()) == EViewportWidgetOverlay_DisplayType::PostProcessSceneViewExtension)
	{
	...
	UE_LOGF(LogVCamOutputProvider, Log, "CreateUMG widget named %ls from class %ls", *UMGWidget->GetName(), *UMGWidget->GetWidgetClass()->GetName());
}
...
void UVCamOutputProviderBase::DisplayUMG()
{
	if (UMGWidget)
	{
		if (UWorld* ActorWorld = GetWorld())
		{
			...
			UE_CLOGF(DisplayType == EViewportWidgetOverlay_DisplayType::PostProcessWithBlendMaterial && !Client, LogVCamOutputProvider, Error, "Failed to find viewport client. The widget will not be rendered.");
			if (DisplayType == EViewportWidgetOverlay_DisplayType::PostProcessWithBlendMaterial && Client)
			{
...
void UVCamOutputProviderBase::DestroyUMG()
{
	if (UMGWidget)
	{
		if (UMGWidget->IsDisplayed())
		{
			...
			FLevelEditorViewportClient* Client = GetTargetLevelViewportClient();
			if (DisplayType == EViewportWidgetOverlay_DisplayType::PostProcessWithBlendMaterial && Client && ModifyViewportPostProcessSettingsDelegateHandle.IsValid())
			{
			...
			UMGWidget->Hide();
			UE_LOGF(LogVCamOutputProvider, Log, "DestroyUMG widget %ls hidden", *UMGWidget->GetName());
		}
		UE_LOGF(LogVCamOutputProvider, Log, "DestroyUMG widget %ls destroyed", *UMGWidget->GetName());
...
bool UVCamOutputProviderBase::NeedsForceLockToViewport() const
{
	...
	return DisplayType == EViewportWidgetOverlay_DisplayType::PostProcessWithBlendMaterial;
}
