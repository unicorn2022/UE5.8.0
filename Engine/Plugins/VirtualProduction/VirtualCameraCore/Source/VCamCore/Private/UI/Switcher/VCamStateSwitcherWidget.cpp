...
bool UVCamStateSwitcherWidget::SetCurrentState(FName NewState, bool bForceUpdate, bool bReinitializeConnections)
{
	...
	if (!StateConfig)
	{
		UE_LOGF(LogVCamStateSwitcher, Warning, "Unknown connection state %ls", *NewState.ToString());
		return false;
	}
	...
	for (int32 i = 0; i < StateConfig->WidgetConfigs.Num(); ++i)
	{
		...
		if (!Widget)
		{
			UE_CLOGF(!WidgetConfig.HasNoWidgetSet(), LogVCamStateSwitcher, Warning, "Failed to find widget at index %d in state %ls", i, *NewState.ToString());
			continue;
		}
