...
bool UVCamWidget::ReinitializeConnections()
{
	...
	{
		UE_LOGF(LogVCamCore, Error, "ReinitializeConnections failed because VCamComponet is not set (for widget %ls of class %ls)", *GetPathName(), *GetClass()->GetPathName());
		return false;
	}
	...
	for (TPair<FName, FVCamConnection>& Connection : Connections)
	{
		...
		if (VCamConnection.ConnectionTargetSettings.HasValidSettings())
		{
			...
			{
				UE_LOGF(LogVCamConnection, Warning, "Widget %ls: Failed to create for VCam Connection with Connection Name: %ls", *GetName(), *ConnectionName.ToString());	
			}
		}
		else
		{
