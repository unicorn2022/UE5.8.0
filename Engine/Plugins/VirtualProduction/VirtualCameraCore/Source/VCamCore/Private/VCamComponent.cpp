...
void UVCamComponent::CheckForErrors()
{
	...
	if (!HasAnyFlags(RF_BeginDestroyed|RF_FinishDestroyed))
	{
		if (!GetTargetCamera())
		{
			UE_LOGF(LogVCamComponent, Error, "Attached Parent should be a CineCamera derived component.");
		}
	}
}
...
void UVCamComponent::Update()
{
	...
	if (bDisableComponentWhenSpawnedBySequencer)
	{
		...
		{
			UE_LOGF(LogVCamComponent, Warning, "%ls was spawned by Sequencer. Disabling the component because \"Disable Component When Spawned By Sequencer\" was true.", *GetFullName(OwningActor->GetOuter()));
			SetEnabled(false);
			return;
		}
	}
...
bool UVCamComponent::AddModifier(const FName Name, const TSubclassOf<UVCamModifier> ModifierClass, UVCamModifier*& CreatedModifier)
{
	...
	{
		UE_LOGF(LogVCamComponent, Warning, "Unable to add Modifier to Stack as another Modifier with the name \"%ls\" exists", *Name.ToString());
		return false;
	}
...
bool UVCamComponent::InsertModifier(const FName Name, int32 Index, const TSubclassOf<UVCamModifier> ModifierClass, UVCamModifier*& CreatedModifier)
{
	...
	{
		UE_LOGF(LogVCamComponent, Warning, "Insert Modifier failed with invalid index %d for stack of size %d.", Index, ModifierStack.Num());
		return false;
	}
...
bool UVCamComponent::SetModifierIndex(int32 OriginalIndex, int32 NewIndex)
{
	...
	{
		UE_LOGF(LogVCamComponent, Warning, "Set Modifier Index failed as the Original Index, %d, was out of range for stack of size %d", OriginalIndex, ModifierStack.Num());
		return false;
	}

	if (!ModifierStack.IsValidIndex(NewIndex))
	{
		UE_LOGF(LogVCamComponent, Warning, "Set Modifier Index failed as the New Index, %d, was out of range for stack of size %d", NewIndex, ModifierStack.Num());
		return false;
	}
...
bool UVCamComponent::InsertOutputProvider(int32 Index, TSubclassOf<UVCamOutputProviderBase> ProviderClass, UVCamOutputProviderBase*& CreatedProvider)
{
	...
	{
		UE_LOGF(LogVCamComponent, Warning, "Insert Output Provider failed with invalid index %d for stack of size %d.", Index, OutputProviders.Num());
		return false;
	}
...
bool UVCamComponent::SetOutputProviderIndex(int32 OriginalIndex, int32 NewIndex)
{
	...
	{
		UE_LOGF(LogVCamComponent, Warning, "Set Output Provider Index failed as the Original Index, %d, was out of range for stack of size %d", OriginalIndex, OutputProviders.Num());
		return false;
	}

	if (!OutputProviders.IsValidIndex(NewIndex))
	{
		UE_LOGF(LogVCamComponent, Warning, "Set Output Provider Index failed as the New Index, %d, was out of range for stack of size %d", NewIndex, OutputProviders.Num());
		return false;
	}
...
void UVCamComponent::ApplyInputProfile()
{
	...
	for (const TPair<FName, FKey>& MappableKeyOverride : InputProfile.MappableKeyOverrides)
	{
		...
		UE_CLOGF(!FailureReason.IsEmpty(), LogVCamComponent, Warning, "Failed to map %ls. Possible Reasons: "
...
void UVCamComponent::SetupVCamSystemsIfNeeded()
{
	if (UE::VCamCore::CanInitVCamInstance(this))
	{
		if (!InputComponent)
		{
			...
			// During PIE world duplication (StaticDuplicateObjectEx), ConditionalPostLoad may run on
			// this component before VCamInput0's ConditionalPostLoad has cleared RF_NeedPostLoad.
			// NewObject with the same name would trigger StaticAllocateObject's replacement path,
			// which asserts on RF_NeedPostLoad. Adopt the existing subobject instead.
			InputComponent = FindObject<UInputComponent>(this, TEXT("VCamInput0"));
			if (!InputComponent)
			{
		...
		{
			UE_LOGF(LogVCamComponent, Error, "LiveLink is not available. Some VCamCore features may not work as expected");
		}
...
void UVCamComponent::ValidateModifierStack(const FString BaseName /*= "NewModifier"*/)
{
	...
	else
	{
		...
		if (bIsDuplicate)
		{
			...
			UE_LOGF(LogVCamComponent, Warning, "Unable to set Modifier Name to \"%ls\" as it is already in use. Resetting Name to previous value \"%ls\"",
