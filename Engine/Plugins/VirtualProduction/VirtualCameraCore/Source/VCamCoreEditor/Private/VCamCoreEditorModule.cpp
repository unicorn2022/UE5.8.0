...
namespace UE::VCamCoreEditor
{
	...
	void FVCamCoreEditorModule::StartupModule()
	{
		...
		FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FVCamCoreEditorModule::RegisterMultiUserFilters);
	}
	...
	void FVCamCoreEditorModule::RegisterConnectionRemapCustomization(TSubclassOf<UVCamWidget> Class, FGetConnectionRemappingCustomization GetterDelegate)
	{
		...
		{
			UE_LOGF(LogVCamEditor,
				Error,
				"RegisterConnectionTargetRemappingCustomizer is designed for native C++ base classes."
					"Blueprints are untested and will likely break due to recompilation and reinstancing."
					"'%ls' is will not be registered!",
				*Class->GetName()
				);
			return;
		}
		...
		{
			UE_LOGF(LogVCamEditor, Error, "Class '%ls' is already registered!", *Class->GetName());
			return;
		}
	...
	void FVCamCoreEditorModule::UnregisterConnectionRemapCustomization(TSubclassOf<UVCamWidget> Class)
	{
		...
		if (bIsInvalidClass)
		{
			UE_LOGF(LogVCamEditor, Error, "Class '%ls' cannot be unregistered!", Class ? *Class->GetName() : TEXT("null"));
			return;
		}
	...
	uint32 FVCamCoreEditorModule::GetAdvancedAssetCategoryForVCam() const
	{
		...
		return AssetTools.RegisterAdvancedAssetCategory("VirtualProduction", LOCTEXT("VirtualProductionCategory", "Virtual Production"));
	}
