...
namespace UE::PixelStreamingVCam
{
	...
	void FVCamPixelStreamingSessionLogic::OnActivate(DecoupledOutputProvider::IOutputProviderEvent& Args)
	{
		...
		if (MediaOutput)
		{
			UE_LOGF(LogPixelStreamingVCam, Log, "Activating PixelStreaming VCam Session. Endpoint: %ls", *MediaOutput->GetStreamer()->GetSignallingServerURL());
	...
	{
		UE_LOGF(LogPixelStreamingVCam, Log, "Create new media capture for Pixel Streaming VCam.");
	...
	void FVCamPixelStreamingSessionLogic::StartCapture(TWeakObjectPtr<UVCamPixelStreamingSession> WeakThisUObjectPtr)
	{
		...
		if (UTextureRenderTarget2D* FinalOutput = WeakThisUObjectPtr->GetFinalOutputRenderTarget())
		{
			...
			UE_LOGF(LogPixelStreamingVCam, Log, "PixelStreaming set with CaptureTextureRenderTarget2D");
		}
		else
		{
			...
			if (TSharedPtr<FSceneViewport> PinnedSceneViewport = SceneViewport.Pin())
			{
				...
				UE_LOGF(LogPixelStreamingVCam, Log, "PixelStreaming set to capture scene viewport.");
			}
		}
	}
	...
	void FVCamPixelStreamingSessionLogic::SetupCustomInputHandling(UVCamPixelStreamingSession* This)
	{
		if (This->GetUMGWidget())
		{
			...
			checkf(UViewportWidgetOverlay::DoesDisplayTypeUsePostProcessSettings(EViewportWidgetOverlay_DisplayType::PostProcessSceneViewExtension), TEXT("DisplayType not set up correctly in constructor!"));
			InputWindow = This->GetUMGWidget()->GetPostProcessDisplayTypeSettingsFor(EViewportWidgetOverlay_DisplayType::PostProcessSceneViewExtension)->GetSlateWindow();
			UE_LOGF(LogPixelStreamingVCam, Log, "InputChannel callback - Routing input to active viewport with UMG");
		...
		else if (ensure(MediaOutput))
		{
			...
			MediaOutput->GetStreamer()->SetInputHandlerType(EPixelStreamingInputType::RouteToWidget);
			UE_LOGF(LogPixelStreamingVCam, Log, "InputChannel callback - Routing input to active viewport");
		}
		...
		if (MediaOutput)
		{
			...
			const IPixelStreamingInputHandler::CommandHandlerFn ControlRequestHandler = [this](FString PlayerId, FString Descriptor, FString ForceString)
			{
				...
				if (bForceControl)
				{
					UE_LOGF(LogPixelStreamingVCam, Log, "Player %ls forced control change", *PlayerId);
				}

				if (!TryTakeTransformControl(PlayerId, bForceControl))
				{
					UE_LOGF(LogPixelStreamingVCam, Log, "Player %ls requested control, but was denied", *PlayerId);
		...
		{
			UE_LOGF(LogPixelStreamingVCam, Error, "Failed to setup custom input handling.");
		}
	}
	...
	void FVCamPixelStreamingSessionLogic::OnCaptureStateChanged(TWeakObjectPtr<UVCamPixelStreamingSession> WeakThisUObjectPtr)
	{
		...
		case EMediaCaptureState::Capturing:
			UE_LOGF(LogPixelStreamingVCam, Log, "Starting media capture for Pixel Streaming VCam.");
			break;
		case EMediaCaptureState::Stopped:
			if (MediaCapture->WasViewportResized())
			{
				UE_LOGF(LogPixelStreamingVCam, Log, "Pixel Streaming VCam capture was stopped due to resize, going to restart capture.");
				// If it was stopped and viewport resized we assume resize caused the stop, so try a restart of capture here.
				SetupCapture(WeakThisUObjectPtr);
			}
			else
			{
				UE_LOGF(LogPixelStreamingVCam, Log, "Stopping media capture for Pixel Streaming VCam.");
			}
			break;
		case EMediaCaptureState::Error:
			if (MediaCapture->WasViewportResized())
			{
				UE_LOGF(LogPixelStreamingVCam, Log, "Pixel Streaming VCam capture hit an error due to resize, going to restart capture.");
				// If it was stopped and viewport resized we assume resize caused the error, so try a restart of capture here.
				SetupCapture(WeakThisUObjectPtr);
			}
			else
			{
				UE_LOGF(LogPixelStreamingVCam, Error, "Pixel Streaming VCam capture hit an error, capturing will stop.");
			}
			break;
		default:
			break;
	...
	void FVCamPixelStreamingSessionLogic::SetTransformControlHolder(const TOptional<FString>& NewPlayerId)
	{
		...
		{
			UE_LOGF(LogPixelStreamingVCam, Log, "Player %ls has released control", *TransformControlHolder.GetValue());
			SendTransformControlStatus(TransformControlHolder.GetValue(), false);
		}

		if (NewPlayerId.IsSet())
		{
			UE_LOGF(LogPixelStreamingVCam, Log, "Player %ls has taken control", *NewPlayerId.GetValue());
