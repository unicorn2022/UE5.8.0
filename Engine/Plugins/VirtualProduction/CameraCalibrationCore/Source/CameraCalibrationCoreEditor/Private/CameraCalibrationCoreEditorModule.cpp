...
void FCameraCalibrationCoreEditorModule::RegisterPlacementModeItems()
{
	auto RegisterPlaceActors = [&]() -> void
	{
		...
		if (!Info)
		{
			UE_LOGF(LogCameraCalibrationCoreEditor, Warning, "Could not find or create VirtualProduction Place Actor Category");
			return;
		}
	...
	if (FApp::CanEverRender())
	{
		...
		{
			PostEngineInitHandle = FCoreDelegates::GetOnPostEngineInit().AddLambda(RegisterPlaceActors);
		}
	}
}
