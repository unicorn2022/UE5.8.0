...
/** Delegate for generating Charuco board textures without OpenCV dependency */
DECLARE_DELEGATE_RetVal_ThreeParams(UTexture2D*, FGenerateCharucoBoardDelegate, const FCharucoBoardConfigShadow&, FIntPoint, int32);
...
public:
	...
	/** Adds the input distortion state, rendering mode and blending parameters to the Lens Distortion Scene View Extension for the input camera actor. */
	UE_API void SetLensDistortionSVEState(ACameraActor* CameraActor, FDisplacementMapBlendingParams DistortionState, ULensDistortionModelHandlerBase* LensDistortionHandler, EDistortionRenderingMode InRenderingMode);
	...
	/** Set the delegate for generating Charuco board textures */
	UE_API void SetCharucoBoardTextureGenerator(FGenerateCharucoBoardDelegate InDelegate);

	/** Generate a Charuco board texture using the registered delegate (if available) */
	UE_API UTexture2D* GenerateCharucoBoardTexture(const FCharucoBoardConfigShadow& BoardConfig, FIntPoint ImageSize, int32 MarginSize = 10);
...
private:
	...
	/** Delegate for generating Charuco board textures without OpenCV dependency */
	FGenerateCharucoBoardDelegate CharucoBoardTextureGenerator;
