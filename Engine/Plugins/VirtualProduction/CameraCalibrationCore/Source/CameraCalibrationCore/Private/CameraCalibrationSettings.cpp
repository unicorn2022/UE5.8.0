...
UCameraCalibrationSettings::UCameraCalibrationSettings()
{
	...
	// Defaults priority such that the SVE runs before OpenColorIO and CompositeCore.
	SceneViewExtensionPriority = 100 + 10 /** OPENCOLORIO_SCENE_VIEW_EXTENSION_PRIORITY + 10 */;
	...
	PostEngineInitHandle = FCoreDelegates::GetOnPostEngineInit().AddLambda(InitOverlayOverrides);
#endif
}
...
void UCameraCalibrationSettings::PostEditChangeChainProperty(struct FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	const FProperty* MemberProperty = ActiveMemberNode ? ActiveMemberNode->GetValue() : nullptr;
	if (!MemberProperty || MemberProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UCameraCalibrationSettings, DisplacementMapResolution))
	{
		DisplacementMapResolutionChangedDelegate.Broadcast(DisplacementMapResolution);
	}
	if (!MemberProperty || MemberProperty->GetFName() == GET_MEMBER_NAME_CHECKED(UCameraCalibrationSettings, CalibrationInputTolerance))
	{
