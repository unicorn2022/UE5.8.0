...
namespace CameraCalibrationTestUtil
{
	...
	bool SafeGetNumberFromJson(const TSharedPtr<FJsonObject>& JsonObj, const FString& FieldName, float& OutValue, float DefaultValue = 0.0f)
	{
		if (!JsonObj.IsValid())
		{
			UE_LOGF(LogTemp, Warning, "Invalid JSON object when getting field '%ls'", *FieldName);
		...
		{
			UE_LOGF(LogTemp, Warning, "Field '%ls' is not a valid number in JSON, using default value %f", *FieldName, DefaultValue);
			OutValue = DefaultValue;
			return false;
		}
	}
	...
	bool SafeGetNumberFromJsonValue(const TSharedPtr<FJsonValue>& JsonValue, float& OutValue, float DefaultValue = 0.0f)
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "Invalid JSON value when parsing number");
			OutValue = DefaultValue;
			return false;
		}
		...
		{
			UE_LOGF(LogTemp, Warning, "JSON value is not a valid number, using default value %f", DefaultValue);
			OutValue = DefaultValue;
			return false;
		}
	}
	...
	bool SaveTestReference(const FString& TestName, const FString& SectionName, TConstArrayView<T> Values)
	{
		...
		{
			UE_LOGF(LogTemp, Log, "Saved test reference data to: %ls", *FilePath);
			return true;
		}
		else
		{
			UE_LOGF(LogTemp, Error, "Failed to save test reference data to: %ls", *FilePath);
			return false;
		}
	}
	...
	bool SaveTestSuite(const FString& TestName, const FTestSuite& TestSuite)
	{
		...
		if (FFileHelper::SaveStringToFile(OutputString, *FilePath))
		{
			UE_LOGF(LogTemp, Log, "Saved test suite to: %ls", *FilePath);
			return true;
		}
		else
		{
			UE_LOGF(LogTemp, Error, "Failed to save test suite to: %ls", *FilePath);
			return false;
		}
	}
	...
	void RunTestSuite(FAutomationTestBase& Test, const FString& TestName, ULensFile* ExternalLensFile, USphericalLensDistortionModelHandler* Handler)
	{
		...
		UE_LOGF(LogTemp, Log, "Running test suite '%ls' with %d test cases", *TestSuite.SuiteName, TestSuite.TestCases.Num());
