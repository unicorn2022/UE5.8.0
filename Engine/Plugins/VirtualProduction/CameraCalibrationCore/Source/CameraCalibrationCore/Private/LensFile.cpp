...
bool ULensFile::EvaluateDistortionParameters(float InFocus, float InZoom, FDistortionInfo& OutEvaluatedValue) const
{
	...
	FBlendParams Params;
	Params.bGenerateBlendedDistortionParams = true;
	Params.GetInterpolatedImageCenter = FBlendParams::FGetInterpolatedImageCenter::CreateUObject(this, &ULensFile::EvaluateImageCenterParameters);
	Params.GetInterpolatedFocalLength = FBlendParams::FGetInterpolatedFocalLength::CreateUObject(this, &ULensFile::EvaluateFocalLength);

	LensInfo.LensModel->GetDefaultObject<ULensModel>()->GetDefaultParameterArray(Params.DefaultDistortionParams);

	FBlendResults Results = LensInterpolationUtils::DistortionMapBlend(DistortionTable, InFocus, InZoom, Params);
	if (!Results.bValid || !Results.BlendedDistortionParams.IsSet())
	{
		return false;
	}
...
{	
	...
	{
		UE_LOGF(LogCameraCalibrationCore, Warning, "Can't evaluate LensFile '%ls' - Invalid Lens Handler", *GetName());
		return false;
	}
	
	if (InLensHandler->GetUndistortionDisplacementMap() == nullptr)
	{
		UE_LOGF(LogCameraCalibrationCore, Warning, "Can't evaluate LensFile '%ls' - Invalid undistortion displacement map in LensHandler '%ls'", *GetName(), *InLensHandler->GetName());
		return false;
	}

	if (InLensHandler->GetDistortionDisplacementMap() == nullptr)
	{
		UE_LOGF(LogCameraCalibrationCore, Warning, "Can't evaluate LensFile '%ls' - Invalid distortion displacement map in LensHandler '%ls'", *GetName(), *InLensHandler->GetName());
		return false;
	}

	if (LensInfo.LensModel == nullptr)
	{
		UE_LOGF(LogCameraCalibrationCore, Warning, "Can't evaluate LensFile '%ls' - Invalid Lens Model", *GetName());
	...
	{
		UE_LOGF(LogCameraCalibrationCore, Warning, "Can't evaluate LensFile '%ls' - LensHandler '%ls' doesn't support lens model '%ls'", *GetName(), *InLensHandler->GetName(), *LensInfo.LensModel.GetDefaultObject()->GetModelName().ToString());
		SetupNoDistortionOutput(InLensHandler);
		return false;
	}
...
void ULensFile::GetBlendState(float InFocus, float InZoom, FVector2D InFilmback, FDisplacementMapBlendingParams& OutBlendState)
{
	...
	if (Results.bValid && Results.BlendingParams.IsSet())
	{
...
bool ULensFile::EvaluateDistortionForParameters(float InFocus, float InZoom, FVector2D InFilmback, ULensDistortionModelHandlerBase* InLensHandler) const
{
	...
	EvaluateImageCenterParameters(InFocus, InZoom, InterpolatedState.ImageCenter);
	EvaluateFocalLength(InFocus, InZoom, InterpolatedState.FocalLengthInfo);
	InterpolatedState.FocalLengthInfo.FxFy *= FxFyScale;
	
	FBlendParams Params;
	Params.bGenerateBlendingParams = true;
	Params.bGenerateBlendedDistortionParams = true;
	Params.bGenerateDistortionMaps = true;
	Params.bGenerateBlendedOverscan = true;
	...
	Params.ProcessDisplacementMaps = FBlendParams::FProcessDisplacementMaps::CreateLambda(
		[InLensHandler, &CameraFilmback, &FxFyScale](const FLensDistortionState& InDistortionState, UTextureRenderTarget2D* InUndistortionMap, UTextureRenderTarget2D* InDistortionMap)
		{
			FLensDistortionState FinalDistortionState(InDistortionState);
			FinalDistortionState.FocalLengthInfo.FxFy *= FxFyScale;

			InLensHandler->SetDistortionState(FinalDistortionState);
			InLensHandler->SetCameraFilmback(CameraFilmback);
			
			InLensHandler->DrawUndistortionDisplacementMap(InUndistortionMap);
			InLensHandler->DrawDistortionDisplacementMap(InDistortionMap);
		}
	);

	Params.GetOverscan = FBlendParams::FGetOverscan::CreateLambda(
		[InLensHandler, &CameraFilmback, &FxFyScale]
		(const FDistortionFocusPoint& FocusPoint, float Zoom, const FLensDistortionState& InDistortionState)
		{
			FLensDistortionState FinalDistortionState(InDistortionState);
			FinalDistortionState.FocalLengthInfo.FxFy *= FxFyScale;
	...
	if (!Results.bValid || !Results.BlendedDistortionParams.IsSet())
	{
		// We were unable to generate blended distortion parameters for some reason. Fallback to default distortion,
		// while keeping blended image center and focal length
	...
	InLensHandler->SetOverscanFactor(Results.BlendedOverscan.Get(1.0f));
...
bool ULensFile::EvaluateDistortionForSTMaps(float InFocus, float InZoom, FVector2D InFilmback, ULensDistortionModelHandlerBase* InLensHandler) const
{
	...
	using FBlendParams = LensInterpolationUtils::FDistortionMapBlendParams<FSTMapTable>;
	using FBlendResults = LensInterpolationUtils::FDistortionMapBlendResults;
	
	if(DerivedDataInFlightCount > 0)
	{
		UE_LOGF(LogCameraCalibrationCore, Verbose, "Can't evaluate LensFile '%ls' - %d data points still being computed. Clearing render target for no distortion", *GetName(), DerivedDataInFlightCount);
		SetupNoDistortionOutput(InLensHandler);
		return true;
	}
	...
	Params.GetOverscan = FBlendParams::FGetOverscan::CreateLambda(
		[this]
		(const FSTMapFocusPoint& FocusPoint, float Zoom, const FLensDistortionState& InDistortionState)
		{
			if (const FSTMapZoomPoint* ZoomPoint = FocusPoint.GetZoomPoint(Zoom))
			{
				return ComputeOverscan(ZoomPoint->DerivedDistortionData.DistortionData, InDistortionState.ImageCenter.PrincipalPoint);
			}

			return 1.0f;
		});
	
	FBlendResults Results = LensInterpolationUtils::DistortionMapBlend(STMapTable, InFocus, InZoom, Params);
	if (!Results.bValid || !Results.UndistortedMaps.IsSet() || !Results.DistortedMaps.IsSet())
	{
		UE_LOGF(LogCameraCalibrationCore, Verbose, "Can't evaluate LensFile '%ls' - No calibrated maps", *GetName());
		SetupNoDistortionOutput(InLensHandler);
		return true;
	}
	...
	BlendingParams.PrincipalPoint = InterpolatedState.ImageCenter.PrincipalPoint;
	...
	//Sets final blended distortion state
	InLensHandler->SetDistortionState(MoveTemp(InterpolatedState));
	InLensHandler->SetCameraFilmback(CameraFilmback);
...
void ULensFile::OnDistortionDerivedDataJobCompleted(const FDerivedDistortionDataJobOutput& JobOutput)
{
	...
	if(FSTMapFocusPoint* FocusPoint = STMapTable.GetFocusPoint(JobOutput.Focus))
	{
		if(FSTMapZoomPoint* ZoomPoint = FocusPoint->GetZoomPoint(JobOutput.Zoom))
		{
			...
			{
				UE_LOGF(LogCameraCalibrationCore, Warning, "Could not derive distortion data for calibrated map point with Focus = '%0.2f' and Zoom = '%0.2f' on LensFile '%ls'", FocusPoint->Focus, ZoomPoint->Zoom, *GetName());
			}
		}
	}
}
...
void ULensFile::PostInitProperties()
{
	...
	{
		// Enable ticking now that construction is complete and we're on the game thread.
		// Constructor uses ETickableTickType::Never because UObjects can be constructed on worker threads.
