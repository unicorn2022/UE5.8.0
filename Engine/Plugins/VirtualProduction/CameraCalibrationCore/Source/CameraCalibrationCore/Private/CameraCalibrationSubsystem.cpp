...
void UCameraCalibrationSubsystem::SetLensDistortionSVEState(ACameraActor* CameraActor, FDisplacementMapBlendingParams DistortionState, ULensDistortionModelHandlerBase* LensDistortionHandler, EDistortionRenderingMode InRenderingMode)
{
	SceneViewExtension->UpdateDistortionState_AnyThread(CameraActor, DistortionState, LensDistortionHandler, InRenderingMode);
}
...
void UCameraCalibrationSubsystem::SetCharucoBoardTextureGenerator(FGenerateCharucoBoardDelegate InDelegate)
{
	CharucoBoardTextureGenerator = InDelegate;
}

UTexture2D* UCameraCalibrationSubsystem::GenerateCharucoBoardTexture(const FCharucoBoardConfigShadow& BoardConfig, FIntPoint ImageSize, int32 MarginSize)
{
	if (CharucoBoardTextureGenerator.IsBound())
	{
		return CharucoBoardTextureGenerator.Execute(BoardConfig, ImageSize, MarginSize);
	}
	else
	{
		UE_LOGF(LogCameraCalibrationCore, Warning, "Charuco board texture generator not registered. OpenCV dependency may not be available.");
		return nullptr;
	}
}
