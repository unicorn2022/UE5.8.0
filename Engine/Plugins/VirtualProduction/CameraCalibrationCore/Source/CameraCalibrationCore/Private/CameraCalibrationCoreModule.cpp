...
void FCameraCalibrationCoreModule::ApplyStartupLensFile()
{
	auto ApplyLensFile = [this]()
	{
		...
		// Try to load from CVar
		{
			...
			if (LensFileName.Len())
			{
				...
				if (StartupLensFile)
				{
					UE_LOGF(LogCameraCalibrationCore, Display, "Loading Lens File specified in CVar CameraCalibration.StartupLensFile: '%ls'", *LensFileName);
				}
			}
		}
		...
		if (StartupLensFile == nullptr)
		{
			...
			if (StartupLensFile)
			{
				UE_LOGF(LogCameraCalibrationCore, Display, "Loading Lens File specified in user settings: '%ls'", *StartupLensFile->GetName());
			}
		}
#endif
		...
		if (StartupLensFile == nullptr)
		{
			...
			if (StartupLensFile)
			{
				UE_LOGF(LogCameraCalibrationCore, Display, "Loading Lens File specified in project settings: '%ls'", *StartupLensFile->GetName());
			}
		}
	...
	if (FApp::CanEverRender())
	{
		...
		{
			PostEngineInitHandle = FCoreDelegates::GetOnPostEngineInit().AddLambda(ApplyLensFile);
		}
	}
}
