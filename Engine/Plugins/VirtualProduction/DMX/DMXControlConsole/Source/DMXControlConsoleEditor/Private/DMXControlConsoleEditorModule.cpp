...
void FDMXControlConsoleEditorModule::StartupModule()
{
	...
	DMXEditorAssetCategory = AssetTools.RegisterAdvancedAssetCategory("VirtualProduction", LOCTEXT("VirtualProductionCategory", "Virtual Production"));

	FAssetCategoryPath VPCategory = FAssetCategoryPath(LOCTEXT("VirtualProductionCategory", "Virtual Production"));
	ControlConsoleCategory = FAssetCategoryPath(VPCategory, LOCTEXT("DMXCategorySection", "DMX"), ECategoryMenuType::Section);

	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FDMXControlConsoleEditorModule::OnPostEnginInit);
