...
bool FDMXZipper::GetData(TArray64<uint8>& OutData)
{
	...
	for (const TPair<FString, TPair<TArray64<uint8>, bool>>& Pair : NewFilesToShouldCompressMap)
	{
		...
		if (!bFileAdded)
		{
			UE_LOGF(LogDMXZip, Error, "Cannot save Zip. File '%ls' added no longer exists on disk!", *Pair.Key);
			return false;
		}
	}
	...
	for (const TPair<FString, uint32>& Pair : OffsetsMap)
	{
		...
		{
			UE_LOGF(LogDMXZip, Error, "Cannot save Zip.Failed to read File '%ls' from Zip when saving it.", *Pair.Key);
			return false;
		}
