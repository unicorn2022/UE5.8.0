...
namespace UE::DMX::GDTF
{
	void FDMXGDTFDescription::InitializeFromDescriptionXml(const TSharedRef<FXmlFile>& DescriptionXml)
	{
		...
		if (!RootNode)
		{
			UE_LOGF(LogDMXGDTF, Warning, "Cannot parse GDTF file. Description node is invalid.");
			return;
		}
		...
		{
			UE_LOGF(LogDMXGDTF, Warning, "Invalid GDTF Description.xml. Major Version is not numerical or not set.");
			return;
		}
		...
		{
			UE_LOGF(LogDMXGDTF, Warning, "Invalid GDTF Description.xml. Minor Version is not numerical or not set.");
			return;
		}
