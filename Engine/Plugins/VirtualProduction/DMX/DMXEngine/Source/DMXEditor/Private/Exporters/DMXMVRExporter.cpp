...
namespace UE::DMX
{
	...
	bool FDMXMVRExporter::ZipGeneralSceneDescription(const TSharedRef<FDMXZipper>& Zip, const UDMXMVRGeneralSceneDescription* GeneralSceneDescription, FText& OutErrorReason)
	{
		...
		{
			UE_LOGF(LogDMXEditor, Error, "Failed to save General Scene Description. See previous errors for details.");
			return false;
		}
	...
	bool FDMXMVRExporter::ZipGDTFs(const TSharedRef<FDMXZipper>& Zip, UDMXLibrary* DMXLibrary)
	{
		...
		for (UDMXEntityFixtureType* FixtureType : FixtureTypesToExport)
		{
			if (FixtureType->GDTFSource.IsNull() || FixtureType->bExportGeneratedGDTF)
			{
				...
				if (!XmlFile.IsValid())
				{
					UE_LOGF(LogDMXEditor, Warning, "Failed to create GDTF from Fixture Type '%ls'. See previous errors.", *FixtureType->Name);
				...
				if (FFileHelper::LoadFileToArray(DescriptionXmlData, *TempXmlFilename))
				{
					...
					{
						UE_LOGF(LogDMXEditor, Warning, "Failed to load temporary GDTF for Fixture Type '%ls'.", *FixtureType->Name);
						bAllZippedSuccessfully = false;
						continue;
					}
				}
				...
				{
					UE_LOGF(LogDMXEditor, Warning, "Failed to load temporary description.xml for Fixture Type '%ls'.", *FixtureType->Name);
					bAllZippedSuccessfully = false;
					continue;
				}
			}
			else
			{
				...
				if (!DMXImportGDTF)
				{
					UE_LOGF(LogDMXEditor, Warning, "Cannot export Fixture Type '%ls' to MVR, but the Fixture Type has a DMX Import Type which is not GDTF.", *FixtureType->Name);
					continue;
				}
				...
				else
				{
					...
					UE_LOGF(LogDMXEditor, Error, "Cannot export '%ls' to MVR File. The asset is missing source data.", GDTF ? *GDTF->GetName() : TEXT("Invalid GDTF Asset"));
				}
			}
		}
