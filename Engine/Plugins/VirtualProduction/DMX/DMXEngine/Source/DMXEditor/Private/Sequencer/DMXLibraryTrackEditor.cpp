...
int32 GetValidPatchMode(UDMXEntityFixturePatch* InPatch)
{
	...
	{
		UE_LOGF(DMXLibraryTrackEditorLog, Warning, "%s: Fixture Patch has null parent type", __FUNCTION__);
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(DMXLibraryTrackEditorLog, Warning, "%s: No active mode set in %ls. Patch will not be recorded.", __FUNCTION__, *InPatch->Name);
	}
...
void FDMXLibraryTrackEditor::HandlePatchSelectedFromAddMenu(UMovieSceneDMXLibraryTrack* Track, UDMXEntityFixturePatch* Patch)
{
	...
	if (ValidActiveMode == INDEX_NONE)
	{
		UE_LOGF(DMXLibraryTrackEditorLog, Warning, "%s: Fixture Patch has no Modes with functions in it", __FUNCTION__);
		return;
	}
