...
bool FDMXLibraryFromMVRImporter::LoadMVRFile(const FString& InFilename)
{
	...
	{
		UE_LOGF(LogDMXEditor, Error, "Cannot read MVR '%ls'. File is not a valid MVR.", *InFilename);
		return false;
	}
	...
	{
		UE_LOGF(LogDMXEditor, Error, "Cannot read General Scene Description from MVR '%ls'. File is not a valid MVR.", *Filename);
		return false;
	}
...
void FDMXLibraryFromMVRImporter::InitializeDMXLibrary(UDMXLibrary* DMXLibrary, const TArray<UDMXImportGDTF*>& GDTFAssets)
{
	...
	for (const UDMXMVRFixtureNode* FixtureNode : FixtureNodes)
	{
		...
		{
			UE_LOGF(LogDMXEditor, Warning, "Skipped creating a Fixture Patch for '%ls', cannot find Mode '%ls' in '%ls'.", *FixtureNode->Name, *FixtureNode->GDTFMode, *FixtureNode->GDTFSpec)
		}
	}
...
TArray<UDMXImportGDTF*> FDMXLibraryFromMVRImporter::CreateGDTFs(UObject* InParent, EObjectFlags InFlags, bool bReimportExisting)
{
	...
	for (const UDMXMVRFixtureNode* FixtureNode : FixtureNodes)
	{
		...
		if (!ImportedGDTFNames.Contains(FixtureNode->GDTFSpec))
		{
			...
			{
				UE_LOGF(LogDMXEditor, Warning, "Cannot find a GDTF file that corresponds to GDTF Spec '%ls' of Fixture Node '%ls'. Ignoring Fixture.", *FixtureNode->GDTFSpec, *FixtureNode->Name)
				{
					continue;
				}
			}
