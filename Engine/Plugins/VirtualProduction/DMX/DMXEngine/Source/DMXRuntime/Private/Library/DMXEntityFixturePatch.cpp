	...
	if (FixtureType)
	{
		...
		if (ensureMsgf(DMXLibrary, TEXT("Cannot create Fixture Patch when Fixture Type's DMX Library is invalid.")))
		{
			...
			if (ConstructionParams.MVRFixtureUUID.IsValid())
			{
				...
				{
					UE_LOGF(LogDMXRuntime, Error, "Trying to create Fixture Patch '%ls' with Unique MVR Fixture ID '%ls' in DMX Library '%ls'.", *DesiredName, *ConstructionParams.MVRFixtureUUID.ToString(), *DMXLibrary->GetName());
					UE_LOGF(LogDMXRuntime, Error, "However Fixture Patch '%ls' in the same Library already uses this Unique MVR Fixture ID.", *(*bOtherFixturePatchWithMVRFixtureUUIDPtr)->Name);
					UE_LOGF(LogDMXRuntime, Error, "Generating a new Unique MVR Fixture ID for the new Fixture Patch instead");
					ensureAlways(0);
...
{		
	...
	if (!ModePtr)
	{
		UE_LOGF(LogDMXRuntime, Error, "Tried to send DMX via Fixture Patch %ls, but its Parent Fixture Type has no Modes set up.", *GetName());
		return;
	}
...
{	
	/** DEPRECATED 4.27 */
	UE_LOGF(LogDMXRuntime, Error, "No clear remote Universe can be deduced in DMXEntityFixturePatch::GetRemoteUniverse. Returning 0.");
	return 0;
}
#endif // WITH_EDITOR
...
bool UDMXEntityFixturePatch::AreCoordinatesValid(const FDMXFixtureMatrix& FixtureMatrix, const FIntPoint& Coordinate, bool bLogged)
{
	...
	if (bLogged)
	{
		UE_LOGF(DMXEntityFixturePatchLog, Error, "Invalid X Coordinate for patch (requested %d, expected in range 0-%d).", Coordinate.X, FixtureMatrix.XCells - 1);
	}
