...
bool UDMXMVRRootNode::InitializeFromGeneralSceneDescriptionXml(const TSharedRef<FXmlFile>& GeneralSceneDescriptionXml)
{
	...
	if (!RootNode)
	{
		UE_LOGF(LogDMXRuntime, Warning, "Cannot parse MVR file. Root node is invalid.");
		return false;
	}
	...
	{
		UE_LOGF(LogDMXRuntime, Warning, "Invalid MVR GeneralSceneDescription.xml. Major Version is not set.");
		return false;
	}
	...
	{
		UE_LOGF(LogDMXRuntime, Warning, "Invalid MVR GeneralSceneDescription.xml. Minor Version is not set.");
		return false;
	}
