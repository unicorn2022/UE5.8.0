...
bool UDMXMVRFixtureNode::InitializeFromFixtureXmlNode(const FXmlNode& FixtureXmlNode)
{
	...
	{
		// Non-optional
		UE_LOGF(LogDMXRuntime, Warning, "Cannot parse Fixture Node as MVR Fixture Name. Failed to parse MVR Node %ls.", *FixtureXmlNode.GetTag());
		return false;
	}
	...
	{
		// Non-optional
		UE_LOGF(LogDMXRuntime, Warning, "Cannot parse Fixture '%ls' from MVR. Failed to parse MVR UUID.", *Name);
		return false;
	}
	...
	if (Matrix.IsSet() && !Matrix.GetValue().IsValid())
	{
		...
		UE_LOGF(LogDMXRuntime, Warning, "Imported Fixture '%ls' from MVR but its Transformation Matrix is not valid, using Identity instead.", *Name);
	...
	{
		// Non-optional
		UE_LOGF(LogDMXRuntime, Warning, "Cannot parse Fixture '%ls' from MVR. No valid GDTF Spec specified.", *Name);
		return false;
	}
	...
	{
		// Non-optional
		UE_LOGF(LogDMXRuntime, Warning, "Cannot parse Fixture '%ls' from MVR. No valid GDTF Mode specified.", *Name);
		return false;
	}
	...
	if (!ParseFixtureID(FixtureXmlNode, FixtureID))
	{
		...
		UE_LOGF(LogDMXRuntime, Warning, "Imported Fixture '%ls' from MVR. No valid Fixture Id specified. Fixture Id set to Fixture Name.", *Name);
		FixtureID = TEXT("");
	}
	...
	if (!ParseUnitNumber(FixtureXmlNode, UnitNumber))
	{
		...
		UE_LOGF(LogDMXRuntime, Warning, "Imported Fixture '%ls' from MVR. No valid Unit Number specified. Unit Number set to ''0'.", *Name);
		UnitNumber = 0;
	}
	...
	{
		// Non-optional
		UE_LOGF(LogDMXRuntime, Warning, "Cannot parse Fixture '%ls' from MVR. No valid Addresses specified.", *Name);
		return false;
	}
...
bool UDMXMVRFixtureNode::IsValid(bool bLogInvalidReason) const
{
	...
	if (!UUID.IsValid())
	{
		...
		{
			UE_LOGF(LogDMXRuntime, Warning, "Fixture '%ls' is not a valid MVR Fixture, its Fixture UUID is invalid.", *BeautifulName);
		}
		return false;
	}
	...
	if (GDTFSpec.IsEmpty())
	{
		if (bLogInvalidReason)
		{
			UE_LOGF(LogDMXRuntime, Warning, "Fixture '%ls' is not a valid MVR Fixture, its GDTFSpec is empty.", *BeautifulName);
		}
		return false;
	}
	...
	if (GDTFMode.IsEmpty())
	{
		if (bLogInvalidReason)
		{
			UE_LOGF(LogDMXRuntime, Warning, "Fixture '%ls' is not a valid MVR Fixture, its GDTFMode is empty.", *BeautifulName);
		}
		return false;
	}
	...
	{		
		if (bLogInvalidReason)
		{
			UE_LOGF(LogDMXRuntime, Warning, "Fixture '%ls' is not a valid MVR Fixture, its Universe is < 0 or larger than the Range supported by UE.", *BeautifulName);
		}
		return false;
	}
	...
	if (Addresses.Address < 1 || Addresses.Address > DMX_MAX_ADDRESS)
	{
		if (bLogInvalidReason)
		{
			UE_LOGF(LogDMXRuntime, Warning, "Fixture '%ls' is not a valid MVR Fixture, its Address is < 1 or > 512.", *BeautifulName);
		}
		return false;
	}
