...
AActor* ADMXMVRSceneActor::SpawnMVRActor(const TSubclassOf<AActor>&ActorClass, UDMXEntityFixturePatch* FixturePatch, const FTransform& Transform, AActor* Template)
{
	...
	{
		UE_LOGF(LogDMXRuntime, Warning, "Cannot spawn fixture for Fixture Patch '%ls'. Fixture Patch has no Fixture Type set.", *FixturePatch->Name);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDMXRuntime, Warning, "Cannot spawn MVR Fixture Actor of Class %ls, the Actor does not specifiy a root component.", *ActorClass->GetName());
