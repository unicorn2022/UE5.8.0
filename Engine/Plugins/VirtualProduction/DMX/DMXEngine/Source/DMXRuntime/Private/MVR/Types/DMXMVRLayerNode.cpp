...
void UDMXMVRLayerNode::InitializeFromLayerXmlNode(const FXmlNode& LayerXmlNode)
{
	...
	{
		UE_LOGF(LogDMXRuntime, Warning, "Found invalid UUID of Layer when parsing MVR File. Skipping Layer.")
		return;
	}
...
void UDMXMVRLayerNode::CreateXmlNodeInParent(FXmlNode& ParentNode) const
{
	...
	if (!UUID.IsValid())
	{
		UE_LOGF(LogDMXRuntime, Warning, "Cannot export MVR Layer '%ls'. Invalid UUID specified.", *NonOptionalName);
		return;
	}
