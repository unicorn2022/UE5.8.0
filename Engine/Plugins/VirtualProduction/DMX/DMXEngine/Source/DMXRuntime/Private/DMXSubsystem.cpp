...
void UDMXSubsystem::SendDMXToOutputPort(FDMXOutputPortReference OutputPortReference, TMap<int32, uint8> ChannelToValueMap, int32 LocalUniverse)
{
	...
	{
		UE_LOGF(DMXSubsystemLog, Error, "Unexpected: Cannot find DMX Port, failed sending DMX with node Send DMX To Outputport.");
	}
}
...
void UDMXSubsystem::GetDMXDataFromInputPort(FDMXInputPortReference InputPortReference, TArray<uint8>& DMXData, int32 LocalUniverse)
{
	...
	{
		UE_LOGF(DMXSubsystemLog, Error, "Unexpected: Cannot find DMX Port, failed reading DMX in node 'Get DMX Data from Input Port'.");
	}
}
...
void UDMXSubsystem::GetFixtureAttributes(const UDMXEntityFixturePatch* InFixturePatch, const TArray<uint8>& DMXBuffer, TMap<FDMXAttributeName, int32>& OutResult)
{
	...
	if (InFixturePatch != nullptr)
	{
		if (const UDMXEntityFixtureType* FixtureType = InFixturePatch->GetFixtureType())
		{
			...
			{
				UE_LOGF(DMXSubsystemLog, Error, "Tried to use Fixture Patch %ls, but its Fixture Type has no Modes set up.", *InFixturePatch->Name);
				return;
			}
		}
	}
...
EDMXFixtureSignalFormat SignalFormatFromBytesNum(uint32 InBytesNum)
{
	...
	{
	case 0:
		UE_LOGF(DMXSubsystemLog, Error, "%s called with InBytesNum = 0", __FUNCTION__);
	...
	default: // InBytesNum is 4 or higher
		UE_LOGF(DMXSubsystemLog, Warning, "%s called with InBytesNum > 4. Only 4 bytes will be used.", __FUNCTION__);
		return EDMXFixtureSignalFormat::E32Bit;
	}
}
...
float UDMXSubsystem::GetNormalizedAttributeValue(UDMXEntityFixturePatch* InFixturePatch, FDMXAttributeName InFunctionAttribute, int32 InValue) const
{
	if (InFixturePatch)
	{
		...
		{
			UE_LOGF(DMXSubsystemLog, Error, "%s: Cannot access the Mode of Fixture Patch %ls. Either it is of fixture type none, or the fixture type has no mode.", __FUNCTION__, *InFixturePatch->Name);
		}
	}
	else
	{
		UE_LOGF(DMXSubsystemLog, Error, "%s: InFixturePatch is invalid.", __FUNCTION__);
	}
