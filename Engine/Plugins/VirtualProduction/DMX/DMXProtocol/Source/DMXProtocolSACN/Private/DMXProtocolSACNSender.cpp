	...
	, bIsMulticast(bInIsMulticast)
{
	...
	if (bIsMulticast)
	{
		UE_LOGF(LogDMXProtocol, VeryVerbose, "Created sACN Multicast Sender at %ls", *NetworkInterfaceInternetAddr->ToString(false));
	}
	else
	{
		UE_LOGF(LogDMXProtocol, VeryVerbose, "Created sACN Sender at %ls sending to %ls", *NetworkInterfaceInternetAddr->ToString(false), *DestinationInternetAddr->ToString(false));
	}
}
...
FDMXProtocolSACNSender::~FDMXProtocolSACNSender()
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Destroyed sACN Sender at %ls sending to %ls", *NetworkInterfaceInternetAddr->ToString(false), *DestinationInternetAddr->ToString(false));
}
	...
	const FString& InUnicastIP)
{
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create sACN sender: Invalid IP address: %ls", *InNetworkInterfaceIP);
		return nullptr;
	}
	...
	if(!NewSocket)
	{
		UE_LOGF(LogDMXProtocol, Warning, "Invalid Network Interface IP %ls for DMX Port. Please update your Output Port in Project Settings -> Plugins -> DMX Plugin", *InNetworkInterfaceIP);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Invalid Unicast IP %ls for DMX Port. Please update your Output Port in Project Settings -> Plugins -> DMX Plugin", *InUnicastIP);
		return nullptr;
	}
	...
	const uint8 TTL)
{
	...
	UE_LOGF(LogDMXProtocol, Warning, "Cannot create a multicast sACN sender. Platform doesn't support UDP multicast groups.");
	return nullptr;
#else 
	...
	if(!NewSocket)
	{
		UE_LOGF(LogDMXProtocol, Warning, "Invalid Network Interface IP %ls for DMX Port. Please update your Output Ports in Project Settings -> Plugins -> DMX Plugin", *InNetworkInterfaceIP);
		return nullptr;
	}
...
void FDMXProtocolSACNSender::SendDMXSignal(const FDMXSignalSharedRef& DMXSignal)
{
	...
	else
	{
		if (!bErrorEverLogged)
		{
			...
			UE_LOGF(LogDMXProtocol, Error, "Socket subsystem failed sending DMX to %ls with Error Code %d", *DestinationInternetAddr->ToString(false), RecvFromError.GetValue());
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Incomplete DMX Packet sent to %ls", *DestinationInternetAddr->ToString(false));
		bErrorEverLogged = true;
	}
}
