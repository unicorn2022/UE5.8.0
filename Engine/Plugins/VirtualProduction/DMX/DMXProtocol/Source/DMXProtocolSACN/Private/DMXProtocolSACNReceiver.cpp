	...
	, Thread(nullptr)
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Created sACN Receiver at %ls", *EndpointInternetAddr->ToString(false));
}
...
FDMXProtocolSACNReceiver::~FDMXProtocolSACNReceiver()
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Destroyed sACN Receiver at %ls", *EndpointInternetAddr->ToString(false));
}
...
TSharedPtr<FDMXProtocolSACNReceiver> FDMXProtocolSACNReceiver::TryCreate(const TSharedPtr<FDMXProtocolSACN, ESPMode::ThreadSafe>& SACNProtocol, const FString& IPAddress)
{
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create sACN receiver: Invalid IP address: %ls", *IPAddress);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create sACN receiver: Error create ListeningSocket for: %ls", *IPAddress);
		return nullptr;
	}
	...
	if (!NewListeningSocket->SetIpPktInfo(true))
	{
		...
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create sACN receiver: Platform does not support IP_PKTINFO required for sACN.");
		return nullptr;
	}
...
void FDMXProtocolSACNReceiver::HandleDataPacket(uint16 UniverseID, const TSharedRef<FArrayReader>& PacketReader)
{
	...
	if (IncomingDMXDMPLayer.STARTCode == ACN_DMX_START_CODE_PER_ADDRESS_PRIORITY)
	{
		...
		{
			UE_LOGF(LogDMXProtocol, Warning, "Received DMX sACN packet(s) that specify a per address priority. This is not supported in the current Engine version.");
