...
bool FDMXProtocolSACN::RegisterInputPort(const TSharedRef<FDMXInputPort, ESPMode::ThreadSafe>& InputPort)
{
	...
	if (!Receiver.IsValid())
	{
		UE_LOGF(LogDMXProtocol, Warning, "Could not create sACN receiver for input port %ls", *InputPort->GetPortName());
...
TArray<TSharedPtr<IDMXSender>> FDMXProtocolSACN::RegisterOutputPort(const TSharedRef<FDMXOutputPort, ESPMode::ThreadSafe>& OutputPort)
{
	...
	else if (CommunicationType == EDMXCommunicationType::Unicast)
	{
		...
		{
			UE_LOGF(LogDMXProtocol, Warning, "Cannot create DMX Protocol sACN Sender for Output Port '%ls'. Port is set to Unicast, but does not specify any Destination Addresses.", *OutputPort->GetPortName());
		}
	...
	else
	{
		...
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create DMX Protocol sACN Sender. The communication type specified is not supported.");
	}
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Could not create sACN sender for Output Port '%ls'", *OutputPort->GetPortName());
	}
