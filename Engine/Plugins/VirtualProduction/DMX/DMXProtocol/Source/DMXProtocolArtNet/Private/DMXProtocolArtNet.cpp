...
bool FDMXProtocolArtNet::RegisterInputPort(const FDMXInputPortSharedRef& InputPort)
{
	...
	if (!Receiver.IsValid())
	{
		UE_LOGF(LogDMXProtocol, Warning, "Could not create Art-Net receiver for input port %ls", *InputPort->GetPortName());
...
TArray<TSharedPtr<IDMXSender>> FDMXProtocolArtNet::RegisterOutputPort(const FDMXOutputPortSharedRef& OutputPort)
{
	...
	{
		UE_LOGF(LogDMXProtocol, Error, "Cannot create DMX Protocol Art-Net Sender. The communication type specified is not supported.");
	}
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Could not create Art-Net sender for output port %ls", *OutputPort->GetPortName());
	}
