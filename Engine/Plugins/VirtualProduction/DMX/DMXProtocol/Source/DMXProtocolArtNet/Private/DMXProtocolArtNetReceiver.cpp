	...
	, Thread(nullptr)
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Created Art-Net Receiver at %ls", *EndpointInternetAddr->ToString(false));
}
...
FDMXProtocolArtNetReceiver::~FDMXProtocolArtNetReceiver()
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Destroyed Art-Net Receiver at %ls", *EndpointInternetAddr->ToString(false));
}
...
TSharedPtr<FDMXProtocolArtNetReceiver> FDMXProtocolArtNetReceiver::TryCreate(const TSharedPtr<FDMXProtocolArtNet, ESPMode::ThreadSafe>& ArtNetProtocol, const FString& IPAddress)
{
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create Art-Net receiver: Invalid IP address: %ls", *IPAddress);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create Art-Net receiver: Error create ListeningSocket for: %ls", *IPAddress);
		return nullptr;
	}
