...
namespace UE::DMX::DMXOutputPort::Private::ConsoleCommands
{
	...
	void LogOutputPortConfiguration(const FDMXOutputPortSharedPtr OutputPort)
	{
		...
		{
			UE_LOGF(LogDMXProtocol, Log, "================================================");
			UE_LOGF(LogDMXProtocol, Log, "Configuration of Output Port '%ls':", *OutputPort->GetPortName());
			UE_LOGF(LogDMXProtocol, Log, " ");

			UE_LOGF(LogDMXProtocol, Log, "Port Name: %ls", *OutputPort->GetPortName());

			FDMXOutputPortConfig PortConfig = OutputPort->MakeOutputPortConfig();
			UE_LOGF(LogDMXProtocol, Log, "Protocol: %ls", *PortConfig.GetProtocolName().ToString());
			UE_LOGF(LogDMXProtocol, Log, "Communication Type: %i", static_cast<int32>(PortConfig.GetCommunicationType()));
			UE_LOGF(LogDMXProtocol, Log, "TimeToLive: %i", PortConfig.GetTimeToLive());
			UE_LOGF(LogDMXProtocol, Log, "Device Address: %ls", *PortConfig.GetDeviceAddress());
			...
			{
				UE_LOGF(LogDMXProtocol, Log, "Destination Address [%i]: %ls", IndexDestinationAddress, *DestinationAddresses[IndexDestinationAddress].DestinationAddressString);
			}

			UE_LOGF(LogDMXProtocol, Log, "Input into Engine: %ls", PortConfig.NeedsLoopbackToEngine() ? TEXT("True") : TEXT("False"));
			UE_LOGF(LogDMXProtocol, Log, "Local Universe Start: %i", PortConfig.GetLocalUniverseStart());
			UE_LOGF(LogDMXProtocol, Log, "Num Universes: %i", PortConfig.GetNumUniverses());
			UE_LOGF(LogDMXProtocol, Log, "Extern Universe Start: %i", PortConfig.GetExternUniverseStart());
			UE_LOGF(LogDMXProtocol, Log, "Priority:	%i", PortConfig.GetPriority());

			UE_LOGF(LogDMXProtocol, Log, "Delay: %f", PortConfig.GetDelay());
			UE_LOGF(LogDMXProtocol, Log, "Delay Frame Rate:	%i/%i", PortConfig.GetDelayFrameRate().Numerator, PortConfig.GetDelayFrameRate().Denominator);

			UE_LOGF(LogDMXProtocol, Log, "Delay in Seconds: %f", OutputPort->GetDelaySeconds());
			UE_LOGF(LogDMXProtocol, Log, "================================================");
		}
	}
	...
	static const FDMXOutputPortSharedPtr FindOutputPortByName(const FString& PortName, bool bPrintToLogIfNotFound = true)
	{
		...
		else if (bPrintToLogIfNotFound)
		{
			if (PortName.IsEmpty())
			{
				UE_LOGF(LogDMXProtocol, Warning, "No Ouptut Port specified for console command. First argument needs to be a valid Port Name.");
			}
			else
			{
				UE_LOGF(LogDMXProtocol, Warning, "Could not find Output Port '%ls'.  Available Output Ports and their names can be found in Project settings -> Plugins -> DMX.", *PortName);
			}
		}
	...
	static bool VerifyConsoleCommandArguments(const FString& ConsoleCommand, int32 MinNumExpectedArgs, const TArray<FString>& Args)
	{
		...
		{			
			...
			UE_LOGF(LogDMXProtocol, Warning, "Console Command %ls failed. Output Port with Name '%ls' does not exist.", *ConsoleCommand, *PortName);
			UE_LOGF(LogDMXProtocol, Warning, "Ports and their Names can be found in Project Settings -> Plugins -> DMX.");

			return false;
		}
		else if (MinNumExpectedArgs > Args.Num())
		{
			UE_LOGF(LogDMXProtocol, Warning, "Console Command %ls failed. Invalid arguments specified.", *ConsoleCommand);
	...
	static FAutoConsoleCommand GDMXSetOutputPortProtocolCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateStatic(
			[](const TArray<FString>& Args)
			{
				...
				{
					UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetOutputPortProtocol failed. Protocol '%ls' does not exist or is not loaded. Available protocols are: ", *ProtocolNameValue.ToString());
					for (const FName& ProtocolName : IDMXProtocol::GetProtocolNames())
					{
						UE_LOGF(LogDMXProtocol, Warning, "%ls", *ProtocolName.ToString());
					}
				}
			})
	);
		...
		FConsoleCommandWithArgsDelegate::CreateStatic(
			[](const TArray<FString>& Args)
			{
				...
				if (!VerifyConsoleCommandArguments(TEXT("DMX.SetOutputPortDeviceAddress"), MinNumExpectedArgs, Args))
				{
					...
					UE_LOGF(LogDMXProtocol, Log, "Logging available Local Network Interface Card IP Addresses:");
					...
					{
						UE_LOGF(LogDMXProtocol, Log, "%i: %ls", IPAddressIndex, *(*AvailableIPAddresses[IPAddressIndex]));
					}
					UE_LOGF(LogDMXProtocol, Log, "================================================");
					return;
				}
				...
				if (OutputPort.IsValid())
				{
					if (const IDMXProtocolPtr& Protocol = OutputPort->GetProtocol())
					{
						...
						if (LexTryParseString<int32>(LocalUniverseStartValue, *LocalUniverseStartValueString))
						{
							...
							{
								UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetOutputPortLocalUniverseStart failed. Local Universe Start '%ls' along with Num Universes '%i' results in a Universe range that is not supported by the Protocol of the Port.", *LocalUniverseStartValueString, NumUniverses);
							}
						}
					}
				}
				...
				if (OutputPort.IsValid())
				{
					if (const IDMXProtocolPtr& Protocol = OutputPort->GetProtocol())
					{
						...
						if (LexTryParseString<int32>(NumUniversesValue, *NumUniversesValueString))
						{
							...
							{
								UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetOutputPortNumUniverses failed. Local Universe Start '%i' along with Num Universes '%ls' results in a Universe range that is not supported by the Protocol of the Port.", LocalUniverseStart, *NumUniversesValueString);
							}
						}
					}
				}
				...
				if (OutputPort.IsValid())
				{
					if (const IDMXProtocolPtr& Protocol = OutputPort->GetProtocol())
					{
						...
						if (LexTryParseString<int32>(ExternUniverseStartValue, *ExternUniverseStartValueString))
						{
							...
							{
								UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetOutputPortExternUniverseStart failed. Extern Universe Start '%ls' along with Num Universes '%i' results in a Universe range that is not supported by the Protocol of the Port.", *ExternUniverseStartValueString, NumUniverses);
							}
						}
					}
				}
			...
			[](const TArray<FString>& Args)
			{
				...
				if (LexTryParseString<double>(DelayValue, *DelayValueString))
				{
					...
					if (Args.Num() == 3)
					{
						...
						{
							UE_LOGF(LogDMXProtocol, Warning, "Could not parse frame rate from console command DMX.SetOutputPortDelay. Note, not all decimal numbers are supported. E.g. 0.345s needs to be written as 345/1000.")
						}
					}
...
FDMXOutputPortSharedRef FDMXOutputPort::CreateFromConfig(FDMXOutputPortConfig& OutputPortConfig)
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Created output port %ls", *NewOutputPort->PortName);
...
{	
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Destroyed output port %ls", *PortName);
}
