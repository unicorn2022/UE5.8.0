...
bool FDMXPort::IsValidPortSlow() const
{
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create DMX Port: Resulting First Universe %i is smaller than Protocol Min %i.", ExternUniverseStart, Protocol->GetMinUniverseID());
		return false;
	}
	...
	{
		UE_LOGF(LogDMXProtocol, Warning, "Cannot create DMX Port: Resulting Last Universe %i is bigger than Protocol Max %i.", ExternUniverseEnd, Protocol->GetMinUniverseID());
		return false;
	}
