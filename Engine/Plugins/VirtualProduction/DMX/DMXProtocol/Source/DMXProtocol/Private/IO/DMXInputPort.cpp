...
namespace UE::DMX::DMXInputPort::Private::ConsoleCommands
{
	void LogInputPortConfiguration(const FDMXInputPortSharedPtr InputPort)
	{
		if (InputPort.IsValid())
		{
			...
			UE_LOGF(LogDMXProtocol, Log, "Configuration of Input Port '%ls':", *InputPort->GetPortName());
			UE_LOGF(LogDMXProtocol, Log, " ");

			UE_LOGF(LogDMXProtocol, Log, "Port Name: %ls", *InputPort->GetPortName());
			...
			UE_LOGF(LogDMXProtocol, Log, "Priority Strategy: %i", static_cast<int32>(PortConfig.GetPortPriorityStrategy()));
	...
	static const FDMXInputPortSharedPtr FindInputPortByName(const FString& PortName, bool bPrintToLogIfNotFound = true)
	{
		...
		else if (bPrintToLogIfNotFound)
		{
			if (PortName.IsEmpty())
			{
				UE_LOGF(LogDMXProtocol, Warning, "No Input Port specified for console command. First argument needs to be a valid Port Name.");
			}
			else
			{
				UE_LOGF(LogDMXProtocol, Warning, "Could not find Input Port '%ls'. Available Input Ports and their names can be found in in Project settings -> Plugins -> DMX.", *PortName);
			}
		}
	...
	static bool VerifyConsoleCommandArguments(const FString& ConsoleCommand, int32 MinNumExpectedArgs, const TArray<FString>& Args)
	{
		...
		else if (!InputPort.IsValid())
		{
			...
			UE_LOGF(LogDMXProtocol, Warning, "Console Command %ls failed. Input Port with Name '%ls' does not exist.", *ConsoleCommand, *PortName);
		...
		else if (MinNumExpectedArgs > Args.Num())
		{
			...
			UE_LOGF(LogDMXProtocol, Warning, "Use '%ls ?' for help. Use '%ls PortName ?' to print the current configuration to logs.", *ConsoleCommand, *ConsoleCommand);
	...
	static FAutoConsoleCommand GDMXSetInputPortProtocolCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateStatic(
			[](const TArray<FString>& Args)
			{
				...
				{
					UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetInputPortProtocol failed. Protocol '%ls' does not exist or is not loaded. Available protocols are: ", *ProtocolNameValue.ToString());
				...
				if (InputPort.IsValid())
				{
					if (const IDMXProtocolPtr& Protocol = InputPort->GetProtocol())
					{
						...
						if (LexTryParseString<int32>(LocalUniverseStartValue, *LocalUniverseStartValueString))
						{
							...
							{
								UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetInputPortLocalUniverseStart failed. Local Universe Start '%ls' along with Num Universes '%i' results in a Universe range that is not supported by the Protocol of the Port.", *LocalUniverseStartValueString, NumUniverses);
							}
						}
					}
				}
				...
				if (InputPort.IsValid())
				{
					if (const IDMXProtocolPtr& Protocol = InputPort->GetProtocol())
					{
						...
						if (LexTryParseString<int32>(NumUniversesValue, *NumUniversesValueString))
						{
							...
							{
								UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetInputPortNumUniverses failed. Local Universe Start '%i' along with Num Universes '%ls' results in a Universe range that is not supported by the Protocol of the Port.", LocalUniverseStart, *NumUniversesValueString);
							}
						}
					}
				}
				...
				if (InputPort.IsValid())
				{
					...
					if (LexTryParseString<int32>(ExternUniverseStartValue, *ExternUniverseStartValueString))
					{
						if (const IDMXProtocolPtr& Protocol = InputPort->GetProtocol())
						{
							...
							{
								UE_LOGF(LogDMXProtocol, Warning, "Console command DMX.SetInputPortExternUniverseStart failed. Extern Universe Start '%ls' along with Num Universes '%i' results in a Universe range that is not supported by the Protocol of the Port.", *ExternUniverseStartValueString, NumUniverses);
							}
						}
					}
				}
...
FDMXInputPortSharedRef FDMXInputPort::CreateFromConfig(FDMXInputPortConfig& InputPortConfig)
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Created input port %ls", *NewInputPort->PortName);
...
FDMXInputPort::~FDMXInputPort()
{
	...
	UE_LOGF(LogDMXProtocol, VeryVerbose, "Destroyed input port %ls", *PortName);
}
