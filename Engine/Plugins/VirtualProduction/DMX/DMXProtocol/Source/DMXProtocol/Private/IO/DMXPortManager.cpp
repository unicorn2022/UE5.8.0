...
void FDMXPortManager::UpdateFromProtocolSettings(bool bForceUpdateRegistrationWithProtocol)
{
	...
	for (FDMXInputPortConfig& InputPortConfig : ProtocolSettings->InputPortConfigs)
	{
		...
		{
			UE_LOGF(LogDMXProtocol, Error, "Input Port '%ls' has no valid PortGUID and can no longer be used. If you changed the DefaultEngine.ini directly, please undo your changes. Otherwise please report the issue.", *InputPortConfig.GetPortName());
		}
	}
	...
	for (FDMXOutputPortConfig& OutputPortConfig : ProtocolSettings->OutputPortConfigs)
	{
		...
		{
			UE_LOGF(LogDMXProtocol, Error, "Output Port '%ls' has no valid GUID and can no longer be used. If you changed the DefaultEngine.ini directly, please undo your changes. Otherwise please report the issue.", *OutputPortConfig.GetPortName());
		}
	}
...
{
	UE_LOGF(LogDMXProtocol, Verbose, "Starting up DMXPortManager");
...
{
	UE_LOGF(LogDMXProtocol, Verbose, "Shutting down DMXPortManager");
