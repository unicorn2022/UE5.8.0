...
void UDMXPixelMappingMatrixComponent::LogInvalidProperties()
{
	...
	if (IsValid(FixturePatch))
	{
		...
		if (!ModePtr)
		{
			UE_LOGF(LogDMXPixelMappingRuntime, Warning, "%ls has no valid Active Mode set. %ls will not receive DMX.", *FixturePatch->GetDisplayName(), *GetName());
		}
		else if (!FixturePatch->GetFixtureType())
		{
			UE_LOGF(LogDMXPixelMappingRuntime, Warning, "%ls has no valid Fixture Type set. %ls will not receive DMX.", *FixturePatch->GetDisplayName(), *GetName());
		}
		else if(Children.Num() != ModePtr->FixtureMatrixConfig.XCells * ModePtr->FixtureMatrixConfig.YCells)
			{
				UE_LOGF(LogDMXPixelMappingRuntime, Warning, "Number of cells in %ls no longer matches %ls. %ls will not function properly.", *GetName(), *FixturePatch->GetFixtureType()->Name, *GetName());
			}
		}
	else
	{
		UE_LOGF(LogDMXPixelMappingRuntime, Warning, "%ls has no valid Fixture Patch set.", *GetName());
	}
}
