	...
	for (const UDMXMVRFixtureNode* MVRFixture : FixtureNodes)
	{
		for (int32 MetaDataIndex = 0; MetaDataIndex < NumMetaDatas; MetaDataIndex++)
		{
			...
			if (MetaDataMVRUUID == MVRFixture->UUID)
			{
				...
				if (!ensureAlwaysMsgf(bValidElementType, TEXT("DatasmithMVRNativeTranslator unexpected Element type when trying to remove MVR element in favor of an MVR Scene Actor.")))
				{
					continue;
				}
