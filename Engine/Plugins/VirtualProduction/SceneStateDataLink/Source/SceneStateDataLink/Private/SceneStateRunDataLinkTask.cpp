...
void FSceneStateRunDataLinkTask::OnOutputData(const FDataLinkExecutor& InExecutor, FConstStructView InOutputDataView, UE::SceneState::FTaskExecutionContext InTaskContext)
{
	...
	{
		UE_LOGF(LogSceneStateDataLink, Warning, "[%ls] Data Link Request Task failed. Output Target could not be resolved.", *ExecutionContext->GetExecutionContextName());
		InTaskContext.FinishTask();
		return;
	}
	...
	if (!StructProperty)
	{
		UE_LOGF(LogSceneStateDataLink, Error, "[%ls] Data Link Request Task failed. Output Target is not bound to a struct property"
	...
	{
		UE_LOGF(LogSceneStateDataLink, Warning, "[%ls] Data Link Request Task failed. Output Target struct ('%ls') and Data Link Output struct ('%ls') do not match!"
			, *ExecutionContext->GetExecutionContextName()
