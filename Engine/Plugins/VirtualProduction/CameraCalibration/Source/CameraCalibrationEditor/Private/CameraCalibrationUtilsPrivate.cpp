...
namespace UE::CameraCalibration::Private
{
	...
	bool FindArucoCalibrationPoint(const TArray<TWeakObjectPtr<UCalibrationPointComponent>>& CalibrationComponents, EArucoDictionary ArucoDictionary, const FArucoMarker& ArucoMarker, FArucoCalibrationPoint& OutArucoCalibrationPoint)
	{
		...
		for (TWeakObjectPtr<UCalibrationPointComponent> Component : CalibrationComponents)
		{
			...
			{
				const FString CornerName = FString::Printf(TEXT("%s-%s"), *OutArucoCalibrationPoint.Name, *CornerNames[CornerIndex]);
	...
	void ClearTexture(UTexture2D* Texture, FColor ClearColor)
	{
		if (Texture)
		{
...

			{
				...
				const SIZE_T SrcBytes = SIZE_T(Pixels.Num()) * SIZE_T(sizeof(FColor));
				const SIZE_T DstBytes = SIZE_T(Mip.BulkData.GetBulkDataSize());

				const SIZE_T NumBytes = FMath::Min(SrcBytes, DstBytes);
		...
		{
			{
				...
				const SIZE_T SrcBytes = SIZE_T(PixelData.Num()) * SIZE_T(sizeof(FColor));
				const SIZE_T DstBytes = SIZE_T(Mip.BulkData.GetBulkDataSize());
				...
				FMemory::Memcpy(TextureData, PixelData.GetData(), NumBytes);
			}
