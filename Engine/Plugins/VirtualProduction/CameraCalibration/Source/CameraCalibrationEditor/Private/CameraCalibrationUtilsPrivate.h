...
#include "CameraCalibrationCharucoBoard.h" // For EArucoDictionaryShadow
...
namespace UE::CameraCalibration::Private
{
	...
	/** Convert shadow enum to OpenCV enum */
	inline EArucoDictionary ShadowToOpenCV(EArucoDictionaryShadow Shadow)
	{
		return static_cast<EArucoDictionary>(Shadow);
	}

	/** Convert OpenCV enum to shadow enum */
	inline EArucoDictionaryShadow OpenCVToShadow(EArucoDictionary OpenCV)
	{
		return static_cast<EArucoDictionaryShadow>(OpenCV);
	}
