...
namespace UE::Private::CameraCalibration::AutomatedTests
{
	namespace Logging
	{
		...
		void LogDistortionParameters(const TArray<float>& DistortionParameters)
		{
			...
			{
				UE_LOGF(LogTestCameraCalibration, Log, "\t\t(%lf, %lf, %lf, %lf, %lf)",
		...
		void LogPoses(const TArray<FTransform>& Poses)
		{
			for (int32 PoseIndex = 0; PoseIndex < Poses.Num(); ++PoseIndex)
			{
				...
				UE_LOGF(LogTestCameraCalibration, Log, "Pose %d:", PoseIndex);
				UE_LOGF(LogTestCameraCalibration, Log, "\t\tLocation: (%lf, %lf, %lf)", Location.X, Location.Y, Location.Z);
				UE_LOGF(LogTestCameraCalibration, Log, "\t\tRotation: (%lf, %lf, %lf)", Rotation.Roll, Rotation.Pitch, Rotation.Yaw);
			}
		}
		...
		void LogPoses(const TArray<FLocationRotation>& Poses)
		{
			for (int32 PoseIndex = 0; PoseIndex < Poses.Num(); ++PoseIndex)
			{
				...
				UE_LOGF(LogTestCameraCalibration, Log, "Pose %d:", PoseIndex);
				UE_LOGF(LogTestCameraCalibration, Log, "\t\tLocation: (%lf, %lf, %lf)", Pose.Location.X, Pose.Location.Y, Pose.Location.Z);
				UE_LOGF(LogTestCameraCalibration, Log, "\t\tRotation: (%lf, %lf, %lf)", Pose.Rotation.Roll, Pose.Rotation.Pitch, Pose.Rotation.Yaw);
			}
		}

		void LogTestDescription(const FCalibrationTest& Test)
		{
			UE_LOGF(LogTestCameraCalibration, Log, "Test #%d:", Test.TestIndex);

			UE_LOGF(LogTestCameraCalibration, Log, " ");

			if (Test.DatasetPath.IsEmpty())
			{
				UE_LOGF(LogTestCameraCalibration, Log, "Camera Profile:");
				UE_LOGF(LogTestCameraCalibration, Log, "Sensor Dimensions: (%.3lfmm x %.3lfmm)", Test.CameraProfile.SensorSize.X, Test.CameraProfile.SensorSize.Y);
				UE_LOGF(LogTestCameraCalibration, Log, "Image Resolution: (%d, %d)", Test.CameraProfile.ImageSize.X, Test.CameraProfile.ImageSize.Y);

				const FVector2D FxFyInPixels = Test.CameraProfile.GetFxFyInPixels();
				UE_LOGF(LogTestCameraCalibration, Log, "Focal Length: %lf mm (%lf pixels)", Test.CameraProfile.FocalLengthInMM, FxFyInPixels.X);

				UE_LOGF(LogTestCameraCalibration, Log, "Image Center: (%lf, %lf)", Test.CameraProfile.ImageCenter.X, Test.CameraProfile.ImageCenter.Y);

				UE_LOGF(LogTestCameraCalibration, Log, "Distortion Parameters:");
				LogDistortionParameters(Test.CameraProfile.DistortionParameters);

				UE_LOGF(LogTestCameraCalibration, Log, " ");

				UE_LOGF(LogTestCameraCalibration, Log, "Checkerboard Profile:");
				UE_LOGF(LogTestCameraCalibration, Log, "Checkerboard Dimensions: %d columns by %d rows", Test.CheckerboardProfile.CheckerboardDimensions.X, Test.CheckerboardProfile.CheckerboardDimensions.Y);
				UE_LOGF(LogTestCameraCalibration, Log, "Checkerboard Square Size: %.2lfcm", Test.CheckerboardProfile.SquareSize);

				UE_LOGF(LogTestCameraCalibration, Log, " ");

				UE_LOGF(LogTestCameraCalibration, Log, "Camera Poses:");
				LogPoses(Test.CameraPoses);

				UE_LOGF(LogTestCameraCalibration, Log, " ");

				UE_LOGF(LogTestCameraCalibration, Log, "Checkerboard Poses:");
				LogPoses(Test.CheckerboardPoses);
			}
			else
			{
				UE_LOGF(LogTestCameraCalibration, Log, "Using dataset: %ls", *Test.DatasetPath);
			}

			UE_LOGF(LogTestCameraCalibration, Log, " ");

			UE_LOGF(LogTestCameraCalibration, Log, "Solver Settings:");
			UE_LOGF(LogTestCameraCalibration, Log, "Focal Length Guess: %lf mm", Test.SolverSettings.FocalLengthGuess);
			UE_LOGF(LogTestCameraCalibration, Log, "Image Center Guess: (%lf, %lf)", Test.SolverSettings.ImageCenterGuess.X, Test.SolverSettings.ImageCenterGuess.Y);
			UE_LOGF(LogTestCameraCalibration, Log, "Use Intrinsic Guess: %ls", *GetSolverFlagString(Test.SolverSettings.bUseIntrinsicGuess));
			UE_LOGF(LogTestCameraCalibration, Log, "Use Extrinsic Guess: %ls", *GetSolverFlagString(Test.SolverSettings.bUseExtrinsicGuess));
			UE_LOGF(LogTestCameraCalibration, Log, "Fix Focal Length: %ls", *GetSolverFlagString(Test.SolverSettings.bFixFocalLength));
			UE_LOGF(LogTestCameraCalibration, Log, "Fix Image Center: %ls", *GetSolverFlagString(Test.SolverSettings.bFixPrincipalPoint));
			UE_LOGF(LogTestCameraCalibration, Log, "Fix Extrinsics: %ls", *GetSolverFlagString(Test.SolverSettings.bFixExtrinsics));
			UE_LOGF(LogTestCameraCalibration, Log, "Fix Distortion: %ls", *GetSolverFlagString(Test.SolverSettings.bFixDistortion));
			UE_LOGF(LogTestCameraCalibration, Log, "Fix Aspect Ratio: %ls", *GetSolverFlagString(Test.SolverSettings.bFixAspectRatio));
		}

		void LogCalibrationResult(const FDistortionCalibrationResult& Result, double FocalLengthInMM)
		{
			UE_LOGF(LogTestCameraCalibration, Log, " ");
			UE_LOGF(LogTestCameraCalibration, Log, "Calibration Result:");
			UE_LOGF(LogTestCameraCalibration, Log, " ");
			UE_LOGF(LogTestCameraCalibration, Log, "FxFy: (%lf, %lf) pixels (%lf mm)", Result.FocalLength.FxFy.X, Result.FocalLength.FxFy.Y, FocalLengthInMM);
			UE_LOGF(LogTestCameraCalibration, Log, "Image Center: (%lf, %lf)", Result.ImageCenter.PrincipalPoint.X, Result.ImageCenter.PrincipalPoint.Y);
			...
			UE_LOGF(LogTestCameraCalibration, Log, "RMSE: %.9lf", Result.ReprojectionError);
		}
	}
	...
	void LoadCalibrationTestsFromFile(const FString FileName, FCalibrationTestSet& TestSet)
	{
		if (TUniquePtr<FArchive> FileReader = TUniquePtr<FArchive>(IFileManager::Get().CreateFileReader(*FileName)))
		{
			...
			if (FJsonSerializer::Deserialize(JsonReader, JsonObject))
			{
				...
				{
					UE_LOGF(LogTestCameraCalibration, Error, "Failed to convert json object to Calibration Test Set structure.");
				}
			}
			else
			{
				UE_LOGF(LogTestCameraCalibration, Error, "Failed to deserialize json file. Check that it is properly formatted.");
			}
		}
		else
		{
			UE_LOGF(LogTestCameraCalibration, Error, "Failed to read test filename");
		}
	}
	...
	void LoadDatasetFromFile(FCalibrationTest& CalibrationTest, TArray<FObjectPoints>& ObjectPoints, TArray<FImagePoints>& ImagePoints)
	{
		...
		if (FoundFiles.Num() < 1)
		{
			UE_LOGF(LogTestCameraCalibration, Error, "The following dataset had no json files to import: %ls", *CalibrationTest.DatasetPath);
			return;
		}
