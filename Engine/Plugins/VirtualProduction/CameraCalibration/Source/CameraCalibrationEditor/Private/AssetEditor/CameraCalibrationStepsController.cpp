...
void FCameraCalibrationStepsController::CreateSteps()
{
	...
	if (!Subsystem)
	{
		UE_LOGF(LogCameraCalibrationEditor, Error, "Could not find UCameraCalibrationSubsystem");
		return;
	}
...
void FCameraCalibrationStepsController::InitializeMediaPlayer()
{
	...
	if (!InternalMediaPlayer.IsValid())
	{
		UE_LOGF(LogCameraCalibrationEditor, Warning, "Failed to create MediaPlayer");
		Cleanup();
		return;
	}
...
bool FCameraCalibrationStepsController::CalculateNormalizedMouseClickPosition(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, FVector2f& OutPosition, ESimulcamViewportPortion ViewportPortion) const
{
	...
	if (ViewportPortion == ESimulcamViewportPortion::CameraFeed)
	{
		FIntRect CropRegion;
		FIntPoint CropSize;
		if (!CalculateCameraFeedCropRegion(CropRegion, CropSize))
		{
			return false;
		}
		...
		// Convert normalized viewport coordinates to media texture coordinates
		const float MediaX = XNormalized * MediaOverlayResolution.X;
		const float MediaY = YNormalized * MediaOverlayResolution.Y;
		
		// Check if the click is within the cropped region
		if (MediaX < CropRegion.Min.X || MediaX > CropRegion.Max.X ||
		    MediaY < CropRegion.Min.Y || MediaY > CropRegion.Max.Y)
		{
			return false;
		}
		
		// Map from cropped region to normalized camera feed coordinates
		XNormalized = (MediaX - CropRegion.Min.X) / CropSize.X;
		YNormalized = (MediaY - CropRegion.Min.Y) / CropSize.Y;

		// Ensure normalized values are within valid range (defensive check)
...
bool FCameraCalibrationStepsController::ReadMediaPixels(TArray<FColor>& Pixels, FIntPoint& Size, FText& OutErrorMessage, ESimulcamViewportPortion ViewportPortion) const
{
	...
	if ((ViewportPortion == ESimulcamViewportPortion::CameraFeed) && LensFilePtr->CameraFeedInfo.IsValid())
	{
		...
		{
			OutErrorMessage = LOCTEXT("InvalidCameraFeedDimensions", "Camera feed dimensions are invalid (zero or negative).");
			return false;
		}
		...
		// Protect against invalid media resolution
		if (MediaResolution.X <= 0 || MediaResolution.Y <= 0)
		{
			OutErrorMessage = LOCTEXT("InvalidMediaResolution", "Media resolution is invalid (zero or negative).");
			return false;
		}
		
		// Use the common function to calculate crop region
		FIntRect CropRegion;
		FIntPoint CropSize;
		if (!CalculateCameraFeedCropRegion(CropRegion, CropSize))
		{
			OutErrorMessage = LOCTEXT("InvalidCropCalculation", "Failed to calculate valid crop region.");
			return false;
		}
		
		// Map each output pixel to the fitted media region
		for (int32 YCoordinate = 0; YCoordinate < Size.Y; YCoordinate++)
		{
			for (int32 XCoordinate = 0; XCoordinate < Size.X; XCoordinate++)
			{
				// Map output coordinates to crop region coordinates
				const int32 ScaledX = CropRegion.Min.X + (XCoordinate * CropSize.X) / Size.X;
				const int32 ScaledY = CropRegion.Min.Y + (YCoordinate * CropSize.Y) / Size.Y;
...
bool FCameraCalibrationStepsController::CalculateCameraFeedCropRegion(FIntRect& OutCropRegion, FIntPoint& OutCropSize) const
{
	ULensFile* LensFilePtr = GetLensFile();
	if (!LensFilePtr || !LensFilePtr->CameraFeedInfo.IsValid())
	{
		return false;
	}
	...
	// Protect against invalid dimensions
	if (CameraFeedDimensions.X <= 0 || CameraFeedDimensions.Y <= 0 || 
	    MediaResolution.X <= 0 || MediaResolution.Y <= 0)
	{
		return false;
	}
	
	// Calculate the center-crop/fit region
	const float MediaAspect = (float)MediaResolution.X / (float)MediaResolution.Y;
	const float DesiredAspect = (float)CameraFeedDimensions.X / (float)CameraFeedDimensions.Y;
	
	if (MediaAspect > DesiredAspect)
	{
		// Media is wider - fit by height, center horizontally
		OutCropSize.Y = MediaResolution.Y;
		OutCropSize.X = FMath::RoundToInt(MediaResolution.Y * DesiredAspect);
		OutCropRegion.Min.X = (MediaResolution.X - OutCropSize.X) / 2;
		OutCropRegion.Min.Y = 0;
	}
	else
	{
		// Media is taller - fit by width, center vertically
		OutCropSize.X = MediaResolution.X;
		OutCropSize.Y = FMath::RoundToInt(MediaResolution.X / DesiredAspect);
		OutCropRegion.Min.X = 0;
		OutCropRegion.Min.Y = (MediaResolution.Y - OutCropSize.Y) / 2;
	}
	
	OutCropRegion.Max = OutCropRegion.Min + OutCropSize;
	
	// Validate crop size
	return (OutCropSize.X > 0 && OutCropSize.Y > 0);
}
