...
struct FLensCaptureSettings
{
	...
	/** The pattern to detect in the media image. CharUco is recommended when available. */
...
private:
	...
	/** Detects a charuco pattern in the input array of pixels (supplied by the media image) and fills out the 2D and 3D points in the output calibration row */
	bool DetectCharucoPattern(TArray<FColor>& Pixels, FIntPoint Size, FIntRect RegionOfInterest, TSharedPtr<FCalibrationRow> OutRow);
	...
	/** Generates the object points in local space. Available in case the solver requires it. */
	void GenerateObjectPointsInLocalSpace(TArray<FObjectPoints>& Samples3d);
