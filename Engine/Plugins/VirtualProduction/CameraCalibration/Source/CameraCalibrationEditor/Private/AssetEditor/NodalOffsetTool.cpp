...
bool UNodalOffsetTool::OnViewportMarqueeSelect(FVector2D StartPosition, FVector2D EndPosition)
{
	if (!bIsActive || !NodalOffsetAlgo)
	{
		return false;
	}

	// Each algo has its own reaction to the marquee selection.
	return NodalOffsetAlgo->OnViewportMarqueeSelect(StartPosition, EndPosition);
}
...
void UNodalOffsetTool::DeleteExportedRow(const int32& RowIndex) const
{
	...
	for (const FString& File : FoundFiles)
	{
		if (File.Contains(RowNumString))
		{
			...
			UE_LOGF(LogCameraCalibrationEditor, Verbose, "Nodal Offset Tool removed calibration dataset file: %ls", *FullPath);
		}
	}
}
...
void UNodalOffsetTool::ExportCalibrationRow(int32 RowIndex, const TSharedRef<FJsonObject>& RowObject, const FImageView& RowImage)
{
	...
	if (TUniquePtr<FArchive> FileWriter = TUniquePtr<FArchive>(IFileManager::Get().CreateFileWriter(*JsonFileName)))
	{
		...
		UE_LOGF(LogCameraCalibrationEditor, Verbose, "Nodal Offset Tool wrote to dataset row file: %ls", *JsonFileName);
	}
...
void UNodalOffsetTool::ExportSessionData(const TSharedRef<FJsonObject>& SessionDataObject)
{
	...
	if (TUniquePtr<FArchive> FileWriter = TUniquePtr<FArchive>(IFileManager::Get().CreateFileWriter(*SessionFileName)))
	{
		...
		UE_LOGF(LogCameraCalibrationEditor, Verbose, "Nodal Offset Tool wrote to dataset session file: %ls", *SessionFileName);
	}
}
...
void UNodalOffsetTool::ImportCalibrationDataset()
{
	...
	FDateTime ImportedSessionDateTime = FDateTime::Now();
	{
		...
		if (TUniquePtr<FArchive> FileReader = TUniquePtr<FArchive>(IFileManager::Get().CreateFileReader(*SessionFile)))
		{
			...
			if (FJsonSerializer::Deserialize(JsonReader, JsonSessionData))
			{
				...
				{
					UE_LOGF(LogCameraCalibrationEditor, Verbose, "Nodal Offset Tool failed to deserialize the date/time from the session file: %ls", *SessionFile);
				}
				...
				{
					UE_LOGF(LogCameraCalibrationEditor, Verbose, "Nodal Offset Tool failed to deserialize the algo name from the session file: %ls", *SessionFile);
				}
			...
			{
				UE_LOGF(LogCameraCalibrationEditor, Verbose, "Nodal Offset Tool failed to deserialize dataset session file: %ls", *SessionFile);
			}
		}
	}
	...
	for (const FString& File : FoundFiles)
	{
		...
		if (TUniquePtr<FArchive> FileReader = TUniquePtr<FArchive>(IFileManager::Get().CreateFileReader(*JsonFileName)))
		{
			...
			{
				UE_LOGF(LogCameraCalibrationEditor, Verbose, "Nodal Offset Tool failed to deserialize the dataset row file: %ls", *JsonFileName);
			}
		}
	}
