...
// Console variable to control whether to generate object points in local space for Charuco calibration
static TAutoConsoleVariable<bool> CVarCharucoUseLocalSpaceObjectPoints(
	TEXT("CameraCalibration.Charuco.UseLocalSpaceObjectPoints"),
	true,
	TEXT("If true, generate object points in local space for Charuco calibration instead of using world space points.\n")
	TEXT("The solvers don't require object points in a canonical plane anymore, but can yield less accurate results otherwise.\n")
	TEXT("Default: true"),
	ECVF_Default
);
...
{
	// Marquee select supports ROI for checkerboard and Charuco detection
	if (CaptureSettings.CalibrationPattern != ECalibrationPattern::Checkerboard && 
	    CaptureSettings.CalibrationPattern != ECalibrationPattern::Charuco)
	{
		return false;
	}

	// Normalize the marquee selection to ensure Min <= Max for both axes
	FIntRect MarqueeSelection;
	MarqueeSelection.Min = FIntPoint(
		FMath::Min(FMath::Floor(StartPosition.X), FMath::Floor(EndPosition.X)),
		FMath::Min(FMath::Floor(StartPosition.Y), FMath::Floor(EndPosition.Y))
	);
	MarqueeSelection.Max = FIntPoint(
		FMath::Max(FMath::Floor(StartPosition.X), FMath::Floor(EndPosition.X)),
		FMath::Max(FMath::Floor(StartPosition.Y), FMath::Floor(EndPosition.Y))
	);
...
bool ULensDistortionTool::CaptureCalibrationData(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, FIntRect RegionOfInterest)
{
	...
	else
	{
		...
		else if (CaptureSettings.CalibrationPattern == ECalibrationPattern::Charuco)
		{
			// If no ROI was provided, use the full image size
			if (RegionOfInterest.IsEmpty())
			{
			...
			bDetectionResult = DetectCharucoPattern(Pixels, ImageSize, RegionOfInterest, NewRow);
		}
...
bool ULensDistortionTool::DetectCharucoPattern(TArray<FColor>& Pixels, FIntPoint Size, FIntRect RegionOfInterest, TSharedPtr<FCalibrationRow> OutRow)
{
	const FText ErrorTitle = LOCTEXT("CaptureError", "Capture Error");
	...
	ACameraCalibrationCharucoBoard* CharucoBoard = Cast<ACameraCalibrationCharucoBoard>(CalibratorActor);
	if (!CharucoBoard)
	{
		const FText ErrorMessage = LOCTEXT("NoCharucoBoardError", "The selected calibrator is not a Charuco board.");
		FMessageDialog::Open(EAppMsgCategory::Error, EAppMsgType::Ok, ErrorMessage, ErrorTitle);
		return false;
	}
	...
	FCharucoBoardConfig BoardConfig;

	BoardConfig.SquaresX = CharucoBoard->NumCornerCols + 1;
	BoardConfig.SquaresY = CharucoBoard->NumCornerRows + 1;
	BoardConfig.SquareSize = CharucoBoard->SquareSideLength;
	BoardConfig.MarkerSize = CharucoBoard->SquareSideLength * CharucoBoard->MarkerSizeRatio;
	BoardConfig.Dictionary = UE::CameraCalibration::Private::ShadowToOpenCV(CharucoBoard->ArucoDictionary);

	// Handle ROI by cropping the image (if needed)

	TArray<FColor> ProcessedPixels;
	FIntPoint ProcessedSize;
	FVector2D ROIOffset(0, 0);
	
	if (!RegionOfInterest.IsEmpty() && RegionOfInterest != FIntRect(FIntPoint(0), Size))
	{
		// Crop the image to the ROI

		const int32 ROIWidth = RegionOfInterest.Width();
		const int32 ROIHeight = RegionOfInterest.Height();

		ProcessedPixels.Reserve(ROIWidth * ROIHeight);
		
		for (int32 Y = RegionOfInterest.Min.Y; Y < RegionOfInterest.Max.Y; ++Y)
		{
			for (int32 X = RegionOfInterest.Min.X; X < RegionOfInterest.Max.X; ++X)
			{
				if (X >= 0 && X < Size.X && Y >= 0 && Y < Size.Y)
				{
					ProcessedPixels.Add(Pixels[Y * Size.X + X]);
				}
				else
				{
					ProcessedPixels.Add(FColor::Black);
				}
			}
		}
		
		ProcessedSize = FIntPoint(ROIWidth, ROIHeight);
		ROIOffset = FVector2D(RegionOfInterest.Min.X, RegionOfInterest.Min.Y);
	}
	else
	{
		// Use the full image
	...
	bool bResult = FOpenCVHelper::IdentifyCharucoCorners(ProcessedPixels, ProcessedSize, BoardConfig, DetectedCorners);
	
	// Adjust detected coordinates by ROI offset
	if (bResult && ROIOffset != FVector2D::ZeroVector)
	{
		for (FVector2f& Corner : DetectedCorners.Corners)
		{
			Corner.X += ROIOffset.X;
			Corner.Y += ROIOffset.Y;
		}
		for (FArucoMarker& Marker : DetectedCorners.Markers)
		{
			for (int32 CornerIdx = 0; CornerIdx < 4; ++CornerIdx)
			{
				Marker.Corners[CornerIdx].X += ROIOffset.X;
				Marker.Corners[CornerIdx].Y += ROIOffset.Y;
			}
		}
	}

	if (!bResult || DetectedCorners.Corners.IsEmpty())
	{
		const FText ErrorMessage = FText::FromString(FString::Printf(TEXT(
			"Failed to detect Charuco corners in the image. "
			"Make sure the board is clearly visible and well-lit. "
			"Expected board: %dx%d squares with ArUco dictionary %s"),
			BoardConfig.SquaresX, BoardConfig.SquaresY,
			*UEnum::GetValueAsString(BoardConfig.Dictionary)
		));
	...
	// Convert detected corners to calibration row format
	//

	// Corners and CornerIds arrays must have the same size
	if (DetectedCorners.Corners.Num() != DetectedCorners.CornerIds.Num())
	{
		const FText ErrorMessage = FText::Format(LOCTEXT("CornerDataMismatchError", 
			"Charuco detection data is corrupted: {0} corners but {1} corner IDs. Data integrity check failed."),
	...
	for (int32 CornerIdx = 0; CornerIdx < DetectedCorners.Corners.Num(); ++CornerIdx)
	{
		...
		// Get the 3D position of this corner from the board
		FVector WorldCorner = CharucoBoard->GetCornerPosition(CornerId);
				
		OutRow->ObjectPoints.Points.Add(WorldCorner);
		OutRow->ImagePoints.Points.Add(FVector2D(ImageCorner.X, ImageCorner.Y));
	}

	// Set target pose from the board transform
	OutRow->TargetPose = CharucoBoard->GetActorTransform();

	// Rescale corners to match overlay texture size before drawing
	if (OverlayTexture)
	{
		const FIntPoint OverlayTextureSize = FIntPoint(OverlayTexture->GetSizeX(), OverlayTexture->GetSizeY());
		const FVector2D TopLeftCorner = (OverlayTextureSize - Size) / 2.0f;
		
		// Create a copy of detected corners with rescaled positions
		FCharucoCorners RescaledCorners = DetectedCorners;
		for (FVector2f& Corner : RescaledCorners.Corners)
		{
			Corner.X += TopLeftCorner.X;
			Corner.Y += TopLeftCorner.Y;
		}
		for (FArucoMarker& Marker : RescaledCorners.Markers)
		{
			for (int32 CornerIdx = 0; CornerIdx < 4; ++CornerIdx)
			{
				Marker.Corners[CornerIdx].X += TopLeftCorner.X;
				Marker.Corners[CornerIdx].Y += TopLeftCorner.Y;
			}
		}
		
		// Draw the rescaled corners on overlay
		FOpenCVHelper::DrawCharucoCorners(RescaledCorners, OverlayTexture);
	}
...
void ULensDistortionTool::CalibrateLens()
{
	...
	{
		// The solvers don't require object points in a canonical plane anymore, but can yield less accurate results otherwise.
		if (CVarCharucoUseLocalSpaceObjectPoints.GetValueOnGameThread())
		{
			GenerateObjectPointsInLocalSpace(Samples3d);
		}
	}
	...
	CalibrationTask = UE::Tasks::Launch(UE_SOURCE_LOCATION, [InSolver = this->Solver, Model, Samples3d, Samples2d, ImageSize, FocalLength, ImageCenter, DistortionGuess, CameraPoses, TargetPoses, PixelAspect, SolverFlags, LensFileEvalInputs]() mutable
		{
			...
			{
				Result.ErrorMessage = FText::FromString(FString::Printf(TEXT("Calibration solver reported the error: %s"), ANSI_TO_TCHAR(Exception.what())));
				UE_LOGF(LogCameraCalibrationEditor, Error, "LensDistortionTool: Solver crashed with std::exception: %ls", ANSI_TO_TCHAR(Exception.what()));
			}
			catch (...)
			{
				Result.ErrorMessage = FText::FromString(TEXT("Calibration solver returned with an unspecified error"));
				UE_LOGF(LogCameraCalibrationEditor, Error, "LensDistortionTool: Solver crashed with unknown exception");
			}
...
void ULensDistortionTool::SaveCalibrationResult()
{
	...
	if (CalibrationResult.Parameters.Parameters.Num() > 0)
	{
		if (LensFile->DataMode != ELensDataMode::Parameters)
		{
			...
			UE_LOGF(LogCameraCalibrationEditor, Log, "The LensFile's data mode was set to ST Map, but the latest calibration result returned distortion parameters. Data mode will change to Parameters.");
		}
	...
	else if (CalibrationResult.STMap.DistortionMap)
	{
		if (LensFile->DataMode != ELensDataMode::STMap)
		{
			...
			UE_LOGF(LogCameraCalibrationEditor, Log, "The LensFile's data mode was set to Parameters, but the latest calibration result returned an ST Map. Data mode will change to ST Map.");
		}
...
{
	// If the camera is not tracked, the distortion solver must initialize the camera pose for each image.
	// However, it may struggle when "real" tracking data is used for the calibrator. So in this case, we replace the tracked calibrator data 
...
void ULensDistortionTool::GenerateObjectPointsInLocalSpace(TArray<FObjectPoints>& Samples3d)
{
	Samples3d.Empty();
	...
	for (const TSharedPtr<FCalibrationRow>& Row : Dataset.CalibrationRows)
	{
		...
		// Get the inverse of the calibrator transform that was used for this capture
		const FTransform InverseTargetPose = Row->TargetPose.Inverse();
		
		for (int32 PointIdx = 0; PointIdx < Row->ObjectPoints.Points.Num(); ++PointIdx)
		{
			const FVector& WorldPoint = Row->ObjectPoints.Points[PointIdx];
			
			// Transform back to local space using the stored target pose
			const FVector LocalPoint = InverseTargetPose.TransformPosition(WorldPoint);
			
			Points3d.Points.Add(LocalPoint);
		}
...
void ULensDistortionTool::DeleteExportedRow(const int32& RowIndex) const
{
	...
	for (const FString& File : FoundFiles)
	{
		if (File.Contains(RowNumString))
		{
			...
			UE_LOGF(LogCameraCalibrationEditor, Verbose, "Lens Distortion Tool removed calibration dataset file: %ls", *FullPath);
		}
	}
}
...
void ULensDistortionTool::ImportCalibrationDataset()
{
	...
	EDatasetVersion ImportedDatasetVersion = EDatasetVersion::Invalid;
	{
		...
		if (TUniquePtr<FArchive> FileReader = TUniquePtr<FArchive>(IFileManager::Get().CreateFileReader(*SessionFile)))
		{
			...
			if (FJsonSerializer::Deserialize(JsonReader, JsonSessionData))
			{
				...
				{
					UE_LOGF(LogCameraCalibrationEditor, Verbose, "Lens Distortion Tool failed to deserialize the date/time from the session file: %ls", *SessionFile);
				}
			...
			{
				UE_LOGF(LogCameraCalibrationEditor, Verbose, "Lens Distortion Tool failed to deserialize dataset session file: %ls", *SessionFile);
			}
		}
	}
	...
	for (const FString& File : FoundFiles)
	{
		...
		if (TUniquePtr<FArchive> FileReader = TUniquePtr<FArchive>(IFileManager::Get().CreateFileReader(*JsonFileName)))
		{
			...
			{
				UE_LOGF(LogCameraCalibrationEditor, Verbose, "Lens Distortion Tool failed to deserialize the dataset row file: %ls", *JsonFileName);
			}
		}
	}
...
void ULensDistortionTool::ExportSessionData()
{
	...
	if (TUniquePtr<FArchive> FileWriter = TUniquePtr<FArchive>(IFileManager::Get().CreateFileWriter(*SessionFileName)))
	{
		...
		UE_LOGF(LogCameraCalibrationEditor, Verbose, "Lens Distortion Tool wrote to dataset session file: %ls", *SessionFileName);
	}
}
...
void ULensDistortionTool::ExportCalibrationRow(TSharedPtr<FCalibrationRow> Row)
{
	...
	if (const TSharedPtr<FJsonObject>& RowObject = FJsonObjectConverter::UStructToJsonObject<FCalibrationRow>(Row.ToSharedRef().Get()))
	{
		...
		if (TUniquePtr<FArchive> FileWriter = TUniquePtr<FArchive>(IFileManager::Get().CreateFileWriter(*JsonFileName)))
		{
			...
			UE_LOGF(LogCameraCalibrationEditor, Verbose, "Lens Distortion Tool wrote to dataset row file: %ls", *JsonFileName);
		}
...
int32 ULensDistortionTool::ImportCalibrationRow(const TSharedRef<FJsonObject>& CalibrationRowObject, const FImage& RowImage, EDatasetVersion DatasetVersion)
{
	...
	else if (DatasetVersion == EDatasetVersion::CombinedAlgoClasses)
	{
		...
		{
			UE_LOGF(LogCameraCalibrationEditor, Warning, "Failed to import calibration row because at least one field could not be deserialized from the json file.");
		}
	}
