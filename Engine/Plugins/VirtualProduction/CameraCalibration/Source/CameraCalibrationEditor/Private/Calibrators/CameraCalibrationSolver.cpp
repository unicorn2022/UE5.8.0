...
static TAutoConsoleVariable<float> CVarRotationStepValue(TEXT("CameraCalibration.RotationStepValue"), 0.05f, TEXT("The value of the initial step size to use when finding an optimal nodal offset rotation that minimizes reprojection error."));
static TAutoConsoleVariable<float> CVarLocationStepValue(TEXT("CameraCalibration.LocationStepValue"), 0.5f, TEXT("The value of the initial step size to use when finding an optimal nodal offset location that minimizes reprojection error."));
	...
	ECalibrationFlags SolverFlags)
{
	...
	// Compute canonical plane detection once to avoid redundant calculations
	const bool bUseCanonicalPlaneInit = CanUseHomographyInitialization(ObjectPointsMat);
	...
	if (!(EnumHasAnyFlags(SolverFlags, ECalibrationFlags::UseIntrinsicGuess)))
	{
		InitCameraIntrinsics(ObjectPointsMat, ImagePointsMat, NumPointsMat, CvImageSize, CameraMatrix, bUseCanonicalPlaneInit);
	}
	...
	for (int ImageIndex = 0; ImageIndex < NumPosesToSolve; ImageIndex++)
	{
		...
		else
		{
			...
			InitCameraExtrinsics(LensModel, ObjectPointsInImage, ImagePointsInImage, CameraMatrix, DistCoeffs, CvImageSize, Rotation, Translation, SolverFlags, bUseCanonicalPlaneInit);
		}
	}
...
void ULensDistortionSolverOpenCV::InitCameraIntrinsics(
	...
	cv::Mat& CameraMatrix,
	bool bUseCanonicalPlaneInit)
{	
	if (bUseCanonicalPlaneInit)
	{
		UE_LOGF(LogCameraCalibrationSolver, Verbose, "Using homography-based initialization for objects point in a canonical plane");
		InitIntrinsicsFromCanonicalPlane(ObjectPoints, ImagePoints, NumPoints, ImageSize, CameraMatrix);
	}
	else
	{
		UE_LOGF(LogCameraCalibrationSolver, Verbose, "Using 3D point initialization for object points in general world coordinates");
		InitIntrinsicsFrom3DPoints(ObjectPoints, ImagePoints, NumPoints, ImageSize, CameraMatrix);
	}
}

void ULensDistortionSolverOpenCV::InitIntrinsicsFromCanonicalPlane(
	const cv::Mat& ObjectPoints,
	const cv::Mat& ImagePoints,
	const cv::Mat& NumPoints,
	const cv::Size ImageSize,
	cv::Mat& CameraMatrix)
{
	// Use homography-based method for 2D plane points (checkerboard, dummy points)
	const int NumImages = NumPoints.cols;

	const double Cx = (ImageSize.width - 1) * 0.5;
	const double Cy = (ImageSize.height - 1) * 0.5;

	// Collect valid homography constraint rows to avoid uninitialized matrix data
	std::vector<double> ValidMatrixARows;
	std::vector<double> ValidVecBRows;
	ValidMatrixARows.reserve(2 * NumImages * 2); // Each image contributes 2 rows with 2 columns
	ValidVecBRows.reserve(2 * NumImages);
	...
	for (int ImageIndex = 0; ImageIndex < NumImages; ++ImageIndex)
	{
		...
		if (HomographyMat.empty() || HomographyMat.rows != 3 || HomographyMat.cols != 3 || HomographyMat.type() != CV_64F)
		{
			UE_LOGF(LogCameraCalibrationSolver, Warning, "findHomography failed or returned invalid matrix for image %d", ImageIndex);
			continue;
		}
		...
		// Store valid constraint rows
		ValidMatrixARows.push_back(H[0] * V[0]);
		ValidMatrixARows.push_back(H[1] * V[1]);
		ValidMatrixARows.push_back(D1[0] * D2[0]);
		ValidMatrixARows.push_back(D1[1] * D2[1]);

		ValidVecBRows.push_back(-H[2] * V[2]);
		ValidVecBRows.push_back(-D1[2] * D2[2]);

		ValidImageCount++;
	}

	// Check if we have enough valid homographies for focal length estimation
	if (ValidImageCount < 2)
	{
		UE_LOGF(LogCameraCalibrationSolver, Warning, "InitIntrinsicsFromCanonicalPlane: Insufficient valid homographies (%d) for focal length estimation. Need at least 2.", ValidImageCount);

		// Fallback to basic initialization
		const double Fx = ImageSize.width;
		const double Fy = ImageSize.width;
	...
	// Build matrices only with valid data
	cv::Mat MatrixA = cv::Mat(2 * ValidImageCount, 2, CV_64F, ValidMatrixARows.data());
	cv::Mat VecB = cv::Mat(2 * ValidImageCount, 1, CV_64F, ValidVecBRows.data());
	...
	constexpr double MinFocalLength = 1e-6;

	if (FxFy[0] < MinFocalLength || FxFy[1] < MinFocalLength)
	{
		UE_LOGF(LogCameraCalibrationSolver, Warning, "InitIntrinsicsFromPlane2d: FxFy did not have reasonable values: [%f, %f]. ",
			FxFy[0], FxFy[1]
		);

		FxFy[0] = FMath::Max(FxFy[0], MinFocalLength);
		FxFy[1] = FMath::Max(FxFy[1], MinFocalLength);
	}
	...
	bool bUseCanonicalPlaneInit)
{
	...
	// Use the same canonical plane detection as intrinsics initialization
	if (bUseCanonicalPlaneInit)
	{
	...
	else // Case of not being in a 2d canonical plane
	{
		UE_LOGF(LogCameraCalibrationSolver, Verbose, "Initializing extrinsics for non-planar 3D points using solvePnP");
		
		// For 3D world coordinates, use cv::solvePnP to get initial pose estimate
		// Convert ObjectPoints format: from (3 x NumPoints) to (NumPoints x 3)
		cv::Mat ObjectPoints3x1 = ObjectPoints.reshape(1, NumPoints);
		
		// Convert ImagePoints format: from (1 x NumPoints, 2-channel) to (NumPoints x 1, 2-channel)  
		cv::Mat ImagePoints2x1 = ImagePoints.reshape(2, NumPoints);
		
		try
		{
			bool bSolveSuccess = cv::solvePnP(
				ObjectPoints3x1,    // (NumPoints x 3) object points
				ImagePoints2x1,     // (NumPoints x 1, 2-channel) image points
				CameraMatrix,       // 3x3 camera matrix
				cv::Mat::zeros(5, 1, CV_64F), // No distortion for initial estimate
				Rotation,           // Output: 3x1 rotation vector
				Translation,        // Output: 3x1 translation vector
			...
			if (!bSolveSuccess)
			{
				UE_LOGF(LogCameraCalibrationSolver, Warning, "solvePnP failed, using identity pose");
		...
		{
			UE_LOGF(LogCameraCalibrationSolver, Warning, "solvePnP exception: %ls, using identity pose", ANSI_TO_TCHAR(e.what()));
			cv::Mat RotationMatrix = cv::Mat::eye(3, 3, CV_64F);
			cv::Rodrigues(RotationMatrix, Rotation);
			Translation.setTo(cv::Scalar::all(0));
		}
...
double FOptimizeNodalOffsetSolver::calc(const double* x) const
{
	...
	UE_LOGF(LogCameraCalibrationSolver, VeryVerbose, "Nodal Offset Candidate:  Rotation: (%lf, %lf, %lf, %lf)  Location: (%lf, %lf, %lf)",
	...
	UE_LOGF(LogCameraCalibrationSolver, VeryVerbose, "Reprojection Error: %lf", RootMeanSquareError);

	return RootMeanSquareError;
}

bool ULensDistortionSolverOpenCV::CanUseHomographyInitialization(const cv::Mat& ObjectPoints, double StdDevTolerance)
{
	if (ObjectPoints.channels() == 2)
	{
		return true;
	}
	...
	// Check if any coordinate has low variance, indicating canonical plane (X=0, Y=0, or Z=0)
	cv::Scalar Mean, StdDev;
	cv::meanStdDev(ObjectPoints, Mean, StdDev);
	
	return (StdDev[0] < StdDevTolerance) || (StdDev[1] < StdDevTolerance) || (StdDev[2] < StdDevTolerance);
}

void ULensDistortionSolverOpenCV::InitIntrinsicsFrom3DPoints(
	const cv::Mat& ObjectPoints,
	const cv::Mat& ImagePoints,
	const cv::Mat& NumPoints,
	const cv::Size ImageSize,
	cv::Mat& CameraMatrix)
{
	// For 3D world coordinates, we'll use cv::calibrateCamera to get initial intrinsic estimates
	// This is more robust than homography-based methods for non XY plane points.
	
	const int32 NumImages = NumPoints.cols;
	
	// Convert the input matrices to the format expected by cv::calibrateCamera
	std::vector<std::vector<cv::Point3f>> ObjectPointsVec;
	std::vector<std::vector<cv::Point2f>> ImagePointsVec;
	
	ObjectPointsVec.reserve(NumImages);
	ImagePointsVec.reserve(NumImages);
	
	int32 ObjectPointsIndex = 0;
	for (int32 ImageIndex = 0; ImageIndex < NumImages; ++ImageIndex)
	{
		const int32 NumImagePoints = NumPoints.ptr<int32>()[ImageIndex];
		
		std::vector<cv::Point3f> ObjectPointsInImage;
		std::vector<cv::Point2f> ImagePointsInImage;
		
		ObjectPointsInImage.reserve(NumImagePoints);
		ImagePointsInImage.reserve(NumImagePoints);
		
		for (int32 PointIndex = 0; PointIndex < NumImagePoints; ++PointIndex)
		{
			const cv::Point3d& ObjectPoint = ObjectPoints.at<cv::Point3d>(0, ObjectPointsIndex + PointIndex);
			const cv::Point2d& ImagePoint = ImagePoints.at<cv::Point2d>(0, ObjectPointsIndex + PointIndex);
			
			ObjectPointsInImage.push_back(cv::Point3f(ObjectPoint.x, ObjectPoint.y, ObjectPoint.z));
			ImagePointsInImage.push_back(cv::Point2f(ImagePoint.x, ImagePoint.y));
		}
		
		ObjectPointsVec.push_back(ObjectPointsInImage);
		ImagePointsVec.push_back(ImagePointsInImage);
		
		ObjectPointsIndex += NumImagePoints;
	}
	
	// Use cv::calibrateCamera to get initial estimates
	...
	try
	{
		...
		UE_LOGF(LogCameraCalibrationSolver, Log, "Initialized intrinsics from 3D points: Fx=%.2f, Fy=%.2f, Cx=%.2f, Cy=%.2f", 
			CameraMatrix.at<double>(0, 0), CameraMatrix.at<double>(1, 1), 
			CameraMatrix.at<double>(0, 2), CameraMatrix.at<double>(1, 2));
	}
	catch (const cv::Exception& e)
	{
		UE_LOGF(LogCameraCalibrationSolver, Warning, "cv::calibrateCamera failed for 3D point initialization: %ls", ANSI_TO_TCHAR(e.what()));
