...
private:
	...
	/** Check if the input points lie in a canonical plane suitable for homography */
	bool CanUseHomographyInitialization(const cv::Mat& ObjectPoints, double Tolerance = 1e-6);

	/** Initialize camera intrinsics from 2D plane points using homography */
	void InitIntrinsicsFromCanonicalPlane(
		const cv::Mat& ObjectPoints,
		const cv::Mat& ImagePoints,
		const cv::Mat& NumPoints,
		const cv::Size ImageSize,
	...
	/** Initialize camera intrinsics from 3D world coordinates (not requiring canonical plane like homography) */
	void InitIntrinsicsFrom3DPoints(
		const cv::Mat& ObjectPoints,
		const cv::Mat& ImagePoints,
		const cv::Mat& NumPoints,
		const cv::Size ImageSize,
