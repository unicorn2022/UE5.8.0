...
void FRemoteControlWebInterfaceModule::StartupModule()
{
	...
	{
		UE_LOGF(LogRemoteControlWebInterface, Display, "Remote Control Web Interface is disabled by default when running outside the editor. Use the -RCWebInterfaceEnable flag when launching in order to use it.");
		return;
	}

	if (CVarRCWebInterfaceAutoStart.GetValueOnGameThread() == 0)
	{
		UE_LOGF(LogRemoteControlWebInterface, Display, "Remote Control Web Interface did not launch WebApp because CVar RCWebInterface.AutoStart is set to 0");
		return;
	}
