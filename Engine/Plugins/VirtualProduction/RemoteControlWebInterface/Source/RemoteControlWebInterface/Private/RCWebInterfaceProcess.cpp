...
void FRemoteControlWebInterfaceProcess::Start()
{
	...
	{
		UE_LOGF(LogRemoteControlWebInterface, Warning, "WebApp folder does not exists (%ls)", *Root);
		return;
	}
...
uint32 FRemoteControlWebInterfaceProcess::Run()
{
	...
	auto LogReadPipe = [&](void* InReadPipe) {
		...
		if (ProcessOutput.Len() > 0)
		{
			...
			for (const FString& Line : Lines)
			{
				...
				{
					UE_LOGF(LogRemoteControlWebInterface, Log, "%ls", *Line);
				}
			}
		}
	...
	UE_LOGF(LogRemoteControlWebInterface, Log, "WebApp started, initial launch will take longer as it will be building the WebApp");
	...
	{
		UE_LOGF(LogRemoteControlWebInterface, Log, "WebApp exited");
	}
