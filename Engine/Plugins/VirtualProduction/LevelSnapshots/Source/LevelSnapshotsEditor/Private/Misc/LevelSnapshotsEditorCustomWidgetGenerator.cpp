	...
	const TWeakPtr<FLevelSnapshotsEditorResultsRow>& InDirectParentRow)
{
	...
	{
		UE_LOGF(LogLevelSnapshots, Warning, "%s: Unsupported Property found named '%ls' with FieldPath: %ls", __FUNCTION__, *InFieldPath->GetAuthoredName(), *InFieldPath.ToString());
	...
	if (InObject->IsValidLowLevel())
	{
		...
		if (OutChain.IsSet() && OutChain.GetValue().GetNumProperties() > 0)
		{
			...
			for (int32 ChainItr = 0; ChainItr < OutChain->GetNumProperties(); ChainItr++)
			{
				...
				UE_LOGF(LogLevelSnapshots, Error, "%s: Property named %ls: PropertyChain with length of %i returned but FProperty at index %i from root is null.",
		...
		else
		{
			...
			UE_LOGF(LogLevelSnapshots, Warning,
				"%s: Property named %ls with owner named '%ls' does not have an owning FProperty or an owning UClass.",
