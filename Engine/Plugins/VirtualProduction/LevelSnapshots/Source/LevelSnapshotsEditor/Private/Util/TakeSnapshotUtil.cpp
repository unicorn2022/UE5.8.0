...
namespace UE::LevelSnapshots::Editor
{
	...
	TAutoConsoleVariable<bool> CVarIgnorePIEWorld(TEXT("LevelSnapshots.IgnorePIEWorld"), true, TEXT("Set whether the Take Snapshot will ignore the PIE world and use the editor world context.  If this is false, then the PIE world will be used to grab a snapshot."));
	
	UWorld* GetWorld(bool bIgnorePIE)
	{
	...
	{
		UWorld* World = GetWorld(CVarIgnorePIEWorld.GetValueOnAnyThread());
		if (!ensure(World))
		{
			return;
		}
...
ULevelSnapshot* SnapshotEditor::TakeLevelSnapshotAndSaveToDisk(UWorld* World, const FString& FileName, const FString& FolderPath, const FString& Description, bool bShouldCreateUniqueFileName, bool bSaveAsync)
{
    ...
    if (bProceedWithSave && !bShouldCreateUniqueFileName)
    {
    	...
    	{
    		UE_LOGF(LogLevelSnapshots, Error, "Asset already exists and is not a snapshot. Aborting.")
	...
	{
		UE_LOGF(LogLevelSnapshots, Error, "Invalid package name. Error: %ls", *ErrorReason.ToString());
		bProceedWithSave = false;
	}
...
void SnapshotEditor::GenerateThumbnailForSnapshotAsset(ULevelSnapshot* Snapshot)
{
	...
	if (GEditor && ensureMsgf(SnapshotAssetData.Num(), TEXT("Failed to find asset data for asset we just saved. Investigate!")))
	{
...
FAutoConsoleCommandWithWorldAndArgs TakeLevelSnaphot(
	TEXT("LevelSnapshots.TakeLevelSnapshot"),
	TEXT("Take a new level snapshot of the current world.")
	TEXT("Arg1 - Name of the level snapshot asset.\n")
	TEXT("Arg2 - Folder path to store the snapshot, e.g. /Game/LevelSnapshots. \n"),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda(
		[](const TArray<FString>& Args, UWorld* World)
		{
			if (Args.Num() == 2)
			{
				FString Description(TEXT("Snapshot taken from console command."));
				
				SnapshotEditor::TakeLevelSnapshotAndSaveToDisk(World, Args[0], Args[1], Description, true, false);
			}
			else
			{
				UE_LOGF(LogLevelSnapshots, Warning, "Incorrect number of arguments for the snapshot command.");				
			}
		}
	)
);
