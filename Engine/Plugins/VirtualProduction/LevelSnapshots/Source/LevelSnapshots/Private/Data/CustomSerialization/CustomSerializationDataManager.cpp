...
int32 UE::LevelSnapshots::Private::FCustomSerializationDataWriter::AddSubobjectSnapshot(UObject* Subobject)
{
	if (!ensure(Subobject) || !ensure(Subobject->IsIn(SerializedObject)))
	{
		...
		UE_LOGF(LogLevelSnapshots, Error, "%ls is not in %ls!", *SubobjectPath, *SerializedObjectPath);
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogLevelSnapshots, Error, "You tried to register an object (%ls) which was already found by standard Level Snapshot serialisation. Is your subobject referenced by an property with a CPF_Edit flag?", *Subobject->GetPathName());
		return ExistingSubobjectIndex;
	}
	if (!ensure(ExistingSubobjectIndex == INDEX_NONE))
	{
		UE_LOGF(LogLevelSnapshots, Error, "You tried to register the same subobject (%ls) twice.", *Subobject->GetPathName());
		return ExistingSubobjectIndex;
	}
