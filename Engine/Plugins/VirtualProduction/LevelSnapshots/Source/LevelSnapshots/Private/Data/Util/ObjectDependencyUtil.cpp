...
namespace UE::LevelSnapshots::Private::Internal
{
	...
	static UObject* CreateSubobjectForSnapshotWorld(FWorldSnapshotData& WorldData, FSnapshotDataCache& Cache, int32 ObjectPathIndex, const FSoftObjectPath& SnapshotPathToSubobject, const FProcessObjectDependency& ProcessObjectDependency, const FString& LocalisationNamespace)
	{
		...
		if (!SubobjectOuter)
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Failed to create '%ls' because its outer could not be created.", *SnapshotPathToSubobject.ToString());
			return nullptr;
		}
		...
		const bool bSubobjectNameIsTaken = [SubobjectOuter, SubobjectName]()
		{
			...
			GetObjectsWithOuter(SubobjectOuter, Subobjects, EGetObjectsFlags::IncludeNestedObjects);
		...
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Failed to create '%ls' because subobject name was already taken.", *SnapshotPathToSubobject.ToString());
			return nullptr;
		}
		...
		if (!Archetype)
		{
			...
			UE_LOGF(LogLevelSnapshots, Warning, "Failed to create '%ls' because class was '%ls' not found. Maybe it was removed?", *SnapshotPathToSubobject.ToString(), *ClassPath.ToString());
			return nullptr;
		}
	...
	static UObject* SerializeOrCreateSubobjectForEditorWorld(FWorldSnapshotData& WorldData, FSubobjectSnapshotData& SubobjectData, FSnapshotDataCache& Cache, UObject* ResolvedObject, const FSoftObjectPath& OriginalObjectPath, const FString& LocalisationNamespace, const FPropertySelectionMap& SelectionMap)
	{
		...
		if (!ExpectedClass)
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Class '%ls' could not be resolved.", *ClassPath.ToString());
			return nullptr;
		}
		...
		{
			UE_LOGF(LogLevelSnapshots, Error, "Object depends on a component that must be recreated and is owned by another actor. Support for this is not yet implemented.");
			return nullptr;
		}
		...
		else 
		{
			...
			if (!Outer)
			{
				UE_LOGF(LogLevelSnapshots, Error, "Failed to create object '%ls' because its outer '%ls' could not be resolved", *OriginalObjectPath.ToString(), *WorldData.SerializedObjectReferences[SubobjectData.OuterIndex].ToString());
				return nullptr;
			}
			...
			if (!Archetype)
			{
				UE_LOGF(LogLevelSnapshots, Error, "Failed to create object '%ls' because its archetype could not be resolved", *OriginalObjectPath.ToString());
				return nullptr;	
			}
	...
	static void EnsureSubobjectIsSerialized(FWorldSnapshotData& WorldData, FSnapshotDataCache& Cache, UObject* ExistingObject, int32 ObjectPathIndex, const FProcessObjectDependency& ProcessObjectDependency, const FString& LocalisationNamespace)
	{
		...
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Skipping serialisation of subobject because classes are different. Snapshot object '%ls' will not contain values saved in snapshot.", *ExistingObject->GetName());
			return;
		}
...
UObject* UE::LevelSnapshots::Private::ResolveObjectDependencyForEditorWorld(FWorldSnapshotData& WorldData, FSnapshotDataCache& Cache, int32 ObjectPathIndex, const FString& LocalisationNamespace, const FPropertySelectionMap& SelectionMap)
{
	...
	UE_LOGF(LogLevelSnapshots, Warning, "Trying to resolve a subobject but found no associated object data.");
	return ResolvedObject;
}
...
int32 UE::LevelSnapshots::Private::AddCustomSubobjectDependency(FWorldSnapshotData& WorldData, UObject* ReferenceFromOriginalObject)
{
	...
	{
		UE_LOGF(LogLevelSnapshots, Warning, "Object %ls was already added as dependency. Investigate", *ReferenceFromOriginalObject->GetName());
		UE_DEBUG_BREAK();
	}
