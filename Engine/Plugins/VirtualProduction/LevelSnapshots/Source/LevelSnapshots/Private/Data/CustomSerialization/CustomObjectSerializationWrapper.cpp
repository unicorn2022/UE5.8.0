		...
		)
	{
		...
		return FRestoreObjectScope([SnapshotObject, SerializationDataGetter, &WorldData, &Cache, &ProcessObjectDependency, LocalisationSnapshotPackage, SerializationDataReader, CustomSerializer]()
		{
			for (int32 i = 0; i < SerializationDataReader.GetNumSubobjects(); ++i)
			{
				...
				{
					UE_LOGF(LogLevelSnapshots, Warning, "FindOrRecreateSubobjectInSnapshotWorld did not return any valid subobject. Skipping subobject restoration...");
					return;
				}
		...
		)
	{
		...
		return FRestoreObjectScope([SnapshotObject, EditorObject, &WorldData, &Cache, &SelectionMap, LocalisationSnapshotPackage, SerializationDataGetter, SerializationDataReader, CustomSerializer]()
		{
			...
			{	
				...
				if (!SnapshotSubobject || !ensure(SnapshotSubobject->IsIn(SnapshotObject)))
				{
					UE_LOGF(LogLevelSnapshots, Warning, "FindOrRecreateSubobjectInSnapshotWorld did not return any valid snapshot subobject. Skipping this subobject for %ls", *EditorObject->GetPathName());
					continue;
				}
				if (!EditorSubobject || !ensure(EditorSubobject->IsIn(EditorObject)))
				{
					UE_LOGF(LogLevelSnapshots, Warning, "FindOrRecreateSubobjectInSnapshotWorld did not return any valid editor subobject. Skipping this subobject for %ls", *EditorObject->GetPathName());
					continue;
				}
				...
				UE_LOGF(LogLevelSnapshots, Warning, "Editor subobject %ls was not restored", *EditorSubobject->GetPathName());	
			}
