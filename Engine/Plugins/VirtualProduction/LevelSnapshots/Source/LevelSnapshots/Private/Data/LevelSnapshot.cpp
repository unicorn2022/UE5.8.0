...
void ULevelSnapshot::ApplySnapshotToWorld(UWorld* TargetWorld, const FPropertySelectionMap& SelectionSet)
{
	...
	UE_LOGF(LogLevelSnapshots, Log, "Applying snapshot %ls to world %ls. %ls", *GetPathName(), *TargetWorld->GetPathName(), *GenerateDebugLogInfo());
	UE_CLOGF(MapPath != FSoftObjectPath(TargetWorld), LogLevelSnapshots, Log, "Snapshot was taken for different world called '%ls'", *MapPath.ToString());
	ON_SCOPE_EXIT
	{
		UE_LOGF(LogLevelSnapshots, Log, "Finished applying snapshot");
	};
...
bool ULevelSnapshot::SnapshotWorld(UWorld* TargetWorld)
{
	...
	{
		UE_LOGF(LogLevelSnapshots, Warning, "Unable To Snapshot World as World was invalid");
		return false;
	}
...
#endif // WITH_EDITOR
		UE_LOGF(LogLevelSnapshots, Warning, "Level snapshots currently only support editors. Snapshots taken in other world types are experimental any may not function as expected.");
	}
...
bool ULevelSnapshot::HasChangedSinceSnapshotWasTaken(AActor* WorldActor)
{
	...
	if (!SavedActorData)
	{
		UE_LOGF(LogLevelSnapshots, Warning, "No data found for actor %ls", *ActorPath.ToString());
		return false;
	}
	...
	const bool bHasChanged = [&]()
	{
		...
		if (!bHasHashInfo || !bNeedsHash || !UE::LevelSnapshots::Private::HasMatchingHash(SavedActorData->Hash, WorldActor))
		{
			...
			UE_CLOGF(!DeserializedActor, LogLevelSnapshots, Warning, "Failed to load snapshot actor %ls. Actor will not be restored.", *WorldActor->GetPathName());
...
void ULevelSnapshot::DiffWorld(UWorld* World, FActorConsumer HandleMatchedActor, FActorPathConsumer HandleRemovedActor, FActorConsumer HandleAddedActor)
{
	...
	UE_LOGF(LogLevelSnapshots, Log, "Diffing snapshot %ls in world %ls. %ls", *GetPathName(), *World->GetPathName(), *GenerateDebugLogInfo());
	ON_SCOPE_EXIT
	{
		UE_LOGF(LogLevelSnapshots, Log, "Finished diffing snapshot");
	};
	...
	TSet<FSoftObjectPath> LoadedLevels;
	{
		...
		for (ULevel* Level : World->GetLevels())
		{
			...
			for (AActor* ActorInLevel : Level->Actors)
			{
				...
				{
					UE_CLOGF(!ExclusionReason.IsEmpty(), LogLevelSnapshots, Warning, "Actor %ls was not considered for removal. Reason: %ls", *ActorInLevel->GetPathName(), *ExclusionReason.ToString());
				}
			}
		}
	}
	...
	// Try to find world actors and call appropriate callback
	{
		...
		SerializedData.ForEachOriginalActor([this, World, &HandleMatchedActor, &HandleRemovedActor, &HandleAddedActor, &AllActors, &LoadedLevels, Settings, &DebugActorName, &SortedItems](const FSoftObjectPath& OriginalActorPath, const FActorSnapshotData& SavedData)
		{
			...
			{
				UE_LOGF(LogLevelSnapshots, Log, "Skipping actor %ls because level %ls is not loaded or does not exist (see Levels window).", *OriginalActorPath.ToString(), *LevelPath.ToString());
				return;
			}
			...
			if (!ActorClass)
			{
				UE_LOGF(LogLevel, Warning, "Cannot find class %ls. Saved actor %ls will not be restored.", *ClassPath.ToString(), *OriginalActorPath.ToString());
				return;
			}
			...
			if (bWasRemovedFromWorld)
			{
				...
				UE_CLOGF(!ExclusionReason.IsEmpty(), LogLevelSnapshots, Warning, "Actor %ls was not considered for recreation. Reason: %ls", *OriginalActorPath.ToString(), *ExclusionReason.ToString());
				return;
			}
			...
			else if (UE::LevelSnapshots::Restorability::ShouldConsiderMatchedActorForRestoration({ *ResolvedActor, SerializedData }, &ExclusionReason))
			{
				...
				UE_LOGF(LogLevelSnapshots, VeryVerbose, "Matched actor %ls", *OriginalActorPath.ToString());
				HandleMatchedActor.Execute(Cast<AActor>(ResolvedActor));
			}
			
			UE_CLOGF(!ExclusionReason.IsEmpty(), LogLevelSnapshots, Warning, "Actor %ls was skipped for modification. Reason: %ls", *OriginalActorPath.ToString(), *ExclusionReason.ToString());
		});
	}
}
...
void ULevelSnapshot::EnsureWorldInitialised()
{
	...
	if (RootSnapshotWorld == nullptr)
	{
		...
		// replace OnAssetsPreDelete and OnPreDestructiveAssetActionWithArgs with PreGarbageCollectConditionalBeginDestroy.
		...
		FEditorDelegates::OnPreDestructiveAssetActionWithArgs.AddWeakLambda(this, [this](FOnPreDestructiveAssetActionArgs& Args)
		{
			const bool bHasInitializedWorld = RootSnapshotWorld != nullptr;
			if (bHasInitializedWorld && Args.Objects.Contains(this))
			{
				DestroyWorld();
			}
		});
...
void ULevelSnapshot::DestroyWorld()
{
	if (ensureAlwaysMsgf(RootSnapshotWorld, TEXT("World was already destroyed.")))
	{
		...
		FEditorDelegates::OnPreDestructiveAssetActionWithArgs.RemoveAll(this);
#endif
