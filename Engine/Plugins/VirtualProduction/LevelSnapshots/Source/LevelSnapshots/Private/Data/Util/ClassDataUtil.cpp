...
namespace UE::LevelSnapshots::Private
{
	...
	TOptional<TNonNullPtr<FClassSnapshotData>> GetObjectArchetypeData(FWorldSnapshotData& WorldData, FClassDataIndex ClassIndex, FSnapshotDataCache& Cache, const FSubobjectArchetypeFallbackInfo& FallbackInfo)
	{
		...
		{
			UE_LOGF(LogLevelSnapshots, Error, "Snapshot data is corrupt. ClassIndex %d is out of bounds for ClassData of size %d.", ClassIndex, WorldData.ClassData.Num());
			return {};
		}
		...
		if (!Class)
		{
			UE_LOGF(LogLevelSnapshots, Error, "Failed to load class %ls", *SavedClassData.ClassPath.ToString());
			return {};
		}
		...
		UE_LOGF(LogLevelSnapshots, Error, "Failed to determine archetype for class: %ls, outer: %ls name: %ls", *SavedClassData.ClassPath.ToString(), *FallbackInfo.SubobjectOuter->GetPathName(), *FallbackInfo.SubobjectName.ToString())
		return {};
	}
	...
	void SerializeClassDefaultsIntoActor(AActor* Actor, FWorldSnapshotData& WorldData, FClassDataIndex ClassIndex, FSnapshotDataCache& Cache)
	{
		...
		else
		{
			UE_LOGF(LogLevelSnapshots, Warning,
				"No CDO saved for class '%ls'. If you changed some class default values for this class, then the affected objects will have the latest values instead of the class defaults at the time the snapshot was taken. Should be nothing major to worry about.",
