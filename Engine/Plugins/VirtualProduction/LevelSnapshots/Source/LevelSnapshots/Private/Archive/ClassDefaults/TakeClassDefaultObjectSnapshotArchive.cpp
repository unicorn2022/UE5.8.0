...
bool UE::LevelSnapshots::Private::FTakeClassDefaultObjectSnapshotArchive::ShouldSkipProperty(const FProperty* InProperty) const
{
	...
	UE_CLOGF(bIsSkippedProperty, LogLevelSnapshots, Verbose, "Skipping property %ls of object %ls because it references a subobject.", *InProperty->GetName(), *GetSerializedObject()->GetPathName());
...
{
#if UE_BUILD_DEBUG
	UE_LOGF(LogLevelSnapshots, VeryVerbose, "FTakeClassDefaultObjectSnapshotArchive: %ls (%ls)", *InSerializedObject->GetPathName(), *InSerializedObject->GetClass()->GetPathName());
#endif
}
