	...
	TOptional<FClassDataIndex> ClassIndex)
{
	...
	UE_LOGF(LogLevelSnapshots, Verbose, "Applying to existing object %ls (class %ls)", *InOriginalObject->GetPathName(), *InOriginalObject->GetClass()->GetPathName());
...
void UE::LevelSnapshots::Private::FApplySnapshotToEditorArchive::ApplyToEditorWorldObjectRecreatedWithArchetype(
	...
	const FPropertySelectionMap& InSelectionMapForResolvingSubobjects)
{
	UE_LOGF(LogLevelSnapshots, Verbose, "Applying to recreated object %ls (class %ls)", *InOriginalObject->GetPathName(), *InOriginalObject->GetClass()->GetPathName());
...
{
#if UE_BUILD_DEBUG
	UE_LOGF(LogLevelSnapshots, VeryVerbose, "FApplySnapshotToEditorArchive: %ls (%ls)", *InOriginalObject->GetPathName(), *InOriginalObject->GetClass()->GetPathName());
#endif
}
