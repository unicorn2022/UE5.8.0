...
bool UE::LevelSnapshots::Private::FSnapshotArchive::ShouldSkipProperty(const FProperty* InProperty) const
{
	...
	UE_LOGF(LogLevelSnapshots, VeryVerbose, "ShouldSkipProperty(%ls %ls)", *PropertyChain, InProperty->IsNative() ? TEXT("C++") : TEXT("BP"));
#endif
...
void UE::LevelSnapshots::Private::FSnapshotArchive::FixUpReference(UObject*& Value) const
{
	if (IsValid(Value))
	{
		...
		if (ResolvedOwningLevel && ResolvedOwningLevel != SerializedObjectOwningLevel)
		{
			...
			UE_LOGF(LogLevelSnapshots, Warning, "Resolved world object %ls for %ls. Referencing objects from other world / levels is not permitted: Nulling reference.", *Value->GetPathName(), *SerializedObject->GetPathName());
			Value = nullptr;
		}
	}
}
