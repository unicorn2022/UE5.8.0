...
namespace UE::LevelSnapshots::Private::Internal::Restore
{
	...
	static void DeserializeIntoEditorWorldActor(AActor* OriginalActor, FActorSnapshotData& ActorData, FWorldSnapshotData& WorldData, FSnapshotDataCache& Cache, UPackage* InLocalisationSnapshotPackage, FSerializeActor SerializeActor, FSerializeComponent SerializeComponent)
	{
		...
		if (!Deserialized)
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Failed to serialize into actor %ls. Skipping...", *OriginalActor->GetName());
			return;
		}
...
#if WITH_EDITOR
		UE_LOGF(LogLevelSnapshots, Verbose, "ActorLabel is \"%ls\" for \"%ls\"", *OriginalActor->GetActorLabel(), *OriginalActor->GetPathName());
#endif
		...
		Internal::Restore::DeserializeComponents(OriginalActor, ActorData, WorldData,
				...
				)
		    {
				...
				UE_CLOGF(!DeserializedCompCounterpart, LogLevelSnapshots, Warning, "Failed to find component called %ls on temp deserialized snapshot actor. Skipping component...", *OriginalCompName.ToString())
...
{
	UE_LOGF(LogLevelSnapshots, Verbose, "========== Apply existing %ls ==========", *OriginalActor->GetPathName());
...
{
	UE_LOGF(LogLevelSnapshots, Verbose, "========== Apply recreated %ls ==========", *OriginalActor->GetPathName());
