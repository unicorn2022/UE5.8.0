...
namespace UE::LevelSnapshots::Private::Internal
{
		...
		struct Local
		{
			static void RecreateRootComponentIfInstanced(AActor* OriginalActor, AActor* SnapshotActor)
			{
				...
				if (bSnapshotIsMissingRoot && bSnapshotWasSavedWithoutRootComponentData)
				{
					...
					UE_CLOGF(SnapshotRoot == nullptr, LogLevelSnapshots, Warning, "Failed to recreate instanced root component for %ls's snapshot counterpart", *OriginalActor->GetPathName());
					if (SnapshotRoot)
					{
			...
			static void HandleAttachParentNotSaved(AActor* OriginalActor, AActor* SnapshotActor, FWorldSnapshotData& WorldData, FSnapshotDataCache& Cache, UPackage* InLocalisationSnapshotPackage)
			{
				...
				{
					UE_LOGF(LogLevelSnapshots, Warning,
						"Actor %ls had no attach parent saved. Its current attach parent %ls was added after the snapshot was added so we when this actor is restored, we'll fallback to unparenting it.",
				...
				{
					UE_LOGF(LogLevelSnapshots, Warning, "Failed to fix up AttachParent for %ls's snapshot counterpart", *OriginalActor->GetPathName());
				}
			}
		};
		...
		UPackage* InLocalisationSnapshotPackage)
	{
		...
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Components created dynamically in the construction script are not supported (%ls). Skipping...", *ComponentToDeserializeInto->GetPathName());
			return;
		}
	...
	static TOptional<TNonNullPtr<AActor>> GetDeserialized(const FSoftObjectPath& OriginalActorPath, FWorldSnapshotData& WorldData, FSnapshotDataCache& Cache, UPackage* InLocalisationSnapshotPackage, bool bWarnIfNotFound)
	{
		...
		UE_LOGF(LogLevelSnapshots, Verbose, "========== Get Deserialized %ls ==========", *OriginalActorPath.ToString());
...
#if WITH_EDITOR
			UE_LOGF(LogLevelSnapshots, Verbose, "ActorLabel is \"%ls\" for \"%ls\" (editor object path \"%ls\")", *PreallocatedActor->GetActorLabel(), *PreallocatedActor->GetPathName(), *OriginalActorPath.ToString());
#endif
		}
...
TOptional<TNonNullPtr<UClass>> UE::LevelSnapshots::Private::GetActorClass(const FSoftObjectPath& OriginalActorPath, FWorldSnapshotData& WorldData, bool bWarnIfNotFound)
{
	...
	if (!ActorData)
	{
		UE_CLOGF(bWarnIfNotFound, LogLevelSnapshots, Warning, "Failed to get the UClass for actor %ls due to missing data", *OriginalActorPath.ToString());
		return {};
	}
	...
	if (!ActorClass)
	{
		UE_LOGF(LogLevelSnapshots, Warning, "Failed to load the UClass for actor %ls.", *OriginalActorPath.ToString());
		return {};
	}
...
TOptional<TNonNullPtr<AActor>> UE::LevelSnapshots::Private::GetPreallocated(const FSoftObjectPath& OriginalActorPath, FWorldSnapshotData& WorldData, FSnapshotDataCache& Cache, bool bWarnIfNotFound)
{
	...
	if (!ActorCache.CachedSnapshotActor.IsValid())
	{
		...
		if (!ActorData)
		{
			UE_CLOGF(bWarnIfNotFound, LogLevelSnapshots, Warning, "Failed to allocate %ls due to missing data", *OriginalActorPath.ToString());
			return {};
		}
		...
		if (!ActorClass)
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Failed to allocate %ls due to missing class.", *OriginalActorPath.ToString());
			return {};
		}
		...
		UE_CLOGF(!bSpawnWasSuccess, LogLevelSnapshots, Error, "Failed to allocate %ls due to missing class.", *OriginalActorPath.ToString());
