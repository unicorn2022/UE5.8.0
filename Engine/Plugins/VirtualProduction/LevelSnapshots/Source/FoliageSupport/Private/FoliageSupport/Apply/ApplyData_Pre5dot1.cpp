...
namespace UE::LevelSnapshots::Foliage::Private
{
		...
		const TMap<FName, UFoliageType*> PreexistingComponentToFoliageType)
	{
		...
		for (auto AssetIt = FoliageAssets.CreateConstIterator(); AssetIt; ++AssetIt)
		{
			...
			if (!FoliageType)
			{
				UE_LOGF(LogLevelSnapshots, Warning, "Foliage type %ls is missing. Component %ls will not be restored.", *AssetIt->Key.ToString(), *FoliageInfoData.GetComponentName().Get(NAME_None).ToString());
				continue;
			}
		...
		const TMap<FName, UFoliageType*> PreexistingComponentToFoliageType)
	{
		...
		for (const FSubobjectFoliageInfoData& InstanceData : SubobjectData)
		{
			...
			if (!FoliageType)
			{
				UE_LOGF(LogLevelSnapshots, Error, "Failed to recreate foliage type. Skipping...");
				continue;
			}
			...
			{
				UE_LOGF(LogLevelSnapshots, Warning, "Skipping restoration of foliage item due to errors.");
				continue;
			}
	...
	void FInstancedFoliageActorData::ApplyData_Pre5dot2(FArchive& Archive, AInstancedFoliageActor* FoliageActor, const FPropertySelectionMap& SelectedProperties, bool bWasRecreated) const
	{
		...
		{
			UE_CLOGF(!bIsSafeData, LogLevelSnapshots, Warning, "Data saved before 5.1 may crash the editor (unlikely).");
		...
		{
			UE_LOGF(LogLevelSnapshots, Warning, "Use \"LevelSnapshots.DangerousAllowFoliageDataPre5dot1 true\" to enable restoring foliage data from before 5.2 (may crash).");
		}
	}
}
