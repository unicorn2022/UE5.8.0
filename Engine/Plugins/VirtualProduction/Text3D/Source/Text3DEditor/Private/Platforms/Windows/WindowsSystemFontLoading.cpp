...
namespace UE::Text3D::Private::Fonts
{
	...
	void ListAvailableFontFiles()
	{
		...
		UE_LOGF(LogText3DEditor, Log, "Font Manager Subsystem: listing system fonts and their typefaces:");
		...
		for (const TPair<FString, FText3DFontFamily>& FontsInfoPair : FontsInfoMap)
		{
			...
			UE_LOGF(LogText3DEditor, Log, "== Font: %ls ==", *FontParameters.FontFamilyName);

			int32 FontFaceIndex = 0;
			for (const TPair<FString, FString>& FontFaceName : FontParameters.FontFacePaths)
			{
				UE_LOGF(LogText3DEditor, Log, "\t\tFace Name: %ls found at %ls", *FontFaceName.Key, *FontFaceName.Value);
				FontFaceIndex++;
			}
		}
	}
