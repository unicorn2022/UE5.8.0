...
const FText3DEditorFont* UText3DEditorFontSubsystem::FindEditorFont(const UFont* InFont) const
{
	...
	{
		UE_LOGF(LogText3DEditor, Warning, "Could not retrieve sanitized font name %ls", *InFont->GetName());
		return nullptr;
	}
...
bool UText3DEditorFontSubsystem::RegisterProjectFont(UFont* InFont)
{
	...
	{
		UE_LOGF(LogText3DEditor, Log, "Ignoring distance field font %ls", *InFont->GetName());
		return false;
	}

	if (!InFont->IsAsset())
	{
		UE_LOGF(LogText3DEditor, Log, "Cannot register font that is not an asset %ls", *InFont->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogText3DEditor, Log, "No composite font found for font %ls", *InFont->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogText3DEditor, Warning, "Could not retrieve font style for %ls", *InFont->GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogText3DEditor, Warning, "Could not retrieve font faces for %ls", *InFont->GetName());
		return false;
	}
