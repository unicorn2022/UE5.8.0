...
public:
	...
	void Update(const UE::Text3D::Renderer::FUpdateParameters& InParameters);
	...
	/** Calls the given functor for every primitive the renderer implementation manages */
	void IterateManagedPrimitives(TFunctionRef<void(TNotNull<const UPrimitiveComponent*>)> InFunc) const;

	/** Get the mesh type that this renderer uses */
	virtual EText3DMeshType GetMeshType() const PURE_VIRTUAL(UText3DRendererBase::GetMeshType, return EText3DMeshType::Unknown; );

	/** Get the implementation name for debug purposes */
	virtual FName GetFriendlyName() const PURE_VIRTUAL(UText3DRendererBase::GetFriendlyName, return NAME_None; );
...
protected:
	...
	virtual void OnCreate() PURE_VIRTUAL(UText3DRendererBase::OnCreate, );

	/** Update rendering state of text characters, called when render state is outdated */
	virtual void OnUpdate(const UE::Text3D::Renderer::FUpdateParameters& InParameters) PURE_VIRTUAL(UText3DRendererBase::OnUpdate, );

	/** Clear rendering state and remove all visible characters */
	virtual void OnClear() PURE_VIRTUAL(UText3DRendererBase::OnClear, );

	/** Destroy and clean the implementation components, called on destroy or deactivation */
	virtual void OnDestroy() PURE_VIRTUAL(UText3DRendererBase::OnDestroy, );

	/** Calculate the bounds of the rendered text */
	virtual FBox OnCalculateBounds() const PURE_VIRTUAL(UText3DRendererBase::OnCalculateBounds, return FBox(ForceInitToZero); );

	/** Calls the given functor for every primitive the renderer implementation manages */
	virtual void OnIterateManagedPrimitives(TFunctionRef<void(TNotNull<const UPrimitiveComponent*>)> InFunc) const
	{
	}
