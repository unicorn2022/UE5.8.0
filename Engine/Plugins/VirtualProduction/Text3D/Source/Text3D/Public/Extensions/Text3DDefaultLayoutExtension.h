...
public:
	...
	/** Get the current method of shaping text */
	UFUNCTION(BlueprintPure, Category = "Text3D|Layout")
	ETextShapingMethod GetTextShapingMethod() const
	{
		return TextShapingMethod;
	}

	/** Set the method to shape text */
	UFUNCTION(BlueprintCallable, Category = "Text3D|Layout")
	void SetTextShapingMethod(ETextShapingMethod InTextShapingMethod);
...
protected:
	...
	virtual void PrepareBuild(EText3DRendererFlags InUpdateFlags) override;
	...
	void OnTextShapingMethodChanged();
	...
	UPROPERTY(EditAnywhere, Getter, Setter, Category = "Layout", meta = (EditCondition = "bUseMaxWidth", DisplayAfter="bUseMaxWidth", ClampMin = 1, AllowPrivateAccess = "true"))
	float MaxWidth = 500.f;

	/** Dictates how to handle the text if it exceeds the max width */
	UPROPERTY(EditAnywhere, Getter, Setter, Category = "Layout", meta = (EditCondition="bUseMaxWidth", DisplayAfter="MaxWidth", AllowPrivateAccess = "true"))
	EText3DMaxWidthHandling MaxWidthBehavior = EText3DMaxWidthHandling::Scale;

	/** Sets a maximum height to the 3D Text */
	UPROPERTY(EditAnywhere, Getter, Setter, Category = "Layout", meta = (EditCondition = "bUseMaxHeight", DisplayAfter="bUseMaxHeight", ClampMin = 1, AllowPrivateAccess = "true"))
	float MaxHeight = 500.0f;

	/** Enables a maximum width to the 3D Text */
	UPROPERTY(EditAnywhere, Getter = "GetUseMaxWidth", Setter = "SetUseMaxWidth", Category = "Layout", meta = (AllowPrivateAccess = "true"))
	bool bUseMaxWidth = false;

	/** Enables a maximum height to the 3D Text */
	UPROPERTY(EditAnywhere, Getter = "GetUseMaxHeight", Setter = "SetUseMaxHeight", Category = "Layout", meta = (AllowPrivateAccess = "true"))
	...
	/** The method to use for shaping the text */
	UPROPERTY(EditAnywhere, Getter, Setter, Category = "Layout", meta = (AllowPrivateAccess = "true"), AdvancedDisplay)
...
private:
	...
	/** Temporary storage for the glyph handles while text is built */
	TArray<UE::Text3D::Geometry::FCachedFontFaceGlyphHandle> GlyphHandles;
};