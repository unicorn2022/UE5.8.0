...
#include "Subsystems/Text3DBuildSystem.h"
...
void UText3DComponent::OnComponentDestroyed(bool bInDestroyingHierarchy)
{
	...
	if (UText3DBuildSystem* BuildSystem = UText3DBuildSystem::Get(this))
	{
		BuildSystem->RemoveTextBuild(this);
	}
...
void UText3DComponent::PostLoad()
{
	...
	// Never perform immediate updates in PostLoad
	const bool bImmediateUpdate = false;
	RequestUpdate(EText3DRendererFlags::All, bImmediateUpdate);
}

void UText3DComponent::RequestUpdate(EText3DRendererFlags InFlags, bool bInImmediateUpdate)
{
	if (InFlags != EText3DRendererFlags::None && !IsTemplate(RF_ClassDefaultObject))
	{
		if (IsRegistered())
		{
			if (UText3DBuildSystem* const BuildSystem = UText3DBuildSystem::Get(this))
			{
				BuildSystem->RequestTextBuild(this, InFlags, bInImmediateUpdate);
			}
		}
		else
		{
			EnumAddFlags(PreRegisterUpdateFlags, InFlags);
		}
	}
}
...
{
	Super::OnRegister();
	RequestUpdate(PreRegisterUpdateFlags);
}
...
TStructOnScope<FActorComponentInstanceData> UText3DComponent::GetComponentInstanceData() const
{
	return MakeStructOnScope<FActorComponentInstanceData, FText3DComponentInstanceData>(this);
}
...
void UText3DComponent::Serialize(FArchive& InArchive)
{
	...
	{
		UE_LOGF(LogText3D, Log, "Text3D : Migrating from %i to %i version", Version, FText3DComponentVersion::LatestVersion)
...
UText3DRendererBase* UText3DComponent::GetTextRenderer(TSubclassOf<UText3DRendererBase> InRendererClass) const
{
	if (!TextRenderer || !InRendererClass.Get())
	{
		return nullptr;
	}
	
	return TextRenderer->IsA(InRendererClass.Get()) ? TextRenderer : nullptr;
}
...
void UText3DComponent::OnTextRendererClassChanged()
{
	...
	if (TextRenderer && TextRenderer->GetClass() == TextRendererClass && TextRenderer->GetOuter() == this)
	{
		return;
	}
	...
	auto TrashObject = [](UObject* InObject)
		{
			...
			const FString UniqueObjectName = MakeUniqueObjectName(TransientPackage, InObject->GetClass(), InObject->GetFName()).ToString();
			InObject->Rename(*UniqueObjectName, TransientPackage, REN_DontCreateRedirectors|REN_NonTransactional|REN_DoNotDirty);
	...
	if (TextRenderer)
	{
		...
		TextRenderer->Clear();
		TextRenderer->Destroy();
		TrashObject(TextRenderer);
		TextRenderer = nullptr;
	}

	const FName RendererName = RendererCDO->GetFriendlyName();

	// Trash any existing renderer, instead of re-using it. It is likely in an invalid state.
	TrashObject(FindObject<UText3DRendererBase>(this, RendererName.ToString()));
	TextRenderer = NewObject<UText3DRendererBase>(this, TextRendererClass.Get(), RendererName, RF_Public);

	const AActor* ContextActor = GetOwner();
	UE_LOGF(LogText3D, Log, "%ls : Text3D renderer changed : old %ls - new %ls", !!ContextActor ? *ContextActor->GetActorNameOrLabel() : TEXT("Invalid owner"), *OldRendererName.ToString(), *TextRenderer->GetFriendlyName().ToString());
...
bool UText3DComponent::GatherExtensions(TAdderReserverRef<UText3DExtensionBase*> OutExtensions) const
{
	// Let's make sure text extension subobjects are valid and usable !
	if (!TokenExtension)
	{
		UE_LOGF(LogText3D, Warning, "Invalid Text3D Token extension, cannot proceed !")
		return false;
	}

	if (!StyleExtension)
	{
		UE_LOGF(LogText3D, Warning, "Invalid Text3D Style extension, cannot proceed !")
		return false;
	}

	if (!CharacterExtension)
	{
		UE_LOGF(LogText3D, Warning, "Invalid Text3D Character extension, cannot proceed !")
		return false;
	}

	if (!GeometryExtension)
	{
		UE_LOGF(LogText3D, Warning, "Invalid Text3D Geometry extension, cannot proceed !")
		return false;
	}

	if (!LayoutExtension)
	{
		UE_LOGF(LogText3D, Warning, "Invalid Text3D Layout extension, cannot proceed !")
		return false;
	}

	if (!MaterialExtension)
	{
		UE_LOGF(LogText3D, Warning, "Invalid Text3D Material extension, cannot proceed !")
		return false;
	}

	if (!RenderingExtension)
	{
		UE_LOGF(LogText3D, Warning, "Invalid Text3D Rendering extension, cannot proceed !")
		return false;
	}

	OutExtensions.Reserve(7 + LayoutEffects.Num());
	OutExtensions.Add(TokenExtension);
	OutExtensions.Add(StyleExtension);
	OutExtensions.Add(CharacterExtension);
	OutExtensions.Add(GeometryExtension);
	OutExtensions.Add(LayoutExtension);
	OutExtensions.Add(MaterialExtension);
	OutExtensions.Add(RenderingExtension);
	...
	for (UText3DLayoutEffectBase* LayoutEffect : LayoutEffects)
	{
		if (LayoutEffect)
		{
			OutExtensions.Add(LayoutEffect);
		}
	}
...
bool UText3DComponent::BeginBuild(TNotNull<EText3DRendererFlags*> InOutFlags)
{
	bIsUpdatingText = true;
	TextPreUpdateDelegate.Broadcast(this, *InOutFlags);

	// If only updating materials, no need to prepare TextRenderer, or Font because the material extension is mostly self contained.
	// TODO: asking extensions whether TextRenderer/etc is needed when building with specific flags?
	if (*InOutFlags == EText3DRendererFlags::Material)
	{
		return true;
	}
	...
	if (!TextRenderer || TextRenderer->GetClass() != TextRendererClass || TextRenderer->GetOuter() != this)
	{
		...
		if (!TextRenderer)
		{
			UE_LOGF(LogText3D, Warning, "Text3D renderer is not valid, cannot proceed");
			return false;
		}
	}
	...
	if (!Font)
	{
		...
		if (!Font)
		{
			UE_LOGF(LogText3D, Warning, "Font object is not valid, Fallback font is not defined in project settings, cannot proceed");
			return false;
		}
	}
	...
	{
		TextRenderer->Create();
		*InOutFlags = EText3DRendererFlags::All;
	}
	return true;
}

void UText3DComponent::FinishBuild(EText3DRendererFlags InFlags)
{
	...
	EnumRemoveFlags(PreRegisterUpdateFlags, InFlags);
	TextPostUpdateDelegate.Broadcast(this, InFlags);
	bIsUpdatingText = false;
}

void UText3DComponent::CancelBuild()
{
	bIsUpdatingText = false;
}

void UText3DComponent::SetUseBlockingBuild(bool bInUseBlockingBuild)
{
