...
void UText3DDefaultStyleExtension::PrepareBuild(EText3DRendererFlags InUpdateFlags)
{
	Super::PrepareBuild(InUpdateFlags);

	TRACE_CPUPROFILER_EVENT_SCOPE(UText3DDefaultStyleExtension::PrepareRebuild);

	if (!DefaultFontStyle.IsValid()
		|| !TextStyleSet.IsValid()
		|| EnumHasAnyFlags(InUpdateFlags, EText3DRendererFlags::Geometry))
	{
