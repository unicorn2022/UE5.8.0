...
EText3DExtensionResult UText3DDefaultMaterialExtension::PreRendererUpdate(const UE::Text3D::Renderer::FUpdateParameters& InParameters)
{
	...
	if (Style != EText3DMaterialStyle::Custom && Style != EText3DMaterialStyle::Invalid)
	{
		...
		{
			UE_LOGF(LogText3D, Error, "Failed to retrieve dynamic material in Text3D material extension")
	...
	if (!MaterialOverrides.IsEmpty())
	{
		...
		if (!StyleExtension)
		{
			UE_LOGF(LogText3D, Warning, "Invalid style extension retrieved in material extension, cannot proceed.")
			return EText3DExtensionResult::Failed;
		}
...
UMaterialInstanceDynamic* UText3DDefaultMaterialExtension::FindOrAdd(EText3DGroupType InGroup, FName InTag)
{
	...
	if (Style != EText3DMaterialStyle::Solid)
	{
...
void UText3DDefaultMaterialExtension::ClearParameterValues()
{
	auto ClearParamValues = [](UMaterialInterface* InMaterial)
		{
	...
	ClearParamValues(FrontMaterial);
	ClearParamValues(BackMaterial);
	ClearParamValues(BevelMaterial);
	ClearParamValues(ExtrudeMaterial);

	for (const FText3DMaterialOverride& Override : MaterialOverrides)
	{
		for (UMaterialInterface* Material : Override.Materials)
		{
			ClearParamValues(Material);
		}
	}

	for (const TPair<FText3DMaterialGroupKey, TObjectPtr<UMaterialInstanceDynamic>>& Pair : GroupDynamicMaterials)
	{
		ClearParamValues(Pair.Value);
	}
}
...
void UText3DDefaultMaterialExtension::PostInitProperties()
{
	...
	{
		// This needs to be called on the game thread. In the case of loading it will get called in PostLoad()
		PreCacheMaterials();
	}
}

void UText3DDefaultMaterialExtension::PostDuplicate(bool bInDuplicateForPIE)
{
	Super::PostDuplicate(bInDuplicateForPIE);

	if (bInDuplicateForPIE)
	{
		// Clear MID parameters as these get duplicated but not updated correctly for PIE.
