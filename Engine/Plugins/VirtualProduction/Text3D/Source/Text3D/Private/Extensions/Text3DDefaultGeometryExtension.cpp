...
void UText3DDefaultGeometryExtension::SetCollisionEnabled(ECollisionEnabled::Type InCollisionEnabled)
{
...
void UText3DDefaultGeometryExtension::PostInitProperties()
{
	Super::PostInitProperties();

	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		const UText3DProjectSettings* const Settings = UText3DProjectSettings::Get();
		if (Settings && Settings->ShouldForceDisableTextCollision())
		{
...
void UText3DDefaultGeometryExtension::PostEditChangeProperty(FPropertyChangedEvent& InEvent)
{
	...
	else if (InEvent.GetMemberPropertyName() == GET_MEMBER_NAME_CHECKED(UText3DDefaultGeometryExtension, CollisionEnabled))
	{
		OnCollisionEnabledChanged();
	}
}
...
ECollisionEnabled::Type UText3DDefaultGeometryExtension::GetGlyphCollisionEnabled() const
{
...
void UText3DDefaultGeometryExtension::PrepareBuild(EText3DRendererFlags InUpdateFlags)
{
	...
	TRACE_CPUPROFILER_EVENT_SCOPE(UText3DDefaultGeometryExtension::PrepareRebuild);

	// Refresh typeface to ensure type face index is up-to-date.
	UText3DComponent* const TextComponent = GetText3DComponent();
	TextComponent->RefreshTypeface();
	...
	if (TypefaceIndex == INDEX_NONE)
	{
		...
		UE_LOGF(LogText3D, Error, "Invalid typeface index returned for font %ls : %i", Font ? *Font->GetName() : TEXT("invalid"), TypefaceIndex);
		return;
	}
	...
	const UText3DRendererBase* TextRenderer = TextComponent->GetTextRendererClass().GetDefaultObject();
	GlyphMeshParameters =
	{
		...
		.MeshType = TextRenderer ? TextRenderer->GetMeshType() : EText3DMeshType::Unknown,
	};
}
...
void UText3DDefaultGeometryExtension::OnCollisionEnabledChanged()
{
	RequestUpdate(EText3DRendererFlags::Geometry);
}
