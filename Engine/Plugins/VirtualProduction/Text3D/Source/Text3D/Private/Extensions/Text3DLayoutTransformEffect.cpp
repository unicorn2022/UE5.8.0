...
void UText3DLayoutTransformEffect::SetLocationStagger(bool bInStagger)
{
	if (bLocationStagger == bInStagger)
	{
		return;
	}

	bLocationStagger = bInStagger;
	OnTransformOptionsChanged();
}
...
void UText3DLayoutTransformEffect::SetScaleStagger(bool bInStagger)
{
	if (bScaleStagger == bInStagger)
	{
		return;
	}

	bScaleStagger = bInStagger;
	OnTransformOptionsChanged();
}
...
void UText3DLayoutTransformEffect::SetRotationStagger(bool bInStagger)
{
	if (bRotationStagger == bInStagger)
	{
		return;
	}

	bRotationStagger = bInStagger;
	OnTransformOptionsChanged();
}
...
void UText3DLayoutTransformEffect::ApplyEffect(uint32 InGlyphIndex, uint32 InGlyphCount)
{
	...
	if (bLocationEnabled)
	{
		const float Effect = CalculateEffect(InGlyphIndex, InGlyphCount, LocationOrder, LocationProgress, LocationEaseCurve, bLocationStagger);
	...
	if (bScaleEnabled)
	{
		const float Effect = CalculateEffect(InGlyphIndex, InGlyphCount, ScaleOrder, ScaleProgress, ScaleEaseCurve, bScaleStagger);
	...
	if (bRotationEnabled)
	{
		const float Effect = CalculateEffect(InGlyphIndex, InGlyphCount, RotationOrder, RotationProgress, RotationEaseCurve, bRotationStagger);
...
float UText3DLayoutTransformEffect::CalculateEffect(int32 InIndex, int32 InTotal, EText3DCharacterEffectOrder InOrder, float InProgress, const UCurveFloat* InEaseCurve, bool bInStagger) const
{
	...
	const float StaggerProgress = bInStagger ? StaggerAmount : 1.0f - CharacterStart;
	const float CharacterProgress = FMath::Clamp((NormalizedProgress - CharacterStart) / StaggerProgress, 0.0f, 1.0f);
