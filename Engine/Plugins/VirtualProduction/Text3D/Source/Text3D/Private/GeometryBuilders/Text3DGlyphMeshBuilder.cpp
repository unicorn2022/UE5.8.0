...
void FText3DGlyphMeshBuilder::CreateMeshes(const FText3DGlyphContourNode& InRoot, const float InExtrude, const float InBevel, const EText3DBevelType InBevelType, const int32 InBevelSegments, const bool bInOutline, const float InOutlineExpand, const EText3DOutlineType InOutlineType)
{
	CreateFrontMesh(InRoot, bInOutline, InOutlineExpand, InOutlineType);
...
void FText3DGlyphMeshBuilder::SetFrontAndBevelTextureCoordinates(const float Bevel)
{
	...
	FBox2f Box;
	TText3DGroupList& Groups = Data.GetGlyph().GetGroups();

	const int32 FirstVertex = Groups[GroupIndex].FirstVertex;
	const int32 LastVertex = Groups[GroupIndex + 1].FirstVertex;

	TVertexAttributesConstRef<FVector3f> Positions = Data.GetGlyph().GetStaticMeshAttributes().GetVertexPositions();
	...
	FStaticMeshAttributes& StaticMeshAttributes = Data.GetGlyph().GetStaticMeshAttributes();
...
{
	const TVertexAttributesRef<FVector3f> VertexPositions = Data.GetGlyph().GetStaticMeshAttributes().GetVertexPositions();
	...
	for (const FVertexID VertexID : Data.GetGlyph().GetMeshDescription().Vertices().GetElementIDs())
	{
	...
	const FVector MeshCenter = FVector(MinX+MaxX, MinY+MaxY, 0) * 0.5;

	// Keep track of the mesh offset that we need to add back.
	MeshOffset  = +2.0 * InNewPivot * FVector(MinX, MinY, 0);
	PivotOffset = -2.0 * InNewPivot * MeshCenter;

	// Apply pivot offset with adjusted values
	const FVector3f PivotOffsetF = FVector3f(PivotOffset);
	for (const FVertexID VertexID : Data.GetGlyph().GetMeshDescription().Vertices().GetElementIDs())
	{
		VertexPositions[VertexID] += PivotOffsetF;
	}
}

void FText3DGlyphMeshBuilder::BuildMesh(FText3DCachedMesh& InMesh, EText3DMeshType InMeshType, UMaterialInterface* InDefaultMaterial)
{
	Data.GetGlyph().Build(InMesh, InMeshType, InDefaultMaterial);
...
{
	return Data.GetGlyph().GetMeshDescription().ComputeBoundingBox();
}
...
void FText3DGlyphMeshBuilder::CreateFrontMesh(const FText3DGlyphContourNode& Root, const bool bOutline, const float OutlineExpand, const EText3DOutlineType InOutlineType)
{
	int32 VertexCount = 0;
...
void FText3DGlyphMeshBuilder::MirrorGroup(const EText3DGroupType TypeIn, const EText3DGroupType TypeOut, const float Extrude)
{
	...
	FMeshDescription& MeshDescription = Data.GetGlyph().GetMeshDescription();
...
void FText3DGlyphMeshBuilder::AddToVertexCount(const FText3DGlyphContourNode& Node, int32& OutVertexCount)
{
	for (const FText3DGlyphContourNode& Child : Node.Children)
	{
...
void FText3DGlyphMeshBuilder::TriangulateAndConvert(const FText3DGlyphContourNode& Node, int32& OutVertexIndex, const bool bOutline, const EText3DOutlineType InOutlineType)
{
	...
	if (!Node.bClockwise)
	{
		...
		auto ProcessContour = [&Contours=Contours, &Data=Data, &VertexCount, &Triangulation, bOutline, InOutlineType](const FText3DGlyphContourNode& NodeIn)
		{
			// Create contour in old format
			FText3DGlyphContour& Contour = Contours.Add();
			...
			for (const FVector2f& Vertex : Polygon.GetVertices())
			{
				...
				// Add point to mesh
				const int32 VertexID = Data.AddVertex(Point->Position, { 1.f, 0.f }, { -1.f, 0.f, 0.f });
...
void FText3DGlyphMeshBuilder::ExpandPointWithoutAddingVertices(const FText3DGlyphPartPtr& Point)
{
