...
void FText3DGlyph::Build(FText3DCachedMesh& InMesh, EText3DMeshType InMeshType, UMaterialInterface* InDefaultMaterial)
{
	// Both dynamic and static build static meshes
	checkf(InMesh.StaticMesh, TEXT("Invalid static mesh object provided to the build function"));
	checkf(InMeshType != EText3DMeshType::Dynamic || InMesh.DynamicMesh, TEXT("Invalid dynamic mesh object provided to the build function"));

	auto AddMaterial = [](UStaticMesh* InMesh, UMaterialInterface* InMaterial, int32 InIndex)
	{
		using namespace UE::Text3D::Material;

		const TConstArrayView<FName> SlotNames = GetSlotNames();
...

	{
		...
		constexpr bool bAllowCpuAccess = true;

		UStaticMesh::FBuildMeshDescriptionsParams BuildParams;
		BuildParams.bCommitMeshDescription = true;
		BuildParams.bFastBuild = true;
		BuildParams.bAllowCpuAccess = bAllowCpuAccess;

		InMesh.StaticMesh->bAllowCPUAccess = bAllowCpuAccess;
	...
	// Dynamic mesh building
	if (InMeshType == EText3DMeshType::Dynamic)
	{
...
const FMeshDescription& FText3DGlyph::GetMeshDescription() const
{
