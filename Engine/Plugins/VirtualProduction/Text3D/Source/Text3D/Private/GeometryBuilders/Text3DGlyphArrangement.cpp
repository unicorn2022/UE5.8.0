...
int32 FText3DGlyphArrangement::InsertPoint(const FVector2d &InPoint, double InTol)
{
    int32 PIdx = FindExistingVertex(InPoint);
    if (PIdx > -1)
    {
        return -1;
    }
    ...
    int32 FoundEdgeToSplit = -1;
    for (int32 EID = 0, ExistingEdgeMax = Graph.MaxEdgeID(); EID < ExistingEdgeMax; EID++)
    {
        if (!Graph.IsEdge(EID))
        {
            continue;
    ...
    if (FoundEdgeToSplit > -1)
    {
        ...
        PointHash.InsertPoint(splitInfo.VNew, InPoint);
        return splitInfo.VNew;
    }

    int32 VID = Graph.AppendVertex(InPoint);
    PointHash.InsertPoint(VID, InPoint);
    return VID;
}
...
bool FText3DGlyphArrangement::InsertSegment(FVector2d InA, FVector2d InB, double InTol)
{
    ...
    int32 a_idx = FindExistingVertex(InA);
    int32 b_idx = FindExistingVertex(InB);
    ...
    for (int32 i = 0, N = Hits.Num(); i < N; ++i)
    {
    ...
    if (a_idx == -1)
    {
        ...
        PointHash.InsertPoint(a_idx, InA);
    }
    if (b_idx == -1)
    {
    ...
    if (b_idx == -1)
    {
        ...
        PointHash.InsertPoint(b_idx, InB);
    }
    ...
    {
        int32 v0 = points[k].VID;
        int32 v1 = points[k + 1].VID;
        if (v0 == v1)
        {
            continue;
        }
        ...
        if (Graph.FindEdge(v0, v1) == FDynamicGraph2d::InvalidID)
        {
            ...
            const int32 EID = Graph.AppendEdge(v0, v1);
...
FIndex2i FText3DGlyphArrangement::SplitSegmentAtDistance(int32 InEID, double InDistance, double InTol)
{
    ...
    else
    {
        ...
        FDynamicGraph2d::FEdgeSplitInfo splitInfo;
        EMeshResult result;
        int32 CrossingVert = FindExistingVertex(Pt);
        if (CrossingVert == -1)
        {
        ...
        {	// position + track added vertex
            ...
            PointHash.InsertPoint(splitInfo.VNew, Pt);
        }
    }
    return FIndex2i(use_vid, new_eid);
}

int32 FText3DGlyphArrangement::FindExistingVertex(const FVector2d& InPoint) const
{
    return FindNearestVertex(InPoint, VertexSnapTol);
}

int32 FText3DGlyphArrangement::FindNearestVertex(const FVector2d& InPoint, double InSearchRadius, int32 InIgnoreVID) const
{
    auto FuncDistSq = [&](int32 B) { return DistanceSquared(InPoint, Graph.GetVertex(B)); };
    auto FuncIgnore = [&](int32 VID) { return VID == InIgnoreVID; };
    TPair<int32, double> found = (InIgnoreVID == -1) ? PointHash.FindNearestInRadius(InPoint, InSearchRadius, FuncDistSq)
...
bool FText3DGlyphArrangement::FindIntersectingEdges(const FVector2d& InA, const FVector2d& InB, TArray<FIntersection>& OutHits, double InTol) const
{
    ...
    for (int32 EID : Graph.EdgeIndices())
    {
        Graph.GetEdgeV(EID, x, y);
        ...
        int32 SideX = (SignX > InTol ? +1 : (SignX < -InTol ? -1 : 0));
        int32 SideY = (SignY > InTol ? +1 : (SignY < -InTol ? -1 : 0));
