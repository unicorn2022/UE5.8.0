...
void UText3DCharacterTransform::PrintDeprecationLog() const
{
	if (const UText3DComponent* Text3DComponent = GetText3DComponent())
	{
		UE_LOGF(LogText3D, Warning, "%ls : Text3DCharacterTransform component is deprecated and should no longer be used, instead prefer using Text3DLayoutTransformEffect on Text3D that replaces this entirely !", Text3DComponent->GetOwner() ? *Text3DComponent->GetOwner()->GetActorNameOrLabel() : TEXT("Invalid Actor"))
	}
}
