...
#if WITH_FREETYPE
		/** 
		 * Cached metrics when the glyph is shaped. 
		 * In between Text3D incremental build phases (specifically between Layout Prepare Build and Layout Pre Renderer Update), 
		 * size metrics might change for an external reason (e.g. a text block widget with the same font).
		 */
		FT_Pos FaceSizeMetricsAscender = 0;
		FT_Pos FaceSizeMetricsDescender = 0;
#endif
	};
