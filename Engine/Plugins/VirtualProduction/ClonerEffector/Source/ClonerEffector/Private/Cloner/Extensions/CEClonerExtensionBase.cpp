...
void UCEClonerExtensionBase::ActivateExtension()
{
	if (!bExtensionActive)
	{
		...
		UE_LOGF(LogCECloner, Verbose, "%ls : Cloner extension activated %ls", *ClonerComponent->GetOwner()->GetActorNameOrLabel(), *GetExtensionName().ToString());
...
void UCEClonerExtensionBase::DeactivateExtension()
{
	if (bExtensionActive)
	{
		...
		const UCEClonerComponent* ClonerComponent = GetClonerComponentChecked();
		UE_LOGF(LogCECloner, Verbose, "%ls : Cloner extension deactivated %ls", *ClonerComponent->GetOwner()->GetActorNameOrLabel(), *GetExtensionName().ToString());
