...
bool UCEClonerEffectorExtension::LinkEffector(AActor* InEffectorActor)
{
	...
	UE_LOGF(LogCECloner, Verbose, "%ls : Effector %ls linked to Cloner", *GetClonerComponent()->GetOwner()->GetActorNameOrLabel(), *InEffectorActor->GetActorNameOrLabel());
...
bool UCEClonerEffectorExtension::UnlinkEffector(AActor* InEffectorActor)
{
	...
	if (EffectorActorsWeak.Remove(InEffectorActor) > 0)
	{
		...
		UE_LOGF(LogCECloner, Verbose, "%ls : Effector %ls unlinked from Cloner", *GetClonerComponent()->GetOwner()->GetActorNameOrLabel(), *InEffectorActor->GetActorNameOrLabel());
	}
