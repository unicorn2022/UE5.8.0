...
void FCEClonerAttachmentTree::SetBehaviorImplementation(const TSharedRef<ICEClonerAttachmentTreeBehavior>& InImplementation)
{
	...
	{
		TArray<AActor*> ActorsToDetach;
		ForEachAttachment([this, &ActorsToDetach](AActor* InActor, FCEClonerAttachmentItem& InAttachment)
		{
			ActorsToDetach.Add(InActor);
			return true;
		});

		for (AActor* const Actor : ActorsToDetach)
		{
			DetachItem(Actor);
		}
...
bool FCEClonerAttachmentTree::UpdateAttachments(bool bInReset)
{
	...
	UE_LOGF(LogCECloner, Verbose, "%ls : updating attachment tree", *AttachmentRoot->GetActorNameOrLabel())
