...
void UCEClonerCollisionExtension::OnExtensionParametersChanged(UCEClonerComponent* InComponent)
{
	...
	if (const UNiagaraDataInterfaceArrayMesh* const MeshArrayDI = UE::ClonerEffector::FindMeshArrayDataInterface(InComponent))
	{
		if (CollisionRadiusMode != ECEClonerCollisionRadiusMode::Manual)
		{
			for (int32 Idx = 0; Idx < CollisionRadii.Num(); Idx++)
			{
				const FNiagaraMeshRendererMeshPropertiesBase& MeshProperties = MeshArrayDI->MeshData[Idx];
...
#if WITH_EDITOR
	if (ShouldInitVisualizerComponent(InComponent) && InitVisualizerComponent())
	{
...
bool UCEClonerCollisionExtension::ShouldInitVisualizerComponent(TNotNull<const UCEClonerComponent*> InComponent) const
{
