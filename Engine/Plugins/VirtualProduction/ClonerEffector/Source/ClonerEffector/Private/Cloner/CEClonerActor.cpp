...
{
	if (GIsEditor && ClonerComponent && ClonerComponent->GetAttachmentCount() == 0)
	{
...
void ACEClonerActor::MigrateDeprecatedProperties()
{
	...
	UE_LOGF(LogCECloner, Warning, "%ls : Cloner migrating from version %i to latest %i, please re-save this asset", *GetActorNameOrLabel(), MigrateToVersion, FCEClonerActorVersion::LatestVersion);
