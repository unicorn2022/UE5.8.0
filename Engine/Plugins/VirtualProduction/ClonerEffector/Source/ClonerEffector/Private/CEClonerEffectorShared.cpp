...
UNiagaraDataInterfaceArrayMesh* UE::ClonerEffector::FindMeshArrayDataInterface(UCEClonerComponent* InClonerComponent)
{
	if (!InClonerComponent || !InClonerComponent->GetAsset())
	{
		return nullptr;
	}
	return UNiagaraFunctionLibrary::GetDataInterface<UNiagaraDataInterfaceArrayMesh>(InClonerComponent, UE::ClonerEffector::MeshArrayName);
}
