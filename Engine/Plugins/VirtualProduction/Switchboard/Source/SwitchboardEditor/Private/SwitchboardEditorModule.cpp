...
bool FSwitchboardEditorModule::CreateNewConfig(const FSwitchboardNewConfigUserOptions& NewConfigUserOptions)
{
	...
	{
		UE_LOGF(LogSwitchboardPlugin, Log, "Could not save temp script arguments file '%ls'", *ScriptArgsFilepath);
		return false;
	}
	
	UE_LOGF(LogSwitchboardPlugin, Verbose, "Will launch Switchboard using script arguments: '%ls' which were saved in: '%ls'",
...
bool FSwitchboardEditorModule::RunProcess(const FString& InExe, const FString& InArgs)
{
	...
	UE_LOGF(LogSwitchboardPlugin, Display, "Launching: %ls %ls", *InExe, *InArgs);
...
TSharedFuture<FSwitchboardVerifyResult> FSwitchboardEditorModule::GetVerifyResult(bool bForceRefresh /* = false */)
{
	...
	if (bForceRefresh || !VerifyResult.IsValid() || VerifyPath != CurrentVenv)
	{
		...
		UE_LOGF(LogSwitchboardPlugin, Log, "Issuing verify for venv: %ls", *CurrentVenv);
		...
		{
			UE_LOGF(LogSwitchboardPlugin, Log, "Verify complete for venv: %ls", *Venv);
			UE_LOGF(LogSwitchboardPlugin, Log, "Verify summary: %d", int(Result.Summary));
			UE_LOGF(LogSwitchboardPlugin, Log, "Verify log: %ls", *Result.Log);
...
bool FSwitchboardEditorModule::OnEditorSettingsModified()
{
	...
	if (!AutolaunchEntryExecutable.IsEmpty())
	{
		...
		{
			UE_LOGF(LogSwitchboardPlugin, Log, "Updating listener auto-launch entry");
			SetListenerAutolaunchEnabled(true);
		}
		else
		{
			UE_LOGF(LogSwitchboardPlugin, Log, "NOT updating listener auto-launch; paths differ:\n\t%ls\n\t%ls",
