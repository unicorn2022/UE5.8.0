    ...
    def launch(self, args: Optional[list[str]] = None):
        ...
        with self.lock:
            ...
            if CONFIG.MUSERVER_FILESHARING_ENABLED.get_value() and CONFIG.MUSERVER_FILESHARING_SRV.get_value():
                args.append('-CONCERTFILESHARINGENABLED=1')
                filesharing = CONFIG.MUSERVER_FILESHARING_SRV.get_value()
                args.append(f'-CONCERTFILESHAREPATH={filesharing}')
                
            args = list(filter(None, args))

            launch_cmd_full = ' '.join(args)
            if sys.platform.startswith('win'):
                ...
                args = launch_cmd_full
        
            LOGGER.info(f'Launching Multi-user Server: {launch_cmd_full}');
