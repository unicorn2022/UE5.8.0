...
class SwitchboardDialog(QtCore.QObject):
    ...
    def __init__(self, script_manager):
        ...
        self.window: Ui_switchboard_window = Ui_switchboard_window()
        ...
        self._recording_end_time: datetime.datetime | None = None
    ...
    def osc_take(self, address, command, value):
        device = self._osc_device_from_address(address, command, value=value)
    ...
    def _osc_device_from_address(self, address, command, value=''):
        device: Device | None = None

        for check_device in self.device_manager.devices():
            if (check_device.address == address[0]) and (check_device.osc_client is not None):
                device = check_device
                break

        if not device:
            LOGGER.warning(f'{address} is not registered with an OSC device in Switchboard')
            return None
    ...
    def osc_record_start(self, address, command, slate, take, description):
        ...
        '''
        device = self._osc_device_from_address(address, command, value=[slate, take, description])
        if not device:
            return
    ...
    def _on_recording_timer(self):
        self._update_recording_status()

    def _fmt_rec_duration(self, timedelta: datetime.timedelta) -> str:
        mm, ss = divmod(int(timedelta.total_seconds() + 0.5), 60)
        hh, mm = divmod(mm, 60)
        result = ''
        if hh:
            result += f'{hh:02}' + (' hours, ' if hh != 1 else ' hour, ')
        if mm or hh:
            result += f'{mm:02}' + (' mins, ' if mm != 1 else ' min, ')
        result += f'{ss:02}'  + (' secs ' if ss != 1 else ' sec ')
        return result

    def _update_recording_status(self):
        self.window.recording_status.setStyleSheet('')

        if not self._recording_start_time:
            self.window.recording_status.setText('Not Recording ')
        elif self._recording_end_time:
            recording_duration = self._recording_end_time - self._recording_start_time
            self.window.recording_status.setText('Last Recording: ' + self._fmt_rec_duration(recording_duration))
        else:
            recording_duration = datetime.datetime.now() - self._recording_start_time
            self.window.recording_status.setText('Recording Duration: ' + self._fmt_rec_duration(recording_duration))
            self.window.recording_status.setStyleSheet('color: #e52d00;')
    ...
    def _record_start(self, slate, take, description, exclude_address=None):
        ...
        new_recording.date = switchboard_utils.date_to_string(self._recording_start_time.date())
    ...
    def _record_stop(self, exclude_address=None):
        ...
        # End the recording
        self._is_recording = False
        self._recording_end_time = datetime.datetime.now()
    ...
    def osc_record_start_confirm(self, address, command, timecode):
        device = self._osc_device_from_address(address, command, value=timecode)
        if not device:
            return
    ...
    def osc_record_stop(self, address, command):
        device = self._osc_device_from_address(address, command)
        if not device:
            return
