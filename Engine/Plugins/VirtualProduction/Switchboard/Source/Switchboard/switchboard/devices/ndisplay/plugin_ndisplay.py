...
from .ndisplay_tools_ui import nDisplayToolsUI
...
class DevicenDisplay(DeviceUnreal):
    ...
    csettings = {
        ...
        'cluster_exec_history': StringListSetting(
            attr_name='cluster_exec_history',
            nice_name='Cluster exec console command history',
            value=[],
            show_ui=False,
        ),
        'custom_exec_commands': StringListSetting(
            attr_name='custom_exec_commands',
            nice_name='Custom Exec Commands',
            value=[],
            show_ui=False,
            tool_tip='List of custom exec commands for the nDisplay Tools UI'
        )
    }

    ndisplay_monitor_ui = None
    ndisplay_monitor = None
    ndisplay_tools_ui = None
    ...
    def generate_unreal_command_line(self, map_name=""):
        ...
        if uproject != "":
            # Skip normpath when the remote isn't Windows: on a Windows
            # host it would flip the Linux/Mac remote's forward slashes
            # to backslashes and break the launch.
            # (It's not obvious why we do it on Windows, either.)
    ...
    def plug_into_ui(cls, menubar, tabs):
        ...
        if not cls.ndisplay_monitor_ui:
            ...
            cls.ndisplay_monitor_ui.replace_exec_history(DevicenDisplay.csettings['cluster_exec_history'].get_value())
            cls.ndisplay_monitor_ui.signal_exec_history_changed.connect(lambda: cls._handle_exec_history_changed())

        # Create Tools UI if it doesn't exist
        if not cls.ndisplay_tools_ui:
            cls.ndisplay_tools_ui = nDisplayToolsUI(
                parent=tabs, plugin_ndisplay=cls)

        # Add our UIs to the main tabs in the UI
        tabs.addTab(cls.ndisplay_monitor_ui, 'nDisplay &Monitor')
        tabs.addTab(cls.ndisplay_tools_ui, 'nDisplay &Tools')

    @classmethod
    def _handle_exec_history_changed(cls):
        if cls.ndisplay_monitor_ui:
            DevicenDisplay.csettings['cluster_exec_history'].update_value(
                list(cls.ndisplay_monitor_ui.exec_history.values()))
