...
from .log_viewer_watcher import log_viewer_watcher
...
class DeviceUnreal(Device):
    ...
    MEDIAPROFILE_CLASS_NAMES = (
        ...
        '/Script/MediaProfile.MediaProfile',  # module renamed in UE5
    )
    ...
    class StaticSignals(QtCore.QObject):
        ...
        clipboard_assets_added_signal = QtCore.Signal()
        project_assets_updated_signal = QtCore.Signal()
    ...
    # Global live log management - only one device can stream live logs at a time
    active_live_log_device: Optional['DeviceUnreal'] = None
    ...
    def __init__(self, name, address, **kwargs):
        ...
        self.unreal_client.delegates["live log data"] = (self.on_live_log_data)

        # Live log streaming state
        self.live_log_streaming = False
        self.live_log_file_path = None
        self.live_log_local_file = None

        # Track log files per process UUID for resuming
        self.process_log_files = {}  # UUID -> {'local_file': pathlib.Path, 'last_position': int}
        ...
        # Populated from the listener's state message on connect.
        self.platform_binary_directory: str = ''
    ...
    def device_widget_registered(self, device_widget):
        ...
        # Store device reference in widget for live log context menu
        device_widget._device = self
        ...
        # hook to live log signal from widget
        device_widget.signal_start_live_log.connect(self.on_start_live_log)
    ...
    def disconnect_listener(self):
        ...
        # Clear process log file tracking on disconnect
        if self.process_log_files:
            LOGGER.info(f"Clearing log file tracking for {len(self.process_log_files)} processes on disconnect")
            self.process_log_files.clear()

        super().disconnect_listener()
        self.unreal_client.disconnect()

    def _remote_path_uses_windows_separators(self) -> bool:
        ''' True for Win64 remotes, and pre-connection (when the remote's
        platform isn't yet known) to preserve prior behavior on Windows
        hosts. Use posixpath join when this is False so forward slashes
        are preserved in paths sent to a Linux/Mac remote.
        '''
        return self.platform_binary_directory in ('', 'Win64')

    @property
    def python_path(self) -> str:
        engine_dir = CONFIG.ENGINE_DIR.get_value(self.name)
        if self._remote_path_uses_windows_separators():
            return os.path.normpath(os.path.join(
                engine_dir, 'Binaries', 'ThirdParty', 'Python3',
                self.platform_binary_directory or 'Win64', 'python.exe'))
    ...
    def _queue_build(
        ...
        if self.target_platform.lower().startswith('win'):
            ...
            ubt_path = posixpath.join(engine_path, 'Build', 'BatchFiles',
                                      'Linux', 'Build.sh')
    ...
    def minimize_windows(self):
        ''' Tries to minimize all windows. Windows-only; no-op elsewhere. '''
    ...
    def generate_unreal_exe_path(self):
        # CONFIG.engine_exe_path keys off the host's sys.platform, which is
        # wrong when the Switchboard host and the target node disagree
        # (e.g. Windows host driving a Linux nDisplay node). Build the
        # path from the remote device's platform_binary_directory instead.
        engine_dir = CONFIG.ENGINE_DIR.get_value(self.name)
        if self._remote_path_uses_windows_separators():
            return CONFIG.engine_exe_path(engine_dir, self.executable_filename)

        exe_basename = self.executable_filename
        if exe_basename.lower().endswith('.exe'):
            exe_basename = exe_basename[:-len('.exe')]
        return posixpath.join(
            engine_dir, 'Binaries', self.platform_binary_directory,
            exe_basename)
    ...
    def build_udpmessaging_args(self) -> list[str]:
        args: list[str] = []
        if self.udpmessaging_multicast_endpoint:
            ...
            args.append(f'-UDPMESSAGING_TRANSPORT_MULTICAST="{self.udpmessaging_multicast_endpoint}"')

        if self.udpmessaging_unicast_endpoint:
            args.append(f'-UDPMESSAGING_TRANSPORT_UNICAST="{self.udpmessaging_unicast_endpoint}"')

        static_endpoints = self.build_udpmessaging_static_endpoint_list()
        if len(static_endpoints) > 0:
            args.append(f'-UDPMESSAGING_TRANSPORT_STATIC="{",".join(static_endpoints)}"')
    ...
    def generate_unreal_command_line_args(self, map_name):
        ...
        # Build command line arguments as an array that will be joined at the end.
        args: list[str] = []

        # DPCVars - accumulate these before adding to args
        dp_cvars = []

        # Project path and map
        args.append(f'"{CONFIG.UPROJECT_PATH.get_value(self.name)}"')
        if map_name:
            args.append(map_name)

        # Extra command line arguments from settings
        if self.extra_cmdline_args_setting:
            args.append(self.extra_cmdline_args_setting)

        # Logging
        args.append(f'Log={self.log_filename}')
        ...
        if CONFIG.MUSERVER_FILESHARING_ENABLED.get_value() and CONFIG.MUSERVER_FILESHARING_SRV.get_value():
            filesharing = CONFIG.MUSERVER_FILESHARING_SRV.get_value()
            args.extend([
                '-CONCERTFILESHARINGENABLED=1',
                f'-CONCERTFILESHAREPATH={filesharing}'])
        ...
        # Filter and format exec commands
        ...
        if supported_roles or DeviceUnreal.any_devices_have_roles():
            args.append('-VPRole=' + '|'.join(supported_roles))
        ...
        if session_id > 0:
            args.append(f'-StageSessionId={session_id}')

        # Stage friendly name
        args.append(f'-StageFriendlyName="{self.name.replace(" ", "_")}"')

        # Max GPU Count (mGPU)
        max_gpu_count = DeviceUnreal.csettings["max_gpu_count"].get_value(self.name)
        try:
            if int(max_gpu_count) > 1:
                args.append(f'-MaxGPUCount={max_gpu_count}')
                dp_cvars.append('r.AllowMultiGPUInEditor=1')
        except ValueError:
            LOGGER.warning(f"Invalid Number of GPUs '{max_gpu_count}'")

        # Slate.bAllowThrottling - Makes ICVFX panel and Vcam more responsive
        ...
        if DeviceUnreal.csettings['auto_decline_package_recovery'].get_value(self.name):
            args.append('-AutoDeclinePackageRecovery')

        # UdpMessaging endpoints
        args.extend(self.build_udpmessaging_args())

        # Disable ensures
        if DeviceUnreal.csettings['disable_ensures'].get_value(self.name):
            args.append('-handleensurepercent=0')

        # Concert take recorder settings
        ...
        args.append(ini_engine)

        # VP OSC server settings
        args.extend([
            f'-VPOSCServerAddress={self.address}',
            f'-VPOSCServerPort={self.device_osc_port}'
        ])
    ...
    def on_program_ended(self, message):
        ...
        # Clean up log file tracking for this process if it's an Unreal process
        if program_name == UnrealJobs.Unreal.value:
            process_uuid_str = str(puuid)
            if process_uuid_str in self.process_log_files:
                LOGGER.info(f"Removing log file tracking for ended Unreal process {process_uuid_str}")
                del self.process_log_files[process_uuid_str]
    ...
    def on_start_live_log(self):
        ''' Starts live log streaming from the remote device. 
        
        This ends up being called from the device context menu 
        when the user wants to view the live log
        '''

        # Check if another device is already streaming the live log

        if DeviceUnreal.active_live_log_device and DeviceUnreal.active_live_log_device != self:
            LOGGER.info(f"Stopping live log streaming on {DeviceUnreal.active_live_log_device.name} to start on {self.name}")
            DeviceUnreal.active_live_log_device._stop_live_log_internal()

        # If already streaming, see if we need to launch an instance of the log viewer.

        if self.live_log_streaming and self.live_log_local_file:
            if self.live_log_local_file.exists():

                # Check if we have a running process for this device
                existing_process = log_viewer_watcher.get_process_for_device(self.name)

                if existing_process and existing_process.poll() is None:
                    LOGGER.info(f"Log viewer for '{self.name}' is already running.")
                else:
                    LOGGER.info(f"Launching live log viewer for '{self.name}'.")
                    self._launch_log_viewer()
                return
            else:
                # Local file was unexpectedly deleted, so we stop it because we need to get the stream from the beginning.
                self._stop_live_log_internal()

        if not self.unreal_client.is_connected:
            LOGGER.error(f"Cannot start live log because device {self.name} is not connected")
            return

        if self.status < DeviceStatus.OPEN:
            LOGGER.error(f"Cannot start live log because Unreal is not running on device {self.name}")
            return

        # Get the UUID of the running Unreal process (if any)

        unreal_process_uuid = None
        unreal_puuids = self.program_start_queue.running_puuids_named(UnrealJobs.Unreal.value)

        if unreal_puuids:
            # Grab the first running Unreal, although we expect there to only be up to one per device.
            unreal_process_uuid = str(unreal_puuids[0])
        else:
            LOGGER.error("Live log: no running Unreal processes found, can't start the live log")
            return

        # Construct the remote log file path
        remote_log_path = self.get_remote_log_path() / self.log_filename
        LOGGER.info(f"Live log: requesting streaming for remote path: {remote_log_path}")

        # Check if we have an existing log file for this process UUID
        start_offset = None
        if unreal_process_uuid in self.process_log_files:
            existing_log = self.process_log_files[unreal_process_uuid]
            if existing_log['local_file'].exists():
                # Reuse the existing log file and resume from where we left off
                self.live_log_local_file = existing_log['local_file']
                start_offset = existing_log['local_file'].stat().st_size
                LOGGER.info(f"Resuming live log from existing file {self.live_log_local_file}, offset: {start_offset} bytes")
            else:
                # File was deleted, remove from tracking
                del self.process_log_files[unreal_process_uuid]
                LOGGER.info(f"Previous log file was deleted, will create new one")

        # Create new local log file if we don't have one
        if self.live_log_local_file is None or not self.live_log_local_file.exists():
            log_dir = DeviceUnreal.get_log_download_dir()
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            local_filename = f"live_{self.name}_{timestamp}.log"
            self.live_log_local_file = pathlib.Path(log_dir) / local_filename

            # Create the local live log file
            try:
                self.live_log_local_file.parent.mkdir(parents=True, exist_ok=True)
                self.live_log_local_file.touch()
                start_offset = 0  # New file, start from beginning
                LOGGER.info(f"Created new live log file: {self.live_log_local_file}")
            except Exception as exception:
                LOGGER.error(f"Failed to create live log file {self.live_log_local_file}: {exception}")
                return

            # Track this new log file for the process
            self.process_log_files[unreal_process_uuid] = {
                'local_file': self.live_log_local_file,
                'last_position': 0
            }

        # Send start live log message to listener

        msgid, msg = message_protocol.create_start_live_log_stream_message(
            str(remote_log_path), tail_only=False, process_uuid=unreal_process_uuid, start_offset=start_offset)

        try:
            self.unreal_client.send_message(msg)
            self.live_log_streaming = True
            self.live_log_file_path = str(remote_log_path)
            DeviceUnreal.active_live_log_device = self

            LOGGER.info(f"Started live log streaming for {self.name} -> {self.live_log_local_file}")

            # Launch the log viewer (and will also register with its process watcher)
            self._launch_log_viewer()

        except Exception as exception:
            LOGGER.error(f"Failed to start live log streaming: {exception}")
            self.live_log_streaming = False
            self.live_log_file_path = None

    def on_live_log_data(self, message):
        ''' Handles incoming live log data from the listener '''

        if not self.live_log_streaming or not self.live_log_local_file:
            LOGGER.warning(f"Received live log data but streaming not active or no local file: streaming={self.live_log_streaming}, file={self.live_log_local_file}")
            return

        # Update tracked position for this process
        unreal_puuids = self.program_start_queue.running_puuids_named(UnrealJobs.Unreal.value)
        if unreal_puuids:
            process_uuid = str(unreal_puuids[0])
            if process_uuid in self.process_log_files:
                # Track the file position (could also use file_position from message)
                self.process_log_files[process_uuid]['last_position'] = self.live_log_local_file.stat().st_size

        try:
            # Check for compressed content first

            if 'content_compressed_base64' in message:
                compressed_base64 = message['content_compressed_base64']
                compressed_bytes = base64.b64decode(compressed_base64)

                # Decompress
                content_bytes = zlib.decompress(compressed_bytes)

                # Uncomment the following to get an idea of the compression ratios achieved.
                #
                #compression_ratio = len(content_bytes) / len(compressed_bytes) if len(compressed_bytes) > 0 else 1.0
                #LOGGER.info(f"Live log compression: {len(compressed_bytes)} -> {len(content_bytes)} bytes (ratio: {compression_ratio:.2f}x)")

                # write to live log file
                with open(self.live_log_local_file, 'ab') as f:
                    f.write(content_bytes)

            elif 'content_base64' in message:

                # If data is not compressed, check the uncompressed content
            ...
            LOGGER.error(f"Failed to write live log data to {self.live_log_local_file}: {e}")

    def _launch_log_viewer(self):
        ''' Launch the log viewer with the live log file '''

        # Currently default viewer for this filetype.
        url = QtCore.QUrl.fromLocalFile(str(self.live_log_local_file))
        QtGui.QDesktopServices.openUrl(url)

    def _stop_live_log_internal(self):
        ''' Stops live log streaming '''

        if not self.live_log_streaming:
            return

        # Note: We intentionally do NOT clear process_log_files here
        # so that we can resume if the same process is still running

        if self.live_log_file_path and self.unreal_client.is_connected:

            # Send stop live log message to listener
            msgid, msg = message_protocol.create_stop_live_log_stream_message(
                self.live_log_file_path)

            try:
                self.unreal_client.send_message(msg)
                LOGGER.info(f"Stopped live log streaming for {self.name}")
            except Exception as e:
                LOGGER.error(f"Failed to send stop live log message: {e}")

        # Terminate any running log viewer processes for this device
        log_viewer_watcher.terminate_log_viewers_for_device(self.name)
        ...
        # Clear global active device, if it was this device.
        if DeviceUnreal.active_live_log_device == self:
            DeviceUnreal.active_live_log_device = None

    @QtCore.Slot()
    def _on_log_viewer_closed(self):
        '''
        Called by the log viewer watcer when the log viewer process associated with this device terminates,
        e.g. the user closed it. When this happens, we automatically stop the live streaming since nobody is
        watching. Currently there is not support for log viewers different to the default.
        '''
        LOGGER.info(f"Log viewer closed for device '{self.name}', stopping live log streaming")

        # Stop live log streaming since the viewer is no longer available
        if self.live_log_streaming:
            self._stop_live_log_internal()

        # Note: We keep the process_log_files tracking so we can resume from the same position
        # if the user reopens the log viewer while the process is still running
    ...
    def analyze_project_assets(cls):
        ...
        # Emit signal to notify UI components that project assets were updated
        cls.static_signals.project_assets_updated_signal.emit()
    ...
    def handle_clipboard_mime_text(cls, text:str):
        ...
        asset_reference_re = re.compile(r"""
            ...
            '                                      # '
            (?P<gamepath>[A-Za-z0-9_/\-]+)         # Gamepath (allow -)
            \.                                     # .
            (?P<name>[A-Za-z0-9_\-]+)              # Name (allow -)
            '                                      # '
            $""", re.VERBOSE)
        ...
        if len(assets):
            ...
            if reply == QtWidgets.QMessageBox.Ok:
                ...
                # Emit signal to notify UI components that assets were added
                cls.static_signals.clipboard_assets_added_signal.emit()
...
class DeviceWidgetUnreal(DeviceWidget):
    ...
    signal_start_live_log = QtCore.Signal(object)
    ...
    def populate_context_menu(self, cmenu: QtWidgets.QMenu):
        ...
        # Remove device is already added by parent widget
        cmenu.addSeparator()

        # Exclude options with checkboxes
        exclude_build_action = cmenu.addAction("Exclude from build")
        exclude_build_action.setCheckable(True)
        exclude_build_action.setChecked(self.exclude_from_build)
        exclude_build_action.triggered.connect(lambda: self.signal_exclude_from_build_toggled.emit())

        exclude_insights_action = cmenu.addAction("Exclude from Insights traces")
        exclude_insights_action.setCheckable(True)
        exclude_insights_action.setChecked(self.exclude_from_insights)
        exclude_insights_action.triggered.connect(lambda: self.signal_exclude_from_insights_toggled.emit())
        ...
        # View Live Log - only if Unreal is running
        if hasattr(self, '_device') and self._device and self._device.status >= DeviceStatus.OPEN:
            cmenu.addAction("View Live Log", lambda: self.signal_start_live_log.emit(self))
        ...
        # Fill DDC option - only if node is connected
        if self.connect_button.isChecked():
