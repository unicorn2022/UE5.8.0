...
def create_start_live_log_stream_message(log_file_path: str, tail_only: bool = False, process_uuid: str = None, start_offset: Optional[int] = None):
    ''' Creates the message to start SBL streaming the live log of the running Unreal/nDisplay process '''

    cmd_id = uuid.uuid4()
    cmd = {
        'command': 'start live log stream',
        'id': str(cmd_id),
        'log_file_path': log_file_path,
        'tail_only': tail_only
    }
    if process_uuid:
        cmd['process_uuid'] = process_uuid
...
def create_stop_live_log_stream_message(log_file_path: str):
    ''' Creates the message to stop SBL streaming the live log of the running Unreal/nDisplay process. '''

    cmd_id = uuid.uuid4()
    cmd = {
        'command': 'stop live log stream',
        'id': str(cmd_id),
        'log_file_path': log_file_path
    }
    message = json.dumps(cmd).encode() + b'\x00'
