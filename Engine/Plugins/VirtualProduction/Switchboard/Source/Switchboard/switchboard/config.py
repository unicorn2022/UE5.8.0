...
class FilePathListSetting(ListSetting):
    '''
    An array setting where each element is a file path. Each row provides a
    line edit and a Browse button that opens a file dialog filtered by
    file_path_filter. This is the list-style counterpart to FilePathSetting.
    '''
    ...
    def create_element(self, override_device_name: str, index: int) -> Tuple[QtWidgets.QWidget, object]:
        ...
        # Stash references so update_element_value can reach the line edit.
        ...
        browse_btn.clicked.connect(
            lambda _checked=False, line_edit=line_edit, override_device_name=override_device_name, index=index:
                self._on_browse_clicked(line_edit, override_device_name, index)
        )
    ...
    @override
    def update_element_value(self, editor_widget: QtWidgets.QWidget, list_value: list, index: int):
        editor_widget.line_edit.setText(list_value[index])
    ...
    def _get_start_path(self, current_value: str) -> str:
        if current_value and os.path.exists(current_value):
            return current_value
        if SETTINGS.LAST_BROWSED_PATH and os.path.exists(SETTINGS.LAST_BROWSED_PATH):
            return SETTINGS.LAST_BROWSED_PATH
        return str(pathlib.Path.home())

    def _on_browse_clicked(self, line_edit: QtWidgets.QLineEdit, override_device_name: str, index: int):
        start_path = self._get_start_path(line_edit.text())
        fs_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            parent=line_edit, dir=start_path, filter=self.file_path_filter)

        if not fs_path:
            return

        fs_path = os.path.normpath(fs_path)
        line_edit.setText(fs_path)
...
class Config(object):
    ...
    def init_muserver(self, data={}):
        self.mu_settings = {
            ...
            "muserver_filesharing_srv": DirectoryPathSetting(
                "muserver_filesharing_srv",
                "Directory or URI for File Sharing Service",
                data.get('muserver_filesharing_srv', '')
            ),
            "muserver_filesharing_enabled": BoolSetting(
                "muserver_filesharing_enabled",
                "Use File Sharing Service",
                data.get('muserver_filesharing_enabled', False)
            ),
        ...
        self.MUSERVER_FILESHARING_SRV = self.mu_settings["muserver_filesharing_srv"]
        self.MUSERVER_FILESHARING_ENABLED = self.mu_settings["muserver_filesharing_enabled"]
    ...
    def save_muserver(self, data):
        ...
        data["muserver_filesharing_srv"] = self.MUSERVER_FILESHARING_SRV.get_value() 
        data["muserver_filesharing_enabled"] = self.MUSERVER_FILESHARING_ENABLED.get_value() 
