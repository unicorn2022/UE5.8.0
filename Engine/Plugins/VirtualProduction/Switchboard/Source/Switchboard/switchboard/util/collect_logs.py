...
def zip_logs(zip_destination: str, config: Config, devices: typing.List[DeviceUnreal]):
    ...
    def copy_db_file(abs_file_path: str, zip_path_name: str, zip_file: ZipFile):
        ...
        # Track the copy path for cleanup before opening connections so a partial
        # failure (e.g. backup error) still removes any file sqlite3 created.
        # Cannot be removed before zip_file is closed.
        files_to_remove.append(copy_path)
        try:
            with contextlib.closing(sqlite3.connect(abs_file_path)) as original, \
                    contextlib.closing(sqlite3.connect(copy_path)) as copy:
    ...
    for file_to_remove in files_to_remove:
        try:
            ...
            # Backup never created (e.g. sqlite3.connect failed); nothing to clean up.
            pass
        except OSError as e:
            # Defensive: any lingering OS lock (Windows WinError 32) or permission issue.
            LOGGER.warning(f"Failed to remove temporary file {file_to_remove}: {e}")
