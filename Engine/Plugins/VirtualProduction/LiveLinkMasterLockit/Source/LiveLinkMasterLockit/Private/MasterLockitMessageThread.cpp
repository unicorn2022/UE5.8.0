...
void FMasterLockitMessageThread::ParsePacket(FArrayReader& InPacketData)
{
	...
	case EMasterLockitMessageType::ServerToClientNotification:
	{
		...
		case EServerNotificationMessageCode::DeviceMetaEvent:
		{
			...
			if (!JsonObject->HasField(FMasterLockitProtocol::CommonCategoryString) ||
				!JsonObject->HasField(FMasterLockitProtocol::VendorDataCategoryString))
			{
				break;
			}

			const TSharedPtr<FJsonObject>& commonData = JsonObject->GetFieldUntyped(FMasterLockitProtocol::CommonCategoryString)->AsObject();
			const TSharedPtr<FJsonObject>& vendorData = JsonObject->GetFieldUntyped(FMasterLockitProtocol::VendorDataCategoryString)->AsObject();
...
void FMasterLockitMessageThread::SendMessageToServer(const FArrayWriter InMessageToSend)
{
	...
	{
		UE_LOGF(LogMasterLockitMessageThread, Warning, "Full message was not sent to the MasterLockit server");
	}
}
...
void FMasterLockitMessageThread::ParseZeissLensData(FArrayReader& InData, const FMasterLockitDevice& Device)
{
	...
	{
		constexpr float SensorSize = 24.9f; // The width of a Super35 sensor, which is 24.9mm
