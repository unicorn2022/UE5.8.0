...
void FLiveLinkMasterLockitSource::OpenConnection()
{
	...
	{
		UE_LOGF(LogMasterLockitPlugin, Warning, "Ill-formed IP Address");
		return;
	}
	...
	{
		UE_LOGF(LogMasterLockitPlugin, Warning, "Could not connect the client socket to %ls:%d", *IPAddr.ToString(), PortNumber);
		return;
	}
