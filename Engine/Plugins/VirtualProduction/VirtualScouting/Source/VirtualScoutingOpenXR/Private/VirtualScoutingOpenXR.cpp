...
void FVirtualScoutingOpenXRExtension::PostCreateInstance(XrInstance InInstance)
{
	...
	if (CVarOpenXRDebugLogging.GetValueOnAnyThread() != 0)
	{
			...
			reinterpret_cast<PFN_xrVoidFunction*>(&PfnXrCreateDebugUtilsMessengerEXT))))
		{
			...
			{
				UE_LOGF(LogVirtualScoutingOpenXRDebug, Log, "XR_EXT_debug_utils messenger ACTIVE");
				return;
			}
		}
	}

	UE_LOGF(LogVirtualScoutingOpenXRDebug, Log, "XR_EXT_debug_utils messenger DISABLED");
}
...
TOptional<FName> FVirtualScoutingOpenXRExtension::TryGetHmdDeviceType()
{
	...
	UE_LOGF(LogVirtualScoutingOpenXR, Error, "Unsupported interaction profile: '%s'", PathBuf);
	return FName();
}
	...
	void* InUserData)
{
	...
	switch (InMessageSeverity)
	{
		...
		default:
			ensureMsgf(false, TEXT("Unhandled XrDebugUtilsMessageSeverityFlagsEXT: %llX"), InMessageSeverity);
