...
void FLiveLinkPrestonMDRSource::InitializeSettings(ULiveLinkSourceSettings* Settings)
{
	...
	{
		UE_LOGF(LogLiveLinkPrestonMDRSource, Warning, "Preston MDR Source coming from Preset is outdated. Consider recreating a Preston MDR Source. Configure it and resave as preset");
	}
}
...
bool FLiveLinkPrestonMDRSource::ShutdownMessageThreadAndSocket()
{
	...
	if (Socket)
	{
		if (!Socket->Close())
		{
			UE_LOGF(LogLiveLinkPrestonMDRSource, Warning, "Preston MDR Source socket failed to close.");
		}
...
void FLiveLinkPrestonMDRSource::OpenConnection()
{
	...
	if (!FIPv4Address::Parse(ConnectionSettings.IPAddress, IPAddr))
	{
		...
		UE_LOGF(LogLiveLinkPrestonMDRSource, Error, "Ill-formed IP Address");
		return;
	}
	...
	UE_LOGF(LogLiveLinkPrestonMDRSource, VeryVerbose, "Connecting to the MDR server at %ls", *Endpoint.ToString());

	if (!Socket->Connect(*Addr))
	{
		SourceStatus = EPrestonSourceStatus::ConnectionFailed;
		UE_LOGF(LogLiveLinkPrestonMDRSource, Error, "Could not connect to the MDR server at %ls", *Endpoint.ToString());
		return;
	}
...
void FLiveLinkPrestonMDRSource::OnConnectionLost()
{
	...
	UE_LOGF(LogLiveLinkPrestonMDRSource, Error, "Connection to the MDR device was lost");	
}
...
void FLiveLinkPrestonMDRSource::OnConnectionFailed()
{
	...
	UE_LOGF(LogLiveLinkPrestonMDRSource, Error, "Could not connect to the MDR server at %ls:%d", *ConnectionSettings.IPAddress, ConnectionSettings.PortNumber);
}
