...
uint32 FPrestonMDRMessageThread::Run()
{
	...
	UE_LOGF(LogPrestonMDRMessageThread, Verbose, "Commands Sent   : %d", NumCommandsSent);
	UE_LOGF(LogPrestonMDRMessageThread, Verbose, "Replies Received: %d", NumRepliedReceived);
...
void FPrestonMDRMessageThread::SendNextCommandToServer()
{
	...
	{
		UE_LOGF(LogPrestonMDRMessageThread, VeryVerbose, "Could not send any new requests to the MDR because the command queue was empty.");
		return;
	}
...
void FPrestonMDRMessageThread::ParseTimecodeStatusMessage(const uint8* const InData)
{
	...
	bIsDropFrameRate = (TimecodeStatus & 0x80) > 0;
}
