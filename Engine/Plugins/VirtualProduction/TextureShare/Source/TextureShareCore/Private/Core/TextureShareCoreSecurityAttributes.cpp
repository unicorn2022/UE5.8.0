...
bool FWindowsSecurityAttributesHelper::Initialize(const ETextureShareSecurityAttributesType InType)
{
	...
	if (!AllocateAndInitializeSid(&SIDAuthWorld, 1,
		...
		&pEveryoneSID))
	{
		UE_LOGF(LogTextureShareCoreWindows, Error, "AllocateAndInitializeSid Error %u", GetLastError());
		Release();
	...
	{
		UE_LOGF(LogTextureShareCoreWindows, Error, "SetEntriesInAcl Error %u", GetLastError());
		Release();
	...
	{
		UE_LOGF(LogTextureShareCoreWindows, Error, "LocalAlloc Error %u", GetLastError());
		Release();
	...
	{
		UE_LOGF(LogTextureShareCoreWindows, Error, "InitializeSecurityDescriptor Error %u", GetLastError());
		Release();
	...
	{
		UE_LOGF(LogTextureShareCoreWindows, Error, "SetSecurityDescriptorDacl Error %u", GetLastError());
		Release();
