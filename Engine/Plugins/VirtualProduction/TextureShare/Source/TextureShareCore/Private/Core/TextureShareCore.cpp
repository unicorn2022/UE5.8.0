...
bool FTextureShareCore::LockInterprocessMemory(const uint32 MaxMillisecondsToWait) const
{
	...
	if (InterprocessMemoryMutex.IsValid())
	{
		...
		UE_LOGF(LogTextureShareCore, Error, "MemoryMutex deadlock. released");
		...
		UE_LOGF(LogTextureShareCore, Error, "MemoryMutex deadlock#2 in-row. Disable mem access");
...
bool FTextureShareCore::InitializeInterprocessMemory()
{
	...
	if (!InterprocessMemoryMutex.IsValid())
	{
		...
		{
			UE_LOGF(LogTextureShareCore, Error, "Can't initialize process shared memory mutex");
	...
	if (!InterprocessMemoryRegion.IsValid())
	{
		...
		if (LockInterprocessMemory(500))
		{
			...
			if (!bResult)
			{
				UE_LOGF(LogTextureShareCore, Error, "Can't initialize shared memory region");
	...
	UE_LOGF(LogTextureShareCore, Error, "Can't lock process shared memory mutex");
...
TSharedPtr<ITextureShareCoreObject, ESPMode::ThreadSafe> FTextureShareCore::GetOrCreateCoreObject(const FString& InShareName, const ETextureShareProcessType InProcessType)
{
	if (bIsSessionActive)
	{
		...
		{
			UE_LOGF(LogTextureShareCore, Error, "GetGetOrCreate: the name is empty");
		...
		if (NewTextureShareCoreObject.IsValid() && NewTextureShareCoreObject->IsActive())
		{
			...
			UE_LOGF(LogTextureShareCore, Verbose, "Created object: '%ls'", *InShareName);

			return NewTextureShareCoreObject;
		}

		NewTextureShareCoreObject.Reset();
		UE_LOGF(LogTextureShareCore, Error, "Create Object: the object with name '%ls' not created", *InShareName);
	}
...
bool FTextureShareCore::RemoveCoreObject(const FString& InShareName)
{
	...
	{
		UE_LOGF(LogTextureShareCore, Verbose, "Removed object '%ls'", *InShareName);
