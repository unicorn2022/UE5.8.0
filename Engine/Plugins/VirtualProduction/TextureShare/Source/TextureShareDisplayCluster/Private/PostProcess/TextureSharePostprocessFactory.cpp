...
TSharedPtr<IDisplayClusterPostProcess, ESPMode::ThreadSafe> FTextureSharePostprocessFactory::Create(const FString& InPostProcessId, const FDisplayClusterConfigurationPostprocess* InConfigurationPostProcess)
{
	...
	{
		UE_LOGF(LogTextureShareDisplayClusterPostProcess, Error, "Can't Instantiating postprocess <%ls> id='%ls' : There can only be one TextureShare postprocess per application. ", *InConfigurationPostProcess->Type, *InPostProcessId);

		return nullptr;
	}

	UE_LOGF(LogTextureShareDisplayClusterPostProcess, Verbose, "Instantiating postprocess <%ls> id='%ls'", *InConfigurationPostProcess->Type, *InPostProcessId);
