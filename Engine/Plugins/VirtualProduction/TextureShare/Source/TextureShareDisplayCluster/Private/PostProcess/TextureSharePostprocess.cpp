...
bool FTextureSharePostprocess::HandleStartScene(IDisplayClusterViewportManager* InViewportManager)
{
	if (TextureShareAPI().IsObjectExist(DisplayClusterStrings::DefaultShareName))
	{
		...
		UE_LOGF(LogTextureShareDisplayClusterPostProcess, Warning, "TextureShareDisplayCluster: TS object for nDisplay are still referenced by someone");
	...
	// Re-use TextureShare object for nDisplay
	{
		...
		if (IsEnabled())
		{
			...
			UE_LOGF(LogTextureShareDisplayClusterPostProcess, Log, "TextureShareDisplayCluster: Initialized");
		...
		else
		{
			...
			UE_LOGF(LogTextureShareDisplayClusterPostProcess, Error, "TextureShareDisplayCluster: Failed - initization failed");
		}
	}
