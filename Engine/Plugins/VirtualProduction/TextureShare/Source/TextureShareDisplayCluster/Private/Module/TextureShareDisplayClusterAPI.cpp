...
bool FTextureShareDisplayClusterAPI::StartupModule()
{
	...
	{
		UE_LOGF(LogTextureShareDisplayCluster, Error, "nDisplay plugin not found.");
	...
	if (RenderMgr)
	{
		...
		{
			UE_LOGF(LogTextureShareDisplayCluster, Verbose, "Registering <%ls> projection policy factory...", *it->Key);

			if (!RenderMgr->RegisterProjectionPolicyFactory(it->Key, it->Value))
			{
				UE_LOGF(LogTextureShareDisplayCluster, Warning, "Couldn't register <%ls> projection policy factory", *it->Key);
			}
		}

		for (TPair<FString, TSharedPtr<IDisplayClusterPostProcessFactory>>& PPFactoryIt : PostprocessAssets)
		{
			UE_LOGF(LogTextureShareDisplayCluster, Verbose, "Registering <%ls> postprocess factory...", *PPFactoryIt.Key);

			if (!RenderMgr->RegisterPostProcessFactory(PPFactoryIt.Key, PPFactoryIt.Value))
			{
				UE_LOGF(LogTextureShareDisplayCluster, Warning, "Couldn't register <%ls> postprocess factory", *PPFactoryIt.Key);
			}
		}
	}
...
void FTextureShareDisplayClusterAPI::ShutdownModule()
{
	if (bModuleActive)
	{
		...
		if (RenderMgr)
		{
			...
			{
				UE_LOGF(LogTextureShareDisplayCluster, Verbose, "Un-registering <%ls> projection factory...", *it->Key);

				if (!RenderMgr->UnregisterProjectionPolicyFactory(it->Key))
				{
					UE_LOGF(LogTextureShareDisplayCluster, Warning, "An error occurred during un-registering the <%ls> projection factory", *it->Key);
				}
			}

			for (TPair<FString, TSharedPtr<IDisplayClusterPostProcessFactory>>& PPFactoryIt : PostprocessAssets)
			{
				UE_LOGF(LogTextureShareDisplayCluster, Verbose, "Un-registering <%ls> postprocess factory...", *PPFactoryIt.Key);

				if (!RenderMgr->UnregisterPostProcessFactory(PPFactoryIt.Key))
				{
					UE_LOGF(LogTextureShareDisplayCluster, Warning, "An error occurred during un-registering the <%ls> postprocess  factory", *PPFactoryIt.Key);
				}
			}
		}
	}
