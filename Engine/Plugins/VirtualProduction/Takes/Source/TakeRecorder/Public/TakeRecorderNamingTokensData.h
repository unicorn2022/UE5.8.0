...
public:
	...
	TAKERECORDER_API TArray<FString>& FindOrAddTokenKeysForField(const FString& InFieldName);

	/** Checks if a token key is in our undefined list. */
	TAKERECORDER_API bool IsTokenKeyUndefined(const FString& InTokenKey) const;
};