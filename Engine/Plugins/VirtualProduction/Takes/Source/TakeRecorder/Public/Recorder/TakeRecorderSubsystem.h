...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FPendingTakeCleared);
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnTakeRecorderSourceModifiedDynamic, UTakeRecorderSource*, Source);
...
public:
	...
	/** If we are using an existing sequence as a base or recording into an existing one. */
	UFUNCTION(BlueprintCallable, Category = TakeRecorder)
	UE_API virtual bool IsRecordingUsingExistingSequence() const override;
	...
	UFUNCTION(BlueprintCallable, Category = TakeRecorder)
	UE_API virtual void IncrementTakeNumber() override;
	...
	/** Retrieve the target record class of the Take Preset. */
	UFUNCTION(BlueprintCallable, Category = TakeRecorder)
	UE_API virtual UClass* GetPresetTargetRecordClass() const override;
	
	/** Set the target record class of the Take Preset. */
	UFUNCTION(BlueprintCallable, Category = TakeRecorder)
	UE_API virtual void SetPresetTargetRecordClass(const TSubclassOf<ULevelSequence> InClass) override;
	// ~End ITakeRecorderSubsystem interface

	/**
	 * Retrieve Additional Settings Objects - eg. The Audio Input Device to access Microphone Recorder settings.
	 */
	UFUNCTION(BlueprintCallable, Category = TakeRecorder, meta = (DeterminesOutputType="SettingsType"))
	UE_API UObject* GetAdditionalSettings(TSubclassOf<UObject> SettingsType);
	...
	/**
	 * Retrieve a multi-cast delegate that is triggered when a source is modified.
	 */
	UE_API UTakeRecorderSources::FOnSourceModified& GetOnRecordingSourceModifiedEvent();

	/**
	 * Retrieve a multi-cast delegate that is triggered when the pending take is cleared.
	 */
	UE_API FOnPendingTakeCleared& GetOnPendingTakeClearedEvent();
...
private:
	...
	/** Multicast delegate when any source has been modified. For native binding. */
	UTakeRecorderSources::FOnSourceModified OnRecordingSourceModifiedEvent;
	/** Multi-cast delegate when the pending take is cleared. For native binding. */
	FOnPendingTakeCleared OnPendingTakeClearedEvent;
...
public:
	...
	/** Multicast delegate when any source has been modified. */
	UPROPERTY(BlueprintAssignable, Category=TakeRecorder)
	FOnTakeRecorderSourceModifiedDynamic TakeRecorderSourceModified;

	/** Multicast delegate when the pending take is cleared. */
	UPROPERTY(BlueprintAssignable, Category=TakeRecorder)
	FPendingTakeCleared PendingTakeCleared;
};
