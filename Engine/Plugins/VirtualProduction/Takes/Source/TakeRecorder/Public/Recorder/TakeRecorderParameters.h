...
struct FTakeRecorderProjectParameters
{
	...
	/** Get the resolved root take save dir. The is the path that considers both the /Game and /Plugin based directories.  */
	TAKERECORDER_API FString GetResolvedRootTakeSaveDir() const;
	...
	*/
	...
	/** If spawnables should be overwritten during record. */
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, Category = "Take Recorder", meta = (ValidEnumValues = "OverwriteLegacy, CreateNew"))
	ETakeRecorderSpawnableOverwriteBehavior SpawnableOverwriteBehavior;

	/** The record behaviour of attachments and hierarchies. */
	UPROPERTY(config, EditAnywhere, BlueprintReadWrite, Category = "Take Recorder", meta = (ValidEnumValues = "None, Attachments, Hierarchy"))
	...
	/** Whether to switch to the simple sequencer view whilst recording. */
	UPROPERTY(config, BlueprintReadWrite, EditAnywhere, Category = "Take Recorder")
	bool bRecordWithSimpleSequencerView = false;
};
...
struct FTakeRecorderParameters
{
	...
	UPROPERTY(BlueprintReadWrite, Category="Take Recorder")
	FTakeRecorderHitchProtectionParameters HitchProtection;
