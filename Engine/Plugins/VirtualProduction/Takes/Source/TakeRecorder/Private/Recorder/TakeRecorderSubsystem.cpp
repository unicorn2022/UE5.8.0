...
bool UTakeRecorderSubsystem::IsRecordingUsingExistingSequence() const
{
	return Implementation->IsRecordingUsingExistingSequence();
}
...
void UTakeRecorderSubsystem::IncrementTakeNumber()
{
	Implementation->IncrementTakeNumber();
}
...
UClass* UTakeRecorderSubsystem::GetPresetTargetRecordClass() const
{
	return Implementation->GetPresetTargetRecordClass();
}

void UTakeRecorderSubsystem::SetPresetTargetRecordClass(const TSubclassOf<ULevelSequence> InClass)
{
	Implementation->SetPresetTargetRecordClass(InClass);
}

UObject* UTakeRecorderSubsystem::GetAdditionalSettings(TSubclassOf<UObject> SettingsType)
{
	if (SettingsType)
	{
		const TArray<TWeakObjectPtr<UObject>> AdditionalSettingsObjects  = GetMutableDefault<UTakeRecorderProjectSettings>()->AdditionalSettings;
		for (const TWeakObjectPtr<UObject>& AdditionalSettingsObject : AdditionalSettingsObjects)
		{
			if (AdditionalSettingsObject.IsValid() && AdditionalSettingsObject->GetClass() == SettingsType)
			{
				return AdditionalSettingsObject.Get();
			}
		}
	}
	return nullptr;
...
UTakeRecorderSources::FOnSourceModified& UTakeRecorderSubsystem::GetOnRecordingSourceModifiedEvent()
{
	return OnRecordingSourceModifiedEvent;
}

FOnPendingTakeCleared& UTakeRecorderSubsystem::GetOnPendingTakeClearedEvent()
{
	return OnPendingTakeClearedEvent;
}
