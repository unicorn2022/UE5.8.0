...
public:
	...
	virtual bool IsRecordingUsingExistingSequence() const override;
	...
	virtual void IncrementTakeNumber() override;
	...
	virtual UClass* GetPresetTargetRecordClass() const override;
	virtual void SetPresetTargetRecordClass(const TSubclassOf<ULevelSequence> InClass) override;
...
private:
	...
	/** Callback when any sources is modified. */
	void OnSourceModified(UTakeRecorderSource* Source);
...
private:
	...
	/** Helps determine if we are recording into an existing take or using one as a base. */
	bool bRecordingUsingExistingSequence = false;
};
