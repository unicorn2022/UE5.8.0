...
bool UTakeRecorder::Initialize ( ULevelSequence* LevelSequenceBase, UTakeRecorderSources* Sources, UTakeMetaData* MetaData, const FTakeRecorderParameters& InParameters, FText* OutError )
{
	...
	if (Parameters.Project.RecordingClockSource == EUpdateClockSource::Timecode)
	{
		...
		UE_LOGF(LogTakesCore, Warning, "Overriding Start at Current Timecode to True since the clock source is synced to Timecode.");
	}
	...
	// Suppress config file I/O during recording to prevent hitches from deferred
	// layout saves (FTabManager::SavePersistentLayout) and other config writes.
	// In-memory config updates still work normally; dirty configs are flushed
	// to disk once when recording stops.
...
#else // WITH_EDITOR
	UTakeMetaData* AssetMetaData = DuplicateObject(MetaData, SequenceAsset);
#endif // WITH_EDITOR
...
bool UTakeRecorder::SetupDestinationAsset(const FTakeRecorderParameters& InParameters, ULevelSequence* LevelSequenceBase, UTakeRecorderSources* Sources, UTakeMetaData* MetaData, FText* OutError, bool bOpenSequencer)
{
	...
	SequenceAsset = LevelSequenceBase;
	...
	{
		SequenceAsset = Cast<ULevelSequence>(PinnedSequencer->GetFocusedMovieSceneSequence());
	}
#endif // WITH_EDITOR
...
{
	// If the recording world has been destroyed (e.g. sandbox switch, map change), cancel gracefully.
	if (!GetWorld())
	{
		Cancel();
		return;
...
#else // WITH_EDITOR
	return TakesUtils::GetRecordTime(SequenceAsset, TimecodeAtStart,
		Parameters.Project.bStartAtCurrentTimecode, TimeDilation);
#endif // WITH_EDITOR
}
...
void UTakeRecorder::InternalTick(float DeltaTime)
{
	...
	UTakeRecorderSources* Sources = ITakeRecorderSourcesManager::GetChecked().FindOrAddSources(SequenceAsset);
...
void UTakeRecorder::PreRecord()
{
	...
	UTakeRecorderSources* Sources = ITakeRecorderSourcesManager::GetChecked().FindSources(SequenceAsset);
	check(Sources);

#if WITH_EDITOR
	UTakeMetaData* AssetMetaData = SequenceAsset->FindMetaData<UTakeMetaData>();
#else // WITH_EDITOR
	// Dummy out just a new UTakeMetaData object as it will not persist from a runtime environment anyway.
	UTakeMetaData* AssetMetaData = NewObject<UTakeMetaData>();
#endif // WITH_EDITOR
	...
	if (Parameters.User.bAutoSerialize)
	{
		...
		if (AssetMetaData->TryGenerateRootAssetPath(Parameters.Project.GetTakeAssetPath(), AssetName))
		{
			...
			{
				UE_LOGF(ManifestSerialization, Warning, "Error Opening Sequence Sequencer File: Subject '%ls' Error '%ls'", *(Name), *(Error.ToString()));
			}
		}
	}
...
#else // WITH_EDITOR
	TakeRecorderSourcesSettings.bSaveRecordedAssets = (!Parameters.bDisableRecordingAndSave && Parameters.User.bSaveRecordedAssets);
#endif // WITH_EDITOR
	...
	if (Sequencer.IsValid())
	{
		// Sequencer spawnable support.
		{
			// RecreateCurveEditor must come before RefreshTree. Stale ids may be present when Take Recorder
			// is recording a spawnable from sequencer, which will eventually trigger ensure(ParentInfo)
			// in the curve editor if we don't recreate it.
	...
	if (AWorldSettings* WorldSettings = RecordingWorld->GetWorldSettings())
	{
		...
		{
			UE_LOGF(LogTakesCore, Warning, "Time dilation cannot be 0. Ignoring time dilation for this recording.");
		}
...
void UTakeRecorder::Start()
{
	...
	if (!ShouldShowNotifications())
	{
		...
		UE_LOGF(LogTakesCore, Log, "Started recording");
	}
...
void UTakeRecorder::StopInternal(const bool bCancelled)
{
	...
	if (bDidEverStartRecording)
	{
		if (!ShouldShowNotifications())
		{
			...
			{
				UE_LOGF(LogTakesCore, Log, "Stopped recording");
			}
			else
			{
				UE_LOGF(LogTakesCore, Log, "Recording cancelled");
			}
		}
		...
		if (bRecordingFinished)
		{
			...
			if (Parameters.User.bAutoLock)
			{
				// When auto-locking, lock the meta data so it can't be changed without implicitly unlocking it now
				AssetMetaData->Lock();
			}
			...
			if (Parameters.User.bSaveRecordedAssets)
			{
				...
				if (Sequencer)
				{
					// Ensure we don't have any external references prior to saving. This automatically handles possessables vs spawnables.
					TakesUtils::CleanupExternalReferences(SequenceAsset);
	...
	if (SequenceAsset)
	{
		...
		UE_LOGF(LogTakesCore, Log, "Finished processing %ls in %0.2f seconds", *PackageName, ElapsedTime);
	}
