...
void UTakeRecorderNearbySpawnedActorSource::HandleActorSpawned(AActor* Actor, class ULevelSequence* InSequence)
{
	...
	UE_LOGF(LogTakesCore, Log, "Actor: %ls IsValid: %d Unreachable: %d PendingKillPending: %d", *Actor->GetName(), ::IsValid(Actor), Actor->IsUnreachable(), Actor->IsPendingKillPending());

	if (UTakeRecorderActorSource* ActorSource = TakeRecorderSourcesUtils::StartRecordingActorDuringSession(Actor, InSequence))
	{
