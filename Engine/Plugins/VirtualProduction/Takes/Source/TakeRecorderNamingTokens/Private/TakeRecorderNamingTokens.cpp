...
void UTakeRecorderNamingTokens::OnCreateDefaultTokens(TArray<FNamingTokenData>& Tokens)
{
	...
	Tokens.Add({
		...
		#else // WITH_EDITOR
			// This loses the settings aspect - to consider if we want to try to bring this to runtime as well.
			const int32 TakeNumDigits = 2;
		#endif // WITH_EDITOR
	        return FText::FromString(FString::Printf(TEXT("%0*d"), TakeNumDigits, TakeMetaData.IsValid() ? TakeMetaData->GetTakeNumber() : 0));
	    })
	});
	...
	Tokens.Add({
	    ...
	    FNamingTokenData::FTokenProcessorDelegateNative::CreateLambda([this]{
	        return FText::FromString(TakeMetaData.IsValid() ? TakeMetaData->GetSlate() : TEXT("Invalid"));
	    })
	});
	...
	Tokens.Add({
		...
		FNamingTokenData::FTokenProcessorDelegateNative::CreateLambda([this]{
			...
			{
			#if WITH_EDITOR
				// GetActorLabel is an Editor only function.
				return FText::FromString(Context->Actor->GetActorLabel());
			#else // WITH_EDITOR
				return FText::FromString(Context->Actor->GetName());
			#endif // WITH_EDITOR
			}

			UE_LOGF(LogTakeRecorderNamingTokens, Verbose, "Attempted to use 'actor' naming token but no context is available.");
	...
	Tokens.Add({
		...
		FNamingTokenData::FTokenProcessorDelegateNative::CreateLambda([this]{
			...
			UE_LOGF(LogTakeRecorderNamingTokens, Verbose, "Attempted to use 'channel' naming token but no context is available.");
