...
namespace TakesUtils
{
	 ...
	 *
	 * Taken from ObjectTools and moved a small subset here to remove the need to call into ObjectTools and allow
	 * the same functionality in a non-Editor environment.
	 */
	TAKESCORE_API FString SanitizeInvalidChars(const FString& InText, const TCHAR* InvalidChars);
	 ...
	 * the same functionality in a non-Editor environment.
	 */
	TAKESCORE_API FString SanitizeObjectName(const FString& InObjectName);

	/**
	 * Replaces all invalid package name characters with _.
	 *
	 * Taken from PackageTools and moved a small subset here to remove the need to call into PackageTools and allow
	 * the same functionality in a non-Editor environment.
	 */
	TAKESCORE_API FString SanitizePackageName(const FString& InPackageName);
	...
	TAKESCORE_API void CleanupExternalReferences(UObject* InObject);
	
	/** 
	 * Save an object to disk. 
	 * @param InObject The object to save.
	 * @param bAlwaysKeepExternalReferences If we should always keep external references.
	 * When false, we clear them. When false and the InObject is a Level Sequence, we perform fine-grained cleanup.
	 */
	TAKESCORE_API void SaveAsset(UObject* InObject, bool bAlwaysKeepExternalReferences = true);
	...
	*/
	...
	{
		auto MakeAssetObjectPath = [](const FString& BaseAssetPath, const FString& AssetName) -> FSoftObjectPath
			{
				return FSoftObjectPath((BaseAssetPath / AssetName) + TEXT(".") + AssetName);
			};

		FString SanitizedBaseAssetPath = SanitizePackageName(BaseAssetPath);
		FString SanitizedBaseAssetName = SanitizePackageName(BaseAssetName);

		FString SourceAssetName = FPaths::MakeValidFileName(SanitizedBaseAssetName);
		FSoftObjectPath AssetObjectPath = MakeAssetObjectPath(SanitizedBaseAssetPath, SourceAssetName);

		FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
		FAssetData AssetData = AssetRegistryModule.Get().GetAssetByObjectPath(AssetObjectPath);

		// if object with same name exists, try a different name until we don't find one
		int32 ExtensionIndex = 0;
		FString AssetName = SourceAssetName;
		while (AssetData.IsValid() && AssetData.GetClass() == AssetType::StaticClass())
		{
			AssetName = FString::Printf(TEXT("%s_%d"), *SourceAssetName, ExtensionIndex);
			AssetObjectPath = MakeAssetObjectPath(SanitizedBaseAssetPath, AssetName);
			AssetData = AssetRegistryModule.Get().GetAssetByObjectPath(AssetObjectPath);

			ExtensionIndex++;
		}

		// Create the new asset in the package we just made
		const FString AssetPath = (SanitizedBaseAssetPath / AssetName);
		...
		UE_LOGF(LogTakesCore, Error, "Couldn't create file for package %ls", *AssetPath);
	...
	/** Find the shared playback state from the current Sequencer. This can return nullptr. */
	TAKESCORE_API TSharedPtr<const UE::MovieScene::FSharedPlaybackState> FindSequencerSharedPlaybackState(ULevelSequence* LevelSequence);
#endif // WITH_EDITOR


	/* The time at which to record, based on timecode */
	TAKESCORE_API FQualifiedFrameTime GetRecordTime(ULevelSequence* SequenceAsset,
		const FTimecode& TimecodeAtStart, bool bStartAtCurrentTimecode, float TimeDilation = 1.f);
	
	/** Retrieve an actor guid for moviescene. */
	TAKESCORE_API FGuid ResolveActorFromSequence(const AActor* InActor, const ULevelSequence* CurrentSequence);

	/** Retrieve an actor guid for the given sequence, providing a shared playback state. This uses the shared playback state as the lookup context for the binding. */
	TAKESCORE_API FGuid ResolveActorFromSequence(const AActor* InActor, const ULevelSequence* CurrentSequence, TSharedPtr<const UE::MovieScene::FSharedPlaybackState> SharedPlaybackState);
	
	/** Checks if a source is considered possessable. */
	TAKESCORE_API bool IsSourcePossessable(const UTakeRecorderSource* InSource);

	/**
	 * Returns true if the track has any keys across all of its sections and channels.
	 */
	TAKESCORE_API bool DoesTrackHaveAnyKeys(UMovieSceneTrack& Track);
} // namespace TakesUtils
