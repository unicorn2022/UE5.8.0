...
#include "TakeRecorderSpawnableBehavior.h"
...
{
	class FTakeRecorderSourcesModule;	
}
	

/**
 * A list of sources to record for any given take. 
 * Is stored and looked up against a level sequence using the ITakeRecorderSourcesManager.
 * Note that within an editor environment, ITakeRecorderSourcesManager will store the sources object
 * as meta-data on ULevelSequence through ULevelSequence::FindMetaData<UTakeRecorderSources>.
 */
...
public:
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnSourceModified, UTakeRecorderSource* Source);
	...
	/** Multicast delegate for when any source is modified. */
	static UE_API FOnSourceModified& OnSourceModified();
	
private:
	friend class UE::TakeRecorderSources::FTakeRecorderSourcesModule;
	
	DECLARE_DELEGATE_RetVal_TwoParams(UTakeRecorderSource*, FOnActorBindingAdded, AActor*, ULevelSequence*);
	...
	static UE_API FOnActorBindingAdded& OnAddSourceForActorBinding();
	
public:
	/**
	 * Notifies sources that a new actor binding has been added.
	 * 
	 * @param InActor The actor the binding was added for.
	 * @param InLevelSequence The level sequence of the binding.
	 */
	UE_API ULevelSequence* AddSourceForActorBinding(AActor* InActor, ULevelSequence* InLevelSequence);
	
	/** Checks if a given actor is possessable given source configuration. */
	UE_API bool IsActorPossessable(const AActor* InActor) const;
...
private:
	static UE_API ULevelSequence* FindExistingSubSequence(const ULevelSequence* InRootSequence, const FString& InSubSequenceAssetName);
	...
	void StartPreRecordingRecursive(const TArray<UTakeRecorderSource*>& InSources, ULevelSequence* InRootSequence, TArray<UTakeRecorderSource*>& NewSourcesOut,
		FManifestSerializer* InManifestSerializer);
	
	/** Calls PreRecording on sources recursively allowing them to create other sources which properly get PreRecording called on them as well. */
	UE_API void PreRecordingRecursive(TArray<UTakeRecorderSource*> InSources, ULevelSequence* InRootSequence, TArray<UTakeRecorderSource*>& NewSourcesOut,
		FManifestSerializer* InManifestSerializer, TSet<FString>& UsedTrackNames);
...
private:
	...
	/** Bindings added for an actor mapped to whether it is possessable or not. Reset on PreRecord. */
	TMap<TWeakObjectPtr<AActor>, bool> AddedActorPossessableBindings;
