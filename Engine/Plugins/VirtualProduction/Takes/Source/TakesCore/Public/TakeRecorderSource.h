...
public:
	...
	*/
	...
	/** Called after all sources have had PreRecording run. */
	virtual void OnAllSourcesPreRecordingFinished() { }
	...
	*/
	...
	/**
	 * Optional function to allow sources to decide if they want to create folders for their contents or not.
	 */
	virtual bool ShouldCreateFolderForContents() const { return true; }
	...
	/**
	 * Whether this source should overwrite existing track data, or leave it, or potentially create new data.
	 * This is primarily used in response to Spawnables, such as we if we want to overwrite subsequence data
	 * or clear existing data in non-subsequence tracks.
	 */
	UE_API virtual bool OverwriteExistingTrackData() const;

	/**
	 * Called by UTakeRecorderSources right before a Source is removed from its internal map.
	 * Allows for any cleanup or logic the Source needs to do when being removed.
	 */
...
public:
	...
	FText GetAddSourceDisplayText() const
	{
		...
		if (Result.IsEmpty())
		{
			ensureMsgf(IsTemplate(RF_ClassDefaultObject) /* This can be called on the CDO. */, TEXT("You must provide localized source name."));
	...
	/** If this source implements the IMovieSceneTrackRecorderHost interface. This is used to avoid RTTI. */
	virtual const class IMovieSceneTrackRecorderHost* AsTrackRecorderHost() const { return nullptr; }
	
	/** If this source is an actor source for the given actor. Used for cross-module communication. */
	virtual bool IsActorSourceForActor(const AActor* Actor) const { return false; }
