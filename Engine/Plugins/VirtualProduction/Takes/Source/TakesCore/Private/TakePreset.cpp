...
FSimpleMulticastDelegate UTakePreset::OnTakePresetAllocatedDelegate;
...
FSimpleMulticastDelegate& UTakePreset::OnTakePresetAllocated()
{
	return OnTakePresetAllocatedDelegate;
}
...
UTakePreset* UTakePreset::AllocateTransientPreset(UTakePreset* TemplatePreset)
{
	...
	OnTakePresetAllocatedDelegate.Broadcast();
...
void UTakePreset::CreateLevelSequence()
{
	...
	const float DefaultStartTime = ProjectSettings->DefaultStartTime;
	const float DefaultDuration = ProjectSettings->DefaultDuration;
#else // WITH_EDITOR
	// These values are the default values from the Project Settings.
	const float DefaultStartTime = 0.0f;
	const float DefaultDuration = 5.0f;
#endif // WITH_EDITOR

	UMovieScene* MovieScene = LevelSequence->GetMovieScene();
	FFrameNumber StartFrame = (DefaultStartTime * MovieScene->GetTickResolution()).RoundToFrame();
	int32        Duration = (DefaultDuration * MovieScene->GetTickResolution()).RoundToFrame().Value;
