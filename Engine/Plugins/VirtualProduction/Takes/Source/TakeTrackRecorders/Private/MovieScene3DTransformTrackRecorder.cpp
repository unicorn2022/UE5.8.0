...
void UMovieScene3DTransformTrackRecorder::CreateTrackImpl()
{
	...
	{
		// If the existing track is not empty, allow it to evaluate during the recording, and we will remove it at the end.
		if (TakesUtils::DoesTrackHaveAnyKeys(*MovieSceneTrack))
		{
			// Track the existing section to remove in finalize.
			// We cannot use the new section to drive the right attachment as it is set to not active so it is does not get evaluated in sequencer while recording.
			// Therefore if we have an attachment, we need to keep any existing data around, and then remove it once we have finished recording to allow
			// the recorded section to then take control.
			for (UMovieSceneSection* Section : MovieSceneTrack->GetAllSections())
			{
				SectionsToRemoveOnFinalize.Add(Section);
			}
		}
		else
		{
	...
	if (MovieSceneTrack.IsValid())
	{
		...
		if (!TransformSerializer.OpenForWrite(FileName, Header, Error))
		{
			...
			UE_LOGF(TransformSerialization, Warning, "Error Opening Transform Sequencer File: Object '%ls' Error '%ls'", *(Name), *(Error.ToString()));
		}
	}
}
...
void UMovieScene3DTransformTrackRecorder::FinalizeTrackImpl()
{
 	...
 	FScopedSlowTask SlowTask(5.0f, NSLOCTEXT("TakeRecorder", "ProcessingTransforms", "Processing Transforms"));

	// Remove any sections we want to remove on finalize
	for (const TWeakObjectPtr<UMovieSceneSection>& WeakSection : SectionsToRemoveOnFinalize)
	{
		if (TStrongObjectPtr<UMovieSceneSection> StrongSection = WeakSection.Pin())
		{
			MovieSceneTrack->RemoveSection(*StrongSection.Get());
		}
	}
	SectionsToRemoveOnFinalize.Reset();
...
bool UMovieScene3DTransformTrackRecorder::ResolveTransformToRecord(FTransform& OutTransform)
{
 	...
 	else if(AActor* Actor = Cast<AActor>(ObjectToRecord.Get()))
 	{
		...
		// if the attach parent has changed, recache its sequence information
		if (AttachParent != ActiveAttachParent)
		{
			if (AttachParent != nullptr && OwningTakeRecorderSource != nullptr)
			{
				ActiveAttachParent = AttachParent;
				AttachParentSequenceInformation = MakeShared<UE::TakesCore::FActorSequenceInformation>(OwningTakeRecorderSource->GetRootLevelSequence(), AttachParent->GetOwner());
			}
			else
			{
				ActiveAttachParent = nullptr;
				AttachParentSequenceInformation.Reset();
			}
		}
		...
		if (AttachParent && OwningTakeRecorderSource)
 		{
			const bool bIsAttachParentControlledBySequence = AttachParentSequenceInformation.IsValid()
				? AttachParentSequenceInformation->IsControlledBySequence()
				: false;
			const bool bIsAttachParentBeingRecorded = OwningTakeRecorderSource->IsOtherActorBeingRecorded(AttachParent->GetOwner());

 			// We capture world space transforms for actors if they're attached, but we're not recording the attachment parent or the parent being controlled by a sequence.
			FTrackRecorderSettings TrackRecorderSettings = OwningTakeRecorderSource->GetTrackRecorderSettings();
			ETakeRecorderAttachRecordBehaviour AttachRecordBehaviour = TrackRecorderSettings.AttachRecordBehaviour;

			switch (AttachRecordBehaviour)
			{
				case ETakeRecorderAttachRecordBehaviour::None:
				{
					bCaptureWorldSpaceTransform = bIsAttachParentBeingRecorded || bIsAttachParentControlledBySequence
						? !TrackRecorderSettings.bRecordToPossessable
						: true;
				} break;
				case ETakeRecorderAttachRecordBehaviour::Attachments:
				{
					bCaptureWorldSpaceTransform = !bIsAttachParentBeingRecorded && !bIsAttachParentControlledBySequence;
				} break;
				default: bCaptureWorldSpaceTransform = false; break;
			}
 		}
...
{
	// Post processing of animation data is only done at editor time.
	// Runtime recording will handle this separately as it cannot use the FBufferedTransformKeys.
#if WITH_EDITOR
	...
	if (AnimTrackRecorder->RootWasRemoved())
	{
		...
		if (SkeletalMeshComponent)
		{
			...
			if (AnimSequence && SkinnedAsset && AnimSequence->GetSkeleton())
			{
				...
				// Need to revisit this when implementing the runtime implementation.
				...
				if (RootBoneName == NAME_None)
				{
					...
					UE_LOGF(LogTakesCore, Log, "No Root Found for (%ls)", *ObjectToRecordName);
					return;
				}
