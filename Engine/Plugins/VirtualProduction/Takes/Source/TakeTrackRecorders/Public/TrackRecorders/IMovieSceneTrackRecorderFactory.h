...
public:
	...
	*/
	...
	/**
	*  Unique Name that's stored in the serialized manifest when it's recorded
	*/
	virtual FName GetSerializedType() const { return FName(); }
	
	/**
	 * Allow a factory to handle individual properties on an object, regardless of the property name.
	 */
	virtual bool CanHandlePropertyInstance() const { return false; }
	
	/**
	 * Allows a factory to determine if it wants to be created for a given host.
	 */
	virtual bool CanCreateTrackRecorderForHost(const IMovieSceneTrackRecorderHost* Host, UObject* ObjectToRecord) const { return true; }
};
