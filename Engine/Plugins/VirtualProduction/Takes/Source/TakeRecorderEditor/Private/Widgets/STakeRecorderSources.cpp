...
private:
	...
	void SetCheckState(const ECheckBoxState NewState, TWeakPtr<STakeRecorderSources> WeakSourcesWidget)
	{
		...
		if (ThisSource && SourcesWidget.IsValid())
		{
			...
			FProperty* EnabledProperty = UTakeRecorderSource::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UTakeRecorderSource, bEnabled));
			...
			if (!SelectedSources.Contains(ThisSource))
			{
				...
				ThisSource->PostEditChangeProperty(PropertyChangedEvent);
			}
			else 
			{
				...
				for (UTakeRecorderSource* SelectedSource : SelectedSources)
				{
					...
					FPropertyChangedEvent PropertyChangedEvent(EnabledProperty, EPropertyChangeType::ValueSet);
					SelectedSource->PostEditChangeProperty(PropertyChangedEvent);
				}
			}
		}
	}
		...
		if (UTakeRecorderSource* Source = WeakSource.Get())
		{
			if (ULevelSequence* OwningSequence = Source->GetTypedOuter<ULevelSequence>())
			{
				if (UTakeMetaData* TakeMetaData = OwningSequence->FindMetaData<UTakeMetaData>())
				{
					...
					const bool bRecordIsIntoSequence = Subsystem != nullptr ? Subsystem->GetTakeRecorderMode() == ETakeRecorderMode::RecordIntoSequence : false;
					if (TakeMetaData->Recorded() && !bRecordIsIntoSequence)
					{
