...
namespace UE::TakeRecorder::Private
{
	...
	{
		return !InTokenKey.Equals(TokenKeySlate, ESearchCase::IgnoreCase) && !InTokenKey.Equals(TokenKeyTake, ESearchCase::IgnoreCase);
	}
	
	/** The token provided matches our own tokens exactly. */
	bool IsExplicitlyOurToken(const FString& InTokenKey)
	{
		return InTokenKey.Equals(TokenKeyTake, ESearchCase::CaseSensitive) || InTokenKey.Equals(TokenKeySlate, ESearchCase::CaseSensitive);
	}
}
...
void STakeRecorderCockpit::UpdateRecordError()
{
	...
	// Recording into an existing sequence with timecode-relative start would shift every
	// existing section by the current timecode delta and replace the playback range,
	// invalidating previously recorded data (UE-375286).
	if (TakeRecorderSubsystem->GetTakeRecorderMode() == ETakeRecorderMode::RecordIntoSequence)
	{
		const FTakeRecorderProjectParameters& ProjectSettings = GetDefault<UTakeRecorderProjectSettings>()->Settings;
		if (ProjectSettings.bStartAtCurrentTimecode)
		{
			RecordErrorText = LOCTEXT("ErrorWidget_RecordIntoSequenceStartAtTimecode",
				"Recording into an existing sequence is not supported when 'Start At Current Timecode' is enabled, "
				"because it would shift the existing sections and invalidate previously recorded data. "
				"Disable 'Start At Current Timecode' in the Take Recorder project settings.");
			return;
		}
		if (ProjectSettings.RecordingClockSource == EUpdateClockSource::Timecode)
		{
			RecordErrorText = LOCTEXT("ErrorWidget_RecordIntoSequenceTimecodeClock",
				"Recording into an existing sequence is not supported when the recording clock source is set to Timecode, "
				"because it forces sections to start at the current timecode and would invalidate previously recorded data. "
				"Change the Recording Clock Source in the Take Recorder project settings.");
			return;
		}
	}

	UTakeRecorderSources* SourcesContainer = ITakeRecorderSourcesManager::GetChecked().FindSources(Sequence);
...
FReply STakeRecorderCockpit::NewRecordingFromThis()
{
	...
	if (DockTab.IsValid())
	{
		...
		if (const TSharedPtr<STakeRecorderPanel> TakeRecorderPanel = TabContent->GetTakeRecorderPanel().Pin())
		{
			// Go through panel rather than subsystem so sequencer remains open if necessary.
			TakeRecorderPanel->ResetToPendingTake();
			TakeRecorderSubsystem->IncrementTakeNumber();
		}
	}
...
void STakeRecorderCockpit::CreateUserTokensUI()
{
	...
	// been fully populated so we don't remove custom tokens that are still in use.
	{
		...
		TMap<FNamingTokenData, FText>& ExistingUserDefinedTokens = GetNamingTokensData()->UserDefinedTokens;
		TMap<FNamingTokenData, FText> ActiveUserTokens;
		for (const FTakeRecorderNamingTokensFieldMapping& UndefinedTokens : GetNamingTokensData()->FieldToUndefinedKeys)
		{
			...
			{
				bool bUserTokenAdded = false;
				FNamingTokenData UndefinedToken(UndefinedTokenStr);
				if (!ActiveUserTokens.Contains(UndefinedToken)
					&& UE::TakeRecorder::Private::CanTokenBeUserDefined(UndefinedTokenStr))
				{
					...
					ActiveUserTokens.Add(UndefinedToken);
					bUserTokenAdded = true;
					
					// Restore any previous values.
					if (FText* ExistingTokenValue = ExistingUserDefinedTokens.Find(UndefinedToken))
					{
						ActiveUserTokens[UndefinedToken] = *ExistingTokenValue;
					}
				}
				if (bUserTokenAdded || UE::TakeRecorder::Private::IsExplicitlyOurToken(UndefinedToken.TokenKey))
				{
		...
		// Overwrite all user tokens with active tokens. This destroys any custom tokens not in use, preventing the token
		// suggestion dropdown from being cluttered with stale entries, since each letter typed technically adds a new token.
		ExistingUserDefinedTokens = MoveTemp(ActiveUserTokens);
	}
