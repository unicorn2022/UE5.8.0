...
TSharedRef<SWidget> STakeRecorderPanel::MakeSimpleViewToggleButton()
{
	return SNew(SCheckBox)
		.Padding(TakeRecorder::ButtonPadding)
		.ToolTipText(NSLOCTEXT("TakeRecorder", "ToggleSequencerSimpleView_Tip", "Auto collapse to simplified sequencer while recording."))
		.Style(FAppStyle::Get(), "ToggleButtonCheckbox")
		.IsEnabled_Lambda([this] 
			{ 
				if (TStrongObjectPtr<UTakeRecorderSubsystem> TRSubsystem = TakeRecorderSubsystem.Pin())
				{
					return !TRSubsystem->IsRecording();
				}
				// Default to enabled if no subsystem is present.
				return true; 
			})
		.IsChecked_Lambda([]()
			{
				if (const UTakeRecorderProjectSettings* Settings = GetDefault<UTakeRecorderProjectSettings>())
				{
					return Settings->Settings.bRecordWithSimpleSequencerView ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
				}
				return ECheckBoxState::Unchecked;
			})
		.OnCheckStateChanged_Lambda([](ECheckBoxState InNewState)
			{
				if (UTakeRecorderProjectSettings* Settings = GetMutableDefault<UTakeRecorderProjectSettings>())
				{
					Settings->Settings.bRecordWithSimpleSequencerView = InNewState == ECheckBoxState::Checked;
					Settings->SaveConfig();
				}
			})
		[
			SNew(SImage)
				.Image(FAppStyle::Get().GetBrush("Sequencer.SimpleView.ToggleToSimpleView"))
		];
}
...
void STakeRecorderPanel::ResetToPendingTake()
{
