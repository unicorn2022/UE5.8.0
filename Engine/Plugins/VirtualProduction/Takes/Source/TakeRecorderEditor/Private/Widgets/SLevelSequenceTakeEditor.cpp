	...
	[
		...
		[
			...
			[
				...
				.HintText(LOCTEXT("SearchSettingsHint", "Search Settings"))
				.OnTextChanged(this, &SLevelSequenceTakeEditor::OnSearchTextChanged)
				.OnTextCommitted(this, &SLevelSequenceTakeEditor::OnSearchTextCommitted)
			]
...
void SLevelSequenceTakeEditor::CheckForNewLevelSequence()
{
	...
	if (CachedLevelSequence != NewLevelSequence)
	{
		...
		UTakeRecorderSources* Sources = ITakeRecorderSourcesManager::GetChecked().FindOrAddSources(NewLevelSequence);
...
TSharedRef<SWidget> SLevelSequenceTakeEditor::OnGenerateSourcesMenu()
{
	TSharedRef<FExtender> Extender = MakeShared<FExtender>();
	{
		...
		UTakeRecorderSources* Sources       = ITakeRecorderSourcesManager::GetChecked().FindOrAddSources(LevelSequence);
...
void SLevelSequenceTakeEditor::OnSearchTextChanged(const FText& InFilterText)
{
	CurrentSearchText = InFilterText;
	const bool bIsSearchActive = !InFilterText.IsEmpty();

	for (const auto& Pair : ClassToDetailsView)
	{
		if (Pair.Value.IsValid())
		{
			Pair.Value->SetSearchText(InFilterText);

			// Collapse details views that have no matching properties to avoid empty "All results have been filtered" messages
			if (bIsSearchActive && Pair.Value->GetPropertiesInOrderDisplayed().Num() == 0)
			{
				Pair.Value->SetVisibility(EVisibility::Collapsed);
			}
			else
			{
				Pair.Value->SetVisibility(EVisibility::Visible);
			}
		}
	}
}

void SLevelSequenceTakeEditor::OnSearchTextCommitted(const FText& InFilterText, ETextCommit::Type CommitType)
{
...
void SLevelSequenceTakeEditor::AddDetails(const TPair<const UClass*, TArray<UObject*> >& Pair, TArray<FObjectKey>& PreviousClasses)
{
	...
	else
	{
		...
		bool bDetailsCanBeEnabled = true;
		if (ULevelSequence* AttrLevelSequence = LevelSequenceAttribute.IsSet() ? LevelSequenceAttribute.Get() : nullptr)
		{
			if (UTakeMetaData* TakeMetaData = AttrLevelSequence->FindMetaData<UTakeMetaData>())
			{
				const UTakeRecorderSubsystem* Subsystem = GEngine->GetEngineSubsystem<UTakeRecorderSubsystem>();
				const bool bRecordIsIntoSequence = Subsystem != nullptr && Subsystem->GetTakeRecorderMode() == ETakeRecorderMode::RecordIntoSequence;
				bDetailsCanBeEnabled = !TakeMetaData->Recorded() || bRecordIsIntoSequence;
			}
		}
		Details->SetEnabled(bDetailsCanBeEnabled);
		...
		{
			Details->SetSearchText(CurrentSearchText);

			if (Details->GetPropertiesInOrderDisplayed().Num() == 0)
			{
