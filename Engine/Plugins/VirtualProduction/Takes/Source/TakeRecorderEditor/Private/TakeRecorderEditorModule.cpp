...
#include "Customization/NamingTokenDirectoryPathCustomization.h"
...
IMPLEMENT_MODULE(FTakeRecorderEditorModule, TakeRecorderEditor);
...
namespace UE::TakeRecorder::Private
{
	...
	/** Get the sequencer for the given TakeRecorders sequence. Can return null. */
	TSharedPtr<ISequencer> GetSequencerForRecorder(UTakeRecorder* InRecorder)
	{
		if (!GIsEditor || !GEditor || InRecorder == nullptr)
		{
			return nullptr;
		}

		ULevelSequence* TargetSequence = InRecorder->GetSequence();
		if (TargetSequence == nullptr)
		{
			return nullptr;
		}

		TSharedPtr<ISequencer> TargetSequencer = nullptr;

		if (UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
		{
			if (IAssetEditorInstance* Editor = AssetEditorSubsystem->FindEditorForAsset(TargetSequence, /*bFocusIfOpen*/ false);
				Editor != nullptr && Editor->GetEditorName() == TEXT("LevelSequenceEditor"))
			{
				ILevelSequenceEditorToolkit* Toolkit = static_cast<ILevelSequenceEditorToolkit*>(Editor);
				TargetSequencer = Toolkit->GetSequencer();
			}
		}

		return TargetSequencer;
	}
} // namespace UE::TakeRecorder::Private
...
namespace
{
	static TSharedRef<SDockTab> SpawnTakesBrowserTab(const FSpawnTabArgs& SpawnTabArgs)
	{
		...
		TSharedRef<SWidget> NewBrowser = ContentBrowser.CreateContentBrowser( ITakeRecorderModule::TakesBrowserInstanceName, NewTab, nullptr );
		...
		ContentBrowser.SyncBrowserToFolders(TakesFolders, true, false, ITakeRecorderModule::TakesBrowserInstanceName);
...
FTakeRecorderEditorModule::FTakeRecorderEditorModule()
	: SequencerSettings(nullptr)
{
}

void FTakeRecorderEditorModule::StartupModule()
{
	...
	if (GIsEditor)
	{
		...
		{
			RegisterMenus();
			bMenusRegisteredAtModuleStartup = true;
		}
		UE::TakeRecorder::Private::InitStatCommands();
	}

	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FTakeRecorderEditorModule::HandleOnPostEngineInit);

	if (GEditor)
	{
		GEditor->OnEditorClose().AddRaw(this, &FTakeRecorderEditorModule::OnEditorClose);
	}
}

void FTakeRecorderEditorModule::ShutdownModule()
{
	// If we have an active recorder, ensure we unhook our callbacks from it.
	if (TStrongObjectPtr<UTakeRecorder> StrongActiveRecorder = WeakActiveRecorder.Pin())
	{
		StrongActiveRecorder->OnRecordingCancelled().RemoveAll(this);
		StrongActiveRecorder->OnRecordingFinished().RemoveAll(this);
		StrongActiveRecorder.Reset();
	}

	if (GEngine)
	{
		if (UTakeRecorderSubsystem* TRSubSystem = GEngine->GetEngineSubsystem<UTakeRecorderSubsystem>())
		{
			TRSubSystem->GetOnRecordingInitializedEvent().RemoveAll(this);
		}
	}
	...
	UnregisterSettings();
	UnregisterMessageLogListing();

	bMenusRegisteredAtModuleStartup = false;

	TimecodeManagement.Reset();
	SequencerHitchVisualizer.Reset();
}

void FTakeRecorderEditorModule::RegisterMenus()
{
...
void FTakeRecorderEditorModule::AddReferencedObjects(FReferenceCollector& Collector)
{
...
void FTakeRecorderEditorModule::RegisterDetailCustomizations()
{
	if (GIsEditor)
	{
		...
		PropertyEditorModule.RegisterCustomPropertyTypeLayout(FName(TEXT("DirectoryPath")),
			FOnGetPropertyTypeCustomizationInstance::CreateLambda(&FNamingTokenDirectoryPathStructCustomization::MakeInstance),
			MakeShared<UE::TakeRecorder::NamingTokens::FDirectoryPathPropertyTypeIdentifier>());
	}
}

void FTakeRecorderEditorModule::UnregisterDetailCustomizations()
{
	if (UObjectInitialized() && !IsEngineExitRequested())
	{
		if (FPropertyEditorModule* PropertyEditorModule = FModuleManager::Get().GetModulePtr<FPropertyEditorModule>("PropertyEditor"))
		{
			...
			PropertyEditorModule->UnregisterCustomPropertyTypeLayout(FName(TEXT("DirectoryPath")),
				MakeShared<UE::TakeRecorder::NamingTokens::FDirectoryPathPropertyTypeIdentifier>());
		}
	}
}

void FTakeRecorderEditorModule::RegisterLevelEditorExtensions()
{
	if (GIsEditor)
	{
		FLevelEditorModule& LevelEditorModule = FModuleManager::LoadModuleChecked<FLevelEditorModule>("LevelEditor");
...
void FTakeRecorderEditorModule::UnregisterLevelEditorExtensions() const
{
	if (FSlateApplication::IsInitialized())
	{
		FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(ITakeRecorderModule::TakeRecorderTabName);
		FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(ITakeRecorderModule::TakesBrowserTabName);
	}
...
void FTakeRecorderEditorModule::RegisterSettings()
{
...
void FTakeRecorderEditorModule::UnregisterSettings()
{
...
void FTakeRecorderEditorModule::RegisterMessageLogListing()
{
	FMessageLogModule& MessageLogModule = FModuleManager::LoadModuleChecked<FMessageLogModule>("MessageLog");
	FMessageLogInitializationOptions InitOptions;
	InitOptions.bShowFilters = true;
	InitOptions.bShowPages = true;
	MessageLogModule.RegisterLogListing("TakeRecorder", LOCTEXT("TakeRecorderLog", "Take Recorder"), InitOptions);
}

void FTakeRecorderEditorModule::UnregisterMessageLogListing()
{
	if (FMessageLogModule* MessageLogModule = FModuleManager::GetModulePtr<FMessageLogModule>("MessageLog"))
	{
		MessageLogModule->UnregisterLogListing("TakeRecorder");
	}
}

void FTakeRecorderEditorModule::OnEditorClose()
{
...
void FTakeRecorderEditorModule::OnRecordingInitialized(UTakeRecorder* InRecorder)
{
	// if we are already tracking a recording, do not replace the data with a new one to avoid dangling delegates.
	if (WeakActiveRecorder.IsValid())
	{
		return;
	}

	WeakActiveRecorder = InRecorder;

	// Attach to the Recorder call backs
	if (InRecorder != nullptr)
	{
		InRecorder->OnRecordingCancelled().AddRaw(this, &FTakeRecorderEditorModule::OnRecordingFinished);
		InRecorder->OnRecordingFinished().AddRaw(this, &FTakeRecorderEditorModule::OnRecordingFinished);
		ConfigureSequencerViewForRecording(InRecorder);
	}
}

void FTakeRecorderEditorModule::OnRecordingFinished(UTakeRecorder* InRecorder)
{
	// If the recorder that has finished is not the one we are tracking, return early.
	if (!WeakActiveRecorder.IsValid() || WeakActiveRecorder.Get() != InRecorder)
	{
		return;
	}
	...
	if (InRecorder != nullptr)
	{
		InRecorder->OnRecordingCancelled().RemoveAll(this);
		InRecorder->OnRecordingFinished().RemoveAll(this);
		RevertSequencerViewForRecording(InRecorder);
	}

	WeakActiveRecorder.Reset();
}

void FTakeRecorderEditorModule::ConfigureSequencerViewForRecording(UTakeRecorder* InRecorder)
{
	TSharedPtr<ISequencer> TargetSequencer = UE::TakeRecorder::Private::GetSequencerForRecorder(InRecorder);
	if (!TargetSequencer.IsValid())
	{
		return;
	}

	if (const UTakeRecorderProjectSettings* ProjectSettings = GetDefault<UTakeRecorderProjectSettings>())
	{
		bWasSequencerSimpleViewBeforeRecording = TargetSequencer->IsSimpleView();
		if (!bWasSequencerSimpleViewBeforeRecording && ProjectSettings->Settings.bRecordWithSimpleSequencerView)
		{
			TrySetSequencerSimpleView(TargetSequencer.ToSharedRef(), true);
		}
	}
}

void FTakeRecorderEditorModule::RevertSequencerViewForRecording(UTakeRecorder* InRecorder)
{
	if (bWasSequencerSimpleViewBeforeRecording)
	{
		// If the sequencer was already in simple view when we started recording, we do not need to do anything.
		return;
	}
	...
	TrySetSequencerSimpleView(TargetSequencer.ToSharedRef(), false);
}

bool FTakeRecorderEditorModule::TrySetSequencerSimpleView(TSharedRef<ISequencer> InSequencer, bool bInShowSimple)
{
	bool bViewTypeChanged = false;
	if (bInShowSimple)
	{
		// Only set the view if it is not already set to simple and it supports simple view
		if (InSequencer->GetHostCapabilities().bSupportsSimpleView && !InSequencer->IsSimpleView())
		{
			InSequencer->EnableSimpleView(true);
			bViewTypeChanged = true;
		}
	}
	else if (InSequencer->IsSimpleView())
	{
		// Only set the view if it is not already set to full.
		InSequencer->EnableSimpleView(false);
		bViewTypeChanged = true;
	}
...
void FTakeRecorderEditorModule::HandleOnPostEngineInit()
{
	if (GEngine)
	{
		if (UTakeRecorderSubsystem* TRSubSystem = GEngine->GetEngineSubsystem<UTakeRecorderSubsystem>())
		{
			TRSubSystem->GetOnRecordingInitializedEvent().AddRaw(this, &FTakeRecorderEditorModule::OnRecordingInitialized);
		}
	}

	if (GIsEditor)
	{
		// If the menus were not registered at module startup, register them now.
		if (!bMenusRegisteredAtModuleStartup)
		{
			RegisterMenus();
		}
	}
