...
void FAnimNode_LiveLinkProp::BuildPoseFromAnimData(const FLiveLinkSubjectFrameData& LiveLinkData, FPoseContext& Output)
{
	...
	if (SkeletonData->BoneNames.Num() == FrameData->Transforms.Num())
	{
		...
		for (int32 i = 0; i < SourceBoneNames.Num(); ++i)
		{
			...
			FTransform LiveLinkTransform = FrameData->Transforms[i];
			LiveLinkTransform.SetLocation(FrameData->Transforms[i].GetLocation() * GlobalScale);

			FTransform BoneTransform = OffsetTransform * LiveLinkTransform ;
