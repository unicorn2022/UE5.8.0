...
void UPCapPropComponent::SetGlobalScale(float NewGlobalScale)
{
	GlobalScale = NewGlobalScale;
	
	//If there is a Skeletal Mesh controlled by an Anim Instance pass the new global scale
	TObjectPtr<USkeletalMeshComponent> ControlledMesh = Cast<USkeletalMeshComponent>(ControlledComponent.GetComponent(GetOwner()));
	if(IsValid(ControlledMesh))
	{
		if(UPCapPropLiveLinkAnimInstance* LiveLinkAnimInstance = Cast<UPCapPropLiveLinkAnimInstance>(ControlledMesh->GetAnimInstance()))
		{
			LiveLinkAnimInstance->SetGlobalScale(GlobalScale);
		}
	}
}
...
void UPCapPropComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	FTransform GlobalScaleTransform = FTransform::Identity;
	GlobalScaleTransform.SetScale3D(FVector(GlobalScale, GlobalScale, GlobalScale));
	...
	if(!bIsControlledBySequencer)
	{
		...
		if(ControlledMesh)
		{
			...
			if(LiveLinkClient.GetSubjectRole_AnyThread(SubjectName) == ULiveLinkAnimationRole::StaticClass())
			{
				...
				{
					FinalTransform.SetComponents(CachedLiveLinkTransform.GetRotation(),(CachedLiveLinkTransform.GetLocation() * GlobalScale) + DynamicConstraintOffset, CachedLiveLinkTransform.GetScale3D());
				}
				else
				{
					FinalTransform.SetComponents(CachedLiveLinkTransform.GetRotation(),(CachedLiveLinkTransform.GetLocation() * GlobalScale), CachedLiveLinkTransform.GetScale3D());
				}
	...
	if(IsValid(ControlledMesh) && LiveLinkClient.GetSubjectRole_AnyThread(SubjectName) == ULiveLinkAnimationRole::StaticClass())
	{
		...
		if(ControlledMesh)
		{
			...
			if(AnimInstance)
			{
				...
				AnimInstance->SetGlobalScale(GlobalScale);
				AnimInstance->SetDynamicConstraintVector(FVector::ZeroVector);
				bIsDirty=false;
			}
		}
	}
