...
class UPCapWorkflowCustomization;
...
public:
	...
	/** Default Workflow UI panels to use in the Mocap Manager panel. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Config, Category="UI", DisplayName = "Default Workflows", meta = ( tooltip="This Data Asset defines the UI panels you wish to have available in Mocap Manager"))
	TSoftObjectPtr<UPCapWorkflowCustomization> MocapManagerDefaultWorkflow;

	/** 
	 * Editor Utility Widget class that will be used for each of the phase UI panels. Note this is not editable in the settings.
	 * It can only be modified in the DefaultPerformanceCaptureWorkflow.ini
	 */
	UPROPERTY(BlueprintReadOnly, Config, NoClear, Category="UI", meta = (MetaClass = "/Script/Blutility.EditorUtilityWidgetBlueprint"))
	TSoftClassPtr<UEditorUtilityWidget> WorkflowPhaseUIBaseClass;
	...
	/** Pointer to the datatable Mocap Manager will use for recording production data. */
