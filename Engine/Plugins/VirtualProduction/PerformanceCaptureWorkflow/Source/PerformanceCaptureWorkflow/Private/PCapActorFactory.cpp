	...
	if(NewActor && Asset)
	{
		if(UPCapPropDataAsset* PropDataAsset = Cast<UPCapPropDataAsset>(Asset))
		{
			...
			if(NewActor->HasAnyFlags(RF_Transient) != true && NewActor->GetClass()!= (PropDataAsset->CustomPropClass.Get())) 
			{
				...
				UE_LOGF(LogPCap, Display, "New PCap Prop Component added to %ls", *NewActor->GetActorLabel())
			}
			...
			{
				UE_LOGF(LogPCap, Display, "Prop spawned from data asset with no valid PcapProp component. %ls", *NewActor->GetActorLabel())
			}
		}
	}
}
