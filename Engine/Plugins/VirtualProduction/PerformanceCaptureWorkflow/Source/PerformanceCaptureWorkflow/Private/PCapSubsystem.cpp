...
// Bind to changes in the Timecode and  Custom Timestep providers.
	if(GEngine)
	{
		GEngine->OnCustomTimeStepChanged().AddUObject(this, &UPerformanceCaptureSubsystem::OnCustomTimestepProviderChanged);
		GEngine->OnTimecodeProviderChanged().AddUObject(this, &UPerformanceCaptureSubsystem::OnTimecodeProviderChanged);
	}
}
...
void UPerformanceCaptureSubsystem::OnTimecodeProviderChanged() const
{
	OnPCapTimecodeProviderChanged.Broadcast();
}

void UPerformanceCaptureSubsystem::OnCustomTimestepProviderChanged() const
{
	OnPCapCustomTimestepProviderChanged.Broadcast();
}
