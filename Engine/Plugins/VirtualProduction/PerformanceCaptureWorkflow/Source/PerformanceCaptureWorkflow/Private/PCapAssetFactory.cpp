...
{
	UPCapDataTable* NewTable = NewObject<UPCapDataTable>(InParent, Name, Flags);

	if (NewTable && FEngineAnalytics::IsAvailable())
	{
		TArray<FAnalyticsEventAttribute> Attributes;
		Attributes.Reserve(3);
		Attributes.Emplace(TEXT("Action"), TEXT("AssetCreated"));
		Attributes.Emplace(TEXT("AssetClass"), UPCapDataTable::StaticClass()->GetName());
		Attributes.Emplace(TEXT("RowStruct"), Struct ? Struct->GetName() : TEXT("None"));
...
UObject* UPCap_DataAssetFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	if (NewDataAsset && FEngineAnalytics::IsAvailable())
	{
		const UClass* CreatedClass = DataAssetClass ? DataAssetClass.Get() : NewDataAsset->GetClass();
		TArray<FAnalyticsEventAttribute> Attributes;
		Attributes.Reserve(2);
		Attributes.Emplace(TEXT("Action"), TEXT("AssetCreated"));
		Attributes.Emplace(TEXT("AssetClass"), CreatedClass->GetName());
		FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.PerformanceCapture"), Attributes);
	}
