...
public:
	...
	*/
	...
	/**
	* Multicast delegate that will execute when the engine's Timecode provider is updated.
	*/
	DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnPCapTimecodeProviderChanged);
	/** Editor Only*/
	UPROPERTY(BlueprintAssignable, Category="Performance Capture|LiveLink")
	FOnPCapTimecodeProviderChanged OnPCapTimecodeProviderChanged;
	
	/**
	* Multicast delegate that will execute when the engine's Timestep provider is updated.
	*/
	DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnPCapCustomTimestepProviderChanged);
	/** Editor Only*/
	UPROPERTY(BlueprintAssignable, Category="Performance Capture|LiveLink")
	FOnPCapCustomTimestepProviderChanged OnPCapCustomTimestepProviderChanged;
...
private:
	...
	void OnTimecodeProviderChanged() const;
	
	void OnCustomTimestepProviderChanged() const;
};
