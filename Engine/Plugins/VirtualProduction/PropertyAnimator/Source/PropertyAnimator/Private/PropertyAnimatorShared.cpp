...
float Easing::Bounce(float InProgress, EPropertyAnimatorEasingType InType)
{
	...
	switch (InType)
	{
		...
		case EPropertyAnimatorEasingType::Out:
			...
			{
				InProgress -= 1.5f / D1;
				return N1 * InProgress * InProgress + 0.75f;
			}
			else if (InProgress < 2.5f / D1)
			{
				InProgress -= 2.25f / D1;
				return N1 * InProgress * InProgress + 0.9375f;
			}
			else
			{
				InProgress -= 2.625f / D1;
				return N1 * InProgress * InProgress + 0.984375f;
			}
...
{
	return InAmplitude * (2 * FMath::Wrap(InTime + InOffset, 0.0, 1.0f / InFrequency) * InFrequency - 1);
}
...
double Wave::Triangle(double InTime, double InAmplitude, double InFrequency, double InOffset)
{
	...
	const double NormalizedTime = FMath::Wrap(InTime + InOffset, 0.0, Period) / Period;
...
double Wave::Pulse(double InTime, double InAmplitude, double InFrequency, double InOffset)
{
	...
	const double NormalizedTime = FMath::Wrap(InTime + InOffset, 0.0, Period);
