...
void UPropertyAnimatorNumericBase::EvaluateProperties(FInstancedPropertyBag& InParameters)
{
		...
		, int32 InRangeMax)->bool
	{
		...
		double Frequency = CycleDuration != 0 ? 1.0 / static_cast<double>(CycleDuration) : 0.0;
		...
		else
		{
		    ...
		    if (PropertyTimeElapsed < 0.0)
		    {
		        ...
		        PropertyTimeElapsed = FMath::Wrap(PropertyTimeElapsed, 0.0, Period);
		        if (PropertyTimeElapsed < 0.0)
		        {
		    ...
		    if (CycleMode == EPropertyAnimatorCycleMode::Loop)
		    {
		        ...
		        const double TimeInPeriod = FMath::Wrap(PropertyTimeElapsed, 0.0, Period);
