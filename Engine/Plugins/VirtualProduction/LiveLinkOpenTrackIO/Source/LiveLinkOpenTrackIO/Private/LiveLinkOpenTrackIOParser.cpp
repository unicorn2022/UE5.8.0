...
void FLiveLinkOpenTrackIOPayload::AddBytes(uint32 InOffset, TArrayView<const uint8> InBytes)
{
	...
	{
		bValidPayload = false;
		UE_LOGF(LogLiveLinkOpenTrackIO, Error, "Payload size limit exceeded %f MB. Use OpenTrackIO.PayloadSizeLimit to adjust this limit.", PayloadSizeLimitInMb);
		return;
	}
...
static bool IsHeaderValid(TArrayView<const uint8> Bytes)
{
	...
	/** Check the header for validity first before trying to read the rest of the packet. */
	{
		...
		{
			UE_LOGF(LogLiveLinkOpenTrackIO, Error, "Invalid OpenTrackIO header. It was %08X instead of %08X",
...
static TOptional<FLiveLinkOpenTrackIOData> ConvertPayloadToData(const FLiveLinkOpenTrackIODatagramHeader& Header, TArrayView<const uint8> Payload)
{
	...
	{
		UE_LOGF(LogLiveLinkOpenTrackIO, Error, "Unsupported encoding.");
	}
...
static TOptional<FLiveLinkOpenTrackIOData> TryEmitOpenTrackData(FLiveLinkOpenTrackIODatagramHeader Header, TArrayView<const uint8> Payload)
{
	...
	else if (Data)
	{
		...
		//
		Data.Reset();
		UE_LOGF(LogLiveLinkOpenTrackIO, Warning, "Unsupported OpenTrackIO protocol.");
	}
...
bool GetHeaderAndPayloadFromBytes(TArrayView<const uint8> Bytes, FOpenTrackIOHeaderWithPayload& OutPayloadContainer)
{
	...
	{
		UE_LOGF(LogLiveLinkOpenTrackIO, Error, "Invalid OpenTrackIO Header Received.");
		return false;		
	}
	...
	if (PayloadSize > Bytes.Num())
	{
		...
		UE_LOGF(LogLiveLinkOpenTrackIO, Error, "Payload size does not match provided OpenTrackIO packet header.");
		return false;
	}
	...
	{
		UE_LOGF(LogLiveLinkOpenTrackIO, Warning, "Failed to verify packet checksum.");
		return false;
	}
	...
	{
		UE_LOGF(LogLiveLinkOpenTrackIO, Warning, "Invalid sequence number for segmented payload. Expected %d but received %d.", PrevSequenceNumber, Header.SequenceNumber);
		return false;
	}
...
TOptional<FLiveLinkOpenTrackIOData> ParsePayload(const FOpenTrackIOHeaderWithPayload& HeaderAndPayload)
{
	...
	{
		UE_LOGF(LogLiveLinkOpenTrackIO, Warning, "Discarding a partial payload because of missing segments.");
		return {};
	}
