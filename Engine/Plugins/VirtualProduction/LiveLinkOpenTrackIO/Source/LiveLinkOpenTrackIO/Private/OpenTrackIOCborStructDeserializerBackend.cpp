...
namespace UE::OpenTrackIOCborStructDeserializerBackend::Private
{
...
bool FOpenTrackIOCborStructDeserializerBackend::GetNextToken(EStructDeserializerBackendTokens& OutToken)
{
	...
	if (CborReader.GetContext().MajorType() == ECborCode::Map && (CborReader.GetContext().AsLength() & 1))
	{
		...
		else
		{
			...
			FString CborMajorTypeAsString = UE::OpenTrackIOCborStructDeserializerBackend::Private::ConvertCborTypeToString(LastContext.MajorType());
...
bool FOpenTrackIOCborStructDeserializerBackend::ReadProperty(FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex)
{
	...
	case ECborCode::TextString:
	{
		...
		UE_LOGF(LogLiveLinkOpenTrackIO, Verbose, "String field %ls with value '%ls' is not supported in FProperty type %ls (%ls)", *Property->GetFName().ToString(), *StringValue, *Property->GetClass()->GetName(), *GetDebugString());
	...
	case ECborCode::ByteString:
	{
		...
		UE_LOGF(LogLiveLinkOpenTrackIO, Verbose, "Error while deserializing field %ls. Unexpected UProperty type %ls. Expected a UByteProperty/UInt8Property to deserialize a TArray<uint8>/TArray<int8>", *Property->GetFName().ToString(), *Property->GetClass()->GetName());
		return false;
	}
	...
	case ECborCode::Prim:
	{
		...
		case ECborCode::False:
		{
			...
			UE_LOGF(LogLiveLinkOpenTrackIO, Verbose, "Boolean field %ls with value '%ls' is not supported in FProperty type %ls (%ls)", *Property->GetFName().ToString(), LastContext.AsBool() ? *(CoreTexts.True.ToString()) : *(CoreTexts.False.ToString()), *Property->GetClass()->GetName(), *GetDebugString());
			return false;
		}
		...
		case ECborCode::Value_4Bytes:
		{
			...
			if (!bWasSet)
			{
				UE_LOGF(LogLiveLinkOpenTrackIO, Verbose, "Float field %ls with value '%f' is not supported in FProperty type %ls (%ls)",
		...
		case ECborCode::Value_8Bytes:
		{
			...
			if (!bWasSet)
			{
				UE_LOGF(LogLiveLinkOpenTrackIO, Verbose, "Double field %ls with value '%f' is not supported in FProperty type %ls (%ls)",
					*Property->GetFName().ToString(), LastContext.AsFloat(), *Property->GetClass()->GetName(), *GetDebugString());
			}
		...
		default:
			UE_LOGF(LogLiveLinkOpenTrackIO, Verbose, "Unsupported primitive type for %ls with value '%f' in FProperty type %ls (%ls)", *Property->GetFName().ToString(), LastContext.AsDouble(), *Property->GetClass()->GetName(), *GetDebugString());
			return false;
		}
	}
	}
