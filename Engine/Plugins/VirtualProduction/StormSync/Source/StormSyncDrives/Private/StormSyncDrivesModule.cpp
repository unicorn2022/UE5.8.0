...
void FStormSyncDrivesModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FStormSyncDrivesModule::OnPostEngineInit);
...
{
	UE_LOGF(LogStormSyncDrives, Display, "FStormSyncDrivesModule::RegisterMountPoint ... MountPoint: %ls, Path: %ls", *InMountPoint.MountPoint, *InMountPoint.MountDirectory.Path);
	...
	UE_LOGF(LogStormSyncDrives, Display, "FStormSyncDrivesModule::RegisterMountPoint ... Try register %ls to %ls", *MountPoint, *InMountPoint.MountDirectory.Path);
...
{
	UE_LOGF(LogStormSyncDrives, Display, "FStormSyncDrivesModule::OnPostEngineInit ...");
	UE_LOGF(LogStormSyncDrives, Display, "\t Mounting Drives based on config");
...
bool FStormSyncDrivesModule::MountDriveHelper(const FStormSyncMountPointConfig& InMountPoint)
{
	if (CachedMountPoints.Contains(InMountPoint.MountPoint))
	{
		...
		{
			UE_LOGF(LogStormSyncDrives, Error, "FStormSyncDrivesModule::UnregisterMountPoint failed with Error: %ls", *ErrorText.ToString());
			return false;
		}
	}
	...
	{
		UE_LOGF(LogStormSyncDrives, Error, "FStormSyncDrivesModule::RegisterMountPoint failed with Error: %ls", *ErrorText.ToString());
		return false;
	}
...
bool FStormSyncDrivesModule::RemoveFromCacheIfNotInConfigs(const TArray<FStormSyncMountPointConfig>& Configs)
{
	...
	for (const FStormSyncMountPointConfig& Config : UnregisterList)
	{
		...
		{
			UE_LOGF(LogStormSyncDrives, Error, "FStormSyncDrivesModule::RemoveFromCacheIfNotInConfigs failed with Error: %ls", *ErrorText.ToString());
			bSuccess = false;
		}
	}
...
{
	UE_LOGF(LogStormSyncDrives, Display, "FStormSyncDrivesModule::ResetMountedDrives ...");
	check(InSettings);
	...
	UE_CLOGF(Filtered.Num() != Configs.Num(), LogStormSyncDrives, Warning, "FStormSyncDrivesModule::ResetMountedDrives settings object contains duplicate values.");
...
{
	UE_LOGF(LogStormSyncDrives, Display, "FStormSyncDrivesModule::UnregisterMountedDrives ...");
	for (const TPair<FString, FStormSyncMountPointConfig>& Cache : CachedMountPoints)
	{
		FText ErrorText;
		if (!UnregisterMountPoint(Cache.Value, ErrorText))
		{
			UE_LOGF(LogStormSyncDrives, Error, "FStormSyncDrivesModule::UnregisterMountedDrives failed with Error: %ls", *ErrorText.ToString());
		}
	}
}
