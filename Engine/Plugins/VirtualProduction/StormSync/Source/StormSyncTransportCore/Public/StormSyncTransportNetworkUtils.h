...
public:
	...
	/**
	 * Returns the list of local adapter addresses as returned by the socket subsystem, with current port from server
	 * endpoint appended to the address list.
	 */
	static TArray<FString> GetLocalAdapterAddresses();

	/**
	 * Returns the current tcp server endpoint address (ip:port).
	 * @remark This is potentially different from the configured endpoint address in case of port collisions, it may be on a different one.
	 */
