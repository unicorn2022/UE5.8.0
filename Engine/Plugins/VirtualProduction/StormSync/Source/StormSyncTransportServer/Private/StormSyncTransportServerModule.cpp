...
void FStormSyncTransportServerModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FStormSyncTransportServerModule::OnPostEngineInit);
...
void FStormSyncTransportServerModule::StartServerEndpoint(const FString& InEndpointFriendlyName)
{
	...
	if (!ServerEndpoint.IsValid())
	{
		...
		if (!ServerEndpoint.IsValid())
		{
			UE_LOGF(LogStormSyncServer, Error, "FStormSyncTransportServerModule::StartServerEndpoint - Failed to create Server Local Endpoint");
			return;
		}
	}
	...
	if (IsRunning())
	{
		UE_LOGF(LogStormSyncServer, Warning, "FStormSyncTransportServerModule::StartServerEndpoint - Server endpoint TCP listener already running");
		return;
	}
...
void FStormSyncTransportServerModule::RegisterConsoleCommands()
{
	...
	ConsoleCommands.Add(IConsoleManager::Get().RegisterConsoleCommand(
		...
		FConsoleCommandWithArgsDelegate::CreateLambda([this](const TArray<FString>& Args)
		{
			...
			UE_LOGF(LogStormSyncServer, Display, "StormSync.Server.Debug - EndpointId: %ls", *AddressId);
		}),
		ECVF_Default
	));
...
{
	UE_LOGF(LogStormSyncServer, Verbose, "FStormSyncTransportServerModule::OnPostEngineInit - Publish ping messages for discover manager and server endpoint ...");
...
void FStormSyncTransportServerModule::ExecuteStopServer(const TArray<FString>& InArgs)
{
	...
	{
		UE_LOGF(LogStormSyncServer, Warning, "FStormSyncTransportServerModule::ExecuteStopServer - Server endpoint already inactive");
	}
}
...
void FStormSyncTransportServerModule::ExecuteServerStatus(const TArray<FString>& InArgs) const
{
	...
	UE_LOGF(LogStormSyncServer, Display, "FStormSyncTransportServerModule::ExecuteServerStatus - %ls", *StatusText.ToString());
}
