...
uint32 FStormSyncTransportTcpServer::Run()
{
	...
	while (bListenerIsRunning && !bStopping)
	{
		...
		if (DeltaTime > 0.1)
		{
			UE_LOGF(LogStormSyncServer, Warning, "Hitch detected; %.3f seconds since prior tick", DeltaTime);
		}
...
bool FStormSyncTransportTcpServer::Tick()
{
	...
	for (const TPair<FIPv4Endpoint, TUniquePtr<FClientConnection>>& Connection : ClientConnections)
	{
		...
		while (ClientSocket->HasPendingData(PendingDataSize))
		{
			...
			{
				UE_LOGF(LogStormSyncServer, Error, "Error while receiving data via endpoint %ls", *ClientEndpoint.ToString());
				continue;
			}
			...
			for (int32 i = 0; i < BytesRead; ++i)
			{
				...
				if (!bHasReceivedSize && i == 3)
				{
					...
					UE_LOGF(LogStormSyncServer, Verbose, "Received size: %d. Setting it for %ls", Size, *ClientEndpoint.ToString());
...
bool FStormSyncTransportTcpServer::StartListening()
{
	...
	if (RawSocket)
	{
		...
		if (SocketListener->IsActive())
		{
			...
			UE_LOGF(LogStormSyncServer, Display, "Started listening on %ls", *SocketListener->GetLocalEndpoint().ToString());
			return true;
		}
	}

	Socket.Reset();
	UE_LOGF(LogStormSyncServer, Error, "Could not create Tcp Listener!");
	return false;
}
...
void FStormSyncTransportTcpServer::StopListening()
{
	...
	{
		UE_LOGF(LogStormSyncServer, Display, "No longer listening on %ls", *SocketListener->GetLocalEndpoint().ToString());
...
{
	UE_LOGF(LogStormSyncServer, Display, "Incoming connection via %ls:%d", *InEndpoint.Address.ToString(), InEndpoint.Port);
...
void FStormSyncTransportTcpServer::HandleIncomingBuffer(const FIPv4Endpoint& InEndpoint, const TSharedPtr<FSocket>& InClientSocket, const TArray<uint8>& InBytes)
{
	...
	UE_LOGF(LogStormSyncServer, Verbose, "\tHandle Incoming buffer via %ls:%d (Total Buffer Size: %d)", *InEndpoint.Address.ToString(), InEndpoint.Port, ReceivedBufferSize);
	...
	if (ClientConnections.Contains(InEndpoint))
	{
		...
		{
			UE_LOGF(LogStormSyncServer, Display, "\tWe received full buffer!!");
...
{
	UE_LOGF(LogStormSyncServer, Display, "Handle received buffer via %ls (Buffer Size: %d)", *InEndpoint.ToString(), InBytes.Num());
...
void FStormSyncTransportTcpServer::CleanUpDisconnectedSockets()
{
	...
	for (const TTuple<FIPv4Endpoint, TUniquePtr<FClientConnection>>& Connection : ClientConnections)
	{
		...
		{
			UE_LOGF(LogStormSyncServer, Warning, "Client %ls has been inactive for more than %.1fs -- closing connection", *RemoteEndpoint.ToString(), ClientTimeout);
...
{
	UE_LOGF(LogStormSyncServer, Display, "Client %ls disconnected", *InClientEndpoint.ToString());
...
bool FStormSyncTransportTcpServer::SendMessage(const FString& InMessage, const FIPv4Endpoint& InEndpoint)
{
	if (ClientConnections.Contains(InEndpoint))
	{
		...
		UE_LOGF(LogStormSyncServer, Verbose, "Sending message to %ls - Message: %ls", *InEndpoint.ToString(), *InMessage);
		int32 BytesSent = 0;
		return ClientSocket->Send(reinterpret_cast<uint8*>(TCHAR_TO_UTF8(*InMessage)), InMessage.Len() + 1, BytesSent);
	}

	UE_LOGF(LogStormSyncServer, Verbose, "Trying to send message to disconnected client %ls - Message: %ls", *InEndpoint.ToString(), *InMessage);
	return false;
}
