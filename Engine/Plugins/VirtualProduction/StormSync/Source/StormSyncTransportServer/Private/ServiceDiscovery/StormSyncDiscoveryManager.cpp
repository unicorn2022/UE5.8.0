	...
	, bEnableDiscoveryPeriodicPublish(bInEnableDiscoveryPeriodicPublish)
{
	...
	if (bRunning)
	{
		UE_LOGF(LogStormSyncServer, Display, "FStormSyncDiscoveryManager::FStormSyncDiscoveryManager - Subscribe to messages");
		MessageEndpoint->Subscribe<FStormSyncTransportConnectMessage>();

		UE_LOGF(LogStormSyncServer, Display, "FStormSyncDiscoveryManager::FStormSyncDiscoveryManager - Start Thread");
...
uint32 FStormSyncDiscoveryManager::Run()
{
	while (bRunning)
	{
		...
		TArray<FStormSyncDelegateItem> DelegateQueue;
		{
			...
			{
				UE_LOGF(LogStormSyncServer, Display, "FStormSyncDiscoveryManager::Run - %ls became invalid", *MessageAddress.ToString());
...
void FStormSyncDiscoveryManager::PublishConnectMessage()
{
	...
	{
		UE_LOGF(LogStormSyncServer, Error, "FStormSyncDiscoveryManager::PublishConnectMessage - Unable to send Connect Message cause Message Endpoint is invalid");
		return;
	}

	UE_LOGF(LogStormSyncServer, VeryVerbose, "FStormSyncTransportClientModule::PublishConnectMessage - Publish Connect Message ...");
	...
	if (Message.IsValid())
	{
		...
		LastPublishTime = FPlatformTime::Seconds();
...
void FStormSyncDiscoveryManager::SendWakeUp()
{
	if (!MessageEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncServer, Error, "FStormSyncDiscoveryManager::SendWakeUp - Unable to send Connect Message cause Message Endpoint is invalid");
		return;
	}

	UE_LOGF(LogStormSyncServer, VeryVerbose, "FStormSyncTransportClientModule::SendWakeUp - Sending Wakeup Request to all connections.");
...
void FStormSyncDiscoveryManager::HandleConnectMessage(const FStormSyncTransportConnectMessage& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& MessageContext)
{
	...
	UE_LOGF(LogStormSyncServer, VeryVerbose, "FStormSyncDiscoveryManager::HandleConnectMessage - Received connect Message from %ls: %ls", *SenderMessageAddress.ToString(), *InMessage.ToString());

	const FMessageAddress MessageEndpointAddress = MessageEndpoint->GetAddress();

	UE_LOGF(LogStormSyncServer, VeryVerbose, "FStormSyncDiscoveryManager::HandleConnectMessage - SenderMessageAddress: %ls, MessageEndpointAddress: %ls (Same: %ls)", *SenderMessageAddress.ToString(), *MessageEndpointAddress.ToString(), SenderMessageAddress == MessageEndpointAddress ? TEXT("true") : TEXT("false"));
...
void FStormSyncDiscoveryManager::RegisterConnection(const FMessageAddress& InMessageAddress, bool bInIsServerRunning)
{
	...
	UE_LOGF(LogStormSyncServer, Display, "FStormSyncDiscoveryManager::HandleConnectMessage - Send Connect Message to %ls...", *InMessageAddress.ToString());
	TUniquePtr<FStormSyncTransportConnectMessage> Message(FMessageEndpoint::MakeMessage<FStormSyncTransportConnectMessage>());
	if (Message.IsValid())
	{
		Message->StormSyncServerAddressId = FStormSyncTransportNetworkUtils::GetServerEndpointMessageAddress();
...
void FStormSyncDiscoveryManager::HandleHeartbeatMessage(const FStormSyncTransportHeartbeatMessage& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& MessageContext)
{
	...
	UE_LOGF(LogStormSyncServer, VeryVerbose, "FStormSyncDiscoveryManager::HandleHeartbeatMessage - Received Heartbeat Message from %ls", *MessageAddress.ToString());
