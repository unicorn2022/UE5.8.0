...
TFuture<TArray<FStormSyncFileDependency>> FStormSyncCoreUtils::GetAvaFileDependenciesAsync(const TArray<FName>& InPackageNames, const bool bInShouldValidatePackages, EAsyncExecution InThreadType)
{
	...
	return Async(InThreadType, [PackageNames = MoveTemp(LocalPackageNames), bInShouldValidatePackages]()
	{
		...
		{
			UE_LOGF(LogStormSyncCore, Error, "FStormSyncCoreUtils::GetAvaFileDependenciesAsync - Error: %ls", *ErrorText.ToString());
			return FileDependencies;
		}
...
bool FStormSyncCoreUtils::CreatePakBuffer(const TArray<FName>& InPackageNames, FArchive& OutPakArchive, FText& OutErrorText, const FOnFileAdded& InOnFileAdded)
{
	...
	UE_LOGF(LogStormSyncCore, Display, "FStormSyncCoreUtils::CreatePakBuffer - Creating Pak file for %d files.", PackageNames.Num());
	
	for (const FName& PackageName : PackageNames)
	{
		const FString PackageNameStr = PackageName.ToString();
		UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils::CreatePakBuffer - Handle `%ls` file to add.", *PackageNameStr);
	...
	UE_LOGF(LogStormSyncCore, Display, "Added %d files to %ls, %lld bytes total, time %.2lfs.", FileCount, *OutPakArchive.GetArchiveName(), OutPakArchive.Tell() - StartOffset, FPlatformTime::Seconds() - StartTime);
...
bool FStormSyncCoreUtils::ExtractPakBuffer(FArchive& InPakArchive, const FStormSyncCoreExtractArgs& InExtractArgs, TArray<FText>& OutErrors)
{
	...
	{
		UE_LOGF(LogStormSyncCore, Error, "FStormSyncCoreUtils:ExtractPakBuffer Cannot Extract package from non-reader archive \"%ls\"", *InPakArchive.GetArchiveName());
		return false;
	}
	
	UE_LOGF(LogStormSyncCore, Display, "FStormSyncCoreUtils:ExtractPakBuffer Extracting package from archive of size: %lld", InPakArchive.TotalSize());
	...
	UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils:ExtractPakBuffer FileCount: %d", FileCount);
...
{
	UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils::GetSyncFileModifiers - InPackageNames: %d, InRemoteDependencies: %d", InPackageNames.Num(), InRemoteDependencies.Num());
	...
	for (const FStormSyncFileDependency& LocalDependency : LocalDependencies)
	{
		...
		if (!MatchingDependency)
		{
			...
			UE_LOGF(LogStormSyncCore, Verbose, "\tAdding Modifier: %ls", *Modifier.ToString());
	...
	for (const FStormSyncFileDependency& RemoteDependency : InRemoteDependencies)
	{
		...
		{
			UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils::GetSyncFileModifiers - Handle %ls", *RemoteDependency.PackageName.ToString());
			UE_LOGF(LogStormSyncCore, Verbose, "\tLocal: %ls", *MatchingDependency->ToString());
			UE_LOGF(LogStormSyncCore, Verbose, "\tRemote: %ls", *RemoteDependency.ToString());
			UE_LOGF(LogStormSyncCore, Verbose, "\tDirty because of mismatch filesize %lld vs %lld", MatchingDependency->FileSize, RemoteDependency.FileSize);
			bIsDirty = true;
		}
		...
		else if (MatchingDependency->FileHash != RemoteDependency.FileHash)
		{
			...
			UE_LOGF(LogStormSyncCore, Verbose, "\tLocal: %ls", *MatchingDependency->ToString());
			UE_LOGF(LogStormSyncCore, Verbose, "\tRemote: %ls", *RemoteDependency.ToString());
			UE_LOGF(LogStormSyncCore, Verbose, "\tDirty because of mismatch file hash %ls vs %ls", *MatchingDependency->FileHash, *RemoteDependency.FileHash);
			bIsDirty = true;
		}
...
void FStormSyncCoreUtils::RecursiveGetDependencies(const FName& InPackageName, TArray<FName>& OutAllDependencies)
{
	...
	{
		UE_LOGF(LogStormSyncCore, Warning, "FStormSyncCoreUtils::RecursiveGetDependencies - GetAssetData failed to load assets for %ls", *InPackageName.ToString());
		return;
	}
	...
	for (const FName& Dependency : Dependencies)
	{
		...
		UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils::RecursiveGetDependencies - Gather dependencies for %ls", *DependencyName);
...
{
	UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils::IsValidDependency - Check dependency (%ls)", *InDependencyName);
	...
	{
		UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils::IsValidDependency - Filter out dependency \"%ls\". Ignored by bExportOnlyGameContent setting", *InDependencyName);
		return false;
	}
	...
	for (const FName IgnoredPackageName : IgnoredPackages)
	{
		...
		{
			UE_LOGF(LogStormSyncCore, Verbose, "FStormSyncCoreUtils::IsValidDependency - Filter out dependency \"%ls\". Ignored by \"%ls\" IgnoredPackages setting", *InDependencyName, *IgnoredPackageName.ToString());
			return false;
		}
	}
