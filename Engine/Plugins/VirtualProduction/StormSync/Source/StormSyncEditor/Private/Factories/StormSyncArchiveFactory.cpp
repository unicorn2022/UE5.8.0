...
{
	UE_LOGF(LogStormSyncEditor, Display, "UStormSyncArchiveFactory::ImportObject InClass: %ls, InOuter: %ls, InName: %ls, Filename: %ls", *GetNameSafe(InClass), *GetNameSafe(InOuter), *InName.ToString(), *Filename);
	...
	if (!Filename.IsEmpty())
	{
		...
		{
			UE_LOGF(LogStormSyncEditor, Error, "Can't find file '%ls' for import", *Filename);
		}
		else
		{
			UE_LOGF(LogStormSyncEditor, Display, "FactoryCreateFile: %ls with %ls (%i %i %ls)", *InClass->GetName(), *GetClass()->GetName(), bCreateNew, bText, *Filename);
...
{
	UE_LOGF(LogStormSyncEditor, Display, "UStormSyncArchiveFactory::FactoryCreateFile InClass: %ls, InOuter: %ls, InName: %ls, Filename: %ls", *GetNameSafe(InClass), *GetNameSafe(InParent), *InName.ToString(), *Filename);
	...
	UE_LOGF(LogStormSyncEditor, Display, "UStormSyncArchiveFactory::FactoryCreateFile for %ls", *GetNameSafe(ArchiveData));
