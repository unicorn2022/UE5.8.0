...
{
	FScopeLock ConnectionsLock(&ConnectionsCriticalSection);
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::ExecuteDumpConnections - Listing active connections (%d)", RegisteredConnections.Num());
	...
	for (const TPair<FMessageAddress, FStormSyncConnectedDevice>& RegisteredConnection : RegisteredConnections)
	{
		...
		UE_LOGF(LogStormSyncEditor, Display, "%ls (%ls) %ls", *Connection.StormSyncServerAddressId, *Connection.HostName, *ConnectionDescription);
	}
}
...
void FStormSyncEditorModule::ExecuteRequestStatusCommand(const TArray<FString>& Args) const
{
	...
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::ExecuteStatusCommand - Argv: %ls", *Argv);
	...
	{
		UE_LOGF(LogStormSyncEditor, Error, "FStormSyncEditorModule::ExecuteStatusCommand - Missing at least one package name to request status.");
		return;
	}
	...
	{
		UE_LOGF(LogStormSyncEditor, Error, "FStormSyncEditorModule::ExecuteStatusCommand - Missing -remote parameter");
		return;
	}
	...
	{
		UE_LOGF(LogStormSyncEditor, Error, "FStormSyncEditorModule::ExecuteStatusCommand - Unable to parse %ls into a Message Address", *RemoteAddressId);
		return;
	}
	...
	{
		UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::ExecuteStatusCommand - Received response! %ls", *Response->ToString());
		SStormSyncStatusWidget::OpenDialog(Response.ToSharedRef());
	});
	
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::ExecuteStatusCommand - Send status request on %ls", *RemoteAddress.ToString());
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::OnServiceDiscoveryConnection - MessageAddressUID: %ls, Hostname: %ls, ProjectName: %ls", *MessageAddressUID, *ConnectedDevice.HostName, *ConnectedDevice.ProjectName);
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::OnServiceDiscoveryStateChange - MessageAddressUID: %ls", *MessageAddressUID);
	...
	if (FMessageAddress::Parse(MessageAddressUID, RemoteMessageAddress) && RegisteredConnections.Contains(RemoteMessageAddress))
	{
		UE_LOGF(LogStormSyncEditor, Display, "\tRemote State Changed to %ls", *UEnum::GetValueAsString(State));
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::OnServiceDiscoveryServerStatusChange - MessageAddressUID: %ls", *MessageAddressUID);
	...
	if (FMessageAddress::Parse(MessageAddressUID, RemoteMessageAddress) && RegisteredConnections.Contains(RemoteMessageAddress))
	{
		UE_LOGF(LogStormSyncEditor, Display, "\tRemote Server Endpoint State Changed to %ls", bIsServerRunning ? TEXT("running") : TEXT("stopped"));
		FStormSyncConnectedDevice& Connection = RegisteredConnections.FindChecked(RemoteMessageAddress);
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncEditorModule::OnServiceDiscoveryDisconnection - MessageAddressUID: %ls", *MessageAddressUID);
