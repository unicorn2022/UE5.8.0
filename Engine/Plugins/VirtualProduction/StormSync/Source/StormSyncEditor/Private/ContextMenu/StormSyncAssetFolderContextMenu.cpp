...
{
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::Initialize - ...");
...
{
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::Shutdown - ...");
}
...
TArray<FAssetData> FStormSyncAssetFolderContextMenu::GetDirtyAssets(const TArray<FName>& InPackageNames)
{
	...
	for (const FName& PackageName : InPackageNames)
	{
		...
		{
			UE_LOGF(LogStormSyncEditor, Warning, "FStormSyncAssetFolderContextMenu::HasDirtyAssets - GetAssetsByPackageName failed to load assets for %ls", *PackageName.ToString());
			continue;
		}
...
{
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::RegisterMenus");
...
{
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::PopulateAssetFolderContextMenu - InMenu: %ls", *GetNameSafe(InMenu));
	check(InMenu);
	...
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::PopulateAssetFolderContextMenu - Result ...");
	UE_LOGF(LogStormSyncEditor, Verbose, "\tSelectedPackagePaths: %d", SelectedPackagePaths.Num());
	for (const FString& SelectedPackagePath : SelectedPackagePaths)
	{
		UE_LOGF(LogStormSyncEditor, Verbose, "\t- %ls", *SelectedPackagePath);
	}

	UE_LOGF(LogStormSyncEditor, Verbose, "\tSelectedAssetPackages: %d", SelectedAssetPackages.Num());
	for (const FString& SelectedAssetPackage : SelectedAssetPackages)
	{
		UE_LOGF(LogStormSyncEditor, Verbose, "\t- %ls", *SelectedAssetPackage);
	}
...
{
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::PopulateAssetFileContextMenu - InMenu: %ls", *GetNameSafe(InMenu));
	check(InMenu);
	...
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::PopulateAssetFileContextMenu - Result ...");
	UE_LOGF(LogStormSyncEditor, Verbose, "\tSelectedAssetPackages: %d", SelectedAssetPackages.Num());
	for (const FName& SelectedPackageName : SelectedAssetPackages)
	{
		UE_LOGF(LogStormSyncEditor, Verbose, "\t- %ls", *SelectedPackageName.ToString());
	}
...
{
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::GetSelectedFilesAndFolders - InMenu: %ls", *GetNameSafe(InMenu));
	check(InMenu);
...
{
	UE_LOGF(LogStormSyncEditor, Verbose, "FStormSyncAssetFolderContextMenu::GetSelectedFiles - InMenu: %ls", *GetNameSafe(InMenu));
	check(InMenu);
...
void FStormSyncAssetFolderContextMenu::BuildPushAssetsMenuEntries(FMenuBuilder& InMenuBuilder, TArray<FName> InPackageNames, const bool bInIsPushing) const
{
	...
	for (const TPair<FMessageAddress, FStormSyncConnectedDevice>& RegisteredConnection : RegisteredConnections)
	{
		...
		{
			UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::BuildPushAssetsMenuEntries - Error parsing message address for storm sync client with id \"%ls\"", *ClientAddressId);
			continue;
		}
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecuteSyncAssetsAction - Package names %d", InPackageNames.Num());
	for (FName PackageName : InPackageNames)
	{
		UE_LOGF(LogStormSyncEditor, Display, "\t- Package name: %ls", *PackageName.ToString());
	}
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecutePushAssetsAction to %ls - Package names %d", *InMessageAddressId, InPackageNames.Num());

	FMessageAddress RemoteMessageAddress;
	if (!FMessageAddress::Parse(InMessageAddressId, RemoteMessageAddress))
	{
		UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::ExecutePushAssetsAction to %ls - Unable to parse into a Message Address", *InMessageAddressId);
		return;
	}
	...
	const FOnStormSyncPushComplete Delegate = FOnStormSyncPushComplete::CreateLambda([](const TSharedPtr<FStormSyncTransportPushResponse>& Response)
	{
		...
		UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecutePushAssetsAction - Got a response: %ls", *Response->ToString());
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecutePullAssetsAction to %ls - Package names %d", *InMessageAddressId, InPackageNames.Num());

	FMessageAddress RemoteMessageAddress;
	if (!FMessageAddress::Parse(InMessageAddressId, RemoteMessageAddress))
	{
		UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::ExecutePullAssetsAction to %ls - Unable to parse into a Message Address", *InMessageAddressId);
		return;
	}
	...
	{
		check(Response.IsValid())
		UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecutePullAssetsAction - Got a response: %ls", *Response->ToString());
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecuteCompareWithAction to %ls - Package names %d", *InMessageAddressId, InPackageNames.Num());

	FMessageAddress RemoteAddress;
	if (!FMessageAddress::Parse(InMessageAddressId, RemoteAddress))
	{
		UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::ExecuteCompareWithAction to %ls - Unable to parse into a Message Address", *InMessageAddressId);
		return;
	}
	...
	{
		UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecuteCompareWithAction - Received response! %ls", *Response->ToString());
...
{
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::ExecuteExportAction - Package names %d", InPackageNames.Num());
	...
	TArray<FName> AllPackageNamesToMove;
	{
		...
		{
			UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::ExecuteExportAction - Failed to gather dependencies: %ls", *ErrorText.ToString());
...
void FStormSyncAssetFolderContextMenu::OnExportWizardCompleted(const TArray<FName>& InPackageNames, const FString& InFilepath)
{
	...
	{
		UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::OnExportWizardCompleted - Some files are invalid, preventing export.");
	...
	// Create buffer based on provided package names
	{
		...
		{
			UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::OnExportWizardCompleted - Error creating file writer");
			return;
		}
		...
		if (!FStormSyncCoreUtils::CreatePakBuffer(InPackageNames, *FileWriter, ErrorText, Delegate))
		{
			...
			UE_LOGF(LogStormSyncEditor, Error, "FStormSyncAssetFolderContextMenu::OnExportWizardCompleted - CreatePakBuffer Error: %ls", *ErrorText.ToString());
			return;
		}
	}

	// Notify user
	UE_LOGF(LogStormSyncEditor, Display, "FStormSyncAssetFolderContextMenu::OnExportWizardCompleted - Created file %ls", *InFilepath);
	...
	Info.Hyperlink = FSimpleDelegate::CreateLambda([AbsolutePath]()
	{
		...
		UE_LOGF(LogStormSyncEditor, 
			Verbose,
			"FStormSyncAssetFolderContextMenu::OnExportWizardCompleted - Clicked on \"Open Folder\" link with Directory: %ls (HyperlinkTarget: %ls)",
			*AbsolutePath,
			*HyperlinkTarget
		);
...
bool FStormSyncAssetFolderContextMenu::WarnOnInvalidPackages(const TArray<FName>& InPackageNames, const FText& InHeadingText, const FSlateBrush* InBrush)
{
	...
	if (!ErrorMessages.IsEmpty())
	{
		...
		UE_LOGF(LogStormSyncEditor, Warning, "FStormSyncAssetFolderContextMenu::WarnOnInvalidPackages - %ls", *NotificationText.ToString());
