...
void FStormSyncTransportClientEndpoint::InitializeMessaging(const FString& InEndpointFriendlyName)
{
	...
	{
		UE_LOGF(LogStormSyncClient, Warning, "FStormSyncTransportClientEndpoint::InitializeMessaging - Message endpoint already initialized");
		return;
	}
	...
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::InitializeMessaging - Default message bus is invalid");
		return;
	}

	// Create endpoint builder
	const FString MessageEndpointName = FString::Printf(TEXT("StormSync%sEndpoint"), *InEndpointFriendlyName);
	UE_LOGF(LogStormSyncClient, Verbose, "FStormSyncTransportClientEndpoint::InitializeMessaging - Setting up %ls message endpoint", *MessageEndpointName);
	...
	// Message subscribes
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::InitializeMessaging - Subscribe to messages");
...
void FStormSyncTransportClientEndpoint::RequestStatus(const FMessageAddress& InRemoteAddress, const TArray<FName>& InPackageNames, const FOnStormSyncRequestStatusComplete& InDoneDelegate) const
{
	if (!StatusService.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::RequestStatus - Status message service is invalid");
		return;
	}
...
void FStormSyncTransportClientEndpoint::RequestPushPackages(const FMessageAddress& InRemoteAddress, const FStormSyncPackageDescriptor& InPackageDescriptor, const TArray<FName>& InPackageNames, const FOnStormSyncPushComplete& InDoneDelegate) const
{
	...
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::RequestPushPackages - Push message service is invalid");
		return;
	}
...
void FStormSyncTransportClientEndpoint::RequestPullPackages(const FMessageAddress& InRemoteAddress, const FStormSyncPackageDescriptor& InPackageDescriptor, const TArray<FName>& InPackageNames, const FOnStormSyncPullComplete& InDoneDelegate) const
{
	...
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::RequestPullPackages - Pull message service is invalid");
		return;
	}
...
void FStormSyncTransportClientEndpoint::AbortStatusRequest(const FGuid& InStatusRequestId) const
{
	if (!StatusService.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::AbortStatusRequest - Status message service is invalid");
		return;
	}
...
void FStormSyncTransportClientEndpoint::AbortPushRequest(const FGuid& InPushRequestId) const
{
	if (!PushService.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::AbortPushRequest - Status message service is invalid");
		return;
	}
...
void FStormSyncTransportClientEndpoint::AbortPullRequest(const FGuid& InPullRequestId) const
{
	if (!PullService.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::AbortPullRequest - Status message service is invalid");
		return;
	}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::StartSendingBuffer - Message: %ls", *InMessage.ToString());
	...
	Async(EAsyncExecution::Thread, [LocalWeakThis, ServerAddress, PackageNames, InDoneDelegate]()
	{
		...
		{
			UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::StartSendingBuffer - CreatePakBuffer Error: %ls", *ErrorText.ToString());
			InDoneDelegate.ExecuteIfBound(MakeShared<FStormSyncSendingBufferPayload>(ErrorText));
			return;
		}
		...
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::StartSendingBuffer - Try sending on %ls", *ServerAddress);
...
TSharedPtr<FStormSyncTransportClientSocket> FStormSyncTransportClientEndpoint::GetOrCreateClientSocket(const FString& InAddress)
{
	...
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientSocket::GetOrCreateClientSocket - Failed to parse endpoint '%ls'.", *InAddress);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientSocket::GetOrCreateClientSocket - Connect failed on socket for %ls", *Endpoint.ToString());
		return nullptr;
	}
...
TSharedPtr<FStormSyncTransportClientSocket> FStormSyncTransportClientEndpoint::GetActiveConnection(const FString& InHostAddress, const TArray<FString>& InHostAdapterAddresses)
{
	...
	{
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::GetActiveConnection - Try connection on %ls", *ServerAddress);
		if (TSharedPtr<FStormSyncTransportClientSocket> Connection = GetOrCreateClientSocket(ServerAddress))
		{
			UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::GetActiveConnection - Succesfful connection on %ls", *ServerAddress);
			return Connection;
		}
	}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::HandlePongMessage - Received pong Message %ls from %ls", *InMessage.ToString(), *InMessageContext->GetSender().ToString());
}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::GetPackageNamesFromModifierInfos - ModifierInfos: %d", InModifierInfos.Num());
...
void FStormSyncTransportClientEndpoint::SendTcpBuffer(const FString& InAddress, const FStormSyncBuffer& InBuffer)
{
	...
	if (!Connection)
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientEndpoint::SendTcpBuffer - Failed to GetOrCreateClientSocket client socket to '%ls'. TCP Client sending buffer will be disabled!", *InAddress);
		return;
	}
	...
	UE_LOGF(LogStormSyncClient, 
		Display,
		"FStormSyncTransportClientEndpoint::SendTcpBuffer for %ls (State: %ls, Sending: %ls)",
		*Endpoint.ToString(),
	...
	else
	{
		UE_LOGF(LogStormSyncClient, 
			Error,
			"FStormSyncTransportClientEndpoint::SendTcpBuffer - We are currently sending data to %ls. Please wait for it to complete before trying to send new data again.",
			*Endpoint.ToString()
		);
	}
}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::OnConnectionClosedForSocket - TCP socket closed for %ls", *InEndpoint.ToString());
}
...
void FStormSyncTransportClientEndpoint::OnConnectionStateChanged(const TSharedPtr<FStormSyncTransportClientSocket> InConnection)
{
	...
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::OnConnectionStateChanged for %ls (State: %ls)", *Endpoint.ToString(), *InConnection->GetReadableConnectionState(State));
	...
	{
		FScopeLock ScopeLock(&ConnectionsCriticalSection);
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::OnConnectionStateChanged - Closed, dispose of %ls connection", *InConnection->GetRemoteEndpoint().ToString());
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientEndpoint::OnTransferComplete - TCP socket done transfering for %ls", *InConnectionAddress);
