...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientSocket::~FStormSyncTransportClientSocket Destructor Closing socket and runnable to '%ls'", *RemoteEndpoint.ToString());
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientSocket::StopTransport Closing socket and runnable to '%ls'", *RemoteEndpoint.ToString());
...
void FStormSyncTransportClientSocket::Stop()
{
	if (Socket)
	{
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientSocket::Stop Closing socket to '%ls'", *RemoteEndpoint.ToString());
		Socket->Close();
		ConnectionClosedDelegate.ExecuteIfBound(RemoteEndpoint);
	}
}
...
{
	UE_LOGF(LogStormSyncClient, Log, "Started Connection to '%ls'", *RemoteEndpoint.ToString());
	...
	while (bListenerIsRunning && !bStopping)
	{
		...
		if (DeltaTime > 1.2f)
		{
			UE_LOGF(LogStormSyncClient, Warning, "Hitch detected; %.3f seconds since prior tick", DeltaTime);
		}
		...
		UE_LOGF(LogStormSyncClient, 
			Verbose,
			"Running ... Connected: %ls (%ls), State: %ls, bListenerIsRunning: %ls %ls",
		...
		{
			UE_LOGF(LogStormSyncClient, Display, "Stopped. Reason: %ls", *StopReason);
		}
...
bool FStormSyncTransportClientSocket::Tick(FString& StopReason)
{
	...
	// check if the socket has closed
	{
		...
		if (!Socket->Recv(&Dummy, 1, BytesRead, ESocketReceiveFlags::Peek))
		{
			...
			UE_LOGF(LogStormSyncClient, Verbose, "%ls", *StopReason);
...
{
	bool bSuccess = false;
	UE_LOGF(LogStormSyncClient, Verbose, "Connection to '%ls', trying...", *RemoteEndpoint.ToString());
	...
	if (Socket && Socket->Connect(RemoteEndpoint.ToInternetAddr().Get()))
	{
		...
		if (SocketConnectionState != SCS_ConnectionError)
		{
			...
			UE_LOGF(LogStormSyncClient, 
				Display,
				"Connection to %ls successful. Connection State: %ls",
				*RemoteEndpoint.ToString(),
		...
		else
		{
			...
			UE_LOGF(LogStormSyncClient, 
				Warning,
				"Socket connected to %ls with invalid state: %ls (Last Error Code: %ls)",
				*RemoteEndpoint.ToString(),
				*GetSocketReadableConnectionState(SocketConnectionState),
				*SocketErrorCode
			);
			UE_LOGF(LogStormSyncClient, Warning, "\t- Ensure the connection is on the correct subnet.");
			UE_LOGF(LogStormSyncClient, Warning, "\t- Ensure the firewall settings are not blocking the connection.");
		}
	}
	...
	if (Socket)
	{
		...
		if (!Socket->Wait(ESocketWaitConditions::WaitForRead, FTimespan::FromSeconds(SocketWaitForReadTimeSeconds)))
		{
			UE_LOGF(LogStormSyncClient, 
				Warning,
				"Connect wait for state packet failed. We were able to connect to %ls but server didn't respond (Last Error Code: %ls)",
				*RemoteEndpoint.ToString(),
		...
		if (Socket->HasPendingData(PendingDataSize))
		{
			...
			{
				UE_LOGF(LogStormSyncClient, Error, "Error while receiving data via endpoint %ls", *RemoteEndpoint.ToString());
				return false;
			}
...
void FStormSyncTransportClientSocket::CloseSocket(const bool bShouldTriggerEvent)
{
	if (Socket)
	{
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientSocket::CloseSocket Closing socket to '%ls'", *RemoteEndpoint.ToString());
		Socket->Close();
...
void FStormSyncTransportClientSocket::SendBuffer(const FStormSyncBuffer& InBuffer)
{
	...
	UE_LOGF(LogStormSyncClient, 
		Display,
		"FStormSyncTransportClientSocket::SendBuffer - Sending buffer of size %d to %ls:%d endpoint (State: %ls)",
		BufferSize,
...
bool FStormSyncTransportClientSocket::Send(const uint8* Data, uint32 Size, const bool bPrependSize)
{
	if (!Socket)
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientSocket::Send - Socket is not active. Make sure to call Connect() prior to sending buffers.");
		return false;
	}
	...
	if (ConnectionState != State_Connected)
	{
		UE_LOGF(LogStormSyncClient, 
			Warning,
			"FStormSyncTransportClientSocket::Send - Connection State seems to be invalid: %ls (Socket State: %ls, Last Socket Error: %ls)",
			*GetReadableConnectionState(ConnectionState),
			*GetSocketReadableConnectionState(Socket->GetConnectionState()),
			*GetSocketReadableErrorCode()
		);
		
		UE_LOGF(LogStormSyncClient, Warning, "Attempted to send a socket message but no socket connected, ignoring...");
		return false;
	}
	...
	UE_LOGF(LogStormSyncClient, Display, "Client: Sending %d bytes (Size: %d) ...", Buffer.Num(), Size);
	if (!BlockingSend(Buffer.GetData(), Buffer.Num()))
	{
		const FString ErrorCode = GetSocketReadableErrorCode();
		UE_LOGF(LogStormSyncClient, Warning, "FStormSyncTransportClientSocket::BlockingSend failed with code %ls", *ErrorCode);
		return false;
	}
...
{
	UE_LOGF(LogStormSyncClient, VeryVerbose, "FStormSyncTransportClientSocket::ParseIncomingMessage from %ls - Message: %ls", *InEndpoint.ToString(), *InMessage);
	...
	if (CommandName == TEXT("size"))
	{
		...
		if (!JsonData->TryGetNumberField(FieldName, ReceivedBytes))
		{
			UE_LOGF(LogStormSyncClient, 
				Error,
				"FStormSyncTransportClientSocket::ParseIncomingMessage \"size\" command missing required field \"%ls\" or not able to convert to number",
				*FieldName
			);
			return false;
		}
