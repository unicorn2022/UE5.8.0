...
void FStormSyncStatusMessageService::RequestStatus(const FMessageAddress& InRemoteAddress, const TArray<FName>& InPackageNames, const FOnStormSyncRequestStatusComplete& DoneDelegate)
{
	...
	.Then([LocalWeakThis, PackageNames = MoveTemp(LocalPackageNames), RemoteAddress = MoveTemp(MessageAddress), DoneDelegate](const TFuture<TArray<FStormSyncFileDependency>> Result)
	{
		...
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncStatusMessageService::RequestStatus - Async FileDependencies is empty, something went wrong");
			return;
		}
		...
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncStatusMessageService::RequestStatus - Unable to get message endpoint");
			return;
		}
		...
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncStatusMessageService::RequestStatus - Push request message is invalid");
			return;
		}
		...
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncStatusMessageService::RequestStatus - FileDependencies: %d", FileDependencies.Num());
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncStatusMessageService::RequestStatus - Message: %ls", *RequestStatusMessage->ToString());
...
void FStormSyncStatusMessageService::HandleStatusPing(const FStormSyncTransportStatusPing& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& MessageContext)
{
	...
	if (!MessageEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncStatusMessageService::HandleStatusPing - Unable to get client endpoint");
		return;
	}

	UE_LOGF(LogStormSyncClient, Display, "FStormSyncStatusMessageService::HandleStatusPing - Sender: %ls", *MessageContext->GetSender().ToString());

	// Send back pong message so that this recipient knows about this editor instance
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncStatusMessageService::HandleStatusPing - Send pong Message back to %ls...", *MessageContext->GetSender().ToString());
...
void FStormSyncStatusMessageService::HandleStatusRequest(const FStormSyncTransportStatusRequest& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& MessageContext)
{
	...
	UE_LOGF(LogStormSyncClient, Display,
		"FStormSyncStatusMessageService::HandleStatusRequest - Message: %ls, Sender: %ls", *InMessage.ToString(), *Sender.ToString());
	...
	Async(EAsyncExecution::Thread, [Sender, MessageEndpointWeak, MessageId = InMessage.MessageId, PackageNames = InMessage.PackageNames, Dependencies = InMessage.Dependencies]()
	{
		...
		UE_LOGF(LogStormSyncClient, Display, "GetSyncFileModifiers for Packages %ls",
		...
		UE_LOGF(LogStormSyncClient, Display, "GetSyncFileModifiers ... - Modifiers: %d", Message->Modifiers.Num());
		for (const FStormSyncFileModifierInfo& Modifier : Message->Modifiers)
		{
			UE_LOGF(LogStormSyncClient, Display, "\t Modifier: %ls", *Modifier.ToString());
		}

		const TSharedPtr<FMessageEndpoint, ESPMode::ThreadSafe> MessageEndpoint = MessageEndpointWeak.Pin();
		if (!MessageEndpoint.IsValid())
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncStatusMessageService::HandleStatusRequest - Unable to get client endpoint");
			return;
		}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncStatusMessageService::HandleStatusResponse - Message: %ls, Sender: %ls (Id: %ls)", *InMessage.ToString(), *MessageContext->GetSender().ToString(), *InMessage.StatusRequestId.ToString());
	...
	{
		UE_LOGF(LogStormSyncClient, Display, "\t Original Modifier: %ls", *Modifier.ToString());
		...
		if (Modifier.ModifierOperation == EStormSyncModifierOperation::Missing)
		{
			...
			{
				UE_LOGF(LogStormSyncClient, Display, "\t\t Adding as missing locally, Modifier: %ls", *Modifier.ToString());
			...
			{
				UE_LOGF(LogStormSyncClient, Display, "\t\t Mismatched state locally, add it as overwrite operation. Modifier: %ls", *Modifier.ToString());
	...
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncStatusMessageService::HandleStatusResponse - NewModifiers: %d", NewModifiers.Num());
	for (const FStormSyncFileModifierInfo& Modifier : NewModifiers)
	{
		UE_LOGF(LogStormSyncClient, Display, "\t New Modifier: %ls", *Modifier.ToString());
	}
