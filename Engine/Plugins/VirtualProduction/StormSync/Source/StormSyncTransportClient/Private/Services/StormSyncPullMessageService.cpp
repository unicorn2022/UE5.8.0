...
void FStormSyncPullMessageService::RequestPullPackages(const FMessageAddress& InRemoteAddress, const FStormSyncPackageDescriptor& InPackageDescriptor, const TArray<FName>& InPackageNames, const FOnStormSyncPullComplete& InDoneDelegate)
{
	...
	.Then([LocalWeakThis, PackageNames = MoveTemp(LocalPackageNames), PackageDescriptor = MoveTemp(LocalPackageDescriptor), Recipient = MoveTemp(MessageAddress), InDoneDelegate](const TFuture<TArray<FStormSyncFileDependency>> Result)
	{
		...
		if (FileDependencies.IsEmpty())
		{
			...
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncPullMessageService::RequestPullPackages - %ls", *ErrorText.ToString());
		...
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::RequestPullPackages - FileDependencies: %d", FileDependencies.Num());
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::RequestPullPackages - Message: %ls", *PullRequestMessage->ToString());
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::RequestPullPackages - Syncing package descriptor %ls", *PullRequestMessage->PackageDescriptor.ToString());
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::HandlePullRequestMessage - Received pull request Message from %ls (%ls)", *InMessageContext->GetSender().ToString(), *InMessage.ToString());
	...
	Async(EAsyncExecution::Thread, [LocalWeakThis, IncomingMessage = MoveTemp(PullRequestMessage), RemoteAddress = MoveTemp(SenderAddress)]()
	{
		...
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncPullMessageService::HandlePullRequestMessage - Invalid local endpoint.");
			return;
		}
		...
		UE_LOGF(LogStormSyncClient, 
			Display,
			"FStormSyncPullMessageService::HandlePullRequestMessage - Initializing tcp connection for %ls and message: %ls",
		...
		if (Response.Modifiers.IsEmpty())
		{
			...
			UE_LOGF(LogStormSyncClient, Display, "\t FStormSyncPullMessageService::HandlePullRequestMessage - %ls", *ErrorText.ToString());
		...
		const FOnStormSyncSendBufferCallback DoneDelegate = FOnStormSyncSendBufferCallback::CreateLambda([Connection, IncomingMessage, LocalThis, RemoteAddress](const TSharedPtr<FStormSyncSendingBufferPayload>& Payload)
		{
			...
			if (!Payload->bSuccess)
			{
				UE_LOGF(LogStormSyncClient, Error, "FStormSyncPullMessageService::HandlePullRequestMessage - %ls", *Payload->ErrorText.ToString());
		...
		if (!Connection.IsValid())
		{
			...
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncPullMessageService::HandlePullRequestMessage - %ls", *ErrorText.ToString());
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::HandlePullResponseMessage - Sender: %ls, Message: %ls", *InMessageContext->GetSender().ToString(), *InMessage.ToString());
...
void FStormSyncPullMessageService::HandleTransferComplete(const FStormSyncTransportPullResponse InResponseMessage, const FMessageAddress InRemoteAddress) const
{
	...
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::HandleTransferComplete - TCP transfer done for message with ID: %ls (Remote: %ls)", *Response.MessageId.ToString(), *InRemoteAddress.ToString());
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::SendPullResponse - Sending to remote %ls payload: %ls", *InRemoteAddress.ToString(), *InResponsePayload.ToString());

	const TSharedPtr<FMessageEndpoint, ESPMode::ThreadSafe> MessageEndpoint = GetMessageEndpoint();
	if (!MessageEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncPullMessageService::SendPullResponse - Unable to get client message endpoint (trying sending to remote: %ls)", *InRemoteAddress.ToString());
		return;
	}

	UE_LOGF(LogStormSyncClient, 
		Display,
		"FStormSyncPullMessageService::SendPullResponse - Sending Pull Response with ID: %ls (Remote: %ls) - Message: %ls",
	...
	if (!Message.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncPullMessageService::SendPullResponse - Pull response message is invalid");
		return;
	}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::ValidateIncomingPullMessage - Validate incoming pull message: %ls", *InMessage.ToString());
	
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncPullMessageService::ValidateIncomingPullMessage - Checking for missing package names on disk ... (%ls)", *InMessage.ToString());
