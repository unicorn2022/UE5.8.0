...
void FStormSyncTransportClientModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FStormSyncTransportClientModule::OnPostEngineInit);
...
void FStormSyncTransportClientModule::StartClientEndpoint(const FString& InEndpointFriendlyName)
{
	if (!ClientEndpoint.IsValid())
	{
		...
		if (!ClientEndpoint.IsValid())
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::StartClientEndpoint - Failed to create Client Local Endpoint");
			return;
		}
	}
...
FMessageEndpointSharedPtr FStormSyncTransportClientModule::GetClientMessageEndpoint() const
{
	if (!ClientEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Warning, "FStormSyncTransportClientModule::GetClientMessageEndpoint - Client endpoint not valid");
		return nullptr;
	}

	if (!ClientEndpoint->IsRunning())
	{
		UE_LOGF(LogStormSyncClient, Warning, "FStormSyncTransportClientModule::GetClientMessageEndpoint - Client endpoint not running");
		return nullptr;
	}
...
void FStormSyncTransportClientModule::SynchronizePackages(const FStormSyncPackageDescriptor& InPackageDescriptor, const TArray<FName>& InPackageNames) const
{
	...
	.Then([MessageEndpoint, PackageNames = MoveTemp(LocalPackageNames), PackageDescriptor = MoveTemp(LocalPackageDescriptor)](const TFuture<TArray<FStormSyncFileDependency>> Result)
	{
		...
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::SynchronizePackages - Async FileDependencies is empty, something went wrong");
			return;
		}

		if (!MessageEndpoint.IsValid())
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::SynchronizePackages - Unable to get client message endpoint");
			return;
		}
		...
		{
			UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::SynchronizePackages - Push request message is invalid");
			return;
		}

		SyncRequestMessage->PackageDescriptor.Dependencies = FileDependencies;

		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::SynchronizePackages - FileDependencies: %d", FileDependencies.Num());
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::SynchronizePackages - Message: %ls", *SyncRequestMessage->ToString());
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::SynchronizePackages - Syncing package descriptor %ls", *SyncRequestMessage->PackageDescriptor.ToString());
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::PushPackages - PackageDescriptor: %ls, InPackageNames: %d, MessageAddressId: %ls", *InPackageDescriptor.ToString(), InPackageNames.Num(), *InMessageAddress.ToString());

	if (!ClientEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::PushPackages - Unable to get client endpoint");
		return;
	}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::PullPackages - PackageDescriptor: %ls, InPackageNames: %d, MessageAddressId: %ls", *InPackageDescriptor.ToString(), InPackageNames.Num(), *InMessageAddress.ToString());

	if (!ClientEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::PullPackages - Unable to get client endpoint");
		return;
	}
...
{
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::RequestPackagesStatus - InRemoteAddress: %ls, InPackageNames: %d", *InRemoteAddress.ToString(), InPackageNames.Num());

	if (!ClientEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::RequestPackagesStatus - Unable to get client endpoint");
		return;
	}
...
void FStormSyncTransportClientModule::PublishStatusPingMessage() const
{
	...
	if (!MessageEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::PublishStatusPingMessage - Unable to send Connect Message cause Message Endpoint is invalid");
		return;
	}
	...
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::PublishStatusPingMessage - Publish Client Connect Message ...");
...
void FStormSyncTransportClientModule::OnWakeup()
{
	...
	if (!MessageEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Warning, "FStormSyncTransportClientModule::OnWakeup - Unable to get client message endpoint");
		return;
	}
...
void FStormSyncTransportClientModule::ExecutePing(const TArray<FString>& InArgs)
{
	...
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::ExecutePing - %ls", *Argv);

	const FMessageEndpointSharedPtr MessageEndpoint = GetClientMessageEndpoint();
	if (!MessageEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Warning, "FStormSyncTransportClientModule::ExecutePing - Unable to get client message endpoint");
		return;
	}
...
FStormSyncPackageDescriptor FStormSyncTransportClientModule::CreatePackageDescriptorFromCommandLine(const FString& InArgv)
{
	...
	if (!FParse::Value(*InArgv, TEXT("-name="), PackageDescriptor.Name))
	{
		...
		UE_LOGF(LogStormSyncClient, Display, "FStormSyncEditorModule::CreatePackageDescriptorFromCommandLine - Missing -name parameter, using default \"%ls\"", *PackageDescriptor.Name);
	}
...
{
	const FString Argv = FString::Join(InArgs, TEXT(" "));
	UE_LOGF(LogStormSyncClient, Display, "FStormSyncTransportClientModule::ExecuteSyncPak - Argv: %ls", *Argv);
	...
	{
		UE_LOGF(LogStormSyncClient, Error, "FStormSyncTransportClientModule::ExecuteSyncPak - Missing at least one package name to sync.");
		return;
	}
...
void FStormSyncTransportClientModule::ExecuteDebug(const TArray<FString>& InArgs)
{
	...
	UE_LOGF(LogStormSyncClient, Display, "StormSync.Client.Debug - EndpointId: %ls", *AddressId);
	const FString InstanceId = FApp::GetInstanceId().ToString();
	UE_LOGF(LogStormSyncClient, Display, "StormSync.Client.Debug - InstanceId: %ls", *InstanceId);
}
...
void FStormSyncTransportClientModule::ExecuteDebugPing(const TArray<FString>& InArgs)
{
	...
	if (!MessageEndpoint.IsValid())
	{
		UE_LOGF(LogStormSyncClient, Error, "StormSync.Client.Debug.Ping - Unable to send Connect Message cause Message Endpoint is invalid");
		return;
	}

	UE_LOGF(LogStormSyncClient, Display, "StormSync.Client.Debug.Ping - Publish Client Connect Message ...");
	FStormSyncTransportStatusPing* Message = FMessageEndpoint::MakeMessage<FStormSyncTransportStatusPing>();
	MessageEndpoint->Publish(Message);
}
