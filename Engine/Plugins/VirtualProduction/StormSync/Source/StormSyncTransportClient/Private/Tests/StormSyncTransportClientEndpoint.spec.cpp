...
public:
	...
	virtual bool Init() override
	{
		...
		if (SocketListener->IsActive())
		{
			...
			UE_LOGF(LogTemp, Display, "FMockTcpServer: Started listening on %ls:%d", *SocketListener->GetLocalEndpoint().Address.ToString(), SocketListener->GetLocalEndpoint().Port);
			return true;
		}

		UE_LOGF(LogTemp, Error, "FMockTcpServer: Could not create Tcp Listener!");
		return false;	
	}
	...
	void StopListening()
	{
		...
		{
			UE_LOGF(LogTemp, Display, "FMockTcpServer: No longer listening on %ls:%d", *SocketListener->GetLocalEndpoint().Address.ToString(), SocketListener->GetLocalEndpoint().Port);
		...
		while (!bStopping)
		{
			if (RemoteConnection.IsValid())
			{
				...
				while (RemoteConnection->HasPendingData(PendingDataSize))
				{
					...
					{
						UE_LOGF(LogTemp, Error, "FMockTcpServer: Error while receiving data via endpoint %ls", *RemoteEndpoint.ToString());
						continue;
					}
					...
					for (int32 i = 0; i < BytesRead; ++i)
					{
						...
						if (!bHasReceivedSize && i == 3)
						{
							...
							UE_LOGF(LogTemp, Verbose, "FMockTcpServer: Received size: %d. Setting it for %ls", Size, *RemoteEndpoint.ToString());
					...
					if (bHasReceivedSize)
					{
						...
						if (BufferSize == BufferExpectedSize)
						{
							...
							UE_LOGF(LogTemp, Display, "\tFMockTcpServer: We received full buffer!!");
...
private:
	...
	{
		UE_LOGF(LogTemp, Display, "FMockTcpServer: Incoming connection -> %ls", *InEndpoint.ToString());
...
void FStormSyncTransportClientEndpointSpec::Define()
{
	Describe(TEXT("Message System"), [this]()
	{
		...
		LatentIt(TEXT("Should send tcp buffer to local server when it receives a sync response message"), [this](const FDoneDelegate& Done)
		{
			...
			{
				AddInfo(FString::Printf(TEXT("Mock TCP Server Received Buffer of size %lld"), Buffer->Num()));
