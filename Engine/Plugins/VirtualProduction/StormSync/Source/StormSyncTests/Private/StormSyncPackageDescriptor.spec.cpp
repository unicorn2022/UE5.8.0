...
void FStormSyncPackageDescriptorSpec::Define()
{
	Describe(TEXT("FStormSyncFileDependency::GetDestFilepath"), [this]()
	{
		...
		It(TEXT("should return expected path, if it is a package from a valid mount point"), [this]()
		{
		...
		It(TEXT("should reject package paths that escape mounted points via path traversal (UE-368657)"), [this]()
		{
			// A malformed/malicious .spak file can contain PackageName entries containing ../ sequences to escape
			// the project's mount points and write to arbitrary locations on disk. GetDestFilepath
			// must return an empty path and a populated failure reason for any such input.
			const TArray<FName> TestPackageNames = {
				TEXT("/Game/../../Foo/Bar"),
				TEXT("/StormSyncTests_Temp/../../Foo/Bar"),
				TEXT("/StormSync/../../Foo/Bar")
			};
			for (const FName& PackageName : TestPackageNames)
			{
				AddInfo(FString::Printf(TEXT("Testing malicious/malformed PackageName: %s"), *PackageName.ToString()));

				FText ErrorText;
				const FString DestFilepath = FStormSyncFileDependency::GetDestFilepath(PackageName, ErrorText);
				TestTrue(FString::Printf(TEXT("DestFilepath is empty for traversal attempt %s"), *PackageName.ToString()), DestFilepath.IsEmpty());
				TestFalse(FString::Printf(TEXT("ErrorText is set for traversal attempt %s"), *PackageName.ToString()), ErrorText.IsEmpty());
			}
		});
