...
int32 FRemoteControlMIDIDevice::ResolveDeviceId(const TArray<FFoundMIDIDevice>& InFoundDevices)
{
	if(InFoundDevices.Num() == 0)
	{
		...
		UE_LOGF(LogRemoteControlProtocolMIDI, Display, "RemoteControlProtocolMIDI bindings specified, but no devices are available.")
	...
	{
		UE_LOGF(LogRemoteControlProtocolMIDI, Warning, "MIDI Device with Id %i was found, but already in use and unavailable.", FoundDeviceId);
		bDeviceIsAvailable = false;
	...
	{
		UE_LOGF(LogRemoteControlProtocolMIDI, Warning, "MIDI Device with user specified Id %i was not found, using Device 1.", DeviceId);
		// We've already checked if theres one or more devices
		return ResolvedDeviceId = InFoundDevices[0].DeviceID;
	}
	else
	{
		UE_LOGF(LogRemoteControlProtocolMIDI, Warning, "MIDI Device with specified name %ls was not found, using Device 1.", *DeviceName.ToString());
		// We've already checked if theres one or more devices
		return ResolvedDeviceId = InFoundDevices[0].DeviceID;
	}
}
