...
DEFINE_LOG_CATEGORY_STATIC(LogFolderHierarchyPanel, Log, All);
			...
			+ SHorizontalBox::Slot()
				...
				[
					SAssignNew(TreeItem->NameWidget, SNamingTokensEditableTextBox)
						...
						.DisplayErrorMessage(!TreeItem->bIsReadOnly)
						...
						.OnValidateTokenizedText(this, &SFolderHierarchyPanel::IsValidFolderName, TreeItem)
	...
	if (SelectedNodes.Num() == 1)
	{
		...
		else if (SelectedTreeItem->Status == ETemplateFolderStatus::MissingDoNotCreate)
		{
			MenuBuilder.AddMenuEntry(
				...
				LOCTEXT("MarkAsCreateToolTip", "Mark this folder so that it will be created when clicking Create Template Folders"),
...
bool SFolderHierarchyPanel::IsValidFolderName(const FText& InText, FText& OutErrorMessage, TSharedPtr<FTemplateFolderTreeItem> TreeItem)
{
	...
	// Try to heuristically determine if this will result in a valid path.
	// If the path contains any tokens, then we don't actually know what its final length will be, but we try our best to warn the user against egregious mistakes.
	const FString ParentPath = FPaths::GetPath(TreeItem->Path.GetInternalPathString());
	const FString ProposedPath = ParentPath / InText.ToString();

	FString ProposedPathForValidation = ProposedPath;
	ProposedPathForValidation.ReplaceInline(TEXT("{"), TEXT(""));
	ProposedPathForValidation.ReplaceInline(TEXT("}"), TEXT(""));
	ProposedPathForValidation.ReplaceInline(TEXT(":"), TEXT(""));

	if (!AssetViewUtils::IsValidPackageForCooking(ProposedPathForValidation, OutErrorMessage))
	{
		return false;
	}

	// Check for duplicate folder names
	TSharedPtr<FTemplateFolderTreeItem> FoundItem = FindItemAtPath(ProposedPath);
...
{
	// Guards against re-entrant SetTemplateFolderName when FMessageDialog::Open's nested message pump dispatches a second OnTextCommitted.
	if (bIsSettingTemplateFolderName)
	{
		return;
	}

	TGuardValue<bool> ReentryGuard(bIsSettingTemplateFolderName, true);

	// Early-out if the folder is read-only, or if the name has not actually changed
	if (TreeItem->bIsReadOnly)
	{
		return;
	}
...
void SFolderHierarchyPanel::DeleteTemplateFolder(TSharedPtr<FTemplateFolderTreeItem> TreeItem)
{
	...
	if (TreeItem->Status == ETemplateFolderStatus::Exists && IsFolderEmpty(TreeItem))
	{
		...
		if (Response == EAppReturnType::Yes)
		{
			// Resolve tokens before deleting
			const FString ResolvedPath = ResolveTemplatePath(TreeItem->Path.GetInternalPathString());
			UDirectoryPlaceholderLibrary::DeletePlaceholdersInPath(ResolvedPath);
			AssetViewUtils::DeleteFolders({ ResolvedPath });
		}
		else if (Response == EAppReturnType::Cancel)
		{
			return;
...
void SFolderHierarchyPanel::CreateFolderFromTemplateRecursive(TSharedPtr<FTemplateFolderTreeItem> TreeItem)
{
	if (TreeItem->Status == ETemplateFolderStatus::MissingCreate)
	{
		...
		// Validate the path before attempting to create on disk.
		FText InvalidPackageError;
		if (!AssetViewUtils::IsValidPackageForCooking(SanitizedPath, InvalidPackageError))
		{
			UE_LOGF(LogFolderHierarchyPanel, Warning, "Cannot create template folder '%ls': %ls", *SanitizedPath, *InvalidPackageError.ToString());
		}
		else
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogFolderHierarchyPanel, Warning, "Failed to create template folder '%ls'", *AbsoluteFilePath);
				}
			}
		}
	}
...
bool SFolderHierarchyPanel::IsFolderEmpty(TSharedPtr<FTemplateFolderTreeItem> TreeItem) const
{
	...
	// Resolve tokens in the template path
	const FString ResolvedPath = ResolveTemplatePath(TreeItem->Path.GetInternalPathString());

	FARFilter Filter;
	Filter.bRecursivePaths = true;
	Filter.PackagePaths.Add(FName(*ResolvedPath));
...
bool SFolderHierarchyPanel::DoesPathExist(const FString& Path) const
{
	...
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::GetModuleChecked<FAssetRegistryModule>("AssetRegistry");
	return AssetRegistryModule.Get().PathExists(ResolveTemplatePath(Path));
}

FString SFolderHierarchyPanel::ResolveTemplatePath(const FString& TemplatePath) const
{
	...
	const FNamingTokenResultData Result = NamingTokensSubsystem->EvaluateTokenString(TemplatePath, FilterArgs);
