...
void SCineAssemblyConfigWindow::Construct(const FArguments& InArgs, const FString& InCreateAssetPath)
{
	...
	CineAssemblyToConfigure->Initialize();

	// Initialize the Assembly's name to a good default
	FAssetToolsModule& AssetToolsModule = FModuleManager::GetModuleChecked<FAssetToolsModule>("AssetTools");
	const TOptional<FString> DefaultAssemblyName = AssetToolsModule.Get().GetDefaultAssetNameForClass(UCineAssembly::StaticClass());

	CineAssemblyToConfigure->AssemblyName.Template = DefaultAssemblyName.Get(TEXT("NewCineAssembly"));
...
TSharedRef<SWidget> SCineAssemblyConfigWindow::MakeSchemaPanel()
{
	...
	Config.OnShouldFilterAsset = FOnShouldFilterAsset::CreateLambda([NoSchemaAssetData](const FAssetData& AssetData) -> bool
		{
			const FName IsHiddenPropertyName = GET_MEMBER_NAME_CHECKED(UCineAssemblySchema, bIsHidden);

			bool bIsSchemaHidden = false;
			AssetData.GetTagValue(IsHiddenPropertyName, bIsSchemaHidden);

			// Filter out any schemas that should be hidden
			return (AssetData == NoSchemaAssetData) ? false : bIsSchemaHidden;
		});
...
void SCineAssemblyConfigWindow::OnSchemaSelected(const FAssetData& AssetData)
{
	...
	if (AssetData.AssetName == NoSchemaName)
	{
		...
		// Re-initialize the Assembly's name to a good default
		FAssetToolsModule& AssetToolsModule = FModuleManager::GetModuleChecked<FAssetToolsModule>("AssetTools");
		const TOptional<FString> DefaultAssemblyName = AssetToolsModule.Get().GetDefaultAssetNameForClass(UCineAssembly::StaticClass());
	...
	{
		CineAssemblyToConfigure->InitializeFromSchema(CineAssemblySchema);

		// If the schema does not override the default Level, reset to the current editor world
		if (!CineAssemblySchema->bOverrideDefaultLevel)
		{
			UWorld* CurrentWorld = GEditor->GetEditorWorldContext().World();
			CineAssemblyToConfigure->Level = FSoftObjectPath(CurrentWorld);
		}
...
FReply SCineAssemblyConfigWindow::OnCreateAssetClicked()
{
	if (ValidateAssetName())
	{
		...
		UCineAssemblyFactory::DuplicateSubsequences(CineAssemblyToConfigure.Get());
		UCineAssemblyFactory::DuplicateAssociatedAssets(CineAssemblyToConfigure.Get());
