...
void UProductionSettings::PostInitProperties()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddUObject(this, &UProductionSettings::OnPostEngineInit);
}
...
{
	// Register the active production's default asset names with AssetTools
	if (const FCinematicProduction* ActiveProduction = GetActiveProductionPtr())
	{
		...
		for (const TPair<TObjectPtr<const UClass>, FString>& Naming : ActiveProduction->DefaultAssetNames)
		{
			if (Naming.Key)
			{
				...
				RegisteredAssetNameClasses.Add(Naming.Key);
			}
		}
	}
}
...
void UProductionSettings::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UProductionSettings, Productions))
	{
		if (PropertyChangedEvent.ChangeType == EPropertyChangeType::ArrayAdd)
		{
			...
			OnProductionCreatedDynamic.Broadcast(Productions[ArrayIndex]);
		}
		else
		{
			// It is possible that a production was just deleted. If it was the active production, funnel the change
			// through SetActiveProduction so all overrides (display rate, start time, subsequence priority, asset names)
			// are unregistered consistently.
			const FCinematicProduction* StillActiveProduction = Algo::FindBy(Productions, ActiveProductionName, &FCinematicProduction::ProductionName);
			const FGuid NewActiveProductionID = StillActiveProduction ? StillActiveProduction->ProductionID : FGuid();

			if (NewActiveProductionID != ActiveProductionID)
			{
				SetActiveProduction(NewActiveProductionID);
			}
		}
...
void UProductionSettings::AddProduction(const FCinematicProduction& ProductionToAdd)
{
	...
	{
		UE_LOGF(LogProductionSettings, Error, "Failed to add Production named %ls because the name is too long. The maximum production name length is %d", *ProductionToAdd.ProductionName, ProductionNameMaxLength);
		return;
	}
	...
	ProductionListChangedDelegate.Broadcast();
	OnProductionCreatedDynamic.Broadcast(ProductionToAdd);
}
...
void UProductionSettings::DuplicateProduction(FGuid ProductionID)
{
	if (const FCinematicProduction* ProductionToCopy = Algo::FindBy(Productions, ProductionID, &FCinematicProduction::ProductionID))
	{
		...
		{
			UE_LOGF(LogProductionSettings, Error, "Failed to duplicate Production named %ls because the name is too long. The maximum production name length is %d", *ProductionToCopy->ProductionName, ProductionNameMaxLength);
			return;
		}
		...
		ProductionListChangedDelegate.Broadcast();
		OnProductionCreatedDynamic.Broadcast(DuplicateProduction);
	}
}
...
void UProductionSettings::NotifySettingsChange(const FCinematicProduction& Production)
{
	UpdateConfig();
	OnProductionSettingsChangedDynamic.Broadcast(Production);
}
...
void UProductionSettings::HandleProductionsChangeChainProperty(const TDoubleLinkedList<FProperty*>::TDoubleLinkedListNode& ProductionsChainNode, FPropertyChangedChainEvent& WithinPropertyChangedEvent)
{
	...
	OnProductionSettingsChangedDynamic.Broadcast(Productions[ProductionIndex]);
}
...
void UProductionSettings::RenameProduction(FGuid ProductionID, FString NewName)
{
	...
	{
		UE_LOGF(LogProductionSettings, Error, "Failed to rename Production to %ls because the name is too long. The maximum production name length is %d", *NewName, ProductionNameMaxLength);
		return;
	}
	...
	if (FCinematicProduction* Production = Algo::FindBy(Productions, ProductionID, &FCinematicProduction::ProductionID))
	{
		...
		SetActiveProductionName();
		NotifySettingsChange(*Production);
	}
}
...
void UProductionSettings::SetActiveProduction(FGuid ProductionID)
{
	...
	// Unregister any default asset names we have registered with AssetTools.
	IAssetTools& AssetTools = FModuleManager::GetModuleChecked<FAssetToolsModule>("AssetTools").Get();
	for (const UClass* Class : RegisteredAssetNameClasses)
	{
		AssetTools.UnregisterDefaultAssetNameForClass(Class);
	}
	RegisteredAssetNameClasses.Empty();
	...
	if (const FCinematicProduction* NewActiveProduction = GetActiveProductionPtr())
	{
		OnActiveProductionSetDynamic.Broadcast(*NewActiveProduction, true);
	}
	else
	{
		static FCinematicProduction InvalidProduction;
		InvalidProduction.ProductionID.Invalidate();
		OnActiveProductionSetDynamic.Broadcast(InvalidProduction, false);
	}
}
...
void UProductionSettings::AddAssetNaming(FGuid ProductionID, const UClass* AssetClass, const FString& DefaultName)
{
	...
	if (FCinematicProduction* Production = Algo::FindBy(Productions, ProductionID, &FCinematicProduction::ProductionID))
	{
		...
		if (IsActiveProduction(ProductionID))
		{
			...
			RegisteredAssetNameClasses.Add(AssetClass);
		}
...
void UProductionSettings::RemoveAssetNaming(FGuid ProductionID, const UClass* AssetClass)
{
	...
	if (FCinematicProduction* Production = Algo::FindBy(Productions, ProductionID, &FCinematicProduction::ProductionID))
	{
		...
		if (IsActiveProduction(ProductionID))
		{
			...
			RegisteredAssetNameClasses.Remove(AssetClass);
		}
