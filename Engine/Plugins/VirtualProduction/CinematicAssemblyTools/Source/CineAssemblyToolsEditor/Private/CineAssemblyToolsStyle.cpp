	...
	: FSlateStyleSet(StyleName)
{
	...
	// Textbox styles
	FEditableTextBoxStyle DetailsEditableTextBoxStyle = FAppStyle::Get().GetWidgetStyle<FEditableTextBoxStyle>("NormalEditableTextBox");
	DetailsEditableTextBoxStyle.SetFont(DEFAULT_FONT("Regular", 8));

	Set("DetailsEditableTextBox", DetailsEditableTextBoxStyle);

	FTextBlockStyle DetailsArgumentStyle = FAppStyle::Get().GetWidgetStyle<FTextBlockStyle>(TEXT("NormalText"));
	DetailsArgumentStyle
		.SetFont(DEFAULT_FONT("Bold", 8))
		.SetColorAndOpacity(FLinearColor(1.0f, 1.0f, 1.0f));

	Set("DetailsEditableTextBoxArguments", DetailsArgumentStyle);
...
FName FCineAssemblyToolsStyle::GetThumbnailBrushNameForSchema(const FGuid& SchemaGuid) const
{
	const FName BrushName = *FString::Printf(TEXT("SchemaThumbnail.%s"), *SchemaGuid.ToString());
	return BrushName;
}

void FCineAssemblyToolsStyle::SetThumbnailBrushTextureForSchema(const FGuid& SchemaGuid, UTexture2D* BrushTexture)
{
	if (FSlateBrush* Result = BrushResources.FindRef(GetThumbnailBrushNameForSchema(SchemaGuid)))
	{
	...
	else
	{
		...
		Set(GetThumbnailBrushNameForSchema(SchemaGuid), NewBrush);
	}
}
