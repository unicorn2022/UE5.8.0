...
#include "Sequencer/SubAssemblyTrackEditor.h"
...
{
	FAssetCategoryPath CineAssemblyAssetCategory = FAssetCategoryPath(EAssetCategoryPaths::Cinematics, LOCTEXT("CineAssemblyCategorySection", "Cinematic Assembly Tools"), ECategoryMenuType::Section);
}
...
void FCineAssemblyToolsEditorModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FCineAssemblyToolsEditorModule::OnPostEngineInit);
	...
	// Register the SubAssembly Track Editor used by the Schema Sequencer tool
	ISequencerModule& SequencerModule = FModuleManager::Get().LoadModuleChecked<ISequencerModule>("Sequencer");
	RegisterSubTrackEditorHandle = SequencerModule.RegisterTrackEditor(FOnCreateTrackEditor::CreateStatic(&FSubAssemblyTrackEditor::CreateTrackEditor));

	// Register a Sequencer customization for UCineAssembly assets being edited in a schema template
	if (TSharedPtr<FSequencerCustomizationManager> CustomizationManager = SequencerModule.GetSequencerCustomizationManager())
	{
		CustomizationManager->RegisterInstancedSequencerCustomization(
			UCineAssembly::StaticClass(),
			FOnGetSequencerCustomizationInstance::CreateLambda([]() -> ISequencerCustomization*
			{
				return new FCineAssemblySchemaSequencerCustomization();
			}));
	}

	// Listen for property changes on UCineAssemblySchema so that the Style brush cache stays in sync
	SchemaPropertyChangedHandle = FCoreUObjectDelegates::OnObjectPropertyChanged.AddRaw(this, &FCineAssemblyToolsEditorModule::OnSchemaPropertyChanged);
}
...
void FCineAssemblyToolsEditorModule::ShutdownModule()
{
	...
	FCoreUObjectDelegates::OnObjectPropertyChanged.Remove(SchemaPropertyChangedHandle);
	...
	{
		SequencerModule->UnRegisterTrackEditor(RegisterSubTrackEditorHandle);

		if (TSharedPtr<FSequencerCustomizationManager> CustomizationManager = SequencerModule->GetSequencerCustomizationManager())
		{
			CustomizationManager->UnregisterInstancedSequencerCustomization(UCineAssembly::StaticClass());
		}
	}
...
void FCineAssemblyToolsEditorModule::OnSchemaPropertyChanged(UObject* Object, FPropertyChangedEvent& PropertyChangedEvent)
{
	const UCineAssemblySchema* Schema = Cast<UCineAssemblySchema>(Object);
	if (!Schema)
	{
		return;
	}

	if (PropertyChangedEvent.GetPropertyName() != GET_MEMBER_NAME_CHECKED(UCineAssemblySchema, ThumbnailTexture))
	{
		return;
	}

	UTexture2D* Texture = Schema->ThumbnailTexture.LoadSynchronous();
	FCineAssemblyToolsStyle::Get().SetThumbnailBrushTextureForSchema(Schema->GetSchemaGuid(), Texture);
}
...
void FCineAssemblyToolsEditorModule::OpenAssemblyForEdit(UCineAssembly* Assembly)
{
	...
	{
		UE_LOGF(LogCineAssemblyToolsEditorModule, Warning, "The Cine Assembly tab could not be opened because the maximum number of Assembly tabs are already open.");
		return;
	}
...
void FCineAssemblyToolsEditorModule::OpenSchemaForEdit(UCineAssemblySchema* Schema)
{
	...
	{
		UE_LOGF(LogCineAssemblyToolsEditorModule, Warning, "The Cine Assembly Schema tab could not be opened because the maximum number of Schema tabs are already open.");
		return;
	}
