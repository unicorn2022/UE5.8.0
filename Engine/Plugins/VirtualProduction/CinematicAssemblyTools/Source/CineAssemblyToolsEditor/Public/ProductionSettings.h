...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnProductionCreated, const FCinematicProduction&, Production);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnProductionSettingsChanged, const FCinematicProduction&, Production);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnActiveProductionSet, const FCinematicProduction&, ActiveProduction, bool, bIsValid);
...
public:
	/** Blueprint-assignable delegate that broadcasts when a new production is created */
	UPROPERTY(BlueprintAssignable, Category = "Cinematic Production", meta = (DisplayName = "On Production Created"))
	FOnProductionCreated OnProductionCreatedDynamic;

	/** Blueprint-assignable delegate that broadcasts when a production's settings are modified */
	UPROPERTY(BlueprintAssignable, Category = "Cinematic Production", meta = (DisplayName = "On Production Settings Changed"))
	FOnProductionSettingsChanged OnProductionSettingsChangedDynamic;

	/** Blueprint-assignable delegate that broadcasts when the active production changes */
	UPROPERTY(BlueprintAssignable, Category = "Cinematic Production", meta = (DisplayName = "On Active Production Set"))
	FOnActiveProductionSet OnActiveProductionSetDynamic;
...
private:
	...
	/** Update the config and broadcast OnProductionSettingsChangedDynamic for the given production */
	void NotifySettingsChange(const FCinematicProduction& Production);
...
private:
	...
	/** Asset classes for which Production Settings has currently registered a default name override with AssetTools. Tracked so we can unregister if the active production changes. */
	TSet<const UClass*> RegisteredAssetNameClasses;
