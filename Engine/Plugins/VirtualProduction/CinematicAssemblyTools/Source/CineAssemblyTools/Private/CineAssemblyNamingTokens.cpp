	...
	auto ParentTokenFunc = [](TWeakObjectPtr<UCineAssembly> InAssembly) -> FText
		{
			...
			{
				const FString PackageName = InAssembly->ParentAssembly.GetLongPackageName();
				if (FPackageName::IsInEngineTransientPackages(PackageName))
				{
					// If the parent assembly is transient, use the resolved AssemblyName value
					const UCineAssembly* ParentAssembly = Cast<UCineAssembly>(InAssembly->ParentAssembly.ResolveObject());
					return ParentAssembly ? ParentAssembly->AssemblyName.Resolved : FText::GetEmpty();
				}
				else
				{
