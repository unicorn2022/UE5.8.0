...
bool FOperatorStackEditorTabInstance::RegisterTab()
{
	...
	TSharedRef<FWorkspaceItem> Category = WorkspaceMenu::GetMenuStructure().GetLevelEditorCategory();

	// Group Operator Stack under Motion Design > Motion Design Tools if the plugin is enabled.
	constexpr auto GroupProjection = Projection(&TSharedRef<FWorkspaceItem>::Get, &FWorkspaceItem::GetFName);
	if (const TSharedRef<FWorkspaceItem>* RootGroup = Algo::FindBy(Category->GetChildItems(), FName("MotionDesignGroup"), GroupProjection))
	{
		if (const TSharedRef<FWorkspaceItem>* ToolsGroup = Algo::FindBy((*RootGroup)->GetChildItems(), FName("MotionDesignToolsGroup"), GroupProjection))
		{
			Category = *ToolsGroup;
		}
	}
...
void FOperatorStackEditorTabInstance::OnSelectionSetChanged(const UTypedElementSelectionSet* InSelection, bool bInForce)
{
	...
	{
		FTSTicker::RemoveTicker(SelectionDelegateHandle);
