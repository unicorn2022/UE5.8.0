...
void FAvaSequencerObjectBinding::BuildSequencerAddMenu(FMenuBuilder& OutMenuBuilder)
{
	...
	// Skip menu extension if the actor selection is the global actor selection, as this is already handled by FLevelSequenceEditorActorBinding
	USelection* const ActorSelection = ToolkitHost->GetEditorModeManager().GetSelectedActors();
	if (!ActorSelection || ActorSelection == GEditor->GetSelectedActors())
	{
		return;
	}
