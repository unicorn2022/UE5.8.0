...
FAvaViewportCameraHistory::FAvaViewportCameraHistory()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FAvaViewportCameraHistory::PostEngineInit);
}
...
void FAvaViewportCameraHistory::OnEndCameraTransform(UObject& InCameraObject)
{
	...
	if (AActor* CameraActor = Cast<AActor>(&InCameraObject))
	{
		...
		UE_LOGF(LogAva, Verbose, "Saved camera transform at %i, head %i, %ls", CameraTransformHistoryIndex, CameraTransformHistoryHeadIndex, *CameraActor->GetActorTransform().Rotator().ToString());
	}
}
...
void FAvaViewportCameraHistory::ExecuteCameraTransformUndo()
{
	...
	if (AActor* CameraActor = UndoPair.Key.Get())
	{
		...
		UE_LOGF(LogAva, Verbose, "Undo camera transform at %i, %ls", CameraTransformIndexToRestore, *CameraActor->GetActorTransform().Rotator().ToString());
	}
}
...
void FAvaViewportCameraHistory::ExecuteCameraTransformRedo()
{
	...
	if (AActor* CameraActor = RedoPair.Key.Get())
	{
		...
		UE_LOGF(LogAva, Verbose, "Redo camera transform at %i, %ls", CameraTransformIndexToRestore, *CameraActor->GetActorTransform().Rotator().ToString());
	}
}
