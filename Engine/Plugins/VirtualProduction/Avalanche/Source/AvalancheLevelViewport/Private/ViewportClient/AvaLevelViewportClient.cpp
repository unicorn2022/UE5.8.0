...
bool FAvaLevelViewportClient::ProcessScreenShots(FViewport* InViewport)
{
	bool bIsRunningAvaScreenshotTest = false;

	for (AScreenshotFunctionalTestAvalanche* Test : TObjectRange<AScreenshotFunctionalTestAvalanche>())
	{
		if (!Test->IsTemplate() && Test->IsRunning())
		{
			return false;
		}
	}
	...
	/*
	if (IFunctionalTestingModule::Get().IsRunning())
	{
		return false;
	}
	*/
