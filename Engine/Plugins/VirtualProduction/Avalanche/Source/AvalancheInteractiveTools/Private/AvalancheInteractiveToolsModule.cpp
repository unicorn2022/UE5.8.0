...
void FAvalancheInteractiveToolsModule::StartupModule()
{
	...
	{
		FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FAvalancheInteractiveToolsModule::OnPostEngineInit);
	}
}
...
void FAvalancheInteractiveToolsModule::RegisterAutoRegisterTools()
{
	for (const UClass* const Class : TObjectRange<UClass>())
	{
		...
		UE_LOGF(LogAvaInteractiveTools, Log, "Tool %ls auto registered", *Class->GetName())
	}
}
