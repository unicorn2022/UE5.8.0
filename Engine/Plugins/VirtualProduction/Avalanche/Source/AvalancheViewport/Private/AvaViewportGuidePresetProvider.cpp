...
namespace UE::AvaViewport::Private
{
	...
	bool LoadPreset(const FString& InPath, TArray<FAvaViewportGuideInfo>& OutGuides, const FVector2f InViewportSize)
	{
		...
		{
			UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: LoadPreset() - Unable to load Json file: %ls", *InPath);
			return false;
		}
		...
		{
			UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: LoadPreset() - Unable to parse file [%ls]. Json=[%ls]", *InPath, *FileJsonString);
			return false;
		}

		if (!RootObject->HasTypedField<EJson::Array>(GuideListFieldName))
		{
			UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: LoadPreset() - Malformed data in json [%ls]. Json=[%ls]", *InPath, *FileJsonString);
			return false;
		}
		...
		for (const TSharedPtr<FJsonValue>& JsonGuide : JsonGuides)
		{
			if (TSharedPtr<FJsonObject> JsonGuideObject = JsonGuide->AsObject())
			{
				...
				{
					UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: LoadPreset() - Malformed data in guide json [%ls]. Json=[%ls]", *InPath, *FileJsonString);
					return false;
				}
			}
			else
	...
	bool SavePreset(const FString& InPath, const TArray<FAvaViewportGuideInfo>& InGuides, const FVector2f InViewportSize)
	{
		...
		for (const FAvaViewportGuideInfo& Guide : InGuides)
		{
			...
			{
				UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: SavePreset() - Unable to create guide json [%ls]", *InPath);
			}
		}
		...
		{
			UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: SavePreset() - Unable to serialize [%ls]. Json=[%ls]", *InPath, *FileJsonString);
			return false;
		}

		if (!FFileHelper::SaveStringToFile(FileJsonString, *InPath))
		{
			UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: SavePreset() - Unable to save Json file: %ls", *InPath);
			return false;
		}
	...
	const FVector2f InViewportSize)
{
	...
	{
		UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: SaveGuidePreset() - Failed [%ls]", *InPresetName);
		return false;
	}

	LastAccessedGuidePresetName = InPresetName;
	UE_LOGF(AvaViewportLog, Log, "Motion Design Viewport Guide: SaveGuidePreset() - Success [%ls]", *InPresetName);
	return true;
}
	...
	const FVector2f InViewportSize)
{
	...
	{
		UE_LOGF(AvaViewportLog, Warning, "Motion Design Viewport Guide: LoadGuidePreset() - Failed [%ls]", *InPresetName);
		return false;
	}

	OutGuides = MoveTemp(LoadedGuides);
	LastAccessedGuidePresetName = InPresetName;
	UE_LOGF(AvaViewportLog, Log, "Motion Design Viewport Guide: LoadGuidePreset() - Success [%ls]", *InPresetName);
	return true;
}
