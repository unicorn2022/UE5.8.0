	...
	, const EBlendMode InBlendMode) const
{
	...
	if (const FAssetData AssetData = AssetRegistryModule.Get().GetAssetByObjectPath(FSoftObjectPath(OutputPath));
		AssetData.IsValid())
	{
		UE_LOGF(LogAvaMask, Warning, "Failed to load MIC at path '%ls' but the asset itself exists.", *OutputPath);
		return nullptr;
	}
#endif
...
#else
	UE_LOGF(LogAvaMask, Warning, "Can't create a MaterialInstanceConstant at runtime, ensure this is done at edit time first.")
	return nullptr;
#endif
}
