	...
	, const EBlendMode InBlendMode)
{
	...
	FAvaMaskMaterialPermutations* MaterialPermutations = MaterialInstanceCache.Find(InParentMaterial);
	if (!MaterialPermutations)
	{
		MaterialPermutations = &MaterialInstanceCache.Add(InParentMaterial, { InParentMaterial, {} });
	}
	check(MaterialPermutations);
	...
	{
		UE_LOGF(LogAvaMask, Warning, "No MID found for the given key, and MaterialInstanceFactories is empty. Returning nullptr.")
		return nullptr;
	}
	...
	{
		UE_LOGF(LogAvaMask, Warning, "No UAvaMaskMaterialFactory was found for class '%ls', using UMaterialInstanceDynamic::Create", *InParentMaterial->StaticClass()->GetName());
