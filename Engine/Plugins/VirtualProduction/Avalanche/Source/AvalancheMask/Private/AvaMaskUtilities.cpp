...
{
#if WITH_EDITOR
	UDMMaterialProperty* GetMaterialPropertyForBlendMode(TNotNull<UMaterialInterface*> InMaterial, EBlendMode InBlendMode)
	{
		UDynamicMaterialInstance* const Material = Cast<UDynamicMaterialInstance>(InMaterial);
		if (!Material)
		{
			return nullptr;
		}

		UDynamicMaterialModelEditorOnlyData* const EditorOnlyData = UDynamicMaterialModelEditorOnlyData::Get(Material->GetMaterialModel());
		if (!EditorOnlyData)
		{
			return nullptr;
		}

		const EDMMaterialPropertyType PropertyType = InBlendMode == EBlendMode::BLEND_Masked
			? EDMMaterialPropertyType::OpacityMask
			: EDMMaterialPropertyType::Opacity;

		return EditorOnlyData->GetMaterialProperty(PropertyType);
	}
#endif
	...
	UMaterialInterface* GetNonTransientParentMaterial(const UMaterialInstance* InMaterialInstance)
	{
		...
		if (!ParentAsset)
		{
			UE_LOGF(LogAvaMask, Warning, "Asset based parent material could not be found for '%ls', returning nullptr.", *InMaterialInstance->GetName());
			return nullptr;
		}
	...
	{
		UDMMaterialProperty* MaterialProperty = GetMaterialPropertyForBlendMode(InMaterial, InBlendMode);
		if (!MaterialProperty)
		{
			return;
		}

		const UMaterialFunctionInterface* CurrentOutputProcessor = MaterialProperty->GetOutputProcessor();
		if (CurrentOutputProcessor != InOutputProcessor)
		{
			MaterialProperty->SetOutputProcessor(InOutputProcessor);
		}
	}

	UMaterialFunctionInterface* GetOutputProcessor(TNotNull<UMaterialInterface*> InMaterial, EBlendMode InBlendMode)
	{
		if (UDMMaterialProperty* MaterialProperty = GetMaterialPropertyForBlendMode(InMaterial, InBlendMode))
		{
