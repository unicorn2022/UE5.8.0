...
public:
	...
	virtual bool SetMaskParameters(UTexture* InTexture, float InBaseOpacity, bool bInInverted = false, const FVector2f& InPadding = FVector2f::Zero(), bool bInApplyFeathering = false, float InOuterFeathering = 0.0f, float InInnerFeathering = 0.0f) = 0;
...
public:
	...
	virtual bool SetMaskParameters(UTexture* InTexture, float InBaseOpacity, bool bInInverted = false, const FVector2f& InPadding = FVector2f::Zero(), bool bInApplyFeathering = false, float InOuterFeathering = 0.0f, float InInnerFeathering = 0.0f) override;
	...
	, float InInnerFeathering)
{
	...
	{
		UE_LOGF(LogAvaMask, Verbose, "SetParameters: Material:%ls", *MaterialInstance->GetName());
...
bool TAvaMaskMaterialHandle<HandleDataType>::HasRequiredParameters(TArray<FString>& OutMissingParameterNames)
{
	...
	UE_LOGF(LogAvaMask, Warning, "HasRequiredParameters: Material was invalid.");
