...
TSharedPtr<IAvaObjectHandle> UAvaObjectHandleSubsystem::MakeHandleDirect(const FAvaMaskMaterialReference& InInstance, FName InTag)
{
	for (const TPair<FIsSupportedFunction, FMakeHandleFunction>& SupportedFactoryPair : ObjectHandleFactories)
	{
		if (SupportedFactoryPair.Key(InInstance, InTag))
		{
			...
			if (!UE::Ava::Internal::IsHandleValid(Handle))
			{
				if (InTag.IsNone())
				{
					UE_LOGF(LogAvaMask, Warning, "Object Handle for '%ls' was created but invalid.", *InInstance.ToString())
				}
				else
				{
					UE_LOGF(LogAvaMask, Warning, "Object Handle for '%ls' (with tag '%ls') was created but invalid.", *InInstance.ToString(), *InTag.ToString())
				}
			}
			return Handle;
		}
	...
	UE_LOGF(LogAvaMask, Verbose, "No ObjectHandle found for '%ls'", *InInstance.ToString());
	return nullptr;
}
