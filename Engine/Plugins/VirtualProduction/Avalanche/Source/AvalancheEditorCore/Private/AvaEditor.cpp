...
void FAvaEditor::Activate(TSharedPtr<IToolkitHost> InOverrideToolkitHost)
{
	...
	// Opt out of ITF gizmos and viewport interactions
	ModeTools.SetSupportsViewportITF(false);

	// Disable viewport interactions to allow legacy gizmos to receive input
	if (UModeManagerInteractiveToolsContext* ToolsContext = ModeTools.GetInteractiveToolsContext())
	{
		ToolsContext->SetViewportInteractionsEnabled(false);
	}
...
void FAvaEditor::Deactivate()
{
	...
	if (FEditorModeTools* const ModeTools = GetEditorModeTools())
	{
		...
		// Standard Level Editor default is to support ITF gizmos and interactions
		ModeTools->SetSupportsViewportITF(true);

		// Re-enable viewport interactions for the Level Editor
		if (UModeManagerInteractiveToolsContext* ToolsContext = ModeTools->GetInteractiveToolsContext())
		{
...
void FAvaEditor::RegisterTabSpawners()
{
	...
	{
		const TSharedRef<FWorkspaceItem> LevelEditorCategory = WorkspaceMenu::GetMenuStructure().GetLevelEditorCategory();

		RootCategory = LevelEditorCategory->AddGroup(TEXT("MotionDesignGroup")
			, LOCTEXT("RootCategoryLabel", "Motion Design")
			, FSlateIcon(FAvaEditorCoreStyle::Get().GetStyleSetName(), "Icons.MotionDesign")
			, /*bSortChildren*/false);

		OutlinerCategory = RootCategory->AddGroup(TEXT("MotionDesignOutlinerGroup")
			, LOCTEXT("OutlinerCategoryLabel", "Motion Design Outliner")
			, FSlateIcon()
			, /*bSortChildren*/true);

		ToolsCategory = RootCategory->AddGroup(TEXT("MotionDesignToolsGroup")
			, LOCTEXT("ToolsCategoryLabel", "Motion Design Tools")
			, FSlateIcon()
			, /*bSortChildren*/true);
	}
	...
	for (const TPair<FName, TSharedRef<IAvaTabSpawner>>& Pair : TabSpawners)
	{
		...
		{
			Pair.Value->RegisterTabSpawner(TabManagerRef, ToolsCategory);
		}
	}
}
...
void FAvaEditor::UnregisterTabSpawners()
{
	...
	if (RootCategory.IsValid() && FModuleManager::Get().IsModuleLoaded("WorkspaceMenuStructure"))
	{
		const TSharedRef<FWorkspaceItem> LevelEditorCategory = WorkspaceMenu::GetMenuStructure().GetLevelEditorCategory();
		LevelEditorCategory->RemoveItem(RootCategory.ToSharedRef());
		RootCategory.Reset();
	}
}
...
const TSharedPtr<FWorkspaceItem>& FAvaEditor::GetRootCategory() const
{
	return RootCategory;
}

const TSharedPtr<FWorkspaceItem>& FAvaEditor::GetOutlinerCategory() const
{
	return OutlinerCategory;
}

const TSharedPtr<FWorkspaceItem>& FAvaEditor::GetToolsCategory() const
{
