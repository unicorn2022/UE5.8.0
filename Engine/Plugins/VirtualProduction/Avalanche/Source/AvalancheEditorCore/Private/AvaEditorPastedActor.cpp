...
TMap<FName, AActor*> FAvaEditorPastedActor::BuildPastedActorMap(TConstArrayView<FAvaEditorPastedActor> InPastedActors, bool bIncludeDuplicatedActors)
{
	...
	for (const FAvaEditorPastedActor& PastedActor : InPastedActors)
	{
		...
		if (PastedActor.IsDuplicate())
		{
			...
			TemplateName = *PastedActor.GetTemplateActor()->GetPathName(PastedActor.GetTemplateActor()->GetWorld());
		}
		else
		{
			// If it comes from a Copy where the Template Actor is invalid, get its name via the sub path string
			TemplateName = *PastedActor.GetTemplateActor().ToSoftObjectPath().GetSubPathString();
		}
