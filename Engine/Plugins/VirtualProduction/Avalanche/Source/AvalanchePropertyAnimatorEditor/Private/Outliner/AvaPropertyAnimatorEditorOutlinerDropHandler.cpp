...
bool FAvaPropertyAnimatorEditorOutlinerDropHandler::DropAnimatorsOnActor(AActor* InActor, EItemDropZone InDropZone) const
{
	...
	UE_LOGF(LogAvaPropertyAnimatorEditorOutlinerDropHandler, Log, "Dropping %i animator(s) on actor %ls"
		, CloneAnimators.Num()
	...
	{
		UE_LOGF(LogAvaPropertyAnimatorEditorOutlinerDropHandler, Warning, "%ls : Could not clone all %i animators to target actor", *InActor->GetActorNameOrLabel(), CloneAnimators.Num());
	...
	if (!FSlateApplication::Get().GetModifierKeys().IsAltDown())
	{
		...
		{
			UE_LOGF(LogAvaPropertyAnimatorEditorOutlinerDropHandler, Warning, "Could not remove the %i cloned animators on source actor", CloneAnimators.Num());
