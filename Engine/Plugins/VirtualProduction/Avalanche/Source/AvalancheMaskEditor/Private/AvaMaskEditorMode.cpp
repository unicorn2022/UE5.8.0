	...
	if (InSelectionSet)
	{
		...
		if (const UTypedElementSelectionSet* SelectionSet = WeakActorSelectionSet.Get())
		{
			...
			if (const AActor* FirstActor = SelectionSet->GetTopSelectedObject<AActor>())
			{
				...
				{
					UGeometryMaskCanvas* const MaskCanvas = GetCanvasReferencedByActor(FirstActor);
					SelectedMaskCanvas = MaskCanvas;
					if (MaskCanvas)
					{
							...
							if (const AActor* WriterActor = WeakWriterActor.Get())
							{
								if (const IGeometryMaskWriteInterface* WriterObject = WriterActor->FindComponentByInterface<IGeometryMaskWriteInterface>())
								{
									if (UAvaGizmoComponent* GizmoComponent = WriterActor->GetComponentByClass<UAvaGizmoComponent>())
									{
										...
										GizmoComponent->SetsStencil(WriterCanvasName == MaskCanvas->GetCanvasName());
										GizmoComponent->SetStencilId(WriterCanvasName != MaskCanvas->GetCanvasName() ? 0 : 150);
									}
								}
							}
						}
						PreviewCanvasId = MaskCanvas->GetCanvasId();
					}
				}
			}
			else
...
UAvaMask2DBaseModifier* UAvaMaskEditorMode::FindOrAddMaskModifier(AActor* InActor, const TSubclassOf<UAvaMask2DBaseModifier>& InMaskModifierType)
{
	if (const UActorModifierCoreSubsystem* ModifierSubsystem = UActorModifierCoreSubsystem::Get())
	{
		...
		if (!MaskModifier)
		{
			...
			{
				UE_LOGF(LogAvaMaskEditor, Error, "Error inserting Mask modifier: %ls", *InsertOp.FailReason->ToString());
			}
			else
			{
				UE_LOGF(LogAvaMaskEditor, Error, "Error inserting Mask modifier.");
			}
		}
		else
		{
