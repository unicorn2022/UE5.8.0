...
FAvaTransitionExecutor::~FAvaTransitionExecutor()
{
	if (IsRunning())
	{
		...
		UE_LOGF(LogAvaTransitionExecutor, Warning
			, "FAvaTransitionExecutor '%p' (in Context %ls) has been destroyed while still running Behaviors!"
			, this
			, *ContextName);
	}
}
	...
	for (const TPair<FAvaTag, FLayerInfo>& Pair : TagLayerInfo)
	{
		...
		for (int32 ExitInstanceIndex : Pair.Value.ExitInstances)
		{
			...
			if (FAvaTransitionScene* TransitionScene = ExitInstance.GetTransitionContext().GetTransitionScene())
			{
				...
				const IAvaTransitionBehavior* TransitionBehavior = ExitInstance.GetBehavior();
				if (TransitionBehavior && TransitionBehavior->GetInstancingMode() == EAvaTransitionInstancingMode::Reuse)
				{
					// Scene reused if the Tree Instancing Mode is set to reuse, and if there is an Enter Instance with matching behavior (i.e. Level)
					bSceneReused |= Pair.Value.EnterInstances.ContainsByPredicate(
						[TransitionBehavior, &Instances = Instances](int32 InEnterInstanceIndex)
						{
							const FAvaTransitionBehaviorInstance& EnterInstance = Instances[InEnterInstanceIndex];
							return EnterInstance.GetBehavior() == TransitionBehavior;
						});
				}
...
void FAvaTransitionExecutor::DetermineTickWorld()
{
	WorldWeak.Reset();

	TArray<UWorld*, TInlineAllocator<1>> CandidateWorlds;

	// Gather all candidate worlds from the instances
	ForEachInstance(
		[&CandidateWorlds](FAvaTransitionBehaviorInstance& InInstance)
		{
			const FAvaTransitionScene* const TransitionScene = InInstance.GetTransitionContext().GetTransitionScene();
			if (!TransitionScene)
			{
				return;
			}
			const ULevel* const Level = TransitionScene->GetLevel();
			if (!Level)
			{
				return;
			}
			if (UWorld* const World = Level->GetWorld())
			{
				CandidateWorlds.AddUnique(World);
			}
		});
	...
	if (GEngine)
	{
		...
		{
			if (CandidateWorlds.Contains(WorldContext.World()))
			{
				WorldWeak = WorldContext.World();
				return;
			}
		}
	}

	// Fallback, no world in the list was found, use the first candidate.
	WorldWeak = CandidateWorlds[0];
}
...
void FAvaTransitionExecutor::Start()
{
	...
	{
		UE_LOGF(LogAvaTransitionExecutor, Error
			, "Trying to start an already-running FAvaTransitionExecutor '%p' (in Context %ls)!"
			, this
			, *ContextName);
		return;
	}
...
UWorld* FAvaTransitionExecutor::GetTickableGameObjectWorld() const
{
