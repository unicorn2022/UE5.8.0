...
void AAvaTransitionPreviewManager::TakeNext()
{
	...
	{
		UE_LOGF(LogAvaTransitionPreview, Error
			, "[%ls]: Failed to load level instance `%ls`."
...
void AAvaTransitionPreviewManager::TakeOut()
{
	...
	NextLevelState.OverrideTransitionLayer = TransitionBehavior->GetTransitionLayer();
...
void AAvaTransitionPreviewManager::TransitionStop()
{
	...
	{
		UE_LOGF(LogAvaTransitionPreview, Error
			, "Transition Executor %ls, but transition is marked as still in progress!"
		...
		ForEachObjectWithOuterBreakable(OuterWorld,
			[&bResetTransactionBuffer](UObject* InObject)
			{
				if (InObject && InObject->HasAnyFlags(RF_Transactional))
				{
					...
					UE_LOGF(LogAvaTransitionPreview
						, Warning
						, "Found RF_Transactional object '%ls' while unloading Level Instance."
