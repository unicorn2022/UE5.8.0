...
TScriptInterface<IAvaTransitionBehavior> UAvaTransitionLibrary::GetTransitionBehavior(UObject* InContextObject)
{
	using namespace UE::AvaTransition;

	if (const IAvaTransitionNodeInterface* NodeInterface = Cast<IAvaTransitionNodeInterface>(InContextObject))
	{
		if (IAvaTransitionBehavior* TransitionBehavior = NodeInterface->GetBehaviorInstanceCache().GetTransitionBehavior())
		{
			return &TransitionBehavior->AsUObject();
		}
	}
	...
	if (ULevel* Level = Private::GetLevelFromContextObject(InContextObject))
	{
		...
		{
			if (IAvaTransitionBehavior* TransitionBehavior = TransitionSubsystem->GetTransitionBehavior(Level))
			{
				return &TransitionBehavior->AsUObject();
			}
		}
...
{
	if (const IAvaTransitionBehavior* TransitionBehavior = UAvaTransitionLibrary::GetTransitionBehavior(InContextObject).GetInterface())
	{
		return TransitionBehavior->GetTransitionTree();
	}
	return nullptr;
}
