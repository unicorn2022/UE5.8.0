...
void FAvaOutliner::Refresh()
{
	...
	if (UAvaOutlinerSubsystem* const OutlinerSubsystem = GetOutlinerSubsystem())
	{
		const TArray<FActorHierarchyChange> ActorHierarchyChanges = MoveTemp(QueuedActorHierarchyChanges);
		QueuedActorHierarchyChanges.Empty();

		for (const FActorHierarchyChange& Change : ActorHierarchyChanges)
		{
			OutlinerSubsystem->BroadcastActorHierarchyChanged(Change.ActorWeak.Get(), Change.ParentWeak.Get(), Change.ChangeType);
		}
	}
}
...
void FAvaOutliner::QueueActorHierarchyChange(AActor* InActor, const AActor* InParentActor, EAvaOutlinerHierarchyChangeType InChangeType)
{
	FActorHierarchyChange& Change = QueuedActorHierarchyChanges.AddDefaulted_GetRef();
	Change.ActorWeak = InActor;
	Change.ParentWeak = InParentActor;
	Change.ChangeType = InChangeType;
}
