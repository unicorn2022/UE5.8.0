...
void FAvaOutlinerActor::OnChildrenAdded(TConstArrayView<FAvaOutlinerItemPtr> InAddedChildren, const FAvaOutlinerAddItemParams& InAddItemParams)
{
	...
	for (const FAvaOutlinerItemPtr& Item : InAddedChildren)
	{
		...
		if (AActor* const ChildActor = ActorItem->GetActor())
		{
			...
			FAvaOutliner& OutlinerPrivate = static_cast<FAvaOutliner&>(Outliner);
			OutlinerPrivate.QueueActorHierarchyChange(ChildActor, ParentActor, ChangeType);
			...
			if (ChildActor && ParentActor)
			{
				...
				if (ChildRoot && ChildRoot->GetAttachParent())
				{
					...
					if (OldParentActor == ParentActor)
					{
						...
						FAvaOutliner& OutlinerPrivate = static_cast<FAvaOutliner&>(Outliner);
						OutlinerPrivate.QueueActorHierarchyChange(ChildActor, ParentActor, EAvaOutlinerHierarchyChangeType::Detached);
					}
				}
			}
		}
