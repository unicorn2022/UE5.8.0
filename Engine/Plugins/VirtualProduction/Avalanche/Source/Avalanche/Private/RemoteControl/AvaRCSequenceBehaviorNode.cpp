...
void UAvaRCSequenceBehaviorNode::OnPassed(URCBehaviour* InBehavior) const
{
	...
	if (!Level)
	{
		UE_LOGF(LogAva, Error, "Sequence behavior failed. Behavior node %ls could not find a valid outer level.", *GetFullName())
		return;
	}
	...
	{
		UE_LOGF(LogAva, Error, "Sequence behavior failed. Behavior node %ls could not find a valid sequence subsystem.", *GetFullName())
		return;
	}
	...
	if (!PlaybackObject)
	{
		UE_LOGF(LogAva, Error, "Sequence behavior failed. Behavior node %ls could not find a valid playback object.", *GetFullName());
		return;
	}
	...
	default:
		UE_LOGF(LogAva, Error, "Sequence behavior failed. Behavior node %ls does not have a valid sequence action.", *GetFullName())
		break;
	}
}
