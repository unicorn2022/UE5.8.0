...
UAvaGizmoComponent::UAvaGizmoComponent()
{
	...
	{
		UE_LOGF(LogAva, Warning, "DefaultMaterial could not be loaded from path: '%ls'", DefaultMaterialPath);
	}
}
		...
		IsValid(Owner))
	{
		...
		ForEachPrimitiveComponent(
			[this](UPrimitiveComponent* InComponent)
			{
				...
				InComponent->bVisibleInReflections = false;
	...
	if (IsValid(Owner) && !Owner->IsActorBeingDestroyed() && !Owner->IsUnreachable())
	{
		...
		for (UPrimitiveComponent* Component : PrimitiveComponents)
		{
			...
			if (FAvaGizmoComponentPrimitiveValues* StoredComponentValues = ComponentValues.Find(ComponentReference))
			{
				...
				Component->bVisibleInReflections = StoredComponentValues->bVisibleInReflections;
				...
				{
					UE_LOGF(LogTemp, Warning, "Material count mismatch when restoring component '%ls' - got %i, expected %i",
						*Component->GetName(),
						ActualNumMaterials,
			...
			IsValid(Owner))
		{
			...
			ForEachPrimitiveComponent(
				[this, Owner, &bWasOneOrMorePrimitiveComponents](UPrimitiveComponent* InComponent)
				{
					...
					StoredComponentValues.bVisibleInReflections = InComponent->bVisibleInReflections;
			...
			{
				UE_LOGF(LogAva, Warning, "Actor '%ls' doesn't contain any components applicable to UAvaGizmoComponent", *Owner->GetName());
			}
		}
	}
}
