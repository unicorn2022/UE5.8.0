...
URemoteControlPreset* AAvaScene::GetOrCreateRemoteControlPreset(bool bInCallModify)
{
	if (!RemoteControlPreset)
	{
		...
		RemoteControlPreset = NewObject<URemoteControlPreset>(this);
		bRemoteControlPresetPresent = true;
		FAvaRemoteControlUtils::RegisterRemoteControlPreset(RemoteControlPreset, /*bInEnsureUniqueId*/ true);
	}
	return RemoteControlPreset;
}

URemoteControlPreset* AAvaScene::GetRemoteControlPreset() const
{
	return RemoteControlPreset;
}

void AAvaScene::DestroyRemoteControlPreset(bool bInCallModify)
{
	if (RemoteControlPreset)
	{
		if (bInCallModify)
		...
		bRemoteControlPresetPresent = false;
	}
}

bool AAvaScene::IsRemoteControlPresetExpectedToBePresent() const
{
	return bRemoteControlPresetPresent;
}
...
void AAvaScene::PostActorCreated()
{
	...
	// by default create an RC preset.
	GetOrCreateRemoteControlPreset();

	RegisterObjects();
}

void AAvaScene::PreInitializeComponents()
{
	Super::PreInitializeComponents();

	// Conditionally sort the items for faster lookup.
	SceneTree.SortItems();

	// Fill the object to node map for fast lookup
	SceneTree.ResolveObjects(GetTypedOuter<UWorld>());
}
...
void AAvaScene::PostLoad()
{
	...
	if (RemoteControlPreset)
	{
		RemoteControlPreset->ClearFlags(RF_DefaultSubObject);
	}
...
#if WITH_EDITOR
void AAvaScene::PostEditUndo()
{
	Super::PostEditUndo();

	// Ensure RC is registered
	FAvaRemoteControlUtils::RegisterRemoteControlPreset(RemoteControlPreset, /*bInEnsureUniqueId*/ true);
}
#endif
