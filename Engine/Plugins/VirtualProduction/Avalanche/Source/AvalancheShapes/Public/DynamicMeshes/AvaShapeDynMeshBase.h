	...
	, public IAvaGizmoObjectInterface
{
	...
	friend class UAvaShapeBuildSystem;
...
public:
	...
	AVALANCHESHAPES_API FAvaShapeMeshData* GetMeshData(int32 InMeshIndex);

	/** find a registered mesh and gets a pointer to it */
	const FAvaShapeMeshData* GetMeshData(int32 InMeshIndex) const;

	const TMap<int32, FAvaShapeMeshData>& GetMeshDatas() const
	{
		return MeshDatas;
	}
...
protected:
	...
	/**
	 * Determines whether shape mesh can be done from a different thread.
	 * All built-in meshes are safe to build in a different thread. This virtual would need to be overriden for any derived class that ensures thread safety.
	 */
	...
	void UpdateMesh(const FMeshUpdateContext& InUpdateContext);

	/** Offsets actor to account for the mesh generated location */
	void OffsetActorToAlign();

	/** Part of update flow that can only be performed safely on the game thread. Should be called before UpdateMesh_AnyThread */
	void PreUpdateMesh();

	/** Part of update flow that can only be performed safely on the game thread. Should be called after UpdateMesh_AnyThread */
	void PostUpdateMesh();
...
private:
	/** Gets the ticker to use for shapes */
	...
	/** the context while mesh is being built */
	TOptional<FMeshUpdateContext> MeshUpdateContext;
	...
	EAvaDynamicMeshUpdateState MeshUpdateStatus = EAvaDynamicMeshUpdateState::UpToDate;
