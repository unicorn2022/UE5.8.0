...
namespace UE::Private::AvaShape
{
	...
	bool EditMesh(const UAvaShapeDynamicMeshBase* InContext, const TFunctionRef<void(FDynamicMesh3&)>& InFunctor, EDynamicMeshChangeType InChangeType = EDynamicMeshChangeType::GeneralEdit, EDynamicMeshAttributeChangeFlags InAttributeChangeType = EDynamicMeshAttributeChangeFlags::Unknown)
	{
		...
		if (UDynamicMeshComponent* MeshComponent = InContext->GetShapeMeshComponent())
		{
			...
			{
				const bool bSkipBroadcastChangeEvents = true; // EditMesh never broadcasts any event. This is done by the UAvaShapeDynamicMeshBase on post mesh update
				Mesh->EditMesh(InFunctor, InChangeType, InAttributeChangeType, bSkipBroadcastChangeEvents);
				return true;
			}
		}
...
void UAvaShapeDynamicMeshBase::UpdateMesh(const FMeshUpdateContext& InUpdateContext)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UAvaShapeDynamicMeshBase::UpdateMesh);

	MeshUpdateContext = InUpdateContext;
	ON_SCOPE_EXIT
	{
		MeshUpdateContext.Reset();
	};
...
void UAvaShapeDynamicMeshBase::OffsetActorToAlign()
{
...
void UAvaShapeDynamicMeshBase::PreUpdateMesh()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UAvaShapeDynamicMeshBase::PreUpdateMesh_GameThread);
	...
	if (UDynamicMeshComponent* DynamicMeshComponent = GetShapeMeshComponent())
	{
		if (UDynamicMesh* DynamicMesh = DynamicMeshComponent->GetDynamicMesh())
		{
			DynamicMesh->OnPreMeshChanged().Broadcast(DynamicMesh, FDynamicMeshChangeInfo());
		}
	}
...
void UAvaShapeDynamicMeshBase::PostUpdateMesh()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UAvaShapeDynamicMeshBase::PostUpdateMesh);
	OffsetActorToAlign();
	...
	if (UDynamicMeshComponent* MeshComponent = GetShapeMeshComponent())
	{
		...
		auto UpdateBaseMaterial = [MeshComponent, &MeshIndex, &MeshMaterial]()
			{
				check(MeshIndex >= 0);
				if (MeshIndex >= MeshComponent->BaseMaterials.Num())
				{
					MeshComponent->BaseMaterials.SetNum(MeshIndex + 1, EAllowShrinking::No);
				}
				MeshComponent->BaseMaterials[MeshIndex] = MeshMaterial;
			};

		for (const TPair<int32, FAvaShapeMeshData>& MeshData : MeshDatas)
		{
			const FAvaShapeMesh& ShapeMesh = MeshData.Value.Mesh;
			MeshIndex = ShapeMesh.GetMeshIndex();
			MeshMaterial = GetMaterial(ShapeMesh.GetMeshIndex());
			UpdateBaseMaterial();
		}

		UE::ActorModifierCore::FRenderStateDirtyReasonScope Scope(UE::ActorModifierCore::ERenderStateDirtyReason::Material | UE::ActorModifierCore::ERenderStateDirtyReason::Geometry);

		if (MeshIndex != INDEX_NONE)
		{
			// Call SetMaterial on last index/material to mark render state dirty, update physical materials, etc.
			MeshComponent->SetMaterial(MeshIndex, MeshMaterial);
		}

		// No need to call MeshComponent->NotifyMeshModified(), as this is called by the delegate too (see UDynamicMeshComponent::UDynamicMeshComponent)
		if (UDynamicMesh* Mesh = MeshComponent->GetDynamicMesh())
		{
			Mesh->OnMeshChanged().Broadcast(Mesh, FDynamicMeshChangeInfo());
		}
...
void UAvaShapeDynamicMeshBase::RunUpdate()
{
	...
	auto ExecUpdate = [this](float=0.f)->bool
		{
			if (GetShapeMeshComponent())
			{
				PreUpdateMesh();

				const FMeshUpdateContext UpdateContext
					{
						.bParallel = false
					};
				UpdateMesh(UpdateContext);
	...
	if (GetRunAsync())
	{
		if (UAvaShapeBuildSystem* const BuildSystem = UAvaShapeBuildSystem::FindBuildSystem(this))
		{
			BuildSystem->QueueShapeBuild(this);
		}
		else
		{
			GetTicker().AddTicker(FTickerDelegate::CreateWeakLambda(this, ExecUpdate));	
		}
	}
	else
	{
...
{
	UDynamicMeshComponent* ShapeMeshComponent = CachedComponent.Get();

	// Only resolve shape mesh component when not in an update. Mesh Updates should've already validated this ahead of time in PreUpdateMesh
	if (!ShapeMeshComponent && !MeshUpdateContext.IsSet())
	{
		if (const AAvaShapeActor* ShapeActor = GetShapeActor())
		{
			ShapeMeshComponent = ShapeActor->GetShapeMeshComponent();
			if (ShapeMeshComponent)
			{
				UAvaShapeDynamicMeshBase* const MutableThis = const_cast<UAvaShapeDynamicMeshBase*>(this);
				MutableThis->CachedComponent = ShapeMeshComponent;
...
FTSTicker& UAvaShapeDynamicMeshBase::GetTicker() const
{
	if (const UWorld* const World = GetWorld())
	{
		if (UAvaSceneSubsystem* const SceneSubsystem = World->GetSubsystem<UAvaSceneSubsystem>())
		{
			return SceneSubsystem->GetTicker();
		}
	}
	UE_LOGF(LogAvaDynamicMesh, Verbose, "No valid Motion Design Scene Subsystem was found to retrieve ticker. Defaulting to Core Ticker.");
...
bool UAvaShapeDynamicMeshBase::GenerateTangents()
{
	...
	// Copied from UGeometryScriptLibrary_MeshNormalsFunctions::ComputeTangents to not broadcast any change
	UE::Private::AvaShape::EditMesh(this,
		[this](FDynamicMesh3& EditMesh)
		{
			constexpr int32 UVLayer = 0;
			if (!EditMesh.HasAttributes() || EditMesh.Attributes()->NumUVLayers() <= UVLayer || EditMesh.Attributes()->NumNormalLayers() == 0)
			{
				return;
			}
			...
			TangentOptions.bParallel = !MeshUpdateContext.IsSet() || !MeshUpdateContext->bParallel; // do not do parallel operation if already parallel.

			FMeshTangentsd Tangents(&EditMesh);
			Tangents.ComputeTriVertexTangents(
				EditMesh.Attributes()->PrimaryNormals(),
				EditMesh.Attributes()->GetUVLayer(UVLayer),
				TangentOptions);
...
bool UAvaShapeDynamicMeshBase::CreateDynamicMesh(FAvaShapeMesh& InMesh)
{
	...
	{
		UE_LOGF(LogAvaDynamicMesh, Warning, "CreateDynamicMesh %ls %i : VerticeIds %i, TriangleIds %i should be empty to create mesh", *GetMeshName(), InMesh.GetMeshIndex(), InMesh.VerticeIds.Num(), InMesh.TriangleIds.Num());
		return false;
	}
	...
	{
		UE_LOGF(LogAvaDynamicMesh, Warning, "CreateDynamicMesh %ls %i : Arrays Vertices %i, Normals %i should have same length, invalid array given", *GetMeshName(), InMesh.GetMeshIndex(), InMesh.Vertices.Num(), InMesh.Normals.Num());
		return false;
	}
	...
	{
		UE_LOGF(LogAvaDynamicMesh, Warning, "CreateDynamicMesh %ls %i : Triangles array should be multiple of 3, %i invalid array given", *GetMeshName(), InMesh.GetMeshIndex(), InMesh.Triangles.Num());
		return false;
	}
	...
	UE::Private::AvaShape::EditMesh(this,
		[this, &InMesh](FDynamicMesh3& Mesh)
		{
			...
			for (int32 t = 0; t < InMesh.Triangles.Num(); t += 3)
			{
				...
				{
					UE_LOGF(LogAvaDynamicMesh, Warning, "CreateDynamicMesh %ls %i : Invalid Vertice idx for triangle %i %i %i", *GetMeshName(), InMesh.GetMeshIndex(), Idx1, Idx2, Idx3);
					continue;
				}
				...
				if (TId < 0)
				{
					UE_LOGF(LogAvaDynamicMesh, Warning, "CreateDynamicMesh %ls %i : Invalid Triangle ID for mesh", *GetMeshName(), InMesh.GetMeshIndex());
					continue;
				}
...
bool UAvaShapeDynamicMeshBase::UpdateDynamicMesh(FAvaShapeMesh& InMesh)
{
	...
	if (!(InMesh.Vertices.Num() == InMesh.Normals.Num()))
	{
		UE_LOGF(LogAvaDynamicMesh, Warning, "UpdateDynamicMesh %ls %i: Arrays Vertices %i, Normals %i should have same length, invalid array given", *GetMeshName(), InMesh.GetMeshIndex(), InMesh.Vertices.Num(), InMesh.Normals.Num());
		return false;
	}
	...
	UE::Private::AvaShape::EditMesh(this,
		[this, InMesh](FDynamicMesh3& EditMesh)
		{
			...
			auto DoWork = [InMesh, &EditMesh, NormalOverlay](int32 Idx)
				{
					// vertices
			...
			if (MeshUpdateContext.IsSet() && MeshUpdateContext->bParallel)
			{
				for (int32 Idx = 0; Idx < InMesh.Vertices.Num(); ++Idx)
				{
					DoWork(Idx);
				}
			}
			...
			{
				ParallelFor(InMesh.Vertices.Num(), DoWork);
			}
		}
...
bool UAvaShapeDynamicMeshBase::ApplyUVsManually(FAvaShapeMesh& InMesh)
{
	...
	return UE::Private::AvaShape::EditMesh(this,
		[this, &InMesh](FDynamicMesh3& EditMesh)
		{
			...
			for (int32 TID : InMesh.TriangleIds)
			{
				...
				for (int32 j = 0; j < 3; ++j)
				{
					...
					{
						UE_LOGF(LogAvaDynamicMesh, Warning, "ApplyUVsManually %ls %i: vertice id %i for uv is invalid, skipping triangle", *GetMeshName(), InMesh.GetMeshIndex(), TriVtx[j]);
						break;
					}
					else
					{
...
bool UAvaShapeDynamicMeshBase::ApplyUVsTransform(FAvaShapeMesh& InMesh, FAvaShapeMaterialUVParameters& InParams, FVector2D ShapeSize, FVector2D UVOffset, float UVFixRotation)
{
	...
	return UE::Private::AvaShape::EditMesh(this,
		...
		, EDynamicMeshChangeType::AttributeEdit
		, EDynamicMeshAttributeChangeFlags::UVs) && bResult;
}
...
bool UAvaShapeDynamicMeshBase::RegisterMesh(FAvaShapeMeshData& NewMeshData)
{
	...
	{
		UE_LOGF(LogAvaDynamicMesh, Warning, "RegisterMesh %ls %i : Already contains mesh cannot add again", *GetMeshName(), NewMeshData.GetMeshIndex());
		return false;
	}
...
const FAvaShapeMeshData* UAvaShapeDynamicMeshBase::GetMeshData(int32 InMeshIndex) const
{
	return MeshDatas.Find(InMeshIndex);
}

FAvaShapeMeshData* UAvaShapeDynamicMeshBase::GetMeshData(int32 InMeshIndex)
{
	return MeshDatas.Find(InMeshIndex);
}
...
void UAvaShapeDynamicMeshBase::MarkAllMeshesDirty()
{
	for (TPair<int32, FAvaShapeMeshData>& MeshData : MeshDatas)
	{
		MeshData.Value.bMeshDirty = true;
	}
...
{
	const bool bResult = UE::Private::AvaShape::EditMesh(this,
		[this](FDynamicMesh3& EditMesh)
		{
