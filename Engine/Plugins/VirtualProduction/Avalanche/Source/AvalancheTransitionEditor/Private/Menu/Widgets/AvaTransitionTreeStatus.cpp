...
void SAvaTransitionTreeStatus::Construct(const FArguments& InArgs, IAvaTransitionBehavior* InTransitionBehavior)
{
...
{
	IAvaTransitionBehavior* const TransitionBehavior = TransitionBehaviorWeak.Get();
	if (TransitionBehavior && !TransitionBehavior->IsEnabled())
	{
...
SAvaTransitionTreeStatus::EStatus SAvaTransitionTreeStatus::GetStatus() const
{
	...
	return TransitionBehavior->IsEnabled()
		? EStatus::Enabled
		: EStatus::Disabled;
}
