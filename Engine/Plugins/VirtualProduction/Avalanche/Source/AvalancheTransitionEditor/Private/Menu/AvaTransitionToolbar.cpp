...
void FAvaTransitionToolbar::ExtendEditorToolbar(UToolMenu* InToolbarMenu)
{
	...
	UAvaTransitionTree* const TransitionTree = Owner.GetTransitionTree();

	IAvaTransitionBehavior* const TransitionBehavior = TransitionTree ? TransitionTree->GetImplementingOuter<IAvaTransitionBehavior>() : nullptr;

	// Tree Status
	if (TSharedPtr<SWidget> TreeStatusWidget = CreateTransitionStatusWidget(TransitionBehavior))
	{
		...
		AllowInReadOnly(Section.AddEntry(FToolMenuEntry::InitWidget(TEXT("TreeStatusWidget")
			, TreeStatusWidget.ToSharedRef()
			, FText::GetEmpty())));
	}

	// Layer Picker
	if (TSharedPtr<SWidget> LayerPicker = CreateTransitionLayerPicker(TransitionBehavior))
	{
