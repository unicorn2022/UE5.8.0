...
#include "Menu/Widgets/AvaTransitionTreeStatus.h"
...
{
	if (!InTransitionBehavior)
	{
		return nullptr;
	}
	return SNew(SAvaTransitionTreeStatus, InTransitionBehavior);
}
...
TSharedPtr<SWidget> CreateTransitionLayerPicker(IAvaTransitionBehavior* InTransitionBehavior)
{
	...
	TSharedPtr<ISinglePropertyView> PropertyView = PropertyEditorModule.CreateSingleProperty(&InTransitionBehavior->AsUObject()
		, TEXT("TransitionLayer")
...
TSharedPtr<SWidget> CreateTransitionInstancingModeSelector(IAvaTransitionBehavior* InTransitionBehavior)
{
	...
	return SNew(SHorizontalBox)
		...
		.VAlign(VAlign_Center)
		[
			SNew(SAvaTransitionTreeInstancingModeSelector, InTransitionBehavior)
		];
}
...
void ValidateTree(UAvaTransitionTree& InTransitionTree)
{
	...
	{
		UE_LOGF(LogAvaEditorTransition, Warning
			, "%ls: Resetting Evaluators due to current schema restrictions."
			, *TreeDebugName);
	...
	EditorData->VisitHierarchy([&TreeDebugName, EditorData](UStateTreeState& State, UStateTreeState*)
	{
		...
		{
			UE_LOGF(LogAvaEditorTransition, Warning
				, "%ls: Resetting Enter Conditions in state %ls due to current schema restrictions."
				, *TreeDebugName
				, *State.GetName());
		...
		if (!EditorData->Schema->AllowMultipleTasks())
		{
			if (!State.Tasks.IsEmpty())
			{
				State.Tasks.Reset();
				UE_LOGF(LogAvaEditorTransition, Warning
					, "%ls: Resetting Tasks in state %ls due to current schema restrictions."
					, *TreeDebugName
					, *State.GetName());
			}
		...
		else
		{
			if (State.SingleTask.Node.IsValid())
			{
				...
				UE_LOGF(LogAvaEditorTransition, Warning
					, "%ls: Resetting Single Task in state %ls due to current schema restrictions."
					, *TreeDebugName
					, *State.GetName());
			}
		}
...
{
	if (IAvaTransitionBehavior* const TransitionBehavior = InTransitionBehaviorWeak.Get())
	{
		...
		TransitionBehavior->SetEnabled(!TransitionBehavior->IsEnabled());
	}
}

bool IsTransitionTreeEnabled(TWeakInterfacePtr<IAvaTransitionBehavior> InTransitionBehaviorWeak)
{
	IAvaTransitionBehavior* const TransitionBehavior = InTransitionBehaviorWeak.Get();
	return TransitionBehavior && TransitionBehavior->IsEnabled();
}
