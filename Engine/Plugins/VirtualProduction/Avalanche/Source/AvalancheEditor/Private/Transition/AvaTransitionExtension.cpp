...
void FAvaTransitionExtension::ExtendSettingsCategory(IDetailCategoryBuilder& InCategoryBuilder)
{
	...
	{
		const TArray<UObject*> TransitionBehaviorObjects = { &TransitionBehavior->AsUObject() };

		InCategoryBuilder.SetDisplayName(LOCTEXT("CategoryDisplayName", "Transition Logic"));

		TransitionBehavior->ForEachDetailsEditableProperty(
			[&InCategoryBuilder, &TransitionBehaviorObjects](const IAvaTransitionBehavior::FPropertyContext& InPropertyContext)
			{
				InCategoryBuilder.AddExternalObjectProperty(TransitionBehaviorObjects, InPropertyContext.Name);
			});
	}
}
...
void FAvaTransitionExtension::OnSceneObjectCreated(UObject* InSceneObject)
{
	...
	IAvaTransitionBehavior* const TransitionBehavior = TransitionSubsystem->GetOrCreateTransitionBehavior();
	...
	TransitionBehavior->SetEnabled(true);
	TransitionBehavior->SetInstancingMode(EAvaTransitionInstancingMode::Reuse);
