...
namespace UE::AvaRundownServerTests
{
	...
	public:
		...
		void HandleRundownChannelImage(const FAvaRundownChannelImage& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
		{
			...
			UE_LOGF(LogAvaRundownServerTests, Log, "Received Rundown Channel Image successfully.");

			const FString SaveName = FString::Printf(TEXT("%s%s"), *FPaths::ProjectSavedDir(), TEXT("ChannelImageTest.jpeg"));
			UE_LOGF(LogAvaRundownServerTests, Display, "Saving Image to %ls", *SaveName);
	...
	TSharedPtr<IAvaRundownServer> GetOrCreateRundownServer()
	{
		...
		if (!RundownServer)
		{
			UE_LOGF(LogAvaRundownServerTests, Log, "Rundown Server not started. Starting one temporarily for the test.");
	...
	void BackupBroadcastConfig()
	{
		...
		UE_LOGF(LogAvaRundownServerTests, Log, "Broadcast config backed up.");
	}
	...
	void RestoreBroadcastConfig()
	{
		...
		{
			UE_LOGF(LogAvaRundownServerTests, Error, "Failed to restore broadcast config: backup file \"%ls\" doesn't exist.", *BackupFilepath);
			return;
		}
		...
		UE_LOGF(LogAvaRundownServerTests, Log, "Broadcast config restored.");
	}
}
...
bool FRundownServerWaitForResponse::Update()
{
	...
	{
		UE_LOGF(LogAvaRundownServerTests, Error, "Test Client is not valid.");
		return true;
	}
...
bool FRundownServerAddChannelDevice::RunTest(const FString& Parameters)
{
	...
	if (OutputDevices->GetChildren().Num() > 0)
	{
		...
		if (ExistingOutput.IsValid())
		{
			...
			UE_LOGF(LogAvaRundownServerTests, Log, "Selected media output for test: \"%ls\".", *Request->MediaOutputName);
		}
	}
...
bool FRundownServerMediaOutputSerialization::RunTest(const FString& Parameters)
{
	for (const UClass* const Class : TObjectRange<UClass>())
	{
		...
		if (bIsMediaOutputClass)
		{
			...
			UE_LOGF(LogAvaRundownServerTests, Display, "Serializing Media Output \"%ls\" to \"%ls\"", *Class->GetName(), *SaveName);
