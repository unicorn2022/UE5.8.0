...
void SAvaRundownInstancedPageList::BindCommands()
{
	...
	//Rundown Commands
	{
		...
		CommandList->MapAction(RundownCommands.LoadAllPages,
			FExecuteAction::CreateSP(this, &SAvaRundownInstancedPageList::LoadAllPages),
			FCanExecuteAction::CreateSP(this, &SAvaRundownInstancedPageList::CanLoadAllPages));

		CommandList->MapAction(RundownCommands.LoadNextPages,
			FExecuteAction::CreateSP(this, &SAvaRundownInstancedPageList::LoadNextPages),
			FCanExecuteAction::CreateSP(this, &SAvaRundownInstancedPageList::CanLoadNextPages));

		CommandList->MapAction(RundownCommands.LoadSelectedPages,
			FExecuteAction::CreateSP(this, &SAvaRundownInstancedPageList::LoadSelectedPages),
			FCanExecuteAction::CreateSP(this, &SAvaRundownInstancedPageList::CanLoadSelectedPages));
		...
		if (FPathViews::GetExtension(File).Equals(TEXT("json"), ESearchCase::IgnoreCase))
		{
			...
			if (LoadRundownFromJson(TmpRundown.Get(), *File, ErrorMessage))
			{
				if (!TmpRundown->GetInstancedPages().Pages.IsEmpty())
				{
					...
					{
						UE_LOGF(LogAvaRundown, Error, "Failed to merge %ls in current rundown.", *File);	
					}
				}
				else
				{
					UE_LOGF(LogAvaRundown, Warning, "Merging %ls has no instanced pages to merge.", *File);
				}
			}
			else
			{
				UE_LOGF(LogAvaRundown, Error, "Failed to load rundown from file \"%ls\". Reason: %ls.", *File, *ErrorMessage.ToString());
			}
		}
		else
		{
			UE_LOGF(LogAvaRundown, Error, "%ls is not a supported file type. Only rundowns in \"json\" format are supported.", *File);
		}
	}
...
void SAvaRundownInstancedPageList::PlaySelectedPage() const
{
	if (UAvaRundown* Rundown = GetRundown())
	{
		...
		Rundown->PlayPages(PageIds, EAvaRundownPagePlayType::PlayFromStart, EAvaRundownPagePlaySource::Editor);
	}
}
...
bool SAvaRundownInstancedPageList::CanLoadAllPages() const
{
	return CanLoadPages(GetAllPageIds());
}

void SAvaRundownInstancedPageList::LoadAllPages()
{
	LoadPages(GetAllPageIds());
}

bool SAvaRundownInstancedPageList::CanLoadNextPages() const
{
	const int32 NextPageId = GetNextPageIdToLoad();
	return CanLoadPages(MakeArrayView(&NextPageId, 1));
}

void SAvaRundownInstancedPageList::LoadNextPages()
{
	const int32 NextPageId = GetNextPageIdToLoad();
	LoadPages(MakeArrayView(&NextPageId, 1));
}

bool SAvaRundownInstancedPageList::CanLoadSelectedPages() const
{
	return CanLoadPages(SelectedPageIds);
}

void SAvaRundownInstancedPageList::LoadSelectedPages()
{
	LoadPages(SelectedPageIds);
}

bool SAvaRundownInstancedPageList::CanLoadPages(TConstArrayView<int32> InPageIds) const
{
	...
	for (int32 PageId : InPageIds)
	{
		if (Rundown->CanPlayPage(PageId, bIsPreview))
		{
			return true;
		}
	}
...
void SAvaRundownInstancedPageList::LoadPages(TConstArrayView<int32> InPageIds)
{
	UAvaRundown* const Rundown = GetRundown();
	if (!Rundown)
	{
		return;
	}
	...
	constexpr bool bIsPreview = false;
	const FName PreviewChannel = NAME_None;

	IAvaRundownPageLoadingManager& PageLoadingManager = Rundown->GetPageLoadingManager();
	for (int32 PageId : InPageIds)
	{
		if (Rundown->CanPlayPage(PageId, bIsPreview))
		{
			PageLoadingManager.RequestLoadPage(PageId, bIsPreview, PreviewChannel);
		}
	}
}
...
TArray<int32> SAvaRundownInstancedPageList::PlayNextPage() const
{
	...
	if (UAvaRundown* Rundown = GetRundown())
	{
		...
		{
			Rundown->PlayPage(PageIdToTakeNext, EAvaRundownPagePlayType::PlayFromStart, EAvaRundownPagePlaySource::Editor);
...
TArray<int32> SAvaRundownInstancedPageList::GetAllPageIds() const
{
	TArray<int32> PageIds;
	PageIds.Reserve(PageViews.Num());
	for (const FAvaRundownPageViewPtr& PageView : PageViews)
	{
		if (PageView.IsValid())
		{
			PageIds.Add(PageView->GetPageId());
		}
	}
	return PageIds;
}
...
int32 SAvaRundownInstancedPageList::GetNextPageIdToLoad() const
{
	const UAvaRundown* const Rundown = GetRundown();
	if (!Rundown)
	{
	...
	// Try to use the program context if available. If not playing program, use preview loading
	const FAvaRundownPageListPlaybackContextCollection* const ContextCollection = Rundown->GetPageListPlaybackContextCollection();
	const bool bUseProgramContext = ContextCollection && ContextCollection->GetContext(/*bPreview*/false, /*PreviewChannel*/NAME_None).IsValid();

	return bUseProgramContext ? GetPageIdToTakeNext() : GetPageIdToPreviewNext();
}
