...
bool FAvaRundownEditor::CanLoadAllPages() const
{
	const TSharedPtr<SAvaRundownInstancedPageList> ActivePageList = GetActiveListWidget();
	return ActivePageList && ActivePageList->CanLoadAllPages();
}

void FAvaRundownEditor::LoadAllPages()
{
	if (const TSharedPtr<SAvaRundownInstancedPageList> ActivePageList = GetActiveListWidget())
	{
		ActivePageList->LoadAllPages();
	}
}

bool FAvaRundownEditor::CanLoadNextPages() const
{
	const TSharedPtr<SAvaRundownInstancedPageList> ActivePageList = GetActiveListWidget();
	return ActivePageList && ActivePageList->CanLoadNextPages();
}

void FAvaRundownEditor::LoadNextPages()
{
	if (const TSharedPtr<SAvaRundownInstancedPageList> ActivePageList = GetActiveListWidget())
	{
		ActivePageList->LoadNextPages();
	}
}

bool FAvaRundownEditor::CanLoadSelectedPages() const
{
	const TSharedPtr<SAvaRundownInstancedPageList> ActivePageList = GetActiveListWidget();
	return ActivePageList && ActivePageList->CanLoadSelectedPages();
}

void FAvaRundownEditor::LoadSelectedPages()
{
	if (const TSharedPtr<SAvaRundownInstancedPageList> ActivePageList = GetActiveListWidget())
	{
		ActivePageList->LoadSelectedPages();
	}
}
...
void FAvaRundownEditor::HandleOnPagePlayerAdded(UAvaRundown* InRundown, UAvaRundownPagePlayer* InPagePlayer)
{
	...
	if (InPagePlayer->bIsPreview)
	{
		...
		if (Settings && Settings->bAutoStartPreviewBroadcastChannel)
		{
			...
			{
				UE_LOGF(LogAvaBroadcast, Warning,
					"Start Broadcast failed: Channel \"%ls\" of Profile \"%ls\" is not valid.",
...
void FAvaRundownEditor::CreateRundownCommands()
{
	//Rundown Commands
	{
		...
		ToolkitCommands->MapAction(RundownCommands.LoadAllPages,
			FExecuteAction::CreateSP(this, &FAvaRundownEditor::LoadAllPages),
			FCanExecuteAction::CreateSP(this, &FAvaRundownEditor::CanLoadAllPages));

		ToolkitCommands->MapAction(RundownCommands.LoadNextPages,
			FExecuteAction::CreateSP(this, &FAvaRundownEditor::LoadNextPages),
			FCanExecuteAction::CreateSP(this, &FAvaRundownEditor::CanLoadNextPages));

		ToolkitCommands->MapAction(RundownCommands.LoadSelectedPages,
			FExecuteAction::CreateSP(this, &FAvaRundownEditor::LoadSelectedPages),
			FCanExecuteAction::CreateSP(this, &FAvaRundownEditor::CanLoadSelectedPages));
...
void FAvaRundownEditor::StartAutoPlayCommand(const TArray<FString>& InArgs)
{
	...
	UE_LOGF(LogAvaRundown, Log, "Rundown auto play started, interval: %f seconds.", PlayInterval);
...
void FAvaRundownEditor::LoadPageCommand(const TArray<FString>& InArgs)
{
	...
	if (!Errors.IsEmpty())
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in Load Page command: %ls", *Errors);
	}
	...
	auto LoadPage = [Rundown](int32 InPageId, bool bInIsPreview, FName InChannelName)
	{
		...
		{
			UE_LOGF(LogAvaRundown, Display, "Loaded page %d for channel \"%ls\".", InPageId, *InChannelName.ToString());
		}
	};
...
void FAvaRundownEditor::UnloadPageCommand(const TArray<FString>& InArgs)
{
	...
	if (!Errors.IsEmpty())
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in Unload Page command: %ls", *Errors);
	}
	...
	auto UnloadPage = [Rundown](int32 InPageId, const FName& InChannelName)
	{
		...
		{
			UE_LOGF(LogAvaRundown, Display, "Unloaded page %d for channel \"%ls\".", InPageId, *InChannelName.ToString());
		}
	};
...
void FAvaRundownEditor::PlayPageCommand(const TArray<FString>& InArgs, EAvaRundownPagePlayType InPlayType)
{
	const bool bPreview = UE::AvaRundown::IsPreviewPlayType(InPlayType);

	FString Errors;
	const TArray<int32> PageIds = ArgumentsToPageIds(InArgs, bPreview, Errors);

	if (!Errors.IsEmpty())
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in Play Page command: %ls", *Errors);
	}
	...
	{
		Rundown->PlayPages(PageIds, InPlayType, EAvaRundownPagePlaySource::Editor);
	}
}
...
void FAvaRundownEditor::ContinuePageCommand(const TArray<FString>& InArgs, bool bInPreview)
{
	...
	if (!Errors.IsEmpty())
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in Continue Page command: %ls", *Errors);
	}
...
void FAvaRundownEditor::PlayNextPageCommand(const TArray<FString>& InArgs, bool bInPreview)
{
	...
	if (!AvaRundown.IsValid())
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in Continue Page command: invalid rundown.");
		return;
	}
	...
	{
		Rundown->PlayPage(NextPageId, bInPreview ? EAvaRundownPagePlayType::PreviewFromFrame : EAvaRundownPagePlayType::PlayFromStart, EAvaRundownPagePlaySource::Editor);
	}
}
...
void FAvaRundownEditor::StopPageCommand(const TArray<FString>& InArgs, bool bInPreview, bool bInForce)
{
	...
	if (!Errors.IsEmpty())
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in \"Stop Page\" command: %ls", *Errors);
	}
...
void FAvaRundownEditor::StopLayersCommand(const TArray<FString>& InArgs, bool bInPreview, bool bInForce)
{
	...
	if (InArgs.Num() < 2)
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in \"Stop Layers\" command: Too few arguments, need at least ChannelName and one layer name");
		return;
	}
	...
	{
		UE_LOGF(LogAvaRundown, Log, "Stopping layers: %ls in channel %ls.", *FString::JoinBy(Layers.Value, TEXT(", "), [](const FAvaTagHandle& InTag){return InTag.ToString();}), *Layers.Key.ToString());
...
void FAvaRundownEditor::TakeToProgramCommand(const TArray<FString>& InArgs)
{
	...
	if (!Errors.IsEmpty())
	{
		UE_LOGF(LogAvaRundown, Error, "Errors in \"Take To Program\" command: %ls", *Errors);
	}
	...
	{
		Rundown->PlayPages(PageIdsToPlay, EAvaRundownPagePlayType::PlayFromStart, EAvaRundownPagePlaySource::Editor);
	}
}
...
void FAvaRundownEditor::StopPageTransitionCommand(const TArray<FString>& InArgs)
{
	...
	if (!Rundown)
	{
		UE_LOGF(LogAvaRundown, Error, "No valid rundown available.");
		return;
	}
	...
	for (const FString& Argument : InArgs)
	{
		for (const FString& ChannelName : ChannelNames)
		{
			if (ChannelName == Argument || ChannelName.Contains(Argument))
			{
				...
				{
					UE_LOGF(LogAvaRundown, Log, "Stopped %d transition(s) on channel %ls.", NumTransitionsStopped, *ChannelName);
				}
			}
		}
	}
...
void FAvaRundownEditor::StartChannelCommand(const TArray<FString>& InArgs)
{
	...
	for (const FString& Argument : InArgs)
	{
		...
		{
			UE_LOGF(LogAvaRundown, Error, "Argument \"%ls\" is not a valid channel name.", *Argument);
		}
	}
}
...
const FAvaRundownEditor::FBindableMacroCommands& FAvaRundownEditor::GetBindableMacroCommands()
{
	if (BindableMacroCommands.IsEmpty())
	{
		...
		BindableMacroCommands.Add(GetName(EAvaRundownEditorMacroCommand::TakeIn), [this](const TArray<FString>& InArgs){PlayPageCommand(InArgs, EAvaRundownPagePlayType::PlayFromStart);});
		...
		BindableMacroCommands.Add(GetName(EAvaRundownEditorMacroCommand::PreviewIn), [this](const TArray<FString>& InArgs){PlayPageCommand(InArgs, EAvaRundownPagePlayType::PreviewFromStart);});
		...
		BindableMacroCommands.Add(GetName(EAvaRundownEditorMacroCommand::PreviewFrame), [this](const TArray<FString>& InArgs){PlayPageCommand(InArgs, EAvaRundownPagePlayType::PreviewFromFrame);});
	}
	...
	, PlayInterval(InTickInterval)
{
	...
	if (const TSharedPtr<FAvaRundownEditor> RundownEditor = RundownEditorWeak.Pin())
	{
		...
		if (Rundown && Rundown->GetInstancedPages().Pages.Num() > 0)
		{
			...
			{
				Rundown->PlayPage(PageId.Value, EAvaRundownPagePlayType::PlayFromStart, EAvaRundownPagePlaySource::Editor);
			}
		}
	}
}
...
void FAvaRundownEditor::FAutoPlayTicker::Tick(float DeltaTime)
{
	...
	if (!Rundown)
	{
		UE_LOGF(LogAvaRundown, Error, "Invalid rundown. Cancelling auto play.");
		...
		for (const int32 SelectedPageId : SelectedPagesId)
		{
			...
			if (SelectedPage && SelectedPage->IsValidPage())
			{
				...
				for (const FAvaRundownChannelPageStatus& Status : Statuses)
				{
					...
					{
						UE_LOGF(LogAvaRundown, Error, "Playback error. Cancelling auto play.");
						RundownEditor->CancelAutoPlay();
						return;
					}
				}
			...
			{
				UE_LOGF(LogAvaRundown, Error, "Current Page is Invalid. Cancelling auto play.");
				RundownEditor->CancelAutoPlay();
				return;
			}
		}
