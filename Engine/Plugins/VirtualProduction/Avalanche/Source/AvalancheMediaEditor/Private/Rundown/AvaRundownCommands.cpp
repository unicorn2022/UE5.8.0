...
void FAvaRundownCommands::RegisterCommands()
{
	...
	UI_COMMAND(LoadAllPages
		, "Load All Pages"
		, "Loads all the pages for play in their set channels"
		, EUserInterfaceActionType::Button
		, FInputChord(EKeys::L, EModifierKey::Control | EModifierKey::Shift | EModifierKey::Alt));

	UI_COMMAND(LoadNextPages
		, "Load Next Pages"
		, "Loads the next pages in program for play in its set channels"
		, EUserInterfaceActionType::Button
		, FInputChord(EKeys::L, EModifierKey::Control | EModifierKey::Shift));

	UI_COMMAND(LoadSelectedPages
		, "Load Selected Pages"
		, "Loads the selected pages for play in their set channels"
		, EUserInterfaceActionType::Button
