	...
	static void ExportToJson(const FToolMenuContext& InContext)
	{
		...
		for (UAvaRundown* Rundown : Context->LoadSelectedObjects<UAvaRundown>())
		{
			if (Rundown)
			{
				...
				{
					UE_LOGF(LogAvaRundown, Error, "Package disallow export.");
					continue;
				}
		...
		for (UAvaRundown* Rundown : Context->LoadSelectedObjects<UAvaRundown>())
		{
			if (Rundown)
			{
				...
				if (Rundown->IsPlaying())
				{
					...
					{
						UE_LOGF(LogAvaRundown, Warning, "Skipping import of rundown \"%ls\"", *Rundown->GetFullName());
						continue;
					}
				...
				if (!ImportFilename.IsEmpty())
				{
					...
					{
						UE_LOGF(LogAvaRundown, Error,
							"Failed to import rundown \"%ls\" from file \"%ls\". Reason: %ls",
