...
namespace UE::AvaRundownEditor::Utils::Private
{
	...
	bool CopyPageInPlace(UAvaRundown* InRundown, int32 InPageId, const FAvaRundownPage& InSourcePage, int32 InTemplateId)
	{
		...
		UE_LOGF(LogAvaRundown, Error, "Failed to copy page in place: page id %d is not found in destination rundown.", InPageId);
		return false;
	}
}
...
TArray<FAvaRundownPage> UE::AvaRundownEditor::Utils::DeserializePagesFromJson(const FString& InJsonString)
{
	...
	{
		UE_LOGF(LogAvaRundown, Warning, "Unable to serialize the pasted text into Json format");
		return {};
	}
	...
	{
		UE_LOGF(LogAvaRundown, Warning, "Missing %ls entry field in pasted text", *Private::PageEntriesName);
		return {};
	}
	...
	for (const TSharedPtr<FJsonValue>& PageEntry : *PageEntries)
	{
		...
		{
			UE_LOGF(LogAvaRundown, Warning, "Invalid page entry. Not an object");
			continue;
		}
		...
		{
			UE_LOGF(LogAvaRundown, Warning, "Unable to convert Page Entry Json Object to Motion Design Page Struct");
		}
	}
	return Pages;
}
		...
		if (URemoteControlPreset* Preset = Instance->GetRemoteControlPreset())
		{
			...
			for (const URCVirtualPropertyBase* Controller : Preset->GetControllers())
			{
				if (!AllControllerValuesBackup.SetControllerValue(Controller->Id, Preset, /*bInIsDefault*/ false))
				{
					...
					UE_LOGF(LogAvaRundown, Error, "Unable to backup preset controllers. Aborting page summary update.");
					return false;
				}
			}
