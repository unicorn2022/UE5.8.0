...
void SAvaRundownPageList::CopySelectedPages()
{
	...
	if (!IsValid(Rundown))
	{
		UE_LOGF(LogAvaRundown, Error, "Can't copy from invalid rundown.");
		return;
	}
...
void SAvaRundownPageList::ExportSelectedPagesToExternalFile(const TCHAR* InType)
{
	...
	if (const TStrongObjectPtr<UAvaRundown> NewRundown = ExportPagesToRundown(SourceRundown, SelectedPageIds))
	{
			...
			if (!ExportFilename.IsEmpty())
			{
				...
				if (!SaveRundownToJson(NewRundown.Get(), *ExportFilename, ErrorMessage))
				{
					UE_LOGF(LogAvaRundown, Error,
						"Failed to export rundown \"%ls\" to file \"%ls\". Reason: %ls",
		...
		{
			UE_LOGF(LogAvaRundown, Error, "Export Pages to external file doesn't support type \"%ls\".", InType);
		}
	}
}
...
void SAvaRundownPageList::PreviewPlaySelectedPage(bool bInToMark) const
{
	...
	{
		const TArray<int32> PageIds = GetPagesToPreviewIn();
		Rundown->PlayPages(PageIds, bInToMark ? EAvaRundownPagePlayType::PreviewFromFrame : EAvaRundownPagePlayType::PreviewFromStart, EAvaRundownPagePlaySource::Editor);
	}
}
...
void SAvaRundownPageList::PreviewPlayNextPage()
{
	if (UAvaRundown* Rundown = GetRundown())
	{
		...
		{
			Rundown->PlayPage(PageIdToPreviewNext, EAvaRundownPagePlayType::PreviewFromStart, EAvaRundownPagePlaySource::Editor);
			DeselectPages();
	...
	static bool IsAvaAsset(const FAssetData& InAssetData, bool bInLogInfo)
	{
		...
		if (AssetType == EMotionDesignAssetType::World)
		{
			...
			if (!SceneTag.IsSet())
			{
				if (bInLogInfo)
				{
					UE_LOGF(LogAvaRundown, Display,
						"Level Asset \"%ls\" is not an Motion Design Scene (Asset Tag not found).",
			...
			if (!SceneTag.Equals(AssetTagValueEnabled))
			{
				...
				{
					UE_LOGF(LogAvaRundown, Display,
						"Level Asset \"%ls\" is an Motion Design Scene but not enabled.",
						*InAssetData.GetSoftObjectPath().ToString());
				}
				return false;						
			}
