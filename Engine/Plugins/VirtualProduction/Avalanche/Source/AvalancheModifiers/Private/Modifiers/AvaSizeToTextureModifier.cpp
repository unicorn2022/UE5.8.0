...
void UAvaSizeToTextureModifier::SavePreState()
{
	if (UAvaShape2DDynMeshBase* const Shape2D = Shape2DWeak.Get())
	{
		// Skip saving pre-state for Motion Design 2D Shapes as the mesh isn't dealt with directly in Apply.
		// @see UAvaSizeToTextureModifier::RestorePreState for more details
	}
	else
	{
		Super::SavePreState();
...
void UAvaSizeToTextureModifier::RestorePreState()
{
	if (UAvaShape2DDynMeshBase* const Shape2D = Shape2DWeak.Get())
	{
		// Don't restore the mesh when dealing with Motion Design Shapes.
		// Apply for 2D shapes only modifies the Size2D of the shape, not the mesh directly.
		// Size2D isn't saved/restore either as this modifier doesn't rely on the Size2D previous state. It only restores back the original size when disabled.
	}
	else
	{
