...
void UAvaTranslucentPriorityModifier::Apply()
{
	...
	if (Mode == EAvaTranslucentPriorityModifierMode::Manual)
	{
		...
		{
			if (SortedComponentState.Modifier == this)
			{
				if (UPrimitiveComponent* const Component = SortedComponentState.PrimitiveComponent)
				{
	...
	else
	{
		...
		for (const FAvaTranslucentPriorityModifierComponentState& SortedComponentState : CachedSortedComponentStates)
		{
			UPrimitiveComponent* const Component = SortedComponentState.PrimitiveComponent;
			if (!Component || !Component->GetOwner() || !SortedComponentState.Modifier)
			{
				continue;
			}
			...
			else if (Component->TranslucencySortPriority != TranslucentSortPriority)
			{
				...
				if (SortedComponentState.Modifier->CachedSortedComponentStates.IsEmpty())
				{
					SortedComponentState.Modifier->CachedSortedComponentStates = CachedSortedComponentStates;
				}
				SortedComponentState.Modifier->MarkModifierDirty();
			}
		}
	}
...
UE::ActorModifierCore::ERenderStateDirtyReason UAvaTranslucentPriorityModifier::GetRenderStateDirtyEventTypes() const
{
	return UE::ActorModifierCore::ERenderStateDirtyReason::TranslucencyPriority
		| UE::ActorModifierCore::ERenderStateDirtyReason::SceneTree;
}
