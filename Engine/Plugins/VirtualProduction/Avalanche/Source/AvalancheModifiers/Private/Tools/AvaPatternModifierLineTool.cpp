...
void UAvaPatternModifierLineTool::PostEditChangeProperty(FPropertyChangedEvent& InEvent)
{
	...
	if (bAnyPropertyChanged || MemberName == GET_MEMBER_NAME_CHECKED(UAvaPatternModifierLineTool, LineCount))
	{
		SetLineCount(LineCount);
	}

	if (bAnyPropertyChanged || LinePropertyNames.Contains(MemberName))
	{
		OnToolPropertiesChanged();
	}
}
