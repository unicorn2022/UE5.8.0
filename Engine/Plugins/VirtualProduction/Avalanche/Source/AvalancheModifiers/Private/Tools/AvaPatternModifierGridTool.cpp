...
void UAvaPatternModifierGridTool::PostEditChangeProperty(FPropertyChangedEvent& InEvent)
{
	...
	const bool bAnyPropertyChanged = MemberName.IsNone();

	if (bAnyPropertyChanged || MemberName == GET_MEMBER_NAME_CHECKED(UAvaPatternModifierGridTool, GridCountX))
	{
		SetGridCountX(GridCountX);
	}
	if (bAnyPropertyChanged || MemberName == GET_MEMBER_NAME_CHECKED(UAvaPatternModifierGridTool, GridCountY))
	{
		SetGridCountY(GridCountY);
	}

	if (bAnyPropertyChanged || GridPropertyNames.Contains(MemberName))
	{
		OnToolPropertiesChanged();
	}
}
