...
void UAvaPatternModifierCircleTool::PostEditChangeProperty(FPropertyChangedEvent& InEvent)
{
	...
	if (bAnyPropertyChanged || MemberName == GET_MEMBER_NAME_CHECKED(UAvaPatternModifierCircleTool, CircleCount))
	{
		SetCircleCount(CircleCount);
	}

	if (bAnyPropertyChanged || CirclePropertyNames.Contains(MemberName))
	{
		OnToolPropertiesChanged();
	}
}
