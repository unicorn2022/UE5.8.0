...
void FAvaMediaModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FAvaMediaModule::PostEngineInit);
	...
	if (IsPlaybackServerManuallyStarted(DummyServerName) || IsRundownServerManuallyStarted(DummyServerName))
	{
		...
		if (IConsoleVariable* AllowThrottling = IConsoleManager::Get().FindConsoleVariable(TEXT("Slate.bAllowThrottling")))
		{
			...
			UE_LOGF(LogAvaMedia, Log, "Setting Slate.bAllowThrottling to false.");
		}
	}
...
void FAvaMediaModule::LaunchGameModeLocalPlaybackServer()
{
	...
	if (!IsPlaybackClientStarted())
	{
		...
		UE_LOGF(LogAvaMedia, Error, "Playback Client must be started prior to starting a local playback server.");
		return;
...
void FAvaMediaModule::StopGameModeLocalPlaybackServer()
{
	...
	if (LocalPlaybackServerProcess.IsValid() && LocalPlaybackServerProcess->IsLaunched())
	{
		...
		UE_LOGF(LogAvaMedia, Log, "Local playback server has been stopped.");
	}
...
void FAvaMediaModule::PostEngineInit()
{
	...
	if (bShouldStartServer && bShouldStartClient)
	{
		if (bIsGameMode)
		{
			UE_LOGF(LogAvaMedia, Log, "Auto start of Playback Client has been suppressed in game mode in favor of Playback Server.");
			bShouldStartClient = false; // In game mode, the client is suppressed.

		}
		else
		{
			UE_LOGF(LogAvaMedia, Log, "Auto start of Playback Server has been suppressed in editor mode in favor of Playback Client.");
...
void FAvaMediaModule::StartRundownServerCommand(const TArray<FString>& Args)
{
	if (RundownServer)
	{
		UE_LOGF(LogAvaMedia, Log, "Rundown Server is already started.");
		return;
	}
	...
	UE_LOGF(LogAvaMedia, Log, "Rundown Server Started.");
}
...
void FAvaMediaModule::StopRundownServerCommand(const TArray<FString>& Args)
{
	if (RundownServer)
	{
		UE_LOGF(LogAvaMedia, Log, "Stopping Rundown Server...");
...
void FAvaMediaModule::StartPlaybackServerCommand(const TArray<FString>& InArgs)
{
	...
	{
		UE_LOGF(LogAvaMedia, Error, "A Playback Server can't be started in the same process as a Playback Client.");
		return;
	}
	...
	if (!AvaPlaybackServer)
	{
		...
		UE_LOGF(LogAvaMedia, Log, "Playback Server Started");
	}
}
...
void FAvaMediaModule::StartPlaybackClientCommand(const TArray<FString>& InArgs)
{
	if (AvaPlaybackClient)
	{
		UE_LOGF(LogAvaMedia, Log, "Playback client already started.");
		return;
	}
	...
	if (AvaPlaybackServer)
	{
		if (!bForce)
		{
			UE_LOGF(LogAvaMedia, Error, "A playback client can't be started in the same process as a playback server.");
			return;
		}

		UE_LOGF(LogAvaMedia, Warning, "Playback server has been stopped in editor mode in favor of playback client.");
		StopPlaybackServer();
	...
	UE_LOGF(LogAvaMedia, Log, "Playback client started");
}
...
void FAvaMediaModule::StartHttpPlaybackServerCommand(const TArray<FString>& InArgs)
{
	if (!AvaPlaybackServer.IsValid())
	{
		UE_LOGF(LogAvaMedia, Error, "Playback Server must be started in order to start Http Playback Server.");
		return;
	}
	...
	if (!AvaPlaybackHttpPlaybackServer->IsRunning())
	{
		...
		UE_LOGF(LogAvaMedia, Log, "Http Playback Server Started");
	}
}
...
void FAvaMediaModule::HandleStatCommand(const TArray<FString>& InArgs)
{
	if (InArgs.IsEmpty())
	{
		UE_LOGF(LogAvaMedia, Error, "Stat Command: No arguments specified.");
		return;
	}
