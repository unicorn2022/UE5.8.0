	...
	: FAvaRundownManagedInstance(InParentCache, InAssetPath)
{
	...
	{
		UE_LOGF(LogAvaMedia, Error, "Unable to create a Managed Motion Design Level Package for %ls", *InAssetPath.ToString());
		return;
	}
	...
	if (!ManagedLevel)
	{
		UE_LOGF(LogAvaMedia, Error, "Unable to duplicate Source Motion Design Level: %ls", *InAssetPath.ToString());
		return;
	}
