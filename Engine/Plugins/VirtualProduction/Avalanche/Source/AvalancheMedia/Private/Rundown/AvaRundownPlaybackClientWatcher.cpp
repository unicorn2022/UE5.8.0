...
void FAvaRundownPlaybackClientWatcher::TryRestorePlaySubPage(int InPageId, const UE::AvaPlaybackClient::Delegates::FPlaybackStatusChangedArgs& InEventArgs) const
{
	...
	if (UAvaBroadcast::Get().GetChannelIndex(ChannelFName) == INDEX_NONE)
	{
		UE_LOGF(LogAvaRundown, Error,
			"%ls Received a playback object on channel \"%ls\" which doesn't exist locally. Playback Server should be reset.",
	...
	{
		UE_LOGF(LogAvaRundown, Error,
			"%ls Asset mismatch (expected (any of): \"%ls\", received: \"%ls\") for restoring page %d. Playback Server should be reset.",
			*GetBriefFrameInfo(),
	...
	{
		UE_LOGF(LogAvaRundown, Error, "%ls Failed to restore page %d. Playback Server should be reset.", *GetBriefFrameInfo(), InPageId);
	}
}
	...
	const UE::AvaPlaybackClient::Delegates::FPlaybackStatusChangedArgs& InEventArgs)
{
	...
	{
		UE_LOGF(LogAvaRundown, Verbose, 
			"%ls Playback Client Watcher: Detected asset stopping Id:%ls from Server \"%ls\".",
		...
		for (UAvaRundownPagePlayer* PagePlayer : Rundown->PagePlayers)
		{
			...
			if (PagePlayer && PagePlayer->ChannelName == InEventArgs.ChannelName)
			{
				if (UAvaRundownPlaybackInstancePlayer* InstancePlayer = PagePlayer->FindInstancePlayerByInstanceId(InEventArgs.InstanceId))
				{
					...
					{
						UE_LOGF(LogAvaRundown, Error, "%ls Playback Client Watcher: Instance Id:%ls asset path mismatch in page player %d.",
							*UE::AvaPlayback::Utils::GetBriefFrameInfo(), *InEventArgs.InstanceId.ToString(), PagePlayer->PageId);
						continue;
					}

					UE_LOGF(LogAvaRundown, Verbose, "%ls Playback Client Watcher: Stopping Instance Id:%ls in page player %d.",
						*UE::AvaPlayback::Utils::GetBriefFrameInfo(), *InEventArgs.InstanceId.ToString(), PagePlayer->PageId);
				...
				{
					UE_LOGF(LogAvaRundown, Verbose, "%ls Playback Client Watcher: Stopping Page player %d, no more instances playing.",
	...
	if (bIsRunning)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAvaRundown, Error, 
					"%ls Playback Client Watcher: Playback Instance Id:%ls on server \"%ls\": user data mismatch \"%ls\", local user data: \"%ls\".",
