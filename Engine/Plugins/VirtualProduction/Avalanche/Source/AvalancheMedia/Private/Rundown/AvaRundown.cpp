...
void UAvaRundown::UpdateAssetForPages(const TArray<int32>& InPageIds, bool bInReimportPage)
{
	for (const int32 SelectedPageId : InPageIds)
	{
		...
		if (!Page.IsValidPage())
		{
			UE_LOGF(LogAvaRundown, Error, "Reimport asset failed: page id %d is not valid.", SelectedPageId);
			continue;
		}
		if (!Page.IsTemplate())
		{
			UE_LOGF(LogAvaRundown, Error, "Reimport asset failed: page id %d is not a template.", SelectedPageId);
			continue;
		}				
...
TArray<UAvaRundown::FLoadedInstanceInfo> UAvaRundown::LoadPage(int32 InPageId,  bool bInPreview, const FName& InPreviewChannelName)
{
	...
	// Built-in Load Options:
	LoadOptions += TEXT(" -CacheShaders");
	...
	// Load RC Assets Async, stream them and request cache its materials
	{
		TSet<FSoftObjectPath> RemoteControlAssets;

		const FAvaPlayableRemoteControlValues& RemoteControlValues = Page.GetRemoteControlValues();
		FAvaPlayableRemoteControlValues::CollectReferencedAssetPaths(RemoteControlValues.ControllerValues, RemoteControlAssets);
		FAvaPlayableRemoteControlValues::CollectReferencedAssetPaths(RemoteControlValues.EntityValues, RemoteControlAssets);

		const FName ShaderProfile = GetDefault<UAvaMaterialCacheSettings>()->GetOfflineProfile();

		for (const FSoftObjectPath& Asset : RemoteControlAssets)
		{
			if (UObject* Object = Asset.ResolveObject())
			{
				UE::AvaPlayback::Utils::StreamRenderAsset(Object);
				UE::AvaPlayback::Utils::CacheObjectMaterials(Object, ShaderProfile);
			}
			else
			{
				Asset.LoadAsync(FLoadSoftObjectPathAsyncDelegate::CreateLambda(
					[ShaderProfile](const FSoftObjectPath&, UObject* InObject)
					{
						if (InObject)
						{
							UE::AvaPlayback::Utils::StreamRenderAsset(InObject);
							UE::AvaPlayback::Utils::CacheObjectMaterials(InObject, ShaderProfile);
						}
					}));
			}
		}
...
TArray<int32> UAvaRundown::PlayPages(const TArray<int32>& InPageIds, EAvaRundownPagePlayType InPlayType, EAvaRundownPagePlaySource InSource)
{
	return PlayPages(InPageIds, InPlayType, UE::AvaRundown::IsPreviewPlayType(InPlayType) ? GetDefaultPreviewChannelName() : NAME_None, InSource);
}

TArray<int32> UAvaRundown::PlayPages(const TArray<int32>& InPageIds, EAvaRundownPagePlayType InPlayType, const FName& InPreviewChannelName, EAvaRundownPagePlaySource InSource)
{
	...
	for (const int32 PageId : InPageIds)
	{
		...
		if (SelectedPage.IsValidPage() && SelectedPage.IsEnabled())
		{
			...
			{
				UE_LOGF(LogAvaRundown, Error, "Page Id:%d failed to play: %ls.", PageId, *FailureReason);
				continue;
			}
	...
	OnPlayPages(PlayedPageIds, InPlayType, InPreviewChannelName, InSource);
	return PlayedPageIds;
}

bool UAvaRundown::PlayPage(int32 InPageId, EAvaRundownPagePlayType InPlayType, EAvaRundownPagePlaySource InSource)
{
	return !PlayPages({InPageId}, InPlayType, InSource).IsEmpty();
}

bool UAvaRundown::PlayPage(int32 InPageId, EAvaRundownPagePlayType InPlayType, const FName& InPreviewChannelName, EAvaRundownPagePlaySource InSource)
{
	return !PlayPages({InPageId}, InPlayType, InPreviewChannelName, InSource).IsEmpty();
}
...
bool UAvaRundown::RestorePlaySubPage(int32 InPageId, int32 InSubPageIndex, const FGuid& InExistingInstanceId, bool bInIsPreview, const FName& InPreviewChannelName)
{
	...
	{
		UE_LOGF(LogAvaRundown, Error,
			"%ls Couldn't restore playback state of page %d on channel \"%ls\": %ls.",
	...
	if (!PagePlayer)
	{
		...
		UE_LOGF(LogAvaRundown, Verbose, "%ls Restored page player for page %d.", *GetBriefFrameInfo(), InPageId);
	}
...
bool UAvaRundown::RemoveSubList(const FAvaRundownPageListReference& InPageListReference)
{
	...
	UE_LOGF(LogAvaRundown, Error, "Remove SubList failed: Invalid SubList Reference: %ls.", *ToString(InPageListReference));
	return false;
}
...
bool UAvaRundown::RenameSubList(const FAvaRundownPageListReference& InPageListReference, const FText& InNewName)
{
	...
	using namespace UE::AvaMedia::Rundown::Private;
	UE_LOGF(LogAvaRundown, Error, "Rename SubList failed: Invalid SubList Reference: %ls.", *ToString(InPageListReference));
	return false;
}
...
void UAvaRundown::OnPlayPages(const TArray<int32>& InPlayedPageIds, EAvaRundownPagePlayType InPlayType, FName InPreviewChannelName, EAvaRundownPagePlaySource InSource)
{
	if (InPlayedPageIds.IsEmpty())
	{
		return;
	}
	...
	if (FEngineAnalytics::IsAvailable())
	{
		...
		FString ChannelStatuses;
		FString MediaOutputs;

		// Called before appending to the string
		auto OnPreAppendString = [](FString& OutString, const TCHAR* InDelimiter)
			{
				if (!OutString.IsEmpty())
				{
					OutString += InDelimiter;
				}
			};

		auto GatherChannelMediaOutputs = [&OnPreAppendString](TConstArrayView<UMediaOutput*> InMediaOutputs)
			{
				FString ChannelMediaOutputs;
				for (UMediaOutput* MediaOutput : InMediaOutputs)
				{
					if (MediaOutput)
					{
						OnPreAppendString(ChannelMediaOutputs, TEXT("|"));
						ChannelMediaOutputs += MediaOutput->GetClass()->GetName();
					}
				}
				return ChannelMediaOutputs;
			};

		if (bIsPreview)
		{
			const FAvaBroadcastOutputChannel& PreviewChannel = Profile.GetChannel(InPreviewChannelName);
			ChannelStatuses += StaticEnum<EAvaBroadcastChannelState>()->GetNameStringByValue(static_cast<int64>(PreviewChannel.GetState()));
			MediaOutputs += GatherChannelMediaOutputs(PreviewChannel.GetMediaOutputs());
			ChannelCount = 1;
		}
		else
		{
			TArray<const FAvaBroadcastOutputChannel*, TInlineAllocator<1>> Channels;
			for (int32 PageId : InPlayedPageIds)
			{
				const FAvaRundownPage& SelectedPage = GetPage(PageId);
				Channels.AddUnique(&Profile.GetChannel(SelectedPage.GetChannelName()));
			}
			ChannelCount = Channels.Num();

			// Gather channel statuses, and the media outputs per channel.
			// Media outputs groups are per channel and separated with ",". A media output group contain media outputs separated by "|".
			// E.g. "MediaOutput1,MediaOutput2|MediaOutput3"
			// Would be a setup for where there's 2 channels: 1st channel has 1 media output, 2nd channel has 2 media outputs.
			for (const FAvaBroadcastOutputChannel* Channel : Channels)
			{
				OnPreAppendString(ChannelStatuses, TEXT(","));
				ChannelStatuses += StaticEnum<EAvaBroadcastChannelState>()->GetNameStringByValue(static_cast<int64>(Channel->GetState()));

				OnPreAppendString(MediaOutputs, TEXT(","));
				MediaOutputs += GatherChannelMediaOutputs(Channel->GetMediaOutputs());
			}
		}

		TArray<FAnalyticsEventAttribute> Attributes;
		Attributes.Reserve(6);
		Attributes.Emplace(TEXT("PlayType"), StaticEnum<EAvaRundownPagePlayType>()->GetNameStringByValue(static_cast<int64>(InPlayType)));
		Attributes.Emplace(TEXT("Source"), StaticEnum<EAvaRundownPagePlaySource>()->GetNameStringByValue(static_cast<int64>(InSource)));
		Attributes.Emplace(TEXT("PageCount"), FString::FromInt(InPlayedPageIds.Num()));
		Attributes.Emplace(TEXT("ChannelCount"), FString::FromInt(ChannelCount));
		Attributes.Emplace(TEXT("ChannelStatuses"), ChannelStatuses);
		Attributes.Emplace(TEXT("MediaOutputs"), MediaOutputs);
		FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.MotionDesign.Rundown.Play"), Attributes);
	}
#endif
}
...
bool UAvaRundown::PlayPageWithTransition(FAvaRundownPageTransitionBuilder& InBuilder, const FAvaRundownPage& InPage, EAvaRundownPagePlayType InPlayType, bool bInIsPreview, const FName& InPreviewChannelName)
{
	...
	// Only allow reuse of instances if not doing a preview frame (or overridden in settings)
	// See UAvaPlayableLocalTransition::Start. It kicks out all the pages at the given layer when doing PreviewFrameTransition.
	// If the instance is re-used, it will kick that out as well.
	const bool bAllowReuse = InPlayType != EAvaRundownPagePlayType::PreviewFromFrame || AvaMediaSettings.bAllowReuseInPreviewFrame;
	...
	for (int32 SubPageIndex = 0; SubPageIndex < NumTemplates; ++SubPageIndex)
	{
		...
		if (bBypassTransitionOnSameValues || SubTemplate.GetTransitionMode(this) == EAvaTransitionInstancingMode::Reuse)
		{
			...
			if (InstancePlayer && InstancePlayer->PlaybackInstance)
			{
				...
				else if (bAllowReuse && SubTemplate.GetTransitionMode(this) == EAvaTransitionInstancingMode::Reuse)
				{
...
int32 UAvaRundown::StopPageTransitionsByPredicate(TFunctionRef<bool(UAvaRundownPageTransition*)> InPredicate)
{
	...
	for (UAvaRundownPageTransition* Transition : TransitionsToStop)
	{
		...
		{
			UE_LOGF(LogAvaRundown, Warning, "A page transition was not properly cleaned up.");
