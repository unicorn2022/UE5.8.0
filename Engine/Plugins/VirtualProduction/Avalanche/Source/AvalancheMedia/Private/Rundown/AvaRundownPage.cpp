...
bool FAvaRundownPage::UpdatePageSummary(const UAvaRundown* InRundown, const TArray<const URemoteControlPreset*>& InPresets, bool bInForceUpdate)
{
	...
	if (!OrderedDescription.IsEmpty())
	{
		...
		for (const TPair<int32, FText>& OrderedValue : OrderedDescription)
		{
			...
			CurrentLength += OrderedValue.Value.ToString().Len();
			if (CurrentLength >= FinishDescriptionWhenExceedLimit)
			{
...
bool FAvaRundownPage::UpdateTransitionLogic()
{
	...
	if (const UObject* LoadedSourceAsset = AssetPath.TryLoad())
	{
		...
		{
			const IAvaTransitionBehavior* TransitionBehavior = FAvaRundownPageAssetUtils::FindTransitionBehavior(SceneInterface);
			bHasTransitionLogic = TransitionBehavior ? TransitionBehavior->IsEnabled() : false;
			TransitionLayerTag = TransitionBehavior ? TransitionBehavior->GetTransitionLayer() : FAvaTagHandle();
			TransitionMode = TransitionBehavior ? TransitionBehavior->GetInstancingMode() : EAvaTransitionInstancingMode::New;
		}
	}
	return false;
}
...
bool FAvaRundownPage::UpdateAsset(const FSoftObjectPath& InAssetPath, bool bInReimportPage)
{
	if (IsComboTemplate())
	{
		UE_LOGF(LogAvaRundown, Error, "Can't update asset on a combo page directly.");
		return false;
	}
...
void FAvaRundownPage::SetChannelName(FName InChannelName)
{
	...
	{
		UE_LOGF(LogAvaBroadcast, Error, "Channel %ls was not found in broadcast channels, using %ls instead.",
	...
	if (Template.IsValidPage() && Template.CombinedTemplateIds.Num())
	{
		if (Template.CombinedTemplateIds.IsValidIndex(InIndex))
		{
			...
			UE_LOGF(LogAvaRundown, Error,
				"Internal error while accessing page %d's template (%d index %d): reference to template Id %d is not valid.",
				GetPageId(), Template.GetPageId(), InIndex, Template.CombinedTemplateIds[InIndex]);
		}
		
		UE_LOGF(LogAvaRundown, Error,
			"Internal error while accessing page %d's template (%d): specified index %d is not valid.",
...
const FAvaRundownPage& FAvaRundownPage::ResolveTemplate(const UAvaRundown* InRundown) const
{
	...
	if (IsValid(InRundown))
	{
		...
		{
			UE_LOGF(LogAvaRundown, Warning,
				"PageId %d is in the template list but has \"IsTemplate\" flag to false.", GetPageId());
		...
		if (InRundown->GetInstancedPages().PageIndices.Contains(GetPageId()))
		{
			...
			UE_LOGF(LogAvaRundown, Error,
				"PageId [%d] is an instanced page, has template id [%d], but that template doesn't exist.",
