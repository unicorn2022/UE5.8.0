	...
	bool ImportPropertyCallback(const TSharedPtr<FJsonValue>& InJsonValue, FProperty* InProperty, void* InValue)
	{
		if (const FStructProperty* StructProperty = CastField<FStructProperty>(InProperty))
		{
			if (StructProperty->Struct == FInstancedStruct::StaticStruct())
			{
				...
				{
					UE_LOGF(LogAvaRundown, Error, "Rundown Json Parsing FInstancedStruct Property: Json value should be of type Object, is of type: %ls", GetJsonTypeName(InJsonValue->Type));
				...
				if (InJsonValue->TryGetObject(InstancedStructObject))
				{
					...
					{
						UE_LOGF(LogAvaRundown, Error, "Rundown Json Parsing FInstancedStruct Property: Json Object should have at least 1 field (has %d)", (*InstancedStructObject)->Values.Num());
						return bHandledButError;
					}

					if ((*InstancedStructObject)->Values.Num() > 1)
					{
						UE_LOGF(LogAvaRundown, Warning, "Rundown Json Parsing FInstancedStruct Property: Json Object should have only 1 field (has %d)", (*InstancedStructObject)->Values.Num());
					}
					...
					for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Value : (*InstancedStructObject)->Values)
					{
						...
						{
							UE_LOGF(LogAvaRundown, Error, "Rundown Json Parsing FInstancedStruct Property: Json value is invalid");
							continue;
						}
						...
						if (const UScriptStruct* Struct = Cast<UScriptStruct>(ScriptPath.ResolveObject()))	// so we got a valid struct
						{
							...
							{
								UE_LOGF(LogAvaRundown, Error, "Rundown Json Parsing FInstancedStruct Property: Json value should be of type Object, is of type: %ls", GetJsonTypeName(Value.Value->Type));
							}
							...
							if (Value.Value->TryGetObject(StructObject))
							{
								...
								UE_LOGF(LogAvaRundown, Error, "Rundown Json Parsing FInstancedStruct Property: Failed to deserialize: \"%ls\"", *DeserializationError.ToString());
							}
						}
						else
						{
							UE_LOGF(LogAvaRundown, Error, "Rundown Json Parsing FInstancedStruct Property: Unknown Script Type: \"%ls\"", *Value.Key);
						}
					}
