...
bool UAvaRundownPagePlayer::Initialize(UAvaRundown* InRundown, const FAvaRundownPage& InPage, bool bInIsPreview, const FName& InPreviewChannel)
{
	if (!InRundown)
	{
		UE_LOGF(LogAvaRundown, Error, "UAvaRundownPagePlayer::Initialize: Invalid rundown.");
		return false;
	}

	if (!InPage.IsValidPage())
	{
		UE_LOGF(LogAvaRundown, Error, "UAvaRundownPagePlayer::Initialize: Invalid page.");
		return false;
	}
...
UAvaRundownPlaybackInstancePlayer* UAvaRundownPagePlayer::LoadInstancePlayer(int32 InSubPageIndex, const FGuid& InInstanceId)
{
	...
	if (!Rundown)
	{
		UE_LOGF(LogAvaRundown, Error, "UAvaRundownPagePlayer::LoadSubPage: Rundown is no longuer valid.");
		return nullptr;
	}
	...
	if (!Page.IsValidPage())
	{
		UE_LOGF(LogAvaRundown, Error, "UAvaRundownPagePlayer::LoadSubPage: Invalid pageId %d.", PageId);
		return nullptr;
	}
	...
	if (RundownWeak.IsValid())
	{
		...
		if (InEventType == EAvaPlayableSequenceEventType::Started)
		{
			UE_LOGF(LogAvaRundown, Verbose,
				"%ls Rundown Page %d (playable %ls): Sequence Started \"%ls\".",
		...
		{
			UE_LOGF(LogAvaRundown, Verbose,
				"%ls Rundown Page %d (playable %ls): Sequence Finished \"%ls\".",
				*GetBriefFrameInfo(), PageId, *InPlayable->GetInstanceId().ToString(), *InSequenceLabel.ToString());
