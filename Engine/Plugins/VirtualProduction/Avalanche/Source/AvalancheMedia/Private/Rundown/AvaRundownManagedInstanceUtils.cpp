...
UPackage* FAvaRundownManagedInstanceUtils::MakeManagedInstancePackage(const FSoftObjectPath& InAssetPath)
{
	...
	if (FindObject<UPackage>( nullptr, *InstancePackageName))
	{
		...
		UE_LOGF(LogAvaMedia, Warning, 
			"Package \"%ls\" for Motion Design Managed Instance already exists. Will use \"%ls\" instead.",
	...
	else
	{
		...
		UE_LOGF(LogAvaMedia, Error, 
			"Unable to create package \"%ls\" for Motion Design Managed Instance.",
			*InstancePackageName);
	}
