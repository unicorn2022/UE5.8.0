...
void FAvaPlaybackHttpServer::RegisterNewRoute(FAvaPlaybackHttpRouteInfo InRouteInfo, const FHttpRequestHandler& Handler, bool bOverrideIfBound /* = false */)
{
	if (HttpRouter.IsValid())
	{
		if (RegisteredRoutes.Find(InRouteInfo.RouteName))
		{
			if (!bOverrideIfBound)
			{
				UE_LOGF(LogAvaPlaybackHttpServer, Error, "Failed to bind route with friendly key %ls - a route at location %ls already exists.", *InRouteInfo.RouteName.ToString(), *InRouteInfo.RoutePath.GetPath());
				return;
			}
			UE_LOGF(LogAvaPlaybackHttpServer, Log, "Overwriting route at friendly key %ls - from %ls to %ls ", *InRouteInfo.RouteName.ToString(), *RegisteredRoutes[InRouteInfo.RouteName].Handle->Path, *InRouteInfo.RoutePath.GetPath());
...
void FAvaPlaybackHttpServer::CleanUpRoute(FName RouteName, bool bFailIfUnbound /* = false */)
{
	if (HttpRouter.IsValid())
	{
		if (RegisteredRoutes.Find(RouteName))
		{
			...
			UE_LOGF(LogAvaPlaybackHttpServer, Log, "Route name %ls was unbound!", *RouteName.ToString());
		}
		else
		{
			UE_LOGF(LogAvaPlaybackHttpServer, Warning, "Route name %ls does not exist, could not unbind.", *RouteName.ToString());
