		...
		bool CheckDependencies(const IAssetRegistry& InAssetRegistry, const FName& InPackageName, int InRecursion)
		{
			...
			for (const FName& Dependency : Dependencies)
			{
				if (!IsPackageIgnored(Dependency.ToString()) && !FPackageName::DoesPackageExist(Dependency.ToString()))
				{
					...
					UE_LOGF(LogAvaPlaybackManager, Verbose, "Package \"%ls\" doesn't exist (dependency of \"%ls\").",
			...
			if (InRecursion < MaxRecursion)
			{
				...
				for (const FName& Dependency : Dependencies)
				{
					...
					if (!IsPackageIgnored(Dependency.ToString()) && !AllDependencies.Contains(Dependency))
					{
						...
						{
							UE_LOGF(LogAvaPlaybackManager, Verbose, "Package \"%ls\" (dependency of \"%ls\") has missing dependencies.",
								*Dependency.ToString(), *InPackageName.ToString());
							return false;
						}
					}
			...
			else
			{
				if (MaxRecursion != 0)
				{
					UE_LOGF(LogAvaPlaybackManager, Warning, "Reached maximum recursion (%d) while evaluating dependencies on package \"%ls\".",
...
UAvaPlaybackGraph* FAvaPlaybackManager::LoadPlaybackObject(const FSoftObjectPath& InAssetPath, const FString& InChannelName, const FString& InLoadOptions) const
{
	...
	if (!LoadedAsset)
	{
		...
		if (!LoadedAsset)
		{
			UE_LOGF(LogAvaPlaybackManager, Error, "Failed to load asset \"%ls\"", *InAssetPath.ToString());
			return nullptr;
		}
	}
	...
	UE_LOGF(LogAvaPlaybackManager, Error,
		"Asset \"%ls\" is not a supported Motion Design playback asset (\"%ls\").",
...
void FAvaPlaybackManager::OnPackageModified(const FName& InPackageName, EAvaPlaybackPackageEventFlags InFlags)
{
	...
	for (TPair<FSoftObjectPath, TSharedPtr<FAvaPlaybackSourceAssetEntry>>& AssetEntry : PlaybackAssetEntries)
	{
		if (AssetEntry.Value)
		{
			...
			if (SourcePackageFName == InPackageName)
			{
				...
				{
					OnPlaybackInstanceInvalidated.Broadcast(InPlaybackInstance);
					UE_LOGF(LogAvaPlaybackManager, Log,
						"Package \"%ls\" being touched caused asset \"%ls\" to be invalidated.",
	...
	{		
		if (UPackage* ExistingPackage = FindPackage(nullptr, *InPackageName.ToString()))
		{
			...
			{
				UE_LOGF(LogAvaPlaybackManager, Log, "Loaded package \"%ls\" being deleted on an external process requires a purge.", *InPackageName.ToString());
				FAvaPlaybackUtils::PurgePackages({ExistingPackage});
			}
			else
			{
				UE_LOGF(LogAvaPlaybackManager, Log, "Loaded package \"%ls\" being touched on an external process requires a reload.", *InPackageName.ToString());
...
bool FAvaPlaybackManager::HandleStatCommand(const TArray<FString>& InArgs)
{
	...
	if (!ViewportClient)
	{
		...
		if (!bFound)
		{
			UE_LOGF(LogAvaPlaybackManager, Warning, "No Active Motion Design Game Instances found to apply the stat '%ls' command on.", *InArgs[0]);
		}
	}
...
{
	UE_LOGF(LogAvaPlaybackManager, Verbose,
		"A sync operation has touched the package \"%ls\" on disk. Playback manager notified.",
...
bool FAvaPlaybackInstance::UpdateStatus()
{
	...
	if (Playback->IsPlaying())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAvaPlaybackManager, Verbose,
					"%ls Playback Instance {%ls} Status Changed: %ls -> %ls because Playable \"%ls\" (%ls) was not found in PLAYING instance.",
	...
	else
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAvaPlaybackManager, Verbose,
					"%ls Playback Instance {%ls} Status Changed: %ls -> %ls because Playable \"%ls\" (%ls) was not found in instance.",
					*GetBriefFrameInfo(), *GetPrettyPlaybackInstanceInfo(this), *StaticEnumToString(Status), *StaticEnumToString(NewStatus),
					*SourcePath.GetAssetName(), *ChannelName);
			}
		}
