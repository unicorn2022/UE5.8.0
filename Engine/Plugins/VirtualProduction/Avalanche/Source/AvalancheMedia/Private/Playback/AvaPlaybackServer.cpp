...
void FAvaPlaybackServer::Init(const FString& InAssignedServerName)
{
	...
	if (MessageEndpoint.IsValid())
	{
		...
		UE_LOGF(LogAvaPlaybackServer, Log, "Motion Design Playback Server \"%ls\" Started.", *ServerName);
	}
...
void FAvaPlaybackServer::HandlePlaybackPing(const FAvaPlaybackPing& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	{
		UE_LOGF(LogAvaPlaybackServer, Log, "Received Manual Ping from %ls", *InContext->GetSender().ToString());
	}
...
void FAvaPlaybackServer::HandleUpdateClientUserData(const FAvaPlaybackUpdateClientUserData& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	UE_LOGF(LogAvaPlaybackServer, Verbose, "Received new user data for client \"%ls\".", *InMessage.ClientName);
	for (const TPair<FString, FString>& UserData : ClientInfo.UserDataEntries)
	{
		UE_LOGF(LogAvaPlaybackServer, Verbose, "User data \"%ls\":\"%ls\".", *UserData.Key, *UserData.Value);
	}
}
...
void FAvaPlaybackServer::HandleAvaInstanceSettingsUpdate(const FAvaPlaybackInstanceSettingsUpdate& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	UE_LOGF(LogAvaPlaybackServer, Verbose, "Received new instance settings from client \"%ls\".", *InMessage.ClientName);
}
...
void FAvaPlaybackServer::HandlePlayableSettingsUpdate(const FAvaPlaybackPlayableSettingsUpdate& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	UE_LOGF(LogAvaPlaybackServer, Verbose, "Received new playable settings from client \"%ls\".", *InMessage.ClientName);
}
...
void FAvaPlaybackServer::HandlePackageEvent(const FAvaPlaybackPackageEvent& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	case EAvaPlaybackPackageEvent::PostSave:
		UE_LOGF(LogAvaPlaybackServer, Verbose, "Client modified package \"%ls\".", *InMessage.PackageName.ToString());
	...
	case EAvaPlaybackPackageEvent::AssetDeleted:
		UE_LOGF(LogAvaPlaybackServer, Verbose, "Client deleted asset in package \"%ls\".", *InMessage.PackageName.ToString());
...
void FAvaPlaybackServer::HandlePlayableTransitionStartRequest(const FAvaPlaybackTransitionStartRequest& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	for (const FGuid& PlayingInstanceId : InMessage.PlayingInstanceIds)
	{
		...
		else
		{
			...
			UE_LOGF(LogAvaPlaybackServer, Warning,
				"Transition \"%ls\" from client \"%ls\": \"Playing\" Instance Id \"%ls\" was not found in active playback instances.",
	...
	for (const FGuid& ExitInstanceId : InMessage.ExitInstanceIds)
	{
		...
		else
		{
			...
			UE_LOGF(LogAvaPlaybackServer, Warning,
				"Transition \"%ls\" from client \"%ls\": \"Exit\" Instance Id \"%ls\" was not found in active playback instances.",
...
void FAvaPlaybackServer::HandlePlayableTransitionStopRequest(const FAvaPlaybackTransitionStopRequest& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	if (!PlaybackInstanceTransitions)
	{
		UE_LOGF(LogAvaPlaybackServer, Error, 
			"Stopping Transition \"%ls\" failed because transition collection is not available.", *InMessage.TransitionId.ToString());
	...
	{
		UE_LOGF(LogAvaPlaybackServer, Warning,
			"Stop Transition request: Transition \"%ls\" was not found in the active list.", *InMessage.TransitionId.ToString());
	}
}
...
void FAvaPlaybackServer::HandleBroadcastSettingsUpdate(const FAvaBroadcastSettingsUpdate& InMessage, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& InContext)
{
	...
	UE_LOGF(LogAvaPlaybackServer, Verbose, "Received new broadcast settings from client \"%ls\".", *InMessage.ClientName);
}
	...
	else if (InMessage.Action == EAvaBroadcastAction::DeleteChannel)
	{
		if (!InMessage.Channel.IsEmpty())
		{
			...
			else
			{
				...
				UE_LOGF(LogAvaPlaybackServer, Error, "Failed to remove channel \"%ls\".", *InMessage.Channel);
			}
		}
		else
		{
			UE_LOGF(LogAvaPlaybackServer, Error, "Received a \"Delete Channel\" command with an empty channel name.");
		}
	}
}
...
{
	UE_LOGF(LogAvaPlaybackServer, Verbose, "Received a broadcast status request from \"%ls\"", *InMessage.ClientName);
	...
	if (PlaybackInstanceTransitions)
	{
		for (const FGuid& TransitionId : PlaybackInstanceTransitions->MarkedForStopAndDiscard)
		{
			if (UAvaPlaybackServerTransition* Transition = PlaybackInstanceTransitions->FindTransition(TransitionId))
			{
				...
				{
					UE_LOGF(LogAvaPlaybackServer, Error, "Playback Transition %ls is still in the list after being stopped.", *TransitionId.ToString());
...
void FAvaPlaybackServer::StartPlaybackCommand(const TArray<FString>& InArgs)
{
	...
	{
		UE_LOGF(LogAvaPlaybackServer, Log, "Arguments: Package.AssetName. Ex: \"/Game/AvaPlayback.AvaPlayback\"");
	}
}
...
void FAvaPlaybackServer::SetUserDataCommand(const TArray<FString>& InArgs)
{
	if (InArgs.Num() >= 2)
	{
		UE_LOGF(LogAvaPlaybackServer, Log, "Setting User Data Key \"%ls\" to Value: \"%ls\".", *InArgs[0], *InArgs[1]);
	...
	else if (InArgs.Num() == 1)
	{
		...
		{
			UE_LOGF(LogAvaPlaybackServer, Log, "Removing User Data Key \"%ls\".", *InArgs[0]);
			RemoveUserData(InArgs[0]);
		}
		else
		{
			UE_LOGF(LogAvaPlaybackServer, Error, "User Data Key \"%ls\" not found.", *InArgs[0]);
		}
	}
}
...
{
	UE_LOGF(LogAvaPlaybackServer, Display, "Playback Server: \"%ls\"", *ServerName);
	UE_LOGF(LogAvaPlaybackServer, Display, "- Endpoint Bus Address: \"%ls\"", MessageEndpoint.IsValid() ? *MessageEndpoint->GetAddress().ToString() : TEXT("Invalid"));
	UE_LOGF(LogAvaPlaybackServer, Display, "- Computer: \"%ls\"", *ComputerName);
	UE_LOGF(LogAvaPlaybackServer, Display, "- ProcessId: %d", ProcessId);
	UE_LOGF(LogAvaPlaybackServer, Display, "- Content Path: \"%ls\"", *ProjectContentPath);

	for (const TPair<FString, FString>& UserData : UserDataEntries)
	{
		UE_LOGF(LogAvaPlaybackServer, Display, "- User data \"%ls\":\"%ls\".", *UserData.Key, *UserData.Value);
	}
	
	for (const TPair<FString, TSharedPtr<FClientInfo>>& Client : Clients)
	{
		const FClientInfo& ClientInfo = *Client.Value;
		UE_LOGF(LogAvaPlaybackServer, Display, "Connected Client: \"%ls\"", *ClientInfo.ClientName);
		UE_LOGF(LogAvaPlaybackServer, Display, "   - Endpoint Bus Address: \"%ls\"", *ClientInfo.Address.ToString());
		UE_LOGF(LogAvaPlaybackServer, Display, "   - Computer: \"%ls\"", *ClientInfo.ComputerName);
		UE_LOGF(LogAvaPlaybackServer, Display, "   - ProcessId: %d", ClientInfo.ProcessId);
		UE_LOGF(LogAvaPlaybackServer, Display, "   - Content Path: \"%ls\"", *ClientInfo.ProjectContentPath);
		...
		UE_LOGF(LogAvaPlaybackServer, Display, "   - BroadcastSettings.ChannelClearColor: (%f, %f, %f, %f)",
		...
		UE_LOGF(LogAvaPlaybackServer, Display, "   - BroadcastSettings.ChannelDefaultPixelFormat: (%ls)",
			GetPixelFormatString(ClientInfo.BroadcastSettings.Settings.ChannelDefaultPixelFormat));
		UE_LOGF(LogAvaPlaybackServer, Display, "   - BroadcastSettings.ChannelDefaultResolution: (%d, %d)",
		...
		UE_LOGF(LogAvaPlaybackServer, Display, "   - BroadcastSettings.bDrawPlaceholderWidget: %ls",
			ClientInfo.BroadcastSettings.Settings.bDrawPlaceholderWidget ? TEXT("true") : TEXT("false"));
		UE_LOGF(LogAvaPlaybackServer, Display, "   - BroadcastSettings.PlaceholderWidgetClass: %ls",
			*ClientInfo.BroadcastSettings.Settings.PlaceholderWidgetClass.ToString());

		UE_LOGF(LogAvaPlaybackServer, Display, "   - MediaSyncManager: %ls.",
		...
		ClientInfo.MediaSyncManager->EnumerateAllTrackedPackages([](const FName& InPackageName, const TOptional<bool>& bInNeedSync)
		{
			...
			{
				UE_LOGF(LogAvaPlaybackServer, Display, "   - Package Sync Status \"%ls\":\"%ls\".",
		...
		{
			UE_LOGF(LogAvaPlaybackServer, Display, "   - Pending Sync Status Request \"%ls\".", *PendingPackage.ToString());
		}
	}

	UE_LOGF(LogAvaPlaybackServer, Display, "Active Playback Instances:");
	for (const TPair<FGuid, TSharedPtr<FAvaPlaybackInstance>>& ActivePlaybackInstance : ActivePlaybackInstances)
	{
		...
		UE_LOGF(LogAvaPlaybackServer, Display, "   - Id:%ls, Channel: %ls, Asset: %ls, Status: %ls, UserData: %ls .",
	...
	UE_LOGF(LogAvaPlaybackServer, Display, "Active Playback Transitions:");
	for (const TPair<FGuid, TObjectPtr<UAvaPlaybackServerTransition>>& TransitionEntry : PlaybackInstanceTransitions->Transitions)
	{
		...
		{
			UE_LOGF(LogAvaPlaybackServer, Display, "   - %ls: %ls",
...
void FAvaPlaybackServer::SendChannelStatusUpdate(const FString& InChannelName, const FAvaBroadcastOutputChannel& InChannel, const FMessageAddress& InSender, bool bInIncludeOutputData)
{
	...
	{
		UE_LOGF(LogAvaPlaybackServer, Warning,
			"The requested channel status update (DataSize: %d) is larger that the safe message size limit (%d).",
...
void FAvaPlaybackServer::ExecutePendingPlaybackCommands(const FDateTime& InUtcNow)
{
	...
	for (TArray<TSharedPtr<FPendingPlaybackCommand>>::TIterator PendingCommandIt(PendingPlaybackCommands); PendingCommandIt; ++PendingCommandIt)
	{
		...
		if (bReschedule)
		{
			...
			{
				UE_LOGF(LogAvaPlaybackServer, Warning, "%ls Discarding Playback Command [%ls] (Timed out after %f seconds) for asset: \"%ls\" (id:%ls) on channel %ls",
		...
		{
			UE_LOGF(LogAvaPlaybackServer, Verbose, "%ls Playback Command [%ls] Executed for asset: \"%ls\" (id:%ls) on channel \"%ls\", received frame [%d], wait time: %.2f ms",
...
TSharedPtr<FAvaPlaybackInstance> FAvaPlaybackServer::GetOrLoadPlaybackInstance(const FGuid& InInstanceId, const FString& InChannelName, const FSoftObjectPath& InAssetPath, const FString& InLoadOptions)
{
	...
	if (PlaybackInstance.IsValid())
	{
		...
		{
			UE_LOGF(LogAvaPlaybackServer, Error
				, "Existing Playback InstanceId \"%ls\" for asset \"%ls\" has the wrong channel \"%ls\", requested \"%ls\"."
		...
		{
			UE_LOGF(LogAvaPlaybackServer, Error
				, "Existing Playback InstanceId \"%ls\" has wrong source asset path \"%ls\", requested \"%ls\"."
...
void FAvaPlaybackServer::LoadPlayback(const FMessageAddress& InReplyToAddress, const FGuid& InInstanceId, const FString& InChannelName, const FSoftObjectPath& InAssetPath, const FString& InLoadOptions)
{
	...
	{
		// Not supported.
		UE_LOGF(LogAvaPlaybackServer, Error, "Specifying invalid path for load command is not supported.");
	}
	else
	{
			...
			if (PlaybackInstanceTransitions)
			{
				for (const TPair<FGuid, TObjectPtr<UAvaPlaybackServerTransition>>& Transition : PlaybackInstanceTransitions->Transitions)
				{
					if (Transition.Value && Transition.Value->ContainsInstance(InInstanceId))
					{
						...
						UE_LOGF(LogAvaPlaybackServer, Error, "%ls Unloading instance \"%ls\" (id:%ls) while it is part of transition %ls.",
	...
	const TArray<FAvaBroadcastOutputData>& InMediaOutputs, bool bInRefreshState)
{
	...
	{
		UE_LOGF(LogAvaPlaybackServer, Error, "Failed to update output config on channel \"%ls\". Channel is live.",
	...
	if (!InMediaOutputs.IsEmpty())
	{
		...
		for (const FAvaBroadcastOutputData& MediaOutputData : InMediaOutputs)
		{
			...
			{
				UE_LOGF(LogAvaPlaybackServer, Warning,
					   "Channel \"%ls\" received an output for another server (\"%ls\")",
	...
	const FString& InClientName, const FMessageAddress& InClientAddress)
{
	if (const TSharedPtr<FClientInfo>* ClientInfo = Clients.Find(InClientName))
	{
		if ((*ClientInfo)->Address != InClientAddress)
		{
			...
			UE_LOGF(LogAvaPlaybackServer, Warning, "Client \"%ls\" Address changed, possible collision between clients with same name.", *InClientName);
...
void FAvaPlaybackServer::RemoveDeadClients(const FDateTime& InCurrentTime)
{
	for (TMap<FString, TSharedPtr<FClientInfo>>::TIterator ServerIter(Clients); ServerIter; ++ServerIter)
	{
		...
		{
			UE_LOGF(LogAvaPlaybackServer, Log, "Client \"%ls\" is not longer sending pings. Removing.",
				   *ServerIter.Key());
...
{
	UE_LOGF(LogAvaPlaybackServer, Log, "Registering new playback client \"%ls\".", *InClientInfo.ClientName);
