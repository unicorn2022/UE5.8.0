...
void FAvaPlaybackSyncManager::SetEnable(bool bInEnabled)
{
	...
	UE_LOGF(LogAvaPlaybackServer, Verbose, "Server's Media Sync Manager is %ls.",
...
void FAvaPlaybackSyncManager::RequestPackageSyncStatus(const FName& InPackageName)
{
	...
	{
		UE_LOGF(LogAvaPlaybackServer, Verbose,
			"Sync Compare of package \"%ls\" with remote \"%ls\" is already pending.",
...
void FAvaPlaybackSyncManager::HandleExpiredRequest(const FName& InPackageName)
{
	...
	UE_LOGF(LogAvaPlaybackServer, Warning,
		"Sync Compare of package \"%ls\" with remote \"%ls\" timed out. Asset will be considered up to date until sync layer can resume operation.",
		*InPackageName.ToString(), *RemoteName);
...
void FAvaPlaybackSyncManager::HandleFailedRequest(const FName& InPackageName, const FString& InErrorMessage)
{
	...
	UE_LOGF(LogAvaPlaybackServer, Error,
		"Sync Compare of package \"%ls\" with remote \"%ls\" failed: %ls. Asset will be considered up to date until sync layer can resume operation.",
