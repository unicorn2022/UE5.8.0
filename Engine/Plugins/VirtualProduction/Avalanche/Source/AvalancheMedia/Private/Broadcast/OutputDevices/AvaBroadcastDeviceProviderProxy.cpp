...
void FAvaBroadcastDeviceProviderProxyManager::ListAllProviders()
{
	if (!IMediaIOCoreModule::IsAvailable())
	{
		UE_LOGF(LogAvaMedia, Error, "IMediaIOCoreModule is not available.");
		return;
	}
	...
	{
		UE_LOGF(LogAvaMedia, Log, "Found DeviceProvider: \"%ls\".", *DeviceProvider->GetFName().ToString());
		...
		if (OutputConfigs.Num() > 0)
		{
			...
			{
				//UE_LOGF(LogAvaMedia, Log, "Found Output Config: \"%ls\".", *DeviceProvider->ToText(Config).ToString());
			...
			{
				UE_LOGF(LogAvaMedia, Log, "Found Output Device: \"%ls\".", *OutputDeviceName);
			}
		}
		else
		{
			UE_LOGF(LogAvaMedia, Log, "No Output Devices Available for \"%ls\".", *DeviceProvider->GetFName().ToString());
		}	
	}
	...
	for (const UClass* Class : TObjectRange<UClass>())
	{
		if (Class->IsChildOf(UMediaOutput::StaticClass()))
		{
			...
			{
				UE_LOGF(LogAvaMedia, Log, "Found DeviceProviderName: \"%ls\" for class \"%ls\".", *DeviceProviderName.ToString(), *Class->GetName());
				...
				if (IMediaIOCoreModule::Get().GetDeviceProvider(DeviceProviderName) == nullptr)
				{
					...
					UE_LOGF(LogAvaMedia, Log, "DeviceProvider \"%ls\" not found in IMediaIOCoreModule.", *DeviceProviderName.ToString());
				}
			}
		}
	}
