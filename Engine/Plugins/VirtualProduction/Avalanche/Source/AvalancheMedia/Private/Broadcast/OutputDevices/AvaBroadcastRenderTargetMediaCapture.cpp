...
bool UAvaBroadcastRenderTargetMediaCapture::PostInitializeCaptureViewport(TSharedPtr<FSceneViewport>& InSceneViewport)
{
	...
	{
		UE_LOGF(LogAvaBroadcastRenderTargetMedia, Error, "Can't start capture of a scene viewport with no back buffer.");
	}
	return bSuccess;
}
...
bool UAvaBroadcastRenderTargetMediaCapture::StartNewCapture(const FIntPoint& InSourceTargetSize, EPixelFormat InSourceTargetFormat)
{
	TRACE_BOOKMARK(TEXT("UAvaBroadcastRenderTargetMediaCapture::StartNewCapture"));
	{
		...
		if (AvaRenderTargetMediaOutput)
		{
			...
			{
				UE_LOGF(LogAvaBroadcastRenderTargetMedia, Error, "Missing render target object: \"%ls\".", *AvaRenderTargetMediaOutput->RenderTarget.ToString());
			}
		}
		else
		{
			UE_LOGF(LogAvaBroadcastRenderTargetMedia, Error, "Internal Error: Media Capture's associated Media Output is not of type \"UAvaBroadcastRenderTargetMediaOutput\".");
		}
	}
