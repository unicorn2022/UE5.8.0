...
bool FAvaBroadcastProfile::RemoveChannel(FName InChannelName)
{
	...
	{
		UE_LOGF(LogAvaBroadcast, Error,
			"Can't remove channel \"%ls\" from profile \"%ls\", not found in profile.",
	...
	{
		UE_LOGF(LogAvaBroadcast, Error,
			"Can't remove channel \"%ls\" (index %d) from profile \"%ls\": invalid index.",
...
FAvaBroadcastOutputChannel& FAvaBroadcastProfile::GetChannelMutable(FName InChannelName)
{
	...
	if (PinnedChannelProfileName != NAME_None)
	{
		...
		UE_LOGF(LogAvaBroadcast, Error,
			"Can't find profile \"%ls\" for pinned channel \"%ls\". Using local channel instead.",
