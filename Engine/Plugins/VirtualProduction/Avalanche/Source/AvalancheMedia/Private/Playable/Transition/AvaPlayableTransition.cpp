...
bool FAvaPlayableTransitionBuilder::AddEnterPlayable(UAvaPlayable* InPlayable, bool bInAllowMultipleAdd)
{
	if (!bInAllowMultipleAdd && ExitPlayablesWeak.Contains(InPlayable))
	{
		...
		UE_LOGF(LogAvaPlayable, Error,
			"Playable Transition setup error: Playable {%ls} can't added as an \"enter\" playable because it is already in the \"exit\" list.",
	...
	if (!bInAllowMultipleAdd && PlayingPlayablesWeak.Contains(InPlayable))
	{
		using namespace UE::AvaPlayableTransition::Private;
		UE_LOGF(LogAvaPlayable, Error,
			"Playable Transition setup error: Playable {%ls} can't added as an \"enter\" playable because it is already in the \"playing\" list.",
			*GetPrettyPlayableInfo(InPlayable));
		return false;
	}
...
bool FAvaPlayableTransitionBuilder::AddPlayingPlayable(UAvaPlayable* InPlayable, bool bInAllowMultipleAdd)
{
	if (!bInAllowMultipleAdd && EnterPlayablesWeak.Contains(InPlayable))
	{
		using namespace UE::AvaPlayableTransition::Private;
		UE_LOGF(LogAvaPlayable, Error,
			"Playable Transition setup error: Playable {%ls} can't added as an \"exit\" playable because it is already in the \"enter\" list.",
			*GetPrettyPlayableInfo(InPlayable));
		return false;
	}
	...
	if (!bInAllowMultipleAdd && ExitPlayablesWeak.Contains(InPlayable))
	{
		using namespace UE::AvaPlayableTransition::Private;
		UE_LOGF(LogAvaPlayable, Error,
			"Playable Transition setup error: Playable {%ls} can't added as an \"exit\" playable because it is already in the \"exit\" list.",
			*GetPrettyPlayableInfo(InPlayable));
		return false;
	}
	...
	if (bCreateRemoteTransition)
	{
		...
		if (ChannelNames.Num() > 1)
		{
			...
			UE_LOGF(LogAvaPlayable, Warning,
				"Playable Transition setup warning: Playables from different channels (%ls) are in the same transition.",
				*ChannelNameList);
		}
		...
		{
			UE_LOGF(LogAvaPlayable, Error,
				"Playable Transition setup error: Couldn't get a channel name from the list of playables.");
		}
