...
bool UAvaPlayableRemoteTransition::Start()
{
	...
	UE_LOGF(LogAvaPlayable, Verbose, "%ls Remote Playable Transition \"%ls\" starting.", *GetBriefFrameInfo(), *GetInstanceName());
	return true;
}
...
void UAvaPlayableRemoteTransition::Stop()
{
	...
	if (RemoteStatusPerServer.Num() > 1 && IsTransitionFinishedOnAllServers())
	{
		auto CheckForDesync = [this](const FString& InValueName, const TFunction<int32(const FRemoteStatusInfo& InStatusInfo)>& InGetValueFunction)
		{
			...
			{
				using namespace UE::AvaPlayback::Utils;
				UE_LOGF(LogAvaPlayable, Warning, "%ls Remote Playable Transition \"%ls\" %ls Desync Detected: Delta Frames: %d.",
	...
	const UE::AvaPlaybackClient::Delegates::FPlaybackTransitionEventArgs& InArgs)
{
	...
	if (bIsTransitionStartingEvent)
	{
		...
		{
			UE_LOGF(LogAvaPlayable, Warning,
				"%ls Remote Playable Transition \"%ls\" discarding \"Starting\" event from server \"%ls\" because it is already finished.",
	...
	if (bIsTransitionFinishedEvent)	// Finished has priority over all other states.
	{
		...
		UE_LOGF(LogAvaPlayable, Verbose, "%ls Remote Playable Transition \"%ls\" ended on server \"%ls\" at frame [%d].",
	...
	if (InArgs.InstanceId.IsValid())
	{
		if (UAvaPlayable* Playable = FindPlayable(InArgs.InstanceId))
		{
			...
			if (bIsStopPlayableEvent)
			{
				UE_LOGF(LogAvaPlayable, Verbose,
					"%ls Remote Playable Transition \"%ls\": Marking playable instance Id \"%ls\" for stop on server \"%ls\".",
			...
			if (!bIsStopPlayableEvent || IsPlayableMarkedForStopOnAllServers(InArgs.InstanceId))
			{
				...
				{
					UE_LOGF(LogAvaPlayable, Verbose,
						"%ls Remote Playable Transition \"%ls\": Playable instance Id \"%ls\" was marked for stop on all servers. Broadcasting locally.",
		...
		else
		{
			UE_LOGF(LogAvaPlayable, Error,
				"%ls Remote Playable Transition \"%ls\" doesn't have playable instance Id \"%ls\".",
				*GetBriefFrameInfo(), *GetInstanceName(), *InArgs.InstanceId.ToString());
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogAvaPlayable, Verbose,
				"%ls Remote Playable Transition \"%ls\": Finish event received from on all servers. Broadcasting locally.",
