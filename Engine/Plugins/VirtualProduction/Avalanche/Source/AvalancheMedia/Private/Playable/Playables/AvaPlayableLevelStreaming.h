...
class UAvaPlayableLevelStreaming : public UAvaPlayable
{
	GENERATED_BODY()
public:
	enum class ECacheShadersOnLevelLoadMethod : uint8
	{
		/** No shaders will be cached on load */
		None,
		/** Shaders will be cached only on the first level load. Subsequent level loads will not cache shaders */
		FirstLoad,
		/** Shaders will be cached every load */
		EveryLoad,
	};
...
protected:
	...
	/** The method of caching shaders when loading the level */
	ECacheShadersOnLevelLoadMethod CacheShadersOnLevelLoadMethod = ECacheShadersOnLevelLoadMethod::None;
	...
	/** True if shaders are currently being compiled */
	bool bCachingShaders = false;

	/** True if shaders have been cached for the level at some point in the past */
	bool bHasEverCachedShaders = false;
