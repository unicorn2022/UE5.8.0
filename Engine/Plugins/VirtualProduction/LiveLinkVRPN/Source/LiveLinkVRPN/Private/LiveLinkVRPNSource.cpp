...
, SubjectName(FName(ConnectionSettings.SubjectName))
{
	...
	default:						UE_LOGF(LogLiveLinkVRPN, Error, "LiveLinkVRPNSource: Device type %d is invalid - this should never happen!", int(DeviceType));
										break;
	}
	...
	if (VRPNDevice.RawPointer != nullptr)
	{
		...
		UE_LOGF(LogLiveLinkVRPN, Log, "LiveLinkVRPNSource: Opened device %ls as type %ls (%d)", *DeviceName, *DeviceTypeToString(DeviceType), int(DeviceType));
	}
	else
	{
		UE_LOGF(LogLiveLinkVRPN, Error, "LiveLinkVRPNSource: Failed to create device or register change handler for device %ls as type %ls (%d)", *DeviceName, *DeviceTypeToString(DeviceType), int(DeviceType));
	}
}
...
FLiveLinkVRPNSource::~FLiveLinkVRPNSource()
{
	...
	if (VRPNDevice.RawPointer != nullptr)
	{
		...
		switch (DeviceType)
		{
		  ...
		  default:						UE_LOGF(LogLiveLinkVRPN, Error, "LiveLinkVRPNSource: Delete device type %d is invalid - this should never happen!", int(DeviceType));
											break;
		}

		if (bUnregisterFailed)
		{
			UE_LOGF(LogLiveLinkVRPN, Error, "LiveLinkVRPNSource: Failed to unregister change handler for device %ls as type %ls (%d)", *DeviceName, *DeviceTypeToString(DeviceType), int(DeviceType));
		}

		VRPNDevice.RawPointer = nullptr;
		UE_LOGF(LogLiveLinkVRPN, Log, "LiveLinkVRPNSource: Closed device %ls as type %ls (%d)", *DeviceName, *DeviceTypeToString(DeviceType), int(DeviceType));
	}
}
...
const FString& FLiveLinkVRPNSource::DeviceTypeToString(const EVRPNDeviceType InDeviceType)
{
	...
	switch (InDeviceType)
	{
		case EVRPNDeviceType::Analog:	return DeviceTypeString_Analog;
		case EVRPNDeviceType::Dial:		return DeviceTypeString_Dial;
		case EVRPNDeviceType::Button:	return DeviceTypeString_Button;
		case EVRPNDeviceType::Tracker:	return DeviceTypeString_Tracker;
	}
	...
	while (!Stopping)
	{
		if (VRPNDevice.RawPointer != nullptr)
		{
			...
			switch (DeviceType)
			{
				...
				default:						UE_LOGF(LogLiveLinkVRPN, Error, "LiveLinkVRPNSource: Run device type %d is invalid - this should never happen!", int(DeviceType));
												break;
			}
		}
	...
	if (!EncounteredSubjects.Contains(SubjectName))
	{
		if (NumProperties > 0)
		{
			...
			switch (DeviceType)
			{
				...
				case EVRPNDeviceType::Tracker:	UE_LOGF(LogLiveLinkVRPN, Error, "LiveLinkVRPNSource: Send device type Tracker should not have any properties - this should never happen!");
												break;

				default:						UE_LOGF(LogLiveLinkVRPN, Error, "LiveLinkVRPNSource: Send invalid device type %d - this should never happen!", int(DeviceType));
												break;
			}
