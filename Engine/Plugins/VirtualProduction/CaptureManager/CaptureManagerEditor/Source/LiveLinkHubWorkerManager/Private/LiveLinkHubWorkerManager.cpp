...
FConnectResponse* FLiveLinkHubWorkerManager::ConnectAccepted(const FConnectRequest& InRequest, const FMessageAddress& InAddress)
{
	...
	{
		UE_LOGF(LogLiveLinkHubWorkerManager, Display, "Connection accepted from: %ls", *InAddress.ToString());
...
{
	UE_LOGF(LogLiveLinkHubWorkerManager, Display, "Connection lost: %ls", *InAddress.ToString());
