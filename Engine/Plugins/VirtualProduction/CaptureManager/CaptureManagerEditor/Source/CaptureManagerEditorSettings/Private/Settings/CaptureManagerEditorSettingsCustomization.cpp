...
void FCaptureManagerEditorSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& InDetailBuilder)
{
	...
	// Video command tokens
	const TSharedRef<IPropertyHandle> VideoCommandTokensHandle = InDetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UCaptureManagerEditorSettings, VideoCommandTokens));
	IDetailPropertyRow* VideoCommandTokensRow = InDetailBuilder.EditDefaultProperty(VideoCommandTokensHandle);
	check(VideoCommandTokensRow);

	FNamingTokenFilterArgs VideoCommandArgs = DefaultArgs;
	if (const TObjectPtr<const UCaptureManagerVideoEncoderTokens> VideoTokens = GetDefault<UCaptureManagerEditorSettings>()->GetVideoEncoderNamingTokens())
	{
		VideoCommandArgs.AdditionalNamespacesToInclude.Add(VideoTokens->GetNamespace());
	}
	BuildSlate(VideoCommandTokensHandle, VideoCommandTokensRow, VideoCommandArgs);

	// Audio command tokens
	const TSharedRef<IPropertyHandle> AudioCommandTokensHandle = InDetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UCaptureManagerEditorSettings, AudioCommandTokens));
	IDetailPropertyRow* AudioCommandTokensRow = InDetailBuilder.EditDefaultProperty(AudioCommandTokensHandle);
	check(AudioCommandTokensRow);

	FNamingTokenFilterArgs AudioCommandArgs = DefaultArgs;
	if (const TObjectPtr<const UCaptureManagerAudioEncoderTokens> AudioTokens = GetDefault<UCaptureManagerEditorSettings>()->GetAudioEncoderNamingTokens())
	{
		AudioCommandArgs.AdditionalNamespacesToInclude.Add(AudioTokens->GetNamespace());
	}
	BuildSlate(AudioCommandTokensHandle, AudioCommandTokensRow, AudioCommandArgs);
