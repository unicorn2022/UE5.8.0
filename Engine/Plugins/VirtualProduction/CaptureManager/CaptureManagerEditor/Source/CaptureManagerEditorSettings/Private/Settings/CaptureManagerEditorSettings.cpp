...
bool UCaptureManagerEditorSettings::StartIngestServer()
{
	...
	UE_LOGF(LogCaptureManagerEditorSettings, Display, "%ls", *IngestServerStartingMessage.ToString());
	...
	UE_LOGF(LogCaptureManagerEditorSettings, Display, "%ls", *Message.ToString());
...
void UCaptureManagerEditorSettings::Initialize()
{
	...
	IModularFeatures::Get().OnModularFeatureUnregistered().AddWeakLambda(this,
		[](const FName& InFeatureName, IModularFeature* InFeature)
		{
			if (UObjectInitialized() && InFeatureName == ILiveLinkClient::ModularFeatureName)
			{
	...
	VideoEncoderNamingTokens = NewObject<UCaptureManagerVideoEncoderTokens>(this, UCaptureManagerVideoEncoderTokens::StaticClass());
	VideoEncoderNamingTokens->CreateDefaultTokens();

	AudioEncoderNamingTokens = NewObject<UCaptureManagerAudioEncoderTokens>(this, UCaptureManagerAudioEncoderTokens::StaticClass());
...
TObjectPtr<const UCaptureManagerVideoEncoderTokens> UCaptureManagerEditorSettings::GetVideoEncoderNamingTokens() const
{
	return VideoEncoderNamingTokens;
}

TObjectPtr<const UCaptureManagerAudioEncoderTokens> UCaptureManagerEditorSettings::GetAudioEncoderNamingTokens() const
{
	return AudioEncoderNamingTokens;
}

UCaptureManagerEditorSettings* UCaptureManagerEditorSettings::GetCaptureManagerEditorSettings()
{
	return GetMutableDefault<UCaptureManagerEditorSettings>();
}

#if WITH_EDITOR
void UCaptureManagerEditorSettings::SetMediaDirectory(const FDirectoryPath& InMediaDirectory)
{
	MediaDirectory = InMediaDirectory;

	// Set flag to indicate this is a programmatic change
	TGuardValue<bool> ProgrammaticSetterGuard(bInProgrammaticSetter, true);

	FProperty* MediaDirectoryProperty = FindFProperty<FProperty>(UCaptureManagerEditorSettings::StaticClass(), GET_MEMBER_NAME_CHECKED(UCaptureManagerEditorSettings, MediaDirectory));
	
	if (MediaDirectoryProperty)
	{
		FPropertyChangedEvent  MediaDirectoryChangedEvent = FPropertyChangedEvent(MediaDirectoryProperty, EPropertyChangeType::ValueSet);
		PostEditChangeProperty(MediaDirectoryChangedEvent);
	}
	else
	{
		FText MediaDirectoryFailedMessage = LOCTEXT("FailedToFindMediaDirectory", "Failed to find MediaDirectory property for reflection. Settings will save but PostEditChangeProperty notifications may not fire correctly.");
		UE_LOGF(LogCaptureManagerEditorSettings, Warning, "%ls", *MediaDirectoryFailedMessage.ToString());
		
		// Manually broadcast the delegate since PostEditChangeProperty won't be called.
		// This ensures listeners are still notified even when reflection data is unavailable.
		OnCaptureManagerEditorSettingsChanged.Broadcast();
	}
	SaveConfig();
}

void UCaptureManagerEditorSettings::SetImportDirectory(const FDirectoryPath& InImportDirectory)
{
	bool bIsValidPath = false;
	FText ValidationError;

	if (!InImportDirectory.Path.IsEmpty())
	{
		if (InImportDirectory.Path.EndsWith(TEXT("/")))
		{
			ValidationError = FText::Format(
				LOCTEXT("ImportDirectoryTrailingSlash",
					"Import Directory path cannot end with a trailing slash. Invalid path: {0}"),
					FText::FromString(InImportDirectory.Path));
		}
		else if (FPackageName::IsValidLongPackageName(InImportDirectory.Path, false))
		{
			FString InContentRoot;
			FString InRelativePath;
			FString InLeafName;

			if (FPackageName::SplitLongPackageName(InImportDirectory.Path, InContentRoot,
				InRelativePath, InLeafName))
			{
				// Note: This list covers the common read-only roots in both UE and UEFN.
				// In UEFN, /Game is also read-only (the UEFN writable root is /VerseDevices), but we
				// cannot detect the environment here without a dependency on GetBaseImportDirectory().
				// Callers in UEFN contexts should ensure they use appropriate paths.
				if (!InContentRoot.StartsWith(TEXT("/Engine")) &&
					!InContentRoot.StartsWith(TEXT("/Script")) &&
					!InContentRoot.StartsWith(TEXT("/Temp")))
				{
					bIsValidPath = true;
				}
				else
				{
					ValidationError = FText::Format(
						LOCTEXT("ImportDirectoryReadOnlyRoot",
							"Import Directory cannot be in a read-only content root: {0}"),
							FText::FromString(InContentRoot));
				}
			}
			else
			{
				ValidationError = FText::Format(
					LOCTEXT("ImportDirectoryCannotParse",
						"Could not parse import directory path: {0}"),
						FText::FromString(InImportDirectory.Path));
			}
		}
		else
		{
			ValidationError = FText::Format(
				LOCTEXT("ImportDirectoryInvalidPackageName",
					"Import Directory must be a valid package path. Invalid path: {0}"),
					FText::FromString(InImportDirectory.Path));
		}
	}
	else
	{
		ValidationError = LOCTEXT("ImportDirectoryEmpty",
			"Import Directory cannot be empty. Provide a valid writeable content path.");
	}

	if (!bIsValidPath)
	{
		UE_LOGF(LogCaptureManagerEditorSettings, Error, "%ls", *ValidationError.ToString());
		ResetImportDirectory();
	}
	else
	{
		ImportDirectory = InImportDirectory;
	}
	...
	FProperty* ImportDirectoryProperty =
		FindFProperty<FProperty>(UCaptureManagerEditorSettings::StaticClass(),
			GET_MEMBER_NAME_CHECKED(UCaptureManagerEditorSettings, ImportDirectory));

	if (ImportDirectoryProperty)
	{
		FPropertyChangedEvent  ImportDirectoryChangedEvent =
			FPropertyChangedEvent(ImportDirectoryProperty, EPropertyChangeType::ValueSet);
		PostEditChangeProperty(ImportDirectoryChangedEvent);
	}
	else
	{
		FText ImportDirectoryFailedMessage = LOCTEXT("FailedToFindImportDirectory", "Failed to find ImportDirectory property for reflection. Settings will save but PostEditChangeProperty notifications may not fire correctly.");
		UE_LOGF(LogCaptureManagerEditorSettings, Warning, "%ls", *ImportDirectoryFailedMessage.ToString());
		
		// PostEditChangeProperty normally broadcasts OnCaptureManagerEditorSettingsChanged,
		// but we couldn't call it due to missing reflection data. Broadcast manually to
		// ensure dependent systems are notified of the settings change.
		OnCaptureManagerEditorSettingsChanged.Broadcast();
	}
}
#endif
...
void UCaptureManagerEditorSettings::PostEditChangeProperty(struct FPropertyChangedEvent& InPropertyChangedEvent)
{
	...
	// Skip initialization if this change is coming from our programmatic setters
	if (!bInProgrammaticSetter)
	{
		InitializeValuesIfNotSet();
	}
...
void UCaptureManagerEditorSettings::InitializeValuesIfNotSet()
{
	...
	{
		MediaDirectory.Path = FPaths::Combine(FPlatformProcess::UserSettingsDir(), Private::DefaultMediaDirectory);
	}
	...
	// Default empty encoder command fields and migrate legacy placeholders
	if (CustomVideoCommandArguments.IsEmpty())
	{
		CustomVideoCommandArguments = FString(UE::CaptureManager::EncoderDefaults::VideoCommandArgs);
	}

	if (CustomAudioCommandArguments.IsEmpty())
	{
		CustomAudioCommandArguments = FString(UE::CaptureManager::EncoderDefaults::AudioCommandArgs);
	}

	CustomVideoCommandArguments.ReplaceInline(TEXT("{InputFileName}"), TEXT("{input}"));
	CustomVideoCommandArguments.ReplaceInline(TEXT("{OutputFileName}"), TEXT("{output}"));
	CustomVideoCommandArguments.ReplaceInline(TEXT("{ConversionFilters}"), TEXT("{params}"));

	CustomAudioCommandArguments.ReplaceInline(TEXT("{InputFileName}"), TEXT("{input}"));
	CustomAudioCommandArguments.ReplaceInline(TEXT("{OutputFileName}"), TEXT("{output}"));
}
...
void UCaptureManagerEditorSettings::OnHubConnectionEstablished(FGuid SourceId)
{
	...
	if (bLaunchIngestServerOnLiveLinkHubConnection)
	{
		...
		if (!LiveLinkHubExportServerModule.IsExportServerRunning())
		{
			...
			UE_LOGF(LogCaptureManagerEditorSettings, Display, "%ls", *ConnectedToHubMessage.ToString());
			StartIngestServer();
		}
	}
}
