	...
	for (int32 Index = 0; Index < OutTakes.Num(); ++Index)
	{
		...
		for (const TPair<UAssetImportTask*, FTakeAssetImportTaskInfo>& TakeImportTask : TakeTasks)
		{
			...
			if (ImportTaskInfo.AudioClipIndex != INDEX_NONE)
			{
				...
				{
					UE_LOGF(LogIngestAssetCreator, Display, "SoundWave asset created successfully");
				}
			}
		}
														  ...
														  FCaptureDataAssetInfo& OutTake)
{
	...
	for (const FCreateAssetsData::FImageSequenceData& ImageSequence : InCreateAssetData.ImageSequences)
	{
		...
		if (VideoAssetResult.HasError())
		{
			...
			UE_LOGF(LogIngestAssetCreator, Error, "Failed to create UImgMediaSource asset for image sequence '%ls'", *(ImageSequence.Name));
			continue;
		}
	...
	for (const FCreateAssetsData::FImageSequenceData& DepthSequence : InCreateAssetData.DepthSequences)
	{
		...
		if (DepthAssetResult.HasError())
		{
			...
			UE_LOGF(LogIngestAssetCreator, Error, "Failed to create UImgMediaSource asset for depth sequence '%ls'", *(DepthSequence.Name));
			continue;
		}
																		  ...
																		  const FCreateAssetsData::FImageSequenceData& InImageSequenceData)
{
	...
	{	
		...
		UE_LOGF(LogIngestAssetCreator, Error, "%ls", *ErrorText.ToString());
...
void FIngestAssetCreator::StampWithTakeMetadataTimecode(const FCreateAssetsData::FAudioData& InAudioClip, const UAssetImportTask* InAssetImportTask, USoundWave* OutSoundWave)
{
	...
	if (bIsValidSampleRate)
	{
		...
		else
		{
			UE_LOGF(
				...
				Error,
				"Audio timecode rate is invalid (%.2f): %ls",
	...
	else
	{
		UE_LOGF(
			LogIngestAssetCreator,
			Error,
			"Audio sample rate is invalid (%d Hz): %ls",
			SampleRate,
			*CreateAssetPathString(InAssetImportTask)
		);
	}
...
bool FIngestAssetCreator::IsValidAudioTimecodeRate(FFrameRate InTimecodeRate, uint32 InNumSamplesPerSecond)
{
	...
	{
		UE_LOGF(LogIngestAssetCreator, Warning, "Timecode rate in the imported audio is %.2f fps, perhaps a missing iXML chunk?", InTimecodeRate.AsDecimal());
	}
...
void FIngestAssetCreator::PrepareSoundWave(const FCreateAssetsData::FAudioData& InAudioClip, const UAssetImportTask* InAssetImportTask, USoundWave* OutSoundWave)
{
		...
		if (!bIsValidTimecodeRate)
		{
			if (InAudioClip.bTimecodePresent)
			{
				UE_LOGF(
					LogIngestAssetCreator,
					Warning,
					"Timecode is present in the audio file, but we could not determine a valid timecode rate (%ls). Falling back to the value extracted from the take metadata (%ls): %ls",
			...
			else
			{
				UE_LOGF(
					LogIngestAssetCreator,
					Warning,
					"Timecode is present in the audio file, but we could not determine a valid timecode rate (%ls). This will need to be fixed manually",
					*CreateTimecodeString(SoundWaveTimecode, SoundWaveTimecodeInfo->TimecodeRate)
				);
			}
		}
		else
		{
			if (InAudioClip.bTimecodePresent)
			{
				...
				UE_LOGF(
					LogIngestAssetCreator,
					Warning,
					"Ignoring the timecode extracted from the take metadata (%ls), the audio already has timecode (%ls): %ls",
					*CreateTimecodeString(InAudioClip.Timecode, InAudioClip.TimecodeRate),
					*CreateTimecodeString(SoundWaveTimecode, SoundWaveTimecodeInfo->TimecodeRate),
					*CreateAssetPathString(InAssetImportTask)
				);
	...
	else if (InAudioClip.bTimecodePresent)
	{
		...
		UE_LOGF(
			LogIngestAssetCreator,
			Display,
			"Imported audio does not have timecode, using the timecode extracted from the take metadata (%ls): %ls",
			*CreateTimecodeString(InAudioClip.Timecode, InAudioClip.TimecodeRate),
			*CreateAssetPathString(InAssetImportTask)
		);
	...
	if (!OutSoundWave->GetTimecodeInfo())
	{
		...
		UE_LOGF(
			LogIngestAssetCreator,
			Warning,
			"No audio timecode in sound wave asset: %ls",
			*CreateAssetPathString(InAssetImportTask)
		);
	}
}
...
#define CHECK_UOBJECT_WARN(Obj, Name) if (!Obj) { UE_LOGF(LogIngestAssetCreator, Warning, "Asset doesn't exist: %ls", *Name); }
#define CHECK_WARN(Condition, Message) if (!(Condition)) { UE_LOGF(LogIngestAssetCreator, Warning, "Checking ingest result failed: %ls", *Message.ToString()); }
		...
		if (!AssetsData.Calibrations.IsEmpty() && !AssetsData.ImageSequences.IsEmpty())
		{
			...
			for (const FCreateAssetsData::FCalibrationData& CalibAssetData : AssetsData.Calibrations)
			{
				for (const FCameraCalibration& Calib : CalibAssetData.CameraCalibrations)
				{
					...
					{
						UE_LOGF(LogIngestAssetCreator, Warning, "The ingested Camera Calibration contains a CameraId \"%ls\" which does not correspond to any of the ingested Image Sequences.\n"
						"Expected: \"%ls\", Found: \"%ls\"", *Calib.CameraId, *Calib.CameraId, *ImageSequenceNamesString);
					}
				}
			}
		}
...
bool FIngestAssetCreator::RemoveAssetsByPath(const FString& InPackagePath)
{
	...
	const int32 DeletedCount = ObjectTools::DeleteAssets(AssetData, false);
	if (DeletedCount != AssetData.Num())
	{
		UE_LOGF(LogIngestAssetCreator, Warning, "RemoveAssetsByPath: deleted %d of %d assets at '%ls'",
			DeletedCount, AssetData.Num(), *InPackagePath
		);
		return false;
	}
