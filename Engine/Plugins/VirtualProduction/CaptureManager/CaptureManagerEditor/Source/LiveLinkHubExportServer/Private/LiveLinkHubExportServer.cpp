...
bool FLiveLinkHubExportServer::Start(uint16 InPort)
{
	...
	{
		UE_LOGF(LogLiveLinkHubExportServer, Display, "LiveLink Hub ingest server running on port %d", SuccessPort.GetValue());
	}
...
bool FLiveLinkHubExportServer::Stop()
{
	...
	{
		UE_LOGF(LogLiveLinkHubExportServer, Display, "LiveLink Hub ingest server stopped");
	}
