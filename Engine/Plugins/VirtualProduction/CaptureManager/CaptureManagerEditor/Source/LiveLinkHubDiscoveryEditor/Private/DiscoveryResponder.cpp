	...
	, bStarted(false)
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FDiscoveryResponder::StartDiscoveryResponder);
}
...
)
{
	...
	UE_LOGF(LogLiveLinkHubDiscoveryEditor, Verbose, "Discovery request from %ls with endpoint ID: %ls", *InRequest.HostName, *InContext->GetSender().ToString());
}
