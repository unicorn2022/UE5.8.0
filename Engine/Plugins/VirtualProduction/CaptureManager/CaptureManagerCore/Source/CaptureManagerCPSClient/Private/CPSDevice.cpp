...
#define CHECK_LOG_RETURN_VAL(InCondition, InFormat, ...) \
	...
	{ \
		...
		UE_LOGF(LogCPSProtocolDevice, Warning, "%ls", *Message); \
...
FConnectionStateChangedEvent::~FConnectionStateChangedEvent() = default;
...
FCPSStateEvent::~FCPSStateEvent() = default;
...
void FCPSDevice::FetchFiles(TUniquePtr<FBaseStream> InStream, TArray<FString> InFileNames)
{
	...
	// Unlock earlier as there is no need to lock the ExportFiles function
	Lock.Unlock();
...
void FCPSDevice::ConnectControlClient(FEmpty)
{
	...
	{
		UE_LOGF(LogCPSProtocolDevice, Verbose, "Failed to connect to %ls:%d", *DeviceIpAddress, DeviceControlPort);
		return;
	}
	...
	UE_LOGF(LogCPSProtocolDevice, Display, "Connected to the CPS Device: %ls:%d", *DeviceIpAddress, DeviceControlPort);
}
...
void FCPSDevice::OnConnectTick()
{
	if (!bIsConnected.load())
	{
		UE_LOGF(LogCPSProtocolDevice, Log, "Connecting to the CPS Device: %ls:%d", *DeviceIpAddress, DeviceControlPort);
