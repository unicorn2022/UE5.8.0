...
public:
	...
	UE_API void SetAddressPath(const FString& InAddressPath);
...
class FListAllCapabilitiesResponse final : public FControlResponse
{
public:
	UE_API FListAllCapabilitiesResponse();

	UE_API TProtocolResult<void> Parse(TSharedPtr<FJsonObject> InBody) override;

	UE_API TArray<FCapability> GetCapabilities();

private:
	TArray<FCapability> Capabilities;

	TProtocolResult<FCapability> ParseCapability(const TSharedPtr<FJsonObject>& InJsonObject);
};

class FSetCapabilityPropertyResponse final : public FControlResponse
{
public:
	UE_API FSetCapabilityPropertyResponse();
};

class FGetCapabilityPropertyResponse final : public FControlResponse
{
public:
	UE_API FGetCapabilityPropertyResponse();

	UE_API TProtocolResult<void> Parse(TSharedPtr<FJsonObject> InBody) override;

	UE_API TProtocolResult<FCapabilityValue> GetValue(ECapabilityValue::Type InExpectedType) const;

private:
	TSharedPtr<FJsonObject> Body;
};

class FExecuteCapabilityCommandResponse final : public FControlResponse
{
public:
	UE_API FExecuteCapabilityCommandResponse();

	UE_API TProtocolResult<void> Parse(TSharedPtr<FJsonObject> InBody) override;

	UE_API TProtocolResult<FCapabilityValue> GetReturnValue(ECapabilityValue::Type InExpectedType) const;

private:
	TSharedPtr<FJsonObject> Body;
};

class FCapabilitySubscribeResponse final : public FControlResponse
{
public:
	UE_API FCapabilitySubscribeResponse();
};

class FCapabilityUnsubscribeResponse final : public FControlResponse
{
public:
	UE_API FCapabilityUnsubscribeResponse();
};
} // namespace UE::CaptureManager
#undef UE_API
