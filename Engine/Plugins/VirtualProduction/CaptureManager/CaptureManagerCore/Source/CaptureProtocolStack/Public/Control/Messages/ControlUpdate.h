...
class FEventUpdate final : public FControlUpdate
{
public:
	UE_API FEventUpdate(FString InAddressPath);

	UE_API virtual UE::CaptureManager::TProtocolResult<void> Parse(TSharedPtr<FJsonObject> InBody) override;

	UE_API const TProtocolResult<TMap<FString, FCapabilityValue>> GetArguments(const TArray<FEventArgumentDescriptor>& InArgumentsDescriptor) const;

private:
	TMap<FString, TSharedPtr<FJsonObject>> Arguments;
};
} // namespace UE::CaptureManager
