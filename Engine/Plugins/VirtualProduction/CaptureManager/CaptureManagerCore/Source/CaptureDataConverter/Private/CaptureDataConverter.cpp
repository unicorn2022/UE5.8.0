...
FCaptureDataConverterResult<void> FCaptureDataConverter::Run(FCaptureDataConverterParams InParams, FProgressReporter InProgressReporter)
{
	...
	NodeParams.StopToken = InParams.ExternalStopToken.IsSet()
		? *InParams.ExternalStopToken
		: StopRequester.CreateToken();
	...
	if (!TakeMetadata.Video.IsEmpty())
	{
		...
		for (const FTakeMetadata::FVideo& Video : Videos)
		{
			...
			if (InParams.VideoEncoderConfig)
			{
				const FCaptureManagerEncoderConfig& Cfg = *InParams.VideoEncoderConfig;
				Node = MakeShared<FCaptureConvertVideoDataThirdParty>(FVideoEncoderCommand(Cfg.EncoderPath, Cfg.EncoderArgs), Video, InParams.TakeOutputDirectory, NodeParams, VideoParams);
			}
			else
			{
	...
	if (!TakeMetadata.Audio.IsEmpty())
	{
		...
		for (const FTakeMetadata::FAudio& Audio : Audios)
		{
			...
			if (InParams.AudioEncoderConfig)
			{
				const FCaptureManagerEncoderConfig& Cfg = *InParams.AudioEncoderConfig;
				Node = MakeShared<FCaptureConvertAudioDataThirdParty>(FAudioEncoderCommand(Cfg.EncoderPath, Cfg.EncoderArgs), Audio, InParams.TakeOutputDirectory, NodeParams, AudioParams);
			}
			else
			{
