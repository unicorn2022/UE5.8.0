...
FCaptureConvertVideoDataThirdParty::FCaptureConvertVideoDataThirdParty(UE::CaptureManager::FVideoEncoderCommand InThirdPartyCommand,
...
FCaptureConvertVideoDataThirdParty::FResult FCaptureConvertVideoDataThirdParty::Run()
{
	...
	if (!Video.PathType.IsSet())
	{
		...
		UE_LOGF(LogCaptureConvertVideoDataThirdParty, Display, "PathType for %ls is unspecified, setting to detected type %ls", *Video.Path, *PathTypeString);
	}
	else
	{
...
FCaptureConvertVideoDataThirdParty::FResult FCaptureConvertVideoDataThirdParty::ConvertData()
{
	...
	if (!FPaths::DirectoryExists(DestinationDirectory))
	{
		...
		if (!bSuccess)
		{
			...
			UE_LOGF(LogCaptureConvertVideoDataThirdParty, Error, "%ls", *Message.ToString());
			return MakeError(MoveTemp(Message));
		}
	}
	...
	const FString ConversionArguments = Private::CreateConversionArguments(VideoParams.OutputPixelFormat, VideoParams.Rotation);
	FProcessRunnerResult RunnerResult = ThirdPartyCommand.Execute(VideoFilePath, ImageFilePath, ConversionArguments, &Params.StopToken);
	...
	if (RunnerResult.HasError())
	{
		...
		switch (RunnerError)
		{
			case EProcessRunnerError::LaunchFailed:
				Message = LOCTEXT("CaptureConvertVideoNodeTP_ProcessNotFound", "Failed to start the process");
				break;
			case EProcessRunnerError::Cancelled:
				Message = LOCTEXT("CaptureConvertVideoNodeTP_AbortedByUser", "Aborted by user");
				break;
			default:
				Message = LOCTEXT("CaptureConvertVideoNodeTP_ErrorRunning", "Error occurred while running the third party encoder.");
				break;
		}
		return MakeError(MoveTemp(Message));
	}
...
FCaptureConvertVideoDataThirdParty::FResult FCaptureConvertVideoDataThirdParty::CopyData()
{
	...
	ImagesToCopy.Sort([&Pattern](const FString& InLeft, const FString& InRight)
	{
		FString Dummy;
