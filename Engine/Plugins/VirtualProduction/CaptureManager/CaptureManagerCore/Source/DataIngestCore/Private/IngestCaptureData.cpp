...
#define INGEST_CAPTURE_DATA_CHECK_AND_RETURN(Condition, Message)						 \
	...
	{                                                                                    \
		...
		UE_LOGF(LogIngestCaptureData, Error, "%ls", *ErrorMessage.ToString());       \
...
void FIngestCaptureData::MakePathsAbsolute(const FString& InBasePath)
{
	for (FVideo& VideoEntry : Video)
	{
		VideoEntry.Path = FPaths::ConvertRelativePathToFull(InBasePath, VideoEntry.Path);
	}

	for (FVideo& DepthEntry : Depth)
	{
		DepthEntry.Path = FPaths::ConvertRelativePathToFull(InBasePath, DepthEntry.Path);
	}

	for (FAudio& AudioEntry : Audio)
	{
		AudioEntry.Path = FPaths::ConvertRelativePathToFull(InBasePath, AudioEntry.Path);
	}

	for (FCalibration& CalibrationEntry : Calibration)
	{
		CalibrationEntry.Path = FPaths::ConvertRelativePathToFull(InBasePath, CalibrationEntry.Path);
	}
}
