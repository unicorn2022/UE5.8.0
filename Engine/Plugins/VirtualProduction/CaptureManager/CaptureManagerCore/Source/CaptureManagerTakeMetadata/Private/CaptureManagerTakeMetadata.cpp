...
struct FTakeMetadataParser::FImpl
{
	...
	{
		using FPtr = TUniquePtr<FRapidjsonValidatorBundle>;
		...
		FRapidjsonValidatorBundle(FRapidjsonValidatorBundle&& InOther) = delete;
		FRapidjsonValidatorBundle(const FRapidjsonValidatorBundle& InOther) = delete;
		FRapidjsonValidatorBundle& operator=(FRapidjsonValidatorBundle&& InOther) = delete;
		FRapidjsonValidatorBundle& operator=(const FRapidjsonValidatorBundle& InOther) = delete;
	};
	...
	TOptional<FRapidjsonValidatorBundle::FPtr> VersionValidatorBundle;

	TMap<FTakeMetadata::FSchemaVersion, FRapidjsonValidatorBundle::FPtr> ValidatorBundles;

	TValueOrError<FRapidjsonValidatorBundle::FPtr, FTakeMetadataParserError> CreateValidator(const FString& InSchemaFileName)
	{
		...
		return MakeValue(MakeUnique<FRapidjsonValidatorBundle>(MoveTemp(Document)));
	}
	...
	TValueOrError<FTakeMetadata::FSchemaVersion, FTakeMetadataParserError> DetermineDocumentVersion(const rapidjson::Document& InDocument, const FString& InSchemasDir)
	{
		if (!VersionValidatorBundle.IsSet())
		{
			...
			TValueOrError<FRapidjsonValidatorBundle::FPtr, FTakeMetadataParserError> VersionValidatorResult = CreateValidator(VersionSchemaFilePath);
		...
		VersionValidatorBundle.GetValue()->Validator.Reset();
		VersionValidatorBundle.GetValue()->Validator.ResetError();

		TOptional<FTakeMetadataValidationError> ValidationError = Validate(InDocument, VersionValidatorBundle.GetValue()->Validator);
	...
	TValueOrError<FValidator, FTakeMetadataParserError> GetDocumentValidator(const rapidjson::Document& InDocument)
	{
		...
		FRapidjsonValidatorBundle::FPtr* ValidatorBundle = ValidatorBundles.Find(Version);
		...
		if (!ValidatorBundle)
		{
			...
			TValueOrError<FRapidjsonValidatorBundle::FPtr, FTakeMetadataParserError> ValidatorResult = CreateValidator(SchemaFilePath);
...
bool FTakeMetadataPathUtils::ValidatePathType(const FString& InPath, FTakeMetadata::FVideo::EPathType InPathType)
{
	...
	{
		UE_LOGF(LogTakeMetadataPathUtils, Warning, "Specified PathType \"Folder\" does not match detected type \"File\" for %ls", *InPath);
		return false;
	}
	else if (FileManager.DirectoryExists(*InPath) && bSpecifiedTypeIsFile)
	{
		UE_LOGF(LogTakeMetadataPathUtils, Warning, "Specified PathType \"File\" does not match detected type \"Folder\" for %ls", *InPath);
		return false;
	}
	return true;
}
