...
struct FLiveLinkFaceTakeInfo
{
	...
	// Set by TryResolveVideoFilePath when the metadata-derived filename does not match the actual file on disk.
	TOptional<FString> VideoFilePathOverride;
...
{
	if (VideoFilePathOverride.IsSet())
	{
		return VideoFilePathOverride.GetValue();
	}
...
bool FLiveLinkFaceMetadataParser::ParseTakeInfo(const FString& InTakeDirectory, FLiveLinkFaceTakeInfo& OutTakeInfo)
{
	...
	if (ParseResult.HasError())
	{
		...
		UE_LOGF(LogLiveLinkFaceMetadata, Warning, "Failed to parse take metadata: %ls", *Error.GetMessageString());
		return false;
	}

	if (!ParseResult.HasValue())
	{
		UE_LOGF(LogLiveLinkFaceMetadata, Error, "Take metadata parse result did not contain an error but no value was provided");
		return false;
	}
...
const FString FLiveLinkFaceMetadataParser::ParseJsonStringFromFile(const FString& InFilePath)
{
	...
	{
		UE_LOGF(LogLiveLinkFaceMetadata, Error, "File not found: %ls", *InFilePath);
		return {};
	}

	if (!FFileHelper::LoadFileToString(JsonStringBuffer, *InFilePath))
	{
		UE_LOGF(LogLiveLinkFaceMetadata, Error, "Failed to load file (check permissions): %ls", *InFilePath);
		return {};
	}
...
TSharedPtr<FJsonObject> FLiveLinkFaceMetadataParser::ParseJsonObjectFromString(const FString& InJsonString)
{
	...
	{
		UE_LOGF(LogLiveLinkFaceMetadata, Error, "Failed to parse json string (check for syntax errors): %ls", *InJsonString);
		return nullptr;
	}
		...
		for (const FString& Line : FrameLogLines)
		{
			...
			else if (LogEntry.EntryType() == FrameLogEntry::AudioType)
			{
				if (!bAudioTimecodeFound)
				{
					...
					// reducing risk for 5.7 we will maintain the current behavior.
...
// If the video file derived from take.json metadata does not exist on disk, scan the directory
// for a single .mov file and use it instead. Returns false only when multiple .mov files are
// found, as the correct one cannot be determined.
static bool TryResolveVideoFilePath(const FString& InTakeDirectory, FLiveLinkFaceTakeInfo& OutTakeInfo)
{
	const FString ExpectedVideoPath = OutTakeInfo.GetVideoFilePath();
	if (IFileManager::Get().FileExists(*ExpectedVideoPath))
	{
		return true;
	}

	TArray<FString> MovFiles;
	IFileManager::Get().FindFiles(MovFiles, *(InTakeDirectory / (TEXT("*") + FLiveLinkFaceStaticFileNames::VideoExt)), true, false);

	if (MovFiles.Num() > 1)
	{
		UE_LOGF(LogLiveLinkFaceMetadata, Warning,
			"Multiple .mov files found in take directory '%ls'; cannot determine correct video file for ingest",
			*InTakeDirectory);
		return false;
	}
	...
	if (MovFiles.Num() == 1)
	{
		UE_LOGF(LogLiveLinkFaceMetadata, Log,
			"Expected video file '%ls' not found; using '%ls' instead",
			*ExpectedVideoPath, *(InTakeDirectory / MovFiles[0]));
		OutTakeInfo.VideoFilePathOverride = InTakeDirectory / MovFiles[0];
	}

	// If no .mov found at all, leave the path as-is. Downstream ingest will surface the missing file error.
	return true;
}
...
TOptional<FTakeMetadata> ParseOldLiveLinkTakeMetadata(const FString& InTakeDirectory, TArray<FText>& OutValidationErrors)
{
	...
	REPORT_IF_ERROR_AND_RETURN(TryResolveVideoFilePath(InTakeDirectory, TakeInfo), LOCTEXT("ParseOldTakeMetadata_MultipleMovFiles", "Multiple .mov files found in take directory; cannot determine correct video file"));
...
TArray<FTakeMetadata::FVideo> ParseOldLiveLinkVideoMetadataFromString(const FString& InJsonString, TArray<FText>& OutValidationErrors)
{
	FLiveLinkFaceTakeInfo TakeInfo;
