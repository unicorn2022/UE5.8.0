...
static void ReportIssue(FText InMessage, const FTakeInfoType& InTakeInfo, TArray<FText>& OutIssues)
{
	...
	UE_LOGF(LogStereoCameraMetadata, Warning, "%ls", *WrapLogMessage(InMessage.ToString(), InTakeInfo));
...
)
{
	...
	switch (AddError)
	{
		...
		case FResolutionResolver::EAddError::InvalidImageWrapper:
			...
			UE_LOGF(LogStereoCameraMetadata, Warning, "%ls",
...
static TSharedPtr<FJsonObject> ParseJsonObjectFromFile(const FString& InFilePath)
{
	...
	{
		UE_LOGF(LogStereoCameraMetadata, Error, "File not found: %ls", *InFilePath);
		return nullptr;
	}

	if (!FFileHelper::LoadFileToString(JsonStringBuffer, *InFilePath))
	{
		UE_LOGF(LogStereoCameraMetadata, Error, "Failed to load file (check permissions): %ls", *InFilePath);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogStereoCameraMetadata, Error, "Failed to load json file (check for syntax errors): %ls", *InFilePath);
		return nullptr;
	}
...
static bool ParseCalibrationFile(const FString& InFileName, TMap<FString, FStereoCameraInfo>& OutCameras)
{
	...
	{
		UE_LOGF(LogStereoCameraMetadata, Warning, "Calibration file is empty: %ls", *InFileName);
		return false;
	}
...
static void CheckForNonANSICharacters(const FStereoCameraTakeInfo& InStereoCameraTakeInfo, TArray<FText>& OutIssues)
{
	...
	for (const FString& NonANSIContainingProperty : NonANSIContainingProperties)
	{
		...
		UE_LOGF(LogStereoCameraMetadata, Warning, "InStereoCameraTakeInfo property '%ls' contains unsupported non-ansi text characters.", *NonANSIContainingProperty);
	}
}
...
TOptional<FStereoCameraTakeInfo> FStereoCameraSystemTakeParser::ParseTakeMetadataFile(const FString& InFileName)
{
	...
	if (CurrentFileName == TEXT("take.json"))
	{
		if (TSharedPtr<FJsonObject> TakeMetadataJson = ParseJsonObjectFromFile(InFileName))
		{
			...
			{
				UE_LOGF(LogStereoCameraMetadata, Warning, "Failed to parse date string '%ls' as an ISO8601 date", *Date);
			}
...
TOptional<FText> CheckResolutions(const FTakeMetadata::FVideo& InCameraTakeInfo, const FCameraCalibration& InCameraCalibration)
{
	...
	{
		UE_LOGF(LogStereoCameraMetadata, Warning, "Could not check resolutions as FrameWidth and FrameHeight have not been set for %ls at %ls", *InCameraTakeInfo.Name, *InCameraTakeInfo.Path);
	}
