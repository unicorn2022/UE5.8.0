...
TArray<uint8> ConvertI420ToBGRA(const UE::CaptureManager::FMediaTextureSample* InSample)
{
	...
	TArray<uint8> NewBuffer;
	NewBuffer.SetNumUninitialized(InSample->Dimensions.X * InSample->Dimensions.Y * Channels);

	uint32 StrideY = InSample->Stride;
	uint32 StrideUV = InSample->Stride / 2;

	int32 BGRAStride = InSample->Dimensions.X * Channels;

	int32 BufferHeight = (InSample->Buffer.Num() * 2) / (InSample->Stride * 3);

	uint32 YPlaneSize = StrideY * BufferHeight;
	uint32 UPlaneSize = StrideUV * (BufferHeight / 2);
...
TArray<FColor> UEConvertI420ToBGRA(const UE::CaptureManager::FMediaTextureSample* InSample)
{
	...
	TArray<FColor> NewBuffer;
	NewBuffer.SetNumUninitialized(InSample->Dimensions.X * InSample->Dimensions.Y);

	uint32 StrideY = InSample->Stride;
	uint32 StrideUV = InSample->Stride / 2;

	int32 BGRAStride = InSample->Dimensions.X;
