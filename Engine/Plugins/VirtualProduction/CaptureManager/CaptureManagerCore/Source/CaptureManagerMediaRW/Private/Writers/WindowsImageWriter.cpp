...
#define WINIW_CHECK_AND_RETURN_ERROR_MESSAGE(Result, Message)                        \
	...
	{                                                                                \
		...
		UE_LOGF(LogWindowsImageWriter, Error, "%ls", *ErrorMessage.ToString());  \
		return ErrorMessage;														 \
	}
...
public:
							 ...
							 EMediaTexturePixelFormat InOutputFormat)
	{
		...
		if (SUCCEEDED(Result))
		{
			...
			if (OutPixelFormat != GUID_NULL)
			{
				...
				if (SUCCEEDED(Result) && bCanConvert)
				{
					...
					else
					{
						...
						UE_LOGF(LogWindowsImageWriter, Warning, "%ls", *Message);
					}
				}
				else
				{
			...
			{
				UE_LOGF(LogWindowsImageWriter, Warning, "Unsupported pixel format provided.");
			}
		}
		else
		{
			UE_LOGF(LogWindowsImageWriter, Warning, "Failed to create converter from WIC");
		}
...
TOptional<FText> FWindowsImageWriter::Append(UE::CaptureManager::FMediaTextureSample* InSample)
{
	...
	UE_LOGF(LogWindowsImageWriter, Verbose, "Output pixel format is %ls", *Private::GetPixelFormatString(InSample->CurrentFormat));
