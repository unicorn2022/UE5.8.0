...
public:
	...
	{
		DisplayName = TEXT("Live Link Face");
	}
...
UCLASS(BlueprintType, meta = (DisplayName = "Live Link Face Device", ToolTip = "Use for ingest from the Live Link Face app and control recording"))
...
public:
	...
	virtual TMap<TSubclassOf<ULiveLinkDeviceCapability>, TObjectPtr<UObject>> OnSaveDeviceCapabilityData() const override;
	virtual void OnLoadDeviceCapabilityData(TMap<TSubclassOf<ULiveLinkDeviceCapability>, TObjectPtr<UObject>> InCapabilityDataMap) override;
...
protected:
	...
	/** Override to configure device-specific properties on the RTSP media source (e.g. Path). Called during source creation. */
	virtual void ConfigureMediaSource(URtspMediaSource* InMediaSource) {}
