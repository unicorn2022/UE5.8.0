...
void UCaptureManagerSettings::InitializeValuesIfNotSet()
{
	...
	{
		DefaultWorkingDirectory.Path = FPaths::Combine(FPlatformProcess::UserSettingsDir(), Private::DefaultWorkingDirectory);
	}

	if (DownloadDirectory.Path.IsEmpty())
	{
		DownloadDirectory.Path = FPaths::Combine(FPlatformProcess::UserSettingsDir(), Private::DefaultDownloadDirectory);
	}
