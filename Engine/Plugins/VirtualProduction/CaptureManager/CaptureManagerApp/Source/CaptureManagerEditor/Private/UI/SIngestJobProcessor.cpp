...
void ResetJobs(TArray<TSharedRef<FIngestJob>> InIngestJobs, TSharedPtr<FIngestJobProcessor> InIngestJobProcessor)
{
	...
	{
		UE_LOGF(LogSIngestJobProcessor, Warning, "Failed to remove some jobs from the queue");
	}
	...
	{
		UE_LOGF(LogSIngestJobProcessor, Warning, "Failed to add some jobs to the queue");
	}
}
}
			...
			SNew(SVerticalBox)
							...
							SNew(SHorizontalBox)
								...
								[
									SNew(SButton)
										...
										{
#if !PLATFORM_WINDOWS
											if (!SIngestJobProcessor::IsThirdPartyEncoderConfigured())
											{
												return LOCTEXT("StartButton_DisabledTooltip",
													"Cannot start until Third Party Encoder is configured");
											}
#endif
											return LOCTEXT("StartButton_Tooltip", "Start ingest of the queued jobs.");
										})
				...
				[
					SNew(SWarningOrErrorBox)
						.Visibility(this, &SIngestJobProcessor::GetThirdPartyEncoderWarningVisibility)
						.MessageStyle(EMessageStyle::Warning)
						.Message(LOCTEXT("ThirdPartyEncoderNotConfigured", "Third Party Encoder is required to start processing"))
						[
							SNew(SButton)
								.TextStyle(FAppStyle::Get(), "DialogButtonText")
								.Text(LOCTEXT("ConfigureThirdPartyEncoderButton", "Configure"))
								.OnClicked_Lambda([]
								{
									FModuleManager::LoadModuleChecked<ISettingsModule>("Settings")
										.ShowViewer("Project", "Plugins", "Capture Manager");
...
#if !PLATFORM_WINDOWS
	if (!IsThirdPartyEncoderConfigured())
	{
		return false;
	}
#endif
...
bool SIngestJobProcessor::IsThirdPartyEncoderConfigured()
{
	const UCaptureManagerSettings* Settings = GetDefault<UCaptureManagerSettings>();
	return Settings->bEnableThirdPartyEncoder && !Settings->ThirdPartyEncoder.FilePath.IsEmpty();
}

EVisibility SIngestJobProcessor::GetThirdPartyEncoderWarningVisibility() const
{
#if PLATFORM_WINDOWS
	return EVisibility::Collapsed;
#else
	return IsThirdPartyEncoderConfigured() ? EVisibility::Collapsed : EVisibility::Visible;
#endif
}
