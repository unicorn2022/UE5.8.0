...
static EImageRotation ToImageRotation(EMediaOrientation InOrientation)
{
	switch (InOrientation)
	{
		case EMediaOrientation::CW90:  return EImageRotation::CW_90;
		case EMediaOrientation::CW180: return EImageRotation::CW_180;
		case EMediaOrientation::CW270: return EImageRotation::CW_270;
		default: return EImageRotation::None;
	}
}
} // namespace UE::CaptureManager
...
void FTakesView::AddTakesToIngestQueue(const TArray<TObjectPtr<UTakeVirtualAsset>>& InTakeAssets, const UIngestJobSettings& InJobSettings)
{
	...
	if (!Controller)
	{
		UE_LOGF(LogTakesView, Error, "Failed to add takes to the ingest queue, controller is not available");
		return;
	}
	...
	{
		EImageRotation ImageRotationToApply = InJobSettings.ImageRotation;
		if (!TakeAsset->Metadata.Video.IsEmpty() && TakeAsset->Metadata.Video[0].Orientation.IsSet())
		{
			ImageRotationToApply = ToImageRotation(UE::CaptureManager::ToUprightRotation(TakeAsset->Metadata.Video[0].Orientation.GetValue()));
		}
	...
	{
		UE_LOGF(LogTakesView, Error, "Some ingest jobs were not added to the queue (%d out of %d added)", NumAdded, ExpectedNumAdded);
	}
}
...
void FTakesView::Refresh()
{
	...
	if (SelectedItem.IsValid() && SelectedItem->Implements<ULiveLinkDeviceCapability_Ingest>())
	{
		RefreshSingleDevice(SelectedItem.Get());
	}
	else
	{
		RefreshAllDevices();
...
void FTakesView::RefreshSingleDevice(TObjectPtr<ULiveLinkDevice> InDevice)
{
	...
	TSharedRef<FUpdateTakeListManager> UpdateTakeListManager = Controller->GetUpdateTakeListManager();
	UpdateTakeListManager->UpdateTakeListForDevice(InDevice->GetDeviceId(), MoveTemp(Callback));
}

void FTakesView::RefreshAllDevices()
{
	TSharedPtr<FCaptureManagerPanelController> Controller = PanelController.Pin();
	...
	AsyncTask(ENamedThreads::GameThread, [this, Controller = PanelController.Pin(), Device = TStrongObjectPtr<ULiveLinkDevice>(InDevice)]()
		{
			...
			if (NumJobsRemoved > 0)
			{
				UE_LOGF(LogTakesView, Display, "Device '%ls' removed. Removed %d corresponding ingest jobs from the queue.", *Device->GetDisplayName().ToString(), NumJobsRemoved);
			}
...
void FTakesView::DeviceSelectionChanged(ULiveLinkDevice* InDevice)
{
	SelectedItem = InDevice;
	RefreshTakesViewDelegate.ExecuteIfBound(true);
}
