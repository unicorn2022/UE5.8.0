...
public:
	FJobsDetailsTabSummoner(TSharedPtr<FLiveLinkHubApplicationBase> InHostingApp, TSharedPtr<FCaptureManagerPanelController> InPanelController)
		...
		, CaptureManagerPanelController(MoveTemp(InPanelController))
	{
	...
	virtual TSharedRef<SWidget> CreateTabBody(const FWorkflowTabSpawnInfo& Info) const override
	{
		return CaptureManagerPanelController->GetIngestJobDetailsWidget();
	}

private:
	TSharedPtr<FCaptureManagerPanelController> CaptureManagerPanelController;
};
	...
	UnrealEndpointManager(FModuleManager::LoadModuleChecked<FCaptureManagerUnrealEndpointModule>("CaptureManagerUnrealEndpoint").GetEndpointManager())
{
	...
	TabLayout = FTabManager::NewLayout("LiveLinkCaptureManagerMode_v1.3");
	...
	CaptureManagerPanelController = FCaptureManagerPanelController::MakeInstance();

	TabFactories.RegisterFactory(MakeShared<FTakeBrowserTabSummoner>(App, CaptureManagerPanelController));
	TabFactories.RegisterFactory(MakeShared<FJobsListTabSummoner>(App, CaptureManagerPanelController));
	TabFactories.RegisterFactory(MakeShared<FJobsDetailsTabSummoner>(App, CaptureManagerPanelController));
}
