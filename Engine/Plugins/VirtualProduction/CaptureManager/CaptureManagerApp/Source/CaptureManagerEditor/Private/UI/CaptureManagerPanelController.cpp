	...
	IngestJobSettingsManager(MakeShared<UE::CaptureManager::FIngestJobSettingsManager>())
{
	...
	UpdateTakeListManager = FUpdateTakeListManager::Create();
...
{
	if (FModuleManager::Get().IsModuleLoaded("LiveLinkDevice"))
	{
		ILiveLinkDeviceModule::Get().OnSelectionChanged().Remove(DeviceSelectionChangedHandle);
	}
...
void FCaptureManagerPanelController::CreateViews()
{
	...
	DeviceSelectionChangedHandle = ILiveLinkDeviceModule::Get().OnSelectionChanged().AddSP(this, &FCaptureManagerPanelController::OnDeviceSelectionChanged);
}
...
TSharedRef<FUpdateTakeListManager> FCaptureManagerPanelController::GetUpdateTakeListManager() const
{
	return UpdateTakeListManager.ToSharedRef();
}

void FCaptureManagerPanelController::OnDeviceSelectionChanged(ULiveLinkDevice* InDevice)
{
	TakesView->DeviceSelectionChanged(InDevice);
}
...
void FCaptureManagerPanelController::OnJobsRemoved(const TArray<FGuid> JobGuids)
{
	...
	if (NumRemoved != JobGuids.Num())
	{
		UE_LOGF(
			...
			"Some ingest job settings were not removed from the settings manager (%d out of %d removed)",
			NumRemoved,
			JobGuids.Num()
		);
	}
...
void FCaptureManagerPanelController::OnFinishedEditingJobProperties(const FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	for (int32 Index = 0; Index < PropertyChangedEvent.GetNumObjectsBeingEdited(); Index++)
	{
		if (const UIngestJobSettings* Settings = Cast<const UIngestJobSettings>(PropertyChangedEvent.GetObjectBeingEdited(Index)))
		{
			...
			{
				UE_LOGF(LogCaptureManagerPanelController, Warning, "Failed to update job settings. Job may not exist or may already be in progress");
			}
		}
	}
}
