...
bool FAugmentedTakeUploadTask::Start(FLiveLinkHubExportClient::FTakeUploadParams InUploadParams)
{
	...
	if (ExportManagerWorkerID != UnsetWorkerID)
	{
		UE_LOGF(
			...
			Error,
			"%ls Failed to start upload. An upload with this ID is already in progress",
			*LogPrefix
		);
		return false;
	}
	...
	if (SharedExportManager)
	{
		...
		if (WorkerID == INDEX_NONE)
		{
			UE_LOGF(
				LogCaptureManagerUnrealEndpoint,
				Error,
				"%ls Failed to start upload. The export manager returned %d for the worker ID",
				*LogPrefix,
				WorkerID
			);
			return false;
	...
	else
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls Failed to start upload. The export manager for this task has been destroyed",
			*LogPrefix
		);
		return false;
	}
...
bool FAugmentedTakeUploadTask::Abort()
{
	...
	if (SharedExportManager)
	{
		...
		else
		{
			UE_LOGF(
				LogCaptureManagerUnrealEndpoint,
				Error,
				"%ls Failed to abort upload. The export manager could not find a worker with this ID",
				*LogPrefix
			);
			return false;
		}
	...
	else
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls Failed to abort upload. The export manager for this take has been destroyed",
			*LogPrefix
		);
		return false;
	}
...
void FUnrealEndpoint::FImpl::StartConnection()
{
	...
	if (bIsRetired)
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Warning,
			"%ls Ignoring request to start connection, this endpoint has been retired",
			*LogPrefix
		);
		return;
	}
	...
	for (TSharedRef<FAugmentedTakeUploadTask>& TaskToAbort : TasksToAbort)
	{
		...
		if (!bIsAborted)
		{
			UE_LOGF(
				LogCaptureManagerUnrealEndpoint,
				Warning,
				"%ls Failed to abort take upload task %ls while stopping the connection",
				*LogPrefix,
			...
			if (InResult.HasError())
			{
				...
				else
				{
					UE_LOGF(
						LogCaptureManagerUnrealEndpoint,
						Error,
						"Failed to report export manager error to the endpoint, the endpoint has been destroyed: ErrorMessage=%ls, ErrorCode=%d",
...
bool FUnrealEndpoint::FImpl::AddTakeUploadTask(TUniquePtr<FTakeUploadTask> InTakeUploadTask)
{
	...
	if (!ensure(InTakeUploadTask))
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls Failed to upload take, the supplied task was invalid (nullptr)",
			*LogPrefix
		);
		return false;
	}
	...
	if (bIsRetired)
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls Failed to upload take, the endpoint has been retired",
			*LogPrefix
		);
		return false;
	}
	...
	if (!ensure(Messenger))
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls, Failed to upload take, the underlying messenger has not been initialized",
			*LogPrefix
		);
		return false;
	}
	...
	if (!ensure(bParseOk))
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls Failed to upload take, we failed to parse the address of the messenger",
			*LogPrefix
		);
		return false;
	}
...
void FUnrealEndpoint::FImpl::CancelTakeUploadTask(const FGuid InTakeUploadTaskID)
{
	...
	if (bIsRetired)
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls Failed to cancel task %ls. This endpoint has been retired",
			*LogPrefix,
	...
	if (FoundTask)
	{
		...
		else
		{
			UE_LOGF(
				LogCaptureManagerUnrealEndpoint,
				Error,
				"%ls Failed to update completion state for task %ls (task has already been destroyed). "
					"Completion state was (Message=%ls, Code=%d)",
				*LogPrefix,
...
)
{
	...
	if (TakeUploadTask)
	{
		...
		else
		{
			UE_LOGF(
				LogCaptureManagerUnrealEndpoint,
				Error,
				"%ls Failed to update progress for task %ls (task has already been destroyed). Progress was %.4f",
				*LogPrefix,
				*InTakeUploadID.ToString(),
				InProgress
			);
	...
	else
	{
		UE_LOGF(
			LogCaptureManagerUnrealEndpoint,
			Error,
			"%ls Failed to update progress for task %ls (task not found). Progress was %.4f",
			*LogPrefix,
			*InTakeUploadID.ToString(),
			InProgress
		);
...
void FUnrealEndpoint::FImpl::OnMessengerConnectResponse(const FConnectResponse& InResponse)
{
	...
	if (!bIsConnected)
	{
		UE_LOGF(LogCaptureManagerUnrealEndpoint, Error, "%ls Failed to connect to the client", *LogPrefix);
	}
}
