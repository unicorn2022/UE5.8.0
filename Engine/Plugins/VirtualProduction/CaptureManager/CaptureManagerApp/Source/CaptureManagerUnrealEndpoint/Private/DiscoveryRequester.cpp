...
TUniquePtr<FDiscoveryRequester::FImpl> FDiscoveryRequester::FImpl::Create(FString InLocalHostName)
{
	...
	if (!MessageEndpoint)
	{
		UE_LOGF(
			...
			Warning,
			"Failed to create message endpoint, discovery will be disabled"
		);
...
)
{
	...
	UE_LOGF(LogLiveLinkHubDiscovery, Verbose, "Getting discovery response from client: %ls, (%ls)", *InContext->GetSender().ToString(), *InResponse.HostName);
	...
	if (bParseOK)
	{
		...
		UE_LOGF(LogLiveLinkHubDiscovery, Display, "New client discovered %ls with IP address %ls and endpoint ID: %ls", *Client.GetHostName(), *Client.GetIPAddress(), *Client.GetClientID().ToString());
...
TUniquePtr<FDiscoveryRequester> FDiscoveryRequester::Create()
{
	...
	if (!LocalHostName.IsSet())
	{
		UE_LOGF(
			LogLiveLinkHubDiscovery,
			Warning,
			"Failed to determine local host name, discovery will be disabled"
		);
		return nullptr;
	}
	...
	if (LocalHostName.GetValue().IsEmpty())
	{
		UE_LOGF(
			LogLiveLinkHubDiscovery,
			Warning,
			"The local host name was invalid (empty), discovery will be disabled"
		);
		return nullptr;
	}
