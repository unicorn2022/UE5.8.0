...
static FString ResolveCommandArgs(const FStringView InDefaultArgs, const FString& InCustomArgs, const FString& InTokenNamespace)
{
	FString CommandArgs = InCustomArgs.IsEmpty() ? FString(InDefaultArgs) : InCustomArgs;

	if (!GEngine)
	{
		UE_LOGF(LogIngestLiveLinkDevice, Warning, "Cannot resolve encoder command tokens: GEngine is unavailable");
		return CommandArgs;
	}
	...
	{
		UE_LOGF(LogIngestLiveLinkDevice, Warning, "Cannot resolve encoder command tokens: NamingTokensSubsystem is unavailable");
		return CommandArgs;
	}

	FNamingTokenFilterArgs FilterArgs;
	FilterArgs.AdditionalNamespacesToInclude.Add(InTokenNamespace);
	FilterArgs.bNativeOnly = true;

	CommandArgs = NamingTokensSubsystem->EvaluateTokenString(CommandArgs, FilterArgs).EvaluatedText.ToString();

	return CommandArgs;
}

static void ApplyThirdPartyEncoderSettings(const UCaptureManagerSettings* InSettings, FCaptureDataConverterParams& OutParams)
{
	if (!InSettings || !InSettings->bEnableThirdPartyEncoder || InSettings->ThirdPartyEncoder.FilePath.IsEmpty())
	{
		return;
	}

	const bool bThirdPartyEncoderIsAvailable = FPlatformProcess::ExecProcess(*InSettings->ThirdPartyEncoder.FilePath, TEXT("-version"), nullptr, nullptr, nullptr);

	if (bThirdPartyEncoderIsAvailable)
	{
		// {input}, {output} and {params} tokens will remain and are resolved with an FString::Format in the encoder command's Execute() function
		if (const UCaptureManagerAudioEncoderTokens* AudioTokens = InSettings->GetAudioEncoderNamingTokens())
		{
			const FString CommandArgs = ResolveCommandArgs(UE::CaptureManager::EncoderDefaults::AudioCommandArgs, InSettings->CustomAudioCommandArguments, AudioTokens->GetNamespace());
			OutParams.AudioEncoderConfig = FCaptureManagerEncoderConfig{InSettings->ThirdPartyEncoder.FilePath, CommandArgs};
		}

		if (const UCaptureManagerVideoEncoderTokens* VideoTokens = InSettings->GetVideoEncoderNamingTokens())
		{
			const FString CommandArgs = ResolveCommandArgs(UE::CaptureManager::EncoderDefaults::VideoCommandArgs, InSettings->CustomVideoCommandArguments, VideoTokens->GetNamespace());
			OutParams.VideoEncoderConfig = FCaptureManagerEncoderConfig{InSettings->ThirdPartyEncoder.FilePath, CommandArgs};
		}
	}
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FBaseIngestDevice_CleanUpWorkingDirectory);
	...
	if (InWorkingDirectory.IsEmpty())
	{
		UE_LOGF(
			...
			"Refused to clean up the ingest working directory. The supplied path was an empty string, which could "
				"have resulted in the unintended removal of the current working directory: %ls",
			*NormalizedFullPath
		);
	...
	{
		UE_LOGF(LogIngestLiveLinkDevice, Display, "Deleting directory: %ls", *NormalizedFullPath);
		...
		if (bIsDeleted)
		{
			UE_LOGF(LogIngestLiveLinkDevice, Display, "Directory deleted: %ls", *NormalizedFullPath);
		}
		else
		{
			const uint32 ErrorCode = FPlatformMisc::GetLastError();
			UE_LOGF(LogIngestLiveLinkDevice, Error, "Failed to delete directory (Error code: %u): %ls", ErrorCode, *NormalizedFullPath);
		}
	}
}
...
void UBaseIngestLiveLinkDevice::FImpl::IngestTake(const UIngestCapability_ProcessHandle* InProcessHandle, const UIngestCapability_Options* InIngestOptions, TSharedPtr<UE::CaptureManager::FTaskProgress> InTaskProgress)
{
	...
	if (!MaybeTakeMetadata.IsSet())
	{
		...
		UE_LOGF(LogIngestLiveLinkDevice, Error, "%ls", *Message);
		Owner->ExecuteProcessFinishedReporter(InProcessHandle, MakeError(FIngestCapability_Error::InvalidArgument, MoveTemp(Message)));
		return;
	}
																				  ...
																				  FCaptureDataConverterParams& InParams)
{
	...
	VideoParams.OutputPixelFormat = ToMediaPixelFormat(InIngestOptions->Video.PixelFormat);
	VideoParams.Rotation = ToMediaOrientation(InIngestOptions->Video.Rotation);
	...
	DepthParams.Rotation = ToMediaOrientation(InIngestOptions->Video.Rotation); // Apply the same rotation as video to depth stream
	...
	{
		Private::ApplyThirdPartyEncoderSettings(CaptureManagerSettings, InParams);
	}
...
{
	Execute_RequestAbortUpdateTakeList(this);
