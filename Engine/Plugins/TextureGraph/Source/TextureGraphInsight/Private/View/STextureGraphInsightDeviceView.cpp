...
public:
	...
	void Refresh()
	{
		if (_recordID.IsDevice())
		{
			...
			else
			{
				...
				{ // This is a true device item, update the data
					...
					if (count == 0)
						SetColorAndOpacity(FLinearColor(0.7f, 0.7f, 0.7f));
					if (count != item->_children.Num())
						SetColorAndOpacity(FLinearColor(1.0f, 0.5f, 0.5f));
					else
		...
		else
		{
			const auto& br = TextureGraphInsight::Instance()->GetSession()->GetRecord().GetBuffer(_recordID);
			if (br.bLeaked)
				SetColorAndOpacity(FLinearColor(1.0f, 0.8f, 0.5f)); 
			else if (br.bErased)
				SetColorAndOpacity(FLinearColor(0.5f, 0.5f, 0.5f));
			else
				SetColorAndOpacity(FLinearColor(0.5f, 1.0f, 1.0f));
