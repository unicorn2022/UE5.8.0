...
DeviceBufferPtr TextureGraphInsightSession::LiveCache::GetDeviceBuffer(RecordID RecId) const
{
	if (RecId.IsBuffer())
	{
		...
		if (!Buffer) 
		{
			...
			{
				UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightActionManagerObserver::GetDeviceBuffer find the buffer from hash but no buffer");
			}
		}
		...
		if (!Buffer) {
			if (BufferRecord.PrevHashValue != 0)
			{
				...
				{
					UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightActionManagerObserver::GetDeviceBuffer find the buffer from prevHash but no buffer");
				}
			}
			else
			{
				UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightActionManagerObserver::GetDeviceBuffer cannot find the buffer in the cache, missing second hash");
			}
		}
...
void TextureGraphInsightSession::EngineCreated()
{
	...
	UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightActionManagerObserver::EngineCreated");
...
void TextureGraphInsightSession::EngineDestroyed()
{
	...
	UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightActionManagerObserver::EngineDestroyed");
...
uint32 TextureGraphInsightSession::ParseDeviceBuffersCache(DeviceType DevType, RecordIDArray& AddedBufferIDs)
{
	...
	if (NumNewBuffers)
	{
		...
		UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::ParseDeviceBuffersCache[%ls]: %d new unknown buffers", *t, NumNewBuffers);
	}
...
void TextureGraphInsightSession::DeviceBuffersUpdated(DeviceType DevType, TextureGraphInsightDeviceObserver::HashNDescArray&& AddedBuffers, TextureGraphInsightDeviceObserver::HashArray&& RemovedBuffers)
{
	...
	for (auto& AddedBuffer : AddedBuffers)
	{
		...
		UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::DeviceBuffersAdded : ptr 0x%llx - hash %llu", key, AddedBuffer.First);
		...
		if (RecId.IsValid())
		{
			if (bAdded)
			{
				UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::DeviceBuffersAdded : %d - hash %llu", RecId.Buffer(), DevBuffRec.HashValue);
				AddedRecIds.emplace_back(RecId);
			}
			else
			{
				UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::DeviceBuffers Updated hash : %d - hash %llu", RecId.Buffer(), DevBuffRec.HashValue);
			}
		}
		else
		{
			UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::DeviceBuffersAdded ERROR invalid buffer: ptr 0x%llx - hash %llu", DevBuffRec.ID, DevBuffRec.HashValue);
		}
	}
	...
	for (int i = 0; i < RemovedBuffers.size(); ++i)
	{
		...
		UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::DeviceBuffersRemoved : ptr 0x%llx - hash %llu - prevhash %llu", bp, h, ph);
		auto RecId = GetRecord().EraseDeviceBufferRecord(DevType, bp);
		if (RecId.IsValid())
		{
			UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::DeviceBuffersRemoved : %d", RecId.Buffer());
			RemovedRecIds.emplace_back(RecId);
		}
		else
		{
			UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::DeviceBuffersAdded ERROR invalid buffer ?");
		}
	}
...
//		UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightSession::BlobberHashesUpdated::Added hash %llu", a);
...
//		auto n = FindOrCreateBlobRecordFromHash(a);
	...
	{
		UE_LOGF(LogTextureGraphInsight, VeryVerbose, "TextureGraphInsightSession::BlobberHashesUpdated::Remapped hash old %llu ==> new %llu", RemappedHashes[2 * i], RemappedHashes[2 * i + 1]);
...
{
	UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightSchedulerObserver::BatchAdded");
}

void TextureGraphInsightSession::BatchDone(JobBatchPtr Batch)
{
	UE_LOGF(LogTextureGraphInsight, Log, "TextureGraphInsightSchedulerObserver::BatchDone");
