...
void UTG_Expression::Serialize(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogTextureGraph, VeryVerbose, "Detected an expression in a previous version");
	}
}
...
void UTG_Expression::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	UE_LOGF(LogTextureGraph, VeryVerbose, "Expression PostEditChangeProperty. ChangeType: %d", (int32)PropertyChangedEvent.ChangeType);
...
void UTG_Expression::LogEvaluation(FTG_EvaluationContext* InContext)
{
	...
	UE_LOGF(LogTextureGraph, VeryVerbose, "%-*ls %ls", UTG_Graph::LogHeaderWidth, *GetDefaultName().ToString(), *UTG_Graph::LogCall(InVarIds, OutVarIds));
}
