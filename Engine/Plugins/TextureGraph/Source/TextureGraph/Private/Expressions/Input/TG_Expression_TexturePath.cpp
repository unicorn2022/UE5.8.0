...
void UTG_Expression_TexturePath::Evaluate(FTG_EvaluationContext* InContext)
{
	...
	// Determine if we actually need to reload
	// Key insight: Don't touch Output unless we're going to replace it!
	bool bOutputIsValid = (Output.Num() > 0) && 
						  Output.Get(0).GetTexture().RasterBlob && 
						  Output.Get(0).GetTexture().RasterBlob->IsValid();
    
	bool bPathChanged = (LocalPath != OutputPath);
	bool bNeedsReload = bPathChanged || !bOutputIsValid;
	...
	if (bNeedsReload)
	{
		if (!Path.IsEmpty() && bValidation)
		{
			...
			else if (FPaths::DirectoryExists(LocalPath))
			{
				...
				{
					// Directory exists but empty - set black
					Output.SetNum(1);
			...
			{
				// Path validation passed but file/dir doesn't exist (edge case)
				Output.SetNum(1);
				Output.Set(0, FTG_Texture::GetBlack());
			}
		}
		...
		{
			// Invalid or empty path - set black as fallback
			Output.SetNum(1);
			Output.Set(0, FTG_Texture::GetBlack());
		}
