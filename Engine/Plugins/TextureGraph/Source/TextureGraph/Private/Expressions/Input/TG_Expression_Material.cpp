...
void UTG_Expression_Material::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		UE_LOGF(LogTextureGraph, VeryVerbose, "Material Expression PostEditChangeProperty.");
...
void UTG_Expression_Material::Initialize()
{
	// If the referenced material is valid, then we need to recreate a duplicate
 	if (InputMaterial.IsValid() && InputMaterial.GetMaterial() != nullptr)
	{
		MaterialCopy = DuplicateObject(InputMaterial.GetMaterial(), this);
		MaterialCopy->ClearFlags(RF_Standalone | RF_Public);
		MaterialCopy->SetFlags(RF_Transient);
	}
...
void UTG_Expression_Material::SetMaterialInternal(UMaterialInterface* InMaterial)
{
	...
	else if (InMaterial->IsA<UMaterial>() || InMaterial->IsA<UMaterialInstance>())
	{
...
void UTG_Expression_Material::SetInputMaterial(const FTG_Material& InParamMaterial)
{
	...
	if (InputMaterial == InParamMaterial)
	{
		...
		if ((!InputMaterial.IsValid()) || (InputMaterial.IsValid() && MaterialCopy && MaterialPermutations.IsValid()))
			return;
	}
...
{
	if (UTG_Node* ParentNode = GetParentNode())
	{
		if (UTG_Pin* InputMaterialPin = ParentNode->GetInputPin("InputMaterial"))
		{
			InputMaterialPin->SetAliasName(NewName);
			TitleName = InputMaterialPin->GetAliasName();
		}
	}
}
