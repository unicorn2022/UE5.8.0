...
{
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Node::PostEditChangeProperty.");
}
...
{
	UE_LOGF(LogTextureGraph, Verbose, "UTG_Node::PropertyChangedEvent ChangeType: %d", PropertyChangedEvent.ChangeType);
...
void UTG_Node::Serialize(FArchive& Ar)
{
	...
	if (!Expression)
	{
		UE_LOGF(LogTextureGraph, Verbose, "    %ls Node: NUll Expression???",
	...
	UE_LOGF(LogTextureGraph, VeryVerbose, "    %ls Node: %ls",
		(Ar.IsSaving() ? TEXT("Saved") : TEXT("Loaded")),
		*GetId().ToString());
}
	...
	for (auto Pin : Pins)
	{
		if (Pin->IsOutput())
		{
			...
			if (!Pin->IsArgArray())
			{
				...
				{
					UE_LOGF(LogTextureGraph, Warning, "Output %ls failed to collect as variant", *(Pin->GetAliasName().ToString()));
				}
			}
			else
			{
...
int UTG_Node::GetAllOutputValues(TArray<FTG_Texture>& OutTextures, TArray<FName>* OutNames) const
{
	...
	for (auto Pin : Pins)
	{
		if (Pin->IsOutput() && Pin->IsArgTexture())
		{
			...
			{
				UE_LOGF(LogTextureGraph, Log, "Output %ls failed to collect as Texture", *(Pin->GetAliasName().ToString()));
			}
		}
	}
