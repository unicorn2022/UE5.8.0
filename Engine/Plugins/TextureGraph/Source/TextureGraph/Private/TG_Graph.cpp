...
{
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Graph::PostEditChangeProperty.");
}
...
bool UTG_Graph::Modify(bool bAlwaysMarkDirty)
{
	...
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Graph::Modify: Graph Modified.");
...
{
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Graph::PostEditUndo.");
...
void UTG_Graph::Serialize(FArchive& Ar)
{
	...
	UE_LOGF(LogTextureGraph, VeryVerbose, "  %ls Graph: %ls",
...
void UTG_Graph::PostLoad()
{
	...
	UE_LOGF(LogTextureGraph, VeryVerbose, "  PostLoad Graph: %ls", *Name);
	...
	for (uint32 i = 0; i < (uint32)Nodes.Num(); ++i)
	{
		...
		if (Node)
		{
			...
			if (!PinsMatchSignature)
			{
				...
				UE_LOGF(LogTextureGraph, VeryVerbose, "Node %ls serialized signature is different from expression", *Node->GetId().ToString());
...
void UTG_Graph::PreSave(FObjectPreSaveContext SaveContext)
{
	...
	UE_LOGF(LogTextureGraph, VeryVerbose, "  PreSave Graph: %ls", *Name);
}
...
{
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Graph::Node Changed")
	NotifyGraphChanged(InNode, bIsTweaking);
}

void UTG_Graph::OnNodeSignatureChanged(UTG_Node* InNode)
{
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Graph::Node Recreate");
...
{
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Graph::Pin Changed")
	...
	ForEachParams([&](const UTG_Pin* Pin, uint32 Index) {
		if (Pin->IsOutput())
		{
			if (Pin->IsArgVariant())
			{
				...
				{
					UE_LOGF(LogTextureGraph, VeryVerbose, "Output %ls variant failed to access", *(Pin->GetAliasName().ToString()));
				}
			}
		}
		});
...
{
	UE_LOGF(LogTextureGraph, VeryVerbose, "UTG_Graph::GetTraversal: Dirty = %ls",
...
void UTG_Graph::Log() const
{
	...
	{
		UE_LOGF(LogTextureGraph, Display, "%ls", *Line);
	}
}
