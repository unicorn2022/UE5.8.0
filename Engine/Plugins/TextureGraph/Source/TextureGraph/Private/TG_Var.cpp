...
template <> FString TG_Var_LogValue(TObjectPtr<UObject>& Value)
{
	if (Value.Get())
	{
		...
		return FString::Printf(TEXT("%s [%s]"), *Name, *Value->GetName());
	}
	else
		return TEXT("nullptr");
}
...
{
	return FString::Printf(TEXT("v%s"), *GetId().ToString());
}
...
void VarPropertySerializer_StructProperty(FTG_Var::VarPropertySerialInfo& Info)
{
	...
	if (!WriterIt)
	{
		...
		UE_LOGF(LogTextureGraph, Warning, "Fails serialize Var %ls - Property %ls FPClass %ls CPPType %ls",
			*Info.Var->LogHead(),
...
void Generic_Simple_ArSerializer(FTG_Var::VarArchiveSerialInfo& Info)
{
	...
	if (Info.Ar.IsSaving())
		UE_LOGF(LogTextureGraph, VeryVerbose, "        Save Var %ls: %ls", *Info.Var->GetId().ToString(), *Info.Var->LogValue());
	T_ValueType& Value = Info.Var->EditAs<T_ValueType>();
	Info.Ar << Value;
	if (Info.Ar.IsLoading())
		UE_LOGF(LogTextureGraph, VeryVerbose, "        Loaded Var %ls: %ls", *Info.Var->GetId().ToString(), *Info.Var->LogValue());
...
void FTG_Var::Serialize(FArchive& Ar, FTG_Id InPinId, const FTG_Argument& InArgument)
{
	...
	if (InArgument.IsPersistentSelfVar())
	{
		...
		{
			UE_LOGF(LogTextureGraph, Warning, "serialize Var %ls: NOT FOUND for %ls", *LogHead(), *InArgument.GetCPPTypeName().ToString());
		}
	}
}
