...
bool FxMaterial_DrawMaterialBase::ValidateMaterialShaderMap(UMaterial * InMaterial, FShaderType* InPixelShaderType)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Material [%ls]: Require the <SceneTexturesStructs> which is not supported by the TextureGraph pipeline", *InMaterial->GetName());
		IsSupported = false;
	}
	if (MaterialShaderMap->NeedsGBuffer())
	{
		UE_LOGF(LogTexture, Warning, "Material [%ls]: Require the <GBuffer> which is not supported by the TextureGraph pipeline", *InMaterial->GetName());
		IsSupported = false;
	}
...
//#define ONLY_IN_DEBUG_CHECK_SHADER_DATA
	...
	//UE_LOGF(LogTexture, Warning, "Material[%ls]:\n[%ls]", *InMaterial->GetName(), MaterialShaderMap->GetDebugDescription());


	const FShaderMapBase* ShaderMap = PixelShader.GetShaderMap();
	
	//UE_LOGF(LogTexture, Warning, "Material[%ls]:\n[%ls]", *InMaterial->GetName(), *ShaderMap->ToString());

	int i = 0;
/*	for (auto& Name : RHIVertexShader->Debug.UniformBufferNames)
	{
		// DEBUG: Log the list of uniform required
		UE_LOGF(LogTexture, Warning, "Vertex Shader [%d]: %ls", i, *Name.ToString());
		++i;
	}*/
	i = 0;
	...
	{
		// DEBUG: Log the list of uniform required
		// UE_LOGF(LogTexture, Warning, "Pixel Shader [%d]: %ls", i, *Name.ToString());
		if (Name == TEXT("SceneTexturesStruct"))
		{
			IsSupported = false;
			UE_LOGF(LogTexture, Warning, "Pixel Shader [%d]: Require the <SceneTexturesStructs> which is not supported by the TextureGraph pipeline", i, *Name.ToString());
		}
		++i;
	}
#endif
