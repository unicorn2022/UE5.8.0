...
FMaterialInstanceStaticSwitchPermutationMap FMaterialInstanceStaticSwitchPermutationMap::Create(UMaterialInterface* InMaterial)
{
	FMaterialInstanceStaticSwitchPermutationMap MISSPMap;
	if (InMaterial)
	{
...
{
	TObjectPtr<UMaterialInstanceConstant>* MICP = PermutationsMap.Find(DefaultKey);
	if (MICP)
	{
...
UMaterialInstanceConstant* FMaterialInstanceStaticSwitchPermutationMap::GetMaterialInstance(const TArray<FStaticSwitchParameter>& Parameters)
{
	...
	TObjectPtr<UMaterialInstanceConstant>* MICP = PermutationsMap.Find(Key);
	if (!MICP)
	{
		UMaterialInstanceConstant* MIC = NewObject<UMaterialInstanceConstant>();
#if WITH_EDITOR
...
bool FMaterialInstanceStaticSwitchPermutationMap::IsValid() const
{
	// If a non-null material is provided to Create, it will be in the PermutationsMap, so if 
	// PermutationsMap has no entries, it implies Create was not used to initialize this struct.
	return !PermutationsMap.IsEmpty();
}
	...
	, VirtualTextureNumWarmupFrames(InNumWarmupFrames)
{
	...
	else if (Material && !MaterialInstance)
	{
		...
		MaterialInstance = TStrongObjectPtr<UMaterialInstanceConstant>(NewObject<UMaterialInstanceConstant>());
#if WITH_EDITOR
