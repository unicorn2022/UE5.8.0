...
void FLevels::InitFromHistogramScan(float InPosition, float InContrast)
{
	// Histogram scan is affecting the input low and high and the mid.
	// InPosition determines the center of the remapped input range
	// Contrast controls the width of the range of remapping
	// Contrast is scaled depending on the InPosition
	// Contrast shrink to 0 when InPosition is at 0 or 1
	// Contrast is whole when InPosition is at 0.5

	float P = 1.0f - FMath::Clamp(InPosition, 0, 1);

	float C = (1.0 - FMath::Clamp(InContrast, 0, 1)) * 0.5f;
	float D = 1.0f - FMath::Abs(0.5f - P) * 2.0f;
	float ScaledC = C * D;
...
void FLevels::InitFromHistogramRange(float InPosition,  float InRange)
{
	// Histogram range is just affecting the output low and high

	float range = FMath::Clamp(InRange, 0.0f, 1.0f) * 0.5f;
	float pos = FMath::Clamp(InPosition, 0.0f, 1.0f);

	OutLow = FMath::Clamp(pos - range, 0.0f, 1.0f);
	OutHigh = FMath::Clamp(pos + range, 0.0f, 1.0f);
