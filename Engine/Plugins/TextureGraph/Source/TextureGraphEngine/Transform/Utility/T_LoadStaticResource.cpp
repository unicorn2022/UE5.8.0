...
cti::continuable<int32> Job_LoadStaticImageResource::PreExecAsync(ENamedThreads::Type execThread, ENamedThreads::Type returnThread)
{
	...
	if (CachedResult)
	{
		UE_LOGF(LogData, Log, "[ChannelSource - %ls] Found cached Result!", *Source->GetAssetUUID());
...
{
	UE_LOGF(LogData, Log, "Loading ChannelSource: %ls [UUID: %ls]", *Source->GetAssetUUID(), *Source->GetAssetUUID());
	...
	return Job::BeginNative(RunInfo)
		.then([this]()
		{
			UE_LOGF(LogData, Log, "[ChannelSource - %ls] Calling source load ...", *Source->GetAssetUUID());
			return Source->Load(RunInfo.Cycle);
		})
		.then([this](TiledBlobRef LoadedSource) mutable
		{
			UE_LOGF(LogData, Log, "[ChannelSource - %ls] Source loaded. Setting up the asset!", *Source->GetAssetUUID());
		...
		{
			UE_LOGF(LogData, Log, "[ChannelSource - %ls] Promise failure!", *Source->GetAssetUUID());
			EndNative();
			return -1;
		});
}