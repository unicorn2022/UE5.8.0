...
//////////////////////////////////////////////////////////////////////////
EObjectFlags Tex::Flags = (RF_Public | RF_Standalone);
...
void Tex::FreeGenericTexture(UTexture** Texture)
{
	if (Texture && *Texture)
	{
		UE_LOGF(LogTexture, Verbose, "Deleting Texture: %ls [Ptr = 0x%llx]", *((*Texture)->GetName()), (*Texture));
...
void Tex::FreeRT(UTextureRenderTarget2D** RT)
{
	if (RT && *RT)
	{
		UE_LOGF(LogTexture, Verbose, "Deleting render target: %ls [Ptr = 0x%llx]", *((*RT)->GetName()), (*RT));
...
void Tex::InitRT(bool ForceFloat /* = false */)
{
	...
	UE_LOGF(LogTexture, VeryVerbose, "InitRT: Freeing existing Texture: %ls", *Desc.Name)

	Free();

	UE_LOGF(LogTexture, VeryVerbose, "InitRT: Existing Texture freed: %ls", *Desc.Name)
	...
	UE_LOGF(LogTexture, VeryVerbose, "InitRT: Creating new render target: %ls", *Name.ToString());
...
void Tex::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	{
		// Clear these pointers so that they don't become dangling if the objects are deleted
		RT = nullptr;
		Texture = nullptr;
		return;
	}
...
UTexture2D* Tex::CreateTexture(uint32 Width, uint32 Height, EPixelFormat Format, bool sRGB, UObject* Package)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Invalid parameters specified for UTexture2D::CreateTransient()");
	}
...
bool Tex::CopyImageToBuffer(EPixelFormat& PixelFormat, int32& Width, int32& Height, TArray<uint8>& InputBuffer, TArray<uint8>& OutputBuffer)
{
	...
	if (ImageFormat != EImageFormat::Invalid)
	{
		...
		if (isOperationSuccess)
		{
			...
			{
				UE_LOGF(LogTexture, Warning, "Error creating Texture. Bit depth is unsupported. (%d)", BitDepth);
				return false;
			}
...
bool Tex::LoadFile(const FString& Filename, const DesiredImageProperties* Props /* = nullptr */)
{
	...
	TArray<uint8> Buffer;
	UE_LOGF(LogTexture, Log, "Trying to load file %ls", *Filename);
	...
	if (FFileHelper::LoadFileToArray(Buffer, *Filename))
	{
		...
		{
			DidLoadBuffer = true;
			UE_LOGF(LogTexture, Log, "Loading file %ls Success", *Filename);
			...
			{
				UE_LOGF(LogTexture, Error, "Exception while loading filename: %ls", *Filename);
				return false;
			}
		}
		else
		{
			UE_LOGF(LogTexture, Log, "CopyImageToBuffer failed for file: %ls", *Filename);
			return false;
		}
	}
	else
	{
		UE_LOGF(LogTexture, Log, "FFileHelper::LoadFileToArray failed for file: %ls", *Filename);
		return false;
	}
...
bool Tex::SaveImage(UTextureRenderTarget2D* RT, const FString& Path, const FString& Filename, bool bIsHDR)
{
	...
	if (!RT)
	{
		UE_LOGF(LogTexture, Warning, "TextureRenderTarget must be non-null.");
	}
	else if (!RT->GetResource())
	{
		UE_LOGF(LogTexture, Warning, "TextureRenderTarget has been released");
	}
	else if (!PathError.IsEmpty())
	{
		UE_LOGF(LogTexture, Warning, "Path is invalid - %ls",*PathError.ToString());
	}
	else if (Filename.IsEmpty())
	{
		UE_LOGF(LogTexture, Warning, "No filename specified. Please provide filename with extension");
	}
	else
	{
		...
		{
			UE_LOGF(LogTexture, Warning, "Failed to access path or file");
		}
	}
	return bSuccess;
}
