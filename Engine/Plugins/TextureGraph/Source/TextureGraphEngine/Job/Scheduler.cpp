...
void Scheduler::Update(float dt)
{
...

	{
		...
		if (CurrentBatch)
		{
			...
			{
				UE_LOGF(LogBatch, Verbose, "Current Batch: %llu been running for %f ms [Max threshold: %f, Jobs: %u/%u]!", 
	...
	if (BatchToRun)
	{
		...
		//UE_LOGF(LogBatch, Log, "Next Batch: %llu [Prev: %llu]", CurrentBatch->GetBatchId(), PreviousBatch ? PreviousBatch->GetBatchId() : 0);
		...
		CurrentBatch->Exec([this] (JobBatch*)	/// Instead of passing it as an argument, JobBatch should be a return type; this is to keep it from cyclic dependancy
			{
				...
				UE_LOGF(LogBatch, Verbose, "Scheduler, Batch fully queued: %llu. Triggering Observer::BatchJobsDone ...", PreviousBatch ? PreviousBatch->GetBatchId() : -1);
				...
				UE_LOGF(LogBatch, Verbose, "Scheduler Observer::BatchJobsDone finished for Batch: %llu", PreviousBatch ? PreviousBatch->GetBatchId() : -1);
			...
			.then([this]()
				{
					...
					UE_LOGF(LogBatch, Verbose, "Scheduler triggering Observer::BatchDone for Batch: %llu ...", PreviousBatch ? PreviousBatch->GetBatchId() : -1);

					ObserverSource->BatchDone(CurrentBatch);

					UE_LOGF(LogBatch, Verbose, "Scheduler Observer::BatchDone finished for Batch: %llu", PreviousBatch ? PreviousBatch->GetBatchId() : -1);
				});
	}
	else
	{
	...
	return Batch->Tick().then([this, Batch, StartTime](JobResultPtr result)
		{
			...
			if (Duration > IdleBatchTimeLimit)
			{
				...
				UE_LOGF(LogBatch, Verbose, "Idle Batch [%ls] time limit offense @ %0.2f ms [Max: %0.2f ms]", *Batch->GetName(), (float)Duration, IdleBatchTimeLimit);
			}
...
void Scheduler::UpdateIdle(bool bForce, bool bIsBusyUpdate)
{
	...
	UE_LOGF(LogIdle_Svc, VeryVerbose, "Scheduler::UpdateIdle");
...
void Scheduler::AddBatch(JobBatchPtr Batch)
{
	...
	check(Batch->GetFrameId())
		UE_LOGF(LogBatch, Verbose, "Adding Batch: %llu [FrameId: %llu, Num batches: %llu]", Batch->GetBatchId(), Batch->GetFrameId(), Batches.size());
#endif 
