...
AsyncInt Job::Bind_Or_Unbind_All_Generic(JobRunInfo InRunInfo, Bind_Unbind_Func BFunc)
{
	if (CanHandleTiles() && Transform->GeneratesData())
	{
		...
		if (!Promises.empty())
		{
			...
			return cti::make_continuable<int32>([this, Count, FWD_PROMISE(FinalPromise), This = this](auto&& promise) mutable
			{
				...
				{
					UE_LOGF(LogJob, VeryVerbose, "Returning bind/unbind promise (multiple): %llu.%ls", This->RunInfo.Batch->BatchId, *This->Transform->GetName());
	...
	else
	{
		return cti::make_continuable<int32>([this, BFunc, This = this](auto&& promise)
		{
			...
			{
				UE_LOGF(LogJob, VeryVerbose, "Returning bind/unbind promise (single): %llu.%ls", This->RunInfo.Batch->BatchId, *This->Transform->GetName());
				promise.set_value(1);
			});
		});
	}

	UE_LOGF(LogJob, VeryVerbose, "Returning bind/unbind promise (none): %llu.%ls", InRunInfo.Batch->BatchId, *Transform->GetName());
...
TiledBlobPtr Job::InitResult(FString InNewName, const BufferDescriptor* InDesiredDesc, int32 NumTilesX /* = 0 */, int32 NumTilesY /* = 0 */)
{
	...
	if (ResultDesc.Width < static_cast<uint32>(EResolution::Auto) || ResultDesc.Width > static_cast<uint32>(EResolution::Resolution8192))
		ResultDesc.Width = static_cast<uint32>(EResolution::Auto);

	if (ResultDesc.Height < static_cast<uint32>(EResolution::Auto) || ResultDesc.Height > static_cast<uint32>(EResolution::Resolution8192))
		ResultDesc.Height = static_cast<uint32>(EResolution::Auto);
		...
		if (CanHandleTiles())
		{
			for (size_t RowId = 0; RowId < TileResults.Rows(); RowId++)
			{
				for (size_t ColId = 0; ColId < TileResults.Rows(); ColId++)
				{
					...
					if (BlobObj)
					{
						UE_LOGF(LogJob, VeryVerbose, "FINISHED: %llu => %ls", JobResult->HashValue->Value(), *BlobObj->Name());
...
bool Job::CheckCulled(JobRunInfo InRunInfo)
{
	...
	for (size_t RowId = 0; RowId < ResultPromise->Rows(); RowId++)
	{
		for (size_t ColId = 0; ColId < ResultPromise->Cols(); ColId++)
		{
			...
			UE_LOGF(LogJob, VeryVerbose, "[Job - %ls] matched tile: %d, %d", *GetName(), (int32)RowId, (int32)ColId);
	...
	UE_LOGF(LogJob, Verbose, "[Job - %ls] All tiles found. Marking as culled!", *GetName());
...
AsyncPrepareResult Job::PrepareTargets(JobBatch* Batch)
{
	...
	for (size_t RowId = 0; RowId < ResultPromise->Rows(); RowId++)
	{
		for (size_t ColId = 0; ColId < ResultPromise->Cols(); ColId++)
		{
			...
			{
				UE_LOGF(LogJob, VeryVerbose, "Transform: %ls (Tile: %d, %d) => %ls", *Transform->GetName(), (int32)RowId, (int32)ColId, *TileBlob->Name());
...
AsyncPrepareResult Job::PrepareResources(JobBatch* Batch)
{
	...
	UE_LOGF(LogJob, VeryVerbose, "Preparing resources for job: %ls", *Transform->GetName());
	...
	UE_LOGF(LogJob, VeryVerbose, "Preparing resources for Transform: %ls", *Transform->GetName());
...
AsyncJobResultPtr Job::RunTile(JobRunInfo InRunInfo, int32 RowId, int32 ColId)
{
	...
	UE_LOGF(LogJob, VeryVerbose, "No cached results for hash: %llu [Recalculating ...]", JobHash->Value());
...
AsyncJobResultPtr Job::Run(JobRunInfo InRunInfo)
{
	...
	UE_LOGF(LogJob, VeryVerbose, "Starting job: %llu.%d.%llu [Transform: %ls]", RunInfo.Batch->GetBatchId(), QueueId, Id, *Transform->GetName());
...
AsyncInt Job::BeginNative(JobRunInfo InRunInfo)
{
	...
	UE_LOGF(LogJob, VeryVerbose, "Job::BeginNative: %llu.%llu.%ls", RunInfo.Batch->GetBatchId(), Id, *Transform->GetName());
...
void Job::MarkJobDone()
{
	...
	if (RunInfo.Batch)
	{
		...
		UE_LOGF(LogJob, VeryVerbose, "Job::Done: %llu.%lld.%ls", RunInfo.Batch->GetBatchId(), GetJobId(), *Transform->GetName());
	}
...
{
	UE_LOGF(LogJob, VeryVerbose, "PreExec::%ls", *Transform->GetName());
...
int32 Job::Exec()
{
	UE_LOGF(LogJob, VeryVerbose, "Exec::%ls", *Transform->GetName());
...
void Job::PostExec()
{
	UE_LOGF(LogJob, VeryVerbose, "PostExec::%ls", *Transform->GetName());
