...
{
	UE_LOGF(LogTransfer_Svc, Verbose, "Svc_DeviceTransfer::Tick");
	...
	if (Count)
	{
		...
		while (!TextureGraphEngine::IsDestroying() && (int32)Promises.size() < Count && Iter != Transfers.end())
		{
			...
			if (ThisTransferInfo.BlobObj->GetBufferRef()->GetOwnerDevice() != ThisTransferInfo.OwnerDevice)
			{
				...
				UE_LOGF(LogTransfer_Svc, Warning, "Buffer [Name: %ls, Hash: %llu] owner device changed between transfer initiation and execution. Original = %ls, Current = %ls [Target: %ls]. Aborting transfer for this buffer ...", 
			...
			{
				UE_LOGF(LogTransfer_Svc, Warning, "Buffer [Name: %ls, Hash: %llu] timestamp changed between transfer initiation and execution. Original = %lf, Current = %lf [Device: %ls => %ls]. Aborting transfer for this buffer ...", 
			...
			{
				UE_LOGF(LogTransfer_Svc, Verbose, "Buffer [Name: %ls, Hash: %llu] starting transfer from %ls => %ls", *BufferName, BufferHash->Value(), *OwnerDeviceNameOrg, *TargetDeviceName);
...
void DeviceTransferService::QueueTransfer(BlobPtr blob, Device* target)
{
	...
	if (iter != Transfers.end())
	{
		...
		UE_LOGF(LogTransfer_Svc, Warning, "Buffer [Hash: %llu] has already been marked for transfer to device: %ls", hash->Value(), *target->Name());
	...
	{
		UE_LOGF(LogTransfer_Svc, Warning, "Buffer [Hash: %llu] is already cached on the target device: %ls", hash->Value(), *target->Name());
		return;
	}
