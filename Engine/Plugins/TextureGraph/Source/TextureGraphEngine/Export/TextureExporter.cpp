...
AsyncInt TextureExporter::ExportRawAsUAsset(RawBufferPtr RawObj,const FExportMapSettings& Setting, const FString& CompletePath, const FName& TextureName)
{
	...
	{
		UE_LOGF(LogTexture, Warning, "Provide path is empty!");
		return cti::make_ready_continuable(-1);
	}
	else if (!PathError.IsEmpty())
	{
		UE_LOGF(LogTexture, Warning, "Invalid file path provided: %ls", *(PathError.ToString()));
		return cti::make_ready_continuable(-1);
	}
		
	if (!RawObj || RawObj->GetData() == nullptr)
	{
		UE_LOGF(LogTexture, Warning, "Can't export with invalid RawBuffer: %ls", (RawObj != nullptr) ? *RawObj->GetName() : TEXT("nullptr"));
		return cti::make_ready_continuable(-1);
	}
	...
	return cti::make_continuable<int32>([RawObj,Setting, CompletePath, TextureName](auto Promise) mutable
	{
		...
		if (AssetData.Num() > 0)
		{
			...
			if (!bIsTextureAsset)
			{
				...
				UE_LOGF(LogTexture, Warning, "Creating a Unique name for the output texture as another asset of different type %ls exists with same name %ls", *AssetType, *PackageName);
			}
		}
		...
		{
			UE_LOGF(LogTexture, Warning, "Export dimensions are invalid and require buffer size larger than is available. Expected to be less than: %zu [Asked: %d, %dx%d]. Clamping ...",
		...
		{
			UE_LOGF(LogTexture, Warning, "Cannot Export specified Pixel Format %d as UTexture2D", PixelFormat);
			Promise.set_value(-1);
			return;
		}
