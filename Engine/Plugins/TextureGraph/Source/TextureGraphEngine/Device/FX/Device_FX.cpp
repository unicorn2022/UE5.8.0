	...
	FString& TotalStats)
{
	...
	for (auto Iter = RTCache.begin(); Iter != RTCache.end(); Iter++)
	{
		...
		if (RenderTargets)
		{
			for (UTextureRenderTarget2D* RT : *RenderTargets)
			{
				...
				FString Tooltip(FString::Printf(TEXT("Ptr : %llx , Hash : %llu, Memory Size : %0.2fKB"), RT, Iter->first, (float)Size / (1024.0f)));
	...
	for (auto& RT : RTUsed)
	{
		...
		FString Tooltip(FString::Printf(TEXT("Ptr : %llx , Memory Size : %0.2fKB"), RT.Get(), (float)Size / (1024.0f)));
...

			{
				...
				for (int32 RowId = 0; RowId < Tiles.Rows(); RowId++)
				{
					...
					for (int32 ColId = 0; ColId < Tiles.Cols(); ColId++)
					{
						...
						UE_LOGF(LogDevice, Verbose, "Combine tiles: %ls [%d, %d] => %llu [%ls]", *FXBuffer->GetName(), RowId, ColId, TileBuffer->Hash(false)->Value(), *TileBuffer->GetName());
...
UTextureRenderTarget2D* Device_FX::CreateRenderTarget(const BufferDescriptor& Desc)
{
	...
	//UE_LOGF(LogTexture, Log, "Mips number: %d", _rt->GetNumMips());
	RT->LODBias = 0;
	//_rt->bAutoGenerateMips = false;

	//_rt->InitCustomFormat(_desc.width, _desc.height, _desc.format, forceLinearGamma);
	//UE_LOGF(LogTexture, Log, "Creating render target: %ls", *_desc.Name);
...
void Device_FX::FreeRenderTarget(HashType HashValue, UTextureRenderTarget2D* RT)
{
	...
	RTUsed.remove(RT);
	UE_LOGF(LogDevice, VeryVerbose, "Reclaimed RT: Name : %ls : %llu [Ptr: 0x%llu, Size: %dx%d]", *RT->GetName(), HashValue, RT, RT->SizeX, RT->SizeY);
...
TexPtr Device_FX::AllocateRenderTarget(const BufferDescriptor& Desc)
{
	...
	if (!List->empty())
	{
		...
		//UE_LOGF(LogDevice, Verbose, "Rename: %ls => %ls", *existingName, *Desc.Name);
		//RT->Rename(*Desc.Name);
...
int32 Device_FX::InitRTResource(TexPtr TextureObj, UTextureRenderTarget* RT)
{
	...
	UE_LOGF(LogDevice, Verbose, "New RT allocation: %ls %llu [Ptr: 0x%p, Size: %ux%u]", *TextureObj->GetDescriptor().Name,
...
void Device_FX::MarkForCollection(TexPtr TextureObj)
{
	...
	UE_LOGF(LogDevice, VeryVerbose, "[Device_FX] Marking Tex for collection: %ls", *TextureObj->GetDescriptor().Name);
...
DeviceBufferRef Device_FX::CreateFromTexAndRaw(TexPtr TextureObj, RawBufferPtr Raw)
{
	...
	if (RefBuffer && !RefBuffer.IsValid())
	{
		...
		UE_LOGF(LogDevice, VeryVerbose, "[Device_FX] HASH ALREADY FOUND TextureObj: %ls Hash:%llu Previously:%ls", *TextureObj->GetDescriptor().Name, Hash->Value(), *RefBuffer->Descriptor().Name);
	...
	UE_LOGF(LogDevice, VeryVerbose, "[Device_FX] TextureObj: %ls Hash:%llu", *TextureObj->GetDescriptor().Name, Hash->Value());
...
void Device_FX::PrintStats()
{
	...
	UE_LOGF(LogDevice, Log, "===== BEGIN Device: FX STATS (Native) =====");

	UE_LOGF(LogDevice, Log, "[USED] RT Count   : %llu", RTUsed.size());
	UE_LOGF(LogDevice, Log, "[USED] RT Mem     : %0.2f MB", (float)RTMemUsed / (1024.0f * 1024.0f));

	UE_LOGF(LogDevice, Log, "[Unused] RT Count : %llu", RTCountUnused);
	UE_LOGF(LogDevice, Log, "[Unused] RT Mem   : %0.2f MB", (float)RTMemUnused / (1024.0f * 1024.0f));

	UE_LOGF(LogDevice, Log, "===== END Device  : FX STATS (Native) =====");
