...
bool DeviceNativeTask::WaitFor(int32 Seconds)
{
	...
	for (auto& PrevTask : Prev)
	{
		...
		if (!PrevTask->IsCulled())
		{
			...
			UE_LOGF(LogDevice, VeryVerbose, "[%llu] Begin Wait::%ls", PrevTask->GetTaskId(), *PrevTask->GetName());
			std::future_status Status = PrevTask->GetFuture().wait_for(std::chrono::seconds(Seconds));

			if (Status == std::future_status::timeout)
			{
				FString WaitChain = DebugWaitChain();
				UE_LOGF(LogDevice, VeryVerbose, "[%llu] Timed out wait for task::%ls. Chain: %ls", PrevTask->GetTaskId(), *PrevTask->GetName(), *WaitChain);
				return false;
			}

			UE_LOGF(LogDevice, VeryVerbose, "[%llu] End Wait::%ls", PrevTask->GetTaskId(), *PrevTask->GetName());
		}
	}
		...
		if (Prev[i])
		{
			...
			else
			{
					...
					!(prevThreadId & ENamedThreads::AnyThread) && !(thisThreadId & ENamedThreads::AnyThread))
				{
					...
					UE_LOGF(LogDevice, Error, "%ls", *errorStr);
