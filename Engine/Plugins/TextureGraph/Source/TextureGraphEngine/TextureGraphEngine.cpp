...
{
	UE_LOGF(LogTextureGraphEngine, Log, "Initialising the TextureGraphEngine!");
...
TextureGraphEngine::~TextureGraphEngine()
{
	...
	UE_LOGF(LogTextureGraphEngine, Log, "Destroying the TextureGraphEngine!");
...
void TextureGraphEngine::DoneTests()
{
	...
	// Currently we're only supporting editor only 
#if WITH_EDITOR
	if (!GEditor || !GInstance || !GInstance->bIsEngineInitDone)
		return;
...
{
	return (IsRunningCookCommandlet() || IsRunningDLCCookCommandlet() || !CVarAllowTextureGraphInCommandlets.GetValueOnAnyThread())
	#if WITH_EDITOR
...
{
	check(IsInGameThread());
	UE_LOGF(LogTextureGraphEngine, Log, "Setting TextureGraphEngine to run mode = true!");
