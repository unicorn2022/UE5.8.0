...
void RenderMesh::UpdateBounds()
{
	...
	UE_LOGF(LogMesh, Log, "Current Bounds: %ls", *(_viewBounds.GetSize().ToString()));
}
...
void RenderMesh::SetMaterial(UMaterialInterface* mat)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "Exception %s", e.what());
	}	
}

void RenderMesh::SpawnActors(UWorld* world)
{
	UE_LOGF(LogMesh, Log, "Spawning actors");
	...
	if (ensure(world))
	{
		for (MeshInfoPtr meshInfo : _meshes)
		{
			...
			UE_LOGF(LogMesh, Log, "Adding mesh section");
...
void RenderMesh::RemoveActors()
{
	...
	for (AActor* actor : _meshActors)
	{
		UE_LOGF(LogMesh, Log, "Removing mesh section");
...
void RenderMesh::DrawBounds(UWorld* world)
{
	...
	UE_LOGF(LogMesh, Log, "MeshName : %ls : MeshCenter : %ls : Mesh Extents : %ls : Min : %ls : Max : %ls", *GetMaterialName(), *_originalBounds.GetCenter().ToString(),*_originalBounds.GetExtent().ToString(), *_originalBounds.Min.ToString(), *_originalBounds.Max.ToString());
...
RenderMeshPtr RenderMesh::Create(const MeshLoadInfo _loadInfo)
{
	...
	{
		UE_LOGF(LogMesh, Log, "MeshAsset : Unkown mesh type %i", int(meshType));
