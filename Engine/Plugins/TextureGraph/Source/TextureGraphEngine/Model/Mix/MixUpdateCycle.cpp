...
void MixUpdateCycle::End()
{
	...
	if (Batch)
	{
		UE_LOGF(LogBatch, Log, "ENDING scene cycle for batch: %llu", Batch->GetBatchId());
		Batch = nullptr;
	}
}
...
JobPtrW MixUpdateCycle::AddJob(int32 InTargetId, JobUPtr JobObj)
{
	...
	{
		UE_LOGF(LogBatch, VeryVerbose, "MixUpdateCycle::AddJob  Job [%llu]: IsCulled", Batch->GetBatchId());
	}
