...
MixManager::~MixManager()
{
	/// Intentionally empty. Exit() drains the queue while engine subsystems are still alive.
}
...
void MixManager::Update(float Delta)
{
	...
	{
		UE_LOGF(LogMixManager, Log, "Rendering mix: %ls [Invalidation FrameId: %llu, Update FrameId: %llu, Discard: %ls", 
...
{
	UE_LOGF(LogMixManager, Log, "Begin MixManager::Suspend");
	...
	/// Drain any pending invalidations now, while engine subsystems are
	/// still alive. After Suspend(), bIsSuspended is true so InvalidateMix
	/// early-returns and no new items can be queued between here and
	/// ~MixManager.
	Queue.clear();
	MixesToFlush.clear();

	UE_LOGF(LogMixManager, Log, "End MixManager::Suspend");
}
