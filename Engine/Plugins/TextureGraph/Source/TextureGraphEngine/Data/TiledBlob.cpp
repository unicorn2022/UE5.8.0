...
TiledBlob::~TiledBlob()
{
	UE_LOGF(LogData, VeryVerbose, "Deleting TiledBlob: %ls [%dx%d]", *Desc.Name, Desc.Width, Desc.Height);
	Tiles.Clear();
}
...
AsyncBufferResultPtr TiledBlob::CombineTiles(bool bTouch, bool bIsArray, uint64 BatchId /* = 0 */)
{
	...
	UE_LOGF(LogData, VeryVerbose, "*** Combine TiledBlob: %ls [Hash: %llu] [T0: %ls] ***", *Desc.Name, HashValue->Value(), *Tile0->Name());
...
AscynCHashPtr TiledBlob::CalcHash()
{
	...
	UE_LOGF(LogBlob, VeryVerbose, "Finalising TiledBlob hash: %ls", *Desc.Name);
...
AsyncDeviceBufferRef TiledBlob::TransferTo(Device* Target)
{
	...
	for (size_t RowId = 0; RowId < Tiles.Rows(); RowId++)
	{
		for (size_t ColId = 0; ColId < Tiles.Cols(); ColId++)
		{
			...
			{
				UE_LOGF(LogData, Log, "TransferTo %ls TiledBlob: %ls Tile [%zu, %zu]", *TargetName, *Desc.Name, RowId, ColId);
		...
		return cti::when_all(Promises.begin(), Promises.end()).then([this, TargetName](auto) mutable
			{
				...
				UE_LOGF(LogData, Log, "TransferTo %ls TiledBlob: %ls DONE", *TargetName, *Desc.Name);
				return Buffer;
			});
	}
...
AsyncBufferResultPtr TiledBlob::Bind(const BlobTransform* Transform, const ResourceBindInfo& BindInfo)
{
	...
	if (Tiles.Rows() && Tiles.Cols())
	{
		...
		if (Tiles.Rows() > 1 || Tiles.Cols() > 1)
		{
			...
			//
			///// We've already calculated the hash from all the child blobs
			//UE_LOGF(LogData, Log, "*** Combine TiledBlob: %ls [Hash: %llu] [T0: %ls] ***", *_desc.name, _hash->Value(), *Tiles[0][0]->Name());
			...
			if (!_ready)
			{
				...
				UE_LOGF(LogData, VeryVerbose, "*** BEGIN Combine TiledBlob: %ls [Hash: %llu] [T0: %ls] ***", *_desc.name, _hash, *Tiles[0][0]->Name());
				double startTime = Util::Time();
				RawBufferPtr Raw = TextureHelper::CombineRaw_Tiles(Tiles, _hash, true);
				UE_LOGF(LogData, VeryVerbose, "*** END Combine TiledBlob: %ls [Hash: %llu] [T0: %ls] ***", *_desc.name, _hash, *Tiles[0][0]->Name());
...
void TiledBlob::FinaliseNow(bool bNoCalcHash, CHashPtr FixedHash)
{
	...
	UE_LOGF(LogJob, Verbose, "Finalised tiled BlobObj: %ls", *Desc.Name);
...
void TiledBlob_Promise::FinaliseNow(bool bNoCalcHash, CHashPtr FixedHash)
{
	...
	UE_LOGF(LogJob, Verbose, "Finalised promised BlobObj: %ls", *Desc.Name);
