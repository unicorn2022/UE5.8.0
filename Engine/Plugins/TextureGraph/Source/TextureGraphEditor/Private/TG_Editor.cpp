...
void FTG_Editor::InitEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, UTextureGraph* InTextureGraph)
{
	...
	DetailsViewArgs.ColumnWidth = 0.70f;
	...
	OutputViewArgs.ColumnWidth = 0.70f;
	...
	ParameterViewArgs.ColumnWidth = 0.70f;
...
{
	UE_LOGF(LogTextureGraphEditor, Log, "Saving Texture Graph %ls", *GetEditingObjects()[0]->GetName());
...
{
	UE_LOGF(LogTextureGraphEditor, Log, "Saving Texture Graph As %ls", *GetEditingObjects()[0]->GetName());
