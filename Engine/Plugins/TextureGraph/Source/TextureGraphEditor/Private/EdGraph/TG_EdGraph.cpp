...
bool UTG_EdGraph::CreateViewModelLinkFromModelPins(const UTG_Pin* pinFrom, const UTG_Pin* pinTo)
{
	...
	UE_LOGF(LogTemp, Warning, "UTG_EdGraph::CreateViewModelLinkFromModelPins Invalid Link attempted %ls => %ls", *pinFrom->GetArgumentName().ToString(), *pinTo->GetArgumentName().ToString())
	return false;
}
...
void UTG_EdGraph::OnNodeSignatureChanged(UTG_Node* InNode)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "UTG_EdGraph::OnNodeSignatureChanged TG_Node not found in the EdNodes? this should not happen.")
	}
}
