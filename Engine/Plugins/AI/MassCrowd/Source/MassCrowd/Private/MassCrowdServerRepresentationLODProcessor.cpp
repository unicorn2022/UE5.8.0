...
void UMassCrowdServerRepresentationLODProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	if (UE::MassCrowd::bDebugCrowdServerRepresentationLOD)
	{
		...
		{
			EntityQuery.ForEachEntityChunk(Context, [this, DebugDrawingWorld](FMassExecutionContext& Context)	
			{
				...
				LODCalculator.DebugDisplayLOD(Context, RepresentationLODFragments, LocationList, DebugDrawingWorld);
			});
		}
	}
