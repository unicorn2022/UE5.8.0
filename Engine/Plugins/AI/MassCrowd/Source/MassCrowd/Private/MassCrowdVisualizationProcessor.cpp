	...
	if (UE::MassCrowd::bDebugCrowdVisualType)
	{
		...
		EntityQuery.ForEachEntityChunk(Context, [this](FMassExecutionContext& Context)
		{
			...
			for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
			{
				...
				{
					DrawDebugBox(World, EntityLocation.GetTransform().GetLocation() + FVector(0.0f, 0.0f, 120.0f), FVector(25.0f), UE::Mass::Representation::RepresentationTypeColors[RepresentationTypeIdx]);
				}
				else
				{ 
					DrawDebugSolidBox(World, EntityLocation.GetTransform().GetLocation() + FVector(0.0f, 0.0f, 120.0f), FVector(25.0f), UE::Mass::Representation::RepresentationTypeColors[RepresentationTypeIdx]);
				}
			}
		});
	}
