...
void UMLAdapterLocalDataCollectionSession::PostInitProperties()
{
	...
	UE_LOGF(LogMLAdapter, Log, "LocalDataCollectionSession: Writing data to %ls", *OutputFilePath);
}
...
void UMLAdapterLocalDataCollectionSession::Tick(float DeltaTime)
{
	...
	if (Agent ==  nullptr)
	{
		UE_LOGF(LogMLAdapter, Log, "LocalDataCollectionSession: Player-controlled agent not found yet.");
		return;
	}
	...
	if (!bSuccess)
	{
		UE_LOGF(LogMLAdapter, Error, "LocalDataCollectionSession: Failed to write data to output file.");
	}
}
