...
void UMLAdapterManager::RecreateManagerInstance()
{
	...
	UE_LOGF(LogMLAdapter, Log, "Creating MLAdapter manager of class %ls", *GetNameSafe(Class));
...
void UMLAdapterManager::StopServer()
{
	...
	{
		UE_LOGF(LogMLAdapter, Log, "Stopping RPC server.");
...
void UMLAdapterManager::StartServer(uint16 Port, EMLAdapterServerMode InMode, uint16 ServerThreads)
{
	...
	UE_LOGF(LogMLAdapter, Log, "Starting RPC server on port %d.", Port);
...
UMLAdapterSession* UMLAdapterManager::CreateNewSession()
{
	...
	UE_LOGF(LogMLAdapter, Log, "Creating MLAdapter session of class %ls", *GetNameSafe(Class));
