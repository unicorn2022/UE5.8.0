...
void UMLAdapterSensor_EnhancedInput::OnAvatarSet(AActor* Avatar)
{
	...
	if (InputComponent != nullptr)
	{
		UE_LOGF(LogMLAdapter, Log, "MLAdapterSensor_EnhancedInput: This sensor already bound to another InputComponent. Cleaning up previous InputComponent and rebinding to new avatar.");
		InputComponent->DestroyComponent();
	}
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			UE_LOGF(LogMLAdapter, Log, "MLAdapterSensor_EnhancedInput: Successfully bound to %ls", *GetNameSafe(PawnClass));
		}
	}
	else
	{
		FString AvatarName = Avatar->GetName();
		UE_LOGF(LogMLAdapter, Warning, "MLAdapterSensor_EnhancedInput: Unable to bind - could not cast to APawn or APlayerController with a Pawn. Avatar name was %ls", *AvatarName);
	}
