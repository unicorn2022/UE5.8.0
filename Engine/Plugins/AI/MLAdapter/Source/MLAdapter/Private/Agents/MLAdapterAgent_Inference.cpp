...
void UMLAdapterAgent_Inference::PostInitProperties()
{
	...
	{
		UE_LOGF(LogMLAdapter, Warning, "Neural network asset data not set");
		return;
	}
	...
	if (!Runtime.IsValid())
	{
		UE_LOGF(LogMLAdapter, Warning, "Runtime %ls is not valid!", *RuntimeName);
		return;
	}
	...
	{
		UE_LOGF(LogMLAdapter, Warning, "Brain accepts only single tensor input.");
		return;
	}
	if (!InputTensorDescs[0].GetShape().IsConcrete())
	{
		UE_LOGF(LogMLAdapter, Warning, "Brain does not accept dynamic tensor input shapes.");
		return;
	}
	...
	{
		UE_LOGF(LogMLAdapter, Warning, "Brain accepts only single tensor output.");
		return;
	}
	...
	{
		UE_LOGF(LogMLAdapter, Warning, "Brain could not resolve output shapes.");
		return;
	}
	...
	{
		UE_LOGF(LogMLAdapter, Warning, "Input tensor should be of non-zero size.");
		return;
	}
	...
	{
		UE_LOGF(LogMLAdapter, Warning, "Output tensor should be of non-zero size.");
		return;
	}
}
...
void UMLAdapterAgent_Inference::Think(const float DeltaTime)
{
	if (Brain == nullptr)
	{
		UE_LOGF(LogMLAdapter, Warning, "Agent beginning to Think but Brain is null, Skipping.");
		return;
	}
	...
	{
		UE_LOGF(LogMLAdapter, Warning, "Brain received not enough data.");
		return;
	}
