...
namespace UE::MassNavigation::Debug
{
	...
	struct FDebugContext
	{
		...
		FDebugContext(const FMassExecutionContext& Context, const UObject* InFallbackLogOwner, const FLogCategoryBase& InCategory, const FMassEntityHandle InEntity, int32 InEntityIndex)
			: LogContext(Context, ShouldLogEverythingWhenRecording())
			, FallbackLogOwner(InFallbackLogOwner)
			, Category(InCategory)
			, World(Mass::Debug::GetDebugDrawWorld(Context))
			, Entity(InEntity)
			, EntityIndex(InEntityIndex)
		{}
		...
		bool ShouldLogEntity(FColor* OutEntityColor = nullptr) const
		{
