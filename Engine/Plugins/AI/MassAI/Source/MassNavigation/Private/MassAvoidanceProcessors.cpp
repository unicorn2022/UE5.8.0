...
void UMassMovingAvoidanceProcessor::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	...
	// Indirect access: fragments read on neighbor entities returned by FindCloseObstacles
	EntityQuery.AddIndirectFragmentRequirement<FTransformFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddIndirectFragmentRequirement<FMassVelocityFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddIndirectFragmentRequirement<FMassMoveTargetFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddIndirectFragmentRequirement<FMassAvoidanceColliderFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddIndirectFragmentRequirement<FAgentRadiusFragment>(EMassFragmentAccess::ReadOnly);
...
void UMassMovingAvoidanceProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	UWorld* DebugDrawWorld = UE::Mass::Debug::GetDebugDrawWorld(Context);
	UWorld* World = Context.GetWorld();
	check(World);
	const double CurrentTime = World->GetTimeSeconds();
	const float DeltaTime = Context.GetDeltaTimeSeconds();

	EntityQuery.ForEachEntityChunk(Context, [FallbackLogOwner = this, &EntityManager, DebugDrawWorld, CurrentTime, DeltaTime](FMassExecutionContext& Context)
	{
		...
		for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
...
#if WITH_MASSGAMEPLAY_DEBUG
			const UE::MassAvoidance::FDebugContext BaseDebugContext(Context, FallbackLogOwner, LogAvoidance, DebugDrawWorld, Entity, EntityIt);
			const UE::MassAvoidance::FDebugContext VelocitiesDebugContext(Context, FallbackLogOwner, LogAvoidanceVelocities, DebugDrawWorld, Entity, EntityIt);
			const UE::MassAvoidance::FDebugContext ObstacleDebugContext(Context, FallbackLogOwner, LogAvoidanceObstacles, DebugDrawWorld, Entity, EntityIt);
			const UE::MassAvoidance::FDebugContext AgentDebugContext(Context, FallbackLogOwner, LogAvoidanceAgents, DebugDrawWorld, Entity, EntityIt);
			...
			if (BaseDebugContext.ShouldLogEntity(&EntityColor))
			{
				...
				UE_VLOG_WIRECIRCLE_THICK(FallbackLogOwner, LogMassNavigation, Log, AgentLocation + ZOffset, FVector::UpVector, AgentRadius, EntityColor, /*Thickness*/4,
			...
			for (const FNavigationObstacleHashGrid2D::ItemIDType OtherEntity : CloseEntities)
			{
				...
				{
					UE_LOGF(LogAvoidanceObstacles, VeryVerbose, "Close entity is invalid, skipped.");
					continue;
				}
...
void UMassStandingAvoidanceProcessor::ConfigureQueries(const TSharedRef<FMassEntityManager>& EntityManager)
{
	...
	EntityQuery.AddIndirectFragmentRequirement<FAgentRadiusFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddIndirectFragmentRequirement<FMassMoveTargetFragment>(EMassFragmentAccess::ReadOnly);
	EntityQuery.AddIndirectFragmentRequirement<FMassGhostLocationFragment>(EMassFragmentAccess::ReadOnly);
...
void UMassStandingAvoidanceProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [FallbackLogOwner = this, &EntityManager, DebugDrawWorld](FMassExecutionContext& Context)
	{
		const UMassNavigationSubsystem* NavigationSubsystem = Context.GetSubsystem<UMassNavigationSubsystem>();
		if (!NavigationSubsystem)
		{
