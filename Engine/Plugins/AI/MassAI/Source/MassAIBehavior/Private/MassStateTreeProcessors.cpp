...
CSV_DEFINE_CATEGORY(MassStateTreeProcessor, false);
	...
	TFunc&& Callback)
{
	...
	for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
	{
		...
		if (InstanceData != nullptr)
		{
			...
			TRACE_INSTANCE(TraceOwner, Entity.AsNumber(), FObjectTrace::GetObjectId(TraceOwner), FMassEntityHandle::StaticStruct(), FString::Printf(TEXT("Entity[%d:%d]"), Entity.Index, Entity.SerialNumber));
			...
			{
				CSV_SCOPED_TIMING_STAT(MassStateTreeProcessor, ExternalDataValidation);
	...
	: EntityQuery(*this)
{
	...
	ObservedTypes.Add(FMassStateTreeInstanceFragment::StaticStruct());
...
void UMassStateTreeActivationProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& ExecutionContext)
{
	...
	EntityQuery.ForEachEntityChunk(ExecutionContext,
		[&EntitiesToSignal, &ActivationCounts, MaxActivationsPerLOD = BehaviorSettings->MaxActivationsPerLOD, TimeInSeconds](FMassExecutionContext& Context)
		{
			...
			checkf(ChunkLOD != EMassLOD::Type::Max, TEXT("Attempting to activate a state tree with an uninitialized LOD"));
...
void UMassStateTreeProcessor::SignalEntities(FMassEntityManager& EntityManager, FMassExecutionContext& Context, FMassSignalNameLookup& EntitySignals)
{
	...
	CSV_SCOPED_TIMING_STAT(MassStateTreeProcessor, Execute);
	...
	auto TickFunc = [TimeInSeconds, &EntitySignals](FMassExecutionContext& ExecutionContext, TFunctionRef<void(FMassEntityHandle)> AddEntityToSignalFunc)
		{
			...
			CSV_CUSTOM_STAT(MassStateTreeProcessor, NumTickedStateTree, ExecutionContext.GetNumEntities(), ECsvCustomStatOp::Accumulate);
