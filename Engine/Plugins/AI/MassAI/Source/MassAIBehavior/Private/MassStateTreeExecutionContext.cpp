...
CSV_DECLARE_CATEGORY_EXTERN(MassStateTreeProcessor);
...
{
	CSV_SCOPED_TIMING_STAT(MassStateTreeProcessor, CollectExternalData);
	...
	for (int32 Index = 0; Index < ExternalDataDescs.Num(); Index++)
	{
		...
		if (UE::Mass::IsA<FMassFragment>(DataDesc.Struct))
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogMass, Error, "Missing Fragment: %ls", *GetNameSafe(ScriptStruct));
		...
		else if (UE::Mass::IsA<FMassSharedFragment>(DataDesc.Struct))
		{
			...
			else
			{
				if (DataDesc.Requirement == EStateTreeExternalDataRequirement::Required)
				{
					UE_LOGF(LogMass, Error, "Missing Shared Fragment: %ls", *GetNameSafe(ScriptStruct));
		...
		else if (UE::Mass::IsA<FMassConstSharedFragment>(DataDesc.Struct))
		{
			...
			else
			{
				if (DataDesc.Requirement == EStateTreeExternalDataRequirement::Required)
				{
					UE_LOGF(LogMass, Error, "Missing Const Shared Fragment: %ls", *GetNameSafe(ScriptStruct));
		...
		else if (DataDesc.Struct && DataDesc.Struct->IsChildOf(UWorldSubsystem::StaticClass()))
		{
			...
			else
			{
				if (DataDesc.Requirement == EStateTreeExternalDataRequirement::Required)
				{
					UE_LOGF(LogMass, Error, "Missing Subsystem: %ls", *GetNameSafe(SubClass));
