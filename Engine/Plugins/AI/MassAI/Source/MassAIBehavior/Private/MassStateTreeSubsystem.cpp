			...
			for (const FMassStateTreeDependency& Dependency : DependenciesView)
			{
				if (Dependency.Type)
				{
					if (const UScriptStruct* ScriptStruct = Cast<const UScriptStruct>(Dependency.Type.Get()))
					{
						...
						{
							UE_LOGF(LogMassBehavior, Error, "Unhandled Mass State Tree dependency %ls", *Dependency.Type->GetName());
						}
					}
					...
					{
						UE_LOGF(LogMassBehavior, Error, "Unhandled Mass State Tree dependency user-type %ls", *Dependency.Type->GetName());
					}
				}
			}
		}
