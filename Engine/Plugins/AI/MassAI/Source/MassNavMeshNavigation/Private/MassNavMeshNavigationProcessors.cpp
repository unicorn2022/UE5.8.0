...
void UMassNavMeshPathFollowProcessor::CheckEndOfPathReached(FMassNavMeshShortPathFragment& ShortPath, 
	const FVector& EntityLocation, const uint8 EndPointIndex, const FMassEntityHandle& Entity, bool bDisplayDebug)
{
	ensure(!ShortPath.bPartialResult);

	// EndReachedDistance is expected to be a small value.
	// The goal is to trigger the path as done once we cross the finish line even if the entity is far from the path segment.
	// For big values this would mean backtracking on multiple segments. We don't want to waste time doing that, and we would be
	// limited by the size short path anyway, so we just do a rough check for big EndReachedDistance values.
	const float DistanceToEndSquared = (EntityLocation - ShortPath.Points[EndPointIndex].Position).SquaredLength();
	if (DistanceToEndSquared < FMath::Square(ShortPath.EndReachedDistance))
	{
		UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Log,
			TEXT("Entity [%s]   Finished path follow on short path, end of path."), *Entity.DebugGetDescription());
		ShortPath.bDone = true;
		return;
	}

	// Normal case: compute distance along final segment to detect if entity reached the end.
	if (ShortPath.MoveTargetProgressDistance > ShortPath.Points[EndPointIndex-1].Distance.Get())
	{
		const float SegmentSize = ShortPath.Points[EndPointIndex].Distance.Get() - ShortPath.Points[EndPointIndex-1].Distance.Get();
		const FVector PreviousPointLocation = ShortPath.Points[EndPointIndex-1].Position;
		const FVector PathDir = (ShortPath.Points[EndPointIndex].Position - PreviousPointLocation).GetSafeNormal();
		...
		{
			UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Log,
				TEXT("Entity [%s]   Finished path follow on short path, end of path. Segment %i-%i"), *Entity.DebugGetDescription(), EndPointIndex-1, EndPointIndex);
			ShortPath.bDone = true;
		}
	}
}
...
void UMassNavMeshPathFollowProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery_Conditional.ForEachEntityChunk(Context, [this, &EntitiesToSignalPathDone](FMassExecutionContext& Context)
	{
		...
		for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
			...
			{
				UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Verbose,
					TEXT("Entity [%s] Updating navmesh path following"), *Entity.DebugGetDescription());
				...
				if (!bWasDone)
				{
					...
					UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Verbose, TEXT("Entity [%s]   ProgressDistance: %.2f, UpdatePointIndex: %i, EntityDistanceToGoal: %.2f, bPartialResult: %i"),
						*Entity.DebugGetDescription(), ShortPath.MoveTargetProgressDistance, UpdatePointIndex, MoveTarget.EntityDistanceToGoal, ShortPath.bPartialResult);
					...
					if (bDisplayDebug)
					{
						...
						const float DistanceAB = FMath::Max(PointB.Distance.Get() - PointA.Distance.Get(), KINDA_SMALL_NUMBER);
						const float T = (EntityProgress - ShortPath.Points[ClosestSegmentIndex].Distance.Get()) / DistanceAB;
					...
					if (ShortPath.MoveTargetProgressDistance <= 0.0f)
					{
						...
						UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Log,
							TEXT("Entity [%s]   before start of path. EntityDistanceToGoal: %.1f, DistanceToGoal: %.1f."),
					...
					else if (ShortPath.MoveTargetProgressDistance <= ShortPath.Points[UpdatePointIndex].Distance.Get())
					{
						...
						const float SegmentDistanceDelta = FMath::Max(NextPoint.Distance.Get() - CurrPoint.Distance.Get(), KINDA_SMALL_NUMBER);
						const float T = (ShortPath.MoveTargetProgressDistance - CurrPoint.Distance.Get()) / SegmentDistanceDelta;
						...
						UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Verbose, TEXT("Entity [%s]   Distance to goal on short path: %.1f."),
							*Entity.DebugGetDescription(), MoveTarget.DistanceToGoal);

						if (!ShortPath.bPartialResult)
						{
							// Last section of the path, wait for the steering to complete the movement.
							UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Log,
								TEXT("Entity [%s]   Waiting to reach the end of path."), *Entity.DebugGetDescription());

							CheckEndOfPathReached(ShortPath, EntityLocation, UpdatePointIndex, Entity, bDisplayDebug);
						}
					}
					else
					{
						...
						{
							UE_CVLOG_UELOG(bDisplayDebug, this, LogMassNavMeshNavigation, Log,
								TEXT("Entity [%s]   Finished path follow on short path."), *Entity.DebugGetDescription());
