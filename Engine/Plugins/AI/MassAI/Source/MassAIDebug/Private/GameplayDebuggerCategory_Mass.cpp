...
void FGameplayDebuggerCategory_Mass::CollectData(APlayerController* OwnerPC, AActor* DebugActor)
{
	...
	if (CachedEntity.IsValid() == true && EntityManager.IsEntityValid(CachedEntity) == false)
	{
		ClearCachedEntity();
	}
	...
	if (bShowShapes && Debugger)
	{
		...
		Debugger->ForEachShape(EMassEntityDebugShape::Box, [this](const UMassDebuggerSubsystem::FShapeDesc& Desc)
			{
		...
		Debugger->ForEachShape(EMassEntityDebugShape::Cone, [this](const UMassDebuggerSubsystem::FShapeDesc& Desc)
			{
		...
		Debugger->ForEachShape(EMassEntityDebugShape::Cylinder, [this](const UMassDebuggerSubsystem::FShapeDesc& Desc)
			{
		...
		Debugger->ForEachShape(EMassEntityDebugShape::Capsule, [this](const UMassDebuggerSubsystem::FShapeDesc& Desc)
			{
		...
		if (CachedEntity.IsSet())
		{
			...
			if (EntityManager.IsEntityActive(CachedEntity))
			{
				...
				else
				{
					...
					Composition.Get<FMassTagBitSet>().DebugGetIndividualNames(ItemNames);
					AddTextLine(FString::Printf(TEXT("{Green}Tags:{White}%s"), *DescriptionBuilder(ItemNames)));
					
					ItemNames.Reset();
					Composition.Get<FMassFragmentBitSet>().DebugGetIndividualNames(ItemNames);
					AddTextLine(FString::Printf(TEXT("{Green}Fragments:{White}%s"), *DescriptionBuilder(ItemNames)));
					
					ItemNames.Reset();
					Composition.Get<FMassChunkFragmentBitSet>().DebugGetIndividualNames(ItemNames);
					AddTextLine(FString::Printf(TEXT("{Green}Chunk Fragments:{White}%s"), *DescriptionBuilder(ItemNames)));

					ItemNames.Reset();
					Composition.Get<FMassSharedFragmentBitSet>().DebugGetIndividualNames(ItemNames);
					AddTextLine(FString::Printf(TEXT("{Green}Shared Fragments:{White}%s"), *DescriptionBuilder(ItemNames)));

					ItemNames.Reset();
					Composition.Get<FMassConstSharedFragmentBitSet>().DebugGetIndividualNames(ItemNames);
	...
	if (bShowNearEntityOverview && OwnerPC)
	{
		...
		if (bShowNearEntityAvoidance)
		{
			...
			EntityColliderQuery.AddRequirement<FMassAvoidanceColliderFragment>(EMassFragmentAccess::ReadOnly, EMassFragmentPresence::Optional);
			...
			EntityColliderQuery.ForEachEntityChunk(Context, [this, ViewLocation, ViewDirection](const FMassExecutionContext& Context)
			{
				...
				const bool bHasCollider = (CollidersList.Num() > 0);
