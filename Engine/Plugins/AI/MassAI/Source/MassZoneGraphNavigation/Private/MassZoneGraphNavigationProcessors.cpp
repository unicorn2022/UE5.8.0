...
UMassZoneGraphLocationInitializer::UMassZoneGraphLocationInitializer()
	: EntityQuery(*this)
{
	ObservedTypes.Add(FMassZoneGraphLaneLocationFragment::StaticStruct());
...
void UMassZoneGraphPathFollowProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery_Conditional.ForEachEntityChunk(Context, [FallbackLogOwner = this, &EntitiesToSignalPathDone, &EntitiesToSignalLaneChanged, DebugDrawWorld](FMassExecutionContext& Context)
	{
		const UZoneGraphSubsystem& ZoneGraphSubsystem = Context.GetSubsystemChecked<UZoneGraphSubsystem>();
		...
		for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
			...
			const UE::MassNavigation::Debug::FDebugContext NavigationDebugContext(Context, FallbackLogOwner, LogMassNavigation, DebugDrawWorld, Entity, EntityIt);
						...
						if (ShortPath.NextLaneHandle.IsValid())
						{
							...
							if (ZoneGraphStorage != nullptr)
							{
								...
								else if (ShortPath.NextExitLinkType == EZoneLaneLinkType::Adjacent)
								{
									...
									float DistanceSqr;
									if (UE::ZoneGraph::Query::FindNearestLocationOnLane(*ZoneGraphStorage, ShortPath.NextLaneHandle,  MoveTarget.Center, std::numeric_limits<float>::infinity(), NewLocation, DistanceSqr))
									{
										float NewLaneLength = 0.f;
										UE::ZoneGraph::Query::GetLaneLength(*ZoneGraphStorage, ShortPath.NextLaneHandle, NewLaneLength);
...
void UMassZoneGraphLaneCacheBoundaryProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQuery.ForEachEntityChunk(Context, [FallbackLogOwner = this, DebugDrawWorld](FMassExecutionContext& Context)
	{
		...
		for (FMassExecutionContext::FEntityIterator EntityIt = Context.CreateEntityIterator(); EntityIt; ++EntityIt)
		{
			...
			const UE::MassNavigation::Debug::FDebugContext ObstacleDebugContext(Context, FallbackLogOwner, LogAvoidanceObstacles, DebugDrawWorld, Entity, EntityIt);
			...
			{
				Points[Index] = CachedLane.LanePoints[FirstSegment + Index];
			}
