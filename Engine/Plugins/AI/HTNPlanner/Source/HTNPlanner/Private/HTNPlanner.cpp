...
bool FHTNPlanner::GeneratePlan(const FHTNDomain& Domain, const FHTNWorldState& InitialWorldState, FHTNResult& Result, const FName& StartTaskName)
{
	...
	{
		// @todo log an error
		UE_CLOGF(!GIsAutomationTesting, LogHTNPlanner, Warning, "Unable to find start task %ls. Make sure your Domain is compiled before using it for planning.", *StartTaskName.ToString());
		return false;
	}
