...
bool FHTNBuilder_Domain::Compile()
{
	...
	for (auto It = CompositeTasks.CreateConstIterator(); It; ++It)
	{
		...
		for (int32 MethodIndex = 0; MethodIndex < CompositeTask.MethodsCount; ++MethodIndex)
		{
			...
			for (int32 TaskIndex = 0; TaskIndex < Method.TasksCount; ++TaskIndex)
			{
				...
				UE_CLOGF(bIssuesFound && !GIsAutomationTesting, LogHTNPlanner, Warning
					, "Task %ls of %ls[%d] not found! Make sure it's defined in your domain. Domain compilation aborted."
	...
	else if (MemoryUsed > 0)	// meaning we have anything scored
	{
		...
		{
			UE_CLOGF(!GIsAutomationTesting, LogHTNPlanner, Warning
				, "Unable to find root task under the name %ls. Falling back to the first compound task, or first primitive task"
