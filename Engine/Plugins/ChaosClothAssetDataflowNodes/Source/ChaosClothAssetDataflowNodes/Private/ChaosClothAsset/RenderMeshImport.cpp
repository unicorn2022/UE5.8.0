...
namespace UE::Chaos::ClothAsset
{
	...
	)
	{
		...
		{
			MaterialPaths.Add(StaticMaterial.MaterialInterface ? StaticMaterial.MaterialInterface.GetPathName() : FString());
		}

		AddRenderSections(ClothCollection, MaterialPaths, OriginalTrianglesName, OriginalVerticesName);
	}

	void FRenderMeshImport::AddRenderSections(
		const TSharedRef<FManagedArrayCollection> ClothCollection,
		const TArray<FSoftObjectPath>& MaterialPaths,
		const FName OriginalTrianglesName,
		const FName OriginalVerticesName
	)
	{
		...
		for (const TPair<int32, FSection>& Section : Sections)
		{
			...
			if (NumVertices && NumTriangles)
			{
				...
				if (MaterialPaths.IsValidIndex(MaterialIndex))
				{
					ClothPatternFacade.SetRenderMaterialSoftObjectPathName(MaterialPaths[MaterialIndex]);
				}
			}
		}
	}

	void FRenderMeshImport::AddRenderPatternSelectionSets(
		const TSharedRef<FManagedArrayCollection> ClothCollection,
	...
	)
	{
		...
		if (!ClothFacade.IsValid())
		{
			// The caller should ensure this is a cloth collection
			return;
		}
		...
		const TConstArrayView<TArray<int32>> OriginalTriangles = ClothFacade.GetUserDefinedAttribute<TArray<int32>>(
			OriginalTrianglesName,
		...
		if (OriginalTriangles.Num())
		{
			...
			const TArray<int32> OriginalToNewTriangles = FClothDataflowTools::GetOriginalToNewIndices<TArray<int32>>(
				OriginalTriangles,
				TriangleToVertexIndex.Num()
			);

			for (const TPair<FName, TSet<int32>>& PatternNameFaces : PatternNamesToIndices)
			{
