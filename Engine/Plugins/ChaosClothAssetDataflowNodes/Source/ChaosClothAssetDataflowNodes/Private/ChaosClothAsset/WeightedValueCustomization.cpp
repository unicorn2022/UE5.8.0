			...
			if (!Private::ImportFabricBoundsProperty(PropertyHandle) && !BuildFabricMapsProperty(PropertyHandle) && !CouldUseFabricsProperty(PropertyHandle))
			{
				return
					...
					.IsChecked_Lambda([WeakHandlePtr]()->ECheckBoxState
						{
							bool bValue = false;
							if (const TSharedPtr<IPropertyHandle> BoolPropertyHandle = WeakHandlePtr.Pin())
							{
					...
					.OnCheckStateChanged_Lambda([WeakHandlePtr](ECheckBoxState CheckBoxState)
						{
							if (const TSharedPtr<IPropertyHandle> BoolPropertyHandle = WeakHandlePtr.Pin())
							{
								BoolPropertyHandle->SetValue(CheckBoxState == ECheckBoxState::Checked, EPropertyValueSetFlags::DefaultFlags);
							}
						});
			}
		}
		...
		TSharedRef<IPropertyHandle>& PropertyHandle)
	{
		...
		return SNew(SNumericEntryBox<float>)
			...
			.Value_Lambda([WeakHandlePtr]()
				{
					float Value = 0.;
					if (const TSharedPtr<IPropertyHandle> FloatPropertyHandle = WeakHandlePtr.Pin())
					{
						if (FloatPropertyHandle->GetValue(Value) == FPropertyAccess::Success)
						{
							return TOptional<float>(Value);
						}
					}
					return TOptional<float>();  // Value couldn't be accessed, return an unset value
				})
			...
			.OnValueChanged_Lambda([this, WeakHandlePtr](float Value)
				{
					if (bIsUsingSlider)
					{
						if (const TSharedPtr<IPropertyHandle> FloatPropertyHandle = WeakHandlePtr.Pin())
						{
							FloatPropertyHandle->SetValue(Value, EPropertyValueSetFlags::InteractiveChange | EPropertyValueSetFlags::NotTransactable);
						}
					}
				})
