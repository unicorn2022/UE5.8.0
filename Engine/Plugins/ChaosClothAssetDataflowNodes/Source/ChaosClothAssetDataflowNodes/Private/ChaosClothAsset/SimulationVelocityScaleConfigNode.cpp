...
void FChaosClothAssetSimulationVelocityScaleConfigNode::AddProperties(FPropertyHelper& PropertyHelper) const
{
	...
	PropertyHelper.SetPropertyEnum(this, &FictitiousForcesModel);
}

void FChaosClothAssetSimulationVelocityScaleConfigNode::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
	Ar.UsingCustomVersion(FFortniteReleaseBranchCustomObjectVersion::GUID);
	if (Ar.CustomVer(FFortniteReleaseBranchCustomObjectVersion::GUID) < FFortniteReleaseBranchCustomObjectVersion::ChaosClothAddNewFictitiousForcesModes)
	{
		FictitiousForcesModel = EChaosClothingSimulationSolverFictitiousForcesModel::Legacy;
	}
}
