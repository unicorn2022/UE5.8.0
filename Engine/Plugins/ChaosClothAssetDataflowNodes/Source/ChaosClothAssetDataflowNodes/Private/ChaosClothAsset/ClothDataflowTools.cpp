...
namespace UE::Chaos::ClothAsset
{
	...
	void FClothDataflowTools::LogAndToastWarning(const FDataflowNode& DataflowNode, const FText& Headline, const FText& Details)
	{
		...
		UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "%ls", *Text.ToString());
	}
	...
	bool FClothDataflowTools::BuildSkeletalMeshModelFromMeshDescription(const FMeshDescription* const InMeshDescription, const FMeshBuildSettings& InBuildSettings, FSkeletalMeshLODModel& SkeletalMeshModel)
	{
		...
		// Convert static mesh linear to sRGB
		TVertexInstanceAttributesRef<FVector4f> VertexInstanceColors = SkeletalMeshAttributes.GetVertexInstanceColors();
		for (const FVertexInstanceID VertexInstanceID : SkeletalMeshGeometry.VertexInstances().GetElementIDs())
		{
			FVector4f VertexColor = VertexInstanceColors.Get(VertexInstanceID);
			VertexColor.X = FMath::Clamp(VertexColor.X, 0.f, 1.f);  // The clamp is required to exactly match the behavior in
			VertexColor.Y = FMath::Clamp(VertexColor.Y, 0.f, 1.f);  // FStaticToSkeletalMeshConverter::InitializeSkeletonFromStaticMesh()
			VertexColor.Z = FMath::Clamp(VertexColor.Z, 0.f, 1.f);  // while allowing to reuse LinearToSRGB
			UE::Geometry::LinearColors::LinearToSRGB(VertexColor);
		...
		if (!MeshUtilities.BuildSkeletalMesh(SkeletalMeshModel, SkeletalMeshName, RootBoneRefSkeleton, LODInfluences, LODWedges, LODFaces, LODPoints, LODPointToRawMap, BuildOptions, &WarningMessages))
		{
			...
			{
				UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "%ls", *Message.ToString());
			}
			return false;
		}
		return true;
	...
	bool FClothDataflowTools::FSimMeshCleanup::RemoveDegenerateTriangles()
	{
		...
		UE_CLOGF(bHasDegenerateTriangles, LogChaosClothAssetDataflowNodes, Display,
			"USD import found and removed %d degenerated triangles out of %d source triangles.", TriangleCount - OutTriangleCount, TriangleCount);
	...
	bool FClothDataflowTools::FSimMeshCleanup::RemoveDuplicateTriangles()
	{
		...
		UE_CLOGF(bHasDuplicatedTriangles, LogChaosClothAssetDataflowNodes, Display,
			"USD import found and removed %d duplicated triangles out of %d source triangles.", TriangleCount - TriangleToVertexIndex.Num(), TriangleCount);
	...
	template CHAOSCLOTHASSETDATAFLOWNODES_API TArray<int32> FClothDataflowTools::GetOriginalToNewIndices<TArray<int32>>(const TConstArrayView<TArray<int32>>& NewToOriginals, int32 NumOriginalIndices);
	template CHAOSCLOTHASSETDATAFLOWNODES_API TArray<int32> FClothDataflowTools::GetOriginalToNewIndices<TSet<int32>>(const TConstArrayView<TSet<int32>>& NewToOriginals, int32 NumOriginalIndices);
	...
	bool FClothDataflowTools::RemoveDuplicateStitches(TArray<TArray<FIntVector2>>& SeamStitches)
	{
		...
		UE_CLOGF(bHasDuplicateStitches, LogChaosClothAssetDataflowNodes, Display,
			"USD import found and removed %d duplicated stitches out of %d source stitches.", NumStitches - OutNumStitches, NumStitches);
