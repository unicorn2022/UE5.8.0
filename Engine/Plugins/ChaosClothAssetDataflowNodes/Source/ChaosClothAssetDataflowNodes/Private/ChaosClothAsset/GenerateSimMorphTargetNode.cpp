...
void FChaosClothAssetGenerateSimMorphTargetNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA<FManagedArrayCollection>(&Collection))
	{
		...
		if (ClothFacade.IsValid())
		{
			if (MorphTargetClothFacade.IsValid())
			{
				if (!MorphTargetName.IsEmpty())
				{
					...
					if (NumSimVertices == NumMorphSimVertices)
					{
						...
						{
							Context.Warning(LOCTEXT("IdenticalCollectionsDetails", "No morph targets generated because the sim collections are identical."), this, Out);
						}
					}
					else
					{
						Context.Warning(
							FText::Format(LOCTEXT("VertexCountMismatchDetail", "Vertex count mismatch {0} != {1}."), NumSimVertices, NumMorphSimVertices)
							, this, Out
						);
					}
				}
			...
			{
				Context.Warning(LOCTEXT("InvalidMorphTargetCollectionDetails", "Input Morph Target Collection is not a valid Cloth Collection."), this, Out);
			}
		}
