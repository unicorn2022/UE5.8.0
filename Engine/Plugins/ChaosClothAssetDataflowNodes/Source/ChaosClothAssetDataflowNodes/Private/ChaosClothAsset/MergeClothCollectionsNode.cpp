			  ...
			  const FName& PropertyName)
	{
		...
		else
		{
			...
			if(!ensure(OutPropertyFacade.IsIntrinsic(OutKeyIndex) == InPropertyFacade.IsIntrinsic(InKeyIndex)))
			{
				...
				UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "MergeClothCollectionsNode: Mismatch in intrinsic flag onto %ls property", *PropertyNameString);
			}
			if(!ensure(OutPropertyFacade.IsLegacy(OutKeyIndex) == InPropertyFacade.IsLegacy(InKeyIndex)))
			{
				const FString PropertyNameString = PropertyName.ToString();
				UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "MergeClothCollectionsNode: Mismatch in legacy flag onto %ls property", *PropertyNameString);
			}
			if(!ensure(OutPropertyFacade.IsInterpolable(OutKeyIndex) == InPropertyFacade.IsInterpolable(InKeyIndex)))
			{
				const FString PropertyNameString = PropertyName.ToString();
				UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "MergeClothCollectionsNode: Mismatch in interpolable flag onto %ls property", *PropertyNameString);
			}
		}
	...
	if (Out->IsA<FManagedArrayCollection>(&Collection))
	{
		...
		for (int32 InputIndex = 1; InputIndex < Collections.Num(); ++InputIndex)
		{
			...
			if (OtherClothFacade.IsValid())
			{
				...
				{
					Context.Warning(IncompatibleErrorDetails, this, Out);
				}
			}
