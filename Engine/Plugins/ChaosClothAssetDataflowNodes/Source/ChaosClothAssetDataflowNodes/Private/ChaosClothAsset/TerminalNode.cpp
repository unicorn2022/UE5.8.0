...
namespace UE::Chaos::ClothAsset::Private
{
	...
	bool ArePropertiesPreserved(const TArray<TSharedRef<const FManagedArrayCollection>>& Collections0, const TArray<TSharedRef<const FManagedArrayCollection>>& Collections1, bool& bNeedsReset)
	{
		...
		{
			FCollectionPropertyConstFacade Property0(Collections0[LODIndex].ToSharedPtr());
			FCollectionPropertyConstFacade Property1(Collections1[LODIndex].ToSharedPtr());
			...
			for (int32 PropertyIndex = 0; PropertyIndex < Property0.Num(); ++PropertyIndex)
			{
				...
				if (Property0.IsIntrinsic(PropertyIndex) || Property1.IsIntrinsic(PropertyIndex))
				{
					if (!FCollectionPropertyConstFacade::AreValuesEqual(Property0, Property1, PropertyIndex))
					{
						return false;  // Intrinsic property change needs full rebuild
					}
				}
				if (!Property0.IsAnimatable(PropertyIndex) || !Property1.IsAnimatable(PropertyIndex))
				{
					if (!FCollectionPropertyConstFacade::AreValuesEqual(Property0, Property1, PropertyIndex))
					{
						bNeedsReset = true;  // Non animatable property change needs hard reset
					}
				}
			}
		}
...
void FChaosClothAssetTerminalNode_v2::SetAssetValue(TObjectPtr<UObject> Asset, UE::Dataflow::FContext& Context) const
{
	if (UChaosClothAsset* ClothAsset = Cast<UChaosClothAsset>(Asset.Get()))
	{
		...
		bool bNeedsReset;
		if (bPreviousChecksumsValid && PreviousChecksum == ClothColllectionChecksum &&
			Private::ArePropertiesPreserved(InClothCollections, ClothCollections, bNeedsReset))
		{
			...
			ClothAsset->SetClothCollections(MoveTemp(ClothCollections), bNeedsReset);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		bool bNeedsReset;
		if (bPreviousChecksumsValid && PreviousChecksum == ClothColllectionChecksum &&
			Private::ArePropertiesPreserved(InClothCollections, RefClothCollections, bNeedsReset))
		{
			// Cloth and property keys match. Just update property values.
