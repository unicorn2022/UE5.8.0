...
{	
		...
		UE::Geometry::FDynamicMesh3& Mesh) // the resulting sim or render mesh
	{
		...
		{
			UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Failed to convert the Cloth Collection to Dynamic Mesh. Cloth Collection is empty.");
			return false;
		}
													   ...
													   const TransferBoneWeightsSettings& TransferSettings)
	{
		...
		for (int32 BatchIndex = 0; BatchIndex < NumBatches; BatchIndex++)
		{
				...
				&TargetDynamicMesh, &ForceInpaintWeightLayer, &TransferSettings, &SourceDynamicMesh, &SourceBVH, &Submeshes, &SubmeshToBaseVs, &bTransferParametersInvalid]()
			{
				...
				for (int32 ComponentIdx = StartIndex; ComponentIdx < EndIndex; ++ComponentIdx)
				{
					...
					{
						UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Failed to transfer skin weights to some of the vertices of the render mesh using inpaint method, falling back to closest point method.");
						...
						{
							UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode:  Failed to transfer skin weights to some of the vertices of the render mesh.");
						}
		...
		{
			UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Transfer method parameters are invalid.");
			return false;
		}
		...
		UE::Geometry::FDynamicMesh3& WeldedSimMesh)
	{
		...
		{
			UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Failed to weld the simulation mesh for LOD.");
			return false;
		}
		...
		{
			UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Transferring skin weights failed.");
			return false;
		}
		...
		const TransferBoneWeightsSettings& TransferSettings)
	{
		...
		{
			UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Failed to create the render dynamic mesh for LOD.");
			return false;
		}
		...
		if (NonManifoldMapping.IsNonManifoldVertexInSource())
		{
			UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Render mesh inpaint method failed due to non manifold geometry, falling back to closest point method instead.");
		...
		else
		{
			...
			{
				UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Failed to transfer skin weights to render mesh.");
				return false;
			}
		}
		...
		if (ClothFacade.IsValid())  // Can only act on the collection if it is a valid cloth collection
		{
			...
			else
			{
				...
				if (SimClothCollection.IsValid())
				{
					...
					{
						Context.Warning(LOCTEXT("InvalidSimRefSkeletonDetails", "Couldn't find a valid reference skeleton from the input sim collection."), this, Out);
						SetValue(Context, MoveTemp(*ClothCollection), &Collection);
						return;
					}
				}
				else
				{
					...
					if (!TargetRefSkeleton)
					{
						Context.Warning(LOCTEXT("InvalidRefSkeletonDetails", "Couldn't find a valid reference skeleton from the input collection."), this, Out);
						SetValue(Context, MoveTemp(*ClothCollection), &Collection);
						return;
					}
				}
			...
			if (!bTransferResult)
			{
				Context.Warning(LOCTEXT("TransferWeightsDetails", "Failed to transfer skinning weights from the source."), this, Out);
				SetValue(Context, MoveTemp(*ClothCollection), &Collection);
				return;
			}
			...
			if (bCheckMaxInfluenceComplience)
			{
				for (int32 VID = 0; VID < ClothFacade.GetNumSimVertices3D(); ++VID)
				{
					...
					{
						UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Maximum number of influences is exceeded for sim vertex %i.", VID);
					}
				}
				...
				for (int32 VID = 0; VID < ClothFacade.GetNumRenderVertices(); ++VID)
				{
					...
					if (!ensure(NumInfluences <= static_cast<int32>(FChaosClothAssetTransferSkinWeightsNode::MaxNumInfluences)))
					{
						UE_LOGF(LogChaosClothAssetDataflowNodes, Warning, "TransferSkinWeightsNode: Maximum number of influences is exceeded for render vertex %i.", VID);
					}
				}
			}
