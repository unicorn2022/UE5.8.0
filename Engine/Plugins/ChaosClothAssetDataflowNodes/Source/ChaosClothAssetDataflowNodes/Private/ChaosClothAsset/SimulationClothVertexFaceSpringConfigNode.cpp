...
#define LOCTEXT_NAMESPACE "SimulationClothVertexFaceSpringConfigNode"
...
void FChaosClothAssetSimulationClothVertexFaceSpringConfigNode::EvaluateClothCollection(UE::Dataflow::FContext& Context, const TSharedRef<FManagedArrayCollection>& ClothCollection) const
{
	...
	auto CheckVertexIndicesValidity = [](const TArray<TArray<int32>>& IndicesArray, const int32 VertexCount) -> bool
		{
			for (const TArray<int32>& Indices : IndicesArray)
			{
	...
	if (ClothFacade.IsValid() && SpringFacade.IsValid())
	{
		...
		const bool bSourceIndicesAreValid = CheckVertexIndicesValidity(SourceIndicesArray, ClothFacade.GetNumSimVertices3D());
		if (!bSourceIndicesAreValid)
		{
			Context.Error(LOCTEXT("VertexFaceSpring_InvalidSourceIndices", "invalid source indices detected, ignoring constraints, please regenerate constraints as the topology of the sim mesh may have changed"), this);
			return;
		}

		const bool bTargetIndicesAreValid = CheckVertexIndicesValidity(TargetIndicesArray, ClothFacade.GetNumSimVertices3D());
		if (!bTargetIndicesAreValid)
		{
			Context.Error(LOCTEXT("VertexFaceSpring_InvalidTargetIndices", "invalid target indices detected, ignoring constraints, please regenerate constraints as the topology of the sim mesh may have changed"), this);
			return;
		}
