...
#define LOCTEXT_NAMESPACE "SimulationClothVertexSpringConfigNode"
...
void FChaosClothAssetSimulationClothVertexSpringConfigNode::EvaluateClothCollection(UE::Dataflow::FContext& Context, const TSharedRef<FManagedArrayCollection>& ClothCollection) const
{
	...
	auto CheckVertexIndicesValidity = [](const TArray<FIntVector2>& IndexPairs, int32 VertexCount)
		{
			for (const FIntVector2& IndexPair : IndexPairs)
			{
				if (IndexPair[0] < 0 || IndexPair[0] >= VertexCount)
				{
					return false;
				}
				if (IndexPair[1] < 0 || IndexPair[1] >= VertexCount)
				{
					return false;
				}
			}
	...
	{
		const bool bVertexIndicesAreValid = CheckVertexIndicesValidity(ConstraintVertices, ClothFacade.GetNumSimVertices3D());
		if (!bVertexIndicesAreValid)
		{
			Context.Error(LOCTEXT("VertexSpring_InvalidVertexIndices", "invalid vertex indices detected, ignoring constraints, please regenerate constraints as the topology of the sim mesh may have changed"), this);
			return;
		}
