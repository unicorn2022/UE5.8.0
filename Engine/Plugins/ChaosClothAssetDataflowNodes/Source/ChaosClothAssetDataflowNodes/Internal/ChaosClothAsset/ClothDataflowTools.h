...
namespace UE::Chaos::ClothAsset
{
	...
	*/
	struct FClothDataflowTools
	{
		...
		UE_API static void LogAndToastWarning(const FDataflowNode& DataflowNode, const FText& Headline, const FText& Details);
		...
		struct FSimMeshCleanup
		{
			...
			UE_API bool RemoveDegenerateTriangles();
			UE_API bool RemoveDuplicateTriangles();
		};
		...
		UE_API static TArray<int32> GetOriginalToNewIndices(const TConstArrayView<T>& NewToOriginals, int32 NumOriginalIndices);

		UE_DEPRECATED(5.6, "Use FSimMeshCleanup instead.")
		UE_API static bool RemoveDegenerateTriangles(
		...
		UE_API static bool RemoveDuplicateStitches(TArray<TArray<FIntVector2>>& SeamStitches);
...
UE_API DECLARE_LOG_CATEGORY_EXTERN(LogChaosClothAssetDataflowNodes, Log, All);
