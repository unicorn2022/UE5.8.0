		...
		] (uint32 Y)
		{
			...
			for (int32 X = 0; X < FullColumns; ++X)
			{
				if constexpr (PIXEL_SIZE == 4)
				{
					...
					// Average of 2 unsigned integers with rounding without overflow extended to work on multiple bytes.
					constexpr uint64 ShiftMask = 0x7F7F7F7F7F7F7F7F;
			
					const uint64 AvgLowBits = (Row0Bits | Row1Bits) - (((Row0Bits ^ Row1Bits) >> 1) & ShiftMask);
					const uint64 AvgHighBits = AvgLowBits >> 32;
					const uint32 Result = (AvgLowBits | AvgHighBits) - (((AvgLowBits ^ AvgHighBits) >> 1) & ShiftMask);
				...
				else
				{
					...
					{
						int32 PixelSum = SourceRow0[C] + SourceRow0[PIXEL_SIZE + C] + SourceRow1[C] + SourceRow1[PIXEL_SIZE + C] + 2;
			...
			if (bStrayColumn)
			{
				...
				{	
					...
					// Average of 2 unsigned integers with rounding without overflow extended to work on multiple bytes.
					constexpr uint32 ShiftMask = 0x7F7F7F7F;
					const uint32 Result = (Row0Bits | Row1Bits) - (((Row0Bits ^ Row1Bits) >> 1) & ShiftMask);
				...
				else
				{
					for (int32 C = 0; C < PIXEL_SIZE; ++C)
					{
						int32 PixelSum = SourceRow0[C] + SourceRow1[C] + 1;
		...
		if (bStrayRow)
		{
			...
			for (int32 X = 0; X < FullColumns; ++X)
			{
				if constexpr (PIXEL_SIZE == 4)
				{
					...
					// Average of 2 unsigned integers with rounding without overflow extended to work on multiple bytes.
					constexpr uint32 ShiftMask = 0x7F7F7F7F;
					const uint32 Result = (Col0Bits | Col1Bits) - (((Col0Bits ^ Col1Bits) >> 1) & ShiftMask);
				...
				else
				{
					for (int32 C = 0; C < PIXEL_SIZE; ++C)
					{
						int32 P = SourceRow0[C] + SourceRow0[PIXEL_SIZE + C] + 1;
						DestRow[C] = (uint8)(P >> 1);
					}
				}
