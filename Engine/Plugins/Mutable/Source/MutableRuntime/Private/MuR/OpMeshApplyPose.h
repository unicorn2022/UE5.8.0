...
{
	inline void SetPoseAsReference(FMesh* InOutResult, const FMesh* PoseMesh, const TArray<int32>& BoneToPoseIndex)
	{
		...
		for (FMesh::FBonePose& BonePose : InOutResult->BonePoses)
		{
			const int32 FoundPoseIndex = BoneToPoseIndex[BonePose.BoneId.Index];
			...
			BonePose.BoneTransform = BonePose.BoneTransform * PoseTransform;
			...
			EnumAddFlags(BonePose.BoneUsageFlags, EBoneUsageFlags::Reshaped);
		}
	}
    ...
    */
    inline void MeshApplyPose(FMesh* Result, const FMesh* BaseMesh, const FMesh* PoseMesh, bool& bOutSuccess)
    {
		...
		TManagedPtr<const FSkeleton> BaseSkeleton = BaseMesh->GetSkeleton();
		TManagedPtr<const FSkeleton> PoseSkeleton = PoseMesh->GetSkeleton();
		if (!BaseSkeleton || !PoseSkeleton)
		{
			bOutSuccess = false;
			return;
		}
		...
		const int32 NumBonesBase = BaseSkeleton->GetNumBones();
		TArray<int32> BoneToPoseIndex;
		BoneToPoseIndex.Init(INDEX_NONE, NumBonesBase);

		for (int32 BoneIndex = 0; BoneIndex < NumBonesBase; ++BoneIndex)
		{
			const int32 BoneNameIndex = PoseSkeleton->BoneNames.Find(BaseSkeleton->BoneNames[BoneIndex]);
			if (BoneNameIndex == INDEX_NONE)
			{
				continue; // BaseSkeleton bone not found in PoseSkeleton.
			}

			const int32 BonePoseIndex = PoseMesh->FindBonePoseByBoneIndex(BoneNameIndex);
			...
			const int32 ParentIndex = BaseSkeleton->BoneParents[BoneIndex];
		...
		const TArray<FBoneIdOrIndex>& BoneMap = BaseMesh->GetBoneMap();
		...
		{
			const int32 PoseIndex = BoneToPoseIndex[BoneMap[Index].Index];
			if (PoseIndex != INDEX_NONE)
			{
				SkinTransforms.Add(PoseMesh->BonePoses[PoseIndex].BoneTransform);
				bBonesAffected = true;
			}
			else
			{
		...
		SetPoseAsReference(Result, PoseMesh, BoneToPoseIndex);
