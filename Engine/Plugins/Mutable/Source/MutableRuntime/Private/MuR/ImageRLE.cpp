...
namespace UE::Mutable::Private
{
	...
	struct FIterativeDecodeTokenRLE_L
	{
		uint32 SrcDataOffset = 0;
		uint16 NumEqual      = 0;
		uint8 NumDifferent   = 0;
		uint8 EqualValue     = 0;
	};
	static_assert(sizeof(FIterativeDecodeTokenRLE_L) == sizeof(uint64));

	struct FSequenceHeaderRLE_L
	{
		uint16 NumEqual;
		uint8 NumDifferent;
		uint8 EqualValue;
	};
	static_assert(sizeof(FSequenceHeaderRLE_L) == 4);

	uint64 GetBeginTokenIterativeDecodeRLE_L(const uint8* Src, int32 NumRows)
	{
		// Skip RLE size and rows header.
		Src += (NumRows + 1) * sizeof(uint32);

		// Read first header.
		FSequenceHeaderRLE_L FirstHeader;
		FMemory::Memcpy(&FirstHeader, Src, sizeof(FSequenceHeaderRLE_L));

		FIterativeDecodeTokenRLE_L Token
		{
			.SrcDataOffset = (NumRows + 1)*sizeof(uint32) + sizeof(FSequenceHeaderRLE_L),
			.NumEqual      = FirstHeader.NumEqual,
			.NumDifferent  = FirstHeader.NumDifferent,
			.EqualValue    = FirstHeader.EqualValue,
		};

		return BitCast<uint64>(Token);
	}

	void IterativeDecodeRLE_L(uint8* Dest, const uint8* Src, uint64& InOutDecodeToken, int32 NumElems)
	{
		FIterativeDecodeTokenRLE_L DecodeToken = BitCast<FIterativeDecodeTokenRLE_L>(InOutDecodeToken);

		// DecodeToken has the information about the previous data header and points to the first differnt value.
		Src = Src + DecodeToken.SrcDataOffset;

		FSequenceHeaderRLE_L SeqHeader
		{
			.NumEqual     = DecodeToken.NumEqual,
			.NumDifferent = DecodeToken.NumDifferent,
			.EqualValue   = DecodeToken.EqualValue,
		};

		for (int32 NumElemsDecoded = 0;;)
		{
			int32 NumEqualToDecode = SeqHeader.NumEqual; 
			int32 NumDifferentToDecode = SeqHeader.NumDifferent;

			if (NumElemsDecoded + NumEqualToDecode > NumElems)
			{
				NumEqualToDecode = NumElems - NumElemsDecoded;
				NumDifferentToDecode = 0;
			}
			else if (NumElemsDecoded + NumEqualToDecode + NumDifferentToDecode > NumElems)
			{
				NumDifferentToDecode = NumElems - (NumElemsDecoded + NumEqualToDecode);
			}

			FMemory::Memset(Dest + NumElemsDecoded, SeqHeader.EqualValue, NumEqualToDecode);
			FMemory::Memcpy(Dest + NumElemsDecoded + NumEqualToDecode, Src, NumDifferentToDecode);

			DecodeToken.NumEqual = SeqHeader.NumEqual - NumEqualToDecode; 
			DecodeToken.NumDifferent = SeqHeader.NumDifferent - NumDifferentToDecode;
			DecodeToken.SrcDataOffset += NumDifferentToDecode;

			Src += NumDifferentToDecode;
			NumElemsDecoded += NumEqualToDecode + NumDifferentToDecode;

			if (NumElemsDecoded >= NumElems)
			{
				break;
			}
		
            // Read next header, at this point Src is already pointing to the next sequence header as all
            // sequence data has been consumed.
            FMemory::Memcpy(&SeqHeader, Src, sizeof(FSequenceHeaderRLE_L));
       
            // We don't allow empty sequences. Stop decoding.
            if (SeqHeader.NumEqual + SeqHeader.NumDifferent == 0)
            {
                check(false);
                break;
            }

            // Point Src to the sequence data.
			Src += sizeof(FSequenceHeaderRLE_L);

			DecodeToken.SrcDataOffset += sizeof(FSequenceHeaderRLE_L);
			DecodeToken.EqualValue = SeqHeader.EqualValue;
			
		}

		InOutDecodeToken = BitCast<uint64>(DecodeToken);
	}
