...
namespace UE::Mutable::Private
{
	...
	void MeshFormatBuffer( const FMeshBufferSet& Source, FMeshBuffer& ResultBuffer, int32 ResultOffsetElements, bool bHasSpecialSemantics, uint32 IDPrefix )
	{
		...
		for (int32 ResultChannelIndex = 0; ResultChannelIndex < ResultBuffer.Channels.Num(); ++ResultChannelIndex)
		{
			...
			if (SourceBufferIndex < 0)
			{
				...
				// If we have to add color channels, we will add them as white, to be neutral.
	...
	)
	{
		...
		EMeshCopyFlags CopyFlags = EMeshCopyFlags::AllFlags;
		EnumRemoveFlags(CopyFlags, EMeshCopyFlags::WithVertexBuffers | EMeshCopyFlags::WithIndexBuffers | EMeshCopyFlags::WithLayouts);

		Result->CopyFrom(*Source, CopyFlags);

		if (bFormatVertices)
		{
			Result->VertexBuffers = Format->VertexBuffers;
		...
		if (bFormatIndices)
		{
			Result->IndexBuffers = Format->IndexBuffers;
		...
		Result->Layouts.Reset(Source->Layouts.Num());
		for (const TManagedPtr<const FLayout>& Layout : Source->Layouts)
		{
