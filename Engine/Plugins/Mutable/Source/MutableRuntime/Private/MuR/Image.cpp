...
namespace UE::Mutable::Private
{
	...
	void FImage::Init(uint32 SizeX, uint32 SizeY, uint32 Lods, EImageFormat Format, EInitializationType InitType)
	{
		...
		const FImageDesc ImageDesc {FImageSize(uint16(SizeX), uint16(SizeY)), Format, EImageFormat::None, static_cast<uint8>(Lods)};
	...
	TManagedRef<FImage> FImage::CreateAsReference(const TPassthroughObjectPtr<UTexture>& ExternalObject, const FImageDesc& Desc, bool bForceLoad)
	{
		TManagedRef<FImage> Result = MakeManaged<FImage>();
		Result->PassthroughObject = ExternalObject;
	...
	TManagedRef<FImage> FImage::StaticUnserialise(FInputArchive& Arch)
	{
		LLM_SCOPE_BYNAME(TEXT("MutableRuntime"));

		TManagedRef<FImage> ResultPtr = MakeManaged<FImage>();
		Arch >> *ResultPtr;
	...
	TManagedPtr<FImage> FImageOperator::ExtractMip(const FImage* This, int32 Mip)
	{
		...
		{
			TManagedPtr<FImage> Result = CreateImage(MipSize.X, MipSize.Y, 1, This->GetFormat(), EInitializationType::NotInitialized);
		...
		TManagedPtr<FImage> Resized = CreateImage(MipSize.X, MipSize.Y, 1, This->GetFormat(), EInitializationType::NotInitialized);
	...
	//---------------------------------------------------------------------------------------------
	bool FImage::IsPlainColor(FVector4f& OutColor) const
	{
		bool Result = true;
  ...
  //       }
	...
	void FImageOperator::FillColor(FImage* Target, FVector4f Color)
	{
		...
		default:
		{
			...
			const FImageFormatData& FormatData = GetImageFormatData(Format);
			TManagedPtr<FImage> BlockImage = CreateImage(
			...
			TManagedPtr<FImage> Converted = ImagePixelFormat(0, BlockImage.Get(), Format);
