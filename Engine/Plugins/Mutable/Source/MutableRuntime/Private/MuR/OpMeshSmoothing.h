...
{
	inline TArray<int32> MakeUniqueVertexMap(TArrayView<const FVector3f> Vertices)
	{
	...
	inline TArray<TArray<int32, TInlineAllocator<8>>> BuildVertexFaces(TArrayView<const uint32> Indices, const TArray<int32>& UniqueVertexMap)
	{
	...
	inline TMap<uint64, UE::Geometry::FIndex2i> BuildEdgesFaces(TArrayView<const uint32> Indices, const TArray<int32>& UniqueVertexMap)
	{
	...
	inline TTuple<TArray<int32>, TArray<int32>> BuildVertexRings(
		...
		const TMap<uint64, UE::Geometry::FIndex2i>& EdgesFaces)
	{
		...
		{
			UE_LOGF(LogMutableCore, Warning, 
				"%d Bowtied vertex rings found in a mesh while computing smoothing operation data, this is currently not supported. " 
	...
	inline void RemoveLaplacianData(FMesh& Mesh)
	{
		const int32 NewNumAdditionalBuffers = Algo::RemoveIf(Mesh.AdditionalBuffers,
		[](const TPair<EMeshBufferType, FMeshBufferSet>& A)
		{
			const bool bIsLaplacianBuffer =
				A.Key == EMeshBufferType::UniqueVertexMap   ||
				A.Key == EMeshBufferType::MeshLaplacianData ||
				A.Key == EMeshBufferType::MeshLaplacianOffsets;
			
			return bIsLaplacianBuffer;
		});

		Mesh.AdditionalBuffers.SetNum(NewNumAdditionalBuffers, EAllowShrinking::No);
	}

	inline void SmoothMeshLaplacian(FMesh& Mesh, bool bRemoveData, int32 Iterations = 1)
	{
		MUTABLE_CPUPROFILER_SCOPE(MeshSmoothing_SmoothMeshLaplacian);

		using BufferEntryType = TPair<EMeshBufferType, FMeshBufferSet>;

		const BufferEntryType* MeshLaplacianDataBuffer = Mesh.AdditionalBuffers.FindByPredicate(
				[](const BufferEntryType& E) { return E.Key == EMeshBufferType::MeshLaplacianData; });

		const BufferEntryType* MeshLaplacianOffsetsBuffer = Mesh.AdditionalBuffers.FindByPredicate(
				[](const BufferEntryType& E) { return E.Key == EMeshBufferType::MeshLaplacianOffsets; });

		const BufferEntryType* MeshUniqueVertexMapBuffer = Mesh.AdditionalBuffers.FindByPredicate(
		...
		{
			UE_LOGF(LogMutableCore, Warning, "Smooth laplacian with source mesh missing some necessary buffers. Ignoring operation.");
			return;
		}
		...
		const UntypedMeshBufferIterator SrcPositionBegin(Mesh.GetVertexBuffers(), EMeshBufferSemantic::Position);

		const int32 NumVertices = Mesh.GetVertexCount();

		TArray<FVector3f> AccumulatedPositions;
		AccumulatedPositions.SetNumUninitialized(NumVertices);

		FMemory::Memcpy(AccumulatedPositions.GetData(), SrcPositionBegin.ptr(), AccumulatedPositions.NumBytes());
		...
		for (int32 VertexIndex = 0, UniqueVertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex)
		{
			...
			AccumulatedPositions[VertexIndex] = VertexPosition + (VertexPositionLaplacian-VertexPosition)*0.5f;
			++UniqueVertexIndex;
		}

		// Copy positions to the collapsed vertices.
		constexpr SIZE_T PositionElemSize = sizeof(FVector3f);

		for (int32 VertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex)
		{
			const int32 MappedIndex = UniqueVertexMap[VertexIndex];  

			FMemory::Memcpy((SrcPositionBegin + VertexIndex).ptr(), &AccumulatedPositions[MappedIndex], PositionElemSize);
		}

		if (bRemoveData)
		{
			RemoveLaplacianData(Mesh);
		}
	}
}
