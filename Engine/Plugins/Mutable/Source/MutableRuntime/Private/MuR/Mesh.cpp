...
TManagedPtr<FMesh> FMesh::StaticUnserialise(FInputArchive& Arch)
{
...
TManagedRef<FMesh> FMesh::CreateAsReference(const TPassthroughObjectPtr<USkeletalMesh>& PasstroughObjectPtr)
{
	TManagedRef<FMesh> Result = MakeManaged<FMesh>();
	Result->PassthroughObject = PasstroughObjectPtr;
...
{
	return PassthroughObject.IsValid();
}
...
TManagedPtr<FMesh> FMesh::Clone() const
{
...
TManagedPtr<FMesh> FMesh::Clone(EMeshCopyFlags InFlags) const
{
    //MUTABLE_CPUPROFILER_SCOPE(MeshClone);
	LLM_SCOPE_BYNAME(TEXT("MutableRuntime"));
...
void FMesh::CopyFrom(const FMesh& From, EMeshCopyFlags InFlags)
{
	...
	Flags = From.Flags;
	PassthroughObject = From.PassthroughObject;
	...
	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithSurfaces))
	{
		...
		ClothSections.SetNum(From.Surfaces.Num());
	}
	...
	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithPhysicsBody))
	{
		...
		PhysicsAssets = From.PhysicsAssets;
	}

	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithGameplayTags))
	{
		GameplayTags = From.GameplayTags;
	}

	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithAssetUserData))
	{
		AssetUserData = From.AssetUserData;
	}
	
	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithAnimSlots))
	{
		AnimationSlots = From.AnimationSlots;
	}
	...
	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithSockets))
	{
	...
	{
		SkeletonObjects = From.SkeletonObjects;
	}
	...
	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithMorphMetaData))
	{
		Morph = From.Morph;
	}
	...
	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithClothData))
	{
		ClothSections = From.ClothSections;
	}

	if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithSkinWeightProfileData))
	{
		SkinWeightProfiles = From.SkinWeightProfiles;
	}
	else if (EnumHasAnyFlags(InFlags, EMeshCopyFlags::WithSkinWeightProfileMetaData))
	{
		const int32 NumSkinWeightProfiles = From.SkinWeightProfiles.Num();
		SkinWeightProfiles.SetNum(NumSkinWeightProfiles);

		for (int32 Index = 0; Index < NumSkinWeightProfiles; ++Index)
		{
			SkinWeightProfiles[Index].CopyMetadata(From.SkinWeightProfiles[Index]);
		}
	}
}
...
bool FMesh::HasClothing() const
{
	for (const FCloth& Cloth : ClothSections)
	{
		if (Cloth.IsValid())
		{
			return true;
...
TManagedPtr<const FSkeleton> FMesh::GetSkeleton() const
{
    return Skeleton;
}


void FMesh::SetSkeleton(const TManagedPtr<const FSkeleton>& InSkeleton)
{
    Skeleton = InSkeleton;
}


void FMesh::ConvertBoneIdsToIndices()
{
	if (ensure(Skeleton))
	{
		// Convert BoneMaps' BoneIds to BoneIndices
		const int32 NumBonesBoneMap = BoneMap.Num();
		for (int32 Index = 0; Index < NumBonesBoneMap; ++Index)
		{
			BoneMap[Index].Index = Skeleton->BoneIds.Find(BoneMap[Index].Id);
		}

		// Convert BonePoses' BoneIds to BoneIndices
		...
		{
			FBoneIdOrIndex& BoneId = BonePoses[Index].BoneId;

			// Bone Poses are sorted to match the skeleton hierarchy. 
			for (; AuxIndex < NumBones; ++AuxIndex)
			{
				if (Skeleton->BoneIds[AuxIndex] == BoneId.Id)
				{
					BoneId.Index = AuxIndex;
					break;
				}
			}

			check(Skeleton->BoneIds.IsValidIndex(BoneId.Index));
		}
	}
}


TManagedPtr<const FPhysicsBody> FMesh::GetPhysicsBody() const
{
    return PhysicsBody;
}


void FMesh::SetPhysicsBody(const TManagedPtr<const FPhysicsBody>& InPhysicsBody)
{
    PhysicsBody = InPhysicsBody;
}


int32 FMesh::AddAdditionalPhysicsBody(const TManagedPtr<const FPhysicsBody>& Body)
{
	return AdditionalPhysicsBodies.Add(Body);
}


TManagedPtr<const FPhysicsBody> FMesh::GetAdditionalPhysicsBody(int32 Index) const
{
...
void FMesh::AddLayout(TManagedPtr<const FLayout> InLayout)
{
	LLM_SCOPE_BYNAME(TEXT("MutableRuntime"));
...
TManagedPtr<const FLayout> FMesh::GetLayout(int32 LayoutIndex) const
{
...
void FMesh::SetLayout(int32 LayoutIndex, TManagedPtr<const FLayout> InLayout)
{
    check(Layouts.IsValidIndex(LayoutIndex));
...
int32 FMesh::FindBonePoseByBoneIndex(int32 Index) const
{
	return BonePoses.IndexOfByPredicate([Index](const FBonePose& Pose) { return Pose.BoneId.Index == Index; });
}
...
void UE::Mutable::Private::FMesh::SetBonePose(int32 Index, FBoneIdOrIndex BoneId, FTransform3f Transform, EBoneUsageFlags BoneUsageFlags, int8 Priority)
{
	check(BonePoses.IsValidIndex(Index));
	if (BonePoses.IsValidIndex(Index))
	{
		BonePoses[Index] = FBonePose{ BoneId, BoneUsageFlags, Priority, Transform };
	}
}


FBoneIdOrIndex FMesh::GetBonePoseId(int32 Index) const
{
	check(BonePoses.IsValidIndex(Index));
...
void FMesh::SetBoneMap(const TArray<FBoneIdOrIndex>& InBoneMap)
{
	BoneMap = InBoneMap;
}


const TArray<FBoneIdOrIndex>& FMesh::GetBoneMap() const
{
	return BoneMap;
}
...
int32 FMesh::GetDataSize() const
{
	...
	for (const FCloth& ClothSection : ClothSections)
	{
		AdditionalBuffersSize += ClothSection.GetDataSize();
	}
...
MUTABLE_IMPLEMENT_POD_SERIALISABLE(EMorphUsageFlags);
MUTABLE_IMPLEMENT_POD_VECTOR_SERIALISABLE(EMorphUsageFlags);
...
void FMeshMorph::Serialise(FOutputArchive& Arch) const
{
	Arch << Names;
	Arch << MaximumValuePerMorph;
	Arch << MinimumValuePerMorph;
	Arch << BatchStartOffsetPerMorph;
	Arch << BatchesPerMorph;

	Arch << SurfacesInUsePerMorph;
	Arch << UsageFlagsPerMorph;

	Arch << NumTotalBatches;
	Arch << PositionPrecision;
	Arch << TangentZPrecision;
}


void FMeshMorph::Unserialise(FInputArchive& Arch)
{
	Arch >> Names;
	Arch >> MaximumValuePerMorph;
	Arch >> MinimumValuePerMorph;
...
void FSkinWeightProfile::CopyMetadata(const FSkinWeightProfile& From)
{
	Name = From.Name;
	bDefaultProfile = From.bDefaultProfile;
	DefaultProfileFromLODIndex = From.DefaultProfileFromLODIndex;
	NumBoneInfluences = From.NumBoneInfluences;
	bUse16BitBoneIndex = From.bUse16BitBoneIndex;
	bUse16BitBoneWeight = From.bUse16BitBoneWeight;
}


void FSkinWeightProfile::Serialise(FOutputArchive& Arch) const
{
	Arch << Name;
	Arch << bDefaultProfile;
	Arch << DefaultProfileFromLODIndex;

	Arch << NumBoneInfluences;
	Arch << bUse16BitBoneIndex;
	Arch << bUse16BitBoneWeight;

	Arch << BoneIDs;
	Arch << BoneWeights;
	Arch << VertexIndexToInfluenceOffset;
}


void FSkinWeightProfile::Unserialise(FInputArchive& Arch)
{
	Arch >> Name;
	Arch >> bDefaultProfile;
	Arch >> DefaultProfileFromLODIndex;
...
MUTABLE_IMPLEMENT_POD_SERIALISABLE(FSkinWeightProfile::FVertexInfo);
MUTABLE_IMPLEMENT_POD_VECTOR_SERIALISABLE(FSkinWeightProfile::FVertexInfo);

void FMeshSocket::Serialise(FOutputArchive& Arch) const
{
	Arch << SocketName;
	Arch << BoneName;
	Arch << RelativeLocation;
	Arch << RelativeRotation;
	Arch << RelativeScale;
	Arch << bForceAlwaysAnimated;
}


void FMeshSocket::Unserialise(FInputArchive& Arch)
{
	Arch >> SocketName;
	Arch >> BoneName;
	Arch >> RelativeLocation;
...
void FMesh::Serialise(FOutputArchive& Arch) const
{
	...
	Arch << Morph;
	Arch << MorphDataBuffer;

	Arch << SkinWeightProfiles;
}
...
bool FMesh::IsSimilar(const FMesh& Other) const
{
	...
	bEqual = bEqual && (PassthroughObject == Other.PassthroughObject);

	bEqual = bEqual && (SkeletonObjects == Other.SkeletonObjects);
        ...
        for(const FMeshBuffer& Buffer : BufferSet.Buffers)
        {
            ...
            for (const FMeshBufferChannel& Channel : Buffer.Channels)
            {
				Out += FString::Printf(TEXT("      Channel with format: %d semantic: %d %d, components: %d, offset: %d\n"),
							EnumToUnderlyingType(Channel.Format), EnumToUnderlyingType(Channel.Semantic), Channel.SemanticIndex, Channel.ComponentCount, Channel.Offset);
...
void MeshCreateCollapsedVertexMap(const UE::Mutable::Private::FMesh* Mesh, TArray<int32>& CollapsedVertices)
{
	...
	for (int32 VertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex)
	{
		...
		VertHash.FindPointsInBall(Vertex, 0.00001f,
