...
namespace UE::Mutable::Private
{
	...
	void MeshClipMeshClassifyVerticesWithAABBTree(TBitArray<>& VertexInsideClipVolume, TNotNull<const FMesh*> Mesh, UE::Geometry::TMeshAABBTree3<GeometryUtils::FMeshAdapter>& ClipVolumeTree)
	{	
		MUTABLE_CPUPROFILER_SCOPE(ClassifyVerticesWithAABBTree);

		// Careful updating BitArray in parallel, operations touch a whole word worth of bits. 
		VertexInsideClipVolume.Init(false, Mesh->GetVertexCount());

		constexpr int32 NumVerticesPerBatch = 1 << 6;
		static_assert((NumVerticesPerBatch & 31) == 0, "NumVerticesPerBatch must be multiple of 32 so words in VertexInsideClipVolume are always accessed by the same batch.");

		int32 NumBatches = FMath::DivideAndRoundUp(Mesh->GetVertexCount(), NumVerticesPerBatch);

		MeshBufferIteratorConst<EMeshBufferFormat::Float32, float, 3> BasePositionIter(Mesh->GetVertexBuffers(), EMeshBufferSemantic::Position);

		ParallelExecutionUtils::InvokeBatchParallelFor(NumBatches, 
		[
			NumVerticesPerBatch,
			PositionsView = TArrayView<const FVector3f>(reinterpret_cast<const FVector3f*>(BasePositionIter.ptr()), Mesh->GetVertexCount()),
			&ClipVolumeTree,
			&VertexInsideClipVolume
		](int32 BatchId)
		{
			int32 BatchVertexBegin = BatchId * NumVerticesPerBatch;
			int32 BatchVertexEnd   = FMath::Min(BatchVertexBegin + NumVerticesPerBatch, PositionsView.Num());

			TArray<MeshIntersection::FHitIntersectionResult> RayHits;
			RayHits.Reserve(32);

			for (int32 VertexIndex = BatchVertexBegin; VertexIndex < BatchVertexEnd; ++VertexIndex)
			{
				FVector3d VertexPosition = FVector3d(PositionsView[VertexIndex]);

				if (!ClipVolumeTree.GetBoundingBox().Contains(VertexPosition))
				{
					continue;
				}

				FVector3d Origin = FVector3d(VertexPosition);
				FVector3d Direction = FVector3d::UnitZ();

				UE::Geometry::FWatertightRay3d Ray(Origin, Direction);
				
				RayHits.Reset();
				ClipVolumeTree.FindAllHitTriangles(Ray, RayHits);

				const int32 NumRayHits = RayHits.Num();

				if (NumRayHits <= 1)
				{
					// If at most one hit then we are done.
					VertexInsideClipVolume[VertexIndex] = NumRayHits > 0;
				}
				else
				{
					// We need to filter very close hits to prevent edge and shared vertices hits to be counted multiple times.
					uint32 ValidHits = 1;
					int32 CompareHitIndex = 0;

					// Hit entries are sorted by distance from the ray origin.
					for (int32 HitIndex = 1; HitIndex < NumRayHits; ++HitIndex)
					{
						if (FMath::Abs(RayHits[CompareHitIndex].Distance - RayHits[HitIndex].Distance) > UE_DOUBLE_KINDA_SMALL_NUMBER)
						{
							++ValidHits;
							CompareHitIndex = HitIndex;
						}
					};

					// Even number of hits means we are outside the clip mesh volume.  
					VertexInsideClipVolume[VertexIndex] = (ValidHits & 1) > 0;
				}
			}
		});
	}
    ...
    void MeshClipMeshClassifyVertices(TBitArray<>& VertexInClipMesh, const FMesh* Base, const FMesh* ClipMesh)
    {
		...
		{
			UE_LOGF(LogMutableCore, Warning, "Mesh operation with a mesh that is not closed as required.");
		}
#endif
...
//                          }
    ...
    void MeshClipWithMesh(FMesh* Result, const FMesh* pBase, const FMesh* ClipMesh, bool bRemoveAllVerticesCulled, bool& bOutSuccess)
    {
		...
		MeshRemoveVerticesWithCullSet(Result, VertexInClipMesh, bRemoveAllVerticesCulled);
    }
