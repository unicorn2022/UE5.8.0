...
// TODO: Deformation mask, to select the intensity of the deformation per-vertex.

// TODO: This is a reference implementation with ample roof for optimization.

namespace UE::Mutable::Private
{
	using FShapeMeshTree = UE::Geometry::TMeshAABBTree3<GeometryUtils::FMeshAdapter>;
			...
			const float ValidityTolerance = UE_KINDA_SMALL_NUMBER)
	{
		...
		const GeometryUtils::FMeshGeometry& ShapeMesh = ShapeMeshTree.GetMesh()->Mesh;
    ...
    //---------------------------------------------------------------------------------------------
    //! Find mesh clusters.
	//! Color {1,1,1,1} is reserved for the non rigid cluster
    //---------------------------------------------------------------------------------------------
	...
	inline void FindRigidClusters(const FMesh* Mesh, const FMeshBindColorChannelUsageMasks& ColorUsageMasks, TArray<TArray<int32>>& OutClusters, int32& OutNonRigidClusterIdx)
	{
		...
		if (ItColorBase.ptr())
		{
			...
			// meshes without color will get white as default.
	...
	inline TTuple<TArray<FReshapePointBindingData>, TArray<int32>, TArray<int32>> BindPhysicsBodies( 
			TArray<const FPhysicsBody*> PhysicsBodies, FShapeMeshTree& ShapeMeshTree, const FMesh& Mesh, const FSkeleton& Skeleton, 
			TConstArrayView<FName> PhysicsToDeform, bool bInvertSelection)
	{
		...
		for (int32 PhysicsBodyIndex = 0; PhysicsBodyIndex < NumPhysicsBodies; ++PhysicsBodyIndex)
		{
			if (const FPhysicsBody* Body = PhysicsBodies[PhysicsBodyIndex])
			{
				...
				{
					const bool bContains = PhysicsToDeform.Contains(Body->GetBodyBoneName(BodyIndex));
					if (bInvertSelection != bContains)
					{
		...
		for (int32 PhysicsBodyIndex = 0; PhysicsBodyIndex < NumPhysicsBodies; ++PhysicsBodyIndex)
		{
			...
			{
				const int32 BoneIndex = Skeleton.BoneNames.Find(Body->GetBodyBoneName(B));
				int32 PoseIndex = Mesh.FindBonePoseByBoneIndex(BoneIndex);

				FTransform3f T = FTransform3f::Identity;
				if (PoseIndex > 0)
				{
					Mesh.GetBonePoseTransform(PoseIndex, T);
				}
	...
	inline TTuple<TArray<FReshapePointBindingData>, TArray<int32>> BindPose(
			const FMesh& Mesh, const FSkeleton& Skeleton, FShapeMeshTree& ShapeMeshTree, TConstArrayView<FName> BonesToDeform, bool bInvertSelection)
	{
		UE::Geometry::FAxisAlignedBox3d ShapeAABBox = ShapeMeshTree.GetBoundingBox();
		...
		for (int32 BoneIndex = 0; BoneIndex < BoneCount; ++BoneIndex)
		{
			...
			const bool bContains = bInvertSelection != BonesToDeform.Contains(Skeleton.BoneNames[Mesh.BonePoses[BoneIndex].BoneId.Index]);
			if (!bContains)
			{
				continue;
			}
    ...
    inline void MeshBindShapeReshape(
			FMesh* Result, UE::Geometry::TMeshAABBTree3<GeometryUtils::FMeshAdapter>& ShapeMeshTree,
			TConstArrayView<FName> BonesToDeform, TConstArrayView<FName> PhysicsToDeform,
			...
			bool& bOutSuccess)
    {
		...
		const bool bReshapeSkeletonInvertSelection = EnumHasAnyFlags(BindFlags, EMeshBindShapeFlags::ReshapeSkeletonInvertSelection);
		const bool bReshapePhysicsVolumesInvertSelection = EnumHasAnyFlags(BindFlags, EMeshBindShapeFlags::ReshapePhysicsVolumesInvertSelection);

		const FMeshBindColorChannelUsageMasks ColorUsagesMasks = MakeColorChannelUsageMasks(ColorChannelUsages);

		// Early out if nothing will be modified and the vertices discarded. return null in this
		// case indicating nothing has modified so the Base Mesh can be reused.
		const bool bSkeletonModification = Result->GetSkeleton() && bReshapeSkeleton;
		const bool bPhysicsModification = (Result->GetPhysicsBody() || Result->AdditionalPhysicsBodies.Num()) && bReshapePhysics;
		...
		if (bReshapeVertices)
		{
			TArray<FReshapeVertexBindingData> VerticesBindData = BindVerticesReshape(Result, ShapeMeshTree, ColorUsagesMasks);
		...
		if (bReshapeSkeleton && Result->GetSkeleton())
		{
			MUTABLE_CPUPROFILER_SCOPE(BindSkeleton);

			TTuple<TArray<FReshapePointBindingData>, TArray<int32>> SkeletonBindingData = 
					BindPose(*Result, *Result->GetSkeleton(), ShapeMeshTree, BonesToDeform, bReshapeSkeletonInvertSelection);
		...
		if (bReshapePhysics && Result->GetSkeleton() && (ResultPhysicsBody || Result->AdditionalPhysicsBodies.Num()))
		{
			MUTABLE_CPUPROFILER_SCOPE(BindPhysicsBody);

			// Gather bodies respecting order, first main physics body then additional bodies.
			// Null entries are need to be able to maintain that order.  
			TArray<const FPhysicsBody*> PhysicsBodiesToBind;
			{
				...
				for (const TManagedPtr<const FPhysicsBody>& Body : Result->AdditionalPhysicsBodies)
				{
			...
			TTuple<TArray<FReshapePointBindingData>, TArray<int32>, TArray<int32>> PhysicsBindingData = 
					BindPhysicsBodies(PhysicsBodiesToBind, ShapeMeshTree, *Result, *Result->GetSkeleton(), PhysicsToDeform, bReshapePhysicsVolumesInvertSelection);
			...
			const FVector3f& Point, FClipDeformVertexBindingData& OutBindData)
	{
		...
		UE::Geometry::FTriangle3d NearestShapeTriangle { 
				(FVector3d)ShapeMesh.Positions[Triangle.A], 
				(FVector3d)ShapeMesh.Positions[Triangle.B], 
				(FVector3d)ShapeMesh.Positions[Triangle.C] };
			...
			const FVector3f& Point, const FVector3f& Normal, FClipDeformVertexBindingData& OutBindData)
	{
		...
		UE::Geometry::FTriangle3d HitShapeTriangle {
				(FVector3d)ShapeMesh.Positions[ShapeTriangleIndices.A],
				(FVector3d)ShapeMesh.Positions[ShapeTriangleIndices.B],
				(FVector3d)ShapeMesh.Positions[ShapeTriangleIndices.C] };
    ...
    inline void MeshBindShapeClipDeform(FMesh* Result, UE::Geometry::TMeshAABBTree3<GeometryUtils::FMeshAdapter>& ShapeTree, EShapeBindingMethod BindingMethod, bool& bOutSuccess)
    {
		...
		TArray<FClipDeformVertexBindingData> VerticesBindData = BindVerticesClipDeform(Result, ShapeTree, BindingMethod);
