...
namespace UE::Mutable::Private
{
	...
	//---------------------------------------------------------------------------------------------
	inline TManagedPtr<FLayout> LayoutFromMesh_RemoveBlocks(const FMesh* InMesh, int32 InLayoutIndex)
	{
		...
		TManagedPtr<const FLayout> Source = InMesh->GetLayout(InLayoutIndex);
		TManagedPtr<FLayout> pResult;
	...
	inline TManagedPtr<FLayout> LayoutRemoveBlocks(const FLayout* Source, const FLayout* ReferenceLayout)
	{
		// Remove blocks that are not in the mesh
		TManagedPtr<FLayout> pResult = Source->Clone();
		int32 DestBlockIndex = 0;
	...
	inline TManagedPtr<FLayout> LayoutMerge(const FLayout* pA, const FLayout* pB )
	{
		TManagedPtr<FLayout> pResult = pA->Clone();
