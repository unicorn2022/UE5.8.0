...
namespace UE::Mutable::Private
{
	...
	TManagedPtr<FPhysicsBody> FPhysicsBody::StaticUnserialise(FInputArchive& Arch)
	{
		TManagedPtr<FPhysicsBody> Result = MakeManaged<FPhysicsBody>();
		Arch >> *Result;
		return Result;
	}
	...
	TManagedPtr<FPhysicsBody> FPhysicsBody::Clone() const
	{
		TManagedPtr<FPhysicsBody> Result = MakeManaged<FPhysicsBody>();
		...
		Result->BodiesBoneNames = BodiesBoneNames;
		Result->BoneIds = BoneIds;

		Result->bBodiesModified = bBodiesModified;
	...
	void FPhysicsBody::SetBody(int32 BodyIndex, FName InBoneName)
	{
		BodiesBoneNames[BodyIndex] = InBoneName;
	}

	FName FPhysicsBody::GetBodyBoneName(int32 BodyIndex) const
	{
		check(BodiesBoneNames.IsValidIndex(BodyIndex));
		return BodiesBoneNames[BodyIndex];
	}
	...
	void FPhysicsBody::ConvertBoneIdsToNames(const TMap<uint32, FName>& ConstantNames)
	{
		if (BodiesBoneNames.IsEmpty())
		{
			BodiesBoneNames.Reserve(BoneIds.Num());

			for (uint32 BoneId : BoneIds)
			{
				BodiesBoneNames.Add(ConstantNames.FindChecked(BoneId));
			}
