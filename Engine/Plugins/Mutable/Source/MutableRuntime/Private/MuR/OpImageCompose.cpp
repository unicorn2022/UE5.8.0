...
namespace UE::Mutable::Private
{
	...
	void FImageOperator::ImageCompose(FImage* Base, const FImage* Block, const box< FIntVector2 >& Rect)
	{
		...
		if (bSubPixelBlockCompose)
		{
			...
			TManagedPtr<FImage> TempBase = ImagePixelFormat(Quality, Base, UncompressedFormat);
			TManagedPtr<FImage> TempBlock = ImagePixelFormat(Quality, Block, UncompressedFormat);
			ImageCompose(TempBase.Get(), TempBlock.Get(), Rect);
			TManagedPtr<FImage> NewBase = ImagePixelFormat(Quality, TempBase.Get(), OriginalFormat);
		...
		else
		{
			...
			{
				//UE_LOGF(LogMutableCore, Warning, "Block Mip Pos : %d, %d", blockMipPos[0], blockMipPos[1]);
