...
namespace UE::Mutable::Private
{
	...
	inline void ImageGradient( FImage* pDest, const FVector4f& c0, const FVector4f& c1 )
	{
		...
		for ( int i=0; i< sizeX; ++i )
		{
            ...
            uint8 color[4];
            color[0] = (uint8)FMath::Max( 0, FMath::Min( 255, int(c[0]*255.0f) ) );
            color[1] = (uint8)FMath::Max( 0, FMath::Min( 255, int(c[1]*255.0f) ) );
            color[2] = (uint8)FMath::Max( 0, FMath::Min( 255, int(c[2]*255.0f) ) );
			color[3] = 255;

			for ( int j=0; j< sizeY; ++j )
			{
				pDestBuf[ (sizeX*j+i)*3 + 0 ] = color[0];
				pDestBuf[ (sizeX*j+i)*3 + 1 ] = color[1];
				pDestBuf[ (sizeX*j+i)*3 + 2 ] = color[2];
			}
		}
	}
