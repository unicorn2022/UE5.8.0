...
namespace UE::Mutable::Private
{
	...
	inline void MeshClipMorphPlane(FMesh* Result, const FMesh* pBase, const FVector3f& Origin, const FVector3f& Normal, float Dist, float Factor, float Radius,
		...
		FName BoneName = NAME_None, float VertexSelectionBoneMaxRadius = -1.f)
	{
		bOutSuccess = true;
		//float Radius = 8.f;
		//float Radius2 = 4.f;
        ...
        TManagedPtr<const FSkeleton> BaseSkeleton = pBase->GetSkeleton();

		TBitArray<> AffectedBoneMapIndices;
		const int32 BaseBoneIndex = BaseSkeleton && BoneName != NAME_None ? BaseSkeleton->BoneNames.Find(BoneName) : INDEX_NONE;
        if (BaseBoneIndex != INDEX_NONE)
		{
			const int32 NumBonesBoneMap = pBase->BoneMap.Num();
			const TArray<FBoneIdOrIndex>& BoneMap = pBase->BoneMap;
