...
void MeshTransformWithBoneInline(FMesh* Mesh, const FMatrix44f& Transform, FName InBoneName, const float InThresholdFactor)
{
	MUTABLE_CPUPROFILER_SCOPE(MeshTransformWithBone);

	if (!Mesh || Mesh->GetVertexCount() == 0 || Mesh->IsReference() || !Mesh->GetSkeleton())
	{
		return;
	}


	TManagedPtr<const FSkeleton>& BaseSkeleton = Mesh->Skeleton;

	const int32 BaseBoneIndex = BaseSkeleton->BoneNames.Find(InBoneName);
	...
	// Find affected bones
	{
		...
		const TArray<FBoneIdOrIndex>& BoneMap = Mesh->BoneMap;
		AffectedBoneMapIndices.SetNum(NumBonesBoneMap, false);

		const int32 NumBones = BaseSkeleton->GetNumBones();
		TBitArray<> AffectedSkeletonBones;
		AffectedSkeletonBones.SetNum(NumBones, false);

		for (int32 BoneIndex = 0; BoneIndex < NumBones; ++BoneIndex)
		{
			const int32 ParentBoneIndex = BaseSkeleton->BoneParents[BoneIndex];
		...
		{
			AffectedBoneMapIndices[Index] = AffectedSkeletonBones[BoneMap[Index].Index];
		}
	}
