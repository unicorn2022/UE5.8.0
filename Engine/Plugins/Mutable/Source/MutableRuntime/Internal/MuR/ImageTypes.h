...
namespace UE::Mutable::Private
{
	...
	inline uint32 GetTypeHash(const EImageFormat& Format)
	{
		return static_cast<uint32>(Format);
	}
	...
	struct FImageDesc
	{
		...
		FImageDesc(const FImageSize& InSize, EImageFormat InFormat, EImageFormat InFormatIfAlpha, uint8 InLods)
			: m_size(InSize), m_format(InFormat), FormatIfAlpha(InFormatIfAlpha), m_lods(InLods)
		{
		}
		...
		inline bool operator==(const FImageDesc& Other) const
		{
			return
				...
				FormatIfAlpha == Other.FormatIfAlpha &&
				m_lods == Other.m_lods;
		}
	};
	...
	inline uint32 GetTypeHash(const EAddressMode& Mode)
	{
		return static_cast<uint32>(Mode);
	}
