...
namespace UE::Mutable::Private
{
	...
	public:
		...
		UE_API TManagedPtr<FPhysicsBody> Clone() const;

		//! Serialisation
        static UE_API void Serialise( const FPhysicsBody*, FOutputArchive& );
		static UE_API TManagedPtr<FPhysicsBody> StaticUnserialise( FInputArchive& );
		...
		UE_API void SetBody(int32 BodyIndex, FName BoneName);

		UE_API FName GetBodyBoneName(int32 Index) const;
		...
		UE_API void ConvertBoneIdsToNames(const TMap<uint32, FName>& ConstantNames);

	public:
		int32 CustomId = -1;

		// Bone name the physics volume aggregate is bound to. 
		TArray<FName> BodiesBoneNames; // Runtime
		TArray<uint32> BoneIds; // Serialized
