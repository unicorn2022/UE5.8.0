 ...
 * @param Node The CO Node we want to get its root CO
 * @param VisitedObjects List of objects used to record the search path already taken. It is used to determine if a cycle has been found.
 * return The root CO. It can be null if a cycle has been found.
 */
...
namespace GraphTraversal
{
	...
	*/
	...
	CUSTOMIZABLEOBJECTEDITOR_API UCustomizableObject* GetRootObject(UCustomizableObject* ChildObject, TArray<const UObject*>* OutVisitedObjects = nullptr);
	CUSTOMIZABLEOBJECTEDITOR_API const UCustomizableObject* GetRootObject(const UCustomizableObject* ChildObject, TArray<const UObject*>* OutVisitedObjects = nullptr);
...
namespace GraphTraversal
{
	...
	/** Returns all the Customizable Objects in a graph starting at the root object.
	 * @return true if the full hierarchy could not be discovered due to UCustomizableObjects still async loading. Async loadings are called automatically. */
	bool GetAllObjectsInGraphIncremental(UCustomizableObject* Object, TSet<UCustomizableObject*>& OutObjects);
