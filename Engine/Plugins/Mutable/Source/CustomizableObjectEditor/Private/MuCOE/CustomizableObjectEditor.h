...
class UCONodeClipSkeletalMeshWithSkeletalMesh;
...
class FEditorLogger : public IMutableEditorLogger
{
public:

	/** Logs message into the Message Log Listing */
	bool LogMessage(TSharedRef<FTokenizedMessage> Message) override;

	/** Claer all the messages of the Message Log Listing */
	void ClearLogMessageList() override;
...
public:
	...
	virtual FEditorModeTools* GetEditorModeTools() override;
...
private:
	...
	TSharedPtr<SWidget> StatsWidget;

	/** Pointer to the CLip Skeletal Mesh with Skeletal Mesh node that was being shown in the CO Editor*/
	TStrongObjectPtr<UCONodeClipSkeletalMeshWithSkeletalMesh> PreviouslySelectedNodeWithPreview = nullptr;
	
	/** Handle for the callback performed when the PreviewMesh of the PreviouslySelectedNodeWithPreview is changed (or any of its related properties) */
	FDelegateHandle PreviewMeshChangedDelegateHandle;
