...
#include "MuCOE/CustomizableObjectSkeletalMeshThumbnailRenderer.h"
...
#include "Nodes/CONodeExternalOperationDetails.h"
#include "Nodes/CONodeModifierTransformWithBone.h"
#include "Nodes/CustomizableObjectNodeModifierClipWithMesh.h"
#include "Nodes/CustomizableObjectNodeModifierTransformInMesh.h"
#include "Nodes/CustomizableObjectNodeTextureParameter.h"
#include "Nodes/ExternalTypeCustomization.h"
...
#include "Nodes/CustomizableObjectNodeComponentMesh.h"
...
#include "Nodes/CustomizableObjectNodeTransformParameter.h"
...
void FCustomizableObjectEditorModule::StartupModule()
{
	...
	RegisterCustomDetails(PropertyModule, UCONodeModifierEditSkeletalMeshSection::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeModifierEditMeshSectionDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCONodeModifierExtendSkeletalMeshSection::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeModifierExtendMeshSectionDetails::MakeInstance));
	...
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeModifierClipWithMesh::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeModifierBaseDetails::MakeInstance));
	...
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeModifierTransformInMesh::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeModifierBaseDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCONodeModifierTransformWithBone::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeModifierBaseDetails::MakeInstance));
	...
	RegisterCustomDetails(PropertyModule, UCONodeSkeletalMeshSection::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeMeshSectionDetails::MakeInstance));
	...
	RegisterCustomDetails(PropertyModule, UCONodeSkeletalMeshMake_V2::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeDetails::MakeInstance));
	...
	RegisterCustomDetails(PropertyModule, UCONodeExternalOperation::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCONodeExternalOperationDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCONodeSkeletalMeshBreak_V2::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCONodeSkeletalMeshModify::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCONodeComponentSkeletalMesh::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCONodeExternalTypeParameter::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeParameterDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeColorParameter::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeParameterDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeEnumParameter::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeParameterDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeFloatParameter::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeParameterDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeMaterialParameter::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeParameterDetails::MakeInstance));
	...
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeTextureParameter::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeParameterDetails::MakeInstance));
	RegisterCustomDetails(PropertyModule, UCustomizableObjectNodeTransformParameter::StaticClass(), FOnGetDetailCustomizationInstance::CreateStatic(&FCustomizableObjectNodeParameterDetails::MakeInstance));
	...
	PropertyModule.RegisterCustomPropertyTypeLayout("InstancedStruct", FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FExternalTypeCustomization::MakeInstance), MakeShared<FExternalTypeTypeIdentifier>());

	PropertyModule.NotifyCustomizationModuleChanged();

	UThumbnailManager::Get().RegisterCustomRenderer(UCustomizableObjectSkeletalMesh::StaticClass(), UCustomizableObjectSkeletalMeshThumbnailRenderer::StaticClass());

	// Register factory
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this,&FCustomizableObjectEditorModule::RegisterFactory);
...
struct FCompilationOutOfDateContext
{
	...
	bool bExternalOperationVersionDiff = false;
...
{
	MUTABLE_EDITOR_CPUPROFILER_SCOPE(IsCompilationOutOfDate_Async)
...
{
	MUTABLE_EDITOR_CPUPROFILER_SCOPE(FCustomizableObjectEditorModule::IsCompilationOutOfDate)
	...
	for (TPair<TObjectPtr<const UScriptStruct>, unsigned> Pair : Object.GetPrivate()->GetModelResources()->ExternalOperationVersions)
	{
		FInstancedStruct OperationInstancedStruct;
		OperationInstancedStruct.InitializeAs(Pair.Key);

		const UE::Mutable::FExternalOperation* Operation = OperationInstancedStruct.GetPtr<UE::Mutable::FExternalOperation>();
		if (!Operation)
		{
			Context->bExternalOperationVersionDiff = true;
			break;
		}
		
		if (Operation->GetVersion() != Pair.Value)
		{
			Context->bExternalOperationVersionDiff = true;
			break;
		}
...
void FCustomizableObjectEditorModule::GetAllObjectsInGraph(UCustomizableObject* Object, TSet<UCustomizableObject*>& OutRelated) const
{
	GraphTraversal::GetAllObjectsInGraph(Object, OutRelated);
}
...
void FCustomizableObjectEditorModule::OnUpstreamCOsLoadedFixup(UCustomizableObject* Object, int32 CustomizableObjectCustomVersion) const
{
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::MovedLODSettingsToMeshComponentNode)
	{
		...
		// Override the LOD Settings found in the UCustomizableObjectNodeComponentMesh of this CO using the LODSettings of the parent CO
		{
			...
			{
				ComponentNode->LODSettings_DEPRECATED = RootObjectLODSettings;
			}
		}
	}
}
...
void FCustomizableObjectEditorModule::EvictSkeletalMeshThumbnailScene(USkeletalMesh* SkeletalMesh) const
{
	if (!SkeletalMesh)
	{
		return;
	...
	FThumbnailRenderingInfo* RenderingInfo = UThumbnailManager::Get().GetRenderingInfo(SkeletalMesh);
	...
	if (UCustomizableObjectSkeletalMeshThumbnailRenderer* ThumbnailRenderer = 
			Cast<UCustomizableObjectSkeletalMeshThumbnailRenderer>(RenderingInfo->Renderer))
	{
		ThumbnailRenderer->EvictThumbnailScene(SkeletalMesh);
	}
}
...
{
	MUTABLE_EDITOR_CPUPROFILER_SCOPE(FCustomizableObjectEditorModule::GetParticipatingObjects);

	FCompilationOptions Options = InOptions ? *InOptions : GetCompilationOptions(*Object);
	...
	auto Visit = [&](UCustomizableObjectNode& Node)
	{
		...
		else if (UCONodeComponentSkeletalMesh* CompNode = Cast<UCONodeComponentSkeletalMesh>(&Node))
		{
		...
		else if (UCONodeSkeletalMeshSection* MeshSectionNode = Cast<UCONodeSkeletalMeshSection>(&Node))
		{
			AddObjectSoft(MeshSectionNode->GetMaterial());
			int32 NumImages = MeshSectionNode->GetNumParameters(EMaterialParameterType::Texture);
			for (int32 ImageIndex = 0; ImageIndex < NumImages; ++ImageIndex)
			{
				AddObject(MeshSectionNode->GetImageReferenceTexture(ImageIndex));
			}
		}
			
		else if (UCONodeMaterialModify* MaterialModifyNode = Cast<UCONodeMaterialModify>(&Node))
		{
			TArray<FEdGraphPinReference> MaterialTextureParameterPins;
			MaterialModifyNode->GetMaterialTextureParameterPins(MaterialTextureParameterPins);
			
			for (FEdGraphPinReference MaterialTextureParameterPin : MaterialTextureParameterPins)
			{
				AddObjectSoft(MaterialModifyNode->GetTexturePinParameterReferenceTexture(*MaterialTextureParameterPin.Get()).ToSoftObjectPath());
			}
		}
		...
		else if (UCustomizableObjectNodeTable* TableNode = Cast<UCustomizableObjectNodeTable>(&Node))
		{
			...
			if (DataTable)
			{
				...
				if (UCompositeDataTable* CompositeDataTable = Cast<UCompositeDataTable>(DataTable))
				{
					const TArray<UDataTable*>& ParentTables = CompositeDataTable->GetParentTables();
					for (UDataTable* ParentTable : ParentTables)
					{
						if (ParentTable)
						{
							AddObject(ParentTable);
						}
					}
				}
			}
		...
		else if (UCustomizableObjectNodeMacroInstance* MacroInstanceNode = Cast<UCustomizableObjectNodeMacroInstance>(&Node))
		{
			AddObject(MacroInstanceNode->ParentMacroLibrary);
		}
	};
...
void FCustomizableObjectEditorModule::AddPendingLoad(UCustomizableObject* Object)
{
	check(IsInGameThread());
	ObjectsPendingLoad.Add(TStrongObjectPtr(Object));
}
...
const TMap<FName, FLinearColor>& FCustomizableObjectEditorModule::GetExternalPinTypeColors() const
{
	return ExternalTypeTypePinColors;
}


void FCustomizableObjectEditorModule::RegisterExternalOperation(const UScriptStruct* Operation)
{
	if (!Operation)
	{
		return;
	...
	auto Register = [&](const UScriptStruct* ExternalPinType)
	{
		if (ExternalPinType->GetCppStructOps()->GetCapabilities().IsAbstract)
		{
			return;
		}

		FName PinType = UEdGraphSchema_CustomizableObject::GetPinCategoryName(ExternalPinType);
		FLinearColor Color;
			
		const FString Hex = ExternalPinType->GetMetaData("PinColor"); // e.g. "#FF00AAFF"
		if (!Hex.IsEmpty())
		{
		...
		{
			const uint32 Hash = GetTypeHash(PinType.ToString());
			Color = FLinearColor::MakeRandomSeededColor(Hash);
		}

		TStrongObjectPtr<const UScriptStruct> StrongExternalPinType = TStrongObjectPtr(ExternalPinType);
	
		TStrongObjectPtr<const UScriptStruct>* Result = ExternalTypeCategories.Find(PinType);
		if (Result && *Result != StrongExternalPinType)
		{
			UE_LOGF(LogMutable, Error, "External Pin Type already registered: %ls", *PinType.ToString());
			return;
		}
			
		ExternalTypeTypePinColors.Add(PinType, Color);
		ExternalTypeFriendlyName.Add(PinType, ExternalPinType->GetDisplayNameText());
		ExternalTypeCategories.Add(PinType, StrongExternalPinType);
			
		++RegisteredExternalType.FindOrAdd(PinType, 0);
			
		check(RegisteredExternalType.Num() == ExternalTypeTypePinColors.Num());
		check(RegisteredExternalType.Num() == ExternalTypeFriendlyName.Num());
		check(RegisteredExternalType.Num() == ExternalTypeCategories.Num());
	};
	
	FInstancedStruct InstancedStruct;
	InstancedStruct.InitializeAs(Operation);
	
	const UE::Mutable::FExternalOperation& ExternalOperation = InstancedStruct.Get<UE::Mutable::FExternalOperation>();

	Register(ExternalOperation.GetOutput().Value);
	
	for (const TPair<FText, const UScriptStruct*>& Input : ExternalOperation.GetInputs())
	{
...
void FCustomizableObjectEditorModule::UnregisterExternalOperation(const UScriptStruct* Operation)
{
	...
	auto Unregister = [&](const UScriptStruct* ExternalPinType)
	{
		if (ExternalPinType->GetCppStructOps()->GetCapabilities().IsAbstract)
		{
			return;
		...
		uint32* Count = RegisteredExternalType.Find(PinType);
		if (!Count)
		{
			return;
		}
		...
		if (*Count == 0)
		{
			ExternalTypeTypePinColors.Remove(PinType);
			ExternalTypeFriendlyName.Remove(PinType);
			ExternalTypeCategories.Remove(PinType);
			
			RegisteredExternalType.Remove(PinType);
	...
	Unregister(ExternalOperation.GetOutput().Value);
...
const TMap<FName, TStrongObjectPtr<const UScriptStruct>>& FCustomizableObjectEditorModule::GetExternalPinTypes() const
{
	return ExternalTypeCategories;
}


const TMap<FName, FText>& FCustomizableObjectEditorModule::GetExternalPinFriendlyName() const
{
	return ExternalTypeFriendlyName;
}
...
void FCustomizableObjectEditorModule::Tick(bool bBlocking)
{
	if (TSet<TStrongObjectPtr<UCustomizableObject>>::TIterator It = ObjectsPendingLoad.CreateIterator())
	{
		UCustomizableObject* Object = It->Get();
		if (IsValid(Object)) // Someone may have called MarkAsGarbage.
		{
			TSet<UCustomizableObject*> RelatedObjects;
			const bool bPendingLoad = GraphTraversal::GetAllObjectsInGraphIncremental(Object, RelatedObjects);

			if (!bPendingLoad)
			{
				bool bHierarchyLoaded = true;
				for (const UCustomizableObject* RelatedObject : RelatedObjects)
				{
					if (RelatedObject && !RelatedObject->HasAnyFlags(EObjectFlags::RF_LoadCompleted))
					{
				...
				if (bHierarchyLoaded)
				{	
					// Traversing the graph upwards does not take into account Group Object Nodes. 
					// A Child Object may reference a parent but not a Group Object. This can cause the loaded CO to not appear in the Related objects. Added manually to fix this.
					RelatedObjects.Add(Object);
				
					for (UCustomizableObject* RelatedObject : RelatedObjects)
					{
						if (ObjectsPendingLoad.Remove(TStrongObjectPtr(RelatedObject)))
						{
							OnUpstreamCOsLoaded(RelatedObject);
							RelatedObject->GetPrivate()->LoadCompiledDataFromDisk();
						}
					}
				}
			}	
...
int32 FCustomizableObjectEditorModule::GetRemainingWork() const
{
	return Compiler->GetRemainingWork() + static_cast<int32>(!ObjectsPendingLoad.IsEmpty());
}
...
{
	return Compiler->GetNumCompileRequests();
}
...
bool FCustomizableObjectEditorModule::IsTickable() const
{
	return UCustomizableObjectSystem::GetInstance()->GetPrivate()->GetRemainingWork() > 0;
}


void FCustomizableObjectEditorModule::Tick(float InDeltaTime)
{
	check(false);
}


TStatId FCustomizableObjectEditorModule::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(FCustomizableObjectEditorModule, STATGROUP_Tickables);
}


void FCustomizableObjectEditorModule::TickCook(float DeltaTime, bool bCookCompete)
{
	MUTABLE_EDITOR_CPUPROFILER_SCOPE(FCustomizableObjectEditorModule::TickCook);
	UCustomizableObjectSystem::GetInstance()->GetPrivate()->MainTick(false);
}
