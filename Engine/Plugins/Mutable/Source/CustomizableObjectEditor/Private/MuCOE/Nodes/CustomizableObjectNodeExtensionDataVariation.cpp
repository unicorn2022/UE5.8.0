...
void UCustomizableObjectNodeExtensionDataVariation::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* InRemapPins)
{
	...
	CustomCreatePin(EGPD_Output, GetCategory(), GetOutputPinName(), FText::FromName(GetDefaultPinName()));

	for (int32 VariationIndex = Variations.Num() - 1; VariationIndex >= 0; --VariationIndex)
	{
		FText FriendlyName = FText::Format(LOCTEXT("VariationPinFriendlyName", "Variation {0} [{1}]"), { VariationIndex, FText::FromString(Variations[VariationIndex].Tag) });
		CustomCreatePin(EGPD_Input, GetCategory(), GetVariationPinName(VariationIndex), FriendlyName);
	}

	CustomCreatePin(EGPD_Input, GetCategory(), GetDefaultPinName(), FText::FromName(GetDefaultPinName()));
}
