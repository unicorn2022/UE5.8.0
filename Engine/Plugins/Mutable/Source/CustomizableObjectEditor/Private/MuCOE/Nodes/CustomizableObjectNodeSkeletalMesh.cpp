...
void UCustomizableObjectNodeSkeletalMesh::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	if (!LoadedMesh)
	{
		...
		DefaultPin = CustomCreatePin(EGPD_Output, Schema->PC_Mesh, TEXT("Mesh"), LOCTEXT("Mesh", "Mesh"), PinData);
		return;
	}
	else
	{
		...
		for (int32 LODIndex = 0; LODIndex < NumLODs; ++LODIndex)
		{
			...
			for (int32 SectionIndex = 0; SectionIndex < NumSections; ++SectionIndex)
			{
				...
				// Mesh
				{
					...
					FText FriendlyName = FText::FromString(FString::Printf(TEXT("LOD %i - %s"), LODIndex, *Section));
					CustomCreatePin(EGPD_Output, Schema->PC_Mesh, FName(*MeshPinName), FriendlyName, PinData);
				}
				...
				if (MaterialInterface)
				{
					...
					for (int32 Index = 0; Index < ImageInfos.Num(); ++Index)
					{
						...
						FText FriendlyName = FText::FromString(FString::Printf(TEXT("LOD %i - %s - %s"), LODIndex, *Section, *ImageNameStr));
						UEdGraphPin* Pin = CustomCreatePin(EGPD_Output, Schema->PC_Texture, FName(*TexturePinName), FriendlyName, PinData);
						Pin->bHidden = true;
					}
				}
			}
...
void UCustomizableObjectNodeSkeletalMesh::BackwardsCompatibleFixup(int32 CustomizableObjectCustomVersion)
{
	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::PassthroughParameter)
	{
		...
		{
			if (Pin->PinName == TEXT("Skeletal Mesh"))
			{
