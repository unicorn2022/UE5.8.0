...
void UCustomizableObjectNodeModifierClipWithMesh::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	CustomCreatePin(EGPD_Input, Schema->PC_Mesh, TEXT("Clip Mesh"), LOCTEXT("ClipMesh", "Clip Mesh"));
		...
		for ( const FGuid& MeshSectionNodeID : ArrayMaterialNodeToClipWithID_DEPRECATED )
		{
			...
			if (UCustomizableObjectNode* ParentNode = GetCustomizableObjectExternalNode<UCustomizableObjectNode>(CustomizableObjectToClipWith_DEPRECATED.Get(), MeshSectionNodeID))
			{
				...
				else
				{
					...
					UE_LOGF(LogMutable, Warning, "[%ls] UCustomizableObjectNodeModifierClipWithMesh version upgrade failed.", *GetOutermost()->GetName());
				}
			...
			{
				UE_LOGF(LogMutable, Log, "[%ls] UCustomizableObjectNodeModifierClipWithMesh has no parent. It will not be upgraded.", *GetOutermost()->GetName());
			}
		}
...
{
	return LOCTEXT("Clip_Mesh_With_Mesh_Modifier", "Clip Mesh With Mesh");
}
