...
void UCustomizableObjectNodeModifierRemoveMeshBlocks::BackwardsCompatibleFixup(int32 CustomizableObjectCustomVersion)
{
		...
		&& BlockIds_DEPRECATED.Num() < Blocks_DEPRECATED.Num())
	{
		...
		if (ParentMaterialNode)
		{
			...
			{
				UE_LOGF(LogMutable, Warning, "[%ls] UCustomizableObjectNodeModifierRemoveMeshBlocks refers to an invalid texture layout index %d. Parent node has %d layouts.",
					*GetOutermost()->GetName(), ParentLayoutIndex, Layouts.Num());
			}
			else if (UCONodeSkeletalMeshSection* ParentMaterial = Cast<UCONodeSkeletalMeshSection>(ParentMaterialNode))
			{
				...
				for (int IndexIndex = BlockIds_DEPRECATED.Num(); IndexIndex < Blocks_DEPRECATED.Num(); ++IndexIndex)
				{
					...
					{
						UE_LOGF(LogMutable, Warning, "[%ls] UCustomizableObjectNodeModifierRemoveMeshBlocks refers to an invalid layout block index %d. Parent node has %d blocks.",
					...
					if (!Id.IsValid())
					{
						UE_LOGF(LogMutable, Warning, "[%ls] UCustomizableObjectNodeModifierRemoveMeshBlocks refers to an valid layout block %d but that block doesn't have an id.",
		...
		{
			UE_LOGF(LogMutable, Log, "[%ls] UCustomizableObjectNodeModifierRemoveMeshBlocks has no parent. It will not be upgraded.", *GetOutermost()->GetName());
		}
	}
	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::UseUVRects)
	{
		...
		if (!ParentMaterialNode)
		{
			...
			UE_LOGF(LogMutable, Warning, "[%ls] UCustomizableObjectNodeModifierRemoveMeshBlocks version upgrade failed.", *GetOutermost()->GetName());
		}
		else
		{
			[&]()
				{
					UEdGraphPin* MeshPin = ParentMaterialNode->GetMeshPin();
					if (!MeshPin)
					{
						return;
					}
			...
			if (Cast<UCONodeSkeletalMeshSection>(ParentMaterialNode))
			{
				for (const FGuid& BlockId : BlockIds_DEPRECATED)
				{
					...
					for (const FCustomizableObjectLayoutBlock& ExistingBlock : Layout->Blocks)
					{
						...
						{
							bSkipBlock = true;
							UE_LOGF(LogMutable, Log, "[%ls] UCustomizableObjectNodeModifierRemoveMeshBlocks has a duplicated layout block id. One has been ignored during version upgrade.", *GetOutermost()->GetName());
							break;
						}
					}
					...
					if (!bFoundInParent)
					{
						UE_LOGF(LogMutable, Warning, "[%ls] UCustomizableObjectNodeModifierRemoveMeshBlocks refers to and invalid layout block. It has been ignored during version upgrade.", *GetOutermost()->GetName());
					}
				}
			}
		}
