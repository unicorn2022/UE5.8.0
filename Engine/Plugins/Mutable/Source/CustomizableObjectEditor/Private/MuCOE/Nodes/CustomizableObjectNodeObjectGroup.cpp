	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UCustomizableObjectNodeObjectGroup, GroupName))
	{
		for (UEdGraphPin* LinkedPin : FollowOutputPinArray(*GroupPin()))
		{
			...
			if (UCustomizableObjectNodeObject* CurrentRootNode = Cast<UCustomizableObjectNodeObject>(Root))
			{
				...
				if (CurrentRootNode && !CurrentRootNode->ParentObject)
				{
...
void UCustomizableObjectNodeObjectGroup::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	UEdGraphPin* ObjectsPin = CustomCreatePin(EGPD_Input, Schema->PC_Object, TEXT("Objects"), LOCTEXT("Objects", "Objects"));
	ObjectsPin->PinType.ContainerType = EPinContainerType::Array;
	
	UEdGraphPin* ProjectorsPin = CustomCreatePin(EGPD_Input, Schema->PC_GroupProjector, TEXT("Projectors"), LOCTEXT("Projectors", "Projectors"));
	ProjectorsPin->PinType.ContainerType = EPinContainerType::Array;
	
	CustomCreatePin(EGPD_Output, Schema->PC_Object, TEXT("Group"), LOCTEXT("Group", "Group"));
}
...
void UCustomizableObjectNodeObjectGroup::SetPinEditableName(const UEdGraphPin& Pin, const FText& InValue)
{
	if (NamePin.Get()->PinId == Pin.PinId)
	{
		FScopedTransaction LocalTransaction(LOCTEXT("UCustomizableObjectNodeObjectGroup_SetGroupNamePinEditableNameTransaction", "Change Group Name PinName"));
		Modify();
