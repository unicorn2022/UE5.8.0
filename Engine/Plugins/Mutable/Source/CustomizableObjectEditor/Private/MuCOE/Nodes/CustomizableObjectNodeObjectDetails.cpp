...
void FCustomizableObjectNodeObjectDetails::CustomizeDetails( IDetailLayoutBuilder& DetailBuilder )
{
	...
	if (BaseObjectNode.IsValid())
	{
		...
		// Hide States and RealTimeMorphSelectionOverrides when this node is a child object (not a base, or has a parent).
		if (!BaseObjectNode->bIsBase || BaseObjectNode->ParentObject)
		{
			DetailBuilder.HideProperty(StatesProperty);
			DetailBuilder.HideProperty("RealTimeMorphSelectionOverrides");
		}
...
void FCustomizableObjectNodeObjectDetails::FillParameterNamesArray()
{
	...
	GraphTraversal::GetAllObjectsInGraph(RootObjet, CustomObjectTree);
