...
void UCustomizableObjectNodeCurve::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	CustomCreatePin(EGPD_Input, Schema->PC_Float, TEXT("Input"), LOCTEXT("Input", "Input"));
	...
	if (CurveAsset)
	{
		...
		for (int32 i = 0; i < Curves.Num(); ++i)
		{
			...
			CustomCreatePin(EGPD_Output, Schema->PC_Float, FName(*PinName), FText::FromString(PinName));
		}
	}
}
...
{
	return LOCTEXT("Curve_Node", "Float Sample Curve");
}
