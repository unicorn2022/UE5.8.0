...
#include UE_INLINE_GENERATED_CPP_BY_NAME(CONodeModifierExtendSkeletalMeshSection)
...
void UCONodeModifierExtendSkeletalMeshSection::BackwardsCompatibleFixup(int32 CustomizableObjectCustomVersion)
{
	Super::BackwardsCompatibleFixup(CustomizableObjectCustomVersion);
	
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::ExtendMaterialRemoveImages)
	{
		const UCONodeSkeletalMeshSection* ParentMaterial = GetCustomizableObjectExternalNode<UCONodeSkeletalMeshSection>(ParentMaterialObject_DEPRECATED.Get(), ParentMaterialNodeId_DEPRECATED);
		if (ParentMaterial)
		{
	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::ConvertEditAndExtendToModifiers)
	{
		...
		if (ParentNode)
		{
			...
			if (UCONodeSkeletalMeshSection* MeshSectionNode = Cast<UCONodeSkeletalMeshSection>(ParentNode))
			{
				...
				MeshSectionNode->Tags.AddUnique(NewLegacyTag);
			}
		...
		else
		{
			...
			UE_LOGF(LogMutable, Log, "[%ls] UCONodeModifierExtendSkeletalMeshSection has no parent. It will not be upgraded.", *GetOutermost()->GetName());
		}
	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::EnableMutableMacrosNewVersion)
	{
		...
		{
			UEdGraphPin* EnableTagsPin = CustomCreatePin(EGPD_Input, UEdGraphSchema_CustomizableObject::PC_String, TEXT("Enable Tags"), LOCTEXT("EnableTags", "Enable Tags"));
			EnableTagsPin->PinType.ContainerType = EPinContainerType::Array;
...
void UCONodeModifierExtendSkeletalMeshSection::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	const UEdGraphSchema_CustomizableObject* Schema = GetDefault<UEdGraphSchema_CustomizableObject>();

	CustomCreatePin(EGPD_Input, Schema->PC_Mesh, TEXT("Add Mesh"), LOCTEXT("AddMesh", "Add Mesh"));
	...
	{
		UEdGraphPin* EnableTagsPin = CustomCreatePin(EGPD_Input, Schema->PC_String, TEXT("Enable Tags"), LOCTEXT("EnableTags", "Enable Tags"));
		EnableTagsPin->PinType.ContainerType = EPinContainerType::Array;
		EnableTagsPin->PinToolTip = "List of Tags that this node will Enable";
		EnableTagsPinRef = EnableTagsPin;
	}
	...
	// Begin texture pins
	{
		...
		for (int32 ImageIndex = 0; ImageIndex < NumImages; ++ImageIndex)
		{
			...
			UEdGraphPin* PinImage = CustomCreatePin(EGPD_Input, Schema->PC_Texture, ImageName, FText::FromName(ImageName));
...
FText UCONodeModifierExtendSkeletalMeshSection::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return LOCTEXT("Extend_MeshSection", "Extend Mesh Section");
}


FString UCONodeModifierExtendSkeletalMeshSection::GetRefreshMessage() const
{
...
UEdGraphPin* UCONodeModifierExtendSkeletalMeshSection::AddMeshPin() const
{
	return FindPin(TEXT("Add Mesh"));
}


UEdGraphPin* UCONodeModifierExtendSkeletalMeshSection::GetEnableTagsPin() const
{
	return EnableTagsPinRef.Get();
}


TArray<UCustomizableObjectLayout*> UCONodeModifierExtendSkeletalMeshSection::GetLayouts() const
{
...
FText UCONodeModifierExtendSkeletalMeshSection::GetTooltipText() const
{
...
bool UCONodeModifierExtendSkeletalMeshSection::IsSingleOutputNode() const
{
	return true;
}


bool UCONodeModifierExtendSkeletalMeshSection::CustomRemovePin(UEdGraphPin& Pin)
{
...
TArray<FString> UCONodeModifierExtendSkeletalMeshSection::GetEnableTags(TArray<const UCustomizableObjectNodeMacroInstance*>* MacroContext)
{ 
...
TArray<FString>* UCONodeModifierExtendSkeletalMeshSection::GetEnableTagsArray()
{
	return &Tags;
}


FString UCONodeModifierExtendSkeletalMeshSection::GetInternalTagDisplayName()
{
