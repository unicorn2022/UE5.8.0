...
void UCustomizableObjectNodeTextureToChannels::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	InputPinReference = CustomCreatePin(EGPD_Input, Schema->PC_Texture, TEXT("Texture"), LOCTEXT("Texture", "Texture"));

	CustomCreatePin(EGPD_Output, Schema->PC_Texture, TEXT("R"), LOCTEXT("R", "R"));

	CustomCreatePin(EGPD_Output, Schema->PC_Texture, TEXT("G"), LOCTEXT("G", "G"));
	
	CustomCreatePin(EGPD_Output, Schema->PC_Texture, TEXT("B"), LOCTEXT("B", "B"));
	
	CustomCreatePin(EGPD_Output, Schema->PC_Texture, TEXT("A"), LOCTEXT("A", "A"));
}
...
{
	return LOCTEXT("Texture_To_Channels", "Texture Break");
}
