	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::ConvertEditAndExtendToModifiers)
	{
		...
		if (ParentNode)
		{
			...
			else
			{
				...
				UE_LOGF(LogMutable, Warning, "[%ls] UCustomizableObjectNodeModifierEditMeshSectionBase version upgrade failed.", *GetOutermost()->GetName());
			}
		...
		{
			UE_LOGF(LogMutable, Log, "[%ls] UCustomizableObjectNodeModifierEditMeshSectionBase has no parent. It will not be upgraded.", *GetOutermost()->GetName());
		}
	}
}
...
UCustomizableObjectLayout* UCustomizableObjectNodeModifierEditMeshSectionBase::GetPossibleParentLayout()
{
	...
	for (UCustomizableObjectNode* Candidate : CandidateNodes)
	{
		...
		if (UCONodeSkeletalMeshSection* MeshSectionNode = Cast<UCONodeSkeletalMeshSection>(Candidate))
		{
			AllLayouts = MeshSectionNode->GetLayouts();
		}

		else if (UCONodeModifierExtendSkeletalMeshSection* ExtendNode = Cast<UCONodeModifierExtendSkeletalMeshSection>(Candidate))
		{
