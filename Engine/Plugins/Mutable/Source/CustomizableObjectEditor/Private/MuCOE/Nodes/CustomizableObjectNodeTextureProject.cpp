...
void UCustomizableObjectNodeTextureProject::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	for ( uint32 LayerIndex = 0; LayerIndex < Textures; ++LayerIndex )
	{
		...
		UEdGraphPin* TexturePin = CustomCreatePin(EGPD_Input, Schema->PC_Texture, FName(*PinName), FText::FromString(PinName));
		
		TexturePinsReferences.Add(FEdGraphPinReference(TexturePin));
		
		PinName = FString::Printf(TEXT("Texture %u"), LayerIndex);
		UEdGraphPin* OutputPin = CustomCreatePin(EGPD_Output, Schema->PC_Texture, FName(*PinName), FText::FromString(PinName));

		OutputPinsReferences.Add(FEdGraphPinReference(OutputPin));
	}

	CustomCreatePin(EGPD_Input, Schema->PC_Mesh, TEXT("Mesh"), LOCTEXT("Mesh", "Mesh"));

	CustomCreatePin(EGPD_Input, Schema->PC_Texture, TEXT("Mesh Mask"), LOCTEXT("MeshMask", "Mesh Mask"));

	UEdGraphPin* AngleFadeStartPin = CustomCreatePin(EGPD_Input, Schema->PC_Float, TEXT("Fade Start Angle"), LOCTEXT("FadeStartAngle", "Fade Start Angle"));
	AngleFadeStartPin->DefaultValue = "90";

	UEdGraphPin* AngleFadeEndPin = CustomCreatePin(EGPD_Input, Schema->PC_Float, TEXT("Fade End Angle"), LOCTEXT("FadeEndAngle", "Fade End Angle"));
	AngleFadeEndPin->DefaultValue = "95";
	
	CustomCreatePin(EGPD_Input, Schema->PC_Projector, TEXT("Projector"), LOCTEXT("Projector", "Projector"));
}
