...
void UCustomizableObjectNodeSkeletalMeshParameter::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	MutableSkeletalMeshPin = CustomCreatePin(EGPD_Output, UEdGraphSchema_CustomizableObject::PC_SkeletalMesh);
	PassthroughPin = CustomCreatePin(EGPD_Output, UEdGraphSchema_CustomizableObject::PC_SkeletalMesh_Object);
		...
		for (int32 LODIndex = 0; LODIndex < ImportedModel->LODModels.Num(); LODIndex++)
		{
			...
			for (int32 SectionIndex = 0; SectionIndex < NumSections; ++SectionIndex)
			{
				...
				// Mesh
				{
					...
					UEdGraphPin* Pin = CustomCreatePin(EGPD_Output, UEdGraphSchema_CustomizableObject::PC_Mesh, FName(*MeshPinName), MeshPinFriendlyName, PinData);
					Pin->bHidden = true;
				}
			}			
		}
...
void UCustomizableObjectNodeSkeletalMeshParameter::BackwardsCompatibleFixup(int32 CustomizableObjectCustomVersion)
{
	...
	{
		PassthroughPin = CustomCreatePin(EGPD_Output, UEdGraphSchema_CustomizableObject::PC_SkeletalMesh_Passthrough);
	}
	
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::PassthroughParameter)
	{
		for (UEdGraphPin* Pin : Pins)
		{
			if (Pin->PinType.PinCategory ==  UEdGraphSchema_CustomizableObject::PC_SkeletalMesh_Passthrough)
			{
				Pin->PinType.PinCategory =  UEdGraphSchema_CustomizableObject::PC_SkeletalMesh_Object;
			}
		}
	}
	
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::SkeletalMeshParameterMutableSkeletalMeshPin)
	{
		MutableSkeletalMeshPin = CustomCreatePin(EGPD_Output, UEdGraphSchema_CustomizableObject::PC_SkeletalMesh);
		
		for (UEdGraphPin* Pin : Pins)
		{
			if (Pin->PinType.PinCategory == UEdGraphSchema_CustomizableObject::PC_SkeletalMesh_Object)
			{
				Pin->PinName = UEdGraphSchema_CustomizableObject::GetPinCategoryName(UEdGraphSchema_CustomizableObject::PC_SkeletalMesh_Object);
				Pin->PinFriendlyName = UEdGraphSchema_CustomizableObject::GetPinFriendlyName(UEdGraphSchema_CustomizableObject::PC_SkeletalMesh_Object);
			}
		}
	}
}
...
bool UCustomizableObjectNodeSkeletalMeshParameter::HasPinViewer() const
{
	return true;
}
