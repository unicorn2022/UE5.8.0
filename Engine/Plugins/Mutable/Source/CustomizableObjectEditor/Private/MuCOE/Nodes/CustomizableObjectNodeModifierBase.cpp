...
bool UCustomizableObjectNodeModifierBase::IsApplicableTo(UCustomizableObjectNode* Candidate)
{
	...
	const FString InternalTag = Candidate->GetInternalTag();
	ensure(!InternalTag.IsEmpty());			// We expect this string to never be empty
	
	TArray<FString> EnabledTags = Candidate->GetEnableTags();
	EnabledTags.Add(InternalTag);
...
void UCustomizableObjectNodeModifierBase::BackwardsCompatibleFixup(int32 CustomizableObjectCustomVersion)
{
	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::AddModifierPin)
	{
		...
		if (OldPin)
		{
			...
			if (!NewPin)
			{
				NewPin = CustomCreatePin(EGPD_Output, Schema->PC_Modifier, TEXT("Modifier"), LOCTEXT("Modifier", "Modifier"));
			}
	...
	if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::EnableMutableMacrosNewVersion)
	{
		...
		{
			UEdGraphPin* Pin = CustomCreatePin(EGPD_Input, UEdGraphSchema_CustomizableObject::PC_String, TEXT("Target Tags"), LOCTEXT("TargetTags", "Target Tags"));
			Pin->PinType.ContainerType = EPinContainerType::Array;
			RequiredTagsPinRef = Pin;
		}
	}
}
