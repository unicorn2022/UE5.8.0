...
{
	UEdGraphPin* OutputPin = CustomCreatePin(EGPD_Output, UEdGraphSchema_CustomizableObject::PC_Texture);
	OutputPin->bDefaultValueIsIgnored = true;
	
	UEdGraphPin* ImagePin = CustomCreatePin(EGPD_Input, UEdGraphSchema_CustomizableObject::PC_Texture);
	BaseImagePinReference = FEdGraphPinReference(ImagePin);
	
	CustomCreatePin(EGPD_Input, UEdGraphSchema_CustomizableObject::PC_Float,  TEXT("Threshold"),  LOCTEXT("Threshold", "Threshold"));
}
