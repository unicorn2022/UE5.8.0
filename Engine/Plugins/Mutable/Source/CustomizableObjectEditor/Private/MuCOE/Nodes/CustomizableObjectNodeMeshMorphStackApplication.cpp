...
void UCustomizableObjectNodeMeshMorphStackApplication::AllocateDefaultPins(UCustomizableObjectNodeRemapPins* RemapPins)
{
	...
	CustomCreatePin(EGPD_Input, UEdGraphSchema_CustomizableObject::PC_Mesh);
	CustomCreatePin(EGPD_Input, UEdGraphSchema_CustomizableObject::PC_Stack);
	
	// Output pins
	
	CustomCreatePin(EGPD_Output, UEdGraphSchema_CustomizableObject::PC_Mesh);
}
...
void UCustomizableObjectNodeMeshMorphStackApplication::BackwardsCompatibleFixup(int32 CustomizableObjectCustomVersion)
{
	...
	else if (CustomizableObjectCustomVersion == FCustomizableObjectCustomVersion::MorphStackCategoryName)
	{
		for (UEdGraphPin* Pin : Pins)
		{
			if (Pin->PinName == TEXT("Stack"))
			{
				Pin->PinName = UEdGraphSchema_CustomizableObject::GetPinCategoryName(UEdGraphSchema_CustomizableObject::PC_Stack);
			}
		}
	}
}
