...
UE::Mutable::Private::Ptr<T> GenerateMutableSourceMacro(const UEdGraphPin& Pin, FMutableGraphGenerationContext& GenerationContext, UE::Mutable::Private::Ptr<T> (*GenerationFunction)(const UEdGraphPin*, FMutableGraphGenerationContext&, ArgTypes...), ArgTypes&&... Args)
{
	...
	if (const UCustomizableObjectNodeMacroInstance* TypedNodeMacro = Cast<UCustomizableObjectNodeMacroInstance>(Node))
	{
		if (const UEdGraphPin* OutputPin = TypedNodeMacro->GetMacroTunnelPin(ECOMacroIOType::COMVT_Output, Pin.PinName))
		{
			if (const UEdGraphPin* FollowPin = FollowInputPin(*OutputPin))
			{
				...
				Result = GenerationFunction(FollowPin, GenerationContext, Forward<ArgTypes>(Args)...);
