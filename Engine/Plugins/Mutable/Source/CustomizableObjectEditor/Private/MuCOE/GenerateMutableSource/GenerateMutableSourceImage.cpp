...
UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImage> GenerateMutableSourceImage(const UEdGraphPin* Pin, FMutableGraphGenerationContext& GenerationContext, int32 ReferenceTextureSize)
{
	...
	{
		if (Pin == TypedNodeTex->TexturePin.Get())
		{
			if (TypedNodeTex->Texture)
			{
				if (UTexture2D* BaseTexture = Cast<UTexture2D>(TypedNodeTex->Texture))
				{
					// Check the specific image cache
					FImageKey ImageKey = FImageKey(Pin);
					...
					else
					{
						...
						ConstantImageNode->SourceDataDescriptor.bNoMipmaps = BaseTexture->MipGenSettings == TextureMipGenSettings::TMGS_NoMipmaps;
				...
				{
					GenerationContext.Log(LOCTEXT("TextureNot2D", "Texture is not a Texture 2D."), Node, EMessageSeverity::Warning);
				}
			}
			else
			{
				GenerationContext.Log(LOCTEXT("MissingImage", "Missing Texture."), Node);
			}
		}
		else if (Pin == TypedNodeTex->PassthroughTexturePin.Get())
		{
			if (TypedNodeTex->Texture)
			{
				UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageConstant> ImageNode = new UE::Mutable::Private::NodeImageConstant();
				ImageNode->Image = GenerateImageConstant(TypedNodeTex->Texture, GenerationContext, true);
	...
	{
		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageObjectParameter> ImageParameterNode = GenerateMutableSourceImageParameter(Pin, GenerationContext);

		const bool bPassthrough = Pin == TypedNodeParam->PassthroughPin.Get();
		if (bPassthrough)
		{
			Result = ImageParameterNode;
		}
		else
		{
			UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageConvert> ImagePassthroughConvertNode = new UE::Mutable::Private::NodeImageConvert();
			ImagePassthroughConvertNode->ImageParameter = ImageParameterNode;

			UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageResize> ResizeNode = new UE::Mutable::Private::NodeImageResize();
			ResizeNode->Base = ImagePassthroughConvertNode;
			...
			{
				const FIntPoint SourceSize = ReferenceTexture->Source.GetLogicalSize();
				
				int32 LODBias = GenerationContext.CompilationContext->Options.TargetPlatform->GetTextureLODSettings().CalculateLODBias(
					SourceSize.X, SourceSize.Y, 
					ReferenceTexture->GetMinTextureSize(), ReferenceTexture->GetMaxTextureSize(),
					ReferenceTexture->LODGroup, ReferenceTexture->LODBias,
					ReferenceTexture->NumCinematicMipLevels, ReferenceTexture->MipGenSettings,
					false, ReferenceTexture->IsStreamable());
				...
				// TODO: Remove Hack
				// Hack - Due to legacy reasons, Mutable adds a LODBias at the top of the root and propagates it to all leaves. 
				// This is wrong and has to be removed. Until then we must compensate the LODBias added at the root to avoid size mismatches.
				if (GenerationContext.CurrentImageProperties)
				{
					TextureSize.X = TextureSize.X << GenerationContext.CurrentImageProperties->BaseLODBias;
					TextureSize.Y = TextureSize.Y << GenerationContext.CurrentImageProperties->BaseLODBias;
				}
			}
			else
			{
			...
			UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageFormat> ImageFormat = GenerateMutableImageFormat(ResizeNode, TypedNodeParam->ReferenceValue, GenerationContext, TypedNodeParam);
			
			// Set expected image formats in game
			ImagePassthroughConvertNode->ImageDescriptor.m_format = ImageFormat->Format;
			ImagePassthroughConvertNode->ImageDescriptor.FormatIfAlpha = ImageFormat->FormatIfAlpha;
	...
	else if (const ICustomizableObjectNodeMeshInterface* TypedNodeMesh = Cast<ICustomizableObjectNodeMeshInterface>(Node))
	{
		...
		{
			ImageNode->Image = GenerateImageConstant(Texture, GenerationContext, false);
	...
	else if (const UCustomizableObjectNodeTextureLayer* TypedNodeLayer = Cast<UCustomizableObjectNodeTextureLayer>(Node))
	{
		...
		{
			if (const UEdGraphPin* OtherPin = FollowInputPin(*TypedNodeLayer->GetLayerPin(LayerIndex)))
			{
				UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImage> MaskNode = nullptr;
				if (const UEdGraphPin* ConnectedPin = FollowInputPin(*TypedNodeLayer->GetMaskPin(LayerIndex)))
				{
				...
				{
					UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeColor> ColorNode = GenerateMutableSourceColor(OtherPin, GenerationContext);

					UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageLayerColor> LayerNode = new UE::Mutable::Private::NodeImageLayerColor;
					LayerNode->Type = Type;
					LayerNode->Base = Result;
					LayerNode->Color = ColorNode;
					LayerNode->Mask = MaskNode;
					Result = LayerNode;
				}			
				else
	...
	else if (const UCONodeSwitch* TypedNodeTextureSwitch = CastSwitch(Node, UEdGraphSchema_CustomizableObject::PC_Texture))
	{
		Result = [&]()
		{
			const UEdGraphPin* SwitchParameter = TypedNodeTextureSwitch->SwitchParameterPinReference.Get();
			...
			if (const UEdGraphPin* ConnectedPin = FollowInputPin(*SwitchParameter))
			{
...

				{
					...
					if (!DoOptionsMatchEnum(*TypedNodeTextureSwitch, *EnumParameter))
					{
				...
				const int32 NumSwitchOptions = TypedNodeTextureSwitch->SwitchPins.Num();
				...
				{
					if (const UEdGraphPin* TexturePin = FollowInputPin(*TypedNodeTextureSwitch->SwitchPins[SelectorIndex].Get()))
					{
	...
	{
		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeColor> color;

		if (const UEdGraphPin* ConnectedPin = FollowInputPin(*TypedNodeFromColor->ColorPin.Get()))
		{
			color = GenerateMutableSourceColor(ConnectedPin, GenerationContext);
		}

		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImagePlainColor> ImageFromColor = new UE::Mutable::Private::NodeImagePlainColor;
		Result = ImageFromColor;

		if (color)
		{
			ImageFromColor->Color = color;
		}

		if (ReferenceTextureSize > 0)
		{
			ImageFromColor->SizeX = ReferenceTextureSize;
			ImageFromColor->SizeY = ReferenceTextureSize;
		}
	}
	...
	{		
		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeColorFromScalars> Color = new UE::Mutable::Private::NodeColorFromScalars();
	...
	{
		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageColorMap> ColorMapNode = new UE::Mutable::Private::NodeImageColorMap();

		Result = ColorMapNode;

		if (const UEdGraphPin* ConnectedPin = FollowInputPin(*TypedNodeColorMap->GetMapPin()))
		{
			...
			ColorMapNode->Map = GradientImageNode;
		}

		if (const UEdGraphPin* ConnectedPin = FollowInputPin(*TypedNodeColorMap->GetMaskPin()))
		{
			UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImage> GradientImageNode = GenerateMutableSourceImage(ConnectedPin, GenerationContext, ReferenceTextureSize);
			ColorMapNode->Mask = GradientImageNode;
		}
		...
		if (const UEdGraphPin* ConnectedPin = FollowInputPin(*TypedNodeColorMap->GetBasePin()))
		{
			...
			ColorMapNode->Base = SourceImageNode;
		}
	}
	...
	else if (const UCONodeSwitch* TypedNodePassThroughTextureSwitch = CastSwitch(Node, UEdGraphSchema_CustomizableObject::PC_Texture_Passthrough))
	{
		Result = [&]()
		{
			const UEdGraphPin* SwitchParameter = TypedNodePassThroughTextureSwitch->SwitchParameterPinReference.Get();
			...
			if (const UEdGraphPin* ConnectedPin = FollowInputPin(*SwitchParameter))
			{
...

				{
					...
					if (!DoOptionsMatchEnum(*TypedNodePassThroughTextureSwitch, *EnumParameter))
					{
						const FText Message = LOCTEXT("MismatchedSwitch", "Switch enum and switch node have different options. Please refresh the switch node to make sure the outcomes are labeled properly.");
						GenerationContext.Log(Message, Node);
						Node->SetRefreshNodeWarning();
				...
				const int32 NumSwitchOptions = TypedNodePassThroughTextureSwitch->SwitchPins.Num();
				...
				{
					if (const UEdGraphPin* TexturePin = FollowInputPin(*TypedNodePassThroughTextureSwitch->SwitchPins[SelectorIndex].Get()))
					{
	...
	// If the node is a plain color node, generate an image out of it
	else if (Pin->PinType.PinCategory == Schema->PC_Color)
	{
		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeColor> ColorNode = GenerateMutableSourceColor(Pin, GenerationContext);

		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImagePlainColor> ImageNode = new UE::Mutable::Private::NodeImagePlainColor;
		ImageNode->SizeX = 16;
		ImageNode->SizeY = 16;
		ImageNode->Color = ColorNode;
		Result = ImageNode;
	}
				...
				if (bSuccess)
				{
					...
					if (GeneratedTable)
					{
						...
						else
						{
							...
							if (Pin->PinType.PinCategory == Schema->PC_Texture_Passthrough)
							{
						...
						if (bSuccess)
						{
							...
							bool bIsPassthrough = (Pin->PinType.PinCategory == Schema->PC_Texture_Passthrough);
	...
	{
		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageExternal> MeshExtension = new UE::Mutable::Private::NodeImageExternal();

		FSourceExternalOptions Options;
		Options.ReferenceTextureSize = ReferenceTextureSize;
	...
	else if (const UCONodeMaterialBreak* TypedNodeMaterialBreak = Cast<UCONodeMaterialBreak>(Node))
	{
		...
		if (const UEdGraphPin* LinkedMaterial = FollowInputPin(*TypedNodeMaterialBreak->MaterialPinRef.Get()))
		{
			...
			if (!ParameterName.IsNone())
			{
				...
				// Store the reference texture into the material generation options to generate all the textures with the correct size
				FGenerationMaterialOptions GenerationOptions;
				GenerationOptions.ReferenceTextureSize = ReferenceTextureSize;
				...
				UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeMaterial> LinkedNode = GenerateMutableSourceMaterial(LinkedMaterial, GenerationContext, GenerationOptions);
...
UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageObjectParameter> GenerateMutableSourceImageParameter(const UEdGraphPin* Pin, FMutableGraphGenerationContext& GenerationContext)
{
	...
	UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageObjectParameter> Result;

	const FGeneratedParameterKey Key = { ParameterNode->NodeGuid, ParameterNode->GetParameterName(&GenerationContext.MacroNodesStack) };
	if (const UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageObjectParameter>* GeneratedImageParameter = GenerationContext.GeneratedImageParameters.Find(Key))
	{
	...
	{
		UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageObjectParameter> TextureNode = new UE::Mutable::Private::NodeImageObjectParameter();
		...
		GenerationContext.TextureParameterDefaultValues.Add(TypedNodeParam->GetParameterName(&GenerationContext.MacroNodesStack), TypedNodeParam->DefaultValue);
	...
	GenerationContext.GeneratedImageParameters.Add(Key, Result);
...
{
	UE::Mutable::Private::Ptr<UE::Mutable::Private::NodeImageFormat> ImageFormat = new UE::Mutable::Private::NodeImageFormat();
	ImageFormat->Source = InSource.get();
	ImageFormat->SetMessageContext(Node);

	// Use an "easy format" as the default value.
	ImageFormat->Format = UE::Mutable::Private::EImageFormat::RGBA_UByte;
	...
	if (ReferenceTexture)
	{
		...
		if (GenerationContext.CompilationContext->Options.TargetPlatform)
		{
			...
			if (!bIsServerOnly && BuildSettingsPerFormatPerLayer.IsEmpty())
			{
				...
				UE_LOGF(LogMutable, Log, "%ls", *ReplacedImageFormatMsg);
			}
		...
		if (BuildSettingsPerFormatPerLayer.IsValidIndex(0) && BuildSettingsPerFormatPerLayer[0].IsValidIndex(0))
		{
			...
			if (GenerationContext.CompilationContext->Options.TextureCompression != ECustomizableObjectTextureCompression::None)
			{
				...
				EPixelFormat UnrealTargetPlatformFormat = EPixelFormat::PF_Unknown;
				EPixelFormat UnrealTargetPlatformFormatAlpha = EPixelFormat::PF_Unknown;

				if (ensure(TextureFormatManager))
				{
					const ITextureFormat* TextureFormat = TextureFormatManager->FindTextureFormat(BuildSettingsPerLayer[0].TextureFormatName);
					if (ensure(TextureFormat))
					{
						UnrealTargetPlatformFormat = TextureFormat->GetEncodedPixelFormat(BuildSettingsPerLayer[0], false);
						UnrealTargetPlatformFormatAlpha = TextureFormat->GetEncodedPixelFormat(BuildSettingsPerLayer[0], true);
					}
				}
				...
				if (GenerationContext.CompilationContext->Options.TargetPlatform)
				{
					...
					if (bUseLA)
					{
						...
						if (bIsNormalMapFormat)
						{
							...
							Swizzle->Sources[0] = InSource;
							Swizzle->Sources[1] = InSource;
							Swizzle->Sources[2] = InSource;
							Swizzle->Sources[3] = InSource;
							Swizzle->SourceChannels[0] = 0;
							Swizzle->SourceChannels[1] = 0;
							Swizzle->SourceChannels[2] = 0;
							Swizzle->SourceChannels[3] = 1;

							ImageFormat->Source = Swizzle;
						}
					}
				}
				...
				ImageFormat->Format = MutableFormat;
				ImageFormat->FormatIfAlpha = MutableFormatIfAlpha;
			}
		}
	}
