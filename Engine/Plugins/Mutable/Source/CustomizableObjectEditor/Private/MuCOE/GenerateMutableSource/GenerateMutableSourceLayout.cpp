...
UE::Mutable::Private::FSourceLayoutBlock ToMutable(const FCustomizableObjectLayoutBlock& UnrealBlock)
{
	...
	if (UnrealBlock.Mask)
	{
		...
		UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FImage> MaskImage = UE::Mutable::Private::MakeManaged<UE::Mutable::Private::FImage>();

		FMutableSourceTextureData Texture(*UnrealBlock.Mask);

		constexpr int32 MipLevel = 0;
		constexpr int32 bLoadMipTail = false;
		EUnrealToMutableConversionError Error = ConvertTextureUnrealSourceToMutable(MaskImage.Get(), Texture, MipLevel, false);
		if (Error != EUnrealToMutableConversionError::Success)
		{
			// This should never happen, so details are not necessary.
			UE_LOGF(LogMutable, Warning, "Failed to convert layout block mask texture.");
		}
		else
		{
