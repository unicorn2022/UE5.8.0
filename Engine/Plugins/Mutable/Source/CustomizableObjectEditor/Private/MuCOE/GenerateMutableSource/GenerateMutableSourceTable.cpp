	...
	const FMutableSourceMeshData& BaseMeshData, FMutableGraphGenerationContext& GenerationContext)
{
	...
	const bool bIgnoreMissingData = GenerationContext.CompilationContext->RootObject->GetPrivate()->bDisableTableMissingDataWarning;
	...
	if (const FSoftObjectProperty* SoftObjectProperty = CastField<FSoftObjectProperty>(ColumnProperty))
	{
		...
		if (SoftObjectProperty->PropertyClass->IsChildOf(USkeletalMesh::StaticClass()))
		{
...

			{
				...
				GetLODAndSection(*GenerationContext.CompilationContext, BaseMeshData, *ReferenceSkeletalMesh, LODIndex, SectionIndex);
			...
			{
				UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FMesh> EmptySkeletalMesh = nullptr;
			...
			if (!AnimSlot.IsEmpty())
			{
				if (uint8* AnimRowData = DataTablePtr->FindRowUnchecked(FName(*RowName)))
				{
					...
					{
						if (uint8* AnimSlotData = AnimSlotProperty->ContainerPtrToValuePtr<uint8>(AnimRowData, 0))
						{
							if (const FIntProperty* IntProperty = CastField<FIntProperty>(AnimSlotProperty))
							{
								...
								AnimBPSlotName = FName(FString::FromInt(IntProperty->GetPropertyValue(AnimSlotData)));
							}
							else if (const FNameProperty* NameProperty = CastField<FNameProperty>(AnimSlotProperty))
							{
								AnimBPSlotName = NameProperty->GetPropertyValue(AnimSlotData);
							}
						}
					}
				}
			...
			if (!AnimBP.IsEmpty())
			{
				if (uint8* AnimRowData = DataTablePtr->FindRowUnchecked(FName(*RowName)))
				{
					...
					{
						if (uint8* AnimBPData = AnimBPProperty->ContainerPtrToValuePtr<uint8>(AnimRowData, 0))
						{
							if (const FSoftClassProperty* SoftClassProperty = CastField<FSoftClassProperty>(AnimBPProperty))
							{
								AnimInstance = SoftClassProperty->GetPropertyValue(AnimBPData).ToSoftObjectPath();
							}
						}
					}
				}
						...
						if (FProperty* GameplayTagProperty = TableNode->FindColumnProperty(FName(*GameplayTag)))
						{
							...
							if (const FStructProperty* StructProperty = CastField<FStructProperty>(GameplayTagProperty))
							{
								if (StructProperty->Struct == TBaseStructure<FGameplayTagContainer>::Get())
								{
									if (GameplayTagData)
									{
										GameplayTags = *reinterpret_cast<FGameplayTagContainer*>(GameplayTagData);
									}
								}
							}
						}
			...
			Source.GameplayTags = GameplayTags;
			Source.AnimBPSlotName = AnimBPSlotName;
			Source.AnimInstance = AnimInstance;

			UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FMesh> MutableMesh = GenerateMutableSkeletalMesh(Source, Source.BaseLODIndex, Source.BaseSectionIndex, GenerationContext, TableNode, Source.bOnlyConnectedLOD);
		...
		else if (SoftObjectProperty->PropertyClass->IsChildOf(UStaticMesh::StaticClass()))
		{
			...
			UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FMesh> MutableMesh = GenerateMutableStaticMesh(StaticMesh, CurrentLOD, BaseMeshData.BaseSectionIndex, GenerationContext, TableNode);
		...
		else if (SoftObjectProperty->PropertyClass->IsChildOf(UTexture::StaticClass()))
		{
			...
			MutableTable->SetCell(CurrentColumn, RowId, GenerateImageConstant(Texture, GenerationContext, bIsPassthroughTexture));
		}
		...
		else if (SoftObjectProperty->PropertyClass->IsChildOf(UMaterialInterface::StaticClass()))
		{
			...
			if (GenerationContext.CurrentTableColumnType == UE::Mutable::Private::ETableColumnType::Material)
			{
				...
				int32 ReferenceMaterialId = GenerationContext.CompilationContext->PassthroughObjectFactory.Add(*MaterialInstance, false);
			...
			if (ParameterIndex != INDEX_NONE && ParameterInfos[ParameterIndex].Name == GenerationContext.CurrentMaterialTableParameter)
			{
				...
				MutableTable->SetCell(ColumnIndex, RowId, GenerateImageConstant(ParameterTexture, GenerationContext, bIsPassthroughTexture));
		...
		else if (SoftObjectProperty->PropertyClass->IsChildOf(UPoseAsset::StaticClass()))
		{
			...
			if (UPoseAsset* PoseAsset = Cast<UPoseAsset>(Object))
			{
				...
				UCustomizableObjectNodeAnimationPose::StaticRetrievePoseInformation(PoseAsset, TableNode->PoseReferenceMesh, ArrayBoneName, ArrayTransform);
...
bool GenerateNoneRow(const UCustomizableObjectNodeTable& TableNode, const UEdGraphPin* Pin, const FString& ColumnName, UE::Mutable::Private::Ptr<UE::Mutable::Private::FTable> MutableTable, FMutableGraphGenerationContext& GenerationContext)
{
	...
	if (Pin->PinType.PinCategory == Schema->PC_Material && GenerationContext.CurrentTableColumnType == UE::Mutable::Private::ETableColumnType::Material)
	{
		...
		int32 MaterialId = GenerationContext.CompilationContext->PassthroughObjectFactory.Add(*GenerationContext.CurrentReferencedMaterial.Get(), false);
		MutableTable->SetCell(ColumnId, RowId, MaterialId);
	}
