...
TSharedRef<FEditorViewportClient> SCustomizableObjectEditorViewport::MakeEditorViewportClient()
{
	...
	if (TSharedPtr<ICustomizableObjectInstanceEditor> Editor = TabBodyPtr.Pin()->WeakEditor.Pin())
	{
		ModeTools = Editor->GetEditorModeTools();
	}

	LevelViewportClient = MakeShareable(new FCustomizableObjectEditorViewportClient(TabBodyPtr.Pin()->WeakEditor, ModeTools, PreviewScenePtr.Pin().Get(), SharedThis(this)));
...
{
	SetTransformWidgetVisibility(EVisibility::Visible);
...
{
	SetTransformWidgetVisibility(EVisibility::Hidden);
...
void SCustomizableObjectEditorViewportTabBody::SetTransformWidgetVisibility(const EVisibility NewVisibilityState) const
{
	if (const TSharedPtr<SWidget> Toolbar = ViewportToolbarTransformWidget.Pin())
	{
		Toolbar->SetVisibility(NewVisibilityState);		
	}
}
