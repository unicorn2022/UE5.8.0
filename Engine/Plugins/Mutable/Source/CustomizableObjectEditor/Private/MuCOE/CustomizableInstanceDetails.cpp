...
{
	if (CustomInstance->GetPrivate()->UpdateLogger)
	{
		// Clear the States tab before loging all the messages
		CustomInstance->GetPrivate()->UpdateLogger->ClearLogMessageList();
	}
...
bool FCustomizableInstanceDetails::GenerateParametersView(IDetailCategoryBuilder& DetailsCategory)
{
	...
	if (CustomInstance->GetPrivate()->bShowOnlyRuntimeParameters)
	{
		...
		for (int32 ParamIndexInState = 0; ParamIndexInState < NumStateParameters; ++ParamIndexInState)
		{
			...
			if (CustomInstance->GetPrivate()->IsParameterRelevant(ParameterSortInfo.ParamIndexInObject) && IsVisible(ParameterSortInfo.ParamIndexInObject))
			{
	...
	else
	{
		...
		for (int32 ParamIndexInObject = 0; ParamIndexInObject < NumObjectParameter; ++ParamIndexInObject)
		{
			...
			if ((!CustomInstance->GetPrivate()->bShowOnlyRelevantParameters || CustomInstance->GetPrivate()->IsParameterRelevant(ParameterSortInfo.ParamIndexInObject)) && IsVisible(ParameterSortInfo.ParamIndexInObject))
			{
				ParameterSortInfo.ParamName = CustomizableObject->GetParameterName(ParameterSortInfo.ParamIndexInObject);
				ParameterSortInfo.ParamUIOrder = CustomizableObject->GetParameterUIMetadata(ParameterSortInfo.ParamName).UIOrder;
				ParametersTree.Add(ParameterSortInfo);
...
TSharedRef<SWidget> FCustomizableInstanceDetails::GenerateParameterWidget(const UCustomizableObject& CustomizableObject, const FString& ParamName, const int32 ParamIndexInObject)
{
	switch (CustomizableObject.GetPrivate()->GetParameterType(ParamIndexInObject))
	{
		...
		{
			FInstancedStruct InstancedStruct = CustomInstance->GetExternalTypeParameterSelectedOption(ParamName);
			...
			ScriptStruct->CopyScriptStruct(StructOnScope->GetStructMemory(), InstancedStruct.GetMemory());

			FPropertyEditorModule& PropertyEditorModule = FModuleManager::LoadModuleChecked<FPropertyEditorModule>("PropertyEditor");

			FDetailsViewArgs DetailViewArgs;
			DetailViewArgs.bAllowSearch = false;
			DetailViewArgs.bShowScrollBar = true;
			DetailViewArgs.NameAreaSettings = FDetailsViewArgs::HideNameArea;

			FStructureDetailsViewArgs StructureDetailsViewArgs;
			StructureDetailsViewArgs.bShowObjects = false;
			
			TSharedRef<IStructureDetailsView> Detailsview = PropertyEditorModule.CreateStructureDetailView(DetailViewArgs, StructureDetailsViewArgs, StructOnScope);
			Detailsview->GetOnFinishedChangingPropertiesDelegate().AddSP(this, &FCustomizableInstanceDetails::OnExternalTypeParameterChanged, StructOnScope, ParamName, -1);
...
TSharedRef<SWidget> FCustomizableInstanceDetails::GenerateFloatWidget(const UCustomizableObject& CustomizableObject, const FString& ParamName)
{
	...
	if (const TSoftObjectPtr<UObject>* FloatDecoratorAsset = UIMetadata.ExtraAssets.Find(UIMetadataKeyWords::FloatDecoratorName))
	{
		...
		if (UTexture2D* DecoratorTexture = Cast<UTexture2D>(UE::Mutable::Private::LoadObject(*FloatDecoratorAsset)))
		{
			...
			[
				...
				.OnMouseCaptureBegin(this, &FCustomizableInstanceDetails::OnFloatParameterSliderBegin, ParamName, -1)
				.OnMouseCaptureEnd(this, &FCustomizableInstanceDetails::OnFloatParameterSliderEnd, ParamName, -1)
	...
	return SNew(SSpinBox<float>)
		...
		.OnBeginSliderMovement(this, &FCustomizableInstanceDetails::OnFloatParameterSliderBegin, ParamName, -1);
}
...
void FCustomizableInstanceDetails::OnFloatParameterSliderBegin(const FString ParamName, int32 RangeIndex)
{
	bUpdatingSlider = true;
	SliderStartValue = CustomInstance->GetFloatParameterSelectedOption(ParamName, RangeIndex);
}


void FCustomizableInstanceDetails::OnFloatParameterSliderEnd(const FString ParamName, int32 RangeIndex)
{
	float Value = CustomInstance->GetFloatParameterSelectedOption(ParamName, RangeIndex);
	
	if (bUpdatingSlider)
	{
		CustomInstance->SetFloatParameterSelectedOption(ParamName, SliderStartValue, RangeIndex); // To be able to undo before the slide movement, before doing the transaction set the value to that original slider position.
	}
	
	{
		FScopedTransaction LocalTransaction(FText::Format(LOCTEXT("OnFloatParameterCommited", "Set Float Parameter: {0}"), FText::FromString(ParamName)));
		CustomInstance->Modify();
		CustomInstance->SetFloatParameterSelectedOption(ParamName, Value, RangeIndex);
	}
...
{
	return CustomInstance->GetVectorParameterSelectedOption(ParamName);
}
...
{
	{
		...
		CustomInstance->Modify();
		CustomInstance->SetVectorParameterSelectedOption(PickerParamName, NewColor);
	}
...
void FCustomizableInstanceDetails::OnTransformParameterCommitted(FTransform NewTransform, ETextCommit::Type Type, const FString ParamName)
{
	...
	const FTransform OldValue = CustomInstance->GetTransformParameterSelectedOption(ParamName);
	if (OldValue.Equals(NewTransform))
	{
		return;
	}

	{
		FScopedTransaction LocalTransaction(FText::Format(LOCTEXT("OnTransformParameterCommited", "Set Transform Parameter: {0}"), FText::FromString(ParamName)));
		CustomInstance->Modify();
...
FReply FCustomizableInstanceDetails::OnProjectorPasteTransform(const FString ParamName, const int32 RangeIndex)
{
...

	{
		...
		CustomInstance->Modify();
		CustomInstance->SetProjectorParameterSelectedOption(ParamName, DefaultValue, RangeIndex);
	}
	...
	for (int32 RangeIndex = 0; RangeIndex < ProjectorParameters[ProjectorParamIndex].RangeValues.Num(); ++RangeIndex)
	{
		...
		[
			...
			[
				...
				.OnBeginSliderMovement(this, &FCustomizableInstanceDetails::OnFloatParameterSliderBegin, OpacitySliderParamName, RangeIndex)
				.Font(LayoutBuilder.Pin()->GetDetailFont())
			]
...
void FCustomizableInstanceDetails::OnExternalTypeParameterChanged(const FPropertyChangedEvent& PropertyChangedEvent, TSharedPtr<FStructOnScope> StructOnScope, FString ParamName, const int32 RangeIndex)
{
	if (const UScriptStruct* ScriptStruct = Cast<UScriptStruct>(StructOnScope->GetStructPtr()))
	{
		CustomInstance->Modify();

		FInstancedStruct InstancedStruct;
		InstancedStruct.InitializeAs(ScriptStruct);
		ScriptStruct->CopyScriptStruct(InstancedStruct.GetMutableMemory(), StructOnScope->GetStructMemory());

		CustomInstance->SetExternalTypeParameterSelectedOption(ParamName, InstancedStruct, RangeIndex);
...
FReply FCustomizableInstanceDetails::OnPasteAllParameters()
{
	...
	bool bImported = false;
	{
		...
		{
			Descriptor.SetCustomizableObject(Descriptor.GetCustomizableObject());
			bImported = true;
		}
	}
...
void FCustomizableInstanceDetails::OnInstanceTransacted(const FTransactionObjectEvent& TransactionEvent)
{
	...
	{
		// We want to make sure that the gizmo is hidden when we do an undo/redo transaction
		GetEditorChecked()->HideGizmo();
	}
}
