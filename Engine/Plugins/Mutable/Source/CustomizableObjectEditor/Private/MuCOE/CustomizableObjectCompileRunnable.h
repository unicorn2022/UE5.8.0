...
public:
	...
	struct FError
	{
		...
		FError(const EMessageSeverity::Type InSeverity, const FText& InMessage, const TArray<const void*, TInlineAllocator<4>>& InContexts, const ELoggerSpamBin InSpamBin = ELoggerSpamBin::ShowAll )
			: Severity(InSeverity), SpamBin(InSpamBin), Message(InMessage), Contexts(InContexts) {}
		FError(const EMessageSeverity::Type InSeverity, const FText& InMessage, const void* InContext, const TSharedPtr<FErrorAttachedData>& InAttachedData, const ELoggerSpamBin InSpamBin = ELoggerSpamBin::ShowAll)
			: Severity(InSeverity), SpamBin(InSpamBin), Message(InMessage), AttachedData(InAttachedData), Contexts{InContext} {}
	};
...
private:
	...
	UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FImage> LoadImageResourceReferenced(int32 ID);
...
public:
	...
	TMap<UE::Mutable::Private::FOperation::ADDRESS, FInstancedStruct> ExternalOperations;
