...
void FCustomizableObjectGraphEditorToolkit::OnEnterText(const FText& NewText, ETextCommit::Type TextType)
{
	...
	UCustomizableObject* CustomizableObject = Cast<UCustomizableObject>(Graph->GetOutermostObject()); // Null if this is a Macro
	...
	for (TObjectPtr<UEdGraphNode> Node : Graph->Nodes)
	{
		...
		{
			LogSearchResult(*Node, "Node", bFound, NodeName, CustomizableObject);
		}
		...
		for (const UEdGraphPin* Pin : Node->GetAllPins())
		{
			...
			{
				LogSearchResult(*Node, "Pin", bFound, PinFriendlyName, CustomizableObject);
			}
		}
		...
		{
			FindProperty(*It, Node, FindString, *Node, bFound, CustomizableObject);
		}
	}
...
void FCustomizableObjectGraphEditorToolkit::FindProperty(const FProperty* Property, const void* InContainer, const FString& FindString, const UObject& Context, bool& bFound, UCustomizableObject* Object)
{
	...
	{
		LogSearchResult(Context, "Property Name", bFound, *PropertyName, Object);
	}
	...
	for (int32 Index = 0; Index < Property->ArrayDim; ++Index)
	{
		...
		if (const FStrProperty* StringProperty = CastField<FStrProperty>(Property))
		{
			...
			{
				LogSearchResult(Context, "Property Value", bFound, *StringResult, Object);
			}
		}
		...
		else if (const FSoftObjectProperty* SoftObjectProperty = CastField<FSoftObjectProperty>(Property))
		{
			...
			{
				LogSearchResult(Context, "Property Value", bFound, ObjectPath, Object);
			}
		}
		else if (const FObjectPropertyBase* ObjectProperty = CastField<FObjectPropertyBase>(Property))
		{
			if (const UObject* ObjectValue = ObjectProperty->GetObjectPropertyValue(ValuePtr))
			{
				...
				{
					LogSearchResult(Context, "Property Value", bFound, Name, Object);
				}
			}
		}
		else if (const FStructProperty* StructProperty = CastField<FStructProperty>(Property))
		{
			...
			{
				FindProperty(*It, ValuePtr, FindString, Context, bFound, Object);
			}
		}
		else if (const FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Property))
		{
			...
			{
				FindProperty(ArrayProperty->Inner, ArrayHelper.GetRawPtr(ValueIdx), FindString, Context, bFound, Object);
			}
		}
		else if (const FSetProperty* SetProperty = CastField<FSetProperty>(Property))
		{
			...
			{
				FindProperty(SetProperty->ElementProp, SetHelper.GetElementPtr(SetIt), FindString, Context, bFound, Object);
			}
		}
		else if (const FMapProperty* MapProperty = CastField<FMapProperty>(Property))
		{
			...
			for (FScriptMapHelper::FIterator MapIt = MapHelper.CreateIterator(); MapIt; ++MapIt)
			{
				...
				FindProperty(MapProperty->KeyProp, MapValuePtr, FindString, Context, bFound, Object);
				FindProperty(MapProperty->ValueProp, MapValuePtr, FindString, Context, bFound, Object);
			}
		}
	}
}
...
void FCustomizableObjectGraphEditorToolkit::LogSearchResult(const UObject& Context, const FString& Type, bool& bIsFirst, const FString& Result, UCustomizableObject* Object) const
{
	if (!bIsFirst)
	{
		bIsFirst = true;

		if (Object && Object->GetPrivate()->CompilationLogger)
		{
			Object->GetPrivate()->CompilationLogger->ClearLogMessageList();
		}
