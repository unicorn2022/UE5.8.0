...
namespace
{
	...
	*/
	 ...
	 * @return : the amount of channels found.
	 */
	int32 GetMeshChannelCount (UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FMesh> MeshPtr)
	{
		check(MeshPtr)
...
namespace MeshConstantTitles
{
	...
	static const FName MeshChannels("Channels");
	static const FName MeshMemory("Size");
}
...
public:
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName) override
	{
		...
		{
			UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FMesh> Mesh = RowElement->MeshPtr;
...
namespace ImageConstantTitles
{
	...
	static const FName ImageMipMaps("Mip Maps");
	static const FName ImageFormat("Format");
	static const FName ImageTotalMemory("Size");
}
...
namespace MaterialConstantTitles
{
	static const FName MaterialID("ID");
	static const FName Material("Material");
	static const FName ImageParametersCount("Images");
	static const FName ScalarParametersCount("Scalars");
	static const FName ColorParametersCount("Colors");
}

class SMutableConstantMaterialRow final : public SMultiColumnTableRow<TSharedPtr<FMutableConstantMaterialElement>>
{
public:
	void Construct(const FArguments& Args, const TSharedRef<STableViewBase>& InOwnerTableView, const TSharedPtr<FMutableConstantMaterialElement>& InRowElement)
	{
		check(InRowElement);
		RowElement = InRowElement;
		
		SMultiColumnTableRow< TSharedPtr<FMutableConstantMaterialElement> >::Construct(
	STableRow::FArguments()
			.ShowSelection(true)
			, InOwnerTableView
		);
	...
	void OnMaterialNavigation() const
	{
		if (RowElement)
		{
			UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FMaterial> MutableMaterial = RowElement->MutableMaterial;
			if (MutableMaterial && MutableMaterial->PassthroughObject.IsResolved())
			{
				const UMaterialInterface* Material = MutableMaterial->PassthroughObject.Get();
				check(Material);
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName) override
	{
		...
		const UE::Mutable::Private::FMaterial* MutableMaterial = RowElement->MutableMaterial.Get();
		if (!MutableMaterial)
		{
			return SNullWidget::NullWidget;
		}
		...
		// Index
		if (ColumnName == MaterialConstantTitles::MaterialID)
		{
			const FText IndexAsText = FText::AsNumber(RowElement->IndexOnSourceVector);
			return SNew(STextBlock).Text(IndexAsText);
		}
		...
		// Material
		if (ColumnName == MaterialConstantTitles::Material)
		{
			// Try to get the actual object and if null then display the ID as fallback
			if (MutableMaterial->PassthroughObject.IsResolved())
			{
				if (const UMaterialInterface* Material = MutableMaterial->PassthroughObject.Get())
				{
					return SNew(SHyperlink)
						.Style(FAppStyle::Get(), TEXT("NavigationHyperlink"))
						.Text(FText::FromString(Material->GetName()))
						.OnNavigate(this, &SMutableConstantMaterialRow::OnMaterialNavigation);
				}
			}
			else
			{
				...
				UE::Mutable::Private::PASSTHROUGH_ID PassthroughMaterialID = MutableMaterial->PassthroughObject.GetId();
		...
		// Image parameters count
		if (ColumnName == MaterialConstantTitles::ImageParametersCount)
		{
			const int32 ImageParametersCount = MutableMaterial->ImageParameters.Num();
			return SNew(STextBlock).Text(FText::AsNumber(ImageParametersCount));
		}
		
		// Color Parameters Count
		if (ColumnName == MaterialConstantTitles::ColorParametersCount)
		{
			const int32 ColorParametersCount = MutableMaterial->ColorParameters.Num();
			return SNew(STextBlock).Text(FText::AsNumber(ColorParametersCount));
		}
		
		// Scalar Parameters Count
		if (ColumnName == MaterialConstantTitles::ScalarParametersCount)
		{
			const int32 ScalarParametersCount = MutableMaterial->ScalarParameters.Num();
			return SNew(STextBlock).Text(FText::AsNumber(ScalarParametersCount));
		}
...
private:
	TSharedPtr<FMutableConstantMaterialElement> RowElement;
};
...
void SMutableConstantsWidget::Construct(const FArguments& InArgs, TSharedPtr<UE::Mutable::Private::FModel> InModel,  TSharedPtr<SMutableCodeViewer> InMutableCodeViewerPtr)
{
	...
	// Panel title
	const FText ConstantsPanelTitle = LOCTEXT("ConstantsPanelName", "Constants : ");
	...
	TArray<TSharedPtr<FMutableConstantImageElement>> DummyImageElementsArray;
	...
	.ListItemsSource(&DummyImageElementsArray)		
	...
	(
		...
		.DefaultLabel(FText::FromName(ImageConstantTitles::ImageID))
		...
		.DefaultLabel(FText::FromName(ImageConstantTitles::ImageSize))
		...
		.DefaultLabel(FText::FromName(ImageConstantTitles::ImageMipMaps))
		...
		.DefaultLabel(FText::FromName(ImageConstantTitles::ImageFormat))
		...
		.DefaultLabel(FText::FromName(ImageConstantTitles::ImageTotalMemory))
	...
	TArray<TSharedPtr<FMutableConstantMeshElement>> DummyMeshElementsArray;
	...
	.OnGenerateRow(this,&SMutableConstantsWidget::OnGenerateMeshRow)
		// We do require to provide something here or the slate to contain the children will not get generated
	.ListItemsSource(&DummyMeshElementsArray)		
	...
	(
		...
		.DefaultLabel(FText::FromName(MeshConstantTitles::MeshID))
		...
		.DefaultLabel(FText::FromName(MeshConstantTitles::MeshVertices))
		.DefaultTooltip(FText(LOCTEXT("MeshVerticesCountColumnToolTip","Amount of vertices")))
		.OnSort(this, &SMutableConstantsWidget::OnMeshTableSortRequested)
		...
		.DefaultLabel(FText::FromName(MeshConstantTitles::MeshIndices))
		...
		.DefaultLabel(FText::FromName(MeshConstantTitles::MeshChannels))
		...
		.DefaultLabel(FText::FromName(MeshConstantTitles::MeshMemory))
	...
	// Hack to allow us to later set the array to be used. if no array is provided then the children slate to contain them later will not exist
	TArray<TSharedPtr<FMutableConstantMaterialElement>> DummyMaterialElementsArray;
	
	// Image Constants List View object and Handler
	TSharedPtr<SListView<TSharedPtr<FMutableConstantMaterialElement>>> ConstantMaterialsListView =
		SNew(SListView<TSharedPtr<FMutableConstantMaterialElement>>)
	.OnGenerateRow(this,&SMutableConstantsWidget::OnGenerateMaterialRow)
		// We do require to provide something here or the slate to contain the children will not get generated
	.ListItemsSource(&DummyMaterialElementsArray)		
	.OnSelectionChanged(this,&SMutableConstantsWidget::OnSelectedMaterialChanged)
	.SelectionMode(ESelectionMode::Single)
	.HeaderRow
	(
		SNew(SHeaderRow)
		+ SHeaderRow::Column(MaterialConstantTitles::MaterialID)
		.DefaultLabel(FText::FromName(MaterialConstantTitles::MaterialID))
		.OnSort(this, &SMutableConstantsWidget::OnMaterialsTableSortRequested)
		.SortMode(this, &SMutableConstantsWidget::GetMaterialListColumnSortMode, MaterialConstantTitles::MaterialID )
		.FillWidth(0.20f)
		
		+ SHeaderRow::Column(MaterialConstantTitles::Material)
		.DefaultLabel(FText::FromName(MaterialConstantTitles::Material))
		.FillWidth(0.40f)
	
		+ SHeaderRow::Column(MaterialConstantTitles::ImageParametersCount)
		.DefaultLabel(FText::FromName(MaterialConstantTitles::ImageParametersCount))
		.OnSort(this, &SMutableConstantsWidget::OnMaterialsTableSortRequested)
		.SortMode(this, &SMutableConstantsWidget::GetMaterialListColumnSortMode, MaterialConstantTitles::ImageParametersCount )
		.FillWidth(0.20f)

		+ SHeaderRow::Column(MaterialConstantTitles::ColorParametersCount)
		.DefaultLabel(FText::FromName(MaterialConstantTitles::ColorParametersCount))
		.OnSort(this, &SMutableConstantsWidget::OnMaterialsTableSortRequested)
		.SortMode(this, &SMutableConstantsWidget::GetMaterialListColumnSortMode, MaterialConstantTitles::ColorParametersCount )
		.FillWidth(0.20f)

		+ SHeaderRow::Column(MaterialConstantTitles::ScalarParametersCount)
		.DefaultLabel(FText::FromName(MaterialConstantTitles::ScalarParametersCount))
		.OnSort(this, &SMutableConstantsWidget::OnMaterialsTableSortRequested)
		.SortMode(this, &SMutableConstantsWidget::GetMaterialListColumnSortMode, MaterialConstantTitles::ScalarParametersCount )
		.FillWidth(0.20f)
	);
	...
	[
		...
		.Padding(0,InBetweenListsVerticalPadding)
		[
			SAssignNew(MaterialsExpandableArea,SExpandableArea)
			.OnAreaExpansionChanged(this,&SMutableConstantsWidget::OnMaterialsRegionExpansionChanged)
			.InitiallyCollapsed(true)
			.AreaTitle(this,&SMutableConstantsWidget::OnDrawMaterialsAreaTitle)
			.BodyContent()
			[
				// Custom slate that will handle the updating of the elements displayed by the ConstantMeshesListView object
				SAssignNew(MaterialistViewHandler ,SMutableMultiPageListView<TSharedPtr<FMutableConstantMaterialElement>>)
				.HostedListView(ConstantMaterialsListView)	
				.ElementsToSeparateInPages(ConstantMaterialElements)
			]
		]
	];
	...
	ExpandableAreas.Add(MaterialsExpandableArea);
}



void SMutableConstantsWidget::SetModel(const TSharedPtr<UE::Mutable::Private::FModel>& InModel)
{
	// Make sure we do not process the setting of the same program object as the one already set
	if (MutableModel == InModel)
	{
		return;
	}
...
TSharedRef<ITableRow> SMutableConstantsWidget::OnGenerateMaterialRow(TSharedPtr<FMutableConstantMaterialElement> MutableConstantMaterialElement,
	const TSharedRef<STableViewBase>& OwnerTable) const
{
	TSharedRef<SMutableConstantMaterialRow> Row = SNew(SMutableConstantMaterialRow, OwnerTable, MutableConstantMaterialElement);
	return Row;
}
...
void SMutableConstantsWidget::OnMaterialsTableSortRequested(EColumnSortPriority::Type ColumnSortPriority, const FName& ColumnID,
	EColumnSortMode::Type ColumnSortMode)
{
	// If the colum has been sorted on one way now do the inverse way of sorting
	MaterialConstantsLastSortedColumnID = ColumnID;
	MaterialListSortMode = ColumnSortMode;

	Algo::StableSort(*ConstantMaterialElements, [&](const TSharedPtr<FMutableConstantMaterialElement>& A, const TSharedPtr<FMutableConstantMaterialElement>& B)
	{
		if (!A || !B)
		{
			return false;
		...
		// Sort by material id
		if (ColumnID == MaterialConstantTitles::MaterialID)
		{
			if (ColumnSortMode == EColumnSortMode::Ascending)
			{
				return A->IndexOnSourceVector < B->IndexOnSourceVector;
		...
		// Sort by Image Parameters count
		if (ColumnID == MaterialConstantTitles::ImageParametersCount)
		{
			if (ColumnSortMode == EColumnSortMode::Ascending)
			{
				return A->MutableMaterial->ImageParameters.Num() < B->MutableMaterial->ImageParameters.Num();
			}
			return A->MutableMaterial->ImageParameters.Num() > B->MutableMaterial->ImageParameters.Num();
		}

		// Sort by Color Parameters count
		if (ColumnID == MaterialConstantTitles::ColorParametersCount)
		{
			if (ColumnSortMode == EColumnSortMode::Ascending)
			{
				return A->MutableMaterial->ColorParameters.Num() < B->MutableMaterial->ColorParameters.Num();
			}
			return A->MutableMaterial->ColorParameters.Num() > B->MutableMaterial->ColorParameters.Num();
		}
		
		// Sort by Scalar Parameters count
		if (ColumnID == MaterialConstantTitles::ScalarParametersCount)
		{
			if (ColumnSortMode == EColumnSortMode::Ascending)
			{
				return A->MutableMaterial->ScalarParameters.Num() < B->MutableMaterial->ScalarParameters.Num();
			}
			return A->MutableMaterial->ScalarParameters.Num() > B->MutableMaterial->ScalarParameters.Num();
		}
	...
	if (MaterialistViewHandler)
	{
		MaterialistViewHandler->RegeneratePage();
	}
}


EColumnSortMode::Type SMutableConstantsWidget::GetMaterialListColumnSortMode(FName ColumnName) const
{
	if (MaterialConstantsLastSortedColumnID != ColumnName)
	{
		return EColumnSortMode::None;
	}

	return MaterialListSortMode;
}
...
void SMutableConstantsWidget::OnMaterialsRegionExpansionChanged(bool bExpanded)
{
	if (bExpanded)
	{
		ContractExpandableAreas(MaterialsExpandableArea);
	}
}
...
void SMutableConstantsWidget::ContractExpandableAreas(const TSharedPtr<SExpandableArea>& InException)
{
	...
	if(!InException)
	{
		UE_LOGF(LogTemp,Warning, "No valid expandable area has been provided as exception : All expandable areas will therefore get contracted");
	}
#endif
...
void SMutableConstantsWidget::LoadConstantStrings()
{
	...
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantStrings.Num();
	...
	for (int32 StringAddressIndex = 0; StringAddressIndex < ConstantsCount; StringAddressIndex++)
	{
		...
		ConstantStringElement->MutableString = MutableModel->GetProgram().ConstantStrings[StringAddressIndex];
	...
	// Cache the size in memory of the constants as a formatted text so it is able to be later used by the UI
...
void SMutableConstantsWidget::LoadConstantImages()
{
	...
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantImages.Num();
	...
	for (int32 ImageIndex = 0; ImageIndex < ConstantsCount; ImageIndex++)
	{
		...
		MutableModel->GetConstant(ImageIndex, ConstantImageElement->ImagePtr, 0,
		[this, ModelReader](int32 RomIndex)
		{
			return MutableModel->RomManager.GetRom<UE::Mutable::Private::FImage>(RomIndex, ModelReader).GetResult();
		},
		[this](int32 x, int32 y, int32 m, UE::Mutable::Private::EImageFormat f, UE::Mutable::Private::EInitializationType i) 
		{ 
			return UE::Mutable::Private::MakeManaged<UE::Mutable::Private::FImage>(x, y, m, f, i); 
		});
...
void SMutableConstantsWidget::LoadConstantMeshes()
{
	...
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantMeshes.Num();
	...
	for (int32 MeshIndex = 0; MeshIndex < ConstantsCount; MeshIndex++)
	{
		...
		MutableModel->GetConstant(MeshIndex, INDEX_NONE, {}, ConstantMeshElement->MeshPtr, UE::Mutable::Private::EMeshContentFlags::AllFlags,
		[this, ModelReader](int32 RomIndex)
		{
			return MutableModel->RomManager.GetRom<UE::Mutable::Private::FMesh>(RomIndex, ModelReader).GetResult();
		},
		[](int32 BudgetReserve)
		{
			return UE::Mutable::Private::MakeManaged<UE::Mutable::Private::FMesh>();
		});
...
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantLayouts.Num();
	...
	{		
		...
		ConstantLayoutElement->Layout = MutableModel->GetProgram().ConstantLayouts[LayoutIndex];
...
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantSkeletons.Num();
	...
	for (int32 SkeletonIndex = 0; SkeletonIndex < ConstantsCount; SkeletonIndex++)
	{
		...
		ConstantSkeletonElement->Skeleton = MutableModel->GetProgram().ConstantSkeletons[SkeletonIndex];
...
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantPhysicsBodies.Num();
	...
	for (int32 PhysicsIndex = 0; PhysicsIndex < ConstantsCount; PhysicsIndex++)
	{
		...
		ConstantPhysicsElement->Physics = MutableModel->GetProgram().ConstantPhysicsBodies[PhysicsIndex];
...
void SMutableConstantsWidget::LoadConstantMaterials()
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantMaterials.Num();
	
	if (ConstantMaterialElements)
	{
		ConstantMaterialElements->Empty(ConstantsCount);
	}
	else
	{
		ConstantMaterialElements = MakeShared<TArray<TSharedPtr<FMutableConstantMaterialElement>>>();
		ConstantMaterialElements->Reserve(ConstantsCount);
	}
	
	UE::Mutable::Private::FOutputMemoryStream Stream;
	UE::Mutable::Private::FOutputArchive Archive{&Stream};
	
	for (int32 MaterialIndex = 0; MaterialIndex < ConstantsCount; MaterialIndex++)
	{
		TSharedPtr<FMutableConstantMaterialElement> ConstantMaterialElement = MakeShared<FMutableConstantMaterialElement>();
		ConstantMaterialElement->MutableMaterial = MutableModel->GetProgram().ConstantMaterials[MaterialIndex];
		ConstantMaterialElement->IndexOnSourceVector = MaterialIndex;
		ConstantMaterialElement->MutableMaterial->Serialise(Archive);

		ConstantMaterialElements->Add(ConstantMaterialElement);
	}

	// Cache the size in memory of the constants as a formatted text so it is able to be later used by the UI
	ConstantMaterialsFormattedSize = GenerateTextForSize(Stream.GetBufferSize());
}
...
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantProjectors.Num();
	...
	for (int32 ProjectorIndex = 0; ProjectorIndex < ConstantsCount; ProjectorIndex++)
	{
		...
		ConstantProjectorElement->Projector = &(MutableModel->GetProgram().ConstantProjectors[ProjectorIndex]);
...
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantMatrices.Num();
	...
	for (int32 MatrixIndex = 0; MatrixIndex < ConstantsCount; MatrixIndex++)
	{
		...
		ConstantMatrixElement->Matrix = MutableModel->GetProgram().ConstantMatrices[MatrixIndex];
...
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantShapes.Num();
	...
	for (int32 ShapeIndex = 0; ShapeIndex < ConstantsCount; ShapeIndex++)
	{
		...
		ConstantShapeElement->Shape = &(MutableModel->GetProgram().ConstantShapes[ShapeIndex]);
...
{
	check(MutableModel);
	const int32 ConstantsCount = MutableModel->GetProgram().ConstantCurves.Num();
	...
	for (int32 CurveIndex = 0; CurveIndex < ConstantsCount; CurveIndex++)
	{
		...
		ConstantCurveElement->Curve = MutableModel->GetProgram().ConstantCurves[CurveIndex];
...
void SMutableConstantsWidget::ClearSelectedConstantItems(UE::Mutable::Private::EDataType ExceptionDataType /* = UE::Mutable::Private::EDataType::None */) const
{
	...
	if (ExceptionDataType != UE::Mutable::Private::EDataType::Material)
	{
		MaterialistViewHandler->ClearSelection();
	}
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::String;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Image;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Mesh;
...
void SMutableConstantsWidget::OnSelectedMaterialChanged(TSharedPtr<FMutableConstantMaterialElement> MutableConstantMaterialElement,
	ESelectInfo::Type SelectionType) const
{
	if (MutableConstantMaterialElement)
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Material;
		
		MutableCodeViewerPtr->ClearSelectedTreeRow();
		ClearSelectedConstantItems(SlateDataType);

		MutableCodeViewerPtr->PreviewMutableMaterial(MutableConstantMaterialElement->MutableMaterial);
		MutableCodeViewerPtr->CacheAddressesRelatedWithConstantResource(SlateDataType,MutableConstantMaterialElement->IndexOnSourceVector);
	}
}
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Layout;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Projector;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Matrix;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Shape;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Curve;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::Skeleton;
	...
	ESelectInfo::Type SelectionType) const
{
	...
	{
		constexpr UE::Mutable::Private::EDataType SlateDataType = UE::Mutable::Private::EDataType::PhysicsAsset;
...
{
	return FText::Format( LOCTEXT("ImageConstantsTitle", "Image Constants ({0}) : {1} "), ConstantImageElements ? ConstantImageElements->Num() : 0, ConstantImagesFormattedSize);
}

FText SMutableConstantsWidget::OnDrawMeshesAreaTitle() const
{
	return FText::Format(LOCTEXT("MeshConstantsTitle", "Mesh Constants ({0}) : {1} "), ConstantMeshElements ? ConstantMeshElements->Num() : 0, ConstantMeshesFormattedSize) ;
}
...
FText SMutableConstantsWidget::OnDrawMaterialsAreaTitle() const
{
	return FText::Format( LOCTEXT("MaterialConstantsTitle", "Material Constants ({0}) : {1} "), ConstantMaterialElements ? ConstantMaterialElements->Num() : 0, ConstantMaterialsFormattedSize);
}
