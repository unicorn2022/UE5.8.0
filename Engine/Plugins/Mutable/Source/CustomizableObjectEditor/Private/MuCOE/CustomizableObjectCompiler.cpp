...
void FCustomizableObjectCompiler::Tick(bool bBlocking)
{
	...
	{
		const int32 NumCompletedRequests = NumCompilationRequests - GetRemainingWork();
...
int32 FCustomizableObjectCompiler::GetRemainingWork() const
{
	return GetNumCompileRequests();
}


int32 FCustomizableObjectCompiler::GetNumCompileRequests() const
{
...
void FCustomizableObjectCompiler::PreloadReferencerAssets()
{
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] Preload asynchronously assets start.", FPlatformTime::Seconds());
...
void FCustomizableObjectCompiler::PreloadingReferencerAssetsCallback(bool bAsync)
{
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] Preload asynchronously assets end.", FPlatformTime::Seconds());
...
void FCustomizableObjectCompiler::Compile(const TSharedRef<FCompilationRequest>& InCompileRequest)
{
	...
	if (!Object)
	{
		UE_LOGF(LogMutable, Warning, "Failed to compile Customizable Object. Object is missing.");
	...
	// Add visual separation in States tab between Compilation Messages and Update Messages
	if(CurrentObject->GetPrivate()->CompilationLogger)
	{
		TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(EMessageSeverity::Info, FText::FromString("=================== Object Compilation ==================="));
		CurrentObject->GetPrivate()->CompilationLogger->LogMessage(Message);
	}
	
	if (!UCustomizableObjectSystem::IsActive())
	{
		CompilerLog( LOCTEXT("CompilationFailure_MutableIsDisabled","Failed to compile Customizable Object. Mutable is disabled. To enable it set the CVar Mutable.Enabled to true."), CurrentObject, EMessageSeverity::Warning);
		CompleteRequest(ECompilationStatePrivate::Completed, ECompilationResultPrivate::Errors);
		return;
	}

	TArray<const UObject*> VisitedCustomizableObjects;
	const UCustomizableObject* RootObject = GraphTraversal::GetRootObject(CurrentObject, &VisitedCustomizableObjects); 
	
	// No root object means a cycle was found when looking for it
	if (!RootObject)
	{
		CompilerLog(LOCTEXT("CompilationFailure_COCircularReferenceFound", "Failed to compile Customizable Object. Customizable Object circular reference found."), CurrentObject, EMessageSeverity::Warning);

		// Report objects part of the cycle
		check(VisitedCustomizableObjects.Num() >= 2);
		if (VisitedCustomizableObjects.Num() >= 2)
		{
			const FString InvalidObjectName = "Unknown";
				
			const FText CycleStartObjectName =  FText::FromString( ensure(VisitedCustomizableObjects[0]) ? VisitedCustomizableObjects[0]->GetName() : InvalidObjectName );
			const FText CycleClosureObjectName = FText::FromString( ensure(VisitedCustomizableObjects[VisitedCustomizableObjects.Num() - 1]) ? VisitedCustomizableObjects[VisitedCustomizableObjects.Num() - 1]->GetName() : InvalidObjectName );
			const FText WarningText = FText::Format(LOCTEXT("CompilationFailure_COCircularReferenceFoundDetails", "Circular reference found starting at '{0}' CO and finishing at '{1}' CO. COs part of the cycle :"),  CycleStartObjectName,  CycleClosureObjectName);
		
			CompilerLog( WarningText , VisitedCustomizableObjects, EMessageSeverity::Warning);
		}
	...
	{
		const FText WarningText = FText::Format(LOCTEXT("CompilationFailure_RootNotImplementingVersionBridgeInterface", "Failed to compile Customizable Object. The VersionBridge asset {0} does not implement the required UCustomizableObjectVersionBridgeInterface"),  FText::FromString(RootObject->VersionBridge.GetName()));
		CompilerLog( WarningText, {RootObject, RootObject->VersionBridge}, EMessageSeverity::Warning);
		CompleteRequest(ECompilationStatePrivate::Completed, ECompilationResultPrivate::Errors);
		return;
	}
	...
	{
		CompilerLog( LOCTEXT("CompilationSuspended_CookInProgress", "Editor compilation suspended. Can not compile COs when the cook commandlet is running."), CurrentObject, EMessageSeverity::Info);
		CompleteRequest(ECompilationStatePrivate::Completed, ECompilationResultPrivate::Errors);
		return;
	}
...

	{
		...
		{
			UE_LOGF(LogMutable, Display, "%ls", *MutableCompilationStartMessage);
		}
		else
		{
			...
			{
				UE_LOGF(LogMutable, Display, "%ls The Compilation will take more time to run due to the chosen optimization level.", *MutableCompilationStartMessage);
			}
			else
			{
				UE_LOGF(LogMutable, Display, "%ls", *MutableCompilationStartMessage);
	...
	{
		UE_LOGF(LogMutable, Display, "Compiling Customizable Object with %d LODBias.", CurrentOptions.DebugBias);
	}
...
void FCustomizableObjectCompiler::EnqueueCompileRequest(const TSharedRef<FCompilationRequest>& CompileRequest, bool bForceRequests)
{
	...
	// Clear all previous log messages
	if (TSharedPtr<class IMutableEditorLogger> Logger = CustomizableObject->GetPrivate()->CompilationLogger)
	{
		Logger->ClearLogMessageList();
	}
	
	{
		...
		if (AssetRegistry.IsSearchAllAssets())
		{
			...
			{
				UE_LOGF(LogMutable, Log, "Waiting for the AR scan to be completed before enqueueing the compilation of the %ls CO.", *CustomizableObject->GetName());
		...
		else
		{
			...
			UE_LOGF(LogMutable, Display, "Performing full %s Asset Registry search as required by the CO compilation process.", "ASYNC");
...
void ProcessChildObjectsRecursively(const UCustomizableObject* ParentObject, FMutableGraphGenerationContext& GenerationContext)
{
	...
	for (FAssetData AssetData : AssetDataArray)
	{
		...
		if (ChildObject->VersionStruct.IsValid())
		{
			...
			{
				UE_LOGF(LogMutable, Warning, "The child Customizable Object [%ls] defines its VersionStruct Property but its root CustomizableObject doesn't define the VersionBridge property. There's no way to verify the VersionStruct has to be included in this compilation, so the child CustomizableObject will be omitted.", 
					*ChildObject->GetName());
				continue;
			}
	...
	FMutableGraphGenerationContext& GenerationContext)
{
	...
	if (!Object->GetPrivate()->GetSource())
	{
		...
		{
			UE_LOGF(LogMutable, Warning, "Compilation failed! Missing EDITORONLY data for Customizable Object [%ls]. The object might have been loaded outside the Cooking context.", *Object->GetName());
		}
	...
	const UCustomizableObject* RootObject = GenerationContext.CompilationContext->RootObject.Get();
	...
	if ((Object->GetPrivate()->GetMeshCompileType() == EMutableCompileMeshType::Full) || GenerationContext.CompilationContext->Options.bIsCooking)
	{
		...
		{
			UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] Begin search for children.", FPlatformTime::Seconds());
			...
			UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] End search for children.", FPlatformTime::Seconds());
		}
	}
	else
	{
	...
	{
		if (UCONodeComponentSkeletalMesh* NodeComponentMesh = Cast<UCONodeComponentSkeletalMesh>(&Node))
		{
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] GenerateMutableSource start.", FPlatformTime::Seconds());
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] GenerateMutableSource end.", FPlatformTime::Seconds());
...
void FCustomizableObjectCompiler::CompileInternal(bool bAsync)
{
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] FCustomizableObjectCompiler::Compile start.", FPlatformTime::Seconds());
	...
	const UCustomizableObject* RootObject = GraphTraversal::GetRootObject(CurrentObject);
	check(RootObject);
	
	CompilationContext = MakeShared<FMutableCompilationContext>(RootObject, SharedThis(this), CurrentOptions);
	...
	AddCompileNotification(LOCTEXT("GraphCompilationMessage", "Compiling Graph"));
	...
	else
	{
		...
		ModelResources->ExternalTypeParameterDefaultValues = MoveTemp(GenerationContext.ExternalTypeParameterDefaultValues);

		ModelResources->ExternalTypeParameterTypes = MoveTemp(GenerationContext.ExternalTypeParameterTypes);
		...
		{
			AddCompileNotification(LOCTEXT("CustomizableObjectCompileInProgress", "Compiling Object"));
...
void FCustomizableObjectCompiler::CompleteRequest(ECompilationStatePrivate State, ECompilationResultPrivate Result)
{
	...
	if (CurrentState == ECompilationStatePrivate::InProgress && CurrentObject)
	{
		...
		{
			PlatformData->Model->RomManager.UnloadRoms();
		}
		...
		if (!CurrentOptions.bIsCooking)
		{
			if (Result == ECompilationResultPrivate::Success || Result == ECompilationResultPrivate::Warnings)
			{
				...
				CurrentObject->GetPrivate()->SetModel(PlatformData->Model, GenerateIdentifier(*CurrentObject), CurrentOptions.bIsCooking);
			}
			else
			{
				...
				CurrentObject->GetPrivate()->SetModel(nullptr, {}, CurrentOptions.bIsCooking);
			}

			CurrentObject->GetPrivate()->PostCompile();
		}

		UE_LOGF(LogMutable, Display, "Finished compiling Customizable Object %ls. Compilation took %5.3f seconds to complete.",
	...
	if (CurrentObject && CurrentObject->GetPrivate()->CompilationLogger)
	{
		TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(EMessageSeverity::Info, FText::FromString("=========================================================="));
		CurrentObject->GetPrivate()->CompilationLogger->LogMessage(Message);
	}
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] Completed compile request.", FPlatformTime::Seconds());
	UE_LOGF(LogMutable, Verbose, "PROFILE: -----------------------------------------------------------");
}
...
bool FCustomizableObjectCompiler::TryPopCompileRequest()
{
	...
	for (TArray<TSharedRef<FCompilationRequest>>::TIterator It = CompileRequests.CreateIterator(); It; ++It)
	{
		...
		if (!Object)
		{
			...
			SelectedCompileRequest->Callback.ExecuteIfBound(Params);
			SelectedCompileRequest->CallbackNative.ExecuteIfBound(Params);
			continue;
		}
				...
				if (Response.Status == EStatus::Ok)
				{
					...
					else
					{
						UE::Mutable::Private::GenerateBulkDataFilesListWithSizeLimit(PlatformData->Model, *PlatformData->ModelStreamableBulkData.Get(),
							Options.TargetPlatform, 0 /* unused */, PlatformData->BulkDataFiles);
					}
				}
				else
				{
...
void FCustomizableObjectCompiler::FinishLoadingStreamableDataFromDDC()
{
	...
	if (PlatformData->Model && PlatformData->ModelResources && PlatformData->ModelStreamableBulkData)
	{
		...
		UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] Finishing Compilation task for CO [%ls].", FPlatformTime::Seconds(), *CurrentObject->GetName());
		TRACE_END_REGION(UE_MUTABLE_COMPILE_REGION);

		UE_LOGF(LogMutable, Display, "Compiled data loaded from DDC");
...
{
	UE_LOGF(LogMutable, Log, "Started Customizable Object Export %ls.", *Object->GetName());
	...
	CompilerOptions.bUseLegacyLayouts = Object->GetPrivate()->ShouldUseLegacyLayouts();
...
void FCustomizableObjectCompiler::FinishCompilationTask()
{
	...
	if (Model)
	{
		const int32 NumStreamingFiles = Model->GetProgram().GetRomCount();
		...
		{
			const uint32 ResourceSize = Model->GetProgram().GetRomSize(FileIndex);
			EMutableFileFlags Flags = Model->GetProgram().IsRomHighRes(FileIndex) ? EMutableFileFlags::HighRes : EMutableFileFlags::None;
	...
	{
		CompilerLog(LOCTEXT("NoModel", "Unable to generate Model"), nullptr, EMessageSeverity::Error);
	}
	...
	// Second part of model resources can be filled now.
	{
		...
		ModelResources->PassthroughObjects = MoveTemp(CompilationContext->PassthroughObjectFactory.GetAll());
		ModelResources->DuplicateObjects = MoveTemp(CompilationContext->PassthroughObjectFactory.GetDuplicate());

		ModelResources->ExternalOperations = MoveTemp(CompileTask->ExternalOperations);

		for (TPair<UE::Mutable::Private::FOperation::ADDRESS, FInstancedStruct> ExternalOperation : ModelResources->ExternalOperations)
		{
			const uint32 Version = ExternalOperation.Value.GetPtr<UE::Mutable::FExternalOperation>()->GetVersion();
			ModelResources->ExternalOperationVersions.Add(ExternalOperation.Value.GetScriptStruct(), Version);
		}
...
void FCustomizableObjectCompiler::FinishSavingDerivedDataTask()
{
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] Finished Saving Derived Data task for CO [%ls].", FPlatformTime::Seconds(), *CurrentObject->GetName());
...
void FCustomizableObjectCompiler::ForceFinishCompilation()
{
	...
	else if (CompileTask.IsValid())
	{
		...
		UE_LOGF(LogMutable, Verbose, "Force Finish Compilation task for Object.");
		TRACE_END_REGION(UE_MUTABLE_COMPILE_REGION);
	}
	...
	else if (SaveDDTask.IsValid())
	{
		...
		UE_LOGF(LogMutable, Verbose, "Forced Finish Saving Derived Data task.");
		TRACE_END_REGION(UE_MUTABLE_SAVEDD_REGION);
	}
...
void FCustomizableObjectCompiler::AddCompileNotification(const FText& CompilationStep) const
{
	...
	.CustomNotification()
	.FixNotification()
	.CustomizableObject(CurrentObject)
	.Log();
}
...
void FCustomizableObjectCompiler::CompilerLog(const FText& Message, const TArray<const UObject*>& Context, const EMessageSeverity::Type MessageSeverity, const bool bAddBaseObjectInfo, const ELoggerSpamBin SpamBin)
{
	if (CompilationLogsContainer.AddMessage(Message, Context, MessageSeverity, SpamBin)) // Cache the message for later reference
	{
		...
		.SpamBin(SpamBin)
		.CustomizableObject(CurrentRequest ? CurrentRequest->GetCustomizableObject() : nullptr)
		.Log(); 
	}
}
...
void FCustomizableObjectCompiler::UpdateCompilerLogData()
{
	...
	for (const FCustomizableObjectCompileRunnable::FError& CompileError : ArrayCompileErrors)
	{
		...
		ObjectArray.Reserve(CompileError.Contexts.Num());
		for (const void* Context : CompileError.Contexts)
		{
			ObjectArray.Add(static_cast<const UObject*>(Context));
		}

		const UObject* CompileErrorContext = ObjectArray.IsValidIndex(0) ? ObjectArray[0] : nullptr; 
		
		if (CompileErrorContext && CompileError.AttachedData)
		{
			if (const UCustomizableObjectNode* Node = Cast<UCustomizableObjectNode>(CompileErrorContext))
			{
