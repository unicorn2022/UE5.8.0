...
public:
	...
	/**
	 * Evaluates the graph starting at the TargetOPAddress and returns an FImage resulting from that
	 * @param TargetOpAddress The OP Address to use as the root to be computed.
	 * @return A pointer to the FImage generated. May be null if the operation does not evaluate to an FImage
	 * @note This method is experimental and should not be used as example
	 */
	UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FImage>  BuildImage(UE::Mutable::Private::FOperation::ADDRESS TargetOpAddress) const;
...
private:
	...
	UE::Mutable::Private::FOperation::ADDRESS SelectedOperationAddress = 0;
	...
	*/
	...
	/** Widget used to show a preview of a mutable FMaterial and the data it holds */
	TSharedPtr<SMutableMaterialViewer> PreviewMaterialViewer;
	...
	*/
	...
	{
		UE::Mutable::Private::FOperation::ADDRESS Parent=0;
		UE::Mutable::Private::FOperation::ADDRESS Child=0;
	...
	*/
	...
	TMap< UE::Mutable::Private::FOperation::ADDRESS, TSharedPtr<FMutableCodeTreeElement>> MainItemPerOp;
	...
	TMap< UE::Mutable::Private::FOperation::ADDRESS, TSharedPtr<FMutableCodeTreeElement>> ExpandedElements;
	...
	void GenerateElement( const int32& InStateIndex, UE::Mutable::Private::FOperation::ADDRESS InParentAddress, const UE::Mutable::Private::FProgram& InProgram);

	
	/** The addresses of the root operations. Cached when this object gets loaded on the Construct method of this slate */
	TArray<UE::Mutable::Private::FOperation::ADDRESS> RootNodeAddresses;
	...
	UE::Mutable::Private::FOperation::ADDRESS HighlightedOperation{};

	/*
	 * Operations computational cost reference collections
	 */

	enum class EEntryLabelColoringPolicy : int32
	{
		BY_COST = 0,
		BY_DATATYPE,
	...
	const TArray<FName> EntryLabelColoringPolicyNames
	{
		"BY_COST",
		"BY_DATATYPE",
	};
	
	/** Determines what should be used to paint the labels */
	EEntryLabelColoringPolicy EntryLabelColoringPolicy = EEntryLabelColoringPolicy::BY_DATATYPE;
	...
	const TArray<FSlateColor> ColorPerComputationalCost
	{
		...
		FSlateColor(FLinearColor(1,0.4f,0.2f,1)),		// Expensive cost color
		FSlateColor(FLinearColor(1,0.1f,0.1f,1))		// Very Expensive cost color
	};
	...
	FSlateColor GetOperationDataTypeColor(UE::Mutable::Private::EDataType DataType) const;
	
public:
	FSlateColor GetOperationColor(const UE::Mutable::Private::EOpType OperationType) const;

private:
	void OnLabelColorPolicyChange(FName PolicyName, ESelectInfo::Type Arg);
	TSharedRef<SWidget>  OnLabelColorPolicyGenerateWidget(FName PolicyName);
	FText GetCurrentLabelColoringPolicyText() const;
	 ...
	 */
	TSharedRef<SWidget> OnGenerateOpNavigationDropDownWidget(TSharedPtr<const FMutableOperationElement> InMutableOperationElement) const;
	...
	struct FElementsSearchCache
	{
		...
		TSet<UE::Mutable::Private::FOperation::ADDRESS> ProcessedAddresses;
		 ...
		 */
		void SetupRootBatch(const TArray<UE::Mutable::Private::FOperation::ADDRESS>& InRootNodeAddresses)
		{
		...
		*/
		void AddToFoundElements(const UE::Mutable::Private::FOperation::ADDRESS OpAddress,const int32 IndexAsChildOfInputAddress,
		 ...
		 */
		void CacheChildrenOfAddressIfNotProcessed(UE::Mutable::Private::FOperation::ADDRESS InParentAddress,
		                                          ...
		                                          TArray<FItemCacheKey>& OutFoundChildrenData)
		{
			if (!ProcessedAddresses.Contains(InParentAddress))
			{
				...
				uint32 ChildIndex = 0;
				UE::Mutable::Private::ForEachReference(InProgram, InParentAddress, [this, &InParentAddress, &ChildIndex,&OutFoundChildrenData]( UE::Mutable::Private::FOperation::ADDRESS ChildAddress)
				{
...
private:
	...
	bool IsConstantResourceUsedByOperation(const int32 IndexOnConstantsArray,
	                                       const UE::Mutable::Private::EDataType ConstantDataType, const UE::Mutable::Private::FOperation::ADDRESS OperationAddress,
	...
	*/
	...
	struct FProcessedOperationsBuffer
	{
		...
		TArray<UE::Mutable::Private::FOperation::ADDRESS> ExpandedOriginalOperations;

		/** Array with the operation addresses of all duplicated expanded operations. */
		TArray<UE::Mutable::Private::FOperation::ADDRESS> ExpandedDuplicatedOperations;
	};
...
public:
	void PreviewMutableImage (UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FImage> InImagePtr);
	void PreviewMutableMesh (UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FMesh> InMeshPtr);
	void PreviewMutableMaterial(UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FMaterial> InMaterialPtr);
	void PreviewMutableLayout(UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FLayout> Layout);
	void PreviewMutableSkeleton(UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FSkeleton> Skeleton);
	void PreviewMutablePhysics(UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FPhysicsBody> Physics);
...
public:
	...
	FMutableOperationElement(UE::Mutable::Private::EOpType InOperationType,FText OperationTypeName,uint32 OperationTypeInstanceCount)
	{
	...
	/** Set the color to be used by this entry so it gets used instead of the one defined by the coloring policy 
	 * @param InEntryColorOverride The color to be used for this entry.
	 */
	void SetEntryColorOverride(const FSlateColor& InEntryColorOverride)
	{
		bEntryColorWasOverriden = true;
		OperationTextOverrideColor = InEntryColorOverride;
	}
	...
	FSlateColor OperationTextOverrideColor;
	bool bEntryColorWasOverriden = false;

	/** Get the type of visibility a slate representing this objects should have. */
...
{
public:
	FMutableCodeTreeElement(int32 InIndexOnTree, const int32& InMutableStateIndex, const TSharedPtr<UE::Mutable::Private::FModel, ESPMode::ThreadSafe>& InModel, UE::Mutable::Private::FOperation::ADDRESS InOperation, const FString& InCaption, TWeakPtr<SMutableCodeViewer> InCodeViewer, UE::Mutable::Private::EOpType InOperationType, const TSharedPtr<FMutableCodeTreeElement>* InDuplicatedOf = nullptr);
...
public:
	...
	UE::Mutable::Private::FOperation::ADDRESS MutableOperation;
...
private:
	...
	/** Code Viewer hosting the tree that uses this object. Will be used to evaluate the code to be displayed by the tree entry. */
	TWeakPtr<SMutableCodeViewer> HostCodeViewer;

	/** The type of the mutable operation. */
	UE::Mutable::Private::EOpType OperationType;
	
	/** The color to be used by the row representing this object */
	FSlateColor ZeroOperationColor = FColor(FColorList::DimGrey);
};
