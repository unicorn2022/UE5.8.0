...
UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FImage> FCustomizableObjectCompileRunnable::LoadImageResourceReferenced(int32 ID)
{
	MUTABLE_CPUPROFILER_SCOPE(LoadResourceReferenced);

	UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FImage> Image;
	...
	Image = UE::Mutable::Private::MakeManaged<UE::Mutable::Private::FImage>();
	constexpr int32 MipmapsToSkip = 0;
	constexpr bool bLoadMipTail = false;
	EUnrealToMutableConversionError Error = ConvertTextureUnrealSourceToMutable(Image.Get(), TextureData, MipmapsToSkip, bLoadMipTail);
	...
	if (Error != EUnrealToMutableConversionError::Success)
	{
		...
		UE_LOGF(LogMutable, Warning, "Failed to load some source texture data for texture ID [%d]. Some textures may be corrupted.", ID);
	}
...
uint32 FCustomizableObjectCompileRunnable::Run()
{
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] FCustomizableObjectCompileRunnable::Run start.", FPlatformTime::Seconds());
	...
	{
		UE_LOGF(LogMutable, Log, "Mutable compile optimization level out of range. Clamping to maximum.");
	...
	CompilerOptions->SetReferencedResourceCallback(
		[this](int32 ID, TSharedPtr<UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FImage>> ResolvedImage, bool bRunImmediatlyIfPossible)
		{
			UE::Tasks::FTask LaunchTask = UE::Tasks::Launch(TEXT("LoadImageReferenceTasks"),
				[ID,ResolvedImage,this]()
				{
					UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FImage> Result = LoadImageResourceReferenced(ID);
		...
		[this](int32 ID, const FString& MorphNameRef, TSharedPtr<UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FMesh>> ResolvedMesh, bool bRunImmediatlyIfPossible) -> UE::Tasks::FTask
		{
			...
			auto ConvertMeshFunc = [MeshData, MorphName, ResolvedMesh, CompletionEvent, WeakCompiler = WeakCompiler, bRunImmediatlyIfPossible]() mutable
				{
					...
					{
						*ResolvedMesh = UE::Mutable::Private::MakeManaged<UE::Mutable::Private::FMesh>();
					...
					else
					{
						...
						{
							Streamable->RequestAsyncLoad(AssetsToStream, OnAssetLoadCallback, FStreamableManager::AsyncLoadHighPriority - 1);
						}
					}
				};
	...
	CompilerOptions->SetDataPackingStrategy( MinResidentMips, Options.EmbeddedDataBytesLimit );

	// We always compile for progressive image generation.
	CompilerOptions->SetEnableProgressiveImages(true);

	// On server builds we don't want materials and images to be generted.
	if (Options.TargetPlatform && Options.TargetPlatform->IsServerOnly())
	{
		CompilerOptions->SetDisableMaterialGeneration(true);
	}
	...
	TFunction<void()> WaitCallback = [WeakCompiler = WeakCompiler]()
	{
		...
		if (System)
		{
			System->GetPrivate()->StreamableManager->Tick(true);
		}
	};

	UE::Mutable::Private::Ptr<UE::Mutable::Private::Compiler> Compiler = new UE::Mutable::Private::Compiler(CompilerOptions, WaitCallback);

	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] FCustomizableObjectCompileRunnable Compile start.", FPlatformTime::Seconds());
	Model = Compiler->Compile(MutableRoot, ExternalOperations);
	...
	for (int32 i = 0; i < pLog->GetMessageCount(); ++i)
	{
		...
		if (MessageType == UE::Mutable::Private::ELMT_WARNING || MessageType == UE::Mutable::Private::ELMT_ERROR)
		{
			...
			const TArray<const void*, TInlineAllocator<4>>* MessageContexts = pLog->GetMessageContexts(i);
			if (ensure(MessageContexts))
			{
				...
				{			
					...
					const void* Context = MessageContexts->IsValidIndex(0) ? (*MessageContexts)[0] : nullptr;
					ArrayErrors.Add(FError(Severity, FText::AsCultureInvariant(Message), Context, ErrorAttachedData, SpamBin));
				}
				else
				{
					ArrayErrors.Add(FError(Severity, FText::AsCultureInvariant(Message), *MessageContexts, SpamBin));
				}
			}
		}
	}
	...
	UE_LOGF(LogMutable, Verbose, "PROFILE: [ %16.8f ] FCustomizableObjectCompileRunnable::Run end.", FPlatformTime::Seconds());
...
void FCustomizableObjectSaveDDRunnable::CachePlatformData()
{
	...
	// Cache ModelStreamables
	{
		...
		{
			UE::Mutable::Private::GenerateBulkDataFilesListWithSizeLimit(PlatformData->Model, *PlatformData->ModelStreamableBulkData.Get(), Options.TargetPlatform, MAX_uint64, PlatformData->BulkDataFiles);
		}
	}
...
void FCustomizableObjectSaveDDRunnable::StoreCachedPlatformDataToDisk()
{
	...
	for (int32 DataType = 0; DataType < NumDataTypes; ++DataType)
	{
		...
		{
			UE_LOGF(LogMutable, Error, "Failed to delete file for data type [%d].", DataType);
			bSuccess = false;
			break;
		}
	...
	if (bSuccess)
	{
		...
		for (TUniquePtr<FArchive>& MemoryWriter : MemoryWriters)
		{
			if (MemoryWriter)
			{
				...
				{
					UE_LOGF(LogMutable, Error, "Failed to write file to disk. File [%ls].", *MemoryWriter->GetArchiveName());
					bSuccess = false;
					break;
				}
			}
