...
void UCustomizableObjectInstanceBaker::BakeInstance(UCustomizableObjectInstance* InTargetInstance, const FBakingConfiguration& InBakingConfig, const TSharedPtr<const FOnBakerFinishedWork> InOnBakerFinishedWork)
{
	...
	{
		UE_LOGF(LogMutable,Error, "No target save path has been provided.")
	...
	if (!InTargetInstance)
	{
		UE_LOGF(LogMutable,Error, "No instance has been provided.")
		FinishBakingOperation(false, nullptr);
		return;
	}
	...
	{
		UE_LOGF(LogMutable,Error, "The provided instance does not have a Customizable Object defined.")
		FinishBakingOperation(false, nullptr);
		return;
	}
...
void UCustomizableObjectInstanceBaker::OnCompilationEnd(const FCompileCallbackParams& InCompileCallbackParams)
{
	if (InCompileCallbackParams.bRequestFailed || InCompileCallbackParams.bErrors)
	{
		...
		UE_LOGF(LogMutable, Display, "The Customizable Object compilation for baking failed. Aborting instance baking operation.");
		FinishBakingOperation(false, nullptr);
		return;
	}
...
void UCustomizableObjectInstanceBaker::OnInstanceUpdated(const FUpdateContext& Result) const
{
	...
	else
	{
		...
		UE_LOGF(LogMutable, Display, "Instance updating for baking failed. Aborting instance baking operation.");
		FinishBakingOperation(false, nullptr);
	}
}
...
void UCustomizableObjectInstanceBaker::Bake() const
{
	...
	TArray<FBakedResourceData> SavedPackagesData;
	{
		...
		UE_LOGF(LogMutable, Log, "Saved assets (%d) :", SavedPackages.Num());
		...
		for (const TPair<UPackage*, const FResourceBakingData>& SavedPackagePair : SavedPackages)
		{
			...
			const FResourceBakingData& PackageData = SavedPackagePair.Value;
			BakeData.SaveType = PackageData.SaveResolutionType;
			BakeData.Prefix = PackageData.GetPrefixSection();
			BakeData.UserGivenName = PackageData.GetUserGivenNameSection();
			BakeData.ComponentName = PackageData.GetComponentNameSection();
			BakeData.ResourceName = PackageData.GetResourceNameSection();
			BakeData.UniqueAssetNameSuffix = PackageData.GetUniqueAssetNameSuffixSection();
			...
			UE_LOGF(LogMutable, Log, "\t%ls --- %ls",  *UEnum::GetValueAsString(BakeData.SaveType), *BakeData.AssetPath);
	...
	UE_LOGF(LogMutable, Display, "Finishing COI baking procedure.");
