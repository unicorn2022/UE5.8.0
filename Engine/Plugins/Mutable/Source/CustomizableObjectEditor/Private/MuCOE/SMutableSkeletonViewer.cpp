...
void SMutableSkeletonViewer::SetSkeleton(UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FSkeleton> InSkeleton)
{
...
void SMutableSkeletonViewer::FillBoneDefinitionArrays()
{
	...
	const int32 BoneCount = Skeleton->GetNumBones();
	for (int32 BoneIndex = 0; BoneIndex < BoneCount; BoneIndex++)
	{
		...
		NewBoneDefinition->BoneName = Skeleton->BoneNames[BoneIndex];

		// Get the parent node if any
		const int32 ParentBoneIndex = Skeleton->BoneParents[BoneIndex];
		if (ParentBoneIndex != INDEX_NONE)
		{
			NewBoneDefinition->ParentBoneIndex = ParentBoneIndex;
			NewBoneDefinition->ParentBoneName = Skeleton->BoneNames[ParentBoneIndex];
