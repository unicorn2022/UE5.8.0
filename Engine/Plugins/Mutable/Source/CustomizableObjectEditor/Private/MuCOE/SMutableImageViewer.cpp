...
void SMutableImageViewer::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (bIsPendingUpdate)
	{
		...
		if (MutableImage && MutableImage->GetSizeX()>0 && MutableImage->GetSizeY()>0)
		{
			...
			UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FImage> ImageToConvert = MutableImage;
...
void SMutableImageViewer::SetImage(const UE::Mutable::Private::TManagedPtr<const UE::Mutable::Private::FImage>& InMutableImage, int32 CurrentLOD)
{
