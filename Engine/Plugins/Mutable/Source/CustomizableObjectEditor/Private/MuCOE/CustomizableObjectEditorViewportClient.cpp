...
FCustomizableObjectEditorViewportClient::FCustomizableObjectEditorViewportClient(TWeakPtr<ICustomizableObjectInstanceEditor> InCustomizableObjectEditor, FEditorModeTools* InModeTools, FPreviewScene* InPreviewScene, const TSharedPtr<SEditorViewport>& EditorViewportWidget)
	: FEditorViewportClient(InModeTools, InPreviewScene, EditorViewportWidget)
...
void FCustomizableObjectEditorViewportClient::ShowGizmoClipMesh(UCustomizableObjectNode& InClipMeshNode, FTransform* InClipMeshTransform, UObject& InClipMesh, int32 LODIndex, int32 SectionIndex, int32 MaterialSlotIndex)
{
	...
	else if (UCONodeClipMeshWithMesh* NodeClipMeshWithMesh = Cast<UCONodeClipMeshWithMesh>(ClipMeshNode.Get()))
	{
		TransformExternallyChangedDelegateHandle = NodeClipMeshWithMesh->TransformChangedDelegate.AddSP(this, &FCustomizableObjectEditorViewportClient::UpdateGizmoClipMeshTransform);
	}
	else if (UCONodeClipSkeletalMeshWithSkeletalMesh* NodeClipSkeletalMeshWithSkeletalMesh = Cast<UCONodeClipSkeletalMeshWithSkeletalMesh>(ClipMeshNode.Get()))
	{
		TransformExternallyChangedDelegateHandle = NodeClipSkeletalMeshWithSkeletalMesh->TransformChangedDelegate.AddSP(this, &FCustomizableObjectEditorViewportClient::UpdateGizmoClipMeshTransform);
	}
	else if (UCONodeTransformInMesh* NodeTransformInMesh = Cast<UCONodeTransformInMesh>(ClipMeshNode.Get()))
	{
		TransformExternallyChangedDelegateHandle = NodeTransformInMesh->TransformChangedDelegate.AddSP(this, &FCustomizableObjectEditorViewportClient::UpdateGizmoClipMeshTransform);
	}
	else
	{
		unimplemented();
	...
	UMaterial* TargetMaterial = ClipMeshMaterial.Get();
	...
	if (UStaticMesh* StaticMesh = Cast<UStaticMesh>(&InClipMesh))
	{
		...
		{
			ClipMeshStaticMeshComp->SetMaterial(MaterialSlotIndex, TargetMaterial);
		}
		else
		{
			const int32 StaticMaterialCount = StaticMesh->GetStaticMaterials().Num();
			for (int32 SlotIndex = 0; SlotIndex < StaticMaterialCount; SlotIndex++)
			{
				ClipMeshStaticMeshComp->SetMaterial(SlotIndex, TargetMaterial);
			}
		}
	...
	else if (USkeletalMesh* SkeletalMesh = Cast<USkeletalMesh>(&InClipMesh))
	{
		...
		if (MaterialSlotIndex >= 0)
		{
			ClipMeshSkeletalMeshComp->SetMaterial(MaterialSlotIndex, TargetMaterial);
		}
		else
		{
			const int32 StaticMaterialCount = SkeletalMesh->GetNumMaterials();
			for (int32 SlotIndex = 0; SlotIndex < StaticMaterialCount; SlotIndex++)
			{
				ClipMeshSkeletalMeshComp->SetMaterial(SlotIndex, TargetMaterial);
			}
		}
...
void FCustomizableObjectEditorViewportClient::HideGizmoClipMesh()
{
	if (WidgetType == EWidgetType::ClipMesh)
	{
		...
		if (ensure(TransformExternallyChangedDelegateHandle.IsValid()))
		{
			...
			{
				NodeClipMeshWithMesh->TransformChangedDelegate.Remove(TransformExternallyChangedDelegateHandle);
			}
			else if (UCONodeClipSkeletalMeshWithSkeletalMesh* NodeClipSkeletalMeshWithSkeletalMesh = Cast<UCONodeClipSkeletalMeshWithSkeletalMesh>(ClipMeshNode.Get()))
			{
				NodeClipSkeletalMeshWithSkeletalMesh->TransformChangedDelegate.Remove(TransformExternallyChangedDelegateHandle);
			}
			else if (UCONodeTransformInMesh* NodeTransformInMesh = Cast<UCONodeTransformInMesh>(ClipMeshNode.Get()))
			{
				NodeTransformInMesh->TransformChangedDelegate.Remove(TransformExternallyChangedDelegateHandle);
			}
			else
			{
				unimplemented();
...
void FCustomizableObjectEditorViewportClient::OnPreSetSkeletalMesh(const FPreSetSkeletalMeshParams& Params)
{
	...
	// Add new components
	for (UE::Mutable::Private::FComponentId ComponentId = 0; ComponentId < CO->GetComponentCount(); ++ComponentId)
	{
		FName Name = CO->GetPrivate()->GetComponentName(ComponentId);
...
public:
	...
	/** Get the object that contins all the user provided settings. It gets filled when the user interacts with the UI so it is always up to date. */
	FBakingConfiguration GetCustomBakingConfiguration() const;
...
protected:
	...
	void OnExportAllResourcesValueChanged(ECheckBoxState InCheckboxState);
	void OnGenerateConstantMaterialInstancesValueChanged(ECheckBoxState InCheckboxState);
	void OnRemoveRestrictedCharactersValueChanged(ECheckBoxState InCheckboxState);

	void OnSkeletalMeshPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);
	void OnSkeletonPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);
	void OnPhysicsAssetPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);
	void OnTexturePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);
	void OnMaterialPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);
	void OnMaterialInstancePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);
	void OnMaterialDynamicInstancePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);
	void OnMaterialConstantInstancePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo);


	EAppReturnType::Type UserResponse = EAppReturnType::Cancel; 
	
	FBakingConfiguration BakeConfiguration;
};
...
void FCustomizableObjectEditorViewportClient::BakeInstance()
{
	...
	if (!Instance)
	{
		UE_LOGF(LogMutable, Error, "No Mutable Customizable Object instance was found in the current editor.");
		return;
	}
	...
	if (!InstanceToBakeCO)
	{
		UE_LOGF(LogMutable, Error, "The Customizable Object instance provided for the baking does not have a Customizable Object set.");
		return;
	}
...
void FCustomizableObjectEditorViewportClient::OnCOForBakingCompilationEnd(const FCompileCallbackParams& InCompileCallbackParams)
{
	...
	{
		UE_LOGF(LogMutable, Warning , "The Customizable Object failed the compilation. Skipping instance baking");
		return;
	}
...
void FCustomizableObjectEditorViewportClient::OnInstanceForBakingUpdate(const FUpdateContext& Result)
{
	...
	{
		UE_LOGF(LogMutable, Warning , "Instance finished update with an error state : %ls. Skipping instance baking", *UEnum::GetValueAsString(Result.UpdateResult));
	...
	if (ensure(BakeTempInstance))
	{
		...
		if (FolderDlg->ShowModal() != EAppReturnType::Cancel)
		{
			BakeCustomizableObjectInstance(
				*BakeTempInstance,
				FolderDlg->GetCustomBakingConfiguration(),
				false);
		}
...
FText FCustomizableObjectEditorViewportClient::GetMeshInfoText() const
{
	...
	for (const TPair<FName, TWeakObjectPtr<UDebugSkelMeshComponent>>& Entry : SkeletalMeshComponents)
	{
		...
		const int32 NumBonesInUse = LODData.RequiredBones.Num();
		const int32 NumBonesMappedToVerts = LODData.ActiveBoneIndices.Num();
		const int32 NumSectionsInUse = LODData.RenderSections.Num();
...
void SMutableSelectFolderDlg::Construct(const FArguments& InArgs)
{
	...
	BakeConfiguration.OutputPath = FPackageName::GetLongPackagePath(InArgs._DefaultAssetPath.ToString());
	if (BakeConfiguration.OutputPath.IsEmpty())
	{
		BakeConfiguration.OutputPath = TEXT("/Game");
	}
	
	// File name
	BakeConfiguration.OutputFilesBaseName = InArgs._DefaultFileName.ToString();

	FPathPickerConfig PathPickerConfig;
	PathPickerConfig.DefaultPath = BakeConfiguration.OutputPath;
	...
	SWindow::Construct(SWindow::FArguments()
		...
		[
			...
			[
				...
				.AreaTitle(LOCTEXT("FilePrefixSettingsTitle", "File Prefix Settings"))
				.AreaTitleFont(FSlateFontInfo(FCoreStyle::GetDefaultFont(), 12, "Regular"))
				.BodyContent()
				[
					SNew(SVerticalBox)
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("SkeletalMeshPrefix", "Skeletal Mesh"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.SkeletalMeshAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnSkeletalMeshPrefixStringChange)
							.MinDesiredWidth(20)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("SkeletonPrefix", "Skeleton"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.SkeletonAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnSkeletonPrefixStringChange)
							.MinDesiredWidth(20)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("PhysicsAssetPrefix", "Physics Asset"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.PhysicsAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnPhysicsAssetPrefixStringChange)
							.MinDesiredWidth(20)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("TexturePrefix", "Texture"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.TextureAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnTexturePrefixStringChange)
							.MinDesiredWidth(20)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("MaterialPrefix", "Material"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.MaterialAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnMaterialPrefixStringChange)
							.MinDesiredWidth(20)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("MaterialInstancePrefix", "Material Instance"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.MaterialInstanceAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnMaterialInstancePrefixStringChange)
							.MinDesiredWidth(20)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("MaterialDynamicInstance", "Material Dynamic Instance"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.MaterialDynamicInstanceAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnMaterialDynamicInstancePrefixStringChange)
							.MinDesiredWidth(20)
						]
					]

					// Material Constant Instance
					+ SVerticalBox::Slot()
					.AutoHeight()
					.Padding(2)
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("MaterialConstantInstancePrefix", "Material Constant Instance"))
						]
						...
						[
							SNew(SEditableTextBox)
							.Text(FText::FromString(BakeConfiguration.MaterialConstantInstanceAssetPrefix))
							.OnTextCommitted(this, &SMutableSelectFolderDlg::OnMaterialConstantInstancePrefixStringChange)
							.MinDesiredWidth(20)
						]
					]
				]
			...
			[
				SNew(SExpandableArea)
				.AreaTitle(LOCTEXT("OtherSettingsTitle", "Other Settings"))
				.AreaTitleFont(FSlateFontInfo(FCoreStyle::GetDefaultFont(), 12, "Regular"))
				.BodyContent()
				[
					SNew(SVerticalBox)
					...
					[
						...
						[
							...
							.HAlign(HAlign_Right)
							.IsChecked(BakeConfiguration.bExportAllResourcesOnBake ? ECheckBoxState::Checked : ECheckBoxState::Unchecked)
							.OnCheckStateChanged(this, &SMutableSelectFolderDlg::OnExportAllResourcesValueChanged)
						]
					]
					...
					[
						...
						[
							...
							.HAlign(HAlign_Right)
							.IsChecked(BakeConfiguration.bGenerateConstantMaterialInstancesOnBake ? ECheckBoxState::Checked : ECheckBoxState::Unchecked)
							.OnCheckStateChanged(this, &SMutableSelectFolderDlg::OnGenerateConstantMaterialInstancesValueChanged)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(LOCTEXT("RemoveRestrictedCharacters", "Remove Restricted Characters  "))
							.ToolTipText(LOCTEXT("RemoveRestrictedCharacters_Tooltip", "Replaces invalid characters in the final composed resource names with '_'."))
						]
						...
						[
							SNew(SCheckBox)
							.ToolTipText(LOCTEXT("RemoveRestrictedCharacters_Checkbox", "Remove Restricted Characters"))
							.HAlign(HAlign_Right)
							.IsChecked(BakeConfiguration.bReplaceRestrictedCharacters ? ECheckBoxState::Checked : ECheckBoxState::Unchecked)
							.OnCheckStateChanged(this, &SMutableSelectFolderDlg::OnRemoveRestrictedCharactersValueChanged)
						]
					]
				]
			]
...
{
	BakeConfiguration.OutputPath = NewPath;
}
...
{
	BakeConfiguration.OutputFilesBaseName = NewName.ToString();
}


void SMutableSelectFolderDlg::OnExportAllResourcesValueChanged(ECheckBoxState InCheckboxState)
{
	BakeConfiguration.bExportAllResourcesOnBake = InCheckboxState == ECheckBoxState::Checked;
}


void SMutableSelectFolderDlg::OnGenerateConstantMaterialInstancesValueChanged(ECheckBoxState InCheckboxState)
{
	BakeConfiguration.bGenerateConstantMaterialInstancesOnBake = InCheckboxState == ECheckBoxState::Checked;
}


void SMutableSelectFolderDlg::OnRemoveRestrictedCharactersValueChanged(ECheckBoxState InCheckboxState)
{
	BakeConfiguration.bReplaceRestrictedCharacters = InCheckboxState == ECheckBoxState::Checked;
}


void SMutableSelectFolderDlg::OnSkeletalMeshPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.SkeletalMeshAssetPrefix = NewPrefix.ToString();
}


void SMutableSelectFolderDlg::OnSkeletonPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.SkeletonAssetPrefix = NewPrefix.ToString();
}


void SMutableSelectFolderDlg::OnPhysicsAssetPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.PhysicsAssetPrefix = NewPrefix.ToString();
}


void SMutableSelectFolderDlg::OnTexturePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.TextureAssetPrefix = NewPrefix.ToString();
}	


void SMutableSelectFolderDlg::OnMaterialPrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.MaterialAssetPrefix = NewPrefix.ToString();
}	


void SMutableSelectFolderDlg::OnMaterialInstancePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.MaterialInstanceAssetPrefix = NewPrefix.ToString();
}	


void SMutableSelectFolderDlg::OnMaterialDynamicInstancePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.MaterialDynamicInstanceAssetPrefix = NewPrefix.ToString();
}	
	

void SMutableSelectFolderDlg::OnMaterialConstantInstancePrefixStringChange(const FText& NewPrefix, ETextCommit::Type CommitInfo)
{
	BakeConfiguration.MaterialConstantInstanceAssetPrefix = NewPrefix.ToString();
}	
...
FBakingConfiguration SMutableSelectFolderDlg::GetCustomBakingConfiguration() const
{
	return BakeConfiguration;
}
