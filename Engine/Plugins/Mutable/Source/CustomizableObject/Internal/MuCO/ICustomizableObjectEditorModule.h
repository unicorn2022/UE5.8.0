...
public:
	...
	virtual void GetAllObjectsInGraph(UCustomizableObject*, TSet<UCustomizableObject*>& OutRelated ) const = 0;
	...
	/** Evict scenes from the UCustomizableObjectSkeletalMeshThumbnailRenderer cache used for InSkeletalMesh */
	virtual void EvictSkeletalMeshThumbnailScene(USkeletalMesh* InSkeletalMesh) const = 0;
	...
	virtual void Tick(bool bBlocking) = 0;
	
	virtual int32 GetRemainingWork() const = 0;
	...
	virtual void AddPendingLoad(UCustomizableObject* Object) = 0;

	virtual void RegisterExternalOperation(const UScriptStruct* Operation) = 0;

	virtual void UnregisterExternalOperation(const UScriptStruct* Operation) = 0;
};
