...
void CustomizableObjectNullErrorMessage()
{
	UE_LOGF(LogMutable, Error,
		"Tried to perform actions on a CustomizableObjectInstance with no CustomizableObject set. Please set the CustomizableObject of the Instance before doing anything with it.");
}
...
void FCustomizableObjectInstanceDescriptor::SaveDescriptor(FArchive& Ar, bool bUseCompactDescriptor)
{
	...
	for (int32 ModelParameterIndex = 0; ModelParameterIndex < ModelParameterCount; ++ModelParameterIndex)
	{
		...
		case EMutableParameterType::Material:
		{
			...
			for (const FCustomizableObjectMaterialParameterValue& P : MaterialParameters)
			{
				if (P.ParameterName == Name)
				{
					Value = P.ParameterValue.GetPathName();
					...
					RangeValues.Empty(P.ParameterRangeValues.Num());	
					Algo::Transform(P.ParameterRangeValues, RangeValues, [](const TObjectPtr<UMaterialInterface>& Element)
					{
						return Element.GetPathName();
					});
		...
		{
			UE_LOGF(LogMutable, Warning, "Found unknown parameter [%ls] when saving a Customizable Object Instance Descriptor", *Name);
			break;
		}
...
void FCustomizableObjectInstanceDescriptor::LoadDescriptor(FArchive& Ar)
{
	...
	for (int32 ParameterIndex = 0; ParameterIndex < ModelParameterCount; ++ParameterIndex)
	{
		...
		case EMutableParameterType::Material:
		{
			...
			Ar << Value;
			Ar << RangeValues;
			for (FCustomizableObjectMaterialParameterValue& P : MaterialParameters)
			{
				if (P.ParameterName == Name)
				{
					P.ParameterValue = ToObject<UMaterialInterface>(Value);
					
					P.ParameterRangeValues.Empty(RangeValues.Num());
					Algo::Transform(RangeValues, P.ParameterRangeValues, [](const FString& Element)
					{
						return ToObject<UMaterialInterface>(Element);
					});
		...
		case EMutableParameterType::None:
		{
			UE_LOGF(LogMutable, Warning, "Found unknown parameter [%ls] when loading a Customizable Object Instance Descriptor", *Name);
			break;
		}
...
TSharedPtr<UE::Mutable::Private::FParameters> FCustomizableObjectInstanceDescriptor::GetParameters() const
{
	...
	TSharedPtr<UE::Mutable::Private::FParameters> MutableParameters = UE::Mutable::Private::FModel::NewParameters(CustomizableObject->GetPrivate()->GetModel(), nullptr);
	...
	for (int32 ParamIndex = 0; ParamIndex < ParamCount; ++ParamIndex)
	{
		...
		case UE::Mutable::Private::EParameterType::Color:
		{
			for (const FCustomizableObjectVectorParameterValue& VectorParameter : VectorParameters)
			{
				...
				{
					MutableParameters->SetColorValue(ParamIndex, VectorParameter.ParameterValue);
			...
			for (const FCustomizableObjectTextureParameterValue& TextureParameter : TextureParameters)
			{
				if (TextureParameter.ParameterName == Name || (Uid.IsValid() && TextureParameter.Id == Uid))
				{
					if (TSharedPtr<UE::Mutable::Private::FRangeIndex> RangeIdxPtr = MutableParameters->NewRangeIndex(ParamIndex))
					{
						...
						{
							RangeIdxPtr->SetPosition(0, RangeIndex);
							MutableParameters->SetTextureValue(ParamIndex, TextureParameter.ParameterRangeValues[RangeIndex], RangeIdxPtr.Get());
						}
					}
					else
					{
						MutableParameters->SetTextureValue(ParamIndex, TextureParameter.ParameterValue);
					}
			...
			for (const FCustomizableObjectSkeletalMeshParameterValue& MeshParameter : SkeletalMeshParameters)
			{
				if (MeshParameter.ParameterName == Name || (Uid.IsValid() && MeshParameter.Id == Uid))
				{
					if (TSharedPtr<UE::Mutable::Private::FRangeIndex> RangeIdxPtr = MutableParameters->NewRangeIndex(ParamIndex))
					{
						...
						{
							RangeIdxPtr->SetPosition(0, RangeIndex);
							MutableParameters->SetSkeletalMeshValue(ParamIndex, MeshParameter.ParameterRangeValues[RangeIndex], RangeIdxPtr.Get());
						}
					}
					else
					{
						MutableParameters->SetSkeletalMeshValue(ParamIndex, MeshParameter.ParameterValue);
					}
		...
		{
			for (const FCustomizableObjectExternalTypeParameterValue& ExternalTypeParameter : ExternalTypeParameters)
			{
				if (ExternalTypeParameter.ParameterName == Name || (Uid.IsValid() && ExternalTypeParameter.Id == Uid))
				{
					if (TSharedPtr<UE::Mutable::Private::FRangeIndex> RangeIdxPtr = MutableParameters->NewRangeIndex(ParamIndex))
					{
						for (int32 RangeIndex = 0; RangeIndex < ExternalTypeParameter.ParameterRangeValues.Num(); ++RangeIndex)
						{
							RangeIdxPtr->SetPosition(0, RangeIndex);
							MutableParameters->SetInstancedStructValue(ParamIndex, UE::Mutable::Private::MakeManaged<const FInstancedStruct>(ExternalTypeParameter.ParameterRangeValues[RangeIndex]), RangeIdxPtr.Get());
						}
					}
					else
					{
						MutableParameters->SetInstancedStructValue(ParamIndex, UE::Mutable::Private::MakeManaged<const FInstancedStruct>(ExternalTypeParameter.ParameterValue));
					}
...
void FCustomizableObjectInstanceDescriptor::ReloadParameters()
{
	...
	TArray<FCustomizableObjectExternalTypeParameterValue> OldExternalTypeParameters = ExternalTypeParameters;
	...
	if (!CustomizableObject->GetPrivate()->GetModel())
	{
		UE_LOGF(LogMutable, Warning, "[ReloadParametersFromObject] No model in object [%ls], generated empty parameters for [%ls] ", *CustomizableObject->GetName(), *CustomizableObject->GetName());
		return;
	}
	...
	{
		UE_LOGF(LogMutable, Warning, "[ReloadParametersFromObject] No model resources in object [%ls], generated empty parameters for [%ls] ", *CustomizableObject->GetName(), *CustomizableObject->GetName());
		return;
	}
	...
	for (int32 ParamIndex = 0; ParamIndex < ParamCount; ++ParamIndex)
	{
		...
		case UE::Mutable::Private::EParameterType::Color:
		{
			...
			{
				FVector4f V;
				MutableParameters->GetColorValue(ParamIndex, V);
			...
			else // Not found in Instance Parameters. Use Mutable Parameters.
			{
				if (TSharedPtr<UE::Mutable::Private::FRangeIndex> RangeIdxPtr = MutableParameters->NewRangeIndex(ParamIndex))
				{
					...
					{	
						...
						Param.ParameterRangeValues[RangeIndex] = MutableParameters->GetTextureValue(ParamIndex, RangeValueIdxPtr.Get());
					}
				}
				else
				{
					Param.ParameterValue = MutableParameters->GetTextureValue(ParamIndex);
				}
			}
			...
			else // Not found in Instance Parameters. Use Mutable Parameters.
			{
				if (TSharedPtr<UE::Mutable::Private::FRangeIndex> RangeIdxPtr = MutableParameters->NewRangeIndex(ParamIndex))
				{
					...
					for (int32 ValueIndex = 0; ValueIndex < ValueCount; ++ValueIndex)
					{
						...
						Param.ParameterRangeValues[RangeIndex] = MutableParameters->GetSkeletalMeshValue(ParamIndex, RangeValueIdxPtr.Get());
					}
				}
				else
				{
					Param.ParameterValue = MutableParameters->GetSkeletalMeshValue(ParamIndex);
				}
			}
		...
		case UE::Mutable::Private::EParameterType::InstancedStruct:
		{
			FCustomizableObjectExternalTypeParameterValue Param;
			Param.ParameterName = Name;
			Param.Id = Uid;

			auto FindByNameAndUid = [&](const FCustomizableObjectExternalTypeParameterValue& P)
				{
					return P.ParameterName == Name || (Uid.IsValid() && P.Id == Uid);
				};

			if (FCustomizableObjectExternalTypeParameterValue* Result = OldExternalTypeParameters.FindByPredicate(FindByNameAndUid))
			{
				if (TSharedPtr<UE::Mutable::Private::FRangeIndex> RangeIdxPtr = MutableParameters->NewRangeIndex(ParamIndex))
				{
					Param.ParameterRangeValues = Result->ParameterRangeValues;
				...
				if (TSharedPtr<UE::Mutable::Private::FRangeIndex> RangeIdxPtr = MutableParameters->NewRangeIndex(ParamIndex))
				{
					...
					for (int32 ValueIndex = 0; ValueIndex < ValueCount; ++ValueIndex)
					{
						...
						if (!Param.ParameterRangeValues.IsValidIndex(RangeIndex))
						{
							...
							for (int32 Index = PreviousNum; Index < Param.ParameterRangeValues.Num(); ++Index)
							{
								Param.ParameterRangeValues[Index] = FInstancedStruct();
							}
						}

						Param.ParameterRangeValues[RangeIndex] = *MutableParameters->GetInstancedStructValue(ParamIndex, RangeValueIdxPtr.Get());
					}
				}
				else
				{
					Param.ParameterValue = *MutableParameters->GetInstancedStructValue(ParamIndex);
				}
			}

			ExternalTypeParameters.Add(Param);
...
void LogParameterNotFoundWarning(const FString& ParameterName, const int32 ObjectParameterIndex, const int32 InstanceParameterIndex, const UCustomizableObject* CustomizableObject, char const* CallingFunction)
{
	UE_LOGF(LogMutable, Warning,
		"%s: Failed to find parameter (%ls) on CO (%ls). CO parameter index: (%d). COI parameter index: (%d)s",
...
FInstancedStruct FCustomizableObjectInstanceDescriptor::GetExternalTypeParameterSelectedOption(const FString& ExternalTypeParameterName, int32 RangeIndex) const
{
	if (!CustomizableObject)
	{
		CustomizableObjectNullErrorMessage();
	...
	const int32 ParameterIndexInObject = CustomizableObject->GetPrivate()->FindParameter(ExternalTypeParameterName);
	const int32 ExternalTypeParamIndex = FindTypedParameterIndex(ExternalTypeParameterName, EMutableParameterType::InstancedStruct);
 
	if (ParameterIndexInObject >= 0 && ExternalTypeParameters.IsValidIndex(ExternalTypeParamIndex))
	{
		// Due to optimizations a parameter may lose its multidimensionality (if it becomes constant). 
		// In that case it means there is only one possible option so it is ok to set it as if wasn't multidimensional.
		bool bIsMultidimensional = CustomizableObject->GetPrivate()->IsParameterMultidimensional(ParameterIndexInObject);
		if (!bIsMultidimensional || RangeIndex == INDEX_NONE)
		{
			return ExternalTypeParameters[ExternalTypeParamIndex].ParameterValue;
		}
		else
		{
			if (ExternalTypeParameters[ExternalTypeParamIndex].ParameterRangeValues.IsValidIndex(RangeIndex))
			{
				return ExternalTypeParameters[ExternalTypeParamIndex].ParameterRangeValues[RangeIndex];
			}
		}
	}
 
	LogParameterNotFoundWarning(ExternalTypeParameterName, ParameterIndexInObject, ExternalTypeParamIndex, CustomizableObject, __FUNCTION__);
	
	return FCustomizableObjectExternalTypeParameterValue::DEFAULT_PARAMETER_VALUE; 
}


void FCustomizableObjectInstanceDescriptor::SetExternalTypeParameterSelectedOption(const FString& ExternalTypeParamName, const FInstancedStruct& ExternalTypeValue, int32 RangeIndex)
{
	if (!CustomizableObject)
	{
		CustomizableObjectNullErrorMessage();
	...
	const int32 ParameterIndexInObject = CustomizableObject->GetPrivate()->FindParameter(ExternalTypeParamName);
	const int32 ParameterIndexInInstance = FindTypedParameterIndex(ExternalTypeParamName, EMutableParameterType::InstancedStruct);
 
	if (ParameterIndexInObject < 0 || ParameterIndexInInstance < 0)
	{
		// Warn and early out since we could not find the parameter to set.
		LogParameterNotFoundWarning(ExternalTypeParamName, ParameterIndexInObject, ParameterIndexInInstance, CustomizableObject, __FUNCTION__);
		return;
	}

	const UModelResources& ModelResources = CustomizableObject->GetPrivate()->GetModelResourcesChecked();
	if (!ExternalTypeValue.GetScriptStruct()->IsChildOf(ModelResources.ExternalTypeParameterDefaultValues[ExternalTypeParamName].GetScriptStruct()))
	{
		UE_LOGF(LogMutable, Error, "External Parameter %ls incorrect type.", *ExternalTypeParamName);
		return;
	}
	...
	if (!bIsMultidimensional || RangeIndex == INDEX_NONE)
	{
		ExternalTypeParameters[ParameterIndexInInstance].ParameterValue = ExternalTypeValue;
	}
	else
	{
		if (!ExternalTypeParameters[ParameterIndexInInstance].ParameterRangeValues.IsValidIndex(RangeIndex))
		{
			const int32 InsertionIndex = ExternalTypeParameters[ParameterIndexInInstance].ParameterRangeValues.Num();
			const int32 NumInsertedElements = RangeIndex + 1 - ExternalTypeParameters[ParameterIndexInInstance].ParameterRangeValues.Num();
			ExternalTypeParameters[ParameterIndexInInstance].ParameterRangeValues.InsertDefaulted(InsertionIndex, NumInsertedElements);
		}
 
		check(ExternalTypeParameters[ParameterIndexInInstance].ParameterRangeValues.IsValidIndex(RangeIndex));
		ExternalTypeParameters[ParameterIndexInInstance].ParameterRangeValues[RangeIndex] = ExternalTypeValue;
	}
}


FVector4f FCustomizableObjectInstanceDescriptor::GetVectorParameterSelectedOption(const FString& VectorParamName) const
{
	if (!CustomizableObject)
	{
		CustomizableObjectNullErrorMessage();
	...
	const int32 VectorParamIndex = FindTypedParameterIndex(VectorParamName, EMutableParameterType::Color);
 
	if (VectorParamIndex == INDEX_NONE)
	{
		LogParameterNotFoundWarning(VectorParamName, ParameterIndexInObject, VectorParamIndex, CustomizableObject, __FUNCTION__);
		return FCustomizableObjectVectorParameterValue::DEFAULT_PARAMETER_VALUE;
	}
 
	// TODO: Multidimensional parameter support
	return VectorParameters.IsValidIndex(VectorParamIndex) ? VectorParameters[VectorParamIndex].ParameterValue : FCustomizableObjectVectorParameterValue::DEFAULT_PARAMETER_VALUE;
}


void FCustomizableObjectInstanceDescriptor::SetVectorParameterSelectedOption(const FString& VectorParamName, const FVector4f& VectorValue)
{
	if (!CustomizableObject)
	{
		CustomizableObjectNullErrorMessage();
...
void FCustomizableObjectInstanceDescriptor::SetProjectorParameterSelectedOption(const FString& ProjectorParamName, const FCustomizableObjectProjector& ProjectorValue, const int32 RangeIndex)
{
	if (!CustomizableObject)
	{
		CustomizableObjectNullErrorMessage();
	...
	if (ProjectorValue.ProjectionType != ProjectorParameter.Value.ProjectionType)
	{
		UE_LOGF(LogMutable, Warning, "Changing the Projection type is not allowed once the CO has been compiled. In order to change it update the CO Projector Parameter settings and recompile the CO.");
	}
 
	// New Value
	FCustomizableObjectProjector ProjectorData = ProjectorValue;
...
{
	FCustomizableObjectProjector ProjectorData = GetProjectorParameterSelectedOption(ProjectorParamName, RangeIndex);
	...
	SetProjectorParameterSelectedOption(ProjectorParamName, ProjectorData, RangeIndex);
}
...
{
	const FCustomizableObjectProjector ProjectorData = GetProjectorParameterSelectedOption(ProjectorParamName, RangeIndex);
 
	OutPos = static_cast<FVector>(ProjectorData.Position);
	OutDirection = static_cast<FVector>(ProjectorData.Direction);
	OutUp = static_cast<FVector>(ProjectorData.Up);
	OutScale = static_cast<FVector>(ProjectorData.Scale);
...
{
	const FCustomizableObjectProjector ProjectorData = GetProjectorParameterSelectedOption(ParamName, RangeIndex);
 
	return static_cast<FVector>(ProjectorData.Position);
}
...
FVector FCustomizableObjectInstanceDescriptor::GetProjectorDirection(const FString& ParamName, const int32 RangeIndex) const
{
	...
	return static_cast<FVector>(ProjectorData.Direction);
}
...
FVector FCustomizableObjectInstanceDescriptor::GetProjectorUp(const FString& ParamName, const int32 RangeIndex) const
{
	...
	return static_cast<FVector>(ProjectorData.Up);
}
...
FVector FCustomizableObjectInstanceDescriptor::GetProjectorScale(const FString& ParamName, const int32 RangeIndex) const
{
	...
	return static_cast<FVector>(ProjectorData.Scale);
}
...
ECustomizableObjectProjectorType FCustomizableObjectInstanceDescriptor::GetProjectorParameterType(const FString& ParamName, const int32 RangeIndex) const
{
	...
	return ProjectorData.ProjectionType;
}
 
 
FCustomizableObjectProjector FCustomizableObjectInstanceDescriptor::GetProjectorParameterSelectedOption(const FString& ParamName, const int32 RangeIndex) const
{
	if (!CustomizableObject)
	{
		CustomizableObjectNullErrorMessage();
...
void FCustomizableObjectInstanceDescriptor::SetCurrentState(const FString& StateName)
{
	...
	{
		UE_LOGF(LogMutable, Error, "%hs: Unknown %ls state.", __FUNCTION__, *StateName);
	}
}
...
void FCustomizableObjectInstanceDescriptor::SetDefaultValue(int32 ParamIndex)
{
	...
	case EMutableParameterType::Color:
	{
		const FLinearColor DefaultValue = CustomizableObject->GetVectorParameterDefaultValue(ParamName);
	...
	case EMutableParameterType::InstancedStruct:
	{
		FInstancedStruct DefaultValue = CustomizableObject->GetExternalTypeParameterDefaultValue(ParamName);

		if (ensure(ExternalTypeParameters.IsValidIndex(TypedIndex)))
		{
			FCustomizableObjectExternalTypeParameterValue& ExternalTypeParameter = ExternalTypeParameters[TypedIndex];
			ExternalTypeParameter.ParameterValue = DefaultValue;
			ExternalTypeParameter.ParameterRangeValues.Empty();
		}
		break;
	}
