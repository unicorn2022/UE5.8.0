...
int32 UCOIBulkUpdateTestCommandlet::Main(const FString& Params)
{
	...
	FName InstancesPackagePath;
	{
		...
		{
			UE_LOGF(LogMutable,Error, "Failed to parse path where to find the Customizable Object Instances to update : %ls",*Params)
			return 1;
		}

		if (InstancesPackagePath.IsNone())
		{
			UE_LOGF(LogMutable,Error, "The path to scan can not be empty")
			return 1;
		}
	}

	UE_LOGF(LogMutable, Log, "(string) instance_search_path : %ls ", *InstancesPackagePath.ToString());
	...
	{
		UE_LOGF(LogMutable,Error, "Aborting Bulk Instance Update Test: No assets could be found at  the provided package path");
		return 1;
	}
	...
	for (const FAssetData& Data : FoundAssetData)
	{
		...
		if (!InstanceCO)
		{
			UE_LOGF(LogMutable,Error, "The instance %ls does not have a CO. This instance will not get tested.", *LoadedInstance->GetName());
			continue;
		}
	...
	UE_LOGF(LogMutable,Display, "Customizable Objects to compile : %u", TotalAmountOfCustomizableObjects);
	UE_LOGF(LogMutable, Log, "(int) customizable_objects_count : %u ", TotalAmountOfCustomizableObjects);

	const uint32 TotalAmountOfInstances = FoundAssetData.Num();
	UE_LOGF(LogMutable,Display, "Customizable Object Instances to update : %u", TotalAmountOfInstances);
	UE_LOGF(LogMutable, Log, "(int) customizable_object_instances_count : %u ", TotalAmountOfInstances);
	...
	for (TTuple<UCustomizableObject*, TArray<TStrongObjectPtr<UCustomizableObjectInstance>>>& MutableResourceTuple : MutableResources)
	{
		...
		UE_LOGF(LogMutable,Display, "The CO \"%ls\" has in total \"%u\" instances.", *CustomizableObjectName, COInstancesCount );
		UE_LOGF(LogMutable, Log, "(int) %ls_instance_count : %u ", *CustomizableObjectName, COInstancesCount);
		...
		for (const TStrongObjectPtr<UCustomizableObjectInstance>& Instance : MutableResourceTuple.Value)
		{
			...
			UE_LOGF(LogMutable, Log, "%u : \"%ls\"", InstanceIndex++, *CustomizableObjectInstanceName);
		}
	}
	...
	for ( TMap<UCustomizableObject*, TArray<TStrongObjectPtr<UCustomizableObjectInstance>>>::TIterator ResourcesIterator = MutableResources.CreateIterator(); ResourcesIterator; ++ResourcesIterator)
	{
		...
		{
			UE_LOGF(LogMutable, Error, "The CO %ls could not be compiled successfully. Skipping the update of all COIs that use it.", *CustomizableObjectName )
			continue;
		}
		...
		UE_LOGF(LogMutable, Display, "Starting update of the \"%u\" instances with CO : \"%ls\".", CustomizableObjectInstanceCount, *CustomizableObjectName);
		...
		while (!Instances.IsEmpty())
		{
			...
			UE_LOGF(LogMutable, Display, "\t( %u / %u ) Processing instance : \"%ls\" .",CurrentInstanceIndex++, TotalAmountOfInstances ,*Instance->GetName());
