...
int32 UCustomizableObjectBulkValidationCommandlet::Main(const FString& Params)
{
	...
	FName CustomizableObjectsSearchPath;
	{
		...
		{
			UE_LOGF(LogMutable, Error, "Failed to parse Customizable Object search path from the provided argument : %ls", *Params)
			return 1;
		}
	...
	TArray<FAssetData> FoundAssetData;
	{
		...
		{
			UE_LOGF(LogMutable, Error, "Aborting Bulk Customizable Object Validation Test: No assets could be found at the provided package path");
			return 1;
		}
		
		// Log all the Customizable Objects to be tested:
		UE_LOGF(LogMutable, Display, "Found a total of %u Customizable Objects to validate. Some may be discarded based on the test settings.", FoundAssetData.Num())
	...
	for (int32 AssetIndex = 0; AssetIndex < FoundAssetsCount; ++AssetIndex)
	{
		...
		if (!FoundObject)
		{
			UE_LOGF(LogMutable, Error, "Failed to load the asset with path : %ls .", *CustomizableObjectAssetData.GetSoftObjectPath().ToString())
			continue;
		}
		...
		UE_LOGF(LogMutable, Display, "Validated %u/%u assets.", AssetIndex + 1, FoundAssetsCount);
	}
