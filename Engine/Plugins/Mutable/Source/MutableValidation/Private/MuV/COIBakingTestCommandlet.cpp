...
void OnCustomizableObjectCompilationEnd(const FCompileCallbackParams& CompilationParameters)
{
	UE_LOGF(LogMutable,Display,
		"The Customizable Object finished the compilation with state : "
...
void OnInstanceUpdateEnd(const FUpdateContext& Result)
{
	...
	UE_LOGF(LogMutable,Display, "Instance finished update with state : %ls.", *UEnum::GetValueAsString(InstanceUpdateResult));
...
int32 UCOIBakingTestCommandlet::Main(const FString& Params)
{
	...
	// Look for the COI to be baked and load it
	{
		...
		{
			UE_LOGF(LogMutable,Error, "Failed to parse Customizable Object Instance package name from provided argument : %ls",*Params)
			return 1;
		}
		...
		if (!FoundObject)
		{
			UE_LOGF(LogMutable,Error, "Failed to retrieve UObject from path %ls",*CustomizableObjectInstanceAssetPath);
			return 1;
		}
		...
		if (!TargetInstance)
		{
			UE_LOGF(LogMutable,Error, "Failed to cast found UObject to UCustomizableObjectInstance.");
			return 1;
		}

		UE_LOGF(LogMutable,Display, "Successfully loaded %ls Customizable Object Instance!", *TargetInstance->GetName());
	}
	...
	{
		UE_LOGF(LogMutable,Error, "The instance %ls does not have a CO to compile : Exiting commandlet.", *TargetInstance->GetName());
		return 1;
	}
	...
	// Compile the Customizable object of the Instance
	{
		...
		UE_LOGF(LogMutable, Display, "Scheduling Customizable Object compilation.");
		ScheduleCOCompilationForBaking(*TargetInstance, CompilationEndDelegate, bPerformPartialCompilation);
		
		// Exit the wait process once the compilation finishes
		while (bIsCustomizableObjectBeingCompiled) 
		{ 
			// Tick the engine
		...
		{
			UE_LOGF(LogMutable, Error, "Failed to successfully compile the target COIs CO. Exiting commandlet.");
			return 1;
		}
	}
	...
	// Update the instance
	{
		...
		UE_LOGF(LogMutable, Display, "Scheduling instance update.");
		...
		{
			UE_LOGF(LogMutable, Error, "Failed to successfully update the target COI. Exiting commandlet.");
			return 1;
		}
	}
	...
	bool bWasBakingSuccessful = false;
	{
		...
		{
			UE_LOGF(LogMutable,Warning, "The directory with path  \" %ls \" does already exist. This may be produced by an incomplete execution of a previous test. Clearing it out before continuing...", *GlobalBakingDirectory);
			if (!IFileManager::Get().DeleteDirectory(*GlobalBakingDirectory,false,true))
			{
				UE_LOGF(LogMutable,Error, "Failed to delete the baking directory at path \" %ls \".", *GlobalBakingDirectory);
				return 1;
			}
		}
		...
		{
			UE_LOGF(LogMutable,Display, "Starting Instance Baking operation.");
			{
				...
				for (const TPair<UPackage*,const FResourceBakingData>& SavedPackagePair : SavedPackages)
				{
					...
					{
						const FString SaveTypeString = Enum->GetNameByIndex(static_cast<uint8>(SavedPackagePair.Value.SaveResolutionType)).ToString();
						UE_LOGF(LogMutable, Display, "Stored package \"%ls\" with \"%ls\" as save resolution type.", *SavedPackage->GetName(), *SaveTypeString)
					}
				}
			}
		...
		{
			UE_LOGF(LogMutable,Error, "Failed to create baking directory at path \" %ls \".", *GlobalBakingDirectory);
			return 1;
		}
	}
	...
	{
		UE_LOGF(LogMutable, Display, "Instance Baking operation has been completed successfully.");
		return 0;
	}
	else
	{
		UE_LOGF(LogMutable, Display, "Instance Baking operation has been completed with errors.");
		return 1;
	}
}
