...
namespace UE::Mutable::Private
{
	...
	* It supports concurrent addition of messages, but not retrieval.
	*/	
	class FErrorLog
	{
	public:
	...
	public:
		...
		//! Get the opaque contexts of a message.
		//! \param Index index of the message from 0 to GetMessageCount()-1
		UE_API const TArray<const void*, TInlineAllocator<4>>* GetMessageContexts(int32 Index) const;
		...
		UE_API void Add(const FString& Message, ErrorLogMessageType Type, TArray<const void*, TInlineAllocator<4>>& Contexts, ErrorLogMessageSpamBin SpamBin = ELMSB_ALL);
