...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpImageResizeLike::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());
		
		Result = HashCombineFast(Result, GetTypeHash(Source));
		Result = HashCombineFast(Result, GetTypeHash(SizeSource));
	...
	void ASTOpImageResizeLike::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			FOperation::ImageResizeLikeArgs Args;
	...
	bool ASTOpImageResizeLike::IsImagePlainConstant(FVector4f& OutColor) const
	{
		bool bResult = true;
