...
namespace UE::Mutable::Private
{
    ...
    public:
        ...
        bool Visit( const Ptr<ASTOp>& node ) override
        {
			...
			{
				recurse = false;
				const ASTOpImageMultiCompose* op = static_cast<const ASTOpImageMultiCompose*>(node.get());

				TArray<bool> NewState;
				NewState.Init(false, int32(EImageFormat::Count));
				RecurseWithState(op->Layout.child(), NewState);
				RecurseWithState(op->Base.child(), NewState);
				RecurseWithState(op->SourceLayout.child(), NewState);
				RecurseWithState(op->SourceImage.child(), NewState);
    ...
    public:
        ...
        bool Visit( const Ptr<ASTOp>& node ) override
        {
            ...
            case EOpType::SK_NEW:
            {
				...
				const ASTOpSkeletalMeshNew* Op = static_cast<const ASTOpSkeletalMeshNew*>(node.get());

				for (const ASTChild& LOD : Op->LODs)
				{
					RecurseWithState(LOD.child(), CurrentSemantics);
				}
            ...
            case EOpType::LD_NEW:
			{
				...
				const ASTOpLODNew* Op = static_cast<const ASTOpLODNew*>(node.get());

				for (const ASTChild& Mesh : Op->Meshes)
				{
					uint64 NewState = GetDefaultState();
					RecurseWithState(Mesh.child(), NewState);
				}
    ...
    void DataOptimise( const CompilerOptions* Options, ASTOpList& roots )
    {
        ...
        ASTOp::Traverse_BottomUp_Unique_NonReentrant( roots, [&](Ptr<ASTOp>& n)
        {
            if (n->GetOpType()==EOpType::IM_CONSTANT)
            {
				...
				TManagedPtr<const FImage> pOld = StaticCastManagedPtr<const FImage>(Typed->GetValue());
				...
				if (pOld->IsPlainColor(PlainColor))
				{
					// It is more efficient to just have an instruction for it instead, to avoid the overhead
					// of data loading. 
					// Warning This eliminates the mips. \TODO: Add support for mips in plaincolor instruction?
				...
				{
					TManagedPtr<FImage> pNew = ImOp.ImagePixelFormat( ImageCompressionQuality, pOld.Get(), EImageFormat::L_UBitRLE );
				...
				{
					TManagedPtr<FImage> pNew = ImOp.ImagePixelFormat( ImageCompressionQuality, pOld.Get(), EImageFormat::L_UByteRLE );
		...
		for (const TPair<Ptr<ASTOpConstantResource>, uint64>& Entry : MeshSemanticsAccumulator.RequiredSemanticsPerConstant)
		{
			...
			TManagedPtr<FMesh> Mesh = StaticCastManagedPtr<const FMesh>(Op->GetValue())->Clone();
