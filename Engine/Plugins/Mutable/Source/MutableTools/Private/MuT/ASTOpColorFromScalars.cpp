...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpColorFromScalars::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());
		
		Result = HashCombineFast(Result, GetTypeHash(V[0]));
		Result = HashCombineFast(Result, GetTypeHash(V[1]));
		Result = HashCombineFast(Result, GetTypeHash(V[2]));
		Result = HashCombineFast(Result, GetTypeHash(V[3]));
	...
	void ASTOpColorFromScalars::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			FOperation::ColorFromScalarsArgs Args;
