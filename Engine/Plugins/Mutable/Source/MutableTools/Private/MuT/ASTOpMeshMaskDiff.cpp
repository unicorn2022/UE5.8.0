...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpMeshMaskDiff::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());

		Result = HashCombineFast(Result, GetTypeHash(Source));
		Result = HashCombineFast(Result, GetTypeHash(Fragment));
	...
	void ASTOpMeshMaskDiff::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			FOperation::MeshMaskDiffArgs Args;
			...
			if (Fragment) 
			{
				Args.Fragment = Fragment->LinkedAddress;
			}
