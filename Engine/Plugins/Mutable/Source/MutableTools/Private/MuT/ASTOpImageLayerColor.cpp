...
namespace UE::Mutable::Private
{
	...
	//-------------------------------------------------------------------------------------------------
	uint32 ASTOpImageLayerColor::Hash() const
	{
	...
	Ptr<ASTOp> ASTOpImageLayerColor::OptimiseSemantic(const FModelOptimizationOptions& options, int32 Pass) const
	{
		...
		if (mask.child() && !(Flags & FOperation::ImageLayerArgs::F_USE_MASK_FROM_BLENDED) )
		{
			FVector4f Color;
			if (mask.child()->IsImagePlainConstant(Color))
			{
		...
		if (!at)
		{
			...
			if (!color || color.child()->GetOpType()==EOpType::CO_CONSTANT)
			{
				...
				if (!bRGBUnchanged)
				{
					if (Flags & FOperation::ImageLayerArgs::FLAGS::F_BASE_RGB_FROM_ALPHA)
					{
						switch (blendType)
						{
		...
		if (!at)
		{
			...
			if (base.child()->GetOpType() == EOpType::IM_SWIZZLE)
			{
				...
				{
					Ptr<ASTOpImageLayerColor> NewOp = UE::Mutable::Private::Clone<ASTOpImageLayerColor>(this);
					NewOp->Flags |= FOperation::ImageLayerArgs::FLAGS::F_BASE_RGB_FROM_ALPHA;
					NewOp->base = Source;
					at = NewOp;
				}
			}
...
//                aOp.Args.ImageLayerColor.base = Program.m_code[baseAt].Args.Conditional.yes;
//                op.args.Conditional.yes = Program.AddOp( aOp );

//                OP bOp = Program.m_code[at];
//                bOp.Args.ImageLayerColor.base = Program.m_code[baseAt].Args.Conditional.no;
