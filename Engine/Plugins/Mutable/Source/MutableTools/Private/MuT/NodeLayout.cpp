...
namespace UE::Mutable::Private
{
	...
	TManagedPtr<FLayout> NodeLayout::BuildRuntimeLayout(uint32 BlockIDPrefix) const
	{
		TManagedPtr<FLayout> GeneratedLayout = MakeManaged<FLayout>();
	...
	void NodeLayout::GenerateLayoutBlocks(const UE::Mutable::Private::TManagedPtr<FMesh>& Mesh, int32 LayoutIndex )
	{
		...
		TArray<bool> VerticesAlreadyClassified;
		{
			...
			TManagedPtr<FLayout> RuntimeLayout = BuildRuntimeLayout(0);
	...
	void NodeLayout::GenerateLayoutBlocksFromUVIslands(const UE::Mutable::Private::TManagedPtr<FMesh>& Mesh, int32 LayoutIndex)
	{
		if (!(Mesh && LayoutIndex >= 0 && Size.X > 0 && Size.Y > 0))
		{
			return;
		...
		for (int32 IslandBlockIndex = 0; IslandBlockIndex < IslandBlocks.Num(); ++IslandBlockIndex)
		{
			...
			int32 SizeX = 1024;
			int32 SizeY = 1024;
			TManagedPtr<FImage> Mask = MakeManaged<FImage>(SizeX, SizeY, 1, EImageFormat::L_UByte, EInitializationType::Black);
