...
//-------------------------------------------------------------------------------------------------
FOperation::ADDRESS ASTOp::FullLink( Ptr<ASTOp>& Root, FProgram& Program, FLinkerOptions* Options )
{
    MUTABLE_CPUPROFILER_SCOPE(AST_FullLink);

    auto Link = [&Program, Options](const Ptr<ASTOp>& Node)
    {
        Node->Link(Program, Options);
		check(!(GetAddressDataType(Node->LinkedAddress) == EDataType::None) || Node->LinkedAddress == 0);
    };

    auto Accept = [](const Ptr<const ASTOp>& Node)
    {
        return Node->LinkedAddress == 0;
    };

    Traverse_BottomUp_Unique(Root, Link, Accept); 

	FOperation::ADDRESS Result = Root->LinkedAddress;
...
void ASTOp::LogHistogram( ASTOpList& roots )
{
 ...
 //   UE_LOGF(LogMutableCore,Log, "Op histogram (%llu ops):", count);
 //   for(int i=0; i<8; ++i)
 //   {
 //       float p = Sorted[i].Key/float(count)*100.0f;
 //       int OpType = (int)Sorted[i].Value;
 //       UE_LOGF(LogMutableCore, Log, "  %5.2f%% : %ls", p, s_opNames[OpType] );
 //   }
	...
	//			// Log the op
	//			UE_LOGF(LogMutableCore, Log, "Multilayer at %x:", n.get());
	//			UE_LOGF(LogMutableCore, Log, "    base is type %ls:", s_opNames[(int)Base->GetOpType()]);
	//			if (Constant)
	//			{
	//				int Format = int( ((UE::Mutable::Private::FImage*)Constant->GetValue().get())->GetFormat() );
	//				UE_LOGF(LogMutableCore, Log, "    constant image format is %d:", Format);
	//			}
	//			else
	//			{
	//				FGetImageDescContext Context;
	//				FImageDesc Desc = Base->GetImageDesc(true, &Context);
	//				UE_LOGF(LogMutableCore, Log, "    estimated image format is %d:", int(Desc.m_format) );
	//			}
	//		}
	//		return true;
	//	});
...
bool ASTOp::IsColorConstant(FVector4f&) const
{
...
void ASTOp::LinkRange(FProgram& Program, const FRangeData& Range, FOperation::ADDRESS& RangeSize, uint16& RangeId)
{
	if (Range.rangeSize)
	{
