...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpImageResize::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());
		
		Result = HashCombineFast(Result, GetTypeHash(Source));
		Result = HashCombineFast(Result, GetTypeHash(Size));
	...
	bool ASTOpImageResize::IsImagePlainConstant(FVector4f& OutColor) const
	{
		bool bResult = true;
		...
		if (Source)
		{
			bResult = Source->IsImagePlainConstant(OutColor);
		}
	...
	Ptr<ASTOp> ASTOpImageResize::OptimiseSize() const
	{
		...
		{
			Ptr<ASTOpImageMultiCompose> NewOp = UE::Mutable::Private::Clone<ASTOpImageMultiCompose>(SourceAt);

			Ptr<ASTOpImageResize> BaseOp = UE::Mutable::Private::Clone<ASTOpImageResize>(this);
			BaseOp->Source = NewOp->Base.child();
			NewOp->Base = BaseOp;

			Ptr<ASTOpImageResize> SourceImageOp = UE::Mutable::Private::Clone<ASTOpImageResize>(this);
			SourceImageOp->Source = NewOp->SourceImage.child();
			NewOp->SourceImage = SourceImageOp;
