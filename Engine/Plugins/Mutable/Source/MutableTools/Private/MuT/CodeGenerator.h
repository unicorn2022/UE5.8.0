...
namespace UE::Mutable::Private
{
	...
	class NodeImageObject;
	class NodeSkeletalMeshObjectConvert;
	class NodeSkeletalMeshObjectSwitch;
	class NodeSkeletalMeshConvert;
	class NodeExternalSwitch;
	extern TAutoConsoleVariable<bool> CVarAllowMulticompose;

	class NodeImageConvert;
	class NodeExternalParameter;
	class NodeExternalOperation;
	class NodeSkeletalMeshMorph;
	class NodeSkeletalMeshReshape;
	class NodeSkeletalMeshModify;
	class NodeSkeletalMeshVariation;
	class NodeSkeletalMeshSwitch;
	class NodeMaterialExternal;
	class NodeScalarExternal;
	class NodeMaterialSkeletalMeshObjectBreak;
	class NodeImageExternal;
	class NodeMeshExternal;
	class ASTOpMaterialModify;
	class ASTOpParameter;
	class FLayout;
	class NodeColorArithmeticOperation;
	class NodeColorConstant;
	class NodeColorFromScalars;
	class NodeColorParameter;
	class NodeColorSwitch;
	class NodeColorTable;
	class NodeColorVariation;
	class NodeColorToSRGB;
	class NodeColorMaterialBreak;
	class NodeImageBinarise;
	class NodeImageColorMap;
	...
	class NodeMeshSkeletalMeshObjectBreak;
	class NodeRange;
	...
	class NodeSkeletalMeshObjectParameter;
	class NodeSkeletalMeshMerge;
	class NodeMatrix;
	class NodeMatrixConstant;
	class NodeMatrixParameter;
	class NodeMeshTransformWithBone;
	class NodeMeshTransformInMesh;
	struct FObjectState;
	struct FProgram;
    ...
    public:
    	...
    	void WaitTask(Tasks::FTask Task);
		...
		*/
		...
		struct FGenericGenerationOptions
		{
			...
			friend bool operator==(const FGenericGenerationOptions&, const FGenericGenerationOptions&) = default;
    	...
    	struct FSkeletalMeshGenerationOptions
    	{
    		FSkeletalMeshGenerationOptions(const FGenericGenerationOptions& BaseOptions, const NodeComponentNew* InComponent)
				: GenericOptions(BaseOptions)
		...
		{
			FLODGenerationOptions(const FGenericGenerationOptions& BaseOptions, int32 InLODIndex, const NodeComponentNew* InComponent, TConstArrayView<int32> InSurfaceIndices)
			{
				Component = InComponent;
				LODIndex = InLODIndex;
				SurfaceIndices = InSurfaceIndices;
				State = BaseOptions.State;
				ActiveTags = BaseOptions.ActiveTags;
			}
			...
			const NodeComponentNew* Component = nullptr;
			int32 LODIndex;
			TConstArrayView<int32> SurfaceIndices;
		};
    ...
    private:
		...
		*/
        ...
        /** */
		Ptr<NodeScalar> GenerateTableVariableNode(const Node&, const FTableCacheKey&, bool bAddNoneOption, const FString& DefaultRowName);
		...
		struct FImageGenerationOptions
		{
			...
			TManagedPtr<const FLayout> LayoutToApply;
			...
			{
				uint32 KeyHash = GetTypeHash((FGenericGenerationOptions&)InKey.GenericOptions);
		...
		void GenerateImage_ColorMap(const FImageGenerationOptions&, FImageGenerationResult&, const NodeImageColorMap*);
		...
		void GenerateImage_LayerColor(const FImageGenerationOptions&, FImageGenerationResult&, const NodeImageLayerColor*);
		void GenerateImage_Resize(const FImageGenerationOptions&, FImageGenerationResult&, const NodeImageResize*);
		void GenerateImage_PlainColor(const FImageGenerationOptions&, FImageGenerationResult&, const NodeImagePlainColor*);
		...
		void GenerateImage_Parameter(const FImageGenerationOptions&, FImageGenerationResult&, const NodeImageObjectParameter*);
		...
		void GenerateImage_External(const FImageGenerationOptions&, FImageGenerationResult&, const NodeImageExternal*);
		void GenerateImage_ParameterConvert(const FImageGenerationOptions&, FImageGenerationResult&, const NodeImageConvert*);

    	void GenerateCrop(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImage&);

		/** */
		TManagedPtr<FImage> GenerateMissingImage(EImageFormat);
		...
		/** */
		Ptr<ASTOp> GenerateImageMulticompose(Ptr<ASTOp> BaseOp, Ptr<ASTOp> LayoutOp, const FImageGenerationOptions& Options, const NodeImagePtrConst& NodeImage, TManagedPtr<const FLayout> SourceLayout, const CodeGenerator::FLayoutBlockDesc& InLayoutBlockDesc );

		/** Generate a layout block-sized image with a mask including all pixels in the blocks defined in the patch node. */
		TManagedPtr<FImage> GenerateImageBlockPatchMask(const NodeSurfaceModifierSurfaceEdit::FTexture&, FIntPoint GridSize, int32 BlockPixelsX, int32 BlockPixelsY, box<FIntVector2> RectInCells);

		/** Generate all the operations to apply the block patching on top of the BlockOp, and masking with PatchMask. */
		Ptr<ASTOp> GenerateImageBlockPatch(Ptr<ASTOp> BlockOp, const NodeSurfaceModifierSurfaceEdit::FTexture&, TManagedPtr<FImage> PatchMask, Ptr<ASTOp> ConditionOp, const FImageGenerationOptions&);
		...
		struct FMaterialGenerationOptions
		{
			FGenericGenerationOptions GenericOptions;
			...
			/** Array with all the tags for the modifiers. */
			TArray<FString> Tags;
			...
			friend bool operator==(const FMaterialGenerationOptions&, const FMaterialGenerationOptions&) = default;

			friend FORCEINLINE uint32 GetTypeHash(const FMaterialGenerationOptions& Options)
			{
				uint32 KeyHash = 0;

				// TODO: Use a more representative Hash.
				KeyHash = HashCombineFast(KeyHash, GetTypeHash(Options.GenericOptions));
		...
		struct FGeneratedMaterialCacheKey
		{
			Ptr<const Node> Node;
			FMaterialGenerationOptions Options;

			friend FORCEINLINE uint32 GetTypeHash(const FGeneratedMaterialCacheKey& InKey)
			{
				uint32 KeyHash = 0;
			...
			friend bool operator==(const FGeneratedMaterialCacheKey&, const FGeneratedMaterialCacheKey&) = default;
		};
    	
		typedef TMap<FGeneratedMaterialCacheKey, FMaterialGenerationResult> FGeneratedMaterialsMap;
		...
		void GenerateMaterial_External(const FMaterialGenerationOptions&, FMaterialGenerationResult& result, const NodeMaterialExternal* node);
    	void GenerateMaterial_BreakSkeletalMeshObject(const FMaterialGenerationOptions&, FMaterialGenerationResult& OutResult, const NodeMaterialSkeletalMeshObjectBreak* InNode);
    	void GenerateMaterial_BreakSkeletalMesh(const FMaterialGenerationOptions&, FMaterialGenerationResult& OutResult, const NodeMaterialSkeletalMeshBreak* InNode);
		void GenerateMaterial_Modify(const FMaterialGenerationOptions&, FMaterialGenerationResult& result, const NodeMaterialModify* node);
		...
		*/
		...
		struct FSharedSurfaceResultOptions
		{
			...
			*/
			...
			/** Store the generated layout and surface operations that we intend to share across LODs.
		...
		*/
		struct FGenerateLayoutConstantState
		{
			...
			TMap<FGeneratedLayoutKey, TManagedPtr<const FLayout>> GeneratedLayouts;
		};
        ...
        FMeshTask GenerateMesh_RemoveMesh(const FMeshGenerationStaticOptions&, FMeshOptionsTask, const NodeMeshRemoveMesh*);
		FMeshTask GenerateMesh_TransformWithBone(const FMeshGenerationStaticOptions&, FMeshOptionsTask, const NodeMeshTransformWithBone*);
    	FMeshTask GenerateMesh_TransformInMesh(const FMeshGenerationStaticOptions&, const FMeshOptionsTask, const NodeMeshTransformInMesh*);
		...
		FMeshTask GenerateMesh_SkeletalMeshBreak(const FMeshGenerationStaticOptions&, FMeshOptionsTask, const NodeMeshSkeletalMeshObjectBreak*);
    	FMeshTask GenerateMesh_External(const FMeshGenerationStaticOptions&, FMeshOptionsTask, const NodeMeshExternal*);
    	...
    	//!
    	TManagedPtr<const FLayout> GenerateLayout(Ptr<const NodeLayout> SourceLayout, uint32 MeshIDPrefix);

    	//-----------------------------------------------------------------------------------------
    	// Skeletal Mesh
    	
    	struct FSkeletalMeshGenerationResult
    	{
    		Ptr<ASTOp> SkeletalMeshOp;
    	};

    	typedef TMap<FGeneratedCacheKey, FSkeletalMeshGenerationResult> FGeneratedSkeletalMeshesMap;

    	struct FGeneratedSkeletalMeshes
    	{
    		UE::FMutex Mutex;
    		FGeneratedSkeletalMeshesMap Map;
    	};
    	FGeneratedSkeletalMeshes GeneratedSkeletalMeshes;
    	
    	void GenerateSkeletalMesh(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const Ptr<const NodeSkeletalMesh>&);
    	void GenerateSkeletalMesh_New(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshNew*);
    	void GenerateSkeletalMesh_Convert(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshConvert*);
		void GenerateSkeletalMesh_Merge(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshMerge*);
    	void GenerateSkeletalMesh_Switch(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshSwitch*);
    	void GenerateSkeletalMesh_Variation(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshVariation*);
		void GenerateSkeletalMesh_Morph(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshMorph*);
		void GenerateSkeletalMesh_Reshape(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshReshape*);
		void GenerateSkeletalMesh_Modify(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshModify*);
		void GenerateSkeletalMesh_Transform(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshTransform*);
		void GenerateSkeletalMesh_TransformWithBone(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshTransformWithBone*);
		void GenerateSkeletalMesh_ClipWithSkeletalMesh(FSkeletalMeshGenerationResult& OutResult, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshClipWithSkeletalMesh*);

    	//-----------------------------------------------------------------------------------------
    	// Skeletal Mesh Object
    	
    	struct FSkeletalMeshObjectGenerationResult
    	{
    		Ptr<ASTOp> SkeletalMeshOp;
    	};

    	typedef TMap<FGeneratedCacheKey, FSkeletalMeshObjectGenerationResult> FGeneratedSkeletalMeshObjectsMap;

    	struct FGeneratedSkeletalMeshObjects
    	{
    		UE::FMutex Mutex;
    		FGeneratedSkeletalMeshObjectsMap Map;
    	};
    	FGeneratedSkeletalMeshObjects GeneratedSkeletalMeshObjects;
    	void GenerateSkeletalMeshObject(FSkeletalMeshObjectGenerationResult& Result, const FSkeletalMeshGenerationOptions&, const Ptr<const NodeSkeletalMeshObject>&);
    	void GenerateSkeletalMeshObject_Convert(FSkeletalMeshObjectGenerationResult& Result, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshObjectConvert*);
    	void GenerateSkeletalMeshObject_Parameter(FSkeletalMeshObjectGenerationResult& Result, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshObjectParameter*);
    	void GenerateSkeletalMeshObject_Switch(FSkeletalMeshObjectGenerationResult& Result, const FSkeletalMeshGenerationOptions&, const NodeSkeletalMeshObjectSwitch*);

    	//-----------------------------------------------------------------------------------------
    	struct FExternalGenerationOptions 
    	{
    		// TODO Propagated options could be a TVariant
    		
    		// GenerateMesh
    		TOptional<FMeshGenerationStaticOptions> MeshStaticOptions;
    		TOptional<FMeshOptionsTask> MeshDynamicOptions;

    		// GenerateImage
    		TOptional<FImageGenerationOptions> ImageOptions;

    		// GenerateScalar
    		TOptional<FGenericGenerationOptions> ScalarOptions;
    		
    		// GenerateColor
    		TOptional<FGenericGenerationOptions> ColorOptions;

    		// GenerateMaterial
    		TOptional<FMaterialGenerationOptions> MaterialOptions;

    		bool operator==(const FExternalGenerationOptions& Other) const
    		{
    			return
    				//MeshStaticOptions == Other.MeshStaticOptions && // Meshes are not cached
    				//MeshDynamicOptions == Other.MeshDynamicOptions &&
   					ImageOptions == Other.ImageOptions &&
 					ScalarOptions == Other.ScalarOptions &&
 					ColorOptions == Other.ColorOptions &&
 					MaterialOptions == Other.MaterialOptions;
    		}

    		friend uint32 GetTypeHash(const FExternalGenerationOptions& InKey)
    		{
    			uint32 Hash = 0;

				// Hash = HashCombineFast(Hash, GetTypeHash(InKey.MeshStaticOptions)); // Meshes are not cached
				// Hash = HashCombineFast(Hash, GetTypeHash(InKey.MeshDynamicOptions));
				Hash = HashCombineFast(Hash, GetTypeHash(InKey.ImageOptions));
				Hash = HashCombineFast(Hash, GetTypeHash(InKey.ScalarOptions));
				Hash = HashCombineFast(Hash, GetTypeHash(InKey.MaterialOptions));
    			
    			return Hash;
    		}
    		
    		FGenericGenerationOptions GetGenericOptions() const
    		{
    			if (MeshStaticOptions.IsSet())
    			{
    				return static_cast<FGenericGenerationOptions>(*MeshStaticOptions);
    			}
    			
    			if (ImageOptions.IsSet())
    			{
    				return ImageOptions->GenericOptions;
    			}
    			
    			if (ScalarOptions.IsSet())
    			{
    				return *ScalarOptions;
    			}
    			
    			if (ColorOptions.IsSet())
    			{
    				return *ColorOptions;
    			}
    			
    			if (MaterialOptions.IsSet())
    			{
    				return MaterialOptions->GenericOptions;
    			}
    	...
    	struct FGeneratedExternalCacheKey
    	{
    		Ptr<const NodeExternal> Node;
    		FExternalGenerationOptions Options;
    		
    		friend uint32 GetTypeHash(const FGeneratedExternalCacheKey& InKey)
    		{
    			uint32 Hash = 0;

    			Hash = HashCombineFast(Hash, ::GetTypeHash(InKey.Node.get()));
    			Hash = HashCombineFast(Hash, GetTypeHash(InKey.Options));
    			
    			return Hash;
    		}

    		bool operator==(const FGeneratedExternalCacheKey& Other) const = default;
    	};
    	
    	typedef TMap<FGeneratedExternalCacheKey, FExternalTask> FGeneratedExternalMap;
    	struct FGeneratedExternal
    	{
    		UE::FMutex Mutex;
    		FGeneratedExternalMap Map;
    	};
    	FGeneratedExternal GeneratedExternal;

    	FExternalTask GenerateExternal(const FExternalGenerationOptions& Options, const Ptr<const NodeExternal>& ExternalNode);
    	FExternalTask GenerateExternal_Operation(const FExternalGenerationOptions& Options, const Ptr<const NodeExternalOperation>& ExternalNode);
    	FExternalTask GenerateExternal_Parameter(const FExternalGenerationOptions& Options, const Ptr<const NodeExternalParameter>& ExtensionNode);
    	FExternalTask GenerateExternal_Switch(const FExternalGenerationOptions& Options, const Ptr<const NodeExternalSwitch>& ExtensionNode);
    	Ptr<ASTOp> GenerateMissingExternalCode(const TCHAR* StrWhere, const void* ErrorContext);
		...
		void GenerateScalar_External(FScalarGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeScalarExternal>&);
		...
		void GenerateColor(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColor>&);
		void GenerateColor_Constant(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorConstant>&);
		void GenerateColor_Parameter(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorParameter>&);
		void GenerateColor_Switch(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorSwitch>&);
		void GenerateColor_SampleImage(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorSampleImage>&);
		void GenerateColor_FromScalars(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorFromScalars>&);
		void GenerateColor_Arithmetic(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorArithmeticOperation>&);
		void GenerateColor_Variation(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorVariation>&);
		void GenerateColor_ToSRGB(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorToSRGB>&);
		void GenerateColor_Table(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorTable>&);
		void GenerateColor_MaterialBreak(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorMaterialBreak>&);
		void GenerateColor_External(FColorGenerationResult&, const FGenericGenerationOptions&, const Ptr<const NodeColorExternal>&);
		Ptr<ASTOp> GenerateMissingColorCode(const TCHAR* strWhere, const void* errorContext);
		...
		struct FLayoutBlockDesc
		{
			...
			uint16 BlockPixelsX = 0;
			uint16 BlockPixelsY = 0;
		...
		bool HasImageBlockModifiers(const FParameterKey& ImageKey, const TArray<FirstPassGenerator::FModifier>&, int32 LODIndex) const;

		void GenerateMaterialImage(
