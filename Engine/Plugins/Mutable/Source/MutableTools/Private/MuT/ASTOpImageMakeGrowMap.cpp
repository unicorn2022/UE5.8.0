...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpImageMakeGrowMap::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());		

		Result = HashCombineFast(Result, GetTypeHash(Mask));
		Result = HashCombineFast(Result, GetTypeHash(Border));
	...
	void ASTOpImageMakeGrowMap::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			FOperation::ImageMakeGrowMapArgs Args;
	...
	bool ASTOpImageMakeGrowMap::IsImagePlainConstant(FVector4f& color) const
	{
		bool res = false;
		if (Mask.child())
		{
			Mask.child()->IsImagePlainConstant(color);
		}
		return res;
	}
