...
namespace UE::Mutable::Private
{
	...
	//-------------------------------------------------------------------------------------------------
	uint32 ASTOpConditional::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());

		Result = HashCombineFast(Result, GetTypeHash(condition));
		Result = HashCombineFast(Result, GetTypeHash(yes));
		Result = HashCombineFast(Result, GetTypeHash(no));
	...
	void ASTOpConditional::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			FOperation::ConditionalArgs Args;
			FMemory::Memzero(Args);

			if (condition) 
			{
				Args.condition = condition->LinkedAddress;
			}

			if (yes) 
			{
				Args.yes = yes->LinkedAddress;
			}

			if (no) 
			{
				Args.no = no->LinkedAddress;
			}
	...
	FImageDesc ASTOpConditional::GetImageDesc(bool returnBestOption, class FGetImageDescContext* context) const
	{
		...
		if (type == EOpType::IM_CONDITIONAL || type == EOpType::MI_CONDITIONAL)
		{
			...
			else
			{
				...
				res.FormatIfAlpha = (yesDesc.FormatIfAlpha == noDesc.FormatIfAlpha) ? yesDesc.FormatIfAlpha : EImageFormat::None;
