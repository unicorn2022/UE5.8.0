...
#include "ASTOpImageParameterConvert.h"
...
namespace UE::Mutable::Private
{
	...
	void CodeGenerator::GenerateImage(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImagePtrConst& Untyped)
	{
		...
		case Node::EType::ImageColorMap: GenerateImage_ColorMap(Options, Result, static_cast<const NodeImageColorMap*>(Node)); break;
		...
		case Node::EType::ImageLayerColor: GenerateImage_LayerColor(Options, Result, static_cast<const NodeImageLayerColor*>(Node)); break;
		case Node::EType::ImageResize: GenerateImage_Resize(Options, Result, static_cast<const NodeImageResize*>(Node)); break;
		case Node::EType::ImagePlainColor: GenerateImage_PlainColor(Options, Result, static_cast<const NodeImagePlainColor*>(Node)); break;
		...
		case Node::EType::ImageObjectParameter: GenerateImage_Parameter(Options, Result, static_cast<const NodeImageObjectParameter*>(Node)); break;
		...
		case Node::EType::ImageExternal: GenerateImage_External(Options, Result, static_cast<const NodeImageExternal*>(Node)); break;
		case Node::EType::ImageConvert: GenerateImage_ParameterConvert(Options, Result, static_cast<const NodeImageConvert*>(Node)); break;
		default: check(false);
		}
   ...
   void CodeGenerator::GenerateImage_Constant(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageConstant* InNode)
    {
		...
		// Image not suitable for multicompose operations
		Result.bCanMulticompose = false;

        // TODO: check duplicates
        TManagedPtr<const FImage> pImage = InNode->Image;
		...
		if (pImage->IsReference())
		{
			...
			ReferenceOp->ID = pImage->PassthroughObject.GetId();
			...
			// as well.
			ReferenceOp->ImageDesc = FImageDesc(pImage->GetSize(), EImageFormat::None, EImageFormat::None, pImage->GetLODCount());
	...
	void CodeGenerator::GenerateImage_Parameter(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageObjectParameter* Node )
    {
		...
		Ptr<ASTOpParameter>* Found = nullptr;
		{
			...
			if (!Found)
			{
				...
				check(bParseOk);
				op->Parameter.Type = EParameterType::Texture;
	...
	void CodeGenerator::GenerateImage_LayerColor(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageLayerColor* Node)
	{		
		MUTABLE_CPUPROFILER_SCOPE(NodeImageLayerColor);
        ...
        if (Node->Base)
        {
			...
			GenerateImage(Options, BaseResult, Node->Base);
			base = BaseResult.op;
			bCanMulticompose = BaseResult.bCanMulticompose;
        }
        else
        {
            // This argument is required
        ...
        if (Node->Mask)
        {
			...
			mask = MaskResult.op;
			bCanMulticompose = bCanMulticompose && MaskResult.bCanMulticompose;
        ...
        if (Node->Color )
        {
            ...
            GenerateColor(ColorResult, Options.GenericOptions, Node->Color);
			color = ColorResult.op;
        }
        else
        {
            // This argument is required
            color = GenerateMissingColorCode(TEXT("Layer color"), Node->GetMessageContext() );
        }
        op->color = color;

        Result.op = op;
		Result.bCanMulticompose = bCanMulticompose;
    }
	...
	void CodeGenerator::GenerateImage_MultiLayer(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageMultiLayer* Node)
	{
        ...
        {
            FRangeGenerationResult rangeResult;
            GenerateRange( rangeResult, Options.GenericOptions, Node->Range );
	...
	void CodeGenerator::GenerateImage_Transform(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageTransform* Node)
    {
		...
		if (Node->OffsetX)
		{
			...
			GenerateScalar(ScalarResult, Options.GenericOptions, Node->OffsetX);
		...
		{
			FScalarGenerationResult ScalarResult;
			GenerateScalar(ScalarResult, Options.GenericOptions, Node->OffsetY);
		...
		{
			FScalarGenerationResult ScalarResult;
			GenerateScalar(ScalarResult, Options.GenericOptions, Node->ScaleX);
		...
		{
			FScalarGenerationResult ScalarResult;
			GenerateScalar(ScalarResult, Options.GenericOptions, Node->ScaleY);
		...
		{
			FScalarGenerationResult ScalarResult;
			GenerateScalar(ScalarResult, Options.GenericOptions, Node->Rotation);
	...
	void CodeGenerator::GenerateImage_Interpolate(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageInterpolate* Node)
	{
        ...
        if ( NodeScalar* Factor = Node->Factor.get() )
        {
			...
			GenerateScalar(ParamResult, Options.GenericOptions, Factor);
	...
	void CodeGenerator::GenerateImage_Swizzle(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageSwizzle* Node)
	{
		...
		for (int32 SourceIndex = 0; SourceIndex < Node->Sources.Num(); ++SourceIndex)
		{
			if (Node->Sources[SourceIndex])
			{
				...
				bCanMulticompose = bCanMulticompose && BaseResult.bCanMulticompose;
				...
				const FImageDesc SourceImageDesc = Source->GetImageDesc();

				const bool bUseFormatWithAlpha = Node->SourceChannels[SourceIndex] == 3 && SourceImageDesc.FormatIfAlpha != EImageFormat::None;
				const EImageFormat SourceFormat = bUseFormatWithAlpha  ? SourceImageDesc.FormatIfAlpha : SourceImageDesc.m_format;
				const EImageFormat TargetFormat = GetUncompressedFormat(SourceFormat);

				if (TargetFormat != SourceFormat)
				{
					// Generate uncompressed format
					Ptr<ASTOpImagePixelFormat> FormatOp = new ASTOpImagePixelFormat();
					FormatOp->Source = Source;
					FormatOp->Format = TargetFormat;
					Source = FormatOp;
				}
	...
	void CodeGenerator::GenerateImage_ColorMap(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageColorMap* Node)
	{
		// Image not suitable for multicompose operations
		Result.bCanMulticompose = false;
        ...
        {
            // This argument is required
            base = GenerateMissingImageCode(TEXT("Colormap base image"), EImageFormat::RGB_UByte, Node->GetMessageContext(), Options);
        }
        base = GenerateImageSize( base, Options.RectSize);
        op->Base = base;
	...
	void CodeGenerator::GenerateImage_Binarise(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageBinarise* Node)
	{
        ...
        {
			FScalarGenerationResult ParamResult;
			GenerateScalar(ParamResult, Options.GenericOptions, ThresholdNode);
	...
	void CodeGenerator::GenerateImage_PlainColor(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImagePlainColor* Node)
	{
		// Source color
        Ptr<ASTOp> base;
        if (Node->Color )
        {
			FColorGenerationResult ColorResult;
            GenerateColor( ColorResult, Options.GenericOptions, Node->Color.get() );
			base = ColorResult.op;
        }
        else
        {
            // This argument is required
            base = GenerateMissingColorCode(TEXT("Image plain color base"), Node->GetMessageContext() );
        }
	...
	void CodeGenerator::GenerateImage_Conditional(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageConditional* Node)
	{
        ...
        if (Node->Parameter)
        {
			...
			GenerateBool(ParamResult, Options.GenericOptions, Node->Parameter.get());
        ...
        Result.op = op;
		Result.bCanMulticompose = NoResult.bCanMulticompose && YesResult.bCanMulticompose;
    }
	...
	void CodeGenerator::GenerateImage_Project(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageProject* Node)
	{
        ...
        if (Node->Projector )
        {
            GenerateProjector( projectorResult, Options.GenericOptions, Node->Projector );
        ...
        if (Node->Mesh )
        {
			...
			MeshStaticOptions.State = Options.GenericOptions.State;
			MeshStaticOptions.ActiveTags = Options.GenericOptions.ActiveTags;
			...
			TManagedPtr<const FLayout> Layout = MeshResult.GeneratedLayouts.IsValidIndex(Node->Layout) ? MeshResult.GeneratedLayouts[Node->Layout].Layout : nullptr;
            ...
            if (projectorResult.type == EProjectorType::Wrapping)
            {
				...
				TManagedPtr<FMesh> FormatMeshResult = MakeManaged<FMesh>();
				CreateMeshOptimisedForWrappingProjection(FormatMeshResult.Get(), Node->Layout);

                cop->SetValue(FormatMeshResult);

                Ptr<ASTOpMeshFormat> FormatOp = new ASTOpMeshFormat();
				FormatOp->Flags = 
					FOperation::MeshFormatArgs::Vertex | 
					FOperation::MeshFormatArgs::Index  | 
					FOperation::MeshFormatArgs::ResetBufferIndices;
            ...
            else
            {
                ...
                Ptr<ASTOpMeshFormat> FormatOp = new ASTOpMeshFormat();
				FormatOp->Flags = FOperation::MeshFormatArgs::Vertex
					| FOperation::MeshFormatArgs::Index
					| FOperation::MeshFormatArgs::ResetBufferIndices;
				FormatOp->Format = cop;
				FormatOp->Source = CurrentMeshToProjectOp;
        ...
        {
            // This argument is required
            TManagedPtr<const FMesh> TempMesh = MakeManaged<FMesh>();
            Ptr<ASTOpConstantResource> cop = new ASTOpConstantResource();
            cop->Type = EOpType::ME_CONSTANT;
            cop->SetValue(TempMesh);
			ProjectOp->Mesh = cop;
		...
		// Fading angles are optional, and stored in a color. If one exists, we generate both.
		if (Node->AngleFadeStart || Node->AngleFadeEnd)
		{
			...
			Ptr<NodeColorFromScalars> pPropsNode = new NodeColorFromScalars();
			...
			GenerateColor(ParamResult, Options.GenericOptions, pPropsNode);
	...
	void CodeGenerator::GenerateImage_Mipmap(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageMipmap* Node)
	{
        ...
        int32 blockX = 0;
        int32 blockY = 0;
        if (Options.ImageLayoutStrategy != CompilerOptions::TextureLayoutStrategy::None)
        {
	...
	void CodeGenerator::GenerateImage_Variation(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageVariation* Node)
	{
        ...
        for ( int32 t = int32(Node->Variations.Num() ) - 1; t >= 0; --t )
        {
            ...
            if (Node->Variations[t].Image )
            {
				...
				bCanMulticompose = bCanMulticompose && VariationResult.bCanMulticompose;
            }
            else
            {
                // This argument is required
	...
	void CodeGenerator::GenerateImage_Table(const FImageGenerationOptions& Options, FImageGenerationResult& Result, const NodeImageTable* InNode)
	{
		...
		Result.op = GenerateTableSwitch<NodeImageTable, ETableColumnType::Image, EOpType::IM_SWITCH>(node,
			[this, InNode, Options, &bCanMulticompose](const NodeImageTable& node, int32 colIndex, int32 row, FErrorLog* LocalErrorLog)
			{
				const FTableValue& CellData = node.Table->GetPrivate()->Rows[row].Values[colIndex];
				TManagedPtr<FImage> pImage = CellData.Image;
				...
				else
				{
					...
					ImageOp = Result.op;
					bCanMulticompose = bCanMulticompose && Result.bCanMulticompose;
	...
	void CodeGenerator::GenerateImage_MaterialBreak(const FImageGenerationOptions& InOptions, FImageGenerationResult& Result, const NodeImageMaterialBreak* Node)
	{
		...
		// Set the parameter key that this break node will generate
		MaterialOp->ParameterKey = Node->ParameterKey;
		...
		MaterialSourceOptions.GenericOptions.State = InOptions.GenericOptions.State;
	...
	void CodeGenerator::GenerateImage_FromMaterialParameter(const FImageGenerationOptions& InOptions, FImageGenerationResult& Result, const NodeImageFromMaterialParameter* InNode)
	{
		...
		ImageOp->ParameterKey = InNode->ImageParameterKey;

		Result.op = ImageOp;
	}


	void CodeGenerator::GenerateImage_External(const FImageGenerationOptions& InOptions, FImageGenerationResult& Result, const NodeImageExternal* InNode)
	{
		FExternalGenerationOptions Options;
		Options.ImageOptions = InOptions;
	...
	void CodeGenerator::GenerateImage_ParameterConvert(const FImageGenerationOptions& InOptions, FImageGenerationResult& Result, const NodeImageConvert* InNode)
	{
		Ptr<ASTOpImageParameterConvert> ReferenceConvert = new ASTOpImageParameterConvert();

		FImageGenerationResult PassthroughImageResult;
		GenerateImage(InOptions, PassthroughImageResult, InNode->ImageParameter);

		ReferenceConvert->ImageParameter = PassthroughImageResult.op;
		ReferenceConvert->ImageDescriptor = InNode->ImageDescriptor;
		
		Result.op = ReferenceConvert;
		Result.Parameter = PassthroughImageResult.Parameter;
	}


	TManagedPtr<FImage> CodeGenerator::GenerateMissingImage(EImageFormat Format)
	{
		...
		TManagedPtr<FImage> GeneratedImage = MakeManaged<FImage>(Size[0], Size[1], 1, Format, EInitializationType::NotInitialized);
	...
	Ptr<ASTOp> CodeGenerator::GenerateMissingImageCode(const TCHAR* strWhere, EImageFormat format, const void* ErrorContext, const FImageGenerationOptions& Options )
	{
		...
		// Make a checkered debug image
		TManagedPtr<FImage> GeneratedImage = GenerateMissingImage( format );

		Ptr<NodeImageConstant> pNode = new NodeImageConstant();
		pNode->Image = GeneratedImage;
    ...
    {
		Ptr<NodeColorConstant> ConstantColor = new NodeColorConstant();
		ConstantColor->Value = InColor;

        Ptr<NodeImagePlainColor> PlainNode = new NodeImagePlainColor();
		PlainNode->Color = ConstantColor;
	...
	Ptr<ASTOp> CodeGenerator::GenerateImageMulticompose(
			Ptr<ASTOp> InBaseImageOp, 
			Ptr<ASTOp> InLayoutOp, 
			const FImageGenerationOptions& InBaseImageOptions,
			const NodeImagePtrConst& NodeImage, 
			TManagedPtr<const FLayout> SourceLayout, 
			const CodeGenerator::FLayoutBlockDesc& InLayoutBlockDesc)
	{
		Ptr<ASTOp> Result = InBaseImageOp;

		// Single block layout to generate the entire SourceImage
		TManagedPtr<FLayout> AuxLayout = SourceLayout->Clone();
		AuxLayout->Blocks.SetNum(1);
		AuxLayout->Blocks[0].Min = FIntVector2::ZeroValue;
		AuxLayout->Blocks[0].Size = AuxLayout->Size;

		// Generate the SourceImage
		FImageGenerationOptions ImageOptions = InBaseImageOptions;
		ImageOptions.LayoutToApply = AuxLayout;
		ImageOptions.LayoutBlockId = AuxLayout->Blocks[0].Id;
		FImageGenerationResult ImageResult;
		GenerateImage(ImageOptions, ImageResult, NodeImage);

		Ptr<ASTOp> SourceImage = ImageResult.op;
		if (SourceImage)
		{
			// SourceImage Layout
			Ptr<ASTOpConstantResource> SourceLayoutOp = new ASTOpConstantResource();
			SourceLayoutOp->Type = EOpType::LA_CONSTANT;
			SourceLayoutOp->SetValue(SourceLayout);

			// Compose layout operation
			Ptr<ASTOpImageMultiCompose> MultiComposeOp = new ASTOpImageMultiCompose();
			MultiComposeOp->Layout = InLayoutOp;
			MultiComposeOp->Base = InBaseImageOp;
			MultiComposeOp->SourceLayout = SourceLayoutOp;
			MultiComposeOp->SourceImage = SourceImage;

			MultiComposeOp->LayoutBlockSizeInPixelsX = InLayoutBlockDesc.BlockPixelsX;
			MultiComposeOp->LayoutBlockSizeInPixelsY = InLayoutBlockDesc.BlockPixelsY;

			FImageDesc SourceDesc = SourceImage->GetImageDesc(true);
			MultiComposeOp->SourceSizeX = SourceDesc.m_size.X;
			MultiComposeOp->SourceSizeY = SourceDesc.m_size.Y;
			
			Result = MultiComposeOp;
		}
