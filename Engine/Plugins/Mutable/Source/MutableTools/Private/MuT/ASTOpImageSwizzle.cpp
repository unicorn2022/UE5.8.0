...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpImageSwizzle::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());

		Result = HashCombineFast(Result, GetTypeHash(Sources[0]));
		Result = HashCombineFast(Result, GetTypeHash(Sources[1]));
		Result = HashCombineFast(Result, GetTypeHash(Sources[2]));
		Result = HashCombineFast(Result, GetTypeHash(Sources[3]));
		Result = HashCombineFast(Result, GetTypeHash(SourceChannels[0]));
		Result = HashCombineFast(Result, GetTypeHash(SourceChannels[1]));
		Result = HashCombineFast(Result, GetTypeHash(SourceChannels[2]));
		Result = HashCombineFast(Result, GetTypeHash(SourceChannels[3]));
	...
	void ASTOpImageSwizzle::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			...
			for (int32 i = 0; i < MUTABLE_OP_MAX_SWIZZLE_CHANNELS; ++i)
			{
				if (Sources[i])
				{
					Args.sources[i] = Sources[i]->LinkedAddress;
				}
	...
	UE::Mutable::Private::Ptr<ASTOp> ASTOpImageSwizzle::OptimiseSink(const FModelOptimizationOptions& options, FOptimizeSinkContext& context) const
	{
		...
		if (!at && bAllChannelsAreTheSameType)
		{
				...
				)
			{
				...
				if (bIsSpecialMultiLayer)
				{
					...
					{
						// Additional optimization is possible here.
						NewLayer->Flags |= FOperation::ImageLayerArgs::FLAGS::F_USE_MASK_FROM_BLENDED;
			...
			//			check(Typed);
			//			// Compare all Args but the source image
			//			FOperation::ImageResizeArgs ArgCopy = FirstResize->op.args.ImageResize;
			//			ArgCopy.source = Typed->op.args.ImageResize.source;

			//			if (FMemory::Memcmp(&ArgCopy, &Typed->op.args.ImageResize, sizeof(FOperation::ImageResizeArgs)) != 0)
			//			{
			...
			// Swizzle down plaincolors.
			if (!at && sourceType == EOpType::IM_PLAINCOLOR)
			{
		...
		// TODO \warning: probably wrong because it doesn't check if the layer color is doing a separated alpha operation.
		// Swizzle of RGB from a source + A from a layer color
		// This can be optimized to apply the layer color on-base directly to the alpha channel to skip the swizzle
		//if ( !at 
		//	&&
		//	Sources[0] && Sources[0]==Sources[1] && Sources[0]==Sources[2]
		//	&&
		//	Sources[3] && Sources[3]->GetOpType()==EOpType::IM_LAYERCOLOR
		//	)
		//{
		//	// Move the swizzle down all the paths
		//	Ptr<ASTOpImageLayerColor> NewLayerColor = UE::Mutable::Private::Clone<ASTOpImageLayerColor>(Sources[3].child());

		//	Ptr<ASTOpImageSwizzle> NewSwizzle = UE::Mutable::Private::Clone<ASTOpImageSwizzle>(this);
		//	NewSwizzle->Sources[3] = NewLayerColor->base.child();

		//	NewLayerColor->blendTypeAlpha = NewLayerColor->blendType;
		//	NewLayerColor->BlendAlphaSourceChannel = SourceChannels[3];
		//	NewLayerColor->blendType = EBlendType::BT_NONE;
		//	NewLayerColor->base = NewSwizzle;
		...
		// \TODO: wrong: the new layer color will always use the alpha from the color, instead of the channel that the swizzle is selecting.
		...
		// Swizzle of RGB from a layer color + A from a different source
		// This can be optimized to apply the layer color on-base directly to the rgb channel to skip the swizzle
		if (!at
			&&
			Sources[0] 
			&& 
			...
			&&
			Sources[0]->GetOpType() == EOpType::IM_LAYERCOLOR
			&& 
			!(Sources[3]==Sources[0]) )
		{
			// Move the swizzle down all the rgb path
			Ptr<ASTOpImageLayerColor> NewLayerColor = UE::Mutable::Private::Clone<ASTOpImageLayerColor>(Sources[0].child());
			check(NewLayerColor);

			Ptr<ASTOpImageSwizzle> NewSwizzle = UE::Mutable::Private::Clone<ASTOpImageSwizzle>(this);
			NewSwizzle->Sources[0] = NewLayerColor->base.child();
			NewSwizzle->Sources[1] = Sources[1] ? NewLayerColor->base.child() : nullptr;
			NewSwizzle->Sources[2] = Sources[2] ? NewLayerColor->base.child() : nullptr;

			NewLayerColor->blendTypeAlpha = EBlendType::BT_NONE;
			NewLayerColor->base = NewSwizzle;
			...
			)
		{
			...
			if ( OldLayer->Flags==FOperation::ImageLayerArgs::FLAGS::F_BLENDED_RGB_FROM_ALPHA
				&&
	...
	bool ASTOpImageSwizzle::IsImagePlainConstant(FVector4f& color) const
	{
	...
	UE::Mutable::Private::Ptr<ASTOp> Sink_ImageSwizzleAST::Visit(UE::Mutable::Private::Ptr<ASTOp> at, const ASTOpImageSwizzle* CurrentSwizzleOp)
	{
		...
		{
			Ptr<ASTOpImageMultiCompose> newOp = UE::Mutable::Private::Clone<ASTOpImageMultiCompose>(at);
			newOp->Base = Visit(newOp->Base.child(), CurrentSwizzleOp);
			newOp->SourceImage = Visit(newOp->SourceImage.child(), CurrentSwizzleOp);
			newAt = newOp;
			break;
		}
		...
		case EOpType::IM_LAYERCOLOR:
		{
			...
			// We need to swizzle the color too
