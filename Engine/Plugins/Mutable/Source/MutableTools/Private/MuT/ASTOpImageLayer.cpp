...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpImageLayer::Hash() const
	{
	...
	Ptr<ASTOp> ASTOpImageLayer::OptimiseSemantic(const FModelOptimizationOptions& options, int32 Pass) const
	{
		...
		if (!at && blendAt && blendAt->GetOpType() == EOpType::IM_PLAINCOLOR)
		{
		...
		{
			FVector4f color;
			if (maskAt->IsImagePlainConstant(color))
			{
				// For masks we only use one channel
				if (FMath::IsNearlyZero(color[0]))
				{
					// If the mask is black, we can skip the entire operation
					at = base.child();
				}
				else if (FMath::IsNearlyEqual(color[0], 1, UE_SMALL_NUMBER))
				{
			...
			)
		{
			...
			if (CurrentMask && CurrentMask->GetOpType() == EOpType::IM_SWIZZLE)
			{
					...
					)
				{
					...
					NewLayer->Flags |= FOperation::ImageLayerArgs::F_USE_MASK_FROM_BLENDED;
					at = NewLayer;
				}
			}
		}
		...
		if (!at)
		{
			...
			if (Pass>0 && !at && blendAt && blendAt->GetOpType() == EOpType::IM_SWIZZLE && Flags==0)
			{
				...
				{
					const FImageDesc SourceImageDesc = TypedBlend->Sources[0]->GetImageDesc(true);
					if (ensure(SourceImageDesc.m_format == EImageFormat::RGBA_UByte))
					{
						Ptr<ASTOpImageLayer> nop = UE::Mutable::Private::Clone<ASTOpImageLayer>(this);
						nop->Flags = Flags | FOperation::ImageLayerArgs::FLAGS::F_BLENDED_RGB_FROM_ALPHA;
	...
	Ptr<ASTOp> ASTOpImageLayer::OptimiseSink(const FModelOptimizationOptions& options, FOptimizeSinkContext& context) const
	{
		...
		if (!at && blendAt)
		{
			...
			case EOpType::IM_SWITCH:
			{
				...
				// If at least a switch option is a plain color, sink the layer into the switch
				...
				for (int32 v = 0; v < BlendSwitch->Cases.Num(); ++v)
				{
					if (BlendSwitch->Cases[v].Branch)
					{
						...
						if (BlendSwitch->Cases[v].Branch->GetOpType() == EOpType::IM_PLAINCOLOR)
						{
							bWorthSinking = true;
							break;
						}
		...
		if (!at && maskAt)
		{
			...
			case EOpType::IM_SWITCH:
			{
				...
				for (int32 v = 0; v < MaskSwitch->Cases.Num(); ++v)
				{
					...
					{
						// \TODO: Use the smarter query function to detect plain images?
						if (MaskSwitch->Cases[v].Branch->GetOpType() == EOpType::IM_PLAINCOLOR)
						{
							bWorthSinking = true;
							break;
						}
