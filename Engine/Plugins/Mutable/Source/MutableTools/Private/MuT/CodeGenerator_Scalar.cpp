...
namespace UE::Mutable::Private
{
	...
	void CodeGenerator::GenerateScalar(FScalarGenerationResult& result, const FGenericGenerationOptions& Options, const Ptr<const NodeScalar>& Untyped)
	{
		...
		else if (Untyped->GetType() == NodeScalarExternal::GetStaticType())
		{
			const NodeScalarExternal* Node = static_cast<const NodeScalarExternal*>(Untyped.get());
			GenerateScalar_External(result, Options, Node);
		}
		else
		{
			check(false);
	...
	void CodeGenerator::GenerateScalar_Arithmetic(FScalarGenerationResult& result, const FGenericGenerationOptions& Options, const Ptr<const NodeScalarArithmeticOperation>& Node)
	{
		...
		{
		case NodeScalarArithmeticOperation::AO_ADD: op->Operation = FOperation::ArithmeticArgs::ADD; break;
		case NodeScalarArithmeticOperation::AO_SUBTRACT: op->Operation = FOperation::ArithmeticArgs::SUBTRACT; break;
		case NodeScalarArithmeticOperation::AO_MULTIPLY: op->Operation = FOperation::ArithmeticArgs::MULTIPLY; break;
		case NodeScalarArithmeticOperation::AO_DIVIDE: op->Operation = FOperation::ArithmeticArgs::DIVIDE; break;
		default:
	...
	void CodeGenerator::GenerateScalar_External(FScalarGenerationResult& Result, const FGenericGenerationOptions& InOptions, const Ptr<const NodeScalarExternal>& InNode)
	{
		FExternalGenerationOptions Options;
		Options.ScalarOptions = InOptions;
