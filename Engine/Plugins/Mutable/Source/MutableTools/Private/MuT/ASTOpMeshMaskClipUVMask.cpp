...
namespace UE::Mutable::Private
{
	...
	//-------------------------------------------------------------------------------------------------
	uint32 ASTOpMeshMaskClipUVMask::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());
		
		Result = HashCombineFast(Result, GetTypeHash(Source));
		Result = HashCombineFast(Result, GetTypeHash(UVSource));
		Result = HashCombineFast(Result, GetTypeHash(MaskImage));
		Result = HashCombineFast(Result, GetTypeHash(MaskLayout));
	...
	void ASTOpMeshMaskClipUVMask::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			FOperation::MeshMaskClipUVMaskArgs Args;
			...
			if (UVSource) 
			{
				Args.UVSource = UVSource->LinkedAddress;
			}

			if (MaskImage) 
			{
				Args.MaskImage = MaskImage->LinkedAddress;
			}

			if (MaskLayout) 
			{
				Args.MaskLayout = MaskLayout->LinkedAddress;
			}
