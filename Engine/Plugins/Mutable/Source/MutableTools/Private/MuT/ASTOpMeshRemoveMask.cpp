...
namespace UE::Mutable::Private
{
	...
	//---------------------------------------------------------------------------------------------
	uint32 ASTOpMeshRemoveMask::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());

		Result = HashCombineFast(Result, GetTypeHash(source));
		
		for (const TPair<ASTChild, ASTChild>& Remove : removes)
		{
			Result = HashCombineFast(Result, GetTypeHash(Remove.Key));
			Result = HashCombineFast(Result, GetTypeHash(Remove.Value));
		}
	...
	void ASTOpMeshRemoveMask::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			...
			{
				FOperation::ADDRESS condition = b.Key ? b.Key->LinkedAddress : 0;
				AppendCode(Program.ByteCode, condition);

				FOperation::ADDRESS remove = b.Value ? b.Value->LinkedAddress : 0;
