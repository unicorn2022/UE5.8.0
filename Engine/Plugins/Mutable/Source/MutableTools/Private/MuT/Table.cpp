...
namespace UE::Mutable::Private
{
    ...
    void FTable::SetCell(int32 Column, uint32 RowId, const UE::Mutable::Private::TManagedPtr<FImage>& Value, const void* ErrorContext)
	{
		int32 Row = m_pD->FindRow(RowId);
		check( Row>=0 );
		check( Column < m_pD->Columns.Num() );
		...
		m_pD->Rows[ Row ].Values[Column].Image = Value;
		m_pD->Rows[ Row ].Values[Column].ErrorContext = ErrorContext;
	}


    void FTable::SetCell( int32 Column, uint32 RowId, const UE::Mutable::Private::TManagedPtr<FMesh>& Value, const void* ErrorContext )
	{
		int32 Row = m_pD->FindRow(RowId);
		check( Row>=0 );
		check( Column < m_pD->Columns.Num() );
