...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpImageMipmap::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());
		
		Result = HashCombineFast(Result, GetTypeHash(Source));
		Result = HashCombineFast(Result, Levels);
	...
	UE::Mutable::Private::Ptr<ASTOp> ASTOpImageMipmap::OptimiseSink(const FModelOptimizationOptions&, FOptimizeSinkContext& context) const
	{
		...
		{
			// Set the mipmap generation on the plaincolor operation
	...
	FImageDesc ASTOpImageMipmap::GetImageDesc(bool returnBestOption, FGetImageDescContext* Context) const
	{
		...
		if (Source.child())
		{
			Result = Source.child()->GetImageDesc(returnBestOption, Context);
		}
		...
		{
			mipLevels = FMath::Max(
				(int)ceilf(logf((float)Result.m_size[0]) / logf(2.0f)),
				(int)ceilf(logf((float)Result.m_size[1]) / logf(2.0f))
			);
		}

		Result.m_lods = FMath::Max(Result.m_lods, (uint8)mipLevels);
	...
	bool ASTOpImageMipmap::IsImagePlainConstant(FVector4f& color) const
	{
		bool res = false;
		if (Source.child())
		{
	...
	UE::Mutable::Private::Ptr<ASTOp> Sink_ImageMipmapAST::Visit(UE::Mutable::Private::Ptr<ASTOp> at, const ASTOpImageMipmap* currentMipmapOp)
	{
		...
		{
			const ASTOpImageMultiCompose* typedAt = static_cast<const ASTOpImageMultiCompose*>(at.get());
			if (!currentMipmapOp->bOnlyTail
				//&&
				// Don't move the mipmapping if we are composing with a mask.
				// TODO: allow mipmapping in the masks, RLE formats, etc.
				...
				)
			{
				...
				NewOp->SourceImage = Visit(SourceImageOp, currentMipmapOp);
