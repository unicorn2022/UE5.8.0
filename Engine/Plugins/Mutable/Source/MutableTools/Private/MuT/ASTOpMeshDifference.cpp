...
namespace UE::Mutable::Private
{
	...
	//---------------------------------------------------------------------------------------------
	uint32 ASTOpMeshDifference::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());

		Result = HashCombineFast(Result, GetTypeHash(Base));
		Result = HashCombineFast(Result, GetTypeHash(Target));
	...
	void ASTOpMeshDifference::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			...
			FOperation::ADDRESS BaseAt = Base ? Base->LinkedAddress : 0;
			AppendCode(Program.ByteCode, BaseAt);

			FOperation::ADDRESS TargetAt = Target ? Target->LinkedAddress : 0;
