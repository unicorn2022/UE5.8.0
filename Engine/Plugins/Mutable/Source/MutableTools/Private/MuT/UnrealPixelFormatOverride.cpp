...
void MutableToImageCore(const UE::Mutable::Private::FImage* InMutable, FImage& CoreImage, int32 LOD, bool bSwizzleRGBHack)
{
	...
	UE::Mutable::Private::TManagedPtr<UE::Mutable::Private::FImage> TempMutable;
...
bool ImageCoreToMutable(const FCompressedImage2D& Compressed, UE::Mutable::Private::FImage* Mutable, int32 LOD)
{
	...
	{
		UE_LOGF(LogMutableCore, Error, "Buffer size mismatch when trying to convert image LOD %d, mutable size is %d and ue size is %lld. Mutable is %d x %d format %d and UE is %d x %d format %d.", 
			LOD, MutableView.Num(), Compressed.RawData.Num(), 
			Mutable->GetSizeX(), Mutable->GetSizeY(), EnumToUnderlyingType(Mutable->GetFormat()),
			Compressed.SizeX, Compressed.SizeY, EnumToUnderlyingType(Compressed.PixelFormat)
			);
