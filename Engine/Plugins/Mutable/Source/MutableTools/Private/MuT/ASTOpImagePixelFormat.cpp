...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpImagePixelFormat::Hash() const
	{
	...
	Ptr<ASTOp> ASTOpImagePixelFormat::OptimiseSemantic(const FModelOptimizationOptions& options, int32 Pass) const
	{
		...
		// Skip this operation if the source op format if fixed and is already the one we want.
		if (Source)
		{
			FGetImageDescContext Context;
			FImageDesc SourceDesc = Source->GetImageDesc(false, &Context);

			const bool bIsFixedFormat = SourceDesc.m_format == SourceDesc.FormatIfAlpha || SourceDesc.FormatIfAlpha == EImageFormat::None;
			if (!Context.bIsFormatFromImageParameter && bIsFixedFormat && SourceDesc.m_format == Format)
			{
				NewOp = Source.child();
			}
		}
	...
	FImageDesc ASTOpImagePixelFormat::GetImageDesc(bool returnBestOption, FGetImageDescContext* context) const
	{
		...
		if (FormatIfAlpha != EImageFormat::None &&
			res.FormatIfAlpha != EImageFormat::None)
		{
			res.FormatIfAlpha = FormatIfAlpha;
		}
		else
		{
			res.FormatIfAlpha = EImageFormat::None;
	...
	bool ASTOpImagePixelFormat::IsImagePlainConstant(FVector4f& color) const
	{
		bool res = false;
		if (Source.child())
		{
			Source.child()->IsImagePlainConstant(color);
		}
		return res;
	}
	...
	UE::Mutable::Private::Ptr<ASTOp> Sink_ImagePixelFormatAST::Visit(UE::Mutable::Private::Ptr<ASTOp> at, const ASTOpImagePixelFormat* currentFormatOp)
	{
		...
		case EOpType::IM_MULTICOMPOSE:
		{
			if (isBlockFormat)
			{
				...
				// TODO: Do not force a format to the Source Image it's from a TextureParameter
				Ptr<ASTOp> SourceImageOp = newOp->SourceImage.child();
				newOp->SourceImage = Visit(SourceImageOp, currentFormatOp);
