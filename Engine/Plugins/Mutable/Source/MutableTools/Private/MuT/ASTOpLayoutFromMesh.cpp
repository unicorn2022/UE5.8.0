...
namespace UE::Mutable::Private
{
	...
	uint32 ASTOpLayoutFromMesh::Hash() const
	{
		uint32 Result = GetTypeHash(GetOpType());

		Result = HashCombineFast(Result, GetTypeHash(Mesh));
		Result = HashCombineFast(Result, GetTypeHash(LayoutIndex));
	...
	{
		UE::Mutable::Private::Ptr<ASTOpLayoutFromMesh> New = new ASTOpLayoutFromMesh();
		New->Mesh = MapChild(Mesh.child());
		New->LayoutIndex = LayoutIndex;
		return New;
	}


	void ASTOpLayoutFromMesh::ForEachChild(const TFunctionRef<void(ASTChild&)> Func)
	{
		Func(Mesh);
	}
	...
	void ASTOpLayoutFromMesh::Link(FProgram& Program, FLinkerOptions*)
	{
		...
		if (!LinkedAddress)
		{
			FOperation::LayoutFromMeshArgs Args;
