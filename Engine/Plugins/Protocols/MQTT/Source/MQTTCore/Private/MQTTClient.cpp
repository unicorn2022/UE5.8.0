	...
	, URL(InURL)
{
	...
	{
		UE_LOGF(LogMQTTCore, Error, "URL is not valid: %ls", *URLMessage.ToString());
		return;
	}
	...
	{
		UE_LOGF(LogMQTTCore, Error, "Cannot create MQTTConnection: Invalid IP address: %ls", *InURL.Host);
		URL.SetInvalid();
	}
	else
	{
...
TFuture<EMQTTConnectReturnCode> FMQTTClient::Connect(const bool bCleanSession)
{
	...
	if(Connection.IsValid())
	{
		...
		else if(ConnectionState != EMQTTState::Connecting)
		{
			...
			UE_LOGF(LogTemp, VeryVerbose, "QUEUED CONNECT");
			Result = Connection->QueueOperation<FMQTTConnectOperation>(
				...
				{
					UE_LOGF(LogTemp, VeryVerbose, "GOT CONNACK");
					if(!Self.IsValid())
					{
					...
					{
						UE_LOGF(LogMQTTCore, Warning, "Couldn't connect to MQTT server: %ls", MQTT::GetMQTTConnectReturnCodeDescription(InResponse.ReturnCode));
					}
...
TFuture<bool> FMQTTClient::Publish(const FString& InTopic, const TArray<uint8>& InPayload, const EMQTTQualityOfService InQoS, const bool bInRetain)
{
	if(Connection.IsValid())
	{
		...
		{
			UE_LOGF(LogMQTTCore, Warning, "Attempted to publish message without topic");
			return MakeFulfilledPromise<bool>(false).GetFuture();			
		}

		if(!FMQTTTopic::IsValid(InTopic))
		{
			UE_LOGF(LogMQTTCore, Warning, "Attempted to publish with an invalid or malformed topic: \'%ls\'", *InTopic);
			return MakeFulfilledPromise<bool>(false).GetFuture();			
		}
...
TFuture<TArray<FMQTTSubscribeResult>> FMQTTClient::Subscribe(const TArray<TPair<FString, EMQTTQualityOfService>>& InTopicFilterQoSPairs)
{
	...
	{
		UE_LOGF(LogMQTTCore, Warning, "Attempted to subscribe without specifying a topic");		
	...
	if(Connection.IsValid())
	{
		...
		for(const TPair<FString, EMQTTQualityOfService>& TopicFilterQoSPair : InTopicFilterQoSPairs)
		{
			...
			{
				UE_LOGF(LogMQTTCore, Warning, "Attempted to subscribe with an empty topic");
				return MakeFulfilledPromise<TArray<FMQTTSubscribeResult>>(FailureResponse).GetFuture();	
			}

			if(!FMQTTTopic::IsValid(TopicFilterQoSPair.Key))
			{
				UE_LOGF(LogMQTTCore, Warning, "Attempted to subscribe with an invalid or malformed topic: \'%ls\'", *TopicFilterQoSPair.Key);
				return MakeFulfilledPromise<TArray<FMQTTSubscribeResult>>(FailureResponse).GetFuture();
			}
		...
		UE_LOGF(LogMQTTCore, Verbose, "Queued Subscribe message with PacketId %d., and Topic Filter: \'%ls\'", PacketId, *TopicFilters[0]);
