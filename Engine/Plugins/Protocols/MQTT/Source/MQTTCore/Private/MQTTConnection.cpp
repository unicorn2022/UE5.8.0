	...
	, Thread(nullptr)
{
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Created MQTTConnection for %ls", *EndpointInternetAddr->ToString(false));
}
...
FMQTTConnection::~FMQTTConnection()
{
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Destroyed MQTTConnection at %ls", *EndpointInternetAddr->ToString(false));
}
...
TSharedPtr<FMQTTConnection, ESPMode::ThreadSafe> FMQTTConnection::TryCreate(const TSharedPtr<FInternetAddr>& InAddr)
{
	...
	{
		UE_LOGF(LogMQTTCore, Error, "Cannot create MQTTConnection, the IP Address was invalid.");
		return nullptr;		
	}
	...
	if (!Socket)
	{
		UE_LOGF(LogMQTTCore, Error, "Cannot create MQTTConnection: Error creating Socket for: %ls: %ls", *InAddr->ToString(true), SocketSubsystem->GetSocketError(SocketSubsystem->GetLastErrorCode()));
		return nullptr;
	}
...
void FMQTTConnection::ProcessMessages()
{
	...
	{
		UE_LOGF(LogMQTTCore, Verbose, "Copy outgoing operations to buffer");
		...
		UE_LOGF(LogMQTTCore, Verbose, "Operations deferred: %d", DeferredOperationCount);
	...
	while(PendingOutgoingOperations.Dequeue(Operation))
	{
		...
		{
			ISocketSubsystem* SocketSubsystem = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM);
			UE_LOGF(LogMQTTCore, Error, "Error sending MQTT packet: %ls: %ls", *EndpointInternetAddr->ToString(true), SocketSubsystem->GetSocketError(SocketSubsystem->GetLastErrorCode()));
		}

		if(ActualBytesSent != ExpectedBytesSent)
		{
			UE_LOGF(LogMQTTCore, Error, "Packet size mismatch: expected %d, was %d", ExpectedBytesSent, ActualBytesSent);
		}
	...
	if(!PendingIncoming.IsEmpty())
	{
		...
		{
#if UE_BUILD_DEBUG
			UE_LOGF(LogMQTTCore, Verbose, "Re-queue pending incoming operation: %ls", *Operation->GetTypeName());
#endif
	...
	{
		UE_LOGF(LogMQTTCore, Verbose, "Processing incoming packets of size: %d", Size);
...
void FMQTTConnection::OpenSocket()
{
	if(Socket)
	{
		...
		if(SocketState != EMQTTSocketState::Stopping)
		{
			...
			ISocketSubsystem* SocketSubsystem = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM);
			const ESocketErrors LastErrorCode = SocketSubsystem->GetLastErrorCode();
			UE_LOGF(LogMQTTCore, Error, "Error connecting socket to: %ls: %ls, Connection state: %ls",
...
{
	UE_LOGF(LogMQTTCore, Verbose, "Abandoning Operations");
...
void FMQTTConnection::HandleMessage(const EMQTTPacketType InMessageType, const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	case EMQTTPacketType::None:
		{
			UE_LOGF(LogMQTTCore, Error, "PacketType was None");
		}
		break;
		
	default:
		{
			UE_LOGF(LogMQTTCore, Error, "Unhandled, unknown message type: %d (%ls)", int(InMessageType), MQTT::GetMQTTPacketTypeName(InMessageType));
		}
	}
}
...
void FMQTTConnection::HandleConnectAcknowledge(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled ConnectAck message.");
}
...
void FMQTTConnection::HandlePublishAcknowledge(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled PublishAck message.");
}
...
void FMQTTConnection::HandlePublishReceived(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled PublishReceive message.");
}
...
void FMQTTConnection::HandlePublishRelease(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled PublishRelease message.");
}
...
void FMQTTConnection::HandlePublishComplete(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled PublishComplete message.");
}
...
void FMQTTConnection::HandleSubscribeAcknowledge(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled SubscribeAck message with PacketId %d.", Message.PacketId);
}
...
void FMQTTConnection::HandleUnsubscribeAcknowledge(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled UnsubscribeAck message.");
}
...
void FMQTTConnection::HandlePingResponse(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled PingResponse message.");
}
...
void FMQTTConnection::HandlePublish(const TSharedRef<FArrayReader>& InPacketReader)
{
	...
	*InPacketReader << Message;
	...
	UE_LOGF(LogMQTTCore, VeryVerbose, "Handled Publish message.");
}
...
void FMQTTConnection::SetState(const EMQTTState InState)
{
	...
	UE_LOGF(LogMQTTCore, Verbose, "Set State to: %ls", MQTT::GetMQTTStateName(InState));
