...
bool FLuaUtils::CallFunction(lua_State* LuaState, const ANSICHAR* FunctionName)
{
	...
	{
		UE_LOGF(LogScriptPlugin, Warning, "Cannot call Lua function %ls: %ls", ANSI_TO_TCHAR(FunctionName), ANSI_TO_TCHAR(lua_tostring(LuaState, -1)));
		bResult = false;
	}
	return bResult;
}
...
static int32 LuaUnrealLog(lua_State* LuaState)
{
	...
	UE_LOGF(LogScriptPlugin, Log, "%ls", *Message);
...
{
	UE_LOGF(LogScriptPlugin, Error, "PANIC: unprotected error in call to Lua API(%ls)", ANSI_TO_TCHAR(lua_tostring(lua_State, -1)));
	return 0;
}
...
bool FLuaContext::Initialize(const FString& Code, UObject* Owner)
{
	...
	{
		UE_LOGF(LogScriptPlugin, Warning, "Cannot load Lua script for %ls: %ls", *Owner->GetPathName(), ANSI_TO_TCHAR(lua_tostring(LuaState, -1)));
	}
	return bResult;
}
...
bool FLuaContext::CallFunction(const FString& FunctionName)
{
	...
	{
		UE_LOGF(LogScriptPlugin, Warning, "Failed to call function '%ls' ", *FunctionName);
	}
...
bool FLuaContext::SetFloatProperty(const FString& PropertyName, float NewValue)
{
	...
	{
		UE_LOGF(LogScriptPlugin, Warning, " [LUA] Global '%ls' is NaN. ", *PropertyName);
	}
...
bool FLuaContext::SetStringProperty(const FString& PropertyName, const FString& NewValue)
{
	...
	UE_CLOGF(!bExists, LogScriptPlugin, Warning, " [LUA] Global '%ls' is not a String. ", *PropertyName);
	return bExists;
}
...
FString FLuaContext::GetStringProperty(const FString& PropertyName)
{
	...
	{
		UE_LOGF(LogScriptPlugin, Warning, " [LUA] Global '%ls' is not a String. ", *PropertyName);
	}
