...
UObject* UScriptFactory::FactoryCreateText(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, const TCHAR* Type, const TCHAR*& Buffer, const TCHAR* BufferEnd, FFeedbackContext* Warn)
{
	...
	{
		UE_LOGF(LogScriptEditorPlugin, Warning, "Failed to import %ls: could not compile script.", *CurrentFilename);
	}
...
EReimportResult::Type UReimportScriptFactory::Reimport(UObject* Obj)
{
	...
	UE_LOGF(LogScriptEditorPlugin, Log, "Performing atomic reimport of [%ls]", *ResolvedSourceFilePath);

	// Ensure that the file provided by the path exists
	if (IFileManager::Get().FileSize(*ResolvedSourceFilePath) == INDEX_NONE)
	{
		UE_LOGF(LogScriptEditorPlugin, Warning, "Cannot reimport: source file cannot be found.");
		return EReimportResult::Failed;
	}
	...
	{
		UE_LOGF(LogScriptEditorPlugin, Log, "Imported successfully");
	...
	{
		UE_LOGF(LogScriptEditorPlugin, Warning, "-- import canceled");
	}
	else
	{
		UE_LOGF(LogScriptEditorPlugin, Warning, "-- import failed");
	}
