...
bool FXGEControllerModule::IsSupported()
{
	...
	if (XGEControllerVariables::Enabled == 1)
	{
		...
		if (!bFound)
		{
			UE_LOGF(LogXGEController, Log, "Cannot use XGE Controller as Incredibuild is not installed on this machine.");
			XGEControllerVariables::Enabled = 0;
		}
		else
		{
			...
			if (Version == 0)
			{
				UE_LOGF(LogXGEController, Display, "Cannot determine XGE version. XGE Shader compilation with the interception interface may fail [%ls].", *XGConsolePath);
			}
			else if (Version < 1002867)
			{
				UE_LOGF(LogXGEController, Display, "XGE version 8.01 (build 1867) or higher is required for XGE shader compilation with the interception interface [%ls].", *XGConsolePath);
				XGEControllerVariables::Enabled = 0;
			}
			...
			{
				UE_LOGF(LogXGEController, Display, "XGE's background service (%ls) is not running - service is likely disabled on this machine [%ls].", XGEBuildServiceExecutableName, *XGConsolePath);
				XGEControllerVariables::Enabled = 0;
			}

			if (!PlatformFile.FileExists(*GetControlWorkerExePath()))
			{
				UE_LOGF(LogXGEController, Display, "XGEControlWorker.exe does not exist, XGE may be disabled in your Build Configuration, cannot use XGE [%ls].", *XGConsolePath);
				XGEControllerVariables::Enabled = 0;
			}
		}
	}
...
void FXGEControllerModule::CleanWorkingDirectory()
{
	...
	{
		UE_LOGF(LogXGEController, Log, "Cleaning working directory: %ls", *RootWorkingDirectory);
...
void FXGEControllerModule::InitializeController()
{
	if (ensureAlwaysMsgf(!bControllerInitialized, TEXT("Multiple initialization of the XGE controller!")))
	{
		...
		if (IsSupported())
		{
			...
			UE_LOGF(LogXGEController, Display, "Initialized XGE controller. XGE tasks will %lsbe spawned on this machine.",
...
void FXGEControllerModule::WriteOutThreadProc()
{
	do
	{
		...
		{
			UE_LOGF(LogXGEController, Verbose, "Waiting for new tasks.");
			WriteOutThreadEvent->Wait();
		}

		if (bShutdown)
		{
			UE_LOGF(LogXGEController, Verbose, "Shutting down communication with the XGE controller.");
			return;
		}
		...
		{
			UE_LOGF(LogXGEController, Fatal, "Failed to create the output XGE named pipe.");
		}
		...
		UE_LOGF(LogXGEController, Verbose, "Launching xgConsole");
		...
		{
			UE_LOGF(LogXGEController, Fatal, "Failed to launch the XGE control worker process.");
		}
		...
		UE_LOGF(LogXGEController, Verbose, "Launching XGEController to fan out tasks");
		...
		{
			UE_LOGF(LogXGEController, Fatal, "Failed to open a connection on the output XGE named pipe.");
		}
		...
		{
			UE_LOGF(LogXGEController, Fatal, "Failed to connect the input XGE named pipe.");
		}
		...
		{
			UE_LOGF(LogXGEController, Fatal, "Failed to pass xgConsole process ID to XGE control worker.");
		}
