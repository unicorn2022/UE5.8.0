...
UENUM()
enum class EEvaluateChooserStructOutputMode
{
	Default,
	SingleOutputStruct,
};
...
private:
	...
	UPROPERTY(EditAnywhere, Category = "Chooser", Meta = (HideEditConditionToggle, EditCondition="bShowModeSetting"))
	EEvaluateChooserMode Mode;

	// This property exist for backwards compatibility with old content (where only single output struct was supported).
	// New content should use "Default" setting to stay in sync with the regular output mode.
	UPROPERTY(EditAnywhere, Category = "Chooser", Meta = (HideEditConditionToggle, EditCondition="bShowModeSetting"))
	EEvaluateChooserStructOutputMode StructOutputMode = EEvaluateChooserStructOutputMode::SingleOutputStruct;
