...
void FEnumColumn::Filter(FChooserEvaluationContext& Context, const FChooserIndexArray& IndexListIn, FChooserIndexArray& IndexListOut) const
{
	...
	void FEnumColumn::SetFromDetails(FInstancedPropertyBag& PropertyBag, int32 ColumnIndex, int32 RowIndex)
	{
		...
		if (FStructView* StructView = Result.TryGetValue())
		{
			...
			if (const FEnumProperty* ValueEnumProperty = CastField<FEnumProperty>(StructDefinition->FindPropertyByName("Value")))
			{
				ValueEnumProperty->GetValue_InContainer(StructView->GetMemory(), &RowValues[RowIndex].Value);
			}
			else if (const FByteProperty* ValueByteProperty = CastField<FByteProperty>(StructDefinition->FindPropertyByName("Value")))
            {
            	ValueByteProperty->GetValue_InContainer(StructView->GetMemory(), &RowValues[RowIndex].Value);
            }
