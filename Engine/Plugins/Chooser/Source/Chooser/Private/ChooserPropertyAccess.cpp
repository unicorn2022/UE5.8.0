...
struct FCompiledBindingCacheId
{
	...
	{
		 return ContextIndex == Other.ContextIndex
				&&Type == Other.Type
...
{
	return HashCombineFast(GetTypeHash(Id.ContextIndex),HashCombineFast(GetTypeHash(Id.Type), GetTypeHash(Id.BindingPath)));
}
...
namespace UE::Chooser
{
		...
		for (int i = 0; i < ContextNum; i++)
		{
			...
			if (ExpectedClassType)
			{
				if (ExpectedClassType->Class)
				{
					if (Context.Params.IsValidIndex(i) && Context.Params[i].IsValid())
					{
						if (FChooserEvaluationInputObject* InputObjectParam = Context.Params[i].GetPtr<FChooserEvaluationInputObject>())
						{
							if (InputObjectParam->Object)
							{
								...
								{
									UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an object of type %ls, but an object of type %ls was passed in.",
										ToCStr(Chooser->GetName()), i, ToCStr(ExpectedClassType->Class->GetName()), ToCStr(InputObjectParam->Object->GetClass()->GetName()));
								}
							}
							else
							{
								UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an object of type %ls, but null was passed in.",
						...
						{
							if (Context.Params[i].GetScriptStruct())
							{
								UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an object of type %ls, but was passed a struct of type %ls.",
									ToCStr(Chooser->GetName()), i, ToCStr(ExpectedClassType->Class->GetName()), ToCStr(Context.Params[i].GetScriptStruct()->GetName()));
							}
							else
							{
								
								UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an object of type %ls, but was passed a null struct param",
									ToCStr(Chooser->GetName()), i, ToCStr(ExpectedClassType->Class->GetName()));
							}
						}
					}
					...
					{
						UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an object of type %ls, but nothing was passed in.",
							ToCStr(Chooser->GetName()), i, ToCStr(ExpectedClassType->Class->GetName()));
					}
				}
			}
				...
				if (ExpectedStructType->Struct)
				{
					if (Context.Params.IsValidIndex(i) && Context.Params[i].IsValid())
					{
						if (FChooserEvaluationInputObject* InputObjectParam = Context.Params[i].GetPtr<FChooserEvaluationInputObject>())
						{
							if (InputObjectParam->Object)
							{
								UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an struct of type %ls, but was passed a object of type %ls.",
									ToCStr(Chooser->GetName()), i, ToCStr(ExpectedStructType->Struct->GetName()), ToCStr(InputObjectParam->Object->GetClass()->GetName()));
							}
							else
							{
								UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an struct of type %ls, but was passed a null object.",
						...
						else
						{
							if (Context.Params[i].GetScriptStruct() != ExpectedStructType->Struct)
							{
								if (Context.Params[i].GetScriptStruct())
								{
									UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an struct of type %ls, but was passed a struct of type %ls.",
										ToCStr(Chooser->GetName()), i, ToCStr(ExpectedStructType->Struct->GetName()), ToCStr(Context.Params[i].GetScriptStruct()->GetName()));
								}
								else
								{
									UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an struct of type %ls, but was passed a null struct parameter.",
										ToCStr(Chooser->GetName()), i, ToCStr(ExpectedStructType->Struct->GetName()));
								}
							}
						}
					...
					{
						UE_LOGF(LogChooser, Error, "Chooser Table: %ls ContextData entry %d expects an struct of type %ls, but nothing was passed in.",
							ToCStr(Chooser->GetName()), i, ToCStr(ExpectedStructType->Struct->GetName()));
					}
				}
			}
			...
			{
				// UAF has it's own child struct, so since this code can't be aware of that type, for now disabling this error
				//UE_LOGF(LogChooser, Error, "Chooser Table: %ls  ContextData entry %i is of unknown type or none.", ToCStr(Chooser->GetName()), i);
			}
		}
	}
	...
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_Chooser_ResolveCompiledPropertyChain);
		...
		{
			UE_LOGF(LogChooser, Error, "Invalid Index {%d} while resolving compiled property chain.", CompiledBinding.ContextIndex);
			return false;
		}
		...
		{
			UE_LOGF(LogChooser, Error, "Property Binding compiled for type: {%ls} is being evaluated on incompatible type: {%ls}.", ToCStr(CompiledBinding.TargetType->GetName()), ToCStr(InputType->GetName()));
			return false;
		}
	...
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_Chooser_ResolvePropertyChain);
