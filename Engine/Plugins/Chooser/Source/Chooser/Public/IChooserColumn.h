...
public:
	virtual ~FChooserColumnBase() = default;
	...
	virtual FInstancedStruct* GetInputValuePtr() { return nullptr; }
	...
	virtual void ReplaceReferences(UObject* ReferenceToReplace, UObject* ReplaceWith) {}
#endif
...
#define CHOOSER_COLUMN_BOILERPLATE2(ParameterType, RowValuesProperty) \
	...
	virtual void DeleteRows(TArrayView<int> RowIndices ) override\
	{\
		for(int32 Index : RowIndices)\
		{\
			if (RowValuesProperty.IsValidIndex(Index)) \
			{\
	...
	{\
		if (RowValuesProperty.IsValidIndex(TargetRowIndex) && static_cast<decltype(this)>(&SourceColumn)->RowValuesProperty.IsValidIndex(SourceRowIndex)) \
		{\
	...
	virtual FInstancedStruct* GetInputValuePtr() override { return &InputValue; };\
