...
public:
	...
	CHOOSER_API void PopulateCookedData(bool bCooking);
	CHOOSER_API void DeleteNestedChooser(UChooserTable* NestedChooser);
	CHOOSER_API void DeleteNestedObject(UObject* ObjectToDelete);
#endif

	bool IsRowDisabled(int32 RowIndex) const
	{
#if WITH_EDITORONLY_DATA
		return !IsCookedData() && DisabledRows.IsValidIndex(RowIndex) && DisabledRows[RowIndex];
#else
		return false;
#endif
	}
#if WITH_EDITOR
	CHOOSER_API	virtual EDataValidationResult IsDataValid(class FDataValidationContext& Context) const override;
	...
	void SetDebugSelectedRow(int32 Index) const { DebugSelectedRows = { Index }; }
	void SetDebugSelectedRows(const TArray<int32>& Indices) const { DebugSelectedRows = Indices; }
	const TArray<int32>& GetDebugSelectedRows() const { return DebugSelectedRows; }
...
private:
	...
	// Rows which were selected last time this chooser was evaluated on DebugTarget
	mutable TArray<int32> DebugSelectedRows;
...
public:
	...
	bool IsCookedData() const
	{
#if WITH_EDITORONLY_DATA
		return bIsCookedPackage;
#else
		return true;
#endif
	}
...
struct FEvaluateChooser : public FObjectChooserBase
{
	...
	#if WITH_EDITOR
		virtual UObject* GetReferencedObject() const override { return Chooser; }
	#endif
	public:
