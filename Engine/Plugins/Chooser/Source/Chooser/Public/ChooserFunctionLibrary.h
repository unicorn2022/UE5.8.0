...
public:
	...
	*/
	...
	/**
	* Get an Object from a ChooserEvaluation context by class or interface
	* Experimental, this feature might be removed without warning, not for production use
	*
	* @param Context			(in) A struct reference to the chooser evaluation context
	* @param ObjectClass		(in) the wanted object class or interface
	*/
	UFUNCTION(BlueprintPure, Category = "Animation | Experimental", meta = (BlueprintThreadSafe))
	static UObject* GetChooserObject(UPARAM(Ref) FChooserEvaluationContext& Context, TSubclassOf<UObject> ObjectClass);
	...
	*/
	...
	/**
	* Add a Struct with output to a ChooserEvaluation context
	*
	* @param Context			(in) A struct reference to the chooser evaluation context
	* @param Object				(in) The Object to add
	*/
	UFUNCTION(BlueprintCallable, CustomThunk, meta = (BlueprintThreadSafe, BlueprintInternalUseOnly = "true", CustomStructureParam = "Value"))
	static UE_API void AddChooserStructInputOutput(UPARAM(Ref) FChooserEvaluationContext& Context, int32 Value);
	
	DECLARE_FUNCTION(execAddChooserStructInputOutput);
	...
	* @param Object				(in) The Object to add
	*/
	UFUNCTION(BlueprintPure, CustomThunk, meta = (BlueprintThreadSafe, BlueprintInternalUseOnly = "true", ArrayParm = "Value"))
	static UE_API void GetChooserStructOutputArray(UPARAM(Ref) FChooserEvaluationContext& Context, int Index, TArray<int32>& Value);
		
	DECLARE_FUNCTION(execGetChooserStructOutputArray);
