...
TSharedRef<SWidget> CreateFloatDistanceColumnWidget(UChooserTable* Chooser, FChooserColumnBase* Column, int Row)
{
	...
	else if (Row == ColumnWidget_SpecialIndex_Header)
	{
		...
		const FText ColumnTooltip = LOCTEXT("Float difference tooltip", "Float Difference Column: rows receive a Score based on how different the input float is from the row value");
	...
	{
		return (FloatDistanceColumn->RowValues.IsValidIndex(Row)) ? FloatDistanceColumn->RowValues[Row].Value : 0;
	})
	.OnValueCommitted_Lambda([Chooser, FloatDistanceColumn, Row](float NewValue, ETextCommit::Type CommitType)
	{
		if (FloatDistanceColumn->RowValues.IsValidIndex(Row))
		{
