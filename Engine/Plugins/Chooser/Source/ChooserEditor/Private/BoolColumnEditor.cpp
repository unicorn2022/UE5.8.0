...
TSharedRef<SWidget> CreateOutputBoolColumnWidget(UChooserTable* Chooser, FChooserColumnBase* Column, int Row)
{
	...
	[
		...
		{
			if (BoolColumn->RowValues.IsValidIndex(Row))
			{
				const FScopedTransaction Transaction(LOCTEXT("Change Bool Value", "Change Bool Value"));
				Chooser->Modify(true);
		...
		{
			const bool value = (BoolColumn->RowValues.IsValidIndex(Row)) ? BoolColumn->RowValues[Row] : false;
...
void CreateBoolPropertyMenus(UObject* TransactionObject, const IHasContextClass* ContextClassOwner, FInstancedStruct* Parameter, FMenuBuilder& MenuBuilder, TFunction<void()> BindingChanged)
{
	CreatePropertyAccessMenus<FBoolContextProperty>("bool", TransactionObject, ContextClassOwner, Parameter, MenuBuilder, BindingChanged);
}
	
void RegisterBoolWidgets()
{
	FObjectChooserWidgetFactories::RegisterParameterMenuCreator(FChooserParameterBoolBase::StaticStruct(), CreateBoolPropertyMenus);
