...
TSharedRef<SWidget> CreateGameplayTagQueryColumnWidget(UChooserTable* Chooser, FChooserColumnBase* Column, int Row)
{
	...
	return SNew(SGameplayTagQueryEntryBox)
		...
		{
			if (GameplayTagQueryColumn->RowValues.IsValidIndex(Row))
			{
...
TSharedRef<SWidget> CreateOutputGameplayTagQueryColumnWidget(UChooserTable* Chooser, FChooserColumnBase* Column, int Row)
{
	...
	return SNew(SGameplayTagQueryEntryBox)
		...
		{
			if (OutputGameplayTagQueryColumn->RowValues.IsValidIndex(Row))
			{
...
void CreateGameplayTagQueryPropertyMenus(UObject* TransactionObject, const IHasContextClass* ContextClassOwner, FInstancedStruct* Parameter, FMenuBuilder& MenuBuilder, TFunction<void()> BindingChanged)
{
	CreatePropertyAccessMenus<FGameplayTagQueryContextProperty>("FGameplayTagQuery", TransactionObject, ContextClassOwner, Parameter, MenuBuilder, BindingChanged);
}
	
void RegisterGameplayTagQueryWidgets()
{
	FObjectChooserWidgetFactories::RegisterParameterMenuCreator(FChooserParameterGameplayTagQueryBase::StaticStruct(), CreateGameplayTagQueryPropertyMenus);
