...
void CreatePropertyAccessMenus(const FString& TypeFilter, UObject* TransactionObject, const IHasContextClass* ContextClassOwner, FInstancedStruct* Parameter, FMenuBuilder& MenuBuilder, TFunction<void ()> BindingChanged)
{
	FOnAddBinding OnAddBinding  = FOnAddBinding::CreateLambda( 
	[Parameter, TransactionObject, ContextClassOwner, BindingChanged](FName InPropertyName, const TArray<FBindingChainElement>& InBindingChain)
	{
		if (Parameter)
		{
			...
			UE::ChooserEditor::SPropertyAccessChainWidget::SetPropertyBinding(const_cast<IHasContextClass*>(ContextClassOwner), InBindingChain, Property.Binding);
			if (BindingChanged.IsSet())
			{
				BindingChanged();
			}
	...
	UE::ChooserEditor::CreatePropertyAccessMenus(TypeFilter, OnAddBinding, ContextClassOwner, Parameter, MenuBuilder);	
}
