...
void FLookupProxyWithOverrideTable::ChooseObject(FChooserEvaluationContext& Context, TSoftObjectPtr<>& Result) const
{
	if (Proxy && OverrideProxyTable)
   	{
   		OverrideProxyTable->FindProxyObject(Proxy->Guid, Context, Result);
   	}
}

FObjectChooserBase::EIteratorStatus FLookupProxyWithOverrideTable::ChooseMulti(FChooserEvaluationContext& Context,
