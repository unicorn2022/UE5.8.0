...
#if WITH_EDITOR
	/**
	* Get all entries in a ProxyTable as a map from ProxyAsset to its resolved UObject value.
	* Editor-only: uses editor-only APIs (GetReferencedObject) that are not available in cooked builds.
	*
	* @param ProxyTable				(in) The ProxyTable asset to read entries from
	* @return Map of ProxyAsset to resolved UObject value
	*/
	UFUNCTION(BlueprintCallable, Category = "Animation", meta = (CallInEditor = "true"))
	static UE_API TMap<UProxyAsset*, UObject*> GetProxyTableEntries(const UProxyTable* ProxyTable);
#endif
};
