...
FString UAutomatedProfileGoTest::GetPerfTestTypeID() const
{
	return TEXT("ProfileGo");
}
...
void UAutomatedProfileGoTest::CompleteTest(bool bSuccess)
{
	...
	// Handle this potentially being destroyed priorty to the timer expiring.
	FTimerDelegate Delegate = FTimerDelegate::CreateWeakLambda(this, [this, bSuccess]()
	{
		if (bSuccess)
		{
			EndTestSuccess();
	...
	GWorld->GetTimerManager().SetTimer(WaitTimerHandle, Delegate, RateInSeconds, false);
	bShouldTick = false;
}
