...
bool UProfileGo::Run(UProfileGoSubsystem* InSubsystem, const TCHAR* InProfileName, const TCHAR* InProfileArgs)
{
	...
	UE_LOGF(LogAutomatedPerfTest, Log, "UProfileGo::ProfileGo(UWorld*, InProfileName = \"%ls\", InProfileArgs = \"%ls\")", InProfileName, InProfileArgs);

	if (!InSubsystem)
	{
		UE_LOGFMT(LogAutomatedPerfTest, Log, "InSubsystem is nullptr! Aborting ProfileGo.");
		return false;
	}
	...
	if (FPaths::ValidatePath(InName) && FPaths::FileExists(InName))
	{
		...
		{
			UE_LOGF(LogAutomatedPerfTest, Log, "Read file at %ls to set profilego scenario: %ls", InProfileName, *InName);
			// Remove newline characters which are convenient to use when writing these files
			InName.ReplaceInline(TEXT("\n"), TEXT(""));
		}
		else
		{
			UE_LOGF(LogAutomatedPerfTest, Error, "Failed to read profilego scenario from file at %ls", InProfileName);
			return false;
		}
	}
...
void UProfileGo::AddNewProfileGoCollection(FProfileGoCollectionAPT NewCollection)
{
	...
	{
		UE_LOGF(LogAutomatedPerfTest, Log, "Collection named %ls already exists!", *NewCollection.Name);
		return;
	}
	...
	for (FString ScenarioName : CollectionScenarios)
	{
		...
		{
			UE_LOGF(LogAutomatedPerfTest, Log, "Scenario named %ls does not exist! Cannot add a Collection containing an undefined Scenario. Use AddNewProfileGoScenario first.", *ScenarioName);
			return;
		}
	}

	GetCollectionTemplates().Add(NewCollection);
	UE_LOGF(LogAutomatedPerfTest, Log, "Added new collection named %ls", *NewCollection.Name);
}
...
void UProfileGo::AddNewProfileGoScenario(FProfileGoScenarioAPT NewScenario)
{
	...
	{
		UE_LOGF(LogAutomatedPerfTest, Log, "Scenario named %ls already exists!", *NewScenario.Name);
		return;
	}

	GetScenarioTemplates().Add(NewScenario);
	UE_LOGF(LogAutomatedPerfTest, Log, "Added new scenario named %ls", *NewScenario.Name);
}
...
bool UProfileGo::AddScenariosToCurrentRequest(const TCHAR* ProfileName, bool bErrorIfNotFound /*= false*/, int InsertIndex /*= INDEX_NONE*/)
{
	...
	{
		FString AbortMessage = FString::Printf(TEXT("Could not find expected scenario or collection named %s. Aborting profilego!"), ProfileName);
		AbortWithError(*AbortMessage);
	}
...
bool UProfileGo::TryFindScenarios(const TCHAR* ProfileName, TArray<FProfileGoScenarioAPT>& OutScenarios)
{
	...
	for (FString ScenarioName : ScenarioNames)
	{
		...
		{
			UE_LOGF(LogAutomatedPerfTest, Log, "Scenario named %ls does not exist! Skipping.", *TrimmedScenarioName);
		}
	}
...
bool UProfileGo::WaitForLoadingAndStreaming(UWorld* InWorld, float DeltaTime, bool bIncrementalResourceStreaming /*= false*/)
{
	...
	if (FStreamingManagerCollection* StreamingManagers = IStreamingManager::Get_Concurrent())
	{
		...
		{
			UE_LOGF(LogAutomatedPerfTest, Log, "TickSettling: Texture streaming did not finish %i requests within %ls seconds. Retrying settle.", NumUnfinishedRequests, *FString::SanitizeFloat(StreamingBlockTimeLimit));
			return true;
		}
	...
	if (IsAsyncLoading())
	{
		...
		UE_LOGFMT(LogAutomatedPerfTest, Log, "TickSettling: Waiting On AsyncLoading");
	}

	TWeakObjectPtr<UWorldPartitionSubsystem> WorldPartition = UWorld::GetSubsystem<UWorldPartitionSubsystem>(InWorld);
	...
	{
		UE_LOGFMT(LogAutomatedPerfTest, Log, "TickSettling: Waiting On World Partition Level Streaming");
		bLoadInProgress = true;
	}
	...
	if (GShaderCompilingManager && GShaderCompilingManager->GetNumOutstandingJobs())
	{
		...
		UE_LOGFMT(LogAutomatedPerfTest, Log, "TickSettling: Waiting On Shaders");
		bLoadInProgress = true;
	}
	...
	if (bIncrementalResourceStreaming && !bLoadInProgress)
	{
		...
		{
			UE_LOGFMT(LogAutomatedPerfTest, Log, "TickSettling: Waiting On Resource Streaming Final update");
			bLoadInProgress = true;
		}
	}
...
void UProfileGo::LoadFromJSON(const FString& Filename)
{
	...
	{
		UE_LOGF(LogAutomatedPerfTest, Warning, "ProfileGo::LoadFromJSON: '%ls' not found.", *Filename);
		return;
	}
...
void UProfileGo::TickCompletedScenario(float DeltaTime)
{
	...
	if (CurrentResult.bCompleted == false)
	{
		...
		UE_LOGF(LogAutomatedPerfTest, Log, "Completed Scenario %ls. (%d/%d)", *CurrentResult.Scenario.Name, CurrentRequest->ScenarioResults.Num(), TotalScenarios);
		return;
	}
...
void UProfileGo::ClientMessage(const TCHAR* InMsg)
{
	...
	UE_LOGF(LogAutomatedPerfTest, Log, "%ls", InMsg);
}
...
bool UProfileGo::TryGenerateScenariosForCurrentRequest(const TCHAR* InProfileName, const TCHAR* InProfileArgs)
{
	...
	const auto FindAndExecuteHandler = [&](bool& bOutCommandResult)
	{
		...
		if (Found != nullptr)
		{
			UE_LOGF(LogAutomatedPerfTest, Verbose, "ProfileGo: Using command handler '%ls' to generate scenario '%ls'", *Found->Key, *ScenarioCommand);
...
bool UProfileGo::TryParseValue(const TCHAR* Stream, const TCHAR* ParamName, FString& OutValue, bool bShouldStopOnSeparator /*= false*/)
{
	...
	if (bSuccess && FPaths::ValidatePath(OutValue) && FPaths::FileExists(OutValue))
	{
		...
		{
			UE_LOGF(LogAutomatedPerfTest, Error, "Failed to read profilego param %ls from file at %ls", ParamName, *OutValue);
		}
	}
...
{
	UE_LOGF(LogAutomatedPerfTest, Log, "Executing command: %ls", *ProfileGoCommand.Command);
...
#if PLATFORM_IOS
		if (ProfileGoCommand.Command.Contains(TEXT("profilegpu")) == false && ProfileGoCommand.Command.Contains(TEXT("screenshot")) == false)
#endif
		{
			if (!ProfileGoCommand.CopyOutputToGameLog && ResultsFile)
			{
				GEngine->Exec(GetWorld(), *ProfileGoCommand.Command, *ResultsFile);
			}
			else
			{
				ConsoleCommand(*ProfileGoCommand.Command);
			}
		}
	}
}
...
bool UProfileGo::TryHandleProfileGoCommandString(const FProfileGoCommandAPT& ProfileGoCommand)
{
	...
	const_cast<FProfileGoCommandAPT&>(ProfileGoCommand).Command = CommandString;
	...
	const auto FindAndExecuteHandler = [&](bool& bOutCommandResult)
	{
		...
		if (Found != nullptr)
		{
			UE_LOGF(LogAutomatedPerfTest, Verbose, "ProfileGo: Using command handler for '%ls' to run '%ls'", *Found->Key, *CommandString);
		    // ExecuteIfBound not supported for delegates with return type
...
void UProfileGo::RegisterInternalCommandDelegates()
{
	...
	CommandHandlerDelegate::CreateLambda([this](FString& CommandString, const FProfileGoCommandAPT& ProfileGoCommand) -> bool
	{
		...
		{
			UE_LOGFMT(LogAutomatedPerfTest, Warning, "Tried to begin csvprofile when csvprofiler was already capturing!");
		}
		else
		{
			...
			{
				FileName = Controller ? Controller->GetTestID() + FileName : FileName;
	...
	CommandHandlerDelegate::CreateLambda([this](FString& CommandString, const FProfileGoCommandAPT&)
	{
		...
		{
			UE_LOGFMT(LogAutomatedPerfTest, Warning, "Tried to end csvprofile when csvprofiler was not capturing!");
		}
		else
		{
	...
	GeneratedScenarioHandlerDelegate::CreateLambda([this](FString& ScenarioCommand, FString& InProfileArgs)
	{
		...
		if (Exists == nullptr)
		{
			...
			if (ScenarioCommand.Split(TEXT(":"), &CommandName, &GridSteps))
			{
				...
				{
					UE_LOGF(LogAutomatedPerfTest, Error, "No valid bounds specified and world doesn't have one either! Skipping scenario %ls", *ProfileName);
					return false;
				}

				if (!NewBounds.IsInside(GridOrigin))
				{
					UE_LOGF(LogAutomatedPerfTest, Error, "Grid origin (%ls) is not inside grid bounds (%ls)! Skipping scenario %ls", *GridOrigin.ToCompactString(), *NewBounds.ToString(), *ProfileName);
					return false;
				}
					...
					for (double PosX = StartPointX; PosX < NewBounds.Max.X; PosX += GridStepX)
					{
						...
						for (double PosY = StartPointY; PosY < NewBounds.Max.Y; PosY += GridStepY)
						{
							...
							for (int SavedRotations = 0; SavedRotations < NumAngles; SavedRotations++)
							{
								...
								ScenarioName.Appendf(TEXT("-%llix%lli"), i, j);
								if (NumAngles > 1)
								{
				...
				{
					UE_LOGF(LogAutomatedPerfTest, Error, "Grid steps are not specified! Skipping scenario %ls", *ProfileName);
					return false;
				}
			}
		}
...
bool UProfileGo::FindBestStandingPoint(UWorld* InWorld, FVector StartXyHalfHeightZ, FVector& OutPos, bool bIgnoreKillVolume /*= false*/, bool bIgnoreBlockingVolume /*= false*/)
{
	...
	if (!InWorld)
	{
		UE_LOGFMT(LogAutomatedPerfTest, Log, "InWorld is nullptr! Cannot trace for best standing point.");
		return false;
	}

	UE_LOGF(LogAutomatedPerfTest, Log, "Line casting from %ls to %ls", *TraceStart.ToString(), *TraceEnd.ToString());
	...
	if (bTracedHit)
	{
		...
		if (WaterHit.bBlockingHit && WaterHit.ImpactPoint.Z > OutPos.Z)
		{
			...
			UE_LOGF(LogAutomatedPerfTest, Log, "Hit water object [%ls]", *WaterHit.GetActor()->GetName());
		}
		if (WorldHit.bBlockingHit && WorldHit.ImpactPoint.Z > OutPos.Z)
		{
			OutPos.Z = WorldHit.ImpactPoint.Z;
			UE_LOGF(LogAutomatedPerfTest, Log, "Hit world object [%ls]", *WorldHit.GetActor()->GetName());
		}
		OutPos.Z += SafetySpace; // Set the location a bit higher so that we don't try to teleport into the world
	}
	else
	{
		UE_LOGF(LogAutomatedPerfTest, Log, "Rejecting location (%.02f,%.02f) due to failed trace. No blocking collision between %.02f and %.02f Z.",
		...
		{
			UE_LOGF(LogAutomatedPerfTest, Log, "Unhit object: %ls at %ls",
	...
	if (!bIgnoreKillVolume)
	{
		for (TObjectIterator<AKillZVolume> It; It; ++It)
		{
			...
			{
				UE_LOGF(LogAutomatedPerfTest, Log, "Rejecting location (%.02f,%.02f,%.02f) due to encompassing KillZVolume %ls", OutPos.X, OutPos.Y, OutPos.Z, *It->GetFullName());
				return false;
			}
		}
	}
	...
	if (!bIgnoreBlockingVolume)
	{
		for (TObjectIterator<ABlockingVolume> It; It; ++It)
		{
			if (It->EncompassesPoint(OutPos, SafetySpace))
			{
				UE_LOGF(LogAutomatedPerfTest, Log, "Rejecting location (%.02f,%.02f,%.02f) due to encompassing BlockingVolume %ls", OutPos.X, OutPos.Y, OutPos.Z, *It->GetFullName());
				return false;
			}
		}
	}
...
TArray<FProfileGoCommandAPT> UProfileGo::CreateCommandList(const FString& InCommandString)
{
	...
	{
		UE_LOGF(LogAutomatedPerfTest, Log, "Looking for group %ls", *CommandString);
		...
		for (const FProfileGoCommandAPT& ProfileGoCommand : GetCommandTemplates())
		{
			...
			{
				bFound = true;
				UE_LOGF(LogAutomatedPerfTest, Log, "Added command %ls with wait %ls from group %ls", *ProfileGoCommand.Command, *FString::SanitizeFloat(ProfileGoCommand.Wait), *ProfileGoCommand.Group);
		...
		if (!bFound)
		{
			...
			UE_LOGF(LogAutomatedPerfTest, Log, "%ls", *DebugInfo);
		}
	}
