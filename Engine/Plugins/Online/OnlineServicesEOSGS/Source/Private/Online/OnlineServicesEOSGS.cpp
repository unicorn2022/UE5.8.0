...
bool FOnlineServicesEOSGS::PreInit()
{
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "[%s] Unable to initialize Socket Subsystem. Error=[%ls]", __FUNCTION__, *ErrorStr);
		return false;
	}
#endif
		...
		if (JoinedLobbies.IsOk())
		{
			for (TSharedRef<const FLobby>& Lobby : JoinedLobbies.GetOkValue().Lobbies)
			{
				if (Lobby->LobbyId == Params.LobbyId)
				{
					...
					return TOnlineResult<FGetResolvedConnectString>({ FString::Printf(TEXT("[%s]"), *TempAddr.ToString(true)) });
#else
