...
TOnlineAsyncOpHandle<FQueryAchievementDefinitions> FAchievementsEOSGS::QueryAchievementDefinitions(FQueryAchievementDefinitions::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FQueryAchievementDefinitions>& InAsyncOp, const EOS_Achievements_OnQueryDefinitionsCompleteCallbackInfo* Data)
		{
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "EOS_Achievements_QueryDefinitions failed with result=[%ls]", *LexToString(Data->ResultCode));
			...
			for (uint32_t AchievementIdx = 0; AchievementIdx < AchievementCount; AchievementIdx++)
			{
				...
				{
					UE_LOGF(LogOnlineServices, Warning, "EOS_Achievements_CopyAchievementDefinitionV2ByIndex failed with result=[%ls]", *LexToString(CopyResult));
				...
				UE_CLOGF(!bDefsApiVersionOk, LogOnlineServices, Warning, "EOS_Achievements_DefinitionV2 version mismatch Expected=%d Actual=%d", ExpectedDefinitionApiVersion, EOSDefinition->ApiVersion);
				...
				for (uint32_t StatThresholdsIdx = 0; StatThresholdsIdx < EOSDefinition->StatThresholdsCount; StatThresholdsIdx++)
				{
					...
					UE_CLOGF(!bStatsApiVersionOk, LogOnlineServices, Warning, "EOS_Achievements_StatThresholds version mismatch Expected=%d Actual=%d", ExpectedStatThresholdsApiVersion, EOSStatThreshold.ApiVersion);
...
TOnlineAsyncOpHandle<FQueryAchievementStates> FAchievementsEOSGS::QueryAchievementStates(FQueryAchievementStates::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FQueryAchievementStates>& InAsyncOp, const EOS_Achievements_OnQueryPlayerAchievementsCompleteCallbackInfo* Data)
		{
			...
			if (Data->ResultCode != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogOnlineServices, Warning, "EOS_Achievements_QueryPlayerAchievements failed with result=[%ls]", *LexToString(Data->ResultCode));
			...
			for (uint32_t AchievementIdx = 0; AchievementIdx < AchievementCount; AchievementIdx++)
			{
				...
				if (CopyResult != EOS_EResult::EOS_Success)
				{
					UE_LOGF(LogOnlineServices, Warning, "EOS_Achievements_CopyPlayerAchievementByIndex failed with result=[%ls]", *LexToString(CopyResult));
				...
				UE_CLOGF(!bPlayerAchievementApiVersionOk, LogOnlineServices, Warning, "EOS_Achievements_PlayerAchievement version mismatch Expected=%d Actual=%d", ExpectedPlayerAchievementApiVersion, EOSPlayerAchievement->ApiVersion);
...
TOnlineAsyncOpHandle<FUnlockAchievements> FAchievementsEOSGS::UnlockAchievements(FUnlockAchievements::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FUnlockAchievements>& InAsyncOp, const EOS_Achievements_OnUnlockAchievementsCompleteCallbackInfo* Data)
		{
			if (Data->ResultCode != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogOnlineServices, Warning, "EOS_Achievements_UnlockAchievements failed with result=[%ls]", *LexToString(Data->ResultCode));
				InAsyncOp.SetError(Errors::FromEOSResult(Data->ResultCode));
				return;
			}

			UE_LOGF(LogOnlineServices, Verbose, "EOS_Achievements_UnlockAchievements succeeded");
...
void FAchievementsEOSGS::HandleAchievementsUnlocked(const EOS_Achievements_OnAchievementsUnlockedCallbackV2Info* Data)
{
	...
	if (LocalAccountId.IsValid())
	{
		...
		UE_LOGF(LogOnlineServices, Verbose, "%s User=%ls AchievId=%ls", __FUNCTION__, *ToLogString(LocalAccountId), *AchievementId);
