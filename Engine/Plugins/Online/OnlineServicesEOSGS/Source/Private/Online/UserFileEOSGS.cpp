...
TOnlineAsyncOpHandle<FUserFileEnumerateFiles> FUserFileEOSGS::EnumerateFiles(FUserFileEnumerateFiles::Params&& InParams)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FUserFileEnumerateFiles>& Op, const EOS_PlayerDataStorage_QueryFileListCallbackInfo* Data)
		{
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "EOS_PlayerDataStorage_QueryFileList failed with result=[%ls]", *LexToString(Data->ResultCode));
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "EOS_PlayerDataStorage_GetFileMetadataCount failed with result=[%ls]", *LexToString(Result));
			...
			for (int32_t FileIdx = 0; FileIdx < NumFiles; FileIdx++)
			{
				...
				if (Result != EOS_EResult::EOS_Success)
				{
					UE_LOGF(LogOnlineServices, Warning, "EOS_PlayerDataStorage_CopyFileMetadataAtIndex failed with result=[%ls]", *LexToString(Result));
					Op.SetError(Errors::FromEOSResult(Result));
					return;
				}
...
TOnlineAsyncOpHandle<FUserFileReadFile> FUserFileEOSGS::ReadFile(FUserFileReadFile::Params&& InParams)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FUserFileReadFile>& Op, const EOS_PlayerDataStorage_ReadFileCallbackInfo* Data)
		{
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "EOS_PlayerDataStorage_ReadFile failed with result=[%ls]", *LexToString(Data->ResultCode));
				Op.SetError(Errors::FromEOSResult(Data->ResultCode));
			}
...
TOnlineAsyncOpHandle<FUserFileWriteFile> FUserFileEOSGS::WriteFile(FUserFileWriteFile::Params&& InParams)
{
	...
	.Then([this](TOnlineAsyncOp<FUserFileWriteFile>& Op, const EOS_PlayerDataStorage_WriteFileCallbackInfo* Data)
	{
		...
		{
			UE_LOGF(LogOnlineServices, Warning, "EOS_PlayerDataStorage_WriteFile failed with result=[%ls]", *LexToString(Data->ResultCode));
			Op.SetError(Errors::FromEOSResult(Data->ResultCode));
		}
...
TOnlineAsyncOpHandle<FUserFileCopyFile> FUserFileEOSGS::CopyFile(FUserFileCopyFile::Params&& InParams)
{
	...
	.Then([this](TOnlineAsyncOp<FUserFileCopyFile>& Op, const EOS_PlayerDataStorage_DuplicateFileCallbackInfo* Data)
	{
		...
		{
			UE_LOGF(LogOnlineServices, Warning, "EOS_PlayerDataStorage_DuplicateFile failed with result=[%ls]", *LexToString(Data->ResultCode));
			Op.SetError(Errors::FromEOSResult(Data->ResultCode));
		}
	})
	.Enqueue(GetSerialQueue());
...
TOnlineAsyncOpHandle<FUserFileDeleteFile> FUserFileEOSGS::DeleteFile(FUserFileDeleteFile::Params&& InParams)
{
	...
	.Then([this](TOnlineAsyncOp<FUserFileDeleteFile>& Op, const EOS_PlayerDataStorage_DeleteFileCallbackInfo* Data)
	{
		...
		{
			UE_LOGF(LogOnlineServices, Warning, "EOS_PlayerDataStorage_DeleteFile failed with result=[%ls]", *LexToString(Data->ResultCode));
			Op.SetError(Errors::FromEOSResult(Data->ResultCode));
		}
	})
	.Enqueue(GetSerialQueue());
...
EOS_PlayerDataStorage_EReadResult EOS_CALL FUserFileEOSGS::OnReadFileDataStatic(const EOS_PlayerDataStorage_ReadFileDataCallbackInfo* Data)
{
	...
	if (ensure(FileContentsPtr))
	{
		...
		UE_LOGF(LogOnlineServices, VeryVerbose, "EOS_PlayerDataStorage_ReadFile Progress Filename=[%ls] %d/%d", UTF8_TO_TCHAR(Data->Filename), FileContents.Num(), Data->TotalFileSizeBytes);
	}
...
{
	UE_LOGF(LogOnlineServices, VeryVerbose, "EOS_PlayerDataStorage_ReadFile Complete Filename=[%ls]", UTF8_TO_TCHAR(Data->Filename));
...
EOS_PlayerDataStorage_EWriteResult EOS_CALL FUserFileEOSGS::OnWriteFileDataStatic(const EOS_PlayerDataStorage_WriteFileDataCallbackInfo* Data, void* OutDataBuffer, uint32_t* OutDataWritten)
{
	...
	*OutDataWritten = BytesToWrite;
	...
	UE_LOGF(LogOnlineServices, VeryVerbose, "EOS_PlayerDataStorage_WriteFile Progress Filename=[%ls] %d/%d", UTF8_TO_TCHAR(Data->Filename), BytesWrittenTotal, FileContents.Num());
...
{
	UE_LOGF(LogOnlineServices, VeryVerbose, "EOS_PlayerDataStorage_WriteFile Complete Filename=[%ls]", UTF8_TO_TCHAR(Data->Filename));
...
{
	UE_LOGF(LogOnlineServices, VeryVerbose, "FUserFileEOSGS TransferProgress Filename=[%ls] %d/%d", UTF8_TO_TCHAR(Data->Filename), Data->BytesTransferred, Data->TotalFileSizeBytes);
}
