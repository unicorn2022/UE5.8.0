...
void FOnlineStoreIOS::QueryStoreCountry()
{
	TWeakPtr<FOnlineStoreIOS> WeakThis = AsWeak();
	[StoreKit2 getCountry: ^ (NSString* _Nonnull Country)
	{
		if (TSharedPtr<FOnlineStoreIOS> SharedThis = WeakThis.Pin())
		{
			const FString CountryString(Country);
			SharedThis->StoreCountry = CountryString;
			SharedThis->bIsStoreCountryInitialized.Store(true);
			
			UE_LOG_ONLINE_STOREV2(Verbose, TEXT("Obtained store country: %s"), *CountryString);
		}
	}];
}

FString FOnlineStoreIOS::GetStoreCountry() const
{
	if (bIsStoreCountryInitialized.Load())
	{
		return StoreCountry;
	}
	
	UE_LOG_ONLINE_STOREV2(Warning, TEXT("Attempting to get store country before it is initialized!"));
	return "";
}
