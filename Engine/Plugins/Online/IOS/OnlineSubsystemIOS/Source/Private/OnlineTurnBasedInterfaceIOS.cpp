	...
	, Match([_Match retain])
{
	if (!Match)
	{
		UE_LOGF(LogTurnBasedInterfaceIOS, Error, "GKTurnBasedMatch required to create a FTurnBasedMatchIOS");
	}
...
int32 FTurnBasedMatchIOS::GetLocalPlayerIndex() const
{
	...
	{
		UE_LOGF(LogTurnBasedInterfaceIOS, Warning, "No Online Identity");
		return 0;
	}
...
int32 FTurnBasedMatchIOS::GetPlayerIndexForPlayer(NSString* PlayerID) const
{
	...
	UE_LOGF(LogTurnBasedInterfaceIOS, Warning, "Failed to find participant %ls in match", StringCast<TCHAR>(reinterpret_cast<const UTF8CHAR*>(PlayerID.UTF8String)).Get());
	return 0;
}
...
void FOnlineTurnBasedIOS::LoadAllMatches(FLoadTurnBasedMatchesSignature MatchesLoadedCallback)
{
	...
	{
		UE_LOGF(LogTurnBasedInterfaceIOS, Warning, "Requesting load all matches whilst we are still loading matches");
		return;
	}
