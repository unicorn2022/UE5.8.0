...
bool FOnlineSubsystemIOS::Init() 
{
	...
	else
	{
		...
		if (IsInAppPurchasingEnabled())
		{
			...
			EMarketplaceType CurrentMarketplace = FMarketplaceKitModule::GetCurrentTypeStatic();
			if (CurrentMarketplace == EMarketplaceType::AppStore || 
				CurrentMarketplace == EMarketplaceType::TestFlight ||
				CurrentMarketplace == EMarketplaceType::Marketplace)
#endif
			{
		...
		if (StoreV2Interface)
		{
			// Initialize the store country
			StoreV2Interface->QueryStoreCountry();
		}
	}
