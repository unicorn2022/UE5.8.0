...
-(TOptional<FOnlinePurchaseTransactionIOS>)TryTranslateTransaction: (SKPaymentTransaction*)Transaction withReceiptData: (TSharedPtr<FString>)ReceiptData
{
    ...
    case SKPaymentTransactionStateDeferred:
        ...
        break;
    default:
        UE_LOG_ONLINE_PURCHASE(Warning, TEXT("FStoreKitPurchaseProxy::TryTranslateTransaction unknown transaction state: %lld"), [Transaction transactionState]);
        break;
    }
...
void FOnlinePurchaseIOS::GetAppReceipt(TFunction<void(const FString&)> OnComplete)
{
	AsyncTask(ENamedThreads::AnyBackgroundThreadNormalTask, [OnComplete]()
	{
		FString ReceiptData = OnlinePurchaseIOSPrivate::ConvertReceiptToString();

		AsyncTask(ENamedThreads::GameThread, [OnComplete, ReceiptData]()
		{
...
bool FOnlinePurchaseIOS::IsAllowedToPurchase(const FUniqueNetId& UserId, FOnlineError& Error)
{
	bool bResult = IsAllowedToPurchase(UserId);
	Error = FOnlineError(bResult ? EOnlineErrorResult::Success : EOnlineErrorResult::Unknown);
	return bResult;
}
...
#if !PLATFORM_TVOS && !PLATFORM_VISIONOS
void FOnlinePurchaseIOS::RequestExternalPurchaseToken(const FOnRequestExternalPurchaseTokenComplete& Delegate) const
{
	FMarketplaceKitModule::RequestCTTokenAsync(Delegate);
}
#endif // !PLATFORM_TVOS && !PLATFORM_VISIONOS
