...
VOICECHAT_API const TCHAR* LexToString(const EVoiceChatChannelType Value);
...
VOICECHAT_API const TCHAR* LexToString(const EVoiceChatAttenuationModel Value);
...
VOICECHAT_API const TCHAR* LexToString(const EVoiceChatTransmitMode Value);
...
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnVoiceChatCallStatsUpdatedDelegate2, const FString& /* ChannelName */, const FVoiceChatCallStats& /* CallStats */);
...
{
public:
	virtual ~IVoiceChatUser() = default;
	 ...
	 */
	UE_DEPRECATED(5.8, "Please use OnVoiceChatCallStatsUpdated2 which adds the ChannelName param")
	virtual FOnVoiceChatCallStatsUpdatedDelegate& OnVoiceChatCallStatsUpdated() = 0;

	/**
	 * Delegate triggered when a call ends on a specific channel, providing the cumulative stats for
	 * the call. Replaces OnVoiceChatCallStatsUpdated to allow per-channel routing in implementations
	 * that may have multiple concurrent voice rooms.
	 */
	virtual FOnVoiceChatCallStatsUpdatedDelegate2& OnVoiceChatCallStatsUpdated2() = 0;

	/**
	 * Returns the current cumulative call-quality stats for the named channel, if available, or an
	 * empty optional if the channel is not currently joined or if the implementation does not produce
	 * stats. The values are running totals since the channel was joined.
	 */
	virtual TOptional<FVoiceChatCallStats> GetChannelCallStats(const FString& ChannelName) const = 0;
	...
	virtual FDelegateHandle RegisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate(const FOnVoiceChatBeforeRecvAudioRenderedDelegate::FDelegate& Delegate) = 0;
	virtual void UnregisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate(FDelegateHandle Handle) = 0;
	
	virtual FDelegateHandle RegisterOnVoiceChatBeforeRecvUnmixedAudioRenderedDelegate(const FOnVoiceChatBeforeRecvAudioRenderedDelegate::FDelegate& Delegate) = 0;
	virtual void UnregisterOnVoiceChatBeforeRecvUnmixedAudioRenderedDelegate(FDelegateHandle Handle) = 0;

	UE_DEPRECATED(5.7, "Please use RegisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate instead. Also now possible to use RegisterOnVoiceChatBeforeRecvUnmixedAudioRenderedDelegate for receiving unmixed audio")
	virtual FDelegateHandle RegisterOnVoiceChatBeforeRecvAudioRenderedDelegate(const FOnVoiceChatBeforeRecvAudioRenderedDelegate::FDelegate& Delegate) final
	{
		return RegisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate(Delegate);
	}
	UE_DEPRECATED(5.7, "Please use UnregisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate instead. Also now possible to use UnregisterOnVoiceChatBeforeRecvUnmixedAudioRenderedDelegate for unmixed audio")
	virtual void UnregisterOnVoiceChatBeforeRecvAudioRenderedDelegate(FDelegateHandle Handle) final
	{
		UnregisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate(Handle);
	}
