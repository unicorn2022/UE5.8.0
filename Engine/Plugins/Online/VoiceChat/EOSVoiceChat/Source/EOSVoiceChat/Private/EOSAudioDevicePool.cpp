...
void FEOSAudioDevicePool::RefreshAudioDevices(const FOnAudioDevicePoolRefreshAudioDevicesCompleteDelegate& Delegate)
{
	...
	RefreshAudioInputDevices(FOnAudioDevicePoolRefreshAudioInputDevicesCompleteDelegate::CreateLambda([RefreshAudioDevicesState](const FVoiceChatResult& InResult) -> void
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "RefreshAudioInputDevicesCompletionDelegate failed, Result=[%ls]", *LexToString(InResult));
	...
	RefreshAudioOutputDevices(FOnAudioDevicePoolRefreshAudioOutputDevicesCompleteDelegate::CreateLambda([RefreshAudioDevicesState](const FVoiceChatResult& InResult) -> void
	{
		if (!InResult.IsSuccess())
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "RefreshAudioOutputDevicesCompletionDelegate failed, Result=[%ls]", *LexToString(InResult));
...
void FEOSAudioDevicePool::RefreshAudioInputDevices(const FOnAudioDevicePoolRefreshAudioInputDevicesCompleteDelegate& Delegate)
{
	...
	UE_LOGF(LogEOSVoiceChat, Verbose, "RefreshAudioInputDevices InputDeviceInfos=[%ls] DefaultInputDeviceInfoIdx=[%d]", *FString::JoinBy(CachedInputDeviceInfos, TEXT(", "), &FVoiceChatDeviceInfo::ToDebugString), DefaultInputDeviceInfoIdx);
...
void FEOSAudioDevicePool::RefreshAudioOutputDevices(const FOnAudioDevicePoolRefreshAudioOutputDevicesCompleteDelegate& Delegate)
{
	...
	UE_LOGF(LogEOSVoiceChat, Verbose, "RefreshAudioOutputDevices OutputDeviceInfos=[%ls] DefaultOutputDeviceInfoIdx=[%d]", *FString::JoinBy(CachedOutputDeviceInfos, TEXT(", "), &FVoiceChatDeviceInfo::ToDebugString), DefaultOutputDeviceInfoIdx);
...
TArray<FVoiceChatDeviceInfo> FEOSAudioDevicePool::GetRtcInputDeviceInfos(int32& OutDefaultDeviceIdx) const
{
	...
	for (uint32_t Index = 0; Index < Count; Index++)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "EOS_RTCAudio_GetAudioInputDeviceByIndex failed: DevicesInfo=nullptr");
		}
	}

	if (Count == 0)
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "EOS_RTCAudio_GetAudioInputDevicesCount failed: DevicesCount=0");
	}
...
TArray<FVoiceChatDeviceInfo> FEOSAudioDevicePool::GetRtcOutputDeviceInfos(int32& OutDefaultDeviceIdx) const
{
	...
	for (uint32_t Index = 0; Index < Count; Index++)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "EOS_RTCAudio_GetAudioOutputDeviceByIndex failed: DevicesInfo=nullptr");
		}
	}

	if (Count == 0)
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "EOS_RTCAudio_GetAudioOutputDevicesCount failed: DevicesCount=0");
	}
