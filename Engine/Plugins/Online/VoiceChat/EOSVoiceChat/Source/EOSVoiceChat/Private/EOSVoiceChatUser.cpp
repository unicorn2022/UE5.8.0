...
void FEOSVoiceChatUser::BindChannelCallbacks(FChannelSession& ChannelSession)
{
	...
	// OnAudioBeforeRender (Mixed)
	{
		...
		ChannelSession.OnMixedAudioBeforeRenderNotificationId = EOS_RTCAudio_AddNotifyAudioBeforeRender(EOS_RTC_GetAudioInterface(GetRtcInterface()), &AudioBeforeRenderOptions, this, &FEOSVoiceChatUser::OnChannelMixedAudioBeforeRenderStatic);
		if (ChannelSession.OnMixedAudioBeforeRenderNotificationId == EOS_INVALID_NOTIFICATIONID)
		{
			EOSVOICECHATUSER_LOG(Warning, TEXT("BindChannelCallbacks EOS_RTCAudio_AddNotifyAudioBeforeRender (Mixed) failed"));
		}
	}
	
	// OnAudioBeforeRender (Unmixed)
	{
		EOS_RTCAudio_AddNotifyAudioBeforeRenderOptions AudioBeforeRenderOptions = {};
		AudioBeforeRenderOptions.ApiVersion = 1;
		UE_EOS_CHECK_API_MISMATCH(EOS_RTCAUDIO_ADDNOTIFYAUDIOBEFORERENDER_API_LATEST, 1);
		...
		ChannelSession.OnUnmixedAudioBeforeRenderNotificationId = EOS_RTCAudio_AddNotifyAudioBeforeRender(EOS_RTC_GetAudioInterface(GetRtcInterface()), &AudioBeforeRenderOptions, this, &FEOSVoiceChatUser::OnChannelUnmixedAudioBeforeRenderStatic);
		if (ChannelSession.OnUnmixedAudioBeforeRenderNotificationId == EOS_INVALID_NOTIFICATIONID)
		{
			EOSVOICECHATUSER_LOG(Warning, TEXT("BindChannelCallbacks EOS_RTCAudio_AddNotifyAudioBeforeRender (Unmixed) failed"));
		}
	}
...
void FEOSVoiceChatUser::UnbindChannelCallbacks(FChannelSession& ChannelSession)
{
	...
	if (ChannelSession.OnMixedAudioBeforeRenderNotificationId != EOS_INVALID_NOTIFICATIONID)
	{
		EOS_RTCAudio_RemoveNotifyAudioBeforeRender(EOS_RTC_GetAudioInterface(GetRtcInterface()), ChannelSession.OnMixedAudioBeforeRenderNotificationId);
		ChannelSession.OnMixedAudioBeforeRenderNotificationId = EOS_INVALID_NOTIFICATIONID;
	}
	
	if (ChannelSession.OnUnmixedAudioBeforeRenderNotificationId != EOS_INVALID_NOTIFICATIONID)
	{
		EOS_RTCAudio_RemoveNotifyAudioBeforeRender(EOS_RTC_GetAudioInterface(GetRtcInterface()), ChannelSession.OnUnmixedAudioBeforeRenderNotificationId);
		ChannelSession.OnUnmixedAudioBeforeRenderNotificationId = EOS_INVALID_NOTIFICATIONID;
	}
...
void EOS_CALL FEOSVoiceChatUser::OnJoinRoomStatic(const EOS_RTC_JoinRoomCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnJoinRoomStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnJoinRoomStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnLeaveRoomStatic(const EOS_RTC_LeaveRoomCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnLeaveRoomStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnLeaveRoomStatic Error CallbackInfo=nullptr")
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnBlockParticipantStatic(const EOS_RTC_BlockParticipantCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnBlockUserStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnBlockUserStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnUpdateParticipantVolumeStatic(const EOS_RTCAudio_UpdateParticipantVolumeCallbackInfo* CallbackInfo)
{
	...
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnUpdateParticipantVolumeStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnUpdateParticipantVolumeStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnUpdateReceivingAudioStatic(const EOS_RTCAudio_UpdateReceivingCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnUpdateReceivingStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnUpdateReceivingStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnChannelDisconnectedStatic(const EOS_RTC_DisconnectedCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelDisconnectedStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelDisconnectedStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnLobbyChannelConnectionChangedStatic(const EOS_Lobby_RTCRoomConnectionChangedCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnLobbyChannelConnectionChangedStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnLobbyChannelConnectionChangedStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnChannelParticipantStatusChangedStatic(const EOS_RTC_ParticipantStatusChangedCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelParticipantStatusChangedStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelParticipantStatusChangedStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnChannelParticipantAudioUpdatedStatic(const EOS_RTCAudio_ParticipantUpdatedCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelParticipantAudioUpdatedStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelParticipantAudioUpdatedStatic Error CallbackInfo=nullptr");
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnChannelMixedAudioBeforeRenderStatic(const EOS_RTCAudio_AudioBeforeRenderCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		if (FEOSVoiceChatUser* EosVoiceChatPtr = static_cast<FEOSVoiceChatUser*>(CallbackInfo->ClientData))
		{
			EosVoiceChatPtr->OnChannelMixedAudioBeforeRender(CallbackInfo);
		}
		else
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelMixedAudioBeforeRenderStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelMixedAudioBeforeRenderStatic Error CallbackInfo=nullptr");
	}
}

void EOS_CALL FEOSVoiceChatUser::OnChannelUnmixedAudioBeforeRenderStatic(const EOS_RTCAudio_AudioBeforeRenderCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		if (FEOSVoiceChatUser* EosVoiceChatPtr = static_cast<FEOSVoiceChatUser*>(CallbackInfo->ClientData))
		{
			EosVoiceChatPtr->OnChannelUnmixedAudioBeforeRender(CallbackInfo);
		}
		else
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelUnmixedAudioBeforeRenderStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelUnmixedAudioBeforeRenderStatic Error CallbackInfo=nullptr");
	}
}

void FEOSVoiceChatUser::OnChannelMixedAudioBeforeRender(const EOS_RTCAudio_AudioBeforeRenderCallbackInfo* CallbackInfo)
{
	if (EOS_RTCAudio_AudioBuffer* Buffer = CallbackInfo->Buffer)
	{
		if (Buffer->Frames)
		{
			...
			FScopeLock Lock(&BeforeRecvMixedAudioRenderedLock);
			OnVoiceChatBeforeRecvMixedAudioRenderedDelegate.Broadcast(Samples, Buffer->SampleRate, Buffer->Channels, bIsSilence, ChannelName, PlayerName);
		}
		else
		{
			EOSVOICECHATUSER_LOG(Warning, TEXT("OnChannelMixedAudioBeforeRender Error Frames=nullptr"));
		}
	}
	else
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("OnChannelMixedAudioBeforeRender Error Buffer=nullptr"));
	}
}

void FEOSVoiceChatUser::OnChannelUnmixedAudioBeforeRender(const EOS_RTCAudio_AudioBeforeRenderCallbackInfo* CallbackInfo)
{
	if (EOS_RTCAudio_AudioBuffer* Buffer = CallbackInfo->Buffer)
	{
		if (Buffer->Frames)
		...
		{
			EOSVOICECHATUSER_LOG(Warning, TEXT("OnChannelUnmixedAudioBeforeRender Error Frames=nullptr"));
		}
	}
	else
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("OnChannelUnmixedAudioBeforeRender Error Buffer=nullptr"));
	}
}
...
void EOS_CALL FEOSVoiceChatUser::OnChannelAudioInputStateStatic(const EOS_RTCAudio_AudioInputStateCallbackInfo* CallbackInfo)
{
	if (CallbackInfo)
	{
		...
		{
			UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelAudioInputStateStatic Error EosVoiceChatPtr=nullptr");
		}
	}
	else
	{
		UE_LOGF(LogEOSVoiceChat, Warning, "OnChannelAudioInputStateStatic Error CallbackInfo=nullptr");
	}
}
...
FEOSVoiceChatUser::FChannelSession::~FChannelSession()
{
	...
	check(OnMixedAudioBeforeRenderNotificationId == EOS_INVALID_NOTIFICATIONID);
	check(OnUnmixedAudioBeforeRenderNotificationId == EOS_INVALID_NOTIFICATIONID);
