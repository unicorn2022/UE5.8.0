...
public:
	...
	UE_DEPRECATED(5.8, "Use FEOSVoiceChat(IEOSSDKManager&) and call SetPlatformHandle() instead")
	...
	// Set the platform config name to use during Initialize(). Only used if no platform handle is set.
	void SetPlatformConfigName(const FString& PlatformConfigName) { OverridePlatformConfigName = PlatformConfigName; }

	// Set an existing platform handle to use during Initialize(). If set, Initialize() will use this
	// handle instead of creating one via named config or inline credentials
	void SetPlatformHandle(const IEOSPlatformHandlePtr& PlatformHandle) { EosPlatformHandle = PlatformHandle; }
...
protected:
	...
	// Override for platform config name, set via SetPlatformConfigName(). If set, Initialize() uses this instead of reading from config file
	TOptional<FString> OverridePlatformConfigName;

#if !UE_BUILD_SHIPPING
	uint8 InitializationPath = 0;
