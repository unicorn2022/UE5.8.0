...
{
	UE_LOGF(LogWebAuth, Display, "FWebAuthModule::StartupModule: Starting WebAuth Module");
	Singleton = this;
	...
	if (WebAuth == nullptr)
	{
		...
		UE_LOGF(LogWebAuth, Display, "FWebAuthModule::StartupModule: WebAuth Module not implemented on this platform");
	}
}

void FWebAuthModule::ShutdownModule()
{
	UE_LOGF(LogWebAuth, Display, "FWebAuthModule::ShutdownModule:");
...
// TODO: Support params, but use test case
	...
	if (FParse::Command(&Cmd, TEXT("Session")))
	{
		...
		{
			UE_LOGF(LogWebAuth, Warning, "usage: WebAuth Session <url> <app scheme>");
		}
		else
		{
			if (IsAvailable())
			{
				UE_LOGF(LogWebAuth, Display, "Starting auth session");
				...
				GetWebAuth().AuthSessionWithURL(
					...
					UE_LOGF(LogWebAuth, Display, "WebAuth Session RedirectURL=[%ls], bHasResponse=[%d]", *RedirectURL, bHasResponse);
				}));
			}
			else
			{
				UE_LOGF(LogWebAuth, Warning, "WebAuth not available");
			}
		}
	}
...
{
	UE_LOGF(LogWebAuth, Display, "FWebAuthModule::Get:");
