...
void MigrateCredentialsData(NSString* EnvironmentName, NSData* CredentialsData)
{
	...
	if (Status == errSecSuccess)
	{
		...
		UE_LOGF(LogWebAuth, Verbose, "Migrated credentials to new location.");
	}
	else
	{
		UE_LOGF(LogWebAuth, Warning, "Failed to migrate credentials. Status = %d", Status);
	}
...
bool FIOSWebAuth::SaveCredentials(const FString& IdStr, const FString& TokenStr, const FString& EnvironmentNameStr)
{
	...
	{
		UE_LOGF(LogWebAuth, Verbose, "Cleared existing credentials");
		return true;
	}
	...
	if (Status == errSecSuccess)
	{
		UE_LOGF(LogWebAuth, Verbose, "Saved credentials");
	}
	else
	{
		UE_LOGF(LogWebAuth, Warning, "Failed to save credentials. Status = %d", Status);
	}
...
bool FIOSWebAuth::LoadCredentials(FString& OutIdStr, FString& OutTokenStr, const FString& EnvironmentNameStr)
{
	...
	if (Result)
	{
		...
		if ([CredentialsArray count] == 2 && [[CredentialsArray objectAtIndex:0] isKindOfClass:[NSString class]] && [[CredentialsArray objectAtIndex:1] isKindOfClass:[NSString class]])
		{
			...
			UE_LOGF(LogWebAuth, Verbose, "Retrieved credentials successfully");
		}
		else
		{
			UE_LOGF(LogWebAuth, Warning, "Retrieved credentials, but they were poorly formatted, failing.");
			Status = errSecParam;
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogWebAuth, Log, "Failed to retrieve credentials, no credentials found");
		}
		else
		{
			UE_LOGF(LogWebAuth, Warning, "Failed to retrieve credentials, status = %d", Status);
		}
	}
