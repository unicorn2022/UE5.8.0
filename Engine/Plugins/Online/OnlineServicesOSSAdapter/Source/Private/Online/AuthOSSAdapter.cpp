...
void FAccountInfoRegistryOSSAdapter::Unregister(FAccountId AccountId)
{
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "[FAccountInfoRegistryOSSAdapter::Unregister] Failed to find account [%ls].", *ToLogString(AccountId));
	}
}
...
TOnlineAsyncOpHandle<FAuthLogin> FAuthOSSAdapter::Login(FAuthLogin::Params&& Params)
{
		...
		[this, AccountInfoOSSAdapter, WeakOp = InAsyncOp.AsWeak(), Promise = MoveTemp(Promise)](int32 LocalUserNum, bool bWasSuccessful, const FUniqueNetId& UserId, const FString& Error) mutable
		{
			if (TSharedPtr<TOnlineAsyncOp<FAuthLogin>> Op = WeakOp.Pin())
			{
				if (bWasSuccessful)
				{
					...
					if (!UserId.IsValid())
					{
						UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::Login][%ls] Failure: Userid was invalid.", *GetSubsystem().GetSubsystemName().ToString());
				...
				{
					UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::Login][%ls] Failure: Failed to login to OSSv1 identity interface. Error %ls", *GetSubsystem().GetSubsystemName().ToString(), *Error);
	...
	.Then([this](TOnlineAsyncOp<FAuthLogin>& InAsyncOp)
	{
		...
		if (!PopulateAttributes(*AccountInfoOSSAdapter))
		{
			...
			[this, AccountInfoOSSAdapter, Promise = MoveTemp(Promise)](int32 LocalUserNum, bool bWasSuccessful) mutable
			{
				if (!bWasSuccessful)
				{
					UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::Login][%ls] Failed to logout account [%ls].", *GetSubsystem().GetSubsystemName().ToString(), *ToLogString(AccountInfoOSSAdapter->AccountId));
				}
				Promise.EmplaceValue();
			});
	...
	.Then([this](TOnlineAsyncOp<FAuthLogin>& InAsyncOp)
	{
		...
		UE_LOGF(LogOnlineServices, Log, "[FAuthOSSAdapter::Login][%ls] Successfully logged in as [%ls]", *GetSubsystem().GetSubsystemName().ToString(), *ToLogString(AccountInfoOSSAdapter->AccountId));
...
TOnlineAsyncOpHandle<FAuthLogout> FAuthOSSAdapter::Logout(FAuthLogout::Params&& Params)
{
	...
	.Then([this](TOnlineAsyncOp<FAuthLogout>& InAsyncOp)
	{
		...
		[this, AccountInfoOSSAdapter, Promise = MoveTemp(Promise)](int32 LocalUserNum, bool bWasSuccessful) mutable
		{
			if (!bWasSuccessful)
			{
				UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::Logout][%ls] Failed to logout account [%ls].", *GetSubsystem().GetSubsystemName().ToString(), *ToLogString(AccountInfoOSSAdapter->AccountId));
			}
			Promise.EmplaceValue();
		});
	...
	.Then([this](TOnlineAsyncOp<FAuthLogout>& InAsyncOp)
	{
		...
		UE_LOGF(LogOnlineServices, Log, "[FAuthOSSAdapter::Logout][%ls] Successfully logged out [%ls]", *GetSubsystem().GetSubsystemName().ToString(), *ToLogString(AccountInfoOSSAdapter->AccountId));
		AccountInfoOSSAdapter->LoginStatus = ELoginStatus::NotLoggedIn;
		OnAuthLoginStatusChangedEvent.Broadcast(FAuthLoginStatusChanged{ AccountInfoOSSAdapter, AccountInfoOSSAdapter->LoginStatus });
				...
				if (TSharedPtr<TOnlineAsyncOp<FAuthQueryExternalAuthToken>> Op = WeakOp.Pin())
				{
					if (bWasSuccessful)
					{
						if (EnumHasAnyFlags(ExternalAuthTokenTranslationTraits->Flags, EExternalAuthTokenTranslationFlags::TokenString))
						{
							...
							{
								UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::QueryExternalAuthToken][%ls] Failed: Token string not found.", *GetSubsystem().GetSubsystemName().ToString());
								Op->SetError(Errors::Unknown());
								Promise.EmplaceValue();
							}
						}
						else if (EnumHasAnyFlags(ExternalAuthTokenTranslationTraits->Flags, EExternalAuthTokenTranslationFlags::TokenBinary))
						{
							...
							{
								UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::QueryExternalAuthToken][%ls] Failed: Token binary data not found.", *GetSubsystem().GetSubsystemName().ToString());
								Op->SetError(Errors::Unknown());
								Promise.EmplaceValue();
							}
						}
					...
					{
						UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::QueryExternalAuthToken][%ls] GetLinkedAccountAuthToken failed.", *GetSubsystem().GetSubsystemName().ToString());
						Op->SetError(Errors::Unknown());
						Promise.EmplaceValue();
					}
				}
...
TOnlineAsyncOpHandle<FAuthHandleLoginStatusChangedImpl> FAuthOSSAdapter::HandleLoginStatusChangedImplOp(FAuthHandleLoginStatusChangedImpl::Params&& Params)
{
	...
	Op->Then([this](TOnlineAsyncOp<FAuthHandleLoginStatusChangedImpl>& InAsyncOp)
	{
		...
		{
			UE_LOGF(LogOnlineServices, Log, "[FAuthOSSAdapter::HandleLoginStatusChangedImplOp][%ls] Login status has not changed. Ignoring event. [%ls] Current Status: %ls",
	...
	.Then([this](TOnlineAsyncOp<FAuthHandleLoginStatusChangedImpl>& InAsyncOp)
	{
		...
		UE_LOGF(LogOnlineServices, Log, "[FAuthOSSAdapter::HandleLoginStatusChangedImplOp][%ls] Login status changed for account [%ls]: %ls => %ls.",
...
bool FAuthOSSAdapter::PopulateAttributes(FAccountInfoOSSAdapter& AccountInfoOSSAdapter) const
{
	...
	UE_LOGF(LogOnlineServices, Warning, "[FAuthOSSAdapter::Login][%ls] Failure: Failed to find UserOnlineAccount for account %ls", *GetSubsystem().GetSubsystemName().ToString(), *ToLogString(AccountInfoOSSAdapter.AccountId));
