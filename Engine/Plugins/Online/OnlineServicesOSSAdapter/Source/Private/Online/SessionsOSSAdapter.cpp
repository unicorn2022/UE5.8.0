...
FSchemaVariant GetV2SessionVariant(const FVariantData& InValue)
{
	...
	default:
		UE_LOGF(LogOnlineServices, Warning, "[GetV2SessionVariant] FVariantData type not supported by FSessionsVariant. No value was set.");
		break;
	}
...
FVariantData GetV1VariantData(const FSchemaVariant& InValue)
{
	...
	case ESchemaAttributeType::String:
		...
		break;
	default:
		UE_LOGF(LogOnlineServices, Warning, "[GetV1VariantData] FSchemaVariant type not supported by FVariantData. No value was set.");
		break;
	}
...
void FSessionOSSAdapter::DumpState() const
{
	...
	UE_LOGF(LogOnlineServices, Log, "V1Session: SessionId [%ls]", *V1Session.GetSessionIdStr());
}
...
TFuture<TOnlineResult<FUpdateSessionSettings>> FSessionsOSSAdapter::UpdateSessionSettingsImpl(const FUpdateSessionSettings::Params& Params)
{
	...
	FOnlineSessionSettings UpdatedV1Settings = BuildV1SettingsForUpdate(Params.LocalAccountId, FoundSession, UpdatedV2Settings);
...
TFuture<TOnlineResult<FFindSessions>> FSessionsOSSAdapter::FindSessionsImpl(const FFindSessions::Params& Params)
{
	...
	if (Params.SessionId.IsSet())
	{
		...
		{
			UE_LOGF(LogOnlineServices, Verbose, "[FSessionsOSSAdapter::FindSessionsImpl] UniqueNetId could not be found for SessionId %ls", *ToLogString(*Params.SessionId));
...
TOptional<FOnlineError> FSessionsOSSAdapter::CheckState(const FFindSessions::Params& Params) const
{
	...
	{
		UE_LOGF(LogOnlineServices, Verbose, "[FSessionsOSSAdapter::FindSessionsImpl] UniqueId not found for LocalAccountId %ls", *ToLogString(Params.LocalAccountId));
	...
	{
		UE_LOGF(LogOnlineServices, Verbose, "[FSessionsOSSAdapter::FindSessionsImpl] Session search already in progress for LocalAccountId %ls", *ToLogString(Params.LocalAccountId));
...
TOptional<FOnlineError> FSessionsOSSAdapter::CheckState(const FJoinSession::Params& Params) const
{
	...
	if (!LocalAccountId->IsValid())
	{
		UE_LOGF(LogOnlineServices, Verbose, "[%ls] UniqueId not found for LocalAccountId %ls", UTF8_TO_TCHAR(__FUNCTION__), *ToLogString(Params.LocalAccountId));
...
TOnlineResult<FGetResolvedConnectString> FSessionsOSSAdapter::GetResolvedConnectString(const FGetResolvedConnectString::Params& Params)
{
	...
	else
	{
		...
		UE_LOGF(LogOnlineServices, Verbose, "[FSessionsOSSAdapter::GetResolvedConnectString] Invalid SessionId %ls", *ToLogString(Params.SessionId));
...
FOnlineSessionSettings FSessionsOSSAdapter::BuildV1SettingsForUpdate(const FAccountId& LocalAccountId, const TSharedRef<ISession>& Session, const FSessionSettings& SessionSettings) const
{
	FOnlineSessionSettings Result;

	Result.bAllowInvites = SessionSettings.bAllowNewMembers;
	Result.bAllowJoinInProgress = SessionSettings.bAllowNewMembers;
	Result.bAllowJoinViaPresence = SessionSettings.bAllowNewMembers;
	Result.bAllowJoinViaPresenceFriendsOnly = SessionSettings.bAllowNewMembers;
	...
	Result.bShouldAdvertise = SessionSettings.bAllowNewMembers;
	if (const FCustomSessionSetting* BuildUniqueId = SessionSettings.CustomSettings.Find(OSS_ADAPTER_SESSIONS_BUILD_UNIQUE_ID))
	{
		Result.BuildUniqueId = BuildUniqueId->Data.GetInt64();
	}
	if (const FCustomSessionSetting* BuildUniqueId = SessionSettings.CustomSettings.Find(OSS_ADAPTER_SESSIONS_USE_LOBBIES_IF_AVAILABLE))
	{
		Result.bUseLobbiesIfAvailable = BuildUniqueId->Data.GetBoolean();
	}
	if (const FCustomSessionSetting* BuildUniqueId = SessionSettings.CustomSettings.Find(OSS_ADAPTER_SESSIONS_USE_LOBBIES_VOICE_CHAT_IF_AVAILABLE))
	{
	...
	if (const FCustomSessionSetting* BuildUniqueId = SessionSettings.CustomSettings.Find(OSS_ADAPTER_SESSIONS_USES_STATS))
	{
		Result.bUsesStats = BuildUniqueId->Data.GetBoolean();
	}
	Result.NumPrivateConnections = SessionSettings.NumMaxConnections;
	Result.NumPublicConnections = SessionSettings.NumMaxConnections;
	Result.SessionIdOverride = Session->GetSessionInfo().SessionIdOverride;

	Result.Settings = GetV1SessionSettings(SessionSettings.CustomSettings);
	Result.Settings.Add(OSS_ADAPTER_SESSIONS_SCHEMA_NAME, SessionSettings.SchemaName.ToString());
	Result.Settings.Add(SETTING_SESSION_TEMPLATE_NAME, SessionSettings.SchemaName.ToString());
...
TSharedRef<FSessionCommon> FSessionsOSSAdapter::BuildV2Session(const FNamedOnlineSession* InSession) const
{
	...
	// Prefer LocalOwnerId (set by EOS, GDK, Steam), fall back to OwningUserId (set by all V1 subsystems including Null)
	if (InSession->LocalOwnerId)
	{
		Session->OwnerAccountId = ServicesOSSAdapter.GetAccountIdRegistry().FindOrAddHandle(InSession->LocalOwnerId->AsShared());
	}
	else if (InSession->OwningUserId)
	{
		Session->OwnerAccountId = ServicesOSSAdapter.GetAccountIdRegistry().FindOrAddHandle(InSession->OwningUserId->AsShared());
	}
