...
void FSocketSubsystemSteamIPModule::StartupModule()
{
	...
	if (SharedModule.AreSteamDllsLoaded() && bIsNotEditor)
	{
		...
		{
			UE_LOGF(LogSockets, Error, "FSocketSubsystemSteamIPModule: Could not initialize SteamSockets, got error: %ls", *Error);
			FSocketSubsystemSteam::Destroy();
		}
	}
	else if (bIsNotEditor)
	{
		UE_LOGF(LogSockets, Warning, "FSocketSubsystemSteamIPModule: Steam SDK %ls libraries not present at %ls or failed to load!", STEAM_SDK_VER, *SharedModule.GetSteamModulePath());
	}
	else
	{
		UE_LOGF(LogSockets, Log, "FSocketSubsystemSteamIPModule: Disabled for editor process.");
	}
}
