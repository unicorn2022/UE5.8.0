...
bool FLanBeacon::Init(int32 Port)
{
	...
	if (ListenSocket != NULL)
	{
		...
		{
			UE_LOGF(LogTemp, Error, "Failed to bind listen socket to addr (%ls) for LAN beacon",
				*ListenAddr->ToString(true));
		}
	}
	else
	{
		UE_LOGF(LogTemp, Error, "Failed to create listen socket for LAN beacon");
	}
...
int32 FLanBeacon::ReceivePacket(uint8* PacketData, int32 BufferSize)
{
	...
	if (ListenSocket != NULL)
	{
		...
		if (BytesRead > 0)
		{
			UE_LOGF(LogTemp, Verbose, "Received %d bytes from %ls", BytesRead, *SockAddr->ToString(true));
		}
	}
...
{
	int32 BytesSent = 0;
	UE_LOGF(LogTemp, Verbose, "Sending %d bytes to %ls", Length, *BroadcastAddr->ToString(true));
...
bool FLANSession::Host(FOnValidQueryPacketDelegate& QueryDelegate)
{
	...
	if (LanBeacon->Init(LanAnnouncePort))
	{
		...
		bSuccess = true;
		UE_LOGF(LogTemp, Verbose, "Listening for LAN beacon requests on %d",	LanAnnouncePort);
	}
	else
	{
		UE_LOGF(LogTemp, Warning, "Failed to init to LAN beacon %ls",	ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetSocketError());
	}
...
bool FLANSession::Search(FNboSerializeToBuffer& Packet, FOnValidResponsePacketDelegate& ResponseDelegate, FOnSearchingTimeoutDelegate& TimeoutDelegate)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "Failed to create socket for lan announce port %ls", ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetSocketError());
		bSuccess = false;
	}
	...
	if (LanBeacon && bSuccess)
	{
		...
		{
			UE_LOGF(LogTemp, Verbose, "Sent query packet...");
		...
		{
			UE_LOGF(LogTemp, Warning, "Failed to send discovery broadcast %ls", ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetSocketError());
			bSuccess = false;
		}
	}
		...
		else
		{
			if (LanBeaconState == ELanBeaconState::Searching)
			{
				...
				if (LanBeacon && (LanQueryRetryTimeLeft <= 0.f) && (LanQueryTimeLeft > LanQueryRetryTime))
				{
					...
					{
						UE_LOGF(LogTemp, Verbose, "Sent retry query packet...");
					}
					else
					{
						UE_LOGF(LogTemp, Warning, "Failed to send retry discovery broadcast %ls", ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetSocketError());
					}
...
bool FLANSession::BroadcastPacket(uint8* Packet, int32 Length)
{
	...
	if (LanBeacon)
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogTemp, Warning, "Failed to send broadcast packet %d", (int32)ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetLastErrorCode());
		}
	}
