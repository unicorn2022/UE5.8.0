...
FOnlineSubsystemDelegates::FOnDefaultOnlineSubsystemPreReloaded FOnlineSubsystemDelegates::OnDefaultOnlineSubsystemPreReloaded;
...
static IModuleInterface* LoadSubsystemModule(const FString& SubsystemName, const TMap<FString, FName>& ModuleRedirects)
{
	...
	if (bAttemptLoadModule)
	{
		...
		{
			UE_CLOGF(bPostStartupModule, LogOnline, Warning, "%s attempting to load module \"%ls\" after FOnlineSubsystemModule::StartupModule. This can result in shutdown issues due to the loaded module unloading before OSS itself. Please add the module to the [OnlineSubsystem] AdditionalModulesToLoad config array to fix.", __FUNCTION__, *ModuleName.ToString());
...
void FOnlineSubsystemModule::ProcessConfigDefinedSubsystems()
{
	...
	{
		UE_LOGF(LogOnline, Verbose, "ConfigDefinedPlatformServices: Associating OnlineSubsystem %ls with identifier %ls", *ServicePair.Value, *ServicePair.Key);
...
void FOnlineSubsystemModule::ProcessConfigDefinedModuleRedirects()
{
	...
	{
		UE_LOGF(LogOnline, Verbose, "ProcessConfigDefinedModuleRedirects: Associating module %ls with OnlineSubsystem %ls", *RedirectPair.Value, *RedirectPair.Key);
...
{
	if (IOnlineSubsystem* OSS = GetOnlineSubsystem(DefaultPlatformService))
	{
		FOnlineSubsystemDelegates::OnDefaultOnlineSubsystemPreReloaded.Broadcast(OSS);
	}
...
{
	FName SubsystemName, InstanceName;
	const FName KeyName = ParseOnlineSubsystemName(InSubsystemName, SubsystemName, InstanceName);
	if (!SubsystemName.IsNone())
	{
		IOnlineSubsystemPtr OnlineSubsystem;
		{
