...
FString IOnlineSubsystem::GetLocalPlatformName()
{
	...
	if (OnlinePlatform.IsEmpty())
	{
		// Only read from config once so we don't re-load ini files if they get unloaded. This value is expected to never change. Use the cvar if you need dynamic changing.
		static bool bReadFromConfig = false;
		static FString ConfigValue;
		if (!bReadFromConfig)
		{
			GConfig->GetString(TEXT("OnlineSubsystem"), TEXT("LocalPlatformName"), ConfigValue, GEngineIni);
			// Regardless of whether we found a string in config or not, do not read again
			bReadFromConfig = true;
		}
		OnlinePlatform = ConfigValue;
	}
...
bool CheckSubsystemDisabledByCommandLine(const FName& SubsystemName)
{
	...
	// Cache the results in a static map to avoid string allocations after the first query
	static TMap<FName, bool> DisableMap;
	static FRWLock DisableMapLock;

	// Check the cache first
	{
		UE::TReadScopeLock ReadScope(DisableMapLock);

		bool* FoundValue = DisableMap.Find(SubsystemName);
		if (FoundValue)
		{
			return *FoundValue;
		}
	...
	// Check the actual command line, add to cache
	bool bIsDisabledByCommandLine = FParse::Param(FCommandLine::Get(), *FString::Printf(TEXT("no%s"), *SubsystemName.ToString()));

	{
		UE::TWriteScopeLock WriteScope(DisableMapLock);
		DisableMap.Add(SubsystemName, bIsDisabledByCommandLine);
	}

	return bIsDisabledByCommandLine;
#else
	return false;
#endif
}

bool CheckSubsystemEnabledByConfig(const FName& SubsystemName, const FName& InstanceName, bool& bOutEnabled)
{
	// Map of [SubsystemName, InstanceName] to bEnabled config value, unset if the value is not specified.
	// Note that the subsystem is queried on it's own so there will always be a [SubsystemName, None] entry in the map
	static TMap<TTuple<FName, FName>, TOptional<bool>> EnableMap;
	static FRWLock EnableMapLock;

	TTuple<FName, FName> MapKeyNames = MakeTuple(SubsystemName, InstanceName);

	// Check the cache first
	{
		UE::TReadScopeLock ReadScope(EnableMapLock);
		TOptional<bool>* FoundEnableValue = EnableMap.Find(MapKeyNames);
		if (FoundEnableValue)
		{
			bool bFoundValueIsSet = FoundEnableValue->IsSet();
			if (bFoundValueIsSet)
			{
				bOutEnabled = FoundEnableValue->GetValue();
			}

			return bFoundValueIsSet;
		}
	}

	// The section name is different if an InstanceName is specified
	FString ConfigSection;
	if (InstanceName == NAME_None)
	{
		ConfigSection = FString::Printf(TEXT("OnlineSubsystem%s"), *SubsystemName.ToString());
	}
	else
	{
		ConfigSection = FString::Printf(TEXT("OnlineSubsystem%s %s"), *SubsystemName.ToString(), *InstanceName.ToString());
	}

	bool bFoundConfigValue = GConfig->GetBool(*ConfigSection, TEXT("bEnabled"), bOutEnabled, GEngineIni);

	// Only set the TOptional if a config value was found. Otherwise being unset means the config value was unspecified.
	TOptional<bool> MapValue;
	if (bFoundConfigValue)
	{
		MapValue = bOutEnabled;
	}

	// Write new value to the cache
	{
		UE::TWriteScopeLock WriteScope(EnableMapLock);
		EnableMap.Add(MapKeyNames, MapValue);
	}

	return bFoundConfigValue;
}

bool IOnlineSubsystem::IsEnabled(const FName& SubsystemName, const FName& InstanceName)
{
	bool bIsDisabledByCommandLine = CheckSubsystemDisabledByCommandLine(SubsystemName);
	if (!bIsDisabledByCommandLine)
	{
		// First check the subsystem by itself
		bool bIsEnabledByConfig = false;
		bool bConfigOptionExists = CheckSubsystemEnabledByConfig(SubsystemName, NAME_None, bIsEnabledByConfig);

		// Then check with the instance name if one is specified
		if (InstanceName != NAME_None)
		{
			bConfigOptionExists |= CheckSubsystemEnabledByConfig(SubsystemName, InstanceName, bIsEnabledByConfig);
		}
