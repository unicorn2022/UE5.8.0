...
{ \
	UE_LOGF(LogOnlinePurchase, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}

#define UE_CLOG_ONLINE_PURCHASE(Conditional, Verbosity, Format, ...) \
{ \
	UE_CLOGF(Conditional, LogOnlinePurchase, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}
...
/**
 * Delegate called when the information dialog shows and completes
 */
DECLARE_DELEGATE_OneParam(FOnShowAlternativeBillingInformationDialogComplete, const FOnlineError& /*Result*/);
...
* delegate which is executed when external purchase token request completes
*/
DECLARE_DELEGATE_TwoParams(FOnRequestExternalPurchaseTokenComplete, bool /*bWasSuccessful*/, FString /*Output*/);
...
{
public:
	virtual ~IOnlinePurchase() = default;
	 ...
	 *
	 * @return true if user is allowed to make a purchase
	 */
	 ...
	 * @param UserId user initiating the request
	 * 
	 * @param Error an error result indicating why the the user isn't allowed when relevant and available.
	 *
	 * @return true if user is allowed to make a purchase
	 */
	virtual bool IsAllowedToPurchase(const FUniqueNetId& UserId, FOnlineError& Error) = 0;
	...
	/**
	 * If the underlying IAP service requires an informational dialog to be shown, show it and trigger 
	 * the delegate on completion with the status
	 * @param Delegate completion callback (guaranteed to be called)
	 */
	virtual void ShowAlternativeBillingInformationDialog(const FOnShowAlternativeBillingInformationDialogComplete& Delegate) {}

	
	/**
	* If the underlying IAP service supports external purchase tokens, request external purchase token
	*
	* @param Delegate completion callback
	*/
	virtual void RequestExternalPurchaseToken(const FOnRequestExternalPurchaseTokenComplete& Delegate) const { Delegate.ExecuteIfBound(false, ""); }
