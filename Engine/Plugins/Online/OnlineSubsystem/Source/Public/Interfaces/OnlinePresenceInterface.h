...
{ \
	UE_LOGF(LogOnlinePresence, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}

#define UE_CLOG_ONLINE_PRESENCE(Conditional, Verbosity, Format, ...) \
{ \
	UE_CLOGF(Conditional, LogOnlinePresence, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}
...
namespace EOnlinePresenceState
{
	 ...
	 */
	ONLINESUBSYSTEM_API const TCHAR* ToString(EOnlinePresenceState::Type EnumVal);

	/**
	 * @return EOnlinePresenceState from the string passed in
	 */
	ONLINESUBSYSTEM_API EOnlinePresenceState::Type FromString(const TCHAR* StringVal);

	UE_DEPRECATED(5.8, "Use ToLocText(EOnlinePresenceState::Online)")
	extern ONLINESUBSYSTEM_API const FText OnlineText;
	UE_DEPRECATED(5.8, "Use ToLocText(EOnlinePresenceState::Offline)")
	extern ONLINESUBSYSTEM_API const FText OfflineText;
	UE_DEPRECATED(5.8, "Use ToLocText(EOnlinePresenceState::Away)")
	extern ONLINESUBSYSTEM_API const FText AwayText;
	UE_DEPRECATED(5.8, "Use ToLocText(EOnlinePresenceState::DoNotDisturb)")
	extern ONLINESUBSYSTEM_API const FText DoNotDisturbText;
	UE_DEPRECATED(5.8, "Use ToLocText(EOnlinePresenceState::Chat)")
	extern ONLINESUBSYSTEM_API const FText ChatText;

	/** 
	 * @return the loc text version of the enum passed in 
	 */
	ONLINESUBSYSTEM_API const FText ToLocText(EOnlinePresenceState::Type EnumVal);
}
...
public:
	...
	virtual ~IOnlinePresence() = default;
