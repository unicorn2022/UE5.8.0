...
{ \
	UE_LOGF(LogOnlineExternalUI, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}

#define UE_CLOG_ONLINE_EXTERNALUI(Conditional, Verbosity, Format, ...) \
{ \
	UE_CLOGF(Conditional, LogOnlineExternalUI, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}
...
public:
	virtual ~IOnlineExternalUI() = default;
	...
	{
		UE_LOGF(LogOnlineExternalUI, Warning, "ShowSendMessageToUserUI is not implemented on this platform!");
