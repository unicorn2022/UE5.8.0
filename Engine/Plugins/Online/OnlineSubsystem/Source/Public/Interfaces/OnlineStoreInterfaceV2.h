...
{ \
	UE_LOGF(LogOnlineStoreV2, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}

#define UE_CLOG_ONLINE_STOREV2(Conditional, Verbosity, Format, ...) \
{ \
	UE_CLOGF(Conditional, LogOnlineStoreV2, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}
...
public:
	...
	virtual ~FOnlineStoreOffer() = default;
...
public:
	...
	virtual ~IOnlineStoreV2()= default;
	...
	*/
	...
	/**
	* Return store country if it has been fetched.
	*/
	virtual FString GetStoreCountry() const { return ""; }
};
