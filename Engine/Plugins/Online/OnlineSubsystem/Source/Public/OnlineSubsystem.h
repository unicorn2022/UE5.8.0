...
{ \
	UE_LOGF(LogOnline, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}

#define UE_CLOG_ONLINE(Conditional, Verbosity, Format, ...) \
{ \
	UE_CLOGF(Conditional, LogOnline, Verbosity, "%ls%ls", ONLINE_LOG_PREFIX, *FString::Printf(Format, ##__VA_ARGS__)); \
}
