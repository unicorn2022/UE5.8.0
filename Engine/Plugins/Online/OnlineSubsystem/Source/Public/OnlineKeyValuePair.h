...
public:
	...
	* @return true if conversion was successful
	*/
	UE_API bool FromJsonValue(FStringView Name, const TSharedRef<class FJsonValue>& JsonValue, FString& OutName);
...
public:
	...
	/**
	 * Apply FVariantData elements to properties of a UStruct that are identical with the previous value.
	 *
	 * @param VariantMap Input variant data
	 * @param StructDefinition layout of the UStruct
	 * @param OutStruct output container for UStruct data
	 * @param CompareStruct the struct to compare OutStruct to when deciding if a property should be updated.
	 * @param CheckFlags property must have this flag to be serialized
	 * @param SkipFlags property cannot have this flag to be serialized
	 * 
	 * @return true if it was successful, false otherwise
	 */
	static UE_API bool ApplyVariantMapToUnchangedProperties(const FOnlineKeyValuePairs<FString, FVariantData>& VariantMap, const UStruct* StructDefinition, void* OutStruct, const void* CompareStruct, int64 CheckFlags, int64 SkipFlags);
