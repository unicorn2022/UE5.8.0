...
FWindowsEOSSDKManager::FWindowsEOSSDKManager()
{
	...
	if (bEnablePlatformIntegration)
	{
		...
		{
			UE_LOGF(LogEOSShared, Warning, "SteamShared module not available, creation of platforms requiring Steam integration will fail, ensure SteamShared plugin is enabled");
		}
	}
#endif
}
...
IEOSPlatformHandlePtr FWindowsEOSSDKManager::CreatePlatform(EOS_Platform_Options& PlatformOptions)
{
	...
	{
		UE_LOGF(LogEOSShared, Warning, "FWindowsEOSSDKManager::CreatePlatform failed. Steam integrated platform is enabled and steam initialization failed. EosPlatformHandle=nullptr");
		return nullptr;
	}
#endif 
...
IEOSPlatformHandlePtr FWindowsEOSSDKManager::CreatePlatform(const FEOSSDKPlatformConfig& PlatformConfig, EOS_Platform_Options& PlatformOptions)
{
	...
	if (PlatformConfig.bEnableRTC)
	{
		...
		static const FString XAudioPath = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*FPaths::Combine(FPaths::EngineDir(), TEXT("Binaries/ThirdParty/Windows/XAudio2_9/x64/xaudio2_9redist.dll")));
#else
