...
namespace
{
	...
	void EOS_CALL EOSLogMessageReceived(const EOS_LogMessage* Message)
	{
		...
		#define EOSLOG(Level) UE_LOGF(LogEOSSDK, Level, "%ls: %ls", *CategoryStr, *MessageStr)
		switch (EOSLogLevel)
		{
...
void FEOSSDKManager::LoadSDKHandle()
{
	...
	{
		UE_LOGF(LogEOSShared, Verbose, "Attempting to load \"%ls\"", *BinaryPath);
		void* Result = FPlatformProcess::GetDllHandle(*BinaryPath);
		UE_CLOGF(Result, LogEOSShared, Log, "Loaded \"%ls\"", *BinaryPath);
		UE_CLOGF(!Result, LogEOSShared, Verbose, "Failed to load \"%ls\"", *BinaryPath);
		return Result;
	};
	...
	if (!SDKHandle)
	{
		...
		if (bDllLoadFailureIsFatal)
		{
			...
			UE_LOGF(LogEOSShared, Fatal, "%s Failed to load EOSSDK binary", __FUNCTION__);
		}
	}
}
#endif
...
EOS_EResult FEOSSDKManager::Initialize()
{
	...
	{
		UE_LOGF(LogEOSShared, Error, "%s Initialize failed, pre-fork", __FUNCTION__);
	...
	if (SDKHandle == nullptr)
	{
		UE_LOGF(LogEOSShared, Log, "%s failed, SDKHandle=nullptr", __FUNCTION__);
		return EOS_EResult::EOS_InvalidState;
	}
#endif
	...
	{
		UE_LOGF(LogEOSShared, Log, "%s Initializing EOSSDK Version:%ls", __FUNCTION__, UTF8_TO_TCHAR(EOS_GetVersion()));
		...
		{
			UE_LOGF(LogEOSShared, Warning, "ProductName=%ls contains invalid characters, please set a valid one", *ProductName);
		...
		UE_EOS_CHECK_API_MISMATCH(EOS_INITIALIZE_API_LATEST, 5);
		...
		if (EosResult == EOS_EResult::EOS_Success)
		{
			...
			if (EosResult != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogEOSShared, Warning, "%s EOS_Logging_SetCallback failed error:%ls", __FUNCTION__, *LexToString(EosResult));
			}

			EosResult = EOS_Logging_SetLogLevel(EOS_ELogCategory::EOS_LC_ALL_CATEGORIES, ConvertLogLevel(LogEOSSDK.GetVerbosity()));
			if (EosResult != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogEOSShared, Warning, "%s EOS_Logging_SetLogLevel failed Verbosity=%ls error=[%ls]", __FUNCTION__, ToString(LogEOSSDK.GetVerbosity()), *LexToString(EosResult));
			}
#endif // !NO_LOGGING
		...
		{
			UE_LOGF(LogEOSShared, Warning, "%s EOS_Initialize failed error:%ls", __FUNCTION__, *LexToString(EosResult));
		}
...
const FEOSSDKPlatformConfig* FEOSSDKManager::GetPlatformConfig(const FString& PlatformConfigName, bool bLoadIfMissing)
{
	...
	{
		UE_LOGF(LogEOSShared, Warning, "%s PlatformConfigName empty", __FUNCTION__);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogEOSShared, Warning, "%s PlatformConfigName \"%ls\" not found", __FUNCTION__, *PlatformConfigName);
		return nullptr;
	}
	...
	if (GConfig->GetString(*SectionName, TEXT("EncryptionKey"), PlatformConfig->EncryptionKey, GEngineIni))
	{
		...
		UE_LOGF(LogEOSShared, Warning, "%s Config section \"EOSSDK.Platform.%ls\" contains deprecated key EncryptionKey, please migrate to ClientEncryptionKey.", __FUNCTION__, *PlatformConfigName);
	}
	...
	UE_LOGF(LogEOSShared, Verbose, "%s Loaded platform config: %ls", __FUNCTION__, *PlatformConfigName);
	return PlatformConfig;
}
...
bool FEOSSDKManager::AddPlatformConfig(const FEOSSDKPlatformConfig& PlatformConfig, bool bOverwriteExistingConfig)
{
	...
	{
		UE_LOGF(LogEOSShared, Warning, "%s Platform name can't be empty", __FUNCTION__);
		return false;
	}

	if (PlatformConfigs.Find(PlatformConfig.Name) && !bOverwriteExistingConfig)
	{
		UE_LOGF(LogEOSShared, Warning, "%s Platform config already exists: %ls", __FUNCTION__, *PlatformConfig.Name);
		return false;
	}

	PlatformConfigs.Emplace(PlatformConfig.Name, PlatformConfig);
	UE_LOGF(LogEOSShared, Verbose, "%s Added platform config: %ls", __FUNCTION__, *PlatformConfig.Name);
	return true;
}
...
void FEOSSDKManager::SetDefaultPlatformConfigName(const FString& PlatformConfigName)
{
	...
	{
		UE_LOGF(LogEOSShared, Verbose, "%s Default platform name changed: New=%ls Old=%ls", __FUNCTION__, *PlatformConfigName, *DefaultPlatformConfigName);
...
EOS_HIntegratedPlatformOptionsContainer FEOSSDKManager::CreateIntegratedPlatformOptionsContainer()
{
	...
	{
		UE_LOGF(LogEOSShared, Warning, "%s EOS_IntegratedPlatform_CreateIntegratedPlatformOptionsContainer Result=[%ls]", __FUNCTION__, *LexToString(CreationResult));
	}
...
void FEOSSDKManager::ApplyIntegratedPlatformOptions(EOS_HIntegratedPlatformOptionsContainer& Container)
{
	...
	{
		UE_LOGF(LogEOSShared, Verbose, "%s Method not supported when running Commandlet", __FUNCTION__);
		Container = nullptr;
		return;
	}
	...
	if (bEnablePlatformIntegration)
	{
		...
		if (Container != nullptr)
		{
			...
			{
				UE_LOGF(LogEOSShared, Warning, "%s EOS_IntegratedPlatformOptionsContainer_Add Result=[%ls]", __FUNCTION__, *LexToString(Result));
			}
		}
	}
	else
...
IEOSPlatformHandlePtr FEOSSDKManager::CreatePlatform(const FString& PlatformConfigName, FName InstanceName)
{
	...
	{
		UE_LOGF(LogEOSShared, Warning, "%s Platform config missing required options", __FUNCTION__);
		return IEOSPlatformHandlePtr();
	}
	...
	if (PlatformMap)
	{
		...
		if (WeakPlatformHandle)
		{
			...
			{
				UE_LOGF(LogEOSShared, Verbose, "%s Found existing platform handle: PlatformConfigName=%ls InstanceName=%ls", __FUNCTION__, *PlatformConfigName, *InstanceName.ToString());
				return Pinned;
			}

			UE_LOGF(LogEOSShared, Verbose, "%s Removing stale platform handle pointer: PlatformConfigName=%ls InstanceName=%ls", __FUNCTION__, *PlatformConfigName, *InstanceName.ToString());
	...
	{
		UE_LOGF(LogEOSShared, Verbose, "%s Created platform handle: PlatformConfigName=%ls InstanceName=%ls", __FUNCTION__, *PlatformConfigName, *InstanceName.ToString());
...
IEOSPlatformHandlePtr FEOSSDKManager::CreatePlatform(EOS_Platform_Options& PlatformOptions)
{
	...
	if (IsInitialized())
	{
		...
		{
			UE_LOGF(LogEOSShared, Warning, "%s failed, EosPlatformHandle=nullptr", __FUNCTION__);
		}
	}
	else
	{
		UE_LOGF(LogEOSShared, Warning, "%s failed, SDK not initialized", __FUNCTION__);
	}
...
void FEOSSDKManager::LoadConfig()
{
	...
	if (GConfig->GetArray(SectionName, TEXT("IntegratedPlatformManagementFlags"), ManagementFlags, GEngineIni))
	{
		...
		for (const FString& ManagementFlagStr : ManagementFlags)
		{
			...
			{
				UE_LOGF(LogEOSShared, Verbose, "%s unknown EOS_EIntegratedPlatformManagementFlags \"%ls\"", __FUNCTION__, *ManagementFlagStr);
			}
...
#if WITH_ENGINE
void FEOSSDKManager::OnBackBufferReady_RenderThread(SWindow& SlateWindow, ISlateViewportProvider& ViewportProvider)
{
	UE_CALL_ONCE([]() {	UE_LOGF(LogEOSShared, VeryVerbose, "%s unimplemented on this platform", __FUNCTION__) });
}
#endif
	...
	for (const IEOSPlatformHandlePtr& ActivePlatform : ActivePlatformsChecked)
	{
		if (EOS_HUI UIHandle = EOS_Platform_GetUIInterface(*ActivePlatform))
		{
			...
			if (LastResult != Result)
			{
				...
				if (Result == EOS_EResult::EOS_Success)
				{
					UE_LOGF(LogEOSShared, Verbose, "%s EOS_UI_PrePresent is succeeding again.", __FUNCTION__);
				}
				else
				{
					UE_LOGF(LogEOSShared, Verbose, "%s EOS_UI_PrePresent failed with error: %ls", __FUNCTION__, *LexToString(Result));
				}
			}
		}
	}
...
void FEOSSDKManager::SetInvokeOverlayButton(const EOS_HPlatform PlatformHandle)
{
	if (bEnableOverlayIntegration)
	{
		if (EOS_HUI UIHandle = EOS_Platform_GetUIInterface(PlatformHandle))
		{
			...
			if (Result != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogEOSShared, Verbose, "%s EOS_UI_SetToggleFriendsButton Result=[%ls]", __FUNCTION__, *LexToString(Result));
			}
		}
	}
}
...
void FEOSSDKManager::OnNetworkConnectionStatusChanged(ENetworkConnectionStatus LastConnectionState, ENetworkConnectionStatus ConnectionState)
{
	...
	UE_LOGF(LogEOSShared, Log, "%s [%ls] -> [%ls]", __FUNCTION__, LexToString(OldNetworkStatus), LexToString(NewNetworkStatus));
...
void FEOSSDKManager::OnApplicationStatusChanged(EOS_EApplicationStatus ApplicationStatus)
{
	...
	UE_LOGF(LogEOSShared, Log, "%s [%ls] -> [%ls]", __FUNCTION__, LexToString(CachedApplicationStatus), LexToString(ApplicationStatus));
...
void FEOSSDKManager::OnLogVerbosityChanged(const FLogCategoryName& CategoryName, ELogVerbosity::Type OldVerbosity, ELogVerbosity::Type NewVerbosity)
{
		...
		CategoryName == LogEOSSDK.GetCategoryName())
	{
		...
		if (EosResult != EOS_EResult::EOS_Success)
		{
			UE_LOGF(LogEOSShared, Warning, "%s EOS_Logging_SetLogLevel Verbosity=[%ls] Result=[%ls]", __FUNCTION__, ToString(NewVerbosity), *LexToString(EosResult));
		}
	}
#endif // !NO_LOGGING
}

FString FEOSSDKManager::GetSDKVersion() const
{
	return FString((const UTF8CHAR*)EOS_GetVersion());
}
...
void FEOSSDKManager::Shutdown()
{
	...
	if (IsInitialized())
	{
		...
		{
			{
				...
				UE_LOGF(LogEOSShared, Warning, "%s Releasing %d remaining platforms", __FUNCTION__, ActivePlatforms.Num());
		...
		UE_LOGF(LogEOSShared, Log, "%s EOS_Shutdown Result=[%ls]", __FUNCTION__, *LexToString(Result));
...
bool FEOSSDKManager::Exec_Runtime(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	{
		UE_LOGF(LogEOSShared, Warning, "%s Unknown exec command: %ls]", __FUNCTION__, Cmd);
	}
...
TSharedRef<IEOSFastTickLock> FEOSSDKManager::GetFastTickLock()
{
	...
	CSV_TRACE_REGION_BEGIN(TEXT("HighPriorityEosLock"), TEXT("OnlineTick"))
...
FEOSFastTickLock::~FEOSFastTickLock()
{
	...
	{
		CSV_TRACE_REGION_END(TEXT("HighPriorityEosLock"), TEXT("OnlineTick"))
