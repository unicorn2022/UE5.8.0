...
void FOnlineSessionNull::OnValidResponsePacketReceived(uint8* PacketData, int32 PacketLength)
{
	if (CurrentSessionSearch.IsValid())
	{
		...
		{
			// Deduplicate by SessionId so that retried queries don't produce duplicate search results, while still allowing 
			// multiple different sessions from the same host to appear 
			const FString& NewSessionId = SearchResult.Session.GetSessionIdStr();
			bool bAlreadyInResults = false;
			for (const FOnlineSessionSearchResult& Existing : CurrentSessionSearch->SearchResults)
			{
				if (Existing.Session.GetSessionIdStr() == NewSessionId)
				{
