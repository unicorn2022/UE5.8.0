...
{
	UE_LOGF(LogParty, Log, "Usage SOCIAL DEBUG CONTEXT=<Name> <COMMAND> <PARAMS>");
}

void USocialDebugTools::PrintExecCommands() const
{
	UE_LOGF(LogParty, Log, "LOGIN <Id> <Auth> <optional CONTEXT=<Name2> <Id2> <Auth2> CONTEXT=<Name3> <Id3> <Auth3> ...>");
	UE_LOGF(LogParty, Log, "LOGOUT");
	UE_LOGF(LogParty, Log, "JOINPARTY <Id> <Auth> <optional FriendName>");
	UE_LOGF(LogParty, Log, "LEAVEPARTY");
	UE_LOGF(LogParty, Log, "AUTOACCEPTFRIENDINVITES");
	UE_LOGF(LogParty, Log, "AUTOACCEPTPARTYINVITES");
	UE_LOGF(LogParty, Log, "JIP");
	UE_LOGF(LogParty, Log, "HELP");
}
	...
	if (FParse::Command(&Cmd, TEXT("DEBUG")))
	{
		...
		else
		{
			...
			if(Instance.Equals(TEXT("all")))
			{
				...
				{
					UE_LOGF(LogParty, Log, "CONTEXT=ALL used, but no OSS contexts found!");
				}
			}
			else
			{
							...
							if (bWasSuccessful)
							{
								...
								if (JoinInfo.IsValid())
								{
									OnlineParty->JoinParty(UserId, *JoinInfo, FOnJoinPartyComplete::CreateLambda([this, Instance, OnlineParty, OnComplete](const FUniqueNetId& UserIdTmp, const FOnlinePartyId& PartyId, const EJoinPartyCompletionResult Result, const int32 NotApprovedReason)
									{
										...
										{
											UE_LOGF(LogParty, Warning, "Party context[%ls] join attempt denied for reason [%d]", *Instance, (int32)Result);
										}
			...
			if (FUniqueNetIdPtr UserId = Context.GetLocalUserId())
			{
				...
				if (OnlineParty.IsValid())
				{
					...
					if (ExistingParty.IsValid())
					{
						...
						{
							UE_LOGF(LogParty, Display, "Sending rep data update for member within party [%ls].", *PartyId.ToDebugString());
...
void USocialDebugTools::SetPartyMemberDataJson(const FString& Instance, const FString& JsonStr, const FSetPartyMemberDataComplete& OnComplete)
{
	...
	{
		UE_LOGF(LogParty, Warning, "SetPartyMemberDataJson didnt parse data from JsonStr=[%ls].", *JsonStr);
	}
	OnComplete.ExecuteIfBound(bResult);
}
...
bool USocialDebugTools::RunCommand(const TCHAR* Cmd, const TArray<FString>& TargetInstances)
{
	if (FParse::Command(&Cmd, TEXT("LOGIN")))
	{
		...
		{
			UE_LOGF(LogParty, Log, "CONTEXT=ALL cannot be used for the LOGIN command!");
		}
		else
		{
			...
			while (!Id.IsEmpty() && !Auth.IsEmpty() && MaxConcurrentLogins >= 0)
			{
				...
				{
					UE_LOGF(LogParty, Display, "Login OSS context[%ls] %ls", *InstanceName, *LexToString(bSuccess));
				}));
	...
	else if (FParse::Command(&Cmd, TEXT("LOGOUT")))
	{
		for (const FString& TargetInstance : TargetInstances)
		{
			...
			{
				UE_LOGF(LogParty, Display, "Logout OSS context[%ls] %ls", *TargetInstance, *LexToString(bSuccess));
			}));
		}
		return true;
	}
	else if (FParse::Command(&Cmd, TEXT("JOINPARTY")))
	{
		...
		for (const FString& TargetInstance : TargetInstances)
		{
			...
			{
				UE_LOGF(LogParty, Display, "Login OSS context[%ls] %ls", *TargetInstance, *LexToString(bSuccess));
				...
				if (bSuccess)
				{
					CleanupParties(TargetInstance, FCleanupPartiesComplete::CreateLambda([this, TargetInstance, FriendName](bool bCleanupPartiesSuccess)
					{
						...
						{
							UE_LOGF(LogParty, Display, "Leave party OSS context[%ls] %ls", *TargetInstance, *LexToString(bLeavePartySuccess));

							JoinParty(TargetInstance, FriendName, FJoinPartyComplete::CreateLambda([this, TargetInstance](bool bJoinPartySuccess)
							{
								UE_LOGF(LogParty, Display, "Join party OSS context[%ls] %ls", *TargetInstance, *LexToString(bJoinPartySuccess));
							}));
						}));
					}));
				}
	...
	else if (FParse::Command(&Cmd, TEXT("JIP")))
	{
		for (const FString& TargetInstance : TargetInstances)
		{
			...
			{
				UE_LOGF(LogParty, Display, "JIP OSS context[%ls] %ls", *TargetInstance, ToString(Result));
			}));
		}
		return true;
	}
	else if (FParse::Command(&Cmd, TEXT("LEAVEPARTY")))
	{
		for (const FString& TargetInstance : TargetInstances)
		{
			...
			{
				UE_LOGF(LogParty, Display, "Leave party OSS context[%ls] %ls", *TargetInstance, *LexToString(bSuccess));
			}));
		}
		return true;
	}
	...
	else if (FParse::Command(&Cmd, TEXT("DUMPPARTY")))
	{
		for (const FString& TargetInstance : TargetInstances)
		{
			UE_LOGF(LogParty, Display, "---DUMPPARTY - party OSS context[%ls]", *TargetInstance);
		...
		if (Context)
		{
			...
			if (OnlineParty.IsValid())
			{
				...
				OnlineParty->JoinParty(LocalUserId, Invitation, FOnJoinPartyComplete::CreateLambda([this, Instance, OnlineParty](const FUniqueNetId& UserId, const FOnlinePartyId& PartyIdTmp, const EJoinPartyCompletionResult Result, const int32 NotApprovedReason)
				{
					...
					{
						UE_LOGF(LogParty, Warning, "Party context[%ls] invite join attempt denied for reason [%d]", *Instance, (int32)Result);
					}
				}));
			}
		}
