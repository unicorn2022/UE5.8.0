...
bool bQueryRecentPlayersAfterPartyUpdates = true;
static FAutoConsoleVariableRef CVarQueryRecentPlayersAfterPartyUpdates(
	TEXT("Social.QueryRecentPlayersAfterPartyUpdates"), 
	bQueryRecentPlayersAfterPartyUpdates,
	TEXT("Query list of recent players when a player joins a party containing at least one non-friend, or a non-friend joins the player's existing party (unless all of the non-friended players are already in the list of recent players)"),
	ECVF_Default);
...
public:
	...
	virtual void ExecuteQuery() override
	{
		...
		if (LocalUserPrimaryId.IsValid())
		{
			...
			if (ensure(PlatformDescription && PrimaryUserInterface))
			{
				...
				UE_LOGF(LogParty, Log, "FSocialQuery_MapExternalIds executing for [%d] users on subsystem [%ls]", ExternalUserIds.Num(), ToString(SubsystemType));
			...
			{
				UE_LOGF(LogParty, Warning, "%ls - PlatformDescription: %llx (%ls) - PrimaryUserInterface: %llx", ANSI_TO_TCHAR(__FUNCTION__), PlatformDescription, PlatformDescription ? *PlatformDescription->Name : TEXT("N/A"), PrimaryUserInterface.Get());
			}
		}
		else
		{
			UE_LOGF(LogParty, Warning, "FSocialQuery_MapExternalIds cannot execute query - unable to get a valid primary net ID for the local player.");
		}
	}
	...
	{
		UE_LOGF(LogParty, Log, "FSocialQuery_MapExternalIds completed query for [%d] users Subsystem=[%ls] bWasSuccessful=[%ls] Error=[%ls]", ExternalIds.Num(), ToString(SubsystemType), *LexToString(bWasSuccessful), *ErrorStr);
			...
			if (PrimaryUserInterface.IsValid() && bWasSuccessful)
			{
				for (const FString& ExternalId : ExternalIds)
				{
					...
					if (!PrimaryId.IsValid())
					{
						...
						UE_LOGF(LogParty, Verbose, "No primary Id exists that corresponds to external Id [%ls]", *ExternalId);
#endif	
					}
...
TSharedRef<ISocialUserList> USocialToolkit::CreateUserList(const FSocialUserListConfig& ListConfig) const
{
	...
	{
		UE_LOGF(LogParty, Verbose, "%ls Found Userlist %ls while creating Userlist %ls with the same list config.", ANSI_TO_TCHAR(__FUNCTION__), *(*FoundUserList).Pin()->GetListConfig().Name, *ListConfig.Name);
...
int32 USocialToolkit::GetLocalUserNum() const
{
	...
	{
		UE_LOGF(LogParty, Verbose, "%s - OwningLocalPlayer is nullptr. Likely the player has logged out.", __FUNCTION__);
			...
			if (const FCommonAccountPtr LocalUserCommonAccountPtr = GetCommonAccountFromV1(CommonConfig, LocalUserNetId->AsShared(), GetFirstFrameworkInstanceName(LocalUserNetId)))
			{
				if(FAccountId LocalUserAccountId = LocalUserCommonAccountPtr->GetId(USocialManager::GetPresenceFrameworkInstance()))
				{
					if (IPresencePtr Presence = OnlineServices->GetPresenceInterface())
					{
						...
						UE_LOGF(LogParty, Verbose, "Updating rich presence with OnlineServices");
...
void USocialToolkit::TrySendFriendInvite(const FString& DisplayNameOrEmail) const
{
	...
	{
		UE_LOGF(LogParty, Log, "SocialToolkit [%d] failed to execute TrySendFriendInvite.", GetLocalUserNum());
	}
}
...
{
	UE_LOGF(LogParty, Log, "SocialToolkit [%d] quietly refreshing PSN FriendInfo on existing users due to an external requery of the friends list.", GetLocalUserNum());
	...
	if (!User && ensureMsgf(USocialToolkit::IsOwnerLoggedIn(), TEXT("Cannot QueueUserDependentAction while local user is logged out! Toolkit [%d], ID [%s], Subsystem [%s]"), GetLocalUserNum(), *SubsystemId.ToDebugString(), ToString(SubsystemType)))
	{
		...
		else
		{
			...
			else
			{
				...
				UE_LOGF(LogParty, VeryVerbose, "Mapping primary Id for unknown, unmapped external Id [%ls] for user action", *SubsystemId.ToDebugString());
...
void USocialToolkit::HandleControllerIdChanged(int32 NewId, int32 OldId)
{
	...
	if (const IOnlineIdentityPtr IdentityInterface = PrimaryOss ? PrimaryOss->GetIdentityInterface() : nullptr)
	{
		...
		{
			UE_CLOGF(OldId != INVALID_CONTROLLERID, LogParty, Error, "SocialToolkit updating controller IDs for local player while logged in. That makes no sense! OldId = [%d], NewId = [%d]", OldId, NewId);
...
void USocialToolkit::NotifySubsystemIdEstablished(USocialUser& SocialUser, ESocialSubsystem SubsystemType, const FUniqueNetIdRepl& SubsystemId)
{
	...
	{
		UE_LOGF(LogParty, VeryVerbose, "Toolkit [%d] establishing subsystem Id [%ls] for user [%ls]", GetLocalUserNum(), *SubsystemId.ToDebugString(), *SocialUser.ToDebugString());
	...
	else
	{
		...
		UE_LOGF(LogParty, Error, "%ls", *LogString);
	}
}
...
{
	UE_LOGF(LogParty, Log, "LocalPlayer [%d] has logged in - starting up SocialToolkit.", GetLocalUserNum());
	...
	for (ESocialSubsystem SubsystemType : USocialManager::GetDefaultSubsystems())
	{
		...
		if (LocalUserNetId.IsValid())
		{
			...
			if (IOnlinePartyPtr PartyInterface = OSS->GetPartyInterface())
			{
				...
				PartyInterface->AddOnPartyDataReceivedDelegate_Handle(FOnPartyDataReceivedDelegate::CreateUObject(this, &USocialToolkit::HandlePartyDataReceived));
			}
			...
			if (SubsystemType == ESocialSubsystem::Primary)
			{
				...
				{
					OnReadPresenceV2CVarChanged = CVar->OnChangedDelegate().AddUObject(this, &USocialToolkit::HandlePresenceReadCVarChanged);
				}
			}
		}
	}
...
{
	UE_LOGF(LogParty, Log, "LocalPlayer [%d] has logged out - wiping user roster from SocialToolkit.", GetLocalUserNum());
	...
	for (ESocialSubsystem SubsystemType : USocialManager::GetDefaultSubsystems())
	{
		if (IOnlineSubsystem* OSS = GetSocialOss(SubsystemType))
		{
			...
			if (PartyInterface.IsValid())
			{
				...
				PartyInterface->ClearOnPartyDataReceivedDelegates(this);
			}
			...
			if (OnReadPresenceV2CVarChanged.IsValid())
			{
				if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("onlineframework.party.shouldreadpresencefromonlineservices")))
				{
					CVar->OnChangedDelegate().Remove(OnReadPresenceV2CVarChanged);
				}

				OnReadPresenceV2CVarChanged.Reset();
			}
	...
	OnPresenceUpdatedHandles.Reset();
...
void USocialToolkit::HandlePlayerLoginStatusChanged(int32 LocalUserNum, ELoginStatus::Type OldStatus, ELoginStatus::Type NewStatus, const FUniqueNetId& NewId)
{
	if (LocalUserNum == GetLocalUserNum())
	{
		if (NewStatus == ELoginStatus::LoggedIn)
		{
			if (AllUsers.Num() != 0)
			{
				UE_LOGF(LogParty, Error, "HandlePlayerLoginStatusChanged: Changed login status but we were not informed their status had changed previously");
...
{
	UE_LOGF(LogParty, Log, "SocialToolkit [%d] finished querying friends list ListName=[%ls] Subsystem=[%ls] bWasSuccessful=[%ls] Error=[%ls].", GetLocalUserNum(), *ListName, ToString(SubsystemType), *LexToString(bWasSuccessful), *ErrorStr);
	if (bWasSuccessful)
	{
...
void USocialToolkit::HandleQueryBlockedPlayersComplete(const FUniqueNetId& UserId, bool bWasSuccessful, const FString& ErrorStr, ESocialSubsystem SubsystemType)
{
	...
	{
		UE_LOGF(LogParty, Log, "SocialToolkit [%d] finished querying blocked players Subsystem=[%ls] bWasSuccessful=[%ls] Error=[%ls].", GetLocalUserNum(), ToString(SubsystemType), *LexToString(bWasSuccessful), *ErrorStr);
...
void USocialToolkit::HandleQueryRecentPlayersComplete(const FUniqueNetId& UserId, const FString& Namespace, bool bWasSuccessful, const FString& ErrorStr, ESocialSubsystem SubsystemType)
{
	if (UserId == *GetLocalUserNetId(SubsystemType))
	{
		UE_LOGF(LogParty, Log, "SocialToolkit [%d] finished querying recent player list Namespace=[%ls] Subsystem=[%ls] bWasSuccessful=[%ls] Error=[%ls].", GetLocalUserNum(), *Namespace, ToString(SubsystemType), *LexToString(bWasSuccessful), *ErrorStr);
...
void USocialToolkit::HandleServicesPresenceUpdated(const UE::Online::FPresenceUpdated& Event, ESocialSubsystem SubsystemType)
{
	if (USocialManager::ShouldReadPresenceFromOnlineServices())
	{
		...
		if (FCommonAccountPtr CommonAccountPtr = GetCommonAccountFromV2(CommonConfig, UpdatedPresence->AccountId, USocialManager::GetPresenceFrameworkInstance()))
		{
			...
			if (FUniqueNetIdPtr UserId = GetV1FromCommonAccount(CommonAccountPtr.ToSharedRef(), USocialManager::GetPresenceFrameworkAdapterInstance()))
			{
				...
				{
					UE_LOGF(LogParty, Error, "Platform presence received for UserId [%ls], but existing SocialUser could not be found.", *UserId->ToDebugString());
				}
			}
		}
	}
	...
	{
		NonReadPresencesReceivedV2.Emplace(Event.UpdatedPresence->AccountId);
	}
}
...
{
	if (!USocialManager::ShouldReadPresenceFromOnlineServices() || SubsystemType == ESocialSubsystem::Platform)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_USocialToolkit_HandlePresenceReceived);
	...
	{
		NonReadPresencesReceivedV1.Emplace(UserId.AsShared());
	}
}

void USocialToolkit::HandlePresenceReadCVarChanged(IConsoleVariable*)
{
	if (USocialManager::ShouldReadPresenceFromOnlineServices())
	{
		using namespace UE::Online;
		...
		FCommonConfig CommonConfig(this);
		IOnlineServicesPtr OnlineServices = CommonConfig.GetServices(USocialManager::GetPresenceFrameworkInstance());
		IPresencePtr Presence = OnlineServices ? OnlineServices->GetPresenceInterface() : nullptr;
		FUniqueNetIdRepl LocalUserIdV1 = LocalUser ? LocalUser->GetUserId(ESocialSubsystem::Primary) : nullptr;
		const FCommonAccountPtr LocalUserCommonAccount = LocalUserIdV1.IsValid() ? GetCommonAccountFromV1(CommonConfig, LocalUserIdV1.GetV1(), GetFirstFrameworkInstanceName(*LocalUserIdV1)) : nullptr;
		const FAccountId LocalUserAccountId = LocalUserCommonAccount ? LocalUserCommonAccount->GetId(USocialManager::GetPresenceFrameworkInstance()) : FAccountId();
		if (Presence && LocalUserAccountId.IsValid())
		{
			const TSet<FAccountId> PreviouslyUpdatedPresences = MoveTemp(NonReadPresencesReceivedV2);
			NonReadPresencesReceivedV2.Empty();
			for (const FAccountId TargetAccountId : PreviouslyUpdatedPresences)
			{
				TOnlineResult<FGetCachedPresence> Result = Presence->GetCachedPresence({
					.LocalAccountId = LocalUserAccountId,
					.TargetAccountId = TargetAccountId
				});

				if (Result.IsOk())
				{
					HandleServicesPresenceUpdated({
						.LocalAccountId = LocalUserAccountId,
						.UpdatedPresence = Result.GetOkValue().Presence
	...
	else 
	{
		if (IOnlineSubsystem* Oss = GetSocialOss(ESocialSubsystem::Primary))
		{
			...
			FUniqueNetIdRepl LocalUserId = LocalUser ? LocalUser->GetUserId(ESocialSubsystem::Primary) : nullptr;
			if (Presence && LocalUserId.IsValid())
			{
				const FUniqueNetIdSet PreviouslyUpdatedNetIds = MoveTemp(NonReadPresencesReceivedV1);
				NonReadPresencesReceivedV1.Empty();
				for (const FUniqueNetIdRef& TargetNetId : PreviouslyUpdatedNetIds)
				{
					TSharedPtr<FOnlineUserPresence> UserPresence;
					EOnlineCachedResult::Type Result = Presence->GetCachedPresenceForApp(*LocalUserId, *TargetNetId, Oss->GetAppId(), UserPresence);
					if (UserPresence)
					{
						HandlePresenceReceived(*TargetNetId, UserPresence.ToSharedRef(), ESocialSubsystem::Primary);
					}
				}
			}
		}
...
{
	UE_LOGF(LogParty, Log, "SocialToolkit [%d] quietly refreshing Friend list due to an external requery of the friends list.", GetLocalUserNum());
			...
			[this, Invite = Invite.AsShared()] (USocialUser& User)
			{
				if (IsInviteAllowedFromUser(User, Invite))
				{
					...
					CanReceiveInviteFrom(User, Invite, [this, Invite, UserId = User.GetUserId(ESocialSubsystem::Primary)](const bool bResult)
					{
						...
						{
							UE_LOGF(LogParty, Log, "USocialToolkit::HandlePartyInviteReceived Invitation is no longer valid. LocalUser=[%ls] Inviter=[%ls]",
								*GetLocalUserNetId(ESocialSubsystem::Primary).ToDebugString(), *UserId.ToDebugString());
							return;
						}

						UE_LOGF(LogParty, Log, "USocialToolkit::HandlePartyInviteReceived LocalUser=[%ls] Inviter=[%ls] CanReceiveInviteFrom=[%ls]",
...
void USocialToolkit::HandlePartyDataReceived(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FName& Namespace, const FOnlinePartyData& PartyData)
{
	if (bQueryRecentPlayersAfterPartyUpdates)
	{
		if (LocalUserId == GetLocalUserNetId(ESocialSubsystem::Primary))
		{
			bool RecentPlayerListRequiresUpdate = false;
			if (const USocialParty* SocialParty = GetSocialManager().GetParty(PartyId))
			{
				for (const UPartyMember* Member : SocialParty->GetPartyMembers())
				{
					const USocialUser& SocialUser = Member->GetSocialUser();
					const FUniqueNetIdRepl SocialUserId = SocialUser.GetUserId(ESocialSubsystem::Primary);
					if (!SocialUserId.IsValid() || *SocialUserId == LocalUserId)
					{
						continue;
					}
					if (!SocialUser.IsFriend() && !SocialUser.IsRecentPlayer())
					{
						RecentPlayerListRequiresUpdate = true;
						break;
					}
				}
			}

			if (RecentPlayerListRequiresUpdate)
			{
				// Get the latest from the IOnlineFriends system
				QueryRecentPlayers();
			}
		}
	}
		...
		QueueUserDependentActionInternal(BlockedPlayerId.AsShared(), SubsystemType, 
			[this, SubsystemType] (USocialUser& User)
			{
				...
				if (FriendsInterface->GetBlockedPlayers(*GetLocalUserNetId(SubsystemType), AllBlockedPlayers))
				{
					...
					{
						UE_LOGF(LogParty, Log, "%ls Block player %ls complete, Establishing Oss info...", ANSI_TO_TCHAR(__FUNCTION__), *BlockedUserId.ToDebugString());
...
void USocialToolkit::HandleBlockListChanged(int32 LocalUserNum, const FString& ListName, ESocialSubsystem SubsystemType)
{
	if (LocalUserNum == GetLocalUserNum())
	{
		UE_LOGF(LogParty, Log, "SocialToolkit [%d] Subsystem=[%ls] Blocklist changed", GetLocalUserNum(), ToString(SubsystemType));
...
void USocialToolkit::ProcessOSSBlockedPlayers(ESocialSubsystem SubsystemType, const TArray<TSharedRef<FOnlineBlockedPlayer>>& InBlockedPlayers)
{
	...
	UE_LOGF(LogParty, Log, "SocialToolkit [%d] Subsystem=[%ls] BlockedPlayers=[%ls] UnblockedPlayers=[%ls]",
		GetLocalUserNum(),
	...
	if (LocalUserId == GetLocalUserNetId(ESocialSubsystem::Primary))
	{
		QueueUserDependentActionInternal(RequesterId.AsShared(), ESocialSubsystem::Primary,
			[this, RequestRef = Request.AsShared()](USocialUser& User)
			{
				...
				{
					UE_LOGF(LogParty, VeryVerbose, "%ls - Join request from blocked user [%ls] ignored", ANSI_TO_TCHAR(__FUNCTION__), *User.GetUserId(ESocialSubsystem::Primary).ToDebugString());
				}
			});
	}
}
