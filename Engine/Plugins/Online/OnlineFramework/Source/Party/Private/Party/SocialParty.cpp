...
void FPartyRepData::PostReplication(const FOnlinePartyRepDataBase& OldData, TSharedPtr<FStreamableHandle> InResourceHandle)
{
	OnPostReplicationProcessedEvent.Broadcast();
}
...
static int32 RejectSelfJoinInProgress = 1;
static FAutoConsoleVariableRef CVar_RejectSelfJoinInProgress(
	TEXT("Party.RejectSelfJoinInProgress"),
	RejectSelfJoinInProgress,
	TEXT("Reject join-in-progress requests where the target is the requesting local player.\n")
	TEXT("1 Enables. 0 disables."),
	ECVF_Default);

static int32 LimitJoinInProgressResponses = 1;
static FAutoConsoleVariableRef CVar_LimitJoinInProgressResponses(
	TEXT("Party.LimitJoinInProgressResponses"),
	LimitJoinInProgressResponses,
	TEXT("Sort, deduplicate, and cap join-in-progress responses stored in member rep data.\n")
	TEXT("1 Enables. 0 disables."),
	ECVF_Default);

static int32 GateInitOnRepDataProcessed = 1;
static FAutoConsoleVariableRef CVar_GateInitOnRepDataProcessed(
	TEXT("Party.GateInitOnRepDataProcessed"),
	GateInitOnRepDataProcessed,
	TEXT("Whether USocialParty initialization waits for FPartyRepData::PostReplication\n")
	TEXT("1 = wait (default; OnPartyJoined sees populated rep data).\n")
	TEXT("0 = legacy behavior, flip on receipt regardless of async asset state."),
	ECVF_Default);
...
FPartyJoinApproval USocialParty::EvaluateJoinRequest(const TArray<IOnlinePartyUserPendingJoinRequestInfoConstRef>& Players, bool bFromJoinRequest) const
{
	...
	else if (!EvaluateJoinRequestAwayCheck(Players, bFromJoinRequest))
	{
...
bool USocialParty::EvaluateJoinRequestAwayCheck(const TArray<IOnlinePartyUserPendingJoinRequestInfoConstRef>& Players, bool bFromJoinRequest) const
{
	...
	USocialUser* SocialUser = GetOwningLocalMember().GetSocialUser(GetOwningLocalUserId());
	const bool bIsAway = SocialUser && (USocialManager::ShouldReadPresenceFromOnlineServices() ? SocialUser->GetOnlineStatusV2() == UE::Online::EUserPresenceStatus::Away : SocialUser->GetOnlineStatus() == EOnlinePresenceState::Away);
	return (!bIsAway) || (GetSocialManager().CanAcceptInvitationsFromAwayUsers() && CheckForSentInvitation());
}
...
bool USocialParty::TryInviteUser(const USocialUser& UserToInvite, const ESocialPartyInviteMethod InviteMethod, const FString& MetaData)
{
	...
	if (CanInviteResult == ESocialPartyInviteFailureReason::Success)
	{
		...
		bool  bMustSendPlatformInvite = USocialSettings::MustAlwaysSendPlatformSessionInvites();
		
		if ((UserPlatformId.IsValid() && (bMustSendPlatformInvite || bIsOnlineOnPlatform)) && 
...
bool USocialParty::TryPromoteMember(const ULocalPlayer& PerformingPlayer, const UPartyMember& PartyMember)
{
	...
	{
		UE_LOGF(LogParty, VeryVerbose, "Party [%ls] Attempting to promote member [%ls]", *ToDebugString(), *PartyMember.ToDebugString(false));
...
bool USocialParty::TryKickMember(const ULocalPlayer& PerformingPlayer, const UPartyMember& PartyMember)
{
	...
	{
		UE_LOGF(LogParty, VeryVerbose, "Party [%ls] Attempting to kick member [%ls]", *ToDebugString(), *PartyMember.ToDebugString(false));
...
void USocialParty::InitializeParty(const TSharedRef<const FOnlineParty>& InOssParty)
{
	...
	if (ensure(!OssParty.IsValid()))
	{
		...
		PartyDataReplicator->OnPostReplicationProcessed().AddUObject(this, &USocialParty::NotifyRepDataProcessed);
		...
		UE_LOGF(LogParty, VeryVerbose, "New party [%ls] created", *ToDebugString());
	}
}
...
void USocialParty::NotifyRepDataProcessed()
{
	// Must fully apply the rep data update before broadcasting initialization complete
...
{
	UE_LOGF(LogParty, Verbose, "Initializing rep data for party [%ls]", *ToDebugString());
	bHasReceivedRepData = true;
}
	...
	if (ensure(MemberId.IsValid()))
	{
		...
		else
		{
			...
			if (ensure(PartyMemberClass))
			{
				...
				{
					UE_LOGF(LogParty, Warning, "Cannot create party member - user [%ls] is not in party [%ls]", *MemberId.ToDebugString(), *PartyId.ToDebugString());
				}
			}
		}
	}
...
void USocialParty::HandlePartyJoinRequestReceived(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const IOnlinePartyPendingJoinRequestInfo& JoinRequestInfo)
{
		...
		JoinApproval.GetApprovalAction() == EApprovalAction::EnqueueAndStartBeacon)
	{
		...
		UE_LOGF(LogParty, Verbose, "[%ls] Enqueuing approval request for %ls", *PartyId.ToString(), *PrimaryJoiningUser->GetUserId()->ToDebugString());
	...
	else
	{
		...
		UE_LOGF(LogParty, Log, "[%ls] Responding to approval request for %ls with %ls", *PartyId.ToString(), *PrimaryJoiningUser->GetUserId()->ToDebugString(), bIsApproved ? TEXT("approved") : TEXT("denied"));
...
void USocialParty::RemovePlayerFromReservationBeacon(const FUniqueNetId& LocalUserId, const FUniqueNetId& PlayerToRemove)
{
	...
	// Enqueue for a more opportune time
	UE_LOGF(LogParty, Verbose, "[%ls] Enqueuing ReservationBeacon update to remove player %ls", *GetPartyId().ToString(), *PlayerToRemove.ToDebugString());
...
void USocialParty::HandleJoinabilityQueryReceived(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const IOnlinePartyPendingJoinRequestInfo& JoinRequestInfo)
{
	if (PartyId == GetPartyId()) 
	{
		...
		UE_LOGF(LogParty, VeryVerbose, "[%ls] Responding to approval request for %ls with %ls", *PartyId.ToString(), *PrimaryJoiningUser->GetUserId()->ToString(), JoinabilityInfo.CanJoin() ? TEXT("approved") : TEXT("denied"));
...
{
	UE_LOGF(LogParty, Verbose, "USocialParty::HandlePartyDataReceived");
	...
	{
		check(PartyDataReplicator.IsValid());
		PartyDataReplicator.ProcessReceivedData(this, PartyData);

		if (GateInitOnRepDataProcessed == 0 && !bHasReceivedRepData)
		{
			bHasReceivedRepData = true;
			TryFinishInitialization();
		}
...
void USocialParty::HandleUpdatePartyConfigComplete(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, EUpdateConfigCompletionResult Result)
{
	if (!OssParty.IsValid())
	{
		UE_LOGF(LogParty, Error, "[%ls] Invalid OssParty value. Party %ls", ANSI_TO_TCHAR(__FUNCTION__), *PartyId.ToDebugString());
		return;
	}
	...
	{
		CurrentConfig = *OssParty->GetConfiguration();
		UE_LOGF(LogParty, Verbose, "[%ls] Success updating Party Config for %ls - OssParty Configuration:%ls",
	...
	{	
		...
		UE_LOGF(LogParty, Error, "[%ls] Failed to update Party Config for %ls - Current Configuration:%ls Oss Configuration:%ls Result:%ls",
			ANSI_TO_TCHAR(__FUNCTION__),
			*PartyId.ToDebugString(),
			*::ToDebugString(CurrentConfig),
...
void USocialParty::HandlePartyMemberPromoted(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& NewLeaderId)
{
	if (PartyId == GetPartyId())
	{
		UE_LOGF(LogParty, VeryVerbose, "Party member [%ls] in party [%ls] promoted", *NewLeaderId.ToDebugString(), *PartyId.ToDebugString());
		...
		OnPartyLeaderChangedEvent.Broadcast(NewLeader);
	}
}
...
void USocialParty::HandleMemberSessionIdChanged(const FSessionId& NewSessionId, UPartyMember* Member)
{
	...
	else if (!ensure(NewSessionId.IsEmpty()))
	{
		...
		UE_LOGF(LogParty, Error, "[%ls]: Member [%ls] claims to be in session [%ls], but we have no record of it.", *OwningLocalUserId.ToDebugString(), *Member->GetDisplayName(), *NewSessionId);
	}
}
...
{
	UE_LOGF(LogParty, Verbose, "Removed Local player [%ls] is no longer in party [%ls].", *LocalUserId.ToDebugString(), *ToDebugString());
...
void USocialParty::HandlePartyMemberExited(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& MemberId, EMemberExitedReason ExitReason)
{
	...
	if (PartyId == GetPartyId())
	{
		...
		{
			UE_LOGF(LogParty, Error, "Party [%ls] received notification that member ID [%ls] has exited, but cannot find them in the party", *ToDebugString(), *MemberId.ToDebugString());
		}
	}
}
...
{
	UE_LOGF(LogParty, VeryVerbose, "Party [%ls] received notification of a party system state change to [%d]", *ToDebugString(), (int32)NewState);
...
void USocialParty::UpdatePartyConfig(bool bResetAccessKey)
{
	...
	UE_LOGF(LogParty, Log, "Party [%ls] attempting to update party config", *ToDebugString());
...
void USocialParty::LeaveParty(const FOnLeavePartyAttemptComplete& OnLeaveAttemptComplete)
{
	...
	{
		UE_LOGF(LogParty, Log, "Attempting to leave party [%ls]", *ToDebugString());
...
void USocialParty::RemoveLocalMember(const FUniqueNetIdRepl& LocalUserId, const FOnLeavePartyAttemptComplete& OnLeaveAttemptComplete)
{
	...
	{
		UE_LOGF(LogParty, Log, "Attempting to leave party LocalUserId=[%ls] [%ls] ", *LocalUserId.ToDebugString(), *ToDebugString());
				...
				if (FNamedOnlineSession* Session = SessionInterface->GetNamedSession(PartyGameSessionName))
				{
					...
					if (ensure(SessionInterface->GetResolvedConnectString(PartyGameSessionName, URL, NAME_BeaconPort)))
					{
						...
						if (LocalReservationBeaconClient)
						{
							...
							UE_LOGF(LogParty, Verbose, "Party [%ls] created reservation beacon [%ls].", *ToDebugString(), *LocalReservationBeaconClient->GetName());
...
void USocialParty::RejectAllPendingJoinRequests()
{
	...
	while (!PendingApprovals.IsEmpty())
	{
		...
		UE_LOGF(LogParty, Log, "[%ls] Responding to approval request for %ls with %ls", *PartyId.ToString(), *PrimaryJoiningUserId.ToDebugString(), ToString(EPartyJoinDenialReason::Busy));
...
{
	APartyBeaconClient* LocalReservationBeaconClient = ReservationBeaconClient.Get();
	UE_LOGF(LogParty, Verbose, "Host connection failed for reservation beacon [%ls]", LocalReservationBeaconClient ? *LocalReservationBeaconClient->GetName() : TEXT(""));
...
void USocialParty::PumpApprovalQueue()
{
	...
	if (PendingApprovals.Peek(NextApproval))
	{
		...
		{
			UE_LOGF(LogParty, Warning, "ReservationBeaconClient is null while trying to process more requests");
			RejectAllPendingJoinRequests();
		}
	}
	else
...
{
	UE_LOGF(LogParty, Verbose, "Reservation request complete with response: %ls", EPartyReservationResult::ToString(ReservationResponse));
...
void USocialParty::CleanupReservationBeacon()
{
	...
	{
		UE_LOGF(LogParty, Verbose, "Party reservation beacon cleanup while in state %ls, pending approvals: %ls", ToString(LocalReservationBeaconClient->GetConnectionState()), !PendingApprovals.IsEmpty() ? TEXT("true") : TEXT("false"));
...
void USocialParty::CleanupSpectatorBeacon()
{
	...
	{
		UE_LOGF(LogParty, Verbose, "Spectator reservation beacon cleanup while in state %ls, pending approvals: %ls", ToString(SpectatorBeaconClient->GetConnectionState()), !PendingApprovals.IsEmpty() ? TEXT("true") : TEXT("false"));
...
void USocialParty::SetIsMissingPlatformSession(bool bInIsMissingPlatformSession)
{
	...
	{
		UE_LOGF(LogParty, VeryVerbose, "Party [%ls] is %ls missing platform session", *ToDebugString(), bInIsMissingPlatformSession ? TEXT("now") : TEXT("no longer"));
...
void USocialParty::SetIsMissingXmppConnection(bool bInMissingXmppConnection)
{
	...
	{
		UE_CLOGF(bIsMissingXmppConnection.IsSet(), LogParty, VeryVerbose, "Party [%ls] is %ls missing XMPP connection", *ToDebugString(), bInMissingXmppConnection ? TEXT("now") : TEXT("no longer"));
...
void USocialParty::SetIsRequestingShutdown(bool bInRequestingShutdown)
{
	...
	{
		UE_CLOGF(bIsRequestingShutdown.IsSet(), LogParty, VeryVerbose, "Party [%ls] is %ls in a version transition", *ToDebugString(), bInRequestingShutdown ? TEXT("now") : TEXT("no longer"));
...
void USocialParty::FinalizePartyLeave(EMemberExitedReason Reason)
{
	...
	UE_LOGF(LogParty, Log, "Local player [%ls] is no longer in party [%ls]. Reason [%ls].", *GetOwningLocalMember().ToDebugString(false), *ToDebugString(), ToString(Reason));
...
void USocialParty::UpdatePlatformSessionLeader(const FString& SessionType)
{
	...
	if (const FPartyPlatformSessionInfo* PlatformSessionInfo = GetRepData().FindSessionInfo(SessionType))
	{
		...
		if (NewSessionOwner)
		{
			UE_LOGF(LogParty, Verbose, "Party [%ls] updating session owner on platform [%ls] to [%ls]", *ToDebugString(), *SessionType, *NewSessionOwner->ToDebugString(false));
		...
		{
			UE_LOGF(LogParty, Verbose, "Party [%ls] no longer has any members on platform [%ls], clearing session info entry.", *ToDebugString(), *SessionType);
...
void USocialParty::AugmentPlatformSessionSettings(FOnlineSessionSettings& SessionSettings) const
{
...
{
	// Refuse immediately if the target is one of the requesting local players -- joining yourself is not meaningful.
	const FUniqueNetIdRepl& TargetId = TargetMember.GetPrimaryNetId();
	const bool bTargetIsSelf = Algo::AnyOf(GetLocalPartyMembersForJoinInProgress(), [&TargetId](const UPartyMember* Member)
	{
		return Member && Member->GetPrimaryNetId() == TargetId;
	});
	if (RejectSelfJoinInProgress != 0 && bTargetIsSelf)
	{
		UE_LOGF(LogParty, Warning, "RequestJoinInProgress: Target %ls is the same as the requesting local player -- ignoring.", *TargetId.ToDebugString());
		CompletionDelegate.ExecuteIfBound(EPartyJoinDenialReason::JoinAttemptAborted);
		return;
	}
	...
	UE_LOGF(LogParty, Log, "RequestJoinInProgress: Sending request Target=%ls Time=%lld", *Request.Target.ToDebugString(), Request.Time);
...
void USocialParty::CancelJoinInProgressRequest()
{
	if (IsJoinInProgeressRequestActive())
	{
		UE_LOGF(LogParty, Log, "RequestJoinInProgress: Canceling join-in-progress request.");
...
void USocialParty::HandleJoinInProgressDataRequestChanged(const FPartyMemberJoinInProgressRequest& Request, UPartyMember* Member)
{
	...
	{
		UE_LOGF(LogParty, Warning, "HandleJoinInProgressDataRequestChanged: Could not find member for request Target=%ls Time=%lld", *Request.Target.ToDebugString(), Request.Time);
		return;
	}
	...
	UE_LOGF(LogParty, Log, "HandleJoinInProgressDataRequestChanged: Received request Requester=%ls Time=%lld", *Member->GetPrimaryNetId().ToDebugString(), Request.Time);
	FPendingMemberApproval PendingApproval;
...
void USocialParty::RespondToJoinInProgressRequest(const FPendingMemberApproval& PendingApproval, const EPartyJoinDenialReason DenialReason)
{
	...
	if (!RequestingMember)
	{
		UE_LOGF(LogParty, Warning, "RespondToJoinInProgressRequest: Could not find member for approval MemberId=%ls", *PendingApproval.Members[0].MemberId.ToDebugString());
		return;
	}
	...
	UE_LOGF(LogParty, Log, "RespondToJoinInProgressRequest: Sending response Requester=%ls RequestTime=%lld ResponseTime=%lld DenialReason=%ls",
	...
	if (LimitJoinInProgressResponses != 0)
	{
		// Sort responses by ResponseTime (descending - newest first).
		Algo::SortBy(Responses, &FPartyMemberJoinInProgressResponse::ResponseTime, TGreater<int64>());

		// Limit responses per requesting member (keep most recent).
		{
			TMap<FUniqueNetIdRepl, int32> ResponseCountPerRequester;
			Responses.SetNum(Algo::StableRemoveIf(Responses, [&ResponseCountPerRequester, this](const FPartyMemberJoinInProgressResponse& Response)
			{
				int32& Count = ResponseCountPerRequester.FindOrAdd(Response.Requester);
				return ++Count > MaxJoinInProgressResponsesPerMember;
			}));
		}

		// Limit total responses (remove oldest, which are at the end after sorting).
		if (Responses.Num() > MaxJoinInProgressResponses)
		{
			Responses.SetNum(MaxJoinInProgressResponses);
		}
	}
...
void USocialParty::HandleJoinInProgressDataResponsesChanged(const TArray<FPartyMemberJoinInProgressResponse>& Responses, UPartyMember* Member)
{
	...
	for (const FPartyMemberJoinInProgressResponse& Response : Responses)
	{
		...
		if (!RequestingMember)
		{
			UE_LOGF(LogParty, Warning, "HandleJoinInProgressDataResponsesChanged: Could not find member for response Requester=%ls RequestTime=%lld ResponseTime=%lld DenialReason=%ls",
				*Response.Requester.ToDebugString(), Response.RequestTime, Response.ResponseTime, ToString(DenialReason));
			continue;
		}
		...
		UE_LOGF(LogParty, Log, "HandleJoinInProgressDataResponsesChanged: Received response Requester=%ls RequestTime=%lld ResponseTime=%lld DenialReason=%ls RTT=%lld",
...
{
	UE_LOGF(LogParty, VeryVerbose, "RunJoinInProgressTimer: Checking for stale data");
	...
	for (UPartyMember* RequestingMember : GetLocalPartyMembersForJoinInProgress())
	{
		if (RequestingMember && ensure(RequestingMember->IsLocalPlayer()))
		{
			...
			if (Request.Time > 0)
			{
				...
				if (Expires <= Now)
				{
					UE_LOGF(LogParty, Verbose, "RunJoinInProgressTimer: Removing request data");
					CallJoinInProgressComplete(EPartyJoinDenialReason::JoinAttemptAborted);
			...
			{
				UE_LOGF(LogParty, Verbose, "RunJoinInProgressTimer: Removing response data, %d remaining", ResponsesToKeep.Num());
	...
	if (NextTimer > 0)
	{
		UE_LOGF(LogParty, Verbose, "RunJoinInProgressTimer: Running again in %lld seconds", NextTimer);
