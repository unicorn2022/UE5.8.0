...
void UPartyMember::InitializePartyMember(const FOnlinePartyMemberConstRef& InOssMember, FSimpleDelegate&& OnInitComplete)
{
	...
	if (ensureAlways(!OssPartyMember.IsValid()))
	{
		...
		UE_LOGF(LogParty, Verbose, "Created new party member [%ls]", *ToDebugString());
	}
}

void UPartyMember::InitializeLocalMemberRepData()
{
	UE_LOGF(LogParty, Verbose, "Initializing rep data for local member [%ls]", *ToDebugString());
	...
	if (const USocialManager::FJoinPartyAttempt* JoinAttempt = CurrentParty.GetSocialManager().GetJoinAttemptInProgress(CurrentParty.GetPartyTypeId()))
	{
		...
		UE_LOGF(LogParty, Verbose, "Join method from join attempt for local member is %ls.", *JoinMethod);
	}
	else
	{
		...
		if (PartyMemberData.IsValid())
		{
			...
			if (PartyMemberData->GetAttribute(TEXT("JoinMethod"), AttrValue))
			{
				...
				UE_LOGF(LogParty, Verbose, "Join method recovered from the Party member data is %ls.", *JoinMethod);
			}
		}
	}
...
{
	UE_LOGF(LogParty, VeryVerbose, "Received updated rep data for member [%ls]", *ToDebugString());

	check(MemberDataReplicator.IsValid());
	MemberDataReplicator.ProcessReceivedData(this, MemberData/*, bHasReceivedInitialData*/);
...
{
	UE_LOGF(LogParty, Log, "Member [%ls] promoted to party leader.", *ToDebugString());
	OnMemberPromotedInternal();
}

void UPartyMember::NotifyMemberDemoted()
{
	UE_LOGF(LogParty, Log, "Member [%ls] is no longer party leader.", *ToDebugString());
	OnMemberDemotedInternal();
}

void UPartyMember::NotifyRemovedFromParty(EMemberExitedReason ExitReason)
{
	UE_LOGF(LogParty, Log, "Member [%ls] is no longer in the party. Reason = [%ls]", *ToDebugString(), ToString(ExitReason));
...
void UPartyMember::FinishInitializing()
{
	...
	UE_LOGF(LogParty, Verbose, "PartyMember [%ls] is now fully initialized.", *ToDebugString());
...
void UPartyMember::InitializeSocialUserForToolkit(USocialToolkit& Toolkit)
{
	...
	UE_LOGF(LogParty, Verbose, "%s - QUDA returned SocialUser [%ls (%p)] for %ls", __FUNCTION__,
	...
	{
		UE_LOGF(LogParty, Log, "%s - Registering Init Complete Handler for [%ls (%p)]", __FUNCTION__,
...
void UPartyMember::HandleSocialUserInitialized(USocialUser& InitializedUser)
{
	...
	UE_LOGF(LogParty, VeryVerbose, "PartyMember [%ls]'s underlying SocialUser has been initialized for local user [%ls]",
...
UPartyMember::FDebugInitializer::~FDebugInitializer()
{
	...
	{
		UE_LOGF(LogParty, VeryVerbose, "%ls [%0.2f] Complete", *ParentDebugString, FPlatformTime::Seconds() - StartTime);
	}
	else
	{
		UE_LOGF(LogParty, Verbose, "%ls [%0.2f] destroyed before initializing completed", *ParentDebugString, FPlatformTime::Seconds() - StartTime);
	}
}

void UPartyMember::FDebugInitializer::AddPendingAction(FString&& InAction)
{
	UE_LOGF(LogParty, VeryVerbose, "%ls Waiting for [%ls]", *ParentDebugString, *InAction);
	PendingActions.AddUnique(MoveTemp(InAction));
}

void UPartyMember::FDebugInitializer::RemovePendingAction(FString&& InAction)
{
	UE_LOGF(LogParty, VeryVerbose, "%ls No longer waiting for [%ls]. Time elapsed %0.2f", *ParentDebugString, *InAction, FPlatformTime::Seconds() - StartTime);
...
void UPartyMember::FDebugInitializer::SetupWarningTimers()
{
	...
	if (WarningTimeSeconds > 0.0)
	{
		...
		{
			UE_LOGF(LogParty, Warning, "%ls [%0.2f] Initialization not complete. Waiting for: %ls", *ParentDebugString, FPlatformTime::Seconds() - StartTime, *GetWaitingForString());
			...
			if ((ErrorTimeSeconds - WarningTimeSeconds) > 0.0)
			{
				...
				{
					UE_LOGF(LogParty, Error, "%ls [%0.2f] Initialization not complete. Waiting for: %ls", *ParentDebugString, FPlatformTime::Seconds() - StartTime, *GetWaitingForString());
					TickHandle.Reset();
					return false;
				});
			}
