...
{
	UE_LOGF(LogParty, Log, "FPartyPlatformSessionManager finding party platform session of user [%ls]", *User.GetDisplayName());
...
{
	UE_LOGF(LogParty, Log, "FPartyPlatformSessionManager finding party platform session [%ls]", *SessionInfo.ToDebugString());
	...
	if (!SessionIdString.IsEmpty() && SessionOwnerId.IsValid())
	{
		...
		if (ensure(LocalUserPlatformId.IsValid()))
		{
			...
			if (SessionId.IsValid())
			{
				...
				{
					UE_LOGF(LogParty, Warning, "PartyPlatformSessionMonitor adding artificial delay of %0.2fs to session find attempt", DelaySeconds);
					...
					FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
						[AsWeakPtr, SessionId, SessionIdString, SessionOwnerId, LocalUserPlatformId, OnAttemptComplete, this](float)
						{
							...
							if (AsWeakPtr.IsValid())
							{
								...
								{
									UE_LOGF(LogParty, Warning, "Forcing session find failure");
								...
								{
									GetSessionInterface()->FindSessionById(*LocalUserPlatformId, *SessionId, *SessionOwnerId.GetV1(), FOnSingleSessionResultCompleteDelegate::CreateSP(this, &FPartyPlatformSessionManager::HandleFindSessionByIdComplete, SessionIdString, SessionOwnerId, OnAttemptComplete));
								}
							}
				...
				return SessionInterface->FindSessionById(*LocalUserPlatformId, *SessionId, *SessionOwnerId.GetV1(), FOnSingleSessionResultCompleteDelegate::CreateSP(this, &FPartyPlatformSessionManager::HandleFindSessionByIdComplete, SessionIdString, SessionOwnerId, OnAttemptComplete));
			}
			else
			{
				UE_LOGF(LogParty, Warning, "PartyPlatformSessionMonitor could not create a session id from string [%ls]", *SessionIdString);
			}
		}
	}
	return false;
...
{
	UE_LOGF(LogParty, Log, "FPartyPlatformSessionManager completed attempt to find platform session [%ls] of user [%ls] by SessionId with result [%d]", *SessionId, *SessionOwnerId.ToDebugString(), (int32)bWasSuccessful);
	...
	else
	{
		...
		{
			UE_LOGF(LogParty, Log, "PartyPlatformSessionManager failed to find PSN party session, requerying PSN friends list now.");
		...
		{
			OnAttemptComplete.ExecuteIfBound(false, FoundSession);
		}
	}
}
...
void FPartyPlatformSessionManager::ProcessCompletedSessionSearch(int32 LocalUserNum, bool bWasSuccessful, const FOnlineSessionSearchResult& FoundSession, const FSessionId& SessionId, const FUniqueNetIdRepl& SessionOwnerId, const FOnFindSessionAttemptComplete& OnAttemptComplete)
{
	...
	UE_LOGF(LogParty, Log, "PartyPlatformSessionManager has fully completed its search for session [%ls] associated with user [%ls] with result [%d]", *SessionId, *SessionOwnerId.ToDebugString(), (int32)bWasSuccessful);
...
{
	UE_LOGF(LogParty, Log, "PartyPlatformSessionManager completed requery of the PSN friends list for user [%ls] with result [%d] and Error [%ls]", *SessionOwnerId.ToDebugString(), (int32)bWasSuccessful, *ErrorStr);
...
{
	UE_LOGF(LogParty, Log, "PartyPlatformSessionManager found [%d] sessions searching by friend ID [%ls] (bWasSuccessful=%d)", FoundSessions.Num(), *SessionOwnerId.ToDebugString(), (int32)bWasSuccessful);
...
FPartyPlatformSessionMonitor::~FPartyPlatformSessionMonitor()
{
	...
	{
		UE_LOGF(LogParty, Error, "PartyPlatformSessionMonitor instance is being destructed without properly shutting down. Undesired and inaccurate session membership will result!");
...
void FPartyPlatformSessionMonitor::EvaluateCurrentSession()
{
	...
	if (const FNamedOnlineSession* Session = SessionInterface->GetNamedSession(NAME_PartySession))
	{
		...
		{
			UE_CLOGF(!ReplicatedSessionId.IsEmpty(), LogParty, Warning, "PartyPlatformSessionMonitor: Local player's session [%ls] does not match replicated session [%ls]", *TrueSessionId, *ReplicatedSessionId);
...
void FPartyPlatformSessionMonitor::Initialize()
{
	...
	UE_LOGF(LogParty, Log, "Initializing PartyPlatformSessionMonitor for party [%ls]", *Party.GetPartyId().ToDebugString());
	...
	Party.OnPlatformSessionUpdateRequested().AddSP(this, &FPartyPlatformSessionMonitor::QueuePlatformSessionUpdate);
...
{
	UE_LOGF(LogParty, Log, "Finalizing shutdown of PartyPlatformSessionMonitor for party of type [%d]", PartyTypeId.GetValue());
...
void FPartyPlatformSessionMonitor::CreateSession(const FUniqueNetIdRepl& LocalUserPlatformId)
{
	...
	if (ensure(LocalUserPlatformId.IsValid()) && ensure(ConfigurePlatformSessionSettings(SessionSettings)))
	{
		...
		{
			UE_LOGF(LogParty, Warning, "PartyPlatformSessionMonitor adding artificial delay of %0.2fs to session creation attempt", DelaySeconds);
			...
			FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
				[AsWeakPtr, SessionSettings, LocalUserPlatformId, this] (float)
				{
					...
					if (AsWeakPtr.IsValid())
					{
						...
						{
							UE_LOGF(LogParty, Warning, "Forcing session creation failure");
		...
		UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor creating session with the following parameters: ");
...
void FPartyPlatformSessionMonitor::AddLocalPlayerToSession(UPartyMember* PartyMember)
{
		...
		!MonitoredParty->IsMissingPlatformSession())
	{
		...
		if (ensure(Session && Session->SessionInfo.IsValid()))
		{
			...
			{
				UE_LOGF(LogParty, Verbose, "AddLocalPlayerToSession: Registering player, PartyMember=%ls PPUID=%ls", *PartyMember->ToDebugString(true), *PartyMemberPlatformUniqueId->ToDebugString());
				...
				{
					UE_LOGF(LogParty, Log, "AddLocalPlayerToSession: Complete User=%ls Result=%ls", *UserId.ToDebugString(), LexToString(Result));
				}));
			}
		}
	}
...
void FPartyPlatformSessionMonitor::RemoveLocalPlayerFromSession(UPartyMember* PartyMember)
{
		...
		!MonitoredParty->IsMissingPlatformSession())
	{
		...
		if (ensure(Session))
		{
			...
			if (PartyMemberPlatformUniqueId.IsValid())
			{
				UE_LOGF(LogParty, Verbose, "RemoveLocalPlayerFromSession: Unregistering player, PartyMember=%ls PPUID=%ls", *PartyMember->ToDebugString(true), *PartyMemberPlatformUniqueId->ToDebugString());
				...
				{
					UE_LOGF(LogParty, Log, "RemoveLocalPlayerFromSession: Complete User=%ls bWasSuccessful=%ls", *UserId.ToDebugString(), *LexToString(bWasSuccessful));
				}));
			}
		}
	}
...
{
	UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor joining platform session [%ls]", *SessionSearchResult.GetSessionIdStr());
	...
	{
		UE_LOGF(LogParty, Warning, "Adding artificial delay of %0.2fs to session join attempt", DelaySeconds);
		...
		FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
			[AsWeakPtr, SearchResultCopy, LocalUserPlatformId, this] (float)
			{
				...
				if (AsWeakPtr.IsValid())
				{
					if (ForcePlatformSessionCreationFailure != 0)
					{
						UE_LOGF(LogParty, Warning, "Forcing session join failure");
	...
	{
		UE_LOGF(LogParty, Warning, "JoinSession call failed for session [%ls].", *SessionSearchResult.GetSessionIdStr());
...
{
	UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor destroying platform party session now.");
...
void FPartyPlatformSessionMonitor::QueuePlatformSessionUpdate()
{
	...
	{
		UE_LOGF(LogParty, Verbose, "PartyPlatformSessionMonitor queuing session update for party [%ls]", *MonitoredParty->ToDebugString())
...
void FPartyPlatformSessionMonitor::HandlePlatformSessionsChanged()
{
	...
	{
		UE_LOGF(LogParty, Log, "Local user just became owner of their party platform session within party [%ls]", *MonitoredParty->ToDebugString());
...
void FPartyPlatformSessionMonitor::HandlePartyLeft(EMemberExitedReason Reason)
{
	...
	if (ensure(MonitoredParty.IsValid()))
	{
		UE_LOGF(LogParty, Log, "Party [%ls] left - shutting down PartyPlatformSessionMonitor", *MonitoredParty->GetPartyId().ToDebugString());
...
void FPartyPlatformSessionMonitor::HandlePartyMemberLeft(UPartyMember* OldMember, const EMemberExitedReason ExitReason)
{
	...
	if (PartyMemberPlatformUniqueId.IsValid())
	{
		UE_LOGF(LogParty, Verbose, "HandlePartyMemberLeft: PartyMember=%ls User=%ls ExitReason=%ls",
			*OldMember->ToDebugString(true), *PartyMemberPlatformUniqueId.ToDebugString(), ToString(ExitReason));
	}
	else
	{
		UE_LOGF(LogParty, Warning, "HandlePartyMemberLeft: PartyMember=Unknown ExitReason=%ls", ToString(ExitReason));
	}
...
void FPartyPlatformSessionMonitor::HandleCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor created platform session SessionName=[%ls], bWasSuccessful=[%d]", *SessionName.ToString(), bWasSuccessful);
...
void FPartyPlatformSessionMonitor::HandleFindSessionComplete(bool bWasSuccessful, const FOnlineSessionSearchResult& FoundSession)
{
	...
	{
		UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor failed to find platform session [%ls]", *TargetSessionId);
		ProcessJoinFailure();
	}
}
...
{
	UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor attempt to join session [%ls] completed with result [%ls]", *TargetSessionId, LexToString(JoinSessionResult));
...
{
	UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor finished destroying party session with result [%d].", (int32)bWasSuccessful);
...
bool FPartyPlatformSessionMonitor::ConfigurePlatformSessionSettings(FOnlineSessionSettings& SessionSettings) const
{
	...
	if (bEstablishedPartySettings)
	{
		...
		MonitoredParty->AugmentPlatformSessionSettings(SessionSettings);
...
void FPartyPlatformSessionMonitor::ProcessJoinFailure()
{
	...
	{
		UE_LOGF(LogParty, Log, "PartyPlatformSessionMonitor targeted platform session [%ls] out of date - retrying with updated id [%ls]", *TargetSessionId, *ExistingSessionInfo->SessionId);
		TargetSessionId = FSessionId();
		...
		if (PlatformSession)
		{
			if (PlatformSession->SessionState >= EOnlineSessionState::Pending && PlatformSession->SessionState <= EOnlineSessionState::Ended)
			{
				if (ConfigurePlatformSessionSettings(PlatformSession->SessionSettings))
				{
					...
					{
						UE_LOGF(LogParty, Warning, "PartyPlatformSessionMonitor call to UpdateSession failed");
					}
				}
			}
...
{
	UE_LOGF(LogParty, Warning, "PartyPlatformSessionMonitor HandleSessionFailure LocalUserId=%ls FailureType=%ls", *LocalUserId.ToDebugString(), LexToString(FailureType));
