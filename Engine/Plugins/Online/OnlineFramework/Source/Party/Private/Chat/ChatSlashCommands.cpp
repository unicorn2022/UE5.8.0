	...
	if (NewSlashCommand.IsValid())
	{
		for (const TSharedPtr<FChatSlashCommand>& Cmd : RegisteredCustomSlashCommands)
		{
			if (Cmd.IsValid())
			{
				...
				{
					UE_LOGF(LogParty, Warning, "Attempting to register duplicate slash command");
					return;
				}
			}
		}
...
void FRegisteredSlashCommands::PrepareInteractionAutocompleteStrings(const TArray<FString>& UserTextTokens) 
{
	...
	{
		UE_LOGF(LogParty, Warning, "Warning: Slash Command Component not initialized with a toolkit. Slash commands will be disabled until intialization.");
	}
}
...
void FInteractionCommandWrapper::CacheStringDataForLocalization() const
{
	...
	{
		//UE_LOGF(LogParty, Warning, "null slash command interaction detected, FInteractionCommandWrapper::RecacheForLocalization");
	}
}
