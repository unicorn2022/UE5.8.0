...
public:
	...
	void FinishInitialization()
	{
		if (CVarSocialUserDeferInitializePauseIds.GetValueOnGameThread().Contains(Parent.GetUserId(ESocialSubsystem::Primary).ToString()))
		{
			...
			UE_LOGF(LogParty, Log, "%s Deferring initialization of user [%ls] until they are removed from party.SocialUserDeferInitializePauseIds", __FUNCTION__, *Parent.ToDebugString());
			...
			DelaySeconds > 0)
		{
			...
			UE_LOGF(LogParty, Log, "%s Deferring initialization of user [%ls] for %d seconds", __FUNCTION__, *Parent.ToDebugString(), DelaySeconds);
...
private:
	...
	static void OnPausedSocialUserIdsChanged(IConsoleVariable* CVar)
	{
		...
		for (TWeakObjectPtr<USocialUser>& WeakSocialUser : PausedSocialUsersCopy)
		{
			...
			{
				UE_LOGF(LogParty, Log, "%s Finishing initialization of user [%ls]", __FUNCTION__, *SocialUser->ToDebugString());
				PausedSocialUsers.Remove(WeakSocialUser);
...
public:
	...
	virtual void ExecuteQuery() override
	{
				...
				if (IUserInfoPtr UserInfo = Services->GetUserInfoInterface())
				{
					...
					if(LocalUserNetId.IsValid())
					{
						if (const FCommonAccountPtr LocalUserCommonAccountPtr = GetCommonAccountFromV1(CommonConfig, LocalUserNetId->AsShared(), GetFirstFrameworkInstanceName(LocalUserNetId)))
						{
							...
							UE_LOGF(LogParty, Log, "FSocialQuery_UserInfoV2 executing for [%d] users on services [%ls]", SocialUserAccountIds.Num(), ToString(SubsystemType));
							...
							UserInfo->QueryUserInfo({ LocalUserAccountId, SocialUserAccountIds })
								...
								{
									UE::Online::FOnlineError Error = Errors::Success();

									if (Result.IsOk())
									{
										UE_LOGF(LogParty, VeryVerbose, "FSocialQuery_UserInfoV2 completed query on services [%ls]", ToString(SubsystemType));
									...
									else
									{
										...
										UE_LOGF(LogParty, Log, "FSocialQuery_UserInfoV2 failed query on services [%ls] with error [%ls]", ToString(SubsystemType), *ToLogString(Error));										
									}
									...
									for (const FAccountId& UserAccountId : SocialUserAccountIds)
									{
										if (const FOnQueryComplete* CompletionCallback = CompletionCallbacksByUserId.Find(UserAccountId))
										{
											CompletionCallback->ExecuteIfBound(SubsystemType, false, MakeError(Error));
										}
									}
		...
		for (const FAccountId& UserAccountId : SocialUserAccountIds)
		{
			if (const FOnQueryComplete* CompletionCallback = CompletionCallbacksByUserId.Find(UserAccountId))
			{
				CompletionCallback->ExecuteIfBound(SubsystemType, false, MakeError(Errors::InvalidState()));
			}
		}
...
public:
	...
	virtual void ExecuteQuery() override
	{
		...
		if (UserInterface.IsValid() && CompletionCallbacksByUserId.Num() > 0)
		{
			...
			UE_LOGF(LogParty, Log, "FSocialQuery_UserInfo executing for [%d] users on subsystem [%ls]", UserIds.Num(), ToString(SubsystemType));
...
private:
	void HandleQueryUserInfoComplete(int32 LocalUserNum, bool bWasSuccessful, const TArray<FUniqueNetIdRef>& UserIds, const FString& ErrorStr)
	{
		...
		UE_LOGF(LogParty, Log, "FSocialQuery_UserInfo completed query for [%d] users on subsystem [%ls] with error [%ls]", UserIds.Num(), ToString(SubsystemType), *ErrorStr);
...
USocialUser::~USocialUser()
{
	if (OnReadUserInfoV2CVarChanged.IsValid())
	{
		...
		{
			CVar->OnChangedDelegate().Remove(OnReadUserInfoV2CVarChanged);
		}

		OnReadUserInfoV2CVarChanged.Reset();
	}
}
...
void USocialUser::InitLocalUser()
{
	...
	UE_LOGF(LogParty, Log, "Initializing local SocialUser for Toolkit [%d]", OwningToolkit.GetLocalUserNum());
	...
	for (ESocialSubsystem SubsystemType : USocialManager::GetDefaultSubsystems())
	{
		...
		if (IOnlineIdentityPtr IdentityInterface = OSS->GetIdentityInterface())
		{
			...
			{
				UE_LOGF(LogParty, Warning, "Local SocialUser unable to establish a valid UniqueId on subsystem [%ls]", ToString(SubsystemType));
			}
		}
	}
...
void USocialUser::Initialize(const FUniqueNetIdRepl& PrimaryId)
{
	...
	{
		UE_LOGF(LogParty, VeryVerbose, "Initializing new SocialUser with ID [%ls]", *PrimaryId.ToDebugString());
		...
		{
			UE_LOGF(LogParty, Error, "User cannot be initialized with ID [%ls] - no primary OSS available.", *PrimaryId.ToDebugString());
		}
	}
}
...
TArray<ESocialSubsystem> USocialUser::GetRelationshipSubsystems(ESocialRelationship Relationship) const
{
	...
	else
	{
		for (const TPair<ESocialSubsystem, FSubsystemUserInfo>& SubsystemInfoPair : SubsystemInfoByType)
		{
			...
			case ESocialRelationship::AnyWithLocal:
				if (Relationship == ESocialRelationship::AnyWithLocal || !IsLocalUser())
				{
...
UE::Online::EUserPresenceStatus USocialUser::GetOnlineStatusV2() const
{
	...
	if (IOnlineServicesPtr OnlineServices = CommonConfig.GetServices(PresenceFrameworkInstance))
	{
		if (IPresencePtr Presence = OnlineServices->GetPresenceInterface())
		{
			...
			for (const TPair<ESocialSubsystem, FSubsystemUserInfo>& SubsystemInfoPair : SubsystemInfoByType)
			{
				if (SubsystemInfoPair.Key == ESocialSubsystem::Primary)
				{
				...
				else if (SubsystemInfoPair.Key == ESocialSubsystem::Platform) // For Platform presence, we'll get it from V1 and convert it
				{
					EOnlinePresenceState::Type OnlineStatus = UE::Online::FPresenceOSSAdapter::StatusV2toV1(Result);
					...
					Result = UE::Online::FPresenceOSSAdapter::StatusV1toV2(OnlineStatus);
				}
			}
		}
	}
	...
	if (!bIsInitialized && NumPendingQueries == 0)
	{
		...
		if (SubsystemInfo)
		{
			...
			else
			{
				...
				UE_LOGF(LogParty, Warning, "%ls User %ls does not have UserInfo!", ANSI_TO_TCHAR(__FUNCTION__), *ToDebugString());
...
{
	UE_LOGF(LogParty, VeryVerbose, "SocialUser [%ls] fully initialized.", *ToDebugString());
	bIsInitialized = true;
#if !UE_BUILD_SHIPPING
...
{
	static const IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("onlineframework.party.shouldreaduserinfofromonlineservices"));
	if (CVar)
	{
		return CVar->GetBool();
	}
...
UE::Online::EOnlinePlatformType USocialUser::GetDisplayNameOnlinePlatformTypeFromUserPlatform(const FString& UserCurrentPlatformStr) const
{
	using namespace UE::Online;

	if (UserCurrentPlatformStr == TEXT("PS5") || UserCurrentPlatformStr == TEXT("PSN"))
	{
		return EOnlinePlatformType::PSN;
	}
	else if (UserCurrentPlatformStr == TEXT("XSX") || UserCurrentPlatformStr == TEXT("XBL"))
	{
		return EOnlinePlatformType::XBL;
	}
	else if (UserCurrentPlatformStr == TEXT("SWT2") || UserCurrentPlatformStr == TEXT("SWT"))
	{
		return EOnlinePlatformType::Nintendo;
	}
	else
	{
		// Intentional fall-through for all other platforms
						...
						if (const FCommonAccountPtr SocialUserCommonAccountPtr = GetCommonAccountFromV1(CommonConfig, SocialUserId->AsShared(), GetFirstFrameworkInstanceName(SocialUserId)))
						{
							...
							if (IOnlineServicesPtr OnlineServices = CommonConfig.GetServices(PresenceFrameworkInstance))
							{
								if (IUserInfoPtr UserInfo = OnlineServices->GetUserInfoInterface())
								{
									...
									if (ShouldSpecifyDisplayNameServicesProvider(SubsystemInfo))
									{
										EOnlinePlatformType OnlinePlatformType = GetDisplayNameOnlinePlatformTypeFromUserPlatform(UserCurrentPlatform.ToString());
									...
									// If we don't get a valid result after specifying the services provider for DisplayName, we'll call again to iterate through Epic and all available platforms
									...
									{
										UE_LOGF(LogParty, Warning, "DisplayName disparity: DisplayNameV1: %ls. DisplayNameV2: %ls", *DisplayName, *DisplayNameV2);
									}
#endif
									if (!DisplayNameV2.IsEmpty())
									{
...
FUserPlatform USocialUser::GetCurrentPlatform() const
{
	...
	else
	{
		...
		if (USocialManager::ShouldReadPresenceFromOnlineServices() || ShouldReadUserInfoFromOnlineServices())
		{
			using namespace UE::Online;
			...
			if (GetCachedPresenceResult.IsOk())
			{
				...
				if (const FPresenceProperty* PlatformAttribute = CachedPresence->Properties.Find(PLATFORM_ATTRIBUTE_KEY))
				{
					PrimaryPresencePlatformStr = PlatformAttribute->Get<FString>();
				}
			}
		}
		else
...
// jntodo: this will need to get moved to a platform-specific uplugin once we have the time
		...
		{
			UE_LOGF(LogOnline, VeryVerbose, "%ls - No Presence found for user, searching Party...", ANSI_TO_TCHAR(__FUNCTION__));
			if (USocialParty* Party = GetOwningToolkit().GetSocialManager().GetPersistentParty())
			{
				if (UPartyMember* PartyMember = Party->GetPartyMember(GetUserId(ESocialSubsystem::Primary)))
				{
					...
					UE_LOGF(LogOnline, VeryVerbose, "%ls - Party Member Found for user! RepDataPlatform: %ls", ANSI_TO_TCHAR(__FUNCTION__), *PartyMemberPlatform.ToString());
...
FString USocialUser::GetPlatformIconMarkupTag(EPlatformIconDisplayRule DisplayRule, FString& OutLegacyString) const
{
	...
	case EPlatformIconDisplayRule::Always:
		UE_LOGF(LogOnline, VeryVerbose, "    %ls - User: %ls - Returning Platform Tag %ls due to ALWAYS", ANSI_TO_TCHAR(__FUNCTION__), *GetDisplayName(), *MarkupTag);
		return MarkupTag;
	case EPlatformIconDisplayRule::AlwaysIfDifferent:
		UE_LOGF(LogOnline, VeryVerbose, "    %ls - User: %ls - CrossplayLocalPlatform? %d Returning %ls due to ALWAYSIFDIFFERENT", ANSI_TO_TCHAR(__FUNCTION__), *GetDisplayName(), UserPlatform.IsCrossplayWithLocalPlatform(), UserPlatform.IsCrossplayWithLocalPlatform() ? *MarkupTag : TEXT(""));
		...
		DisplayRule == EPlatformIconDisplayRule::AlwaysIfDifferentWhenInCrossplayParty)
	{
		if (USocialParty* Party = GetOwningToolkit().GetSocialManager().GetPersistentParty())
		{
			...
			{
				UE_LOGF(LogOnline, VeryVerbose, "    %ls - User: %ls - Returning nothing due to Not Being In Crossplay with WHENINCROSSPLAYPARTY specified", ANSI_TO_TCHAR(__FUNCTION__), *GetDisplayName());
				OutLegacyString = TEXT("");
				return TEXT("");
			}
		}
	...
	{
		UE_LOGF(LogOnline, VeryVerbose, "    %ls - User: %ls - Returning Platform Tag %ls, DisplayRule: %d, bIsCrossplayWithMe: %d", ANSI_TO_TCHAR(__FUNCTION__), *GetDisplayName(), *MarkupTag, (int32)DisplayRule, bIsCrossplayWithMe);
		return MarkupTag;
	}

	UE_LOGF(LogOnline, VeryVerbose, "    %ls - User: %ls - Returning nothing, likely due to AlwaysIfDifferentWhenInCrossplayParty and not being different, being in a Crossplay Party, and not being Different", ANSI_TO_TCHAR(__FUNCTION__), *GetDisplayName());
	OutLegacyString = TEXT("");
	return TEXT("");
}
...
bool USocialUser::IsOnline() const
{
	...
	for (const TPair<ESocialSubsystem, FSubsystemUserInfo>& SubsystemInfoPair : SubsystemInfoByType)
	{
		if ((USocialManager::ShouldReadPresenceFromOnlineServices() || ShouldReadUserInfoFromOnlineServices()) && SubsystemInfoPair.Key == ESocialSubsystem::Primary)
		{
			using namespace UE::Online;
			using namespace UE::OnlineFramework;
...
{
	return GetOwningToolkit().GetSocialManager().ValidateJoinTarget(*this, PartyTypeId, USocialManager::FValidateJoinTargetOptions{ .bCheckPlatformSession = bCheckPlatformSession, .bIsActivelyJoining = false });
}
...
{
	UE_LOGF(LogParty, VeryVerbose, "%ls - User [%ls] sent a join request to [%ls] with result[%ls]", ANSI_TO_TCHAR(__FUNCTION__), *LocalUserId.ToDebugString(), *PartyLeaderId.ToDebugString(), ToString(Result));
...
void USocialUser::NotifyUserUnblocked(ESocialSubsystem SubsystemType)
{
	if (FSubsystemUserInfo* SubsystemInfo = SubsystemInfoByType.Find(SubsystemType))
	{
		...
		{
			UE_LOGF(LogParty, Log, "SocialUser [%ls] has been unblocked on [%ls], but still has valid blocked player info. Possible leak via hard ref somewhere.",*ToDebugString(), ToString(SubsystemType));
...
void USocialUser::NotifyFriendInviteRemoved(ESocialSubsystem SubsystemType)
{
	if (FSubsystemUserInfo* SubsystemInfo = SubsystemInfoByType.Find(SubsystemType))
	{
		...
		if (SubsystemInfo->FriendInfo.IsValid())
		{
			UE_LOGF(LogParty, Log, "SocialUser [%ls] has cancelled a friend invite on [%ls], but still has valid friend info. Possible leak via hard ref somewhere.", *ToDebugString(), ToString(SubsystemType));
...
void USocialUser::NotifyUserUnfriended(ESocialSubsystem SubsystemType)
{
	if (FSubsystemUserInfo* SubsystemInfo = SubsystemInfoByType.Find(SubsystemType))
	{
		...
		if (SubsystemInfo->FriendInfo.IsValid())
		{
			UE_LOGF(LogParty, Log, "SocialUser [%ls] has been unfriended on [%ls], but still has valid friend info. Possible leak via hard ref somewhere.", *ToDebugString(), ToString(SubsystemType));
			SubsystemInfo->FriendInfo.Reset();
		}
...
void USocialUser::EstablishOssInfo(const TSharedRef<FOnlineFriend>& InFriendInfo, ESocialSubsystem SubsystemType)
{
	...
	{
		/*UE_CLOGF(!SubsystemInfo.FriendInfo.IsValid(), LogParty, Warning, "SocialUser [%ls] is establishing new friend info on [%ls], but the existing info is still valid.",
...
void USocialUser::EstablishOssInfo(const TSharedRef<FOnlineBlockedPlayer>& InBlockedPlayerInfo, ESocialSubsystem SubsystemType)
{
	...
	{
		UE_CLOGF(SubsystemInfo.BlockedPlayerInfo.IsValid(), LogParty, Warning, "SocialUser [%ls] is establishing new blocked player info on [%ls], but the existing info is still valid.",
			*ToDebugString(), ToString(SubsystemType));
...
void USocialUser::EstablishOssInfo(const TSharedRef<FOnlineRecentPlayer>& InRecentPlayerInfo, ESocialSubsystem SubsystemType)
{
	...
	{
		UE_CLOGF(SubsystemInfo.RecentPlayerInfo.IsValid(), LogParty, Warning, "SocialUser [%ls] is establishing new recent player info on [%ls], but the existing info is still valid.",
			*ToDebugString(), ToString(SubsystemType));
	...
	if (ensure(!SubsystemInfoByType.Contains(SubsystemType)) && ensure(SubsystemId.IsValid()))
	{
		...
		if (ensure(OSS))
		{
			...
			if (UserInterface.IsValid())
			{
				...
				{
					UE_LOGF(LogParty, VeryVerbose, "SocialUser [%ls] querying user info on subsystem [%ls]", *ToDebugString(), ToString(SubsystemType));

					// No valid user info exists on this subsystem, so queue up a query for it in both subsystem and services, if enabled
		...
		{
			auto QueryUserInfoV2 = [WeakThis = TWeakObjectPtr<USocialUser>(this)]()
			{
				if (USocialUser* StrongThis = WeakThis.Get())
				{
					if(StrongThis->ShouldReadUserInfoFromOnlineServices())
					{
						using namespace UE::OnlineFramework;

						UE_LOGF(LogParty, VeryVerbose, "SocialUser [%ls] querying user info on services [%ls]", *StrongThis->ToDebugString(), ToString(ESocialSubsystem::Primary));

						const FUniqueNetIdRepl UserId = StrongThis->GetUserId(ESocialSubsystem::Primary);
						if (UserId.IsValid())
						{
							FCommonConfig CommonConfig(StrongThis);
							if (const FCommonAccountPtr CommonAccountPtr = GetCommonAccountFromV1(CommonConfig, UserId->AsShared(), GetFirstFrameworkInstanceName(UserId)))
							{
								UE::Online::FAccountId AccountId = CommonAccountPtr->GetId(USocialManager::GetPresenceFrameworkInstance());
								if (AccountId.IsValid())
								{
									...
									FSocialQueryManager::AddUserId<FSocialQuery_UserInfoV2>(StrongThis->GetOwningToolkit(), ESocialSubsystem::Primary, AccountId, QueryCompleteHandler);
												
									if (!StrongThis->bIsInitialized)
									{
			...
			// We want to re-query user info whenever the functionality is enabled
			if (!OnReadUserInfoV2CVarChanged.IsValid())
			{
				if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("onlineframework.party.shouldreaduserinfofromonlineservices")))
				{
					OnReadUserInfoV2CVarChanged = CVar->OnChangedDelegate().AddWeakLambda(this, [QueryUserInfoV2](IConsoleVariable*)
						{
							QueryUserInfoV2();
						});
				}
					...
					if (const FSocialPlatformDescription* PlatformDescription = USocialSettings::GetSocialPlatformDescriptionForOnlineSubsystem(MissingOSS->GetSubsystemName()))
					{
						if (!PlatformDescription->ExternalAccountType.IsEmpty())
						{
							...
							if (UserInfo->GetUserAttribute(SubsystemIdKey, SubsystemIdStr) && !SubsystemIdStr.IsEmpty())
							{
								...
								{
									UE_LOGF(LogParty, Error, "SocialUser [%ls] Failed to create SubsystemId for SubsystemType: [%ls] SubsystemIdKey [%ls] IdPrefix: [%ls] SubsystemIdStr: [%ls] for OnlineUser [%ls]", *ToDebugString(), ToString(SubsystemType), *SubsystemIdKey, *IdPrefix, *SubsystemIdStr, *UserInfo->GetUserId()->ToDebugString());
								}
							}
						}
					}
...
bool USocialUser::ShouldSpecifyDisplayNameServicesProvider(const FSubsystemUserInfo* SubsystemInfo) const
{
...
void USocialUser::HandleQueryUserInfoComplete(ESocialSubsystem SubsystemType, bool bWasSuccessful, const TSharedPtr<FOnlineUser>& UserInfo)
{
	...
	UE_LOGF(LogParty, VeryVerbose, "User [%ls] finished querying user info on subsystem [%ls] with result [%d]. [%d] queries still pending.", *ToDebugString(), ToString(SubsystemType), bWasSuccessful, NumPendingQueries);
	TryBroadcastInitializationComplete();
}
...
void USocialUser::HandleQueryUserInfoV2Complete(ESocialSubsystem SubsystemType, bool bWasSuccessful, const TValueOrError<TSharedRef<UE::Online::FUserInfo>, UE::Online::FOnlineError>& Result)
{
	...
	UE_LOGF(LogParty, VeryVerbose, "User [%ls] finished querying user info on services [%ls] with result [%d]. [%d] queries still pending.", *ToDebugString(), ToString(SubsystemType), bWasSuccessful, NumPendingQueries);
	TryBroadcastInitializationComplete();
}
...
void USocialUser::HandleSetNicknameComplete(int32 LocalUserNum, const FUniqueNetId& FriendId, const FString& ListName, const FOnlineError& Error)
{
	...
	{
		UE_LOGF(LogOnline, Log, "Set nickname request failed for user: %ls with error message: %ls", *FriendId.ToDebugString(), *Error.GetErrorMessage().ToString());
	}
