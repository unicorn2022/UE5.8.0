...
namespace UE::OnlineFramework::CVars
{
	...
	bool bRemoveJoinAttemptInFinishFirst = true;
	static FAutoConsoleVariableRef CVarRemoveJoinAttemptInFinishFirst(
		TEXT("Party.RemoveJoinAttemptInFinishFirst"), bRemoveJoinAttemptInFinishFirst,
		TEXT("Temporary cvar (remove after proven stable), whether to remove the join attempt in FinishJoinPartyAttempt before any other calls"),
		ECVF_Default);
}
//////////////////////////////////////////////////////////////////////////
// FJoinPartyAttempt
...
void USocialManager::CreateParty(const FOnlinePartyTypeId& PartyTypeId, const FPartyConfiguration& PartyConfig, const FOnCreatePartyAttemptComplete& OnCreatePartyComplete)
{
	...
	{
		UE_LOGF(LogParty, Warning, "Existing party [%ls] of type [%d] found when trying to create a new one. Cannot create new one until the existing one has been left.", *ExistingParty->GetPartyId().ToDebugString(), PartyTypeId.GetValue());
	...
	else
	{
		...
		{
			UE_LOGF(LogParty, Warning, "Cannot create party of type [%d] - PartyInterface.IsValid=%ls PrimaryLocalUserId.IsValid=%ls primary OSS [%ls]", PartyTypeId.GetValue(), *LexToString(PartyInterface.IsValid()), *LexToString(PrimaryLocalUserId.IsValid()), *GetSocialOssName(ESocialSubsystem::Primary).ToString());
...
{
	UE_LOGF(LogParty, Log, "Attempting to create new persistent party");
...
{
	UE_LOGF(LogParty, VeryVerbose, "Validating join attempt of party of type [%d]", PartyTypeId.GetValue());
...
{
	return ValidateJoinTarget(UserToJoin, PartyTypeId, FValidateJoinTargetOptions{});
}

bool USocialManager::ShouldReadPresenceFromOnlineServices()
{
	static const IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("onlineframework.party.shouldreadpresencefromonlineservices"));
	if (CVar)
	{
		return CVar->GetBool();
	}
...
{
	return ValidateJoinTarget(UserToJoin, PartyTypeId, FValidateJoinTargetOptions{ .bCheckPlatformSession = bCheckPlatformSession });
}

FJoinPartyResult USocialManager::ValidateJoinTarget(const USocialUser& UserToJoin, const FOnlinePartyTypeId& PartyTypeId, const FValidateJoinTargetOptions& ValidateOptions) const
{
	UE_LOGF(LogParty, VeryVerbose, "Validating user [%ls] as join target of party type [%d]", *UserToJoin.ToDebugString(), PartyTypeId.GetValue());
		...
		else if (UserToJoin.IsFriend(ESocialSubsystem::Platform))
		{
			...
			if (const FOnlineUserPresence* PlatformPresence = UserToJoin.GetFriendPresenceInfo(ESocialSubsystem::Platform))
			{
				...
				else if (!UserToJoin.HasSentPartyInvite(PartyTypeId))
				{
					...
					{
						if (ValidateOptions.bCheckPlatformSession)
						{
...
{
	UE_LOGF(LogParty, Log, "Attempting to join user [%ls]'s party of type [%d] by [%ls]", *UserToJoin.ToDebugString(), PartyTypeId.GetValue(), *JoinMethod.ToString());
	...
	FJoinPartyResult ValidationResult = ValidateJoinTarget(UserToJoin, PartyTypeId, FValidateJoinTargetOptions{});
	bool bFailed = false;
	if (ValidationResult.WasSuccessful())
	{
		...
		{
			bFailed = true;
			ValidationResult = FJoinPartyResult(EJoinPartyCompletionResult::UnknownClientFailure);
			JoinAttemptsByTypeId.Remove(PartyTypeId);
		}
	}
	else
	{
	...
	if (bFailed)
	{
		...
		// We don't do the standard FinishJoinPartyAttempt here because this entry isn't actually in our map of join attempts yet
...
{
	UE_LOGF(LogParty, Log, "JoinPartyAttempt [%ls] completed with result [%ls] and reason [%ls]", *JoinAttemptInfo.ToDebugString(), ToString(Result.GetResult()), ToString(Result.GetDenialReason().GetReason()));
}
		...
		if (PrimaryUserId.IsValid() && SecondaryUserId.IsValid())
		{
			...
			if (PersistentParty->GetPartyMember(SecondaryUserId))
			{
				PersistentParty->RemoveLocalMember(SecondaryUserId, USocialParty::FOnLeavePartyAttemptComplete::CreateWeakLambda(this, [this, LocalPlayerNum, JoinDelegate](ELeavePartyCompletionResult LeaveResult)
				{
					...
					{
						UE_LOGF(LogParty, Warning, "RegisterSecondaryPlayer RemoveLocalMember failed LeaveResult=%ls", ToString(LeaveResult));
...
void USocialManager::JoinPartyInternal(FJoinPartyAttempt& JoinAttempt)
{
	...
	{
		UE_LOGF(LogParty, Error, "USocialManager::JoinPartyInternal Invalid LocalUserId=[%ls] on primary subsystem.", *LocalUserId.ToDebugString());
	}
}
...
void USocialManager::FinishJoinPartyAttempt(FJoinPartyAttempt& JoinAttemptToDestroy, const FJoinPartyResult& JoinResult)
{
	...
	if (UE::OnlineFramework::CVars::bRemoveJoinAttemptInFinishFirst)
	{
		FJoinPartyAttempt MovedJoinAttempt = MoveTemp(JoinAttemptToDestroy); // JoinAttemptToDestroy is garbage after this. Use MovedJoinAttempt.
		JoinAttemptsByTypeId.Remove(MovedJoinAttempt.PartyTypeId);
		OnJoinPartyAttemptCompleteInternal(MovedJoinAttempt, JoinResult);
		MovedJoinAttempt.OnJoinComplete.ExecuteIfBound(JoinResult);
	}
	else
	{
	...
	if (bWasPersistentPartyJoinAttempt && !JoinResult.WasSuccessful() && !GetPersistentParty())
	{
		...
		UE_CLOGF(!GetFirstLocalUserToolkit()->CanAutoRecreatePersistentParty(), LogParty, Warning, "FinishJoinPartyAttempt creating a persistent party when we shouldn't!");
...
{
	UE_LOGF(LogParty, Log, "RestorePartyStateFromPartySystem");
	FUniqueNetIdRepl LocalUserId = GetFirstLocalUserId(ESocialSubsystem::Primary);
...
void USocialManager::OnRestorePartiesComplete(const FUniqueNetId& LocalUserId, const FOnlineError& Result, const FOnRestorePartyStateFromPartySystemComplete OnRestoreComplete)
{
	...
	if (Result.WasSuccessful())
	{
		...
		for (const TSharedRef<const FOnlinePartyId>& PartyId : JoinedParties)
		{
			...
			{
				UE_LOGF(LogParty, Warning, "OnRestorePartiesComplete: User=[%ls] Party=[%ls] Type=%d failed to establish party", *LocalUserId.ToDebugString(), *PartyId->ToDebugString(), Party->PartyTypeId.GetValue());
			}
		}
	}
	...
	if (FJoinPartyAttempt* JoinAttempt = JoinAttemptsByTypeId.Find(PartyTypeId))
	{
		if (Result.EnumResult == EJoinPartyCompletionResult::Succeeded)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogParty, Log, "HandleQueryJoinabilityComplete: User=[%ls] Party=[%ls] Delaying join until persistent party creation has completed.", *LocalUserId.ToDebugString(), *PartyId.ToDebugString());
...
void USocialManager::HandleCreatePartyComplete(const FUniqueNetId& LocalUserId, const TSharedPtr<const FOnlinePartyId>& PartyId, ECreatePartyCompletionResult Result, FOnlinePartyTypeId PartyTypeId, FOnCreatePartyAttemptComplete CompletionDelegate)
{
	...
	UE_LOGF(LogParty, Verbose, "Finished trying to create party [%ls] with result [%ls]", PartyId.IsValid() ? *PartyId->ToDebugString() : TEXT("Invalid"), ToString(LocalCreationResult));
	{
...
void USocialManager::HandleJoinPartyComplete(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, EJoinPartyCompletionResult Result, int32 NotApprovedReasonCode, FOnlinePartyTypeId PartyTypeId)
{
	...
	UE_LOGF(LogParty, Log, "Attempt to join party of type [%d] completed with result [%ls] and reason code [%d] by user is [%ls] ", PartyTypeId.GetValue(), ToString(Result), NotApprovedReasonCode, *LocalUserId.ToDebugString());
	...
	else
	{
		...
		UE_LOGF(LogParty, Error, "Attempt to join party of type [%d] completed with result [%ls], but there is no existing FJoinPartyAttempt object.", PartyTypeId.GetValue(), ToString(Result));
		if (!ensure(!JoinResult.WasSuccessful()))
		{
			UE_LOGF(LogParty, Error, "Auto-bailing on party of type [%d] - cannot finish establishing it without a valid FJoinPartyAttempt.", PartyTypeId.GetValue());
			IOnlinePartyPtr PartyInterface = Online::GetPartyInterfaceChecked(GetWorld());
...
void USocialManager::HandlePersistentPartyStateChanged(EPartyState NewState, EPartyState PreviousState, USocialParty* PersistentParty)
{
	...
	UE_LOGF(LogParty, Log, "Persistent party state changed to %ls", ToString(NewState));
...
void USocialManager::HandleLeavePartyForJoinComplete(ELeavePartyCompletionResult LeaveResult, USocialParty* LeftParty)
{
	...
	if (LeftParty)
	{
		UE_LOGF(LogParty, Log, "Attempt to leave party [%ls] for pending join completed with result [%ls]", *LeftParty->ToDebugString(), ToString(LeaveResult));
	}
}
	...
	if (LeftParty)
	{
		...
		if (FJoinPartyAttempt* JoinAttempt = JoinAttemptsByTypeId.Find(PartyTypeId))
		{
			...
			else
			{
				...
				UE_LOGF(LogParty, Log, "Finished leaving party [%ls] before the current join attempt established join info. Cancelling join attempt.", *LeftParty->ToDebugString());
		...
		{
			UE_LOGF(LogParty, Verbose, "Finished leaving persistent party without a join/rejoin target. Creating a new persistent party now.");
