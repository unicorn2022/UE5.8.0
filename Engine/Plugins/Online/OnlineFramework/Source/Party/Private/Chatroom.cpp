		...
		if (LocalUserId.IsValid())
		{
			...
			if (ChatInt.IsValid())
			{
				if (!IsAlreadyInChatRoom(LocalUserId, ChatRoomId))
				{
					...
					{
						UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::CreateOrJoinChatRoom] attempting to join %ls", *ChatRoomId);
					...
					else
					{
						...
						{
							UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::CreateOrJoinChatRoom] already joining %ls", *ChatRoomId);
						}
						else
						{
							UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::CreateOrJoinChatRoom] can't join %ls already joining %ls", *ChatRoomId, *CurrentChatRoomId);
				...
				{
					UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::CreateOrJoinChatRoom] already joined %ls", *ChatRoomId);
				}
			}
		}
		else
		{
			UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::CreateOrJoinChatRoom] invalid user id");
			bShouldFireDelegate = true;
		}
	}
	else
	{
		UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::CreateOrJoinChatRoom] invalid chat room id");
		bShouldFireDelegate = true;
	}
...
void UChatroom::OnChatRoomCreatedOrJoined(const FUniqueNetId& LocalUserId, const FChatRoomId& RoomId, bool bWasSuccessful, const FString& Error, FOnChatRoomCreatedOrJoined CompletionDelegate, FChatRoomConfig RoomConfig)
{
	...
	{
		UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::OnChatRoomCreatedOrJoined] %ls joined room %ls Success: %d%ls", *LocalUserId.ToString(), *RoomId, bWasSuccessful, !Error.IsEmpty() ? *FString::Printf(TEXT(" %s"), *Error) : TEXT(""));
		...
		if (!bAlreadyJoining)
		{
			...
			else
			{
				...
				{
					NumChatRoomRetries++;
					UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::OnChatRoomCreatedOrJoined] retry %d/%d", NumChatRoomRetries, MaxChatRoomRetries);
				...
				{
					UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::OnChatRoomCreatedOrJoined] exceeded %d retries", MaxChatRoomRetries);
				}
		...
		{
			UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::OnChatRoomCreatedForRejoin] already attempting to join %ls", *CurrentChatRoomId);
		}
	}
	else
	{
		UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::OnChatRoomCreatedForRejoin] other chat room detected %ls, waiting for %ls", *RoomId, *CurrentChatRoomId);
	}
}
...
void UChatroom::LeaveChatRoom(const FUniqueNetIdRepl& LocalUserId, const FOnChatRoomLeft& CompletionDelegate)
{
	if (!CurrentChatRoomId.IsEmpty())
	{
		UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::LeaveChatRoom] %ls leaving chat room %ls",
		...
		{
			UE_LOGF(LogOnlineChat, Display, "[UChatroom::LeaveChatRoom] Left chat while not logged in");
	...
	{
		UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::LeaveChatRoom] %ls no chat room to leave.", *LocalUserId.ToDebugString());
	}
}
...
void UChatroom::OnChatRoomLeft(const FUniqueNetId& LocalUserId, const FChatRoomId& RoomId, bool bWasSuccessful, const FString& Error, FChatRoomId ChatRoomIdCopy, FOnChatRoomLeft CompletionDelegate)
{
	if (ChatRoomIdCopy == RoomId)
	{
		...
		UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::OnChatRoomLeft] %ls left chat room %ls Success: %d%ls", *LocalUserId.ToDebugString(), *RoomId, bWasSuccessful, !Error.IsEmpty() ? *FString::Printf(TEXT(" %s"), *Error) : TEXT(""));
	...
	{
		UE_LOGF(LogOnlineChat, Verbose, "[UChatroom::OnChatRoomLeft] other chat room detected %ls, waiting for %ls", *RoomId, *ChatRoomIdCopy);
	}
}
...
UWorld* UChatroom::GetWorld() const
{
	...
	{
		UE_LOGF(LogOnlineChat, Warning, "[UChatroom::GetWorld] Called GetWorld on the CDO");
		return nullptr;
	}
	else
	{
