...
public:
	...
	/** Fires after received rep data has been fully applied */
	FSimpleMulticastDelegate& OnPostReplicationProcessed() const { return OnPostReplicationProcessedEvent; }
...
protected:
	...
	UE_API virtual void PostReplication(const FOnlinePartyRepDataBase& OldData, TSharedPtr<FStreamableHandle> InResourceHandle) override;
...
private:
	...
	mutable FSimpleMulticastDelegate OnPostReplicationProcessedEvent;
};
...
public:
	...
	/**
	 * Event triggered when the party leader changes.
	 * @param NewLeader the new leader. Generally non-null, but there may be race conditions where we get the leader id before we get the leader's data.
	 */
	DECLARE_EVENT_OneParam(USocialParty, FOnPartyLeaderChanged, UPartyMember* /*NewLeader*/);
	FOnPartyLeaderChanged& OnPartyLeaderChanged() const { return OnPartyLeaderChangedEvent; }
	...
	/** Hook for subclasses to add game-specific keys to NAME_PartySession (default: no-op). */
	UE_API virtual void AugmentPlatformSessionSettings(FOnlineSessionSettings& SessionSettings) const;

	/** Fired by subclasses to ask FPartyPlatformSessionMonitor to re-advertise the platform session. */
	DECLARE_EVENT(USocialParty, FOnPlatformSessionUpdateRequested);
	FOnPlatformSessionUpdateRequested& OnPlatformSessionUpdateRequested() const { return OnPlatformSessionUpdateRequestedEvent; }
...
protected:
	...
	UE_API void NotifyRepDataProcessed();
...
protected:
	...
	/**
	 * Perform the Away status check for an incoming join request
	 * @return true if the join request passes the away check
	 */
	UE_API virtual bool EvaluateJoinRequestAwayCheck(const TArray<IOnlinePartyUserPendingJoinRequestInfoConstRef>& Players, bool bFromJoinRequest) const;
...
private:
	...
	*/
	...
	/** Maximum number of join in progress responses to store in member rep data. Oldest responses are removed first. */
	UPROPERTY(config)
	int32 MaxJoinInProgressResponses = 12;

	/** Maximum number of join in progress responses to store per requesting member. Oldest responses are removed first. */
	UPROPERTY(config)
	int32 MaxJoinInProgressResponsesPerMember = 5;
	...
	mutable FOnPartyLeaderChanged OnPartyLeaderChangedEvent;
	...
	mutable FOnPlatformSessionUpdateRequested OnPlatformSessionUpdateRequestedEvent;
};
