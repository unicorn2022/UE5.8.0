...
PARTY_API const TCHAR* ToString(EPartyJoinDenialReason Type);
...
PARTY_API const TCHAR* LexToString(ESocialPartyInviteFailureReason Type);
...
struct FOnlinePartyRepDataBase
{
	...
	virtual ~FOnlinePartyRepDataBase() = default;

protected:
	/**
	 * Gets any assets that should be async loaded before PostReplication is called.
	 * This is only used if Party.ReplicationAsyncLoadingEnabled is enabled.
	 */
	virtual void GetAssetsToAsyncLoad(TArray<FSoftObjectPath>& OutAssets) const {}
	 ...
	 * InResourceHandle will contain any resources returned by GetAssetsToAsyncLoad that were loaded.
	 * This handle should be retained to keep these resources in memory.
	 */
	virtual void PostReplication(const FOnlinePartyRepDataBase& OldData, TSharedPtr<FStreamableHandle> InResourceHandle) {}
	UE_DEPRECATED(5.8, "Please use version taking InResourceHandle")
...
PARTY_API const TCHAR* ToString(EPartyType Type);
PARTY_API const TCHAR* ToString(EApprovalAction Type);

/** Defines the means used to join a party*/
namespace PartyJoinMethod
{
	// User has created the party
	extern PARTY_API const FName Creation;
	// User has joined the party via invitation
	extern PARTY_API const FName Invitation;
	// user has joined after requesting access
	extern PARTY_API const FName RequestToJoin;
	// User has joined the party via presence
	extern PARTY_API const FName Presence;
	// User has joined the party using a platform option
	extern PARTY_API const FName PlatformSession;
	// User has joined the party via command line
	extern PARTY_API const FName CommandLineJoin;
	// User has joined via unknown/undocumented process
	extern PARTY_API const FName Unspecified;
}
