...
void USocialChatManager::JoinChatRoomPublic(const FChatRoomId& RoomId, const FChatRoomConfig& InChatRoomConfig /*= FChatRoomConfig()*/, ESocialSubsystem InSocialSubsystem /*= ESocialSubsystem::Primary*/)
{
	if (!RoomId.IsEmpty())
	{
		...
		if (ChatInterface.IsValid())
		{
			...
			UE_LOGF(LogOnline, VeryVerbose, "USocialChatManager::JoinChatRoomPublic - Attempting to join room %ls", *RoomId);
		}
		else
		{
			UE_LOGF(LogOnline, Warning, "USocialChatManager::JoinChatRoomPublic - Missing ChatInterface for subsystem %ls when asked to join room %ls", LexToString(InSocialSubsystem), *RoomId);
		}
	}
	else
	{
		UE_LOGF(LogOnline, Warning, "USocialChatManager::JoinChatRoomPublic - Missing RoomId when asked to join room");
	}
}
...
void USocialChatManager::JoinChatRoomPrivate(const FChatRoomId& RoomId, const FChatRoomConfig& InChatRoomConfig /*= FChatRoomConfig()*/, ESocialSubsystem InSocialSubsystem /*= ESocialSubsystem::Primary*/)
{
	if (!RoomId.IsEmpty())
	{
		...
		if (ChatInterface.IsValid())
		{
			...
			UE_LOGF(LogOnline, VeryVerbose, "USocialChatManager::JoinChatRoomPrivate - Attempting to join room %ls", *RoomId);
		}
		else
		{
			UE_LOGF(LogOnline, Warning, "USocialChatManager::JoinChatRoomPrivate - Missing ChatInterface for subsystem %ls when asked to join room %ls", LexToString(InSocialSubsystem), *RoomId);
		}
	}
	else
	{
		UE_LOGF(LogOnline, Warning, "USocialChatManager::JoinChatRoomPrivate - Missing RoomId when asked to join room");
	}
}
...
void USocialChatManager::ExitChatRoom(const FChatRoomId& RoomId, ESocialSubsystem InSocialSubsystem /*= ESocialSubsystem::Primary*/)
{
	if (!RoomId.IsEmpty())
	{
		...
		if (ChatInterface.IsValid())
		{
			...
			UE_LOGF(LogOnline, VeryVerbose, "USocialChatManager::ExitChatRoom - Attempting to exit room %ls", *RoomId);
		}
		else
		{
			UE_LOGF(LogOnline, Warning, "USocialChatManager::ExitChatRoom - Missing ChatInterface for subsystem %ls when asked to join room %ls", LexToString(InSocialSubsystem), *RoomId);
		}
	}
	else
	{
		UE_LOGF(LogOnline, Warning, "USocialChatManager::ExitChatRoom - Missing RoomId when asked to join room");
	}
}
