...
public:
	...
	UE_API virtual bool InviteToParty(const FOnlinePartyTypeId& PartyTypeId, const ESocialPartyInviteMethod InviteMethod = ESocialPartyInviteMethod::Other, const FString& MetaData = FString()) const;
...
protected:
	...
	// This method should be extended in derived classes to make any additional subsystem user info checks needed that might modify the services provider used in GetDisplayName
	UE_API virtual bool ShouldSpecifyDisplayNameServicesProvider(const FSubsystemUserInfo* SubsystemInfo) const;
	UE_API virtual UE::Online::EOnlinePlatformType GetDisplayNameOnlinePlatformTypeFromUserPlatform(const FString& UserCurrentPlatformStr) const;
...
private:
	...
	FDelegateHandle OnReadUserInfoV2CVarChanged;
