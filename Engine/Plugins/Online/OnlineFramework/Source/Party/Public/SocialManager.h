...
#define ABORT_DURING_SHUTDOWN() if (IsEngineExitRequested() || bShutdownPending) { UE_LOGF(LogParty, Log, "%ls - Received callback during shutdown: IsEngineExitRequested=%ls, bShutdownPending=%ls.", ANSI_TO_TCHAR(__FUNCTION__), *LexToString(IsEngineExitRequested()), *LexToString(bShutdownPending)); return; }
...
public:
	...
	UE_DEPRECATED(5.7, "Use ValidateJoinTarget that takes an options struct")
	...
	struct FValidateJoinTargetOptions
	{
		...
		bool bIsActivelyJoining = true;
	};
	/** Validates that the target user has valid join info for us to use and that we can join any party of the given type */
	UE_API virtual FJoinPartyResult ValidateJoinTarget(const USocialUser& UserToJoin, const FOnlinePartyTypeId& PartyTypeId, const FValidateJoinTargetOptions& ValidateOptions) const;
...
protected:
	...
	UE_API virtual void HandleWorldEstablished(UWorld* World);
private:
