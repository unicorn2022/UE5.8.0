...
{
	UE_LOGF(LogLobbyBeacon, Verbose, "Lobby beacon connection established.");
...
void ALobbyBeaconClient::ConnectToLobby(const FOnlineSessionSearchResult& DesiredHost)
{
	...
	if (OnlineSub)
	{
		...
		if (SessionInt.IsValid())
		{
			...
			if (SessionInt->GetResolvedConnectString(DesiredHost, NAME_BeaconPort, ConnectInfo))
			{
				...
				{
					UE_LOGF(LogLobbyBeacon, Warning, "ConnectToLobby: Failure to init client beacon with %ls.", *ConnectURL.ToString());
				}
			}
		}
	}
	...
	if (!bSuccess)
	{
		AOnlineBeacon::OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_lobbyclient_init_failure"));
	}
}

void ALobbyBeaconClient::ClientJoinGame_Implementation()
{
	UE_LOGF(LogLobbyBeacon, Log, "ClientJoinGame signal %d", bLoggedIn);
	if (bLoggedIn)
	{
...
void ALobbyBeaconClient::SetPartyOwnerId(const FUniqueNetIdRepl& InUniqueId, const FUniqueNetIdRepl& InPartyOwnerId)
{
	...
	{
		UE_LOGF(LogLobbyBeacon, Warning, "Not logged in when calling SetPartyOwnerId");
	}
}
...
void ALobbyBeaconClient::DisconnectFromLobby()
{
	if (bLoggedIn)
	{
		UE_LOGF(LogLobbyBeacon, Log, "DisconnectFromLobby %ls Id: %ls", *GetName(), PlayerState ? *PlayerState->UniqueId->ToString() : TEXT("Unknown"));
		ServerDisconnectFromLobby();
		bLoggedIn = false;
	}
	else
	{
		UE_LOGF(LogLobbyBeacon, Verbose, "Not logged in when calling DisconnectFromLobby");
	}
}
...
void ALobbyBeaconClient::JoiningServer()
{
	if (bLoggedIn)
	{
		...
		{
			UE_LOGF(LogLobbyBeacon, Log, "JoiningServer %ls Id: %ls", *GetName(), PlayerState ? *PlayerState->UniqueId->ToString() : TEXT("Unknown"));
			LobbyJoinServerState = ELobbyBeaconJoinState::SentJoinRequest;
			ServerNotifyJoiningServer();
		}
		else
		{
			UE_LOGF(LogLobbyBeacon, Warning, "Already joining server, skipping %d", static_cast<int32>(LobbyJoinServerState));
		}
	}
	else
	{
		UE_LOGF(LogLobbyBeacon, Warning, "Not logged in when calling JoiningServer");
	}
}
...
void ALobbyBeaconClient::KickPlayer(const FUniqueNetIdRepl& PlayerToKick, const FText& Reason)
{
	...
	{
		UE_LOGF(LogLobbyBeacon, Warning, "Not logged in when calling KickPlayer");
	}
}
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ServerLoginPlayer %ls %ls.", *InUniqueId.ToString(), *UrlString);
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ClientLoginComplete %ls %ls.", *InUniqueId.ToString(), bWasSuccessful ? TEXT("Success") : TEXT("Failure"));
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ServerDisconnectFromLobby %ls Id: %ls", *GetName(), PlayerState ? *PlayerState->UniqueId->ToString() : TEXT("Unknown"));
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ServerNotifyJoiningGame %ls Id: %ls", *GetName(), PlayerState ? *PlayerState->UniqueId->ToString() : TEXT("Unknown"));
	ALobbyBeaconHost* BeaconHost = Cast<ALobbyBeaconHost>(GetBeaconOwner());
	if (BeaconHost)
	{
...
void ALobbyBeaconClient::AckJoiningServer()
{
	...
	{
		UE_LOGF(LogLobbyBeacon, Log, "AckJoiningServer %ls Id: %ls", *GetName(), PlayerState ? *PlayerState->UniqueId->ToString() : TEXT("Unknown"));
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ClientAckJoiningServer %ls Id: %ls LoggedIn: %d", *GetName(), PlayerState ? *PlayerState->UniqueId->ToString() : TEXT("Unknown"), bLoggedIn);
	LobbyJoinServerState = ELobbyBeaconJoinState::JoinRequestAcknowledged;
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ServerKickPlayer %ls -> %ls", (PlayerState != nullptr ? *PlayerState->UniqueId.ToString() : TEXT("")),
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ClientPlayerJoined %ls %ls.", *NewPlayerName.ToString(), *InUniqueId.ToDebugString());
...
{
	UE_LOGF(LogLobbyBeacon, Log, "ClientPlayerLeft %ls", *InUniqueId.ToDebugString());
