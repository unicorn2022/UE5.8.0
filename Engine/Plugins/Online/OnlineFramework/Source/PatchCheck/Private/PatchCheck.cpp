...
void FPatchCheck::RefreshConfig()
{
	...
	UE_LOGF(LogPatchCheck, VeryVerbose, "[%s] [bCheckPlatformOSSForUpdate=%ls], [bCheckOSSForUpdate=%ls], [bPlatformEnvironmentDetectionEnabled=%ls]",
...
{
	UE_LOGF(LogPatchCheck, VeryVerbose, "[%s] [Result=%ls]", __FUNCTION__, *Result.ToLogString());
	...
	else
	{
		...
		{
			UE_LOGF(LogPatchCheck, Warning, "[%s] Failed to complete login because patch is required", __FUNCTION__);
		...
		else
		{
			...
			{
				UE_LOGF(LogPatchCheck, Warning, "[%s] Failed to detect online environment for the platform, no user signed in", __FUNCTION__);
				PatchCheckComplete(EPatchCheckResult::NoLoggedInUser);
			}
			else
			{
				// just a platform env error, assume production and keep going
				UE_LOGF(LogPatchCheck, Warning, "[%s] Failed to detect online environment for the platform", __FUNCTION__);
				bPlatformEnvironmentDetected = true;
				HandleOSSPatchCheck();
			}
		}
...
{
	UE_LOGF(LogPatchCheck, VeryVerbose, "[%s] [bIsCheckInProgress=%ls], [bPlatformEnvironmentDetectionEnabled=%ls], [bPlatformEnvironmentDetected=%ls]",
...
{
	UE_LOGF(LogPatchCheck, VeryVerbose, "[%s]", __FUNCTION__);
	...
	{
		UE_LOGF(LogPatchCheck, Warning, "Patch check disabled for both Platform and Default OSS");
	}
}
...
void FPatchCheck::StartPlatformOSSPatchCheck()
{
	...
	if (PlatformOnlineIdentity.IsValid())
	{
		...
		{
			UE_LOGF(LogPatchCheck, Warning, "No valid platform user id when starting patch check!");
...
void FPatchCheck::StartOSSPatchCheck()
{
	...
	UE_LOGF(LogPatchCheck, Verbose, "[%s] [bSkipPatchCheck=%ls]", __FUNCTION__, *LexToString(bSkipPatchCheck));
	if (TSharedPtr<IPatchCheckStatsCollector> StrongStats = Stats.Pin())
	{
...
bool FPatchCheck::SkipPatchCheck() const
{
	...
	UE_LOGF(LogPatchCheck, VeryVerbose, "[%s] [bSkipPatchCheck=%ls], [bForcePatchCheck=%ls], [bEnvironmentWantsPatchCheck=%ls], [bSkipDueToEditor=%ls], [bSkipDueToDedicatedServer=%ls], [bForceSkipCheck=%ls], [bSkipDueToUnattended=%ls]",
...
void FPatchCheck::OnCheckForPatchComplete(const FUniqueNetId& UniqueId, EUserPrivileges::Type Privilege, uint32 PrivilegeResult, bool bConsoleCheck)
{
	...
	{
		UE_LOGF(LogPatchCheck, Log, "[%s] Simulating patch check failure.", __FUNCTION__);
		Result = EPatchCheckResult::PatchCheckFailure;
	}
	...
	{
		UE_LOGF(LogPatchCheck, Log, "[%s] Simulating required patch available.", __FUNCTION__);
		Result = EPatchCheckResult::PatchRequired;
	}
#endif

	UE_LOGF(LogPatchCheck, Verbose, "[%s] [Type=%ls], [Privilege=%ls], [PrivilegeResult=%ls], [PrivilegeResultValue=%d], [PatchCheckResult=%ls]",
	...
	if (Result == EPatchCheckResult::PatchCheckFailure)
	{
		if (CVarPatchCheckFailOpenOnError.GetValueOnGameThread())
		{
			...
			UE_LOGF(LogPatchCheck, Log, "[%s] Remapping failure to NoPatchRequired. [Type=%ls], [LastSuccess=%ls]",
...
{
	UE_LOGF(LogPatchCheck, Log, "[%s] [PatchResult=%ls]", __FUNCTION__, LexToString(PatchResult));
	if (TSharedPtr<IPatchCheckStatsCollector> StrongStats = Stats.Pin())
	{
