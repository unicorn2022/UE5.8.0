...
void FRegionQosInstance::GetSubregionPreferences(TArray<FString>& OutSubregions) const
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("McpSubregion="), OverrideSubregion))
	{
		...
		OutSubregions.Add(OverrideSubregion);
		UE_LOGF(LogQos, Verbose, "[FRegionQosInstance::GetSubregionPreferences] overriding subregion to %ls", *OverrideSubregion);
		return;
	}
#endif
...
{
	UE_LOGF(LogQos, VeryVerbose, "[FRegionQosInstance::SortDatacenterOptionsByAvgPingAsc] sorting datacenter results for region %ls", *GetRegionId());
...
{
	UE_LOGF(LogQos, VeryVerbose, "[FRegionQosInstance::SortDatacenterSubspacesByRecommended] sorting datacenter results for region %ls", *GetRegionId());
...
{
	UE_LOGF(LogQos, VeryVerbose, "Region %ls:", *GetRegionId());

	for (const FDatacenterQosInstance& Datacenter : DatacenterOptions)
	{
		UE_LOGF(LogQos, VeryVerbose, "  %ls  avg ping: %dms, num responses: %d", *Datacenter.Definition.Id, Datacenter.AvgPingMs, Datacenter.NumResponses);
	}
}
...
{
	UE_LOGF(LogQos, Log, "[UQosRegionManager::PostReloadConfig] begin");
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		for (int32 RegionIdx = RegionOptions.Num() - 1; RegionIdx >= 0; RegionIdx--)
		{
			...
			UE_LOGF(LogQos, Log, "[UQosRegionManager::PostReloadConfig] RegionId '%ls' found in RegionDefinitions? %d", *RegionOption.Definition.RegionId, bFound);
			if (!bFound)
			{
		...
		for (int32 RegionIdx = 0; RegionIdx < RegionDefinitions.Num(); RegionIdx++)
		{
			...
			UE_LOGF(LogQos, Log, "[UQosRegionManager::PostReloadConfig] RegionId '%ls' found in RegionOptions? %d", *RegionDef.RegionId, bFound);
		...
		UE_LOGF(LogQos, Log, "[UQosRegionManager::PostReloadConfig] firing OnQoSSettingsChanged delegate");
...
bool UQosRegionManager::IsSubspaceBiasOrderEnabled() const
{
	...
	{
		UE_LOGF(LogQos, VeryVerbose, "[UQosRegionManager::IsSubspaceBiasOrderEnabled] overridden by command-line to: %ls", *LexToString(bEnabledCommandLine));
		return bEnabledCommandLine;
	}

	UE_LOGF(LogQos, VeryVerbose, "[UQosRegionManager::IsSubspaceBiasOrderEnabled] set in ini or defaulted to: %ls", *LexToString(bEnableSubspaceBiasOrder));
...
void UQosRegionManager::RefreshUsableSubregions()
{
	...
	UE_LOGF(LogQos, Log, "[UQosRegionManager::RefreshUsableSubregions] Refreshed subreion list. [%ls]", *FString::Join(UsableSubregions, TEXT(",")));
}
...
void UQosRegionManager::BeginQosEvaluation(UWorld* World, const TSharedPtr<IAnalyticsProvider>& AnalyticsProvider, const FSimpleDelegate& OnComplete)
{
	...
	UE_LOGF(LogQos, Log, "[UQosRegionManager::BeginQosEvaluation] starting!");
	...
	{
		UE_LOGF(LogQos, Log, "[UQosRegionManager::BeginQosEvaluation] cached results are valid (from %.2f sec ago), using those", (FDateTime::UtcNow() - LastCheckTimestamp).GetTotalSeconds());
	...
	{
		UE_LOGF(LogQos, Log, "[UQosRegionManager::BeginQosEvaluation] no eval in progress, creating new evaluator");
...
void UQosRegionManager::OnQosEvaluationComplete(EQosCompletionResult Result, const TArray<FDatacenterQosInstance>& DatacenterInstances, FString* OutSelectedRegion, FString* OutSelectedSubRegion)
{
	...
	UE_LOGF(LogQos, Verbose, "[UQosRegionManager::OnQosEvaluationComplete] bSubspaceBiasEnabled=%ls", *LexToString(bSubspaceBiasEnabled));
	...
	for (const FQosRegionInfo& RegionInfo : RegionDefinitions)
	{
		if (RegionInfo.IsPingable())
		{
			if (DatacenterMap.Num(RegionInfo.RegionId))
			{
				...
				UE_LOGF(LogQos, VeryVerbose, "[UQosRegionManager::OnQosEvaluationComplete] pre-sort results for region=%ls, bias=%ls",
				...
				UE_LOGF(LogQos, VeryVerbose, "[UQosRegionManager::OnQosEvaluationComplete] post-sort results for region=%ls, bias=%ls",
					*RegionInfo.RegionId, *LexToString(bSubspaceBias));
				NewRegion->LogDatacenterResults();
			}
			else
			{
				UE_LOGF(LogQos, Warning, "No datacenters for region %ls", *RegionInfo.RegionId);
			}
		}
	}
	...
	UE_LOGF(LogQos, Log, "[UQosRegionManager::OnQosEvaluationComplete] ping eval has completed.  Best region is '%ls', recommended subregion is '%ls'",
...
FString UQosRegionManager::GetRegionId() const
{
	if (!ForceRegionId.IsEmpty())
	{
		//UE_LOGF(LogQos, VeryVerbose, "[UQosRegionManager::GetRegionId] Force region: \"%ls\"", *ForceRegionId);
	...
	if (QosEvalResult == EQosCompletionResult::Invalid)
	{
		UE_LOGF(LogQos, VeryVerbose, "[UQosRegionManager::GetRegionId] No QoS result: \"%ls\"", NO_REGION);
	...
	if (SelectedRegionId.IsEmpty())
	{
		...
		UE_LOGF(LogQos, Verbose, "No region currently set.");
		return NO_REGION;
	}

	UE_LOGF(LogQos, VeryVerbose, "[UQosRegionManager::GetRegionId] Selected region: \"%ls\"", *SelectedRegionId);
...
FString UQosRegionManager::GetBestRegion() const
{
	if (!ForceRegionId.IsEmpty())
	{
		UE_LOGF(LogQos, Verbose, "[UQosRegionManager::GetBestRegion] Force region: %ls", *ForceRegionId);
	...
	UE_LOGF(LogQos, Verbose, "[UQosRegionManager::GetBestRegion] Best region: \"%ls\"  (Current selected: \"%ls\")",
...
TMap<FString, int64> UQosRegionManager::GetAllSubregionPingsMap() const
{
	TMap<FString, int64> Pings;
	for (const FRegionQosInstance& Region : RegionOptions)
	{
		for (const FDatacenterQosInstance& Option : Region.DatacenterOptions)
		{
			Pings.Add(Option.Definition.Id, Option.AvgPingMs);
		}
	}
	return Pings;
}
...
void UQosRegionManager::ForceSelectRegion(const FString& InRegionId)
{
	if (!bRegionForcedViaCommandline)
	{
		...
		UE_LOGF(LogQos, Verbose, "[UQosRegionManager::ForceSelectRegion] Force region: \"%ls\"", *ForceRegionId);

		// make sure we can select this region
		if (!SetSelectedRegion(ForceRegionId, true))
		{
			UE_LOGF(LogQos, Log, "Failed to force set region id %ls", *ForceRegionId);
			ForceRegionId.Empty();
		}
	}
	else
	{
		UE_LOGF(LogQos, Log, "Forcing region %ls skipped because commandline override used %ls", *InRegionId, *ForceRegionId);
	}
}
...
void UQosRegionManager::TrySetDefaultRegion()
{
	if (!IsRunningDedicatedServer())
	{
		...
		UE_LOGF(LogQos, Verbose, "[UQosRegionManager::TrySetDefaultRegion] setting default from GetRegionId() (%ls)", *RegionId);
		...
		if (!SetSelectedRegion(RegionId))
		{
			...
			UE_LOGF(LogQos, Verbose, "[UQosRegionManager::TrySetDefaultRegion] setting default from best (%ls)", *BestRegionId);

			if (!SetSelectedRegion(BestRegionId))
			{
				UE_LOGF(LogQos, Warning, "Unable to set a good region!  Wanted to set '%ls'; fall back to '%ls' failed; current selected: '%ls'",
...
bool UQosRegionManager::IsUsableRegion(const FString& InRegionId) const
{
	...
	UE_LOGF(LogQos, Log, "IsUsableRegion: failed to find region id %ls", *InRegionId);
	return false;
}
...
bool UQosRegionManager::SetSelectedRegion(const FString& InRegionId, bool bForce)
{
		...
		for (const FRegionQosInstance& RegionInfo : LocalRegionOptions)
		{
			if (RegionInfo.Definition.RegionId == RegionId)
			{
				if (RegionInfo.IsUsable())
				{
					...
					UE_LOGF(LogQos, Verbose, "[UQosRegionManager::SetSelectedRegion] Old: \"%ls\"  New: \"%ls\"  (force? %ls)",
	...
	{
		UE_LOGF(LogQos, Log, "SetSelectedRegion: failed to find region id %ls", *InRegionId);
	}
	return false;
}
...
{
	UE_LOGF(LogQos, Verbose, "[UQosRegionManager::ClearSelectedRegion] Selected region: \"%ls\"  Forced region: \"%ls\"  bRegionForcedViaCommandline=%ls",
...
void UQosRegionManager::SanityCheckDefinitions() const
{
	...
	{
		UE_CLOGF(!Region.IsValid(), LogQos, Warning, "Invalid QOS region entry!");
	}

	// Check data syntax
	for (const FQosDatacenterInfo& Datacenter : DatacenterDefinitions)
	{
		UE_CLOGF(!Datacenter.IsValid(), LogQos, Warning, "Invalid QOS datacenter entry!");
	}
	...
	for (const FQosDatacenterInfo& Datacenter : DatacenterDefinitions)
	{
		...
		{
			UE_LOGF(LogQos, Warning, "Datacenter %ls has undefined parent region %ls", *Datacenter.Id, *Datacenter.RegionId);
		}
	}
	...
	for (const FQosRegionInfo& Region : RegionDefinitions)
	{
		...
		if (NumDatacenters == 0)
		{
			UE_LOGF(LogQos, Warning, "Region %ls has no datacenters", *Region.RegionId);
		}

		if (NumDatacenters > 0 && NumPingableDatacenters == 0)
		{
			UE_LOGF(LogQos, Warning, "Region %ls has %d datacenters, all disabled", *Region.RegionId, NumDatacenters);
		}
	}
	...
	for (const FQosRegionInfo& Region : RegionDefinitions)
	{
		if (Region.IsAutoAssignable())
		{
			...
			UE_LOGF(LogQos, Display, "AutoRegion %ls: %d datacenters available", *Region.RegionId, NumPingableDatacenters);
		}
	}
	...
	{
		UE_LOGF(LogQos, Warning, "No auto assignable regions available!");
	}
}

void UQosRegionManager::DumpRegionStats() const
{
	UE_LOGF(LogQos, Display, "Region Info:");
	UE_LOGF(LogQos, Display, "Current: %ls ", *SelectedRegionId);
	if (!ForceRegionId.IsEmpty())
	{
		UE_LOGF(LogQos, Display, "Forced: %ls ", *ForceRegionId);
	}
	...
	UE_LOGF(LogQos, Display, "Definitions:");
	for (const FQosRegionInfo& RegionDef : RegionDefinitions)
	{
		...
		UE_LOGF(LogQos, Display, "\tRegion: %ls [%ls] (%d datacenters)", *RegionDef.DisplayName.ToString(), *RegionDef.RegionId, OutValues.Num());
		UE_LOGF(LogQos, Display, "\t Enabled: %d Visible: %d Beta: %d", RegionDef.bEnabled, RegionDef.bVisible, RegionDef.bAutoAssignable);
			...
			for (const FDatacenterQosInstance& Datacenter : (*RegionInst)->DatacenterOptions)
			{
				for (const FQosDatacenterInfo* DatacenterDef : OutValues)
				{
					if (DatacenterDef->Id == Datacenter.Definition.Id)
					{
						...
						UE_LOGF(LogQos, Display, "\t  Datacenter: %ls%ls %dms (%0.2f%%) %ls", 
		...
		for (const FQosDatacenterInfo* DatacenterDef : OutValues)
		{
			UE_CLOGF(!FoundSubregions.Contains(DatacenterDef->Id), LogQos, Display, "\t  Datacenter: %ls%ls", *DatacenterDef->Id, !DatacenterDef->bEnabled ? TEXT(" Disabled") : TEXT(""));
		}

		if (!RegionInst)
		{
			UE_LOGF(LogQos, Display, "No instances for region");
		}
	}

	UE_LOGF(LogQos, Display, "Results: %ls", ToString(QosEvalResult));
...
{
	UE_LOGF(LogQos, Log, "[UQosRegionManager::RegisterQoSSettingsChangedDelegate] delegate was replaced");
	// add to the completion delegate
