...
void UQosEvaluator::FindDatacenters(const FQosParams& InParams, const TArray<FQosRegionInfo>& InRegions, const TArray<FQosDatacenterInfo>& InDatacenters, const FOnQosSearchComplete& InCompletionDelegate)
{
			...
			if (Region.IsPingable())
			{
				...
				for (const FQosDatacenterInfo& Datacenter : InDatacenters)
				{
					if (Datacenter.RegionId == Region.RegionId)
					{
						...
						{
							UE_LOGF(LogQos, Verbose, "Skipping datacenter [%ls]", *Datacenter.Id);
						}
					}
				}
				...
				if (NumDatacenters == 0)
				{
					UE_LOGF(LogQos, Warning, "Region [%ls] has no usable datacenters", *Region.RegionId);
				}
			}
			else
			{
				UE_LOGF(LogQos, Verbose, "Skipping region [%ls]", *Region.RegionId);
			}
		}
	...
	{
		UE_LOGF(LogQos, Log, "Qos evaluation already in progress, ignoring");
...
{
	UE_LOGF(LogQos, Verbose, "ping average detail per datacenter:");
	for (FDatacenterQosInstance& Datacenter : Datacenters)
	{
		...
		UE_LOGF(LogQos, Verbose, "\t%ls (%ls): %d/%d queries succeeded, average ping: %dms (adj: %dms)", *Datacenter.Definition.Id, *Datacenter.Definition.RegionId, NumResults, NumResults + NumUnreachable, Datacenter.AvgPingMs, RawAvgPing);
...
{
	UE_LOGF(LogQos, Log, "UQosEvaluator OnEchoManyCompleted; result status=%d", int(FinalResult.Status));
	...
	for (const FIcmpEchoManyResult& EchoManyResult : FinalResult.AllResults)
	{
		...
		if (FoundDatacenter)
		{
			...
			UE_LOGF(LogQos, VeryVerbose, "Ping Complete %ls %ls: %d",
			...
			if (QosStats.IsValid())
			{
				...
				UE_LOGF(LogQos, VeryVerbose, "Record Qos attempt; ping=%d ms", PingInMs);
			...
			UE_LOGF(LogQos, VeryVerbose, "Got %d/%d ping results (%d reachable) for datacenter (%ls, %ls); status=%d",
		...
		{
			UE_LOGF(LogQos, Verbose, "could not find matching datacenter for result (%ls) from %ls:%d!", LexToString(Result.Status), *Target.Address, Target.Port);
		}
	}
	...
	{
		UE_LOGF(LogQos, Verbose, "Qos complete in %0.2f s  (all regions: %ls)",
	...
	{
		UE_LOGF(LogQos, Verbose, "Qos failed after in %0.2f s", FPlatformTime::Seconds() - StartTimestamp);
	...
	{
		UE_LOGF(LogQos, Verbose, "Qos canceled after %0.2f s", FPlatformTime::Seconds() - StartTimestamp);
	...
	UE_LOGF(LogQos, Verbose, "UQosEvaluator OnEchoManyCompleted; Qos result=%d", int(CompletionResult));
}
	...
	const TArray<FDatacenterQosInstance>& Datacenters, int32 NumTestsPerRegion)
{
	for (const FDatacenterQosInstance& Datacenter : Datacenters)
	{
		...
		{
			UE_LOGF(LogQos, Verbose, "Datacenter disabled %ls", *Datacenter.Definition.ToDebugString());
		}
	}
