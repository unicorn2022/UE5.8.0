		...
		for (const FPlayTimeLimitUserPtr& User : Users)
		{
			...
			if (User->HasTimeLimit())
			{
				...
				if (bRewardRateChanged)
				{
					...
					UE_LOGF(LogPlayTimeLimit, Log, "FPlayTimeLimitImpl: User [%ls] RewardRate changed from %0.2f to %0.2f", *User->GetUserId()->ToDebugString(), LastKnownRewardRate, RewardRate);
...
void FPlayTimeLimitImpl::RegisterUser(const FUniqueNetId& UserId)
{
	if (!Users.ContainsByPredicate([&UserId](const FPlayTimeLimitUserPtr& User) { return *User->GetUserId() == UserId; }))
	{
		if (OnRequestCreateUser.IsBound())
		{
			...
			{
				UE_LOGF(LogPlayTimeLimit, Warning, "FPlayTimeLimitImpl: OnRequestCreateUser Delegate returned a null User.");
			}
		}
		else
		{
			UE_LOGF(LogPlayTimeLimit, Warning, "FPlayTimeLimitImpl: No OnRequestCreateUser delegate bound.");
		}
	}
	else
	{
		UE_LOGF(LogPlayTimeLimit, Log, "FPlayTimeLimitImpl: User [%ls] already registered", *UserId.ToDebugString());
	}
}
...
void FPlayTimeLimitImpl::UnregisterUser(const FUniqueNetId& UserId)
{
	...
	{
		UE_LOGF(LogPlayTimeLimit, Log, "FPlayTimeLimitImpl: User [%ls] not registered", *UserId.ToDebugString());
	}
}
...
void FPlayTimeLimitImpl::MockUser(const FUniqueNetId& UserId, const bool bHasTimeLimit, const double CurrentPlayTimeMinutes)
{
	...
	if (ExistingIndex != INDEX_NONE)
	{
		...
		UE_LOGF(LogPlayTimeLimit, Log, "MockUser: UserId=%ls, bHasTimeLimit=%ls, CurrentPlayTimeMinutes=%d, SecondsToNextNotification=%d", *UserId.ToDebugString(), bHasTimeLimit ? TEXT("true") : TEXT("false"), static_cast<int32>(CurrentPlayTimeMinutes), SecondsToNextNotification);
	}
#endif
}
...
{
	UE_LOGF(LogPlayTimeLimit, Display, "FPlayTimeLimitImpl::DumpState: Begin");
	if (Users.Num() != 0)
	{
		const double Now = FPlatformTime::Seconds();
		for (const FPlayTimeLimitUserPtr& User : Users)
		{
			...
			UE_LOGF(LogPlayTimeLimit, Display, "  User [%ls]", *User->GetUserId()->ToDebugString());
			UE_LOGF(LogPlayTimeLimit, Display, "    HasTimeLimit: [%ls]", User->HasTimeLimit() ? TEXT("true") : TEXT("false"));
			UE_LOGF(LogPlayTimeLimit, Display, "    NextNotificationTime: [%ls]", *NextNotificationTimeString);
			UE_LOGF(LogPlayTimeLimit, Display, "    LastKnownRewardRate: %0.2f", User->GetLastKnownRewardRate());
			UE_LOGF(LogPlayTimeLimit, Display, "    RewardRate: %0.2f", User->GetRewardRate());
			UE_LOGF(LogPlayTimeLimit, Display, "    PlayTimeMinutes: %d", User->GetPlayTimeMinutes());
		}
	}
	else
	{
		UE_LOGF(LogPlayTimeLimit, Display, "No users");
	}
	UE_LOGF(LogPlayTimeLimit, Display, "FPlayTimeLimitImpl::DumpState: End");
}
...
bool FPlayTimeLimitImpl::HasTimeLimit(const FUniqueNetId& UserId)
{
	...
	{
		UE_LOGF(LogPlayTimeLimit, Warning, "HasTimeLimit: UserId [%ls] is not registered", *UserId.ToDebugString());
	}
...
int32 FPlayTimeLimitImpl::GetPlayTimeMinutes(const FUniqueNetId& UserId)
{
	...
	{
		UE_LOGF(LogPlayTimeLimit, Warning, "GetPlayTimeMinutes: UserId [%ls] is not registered", *UserId.ToDebugString());
	}
...
float FPlayTimeLimitImpl::GetRewardRate(const FUniqueNetId& UserId)
{
	...
	{
		UE_LOGF(LogPlayTimeLimit, Warning, "GetRewardRate: UserId [%ls] is not registered", *UserId.ToDebugString());
	}
	if (RewardRate > 1.0f || RewardRate < 0.0f)
	{
		...
		if (!bWarned)
		{
			...
			UE_LOGF(LogPlayTimeLimit, Warning, "GetRewardRate: Received RewardRate=%0.2f (Expected range: [0.0, 1.0]). PlayTimeMinutes=%d. Clamping to the expected range.", RewardRate, PlayTimeMinutes);
			bWarned = true;
		}
