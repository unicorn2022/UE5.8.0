...
bool FPlayTimeLimitModule::Exec_Runtime(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FParse::Command(&Cmd, TEXT("PlayTimeLimit")))
	{
			...
			else if (FParse::Command(&Cmd, TEXT("SETUSERLIMITS")))
			{
				...
				if (OnlineSubsystem)
				{
					...
					if (IdentityInt.IsValid())
					{
						...
						{
							UE_LOGF(LogPlayTimeLimit, Warning, "SETUSERLIMITS: Could not get player id from user num=%d, ensure you are logged in first", LocalUserNum);
						}
					}
					else
					{
						UE_LOGF(LogPlayTimeLimit, Warning, "SETUSERLIMITS: Missing Identity interface");
					}
				}
				else
				{
					UE_LOGF(LogPlayTimeLimit, Warning, "SETUSERLIMITS: Missing OnlineSubsystem");
				}
			}
			else
			{
		...
		{
			UE_LOGF(LogPlayTimeLimit, Warning, "PlayTimeLimit is not enabled by config file");
		}
	}
	return bWasHandled;
}
