...
UUpdateManager::EUpdateStartResult UUpdateManager::StartUpdateCheckInternal(const FUpdateContextDefinition& ContextDefinition)
{
	...
	UE_LOGF(LogHotfixManager, Log, "Starting update check using context \"%ls\"", *ContextDefinition.Name);

	if (!ChecksEnabled() || !ContextDefinition.bEnabled)
	{
		UE_LOGF(LogHotfixManager, Display, "Update checks disabled!");
		...
		CurrentUpdateState == EUpdateState::UpdateComplete))
	{
		...
		if (bForceCheck || TimeSinceCheck >= CacheTimer || UOnlineHotfixManager::bAlwaysReapplyHotfixes)
		{
			if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
			{
				StrongStatsCollector->OnCheckStarted({
		...
		else
		{
			...
			UE_LOGF(LogHotfixManager, Display, "Returning cached update result %d. Cache timeout in %.2f/%.2f seconds.", (int32)LastResult, CacheTimer - TimeSinceCheck, CacheTimer);
	...
	{
		UE_LOGF(LogHotfixManager, Display, "Update already in progress");
	}
...
{
	UE_LOGF(LogHotfixManager, Display, "CheckComplete %ls", UpdateCompletionEnum ? *UpdateCompletionEnum->GetNameStringByValue((int64)Result) : TEXT("Invalid"));
	...
	if (DbgVal >= 0 && DbgVal <= (int32)EUpdateCompletionStatus::UpdateFailure_NotLoggedIn)
	{
		...
		UE_LOGF(LogHotfixManager, Display, "CheckComplete OVERRIDE! %ls", UpdateCompletionEnum ? *UpdateCompletionEnum->GetNameStringByValue((int64)Result) : TEXT("Invalid"));
	}
#endif
	...
	{
		UE_LOGF(LogHotfixManager, Display, "External CheckComplete %ls", UpdateCompletionEnum ? *UpdateCompletionEnum->GetNameStringByValue((int64)Result) : TEXT("Invalid"));
		if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
		{
...
void UUpdateManager::StartHotfixCheck()
{
	...
	else
	{
		...
		else
		{
			if (HotfixManager)
			{
				HotfixManager->OnHotfixProcessSkipped();
			}
...
{
	UE_LOGF(LogHotfixManager, VeryVerbose, "OnHotfixProcessedFile %ls", *FriendlyName);
...
{
	UE_LOGF(LogHotfixManager, Display, "OnHotfixCheckComplete %d", (int32)Result);
...
{
	UE_LOGF(LogHotfixManager, Display, "HotfixAvailabilityCheckComplete %d", (int32)Result);

	auto CompletionDelegate = [this, Result]()
	{
		UE_LOGF(LogHotfixManager, Display, "External HotfixAvailabilityCheckComplete %d", (int32)Result);
		if (TSharedPtr<IUpdateManagerStatsCollector> StrongStatsCollector = StatsCollector.Pin())
		{
			StrongStatsCollector->OnCheckStep_HotfixCheckComplete(Result);
		}
...
void UUpdateManager::SetUpdateState(EUpdateState NewState)
{
	...
	{
		FString OldStateStr = UpdateStateEnum ? *UpdateStateEnum->GetNameStringByValue((int64)CurrentUpdateState) : TEXT("Invalid");
		FString NewStateStr = UpdateStateEnum ? *UpdateStateEnum->GetNameStringByValue((int64)NewState) : TEXT("Invalid");

#if CSV_PROFILER_STATS || MISCTRACE_ENABLED
		// Add timing regions
		if (CurrentUpdateState != EUpdateState::UpdateIdle)
		{
			CSV_TRACE_REGION_END(*OldStateStr, TEXT("Hotfix"))
		}
		if (NewState != EUpdateState::UpdateIdle && NewState != EUpdateState::UpdateComplete)
		{
			CSV_TRACE_REGION_BEGIN(*NewStateStr, TEXT("Hotfix"))
		}
#endif

		UE_LOGF(LogHotfixManager, Display, "Update State %ls -> %ls", *OldStateStr, *NewStateStr);
...
bool UUpdateManager::Tick(float InDeltaTime)
{
	...
	if (CurrentUpdateState == EUpdateState::WaitingOnInitialLoad)
	{
		...
		//UE_LOGF(LogInit, Log, "Load progress %f (%d %d)", GetLoadProgress(), GetNumAsyncPackages(), WorstNumFilesPendingLoadViewed);

		if (!IsAsyncLoading())
		{
			const double LoadTime = FPlatformTime::Seconds() - LoadStartTime;
			UE_LOGF(LogHotfixManager, Log, "Finished initial load/hotfix phase in %fs", LoadTime);
...
const FUpdateContextDefinition& UUpdateManager::GetContextDefinition(const FString& ContextName) const
{
	...
	UE_LOGF(LogHotfixManager, Warning, "Failed to find update context \"%ls\". Using \"Unknown\" definition.", *ContextName);
