...
/** The prefix used when there no custom config so we don't try to match empty strings */
#define HOTFIX_NO_CUSTOM_CONFIG TEXT("##NOCONFIG##")

// do we support loading local HF files?
#define UE_TITLE_FILE_DEV_LOCAL_FILES_ENABLED !UE_BUILD_SHIPPING
...
/* static */ bool UOnlineHotfixManager::bAlwaysReapplyHotfixes = false;
static FAutoConsoleVariableRef CVarAlwaysReapplyHotfixes(
	TEXT("hotfix.AlwaysReapply"),
	UOnlineHotfixManager::bAlwaysReapplyHotfixes,
	TEXT("Whether hotfixes should be downloaded and reapplied on every check, for debugging"));
...
namespace OnlineHotfixManagerCVars
{
	...
	static bool bUseHotfixOnLoad = false;
	static FAutoConsoleVariableRef UseHotfixOnLoadCVar(
		TEXT("hotfix.UseHotfixOnLoad"),
		bUseHotfixOnLoad,
		TEXT("If true, hotfix assets as they are loaded instead of force-loading all hotfixable assets and applying hotfixes eagerly"),
		ECVF_Default);

	static bool bHotfixOnLoadIncludesOnDiskPluginInis = true;
	static FAutoConsoleVariableRef HotfixOnLoadIncludesOnDiskPluginInisCVar(
		TEXT("hotfix.HotfixOnLoadIncludesOnDiskPluginInis"),
		bHotfixOnLoadIncludesOnDiskPluginInis,
		TEXT("If true, when the hotfix on load system is active it will also load and apply syntax from on-disk plugin inis"),
		ECVF_Default);
}
...
void UOnlineHotfixManager::BroadcastTableChanges(const TSet<UDataTable*>& ChangedDataTables, const TSet<UCurveTable*>& ChangedCurveTables)
{
	...
	ChangedOrRemovedPakCount(0)
{
	...
	if (this != GetClass()->GetDefaultObject())
	{
		...
		UE::DynamicConfig::RemovePluginHotfixesFromDynamicBranch.AddUObject(this, &UOnlineHotfixManager::RemoveHotfixesFromDynamicBranch);
	}
}
...
UClass* UOnlineHotfixManager::GetConfiguredHotfixManagerClass()
{
	...
	if (HotfixManagerClass == nullptr)
	{
		...
		{
			UE_LOGF(LogHotfixManager, Display, "Unable to locate hotfix manager class '%ls'. Trying '%ls' instead.", *HotfixManagerClassName, *FallbackClassName)
		...
		if (HotfixManagerClass == nullptr)
		{
			UE_LOGF(LogHotfixManager, Warning, "Failed to find requested hotfix manager class '%ls'. Falling back to default.", *HotfixManagerClassName);
			// Just use the default class if it couldn't load what was specified
			HotfixManagerClass = UOnlineHotfixManager::StaticClass();
		}
	}

	return HotfixManagerClass;
}
...
UOnlineHotfixManager* UOnlineHotfixManager::Get(UWorld* World)
{
	...
	if (OnlineSub != nullptr)
	{
		...
		{
			UClass* HotfixManagerClass = GetConfiguredHotfixManagerClass();
...
void UOnlineHotfixManager::PostInitProperties()
{
	...
	CustomConfigPrefix = FConfigCacheIni::GetCustomConfigString().IsEmpty() ? HOTFIX_NO_CUSTOM_CONFIG : (DebugPrefix + FConfigCacheIni::GetCustomConfigString());
	CustomConfigPrefix += HOTFIX_SEPARATOR;

	// Build the default prefix too
	DefaultPrefix = DebugPrefix + TEXT("Default");

#if UE_TITLE_FILE_DEV_LOCAL_FILES_ENABLED
	// if we are going to load from local hotfix files, use the OnlineTitleFileNull class, which only loads from local files
	FString TitleFileLocalPath, HotfixLocalPath;
	if (FParse::Value(FCommandLine::Get(), TEXT("TitleFileLocalPath="), TitleFileLocalPath) || FParse::Value(FCommandLine::Get(), TEXT("HotfixLocalPath="), HotfixLocalPath))
	{
		// if HotfixLocalPath is specified, check that it either matches an existing TitleFileLocalPath, or if it doesn't exist, then alias it to 
		// TitleFileLocalPath so that the OnlineTitleFileNull code can use the proper option
		if (!HotfixLocalPath.IsEmpty())
		{
			if (TitleFileLocalPath.IsEmpty())
			{
				FString LocalCommandLine = FString::Printf(TEXT(" TitleFileLocalPath=\"%s\""), *HotfixLocalPath);
				FCommandLine::Append(*LocalCommandLine);
			}
			else
			{
				ensure(HotfixLocalPath == TitleFileLocalPath);
			}
		}
...
void UOnlineHotfixManager::BeginDestroy()
{
	TeardownHotfixOnLoad();
	Super::BeginDestroy();
}

void UOnlineHotfixManager::ConfigureHotfixOnLoad()
{
	// Evaluate the hotfix-on-load CVar once. This cannot run in PostInitProperties
	// because config files have not been downloaded yet. Latched so that mid-session
	// CVar changes do not arm the delegate against an empty asset registry.
	if (bConfiguredHotfixOnLoad)
	{
		return;
	}
	bConfiguredHotfixOnLoad = true;

	if (!OnlineHotfixManagerCVars::bUseHotfixOnLoad)
	{
		return;
	}

	for (const FSoftClassPath& HotfixableClass : HotfixableAssetClasses)
	{
		if (UClass* ResolvedClass = HotfixableClass.TryLoadClass<UObject>())
		{
			ResolvedHotfixableAssetClasses.Add(ResolvedClass);
			FCoreUObjectDelegates::RegisterHotfixableAssetClass(ResolvedClass);
		}
		else
		{
			UE_LOGF(LogHotfixManager, Warning, "Unable to locate hotfixable asset class '%ls'", *HotfixableClass.ToString());
		}
	}
	OnHotfixableAssetLoadedHandle = FCoreUObjectDelegates::OnHotfixableAssetLoaded.AddUObject(this, &UOnlineHotfixManager::OnHotfixableAssetLoaded);
	bHotfixOnLoadEnabled = true;
}

void UOnlineHotfixManager::OnHotfixProcessSkipped()
{
	ConfigureHotfixOnLoad();
}

void UOnlineHotfixManager::TeardownHotfixOnLoad()
{
	if (!bHotfixOnLoadEnabled)
	{
		return;
	}

	FCoreUObjectDelegates::OnHotfixableAssetLoaded.Remove(OnHotfixableAssetLoadedHandle);
	for (UClass* ResolvedClass : ResolvedHotfixableAssetClasses)
	{
		FCoreUObjectDelegates::UnregisterHotfixableAssetClass(ResolvedClass);
	}
	ResolvedHotfixableAssetClasses.Empty();
	bHotfixOnLoadEnabled = false;
}
...
void UOnlineHotfixManager::OnHotfixableAssetLoaded(UObject* Asset)
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_UOnlineHotfixManager_OnHotfixableAssetLoaded);

	ensure(IsInGameThread());

	// Early out if we don't have any hotfixes
	if (!AssetHotfixRegistry.HasPendingHotfixes())
	{
		return;
	}
	...
	// Apply all hotfixes to this asset
	bool bEncounteredErrors = false;
	if (AssetHotfixRegistry.TryApplyHotfixes(Asset, this, bEncounteredErrors, &ChangedDataTables, &ChangedCurveTables))
	{
		UE_LOGF(LogHotfixManager, Log, "Hotfixed '%ls'", *Asset->GetFullName());
		BroadcastTableChanges(ChangedDataTables, ChangedCurveTables);
	}
}

void UOnlineHotfixManager::StartHotfixProcess()
{
	UE_LOGF(LogHotfixManager, Log, "Starting Hotfix Process");

	// Patching the editor this way seems like a bad idea
	const bool bShouldHotfix = ShouldPerformHotfix();
	if (!bShouldHotfix)
	{
		UE_LOGF(LogHotfixManager, Warning, "Hotfixing skipped when not running game/server");
	...
	{
		UE_LOGF(LogHotfixManager, Warning, "Hotfixing already in progress");
		return;
	}
	...
	{
		UE_LOGF(LogHotfixManager, Error, "Failed to start the hotfixing process due to no OnlineTitleInterface present for OSS(%ls)", *OSSName);
		FinalizeHotfixProcess(EHotfixResult::Failed);
	}
}
...
struct FHotfixFileSortPredicate
{
	struct FHotfixFileNameSortPredicate
	{
		...
		const FString CustomConfigPrefix;

		FHotfixFileNameSortPredicate(const FString& InPlatformPrefix, const FString& InServerPrefix, const FString& InDefaultPrefix, const FString& InCustomConfigPrefix) :
			PlatformPrefix(InPlatformPrefix),
			ServerPrefix(InServerPrefix),
			DefaultPrefix(InDefaultPrefix),
			CustomConfigPrefix(InCustomConfigPrefix)
		{
		}
		...
		int32 GetPriorityForCompare(const FString& InHotfixName) const
		{
			...
			if (InHotfixName.EndsWith(TEXT("INI")))
			{
				...
				// CustomConfig trumps platform
				else if (HotfixName.StartsWith(CustomConfigPrefix))
				{
					Priority = 40;
				}
	...
	FHotfixFileSortPredicate(const FString& InPlatformPrefix, const FString& InServerPrefix, const FString& InDefaultPrefix, const FString& InCustomConfigPrefix) :
		FileNameSorter(InPlatformPrefix, InServerPrefix, InDefaultPrefix, InCustomConfigPrefix)
	{
	}
...
{
	UE_LOGF(LogHotfixManager, Log, "EnumerateFiles Http Request Complete");
	...
	if (bWasSuccessful)
	{
		...
		ChangedHotfixFileList.Sort<FHotfixFileSortPredicate>(FHotfixFileSortPredicate(PlatformPrefix, ServerPrefix, DefaultPrefix, CustomConfigPrefix));
		...
		else
		{
			...
			{
				UE_LOGF(LogHotfixManager, Display, "Files have been removed since last check. Reverting.");
			...
			{
				UE_LOGF(LogHotfixManager, Display, "Returned hotfix data is the same as last application, skipping the apply phase");
				FinalizeHotfixProcess(EHotfixResult::SuccessNoChange);
			}
		}
	}
	...
	{
		UE_LOGF(LogHotfixManager, Warning, "Enumeration of hotfix files failed");
		FinalizeHotfixProcess(EHotfixResult::Failed);
	}
}
...
void UOnlineHotfixManager::CheckAvailability(FOnHotfixAvailableComplete& InCompletionDelegate)
{
	...
	if (!bShouldHotfix)
	{
		UE_LOGF(LogHotfixManager, Warning, "Hotfixing availability skipped when not running game/server");
	...
	if (bHotfixingInProgress)
	{
		UE_LOGF(LogHotfixManager, Warning, "Hotfixing availability skipped because hotfix in progress");
	...
	{
		UE_LOGF(LogHotfixManager, Error, "Failed to start the hotfix check process due to no OnlineTitleInterface present for OSS(%ls)", *OSSName);
		FinalizeHotfixProcess(EHotfixResult::Failed);
	}
}
...
void UOnlineHotfixManager::OnEnumerateFilesForAvailabilityComplete(bool bWasSuccessful, const FString& ErrorStr, FOnHotfixAvailableComplete InCompletionDelegate)
{
	...
	if (bWasSuccessful)
	{
		...
		{
			UE_LOGF(LogHotfixManager, Display, "Hotfix files available");
			Result = EHotfixResult::Success;
		}
		else
		{
			UE_LOGF(LogHotfixManager, Display, "Returned hotfix data is the same as last application, returning nothing to do");
...
void UOnlineHotfixManager::BuildHotfixFileListDeltas()
{
	...
	for (const FCloudFileHeader& CurrentHeader : HotfixFileList)
	{
		...
		if (!bFoundMatch || bAlwaysReapplyHotfixes)
		{
	...
	UE_LOGF(LogHotfixManager, Log, "Hotfix deltas: updated:[%ls], removed:[%ls]",
...
void UOnlineHotfixManager::ReadHotfixFiles()
{
	if (ChangedHotfixFileList.Num())
	{
		...
		for (const FCloudFileHeader& FileHeader : ChangedHotfixFileList)
		{
			UE_LOGF(LogHotfixManager, VeryVerbose, "HF: %ls %ls %d ", *FileHeader.DLName, *FileHeader.FileName, FileHeader.FileSize);
	...
	{
		UE_LOGF(LogHotfixManager, Display, "No hotfix files need to be downloaded");
		FinalizeHotfixProcess(EHotfixResult::Success);
	}
}
...
void UOnlineHotfixManager::OnReadFileComplete(bool bWasSuccessful, const FString& FileName)
{
	if (PendingHotfixFiles.Contains(FileName))
	{
		if (bWasSuccessful)
		{
			...
			UE_LOGF(LogHotfixManager, Log, "Hotfix file (%ls) downloaded. Size was (%d)", *GetFriendlyNameFromDLName(FileName), Header->FileSize);
				...
				AsyncFlushContext->Flush(
					FOnAsyncLoadingFlushComplete::CreateWeakLambda(
						...
						[this]()
						{
							...
							FinalizeHotfixProcess(Result);
						}));
			}
		}
		else
		{
			UE_LOGF(LogHotfixManager, Error, "Hotfix file (%ls) failed to download", *GetFriendlyNameFromDLName(FileName));
			FinalizeHotfixProcess(EHotfixResult::Failed);
		}
	}
}
...
EHotfixResult UOnlineHotfixManager::ApplyHotfix()
{
	...
	{
		UE_LOGF(LogHotfixManager, Log, "[DynamicHotfixContents] Removing ini data for all pending hotfixes");
	...
	for (const FCloudFileHeader& FileHeader : RemovedHotfixFileList)
	{
		...
		if (!bEmptyDynamicHotfixContentsOnNewHotfix)
		{
			...
			UE_LOGF(LogHotfixManager, Log, "[DynamicHotfixContents] Removing ini data for pending hotfix %ls (%ls)", *FileHeader.FileName, *HotfixTag);
	...
	for (const FCloudFileHeader& FileHeader : ChangedHotfixFileList)
	{
		...
		{
			UE_LOGF(LogHotfixManager, Error, "Couldn't apply hotfix file (%ls)", *FileHeader.FileName);
	...
	UE_LOGF(LogHotfixManager, Display, "Hotfix data has been successfully applied");
	EHotfixResult Result = EHotfixResult::Success;
	if (ChangedOrRemovedPakCount > 0)
	{
		UE_LOGF(LogHotfixManager, Display, "Hotfix has changed or removed PAK files so a relaunch of the app is needed");
		Result = EHotfixResult::SuccessNeedsRelaunch;
	}
	else if (bHotfixNeedsMapReload)
	{
		UE_LOGF(LogHotfixManager, Display, "Hotfix has detected PAK files containing currently loaded maps, so a level load is needed");
...
void UOnlineHotfixManager::ApplyLocalTestHotfix(FString Filename)
{
	...
	if (FFileHelper::LoadFileToString(IniData, *Filename))
	{
		...
		{
			UE_LOGF(LogHotfixManager, Display, "Successfully applied test hotfix file %ls", *Filename);
		}
		else
		{
			UE_LOGF(LogHotfixManager, Display, "Failed to apply test hotfix file %ls", *Filename);
		}
	}
	else
	{
		UE_LOGF(LogHotfixManager, Warning, "Unable to read test hotfix file '%ls'", *Filename);
	}
}
#endif

void UOnlineHotfixManager::FinalizeHotfixProcess(EHotfixResult HotfixResult)
{
#if !UE_BUILD_SHIPPING
	...
	{
		if (IsHotfixOnLoadEnabled())
		{
			// Always parse asset hotfixes, even if we received no actual hotfix data, so that baked and command line
			// hotfixes for plugin-based assets get registered and applied via the on-load delegate when those assets load.
			ParseAssetHotfixes();
#if !UE_BUILD_SHIPPING
			ValidateHotfixableAssetClasses(AssetHotfixRegistry.GetAllPendingPaths());
#endif
			// Unconditonally apply hotfixes the first time we get a non-failure response
			// Afterwards, only apply hotfixes if the data actually changed
			if (!bHasAppliedHotfixesToLoadedAssets || HotfixResult != EHotfixResult::SuccessNoChange)
			{
				ApplyHotfixesToLoadedAssets();
				bHasAppliedHotfixesToLoadedAssets = true;
			}
		}
		else if (HotfixResult != EHotfixResult::SuccessNoChange)
		{
	...
	FGenericPlatformMisc::TickHotfixables();
...
bool UOnlineHotfixManager::WantsHotfixProcessing(const FCloudFileHeader& FileHeader)
{
	...
	if (Extension == TEXT("INI"))
	{
		...
		if (IsCompatibleHotfixFile(FileHeader.FileName, CloudFilename))
		{
			...
			bool bWantsCustomConfigPrefix = CloudFilename.StartsWith(CustomConfigPrefix);

			if (bWantsPlatformHotfix)
			{
				UE_LOGF(LogHotfixManager, Log, "Using platform hotfix %ls", *FileHeader.FileName);
			}
			else if (bWantsServerHotfix)
			{
				UE_LOGF(LogHotfixManager, Log, "Using server hotfix %ls", *FileHeader.FileName);
			}
			else if (bWantsDefaultHotfix)
			{
				UE_LOGF(LogHotfixManager, Log, "Using default hotfix %ls", *FileHeader.FileName);
			}
			else if (bWantsCustomConfigPrefix)
			{
				UE_LOGF(LogHotfixManager, Log, "Using customconfig hotfix %ls", *FileHeader.FileName);
			}

			return bWantsPlatformHotfix || bWantsServerHotfix || bWantsDefaultHotfix || bWantsCustomConfigPrefix;
		}
		else
		{
			UE_LOGF(LogHotfixManager, Verbose, "File not compatible %ls, skipping.", *FileHeader.FileName);
			return false;
		}
	}
	...
	else
	{
		...
		if (OnlineTitleFile->GetFileContents(FileHeader.DLName, FileData))
		{
			if (PreProcessDownloadedFileData(FileHeader, FileData))
			{
				...
				if (Extension == TEXT("INI"))
				{
					UE_LOGF(LogHotfixManager, Log, "Applying hotfix %ls", *FileHeader.FileName);
			...
			{
				UE_LOGF(LogHotfixManager, Warning, "Failed to process contents of %ls", *FileHeader.FileName);
			}
		}
		else
		{
			UE_LOGF(LogHotfixManager, Warning, "Failed to get contents of %ls", *FileHeader.FileName);
		}
	}
...
FString UOnlineHotfixManager::GetStrippedConfigFileName(const FString& IniName)
{
	...
	else if (StrippedIniName.StartsWith(CustomConfigPrefix))
	{
		StrippedIniName = IniName.Right(StrippedIniName.Len() - CustomConfigPrefix.Len());
	}
...
FConfigBranch* UOnlineHotfixManager::GetBranch(const FString& IniName)
{
	...
	// Find the branch by basename or full filename - simple case
	FConfigBranch* Branch = GConfig->FindBranch(*StrippedIniNameNoExtension, StrippedIniNameNoExtension);

	// If we can't find anything, then we'll look for a branch matching the suffix that has an active plugin tag matching the name.
	if (!Branch)
	{
		const auto FindActivePluginBranch = [&StrippedIniNameNoExtension, &Branch](const FString& BranchName)
		{
			if (!Branch && StrippedIniNameNoExtension.EndsWith(BranchName))
			{
				const FString ActivePluginName = StrippedIniNameNoExtension.LeftChop(BranchName.Len()); \
				Branch = GConfig->FindBranchWithDynamicLayerTag(FName(*BranchName), FName(*ActivePluginName));
			}
		};

#define FIND_BRANCH(BranchName) FindActivePluginBranch(#BranchName);
		ENUMERATE_KNOWN_INI_FILES(FIND_BRANCH);
#undef FIND_BRANCH
	}
...
bool UOnlineHotfixManager::HotfixIniFile(const FString& FileName, const FString& IniData)
{
	...
	if (bUseNewDynamicLayers)
	{
		...
		{
			// If this file is a known type of ini type, such as DefaultEngine.ini, then we'll find a branch here such as Engine.
			// This will also work for an active GFP that has applied its config.
			...
			// If not, then it's probably a GFP hotfix that will be applied whenever the plugin activates.
			else
			{
				const FString HotfixTag = GetStrippedConfigFileName(FileName).Replace(TEXT(".ini"), TEXT(""));

				UE_LOGF(LogHotfixManager, Log, "[DynamicHotfixContents] Storing ini data for pending hotfix %ls (%ls)", *FileName, *HotfixTag);
...
bool UOnlineHotfixManager::HotfixPakFile(const FCloudFileHeader& FileHeader)
{
	...
	{
		UE_LOGF(LogHotfixManager, Error, "PAK file (%ls) could not be mounted because MountPak is not bound", *FileHeader.FileName);
		return false;
	}
	...
	if (IPakFile* PakFile = FCoreDelegates::MountPak.Execute(PakLocation, 0))
	{
		...
		UE_LOGF(LogHotfixManager, Log, "Hotfix mounted PAK file (%ls)", *FileHeader.FileName);
		...
		IniList.Sort<FHotfixFileSortPredicate>(FHotfixFileSortPredicate(PlatformPrefix, ServerPrefix, DefaultPrefix, CustomConfigPrefix));
		...
		UE_LOGF(LogHotfixManager, Log, "Processing pak file (%ls) took %f seconds and resulted in (%d) INIs being reloaded",
			*FileHeader.FileName, FPlatformTime::Seconds() - StartTime, NumInisReloaded);
#if !UE_BUILD_SHIPPING
		if (bLogMountedPakContents)
		{
			UE_LOGF(LogHotfixManager, Log, "Files in pak file (%ls):", *FileHeader.FileName);
			for (const FString& FileName : Visitor.Files)
			{
				UE_LOGF(LogHotfixManager, Log, "\t\t%ls", *FileName);
			}
		}
#endif
		return true;
...
bool UOnlineHotfixManager::HotfixPakIniFile(const FString& FileName)
{
	...
	{
		UE_LOGF(LogHotfixManager, Log, "Hotfix failed to merge INI (%ls) found in a PAK file", *FileName);
		return false;
	}
	UE_LOGF(LogHotfixManager, Log, "Hotfix merged INI (%ls) found in a PAK file", *FileName);
	...
	UE_LOGF(LogHotfixManager, Log, "Updating config from %ls took %f seconds reloading %d objects",
	...
	for (const FCloudFileHeader& FileHeader : ChangedHotfixFileList)
	{
		for (int32 Index = 0; Index < MountedPakFiles.Num(); Index++)
		{
			if (MountedPakFiles[Index] == FileHeader.DLName)
			{
				...
				UE_LOGF(LogHotfixManager, Log, "Hotfix unmounted PAK file (%ls) so it can be redownloaded", *FileHeader.FileName);
				break;
			}
		}
	}
	...
	for (const FCloudFileHeader& FileHeader : RemovedHotfixFileList)
	{
		for (int32 Index = 0; Index < MountedPakFiles.Num(); Index++)
		{
			if (MountedPakFiles[Index] == FileHeader.DLName)
			{
				...
				MountedPakFiles.RemoveAt(Index);
				ChangedOrRemovedPakCount++;
				UE_LOGF(LogHotfixManager, Log, "Hotfix unmounted PAK file (%ls) since it was removed from the hotfix set", *FileHeader.FileName);
				break;
			}
		}
	}
...
void UOnlineHotfixManager::RestoreBackupIniFiles()
{
		...
		for (UClass* Class : RestoredClasses)
		{
			if (Class->HasAnyClassFlags(CLASS_Config))
			{
				...
				for (UObject* Object : Objects)
				{
					if (IsValid(Object))
					{
						UE_LOGF(LogHotfixManager, Verbose, "Restoring %ls", *Object->GetPathName());
						Object->ReloadConfig();
						NumObjectsReloaded++;
					}
				}
	...
	UE_LOGF(LogHotfixManager, Log, "Restoring config for %d changed classes took %f seconds reloading %d objects",
		ClassesToRestore.Num(), FPlatformTime::Seconds() - StartTime, NumObjectsReloaded);
}

void UOnlineHotfixManager::PatchAssetsFromIniFiles()
{
	UE_LOGF(LogHotfixManager, Display, "Checking for assets to be patched using data from 'AssetHotfix' section in the Game .ini file");
	...
	if (AssetHotfixConfigSection != nullptr)
	{
		...
		for (FConfigSection::TConstIterator It(*AssetHotfixConfigSection); It; ++It)
		{
				...
				if (!DataLine.IsEmpty())
				{
					...
					if (Tokens.Num() == 3 || Tokens.Num() == 5)
					{
						...
						if (!bAddAssetToHotfixedList)
						{
							...
							{
								UE_LOGF(LogHotfixManager, Error, "[Item: %d] %ls: %ls", TotalPatchableAssets, *GetPathNameSafe(Asset), *ProblemString);
							}
						}
						else
						{
			...
			{
				UE_LOGF(LogHotfixManager, Error, "[Item: %d] Invalid patchable asset type '%ls' - supported types: %ls", TotalPatchableAssets, *It.Key().ToString(), *PatchableAssetClassesStr);
			}
		}
	...
	{
		UE_LOGF(LogHotfixManager, Display, "Successfully patched all %i assets from the 'AssetHotfix' section in the Game .ini file. These assets will be forced to remain loaded.", AssetsHotfixedFromIniFiles.Num());
	}
	else
	{
		...
		UE_LOGF(LogHotfixManager, Warning, "Only %i of %i assets were successfully patched from 'AssetHotfix' section in the Game .ini file. The patched assets will be forced to remain loaded. Any assets that failed to patch may be left in an invalid state! Failure may occur due to the plugin not being installed and mounted.", AssetsHotfixedFromIniFiles.Num(), TotalPatchableAssets);
	}
}

void UOnlineHotfixManager::RegisterAssetHotfixEntry(FName AssetPath, FPendingAssetHotfix Hotfix)
{
	AssetHotfixRegistry.StoreHotfixEntry(AssetPath, MoveTemp(Hotfix));
}

void UOnlineHotfixManager::ApplyRegisteredHotfixesForAsset(UObject* Asset)
{
	TSet<UDataTable*> ChangedDataTables;
	TSet<UCurveTable*> ChangedCurveTables;
	bool bEncounteredErrors = false;
...
void UOnlineHotfixManager::ParseAssetHotfixes()
{
	// Clear base-ini entries (tagged NAME_None) from a previous hotfix cycle before
	// re-parsing. GFP-sourced entries (named tags) are managed separately via
	// RemoveHotfixesFromDynamicBranch and are intentionally left intact here.
	AssetHotfixRegistry.RemoveEntriesFromSource(NAME_None);

	// Everything should be under the 'AssetHotfix' section in Game.ini
	const FConfigSection* AssetHotfixConfigSection = GConfig->GetSection(TEXT("AssetHotfix"), false, GGameIni);
	if (AssetHotfixConfigSection != nullptr)
	{
		AssetHotfixRegistry.ParseAndStoreHotfixEntries(NAME_None, *AssetHotfixConfigSection);
	}
}

void UOnlineHotfixManager::ApplyHotfixesToLoadedAssets()
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_UOnlineHotfixManager_ApplyHotfixesToLoadedAssets);
	TArray<FName> AllPaths = AssetHotfixRegistry.GetAllPendingPaths();
	ApplyHotfixesToLoadedAssets(AllPaths);
}

void UOnlineHotfixManager::ApplyHotfixesToLoadedAssets(TConstArrayView<FName> AssetPaths)
{
	TSet<UDataTable*> ChangedDataTables;
	TSet<UCurveTable*> ChangedCurveTables;

	FString PathString;
	int32 NumHotfixesApplied = 0;
	int32 NumEncounteredError = 0;
	for (FName Path : AssetPaths)
	{
		Path.ToString(PathString);
		UObject* AssetInMemory = FindObject<UObject>(nullptr, PathString);

		// If we found a package then get the primary asset in the package
		// The [AssetHotfix] section uses package paths and not object paths
		if (AssetInMemory && AssetInMemory->IsA<UPackage>())
		{
			FString AssetName = FPackageName::GetShortName(PathString);
			AssetInMemory = FindObject<UObject>(AssetInMemory, *AssetName);
		}

		if (AssetInMemory)
		{
			bool bEncounteredErrors = false;
			if (AssetHotfixRegistry.TryApplyHotfixes(AssetInMemory, this, bEncounteredErrors, &ChangedDataTables, &ChangedCurveTables))
			{
				NumHotfixesApplied++;
				UE_LOGF(LogHotfixManager, Log, "Hotfixed '%ls'", *AssetInMemory->GetFullName());
			}

			if (bEncounteredErrors)
			{
				NumEncounteredError++;
			}
		}
	...
	else if (NumHotfixesApplied == AssetPaths.Num())
	{
		if (NumEncounteredError > 0)
		{
			UE_LOGF(LogHotfixManager, Warning, "Patched all %i of %i assets from 'AssetHotfix' section in the Game .ini file but encountered %i partial errors applying patches to asset. Possible culprits include bad syntax in plugin Game .ini files. Inspect log for patch errors for more details.", NumHotfixesApplied, AssetPaths.Num(), NumEncounteredError);
		}
		else
		{
			UE_LOGF(LogHotfixManager, Display, "Successfully patched all %i assets from the 'AssetHotfix' section in the Game .ini file.", AssetPaths.Num());
		}
	}
	else
	{
		UE_LOGF(LogHotfixManager, Warning, "Only %i of %i assets were successfully patched from 'AssetHotfix' section in the Game .ini file. Inspect log for patch errors for more details.", NumHotfixesApplied, AssetPaths.Num());
	}
...
#if !UE_BUILD_SHIPPING
void UOnlineHotfixManager::ValidateHotfixableAssetClasses(TConstArrayView<FName> AssetPaths)
{
	TMap<FName, FAssetData> PackageToAsset;
	UE::AssetRegistry::GetAssetForPackages(AssetPaths, PackageToAsset);

	for (FName AssetPath : AssetPaths)
	{
		const FAssetData* AssetData = PackageToAsset.Find(AssetPath);
		if (!AssetData)
		{
			UE_LOGF(LogHotfixManager, Warning,
				"Hotfix entry targets asset '%ls' which was not found in the asset registry.",
				*AssetPath.ToString());
			continue;
		}

		UClass* AssetClass = AssetData->GetClass();
		if (AssetClass && !FCoreUObjectDelegates::IsHotfixableAssetClass(AssetClass))
		{
			UE_LOGF(LogHotfixManager, Warning,
				"Hotfix entry targets '%ls' (class '%ls') which is not registered as hotfixable. "
				"Add it to HotfixableAssetClasses in [/Script/Hotfix.OnlineHotfixManager].",
				*AssetPath.ToString(), *AssetClass->GetName());
		}
	}
}
#endif
...
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_UOnlineHotfixManager_ReloadConfigsFromIniFiles);
		...
		for (UClass* Class : RestoredClasses)
		{
			if (Class->HasAnyClassFlags(CLASS_Config))
			{
				...
				for (UObject* Object : Objects)
				{
					if (IsValid(Object))
					{
						UE_LOGF(LogHotfixManager, Verbose, "Reloading %ls", *Object->GetPathName());
						Object->ReloadConfig();
						NumObjectsReloaded++;
					}
				}
	...
	UE_LOGF(LogHotfixManager, Log, "Reloading config for %d changed classes took %f seconds reloading %d objects",
			...
			if (DataTableRow)
			{
				...
				if (RowData)
				{
					...
					if (NumProp)
					{
						if (NewValue.IsNumeric())
						{
							...
							if (NumProp->IsInteger())
							{
								...
								OnHotfixTableValueInt64WithSource(*Asset, RowName, ColumnName, OldPropertyValue, NewPropertyValue, SourceTag);
							...
							else
							{
								...
								OnHotfixTableValueDoubleWithSource(*Asset, RowName, ColumnName, OldPropertyValue, NewPropertyValue, SourceTag);
								bWasDataTableChanged = true;
								UE_LOGF(LogHotfixManager, VeryVerbose, "Data table %ls row %ls updated column %ls from %.2f to %.2f.", *AssetPath, *RowName, *ColumnName, OldPropertyValue, NewPropertyValue);
							}
						}
						// Not a number.
						else
					...
					else if (StrProp)
					{
						...
						OnHotfixTableValueStringWithSource(*Asset, RowName, ColumnName, OldPropertyValue, NewPropertyValue, SourceTag);
						bWasDataTableChanged = true;
						UE_LOGF(LogHotfixManager, VeryVerbose, "Data table %ls row %ls updated column %ls from %ls to %ls.", *AssetPath, *RowName, *ColumnName, *OldPropertyValue, *NewPropertyValue);
					}
					// FName property
					else if (NameProp)
					{
						...
						OnHotfixTableValueNameWithSource(*Asset, RowName, ColumnName, OldPropertyValue, NewPropertyValue, SourceTag);
						bWasDataTableChanged = true;
						UE_LOGF(LogHotfixManager, VeryVerbose, "Data table %ls row %ls updated column %ls from %ls to %ls.", *AssetPath, *RowName, *ColumnName, *OldPropertyValue.ToString(), *NewPropertyValue.ToString());
					}
					...
					else if (SoftObjProp)
					{
						...
						OnHotfixTableValueSoftObjectWithSource(*Asset, RowName, ColumnName, OldPropertyValue, NewPropertyValue, SourceTag);
						bWasDataTableChanged = true;
						UE_LOGF(LogHotfixManager, VeryVerbose, "Data table %ls row %ls updated column %ls from %ls to %ls.", *AssetPath, *RowName, *ColumnName, *OldPropertyValue.ToString(), *NewPropertyValue.ToString());
					}
					else
					{
						...
						{
							bWasDataTableChanged = true;
							UE_LOGF(LogHotfixManager, VeryVerbose, "Data table %ls row %ls updated column %ls from %ls to %ls.", *AssetPath, *RowName, *ColumnName, *OldPropertyValue, *NewValue);
						}
						else
						{
		...
		if (ColumnName.IsNumeric())
		{
			...
			if (CurveTableRow)
			{
				...
				if (NewValue.IsNumeric())
				{
					...
					{
						OnHotfixTableValueFloatWithSource(*Asset, RowName, ColumnName, OldPropertyValue, NewPropertyValue, SourceTag);
						bWasCurveTableChanged = true;

						if (bWasExistingKey)
						{
							UE_LOGF(LogHotfixManager, VeryVerbose, "Curve table %ls row %ls updated column %ls from %.2f to %.2f.", *AssetPath, *RowName, *ColumnName, OldPropertyValue, NewPropertyValue);
						}
						else
						{
							UE_LOGF(LogHotfixManager, VeryVerbose, "Curve table %ls row %ls added column %ls with value %.2f.", *AssetPath, *RowName, *ColumnName, NewPropertyValue);
						}
					}
					else
					{
		...
		if (ColumnName.IsNumeric())
		{
			...
			if (CurveFloat->FloatCurve.IsKeyHandleValid(Key))
			{
				if (NewValue.IsNumeric())
				{
					...
					UE_LOGF(LogHotfixManager, VeryVerbose, "Curve float %ls updated column %ls from %.2f to %.2f.", *AssetPath, *ColumnName, OldPropertyValue, NewPropertyValue);
				}
				else
				{
					const FString Problem(FString::Printf(TEXT("The new value %s is not a number when it should be."), *NewValue));
...
void UOnlineHotfixManager::HotfixDynamicBranch(const FName& Tag, const FName& Branch, class FConfigModificationTracker* ModificationTracker)
{
	...
	UE_LOGF(LogHotfixManager, VeryVerbose, "[DynamicHotfixContents] HotfixDynamicBranch: Searching for hotfix for base name: %ls. Result: %ls", *HotfixBaseName, HotfixesForTag ? TEXT("found") : TEXT("not found"));

	TArray<FName> AffectedPaths;
	const bool bCheckForNewHotfixes = IsHotfixOnLoadEnabled() && Branch == NAME_Game;

	if (OnlineHotfixManagerCVars::bHotfixOnLoadIncludesOnDiskPluginInis && bCheckForNewHotfixes)
	{
		// Asset hotfixes in on-disk plugin ini files are not applied until the plugin is activated, which means that they are not picked up within ParseAssetHotfixes().
		// Instead, identify the hotfixes as the plugin activates and track them now
		if (FConfigBranch* BranchToHotfix = GConfig->FindBranch(Branch, FString()))
		{
			for (const FConfigCommandStream* Layer : BranchToHotfix->DynamicLayers)
			{
				if (Layer && Layer->Tag == Tag && !Layer->IsEmpty())
				{
					UE_LOGF(LogHotfixManager, VeryVerbose, "HotfixDynamicBranch: searching for [AssetHotfix] entries in local config for plugin %ls", *Tag.ToString());

					const static FString AssetHotfixString = FString(TEXT("AssetHotfix"));
					if (const FConfigCommandStreamSection* Section = Layer->Find(AssetHotfixString))
					{
						TArray<FName> LocalPaths = AssetHotfixRegistry.ParseAndStoreHotfixEntries(Tag, *Section);
						if (LocalPaths.Num() > 0)
						{
							UE_LOGF(LogHotfixManager, Log, "HotfixDynamicBranch: parsed %d [AssetHotfix] entries from local config for plugin %ls", LocalPaths.Num(), *Tag.ToString());
							AffectedPaths.Append(MoveTemp(LocalPaths));
						}
					}
				}
			}
	...
	if (HotfixesForTag)
	{
		...
		{
			UE_LOGF(LogHotfixManager, Log, "[DynamicHotfixContents] HotfixDynamicBranch: applying hotfix %ls for tag %ls on branch %ls", *HotfixFilenameAndContents.Key, *Tag.ToString(), *Branch.ToString());
			...
			{
				UE_LOGF(LogHotfixManager, Warning, "[DynamicHotfixContents] HotfixDynamicBranch: failed to apply hotfix");
			}

			// This specifically checks for 'Game' branch as [AssetHotfix] must be in Game ini files.
			if (bCheckForNewHotfixes)
			{
				AffectedPaths.Append(AssetHotfixRegistry.ParseAndStoreFromRawIniData(Tag, HotfixFilenameAndContents.Value));
			}
		}
	}
	...
	{
#if !UE_BUILD_SHIPPING
		ValidateHotfixableAssetClasses(AffectedPaths);
#endif
		ApplyHotfixesToLoadedAssets(AffectedPaths);
	}
}

void UOnlineHotfixManager::RemoveHotfixesFromDynamicBranch(const FName& Tag)
{
	AssetHotfixRegistry.RemoveEntriesFromSource(Tag);
	OnDynamicHotfixesRemoved(Tag);
}
...
void UOnlineHotfixManager::ReloadObjectsAffectedByConfigFile(const FString& IniDataFileName, const FString& IniData, const FString& ConfigFilename, TArray<FString>& ReloadedClassesPathNames, bool bUseLoadConfig)
{
		...
		if (StartIndex > -1)
		{
			...
			if (EndIndex > StartIndex)
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogHotfixManager, Warning, "Specified per-object class %ls was not found", *ClassName);
					}
				}
				StartIndex = EndIndex;
			}
	...
	{
		UE_LOGF(LogHotfixManager, Verbose, "Reloading %ls", *ReloadObject->GetPathName());
	...
	UE_LOGF(LogHotfixManager, Log, "Updating config from %ls took %f seconds and reloaded %d objects",
...
protected:
	virtual bool Exec_Runtime(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar) override
	{
		...
		else if (FParse::Command(&Cmd, TEXT("TESTHOTFIXSORT")))
		{
			...
			TestList.Add(Header);
			TestList.Sort<FHotfixFileSortPredicate>(FHotfixFileSortPredicate(TEXT("PS4_"), TEXT("DedicatedServer"), TEXT("Default"), HOTFIX_NO_CUSTOM_CONFIG));

			UE_LOGF(LogHotfixManager, Log, "Hotfixing sort is:");
			for (const FCloudFileHeader& FileHeader : TestList)
			{
				UE_LOGF(LogHotfixManager, Log, "\t%ls", *FileHeader.FileName);
			}
			...
			TestList2.Sort<FHotfixFileSortPredicate>(FHotfixFileSortPredicate(TEXT("PS4_"), TEXT("DedicatedServer"), TEXT("Default"), HOTFIX_NO_CUSTOM_CONFIG));

			UE_LOGF(LogHotfixManager, Log, "Hotfixing PAK INI file sort is:");
			for (const FString& IniName : TestList2)
			{
				UE_LOGF(LogHotfixManager, Log, "\t%ls", *IniName);
			}
			return true;
		}
		else if (FParse::Command(&Cmd, TEXT("LOGHOTFIXASSETLIST")))
		{
			if (UOnlineHotfixManager* HotfixManager = UOnlineHotfixManager::Get(InWorld))
			{
				...
				UE_LOGF(LogHotfixManager, Log, "Logging contetents of AssetsHotfixedFromIniFiles. Asset count: %d", AssetCount);
				for (const TObjectPtr<UObject>& ObjPtr : HotfixManager->AssetsHotfixedFromIniFiles)
				{
					++CurrentAsset;
					UE_LOGF(LogHotfixManager, Log, "\t[%d/%d]: %ls", CurrentAsset, AssetCount, *GetPathNameSafe(ObjPtr.Get()));
				}
				UE_LOGF(LogHotfixManager, Log, "Logging complete.");
			}
			return true;
		}
		return false;
