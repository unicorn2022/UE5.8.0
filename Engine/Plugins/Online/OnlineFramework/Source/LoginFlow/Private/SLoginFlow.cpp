...
private:
	...
	{
		UE_LOGF(LogLoginFlow, Log, "HandleBeforePopup %ls %ls", *Url, *Target);
	...
	{
		UE_LOGF(LogLoginFlow, Warning, "HandleLoadError");
	...
	{
		UE_LOGF(LogLoginFlow, Warning, "HandleOverlayBrowserLoadError");
	...
	{
		//UE_LOGF(LogLoginFlow, Log, "HandleLoadUrl %ls %ls", *Method, *Url);
		return false;
	}
	...
	void HandleBrowserUrlChanged(const FText& Url)
	{
		#if (0) // HandleBrowserBeforeBrowse seems to do all that is required atm
		{
			...
			UE_LOGF(LogLoginFlow, Log, "HandleBrowserUrlChanged Current: %ls New: %ls", *CurrentURL, *Url.ToString());
	...
	{
		UE_LOGF(LogLoginFlow, Log, "HandleBrowserCloseWindow");
	...
	{
		UE_LOGF(LogLoginFlow, Log, "HandleBrowserCreateWindow");
	...
	{
		UE_LOGF(LogLoginFlow, Log, "HandleBrowserBeforeBrowse Main:%d Redirect:%d URL: %ls", Request.bIsMainFrame, Request.bIsRedirect, *Url);
