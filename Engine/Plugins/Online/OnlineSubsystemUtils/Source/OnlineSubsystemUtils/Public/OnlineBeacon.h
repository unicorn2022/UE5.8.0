...
enum class EBeaconFailureReason : uint8
{
	Unknown = 0,
	TransportError = 1,   
	VersionMismatch = 2,
...
inline const TCHAR* LexToString(EBeaconFailureReason Value)
{
	switch (Value)
	{
	case EBeaconFailureReason::Unknown:
		return TEXT("Unknown");
	case EBeaconFailureReason::TransportError:
		return TEXT("TransportError");
	case EBeaconFailureReason::VersionMismatch:
		return TEXT("VersionMismatch");
	case EBeaconFailureReason::InitFailure:
		return TEXT("FailedToInitBeacon");
	case EBeaconFailureReason::EncryptionFailure:
		return TEXT("EncryptionFailure");
	case EBeaconFailureReason::ApplicationError:
		return TEXT("ApplicationError");
	case EBeaconFailureReason::TimeOut:
		return TEXT("TimeOut");
	default:
		return TEXT("Unknown");
	}
...
class AOnlineBeacon : public AActor, public FNetworkNotify
{
	...
	void PauseBeaconRequests(bool bPause)
	{
		...
		if (bPause)
		{
			UE_LOGF(LogBeacon, Verbose, "All Beacon Requests Paused.");
		...
		{
			UE_LOGF(LogBeacon, Verbose, "All Beacon Requests Resumed.");
...
protected:
	...
	UE_DEPRECATED(5.7, "Deprecated - Move to OnFailure(FBeaconFailureContext).")
	ONLINESUBSYSTEMUTILS_API virtual void OnFailure();

	ONLINESUBSYSTEMUTILS_API virtual void OnFailure(EBeaconFailureReason Reason, FStringView ErrorMessage);
