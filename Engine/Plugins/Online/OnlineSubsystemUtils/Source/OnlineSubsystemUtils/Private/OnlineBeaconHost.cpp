...
void AOnlineBeaconHost::OnNetCleanup(UNetConnection* Connection)
{
	...
	UE_LOGF(LogBeacon, Log, "%ls: Cleaning up in-progress connection due to closure.", *GetDebugName(Connection));
...
void AOnlineBeaconHost::OnConnectionClosed(UNetConnection* Connection)
{
	...
	if (ClientActor)
	{
		...
		UE_LOGF(LogBeacon, Verbose, "%ls: Connection closed. BeaconType: %ls, BeaconActor: %ls, HostObject: %ls",
	...
	if(URL.Valid)
	{
		if (InitBase() && NetDriver)
		{
			...
			else
			{
				...
				UE_LOGF(LogBeacon, Log, "%ls: AOnlineBeaconHost::InitHost failed", *GetName());
				OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_host_init_failure"));
			}
		}
	}
...
void AOnlineBeaconHost::NotifyControlMessage(UNetConnection* Connection, uint8 MessageType, class FInBunch& Bunch)
{
	if (NetDriver->ServerConnection == nullptr && Connection)
	{
		...
		UE_LOGF(LogBeacon, Verbose, "%ls: Host received: %ls", *GetDebugName(Connection), FNetControlMessageInfo::GetName(MessageType));
#endif
	...
	case NMT_Hello:
		{
			...
			UE_LOGF(LogBeacon, Log, "%ls: Beacon Hello", *GetDebugName(Connection));
			...
			if (!bIsCompatible)
			{
				...
				UE_LOGF(LogBeacon, Error, "Client not network compatible %ls: LocalNetVersion=%u, RemoteNetVersion=%u, LocalNetFeatures=%ls, RemoteNetFeatures=%ls", 
	...
	case NMT_Login:
		{
			...
			UE_LOGF(LogBeacon, Log, "%ls: Login request: %ls userId: %ls platform: %ls",
	...
	case NMT_Netspeed:
		{
			...
			UE_LOGF(LogBeacon, Log, "%ls: Client netspeed is %i", *GetDebugName(Connection), Connection->CurrentNetSpeed);
		}
		break;
	case NMT_BeaconJoin:
		{
			...
			if (bAuthRequired)
			{
				...
				UE_LOGF(LogBeacon, Log, "%ls: Beacon Join %ls %ls", *GetDebugName(Connection), *BeaconType, *UniqueId.ToDebugString());
			}
			else
			{
				UE_LOGF(LogBeacon, Log, "%ls: Beacon Join %ls %ls (unauthenticated)", *GetDebugName(Connection), *BeaconType, *UniqueId.ToDebugString());
			}
	...
	case NMT_BeaconNetGUIDAck:
		{
			...
			{
				UE_LOGF(LogBeacon, Verbose, "%ls: BeaconType: %ls, Delay for handshake completion: %.3f", *GetDebugName(Connection), *BeaconType, Delay);
			...
			{
				UE_LOGF(LogBeacon, Verbose, "%ls: BeaconType: %ls, Handshake will never complete, client will either timeout or hang if timeouts are disabled.", *GetDebugName(Connection), *BeaconType);
			}
		}
...
void AOnlineBeaconHost::FinishHandshake(UNetConnection* Connection, FString BeaconType)
{
	...
	UE_LOGF(LogBeacon, Log, "%ls: Handshake complete.", *GetDebugName(Connection));
...
void AOnlineBeaconHost::SendFailurePacket(UNetConnection* Connection, FNetCloseResult&& CloseReason, const FText& ErrorText)
{
	if (Connection != nullptr)
	{
		...
		UE_LOGF(LogBeacon, Log, "%ls: Send failure: %ls", ToCStr(GetDebugName(Connection)), ToCStr(ErrorMsg));
...
void AOnlineBeaconHost::CloseHandshakeConnection(UNetConnection* Connection)
{
	...
	{
		UE_LOGF(LogBeacon, Verbose, "%ls: Closing connection: %ls", *GetDebugName(Connection), *Connection->PlayerId.ToDebugString());
		OnConnectionClosed(Connection);
		Connection->OwningActor = nullptr; // don't notify us again
		Connection->FlushNet(true);
		Connection->Close();
		UE_LOGF(LogBeacon, Verbose, "--------------------------------");
	}
}
...
void AOnlineBeaconHost::DisconnectClient(AOnlineBeaconClient* ClientActor)
{
	if (IsValid(ClientActor) && ClientActor->GetConnectionState() != EBeaconConnectionState::Closed)
	{
		...
		UE_LOGF(LogBeacon, Log, "%ls: DisconnectClient for %ls. UNetDriver %ls State %d",
			*GetDebugName(Connection),
	...
	if (FConnectionState* ConnState = ConnectionState.Find(Connection))
	{
		if (ConnState->bHasSentLogin && !ConnState->bHasSentWelcome)
		{
			...
			if (Error.WasSuccessful())
			{
				...
				UE_LOGF(LogBeacon, Verbose, "%ls: OnAuthenticationVerificationComplete: Successfully authenticated.", *GetDebugName(Connection));
			...
			{
				// Auth failure.
				UE_LOGF(LogBeacon, Log, "%ls: OnAuthenticationVerificationComplete: Failed to authenticate error: %ls", *GetDebugName(Connection), *Error.ToLogString());
		...
		{
			UE_LOGF(LogBeacon, Warning, "%ls: OnAuthenticationVerificationComplete: Invalid beacon connection state for auth challenge result. error: %ls", *GetDebugName(Connection), *Error.ToLogString());
		}
	}
	else
	{
		UE_LOGF(LogBeacon, Warning, "%ls: OnAuthenticationVerificationComplete: Failed to find connection state. error: %ls", *GetDebugName(Connection), *Error.ToLogString());
	}
}
...
void AOnlineBeaconHost::OnHelloSequenceComplete(UNetConnection* Connection)
{
	if (Connection == nullptr)
	{
		UE_LOGF(LogBeacon, Log, "%ls: OnlineBeaconHost::OnHelloSequenceComplete: Connection is null.", *GetDebugName());
		return;
	}
	if (Connection->GetConnectionState() == USOCK_Invalid || Connection->GetConnectionState() == USOCK_Closed || Connection->Driver == nullptr)
	{
		UE_LOGF(LogBeacon, Log, "%ls: OnlineBeaconHost::OnHelloSequenceComplete: connection in invalid state. %ls", *GetDebugName(Connection), *Connection->Describe());
		return;
	}
	FConnectionState* ConnState = ConnectionState.Find(Connection);
	if (ConnState == nullptr)
	{
		UE_LOGF(LogBeacon, Log, "%ls: OnlineBeaconHost::OnHelloSequenceComplete: unable to find connection state.", *GetDebugName(Connection));
		return;
	}
	if (ConnState->bHasSentWelcome)
	{
		UE_LOGF(LogBeacon, Log, "%ls: OnlineBeaconHost::OnHelloSequenceComplete: sequence error, user already sent welcome.", *GetDebugName(Connection));
		return;
	}
	...
	if (bAuthRequired)
	{
		...
		UE_LOGF(LogBeacon, Verbose, "%ls: Sending challenge.", *GetDebugName(Connection));
	...
	else
	{
		...
		UE_LOGF(LogBeacon, Verbose, "%ls: Connection welcomed.", *GetDebugName(Connection));
	...
	if (Connection)
	{
		if (Connection->GetConnectionState() != USOCK_Invalid && Connection->GetConnectionState() != USOCK_Closed && Connection->Driver)
		{
			...
			else
			{
				...
				UE_LOGF(LogBeacon, Warning, "%ls: OnlineBeaconHost::OnEncryptionResponse: encryption failure [%ls] %ls", *GetDebugName(Connection), *ResponseStr, *Response.ErrorMsg);
		...
		{
			UE_LOGF(LogBeacon, Warning, "%ls: OnlineBeaconHost::OnEncryptionResponse: connection in invalid state. %ls", *GetDebugName(Connection), *Connection->Describe());
		}
	}
	else
	{
		UE_LOGF(LogBeacon, Warning, "%ls: OnlineBeaconHost::OnEncryptionResponse: Connection is null.", *GetDebugName());
	}
}
...
void AOnlineBeaconHost::RegisterHost(AOnlineBeaconHostObject* NewHostObject)
{
	...
	{
		UE_LOGF(LogBeacon, Warning, "%ls: Beacon host type %ls already exists", *GetName(), *BeaconType);
	}
}
