...
#if WITH_EDITOR
static bool HasAuthCommandLineStatic()
{
	FString Id, Token, Type;
	return FParse::Value(FCommandLine::Get(), TEXT("AUTH_LOGIN="), Id) &&
...
public:
	virtual bool SupportsOnlinePIE() const override
	{
		...
		if ((OnlinePIESettings->bOnlinePIEEnabled && GetNumPIELogins() > 0) || HasAuthCommandLineStatic())
		{
	...
	virtual bool IsOnlinePIEEnabled() const override
	{
		...
		const UOnlinePIESettings* OnlinePIESettings = GetDefault<UOnlinePIESettings>();
		return OnlinePIESettings->bOnlinePIEEnabled || HasAuthCommandLineStatic();
	}
	...
	virtual int32 GetNumPIELogins() const override
	{
		...
		if (NumValidLogins == 0 && HasAuthCommandLineStatic())
		{
			NumValidLogins++;
		}
...
#if WITH_EDITOR
bool IOnlineSubsystemUtils::IsOnlinePIEEnabledStatic()
{
	if (UObjectInitialized())
	{
		if (IOnlineSubsystemUtils* OnlineUtils = Online::GetUtils())
		{
			return OnlineUtils->IsOnlinePIEEnabled();
		}
	}

	// Fallback: read INI directly (CDO not available yet)
	...
	{
		TArray<FString> PIELogins;
		GConfig->GetArray(TEXT("/Script/OnlineSubsystemUtils.OnlinePIESettings"), TEXT("Logins"), PIELogins, GEditorPerProjectIni);
		if (PIELogins.Num() == 0)
		{
	...
	return bOnlinePIEEnabled || HasAuthCommandLineStatic();
}
#endif
