...
void UOnlineEngineInterfaceImpl::PIEEndCleanup(FName OnlineIdentifier)
{
	IOnlineSubsystem* Subsystem = IOnlineSubsystem::Get(OnlineIdentifier);

	if (FOnlineSubsystemImpl* SubsystemImpl = StaticCast<FOnlineSubsystemImpl*>(Subsystem))
	{
		SubsystemImpl->PIEEndCleanup();
	}
}
