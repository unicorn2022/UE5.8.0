...
{
	UE_LOGF(LogPartyBeacon, Display, "Party Reservation:");
	UE_LOGF(LogPartyBeacon, Display, "  TeamNum: %d", TeamNum);
	UE_LOGF(LogPartyBeacon, Display, "  PartyLeader: %ls", *PartyLeader.ToDebugString());
	UE_LOGF(LogPartyBeacon, Display, "  PartyMembers(%d):", PartyMembers.Num());
	
	int32 PartyMemberIdx = 0;
	for (const FPlayerReservation& PartyMember : PartyMembers)
	{
		UE_LOGF(LogPartyBeacon, Display, "    Member %d", PartyMemberIdx);
		++PartyMemberIdx;
		UE_LOGF(LogPartyBeacon, Display, "      UniqueId: %ls", *PartyMember.UniqueId.ToDebugString());
		UE_LOGF(LogPartyBeacon, Display, "		Crossplay: %ls", *LexToString(PartyMember.bAllowCrossplay));
#if UE_BUILD_SHIPPING
		UE_LOGF(LogPartyBeacon, Display, "      ValidationStr: %d bytes", PartyMember.ValidationStr.Len());
#else
		UE_LOGF(LogPartyBeacon, Display, "      ValidationStr: %ls", *PartyMember.ValidationStr);
#endif
		UE_LOGF(LogPartyBeacon, Display, "      ElapsedTime: %0.2f", PartyMember.ElapsedTime);
	}
}

void FPartyReservation::DumpUniqueIdsOnly() const
{
	UE_LOGF(LogPartyBeacon, Display, "Party Reservation UniqueIds:");
...
void UPartyBeaconState::InitTeamArray()
{
	...
	UE_LOGF(LogPartyBeacon, Display,
		"Beacon State: team count (%d), team size (%d), host team (%d)",
		NumTeams,
		NumPlayersPerTeam,
		ReservedHostTeamNum);
}
...
bool UPartyBeaconState::ReconfigureTeamAndPlayerCount(int32 InNumTeams, int32 InNumPlayersPerTeam, int32 InNumReservations)
{
	...
	if (NumConsumedReservations <= InNumReservations)
	{
		...
		if (NumTeams > InNumTeams)
		{
			...
			for (int32 TeamIdx = InNumTeams; TeamIdx < NumTeams; TeamIdx++)
			{
				...
				{
					bTeamError = true;
					UE_LOGF(LogPartyBeacon, Warning, "Beacon has players on a team about to be removed.");
				}
			}
		}
		...
		if (NumPlayersPerTeam > InNumPlayersPerTeam)
		{
			for (int32 TeamIdx = 0; TeamIdx<NumTeams; TeamIdx++)
			{
				...
				{
					bTeamSizeError = true;
					UE_LOGF(LogPartyBeacon, Warning, "Beacon has too many players on a team about to be resized.");
				}
			}
		}
		...
		if (!bTeamError && !bTeamSizeError)
		{
			...
			UE_LOGF(LogPartyBeacon, Display,
				"Reconfiguring to team count (%d), team size (%d)",
				NumTeams,
				NumPlayersPerTeam);
		}
	}
	...
	{
		UE_LOGF(LogPartyBeacon, Warning, "Beacon has too many consumed reservations for this reconfiguration, ignoring request.");
	}
...
int32 UPartyBeaconState::GetTeamForCurrentPlayer(const FUniqueNetId& PlayerId) const
{
	...
	if (PlayerId.IsValid())
	{
		...
		UE_LOGF(LogPartyBeacon, VeryVerbose, "Assigning player %ls to team %d",
			*PlayerId.ToString(),
			TeamNum);
	}
	else
	{
		UE_LOGF(LogPartyBeacon, Display, "Invalid player when attempting to find team assignment");
	}
...
int32 UPartyBeaconState::GetPlayersOnTeam(int32 TeamIndex, TArray<FUniqueNetIdRepl>& TeamMembers) const
{
	...
	{
		UE_LOGF(LogPartyBeacon, Warning, "GetPlayersOnTeam: Invalid team index %d", TeamIndex);
	}
...
int32 UPartyBeaconState::GetTeamAssignment(const FPartyReservation& Party)
{
	if (NumTeams > 1)
	{
		if (TeamAssignmentMethod == ETeamAssignmentMethod::Manual)
		{
			...
			{
				UE_LOGF(LogPartyBeacon, Warning, "UPartyBeaconHost::GetTeamAssignment: manually assigned team %d has no room.", Party.TeamNum);
				return INDEX_NONE;
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogPartyBeacon, Warning, "UPartyBeaconHost::GetTeamAssignment: couldn't find an open team for party members.");
				return INDEX_NONE;
			}
		}
	}
...
void UPartyBeaconState::BestFitTeamAssignmentJiggle()
{
	if (TeamAssignmentMethod == ETeamAssignmentMethod::BestFit &&
		NumTeams > 1)
	{
		UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::BestFitTeamAssignmentJiggle NumTeams=%d", NumTeams);
		...
		for (FPartyReservation* Reservation : ReservationsToJiggle)
		{
			...
			if (Reservation->TeamNum == -1)
			{
				UE_LOGF(LogPartyBeacon, Warning, "UPartyBeaconHost::BestFitTeamAssignmentJiggle: could not reassign to a team!");
			}
		}
...
bool UPartyBeaconState::CrossPlayAllowed(const FPartyReservation& ReservationRequest) const
{
	...
	if (ExistingPlatforms.Num() > 0)
	{
		...
		UE_LOGF(LogPartyBeacon, Verbose, "Existing: %ls", *ExistingStr);
		...
		UE_LOGF(LogPartyBeacon, Verbose, "NewParty: %ls", *PartyPlatformStr);

		UE_LOGF(LogPartyBeacon, Log, "UPartyBeaconState::CrossPlayAllowed bEveryoneAllowsCrossplay:%ls bPartyAllowsCrossplay:%ls bCrossConsoleAllowed:%ls bExistingPlayersOk:%ls bIncomingPlayersOk:%ls bCrossPlayAllowed:%ls",
...
bool UPartyBeaconState::DoesReservationFit(const FPartyReservation& ReservationRequest) const
{
	...
	UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::DoesReservationFit: Incoming Party Size: %d Num Players Per Team: %d NumConsumedReservations: %d MaxReservations: %d", IncomingPartySize, NumPlayersPerTeam, NumConsumedReservations, MaxReservations);
...
bool UPartyBeaconState::AddReservation(const FPartyReservation& ReservationRequest)
{
	...
	{
		UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::AddReservation");
	...
	if (TeamAssignment != INDEX_NONE)
	{
		...
		UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::AddReservation: Setting NumConsumedReservations to %d", NumConsumedReservations);
...
bool UPartyBeaconState::RemoveReservation(const FUniqueNetIdRepl& PartyLeader)
{
	...
	if (ExistingReservationIdx != INDEX_NONE)
	{
		...
		UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::RemoveReservation: %ls, setting NumConsumedReservations to %d", *PartyLeader.ToString(), NumConsumedReservations);
	...
	if (InPartyMemberId.IsValid() && !InAuthTicket.IsEmpty())
	{
		...
		for (int32 ResIdx = 0; ResIdx < Reservations.Num() && !bFoundReservation; ResIdx++)
		{
			...
			if (PlayerRes)
			{
				...
				{
					UE_LOGF(LogPartyBeacon, VeryVerbose, "Setting auth ticket for member %ls.", *InPartyMemberId.ToDebugString());
				}
				else if (PlayerRes->ValidationStr != InAuthTicket)
				{
					UE_LOGF(LogPartyBeacon, Warning, "Auth ticket changing for member %ls.", *InPartyMemberId.ToDebugString());
				}
		...
		{
			UE_LOGF(LogPartyBeacon, Warning, "Found no reservation for player %ls, while registering auth ticket.", *InPartyMemberId.ToDebugString());
		}
	}
}
...
void UPartyBeaconState::UpdatePartyLeader(const FUniqueNetIdRepl& InPartyMemberId, const FUniqueNetIdRepl& NewPartyLeaderId)
{
	if (InPartyMemberId.IsValid() && NewPartyLeaderId.IsValid())
	{
				...
				if (NewPartyLeaderReservationIdx != PartyMemberReservationIdx)
				{
					...
					if (!DestinationReservation || DestinationReservation->CanPlayerMigrateFromReservation(*PriorReservation))
					{
						...
						if (ensure(PriorPlayerReservationIdx != INDEX_NONE))
						{
							...
							{
								UE_LOGF(LogPartyBeacon, Verbose, "UpdatePartyLeader:  Moving player %ls from reservation under party leader %ls, to reservation under party leader %ls",
							...
							{
								UE_LOGF(LogPartyBeacon, Verbose, "UpdatePartyLeader:  Moving player %ls from reservation under party leader %ls, to new reservation with leader %ls",
									*InPartyMemberId.ToString(), *PriorReservation->PartyLeader.ToString(), *NewPartyLeaderId.ToString());
							...
							{
								UE_LOGF(LogPartyBeacon, Verbose, "UpdatePartyLeader:  Removing now empty reservation that had party leader %ls", *PriorReservation->PartyLeader.ToString());
						...
						{
							UE_LOGF(LogPartyBeacon, Warning, "UpdatePartyLeader:  Member %ls not found in their own reservation!", *InPartyMemberId.ToDebugString());
						}
					}
					else
					{
						UE_LOGF(LogPartyBeacon, Warning, "UpdatePartyLeader:  Unable to migrate player %ls from reservation under leader %ls to existing reservation with leader %ls",
							*InPartyMemberId.ToString(), *PriorReservation->PartyLeader.ToString(), *NewPartyLeaderId.ToString());
					}
				}
				else
				{
					UE_LOGF(LogPartyBeacon, VeryVerbose, "UpdatePartyLeader:  Player %ls already under party leader %ls, no action taken",
		...
		{
			UE_LOGF(LogPartyBeacon, Warning, "UpdatePartyLeader:  No reservation found for player %ls!", *InPartyMemberId.ToDebugString());
		}
	}
}
	...
	if (ResIdx != INDEX_NONE && OtherResIdx != INDEX_NONE)
	{
		...
		if (PartyRes.TeamNum != OtherPartyRes.TeamNum)
		{
			...
			if (bValidTeamSizeA && bValidTeamSizeB)
			{
				if (UE_LOG_ACTIVE(LogPartyBeacon, Verbose))
				{
					UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::SwapTeams: %ls %ls", *PartyLeader.ToString(), *OtherPartyLeader.ToString());
					PartyRes.Dump();
					OtherPartyRes.Dump();
				}
...
{
	UE_LOGF(LogPartyBeacon, VeryVerbose, "UPartyBeaconState::RemovePlayer: %ls", *PlayerId.ToDebugString());
	...
	for (int32 ResIdx = 0; ResIdx < Reservations.Num() && !bWasRemoved; ResIdx++)
	{
		...
		{
			UE_LOGF(LogPartyBeacon, Display, "UPartyBeaconState::RemovePlayer: Party leader %ls has left the party, %d members in reservation", *PlayerId.ToString(), Reservation.PartyMembers.Num());
			if (UE_LOG_ACTIVE(LogPartyBeacon, Verbose))
			{
				Reservation.Dump();
			}
			...
			for (int32 PlayerIdx = 0; PlayerIdx < Reservation.PartyMembers.Num(); PlayerIdx++)
			{
					...
					GetExistingReservation(PlayerEntry.UniqueId) == INDEX_NONE)
				{
					...
					UE_LOGF(LogPartyBeacon, Display, "UPartyBeaconState::RemovePlayer: Promoting member %ls to leader", *PlayerEntry.UniqueId.ToDebugString());
			...
			{
				UE_LOGF(LogPartyBeacon, Display, "UPartyBeaconState::RemovePlayer: Failed to find a player to promote to leader");
			}
		...
		for (int32 PlayerIdx = 0; PlayerIdx < Reservation.PartyMembers.Num(); PlayerIdx++)
		{
			...
			if (PlayerEntry.UniqueId == PlayerId)
			{
				...
				if (UE_LOG_ACTIVE(LogPartyBeacon, Verbose))
				{
					UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::RemovePlayer: Player found in reservation with leader %ls, setting NumConsumedReservations to %d", *Reservation.PartyLeader.ToString(), NumConsumedReservations);
					Reservation.Dump();
				}
				SanityCheckReservations(true);
			}
		...
		{
			UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::RemovePlayer: Empty reservation found with leader %ls, removing", *Reservation.PartyLeader.ToString());
	...
	if (bWasRemoved)
	{
		...
		UE_LOGF(LogPartyBeacon, Verbose, "UPartyBeaconState::RemovePlayer: Player removed, calling BestFitTeamAssignmentJiggle");
		BestFitTeamAssignmentJiggle();
	}
...
bool UPartyBeaconState::GetPartyLeader(const FUniqueNetIdRepl& InPartyMemberId, FUniqueNetIdRepl& OutPartyLeaderId) const
{
	...
	if (InPartyMemberId.IsValid())
	{
		for (int32 ResIdx = 0; ResIdx < Reservations.Num() && !bFoundReservation; ResIdx++)
		{
			...
			if (PlayerRes)
			{
				UE_LOGF(LogPartyBeacon, Display, "Found party leader for member %ls.", *InPartyMemberId.ToDebugString());
		...
		if (!bFoundReservation)
		{
			UE_LOGF(LogPartyBeacon, Warning, "Found no reservation for player %ls, while looking for party leader.", *InPartyMemberId.ToDebugString());
		}
	}
...
void UPartyBeaconState::DumpReservations() const
{
	...
	UE_LOGF(LogPartyBeacon, Display, "Session that reservations are for: %ls", *SessionName.ToString());
	UE_LOGF(LogPartyBeacon, Display, "Number of teams: %d", NumTeams);
	UE_LOGF(LogPartyBeacon, Display, "Number players per team: %d", NumPlayersPerTeam);
	UE_LOGF(LogPartyBeacon, Display, "Number total reservations: %d", MaxReservations);
	UE_LOGF(LogPartyBeacon, Display, "Number consumed reservations: %d", NumConsumedReservations);
	UE_LOGF(LogPartyBeacon, Display, "Number of party reservations: %d", Reservations.Num());
	...
	for (int32 PartyIndex = 0; PartyIndex < Reservations.Num(); PartyIndex++)
	{
		...
		UE_LOGF(LogPartyBeacon, Display, "\t Party leader: %ls", *NetId->ToDebugString());
		UE_LOGF(LogPartyBeacon, Display, "\t Party team: %d", Reservations[PartyIndex].TeamNum);
		UE_LOGF(LogPartyBeacon, Display, "\t Party size: %d", Reservations[PartyIndex].PartyMembers.Num());
		...
		for (int32 MemberIndex = 0; MemberIndex < Reservations[PartyIndex].PartyMembers.Num(); MemberIndex++)
		{
			...
			UE_LOGF(LogPartyBeacon, Display, "\t  Party member: %ls [%ls] Cross: %ls", *PlayerRes.UniqueId->ToString(), *PlayerRes.Platform, *LexToString(PlayerRes.bAllowCrossplay));
		}
	}
	UE_LOGF(LogPartyBeacon, Display, "");
}
