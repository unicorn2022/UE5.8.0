...
{
	UE_LOGF(LogSpectatorBeacon, Verbose, "InitHostBeacon MaxSize:%d", InMaxReservations);
...
bool ASpectatorBeaconHost::InitFromBeaconState(USpectatorBeaconState* PrevState)
{
	if (!State && PrevState)
	{
		UE_LOGF(LogSpectatorBeacon, Verbose, "InitFromBeaconState MaxSize:%d", PrevState->MaxReservations);
		State = PrevState;
		return true;
	}
			...
			if (Session)
			{
				...
				for (int32 ResIdx = 0; ResIdx < Reservations.Num(); ResIdx++)
				{
					...
					for (int32 ClientIdx = 0; ClientIdx < ClientActors.Num(); ClientIdx++)
					{
						...
						{
							UE_LOGF(LogSpectatorBeacon, Error, "Missing SpectatorBeaconClient found in ClientActors array");
					...
					else
					{
							...
							bIsSessionOwner)
						{
							...
							{
								UE_LOGF(LogSpectatorBeacon, Display, "Beacon (%ls): pending player %ls found in session (%ls), removing.",
									*GetName(),
				...
				if (bLogoutOnSessionTimeout)
				{
					...
					for (int32 LogoutIdx = 0; LogoutIdx < PlayersToLogout.Num(); LogoutIdx++)
					{
						...
						UE_LOGF(LogSpectatorBeacon, Display, "Beacon (%ls): pending player logout due to timeout for %ls, elapsed time = %0.3f, removing",
							*GetName(),
...
void ASpectatorBeaconHost::SendReservationUpdates()
{
	if (State && ClientActors.Num() > 0)
	{
		...
		if (NumRemaining < MaxReservations)
		{
			if (NumRemaining > 0)
			{
				UE_LOGF(LogSpectatorBeacon, Verbose, "Sending reservation update %d", NumRemaining);
			...
			{
				UE_LOGF(LogSpectatorBeacon, Verbose, "Sending reservation full");
				for (AOnlineBeaconClient* ClientActor : ClientActors)
				{
					ASpectatorBeaconClient* SpectatorBeaconClient = Cast<ASpectatorBeaconClient>(ClientActor);
					if (SpectatorBeaconClient)
...
void ASpectatorBeaconHost::NewPlayerAdded(const FPlayerReservation& NewPlayer)
{
	if (NewPlayer.UniqueId.IsValid())
	{
		if (State)
		{
			...
			if (FoundIdx == INDEX_NONE)
			{
				UE_LOGF(LogSpectatorBeacon, Verbose, "Beacon adding pending player %ls", *NewPlayer.UniqueId.ToDebugString());
		...
		{
			UE_LOGF(LogSpectatorBeacon, Warning, "Beacon skipping PlayersPendingJoin for beacon with no state!");
		}
	}
	else
	{
		UE_LOGF(LogSpectatorBeacon, Warning, "Beacon skipping PlayersPendingJoin for invalid player!");
	}
}
...
void ASpectatorBeaconHost::HandlePlayerLogout(const FUniqueNetIdRepl& PlayerId)
{
	if (PlayerId.IsValid())
	{
		UE_LOGF(LogSpectatorBeacon, Verbose, "HandlePlayerLogout %ls", *PlayerId->ToDebugString());
...
bool ASpectatorBeaconHost::PlayerHasReservation(const FUniqueNetId& PlayerId) const
{
	...
	{
		UE_LOGF(LogSpectatorBeacon, Warning,
		...
		else
		{
			...
			const FPlayerReservation& Spectator = ReservationRequest.Spectator;
			{
				...
				if (MemberExistingSpectatorReservationIdx != INDEX_NONE)
				{
					...
					UE_LOGF(LogSpectatorBeacon, Display, "ASpectatorBeaconHost::AddSpectatorReservation: Found existing reservation for spectator %ls", *Spectator.UniqueId.ToDebugString());
				...
				else
				{
					...
					if (FoundIdx != INDEX_NONE)
					{
						UE_LOGF(LogSpectatorBeacon, Display, "ASpectatorBeaconHost::AddSpectatorReservation: Found spectator %ls in the pending player list", *Spectator.UniqueId.ToDebugString());
						ReservationRequest.Dump();
						bContainsExistingMembers = true;
					}
				}
...
ESpectatorReservationResult::Type ASpectatorBeaconHost::RemoveSpectatorReservation(const FUniqueNetIdRepl& Spectator)
{
	...
	{
		UE_LOGF(LogSpectatorBeacon, Warning, "Failed to find reservation to cancel for spectator %ls:", Spectator.IsValid() ? *Spectator->ToString() : TEXT("INVALID"));
...
{
	UE_LOGF(LogSpectatorBeacon, Verbose, "ProcessReservationRequest %ls SessionId %ls Spectator: %ls from (%ls)",
	...
	if (Client)
	{
		...
		UE_LOGF(LogSpectatorBeacon, Verbose, "ProcessReservationRequest result: %ls", ESpectatorReservationResult::ToString(Result));
		if (UE_LOG_ACTIVE(LogSpectatorBeacon, Verbose) &&
			(Result != ESpectatorReservationResult::ReservationAccepted))
		{
			DumpReservations();
...
{
	UE_LOGF(LogSpectatorBeacon, Verbose, "ProcessCancelReservationRequest %ls Spectator: %ls from (%ls)",
		Client ? *Client->GetName() : TEXT("NULL"),
	...
	if (Client)
	{
		...
		UE_LOGF(LogSpectatorBeacon, Verbose, "ProcessCancelReservationRequest result: %ls", ESpectatorReservationResult::ToString(Result));
		if (UE_LOG_ACTIVE(LogSpectatorBeacon, Verbose) &&
...
{
	UE_LOGF(LogSpectatorBeacon, Display, "Debug info for Beacon: %ls", *GetBeaconType());
	if (State)
	{
