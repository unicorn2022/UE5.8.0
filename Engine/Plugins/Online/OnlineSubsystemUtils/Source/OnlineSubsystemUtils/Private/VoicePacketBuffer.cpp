...
{
#if DEBUG_POPPING
	UE_LOGF(LogAudio, Log, "Beginning to pop %d samples...", RequestedSamples);
#endif
	...
	{
#if DEBUG_POPPING
		UE_LOGF(LogAudio, Log, "  Loop start, index %d ", DestBufferSampleIndex);
#endif
		...
		if (ListHead == nullptr)
		{
			...
			{
#if DEBUG_POPPING
				UE_LOGF(LogAudio, Warning, "Voip listener now idle.");
#endif
				bIsIdle = true;
			}
...
#if DEBUG_POPPING
			UE_LOGF(LogAudio, Log, "    Sample counter %d behind next packet. Injecting %d samples of silence.", SampleOffset, NumSilentSamples);
#endif
		...
		else if (ListHead->SamplesLeft <=  (int32) (RequestedSamples - DestBufferSampleIndex))
		{
			...
			UE_LOGF(LogAudio, Log, "    Copied %d samples from packet and reached end of packet.", NumSamplesLeftInPacketBuffer);
#endif
		...
		else
		{
			...
			UE_LOGF(LogAudio, Log, "    Copied %d samples from packet and reached end of buffer.", SamplesLeft);
#endif
...
#if DEBUG_POPPING
		UE_LOGF(LogAudio, Log, "  Ended loop with %d samples popped to buffer.", DestBufferSampleIndex);
#endif
	}
#if DEBUG_POPPING
	check(DestBufferSampleIndex == RequestedSamples);
	UE_LOGF(LogAudio, Log, "Finished PopAudio.");
#endif
...
{
#if DEBUG_POPPING
	UE_LOGF(LogAudio, Log, "Beginning to seek forward %d samples...", InNumSamples);
#endif
	...
	{
#if DEBUG_POPPING
		UE_LOGF(LogAudio, Log, "  Loop start, index %d ", NumSamplesSeeked);
#endif
...
void FVoicePacketBuffer::PushPacket(const void* InBuffer, int32 NumBytes, uint64 InStartSample, EVoipStreamDataFormat Format)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogAudio, Warning, "Voice packet buffer filled to capacity of %d packets; packet dropped.", PacketBuffer.Num());
		}
...
#if DEBUG_SORTING
	UE_LOGF(LogAudio, Log, "Packet sort order: ");
	FSortedVoicePacketNode* PacketPtr = ListHead;
	int32 LoggedPatcketIndex = 0;
	uint64 CachedSampleStart = 0;
	while (PacketPtr != nullptr)
	{
		UE_LOGF(LogAudio, Log, "    Packet %d: Sample Start %d", ++LoggedPatcketIndex, PacketPtr->StartSample);
	...
	if (LoggedPatcketIndex > 6)
	{
		...
		UE_LOGF(LogAudio, Log, "Several Packets buffered.");
	}
	UE_LOGF(LogAudio, Log, "End of packet list.");
#endif
}
