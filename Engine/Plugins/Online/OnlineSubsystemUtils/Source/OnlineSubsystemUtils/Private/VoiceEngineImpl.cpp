...
bool FVoiceEngineImpl::Init(int32 MaxLocalTalkers, int32 MaxRemoteTalkers)
{
	...
	if (OnlineSub && !OnlineSub->IsDedicated())
	{
		...
		if (VoiceModule.IsVoiceEnabled())
		{
			...
			{
				UE_LOGF(LogVoice, Warning, "Voice capture initialization failed!");
			}
		}
		else
		{
			UE_LOGF(LogVoice, Log, "Voice module disabled by config [Voice].bEnabled");
		}
	}
	...
	: NumChannelsComingIn(InNumChannels)
{
	...
	if (PlatformEndpoint.IsValid())
	{
		...
		{
			UE_LOGF(LogVoice, Warning, "Failed to find device %ls, using default output device.", *InEndpointName);
