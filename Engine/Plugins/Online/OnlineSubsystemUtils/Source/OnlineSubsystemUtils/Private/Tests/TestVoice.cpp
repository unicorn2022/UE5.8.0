...
#include "Tests/TestVoiceData.inl"
...
bool FTestVoice::Tick(float DeltaTime)
{
	...
	if (VoiceCapture.IsValid())
	{
		...
		if (VoiceComp)
		{
			...
			{
				UE_LOGF(LogVoice, Log, "VOIP audio component %ls playing!", bIsPlaying ? TEXT("is") : TEXT("is not"));
			...
			{
				UE_LOGF(LogVoice, Log, "VOIP audio component starved %d frames!", StarvedDataCount);
			}
			...
			else
			{
				...
				{
					//UE_LOGF(LogVoice, Log, "Getting data! %d", NewVoiceDataBytes);
					...
					{
						UE_LOGF(LogVoice, Log, "Out of order or ambiguous sample count detected! This sample count: %llu Previous sample count: %llu", SampleCount, CachedSampleCount);
					}
			...
			if (bDoWork && TotalVoiceBytes > 0)
			{
				...
				if (VoiceEncoder.IsValid())
				{
					...
					if (LastRemainderSize > 0)
					{
						...
						{
							UE_LOGF(LogVoice, Verbose, "Overflow!");
				...
				if (bUseDecompressed)
				{
					...
					{
						//UE_LOGF(LogVoice, Log, "Queuing uncompressed data! %d", UncompressedDataSize);
						if (bZeroOutput)
						{
				...
				{
					//UE_LOGF(LogVoice, Log, "Queuing original data! %d", UncompressedDataSize);
				...
				if (VoiceDataPtr && VoiceDataSize > 0)
				{
					...
					{
						UE_LOGF(LogVoice, Warning, "UncompressedDataQueue Overflow!");
					}
				}
				...
				{
					UE_LOGF(LogVoice, Log, "Playback started");
					VoiceComp->Play();
				}
			}
		}
...
bool FTestVoice::Exec_Runtime(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	else if (FParse::Command(&Cmd, TEXT("vcdevice")))
	{
		...
		if (!NewDeviceName.IsEmpty())
		{
			...
			if (VoiceCapture.IsValid())
			{
				...
				{
					UE_LOGF(LogVoice, Warning, "Failed to change device name %ls", *DeviceName);
				}
			}
		}
	...
	else if (FParse::Command(&Cmd, TEXT("vcvbr")))
	{
		...
		if (VoiceEncoder.IsValid())
		{
			...
			{
				UE_LOGF(LogVoice, Warning, "Failed to set VBR %d", bVBR);
			}
		}
	...
	else if (FParse::Command(&Cmd, TEXT("vcbitrate")))
	{
		...
		if (VoiceEncoder.IsValid() && NewBitrate > 0)
		{
			...
			{
				UE_LOGF(LogVoice, Warning, "Failed to set bitrate %d", NewBitrate);
			}
		}
	...
	else if (FParse::Command(&Cmd, TEXT("vccomplexity")))
	{
		...
		if (VoiceEncoder.IsValid() && NewComplexity >= 0)
		{
			...
			{
				UE_LOGF(LogVoice, Warning, "Failed to set complexity %d", NewComplexity);
			}
		}
