...
namespace UE::Net::Private
{
	...
	// Relates to the warning message which fires when trying to run HandlerComponents in parallel that do not support parallel execution
	// This latch makes sure we only first the warning once across all threads
	static std::atomic<bool> bHasLoggedHandlerParallelSupportWarning = false;
}
...
void UIpConnection::Tick(float DeltaSeconds)
{
	...
	if (!TryConsumeAddressResolution())
	{
		// TryConsumeAddressResolution only advances the state machine from TryNextAddress
		// (via HasPendingAddressResults guard), never from Connected/Error (safe to call again).
		ECheckAddressResolutionResult CheckResult = Resolver->CheckAddressResolution();
		if (CheckResult == ECheckAddressResolutionResult::Connected)
		{
	...
	if (!!GNetDebugInitialConnect && InTotalPackets == 0 && Driver != nullptr && Driver->ServerConnection != nullptr)
	{
		...
		if (ElapsedLogTime >= static_cast<double>(GNetDebugInitialConnectLogFrequency))
		{
			...
			UE_LOGF(LogNet, Log, "Initial Connect Diagnostics: Sent '%i' packets in last '%f' seconds, no packets received yet.",
			...
			!IpDriver->IsServer())
		{
			...
			if (CooldownCompareTimeDiff > RecreateCooldown && ThresholdCompareTimeDiff > GNetRecreateSocketTimeoutThreshold)
			{
				...
				UE_LOGF(LogNet, Warning,
						"Passed socket recreate threshold. LastReceiveTimeDiff: %f, LastRecvAckTimeDiff: %f, Threshold: %f, Result: %ls",
...
bool UIpConnection::TryConsumeAddressResolution(const TCHAR* LogSuffix)
{
	using namespace UE::Net::Private;

	// Only call CheckAddressResolution() when we know there are pending results (TryNextAddress state).
	// It returns the current ResolutionState and transitions to the next, so calling it here would
	// consume Error/Connected before the caller can observe and handle them via their own call.
	if (!Resolver->HasPendingAddressResults())
	{
		return false;
	}
	...
	if (CheckResult == ECheckAddressResolutionResult::TryFirstAddress || CheckResult == ECheckAddressResolutionResult::TryNextAddress)
	{
		...
		UE_LOGF(LogNet, Log, "Address resolution complete, connection socket assigned (resolved address: %ls)%ls",
			*LowLevelGetRemoteAddress(true), LogSuffix);
...
void UIpConnection::LowLevelSend(void* Data, int32 CountBits, FOutPacketTraits& Traits)
{
	...
	{
		UE_LOGF(LogNet, Verbose, "Skipping send task as we are waiting on the next resolution step");
		return;
	}
...
void UIpConnection::SendToRemote(uint8* DataToSend, int32 CountBits, FOutPacketTraits& Traits) {
	...
	{
#if WITH_SERVER_CODE
		//Flag a warning if we are trying to run non-parallel Handlers in a ParallelPhase
		UE_AUTORTFM_OPEN 
		{
			const UReplicationSystem* ReplicationSystem = Driver->GetReplicationSystem();
			if(ReplicationSystem && ReplicationSystem->IsInParallelPhase())
			{
				bool bExpected = false;
				if(!Handler->CanRunHandlerComponentsOnParallelOutgoingConnections() && bHasLoggedHandlerParallelSupportWarning.compare_exchange_strong(bExpected, true, std::memory_order_relaxed))
				{
					UE_LOGF(LogNet, Warning, "UIpConnection is trying to run non-parallel-compatible HandlerComponents in parallel. These HandlerComponents will be updated in a sequential order instead of simultaneously:");
					Handler->LogHandlerComponentList();
				}
			}
		};
	...
	if (!bBlockSend)
	{
		...
		{
			UE_LOGF(LogNet, Warning, "UIpConnection::LowLevelSend: CountBytes > MaxPacketSize! Count: %i, MaxPacket: %i %ls", CountBytes, MaxPacket, *Describe());
		}
		...
		if (CountBytes > 0)
		{
			...
			else
			{
				...
				else if (ISocketSubsystem* const SocketSubsystem = Driver->GetSocketSubsystem())
				{
...
{
	UE_CLOGF(SocketError_RecvDelayStartTime > 0.0, LogNet, Log,
			"UIpConnection::ReceivedRawPacket: Recovered from socket errors. %ls Connection", ToCStr(Describe()));
...
void UIpConnection::HandleSocketSendResult(const FSocketSendResult& Result, ISocketSubsystem* SocketSubsystem)
{
	...
	{
		UE_CLOGF(SocketError_SendDelayStartTime > 0.f, LogNet, Log, "UIpConnection::HandleSocketSendResult: Recovered from socket errors. %ls Connection", *Describe());
	...
	else if (Result.Error != SE_EWOULDBLOCK)
	{
		...
		if (SocketErrorDisconnectDelay > 0.f)
		{
			...
			{
				UE_LOGF(LogNet, Log, "UIpConnection::HandleSocketSendResult: Socket->SendTo failed with error %i (%ls). %ls Connection beginning close timeout (Timeout = %f).",
...
void UIpConnection::HandleSocketRecvError(class UNetDriver* NetDriver, const FString& ErrorString)
{
	...
	if (SocketErrorDisconnectDelay > 0.f)
	{
		...
		{
			UE_LOGF(LogNet, Log, "%ls. %ls Connection beginning close timeout (Timeout = %f).",
				*ErrorString,
				*Describe(),
				SocketErrorDisconnectDelay);
