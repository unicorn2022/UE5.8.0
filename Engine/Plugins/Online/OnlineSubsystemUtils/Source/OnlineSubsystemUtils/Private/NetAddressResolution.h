...
private:
	 ...
	 * @return						False if synchronous resolution failed immediately (connection is already closed); true otherwise
	 */
	bool InitConnect(UNetConnection* ServerConnection, ISocketSubsystem* SocketSubsystem, const FSocket* ActiveSocket, const FURL& ConnectURL);
...
public:
	...
	/**
	 * Whether or not address resolution has pending results ready to be attempted
	 * (i.e., the state machine is in TryNextAddress). CheckAddressResolution() may still
	 * fail to yield an address (e.g., FindSocketError if no socket matches the protocol).
	 *
	 * @return		Whether or not there are pending address results to attempt
	 */
	bool HasPendingAddressResults() const
	{
		return ResolutionState == EAddressResolutionState::TryNextAddress;
	}
...
private:
	...
	/**
	 * Applies resolved addresses to the connection's resolver. On failure, closes the connection.
	 * In non-shipping builds, also injects any debug address from net.DebugAppendResolverAddress.
	 *
	 * @param Results		The address info result from GetAddressInfo/GetAddressInfoAsync
	 * @param Connection	The owning connection (used for logging, error handling, and socket subsystem access)
	 * @param ConnectURL	The original connection URL
	 * @param LogSuffix		Optional suffix for log lines (e.g., " [Sync]")
	 * @return				True if results were applied successfully, false on error
	 */
	bool ApplyResolutionResults(const FAddressInfoResult& Results, UIpConnection* Connection, const FURL& ConnectURL, const TCHAR* LogSuffix = TEXT(""));
