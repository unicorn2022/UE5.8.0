...
private:
	...
	~FPacketIterator()
	{
		...
		{
			UE_LOGF(LogNet, Warning, "Took too long to receive packets. Time: %2.2f %ls", DeltaReceiveTime, *Driver->GetName());
		}
	}
		...
		if (bCheckReceiveTime && IterationCount > 0)
		{
			...
			if (NumIterationUntilTimeTest <= 0)
			{
				...
				if (CurrentTime > BailOutTime)
				{
					...
					UE_LOGF(LogNet, Verbose, "Stopping packet reception after processing for more than %f seconds. %ls",
				...
				else
				{
					...
					if (bHasPacket)
					{
						...
						if (bReceivedPacket)
						{
							...
							{
								UE_LOGF(LogNet, Warning, "IpNetDriver receive thread received a packet of %d bytes, which is larger than the data buffer size of %d bytes.",
...
FUniqueSocket UIpNetDriver::CreateSocketForProtocol(const FName& ProtocolType)
{
	...
	{
		UE_LOGF(LogNet, Warning, "UIpNetDriver::CreateSocket: Unable to find socket subsystem");
		return NULL;
	}
...
FUniqueSocket UIpNetDriver::CreateAndBindSocket(TSharedRef<FInternetAddr> BindAddr, int32 Port, bool bReuseAddressAndPort, int32 DesiredRecvSize, int32 DesiredSendSize, FString& Error)
{
	...
	{
		UE_LOGF(LogNet, Log, "setsockopt with SO_REUSEADDR failed");
	}

	if (NewSocket->SetRecvErr() == false)
	{
		UE_LOGF(LogNet, Log, "setsockopt with IP_RECVERR failed");
	}
	...
	UE_LOGF(LogInit, Log, "%ls: Socket queue. Rx: %i (config %i) Tx: %i (config %i)", SocketSubsystem->GetSocketAPIName(),
	...
	if (BoundPort == 0)
	{
		...
		{
			UE_LOGF(LogNet, Fatal, "Fatal error: %ls", *Error);
		}
...
bool UIpNetDriver::InitBase( bool bInitAsClient, FNetworkNotify* InNotify, const FURL& URL, bool bReuseAddressAndPort, FString& Error )
{
	...
	if (SocketSubsystem == nullptr)
	{
		UE_LOGF(LogNet, Warning, "Unable to find socket subsystem");
		return false;
	}
	...
	{
		UE_LOGF(LogNet, Error, "InitBindSockets failed: %ls", ToCStr(Error));
	...
	if (bRecvMultiEnabled && !bRecvThreadEnabled)
	{
		...
		if (bSupportsRecvMulti)
		{
			...
			UE_LOGF(LogNet, Log, "NetDriver RecvMulti state size: %zu, Retrieve Timestamps: %i", MemArc.GetMax(),
					(uint32)bRetrieveTimestamps);
		}
		else
		{
			UE_LOGF(LogNet, Warning, "NetDriver could not enable RecvMulti, as current socket subsystem does not support it.");
		}
	}
	else if (bRecvMultiEnabled && bRecvThreadEnabled)
	{
		UE_LOGF(LogNet, Warning, "NetDriver RecvMulti is not yet supported with the Receive Thread enabled.");
	}
...
bool UIpNetDriver::InitConnect( FNetworkNotify* InNotify, const FURL& ConnectURL, FString& Error )
{
	...
	{
		UE_LOGF(LogNet, Warning, "Failed to init net driver ConnectURL: %ls: %ls", *ConnectURL.ToString(), *Error);
		return false;
	}
	...
	if (!Resolver->InitConnect(ServerConnection, SocketSubsystem, GetSocket(), ConnectURL))
	{
		Error = TEXT("Address resolution failed during InitConnect");
		return false;
	}
	...
	UE_LOGF(LogNet, Log, "Game client on port %i, rate %i", ConnectURL.Port, ServerConnection->CurrentNetSpeed );
...
bool UIpNetDriver::InitListen( FNetworkNotify* InNotify, FURL& LocalURL, bool bReuseAddressAndPort, FString& Error )
{
	...
	{
		UE_LOGF(LogNet, Warning, "Failed to init net driver ListenURL: %ls: %ls", *LocalURL.ToString(), *Error);
		return false;
	}
	...
	UE_LOGF(LogNet, Log, "%ls IpNetDriver listening on port %i", *GetDescription(), LocalURL.Port );
	...
	for (FPacketIterator It(this); It; ++It)
	{
			...
			else if (ReceivedPacket.Error != SE_ECONNRESET && ReceivedPacket.Error != SE_UDP_ERR_PORT_UNREACH)
			{
				...
				if (ReceivedPacket.Error == SE_EMSGSIZE)
				{
					...
					if (MyServerConnection)
					{
						...
						{
							UE_LOGF(LogNet, Log, "Received packet with bytes > max MTU from an incoming IP address that doesn't match expected server address: Actual: %ls Expected: %ls",
					...
					{
						UE_LOGF(LogSecurity, Warning, "%ls: Malformed_Packet: Received Packet with bytes > max MTU", *Connection->RemoteAddressToString());
					}
				}
				else
				{
				...
				else
				{
					...
					UE_CLOGF(!DDoS.CheckLogRestrictions(), LogNet, Warning, "%ls", *ErrorString);
				}
		...
		if( bOk == false )
		{
			if( Connection )
			{
				if( Connection != GetServerConnection() )
				{
					...
					if ((Connection->GetConnectionState() != USOCK_Open) || (!AllowPlayerPortUnreach))
					{
						if (LogPortUnreach)
						{
							UE_LOGF(LogNet, Log, "Received ICMP port unreachable from client %ls.  Disconnecting.",
								*FromAddr->ToString(true));
						}
						Connection->CleanUp();
					}
			...
			else
			{
				...
				{
					UE_LOGF(LogNet, Log, "Received ICMP port unreachable from %ls.  No matching connection found.",
						*FromAddr->ToString(true));
				}
			}
		}
...
FSocket* UIpNetDriver::GetSocket()
{
	...
	{
		// Null here triggers the SocketNotReady path in handshake
...
UNetConnection* UIpNetDriver::ProcessConnectionlessPacket(FReceivedPacketView& PacketRef, const FPacketBufferView& WorkingBuffer)
{
	...
	if (Notify != nullptr && ConnectionlessHandler.IsValid() && StatelessConnectComponent.IsValid())
	{
		...
		if (Result == EIncomingResult::Success)
		{
			...
			if (bPassedChallenge)
			{
				...
				{
					UE_LOGF(LogNet, Log, "Finding connection to update to new address: %ls", *IncomingAddress);
					...
					if (FoundConn != nullptr)
					{
						...
						if (RemoteAddrRef->IsPortValid(Address->GetPort()))
						{
						...
						UE_LOGF(LogNet, Log, "Updated IP address for connection. Connection = %ls, Old Address = %ls", *FoundConn->Describe(), *OldAddress);
					}
					else
					{
						UE_LOGF(LogNet, Log, "Failed to find an existing connection with a matching cookie. Restarted Handshake failed.");
					}
				}
	...
	{
		UE_CLOGF(!DDoS.CheckLogRestrictions(), LogNet, Log, "Accepting connection without handshake, due to '-NoPacketHandler'.")
	...
	{
		UE_LOGF(LogNet, Log, "Invalid Notify (%i) or ConnectionlessHandler (%i) or StatelessConnectComponent (%i); can't accept connections.",
	...
	if (bPassedChallenge)
	{
		if (!bRestartedHandshake)
		{
			...
			UE_LOGF(LogNet, Log, "Server accepting post-challenge connection from: %ls", *IncomingAddress);
	...
	{
		UE_LOGF(LogNet, VeryVerbose, "Server failed post-challenge connection from: %ls", *IncomingAddress);
	}
...
void UIpNetDriver::LowLevelSend(TSharedPtr<const FInternetAddr> Address, void* Data, int32 CountBits, FOutPacketTraits& Traits)
{
	...
	{
		UE_LOGF(LogNet, Warning, "UIpNetDriver::LowLevelSend: Invalid send address '%ls'", *Address->ToString(true));
	}
}
...
void UIpNetDriver::LowLevelDestroy()
{
	...
	if(CurrentSocket != nullptr && !HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		{
			UE_LOGF(LogNet, Log, "Shutting down and waiting for socket receive thread for %ls", *GetDescription());
			...
			if (!CurrentSocket->Shutdown(ESocketShutdownMode::Read))
			{
				...
				UE_LOGF(LogNet, Log, "UIpNetDriver::LowLevelDestroy Socket->Shutdown returned error %ls (%d) for %ls", SocketSubsystem->GetSocketError(ShutdownError), static_cast<int>(ShutdownError), *GetDescription());
			}
		...
		{
			UE_LOGF(LogExit, Log, "closesocket error (%i)", (int32)SocketSubsystem->GetLastErrorCode() );
		}
		...
		UE_LOGF(LogExit, Log, "%ls shut down",*GetDescription() );
	}
	...
	if (ConnectionlessHandler.IsValid() && StatelessConnectComponent.IsValid())
	{
		...
		for (int32 i = 0; i < ClientConnections.Num() && NumConnections-- > 0; i++)
		{
			...
			if (UIpConnection* const TestConnection = Cast<UIpConnection>(ClientConnections[i]))
			{
				...
				UE_LOGF(LogNet, Log, "TestSuddenPortChange - Changed this connection: %ls.", *TestConnection->Describe());
			}
		}
	}
}
...
void UIpNetDriver::TrackAndLogNewIP(const FInternetAddr& InAddr)
{
	...
	if (LIKELY(!bExceededIPAggregationLimit))
	{
		...
		if (bLogIPHit)
		{
			UE_LOGF(LogNet, Log, "NotifyAcceptingConnection accepted from: %ls", ToCStr(InAddr.ToString(true)));
		}
...
void UIpNetDriver::TickNewIPTracking(float DeltaTime)
{
	...
	if (bCheckAggregatedIPs)
	{
		if (bExceededIPAggregationLimit)
		{
			UE_LOGF(LogNet, Log, "NotifyAcceptingConnection accepted aggregation: Last tracking period exceeded limit.");
		}
		else if (AggregatedIPsToLog.Num() > 0)
		{
			...
			UE_LOGF(LogNet, Log, "%ls", ToCStr(AggregateIPLogStr.ToString()));
		}
...
UE::Net::ERecreateSocketResult UIpNetDriver::RecreateSocket(int32 OverridePort)
{
	...
	if (bSupportsRecreateSocket && ServerConnection != nullptr && !ServerConnection->IsReplay())
	{
		...
		if (CurState == ESocketState::Ready)
		{
			...
			if (NewSocket.IsValid())
			{
				UE_LOGF(LogNet, Log, "Recreated socket for bind address: %ls", ToCStr(LocalAddr->ToString(true)));
			...
			{
				UE_LOGF(LogNet, Warning, "Could not recreate socket for bind address %ls, got error %ls", ToCStr(LocalAddr->ToString(false)),
						ToCStr(Error));
...
uint32 UIpNetDriver::FReceiveThreadRunnable::Run()
{
	...
	UE_LOGF(LogNet, Log, "UIpNetDriver::FReceiveThreadRunnable::Run starting up.");
	...
	UE_LOGF(LogNet, Log, "UIpNetDriver::FReceiveThreadRunnable::Run returning.");
