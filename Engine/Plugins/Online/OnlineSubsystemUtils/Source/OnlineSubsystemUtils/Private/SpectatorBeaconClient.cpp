...
void ASpectatorBeaconClient::ClearTimers(bool bCallFailSafeIfNeeded)
{
	...
	if (World)
	{
		...
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "ClearTimers: Pending reservation response cleared.");
		}

		if (PendingCancelResponseTimerHandle.IsValid())
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "ClearTimers: Pending cancel response cleared.");
		}

		if (PendingReservationUpdateTimerHandle.IsValid())
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "ClearTimers: Pending reservation update cleared.");
		}

		if (PendingReservationFullTimerHandle.IsValid())
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "ClearTimers: Pending reservation full cleared.");
		}

		if (CancelRPCFailsafe.IsValid())
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "ClearTimers: Cancel failsafe cleared.");
		}
...
bool ASpectatorBeaconClient::RequestReservation(const FString& ConnectInfoStr, const FString& InSessionId, const FUniqueNetIdRepl& RequestingSpectator, const FPlayerReservation& Spectator)
{
	...
	UE_LOGF(LogSpectatorBeacon, Verbose, "SpectatorBeaconClient RequestReservation() connectURL: %ls", *ConnectURL.ToString());
	...
	{
		UE_LOGF(LogSpectatorBeacon, Warning, "RequestReservation: Failure to init client beacon with %ls.", *ConnectURL.ToString());
		RequestType = ESpectatorClientRequestType::NonePending;
	}

	if (!bSuccess)
	{
		AOnlineBeacon::OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_spectator_init_failure"));
	}
...
bool ASpectatorBeaconClient::RequestReservation(const FOnlineSessionSearchResult& DesiredHost, const FUniqueNetIdRepl& RequestingSpectator, const FPlayerReservation& Spectator)
{
	...
	AOnlineBeacon::OnFailure(EBeaconFailureReason::ApplicationError, TEXTVIEW("beacon_spectator_request_reservation_failure"));
...
void ASpectatorBeaconClient::CancelReservation()
{
	if (ensure(PendingReservation.SpectatorId.IsValid()))
	{
		...
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "Sending cancel reservation request.");
		...
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "Reservation request never sent, no need to send cancelation request.");
			OnCancelledComplete();
		}
	}
	else
	{
		UE_LOGF(LogSpectatorBeacon, Verbose, "Unable to cancel reservation request with invalid spectator.");
		OnCancelledComplete();
	}
}
...
void ASpectatorBeaconClient::OnConnected()
{
	if (!bCancelReservation)
	{
		...
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon connection established, sending join reservation request.");
			ServerReservationRequest(DestSessionId, PendingReservation);
			bPendingReservationSent = true;
		}
		else
		{
			UE_LOGF(LogSpectatorBeacon, Warning, "Failed to handle reservation request type %ls", ToString(RequestType));
			AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_spectator_reservation_failure"));
		}
	}
	else
	{
		UE_LOGF(LogSpectatorBeacon, Verbose, "Reservation request previously canceled, nothing sent.");
		OnCancelledComplete();
	}
}
...
void ASpectatorBeaconClient::ClientReservationResponse_Implementation(ESpectatorReservationResult::Type ReservationResponse)
{
	if (!bCancelReservation)
	{
		...
		if (Rate > 0.0f)
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon response received %ls, waiting %fs to notify", ESpectatorReservationResult::ToString(ReservationResponse), Rate);
	...
	{
		UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon response received %ls, ignored due to cancel in progress", ESpectatorReservationResult::ToString(ReservationResponse));
...
void ASpectatorBeaconClient::ProcessReservationResponse(ESpectatorReservationResult::Type ReservationResponse)
{
	if (!bCancelReservation)
	{
		UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon response received %ls", ESpectatorReservationResult::ToString(ReservationResponse));
...
void ASpectatorBeaconClient::ClientCancelReservationResponse_Implementation(ESpectatorReservationResult::Type ReservationResponse)
{
	...
	if (Rate > 0.0f)
	{
		UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon cancellation response received %ls, waiting %fs to notify", ESpectatorReservationResult::ToString(ReservationResponse), Rate);
...
void ASpectatorBeaconClient::ClientSendReservationUpdates_Implementation(int32 NumRemainingReservations)
{
	if (!bCancelReservation)
	{
		...
		if (Rate > 0.0f)
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon reservations remaining %d, waiting %fs to notify", NumRemainingReservations, Rate);
...
{
	UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon reservations remaining %d", NumRemainingReservations);
...
void ASpectatorBeaconClient::ClientSendReservationFull_Implementation()
{
	if (!bCancelReservation)
	{
		...
		if (Rate > 0.0f)
		{
			UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon reservations full, waiting %fs to notify", Rate);
...
{
	UE_LOGF(LogSpectatorBeacon, Verbose, "Spectator beacon reservations full");
