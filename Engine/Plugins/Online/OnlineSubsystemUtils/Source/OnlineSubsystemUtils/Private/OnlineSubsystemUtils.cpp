...
UAudioComponent* CreateVoiceAudioComponent(uint32 SampleRate, int32 NumChannels)
{
	...
	if (GEngine != nullptr)
	{
		if (FAudioDeviceHandle AudioDevice = GEngine->GetMainAudioDevice())
		{
			...
			{
				UE_LOGF(LogVoiceDecode, Warning, "Unable to create voice audio component!");
			}
		}
	}
...
UVoipListenerSynthComponent* CreateVoiceSynthComponent(UWorld* World, uint32 SampleRate)
{
	...
	if (World)
	{
		if (FAudioDeviceHandle AudioDeviceHandle = World->GetAudioDevice())
		{
			...
			{
				UE_LOGF(LogVoiceDecode, Warning, "Unable to create voice synth component!");
			}
		}
	}
...
UWorld* GetWorldForOnline(FName InstanceName)
{
	...
	{
		FWorldContext* WorldContext = GEngine->GetWorldContextFromHandle(InstanceName);
		if (WorldContext)
		{
		...
		if (NetDriver && NetDriver->GetNetMode() < NM_Client)
		{
			...
			if (Colon != INDEX_NONE)
			{
				...
				if (PortStr.IsNumeric())
				{
					...
					{
						UE_LOGF(LogOnline, Verbose, "GetPortFromNetDriver: parsed port %d from address '%ls' is out of range [0, %u]; returning 0.", ParsedPort, *AddressStr, MAX_uint16);
					}
				}
				else
				{
					UE_LOGF(LogOnline, Verbose, "GetPortFromNetDriver: trailing token '%ls' from address '%ls' is not numeric; returning 0.", *PortStr, *AddressStr);
				}
			}
		}
	}
...
uint64 GetBaseVoiceChatTeamId(const UWorld* World)
{
	...
	if (NetDriver)
	{
		...
		if (!AddressStr.IsEmpty())
		{
			...
			{
				// Address is not parseable as IP (e.g. EOS:<PUID>); hash it for a deterministic per-host base id.
				OutAddr = GetTypeHash(AddressStr);
			}

			const uint32 ProcId = FPlatformProcess::GetCurrentProcessId();
			// Layout: bits 32..63 = IP-or-address-hash | bits 24..31 = reserved for TeamIndex (filled in by GetVoiceChatTeamId) | bits 0..23 = ProcessId
...
bool FOnlineBeaconClientFilter::Parse(const TCHAR* Cmd, FOnlineBeaconClientFilter& OutFilter, FOnlineBeaconClientFilterFlags Flags)
{
	...
	if (FParse::Value(Cmd, TEXT("UniqueId="), UniqueIdStr))
	{
		...
		if (!UniqueId.IsValid())
		{
			UE_LOGF(LogBeacon, Warning, "Beacon filter parse failure - unable to parse UniqueId: %ls", *UniqueIdStr);
	...
	{
		UE_LOGF(LogBeacon, Warning, "Beacon filter parse failure - no arguments found. Wildcard filter is not allowed.");
		bArgumentsValid = false;
	}
...
bool HandleBeaconCommands(IOnlineSubsystem* InOnlineSub, const UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FParse::Command(&Cmd, TEXT("DUMP")))
	{
		...
		{
			UE_LOGF(LogBeacon, Log, "Dumping beacon info based on filter: %ls.", *BeaconFilter.ToString());
			...
			{
				UE_LOGF(LogBeacon, Log, "    AOnlineBeaconClient: %ls", *GetBeaconLogString(*ClientBeacon));
			}
			UE_LOGF(LogBeacon, Log, "Dump complete. Found %d beacons.", FilteredBeacons.Num());
		}
		else
		{
			UE_LOGF(LogBeacon, Warning, "DUMP command failure - unable to parse arguments.");
		}
	}
	else if (FParse::Command(&Cmd, TEXT("DISCONNECT")))
	{
		...
		{
			UE_LOGF(LogBeacon, Log, "Disconnecting beacons based on filter: %ls.", *BeaconFilter.ToString());
			...
			for (AOnlineBeaconClient* ClientBeacon : GetFilteredOnlineBeaconClientInstances(BeaconFilter))
			{
				...
				{
					UE_LOGF(LogBeacon, Log, "    Disconnecting beacon: %ls", *GetBeaconLogString(*ClientBeacon));
				...
				{
					UE_LOGF(LogBeacon, Log, "    Beacon not in a valid state to disconnect: %ls", *GetBeaconLogString(*ClientBeacon));
				}
			}

			UE_LOGF(LogBeacon, Log, "Disconnect complete. %d beacons affected.", NumDisconnected);
		}
		else
		{
			UE_LOGF(LogBeacon, Warning, "DISCONNECT command failure - unable to parse arguments.");
		}
	}
