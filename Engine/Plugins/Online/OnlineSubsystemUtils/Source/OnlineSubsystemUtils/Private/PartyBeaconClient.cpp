...
void APartyBeaconClient::ClearTimers(bool bCallFailSafeIfNeeded)
{
	...
	{
		UE_LOGF(LogBeacon, Verbose, "Clearing timers with cancel reservation in flight.  Calling Failsafe.");
	...
	if (World)
	{
		...
		{
			UE_LOGF(LogPartyBeacon, Verbose, "ClearTimers: Pending reservation response cleared.");
		}

		if (PendingCancelResponseTimerHandle.IsValid())
		{
			UE_LOGF(LogPartyBeacon, Verbose, "ClearTimers: Pending cancel response cleared.");
		}

		if (PendingReservationUpdateTimerHandle.IsValid())
		{
			UE_LOGF(LogPartyBeacon, Verbose, "ClearTimers: Pending reservation update cleared.");
		}

		if (PendingReservationFullTimerHandle.IsValid())
		{
			UE_LOGF(LogPartyBeacon, Verbose, "ClearTimers: Pending reservation full cleared.");
		}

		if (CancelRPCFailsafe.IsValid())
		{
			UE_LOGF(LogPartyBeacon, Verbose, "ClearTimers: Cancel failsafe cleared.");
		}
...
bool APartyBeaconClient::RequestReservation(const FString& ConnectInfoStr, const FString& InSessionId, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FPlayerReservation>& PartyMembers)
{
	...
	{
		UE_LOGF(LogPartyBeacon, Warning, "RequestReservation: Failure to init client beacon with %ls.", *ConnectURL.ToString());
		RequestType = EClientRequestType::NonePending;
	}

	if (!bSuccess)
	{
		AOnlineBeacon::OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_party_init_failure"));
	}
...
bool APartyBeaconClient::RequestReservation(const FOnlineSessionSearchResult& DesiredHost, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FPlayerReservation>& PartyMembers)
{
	...
	AOnlineBeacon::OnFailure(EBeaconFailureReason::ApplicationError, TEXTVIEW("beacon_party_request_reservation_failure"));
...
bool APartyBeaconClient::RequestReservationUpdate(const FString& ConnectInfoStr, const FString& InSessionId, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FPlayerReservation>& PlayersToModify, bool bRemovePlayers)
{
	...
	{
		UE_LOGF(LogPartyBeacon, Warning, "APartyBeaconClient::RequestReservationUpdate: Missing ConnectInfoStr ('%ls') or SessionId ('%ls').", *ConnectInfoStr, *InSessionId);
	}
...
bool APartyBeaconClient::RequestAddOrUpdateReservation(const FString& ConnectInfoStr, const FString& InSessionId, const FUniqueNetIdRepl& RequestingPartyLeader, const TArray<FPlayerReservation>& PartyMembers)
{
	...
	{
		UE_LOGF(LogPartyBeacon, Warning, "%ls - Failure to init client beacon with %ls.", ANSI_TO_TCHAR(__FUNCTION__ ), *ConnectURL.ToString());
		RequestType = EClientRequestType::NonePending;
	}

	if (!bSuccess)
	{
		AOnlineBeacon::OnFailure(EBeaconFailureReason::ApplicationError, TEXTVIEW("beacon_party_update_reservation_failure"));
	}
...
void APartyBeaconClient::CancelReservation()
{
	if (ensure(PendingReservation.PartyLeader.IsValid()))
	{
		...
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Sending cancel reservation request.");
		...
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Reservation request never sent, no need to send cancelation request.");
			OnCancelledComplete();
		}
	}
	else
	{
		UE_LOGF(LogPartyBeacon, Verbose, "Unable to cancel reservation request with invalid party leader.");
		OnCancelledComplete();
	}
}
...
void APartyBeaconClient::OnConnected()
{
	if (!bCancelReservation)
	{
		...
		{
			UE_LOGF(LogPartyBeacon, Verbose, "%ls - Party beacon connection established, sending add or update reservation request.", ANSI_TO_TCHAR(__FUNCTION__));
		...
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Party beacon connection established, sending join reservation request.");
			ServerReservationRequest(DestSessionId, PendingReservation);
			bPendingReservationSent = true;
		}
		else if (RequestType == EClientRequestType::ReservationUpdate)
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Party beacon connection established, sending reservation update request.");
			ServerUpdateReservationRequest(DestSessionId, PendingReservation);
			bPendingReservationSent = true;
		}
		else if (RequestType == EClientRequestType::ReservationRemoveMembers)
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Party beacon connection established, sending reservation update request to remove player.");
			ServerRemoveMemberFromReservationRequest(DestSessionId, PendingReservation);
			bPendingReservationSent = true;
		}
		else
		{
			UE_LOGF(LogPartyBeacon, Warning, "Failed to handle reservation request type %ls", ToString(RequestType));
			AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_party_reservation_failure"));
		}
	}
	else
	{
		UE_LOGF(LogPartyBeacon, Verbose, "Reservation request previously canceled, nothing sent.");
		OnCancelledComplete();
	}
}
...
void APartyBeaconClient::ClientReservationResponse_Implementation(EPartyReservationResult::Type ReservationResponse)
{
	if (!bCancelReservation)
	{
		...
		if (Rate > 0.0f)
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Party beacon response received %ls, waiting %fs to notify", EPartyReservationResult::ToString(ReservationResponse), Rate);
	...
	{
		UE_LOGF(LogPartyBeacon, Verbose, "Party beacon response received %ls, ignored due to cancel in progress", EPartyReservationResult::ToString(ReservationResponse));
...
void APartyBeaconClient::ProcessReservationResponse(EPartyReservationResult::Type ReservationResponse)
{
	if (!bCancelReservation)
	{
		UE_LOGF(LogPartyBeacon, Verbose, "Party beacon response received %ls", EPartyReservationResult::ToString(ReservationResponse));
...
void APartyBeaconClient::ClientCancelReservationResponse_Implementation(EPartyReservationResult::Type ReservationResponse)
{
	...
	if (Rate > 0.0f)
	{
		UE_LOGF(LogPartyBeacon, Verbose, "Party beacon cancellation response received %ls, waiting %fs to notify", EPartyReservationResult::ToString(ReservationResponse), Rate);
...
void APartyBeaconClient::ClientSendReservationUpdates_Implementation(int32 NumRemainingReservations)
{
	if (!bCancelReservation)
	{
		...
		if (Rate > 0.0f)
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Party beacon reservations remaining %d, waiting %fs to notify", NumRemainingReservations, Rate);
...
{
	UE_LOGF(LogPartyBeacon, Verbose, "Party beacon reservations remaining %d", NumRemainingReservations);
...
void APartyBeaconClient::ClientSendReservationFull_Implementation()
{
	if (!bCancelReservation)
	{
		...
		if (Rate > 0.0f)
		{
			UE_LOGF(LogPartyBeacon, Verbose, "Party beacon reservations full, waiting %fs to notify", Rate);
...
{
	UE_LOGF(LogPartyBeacon, Verbose, "Party beacon reservations full");
