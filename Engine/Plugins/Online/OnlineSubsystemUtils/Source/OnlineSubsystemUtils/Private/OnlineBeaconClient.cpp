...
bool AOnlineBeaconClient::InitClient(const FOnlineBeaconClientInitParams& Params)
{
	...
	if(Params.URL.Valid)
	{
		if (InitBase() && NetDriver)
		{
			...
			NetDriver->SetReplicateTransactionally(Params.bReplicateTransactionally);

			FString Error;
			if (NetDriver->InitConnect(this, Params.URL, Error))
			{
				...
				if (World)
				{
					...
					else
					{
						...
						UE_LOGF(LogBeacon, Warning, "AOnlineBeaconClient::InitClient BeginHandshaking failed");
						...
						if (GetConnectionState() == EBeaconConnectionState::Pending)
						{
							...
							AOnlineBeacon::OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_client_init_handshake_failure"));
						}
					}
				}
				else
				{
					...
					UE_LOGF(LogBeacon, Log, "AOnlineBeaconClient::InitClient failed - no world");
					SetConnectionState(EBeaconConnectionState::Invalid);
					AOnlineBeacon::OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_client_init_no_world"));
				}
			}
			else
			{
				// error initializing the network stack...
				UE_LOGF(LogBeacon, Log, "AOnlineBeaconClient::InitClient failed");
				SetConnectionState(EBeaconConnectionState::Invalid);
				AOnlineBeacon::OnFailure(EBeaconFailureReason::InitFailure, TEXTVIEW("beacon_client_init_failure"));
			}
		}
	}
...
{
	FOnlineBeaconClientInitParams Params;
	Params.URL = URL;
...
void AOnlineBeaconClient::Tick(float DeltaTime)
{
	if (NetDriver != nullptr && NetDriver->ServerConnection != nullptr)
	{
		...
		{
			UE_LOGF(LogBeacon, Verbose, "Client beacon (%ls) socket has closed, triggering failure.", *GetName());
			AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_client_connection_closed_remotely"));
		}
	}
}
...
void AOnlineBeaconClient::SendInitialJoin()
{
	...
	if (ensure(ServerConn != nullptr))
	{
		...
		{
			UE_LOGF(LogNet, Error, "AOnlineBeaconClient::SendInitialJoin: EncryptionToken is empty when 'net.AllowEncryption' requires it.");

			SetConnectionState(EBeaconConnectionState::Invalid);
			Super::OnFailure(EBeaconFailureReason::EncryptionFailure, TEXTVIEW("beacon_client_empty_encryption_token"));
		}
	}
}
...
{
	UE_LOGF(LogBeacon, Verbose, "Client beacon (%ls) connection failure, handling connection timeout.", *GetName());
	SetConnectionState(EBeaconConnectionState::Invalid);
...
{
	UE_LOGF(LogBeacon, Verbose, "%ls: Received first RPC from server", *GetName());
...
void AOnlineBeaconClient::NotifyControlMessage(UNetConnection* Connection, uint8 MessageType, class FInBunch& Bunch)
{
	if(NetDriver->ServerConnection)
	{
		...
		UE_LOGF(LogBeacon, Log, "%ls[%ls] Client received: %ls", *GetName(), *Connection->GetName(), FNetControlMessageInfo::GetName(MessageType));
#endif
		switch (MessageType)
		{
		case NMT_EncryptionAck:
			{
				...
				else
				{
					...
					else
					{
						...
						UE_LOGF(LogBeacon, Warning, "%ls: No delegate available to handle encryption ack, disconnecting.", *Connection->GetName());
						AOnlineBeacon::OnFailure(EBeaconFailureReason::EncryptionFailure, TEXTVIEW("beacon_client_encryption_delegate_missing"));
					}
				}
				break;
			}
		case NMT_Challenge:
		{
			...
			{
				// Force close the session
				UE_LOGF(LogBeacon, Warning, "%ls: Unable to parse challenge request message.", *Connection->GetName());
				AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_client_parse_challenge_error"));
			}
			break;
		}
		case NMT_BeaconWelcome:
			{
				...
				{
					// Force close the session
					UE_LOGF(LogBeacon, Log, "Beacon close from invalid beacon type");
					AOnlineBeacon::OnFailure(EBeaconFailureReason::VersionMismatch, TEXTVIEW("beacon_client_invalid_beacon_type"));
				}
				break;
			}
				...
				if (FNetControlMessage<NMT_BeaconAssignGUID>::Receive(Bunch, NetID))
				{
					if (NetID.IsValid())
					{
						...
						if (!Connection->Driver->bNoTimeouts)
						{
							...
							UE_LOGF(LogBeacon, Verbose, "%ls: Waiting for first RPC from server, timeout: %.3f", *GetName(), Timeout);
							FTimerDelegate TimerDelegate;
							TimerDelegate.BindWeakLambda(this, [this]() mutable
								{
									this->AOnlineBeacon::OnFailure(EBeaconFailureReason::TimeOut, TEXTVIEW("beacon_client_rpc_timeout"));
								});
					...
					{
						// Force close the session
						UE_LOGF(LogBeacon, Log, "Beacon close from invalid NetGUID");
						AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_client_invalid_netguid"));
					}
				}
		...
		case NMT_Failure:
			{
				...
				if (FNetControlMessage<NMT_Failure>::Receive(Bunch, ErrorMsg))
				{
					...
					// Force close the session
					UE_LOGF(LogBeacon, Log, "Beacon close from NMT_Failure %ls", *ErrorMsg);
					AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_client_nmt_failure"));
				}
		...
		default:
			{
				// Force close the session
				UE_LOGF(LogBeacon, Log, "Beacon close from unexpected control message");
				AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_client_unexpected_control_message"));
				break;
			}
		}
	}	
	...
	if (Connection)
	{
		if (Connection->GetConnectionState() != USOCK_Invalid && Connection->GetConnectionState() != USOCK_Closed && Connection->Driver)
		{
			...
			else
			{
				...
				UE_LOGF(LogBeacon, Warning, "AOnlineBeaconClient::FinalizeEncryptedConnection: encryption failure [%ls] %ls", *ResponseStr, *Response.ErrorMsg);
				AOnlineBeacon::OnFailure(EBeaconFailureReason::EncryptionFailure, TEXTVIEW("beacon_client_encryption_failure"));
			}
		}
		else
		{
			UE_LOGF(LogBeacon, Warning, "AOnlineBeaconClient::FinalizeEncryptedConnection: connection in invalid state. %ls", *Connection->Describe());
			AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_client_invalid_state_connection"));
		}
	}
	else
	{
		UE_LOGF(LogBeacon, Warning, "AOnlineBeaconClient::FinalizeEncryptedConnection: Connection is null.");
		AOnlineBeacon::OnFailure(EBeaconFailureReason::TransportError, TEXTVIEW("beacon_client_null_connection"));
	}
}
