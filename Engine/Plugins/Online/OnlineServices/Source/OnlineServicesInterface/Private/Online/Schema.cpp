...
void LogSchemaDefinition(const FSchemaDefinition& Definition)
{
	if (UE_LOG_ACTIVE(LogOnlineSchema, Verbose))
	{
		...
		UE_LOGF(LogOnlineSchema, Verbose, "[SchemaDefinition] SchemaName: [%ls]", *SchemaName);
		...
		{
			UE_LOGF(LogOnlineSchema, Verbose, "[SchemaDefinition][%ls] Parent: [%ls]", *SchemaName, *ParentSchema);
		}
		UE_LOGF(LogOnlineSchema, Verbose, "[SchemaDefinition][%ls] Categories: [%ls]", *SchemaName, *GetCategoryString(Definition));
		for (const TPair<FSchemaCategoryId, FSchemaCategoryDefinition>& CategoryPair : Definition.Categories)
		{
			...
			{
				UE_LOGF(LogOnlineSchema, Verbose, "[SchemaDefinition][%ls][%ls] SchemaCompatibilityAttributeId: [%ls]", *SchemaName, *CategoryName, *Category.SchemaCompatibilityAttributeId.ToString());
			}
			if (Category.SchemaCompatibilityServiceAttributeId != FSchemaId())
			{
				UE_LOGF(LogOnlineSchema, Verbose, "[SchemaDefinition][%ls][%ls] SchemaCompatibilityServiceAttributeId: [%ls]", *SchemaName, *CategoryName, *Category.SchemaCompatibilityServiceAttributeId.ToString());
			}
			...
			for (const TPair<FSchemaAttributeId, FSchemaAttributeDefinition>& AttributeDefinitionPair : Category.SchemaAttributeDefinitions)
			{
				...
				UE_LOGF(LogOnlineSchema, Verbose, "[SchemaDefinition][%ls][%ls][%ls] Type: [%ls], Flags: [%ls], MaxSize: [%d], ServiceAttributeId: [%ls]",
...
bool FSchemaRegistry::ParseConfig(const FSchemaRegistryDescriptorConfig& Config)
{
	...
	for (const FSchemaServiceAttributeDescriptor& SchemaServiceAttributeDescriptor : Config.ServiceAttributeDescriptors)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Duplicate schema service attribute descriptor found: [%ls]", *SchemaServiceAttributeDescriptor.Id.ToString());
	...
	for (const FSchemaServiceDescriptor& SchemaServiceDescriptor : Config.ServiceDescriptors)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Duplicate schema service descriptor found: [%ls]", *SchemaServiceDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
		...
		for (const FSchemaServiceAttributeId& SchemaServiceAttributeId : SchemaServiceDescriptor.AttributeIds)
		{
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema service attribute %ls.%ls: Attribute id has already been used.",
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema service attribute %ls.%ls: Attribute does not exist.",
					*SchemaServiceDescriptor.Id.ToString(),
					*SchemaServiceAttributeId.ToString());
				ParsedSuccessfully = false;
				continue;
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema service attribute %ls.%ls: A valid supported type must be selected.",
					*SchemaServiceDescriptor.Id.ToString(),
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema service attribute %ls.%ls: A valid max size must be set for variable length data.",
					*SchemaServiceDescriptor.Id.ToString(),
					*SchemaServiceAttributeDescriptor.Id.ToString());
				ParsedSuccessfully = false;
			}
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema service attribute %ls.%ls: Either Public or Private visibility must be set.",
					*SchemaServiceDescriptor.Id.ToString(),
					*SchemaServiceAttributeDescriptor.Id.ToString());
				ParsedSuccessfully = false;
			}
	...
	for (const FSchemaCategoryDescriptor& SchemaCategoryDescriptor : Config.SchemaCategoryDescriptors)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Duplicate schema category descriptor found: %ls", *SchemaCategoryDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema category %ls: Service descriptor id %ls not found.",
	...
	// Populate known schema. Verify what can be verified statically. Hierarchy validation is performed below.
	...
	for (const FSchemaDescriptor& SchemaDescriptor : Config.SchemaDescriptors)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Duplicate schema descriptor found: %ls", *SchemaDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
		...
		if (SchemaDescriptor.ParentId == FSchemaId() && SchemaDescriptor.CategoryIds.IsEmpty())
		{
			...
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema %ls: Base schema must define at least one category.",
		...
		if (SchemaDescriptor.ParentId != FSchemaId() && !SchemaDescriptor.CategoryIds.IsEmpty())
		{
			...
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema %ls: Child schema cannot define categories.",
				*SchemaDescriptor.Id.ToString());
			ParsedSuccessfully = false;
		}
		...
		for (const FSchemaCategoryId& CategoryId : SchemaDescriptor.CategoryIds)
		{
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema category %ls.%ls: Category id has already been used.",
					*SchemaDescriptor.Id.ToString(),
					*CategoryId.ToString());
				ParsedSuccessfully = false;
				continue;
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema category %ls.%ls: Category does not exist.",
					*SchemaDescriptor.Id.ToString(),
					*CategoryId.ToString());
				ParsedSuccessfully = false;
			}
	...
	for (const FSchemaAttributeDescriptor& SchemaAttributeDescriptor : Config.SchemaAttributeDescriptors)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Duplicate schema attribute descriptor found: %ls", *SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
			continue;
		}
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: A valid type must be set.",
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: A valid max size must be set for variable length data.",
				*SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
		}
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: UpdateGroupId must be set to either 0 or a positive integer value.",
				*SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
		}
		...
		if (EnumHasAllFlags(AttributeFlags, ESchemaAttributeFlags::SchemaCompatibilityId))
		{
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: SchemaCompatibilityId field may only be set to Int64.",
					*SchemaAttributeDescriptor.Id.ToString());
				ParsedSuccessfully = false;
			}
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: SchemaCompatibilityId attribute may not be private.",
					*SchemaAttributeDescriptor.Id.ToString());
				ParsedSuccessfully = false;
			}
		}
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: Public and Private visibility are mutually exclusive.",
				*SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
		}
		if (!EnumHasAnyFlags(AttributeFlags, ESchemaAttributeFlags::Public | ESchemaAttributeFlags::Private))
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: Either Public or Private visibility must be set.",
				*SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
		}
		if (EnumHasAllFlags(AttributeFlags, ESchemaAttributeFlags::Searchable | ESchemaAttributeFlags::Private))
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls: Searchable attributes may not be private.",
				*SchemaAttributeDescriptor.Id.ToString());
			ParsedSuccessfully = false;
		}
	}
	...
	for (const FSchemaCategoryAttributesDescriptor& SchemaCategoryAttributesDescriptor : Config.SchemaCategoryAttributeDescriptors)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Duplicate schema additional category attributes descriptor found: %ls.%ls",
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid additional category attributes %ls.%ls: Schema does not exist",
				*SchemaCategoryAttributesDescriptor.SchemaId.ToString(),
				*SchemaCategoryAttributesDescriptor.CategoryId.ToString());
			ParsedSuccessfully = false;
			continue;
		...
		{
			UE_LOGF(LogOnlineSchema, Error, "Invalid additional category attributes %ls.%ls: Category does not exist",
				*SchemaCategoryAttributesDescriptor.SchemaId.ToString(),
				*SchemaCategoryAttributesDescriptor.CategoryId.ToString());
			ParsedSuccessfully = false;
			continue;
		...
		for (const FSchemaAttributeId& AttributeId : SchemaCategoryAttributesDescriptor.AttributeIds)
		{
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid category attributes %ls.%ls: Attribute %ls duplicated",
					*SchemaCategoryAttributesDescriptor.SchemaId.ToString(),
					*SchemaCategoryAttributesDescriptor.CategoryId.ToString(),
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Invalid category attributes %ls.%ls: Attribute %ls does not exist",
					*SchemaCategoryAttributesDescriptor.SchemaId.ToString(),
					*SchemaCategoryAttributesDescriptor.CategoryId.ToString(),
					*AttributeId.ToString());
				bAttributesParsedSuccessfully = false;
	...
	for (const TPair<FSchemaId, const FSchemaDescriptor*>& SchemaDescriptorPair : KnownSchemaDescriptors)
	{
		...
		while (TestSchemaDescriptor)
		{
			...
			{
				UE_LOGF(LogOnlineSchema, Error, "Circular parent dependency found in schema: %ls", *SchemaDescriptorToVerify.Id.ToString());
				ParsedSuccessfully = false;
				break;
			}
			...
			if (TestSchemaDescriptor->ParentId != FSchemaId())
			{
				...
				{
					UE_LOGF(LogOnlineSchema, Error, "Invalid schema %ls: Parent schema %ls does not exist.",
		...
		for (const FSchemaCategoryId& SchemaCategoryId : RootSchemaDescriptor.CategoryIds)
		{
			...
			if (SchemaCategoryDescriptor.ServiceDescriptorId != FSchemaServiceDescriptorId())
			{
				...
				{
					UE_LOGF(LogOnlineSchema, Error, "Invalid schema category %ls.%ls: Service descriptor id %ls has already been used by another category.",
						*SchemaDescriptorToVerify.Id.ToString(),
						*SchemaCategoryDescriptor.Id.ToString(),
						*SchemaCategoryDescriptor.ServiceDescriptorId.ToString());
					ParsedSuccessfully = false;
			...
			for (const FSchemaId SchemaHierarchyLayerId : SchemaHierarchy)
			{
				...
				for (const FSchemaAttributeId& SchemaAttributeId : *FoundCategoryAttributes)
				{
					...
					{
						UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls.%ls.%ls: Attribute id has already been used.",
							*SchemaDescriptorToVerify.Id.ToString(),
							*SchemaCategoryDescriptor.Id.ToString(),
					...
					{
						UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls.%ls.%ls: Searchable fields may only exist in the base schema.",
							*SchemaDescriptorToVerify.Id.ToString(),
							*SchemaCategoryDescriptor.Id.ToString(),
							*SchemaAttributeDescriptor.Id.ToString());
						ParsedSuccessfully = false;
					...
					if (EnumHasAllFlags(AttributeFlags, ESchemaAttributeFlags::SchemaCompatibilityId))
					{
						...
						{
							UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls.%ls.%ls: SchemaCompatibilityId field may only exist in the base schema.",
								*SchemaDescriptorToVerify.Id.ToString(),
								*SchemaCategoryDescriptor.Id.ToString(),
								*SchemaAttributeDescriptor.Id.ToString());
							ParsedSuccessfully = false;
						...
						{
							UE_LOGF(LogOnlineSchema, Error, "Invalid schema attribute %ls.%ls.%ls: SchemaCompatibilityId field has already been used by attribute %ls.",
								*SchemaDescriptorToVerify.Id.ToString(),
								*SchemaCategoryDescriptor.Id.ToString(),
								*SchemaAttributeDescriptor.Id.ToString(),
	...
	if (ParsedSuccessfully)
	{
		...
		for (const TPair<FSchemaId, const FSchemaDescriptor*>& SchemaDescriptorPair : KnownSchemaDescriptors)
		{
					...
					for (const FSchemaId& SchemaHierarchyLayerId : SchemaHierarchy)
					{
						...
						for (const FSchemaAttributeId& SchemaAttributeId : *FoundCategoryAttributeIds)
						{
							...
							if (FoundServiceAttributeDescriptor)
							{
								...
								// Stable remove to preserve ordering with compact tset enabled.
								// Automated tests (eg System.Engine.Online.ClientLobbyDataTest) depend on this order.
								PublicAttributes.RemoveStable(FoundServiceAttributeDescriptor);
								PrivateAttributes.RemoveStable(FoundServiceAttributeDescriptor);
								SearchableAttributes.RemoveStable(FoundServiceAttributeDescriptor);
							...
							{
								UE_LOGF(LogOnlineSchema, Error, "Failed to find service attribute to fit schema attribute %ls.%ls.%ls.",
			...
			// Build compatibility id from the schema and data CRCs.
			{
				...
				{
					UE_LOGF(LogOnlineSchema, Error, "Invalid schema %ls: CRC collision processing schema name, please rename the schema to avoid collision.",
						*SchemaDefinition->Id.ToString());
					ParsedSuccessfully = false;
				}
				...
				UE_LOGF(LogOnlineSchema, Verbose, "Building compatability id: SchemaId: [%ls], SchemaIdCrc: [0x%04X], SchemaDataCrc [0x%04X].",
	...
	: SchemaRegistry(SchemaRegistry)
{
	...
	if (!bInitSuccess)
	{
		UE_LOGF(LogOnlineSchema, Error, "[FSchemaCategoryInstance] init failed. DerivedSchema[%ls], BaseSchema[%ls], CategoryId[%ls]",
...
TOnlineResult<FSchemaCategoryInstancePrepareClientChanges> FSchemaCategoryInstanceBase::PrepareClientChanges(FSchemaCategoryInstancePrepareClientChanges::Params&& Params) const
{
	...
	{
		UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] Serialization schema category definition not found.");
	...
	if (Params.ClientChanges.SchemaId)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] Schema change failed: NewDerivedSchema[%ls], BaseSchema[%ls]",
	...
	for (TPair<FSchemaAttributeId, FSchemaVariant>& UpdatedAttributeData : Params.ClientChanges.UpdatedAttributes)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] %ls.%ls.%ls: Attribute definition does not exist in schema category.",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
		...
		if (EnumHasAnyFlags(AttributeDefinition->Flags, ESchemaAttributeFlags::SchemaCompatibilityId))
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] %ls.%ls.%ls: It is not valid to set SchemaCompatibilityId in an attribute update",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
		...
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] %ls.%ls.%ls: Attribute type %ls does not match the expected type %ls.",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
				*AttributeDefinition->Id.ToString(),
		...
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] %ls.%ls.%ls: Variably sized attribute exceeds maximum defined length. %d > %d.",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
				*AttributeDefinition->Id.ToString(),
		...
		if (AttributeDefinition->ServiceAttributeId != FSchemaServiceAttributeId())
		{
			...
			{
				UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] %ls.%ls.%ls: Unable to find service attribute definition for service attribute %ls.",
					*SchemaDefinition->Id.ToString(),
					*SchemaCategoryDefinition->Id.ToString(),
					*AttributeDefinition->Id.ToString(),
	...
	if (SchemaCategoryDefinition->SchemaCompatibilityAttributeId != FSchemaAttributeId() && LastSentSchemaCompatibilityId == 0)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareClientChanges] %ls.%ls.%ls: Failed to find attribute definition for SchemaCompatibilityId field.",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
...
TOnlineResult<FSchemaCategoryInstancePrepareServiceSnapshot> FSchemaCategoryInstanceBase::PrepareServiceSnapshot(FSchemaCategoryInstancePrepareServiceSnapshot::Params&& Params) const
{
	...
	if (!GetSerializationSchema(&SchemaDefinition, &SchemaCategoryDefinition))
	{
		UE_LOGF(LogOnlineSchema, Warning, "[CommitServiceSnapshot] Serialization schema category definition not found.");
	...
	if (Params.SchemaId)
	{
		...
		if (SchemaCompatibilityAttributeData)
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareServiceSnapshot] %ls.%ls: Unable to handle user schema swap when service defines a schema compatibility id.",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString());
			return TOnlineResult<FSchemaCategoryInstancePrepareServiceSnapshot>(Errors::InvalidParams());
		}
		...
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareServiceSnapshot] %ls.%ls: Failed to find valid definition for SchemaId %ls.",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
	...
	for (TPair<FSchemaServiceAttributeId, FSchemaVariant>& SchemaServiceAttributeData : Params.ServiceSnapshot.Attributes)
	{
		...
		{
			UE_LOGF(LogOnlineSchema, Warning, "[PrepareServiceSnapshot] %ls.%ls.%ls: Service attribute definition does not exist in schema category.",
				*SchemaDefinition->Id.ToString(),
				*SchemaCategoryDefinition->Id.ToString(),
				*SchemaServiceAttributeId.ToString());
	...
	ESchemaServiceAttributeFlags& OutSchemaServiceAttributeFlags)
{
	if (ensure(IsValid()))
	{
		if (const FSchemaAttributeDefinition* AttributeDefinition = BaseSchemaCategoryDefinition->SchemaAttributeDefinitions.Find(Id))
		{
			if (AttributeDefinition->Type == Data.GetType())
			{
				...
				if (bIsDataSizeValid)
				{
					...
					if (AttributeDefinition->ServiceAttributeId != FSchemaServiceAttributeId())
					{
						...
						{
							UE_LOGF(LogOnlineSchema, Warning, "[VerifyBaseAttributeData] %ls.%ls.%ls: Unable to find service attribute definition for service attribute %ls.",
								*BaseSchemaCategoryDefinition->Id.ToString(),
								*BaseSchemaCategoryDefinition->Id.ToString(),
								*AttributeDefinition->Id.ToString(),
								*AttributeDefinition->ServiceAttributeId.ToString());
			...
			{
				UE_LOGF(LogOnlineSchema, Verbose, "[VerifyBaseAttributeData] Schema attribute %ls.%ls.%ls set with invalid type %ls. Expected type %ls",
		...
		{
			UE_LOGF(LogOnlineSchema, Verbose, "[VerifyBaseAttributeData] Attribute %ls not found in schema definition %ls.%ls",
				*Id.ToString(),
				*BaseSchemaDefinition->Id.ToString(),
				*BaseSchemaCategoryDefinition->Id.ToString());
			return false;
	...
	{
		UE_LOGF(LogOnlineSchema, Verbose, "[VerifyBaseAttributeData] Unable to set attribute %ls. Schema is not valid",
			*Id.ToString());
		return false;
	}
}
	...
	const FSchemaCategoryDefinition** OutSchemaCategoryDefinition) const
{
	...
	{
		UE_LOGF(LogOnlineSchema, Error, "[InitializeSchemaDefinition] init error: Unable to find definition for schema %ls",
			*SchemaId.ToString());
		return false;
	}
}
	...
	const FSchemaCategoryDefinition** OutSchemaCategoryDefinition) const
{
	...
	{
		UE_LOGF(LogOnlineSchema, Warning, "[InitializeSchemaDefinition] Invalid schema definition. New schema %ls is not a child of %ls.",
	...
	{
		UE_LOGF(LogOnlineSchema, Error, "[InitializeSchemaDefinition] init error: Unable to find category %ls in schema %ls",
			*CategoryId.ToString(),
			*NewDefinition->Id.ToString());
		return false;
	}
