...
namespace UE::Online::Errors {
	...
	inline FOnlineError FromHRESULT(HRESULT Result)
	{
		...
		return FOnlineError(ErrorCodeFromHRESULT(Result), MakeShared<FOnlineErrorDetails, ESPMode::ThreadSafe>(FString::Printf(TEXT("Windows.%llx"), ErrorCodeFromHRESULT(Result)), Internal_HResultToString(Result), FText::FromString(Internal_HResultToString(Result))));
	}
