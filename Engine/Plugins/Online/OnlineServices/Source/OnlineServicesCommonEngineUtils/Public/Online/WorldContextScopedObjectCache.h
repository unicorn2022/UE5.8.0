...
public:
	...
	TSharedPtr<ObjectType> FindOrAdd(FName ContextName, const TFunctionRef<TSharedRef<ObjectType>()>& Create, bool bSkipWorldContextCheck = false)
	{
		...
		UE_LOGF(LogOnlineServices, Log, "[FWorldContextScopedObjectCache::FindOrAdd] Trying to create object for context that does not exist: %ls", *ContextName.ToString());
		return nullptr;
#endif // UE_EDITOR && !UE_IS_COOKED_EDITOR
	}
