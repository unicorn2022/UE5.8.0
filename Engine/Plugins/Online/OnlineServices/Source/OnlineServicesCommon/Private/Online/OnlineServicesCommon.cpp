...
struct FOnlineServicesCommonConfig
{
	...
	float SecondsToSleepForOutstandingOperations = 0.01f;
...
void FOnlineServicesCommon::Init()
{
	OpCache.SetLoadConfigFn(
		[this](FOperationConfig& OperationConfig, const TArray<FString>& SectionHierarchy)
		{
			return LoadConfig(OperationConfig, SectionHierarchy);
		});
	...
	UE_LOGF(LogOnlineServices, Log, "%p %ls online services instance initialize", this, LexToString(GetServicesProvider()));
}

void FOnlineServicesCommon::Flush(EAsyncOpFlushReason FlushReason)
{
	UE_LOGF(LogOnlineServices, Log, "%p %ls online services instance flushing remain operations", this, LexToString(GetServicesProvider()));
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "%p %ls online services flush timed out, cancelling all operations", this, LexToString(GetServicesProvider()));
...
void FOnlineServicesCommon::Destroy()
{
	...
	UE_LOGF(LogOnlineServices, Log, "%p %ls online services instance destroy", this, LexToString(GetServicesProvider()));
