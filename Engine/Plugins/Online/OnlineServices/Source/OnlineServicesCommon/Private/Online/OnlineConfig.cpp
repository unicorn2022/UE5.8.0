...
const TCHAR* ParseField(const TCHAR* InStr, TMap<FString, FString>& OutMap)
{
	...
	if (*Str)
	{
		...
		if (*Str == TCHAR('\"'))
		{
			...
			if (*Str != TCHAR('\"'))
			{
				UE_LOGF(LogOnlineServicesConfig, Warning, "Unmatched quoted string: %ls", InStr);
				return nullptr;
			}
		...
		else if (*Str == TCHAR('('))
		{
			...
			while (*Str && (SubCount > 0 || *Str != TCHAR(')')))
			{
				...
				else if(*Str == TCHAR(')'))
				{
					...
					if (SubCount < 0)
					{
						UE_LOGF(LogOnlineServicesConfig, Warning, "Too many closing parenthesis in: %ls", InStr);
						return nullptr;
					}
				}
				Str++;
...
TArray<IOnlineConfigStructPtr> FOnlineConfigStructGConfig::CreateStructArray(const FString& InConfigValue)
{
	...
	if (!InConfigValue.IsEmpty())
	{
		...
		if(*Str == TCHAR('('))
		{
			...
			if (Str && *Str != TCHAR(')'))
			{
				UE_LOGF(LogOnlineServicesConfig, Warning, "Unmatched opening parenthesis: %ls", Str);
			}
		}
	}
...
const TCHAR* FOnlineConfigStructGConfig::ParseStruct(const TCHAR* InStr, IOnlineConfigStructPtr& OutPtr)
{
	...
	{
		UE_LOGF(LogOnlineServicesConfig, Warning, "Failed to parse struct: %ls", Str);
		return nullptr;
	}
		...
		if (*Str == TCHAR('('))
		{
			...
			while (*Str && *Str != TCHAR(')'))
			{
				...
				if (*Str == TCHAR('\"'))
				{
					...
					if (*Str != TCHAR('\"'))
					{
						UE_LOGF(LogOnlineServicesConfig, Warning, "Unmatched quoted string: %ls", Str);
						return Result;
					}
