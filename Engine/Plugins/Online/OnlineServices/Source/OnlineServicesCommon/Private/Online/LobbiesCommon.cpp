...
void FLobbiesCommon::Initialize()
{
	...
	{
		UE_LOGF(LogOnlineServices, Error, "[FLobbiesCommon::Initialize] Failed to initialize schema registry");
	}
}
...
TOnlineResult<FGetPresenceLobby> FLobbiesCommon::GetPresenceLobby(FGetPresenceLobby::Params&& Params)
{
	...
	{
		UE_LOGF(LogTemp, Verbose, "[%ls] Value of Params.LocalAccountId invalid.", UTF8_TO_TCHAR(__FUNCTION__));
	...
	if (const FLobbyId* PresenceLobbyId = PresenceLobbiesUserMap.Find(Params.LocalAccountId))
	{
		...
		if (GetJoinedLobbiesResult.IsOk())
		{
			...
			UE_LOGF(LogTemp, Verbose, "[%ls] Presence Lobby was not found among Joined Lobbies", UTF8_TO_TCHAR(__FUNCTION__));
			return TOnlineResult<FGetPresenceLobby>({ Errors::NotFound() });
		}
		else
		{
			UE_LOGF(LogTemp, Verbose, "[%ls] GetJoinedLobbies was not successful", UTF8_TO_TCHAR(__FUNCTION__));
			return TOnlineResult<FGetPresenceLobby>(GetJoinedLobbiesResult.GetErrorValue());
		}
	}
	else
	{
		UE_LOGF(LogTemp, Verbose, "[%ls] No data found for user [%ls].", UTF8_TO_TCHAR(__FUNCTION__), *ToLogString(Params.LocalAccountId));
...
TOnlineResult<FIsPresenceLobby> FLobbiesCommon::IsPresenceLobby(FIsPresenceLobby::Params&& Params)
{
	...
	{
		UE_LOGF(LogTemp, Verbose, "[%ls] Value of Params.LobbyId invalid.", UTF8_TO_TCHAR(__FUNCTION__));
		return TOnlineResult<FIsPresenceLobby>(Errors::InvalidParams());
	}
