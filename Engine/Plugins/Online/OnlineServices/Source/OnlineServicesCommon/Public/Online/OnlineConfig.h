...
{
public:
	virtual ~IOnlineConfigProvider() = default;
 ...
 *
 * @param Provider The config provider
 * @param SectionHierarchy The config sections to load values from
 * @param Value The struct whose fields will be loaded from config
 *
 * @return true if any values were loaded
 */
template <typename T>
auto LoadConfig(IOnlineConfigProvider& Provider, const TArray<FString>& SectionHierarchy, T& Value)
	-> std::enable_if_t<TModels_V<Meta::COnlineMetadataAvailable, T>, bool>
{
	bool bLoadedValue = false;
	for (const FString& Section : SectionHierarchy)
	{
		...
		{
			UE_LOGF(LogOnlineServices, Warning, "Space in online config section has been deprecated, use : instead in [%ls]", *Section);
		}
 ...
 */
template <typename T>
auto LoadConfig(IOnlineConfigProvider& Provider, const TArray<FString>& SectionHierarchy, const TCHAR* Key, T& Value)
-> std::enable_if_t<TModels_V<Meta::COnlineMetadataAvailable, T>, bool>
{
	bool bLoadedValue = false;
	for (const FString& Section : SectionHierarchy)
