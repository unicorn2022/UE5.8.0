...
struct FStatDefinition
{
	...
	FStatValue DefaultValue = FStatValue(static_cast<int64>(0));
};
...
public:
	...
	// Example implementation that uses the event to stats mapping rules
	// In this version we assume that FTriggerEvent::Params is a mapping of property names to values,
	// Assumes that FTriggerEvent::Params is defined as TMap<FString, FSchemaVariant>
	// and that each stat referenced in the event has a defined modify rule: Set, Sum, Largest, Smallest
	// After applying the stat changes to the base class cache, we're also broadcasting an update through OnStatsUpdatedEvent.Broadcast().
...
protected:
	UE_API virtual TOnlineAsyncOpRef<FUpdateStats> UpdateStats_Implementation(FUpdateStats::Params&& Params);
	UE_API virtual void ReadStatsFromCache(const FUserStats* ExistingUserStats, const TArray<FString>& StatNames, TMap<FString, FStatValue>& OutStats) const;
	UE_API virtual void CacheUserStats(const FUserStats& UserStats);
