...
{
public:
	virtual ~FOnlineAsyncOp() = default;
...
protected:
	template <typename CallableType>
	void ExecuteImpl(const TCHAR* DebugName, FOnlineAsyncExecutionPolicy ExecutionPolicy, CallableType&& Callable)
	{
		...
		case EOnlineAsyncExecutionPolicy::RunOnGameThread:
			...
			{
				ExecuteOnGameThread(DebugName, MoveTemp(Callable));
			}
			break;

		case EOnlineAsyncExecutionPolicy::RunOnNextTick:
			FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda([DebugName, LambdaCallable = Callable](float dt) mutable
				{
					ExecuteOnGameThread(DebugName, MoveTemp(LambdaCallable));
					return false;
				}));
			break;
	...
	void ExecuteNextStepImpl(const TCHAR* DebugName, const TSharedRef<FOnlineAsyncOp>& SharedThis)
	{
		const int32 StepToExecute = NextStep;
		++NextStep;
		if (StepToExecute < Steps.Num())
		{
			ExecuteImpl(DebugName, Steps[StepToExecute]->GetExecutionPolicy(),
				[this, StepToExecute, WeakThis = TWeakPtr<FOnlineAsyncOp>(SharedThis)]()
				{
					if (TSharedPtr<FOnlineAsyncOp> PinnedThis = WeakThis.Pin())
					{
	...
	void ExecuteRepeatStepImpl(const TCHAR* DebugName, const TSharedRef<FOnlineAsyncOp>& SharedThis)
	{
...
public:
	...
	{
		UE_LOGF(LogOnlineServices, Log, "%p %ls clear complete callbacks while in state %ls", this, OpType::Name, LexToString(SharedState->State));
	...
	void Enqueue(QueueType& Queue, double DelayInSeconds = 0.0)
	{
		...
		UE_LOGF(LogOnlineServices, Verbose, "%p %ls op state set to: %ls", this, OpType::Name, LexToString(SharedState->State));
	...
	void ExecuteNextStep()
	{
		if (!IsComplete())
		{
			ExecuteNextStepImpl(OpType::Name, this->AsShared());
		}
	}
	...
	void ExecuteRepeatStep()
	{
		if (!IsComplete())
		{
			ExecuteRepeatStepImpl(OpType::Name, this->AsShared());
		}
	}
	...
	{
		ExecuteImpl(OpType::Name, ExecutionPolicy, Callable);
	}
...
protected:
	...
	void SetResultAndState(TOnlineResult<OpType>&& Result, EAsyncOpState State)
	{
		if (IsInGameThread())
		{
			...
			UE_LOGF(LogOnlineServices, Verbose, "%p %ls op state set to: %ls", this, OpType::Name, LexToString(State));
