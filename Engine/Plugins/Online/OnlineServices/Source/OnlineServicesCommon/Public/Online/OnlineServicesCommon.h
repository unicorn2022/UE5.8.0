...
public:
	...
	UE_DEPRECATED(5.6, "GetConfigSectionHeiarchy has been renamed GetConfigSectionHierarchy")
	TArray<FString> GetConfigSectionHeiarchy(const FString& OperationName = FString()) const
	{
		return GetConfigSectionHierarchy(OperationName);
	}

	UE_DEPRECATED(5.8, "GetConfigSectionHeirarchy has been renamed GetConfigSectionHierarchy")
	TArray<FString> GetConfigSectionHeirarchy(const FString& OperationName = FString()) const
	{
		return GetConfigSectionHierarchy(OperationName);
	}
	
	TArray<FString> GetConfigSectionHierarchy(const FString& OperationName = FString()) const
	{
		TArray<FString> SectionHierarchy;
		FString SectionName = TEXT("OnlineServices");
		SectionHierarchy.Add(SectionName);
	...
	bool LoadConfig(StructType& Struct, const FString& InterfaceName = FString(), const FString& OperationName = FString()) const
	{
		...
		auto PushSection = [&SectionHierarchy](const TArray<const FStringView>& SectionNames)
		{
			if (Algo::NoneOf(SectionNames, UE_PROJECTION_MEMBER(FStringView, IsEmpty)))
			{
				SectionHierarchy.Emplace(FString::Join(SectionNames, TEXT(".")));
			}
		};
		...
		return LoadConfig(Struct, SectionHierarchy);
	}
	
	UE_DEPRECATED(5.6, "GetConfigSectionHeirachWithOverrides has been renamed GetConfigSectionHierarchyWithOverrides")
	TArray<FString> GetConfigSectionHeirachWithOverrides(const TArray<FString>& SectionHierarchy) const
	{
		return GetConfigSectionHierarchyWithOverrides(SectionHierarchy);
	}

	UE_DEPRECATED(5.8, "GetConfigSectionHeirarchyWithOverrides has been renamed GetConfigSectionHierarchyWithOverrides")
	TArray<FString> GetConfigSectionHeirarchyWithOverrides(const TArray<FString>& SectionHierarchy) const
	{
		return GetConfigSectionHierarchyWithOverrides(SectionHierarchy);
	}
	 ...
	 * 
	 * @param SectionHierarchy Array of config sections to load values from
	 * 
	 * @return Array of the sections with overrides for values to be loaded from
	 */
	TArray<FString> GetConfigSectionHierarchyWithOverrides(const TArray<FString>& SectionHierarchy) const
	{
		...
		TArray<FString> SectionHierarchyWithOverrides;
		for (const FString& Section : SectionHierarchy)
		{
			SectionHierarchyWithOverrides.Add(Section);
			...
			for (const FString& Override : AllConfigSectionOverrides)
			{
				...
				SectionHierarchyWithOverrides.Add(OverrideSection);
		...
		return SectionHierarchyWithOverrides;
	}

	/**
	 * Load a config struct for a section hierarchy, also using the ConfigSectionOverrides
	 *
	 * @param Struct Struct to populate with values from config
	 * @param SectionHierarchy Array of config sections to load values from
	 *
	 ...
	 */
	template <typename StructType>
	bool LoadConfig(StructType& Struct, const TArray<FString>& SectionHierarchy) const
	{
		return ::UE::Online::LoadConfig(*ConfigProvider, GetConfigSectionHierarchyWithOverrides(SectionHierarchy), Struct);
	}
	...
	TOnlineAsyncOpRef<OpType> GetOp(typename OpType::Params&& Params)
	{
		...
		return OpCache.GetOp<OpType>(MoveTemp(Params), GetConfigSectionHierarchy());
	}
	...
	{
		LogErrorIfOnlineServicesHasShutdown<OpType>();
		return OpCache.GetJoinableOp<OpType, ParamsFuncsType>(MoveTemp(Params), GetConfigSectionHierarchy());
	}
	...
	{
		LogErrorIfOnlineServicesHasShutdown<OpType>();
		return OpCache.GetMergeableOp<OpType, ParamsFuncsType>(MoveTemp(Params), GetConfigSectionHierarchy());
	}

	/* Get op (Interface) */
	template <typename OpType>
	TOnlineAsyncOpRef<OpType> GetOp(typename OpType::Params&& Params, const TArray<FString>& ConfigSectionHierarchy)
	{
		LogErrorIfOnlineServicesHasShutdown<OpType>();
		return OpCache.GetOp<OpType>(MoveTemp(Params), ConfigSectionHierarchy);
	}

	template <typename OpType, typename ParamsFuncsType = TJoinableOpParamsFuncs<OpType>>
	TOnlineAsyncOpRef<OpType> GetJoinableOp(typename OpType::Params&& Params, const TArray<FString>& ConfigSectionHierarchy)
	{
		LogErrorIfOnlineServicesHasShutdown<OpType>();
		return OpCache.GetJoinableOp<OpType, ParamsFuncsType>(MoveTemp(Params), ConfigSectionHierarchy);
	}

	template <typename OpType, typename ParamsFuncsType = TMergeableOpParamsFuncs<OpType>>
	TOnlineAsyncOpRef<OpType> GetMergeableOp(typename OpType::Params&& Params, const TArray<FString>& ConfigSectionHierarchy)
	{
		LogErrorIfOnlineServicesHasShutdown<OpType>();
		return OpCache.GetMergeableOp<OpType, ParamsFuncsType>(MoveTemp(Params), ConfigSectionHierarchy);
	}
...
protected:
	...
	{
		UE_CLOGF(bPreShutdownComplete, LogOnlineServices, Error, "Can't add op %ls anymore after shutdown in %p %ls online services!", OpType::Name, this, LexToString(GetServicesProvider()));
	}
