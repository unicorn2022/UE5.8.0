...
IEOSPlatformHandlePtr FOnlineServicesEpicCommonPlatformFactory::CreatePlatform(FName InstanceName, FName InstanceConfigName)
{
	...
	if (!SDKManager)
	{
		UE_LOGF(LogOnlineServices, Error, "[FOnlineServicesEpicCommonPlatformFactory::CreatePlatform] EOSSDK has not been loaded.");
		return {};
	}

	if (!SDKManager->IsInitialized())
	{
		UE_LOGF(LogOnlineServices, Error, "[FOnlineServicesEpicCommonPlatformFactory::CreatePlatform] EOSSDK has not been initialized.");
		return {};
	}
	...
	if (PlatformConfigName.IsEmpty())
	{
		...
		if (PlatformConfigName.IsEmpty())
		{
			UE_LOGF(LogOnlineServices, Verbose, "[FOnlineServicesEpicCommonPlatformFactory::CreatePlatform] Could not find platform config.");
			return {};
		}
	}
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "[FOnlineServicesEpicCommonPlatformFactory::CreatePlatform] Failed to create platform.");
	}
...
FString FOnlineServicesEpicCommonPlatformFactory::LoadEOSPlatformConfig(IEOSSDKManager* const SDKManager)
{
	...
	UE_LOGF(LogOnlineServices, Warning, "[LoadEOSPlatformConfig] Using legacy config from %ls, use EOSShared named config instead.", *ConfigSectionName);
