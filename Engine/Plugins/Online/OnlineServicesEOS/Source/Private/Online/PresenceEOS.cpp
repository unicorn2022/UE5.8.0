...
namespace Meta {
	BEGIN_ONLINE_STRUCT_META(FPresenceEOSConfig)
		...
		ONLINE_STRUCT_FIELD(FPresenceEOSConfig, PresenceRetryDelaySec),
		ONLINE_STRUCT_FIELD(FPresenceEOSConfig, MaxPresenceRetries)
...
FPresenceEOS::~FPresenceEOS() = default;
...
void FPresenceEOS::UpdateConfig()
{
	...
	PresenceRetryDelaySec = Config.PresenceRetryDelaySec;
	MaxPresenceRetries = Config.MaxPresenceRetries;
}
...
void FPresenceEOS::HandleAuthLoginStatusChanged(const FAuthLoginStatusChanged& EventParameters)
{
					...
					.Next([WeakThis = AsWeak(), LocalAccountId](const FAccountId& PresenceAccountId)
						{
							...
							{
								UE_LOGF(LogOnlineServices, Verbose, "OnEOSPresenceUpdate: LocalAccountId=[%ls] PresenceAccountId=[%ls]", *ToLogString(LocalAccountId), *ToLogString(PresenceAccountId));
	...
	else if (EventParameters.LoginStatus == ELoginStatus::NotLoggedIn)
	{
		PresenceLists.Remove(EventParameters.AccountInfo->AccountId);
		ClearPresenceRetry(EventParameters.AccountInfo->AccountId);
	}
}
...
TOnlineAsyncOpHandle<FQueryPresence> FPresenceEOS::QueryPresence(FQueryPresence::Params&& InParams)
{
	...
	if (!Op->IsReady())
	{
		...
		Op->Then([this](TOnlineAsyncOp<FQueryPresence>& InAsyncOp, TPromise<const EOS_Presence_QueryPresenceCallbackInfo*>&& Promise)
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "QueryPresenceResult: [%ls]", *LexToString(Data->ResultCode));
...
TOnlineAsyncOpHandle<FUpdatePresence> FPresenceEOS::UpdatePresence(FUpdatePresence::Params&& InParams)
{
	...
	// Compute removed properties BEFORE applying optimistic cache update,
	// since ApplyFullPresenceToCache will overwrite the cached presence with the new one.
	...
	{
		TSharedRef<FUserPresence> OldPresence = FindOrCreatePresence(Params.LocalAccountId, Params.LocalAccountId);
		for (const TPair<FPresenceProperties::KeyType, FPresenceProperty>& Pair : OldPresence->Properties)
		{
	...
	// Optimistic cache update: apply presence to local cache immediately before enqueue,
	// so that GetCachedPresence returns up-to-date data even if multiple ops are queued.
	CancelPresenceRetry(Params.LocalAccountId);
	ApplyFullPresenceToCache(Params.LocalAccountId, *Presence);

	Op->Then([this, RemovedProperties = MoveTemp(RemovedProperties)](TOnlineAsyncOp<FUpdatePresence>& InAsyncOp, TPromise<const EOS_Presence_SetPresenceCallbackInfo*>&& Promise) mutable
	{
		...
		if (CreatePresenceModificationResult == EOS_EResult::EOS_Success)
		{
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "UpdatePresence: EOS_PresenceModification_SetStatus failed with result %ls", *LexToString(SetStatusResult));
			...
			if (!Presence->RichPresenceString.IsEmpty())
			{
				RichPresenceString = Presence->RichPresenceString;
			}
			else if (!Presence->StatusString.IsEmpty()) // RichPresenceString is the intended way to set presence in UE, but we'll keep reading StatusString for backwards compatibility
			{						
				RichPresenceString = Presence->StatusString;
			}
			...
			const FTCHARToUTF8 Utf8RawRichText(*RichPresenceString);
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "UpdatePresence: EOS_PresenceModification_SetRawRichText failed with result %ls", *LexToString(SetRawRichTextResult));
			...
			if (RemovedProperties.Num() > 0)
			{
				...
				for (const FString& RemovedProperty : RemovedProperties)
				{
					...
					UE_LOGF(LogOnlineServices, VeryVerbose, "UpdatePresence: Removing field %ls", *RemovedProperty); // Temp logging
				}
				...
				{
					UE_LOGF(LogOnlineServices, Warning, "UpdatePresence: EOS_PresenceModification_DeleteDataOptions failed with result %ls", *LexToString(DeleteDataResult));
			...
			if (Presence->Properties.Num() > 0)
			{
				if (Presence->Properties.Num() > EOS_PRESENCE_DATA_MAX_KEYS)
				{
					...
					UE_LOGF(LogOnlineServices, Warning, "UpdatePresence: Too many presence keys.  %u/%u", Presence->Properties.Num(), EOS_PRESENCE_DATA_MAX_KEYS);
				...
				for (const TPair<FPresenceProperties::KeyType, FPresenceProperty>& UpdatedProperty : Presence->Properties)
				{
					...
					if (UpdatedPropertyValueResult.HasValue())
					{
						...
						UE_LOGF(LogOnlineServices, VeryVerbose, "PartialUpdatePresence: Set field [%ls] to [%ls]", *UpdatedProperty.Key, *UpdatedPropertyValueResult.GetValue());
					}
					else
					{
						UE_LOGF(LogOnlineServices, Warning, "PartialUpdatePresence: Unsupported data type passed for presence attribute [%ls]", *UpdatedProperty.Key);
						InAsyncOp.SetError(Errors::InvalidParams());
						Promise.EmplaceValue();
						return;
					}
				...
				{
					UE_LOGF(LogOnlineServices, Warning, "UpdatePresence: EOS_PresenceModification_SetData failed with result %ls", *LexToString(SetDataResult));
	...
	.Then([this](TOnlineAsyncOp<FUpdatePresence>& InAsyncOp, const EOS_Presence_SetPresenceCallbackInfo* Data) mutable
	{
		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			ClearPresenceRetry(InAsyncOp.GetParams().LocalAccountId);
		...
		else
		{
			...
			UE_LOGF(LogOnlineServices, Warning, "UpdatePresence: EOS_Presence_SetPresence failed with result [%ls] for user [%ls], scheduling retry", *LexToString(Data->ResultCode), *ToLogString(Params.LocalAccountId));
			
			SchedulePresenceRetry(Params.LocalAccountId);
			InAsyncOp.SetError(Errors::FromEOSResult(Data->ResultCode));
		}
	})
	.Enqueue(GetSerialQueue(), AsyncOpEnqueueDelay);
...
TOnlineAsyncOpHandle<FPartialUpdatePresence> FPresenceEOS::PartialUpdatePresence(FPartialUpdatePresence::Params&& InParams)
{
	...
	// Capture LocalAccountId and pre-modified mutations before MoveTemp consumes InParams.
	// We need IncomingMutations to apply to the local cache on every call (not only when
	// the op is newly created): if this call merges into an already-Queued op via
	// GetMergeableOp, the op's params absorb the new mutations and ship them over the wire,
	// but the previous initialization path won't run again - which previously left the local
	// cache reflecting only the first call's mutations. See FORT-1089793 for the regression
	// this fixes.
	const FAccountId LocalAccountId = InParams.LocalAccountId;
	FPartialUpdatePresence::Params::FMutations IncomingMutations = InParams.Mutations;
	ModifyPartialPresenceUpdate(IncomingMutations);

	TOnlineAsyncOpRef<FPartialUpdatePresence> Op = GetMergeableOp<FPartialUpdatePresence>(MoveTemp(InParams));

	// Reject logged-out callers before any cache side effects. This check now runs on every
	// call (not just the init path as before), which is stricter but strictly safer: we never
	// leak mutations into the local cache for an account that isn't logged in.
	if (!Services.Get<IAuth>()->IsLoggedIn(LocalAccountId))
	{
		Op->SetError(Errors::NotLoggedIn());
		return Op->GetHandle();
	}

	// Optimistic cache update + retry cancel: happens for every call, including merged calls,
	// so the local cache stays in sync with the mutations the op will send on the wire.
	CancelPresenceRetry(LocalAccountId);
	ApplyMutationsToCache(LocalAccountId, IncomingMutations);
		...
		Op->Then([this](TOnlineAsyncOp<FPartialUpdatePresence>& InAsyncOp, TPromise<const EOS_Presence_SetPresenceCallbackInfo*>&& Promise) mutable
			{
				...
				if (CreatePresenceModificationResult == EOS_EResult::EOS_Success)
				{
					...
					{
						RichPresenceString = Mutations.RichPresenceString;
					}
					else if (Mutations.StatusString.IsSet()) // RichPresenceString is the intended way to set presence in UE, but we'll keep reading StatusString for backwards compatibility
					{						
						RichPresenceString = Mutations.StatusString;
					}

					if (RichPresenceString.IsSet())
					{
						...
						const FTCHARToUTF8 Utf8RawRichText(*RichPresenceString.GetValue());
						RawRichTextOptions.RichText = Utf8RawRichText.Get();
					...
					{
						// TODO: Move this check higher.  Needs to take into account number of present fields (not just ones updated) and removed fields.
						UE_LOGF(LogOnlineServices, Warning, "UpdatePresence: Too many presence keys.  %u/%u", Mutations.UpdatedProperties.Num(), EOS_PRESENCE_DATA_MAX_KEYS);
						InAsyncOp.SetError(Errors::InvalidParams());
						Promise.EmplaceValue();
						return;
					}
					...
					if (!Records.IsEmpty())
					{
						...
						if (SetDataResult != EOS_EResult::EOS_Success)
						{
							UE_LOGF(LogOnlineServices, Warning, "PartialUpdatePresence: EOS_PresenceModification_SetData failed with result %ls", *LexToString(SetDataResult));
							InAsyncOp.SetError(Errors::FromEOSResult(SetDataResult));
							Promise.EmplaceValue();
							return;
						}
					...
					GetServices<FOnlineServicesEpicCommon>().AddEOSSDKFastTick(InAsyncOp);
			...
			{
				GetServices<FOnlineServicesEpicCommon>().RemoveEOSSDKFastTick(InAsyncOp);
				...
				else
				{
					...
					UE_LOGF(LogOnlineServices, Warning, "PartialUpdatePresence: EOS_Presence_SetPresence failed with result [%ls] for user [%ls], scheduling retry", *LexToString(Data->ResultCode), *ToLogString(Params.LocalAccountId));
...
void FPresenceEOS::ApplyMutationsToCache(FAccountId LocalAccountId, const FPartialUpdatePresence::Params::FMutations& Mutations)
{
	TSharedRef<FUserPresence> LocalUserPresence = FindOrCreatePresence(LocalAccountId, LocalAccountId);
	TSharedRef<FUserPresence> Updated = ApplyPresenceMutations(*LocalUserPresence, Mutations);
	const bool bChanged = *LocalUserPresence != *Updated;

	UE_LOGF(LogOnlineServices, VeryVerbose, "ApplyMutationsToCache: LocalAccountId=[%ls] Changed=%ls UpdatedProperties=%d RemovedProperties=%d",
		*ToLogString(LocalAccountId), *::LexToString(bChanged), Mutations.UpdatedProperties.Num(), Mutations.RemovedProperties.Num());

	if (bChanged)
	{
		*LocalUserPresence = MoveTemp(*Updated);
		FPresenceUpdated PresenceUpdatedParams = { LocalAccountId, LocalUserPresence };
		OnPresenceUpdatedEvent.Broadcast(PresenceUpdatedParams);
	}
}

void FPresenceEOS::ApplyFullPresenceToCache(FAccountId LocalAccountId, const FUserPresence& NewPresence)
{
	TSharedRef<FUserPresence> LocalUserPresence = FindOrCreatePresence(LocalAccountId, LocalAccountId);

	if (*LocalUserPresence != NewPresence)
	{
		*LocalUserPresence = NewPresence;
		FPresenceUpdated PresenceUpdatedParams = { LocalAccountId, LocalUserPresence };
		OnPresenceUpdatedEvent.Broadcast(PresenceUpdatedParams);
	}
}

void FPresenceEOS::SchedulePresenceRetry(FAccountId LocalAccountId)
{
	FPendingPresenceRetry& Retry = PendingRetries.FindOrAdd(LocalAccountId);
	Retry.bPendingRetry = true;

	if (!RetryTickerHandle.IsValid())
	{
		UE_LOGF(LogOnlineServices, Log, "PresenceEOS: Scheduling presence retry for user [%ls] in %.1f seconds", *ToLogString(LocalAccountId), PresenceRetryDelaySec);

		RetryTickerHandle = FTSTicker::GetCoreTicker().AddTicker(
			TEXT("PresenceEOSRetry"),
			static_cast<float>(PresenceRetryDelaySec),
			[WeakThis = AsWeak()](float)
			{
				if (TSharedPtr<FPresenceEOS> StrongThis = StaticCastSharedPtr<FPresenceEOS>(WeakThis.Pin()))
				{
					StrongThis->RetryTickerHandle.Reset();
					StrongThis->StartPresenceRetry();
				}
...
void FPresenceEOS::StartPresenceRetry()
{
	// Snapshot the current set of users needing retries, don't remove them.
	// Entries are only cleared when an EOS update succeeds or the user logs out.
	TArray<FAccountId> UsersToRetry;
	PendingRetries.GetKeys(UsersToRetry);

	for (const FAccountId& LocalAccountId : UsersToRetry)
	{
		// Note: This pointer is invalidated if PendingRetries is structurally modified (e.g., ClearPresenceRetry on another user).
		// Safe here because we only use it before any map removals for this iteration, and re-Find for each user.
		FPendingPresenceRetry* Retry = PendingRetries.Find(LocalAccountId);
		if (!Retry || !Retry->bPendingRetry)
		{
			continue;
		}

		Retry->RetryCount++;
		if (MaxPresenceRetries >= 0 && Retry->RetryCount > MaxPresenceRetries)
		{
			UE_LOGF(LogOnlineServices, Warning, "PresenceEOS: Max retries (%d) exceeded for presence update, dropping. User=[%ls]", MaxPresenceRetries, *ToLogString(LocalAccountId));
			ClearPresenceRetry(LocalAccountId);
			continue;
		}

		UE_LOGF(LogOnlineServices, Log, "PresenceEOS: Retrying presence update for user [%ls] (attempt %d)", *ToLogString(LocalAccountId), Retry->RetryCount);

		// Read our optimistic cache — this is the desired state we want EOS to match.
		TOnlineResult<FGetCachedPresence> CachedPresenceResult = GetCachedPresence({ LocalAccountId, LocalAccountId });
		if (!CachedPresenceResult.IsOk())
		{
			UE_LOGF(LogOnlineServices, Warning, "PresenceEOS: Could not get cached presence for retry, clearing. User=[%ls]", *ToLogString(LocalAccountId));
			ClearPresenceRetry(LocalAccountId);
			continue;
		}

		const TSharedRef<const FUserPresence>& CachedPresence = CachedPresenceResult.GetOkValue().Presence;

		// Read the EOS SDK's own presence cache to determine what it currently has stored.
		// Build a partial update (FMutations) representing the delta between EOS and our cache.
		const EOS_EpicAccountId EpicAccountId = GetEpicAccountId(LocalAccountId);
		if (EpicAccountId == nullptr)
		{
			UE_LOGF(LogOnlineServices, Warning, "PresenceEOS: No EpicAccountId for retry, clearing. User=[%ls]", *ToLogString(LocalAccountId));
			ClearPresenceRetry(LocalAccountId);
			continue;
		}
		...
		EOS_EResult CopyResult = EOS_Presence_CopyPresence(PresenceHandle, &CopyOptions, &PresenceInfo);
		if (CopyResult != EOS_EResult::EOS_Success)
		{
			UE_LOGF(LogOnlineServices, Warning, "PresenceEOS: CopyPresence failed during retry (result [%ls]), clearing. User=[%ls]", *LexToString(CopyResult), *ToLogString(LocalAccountId));
			ClearPresenceRetry(LocalAccountId);
			continue;
		}
		...
		Params.Mutations = BuildPresenceDelta(PresenceInfo, *CachedPresence);

		EOS_Presence_Info_Release(PresenceInfo);

		PartialUpdatePresence(MoveTemp(Params));
	}
}

FPartialUpdatePresence::Params::FMutations FPresenceEOS::BuildPresenceDelta(const EOS_Presence_Info* EOSPresenceInfo, const FUserPresence& DesiredPresence)
{
	FPartialUpdatePresence::Params::FMutations Mutations;

	// Status
	EUserPresenceStatus EOSStatus = ToEPresenceState(EOSPresenceInfo->Status);
	if (EOSStatus != DesiredPresence.Status)
	{
		Mutations.Status = DesiredPresence.Status;
	}

	// Rich presence string (PartialUpdatePresence uses RichPresenceString with StatusString as fallback for SetRawRichText)
	FString EOSRichText = UTF8_TO_TCHAR(EOSPresenceInfo->RichText);
	if (EOSRichText != DesiredPresence.RichPresenceString)
	{
		Mutations.RichPresenceString = DesiredPresence.RichPresenceString;
		Mutations.StatusString = DesiredPresence.StatusString;
	}

	// Properties: keys in EOS but not in desired state → removals
	if (EOSPresenceInfo->Records)
	{
		TArrayView<const EOS_Presence_DataRecord> Records(EOSPresenceInfo->Records, EOSPresenceInfo->RecordsCount);
		for (const EOS_Presence_DataRecord& Record : Records)
		{
			FString RecordKey = UTF8_TO_TCHAR(Record.Key);
			if (!DesiredPresence.Properties.Contains(RecordKey))
			{
				Mutations.RemovedProperties.Add(MoveTemp(RecordKey));
			}
		}
	}

	// Properties: send all keys from the desired state, not just the ones that differ.
	// A value-level diff would require parsing EOS record strings back into variants and
	// comparing each type, which isn't worth the complexity for a retry path that fires rarely.
	Mutations.UpdatedProperties = DesiredPresence.Properties;

	return Mutations;
}

void FPresenceEOS::CancelPresenceRetry(FAccountId LocalAccountId)
{
	// Only clear bPendingRetry, not RetryCount.
	// The retry path also calls PartialUpdatePresence, which calls this method. Resetting the count here would create an infinite retry loop.
	if (FPendingPresenceRetry* Retry = PendingRetries.Find(LocalAccountId))
	{
		Retry->bPendingRetry = false;
	}
}

void FPresenceEOS::ClearPresenceRetry(FAccountId LocalAccountId)
{
	PendingRetries.Remove(LocalAccountId);
}

EOnlinePlatformType FPresenceEOS::IntegratedPlatform_To_OnlinePlatformType(const FString& IntegratedPlatformStr)
{
...
bool FPresenceEOS::ReadOnlinePlatformType(const EOS_Presence_Info* PresenceInfo, EOnlinePlatformType& OutOnlinePlatformType)
{
	...
	if (!IntegratedPlatformStr.IsEmpty())
	{
		...
		if (OutOnlinePlatformType != OnlinePlatformType)
		{
			OutOnlinePlatformType = OnlinePlatformType;
...
void FPresenceEOS::UpdateUserPresence(FAccountId LocalAccountId, FAccountId PresenceAccountId)
{
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "[%s] No EpicAccountId found for FAccountId [%ls]", __FUNCTION__, *ToLogString(LocalAccountId));
		return;
	}
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "[%s] No EpicAccountId found for FAccountId [%ls]", __FUNCTION__, *ToLogString(PresenceAccountId));
		return;
	}

	TSharedRef<FUserPresence> UserPresence = FindOrCreatePresence(LocalAccountId, PresenceAccountId);

	// We remove the SentTime attribute before the comparison
	UserPresence->Properties.Remove(SENT_TIME_ATTRIBUTE_KEY);

	// Snapshot the current state so we can detect changes at the end via operator==.
	const FUserPresence OldPresence = *UserPresence;
	bool bPresenceChanged = false;
	...
	if (CopyPresenceResult == EOS_EResult::EOS_Success)
	{
		...
		UserPresence->Status = ToEPresenceState(PresenceInfo->Status);

		FString NewRichPresenceString = UTF8_TO_TCHAR(PresenceInfo->RichText);
		
		// RichPresenceString is the intended way to set presence in UE, but we'll keep populating StatusString for backwards compatibility
		UserPresence->StatusString = NewRichPresenceString;
		UserPresence->RichPresenceString = MoveTemp(NewRichPresenceString);

		const FString ProductIdString = UTF8_TO_TCHAR(PresenceInfo->ProductId);
		if (IEOSSDKManager* Manager = IEOSSDKManager::Get())
		{
			...
			{
				const FString PlatformConfigProductIdString = Config->ProductId;
		...
		if (PresenceInfo->Records)
		{
			...
			for (const EOS_Presence_DataRecord& Record : Records)
			{
				...
				if (RecordValueResult.HasValue())
				{
					...
					UserPresence->Properties.Add(MoveTemp(RecordKey), RecordValueResult.GetValue());
				}
				else
				{
					UE_LOGF(LogOnlineServices, Warning, "UpdateUserPresence: Unsupported data type passed for presence attribute [%ls]", *RecordKey);
				}
			}
		...
		// We update the Platform attribute
		if (PresenceInfo->Platform)
		{
			UserPresence->Properties.AddVariant(PLATFORM_ATTRIBUTE_KEY, FString(UTF8_TO_TCHAR(PresenceInfo->Platform)));
		}
		else
		{
			UserPresence->Properties.Remove(PLATFORM_ATTRIBUTE_KEY);
		}

		ReadOnlinePlatformType(PresenceInfo, UserPresence->PlatformType);

		bPresenceChanged = *UserPresence != OldPresence;

		EOS_Presence_Info_Release(PresenceInfo);
	}
	else
	{
		UE_LOGF(LogOnlineServices, Warning, "UpdateUserPresence: CopyPresence failed with result: %ls", *LexToString(CopyPresenceResult));
	}

	// We set the SentTime attribute to the current time. Pending access to the actual sent time via EOSSDK
	// We set this after the comparison because it will always differ
	UserPresence->Properties.AddVariant<int64>(SENT_TIME_ATTRIBUTE_KEY, FDateTime::UtcNow().GetTicks());
...
void FPresenceEOS::HandlePresenceChanged(const EOS_Presence_PresenceChangedCallbackInfo* Data)
{
		...
		.Next([WeakThis = AsWeak(), LocalAccountId, Func = __FUNCTION__](const FAccountId& PresenceAccountId)
			{
				if (TSharedPtr<FPresenceEOS> StrongThis = StaticCastSharedPtr<FPresenceEOS>(WeakThis.Pin()))
				{
					const bool bIsLocalUser = (PresenceAccountId == LocalAccountId);
					
					UE_LOGF(LogOnlineServices, Verbose, "%s LocalAccountId=[%ls] PresenceAccountId=[%ls]%s", Func, *ToLogString(LocalAccountId), *ToLogString(PresenceAccountId), bIsLocalUser ? " (skipping echo)" : "");

					// Skip processing EOS notifications for the local user's own presence.
					// The local cache is updated optimistically in PartialUpdatePresence/UpdatePresence,
					// and the EOS echo-back notification could overwrite newer optimistic state.
					if (!bIsLocalUser)
					{
						StrongThis->UpdateUserPresence(LocalAccountId, PresenceAccountId);
					}
	...
	{
		UE_LOGF(LogOnlineServices, Verbose, "%s Account id not found for Epic id [%ls]. Will retry after login completes]", __FUNCTION__, *LexToString(Data->LocalUserId));
