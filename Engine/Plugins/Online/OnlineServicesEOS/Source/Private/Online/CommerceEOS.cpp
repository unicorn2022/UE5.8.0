...
TOnlineAsyncOpHandle<FCommerceCheckout> FCommerceEOS::Checkout(FCommerceCheckout::Params&& Params)
{
	...
	.Then([this](TOnlineAsyncOp<FCommerceCheckout>& Op, const EOS_Ecom_CheckoutCallbackInfo* Data)
	{
		...
		if (Result != EOS_EResult::EOS_Success)
		{
			...
			{
				UE_LOGF(LogOnlineServices, Log, "EOS_Ecom_Checkout was canceled by user.");
			}
			else if (Result == EOS_EResult::EOS_Ecom_PurchaseProcessing)
			{
				...
				UE_LOGF(LogOnlineServices, Log, "QueryEntitlements will need to be called to confirm the purchase. This can be done via a UI button.");
			}
			else
			{
				UE_LOGF(LogOnlineServices, Error, "EOS_Ecom_Checkout: failed with error (%ls)", *LexToString(Data->ResultCode));
			}
			Op.SetError(Errors::FromEOSResult(Result));
			return;
		}
...
TOnlineAsyncOpHandle<FCommerceQueryTransactionEntitlements> FCommerceEOS::QueryTransactionEntitlements(FCommerceQueryTransactionEntitlements::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		Op->Then([this](TOnlineAsyncOp<FCommerceQueryTransactionEntitlements>& Op)
		{
			...
			{
				UE_LOGF(LogOnlineServices, Error, "EOS_Ecom_CopyTransactionById: failed with error (%ls)", *LexToString(CopyTransactionResult));
...
TOnlineAsyncOpHandle<FCommerceQueryEntitlements> FCommerceEOS::QueryEntitlements(FCommerceQueryEntitlements::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FCommerceQueryEntitlements>& Op, const EOS_Ecom_QueryEntitlementsCallbackInfo* Data)
		{
			...
			if (Result != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogOnlineServices, Error, "EOS_Ecom_QueryEntitlements: failed with error (%ls)", *LexToString(Data->ResultCode));
				Op.SetError(Errors::FromEOSResult(Result));
				return;
			}
			...
			UE_LOGF(LogOnlineServices, VeryVerbose, "Entitlements Count: %d", Count);
			...
			for (uint32 Index = 0; Index < Count; Index++)
			{
				...
				UE_LOGF(LogOnlineServices, VeryVerbose, "	Entitlement %d:", Index);
				...
				{
					UE_LOGF(LogOnlineServices, Error, "EOS_Ecom_CopyEntitlementByIndex: failed with error (%ls) (proceeding with operation)", *LexToString(CopyResult));
					continue;
				}
				...
				UE_LOGF(LogOnlineServices, VeryVerbose, "		Entitlement Id: %ls", *Entitlement.EntitlementId);
				UE_LOGF(LogOnlineServices, VeryVerbose, "		Product Id: %ls", *Entitlement.ProductId);
		...
		.Then([this](TOnlineAsyncOp<FCommerceQueryEntitlements>& Op, const EOS_Ecom_QueryOwnershipBySandboxIdsCallbackInfo* Data)
		{
			...
			if (Result != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogOnlineServices, Error, "EOS_Ecom_QueryOwnership: failed with error (%ls)", *LexToString(Data->ResultCode));
				Op.SetError(Errors::FromEOSResult(Result));
				return;
			}
			...
			UE_LOGF(LogOnlineServices, VeryVerbose, "Sandbox Id Ownership Count: %d", Data->SandboxIdItemOwnershipsCount);
			...
			{
				UE_LOGF(LogOnlineServices, VeryVerbose, "	Sandbox Id: %s", SandboxOwnershipEntry.SandboxId);
				UE_LOGF(LogOnlineServices, VeryVerbose, "	Ownership Count: %d", SandboxOwnershipEntry.OwnedCatalogItemIdsCount);
				...
				for (uint32_t CatalogItemIndex = 0; CatalogItemIndex < SandboxOwnershipEntry.OwnedCatalogItemIdsCount; CatalogItemIndex++)
				{
					...
					UE_LOGF(LogOnlineServices, VeryVerbose, "		Catalog Item %d", CatalogItemIndex);
					UE_LOGF(LogOnlineServices, VeryVerbose, "			Catalog Item Id: %s", CatalogItemId);
				}
			}
...
TOnlineAsyncOpHandle<FCommerceRedeemEntitlement> FCommerceEOS::RedeemEntitlement(FCommerceRedeemEntitlement::Params&& Params)
{
	...
	.Then([this](TOnlineAsyncOp<FCommerceRedeemEntitlement>& Op, const EOS_Ecom_RedeemEntitlementsCallbackInfo* Data)
	{
		...
		if (Result != EOS_EResult::EOS_Success)
		{
			UE_LOGF(LogOnlineServices, Error, "EOS_Ecom_RedeemEntitlements: failed with error (%ls)", *LexToString(Data->ResultCode));
			Op.SetError(Errors::FromEOSResult(Result));
			return;
		}
		Op.SetResult({});
...
TOnlineAsyncOpHandle<FCommerceRetrieveS2SToken> FCommerceEOS::RetrieveS2SToken(FCommerceRetrieveS2SToken::Params&& Params)
{
	...
	if (!Op->IsReady())
	{
		...
		.Then([this](TOnlineAsyncOp<FCommerceRetrieveS2SToken>& Op, const EOS_Ecom_QueryOwnershipTokenCallbackInfo* Data)
		{
			...
			if (Result != EOS_EResult::EOS_Success)
			{
				UE_LOGF(LogOnlineServices, Error, "EOS_Ecom_QueryOwnershipToken: failed with error (%ls)", *LexToString(Data->ResultCode));
				Op.SetError(Errors::FromEOSResult(Result));
				return;
			}
