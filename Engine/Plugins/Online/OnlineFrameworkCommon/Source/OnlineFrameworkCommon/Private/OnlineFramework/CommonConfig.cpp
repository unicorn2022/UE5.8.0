...
FCommonConfig::FCommonConfig(const UObject* ContextObject)
{
	...
	if (ContextObject)
	{
		if (const UWorld* const World = ContextObject->GetWorld())
		{
			if (World->IsPlayInEditor())
			{
				...
				else
				{
					...
					UE_LOGF(LogOnlineFrameworkCommon, Warning, "[FCommonFrameworkContext] PIE world without a world context");
		...
		else
		{
			...
			UE_LOGF(LogOnlineFrameworkCommon, Warning, "[FCommonFrameworkContext] Editor context without a world");
			ContextType = ECommonConfigContextType::Editor;
		}
	}
	else
