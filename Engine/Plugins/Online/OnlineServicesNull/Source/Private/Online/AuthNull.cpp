...
void FAccountInfoRegistryNULL::Unregister(FAccountId AccountId)
{
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "[FAccountInfoRegistryNULL::Unregister] Failed to find account [%ls].", *ToLogString(AccountId));
	}
}
...
#if WITH_APPLICATION_CORE // for IPlatformInputDeviceMapper
...
TArray<uint8> FOnlineAccountIdRegistryNull::ToReplicationData(const FAccountId& AccountId) const
{
	...
	if (Registry.ValidateOnlineId(AccountId))
	{
		...
		UE_LOGF(LogOnlineServices, VeryVerbose, "[FOnlineAccountIdRegistryNull::ToReplicationData] StringToBytes on [%ls] returned %d len", *AccountIdString, ReplicationData.Num())
	}
