	...
	: Data(MakeShared<FLobby>())
{
	...
	// Guard SetPort with IsPortValid: GetPortFromNetDriver can return out-of-range values for non-IP addresses (e.g. EOS:<PUID>).
	const int32 NetDriverPort = GetPortFromNetDriver(NAME_None);
	if (HostAddr->IsPortValid(NetDriverPort))
	{
		HostAddr->SetPort(NetDriverPort);
	}
...
{
	UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::FindLobbies");
...
TOnlineAsyncOpHandle<FJoinLobby> FLobbiesNull::JoinLobby(FJoinLobby::Params&& Params)
{
	...
	else
	{
		if (!Lobby.IsValid())
		{
			UE_LOGF(LogOnlineServices, Warning, "Failed to find a lobby with ID %d", Op->GetParams().LobbyId.GetHandle())
		}
		else
		{
			UE_LOGF(LogOnlineServices, Warning, "Lobby with name %ls already exists", *LobbyName.ToString());
		}
...
{
	UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::FindLANSession");
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "FLobbiesNull::FindLANSession: Failed, returning");
		Return = ONLINE_FAIL;
	...
	{
		UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::FindLANSession: searching....");
	}
...
{
	UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::UpdateLANStatus");
	...
	if ( NeedsToAdvertise() )
	{
		...
		{
			UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::UpdateLANStatus- Hosting..");
			...
			{
				UE_LOGF(LogOnlineServices, Warning, "FLobbiesNull::Failed to host!");
				Result = ONLINE_FAIL;
...
void FLobbiesNull::AppendLobbyToPacket(FNboSerializeToBuffer& Packet, const TSharedRef<FLobbyNull>& Lobby)
{
	...
	UE_LOGF(LogOnlineServices, Verbose, "AppendLobbyToPacket: Appending IP %ls", *Lobby->HostAddr->ToString(true));
...
{
	UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::OnValidQueryPacketReceived");
	...
	for (TPair<FName, TSharedRef<FLobbyNull>>& Pair : NamedLobbies)
	{
		...
		{
			UE_LOGF(LogOnlineServices, Warning, "LAN broadcast packet overflow, cannot broadcast on LAN");
		}
	}
}
...
void FLobbiesNull::ReadLobbyFromPacket(FNboSerializeFromBuffer& Packet, const TSharedRef<FLobbyNull>& Session)
{
	...
	UE_LOGF(LogOnlineServices, Verbose, "Reading session information from server");
	...
	UE_LOGF(LogOnlineServices, Verbose, "ReadLobby: Got IP %ls", *Session->HostAddr->ToString(true));
...
{
	UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::OnValidResponsePacketReceived");
	if (CurrentLobbySearch.IsValid())
	{
	...
	{
		UE_LOGF(LogOnlineServices, Warning, "Failed to create new online game settings object");
	}
}

uint32 FLobbiesNull::FinalizeLANSearch()
{
	UE_LOGF(LogOnlineServices, Verbose, "FLobbiesNull::FinalizeLANSearch");
	if (LANSessionManager.GetBeaconState() == ELanBeaconState::Searching)
	{
		LANSessionManager.StopLANSession();
	}
...
{
	UE_LOGF(LogOnlineServices, Warning, "FLobbiesNull::OnLANSearchTimeout");
	FinalizeLANSearch();
	...
	if (CurrentLobbySearch.IsValid())
	{
		...
		{
			UE_LOGF(LogOnlineServices, Warning, "FLobbiesNull::OnLANSearchTimeout: Had results!");
	...
	UE_LOGF(LogOnlineServices, Warning, "FLobbiesNull::OnLANSearchTimeout: no results");
