	...
	, LastSocketError(ESocketErrors::SE_NO_ERROR)
{
#if WITH_EOS_P2P
	bool bEnable = true;
	GConfig->GetBool(TEXT("SocketSubsystemEOS"), TEXT("bEnable"), bEnable, GEngineIni);
	...
	if (bEnable)
	{
		...
		{
			UE_LOGF(LogSocketSubsystemEOS, Error, "FOnlineSubsystemEOS: failed to init EOS platform, couldn't get p2p handle");
		}
	}
#endif // WITH_EOS_P2P
}
...
bool FSocketSubsystemEOS::Init(FString& Error)
{
	...
	if (P2PHandle)
	{
		...
		if (!RelayControlStr.IsEmpty())
		{
			...
			{
				UE_LOGF(LogSocketSubsystemEOS, Warning, "[FSocketSubsystemEOS::Init] EOS_P2P_SetRelayControl failed with result [%ls]", *LexToString(Result));
			}
		}
	}
#endif // WITH_EOS_P2P
...
TSharedRef<FInternetAddr> FSocketSubsystemEOS::GetLocalBindAddr(const UWorld* const OwningWorld, FOutputDevice& Out)
{
	...
	{
		UE_LOGF(LogSocketSubsystemEOS, Error, "No local user set in LocalBindAddr");
	}
#endif // WITH_EOS_P2P
