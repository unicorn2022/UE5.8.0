...
{
	// FORT-1067735 discount-offer wire-format constants.
	// Mirror EOnlineStoreOfferDiscountType in
	// Engine/Plugins/Online/OnlineSubsystem/Source/Public/Interfaces/OnlineStoreInterfaceV2.h.
	// Java enums don't cross JNI cleanly, so we transmit raw int ordinals across the boundary.
	private static final int DISCOUNT_TYPE_NOT_ON_SALE = 0;
	private static final int DISCOUNT_TYPE_PAY_AMOUNT = 3;
	// Number of leading offer-token characters logged in [FORT-1067735] traces. The full token is
	// long and opaque; the prefix is sufficient to correlate query and purchase log lines.
	private static final int OFFER_TOKEN_LOG_PREVIEW_LEN = 8;

	// Our IAB helper interface provided by google.
	private final BillingClient Client;
	
	// The IAP Billing config.
	private BillingConfig ClientBillingConfig;

	// Flag that determines whether the store is ready to use.
	private boolean bIsIapSetup = false;

	// Flag of if alternative billing only was set up
	private boolean bIsAlternativeBillingOnlyActive = false;

	// Cached result of isAlternativeBillingOnlyAvailableAsync
	private boolean bIsAlternativeBillingOnlyAvailable = false;
	...
	public GooglePlayStoreHelper(GameActivity InGameActivity, Logger InLog, boolean InEnableAlternativeBillingOnly)
	{
		Log = InLog;
		...
		BillingClient.Builder BillingClientBuilder = BillingClient.newBuilder(MainActivity)
			.setListener(this)
			.enablePendingPurchases(PendingPurchases)
			.enableAutoServiceReconnection();
		
		if (InEnableAlternativeBillingOnly)
		{
			bIsAlternativeBillingOnlyActive = true;
			BillingClientBuilder.enableAlternativeBillingOnly();
		}
		
		Client = BillingClientBuilder
			.build();
		...
		Client.startConnection(new BillingClientStateListener() {
			...
			public void onBillingSetupFinished(@NonNull BillingResult billingResult)
			{
				if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK)
				{
					...
					Client.getBillingConfigAsync(GetBillingConfigParams.newBuilder().build(), new BillingConfigResponseListener()
					{
						@Override
						public void onBillingConfigResponse(BillingResult billingResult, BillingConfig billingConfig)
						{
							...
							{
								Log.debug("[GooglePlayStoreHelper] - Billing Config is now valid");
								ClientBillingConfig = billingConfig;
							}
						}
					});

					if (bIsAlternativeBillingOnlyActive)
					{
						Client.isAlternativeBillingOnlyAvailableAsync(new AlternativeBillingOnlyAvailabilityListener() {
							@Override
							public void onAlternativeBillingOnlyAvailabilityResponse(
								BillingResult billingResult) 
								{
									bIsAlternativeBillingOnlyAvailable = billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK;
									Log.debug("[GooglePlayStoreHelper] - bIsAlternativeBillingOnlyAvailable: " + bIsAlternativeBillingOnlyAvailable + " error " + TranslateBillingResult(billingResult));
								}
						});
					}
				}
	...
	{
		Log.debug("[GooglePlayStoreHelper] - GetBillingCountryCode");
		if (ClientBillingConfig != null)
		{
			return ClientBillingConfig.getCountryCode();
		}
		Log.debug("[GooglePlayStoreHelper] - ClientBillingConfig is invalid. Can't get the country code.");
		return "";
	}

	@Override
	public boolean GetIsAlternativeBillingOnlyAvailable()
	{
		Log.debug("[GooglePlayStoreHelper] - GetIsAlternativeBillingOnlyAvailable");
		
		return bIsAlternativeBillingOnlyAvailable;
	}

	@Override
	public boolean ShowAlternativeBillingOnlyInformationDialog()
	{
		if (!bIsAlternativeBillingOnlyActive)
		{
			return true;
		...
		AlternativeBillingOnlyInformationDialogListener listener =
			new AlternativeBillingOnlyInformationDialogListener() {
				...
				public void onAlternativeBillingOnlyInformationDialogResponse(
					BillingResult billingResult) {
						Log.debug("[GooglePlayStoreHelper] - NativeAlternativeBillingOnlyInformationDialogComplete");
						NativeAlternativeBillingOnlyInformationDialogComplete(billingResult.getResponseCode());
					}
			};

		Log.debug("[GooglePlayStoreHelper] - showAlternativeBillingOnlyInformationDialog");
		BillingResult billingResult =
			Client.showAlternativeBillingOnlyInformationDialog(MainActivity,
				listener);

		Log.debug("[GooglePlayStoreHelper] - showAlternativeBillingOnlyInformationDialog return " + TranslateBillingResult(billingResult));
	...
	public boolean QueryInAppPurchases(String[] InProductIDs)
	{
		...
		if (InProductIDs.length > 0)
		{
			...
			ProductDetailsResponseListener AggregatedListener = new ProductDetailsResponseListener() {
				...
				@Override
				public synchronized void onProductDetailsResponse(@NonNull BillingResult Result, @NonNull QueryProductDetailsResult ProductDetailsList)
				{
					...
					{
						AllProductDetails.addAll(ProductDetailsList.getProductDetailsList());
						AwaitingResponseCount -= 1;
						Log.debug("[GooglePlayStoreHelper] - QueryInAppPurchases - Received new " + ProductDetailsList.getProductDetailsList().size() + " products. Waiting for additional responses: " + (AwaitingResponseCount > 0));
						if (AwaitingResponseCount == 0)
						{
							...
							NativeQueryComplete(BillingClient.BillingResponseCode.OK, DetailArrays.ProductIds, DetailArrays.Titles, DetailArrays.Descriptions, DetailArrays.Prices, DetailArrays.PricesRaw, DetailArrays.CurrencyCodes, DetailArrays.RegularPrices, DetailArrays.RegularPricesRaw, DetailArrays.DiscountTypes);
						}
					}
					else
					{
						...
						NativeQueryComplete(ResponseCode, null, null, null, null, null, null, null, null, null);
					}
	...
	public boolean BeginPurchase(final String[] ProductIds, final String ObfuscatedAccountId)
	{
		...
		Client.queryProductDetailsAsync(Params, new ProductDetailsResponseListener() {
			@Override
			public void onProductDetailsResponse(@NonNull BillingResult Result, @NonNull QueryProductDetailsResult ProductDetailsList)
			{
				if(Result.getResponseCode() == BillingClient.BillingResponseCode.OK)
				{
					Log.debug("[GooglePlayStoreHelper] - BeginPurchase - CreateBillingFlowParams with " + ProductDetailsList.getProductDetailsList().stream().map(ProductDetails::getName).collect(Collectors.joining(",")));
					final BillingFlowParams Params = CreateBillingFlowParams(ProductIds, ProductDetailsList.getProductDetailsList(), ObfuscatedAccountId);
					if(Params == null)
					{
					...
					MainActivity.runOnUiThread(new Runnable()
					{
						@Override
						public void run() {
							Log.debug("[GooglePlayStoreHelper] - BeginPurchase - launchBillingFlow " + ProductDetailsList.getProductDetailsList().stream().map(ProductDetails::getName).collect(Collectors.joining(",")));
						...
						else
						{
							...
							Client.queryProductDetailsAsync(QuerySubs, new ProductDetailsResponseListener() {
								...
								public void onProductDetailsResponse(@NonNull BillingResult Result, @NonNull QueryProductDetailsResult ProductDetailsList)
								{
									...
									if (ResponseCode == BillingClient.BillingResponseCode.OK)
									{
										Set<String> SubscriptionProductIds = GetSubscriptionStoreProductIds(ProductDetailsList.getProductDetailsList());
	...
	/**
	 * Result of selecting the relevant entries from ProductDetails.getOneTimePurchaseOfferDetailsList()
	 * for a multi-offer one-time product. Either or both fields may be null:
	 *   DefaultOffer == null  &&  DiscountOffer == null  -> the product had no list (legacy single-offer config)
	 *   DefaultOffer != null  &&  DiscountOffer == null  -> multi-offer product with no active discount
	 *   DefaultOffer != null  &&  DiscountOffer != null  -> multi-offer product with discount (FORT-1067735 case)
	 *   DefaultOffer == null  &&  DiscountOffer != null  -> defensive: list contained only discount entries, no default
	 */
	static class OneTimeOfferSelection
	{
		ProductDetails.OneTimePurchaseOfferDetails DefaultOffer;   // first entry where getOfferId() == null
		ProductDetails.OneTimePurchaseOfferDetails DiscountOffer;  // first entry where getOfferId() != null
	}

	/**
	 * Select the default and (optionally) discount one-time-purchase offers for a product.
	 *
	 * Per Google Play Billing Library v6+ conventions:
	 *   - The entry with offerId == null is the default/full-priced offer.
	 *   - Entries with non-null offerId are discount/preorder/rental offers.
	 *
	 * This helper is the single source of truth for "which offer represents the regular price"
	 * vs "which offer we charge for". Used by both ListOfProductDetailsAsArrays (query path)
	 * and EncodedProductId.CreateProductDetailsParams (purchase path) so the advertised price
	 * and the charged price cannot drift.
	 *
	 * Returns a selection whose fields are both null when getOneTimePurchaseOfferDetailsList()
	 * returns null (legacy single-offer product). Callers should fall back to the deprecated
	 * singular getOneTimePurchaseOfferDetails() in that case.
	 */
	static OneTimeOfferSelection SelectOneTimeOffers(ProductDetails Product)
	{
		OneTimeOfferSelection Result = new OneTimeOfferSelection();
		List<ProductDetails.OneTimePurchaseOfferDetails> OfferList = Product.getOneTimePurchaseOfferDetailsList();
		if (OfferList == null || OfferList.isEmpty())
		{
			return Result;
		}
		for (ProductDetails.OneTimePurchaseOfferDetails Offer : OfferList)
		{
			if (Offer.getOfferId() == null && Result.DefaultOffer == null)
			{
				Result.DefaultOffer = Offer;
			}
			else if (Offer.getOfferId() != null && Result.DiscountOffer == null)
			{
				Result.DiscountOffer = Offer;
			}
			if (Result.DefaultOffer != null && Result.DiscountOffer != null)
			{
				break;
			}
		}
	...
	class ListOfProductDetailsAsArrays
	{
		...
		// FORT-1067735 discount-offer fields. RegularPrices[i] is "" / RegularPricesRaw[i] is -1 /
		// DiscountTypes[i] is 0 (NotOnSale) when product[i] is not on sale.
		public String[] RegularPrices;
		public long[] RegularPricesRaw;
		public int[] DiscountTypes;
		...
		public ListOfProductDetailsAsArrays(List<ProductDetails> InProductDetailsList)
		{
			...
			RegularPrices = new String[TotalEntryCount];
			RegularPricesRaw = new long[TotalEntryCount];
			DiscountTypes = new int[TotalEntryCount];
			...
			for (ProductDetails ThisProduct : InProductDetailsList)
			{
				...
				switch(ThisProduct.getProductType())
				{
					case BillingClient.ProductType.INAPP:
					{
						// FORT-1067735: prefer the multi-offer list (getOneTimePurchaseOfferDetailsList) when present
						// so we can apply Play Console discount offers and surface strikethrough pricing to the engine.
						// Fall back to the singular getOneTimePurchaseOfferDetails() for legacy single-offer products.
						OneTimeOfferSelection Selection = SelectOneTimeOffers(ThisProduct);

						ProductIds[Index] = ThisProduct.getProductId();
						// Default discount sentinels — overwritten below in the default+discount case.
						RegularPrices[Index] = "";
						RegularPricesRaw[Index] = -1;
						DiscountTypes[Index] = DISCOUNT_TYPE_NOT_ON_SALE;

						if (Selection.DefaultOffer != null && Selection.DiscountOffer != null)
						{
							// Multi-offer product with active discount: advertise discount as current, default as regular.
							Prices[Index] = Selection.DiscountOffer.getFormattedPrice();
							PricesRaw[Index] = Selection.DiscountOffer.getPriceAmountMicros();
							CurrencyCodes[Index] = Selection.DiscountOffer.getPriceCurrencyCode();
							RegularPrices[Index] = Selection.DefaultOffer.getFormattedPrice();
							RegularPricesRaw[Index] = Selection.DefaultOffer.getPriceAmountMicros();
							DiscountTypes[Index] = DISCOUNT_TYPE_PAY_AMOUNT;
							String OfferTokenPreview = Selection.DiscountOffer.getOfferToken();
							OfferTokenPreview = (OfferTokenPreview != null && OfferTokenPreview.length() > OFFER_TOKEN_LOG_PREVIEW_LEN) ? OfferTokenPreview.substring(0, OFFER_TOKEN_LOG_PREVIEW_LEN) + "..." : OfferTokenPreview;
							Log.debug("[GooglePlayStoreHelper] - multi-offer with discount for " + ThisProduct.getProductId() + ": default=" + Selection.DefaultOffer.getFormattedPrice() + " discount=" + Selection.DiscountOffer.getFormattedPrice() + " offerToken=" + OfferTokenPreview);
						}
						else if (Selection.DefaultOffer != null)
						{
							// Multi-offer product, no active discount.
							Prices[Index] = Selection.DefaultOffer.getFormattedPrice();
							PricesRaw[Index] = Selection.DefaultOffer.getPriceAmountMicros();
							CurrencyCodes[Index] = Selection.DefaultOffer.getPriceCurrencyCode();
							Log.debug("[GooglePlayStoreHelper] - multi-offer default-only for " + ThisProduct.getProductId() + ": price=" + Selection.DefaultOffer.getFormattedPrice());
						}
						else if (Selection.DiscountOffer != null)
						{
							// Defensive: list contained only non-null-offerId entries with no default. No strikethrough baseline.
							Prices[Index] = Selection.DiscountOffer.getFormattedPrice();
							PricesRaw[Index] = Selection.DiscountOffer.getPriceAmountMicros();
							CurrencyCodes[Index] = Selection.DiscountOffer.getPriceCurrencyCode();
							Log.debug("[GooglePlayStoreHelper] - multi-offer discount-only (no default present) for " + ThisProduct.getProductId() + ": price=" + Selection.DiscountOffer.getFormattedPrice());
						}
						else
						{
							// Legacy product: getOneTimePurchaseOfferDetailsList() returned null. Use the singular getter.
							ProductDetails.OneTimePurchaseOfferDetails PriceDetails = ThisProduct.getOneTimePurchaseOfferDetails();
							Log.debug("[GooglePlayStoreHelper] - legacy single-offer for " + ThisProduct.getProductId() + " price: " + PriceDetails.getFormattedPrice());
					...
					case BillingClient.ProductType.SUBS:
						for (ProductDetails.SubscriptionOfferDetails SubsOfferDetails: ThisProduct.getSubscriptionOfferDetails())
						{
							...
							for( ProductDetails.PricingPhase PricingPhase: SubsOfferDetails.getPricingPhases().getPricingPhaseList())
							{
								...
								// FORT-1067735 sentinels: subscriptions are out of scope for discount-offer handling.
								RegularPrices[Index] = "";
								RegularPricesRaw[Index] = -1;
								DiscountTypes[Index] = DISCOUNT_TYPE_NOT_ON_SALE;
	...
	static class EncodedProductId
	{
		...
		public BillingFlowParams.ProductDetailsParams CreateProductDetailsParams(List<ProductDetails> InProductDetails)
		{
			...
			{
				// FORT-1067735: For multi-offer one-time products, select the discount offer (or fall back to the default)
				// and pass its OfferToken to BillingFlowParams. Without setOfferToken, Google Play defaults to the full-priced
				// offer regardless of what the UI advertised. For legacy single-offer products (list is null/empty),
				// SelectOneTimeOffers returns both fields null and we build the params without setOfferToken — today's behavior.
				return InProductDetails.stream()
					.filter(Details -> Details.getProductId().equals(StoreProductId) && Details.getProductType().equals(BillingClient.ProductType.INAPP))
					.findFirst()
					.map(Details -> {
						OneTimeOfferSelection Sel = SelectOneTimeOffers(Details);
						ProductDetails.OneTimePurchaseOfferDetails Chosen = (Sel.DiscountOffer != null) ? Sel.DiscountOffer : Sel.DefaultOffer;
						...
						if (Chosen != null)
						{
							String OfferTokenPreview = Chosen.getOfferToken();
							OfferTokenPreview = (OfferTokenPreview != null && OfferTokenPreview.length() > OFFER_TOKEN_LOG_PREVIEW_LEN) ? OfferTokenPreview.substring(0, OFFER_TOKEN_LOG_PREVIEW_LEN) + "..." : OfferTokenPreview;
							// Log.debug not available on the static EncodedProductId class — caller's BeginPurchase already logs the product id.
							android.util.Log.d("GooglePlayStoreHelper", "[FORT-1067735] applying offer token for " + StoreProductId + " token=" + OfferTokenPreview);
							Builder.setOfferToken(Chosen.getOfferToken());
						}
	...
	public native void NativeQueryComplete(int ResponseCode, String[] ProductIds, String[] Titles, String[] Descriptions, String[] Prices, long[] PricesRaw, String[] CurrencyCodes, String[] RegularPrices, long[] RegularPricesRaw, int[] DiscountTypes);
	...
	public native void NativeAlternativeBillingOnlyInformationDialogComplete(int ResponseCode);
}
