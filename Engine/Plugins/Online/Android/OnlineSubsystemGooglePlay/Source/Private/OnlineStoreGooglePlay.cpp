...
{
	/**
	 * Convert Google Play's micro-unit price (1,000,000 = 1 currency unit) to the engine's
	 * minor-unit integer price (e.g., cents for USD) per the active culture's currency formatting rules.
	 * Used identically for both NumericPrice and RegularPrice so the two cannot drift in rounding.
	 */
	inline int64 ConvertMicrosToMinorUnits(jlong PriceMicros, const FNumberFormattingOptions& FormattingOptions)
	{
		// Value provided by Google Billing Library is in micro units. Divide by 10^6 and multiply by
		// 10^FormattingOptions.MaximumFractionalDigits to match the engine's minor-unit representation.
		const double Val = static_cast<double>(PriceMicros) * static_cast<double>(FMath::Pow(10.0f, FormattingOptions.MaximumFractionalDigits - 6));
		return FMath::TruncToInt(Val + 0.5);
	}
}

void JNICALL UE::Jni::FGooglePlayStoreHelper::NativeQueryComplete(JNIEnv* jenv, jobject /*Thiz*/, jint ResponseCode, Java::Lang::TArray<jstring>* ProductIDs, Java::Lang::TArray<jstring>* Titles, Java::Lang::TArray<jstring>* Descriptions, Java::Lang::TArray<jstring>* Prices, Java::Lang::TArray<jlong>* PriceValuesRaw, Java::Lang::TArray<jstring>* CurrencyCodes, Java::Lang::TArray<jstring>* RegularPrices, Java::Lang::TArray<jlong>* RegularPriceValuesRaw, Java::Lang::TArray<jint>* DiscountTypes)
{
	...
	if (jenv && bWasSuccessful)
	{
		...
		jsize NumRegularPrices = jenv->GetArrayLength(RegularPrices);
		jsize NumRegularPricesRaw = jenv->GetArrayLength(RegularPriceValuesRaw);
		jsize NumDiscountTypes = jenv->GetArrayLength(DiscountTypes);

		ensure((NumProducts == NumTitles) && (NumProducts == NumDescriptions) && (NumProducts == NumPrices) && (NumProducts == NumPricesRaw) && (NumProducts == NumCurrencyCodes) && (NumProducts == NumRegularPrices) && (NumProducts == NumRegularPricesRaw) && (NumProducts == NumDiscountTypes));

		jlong* PriceValues = jenv->GetLongArrayElements(PriceValuesRaw, 0);
		jlong* RegularPriceValues = jenv->GetLongArrayElements(RegularPriceValuesRaw, 0);
		jint* DiscountTypeValues = jenv->GetIntArrayElements(DiscountTypes, 0);
		...
		for (jsize Idx = 0; Idx < NumProducts; Idx++)
		{
			...
			NewProductInfo.NumericPrice = ConvertMicrosToMinorUnits(PriceValues[Idx], FormattingOptions);

			// Discount-offer fields (FORT-1067735). Java side sends RegularPricesRaw=-1 / DiscountType=NotOnSale(0) when no sale.
			NewProductInfo.RegularPriceText = FText::FromString(FJavaHelper::FStringFromLocalRef(jenv, (jstring)jenv->GetObjectArrayElement(RegularPrices, Idx)));
			NewProductInfo.RegularPrice = (RegularPriceValues[Idx] >= 0)
				? ConvertMicrosToMinorUnits(RegularPriceValues[Idx], FormattingOptions)
				: -1;
			NewProductInfo.DiscountType = static_cast<EOnlineStoreOfferDiscountType>(DiscountTypeValues[Idx]);
			...
			UE_LOG_ONLINE_STOREV2(Verbose, TEXT("\nProduct Identifier: %s, Name: %s, Description: %s, Price: %s, Price Raw: %lld, Currency Code: %s, RegularPrice: %lld, RegularPriceText: %s, DiscountType: %d\n"),
				...
				*NewProductInfo.CurrencyCode,
				NewProductInfo.RegularPrice,
				*NewProductInfo.RegularPriceText.ToString(),
				static_cast<int32>(NewProductInfo.DiscountType));
		}
		jenv->ReleaseLongArrayElements(PriceValuesRaw, PriceValues, JNI_ABORT);
		jenv->ReleaseLongArrayElements(RegularPriceValuesRaw, RegularPriceValues, JNI_ABORT);
		jenv->ReleaseIntArrayElements(DiscountTypes, DiscountTypeValues, JNI_ABORT);
	}
