...
void FOnlinePurchaseGooglePlay::Init()
{
	...
	bool EnableAlternativeBillingOnly = false;
	GConfig->GetBool(TEXT("OnlineSubsystemGooglePlay.Store"), TEXT("EnableAlternativeBillingOnly"), EnableAlternativeBillingOnly, GEngineIni);

	extern void AndroidThunkCpp_Iap_SetupIapService(const FString&, bool);
	AndroidThunkCpp_Iap_SetupIapService(GooglePlayLicenseKey, EnableAlternativeBillingOnly);
}
...
bool FOnlinePurchaseGooglePlay::IsAllowedToPurchase(const FUniqueNetId& UserId, FOnlineError& Error)
{
	Error =  FOnlineError(EOnlineErrorResult::Success);
	return true;
}

bool FOnlinePurchaseGooglePlay::IsBillingClientConnected()
{
	extern bool AndroidThunkCpp_Iap_IsAllowedToMakePurchases();
	bool bIsBillingClientConnected = AndroidThunkCpp_Iap_IsAllowedToMakePurchases();
	UE_LOG_ONLINE_PURCHASE(Verbose, TEXT("FOnlinePurchaseGooglePlay::IsBillingClientConnected: %d"), bIsBillingClientConnected);
	return bIsBillingClientConnected;
}
...
void FOnlinePurchaseGooglePlay::Checkout(const FUniqueNetId& UserId, const FPurchaseCheckoutRequest& CheckoutRequest, const FOnPurchaseCheckoutComplete& Delegate)
{
	...
	if (IsBillingClientConnected())
	{
		...
		if (NumOffers > 0)
		{
			...
			else
			{
				...
				ErrorMessage = NSLOCTEXT("GooglePlayPurchase", "ErrorFailedToBeginPurchase", "Failed to checkout, failed to begin the purchase.");
	...
	{
		ErrorMessage = NSLOCTEXT("GooglePlayPurchase", "ErrorBillingClientNotConnected", "Failed to checkout, the billing client is not connected.");
		RequestedTransaction->PurchaseInfo.TransactionState = EPurchaseTransactionState::Failed;
	}
...
void FOnlinePurchaseGooglePlay::ShowAlternativeBillingInformationDialog(const FOnShowAlternativeBillingInformationDialogComplete& Delegate)
{
	UE_LOG_ONLINE_PURCHASE(Verbose, TEXT("FOnlinePurchaseGooglePlay::ShowAlternativeBillingOnlyInformationDialog"));

	if (!bRequestedAlternativeBillingOnlyInformationDialog)
	{
		bRequestedAlternativeBillingOnlyInformationDialog = true;
		AlternativeBillingOnlyInformationDialogComplete = Delegate;

		extern bool AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog();
		if (!AndroidThunkJava_IapShowAlternativeBillingOnlyInformationDialog())
		{
			UE_LOG_ONLINE_PURCHASE(Verbose, TEXT("FOnlinePurchaseGooglePlay::ShowAlternativeBillingInformationDialog failed to start dialog"));

			Subsystem->ExecuteNextTick([this]() {
				FOnlineError Result(EOnlineErrorResult::RequestFailure);
				AlternativeBillingOnlyInformationDialogComplete.ExecuteIfBound(Result);
				AlternativeBillingOnlyInformationDialogComplete.Unbind();
				bRequestedAlternativeBillingOnlyInformationDialog = false;
			});
		}
	}
	else
	{
		...
		UE_LOG_ONLINE_PURCHASE(Verbose, TEXT("FOnlinePurchaseGooglePlay::ShowAlternativeBillingInformationDialog already in progress."));
...
void FOnlinePurchaseGooglePlay::OnAlternativeBillingOnlyInformationDialogComplete(EGooglePlayBillingResponseCode InResponseCode)
{
	UE_LOG_ONLINE_PURCHASE(Log, TEXT("FOnlinePurchaseGooglePlay::OnAlternativeBillingOnlyInformationDialogComplete %s"), LexToString(InResponseCode));
	...
	if (bRequestedAlternativeBillingOnlyInformationDialog)
	{
		...
		if (!bSuccess)
		{
			Result.SetFromErrorCode(LexToString(InResponseCode));
			UE_LOG_ONLINE_PURCHASE(Log, TEXT("OnAlternativeBillingOnlyInformationDialogComplete failed %s"), LexToString(InResponseCode));
		}
	...
	{
		UE_LOG_ONLINE_PURCHASE(Warning, TEXT("FOnlinePurchaseGooglePlay::OnAlternativeBillingOnlyInformationDialogComplete unexpected call %s"), LexToString(InResponseCode));
	}
}
	...
	if(bIsInProgressTransaction)
	{
		...
		switch (Result)
		{
			case EPurchaseTransactionState::Failed:
				FinalResult.SetFromErrorCode(FString::Printf(TEXT("com.epicgames.purchase.googleplay.failure[%d:%s]"), EnumToUnderlyingType(InResponseCode), LexToString(InResponseCode)));
				...
				if (InResponseCode == EGooglePlayBillingResponseCode::BillingUnavailable)
				{
					FinalResult.SetErrorRaw(FString::Printf(TEXT("%s.noDelayedRedeemAndError"), *FinalResult.GetErrorCode()));
				}
				break;
			...
			case EPurchaseTransactionState::Invalid:
				FinalResult.SetFromErrorCode(FString::Printf(TEXT("com.epicgames.purchase.googleplay.invalid[%d:%s]"), EnumToUnderlyingType(InResponseCode), LexToString(InResponseCode)));
...
void JNICALL UE::Jni::FGooglePlayStoreHelper::NativeAlternativeBillingOnlyInformationDialogComplete(JNIEnv* jenv, jobject /*Thiz*/, jint JavaResponseCode)
{
	EGooglePlayBillingResponseCode EGPResponse = (EGooglePlayBillingResponseCode)JavaResponseCode;

	UE_LOG_ONLINE_PURCHASE(Verbose, TEXT("NativeAlternativeBillingOnlyInformationDialogComplete received response code: %s\n"), LexToString(EGPResponse));

	if (auto OnlineSubGP = static_cast<FOnlineSubsystemGooglePlay* const>(IOnlineSubsystem::Get(GOOGLEPLAY_SUBSYSTEM)))
	{
		OnlineSubGP->ExecuteNextTick([OnlineSubGP, EGPResponse]()
		{
			TSharedPtr<FOnlinePurchaseGooglePlay> PurchaseInt = StaticCastSharedPtr<FOnlinePurchaseGooglePlay>(OnlineSubGP->GetPurchaseInterface());
			PurchaseInt->OnAlternativeBillingOnlyInformationDialogComplete(EGPResponse);
		});
	}
}
