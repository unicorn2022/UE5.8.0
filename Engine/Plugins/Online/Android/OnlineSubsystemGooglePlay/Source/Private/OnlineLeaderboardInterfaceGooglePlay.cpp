	...
	for(int32 LeaderboardIdx = 0; LeaderboardIdx < WriteObject.LeaderboardNames.Num(); ++LeaderboardIdx)
	{
		...
		for(auto &[Key, Stat]: WriteObject.Properties)
		{
			...
			switch(Stat.GetType())
			{
				...
				default:
				{
					UE_LOG_ONLINE_LEADERBOARD(Warning, TEXT("Unsupported stat type %s"), EOnlineKeyValuePairDataType::ToString(Stat.GetType()));
					continue;
				}
			}
			UE_LOG_ONLINE_LEADERBOARD(Display, TEXT("FOnlineLeaderboardsGooglePlay::WriteLeaderboards() %s value Score: %s"), EOnlineKeyValuePairDataType::ToString(Stat.GetType()), *Stat.ToString());
