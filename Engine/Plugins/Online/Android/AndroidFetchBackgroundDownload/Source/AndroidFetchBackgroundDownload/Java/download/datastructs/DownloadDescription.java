...
public class DownloadDescription
{
	...
	// Fields used for final metrics
	public long DownloadDuration = 0;
	public long DownloadStartTimeUTC = 0;
	public long DownloadEndTimeUTC = 0;
	
	public DownloadDescription()
	{
	}

	public DownloadDescription(String RequestID, String DestinationLocation, int MaxRetryCount, int IndividualURLRetryCount, int RequestPriority, int GroupID, String[] URLs, boolean bIsPaused, long TotalBytesNeeded)
	{
		this.RequestID = RequestID;
		this.DestinationLocation = DestinationLocation;
		this.MaxRetryCount = MaxRetryCount;
		this.IndividualURLRetryCount = IndividualURLRetryCount;
		this.RequestPriority = RequestPriority;
		this.GroupID = GroupID;
		this.URLs = URLs;
		this.bIsPaused = bIsPaused;
		this.TotalBytesNeeded = TotalBytesNeeded;
	}
	...
	public static DownloadDescription FromJSON(String JSONString)
	{
		...
		try
		{
			...
			if (false == jObject.isNull(URLKey))
			{
				...
				ReturnedDescription.URLs = IntStream.range(0, JSONURLs.length()).mapToObj(JSONURLs::optString).toArray(String[]::new);
			}
			...
			if (false == jObject.isNull(TotalBytesNeededKey))
			{
				ReturnedDescription.TotalBytesNeeded = jObject.optLong(TotalBytesNeededKey);
			}
		}
	...
	public String ToJSON()
	{
		...
		try 
		{
			...
			JSONObj.put(TotalBytesNeededKey, TotalBytesNeeded);
		}
		catch( JSONException e )
		{
			e.printStackTrace();
	...
	{
		return ((this.URLs.length == Other.URLs.length) && (!this.HasNewURLs(Other)));
	}
	...
	{
		return !List.of(this.URLs).containsAll(List.of(Other.URLs));
	}
	...
	private static final String TotalBytesNeededKey = "TotalBytesNeeded";
}