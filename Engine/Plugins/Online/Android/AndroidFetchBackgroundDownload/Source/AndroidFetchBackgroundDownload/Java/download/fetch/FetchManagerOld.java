...
public class FetchManagerOld implements FetchDownloadProgressOwner, FetchEnqueueResultListener, FetchManagerInterface
{
	...
	private FetchManagerOld()
	{
		RequestedDownloads =  new HashMap<String,DownloadDescription>();
	}
	
	public static synchronized FetchManagerOld GetSharedManager()
	{
	...
	{
		runningState = ETaskRunningState.Unknown;
		InitFetch(context, QueueDescription.MaxConcurrentDownloads);
	...
	private synchronized void ReconcileDownloadDescriptions(DownloadQueueDescription QueueDescription)
	{
		...
		HashMap<String, DownloadDescription> TempRequestedDownloadsCopy = RequestedDownloads;
		RequestedDownloads = new HashMap<String, DownloadDescription>();
		...
		for (int DescriptionIndex = 0; DescriptionIndex < NumNewDownloadDescriptions; ++DescriptionIndex)
		{
			...
			DownloadDescription ActiveDescription = TempRequestedDownloadsCopy.get(NewDownloadDescription.DestinationLocation);
			boolean LastRequest = (NumNewDownloadDescriptions-1) == DescriptionIndex;
			
			if (ActiveDescription == null)
			{
				//Don't even have an entry in the old list, so it's completely new
	...
	{
		CompletedDownloads.remove(Description.DestinationLocation);
		FailedDownloads.remove(Description.DestinationLocation);
	}
	...
	public synchronized ECompleteReason GetCompleteReasonForDownload(DownloadDescription Description)
	{
		...
		if (FailedDownloads.containsKey(Description.DestinationLocation))
		{
	...
	public synchronized void QueueNewDownloadDescription(DownloadDescription Description, boolean LastRequest)
	{
		...
		RequestedDownloads.put(Description.DestinationLocation, Description);
		ResetDownloadCompleteTracking(Description);

		if (!CheckForPreviouslyCompletedDownload(Description))
		{	
			FetchEnqueueResultListener.FetchEnqueueRequestCallback RequestCallback = new FetchEnqueueResultListener.FetchEnqueueRequestCallback(this, Description.DestinationLocation);
			FetchEnqueueResultListener.FetchEnqueueErrorCallback ErrorCallback = new FetchEnqueueResultListener.FetchEnqueueErrorCallback(this, Description.DestinationLocation);
	...
	{
		boolean bInCompleteList = CompletedDownloads.containsKey(Description.DestinationLocation);
	...
	public synchronized void PauseDownload(String DestinationLocation, boolean bPause)
	{
		...
		if (!IsFetchInstanceValid())
		{
			Log.error("Call to PauseDownload after FetchInstance has been invalidated. Skipping as no work can be done. DestinationLocation: " + DestinationLocation);
			return;
		}	

		DownloadDescription MatchedDesc = RequestedDownloads.get(DestinationLocation);
		if (null == MatchedDesc)
		{
			Log.error("No DownloadDescription found for DestinationLocation " + DestinationLocation +" during PauseDownload!");
			return;
		}
	...
	public synchronized void CancelDownload(String DestinationLocation)
	{
		if (!IsFetchInstanceValid())
		{
			Log.error("Call to CancelDownload with an invalid fetch instance. Skipping fetch work to avoid crashing. DestinationLocation: " + DestinationLocation);
			return;
		}
		...
		if (null == MatchedDesc)
		{
			Log.error("No DownloadDescription found for DestinationLocation " + DestinationLocation +" during CancelDownload!");
			return;
		}
		
		if (false == MatchedDesc.bIsCancelled)
		{
			Log.verbose("Cancelling download:" + MatchedDesc.RequestID);
			
			//Set download as complete since it was cancelled intentionally
			MatchedDesc.PreviousDownloadPercent = 100;
			CompletedDownloads.put(DestinationLocation, MatchedDesc);
	...
	private synchronized Request BuildFetchRequest(DownloadDescription Description)
	{
		...
		if ((URL == null) || (Description.DestinationLocation == null))
		{
			Log.error("URL or DestinationLocation to build Uri from are null! Can not create a valid FetchRequest! RequestID:" + Description.RequestID);
			return null;
		}
		...
		FetchRequest.setTag(Description.DestinationLocation);
	...
	private synchronized void HandleChangedDownloadDescription(DownloadDescription OldDescription, DownloadDescription NewDescription, boolean LastRequest)
	{
		...
		FetchInstance.getDownloadsByTag(OldDescription.DestinationLocation, RecreateFunc);
	}
	...
	private String GetNextURL(DownloadDescription Description)
	{
		...
		final int NumURLs = Description.URLs.length;
		final int URLIndexToUse = Description.CurrentRetryCount % NumURLs;
		
		return Description.URLs[URLIndexToUse];
	}
	...
	private synchronized void InitFetch(Context context, int MaxConcurrentDownloads)
	{
...
//		synchronized(this)
	...
	private class RecreateDownloadByTagFunc extends CancelDownloadByTagFunc
	{
		...
		public RecreateDownloadByTagFunc(FetchManagerOld Owner, DownloadDescription RecreateDescription, boolean LastRequest)
		{
			this.Owner = Owner;
	...
	{
		Log.verbose("OnDownloadQueued: " + GetDestinationLocation(download));
	...
	{
		String DestinationLocation = GetDestinationLocation(download);
		Log.verbose("OnDownloadStarted: " + DestinationLocation);

		DownloadDescription MatchedDownload = RequestedDownloads.get(DestinationLocation);
		if (null == MatchedDownload)
		{
			Log.error("OnDownloadStarted called from DownloadProgressOwner implementation with a download that doesn't match any DownloadDesc! Download's Tag: " + DestinationLocation);
			return;
		}
	...
	public void OnDownloadProgress(@NonNull Download download, boolean indeterminate, long downloadedBytesPerSecond, long etaInMilliSeconds)
	{
		...
		Log.verbose("OnDownloadProgress: " + DestinationLocation + " bytes/s:" + downloadedBytesPerSecond + " eta:" + etaInMilliSeconds);

		DownloadDescription MatchedDownload = RequestedDownloads.get(DestinationLocation);
		if (null == MatchedDownload)
		{
			Log.error("OnDownloadProgress called from DownloadProgressOwner implementation with a download that doesn't match any DownloadDesc! Download's Tag: " + DestinationLocation);
			return;
		}
		...
		if (HasValidProgressCallback(MatchedDownload))
		{
			...
			if (MatchedDownload.TotalDownloadedBytes > TotalDownloaded)
			{
				TotalDownloadedSinceLastCall = 0;
			}
			else
			{
				TotalDownloadedSinceLastCall = (TotalDownloaded - MatchedDownload.TotalDownloadedBytes);
			}
			...
			MatchedDownload.ProgressListener.OnDownloadProgress(GetDestinationLocation(download), TotalDownloaded, TotalDownloadedSinceLastCall);
	...
	public synchronized void OnDownloadChangePauseState(@NonNull Download download, boolean bIsPaused)
	{
		...
		String DestinationLocation = GetDestinationLocation(download);
		Log.verbose("OnDownloadChangePauseState: " + DestinationLocation);

		DownloadDescription MatchedDownload = RequestedDownloads.get(DestinationLocation);
		if (null == MatchedDownload)
		{
			Log.error("OnDownloadChangePauseState called from DownloadProgressOwner implementation with a download that doesn't match any DownloadDesc! Download's Tag: " + DestinationLocation);
			return;
		}
		
		if (!IsFetchInstanceValid())
		{
			Log.error("Call to OnDownloadChangePauseState with an invalid fetch instance. Skipping fetch work to avoid crashing. RequestID: " + MatchedDownload.RequestID);
			return;
		}
		...
		{
			Log.verbose("OnDownloadChangePauseState required Fetch action. RequestID:" + MatchedDownload.RequestID + " New bIsPaused:" + MatchedDownload.bIsPaused + " Old Fetch bIsPaused:" + bIsPaused);			
	...
	//This version is the DownloadProgressListener version of this function! See CompleteDownload for FetchManagerOld implementation
	@Override
	public void OnDownloadComplete(@NonNull Download download, ECompleteReason completeReason)
	{
		String DestinationLocation = GetDestinationLocation(download);
		Log.debug("OnDownloadComplete: " + DestinationLocation + " Reason:" + completeReason);

		DownloadDescription MatchedDownload = RequestedDownloads.get(DestinationLocation);
		if (null == MatchedDownload)
		{
			Log.error("OnDownloadComplete called from DownloadProgressOwner implementation with a download that doesn't match any DownloadDesc! Download's Tag: " + DestinationLocation);
			return;
		}
	...
	//
	@Override
	public void OnFetchEnqueueRequestCallback(@NonNull String DestinationLocation, @NonNull Request EnqueuedRequest)
	{
		Log.verbose("Enqueued Request Success. DestinationLocation:" + DestinationLocation);
		
		DownloadDescription MatchingDescription = RequestedDownloads.get(DestinationLocation);
		if (null == MatchingDescription)
		{
			Log.error("OnFetchEnqueueRequestCallback called on request is missing from our RequestedDownloads list! DestinationLocation:" + DestinationLocation);
			return;
		}

		MatchingDescription.ProgressListener.OnDownloadEnqueued(MatchingDescription);
	}

	@Override
	public synchronized void OnFetchEnqueueErrorCallback(@NonNull String DestinationLocation, @NonNull Error EnqueueError)
	{
		Log.error("Error Enqueing Request! " + DestinationLocation + " Error: " + EnqueueError);
		...
		{
			Log.error("Enqueue Error " + EnqueueError + " could be because of a previous enqueue already succeeding. Checking if Request is already enqueued before erroring. " + DestinationLocation);
						
			CheckForQueuedDownload CheckForQueueFunc = new CheckForQueuedDownload(this, DestinationLocation);
			FetchInstance.getDownloadsByTag(DestinationLocation, CheckForQueueFunc);
		...
		if (null == MatchingDescription)
		{
			Log.error("OnFetchEnqueueErrorCallback called on request is missing from our RequestedDownloads list! DestinationLocation:" + DestinationLocation);
			return;
		}

		RetryDownload(DestinationLocation);

		MatchingDescription.ProgressListener.OnDownloadEnqueued(MatchingDescription);
	}
	
	public synchronized void HandleEnqueueCheckCallback(String DestinationLocation, @Nullable Download FetchDownload)
	{
		...
		if (null == MatchingDescription)
		{
			Log.debug("No DownloadDescription matches " + DestinationLocation + " can not send enqueue callback with result " + bIsAlreadyEnqueued);
			return;
		}
	...
	//if UEDownloadableWorker is calling into it, the only thing it should need is the DestinationLocation
	@Override
	public void RetryDownload(String DestinationLocation)
	{
		RetryDownload(DestinationLocation, null);
	}
	
	private synchronized void RetryDownload(String DestinationLocation, @Nullable Download FetchDownload)
	{
		DownloadDescription MatchingDescription = RequestedDownloads.get(DestinationLocation);
		if (null == MatchingDescription)
		{
			Log.error("RetryDownload called on invalid download that was never requested! DestinationLocation:" + DestinationLocation);
			return;
		}

		if (!IsFetchInstanceValid())
		{
			Log.debug("Call to RetryDownload with an invalid fetch instance. Skipping fetch work and completing to avoid crashing. RequestID:" + MatchingDescription.RequestID);
	...
	{
		public CheckForQueuedDownload(FetchManagerOld Owner, String DestinationLocation)
		{
			this.Owner = Owner;
		...
		public void call(List<Download> MatchingDownloads)
		{
			...
			{
				Owner.HandleEnqueueCheckCallback(DestinationLocation, MatchingDownloads.get(0));
			}
	...
	//Helper class to handle routing calls from the FetchInstance's hasActiveDownloads function back to our FetchManagerOld
	private class CheckForActiveDownloadsFunc implements Func<Boolean>
	{
		...
		public CheckForActiveDownloadsFunc(FetchManagerOld Owner, DownloadProgressListener ProgressListener)
		{
			this.Owner = Owner;
	...
	private synchronized void CompleteDownload(DownloadDescription DownloadDesc, ECompleteReason CompleteReason, boolean CheckForAllCompleted)
	{
		...
		CompletedDownloads.put(DownloadDesc.DestinationLocation, DownloadDesc);
		...
		if (CompleteReason == FetchRequestProgressListener.ECompleteReason.Success)
		{
			...
			else
			{
				...
				if (!CompletedFile.renameTo(NewFileAtDestinationLocation))
				{
					...
					FailedDownloads.put(DownloadDesc.DestinationLocation, DownloadDesc);
				}
			}
		}
		else
		...
		DownloadDesc.ProgressListener.OnDownloadMetrics(DownloadDesc.DestinationLocation, DownloadDesc.TotalDownloadedBytes, DownloadDuration, DownloadDesc.DownloadStartTime, DownloadFinishTime);

		DownloadDesc.ProgressListener.OnDownloadComplete(DownloadDesc.DestinationLocation, CompleteReasonToSend);
	...
	private String GetDestinationLocation(Download download)
	{
		if (null != download)
		{
	...
	//Used to determine if the reason a download completed was because of action we took. IE: Cancelling, deleting, removing, etc that happened through calls to the FetchManagerOld.
	...
	private static volatile FetchManagerOld SharedManager = new FetchManagerOld();
	...
	public Logger Log = new Logger("UE", "FetchManagerOld");
