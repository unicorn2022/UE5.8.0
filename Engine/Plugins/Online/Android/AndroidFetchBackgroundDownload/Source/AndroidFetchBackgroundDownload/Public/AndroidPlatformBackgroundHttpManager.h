...
protected:
	...
	FAndroidBackgroundHttpRequestPtr FindRequestByDestinationLocation(const FString& DestinationLocation);
	...
	void Java_OnDownloadProgress(jobject UnderlyingWorker, FString DestinationLocation, int64_t TotalBytesWritten, int64_t BytesWrittenSinceLastCall);
	void Java_OnDownloadComplete(jobject UnderlyingWorker, FString DestinationLocation, bool bWasSuccess);
	void Java_OnDownloadMetrics(jobject UnderlyingWorker, FString DestinationLocation, int64_t TotalBytesDownloaded, int64_t DownloadDuration, int64_t DownloadStartTimeUTC, int64_t DownloadEndTimeUTC);
...
private:
	...
	struct FJobStopData
	{
		...
		int32 LastReportedSecs = 0;
		double GetWaitTimeInSeconds() const {
			return (FDateTime::UtcNow() - StartTime).GetTotalSeconds();
		}
	};
	TOptional<FJobStopData> JobStopData;
...
protected:
	...
	friend UE::Online::Download::Android::FDownloadManager;
};
