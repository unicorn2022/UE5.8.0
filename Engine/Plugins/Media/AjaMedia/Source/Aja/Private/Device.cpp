	...
	namespace Private
	{
		...
		*****************************************************************************/
		...
		std::shared_ptr<DeviceConnection> DeviceCache::GetDevice(const AJADeviceOptions& InDeviceOption)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "DeviceCache: The device index '%d' is invalid.\n", InDeviceOption.DeviceIndex);
		...
		*****************************************************************************/
		...
		bool DeviceConnection::CommandList::RegisterChannel(ETransportType InTransportType, NTV2InputSource InInputSource, NTV2Channel InChannel, bool bInIsInput, bool bInAsGenlock, bool bConnectChannel, ETimecodeFormat InTimecodeFormat, EPixelFormat InUEPixelFormat, NTV2VideoFormat InDesiredInputFormat, FAjaHDROptions& InOutHDROptions, bool bInAsOwner, bool bInIsAutoDetected)
		{
			...
			else
			{
				...
				if (bReinitialize || (FoundChannel->bGenlockChannel && !bInAsGenlock))
				{
					...
					{
						UE_LOGF(LogAjaCore, Warning,  "Device: Autodetect reinitialized channel '%d' on device '%s' but the channel was not in autodetect mode.\n"
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Trying to enable the channel '%d' on device '%s' as the owner but it was already enabled. Do you already have an '%s' running on the same channel?\n"
						, uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str()
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Trying to enable the channel '%d' on device '%s' as a '%s' but it was already enabled as a '%s'.\n"
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Trying to enable the channel '%d' on device '%s' as an '%s' but it was already enabled as an '%s'.\n"
						, uint32_t(InChannel) + 1
						, Connection.Card->GetDisplayName().c_str()
				...
				if (bResult)
				{
					...
					{
						UE_LOGF(LogAjaCore, Error, "Device: Initialization of the input failed for channel %d on device %s. %s", uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str(), FailureReason.c_str());
						bResult = false;
					}
					...
					if (bResult && !bInIsAutoDetected && VideoFormat != FoundChannel->VideoFormat)
					{
						...
						UE_LOGF(LogAjaCore, Error, "Device: Try to enable with the video format '%s' the channel '%d' on device '%s' that was already enabled with '%s'.\n"
				...
				{
					UE_LOGF(LogAjaCore, Warning,  "Device: Try to enable the channel '%d' on device '%s' with a timecode format that is not the same as the previous one. Timecode may not be decoded properly.\n"
						, uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str());
				}
		...
		bool DeviceConnection::CommandList::UnregisterChannel(NTV2Channel InChannel, bool bInAsOwner)
		{
			...
			if (FoundIterator == std::end(ConnectionChannelInfos))
			{
				UE_LOGF(LogAjaCore, Error, "Device: Try to unregister the channel '%d' on device '%s' when it's not registered.\n", uint32_t(InChannel) + 1, Connection.Card->GetDisplayName().c_str());
				assert(false);
				return false;
			}
		...
		bool DeviceConnection::CommandList::RegisterAnalogLtc(EAnalogLTCSource InSource, NTV2FrameRate InFrameRate, bool bUseReferencePin)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "Device: Trying to read LTC from the reference input '%d' on device '%s' but it is not supported by the device.\n", uint32_t(InSource) + 1, Connection.Card->GetDisplayName().c_str());
				return false;
			}
			...
			{
				UE_LOGF(LogAjaCore, Error, "Device: Trying to read LTC from the LTC input '%d' on device '%s' but it is not supported by the device.\n", uint32_t(InSource) + 1, Connection.Card->GetDisplayName().c_str());
				bResult = false;
			}
			...
			{
				UE_LOGF(LogAjaCore, Error, "Device: The device '%s' is already reading timecode from LTC input with frame rate '%s' and only one is supported.", Connection.Card->GetDisplayName().c_str(), NTV2FrameRateToString(Connection.LTCFrameRate).c_str());
				bResult = false;
			}
			...
			if (bResult && bUseReferencePin)
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Trying to read LTC from reference pin on device '%s' but it doesn't have that capability.\n", Connection.Card->GetDisplayName().c_str());
					bResult = false;
				}
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: The reference pin is already used to read LTC timecode on device '%s'.\n", Connection.Card->GetDisplayName().c_str());
					bResult = false;
				}
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Trying to read LTC from the reference input on device '%s' but the reference is already used as a genlock source.\n", Connection.Card->GetDisplayName().c_str());
					bResult = false;
				}
			}
			...
			if (bResult)
			{
				...
				if (Found != std::end(Connection.ChannelInfos))
				{
					...
					{
						UE_LOGF(LogAjaCore, Error, "Device: Trying to read LTC with FrameRate '%s' but it's not compatible with card's (channel 1) current FrameRate of '%s' on device '%s'.\n", NTV2FrameRateToString(InFrameRate).c_str(), NTV2FrameRateToString(ChannelFrameRate).c_str(), Connection.Card->GetDisplayName().c_str());
						bResult = false;
					}
				}
				else
		...
		bool DeviceConnection::CommandList::RegisterReference(EAJAReferenceType InOutputReferenceType, NTV2Channel InOutputReferenceChannel)
		{
			...
			if (Connection.bOutputReferenceSet)
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Couldn't set the reference (%s) for output on device %s. The input was already setup with %d.\n", Helpers::ReferenceTypeToString(InOutputReferenceType), Connection.Card->GetDisplayName().c_str(), Connection.OutputReferenceChannel);
					return false;
				}
			...
			case EAJAReferenceType::EAJA_REFERENCETYPE_EXTERNAL:
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Couldn't set the reference (%s) for output channel %d on device %s. The reference is used to read analog LTC.\n", Helpers::ReferenceTypeToString(InOutputReferenceType), InOutputReferenceChannel, Connection.Card->GetDisplayName().c_str());
					return false;
				}
				AJA_CHECK(Connection.Card->SetLTCInputEnable(false));
			...
			case EAJAReferenceType::EAJA_REFERENCETYPE_INPUT:
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Couldn't set the reference source on device '%s. The Channel index is invalid.", Connection.Card->GetDisplayName().c_str());
					return false;
				}
		...
		*****************************************************************************/
		CNTV2Card* DeviceConnection::NewCNTV2Card(uint32_t InDeviceIndex)
		{
			...
			while (!Result->IsDeviceReady())
			{
				...
				{
					UE_LOGF(LogAjaCore, Warning,  "Device: Can't get the device initialized.\n");
					delete Result;
					return nullptr;
				}
				++Counter;
		...
		FDelegateHandle DeviceConnection::SetOnInterruptEvent(NTV2Channel InChannel, AJA::Private::DeviceConnection::FOnVerticalInterrupt::FDelegate OnInterrupt)
		{
			...
			if (FoundIt != std::end(ChannelInfos))
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "No sync thread available when calling SetOnInterruptEvent.");
				}
			}
		...
		bool DeviceConnection::Lock_EnableChannel_SDILinkHelper(CNTV2Card* InCard, ETransportType InTransportType, NTV2Channel InChannel) const
		{
			...
			if (NumberOfChannels != 1)
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Can't do a %s on channel '%d'. Did you mean channel '%d'?"
						, Helpers::TransportTypeToString(InTransportType)
						, uint32_t(InChannel) + 1
			...
			for (ChannelInfo* Itt : ChannelInfos)
			{
				if (Itt->Channel == InChannel)
				{
					...
					{
						UE_LOGF(LogAjaCore, Error, "Device: Trying to enable the channel '%d' on device '%s' as a '%s' but channel '%d' was already enabled.\n"
							, int32_t(InChannel) + 1
				...
				else
				{
					...
					{
						UE_LOGF(LogAjaCore, Error, "Device: Trying to enable the channel '%d' on device '%s' as a '%s' but it was already enabled as a '%s' by channel '%d'.\n"
							, uint32_t(InChannel) + 1
							, InCard->GetDisplayName().c_str()
							, Helpers::TransportTypeToString(InTransportType)
		...
		bool DeviceConnection::Lock_EnableChannel_Helper(CNTV2Card* InCard, ETransportType InTransportType, NTV2InputSource InInputSource, NTV2Channel InChannel, bool bIsInput, bool bConnectChannel, bool bIsAutoDetected, ETimecodeFormat InTimecodeFormat, EPixelFormat InUEPixelFormat, NTV2VideoFormat InDesiredInputFormat, NTV2VideoFormat& OutFoundVideoFormat, FAjaHDROptions& InOutHDROptions)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "Device: The device '%s' doesn't support channel '%d'", InCard->GetDisplayName().c_str(), uint32_t(InChannel) + 1);
				return false;
			}
			...
			if (NotMultiVideoFormat != NTV2_FORMAT_UNKNOWN && NotMultiVideoFormat != OutFoundVideoFormat)
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: The device '%s' doesn't support Multi Format and channel %d requested format ('%s') which is not the same as previous format ('%s').", InCard->GetDisplayName().c_str(), uint32_t(InChannel) + 1, NTV2VideoFormatToString(OutFoundVideoFormat).c_str(), NTV2VideoFormatToString(NotMultiVideoFormat).c_str());
					return false;
				}
			}
				...
				if (GetFrameRateFamily(DeviceFramerate) != NewVideoFormatFrameRateFamily)
				{
					...
					for (ChannelInfo* Info : ChannelInfos)
					{
						if (Info)
						{
							...
							{
								UE_LOGF(LogAjaCore, Error, "Device: Can't enable channel %d device '%s' because its framerate doesn't match an existing channel's framerate family.", uint32_t(InChannel) + 1, InCard->GetDisplayName().c_str());
								return false;
							}
						}
					}
			...
			if (bIsInput && bIsSDI)
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Device: Initialization of the input failed for channel %d on device '%s'. %s", uint32_t(InChannel) + 1, InCard->GetDisplayName().c_str(), FailureReason.c_str());
					return false;
				}
				
				if (!Helpers::GetInputHDRMetadata(InCard, InChannel, InOutHDROptions))
				{
					UE_LOGF(LogAjaCore, Error, "Device: Could not fetch HDR metadata from channel %d on device '%s'.", uint32_t(InChannel) + 1, InCard->GetDisplayName().c_str());
					return false;
				}
			}
		...
		bool DeviceConnection::Lock_Initialize()
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "Device: Couldn't get current stream using Aja. Something is wrong when tryin to access registers on device '%s'.", Card->GetDisplayName().c_str());
				delete Card;
				Card = nullptr;
				return false;
			}
			...
			if (!Card->AcquireStreamForApplication(UEAppType, static_cast<uint32_t>(AJAProcess::GetPid())))
			{
				...
				if (CurrentAppPID)
				{
					...
					UE_LOGF(LogAjaCore, Error, "Device: Couldn't acquire stream for Unreal Engine application on device '%s'. Used by application '%s' with PID '%d'.", Card->GetDisplayName().c_str(), FourCCBuffer, CurrentAppPID);
				}
				else
				{
					UE_LOGF(LogAjaCore, Error, "Device: Couldn't acquire stream for Unreal Engine application on device '%s'.", Card->GetDisplayName().c_str());
				}
			...
			{
				UE_LOGF(LogAjaCore,  Display, "Connected to %s with SDK %d.%d.%d.%d", Card->GetDeviceVersionString().c_str(), AJA_NTV2_SDK_VERSION_MAJOR, AJA_NTV2_SDK_VERSION_MINOR, AJA_NTV2_SDK_VERSION_POINT, AJA_NTV2_SDK_BUILD_NUMBER);
				if (AJA_NTV2_SDK_VERSION_MAJOR < 14)
				{
					UE_LOGF(LogAjaCore, Warning,  "Unreal Engine's implementation of AJA support SDK 14 and up.");
				}
			}
			...
			if (DeviceOption.bWantMultiFormatMode)
			{
				...
				{
					UE_LOGF(LogAjaCore, Warning,  "Device: The device '%s' doesn't support Multi Format.", Card->GetDisplayName().c_str());
				}
			}
		...
		uint32_t DeviceConnection::Lock_AcquireBaseFrameIndex(NTV2Channel InChannel, NTV2VideoFormat InVideoFormat) const
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "Device: Too many Frames Indexes requested on device '%s'.\n", Card->GetDisplayName().c_str());
