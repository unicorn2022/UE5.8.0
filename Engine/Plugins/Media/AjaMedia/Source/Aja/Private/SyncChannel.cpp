	...
	namespace Private
	{
		...
		*****************************************************************************/
			...
			, bStopRequested(false)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "SyncChannel: The port index '%d' is invalid.\n", InOptions.ChannelIndex);
			}
			else
			{
		...
		bool SyncChannelImpl::Thread_Configure(DeviceConnection::CommandList& InCommandList)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "SyncChannel: The expected video format is invalid for %d.\n", uint32_t(Channel) + 1);
				return false;
			}
			...
			{
				UE_LOGF(LogAjaCore, Error, "SyncChannel: Was not able to lock on device %s for channel %d.\n", GetDevice().GetDisplayName().c_str(), uint32_t(Channel) + 1);
				return false;
			}
		...
		bool SyncChannelImpl::WaitForSync()
		{
			if (!SyncOption.bOutput)
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "Sync: The VideoFormat was invalid for channel %d on device %s. %s\n"
						, uint32_t(Channel) + 1
				...
				if (!Helpers::CompareFormats(DesiredVideoFormat, FoundFormat.value(), FailureReason))
				{
					...
					{
						UE_LOGF(LogAjaCore, Error, "Sync: The VideoFormat changed for channel %d on device %s. %s\n"
							, uint32_t(Channel) + 1
							, GetDevice().GetDisplayName().c_str()
							, FailureReason.c_str()
						);
			...
			if (!bResult && !SyncOption.bStopOnCardTimeout)
			{
				...
				UE_LOGF(LogAjaCore, Error, "SyncChannel: Was not able to wait for an interrupt signal on device %s for channel %d.\n", GetDevice().GetDisplayName().c_str(), uint32_t(Channel) + 1);
				bResult = true;
			}
