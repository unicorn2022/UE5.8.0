		...
		*****************************************************************************/
		...
		bool TimecodeChannelImpl::Thread_Configure(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (TimecodeOption.bUseDedicatedPin)
			{
				...
				default:
					UE_LOGF(LogAjaCore, Error, "TimecodeChannel: The LTC source Index is invalid on device %s.\n", GetDevice().GetDisplayName().c_str());
					return false;
				}
			...
			else
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "TimecodeChannel: The expected video format is invalid for %d.\n", uint32_t(Channel) + 1);
					return false;
				}
				...
				{
					UE_LOGF(LogAjaCore, Error, "TimecodeChannel: Was not able to lock on device %s for channel %d.\n", GetDevice().GetDisplayName().c_str(), uint32_t(Channel) + 1);
					return false;
				}
			}
