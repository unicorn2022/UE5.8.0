	...
	namespace Private
	{
		...
		*****************************************************************************/
		...
		bool OutputChannelThread::SetAncillaryFrameData(const AJAOutputFrameBufferData& InFrameData, uint8_t* InAncillaryBuffer, uint32_t InAncillaryBufferSize)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "SetFrameData: Can't set the ancillary. The buffer is invalid or the buffer size is not the same as the AJA for output channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		bool OutputChannelThread::SetAudioFrameData(const AJAOutputFrameBufferData& InFrameData, uint8_t* InAudioBuffer, uint32_t InAudioBufferSize)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "SetAudioFrameData: Can't set the audio. The buffer size is not the same as the AJA Audio Format for output channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		bool OutputChannelThread::SetVideoFrameData(const AJAOutputFrameBufferData& InFrameData, uint8_t* InVideoBuffer, uint32_t InVideoBufferSize)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "SetVideoFrameData: Can't set the video. The buffer is invalid or the buffer size is not the same as the AJA Video Format for output channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		bool OutputChannelThread::SetVideoFrameData(const AJAOutputFrameBufferData& InFrameData, FRHITexture* RHITexture)
		{
			...
			if (!bIsProgressive)
			{
				UE_LOGF(LogAjaCore, Error, "SetVideoFrameData: Can't set the video, GPUTextureTransfer is not supported with interlaced video for output channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
			...
			if (!RHITexture)
			{
				UE_LOGF(LogAjaCore, Error, "SetVideoFrameData: Can't set the video. The RHI texture is invalid for output channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}

			if (!TextureTransfer || !GetOptions().bUseGPUDMA)
			{
				UE_LOGF(LogAjaCore, Error, "SetVideoFrameData: Can't set the video. GPU DMA was not setup correctly for output channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		bool OutputChannelThread::ShouldCaptureThisFrame(::FTimecode Timecode, uint32 SourceFrameNumber)
		{
			...
			if (bInterlace && Timecode.Frames % 2 != 0 && GetOptions().bOutputInterlaceOnEvenFrames)
			{
				...
				{
					UE_LOGF(LogAjaCore, Verbose, "Skipping output because no writable frame is available.");
				}
				else
				{
					for (Frame* Frame : FrameReadyToWrite)
					{
						UE_LOGF(LogAjaCore, VeryVerbose, "\t F1: %d, F2: %d", Frame->FrameIdentifier, Frame->FrameIdentifierF2);
					}
				}
			...
			if(bResult && UseAudio())
			{
				...
				if (ensureMsgf(AudioBufferSize > 0, TEXT("Audio could not be initialized.")))
				{
					...
					if (GetOptions().bUseAutoCirculating)
					{
						...
						if (bResult == false)
						{
							UE_LOGF(LogAjaCore, Error, "ConfigureAudio: Could not start audio output for audio system %d for channel %d on device %s.", uint32_t(AudioSystem), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						}
					}
				}
			}
		...
		void OutputChannelThread::Thread_AutoCirculateLoop()
		{
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bRunning)
				{
					UE_LOGF(LogAjaCore, Error, "AutoCirculate: Can't wait for the output field for channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				...
				if (!bRunning)
				{
					UE_LOGF(LogAjaCore, Error, "AutoCirculate: Can't get the status for output channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					break;
				}
		...
		void OutputChannelThread::Thread_PingPongLoop()
		{
			...
			{
				UE_LOGF(LogAjaCore, Warning, "PingPong: Dropped %d frames while outputting on channel %d on device %s.\n", LastFrameDropCount, uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
			};
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bRunning)
				{
					UE_LOGF(LogAjaCore, Error, "PingPong: Can't wait for the output field for channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					if (GetOptions().bStopOnTimeout)
					{
						break;
					}
				...
				if (AvailableReadingFrame)
				{
					...
					if (!bRunning)
					{
						UE_LOGF(LogAjaCore, Error, "PingPong: Can't do the DMA frame transfer for channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						break;
					}
		...
		void OutputChannelThread::Thread_TestInterlacedOutput(Frame* InFrame)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "INTERLACED TEST - The 2 lines are the same color. %d\n", InterlacedTest_FrameCounter);
			}
			if (bIsFirstLineWhite != bInterlacedTest_ExpectFirstLineToBeWhite)
			{
				...
				UE_LOGF(LogAjaCore, Error, "INTERLACED TEST - The lines has swap color. %d\n", InterlacedTest_FrameCounter);
			}
			++InterlacedTest_FrameCounter;
		}
		...
		bool OutputChannelThread::Thread_HandleAudio(const FString& OutputMethod, Frame* AvailableReadingFrame)
		{
			...
			{
				UE_LOGF(LogAjaCore, Verbose, "%ls: Resuming audio output.", *OutputMethod);
			...
			else if (!bAudioOutputRunning)
			{
				...
				UE_LOGF(LogAjaCore, Verbose, "%ls: Starting audio output.", *OutputMethod);
			...
			if (bAudioSuccess)
			{
				...
				UE_LOGF(LogAjaCore, Verbose, "%ls: Play head: %d, Write Head: %d, Offset: %f", *OutputMethod, AudioPlayheadLastPosition.load(), CurrentAudioWriteOffset.load(), OffsetInSeconds);
				...
				{
					UE_LOGF(LogAjaCore, Warning, "%ls: Audio play head is catching up to write head for channel %d on device %s.\n", *OutputMethod, uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());

					bool bPausePlayout = true;
					GetDevice().SetAudioOutputPause(AudioSystem, bPausePlayout);

					UE_LOGF(LogAjaCore, Verbose, "%ls Pausing audio output because the play head is catching up to the write head. Last playhead position: %d", *OutputMethod, AudioPlayheadLastPosition.load());
				}
			}
			else
			{
				UE_LOGF(LogAjaCore, Verbose, "%ls Audio output failed.", *OutputMethod);
			}
		...
		bool OutputChannelThread::DMAWriteAudio(const uint8_t* InAudioBuffer, int32_t BufferSize)
		{
			...
			UE_CLOGF(!bAudioSuccess, LogAjaCore, Verbose, "IsAudioOutputRunning returned false.");

			bool bIsPaused = false;
			bAudioSuccess &= GetDevice().GetAudioOutputPause(AudioSystem, bIsPaused);

			UE_CLOGF(!bAudioSuccess, LogAjaCore, Verbose, "GetAudioOutputPause returned false.");
			...
			if (!bAudioOutputRunning)
			{
				...
				UE_LOGF(LogAjaCore, Verbose, "PingPong: Starting audio output.");
				bAudioSuccess &= GetDevice().StartAudioOutput(AudioSystem); // This resets the playhead to 0, we have to override that to something closer to the write head in order to reduce audio delay
			}

			double OffsetInSeconds = 0;
			bAudioSuccess &= Thread_GetAudioOffsetInSeconds(OffsetInSeconds);
			UE_CLOGF(!bAudioSuccess, LogAjaCore, Verbose, "Thread_GetAudioOffsetInSeconds returned false.");
			ULWord PlayHeadPosition = 0;
			bAudioSuccess &= GetDevice().ReadAudioLastOut(PlayHeadPosition, AudioSystem);
			UE_CLOGF(!bAudioSuccess, LogAjaCore, Verbose, "ReadAudioLastOut returned false.");
			
			AudioPlayheadLastPosition = PlayHeadPosition;
			SET_FLOAT_STAT(STAT_AjaMediaCapture_Audio_Delay, OffsetInSeconds);
			UE_LOGF(LogAjaCore, Verbose, "%ls: Play head: %d, Write Head: %d, Offset: %f", TEXT("PingPong"), AudioPlayheadLastPosition.load(), CurrentAudioWriteOffset.load(), OffsetInSeconds);
		...
		bool OutputChannelThread::Thread_HandleLostFrameAudio()
		{
			...
			if (bAudioOutputRunning)
			{
				...
				if (bAudioSuccess && Thread_ShouldPauseAudioOutput())
				{
					...
					UE_LOGF(LogAjaCore, Warning, "Pausing audio output. Last playhead position: %d", AudioPlayheadLastPosition.load());
				}
			}
