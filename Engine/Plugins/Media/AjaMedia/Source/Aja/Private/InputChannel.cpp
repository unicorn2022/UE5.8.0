	...
	namespace Private
	{
		...
		*****************************************************************************/
		...
		bool InputChannelThread::DeviceThread_ConfigureAutoCirculate(DeviceConnection::CommandList& InCommandList)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "AutoCirculate: The device '%s' couldn't not capture.", GetDevice().GetDisplayName().c_str());
				return false;
			}
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bRunning)
				{
					UE_LOGF(LogAjaCore, Error, "AutoCirculate: Can't get the status for channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					bRunning = false;
				}
				...
				{
					UE_LOGF(LogAjaCore, Error, "AutoCirculate: Can't get the Channel running for channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					bRunning = false;
				}
		...
		void InputChannelThread::Thread_AutoCirculateLoop()
		{
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (ChannelStatus.IsRunning() && ChannelStatus.HasAvailableInputFrame())
				{
					...
					if (!bRunning)
					{
						UE_LOGF(LogAjaCore, Error, "AutoCirculate: Can't transfer the buffer for channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						break;
					}
					...
					if (!bWaitResult)
					{
						...
						{
							UE_LOGF(LogAjaCore, Warning,  "AutoCirculate: The device '%s' missed an interrupt signal.", GetDevice().GetDisplayName().c_str());
						}
					}
				}
				else
		...
		bool InputChannelThread::DeviceThread_ConfigurePingPong(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (!::NTV2DeviceCanDoCapture(DeviceId))
			{
				UE_LOGF(LogAjaCore, Error, "PingPong: The device '%s' couldn't not capture.", GetDevice().GetDisplayName().c_str());
				return false;
			}
		...
		void InputChannelThread::Thread_PingPongLoop()
		{
			...
			while (bRunning && !bStopRequested)
			{
				...
				if (!bWaitResult)
				{
					...
					{
						UE_LOGF(LogAjaCore, Warning,  "PingPong: The device '%s' missed an interrupt signal.", GetDevice().GetDisplayName().c_str());
						::Sleep(0);
						continue;
					}
				}
				...
				if (!bRunning)
				{
					UE_LOGF(LogAjaCore, Error, "PingPong: Can't wait for the input field for channel %d on device %s.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					break;
				}
		...
		InputChannelThread::EVerifyFormatResult InputChannelThread::VerifyFormat()
		{
			...
			{ 
				...
				while (Counter * SleepMilli < TimeoutMilli)
				{
					...
					if (Counter * SleepMilli > TimeoutMilli)
					{
						UE_LOGF(LogAjaCore, Error, "%s: Can't get the Channel running for channel %d on device %s.\n", ModeName->c_str(), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						break;
					}
				...
				if (!FoundFormat.has_value())
				{
					...
					if (!bAutoDetectFormat)
					{
						UE_LOGF(LogAjaCore, Error, "%s: Could not detect format for channel %d on device %s. %s\n"
							, ModeName->c_str()
							, uint32_t(Channel) + 1
			...
			if (!Helpers::CompareFormats(DesiredVideoFormat, FoundFormat.value(), FailureReason))
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "%s: The VideoFormat changed for channel %d on device %s. %s\n"
						, ModeName->c_str()
						, uint32_t(Channel) + 1
						, GetDevice().GetDisplayName().c_str()
						, FailureReason.c_str()
