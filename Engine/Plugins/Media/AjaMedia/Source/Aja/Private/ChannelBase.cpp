	...
	namespace Private
	{
		...
		*****************************************************************************/
			...
			, Options(InOptions)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "AJAChannel: The port index '%d' is invalid.\n", Options.ChannelIndex);
			}
			else
			{
			...
			if (UseKey())
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "AJAChannel: The key port index '%d' is invalid.\n", Options.KeyChannelIndex);
					Options.bUseKey = false;
				}
				else if (Options.KeyChannelIndex == Options.ChannelIndex)
				{
					UE_LOGF(LogAjaCore, Error, "AJAChannel: The key port index '%d' is the same as the port index '%d'.\n", Options.KeyChannelIndex, Options.ChannelIndex);
					Options.bUseKey = false;
				}
				else
				{
		...
		bool ChannelThreadBase::ChannelThread_Initialization(DeviceConnection::CommandList& InCommandList)
		{
			...
			{
				UE_LOGF(LogAjaCore, Error, "ConfigureVideo: The expected video format is invalid for %d.\n", uint32_t(Channel) + 1);
				return false;
			}
			...
			{
				UE_LOGF(LogAjaCore, Error, "ConfigureVideo: 4k Key and Fill output is not supported for this card.\n");
				return false;
			}

			if (Options.bDirectlyWriteAudio && Options.bUseAutoCirculating)
			{
				UE_LOGF(LogAjaCore, Error, "ConfigureVideo: Direct audio write is only available outside of auto-circulate mode..\n");
				return false;
			}
		...
		bool ChannelThreadBase::DeviceThread_ConfigureVideo(DeviceConnection::CommandList& InCommandList)
		{
			...
			if ((bValidInput || bValidOutput) && bValidKey)
			{
				if (IsInput())
				{
					...
					{
						UE_LOGF(LogAjaCore, Error, "Device: Initialization of the input failed for channel %d on device '%s'. '%s'", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str(), FailureReason.c_str());
						return false;
					}
				}
				else
				{
					...
					{
						UE_LOGF(LogAjaCore, Error, "ConfigureVideo: Unknown video format for output channel %d on device '%s'.\n", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						return false;
					}
				}
				...
				{
					UE_LOGF(LogAjaCore, Error, "ConfigureVideo: The device '%s' doesn't support the video format '%s'.\n", GetDevice().GetDisplayName().c_str(), ::NTV2VideoFormatToString(VideoFormat).c_str());
					return false;
				}
				...
				{
					UE_LOGF(LogAjaCore, Error, "ConfigureVideo: The device '%s' doesn't support the pixel format '%s'.\n", GetDevice().GetDisplayName().c_str(), ::NTV2FrameBufferFormatToString(PixelFormat).c_str());
					return false;
				}
				...
				if (IsOutput())
				{
					...
					if (GetOptions().OutputReferenceType == AJA::EAJAReferenceType::EAJA_REFERENCETYPE_INPUT)
					{
						...
						{
							UE_LOGF(LogAjaCore, Error, "ConfigureVideo: The synchronization port index '%d' is invalid.\n", GetOptions().SynchronizeChannelIndex);
							return false;
						}
						else
						{
			...
			{
				UE_LOGF(LogAjaCore, Error, "ConfigureVideo: Invalid video options for channel %d on device '%s'.", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
				return false;
			}
			...
			else
			{
				...
				if (Helpers::IsSdiTransport(GetOptions().TransportType))
				{
					if (!Helpers::SetSDIOutLevelAtoLevelBConversion(Device->GetCard(), GetOptions().TransportType, Channel, VideoFormat, GetOptions().bConvertOutputLevelAToB))
					{
						...
						{
							UE_LOGF(LogAjaCore, Warning,  "ConfigureVideo: Can't convert level A to B for channel %d on device '%s'.", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
						}
					}
					...
					if (UseKey())
					{
						...
						if (!InCommandList.IsChannelConnect(KeyChannel))
						{
							if (!Helpers::SetSDIOutLevelAtoLevelBConversion(Device->GetCard(), GetOptions().TransportType, KeyChannel, VideoFormat, GetOptions().bConvertOutputLevelAToB))
							{
								if (GetOptions().bConvertOutputLevelAToB)
								{
									UE_LOGF(LogAjaCore, Warning,  "ConfigureVideo: Can't convert level A to B on key for channel %d on device '%s'.", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
								}
							}
		...
		bool ChannelThreadBase::DeviceThread_ConfigureAnc(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (UseAncillary() || UseAncillaryField2())
			{
				...
				if (!bSupportAnc)
				{
					UE_LOGF(LogAjaCore, Warning,  "ConfigureAncillary: Device doesn't support Anc.\n");
		...
		bool ChannelThreadBase::DeviceThread_ConfigureAudio(DeviceConnection::CommandList& InCommandList)
		{
			...
			if (UseAudio() && GetOptions().CallbackInterface != nullptr)
			{
				...
				{
					UE_LOGF(LogAjaCore, Error, "ConfigureAudio: Could not get valid AudioSystem from InputSource %d for channel %d on device %s.", uint32_t(InputSource), uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					return false;
				}
			}
			else
			{
				if (UseAudio())
				{
					UE_LOGF(LogAjaCore, Error, "ConfigureAudio: Invalid audio options for channel %d on device %s.", uint32_t(Channel) + 1, GetDevice().GetDisplayName().c_str());
					return false;
				}
			}
			...
			if (NTV2_IS_VALID_AUDIO_SYSTEM(AudioSystem))
			{
				...
				if (NumberOfAudioChannel > MaxNumOfChannels)
				{
					...
					UE_LOGF(LogAjaCore, Warning,  "ConfigureAudio: Changed number of audio channel to %d.\n", NumberOfAudioChannel);
				}
