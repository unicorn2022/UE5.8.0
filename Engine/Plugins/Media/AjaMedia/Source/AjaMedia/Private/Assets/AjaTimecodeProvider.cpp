...
struct UAjaTimecodeProvider::FAJACallback : public AJA::IAJATimecodeChannelCallbackInterface
{
	...
	virtual void OnInitializationCompleted(bool bSucceed) override
	{
		...
		if (!bSucceed)
		{
			UE_LOGF(LogAjaMedia, Error, "The initialization of '%ls' failed. The TimecodeProvider won't be synchronized.", *Owner->GetName());
		}
	}
};
...
bool UAjaTimecodeProvider::Initialize(class UEngine* InEngine)
{
	...
	{
		State = ETimecodeProviderSynchronizationState::Error;
		UE_LOGF(LogAjaMedia, Warning, "The TimecodeProvider '%ls' can't be initialized. AJA is not initialized on your machine.", *GetName());
		return false;
	}
	...
	{
		State = ETimecodeProviderSynchronizationState::Error;
		UE_LOGF(LogAjaMedia, Warning, "The TimecodeProvider '%ls' can't be initialized because Aja card cannot be used. Are you in a Commandlet? You may override this behavior by launching with -ForceAjaUsage", *GetName());
		return false;
	}
	...
	else
	{
		...
		{
			State = ETimecodeProviderSynchronizationState::Error;
			UE_LOGF(LogAjaMedia, Warning, "The TimecodeProvider '%ls' configuration is invalid.", *GetName());
			return false;
		}
	}
...
void UAjaTimecodeProvider::OnConfigurationAutoDetected(TArray<FAjaDeviceProvider::FMediaIOConfigurationWithTimecodeFormat> InConfigurations, class UEngine* InEngine)
{
	...
	{
		UE_LOGF(LogAjaMedia, Warning, "No configuration was detected for TimecodeProvider '%ls'", *GetName());
		ReleaseResources();
		return;
	}
