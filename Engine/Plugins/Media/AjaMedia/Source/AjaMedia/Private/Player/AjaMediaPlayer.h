...
#if WITH_EDITOR
	virtual bool ShouldAutoRecordSourceOpenedEvent() const override;
#endif
...
#if WITH_EDITOR
	/**
	 * Surface the auto-detected configuration on the source asset so the Media Profile editor can
	 * display the actual signal format instead of the stored default. Safe to call from any thread;
	 * the UObject mutation is dispatched onto the game thread.
	 */
	void PublishAutoDetectedFormatToSource(const FMediaIOConfiguration& InDetectedConfiguration);
#endif
...
private:
	...
	TSharedPtr<FNativeMediaSourceColorSettings, ESPMode::ThreadSafe> AjaColorSettings;
