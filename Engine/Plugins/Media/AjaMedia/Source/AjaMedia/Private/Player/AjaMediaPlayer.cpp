...
FAjaMediaPlayer::FAjaMediaPlayer(IMediaEventSink& InEventSink)
	...
	, AjaColorSettings(MakeShared<FNativeMediaSourceColorSettings>())
...
bool FAjaMediaPlayer::Open(const FString& Url, const IMediaOptions* Options)
{
	...
	{
		UE_LOGF(LogAjaMedia, Warning, "The AjaMediaPlayer can't open URL '%ls' because Aja card cannot be used. Are you in a Commandlet? You may override this behavior by launching with -ForceAjaUsage", *Url);
		return false;
	}
...
void FAjaMediaPlayer::VerifyFrameDropCount()
{
	...
	if (bVerifyFrameDropCount)
	{
		...
		if (FrameDropCount > LastFrameDropCount)
		{
			...
			{
				UE_LOGF(LogAjaMedia, Warning, "Loosing frames on AJA input %ls. The current count is %d.", *GetUrl(), PreviousFrameDropCount);
			}
		}
		else if (PreviousFrameDropCount > 0)
		{
			UE_LOGF(LogAjaMedia, Warning, "Lost %d frames on input %ls. Unreal Engine's frame rate is too slow and the capture card was not able to send the frame(s) to Unreal.", PreviousFrameDropCount, *GetUrl());
		...
		if (!IsJustInTimeRenderingEnabled())
		{
			...
			{
				UE_LOGF(LogAjaMedia, Warning, "Lost %d metadata frames on input %ls. Frame rate is either too slow or buffering capacity is too small.", DeltaMetadataDropCount, *GetUrl());
			}
			...
			{
				UE_LOGF(LogAjaMedia, Warning, "Lost %d audio frames on input %ls. Frame rate is either too slow or buffering capacity is too small.", DeltaAudioDropCount, *GetUrl());
			}
			...
			{
				UE_LOGF(LogAjaMedia, Warning, "Lost %d video frames on input %ls. Frame rate is either too slow or buffering capacity is too small.", DeltaVideoDropCount, *GetUrl());
			}
		}
	}
}
...
void FAjaMediaPlayer::OnFormatChange(AJA::FAJAVideoFormat Format)
{
	...
	UE_LOGF(LogAjaMedia, Display, "New configuration was detected for AjaMediaPlayer: %ls", *VideoTrackFormat.TypeName);
...
bool FAjaMediaPlayer::OnRequestInputBuffer(const AJA::AJARequestInputBufferData& InRequestBuffer, AJA::AJARequestedInputBufferData& OutRequestedBuffer)
{
	...
	if (bUseVideo && InRequestBuffer.VideoBufferSize > 0 && InRequestBuffer.bIsProgressivePicture)
	{
		...
		if (!bIsJustInTimeRenderingEnabled && bCanUseGPUTextureTransfer)
		{
			...
			{
				UE_LOGF(LogAjaMedia, Error, "No texture available while doing a gpu texture transfer.");
				return false;
			}
		}
...
bool FAjaMediaPlayer::OnInputFrameReceived(const AJA::AJAInputFrameData& InInputFrame, const AJA::AJAAncillaryFrameData& InAncillaryFrame, const AJA::AJAAudioFrameData& InAudioFrame, const AJA::AJAVideoFrameData& InVideoFrame)
{
	...
	if (bUseFrameTimecode)
	{
		...
		{
			UE_LOGF(LogAjaMedia, Warning, "Input %ls received an invalid Timecode frame number (%d) for the current frame rate (%ls).", *GetUrl(), InInputFrame.Timecode.Frames, *VideoFrameRate.ToPrettyText().ToString());
		}
		...
		{
			UE_LOGF(LogAjaMedia, Log, "Input %ls has timecode : %02d:%02d:%02d:%02d", *GetUrl(), InInputFrame.Timecode.Hours, InInputFrame.Timecode.Minutes, InInputFrame.Timecode.Seconds, InInputFrame.Timecode.Frames);
		}
	}
	...
	if (bUseVideo && InVideoFrame.VideoBuffer)
	{
		...
		ensure(AjaColorSettings.IsValid());

		const UE::Color::EEncoding SourceEncoding = SourceColorSettings.IsValid() ? SourceColorSettings->GetEncodingOverride() : UE::Color::EEncoding::None;
		if (SourceEncoding != UE::Color::EEncoding::None)
		{
			...
			AjaColorSettings->SetEncodingOverride(SourceEncoding);
			AjaColorSettings->SetReferenceWhiteOverride(SourceColorSettings->GetReferenceWhiteOverride());
		}
		else
		{
			...
			case EAjaHDRMetadataEOTF::SDR:
				AjaColorSettings->SetEncodingOverride(UE::Color::EEncoding::sRGB); // Missing support for BT.1886 so we default to sRGB instead.
				break;
			case EAjaHDRMetadataEOTF::PQ:
				AjaColorSettings->SetEncodingOverride(UE::Color::EEncoding::ST2084);
				break;
			case EAjaHDRMetadataEOTF::HLG:
				AjaColorSettings->SetEncodingOverride(UE::Color::EEncoding::HLG);
				break;
			default:
				checkNoEntry();
			}
			...
			AjaColorSettings->SetReferenceWhiteOverride(UE::Color::EReferenceWhite::None);
		}

		if (SourceColorSettings.IsValid() && SourceColorSettings->HasColorSpaceOverride())
		{
			AjaColorSettings->SetColorSpaceOverride(SourceColorSettings->GetColorSpaceOverride(UE::Color::FColorSpace::GetSRGB()));
		}
		else
		{
			...
			case EAjaHDRMetadataGamut::Rec709:
				AjaColorSettings->SetColorSpaceOverride(UE::Color::FColorSpace::GetSRGB());
				break;
			case EAjaHDRMetadataGamut::Rec2020:
				AjaColorSettings->SetColorSpaceOverride(UE::Color::FColorSpace::GetRec2020());
				break;
			default:
				checkNoEntry();
			}
		...
		{
			if (AjaThreadCurrentTextureSample->SetProperties(InVideoFrame.Stride, InVideoFrame.Width, InVideoFrame.Height, VideoSampleFormat, DecodedTime, VideoFrameRate, DecodedTimecode, AjaColorSettings))
			{
		...
		else
		{
			...
			else
			{
				if (InVideoFrame.bIsProgressivePicture)
				{
					...
					if (TextureSample->InitializeProgressive(InVideoFrame, VideoSampleFormat, DecodedTime, VideoFrameRate, DecodedTimecode, AjaColorSettings))
					{
...
bool FAjaMediaPlayer::Open_Internal(const FString& Url, const IMediaOptions* Options, AJA::AJAInputOutputChannelOptions AjaOptions)
{
	...
	{
		UE_LOGF(LogAjaMedia, Warning, "The AJA port couldn't be opened.");
...
void FAjaMediaPlayer::OnAutoDetected(TArray<FAjaDeviceProvider::FMediaIOConfigurationWithTimecodeFormat> Configurations, FString Url, const IMediaOptions* Options, bool bAutoDetectVideoFormat, bool bAutoDetectTimecodeFormat)
{
...
#if WITH_EDITOR
				PublishAutoDetectedFormatToSource(Configuration.Configuration);
#endif
			}
	...
	{
		UE_LOGF(LogAjaMedia, Warning, "No configuration was detected for MediaPlayer.");
		Close();
		return;
	}
...
#if WITH_EDITOR
void FAjaMediaPlayer::PublishAutoDetectedFormatToSource(const FMediaIOConfiguration& InDetectedConfiguration)
{
	TWeakObjectPtr<UObject> WeakSourceObj = MediaSourceObject;
	FMediaIOConfiguration Detected = InDetectedConfiguration;
	AsyncTask(ENamedThreads::GameThread, [WeakSourceObj, Detected]()
	{
		if (UCaptureCardMediaSource* Source = Cast<UCaptureCardMediaSource>(WeakSourceObj.Get()))
		{
...
#if WITH_EDITOR
bool FAjaMediaPlayer::ShouldAutoRecordSourceOpenedEvent() const
{
	// AJA can open asynchronously, so it will call RecordSourceOpenedEvent manually
	return false;
}
#endif
...
#if WITH_EDITOR
FString FAjaMediaPlayer::GetAnalyticsEventPrefix() const
{
	return TEXT("MediaFramework.Aja");
}

void FAjaMediaPlayer::GetAnalyticsEventAttributes(const FString& EventName, TArray<FAnalyticsEventAttribute>& InOutAttributes) const
{
	FMediaIOCorePlayerBase::GetAnalyticsEventAttributes(EventName, InOutAttributes);
	
	if (InputChannel)
	{
		const int64 DeviceIndex = InputChannel->GetDeviceOptions().DeviceIndex;
	...
	if (AjaColorSettings->HasColorSpaceOverride())
	{
		const UE::Color::FColorSpace& ColorSpace = AjaColorSettings->GetColorSpaceOverride(UE::Color::FColorSpace::GetSRGB());
	...
	if (AjaColorSettings->HasEncodingOverride())
	{
		UE::Color::EEncoding Encoding = AjaColorSettings->GetEncodingOverride();
