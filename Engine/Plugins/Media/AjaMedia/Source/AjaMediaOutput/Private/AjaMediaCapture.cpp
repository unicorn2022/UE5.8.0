...
bool UAjaMediaCapture::ValidateMediaOutput() const
{
	...
	if (!AjaMediaOutput)
	{
		UE_LOGF(LogAjaMediaOutput, Error, "Can not start the capture. MediaSource's class is not supported.");
		return false;
	}
...
bool UAjaMediaCapture::UpdateMediaTextureImpl(UMediaTexture* InMediaTexture)
{
	RestoreViewportTextureAlpha(GetCapturingSceneViewport());
	return true;
}
...
#if WITH_EDITOR
FString UAjaMediaCapture::GetAnalyticsEventPrefix() const
{
	return TEXT("MediaFramework.Aja"); 
}

void UAjaMediaCapture::GetAnalyticsEventAttributes(const FString& EventName, TArray<FAnalyticsEventAttribute>& InOutAttributes) const
{
	Super::GetAnalyticsEventAttributes(EventName, InOutAttributes);

	UAjaMediaOutput* AjaMediaOutput = CastChecked<UAjaMediaOutput>(MediaOutput);

	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("Framerate"), AjaMediaOutput->OutputConfiguration.MediaConfiguration.MediaMode.FrameRate.ToPrettyText().ToString()));
	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("Device"), AjaMediaOutput->OutputConfiguration.MediaConfiguration.MediaConnection.Device.DeviceName));

	if (IMediaIOCoreDeviceProvider* DeviceProvider = IMediaIOCoreModule::Get().GetDeviceProvider(FAjaDeviceProvider::GetProviderName()))
	{
		const FString DestStr = FString::Format(TEXT("{0} {1}"), {
			DeviceProvider->GetTransportName(
				AjaMediaOutput->OutputConfiguration.MediaConfiguration.MediaConnection.TransportType,
				AjaMediaOutput->OutputConfiguration.MediaConfiguration.MediaConnection.QuadTransportType).ToString(),
			AjaMediaOutput->OutputConfiguration.MediaConfiguration.MediaConnection.PortIdentifier });
		InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("Destination"), DestStr));
	}

	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("Standard"), UEnum::GetValueAsString(AjaMediaOutput->OutputConfiguration.MediaConfiguration.MediaMode.Standard)));
	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("Reference"), UEnum::GetValueAsString(AjaMediaOutput->OutputConfiguration.OutputReference)));
}
#endif
...
bool UAjaMediaCapture::InitAJA(UAjaMediaOutput* InAjaMediaOutput)
{
	...
	{
		UE_LOGF(LogAjaMediaOutput, Warning, "The AjaMediaCapture can't open MediaOutput '%ls' because Aja card cannot be used. Are you in a Commandlet? You may override this behavior by launching with -ForceAjaUsage", *InAjaMediaOutput->GetName());
		return false;
	}
	...
	if (ShouldCaptureRHIResource())
	{
		...
		{
			UE_LOGF(LogAjaMediaOutput, Warning, "GPU DMA is not supported with interlaced, defaulting to regular path.");
	...
	if (bOutputAudio)
	{
		...
		{
			UE_LOGF(LogAjaMediaOutput, Error, "Failed to initialize audio output.");
		}
	}
	...
	{
		UE_LOGF(LogAjaMediaOutput, Warning, "The AJA output port for '%ls' could not be opened.", *InAjaMediaOutput->GetName());
		OutputChannel.Reset();
		OutputCallback.Reset();
		return false;
	}
	...
	if (bWaitForSyncEvent)
	{
		...
		if (bLockToVsync)
		{
			UE_LOGF(LogAjaMediaOutput, Warning, "The Engine use VSync and something wants to wait for the sync event. This may break the \"gen-lock\".");
		}
	...
	if (ShouldCaptureRHIResource())
	{
		UE_LOGF(LogAjaMediaOutput, Display, "Aja capture started using GPU Direct");
	}
...
bool UAjaMediaCapture::FAjaOutputCallback::OnOutputFrameCopied(const AJA::AJAOutputFrameData& InFrameData)
{
	...
	if (Owner->bLogDropFrame)
	{
		if (FrameDropCount > LastFrameDropCount)
		{
			...
			{
				UE_LOGF(LogAjaMediaOutput, Warning, "Loosing frames on AJA output %ls. The current count is %lld.", *Owner->PortName, PreviousDroppedCount);
			}
		}
		else if (PreviousDroppedCount > 0)
		{
			UE_LOGF(LogAjaMediaOutput, Warning, "Lost %lld frames on AJA output %ls. Frame rate may be too slow.", PreviousDroppedCount, *Owner->PortName);
