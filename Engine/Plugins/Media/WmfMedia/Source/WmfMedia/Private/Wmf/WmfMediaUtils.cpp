	...
	void InitializeSupportedTypes()
	{
		...
		for (const FAudioFormat& Format : SupportedAudioFormats)
		{
			for (UINT32 NumChannels : SupportedAudioChannels)
			{
				...
				{
					UE_LOGF(LogWmfMedia, Error, "Failed to initialize supported audio type: %ls, %i bits/sample, %i channels", *SubTypeToString(*Format.SubType), Format.BitsPerSample, NumChannels);
				}
		...
		// initialize binary types
		{
			...
			if (FAILED(::MFCreateMediaType(&MediaType)) ||
				...
				FAILED(MediaType->SetUINT32(MF_MT_ALL_SAMPLES_INDEPENDENT, TRUE)))
			{
				UE_LOGF(LogWmfMedia, Error, "Failed to initialize supported binary types");
			}
		...
		// initialize SAMI types
		{
			...
			if (FAILED(::MFCreateMediaType(&MediaType)) ||
				...
				FAILED(MediaType->SetUINT32(MF_MT_ALL_SAMPLES_INDEPENDENT, TRUE)))
			{
				UE_LOGF(LogWmfMedia, Error, "Failed to initialize supported SAMI types");
			}
		...
		for (GUID const* const Format : SupportedVideoFormats)
		{
			...
			{
				UE_LOGF(LogWmfMedia, Error, "Failed to initialize supported video type: %ls", *SubTypeToString(*Format));
			}
...
namespace WmfMedia
{
	...
	TComPtr<IMFMediaType> CreateOutputType(IMFMediaType& InputType, bool AllowNonStandardCodecs, bool IsVideoDevice)
	{
		GUID MajorType;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to get major type: %ls", *ResultToString(Result));
				return NULL;
			}
		}
		...
		GUID SubType;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to get sub-type: %ls", *ResultToString(Result));
				return NULL;
			}
		}
		...
		TComPtr<IMFMediaType> OutputType;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to create %ls output type: %ls", *MajorTypeToString(MajorType), *ResultToString(Result));
				return NULL;
			}
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to initialize %ls output type: %ls", *MajorTypeToString(MajorType), *ResultToString(Result));
				return NULL;
			}
		}
		...
		if (MajorType == MFMediaType_Audio)
		{
			...
			if (IsFormatFamily(SubType, MFMPEG4Format_Base))
			{
				...
				{
					UE_LOGF(LogWmfMedia, Verbose, "Allowing non-standard MP4 audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				}
				else
				{
					...
					{
						UE_LOGF(LogWmfMedia, Warning, "Skipping non-standard MP4 audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
						return NULL;
					}
				}
			}
			else if (!IsFormatFamily(SubType, MFAudioFormat_Base))
			{
				if (AllowNonStandardCodecs)
				{
					UE_LOGF(LogWmfMedia, Verbose, "Allowing non-standard audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				}
				else
				{
					UE_LOGF(LogWmfMedia, Warning, "Skipping non-standard audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
					return NULL;
				}
			}
			...
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to initialize audio output type");
				return NULL;
			}
			...
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to copy audio output type attributes");
				return NULL;
			}
		}
		else if (MajorType == MFMediaType_Binary)
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to initialize binary output type: %ls", *ResultToString(Result));
				return NULL;
			}
		}
		else if (MajorType == MFMediaType_SAMI)
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to initialize caption output type: %ls", *ResultToString(Result));
				return NULL;
			}
		}
		else if (MajorType == MFMediaType_Video)
		{
			...
			if (!IsFormatFamily(SubType, MFVideoFormat_Base))
			{
				if (AllowNonStandardCodecs)
				{
					UE_LOGF(LogWmfMedia, Verbose, "Allowing non-standard video type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				}
				else
				{
					UE_LOGF(LogWmfMedia, Warning, "Skipping non-standard video type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
					return NULL;
				}
			}
			...
			if ((SubType == MFVideoFormat_H264) || (SubType == MFVideoFormat_H264_ES))
			{
				...
				{
					UE_LOGF(LogWmfMedia, Warning, "H264 video type requires Windows 8 or newer (your version is %ls)", *FPlatformMisc::GetOSVersion());
					return NULL;
				}
			}
			...
			if ((SubType == MFVideoFormat_HEVC) || (SubType == MFVideoFormat_HEVC_ES))
			{
				if (!FPlatformMisc::VerifyWindowsVersion(10, 0) /*Win10*/)
				{
					...
					{
						UE_LOGF(LogWmfMedia, Warning, "HEVC video type requires Windows 10 or newer (your version is %ls)", *FPlatformMisc::GetOSVersion());
						return NULL;
					}

					UE_LOGF(LogWmfMedia, Warning, "HEVC video type requires Windows 10 or newer (your version is %ls), and game must be manifested for Windows 10", *FPlatformMisc::GetOSVersion());
				}
			}
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to set video output type: %ls", *ResultToString(Result));
				return NULL;
			}

			UE_LOGF(LogWmfMedia, Verbose, "SubType: { 0x%08X, 0x%04X, 0x%04X, { 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X } };",
				SubType.Data1,
				SubType.Data2,
				SubType.Data3,
				SubType.Data4[0],
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Warning, "Failed to set video output sub-type: %ls", *ResultToString(Result));
				return NULL;
			}
			...
			if (IsVideoDevice)
			{
				...
				{
					UE_LOGF(LogWmfMedia, Warning, "Failed to copy video output type attributes");
					return NULL;
				}
			}
		}
	...
	void EnumerateCaptureDevices(GUID DeviceType, TArray<TComPtr<IMFActivate>>& OutDevices)
	{
		...
		TComPtr<IMFAttributes> Attributes;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Error, "Failed to create capture device enumeration attributes: %ls", *ResultToString(Result));
				return;
			}
		}
		...
		// request capture devices
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Error, "Failed to set capture device enumeration type: %ls", *ResultToString(Result));
				return;
			}
		}
...
#pragma warning(pop)
	...
	TComPtr<IMFMediaSource> ResolveMediaSource(TSharedPtr<FArchive, ESPMode::ThreadSafe> Archive, const FString& Url, bool Precache)
	{
		if (!Archive.IsValid())
		{
			...
			if (IsAudioDevice || Url.StartsWith(TEXT("vidcap://")))
			{
				...
				TComPtr<IMFAttributes> Attributes;
				{
					...
					if (FAILED(Result))
					{
						UE_LOGF(LogWmfMedia, Error, "Failed to create capture device attributes: %ls", *ResultToString(Result));
						return NULL;
					}
					...
					if (FAILED(Result))
					{
						UE_LOGF(LogWmfMedia, Error, "Failed to set capture device source type attribute: %ls", *ResultToString(Result));
						return NULL;
					}
					...
					if (FAILED(Result))
					{
						UE_LOGF(LogWmfMedia, Error, "Failed to set capture device endpoint/symlink attribute: %ls", *ResultToString(Result));
						return NULL;
					}
				}
				...
				TComPtr<IMFMediaSource> MediaSource;
				{
					...
					if (FAILED(Result))
					{
						UE_LOGF(LogWmfMedia, Error, "Failed to create capture device media source: %ls", *ResultToString(Result));
						return NULL;
					}
				}
			...
			if (Url.StartsWith(TEXT("file://")))
			{
				...
				if (!Archive.IsValid())
				{
					UE_LOGF(LogWmfMedia, Error, "Failed to open or read media file %ls", FilePath);
					return NULL;
				}

				if (Archive->TotalSize() == 0)
				{
					UE_LOGF(LogWmfMedia, Error, "Cannot open media from empty file %ls.", FilePath);
					return NULL;
				}
			}
		}
		...
		TComPtr<IMFSourceResolver> SourceResolver;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Error, "Failed to create media source resolver: %ls", *ResultToString(Result));
				return NULL;
			}
		}
		...
		TComPtr<IUnknown> SourceObject;
		{
			...
			if (Archive.IsValid())
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogWmfMedia, Error, "Failed to resolve byte stream %ls: %ls", *Url, *ResultToString(Result));
					return NULL;
				}
			}
			else
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogWmfMedia, Error, "Failed to resolve URL %ls: %ls", *Url, *ResultToString(Result));
					return NULL;
				}
			}
		}
		...
		TComPtr<IMFMediaSource> MediaSource;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Error, "Failed to query media source interface: %ls", *ResultToString(Result));
				return NULL;
			}
		}
