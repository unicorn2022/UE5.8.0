...
{
	UE_LOGF(LogWmfMedia, Verbose, "StreamSink %p: Created with stream type %ls", this, *WmfMedia::MajorTypeToString(StreamType));
}
...
FWmfMediaStreamSink::~FWmfMediaStreamSink()
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "StreamSink %p: Destroyed", this);
}
...
bool FWmfMediaStreamSink::Initialize(FWmfMediaSink& InOwner)
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "StreamSink %p: Failed to create event queue for stream sink: %ls", this, *WmfMedia::ResultToString(Result));
		return false;
	}
...
{
	FScopeLock Lock(&CriticalSection);
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink::Pause");
...
HRESULT FWmfMediaStreamSink::Restart()
{
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p:Restart Request Sample", this);
...
HRESULT FWmfMediaStreamSink::Start()
{
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p:Start Request Sample", this);
...
HRESULT FWmfMediaStreamSink::Stop()
{
	...
	{
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink::Stop");
...
STDMETHODIMP FWmfMediaStreamSink::IsMediaTypeSupported(IMFMediaType* pMediaType, _Outptr_opt_result_maybenull_ IMFMediaType** ppMediaType)
{
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Checking if media type is supported:\n%ls", this, *WmfMedia::DumpAttributes(*pMediaType));
	...
	{
		UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Media type doesn't match stream type %ls", this, *WmfMedia::MajorTypeToString(StreamType));
		return MF_E_INVALIDMEDIATYPE;
	}
	...
	for (const TComPtr<IMFMediaType>& MediaType : WmfMedia::GetSupportedMediaTypes(StreamType))
	{
		...
		{
			UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Media type is supported", this);
			return S_OK;
		}
	}

	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Media type is not supported", this);
...
STDMETHODIMP FWmfMediaStreamSink::SetCurrentMediaType(IMFMediaType* pMediaType)
{
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Setting current media type:\n%ls", this, *WmfMedia::DumpAttributes(*pMediaType));
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Tried to set unsupported media type", this);
		return Result;
	}

	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Current media type set", this);
...
STDMETHODIMP FWmfMediaStreamSink::PlaceMarker(MFSTREAMSINK_MARKER_TYPE eMarkerType, __RPC__in const PROPVARIANT* pvarMarkerValue, __RPC__in const PROPVARIANT* pvarContextValue)
{
	...
	if (pvarContextValue != NULL)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "StreamSink %p: Failed to copy marker context: %ls", this, *WmfMedia::ResultToString(Result));
...
{
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Process Sample", this);
	...
	if (!CurrentMediaType.IsValid())
	{
		UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink %p: Stream received a sample while not having a valid media type set", this);
		return MF_E_INVALIDMEDIATYPE;
	}
	...
	LONGLONG Time = 0;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, VeryVerbose, "Failed to get time from sink sample: %ls", *WmfMedia::ResultToString(Result));
			return Result;
		}
	}

	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink::ProcessSample Sample time %f", FTimespan::FromMicroseconds(Time / 10).GetTotalSeconds());
	...
	if (!bSeekInProgress)
	{
		...
		{
			UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink::ProcessSample Request Sample2");
			QueueEvent(MEStreamSinkRequestSample, GUID_NULL, S_OK, NULL);
			++NumRequestsInFlight;
		}
	}
...
void FWmfMediaStreamSink::CopyTextureAndEnqueueSample(IMFSample* pSample)
{
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "StreamSink::CopyTextureAndEnqueueSample Queue Size: %d", VideoSampleQueue->Num());

	if (!IsSampleStillDesired())
	{
		UE_LOGF(LogWmfMedia, VeryVerbose, "Sample is from previous sequence, dropping sample");
		return;
	}

	if (IsVideoSampleQueueFull())
	{
		UE_LOGF(LogWmfMedia, VeryVerbose, "Queue is full, dropping sample");
		return;
	}
	...
	if (SUCCEEDED(CurrentMediaType->GetGUID(MF_MT_SUBTYPE, &Guid)))
	{
		...
		if ((Decoder != nullptr) && (Decoder->IsExternalBufferEnabled()))
		{
			...
			{
				UE_LOGF(LogWmfMedia, Error, "External buffer not found for time %f",
			...
			UE_LOGF(LogWmfMedia, VeryVerbose, "Enqueued external buffer onto VideoSampleQueue.");
		}
		else
		{
			...
			if (RHIGetInterfaceType() == ERHIInterfaceType::D3D11)
			{
				...
				UE_LOGF(LogWmfMedia, VeryVerbose, "CopySubresourceRegion() ViewIndex:%d Time:%f", dwViewIndex, FTimespan::FromMicroseconds(SampleTime / 10).GetTotalSeconds());
				...
				if (KeyedMutex)
				{
					...
					if (KeyedMutex->AcquireSync(0, 0) == S_OK)
					{
						...
						VideoSampleQueue->Enqueue(TextureSample);
						UE_LOGF(LogWmfMedia, VeryVerbose, "Enqueued onto VideoSampleQueue.");
					}
				}
			}
			else
	...
	else
	{
		...
		{
			UE_LOGF(LogWmfMedia, Log, "StreamSink %p: Unable to query MF_MT_SUBTYPE GUID of current media type", this);
