...
STDMETHODIMP FWmfMediaSink::SetPresentationClock(__RPC__in_opt IMFPresentationClock* pPresentationClock)
{
	...
	if (PresentationClock.IsValid())
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Error, "Failed to remove media sink from presentation clock: %ls", *WmfMedia::ResultToString(Result));
			return Result;
		}
	}
	...
	if (pPresentationClock != NULL)
	{
		...
		HRESULT Result = pPresentationClock->QueryInterface(IID_PPV_ARGS(&RateControl));
 		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Error, "Failed to retrieve rate interface from presentation clock: %ls", *WmfMedia::ResultToString(Result));
			return Result;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Error, "Failed to retrieve rate from presentation clock: %ls", *WmfMedia::ResultToString(Result));
			return Result;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Error, "Failed to add media sink to presentation clock: %ls", *WmfMedia::ResultToString(Result));
			return Result;
		}
	}
	else
...
bool FWmfMediaSink::CreateDXGIManagerAndDevice()
{
	...
	if (nullptr == DXGIManager)
	{
		if (GDynamicRHI == nullptr)
		{
			UE_LOGF(LogWmfMedia, Error, "Invalid Dynamic RHI");
			return false;
		}
		...
		if(RHIType == ERHIInterfaceType::D3D11)
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Error, "Unable to Create DXGI Device Manager: %ls", *WmfMedia::ResultToString(Result));
				return false;
			}

			ID3D11Device* PreExistingD3D11Device = GetID3D11DynamicRHI()->RHIGetDevice();
		...
		else
		{
			DXGIManager.Reset();
			D3D11Device.Reset();
			UE_LOGF(LogWmfMedia, Verbose, "Dynamic RHI is not D3D11, skipping DX11 device creation for AV decoding");
			return true;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Error, "Unable to Create D3D11 Device: %ls", *WmfMedia::ResultToString(Result));
			return false;
		}

		if (FeatureLevel < D3D_FEATURE_LEVEL_9_3)
		{
			UE_LOGF(LogWmfMedia, Error, "Unable to Create D3D11 Device with feature level 9.3 or above");
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Error, "Unable to Reset D3D11 Device: %ls", *WmfMedia::ResultToString(Result));
			return false;
		}
		...
		{
			UE_LOGF(LogWmfMedia, Error, "Unable to activate multi-threading on device: %p", D3D11Device.Get());
			return false;
		}

		UE_LOGF(LogWmfMedia, Verbose, "D3D11 Device Created: %p", D3D11Device.Get());
		return true;
	}
	else
	{
