...
{
	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Created", this);
...
FWmfMediaSession::~FWmfMediaSession()
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Destroyed", this);
}
...
bool FWmfMediaSession::Initialize(bool LowLatency)
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Initializing (LowLatency: %d)", this, LowLatency);
	...
	TComPtr<IMFAttributes> Attributes;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to create media session attributes: %ls", this, *WmfMedia::ResultToString(Result));
			return false;
		}
	}
	...
	if (LowLatency)
	{
		if (FPlatformMisc::VerifyWindowsVersion(6, 2))
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to set low latency session attribute: %ls", this, *WmfMedia::ResultToString(Result));
			}
		}
		else
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Low latency media processing requires Windows 8 or newer", this);
		}
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to create media session: %ls", this, *WmfMedia::ResultToString(Result));
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to start media session event processing: %ls", this, *WmfMedia::ResultToString(Result));
		return false;
	}
...
bool FWmfMediaSession::SetTopology(const TComPtr<IMFTopology>& InTopology, FTimespan InDuration)
{
	...
	EnqueueWorkItem(FWorkItem([this, InTopology, InDuration](TComPtr<IMFMediaEvent> MediaEvent, MediaEventType EventType, HRESULT EventStatus, FBaseWorkItemState* BaseState) -> EWorkItemResult
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session: %p: Setting new partial topology %p (duration = %ls)", this, InTopology.Get(), *InDuration.ToString());
			...
			if (SessionState == EMediaState::Preparing)
			{
				...
				if (InTopology != nullptr)
				{
					...
					if (FAILED(Result))
					{
						UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to set partial topology %p: %ls", this, InTopology.Get(), *WmfMedia::ResultToString(Result));
...
void FWmfMediaSession::Shutdown()
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Session: %p: Shutting down", this);
	...
	if (SessionState != EMediaState::Error)
	{
		...
		if (hr == S_OK)
		{
			...
			UE_LOGF(LogWmfMedia, Verbose, "Session: %p: Close", this);
		}
		else
		{
			UE_LOGF(LogWmfMedia, Warning, "Session: %p: Unable to close", this);
		}
	}
...
bool FWmfMediaSession::Seek(const FTimespan& Time, const FMediaSeekParams& InAdditionalParams)
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Seeking to %ls with sequence index %d", this, *Time.ToString(), InAdditionalParams.NewSequenceIndex.GetValue());

	// validate seek
	if (!CanControl(EMediaControl::Seek))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Media source doesn't support seeking", this);
		return false;
	}

	if ((Time < FTimespan::Zero()) || (Time > CurrentDuration))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Invalid seek time %ls (media duration is %ls)", this, *Time.ToString(), *CurrentDuration.ToString());
		return false;
	}
	...
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Cannot seek while closed or in error state", this);
		return false;
	}
...
bool FWmfMediaSession::SetRate(float Rate)
{
	...
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: The rate %f is not supported", this, Rate);
		return false;
	}
	...
	EnqueueWorkItem(FWorkItem([this, Rate](TComPtr<IMFMediaEvent> MediaEvent, MediaEventType EventType, HRESULT EventStatus, FBaseWorkItemState* BaseState) -> EWorkItemResult
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Setting rate to %f", this, Rate);
...
STDMETHODIMP FWmfMediaSession::Invoke(IMFAsyncResult* AsyncResult)
{
	...
	TComPtr<IMFMediaEvent> Event;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to get event: %ls", this, *WmfMedia::ResultToString(Result));
			return S_OK;
		}
	}
	...
	MediaEventType EventType = MEUnknown;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to get session event type: %ls", this, *WmfMedia::ResultToString(Result));
			return S_OK;
		}
	}
	...
	HRESULT EventStatus = S_FALSE;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to get event status: %ls", this, *WmfMedia::ResultToString(Result));
		}
	}

	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Event [%ls]: %ls", this, *WmfMedia::MediaEventToString(EventType), *WmfMedia::ResultToString(EventStatus));
	...
	if ((EventType != MESessionClosed) && (SessionState != EMediaState::Error) && (MediaSession != NULL))
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to request next session event; aborting playback: %ls", this, *WmfMedia::ResultToString(Result));
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "Session %p: CurrentState: %ls, CurrentRate: %f, CurrentTime: %ls, SessionState: %ls, SessionRate: %f",
		this,
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "Session %p: WaitingForEnd:%d",
		this,
		bIsWaitingForEnd);
...
FWmfMediaSession::EWorkItemResult FWmfMediaSession::CommitRate(float Rate, FRateWorkItemState* State, MediaEventType EventType)
{
	...
	UE_LOGF(LogWmfMedia, VeryVerbose, "Session %p: Committing rate %f [S=%d]", this, Rate, int(State->State));
		...
		if (State->State == FRateWorkItemState::EState::Begin)
		{
			if (Rate == 0.0f)
			{
				if (SessionState == EMediaState::Playing)
				{
					...
					if (FAILED(Result))
					{
						UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to pause session: %ls", this, *WmfMedia::ResultToString(Result));
						return EWorkItemResult::Done;
					}
			...
			else
			{
				if (SessionState != EMediaState::Playing)
				{
					...
					if (FAILED(Result))
					{
						UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to start session: %ls", this, *WmfMedia::ResultToString(Result));
						return EWorkItemResult::Done;
					}
	...
	if (State->State == FRateWorkItemState::EState::Begin)
	{
		...
		if (Rate != 0.0f && ((UnpausedSessionRate * Rate) < 0.0f))
		{
			...
			if (SessionState != EMediaState::Stopped)
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to stop for rate change: %ls", this, *WmfMedia::ResultToString(Result));
					return EWorkItemResult::Done;
				}
						...
						if (CanScrub)
						{
							...
							*/
							...
							if (UnpausedSessionRate >= 0.0f)
							{
								...
								if (FAILED(Result))
								{
									UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to pause for rate change: %ls", this, *WmfMedia::ResultToString(Result));
									return EWorkItemResult::Done;
								}
	...
	// ...and set the rate
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to change rate: %ls", this, *WmfMedia::ResultToString(Result));
			return EWorkItemResult::Done;
		}
	}
...
bool FWmfMediaSession::CommitTime(FTimespan Time, bool bIsSeek)
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Committing time %ls (Seek=%d)", this, *TimeString, bIsSeek);
	...
	if (!bCanSeek)
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Starting from <current>, because media can't seek", this);
...
FWmfMediaSession::EWorkItemResult FWmfMediaSession::CommitTopology(IMFTopology* Topology, FTopologyWorkItemState* State, MediaEventType EventType)
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Committing topology %p [S=%d]", this, Topology, int(State->State));
	...
	if (State->State == FTopologyWorkItemState::EState::Begin)
	{
		...
		if (SessionState != EMediaState::Stopped)
		{
			...
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Stopping session for topology change", this);

			// topology change requires transition to Stopped; playback is resumed afterwards
			HRESULT Result = MediaSession->Stop();
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to stop for topology change: %ls", this, *WmfMedia::ResultToString(Result));
				return EWorkItemResult::Done;
			}
	...
	else if (State->State == FTopologyWorkItemState::EState::ReadyNeedsRestart)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to start after topology change: %ls", this, *WmfMedia::ResultToString(Result));
			return EWorkItemResult::Done;
		}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to clear queued topologies: %ls", this, *WmfMedia::ResultToString(Result));
		return EWorkItemResult::Done;
	}
	...
	if (Topology != nullptr)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to set topology %p: %ls", Topology, this, *WmfMedia::ResultToString(Result));
			return EWorkItemResult::Done;
		}
	}
	else
	...
	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Committed topology %p", this, Topology);
...
void FWmfMediaSession::UpdateCharacteristics()
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Session clock unavailable: %ls", this, *WmfMedia::ResultToString(Result));
	}
	else
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Presentation clock unavailable: %ls", this, *WmfMedia::ResultToString(Result));
		}
		else
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Presentation clock ready", this);
		}
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Rate control service unavailable: %ls", this, *WmfMedia::ResultToString(Result));
	}
	else
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Rate control ready", this);

		float NewRate;
		if (FAILED(RateControl->GetRate(FALSE, &NewRate)))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to initialize current rate", this);
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Rate support service unavailable: %ls", this, *WmfMedia::ResultToString(Result));
	}
	else
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Rate support ready", this);
	}
...
{
	UE_LOGF(LogWmfMedia, Error, "An error occurred in the media session: %ls", *WmfMedia::ResultToString(EventStatus));
...
{
	UE_LOGF(LogWmfMedia, VeryVerbose, "FWmfMediaSession::HandleSessionEnded ShouldLoop:%d", ShouldLoop);
...
void FWmfMediaSession::HandleSessionStarted(HRESULT EventStatus)
{
	if (SUCCEEDED(EventStatus))
	{
		...
		UE_LOGF(LogWmfMedia, VeryVerbose, "Session - Started: R=%f SState=%d", SessionRate, int(SessionState));
...
void FWmfMediaSession::HandleSessionTopologySet(HRESULT EventStatus, IMFMediaEvent& Event)
{
	if (SUCCEEDED(EventStatus))
	{
		...
		if (SUCCEEDED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Topology %p set as current", this, CurrentTopology.Get());
			return;
		}

		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to get topology that was set: %ls", this, *WmfMedia::ResultToString(Result));
	}
...
void FWmfMediaSession::HandleSessionTopologyStatus(HRESULT EventStatus, IMFMediaEvent& Event)
{
	...
	MF_TOPOSTATUS TopologyStatus = MF_TOPOSTATUS_INVALID;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to get topology status: %ls", this, *WmfMedia::ResultToString(Result));
			return;
		}
	}
	...
	TComPtr<IMFTopology> Topology;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to get topology from topology status event: %ls", this, *WmfMedia::ResultToString(Result));
			return;
		}
	}

	UE_LOGF(LogWmfMedia, Verbose, "Session %p: Topology %p changed status to %ls", this, Topology.Get(), *WmfMedia::TopologyStatusToString(TopologyStatus));
	...
	if (FAILED(EventStatus))
	{
		if (SessionState == EMediaState::Preparing)
		{
			UE_LOGF(LogWmfMedia, Error, "An error occured when preparing the topology");
	...
	TComPtr<IMFTopology> FullTopology;
	{
		...
		if (SUCCEEDED(Result))
		{
			...
			{
				UE_LOGF(LogWmfMedia, Verbose, "Session %p: Full topology %p from media session doesn't match current topology %p", this, FullTopology.Get(), CurrentTopology.Get());
			}
		}
		else
		{
			UE_LOGF(LogWmfMedia, Verbose, "Session %p: Failed to get full topology from media session", this, *WmfMedia::ResultToString(Result));
		}
	}
#endif
	...
	if (SessionState == EMediaState::Preparing)
	{
		UE_LOGF(LogWmfMedia, Verbose, "Session %p: Topology %p ready", this, CurrentTopology.Get());
		...
		JamWorkItem(FWorkItem([this](TComPtr<IMFMediaEvent> MediaEvent, MediaEventType EventType, HRESULT EventStatus, FBaseWorkItemState* BaseState) -> EWorkItemResult
			{
				...
				UE_LOGF(LogWmfMedia, Verbose, "Session %p: 'Re-Scrubbing' after topology change [SState=%d X=%d E=%d]", this, int(State->State), EventType == MESessionStarted, int(EventType));
