...
TArray<uint8>* WmfMediaDecoder::AllocateExternalBuffer(uint64 InTimeStamp, int32 InSize)
{
	...
	if (pBuffer == nullptr)
	{
		...
		else
		{
			...
			UE_LOGF(LogWmfMedia, VeryVerbose, "WmfMediaDecoder::AllocateExternalBuffer new array.");
	...
	if (pBuffer->Num() != InSize)
	{
		...
		// Allocate new size.
		UE_LOGF(LogWmfMedia, VeryVerbose, "WmfMediaDecoder::AllocateExternalBuffer new buffer size:%d old:%d", InSize, pBuffer->Num());
...
void WmfMediaDecoder::ReturnExternalBufferInternal(TArray<uint8>& InBuffer)
{
	...
	if (pEmptyBuffer == nullptr)
	{
		...
		UE_LOGF(LogWmfMedia, VeryVerbose, "WmfMediaDecoder::ReturnExternalBufferInternal new array.");
	}
