...
TComPtr<IMFTopology> FWmfMediaTracks::CreateTopology()
{
	...
	TComPtr<IMFTopology> Topology;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to create playback topology: %ls", this, *WmfMedia::ResultToString(Result));
			return NULL;
		}
	}
	...
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Created playback topology %p (media source %p)", this, Topology.Get(), MediaSource.Get());
		...
		WmfMediaSettings->bAreHardwareAcceleratedCodecRegistered)
	{
		...
		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Video (media source %p) will be decoded on %ls", this, MediaSource.Get(), bHardwareAccelerated ? TEXT("GPU") : TEXT("CPU"));
...
void FWmfMediaTracks::Initialize(IMFMediaSource* InMediaSource, const FString& Url, const FMediaPlayerOptions* PlayerOptions)
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Tracks: %p: Initializing (media source %p)", this, InMediaSource);
	...
	TComPtr<IMFPresentationDescriptor> NewPresentationDescriptor;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to create presentation descriptor: %ls", this, *WmfMedia::ResultToString(Result));
			return;
		}
	}
	...
	DWORD StreamCount = 0;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to get stream count: %ls", this, *WmfMedia::ResultToString(Result));
			return;
		}

		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Found %i streams", this, StreamCount);
	}
	...
	if (!AllStreamsAdded)
	{
		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Not all available streams were added to the track collection", this);
	}
...
{
	UE_LOGF(LogWmfMedia, Verbose, "Tracks: %p: Shutting down (media source %p)", this, MediaSource.Get());
...
{
	UE_LOGF(LogWmfMedia, VeryVerbose, "FWmfMediaTracks::SetSessionState %d", int(InState));
...
{
	UE_LOGF(LogWmfMedia, VeryVerbose, "FWmfMediaTracks::SeekStarted %f", InTime.GetTotalSeconds());
	...
	Session->EnqueueWorkItem(FWmfMediaSession::FWorkItem([this](TComPtr<IMFMediaEvent> MediaEvent, MediaEventType EventType, HRESULT EventStatus, FWmfMediaSession::FBaseWorkItemState* BaseState) -> FWmfMediaSession::EWorkItemResult
		{
			...
			UE_LOGF(LogWmfMedia, VeryVerbose, "FWmfMediaTracks::FlushSamples");
...
bool FWmfMediaTracks::SelectTrack(EMediaTrackType TrackType, int32 TrackIndex)
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Selecting %ls track %i", this, *MediaUtils::TrackTypeToString(TrackType), TrackIndex);
	...
	if (*SelectedTrack != INDEX_NONE)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to deselect stream %i on presentation descriptor: %ls", this, StreamIndex, *WmfMedia::ResultToString(Result));
			return false;
		}

		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Disabled stream %i", this, StreamIndex);
	...
	if (TrackIndex != INDEX_NONE)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to enable %ls track %i (stream %i): %ls", this, *MediaUtils::TrackTypeToString(TrackType), TrackIndex, StreamIndex, *WmfMedia::ResultToString(Result));
			return false;
		}

		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Enabled stream %i", this, StreamIndex);
...
{
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Setting format on %ls track %i to %i", this, *MediaUtils::TrackTypeToString(TrackType), TrackIndex, FormatIndex);
	...
	// set track format
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Set format %i instead of %i on %ls track %i (%i formats)", this, FormatIndex, Track.SelectedFormat, *MediaUtils::TrackTypeToString(TrackType), TrackIndex, Track.Formats.Num());
...
{
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Setting frame rate on format %i of video track %i to %f", this, FormatIndex, TrackIndex, FrameRate);
...
bool FWmfMediaTracks::AddTrackToTopology(const FTrack& Track, IMFTopology& Topology)
{
	...
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Adding stream %i to topology", this, Track.StreamIndex);
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Input type:\n%ls", this, *WmfMedia::DumpAttributes(*Format.InputType.Get()));
	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Output type:\n%ls", this, *WmfMedia::DumpAttributes(*Format.OutputType.Get()));
#endif
	...
	check(Track.Handler.IsValid());
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to set current media type for stream %i: %ls", this, Track.StreamIndex, *WmfMedia::ResultToString(Result));
			return false;
		}
	}
	...
	if ((MajorType == MFMediaType_Audio) && GetDefault<UWmfMediaSettings>()->NativeAudioOut)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to create audio renderer for stream %i: %ls", this, Track.StreamIndex, *WmfMedia::ResultToString(Result));
			return false;
		}
		...
		if (IHeadMountedDisplayModule::IsAvailable())
		{
			...
			if (!AudioOutputDevice.IsEmpty())
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to override HMD audio output device for stream %i: %ls", this, Track.StreamIndex, *WmfMedia::ResultToString(Result));
					return false;
				}
			}
		}
	...
	else
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to create sampler grabber sink for stream %i: %ls", this, Track.StreamIndex, *WmfMedia::ResultToString(Result));
			return false;
		}
	}
		...
		FWmfMediaStreamSink::Create(MFMediaType_Video, MediaStreamSink))
	{
		...
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to configure output node for stream %i", this, Track.StreamIndex);
			return false;
		}
...
//TODO: >>> SOFTWARE VERSION OR WHAT? THIS IS ALL 100% DIFFERENT?!?!? WTH? (just a "standard" output node takes over) ===========> Sounds like a VERY bad idea!
		...
		if (!VideoSamplePool)
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to configure output node for stream (no memory) %i", this, Track.StreamIndex);
			return false;
		}
	...
	{
		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to configure source node for stream %i", this, Track.StreamIndex);
		return false;
	}
	...
	{
		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Unable to query MF_MT_SUBTYPE", this);
		return false;
	}
		...
		Transform)
	{
		...
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to configure decoder node for stream %i", this, Track.StreamIndex);
			return false;
		}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to connect topology nodes for stream %i: %ls", this, Track.StreamIndex, *WmfMedia::ResultToString(Result));
		return false;
	}

	UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Added stream %i to topology", this, Track.StreamIndex);
...
bool FWmfMediaTracks::AddStreamToTracks(uint32 StreamIndex, bool IsVideoDevice, FString& OutInfo)
{
	...
	TComPtr<IMFStreamDescriptor> StreamDescriptor;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to get stream descriptor for stream %i: %ls", this, StreamIndex, *WmfMedia::ResultToString(Result));
		...
		if (Selected == TRUE)
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to deselect stream %i: %ls", this, StreamIndex, *WmfMedia::ResultToString(Result));
			}
		}
	}
	...
	TComPtr<IMFMediaTypeHandler> Handler;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to get media type handler for stream %i: %ls", this, StreamIndex, *WmfMedia::ResultToString(Result));
	...
	GUID MajorType;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to determine major type of stream %i: %ls", this, StreamIndex, *WmfMedia::ResultToString(Result));
			OutInfo += TEXT("\tfailed to determine MajorType\n");

			return false;
		}

		UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Major type of stream %i is %ls", this, StreamIndex, *WmfMedia::MajorTypeToString(MajorType));
		...
		if ((MajorType != MFMediaType_Audio) &&
			...
			(MajorType != MFMediaType_Video))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Unsupported major type %ls of stream %i", this, *WmfMedia::MajorTypeToString(MajorType), StreamIndex);
	...
	DWORD NumMediaTypes = 0;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to get number of track formats in stream %i", this, StreamIndex);
	...
	TComPtr<IMFMediaType> CurrentMediaType;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to get current media type in stream %i: %ls", this, StreamIndex, *WmfMedia::ResultToString(Result));
		}
	}
	...
	for (DWORD TypeIndex = 0; TypeIndex < NumMediaTypes; ++TypeIndex)
	{
		...
		else
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to get sub-type of format %i in stream %i: %ls", this, TypeIndex, StreamIndex, *WmfMedia::ResultToString(Result));
		...
		else if (MajorType == MFMediaType_Video)
		{
			GUID OutputSubType;
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Failed to get video output sub-type for stream %i: %ls", this, StreamIndex, *WmfMedia::ResultToString(Result));
			...
			{
				UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Skipping stream %i, because it is most likely a still image stream", this, StreamIndex);
	...
	if (Track->SelectedFormat == INDEX_NONE)
	{
		for (int32 FormatIndex = 0; FormatIndex < Track->Formats.Num(); ++FormatIndex)
		{
			...
			if (SUCCEEDED(Result))
			{
				UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: Picked default format %i for stream %i", this, FormatIndex, StreamIndex);
				Track->SelectedFormat = FormatIndex;
				break;
			}
		}
		...
		if (Track->SelectedFormat == INDEX_NONE)
		{
			UE_LOGF(LogWmfMedia, Verbose, "Tracks %p: No supported media types found in stream %i", this, StreamIndex);
