...
void UAppleProResEncoderProtocol::ProcessFrame(FCapturedFrameData InFrame)
{
	...
	if (ReturnValue == 0)
	{
		...
		UE_LOGF(LogAppleProResMedia, Verbose, "Processing Frame:%dx%d Frame:%d Conversion:%fms Codec:%fms Total:%fms",
	...
	{
		UE_LOGF(LogAppleProResMedia, Error, "Unable to encode Frame:%lld", CurrentTime.value);
	}
}
