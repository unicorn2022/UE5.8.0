...
bool FAppleProResEncoder::Initialize()
{
	...
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to initialize ProRes Encoder.");
		return false;
	}
	...
	{
		UE_LOGF(LogAppleProResMedia, Display, "Deleting existing file \"%ls\" (%lld bytes) before initializing new one.", *Options.OutputFilename, IFileManager::Get().FileSize(*Options.OutputFilename));
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to create file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to set Movie Timescale");
		return false;
	}
	...
	// Add our Video stream
	{
		...
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to add video track to file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
		Status = ProResFileWriterSetTrackMediaTimescale(FileWriter, VideoTrackId, ProResTimescale);
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to set video track timescale to file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
	}
	...
	// Add a Audio stream
	{
		if (Options.bIncludeAudio)
		{
			...
			if (Status != 0)
			{
				UE_LOGF(LogAppleProResMedia, Error, "Failed to add audio track to file writer at path: \"%ls\".", *Options.OutputFilename);
				return false;
			}
			...
			if (Status != 0)
			{
				UE_LOGF(LogAppleProResMedia, Error, "Failed to set audio track timescale to file writer at path: \"%ls\".", *Options.OutputFilename);
				return false;
			}
		}
	}
	...
	// Add a Timecode stream
	{
		...
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to add timecode track to file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
		Status = ProResFileWriterSetTrackMediaTimescale(FileWriter, TimecodeTrackId, ProResTimescale);
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to set timecode track timescale to file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
	}
	...
	// Set up the Video Track 
	{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to initialize video track with specified settings for file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
		...
		if (!bSuccess)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to initialize timecode track with specified settings for file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
		...
		if (Options.bIncludeAudio)
		{
			...
			if (!bSuccess)
			{
				UE_LOGF(LogAppleProResMedia, Error, "Failed to initialize audio track with specified settings for file writer at path: \"%ls\".", *Options.OutputFilename);
				return false;
			}
		}
	}
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to begin session with file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
...
bool FAppleProResEncoder::InitializeVideoTrack()
{
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to set preferred chunk size for video track for file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to create a VideoFormatDescription from settings for file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
...
bool FAppleProResEncoder::InitializeTimecodeTrack()
{
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to create a TimecodeFormatDescription from settings for file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
...
bool FAppleProResEncoder::InitializeAudioTrack()
{
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to create a AudioFormatDescription from settings for file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
...
void FAppleProResEncoder::Finalize()
{
	...
	if (FileWriter)
	{
		...
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to Mark End of Data for Video Track for file writer at path: \"%ls\".", *Options.OutputFilename);
		}

		Status = ProResFileWriterMarkEndOfDataForTrack(FileWriter, TimecodeTrackId);
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to Mark End of Data for Timecode Track for file writer at path: \"%ls\".", *Options.OutputFilename);
		}
		...
		if (Options.bIncludeAudio)
		{
			...
			if (Status != 0)
			{
				UE_LOGF(LogAppleProResMedia, Error, "Failed to Mark End of Data for Audio Track for file writer at path: \"%ls\".", *Options.OutputFilename);
			}
		}
		...
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to End Writer Session for file writer at path: \"%ls\".", *Options.OutputFilename);
		}

		Status = ProResFileWriterFinish(FileWriter);
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to Finish Writer Session for file writer at path: \"%ls\".", *Options.OutputFilename);
		}

		Status = ProResFileWriterInvalidate(FileWriter);
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to Invalidate Writer for file writer at path: \"%ls\".", *Options.OutputFilename);
		}
	}
...
bool FAppleProResEncoder::WriteFrame(const FImagePixelData* InPixelData)
{
	...
	if (ReturnValue != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to encode frame for Video Track for file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
	...
	if (Status != 0)
	{
		UE_LOGF(LogAppleProResMedia, Error, "Failed to add sample to Video Track for file writer at path: \"%ls\".", *Options.OutputFilename);
		return false;
	}
	...
	if (TimecodePayload)
	{
		...
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to add sample to Timecode Track for file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
	}
...
bool FAppleProResEncoder::WriteAudioSample(const TArrayView<int16>& InAudioSamples)
{
	...
	for (int32 SampleIdx = 0; SampleIdx < NumFramesOfData; SampleIdx++)
	{
		...
		if (Status != 0)
		{
			UE_LOGF(LogAppleProResMedia, Error, "Failed to add sample to Audio Track for file writer at path: \"%ls\".", *Options.OutputFilename);
			return false;
		}
