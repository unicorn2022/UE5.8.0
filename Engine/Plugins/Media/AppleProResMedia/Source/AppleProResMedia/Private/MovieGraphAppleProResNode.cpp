...
{
	static const FSlateIcon AppleProResIcon = FSlateIcon(FName("MovieRenderPipelineStyle"), "MovieRenderPipeline.Graph.Icon.RenderMovieFile");

	OutColor = FLinearColor::White;
	return AppleProResIcon;
}

FText UMovieGraphAppleProResNode::GetBasicConfigShortDisplayName() const
{
	return NSLOCTEXT("MovieGraphNodes", "BasicConfigShortName_PRORES", "PRORES");
}
#endif // WITH_EDITOR
...
bool UMovieGraphAppleProResNode::Initialize_EncodeThread(MovieRenderGraph::IVideoCodecWriter* InWriter)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Failed to initialize Apple ProRes writer.");
		return false;
	}
