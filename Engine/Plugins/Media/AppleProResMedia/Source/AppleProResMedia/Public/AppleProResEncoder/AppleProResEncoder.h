...
public:
	UE_API FAppleProResEncoder(const FAppleProResEncoderOptions& InOptions);
	UE_API ~FAppleProResEncoder();
	...
	UE_API bool WriteFrame(const FImagePixelData* InPixelData);
	
	/** Appends a new audio sample onto the audio stream. */
	UE_API bool WriteAudioSample(const TArrayView<int16>& InAudioSamples);

	/** Gets the options that the encoder was initialized with. */
	UE_API const FAppleProResEncoderOptions& GetOptions() const;

	bool IsInitialized() const { return bInitialized; }

private:
	UE_API bool InitializeVideoTrack();
	UE_API bool InitializeTimecodeTrack();
	UE_API bool InitializeAudioTrack();
