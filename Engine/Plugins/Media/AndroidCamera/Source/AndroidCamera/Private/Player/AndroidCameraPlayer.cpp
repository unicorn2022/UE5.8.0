...
bool FAndroidCameraPlayer::Open(const FString& Url, const IMediaOptions* /*Options*/)
{
	...
	if (Url.StartsWith(TEXT("vidcam://")))
	{
		...
		{
			UE_LOGF(LogAndroidCamera, Warning, "Failed to set data source for vidcam %ls", *FilePath);
			return false;
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogAndroidCamera, Warning, "Failed to set data source for URL %ls", *Url);
			return false;
		}
	}
...
static void ConditionalInitializeVideoTexture_RenderThread(FRHICommandListBase& RHICmdList, TWeakPtr<FJavaAndroidCameraPlayer, ESPMode::ThreadSafe> JavaCameraPlayerPtr, FGuid PlayerGuid)
{
	...
	if (VideoTexture == nullptr)
	{
		...
		if (VideoTexture == nullptr)
		{
			UE_LOGF(LogAndroidCamera, Warning, "CreateTexture failed!");
			return;
		}
...
bool FAndroidCameraPlayer::SelectTrack(EMediaTrackType TrackType, int32 TrackIndex)
{
	...
	case EMediaTrackType::Audio:
		...
		{
			UE_LOGF(LogAndroidCamera, Verbose, "Player %p: Selecting audio track %i instead of %i (%i tracks)", this, TrackIndex, SelectedVideoTrack, AudioTracks.Num());

			if (TrackIndex == INDEX_NONE)
			{
				UE_LOGF(LogAndroidCamera, VeryVerbose, "Player %p: Disabling audio", this);
			...
			else
			{
				...
				UE_LOGF(LogAndroidCamera, VeryVerbose, "Player %p: Enabling audio", this);
	...
	case EMediaTrackType::Caption:
		...
		{
			UE_LOGF(LogAndroidCamera, Verbose, "Player %p: Selecting caption track %i instead of %i (%i tracks)", this, TrackIndex, SelectedCaptionTrack, CaptionTracks.Num());

			if (TrackIndex == INDEX_NONE)
			{
				UE_LOGF(LogAndroidCamera, VeryVerbose, "Player %p: Disabling captions", this);
			}
			else
			{
				...
				UE_LOGF(LogAndroidCamera, VeryVerbose, "Player %p: Enabling captions", this);
			}
	...
	case EMediaTrackType::Video:
		...
		{
			UE_LOGF(LogAndroidCamera, Verbose, "Player %p: Selecting video track %i instead of %i (%i tracks).", this, TrackIndex, SelectedVideoTrack, VideoTracks.Num());

			if (TrackIndex == INDEX_NONE)
			{
				UE_LOGF(LogAndroidCamera, VeryVerbose, "Player %p: Disabling video", this);
				JavaCameraPlayer->SetVideoEnabled(false);
			}
			else
			{
				...
				UE_LOGF(LogAndroidCamera, VeryVerbose, "Player %p: Enabling video", this);
				JavaCameraPlayer->SetVideoEnabled(true);
			}
...
bool FAndroidCameraPlayer::Seek(const FTimespan& Time)
{
	if ((CurrentState == EMediaState::Closed) ||
		...
		(CurrentState == EMediaState::Preparing))
	{
		UE_LOGF(LogAndroidCamera, Warning, "Cannot seek while closed, preparing, or in error state");
		return false;
	}

	UE_LOGF(LogAndroidCamera, Verbose, "Player %p: Seeking to %ls", this, *Time.ToString());
...
bool FAndroidCameraPlayer::SetRate(float Rate)
{
	if ((CurrentState == EMediaState::Closed) ||
		(CurrentState == EMediaState::Error))
	{
		UE_LOGF(LogAndroidCamera, Warning, "Cannot set rate while closed, preparing, or in error state");
		return false;
	}
	...
	if (CurrentState == EMediaState::Preparing && Rate == 1.0f)
	{
		...
		{
			UE_LOGF(LogAndroidCamera, Warning, "It's unnecessary to Play the camera player repeatedly.");
			return true;
		}

		UE_LOGF(LogAndroidCamera, Log, "Camera is under preparation.");
		...
		int ret = pthread_create(&DeferredSetRateThread, NULL, [] (void* arg) -> void* {
			...
			if (Arg->ThisPtr.IsValid())
			{
				...
				UE_LOGF(LogAndroidCamera, Log, "Waiting for camera to become ready...");
				...
				while (true)
				{
					...
					{
						UE_LOGF(LogAndroidCamera, Log, "Timed out after waiting for camera preparation for 3 seconds.");
						break;
					}
					if (Arg->ThisPtr->StopSetRateThread)
					{
						UE_LOGF(LogAndroidCamera, Log, "Stopped waiting for camera preparation.");
						break;
					}
					...
					{
						UE_LOGF(LogAndroidCamera, Log, "Camera is ready.");
			...
			{
				UE_LOGF(LogAndroidCamera, Error, "The camera player object has been deleted.");
			}
	...
	UE_LOGF(LogAndroidCamera, Verbose, "Player %p: Setting rate from to %f to %f", this, GetRate(), Rate);
	...
	{
		UE_LOGF(LogAndroidCamera, Warning, "The rate %f is not supported", Rate);
		return false;
	}
...
void FAndroidCameraPlayer::HandleApplicationHasEnteredForeground()
{
	...
	if (JavaCameraPlayer.IsValid() && CurrentState == EMediaState::Closed)
	{
		...
		{
			UE_LOGF(LogAndroidCamera, Warning, "HandleApplicationHasEnteredForeground - RestoreCamera failed.");
		}
	}
}
