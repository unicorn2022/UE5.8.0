...
void SMediaPlayerEditorMediaDetails::Construct(const FArguments& InArgs,
	UMediaPlayer* InMediaPlayer, UMediaTexture* InMediaTexture, FName InTextStyleName)
{
	// Remark: these can be null.
	MediaPlayerWeak = InMediaPlayer;
	MediaTextureWeak = InMediaTexture;

	const ISlateStyle& SlateStyle = FAppStyle::Get();
	const FName StyleName = InTextStyleName.IsNone() ? TEXT("NormalText") : InTextStyleName;
				...
				[
					...
					+ SHorizontalBox::Slot()
						[
							...
							// Video decoder provider
							+ SVerticalBox::Slot()
								.AutoHeight()
								.VAlign(VAlign_Center)
								.Padding(4.0f)
								[
									SAssignNew(VideoDecoderProvider, STextBlock)
									.TextStyle(SlateStyle, StyleName)
								]

							// Audio decoder provider
							+ SVerticalBox::Slot()
								.AutoHeight()
								.VAlign(VAlign_Center)
								.Padding(4.0f)
								[
									SAssignNew(AudioDecoderProvider, STextBlock)
									.TextStyle(SlateStyle, StyleName)
								]
						]
				]
...
UMediaPlayer* SMediaPlayerEditorMediaDetails::GetMediaPlayer() const
{
	return MediaPlayerWeak.Get();
}

UMediaTexture* SMediaPlayerEditorMediaDetails::GetMediaTexture() const
{
	return MediaTextureWeak.Get();
}
...
void SMediaPlayerEditorMediaDetails::UpdateDetails()
{
	...
	FString VideoDecoderImpl;
	FString AudioDecoderImpl;

	const UMediaPlayer* MediaPlayer = GetMediaPlayer();
	...
	if (MediaPlayer != nullptr)
	{
		...
		MediaPlayer->GetMediaInfo<FString>(VideoDecoderImpl, UMediaPlayer::MediaInfoNameVideoDecoderProvider.Resolve());
		MediaPlayer->GetMediaInfo<FString>(AudioDecoderImpl, UMediaPlayer::MediaInfoNameAudioDecoderProvider.Resolve());
	}
	...
	*/
	...
	if (!VideoDecoderImpl.IsEmpty())
	{
		VideoDecoderProvider->SetText(FText::Format(LOCTEXT("VideoDecoderProvider", "Video decoder: {0}"), FText::FromString(VideoDecoderImpl)));
		VideoDecoderProvider->SetVisibility(EVisibility::Visible);
	}
	else
	{
		VideoDecoderProvider->SetVisibility(EVisibility::Collapsed);
	}
	if (!AudioDecoderImpl.IsEmpty())
	{
		AudioDecoderProvider->SetText(FText::Format(LOCTEXT("AudioDecoderProvider", "Audio decoder: {0}"), FText::FromString(AudioDecoderImpl)));
		AudioDecoderProvider->SetVisibility(EVisibility::Visible);
	}
	else
	{
		AudioDecoderProvider->SetVisibility(EVisibility::Collapsed);
	}
}
