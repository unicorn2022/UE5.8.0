	...
	if (SliceInfos.Num())
	{
		...
		if (SliceInfos[0].Header.bIsIRAP)
		{
			...
			if (!DPB.IsValid())
			{
				...
				const int32 NumFrames = DPBSize + 2;	// 1 extra for the current frame that's not in the DPB yet, and 1 extra that acts as a 'missing' frame.
			...
			// Remember the type of IRAP (see below).
			BitstreamParamsH265.AssociatedIRAP = SliceInfos[0].NalUnitType;
			++BitstreamParamsH265.IRAPCount;
		}
		/*
			Filter out any RASL pictures for which the associated IRAP, that is the first coded picture, is a CRA or BLA.

			We do this because the DXVA HEVC spec states:
				"The host decoder is recommended to send only decodable pictures to accelerator. For example, when the associated IRAP picture
				is a BLA picture or is a CRA picture that is the first coded picture in the bitstream, the host decoder is recommended not to
				send the associated RASL pictures to accelerator if any such RASL pictures are present in the bitstream. 
				However, the accelerator shall be robust enough to handle any non-decodable pictures."

			And the "However" statement does not hold true. There are accelerators where sending down these pictures will produce
			garbage frames. So we need to filter them out.
		*/
		if (BitstreamParamsH265.IRAPCount <= 1 && (BitstreamParamsH265.AssociatedIRAP == 21 || BitstreamParamsH265.AssociatedIRAP == 16 || BitstreamParamsH265.AssociatedIRAP == 17 || BitstreamParamsH265.AssociatedIRAP == 18))
		{
			for(int32 i=0; i<SliceInfos.Num(); ++i)
			{
				if (SliceInfos[i].Header.NalUnitType == 8 || SliceInfos[i].Header.NalUnitType == 9)
				{
					SliceInfos.RemoveAt(i);
					--i;
				}
			}
		}
...
IElectraDecoder::EDecoderError FD3D12VideoDecoder_H265::SendEndOfData()
{
	...
	BitstreamParamsH265.bIsFirstInSequence = true;
	BitstreamParamsH265.AssociatedIRAP = -1;
	BitstreamParamsH265.IRAPCount = 0;
...
{
	// When there are no slices we are done.
	if (InSliceInfos.IsEmpty())
	{
		return IElectraDecoder::EDecoderError::None;
	}
	...
	{
		UE_LOGF(LogD3D12VideoDecodersElectra, Warning, "DecodeSlicesH265() waited too long for the previous operation to complete. Trying again later.");
		return IElectraDecoder::EDecoderError::NoBuffer;
	}
	...
	for(int32 ns=0; ns<InSliceInfos.Num(); ++ns)
	{
		...
		input.SliceHeaders[ns].SliceBytesInBuffer = SliceSize;
		...
		*BufferPtr++ = 1;
		...
		if (SliceSize != Align(SliceSize, 128))
		{
...
IElectraDecoder::EDecoderError FD3D12VideoDecoder_H265::HandleOutputListH265(const TArray<ElectraDecodersUtil::MPEG::H265::FDecodedPictureBuffer::FDPBOutputFrame>& InOutputFrameInfos)
{
	...
	for(int32 i=0; i<InOutputFrameInfos.Num(); ++i)
	{
		...
		//UE_LOGF(LogD3D12VideoDecodersElectra, Log, "Output frame %d, %lld", InOutputFrameInfos[i].UserFrameInfo.IndexInBuffer, (long long int)InOutputFrameInfos[i].UserFrameInfo.PTS.GetTicks());
