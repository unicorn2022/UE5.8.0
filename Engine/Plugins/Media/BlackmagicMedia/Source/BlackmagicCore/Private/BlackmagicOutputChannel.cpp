	...
	namespace Private
	{
		...
		*****************************************************************************/
		...
		bool FOutputChannel::Initialize()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Couldn't get the attribute flags for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}

			if ((VideoCapabilities & bmdDeviceSupportsPlayback) == 0)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Device '%d' doesn't support playback mode.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not create output buffers for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not find the pixel format '%d'.", (int32_t)ChannelOptions.PixelFormat);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not create notification callback object for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not set callback for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FOutputChannel::InitializeDeckLink()
		{
			...
			if (DeckLink == nullptr)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not find the device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::InitializeInterfaces()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not obtain the IDeckLinkAttributes interface for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not obtain the IDeckLinkOutput interface for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not obtain the IDeckLinkConfiguration interface for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FOutputChannel::InitializeLinkConfiguration()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not change device '%d' link mode.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Configuration == bmdLinkConfigurationQuadLink)
			{
				...
				{
					UE_LOGF(LogBlackmagicCore, Error,"Could not change device '%d' Quad-link output. It is not supported by the device.", ChannelInfo.DeviceIndex);
					return false;
				}
				else if (Result == E_FAIL && Result == E_INVALIDARG)
				{
					UE_LOGF(LogBlackmagicCore, Error,"Could not change device %d Quad-link output to Square Division Quad Split mode.", ChannelInfo.DeviceIndex);
					return false;
				}
			}
		...
		void FOutputChannel::OnScheduledFrameCompleted(IDeckLinkVideoFrame* InCompletedFrame, BMDOutputFrameCompletionResult InResult)
		{
...

			{
				...
				UE_LOGF(LogBlackmagicCore, Verbose, "\tOnScheduledFrame %d, %d:%d", FoundFrame->FrameIdentifier, FoundFrame->Timecode.Seconds, FoundFrame->Timecode.Frames);

				check(FoundFrame);
				if (FoundFrame->InFlightCount == 0)
				{
					UE_LOGF(LogBlackmagicCore, Error,"No frame is currently in flight, verify that the engine framerate is high enough to support the output framerate.");
				}
				else
				{
				...
				if (InFlightFrames <= 0)
				{
					...
					UE_LOGF(LogBlackmagicCore, Verbose, "\tResending last frame: %d", LastScheduledFrame.load()->FrameIdentifier);
		...
		{
			{
				...
				UE_LOGF(LogBlackmagicCore, Warning,"RenderAudio, %d, NumSamples: %d", millisec_since_epoch, BufferedSamples);
#endif
		...
		bool FOutputChannel::ScheduleOutputFrame(FOutputFrame* InFrame)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogBlackmagicCore, VeryVerbose, "Scheduled frame starting with even timecode. (%d)", InFrame->Timecode.Frames);
				}
				else
				{
					UE_LOGF(LogBlackmagicCore, VeryVerbose, "Scheduled frame starting with odd timecode. (%d)", InFrame->Timecode.Frames);
				}
			}
			...
			UE_LOGF(LogBlackmagicCore, Error,"Could not schedule frame for Output device %d", ChannelInfo.DeviceIndex);
			return false;
		}
		...
		void FOutputChannel::AudioThread_ScheduleAudioSamples()
		{
			while (true)
			{
				...
				if (bIsPrerollingAudio && NumBufferedAudioSamples < AudioSamplesWaterLevel)
				{
					...
					{
						UE_LOGF(LogBlackmagicCore, Error,"Could not preroll audio for Output device %d", ChannelInfo.DeviceIndex);
					}
				}
...
#if AUDIO_DEBUGGING
						auto millisec_since_epoch = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
						UE_LOGF(LogBlackmagicCore, Warning,"(%d) Samples Buffered: %d, Expected: %d, Written: %d, PacketsToScheduleSize: %d", millisec_since_epoch, NumBufferedAudioSamples.load(), PacketToSchedule->NumAudioSamples, SamplesWritten, AudioPacketsToSchedule.Size());
#endif
					}
				}
				...
				{
					UE_LOGF(LogBlackmagicCore, Error,"Could not schedule audio for Output device %d", ChannelInfo.DeviceIndex);
				}
			}
		}
		...
		bool FOutputChannel::VerifyFormatSupport()
		{
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not check if the video mode is supported on device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not get the display mode on device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not get a valid frame rate for output on device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (!bSupported)
			{
				UE_LOGF(LogBlackmagicCore, Error,"The video format '%d' is not support on device '%d'.", DisplayMode, ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::VerifyIfDeviceUsed()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Couldn't get the DeckLinkStatus for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Couldn't get the Busy status for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Device %d can't be used because it's already being used as an output.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Couldn't get the DuplexMode status for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Device %d is inactive.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (DuplexStatus == ENUM(BMDDuplexMode)::bmdDuplexHalf)
			{
				...
				{
					UE_LOGF(LogBlackmagicCore, Error,"Device %d can't be used because it's already being used as an input and it can't do output at the same time.", ChannelInfo.DeviceIndex);
					return false;
				}
			}
		...
		bool FOutputChannel::InitializeKeyer()
		{
			...
			if (ChannelOptions.bOutputKey)
			{
				if (!Helpers::Is8Bits(ChannelOptions.PixelFormat))
				{
					UE_LOGF(LogBlackmagicCore, Warning,"Can only key if the pixel format contains alpha.");
				}

				HRESULT Result = DeckLink->QueryInterface(IID_IDeckLinkKeyer, (void**)&Keyer);
				if (Result != S_OK)
				{
					UE_LOGF(LogBlackmagicCore, Error,"Could not obtain the IDeckLinkKeyer interface for device '%d'.", ChannelInfo.DeviceIndex);
					return false;
				}
				...
				if (Result != S_OK)
				{
					UE_LOGF(LogBlackmagicCore, Error,"Could not enable external Keyer for device '%d'. Does the device need special configuration in Blackmagic Desktop Video Setup utility?", ChannelInfo.DeviceIndex);
					return false;
				}
				...
				if (Result != S_OK)
				{
					UE_LOGF(LogBlackmagicCore, Error,"Could not set Keyer level for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
					return false;
				}
			}
		...
		bool FOutputChannel::InitializeAudio()
		{
			...
			{
				check(false);
				UE_LOGF(LogBlackmagicCore, Error,"Unsupported audio bit depth.");
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not enable audio output for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}

			if (FAILED(DeckLinkOutput->BeginAudioPreroll()))
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not begin audio preroll for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::InitializeVideo()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not enable video output for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not send preroll frames for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not start playback for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FOutputChannel::SendPreRollFrames()
		{
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Can't initiate PreRoll. Need at least 3 frames and only '%zu' are available.", FrameReadyToWrite.size());
				return false;
			}
		...
		void FOutputChannel::EmbedTimecodeInFrame(FOutputFrame* InFrame)
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Warning,"Could not set Timecode on frame for Output device %d", ChannelInfo.DeviceIndex);
			}
		}
		...
		FOutputChannel::~FOutputChannel()
		{
			...
			if (DeckLink)
			{
				...
				if (Result == S_OK)
				{
					...
					if ((BusyStatus & bmdDevicePlaybackBusy) != 0)
					{
						UE_LOGF(LogBlackmagicCore, Warning,"Could not stop capture activity on device '%d' while closing it.", ChannelInfo.DeviceIndex);
					}
				}
			}
		...
		bool FOutputChannel::SetVideoFrameData(const FFrameDescriptor& InFrame)
		{
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"SetVideoFrameData: Can't set the video. The buffer is invalid or the buffer size is not the same as the Blackmagic Video Format for output channel %d.\n", ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FOutputChannel::SetVideoFrameData(FFrameDescriptor_GPUDMA& InFrame)
		{
			...
			if (!bIsProgressive)
			{
				UE_LOGF(LogBlackmagicCore, Error,"SetVideoFrameData: Can't set the video, GPUTextureTransfer is not supported with interlaced video.\n");
				return false;
			}

			if (InFrame.RHITexture == nullptr)
			{
				UE_LOGF(LogBlackmagicCore, Error,"SetVideoFrameData: Can't set the video. The texture is invalid.\n");
				return false;
			}

			if (!ChannelOptions.bUseGPUDMA || !TextureTransfer)
			{
				UE_LOGF(LogBlackmagicCore, Error,"SetVideoFrameData: Can't set the video. GPU DMA was not setup correctly.\n");
				return false;
			}
		...
		bool FOutputChannel::ShouldCaptureThisFrame(::FTimecode Timecode, uint32 FrameNumber)
		{
			...
			if (ChannelOptions.IsInterlaced() && Timecode.Frames % 2 != 0 && ChannelOptions.bOutputInterlaceOnEvenFrames)
			{
				...
				{
					UE_LOGF(LogBlackmagicCore, Verbose, "Skipping output because no writable frame is available.");
				}
				else
				{
					for (FOutputFrame* Frame : FrameReadyToWrite)
					{
						UE_LOGF(LogBlackmagicCore, VeryVerbose, "\t F1: %d, F2: %d", Frame->FrameIdentifier, Frame->FrameIdentifierF2);
					}
				}
		...
		bool FOutputChannel::SetAudioFrameData(const FAudioSamplesDescriptor& InSamples)
		{
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"SetAudioFrameData: Can't set the audio. The buffer is invalid or the buffer size is not the same as the Blackmagic Audio Format for output channel %d.\n", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (FAudioPacket* Packet = AudioPacketPool.Acquire())
			{
				...
				UE_LOGF(LogBlackmagicCore, Error,"SetAudio (%d), (%d:%d) %d samples", millisec_since_epoch, InSamples.Timecode.Seconds, InSamples.Timecode.Frames, Packet->NumAudioSamples);;
#endif
			...
			if (!ChannelOptions.bOutputInterlacedFieldsTimecodeNeedToMatch)
			{
				...
				{ 
					if (!InFrame.bEvenFrame)
					{
						...
						{
							UE_LOGF(LogBlackmagicCore, Warning,"Could not find frame. id %d, total flight %d, available %zu", InFrame.FrameIdentifier, InFlightFrames.load(), FrameReadyToWrite.size());
						}
		...
		void FOutputChannel::PushWhenFrameReady(FOutputFrame* InFrame)
		{
			...
			{
				UE_LOGF(LogBlackmagicCore, Verbose, "Pushing frame %d, FrameReadyToWriteSize = %zu", InFrame->FrameIdentifier, FrameReadyToWrite.size());
		...
		HRESULT FDecklinkVideoFrame::GetInt(BMDDeckLinkFrameMetadataID MetadataId, LONGLONG* Value)
		{
			...
			case bmdDeckLinkFrameMetadataColorspace:
			{
				...
				default:
					if (IsHDRLoggingOK())
					{
						UE_LOGF(LogBlackmagicCore, Warning, "Invalid color space detected.");
					}
