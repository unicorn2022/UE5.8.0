...
static TAutoConsoleVariable<bool> CVarBlackmagicAlwaysLockToFieldRate(
	TEXT("Blackmagic.Genlock.AlwaysLockToFieldRate"),
	true,
	TEXT("When true, the synthetic odd-field event is fired on every received frame regardless of whether the SDK reports an interlaced display mode. This allows picking a non input source and still getting the desired frame rate (e.g. 60Hz for an interlaced source instead of 30Hz). When false, the event is only fired when the source is interlaced."),
	ECVF_Default);
...
namespace BlackmagicDesign
{
	namespace Private
	{
		...
		HRESULT FInputChannelNotificationCallback::VideoInputFormatChanged(BMDVideoInputFormatChangedEvents InNotificationEvents, IDeckLinkDisplayMode* InNewDisplayMode, BMDDetectedVideoInputFormatFlags InDetectedSignalFlags)
		{
			...
			if ((InNotificationEvents & bmdVideoInputDisplayModeChanged) != 0)
			{
				...
				if (InNewDisplayMode->GetDisplayMode() != InputChannel->DisplayMode)
				{
					...
					UE_LOGF(LogBlackmagicCore, Display,"The video input format changed on the device '%d'. The new display mode is '%s'", InputChannel->ChannelInfo.DeviceIndex, DisplayModeName.c_str());
			...
			if ((InNotificationEvents & bmdVideoInputColorspaceChanged) != 0)
			{
				...
				if ((InDetectedSignalFlags & bmdDetectedVideoInputYCbCr422) == 0)
				{
					...
					{
						UE_LOGF(LogBlackmagicCore, Display,"The color space have changed on the device '%d'. Only YUV is supported.", InputChannel->ChannelInfo.DeviceIndex);

						if (InDetectedSignalFlags == bmdDetectedVideoInputYCbCr422)
						{
							UE_LOGF(LogBlackmagicCore, Display,"VideoInputYCbCr422");
						}
						if (InDetectedSignalFlags == bmdDetectedVideoInputRGB444)
						{
							UE_LOGF(LogBlackmagicCore, Display,"VideoInputRGB444");
						}
					}
				}
			}
			
			// Always derive the field dominance from the new display mode, regardless of which notification flags
			// were set. The Blackmagic driver can report a display mode change without also flagging a field
			// dominance change (e.g. switching from 1080p30 to 1080i60).
			...
			if ((InNotificationEvents & bmdVideoInputFieldDominanceChanged) != 0)
			{
				...
				{
					UE_LOGF(LogBlackmagicCore, Display,"The FieldDominance have changed on the device '%d'.", InputChannel->ChannelInfo.DeviceIndex);
					bProcessFormatChanged = true;
				}
			}
		...
		HRESULT FInputChannelNotificationCallback::VideoInputFrameArrived(IDeckLinkVideoInputFrame* InVideoFrame, IDeckLinkAudioInputPacket* InAudioPacket)
		{
			...
			if (Helpers::IsInterlacedDisplayMode(InputChannel->DisplayMode) && (FrameInfo.bHasInputSource || CVarBlackmagicAlwaysLockToFieldRate.GetValueOnAnyThread()))
			{
				...
				{
					UE_LOGF(LogBlackmagicCore, Warning,"Video frame has arrived faster than expected on device '%d'", InputChannel->ChannelInfo.DeviceIndex);
				}
			}
		...
		void FInputChannelNotificationCallback::GetHDRMetaData(IInputEventCallback::FFrameReceivedInfo& FrameInfo, IDeckLinkVideoInputFrame* InVideoFrame)
		{
			...
			if ((FrameInfo.HDRMetaData.bIsAvailable) && (FrameInfo.bHasInputSource))
			{
				...
				if (Result == S_OK)
				{
					...
					{
						UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR EOTF for device '%d'.",
					...
					{
						UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR colourspace for device '%d'.",
							InputChannel->ChannelInfo.DeviceIndex);
					}

					// These fields are populated for PQ streams, not for HLG signals.
					if (FrameInfo.HDRMetaData.EOTF == EHDRMetaDataEOTF::PQ)
					{
				...
				if (IsHDRLoggingOK())
				{
					switch (MetaDataID)
					{
						case bmdDeckLinkFrameMetadataHDRDisplayPrimariesRedX:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR display primaries red X.");
							break;
						case bmdDeckLinkFrameMetadataHDRDisplayPrimariesRedY:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR display primaries red Y.");
							break;
						case bmdDeckLinkFrameMetadataHDRDisplayPrimariesGreenX:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR display primaries green X.");
							break;
						case bmdDeckLinkFrameMetadataHDRDisplayPrimariesGreenY:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR display primaries green Y.");
							break;
						case bmdDeckLinkFrameMetadataHDRDisplayPrimariesBlueX:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR display primaries blue X.");
							break;
						case bmdDeckLinkFrameMetadataHDRDisplayPrimariesBlueY:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR display primaries blue Y.");
							break;
						case bmdDeckLinkFrameMetadataHDRWhitePointX:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR white point X.");
							break;
						case bmdDeckLinkFrameMetadataHDRWhitePointY:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR white point Y.");
							break;
						case bmdDeckLinkFrameMetadataHDRMaxDisplayMasteringLuminance:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR maximum mastering luminance.");
							break;
						case bmdDeckLinkFrameMetadataHDRMinDisplayMasteringLuminance:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR minimum mastering luminance.");
							break;
						case bmdDeckLinkFrameMetadataHDRMaximumContentLightLevel:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR maximum content light level.");
							break;
						case bmdDeckLinkFrameMetadataHDRMaximumFrameAverageLightLevel:
							UE_LOGF(LogBlackmagicCore, Warning,"Could not get meta data HDR maximum frame average light level.");
							break;
					}
				}
			}
		...
		bool FInputChannel::InitializeInterfaces()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not obtain the IDeckLinkInput interface for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not obtain the IDeckLinkStatus interface for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FInputChannel::VerifyFormatSupport()
		{
			...
			if (!bSupported)
			{
				UE_LOGF(LogBlackmagicCore, Error,"The desired video mode '%s' is not supported on device '%d'.", BlackmagicPlatform::GetName(DeckLinkDisplayMode.Get()).c_str(), ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK || Denominator == 0)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not get a valid frame rate for input on device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
		...
		bool FInputChannel::VerifyDeviceUsage()
		{
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"Device %d can't be used because it's already being used as an input.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (DuplexStatus == ENUM(BMDDuplexMode)::bmdDuplexHalf)
			{
				...
				{
					UE_LOGF(LogBlackmagicCore, Error,"Device %d can't be used because it's already being used as an output and it can't do input at the same time.", ChannelInfo.DeviceIndex);
					return false;
				}
			}
		...
		bool FInputChannel::InitializeAudio()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not enable the audio input on device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
...
;
			...
			{
				UE_LOGF(LogBlackmagicCore, Error,"The device '%d' doesn't have a dedicated LTC input - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
		...
		bool FInputChannel::InitializeStream()
		{
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not enable video input for device '%d'.", ChannelInfo.DeviceIndex);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not flush the streams video for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
			...
			if (Result != S_OK)
			{
				UE_LOGF(LogBlackmagicCore, Error,"Could not start the streams video for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
				return false;
			}
			bStreamStarted = true;
		...
		bool FInputChannel::InitializeListener_Helper(const FListener InListener)
		{
			...
			if (bNeedToPauseStream)
			{
				...
				if (Result != S_OK)
				{
					UE_LOGF(LogBlackmagicCore, Error,"Could not pause the the streams video for device '%d' - result = %08x.", ChannelInfo.DeviceIndex, Result);
					return false;
				}
			}
			...
			if (bResult && InListener.Option.bReadVideo)
			{
				if (ReadVideoCounter == 0)
				{
					...
					{
						UE_LOGF(LogBlackmagicCore, Error,"Could not find the pixel format '%d'.", (int32_t)InListener.Option.PixelFormat);
						bResult = false;
					}
