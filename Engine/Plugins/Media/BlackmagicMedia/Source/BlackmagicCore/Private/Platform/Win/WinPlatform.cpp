...
namespace BlackmagicPlatform
{
	...
	bool InitializeAPI()
	{
		...
		if (!bResult)
		{
			UE_LOGF(LogBlackmagicCore, Error,"Initialization of Blackmagic COM failed");
		}
		return bResult;
	}
	...
	IDeckLinkIterator* CreateDeckLinkIterator()
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogBlackmagicCore, Error,"A DeckLink iterator could not be created - %08x. The DeckLink drivers may not be installed.", Result);
	...
	IDeckLinkVideoConversion* CreateDeckLinkVideoConversion()
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogBlackmagicCore, Error,"A DeckLink video conversion could not be created - %08x. The DeckLink drivers may not be installed.", Result);
