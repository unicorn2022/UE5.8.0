...
bool UBlackmagicMediaSource::Validate() const
{
	...
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaConfiguration '%ls' is invalid.", *GetName());
		return false;
	}

	if (!IBlackmagicMediaModule::Get().IsInitialized())
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "Can't validate MediaSource '%ls'. the Blackmagic library was not initialized.", *GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' use the device '%ls' that doesn't exist on this machine.", *GetName(), *MediaConfiguration.MediaConnection.Device.DeviceName.ToString());
		return false;
	}

	if (!DeviceInfo.bIsSupported)
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' use the device '%ls' that is not supported by the Blackmagic SDK.", *GetName(), *MediaConfiguration.MediaConnection.Device.DeviceName.ToString());
		return false;
	}

	if (!DeviceInfo.bCanDoCapture)
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' use the device '%ls' that can't capture.", *GetName(), *MediaConfiguration.MediaConnection.Device.DeviceName.ToString());
		return false;
	}

	if (bUseTimeSynchronization && AutoDetectableTimecodeFormat == EMediaIOAutoDetectableTimecodeFormat::None)
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' use time synchronization but doesn't enabled the timecode.", *GetName());
		return false;
	}
	...
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' uses 'Timecode' evaluation type which requires time synchronization and timecode enabled.", *GetName());
		return false;
	}

	if (bFramelock && (!bRenderJIT || !bUseTimeSynchronization || AutoDetectableTimecodeFormat == EMediaIOAutoDetectableTimecodeFormat::None))
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' uses 'Framelock' which requires JIT rendering, time synchronization and timecode enabled.", *GetName());
		return false;
	}

	if (!bRenderJIT && EvaluationType == EMediaIOSampleEvaluationType::Latest)
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' uses 'Latest' evaluation type which requires JIT rendering.", *GetName());
		return false;
	}

	if (bFramelock)
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The MediaSource '%ls' uses 'Framelock' which has not been implemented yet. This option will be ignored.", *GetName());
	}
