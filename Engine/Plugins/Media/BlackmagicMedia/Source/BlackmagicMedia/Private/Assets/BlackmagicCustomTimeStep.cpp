...
namespace BlackmagicCustomTimeStepHelpers
{
	...
	public:
		...
		bool WaitForSync()
		{
			...
			{
				UE_LOGF(LogBlackmagicMedia, Warning, "The Engine couldn't run fast enough to keep up with the CustomTimeStep Sync. '%lld' frame(s) was dropped.", SyncCountDelta - ExpectedSyncCountDelta);
			}
...
bool UBlackmagicCustomTimeStep::Initialize(class UEngine* InEngine)
{
	...
	{
		UE_LOGF(LogBlackmagicMedia, Error, "The CustomTimeStep '%ls' can't be initialized. Blackmagic is not initialized on your machine.", *GetName());
		return false;
	}
...
bool UBlackmagicCustomTimeStep::UpdateTimeStep(class UEngine* InEngine)
{
	...
	if (CurrentState == ECustomTimeStepSynchronizationState::Synchronized)
	{
		...
		if (!bWarnedAboutVSync)
		{
			...
			if (bLockToVsync)
			{
				UE_LOGF(LogBlackmagicMedia, Warning, "The Engine is using VSync and may break the 'genlock'");
