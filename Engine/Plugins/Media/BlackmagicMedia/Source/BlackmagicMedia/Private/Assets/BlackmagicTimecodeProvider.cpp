	...
	private:
		...
		virtual void OnFrameReceived(const BlackmagicDesign::IInputEventCallback::FFrameReceivedInfo& InFrameInfo) override
		{
			...
			if (InFrameInfo.bHaveTimecode)
			{
				...
				{
					UE_LOGF(LogBlackmagicMedia, Warning, "BlackmagicTimecodeProvider input '%d' received an invalid Timecode frame number (%d) for the current frame rate (%ls).", ChannelInfo.DeviceIndex, InFrameInfo.Timecode.Frames, *FrameRate.ToPrettyText().ToString());
				}
			...
			else if (!bHasWarnedMissingTimecode)
			{
				...
				UE_LOGF(LogBlackmagicMedia, Warning, "BlackmagicTimecodeProvider input '%d' didn't receive any timecode in the last frame. Is your source configured correctly?", ChannelInfo.DeviceIndex);
			}
		}
		...
		virtual void OnFrameFormatChanged(const BlackmagicDesign::FFormatInfo& NewFormat) override
		{
			...
			UE_LOGF(LogBlackmagicMedia, Error, "The video format changed.");
...
bool UBlackmagicTimecodeProvider::Initialize(class UEngine* InEngine)
{
	...
	{
		UE_LOGF(LogBlackmagicMedia, Warning, "The configuration of '%ls' is not valid.", *GetName());
		return false;
	}

	if (!IBlackmagicMediaModule::Get().IsInitialized())
	{
		UE_LOGF(LogBlackmagicMedia, Error, "The TimecodeProvider '%ls' can't be initialized. Blackmagic is not initialized on your machine.", *GetName());
		return false;
	}

	if (TimecodeConfiguration.TimecodeFormat == EMediaIOAutoDetectableTimecodeFormat::None)
	{
		UE_LOGF(LogBlackmagicMedia, Error, "The TimecodeProvider '%ls' can't be initialized. Selected timecode format is invalid.", *GetName());
		return false;
	}
