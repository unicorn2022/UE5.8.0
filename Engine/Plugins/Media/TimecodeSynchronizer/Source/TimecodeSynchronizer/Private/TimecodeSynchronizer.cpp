	...
	private:
		...
		void ValidateSource(const FTimecodeSynchronizerActiveTimecodedInputSource& InputSource)
		{
			...
			if (!bUseRollover)
			{
				...
				{
					UE_LOGF(LogTimecodeSynchronizer, Warning, "Source %ls reported out of order frame times (Oldest = %d | Newest = %d)",
			...
			else
			{
				...
				{
					UE_LOGF(LogTimecodeSynchronizer, Warning, "Source %ls reported frames that go beyond expected rollover point (Oldest = %d | Newest = %d | Rollover = %d",
...
void UTimecodeSynchronizer::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	if (TimecodeProviderType == ETimecodeSynchronizationTimecodeType::InputSource)
	{
		...
		{
			MainSynchronizationSourceIndex = INDEX_NONE;
			UE_LOGF(LogTimecodeSynchronizer, Warning, "The MainSynchronizationSourceIndex is not valid.");
		}
	}
...
FFrameTime UTimecodeSynchronizer::GetProviderFrameTime() const
{
	...
	if (TimecodeProviderType == ETimecodeSynchronizationTimecodeType::InputSource)
	{
		if (SynchronizedSources.IsValidIndex(ActiveMainSynchronizationTimecodedSourceIndex))
		{
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Log, "Unable to get frame time - Specified source was not ready.");
			}
		}
		else
		{
			UE_LOGF(LogTimecodeSynchronizer, Log, "Unable to get frame time - Invalid source specified.");
		}
	}
...
void UTimecodeSynchronizer::Register()
{
	if (!bRegistered)
	{
		...
		if (FrameRateSource == ETimecodeSynchronizationFrameRateSources::EngineCustomTimeStepFrameRate)
		{
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Error, "Engine does not have Genlock in place.");
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Error, "Engine CustomTimeStep must be a FixedFrameRateCustomTimeStep.");
				SwitchState(ESynchronizationState::Error);
				return;
			}
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "TimecodeProvider failed to be set on Engine.");
			SwitchState(ESynchronizationState::Error);
			return;
		}
...
void UTimecodeSynchronizer::OnTimecodeProviderChanged()
{
	...
	{
		bShouldResetTimecodeProvider = false;
		UE_LOGF(LogTimecodeSynchronizer, Error, "The Engine's TimecodeProvider changed.");
		SwitchState(ESynchronizationState::Error);
		return;
	}
}
...
void UTimecodeSynchronizer::Tick(float DeltaTime)
{
	...
	if (IsSynchronizing() && bUsePreRollingTimeout)
	{
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "PreRoll Timeout.");
			SwitchState(ESynchronizationState::Error);
		}
	}
}
...
bool UTimecodeSynchronizer::StartSynchronization()
{
	...
	{
		UE_LOGF(LogTimecodeSynchronizer, Log, "Already synchronizing or synchronized.");
		return true;
	}
	else
	{
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "StartSynchronization called without properly closing sources");
			CloseSources();
		}
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Warning, "No sources available to synchronize.");
			SwitchState(ESynchronizationState::Error);
		}
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Warning, "The Main Synchronization Source could not be found.");
			SwitchState(ESynchronizationState::Error);
		}
		else
		{
...
bool UTimecodeSynchronizer::Tick_TestGenlock()
{
	if (FrameRateSource == ETimecodeSynchronizationFrameRateSources::EngineCustomTimeStepFrameRate)
	{
		if (RegisteredCustomTimeStep == nullptr)
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "The registered Genlock source is invalid.");
			SwitchState(ESynchronizationState::Error);
			return false;
		}
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "The registered Genlock source is not the Engine CustomTimeStep.");
			SwitchState(ESynchronizationState::Error);
			return false;
		}
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "The Genlock source stopped while synchronizing.");
			SwitchState(ESynchronizationState::Error);
			return false;
		}
...
bool UTimecodeSynchronizer::Tick_TestTimecode()
{
	if (GEngine->GetTimecodeProvider() != this)
	{
		UE_LOGF(LogTimecodeSynchronizer, Error, "The registered TimecodeProvider is not the Engine TimecodeProvider.");
		SwitchState(ESynchronizationState::Error);
		return false;
	}
	...
	if (TimecodeProviderType == ETimecodeSynchronizationTimecodeType::InputSource)
	{
		if (!SynchronizedSources.IsValidIndex(ActiveMainSynchronizationTimecodedSourceIndex))
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "The InputSource '%d' that we try to synchronize on is not valid.", ActiveMainSynchronizationTimecodedSourceIndex);
			SwitchState(ESynchronizationState::Error);
			return false;
		}
	...
	else 
	{
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "The TimecodeProvider became invalid while synchronizing.");
			SwitchState(ESynchronizationState::Error);
			return false;
		}
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "The TimecodeProvider stopped while synchronizing.");
			SwitchState(ESynchronizationState::Error);
			return false;
		}
		...
		{
			UE_LOGF(LogTimecodeSynchronizer, Error, "The TimecodeProvider frame rate do not correspond to the specified frame rate.");
			SwitchState(ESynchronizationState::Error);
		}
...
void UTimecodeSynchronizer::TickPreRolling_WaitReadiness()
{
	...
	for (const FTimecodeSynchronizerActiveTimecodedInputSource& InputSource : SynchronizedSources)
	{
		if (InputSource.IsReady())
		{
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Warning, "Source %ls doesn't have a frame rate common to TimecodeSynchronizer frame rate.", *InputSource.GetDisplayName())
			}
		}
		else
		{
...
void UTimecodeSynchronizer::Tick_Synchronized()
{
	...
	if (CachedSyncState.RolloverFrame.IsSet())
	{
		for (const FTimecodeSynchronizerActiveTimecodedInputSource& InputSource : SynchronizedSources)
		{
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Warning, "Source '%ls' doesn't have the timecode ready.", *InputSource.GetDisplayName());
			}
		}
	}
	else
...
void UTimecodeSynchronizer::OpenSources()
{
	...
	for (int32 Index = 0; Index < TimeSynchronizationInputSources.Num(); ++Index)
	{
		if (UTimeSynchronizationSource* InputSource = TimeSynchronizationInputSources[Index])
		{
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Warning, "Source %d, could not be open.", Index);
			}
		}
	}
...
void UTimecodeSynchronizer::UpdateSourceStates()
{
	...
	if (GFrameCounter != LastUpdatedSources)
	{
		...
		if (InvalidSources.Num() > 0)
		{
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Error, "Source '%ls' is not valid during State '%ls'", *(InvalidSource->GetDisplayName()), *StateString);
			}
		}
		...
		if (bTreatUnreadyAsError && UnreadySources.Num() > 0)
		{
			...
			{
				UE_LOGF(LogTimecodeSynchronizer, Error, "Source '%ls' became unready during State '%ls'", *(UnreadySource->GetDisplayName()), *StateString);
			}
		}
