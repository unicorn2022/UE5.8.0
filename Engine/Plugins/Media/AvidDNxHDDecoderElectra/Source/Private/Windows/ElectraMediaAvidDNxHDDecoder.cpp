...
public:
	...
	virtual bool GetBufferTextureSyncByIndex(int32 InBufferIndex, FElectraDecoderOutputSync& SyncObject) const override
	{
		...
		if (InBufferIndex == 0)
		{
			SyncObject = { GPUBuffer.Fence.GetReference(), GPUBuffer.FenceValue, nullptr };
			return true;
		}
#endif
	return false;
...
public:
	...
	FVideoDecoderAvidDNxHDElectra(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
class FAvidDNxHDVideoDecoderElectraFactory : public IElectraCodecFactory, public IElectraCodecFactory::IProviderInformation, public IElectraCodecModularFeature, public TSharedFromThis<FAvidDNxHDVideoDecoderElectraFactory, ESPMode::ThreadSafe>
{
public:
	...
	{
		return InCodecFormat.Properties.IsType<Electra::FCodecTypeFormat::FVideo>() ? MakeShared<FVideoDecoderAvidDNxHDElectra, ESPMode::ThreadSafe>(InCodecFormat, InAdditionalOptions) : TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe>();
	}
...
void FElectraMediaAvidDNxHDDecoder::Startup()
{
	...
	{
		UE_LOGF(LogAvidDNxHDElectraDecoder, Error, "Failed to load required library. Plug-in will not be functional.");
		return;
	}
/*
...
*/
	...
	{
		UE_LOGF(LogAvidDNxHDElectraDecoder, Error, "DNX_Initialize() returned error %d. Plug-in will not be functional.", Result);
		return;
	}
...
FVideoDecoderAvidDNxHDElectra::FVideoDecoderAvidDNxHDElectra(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
...
int32 FVideoDecoderAvidDNxHDElectra::FDecoderHandle::PrepareForDecoding(const void* InInput, uint32 InInputSize, int32 InNumDecodeThreads, bool bIgnoreAlpha)
{
	...
	if (Result != DNX_NO_ERROR)
	{
		UE_LOGF(LogAvidDNxHDElectraDecoder, Log, "DNX_GetInfoFromCompressedFrame() failed with %d", Result);
		return Result;
	}
	...
	if (!Handle)
	{
		...
		if (Result != DNX_NO_ERROR)
		{
			UE_LOGF(LogAvidDNxHDElectraDecoder, Log, "DNX_CreateDecoder() failed with %d", Result);
			return Result;
		}
...
bool FVideoDecoderAvidDNxHDElectra::FDecoderHandle::SetupDecompressStruct()
{
	...
	if (CurrentCompressedParams.colorFormat == DNX_ColorFormat_t::DNX_CF_YCbCr)
	{
		switch(CurrentCompressedParams.depth)
		{
			case 8:
			{
				...
				{
					UE_LOGF(LogAvidDNxHDElectraDecoder, Log, "Compressed alpha with YCbYCr is not currently handled");
					return false;
				}
			...
			default:
			{
				UE_LOGF(LogAvidDNxHDElectraDecoder, Log, "Compressed %d bit depth is not currently handled", CurrentCompressedParams.depth);
				return false;
			}
		}
	}
	...
	{
		UE_LOGF(LogAvidDNxHDElectraDecoder, Log, "Compressed RGB format is not currently handled");
		return false;
	}
