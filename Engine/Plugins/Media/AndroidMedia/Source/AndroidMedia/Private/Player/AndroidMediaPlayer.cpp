...
bool FAndroidMediaPlayer::Open(const FString& Url, const IMediaOptions* /*Options*/)
{
	...
	if (Url.StartsWith(TEXT("file://")))
	{
		...
		if (!PlatformFile.FileExists(*FilePath))
		{
			...
			{
				UE_LOGF(LogAndroidMedia, Warning, "File doesn't exist %ls.", *FilePath);
				return false;
			}
			...
			if (FileEntry.CompressionMethodIndex == 0 && !FileEntry.IsEncrypted())
			{
				...
				//UE_LOGF(LogAndroidMedia, Warning, "not compressed or encrypted PAK: Filename for PAK is: %ls", *PakFilename);
				//UE_LOGF(LogAndroidMedia, Warning, "PAK Offset: %lld,  Size: %lld (header=%lld)", OffsetInPak, FileSize, PakHeaderSize);
				...
				//UE_LOGF(LogAndroidMedia, Warning, "Final Offset: %lld in %ls", FileOffset, *FileRootPath);

				if (!JavaMediaPlayer->SetDataSource(FileRootPath, FileOffset, FileSize))
				{
					UE_LOGF(LogAndroidMedia, Warning, "Failed to set data source for file in PAK %ls", *FilePath);
					return false;
				}
			}
			else
			{
				...
				{
					UE_LOGF(LogAndroidMedia, Warning, "File cannot be played on this version of Android due to PAK file with compression or encryption: %ls.", *FilePath);
					return false;
				}
				...
				{
					UE_LOGF(LogAndroidMedia, Warning, "Failed to set data source for archive %ls", *FilePath);
					return false;
				}
			}
		}
		else
		{
			...
			if (PlatformFile.IsAsset(*FilePath))
			{
				...
				{
					UE_LOGF(LogAndroidMedia, Warning, "Failed to set data source for asset %ls", *FilePath);
					return false;
				}
			}
			else
			{
				if (!JavaMediaPlayer->SetDataSource(FileRootPath, FileOffset, FileSize))
				{
					UE_LOGF(LogAndroidMedia, Warning, "Failed to set data source for file %ls", *FilePath);
					return false;
				}
			}
		}
	...
	else
	{
		...
		{
			UE_LOGF(LogAndroidMedia, Warning, "Failed to set data source for URL %ls", *Url);
			return false;
		}
	}
	...
	{
		UE_LOGF(LogAndroidMedia, Warning, "Failed to prepare media source %ls", *Url);
		return false;
	}
...
bool FAndroidMediaPlayer::Open(const TSharedRef<FArchive, ESPMode::ThreadSafe>& Archive, const FString& OriginalUrl, const IMediaOptions* /*Options*/)
{
	...
	{
		UE_LOGF(LogAndroidMedia, Verbose, "Player %p: Cannot open media from archive (archive is empty)", this);
		return false;
	}

	if (OriginalUrl.IsEmpty())
	{
		UE_LOGF(LogAndroidMedia, Verbose, "Player %p: Cannot open media from archive (no original URL provided)", this);
		return false;
	}
	...
	if (FAndroidMisc::GetAndroidBuildVersion() < 23)
	{
		UE_LOGF(LogAndroidMedia, Warning, "File cannot be played on this version of Android due to PAK file with compression or encryption: %ls.", *OriginalUrl);
		return false;
	}

	if (!JavaMediaPlayer->SetDataSource(Archive))
	{
		UE_LOGF(LogAndroidMedia, Warning, "Failed to set data source for archive %ls", *OriginalUrl);
		return false;
	}
	...
	if (!JavaMediaPlayer->PrepareAsync())
	{
		UE_LOGF(LogAndroidMedia, Warning, "Failed to prepare media source %ls", *OriginalUrl);
		return false;
	}
...
static void ConditionalInitializeVideoTexture_RenderThread(FRHICommandListBase& RHICmdList, TWeakPtr<FJavaAndroidMediaPlayer, ESPMode::ThreadSafe> JavaMediaPlayerPtr, FGuid PlayerGuid)
{
	...
	if (VideoTexture == nullptr)
	{
		...
		if (VideoTexture == nullptr)
		{
			UE_LOGF(LogAndroidMedia, Warning, "CreateTexture failed!");
			return;
		}
...
bool FAndroidMediaPlayer::SelectTrack(EMediaTrackType TrackType, int32 TrackIndex)
{
	...
	case EMediaTrackType::Audio:
		...
		{
			UE_LOGF(LogAndroidMedia, Verbose, "Player %p: Selecting audio track %i instead of %i (%i tracks)", this, TrackIndex, SelectedVideoTrack, AudioTracks.Num());

			if (TrackIndex == INDEX_NONE)
			{
				UE_LOGF(LogAndroidMedia, VeryVerbose, "Player %p: Disabling audio", this);
			...
			else
			{
				...
				UE_LOGF(LogAndroidMedia, VeryVerbose, "Player %p: Enabling audio", this);
	...
	case EMediaTrackType::Caption:
		...
		{
			UE_LOGF(LogAndroidMedia, Verbose, "Player %p: Selecting caption track %i instead of %i (%i tracks)", this, TrackIndex, SelectedCaptionTrack, CaptionTracks.Num());

			if (TrackIndex == INDEX_NONE)
			{
				UE_LOGF(LogAndroidMedia, VeryVerbose, "Player %p: Disabling captions", this);
			}
			else
			{
				...
				UE_LOGF(LogAndroidMedia, VeryVerbose, "Player %p: Enabling captions", this);
			}
	...
	case EMediaTrackType::Video:
		...
		{
			UE_LOGF(LogAndroidMedia, Verbose, "Player %p: Selecting video track %i instead of %i (%i tracks).", this, TrackIndex, SelectedVideoTrack, VideoTracks.Num());

			if (TrackIndex == INDEX_NONE)
			{
				UE_LOGF(LogAndroidMedia, VeryVerbose, "Player %p: Disabling video", this);
				JavaMediaPlayer->SetVideoEnabled(false);
			}
			else
			{
				...
				UE_LOGF(LogAndroidMedia, VeryVerbose, "Player %p: Enabling video", this);
				JavaMediaPlayer->SetVideoEnabled(true);
			}
...
{
	UE_LOGF(LogAndroidMedia, Verbose, "Player %p: Seeking to %ls", this, *Time.ToString());

	if ((CurrentState == EMediaState::Closed) ||
		(CurrentState == EMediaState::Error) ||
		(CurrentState == EMediaState::Preparing))
	{
		UE_LOGF(LogAndroidMedia, Warning, "Cannot seek while closed, preparing, or in error state");
		return false;
	}
...
bool FAndroidMediaPlayer::SetRate(float Rate)
{
	if ((CurrentState == EMediaState::Closed) ||
		...
		(CurrentState == EMediaState::Preparing))
	{
		UE_LOGF(LogAndroidMedia, Warning, "Cannot set rate while closed, preparing, or in error state");
		return false;
	}
	...
	UE_LOGF(LogAndroidMedia, Verbose, "Player %p: Setting rate from to %f to %f", this, GetRate(), Rate);
	...
	{
		UE_LOGF(LogAndroidMedia, Warning, "The rate %f is not supported", Rate);
		return false;
	}
