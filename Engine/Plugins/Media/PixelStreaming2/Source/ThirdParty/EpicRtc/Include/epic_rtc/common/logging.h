...
class EpicRtcLoggerInterface : public EpicRtcRefCountInterface
{
public:
    virtual void Log(const EpicRtcLogMessage& message) = 0;

    // Prevent copying
    EpicRtcLoggerInterface(const EpicRtcLoggerInterface&) = delete;
    EpicRtcLoggerInterface& operator=(const EpicRtcLoggerInterface&) = delete;

protected:
    // Only class Implementation can be constructed or destroyed
    EPICRTC_API EpicRtcLoggerInterface() = default;
    virtual EPICRTC_API ~EpicRtcLoggerInterface() = default;
};
