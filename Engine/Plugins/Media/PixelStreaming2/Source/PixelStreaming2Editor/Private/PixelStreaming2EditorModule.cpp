...
namespace UE::EditorPixelStreaming2
{
	...
	void FPixelStreaming2EditorModule::StartStreaming(EPixelStreaming2EditorStreamTypes InStreamType)
	{
		...
		switch (InStreamType)
		{
			...
			default:
			{
				UE_LOGF(LogPixelStreaming2Editor, Warning, "Specified Stream Type doesn't have an associated FPixelStreaming2VideoProducer");
			}
	...
	void FPixelStreaming2EditorModule::StartSignalling()
	{
		...
		LaunchArgs.ProcessArgs = FString::Printf(TEXT("--HttpPort=%d --StreamerPort=%d --ServeHttps=%s --EnableIPv6=true %s"), ViewerPort, StreamerPort, bServeHttps ? TEXT("true") : TEXT("false"), bServeHttps ? *CertificateArgs : TEXT(""));

		FString AllowedOrigins = UPixelStreaming2PluginSettings::CVarAllowedOrigins.GetValueOnAnyThread();
		if (!AllowedOrigins.IsEmpty())
		{
			LaunchArgs.ProcessArgs += FString::Printf(TEXT(" --AllowedOrigins=%s"), *AllowedOrigins);
		}
