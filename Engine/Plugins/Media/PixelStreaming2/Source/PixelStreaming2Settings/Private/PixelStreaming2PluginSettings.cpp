...
static const TSet<TPair<FString, FString>> GetCmdArg = {
	...
	{ "PixelStreaming2.InitializeDefaultStreamer", "InitializeDefaultStreamer" },
	...
	{ "PixelStreaming2.InternalSignallingServer.AllowedOrigins", "AllowedOrigins" }
};
...
TAutoConsoleVariable<bool> UPixelStreaming2PluginSettings::CVarLogStats(
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			if (FDelegates *Delegations = Delegates())
			{
				Delegations->OnLogStatsChanged.Broadcast(Var);
			}
		}),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnEpicRtcLogFilterChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnDecoupleFramerateChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnUseMediaCaptureChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);

TAutoConsoleVariable<bool> UPixelStreaming2PluginSettings::CVarInitializeDefaultStreamer(
	TEXT("PixelStreaming2.InitializeDefaultStreamer"),
	true,
	TEXT("If the default Pixel Streaming 2 streamer should be initialized. You may wish to set this to false if you only plan on receiving streams. Default: true"),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnCaptureUseFenceChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnDebugDumpAudioChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);
...
TAutoConsoleVariable<bool> UPixelStreaming2PluginSettings::CVarEncoderEnableSimulcast(
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			VerifyCVarVideoSettings(nullptr); 
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnSimulcastEnabledChanged.Broadcast(Var);
			}
		}),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			...
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnScalabilityModeChanged.Broadcast(Var);
			}
		}),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var)
		{
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnEncoderDebugDumpFrameChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnWebRTCFpsChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);
...
TAutoConsoleVariable<int32> UPixelStreaming2PluginSettings::CVarWebRTCMinBitrate(
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{ 
			if (FDelegates* Delegations = Delegates())
			{ 
				Delegations->OnWebRTCBitrateChanged.Broadcast(Var); 
			} 
		}),
	ECVF_RenderThreadSafe);
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var)
		{
			if (FDelegates* Delegations = Delegates())
			{ 
				Delegations->OnWebRTCDisableStatsChanged.Broadcast(Var); 
			} 
		}),
	ECVF_Default);
...
TAutoConsoleVariable<FString> UPixelStreaming2PluginSettings::CVarInputKeyFilter(
	...
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* Var) 
		{ 
			if (FDelegates* Delegations = Delegates())
			{
				Delegations->OnInputKeyFilterChanged.Broadcast(Var); 
			}
		}),
	ECVF_Default);
// End Input CVars

// Begin Signalling Server CVars
TAutoConsoleVariable<FString> UPixelStreaming2PluginSettings::CVarAllowedOrigins(
	TEXT("PixelStreaming2.InternalSignallingServer.AllowedOrigins"),
	TEXT("http://127.0.0.1,https://127.0.0.1"),
	TEXT("Comma-separated list of allowed origins for WebSocket connection filtering. Supports wildcards (*). Use '*' to allow all origins. Leave empty to disable filtering."),
	ECVF_Default);
// End Signalling Server CVars
