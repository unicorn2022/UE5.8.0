...
public:
	...
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
		...
		DisplayName = "Enable Pixel Streaming 2 Toolbar",
		ToolTip = "Adds the Pixel Streaming 2 toolbar to the main viewport toolbar."
		))
	bool EnablePixelStreamingToolbar = true;
	...
	static UE_API TAutoConsoleVariable<bool> CVarInitializeDefaultStreamer;
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
		DisplayName = "Initialize Default Streamer",
		ToolTip = "If the default Pixel Streaming 2 streamer should be initialized. You may wish to set this to false if you only plan on receiving streams. Default: true"
		))
	bool InitializeDefaultStreamer = true;
	...
	UPROPERTY(config, EditAnywhere, Category = "PixelStreaming", meta = (
		...
		EditCondition="InitializeDefaultStreamer"
		))
	...
	static UE_API TAutoConsoleVariable<FString> CVarAllowedOrigins;
	UPROPERTY(config, EditAnywhere, Category = "SignallingServer", meta = (
		DisplayName = "Allowed Origins",
		ToolTip = "Comma-separated list of allowed origins for WebSocket connection filtering. Supports wildcards (*). Use '*' to allow all origins. Leave empty to disable filtering. Examples: \"*\"  or \"https://example.com,http://192.168.0.*:8080\". Default: \"http://127.0.0.1,https://127.0.0.1\""
		))
	FString AllowedOrigins = TEXT("http://127.0.0.1,https://127.0.0.1");
