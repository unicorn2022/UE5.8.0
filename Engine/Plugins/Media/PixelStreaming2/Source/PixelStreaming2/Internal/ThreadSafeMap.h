...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		 ...
		 * @return whether or not the key was found
		 */
		FORCEINLINE ValueType FindAndRemoveChecked(KeyInitType Key)
		{
			FScopeLock Lock(&Mutex);
		...
		FORCEINLINE ValueType& operator[](KeyInitType Key)
		{
			FScopeLock Lock(&Mutex);
		...
		FORCEINLINE const ValueType& operator[](KeyInitType Key) const
		{
			FScopeLock Lock(&Mutex);
		...
		[[nodiscard]] FORCEINLINE bool Contains(KeyInitType Key)
		{
			FScopeLock Lock(&Mutex);
