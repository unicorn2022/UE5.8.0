...
namespace UE::PixelStreaming2
{
	...
	void FPixelStreaming2Module::StartupModule()
	{
			...
			))
		{
			...
			UE_LOGF(LogPixelStreaming2, Warning, "Only D3D11/D3D12/Vulkan/Metal/OpenGL(Android only) Dynamic RHI is supported. Detected %ls", GDynamicRHI != nullptr ? GDynamicRHI->GetName() : TEXT("[null]"));
	#endif
			return;
		}
		...
		FCoreDelegates::OnAllModuleLoadingPhasesComplete.AddLambda([this]() {
			...
			if (!GIsEditor && UPixelStreaming2PluginSettings::CVarInitializeDefaultStreamer.GetValueOnAnyThread())
			{
	...
	TSharedPtr<IPixelStreaming2Streamer> FPixelStreaming2Module::CreateStreamer(const FString& StreamerId, TOptional<FString> Type)
	{
		...
		IPixelStreaming2StreamerFactory*	 Factory = IPixelStreaming2StreamerFactory::Get(Type.Get(UPixelStreaming2PluginSettings::CVarDefaultStreamerType.GetValueOnAnyThread()));
