...
bool UPixelStreaming2AudioComponent::StreamerListenTo(FString StreamerId, FString PlayerToListenTo)
{
	...
	{
		UE_LOGF(LogPixelStreaming2, Verbose, "Pixel Streaming audio component could not listen to anything because Pixel Streaming module is not loaded. This is expected on dedicated servers.");
		return false;
	}
...
void UPixelStreaming2AudioComponent::ConsumeRawPCM(const int16_t* AudioData, int SampleRate, size_t InNumChannels, size_t NumFrames)
{
...
void UPixelStreaming2AudioComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	{
		UE_LOGF(LogPixelStreaming2, Log, "PixelStreaming2 audio component found a WebRTC peer to listen to.");
	}
}
