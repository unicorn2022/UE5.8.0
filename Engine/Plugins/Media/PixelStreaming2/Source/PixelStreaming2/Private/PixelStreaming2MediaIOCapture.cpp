...
BEGIN_SHADER_PARAMETER_STRUCT(FWriteFenceParameters, )
	...
	FVector2D											   CropV)
{
	...
	{
		TWeakObjectPtr<UPixelStreaming2MediaIOCapture> WeakThis(this);

		FWriteFenceParameters* PassParams = GraphBuilder.AllocParameters<FWriteFenceParameters>();
		PassParams->Texture = OutputTexture;
		...
		GraphBuilder.AddPass(
			...
			[WeakThis, PassParams](FRDGAsyncTask, FRHICommandList& RHICmdList) {
				if (TStrongObjectPtr<UPixelStreaming2MediaIOCapture> This = WeakThis.Pin())
				{
					FGPUFenceRHIRef Fence = GDynamicRHI->RHICreateGPUFence(TEXT("UPixelStreaming2MediaIOCapture Fence"));
					This->Fences.Add(reinterpret_cast<uintptr_t>(PassParams->Texture->GetRHI()), Fence);
...
{
	UE_LOGF(LogPixelStreaming2, Log, "Initializing Media IO capture for Pixel Streaming.");
...
void UPixelStreaming2MediaIOCapture::WaitForGPU(FRHITexture* InRHITexture)
{
	...
	double MaxWaitTime = FPlatformTime::Seconds() + 0.1;
	while (!Fences.RemoveAndCopyValue(reinterpret_cast<uintptr_t>(InRHITexture), Fence))
	{
		if (FPlatformTime::Seconds() >= MaxWaitTime)
		{
			UE_LOGFMT(LogPixelStreaming2, Error, "UPixelStreaming2MediaIOCapture::WaitForGPU: Timed out waiting for fence to be added in pass!");
			return;
		}
	...
	if (!Fence.IsValid())
	{
		UE_LOGFMT(LogPixelStreaming2, Error, "UPixelStreaming2MediaIOCapture::WaitForGPU: Found fence is invalid!");
		return;
	}

	MaxWaitTime = FPlatformTime::Seconds() + 0.1;
	while (!Fence->Poll())
	{
		if (FPlatformTime::Seconds() >= MaxWaitTime)
		{
			UE_LOGFMT(LogPixelStreaming2, Error, "UPixelStreaming2MediaIOCapture::WaitForGPU: Timed out waiting for fence to be written!");
			return;
		}
...
void UPixelStreaming2MediaIOCapture::ViewportResized(FViewport* Viewport, uint32 ResizeCode)
{
	...
	{
		UE_LOGF(LogPixelStreaming2, Warning, "Stopping PixelStreaming MediaIO capture because viewport was resized.");
		StopCapture(false);
	}
}
