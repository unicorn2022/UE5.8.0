...
namespace UE::PixelStreaming2
{
	...
	void FPixelStreaming2RTCStreamPlayer::OnDataTrackMessage(EpicRtcDataTrackInterface* DataTrack, const EpicRtcDataFrame& DataFrame)
	{
	}
	...
	void FPixelStreaming2RTCStreamPlayer::ConsumeRawPCM(const int16_t* AudioData, int SampleRate, size_t NumChannels, size_t NumFrames)
	{
		if (AudioData == nullptr || SampleRate <= 0 || NumChannels == 0 || NumFrames == 0)
		{
			return;
		}
		...
		AudioSample->Initialize(AudioBuffer.GetData(), DataSize, NumChannels, SampleRate, FTimespan::Zero(), Duration);
