...
namespace UE::PixelStreaming2
{
	...
	bool FPS2EpicRtcConferenceLifetimeTest::RunTest(const FString& Parameters)
	{
...

		{
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate conference. Unexpected result. Expected (%ls), Actual (%ls)", *ToString(EpicRtcErrorCode::ConferenceDoesNotExists), *ToString(Result));
				return false;
			}

			if (NullConference.GetReference() != nullptr)
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate conference. Expected NullConference to reference a nullptr");
				return false;
			}
		}
	...
	bool FPS2EpicRtcConferenceCreateTest::RunTest(const FString& Parameters)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate conference. Unexpected result. Expected (%ls), Actual (%ls)", *ToString(EpicRtcErrorCode::ConferenceAlreadyExists), *ToString(Result));
			return false;
		}
	...
	bool FPS2EpicRtcConferenceGetTest::RunTest(const FString& Parameters)
	{
...

		{
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Expected Conference and SameConference to reference the same pointer");
				return false;
			}
