...
namespace UE::PixelStreaming2
{
	...
	bool FPS2EpicRtcRoomLifetimeTest::RunTest(const FString& Parameters)
	{
		...
		Manager->OnSessionErrorUpdateNative.AddLambda([](EpicRtcErrorCode Error) {
			UE_LOGF(LogPixelStreaming2RTC, Error, "Manager->OnSessionErrorUpdate() {%d}", EnumToUnderlyingType(Error));
		});

		Manager->OnRoomStateUpdateNative.AddLambda([this, &RoomState](const EpicRtcRoomState State) {
			UE_LOGF(LogPixelStreaming2RTC, Log, "Manager->OnRoomStateUpdate() Old State (%ls), New State (%ls)", *ToString(RoomState), *ToString(State));
			RoomState = State;
		});
