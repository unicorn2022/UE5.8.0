...
namespace UE::PixelStreaming2
{
	...
	{
		UE_LOGF(LogPixelStreaming2RTC, Log, "FSendCustomMessageToStreamer: %ls", *MessageType);
		if (Player->DataChannelAvailable())
		{
			if (!Player->SendMessage(MessageType, Body))
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Data channel send message failed.");
			}
		}
		else
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "No DataChannel on player.");
		}
	...
	{
		UE_LOGF(LogPixelStreaming2RTC, Log, "SendDataChannelMessageToStreamer: %ls, %ls", *MessageType, *Body);
		if (Player->DataChannelAvailable())
		{
			if (!Player->SendMessage(MessageType, Body))
			{
	...
	{
		UE_LOGF(LogPixelStreaming2RTC, Log, "SendDataChannelMessageFromStreamer: %ls, %ls", *MessageType, *Body);
		if (Streamer)
		{
	...
	bool FWaitForFrameReceived::Update()
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "Successfully received streamed frame.");
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "%ls", *WidthTestString);
			}
			else
			{
				UE_LOGF(LogPixelStreaming2RTC, Log, "%ls", *WidthTestString);
			}

			FEpicRtcVideoBuffer* const BaseFrameBuffer = static_cast<FEpicRtcVideoBuffer*>(Buffer.GetReference());
			if (!BaseFrameBuffer)
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Invalid frame buffer");
				return true;
			}

			if (BaseFrameBuffer->GetBufferFormat() != PixelCaptureBufferFormat::FORMAT_I420)
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Invalid Pixel Format");
				return true;
			}
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "%ls", *PixelTestString);
			}
			else
			{
				UE_LOGF(LogPixelStreaming2RTC, Log, "%ls", *PixelTestString);
			}
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Timed out waiting to receive a frame of video through the video sink.");
			return true;
		}
		return false;
	}
	...
	bool FWaitForDataChannelOrTimeout::Update()
	{
		...
		if (DeltaTime > TimeoutSeconds)
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Timed out waiting to data channel to be available.");
			return true;
		}
	...
	bool FWaitForDataChannelMessageOrTimeout::Update()
	{
		...
		if (DeltaTime > TimeoutSeconds)
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Player timed out waiting for a datachannel message.");
			return true;
		}
	...
	TSharedPtr<IPixelStreaming2Streamer> CreateStreamer(const FString& StreamerName, int StreamerPort)
	{
		...
		OutStreamer->OnStreamingStarted().AddLambda([](IPixelStreaming2Streamer*) {
			UE_LOGF(LogPixelStreaming2RTC, Verbose, "CreateStreamer: Streamer Connected");
		});
	...
	TSharedPtr<UE::PixelStreaming2Servers::IServer> CreateSignallingServer(int StreamerPort, int PlayerPort)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to launch signalling server.");
		}
		UE_LOGF(LogPixelStreaming2RTC, Log, "Signalling server launched=%ls", bLaunchedSignallingServer ? TEXT("true") : TEXT("false"));
