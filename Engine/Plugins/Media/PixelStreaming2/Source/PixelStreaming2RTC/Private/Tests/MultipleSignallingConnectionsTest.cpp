...
namespace UE::PixelStreaming2
{
	...
	bool FWaitForStreamConnected::Update()
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Streamer not found");
			return true;
		}
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Timed out waiting for streamer to dis/connect to signalling server.");
			return true;
		}

		if (*bIsDisconnected.Get() == true)
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Streamer should not be Disconnected");
		}

		if (*bIsConnected.Get() == true)
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "Streamer is Connected as expected");
			return true;
		}
	...
	bool FWaitForStreamDisconnected::Update()
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "Streamer is Disconnected as expected");
			return true;
		}
	...
	bool FCheckNumConnected::Update()
	{
		...
		if (bHasNumStreamers)
		{
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Log, "Expected %d streamers and found %d", NumExpected, NumStreamers);
				return true;
			}
			else
			{
		...
		if (DeltaTime > TimeoutSeconds)
		{
			if (bHasNumStreamers)
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Expected %d streamers but found %d", NumExpected, NumStreamers);
			}
			else
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Timed out waiting for number of streamers to be retrieved.");
			}
			return true;
		}
	...
	{
		UE_LOGF(LogPixelStreaming2RTC, Log, "----------- ConnectAndDisconnectMultipleStreamersEmbeddedCirrus -----------");
		...
		UE_LOGF(LogPixelStreaming2RTC, Log, "Embedded cirrus launched: %ls", bLaunched ? TEXT("true") : TEXT("false"));
