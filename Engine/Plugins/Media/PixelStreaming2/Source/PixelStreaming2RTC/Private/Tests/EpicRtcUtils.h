...
namespace UE::PixelStreaming2
{
	...
	public:
		...
		DECLARE_TS_MULTICAST_DELEGATE_TwoParams(FOnDataTrackMessage, EpicRtcDataTrackInterface*, const EpicRtcDataFrame&);
	...
	public:
		...
		virtual void OnDataTrackMessage(EpicRtcDataTrackInterface* DataTrack, const EpicRtcDataFrame& DataFrame) override
		{
			OnDataTrackMessageNative.Broadcast(DataTrack, DataFrame);
		}
	...
	public:
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "FMockWebSocketFactory");
		}
	...
	bool ValidateRefCount(TRefCountPtr<RefCountClass>& Class, FString Name, uint32_t ExpectedCount)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate %ls. GetReference() = nullptr", *Name);
			return false;
		}

		if (Class->Count() != ExpectedCount)
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate %ls. Has invalid reference count. Expected (%d), Actual (%d)", *Name, ExpectedCount, Class->Count());
			return false;
		}
	...
	bool ValidateResultRefCount(TRefCountPtr<RefCountClass>& Class, FString Name, EpicRtcErrorCode Result, TArray<EpicRtcErrorCode> ExpectedResult, uint32_t ExpectedCount)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate %ls. Unexpected result. Expected one of ([%ls]), Actual (%ls)", *Name, *ToString(ExpectedResult), *ToString(Result));
			return false;
		}
	...
	inline bool ValidatePlatform(TRefCountPtr<EpicRtcPlatformInterface>& Platform, EpicRtcErrorCode Result, TArray<EpicRtcErrorCode> ExpectedResult, uint8_t ExpectedCount)
	{
		...
		if (!ExpectedResult.Contains(Result))
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate platform. Unexpected result. Expected one of ([%ls]), Actual (%ls)", *ToString(ExpectedResult), *ToString(Result));
			return false;
		}

		if (Platform.GetReference() == nullptr)
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate platform. Platform.GetReference() = nullptr");
			return false;
		}
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "Failed to validate platform. Platform has invalid reference count. Expected (%d), Actual (%d)", ExpectedCount, Platform->Count());
			return false;
		}
