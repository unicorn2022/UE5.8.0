...
namespace UE::PixelStreaming2
{
	...
	void FEpicRtcWebsocket::Disconnect(const EpicRtcStringView InReason)
	{
		...
		if (WebSocket->IsConnected() && !bCloseRequested)
		{
			...
			UE_LOGF(LogEpicRtcWebsocket, Log, "Closing websocket to %ls", *Url);
	...
	{
		UE_LOGF(LogEpicRtcWebsocket, Log, "Websocket connection made to: %ls", *Url);
	...
	void FEpicRtcWebsocket::OnConnectionError(const FString& Error)
	{
		...
		UE_LOGF(LogEpicRtcWebsocket, Log, "Failed to connect to %ls - signalling server may not be up yet. Message: \"%ls\"", *Url, *Error);
		Observer->OnClosed();
	}

	void FEpicRtcWebsocket::OnClosed(int32 StatusCode, const FString& Reason, bool bWasClean)
	{
		UE_LOGF(LogEpicRtcWebsocket, Log, "Closed connection to %ls - \n\tstatus %d\n\treason: %ls\n\twas clean: %ls", *Url, StatusCode, *Reason, bWasClean ? TEXT("true") : TEXT("false"));
		Observer->OnClosed();
	}
