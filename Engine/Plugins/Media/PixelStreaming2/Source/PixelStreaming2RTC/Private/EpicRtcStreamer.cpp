...
namespace UE::PixelStreaming2
{
	...
	void FEpicRtcStreamer::SetInputHandler(TSharedPtr<IPixelStreaming2InputHandler> Handler)
	{
		...
		// Re-create the freeze frame manager with the new input handler
		...
		// We need to update the protocol for each existing participant's data track
		TSharedPtr<IPixelStreaming2InputHandler> SharedInputHandler = InputHandler;
		Participants->Apply([SharedInputHandler](FString PlayerId, TSharedPtr<FPlayerContext> Participant) {
			TSharedPtr<IPixelStreaming2DataProtocol> Protocol = SharedInputHandler->GetFromStreamerProtocol();
			if (Participant->DataTrack && Protocol)
			{
				Participant->DataTrack->SetDataProtocol(Protocol);
			}
		});
	}
	...
	void FEpicRtcStreamer::RefreshStreamBitrate()
	{
		...
		// We need to iterate over just the keys as we can potentially deadlock during player connection with OnDataTrackState having locked the peer connection
		// and SetConnectionRates needing to use the peer connection. Iterating over just the keys is the easiest way to prevent this
	...
	void FEpicRtcStreamer::SendProtocol(FString PlayerId) const
	{
		if (!InputHandler)
		{
			UE_LOGFMT(LogPixelStreaming2RTC, Warning, "Cannot send protocol when input handler is not valid!");
			return;
		}
		...
		for (TSharedPtr<IPixelStreaming2DataProtocol> Protocol : Protocols)
		{
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Warning, "Cannot serialize protocol json object");
				return;
			}
	...
	void FEpicRtcStreamer::HandleRelayStatusMessage(const uint8_t* Data, uint32_t Size, EpicRtcDataTrackInterface* DataTrack)
	{
		...
		if (bIsOn)
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::HandleRelayStatusMessage() Adding new PlayerId [%ls] with DataTrackId [%ls]", *PlayerId, *DataTrackId);
			...
			if (FindPlayerFromTrack(DataTrack, SFUId))
			{
				...
				if (SFUDataTrack)
				{
					...
					if (!InputHandler)
					{
						UE_LOGFMT(LogPixelStreaming2RTC, Warning, "Cannot create SFU data track with an invalid input handler!");
						return;
					}
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::HandleRelayStatusMessage() Failed to find SFU PlayerContext");
			}
		}
		else
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::HandleRelayStatusMessage() Removing PlayerId [%ls] with DataTrackId [%ls]", *PlayerId, *DataTrackId);
		...
		switch (State)
		{
			...
			case EpicRtcSessionState::Connected: // Indicates session is connected to signalling server.
			{
				...
				EpicRtcRoomConfig RoomConfig = {
					...
					._joinParameters = EpicRtcSignallingJoinParameters{
						._options = nullptr,
						._reservedAudioStreamsMaxPoolSize = 0,
						._audioOnly = false
					},
	...
	void FEpicRtcStreamer::OnRoomJoinedUpdate(EpicRtcParticipantInterface* ParticipantInterface)
	{
		...
		UE_LOGF(LogPixelStreaming2RTC, Log, "Player (%ls) joined", *ParticipantId);
		...
		TArray<TRefCountPtr<EpicRtcVideoCodecInfoInterface>> PreferredCodecs;
		for (auto& Codec : UPixelStreaming2PluginSettings::GetCodecPreferences())
		{
			TRefCountPtr<EpicRtcVideoCodecInfoInterface> CodecInfo(new FEpicRtcVideoCodecInfo(ToEpicRtcVideoCodec(Codec)));
			PreferredCodecs.Add(CodecInfo);
		}

		TRefCountPtr<EpicRtcVideoCodecInfoArrayInterface> VideoCodecInfoArray(new FVideoCodecInfoArray(PreferredCodecs));
		Result = ConnectionInterface->SetPreferredVideoCodecs(ToEpicRtcStringView(Utf8ParticipantId), VideoCodecInfoArray);
		if (Result != EpicRtcErrorCode::Ok)
		{
			UE_LOGFMT(LogPixelStreaming2RTC, Error, "FEpicRtcStreamer::OnRoomJoinedUpdate: Failed to set preferred video codecs. SetPreferredVideoCodecs returned {0}", *ToString(Result));
			return;
		}
	...
	void FEpicRtcStreamer::OnRoomLeftUpdate(const EpicRtcStringView Participant)
	{
		...
		UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnRoomLeftUpdate(Participant [%ls] left the room.)", *ParticipantId);
	...
	void FEpicRtcStreamer::OnDataTrackUpdate(EpicRtcParticipantInterface* ParticipantInterface, EpicRtcDataTrackInterface* DataTrack)
	{
		...
		if (TSharedPtr<FPlayerContext> Participant = Participants->FindRef(ParticipantId); Participant.IsValid())
		{
			if (!Participant->DataTrack)
			{
				if (!InputHandler)
				{
					UE_LOGFMT(LogPixelStreaming2RTC, Warning, "Cannot create participant data track when input handler is invalid!");
					return;
				}
	...
	[[nodiscard]] EpicRtcSdpInterface* FEpicRtcStreamer::OnLocalSdpUpdate(EpicRtcParticipantInterface* Participant, EpicRtcSdpInterface* Sdp)
	{
		...
		UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnLocalSdpUpdate(Participant [%ls], Type [%ls])", *ParticipantId, *SdpType);
	...
	[[nodiscard]] EpicRtcSdpInterface* FEpicRtcStreamer::OnRemoteSdpUpdate(EpicRtcParticipantInterface* Participant, EpicRtcSdpInterface* Sdp)
	{
		...
		UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnRemoteSdpUpdate(Participant [%ls], Type [%ls])", *ParticipantId, *SdpType);
	...
	void FEpicRtcStreamer::OnAudioTrackMuted(EpicRtcAudioTrackInterface* AudioTrack, EpicRtcBool bIsMuted)
	{
		...
		UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnAudioTrackMuted(AudioTrack [%ls], bIsMuted[%ls], PlayerId[%ls])", *AudioTrackId, bIsMuted ? TEXT("True") : TEXT("False"), *PlayerId);

		if (!bFoundPlayer)
		{
			UE_LOGF(LogPixelStreaming2RTC, Warning, "FEpicRtcStreamer::OnAudioTrackMuted(Failed to find a player for audio track [%ls])", *AudioTrackId);
			return;
		}
	...
	void FEpicRtcStreamer::OnAudioTrackFrame(EpicRtcAudioTrackInterface* AudioTrack, const EpicRtcAudioFrame& Frame)
	{
		...
		if (!bFoundPlayer)
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnAudioTrackFrame(Failed to find a player for audio track [%ls])", *AudioTrackId);
			return;
		}
	...
	void FEpicRtcStreamer::OnVideoTrackMuted(EpicRtcVideoTrackInterface* VideoTrack, EpicRtcBool bIsMuted)
	{
		...
		UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnVideoTrackMuted(VideoTrack [%ls], bIsMuted[%ls], PlayerId[%ls])", *VideoTrackId, bIsMuted ? TEXT("True") : TEXT("False"), *PlayerId);

		if (!bFoundPlayer)
		{
			UE_LOGF(LogPixelStreaming2RTC, Warning, "FEpicRtcStreamer::OnVideoTrackMuted(Failed to find a player for video track [%ls])", *VideoTrackId);
			return;
		}
	...
	void FEpicRtcStreamer::OnVideoTrackFrame(EpicRtcVideoTrackInterface* VideoTrack, const EpicRtcVideoFrame& Frame)
	{
		...
		if (!bFoundPlayer)
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnVideoTrackFrame(Failed to find a player for video track [%ls])", *VideoTrackId);
			return;
		}
	...
	void FEpicRtcStreamer::OnDataTrackMessage(EpicRtcDataTrackInterface* DataTrack, const EpicRtcDataFrame& DataFrame)
	{
		const uint8_t* Data = DataFrame._data;
		uint32_t	   DataSize = DataFrame._size;
		...
		if (!InputHandler)
		{
			UE_LOGFMT(LogPixelStreaming2RTC, Warning, "Cannot handle data track message when input handler is invalid!");
			return;
		}
		...
		if (Type == ToStreamerProtocol->Find(EPixelStreaming2ToStreamerMessage::Multiplexed)->GetID())
		{
			...
			Type = Data[0];
			UE_LOGF(LogPixelStreaming2RTC, VeryVerbose, "FEpicRtcStreamer::OnDataTrackMessage(Received multiplexed message of type [%d] with PlayerId [%ls])", Type, *PlayerId);
		}
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Log, "FEpicRtcStreamer::OnDataTrackMessage(Failed to find a player for data track [%ls])", *DataTrackId);
			return;
		}
	...
	TSharedPtr<IPixelStreaming2Streamer> FRTCStreamerFactory::CreateNewStreamer(const FString& StreamerId)
	{
		...
		NewStreamer->SetInputHandler(FRTCInputHandler::Create());
