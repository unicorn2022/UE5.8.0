	...
	EpicRtcVideoCodecInfoArrayInterface* FEpicRtcVideoEncoderInitializer::GetSupportedCodecs()
	{
		...
		if ((SelectedCodec == EVideoCodec::H264 || SelectedCodec == EVideoCodec::AV1 || bNegotiateCodecs) && IsRHIDeviceNVIDIA())
		{
			...
			if (!bCanCreateHardwareEncoder)
			{
				...
				UE_LOGF(LogPixelStreaming2RTC, Warning, "No more HW encoders available. Falling back to software encoding");
		...
		if (!bNegotiateCodecs)
		{
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Selected codec was denied - most like due to lack of hw encoder sessions.");
			}
			...
			{
				UE_LOGF(LogPixelStreaming2RTC, Error, "Selected codec was not a supported codec.");
			}
		}
		else
		{
			if (UPixelStreaming2PluginSettings::CVarEncoderEnableSimulcast.GetValueOnAnyThread())
			{
				...
				DenyListedCodecs.Add(EVideoCodec::AV1);
				UE_LOGF(LogPixelStreaming2RTC, Warning, "Removing VP9 and AV1 from negotiable codecs due to simulcast being enabled");
			}
