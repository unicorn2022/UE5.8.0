...
namespace UE::PixelStreaming2
{
	...
	void FFreezeFrame::StartFreeze(UTexture2D* Texture)
	{
		if (Texture)
		{
			...
			([this, Texture](FRHICommandListImmediate& RHICmdList) {
				...
				if (!TextureRHI)
				{
					UE_LOGF(LogPixelStreaming2RTC, Error, "Attempting freeze frame with texture %ls with no texture RHI", *Texture->GetName());
					return;
				}
	...
	void FFreezeFrame::SendFreezeFrame(TArray<FColor> RawData, const FIntRect& Rect)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2RTC, Error, "JPEG image wrapper failed to accept frame data");
		}
	}
