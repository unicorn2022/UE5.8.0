...
namespace UE::PixelStreaming2
{
	...
	void FRTCStatsCollector::Process(const EpicRtcConnectionStats& InStats)
	{
		...
		for (uint64 i = 0; i < InStats._localVideoTracks._size; i++)
		{
			...
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu]"), *RTCStatCategories::VideoSource, i));
			...
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu]"), *RTCStatCategories::VideoCodec, i));
			...
			for (int j = 0; j < LocalVideoTrackStats._rtp._size; j++)
			{
				...
				{
					FName SinkName = FName(*FString::Printf(TEXT("%s [%llu] (%s)"), *RTCStatCategories::LocalVideoTrack, i, *ToString(RtpStats._local._rid)));
		...
		for (uint64 i = 0; i < InStats._localAudioTracks._size; i++)
		{
			...
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu]"), *RTCStatCategories::AudioSource, i));
			...
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu]"), *RTCStatCategories::AudioCodec, i));
			...
			if (!SsrcSinks.Contains(RtpStats._local._ssrc))
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu] (%s)"), *RTCStatCategories::LocalAudioTrack, i, *ToString(RtpStats._local._rid)));
		...
		for (uint64 i = 0; i < InStats._remoteVideoTracks._size; i++)
		{
			...
			if (!SsrcSinks.Contains(RtpStats._local._ssrc))
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu] (%u)"), *RTCStatCategories::RemoteVideoTrack, i, RtpStats._local._ssrc));
		...
		for (uint64 i = 0; i < InStats._remoteAudioTracks._size; i++)
		{
			...
			if (!SsrcSinks.Contains(RtpStats._local._ssrc))
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu] (%u)"), *RTCStatCategories::RemoteAudioTrack, i, RtpStats._local._ssrc));
				SsrcSinks.Add(RtpStats._local._ssrc, MakeUnique<FRTPRemoteTrackStatsSink>(SinkName));
			}
		...
		for (uint64 i = 0; i < InStats._dataTracks._size; i++)
		{
			...
			{
				FName SinkName = FName(*FString::Printf(TEXT("%s [%llu]"), *RTCStatCategories::DataChannel, i));
		...
		: FStatsSink(InCategory)
	{
		...
		Stats.Add(PixelStreaming2StatNames::Jitter, FStat({ .Name = PixelStreaming2StatNames::Jitter }, 0.f));
		Stats.Add(PixelStreaming2StatNames::RoundTripTime, FStat({ .Name = PixelStreaming2StatNames::RoundTripTime }, 0.f));
		Stats.Add(PixelStreaming2StatNames::DecoderImplementation, FStat({ .Name = PixelStreaming2StatNames::DecoderImplementation }, FString(TEXT(""))));
	...
	void FRTCStatsCollector::FRTPRemoteTrackStatsSink::Process(const EpicRtcRemoteTrackRtpStats& InStats, const FString& PeerId, double SecondsDelta)
	{
		...
		for (TPair<FName, FStat>& Tuple : Stats)
		{
			...
			else if (Tuple.Key == PixelStreaming2StatNames::DecoderImplementation)
			{
				NewValue = FStatVariant(TInPlaceType<FString>(), ToString(InStats._local._decoderImplementation));
			}
			else if (Tuple.Key == PixelStreaming2StatNames::RoundTripTime)
			{
				NewValue = FStatVariant(TInPlaceType<double>(), InStats._remote._roundTripTime);
