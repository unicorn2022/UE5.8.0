...
{
	SoundGenerator->Initialize(InParams);
...
void UPixelStreaming2Peer::ConsumeRawPCM(const int16_t* AudioData, int SampleRate, size_t InNumChannels, size_t NumFrames)
{
	// Sound generator has not been initialized yet.
	if (!SoundGenerator || !SoundGenerator->IsInitialized())
	{
		return;
	}

	SoundGenerator->AddAudio(AudioData, SampleRate, InNumChannels, NumFrames);
}
...
void UPixelStreaming2Peer::OnAudioTrackUpdate(EpicRtcParticipantInterface* Participant, EpicRtcAudioTrackInterface* AudioTrack)
{
	...
	UE_LOGF(LogPixelStreaming2RTC, VeryVerbose, "UPixelStreaming2Peer::OnAudioTrackUpdate(Participant [%ls], VideoTrack [%ls])", *ParticipantId, *VideoTrackId);
...
void UPixelStreaming2Peer::OnVideoTrackUpdate(EpicRtcParticipantInterface* Participant, EpicRtcVideoTrackInterface* VideoTrack)
{
	...
	UE_LOGF(LogPixelStreaming2RTC, VeryVerbose, "UPixelStreaming2Peer::OnVideoTrackUpdate(Participant [%ls], VideoTrack [%ls])", *ParticipantId, *VideoTrackId);
...
void UPixelStreaming2Peer::OnDataTrackMessage(EpicRtcDataTrackInterface* DataTrack, const EpicRtcDataFrame& DataFrame)
{
}
