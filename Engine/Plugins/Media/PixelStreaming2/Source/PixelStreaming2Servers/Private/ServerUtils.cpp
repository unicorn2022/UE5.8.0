...
namespace UE::PixelStreaming2Servers::Utils
{
	...
	TSharedPtr<FMonitoredProcess> LaunchChildProcess(FString ExecutableAbsPath, FString Args, FString LogPrefix, bool bRunAsScript)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2Servers, Error, "Cannot start child process - the specified file did not exist. File=%ls", *ExecutableAbsPath);
...
// Get the executable we will use to run the scripts (e.g. cmd.exe on Windows)
...
#else
			UE_LOGF(LogPixelStreaming2Servers, Error, "Unsupported platform for Pixel Streaming.");
			return TSharedPtr<FMonitoredProcess>();
#endif
		}
		...
		ChildProcess->OnOutput().BindLambda([LogPrefix](FString Output) {
			UE_LOGF(LogPixelStreaming2Servers, Log, "%ls - %ls", *LogPrefix, *Output);
		});
		// Run the child process
		UE_LOGF(LogPixelStreaming2Servers, Log, "Launch child process - %ls %ls", *ExecutableAbsPath, *Args);
		ChildProcess->Launch();
		return ChildProcess;
	}
...
#else
		UE_LOGF(LogPixelStreaming2Servers, Error, "Unsupported platform for Pixel Streaming scripts.");
#endif
		return false;
	}
	...
	TSharedPtr<FMonitoredProcess> DownloadPixelStreaming2Servers(bool bSkipIfPresent)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2Servers, Log, "Found pixel streaming servers, skipping download.");
			// empty process
			return TSharedPtr<FMonitoredProcess>();
		}
		...
		{
			UE_LOGF(LogPixelStreaming2Servers, Error, "Could not download ps servers, no PixelStreaming2/Resources/WebServers directory found.");
		}
	...
	FString ToString(TSharedPtr<FJsonObject> JSONObj)
	{
		...
		if (!bSerialized)
		{
			UE_LOGF(LogPixelStreaming2Servers, Error, "Failed to stringify JSON object.");
		}
		return Res;
	}
