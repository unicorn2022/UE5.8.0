...
namespace UE::PixelStreaming2Servers
{
	...
	bool FCheckNumStreamers::Update()
	{
		if (Server && Server->IsReady())
		{
			...
			if (bTestPassed)
			{
				UE_LOGF(LogPixelStreaming2Servers, Log, "Success: %ls", *LogString);
				return true;
			}
			else if (GetCurrentRunTime() > 2.0f)
			{
				UE_LOGF(LogPixelStreaming2Servers, Error, "Failed (timed out after 2s): %ls", *LogString);
				return true;
			}
		}
		return false;
	...
	bool FStartWebsocketClient::Update()
	{
		...
		{
			UE_LOGF(LogPixelStreaming2Servers, Error, "Timed out after 2s of waiting for websocket to connect");
			return true;
		}
		...
		if (Probe->Probe())
		{
			UE_LOGF(LogPixelStreaming2Servers, Log, "Websocket client probe connected.");
			return true;
		}
		else
		{
	...
	bool FCloseWebsocketClient::Update()
	{
		if (GetCurrentRunTime() > 2.0f)
		{
			UE_LOGF(LogPixelStreaming2Servers, Error, "Timed out after 2s of waiting for websocket to close");
			return true;
		}
		...
		{
			Probe->Close();
			UE_LOGF(LogPixelStreaming2Servers, Log, "Asked websocket client probe to close.");
			return false;
		}
		else
		{
			UE_LOGF(LogPixelStreaming2Servers, Log, "Websocket client probe closed.");
			return true;
		}
	}
	...
	{
		UE_LOGF(LogPixelStreaming2Servers, Log, "----------- LaunchEmbeddedCirrusTest -----------");
		...
		UE_LOGF(LogPixelStreaming2Servers, Log, "Embedded cirrus launched: %ls", bLaunched ? TEXT("true") : TEXT("false"));
		...
		SignallingServer->OnReady.AddLambda([this](TMap<EEndpoint, FURL> Endpoints) {
			...
			UE_LOGF(LogPixelStreaming2Servers, Log, "Http address for webserver. Actual=%ls | Expected=%ls", *ActualWebserverUrl, *ExpectedWebserverAddress);
			...
			UE_LOGF(LogPixelStreaming2Servers, Log, "Websocket address for streamer messages. Actual=%ls | Expected=%ls", *ActualStreamerUrl, *ExpectedStreamerAddress);
			...
			UE_LOGF(LogPixelStreaming2Servers, Log, "Websocket address for player messages. Actual=%ls | Expected=%ls", *ActualPlayersUrl, *ExpectedPlayerWSAddress);
	...
	{
		UE_LOGF(LogPixelStreaming2Servers, Log, "----------- TwoWebsocketToEmbeddedCirrus -----------");
