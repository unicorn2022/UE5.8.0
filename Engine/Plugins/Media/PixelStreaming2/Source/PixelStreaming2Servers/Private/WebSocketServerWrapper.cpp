...
namespace UE::PixelStreaming2Servers
{
	...
	void FWebSocketServerWrapper::SetFilterConnectionCallback(FWebSocketFilterConnectionCallback InFilterConnectionCallback)
	{
		FilterConnectionCallback = MoveTemp(InFilterConnectionCallback);
		if (WSServer)
		{
			WSServer->SetFilterConnectionCallback(FilterConnectionCallback);
		}
	}
	...
	bool FWebSocketServerWrapper::Launch(uint16 Port)
	{
		...
		if (FilterConnectionCallback.IsBound())
		{
			WSServer->SetFilterConnectionCallback(FilterConnectionCallback);
		}

		WSServer->SetNetworkProtocol(NetworkProtocol);
		...
		if (!bLaunched)
		{
			UE_LOGF(LogPixelStreaming2Servers, Error, "Failed to launch Websocket server on port %d", Port);
			WSServer.Reset();
			OnClientConnectedCallback.Unbind();
		}
		else
		{
			const bool bIPv4 = EnumHasAnyFlags(NetworkProtocol, IWebSocketServer::ENetworkProtocol::IPv4);
			const bool bIPv6 = EnumHasAnyFlags(NetworkProtocol, IWebSocketServer::ENetworkProtocol::IPv6);
			const TCHAR* ProtocolStr = (bIPv4 && bIPv6) ? TEXT("IPv4+IPv6") : bIPv6 ? TEXT("IPv6") : TEXT("IPv4");
			UE_LOGF(LogPixelStreaming2Servers, Log, "Started Websocket server on port %d (%ls)", Port, ProtocolStr);
		}
		return bLaunched;
	}
	...
	bool FWebSocketServerWrapper::Close(uint16 ConnectionId)
	{
		...
		{
			UE_LOGF(LogPixelStreaming2Servers, Warning, "Could not close websocket connection because there was no connection=%d.", ConnectionId);
			return false;
		}
	}
	...
	bool FWebSocketServerWrapper::Send(uint16 ConnectionId, FString Message) const
	{
		...
		{
			UE_LOGF(LogPixelStreaming2Servers, Warning, "Did not send websocket message because there was no connection=%d.", ConnectionId);
			return false;
		}
	}
	...
	void FWebSocketServerWrapper::OnConnectionOpened(INetworkingWebSocket* Socket)
	{
		if (!Socket)
		{
			UE_LOGF(LogPixelStreaming2Servers, Error, "Websocket client connected with a null socket.");
			return;
		}
		else
		{
			UE_LOGF(LogPixelStreaming2Servers, Log, "Websocket client connected. Remote=%ls | Local=%ls", *Socket->RemoteEndPoint(true), *Socket->LocalEndPoint(true));
		}
