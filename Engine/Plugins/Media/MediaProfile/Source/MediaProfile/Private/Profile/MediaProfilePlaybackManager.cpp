...
namespace MediaProfilePlaybackManager
{
	...
	/** Common interface for all types of media capture viewport clients */
	class IMediaCaptureViewportClient : public TSharedFromThis<IMediaCaptureViewportClient>
	{
	public:
		virtual ~IMediaCaptureViewportClient() = default;
	
		virtual void SetPIE(bool bInIsPIE) = 0;
		virtual TSharedRef<FViewportClient> AsViewportClient() = 0;
	};
	
#if WITH_EDITOR
	class FMediaCaptureEditorViewportClient : public FLevelEditorViewportClient, public IMediaCaptureViewportClient
	{
	private:
	...
	public:
		...
		virtual void SetPIE(bool bInIsPIE) override
		{
			FWorldContext* PIEWorldContext = GEditor->GetPIEWorldContext();
			if (bInIsPIE)
			{
		...
		virtual TSharedRef<FViewportClient> AsViewportClient() override
		{
	...
	class FMediaCaptureGameViewportClient : public FCommonViewportClient, public IMediaCaptureViewportClient
	{
		virtual void SetPIE(bool bInIsPIE) override { }
		virtual TSharedRef<FViewportClient> AsViewportClient() override { return SharedThis(this); }
	};
	...
	public:
		...
		TSharedPtr<IMediaCaptureViewportClient> ViewportClient;
	...
	TSharedPtr<FManagedViewport> CreateManagedViewport(UMediaOutput* InMediaOutput, EViewModeIndex InViewMode)
	{
		...
		if (MediaProfileModule.CanCaptureWithLevelEditorViewportClients())
		{
			...
			ManagedViewport->Viewport = FSceneViewport::Create(ViewportClient, ManagedViewport->ViewportWidget);
...
#endif
		{
			...
			ManagedViewport->ViewportClient = ViewportClient;
			ManagedViewport->Viewport = FSceneViewport::Create(ViewportClient, nullptr);
		}
	...
	public:
			...
			: MediaOutput(InMediaOutput)
		{
			...
			if (!Viewport.IsValid())
			{
				if (TSharedPtr<IAssetViewport> ActiveViewportInterface = FindActiveViewportInterface())
				{
					...
					CachedViewportFlags.bDrawAlphaBlendedCheckerboard = ViewportClient.ChannelMaskParams.bDrawAlphaBlendedCheckerboard;
					...
					LevelEditorModule.OnGetVisualizeCheckerboardEnabled().BindLambda([] { return false; });
					...
					if (InCaptureOptions.ResizeMethod == EMediaCaptureResizeMethod::ResizeSource)
					{
						if (TSharedPtr<FSceneViewport> LevelViewport = ActiveViewportInterface->GetSharedActiveViewport())
						{
							...
							LevelViewport->InvalidateHitProxy();
						}
					}
				}
			}
		...
		~FActiveViewport()
		{
			...
			if (TSharedPtr<IAssetViewport> PinnedViewportInterface = ViewportInterface.Pin())
			{
				...
				ViewportClient.ChannelMaskParams.bDrawAlphaBlendedCheckerboard = CachedViewportFlags.bDrawAlphaBlendedCheckerboard;
				ViewportClient.VisibilityDelegate = CachedViewportFlags.VisibilityDelegate;

				FLevelEditorModule& LevelEditorModule = FModuleManager::GetModuleChecked<FLevelEditorModule>(TEXT("LevelEditor"));
				LevelEditorModule.OnGetVisualizeCheckerboardEnabled().Unbind();
	...
	/** Keeps track of what media textures are being captured by media outputs in the media profile to help determine when the captures should be automatically restarted on changes */
	class FMediaTextureCapture
	{
	public:
		FMediaTextureCapture(UMediaOutput* InMediaOutput)
			: MediaOutput(InMediaOutput)
		{ }

		/** Updates the media texture and capture options of the media texture capture */
		void UpdateMediaTexture(UMediaTexture* InMediaTexture, const FMediaCaptureOptions& InCaptureOptions, const FMediaCaptureTransform& InCaptureTransform, bool bInAutoRestartCapture)
		{
			MediaTexture = InMediaTexture;
			CaptureOptions = InCaptureOptions;
			CaptureTransform = InCaptureTransform;
			bAutoRestartCapture = bInAutoRestartCapture;
			
			if (MediaTexture.IsValid())
			{
				LastMediaTextureSize = FIntPoint(MediaTexture->GetWidth(), MediaTexture->GetHeight());
			}
		}
		
		/** Checks whether the media texture has changed size since the last check */
		bool NeedsCaptureRestart()
		{
			if (!MediaOutput.IsValid() || !MediaTexture.IsValid())
			{
				return false;
			}
			
			const FIntPoint MediaTextureSize = FIntPoint(MediaTexture->GetWidth(), MediaTexture->GetHeight());
			const bool bTextureSizeChanged = MediaTextureSize != LastMediaTextureSize;
			LastMediaTextureSize = MediaTextureSize;
			return bTextureSizeChanged && bAutoRestartCapture;
		}
		
		/** The media output that is capturing the media texture */
		TWeakObjectPtr<UMediaOutput> MediaOutput = nullptr;
		
		/** The media texture being captured */
		TWeakObjectPtr<UMediaTexture> MediaTexture = nullptr;
		
		/** The options the media texture capture was initialized with */
		FMediaCaptureOptions CaptureOptions;
		
		/** The transform the media texture capture was initialized with */
		FMediaCaptureTransform CaptureTransform;
		
		/** Whether the media texture capture can auto-restart when a change is detected */
		bool bAutoRestartCapture = false;
		
		/** Keeps track of the last media texture size to recognize changes in media texture resolution that indicate a capture restart is needed */
		FIntPoint LastMediaTextureSize = FIntPoint::ZeroValue;
	};
}

bool MediaProfilePlaybackManager::IsActiveViewportCaptureAllowed()
{
	static TOptional<bool> bAllowActiveViewportCapture;
	if (!bAllowActiveViewportCapture.IsSet())
	{
		// If the config param does not exist in the ini, default to true
		bool bConfigValue = true;
		GConfig->GetBool(TEXT("MediaProfile"), TEXT("bAllowActiveViewportCapture"), bConfigValue, GEngineIni);
		bAllowActiveViewportCapture = bConfigValue;
	}
	return bAllowActiveViewportCapture.Get(true);
}

bool MediaProfilePlaybackManager::IsManagedViewportCaptureAllowed()
{
	static TOptional<bool> bAllowManagedViewportCapture;
	if (!bAllowManagedViewportCapture.IsSet())
	{
		// If the config param does not exist in the ini, default to true
		bool bConfigValue = true;
		GConfig->GetBool(TEXT("MediaProfile"), TEXT("bAllowManagedViewportCapture"), bConfigValue, GEngineIni);
		bAllowManagedViewportCapture = bConfigValue;
	}
	return bAllowManagedViewportCapture.Get(true);
}
...
void UMediaProfilePlaybackManager::Tick(float DeltaTime)
{
	using namespace MediaProfilePlaybackManager;
	
	TArray<TSharedPtr<FMediaTextureCapture>> ChangedMediaTextures;
	for (TSharedPtr<FMediaTextureCapture>& MediaTextureCapture : MediaTextureCaptures)
	{
		if (!MediaTextureCapture.IsValid())
		{
			continue;
		}
		
		if (MediaTextureCapture->NeedsCaptureRestart())
		{
			ChangedMediaTextures.Add(MediaTextureCapture);
		}
	}
	
	for (TSharedPtr<FMediaTextureCapture>& ChangedMediaTexture : ChangedMediaTextures)
	{
		RestartMediaTextureOutput(
			ChangedMediaTexture->MediaOutput.Get(),
			ChangedMediaTexture->MediaTexture.Get(), 
			ChangedMediaTexture->CaptureOptions,
			ChangedMediaTexture->CaptureTransform,
			ChangedMediaTexture->bAutoRestartCapture);
	}
}

ETickableTickType UMediaProfilePlaybackManager::GetTickableTickType() const
{
	return HasAnyFlags(RF_ClassDefaultObject) ? ETickableTickType::Never : ETickableTickType::Conditional;
}

bool UMediaProfilePlaybackManager::IsTickable() const
{
	// Keep track of whether the media textures being captured have changed size, requiring a capture restart
	// Check every tick if their size has changed, and if so, call Restart on the capture
	return !MediaTextureCaptures.IsEmpty();
}

UWorld* UMediaProfilePlaybackManager::GetTickableGameObjectWorld() const
{
	return GetWorld();
}

TStatId UMediaProfilePlaybackManager::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(UMediaProfilePlaybackManager, STATGROUP_Tickables);
}
...
bool UMediaProfilePlaybackManager::IsSourceOpenFromIndex(int32 InMediaSourceIndex, void* InConsumer) const
{
	...
	if (!MediaPlayer->GetPlayerFacade()->GetPlayer().IsValid())
	{
		return false;
	}
...
TSharedPtr<FViewportClient> UMediaProfilePlaybackManager::GetOrCreateManagedViewportFromIndex(int32 InMediaOutputIndex, EViewModeIndex InViewMode, void* Consumer)
{
	...
	if (!IsManagedViewportCaptureAllowed())
	{
		return nullptr;
	}
	...
	if (ExistingViewportIndex != INDEX_NONE)
	{
		...
		return ManagedOutputViewports[ExistingViewportIndex]->ViewportClient->AsViewportClient();
	}
	...
	return NewManagedViewport->ViewportClient->AsViewportClient();
}
...
UMediaCapture* UMediaProfilePlaybackManager::OpenManagedViewportOutputFromIndex(int32 InMediaOutputIndex, const FMediaCaptureOptions& InCaptureOptions, bool bAutoRestartCapture)
{
	...
	// If the capture failed to start, mark the capture as errored, clean up its capture resources, and return it
	CapturesWithError.Add(MediaCapture);
	CleanUpCaptureResources(MediaOutput);
...
TSharedPtr<FSceneViewport> UMediaProfilePlaybackManager::GetActiveViewportFromIndex(int32 InMediaOutputIndex) const
{
	...
	if (!IsActiveViewportCaptureAllowed())
	{
		return nullptr;
	}
...
UMediaCapture* UMediaProfilePlaybackManager::OpenMediaTextureOutputFromIndex(int32 InMediaOutputIndex, UMediaTexture* InMediaTexture, const FMediaCaptureOptions& InCaptureOptions, const FMediaCaptureTransform& InTransform, bool bAutoRestartCapture)
{
	UMediaProfile* MediaProfile = GetOwningMediaProfile();
	if (!MediaProfile)
	{
	...
	if (MediaCapture->CaptureMediaTexture(InMediaTexture, InCaptureOptions, InTransform.GetRotationValue(true), InTransform.GetScaleValue()))
	{
		int32 MediaTextureCaptureIndex = MediaTextureCaptures.IndexOfByPredicate([MediaOutput](const TSharedPtr<MediaProfilePlaybackManager::FMediaTextureCapture>& InOutputMediaTexture)
		{
			return InOutputMediaTexture->MediaOutput == MediaOutput;
		});

		if (MediaTextureCaptureIndex == INDEX_NONE)
		{
			MediaTextureCaptureIndex = MediaTextureCaptures.Add(MakeShared<MediaProfilePlaybackManager::FMediaTextureCapture>(MediaOutput));
		}

		MediaTextureCaptures[MediaTextureCaptureIndex]->UpdateMediaTexture(InMediaTexture, InCaptureOptions, InTransform, bAutoRestartCapture);

#if WITH_EDITOR
		if (bAutoRestartCapture)
		{
			MediaOutput->OnOutputModified().AddUObject(this, &UMediaProfilePlaybackManager::RestartMediaTextureOutput, InMediaTexture, InCaptureOptions, InTransform, bAutoRestartCapture);
		}
#endif
...
UMediaCapture* UMediaProfilePlaybackManager::OpenMediaTextureOutput(UMediaOutput* InMediaOutput, UMediaTexture* InMediaTexture, const FMediaCaptureOptions& InCaptureOptions, const FMediaCaptureTransform& InTransform, bool bAutoRestartCapture)
{
	UMediaProfile* MediaProfile = GetOwningMediaProfile();
	if (!MediaProfile)
	{
	...
	int32 MediaOutputIndex = MediaProfile->FindMediaOutputIndex(InMediaOutput);
	return OpenMediaTextureOutputFromIndex(MediaOutputIndex, InMediaTexture, InCaptureOptions, InTransform, bAutoRestartCapture);
}
...
void UMediaProfilePlaybackManager::RestartMediaTextureOutput(UMediaOutput* InMediaOutput, UMediaTexture* InMediaTexture, FMediaCaptureOptions InCaptureOptions, FMediaCaptureTransform InTransform, bool bAutoRestartCapture)
{
	TWeakObjectPtr<UMediaTexture> WeakMediaTexture = InMediaTexture;
	TWeakObjectPtr<UMediaOutput> WeakMediaOutput = InMediaOutput;
	auto Callback = [WeakMediaTexture, WeakMediaOutput, InCaptureOptions, InTransform, bAutoRestartCapture](UMediaProfilePlaybackManager* Manager)
	{
		TStrongObjectPtr<UMediaTexture> MediaTexture = WeakMediaTexture.Pin();
		TStrongObjectPtr<UMediaOutput> MediaOutput = WeakMediaOutput.Pin();

		if (Manager && MediaTexture && MediaOutput)
		{
			Manager->OpenMediaTextureOutput(MediaOutput.Get(), MediaTexture.Get(), InCaptureOptions, InTransform, bAutoRestartCapture);
		}
	};
	QueueRestartCapture(InMediaOutput, Callback);
}
...
void UMediaProfilePlaybackManager::OnCaptureStateChanged(UMediaCapture* InMediaCapture)
{
	...
	else if (InMediaCapture->GetState() == EMediaCaptureState::Stopped)
	{
		// If the capture has been stopped due to an error, try and clean up any resources the media capture was using
		if (CapturesWithError.Contains(InMediaCapture))
		{
			if (const int32* MediaOutputIndex = MediaOutputCaptures.FindKey(InMediaCapture))
			{
				UMediaProfile* MediaProfile = GetOwningMediaProfile();
				if (!MediaProfile)
				{
				...
				UMediaOutput* MediaOutput = MediaProfile->GetMediaOutput(*MediaOutputIndex);
				if (!IsValid(MediaOutput))
				{
					return;
				}
...
void UMediaProfilePlaybackManager::CleanUpCaptureResources(UMediaOutput* InMediaOutput)
{
	ManagedOutputViewports.RemoveAll([InMediaOutput](const TSharedPtr<MediaProfilePlaybackManager::FManagedViewport>& InManagedViewport)
		{
			return InManagedViewport->MediaOutput == InMediaOutput && InManagedViewport->Consumers.IsEmpty();
		});

	ActiveOutputViewports.RemoveAll([InMediaOutput](const TSharedPtr<MediaProfilePlaybackManager::FActiveViewport>& InViewport)
	{
		return InViewport->MediaOutput == InMediaOutput;
	});

	MediaTextureCaptures.RemoveAll([InMediaOutput](const TSharedPtr<MediaProfilePlaybackManager::FMediaTextureCapture>& InMediaTextureCapture)
	{
		return !InMediaTextureCapture.IsValid() || InMediaTextureCapture->MediaOutput.Get() == InMediaOutput;
	});
...
void UMediaProfilePlaybackManager::OnPostPIEStarted(bool bIsSimulating)
{
	...
	{
		if (ManagedViewport->ViewportClient.IsValid())
		{
			ManagedViewport->ViewportClient->SetPIE(true);
		}
	}
}
...
void UMediaProfilePlaybackManager::OnPrePIEEnded(bool bIsSimulating)
{
	...
	for (const TSharedPtr<FManagedViewport>& ManagedViewport : ManagedOutputViewports)
	{
		if (ManagedViewport->ViewportClient.IsValid())
		{
			ManagedViewport->ViewportClient->SetPIE(false);
		}
	}
}
