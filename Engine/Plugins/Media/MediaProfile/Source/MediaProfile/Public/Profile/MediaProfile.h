...
class MEDIAPROFILE_API UMediaProfile : public UObject
{
	GENERATED_BODY()
...
#if WITH_EDITOR
	/**
	 * Gets the index of a media source whose label matches the specified label if it was found in the media profile's list of sources
	 * @param InLabel The label of the media source to find
	 * @return The index of the source in the profile's sources list, or INDEX_NONE if it was not found
	 */
	int32 FindMediaSourceIndexByLabel(const FString& InLabel) const;
#endif
...
#if WITH_EDITOR
	/**
	 * Gets the index of a media output with the specified label if it was found in the media profile's list of outputs
	 * @param InLabel The label of the media output to find
	 * @return The index of the output in the profile's output list, or INDEX_NONE if it was not found
	 */
	int32 FindMediaOutputIndexByLabel(const FString& InLabel) const;
#endif
...
public:
	...
	UMediaProfilePlaybackManager* GetPlaybackManager() const;
...
private:
	...
	/** Allows the LLH handler to directly read/write protected fields for session serialization. */
	friend class FLiveLinkHubMediaProfileHandler;
};
