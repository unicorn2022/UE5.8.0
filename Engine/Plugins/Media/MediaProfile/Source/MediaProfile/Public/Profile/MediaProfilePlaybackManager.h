...
namespace MediaProfilePlaybackManager
{
	...
	/**
	 * Queries a config property (MediaProfile.bAllowActiveViewportCapture, in DefaultEngine.ini) to determine if active viewport capture is allowed.
	 * If disabled, attempting to start active viewport captures in the playback manager will fail.
	 */
	MEDIAPROFILE_API bool IsActiveViewportCaptureAllowed();
	
	/**
	 * Queries a config property (MediaProfile.bAllowManagedViewportCapture, in DefaultEngine.ini) to determine if managed viewport capture is allowed.
	 * If disabled, attempting to start managed viewport captures in the playback manager will fail.
	 */
	MEDIAPROFILE_API bool IsManagedViewportCaptureAllowed();
}

/** Manages playback of media items within a media profile */
UCLASS()
class MEDIAPROFILE_API UMediaProfilePlaybackManager : public UObject, public FTickableGameObject
{
	GENERATED_BODY()
...
public:
	...
	/**
	 * Starts a capture of a media texture for a specified media output
	 * @param InMediaOutputIndex The index of the media output in the media profile
	 * @param InMediaTexture The media texture to capture
	 * @param InCaptureOptions The capture options to capture with
	 * @param InTransform The transform to apply to the media texture on output
	 * @param bAutoRestartCapture Indicates that this capture will automatically restart if any modifications to the media output are detected
	 * @return The media capture object, if the capture was started successfully; otherwise, null
	 */
	UMediaCapture* OpenMediaTextureOutputFromIndex(int32 InMediaOutputIndex, UMediaTexture* InMediaTexture, const FMediaCaptureOptions& InCaptureOptions, const FMediaCaptureTransform& InTransform, bool bAutoRestartCapture = false);
	 ...
	 * @return The media capture object, if the capture was started successfully; otherwise, null
	 */
	UMediaCapture* OpenMediaTextureOutput(UMediaOutput* InMediaOutput, UMediaTexture* InMediaTexture, const FMediaCaptureOptions& InCaptureOptions, const FMediaCaptureTransform& InTransform, bool bAutoRestartCapture = false);
	...
	/**
	 * Restarts a capture of a media texture for a specified media output
	 * @param InMediaOutput The media output in the media profile
	 * @param InMediaTexture The media texture to capture
	 * @param InCaptureOptions The capture options to capture with
	 * @param InTransform The transform to apply to the media texture on output
	 * @param bAutoRestartCapture Indicates that this capture will automatically restart if any modifications to the media output are detected
	 */
	void RestartMediaTextureOutput(UMediaOutput* InMediaOutput, UMediaTexture* InMediaTexture, FMediaCaptureOptions InCaptureOptions, FMediaCaptureTransform InTransform, bool bAutoRestartCapture = false);
...
private:
	...
	/** Cleans up any managed resources used by the specified media output, such as active viewports or managed viewports */
	void CleanUpCaptureResources(UMediaOutput* InMediaOutput);
...
private:
	...
	/** A list of media textures that are being captured from */
	TArray<TSharedPtr<MediaProfilePlaybackManager::FMediaTextureCapture>> MediaTextureCaptures;
