...
void SMediaProfileViewport::SetSelectedMediaItems(const TArray<int32>& InSelectedMediaSourceIndices, const TArray<int32>& InSelectedMediaOutputIndices)
{
	...
	TArray<int32> MediaSourceIndices = InSelectedMediaSourceIndices;
	TArray<int32> MediaOutputIndices = InSelectedMediaOutputIndices;
	
	// If a selected media source or media output is already being displayed in the panel contents as a locked panel,
	// don't add a new panel for it, only configure sources and outputs that aren't already being displayed
	for (const FPanelContent& Panel : PanelContents)
	{
		if (Panel.bIsLocked)
		{
			if (Panel.MediaType->IsChildOf(UMediaSource::StaticClass()))
			{
				MediaSourceIndices.Remove(Panel.MediaItemIndex);
			}
			else if (Panel.MediaType->IsChildOf(UMediaOutput::StaticClass()))
			{
				MediaOutputIndices.Remove(Panel.MediaItemIndex);
			}
		}
	}

	constexpr int32 MaxNumPanels = 4;
	const int NumMediaItems = MediaSourceIndices.Num() + MediaOutputIndices.Num();
	int32 PanelIndex = 0;
	...
	for (int32 Index = 0; Index < NumMediaItems; ++Index)
	{
		...
		const int32 MediaItemIndex = Index < MediaSourceIndices.Num() ? MediaSourceIndices[Index] : MediaOutputIndices[Index - MediaSourceIndices.Num()];
		
		UClass* MediaType;
		if (Index < MediaSourceIndices.Num())
		{
...
void SMediaProfileViewport::SetImmersivePanel()
{
	...
	// If there are no panel contents, don't allow immersive since there is no viewport toolbar that allows users to exit immersive
	if (PanelContents.IsEmpty())
	{
		return;
	}

	bImmersive = true;
	ParentWindow->SetFullWindowOverlayContent(LayoutContainer);

	// Reset keyboard focus back the panel widget after it has been repositioned in the panel layout
	// so that keyboard shortcuts still work for that panel without the user having to click on it again
	FSlateApplication::Get().ClearKeyboardFocus();
	FSlateApplication::Get().SetKeyboardFocus(PanelContents[0].Widget);
}

bool SMediaProfileViewport::IsPanelImmersive() const
{
	return bImmersive;
}
...
void SMediaProfileViewport::SaveLayout(FMediaProfileEditorLayoutSettings& InOutLayoutSettings)
{
	UMediaProfile* MediaProfile = nullptr;
	if (MediaProfileEditor.IsValid())
	{
	...
	InOutLayoutSettings.ViewportLayout = (uint8)CurrentLayout;
	InOutLayoutSettings.ViewportOrientation = (uint8)CurrentOrientation;
	
	InOutLayoutSettings.Panels.Empty();
	for (const FPanelContent& Panel : PanelContents)
	{
		FMediaProfileEditorLayoutPanelSettings PanelSettings;
		if (Panel.MediaType->IsChildOf(UMediaSource::StaticClass()))
		{
			PanelSettings.MediaItemName = MediaProfile->GetLabelForMediaSource(Panel.MediaItemIndex);
			PanelSettings.bMediaSource = true;
		}
		else
		{
			PanelSettings.MediaItemName = MediaProfile->GetLabelForMediaOutput(Panel.MediaItemIndex);
			PanelSettings.bMediaSource = false;
		}
		
		InOutLayoutSettings.Panels.Add(PanelSettings);
	}
}

void SMediaProfileViewport::LoadLayout(const FMediaProfileEditorLayoutSettings& InLayoutSettings)
{
	UMediaProfile* MediaProfile = nullptr;
	if (MediaProfileEditor.IsValid())
	{
	...
	CurrentLayout = (EViewportLayout)InLayoutSettings.ViewportLayout;
	CurrentOrientation = (EViewportOrientation)InLayoutSettings.ViewportOrientation;
	
	PanelContents.Empty();
	for (const FMediaProfileEditorLayoutPanelSettings& PanelSettings : InLayoutSettings.Panels)
	{
		FPanelContent Panel;
		
		if (PanelSettings.bMediaSource)
		{
			Panel.MediaItemIndex = MediaProfile->FindMediaSourceIndexByLabel(PanelSettings.MediaItemName);

			if (UMediaSource* MediaSource = MediaProfile->GetMediaSource(Panel.MediaItemIndex))
			{
				Panel.MediaType = MediaSource->GetClass();
			}
			else
			{
				Panel.MediaType = UMediaSource::StaticClass();
			}
		}
		else
		{
			Panel.MediaItemIndex = MediaProfile->FindMediaOutputIndexByLabel(PanelSettings.MediaItemName);

			if (UMediaOutput* MediaOutput = MediaProfile->GetMediaOutput(Panel.MediaItemIndex))
			{
				Panel.MediaType = MediaOutput->GetClass();
			}
			else
			{
				Panel.MediaType = UMediaOutput::StaticClass();
			}
		}
...
void SMediaProfileViewport::ClearLayout()
{
	CurrentLayout = EViewportLayout::OnePanel;
	CurrentOrientation = EViewportOrientation::Left;
