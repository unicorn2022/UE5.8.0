...
void SMediaFrameworkVideoInputDisplay::OnMediaClosed()
{
	...
	if (MediaPlayer && MediaTexture)
	{
		...
		if (MediaState == EMediaState::Closed || MediaState == EMediaState::Error)
		{
			...
			{
				UE_LOGF(LogMediaFrameworkUtilitiesEditor, Warning, "The MediaTexture '%ls' doesn't reference the MediaPlayer '%ls' anymore.", *MediaTexture->GetName(), *MediaPlayer->GetName());
			}
...
void SMediaFrameworkVideoInputMediaSourceDisplay::Construct(const FArguments& InArgs)
{
	...
	if (MediaSource && MediaTexture)
	{
		...
		if (MediaPlayer)
		{
			...
			{
				UE_LOGF(LogMediaFrameworkUtilitiesEditor, Warning, "The MediaPlayer '%ls' will be closed when a PIE session starts or stop. You should uncheck 'Affected By PIE Handling' on the MediaPlayer.", *MediaPlayer->GetName());
			}
		...
		{
			UE_LOGF(LogMediaFrameworkUtilitiesEditor, Error, "There is not MediaPlayer associated with the MediaTexture '%ls'.", *MediaTexture->GetName());
		}
