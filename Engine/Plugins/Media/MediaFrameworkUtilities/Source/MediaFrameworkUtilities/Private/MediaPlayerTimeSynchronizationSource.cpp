	...
	if (PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UMediaPlayerTimeSynchronizationSource, MediaSource))
	{
		if (bUseForSynchronization && MediaSource)
		{
			...
			if (SynchronizableMediaSource == nullptr || !SynchronizableMediaSource->bUseTimeSynchronization)
			{
				...
				UE_LOGF(LogMediaFrameworkUtilities, Warning, "MediaSource %ls doesn't have the option to use time synchronization enabled.", *MediaSource->GetName());
			}
		}
	}
...
bool UMediaPlayerTimeSynchronizationSource::Open(const FTimeSynchronizationOpenData& InOpenData)
{
	...
	if (MediaSource && MediaTexture)
	{
		...
		if (MediaPlayer)
		{
			...
			{
				UE_LOGF(LogMediaFrameworkUtilities, Error, "MediaSource '%ls' doesn't support timecode synchronization", *MediaSource->GetName());
			}
			else
			{
				...
				if (!bResult)
				{
					UE_LOGF(LogMediaFrameworkUtilities, Error, "Player could not be started.");
				}
				else
				{
					...
					if (!Player.IsValid())
					{
						UE_LOGF(LogMediaFrameworkUtilities, Error, "Current player is invalid.");
						MediaPlayer->Close();
					}
				}
			}
