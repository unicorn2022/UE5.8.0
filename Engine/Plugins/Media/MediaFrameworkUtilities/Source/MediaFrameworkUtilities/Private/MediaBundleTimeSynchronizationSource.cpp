	...
	if (PropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(UMediaBundleTimeSynchronizationSource, MediaBundle))
	{
		if (bUseForSynchronization && MediaBundle && MediaBundle->GetMediaSource())
		{
			...
			if (SynchronizableMediaSource == nullptr || !SynchronizableMediaSource->bUseTimeSynchronization)
			{
				...
				UE_LOGF(LogMediaFrameworkUtilities, Warning, "MediaSource '%ls' doesn't have the option to use time synchronization enabled.", *MediaBundle->MediaSource->GetName());
			}
		}
	}
...
bool UMediaBundleTimeSynchronizationSource::Open(const FTimeSynchronizationOpenData& InOpenData)
{
	...
	if (MediaBundle)
	{
		...
		if (MediaPlayer)
		{
			...
			{
				UE_LOGF(LogMediaFrameworkUtilities, Error, "MediaBundle '%ls' doesn't support timecode synchronization", *MediaBundle->GetName());
			}
			else
			{
				...
				if (!bResult)
				{
					UE_LOGF(LogMediaFrameworkUtilities, Error, "MediaBundle '%ls' could not be started.", *MediaBundle->GetName());
				}
				else
				{
					...
					if (!Player.IsValid())
					{
						UE_LOGF(LogMediaFrameworkUtilities, Error, "Player, for MediaBundle '%ls', is invalid.", *MediaBundle->GetName());
