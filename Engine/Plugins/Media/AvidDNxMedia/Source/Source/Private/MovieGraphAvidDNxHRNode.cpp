...
{
	static const FSlateIcon AvidDNxHRIcon = FSlateIcon(FName("MovieRenderPipelineStyle"), "MovieRenderPipeline.Graph.Icon.RenderMovieFile");

	OutColor = FLinearColor::White;
	return AvidDNxHRIcon;
}

FText UMovieGraphAvidDNxHRNode::GetBasicConfigShortDisplayName() const
{
	return NSLOCTEXT("MovieGraphNodes", "BasicConfigShortName_AVID", "AVID");
}
#endif // WITH_EDITOR
...
bool UMovieGraphAvidDNxHRNode::Initialize_EncodeThread(MovieRenderGraph::IVideoCodecWriter* InWriter)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Failed to initialize Avid DNxHR writer.");
		return false;
	}
