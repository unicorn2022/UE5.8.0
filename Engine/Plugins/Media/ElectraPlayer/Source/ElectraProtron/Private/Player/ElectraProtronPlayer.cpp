...
bool FElectraProtronPlayer::Open(const FString& InUrl, const IMediaOptions* InOptions)
{
	...
	if (!InUrl.StartsWith(TEXT("file://")))
	{
		...
		UE_LOGF(LogElectraProtron, Error, "%ls", *LastError);
		return false;
	}
...
TOptional<FTimespan> FElectraProtronPlayer::GetTrackDuration(EMediaTrackType InTrackType, int32 InTrackIndex) const
{
	TSharedPtr<FImpl, ESPMode::ThreadSafe> Player(GetCurrentPlayer());
	return Player.IsValid() ? Player->GetTrackDuration(InTrackType, InTrackIndex) : TOptional<FTimespan>();
}
