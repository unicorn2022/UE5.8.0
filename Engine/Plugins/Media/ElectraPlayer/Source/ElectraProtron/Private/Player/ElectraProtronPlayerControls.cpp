...
TRangeSet<float> FElectraProtronPlayer::FImpl::GetSupportedRates(EMediaRateThinning InThinning)
{
	...
	if (!bAreRatesValid)
	{
		...
		// No audio?
		if (TrackSelection.SelectedTrackIndex[CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio)] == -1)
		{
			// Video selected?
			const auto TypeIdx = CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video);
...
void FElectraProtronPlayer::FImpl::HandleActiveTrackChanges()
{
	if (TrackSelection.bChanged)
	{
			...
			if (TrackSelection.SelectedTrackIndex[InCodecType] != TrackSelection.ActiveTrackIndex[InCodecType])
			{
				if (TrackSelection.SelectedTrackIndex[InCodecType] >= 0)
				{
					...
					if (Track.IsValid() && TrackSampleBuffers.Contains(Track->TrackID))
					{
						...
						if (tsb.IsValid())
						{
							if (InCodecType == CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video))
							{
								CurrentSampleQueueInterface->SetVideoTrackEnabled(true);
							}
							else if (InCodecType == CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio))
							{
								CurrentSampleQueueInterface->SetAudioTrackEnabled(true);
							}
				...
				else
				{
					...
					if (InCodecType == CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video))
					{
						CurrentSampleQueueInterface->SetVideoTrackEnabled(false);
					}
					else if (InCodecType == CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio))
					{
						CurrentSampleQueueInterface->SetAudioTrackEnabled(false);
					}
		...
		auto videoCI = CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video);
		auto audioCI = CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio);
		...
		if (CurrentRate < 0.0f || SharedPlayParams->PlaybackDirection < 0.0f)
		{
			...
			if (!bReversePossible)
			{
				UE_LOGF(LogElectraProtron, Warning, "New track selection disallows reverse playback. Switching to pause.");
				// Pause.
				IntendedRate = 0.0f;
				HandleRateChanges();
			}
...
void FElectraProtronPlayer::FImpl::HandleSeekRequest(const FSeekRequest& InSeekRequest)
{
	...
	UpdateTrackLoader(CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video));
	UpdateTrackLoader(CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio));
