...
public:
	...
	public:
		...
		void SeekIssuedTo(FTimespan InToTime, TOptional<int32> InNextSequenceIndex)
		{
			...
			bVideoSampleWillBeReady = false;
			bAudioSampleWillBeReady = false;
		}

		void SetVideoTrackEnabled(bool bInEnabled)
		{
			bIsVideoTrackEnabled = bInEnabled;
		}
		void SetAudioTrackEnabled(bool bInEnabled)
		{
			bIsAudioTrackEnabled = bInEnabled;
		}
		bool IsVideoTrackEnabled() const
		{
			return bIsVideoTrackEnabled;
		}
		bool IsAudioTrackEnabled() const
		{
			return bIsAudioTrackEnabled;
		}
		void VideoSampleWillBeReady()
		{
			bVideoSampleWillBeReady = true;
		}
		void AudioSampleWillBeReady()
		{
			bAudioSampleWillBeReady = true;
		}
		bool AreRequiredSamplesReady() const
		{
			return (!bIsVideoTrackEnabled || (bIsVideoTrackEnabled && bVideoSampleWillBeReady)) &&
				   (!bIsAudioTrackEnabled || (bIsAudioTrackEnabled && bAudioSampleWillBeReady));
		}
	...
	private:
		...
		bool bIsVideoTrackEnabled = false;
		bool bIsAudioTrackEnabled = false;
		bool bVideoSampleWillBeReady = false;
		bool bAudioSampleWillBeReady = false;
	};
	...
	TOptional<FTimespan> GetTrackDuration(EMediaTrackType InTrackType, int32 InTrackIndex);
...
private:
	static constexpr int32 CodecTypeIndex(const Electra::FCodecTypeFormat::EType InType)
	{ return (int32) InType; }
	...
	struct FTrackInfo
	{
		...
		Electra::FDRMTypeFormat DrmFormat;
		Electra::FDecoderInformation DecoderInformation;

		FTimespan Duration;
		TSharedPtr<MP4Boxes::FMP4BoxTRAK, ESPMode::ThreadSafe> TrackBox;
	...
	using FTrackIterator = TSharedPtr<MP4Boxes::FMP4Track::FIterator, ESPMode::ThreadSafe>;
	...
	const int32 kCodecTrackIndexMap[(int32) Electra::FCodecTypeFormat::EType::MAX] =
	{
		CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video),
		CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio),
		CodecTypeIndex(Electra::FCodecTypeFormat::EType::Subtitle),
		CodecTypeIndex(Electra::FCodecTypeFormat::EType::Timecode)
	};
	...
	private:
		...
		TSharedPtr<FSharedPlayParams, ESPMode::ThreadSafe> SharedPlayParams;
		TSharedPtr<MP4Utilities::FMP4FileDataReader, ESPMode::ThreadSafe> Reader;
		FString LastErrorMessage;
		...
		FMP4SamplePtr RetrieveSample(const FMP4TrackSampleBufferPtr& InTrackSampleBuffer, const FTrackIterator& InSampleIt, const FTrackIterator& InOptionalTimecodeIt, const Electra::FCodecTypeFormat::FTMCDTimecode& InTimecodeInfo);
	...
	FLoaderThread VideoLoaderThread {Config, CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video)};
	FLoaderThread AudioLoaderThread {Config, CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio)};

	FDecoderThread VideoDecoderThread {Config, CodecTypeIndex(Electra::FCodecTypeFormat::EType::Video)};
	FDecoderThread AudioDecoderThread {Config, CodecTypeIndex(Electra::FCodecTypeFormat::EType::Audio)};
};
