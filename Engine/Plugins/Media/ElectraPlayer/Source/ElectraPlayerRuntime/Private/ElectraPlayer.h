...
public:
	...
	TOptional<FTimespan> GetTrackDuration(EMediaTrackType InTrackType, int32 InTrackIndex) const override;
	...
	void ReportMediaMetadataChanged(TSharedPtrTS<Electra::FMetadataParser> InMetadata) override;
	...
	void ReportVideoFrameStats(const Metrics::FFrameStats& InFrameStats) override;
	void ReportAudioFrameStats(const Metrics::FFrameStats& InFrameStats) override;
...
private:
	...
	void HandlePlayerMediaMetadataChanged(const TSharedPtrTS<Electra::FMetadataParser>& InMetadata);
	...
	void HandlePlayerEventDroppedVideoFrame(const Metrics::FFrameStats& InFrameStats);
	void HandlePlayerEventDroppedAudioFrame(const Metrics::FFrameStats& InFrameStats);
...
private:
	...
	public:
		...
		static void DoCloseAsync(TSharedPtr<FInternalPlayerImpl, ESPMode::ThreadSafe>&& InPlayer, uint32 InPlayerID, TSharedPtr<IAsyncResourceReleaseNotification, ESPMode::ThreadSafe> InAsyncResourceReleaseNotification, bool bReleasePools, int64 TexturePoolDecoderID);
	};
	...
	{
		FPlayerMetricEvent_MediaMetadataChange(const TSharedPtrTS<Electra::FMetadataParser>& InMetadata) : FPlayerMetricEventBase(EType::MediaMetadataChanged), NewMetadata(InMetadata) {}
		TSharedPtrTS<Electra::FMetadataParser> NewMetadata;
	};
	...
	struct FPlayerMetricEvent_DroppedFrame : public FPlayerMetricEventBase
	{
		FPlayerMetricEvent_DroppedFrame(EType InType, const Metrics::FFrameStats& InFrameStats)	: FPlayerMetricEventBase(InType), FrameStats(InFrameStats) {}
	...
	struct FStatistics
	{
		...
		void Reset()
		{
			...
			NumErrConnDrops = 0;
			NumErrOther = 0;
			NumTotalAddedVideoFrames = 0;
			NumTotalDroppedVideoFrames = 0;
			AvgVideoDropsPerInterval = 0;
			MinVideoDropsPerInterval = 0;
			MaxVideoDropsPerInterval = 0;
			VideoDropIntervalMs = 0;
			VideoNumIntervals = 0;
			VideoNumIntervalsWithDrops = 0;
		}
		...
		uint32					NumErrOther;
		uint64					NumTotalAddedVideoFrames;
		uint64					NumTotalDroppedVideoFrames;
		uint32					AvgVideoDropsPerInterval;
		uint32					MinVideoDropsPerInterval;
		uint32					MaxVideoDropsPerInterval;
		uint32					VideoDropIntervalMs;
		uint32					VideoNumIntervals;
		uint32					VideoNumIntervalsWithDrops;
	};
	...
	bool bRetainPoolsForReuse = false;
	...
	mutable FCriticalSection VideoFormatLock;
	FVideoStreamFormat CurrentlyActiveVideoStreamFormat;
