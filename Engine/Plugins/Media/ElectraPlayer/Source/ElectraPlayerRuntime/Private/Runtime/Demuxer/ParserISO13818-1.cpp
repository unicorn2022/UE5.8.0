...
IParserISO13818_1::EParseState FParserISO13818_1::ParseNextPacket(IPlayerSessionServices* InPlayerSession, IGenericDataReader* InDataReader)
{
	...
	if (NumBytesRead == 188)
	{
		...
		if (!((PID >= 5 && PID <= 15) || (PID == 0x1fff)))
		{
			...
			{
				UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Dropping duplicate packet with `continuity_counter` %d in PID %u", Current.ContinuityCounter, PID);
			}
			else
			{
				// Payload
				if (bHasPayload)
				{
					...
					if (PayloadSize)
					{
						...
						{
							UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Dropping packet for PID %u", PID);
						}
						if (PIDStream.IsValid())
						{
...
IParserISO13818_1::EParseState FParserISO13818_1::Parse(IPlayerSessionServices* InPlayerSession, IGenericDataReader* InDataReader)
{
	switch(ParseState)
	{
		...
		case EParseState::NewProgram:
		{
			...
			{
				UE_LOGF(LogElectraMPEGTSParser, Log, "Received new program, but user did not handle it.");
			}
			ParseState = EParseState::Continue;
			break;
		}
				...
				for(int32 i=0; i<sel.SelectedStreamPIDs.Num(); ++i)
				{
					if (sel.SelectedStreamPIDs[i] >= 32)
					{
						if (pm->PESPIDStream.Contains(sel.SelectedStreamPIDs[i]))
						{
							...
							{
								UE_LOGF(LogElectraMPEGTSParser, Warning, "PID %d not present in stream map.", sel.SelectedStreamPIDs[i]);
							}
						}
						else
						{
							UE_LOGF(LogElectraMPEGTSParser, Log, "User selected PES PID %d which does not exist in the selected program.", sel.SelectedStreamPIDs[i]);
						}
					}
					else
					{
						UE_LOGF(LogElectraMPEGTSParser, Warning, "Invalid PID %d specified in program stream selection.", sel.SelectedStreamPIDs[i]);
					}
				}
			}
			else
			{
				UE_LOGF(LogElectraMPEGTSParser, Log, "User selected program %d which does not exist in the PAT.", sel.ProgramNumber);
			}
		}
		else
		{
			UE_LOGF(LogElectraMPEGTSParser, Log, "Not enabling any PES stream since user requested no program.");
		}
		PendingUserProgramSelection.Reset();
	}
}
...
void FParserISO13818_1::FPIDStream::ExtractValidSections(TArray<TSharedPtrTS<TArray<uint8>>>& OutDataSections)
{
	...
	while(GatheringSection.bIsOpen)
	{
		...
		if (bCheckCRC)
		{
			...
			if (crc)
			{
				UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Mismatching `CRC_32` in section, ignoring section!");
				bIsGood = false;
			}
		}
		if (bIsGood)
...
void FParserISO13818_1::FPIDStream::ExtractValidPESPackets(TArray<TSharedPtrTS<FPESData>>& OutPESSections, const FCurrent& InCurrent, int32 InNumBytesAddedNow)
{
	...
	while(GatheringSection.bIsOpen)
	{
		...
		if (GatheringSection.TotalSize < 0 && AccumulationBuffer.Num() >= 6)
		{
			...
			{
				UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Supposed PES packet does not start with 0x00 0x00 0x01, ignoring!");
				GatheringSection.Reset();
				AccumulationBuffer.Empty();
				return;
			}
			...
			{
				UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "PES packet size given as 0 for a non-video stream type 0x(%02x), ignoring!", AccumulationBuffer[3]);
				GatheringSection.Reset();
				AccumulationBuffer.Empty();
				return;
			}
		...
		if ((GatheringSection.TotalSize < 0) || (GatheringSection.TotalSize > 0 && AccumulationBuffer.Num() < GatheringSection.TotalSize))
		{
			...
			{
				UE_LOGF(LogElectraMPEGTSParser, Verbose, "PES packet with given size of %d on PID %d is incomplete at end of stream, ignoring!", GatheringSection.TotalSize, PID);
			}
			return;
		}
...
bool FParserISO13818_1::FPIDStream::ProcessPayload(TArray<FPayload>& OutPayloadResults, FBitstreamReader& InOutBR, const FCurrent& InCurrent)
{
	...
	if (Type == EType::Section)
	{
		...
		{
			UE_LOGF(LogElectraMPEGTSParser, Verbose, "Mismatching `continuity_counter` in packet, dropping.");
			GatheringSection.Reset();
			return false;
		}
		if (InCurrent.bErrorIndicator)
		{
			UE_LOGF(LogElectraMPEGTSParser, Verbose, "`transport_error_indicator` set in PSI packet, dropping.");
			GatheringSection.Reset();
			return false;
		}
		...
		if (InCurrent.bIsStart)
		{
			...
			{
				UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Section `pointer_field` points outside the packet, ignoring section!");
				GatheringSection.Reset();
				return false;
			}
	...
	else
	{
		...
		if (InCurrent.bErrorIndicator)
		{
			UE_LOGF(LogElectraMPEGTSParser, Verbose, "`transport_error_indicator` set in PES packet, dropping.");
			GatheringSection.Reset();
			AccumulationBuffer.Empty();
			return false;
		}
...
FParserISO13818_1::ETableResult FParserISO13818_1::HandlePSITable(const FPayload& InTablePayload)
{
	...
	if (table_id == 0xff)
	{
		UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Section `table_id` is set to fobidden value, ignoring section!");
	...
	{
		UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Section has `section_syntax_indicator` set to 0 for table_id %u, ignoring section!", table_id);
		return ETableResult::Continue;
	}
	uint32 Zero = br.GetBits(1);
	if ((table_id == 0 || table_id == 1 || table_id == 2 || table_id == 3) && Zero != 0)
	{
		UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Section has `0` set to 1 for table_id %u, ignoring section!", table_id);
		return ETableResult::Continue;
	}
	...
	{
		UE_LOGF(LogElectraMPEGTSParser, VeryVerbose, "Section has `section_length` %u exceeding %u, ignoring section!", section_length, SectionMaxSize);
		return ETableResult::Continue;
	}
...
bool FParserISO13818_1::ProcessPAT(const FPSITable& InTableInfo, FBitstreamReader& InOutBR)
{
	...
	if (!CurrentProgramAssociation.IsValid() || (CurrentProgramAssociation->VersionNumber != NewProgramAssociation->VersionNumber && InTableInfo.CurrentNext))
	{
		...
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "PAT change detected. This is not supported at the moment. Ignoring new PAT.");
...
bool FParserISO13818_1::ProcessPMT(const FPSITable& InTableInfo, FBitstreamReader& InOutBR)
{
	...
	if (!CurrentProgramMap.IsValid() || (CurrentProgramMap->VersionNumber != NewProgramMap->VersionNumber && InTableInfo.CurrentNext))
	{
		...
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "PMT change detected. This is not supported at the moment. Ignoring new PMT.");
			// Do we need to clear current programs?
			return false;
		}
		...
		if (CurrentProgramAssociation.IsValid())
		{
			if (CurrentProgramAssociation->ProgramPIDMap.Contains(InTableInfo.TransportOrProgramStreamId))
			{
				...
				{
					UE_LOGF(LogElectraMPEGTSParser, Warning, "New PMT encountered for program %d on PID %d that was using PID %d until now.", InTableInfo.TransportOrProgramStreamId, InTableInfo.PID, CurrentProgramAssociation->ProgramPIDMap[InTableInfo.TransportOrProgramStreamId]);
				}
			}
			else
			{
				UE_LOGF(LogElectraMPEGTSParser, Warning, "New PMT encountered for program %d on PID %d that is not listed in the current PAT.", InTableInfo.TransportOrProgramStreamId, InTableInfo.PID);
			}
		}
		else
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "New PMT encountered for program %d on PID %d without having an established PAT yet.", InTableInfo.TransportOrProgramStreamId, InTableInfo.PID);
		}
...
FParserISO13818_1::EPESPacketResult FParserISO13818_1::ParseADTSAAC(TArray<FESPacket>& OutPackets, FBitstreamReader& InOutBR, const FDTSPTS& InDTSPTS, FResidualPESData* InResidualData, bool bFlushResiduals)
{
	...
	for(int32 npkt=0; ; ++npkt)
	{
		if (br.GetRemainingByteLength() < 7)
		{
			if (!InResidualData)
			{
				UE_LOGF(LogElectraMPEGTSParser, Warning, "Remaining PES packet data too small to contain an ADTS header. Incorrect multiplex?");
			}
			return EPESPacketResult::Truncated;
		}
		if (br.GetBits(12) != 0xfff)
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "Incorrect sync value in ADTS header. Incorrect multiplex?");
			return EPESPacketResult::Invalid;
		}
		...
		if (num_frames > 0)
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "Multiple RDBs in ADTS frame is not supported!");
			return EPESPacketResult::Invalid;
		}
		if (channel_configuration == 0)
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "Channel configuration 0 is not supported!");
			return EPESPacketResult::Invalid;
		}
...
FParserISO13818_1::EPESPacketResult FParserISO13818_1::ParseMPEGAudio(TArray<FESPacket>& OutPackets, FBitstreamReader& InOutBR, const FDTSPTS& InDTSPTS, FResidualPESData* InResidualData, bool bFlushResiduals)
{
	...
	for(int32 npkt=0; ; ++npkt)
	{
		if (NumBytesRemaining < 4)
		{
			if (!InResidualData)
			{
				UE_LOGF(LogElectraMPEGTSParser, Warning, "Remaining PES packet data too small to contain an MPEG audio frame header. Incorrect multiplex?");
			}
			return EPESPacketResult::Truncated;
		}
			...
			(InData[2] & 0x0c) != 0x0c)								// sample rate index not 3
		{
			...
			if ((HeaderValue & MPEGHeaderMask) != MPEGHeaderExpectedValue)
			{
				...
				UE_LOGF(LogElectraMPEGTSParser, Warning, "Mismatching frame header between consecutive audio frames. Incorrect multiplex?");
				return EPESPacketResult::Invalid;
			}
			...
			if (NumBytesRemaining < (int32)FrameSize)
			{
				if (!InResidualData)
				{
					UE_LOGF(LogElectraMPEGTSParser, Warning, "Remaining PES packet data too small for a complete MPEG audio frame. Incorrect multiplex?");
				}
				return EPESPacketResult::Truncated;
			}
		...
		else
		{
			...
			UE_LOGF(LogElectraMPEGTSParser, Warning, "Incorrect sync value in MPEG audio frame header. Incorrect multiplex?");
			return EPESPacketResult::Invalid;
		}
	}
	return EPESPacketResult::Ok;
	...
	for(int32 npkt=0; ; ++npkt)
	{
		...
		if (ElectraDecodersUtil::MPEG::H264::ParseBitstreamForNALUs(NALUs, InData, InDataLength))
		{
			...
			if (NumAUDNalus >= (bFlushResiduals ? 1 : 2))
			{
				...
				if (FirstNALUIndex != 0)
				{
					UE_LOGF(LogElectraMPEGTSParser, Warning, "First NALU in AVC packet is not an AUD");
				}
		...
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "Failed to parse the AVC packet for NALUs");
			return EPESPacketResult::Invalid;
		}
	}
	return EPESPacketResult::Ok;
	...
	for(int32 npkt=0; ; ++npkt)
	{
		...
		if (ElectraDecodersUtil::MPEG::H265::ParseBitstreamForNALUs(NALUs, InData, InDataLength))
		{
			...
			if (NumAUDNalus >= (bFlushResiduals ? 1 : 2))
			{
				...
				if (FirstNALUIndex != 0)
				{
					UE_LOGF(LogElectraMPEGTSParser, Warning, "First NUT in HEVC packet is not an AUD");
				}
		...
		{
			UE_LOGF(LogElectraMPEGTSParser, Warning, "Failed to parse the HEVC packet for NUTs");
			return EPESPacketResult::Invalid;
		}
	}
	return EPESPacketResult::Ok;
