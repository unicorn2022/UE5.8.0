...
public:
	...
	static TSharedPtr<IElectraCodecFactory, ESPMode::ThreadSafe> GetDecoderFactory(TMap<FString, FVariant>& OutAddtlCfg, const FCodecTypeFormat& InCodecTypeFormat);
...
{
	Electra::FCodecTypeFormat ctf = InCodecInfo.ToCodecTypeFormat();
	TMap<FString, FVariant> AddtlCfg;
	return FAudioDecoderImpl::GetDecoderFactory(AddtlCfg, ctf).IsValid();
}
...
TSharedPtr<IElectraCodecFactory, ESPMode::ThreadSafe> FAudioDecoderImpl::GetDecoderFactory(TMap<FString, FVariant>& OutAddtlCfg, const FCodecTypeFormat& InCodecTypeFormat)
{
	...
	if (!FactoryModule || !InCodecTypeFormat.Properties.IsType<FCodecTypeFormat::FAudio>())
	{
		return nullptr;
	}
	TMap<FString, FVariant> NoAdditionalOptions;
	return FactoryModule->GetBestDecoderFactoryForFormat(OutAddtlCfg, InCodecTypeFormat, NoAdditionalOptions);
}
...
bool FAudioDecoderImpl::InternalDecoderCreate()
{
	...
	FCodecTypeFormat CodecTypeFormat = CurrentCodecData->ParsedInfo.ToCodecTypeFormat();
	if (CodecTypeFormat.DCR.IsEmpty() && CurrentCodecData->RawCSD.Num())
	{
		CodecTypeFormat.DCR = CurrentCodecData->RawCSD;
	}
	if (CodecTypeFormat.CSD.IsEmpty() && CurrentCodecData->CodecSpecificData.Num())
	{
		CodecTypeFormat.CSD = CurrentCodecData->CodecSpecificData;
	}

	TMap<FString, FVariant> AddtlCfg;
	TSharedPtr<IElectraCodecFactory, ESPMode::ThreadSafe> Factory = GetDecoderFactory(AddtlCfg, CodecTypeFormat);
	if (!Factory.IsValid())
	{
	...
	if (SessionServices)
	{
		SessionServices->GetMediaInfoDictionary().Set(MediaInfoAudioDecoderProvider, FVariantValue(Factory->GetProviderInformation().GetName()));
	}

	DecoderInstance = Factory->CreateDecoder(CodecTypeFormat, AddtlCfg);
	...
	bMustBeSuspendedInBackground = MustSuspendDecodersInBackground(ElectraDecodersUtil::GetVariantValueSafeI64(Features, IElectraDecoderFeature::MustBeSuspendedInBackground, 0) != 0);
		...
		if (NextAccessUnits.Wait(1000 * 10))
		{
			...
			if (CurrentAccessUnit.IsValid())
			{
				...
				if (CurrentCodecData != CurrentAccessUnit->AccessUnit->AUCodecData && CurrentAccessUnit->AccessUnit->AUCodecData.IsValid())
				{
					...
					CSDOptions.Empty();
					// Prepare the codec type format structure.
					FCodecTypeFormat ctf = CurrentCodecData->ParsedInfo.ToCodecTypeFormat();
					if (CurrentCodecData->RawCSD.Num())
					{
						ctf.DCR = CurrentCodecData->RawCSD;
					}
					if (CurrentCodecData->CodecSpecificData.Num())
					{
						ctf.CSD = CurrentCodecData->CodecSpecificData;
					}
					// Get CSD options from codec type format
					if (ctf.DCR.Num())
					{
						CSDOptions.Emplace(TEXT("dcr"), FVariant(ctf.DCR));
					}
					if (ctf.CSD.Num())
					{
						CSDOptions.Emplace(TEXT("csd"), FVariant(ctf.CSD));
					}
					for(auto &It : ctf.ExtraBoxes)
					{
						FString BoxName = MP4Utilities::Printable4CC(It.Key);
						CSDOptions.Emplace(BoxName, It.Value);
					}
				}
...
IElectraDecoder::ECSDCompatibility FAudioDecoderImpl::IsCompatibleWith()
{
	...
	if (DecoderInstance.IsValid() && CurrentAccessUnit.IsValid())
	{
		...
		{
			// Prepare the codec type format structure.
			FCodecTypeFormat ctf = CurrentAccessUnit->AccessUnit->AUCodecData->ParsedInfo.ToCodecTypeFormat();
			if (CurrentAccessUnit->AccessUnit->AUCodecData->RawCSD.Num())
			{
				ctf.DCR = CurrentAccessUnit->AccessUnit->AUCodecData->RawCSD;
			}
			if (CurrentAccessUnit->AccessUnit->AUCodecData->CodecSpecificData.Num())
			{
				ctf.CSD = CurrentAccessUnit->AccessUnit->AUCodecData->CodecSpecificData;
			}
			// Get CSD options from codec type format
			if (ctf.DCR.Num())
			{
				TestCSDOptions.Emplace(TEXT("dcr"), FVariant(ctf.DCR));
			}
			if (ctf.CSD.Num())
			{
				TestCSDOptions.Emplace(TEXT("csd"), FVariant(ctf.CSD));
			}
			for(auto &It : ctf.ExtraBoxes)
			{
				FString BoxName = MP4Utilities::Printable4CC(It.Key);
				TestCSDOptions.Emplace(BoxName, It.Value);
			}
...
bool FAudioDecoderImpl::CheckBackgrounding()
{
	...
	{
		UE_LOGF(LogElectraPlayer, Log, "FAudioDecoderImpl(%p): OnSuspending", this);
		if (DecoderInstance.IsValid())
		{
		...
		UE_LOGF(LogElectraPlayer, Log, "FAudioDecoderImpl(%p): OnResuming", this);
		if (DecoderInstance.IsValid())
		{
