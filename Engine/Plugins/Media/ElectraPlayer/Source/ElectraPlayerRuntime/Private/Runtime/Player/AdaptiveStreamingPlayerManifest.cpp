...
#include "Player/isobmff/PlaylistReaderISOBMFF.h"
...
static TAutoConsoleVariable<bool> CVarElectraUseLegacyMP4Player(
	TEXT("Electra.UseLegacyMP4Player"),
	false,
	TEXT("Use old sequentially reading mp4 player instead of new player.\n")
	TEXT(" true: use old player, false use new player"),
	ECVF_Default);
...
void FAdaptiveStreamingPlayer::InternalLoadManifest(const FString& InURL, const FString& MimeType)
{
	...
	if (CurrentState == EPlayerState::eState_Idle)
	{
		...
		if (mimeType.Len())
		{
			...
			{
				if (CVarElectraUseLegacyMP4Player.GetValueOnAnyThread())
				{
					ManifestReader = IPlaylistReaderMP4::Create(this);
				}
				else
				{
					ManifestReader = IPlaylistReaderISOBMFF::Create(this);
				}
			}
...
bool FAdaptiveStreamingPlayer::SelectManifest()
{
	if (ManifestReader.IsValid())
	{
		...
		// If this is an audio-only stream we set the audio buffer configuration from that of audio-only.
		if (VideoTrackMetadata.IsEmpty() && !AudioTrackMetadata.IsEmpty())
		{
			PlayerConfig.StreamBufferConfigAudio = PlayerConfig.StreamBufferConfigAudioOnly;
		}


		// Let the ABR know the format as well.
		StreamSelector->SetFormatType(ManifestReader->GetMediaFormatType());
		...
		switch(ManifestReader->GetMediaFormatType())
		{
		...
		// For formats that do not reload the playlist every so often we can get rid of the manifest reader.
		if (!Manifest->ReloadsPlaylistPeriodically())
		{
...
{
	TSharedPtrTS<FMetadataParser> NextMetadata;
