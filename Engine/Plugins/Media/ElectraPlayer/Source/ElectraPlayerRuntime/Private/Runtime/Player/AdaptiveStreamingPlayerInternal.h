...
#define INTERR_GENERAL_FAILURE					8
...
public:
	...
	virtual ~FMediaRenderClock() = default;
...
struct FDroppedFrameStats
{
	...
	void Clear()
	{
		...
		PausedDelta = -1;
		NumIntervals = 0;
		NumIntervalsWithDrops = 0;
	}
	TArray<FEntry> Entries;
	TRollingAverage<int64> AverageDropsPerInterval;
	int64 LastProcessedTime = -1;
	int64 PausedDelta = -1;
	int64 ReportingIntervalMs = -1;
	uint32 NumIntervals = 0;
	uint32 NumIntervalsWithDrops = 0;
};
...
struct FMetricEvent
{
	...
	struct FParam
	{
		...
		{
			TSharedPtrTS<FMetadataParser> NewMetadata;
		};
		FString								URL;
		...
		Metrics::FFrameStats				FrameStats;
	...
	static TSharedPtrTS<FMetricEvent> ReportMediaMetadataChanged(const TSharedPtrTS<FMetadataParser>& InNextMetadata)
	{
		TSharedPtrTS<FMetricEvent> Evt = MakeSharedTS<FMetricEvent>();
	...
	static TSharedPtrTS<FMetricEvent> ReportVideoFrameStats(uint64 InNumTotalFrames, uint64 InNumTotalDroppedFrames, uint64 InNumNewDroppedFrames, const FDroppedFrameStats& InDropStats)
	{
		TSharedPtrTS<FMetricEvent> Evt = MakeSharedTS<FMetricEvent>();
		Evt->Type = EType::VideoFrameStats;
		Evt->Param.FrameStats.NumTotal = InNumTotalFrames;
		Evt->Param.FrameStats.NumDropped = InNumTotalDroppedFrames;
		Evt->Param.FrameStats.NumNewDrops = (uint32) InNumNewDroppedFrames;
		Evt->Param.FrameStats.AvgDropsPerInterval = (uint32)InDropStats.AverageDropsPerInterval.GetAverage();
		Evt->Param.FrameStats.MinDropsPerInterval = (uint32)InDropStats.AverageDropsPerInterval.GetMin();
		Evt->Param.FrameStats.MaxDropsPerInterval = (uint32)InDropStats.AverageDropsPerInterval.GetMax();
		Evt->Param.FrameStats.DropIntervalMs = (uint32)(InDropStats.ReportingIntervalMs > 0 ? InDropStats.ReportingIntervalMs : 0);
		Evt->Param.FrameStats.NumIntervals = InDropStats.NumIntervals;
		Evt->Param.FrameStats.NumIntervalsWithDrops = InDropStats.NumIntervalsWithDrops;
		return Evt;
	}
...
public:
	...
	TSharedPtr<FMediaSamples, ESPMode::ThreadSafe> GetMediaSamples() override;
...
private:
	...
	struct FVideoOutput
	{
		void Close()
		{
			...
			{
				OutputHandler->SetMediaSamples(nullptr);
				OutputHandler.Reset();
			}
		}
		...
		{
			return OutputHandler.IsValid() ? OutputHandler->GetTotalAddedSamples() : (uint64)0;
		}
		uint64 GetTotalNumDroppedSamples()
		{
			return OutputHandler.IsValid() ? OutputHandler->GetTotalNumDroppedSamples() : (uint64)0;
		}
		uint64 GetNewlyNumDroppedSamples()
		{
			return OutputHandler.IsValid() ? OutputHandler->GetNewlyNumDroppedSamples(true) : (uint64)0;
		}
		uint64 PeekNewlyNumDroppedSamples()
		{
			return OutputHandler.IsValid() ? OutputHandler->GetNewlyNumDroppedSamples(false) : (uint64)0;
		}
	...
	struct FMediaMetadataUpdate
	{
		...
		void AddEntry(const FTimeValue& InValidFrom, const TSharedPtrTS<FMetadataParser>& InMetadata, bool bInTriggerInternalRefresh)
		{
		...
		TSharedPtrTS<FMetadataParser> GetActive() const
		{
		...
		struct FEntry
		{
			...
			TSharedPtrTS<FMetadataParser> Metadata;
			bool bTriggerInternalRefresh = false;
		};
		TArray<FEntry> NextEntries;
		TSharedPtrTS<FMetadataParser> ActiveMetadata;
	...
	bool GetHighestVideoProfileResolution(FStreamCodecInformation& OutStreamInfo, TSharedPtrTS<const FBufferSourceInfo> InBufferSource, const FTimeValue& AtTime, const FStreamCodecInformation& InForCodec);
	...
	void UpdateFrameStats(bool bFinal);
	...
	void GetStreamBufferUtilization(FAccessUnitBufferInfo& OutInfo, const FAccessUnit* InForThis);
	...
	TSharedPtr<FMediaSamples, ESPMode::ThreadSafe>						EmptyMediaSamples;
	...
	FDroppedFrameStats													DroppedFrameStats;
