...
namespace Stream
{
extern TAutoConsoleVariable<bool> CVarElectraStreamUseLegacyMP4;
}

struct FMediaPlaylistAndStateHLS;

class FParsedMOOVCommon
{
public:
	virtual ~FParsedMOOVCommon() = default;

	struct FTrackInfo
	{
		TSharedPtr<MP4Boxes::FMP4BoxTRAK, ESPMode::ThreadSafe> TrakBox;
	...
	bool Parse(TConstArrayView<uint8> InInitSegmentData)
	{
		TSharedPtr<MP4Boxes::FMP4BoxMOOV, ESPMode::ThreadSafe> MOOV;
		TArray<TSharedPtr<MP4Utilities::FMP4BoxData, ESPMode::ThreadSafe>> RootBoxes;
		if (MP4Utilities::FMP4BoxLocator::LocateAndReadRootBoxesFromBuffer(RootBoxes, InInitSegmentData))
		{
			...
			for(int32 i=0; i<RootBoxes.Num(); ++i)
			{
				...
				if (tp.ParseBoxTree(RootBoxes[i]))
				{
					...
					{
						MOOV = StaticCastSharedPtr<MP4Boxes::FMP4BoxMOOV>(ParsedRootBoxes[i]);
						break;
					}
				}
				else
	...
	bool Parse(TSharedPtr<MP4Boxes::FMP4BoxMOOV, ESPMode::ThreadSafe> InMOOV)
	{
		if (!InMOOV)
		{
			return false;
		...
		MoovBox = InMOOV;
		MvexBox = MoovBox->FindBoxRecursive<MP4Boxes::FMP4BoxMVEX>(MP4Utilities::MakeBoxAtom('m','v','e','x'), 0);
		if (!MvexBox)
		{
			return false;
		}
		auto Udta = MoovBox->FindBoxRecursive<MP4Boxes::FMP4BoxUDTA>(MP4Utilities::MakeBoxAtom('u','d','t','a'), 0);
		MetaBox = Udta ? Udta->FindBoxRecursive<MP4Boxes::FMP4BoxMETA>(MP4Utilities::MakeBoxAtom('m','e','t','a'), 0) : nullptr;
		...
		for(int32 nTk=0; nTk<AllTracks.Num(); ++nTk)
		{
			auto Tkhd = AllTracks[nTk]->FindBoxRecursive<MP4Boxes::FMP4BoxTKHD>(MP4Utilities::MakeBoxAtom('t','k','h','d'), 0);
			auto Mdhd = AllTracks[nTk]->FindBoxRecursive<MP4Boxes::FMP4BoxMDHD>(MP4Utilities::MakeBoxAtom('m','d','h','d'), 1);
			if (!Tkhd || !Mdhd)
			{
				continue;
			}
			FTrackInfo ti;
			ti.TrakBox = AllTracks[nTk];
			ti.TrackID = Tkhd->GetTrackID();
			ti.TrackTimescale = Mdhd->GetTimescale();

			FString Msg;
			if (!ElectraDecodersUtil::MP4::GetTrackFormatInfo(Msg, ti.CodecFormat, ti.DrmFormat, ti.DecoderInformation, AllTracks[nTk], ti.TrackID, true))
			{
				continue;
			}
			if (!ti.DecoderInformation.bIsDecodable)
			{
				continue;
			}
			if (ti.CodecFormat.Type != FCodecTypeFormat::EType::Video && ti.CodecFormat.Type != FCodecTypeFormat::EType::Audio && ti.CodecFormat.Type != FCodecTypeFormat::EType::Subtitle)
			{
				continue;
			}
			TrackMap.Add(ti.TrackID, ti);
		}
		MoovBox->GetAllBoxInstances(PsshBoxes, MP4Utilities::MakeBoxAtom('p','s','s','h'));
		return true;
	}

	const TMap<uint32, FTrackInfo>& GetTrackMap() const
	{ return TrackMap; }

	TSharedPtr<MP4Boxes::FMP4BoxMVEX, ESPMode::ThreadSafe> GetMVEXBox() const
	{ return MvexBox; }

	TSharedPtr<MP4Boxes::FMP4BoxMETA, ESPMode::ThreadSafe> GetMETABox() const
	{ return MetaBox; }
	const TArray<TSharedPtr<MP4Boxes::FMP4BoxPSSH, ESPMode::ThreadSafe>>& GetPSSHBoxes() const
	{ return PsshBoxes; }
private:
	TSharedPtr<MP4Boxes::FMP4BoxMOOV, ESPMode::ThreadSafe> MoovBox;
	...
	TMap<uint32, FTrackInfo> TrackMap;
};
...
private:
	...
	struct FStreamHandler : public FMediaThread, public IGenericDataReader
	{
		...
		class FBoxDataLoader : public MP4Utilities::IMP4DataReaderBase, public TSharedFromThis<FBoxDataLoader, ESPMode::ThreadSafe>
		{
		public:
			FBoxDataLoader(IGenericDataReader* InGenericReader)
				: GenericReader(InGenericReader)
			{ }
			virtual ~FBoxDataLoader() = default;
			int64 ReadData(void* InOutBuffer, int64 InNumBytes, int64 InFromOffset, FCancellationCheckDelegate InCheckCancellationDelegate) override
			{
				...
				int64 nr = GenericReader->ReadData(InOutBuffer, InNumBytes, InFromOffset);
				return nr == InNumBytes ? nr : GenericReader->HasReadBeenAborted() ? EResult::Canceled : EResult::ReadError;
			}
			int64 GetTotalFileSize() override
			{ return GenericReader->GetTotalSize(); }
			int64 GetCurrentFileOffset() override
			{ return GenericReader->GetCurrentOffset(); }
			bool HasReachedEOF() override
			{ return GenericReader->HasReachedEOF(); }
			FString GetLastError() override
			{ return FString(); }
		private:
			IGenericDataReader* GenericReader = nullptr;
		};
		...
		void SignalWork();
		void WorkerThread();
		EHandleResult SetError(const FString& InMessage, uint16 InCode);
		...
		EHandleResult HandleMP4MediaLegacy(TSharedPtrTS<FStreamSegmentRequestCommon> InRequest);
		...
		void HandleMP4Metadata(const TSharedPtrTS<FStreamSegmentRequestCommon>& InRequest, const TSharedPtr<MP4Boxes::FMP4BoxMETA, ESPMode::ThreadSafe>& InMoovMetaBox, const TSharedPtr<MP4Boxes::FMP4BoxMETA, ESPMode::ThreadSafe>& InMoofMetaBox);
		void HandleMP4EventMessages(const TSharedPtrTS<FStreamSegmentRequestCommon>& InRequest, const TSharedPtr<MP4Boxes::FMP4BoxSIDX, ESPMode::ThreadSafe>& InSidxBox, const TArray<TSharedPtr<MP4Boxes::FMP4BoxEMSG, ESPMode::ThreadSafe>>& InEventMessageBoxes);
		void HandleMP4MetadataLegacy(const TSharedPtrTS<FStreamSegmentRequestCommon>& InRequest, const TSharedPtrTS<IParserISO14496_12>& InMP4Parser, const TSharedPtrTS<const IParserISO14496_12>& InMP4InitSegment, const FTimeValue& InBaseTime);
		void HandleMP4EventMessagesLegacy(const TSharedPtrTS<FStreamSegmentRequestCommon>& InRequest, const TSharedPtrTS<IParserISO14496_12>& InMP4Parser);
		...
		TVariant<FNoInitSegmentData, TSharedPtrTS<FParsedMOOVCommon>, TSharedPtrTS<const IParserISO14496_12>, TSharedPtrTS<const IParserMKV>, TSharedPtrTS<const TArray<uint8>>> InitSegmentData;
		Metrics::FSegmentDownloadStats DownloadStats;
