...
namespace Electra
{
	...
	void FBufferedDataReader::CreateNewArea(int64 NumBytes, int64 FromOffset)
	{
		...
		{
			UE_LOGF(LogElectraBufferReader, VeryVerbose, "CreateNewArea(): clamp request to end of file size");
		...
		if (NewArea->Data)
		{
			...
			UE_LOGF(LogElectraBufferReader, VeryVerbose, "CreateNewArea(): read %lld bytes from offset %lld; total file size = %lld, received %lld bytes", (long long int)NumBytes, (long long int)FromOffset, (long long int)TotalSize, (long long int)NumRead);
			if (TotalSize >= 0)
			{
	...
	bool FBufferedDataReader::EnlargeCurrentAreaBy(int64 NumBytesToAdd)
	{
		...
		{
			UE_LOGF(LogElectraBufferReader, VeryVerbose, "EnlargeCurrentAreaBy(): clamp request of %lld to %lld at end of file", (long long int)NumBytesToAdd, (long long int)(TotalDataSize - (CurrentArea->StartOffset + CurrentArea->Size)));
		...
		{
			UE_LOGF(LogElectraBufferReader, VeryVerbose, "EnlargeCurrentAreaBy(): reaching into area of offset %lld, clamping", (long long int)Next->StartOffset);
		...
		if (CurrentArea->Data)
		{
			...
			UE_LOGF(LogElectraBufferReader, VeryVerbose, "CreateNewArea(): read %lld bytes from offset %lld; total file size = %lld, received %lld bytes", (long long int)NumBytesToAdd, (long long int)(CurrentArea->StartOffset + CurrentArea->Size), (long long int)TotalSize, (long long int)NumRead);
			if (TotalSize >= 0)
			{
				TotalDataSize = TotalSize;
			}
			if (NumRead >= 0)
			{
				...
				{
					UE_LOGF(LogElectraBufferReader, VeryVerbose, "EnlargeCurrentAreaBy(): reached the end of the file");
				...
				if (Next)
				{
					UE_LOGF(LogElectraBufferReader, VeryVerbose, "EnlargeCurrentAreaBy(): merging blocks from offsets %lld and %lld into one", (long long int)CurrentArea->StartOffset, (long long int)Next->StartOffset);
	...
	bool FBufferedDataReader::SeekTo(int64 AbsolutePosition)
	{
		...
		{
			UE_LOGF(LogElectraBufferReader, VeryVerbose, "SeekTo(): Seeking beyond the end of the file");
			return false;
		}
