...
public:
	...
	void SetMediaSamples(TSharedPtr<FMediaSamples, ESPMode::ThreadSafe> InMediaSamples) override;

	void Open(const FParamDict& Options) override;
	void Close() override;
	void Start() override;
	void Stop() override;
	void UpdatePlaybackPosition(FTimeValue InAbsolutePosition, FTimeValue InLocalPosition) override;

	FTimeValue GetStreamedDeliveryTimeOffset() override;
...
void FSubtitleDecoder::SetMediaSamples(TSharedPtr<FMediaSamples, ESPMode::ThreadSafe> InMediaSamples)
{
...
void FSubtitleDecoder::OnDecodedSubtitleReceived(ISubtitleDecoderOutputPtr DecodedSubtitle)
{
	if (DecodedSubtitle.IsValid())
	{
		...
		if (MediaSamples.IsValid())
		{
			...
			FUTF8ToTCHAR cnv((const ANSICHAR*)DecodedSubtitle->GetData().GetData(), DecodedSubtitle->GetData().Num());

			TSharedRef<FElectraSubtitleSample, ESPMode::ThreadSafe> SubtitleSample = MakeShared<FElectraSubtitleSample, ESPMode::ThreadSafe>();
			SubtitleSample->Timestamp.SetTimeAndIndexValue(ts.Time, ts.SequenceIndex); 
			SubtitleSample->Duration = DecodedSubtitle->GetDuration();
			SubtitleSample->Text = FString::ConstructFromPtrSize(cnv.Get(), cnv.Length());
			...
			!CurrentConfig.CodecInfo.Equals(AccessUnit->AUCodecData->ParsedInfo))
		{
			...
			if (AccessUnit && AccessUnit->AUCodecData.IsValid())
			{
				...
				if (PluginDecoder.IsValid())
				{
					...
					CurrentMediaSamples = MoveTemp(MediaSamples);
...
void FSubtitleDecoder::AUdataPushAU(FAccessUnit* AccessUnit)
{
	if (PluginDecoder.IsValid())
	{
		if (!AccessUnit->bIsDummyData)
		{
			...
			FTimeValue PTS(AccessUnit->PTS);
			FTimeValue Duration(AccessUnit->Duration);
			// Check if the duration is zero. This might happen with the last sample that is usually used to send
			// blank text to remove the subtitle from the screen. We cannot have zero duration for media samples
			// as this leaves their time interval empty, causing issues with timestamp overlap checks.
			if (Duration.IsZero())
			{
				// Set a short duration for overlap tests to work. Adjust the PTS down by that amount as well.
				Duration.SetFromMicroseconds(10);
				PTS -= Duration;
			}
			PluginDecoder->AddStreamedSubtitleData(TArray<uint8>((const uint8*)AccessUnit->AUData, (int32)AccessUnit->AUSize), PTS, Duration, Addtl);
		}
	}
}
...
void FSubtitleDecoder::FlushSamples()
{
	auto MediaSamples = CurrentMediaSamples;
	if (MediaSamples.IsValid())
	{
