...
namespace DASHUrlHelpers
{
	...
	bool GetAllCandidateBaseURLs(TArray<FDASHMediaURL>& OutCandidateURLs, TSharedPtrTS<const IDashMPDElement> StartingAt, const FDASHMediaURL& InMPDUrl, bool bIsDVBDash)
	{
		...
		{
			UE_LOGF(LogElectraPlayer, Error, "Could not parse the MPD URL when building element URL");
			return false;
		}
		...
		for(int32 nL=0; nL<HierarchicalBaseURLs.Num(); ++nL)
		{
			for(int32 nI=0; nI<Base.Num(); ++nI)
			{
				...
				for(int32 j=0; j<HierarchicalBaseURLs[nL].Num(); ++j)
				{
					...
					{
						UE_LOGF(LogElectraPlayer, Error, "Could not parse a <BaseURL> element while resolving URL");
						return false;
					}
					...
					if (nextUrl.IsAbsolute())
					{
						...
						{
							UE_LOGF(LogElectraPlayer, Error, "Invalid @priority found on <BaseURL> element while resolving URL");
							return false;
						}
						nxt.DVBpriority = bu->GetDVBPriority().GetWithDefault(1);
						if (bu->GetDVBWeight().IsSet() && bu->GetDVBWeight().Value() < 0)
						{
							UE_LOGF(LogElectraPlayer, Error, "Invalid @weight found on <BaseURL> element while resolving URL");
							return false;
						}
					...
					{
						UE_LOGF(LogElectraPlayer, Warning, "Inconsistent @availabilityTimeComplete found on hierarchical <BaseURL> elements while resolving URL");
	 ...
	 * At most one BaseURL per level is added to the output, with only the first BaseURL element being used.
	 * Results are added in hierarchy order, eg. MPD, Period, Adaptation, Representation.
	 */
	void GetAllHierarchyBaseURLs(IPlayerSessionServices* InPlayerSessionServices, TArray<TSharedPtrTS<const FDashMPD_BaseURLType>>& OutBaseURLs, TSharedPtrTS<const IDashMPDElement> StartingAt)
	{
		while(StartingAt.IsValid())
		{
			const TArray<TSharedPtrTS<FDashMPD_BaseURLType>>& ElementBaseURLs = StartingAt->GetBaseURLs();
	...
	if (XElem.IsValid())
	{
		...
		if (!XlinkHRef.Equals(XLinkResolveToZero))
		{
			...
			else
			{
				...
				DASHUrlHelpers::GetAllHierarchyBaseURLs(PlayerSessionServices, OutBaseURLs, XElem->GetParentElement());
			...
			for(int32 nRepr=0, nReprMax=MPDRepresentations.Num(); nRepr<nReprMax; ++nRepr)
			{
				...
				*/
				...
				if (SegmentTemplate.Num())
				{
					...
					DASHUrlHelpers::GetAllHierarchyBaseURLs(PlayerSessionServices, BaseURLElements, MPDRepresentation);
