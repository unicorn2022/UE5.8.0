...
namespace Metrics
{
	...
	struct FFrameStats
	{
		uint64 NumTotal = 0;
		uint64 NumDropped = 0;
		uint32 NumNewDrops = 0;
		uint32 AvgDropsPerInterval = 0;
		uint32 MinDropsPerInterval = 0;
		uint32 MaxDropsPerInterval = 0;
		uint32 DropIntervalMs = 0;
...
public:
	 ...
	 */
	virtual void ReportMediaMetadataChanged(TSharedPtrTS<FMetadataParser> Metadata) = 0;
	...
	virtual void ReportVideoFrameStats(const Metrics::FFrameStats& InFrameStats) = 0;
	virtual void ReportAudioFrameStats(const Metrics::FFrameStats& InFrameStats) = 0;
};
