...
namespace Electra
{
			...
			struct FRequestResponse
			{
				...
				int64 SizeRemaining() const
				{
					...
					if (bIsSubRangeRequest)
					{
						...
						{
							UE_LOGF(LogElectraHTTPManager, Error, "Unknown document size in sub ranged download");
						}
					}
					return 0;
				}
	...
	void FElectraHttpManager::AddRequest(TSharedPtrTS<FRequest> Request, bool bAutoRemoveWhenComplete)
	{
		...
		if (!bTerminate)
		{
			...
			HTTPStreamHandler->TriggerWorkSignal();
		}
	}
	...
	bool FElectraHttpManager::PrepareHTTPHandle(const FTimeValue& Now, FHandle* Handle, const TSharedPtrTS<FRequest>& Request, bool bIsFirstSetup)
	{
		...
		{
			UE_LOGF(LogElectraHTTPManager, Error, "FElectraHttpManager is not available, cannot create request.");
			return false;
		}
	}
			...
			else
			{
				...
				if (Handle)
				{
					...
					if (Handle->NeedsFetchOfUncachedParts())
					{
						...
						{
							Request->ConnectionInfo.StatusInfo.ErrorDetail.SetError(UEMEDIA_ERROR_INTERNAL).SetFacility(Facility::EFacility::HTTPReader).SetMessage(TEXT("HTTP request failed on ProcessRequest()"));
							RequestsCompleted.Enqueue(Request);
						}
					}
				}
	...
	bool FElectraHttpManager::StartHTTPManager()
	{
		...
		{
			UE_LOGF(LogElectraHTTPManager, Error, "Failed to create an FElectraHttpManager instance.");
			return false;
		}
	}
						...
						if (!ci.bWasAborted && Response->GetResponseData().HasAllDataBeenConsumed())
						{
							...
							else
							{
								...
								if (PrepareHTTPHandle(Now, Handle, Request, false))
								{
									...
									{
										ci.StatusInfo.ErrorDetail.SetError(UEMEDIA_ERROR_INTERNAL).SetFacility(Facility::EFacility::HTTPReader).SetMessage(TEXT("HTTP sub request failed on AddRequest()"));
									}
								}
								else
								{
	...
	void FElectraHttpManager::FFileStream::SetConnected(TSharedPtrTS<FRequest> Request)
	{
		if (!bIsConnected)
		{
			...
			Request->ConnectionInfo.ContentType = TEXT("application/octet-stream");
			Request->ConnectionInfo.EffectiveURL = Request->Parameters.URL;
