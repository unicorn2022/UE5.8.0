...
namespace Electra
{
	...
	public:
		...
		//! Returns the media format of this presentation.
		virtual EMediaFormatType GetMediaFormatType() const = 0;
		//! Returns true if the media playlist will be updated periodically, false if not.
		virtual bool ReloadsPlaylistPeriodically() const = 0;
		//! Returns true if there could be more than one track per kind to switch between.
		virtual bool SupportsMultipleTracksPerKind() const = 0;
		//! Returns true if during playback the order of tracks can change (like in DASH period transitions).
		virtual bool OrderOfTracksCanChangeDynamically() const = 0;
		...
		virtual void UpdateRunningMetaData(TSharedPtrTS<FMetadataParser> InUpdatedMetaData) = 0;
	...
	{
	public:
		static TSharedPtrTS<IPlayerMessage> Create(FTimeValue InValidFrom, TSharedPtrTS<FMetadataParser> InMetadata, bool bInTriggerInternalRefresh)
		{
		...
		TSharedPtrTS<FMetadataParser> GetMetadata() const
		{
			return Metadata;
		}
	...
	private:
		FPlaylistMetadataUpdateMessage(FTimeValue InValidFrom, TSharedPtrTS<FMetadataParser> InMetadata, bool bInTriggerInternalRefresh)
