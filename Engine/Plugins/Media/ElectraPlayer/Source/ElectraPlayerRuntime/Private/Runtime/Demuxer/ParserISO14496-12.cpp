...
#define RETURN_IF_ERROR(expr)											\
	...
	protected:
		UEMediaError ReadAndParseAttributes(FMP4ParseInfo* ParseInfo) override
		{
			...
			{
				UE_LOGF(LogElectraMP4Parser, Error, "ERROR: ElectraPlayer/FMP4BoxTX3GSampleEntry::ReadAndParseAttributes(): Not enough data for the 'ftab' box");
				return UEMEDIA_ERROR_FORMAT_ERROR;
			}
			...
			for(int32 nEntry=0; nEntry<EntryCount; ++nEntry)
			{
				...
				if (FontNameLength)
				{
					...
					{
						UE_LOGF(LogElectraMP4Parser, Error, "ERROR: ElectraPlayer/FMP4BoxTX3GSampleEntry::ReadAndParseAttributes(): Font name uses UTF16 which is not supported");
						return UEMEDIA_ERROR_FORMAT_ERROR;
					}
	...
	protected:
		UEMediaError ReadAndParseAttributes(FMP4ParseInfo* ParseInfo) override
		{
			...
			{
				UE_LOGF(LogElectraMP4Parser, Error, "ERROR: ElectraPlayer/FMP4BoxWVTTSampleEntry::ReadAndParseAttributes(): Required 'vttC' box not found!");
				return UEMEDIA_ERROR_FORMAT_ERROR;
			}
			return Error;
		}
				...
				for(uint32 i = 0; i < SampleCount; ++i)
				{
					...
					if (Flags & 0x800)
					{
						if (Version == 0)
						{
							...
							{
								UE_LOGF(LogElectraMP4Parser, Error, "WARNING: ElectraPlayer/FMP4BoxTRUN::ReadAndParseAttributes(): Version 0 time value cannot be represented as a signed value");
								return UEMEDIA_ERROR_FORMAT_ERROR;
							}
		...
		else
		{
			while(1)
			{
				...
				*((uint32*)&boxName) = MEDIA_TO_BIG_ENDIAN(BoxType);
				//UE_LOGF(LogElectraMP4Parser, Log, "Next box: '%ls' %lld B @ %lld (Parent: '%ls')", *FMP4Box::GetBoxNameString(boxName), (long long int)BoxSize, (long long int)BoxStartOffset, ParentBox ? *ParentBox->GetBoxName() : TEXT("****"));
#endif
				...
				if (ParseContinuation == IParserISO14496_12::IBoxCallback::EParseContinuation::Continue)
				{
					...
					switch(BoxType)
					{
						...
						default:
#if MEDIA_DEBUG_HAS_BOX_NAMES
//							UE_LOGF(LogElectraMP4Parser, Log, "Ignoring mp4 box: '%ls' %lld B @ %lld", *FMP4Box::GetBoxNameString(boxName), (long long int)BoxSize, (long long int)BoxStartOffset);
#endif
							NextBox = new FMP4BoxIgnored(BoxType, BoxSize, BoxStartOffset, BoxDataOffset, true);
							break;
					}
				...
				switch(SampleEntry->GetType())
				{
					case FMP4Box::kBox_avcC:
					{
						...
						if (AVCCBox->GetDecoderConfigurationRecord().IsSet())
						{
							...
							Track->CodecInformation.SetCodec4CC(InAvcType == 1 ? Utils::Make4CC('a','v','c','1') : Utils::Make4CC('a','v','c','3'));
							Track->CodecInformation.SetStreamLanguageTag(Track->GetLanguageTag());
							Track->CodecInformation.SetDecoderConfigRecord(Track->CodecSpecificDataAVC.GetRawData());
							Track->CodecInformation.SetCodecSpecificData(Track->CodecSpecificDataAVC.GetCodecSpecificData());
				...
				switch(SampleEntry->GetType())
				{
					case FMP4Box::kBox_hvcC:
					{
						...
						if (HVCCBox->GetDecoderConfigurationRecord().IsSet())
						{
							...
							Track->CodecInformation.SetCodec4CC(Utils::Make4CC('h','e','v','1'));
							Track->CodecInformation.SetStreamLanguageTag(Track->GetLanguageTag());
							...
							Track->CodecInformation.SetDecoderConfigRecord(Track->CodecSpecificDataHEVC.GetRawData());
							Track->CodecInformation.SetCodecSpecificData(Track->CodecSpecificDataHEVC.GetCodecSpecificData());
							...
							else if (!Track->CodecSpecificDataHEVC.GetParsedSequenceParameterSets().IsEmpty())
							{
								...
								Track->CodecInformation.SetCodec4CC(Utils::Make4CC('h','v','c','1'));
								bGotVideoFormat = true;
							}
							else
							{
		...
		auto HandleESDS = [PlayerSession, Track](const FMP4BoxAudioSampleEntry* AudioSampleEntry, const FMP4BoxESDS* ESDSBox, bool& bGotAudioFormat, bool& bIsSupported) -> bool
		{
			...
			if (bOk)
			{
					...
					Track->CodecSpecificDataMP4A.GetStreamType() == MPEG::FESDescriptor::FStreamType::AudioStream)
				{
					...
					Track->CodecInformation.SetCodec4CC(Utils::Make4CC('m','p','4','a'));
					Track->CodecInformation.SetStreamLanguageTag(Track->GetLanguageTag());
	...
	UEMediaError FParserISO14496_12::ParseTX3GSampleType(IPlayerSessionServices* PlayerSession, FTrack* Track, const FMP4Box* SampleBox)
	{
		...
		if (ISubtitleDecoder::IsSupported(TEXT(""), TEXT("tx3g")))
		{
			...
			Track->CodecInformation.SetCodec4CC(Utils::Make4CC('t','x','3','g'));
	...
	UEMediaError FParserISO14496_12::ParseWVTTSampleType(IPlayerSessionServices* PlayerSession, FTrack* Track, const FMP4Box* SampleBox)
	{
		...
		if (ISubtitleDecoder::IsSupported(TEXT(""), TEXT("wvtt")))
		{
			...
			Track->CodecInformation.SetCodec4CC(Utils::Make4CC('w','v','t','t'));
			Track->CodecInformation.SetCodecSpecificData(Track->CodecSpecificDataRAW);
			Track->CodecInformation.SetStreamLanguageTag(Track->GetLanguageTag());
	...
	UEMediaError FParserISO14496_12::ParseSTPPSampleType(IPlayerSessionServices* PlayerSession, FTrack* Track, const FMP4Box* SampleBox)
	{
		...
		if (ISubtitleDecoder::IsSupported(TEXT(""), TEXT("stpp")))
		{
			...
			Track->CodecInformation.SetCodec4CC(Utils::Make4CC('s','t','p','p'));
			Track->CodecInformation.SetCodecSpecificData(Track->CodecSpecificDataRAW);
			Track->CodecInformation.SetStreamLanguageTag(Track->GetLanguageTag());
		...
		if (Track && STSDBox)
		{
			...
			switch(STSDBox->GetSampleType())
			{
				case FMP4BoxSTSD::ESampleType::Video:
				{
					...
					CodecInformation.UpdateFromExtraBoxes();
								...
								if (ee.MediaRateInteger == 1 && ee.MediaRateFraction == 0)
								{
									...
									if (ee.MediaTime == -1)
									{
										...
										else if (!bHaveEmptyEditWarned)
										{
											...
											UE_LOGF(LogElectraMP4Parser, Warning, "Found another empty edit in 'elst' box, ignoring");
										}
									}
									else if (ee.MediaTime >= 0)
									{
										...
										else if (!bHaveOffsetEditWarned)
										{
											...
											UE_LOGF(LogElectraMP4Parser, Warning, "Found another media edit in 'elst' box, ignoring");
										}
									}
								}
							}
