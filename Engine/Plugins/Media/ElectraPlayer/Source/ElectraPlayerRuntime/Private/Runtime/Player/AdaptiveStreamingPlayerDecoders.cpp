...
bool FAdaptiveStreamingPlayer::GetHighestVideoProfileResolution(FStreamCodecInformation& OutStreamInfo, TSharedPtrTS<const FBufferSourceInfo> InBufferSource, const FTimeValue& AtTime, const FStreamCodecInformation& InForCodec)
{
	if (!InBufferSource)
	{
		return false;
	...
	const FString& PeriodID(InBufferSource->PeriodID);
	if (PeriodID.Len())
	{
		for(int32 i=0; i<ActivePeriods.Num(); ++i)
		{
			if (ActivePeriods[i].ID.Equals(PeriodID))
			{
	...
	if (Asset.IsValid())
	{
		TSharedPtrTS<IPlaybackAssetAdaptationSet> VideoAdaptationSet;
		if (InBufferSource->HardIndex >= 0 && InBufferSource->HardIndex < Asset->GetNumberOfAdaptationSets(EStreamType::Video))
		{
			VideoAdaptationSet = Asset->GetAdaptationSetByTypeAndIndex(EStreamType::Video, InBufferSource->HardIndex);
		}
		if (VideoAdaptationSet && VideoAdaptationSet->GetNumberOfRepresentations())
		{
			TArray<FStreamCodecInformation> VideoCodecInfos;
			for(int32 i=0, iMax=VideoAdaptationSet->GetNumberOfRepresentations(); i<iMax; ++i)
			{
				VideoCodecInfos.Push(VideoAdaptationSet->GetRepresentationByIndex(i)->GetCodecInformation());
			}
			// The representations could be a mix of profile and resolutions, although not very likely.
			// We consider profile, level and resolution separately, which may produce an amalgamation of values that
			// does not appear as such on any particular stream. We do need to know the worst profile and resolution
			// that can occur.
			OutStreamInfo = VideoCodecInfos[0];
			int32 Profile = VideoCodecInfos[0].GetProfile();
			int32 Level = VideoCodecInfos[0].GetProfileLevel();
			for(int32 i=1; i<VideoCodecInfos.Num(); ++i)
			{
				if (VideoCodecInfos[i].GetResolution().Width * VideoCodecInfos[i].GetResolution().Height > OutStreamInfo.GetResolution().Width * OutStreamInfo.GetResolution().Height)
				{
					OutStreamInfo = VideoCodecInfos[i];
				}
				if (VideoCodecInfos[i].GetProfile() > Profile)
				{
					Profile = VideoCodecInfos[i].GetProfile();
				}
				if (VideoCodecInfos[i].GetProfileLevel() > Level)
				{
					Level = VideoCodecInfos[i].GetProfileLevel();
				}
			}
			OutStreamInfo.SetProfile(Profile);
			OutStreamInfo.SetProfileLevel(Level);
			return true;
		}
	}
	return false;
	...
	if (type == EStreamType::Video)
	{
		if (VideoDecoder.Decoder == nullptr)
		{
			...
			if (AccessUnit)
			{
				...
				bOk = GetHighestVideoProfileResolution(HighestStream, AccessUnit->BufferSourceInfo, DecodeTime, VideoDecoder.CurrentCodecInfo);
		...
		if (SubtitleDecoder.Decoder == nullptr)
		{
			...
			if (AccessUnit)
			{
				...
				if (SubtitleDecoder.Decoder)
				{
					...
					SubtitleDecoder.Decoder->SetMediaSamples(CurrentMediaSamples);
...
void FAdaptiveStreamingPlayer::FeedDecoder(EStreamType Type, IAccessUnitBufferInterface* Decoder, bool bHandleUnderrun)
{
	...
	// If there is a seek pending we do not feed the decoders since their output will be discarded
	// by the media sample queues anyway due to non-matching sequence indices.
	// This has the side effect of quickly draining our buffers, possibly resulting in a buffer
	// underrun that will not even apply due to the upcoming seek.
	if (SeekVars.HasPendingSeek())
	{
		return;
	}
		...
		if (PeekedAU)
		{
			...
			if (CurrentCodecInfo && PeekedAU->AUCodecData.IsValid())
			{
					...
					(((*LastBufferSourceInfo)->PeriodID != PeekedAU->BufferSourceInfo->PeriodID) || ((*LastBufferSourceInfo)->PeriodAdaptationSetID != PeekedAU->BufferSourceInfo->PeriodAdaptationSetID)))
				{
					...
					FStreamCodecInformation HighestStream;
					if (GetHighestVideoProfileResolution(HighestStream, PeekedAU->BufferSourceInfo, PeekedAU->EarliestPTS.IsValid() ? PeekedAU->EarliestPTS : PeekedAU->PTS, PeekedAU->AUCodecData->ParsedInfo))
					{
