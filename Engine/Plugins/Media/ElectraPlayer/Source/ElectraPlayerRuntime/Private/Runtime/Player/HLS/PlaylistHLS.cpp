...
void FActiveHLSPlaylist::FTimelineMediaAsset::TransformIntoReplayEvent()
{
	...
	if (bHasProgramDateTime)
	{
		UE_LOGF(LogElectraPlayer, Warning, "HLS stream uses program-date-time, cannot use as replay event.");
		return;
	}
	...
	for(int32 i=0,iMax=frgs.Num(); i<iMax; ++i)
	{
		if (frgs[i].Name.Equals(CustomOptions::Custom_EpicUTCNow))
		{
			...
			{
				UE_LOGF(LogElectraPlayer, Warning, "Could not parse the custom UTC now value, disabling replay event.");
				return;
			}
			break;
		}
		...
		{
			UE_LOGF(LogElectraPlayer, Warning, "Fetching UTC time from a custom URL for a replay event is not supported with HLS.");
		}
	}
	...
	if (Params.Num())
	{
		...
		ReplayEvent.BaseStartTime = Start;
...
EMediaFormatType FActiveHLSPlaylist::GetMediaFormatType() const
{
	return EMediaFormatType::HLS;
}

bool FActiveHLSPlaylist::ReloadsPlaylistPeriodically() const
{
	// This is true even for VoD since each variant has its own playlist that gets loaded just in time.
	return true;
}

bool FActiveHLSPlaylist::SupportsMultipleTracksPerKind() const
{
	return true;
}

bool FActiveHLSPlaylist::OrderOfTracksCanChangeDynamically() const
{
	return false;
}
...
void FActiveHLSPlaylist::UpdateRunningMetaData(TSharedPtrTS<FMetadataParser> InUpdatedMetaData)
{
	// Not used with HLS
}
...
IManifest::FResult FActiveHLSPlaylist::FPlayPeriod::GetStartingSegment(TSharedPtrTS<IStreamSegment>& OutSegment, const FPlayerSequenceState& InSequenceState, const FPlayStartPosition& InStartPosition, ESearchType InSearchType)
{
	...
	for(int32 nStr=0; nStr<LoadReq.Num(); ++nStr)
	{
		...
		{
			return IManifest::FResult(IManifest::FResult::EType::Error).SetErrorDetail(TimelineMediaAsset->GetLastError());
		}
