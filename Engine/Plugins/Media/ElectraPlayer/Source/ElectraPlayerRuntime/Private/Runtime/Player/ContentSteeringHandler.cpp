	...
	*/
	...
	if (!InitialParams.bAllowAnyPathwayNames)
	{
		for(auto& It : CurrentCDNPriorities)
		{
			...
			{
				UE_LOGF(LogElectraPlayer, Error, "Invalid initial pathway name \"%ls\"", *It);
				return false;
			}
		}
	}
	if (InitialParams.CustomFirstCDNPrioritization.Len())
	{
		...
		*/
		...
		{
			UE_LOGF(LogElectraPlayer, Error, "Invalid custom initial CDN selection parameters");
			return false;
		}
...
void FContentSteeringHandler::UpdateWithSteeringServerResponse(const FString& InResponse, int32 InHTTPStatusCode, const TArray<HTTP::FHTTPHeader>& InResponseHeaders)
{
	...
	if (!InResponse.IsEmpty() && InHTTPStatusCode == 200)
	{
		...
		if (bIsValid)
		{
			...
			{
				UE_LOGF(LogElectraPlayer, Error, "Steering manifest is missing mandatory VERSION field");
				bIsValid = false;
			}
			if (VERSION == 1)
			{
				...
				{
					UE_LOGF(LogElectraPlayer, Error, "Steering manifest is missing mandatory TTL field");
				...
				{
					UE_LOGF(LogElectraPlayer, Error, "Steering manifest has bad TTL field of %d seconds", TTL);
					bIsValid = false;
				}
				...
				{
					UE_LOGF(LogElectraPlayer, Error, "Steering manifest is missing mandatory PATHWAY-PRIORITY field");
					bIsValid = false;
				}
				...
				if (SteerParams->TryGetArrayField(TEXT("PATHWAY-CLONES"), PATHWAY_CLONES))
				{
					for(auto& pwclnv : *PATHWAY_CLONES)
					{
						...
						{
							UE_LOGF(LogElectraPlayer, Error, "Steering manifest PATHWAY-CLONES array element is not an object");
							bIsValid = false;
							continue;
						}
						...
						{
							UE_LOGF(LogElectraPlayer, Error, "Steering manifest is missing mandatory BASE-ID field in PATHWAY-CLONES");
							bIsValid = false;
							continue;
						}
						if (!(*pwcln)->TryGetStringField(TEXT("ID"), CloneEntry.Id))
						{
							UE_LOGF(LogElectraPlayer, Error, "Steering manifest is missing mandatory ID field in PATHWAY-CLONES");
							bIsValid = false;
							continue;
						}
						...
						{
							UE_LOGF(LogElectraPlayer, Error, "Steering manifest is missing mandatory URI-REPLACEMENT field in PATHWAY-CLONES");
							bIsValid = false;
							continue;
						}
			...
			else
			{
				if (VERSION > 1)
				{
					UE_LOGF(LogElectraPlayer, Error, "Steering manifest VERSION %d is not yet understood", VERSION);
				}
				bIsValid = false;
