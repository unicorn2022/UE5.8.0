...
void FPlaylistReaderMP4::WorkerThread()
{
	...
	const TArray<uint32> ReadBoxes { MP4Utilities::MakeBoxAtom('f','t','y','p'), MP4Utilities::MakeBoxAtom('m','o','o','v') };
	...
	else if (bGotBoxes && !bAbort && !bHasErrored)
	{
		...
		if (BoxInfos.ContainsByPredicate([InType=MP4Utilities::MakeBoxAtom('f','t','y','p')](const UtilsMP4::FMP4RootBoxLocator::FBoxInfo& InBox){return InBox.Type == InType;}) &&
			BoxInfos.ContainsByPredicate([InType=MP4Utilities::MakeBoxAtom('m','o','o','v')](const UtilsMP4::FMP4RootBoxLocator::FBoxInfo& InBox){return InBox.Type == InType;}))
		{
			if (BoxInfos.ContainsByPredicate([InType=MP4Utilities::MakeBoxAtom('m','d','a','t')](const UtilsMP4::FMP4RootBoxLocator::FBoxInfo& InBox){return InBox.Type == InType;}))
			{
			...
			ParseBuffer = BoxInfos.FindByPredicate([InType=MP4Utilities::MakeBoxAtom('f','t','y','p')](const UtilsMP4::FMP4RootBoxLocator::FBoxInfo& InBox){return InBox.Type == InType;})->DataBuffer;
			ParsePos = 0;
			...
			{
				ParseBuffer = BoxInfos.FindByPredicate([InType=MP4Utilities::MakeBoxAtom('m','o','o','v')](const UtilsMP4::FMP4RootBoxLocator::FBoxInfo& InBox){return InBox.Type == InType;})->DataBuffer;
				ParsePos = 0;
				ParseBufferSize = ParseBuffer->Num();
