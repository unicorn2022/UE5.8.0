...
static TAutoConsoleVariable<bool> CVarElectraRetainPools(
	TEXT("Electra.RetainPools"),
	false,
	TEXT("Retains video and audio sample pools in player for reuse.\n")
	TEXT(" false: don't retain, create new pools for each instance; true: retain pools"),
	ECVF_Default);
	...
	, ReportSubtitlesMetricsDelegate(InReportSubtitlesFileMetricsDelegate)
{
	...
	bRetainPoolsForReuse = CVarElectraRetainPools.GetValueOnAnyThread();
	if (bRetainPoolsForReuse)
	{
...
FElectraPlayer::~FElectraPlayer()
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] ~FElectraPlayer() finished.", PlayerUniqueID);
...
bool FElectraPlayer::OpenInternal(const FString& InUrl, const IMediaOptions* InOptions, const FMediaPlayerOptions* InPlayerOptions)
{
	...
	{
		UE_LOGF(LogElectraPlayer, Error, "[%u] IMediaPlayer::Open: Options == nullptr", PlayerUniqueID);
	...
	UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::Open", PlayerUniqueID);
	...
	if (InPlayerOptions)
	{
		if (InPlayerOptions->Loop != EMediaPlayerOptionBooleanOverride::UseMediaPlayerSetting)
		{
			...
			UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::Open: Setting initial loop state to %ls", PlayerUniqueID, bEnableLooping ? TEXT("enabled") : TEXT("disabled"));
		}
		if (InPlayerOptions->SeekTimeType != EMediaPlayerOptionSeekTimeType::Ignored)
		{
			// If an explicit time is set we only use it if it is not zero. Zero is the default value and most of the time means nothing in particular.
			// Playback will start at the beginning for VoD anyway and if this is an event using zero causes problems
			if (InPlayerOptions->SeekTime > FTimespan::Zero())
			{
				PlaystartOptions.TimeOffset = InPlayerOptions->SeekTime;
				UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::Open: Setting initial seek time to %.3f seconds", PlayerUniqueID, InPlayerOptions->SeekTime.GetTotalSeconds());
			}
			else
			{
				UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::Open: Setting initial seek time of zero will be ignored as it implies the beginning.", PlayerUniqueID);
			}
		}
		if (InPlayerOptions->TrackSelection == EMediaPlayerOptionTrackSelectMode::UseLanguageCodes)
		{
			if (InPlayerOptions->TracksByLanguage.Video.Len())
			{
				...
				UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Asking for initial video language \"%ls\"", PlayerUniqueID, *InPlayerOptions->TracksByLanguage.Video);
			}
			if (InPlayerOptions->TracksByLanguage.Audio.Len())
			{
				...
				UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Asking for initial audio language \"%ls\"", PlayerUniqueID, *InPlayerOptions->TracksByLanguage.Audio);
			}
			if (InPlayerOptions->TracksByLanguage.Subtitle.Len())
			{
				...
				UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Asking for initial subtitle language \"%ls\"", PlayerUniqueID, *InPlayerOptions->TracksByLanguage.Subtitle);
			}
		}
	...
	if (bNoPreloading)
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: No preloading after opening media", PlayerUniqueID);
	}
	...
	if (InitialStreamBitrate > 0)
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Using initial bitrate of %d bits/second", PlayerUniqueID, (int32)InitialStreamBitrate);
	}
	...
	if (MediaMimeType.Len())
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Setting media mime type to \"%ls\"", PlayerUniqueID, *MediaMimeType);
	}
	...
	if (MaxVerticalHeight > 0)
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Limiting vertical resolution to %d for all streams", PlayerUniqueID, (int32)MaxVerticalHeight);
	}
	...
	if (MaxVerticalHeightAt60 > 0)
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Limiting vertical resolution to %d for streams >30fps", PlayerUniqueID, (int32)MaxVerticalHeightAt60);
	}
	...
	if (LiveEdgeDistanceForNormalPresentation > 0.0)
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Setting distance to live edge for normal presentations to %.3f seconds", PlayerUniqueID, LiveEdgeDistanceForNormalPresentation);
	}
	...
	if (bThrowErrorWhenRebuffering)
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: Throw playback error when rebuffering", PlayerUniqueID);
	}
	...
	if (CDNHTTPStatusDenyStream.Len())
	{
		...
		if (HTTPStatus > 0 && HTTPStatus < 1000)
		{
			...
			UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaPlayer::Open: CDN HTTP status %d will deny a stream permanently", PlayerUniqueID, HTTPStatus);
		}
	}
	...
	{
		UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::Open: Limiting max resolution to %d", PlayerUniqueID, (int32)MaxVerticalStreamResolution);
	...
	{
		UE_LOGF(LogElectraPlayer, Log, "[%u] Limiting max streaming bandwidth to %d bps", PlayerUniqueID, (int32)MaxBandwidthForStreaming);
	...
	if (!bRetainPoolsForReuse)
	{
		CreateParams.OutputTexturePool = MakeShareable(new FElectraTextureSamplePool);
		CreateParams.OutputAudioSamplePool = MakeShareable(new FElectraAudioSamplePool);
	#if PLATFORM_ANDROID
		OutputTexturePoolDecoderID = FElectraDecodersPlatformResources::RegisterOutputTexturePool(CreateParams.OutputTexturePool);
	#endif
	}
	else
	{
		CreateParams.OutputTexturePool = OutputTexturePool;
		CreateParams.OutputAudioSamplePool = OutputAudioSamplePool;
	}
	...
	UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::Open(%ls)", PlayerUniqueID, *SanitizeMessage(MediaUrl));
...
void FElectraPlayer::CloseInternal()
{
	...
	UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::Close()", PlayerUniqueID);
	...
	if (Player->AdaptivePlayer.IsValid())
	{
		if (MediaPlayerEventReceiver.IsValid())
		{
			...
			Player->AdaptivePlayer->RemoveAEMSReceiver(MediaPlayerEventReceiver, TEXT("*"), TEXT(""), IAdaptiveStreamingPlayerAEMSReceiver::EDispatchMode::OnReceive);
	...
	FInternalPlayerImpl::DoCloseAsync(MoveTemp(Player), PlayerUniqueID, AsyncResourceReleaseNotification, bRetainPoolsForReuse==false, OutputTexturePoolDecoderID);
}

void FElectraPlayer::FInternalPlayerImpl::DoCloseAsync(TSharedPtr<FInternalPlayerImpl, ESPMode::ThreadSafe>&& InPlayer, uint32 InPlayerID, TSharedPtr<IAsyncResourceReleaseNotification, ESPMode::ThreadSafe> InAsyncResourceReleaseNotification, bool bReleasePools, int64 TexturePoolDecoderID)
{
	TFunction<void()> CloseTask = [InPlayer, InPlayerID, InAsyncResourceReleaseNotification, bReleasePools, TexturePoolDecoderID]()
	{
		...
		{
		#if PLATFORM_ANDROID
			FElectraDecodersPlatformResources::UnregisterOutputTexturePool(TexturePoolDecoderID);
		#endif
		}

		double TimeCloseEnded = FPlatformTime::Seconds();
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] DoCloseAsync() finished after %.3f msec!", InPlayerID, (TimeCloseEnded - TimeCloseBegan) * 1000.0);
	...
	if (Player.IsValid() && PlayerState.State != EMediaState::Error)
	{
		...
		if (VerticalResoLimit.IsValid())
		{
			...
			if (NewVerticalStreamResolution != PlaystartOptions.MaxVerticalStreamResolution.Get(0))
			{
				...
				UE_LOGF(LogElectraPlayer, Log, "[%u] Limiting max vertical resolution to %d", PlayerUniqueID, (int32)NewVerticalStreamResolution);
		...
		if (BandwidthLimit.IsValid())
		{
			...
			if (NewBandwidthForStreaming != PlaystartOptions.MaxBandwidthForStreaming.Get(0))
			{
				...
				UE_LOGF(LogElectraPlayer, Log, "[%u] Limiting max streaming bandwidth to %d bps", PlayerUniqueID, (int32)NewBandwidthForStreaming);
...
void FElectraPlayer::HandleDeferredPlayerEvents()
{
	...
	while(DeferredPlayerEvents.Dequeue(Event))
	{
		switch(Event->Type)
		{
			...
			{
				FPlayerMetricEvent_DroppedFrame* Ev = static_cast<FPlayerMetricEvent_DroppedFrame*>(Event.Get());
				HandlePlayerEventDroppedVideoFrame(Ev->FrameStats);
				break;
			}
			case FPlayerMetricEventBase::EType::DroppedAudioFrame:
			{
				FPlayerMetricEvent_DroppedFrame* Ev = static_cast<FPlayerMetricEvent_DroppedFrame*>(Event.Get());
				HandlePlayerEventDroppedAudioFrame(Ev->FrameStats);
				break;
			}
			default:
			{
...
void FElectraPlayer::HandlePlayerEventOpenSource(const FString& URL)
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] Opening stream at \"%ls\"", PlayerUniqueID, *SanitizeMessage(URL));
...
{
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] Received main playlist from \"%ls\"", PlayerUniqueID, *SanitizeMessage(EffectiveURL));
...
void FElectraPlayer::HandlePlayerEventReceivedPlaylists()
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] Received initial stream playlists", PlayerUniqueID);
	...
	if (NumTracksVideo)
	{
		for(int32 i=0; i<VideoStreamMetaData[0].StreamDetails.Num(); ++i)
		{
			...
			StatisticsLock.Unlock();
			UE_LOGF(LogElectraPlayer, Verbose, "[%u] Found %d * %d video stream at bitrate %d", PlayerUniqueID,
	...
	if (NumTracksAudio)
	{
		for(int32 i=0; i<AudioStreamMetaData[0].StreamDetails.Num(); ++i)
		{
			...
			StatisticsLock.Unlock();
			UE_LOGF(LogElectraPlayer, Verbose, "[%u] Found audio stream at bitrate %d", PlayerUniqueID,
...
void FElectraPlayer::HandlePlayerEventLicenseKey(const Electra::Metrics::FLicenseKeyStats& LicenseKeyStats)
{
	...
	{
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] License key obtained", PlayerUniqueID);
		FScopeLock Lock(&StatisticsLock);
		Statistics.AddMessageToHistory(TEXT("Obtained license key"));
	}
	else
	{
		UE_LOGF(LogElectraPlayer, Log, "[%u] License key error \"%ls\"", PlayerUniqueID, *LicenseKeyStats.FailureReason);
		FScopeLock Lock(&StatisticsLock);
...
void FElectraPlayer::HandlePlayerEventBufferingStart(Electra::Metrics::EBufferingReason BufferingReason)
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] %ls", PlayerUniqueID, *Msg);
...
void FElectraPlayer::HandlePlayerEventBufferingEnd(Electra::Metrics::EBufferingReason BufferingReason)
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] %ls buffering ended after %.3fs", PlayerUniqueID, Electra::Metrics::GetBufferingReasonString(BufferingReason), BufferingDuration);
...
{
//	FScopeLock Lock(&StatisticsLock);
	UE_LOGF(LogElectraPlayer, VeryVerbose, "[%u] Observed bandwidth of %lld Kbps; throughput = %lld Kbps; latency = %.3fs", PlayerUniqueID, EffectiveBps/1000, ThroughputBps/1000, LatencyInSeconds);
}
...
void FElectraPlayer::HandlePlayerEventSegmentDownload(const Electra::Metrics::FSegmentDownloadStats& SegmentDownloadStats)
{
	...
	{
		UE_LOGF(LogElectraPlayer, VeryVerbose, "[%u] Downloaded %ls segment at bitrate %d: Playback time = %.3fs, duration = %.3fs, download time = %.3fs, URL=%ls \"%ls\"", PlayerUniqueID, Electra::GetStreamTypeName(SegmentDownloadStats.StreamType), SegmentDownloadStats.Bitrate, SegmentDownloadStats.PresentationTime, SegmentDownloadStats.Duration, SegmentDownloadStats.TimeToDownload, *SegmentDownloadStats.Range, *SanitizeMessage(SegmentDownloadStats.URL.URL));
	}
	...
	{
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] %ls segment download issue (%ls): retry:%d, success:%d, aborted:%d, filler:%d", PlayerUniqueID, Electra::Metrics::GetSegmentTypeString(SegmentDownloadStats.SegmentType), *SegmentDownloadStats.FailureReason, SegmentDownloadStats.RetryNumber, SegmentDownloadStats.bWasSuccessful, SegmentDownloadStats.bWasAborted, SegmentDownloadStats.bInsertedFillerData);
...
void FElectraPlayer::HandlePlayerEventVideoQualityChange(int32 NewBitrate, int32 PreviousBitrate, bool bIsDrasticDownswitch)
{
	...
	if (bIsDrasticDownswitch)
	{
		UE_LOGF(LogElectraPlayer, Log, "[%u] Player switched video quality drastically down to %d bps from %d bps. %d upswitches, %d downswitches (%d drastic ones)", PlayerUniqueID, NewBitrate, PreviousBitrate, Statistics.NumVideoQualityUpswitches, Statistics.NumVideoQualityDownswitches, Statistics.NumVideoQualityDrasticDownswitches);
	}
	else
	{
		UE_LOGF(LogElectraPlayer, Log, "[%u] Player switched video quality to %d bps from %d bps. %d upswitches, %d downswitches (%d drastic ones)", PlayerUniqueID, NewBitrate, PreviousBitrate, Statistics.NumVideoQualityUpswitches, Statistics.NumVideoQualityDownswitches, Statistics.NumVideoQualityDrasticDownswitches);
	}
...
void FElectraPlayer::HandlePlayerEventAudioQualityChange(int32 NewBitrate, int32 PreviousBitrate, bool bIsDrasticDownswitch)
{
	...
	{
		UE_LOGF(LogElectraPlayer, Log, "[%u] Player switched audio quality to %d bps from %d bps. %d upswitches, %d downswitches (%d drastic ones)", PlayerUniqueID, NewBitrate, PreviousBitrate, Statistics.NumAudioQualityUpswitches, Statistics.NumAudioQualityDownswitches, Statistics.NumAudioQualityDrasticDownswitches);
	}
...
void FElectraPlayer::HandlePlayerEventCodecFormatChange(const Electra::FStreamCodecInformation& NewDecodingFormat)
{
	if (NewDecodingFormat.IsVideoCodec())
	{
		...
		CurrentlyActiveVideoStreamFormat.Bitrate = NewDecodingFormat.GetBitrate();
		CurrentlyActiveVideoStreamFormat.Resolution.X = NewDecodingFormat.GetResolution().Width;
		CurrentlyActiveVideoStreamFormat.Resolution.Y = NewDecodingFormat.GetResolution().Height;
		CurrentlyActiveVideoStreamFormat.FrameRate = NewDecodingFormat.GetFrameRate().IsValid() ? NewDecodingFormat.GetFrameRate().GetAsDouble() : 0.0;
	}
}
...
void FElectraPlayer::HandlePlayerEventPrerollStart()
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] Player starts prerolling to warm decoders and renderers", PlayerUniqueID);
...
void FElectraPlayer::HandlePlayerEventPrerollEnd()
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] Player prerolling complete", PlayerUniqueID);
	Statistics.LastState = "Ready";
...
void FElectraPlayer::HandlePlayerEventPlaybackStart()
{
	...
	if (Player.IsValid())
	{
		...
		UE_LOGF(LogElectraPlayer, Log, "[%u] Playback started at play position %.3f", PlayerUniqueID, PlayPos);
...
void FElectraPlayer::HandlePlayerEventPlaybackPaused()
{
	...
	if (Player.IsValid())
	{
		...
		double PlayPos = Player->AdaptivePlayer->GetPlayPosition().GetAsSeconds();
		UE_LOGF(LogElectraPlayer, Log, "[%u] Playback paused at play position %.3f", PlayerUniqueID, PlayPos);
...
void FElectraPlayer::HandlePlayerEventPlaybackResumed()
{
	...
	if (Player.IsValid())
	{
		...
		Statistics.LastState = "Playing";
		double PlayPos = Player->AdaptivePlayer->GetPlayPosition().GetAsSeconds();
		UE_LOGF(LogElectraPlayer, Log, "[%u] Playback resumed at play position %.3f", PlayerUniqueID, PlayPos);
...
void FElectraPlayer::HandlePlayerEventPlaybackEnded()
{
	...
	if (Player.IsValid())
	{
		...
		UE_LOGF(LogElectraPlayer, Log, "[%u] Playback reached end at play position %.3f", PlayerUniqueID, PlayPos);
...
void FElectraPlayer::HandlePlayerEventJumpInPlayPosition(const Electra::FTimeValue& ToNewTime, const Electra::FTimeValue& FromTime, Electra::Metrics::ETimeJumpReason TimejumpReason)
{
	...
	if (Player.IsValid())
	{
		...
		if (TimejumpReason == Electra::Metrics::ETimeJumpReason::UserSeek)
		{
			...
			UE_LOGF(LogElectraPlayer, Verbose, "[%u] Jump in play position from %.3f to %.3f", PlayerUniqueID, FromTime.GetAsSeconds(), ToNewTime.GetAsSeconds());
		}
		else if (TimejumpReason == Electra::Metrics::ETimeJumpReason::Looping)
		{
			...
			UE_LOGF(LogElectraPlayer, Log, "[%u] Looping (%lld) from %.3f to %.3f", PlayerUniqueID, loopState.Count, FromTime.GetAsSeconds(), ToNewTime.GetAsSeconds());
...
void FElectraPlayer::HandlePlayerEventPlaybackStopped()
{
	...
	if (Player.IsValid())
	{
		...
		UE_LOGF(LogElectraPlayer, Log, "[%u] Playback stopped. Last play position %.3f", PlayerUniqueID, PlayPos);
...
{
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] Seek completed", PlayerUniqueID);
	MediaStateOnSeekFinished();
}

void FElectraPlayer::HandlePlayerMediaMetadataChanged(const TSharedPtrTS<Electra::FMetadataParser>& InMetadata)
{
	if (InMetadata.IsValid())
	{
		TSharedPtr<TMap<FString, TArray<TUniquePtr<IMediaMetadataItem>>>, ESPMode::ThreadSafe> PlayerMeta = InMetadata->GetMediaStreamMetadata();
		if (PlayerMeta.IsValid())
		{
			CurrentStreamMetadata = MoveTemp(PlayerMeta);
...
void FElectraPlayer::HandlePlayerEventError(const FString& ErrorReason)
{
	...
	UE_LOGF(LogElectraPlayer, Error, "[%u] ReportError: \"%ls\"", PlayerUniqueID, *SanitizeMessage(ErrorReason));
...
void FElectraPlayer::HandlePlayerEventLogMessage(Electra::IInfoLog::ELevel InLogLevel, const FString& InLogMessage, int64 InPlayerWallclockMilliseconds)
{
	...
	switch(InLogLevel)
	{
		...
		{
			UE_LOGF(LogElectraPlayer, Error, "[%u] %ls", PlayerUniqueID, *m);
			FScopeLock Lock(&StatisticsLock);
		...
		{
			UE_LOGF(LogElectraPlayer, Warning, "[%u] %ls", PlayerUniqueID, *m);
			FScopeLock Lock(&StatisticsLock);
			Statistics.AddMessageToHistory(m);
			break;
		}
		...
		{
			UE_LOGF(LogElectraPlayer, Log, "[%u] %ls", PlayerUniqueID, *m);
			break;
		}
		case Electra::IInfoLog::ELevel::Verbose:
		{
			UE_LOGF(LogElectraPlayer, Verbose, "[%u] %ls", PlayerUniqueID, *m);
			break;
		}
	}
}

void FElectraPlayer::HandlePlayerEventDroppedVideoFrame(const Metrics::FFrameStats& InFrameStats)
{
	Statistics.NumTotalAddedVideoFrames = InFrameStats.NumTotal;
	Statistics.NumTotalDroppedVideoFrames = InFrameStats.NumDropped;
	Statistics.AvgVideoDropsPerInterval = InFrameStats.AvgDropsPerInterval;
	Statistics.MinVideoDropsPerInterval = InFrameStats.MinDropsPerInterval;
	Statistics.MaxVideoDropsPerInterval = InFrameStats.MaxDropsPerInterval;
	Statistics.VideoDropIntervalMs = InFrameStats.DropIntervalMs;
	Statistics.VideoNumIntervals = InFrameStats.NumIntervals;
	Statistics.VideoNumIntervalsWithDrops = InFrameStats.NumIntervalsWithDrops;
}

void FElectraPlayer::HandlePlayerEventDroppedAudioFrame(const Metrics::FFrameStats& InFrameStats)
{
}
...
void FElectraPlayer::LogStatistics()
{
	...
	UE_LOG(LogElectraPlayer, Verbose, TEXT(
		...
		"Bytes of video data streamed: %lld from %lld segments\n"\
		"Bytes of audio data streamed: %lld from %lld segments\n"\
		"Video quality percentage:\n%s"\
		"Audio quality percentage:\n%s"\
		"Video frames dropped: %lld of %lld total, %u avg (%u max) per %u ms interval (drops in %.1f%% of all intervals)\n"\
		...
		*SanitizeMessage(Statistics.InitialURL),
		...
		(long long int)Statistics.NumVideoSegmentsStreamed,
		(long long int)Statistics.NumAudioDatabytesStreamed,
		(long long int)Statistics.NumAudioSegmentsStreamed,
		*VideoSegsPercentage,
		*AudioSegsPercentage,
		(long long int)Statistics.NumTotalDroppedVideoFrames,
		(long long int)Statistics.NumTotalAddedVideoFrames,
		Statistics.AvgVideoDropsPerInterval,
		Statistics.MaxVideoDropsPerInterval,
		Statistics.VideoDropIntervalMs,
		Statistics.VideoNumIntervals ? ((int64)Statistics.VideoNumIntervalsWithDrops * 10000 / Statistics.VideoNumIntervals) / 100.0 : 0.0,
	...
	if (Statistics.LastError.Len())
	{
		...
		UE_LOGF(LogElectraPlayer, Verbose, "Most recent log messages:\n%ls", *MessageHistory);
	}
}
...
void FElectraPlayer::SendAnalyticMetrics(const TSharedPtr<IAnalyticsProviderET>& AnalyticsProvider, const FGuid& InPlayerGuid)
{
	...
	{
		UE_LOGF(LogElectraPlayer, Verbose, "[%u] Submitting analytics during playback, some data may be incomplete", PlayerUniqueID);
		// Try to fill in some of the blanks.
		UpdatePlayEndStatistics();
	}

	UE_LOGF(LogElectraPlayer, Verbose, "[%u] Submitting analytics", PlayerUniqueID);
...
// FIXME: the difference is pointless as it does not tell how long playback was really performed for unless we are tracking an uninterrupted playback of a Live session.
	...
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("NumVideoSegmentsFetched"), (uint32) Statistics.NumVideoSegmentsStreamed));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("NumAudioSegmentsFetched"), (uint32) Statistics.NumAudioSegmentsStreamed));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("NumTotalSegmentsFetched"), (uint32) Statistics.NumVideoSegmentsStreamed + Statistics.NumAudioSegmentsStreamed));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("NumVideoFramesAddedTotal"), (uint32) Statistics.NumTotalAddedVideoFrames));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("NumVideoFramesDroppedTotal"), (uint32) Statistics.NumTotalDroppedVideoFrames));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("VideoDropIntervalMs"), (uint32) Statistics.VideoDropIntervalMs));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("AvgVideoDropsPerInterval"), (uint32) Statistics.AvgVideoDropsPerInterval));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("MinVideoDropsPerInterval"), (uint32) Statistics.MinVideoDropsPerInterval));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("MaxVideoDropsPerInterval"), (uint32) Statistics.MaxVideoDropsPerInterval));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("VideoNumIntervals"), (uint32) Statistics.VideoNumIntervals));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("VideoNumIntervalsWithDrops"), (uint32) Statistics.VideoNumIntervalsWithDrops));
	ParamArray.Add(FAnalyticsEventAttribute(TEXT("VideoIntervalsWithDropsPercentage"), (double)(Statistics.VideoNumIntervals ? ((int64)Statistics.VideoNumIntervalsWithDrops * 10000 / Statistics.VideoNumIntervals) / 100.0 : 0.0)));
...
void FElectraPlayer::OnMediaPlayerEventReceived(TSharedPtrTS<IAdaptiveStreamingPlayerAEMSEvent> InEvent, IAdaptiveStreamingPlayerAEMSReceiver::EDispatchMode InDispatchMode)
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] %ls event %ls with \"%ls\", \"%ls\", \"%ls\" PTS @ %.3f for %.3fs", PlayerUniqueID,
	...
	if (Player.IsValid())
	{
		...
		Player->AdaptivePlayer->GetMediaSamples()->AddMetadata(MetaDataSample);
	}
}
...
{
	auto Player = CurrentPlayer;
	return Player.IsValid() ? *Player->AdaptivePlayer->GetMediaSamples() : *EmptyMediaSamples;
}
		...
		case EMediaCacheState::Pending:
		{
			...
			if (Player.IsValid())
			{
				...
				if (InState == EMediaCacheState::Loaded)
				{
					...
					if (Player->AdaptivePlayer->GetMediaSamples()->PeekVideoSampleTimeRanges(QueuedRange) && QueuedRange.Num())
					{
...
bool FElectraPlayer::SetLooping(bool bInLooping)
{
	...
	UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaPlayer::SetLooping(%ls)", PlayerUniqueID, bEnableLooping.GetValue()?TEXT("true"):TEXT("false"));
	return true;
}
...
bool FElectraPlayer::SetRate(float InRate)
{
	...
	UE_LOGF(LogElectraPlayer, Log, "[%u] IMediaControls::SetRate(%.3f)", PlayerUniqueID, InRate);
...
bool FElectraPlayer::Seek(const FTimespan& InNewTime, const FMediaSeekParams& InAdditionalParams)
{
	...
	UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaControls::Seek() to %#.4f (%ls)", PlayerUniqueID, InNewTime.GetTotalSeconds(), *InNewTime.ToString(TEXT("%h:%m:%s.%f")));
...
bool FElectraPlayer::SetPlaybackTimeRange(const TRange<FTimespan>& InTimeRange)
{
	...
	if (Player.IsValid())
	{
		...
		if (!CurrentPlaybackRange.IsEmpty())
		{
			...
			{
				UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaControls::SetPlaybackTimeRange(%#.4f - %#.4f)", PlayerUniqueID, Range.Start.GetValue().GetAsSeconds(), Range.End.GetValue().GetAsSeconds());
			}
			else if (Range.Start.IsSet())
			{
				UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaControls::SetPlaybackTimeRange(%#.4f - n/a)", PlayerUniqueID, Range.Start.GetValue().GetAsSeconds());
			}
			else if (Range.End.IsSet())
			{
				UE_LOGF(LogElectraPlayer, Verbose, "[%u] IMediaControls::SetPlaybackTimeRange(n/a - %#.4f)", PlayerUniqueID, Range.End.GetValue().GetAsSeconds());
			}
		}
	}
	return true;
...
bool FElectraPlayer::GetVideoTrackFormat(int32 InTrackIndex, int32 InFormatIndex, FMediaVideoTrackFormat& OutFormat) const
{
	if (InTrackIndex >= 0 && InTrackIndex < NumTracksVideo && InFormatIndex == 0)
	{
		...
		if (Meta.IsValid())
		{
			...
			{
				FScopeLock lock(&VideoFormatLock);
				OutFormat.FrameRate = CurrentlyActiveVideoStreamFormat.FrameRate;
			}
...
TOptional<FTimespan> FElectraPlayer::GetTrackDuration(EMediaTrackType InTrackType, int32 InTrackIndex) const
{
	TSharedPtr<Electra::FTrackMetadata, ESPMode::ThreadSafe> Meta = GetTrackStreamMetadata(InTrackType, InTrackIndex);
	return Meta.IsValid() ? Meta->Duration : TOptional<FTimespan>();
}
...
void FElectraPlayer::ReportMediaMetadataChanged(TSharedPtrTS<Electra::FMetadataParser> InMetadata)
{
...
void FElectraPlayer::ReportVideoFrameStats(const Metrics::FFrameStats& InFrameStats)
{
	DeferredPlayerEvents.Enqueue(MakeSharedTS<FPlayerMetricEvent_DroppedFrame>(FPlayerMetricEventBase::EType::DroppedVideoFrame, InFrameStats));
}

void FElectraPlayer::ReportAudioFrameStats(const Metrics::FFrameStats& InFrameStats)
{
	DeferredPlayerEvents.Enqueue(MakeSharedTS<FPlayerMetricEvent_DroppedFrame>(FPlayerMetricEventBase::EType::DroppedAudioFrame, InFrameStats));
}
