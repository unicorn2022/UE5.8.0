...
void FMediaPlateEditorModule::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FMediaPlateEditorModule::OnPostEngineInit);
}
...
void FMediaPlateEditorModule::RegisterPlacementModeItems()
{
	auto RegisterPlaceActors = [this]() -> void
	{
		...
		if (!Info)
		{
			UE_LOGF(LogMediaPlateEditor, Warning, "Could not find or create Media Place Actor Category");
			return;
		}
