...
bool FMediaPlateCustomization::IsMediaPlateEventAllowedForPlayer(EMediaPlateEventState InState, UMediaPlayer* InMediaPlayer)
{
	...
	case EMediaPlateEventState::PlayReverse:
		// Is player paused or fast forwarding/rewinding?
		return InMediaPlayer->IsReady() && InMediaPlayer->SupportsRate(-1.f, /* Unthinned */ true)
			&& (!InMediaPlayer->IsPlaying() || InMediaPlayer->GetRate() != -1.0f);
	...
	case EMediaPlateEventState::JumpToEnd:
		return InMediaPlayer->IsReady() && InMediaPlayer->SupportsSeeking() && InMediaPlayer->GetTime() < InMediaPlayer->GetDuration();

	case EMediaPlateEventState::StepForward:
	case EMediaPlateEventState::StepBackward:
		return InMediaPlayer->IsReady() && InMediaPlayer->SupportsSeeking() && InMediaPlayer->IsPaused();
