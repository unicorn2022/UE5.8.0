	...
	, TimeRemaining(InWaitTimeout)
{
	if (InMediaPlateComponent)
	{
		...
		{
			UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Failed initial open: %ls", OutputLink, *URL);
			bSawError = true;
		}
	}
	else
	{
		UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Failed initial open because no media source given", OutputLink);
		bSawError = true;
	}
}
...
void FMediaPlateOpenLatentAction::UpdateOperation(FLatentResponse& InResponse)
{
	...
	{
		UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Saw media open failed event. %ls", OutputLink, *URL);
	...
	{
		UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Media Plate Component object was deleted. %ls", OutputLink, *URL);
		FailedOperation(InResponse);
		return;
	}
	...
	{
		UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Media player object was deleted. %ls", OutputLink, *URL);
		FailedOperation(InResponse);
		return;
	}
	...
	{
		UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Media player is in Error state. %ls", OutputLink, *URL);
		FailedOperation(InResponse);
		return;
	}
	...
	{
		UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Media player is closed. %ls", OutputLink, *URL);
		FailedOperation(InResponse);
		return;
	}
	...
	{
		UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Is preparing ... %ls (Time out in %f seconds)", OutputLink, *URL,  TimeRemaining);
	}
	else if (MediaPlayer->IsReady())
	{
		if (!bSawIsReady)
		{
			...
			UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: IsReady() ... %ls", OutputLink, *URL);
			bSawIsReady = true;
		}
	...
	{
		UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Waiting for IsReady() ... %ls (Time out in %f seconds)", OutputLink, *URL, TimeRemaining);
	}
	...
	{
		UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Timed out. %ls", OutputLink, *URL);
		FailedOperation(InResponse);
		return;
	}
}
...
FMediaPlateOpenLatentAction::OperationUpdateResult FMediaPlateOpenLatentAction::UpdateOperationPlayerReady(UMediaPlayer* InMediaPlayer, FLatentResponse& InResponse)
{
	if (bSawMediaOpened)
	{
		...
		if (!SeekTime.IsZero() || bSeekForTextureRequested)
		{
			...
			{
				UE_LOGF(LogMediaPlate, Warning, "[%d] Media Plate Open Latent: Media player seeking to time out of bounds. Seek: %ls, Duration: %ls, URL: %ls",
					OutputLink, *SeekTime.ToString(), *InMediaPlayer->GetDuration().ToString(), *URL);
				FailedOperation(InResponse);
				return OperationUpdateResult::Completed;
			}
			
			UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Waiting for seek completed event ... (Time out in %f seconds)", OutputLink, TimeRemaining);
		}
		else
		{
			return UpdateOperationConditionalWaitForTexture(InResponse);
	...
	{
		UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Waiting for opened event ... (Time out in %f seconds)", OutputLink, TimeRemaining);
	}
...
FMediaPlateOpenLatentAction::OperationUpdateResult FMediaPlateOpenLatentAction::UpdateOperationConditionalWaitForTexture(FLatentResponse& InResponse) const
{
	if (bWaitForTexture)
	{
		...
		{
			UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Triggering output pin after media texture ready. Success: %d, %ls", OutputLink, OutSuccess, *URL);
			CompleteOperation(InResponse);
			return OperationUpdateResult::Completed;
		}

		UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Waiting for texture ... (Time out in %f seconds)", OutputLink, TimeRemaining);	
		return OperationUpdateResult::Continue;
	}
	
	UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Triggering output pin after seek completed. Success: %d, %ls", OutputLink, OutSuccess, *URL);
	CompleteOperation(InResponse);
	return OperationUpdateResult::Completed;
}
...
{
	UE_LOGF(LogMediaPlate, Verbose, "[%d] Media Plate Open Latent: Saw event: %ls", OutputLink, *MediaUtils::EventToString(InEvent));
