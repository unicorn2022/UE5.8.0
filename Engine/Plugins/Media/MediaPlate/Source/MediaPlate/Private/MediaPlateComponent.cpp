	...
	if (MediaPlayer != nullptr)
	{
		...
		if (IntendedPlaybackState != ActualPlaybackState)
		{
			if (IntendedPlaybackState != PendingPlaybackState)
			{
				...
				{
					if (FMath::IsNearlyZero(CurrentRate) || !MediaPlayer->SupportsRate(CurrentRate, /* Unthinned */ true))
					{
						CurrentRate = 1.f;
					}
...
UMediaTexture* UMediaPlateComponent::GetMediaTexture(int32 Index)
{
	...
	{
		UE_CALL_ONCE([Index](){
			UE_LOGF(LogMediaPlate, Warning, "Material does not support texture index %d. Either remove the number of cross fades or change the material.", Index);
		});
	}
	return MediaTexture;
}
	...
	if (IsVisible())
	{
		if (MediaPlayer != nullptr)
		{
			if (UMediaSource* MediaSource = UE::MediaPlateComponent::GetMediaSource(GetMediaPlaylist(), PlaylistIndex))
			{
				...
				{
					UE_LOGF(LogMediaPlate, Warning, "Open Failed: failed to open media source \"%ls\".", *MediaSource->GetUrl());	
				}
			}
			else
			{
				UE_LOGF(LogMediaPlate, Warning, "Open Failed: no media source specified.");
			}
		}
		else
		{
			UE_LOGF(LogMediaPlate, Warning, "Open Failed: missing media player.");
		}
	}
	else
	{
...
void UMediaPlateComponent::PlayReverse()
{
	IntendedPlaybackState = EPlaybackState::Playing;
	CurrentRate = -1.f;
}
...
UMediaSoundComponent* UMediaPlateComponent::CreateMediaSoundComponent()
{
	...
	if (!MediaPlayer)
	{
		UE_LOGF(LogMediaPlate, Error, "Failed to create MediaSoundComponent: Missing Media Player.");
		return nullptr;
	}
	...
	UE_LOGF(LogMediaPlate, Error, "Failed to create MediaSoundComponent.");
	return nullptr;
}
...
void UMediaPlateComponent::SwitchStates(EMediaPlateEventState State)
{
	...
	case EMediaPlateEventState::Play:
		...
		break;
	case EMediaPlateEventState::PlayReverse:
		{
			PlayReverse();
		}
		break;
	...
	case EMediaPlateEventState::Previous:
		...
		break;
	case EMediaPlateEventState::JumpToEnd:
		{
			if (MediaPlayer != nullptr)
			{
		...
		break;
	case EMediaPlateEventState::StepForward:
		{
			if (MediaPlayer != nullptr)
			{
		...
		break;
	case EMediaPlateEventState::StepBackward:
		{
			if (MediaPlayer != nullptr)
			{
				// TODO: Convert to underlying media player command when framework support is added.
