	...
	public:
		...
		bool ReadString(FString& OutString, uint16 NumBytes)
		{
			...
			if (ReadBytes(Buf.GetData(), NumBytes))
			{
				...
				{
					UE_LOGF(LogElectraSubtitles, Error, "WVTT uses UTF16 which is not supported");
					return false;
				}
...
bool FElectraSubtitleDecoderWVTT::InitializeStreamWithCSD(const TArray<uint8>& InCSD, const Electra::FParamDict& InAdditionalInfo)
{
	...
	else
	{
		#define RETURN_IF_ERROR(expr)																				\
			if (!expr)																								\
			{																										\
				UE_LOGF(LogElectraSubtitles, Error, "Not enough CSD bytes to parse WVTT text sample entry");	\
				return false;																						\
			}
		...
		while(r.GetNumBytesRemaining() > 0)
		{
			...
			{
				UE_LOGF(LogElectraSubtitles, Error, "Bad WVTT box in CSD, ignoring.");
				break;
			}
		...
		{
			UE_LOGF(LogElectraSubtitles, Error, "Bad WVTT configuration box 'vttC'!");
			return false;
		}
...
void FElectraSubtitleDecoderWVTT::AddStreamedSubtitleData(const TArray<uint8>& InData, Electra::FTimeValue InAbsoluteTimestamp, Electra::FTimeValue InDuration, const Electra::FParamDict& InAdditionalInfo)
{
	if (bNeedsParsing)
	{
		...
		{
			UE_LOGF(LogElectraSubtitles, Error, "Bad WebVTT document, ignoring.");
			return;
		}
	}
	else
	{
		#define RETURN_IF_ERROR(expr)																\
			if (!expr)																				\
			{																						\
				UE_LOGF(LogElectraSubtitles, Error, "Bad WVTT text sample box, ignoring.");	\
				return;																				\
			}
