	...
	if (!SubtitleHandler.IsValid())
	{
		...
		if (Parser->ParseXMLDocument(InData, Electra::FParamDict()))
		{
			...
			else
			{
				...
				UE_LOGF(LogElectraSubtitles, Error, "TTML error: %ls", *LastErrorMsg);
			}
		}
		else
		{
			LastErrorMsg = Parser->GetLastErrorMessage();
			UE_LOGF(LogElectraSubtitles, Error, "%ls", *LastErrorMsg);
		}
	}
}
	...
	if (ActiveTimelineHandlers.Num())
	{
		...
		if (Active.Handler->UpdateActiveRange(Range))
		{
			...
			if (Subtitles.Num())
			{
				...
				//UE_LOGF(LogElectraSubtitles, Log, "[%.3f-%.3f] @%.3f: %.3f-%.3f: %ls", Active.AbsoluteTimeRange.Start.GetAsSeconds(), Active.AbsoluteTimeRange.End.GetAsSeconds(), InAbsolutePosition.GetAsSeconds(), FirstActiveTime.GetAsSeconds(), (FirstActiveTime+Duration).GetAsSeconds(), *Text);
				...
				if (DocumentType == EDocumentType::STPP && bSendEmptySubtitleDuringGaps && Active.AbsoluteTimeRange.End > LastActiveTime)
				{
					...
					Out->SetDuration(Duration);
					ParsedSubtitleDelegate.Broadcast(Out);
					//UE_LOGF(LogElectraSubtitles, Log, "[%.3f-%.3f] @%.3f: %.3f-%.3f:", Active.AbsoluteTimeRange.Start.GetAsSeconds(), Active.AbsoluteTimeRange.End.GetAsSeconds(), InAbsolutePosition.GetAsSeconds(), LastActiveTime.GetAsSeconds(), (LastActiveTime+Duration).GetAsSeconds());
				}
			}
		}
	}
