...
namespace MP4Boxes
{
	...
	struct FSTTSBoxIterator
	{
		void SetBox(const TSharedPtr<FMP4BoxSTTS, ESPMode::ThreadSafe>& InSttsBox)
		{
			...
			ConstantSampleDuration = SttsBox->GetConstantSampleDuration();
			Time = 0;
			CurrentSampleNum = 0;
			LocalPosInCurrent = 0;
		...
		{ return Time; }
		int64 GetConstantSampleDuration() const
		{ return ConstantSampleDuration; }
	...
	private:
		...
		int64 Time = 0;
		int64 ConstantSampleDuration = 0;
	...
	struct FSTSCBoxIterator
	{
		...
		bool IsFinalChunk() const
		{
			...
			const uint32 NumSmpInLast = StscBox->GetEntries().Num() ? StscBox->GetEntries().Last().samples_per_chunk : 0;
			const uint32 FirstSampleNumInLastChunk = NumTotalSamples - NumSmpInLast;
			return CurrentSampleNum >= FirstSampleNumInLastChunk;
		}
	private:
	...
	struct FSTSZBoxIterator
	{
		...
		TSharedPtr<FMP4BoxSTSZ, ESPMode::ThreadSafe> GetBox() const
		{
			return StszBox;
		}
	...
	{
		void SetBox(const TSharedPtr<FMP4BoxSUBS, ESPMode::ThreadSafe>& InSubsBox, uint32 InMaxSamples)
		{
			NumTotalSamples = InMaxSamples;
			FirstDescribedSampleNum = 0;
			LastDescribedSampleNum = 0;
			CurrentEntryIndex = 0;
			CurrentSampleNum = 0;
			SubsBox = InSubsBox;
			if (SubsBox.IsValid())
			{
				int32 NumEntries = SubsBox->GetEntries().Num();
				if (NumEntries)
				{
					FirstDescribedSampleNum = SubsBox->GetEntries()[0].GetSampleNumber();
					LastDescribedSampleNum = SubsBox->GetEntries().Last().GetSampleNumber();
				}
			}
		}
		...
		void SetToSampleNumber(uint32 InSampleNum)
		{
			InSampleNum = InSampleNum < NumTotalSamples ? InSampleNum : NumTotalSamples-1;
			CurrentSampleNum = InSampleNum;
			const uint32 snumPlus1 = CurrentSampleNum + 1;
			if (!SubsBox.IsValid())
			{
				return;
			}
			if (snumPlus1 <= FirstDescribedSampleNum)
			{
				CurrentEntryIndex = 0;
			}
			else if (snumPlus1 > LastDescribedSampleNum)
			{
				CurrentEntryIndex = SubsBox->GetEntries().Num() ? SubsBox->GetEntries().Num() - 1 : 0;
			}
			else
			{
				// Have to locate the entry for the sample number
				const TArray<FMP4BoxSUBS::FEntry>& Entries = SubsBox->GetEntries();
				for(int32 i=0, iMax=Entries.Num(); i<iMax; ++i)
				{
					if (Entries[i].GetSampleNumber() >= snumPlus1)
					{
						CurrentEntryIndex = i;
						break;
					}
		...
		bool Next()
		{
			...
			if (SubsBox.IsValid())
			{
				if (CurrentSampleNum > LastDescribedSampleNum)
				{
					CurrentEntryIndex = SubsBox->GetEntries().Num() ? SubsBox->GetEntries().Num() - 1 : 0;
				}
				else
				{
					check(CurrentEntryIndex >= 0 && CurrentEntryIndex < SubsBox->GetEntries().Num());
					// If the current subsample list entries sample number equals the current sample
					// number we need to advance the list index. This is because the sample numbers
					// in the list are 1-based while our internal sample number is 0-based.
					// The next entry in the subsample list may not be for the next sample due to
					// the list being sparsely coded, but that is checked for in `HaveSubsamplesForCurrent()`.
					while(CurrentSampleNum >= SubsBox->GetEntries()[CurrentEntryIndex].GetSampleNumber())
					{
						if (++CurrentEntryIndex >= SubsBox->GetEntries().Num())
						{
							--CurrentEntryIndex;
							break;
						}
		...
		bool Prev()
		{
			...
			if (SubsBox.IsValid())
			{
				if (CurrentSampleNum < FirstDescribedSampleNum)
				{
					CurrentEntryIndex = 0;
				}
				else if (CurrentEntryIndex >= 0 && CurrentEntryIndex < SubsBox->GetEntries().Num())
				{
					// See comments up in `Next()`
					while(1+CurrentSampleNum < SubsBox->GetEntries()[CurrentEntryIndex].GetSampleNumber())
					{
		...
		{
			return SubsBox.IsValid() ? TOptional<uint32>(SubsBox->GetFlags()) : TOptional<uint32>();
		}

		bool HaveSubsamplesForCurrent() const
		{
			if (!FirstDescribedSampleNum || (CurrentSampleNum+1 < FirstDescribedSampleNum) || (CurrentSampleNum+1 > LastDescribedSampleNum) || CurrentEntryIndex >= SubsBox->GetEntries().Num())
			{
				return false;
			}
			return CurrentSampleNum+1 == SubsBox->GetEntries()[CurrentEntryIndex].GetSampleNumber();
		}

		const FMP4BoxSUBS::FSubsampleList& GetCurrentSubsamples() const
		{
			return HaveSubsamplesForCurrent() ? SubsBox->GetEntries()[CurrentEntryIndex].GetEntries() : EmptySubsampleList;
		}
	...
	private:
		TSharedPtr<FMP4BoxSUBS, ESPMode::ThreadSafe> SubsBox;
		FMP4BoxSUBS::FSubsampleList EmptySubsampleList;

		uint32 NumTotalSamples = 0;
		uint32 FirstDescribedSampleNum = 0;
		uint32 LastDescribedSampleNum = 0;
		uint32 CurrentSampleNum = 0;
		int32 CurrentEntryIndex = 0;
	};
	...
	struct FSENCBoxIterator
	{
		void SetBox(const TSharedPtr<FMP4BoxTENC, ESPMode::ThreadSafe>& InTencBox, const TSharedPtr<FMP4BoxSENC, ESPMode::ThreadSafe>& InSencBox, uint32 InMaxSamples)
		{
			NumTotalSamples = InMaxSamples;
			CurrentSampleNum = 0;
			SencBox = InSencBox;
			if (SencBox.IsValid())
			{
				NumBoxSamples = SencBox->GetEntries().Num();
			}
			if (InTencBox.IsValid())
			{
				DefaultEntry.IV = InTencBox->GetDefaultConstantIV();
			}
		}
		...
		const FMP4BoxSENC::FEntry& GetCurrentEntry() const
		{
			return CurrentSampleNum < NumBoxSamples ? SencBox->GetEntries()[CurrentSampleNum] : DefaultEntry;
		}

	private:
		TSharedPtr<FMP4BoxSENC, ESPMode::ThreadSafe> SencBox;
		FMP4BoxSENC::FEntry DefaultEntry;
		uint32 NumTotalSamples = 0;
		uint32 NumBoxSamples = 0;
		uint32 CurrentSampleNum = 0;
	};
