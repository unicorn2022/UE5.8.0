...
#include "MP4Utilities.h"		// for parsing the 'dOps' box
...
public:
	...
	FElectraOpusDecoder(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
class FElectraCommonAudioOpusDecoderFactory : public IElectraCodecFactory, public IElectraCodecFactory::IProviderInformation, public IElectraCodecModularFeature, public TSharedFromThis<FElectraCommonAudioOpusDecoderFactory, ESPMode::ThreadSafe>
{
public:
	virtual ~FElectraCommonAudioOpusDecoderFactory()
	{ }

	FElectraCommonAudioOpusDecoderFactory()
	{
		LibVersion = ANSI_TO_TCHAR(opus_get_version_string());
	}
	...
	int32 SupportsDecoding(TMap<FString, FVariant>& OutFormatInfo, const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions) const override
	{
		...
		if (!InCodecFormat.RFC6381.Equals(TEXT("Opus"), ESearchCase::IgnoreCase))
		{
			return 0;
		}
	...
	{
		return InCodecFormat.Properties.IsType<Electra::FCodecTypeFormat::FAudio>() ? MakeShared<FElectraOpusDecoder, ESPMode::ThreadSafe>(InCodecFormat, InAdditionalOptions) : TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe>();
	}
...
FElectraOpusDecoder::FElectraOpusDecoder(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	if (InCodecFormat.DCR.Num())
	{
		bHaveParseddOps = Parse_dOps(DOpsConfig, InCodecFormat.DCR, false);
	}
	if (!bHaveParseddOps && InCodecFormat.ExtraBoxes.Contains(MP4Utilities::MakeBoxAtom('d','O','p','s')))
	{
		bHaveParseddOps = Parse_dOps(DOpsConfig, InCodecFormat.ExtraBoxes[MP4Utilities::MakeBoxAtom('d','O','p','s')], false);
	}
}
...
bool FElectraOpusDecoder::Parse_dOps(FDOpsConfig& OutConfig, const TArray<uint8>& IndOpsBox, bool bFailOnError)
{
	...
	MP4Utilities::FMP4AtomReader rd(IndOpsBox);
	uint32 Value32;
	uint16 Value16;
	uint8 Value8;
...
IElectraDecoder::ECSDCompatibility FElectraOpusDecoder::IsCompatibleWith(const TMap<FString, FVariant>& CSDAndAdditionalOptions)
{
	...
	FDOpsConfig cfg;
	if (!Parse_dOps(cfg, DCR.Num() ? DCR : CSD, false))
	{
...
IElectraDecoder::EDecoderError FElectraOpusDecoder::DecodeAccessUnit(const FInputAccessUnit& InInputAccessUnit, const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		if (!bHaveParseddOps)
		{
			...
			{
				bHaveParseddOps = Parse_dOps(DOpsConfig, ElectraDecodersUtil::GetVariantValueUInt8Array(InAdditionalOptions, TEXT("dcr")), true);
			}
			if (!bHaveParseddOps && InAdditionalOptions.Contains(TEXT("dOps")))
			{
				bHaveParseddOps = Parse_dOps(DOpsConfig, ElectraDecodersUtil::GetVariantValueUInt8Array(InAdditionalOptions, TEXT("dOps")), true);
			}
			if (!bHaveParseddOps)
			{
				return IElectraDecoder::EDecoderError::Error;
