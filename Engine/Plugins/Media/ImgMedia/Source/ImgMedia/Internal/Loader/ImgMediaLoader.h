...
public:
	 ...
	 * @param InGlobalCache             The global frame cache shared across loaders.
	 * @param InSourceColorSettings     Color space and encoding overrides for the source, or null for defaults.
	 * @param bInFillGapsInSequence     If true, gaps in the image sequence are filled with blank frames.
	 * @param InLayerName               EXR layer to select. Empty string reads the default (un-prefixed) channels.
	 * @param InSmartCacheSettings      Settings controlling the smart cache pre-fetching behavior.
	 * @param InEventSink               Optional event sink for Internal_RequestVideoSampleRefresh; the
	 *                                  caller owns the sink and must keep it alive. Null disables.
	 * @param InPlayerGuid              Player's identifier (same value lives on FMediaPlayerFacade and FImgMediaPlayer);
	 *                                  used in log lines for cross-layer correlation. Defaults to an empty GUID.
	 */
	FImgMediaLoader(const TSharedRef<FImgMediaScheduler, ESPMode::ThreadSafe>& InScheduler,
		...
		IMediaEventSink* InEventSink = nullptr,
		const FGuid& InPlayerGuid = FGuid());

	/** Virtual destructor. */
	virtual ~FImgMediaLoader();

	/**
	 * Per-frame tick driven from FImgMediaPlayer::TickInput. Invalidates the
	 * tile-visibility snapshot so the next frame's first worker rebuilds it,
	 * and (when enabled) draws the debug overlay for the previous frame's
	 * snapshot. Called on the game thread, bounded by the player's lifetime.
	 */
	void TickFrame();
...
public:
	...
	* Used to validate if it is safe to call GetReader().
	*/
	bool IsReaderValid() const
	{
	...
	/**
	 * Get the tile-visibility resolver used by this loader.
	 * Always non-null; consumers register providers on this to influence
	 * which tiles get loaded. When no providers register, the loader falls
	 * back to loading every tile at every mip level.
	 */
	const TSharedPtr<FImgMediaTileVisibilityResolver, ESPMode::ThreadSafe>& GetTileVisibilityResolver() const
	{
		return TileVisibilityResolver;
	}

	/** Player identifier mirrored from FImgMediaPlayer; useful for cross-layer log correlation. */
	...
	/** Gets the layer name being read in the sequence, if the images are EXR images */
	const FString& GetLayerName() const { return LayerName; }
	...
	/**
	 * Get the number of bytes per pixel for each image.
	 */
	...
	/**
	 * Get the number of images/frames in a single mip level.
	 *
	...
	int32 GetNumImages() const
	{
		if (bIsContainerFile)
		{
			return NumContainerFrames;
		}
		return ImagePaths.Num() > 0 ? ImagePaths[0].Num() : 0;
	}

	/** Whether the source is a container file (e.g. .tmv) rather than a file sequence. */
	bool IsContainerFile() const
	{
		return bIsContainerFile;
	}
	...
	/** Mirror the owning player's paused state into the loader; used by Update / IsCachedFrameInRange. */
...
protected:
	 ...
	 * @param OutMipsAndTiles Will be filled in with what tiles are actually needed.
	 */
	void GetDesiredMipTiles(int32 FrameIndex, TMap<int32, FMediaTileSelection>& OutMipsAndTiles) const;
	 ...
	 * @param Extension If the filename has an extension, it will be ignored (in case there is a number in the extension).
	 * @return True if it found a number.
	 */
	bool GetNumberAtEndOfString(int32& Number, const FString& String, const FString& Extension) const;
	 ...
	 *		5			7
	 */
	int32 GetSkipFrame(int32 FrameIndex) const;
...
private:
	 ...
	 * Not used for container files.
	 */
	TArray<TArray<FString>> ImagePaths;

	/** Whether the source is a container file rather than a file sequence. */
	bool bIsContainerFile = false;

	/** Number of frames in the container (only valid when bIsContainerFile is true). */
	int32 NumContainerFrames = 0;
	...
	/** Stores number of mip levels. */
	int32 NumMipLevels;

	/** Stores number of bytes per pixel for each image. */
	...
	/** Tile-visibility resolver. Always created; dormant until consumers register providers. */
	TSharedPtr<FImgMediaTileVisibilityResolver, ESPMode::ThreadSafe> TileVisibilityResolver;
...
private:
	...
	/** Loop flag stashed from the last RequestFrame so TickFrame's paused-replay Update sees the right wrap-around state. */
	bool LastRequestedLoop = false;

	/** Player-supplied sink for Internal_RequestVideoSampleRefresh events. Outlives the loader; null disables. */
	IMediaEventSink* EventSink = nullptr;

	/** Player identifier (mirrors FImgMediaPlayer::PlayerGuid); included in log lines for cross-layer correlation. */
	FGuid PlayerGuid;

	/** Mirror of the owning player's paused state; pushed via SetIsPaused. Atomic because NotifyWorkComplete reads it from worker threads. */
