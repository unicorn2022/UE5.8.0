...
FString UImgMediaSource::SanitizeTokenizedSequencePath(const FString& InPath)
{
	...
	// Re-check after expansion: tokens or normalization can produce a UNC from a clean input.
	if (!UE::MediaAssets::IsSourcePathAllowed(ExpandedPath))
	{
		UE_LOGF(LogImgMedia, Warning, "Rejecting non-local sequence path: %ls", *InPath);
		return FString();
	}
...
{
	// Runtime breadcrumb for unmigrated external callers - the deprecation marker is easy to
	// miss and the function silently does nothing, so a tile-visibility regression would
	// otherwise surface only as degraded mip selection at playback time.
	UE_LOG(LogImgMedia, Warning,
		TEXT("UImgMediaSource::AddTargetObject is deprecated and is now a no-op. ")
		TEXT("Tile-visibility consumers should register through FMediaTextureTracker. Actor: %s"),
		*GetNameSafe(InActor));
}
...
{
	UE_LOG(LogImgMedia, Warning,
		TEXT("UImgMediaSource::RemoveTargetObject is deprecated and is now a no-op. ")
		TEXT("Tile-visibility consumers should register through FMediaTextureTracker. Actor: %s"),
		*GetNameSafe(InActor));
}
...
FString UImgMediaSource::GetMediaOption(const FName& Key, const FString& DefaultValue) const
{
	...
	if (Key == ImgMedia::LayerNameOption)
	{
		return LayerName;
	}
...
FString UImgMediaSource::GetFullPath() const
{
	...
	if (FPaths::IsRelative(ExpandedSequencePath))
	{
		for (const FString& BasePath : UE::ImgMediaSource::Private::GetPossibleBasePaths())
		{
			const FString Candidate = FPaths::ConvertRelativePathToFull(FPaths::Combine(BasePath, ExpandedSequencePath));
		...
		if (FullPath.IsEmpty())
		{
			FullPath = FPaths::ConvertRelativePathToFull(FPaths::Combine(FPaths::ProjectDir(), ExpandedSequencePath));
		}
	}
	else
	{
		FullPath = ExpandedSequencePath;
	}

	// Re-check after resolution: token expansion can produce a UNC from a clean stored path.
	if (!UE::MediaAssets::IsSourcePathAllowed(FullPath))
	{
		UE_LOGF(LogImgMedia, Warning, "Rejecting non-local sequence path: %ls", *FullPath);
		return FString();
	}
