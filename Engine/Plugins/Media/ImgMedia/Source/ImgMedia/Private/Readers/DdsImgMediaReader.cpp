...
namespace ImgMedia
{
	namespace DDS
	{
		/** Descriptor for a DDS pixel format. */
		struct FDDSFormatInfo
		{
			UE::DDS::EDXGIFormat Format;
			uint32 Id;
			uint8 BytesPerUnit;
			bool bIsBlockCompressed;
		};

		static const FDDSFormatInfo FormatInfoList[] = 
		{
			#define RGBFMT(Name, Id, BytesPerUnit) { UE::DDS::EDXGIFormat::Name, Id, BytesPerUnit, false },
			#define BCNFMT(Name, Id, BytesPerUnit) { UE::DDS::EDXGIFormat::Name, Id, BytesPerUnit, true },
			#define ODDFMT(Name, Id) { UE::DDS::EDXGIFormat::Name, Id, 0, false },
			UE_DXGI_FORMAT_LIST
			#undef RGBFMT
			#undef BCNFMT
			#undef ODDFMT
		...
		static const FDDSFormatInfo* GetDDSFormatInfo(UE::DDS::EDXGIFormat InFormat)
		{
			// need to handle this special because UNKNOWN _does_ appear in the FormatInfoList,
			// but we don't want to treat it as legal
			if (InFormat == UE::DDS::EDXGIFormat::UNKNOWN)
			{
				return nullptr;
			}

			for (size_t Index = 0; Index < sizeof(FormatInfoList)/sizeof(*FormatInfoList); ++Index)
			{
				if (InFormat == FormatInfoList[Index].Format) 
				{
					return &FormatInfoList[Index];
				}
			}
		...
		bool IsFormatBlockCompressed(UE::DDS::EDXGIFormat InDXGIFormat)
		{
			const FDDSFormatInfo* FormatInfo = GetDDSFormatInfo(InDXGIFormat);
			return FormatInfo ? FormatInfo->bIsBlockCompressed : false;
		}
		...
		bool GetMediaTextureSampleFormat(const UE::DDS::EDXGIFormat& InDXGIFormat, EMediaTextureSampleFormat& OutFormat, bool& bOutIsBlockCompressed)
		{
			...
			default:
				UE_LOGF(LogImgMedia, Error, "Unsupported compression format, only BC1/DXT1, BC3/DXT5, BC4 & FloatRGBA are currently supported.");
		...
		/*
		 * For block compressed formats, only consider mips larger than the block size as valid. 
		 */
		inline bool IsMipValid(const UE::DDS::FDDSMip& InMip, bool bIsBlockCompressed)
		{
		...
		{
			const bool bIsBlockCompressed = IsFormatBlockCompressed(DDS->DXGIFormat);
			SIZE_T TotalMipDataSize = 0;
			for (const UE::DDS::FDDSMip& Mip : DDS->Mips)
			{
				if (!IsMipValid(Mip, bIsBlockCompressed))
				{
					break;
				}
		...
		/** Returns true if the given format is a floating point type. */
		bool IsFloatFormat(UE::DDS::EDXGIFormat InFormat)
		{
			switch (InFormat)
			{
			case UE::DDS::EDXGIFormat::R32G32B32A32_FLOAT:
			case UE::DDS::EDXGIFormat::R32G32B32_FLOAT:
			case UE::DDS::EDXGIFormat::R16G16B16A16_FLOAT:
			case UE::DDS::EDXGIFormat::R32G32_FLOAT:
			case UE::DDS::EDXGIFormat::R11G11B10_FLOAT:
			case UE::DDS::EDXGIFormat::R16G16_FLOAT:
			case UE::DDS::EDXGIFormat::D32_FLOAT:
			case UE::DDS::EDXGIFormat::R32_FLOAT:
			case UE::DDS::EDXGIFormat::R16_FLOAT:
				return true;
			default:
				return false;
			}
		...
		{
			// Float formats are considered linear.
			if (IsFloatFormat(DDS->DXGIFormat))
			{
				return false;
			}
...
public:
	...
	TUniquePtr<UE::DDS::FDDSFile> ReadHeaderAndPrepare()
	{
		...
		if (!DDS.IsValid())
		{
			...
			{
				UE_LOGF(LogImgMedia, Warning, "Failed to load DDS (Error=%d) [%ls]", (int)Error, *Filename);
			}
...
bool FDdsImgMediaReader::ReadFrame(int32 FrameId, const TMap<int32, FMediaTileSelection>& InMipTiles, TSharedPtr<FImgMediaFrame, ESPMode::ThreadSafe> OutFrame)
{
	...
	if (!DDS.IsValid())
	{
		UE_LOGF(LogImgMedia, Warning, "FDdsImgMediaReader: Failed to load image %ls", *ImagePath);
		return false;
	}
