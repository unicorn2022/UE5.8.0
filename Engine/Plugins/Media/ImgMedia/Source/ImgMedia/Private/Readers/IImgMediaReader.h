...
struct FImgMediaMipTiles final
{
	...
	*/
	 ...
	 */
	bool ContainsMipTiles(const TMap<int32, FMediaTileSelection>& InRequestedMipTiles)
	{
	 ...
	 */
	bool ContainsTiles(const int32 InMipLevel, const FMediaTileSelection& InSelection)
	{
		FScopeLock Lock(&CriticalSection);

		if (const FMediaTileSelection* CachedSelection = MipTiles.Find(InMipLevel))
		{
	 ...
	 */
	void Include(const int32 InMipLevel, const FMediaTileSelection& InSelection)
	{
		FScopeLock Lock(&CriticalSection);

		if (FMediaTileSelection* Tiles = MipTiles.Find(InMipLevel))
		{
	 ...
	 * @param InSelection New tile selection.
	 */
	void Emplace(const int32 InMipLevel, const FMediaTileSelection& InSelection)
	{
		FScopeLock Lock(&CriticalSection);
	 ...
	 */
	bool GetSelection(const int32 InMipLevel, FMediaTileSelection& OutSelection) const
	{
		FScopeLock Lock(&CriticalSection);

		if (const FMediaTileSelection* Selection = MipTiles.Find(InMipLevel))
		{
	 ...
	 * @return true if mip level was found
	 */
	bool GetVisibleRegions(const int32 InMipLevel, const FMediaTileSelection& InSelection, TArray<FIntRect>& OutRegions) const
	{
		FScopeLock Lock(&CriticalSection);
	...
	TMap<int32, FMediaTileSelection>& GetDataUnsafe()
	{
		return MipTiles;
	}

	/** Thread-unsafe data getter: the caller is responsible for locking. */
	const TMap<int32, FMediaTileSelection>& GetDataUnsafe() const
	{
		return MipTiles;
	}
...
private:
	...
	TMap<int32, FMediaTileSelection> MipTiles;
};
...
public:
	 ...
	 * @see GetFrameInfo
	 */
	virtual bool ReadFrame(int32 FrameId, const TMap<int32, FMediaTileSelection>& InMipTiles, TSharedPtr<FImgMediaFrame, ESPMode::ThreadSafe> OutFrame) = 0;
	...
	/** Query media metadata by name. Returns an empty variant if not available. */
	virtual FVariant GetMediaInfo(FName InfoName) const { return FVariant(); }

public:

	/** Virtual destructor. */
	virtual ~IImgMediaReader() = default;
};
