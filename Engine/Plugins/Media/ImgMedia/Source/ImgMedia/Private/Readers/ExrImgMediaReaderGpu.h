...
public:
	...
	/** Constructor. */
	FExrImgMediaReaderGpu(const TSharedRef<FImgMediaLoader, ESPMode::ThreadSafe>& InLoader, const FString& InLayerName = FString());
...
private:
	...
	/** If true, then just use the CPU to read the file. Written once with memory_order_release, read lock-free with memory_order_acquire. */
	std::atomic_bool bFallBackToCPU;
};
...
using FExrConvertBufferCallback = TDelegate<bool(FRHICommandListImmediate& RHICmdList, FTextureRHIRef RenderTargetTextureRHI, TMap<int32, FStructuredBufferPoolItemSharedPtr>& MipBuffers, const FSampleConverterParameters ConverterParams)>;
...
public:
	...
	virtual ~FExrMediaTextureSampleConverter() = default;
