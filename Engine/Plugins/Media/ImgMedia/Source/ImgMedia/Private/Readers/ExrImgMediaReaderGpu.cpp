...
FExrImgMediaReaderGpu::FExrImgMediaReaderGpu(const TSharedRef<FImgMediaLoader, ESPMode::ThreadSafe>& InLoader, const FString& InLayerName)
	: FExrImgMediaReader(InLoader, InLayerName)
...
bool FExrImgMediaReaderGpu::GetFrameInfo(const FString& ImagePath, FImgMediaFrameInfo& OutInfo)
{
	// Skip the CPU reader's single-mip clamp; the GPU path reads in-file mip levels natively.
	return GetInfo(ImagePath, OutInfo);
}

FExrImgMediaReader::EReadResult FExrImgMediaReaderGpu::ReadMip
	( const int32 CurrentMipLevel
	, const FMediaTileSelection& CurrentTileSelection
	...
	, const FString& ImagePath)
{
	...
	const FIntPoint CurrentMipDim = (ConverterParams.FullResolution / MipLevelDiv).ComponentMax(FIntPoint(1,1));
	const FImgMediaFrameInfo& FrameInfo = ConverterParams.FrameInfo;
	const FIntPoint CurrentNumTiles = FIntPoint(
		FMath::Max(1, FMath::CeilToInt(static_cast<float>(FrameInfo.NumTiles.X) / MipLevelDiv)),
		FMath::Max(1, FMath::CeilToInt(static_cast<float>(FrameInfo.NumTiles.Y) / MipLevelDiv)));

	const SIZE_T BufferSize = GetBufferSize(CurrentMipDim, FrameInfo.NumChannels, FrameInfo.bHasTiles, CurrentNumTiles);
			...
			ENQUEUE_RENDER_COMMAND(CopyFromUploadBuffer)([SampleConverter, BufferData, BufferRegionsToCopy, FrameId = ConverterParams.FrameId](FRHICommandListImmediate& RHICmdList)
				{
					...
					RHICmdList.Transition(FRHITransitionInfo(BufferData->ShaderAccessBufferRef, ERHIAccess::SRVMask, ERHIAccess::CopyDest));
					...
					RHICmdList.Transition(FRHITransitionInfo(BufferData->ShaderAccessBufferRef, ERHIAccess::CopyDest, ERHIAccess::SRVMask));
				});
		}
	}
	else
	{
		UE_LOGF(LogImgMedia, Error, "Could not load %ls", *ImagePath);
		return Fail;
	}
...
bool FExrImgMediaReaderGpu::ReadFrame(int32 FrameId, const TMap<int32, FMediaTileSelection>& InMipTiles, TSharedPtr<FImgMediaFrame, ESPMode::ThreadSafe> OutFrame)
{
	// Fall back to cpu?
	if (bFallBackToCPU.load(std::memory_order_acquire))
	{
	...
	/**
	 * The GPU raw block reader cannot perform per-channel selection. Any file
	 * that warrants a LayerName almost certainly has more than four channels
	 * total, which IsOptimizedForGpu() would already have rejected. Delegate
	 * all layer-name reads to the CPU reader which handles them correctly.
	 */
	if (!LayerName.IsEmpty())
	{
		UE_LOGFMT(LogImgMedia, Warning,
			"GPU reader does not support EXR layer selection (layer '{0}'). Falling back to CPU reader.",
			LayerName);
		bFallBackToCPU.store(true, std::memory_order_release);
...

	{
		...
		TMap<int32, FMediaTileSelection> InMipTilesCopy = InMipTiles;
		...
		// Loop over all mips.
		for (const TPair<int32, FMediaTileSelection>& TilesPerMip : InMipTilesCopy)
		{
			...
			case FExrImgMediaReader::Fail:
				{
					...
					if (GetInfo(ImagePath, Info))
					{
						...
						{
							UE_LOGF(LogImgMedia, Error, "GPU Reader cannot read compressed file %ls.", *ImagePath);
							UE_LOGF(LogImgMedia, Error, "Compressed and uncompressed files should not be mixed in a single sequence.");
						}
					}
	...
	UE_LOGF(LogImgMedia, Verbose, "Reader %p: Read Pixels Complete. %i", this, FrameId);
	return true;
}
...
FExrImgMediaReaderGpu::EReadResult FExrImgMediaReaderGpu::ReadInChunks(uint16* Buffer, const FString& ImagePath, int32 FrameId, const FIntPoint& Dim, int32 BufferSize)
{
	...
	for (int32 Row = 0; Row <= NumChunks; Row++)
	{
		...
		// Check to see if the frame was canceled.
		{
			...
			{
				UE_LOGF(LogImgMedia, Verbose, "Reader %p: Canceling Frame %i At chunk # %i", this, FrameId, Row);
				bResult = Cancelled;
				break;
			}
		}
	...
	auto RenderThreadSwizzler = [] (FRHICommandListImmediate& RHICmdList, FTextureRHIRef RenderTargetTextureRHI, TMap<int32, FStructuredBufferPoolItemSharedPtr>& MipBuffers, const FSampleConverterParameters ConverterParams)->bool
	{
			...
			, const TArray<FIntRect>& MipViewports)
		{
			...
			if (ConverterParams.SourceColorSettings.IsValid())
			{
				...
				const UE::Color::EEncoding EncodingOverride = ConverterParams.SourceColorSettings->GetEncodingOverride();
				Parameters.EOTF = static_cast<uint32>(EncodingOverride);
				...
				const UE::Color::EReferenceWhite ReferenceWhite = ConverterParams.SourceColorSettings->GetReferenceWhiteOverride();
				const float NormalizationFactor = UE::Color::GetReferenceWhiteLinearScale(EncodingOverride, ReferenceWhite);
				if (!FMath::IsNearlyEqual(NormalizationFactor, 1.0f))
				{
					Parameters.ColorSpaceMatrix = Parameters.ColorSpaceMatrix.ApplyScale(NormalizationFactor);
				}
			}
			else
			{
		...
		for (int32 MipLevel = 0; MipLevel <= MipToUpscale; MipLevel++)
		{
			...
			const FIntPoint Dim = (ConverterParams.FullResolution / MipLevelDiv).ComponentMax(FIntPoint(1,1));

			{
				...
				{
					UE_LOGF(LogImgMedia, Warning, "Requested mip could not be found %d", MipToUpscale);
				}
				...
				RenderMip(RHICmdList, RenderTargetTextureRHI, ConverterParams, MipToUpscale, MipLevel, BufferDataToUpscale, (ConverterParams.FullResolution / (1 << MipToUpscale)).ComponentMax(FIntPoint(1,1)), Dim, FakeViewport);
			}
		}
		...
		ENQUEUE_RENDER_COMMAND(CreatePooledBuffer)([AllocatedBuffer, AllocSize](FRHICommandListImmediate& RHICmdList)
			{
...

				{
					...
					if (bExrReaderUseUploadHeap)
					{
						...
						RHICmdList.Transition(FRHITransitionInfo(AllocatedBuffer->UploadBufferRef, CreateDesc.InitialState, ERHIAccess::CopySrc));
					}
				}
