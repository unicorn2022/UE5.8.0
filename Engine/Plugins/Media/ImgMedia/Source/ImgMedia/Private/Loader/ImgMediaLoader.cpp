...
#include "Readers/TmvImgMediaReader.h"
...
static TAutoConsoleVariable<bool> CVarImgMediaPausedRefreshEnabled(
	TEXT("ImgMedia.PausedRefresh"),
	true,
	TEXT("Enables player tile visibility refresh while paused.\n")
	TEXT("   0: off\n")
	TEXT("   1: on (default)\n"),
	ECVF_Default);
...
namespace ImgMediaLoader
{
	void CheckAndUpdateImgDimensions(FIntPoint& InOutSequenceDim, const FIntPoint& InNewDim)
	{
		if (InOutSequenceDim != InNewDim)
		{
			...
			UE_LOGF(LogImgMedia, Warning, "Image sequence has inconsistent dimensions. The original sequence dimension (%dx%d) is changed to the new dimension (%dx%d).",
	...
	bool IsCachedFrameInRange(int32 CurrentFrame, int32 OriginFrame, int32 OffsetCount, int32 TotalNumFrames, float PlayRate, bool bInIsPaused)
	{
		if (OffsetCount <= 0)
		{
			return false;
		...
		// Unless the player is paused, we start one frame away, since it's generally too late to enqueue additional
		// update work for the current cached frame.
		const int32 StartOffset = bInIsPaused ? 0 : 1;

		for (int32 Offset = StartOffset; Offset <= OffsetCount; ++Offset)
		{
...
FImgMediaLoader::FImgMediaLoader(const TSharedRef<FImgMediaScheduler, ESPMode::ThreadSafe>& InScheduler,
	...
	, TileVisibilityResolver(MakeShared<FImgMediaTileVisibilityResolver, ESPMode::ThreadSafe>())
...
#endif
{
	...
	UE_LOGF(LogImgMedia, Verbose, "Loader %p [%ls]: Created", this, *PlayerGuid.ToString());
}


FImgMediaLoader::~FImgMediaLoader()
{
	UE_LOGF(LogImgMedia, Verbose, "Loader %p [%ls]: Destroyed", this, *PlayerGuid.ToString());
...
void FImgMediaLoader::TickFrame()
{
	if (!TileVisibilityResolver.IsValid())
	{
		return;
	}

	// Debug overlay reads the snapshot used by the previous frame's workers.
	// Invalidate after so the next frame's first GetSnapshot() rebuilds.
	if (CVarImgMediaMipMapDebugEnable.GetValueOnGameThread())
	{
		TileVisibilityResolver->AddVisibilitySnapshotDebugOverlay();
	}
	TileVisibilityResolver->InvalidateSnapshot();

	// Paused-Refresh: drive Update here to detect tile visibility changes and trigger refresh.
	// Note: a 1-frame sequence is functionally paused, so it will go through the paused refresh path.
	const bool bShouldRefresh = bIsPaused || GetNumImages() == 1;
	if (bShouldRefresh && LastRequestedFrame != INDEX_NONE && CVarImgMediaPausedRefreshEnabled.GetValueOnGameThread())
	{
		Update(LastRequestedFrame, 0.0f, LastRequestedLoop);
	}
}
...
uint64 FImgMediaLoader::GetBitRate() const
{
	...
	return SequenceDim.X * SequenceDim.Y * NumBytesPerPixel * 8 * SequenceFrameRate.AsDecimal();
}
...
const FString& FImgMediaLoader::GetImagePath(int32 FrameNumber, int32 MipLevel) const
{
	...
	{
		UE_LOGF(LogImgMedia, Error, "Loader %p: GetImagePath has wrong parameters, FrameNumber:%d MipLevel:%d.",
...
{
	const bool bHasResolverProviders = TileVisibilityResolver.IsValid() && TileVisibilityResolver->HasProviders();
	return bHasResolverProviders && (NumMipLevels > 1 || IsTiled());
}
...
void FImgMediaLoader::SetIsPaused(bool bInIsPaused)
{
	const bool bWasPaused = bIsPaused.exchange(bInIsPaused, std::memory_order_relaxed);
#if WITH_EDITOR
	if (bInIsPaused && !bWasPaused)
	{
		// Rolling-window measurements across a pause boundary are meaningless.
...
IQueuedWork* FImgMediaLoader::GetWork()
{
	...
	TMap<int32, FMediaTileSelection> DesiredMipsAndTiles;
	...
	if (MinimumMipLevelToUpscale >= 0 && NumMipLevels <= 1)
	{
		UE_CALL_ONCE([ImagePath = GetImagePath(0, 0)]
			{
				UE_LOGF(LogImgMedia, Display, "No upscaling for sequence without mips: %ls", *ImagePath);
			});
	}
...
{
	UE_LOGF(LogImgMedia, Verbose, "Loader %p: Initializing with %ls (FrameRateOverride = %ls, Loop = %i)",
		this,
		*SequencePath,
...
bool FImgMediaLoader::RequestFrame(int32 FrameNumber, float PlayRate, bool Loop)
{
	...
	if ((FrameNumber == INDEX_NONE) || (FrameNumber == LastRequestedFrame))
	{
		...
		UE_LOGF(LogImgMedia, VeryVerbose, "Loader %p: Skipping frame %i for time %ls last %d", this, FrameNumber, *FrameNumberToTime(FrameNumber).ToString(TEXT("%h:%m:%s.%t")), LastRequestedFrame);
		return false;
	}

	UE_LOGF(LogImgMedia, VeryVerbose, "Loader %p: Requesting frame %i for time %ls", this, FrameNumber, *FrameNumberToTime(FrameNumber).ToString(TEXT("%h:%m:%s.%t")));
...
void FImgMediaLoader::HandleGlobalCacheFrameEviction()
{
	...
	if (LastRequestedFrame != INDEX_NONE)
	{
		...
		if ((PendingFrameNumbers.Num() == 0) && (QueuedFrameNumbers.Num() == 0))
		{
			...
			if (RetryCount > 1)
			{
				UE_LOGF(LogImgMedia, Error, "Reloading frames. The global cache may be too small.");
				LastRequestedFrame = INDEX_NONE;
				RetryCount = 0;
			}
		}
...
bool FImgMediaLoader::LoadSequence(const FString& SequencePath, const FFrameRate& FrameRateOverride, bool Loop, FImgMediaFrameInfo& FirstFrameInfo)
{
	...
	// Check if SequencePath is a container file (e.g. .tmv) rather than a directory.
	bIsContainerFile = FTmvImgMediaReader::IsContainerFileExtensionSupported(FPaths::GetExtension(SequencePath))
		&& FPaths::FileExists(SequencePath);

	TSharedPtr<FTmvImgMediaReader, ESPMode::ThreadSafe> ContainerReader;

	if (bIsContainerFile)
	{
		// Container file: create the reader first, get frame count from the demuxer track info.
		ContainerReader = FTmvImgMediaReader::CreateReader(AsShared(), SequencePath);
		if (!ContainerReader.IsValid())
		{
			UE_LOGF(LogImgMedia, Error, "Failed to create container reader for %ls", *SequencePath);
			return false;
		}

		const FTmvMediaDemuxerTrackInfo* VideoTrackInfo = ContainerReader->GetVideoTrackInfo();

		if (!VideoTrackInfo)
		{
			UE_LOGF(LogImgMedia, Error, "Container reader for %ls has no video track.", *SequencePath);
			return false;
		}
		
		NumContainerFrames = static_cast<int32>(VideoTrackInfo->NumSamples);

		if (NumContainerFrames <= 0)
		{
			UE_LOGF(LogImgMedia, Error, "Container %ls has no frames", *SequencePath);
			return false;
		}
	}
	else
	{
		// File sequence: scan directory for individual frame files.
		TArray<FString> FoundPaths;
		FindFiles(SequencePath, FoundPaths);
		if (FoundPaths.Num() == 0)
		{
			UE_LOGF(LogImgMedia, Error, "The directory %ls does not contain any image files", *SequencePath);
			return false;
		}
		ImagePaths.Emplace(MoveTemp(FoundPaths));
	...
	// create image reader (if not already created for container path)
	if (!Reader.IsValid())
	{
		...
		if (FirstExtension == TEXT("exr"))
		{
			...
			Reader = FExrImgMediaReader::GetReader(AsShared(), ImagePaths[0][0], LayerName);
#else
			UE_LOGF(LogImgMedia, Error, "EXR image sequences are currently supported on macOS, Linux and Windows only");
			return false;
#endif
		}
		...
		else if (FTmvImgMediaReader::IsImageFileExtensionSupported(FirstExtension))
		{
			Reader = FTmvImgMediaReader::CreateReader(AsShared(), ImagePaths[0][0]);
		}
		else
		{
			Reader = MakeShareable(new FGenericImgMediaReader(AsShared()));
		}

		if (Reader.IsValid() == false)
		{
			UE_LOGF(LogImgMedia, Error, "Reader is not valid for %ls.", *ImagePaths[0][0]);
			return false;
		}
	}
	...
	/** Include LayerName in the cache key so that different layer selections
	 *  for the same sequence path map to separate cache buckets. Empty LayerName
	 *  uses the path alone, preserving the existing key for the default case.
	 * 
	 *  The layer name is stored as its hash rather than the raw string because
	 *  FName comparison is case-insensitive: embedding a uint32 keeps the key
	 *  case-sensitive (numbers are unaffected by FName's case folding).
	 */
	SequenceName = LayerName.IsEmpty()
		? FName(*SequencePath)
		: FName(*FString::Printf(TEXT("%s?layer=%u"), *SequencePath, UE::HashStringFNV1a32(MakeStringView(LayerName))));
	...
	// fetch sequence attributes from first image
	{
		...
		else if (!Reader->GetFrameInfo(bIsContainerFile ? SequencePath : ImagePaths[0][0], FirstFrameInfo))
		{
			UE_LOGF(LogImgMedia, Error, "Failed to get frame information from first image in %ls", *SequencePath);
			return false;
		}
	}
	...
	{
		UE_LOGF(LogImgMedia, Error, "The first image in sequence %ls does not have a valid frame size", *SequencePath);
		return false;
	}

	if (FirstFrameInfo.Dim.GetMin() <= 0)
	{
		UE_LOGF(LogImgMedia, Error, "The first image in sequence %ls does not have a valid dimension", *SequencePath);
		return false;
	}

	NumMipLevels = FMath::Max(FirstFrameInfo.NumMipLevels, bIsContainerFile ? 1 : ImagePaths.Num());
	NumBytesPerPixel = FirstFrameInfo.NumBytesPerPixel;
	...
	// Push the sequence info into the resolver so it's ready when providers register.
	{
		FMediaSequenceInfo SequenceInfo;
		SequenceInfo.Name = SequenceName;
		SequenceInfo.NumMipLevels = FMath::Max(1, NumMipLevels);
		SequenceInfo.Dim = SequenceDim;
		SequenceInfo.TilingDescription = TilingDescription;
		...
		TileVisibilityResolver->SetSequenceInfo(SequenceInfo);
	}
	...
	constexpr int32 MinNumberOfFramesToLoad = 2;
	...
	else
	{
		...
		const int32 NumImages = GetNumImages();
		const int32 MaxFramesToLoad = FMath::Min((int32)(CacheSize / UncompressedSize), NumImages);
		const int32 MinFramesToLoad = FMath::Min(MinNumberOfFramesToLoad, NumImages);

		if (Settings->bEnablePrecachingTimeInterval)
		{
			// Desired number of frames specified by a time interval, clamped by the max desired size.
			NumFramesToLoad = FMath::Clamp(Settings->PrecachingTimeInterval / SequenceFrameRate.AsInterval(), MinFramesToLoad, MaxFramesToLoad);
		}
		else
		{
			// Desired number of frames specified by the max desired size.
			NumFramesToLoad = FMath::Max(MaxFramesToLoad, MinFramesToLoad);
		}

		// Apply look ahead/behind ratio.
		const float LoadBehindScale = FMath::Clamp(Settings->CacheBehindPercentage, 0.0f, 100.0f) / 100.0f;
		NumLoadBehind = FMath::TruncToInt32(LoadBehindScale * NumFramesToLoad);
...
void FImgMediaLoader::FindFiles(const FString& SequencePath, TArray<FString>& OutputPaths)
{
	...
	UE_LOGF(LogImgMedia, Verbose, "Loader %p: Found %i image files in %ls", this, FoundFiles.Num(), *SequencePath);

	// Same list of extensions as FGenericImgMediaReader::GetFrameInfoFromImage() with the addition of exr & dds
	TSet<FString> SupportedImageFileExtensions = { TEXT("exr"), TEXT("jpg"), TEXT("jpeg"), TEXT("png"), TEXT("bmp"), TEXT("dds") };
	SupportedImageFileExtensions.Append(FTmvImgMediaReader::GetSupportedImageFileExtensions());
	...
	for (FString& File : FoundFiles)
	{
		...
		if (GetNumberAtEndOfString(FrameNumber, File, FileExtension))
		{
	...
	for (const auto& Pair : SortedFoundFiles)
	{
		...
		if (bFillGapsInSequence)
		{
			...
			if ((LastIndex != -1) && (LastIndex < ThisIndex - 1))
			{
				...
				{
					UE_LOGF(LogImgMedia, Warning,
						"Loader %p, sequence \"%ls\": Ignoring large frame gap (%d frames) from frame %d to %d." " If this is expected, adjust cvar \"ImgMedia.MaxFrameSequenceGap\" (current: %d) accordingly.",
...
void FImgMediaLoader::FindMips(const FString& SequencePath)
{
	...
	int32 Index = 0;
	const bool bFoundDelimeter = SequenceDir.FindLastChar(TEXT('x'), Index);
	if (bFoundDelimeter)
	{
		...
		while (MipLevelWidth > 1 && MipLevelHeight > 1)
		{
			...
			{
				UE_LOGF(LogImgMedia, Error, "Loader %p: Found %d images, expected %d, for %ls",
			...
			{
				UE_LOGF(LogImgMedia, Error, "Loader %p: Found too many mipmap levels (max:%d) for %ls",
	...
	if (ImagePaths.Num() > 1)
	{
		UE_CALL_ONCE([&] { UE_LOGF(LogImgMedia, Warning, "Loading mips from separate folders will be deprecated soon (%ls). Please prefer using TMV, EXR or DDS with mip levels.", *SequencePath) });
	}
}
...
void FImgMediaLoader::Update(int32 PlayHeadFrame, float PlayRate, bool Loop)
{
	...
	for (int32 Idx = QueuedFrameNumbers.Num() - 1; Idx >= 0; --Idx)
	{
		...
		{
			UE_LOGF(LogImgMedia, Verbose, "Loader %p: Removed Frame %i", this, FrameNumber);
	...
	for (int32 FrameNumber : FramesToLoad)
	{
		...
		else if(ImgMediaLoader::IsCachedFrameInRange(FrameNumber, PlayHeadFrame, FrameInvalidationMaxCount, NumImagePaths, PlayRate, bIsPaused))
		{
			...
			if (NeedFrame)
			{
				UE_LOGF(LogImgMedia, VeryVerbose, "Loader %p [%ls]: Invalidating frame %i at play head %i due to missing mips or tiles.", this, *PlayerGuid.ToString(), FrameNumber, PlayHeadFrame);
			}
		}
		...
		if (NeedFrame)
		{
			...
			if (!FoundFrame)
			{
				...
				// For file sequences, check if the path is empty (gap-filled placeholder).
				// Containers always have valid frames for all sample indices.
				if (!bIsContainerFile && ImagePaths[0][FrameNumber].Len() == 0)
				{
					TryAddEmptyFrame(FrameNumber);
				}
				else
...
{
	bool bFrameWasAdded = false;
	bool bFrameMatchesLastRequested = false;
	{
		FScopeLock Lock(&CriticalSection);
		...
		// Make sure the frame is no longer canceled, otherwise we might end up cancelling this frame
...
#if WITH_EDITOR
		// Update bandwidth. (Keep inside CriticalSection)
		...
		// Captured under the lock - LastRequestedFrame is written from the game thread without
		// synchronization with this worker callback, so the comparison must happen here.
		bFrameMatchesLastRequested = (FrameNumber == LastRequestedFrame);
	}

	// Tiles for the displayed frame just changed; signal the facade to drop its sample cache.
	// Outside CriticalSection because ReceiveMediaEvent takes its own locks on the facade side.
	// Note: a 1-frame sequence is functionally paused, so it will go through the paused refresh path.
	const bool bShouldRefresh = bIsPaused.load(std::memory_order_relaxed) || GetNumImages() == 1;
	if (bShouldRefresh && bFrameWasAdded && bFrameMatchesLastRequested
		&& EventSink != nullptr
		&& CVarImgMediaPausedRefreshEnabled.GetValueOnAnyThread())
	{
		EventSink->ReceiveMediaEvent(EMediaEvent::Internal_RequestVideoSampleRefresh);
	}
}
...
void FImgMediaLoader::AddFrameToCache(int32 FrameNumber, const TSharedPtr<FImgMediaFrame, ESPMode::ThreadSafe>& Frame)
{
	if (Frame.IsValid())
	{
		UE_LOGF(LogImgMedia, VeryVerbose, "Loader %p: Loaded frame %i", this, FrameNumber);
		if (UseGlobalCache)
		{
			if (GlobalCache->FindAndTouch(SequenceName, FrameNumber) == nullptr)
			{
				const FString& FrameKey = bIsContainerFile ? SequenceName.ToString() : ImagePaths[0][FrameNumber];
				GlobalCache->AddFrame(FrameKey, SequenceName, FrameNumber, Frame, MipsInSeparateFiles());
			}
		}
		else
		{
...
void FImgMediaLoader::GetDesiredMipTiles(int32 FrameIndex, TMap<int32, FMediaTileSelection>& OutMipsAndTiles) const
{
	// note: While currently unused, FrameIndex could become useful for recorded frames.

	if (HasMipsOrTilesEnabled())
	{
		if (const TSharedPtr<FMediaTileVisibilityRequest, ESPMode::ThreadSafe> Snapshot = TileVisibilityResolver->GetSnapshot())
		{
			OutMipsAndTiles = Snapshot->VisibleTiles;
			return;
		}
	}
	...
	{
		OutMipsAndTiles.Emplace(MipLevel, FMediaTileSelection::CreateForTargetMipLevel(SequenceDim, TilingDescription.TileSize, MipLevel, true));
	}
}

int32 FImgMediaLoader::GetDesiredMinimumMipLevelToUpscale() const
{
	if (HasMipsOrTilesEnabled())
	{
		return TileVisibilityResolver->GetMinimumMipLevelToUpscale();
	}
...
FTimespan FImgMediaLoader::ModuloTime(FTimespan Time) const
{
...
bool FImgMediaLoader::GetNumberAtEndOfString(int32 &Number, const FString& String, const FString& Extension) const
{
	bool bFoundNumber = false;

	// Find the first digit starting from the right, excluding the extension (in case it has a number).
	int32 Index = String.Len() - 1 - Extension.Len();
	for (; Index >= 0; --Index)
	{
		const bool bIsDigit = FChar::IsDigit(String[Index]);
		if (bIsDigit)
		{
			break;
		}
...
int32 FImgMediaLoader::GetSkipFrame(int32 FrameIndex) const
{
	int32 Mult = (1 << SkipFramesLevel);
	int32 Mask = ~(Mult - 1);
