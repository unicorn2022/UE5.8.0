...
public:
	...
	/** (EXR only) Name of the layer to read. Empty reads the default (un-prefixed) channels. Supports wildcards (* and ?). */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category=Sequence, AdvancedDisplay, meta=(DisplayName="Layer Name (EXR)"))
	FString LayerName;
...
public:
	...
	/**
	 * Deprecated. Tile-visibility registration has moved to FMediaTextureTracker; consumers
	 * populate FMediaTextureTrackerObject::TileVisibilityProvider (or the legacy enum fields)
	 * directly instead of registering actors against the source. This function is now a no-op.
	 *
	 * @param InActor Ignored.
	 */
	UFUNCTION(BlueprintCallable, Category = "ImgMedia|ImgMediaSource", meta = (DeprecatedFunction, DeprecationMessage = "AddTargetObject is no longer needed. Tile-visibility consumers register through FMediaTextureTracker."))
	UE_DEPRECATED(5.8, "AddTargetObject is no longer needed. Tile-visibility consumers register through FMediaTextureTracker.")
	UE_API void AddTargetObject(AActor* InActor);

	/**
	 * Deprecated. Pair to AddTargetObject; now a no-op for the same reason.
	 *
	 * @param InActor Ignored.
	 */
	UFUNCTION(BlueprintCallable, Category = "ImgMedia|ImgMediaSource", meta = (DeprecatedFunction, DeprecationMessage = "RemoveTargetObject is no longer needed. Tile-visibility consumers register through FMediaTextureTracker."))
	UE_DEPRECATED(5.8, "RemoveTargetObject is no longer needed. Tile-visibility consumers register through FMediaTextureTracker.")
