...
namespace
{
	bool ReadBytes(FILE* FileHandle, char* Buffer, int32 Length)
	{
		...
		{
			UE_LOGF(LogExrReaderGpu, Error, "Error reading bytes");
			return false;
		}
		
		if (Result != Length)
		{
			UE_LOGF(LogExrReaderGpu, Error, "Error reading file %i %i", ferror(FileHandle), feof(FileHandle));
			return false;
		}
		return true;
	}
	...
	bool CheckIsNullTerminated(const char(&StringToCheck)[Length])
	{
		...
		UE_LOGF(LogExrReaderGpu, Error, "Invalid EXR file with string that was never terminated");
		return false;
	}
...
bool FExrReader::ReadMagicNumberAndVersionField(FILE* FileHandle)
{
	...
	{
		UE_LOGF(LogExrReaderGpu, Error, "Invalid EXR file has been detected.");
		return false;
	}
...
bool FExrReader::ReadHeaderData(FILE* FileHandle)
{
	...
	while (true)
	{
		...
		{
			UE_LOGF(LogExrReaderGpu, Error, "Issue reading attribute from EXR: %s", AttributeName);
			return false;
		}
	}
	return true;
...
bool FExrReader::ReadLineOrTileOffsets(FILE* FileHandle, ELineOrder LineOrder, TArray<int64>& LineOrTileOffsets)
{
	...
	{
		UE_LOGF(LogExrReaderGpu, Error, "Unsupported Line order in EXR: %d", LineOrder);
		return false;
	}
	...
	, int64 PixelSize)
{
	...
	{
		int32 MipLevelDiv = 1 << MipLevel;

		// Resolution of the texture in pixels for this mip level.
		FIntPoint MipResolution = (FullTextureResolution / MipLevelDiv).ComponentMax(FIntPoint(1,1));
...
bool FExrReader::OpenExrAndPrepareForPixelReading(FString FilePath, const TArray<int32>& NumOffsetsPerLevel)
{
	...
	{
		UE_LOGF(LogExrReaderGpu, Error, "The file has already been open for reading but never closed.");
		return false;
	}
...
bool FExrReader::ReadExrImageChunk(void* Buffer, int64 ChunkSize)
{
	if (FileHandle == nullptr)
	{
		UE_LOGF(LogExrReaderGpu, Error, "File is not open for reading. Please use OpenExrAndPrepareForPixelReading. ");
		return false;
	}
	if (Buffer == nullptr)
	{
		UE_LOGF(LogExrReaderGpu, Error, "Buffer provided is invalid. Please provide a valid buffer. ");
		return false;
	}
	...
	{
		UE_LOGF(LogExrReaderGpu, Error, "Issue reading EXR chunk. ");
	}
...
bool FExrReader::SeekTileWithinFile(const int32 StartTileIndex, const int32 Level, int64& OutBufferOffset)
{
	...
	{
		UE_LOGF(LogExrReaderGpu, Error, "Tile index is invalid.");
		return false;
	}
