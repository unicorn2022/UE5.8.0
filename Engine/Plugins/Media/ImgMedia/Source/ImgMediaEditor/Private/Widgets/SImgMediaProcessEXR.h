...
private:
	...
	/**
	 * Handles media events from the player (e.g. MediaOpenFailed).
	 *
	 * @param InEvent The media event received from the player.
	 */
	void HandleMediaPlayerEvent(EMediaEvent InEvent);

	/**
	 * Cancels processing and reports an error in the notification and log.
	 *
	 * @param InMessage Human-readable error message.
	 */
	void CancelWithError(const FText& InMessage);
	...
	/** Handle for OnMediaEvent subscription on MediaPlayer. */
	FDelegateHandle MediaEventHandle;
	...
	/** Current frame index we are processing. */
	int32 CurrentFrameIndex = 0;
	/** Wall-clock time of the last forward progress (frame processed or player ready). Used to time out stuck players. */
	double LastProgressTimeSeconds = 0.0;
	...
	/**
	 * Counter for the number of "async tasks" that are currently running.
	 * This used by the main thread to protect against the widget being deleted while in use by the async tasks.
	 */
	std::atomic<int32> NumAsyncTasks = 0;
};
