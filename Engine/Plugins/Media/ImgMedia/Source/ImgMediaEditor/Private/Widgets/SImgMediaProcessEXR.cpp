...
namespace UE::ImgMediaEditor::ProcessEXR
{
	/** Maximum wall-clock seconds to wait for the media player to deliver the next frame before aborting. */
	static TAutoConsoleVariable<float> CVarProgressTimeoutSeconds(
		TEXT("ImgMedia.ProcessEXR.ProgressTimeoutSeconds"),
		30.0f,
		TEXT("Wall-clock seconds the Process EXR widget will wait for the media player to deliver the next frame before aborting with an error. Set <= 0 to disable the watchdog."),
		ECVF_Default);

	/**
	 * Helper function to set the Notification text from a worker thread.
	 * @param InNotificationWeak Weak reference to the Notification object.
	 * @param InText Text to set.
	 */
	void SetNotificationTextSafe(const TWeakPtr<SNotificationItem>& InNotificationWeak, const FText& InText)
	{
		Async(EAsyncExecution::TaskGraphMainThread, [NotificationWeak = InNotificationWeak, Text = InText]()
		{
			if (TSharedPtr<SNotificationItem> Notification = NotificationWeak.Pin())
			{
	...
	/**
	 * Helper function to set the Notification text from a worker thread.
	 * @param InNotification Reference to the Notification object.
	 * @param InText Text to set.
	 */
	void SetNotificationTextSafe(const TSharedPtr<SNotificationItem>& InNotification, const FText& InText)
	{
		if (InNotification.IsValid())
		{
			SetNotificationTextSafe(InNotification.ToWeakPtr(), InText);
		}
	}
}
...
SImgMediaProcessEXR::~SImgMediaProcessEXR()
{
	...
	// Check if we have ongoing async threads preventing us from deleting the widget.
	if (NumAsyncTasks.load() > 0)
	{
		// Signal tasks to stop.
		bIsCancelling = true;

		// Wait for all async tasks to complete.
		while (NumAsyncTasks.load() > 0)
		{
		...
		if (ConfirmNotification.IsValid())
		{
			...
			ConfirmNotification->SetCompletionState(SNotificationItem::CS_Fail);
...
FReply SImgMediaProcessEXR::OnProcessImagesClicked()
{
	if (bIsProcessing == false)
	{
		...
		if (bUsePlayer)
		{
			...
			MediaEventHandle = MediaPlayer->OnMediaEvent().AddRaw(this, &SImgMediaProcessEXR::HandleMediaPlayerEvent);
			...
			{
				CancelWithError(FText::Format(
					LOCTEXT("FailedToSpawnMediaSource", "Failed to create a media source for \"{0}\". Verify that the path is valid and the file format is supported by an installed media player."),
					FText::FromString(Options->InputPath.FilePath)));
			...
			LastProgressTimeSeconds = FPlatformTime::Seconds();
		...
		{
			TWeakPtr<SImgMediaProcessEXR> WeakSelf = TSharedRef<SImgMediaProcessEXR>(AsShared(), this).ToWeakPtr();

			// Start async task to process files.
			Async(EAsyncExecution::Thread, [WeakSelf]()
			{
				// Self is only pinned temporarily for thread safety, then released to allow the job to be cancelled (by calling destructor)
				// if the panel is closed. For the remainer of the scope, we protect the validity of the Self pointer with NumAsyncTasks
				// counter instead (see destructor).
				SImgMediaProcessEXR* SelfPtr;
				{
					const TSharedPtr<SImgMediaProcessEXR> Self = WeakSelf.Pin();
					if (!Self)
					{
						return;
					}
					++Self->NumAsyncTasks;	// This will protect from destruction while async task is going.
					SelfPtr = Self.Get();	// Keep raw pointer for the remainer of the scope.
				}

				SelfPtr->ProcessAllImages();
				--SelfPtr->NumAsyncTasks;

				// Close notification. Must be run on the main thread.
				Async(EAsyncExecution::TaskGraphMainThread, [WeakSelf]()
				{
					if (TSharedPtr<SImgMediaProcessEXR> Self = WeakSelf.Pin())
					{
						if (Self->ConfirmNotification.IsValid())
						{
							Self->ConfirmNotification->SetEnabled(false);
							Self->ConfirmNotification->SetCompletionState((Self->bIsCancelling) ? SNotificationItem::CS_Fail : SNotificationItem::CS_Success);
							Self->ConfirmNotification->ExpireAndFadeout();
						}

						// Done with processing.
						Self->bIsProcessing = false;
						Self->bIsCancelling = false;
						Self->UpdateWidgets();
					}
				});
			});
...
{
	using namespace UE::ImgMediaEditor::ProcessEXR;
	...
	if (!FPaths::DirectoryExists(OutPath))
	{
		...
		{
			CancelWithError(FText::Format(LOCTEXT("FailedToCreateOutputTree", "Failed to create output directory \"{0}\""), FText::FromString(OutPath)));
			return;
		}
	}

	// Get source files.
	const FString InputPath = FPaths::GetPath(Options->InputPath.FilePath);
	if (InputPath.IsEmpty())
	{
		CancelWithError(FText::Format(LOCTEXT("InvalidInputPath", "Specified input path \"{0}\" is invalid."), FText::FromString(Options->InputPath.FilePath)));
		return;
	}

	const FString SequencePath = FPaths::ConvertRelativePathToFull(InputPath);
	if (!FPaths::DirectoryExists(SequencePath))
	{
		CancelWithError(FText::Format(LOCTEXT("InvalidSequencePath", "Specified input directory directory \"{0}\" does not exist."), FText::FromString(SequencePath)));
		return;
	}
	...
	FoundFiles.Sort();
	UE_LOGF(LogImgMediaEditor, Warning, "Found %i image files in %ls to import.", FoundFiles.Num(), *SequencePath);
	if (FoundFiles.Num() == 0)
	{
		CancelWithError(LOCTEXT("NoFilesToImport", "No files to import."));
		return;
	}
	...
	{
		CancelWithError(FText::Format(LOCTEXT("InvalidFileFormat", "Invalid file format \"{0}\""), FText::FromString(Ext)));
		return;
	}
	...
	for (const FString& FileName : FoundFiles)
	{
		...
		SetNotificationTextSafe(ConfirmNotification,
			FText::Format(LOCTEXT("ImgMediaCompleted", "ImgMedia Completed {0}/{1}"), FText::AsNumber(FileIndex+1), FText::AsNumber(TotalNum)));
			...
			TileBorder, bEnableMips]() mutable
		{
			...
			// Load image into buffer.
			{
				...
				{
					UE_LOGF(LogImgMediaEditor, Error, "Failed to load %ls", *FullFileName);
					--NumActive;
					return;
				}
			}
	...
	const FImageParameters& ImageParameters, const FString& InName)
{
	...
	UE_LOGF(LogImgMediaEditor, Error, "EXR not supported on this platform.");
#endif // IMGMEDIAEDITOR_EXR_SUPPORTED_PLATFORM
}
	...
	int32 BytesPerPixel)
{
	...
	{
		UE_LOGF(LogImgMediaEditor, Error, "Tile border is larger than tile size. Clamping to tile size.");
...
void SImgMediaProcessEXR::HandleProcessing()
{
	...
	if (bIsProcessing)
	{
		...
		if ((MediaPlayer != nullptr) && (bIsCancelling == false))
		{
			...
			UE_LOGF(LogImgMediaEditor, Verbose,
				"ProcessImages Time:%f PlayerTime:%f Duration:%f Frame:%d",
			...
			if (MediaPlayer->IsClosed() || MediaPlayer->HasError())
			{
				CancelWithError(LOCTEXT("ErrorPlayerStopped", "Player has stopped playing"));
				bShouldExit = true;
			}
			// Watchdog: have we made no progress for too long?
			else if (const float ProgressTimeoutSeconds = CVarProgressTimeoutSeconds.GetValueOnGameThread();
				(ProgressTimeoutSeconds > 0.0f) && ((FPlatformTime::Seconds() - LastProgressTimeSeconds) > ProgressTimeoutSeconds))
			{
				CancelWithError(FText::Format(
					LOCTEXT("ErrorPlayerStalled", "Media player did not produce a frame for {0} seconds (frame {1} of {2}). Aborting."),
					FText::AsNumber((int32)ProgressTimeoutSeconds),
					FText::AsNumber(CurrentFrameIndex),
					FText::AsNumber((int32)(MediaPlayer->GetDuration().GetTotalSeconds() / FMath::Max(FrameDuration.GetTotalSeconds(), KINDA_SMALL_NUMBER)))));
				bShouldExit = true;
			}
			...
			else if ((MediaPlayer->IsPreparing() == false) && (CurrentFrameIndex == PlayerFrame))
			{
				...
				if (RenderTarget != nullptr)
				{
					...
					{
						UE_LOGF(LogImgMediaEditor, Error, "ProcessImages failed to get raw data.");
					}
...
void SImgMediaProcessEXR::HandleMediaPlayerEvent(EMediaEvent InEvent)
{
	using namespace UE::ImgMediaEditor::ProcessEXR;

	const FString EventName = MediaUtils::EventToString(InEvent);
	UE_LOGF(LogImgMediaEditor, Verbose, "ProcessEXR: Media event received: %ls", *EventName);

	switch (InEvent)
	{
	case EMediaEvent::MediaOpenFailed:
		CancelWithError(FText::Format(
			LOCTEXT("MediaOpenFailed", "Failed to open media source \"{0}\". Check that the path exists and the format is supported."),
			FText::FromString(Options.IsValid() ? Options->InputPath.FilePath : FString())));
		break;

	case EMediaEvent::MediaClosed:
		// Only treat as an error if we were actively processing (not the normal end-of-run close).
		if (bIsProcessing && !bIsCancelling)
		{
			CancelWithError(LOCTEXT("MediaClosedUnexpectedly", "Media source was closed unexpectedly during processing."));
		}
		break;
...
void SImgMediaProcessEXR::CancelWithError(const FText& InMessage)
{
	using namespace UE::ImgMediaEditor::ProcessEXR;

	UE_LOGF(LogImgMediaEditor, Error, "ProcessEXR: %ls", *InMessage.ToString());
	SetNotificationTextSafe(ConfirmNotification, InMessage);
	bIsCancelling = true;
}
...
void SImgMediaProcessEXR::CreateRenderTarget()
{
	...
	{
		CancelWithError(FText::Format(
			LOCTEXT("InvalidVideoDimensions", "Media player reported invalid video dimensions ({0} x {1}). The media may not have a video track or the player has not finished preparing."),
			FText::AsNumber(VideoDim.X), FText::AsNumber(VideoDim.Y)));
		return;
	}

	RenderTarget = NewObject<UTextureRenderTarget2D>(GetTransientPackage(), TEXT("ImgMediaProcessImages"));
	RenderTarget->RenderTargetFormat = RTF_RGBA16f;
	RenderTarget->InitAutoFormat(VideoDim.X, VideoDim.Y);
...
void SImgMediaProcessEXR::CleanUp()
{
	...
	{
		if (MediaEventHandle.IsValid())
		{
			MediaPlayer->OnMediaEvent().Remove(MediaEventHandle);
			MediaEventHandle.Reset();
		}
		MediaPlayer->Close();
