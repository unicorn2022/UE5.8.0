...
public:
	...
	/**
	 * Maximum memory size of the pre-caching sliding interval (in GB; default = 1 GB).
	 * The pre-caching sliding interval can further be configured by specifying
	 * a time interval (see PrecachingTimeInterval). In that case, the effective
	 * interval will be the minimum of those two settings (size in GB or time span).
	 */
	UPROPERTY(config, EditAnywhere, Category=Caching, meta=(ClampMin=0))
	float CacheSizeGB;

	/**
	 * Enables pre-caching time interval strategy.
	 */
	UPROPERTY(config, EditAnywhere, Category=Caching, meta=(InlineEditConditionToggle))
	bool bEnablePrecachingTimeInterval;

	/**
	 * Specify the pre-caching sliding time interval, in seconds.
	 * The player will pre-cache the specified number of frames corresponding to the time interval relative to the play head.
	 * The cache behind percentage still applies to determine how much frames to pre-cache behind vs ahead of the play head.
	 */
	UPROPERTY(EditAnywhere, Category=Caching, meta=(ClampMin=0, EditCondition="bEnablePrecachingTimeInterval"))
	float PrecachingTimeInterval;
