...
/**
 * Reads EXR files with per-channel control using the general Imf::InputFile API.
 * Unlike FRgbaInputFile, this allows selecting specific named channels for each RGBA output slot.
 */
class FExrInputFile
{
public:

	UE_API FExrInputFile(const FString& FilePath, uint16 ThreadCount);
	UE_API ~FExrInputFile();
...
public:
	 ...
	 *
	 * Each channel parameter is a full EXR channel name (e.g., "R", "beauty.G", "Zdepth.Z").
	 * Empty string means fill with a default value (0 for RGB slots, 1 for A slot).
	 * The same channel name may be used for multiple slots (e.g., all RGB = "Zdepth.Z" for grayscale).
	 *
	 * @param Buffer		Destination buffer, must hold BufferDim.X * BufferDim.Y * 4 * sizeof(uint16) bytes.
	 * @param BufferDim		Width and height of the buffer (used for stride calculation).
	 * @param ChannelR		EXR channel name to read into the R slot.
	 * @param ChannelG		EXR channel name to read into the G slot.
	 * @param ChannelB		EXR channel name to read into the B slot.
	 * @param ChannelA		EXR channel name to read into the A slot.
	 * @param StartY		First scanline to read (0-based, relative to data window).
	 * @param EndY			Last scanline to read (inclusive, 0-based, relative to data window).
	 */
	UE_API void ReadPixels(
		void* Buffer,
...
public:
	...
	UE_API FIntPoint GetDataWindow() const;
	UE_API FFrameRate GetFrameRate(const FFrameRate& DefaultValue) const;
	UE_API int64 GetUncompressedSize() const;
	UE_API int64 GetUncompressedMipSize(int32 InMipLevel) const;
	UE_API int32 GetNumChannels() const;
	UE_API int32 GetPixelSize() const;
	UE_API bool ContainsMips() const;
	UE_API int32 GetNumMipLevels() const;
	...
	/**
	 * Get the names of all channels in the file.
	 *
	 * @param OutChannelNames Will contain all channel names (e.g., "R", "G", "B", "Zdepth.Z").
	 */
	UE_API void GetChannelNames(TArray<FString>& OutChannelNames) const;
