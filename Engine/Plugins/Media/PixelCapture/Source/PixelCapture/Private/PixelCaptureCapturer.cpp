...
namespace UE::PixelCapture
{
	void MarkCPUWorkStart(TSharedPtr<IPixelCaptureOutputFrame> OutputBuffer)
	{
		if (!OutputBuffer)
		{
			UE_LOGF(LogPixelCapture, Warning, "Unable to update metadata on a null output buffer!");
			return;
		}
...
void FPixelCaptureCapturer::Capture(const IPixelCaptureInputFrame& InputFrame)
{
	...
	if (!OutputBuffer)
	{
		UE_LOGF(LogPixelCapture, Error, "Failed to obtain a produce buffer.");
		return;
	}
...
void FPixelCaptureCapturer::EndProcess(TSharedPtr<IPixelCaptureOutputFrame> OutputBuffer)
{
	if (!OutputBuffer)
	{
		UE_LOGF(LogPixelCapture, Warning, "Unable to end process on a null output buffer!");
		return;
	}
