...
{
	// OpenGL needs PF_R8G8B8A8
	EPixelFormat PixelFormat = RHIGetInterfaceType() == ERHIInterfaceType::OpenGL ? PF_R8G8B8A8 : PF_B8G8R8A8;

	FRHITextureCreateDesc TextureDesc =
		FRHITextureCreateDesc::Create2D(TEXT("FPixelCaptureCapturerI420ToRHI Texture"), InputWidth, InputHeight, PixelFormat)
...
#if !PLATFORM_ANDROID
	// TODO (william.belcher): Android doesn't seem to like the External/Shared flags - investigate further
	...
	return new FPixelCaptureOutputFrameRHI(RHICmdList.CreateTexture(TextureDesc));
}
...
void FPixelCaptureCapturerI420ToRHI::BeginProcess(const IPixelCaptureInputFrame& InputFrame, TSharedPtr<IPixelCaptureOutputFrame> OutputBuffer)
{
	...
	if (InputFrame.GetType() == StaticCast<int32>(PixelCaptureBufferFormat::FORMAT_I420))
	{
		...
		uint32 FourCC = (RHIGetInterfaceType() == ERHIInterfaceType::OpenGL) ? libyuv::FOURCC_ABGR : libyuv::FOURCC_ARGB;
