...
static const FLazyName DrawerId = "Library";

class FLibraryDrawerConfig : public ISidebarDrawerContent
{
public:
	FLibraryDrawerConfig(TSharedRef<SMediaViewerLibraryPrivate> InLibraryWidget)
		: LibraryWidget(InLibraryWidget)
	{
	}
	...
	virtual FName GetSectionId() const
	{
		return DrawerId;
	}

	virtual FText GetSectionDisplayText() const
	{
		return LOCTEXT("Library", "Library");
	}
	...
	virtual TOptional<float> GetSectionFillHeight() const
	{
		return 1.f;
	}
	...
	virtual TSharedRef<SWidget> CreateContentWidget()
	{
		return LibraryWidget;
	}

private:
	TSharedRef<SMediaViewerLibraryPrivate> LibraryWidget;
};
...
{
	// Warm shader caches now so the editor isn't stalled the first time we render with
	// these materials in user-facing paths. No-op after the first call.
	UE::MediaViewer::Private::FMediaViewerUtils::PreloadMaterialAssets();
	...
	LibraryDrawerConfig.InitialState = SidebarState.FindOrAddDrawerState(DrawerId.Resolve());

	Sidebar->RegisterDrawer(MoveTemp(LibraryDrawerConfig));
	Sidebar->RegisterDrawerSection(DrawerId, MakeShared<FLibraryDrawerConfig>(Library.ToSharedRef()));
...
void SMediaViewer::BindCommands()
{
	...
	CommandList->MapAction(
		...
		FExecuteAction::CreateSP(this, &SMediaViewer::Mirror, EMediaImageMirrorPlane::Horizontal)
	);

	CommandList->MapAction(
		Commands.MirrorVertical,
		FExecuteAction::CreateSP(this, &SMediaViewer::Mirror, EMediaImageMirrorPlane::Vertical)
	);

	CommandList->MapAction(
		Commands.MirrorDepth,
		FExecuteAction::CreateSP(this, &SMediaViewer::Mirror, EMediaImageMirrorPlane::Depth)
	);
...
void SMediaViewer::CreateDelegates()
{
	...
	Delegates->MirrorAll.BindSP(this, &SMediaViewer::MirrorAll);
...
void SMediaViewer::MirrorAll(EMediaImageMirrorPlane InPlane, bool bInSet)
{
	for (int32 Index = 0; Index < static_cast<int32>(EMediaImageViewerPosition::COUNT); ++Index)
	{
		ImageViewers[Index]->SetMirrored(InPlane, bInSet);
	}
}
...
void SMediaViewer::TakeSnapshot()
{
	...
	Texture->Rename(*AssetName, Package, REN_DontCreateRedirectors | REN_DoNotDirty | REN_AllowPackageLinkerMismatch | REN_NonTransactional);
...
void SMediaViewer::OpenBookmark(int32 InIndex)
{
	...
	UE_LOGF(LogMediaViewer, Log, "Bookmark %i loaded.", InIndex + 1);
...
void SMediaViewer::SaveBookmark(int32 InIndex)
{
	...
	UE_LOGF(LogMediaViewer, Log, "Bookmark %i saved.", InIndex + 1);
...
void SMediaViewer::Mirror(EMediaImageMirrorPlane InPlane)
{
	const FGuid& NullImageId = FNullImageViewer::GetNullImageViewer()->GetInfo().Id;
