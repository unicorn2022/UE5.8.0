...
public:
	...
	/** Either mirror this viewer or all viewers if transforms are locked. */
	void Try_Mirror(EMediaImageMirrorPlane InPlane);
...
protected:
	...
	ECheckBoxState IsAutoFilteringEnabled() const;
	void ToggleAutoFiltering();

	bool CanTogglePixelGrid() const;
	ECheckBoxState IsPixelGridEnabled() const;
	...
	/** Interact with the image viewer's mirror settings. */
	ECheckBoxState GetMirroredState(EMediaImageMirrorPlane InPlane) const;
	void Mirror(EMediaImageMirrorPlane InPlane);
