...
public:
	...
	/**
	 * Constructs a viewer for InTexture but overrides the display name shown in the viewer
	 * UI with InDisplayName. Use this when the texture's UObject name (which cannot contain
	 * dots or other characters allowed in file names) does not match the label the user
	 * expects to see.
	 */
	FTexture2DImageViewer(TNotNull<UTexture2D*> InTexture, const FText& InDisplayName);
