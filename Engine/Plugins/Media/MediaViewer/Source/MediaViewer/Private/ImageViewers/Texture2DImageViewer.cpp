...
FTexture2DImageViewer::FTexture2DImageViewer(TNotNull<UTexture2D*> InTexture, const FText& InDisplayName)
	: FTexture2DImageViewer(InTexture)
{
	ImageInfo.DisplayName = InDisplayName;
}
...
TOptional<TVariant<FColor, FLinearColor>> FTexture2DImageViewer::GetPixelColor(const FIntPoint& InPixelCoords, int32 InMipLevel) const
{
	...
	if (const FLinearColor* PixelColor = SampleCache->GetPixelColor(MirroredPixelCoords))
	{
		TVariant<FColor, FLinearColor> PixelColorVariant;
		PixelColorVariant.Set<FLinearColor>(FMediaViewerUtils::CorrectGamma(*PixelColor));
...
void FTexture2DImageViewer::NotifyPostChange(const FPropertyChangedEvent& InPropertyChangedEvent, FProperty* InPropertyThatChanged)
{
...
void FTexture2DImageViewer::OnMirrored()
{
