...
namespace LevelEditorViewport
{

	const FGuid ViewportGuids[4] = {
		FGuid(0xFDD736C4, 0xFD7A98CB, 0x7C28F833, 0xD00B4BB3),
		FGuid(0xB7929682, 0x749CED27, 0x357D74D5, 0xC5E19053),
		FGuid(0xD82E7C42, 0xB038A8A1, 0x9C608700, 0x9C3B589C),
		FGuid(0xF0FE7988, 0xE5DE498B, 0x4AC6026B, 0x255B3BEC)
	};

	int32 GetTabIndex(const FString& InValue)
	{
		if (InValue.Len() > 1 && InValue[1] == TEXT('_')
			&& InValue[0] >= TEXT('0') && InValue[0] <= TEXT('3'))
		{
			return InValue[0] - TEXT('0');
		}

		return 0;
	}

	int32 GetViewportIndex(const FString& InValue)
	{
		const int32 LastIndex = InValue.Len() - 1;

		for (int32 Index = LastIndex; Index >= 0; --Index)
		{
			if (InValue[Index] >= TEXT('0') && InValue[Index] <= TEXT('9'))
			{
				continue;
			}
			...
			// We've found a non-number
			return FCString::Atoi(*InValue.Mid(Index + 1));
		}

		// Whole string is a number... somehow!
 ...
 * Now also processed tab index (X_...)
 */
...
{
	using namespace LevelEditorViewport;

	const int32 TabIndex = GetTabIndex(InViewportValue);
	const int32 ViewportIndex = GetViewportIndex(InViewportValue);

	return FText::FromString(FString(TEXT("Tab")) + FString::FromInt(TabIndex + 1)
		+ TEXT(":")
		+ FString(TEXT("Viewport")) + FString::FromInt(ViewportIndex + 1));
}
...
FGuid FLevelEditorViewportImageViewer::FItem::GetIdForViewport(const FString& InConfigKey, bool bCreateIfInvalid)
{
	if (!InConfigKey.IsEmpty())
	{
		...
		const int32 TabIndex = FMath::Clamp(GetTabIndex(InConfigKey), 0, 3);
		const int32 ViewportIndex = FMath::Clamp(GetViewportIndex(InConfigKey), 0, 3);

		if (TabIndex == 0)
		{
			return ViewportGuids[ViewportIndex];
		}

		FGuid ViewportGuid = ViewportGuids[ViewportIndex];
		ViewportGuid.D += TabIndex;
...
TSharedPtr<FSceneViewport> FLevelEditorViewportImageViewer::FItem::GetViewportFromStringValue(const FString& InStringValue)
{
	...
	const int32 TabIndex = GetTabIndex(InStringValue);
	const int32 ViewportIndex = GetViewportIndex(InStringValue);

	if (!Viewports.Contains(TabIndex))
	{
		return nullptr;
	}

	if (!Viewports[TabIndex].IsValidIndex(ViewportIndex))
	{
		return nullptr;
	}

	return Viewports[TabIndex][ViewportIndex]->GetSharedActiveViewport();
}
