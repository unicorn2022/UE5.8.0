...
void FMediaViewerCommands::RegisterCommands()
{
	...
	UI_COMMAND(
		...
		"Toggles automatic filtering. When enabled, bilinear filtering is disabled once the zoom level reaches the configured threshold, allowing individual pixels to be seen clearly.",
		EUserInterfaceActionType::ToggleButton,
		FInputChord(EKeys::I)
	);
	...
	UI_COMMAND(
		TogglePixelGrid,
		"Display Pixel Grid",
		"Toggles the pixel grid overlay. When enabled, a grid is drawn over the image at high zoom levels to make individual pixel boundaries clearly visible.",
		EUserInterfaceActionType::ToggleButton,
		FInputChord(EKeys::G)
	);
	...
	UI_COMMAND(
		MirrorDepth,
		"Mirror Depth",
		"Mirror the image on the depth axis.",
		EUserInterfaceActionType::ToggleButton,
		FInputChord(EKeys::B)
	);
