	...
	, CodecSurfaceReadEvent(nullptr)
{
	if (FAndroidMisc::ShouldUseVulkan()) // GpuOutputPath is only available for Vulkan right now (and experimental)
	{
		...
		{
			UE_LOGF(LogElectraSamples, Log, "Selecting CPU path because GPU path is disabled via Electra.AndroidUseGpuOutputPath = 0");
		}
		else
		{
			...
			else
			{
				...
				UE_LOGF(LogElectraSamples, Log, "Selecting GPU path because it is enabled via Electra.AndroidUseGpuOutputPath = 1");
			}
		}
	}
	else
	{
		UE_LOGF(LogElectraSamples, Log, "Selecting CPU path because we are on OES");
	}
	...
	FImageResources_ReleaseFN = FindMethod(JEnv, FImageResourcesClass, "Release", "()V", false);
	...
	if (IsInGameThread())
	{
		...
		ENQUEUE_RENDER_COMMAND(DestroyElectraTextureSample)([this](FRHICommandListImmediate& RHICmdList)
		{
			RHICmdList.EnqueueLambda([this](FRHICommandListImmediate& CmdList)
			{
				...
				JEnv->DeleteGlobalRef(FImageResourcesClass);
				JEnv->DeleteGlobalRef(FFrameUpdateInfoClass);
				CodecSurfaceToDelete = nullptr;
				FImageResourcesClass = nullptr;
				FFrameUpdateInfoClass = nullptr;
			});
		});
		FlushRenderingCommands();
	}
...
FTextureRHIRef FElectraTextureSample::InitializeTextureOES(AHardwareBuffer* HardwareBuffer)
{
	...
	RHICmdList.EnqueueLambda([WeakThis{ AsWeak() }, &InDstTexture](FRHICommandListImmediate& CmdList)
	{
		...
		if (TSharedPtr<FElectraTextureSample, ESPMode::ThreadSafe> This = WeakThis.Pin())
		{
			...
			{
				UE_LOGF(LogElectraSamples, Warning, "Could not get native client buffer!");
				return;
			}
			...
			if (EglImage == 0)
			{
				UE_LOGF(LogElectraSamples, Warning, "Could not create EGLimage from native client buffer! B=0x%x E=0x%x", NativeClientBuffer, eglGetError());
				return;
			}
...
void FElectraTextureSample::CleanupImageResources()
{
	if (ImageResources)
	{
		...
		{
			FRHICommandListImmediate::Get().EnqueueLambda([Resources, ReleaseFN](FRHICommandListImmediate&)
			{
				CleanupImageResourcesJNI(Resources, ReleaseFN);
			});
		}
