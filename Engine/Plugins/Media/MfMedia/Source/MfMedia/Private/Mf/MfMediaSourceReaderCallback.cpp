...
{
	UE_LOGF(LogMfMedia, VeryVerbose, "Callback %p: Destroyed)", this);
}
...
STDMETHODIMP FMfMediaSourceReaderCallback::OnEvent(DWORD, IMFMediaEvent* Event)
{
	...
	MediaEventType EventType = MEUnknown;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogMfMedia, VeryVerbose, "Callback %p: Failed to get session event type: %ls", this, *MfMedia::ResultToString(Result));
			return Result;
		}
	}
	...
	HRESULT EventStatus = S_FALSE;
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogMfMedia, VeryVerbose, "Callback %p: Failed to get event status: %ls", this, *MfMedia::ResultToString(Result));
			return Result;
		}
	}

	UE_LOGF(LogMfMedia, VeryVerbose, "Callback %p: Event [%ls]: %ls", this, *MfMedia::MediaEventToString(EventType), *MfMedia::ResultToString(EventStatus));
...
STDMETHODIMP FMfMediaSourceReaderCallback::OnReadSample(HRESULT Status, DWORD StreamIndex, DWORD StreamFlags, LONGLONG Timestamp, IMFSample* Sample)
{
		...
		UE_LOGF(LogMfMedia, VeryVerbose, "Callback %p: Sample read: %ls (stream = %i, status = %i, flags = %i)", this, *FTimespan(Timestamp).ToString(), StreamIndex, Status, StreamFlags);
	#endif
