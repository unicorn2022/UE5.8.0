...
namespace MfMedia
{
	TComPtr<IMFMediaType> CreateOutputType(const GUID& MajorType, const GUID& SubType, bool AllowNonStandardCodecs)
	{
		TComPtr<IMFMediaType> OutputType;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Warning, "Failed to create %ls output type: %ls", *MajorTypeToString(MajorType), *ResultToString(Result));
				return NULL;
			}
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Warning, "Failed to initialize %ls output type: %ls", *MajorTypeToString(MajorType), *ResultToString(Result));
				return NULL;
			}
		}
		...
		if (MajorType == MFMediaType_Audio)
		{
			...
			{
				UE_LOGF(LogMfMedia, Warning, "Skipping unsupported audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				return NULL;
			}
#else
			if (FMemory::Memcmp(&SubType.Data2, &MFMPEG4Format_Base.Data2, 12) == 0)
			{
				...
				{
					UE_LOGF(LogMfMedia, Verbose, "Allowing non-standard MP4 audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				}
				else
				{
					...
					{
						UE_LOGF(LogMfMedia, Warning, "Skipping non-standard MP4 audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
						return NULL;
					}
				}
			}
			else if (FMemory::Memcmp(&SubType.Data2, &MFAudioFormat_Base.Data2, 12) != 0)
			{
				if (AllowNonStandardCodecs)
				{
					UE_LOGF(LogMfMedia, Verbose, "Allowing non-standard audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				}
				else
				{
					UE_LOGF(LogMfMedia, Warning, "Skipping non-standard audio type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
					return NULL;
				}
			}
			...
			{
				UE_LOGF(LogMfMedia, Warning, "Failed to initialize audio output type");
				return NULL;
			}
		}
		else if (MajorType == MFMediaType_SAMI)
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Warning, "Failed to initialize caption output type: %ls", *ResultToString(Result));
				return NULL;
			}
		}
		else if (MajorType == MFMediaType_Video)
		{
			...
			{
				UE_LOGF(LogMfMedia, Warning, "Unsupported video type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				return NULL;
			}
#else
			if (FMemory::Memcmp(&SubType.Data2, &MFVideoFormat_Base.Data2, 12) != 0)
			{
				if (AllowNonStandardCodecs)
				{
					UE_LOGF(LogMfMedia, Verbose, "Allowing non-standard video type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
				}
				else
				{
					UE_LOGF(LogMfMedia, Warning, "Skipping non-standard video type %ls (%ls) \"%ls\"", *SubTypeToString(SubType), *GuidToString(SubType), *FourccToString(SubType.Data1));
					return NULL;
				}
			}
#if PLATFORM_WINDOWS
			if ((SubType == MFVideoFormat_H264) || (SubType == MFVideoFormat_H264_ES))
			{
				...
				{
					UE_LOGF(LogMfMedia, Warning, "H264 video type requires Windows 7 or newer (your version is %ls)", *FPlatformMisc::GetOSVersion());
					return NULL;
				}
			}
			...
			if ((SubType == MFVideoFormat_HEVC) || (SubType == MFVideoFormat_HEVC_ES))
			{
				if (!FPlatformMisc::VerifyWindowsVersion(10, 0) /*Win10*/)
				{
					...
					{
						UE_LOGF(LogMfMedia, Warning, "HEVC video type requires Windows 10 or newer (your version is %ls", *FPlatformMisc::GetOSVersion());
						return NULL;
					}

					UE_LOGF(LogMfMedia, Warning, "HEVC video type requires Windows 10 or newer (your version is %ls), and game must be manifested for Windows 10", *FPlatformMisc::GetOSVersion());
				}
			}
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Warning, "Failed to set video output type: %ls", *ResultToString(Result));
				return NULL;
			}
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Warning, "Failed to set video output sub-type: %ls", *ResultToString(Result));
				return NULL;
			}
		}
		else
	...
	TComPtr<IMFMediaSource> ResolveMediaSource(TSharedPtr<FArchive, ESPMode::ThreadSafe> Archive, const FString& Url, bool Precache)
	{
		...
		if (!Archive.IsValid() && Url.StartsWith(TEXT("file://")))
		{
			...
			if (!Archive.IsValid())
			{
				UE_LOGF(LogMfMedia, Error, "Failed to open or read media file %ls", FilePath);
				return NULL;
			}

			if (Archive->TotalSize() == 0)
			{
				UE_LOGF(LogMfMedia, Error, "Cannot open media from empty file %ls.", FilePath);
				return NULL;
			}
		}
		...
		TComPtr<IMFSourceResolver> SourceResolver;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Error, "Failed to create media source resolver: %ls", *MfMedia::ResultToString(Result));
				return NULL;
			}
		}
		...
		TComPtr<IUnknown> SourceObject;
		{
			...
			if (Archive.IsValid())
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogMfMedia, Error, "Failed to resolve byte stream %ls: %ls", *Url, *MfMedia::ResultToString(Result));
					return NULL;
				}
			}
			else
			{
				...
				if (FAILED(Result))
				{
					UE_LOGF(LogMfMedia, Error, "Failed to resolve URL %ls: %ls", *Url, *MfMedia::ResultToString(Result));
					return NULL;
				}
			}
		}
		...
		TComPtr<IMFMediaSource> MediaSource;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Error, "Failed to query media source interface: %ls", *MfMedia::ResultToString(Result));
				return NULL;
			}
		}
