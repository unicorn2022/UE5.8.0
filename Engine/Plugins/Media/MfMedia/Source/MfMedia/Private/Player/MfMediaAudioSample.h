...
public:
	...
	/** Virtual destructor. */
	virtual ~FMfMediaAudioSample() = default;
...
public:
	...
	bool Initialize(IMFMediaType& InMediaType, IMFSample& InSample, uint32 InNumChannels, uint32 InSampleRate)
	{
		...
		TComPtr<IMFMediaBuffer> Buffer = NULL;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, VeryVerbose, "Failed to get contiguous buffer from audio sample (%ls)", *MfMedia::ResultToString(Result));
				return false;
			}
		}
		...
		DWORD BufferSize = 0;
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, VeryVerbose, "Failed to lock contiguous buffer from audio sample (%ls)", *MfMedia::ResultToString(Result));
				return false;
			}
		}
