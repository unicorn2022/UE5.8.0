...
bool FMfMediaPlayer::Open(const TSharedRef<FArchive, ESPMode::ThreadSafe>& Archive, const FString& OriginalUrl, const IMediaOptions* /*Options*/)
{
	...
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Cannot open media from archive (archive is empty).", this);
		return false;
	}

	if (OriginalUrl.IsEmpty())
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Cannot open media from archive (no original URL provided).", this);
		return false;
	}
...
bool FMfMediaPlayer::CommitTime(FTimespan Time)
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Failed to set source reader position to %ls: %ls", this, *Time.ToString(), *MfMedia::ResultToString(Result));
		return false;
	}
...
{
	UE_LOGF(LogMfMedia, VeryVerbose, "Player %p: Initializing %ls (archive = %ls, precache = %ls)", this, *Url, Archive.IsValid() ? TEXT("yes") : TEXT("no"), Precache ? TEXT("yes") : TEXT("no"));
...
void FMfMediaPlayer::UpdateCharacteristics()
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Failed to get media source characteristics: %ls", this, *MfMedia::ResultToString(Result));
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Failed to query service interface: %ls", this, *MfMedia::ResultToString(Result));
		return;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMfMedia, Log, "Rate control service unavailable: %ls", *MfMedia::ResultToString(Result));
	}
	else
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Rate control ready", this);
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMfMedia, Log, "Rate support service unavailable: %ls", *MfMedia::ResultToString(Result));
	}
	else
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Rate support ready", this);
	}
...
bool FMfMediaPlayer::Seek(const FTimespan& Time)
{
	...
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Cannot seek while closed or in error state", this);
		return false;
	}

	if ((Time < FTimespan::Zero()) || (Time > CurrentDuration))
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Invalid seek time %ls (media duration is %ls)", this, *Time.ToString(), *CurrentDuration.ToString());
		return false;
	}

	UE_LOGF(LogMfMedia, Verbose, "Player %p: Seeking to %ls", this, *Time.ToString());
...
bool FMfMediaPlayer::SetRate(float Rate)
{
	...
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Setting rate from to %f to %f (unthinned)", this, CurrentRate, Rate);
		Thin = FALSE;
	}
	else if (ThinnedRates.Contains(Rate))
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: Setting rate from to %f to %f (thinned)", this, CurrentRate, Rate);
		Thin = TRUE;
	}
	else
	{
		UE_LOGF(LogMfMedia, Verbose, "Player %p: The rate %f is not supported", this, Rate);
		return false;
	}
	...
	if (RateControl != NULL)
	{
		if (Rate <= 0.0f)
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Verbose, "Player %p: Failed to commit rate change from %f to zero: %ls", this, CurrentRate, *MfMedia::ResultToString(Result));
				return false;
			}
		}
		else if (!ThinnedRates.IsEmpty())
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, Verbose, "Player %p: Failed to commit rate change from %f to %f [%ls]: %ls", this, CurrentRate, Rate, ThinnedString, *MfMedia::ResultToString(Result));
				return false;
			}
		}
	}
