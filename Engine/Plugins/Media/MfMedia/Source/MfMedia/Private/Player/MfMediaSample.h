...
protected:
	...
	bool InitializeSample(IMFSample& InSample)
	{
		// get sample duration
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, VeryVerbose, "Failed to get sample duration from media sample (%ls)", *MfMedia::ResultToString(Result));
				return false;
			}
		...
		// get sample flags
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, VeryVerbose, "Failed to get sample flags from media sample (%ls)", *MfMedia::ResultToString(Result));
				return false;
			}
		}
		...
		// get sample time
		{
			...
			if (FAILED(Result))
			{
				UE_LOGF(LogMfMedia, VeryVerbose, "Failed to get sample time from media sample (%ls)", *MfMedia::ResultToString(Result));
				return false;
			}
