...
#include "MP4Utilities.h"	// to read the ProRes header, which is given in the form of an MP4 Atom
...
public:
	...
	FVideoDecoderProResElectra(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
class FProResVideoDecoderElectraFactory : public IElectraCodecFactory, public IElectraCodecFactory::IProviderInformation, public IElectraCodecModularFeature, public TSharedFromThis<FProResVideoDecoderElectraFactory, ESPMode::ThreadSafe>
{
public:
	static constexpr uint32 Make4CC(const uint8 A, const uint8 B, const uint8 C, const uint8 D)
	{
	...
	int32 SupportsDecoding(TMap<FString, FVariant>& OutFormatInfo, const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions) const override
	{
		...
		switch(InCodecFormat.FourCC)
		{
			...
			{
				Name = TEXT("Apple ProRes 422 High Quality");
				break;
			}
			case Make4CC('a','p','c','n'):
			{
				Name = TEXT("Apple ProRes 422 Standard Definition");
				break;
			}
			case Make4CC('a','p','c','s'):
			{
				Name = TEXT("Apple ProRes 422 LT");
				break;
			}
			case Make4CC('a','p','c','o'):
			{
				Name = TEXT("Apple ProRes 422 Proxy");
				break;
			}
			case Make4CC('a','p','4','h'):
			{
				Name = TEXT("Apple ProRes 4444");
				break;
			}
			case Make4CC('a','p','4','x'):
			{
				Name = TEXT("Apple ProRes 4444 XQ");
				break;
			}
			case Make4CC('a','p','r','n'):
			{
				Name = TEXT("Apple ProRes RAW");
				break;
			}
			case Make4CC('a','p','r','h'):
			{
				Name = TEXT("Apple ProRes RAW HQ");
				break;
			}
			default:
			{
		...
		if (bIsSupported)
		{
			...
			{
				OutFormatInfo.Emplace(IElectraDecoderFormatInfo::HumanReadableFormatName, FVariant(Name));
			}
			else
			{
	...
	{
		return InCodecFormat.Properties.IsType<Electra::FCodecTypeFormat::FVideo>() ? MakeShared<FVideoDecoderProResElectra, ESPMode::ThreadSafe>(InCodecFormat, InAdditionalOptions) : TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe>();
	}
...
FVideoDecoderProResElectra::FVideoDecoderProResElectra(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
...
bool FVideoDecoderProResElectra::ParseHeader(const void* InData, int32 InNumData)
{
	...
	CurrentHeader.Reset();
	MP4Utilities::FMP4AtomReader Reader(MakeConstArrayView<const uint8>(reinterpret_cast<const uint8*>(InData), InNumData));
	Reader.Read(CurrentHeader.FrameSizeInBytes);
	Reader.Read(CurrentHeader.FrameType);
	if (CurrentHeader.FrameType != MP4Utilities::MakeBoxAtom('i','c','p','f'))
	{
		return false;
	}
...
bool FVideoDecoderProResElectra::InternalDecoderCreate(const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	delete [] PixelFormatList;
	PixelFormatList = nullptr;
