...
namespace UE::PixelStreamingServers
{
	...
	bool FCheckNumStreamers::Update()
	{
		if (Server && Server->IsReady())
		{
			...
			if (bTestPassed)
			{
				UE_LOGF(LogPixelStreamingServers, Log, "Success: %ls", *LogString);
				return true;
			}
			else if (GetCurrentRunTime() > 2.0f)
			{
				UE_LOGF(LogPixelStreamingServers, Error, "Failed (timed out after 2s): %ls", *LogString);
				return true;
			}
		}
		return false;
	...
	bool FStartWebsocketClient::Update()
	{
		...
		{
			UE_LOGF(LogPixelStreamingServers, Error, "Timed out after 2s of waiting for websocket to connect");
			return true;
		}
		...
		if (Probe->Probe())
		{
			UE_LOGF(LogPixelStreamingServers, Log, "Websocket client probe connected.");
			return true;
		}
		else
		{
	...
	bool FCloseWebsocketClient::Update()
	{
		if (GetCurrentRunTime() > 2.0f)
		{
			UE_LOGF(LogPixelStreamingServers, Error, "Timed out after 2s of waiting for websocket to close");
			return true;
		}
		...
		{
			Probe->Close();
			UE_LOGF(LogPixelStreamingServers, Log, "Asked websocket client probe to close.");
			return false;
		}
		else
		{
			UE_LOGF(LogPixelStreamingServers, Log, "Websocket client probe closed.");
			return true;
		}
	}
	...
	#elif PLATFORM_MAC
		UE_LOGF(LogPixelStreamingServers, Error, "No cirrus binaries exist for Mac!");
		return "";
	#else
		UE_LOGF(LogPixelStreamingServers, Error, "Unsupported platform for Pixel Streaming.");
		return "";
	#endif
	}
	...
	{
		UE_LOGF(LogPixelStreamingServers, Log, "----------- LaunchDownloadedCirrusTest -----------");
		...
		SignallingServer->OnReady.AddLambda([this](TMap<EEndpoint, FURL> Endpoints) {
			...
			UE_LOGF(LogPixelStreamingServers, Log, "Http address for webserver. Actual=%ls | Expected=%ls", *ActualWebserverUrl, *ExpectedWebserverAddress);
			...
			UE_LOGF(LogPixelStreamingServers, Log, "Websocket address for streamer messages. Actual=%ls | Expected=%ls", *ActualStreamerUrl, *ExpectedStreamerAddress);
			...
			UE_LOGF(LogPixelStreamingServers, Log, "Websocket address for player messages. Actual=%ls | Expected=%ls", *ActualPlayersUrl, *ExpectedPlayerWSAddress);
			...
			UE_LOGF(LogPixelStreamingServers, Log, "Websocket address for SFU messages. Actual=%ls | Expected=%ls", *ActualSFUUrl, *ExpectedSFUAddress);
			...
			UE_LOGF(LogPixelStreamingServers, Log, "Websocket address for matchmaker messages. Actual=%ls | Expected=%ls", *ActualMatchmakerUrl, *ExpectedMatchmakerAddress);
	...
	{
		UE_LOGF(LogPixelStreamingServers, Log, "----------- LaunchEmbeddedCirrusTest -----------");
		...
		bool bLaunched = SignallingServer->Launch(LaunchArgs);
		UE_LOGF(LogPixelStreamingServers, Log, "Embedded cirrus launched: %ls", bLaunched ? TEXT("true") : TEXT("false"));
	...
	{
		UE_LOGF(LogPixelStreamingServers, Log, "----------- TwoWebsocketToEmbeddedCirrus -----------");
