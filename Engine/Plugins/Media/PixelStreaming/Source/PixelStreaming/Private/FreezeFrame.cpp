...
namespace UE::PixelStreaming
{
	...
	void FFreezeFrame::StartFreeze(UTexture2D* Texture)
	{
		if (Texture)
		{
			...
			([this, Texture](FRHICommandListImmediate& RHICmdList) {
				...
				if (!TextureRHI)
				{
					UE_LOGF(LogPixelStreaming, Error, "Attempting freeze frame with texture %ls with no texture RHI", *Texture->GetName());
					return;
				}
	...
	void FFreezeFrame::SendFreezeFrame(TArray<FColor> RawData, const FIntRect & Rect)
	{
		...
		if (bSuccess)
		{
			...
			Players->Apply([&JpegBytes, this](FPixelStreamingPlayerId PlayerId, FPlayerContext& PlayerContext) {
				if (PlayerContext.DataChannel)
				{
					...
					{
						UE_LOGF(LogPixelStreaming, Error, "Could not find datachannel message with ID FreezeFrame.");
					}
				}
			});
		...
		{
			UE_LOGF(LogPixelStreaming, Error, "JPEG image wrapper failed to accept frame data");
		}
	}
