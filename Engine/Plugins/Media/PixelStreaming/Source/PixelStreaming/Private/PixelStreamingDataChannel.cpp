...
bool FPixelStreamingDataChannel::SendArbitraryData(uint8 Type, const TArray64<uint8>& DataBytes) const
{
	...
	if (!SendChannel)
	{
		UE_LOGF(LogPixelStreaming, Error, "Cannot send arbitrary data when data channel is null.");
		return false;
	}
	...
	while (BytesTransmitted < DataSize)
	{
		...
		{
			UE_LOGF(LogPixelStreaming, Error, "Failed to send data channel packet");
			return false;
		}
