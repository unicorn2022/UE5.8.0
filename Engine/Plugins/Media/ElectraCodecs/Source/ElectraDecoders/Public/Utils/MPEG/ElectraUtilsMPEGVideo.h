...
namespace ElectraDecodersUtil
{
	namespace MPEG
	{
		...
		struct FSEImastering_display_colour_volume
		{
			...
			bool operator==(const FSEImastering_display_colour_volume& rhs) const
			{
				return display_primaries_x[0] == rhs.display_primaries_x[0] &&
					   display_primaries_y[0] == rhs.display_primaries_y[0] &&
					   display_primaries_x[1] == rhs.display_primaries_x[1] &&
					   display_primaries_y[1] == rhs.display_primaries_y[1] &&
					   display_primaries_x[2] == rhs.display_primaries_x[2] &&
					   display_primaries_y[2] == rhs.display_primaries_y[2] &&
					   white_point_x == rhs.white_point_x &&
					   white_point_y == rhs.white_point_y &&
					   max_display_mastering_luminance == rhs.max_display_mastering_luminance &&
					   min_display_mastering_luminance == rhs.min_display_mastering_luminance;
			}
			bool operator!=(const FSEImastering_display_colour_volume& rhs) const
			{
				return !operator==(rhs);
		...
		struct FSEIcontent_light_level_info
		{
			...
			bool operator==(const FSEIcontent_light_level_info& rhs) const
			{
				return max_content_light_level == rhs.max_content_light_level && max_pic_average_light_level == rhs.max_pic_average_light_level;
			}
			bool operator!=(const FSEIcontent_light_level_info& rhs) const
			{
				return !operator==(rhs);
