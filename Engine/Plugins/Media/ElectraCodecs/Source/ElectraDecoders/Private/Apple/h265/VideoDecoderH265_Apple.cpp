...
{
	// The decoder may not output frames in display order depending on the presence of B-frames.
	// We may need to hold back a number of frames and sort them by PTS to get them in display order.
	// This value can be configured using the "max_output_buffers" additional option.
	static constexpr int32 kDefaultNumReorderFrames = 5;

public:
	FElectraVideoDecoderH265_Apple(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
private:
	...
	int32 NumReorderFrames = kDefaultNumReorderFrames;
};
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderH265_Apple::Create(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	TSharedPtr<FElectraVideoDecoderH265_Apple, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderH265_Apple>(InCodecFormat, InAdditionalOptions);
	return New;
}

FElectraVideoDecoderH265_Apple::FElectraVideoDecoderH265_Apple(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
	: InitialCodecFormat(InCodecFormat)
	, InitialCreationOptions(InAdditionalOptions)
{
	const int32 MaxOutputBuffers = static_cast<int32>(ElectraDecodersUtil::GetVariantValueSafeU64(InitialCreationOptions, TEXT("max_output_buffers"), kDefaultNumReorderFrames));
	NumReorderFrames = FMath::Max(MaxOutputBuffers, 1);
}
...
bool FElectraVideoDecoderH265_Apple::CheckForAvailableOutput()
{
	...
	bool bHaveEnough = ReadyImages.Num() >= NumReorderFrames || DecodeState == EDecodeState::Draining;
	return bHaveEnough;
}
