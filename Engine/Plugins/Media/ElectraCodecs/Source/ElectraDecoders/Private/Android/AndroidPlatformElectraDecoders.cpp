...
namespace PlatformElectraDecodersAndroid
{
	...
	TAutoConsoleVariable<bool> CVarElectraDecoderAndroidRebasePTS(
		TEXT("ElectraDecoders.AndroidRebasePTS"),
		true,
		TEXT("Rebase PTS to zero for Android decoders.\n")
		TEXT(" true: rebase so values start at 0, false use original values"),
		ECVF_Default);
}
