...
{
public:
	FElectraVideoDecoderVPx_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderVPx_Android::Create(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	TSharedPtr<FElectraVideoDecoderVPx_Android, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderVPx_Android>(InCodecFormat, InAdditionalOptions);
	return New;
}


FElectraVideoDecoderVPx_Android::FElectraVideoDecoderVPx_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
	...
	, InitialCreationOptions(InAdditionalOptions)
{
	...
	ActiveCodec = InitialCodecFormat.FourCC == ElectraDecodersUtil::Make4CC('v','p','0','9') ? ECodec::VP9 : ECodec::VP8;
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		if (InputBufferIndex >= 0)
		{
			...
			if (!DecoderPTSOffset.IsSet())
			{
				...
				DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderVPx::DecodeAccessUnit() - Rebase PTS offset %lld"), (long long int)DecoderPTSOffset.GetValue());
			}
			...
			DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderVPx::DecodeAccessUnit() - queue input %lld (%lld), 0x%x"), (long long int)PTSTicks, (long long int)InInputAccessUnit.PTS.GetTicks(), InInputAccessUnit.Flags);
			if (Result == 0)
			{
...
FElectraVideoDecoderVPx_Android::EConvertResult FElectraVideoDecoderVPx_Android::ConvertDecoderOutput(const IElectraVPxVideoDecoderAndroidJava::FOutputBufferInfo& InInfo)
{
	...
	{
		DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderVPx::ConvertDecoderOutput() - Discard output %lld (%lld), 0x%x"), (long long int)In->DecoderPTS, (long long int)In->AccessUnit.PTS.GetTicks(), In->AccessUnit.Flags);
		if (DecoderInstance.IsValid())
		{
	...
	DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderVPx::ConvertDecoderOutput() - Convert output %lld (%lld), 0x%x"), (long long int)In->DecoderPTS, (long long int)In->AccessUnit.PTS.GetTicks(), In->AccessUnit.Flags);
