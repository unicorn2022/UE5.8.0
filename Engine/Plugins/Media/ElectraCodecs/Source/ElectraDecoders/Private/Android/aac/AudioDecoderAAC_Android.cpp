...
{
public:
	FElectraAudioDecoderAAC_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraAudioDecoderAAC_Android::Create(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	TSharedPtr<FElectraAudioDecoderAAC_Android, ESPMode::ThreadSafe> New = MakeShared<FElectraAudioDecoderAAC_Android>(InCodecFormat, InAdditionalOptions);
	return New;
}


FElectraAudioDecoderAAC_Android::FElectraAudioDecoderAAC_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	if (InCodecFormat.CSD.Num())
	{
		TSharedPtr<ElectraDecodersUtil::MPEG::FAACDecoderConfigurationRecord, ESPMode::ThreadSafe> NewConfigRecord = MakeShared<ElectraDecodersUtil::MPEG::FAACDecoderConfigurationRecord, ESPMode::ThreadSafe>();
		if (NewConfigRecord->Parse(InCodecFormat.CSD))
		{
			ConfigRecord = MoveTemp(NewConfigRecord);
		}
	}
}
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		if (InputBufferIndex >= 0)
		{
			...
			if (!DecoderPTSOffset.IsSet())
			{
				...
				DETAILLOG(LogElectraDecoders, Log, TEXT("AudioDecoderAAC::DecodeAccessUnit() - Rebase PTS offset %lld"), (long long int)DecoderPTSOffset.GetValue());
			}
			...
			DETAILLOG(LogElectraDecoders, Log, TEXT("AudioDecoderAAC::DecodeAccessUnit() - queue input %lld (%lld)"), (long long int)PTSTicks, (long long int)InInputAccessUnit.PTS.GetTicks());
			int32 Result = DecoderInstance->QueueInputBuffer(InputBufferIndex, InInputAccessUnit.Data, InInputAccessUnit.DataSize, PTSTicks);
			if (Result == 0)
			{
				bDidEnqueueInputBuffer = true;
				LastPushedPresentationTimeUs = PTSTicks;
