...
{
extern TAutoConsoleVariable<bool> CVarElectraDecoderAndroidRebasePTS;
}
...
{
public:
	FElectraVideoDecoderH264_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
private:
	...
	TOptional<int64> DecoderPTSOffset;
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderH264_Android::Create(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	TSharedPtr<FElectraVideoDecoderH264_Android, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderH264_Android>(InCodecFormat, InAdditionalOptions);
	return New;
}


FElectraVideoDecoderH264_Android::FElectraVideoDecoderH264_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
	: InitialCodecFormat(InCodecFormat)
	, InitialCreationOptions(InAdditionalOptions)
{
	ResourceInstance = FElectraDecodersPlatformResources::CreateResourceInstance(ElectraDecodersUtil::GetVariantValueSafeI64(InitialCreationOptions, FElectraDecodersPlatformResources::OptionKeyOutputTexturePoolID().ToString(), 0));
	...
	InitialMaxValues.Width = (int32) Align(ElectraDecodersUtil::GetVariantValueSafeI64(InitialCreationOptions, TEXT("max_width"), 0), 16);
	InitialMaxValues.Height = (int32) Align(ElectraDecodersUtil::GetVariantValueSafeI64(InitialCreationOptions, TEXT("max_height"), 0), 16);
	const FVariant* Var = InitialCreationOptions.Find(TEXT("max_codecprofile"));
	if (Var && Var->GetType() == EVariantTypes::String)
	{
		Electra::FCodecTypeFormat ctf;
		ctf.RFC6381 = Var->GetValue<FString>();
		if (FElectraCodecFormatUtilsModularFeature::SetupCodecTypeFormat(ctf))
		{
			const Electra::FCodecTypeFormat::FVideo& vid = ctf.Properties.Get<Electra::FCodecTypeFormat::FVideo>();
			InitialMaxValues.Profile = vid.Profile.Profile;
			InitialMaxValues.Level = vid.Profile.Level;
		}
	}
}
...
IElectraDecoder::ECSDCompatibility FElectraVideoDecoderH264_Android::IsCompatibleWith(const TMap<FString, FVariant>& CSDAndAdditionalOptions)
{
	...
	if (!NewSPS.IsValid())
	{
		UE_LOGF(LogElectraDecoders, Warning, "No CSD provided to IsCompatibleWith(), returning to drain and reset.");
...
IElectraDecoder::EDecoderError FElectraVideoDecoderH264_Android::DecodeAccessUnit(const FInputAccessUnit& InInputAccessUnit, const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		{
			// The decoder could have issues when the PTS is large as is common in a Live stream.
			// We rebase it to work around this issue.
			int64 PTSTicks = InInputAccessUnit.PTS.GetTicks();
			if (!DecoderPTSOffset.IsSet())
			{
				DecoderPTSOffset = PlatformElectraDecodersAndroid::CVarElectraDecoderAndroidRebasePTS.GetValueOnAnyThread() ? PTSTicks : 0;
				DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH264::DecodeAccessUnit() - Rebase PTS offset %lld"), (long long int)DecoderPTSOffset.GetValue());
			}
			PTSTicks -= DecoderPTSOffset.GetValue();
			// Also the decoder internally truncates nanoseconds to microseconds which it then
			// sticks onto the decoded output, so we cannot use the PTS as a "user value".
			// We truncate the value ourselves to account for this.
			PTSTicks /= 10;		// Convert from HNS to microseconds
			...
			if ((InInputAccessUnit.Flags & EElectraDecoderFlags::IsSyncSample) != EElectraDecoderFlags::None)
			{
				...
				Result = DecoderInstance->QueueInputBuffer(InputBufferIndex, CSD.GetData(), CSD.Num(), InInputAccessUnit.Data, InInputAccessUnit.DataSize, PTSTicks);
			}
			else
			{
				Result = DecoderInstance->QueueInputBuffer(InputBufferIndex, nullptr, 0, InInputAccessUnit.Data, InInputAccessUnit.DataSize, PTSTicks);
			}
			DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH264::DecodeAccessUnit() - queue input %lld (%lld), 0x%x"), (long long int)PTSTicks, (long long int)InInputAccessUnit.PTS.GetTicks(), InInputAccessUnit.Flags);
			if (Result == 0)
			{
				LastPushedPresentationTime = PTSTicks;
...
FElectraVideoDecoderH264_Android::EConvertResult FElectraVideoDecoderH264_Android::ConvertDecoderOutput(const IElectraH264VideoDecoderAndroidJava::FOutputBufferInfo& InInfo)
{
	...
	{
		if (InDecoderInput[nInDec]->DecoderPTS == InInfo.PresentationTimestamp)
		{
	...
	{
		DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH264::ConvertDecoderOutput() - Discard output %lld (%lld), 0x%x"), (long long int)In->DecoderPTS, (long long int)In->AccessUnit.PTS.GetTicks(), In->AccessUnit.Flags);
		if (DecoderInstance.IsValid())
		{
	...
	DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH264::ConvertDecoderOutput() - Convert output %lld (%lld), 0x%x"), (long long int)In->DecoderPTS, (long long int)In->AccessUnit.PTS.GetTicks(), In->AccessUnit.Flags);
