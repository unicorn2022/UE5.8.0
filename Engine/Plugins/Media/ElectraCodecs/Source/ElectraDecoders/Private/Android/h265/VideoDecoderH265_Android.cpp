...
{
public:
	FElectraVideoDecoderH265_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderH265_Android::Create(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	TSharedPtr<FElectraVideoDecoderH265_Android, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderH265_Android>(InCodecFormat, InAdditionalOptions);
	return New;
}


FElectraVideoDecoderH265_Android::FElectraVideoDecoderH265_Android(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		if (InputBufferIndex >= 0)
		{
			...
			if (!DecoderPTSOffset.IsSet())
			{
				...
				DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH265::DecodeAccessUnit() - Rebase PTS offset %lld"), (long long int)DecoderPTSOffset.GetValue());
			}
			...
			DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH265::DecodeAccessUnit() - queue input %lld (%lld), 0x%x"), (long long int)PTSTicks, (long long int)InInputAccessUnit.PTS.GetTicks(), InInputAccessUnit.Flags);
			if (Result == 0)
			{
...
FElectraVideoDecoderH265_Android::EConvertResult FElectraVideoDecoderH265_Android::ConvertDecoderOutput(const IElectraH265VideoDecoderAndroidJava::FOutputBufferInfo& InInfo)
{
	...
	{
		DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH265::ConvertDecoderOutput() - Discard output %lld (%lld), 0x%x"), (long long int)In->DecoderPTS, (long long int)In->AccessUnit.PTS.GetTicks(), In->AccessUnit.Flags);
		if (DecoderInstance.IsValid())
		{
	...
	DETAILLOG(LogElectraDecoders, Log, TEXT("VideoDecoderH265::ConvertDecoderOutput() - Convert output %lld (%lld), 0x%x"), (long long int)In->DecoderPTS, (long long int)In->AccessUnit.PTS.GetTicks(), In->AccessUnit.Flags);
