...
class FH265VideoDecoderFactoryWindows : public IElectraCodecFactory, public IElectraCodecFactory::IProviderInformation
{
public:
	...
	int32 SupportsDecoding(TMap<FString, FVariant>& OutFormatInfo, const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions) const override
	{
		...
		double fps = vi.FrameRate.IsValid() ? vi.FrameRate.AsDecimal() : 0.0;
		...
		for(int32 i=0; i<Configs.Num(); ++i)
		{
			...
			if (Cfg.Profile == vi.Profile.Profile)
			{
				if (vi.Profile.Level > (uint32)Cfg.Level)
				{
					continue;
				}
	...
	{
		return InCodecFormat.Properties.IsType<Electra::FCodecTypeFormat::FVideo>() ? IElectraVideoDecoderH265_DX::Create(InCodecFormat, InAdditionalOptions) : TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe>();
	}
...
void IElectraVideoDecoderH265_DX::PlatformGetSupportedConfigurations(TArray<FSupportedConfiguration>& OutSupportedConfigurations)
{
	if (IElectraVideoDecoderH265_DX_Platform::bDecoderConfigurationsDirty)
	{
		...
		// Main
		IElectraVideoDecoderH265_DX_Platform::DecoderConfigurations.Emplace(IElectraVideoDecoderH265_DX::FSupportedConfiguration(0, 1, 0, 156, 120, 4096, 2304, 0));
		// Main10
		IElectraVideoDecoderH265_DX_Platform::DecoderConfigurations.Emplace(IElectraVideoDecoderH265_DX::FSupportedConfiguration(0, 2, 0, 156, 120, 4096, 2304, 0));
