...
bool FElectraDecodersPlatformResourcesWindows::GetD3DDeviceAndVersion(void** OutD3DDevice, int32* OutD3DVersion)
{
	...
	case ERHIInterfaceType::D3D11:
		*OutD3DDevice = GetID3D11DynamicRHI()->RHIGetDevice();
		*OutD3DVersion = 11000;
		return true;

	case ERHIInterfaceType::D3D12:
		*OutD3DDevice = GetID3D12DynamicRHI()->RHIGetDevice_NoMGPU();
		*OutD3DVersion = 12000;
		return true;
...
static void TriggerDataCopy(TRefCountPtr<ID3D12Resource> InDstResource, TRefCountPtr<ID3D12Resource> InSrcResource, TRefCountPtr<ID3D12GraphicsCommandList> D3DCmdList, TRefCountPtr<ID3D12CommandAllocator> D3DCmdAllocator, TRefCountPtr<ID3D12Fence> D3DFence, uint64 FenceValue, const TSharedPtr<std::atomic<bool>>& FenceSignalled, const FElectraDecoderOutputSync& OutputSync)
{
	...
	GetID3D12PlatformDynamicRHI()->RHIRunOnQueue(ED3D12RHIRunOnQueueType::Copy,
		[DstResource = InDstResource, SrcResource = InSrcResource, CmdList = D3DCmdList, CmdAllocator = D3DCmdAllocator, DestFence = D3DFence, DestFenceValue = FenceValue, DestFenceSignalled = FenceSignalled, OutputSync](ID3D12CommandQueue* D3DCmdQueue)
		{
			...
			checkf(!*DestFenceSignalled, TEXT("Electra texture sample copy fence should not have been signalled yet at this point"));

			*DestFenceSignalled = true;
													 ...
													 uint32 MaxWidth, uint32 MaxHeight, uint32 MaxOutputBuffers)
{
	...
	*/
	...
	InOutTextureSampleToSetup->FenceSignalled = MakeShared<std::atomic<bool>>(false);
	...
	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	// Schedule the copy right away. Any needed synchronization will be done in the copy-queue by the GPU
	TriggerDataCopy(InOutTextureSampleToSetup->TextureDX12, InResource, InOutTextureSampleToSetup->D3DCmdList, InOutTextureSampleToSetup->D3DCmdAllocator, InOutTextureSampleToSetup->D3DFence, InOutTextureSampleToSetup->FenceValue, InOutTextureSampleToSetup->FenceSignalled, OutputSync);
