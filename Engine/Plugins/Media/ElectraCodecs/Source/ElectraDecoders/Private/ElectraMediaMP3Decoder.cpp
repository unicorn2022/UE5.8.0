...
public:
	...
	FElectraMP3Decoder(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
class FElectraCommonAudioMP3DecoderFactory : public IElectraCodecFactory, public IElectraCodecFactory::IProviderInformation, public IElectraCodecModularFeature, public TSharedFromThis<FElectraCommonAudioMP3DecoderFactory, ESPMode::ThreadSafe>
{
public:
	virtual ~FElectraCommonAudioMP3DecoderFactory()
	{ }

	FElectraCommonAudioMP3DecoderFactory()
	{
#if WITH_DR_LIBS_MP3 && defined(DR_LIBS_VERSION_NUMBER)
		LibVersion = ANSI_TO_TCHAR(DR_LIBS_VERSION_NUMBER);
#endif
	}
	...
	int32 SupportsDecoding(TMap<FString, FVariant>& OutFormatInfo, const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions) const override
	{
		...
		if (!Permitted4CCs.Contains(InCodecFormat.RFC6381))
		{
			return 0;
		}
		return 5;
	...
	{
		return InCodecFormat.Properties.IsType<Electra::FCodecTypeFormat::FAudio>() ? MakeShared<FElectraMP3Decoder, ESPMode::ThreadSafe>(InCodecFormat, InAdditionalOptions) : TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe>();
	}
...
FElectraMP3Decoder::FElectraMP3Decoder(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
}
...
bool FElectraMP3Decoder::ParseMpegAudioHeader(FMpegAudioConfig& OutConfig, const void* InData, int64 InDataSize)
{
	if (InDataSize >= 4)
	{
		...
		if (ElectraDecodersUtil::MPEG::UtilsMPEG123::HasValidSync(HeaderValue))
		{
			...
			OutConfig.Bitrate = ElectraDecodersUtil::MPEG::UtilsMPEG123::GetBitrate(HeaderValue);
