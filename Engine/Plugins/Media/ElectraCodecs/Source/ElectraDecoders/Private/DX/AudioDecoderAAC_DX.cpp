...
{
public:
	FElectraAudioDecoderAAC_DX(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraAudioDecoderAAC_DX::Create(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	TSharedPtr<FElectraAudioDecoderAAC_DX, ESPMode::ThreadSafe> New = MakeShared<FElectraAudioDecoderAAC_DX>(InCodecFormat, InAdditionalOptions);
	return New;
}


FElectraAudioDecoderAAC_DX::FElectraAudioDecoderAAC_DX(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	if (InCodecFormat.CSD.Num())
	{
		...
		if (NewConfigRecord->ParseFrom(InCodecFormat.CSD.GetData(), InCodecFormat.CSD.Num()))
		{
