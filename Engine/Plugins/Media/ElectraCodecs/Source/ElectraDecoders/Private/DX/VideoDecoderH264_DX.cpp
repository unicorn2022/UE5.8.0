...
public:
	...
	void* GetPlatformOutputHandle(EElectraDecoderPlatformOutputHandleType InTypeOfHandle) const override
	{
		switch(InTypeOfHandle)
		{
			...
			{
				// Check for the GPU resource directly rather than inferring from !MFSample.IsValid(),
				// since both will coexist when force_cpu_output is set.
				return Buffer.Resource.IsValid() ? static_cast<IElectraDecoderVideoOutputImageBuffers*>(const_cast<FElectraVideoDecoderOutputH264_DX*>(this)) : nullptr;
			}
#endif
...
{
public:
	FElectraVideoDecoderH264_DX(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe> IElectraVideoDecoderH264_DX::Create(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	TSharedPtr<FElectraVideoDecoderH264_DX, ESPMode::ThreadSafe> New = MakeShared<FElectraVideoDecoderH264_DX>(InCodecFormat, InAdditionalOptions);
	return New;
}


FElectraVideoDecoderH264_DX::FElectraVideoDecoderH264_DX(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
	: InitialCodecFormat(InCodecFormat)
	, InitialCreationOptions(InAdditionalOptions)
{
	MaxWidth = Align((uint32)ElectraDecodersUtil::GetVariantValueSafeU64(InitialCreationOptions, TEXT("max_width"), 1920), 16u);
	MaxHeight = Align((uint32)ElectraDecodersUtil::GetVariantValueSafeU64(InitialCreationOptions, TEXT("max_height"), 1080), 16u);
	...
	bForceCPUOutput = !!ElectraDecodersUtil::GetVariantValueSafeI64(InitialCreationOptions, TEXT("force_cpu_output"), 0);
}
...
void FElectraVideoDecoderH264_DX::CopyBufferData(TRefCountPtr<IMF2DBuffer> Buffer2D, TRefCountPtr<ID3D12Resource> BufferResource, TRefCountPtr<ID3D12Fence> BufferFence, uint64 BufferFenceValue, uint32 BufferPitch, int32 DecodedHeight)
{
	uint8* Data = nullptr;
	LONG Pitch = 0;
	TRefCountPtr<IMF2DBuffer2> Buffer2D2;
	HRESULT Res = Buffer2D->QueryInterface(IID_PPV_ARGS(Buffer2D2.GetInitReference()));
	if (SUCCEEDED(Res))
	{
		uint8* Start = nullptr;
		DWORD Length = 0;
		Res = Buffer2D2->Lock2DSize(MF2DBuffer_LockFlags_Read, &Data, &Pitch, &Start, &Length);
		if (SUCCEEDED(Res))
		{
			check(Length >= static_cast<DWORD>(Pitch * DecodedHeight * 3 / 2));
		}
		else
		{
			checkNoEntry();
	...
	{
		Res = Buffer2D->Lock2D(&Data, &Pitch);
		check(SUCCEEDED(Res));
	}

	ID3D12DynamicRHI* const D3D12RHI = GetID3D12DynamicRHI();
	ID3D12Device* const Device = D3D12RHI->RHIGetDevice_NoMGPU();
	...
	*/
	...
	if (bUseGPUBuffers)
	{
		...
		if (Result == S_OK)
		{
			...
			if (bForceCPUOutput)
			{
				// Also expose the MFSample via GetPlatformOutputHandle(MFSample) for CPU buffer extraction
				NewOutput->MFSample = DecodedOutputSample;
			}
		}
