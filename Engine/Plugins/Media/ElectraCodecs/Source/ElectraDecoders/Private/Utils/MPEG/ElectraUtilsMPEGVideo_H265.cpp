	...
	namespace MPEG
	{
		namespace H265
		{
			...
			bool ParseBitstreamForNALUs(TArray<FNaluInfo>& OutNALUs, const uint8* InBitstream, uint64 InBitstreamLength)
			{
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Bitstream too short to parse for NALUs");
					return false;
				}
				...
				for(uint64 iMax=InBitstreamLength-2; i<iMax; )
				{
					...
					else if (InBitstream[i+2] == 1)
					{
						if (InBitstream[i+1] == 0 && InBitstream[i] == 0)
						{
							...
							if ((nut & 0x8000U) != 0)
							{
								UE_LOGF(LogElectraDecoders, Error, "Forbidden zero bit in NAL header is not zero!");
								return false;
							}
							NalInfo.Type = nut >> 9;
				...
				if (OutNALUs.IsEmpty())
				{
					UE_LOGF(LogElectraDecoders, Error, "No NALUs found in bitstream!");
					return false;
				}
			...
			FString FSequenceParameterSet::GetFormatInfo(uint32 InProfileIDC, uint32 InLevelIDC, uint64 InConstraintFlags)
			{
				...
				if (InLevelIDC)
				{
					fi.Append(FString::Printf(TEXT(", level %d.%d"), InLevelIDC/30, (InLevelIDC%30)/3));
				}
				else
				{
			...
			bool FHRDParameters::Parse(FBitstreamReaderH265& br, bool commonInfPresentFlag, uint32 maxNumSubLayersMinus1)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)															\
					if (!(expr))																				\
					{																							\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in hrd_parameters", elem);\
						return false;																			\
					}
			...
			bool FVUIParameters::Parse(FBitstreamReaderH265& br, uint32 In_sps_max_sub_layers_minus1)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)																\
					if (!(expr))																					\
					{																								\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in vui_parameters", elem);	\
						return false;																				\
					}
			...
			bool FProfileTierLevel::Parse(FBitstreamReaderH265& br, bool profilePresentFlag, uint32 maxNumSubLayersMinus1)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)																\
					if (!(expr))																					\
					{																								\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in ProfileTierLevel", elem);	\
						return false;																				\
					}
			...
			bool FStRefPicSet::Parse(FBitstreamReaderH265& br, int32 stRpsIdx, const TArray<FStRefPicSet>& InReferencePicSetList, int32 InReferencePicSetListNum)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)																\
					if (!(expr))																					\
					{																								\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in st_ref_pic_set", elem);	\
						return false;																				\
					}
			...
			bool ELECTRADECODERS_API ParseVideoParameterSet(TMap<uint32, FVideoParameterSet>& InOutVideoParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)													\
					if (!(expr))																		\
					{																					\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in VPS", elem);	\
						return false;																	\
					}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Not a VPS NALU!");
					return false;
				}
				if (br.GetRemainingByteLength() < 4)
				{
					UE_LOGF(LogElectraDecoders, Error, "Insufficient bytes in bitstream to be a VPS NALU!");
					return false;
				}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "vps_base_layer_internal_flag==0 && vps_base_layer_available_flag==1 is not supported!");
					return false;
				}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Bad rbsp_trailing_bits in VPS NALU!");
				}
				return true;
				#undef RANGE_CHECK_FAILURE
			}
			...
			bool ParseSequenceParameterSet(TMap<uint32, FSequenceParameterSet>& InOutSequenceParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)													\
					if (!(expr))																		\
					{																					\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in SPS", elem);	\
						return false;																	\
					}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Not an SPS NALU!");
					return false;
				}
				if (br.GetRemainingByteLength() < 4)
				{
					UE_LOGF(LogElectraDecoders, Error, "Insufficient bytes in bitstream to be an SPS NALU!");
					return false;
				}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Range extensions are not supported");
					return false;
				}

				if (sps.sps_multilayer_extension_flag)
				{
					UE_LOGF(LogElectraDecoders, Error, "Multilayer extensions are not supported");
					return false;
				}

				if (sps.sps_3d_extension_flag)
				{
					UE_LOGF(LogElectraDecoders, Error, "3D extensions are not supported");
					return false;
				}

				if (sps.sps_scc_extension_flag)
				{
					UE_LOGF(LogElectraDecoders, Error, "Screen content coding extensions are not supported");
					return false;
				}
				...
				if (!br.rbsp_trailing_bits())
				{
					UE_LOGF(LogElectraDecoders, Error, "Bad rbsp_trailing_bits in SPS NALU!");
					return false;
				}
			...
			bool ParsePictureParameterSet(TMap<uint32, FPictureParameterSet>& InOutPictureParameterSets, const TMap<uint32, FSequenceParameterSet>& InSequenceParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)													\
					if (!(expr))																		\
					{																					\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in PPS", elem);	\
						return false;																	\
					}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Not a PPS NALU!");
					return false;
				}
				if (br.GetRemainingByteLength() < 4)
				{
					UE_LOGF(LogElectraDecoders, Error, "Insufficient bytes in bitstream to be a PPS NALU!");
					return false;
				}
				...
				if (!br.rbsp_trailing_bits())
				{
					UE_LOGF(LogElectraDecoders, Error, "Bad rbsp_trailing_bits in PPS NALU!");
					return false;
				}
				return true;
				#undef RANGE_CHECK_FAILURE
			...
			{
				UE_LOGF(LogElectraDecoders, Error, "Range extension is not supported");
				return false;
			}

			bool FPictureParameterSet::FPPSMultilayerExtension::Parse(FBitstreamReaderH265& br)
			{
				UE_LOGF(LogElectraDecoders, Error, "Multilayer extension is not supported");
				return false;
				/*
				#define RANGE_CHECK_FAILURE(expr, elem)																		\
					if (!(expr))																							\
					{																										\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in pps_multilayer_extension(", elem);	\
						return false;																						\
					}
			...
			{
				UE_LOGF(LogElectraDecoders, Error, "3D extension is not supported");
				return false;
			}

			bool FPictureParameterSet::FPPSSCCExtension::Parse(FBitstreamReaderH265& br)
			{
				UE_LOGF(LogElectraDecoders, Error, "SCC extension is not supported");
				return false;
			}
			...
			bool ParseSliceHeader(TUniquePtr<FRBSP>& OutRBSP, FBitstreamReaderH265& OutRBSPReader, FSliceSegmentHeader& OutSlice, const TMap<uint32, FVideoParameterSet>& InVideoParameterSets, const TMap<uint32, FSequenceParameterSet>& InSequenceParameterSets, const TMap<uint32, FPictureParameterSet>& InPictureParameterSets, const uint8* InBitstream, uint64 InBitstreamLenInBytes)
			{
				#define RANGE_CHECK_FAILURE(expr, elem)																	\
					if (!(expr))																						\
					{																									\
						UE_LOGF(LogElectraDecoders, Error, "Invalid value for %ls in slice_segment_header", elem);	\
						return false;																					\
					}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Not a supported slice type (%d)!", nal_unit_type);
					return false;
				}
				...
				if (!ppsPtr)
				{
					UE_LOGF(LogElectraDecoders, Error, "Reference picture parameter set not found!");
					return false;
				}
				...
				if (!spsPtr)
				{
					UE_LOGF(LogElectraDecoders, Error, "Reference sequence parameter set not found!");
					return false;
				}
				...
				if (!OutSlice.dependent_slice_segment_flag)
				{
					...
					if (!bIsIDR)
					{
						...
						{
							RANGE_CHECK_FAILURE(false, TEXT("sps.num_short_term_ref_pic_sets is 0"));
						}
				...
				{
					UE_LOGF(LogElectraDecoders, Error, "Bad byte_alignment in slice_segment_header!");
					return false;
				}
				return true;
				#undef RANGE_CHECK_FAILURE
				...
				for(int32 i=0; i<DPBEntries.Num(); ++i)
				{
					if (DPBEntries[i].DPBIndex >= 0)
					{
						if (!DPBEntries[i].bWasAlreadyOutput && !DPBEntries[i].bIsMissing)
						{
							...
							UE_LOGF(LogElectraDecoders, Log, "--- OUTPUT: POC %d `flush to empty`", DPBEntries[i].POC);
#endif
						}
					}
					DPBEntries[i].Reset();
			...
			void FDecodedPictureBuffer::BumpPictures(TArray<FDPBOutputFrame>& OutFrames, const FSliceSegmentHeader& InFromSliceHeader, bool bAdditionalBumping)
			{
				...
				while(1)
				{
					...
					if (NumNeededForOutput > sps_max_num_reorder_pics || (sps_max_latency_increase_plus1 !=0 && NumLatencyCount > 0) || (!bAdditionalBumping && NumInDPB >= sps_max_dec_pic_buffering))
					{
						...
						{
							UE_LOGF(LogElectraDecoders, Log, "--- OUTPUT %d: POC %d `sps_max_num_reorder_pics`", bAdditionalBumping, DPBEntries[IndexOfSmallestPOC].POC);
						}
						if (sps_max_latency_increase_plus1 !=0 && NumLatencyCount > 0)
						{
							UE_LOGF(LogElectraDecoders, Log, "--- OUTPUT %d: POC %d `sps_max_latency_increase_plus1`", bAdditionalBumping, DPBEntries[IndexOfSmallestPOC].POC);
						}
						if (!bAdditionalBumping && NumInDPB >= sps_max_dec_pic_buffering)
						{
							UE_LOGF(LogElectraDecoders, Log, "--- OUTPUT %d: POC %d `sps_max_dec_pic_buffering`", bAdditionalBumping, DPBEntries[IndexOfSmallestPOC].POC);
						}
#endif
			...
			bool FDecodedPictureBuffer::SetLastError(const FString& InLastErrorMsg)
			{
				...
				UE_LOGF(LogElectraDecoders, Error, "%ls", *InLastErrorMsg);
				return false;
			}
