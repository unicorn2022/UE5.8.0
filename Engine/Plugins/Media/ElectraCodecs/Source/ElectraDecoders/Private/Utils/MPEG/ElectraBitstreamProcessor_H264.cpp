...
FElectraDecoderBitstreamProcessorH264::FElectraDecoderBitstreamProcessorH264(const TMap<FString, FVariant>& InDecoderParams, const Electra::FCodecTypeFormat& InInitialCodecFormat, const TMap<FString, FVariant>& InAdditionalFormatParams)
{
...
IElectraDecoderBitstreamProcessor::EProcessResult FElectraDecoderBitstreamProcessorH264::ProcessInputForDecoding(TSharedPtr<IElectraDecoderBitstreamInfo, ESPMode::ThreadSafe>& OutBSI, FElectraDecoderInputAccessUnit& InOutAccessUnit, const TMap<FString, FVariant>& InAccessUnitSidebandData)
{
	...
	while(NALU < LastNALU)
	{
		uint32 naluLen = MP4Utilities::GetFromBigEndian(*NALU);
		...
		{
			*NALU = MP4Utilities::GetFromBigEndian(0x00000001U);
		}
