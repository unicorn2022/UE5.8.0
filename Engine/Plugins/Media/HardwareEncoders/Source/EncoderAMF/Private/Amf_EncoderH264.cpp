...
namespace AVEncoder
{
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		{
			UE_LOGF(LogEncoderAMF, Error, "Amf not avaliable.");
			return false;
		}
		...
		default:
			UE_LOGF(LogEncoderAMF, Error, "Frame format %ls is not currently supported by Amf Encoder on this platform.", *ToString(FrameFormat));
			return false;
		}
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		if (!EncoderDevice)
		{
			UE_LOGF(LogEncoderAMF, Error, "Amf needs an encoder device.");
			return false;
		}
		...
		{	
			if (RHIType != ERHIInterfaceType::Vulkan)
			{
				...
				{
					UE_LOGF(LogEncoderAMF, Error, "Amf component not initialised");
				}
			}
			else
			{
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			if(Res != AMF_OK)
			{
				UE_LOGF(LogEncoderAMF, Error, "AMF failed to encode frame.");
			}
		}
	}
	...
	void FVideoEncoderAmf_H264::FAMFLayer::MaybeReconfigure()
	{
		...
		{	
			...
			if (CurrentConfig.Width != CurrentWidth || CurrentConfig.Height != CurrentHeight || CurrentConfig.MaxFramerate != CurrentFrameRate)
			{
				...
				if (Result != AMF_OK)
				{
					UE_LOGF(LogEncoderAMF, Error, "Amf failed to ReInit for config change");
				}
			}
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to set min qp");
			}
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to set max qp");
			}
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to set target bitrate");
			}
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to set rate control method");
			}
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to set peak bitrate");
			}
#endif
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		if (Buffer)
		{
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				...
				{
					UE_LOGF(LogEncoderAMF, Error, "Amf failed to force IDR picture type");
				}

				if (Buffer->Surface->SetProperty(AMF_VIDEO_ENCODER_INSERT_SPS, true) != AMF_OK)
				{				
					UE_LOGF(LogEncoderAMF, Error, "Amf failed to force SPS");
				}		
					
				if (Buffer->Surface->SetProperty(AMF_VIDEO_ENCODER_INSERT_PPS, true) != AMF_OK)
				{				
					UE_LOGF(LogEncoderAMF, Error, "Amf failed to force PPS");
				}
			}
			...
			if (Buffer.IsValid())
			{
				...
				{
					UE_LOGF(LogEncoderAMF, Error, "Amf submit error with %d", Result);
					// release input frame
					frame->Release();
				}
				else
	...
	void FVideoEncoderAmf_H264::FAMFLayer::ProcessFrameBlocking()
	{
		...
		while(bIsProcessingFrame)
		{
			...
			{
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to get picture type.");
			}
			else if (PictureType == AMF_VIDEO_ENCODER_OUTPUT_DATA_TYPE_IDR)
			{
				UE_LOGF(LogEncoderAMF, Verbose, "Generated IDR Frame");
				PRAGMA_DISABLE_DEPRECATION_WARNINGS
			...
			if (RHIGetInterfaceType() != ERHIInterfaceType::Vulkan) // Amf with Vulkan doesn't currently support statistics
			{
				...
				PRAGMA_ENABLE_DEPRECATION_WARNINGS
				{
					UE_LOGF(LogEncoderAMF, Error, "Amf failed to get frame QP.");
				}
			}
			...
			{
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to get encode start time.");
			}
			...
			{
				UE_LOGF(LogEncoderAMF, Fatal, "Amf failed to get buffer input frame.");
			}
	...
	TSharedPtr<FVideoEncoderAmf_H264::FAMFLayer::FInputOutput> FVideoEncoderAmf_H264::FAMFLayer::GetOrCreateSurface(const TSharedPtr<FVideoEncoderInputFrameImpl> InFrame)
	{
		...
		default:
			UE_LOGF(LogEncoderAMF, Error, "Video Frame Format %ls not supported by Amf on this platform.", *ToString(InFrame->GetFormat()));
			break;
		}
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		{
			UE_LOGF(LogEncoderAMF, Fatal, "Got passed a null pointer.");
				return nullptr;
		}
		...
		if (!Buffer)
		{
			...
			{
				InFrame->Release();
				UE_LOGF(LogEncoderAMF, Error, "Amf failed to create buffer.");
			}
			else
			{
	...
	bool FVideoEncoderAmf_H264::FAMFLayer::CreateSurface(TSharedPtr<FVideoEncoderAmf_H264::FAMFLayer::FInputOutput>& OutBuffer, const TSharedPtr<FVideoEncoderInputFrameImpl> SourceFrame, void* TextureToCompress)
	{
		...
		if (TextureToCompress)
		{
			...
			case EVideoFrameFormat::Undefined:
			default:
				UE_LOGF(LogEncoderAMF, Error, "Video format %ls not inplemented for Amf on this platform", *ToString(SourceFrame->GetFormat()));
				break;
			}
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
		...
		{
			UE_LOGF(LogEncoderAMF, Error, "Amf received nullptr to texture.");
			return false;
		}
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		{
			UE_LOGF(LogEncoderAMF, Warning, "Failed to created AMF encoder on AMD hardware. Consider trying a different driver version.");
			return false;
		}
		...
		{
			UE_LOGF(LogEncoderAMF, Error, "Failed to query min/max h264 level supported by Amf (reported max=%d).", LevelMax);
			bSuccess = false;
		}
