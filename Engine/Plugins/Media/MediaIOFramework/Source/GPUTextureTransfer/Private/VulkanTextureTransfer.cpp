...
{
	DVPStatus FVulkanTextureTransfer::Init_Impl()
	{
		IVulkanDynamicRHI* VulkanRHI = GetIVulkanDynamicRHI();

		VulkanDevice = VulkanRHI->RHIGetVkDevice();
		VulkanQueue = VulkanRHI->RHIGetGraphicsVkQueue();

		uint8 RHIDeviceUUID[16] {};
		FMemory::Memcpy(RHIDeviceUUID, VulkanRHI->RHIGetVulkanDeviceUUID(), 16);

		return dvpInitVkDevice(VulkanDevice, 0, RHIDeviceUUID);
	}
