...
ON_SCOPE_EXIT{ UE_LOGF(LogGPUTextureTransfer, Verbose, "%ls duration: %d ms", L#FuncName,	AutoTimer.GetTime()); }
#else
#define LOG_PERF(...)
#endif
...
{\
	...
	{\
		UE_LOGF(LogGPUTextureTransfer, Error, "GPUDirect call %ls failed. Error:  %d", L""#call, _Status);\
	}\
}
...
namespace UE::GPUTextureTransfer::Private
{
	...
	bool FTextureTransferBase::Initialize()
	{
		...
		if (Init_Impl() != DVP_STATUS_OK)
		{
			UE_LOGF(LogGPUTextureTransfer, Display, "GPU Direct failed to initialize.");
			return false;
		}
		...
		if (ensureAlways(Info))
		{
			if (Info->GPUMemorySync && Info->SystemMemorySync)
			{
				...
				if (TransferDirection == ETransferDirection::GPU_TO_CPU)
				{
					...
					{
						bSuccess = false;
						UE_LOGF(LogGPUTextureTransfer, Error, "GPU Direct failed to sync.");
					}
				}
			}
			else
			{
				bSuccess = false;
				UE_LOGF(LogGPUTextureTransfer, Error, "Sync info was cleared prematurely while performing a GPU DMA Transfer sync");
			}
		}
	...
	bool FTextureTransferBase::TransferTexture(void* InBuffer, FRHITexture* InRHITexture, ETransferDirection TransferDirection)
	{
		...
		if (BufferInfo->GPUMemorySync && BufferInfo->SystemMemorySync)
		{
			...
			{
				UE_LOGF(LogGPUTextureTransfer, Error, "Error while performing a GPU transfer texture. Error: '%d'.", Status);
				return false;
		...
		{
			UE_LOGF(LogGPUTextureTransfer, Warning, "Error while performing a GPU transfer texture: A sync object was not found.");
		}
