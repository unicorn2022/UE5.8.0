		...
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
			{
				...
				UE_LOGF(LogMediaIOAudioDebug, Display, "Starting to dump audio for %d seconds", SecondsToDumpFor);
	...
	struct FAudioDebugDump
	{
		...
		void WriteToWavFile()
		{
			if (bStarted && CollectedSamples.Num())
			{
				...
				{
					UE_LOGF(LogMediaIOAudioDebug, Error, "Could not write wav file!");
				}
			}
		}
	...
	struct FAudioDebugDumpSystem
	{
		...
		void ProcessSamples(const FString& BaseFileName, const TArray<Type>& BufferToWrite)
		{
			if (FAudioDebugDump* DebugDump = DebugInfoMap.Find(BaseFileName))
			{
				...
				if (!DebugDump->bStarted)
				{
					UE_LOGF(LogMediaIOAudioDebug, Display, "Finished audio dump for %ls", *DebugDump->BaseFileName);
