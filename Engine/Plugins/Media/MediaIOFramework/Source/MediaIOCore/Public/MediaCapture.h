...
public:
	...
	/**
	 * When enabled, editor-only overlays such as actor sprites (player start, light icons, etc.)
	 * are hidden during capture. Has no effect in non-editor builds.
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="MediaCapture")
	bool bDisableEditorSpritesDuringCapture = true;
...
UENUM()
enum class EMediaCaptureRotation : uint8
{
...
struct FMediaCaptureTransform
{
	...
	EMediaCaptureRotation Rotation = EMediaCaptureRotation::None;
	...
	UE_API float GetRotationValue(bool bInDegrees = false) const;
	UE_API FVector2f GetScaleValue() const;
};
...
public:
	...
	/**
	 * Stop the actual capture if there is one.
	 * Then capture every frame for a MediaTexture.
	 * The MediaTexture needs to be of the same size and have the same pixel format as requested by the media output.
	 * @return True if the capture was successfully started
	 */
	UFUNCTION(BlueprintCallable, Category = "Media|Output")
	UE_API bool CaptureMediaTexture(UMediaTexture* InMediaTexture, FMediaCaptureOptions CaptureOptions, float Rotation, const FVector2f& Scale);
	...
	/** Called after initialize for media texture capture type */
	virtual bool PostInitializeCaptureMediaTexture(UMediaTexture* InMediaTexture) { return true; }
	...
	virtual bool UpdateMediaTextureImpl(UMediaTexture* InMediaTexture) { return true; }
...
#if WITH_EDITOR
	/** Gets the prefix to attach to engine analytics events for this media capture */
	UE_API virtual FString GetAnalyticsEventPrefix() const { return TEXT("MediaFramework.MediaCapture"); }
	
	/** Allows media captures to add additional analytics event attributes to an analytics event being recorded */
	UE_API virtual void GetAnalyticsEventAttributes(const FString& EventName, TArray<FAnalyticsEventAttribute>& InOutAttributes) const;
	
	/** Records a new event to engine analytics to indicate the capture has been started */
	UE_API void RecordCaptureStartedEvent() const;
	
	/**
	 * Gets whether the media capture should automatically record a capture started event after InitializeCapture is called.
	 * Media captures that operate asynchronously should set this to false and call RecordCaptureStartedEvent manually when appropriate.
	 */
	UE_API virtual bool ShouldAutoRecordCaptureStartedEvent() const { return true; }
#endif
...
private:
	...
	void OnViewportRendered_GameThread(FViewport* Viewport);
	...
	void OnBackBufferReadyToPresent(SWindow& SlateWindow, ISlateViewportProvider& ViewportProvider);
