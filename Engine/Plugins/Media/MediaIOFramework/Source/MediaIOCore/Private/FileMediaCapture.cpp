...
void UFileMediaCapture::OnFrameCaptured_RenderingThread(const FCaptureBaseData& InBaseData, TSharedPtr<FMediaCaptureUserData, ESPMode::ThreadSafe> InUserData, void* InBuffer, int32 Width, int32 Height, int32 BytesPerRow)
{
	...
	ImageTask->Filename = FString::Printf(TEXT("%s%05lld.%s"), *BaseFilePathName, InBaseData.SourceFrameNumber, ImageModulePtr->GetExtension(ImageFormat));
	...
	if (PixelFormat == PF_B8G8R8A8)
	{
		...
		{
			UE_LOGF(LogMediaIOCore, Error, "File media capture only supports tightly packed rows. Expected stride: %d. Received stride: %d. It might also mean that output resolution is too small.", Width * 4, BytesPerRow);
			SetState(EMediaCaptureState::Error);
			return;
		}
	...
	else if (PixelFormat == PF_FloatRGBA)
	{
		...
		{
			UE_LOGF(LogMediaIOCore, Error, "File media capture only supports tightly packed rows. Expected stride: %d. Received stride: %d. It might also mean that output resolution is too small.", Width * 8, BytesPerRow);
			SetState(EMediaCaptureState::Error);
			return;
		}
