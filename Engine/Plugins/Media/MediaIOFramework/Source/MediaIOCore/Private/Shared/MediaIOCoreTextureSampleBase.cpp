...
bool FMediaIOCoreTextureSampleBase::Initialize(const void* InVideoBuffer, uint32 InBufferSize, uint32 InStride, uint32 InWidth, uint32 InHeight, EMediaTextureSampleFormat InSampleFormat, FTimespan InTime, const FFrameRate& InFrameRate, const TOptional<FTimecode>& InTimecode, TSharedPtr<FNativeMediaSourceColorSettings, ESPMode::ThreadSafe> InSourceColorSettings)
{
	FreeSample();

	if (!SetProperties(InStride, InWidth, InHeight, InSampleFormat, InTime, InFrameRate, InTimecode, InSourceColorSettings))
	{
		return false;
	}
...
bool FMediaIOCoreTextureSampleBase::Initialize(TArray<uint8>&& InVideoBuffer, uint32 InStride, uint32 InWidth, uint32 InHeight, EMediaTextureSampleFormat InSampleFormat, FTimespan InTime, const FFrameRate& InFrameRate, const TOptional<FTimecode>& InTimecode, TSharedPtr<FNativeMediaSourceColorSettings, ESPMode::ThreadSafe> InSourceColorSettings)
{
	FreeSample();
...
bool FMediaIOCoreTextureSampleBase::SetProperties(uint32 InStride, uint32 InWidth, uint32 InHeight, EMediaTextureSampleFormat InSampleFormat, FTimespan InTime, const FFrameRate& InFrameRate, const TOptional<FTimecode>& InTimecode, TSharedPtr<FNativeMediaSourceColorSettings, ESPMode::ThreadSafe> InSourceColorSettings)
{
...
bool FMediaIOCoreTextureSampleBase::InitializeWithEvenOddLine(bool bUseEvenLine, const void* InVideoBuffer, uint32 InBufferSize, uint32 InStride, uint32 InWidth, uint32 InHeight, EMediaTextureSampleFormat InSampleFormat, FTimespan InTime, const FFrameRate& InFrameRate, const TOptional<FTimecode>& InTimecode, TSharedPtr<FNativeMediaSourceColorSettings, ESPMode::ThreadSafe> InSourceColorSettings)
{
	...
	if (!SetProperties(InStride, InWidth, InHeight/2, InSampleFormat, InTime, InFrameRate, InTimecode, InSourceColorSettings))
	{
		return false;
	}
...
void FMediaIOCoreTextureSampleBase::CopyConfiguration(const TSharedPtr<FMediaIOCoreTextureSampleBase>& SourceSample)
{
	...
	SourceColorSettings = SourceSample->SourceColorSettings;
...
{
	return Initialize(InVideoBuffer, InBufferSize, InStride, InWidth, InHeight, InSampleFormat, InTime, InFrameRate, InTimecode, nullptr);
}

bool FMediaIOCoreTextureSampleBase::Initialize(const TArray<uint8>& InVideoBuffer, uint32 InStride, uint32 InWidth, uint32 InHeight, EMediaTextureSampleFormat InSampleFormat, FTimespan InTime, const FFrameRate& InFrameRate, const TOptional<FTimecode>& InTimecode, const UE::MediaIOCore::FColorFormatArgs& InColorFormatArgs)
{
	return Initialize(InVideoBuffer, InStride, InWidth, InHeight, InSampleFormat, InTime, InFrameRate, InTimecode, nullptr);
}
...
{
	return SetProperties(InStride, InWidth, InHeight, InSampleFormat, InTime, InFrameRate, InTimecode, nullptr);
}

bool FMediaIOCoreTextureSampleBase::InitializeWithEvenOddLine(bool bUseEvenLine, const void* InVideoBuffer, uint32 InBufferSize, uint32 InStride, uint32 InWidth, uint32 InHeight, EMediaTextureSampleFormat InSampleFormat, FTimespan InTime, const FFrameRate& InFrameRate, const TOptional<FTimecode>& InTimecode, const UE::MediaIOCore::FColorFormatArgs& InColorFormatArgs)
{
	return InitializeWithEvenOddLine(bUseEvenLine, InVideoBuffer, InBufferSize, InStride, InWidth, InHeight, InSampleFormat, InTime, InFrameRate, InTimecode, nullptr);
}
...
const FMatrix& FMediaIOCoreTextureSampleBase::GetYUVToRGBMatrix() const
{
	...
	const FColorSpace& SourceColorSpace = GetSourceColorSpace();

	if (SourceColorSpace.Equals(FColorSpace::GetSRGB()))
	{
		return MediaShaders::YuvToRgbRec709Scaled;
	}
	
	if (SourceColorSpace.Equals(UE::Color::FColorSpace::GetRec2020()))
	{
...
const UE::Color::FColorSpace& FMediaIOCoreTextureSampleBase::GetSourceColorSpace() const
{
	if (SourceColorSettings.IsValid())
	{
		return SourceColorSettings->GetColorSpaceOverride(UE::Color::FColorSpace::GetSRGB());
	}

	return IMediaTextureSample::GetSourceColorSpace();
}

bool FMediaIOCoreTextureSampleBase::HasEncodingOverride() const
{
	return SourceColorSettings.IsValid() ? SourceColorSettings->HasEncodingOverride() : false;
}

bool FMediaIOCoreTextureSampleBase::HasColorSpaceOverride() const
{
	return SourceColorSettings.IsValid() ? SourceColorSettings->HasColorSpaceOverride() : false;
}
...
UE::Color::EChromaticAdaptationMethod FMediaIOCoreTextureSampleBase::GetChromaticAdapationMethod() const
{
	if (SourceColorSettings.IsValid())
	{
...
{
	// OCIO handles color conversion itself.
	if (ColorConversionSettings && ColorConversionSettings->IsValid())
	{
		return 1.0f;
	}
	...
	// User override
	// Note that None encodings automatically reset the reference white to None upstream.
	if (SourceColorSettings.IsValid())
	{
		const UE::Color::EReferenceWhite UserReferenceWhite = SourceColorSettings->GetReferenceWhiteOverride();
		if (UserReferenceWhite != UE::Color::EReferenceWhite::None)
		{
			return UE::Color::GetReferenceWhiteLinearScale(CurrentEncoding, UserReferenceWhite);
		}
	}

	// MediaIO default: HDR streams follow BT.2408.
	if (CurrentEncoding == UE::Color::EEncoding::ST2084 || CurrentEncoding == UE::Color::EEncoding::HLG)
	{
		return UE::Color::GetReferenceWhiteLinearScale(CurrentEncoding, UE::Color::EReferenceWhite::BT2408);
	}
