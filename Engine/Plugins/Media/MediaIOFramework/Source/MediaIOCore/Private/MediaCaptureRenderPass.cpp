...
DECLARE_GPU_STAT(MediaCapture_Uncompression);
...
namespace UE::MediaCapture
{
	...
	FRenderPass FRenderPass::CreateUncompressionPass(const UMediaCapture* MediaCapture)
	{
		FRenderPass UncompressionPass;
		UncompressionPass.Name = "Uncompression";
		UncompressionPass.OutputType = ERDGViewableResourceType::Texture;
		UncompressionPass.ShouldExecutePassDelegate = FRenderPass::FShouldExecutePass::CreateLambda([](const UE::MediaCaptureData::FCaptureFrameArgs& Args, FRDGViewableResource* InputResource)
		{
			check(InputResource->Type == ERDGViewableResourceType::Texture);
			FRDGTextureRef SourceRGBTexture = static_cast<FRDGTextureRef>(InputResource);
			return IsBlockCompressedFormat(SourceRGBTexture->Desc.Format);
		});
		UncompressionPass.InitializePassOutputDelegate = FRenderPass::FInitializePassOutput::CreateLambda([](const UE::MediaCapture::FInitializePassOutputArgs& Args, uint32 FrameId)
		{
			check(Args.InputResource && Args.InputResource->Type == ERDGViewableResourceType::Texture);
			FRDGTextureDesc InputResourceDesc = static_cast<FRDGTextureRef>(Args.InputResource)->Desc;
			
			FRDGTextureDesc UncompressedOutputTextureDesc = InputResourceDesc;
			UncompressedOutputTextureDesc.Reset();
			UncompressedOutputTextureDesc.NumMips = 1;
			UncompressedOutputTextureDesc.Flags = TexCreate_None;
			UncompressedOutputTextureDesc.Flags |= TexCreate_RenderTargetable | TexCreate_ShaderResource | TexCreate_UAV | TexCreate_NoFastClear;
			UncompressedOutputTextureDesc.ClearValue = FClearValueBinding(FLinearColor(0, 0, 0, 0));
			UncompressedOutputTextureDesc.Format = PF_FloatRGBA;

			TRefCountPtr<IPooledRenderTarget> RenderTarget = AllocatePooledTexture(UncompressedOutputTextureDesc, TEXT("MediaCapture Uncompressed Output"));
			return RenderTargetResource(MoveTemp(RenderTarget));
		});

		UncompressionPass.ExecutePassDelegate = FRenderPass::FExecutePass::CreateLambda([](const UE::MediaCaptureData::FCaptureFrameArgs& Args, const TSharedPtr<UE::MediaCaptureData::FCaptureFrame>& CapturingFrame, FRDGViewableResource* InputResource, FRDGViewableResource* OutputResource)
		{
			// At the moment we only support Texture inputs
			check(InputResource->Type == ERDGViewableResourceType::Texture);
			...
			RDG_EVENT_SCOPE_STAT(Args.GraphBuilder, MediaCapture_Uncompression, "MediaCapture_Uncompression");
			TRACE_CPUPROFILER_EVENT_SCOPE(UMediaCapture::Uncompression);
			
			FBaseRenderPassArgs UncompressionPassArgs;
			UncompressionPassArgs.SourceRGBTexture = static_cast<FRDGTextureRef>(InputResource);
			UncompressionPassArgs.CopyInfo = CopyInfo;
			
			AddUncompressionPass(Args, UncompressionPassArgs, OutputResource);
		});
		
		return UncompressionPass;
	}
	...
	TArray<FRenderPass> FRenderPass::CreateRenderPasses(const UMediaCapture* MediaCapture)
	{
		...
		RenderPasses.Add(CreateUncompressionPass(MediaCapture));
		...
		if (MediaCapture->GetDesiredCaptureOptions().ResizeMethod == EMediaCaptureResizeMethod::ResizeInRenderPass)
		{
			...
			{
				UE_LOGF(LogMediaIOCore, Warning, "The capture for %ls will not be cropped because ResizeMethod was already set to \'Resize in Render Pass\' and these options are mutually exclusive.", *MediaCapture->GetMediaOutputName());
			}
			else
			{
	...
	void FRenderPass::AddUncompressionPass(const UE::MediaCaptureData::FCaptureFrameArgs& Args, const FBaseRenderPassArgs& UncompressionPassArgs, FRDGViewableResource* OutputResource)
	{
		if (OutputResource->Type == FRDGBuffer::StaticType)
		{
			return;
		...
		// Very basic uncompression that draws the compressed texture into a PF_FloatRGBA texture
		AddDrawTexturePass(Args.GraphBuilder, GetGlobalShaderMap(GMaxRHIFeatureLevel), UncompressionPassArgs.SourceRGBTexture, OutputTexture, FRDGDrawTextureInfo());
	}
	...
	void FRenderPipeline::AddPass(const FRenderPass& InPass, const FName& AfterPass)
	{
		int32 Index = RenderPasses.IndexOfByPredicate([&AfterPass](const FRenderPass& RenderPass)
		{
			return RenderPass.Name == AfterPass;
		});
		
		RenderPasses.Insert(InPass, Index + 1);
	}
	...
	FRDGViewableResource* FRenderPipeline::ExecutePasses_RenderThread(const UE::MediaCaptureData::FCaptureFrameArgs& Args, const TSharedPtr<MediaCaptureData::FCaptureFrame>& CapturingFrame, FRDGBuilder& GraphBuilder, FRDGTextureRef InputRGBTexture) const
	{
		...
		if (CapturingFrame->bMediaCaptureActive)
		{
			for (const UE::MediaCapture::FRenderPass& RenderPass : RenderPasses)
			{
				...
				{
					UE_LOGF(LogMediaIOCore, Error, "The capture will stop for '%ls'. A capture frame had an invalid render resource.", *MediaCapture->GetMediaOutputName());
