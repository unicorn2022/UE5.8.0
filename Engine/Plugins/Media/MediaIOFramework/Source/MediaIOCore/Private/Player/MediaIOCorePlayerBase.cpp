...
bool FMediaIOCorePlayerBase::Open(const FString& Url, const IMediaOptions* Options)
{
	...
	OpenUrl = Url;
	MediaSourceObject = const_cast<UObject*>(Options->ToUObject());
...
void FMediaIOCorePlayerBase::TickTimeManagement()
{
	if (EvaluationType == EMediaIOSampleEvaluationType::Timecode)
	{
		...
		if (CurrentFrameTime.IsSet())
		{
			if (!bWarnedIncompatibleFrameRate)
			{
				if (!CurrentFrameTime->Rate.IsMultipleOf(VideoFrameRate) && !VideoFrameRate.IsMultipleOf(CurrentFrameTime->Rate))
				{
					...
					UE_LOGF(LogMediaIOCore, Warning, "The video's frame rate is incompatible with engine's frame rate. %ls", *OpenUrl);
				}
			}
		...
		{
			UE_LOGF(LogMediaIOCore, Warning, "The video '%ls' is configured to use timecode but none is available on the engine.", *OpenUrl);
		}
	}
	else
	{
...
bool FMediaIOCorePlayerBase::ReadMediaOptions(const IMediaOptions* Options)
{
	...
	TSharedPtr<IMediaOptions::FDataContainer, ESPMode::ThreadSafe> DefaultOption;
	const TSharedPtr<UCaptureCardMediaSource::FOpenColorIODataContainer> OCIODataContainer = StaticCastSharedPtr<UCaptureCardMediaSource::FOpenColorIODataContainer>(Options->GetMediaOption(UE::CaptureCardMediaSource::OpenColorIOSettings, DefaultOption));
	...
	TSharedPtr<IMediaOptions::FDataContainer, ESPMode::ThreadSafe> DataContainer = Options->GetMediaOption(UE::CaptureCardMediaSource::SourceColorSettingsOption, DefaultOption);
...
void FMediaIOCorePlayerBase::RegisterSampleBuffer(const TSharedPtr<FMediaIOCoreTextureSampleBase>& InSample)
{
	if (!RegisteredBuffers.Contains(InSample->GetMutableBuffer()))
	{
		...
		ENQUEUE_RENDER_COMMAND(RegisterSampleBuffers)([this, InSample](FRHICommandListImmediate&)
		{
			...
			{
				UE_LOGF(LogMediaIOCore, Error, "A buffer was not available while performing a gpu texture transfer.");
				return;
			}

			if (!InSample->GetTexture())
			{
				UE_LOGF(LogMediaIOCore, Error, "A texture was not available while performing a gpu texture transfer.");
				return;
			}
			...
			UE_LOGF(LogMediaIOCore, Verbose, "Registering buffer %llu", reinterpret_cast<uintptr_t>(InSample->GetMutableBuffer()));
	...
	{
		UE_LOGF(LogMediaIOCore, VeryVerbose, "Buffer already registered: %llu", reinterpret_cast<uintptr_t>(InSample->GetMutableBuffer()));
	}
}
...
void FMediaIOCorePlayerBase::CreateAndRegisterTextures()
{
	...
	ENQUEUE_RENDER_COMMAND(MediaIOCorePlayerCreateTextures)(
		[this](FRHICommandListImmediate& CommandList)
		{
			...
			{
				UE_LOGF(LogMediaIOCore, Error, "Could not register textures for gpu texture transfer, as the GPUTextureTransfer object was null.");
				return;
			}
			...
			for (uint8 Index = 0; Index < TexturesAmount; Index++)
			{
				...
				UE_LOGF(LogMediaIOCore, Verbose, "Registering texture %llu", reinterpret_cast<uintptr_t>(RHITexture->GetNativeResource()));
	...
	ENQUEUE_RENDER_COMMAND(UnregisterSampleBuffers)([TextureTransfer = GPUTextureTransfer, TexturesToUnregister = RegisteredTextures](FRHICommandList& CommandList)
		{
			if (TextureTransfer)
			{
				...
				{
					UE_LOGF(LogMediaIOCore, Verbose, "Unregistering texture %llu", reinterpret_cast<uintptr_t>(RHITexture->GetNativeResource()));
...
void FMediaIOCorePlayerBase::PreGPUTransfer(const TSharedPtr<FMediaIOCoreTextureSampleBase>& InSample)
{
	...
	{
		UE_LOGF(LogMediaIOCore, Display, "Unregistered texture %llu encountered while doing a gpu texture transfer.", Texture ? reinterpret_cast<uintptr_t>(Texture->GetNativeResource()) : 0);
	}
}
		...
		[MediaPlayerWeakPtr = TWeakPtr<FMediaIOCorePlayerBase>(AsShared()), InSample](FRHICommandListImmediate& RHICmdList)
	{
		if (InSample->GetTexture())
		{
				...
				MediaPlayerPtr != nullptr && MediaPlayerPtr->GPUTextureTransfer != nullptr)
			{
				...
				UE_LOGF(LogMediaIOCore, Verbose, "Starting Transfer with buffer %llu", reinterpret_cast<uintptr_t>(Buffer));
...
void FMediaIOCorePlayerBase::SetEvaluationType(ETimedDataInputEvaluationType Evaluation)
{
	...
	if (bNewEvaluationModeSet)
	{
		...
		if (UCaptureCardMediaSource* CaptureSource = Cast<UCaptureCardMediaSource>(MediaSourceObject.Get()))
		{
			CaptureSource->EvaluationType = EvaluationType;
			if (EvaluationType == EMediaIOSampleEvaluationType::Timecode)
			{
				CaptureSource->bUseTimeSynchronization = true;
			}
		}
	}
}
...
void FMediaIOCorePlayerBase::SetEvaluationOffsetInSeconds(double Offset)
{
	...
	if (UTimeSynchronizableMediaSource* Source = Cast<UTimeSynchronizableMediaSource>(MediaSourceObject.Get()))
	{
		Source->FrameDelay = FrameDelay;
		Source->TimeDelay = TimeDelay;
	}
}
...
UObject* FMediaIOCorePlayerBase::GetSettingsObject() const
{
	return MediaSourceObject.Get();
}
...
bool FMediaIOCorePlayerBase::JustInTimeSampleRender_RenderThread(FRHICommandListImmediate& RHICmdList, FTextureRHIRef& InDestinationTexture, TSharedPtr<FMediaIOCoreTextureSampleBase>& JITRProxySample)
{
	...
	{
		UE_LOGF(LogMediaIOCore, Warning, "Invalid JITR sample.");
		return false;
	}
	...
	if (!SourceSample)
	{
		UE_LOGF(LogMediaIOCore, Warning, "JustInTimeSampleRender couldn't find a sample to render");
		return false;
	}
...
TSharedPtr<FMediaIOCoreTextureSampleBase> FMediaIOCorePlayerBase::PickSampleToRenderFramelocked_RenderThread(const FFrameInfo& InFrameInformation)
{
	...
	UE_LOGF(LogMediaIOCore, Warning, "Timeout waiting for frame #%llu to be rendered.", FrameNumber);
	// Just return the most recent sample available in the queue
	return nullptr;
}
...
#if WITH_EDITOR
FString FMediaIOCorePlayerBase::GetAnalyticsEventPrefix() const
{
	return TEXT("MediaFramework.MediaIOCorePlayerBase");
}

void FMediaIOCorePlayerBase::RecordSourceOpenedEvent() const
{
	if (FEngineAnalytics::IsAvailable())
	{
		const FString EventName = TEXT("SourceOpened");
...
void FMediaIOCorePlayerBase::GetAnalyticsEventAttributes(const FString& EventName, TArray<FAnalyticsEventAttribute>& InOutAttributes) const
{
	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("ResolutionWidth"), FString::Printf(TEXT("%d"), VideoTrackFormat.Dim.X)));
	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("ResolutionHeight"), FString::Printf(TEXT("%d"), VideoTrackFormat.Dim.Y)));
