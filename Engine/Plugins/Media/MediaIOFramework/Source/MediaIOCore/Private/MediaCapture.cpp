...
#include "MediaCaptureSources.inl"
...
float FMediaCaptureTransform::GetRotationValue(bool bInDegrees) const
{
	float RotationValue;
	switch (Rotation)
	{
	case EMediaCaptureRotation::CW90:
		RotationValue = -90.0f;
		break;
	case EMediaCaptureRotation::CCW90:
		RotationValue = 90.0f;
		break;
	case EMediaCaptureRotation::CW180:
		RotationValue = 180.0f;
		break;
	case EMediaCaptureRotation::None:
	default: 
		RotationValue = 0.0f;
		break;
	}
	
	return bInDegrees ? RotationValue : FMath::DegreesToRadians(RotationValue);
}

FVector2f FMediaCaptureTransform::GetScaleValue() const
{
...
void UMediaCapture::BeginDestroy()
{
	...
	{
		UE_LOGF(LogMediaIOCore, Warning, "%ls will be destroyed and the capture was not stopped.", *GetName());
	}
	StopCapture(false);
...
bool UMediaCapture::CaptureActiveSceneViewport(FMediaCaptureOptions CaptureOptions)
{
	...
	{
		UE_LOGF(LogMediaIOCore, Warning, "Can not start the capture. No viewport could be found.");
		return false;
	}
...
bool UMediaCapture::CaptureMediaTexture(UMediaTexture* InMediaTexture, FMediaCaptureOptions CaptureOptions, float Rotation, const FVector2f& Scale)
{
	using namespace UE::MediaCapture::Private;
	return StartSourceCapture(MakeShared<FMediaTextureCaptureSource>(this, CaptureOptions, InMediaTexture, Rotation, Scale));
}
...
bool UMediaCapture::StartSourceCapture(TSharedPtr<UE::MediaCapture::Private::FCaptureSource> InSource)
{
	...
	// BackBufferReady captures the final composited frame including UI, so alpha is not meaningful.
	// Force it to one to avoid outputting garbage alpha values.
	if (DesiredCaptureOptions.CapturePhase == EMediaCapturePhase::BackBufferReady)
	{
		DesiredCaptureOptions.bForceAlphaToOneOnConversion = true;
	}
...
#if WITH_EDITOR
		if (ShouldAutoRecordCaptureStartedEvent())
		{
			RecordCaptureStartedEvent();
		}
#endif
	}
	...
	if (bInitialized)
	{
		...
		CaptureSource->ApplyShowFlagChanges();

		CaptureSource->AddPasses(*CaptureRenderPipeline);
		
		if (DesiredCaptureOptions.CapturePhase != EMediaCapturePhase::EndFrame && DesiredCaptureOptions.CapturePhase != EMediaCapturePhase::BackBufferReady)
		{
			...
			{
				RenderTarget = CaptureSource->GetSceneViewport().Get();
			}
			...
			{
				UE_LOGF(LogMediaIOCore, Warning, "Could not acquire a render target to initialize the scene view extension. Consider changing the capture phase to End Frame.");
			}
		}
		...
		if (CaptureSource->GetCaptureType() == UE::MediaCapture::Private::ECaptureType::Immediate)
		{
			if (DesiredCaptureOptions.CapturePhase == EMediaCapturePhase::BackBufferReady)
			{
				...
				{
					FSlateApplication::Get().GetRenderer()->OnBackBufferReadyToPresent().AddUObject(this, &UMediaCapture::OnBackBufferReadyToPresent);
				}
			}
		...
		else
		{
			...
			UGameViewportClient::OnViewportRendered().AddUObject(this, &UMediaCapture::OnViewportRendered_GameThread);
		}
	}
	else
	{
	...
	{
		UE_LOGF(LogMediaIOCore, Display, "Experimental scheduling and AnyThread Capture was disabled because the current RHI does not support Multithreading.");
	}
	else if (!SupportsAnyThreadCapture())
	{
		UE_LOGF(LogMediaIOCore, Display, "AnyThread Capture was disabled because the media capture implementation does not have a AnyThread callback or explicitly disabled AnyThread capture.");
	}
...
#if WITH_EDITOR
void UMediaCapture::GetAnalyticsEventAttributes(const FString& EventName, TArray<FAnalyticsEventAttribute>& InOutAttributes) const
{
	const FIntPoint Resolution = GetDesiredSize();

	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("CaptureType"), GetCaptureSourceType()));
	...
	InOutAttributes.Add(FAnalyticsEventAttribute(TEXT("UsesOCIO"), DesiredCaptureOptions.ColorConversionSettings.IsValid() ? TEXT("Yes") : TEXT("No")));
}

void UMediaCapture::RecordCaptureStartedEvent() const
{
	if (FEngineAnalytics::IsAvailable())
	{
		const FString EventName = TEXT("CaptureStarted");

		TArray<FAnalyticsEventAttribute> EventAttributes;
		GetAnalyticsEventAttributes(EventName, EventAttributes);

		FEngineAnalytics::GetProvider().RecordEvent(GetAnalyticsEventPrefix() + EventName, EventAttributes);
	}
}
#endif
...
void UMediaCapture::CaptureImmediate_RenderThread(FRDGBuilder& GraphBuilder, FRHITexture* InSourceTexture, FIntRect SourceViewRect)
{
	...
	{
		UE_LOGF(LogMediaIOCore, VeryVerbose, "[%ls] - Failed to capture resource.", *MediaOutputName);
	}
}
...
bool UMediaCapture::CaptureImmediate_RenderThread(const UE::MediaCaptureData::FCaptureFrameArgs& Args)
{
	...
	{
		UE_LOGF(LogMediaIOCore, VeryVerbose, "Could not capture frame. Output resources haven't been initialized");
		return false;
	}
	...
	{
		UE_LOGF(LogMediaIOCore, Error, "[%ls] - Trying to capture a RHI resource with another capture type.", *MediaOutputName);
	...
	{
		UE_LOGF(LogMediaIOCore, Error, "[%ls] - Trying to capture a RHI resource with an invalid source.", *MediaOutputName);
		SetState(EMediaCaptureState::Error);
		return false;
	}
	...
	{
		UE_LOGF(LogMediaIOCore, Warning, "Can't capture frame. Could not find the matching game frame %llu.", GFrameCounterRenderThread);
		return false;
	}
	...
	if (!CapturingFrame && GetState() != EMediaCaptureState::StopRequested)
	{
		if (DesiredCaptureOptions.OverrunAction == EMediaCaptureOverrunAction::Flush)
		{
			...
			if (!CapturingFrame)
			{
				UE_LOGF(LogMediaIOCore, Error, "[%ls] - No frames available for capture for frame %llu. This should not happen.",
		...
		else
		{
			...
			UE_LOGF(LogMediaIOCore, Warning, "[%ls] - No frames available for capture of frame %llu. Skipping",
				*MediaOutputName, GFrameCounterRenderThread);
		}
	}
	...
	UE_LOGF(LogMediaIOCore, Verbose, "[%ls - %ls] - Capturing frame %d", *MediaOutputName, *FThreadManager::GetThreadName(FPlatformTLS::GetCurrentThreadId()), CapturingFrame ? CapturingFrame->FrameId : -1);
	...
	if (!UseExperimentalScheduling())
	{
		...
		{
			UE_LOGF(LogMediaIOCore, Verbose, "[%ls - %ls] - Processing pending frame %d", *MediaOutputName, *FThreadManager::GetThreadName(FPlatformTLS::GetCurrentThreadId()), NextPending ? NextPending->FrameId : -1);
...
void UMediaCapture::RestartCapture()
{
	...
	UE_LOGF(LogMediaIOCore, Log, "Media Capture restarting for new size %dx%d", CaptureSource->GetSize().X, CaptureSource->GetSize().Y);
...
void UMediaCapture::OnBackBufferReadyToPresent(SWindow& SlateWindow, ISlateViewportProvider& ViewportProvider)
{
	if (GetState() != EMediaCaptureState::Stopped)
	{
		if (CaptureSource->GetTargetWindow().Get() == &SlateWindow)
		{
			FRDGBuilder GraphBuilder(FRHICommandListImmediate::Get());
			TryCaptureImmediate_RenderThread(GraphBuilder, ViewportProvider.GetBackBufferResource());
...
void UMediaCapture::ProcessCapture_GameThread()
{
	...
	if (!CapturingFrame && GetState() != EMediaCaptureState::StopRequested)
	{
		if (DesiredCaptureOptions.OverrunAction == EMediaCaptureOverrunAction::Flush)
		{
			...
			UE_LOGF(LogMediaIOCore, VeryVerbose, "[%ls] - Flushing commands.", *MediaOutputName);
			...
			if (!CapturingFrame)
			{
				UE_LOGF(LogMediaIOCore, Error, "[%ls] - Flushing commands didn't give us back available frames to process." , *MediaOutputName);
				StopCapture(false);
				return;
			}
		}
		else if (DesiredCaptureOptions.OverrunAction == EMediaCaptureOverrunAction::Skip)
		{
			...
			UE_LOGF(LogMediaIOCore, Verbose, "[%ls] - No frames available for capture. Skipping", *MediaOutputName);
		}
	}
...
void UMediaCapture::PrepareAndDispatchCapture_GameThread(const TSharedPtr<UE::MediaCaptureData::FCaptureFrame>& CapturingFrame)
{
	...
	//The Lock only synchronize while we are copying the value to the enqueue. The viewport and the rendertarget may change while we are in the enqueue command.
	{
		...
		if (CaptureSource.IsValid())
		{
			...
			UE_LOGF(LogMediaIOCore, Verbose, "[%ls] - Queuing frame to capture %d", *MediaOutputName, CapturingFrame ? CapturingFrame->FrameId : -1);
				...
				[WeakThis, CaptureSource_RT = CaptureSource, CapturingFrame, InDesiredSize = DesiredSize, SourceGPUMask](FRHICommandListImmediate& RHICmdList)
			{
				...
				// Capture frame
				{
					...
					UE_LOGF(LogMediaIOCore, Verbose, "[%ls] - Captured frame %d", *This->MediaOutputName, CapturingFrame ? CapturingFrame->FrameId : -1);
				...
				if (!This->UseExperimentalScheduling())
				{
					...
					UE_LOGF(LogMediaIOCore, Verbose, "[%ls] - Processing pending frame %d", *This->MediaOutputName, NextPending ? NextPending->FrameId : -1);
					if (NextPending)
					{
...
void UMediaCapture::InitializeOutputResources(int32 InNumberOfBuffers)
{
	...
	{
		UE_LOGF(LogMediaIOCore, Error, "Can't start the capture. The size requested is negative or zero.");
		SetState(EMediaCaptureState::Error);
		return;
	}
	...
	[WeakThis](FRHICommandListImmediate& RHICmdList)
		{
			...
			for (int32 Index = 0; Index < This->NumberOfCaptureFrame; ++Index)
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogMediaIOCore, Error, "Can't start the capture. Trying to allocate buffer resource but number of elements to allocate was 0.");
...
bool UMediaCapture::ValidateMediaOutput() const
{
	...
	{
		UE_LOGF(LogMediaIOCore, Error, "Can not start the capture. The Media Output is invalid.");
		return false;
	}
	...
	{
		UE_LOGF(LogMediaIOCore, Error, "Can not start the capture. %ls.", *FailureReason);
		return false;
	}

	if(DesiredCaptureOptions.bAutostopOnCapture && DesiredCaptureOptions.NumberOfFramesToCapture < 1)
	{
		UE_LOGF(LogMediaIOCore, Error, "Can not start the capture. Please set the Number Of Frames To Capture when using Autostop On Capture in the Media Capture Options");
		return false;
	}
...
void UMediaCapture::OnViewportRendered_GameThread(FViewport* Viewport)
{
	if (CaptureSource.IsValid() && CaptureSource->GetSceneViewport().Get() == Viewport)
	{
...
void UMediaCapture::OnEndFrame_GameThread()
{
	...
	// Scene viewport captures are processed during viewport rendering (via OnViewportRendered_GameThread).
	if (!CaptureSource.IsValid() || !CaptureSource->GetSceneViewport().IsValid())
	{
		ProcessCapture_GameThread();
	}
}
	...
	if (CapturingFrame)
	{
		...
		if (Args.RDGResourceToCapture || Args.ResourceToCapture)
		{
			...
			if (!CapturingFrame->bPrerollFrame && !bIsReadyToStartOutput && !bLoggedOutputNotReady)
			{
				...
				UE_LOGF(LogMediaIOCore, Display, "Skipping frame since capture implementation is not ready to output yet.");
			}

			if (!CapturingFrame->bPrerollFrame && bIsReadyToStartOutput)
			{
				UE_LOGF(LogMediaIOCore, Verbose, "Capturing %ls frame (%d)", FApp::GetTimecode().Frames % 2 == 0 ? TEXT("Even") : TEXT("Odd"), FApp::GetTimecode().Frames);
				...
				if (Args.MediaCapture->ShouldCaptureThisFrame(CapturingFrame->CaptureBaseData))
				{
					...
					if (DesiredOutputResourceType == EMediaCaptureResourceType::Texture)
					{
						...
						else
						{
							...
							UE_LOGF(LogMediaIOCore, Error, "The capture will stop for '%ls'. Capture frame was expected to use Texture resource but wasn't.", *Args.MediaCapture->MediaOutputName);
						}
					}
					else
					{
						...
						{
							bBeforeCaptureSuccess = false;
							UE_LOGF(LogMediaIOCore, Error, "The capture will stop for '%ls'. Capture frame was expected to use Buffer resource but wasn't.", *Args.MediaCapture->MediaOutputName);
						}
					}
				...
				else
				{
					...
					UE_LOGF(LogMediaIOCore, Verbose, "=============================================================== Skipping frame %d", FApp::GetTimecode().Frames);
				}
			}
			else
			{
		...
		{
			UE_LOGF(LogMediaIOCore, Error, "The capture will stop for '%ls'. No texture was acquired for the capture.", *Args.MediaCapture->MediaOutputName);
		}
...
bool UMediaCapture::ProcessReadyFrame_RenderThread(FRHICommandListImmediate& RHICmdList, UMediaCapture* InMediaCapture, const TSharedPtr<UE::MediaCaptureData::FCaptureFrame>& ReadyFrame)
{
	...
	if (InMediaCapture->GetState() != EMediaCaptureState::Error)
	{
		if (ReadyFrame->bReadbackRequested)
		{
			...
			{
				UE_LOGF(LogMediaIOCore, VeryVerbose, "[%ls] - Readback %d not ready. Skipping.", *InMediaCapture->MediaOutputName, ReadyFrame->FrameId);
			...
			{
				{
					...
					TRACE_CPUPROFILER_EVENT_SCOPE_TEXT(*FString::Printf(TEXT("MediaCapture Output Frame %lld"), ReadyFrame->CaptureBaseData.SourceFrameNumberRenderThread % 10));
			...
			if (bIsReadbackReady)
			{
				UE_LOGF(LogMediaIOCore, Verbose, "[%ls - %ls] - Completed pending frame %d.", *InMediaCapture->MediaOutputName, *FThreadManager::GetThreadName(FPlatformTLS::GetCurrentThreadId()), ReadyFrame->FrameId);
...
{
	UE_LOGF(LogMediaIOCore, VeryVerbose, "%ls", *FrameManager->GetFramesState());
}
...
void UMediaCapture::PostCaptureCleanup()
{
	...
	if (CaptureSource)
	{
		CaptureSource->RestoreShowFlags();
		constexpr bool bFlushRenderingCommands = false;
		CaptureSource->ResetSourceBufferSize(bFlushRenderingCommands);
		CaptureSource.Reset();
	}
