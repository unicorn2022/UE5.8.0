...
namespace UE::MediaCapture
{
	...
	struct FRenderPass
	{
		...
		/** Add a pass to uncompress a compressed texture into a proper uncompressed format (currently PF_FloatRGBA) */
		static void AddUncompressionPass(const UE::MediaCaptureData::FCaptureFrameArgs& Args, const FBaseRenderPassArgs& UncompressionPassArgs, FRDGViewableResource* OutputResource);
	...
	private:
		...
		/** Creates an uncompression pass according to the capture settings. */
		static FRenderPass CreateUncompressionPass(const UMediaCapture* MediaCapture);
	};
	...
	public:
		...
		/** Adds a render pass to the media capture render pipeline, optionally positioning it after the pass with the specified name */
		void AddPass(const FRenderPass& InPass, const FName& AfterPass = NAME_None);
