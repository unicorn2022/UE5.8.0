...
public:
	...
	FVideoDecoderVPxElectra(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
private:
	struct FDecoderInput
	{
		~FDecoderInput()
		{
			...
			if (InputAlphaDataCopy)
			{
				FMemory::Free(InputAlphaDataCopy);
			}
		}
...
class FVPxVideoDecoderElectraFactory : public IElectraCodecFactory, public IElectraCodecFactory::IProviderInformation, public IElectraCodecModularFeature, public TSharedFromThis<FVPxVideoDecoderElectraFactory, ESPMode::ThreadSafe>
{
public:
	virtual ~FVPxVideoDecoderElectraFactory()
	{ }

	FVPxVideoDecoderElectraFactory()
	{
		LibVersion = ANSI_TO_TCHAR(vpx_codec_version_str());
	}
	...
	int32 SupportsDecoding(TMap<FString, FVariant>& OutFormatInfo, const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions) const override
	{
		...
		if (InCodecFormat.RFC6381.StartsWith(TEXT("vp08"), ESearchCase::IgnoreCase))
		{
			return 1;
		}
		else if (InCodecFormat.RFC6381.StartsWith(TEXT("vp09"), ESearchCase::IgnoreCase))
		{
			if (vi.BitDepth > 8)
			{
	...
	{
		return InCodecFormat.Properties.IsType<Electra::FCodecTypeFormat::FVideo>() ? MakeShared<FVideoDecoderVPxElectra, ESPMode::ThreadSafe>(InCodecFormat, InAdditionalOptions) : TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe>();
	}
...
FVideoDecoderVPxElectra::FVideoDecoderVPxElectra(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
...
FVideoDecoderVPxElectra::EConvertResult FVideoDecoderVPxElectra::ConvertDecoderOutput(vpx_image_t* InDecodedImage, vpx_image_t* InDecodedAlphaImage)
{
	...
	InDecoderInput.RemoveAll([FramePTS=In->AccessUnit.PTS](const TSharedPtr<FDecoderInput, ESPMode::NotThreadSafe>& e)
	{
