...
static int32 AVFMediaForceDecodeBGRA = 1;
FAutoConsoleVariableRef CVarAVFMediaForceDecodeBGRA(
	TEXT("m.avf.ForceDecodeBGRA"),
	AVFMediaForceDecodeBGRA,
	TEXT("Selects how H.264 file playback is decoded on Apple platforms. Other codecs and live capture\n")
	TEXT("always decode to BGRA regardless of this setting.\n")
	TEXT("1: Decode directly to BGRA via AVFoundation (Default). Produces a CPU-readable buffer.\n")
	TEXT("0: Decode to NV12 and convert to BGRA in a UE shader. Saves a small amount of memory bandwidth\n")
	TEXT("   at the cost of an extra GPU pass."),
	ECVF_ReadOnly);
...
bool FAvfMediaTracks::SelectTrack(EMediaTrackType TrackType, int32 TrackIndex)
{
	...
	case EMediaTrackType::Audio:
		...
		{
			UE_LOGF(LogAvfMedia, Verbose, "Selecting audio track %i instead of %i (%i tracks).", TrackIndex, SelectedAudioTrack, AudioTracks.Num());

			// disable current track
			if (SelectedAudioTrack != INDEX_NONE)
			{
				UE_LOGF(LogAvfMedia, VeryVerbose, "Disabling audio track %i", SelectedAudioTrack);
			...
			if (TrackIndex != INDEX_NONE)
			{
				...
				UE_LOGF(LogAvfMedia, VeryVerbose, "Enabling audio track %i", TrackIndex);
			}
	...
	case EMediaTrackType::Caption:
		...
		{
			UE_LOGF(LogAvfMedia, Verbose, "Selecting caption track %i instead of %i (%i tracks).", TrackIndex, SelectedCaptionTrack, CaptionTracks.Num());

			// disable current track
			if (SelectedCaptionTrack != INDEX_NONE)
			{
				UE_LOGF(LogAvfMedia, VeryVerbose, "Disabling caption track %i", SelectedCaptionTrack);
			...
			if (TrackIndex != INDEX_NONE)
			{
				...
				UE_LOGF(LogAvfMedia, VeryVerbose, "Enabling caption track %i", TrackIndex);
	...
	case EMediaTrackType::Video:
		...
		{
			UE_LOGF(LogAvfMedia, Verbose, "Selecting video track %i instead of %i (%i tracks)", TrackIndex, SelectedVideoTrack, VideoTracks.Num());

			// disable current track
			if (SelectedVideoTrack != INDEX_NONE)
			{
				UE_LOGF(LogAvfMedia, VeryVerbose, "Disabling video track %i", SelectedVideoTrack);
			...
			if (TrackIndex != INDEX_NONE)
			{
				...
				UE_LOGF(LogAvfMedia, VeryVerbose, "Enabling video track %i", TrackIndex);
