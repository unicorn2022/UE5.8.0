...
bool FAvfMediaPlayer::Open(const FString& Url, const IMediaOptions* /*Options*/)
{
	...
	if (nsMediaUrl == nil)
	{
		UE_LOGF(LogAvfMedia, Error, "Failed to open Media file: %ls", *Url);
	...
	if (!MediaPlayer)
	{
		UE_LOGF(LogAvfMedia, Error, "Failed to create instance of an AVPlayer");
		return false;
	}
	...
	if (PlayerItem == nil)
	{
		UE_LOGF(LogAvfMedia, Error, "Failed to open player item with Url: %ls", *Url);
		return false;
	}
	...
	[[PlayerItem asset] loadValuesAsynchronouslyForKeys:@[@"tracks"] completionHandler:^
	{
		...
		else if (Error != nullptr)
		{
			...
			UE_LOGF(LogAvfMedia, Warning, "Failed to load video tracks. [%ls]", *FString(errstr));
