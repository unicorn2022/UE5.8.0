...
#include "MP4Utilities.h"				// for parsing the 'dfLa' box
...
public:
	...
	FElectraFlacDecoder(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions);
...
private:
	...
	bool Parse_dfLa(FdfLaConfig& OutConfig, const TArray<uint8>& IndfLaBox, bool bFailOnError, bool bFullBox);
...
class FElectraCommonAudioFlacDecoderFactory : public IElectraCodecFactory, public IElectraCodecFactory::IProviderInformation, public IElectraCodecModularFeature, public TSharedFromThis<FElectraCommonAudioFlacDecoderFactory, ESPMode::ThreadSafe>
{
public:
	virtual ~FElectraCommonAudioFlacDecoderFactory()
	{ }

	FElectraCommonAudioFlacDecoderFactory()
	{
		LibName = ANSI_TO_TCHAR(FLAC__VENDOR_STRING);
		LibVersion = ANSI_TO_TCHAR(FLAC__VERSION_STRING);
	}
	...
	int32 SupportsDecoding(TMap<FString, FVariant>& OutFormatInfo, const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions) const override
	{
		...
		if (!InCodecFormat.RFC6381.Equals(TEXT("flac"), ESearchCase::IgnoreCase))
		{
			return 0;
		}
	...
	{
		return InCodecFormat.Properties.IsType<Electra::FCodecTypeFormat::FAudio>() ? MakeShared<FElectraFlacDecoder, ESPMode::ThreadSafe>(InCodecFormat, InAdditionalOptions) : TSharedPtr<IElectraDecoder, ESPMode::ThreadSafe>();
	}
...
FElectraFlacDecoder::FElectraFlacDecoder(const Electra::FCodecTypeFormat& InCodecFormat, const TMap<FString, FVariant>& InAdditionalOptions)
{
	if (InCodecFormat.DCR.Num())
	{
		bHaveParseddfLa = Parse_dfLa(DfLaConfig, InCodecFormat.DCR, false, false);
	}
	if (!bHaveParseddfLa && InCodecFormat.ExtraBoxes.Contains(MP4Utilities::MakeBoxAtom('d','f','L','a')))
	{
		bHaveParseddfLa = Parse_dfLa(DfLaConfig, InCodecFormat.ExtraBoxes[MP4Utilities::MakeBoxAtom('d','f','L','a')], false, true);
	}
}
...
bool FElectraFlacDecoder::Parse_dfLa(FdfLaConfig& OutConfig, const TArray<uint8>& IndfLaBox, bool bFailOnError, bool bFullBox)
{
	if (IndfLaBox.Num() == 0)
	{
	...
	MP4Utilities::FMP4AtomReader rd(IndfLaBox);
	uint8 Value8 = 0;
	if (bFullBox)
	{
	...
	int32 LenToGo = IndfLaBox.Num() - (bFullBox ? 4 : 0);
	while(1)
	{
		if (LenToGo < 4)
		{
...
IElectraDecoder::ECSDCompatibility FElectraFlacDecoder::IsCompatibleWith(const TMap<FString, FVariant>& CSDAndAdditionalOptions)
{
	...
	TArray<uint8> DCR = ElectraDecodersUtil::GetVariantValueUInt8Array(CSDAndAdditionalOptions, TEXT("dcr"));
	TArray<uint8> CSD = ElectraDecodersUtil::GetVariantValueUInt8Array(CSDAndAdditionalOptions, TEXT("csd"));
	FdfLaConfig cfg;
	if (!Parse_dfLa(cfg, DCR.Num() ? DCR : CSD, false, false))
	{
...
IElectraDecoder::EDecoderError FElectraFlacDecoder::DecodeAccessUnit(const FInputAccessUnit& InInputAccessUnit, const TMap<FString, FVariant>& InAdditionalOptions)
{
	...
	if (InInputAccessUnit.Data && InInputAccessUnit.DataSize)
	{
		...
		if (!bHaveParseddfLa)
		{
			if (InAdditionalOptions.Contains(TEXT("dcr")))
			{
				bHaveParseddfLa = Parse_dfLa(DfLaConfig, ElectraDecodersUtil::GetVariantValueUInt8Array(InAdditionalOptions, TEXT("dcr")), true, false);
			}
			if (!bHaveParseddfLa && InAdditionalOptions.Contains(TEXT("dfLa")))
			{
				bHaveParseddfLa = Parse_dfLa(DfLaConfig, ElectraDecodersUtil::GetVariantValueUInt8Array(InAdditionalOptions, TEXT("dfLa")), true, true);
			}
			if (!bHaveParseddfLa)
			{
				return IElectraDecoder::EDecoderError::Error;
