...
TSharedRef<FExtender> FLightWeightInstancesEditorModule::CreateLevelViewportContextMenuExtender(const TSharedRef<FUICommandList> CommandList, const TArray<AActor*> InActors)
{
	...
	for (AActor* Actor : InActors)
	{
		...
		{
			UE_LOGF(LogLWIEditor, Warning, "Unable to convert actors of multiple types to light weight instances");
			return Extender;
		}
	}
	...
	if (InActors.Num() > 0)
	{
		...
		Extender->AddMenuExtension("ActorConvert", EExtensionHook::After, LevelEditorCommandBindings, FMenuExtensionDelegate::CreateLambda(
			[this, ActorName, InActors](FMenuBuilder& MenuBuilder) {
				...
				const bool bCanExecute = !InActors[0]->GetClass()->IsChildOf(ADEPRECATED_LightWeightInstanceManager::StaticClass());

				MenuBuilder.AddMenuEntry(
					FText::Format(LOCTEXT("ConvertSelectedActorsToLWIsText", "(DEPRECATED) Convert {0} To Light Weight Instances"), ActorName),
					LOCTEXT("ConvertSelectedActorsToLWIsTooltip", "(DEPRECATED) Convert the selected actors to light weight instances."),
...
void FLightWeightInstancesEditorModule::ConvertActorsToLWIsUIAction(const TArray<AActor*> InActors) const
{
	...
	{
		UE_LOGF(LogLWIEditor, Log, "Unable to convert unspecified actors to light weight instances");
		return;
	}
	...
	for (AActor* Actor : InActors)
	{
		...
		ADEPRECATED_LightWeightInstanceManager* Manager = FLightWeightInstanceSubsystem::Get().FindOrAddLightWeightInstanceManager(*Actor->GetClass(), *Actor->GetWorld(), Actor->GetActorLocation(), DataLayerInstance);
		check(Manager);
