	...
	if (GEngine)
	{
		...
		for (UObject* SelectedObject : SelectedAssets)
		{
			...
			if (IsValid(SelectedSound))
			{
				...
				{
					UE_LOGFMT(LogSubtitlesAndClosedCaptions, Warning, "Can't remove legacy subtitles: migrate them to the new system first with Convert Legacy Subtitles.");
				}
			}
			else
			{
				UE_LOGFMT(LogSubtitlesAndClosedCaptions, Warning, "Remove Legacy Subtitles can only be used on SoundWave assets.");
			}
		}
...
void USubtitleMigrationUtility::ConvertLegacySubtitles()
{
	...
	if (GEngine)
	{
		...
		for (UObject* SelectedObject : SelectedAssets)
		{
			...
			{
				UE_LOGFMT(LogSubtitlesAndClosedCaptions, Warning, "Convert Legacy Subtitles can only be used on SoundWave assets.");
			}
		}
