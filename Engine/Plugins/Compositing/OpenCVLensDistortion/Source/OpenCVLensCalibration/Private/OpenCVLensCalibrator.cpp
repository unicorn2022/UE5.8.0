...
{
#if WITH_OPENCV == 0
	UE_LOGF(LogOpenCVLensCalibration, Error, "OpenCV isn't enabled. Calibration won`t work as expected.");
#endif
...
bool UOpenCVLensCalibrator::FeedRenderTarget(UTextureRenderTarget2D* InTextureRT)
{
	...
	{
		UE_LOGF(LogOpenCVLensCalibration, Error, "Invalid render target was fed to LensCalibrator");
		return false;
	}
	...
	default:
	{
		UE_LOGF(LogOpenCVLensCalibration, Warning, "Invalid pixel format in render target %ls", *InTextureRT->GetName());
		break;
	}
	}
...
bool UOpenCVLensCalibrator::CalculateLensParameters(FOpenCVLensDistortionParameters& OutLensDistortionParameters, float& OutMarginOfError, FOpenCVCameraViewInfo& OutCameraViewInfo)
{
	...
	OutMarginOfError = FLT_MAX;
	{
		...
		if (bUseFisheyeModel)
		{
			...
			{
				UE_LOGF(LogOpenCVLensCalibration, Warning, "Fisheye calibration requires at least 2 samples.");
				return false;
			}
		}
		else
