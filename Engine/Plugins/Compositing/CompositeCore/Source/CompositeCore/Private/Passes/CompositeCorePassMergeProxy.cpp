...
class FCompositeCoreMergeShader : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, bBlendInDisplaySpace)
		SHADER_PARAMETER(uint32, bLuminanceOnly)
...
namespace UE
{
	namespace CompositeCore
	{
		...
		FMergePassProxy::FMergePassProxy(FPassInputDeclArray InPassDeclaredInputs, ECompositeCoreMergeOp InMergeOp, const TCHAR* InParentName, ELensDistortionHandling bInUseLensDistortion, bool bInUltimatteBlend)
			: FCompositeCorePassProxy(MoveTemp(InPassDeclaredInputs))
			, MergeOp(InMergeOp)
			, ParentName(InParentName)
			, bUseLensDistortion(bInUseLensDistortion)
			, bUltimatteBlend(bInUltimatteBlend)
		{
		}
		...
		FPassTexture FMergePassProxy::Add(FRDGBuilder& GraphBuilder, const FSceneView& InView, const FPassInputArray& Inputs, const FPassContext& PassContext) const
		{
			...
			UE_LOGF(LogCompositeCore, VeryVerbose, "CompositeCore.Merge: %ls", ParentName);
			...
			if (!Output.IsValid())
			{
				...
				DynamicDebugName = FString::Printf(TEXT("%sOutput"), ParentName);

				Output = CreateOutputRenderTarget(GraphBuilder, InView, PassContext.OutputViewRect, Desc, *DynamicDebugName);
			}
			...
			PassParameters->bBlendInDisplaySpace = static_cast<uint32>(bUltimatteBlend);
			PassParameters->bLuminanceOnly = static_cast<uint32>(bLuminanceOnly);
