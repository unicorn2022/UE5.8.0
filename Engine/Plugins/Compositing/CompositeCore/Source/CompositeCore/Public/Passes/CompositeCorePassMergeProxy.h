...
namespace UE
{
	namespace CompositeCore
	{
		...
		public:
			...
			/** Constructor */
			UE_API FMergePassProxy(FPassInputDeclArray InPassDeclaredInputs, ECompositeCoreMergeOp InMergeOp, const TCHAR* InParentName, ELensDistortionHandling bInUseLensDistortion = ELensDistortionHandling::Enabled, bool bInUltimatteBlend = false);
		...
		public:
			...
			const TCHAR* ParentName = TEXT("MergePass");
			...
			/** When enabled, both inputs are encoded to sRGB before blending and decoded back to linear after. Matches Ultimatte Key & Fill workflows where content is in display space. */
			bool bUltimatteBlend = false;

			/**
			 * Divide-only: when true, the matte's RGB uses a single luminance ratio applied uniformly to all channels
			 * instead of independent per-channel ratios.
			 */
