...
bool FCompositeRenderTargetPool::ConditionalAcquireTarget(const UObject* InAssignee, TObjectPtr<UTextureRenderTarget2D>& InOutTarget, const FIntPoint& InSize, ETextureRenderTargetFormat InFormat)
{
	...
	{
		UE_LOGFMT(LogComposite, Verbose, "Requested render target size of {0}x{1} is invalid.", InSize.X, InSize.Y);
		return false;
	}
...
void FCompositeRenderTargetPool::ReleaseTarget(TObjectPtr<UTextureRenderTarget2D>& InTarget)
{
	...
	{
		UE_LOGF(LogComposite, Warning, "Attempting to release a render target that doesn't belong to this pool.");
	}
}
...
{
	UE_LOGF(LogComposite, VeryVerbose, "Empty pool.");
...
void FCompositeRenderTargetPool::Empty()
{
	...
	UE_LOGF(LogComposite, VeryVerbose, "Empty assigned render target.");
...
void FCompositeRenderTargetPool::Tick(float /*DeltaSeconds*/)
{
	...
	for (auto TargetIt = AssignedTargets.CreateIterator(); TargetIt; ++TargetIt)
	{
		...
		if (!Assignee.IsValid())
		{
			...
			UE_LOGF(LogComposite, Display, "Releasing assigned render target with invalid assignee object - possible leak?");
		}
	}
}
