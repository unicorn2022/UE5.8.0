...
void UOpenColorIOConfiguration::ReloadExistingColorspaces(bool bForce)
{
	...
	if (Config && Config->IsValid())
	{
		...
		if (ConfigHash != NewConfigHash || bForce)
		{
			...
			for (const FOpenColorIOColorSpace& ExistingColorSpace : ColorSpacesToBeReloaded)
			{
				...
				if (ColorSpaceIndex < 0)
				{
					...
					UE_LOGF(LogOpenColorIO, Display, "Removing %ls, not found.", *ExistingColorSpace.ToString());
					continue;
				}
			...
			for (const FOpenColorIODisplayView& ExistingDisplayView : DisplayViewsToBeReloaded)
			{
				...
				{
					UE_LOGF(LogOpenColorIO, Display, "Removing %ls, not found.", *ExistingDisplayView.ToString());
				}
			}
		...
		{
			UE_LOGF(LogOpenColorIO, Verbose, "Reload skipped, no differences identified.");
		}
	}
	else
	{
	...
	if (!MarkPackageDirty())
	{
		UE_LOGF(LogOpenColorIO, Display, "%ls should be resaved for faster loads. Changes in the config or the library version were detected.", *GetPathName());
	}
#endif
}
...
void UOpenColorIOConfiguration::CreateColorTransform(const FString& InSourceColorSpace, const FString& InDestinationColorSpace)
{
	...
	{
		UE_LOGF(LogOpenColorIO, Log, "OCIOConfig already contains %ls to %ls transform.", *InSourceColorSpace, *InDestinationColorSpace);
		return;
	}
	...
	{
		UE_LOGF(LogOpenColorIO, Warning, "Could not create color space transform from %ls to %ls. Verify your OCIO config file, it may have errors in it.", *InSourceColorSpace, *InDestinationColorSpace);
	}
}
...
void UOpenColorIOConfiguration::CreateColorTransform(const FString& InSourceColorSpace, const FString& InDisplay, const FString& InView, EOpenColorIOViewTransformDirection InDirection)
{
	...
	{
		UE_LOGF(LogOpenColorIO, Log, "OCIOConfig already contains %ls to %ls-%ls transform.", *InSourceColorSpace, *InDisplay, *InView);
		return;
	}
	...
	{
		UE_LOGF(LogOpenColorIO, Warning, "Could not create color space transform from %ls to %ls - %ls. Verify your OCIO config file, it may have errors in it.", *InSourceColorSpace, *InDisplay, *InView);
	}
}
...
void UOpenColorIOConfiguration::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
#if WITH_EDITOR
	if (GetLinkerCustomVersion(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::OpenColorIOv251)
	{
		// Since ACES 2.0 transforms are significant breaking change, we ease the transition by keeping old configs at the previous version.
		// TODO: Refactor configuration assets so that the external config is no longer "live" and instead only used on import.
		if (ConfigurationFile.FilePath == TEXT("ocio://default") || ConfigurationFile.FilePath == TEXT("ocio://cg-config-latest"))
		{
			ConfigurationFile.FilePath = TEXT("ocio://cg-config-v2.2.0_aces-v1.3_ocio-v2.4");
		}
		else if (ConfigurationFile.FilePath == TEXT("ocio://studio-config-latest"))
		{
			ConfigurationFile.FilePath = TEXT("ocio://studio-config-v2.2.0_aces-v1.3_ocio-v2.4");
		}
	}
...
void UOpenColorIOConfiguration::LoadConfiguration()
{
	...
	if (Config->IsValid())
	{
		...
		UE_LOGF(LogOpenColorIO, Verbose, "Loaded OCIO configuration file %ls", *FilePath);
	}
	else
	{
		UE_LOGF(LogOpenColorIO, Error, "Could not load OCIO configuration file %ls. Verify that the path is good or that the file is valid.", *FilePath);
	}
#endif //WITH_OCIO
}
...
bool UOpenColorIOConfiguration::GetHash(FString& OutHash) const
{
	...
	if (Config && Config->IsValid())
	{
		...
		if (CacheID.IsEmpty())
		{
			UE_LOGF(LogOpenColorIO, Warning, "Failed to get cache ID: forcing constant transform recreation. Please fix invalid config.");
			return false;
		}
...
void UOpenColorIOConfiguration::SetConfigurationFile(const FFilePath& InFilePath)
{
	ConfigurationFile = InFilePath;

	const FString EngineFullPath = FPaths::ConvertRelativePathToFull(FPaths::EngineDir());

	if (!EngineFullPath.IsEmpty() && ConfigurationFile.FilePath.Contains(EngineFullPath))
	{
		// Automatically replace the engine path with its {Engine} token for better compatilibilty across different machines.
		ConfigurationFile.FilePath = ConfigurationFile.FilePath.Replace(*EngineFullPath, TEXT("{Engine}/"));
	}
