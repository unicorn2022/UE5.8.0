...
void UOpenColorIOColorTransform::CacheShadersForResources(EShaderPlatform InShaderPlatform, FOpenColorIOTransformResource* InResourceToCache, bool bApplyCompletedShaderMapForRendering, bool bIsSynchronous, const ITargetPlatform* TargetPlatform)
{
	...
	if (!bSuccess)
	{
		...
		{
			UE_LOGF(LogOpenColorIO, Warning, "	%ls", *CompileErrors[ErrorIndex]);
		}
	}
}
...
bool UOpenColorIOColorTransform::GetTransformProcessor(FOpenColorIOWrapperProcessor& OutProcessor, const TMap<FString, FString>& InContextOverride) const
{
	...
	if (!ConfigWrapper)
	{
		UE_LOGF(LogOpenColorIO, Error, "Failed to create processor for color transform %ls. Configuration file [%ls] was invalid.", *GetTransformFriendlyName(), ConfigurationOwner ? *ConfigurationOwner->GetConfigurationFile().FilePath : TEXT("Missing"));
		return false;
	}
...
void UOpenColorIOColorTransform::PostLoad()
{
	...
	if (ConfigurationOwner)
	{
		...
		{
			UE_LOGF(LogOpenColorIO, Verbose, "ConfigurationOwner does not contain [%ls], aborting transform load.", *GetTransformFriendlyName());
		}
	}
	else
	{
		UE_LOGF(LogOpenColorIO, Warning, "Outer is not an UOpenColorIOConfiguration. Outer class: %ls, Outer name: %ls. ", *GetOuter()->GetClass()->GetName(), *GetOuter()->GetName());
	}
...
bool UOpenColorIOColorTransform::UpdateShaderInfo(FString& OutShaderCodeHash, FString& OutShaderCode, FString& OutRawConfigHash)
{
	...
	{
		UE_LOGF(LogOpenColorIO, Error, "Failed to fetch shader info for color transform %ls. Configuration file [%ls] was invalid.", *GetTransformFriendlyName(), ConfigurationOwner ? *ConfigurationOwner->GetConfigurationFile().FilePath : TEXT("Missing"));
	}
