...
void FOpenColorIOShaderCompilationManager::ProcessAsyncResults()
{
	...
	for (int32 JobIndex = JobQueue.Num() - 1; JobIndex >= 0; JobIndex--)
	{
		...
		{
			UE_LOGF(LogShaders, Verbose, "GPU shader compile succeeded. Id %d", CurrentJob->Id);
		}
		else
		{
			UE_LOGF(LogShaders, Warning, "GPU shader compile failed! Id: %d Name: %ls", CurrentJob->Id, *CurrentJob->Input.DebugGroupName);
		}
		...
		if (ShaderMap && ColorTransforms)
		{
			...
			for (int32 JobIndex = 0; JobIndex < ResultArray.Num(); JobIndex++)
			{
				...
				if (GShowOpenColorIOShaderWarnings || !CurrentJob->bSucceeded)
				{
					...
					{
						UE_LOGF(LogShaders, Log, "There were errors for job \"%ls\"", *CurrentJob->Input.DebugGroupName)
						for (const FShaderCompilerError& Error : CurrentJob->Output.Errors)
						{
							UE_LOGF(LogShaders, Log, "Error: %ls", *Error.GetErrorString())
						}
					}
				}
				else
				{
					UE_LOGF(LogShaders, Log, "There were NO errors for job \"%ls\"", *CurrentJob->Input.DebugGroupName);
				}
			}
				...
				for (FOpenColorIOTransformResource* ColorTransform : ColorTransformArray)
				{
					...
					if (ColorTransform->IsSame(CompletedShaderMap->GetShaderMapId()))
					{
						if (Errors.Num() != 0)
						{
							...
							UE_LOGF(LogOpenColorIOShaderCompiler, Log, "Compile output as text:");
							UE_LOGF(LogOpenColorIOShaderCompiler, Log, "===================================================================================");
							TArray<FString> OutputByLines;
							SourceCode.ParseIntoArrayLines(OutputByLines, false);
							for (int32 i = 0; i < OutputByLines.Num(); i++)
							{
								UE_LOGF(LogOpenColorIOShaderCompiler, Log, "/*%04d*/\t\t%ls", i + 1, *OutputByLines[i]);
							}
							UE_LOGF(LogOpenColorIOShaderCompiler, Log, "===================================================================================");
						}
						...
						if (!bSuccess)
						{
							...
							for (int32 ErrorIndex = 0; ErrorIndex < Errors.Num(); ErrorIndex++)
							{
								...
								UE_LOGF(LogOpenColorIOShaderCompiler, Warning, "	%ls", *ErrorMessage);
							}
						}
						else
						{
							...
							{
								UE_LOGF(LogOpenColorIOShaderCompiler, Warning, "Warnings while compiling OpenColorIO ColorTransform %ls for platform %ls:",
									*ColorTransform->GetFriendlyName(),
									*LegacyShaderPlatformToShaderFormat(ShaderMap->GetShaderPlatform()).ToString());
								for (int32 ErrorIndex = 0; ErrorIndex < Errors.Num(); ErrorIndex++)
								{
									UE_LOGF(LogOpenColorIOShaderCompiler, Warning, "	%ls", *Errors[ErrorIndex]);
								}
							}
						}
					}
