...
OPENCOLORIO_API void FOpenColorIOTransformResource::RemoveOutstandingCompileId(const int32 InOldOutstandingCompileShaderMapId)
{
	...
	{
		UE_LOGF(LogShaders, Log, "RemoveOutstandingCompileId %p %d", this, InOldOutstandingCompileShaderMapId);
	}
}
...
OPENCOLORIO_API void FOpenColorIOTransformResource::GetShaderMapId(EShaderPlatform InPlatform, const ITargetPlatform* TargetPlatform, FOpenColorIOShaderMapId& OutId) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogShaders, Error, "FOpenColorIOTransformResource::GetShaderMapId: TargetPlatform is not null, but a cooked executable cannot target platforms other than its own.");
		}
...
void FOpenColorIOTransformResource::SerializeShaderMap(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogShaders, Fatal, "This platform requires cooked packages, and shaders were not cooked into this OCIO transform %ls.", *GetFriendlyName());
	}
	...
	if (bCooked)
	{
		if (Ar.IsCooking())
		{
			...
			//{
			//	FString Name;
			//	UE_LOGF(LogShaders, Error, "Failed to compile OpenColorIO shader %ls.", *GetFriendlyName());
			//}
#endif
		}
		else
...
void FOpenColorIOTransformResource::SetupResource(ERHIFeatureLevel::Type InFeatureLevel, const FString& InShaderCodeHash, const FString& InShadercode, const FString& InRawConfigHash, const FString& InFriendlyName, const FName& InAssetPath, FShaderParametersMetadata* InShaderParameterMetadata)
{
	...
	{
		UE_LOGF(LogOpenColorIO, Error, "OpenColorIO shader transform resource is expecting a valid shader parameter metadata for compilation.");
	}
...
bool FOpenColorIOTransformResource::CacheShaders(const FOpenColorIOShaderMapId& InShaderMapId, EShaderPlatform InPlatform, bool bApplyCompletedShaderMapForRendering, bool bSynchronous)
{
	bool bSucceeded = false;
	{
		...
		else
		{
			...
			if ((!GameThreadShaderMap || !GameThreadShaderMap->IsComplete(this, true)) && !FPlatformProperties::RequiresCookedData())
			{
				...
				{
					UE_LOGF(LogOpenColorIO, Display, "Loaded shader %ls for transform %ls from DDC", *GameThreadShaderMap->GetFriendlyName(), *GetFriendlyName());
				}
				else
				{
					UE_LOGF(LogOpenColorIO, Display, "Shader for transform %ls was not loaded from the DDC, needs recompile.", *GetFriendlyName());
				}
#endif // WITH_EDITOR
			}
		}
	...
	if (!GameThreadShaderMap || !(bAssumeShaderMapIsComplete || GameThreadShaderMap->IsComplete(this, false)))
	{
		...
		{
			UE_LOGF(LogShaders, Log, "%ls cached shader map for color transform %ls, compiling.", GameThreadShaderMap? TEXT("Incomplete") : TEXT("Missing"), *GetFriendlyName());
