...
void UCompositingMediaCaptureOutput::RelayOutput_Implementation(UTexture* RenderResult, UComposurePostProcessingPassProxy* PostProcessProxy)
{
	...
	else if (CaptureOutput)
	{
		...
		if (FCompositingElementPassUtils::GetTargetFormatFromPixelFormat(CaptureOutput->GetRequestedPixelFormat(), OutputFormat))
		{
			...
			{
				UE_LOGF(Composure, Warning, "Desired media capture size is too big - unable to convert the render result. Invalid Media Output?");
			}
		}
		else
		{
			UE_LOGF(Composure, Warning, "Requested media capture format is unsupported - unable to convert the render result. Invalid Media Output?");
		}
	}
