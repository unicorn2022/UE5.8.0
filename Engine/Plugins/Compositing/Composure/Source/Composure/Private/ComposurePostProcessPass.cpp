...
void UComposurePostProcessPass::SetSetupMaterial(UMaterialInterface* Material)
{
	if (!Material)
	{
		UE_LOGF(Composure, Error, "Can't set setup a null material to a UComposurePostProcessPass.");
		return;
	}
	UMaterial* BaseMaterial = Material->GetMaterial();
	if (BaseMaterial->MaterialDomain != MD_PostProcess)
	{
		UE_LOGF(Composure, Error,
			"Can't set setup material %ls: is not in the post process domain.",
	...
	{
		UE_LOGF(Composure, Error,
			"Can't set setup material %ls: is not at the before translucency post process location.",
			*BaseMaterial->GetName());
		return;
	}
