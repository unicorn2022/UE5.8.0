...
UTextureRenderTarget2D* ACompositingElement::RequestNamedRenderTarget(const FName ReferenceName, const float RenderPercentage, ETargetUsageFlags UsageTag)
{
	...
	{
		UE_LOGF(Composure, Warning, "Requesting a render target for usage that is currently freeze-framed - everything should be static while 'frozen'.");
	}
	...
	if (bPreExists && PreExistingTarget)
	{
		...
		{
			UE_LOGF(Composure, Warning, "Requesting a new render target using the name of one that is already in use - and persistent! Returning that to you instead.");
...
bool ACompositingElement::ReleaseOwnedTarget(UTextureRenderTarget2D* OwnedTarget)
{
	if (OwnedTarget && RenderTargetPool.IsValid())
	{
		...
		{
			UE_LOGF(Composure, Error, "Blocked an attempt to release a render target that is currently freeze-framed - everything should be static while 'frozen'.");
		}
	}
	return false;
}
...
void ACompositingElement::RegisterTaggedPassResult(FName ReferenceName, UTexture* PassResult, ETargetUsageFlags UsageFlags)
{
	if (!ReferenceName.IsNone())
	{
		...
		if (!!(PassResultsTable.FindUsageTags(ReferenceName) & (int32)ETargetUsageFlags::USAGE_Persistent))
		{
			...
			UE_CLOGF(PreExistingResult != PassResult, Composure, Error, "Attempting to register a pass result over a pre-existing persistent one. Blocking this action (release the persistent target first).");
		}
		else
		{
