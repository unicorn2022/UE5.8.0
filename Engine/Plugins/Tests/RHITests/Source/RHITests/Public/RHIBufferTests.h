...
private:
	...
	)
	{
		...
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test skipped. \"%ls\". Unsupported format.", *TestName);
			return true;
		}
        ...
        if (!bStructured)
        {
			checkf(uint32(RHIGetMinimumAlignmentForBufferBackedSRV(Format)) < BufferSize, TEXT("Minimum aligned buffer size is %lld, buffer size is %d, PixelFormat: %d"), RHIGetMinimumAlignmentForBufferBackedSRV(Format), BufferSize, (uint32_t)Format);
