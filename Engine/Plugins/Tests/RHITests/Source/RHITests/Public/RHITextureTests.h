...
public:
	...
	static bool Test_RHIFormat_WithParams(FRHICommandListImmediate& RHICmdList, EPixelFormat ResourceFormat, EPixelFormat SRVFormat, EPixelFormat UAVFormat, ETextureCreateFlags Flags)
	{
		...
		bool bResult = true;
		FString TestName = FString::Printf(TEXT("Test_RHIFormat (%s, %s, %s, %lld)"), GPixelFormats[ResourceFormat].Name, GPixelFormats[SRVFormat].Name, GPixelFormats[UAVFormat].Name, EnumToUnderlyingType(Flags));
		{
	...
	static bool Test_RHIFormat_RenderTargetFormat(FRHICommandListImmediate& RHICmdList, EPixelFormat Format, bool bAllowUAV)
	{
		...
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Skipping test for lack of format support. \"Test_RHIFormat_RenderTargetFormat (%ls)\"", GPixelFormats[Format].Name);
		}
		return bResult;
	}
	...
	static bool Test_RHIFormat_DepthFormat(FRHICommandListImmediate& RHICmdList, EPixelFormat ResourceFormat)
	{
		...
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Skipping test for lack of format support. \"Test_RHIFormat_DepthFormat (%ls)\"", GPixelFormats[ResourceFormat].Name);
		}
		return bResult;
	}
	...
	static bool Test_UpdateTexture_Impl(FRHICommandListImmediate& RHICmdList, EPixelFormat Format, ETextureUpdateType UpdateType)
	{
		...
		FString TestName = FString::Printf(TEXT("Test_UpdateTexture (%s) (%s)"), GPixelFormats[Format].Name, GetName(UpdateType));
		{
			...
			{
				UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test skipped (format not supported). \"%ls\"", *TestName);
				return true;
			}
			FRHITextureCreateDesc Desc;
			...
			if (UpdateType == ETextureUpdateType::Texture3D)
			{
				...
				{
					UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test skipped (format not supported as Texture3D). \"%ls\"", *TestName);
					return true;
				}
			...
			else
			{
				...
				{
					UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test skipped (format not supported as Texture2D). \"%ls\"", *TestName);
					return true;
				}
			...
			if (Texture == nullptr)
			{
				UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test failed (couldn't create texture). \"%ls\"", *TestName);
				return false;
			}
...
#define RUN_TEST_ON_FORMAT(Format) RunTestOnFormat(Format);
		FOREACH_ENUM_EPIXELFORMAT_CORE(RUN_TEST_ON_FORMAT);
#undef RUN_TEST_ON_FORMAT
	...
	static bool Test_RHICopyTexture(FRHICommandListImmediate& RHICmdList)
	{
		...
		const auto CheckTestData = [&](const void* Ptr, int32 Stride, FIntPoint CheckSize, FIntPoint DataOffset, FIntPoint SrcSize, FIntPoint SrcOffset, int32 ImageIndex) -> bool
		{
			...
			for (int32 Y = DataOffset.Y; Y < DataOffset.Y + CheckSize.Y; Y++)
			{
				...
				for (int32 X = DataOffset.X; X < DataOffset.X + CheckSize.X; X++, Index++)
				{
					if (Row[X] != Index)
					{
						UE_LOGF(LogRHIUnitTestCommandlet, Error,
							"Dumping Test Data:\n" "\tCoordinates: [%d, %d]\n" "\tExpected Value: %d\n" "\tActual Value: %d\n" "%ls",
...

		{
			// Copy to identical texture
			{
				...
				FTextureRHIRef SourceTexture = RHICmdList.CreateTexture(SourceDesc);
				...
				FTextureRHIRef DestTexture = RHICmdList.CreateTexture(DestDesc);
			...
			// Copy source subrect to destination readback buffer
			{
				...
				{
					UE_LOGF(LogRHIUnitTestCommandlet, Error, "Test failed. \"%ls\"", TestName);
				}
				Readback.Unlock();
			...
			// Read surface data
			{
				...
				const FRHITextureCreateDesc SourceDesc =
					FRHITextureCreateDesc::Create2D(TestName, Extent, PF_A2B10G10R10)
					// Force as non-static
					.AddFlags(ETextureCreateFlags::RenderTargetable | ETextureCreateFlags::NoFastClear);
				...
				const auto ValidateRHIReadSurfaceData = [&]()
				{
					...
					{
						UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test failed. \"%ls\"", TestName);
						UE_LOGF(LogRHIUnitTestCommandlet, Display, "%ls results:\n%ls", TestName, *SampleStr);
						for (const auto& InvalidSample : InvalidSamples)
						{
							UE_LOGF(LogRHIUnitTestCommandlet, Display, "[%2d, %2d]: [%d, %d, %d, %d] != [%d, %d, %d, %d]",
	...
	static bool Test_MultipleLockTexture2D(FRHICommandListImmediate& RHICmdList)
	{
		...
		FTextureRHIRef Texture_SingleLock = RHICmdList.CreateTexture(DescArray);
		FTextureRHIRef Texture_MultipleLock = RHICmdList.CreateTexture(DescArray);
		...
		FTextureRHIRef SingleLockStagingTexture = RHICmdList.CreateTexture(Desc);
		FTextureRHIRef MultiLockStagingTexture = RHICmdList.CreateTexture(Desc);
