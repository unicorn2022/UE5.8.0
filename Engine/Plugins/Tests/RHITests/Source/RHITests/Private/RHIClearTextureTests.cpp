...
static void Test_ClearTextureBeginCapture(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		RHICmdList.EndDrawingViewport(nullptr, FRHIPresentArgs(/*.FrameCounter*/ GFrameCounterRenderThread, /*.bPresent*/ true, /*.bLockToVsync*/ false));
...
struct FStagingTexturePool
{
	...
	void PreallocateStagingTexture(FRHICommandListBase& RHICmdList, EPixelFormat PixelFormat, uint32 NumTextures);
...
FTextureRHIRef CreateTexture2DAtlasRT(FRHICommandListBase& RHICmdList, EPixelFormat PixelFormat, FUnorderedAccessViewRHIRef* OutUAV)
{
	FTextureRHIRef Texture2DAtlasRT = RHICmdList.CreateTexture(CreateTexture2DAtlasDesc(PixelFormat, ERHIAccess::UAVCompute, TexCreate_UAV | TexCreate_ShaderResource, TEXT("Texture2DAtlasRT")));
...
FTextureRHIRef CreateTexture2DAtlasStaging(FRHICommandListBase& RHICmdList, EPixelFormat PixelFormat)
{
	return RHICmdList.CreateTexture(CreateTexture2DAtlasDesc(PixelFormat, ERHIAccess::CPURead, TexCreate_CPUReadback, TEXT("Texture2DAtlasStaging")));
}

void FStagingTexturePool::PreallocateStagingTexture(FRHICommandListBase& RHICmdList, EPixelFormat PixelFormat, uint32 NumTextures)
{
	...
	{
		FTextureRHIRef Texture = CreateTexture2DAtlasStaging(RHICmdList, PixelFormat);
...
static FString GetTextureName(const FRHITextureDesc& Desc)
{
	...
	return FString::Printf(TEXT("(%s %dx%d%s %d mips, 0x%llX) %s %s, %s"), GetTextureDimensionString(Desc.Dimension), Desc.Extent.X, Desc.Extent.Y, *TextureSuffix, Desc.NumMips, EnumToUnderlyingType(Desc.Flags), GPixelFormats[Desc.Format].Name,
...
struct FTestContext
{
	...
	void SetClearTestResult(uint32 GlobalTextureIndex, int32 NumClearSuccess, const FTestStageFailure& TestStageFailure)
	{
		...
		if (NumClearSuccess > 0)
		{
			...
			{
 				UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. Test_ClearTexture \"%ls (%d clears)\"",
...
{
	FTextureRHIRef SourceTexture = RHICmdList.CreateTexture(RHITextureCreateDesc);
	...
	FTextureRHIRef DestTextureStaging = CreateTexture2DAtlasStaging(RHICmdList, SourceTexture->GetDesc().Format);
...
// 	const TArray<int32> ExtentsArray = { 16 };
	...
	{
		UE_LOGF(LogRHIUnitTestCommandlet, Error, "Test failed. \"Test_ClearTexture\" 2D atlasing failed, aborting");
		return false;
	}
	...
	bool bResult = true;
	{
		...
		for (int32 TextureIndex = 0; TextureIndex < RHITextureCreateDescs.Num(); ++TextureIndex)
		{
			...
			{
				CLEAR_TEST_SCOPED_NAMED_EVENT_TEXT(TEXT("CreateTexture"), FColor::Magenta);
				Texture = RHICmdList.CreateTexture(RHITextureCreateDescs[TextureIndex]);
		...
		for (int32 TextureIndex = 0; TextureIndex < TestContext.RHITextureCreateDescs.Num(); ++TextureIndex)
		{
			...
			if (TextureNumClears != TextureNumSuccess)
			{
				...
				UE_LOGF(LogRHIUnitTestCommandlet, Error, "Test failed. \"Test_ClearTexture for %ls. %ls (%d/%d clear tests)\"", 
		...
		if (bResult)
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. \"Test_ClearTexture (%d/%d clear tests)\"", NumClearsSuccess, NumClearsDone);
		}
		else
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Error, "Test failed. \"Test_ClearTexture (%d/%d clear tests)\"", NumClearsSuccess, NumClearsDone);
		}
