...
bool FRHIReadbackTests::Test_BufferReadback(FRHICommandListImmediate& RHICmdList)
{
...

	{
		...
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. \"Typed Buffer Readback\"");
		}
...

	{
		...
		for (uint32 Index = 0; Index < NumElements; ++Index)
		{
			...
			{
				UE_LOGF(LogRHIUnitTestCommandlet, Error, "Test failed. \"Byte Address Buffer Readback\"\n\nIndex: %u; Expected Data: %u; Actual Data: %u\n\n", Index, ExpectedVal, ByteAddressBufferData[Index]);
		...
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. \"Byte Address Buffer Readback\"");
		}
...

	{
		...
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. \"Structured Buffer Readback\"");
		}
...
bool FRHIReadbackTests::Test_TextureReadback(FRHICommandListImmediate& RHICmdList)
{
	...
	//
	{
		...
		Texture2D = RHICmdList.CreateTexture(Texture2DDesc);
...

	{
		...
		Texture3D = RHICmdList.CreateTexture(Texture3DDesc);
...

	{
		...
		TextureArray = RHICmdList.CreateTexture(TextureArrayDesc);
...

	{
		...
		for (int32 Y = 0; Y < ReadbackSize2D.Y && bTexture2DResult; ++Y)
		{
			for (int32 X = 0; X < ReadbackSize2D.X; ++X)
			{
				...
				{
					UE_LOGF(LogRHIUnitTestCommandlet, Error, "Test failed. \"Texture 2D Readback\"\n\nCoords: (%u, %u); Expected Data: %u; Actual Data: %u\n\n", X, Y, ExpectedVal, RowStart[X]);
		...
		if (bTexture2DResult)
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. \"Texture 2D Readback\"");
		}
...

	{
		...
		for (int32 Z = 0; Z < ReadbackSize3D.Z && bTexture3DResult; ++Z)
		{
			...
			for (int32 Y = 0; Y < ReadbackSize3D.Y && bTexture3DResult; ++Y)
			{
				for (int32 X = 0; X < ReadbackSize3D.X; ++X)
				{
					...
					if (RowStart[X] != ExpectedVal)
					{
						UE_LOGF(LogRHIUnitTestCommandlet, Error, "Test failed. \"Texture 3D Readback\"\n\nCoords: (%u, %u, %u); Expected Data: %u; Actual Data: %u\n\n", X, Y, Z, ExpectedVal, RowStart[X]);
		...
		if (bTexture3DResult)
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. \"Texture 3D Readback\"");
		}
...

	{
		...
		{
			UE_LOGF(LogRHIUnitTestCommandlet, Display, "Test passed. \"Texture Array Readback\"");
		}
