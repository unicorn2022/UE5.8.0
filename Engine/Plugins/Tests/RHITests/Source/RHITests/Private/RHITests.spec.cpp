...
void FAutomationRHITest::Define()
{
	...
	Describe("Test RHI Graphics UAV Binding", [this]()
	{
		...
		It("Vertex shader UAV", [this]()
		{
			...
			TestEqual("Vertex shader UAV failed", bResult, 1);
		});
	});
	...
	Describe("Test RHI Bindless", [this]()
	{
		...
		It("RHI Descriptor Range SRV", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIBindlessTests::Test_DescriptorRange_SRV);
			TestEqual("Descriptor Range SRV failed", bResult, 1);
		});
		It("RHI Descriptor Range Update With Offset", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIBindlessTests::Test_DescriptorRange_UpdateWithOffset);
			TestEqual("Descriptor Range Update With Offset failed", bResult, 1);
		});
		It("RHI Descriptor Range UAV", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIBindlessTests::Test_DescriptorRange_UAV);
			TestEqual("Descriptor Range UAV failed", bResult, 1);
		});
		It("RHI Descriptor Range SRV And UAV", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIBindlessTests::Test_DescriptorRange_SRV_And_UAV);
			TestEqual("Descriptor Range SRV And UAV failed", bResult, 1);
		});
		It("RHI Descriptor Range Mixed Resource Types", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIBindlessTests::Test_DescriptorRange_MixedResourceTypes);
			TestEqual("Descriptor Range Mixed Resource Types failed", bResult, 1);
		});
		It("RHI Bindless Accessible Uniform Buffer", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIBindlessTests::Test_BindlessAccessibleUniformBuffer);
			TestEqual("Bindless Accessible Uniform Buffer failed", bResult, 1);
		});

	});

	Describe("Root Constants", [this]
	{
		It("RHI Graphics Shader Root Constants", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIRootConstantsTests::Test_GraphicsShaderRootConstants);
			TestEqual("Graphics Shader Root Constants failed", bResult, 1);
		});

		It("RHI Compute Shader Root Constants", [this]()
		{
			bool bResult = RunOnRenderThreadSynchronous(RHIRootConstantsTests::Test_ComputeShaderRootConstants);
			TestEqual("Compute Shader Root Constants failed", bResult, 1);
		});
	});
}
