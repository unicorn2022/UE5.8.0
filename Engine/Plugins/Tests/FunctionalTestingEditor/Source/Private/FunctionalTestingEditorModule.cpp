...
void OpenMapAndFocusActor(const TArray<FString>& Args)
{
	if ( Args.Num() != 2 )
	{
		UE_LOGF(LogConsoleResponse, Display, "Automate.OpenMapAndFocusActor failed, the number of arguments is wrong.  Automate.OpenMapAndFocusActor MapObjectPath ActorName\n");
		return;
	}
