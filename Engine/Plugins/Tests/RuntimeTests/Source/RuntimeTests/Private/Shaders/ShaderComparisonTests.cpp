...
bool FViewportScreenshotCommand::Update()
{
    ...
    UE_LOGF(ShaderComparisonTests, Log, "Taking Screenshot %ls.", *ScreenshotFileName);
    return true;
}
...
bool FSetCamera::Update()
{
    ...
    if (!PlayerController)
    {
        UE_LOGF(ShaderComparisonTests, Error, "Unable to get PlayerController");
        return true;
    }
    PlayerController->SetViewTarget(Camera, FViewTargetTransitionParams());
    UE_LOGF(ShaderComparisonTests, Log, "Modifying camera.");
    return true;
}
...
#define NUM_CAMERAS 15
    ...
    AddCommand(new FDelayedFunctionLatentCommand([=] {
        ...
        for (int i = 0; i < ShaderNames.Num(); ++i) {
            ...
            UE_LOGF(ShaderComparisonTests, Log, "Modifying shader %ls.", *ShaderNames[i]);
        }
    }));
    ...
    AddCommand(new FDelayedFunctionLatentCommand([=] {
        ...
        for (int i = 0; i < NUM_CAMERAS; ++i)
        {
            ...
            PathName = "../../../" + PathName;
            UE_LOGF(ShaderComparisonTests, Log, "Screenshots are at %ls and %ls.", *RealScreenshotFileName[i], *RealFP16ScreenshotFileName[i]);
            UE_LOGF(ShaderComparisonTests, Log, "Difference is stored in %ls", *PathName);
#if WITH_EDITOR
            ...
            UE_LOGF(ShaderComparisonTests, Log, "Similarity is %f after %d.", similarity, i);
#endif 
        }
    
        UE_LOGF(ShaderComparisonTests, Log, "FP16 ran in %f, with FP32 in %f.", (*FP16AverageTimes)[2], (*AverageTimes)[2]);
        if ((*FP16AverageTimes)[2] > (*AverageTimes)[2] + 0.2)
        {
            UE_LOGF(ShaderComparisonTests, Error, "FP16 is slower than FP32!");
        }
        float PerfDelta = ((*AverageTimes)[2] - (*FP16AverageTimes)[2])/10 + 1;
        float ImageDelta = (1 - similarity) / 2.f;
        UE_LOGF(ShaderComparisonTests, Log, "Perf delta is %f and Image delta is %f.", PerfDelta, ImageDelta);
        if (ImageDelta / PerfDelta > 0.04)
        {
            UE_LOGF(ShaderComparisonTests, Error, "The quality detriment doesn't outweigh the performance.");
        }
    }));
