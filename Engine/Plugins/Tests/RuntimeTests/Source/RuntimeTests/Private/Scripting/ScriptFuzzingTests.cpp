...
UObject* CreateFuzzingHostObject(UWorld* World, UClass* DesiredClass, TArray<UObject*>& CreatedHosts)
{
	...
	if (!DesiredClass->HasAnyClassFlags(CLASS_Abstract))
	{
		if (DesiredClass->IsChildOf(AActor::StaticClass()))
		{
			...
			{
				UE_LOGF(LogTemp, Warning, "Cannot fuzz non-static methods in %ls%ls as it is marked NotPlaceable", DesiredClass->GetPrefixCPP(), *DesiredClass->GetName());
			}
			else
			{
		...
		else
		{
			...
			{
				UE_LOGF(LogTemp, Log, "%ls%ls has a desired class within of %ls", DesiredClass->GetPrefixCPP(), *DesiredClass->GetName(), *DesiredWithin->GetName());
			...
			{
				UE_LOGF(LogTemp, Warning, "Cannot fuzz non-static methods in %ls%ls, was unable to create an appropriate outer to satisify a ClassWithin constraint", DesiredClass->GetPrefixCPP(), *DesiredClass->GetName());
			}
		}
	}
	else
	{
		UE_LOGF(LogTemp, Warning, "Cannot fuzz non-static methods in %ls%ls as it is marked Abstract; we might be able to in derived classes", DesiredClass->GetPrefixCPP(), *DesiredClass->GetName());
	}
			...
			if (bHasFunctions)
			{
				...
				for (TFieldIterator<UFunction> FuncIt(TestClass); FuncIt; ++FuncIt)
				{
					...
					if (Function->HasAnyFunctionFlags(FUNC_BlueprintCallable | FUNC_BlueprintPure))
					{
						...
						{
							UE_LOGF(LogTemp, Log, "Fuzzing %ls%ls::%ls() on %ls", TestClass->GetPrefixCPP(), *TestClass->GetName(), *Function->GetName(), *TestInstance->GetName());
				...
				{
					UE_LOGF(LogTemp, Warning, "Failed to fuzz %d non-static functions on %ls%ls because we could not make an object to test it on", NumFunctionsWeFailedToFuzzDueToNoObject, TestClass->GetPrefixCPP(), *TestClass->GetName());
				}
