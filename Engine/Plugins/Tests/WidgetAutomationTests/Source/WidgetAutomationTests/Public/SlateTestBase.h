			...
			TestObj->AddCommand(new FDelayedFunctionLatentCommand([this, WidgetRef = GetSubjectWidget(), TestObj, InvalidateReason, PropertyName, MyWindow = MainWindow]()
				{
					...
					if (WidgetRef->IsKeyInDataValidations(FTestIONames::NAME_BadTestFile))
					{
						...
						TestObj->AddError(FString::Printf(TEXT("In property %s in %s. The ground truth JSON file '%s' for this test was not found and could not be created. Did you forget to call FindOrCreateTestJSONFile in the setup of your test?"), *PropertyName, GetGeneratedTypeName<T>(), *FileLocation));
					}
					else
					{
						...
						if (EnumHasAnyFlags(InvalidateReason, EInvalidateWidgetReason::Paint))
						{
							...
							TestObj->AddErrorIfFalse(WidgetRef->IsKeyInDataValidations(FTestFunctionNames::NAME_OnPaint) && WidgetRef->GetValueInDataValidations(FTestFunctionNames::NAME_OnPaint), FString::Printf(TEXT("In %s: The list of elements to draw is wrong or does not exist."), GetGeneratedTypeName<T>()));
						}
					}
