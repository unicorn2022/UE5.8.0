...
{
	MyJsonObject->SetStringField(TEXT("AllottedGeometry"), MyGeometry.ToString());
}
...
void FSlateTestHelper::ToJson(const FWidgetStyle& InWidgetStyle, TSharedPtr<FJsonObject> MyJsonObject)
{
	...
	InnerJsonObject->SetStringField(TEXT("ColorAndOpacityTint"), InWidgetStyle.GetColorAndOpacityTint().ToString());
	InnerJsonObject->SetStringField(TEXT("ForegroundColor"), InWidgetStyle.GetForegroundColor().ToString());
	InnerJsonObject->SetStringField(TEXT("SubduedForeground"), InWidgetStyle.GetSubduedForegroundColor().ToString());
	MyJsonObject->SetObjectField(TEXT("WidgetStyle"), InnerJsonObject);
}

void FSlateTestHelper::ToJson(const FSlateShapedTextElement& ShapedTextElement, TSharedPtr<FJsonObject> MyJsonObject)
{
	TSharedPtr<FJsonObject> InnerJsonObject = MakeShared<FJsonObject>();
	InnerJsonObject->SetStringField(TEXT("Tint"), ShapedTextElement.GetTint().ToString());
	InnerJsonObject->SetStringField(TEXT("OutlineTint"), ShapedTextElement.GetOutlineTint().ToString());
	MyJsonObject->SetObjectField(TEXT("ShapedTextElement"), InnerJsonObject);
}
}
#endif