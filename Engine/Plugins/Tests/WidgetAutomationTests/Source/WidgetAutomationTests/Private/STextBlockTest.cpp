...
public:
	...
	{
		GetSubjectWidget()->SetColorAndOpacity(FSlateColor(FLinearColor(0.5f,0.4f,0.3f,0.9f)));
	}
	...
	void Validate(FAutomationTestBase* TestObj) override
	{
		...
		ValidatePropertyChange<FSlateColor>(TestObj, &STextBlock::GetColorAndOpacity, "ColorAndOpacity", FSlateColor(FLinearColor(0.5f, 0.4f, 0.3f, 0.9f)), EInvalidateWidgetReason::Layout|EInvalidateWidgetReason::Paint);
		...
		TestObj->AddCommand(new FDelayedFunctionLatentCommand([this, TestObj]()
			{
				if (!GetMainWindow()->Advanced_IsInvalidationRoot())
				{
					...
					if (TSharedPtr<FJsonObject> JsonObject = GetSubjectWidget()->GetTestJSONObject())
					{
						...
						TestObj->AddErrorIfFalse(PayloadTint == FLinearColor(0.5f, 0.4f, 0.3f, 0.9f).ToString(), FString::Printf(TEXT("In %s: The tint of the payload does not match the passed color. Expected: %s, Actual: %s"), GetGeneratedTypeName<STextBlock>(), *FLinearColor(0.5f, 0.4f, 0.3f, 0.9f).ToString(), *PayloadTint));
					}
				}
			}, 0.1f));
	}
