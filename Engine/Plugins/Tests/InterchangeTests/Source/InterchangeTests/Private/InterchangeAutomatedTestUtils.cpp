...
bool FInterchangeCaptureScreenshotCommand::Update()
{
	...
	if (TestStepData->ScreenshotTask == nullptr)
	{
		...
		constexpr bool bMaskEnabled = false;
		constexpr bool bCaptureHDR = false;
		constexpr float Delay = 0.0f;

		TestStepData->ScreenshotTask = UAutomationBlueprintFunctionLibrary::TakeHighResScreenshot(
			1280, 720, 
			GetScreenshotNameString(*TestStepData), 
			nullptr, 
			bMaskEnabled, bCaptureHDR, 
			ScreenshotParameters.ComparisonTolerance,
			TEXT(""),
			Delay,
			ScreenshotParameters.bForceGameView);
	}
