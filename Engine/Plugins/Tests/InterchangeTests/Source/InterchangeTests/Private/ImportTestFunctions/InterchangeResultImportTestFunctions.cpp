...
FInterchangeTestFunctionResult UInterchangeResultImportTestFunctions::CheckIfErrorOrWarningWasGenerated(UInterchangeResultsContainer* ResultsContainer, TSubclassOf<UInterchangeResult> ErrorOrWarningClass, const FString& Message)
{
	...
	for (UInterchangeResult* ResultObject : ResultsContainer->GetResults())
	{
		if (ResultObject->IsA(ErrorOrWarningClass))
		{
			...
			const FString ResultMessage = ResultObject->GetText().ToString();
			if (!Message.IsEmpty() && ResultMessage != Message)
			{
				Result.AddError(FString::Printf(TEXT("Expected '%s' error message text, found '%s'."), *Message, *ResultMessage));
			}
		}
	}
