...
FInterchangeTestFunctionResult UTextureImportTestFunctions::CheckTextureType(const TArray<UTexture*>& Textures, TSubclassOf<UTexture> ExpectedTextureType, int32 ExpectedNumber)
{
	FInterchangeTestFunctionResult Result;
	int32 Count = 0;
	for (UTexture* Texture : Textures)
	{
		if (Texture->IsA(ExpectedTextureType.Get()))
		{
			Count++;
		}
	}
	...
	{
		Result.AddError(FString::Printf(TEXT("Expected to have %d textures of type %s, imported %d."), ExpectedNumber, *ExpectedTextureType.Get()->GetName(), Count));
	}
