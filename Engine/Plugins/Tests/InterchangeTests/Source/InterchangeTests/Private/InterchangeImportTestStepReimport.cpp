...
void  UInterchangeImportTestStepReimport::EditPipelineSettings()
{
	...
	if (SourceFilePathString.IsEmpty())
	{
		UE_LOGF(LogInterchangeTests, Error, "Import Step doesn't have a valid Source File to get the default pipeline stack.");
		return;
	}
	...
	{
		UE_LOGF(LogInterchangeTests, Error, "No valid import step found. Make sure this Reimport Step is part of an Interchange Test Plan Asset and is not used independently.");
		return;
	}
