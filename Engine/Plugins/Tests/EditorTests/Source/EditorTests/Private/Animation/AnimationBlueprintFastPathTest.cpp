...
bool FCompileAnimBlueprintLatentCommand::Update()
{
	...
	if (AnimBlueprint != nullptr)
	{
		...
		{
			UE_LOGF(LogAnimBlueprintFastPathTests, Display, "Anim blueprint compiled successfully (%ls)", *AnimBlueprint->GetName());
		}
		else
		{
			UE_LOGF(LogAnimBlueprintFastPathTests, Error, "Anim blueprint is in an unexpected state after compiling (%ls)", *AnimBlueprint->GetName());
		}
	}
...
bool FCheckFastPathLatentCommand::Update()
{
	...
	if (AnimBlueprint != nullptr)
	{
		if (AnimBlueprint->GeneratedClass)
		{
						...
						if (AnimNode)
						{
							if (AnimNode->GetEvaluateGraphExposedInputs().GetHandlerStruct() == FAnimNodeExposedValueHandler_PropertyAccess::StaticStruct())
							{
								...
								if (Payload.bCheckEnabled)
								{
									...
									{
										UE_LOGF(LogAnimBlueprintFastPathTests, Error, "Found blueprint VM call (%ls) in fast-path Anim Blueprint (%ls)", *Handler->BoundFunction.ToString(), *AnimBlueprint->GetName());
									}
									for (const FExposedValueCopyRecord& CopyRecord : Handler->CopyRecords)
									{
										if (CopyRecord.CopyIndex == INDEX_NONE)
										{
											UE_LOGF(LogAnimBlueprintFastPathTests, Error, "Anim blueprint has an invalid copy index (%ls)", *AnimBlueprint->GetName());
										}
									}
								}
								else
								{
									if (Handler->BoundFunction == NAME_None)
									{
										UE_LOGF(LogAnimBlueprintFastPathTests, Error, "No function bound for node evaluation (%ls)", *AnimBlueprint->GetName());
									}
									if (Handler->CopyRecords.Num() > 0)
									{
										UE_LOGF(LogAnimBlueprintFastPathTests, Error, "Found copy records in non-fast-path node evaluator (%ls)", *AnimBlueprint->GetName());
									}
								}
							}
						}
			...
			{
				UE_LOGF(LogAnimBlueprintFastPathTests, Error, "Anim blueprint has an invalid generated class or CDO (%ls)", *AnimBlueprint->GetName());
			}
		}
		else
		{
			UE_LOGF(LogAnimBlueprintFastPathTests, Error, "Anim blueprint has no generated class (%ls)", *AnimBlueprint->GetName());
		}
	}
