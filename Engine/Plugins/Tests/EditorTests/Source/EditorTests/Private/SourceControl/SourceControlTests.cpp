...
bool FSetProviderLatentCommand::Update()
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "Could not set provider to '%ls'", *ProviderName.ToString());
	}
	return true;
}
...
bool FRevertLatentCommand::Update()
{
	...
	{		
		...
		{
			UE_LOGF(LogSourceControl, Error, "Could not retrieve state for file '%ls'", *AsyncHelper.GetParameter());
		}
		else
		{
			if(SourceControlState->IsSourceControlled())
			{
				...
				{
					UE_LOGF(LogSourceControl, Error, "Unexpected state following Revert operation for file '%ls'", *AsyncHelper.GetParameter());
				}
			}
		}
	}
...
bool FCheckOutLatentCommand::Update()
{
	...
	if(AsyncHelper.IsDone())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Unexpected state following Check Out operation for file '%ls'", *AsyncHelper.GetParameter());
			}
		}
	}
...
bool FCreatePackageLatentCommand::Update()
{
	...
	if(FPlatformFileManager::Get().GetPlatformFile().CopyFile(*NewPackageName, *OriginalPackageName))
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "Could not load temporary package '%ls'", *PackageName);
		}
	}
	else
	{
		UE_LOGF(LogSourceControl, Error, "Could not create temporary package to add '%ls'", *PackageName);
	}
...
bool FDeletePackageLatentCommand::Update()
{
	...
	if(Package != NULL)
	{
		...
		if(UPackageTools::UnloadPackages(Packages))
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Could not delete temporary package '%ls'", *PackageFileName);
			}
		}
		else
		{
			UE_LOGF(LogSourceControl, Error, "Could not unload temporary package '%ls'", *Parameter);
		}
	}
	else
	{
		UE_LOGF(LogSourceControl, Error, "Could not find temporary package '%ls'", *Parameter);
	}
...
bool FMarkForAddLatentCommand::Update()
{
	...
	if(AsyncHelper.IsDone())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Unexpected state following Mark For Add operation for file '%ls'", *AsyncHelper.GetParameter());
			}
		}
	}
...
bool FDeleteLatentCommand::Update()
{
	...
	if(AsyncHelper.IsDone())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Unexpected state following Delete operation for file '%ls'", *AsyncHelper.GetParameter());
			}
		}
	}
...
bool FCheckInLatentCommand::Update()
{
	...
	if(AsyncHelper.IsDone())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Unexpected state following Check In operation for file '%ls'", *AsyncHelper.GetParameter());
			}
		}
	}
...
bool FEditTextureLatentCommand::Update()
{
	...
	if(Package != NULL)
	{
		...
		if(!UPackage::SavePackage(Package, nullptr,
			*FPackageName::LongPackageNameToFilename(PackageName, FPackageName::GetAssetPackageExtension()),
			SaveArgs))
		{
			UE_LOGF(LogSourceControl, Error, "Could not save package: '%ls'", *PackageName);
		}
		TArray<UPackage*> Packages;
		Packages.Add(Package);
	...
	{
		UE_LOGF(LogSourceControl, Error, "Could not find package for edit: '%ls'", *PackageName);
	}
...
bool FSyncLatentCommand::Update()
{
	...
	if(AsyncHelper.IsDone())
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Unexpected state following Sync operation for file '%ls'", *AsyncHelper.GetParameter());
			}
		}
	}
...
bool FGetStateLatentCommand::Update()
{
	...
	if(!SourceControlState.IsValid())
	{
		UE_LOGF(LogSourceControl, Error, "Failed to get a valid state for file: %ls", *Filename);
	}
	else
	{
		if(!SourceControlState->IsCheckedOut())
		{
			UE_LOGF(LogSourceControl, Error, "File '%ls' should be checked out, but isnt.", *Filename);
		}
		else
		{
			if(SourceControlState->GetHistorySize() == 0)
			{
				UE_LOGF(LogSourceControl, Error, "Failed to get a valid history for file: %ls", *Filename);
			}
			else
			{
				...
				{
					UE_LOGF(LogSourceControl, Error, "Failed to get a valid history item 0 for file: %ls", *Filename);
				}
			}
		}
	}
...
bool FGetLabelLatentCommand::Update()
{
	...
	if(Labels.Num() == 0)
	{
		UE_LOGF(LogSourceControl, Error, "No labels available that use the spec '%ls'", *LabelAndFilenames.Label);
	}
	else
	{
		...
		if(Revisions.Num() == 0)
		{
			UE_LOGF(LogSourceControl, Error, "No revisions of file '%ls' found at label '%ls'", *LabelAndFilenames.Filenames[0], *LabelAndFilenames.Label);
		}
		else
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Could not get revision of file '%ls' using label '%ls'", *LabelAndFilenames.Filenames[0], *LabelAndFilenames.Label);
			}
			...
			{
				UE_LOGF(LogSourceControl, Error, "Could not get annotated revision of file '%ls' using label '%ls'", *LabelAndFilenames.Filenames[0], *LabelAndFilenames.Label);
			}
		}
	}
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if(TempGetFilename.Len() == 0 || !FPaths::FileExists(TempGetFilename))
				{
					UE_LOGF(LogSourceControl, Error, "Could not get revision of file '%ls'", *Filename);
				}
			}
		}
	}
