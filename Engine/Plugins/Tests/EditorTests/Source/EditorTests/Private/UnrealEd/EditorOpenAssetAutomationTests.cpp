...
bool FOpenAssetEditors::RunTest(const FString& LongAssetPath)
{
	...
	if ( Object == nullptr )
	{
		UE_LOGF(LogEditorAutomationTests, Error, "Failed to Open Asset '%ls'.", *LongAssetPath);
		return false;
	}
	...
	AddCommand(new FDelayedFunctionLatentCommand([WeakObjPtr=TWeakObjectPtr<UObject>(Object),LongAssetPath] {
		...
		{		
			...
			if (Obj == nullptr)
			{
				UE_LOGF(LogEditorAutomationTests, Error, "Asset '%ls' did not exist when evaluating.", *LongAssetPath);
			}
		}
		if (Obj && Obj->GetOutermost()->IsDirty() )
		{
			UE_LOGF(LogEditorAutomationTests, Error, "Asset '%ls' was dirty after opening it.", *LongAssetPath);
		}
	}));
