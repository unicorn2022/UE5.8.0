...
struct FAutoReimportTestPayload
{
	...
	{	
		if (!IFileManager::Get().MakeDirectory(*Config.Directory, true))
		{
			...
			UE_LOGF(LogAutoReimportTests, Error, "Failed to create the test cache folder %ls. LastError code: %d", *Config.Directory, ErrorCode);
		}
		// Just to be sure...
		if (!IFileManager::Get().MakeDirectory(*InWorkingDir, true))
		{
			uint32 ErrorCode = FPlatformMisc::GetLastError();
			UE_LOGF(LogAutoReimportTests, Error, "Failed to create the test cache folder %ls (retry operation). LastError code: %d", *InWorkingDir, ErrorCode);
		}
	}
	~FAutoReimportTestPayload()
	{
		...
		{
			UE_LOGF(LogAutoReimportTests, Error, "Failed to delete the test cache folder %ls.", *WorkingDir);
		}
	}
...
bool CopyTestFiles(const FAutoReimportTestPayload& Test, const TArray<FSrcDstFilenames>& Files)
{
	...
	for (const auto& File : Files)
	{
		...
		if (!bCopied)
		{
			UE_LOGF(LogAutoReimportTests, Error, "Failed to copy source file %ls to the test directory as %ls.", File.Src, File.Dst);
			return false;
		}
	}
...
bool FAutoReimportSimpleCreateTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		auto SimpleCreateFunction = [=](const bool bLastIteration)->bool
		{
			...
			if (Changes.Num() != 1)
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect number of changes reported (%d != 1).", Changes.Num());
			}
			// Check that we got a relative path
			else if (!Changes[0].Filename.Get().Equals(Filename))
			{
				UE_LOGF(LogAutoReimportTests, Error, "Path reported incorrectly (%ls != %ls).", *Changes[0].Filename.Get(), Filename);
			}
			// Check it was the correct type
			else if (Changes[0].Action != DirectoryWatcher::EFileAction::Added)
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for added file %ls.", Filename);
			}
			else
			{
				...
				{
					UE_LOGF(LogAutoReimportTests, Error, "Add transaction was not applied correctly.");
				}
			}
			return true;
		};
...
bool FAutoReimportSimpleModifyTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		auto SimpleModifyFunction = [=](const bool bLastIteration)->bool
		{
			...
			{
				UE_LOGF(LogAutoReimportTests, Error, "Path reported incorrectly (%ls != %ls).", *Changes[0].Filename.Get(), DstFilename);
			}
			// Check it was the correct type
			else if (Changes[0].Action != DirectoryWatcher::EFileAction::Modified)
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for modified file %ls.", DstFilename);
			}
			else
			{
				...
				{
					UE_LOGF(LogAutoReimportTests, Error, "Modify transaction was not applied correctly.");
				}
			}
			return true;
		};
...
bool FAutoReimportSimpleDeleteTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		if (!bDeleted)
		{
			UE_LOGF(LogAutoReimportTests, Error, "Failed to delete source file from the test directory.");
			return;
		}
		...
		auto SimpleDeleteFunction = [=](const bool bLastIteration)->bool
		{
			...
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for deleted file %ls.", Filename);
			}
			else
			{
				Test->FileCache->CompleteTransaction(MoveTemp(Changes[0]));
				...
				if (Test->FileCache->FindFileData(Filename))
				{
					UE_LOGF(LogAutoReimportTests, Error, "Remove transaction was not applied correctly.");
				}
			}
			return true;
		};
...
bool FAutoReimportSimpleRenameTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		auto SimpleRenameFunction = [=](const bool bLastIteration)->bool
		{
			...
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for renamed file %ls -> %ls.", SrcFilename, DstFilename);
			}
			else if (!Changes[0].Filename.Get().Equals(DstFilename))
			{
				UE_LOGF(LogAutoReimportTests, Error, "Source path reported incorrectly (%ls != %ls).", *Changes[0].Filename.Get(), DstFilename);
			}
			else if (!Changes[0].MovedFromFilename.Get().Equals(SrcFilename))
			{
				UE_LOGF(LogAutoReimportTests, Error, "Destination path reported incorrectly (%ls != %ls).", *Changes[0].MovedFromFilename.Get(), SrcFilename);
			}
			else
			{
				Test->FileCache->CompleteTransaction(MoveTemp(Changes[0]));
				...
				{
					UE_LOGF(LogAutoReimportTests, Error, "Rename transaction was not applied correctly.");
				}
			}
			return true;
		};
...
bool FAutoReimportSimpleMoveExternallyTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		{
			UE_LOGF(LogAutoReimportTests, Error, "Could not find file data for initial file %ls.", SrcFilename);
			return;
		}
		...
		auto MoveExternallyFunction = [=](const bool bLastIteration)->bool
		{
			...
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for removed file %ls -> %ls.", SrcFilename, DstFilename);
			}
			else if (!Changes[0].Filename.Get().Equals(SrcFilename))
			{
				UE_LOGF(LogAutoReimportTests, Error, "Source path reported incorrectly (%ls != %ls).", *Changes[0].Filename.Get(), SrcFilename);
			}
			else
			{
				Test->FileCache->CompleteTransaction(MoveTemp(Changes[0]));
				...
				if (Test->FileCache->FindFileData(SrcFilename))
				{
					UE_LOGF(LogAutoReimportTests, Error, "Found data for file that should have been removed (%ls).", SrcFilename);
				}
			}
			return true;
		};
...
bool FAutoReimportRestartDetectionTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		Test->WaitForStartup([=]{
			...
			{
				UE_LOGF(LogAutoReimportTests, Log, "Change %d for file %ls.", (uint8)Change.Action, *Change.Filename.Get());
			}
			...
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for modified file %ls.", SrcFilename);
				return;
			}
			else
			{
			...
			Test->WaitForStartup([=]{
				...
				{
					UE_LOGF(LogAutoReimportTests, Error, "Shouldn't have reported changes when bDetectChangesSinceLastRun is false (%d change(s) received).", OutstandingChanges.Num());
					return;
				}
...
bool FAutoReimportMultipleChangesTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		auto MultipleChangesFunction = [=](const bool bLastIteration)->bool
		{
			...
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for added file %ls.", Filename2);
			}
			return true;
		};
...
bool FAutoReimportChangeExtensionsTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		auto ChangeExtensionsFunction = [=](const bool bLastIteration)->bool
		{
			...
			{
				UE_LOGF(LogAutoReimportTests, Error, "Incorrect action for added file %ls.", SrcFilename3);
				return true;
			}
			else if (!Changes[0].Filename.Get().Equals(SrcFilename3))
			{
				UE_LOGF(LogAutoReimportTests, Error, "Added file path reported incorrectly (%ls != %ls).", *Changes[0].Filename.Get(), SrcFilename3);
				return true;
			}
			Test->FileCache->CompleteTransaction(MoveTemp(Changes[0]));
			...
			Test->WaitForStartup([=]{

				{
					...
					if (OutstandingChanges.Num() != 0)
					{
						UE_LOGF(LogAutoReimportTests, Error, "Incorrect number of changes reported (%d != 0).", OutstandingChanges.Num());
						return;
					}
				}
				...
				auto ChangeExtensionsInternalFunction = [=](const bool bLastIteration)->bool
				{
					...
					{
						UE_LOGF(LogAutoReimportTests, Error, "Incorrect number of changes reported (%d != 1).", OutstandingChanges.Num());
					}
					return true;
				};
...
bool FAutoReimportWildcardFiltersTest::RunTest(const FString& Parameters)
{
	...
	Test->WaitForStartup([=]{
		...
		{
			UE_LOGF(LogAutoReimportTests, Error, "Couldn't find data for %ls.", DstFilename3);
		}
		else if (Test->FileCache->FindFileData(DstFilename2))
		{
			UE_LOGF(LogAutoReimportTests, Error, "Erroneously found data for %ls.", DstFilename2);
		}
		else if (Test->FileCache->FindFileData(DstFilename1))
		{
			UE_LOGF(LogAutoReimportTests, Error, "Erroneously found data for %ls.", DstFilename1);
		}
		else
		{
			...
			auto WildcardFiltersFunction = [=](const bool bLastIteration)->bool
			{
				...
				{
					UE_LOGF(LogAutoReimportTests, Error, "Modified file path reported incorrectly (%ls != %ls).", *Changes[0].Filename.Get(), DstFilename3);
				}
				return true;
			};
