...
namespace GeometryPromotionHelpers
{
	...
	{
		UE_LOGF(LogGeometryTests, Log, "Adding an '%ls' brush.", *InBrushType);
	...
	*/
	void TestGeometryUndoRedo(ULevel* CurrentLevel, int32 VertsBefore, int32 VertsAfter, const FString& BrushType, bool bAdditive)
	{
		...
		{
			UE_LOGF(LogEditorAutomationTests, Log, "Placed a %ls in %ls mode", *BrushType, bAdditive ? TEXT("additive") : TEXT("subtractive"));
		}
		else
		{
			UE_LOGF(LogEditorAutomationTests, Error, "Failed to place a %ls in %ls mode", *BrushType, bAdditive ? TEXT("additive") : TEXT("subtractive"));
		}
		...
		{
			UE_LOGF(LogEditorAutomationTests, Log, "Undo successful for %ls", *BrushType);
		}
		else
		{
			UE_LOGF(LogEditorAutomationTests, Error, "Undo failed for %ls", *BrushType);
		}
		...
		{
			UE_LOGF(LogEditorAutomationTests, Log, "Redo successful for %ls", *BrushType);
		}
		else
		{
			UE_LOGF(LogEditorAutomationTests, Error, "Redo failed for %ls", *BrushType);
		}
	}
}
...
bool FGeometryPromotionValidation::RunTest(const FString& Parameters)
{
	...
	//** TEST **//
	//Cube Additive Brush
	UE_LOGF(LogGeometryTests, Log, "Adding an 'Additive Cube' brush.");
	...
	UE_LOGF(LogGeometryTests, Log, "The number of BSP Brushes Found in the level: Additive: %i, Subtractive: %i", AdditiveBSP.Num(), SubtractiveBSP.Num());
...
bool FBSPCubeUndoRedoPlacement::RunTest(const FString& Parameters)
{
	...
	UE_LOGF(LogGeometryTests, Error, "Unable to test Undo/Redo since the brush actor was not placed into the level.")
	return false;
}
