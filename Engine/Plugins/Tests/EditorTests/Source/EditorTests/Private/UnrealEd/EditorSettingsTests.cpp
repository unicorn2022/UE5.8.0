...
namespace EditorSettingsTestUtils
{
	...
	*/
	static void ImportSettings(FString& IniSettings, const FString& SourceFilename)
	{
		...
		{
			UE_LOGF(LogEditorSettingsTests, Error, "Failed to load settings from %ls", *SourceFilename);
		}
	}
	...
	*/
	static FInputChord SetKeybinding(const FString& CommandContext, const FString& Command, const FKey Key, const EModifierKey::Type ModifierKey)
	{
		...
		if (!FEditorPromotionTestUtilities::SetEditorKeybinding(CommandContext, Command, NewChord))
		{
			...
			UE_LOGF(LogEditorSettingsTests, Error, "Could not find keybinding for %ls using context %ls", *Command, *CommandContext);
		}
		return NewChord;
	}
	...
	*/
	static void CompareKeybindings(const FString& CommandContext, const FString& Command, FInputChord ExpectedChord)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogEditorSettingsTests, Display, "%ls keybinding correct.", *Command);
			}
			else
			{
				UE_LOGF(LogEditorSettingsTests, Error, "%ls keybinding incorrect.", *Command);
			}
		}
	}
}
...
bool FSettingsCheckForPIECommand::Update()
{
	...
	{
		//Success
		UE_LOGF(LogEditorSettingsTests, Display, "PlayInEditor keyboard shortcut success");
		FEditorPromotionTestUtilities::EndPIE();
	}
	else
	{
		UE_LOGF(LogEditorSettingsTests, Error, "PlayInEditor keyboard shortcut failed");
	}
	return true;
}
...
{
	UE_LOGF(LogEditorSettingsTests, Display, "Exporting Current keybindings and editor settings");
	...
	UE_LOGF(LogEditorSettingsTests, Display, "Binding create empty layer shortcut");
	...
	UE_LOGF(LogEditorSettingsTests, Display, "Binding request rename layer shortcut");
	...
	// Bind CTRL+L to PIE
	UE_LOGF(LogEditorSettingsTests, Display, "Binding play shortcut (PIE)");
	...
	UE_LOGF(LogEditorSettingsTests, Display, "Exporting keybind");
	...
	else
	{
		...
		#if (0)
		{
			...
			UE_LOGF(LogEditorSettingsTests, Display, "Sent PIE keyboard shortcut");
	...
	UE_LOGF(LogEditorSettingsTests, Display, "Reloaded original keybindings");
...
bool FEditorSettingsPreferencesTest::RunTest(const FString& Parameters)
{
	...
	UE_LOGF(LogEditorSettingsTests, Display, "Enabled UseSmallToolBarIcons");
	...
	UE_LOGF(LogEditorSettingsTests, Display, "Set UseSmallToolBarIcons to original state");

	EditorSettingsTestUtils::ImportEditorSettings(TargetOriginalPreferenceFile);
	UE_LOGF(LogEditorSettingsTests, Display, "Reloaded original preferences");
