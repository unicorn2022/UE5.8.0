...
bool FLoadAllMapsInEditorTest::RunTest(const FString& Parameters)
{
	...
	if (FEngineAnalytics::IsAvailable())
	{
		...
		UE_LOGF(LogEditorAutomationTests, Log, "AnayticsTest: Load All Maps automation triggered and Editor.Usage.TestEvent analytic event has been fired.");
	}
...

	{
		...
		UE_LOGF(LogEditorAutomationTests, Display, "Map '%ls' took %.3f to load", *MapName, FPlatformTime::Seconds() - MapLoadStartTime);
...
bool FConvertToValidation::RunTest(const FString& Parameters)
{
	...
	FString AssetPackageName;
	{
		...
		{
			UE_LOGF(LogEditorAutomationTests, Error, "Failed to save ConvertToBSPToStaticMesh.");
		}
	}
...
bool FStaticMeshPlacement::RunTest(const FString& Parameters)
{
	...
	{
		UE_LOGF(LogEditorAutomationTests, Error, "Failed to find material: %ls", *MaterialName);
	}
...
bool FTraceAllTimelinesAutomationTest::RunTest(const FString& BlueprintName)
{
	...
	{
		UE_LOGF(LogEditorAutomationTests, Error, "Failed to load invalid blueprint, or blueprint parent no longer exists.");
		return false;
	}
	...
	if (AllTimelines.Num() > 0)
	{
		...
		for (auto TimelineIter = AllTimelines.CreateIterator(); TimelineIter; ++TimelineIter)
		{
			...
			UE_LOGF(LogEditorAutomationTests, Log, "TraceTimeline: %ls [%ls]", *TimelinePtr->GetPathName(), *TimelinePtr->TimelineName.ToString());
			...
			for (int32 EventIndex = 0; EventIndex < AllEventNodes.Num(); ++EventIndex)
			{
				...
				if (bIsBadEvent)
				{
					...
					UE_LOGF(LogEditorAutomationTests, Error, "%ls", *OutputText);
					bPassed = false;
				}
				else if (bIsWarningEvent)
				{
					UE_LOGF(LogEditorAutomationTests, Warning, "%ls", *(OutputText + TEXT(" (could not find function calling this event)")));
				}

				// I always want it in the log as well as the automation test log.
				UE_LOGF(LogEditorAutomationTests, Log, "%ls", *OutputText);
			}
		}
	}
			...
			for (int32 i = 0; i < MapToLaunch.Num(); ++i)
			{
				...
				if (!DeviceID.IsEmpty() && !DeviceID.Equals(TEXT("None")))
				{
					if (Filename.Contains(TEXT("/Engine/"), ESearchCase::IgnoreCase, ESearchDir::FromStart))
					{
						...
						{
							UE_LOGF(LogEditorAutomationTests, Error, "Invalid asset path: %ls.", *Filename);
						}
					}
					else
					{
