...
public:
	...
	static void CloseBlueprint(UBlueprint* const BlueprintObj)
	{
		...
		{
			UE_LOGF(LogBlueprintAutomationTests, Log, "Closing '%ls' so we don't invalidate the open version when unloading it.", *BlueprintObj->GetName());
	...
	static void UnloadBlueprint(UBlueprint* const BlueprintObj, bool bForceFlush = false)
	{
		...
		{
			UE_LOGF(LogBlueprintAutomationTests, Log, "No need to unload '%ls' from the transient package.", *BlueprintObj->GetName());
		}
		else if (OldPackage->IsRooted() || BlueprintObj->IsRooted())
		{
			UE_LOGF(LogBlueprintAutomationTests, Error, "Cannot unload '%ls' when its root is set (it will not be garbage collected, leaving it in an erroneous state).", *OldPackage->GetName());
		}
		else if (OldPackage->IsDirty())
		{
			UE_LOGF(LogBlueprintAutomationTests, Error, "Cannot unload '%ls' when it has unsaved changes (save the asset and then try again).", *OldPackage->GetName());
		}
		else 
		{
	...
	static bool CompileBlueprint(const FString& BlueprintAssetPath)
	{
		...
		{
			UE_LOGF(LogBlueprintAutomationTests, Error, "Failed to compile invalid blueprint, or blueprint parent no longer exists.");
			return false;
		}
...
{
	UE_LOGF(LogBlueprintAutomationTests, Log, "Beginning compile test for %ls", *Parameters);
