...
namespace EditorBuildPromotionTestUtils
{
	...
	*/
	static void CompileBlueprint(UBlueprint* InBlueprint)
	{
		...
		{
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Blueprint compiled successfully (%ls)", *InBlueprint->GetName());
		}
		else if (InBlueprint->Status == EBlueprintStatus::BS_UpToDateWithWarnings)
		{
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Blueprint compiled successfully with warnings(%ls)", *InBlueprint->GetName());
		}
		else if (InBlueprint->Status == EBlueprintStatus::BS_Error)
		{
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Blueprint failed to compile (%ls)", *InBlueprint->GetName());
		}
		else
		{
			UE_LOGF(LogEditorBuildPromotionTests, Error, "Blueprint is in an unexpected state after compiling (%ls)", *InBlueprint->GetName());
		}
	}
	...
	*/
	static void BuildLighting()
	{
		...
		if (GUsingNullRHI)
		{
			UE_LOGF(LogEditorBuildPromotionTests, Log, "SKIPPED Build Lighting Step.  You're currently running with -NullRHI.");
			return;
		}
	...
	*/
	static AStaticMeshActor* AddDefaultMeshToLevel(const FVector& Location)
	{
		...
		if (AssetPackagePath.Len() > 0)
		{
			...
			{
				UE_LOGF(LogEditorBuildPromotionTests, Error, "DefaultStaticMeshAsset is invalid.");
			}
		}
		else
		{
			UE_LOGF(LogEditorBuildPromotionTests, Warning, "Can't add Static Mesh to level because no DefaultMeshAsset is defined.");
		}
	...
	*/
	static bool ApplyMaterialToStaticMesh(AStaticMeshActor* StaticMesh, UMaterialInterface* Material)
	{
		...
		{
			UE_LOGF(LogEditorBuildPromotionTests, Warning, "Failed to apply material to static mesh because mesh does not exist");
			return false;
		}
	}
	...
	static void PlaceImportedAsset(UObject* InObject, FVector& PlaceLocation)
	{
		if (InObject)
		{
			...
			if (PlacedActor)
			{
				...
				PlaceLocation.Y += 100;
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Placed %ls in the level", *InObject->GetName());
			}
			else
			{
				UE_LOGF(LogEditorBuildPromotionTests, Error, "Failed to place %ls in the level", *InObject->GetName());
			}
		}
	}
	...
	*/
	static void PerformCleanup()
	{
		...
		for (int32 AssetIdx = 0; AssetIdx < AssetList.Num(); ++AssetIdx)
		{
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Removing asset: %ls", *AssetList[AssetIdx].AssetName.ToString());
		...
		UE_LOGF(LogEditorBuildPromotionTests, Display, "Clearing Path: %ls", *FEditorPromotionTestUtilities::GetGamePath());
		...
		UE_LOGF(LogEditorBuildPromotionTests, Display, "Deleting Folder: %ls", *PackageDirectory);
		...
		UE_LOGF(LogEditorBuildPromotionTests, Display, "Deleting Map: %ls", *MapFilePath);
	}
	...
	private:
		...
		*/
		void OpenAssetEditor()
		{
			...
			{
				UE_LOGF(LogEditorBuildPromotionTests, Error, "Failed to open the asset editor!");
		...
		*/
		void WaitForEditor()
		{
			...
			if (ActiveWindow.IsValid())
			{
				...
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Opened asset (%ls)", *CurrentAsset->GetClass()->GetName());
			...
			else
			{
				...
				if (WaitingForEditorCount > MaxWaitForEditorTicks)
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Error, "Timed out waiting for editor window");
		...
		*/
		void ChangeProperty()
		{
			...
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Modified asset.  %ls = %ls", *PropertyName, *NewPropertyValue);
			...
			{
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Undo %ls change successful", *PropertyName);
			}
			else
			{
				UE_LOGF(LogEditorBuildPromotionTests, Error, "Undo didn't change property back to original value");
			}
			...
			{
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Redo %ls change successful", *PropertyName);
			}
			else
			{
				UE_LOGF(LogEditorBuildPromotionTests, Error, "Redo didnt' change property back to the modified value");
			}
			...
			if (CurrentAsset && PackagePath.Len() > 0)
			{
				...
				if (AssetPackage)
				{
					...
					{
						UE_LOGF(LogEditorBuildPromotionTests, Display, "Saved asset");
					}
					else
					{
						UE_LOGF(LogEditorBuildPromotionTests, Display, "Unable to save asset");
					}
				}
			}
			...
			else
			{
				...
				if (PlacedActor)
				{
					UE_LOGF(LogEditorBuildPromotionTests, Display, "Placed %ls in the level", *GetNameSafe(CurrentAsset));
					PlacedActor->SetActorLocation(PlaceLocation);
				}
				else
				{
					UE_LOGF(LogEditorBuildPromotionTests, Warning, "Unable to place asset");
				}
			}
	...
	private:
		...
		*/
		bool LogSummary()
		{
			...
			{
				UE_LOGF(LogEditorBuildPromotionTests, Display, "\nSummary:");
				for (int32 i = 0; i < SummaryLines.Num(); ++i)
				{
					UE_LOGF(LogEditorBuildPromotionTests, Display, "    %ls", *SummaryLines[i]);
				}
			}
			...
			if (SkippedTests.Num() > 0)
			{
				...
				{
					UE_LOGF(LogEditorBuildPromotionTests, Display, "    %ls", *SkippedTests[i]);
				}
			}
		...
		*/
		bool Geometry_LevelCreationAndSetup()
		{
			...
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Created an empty level");
			...
			if (DirectionalLight)
			{
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Placed a directional light");
			}
			else
			{
				UE_LOGF(LogEditorBuildPromotionTests, Error, "Failed to place directional light");
			}
		...
		*/
		bool Lighting_BuildLighting_Part1()
		{
			...
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Set the lighting quality to Production");
		...
		*/
		bool Lighting_BuildLighting_Part2()
		{
			if (!GUnrealEd->IsLightingBuildCurrentlyRunning())
			{
				...
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Built Lighting");
				return true;
			}
			return false;
		}
...
{ \
	...
	{ \
		...
		UE_LOGF(LogEditorBuildPromotionTests, Log, "No asset import path set for %ls", TEXT(#ImportSetting)); \
	} \
}
		...
		*/
		bool Workflow_ImportWorkflow()
		{
			...
			if (SurroundFilePath.Len() > 0)
			{
				...
				{
					UE_LOGF(LogEditorBuildPromotionTests, Error, "Failed to create surround sound asset at (%ls)", *SurroundFilePath);
				}
			}
			else
			{
			...
			for (int32 i = 0; i < AssetsToImport.Num(); ++i)
			{
				...
				if (FilePath.Len() > 0)
				{
					...
					if (FactoryClass)
					{
						...
						{
							UE_LOGF(LogEditorBuildPromotionTests, Warning, "Failed to create asset (%ls) with factory (%ls)", *FilePath, *FactoryClass->GetName());
						}
					}
					else
					{
						UE_LOGF(LogEditorBuildPromotionTests, Warning, "Couldn't find import factory to use on assset (%ls)", *FilePath);
					}
				} 
				else
				{
					UE_LOGF(LogEditorBuildPromotionTests, Log, "No asset import path set for OtherAssetsToImport.  Index: %i", i);
				}
			}
...
#undef IMPORT_ASSET_WITH_FACTORY
		...
		bool ContentBrowser_SourceControl_Part1()
		{
			...
			if (SourceControlMaterialPath.Len() > 0)
			{
				...
				if (SCTestMat)
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Display, "Opened the material editor for: %ls", *SCTestMat->GetName());
				...
				{
					UE_LOGF(LogEditorBuildPromotionTests, Error, "Failed to find material to modify for Content Browser tests.");
				}
			}
			else
			{
		...
		{			
			if (SCTestMat)
			{
				if (!AsyncHelper.IsDispatched())
				{
					...
					{
						UE_LOGF(LogEditorBuildPromotionTests, Warning, "Failed to check out '%ls'", *AsyncHelper.GetParameter());
						return true;
					}
				...
				if (AsyncHelper.IsDone())
				{
					...
					{
						UE_LOGF(LogEditorBuildPromotionTests, Error, "Could not retrieve state for file '%ls'", *AsyncHelper.GetParameter());
					}
					else
					{
						if (!SourceControlState->IsCheckedOut())
						{
							UE_LOGF(LogEditorBuildPromotionTests, Error, "Unexpected state following Check Out operation for file '%ls'", *AsyncHelper.GetParameter());
						}
						else
						{
							UE_LOGF(LogEditorBuildPromotionTests, Display, "Checked out the revision control material");
						}
					}
				}
		...
		*/
		bool ContentBrowser_SourceControl_Part3()
		{
			if (SCTestMat)
			{
				...
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Changing revision control test to %ls", *ChosenMaterialColor);
		...
		bool ContentBrowser_SourceControl_Part4()
		{
			if (SCTestMat)
			{
				if (!AsyncHelper.IsDispatched())
				{
					...
					{
						UE_LOGF(LogEditorBuildPromotionTests, Warning, "Failed to check in '%ls'", *AsyncHelper.GetParameter());
						return true;
					}
				...
				if (AsyncHelper.IsDone())
				{
					...
					else
					{
						...
						{
							UE_LOGF(LogEditorBuildPromotionTests, Error, "Unexpected state following Check In operation for file '%ls'", *AsyncHelper.GetParameter());
						}
						else
						{
							UE_LOGF(LogEditorBuildPromotionTests, Display, "Checked in the revision control material");
						}
					}
					return true;
				}
		...
		*/
		...
		{
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Closed the material editor");
			GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->CloseAllAssetEditors();
			return true;
		}
		...
		bool ContentBrowser_OpenAssets_Part1()
		{
			...
			if (AssetPackagePath.Len() > 0)
			{
				...
				else
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: BlueprintAsset not found");
				}
			}
			else
			{
				SkippedTests.Add(TEXT("ContentBrowser: Open Blueprint. (No file path)"));
				UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: BlueprintAsset setting is empty");
			}
			...
			if (AssetPackagePath.Len() > 0)
			{
				...
				else
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: MaterialAsset not found");
				}
			}
			else
			{
				SkippedTests.Add(TEXT("ContentBrowser: Open Material. (No file path)"));
				UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: MaterialAsset setting is empty");
			}
			...
			if (AssetPackagePath.Len() > 0)
			{
				...
				else
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: ParticleSystemAsset not found");
				}
			}
			else
			{
				SkippedTests.Add(TEXT("ContentBrowser: Open ParticleSystem. (No file path)"));
				UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: ParticleSystemAsset setting is empty");
			}
			...
			if (AssetPackagePath.Len() > 0)
			{
				...
				else
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: SkeletalMeshAsset not found");
				}
			}
			else
			{
				SkippedTests.Add(TEXT("ContentBrowser: Open SkeletalMesh. (No file path)"));
				UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: SkeletalMeshAsset setting is empty");
			}
			...
			if (AssetPackagePath.Len() > 0)
			{
				...
				else
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: StaticMeshAsset not found");
				}
			}
			else
			{
				SkippedTests.Add(TEXT("ContentBrowser: Open StaticMesh. (No file path)"));
				UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: StaticMeshAsset setting is empty");
			}
			...
			if (AssetPackagePath.Len() > 0)
			{
				...
				else
				{
					...
					UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: TextureAsset not found");
				}
			}
			else
			{
				SkippedTests.Add(TEXT("ContentBrowser: Open Texture. (No file path)"));
				UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping Asset: TextureAsset setting is empty");
			}
		...
		*/
		bool ContentBrowser_ReimportAsset()
		{
			if (ReimportMesh)
			{
				...
				{
					UE_LOGF(LogEditorBuildPromotionTests, Display, "Reimported asset %ls", *ReimportMesh->GetName());
				}
				else
				{
					UE_LOGF(LogEditorBuildPromotionTests, Error, "Failed to reimport asset %ls", *ReimportMesh->GetName());
				}
			}
			else
			{
		...
		*/
		bool ContentBrowser_AssignAMaterial()
		{
			...
			{
				UE_LOGF(LogEditorBuildPromotionTests, Warning, "Skipping material assignment test: no default material defined.");
				return true;
			}
			...
			if (!DefaultMaterial)
			{
				UE_LOGF(LogEditorBuildPromotionTests, Error, "Failed to load default material asset.");
				return false;
			}
			...
			{
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Successfully assigned material to static mesh.");
		...
		*/
		bool Blueprint_LevelScript_Part5()
		{
			if (BlueprintObject)
			{
				...
				GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->CloseAllAssetEditors();
				//UE_LOGF(LogEditorBuildPromotionTests, Display, "Saving the blueprint");
				//SaveBlueprint();
			}
			return true;
		}
		...
		*/
		bool Building_BuildLevel_Part1()
		{
			...
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Turning level visibility off");
		...
		*/
		bool  Building_BuildLevel_Part2()
		{
			...
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Taking a screenshot");
			FEditorPromotionTestUtilities::TakeScreenshot(TEXT("VisibilityOff"), FAutomationScreenshotOptions(EComparisonTolerance::Low));

			UE_LOGF(LogEditorBuildPromotionTests, Display, "Turning level visibility on");
		...
		*/
		bool  Building_BuildLevel_Part3()
		{
			...
			UE_LOGF(LogEditorBuildPromotionTests, Display, "Rebuilt the map");

			if (World->GetNavigationSystem())
			{
				// Invoke navmesh generator
				FNavigationSystem::Build(*World);
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Built navigation");
			}
		...
		*/
		bool Building_BuildLevel_Part4()
		{
			if (!GUnrealEd->IsLightingBuildCurrentlyRunning())
			{
				...
				UE_LOGF(LogEditorBuildPromotionTests, Display, "Saved the Level (EditorBuildPromotionTest)");
...
{
	UE_LOGF(LogEditorBuildPromotionTests, Display, "Ending PIE");
	FEditorPromotionTestUtilities::EndPIE();
	return true;
}
...
bool FPIEExecCommand::Update()
{
	...
	{
		UE_LOGF(LogEditorBuildPromotionTests, Error, "Tried to execute a PIE command but PIE is not running. (%ls)", *ExecCommand);
	}
	return true;
}
...
bool FBuildPromotionPIETest::RunTest(const FString& Parameters)
{
	...
	UE_LOGF(LogEditorBuildPromotionTests, Display, "Loading Map: %ls", *MapName);

	EditorBuildPromotionTestUtils::StartPIE(false);
	UE_LOGF(LogEditorBuildPromotionTests, Display, "Starting PIE");
	...
	if (AllWindows.Num() == 0)
	{
		UE_LOGF(LogEditorAutomationTests, Error, "ERROR: Could not find the main editor window.");
		UE_LOGF(LogEditorAutomationTests, Display, "Test FAILED");
		return true;
	}
	...
	UE_LOGF(LogEditorBuildPromotionTests, Display, "Toggling \"Stat Memory\" and taking a screenshot");
