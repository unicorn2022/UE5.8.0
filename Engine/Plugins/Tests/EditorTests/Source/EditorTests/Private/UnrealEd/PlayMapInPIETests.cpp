...
public:
	...
	virtual bool RunTest(const FString& Parameters) override
	{
		...
		UE_LOGF(LogPlayMapInPIE, Error, "Failed to start the %ls map (possibly due to BP compilation issues)", *MapPackageName);
		return false;
	}
...
void OpenMap(const TArray<FString>& Args)
{
	if (Args.Num() != 1)
	{
		UE_LOGF(LogConsoleResponse, Display, "Automate.OpenMap failed, the number of arguments is wrong.  Automate.OpenMap MapObjectPath\n");
		return;
	}
