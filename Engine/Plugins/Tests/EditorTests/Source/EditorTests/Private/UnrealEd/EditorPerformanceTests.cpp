...
{
	UE_LOGF(LogEditorAutomationTests, Log, "Begin generating the editor performance charts.");
	...
	UE_LOGF(LogEditorAutomationTests, Display, "AVG FPS: '%.1f'", AverageFPS);
	UE_LOGF(LogEditorAutomationTests, Display, "AVG Frame Time: '%.1f' ms", AverageFrameTime);
	UE_LOGF(LogEditorAutomationTests, Display, "AVG Used Physical Memory: '%.0f' kb", MemoryUsedPhysical);
	UE_LOGF(LogEditorAutomationTests, Display, "AVG Used Virtual Memory: '%.0f' kb", MemoryUsedVirtualAvg);
	UE_LOGF(LogEditorAutomationTests, Display, "Performance csv file is located here: %ls", *FPaths::ConvertRelativePathToFull(PerfCSVFilePath));
	UE_LOGF(LogEditorAutomationTests, Log, "Performance csv file is located here: %ls", *FPaths::ConvertRelativePathToFull(PerfCSVFilePath));
	UE_LOGF(LogEditorAutomationTests, Log, "Raw performance csv file is located here: %ls", *FPaths::ConvertRelativePathToFull(RAWCSVFilePath));
}
...
bool FMapPerformanceInEditor::RunTest(const FString& Parameters)
{
	...
	for (auto Iter = AutomationTestSettings->EditorPerformanceTestMaps.CreateConstIterator(); Iter; ++Iter)
	{
		if ( Iter->PerformanceTestmap.GetLongPackageName() == EditorPerformanceData.MapName )
		{
			...
			{
				UE_LOGF(LogEditorAutomationTests, Warning, "Please set the test timer for '%ls' in the automation preferences or engine.ini.", *EditorPerformanceData.MapName);
			}
			else
			if (EditorPerformanceData.TestDuration < 0)
			{
				UE_LOGF(LogEditorAutomationTests, Error, "Test timer preference option for '%ls' is less than 0.", *EditorPerformanceData.MapName);
				return false;
			}
		}
	}

	UE_LOGF(LogEditorAutomationTests, Log, "Running the performance capture test for %.0i seconds on %ls", EditorPerformanceData.TestDuration, *EditorPerformanceData.MapName);
