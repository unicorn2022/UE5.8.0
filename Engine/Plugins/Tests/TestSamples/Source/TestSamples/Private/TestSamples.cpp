...
void FTestSamplesModule::OnTestStart(FAutomationTestBase* Test)
{
	if (Test != nullptr)
	{
		UE_LOGF(LogTestSamples, Verbose, "Starting test: %ls", *Test->GetTestFullName())
	}
}
void FTestSamplesModule::OnTestEnd(FAutomationTestBase* Test)
{
	if (Test != nullptr)
	{
		UE_LOGF(LogTestSamples, Verbose, "Ending test: %ls", *Test->GetTestFullName())
	}
...
{
	UE_LOGF(LogTestSamples, Verbose, "Start running tests");
}

void FTestSamplesModule::OnAfterAllTests()
{
	UE_LOGF(LogTestSamples, Verbose, "Running tests completed");
}

void FTestSamplesModule::OnEnteringTestSection(const FString& Section)
{
	UE_LOGF(LogTestSamples, Verbose, "Entering section %ls", *Section);
}

void FTestSamplesModule::OnLeavingTestSection(const FString& Section)
{
	UE_LOGF(LogTestSamples, Verbose, "Leaving section %ls", *Section);
}
