...
void UAudioParameterComponent::SetTriggerParameter(FName InName)
{
	...
	UE_LOGF(LogAudio, Warning, "Trigger parameter not supported for UAudioParameterComponent");
}
...
void UAudioParameterComponent::LogParameter(FAudioParameter& InParam)
{
	...
	UE_LOGF(LogAudioParameterComponent, VeryVerbose, "%ls: Set %ls %ls to %ls", *Owner->GetName(), *ParamTypeString, *InParam.ParamName.ToString(), *ParamValueString)
}
