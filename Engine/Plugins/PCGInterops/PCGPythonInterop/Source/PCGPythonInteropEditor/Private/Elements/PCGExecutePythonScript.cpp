...
bool FPCGExecutePythonScriptElement::ExecuteInternal(FPCGContext* InContext) const
{
	...
	TStringBuilder<0> CommandBuilder;
	...
	PythonSource.Command = CommandBuilder;
		...
		for (const FPythonLogOutputEntry& LogOutputEntry : PythonSource.LogOutput)
		{
			switch (LogOutputEntry.Type)
			{
				case EPythonLogOutputType::Info:
					UE_LOGF(LogPCGPython, Log, "Python [Log]: [%ls]", *LogOutputEntry.Output);
					break;
				case EPythonLogOutputType::Warning:
					UE_LOGF(LogPCGPython, Warning, "Python [Warning]: [%ls]", *LogOutputEntry.Output);
					break;
				case EPythonLogOutputType::Error:
					UE_LOGF(LogPCGPython, Error, "Python [Error]: [%ls]", *LogOutputEntry.Output);
					break;
			}
		}
	}
...
#else // WITH_EDITOR
	PCGLog::LogErrorOnGraph(LOCTEXT("NodeIsEditorOnly", "The Execute Python Script node is currently Editor-only and should not be used at runtime."), InContext);
#endif // WITH_EDITOR
