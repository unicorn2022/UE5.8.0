	...
	bool SampleOnePointPerTriangle(const UPCGMeshSamplerSettings* Settings, FPCGMeshSamplerContext* Context, FPCGMeshSamplerContext::SetPointDensityFunc SetPointDensityPtr)
	{
		...
		auto IterationBody = [Context, Settings, SetPointDensityPtr](int32 StartReadIndex, int32 StartWriteIndex, int32 Count)
		{
			...
			for (int32 ReadIndex = StartReadIndex; ReadIndex < StartReadIndex + Count; ++ReadIndex)
			{
				...
				Context->SetPointColorAndDensity(SetPointDensityPtr, TriangleId, BarycentricCoord, OutPoint, DataIndex, Settings->bHasDefaultColorOverride ? Settings->DefaultColorOverride : TOptional<FLinearColor>{});
			...
			for (int32 DataIndex = 0; DataIndex < Context->DynamicMeshes.Num(); ++DataIndex)
			{
				...
				auto SamplingFuture = [Settings, Context, Seed = Context->GetSeed(), SetPointDensityPtr, DataIndex]() -> bool
				{
					...
					for (int32 i = 0; i < PointSampling.Samples.Num(); ++i)
					{
						...
						Context->SetPointColorAndDensity(SetPointDensityPtr, TriangleId, BarycentricCoords, OutPoint, DataIndex, Settings->bHasDefaultColorOverride ? Settings->DefaultColorOverride : TOptional<FLinearColor>{});
...
void FPCGMeshSamplerContext::SetPointColorAndDensity(SetPointDensityFunc SetPointDensityFuncPtr, int32 TriangleId, const FVector& BarycentricCoord, FPCGPoint& OutPoint, int32 DataIndex, TOptional<FLinearColor> DefaultColor)
{
	FLinearColor Color;
	bool bValidVertexColor = false;
	UGeometryScriptLibrary_MeshQueryFunctions::GetInterpolatedTriangleVertexColor(DynamicMeshes[DataIndex], TriangleId, BarycentricCoord, DefaultColor ? DefaultColor.GetValue() : FLinearColor::White, bValidVertexColor, Color);
	OutPoint.Color = Color;
...
#if WITH_EDITOR
	if (GetLinkerCustomVersion(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::PCGMeshSamplerDefaultColorPCGSubdivideSplineBoundsFixes)
	{
		// For all previous nodes, we'll force this option to false. for retro-compatibility
		bHasDefaultColorOverride = false;
	}
#endif // WITH_EDITOR
}
...
{
	if (Mesh.IsNull() || IsPropertyOverriddenByPin(GET_MEMBER_NAME_CHECKED(UPCGMeshSamplerSettings, Mesh)))
	{
		return;
	}
...
bool FPCGMeshSamplerElement::PrepareDataInternal(FPCGContext* InContext) const
{
	...
	{
		const bool bNeedToWait = !Context->InitializeAndRequestLoad(PCGPinConstants::DefaultInputLabel, Settings->InputSource, { Settings->Mesh.ToSoftObjectPath() }, /*bPersistAllData=*/false, /*bSilenceErrorOnEmptyObjectPath=*/true, Settings->bSynchronousLoad);
	...
	for (const auto& [Path, DummyIndex, DummyIndex2] : Context->PathsToObjectsAndDataIndex)
	{
		...
		// For tracking purposes, keep the original object
		const UObject* OriginalObject = Object;

		// Check if the object is a Blueprint of type Actor, if so, extract the class to be read just after
		if (const UBlueprint* Blueprint = Cast<const UBlueprint>(Object))
		{
			if (UClass* BlueprintClass = Blueprint->GeneratedClass.Get(); BlueprintClass && BlueprintClass->IsChildOf<AActor>())
			{
				Object = BlueprintClass;
			}
		}
		...
		// Check if the UObject is a class of an actor. If so, find the all scene components
		if (UClass* Class = Cast<UClass>(Object); Class && Class->IsChildOf<AActor>())
		{
			AActor::ForEachComponentOfActorClassDefault<USceneComponent>(Class, [&Candidates](const USceneComponent* Component)
				{ 
					Candidates.Add(const_cast<USceneComponent*>(Component));
					return true; // Continue until we find them all
				});
		}
		else if (AActor* Actor = Cast<AActor>(Object))
		{
			Actor->ForEachComponent<USceneComponent>(/*bIncludeFromChildActor=*/ false, [&Candidates](const USceneComponent* Component)
				{
					Candidates.Add(const_cast<USceneComponent*>(Component));
				});
		}
		...
		bool bDynMeshAdded = false;

		// @todo_pcg: Better support of multiple candidates. For now, take the first valid one.
		for (UObject* Candidate : Candidates)
		{
			if (USceneComponent* SceneComponent = Cast<USceneComponent>(Candidate))
			{
...
#if WITH_EDITOR
					// Track the scene component if is it different from the original object. The original object is tracked outside of this loop.
					if (OriginalObject != SceneComponent)
					{
			...
			else if (Object->IsA<UStaticMesh>() || Object->IsA<USkeletalMesh>())
			{
				...
				{
					UGeometryScriptLibrary_StaticMeshFunctions::CopyMeshFromStaticMeshV2(StaticMesh, Context->DynamicMeshes.Last(), Options, MeshReadLOD, Outcome, /*bUseSectionMaterials=*/false, Debug);
					
					if (Outcome == EGeometryScriptOutcomePins::Success && Settings->bOutputMaterialInfo)
					{
						TArray<FName> MaterialSlotNames;
						UGeometryScriptLibrary_StaticMeshFunctions::GetMaterialListFromStaticMesh(StaticMesh, Context->AssetMaterialList.Emplace_GetRef(), MaterialSlotNames, Debug);
					}
				}
				else
				{
					...
					UGeometryScriptLibrary_StaticMeshFunctions::CopyMeshFromSkeletalMesh(SkeletalMesh, Context->DynamicMeshes.Last(), Options, MeshReadLOD, Outcome, Debug);
					if (Outcome == EGeometryScriptOutcomePins::Success && Settings->bOutputMaterialInfo)
					{
						TArray<FName> MaterialSlotNames;
						UGeometryScriptLibrary_StaticMeshFunctions::GetMaterialListFromSkeletalMesh(SkeletalMesh, Context->AssetMaterialList.Emplace_GetRef(), MaterialSlotNames, Debug);
					}
				}
			}
		...
		if (Outcome != EGeometryScriptOutcomePins::Success)
		{
			PCGLog::LogErrorOnGraph(FText::Format(LOCTEXT("ObjectToDynamicMeshFailed", "Object to Dynamic mesh failed for object {0}. Either it is not a supported type (StaticMesh/SkeletalMesh/Actor/SceneComponent) or there was a conversion issue."), FText::FromString(Path.ToString())));
		...
		{
#if WITH_EDITOR
			// Track the object, only if the extraction worked
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		case EPCGMeshSamplingMethod::OnePointPerVertex:
		{
			bool bHasValidColors, Dummy, Dummy2;
			...
			UGeometryScriptLibrary_MeshVertexColorFunctions::GetMeshPerVertexColors(DynamicMesh, Context->Colors[i], /*bIsValidColorSet=*/bHasValidColors, /*bHasVertexIDGaps=*/Dummy2);
			...
			{
				for (FLinearColor& Color : *(Context->Colors[i].List))
				{
					Color = Settings->DefaultColorOverride;
				}
			}
