...
public:
	/** 
	* Can provide a list of inputs to sample the meshes from. It can be a list of StaticMeshes/SkeletalMeshes, a list of Actors that have a Scene Component (like a Static Mesh Component), or a list of Scene Components directly.
	...
	* Each entry (either in the same data or separate data) will produce a unique output data.
	*/
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Settings")
	bool bExtractMeshFromInput = false;

	/** Soft Object Path to the mesh to sample from. Supports Static Meshes and Skeletal Meshes. Will be loaded. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = Settings, meta = (PCG_Overridable, PCG_OverrideAliases="StaticMesh", AllowedClasses="/Script/Engine.StaticMesh, /Script/Engine.SkeletalMesh", EditCondition = "!bExtractMeshFromInput", EditConditionHides, ShowAfter = "bExtractMeshFromInput"))
	 ...
	 * increasing its influence over the density from the point's center up to two times the bounds. At 1, it will
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void SetPointColorAndDensity(SetPointDensityFunc SetPointDensityFuncPtr, int32 TriangleId, const FVector& BarycentricCoord, FPCGPoint& OutPoint, int32 DataIndex, TOptional<FLinearColor> DefaultColor);
