...
static FString DetectSubversionPath()
{
	...
	if (!SVNPath.IsEmpty())
	{
		...
		{
			UE_LOGF(LogSourceControl, Log, "Using user-supplied path %ls for svn operations", *FPaths::ConvertRelativePathToFull(SVNPath));
			return SVNPath;
		}
		
		UE_LOGF(LogSourceControl, Log, "Specified svn executable (%ls) does not exist. Falling back to default behaviour.", *SVNPath);
	}
	...
	if (!bPathIsValid)
	{
		UE_LOGF(LogSourceControl, Log, "Unable to detect system-level svn binary.");
		SVNPath = DefaultPath;
	}

	UE_LOGF(LogSourceControl, Log, "Using path %ls for svn operations", *FPaths::ConvertRelativePathToFull(SVNPath));
	return SVNPath;
}
...
static bool RunCommandInternal(const FString& InCommand, const TArray<FString>& InFiles, const TArray<FString>& InParameters, FString& OutResults, TArray<FString>& OutErrorMessages, const FString& UserName, const FString& Password)
{
	...
	UE_LOGF(LogSourceControl, Log, "Attempting \"svn %ls --password ********\"", *FullCommand);
...
bool CheckFilename(const FString& InString)
{
	if( InString.Contains(TEXT("...")) ||
		...
		InString.Contains(TEXT("?")))
	{
		UE_LOGF(LogSourceControl, Warning, "Filename '%ls' with wildcards is not supported by Subversion", *InString);
		return false;
	}
