...
static bool _StartBackgroundPlasticShell(const FString& InPathToPlasticBinary, const FString& InWorkingDirectory)
{
	...
	{
		UE_LOGF(LogSourceControl, Warning, "Failed to launch 'cm shell'"); // not a bug, just no Unity Version Control cli found
		_CleanupBackgroundCommandLineShell();
	}
	else
	{
		const double ElapsedTime = (FPlatformTime::Seconds() - StartTimestamp);
		UE_LOGF(LogSourceControl, Verbose, "_StartBackgroundPlasticShell: '%ls %ls' ok (in %.3lfs, handle %lld)", *InPathToPlasticBinary, *FullCommand, ElapsedTime, uint64(ShellProcessHandle.Get()));
...
static void _ExitBackgroundCommandLineShell(const bool bInForceExit = false)
{
	...
	if (ShellProcessHandle.IsValid())
	{
		if (FPlatformProcess::IsProcRunning(ShellProcessHandle))
		{
			if (bInForceExit)
			{
				UE_LOGF(LogSourceControl, Verbose, "_ExitBackgroundCommandLineShell: TerminateProc");
				FPlatformProcess::TerminateProc(ShellProcessHandle);
			}
			else
			{
				// Tell the 'cm shell' to exit
				UE_LOGF(LogSourceControl, Verbose, "_ExitBackgroundCommandLineShell: exit...");
				...
				while (FPlatformProcess::IsProcRunning(ShellProcessHandle))
				{
					...
					{
						UE_LOGF(LogSourceControl, Warning, "_ExitBackgroundCommandLineShell: cm shell didn't stop gracefully in %lfs.", Timeout);
						FPlatformProcess::TerminateProc(ShellProcessHandle);
						break;
					}
		...
		{
			UE_LOGF(LogSourceControl, Verbose, "_ExitBackgroundCommandLineShell: 'cm shell' already stopped");
		}
...
static bool _RunCommandInternal(const FString& InCommand, const TArray<FString>& InParameters, const TArray<FString>& InFiles, FString& OutResults, FString& OutErrors)
{
	...
	if (!FPlatformProcess::IsProcRunning(ShellProcessHandle))
	{
		UE_LOGF(LogSourceControl, Warning, "RunCommand: 'cm shell' has stopped. Restarting!");
	...
	UE_LOGF(LogSourceControl, Verbose, "RunCommand: '%ls' (%d chars, %d files)", *LoggableCommand, FullCommand.Len()+1, InFiles.Num());
	...
	while (FPlatformProcess::IsProcRunning(ShellProcessHandle))
	{
		...
		else if ((FPlatformTime::Seconds() - LastLog > LogInterval) && (PreviousLogLen < OutResults.Len()))
		{
			...
			UE_LOGF(LogSourceControl, Log, "RunCommand: '%ls' in progress for %.3lfs... (%d chars):\n%ls", *InCommand, (FPlatformTime::Seconds() - StartTimestamp), OutResults.Len() - PreviousLogLen, *OutResults.Mid(PreviousLogLen));
		...
		else if (FPlatformTime::Seconds() - LastActivity > Timeout)
		{
			...
			UE_LOGF(LogSourceControl, Error, "RunCommand: '%ls' TIMEOUT after %.3lfs output (%d chars):\n%ls", *InCommand, (FPlatformTime::Seconds() - StartTimestamp), OutResults.Len(), *OutResults.Mid(PreviousLogLen));
			_RestartBackgroundCommandLineShell(true);
		...
		{
			UE_LOGF(LogSourceControl, Warning, "RunCommand: '%ls' Engine Exit was requested after %.3lfs output (%d chars):\n%ls", *InCommand, (FPlatformTime::Seconds() - StartTimestamp), OutResults.Len() - PreviousLogLen, *OutResults.Mid(PreviousLogLen));
	...
	if (!InCommand.Equals(TEXT("exit")))
	{
		if (!FPlatformProcess::IsProcRunning(ShellProcessHandle))
		{
			...
			UE_LOGF(LogSourceControl, Error, "RunCommand: '%ls' 'cm shell' stopped after %.3lfs output (%d chars):\n%ls", *LoggableCommand, ElapsedTime, OutResults.Len(), *OutResults.Left(4096)); // Limit result size to 4096 characters
		}
		else if (!bResult)
		{
			UE_LOGF(LogSourceControl, Warning, "RunCommand: '%ls' (in %.3lfs) output (%d chars):\n%ls", *LoggableCommand, ElapsedTime, OutResults.Len(), *OutResults.Left(4096)); // Limit result size to 4096 characters
		}
		else
		{
			if (PreviousLogLen > 0)
			{
				UE_LOGF(LogSourceControl, Log, "RunCommand: '%ls' (in %.3lfs) output (%d chars):\n%ls", *LoggableCommand, ElapsedTime, OutResults.Len(), *OutResults.Mid(PreviousLogLen).Left(4096)); // Limit result size to 4096 characters
			}
			else
			{
				if (OutResults.Len() <= 200) // Limit result size to 200 characters
				{
					UE_LOGF(LogSourceControl, Log, "RunCommand: '%ls' (in %.3lfs) output (%d chars):\n%ls", *LoggableCommand, ElapsedTime, OutResults.Len(), *OutResults);
				}
				else
				{
					UE_LOGF(LogSourceControl, Log, "RunCommand: '%ls' (in %.3lfs) (output %d chars not displayed)", *LoggableCommand, ElapsedTime, OutResults.Len());
					UE_LOGF(LogSourceControl, Verbose, "\n%ls", *OutResults.Left(4096));; // Limit result size to 4096 characters
				}
			}
		}
	}
	...
	ShellCumulatedTime += ElapsedTime;
	UE_LOGF(LogSourceControl, Verbose, "RunCommand: cumulated time spent in shell: %.3lfs (count %zu)", ShellCumulatedTime, ShellCommandCounter);
