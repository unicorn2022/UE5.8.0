...
static bool OpenDesktopApplication(const FString& InCommandLineArguments)
{
	...
	UE_LOGF(LogSourceControl, Log, "Opening the Desktop application (%ls %ls)", *DesktopAppPath, *InCommandLineArguments);
	...
	if (!Proc.IsValid())
	{
		UE_LOGF(LogSourceControl, Error, "Opening the Desktop application (%ls %ls) failed.", *DesktopAppPath, *InCommandLineArguments);
...
static bool RunStatus(const FString& InDir, TArray<FString>&& InFiles, const EStatusSearchType InSearchType, TArray<FString>& OutErrorMessages, TArray<FPlasticSourceControlState>& OutStates, int32& OutChangeset)
{
	...
	if (bResult)
	{
		...
		if (bWholeDirectory)
		{
			...
			UE_LOGF(LogSourceControl, Verbose, "RunStatus(%ls): 1) special case for status of a directory:", *InDir);
		...
		else
		{
			...
			UE_LOGF(LogSourceControl, Verbose, "RunStatus(%ls...): 2) general case for %d file(s) in a directory (%ls)", *InFiles[0], InFiles.Num(), *InDir);
...
static bool RunCheckMergeStatus(const TArray<FString>& InFiles, TArray<FString>& OutErrorMessages, TArray<FPlasticSourceControlState>& OutStates)
{
	...
	if (FPaths::FileExists(MergeProgressFilename))
	{
		...
		{
			UE_LOGF(LogSourceControl, Verbose, "RunCheckMergeStatus: %ls:\n%ls", *MergeProgressFilename, *MergeProgressContent);
			...
			if (MergeFromIndex > INDEX_NONE)
			{
				...
				if (MergeType.FindChar(TEXT(' '), SpaceBeforeChangesetIndex))
				{
					...
					for (const FString& Result : Results)
					{
						...
						UE_LOGF(LogSourceControl, Log, "MergeConflict.Filename: '%ls'", *MergeConflict.Filename);
						for (FPlasticSourceControlState& State : OutStates)
						{
							UE_LOGF(LogSourceControl, Log, "State.LocalFilename: '%ls'", *State.LocalFilename);
							if (State.LocalFilename.EndsWith(MergeConflict.Filename, ESearchCase::CaseSensitive))
							{
								UE_LOGF(LogSourceControl, Verbose, "MergeConflict '%ls' found Base cs:%ls From cs:%ls", *MergeConflict.Filename, *MergeConflict.BaseChangeset, *MergeConflict.SourceChangeset);
...
bool RunUpdateStatus(const TArray<FString>& InFiles, const EStatusSearchType InSearchType, const bool bInUpdateHistory, TArray<FString>& OutErrorMessages, TArray<FPlasticSourceControlState>& OutStates, int32& OutChangeset)
{
	...
	for (const FString& File : InFiles)
	{
		...
		{
			UE_LOGF(LogSourceControl, Verbose, "%ls is out of the Workspace", *File);
			continue;
		}
	...
	if (InFiles.Num() > 0)
	{
		UE_LOGF(LogSourceControl, Verbose, "RunUpdateStatus: %d file(s)/%d directory(ies) ('%ls'...)", InFiles.Num(), GroupOfFiles.Num(), *InFiles[0]);
	}
	else
	{
		UE_LOGF(LogSourceControl, Warning, "RunUpdateStatus: NO file");
	}
