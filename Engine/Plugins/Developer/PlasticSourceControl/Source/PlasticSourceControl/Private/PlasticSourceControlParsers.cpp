...
static EWorkspaceState StateFromStatus(const FString& InFileStatus, const bool bInUsesCheckedOutChanged)
{
	...
	{
		UE_LOGF(LogSourceControl, Warning, "Unknown file status '%ls'", *InFileStatus);
...
static FPlasticSourceControlState StateFromStatusResult(const FString& InResult, const bool bInUsesCheckedOutChanged)
{
	...
	UE_LOGF(LogSourceControl, Warning, "%ls", *InResult);
...
void ParseFileStatusResult(TArray<FString>&& InFiles, const TArray<FString>& InResults, TArray<FPlasticSourceControlState>& OutStates)
{
	...
	for (FString& InFile : InFiles)
	{
		...
		if (OutStates.Num() < 20)
		{
			UE_LOGF(LogSourceControl, Verbose, "%ls = %d:%ls", *File, static_cast<uint32>(FileState.WorkspaceState), FileState.ToString());
		}
	...
	if (OutStates.Num() > 20)
	{
		UE_LOGF(LogSourceControl, Verbose, "[...] %d more files", OutStates.Num() - 20);
	}
}
...
void ParseDirectoryStatusResult(const FString& InDir, const TArray<FString>& InResults, TArray<FPlasticSourceControlState>& OutStates)
{
	...
	for (const FString& InResult : InResults)
	{
		...
		{
			UE_LOGF(LogSourceControl, Verbose, "%ls = %d:%ls", *FileState.LocalFilename, static_cast<uint32>(FileState.WorkspaceState), FileState.ToString());
...
void ParseFileinfoResults(const TArray<FString>& InResults, TArray<FPlasticSourceControlState>& InOutStates)
{
	...
	for (int32 IdxResult = 0; IdxResult < InResults.Num(); IdxResult++)
	{
		...
		if (IdxResult < 20)
		{
			UE_LOGF(LogSourceControl, Verbose, "%ls: %d;%d %ls by '%ls' (%ls)", *File, FileState.LocalRevisionChangeset, FileState.DepotRevisionChangeset, *FileState.RepSpec, *FileState.LockedBy, *FileState.LockedWhere);
		}
	}
	// debug log (if too many files)
	if (InResults.Num() > 20)
	{
		UE_LOGF(LogSourceControl, Verbose, "[...] %d more files", InResults.Num() - 20);
	}
}
...
bool ParseHistoryResults(const bool bInUpdateHistory, const FString& InXmlFilename, TArray<FPlasticSourceControlState>& InOutStates)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseHistoryResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
bool ParseUpdateResults(const FString& InResults, TArray<FString>& OutFiles)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseUpdateResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
bool ParseChangelistsResults(const FString& InXmlFilename, TArray<FPlasticSourceControlChangelistState>& OutChangelistsStates, TArray<TArray<FPlasticSourceControlState>>& OutCLFilesStates)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseChangelistsResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
EWorkspaceState ParseShelveFileStatus(const TCHAR InFileStatus)
{
	...
	{
		UE_LOGF(LogSourceControl, Warning, "Unknown file status '%c'", InFileStatus);
...
bool ParseShelvesResults(const FString& InResults, TArray<FPlasticSourceControlChangelistState>& InOutChangelistsStates)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseShelvesResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
bool ParseShelvesResult(const FString& InResults, FString& OutComment, FDateTime& OutDate, FString& OutOwner)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseShelvesResult: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
bool ParseChangesetsResults(const FString& InXmlFilename, TArray<FPlasticSourceControlChangesetRef>& OutChangesets)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseChangesetesResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
bool ParseLogResults(const FString& InXmlFilename, const FPlasticSourceControlChangesetRef& InChangeset, TArray<FPlasticSourceControlStateRef>& OutFiles)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseLogResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
bool ParseBranchesResults(const FString& InXmlFilename, TArray<FPlasticSourceControlBranchRef>& OutBranches)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseBranchesResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
...
bool ParseMergeResults(const FString& InResult, TArray<FString>& OutFiles)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "ParseMergeResults: XML parse error '%ls'", *XmlFile.GetLastError())
	}
