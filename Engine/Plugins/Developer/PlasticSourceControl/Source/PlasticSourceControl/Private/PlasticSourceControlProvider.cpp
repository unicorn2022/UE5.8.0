...
void FPlasticSourceControlProvider::Init(bool bForceConnection)
{
	...
	if (!bPlasticAvailable)
	{
		...
		if (Plugin.IsValid())
		{
			...
			UE_LOGF(LogSourceControl, Log, "Unity Version Control (formerly Plastic SCM) plugin %ls", *PluginVersion);
		}
...
ECommandResult::Type FPlasticSourceControlProvider::GetState(const TArray<FString>& InFiles, TArray< TSharedRef<ISourceControlState, ESPMode::ThreadSafe> >& OutState, EStateCacheUsage::Type InStateCacheUsage)
{
	...
	{
		UE_LOGF(LogSourceControl, Log, "GetState: ForceUpdate");
	...
	ECommandResult::Type FPlasticSourceControlProvider::Execute(const FSourceControlOperationRef& InOperation, FSourceControlChangelistPtr InChangelist, const TArray<FString>& InFiles, EConcurrency::Type InConcurrency, const FSourceControlOperationComplete& InOperationCompleteDelegate)
{
	...
	{
		UE_LOGF(LogSourceControl, Warning, "'%ls': only Connect operation allowed without a workspace", *InOperation->GetName().ToString());
	...
	if (InConcurrency == EConcurrency::Synchronous)
	{
		...
		UE_LOGF(LogSourceControl, Log, "ExecuteSynchronousCommand: %ls", *InOperation->GetName().ToString());
	...
	else
	{
		...
		UE_LOGF(LogSourceControl, Log, "IssueAsynchronousCommand: %ls", *InOperation->GetName().ToString());
...
bool FPlasticSourceControlProvider::UsesSoftRevertOnDelete() const
{
	return false;
}
...
TOptional<bool> FPlasticSourceControlProvider::HasChangesToSync() const
{
	return TOptional<bool>(); // NOTE: used by code in UE5's Status Bar but currently dormant as far as I can tell
}

TOptional<bool> FPlasticSourceControlProvider::HasChangesToCheckIn() const
{
	return TOptional<bool>(); // NOTE: used by code in UE5's Status Bar but currently dormant as far as I can tell
}
...
void FPlasticSourceControlProvider::Tick()
{
	...
	for (int32 CommandIndex = 0; CommandIndex < CommandQueue.Num(); ++CommandIndex)
	{
		...
		if (Command.bExecuteProcessed)
		{
			...
			{
				UE_LOGF(LogSourceControl, Log, "%ls of %d items processed in %.3lfs", *Command.Operation->GetName().ToString(), Command.Files.Num(), (FPlatformTime::Seconds() - Command.StartTimestamp));
			}
			else if (Command.Files.Num() == 1)
			{
				UE_LOGF(LogSourceControl, Log, "%ls of %ls processed in %.3lfs", *Command.Operation->GetName().ToString(), *Command.Files[0], (FPlatformTime::Seconds() - Command.StartTimestamp));
			}
			else
			{
				UE_LOGF(LogSourceControl, Log, "%ls processed in %.3lfs", *Command.Operation->GetName().ToString(), (FPlatformTime::Seconds() - Command.StartTimestamp));
			}
...
ECommandResult::Type FPlasticSourceControlProvider::ExecuteSynchronousCommand(FPlasticSourceControlCommand& InCommand, const FText& Task)
{
	...
	// Display the progress dialog if a string was provided
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "Command '%ls' Failed!", *InCommand.Operation->GetName().ToString());
		}
	}
