...
bool DeleteChangelist(const FPlasticSourceControlProvider& PlasticSourceControlProvider, const FPlasticSourceControlChangelist& InChangelist, TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	...
	else
	{
		...
		UE_LOGF(LogSourceControl, Verbose, "DeleteChangelist(%ls)", *InChangelist.GetName());
		return PlasticSourceControlUtils::RunCommand(TEXT("changelist"), Parameters, Files, OutResults, OutErrorMessages);
	}
}
...
bool FPlasticCheckInWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	if (Files.Num() == 0)
	{
		UE_LOGF(LogSourceControl, Warning, "Checkin: No files provided");
		return false;
	}
	...
	if (FilesToAdd.Num() > 0)
	{
		UE_LOGF(LogSourceControl, Log, "CheckIn: Adding %d file(s) to source control", FilesToAdd.Num());
		if (!GetProvider().IsPartialWorkspace())
		{
	...
	UE_LOGF(LogSourceControl, Verbose, "CheckIn: %d file(s) Description: '%ls'", Files.Num(), *Description.ToString());
	...
	if (!CommitMsgFile.GetFilename().IsEmpty())
	{
		...
		if (InCommand.bCommandSuccessful)
		{
			...
			UE_LOGF(LogSourceControl, Log, "CheckIn successful");
		}
...
bool FPlasticMarkForAddWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	{
		UE_LOGF(LogSourceControl, Warning, "MarkforAdd: No files provided");
	}
...
bool FPlasticRevertWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	while (i < Files.Num())
	{
		...
		if (!State->CanRevert())
		{
			...
			UE_LOGF(LogSourceControl, Log, "File '%ls' cannot be reverted", *File);
			Files.RemoveAt(i);
			continue;
		}
		i++;
...
static TArray<FString> GetFilesCheckedOut(FPlasticSourceControlProvider& InSourceControlProvider, const TArray<FString>& InFiles)
{
	...
	for (const auto& File : InFiles)
	{
		...
		{
			UE_LOGF(LogSourceControl, Warning, "GetFilesCheckedOut: File '%ls' is not checked out and cannot be moved to another changelist", *File);
		}
	}
...
bool FPlasticUpdateStatusWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	UE_LOGF(LogSourceControl, Log, "status of %d items (ShouldUpdateHistory=%d, ShouldGetOpenedOnly=%d, ShouldUpdateModifiedState=%d)",
	...
	if (Files.Num() > 0)
	{
		...
		if (!InCommand.bCommandSuccessful)
		{
			...
			UE_LOGF(LogSourceControl, Warning, "Error on 'status', execute a 'checkconnection' to test the connection to the server");
...
bool FPlasticCopyWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	if (InCommand.Files.Num() == 1)
	{
		...
		if (bIsMoveOperation)
		{
			UE_LOGF(LogSourceControl, Log, "Moving %ls to %ls...", *Origin, *Destination);
	...
	{
		UE_LOGF(LogSourceControl, Error, "Copy is working for one file only: %d provided!", InCommand.Files.Num());
	}
...
bool FPlasticResolveWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	for (const FString& File : InCommand.Files)
	{
		...
		UE_LOGF(LogSourceControl, Log, "resolve %ls", *State->PendingResolveInfo.BaseFile);
...
static FPlasticSourceControlChangelist CreatePendingChangelist(FPlasticSourceControlProvider& PlasticSourceControlProvider, const FString& InDescription, TArray<FString>& InInfoMessages, TArray<FString>& InErrorMessages)
{
	...
	else
	{
		...
		Parameters.Add(TEXT("--persistent")); // Create a persistent changelist to stay close to Perforce behavior
		UE_LOGF(LogSourceControl, Verbose, "CreatePendingChangelist(%ls):\n\"%ls\"", *NewChangelist.GetName(), *InDescription);
		bCommandSuccessful = PlasticSourceControlUtils::RunCommand(TEXT("changelist"), Parameters, TArray<FString>(), InInfoMessages, InErrorMessages);
	}
...
static bool EditChangelistDescription(const FPlasticSourceControlProvider& PlasticSourceControlProvider, const FPlasticSourceControlChangelist& InChangelist, const FString& InDescription, TArray<FString>& InInfoMessages, TArray<FString>& InErrorMessages)
{
	...
	else
	{
		...
		const FScopedTempFile ChangelistDescriptionFile(InDescription);
		Parameters.Add(FString::Printf(TEXT("--descriptionfile=\"%s\""), *ChangelistDescriptionFile.GetFilename()));
		UE_LOGF(LogSourceControl, Verbose, "EditChangelistDescription(%ls\n%ls)", *InChangelist.GetName(), *InDescription);
		return PlasticSourceControlUtils::RunCommand(TEXT("changelist"), Parameters, TArray<FString>(), InInfoMessages, InErrorMessages);
	}
}
...
static bool MoveFilesToChangelist(const FPlasticSourceControlProvider& InSourceControlProvider, const FPlasticSourceControlChangelist& InChangelist, const TArray<FString>& InFiles, TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	if (InFiles.Num() > 0)
	{
		...
		else
		{
			...
			Parameters.Add(FString::Printf(TEXT("--namefile=\"%s\""), *ChangelistNameFile.GetFilename()));
			Parameters.Add(TEXT("add"));
			UE_LOGF(LogSourceControl, Verbose, "MoveFilesToChangelist: %ls", *InChangelist.GetName());
			return PlasticSourceControlUtils::RunCommand(TEXT("changelist"), Parameters, InFiles, OutResults, OutErrorMessages);
		}
	}
	return true;
...
bool FPlasticNewChangelistWorker::Execute(class FPlasticSourceControlCommand& InCommand)
{
	...
	if (NewChangelist.IsInitialized())
	{
		...
		if (InCommand.Files.Num() > 0)
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "No file can be moved.");
...
bool FPlasticShelveWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	if (InCommand.Changelist.IsInitialized())
	{
		...
		for (FString& FileToShelve : FilesToShelve)
		{
			if (!ChangelistState->Files.ContainsByPredicate([&FileToShelve](const FSourceControlStateRef& FileState) { return FileToShelve == FileState->GetFilename(); }))
			{
				...
				UE_LOGF(LogSourceControl, Error, "The file /%ls is not in the changelist anymore, so the shelve cannot be updated. Unshelve the corresponding change and retry.", *FileToShelve);
				InCommand.bCommandSuccessful = false;
			}
		}
	}
	...
	if (InCommand.bCommandSuccessful)
	{
		...
		while (i < FilesToShelve.Num())
		{
			...
			{
				FPaths::MakePathRelativeTo(FileToShelve, *FPaths::ProjectDir());
				UE_LOGF(LogSourceControl, Warning, "The file /%ls is unchanged, it cannot be shelved.", *FileToShelve);
		...
		{
			UE_LOGF(LogSourceControl, Error, "No file to Shelve");
			InCommand.bCommandSuccessful = false;
		}
		if (InCommand.bCommandSuccessful)
		{
...
bool FPlasticUnshelveWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	for (FString& File : InCommand.Files)
	{
		if (ChangelistState->Files.FindByPredicate([&File](const FSourceControlStateRef& FileState) { return FileState->GetFilename().Equals(File, ESearchCase::IgnoreCase); }))
		{
			...
			UE_LOGF(LogSourceControl, Error, "Revert /%ls before unshelving the corresponding change from the shelve.", *File);
			InCommand.bCommandSuccessful = false;
		}
	}
	...
	if (InCommand.bCommandSuccessful)
	{
		...
		if (InCommand.Files.Num() < ChangelistState->ShelvedFiles.Num())
		{
			if (GetProvider().GetPlasticScmVersion() < PlasticSourceControlVersions::ShelvesetApplySelection)
			{
				...
				UE_LOGF(LogSourceControl, Error,
					"Unity Version Control %ls cannot unshelve a selection of files from a shelve. Unshelve them all at once or update to %ls or above.",
	...
	if (InCommand.Files.Num() < ChangelistState->ShelvedFiles.Num())
	{
		for (const auto& ShelveState : ChangelistState->ShelvedFiles)
		{
			if (!InCommand.Files.Contains(ShelveState->GetFilename()))
			{
				...
				{
					FPaths::MakePathRelativeTo(File, *FPaths::ProjectDir());
					UE_LOGF(LogSourceControl, Error, "The file /%ls is not in the changelist anymore, so the shelve cannot be updated. Unshelve the corresponding change and retry.", *File);
					InCommand.bCommandSuccessful = false;
				}
			}
		}
...
bool FPlasticGetChangelistDetailsWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	UE_LOGF(LogSourceControl, Log, "GetChangelistDetails: %d files in shelve %ls", BaseRevisions.Num(), *ShelveId);
	...
	for (auto& Revision : BaseRevisions)
	{
		...
		UE_LOGF(LogSourceControl, Log, "GetChangelistDetails: %ls baserevid:%d %ls", *Revision.Filename, Revision.RevisionId, *Revision.Action);
...
bool FPlasticGetFileWorker::Execute(FPlasticSourceControlCommand& InCommand)
{
	...
	if (Operation->IsShelve())
	{
		...
		UE_LOGF(LogSourceControl, Log, "GetFile(ShelveId:%d)", SourceControlRevision.ShelveId);
	}
	else
	{
		SourceControlRevision.RevisionId = FCString::Atoi(*Operation->GetRevisionNumber());
		UE_LOGF(LogSourceControl, Log, "GetFile(revid:%d)", SourceControlRevision.RevisionId);
	}
