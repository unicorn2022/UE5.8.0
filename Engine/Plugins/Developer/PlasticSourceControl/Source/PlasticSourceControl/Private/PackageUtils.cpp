...
void UnlinkPackages(const TArray<FString>& InFiles)
{
	...
	{
		UE_LOGF(LogSourceControl, Log, "Reseted Loader for %d Packages", LoadedPackages.Num());
	}
}
	...
	if (UWorld* CurrentWorld = GetCurrentWorld())
	{
		...
		if (!bHasCurrentMapBeenUpdated)
		{
			...
			if (bNeedReloadCurrentMap)
			{
				...
				UE_LOGF(LogSourceControl, Log, "Reload: %ls", *CurrentMapPath);
			}
		}
	}
...
{
	UE_LOGF(LogSourceControl, Verbose, "Reloading %d Packages...", InPackages.Num());
...
void ReloadPackages(const TArray<FString>& InFiles)
{
	...
	{
		UE_LOGF(LogSourceControl, Verbose, "No package to reload");
	}
}
