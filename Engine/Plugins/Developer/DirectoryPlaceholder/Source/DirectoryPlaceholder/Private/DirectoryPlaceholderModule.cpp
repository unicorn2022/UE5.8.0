	...
	for (const FFileChangeData& FileChange : InFileChanges)
	{
		...
		if (FPaths::DirectoryExists(FileChange.Filename) && (FileChange.Action == FFileChangeData::FCA_Added))
		{
			...
			if (PlaceholderAsset)
			{
				...
				UE_LOGF(LogDirectoryPlaceholder, Verbose, "New Directory Placeholder was created in %ls", *PackagePath);
			}
			else
			{
				UE_LOGF(LogDirectoryPlaceholder, Warning, "Failed to create new Directory Plcaeholder in %ls", *PackagePath);
			}
		}
	}
...
void FDirectoryPlaceholderModule::OnDeleteFolders(const TArray<FContentBrowserItemPath>& PathsToDelete)
{
	...
	UE_LOGF(LogDirectoryPlaceholder, Verbose, "Deleted %d Directory Placeholders", NumAssetsDeleted);
}
