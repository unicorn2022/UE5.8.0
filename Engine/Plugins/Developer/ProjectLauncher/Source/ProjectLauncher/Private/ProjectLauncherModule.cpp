...
public:
	...
	virtual void StartupModule() override
	{
		...
		RegisterExtension( MakeShared<ProjectLauncher::FBuildCookRunLaunchExtension>() );

		// register custom styles & other UI elements
	...
	virtual void ShutdownModule() override
	{
		...
		ProjectLauncher::ShutdownLaunchExtensions();
		Model.Reset();
	}
...
private:
	...
	TSharedRef<SDockTab> HandleSpawnTab(const FSpawnTabArgs& SpawnTabArgs)
	{
		...
		if (!Model.IsValid())
		{
			Model = MakeShared<ProjectLauncher::FModel>(TargetDeviceServicesModule.GetDeviceProxyManager(), LauncherServicesModule.CreateLauncher(), LauncherServicesModule.GetProfileManager() );
			Model->Init();
		}
		
		auto OnTabActivated = [this](TSharedRef<SDockTab>, ETabActivationCause Cause)
		{
		...
		[
			SNew(SProjectLauncher, Model.ToSharedRef())
		];
	}
