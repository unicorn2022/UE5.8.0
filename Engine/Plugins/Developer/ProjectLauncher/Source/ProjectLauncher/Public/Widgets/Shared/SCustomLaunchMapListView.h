...
enum class EMapSourceFlags : uint8
{
	None                  = 0,
...
ENUM_CLASS_FLAGS(EMapSourceFlags);
...
public:
	...
	DECLARE_DELEGATE_OneParam(FOnMapFilterChanged, EMapSourceFlags);
	...
	SLATE_BEGIN_ARGS(SCustomLaunchMapListView){}
		...
		SLATE_EVENT(FOnMapFilterChanged, OnMapFilterChanged)
		SLATE_ARGUMENT(EMapSourceFlags, MapSourceFlags)
	SLATE_END_ARGS()
...
protected:
	...
	FOnMapFilterChanged OnMapFilterChanged;

private:
	bool bShowFolders = true; // whether to display the available maps in a hierarchy or flat list

	// Content visibility flags (toggled via filter menu)
	EMapSourceFlags MapSourceFlags = EMapSourceFlags::None;

	UE_API TSharedRef<SWidget> MakeFilterMenuContent();
	bool HasAnyFilterEnabled() const;
