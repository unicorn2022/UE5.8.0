...
void SNamingTokenDataTreeViewWidget::Construct(const FArguments& InArgs)
{
	...
	NamespacePriority = InArgs._NamespacePriority;
	...
	PopulateTreeItems(InArgs._FilterArgs);
...
void SNamingTokenDataTreeViewWidget::PopulateTreeItems(const FNamingTokenFilterArgs& InFilterArgs)
{
	RootTreeItems.Reset();
	if (GEngine)
	{
		...
		{
			TArray<FString> Namespaces = NamingTokensSubsystem->GetAllNamespaces(/*bIncludePrivateNamespaces*/ true);
			RootTreeItems.Reserve(Namespaces.Num());

			// Sort namespaces so any matching the priority list are displayed first, in that order.
			// Any other namespaces preserve their order. It's better to do this before building out final lists
			// so we can record the proper index order for navigation next.
			if (!NamespacePriority.IsEmpty())
			{
				TMap<FString, int32> NamespaceOrder;
				NamespaceOrder.Reserve(NamespacePriority.Num());
				for (int32 Idx = 0; Idx < NamespacePriority.Num(); ++Idx)
				{
					NamespaceOrder.Add(NamespacePriority[Idx], Idx);
				}

				Namespaces.StableSort([&](const FString& A, const FString& B)
				{
					const int32* IndexA = NamespaceOrder.Find(A);
					const int32* IndexB = NamespaceOrder.Find(B);
			...
			for (const FString& Namespace : Namespaces)
			{
				...
				{
					const bool bIsPrivate = NamingTokens->IsPrivateNamespace();
					// Filter out any private namespaces unless specified.
					if (bIsPrivate && !InFilterArgs.AdditionalNamespacesToInclude.Contains(Namespace))
					{
						continue;
					}
...
void SNamingTokenDataTreeViewWidget::FilterTreeItems(const FString& InFilterText)
{
	...
	FString SanitizedFilterText = InFilterText;
	SanitizedFilterText.RemoveSpacesInline();
	if (LastFilterText == SanitizedFilterText && FilteredRootTreeItems.Num() > 0)
	{
		if (const FNamingTokenDataTreeItemPtr CurrentlySelectedItem = GetSelectedItem())
		{
			...
			bSelectingNamespace = SanitizedFilterText == CurrentlySelectedItem->Namespace && CurrentlySelectedItem->IsNamespace();
		}
	}
	LastFilterText = SanitizedFilterText;
	FString CompleteOrPartialNamespace = UE::NamingTokens::Utils::GetNamespaceFromTokenKey(SanitizedFilterText);
	FString PartialToken = UE::NamingTokens::Utils::RemoveNamespaceFromTokenKey(SanitizedFilterText);
	...
	if (bSearchNamespaceAndToken)
	{
		...
		CompleteOrPartialNamespace = SanitizedFilterText;
		PartialToken = SanitizedFilterText;
	}
