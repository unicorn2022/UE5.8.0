 ...
 */
UCLASS(MinimalAPI, Config = NamingTokens, DefaultConfig, ClassGroup = UI, PrioritizeCategories = ("NamingTokens", "Content", "Appearance", "Behavior"), meta = (Category = "NamingTokens UI", DisplayName = "Naming Tokens Editable Text Box"))
...
public:
	...
	UFUNCTION(BlueprintSetter)
	NAMINGTOKENSUI_API void SetShowUnknownTokenWarning(bool bValue);

	/** Get whether the widget warns when token(s) in the tokenized text are not recognized. */
	UFUNCTION(BlueprintGetter)
	NAMINGTOKENSUI_API bool GetShowUnknownTokenWarning() const;

	/** Set whether the widget should warn when token(s) in the tokenized text evaluate to an empty value. */
	UFUNCTION(BlueprintSetter)
	NAMINGTOKENSUI_API void SetShowUnsetTokenWarning(bool bValue);

	/** Get whether the widget warns when token(s) in the tokenized text evaluate to an empty value. */
	UFUNCTION(BlueprintGetter)
	NAMINGTOKENSUI_API bool GetShowUnsetTokenWarning() const;
...
protected:
	...
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category=Behavior, meta = (EditCondition="bEnableSuggestionDropdown", EditConditionHides, DisplayAfter=bFullyQualifyFilteredNamespaces))
	...
	/** If namespaces included in the FilterArgs.AdditionalNamespacesToInclude should autocomplete. If false, only the token is inserted. */
	UPROPERTY(EditAnywhere, AdvancedDisplay, Category=Behavior, meta = (EditCondition="bEnableSuggestionDropdown", EditConditionHides))
	...
	/** Display a warning when token(s) in the tokenized text are not recognized (unknown tokens). */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadWrite, BlueprintSetter=SetShowUnknownTokenWarning, BlueprintGetter=GetShowUnknownTokenWarning, Category=Appearance)
	bool bShowUnknownTokenWarning = true;

	/** Display a warning when token(s) in the tokenized text evaluate to an empty value (unset tokens). */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadWrite, BlueprintSetter=SetShowUnsetTokenWarning, BlueprintGetter=GetShowUnsetTokenWarning, Category=Appearance)
