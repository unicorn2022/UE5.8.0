...
static FAutoConsoleCommandWithWorldAndArgs CVarAnimSharing_Enabled(
	...
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		if (Args.Num() != 0)
		{
			...
			UE_LOGF(LogAnimationSharing, Log, "Animation Sharing System - %ls", GAnimationSharingEnabled ? TEXT("Enabled") : TEXT("Disabled"));
		}
	}),
	ECVF_Cheat);
...
void UAnimationSharingManager::AddSetup(const UAnimationSharingSetup* InSetup)
{
	...
	if (InSetup)
	{
		if (!AnimationSharingSetups.Contains(InSetup))
		{
			...
			NewScalabilitySettings.RequireCurvesDuringBlends = NewScalabilitySettings.RequireCurvesDuringBlends.GetValueForPlatform(PlatformName);
	#endif
		...
		{
			UE_LOGF(LogAnimationSharing, Error, "Passed duplicate UAnimationSharingSetup '%ls' to add to AnimationSharingManager!", *InSetup->GetName());
		}
	}
	else
	{
		UE_LOGF(LogAnimationSharing, Error, "Passed null UAnimationSharingSetup to add to AnimationSharingManager!");
	}
}
...
void UAnimationSharingManager::RemoveSetup(const UAnimationSharingSetup* InSetup)
{
	if (InSetup)
	{
		...
		if (SetupIdx != INDEX_NONE)
		{
			for (int32 PerSkeletonDataIdx = 0; PerSkeletonDataIdx < PerSkeletonData.Num() ; ++PerSkeletonDataIdx)
			{
				...
				if (PerSkeletonData[PerSkeletonDataIdx]->OwnerSetup == InSetup)
				{
					...
					{
						UE_LOGF(LogAnimationSharing, Error, "There are still actors registered to skeleton '%ls' when animation sharing setup '%ls' is being removed.", 
		...
		{
			UE_LOGF(LogAnimationSharing, Error, "Passed UAnimationSharingSetup '%ls' to remove from AnimationSharingManager that was not registered!", *InSetup->GetName());
		}
	}
	else
	{
		UE_LOGF(LogAnimationSharing, Error, "Passed null UAnimationSharingSetup to remove from AnimationSharingManager!");
	}
}
...
void UAnimationSharingManager::SetupPerSkeletonData(const FPerSkeletonAnimationSharingSetup& SkeletonSetup, const UAnimationSharingSetup* SharingSetup, const FAnimationSharingScalability& ScalabilitySettingsForSkeleton)
{
	...
	if (PerSkeletonData.Num() < MaxSkeletonIndex)
	{
		...
		if (Skeleton && StateEnum && Processor)
		{
			...
			if (!Skeletons.Contains(Skeleton))
			{
				...
				{
					UE_LOGF(LogAnimationSharing, Error, "Errors found when initializing Animation Sharing Data for Skeleton (%ls)!",
						Skeleton ? *Skeleton->GetName() : TEXT("None"));
				}

				if (NextSkeletonID > MaxSkeletonID)
				{
					UE_LOGF(LogAnimationSharing, Error, "Reached max number of skeletons IDs supported by anim sharing manager. Skeleton Id will be rolled over!");
					NextSkeletonID = 0;
				}
			}
			else
			{
				UE_LOGF(LogAnimationSharing, Error, "Skeleton (%ls) is being registered by sharing setup (%ls) but skeleton was already added previously!",
					Skeleton ? *Skeleton->GetName() : TEXT("None"), SharingSetup ? *SharingSetup->GetName() : TEXT("None"));
			}
		}
		else
		{
			UE_LOGF(LogAnimationSharing, Error, "Invalid Skeleton (%ls), State Enum (%ls) or State Processor (%ls)!", 
				Skeleton ? *Skeleton->GetName() : TEXT("None"),
	...
	{
		UE_LOGF(LogAnimationSharing, Error, "Skeleton (%ls) could not be added as reached max number of skeletons supported by anim sharing manager!",
			Skeleton ? *Skeleton->GetName() : TEXT("None"));
	}
}
...
bool UAnimationSharingManager::RegisterActor(AActor* InActor, FUpdateActorHandle CallbackDelegate)
{
	...
	if (InActor)
	{
		...
		return RegisterActorWithSkeleton(InActor, UsedSkeleton, CallbackDelegate);
	}

	return false;
}

bool UAnimationSharingManager::RegisterActorWithSkeleton(AActor* InActor, const USkeleton* SharingSkeleton, FUpdateActorHandle CallbackDelegate)
{
	if (!UAnimationSharingManager::AnimationSharingEnabled())
	{
		return false;
...
bool UAnimationSharingManager::RegisterActorWithSkeletonBP(AActor* InActor, const USkeleton* SharingSkeleton)
{
	return RegisterActorWithSkeleton(InActor, SharingSkeleton, FUpdateActorHandle::CreateLambda([](int32 A) {}));
}

bool UAnimationSharingManager::UnregisterActor(AActor* InActor)
{
	if (!UAnimationSharingManager::AnimationSharingEnabled())
	{
		return false;
	...
	// Return whether or not the actor was unregistered from at least one of the Anim Sharing Instances
	return UnregisterCounter > 0;
}
...
USkeletalMeshComponent* UAnimSharingInstance::GetComponent(const FPerComponentData& Component)
{
	...
	UE_LOGF(LogAnimationSharing, Warning, "Stale SkeletalMeshComponent\n\tActor Index: %i\n\tActor: %ls", Component.ActorIndex, RegisteredActors[Component.ActorIndex].Get() ? *RegisteredActors[Component.ActorIndex]->GetName() : TEXT("None"));
...
bool UAnimSharingInstance::Setup(UAnimationSharingManager* AnimationSharingManager, const FPerSkeletonAnimationSharingSetup& SkeletonSetup, const FAnimationSharingScalability* InScalabilitySettings, uint32 InSkeletonID)
{
	...
	if (SkeletalMesh && StateProcessor)
	{
		...
		for (const FAnimationStateEntry& StateEntry : SkeletonSetup.AnimationStates)
		{
			...
			if (!PerStateData.FindByPredicate([StateValue](const FPerStateData& State) { return State.StateEnumValue == StateValue; }))
			{
				...
				{
					UE_LOGF(LogAnimationSharing, Error, "No Components available for State %ls", *StateEnum->GetDisplayNameTextByValue(StateValue).ToString());
					bErrors = true;
				}
			}
			else
			{
				UE_LOGF(LogAnimationSharing, Error, "Duplicate entries in Animation Setup for State %ls", *StateEnum->GetDisplayNameTextByValue(StateValue).ToString());
				bErrors = true;
			}
		}
		...
		if (!bErrors && ScalabilitySettings.UseBlendTransitions.Default)
		{
			...
			for (uint32 BlendIndex = 0; BlendIndex < TotalNumberOfBlendActorsRequired; ++BlendIndex)
			{
				...
				BlendComponent->bPropagateCurvesToFollowers = ScalabilitySettings.RequireCurvesDuringBlends.Default;
	...
	{
		UE_LOGF(LogAnimationSharing, Error, "Invalid Skeletal Mesh or State Processing Class");
		bErrors = true;
	}
...
void UAnimSharingInstance::SetupState(FPerStateData& StateData, const FAnimationStateEntry& StateEntry, USkeletalMesh* SkeletalMesh, const FPerSkeletonAnimationSharingSetup& SkeletonSetup, uint32 InSkeletonID)
{
	...
	StateData.bShouldRandomizeOnDemandAnims = StateEntry.bRandomizeOnDemandAnims;
	...
	for (int32 SetupIndex = 0; SetupIndex < StateEntry.AnimationSetups.Num(); ++SetupIndex)
	{
		...
		{
			UE_LOGF(LogAnimationSharing, Error, "Animation setup entry for state %ls without either a valid Animation Blueprint Class or Animation Sequence", StateEnum ? *StateEnum->GetName() : TEXT("None"));
			continue;
		}
		...
		if (bEnabled && (AnimBPClass || AnimSequence))
		{
			...
			for (uint32 InstanceIndex = 0; InstanceIndex < NumInstances; ++InstanceIndex)
			{
				if (!StateData.bIsAdditive)
				{
					...
					// Force component to always run at LOD0, to ensure all follower-components will always be at the same or higher LOD level for bone transforms
					...
					if (AnimBPClass != nullptr && AnimSequence != nullptr && !StateEntry.bOnDemand)
					{
						...
						if (UAnimSharingStateInstance* AnimInstance = Cast<UAnimSharingStateInstance>(Component->GetAnimInstance()))
						{
							...
							/** Set the current animation length */
				...
				{	
					...
					// Force component to always run at LOD0, to ensure all follower-components will always be at the same or higher LOD level for bone transforms
					AdditiveComponent->SetForcedLOD(1);
...
#if LOG_STATES 
			UE_LOGF(LogAnimationSharing, Log, "Finished on demand [%i] %ls", InstanceIndex, *StateEnum->GetDisplayNameTextByValue(Instance.State).ToString());
#endif
...
#if LOG_STATES 
						UE_LOGF(LogAnimationSharing, Log, "Returning [%i] to %ls", ActorIndex, *StateEnum->GetDisplayNameTextByValue(PreviousActorState).ToString());
#endif
					}
				}
...
#if LOG_STATES
					UE_LOGF(LogAnimationSharing, Log, "Forwarding [%i] to %ls", ActorIndex, *StateEnum->GetDisplayNameTextByValue(Instance.ForwardState).ToString());
#endif						
				}
...
#if LOG_STATES 
						UE_LOGF(LogAnimationSharing, Log, "Setting [%i] to %ls", ActorIndex, *StateEnum->GetDisplayNameTextByValue(CurrentState).ToString());
#endif
					}
				}
				else
...
#if LOG_STATES 
					UE_LOGF(LogAnimationSharing, Log, "TODO-ing [%i]", ActorIndex);
#endif
				}					
			}			
			...
			for (uint32 ActorIndex : Instance.ActorIndices)
			{
				...
				if (BlendToState != (uint8)INDEX_NONE)
				{
					...
					if (BlendIndex != INDEX_NONE)
					{
						...
						// Make sure to update actor data with running blend instance (index)
						PerActorData[ActorIndex].BlendInstanceIndex = BlendIndex;
#if LOG_STATES 
						UE_LOGF(LogAnimationSharing, Log, "Blending [%i] out from %ls to %ls", ActorIndex, *StateEnum->GetDisplayNameTextByValue(Instance.State).ToString(), *StateEnum->GetDisplayNameTextByValue(BlendToState).ToString());
#endif
					}
				}
	...
	for (int32 ActorIndex = 0; ActorIndex < RegisteredActors.Num(); ++ActorIndex)
	{
		...
		if (Actor)
		{
			...
			if (CurrentState != PreviousState)
			{
				...
				{
#if LOG_STATES 
					UE_LOGF(LogAnimationSharing, Log, "Setting %i state to %i previous %i | %i", ActorIndex, CurrentState, PreviousState, ActorData.PermutationIndex);
#endif
				...
				if (!bShouldProcess || bShouldNotProcess)
				{
					UpdateState();
#if LOG_STATES 
					UE_LOGF(LogAnimationSharing, Log, "Changing state to %ls from %ls while running on demand %i (%i)", *StateEnum->GetDisplayNameTextByValue(CurrentState).ToString(), *StateEnum->GetDisplayNameTextByValue(ActorData.PreviousState).ToString(), ActorIndex, ActorData.PermutationIndex);
#endif
				}
				...
				else if (PerStateData[CurrentState].bIsAdditive)
				{
					...
					{
						USkeletalMeshComponent* BaseComponent = nullptr;
						if (ActorData.bBlending)
						{
							// When running a blend (either between regular states, or in/out of on-demand states), use the blend component as the additive base
							if (BlendInstances.IsValidIndex(ActorData.BlendInstanceIndex))
							{
								BaseComponent = BlendInstances[ActorData.BlendInstanceIndex].TransitionBlendInstance->GetComponent();
							}
							
						}
						else if (ActorData.bRunningOnDemand)
						{
							// When running an on-demand, use the per-state-component used by/running the on demand instance as the additive base
							if(OnDemandInstances.IsValidIndex(ActorData.OnDemandInstanceIndex))
							{
								const FOnDemandInstance& RunningOnDemandInstance = OnDemandInstances[ActorData.OnDemandInstanceIndex];
								BaseComponent = PerStateData[RunningOnDemandInstance.State].Components[RunningOnDemandInstance.UsedPerStateComponentIndex];
							}
						
						}
						else if (PerStateData[PreviousState].Components.IsValidIndex(ActorData.PermutationIndex))
						{
							// When running a regular state, use the permutation component as the additive base 
							BaseComponent = PerStateData[PreviousState].Components[ActorData.PermutationIndex];
						}

						// If we somehow did not manage to find a valid base component, ignore the additive request
						if (BaseComponent)
						{
							const uint32 AdditiveInstanceIndex = SetupAdditiveInstance(CurrentState, BaseComponent);
						...
						{
							UE_LOGF(LogAnimationSharing, Warning, "Failed to setup additive state %ls for %i. OnDemand %i %i. Blend %i %i. State %i %i", *StateEnum->GetDisplayNameTextByValue(CurrentState).ToString(), ActorIndex, ActorData.bRunningOnDemand, ActorData.OnDemandInstanceIndex, ActorData.bBlending, ActorData.BlendInstanceIndex, PreviousState, ActorData.PermutationIndex);
						}
					}
				}
				...
				else
				{
					...
					if (bShouldBlend)
					{
						...
						bShouldSwitch = (ActorData.BlendInstanceIndex == INDEX_NONE);
#if LOG_STATES 
						UE_LOGF(LogAnimationSharing, Log, "Changing state to %ls from %ls with blend %i", *StateEnum->GetDisplayNameTextByValue(CurrentState).ToString(), *StateEnum->GetDisplayNameTextByValue(PreviousState).ToString(), ActorIndex);
#endif
					}
...
#if LOG_STATES 
						UE_LOGF(LogAnimationSharing, Log, "Changing state to %ls from %ls %i", *StateEnum->GetDisplayNameTextByValue(CurrentState).ToString(), *StateEnum->GetDisplayNameTextByValue(PreviousState).ToString(), ActorIndex);
#endif
					}
			...
			else if (!ActorData.bRunningOnDemand && !ActorData.bBlending)
			{
				...
				{
					UE_LOGF(LogAnimationSharing, Log, "Invalid permutation for actor %i is out of range of %i for state %ls by actor %i", ActorData.PermutationIndex, PerStateData[ActorData.CurrentState].Components.Num(), *StateEnum->GetDisplayNameTextByValue(ActorData.CurrentState).ToString(), ActorIndex);
				}
				else if (!PerStateData[ActorData.CurrentState].Components[ActorData.PermutationIndex]->IsComponentTickEnabled())
				{
					UE_LOGF(LogAnimationSharing, Log, "Component not active %i for state %ls by actor %i", ActorData.PermutationIndex, *StateEnum->GetDisplayNameTextByValue(ActorData.CurrentState).ToString(), ActorIndex);
				}
#endif 
...
void UAnimSharingInstance::TickBlendInstances()
{
	...
	for (int32 InstanceIndex = 0; InstanceIndex < BlendInstances.Num(); ++InstanceIndex)
	{
		...
		{
#if LOG_STATES
			UE_LOGF(LogAnimationSharing, Log, "Finished blend %ls from %ls [%i]", *StateEnum->GetDisplayNameTextByValue(Instance.StateTo).ToString(), *StateEnum->GetDisplayNameTextByValue(Instance.StateFrom).ToString(), InstanceIndex);
#endif
...
#if LOG_STATES
					UE_LOGF(LogAnimationSharing, Log, "Setting %i to on-demand component %i [%i]", ActorIndex, Instance.ToOnDemandInstanceIndex, InstanceIndex);
#endif
			...
			{				
				for (uint32 ActorIndex : Instance.ActorIndices)
				{
					...
					{
#if LOG_STATES 
						UE_LOGF(LogAnimationSharing, Log, "Setting %i to state %i | %i [%i]", ActorIndex, Instance.StateTo, Instance.ToPermutationIndex, InstanceIndex);
#endif
			...
			{
				PerActorData[ActorIndex].BlendInstanceIndex = INDEX_NONE;
				PerActorData[ActorIndex].bBlending = false;
			}
...
void UAnimSharingInstance::SetComponentUsage(bool bUsage, uint8 StateIndex, uint32 ComponentIndex)
{
	...
	{
		UE_LOGF(LogAnimationSharing, Log, "Invalid set component usage %i is out of range of %i for state %ls by component %i", ComponentIndex, PerStateData[StateIndex].Components.Num(), *StateEnum->GetDisplayNameTextByValue(StateIndex).ToString(), ComponentIndex);
	}
#endif
...
void UAnimSharingInstance::SetLeaderComponentForActor(uint32 ActorIndex, USkeletalMeshComponent* Component)
{
	...
	{
#if LOG_STATES
		UE_LOGF(LogAnimationSharing, Log, "Updating additive instance base component %ls for %i", *StateEnum->GetDisplayNameTextByValue(AdditiveInstances[ActorData.AdditiveInstanceIndex].State).ToString(), ActorIndex);
#endif // LOG_STATES
...
void UAnimSharingInstance::SetupFollowerComponent(uint8 CurrentState, uint32 ActorIndex)
{
	...
	if (StateData.Components.Num() == 0)
	{	
		UE_LOGF(LogAnimationSharing, Warning, "No Leader Components available for state %ls, make sure to set up an Animation Sequence/Blueprint ", *StateEnum->GetDisplayNameTextByValue(CurrentState).ToString());
		return;
	}
...
void UAnimSharingInstance::SetPermutationFollowerComponent(uint8 StateIndex, uint32 ActorIndex, uint32 PermutationIndex)
{
	...
	{
		UE_LOGF(LogAnimationSharing, Log, "Invalid set component usage %i is out of range of %i for state %ls by actor %i", PermutationIndex, StateData.Components.Num(), *StateEnum->GetDisplayNameTextByValue(StateIndex).ToString(), ActorIndex);
	}
#endif
...
uint32 UAnimSharingInstance::SetupOnDemandInstance(uint8 StateIndex)
{
	...
	else
	{
		...
		{
			auto FindRandomAvailableIndex = [&StateData]() -> uint32
				{
					TBitArray<>& ComponentFrameBits = StateData.InUseComponentFrameBits;
					TArray<uint32> ZeroBitIndices;
					ZeroBitIndices.Reserve(ComponentFrameBits.Num());

					for (int32 Index = 0; Index < ComponentFrameBits.Num(); ++Index)
					{
						if (ComponentFrameBits[Index] == 0)
						{
							ZeroBitIndices.Add(Index);
						}
					}

					if (ZeroBitIndices.Num() > 0)
					{
						const uint32 RandomIndex = FMath::RandHelper(ZeroBitIndices.Num());
						ComponentFrameBits[ZeroBitIndices[RandomIndex]] = 1;
						return ZeroBitIndices[RandomIndex];
					}

					// No available components this frame.
					return INDEX_NONE;
				};

			const uint32 AvailableIndex = StateData.bShouldRandomizeOnDemandAnims && StateData.InUseComponentFrameBits.Num() > 1 ? FindRandomAvailableIndex() : StateData.InUseComponentFrameBits.FindAndSetFirstZeroBit();
...
#if LOG_STATES 
				UE_LOGF(LogAnimationSharing, Log, "Setup on demand state %ls %i", *StateEnum->GetDisplayNameTextByValue(StateIndex).ToString(), InstanceIndex);
#endif
			}
			else
			{
				...
				else
				{
					...
					UE_LOGF(LogAnimationSharing, Warning, "No more on demand components available");
				}
			}
		}
	}
...
uint32 UAnimSharingInstance::SetupAdditiveInstance(uint8 StateIndex, USkeletalMeshComponent* InBaseComponent)
{
	uint32 InstanceIndex = INDEX_NONE;
...
#if LOG_STATES 
			UE_LOGF(LogAnimationSharing, Log, "Starting blend from %ls to %ls [%ls]", *StateEnum->GetDisplayNameTextByValue(BlendInstance.StateFrom).ToString(), *StateEnum->GetDisplayNameTextByValue(BlendInstance.StateTo).ToString(), *ActorIndicesString);
#endif
...
bool UAnimationSharingManager::CheckDataForActor(AActor* InActor) const
{
	...
	{
		UE_LOGF(LogAnimationSharing, Warning, "CheckActorData Failed! Reason: Actor %ls is not registered. PerSkeletonData Num: %d",
	...
	{
		UE_LOGF(LogAnimationSharing, Warning, "CheckActorData Failed! Reason: ActorIndex '%d' for '%ls' is not a valid index in PerActorData. SkeletonDataIdx: %d",
	...
	{
		UE_LOGF(LogAnimationSharing, Warning, "CheckActorData Failed! Reason: ComponentIndices for '%ls' is empty. SkeletonDataIdx: %d ActorDataIdx: %d",
	...
	for (int32 ComponentIndex : ActorData.ComponentIndices)
	{
		...
		{
			UE_LOGF(LogAnimationSharing, Warning, "CheckActorData Failed! Reason: ComponentIndex '%d' for '%ls' is not a valid index in PerComponentData. PerComponentData.Num: %d",
...
{
	UE_LOGF(LogAnimationSharing, Log, "PerSkeletonData Num: %d", PerSkeletonData.Num());
	for (int32 SkeletonIndex = 0; SkeletonIndex < PerSkeletonData.Num(); ++SkeletonIndex)
	{
		if (const UAnimSharingInstance* SkeletonData = PerSkeletonData[SkeletonIndex])
		{
			UE_LOGF(LogAnimationSharing, Log, "\t RegisteredActors Num: %d", SkeletonData->RegisteredActors.Num());
			for (int32 Idx = 0; Idx < SkeletonData->RegisteredActors.Num(); Idx++)
			{
				UE_LOGF(LogAnimationSharing, Log, "\t\t Idx: %d Actor: %ls", Idx, *GetNameSafe(SkeletonData->RegisteredActors[Idx]));
			}

			UE_LOGF(LogAnimationSharing, Log, "\t PerActorData Num: %d", SkeletonData->PerActorData.Num());
			for (int32 Idx = 0; Idx < SkeletonData->PerActorData.Num(); Idx++)
			{
				...
				UE_LOGF(LogAnimationSharing, Log, "\t\t Idx: %d CurrState: %d PrevState: %d PermutationIdx: %d ComponentIndices: %ls",
					Idx, ActorData.CurrentState, ActorData.PreviousState, ActorData.PermutationIndex, *ComponentIndicesStr);
			}

			UE_LOGF(LogAnimationSharing, Log, "\t PerComponentData Num: %d", SkeletonData->PerComponentData.Num());
			for (int32 Idx = 0; Idx < SkeletonData->PerComponentData.Num(); Idx++)
			{
				...
				UE_LOGF(LogAnimationSharing, Log, "\t\t Idx: %d Comp: %ls CompOwner: %ls ActorIndex: %d",
