...
struct FAnimationStateEntry
{
	GENERATED_BODY()
public:
	FAnimationStateEntry() : State(0), bOnDemand(false), bRandomizeOnDemandAnims(false), bAdditive(false), BlendTime(0.f), bReturnToPreviousState(false), bSetNextState(false), NextState(0), MaximumNumberOfConcurrentInstances(1), WiggleTimePercentage(0.1f), bRequiresCurves(false) {}
	...
	/* Flag that determines whether or not we attempt to randomize the selection of OnDemand animations, if the total number of components for the OnDemand state is more than 1. */
	UPROPERTY(EditAnywhere, Category = AnimSharing, meta = (EditCondition = "bOnDemand"))
	bool bRandomizeOnDemandAnims;
...
public:
	...
	/** Whether or not curves are required during blends */
	UPROPERTY(EditAnywhere, Category = AnimationSharing)
	FPerPlatformBool RequireCurvesDuringBlends;
};
