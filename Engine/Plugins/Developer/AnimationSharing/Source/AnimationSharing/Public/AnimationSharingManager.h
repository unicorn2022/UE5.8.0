...
{
	FPerStateData() : bIsOnDemand(false), bIsAdditive(false), bShouldRandomizeOnDemandAnims(false), BlendTime(0.f), CurrentFrameOnDemandIndex(INDEX_NONE), StateEnumValue(INDEX_NONE), AdditiveAnimationSequence(nullptr) {}
	...
	/* Flag to allow random selection of OnDemand components so we don't always get only the first avaialable component. */
	bool bShouldRandomizeOnDemandAnims;
...
public:
	...
	UE_DEPRECATED(5.8, "SetupAdditiveInstance with state and component index has been deprecated as it causes invalid behaviour, use new signature instead")
	uint32 SetupAdditiveInstance(uint8 StateIndex, uint8 FromState, uint8 StateComponentIndex) { return INDEX_NONE; }
	UE_API uint32 SetupAdditiveInstance(uint8 StateIndex, USkeletalMeshComponent* InBaseComponent);
...
public:
	...
	UE_API bool RegisterActorWithSkeletonBP(AActor* InActor, const USkeleton* SharingSkeleton);
	...
	UE_API bool RegisterActor(AActor* InActor, FUpdateActorHandle Delegate);

	/** Registers actor with the animation sharing system according to the SharingSkeleton's sharing setup (if available) */
	UE_API bool RegisterActorWithSkeleton(AActor* InActor, const USkeleton* SharingSkeleton, FUpdateActorHandle Delegate);

	/** Unregisters actor with the animation sharing system */
	UE_API bool UnregisterActor(AActor* InActor);
