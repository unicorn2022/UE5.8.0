...
ISourceControlProvider::FInitResult FPerforceSourceControlProvider::ParseCommandLineSettings(EInitFlags InitFlags)
{
	...
	if (EnumHasAnyFlags(InitFlags, EInitFlags::AttemptConnection))
	{
		...
		if (EnumHasAnyFlags(InitFlags, EInitFlags::SuppressErrorLogging))
		{
			ConnectionOptions |= EConnectionOptions::SuppressErrorLogging;
		}
...
bool FPerforceSourceControlProvider::UsesSoftRevertOnDelete() const
{
	return false;
}
...
TOptional<bool> FPerforceSourceControlProvider::HasChangesToSync() const
{
	return TOptional<bool>();
}

TOptional<bool> FPerforceSourceControlProvider::HasChangesToCheckIn() const
{
	return TOptional<bool>();
}
...
void FPerforceSourceControlProvider::OutputCommandMessages(const FPerforceSourceControlCommand& InCommand) const
{
	...
	else // On background threads we must log directly as FMessageLog internals cannot be assumed to be thread safe
	{
		if (InCommand.Operation->ShouldLogErrors())
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "Command: %ls, Error: %ls", *InCommand.Operation->GetName().ToString(), *Error.ToString());
			}
		}

		for (const FText& Info : InCommand.ResultInfo.InfoMessages)
		{
			UE_LOGF(LogSourceControl, Log, "Command: %ls, Info: %ls", *InCommand.Operation->GetName().ToString(), *Info.ToString());
		}
	}
}
...
void FPerforceSourceControlProvider::Tick()
{
	...
	// Close idle connections when they haven't seen any activity for IdleConnectionDisconnectSeconds
	// Disabled for commandlets, which are often long running processes
	if (PersistentConnection && !IsRunningCommandlet())
	{
		...
		{
			UE_LOGF(LogSourceControl, Display,
				"Persistent perforce connection has not been used in %0.f seconds. Dropping connection",
...
ECommandResult::Type FPerforceSourceControlProvider::SwitchWorkspace(FStringView NewWorkspaceName, FSourceControlResultInfo& OutResultInfo, FString* OutOldWorkspaceName)
{
	...
	{
		UE_LOGF(LogSourceControl, Log, "Waiting on pending commands before switching workspace");
	...
	UE_LOGF(LogSourceControl, Log, "Switched workspaces from '%ls' to '%ls'", *OldWorkspaceName, *WorkspaceName);
