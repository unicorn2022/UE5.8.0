...
bool GParseIgnores = false;
static FAutoConsoleVariableRef CVarParseIgnores(
	TEXT("p4.ParseIgnores"),
	GParseIgnores,
	TEXT("Use p4ignore behavior to exclude files from revision control consideration."),
	ECVF_Default
);

}

namespace PerforceWorkersTedsUtility {
	
void UpdateEditorDataStorage(const FPerforceSourceControlState& State)
	{
		...
		const RowHandle Row = FindOrAddPackageRow(*DataStorage, State.GetFilename());
		if (Row == InvalidRowHandle)
		{
			return;
		}

		FSccStateColumn StateColumn;

		if (State.IsSourceControlled())
		{
			// Modification priority mirrors the previous call-order: each match overwrites
			// the prior value, so IsConflicted wins over IsDeleted wins over IsAdded wins
			// over IsModified. If none match, Modification stays ESCCModification::None.
			if (State.IsModified())   StateColumn.Modification = ESCCModification::Modified;
			if (State.IsAdded())      StateColumn.Modification = ESCCModification::Added;
			if (State.IsDeleted())    StateColumn.Modification = ESCCModification::Removed;
			if (State.IsConflicted()) StateColumn.Modification = ESCCModification::Conflicted;

			const bool bIsCheckedOutByOther = State.IsCheckedOutOther(nullptr);
			if (bIsCheckedOutByOther && State.bExclusiveCheckout)
			{
				StateColumn.bIsExternallyLocked = true;
			}

			StateColumn.bIsLocked           = State.IsCheckedOut();
			StateColumn.bIsExternallyEdited = bIsCheckedOutByOther;
			StateColumn.bIsNotCurrent       = (State.LocalRevNumber < State.DepotRevNumber);
		}
		else
		{
			StateColumn.Modification = ESCCModification::NotInDepot;
		}

#if SOURCE_CONTROL_WITH_SLATE
		const FSlateIcon Icon = State.GetIcon();
		StateColumn.IconStyleSetName = Icon.GetStyleSetName();
		StateColumn.IconStyleName    = Icon.GetStyleName();
		StateColumn.IconOverlayName  = Icon.GetOverlayStyleName();
		StateColumn.DisplayTooltip   = State.GetDisplayTooltip();
#endif

		ApplySccStateUpdate(*DataStorage, Row, StateColumn);
	};
}
...
static void ParseRecordSetForState(const FP4RecordSet& InRecords, TMap<FString, EPerforceState::Type>& OutResults, bool bConsiderDepotFile = false)
{
	...
	for (const FP4Record& ClientRecord : InRecords)
	{
		...
		if(Action.Len() > 0)
		{
			...
			else if(Action == TEXT("branch") || Action == TEXT("integrate"))
			{
...
static void UpdateChangelistState(FPerforceSourceControlProvider& SCCProvider, const FPerforceSourceControlChangelist& InChangelist, const TMap<FString, EPerforceState::Type>& InOperationResults)
{
	...
	if (InChangelist.IsInitialized())
	{
		...
		{
			TSharedPtr<FPerforceSourceControlState, ESPMode::ThreadSafe> State;
			...
			switch (It.Value())
			{
				...
				case EPerforceState::Branched:
					State = SCCProvider.GetStateInternal(It.Key());
					ChangelistState->Files.Add(State.ToSharedRef());
...
static bool UpdateCachedStates(FPerforceSourceControlProvider& SCCProvider, const TMap<FString, EPerforceState::Type>& InResults)
{
	...
	for(TMap<FString, EPerforceState::Type>::TConstIterator It(InResults); It; ++It)
	{
		...
		PerforceWorkersTedsUtility::UpdateEditorDataStorage(*State);
	}
...
static void ParseUpdateStatusResults(const FP4RecordSet& InRecords, const TArray<FText>& ErrorMessages, TArray<FPerforceSourceControlState>& OutStates, const TArray<FString>& ContentRoots, TMap<FString, FBranchModification>& BranchModifications)
{
	...
	for (const FP4Record& ClientRecord : InRecords)
	{
		...
		{
			if(Action.Len() > 0 && (Action == TEXT("branch") || Action == TEXT("integrate")))
			{
...
static void ParseOpenedResults(const FP4RecordSet& InRecords, const FString& ClientName, const FString& ClientRoot, TArray<FPerforceSourceControlState>& OutResults)
{
	...
	for (const FP4Record& ClientRecord : InRecords)
	{
		...
		if (Action.Len() > 0)
		{
			...
			{
				OutState.State = EPerforceState::Branched;
			}
		}
...
static void ParseShelvedResults(const FP4RecordSet& InRecords, TMap<FString, EPerforceState::Type>& OutResults)
{
	...
	for (const FP4Record& Record : InRecords)
	{
		...
		if (Action.Len() > 0 && DepotFileName.Len() > 0)
		{
			...
			{
				OutResults.Add(DepotFileName, EPerforceState::Branched);
			}
		}
	}
}
...
bool FPerforceUpdateStatusWorker::Execute(FPerforceSourceControlCommand& InCommand)
{
	...
	if (!InCommand.IsCanceled() && ScopedConnection.IsValid())
	{
		...
		if(InCommand.Files.Num())
		{
			...
			// So we want to suppress that log output unless the user is running with LogSourceControl as verbose.
			...
			// Parse ignores for files that landed in NotInDepot as 'fstat' output does not differentiate between NotInDepot vs Ignore.
			if (GParseIgnores)
			{
				for (FPerforceSourceControlState& State : OutStates)
				{
					if (State.State == EPerforceState::NotInDepot)
					{
						const bool bIsIgnored = Connection.IsIgnored(State.GetFilename());
						if (bIsIgnored)
						{
							State.State = EPerforceState::Ignore;
						}
					}
				}
			}
