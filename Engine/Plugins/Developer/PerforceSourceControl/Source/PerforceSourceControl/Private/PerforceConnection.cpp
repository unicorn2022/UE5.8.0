...
bool FPerforceConnection::Login(const FPerforceConnectionInfo& InConnectionInfo)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "Login failed");
		for (const FText& ErrorMessage : ResultInfo.ErrorMessages)
		{
			UE_LOGF(LogSourceControl, Error, "%ls", *ErrorMessage.ToString());
		}
	}
	...
	FPerforceConnectionInfo& OutSettings, ISourceControlProvider::FInitResult::FConnectionErrors& OutConnectionErrors)
{
	...
	if (!bResult)
	{
		...
		if (!EnumHasAllFlags(Options, EConnectionOptions::SuppressErrorLogging))
		{
			FTSMessageLog SourceControlLog("SourceControl");
	...
	if (bEstablishedConnection)
	{
		...
		if (bCommandOK)
		{
			...
			for (const FP4Record& ClientRecord : Records)
			{
				...
				{
					UE_LOGF(LogSourceControl, Verbose, " %ls client specs rejected due to host name mismatch (%ls)", *ClientName, *HostName);
				}
			}
		}
...
bool FPerforceConnection::GetProjectWorkspaces(const FPerforceConnectionInfo& InConnectionInfo, FOnIsCancelled InOnIsCancelled, TArray<FString>& OutWorkspaceNames, FSourceControlResultInfo& OutResultInfo)
{
	...
	if (GetWorkspaces(InConnectionInfo, MoveTemp(InOnIsCancelled), WorkspaceInfos, OutResultInfo))
	{
		...
		for (FScWorkspaceInfo& WorkspaceInfo : WorkspaceInfos)
		{
			...
			{
				UE_LOGF(LogSourceControl, Verbose, " %ls client specs rejected due to root directory mismatch (%ls)", *WorkspaceInfo.Name, *WorkspaceInfo.RootDir);
			}
		}
...
bool FPerforceConnection::IsIgnored(const FString& InFile)
{
	if (Ignore* P4Ignore = P4Client.GetIgnore())
	{
		const StrPtr& IgnoreFile = P4Client.GetIgnoreFile();
		const StrBuf File(FROM_TCHAR(*InFile, bIsUnicode));

		return P4Ignore->Reject(File, IgnoreFile, nullptr) != 0;
	}
...
void FPerforceConnection::Disconnect()
{
	...
	if (P4Error.Test())
	{
		StrBuf ErrorMessage;
		P4Error.Fmt(&ErrorMessage);
		UE_LOGF(LogSourceControl, Error, "P4ERROR: Failed to disconnect from Server.");
		UE_LOGF(LogSourceControl, Error, "%ls", TO_TCHAR(ErrorMessage.Text(), bIsUnicode));
	}
}
                                        ...
                                        FOnIsCancelled InIsCancelled, bool& OutConnectionDropped, ERunCommandFlags RunFlags)
{
	...
	if (bLogCommandDetails)
	{
		UE_LOGF( LogSourceControl, Log, "Attempting 'p4 %ls'", *FullCommand );
	}
	...
	if (bLogCommandDetails)
	{
		...
		{
			UE_LOGF(LogSourceControl, Log, "P4 execution time: %0.4f seconds. Command: %ls", ExecutionTime, *FullCommand);
		}
		else
		{
			UE_LOGF(LogSourceControl, VeryVerbose, "P4 execution time: %0.4f seconds. Command: %ls", ExecutionTime, *FullCommand);
		}
	}
...
void FPerforceConnection::EstablishConnection(const FPerforceConnectionInfo& InConnectionInfo, FSourceControlResultInfo* OutResultInfo)
{
	...
	UE_LOGF(LogSourceControl, Verbose, "Attempting P4 connection: %ls/%ls", *InConnectionInfo.Port, *InConnectionInfo.UserName);
	...
	if (P4Error.Test())
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "P4ERROR: Invalid connection to server.");
			UE_LOGF(LogSourceControl, Error, "%ls", ANSI_TO_TCHAR(ErrorMessage.Text()));
		}
	}
	else
	{
		...
		UE_LOGF(LogSourceControl, Verbose, " ... checking unicode status");
		...
		if (RunCommand(TEXT("info"), Params, Records, nullptr, DummyResultInfo, FOnIsCancelled(), bConnectionDropped, RunFlags))
		{
			...
			if(bIsUnicode)
			{
				...
				UE_LOGF(LogSourceControl, Verbose, " server is unicode");
			}
			...
			UE_LOGF(LogSourceControl, Verbose, " ... getting info");
			bConnectionDropped = false;
			if (RunCommand(TEXT("info"), Params, Records, nullptr, DummyResultInfo, FOnIsCancelled(), bConnectionDropped, RunFlags))
			{
				UE_LOGF(LogSourceControl, Verbose, " ... getting clientroot");
