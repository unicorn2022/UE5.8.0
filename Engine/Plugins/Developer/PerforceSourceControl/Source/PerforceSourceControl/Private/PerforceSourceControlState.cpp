...
namespace UE::RevisionControl::Perforce
{
	static bool bAllowRevertDeletedFiles = false;
	
	static FAutoConsoleVariableRef CVarAllowRevertDeletedFiles(
		TEXT("RevisionControl.Perforce.AllowRevertingDeletedFiles"),
		bAllowRevertDeletedFiles,
		TEXT("If true, deleted files can be reverted in the editor"),
		ECVF_Default
	);
}
...
FText FPerforceSourceControlState::GetDisplayTooltip() const
{
	if (IsConflicted())
	{
		return LOCTEXT("Conflicted_Tooltip", "The file has local changes that need to be resolved with changes submitted to the Perforce depot");
	}
	else if( !IsCurrent() )
	{
		return LOCTEXT("NotCurrent_Tooltip", "The file is not at the head revision");
	}
	else if (State != EPerforceState::CheckedOut && State != EPerforceState::CheckedOutOther)
	{
		if (IsCheckedOutInOtherBranch())
	...
	case EPerforceState::ReadOnly:
		return LOCTEXT("ReadOnly_Tooltip", "The file is marked locally as read-only");
	case EPerforceState::NotInDepot:
		return LOCTEXT("NotInDepot_Tooltip", "The file is not present in the Perforce depot");
	case EPerforceState::CheckedOutOther:
		return FText::Format( LOCTEXT("CheckedOutOther_Tooltip", "Checked out by: {0}"), FText::FromString(OtherUserCheckedOut) );
	case EPerforceState::Ignore:
		return LOCTEXT("Ignore_Tooltip", "The file is ignored by Perforce");
	case EPerforceState::OpenForAdd:
		return LOCTEXT("OpenedForAdd_Tooltip", "The file is opened for add");
	case EPerforceState::MarkedForDelete:
		return LOCTEXT("MarkedForDelete_Tooltip", "The file is marked for delete");
	case EPerforceState::Branched:
		return LOCTEXT("Branched_Tooltip", "The file is opened for branching");
	}
}
...
{
	bool bCanRevert = IsCheckedOut() || IsAdded();

	if (UE::RevisionControl::Perforce::CVarAllowRevertDeletedFiles->GetBool())
	{
		bCanRevert |= IsDeleted();
	}
