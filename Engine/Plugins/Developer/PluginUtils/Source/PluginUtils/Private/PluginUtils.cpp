...
namespace PluginUtils
{
				...
				if (FilesToScan.Num() > 0)
				{
					...
					for (const FString& File : FilesToScan)
					{
						...
						for (FAssetData AssetData : Assets)
						{
							...
							// ANY assets created/duplicated from a plugin template should have its package localization ID reset to avoid conflicts.
	...
	void InitializeNewPluginParamsWithDescriptor(const FString& PluginName, const FPluginUtils::FNewPluginParams& CreationParams, FPluginUtils::FNewPluginParamsWithDescriptor& ExCreationParams)
	{
		...
		ExCreationParams.PostCreatePythonScriptPath            = CreationParams.PostCreatePythonScriptPath;
		ExCreationParams.PostCreatePythonScriptArguments       = CreationParams.PostCreatePythonScriptArguments;
		ExCreationParams.PostCreatePythonScriptAdditionalArguments = CreationParams.PostCreatePythonScriptAdditionalArguments;
		...
		ExCreationParams.Descriptor.bEnableSceneGraphAnimation = CreationParams.bEnableSceneGraphAnimation;
	...
	FString GetTemplatePluginNames(const TArray<FString> TemplateFolders)
    {
    	FString PluginNames;
    	for (const FString& TemplateFolder : TemplateFolders)
    	{
        	TArray<FString> UPluginFiles;
        	IFileManager::Get().FindFiles(UPluginFiles, *TemplateFolder, TEXT("*.uplugin"));
    		...
    		for (const FString& UPluginFile : UPluginFiles)
    		{
				...
				// Strip the .uplugin extension from each file
                PluginNames += FPaths::GetBaseFilename(UPluginFile);
			}
    	}
	...
	bool RunPostCreatePythonScript(const FString& PluginName, const FString& PluginFolder, const TArray<FString> TemplateFolders, const FString& ScriptPath, const FString& TemplateArgs, const FString& AdditionalArgs, const bool IsEnginePlugin, FText& OutFailReason)
	{
#if COMPILE_PYTHON
		const TSharedPtr<IPlugin> PythonPlugin = IPluginManager::Get().FindPlugin(TEXT("PythonScriptPlugin"));
		if (!PythonPlugin.IsValid() || !PythonPlugin->IsEnabled())
		{
			OutFailReason = LOCTEXT("PostCreatePythonNotAvailable", "PythonScriptPlugin not loaded and enabled. Post Create Python Script cannot run without it.");
			return false;
		}
		...
		{
			OutFailReason = LOCTEXT("PostCreatePythonModuleNotAvailable", "PythonScriptPlugin module not found. Post Create Python Script cannot run without it.");
			return false;
		}
		
		if (!PythonScriptPlugin->IsPythonAvailable())
		{
			OutFailReason = LOCTEXT("PostCreatePythonNotAvailableOnTarget", "Engine does not have Python support enabled. Python is required to run Post Create python scripts.");
			return false;
		}
		
		if (!PythonScriptPlugin->IsPythonInitialized())
		{
			OutFailReason = LOCTEXT("PostCreatePythonNotInitialized", "PythonScriptPlugin loaded and enabled but not initialized. Wait until it is initialized and try again.");
			return false;
		}
		
		const FString AbsScriptPath = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*ScriptPath);
		if (!FPaths::FileExists(AbsScriptPath))
		{
			OutFailReason = FText::Format(LOCTEXT("PostCreateScriptNotFound", "Post Create Python Script not found in: {0}"), FText::FromString(AbsScriptPath));
			return false;
		}
		
		const FString PluginRoot = IFileManager::Get().ConvertToAbsolutePathForExternalAppForRead(*PluginFolder);
		if (!FPaths::DirectoryExists(PluginRoot))
		{
			OutFailReason = FText::Format(LOCTEXT("PostCreatePluginNotFound", "Plugin {0} not found in: {1}"), FText::FromString(PluginName), FText::FromString(PluginRoot));
			return false;
		}
		
		auto ValidateArgsBlob = [](const FString& In, FText& OutError) -> bool
			{
				if (In.IsEmpty())
				{
				...
				{
					const TCHAR Char = In[i];
					
					// Invalid characters
					if (Char == '\0' || Char == '\r' || Char == '\n')
					{
						OutError = FText::Format(LOCTEXT("PostCreatePythonForbiddenCharsInArg", "Arguments passed in (\"{0}\") contain forbidden control characters."), FText::FromString(In));
						return false;
					}
				...
				if (bInQuotes)
				{
					OutError = FText::Format(LOCTEXT("PostCreatePythonUnbalancedQuotes", "Arguments passed in (\"{0}\") contain unbalanced quotes."), FText::FromString(In));
					return false;
				}
		...
		if (!ValidateArgsBlob(TemplateArgs, OutFailReason) || !ValidateArgsBlob(AdditionalArgs, OutFailReason))
		{
			return false;
		}
		
		TArray<FString> Argv;
		Argv.Add(AbsScriptPath);
		
		Argv.Add(TEXT("--PluginName"));
		Argv.Add(PluginName);
		
		Argv.Add(TEXT("--PluginRoot"));
		Argv.Add(PluginRoot);
		
		Argv.Add(TEXT("--IsEnginePlugin"));
		Argv.Add(IsEnginePlugin ? TEXT("1") : TEXT("0"));
		
		for (const FString& Folder : TemplateFolders)
		{
			Argv.Add(TEXT("--TemplatePath"));
			Argv.Add(FPaths::ConvertRelativePathToFull(Folder));
		}
		
		auto TokenizeArgsString = [](const FString& In, TArray<FString>& OutArgv) -> void
			{
				if (In.IsEmpty())
				{
				...
				const TCHAR* Stream = *In;
				FString Arg;
				while(FParse::Token(Stream, Arg, /*bUseEscape=*/ true))
				{
					OutArgv.Add(Arg);
				}
			};
			
		TokenizeArgsString(TemplateArgs, Argv);
		TokenizeArgsString(AdditionalArgs, Argv);
		
		auto EscapeAndQuote = [](const FString& In) -> FString
			{
				const bool bAlreadyQuoted = In.Len() >= 2 && In.StartsWith(TEXT("\"")) && In.EndsWith(TEXT("\""));
				FString InternalString = bAlreadyQuoted ? In.Mid(1, In.Len() - 2) : In;
				
				// Escape content
				InternalString = InternalString.Replace(TEXT("\\"), TEXT("\\\\"));
				InternalString = InternalString.Replace(TEXT("\""), TEXT("\\\""));
				
				const bool bNeedsQuotes = InternalString.Contains(TEXT("\"")) || InternalString.Contains(TEXT(" ")) || InternalString.Contains(TEXT("\t"));				
				return bNeedsQuotes ? FString::Printf(TEXT("\"%s\""), *InternalString) : InternalString;
			};
		
		FString CommandLine;
		for (const FString& Arg : Argv)
		{
			CommandLine += EscapeAndQuote(Arg) + TEXT(" ");
		}
		
		FPythonCommandEx PythonCommand;
		PythonCommand.ExecutionMode = EPythonCommandExecutionMode::ExecuteFile;
		PythonCommand.Flags |= EPythonCommandFlags::Unattended;
		PythonCommand.Command = CommandLine;
		
		FScopedSlowTask Progress(/*InAmountOfWork=*/ 1.f, LOCTEXT("RunningPostCreatePython", "Running post-create Python script..."), /*bInEnabled=*/ true, *GWarn);
		Progress.MakeDialog(/*bShowCancelButton=*/ false);
		Progress.EnterProgressFrame(1.f);
		
		UE_LOGF(LogPluginUtils, Display, "Running python post-create script...");
		const bool bSuccess = PythonScriptPlugin->ExecPythonCommandEx(PythonCommand);
		UE_LOGF(LogPluginUtils, Display, "Python post-create script process completed.");
		
		for (const FPythonLogOutputEntry& LogOutputEntry : PythonCommand.LogOutput)
		{
			switch (LogOutputEntry.Type)
			{
			case EPythonLogOutputType::Error:
				UE_LOGF(LogPluginUtils, Error, "[PythonPostCreate] %ls", *LogOutputEntry.Output);
				break;
			case EPythonLogOutputType::Warning:
				UE_LOGF(LogPluginUtils, Warning, "[PythonPostCreate] %ls", *LogOutputEntry.Output);
				break;
			case EPythonLogOutputType::Info:
				UE_LOGF(LogPluginUtils, Display, "[PythonPostCreate] %ls", *LogOutputEntry.Output);
				break;
			}
		}
		...
		if (!bSuccess)
		{
			OutFailReason = LOCTEXT("PostCreatePythonFailed", "Post Create Python Script failed. Check log for details.");
			return false;
		}
	...
	TSharedPtr<IPlugin> CreateAndLoadNewPluginInternal(const FString& PluginName, const FString& NameToReplace, const FString& PluginLocation, const FString& PluginFilePath, const FPluginUtils::FNewPluginParamsWithDescriptor& CreationParams, FPluginUtils::FLoadPluginParams& LoadParams)
	{
		...
		if (!CreationParams.PostCreatePythonScriptPath.IsEmpty())
		{
			const bool bIsEnginePlugin = FPaths::IsUnderDirectory(PluginFilePath, FPaths::EnginePluginsDir());
				
			FText OutPythonScriptFailReason;
			const bool bPythonOk = RunPostCreatePythonScript(
				PluginName,
				PluginFolder,
				CreationParams.TemplateFolders,
				CreationParams.PostCreatePythonScriptPath,
				CreationParams.PostCreatePythonScriptArguments,
				CreationParams.PostCreatePythonScriptAdditionalArguments,
				bIsEnginePlugin,
				OutPythonScriptFailReason
			);
			...
			if (!bPythonOk)
			{
				if (LoadParams.OutFailReason)
				{
					*LoadParams.OutFailReason = OutPythonScriptFailReason;
				}
...
bool FPluginUtils::UnloadPlugins(const TConstArrayView<TSharedRef<IPlugin>> Plugins, FText* OutFailReason /*= nullptr*/)
{
...

	{
		...
		if (!UnloadPluginsAssets(Plugins, &ErrorMsg))
		{
			...
			UE_LOGF(LogPluginUtils, Error, "Failed to unload some assets prior to unmounting plugins\n%ls", *ErrorMsg.ToString());
		}
	}
	...
	bool bSuccess = true;
	{
		...
		for (const TSharedRef<IPlugin>& Plugin : Plugins)
		{
			if (Plugin->IsMounted())
			{
				...
				{
					UE_LOGF(LogPluginUtils, Error, "Plugin %ls cannot be unloaded: %ls", *Plugin->GetName(), *FailReason.ToString());
...
bool FPluginUtils::UnloadPluginsAssets(const TSet<FString>& PluginNames, FText* OutFailReason /*= nullptr*/)
{
	...
	if (!PluginNames.IsEmpty())
	{
		...
		UE_LOGF(LogPluginUtils, Log, "Unloading assets from %d plugins took %0.2f sec", PluginNames.Num(), FPlatformTime::Seconds() - StartTime);
	}
	return bSuccess;
}
