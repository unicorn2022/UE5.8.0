...
public:
	...
	struct FNewPluginParams
	{
		...
		bool bEnableSceneGraphAnimation = false;
		...
		/**
		 * Strings used to run a python script once the plugin has been created.
		 * If PostCreatePythonScriptPath is empty, it will not run anything.
		 */
		FString PostCreatePythonScriptPath;
		FString PostCreatePythonScriptArguments;
		FString PostCreatePythonScriptAdditionalArguments;
	};
	 ...
	 * @note LoadParams.OutFailReason outputs the reason the plugin creation or loading failed (if applicable)
	 ...
	 */
	static PLUGINUTILS_API TSharedPtr<IPlugin> LoadPlugin(const FString& PluginName, const FString& PluginLocation, FLoadPluginParams& LoadParams);
	 ...
	 */
	static PLUGINUTILS_API TSharedPtr<IPlugin> FindLoadedPlugin(const FString& PluginDescriptorFileName);
