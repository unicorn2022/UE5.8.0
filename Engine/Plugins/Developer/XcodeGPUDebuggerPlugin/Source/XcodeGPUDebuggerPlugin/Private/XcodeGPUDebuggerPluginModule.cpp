...
public:
	...
	static void BeginFrameCapture(const FString& CaptureFileName)
	{
		...
		MTL::Device* MetalDevice = GetIMetalDynamicRHI()->RHIGetDevice();
		...
		if (!CaptureManager->startCapture(CaptureDescriptor,&Error))
		{
			...
			UE_LOGF(XcodeGPUDebuggerPlugin, Warning, "Failed to start capture (%ls)", *ErrorString);
		}
	}

	static void EndFrameCapture(FRHICommandListImmediate& RHICmdList)
	{
...
{
	UE_LOGF(XcodeGPUDebuggerPlugin, Log, "Creating dummy input device (for intercepting engine ticks)");
...
void FXcodeGPUDebuggerPluginModule::StartupModule()
{
	...
	if (GUsingNullRHI)
	{
		UE_LOGF(XcodeGPUDebuggerPlugin, Display, "Xcode GPU Debugger plugin will not be loaded (headless UE4 instance).");
		return;
	}
	...
	UE_LOGF(XcodeGPUDebuggerPlugin, Log, "Xcode GPU Debugger plugin is ready!");
#endif // !UE_BUILD_SHIPPING
}

void FXcodeGPUDebuggerPluginModule::BeginFrameCapture(const FString& InCaptureFileName)
{
	UE_LOGF(XcodeGPUDebuggerPlugin, Log, "Capturing the frame and running Xcode (to file '%ls.gputrace')", *InCaptureFileName);
...
void FXcodeGPUDebuggerPluginModule::EndFrameCapture(void* HWnd, uint32 Flags, const FString& DestFileName)
{
	ENQUEUE_RENDER_COMMAND(EndXcodeGPUDebuggerCapture)(
		...
		{
			FXcodeGPUDebuggerFrameCapturer::EndFrameCapture(RHICmdList);
