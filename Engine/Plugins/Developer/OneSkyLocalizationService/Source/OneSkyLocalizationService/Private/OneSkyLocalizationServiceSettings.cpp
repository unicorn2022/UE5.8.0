...
void FOneSkyLocalizationServiceSettings::LoadSettings()
{
	...
	if (ConnectionInfo.ApiKey.IsEmpty() && ConnectionInfo.ApiSecret.IsEmpty())
	{
		...
		for (FString CredentialsFile : CredentialsFileNames)
		{
			...
			{
				UE_LOGF(LogLocalizationService, Log, "Failed to load OneSky credentials file %ls.", *CredentialsFile);
				continue;
			}
			...
			{
				UE_LOGF(LogLocalizationService, Log, "Invalid JSON in OneSky credentials file %ls.", *CredentialsFile);
				continue;
			}
			...
			for (TSharedPtr<FJsonValue> CredentialsItem : CredentialsArray)
			{
				...
				{
					UE_LOGF(LogLocalizationService, Log, "Credentials file %ls is missing ApiKey for connection name %ls", *CredentialsFile, *ConnectionInfo.Name);
				}
				...
				{
					UE_LOGF(LogLocalizationService, Log, "Credentials file %ls is missing ApiSecret for connection name %ls", *CredentialsFile, *ConnectionInfo.Name);
				}
