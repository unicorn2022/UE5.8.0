...
bool FCLionSourceCodeAccessor::OpenFileAtLine(const FString& FullPath, int32 LineNumber, int32 ColumnNumber)
{
	...
	if (!Proc.IsValid())
	{
		UE_LOGF(LogCLionAccessor, Warning, "Opening file (%ls) at a specific line failed.", *Path);
...
bool FCLionSourceCodeAccessor::OpenSourceFiles(const TArray<FString>& AbsoluteSourcePaths)
{
	...
	if (!Proc.IsValid())
	{
		UE_LOGF(LogCLionAccessor, Warning, "Opening the source file (%ls) failed.", *SourceFilesList);
		FPlatformProcess::CloseProc(Proc);
		return false;
	}
