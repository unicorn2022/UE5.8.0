...
void FSourceFilterPresets::LoadPreset(const FSoftObjectPath& PresetPath)
{
	if (USourceFilterCollection* Preset = Cast<USourceFilterCollection>(PresetPath.TryLoad()))
	{
		...
		UE_LOGF(SourceFilterPresets, Display, "Loaded Trace Source Filter preset: %ls\n", *PresetPath.ToString());
	}
	else
	{
		// Unable to load
		UE_LOGF(SourceFilterPresets, Display, "Failed to find preset: %ls\n", *PresetPath.ToString());
	}
}
...
void FSourceFilterPresets::LoadPresetCommand(const TArray<FString>& Arguments)
{
	...
	if (Arguments.Num() == 1)
	{
		...
		if (LexTryParseString(Index, *Arguments[0]))
		{
			...
			{
				UE_LOGF(SourceFilterPresets, Display, "Invalid preset index\n");
	...
	{
		UE_LOGF(SourceFilterPresets, Display, "Expecting a single argument:\n"
			"Integer: index of preset within Trace.ListPresets output\n"
			"or\n"
			"String: path to UTraceFilterCollection object\n");
	}
}
