...
void FGitSourceControlProvider::CheckRepositoryStatus(const FString& InPathToGitBinary)
{
	...
	if(bGitRepositoryFound)
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "'%ls' is not a valid Git repository", *PathToRepositoryRoot);
		}
	}
	else
	{
		UE_LOGF(LogSourceControl, Warning, "'%ls' is not part of a Git repository", *FPaths::ProjectDir());
	}
