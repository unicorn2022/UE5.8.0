...
FGitScopedTempFile::FGitScopedTempFile(const FText& InText)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "Failed to write to temp file: %ls", *Filename);
	}
}
...
FGitScopedTempFile::~FGitScopedTempFile()
{
	if(FPaths::FileExists(Filename))
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "Failed to delete temp file: %ls", *Filename);
		}
	}
}
...
static bool RunCommandInternalRaw(const FString& InCommand, const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InParameters, const TArray<FString>& InFiles, FString& OutResults, FString& OutErrors)
{
...
#if UE_BUILD_DEBUG
	UE_LOGF(LogSourceControl, Log, "RunCommandInternalRaw: 'git %ls'", *LogableCommand);
#endif
	...
	{
		UE_LOGF(LogSourceControl, Log, "RunCommandInternalRaw: 'OutResults=n/a'");
	}
	else
	{
		UE_LOGF(LogSourceControl, Log, "RunCommandInternalRaw(%ls): OutResults=\n%ls", *InCommand, *OutResults);
	}
	...
	if(ReturnCode != 0)
	{
		...
		{
			UE_LOGF(LogSourceControl, Warning, "RunCommandInternalRaw: 'OutErrors=n/a'");
		}
		else
		{
			UE_LOGF(LogSourceControl, Warning, "RunCommandInternalRaw(%ls): OutErrors=\n%ls", *InCommand, *OutErrors);
		}
	}
#endif
...
bool RunDumpToFile(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InParameter, const FString& InDumpFileName)
{
	...
	if(ProcessHandle.IsValid())
	{
		...
		if(ReturnCode == 0)
		{
			...
			{
				UE_LOGF(LogSourceControl, Log, "Writed '%ls' (%do)", *InDumpFileName, BinaryFileContent.Num());
			}
			else
			{
				UE_LOGF(LogSourceControl, Error, "Could not write %ls", *InDumpFileName);
				ReturnCode = -1;
			}
		}
		else
		{
			UE_LOGF(LogSourceControl, Error, "DumpToFile: ReturnCode=%d", ReturnCode);
		}
	...
	{
		UE_LOGF(LogSourceControl, Error, "Failed to launch 'git cat-file'");
	}
