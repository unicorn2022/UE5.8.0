...
void FPixWinPluginModule::StartupModule()
{
	...
	if (PixGraphicsAnalysisInterface->IsValid())
	{
		...
		FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FPixWinPluginModule::OnPostEngineInit);

		UE_LOGF(PixWinPlugin, Log, "PIX capture plugin is ready!");
	}
	else
#endif
	{
		UE_LOGF(PixWinPlugin, Log, "PIX capture plugin failed to initialize! Check that the process is launched from PIX.");
	}
}
...
{
	UE_LOGF(PixWinPlugin, Log, "Creating dummy input device (for intercepting engine ticks)");
...
void FPixWinPluginModule::DoFrameCaptureCurrentViewport(FViewport* InViewport, uint32 InFlags, const FString& InDestFileName)
{
	...
	if (!Viewport && GEditor)
	{
		...
		if (!Viewport)
		{
			const TArray<class FEditorViewportClient*>& ViewportClients = GEditor->GetAllViewportClients();
			if (ViewportClients.Num() > 0)
			{
				Viewport = ViewportClients[0]->Viewport;
			}
		}
	}
#endif // WITH_EDITOR
...
{
#if PIX_PLUGIN_ENABLED
	UE_LOGF(PixWinPlugin, Log, "Capturing a frame in PIX");
...
void FPixWinPluginModule::EndFrameCapture(uint32 InFlags, const FString& DestFileName)
{
	...
	ENQUEUE_RENDER_COMMAND(PixWinEndFrameCapture)(
		[Pix, InFlags, DestFileName](FRHICommandListImmediate& RHICommandList)
		{
			if (Pix && Pix->IsValid())
			{
				RHICommandList.SubmitAndBlockUntilGPUIdle();
				Pix->EndCapture();
