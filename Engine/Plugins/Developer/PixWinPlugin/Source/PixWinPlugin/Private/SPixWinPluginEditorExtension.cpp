...
void FPixWinPluginEditorExtension::Initialize(FPixWinPluginModule* ThePlugin)
{
	if (GUsingNullRHI)
	{
		UE_LOGF(PixWinPlugin, Display, "PixWin Plugin will not be loaded because a Null RHI (Cook Server, perhaps) is being used.");
		return;
	}
	...
	if (!IsRunningGame())
	{
		...
		FToolMenuEntry& Entry = RightSection.AddMenuEntry("PixWinCaptureFrame", FPixWinPluginCommands::Get().CaptureFrame);
