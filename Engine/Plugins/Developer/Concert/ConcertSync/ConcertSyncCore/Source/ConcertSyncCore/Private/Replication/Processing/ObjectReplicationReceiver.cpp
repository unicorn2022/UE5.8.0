...
namespace UE::ConcertSyncCore
{
	...
	void FObjectReplicationReceiver::HandleBatchReplicationEvent(const FConcertSessionContext& SessionContext, const FConcertReplication_BatchReplicationEvent& Event)
	{
		...
		{
			UE_LOGF(LogConcert, Log, "Received %u streams with %u object changes from endpoint %ls. Cached %u object changes with a total of new %u cache insertions and %u cache updates.",
				Event.Streams.Num(),
				NumObjects,
			...
			UE_CLOGF(NumRejectedObjectChanges > 0, LogConcert, Warning, "Rejected %d object changes.", NumRejectedObjectChanges);
		}
	}
}
