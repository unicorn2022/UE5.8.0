...
IConcertFileSharingService* FConcertLocalFileSharingService::Clone() const
{
	return new FConcertLocalFileSharingService(*this);
}

bool FConcertLocalFileSharingService::IsEnabled() const
{
	// If the user has specified a local file sharing service then we are always enabled.  
	return true;
}

void FConcertLocalFileSharingService::SetEnabled(bool bInEnabled)
{
	// We do not support disabling this service. If the user has specified a local file service then we always use it.
}

void FConcertLocalFileSharingService::SetPath(const FString& Path)
{
	// We do not support setting althernative paths with this file service. 
}

FString FConcertLocalFileSharingService::GetPath() const
{
	return SharedRootPathname;
}

void FConcertLocalFileSharingService::Delete(const FString& OutFileId)
{
	// This class does not handle the delete method.
	UE_LOGF(LogConcert, Warning, "LocalFileSharingService - Delete is not supported with this file sharing service. Leaving %ls inplace.", *OutFileId);
}
...
TSharedPtr<FArchive> FConcertLocalFileSharingService::CreateWriter(FString& OutFileId)
{
	OutFileId = SharedRootPathname / FGuid::NewGuid().ToString();
