...
struct FCompressionMethodAndBias
{
	FCompressionMethodAndBias() = default;
	FCompressionMethodAndBias(EConcertCompressionMethod InMethod, EConcertCompressionBias InBias)
		:  Method(InMethod)
		  ,Bias(InBias)
	{
	}
	...
	EConcertCompressionMethod Method = EConcertCompressionMethod::Uncompressed;
	EConcertCompressionBias Bias = EConcertCompressionBias::None;


	/** Non-shifted conversion into a int32 value. This version is used when storing into the database record as all values are int32 */
	uint32 ToInt32() const
	{
		const uint32 CompressionMethodAndBias = static_cast<int32>(Method) | static_cast<int32>(Bias);
		return CompressionMethodAndBias;
	}

	/** Non-shifted conversion into a enum types. This version is used when reading a int32 table entry that corresponds to the compression flags. */
	void FromInt32(int32 InCompressionAndBias)
	{
		Method = static_cast<EConcertCompressionMethod>(InCompressionAndBias & (static_cast<int32>(EConcertCompressionMethod::Zlib) | static_cast<int32>(EConcertCompressionMethod::Oodle)));
		Bias = static_cast<EConcertCompressionBias>(InCompressionAndBias & (static_cast<int32>(EConcertCompressionBias::Speed) | static_cast<int32>(EConcertCompressionBias::Size)));
		check(IsValid());
	}

	/** Non-shifted conversion into a uint32 value. This version is used when serializing into a transaction record. */
	uint32 ToUint32() const
	{
		const uint32 CompressionMethodAndBias = static_cast<uint32>(Method) | static_cast<uint32>(Bias);
		return CompressionMethodAndBias;
	}

	/** Non-shifted conversion into a enum types. This version is used when deserializing from a transaction record. */
	void FromUint32(uint32 InCompressionAndBias)
	{
		Method = static_cast<EConcertCompressionMethod>(InCompressionAndBias & (static_cast<uint32>(EConcertCompressionMethod::Zlib) | static_cast<uint32>(EConcertCompressionMethod::Oodle)));
		Bias = static_cast<EConcertCompressionBias>(InCompressionAndBias & (static_cast<uint32>(EConcertCompressionBias::Speed) | static_cast<uint32>(EConcertCompressionBias::Size)));
		check(IsValid());
	}

	bool IsValid()
	{
		const bool bMethodValid = (Method == EConcertCompressionMethod::Uncompressed
								|| Method == EConcertCompressionMethod::Oodle
								|| Method == EConcertCompressionMethod::Zlib);
		const bool bBiasValid = ( Bias == EConcertCompressionBias::None
								|| Bias == EConcertCompressionBias::Speed
								|| Bias == EConcertCompressionBias::Size);
		return bMethodValid && bBiasValid;
	}

	/** Shifted conversion which is used when writing the package blob. We write this in the 24 MSB of uint32 */
	uint32 ToUint32_Shifted() const
	{
		const uint32 CompressionMethodAndBias = static_cast<uint8>(Method) | static_cast<uint8>(Bias);
		return CompressionMethodAndBias << 8;
	}

	/** Convert a method and bias value that was stored in the 24 MSB back into enum values. */
	void FromUint32_Shifted(uint32 InSerializedDataVersion)
	{
		const uint32 CompressionAndBias = (InSerializedDataVersion >> 8);
	
		Method = static_cast<EConcertCompressionMethod>(CompressionAndBias & (static_cast<uint32>(EConcertCompressionMethod::Zlib) | static_cast<uint32>(EConcertCompressionMethod::Oodle)));
		Bias = static_cast<EConcertCompressionBias>(CompressionAndBias & (static_cast<uint32>(EConcertCompressionBias::Speed) | static_cast<uint32>(EConcertCompressionBias::Size)));
		check(IsValid());
	}
};
...
bool WriteTransaction(const FStructOnScope& InTransaction, TArray<uint8>& OutSerializedTransactionData)
{
	...
	{
		FCompressionMethodAndBias MethodAndBias(UE::Concert::Compression::GetCompressionMethod(UncompressedTransactionSize),
												UE::Concert::Compression::GetConcertCompressionBias(UE::Concert::Compression::GetCompressionFlags()));  

		uint32 StoredFormat = MethodAndBias.ToUint32();
		Ar.SerializeIntPacked(StoredFormat);
		if (UE::Concert::Compression::DataIsCompressed(MethodAndBias.Method))
		{
			Ar.SerializeCompressed(UncompressedTransaction.GetData(), UncompressedTransactionSize,
								   UE::Concert::Compression::GetCompressionAlgorithm(MethodAndBias.Method),
								   UE::Concert::Compression::GetCoreCompressionFlags(MethodAndBias.Bias));
		}
		else
		{
...
bool ReadTransaction(const TArray<uint8>& InSerializedTransactionData, FStructOnScope& OutTransaction)
{
	...
	if (UncompressedTransactionSize > 0)
	{
		...
		FCompressionMethodAndBias Format(EConcertCompressionMethod::Zlib, EConcertCompressionBias::None);
		if (SerializedDataVersion > 1)
		{
			uint32 SerializedDataFormat = 0;
			Ar.SerializeIntPacked(SerializedDataFormat);
			
			Format.FromUint32(SerializedDataFormat);
		}

		if (UE::Concert::Compression::DataIsCompressed(Format.Method))
		{
			Ar.SerializeCompressed(UncompressedTransaction.GetData(), UncompressedTransactionSize,
								   UE::Concert::Compression::GetCompressionAlgorithm(Format.Method),
								   UE::Concert::Compression::GetCoreCompressionFlags(Format.Bias));
		}
		else
		{
			Ar.Serialize(UncompressedTransaction.GetData(), UncompressedTransactionSize);
...
void WritePackageBlobVersionInfo(FArchive& Ar, FCompressionMethodAndBias InCompressionMethodAndBias)
{
	...
	uint32 SerializedDataVersion = static_cast<uint32>(PackageDataUtil::DataVersion) | InCompressionMethodAndBias.ToUint32_Shifted();
	Ar.SerializeIntPacked(SerializedDataVersion);
}


/** Reads the first 4 bytes of the archive and detect how the package data was written in the blob. */
FCompressionMethodAndBias ParsePackageDataFormat(FArchive& Ar)
{
	...
	if (ExtractedDataVersion == 1)
	{
		...
		return FCompressionMethodAndBias(EConcertCompressionMethod::Zlib, EConcertCompressionBias::None);
	}

	// In Version 2, the package data format was encoded after the 8 LSB used for 'DataVersion'
	FCompressionMethodAndBias MethodAndBias;
	MethodAndBias.FromUint32_Shifted(SerializedDataVersion & 0xFFFFFF00);
	return MethodAndBias;
}
...
void WritePackageHeader(FArchive& Ar, EConcertCompressionMethod CompressionMethod, EConcertCompressionBias CompressionBias)
{
	// First 4-bytes contains information about the format.
	FCompressionMethodAndBias PackageDataFormat(CompressionMethod, CompressionBias);
...
TTuple<FCompressionMethodAndBias, int64> ReadPackageHeader(FArchive& Ar)
{
	// Consume the first bytes to read the package version and package data format.
	FCompressionMethodAndBias PackageDataFormat = ParsePackageDataFormat(Ar);
...
bool WillUseMemoryWriter(int64 DataSize)
{
	...
	return DataSize > 0 && (DataIsCompressed(GetCompressionMethod(DataSize)) ||
							ShouldCachePackageBlob(DataSize + PackageDataUtil::EstimatePackageBlobMetaDataSize()));
}

void WritePackageCompressed(FConcertPackageDataStream& PackageDataStream, FArchive& DstAr, FCompressionMethodAndBias DataFormat)
{
	...
	if (PackageDataStream.DataBlob) // Optimization to avoid transfering from the archive to a temporary buffer.
	{
		DstAr.SerializeCompressed(const_cast<uint8*>(PackageDataStream.DataBlob->GetData()), UncompressedPackageSize,
								  UE::Concert::Compression::GetCompressionAlgorithm(DataFormat.Method),
								  UE::Concert::Compression::GetCoreCompressionFlags(DataFormat.Bias));
	}
	else
	{
...
void WritePackageUncompressed(FConcertPackageDataStream& PackageDataStream, FArchive& DstAr)
{
	...
	if (PackageDataStream.DataSize > 0)
	{
		...
		{
			UE_LOGF(LogConcert, Error, "Failed copy package data stream to destination.");
		}
	}
}
...
WritePackageResult WritePackage(FConcertPackageDataStream& PackageDataStream, FArchive* DstAr)
{
	...
	EConcertCompressionMethod CompressionMethod = UE::Concert::Compression::GetCompressionMethod(PackageDataStream.DataSize);
	EConcertCompressionBias CompressionBias = UE::Concert::Compression::GetConcertCompressionBias(UE::Concert::Compression::GetCompressionFlags());
	...
	WritePackageHeader(*DstAr, CompressionMethod, CompressionBias);
	if (UE::Concert::Compression::DataIsCompressed(CompressionMethod))
	{
		WritePackageCompressed(PackageDataStream, *DstAr, FCompressionMethodAndBias(CompressionMethod, CompressionBias));
...
bool ExtractPackageData(FArchive& PackageBlobAr, const TFunctionRef<void(FConcertPackageDataStream&)>& PackageDataStreamFallbackFn)
{
	...
	// Read the header.
	TTuple<FCompressionMethodAndBias, int64> Header = ReadPackageHeader(PackageBlobAr);
	FCompressionMethodAndBias PackageFormat = Header.Get<0>();
	...
	if (UE::Concert::Compression::DataIsCompressed(PackageFormat.Method))
	{
		...
		if (UncompressedPackageSize > 0)
		{
			...
			PackageBlobAr.SerializeCompressed(DecompressedData.GetData(), UncompressedPackageSize,
											  UE::Concert::Compression::GetCompressionAlgorithm(PackageFormat.Method),
											  UE::Concert::Compression::GetCoreCompressionFlags(PackageFormat.Bias));
		}
	...
	{
		check(UE::Concert::Compression::DataIsUncompressed(PackageFormat.Method));
...
{
	FCompressionMethodAndBias Converted;
	Converted.FromInt32(Flags);
	
	Data.PayloadCompressionMethod = Converted.Method;
	Data.PayloadCompressionBias = Converted.Bias;
}

template <typename SerializedDataT>
int32 ConvertSerializedPayloadFlagsToInt32(const SerializedDataT& Data)
{
	FCompressionMethodAndBias MethodAndBias(Data.PayloadCompressionMethod, Data.PayloadCompressionBias);
	return MethodAndBias.ToInt32();
}
...
bool FConcertSyncSessionDatabase::Open(const FString& InSessionPath, const ESQLiteDatabaseOpenMode InOpenMode)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to open session database for '%ls': %ls", *InSessionPath, *GetLastError());
		return false;
	}
	...
	{
		Close();
		UE_LOGF(LogConcert, Error, "Failed to open session database for '%ls': Database is too new (version %d, expected <= %d)", *InSessionPath, LoadedDatabaseVersion, (int32)FConcertSyncSessionDatabaseVersion::Current);
		return false;
	}
...
bool FConcertSyncSessionDatabase::Close(const bool InDeleteDatabase)
{
	...
	if (!Database->Close())
	{
		UE_LOGF(LogConcert, Error, "Failed to close session database for '%ls': %ls", *SessionPath, *GetLastError());
		return false;
	}
...
bool HandleWritePackageResult(const FName& PackageName, const FString& InDstPackageBlobPathname, TUniquePtr<FConcertFileCache>& Cache, PackageDataUtil::WritePackageResult Result)
{
	auto SaveResult = [](const FString& InDstPackageBlobPathname, TUniquePtr<FConcertFileCache>& Cache, PackageDataUtil::WritePackageResult Result)
	{
		if (Result.CacheData.Num()>0)
		{
			...
			UE_LOGF(LogConcert, Warning, "Failed to handle package write result of file '%ls'.", *InDstPackageBlobPathname);
			return false;
		}
		return true;
	};
...
bool FConcertSyncSessionDatabase::LoadPackage(const FString& InPackageBlobFilename, const TFunctionRef<void(FConcertPackageDataStream&)>& PackageDataStreamFn) const
{
	...
	{
		UE_LOGF(LogConcert, Warning, "Failed to read the size of database file '%ls'.", *InPackageBlobFilename);
		return false;
	}
...
void FConcertSyncSessionDatabase::UpdateAsynchronousTasks()
{
	for (auto It = DeferredLargePackageIOPtr->GetMap().CreateIterator(); It; ++It)
	{
		if (It->Value->AsyncTask.IsCompleted())
		{
			...
			{
				UE_LOGF(LogConcert, Error, "Async task failed to write package %ls", *It->Value->CachedPackageName.ToString());
			}
			It.RemoveCurrent();
		}
	}
