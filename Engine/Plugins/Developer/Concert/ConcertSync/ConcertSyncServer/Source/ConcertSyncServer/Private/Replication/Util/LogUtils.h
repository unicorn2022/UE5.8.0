...
namespace UE::ConcertSyncServer
{
	...
	void LogNetworkMessage(const TMessage& Message, TGetClientName GetClientName = [](){ return TOptional<FString>{}; })
	{
		...
		{
			UE_LOGF(LogConcert, Log, "%ls from %ls\n%ls", *TMessage::StaticStruct()->GetName(), **ClientName, *JsonString);
		}
		else
		{
			UE_LOGF(LogConcert, Log, "%ls\n%ls", *TMessage::StaticStruct()->GetName(), *JsonString);
		}
	}
