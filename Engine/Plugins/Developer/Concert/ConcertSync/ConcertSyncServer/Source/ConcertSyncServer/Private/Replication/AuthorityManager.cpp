...
namespace UE::ConcertSyncServer::Replication
{
		...
		)
	{
		...
		UE_CLOGF(Request.TakeAuthority.IsEmpty() && Request.ReleaseAuthority.IsEmpty(), LogConcert, Warning, "Received invalid authority request (TakeAuthority.Num() == 0 and ReleaseAuthority.Num() == 0)");
		...
		)
	{
		...
		Private::ForEachReplicatedObject(Request.TakeAuthority, [this, &OutRejectedObjects, bShouldLog, &bMadeChanges, &AuthorityData, &ClientId](const FStreamId& StreamId, const FSoftObjectPath& ObjectPath)
		{
			...
			{
				UE_CLOGF(bShouldLog, LogConcert, Log, "Transferred authority of %ls to client %ls for their stream %ls", *ObjectPath.ToString(), *ClientId.ToString(EGuidFormats::Short), *StreamId.ToString(EGuidFormats::Short));
				AuthorityData.OwnedObjects.FindOrAdd(StreamId).Add(ObjectPath);
				bMadeChanges = true;
			}
			else
			{
				UE_CLOGF(bShouldLog, LogConcert, Log, "Rejected %ls request of authority over %ls in stream %ls", *ClientId.ToString(EGuidFormats::Short), *ObjectPath.ToString(), *StreamId.ToString(EGuidFormats::Short));
