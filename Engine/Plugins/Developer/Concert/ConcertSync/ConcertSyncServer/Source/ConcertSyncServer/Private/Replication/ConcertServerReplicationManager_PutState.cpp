...
namespace UE::ConcertSyncServer::Replication
{
	namespace Private
	{
			...
			)
		{
			...
			UE_CLOGF(!bSuccess, LogConcert, Error, "Failed to apply mute request as part of put state request from %ls", *RequestingEndpointId.ToString());
		}
	}
	...
	)
	{
		for (const TPair<FGuid, FConcertObjectInStreamArray>& AuthorityPair : Request.NewAuthorityState)
		{
			...
			UE_CLOGF(!RejectedObjects.IsEmpty(), LogConcert, Warning, "Authority portion for client %ls was not validated correctly for request %ls.", *EndpointId.ToString(), *Private::RequestToString(Request));
#endif
		}
	}
}
