...
FConcertServerWorkspace::FConcertServerWorkspace(const TSharedRef<FConcertSyncServerLiveSession>& InLiveSession, TSharedPtr<IConcertFileSharingService> InFileSharingService)
	: FileSharingService(InFileSharingService ? InFileSharingService->Clone() : nullptr)
...
void FConcertServerWorkspace::BindSession(const TSharedRef<FConcertSyncServerLiveSession>& InLiveSession)
{
	...
	{
		FString BasePath = FileSharingService->GetPath();
		FString FullPath = BasePath / LiveSession->GetSession().GetName() / TEXT("Server") / LiveSession->GetSession().GetId().ToString();
		FileSharingService->SetPath(FullPath);
	}
}
...
void FConcertServerWorkspace::UnbindSession()
{
	if (LiveSession.IsValid())
	{
		if (FileSharingService)
		{
			FileSharingService->Delete(FileSharingService->GetPath());
		}
...
void FConcertServerWorkspace::SetEndpoint(const FGuid& InEndpointId, const FConcertSyncEndpointData& InEndpointData)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to set endpoint '%ls' on live session '%ls': %ls", *InEndpointId.ToString(), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
void FConcertServerWorkspace::SendSyncEndpointEvent(const FGuid& InTargetEndpointId, const FGuid& InSyncEndpointId, const int32 InNumRemainingSyncEvents) const
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to get endpoint '%ls' from live session '%ls': %ls", *InSyncEndpointId.ToString(), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
void FConcertServerWorkspace::AddConnectionActivity(const FConcertSyncConnectionActivity& InConnectionActivity)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to set connection activity '%ls' on live session '%ls': %ls", *LexToString(ActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
void FConcertServerWorkspace::SendSyncConnectionActivityEvent(const FGuid& InTargetEndpointId, const int64 InSyncActivityId, const int32 InNumRemainingSyncEvents) const
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to get connection activity '%ls' from live session '%ls': %ls", *LexToString(InSyncActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
void FConcertServerWorkspace::AddLockActivity(const FConcertSyncLockActivity& InLockActivity)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to set lock activity '%ls' on live session '%ls': %ls", *LexToString(ActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
void FConcertServerWorkspace::SendSyncLockActivityEvent(const FGuid& InTargetEndpointId, const int64 InSyncActivityId, const int32 InNumRemainingSyncEvents) const
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to get lock activity '%ls' from live session '%ls': %ls", *LexToString(InSyncActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
void FConcertServerWorkspace::AddTransactionActivity(const FConcertSyncTransactionActivity& InTransactionActivity)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to set transaction activity '%ls' on live session '%ls': %ls", *LexToString(ActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
TOptional<FConcertWorkspaceSyncActivityEvent> FConcertServerWorkspace::SyncTransactionActivityEvent(const int64 InSyncActivityId, const int32 InNumRemainingSyncEvents, const bool InLiveOnly) const
{
	...
	if (LiveSession->GetSessionDatabase().GetActivity(InSyncActivityId, SyncActivity))
	{
		...
		{
			UE_LOGF(LogConcert, Error, "Failed to get transaction event '%ls' from live session '%ls': %ls", *LexToString(SyncActivity.EventId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
		}
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to get transaction activity '%ls' from live session '%ls': %ls", *LexToString(InSyncActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
	return {};
}
...
void FConcertServerWorkspace::AddPackageActivity(const FConcertSyncActivity& InPackageActivityBasePart, const FConcertPackageInfo& PackageInfo, FConcertPackageDataStream& InPackageDataStream)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to set package activity '%ls' on live session '%ls': %ls", *LexToString(ActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
...
TOptional<FConcertWorkspaceSyncActivityEvent> FConcertServerWorkspace::MakeSyncActivityEvent(const int64 InSyncActivityId, bool bInHeadOnly) const
{
	...
	if (LiveSession->GetSessionDatabase().GetActivity(InSyncActivityId, SyncActivity))
	{
		...
		if (!bMetaDataOnly) // Package data is needed.
		{
			...
			{
				UE_LOGF(LogConcert, Error,
					   "Failed to get package event '%ls' from live session '%ls': %ls",
		...
		{
			UE_LOGF(LogConcert, Error, "Failed to get package event '%ls' from live session '%ls': %ls",
				   *LexToString(SyncActivity.EventId),
				   *LiveSession->GetSession().GetName(),
				   *LiveSession->GetSessionDatabase().GetLastError());
		}
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to get package activity '%ls' from live session '%ls': %ls",
...
void FConcertServerWorkspace::OnAddReplicationActivity(const int64 ActivityId, const bool bSuccess)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to set replication activity '%ls' on live session '%ls': %ls", *LexToString(ActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}	
}
...
void FConcertServerWorkspace::SendSyncReplicationActivityEvent(const FGuid& InTargetEndpointId, const int64 InSyncActivityId, const int32 InNumRemainingSyncEvents)
{
	...
	{
		UE_LOGF(LogConcert, Error, "Failed to get replication activity '%ls' from live session '%ls': %ls", *LexToString(InSyncActivityId), *LiveSession->GetSession().GetName(), *LiveSession->GetSessionDatabase().GetLastError());
	}
}
	...
	if (LiveSession->GetSessionDatabase().GetActivity(InActivityId, Activity))
	{
		...
		if (LiveSession->GetSessionDatabase().GetEndpoint(Activity.EndpointId, EndpointData))
		{
			...
			if (Activity.EventSummary.GetPayload(ActivitySummary))
			{
				...
				UE_LOGF(LogConcert, Display, "Endpoint '%ls' produced activity '%ls': %ls", *Activity.EndpointId.ToString(), *LexToString(Activity.ActivityId), *ActivitySummaryPtr->ToDisplayText(FText::AsCultureInvariant(EndpointData.ClientInfo.DisplayName)).ToString());
			}
		}
	}
}

bool FConcertServerWorkspace::IsFileSharingEnabled() const
{
	return FileSharingService && FileSharingService->IsEnabled() && EnumHasAnyFlags(LiveSession->GetSessionFlags(), EConcertSyncSessionFlags::EnableFileSharing);
}
...
{
	if (!Package.FileId.IsEmpty() && ensure(FileSharingService))
	{
		return FileSharingService->CreateReader(Package.FileId);
	}
	else if (Package.PackageData.Bytes.Num() > 0)
	{
		return MakeShared<FMemoryReader>(Package.PackageData.Bytes); // Package data is embedded in the activity object.		
	}
