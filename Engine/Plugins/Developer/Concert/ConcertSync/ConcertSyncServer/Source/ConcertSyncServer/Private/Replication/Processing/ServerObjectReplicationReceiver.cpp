...
namespace UE::ConcertSyncServer::Replication
{
		...
		) const
	{
		...
		UE_CLOGF(!bHasAuthority, LogConcert, Verbose, "Dropping %ls because the client does not have authority over it.", *ReplicatedObjectInfo.ToString());
		UE_CLOGF(!bHasSyncControl, LogConcert, Verbose, "Dropping %ls because the client does not have sync control over it.", *ReplicatedObjectInfo.ToString());
