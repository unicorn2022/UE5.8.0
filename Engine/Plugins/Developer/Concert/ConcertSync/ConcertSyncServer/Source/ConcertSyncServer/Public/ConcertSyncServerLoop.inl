...
{
	UE_LOGF(LogSyncServer, Display, "%ls Shutdown", *ServiceFriendlyName);
...
int32 ConcertSyncServerLoop(const TCHAR* CommandLine, const FConcertSyncServerLoopInitArgs& InitArgs)
{
	...
	if (Result < 0)
	{
		UE_LOGF(LogSyncServer, Error, "EngineLoop PreInit failed!");
	...
	if (InitArgs.bShowConsole)
	{
		...
		UE_LOGF(LogSyncServer, Display, "Launch : %ls", CommandLine);
	}
	...
	{
		UE_LOGF(LogSyncServer, Error, "ConcertSyncServer Module is needed for proper execution. Initialization failed!");
	...
	if (!ServerConfig)
	{
		UE_LOGF(LogSyncServer, Error, "%ls Configuration Failed!", *InitArgs.ServiceFriendlyName);
	...
	{
		UE_LOGF(LogSyncServer, Error, "The 'UDP Messaging' and 'QUIC Messaging' plugins are disabled. "
	...
	ConcertSyncServer->SetFileSharingService(InitArgs.GetFileSharingServiceFunc ? InitArgs.GetFileSharingServiceFunc() : InitArgs.FileSharingService);
		...
		if (!ConcertServer->GetLiveSessionIdByName(ServerConfig->DefaultSessionName).IsValid())
		{
			...
			if (!ServerConfig->DefaultSessionToRestore.IsEmpty())
			{
				...
				if (ArchivedSessionId.IsValid())
				{
					...
					if (!bSuccess)
					{
						UE_LOGF(LogSyncServer, Error, "%ls failed to restore archived session '%ls' due to '%ls'. Creating new session instead...", *InitArgs.ServiceFriendlyName, *ServerConfig->DefaultSessionToRestore, *FailureReason.ToString());
					}
				}
				else
				{
					UE_LOGF(LogSyncServer, Error, "%ls could not find archived session '%ls' to restore. Creating new session instead...", *InitArgs.ServiceFriendlyName, *ServerConfig->DefaultSessionToRestore);
				}
			}
			...
			if (!bSuccess)
			{
				...
				if (!bSuccess)
				{
					UE_LOGF(LogSyncServer, Error, "%ls failed to create session '%ls' due to '%ls'.", *InitArgs.ServiceFriendlyName, *ServerConfig->DefaultSessionName, *FailureReason.ToString());
				}
			}
		}
	}

	UE_LOGF(LogSyncServer, Display, "%ls Initialized (Name: %ls, Version: %d.%d, Role: %ls)", *InitArgs.ServiceFriendlyName, *ConcertSyncServer->GetConcertServer()->GetServerInfo().ServerName, ENGINE_MAJOR_VERSION, ENGINE_MINOR_VERSION, *ConcertSyncServer->GetConcertServer()->GetRole());
