...
namespace UE::ConcertSyncClient::Replication
{
	...
	{
		UE_LOGF(LogConcert, Warning, "JoinReplicationSession requested while already in a session");
	...
	TFuture<FConcertReplication_QueryReplicationInfo_Response> FReplicationManagerState_Connected::QueryClientInfo(FConcertReplication_QueryReplicationInfo_Request Args)
	{
		...
		{
			UE_LOGF(LogConcert, Warning, "Request QueryClientInfo is pointless because SkipAuthority and SkipStreamInfo are both set. Returning immediately...");
		...
		)
	{
		...
		for (const TPair<FSoftObjectPath, FConcertStreamArray>& TakeAuthority : Request.TakeAuthority)
		{
			...
			UE_CLOGF(ReplicatedStreams.StreamIds.IsEmpty(), LogConcert, Warning, "Your FConcertReplication_ChangeAuthority_Request::TakeAuthority request contained empty stream ID array for object %ls", *ReplicatedObject.ToString());
