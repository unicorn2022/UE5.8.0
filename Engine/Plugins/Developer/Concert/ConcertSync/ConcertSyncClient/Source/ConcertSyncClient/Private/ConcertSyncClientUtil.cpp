...
bool CanPerformBlockingAction(const bool bBlockDuringInteraction)
{
	...
	return !(bIsInteracting || UE::IsSavingPackage() || IsGarbageCollecting());
}
...
FGetObjectResult GetObject(const FConcertObjectId& InObjectId, const FName InNewName, const FName InNewOuterPath, const FName InNewPackageName, const FSoftObjectPath& InSourceObject, const bool bAllowCreate)
{
	...
	auto AssignExternalPackage = [bIsPackageChange, &ObjectPackageToAssign, &ObjectNameToCreate](UObject* InObject)
	{
		if (bIsPackageChange)
		{
			...
			{
				UE_LOGF(LogConcert, Warning, "Package '%ls' could not be found and assigned to Object '%ls'.", *ObjectPackageToAssign.ToString(), *ObjectNameToCreate.ToString());
			}
		}
	};
	...
	if (UObject* ExistingObjectOuter = FSoftObjectPath(ObjectOuterPathToFind).ResolveObject())
	{
		...
		if (ExistingObject)
		{
			...
			if (bIsRename || bIsOuterChange)
			{
				...
				{
					UE_LOGF(LogConcert, Warning, "Attempted to rename '%ls' over '%ls'. Re-using the found object instead of performing the rename!", *ExistingObject->GetPathName(), *NewObject->GetPathName());
	...
	if (NewObjectOuter)
	{
		...
		if (bAllowCreate)
		{
			...
			if (!InSourceObject.IsNull())
			{
				...
				UE_CLOGF(!SourceObject, LogConcert, Warning, "Failed to find source object '%ls' for '%ls'. This object will be created from its CDO instead.", *InSourceObject.ToString(), *ObjectNameToCreate.ToString());
			}
			if (SourceObject && SourceObject->GetClass() != ObjectClass)
			{
				UE_LOGF(LogConcert, Warning, "Discarding source object '%ls' for '%ls' as it was not the expected class (%ls). This object will be created from its CDO instead.", *InSourceObject.ToString(), *ObjectNameToCreate.ToString(), *ObjectClass->GetPathName());
				...
				if (ObjectClass->IsChildOf<AActor>())
				{
					...
					if (ULevel* OuterLevel = Cast<ULevel>(NewObjectOuter))
					{
						...
						if (OwnerWorld)
						{
							...
							{
								UE_LOGF(LogConcert, Warning, "Actor '%ls' already exists! Expected class: '%ls'", *ExistingObjectOfDifferentClass->GetFullName(), *ObjectClass->GetPathName());
						...
						{
							UE_LOGF(LogConcert, Warning, "Actor '%ls' could not find an owner World! This is unexpected and the Actor will be created via NewObject rather than SpawnActor.", *ObjectNameToCreate.ToString());
						}
					}
					else
					{
						UE_LOGF(LogConcert, Warning, "Actor '%ls' wasn't directly outered to a Level! This is unexpected and the Actor will be created via NewObject rather than SpawnActor.", *ObjectNameToCreate.ToString());
					}
				}
				else
				{
