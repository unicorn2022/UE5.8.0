...
namespace UE::ConcertSyncClient::Replication
{
	FConcertClientReplicationBridge::FConcertClientReplicationBridge()
	{
		...
		if (!GEngine)
		{
			FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FConcertClientReplicationBridge::SetupEngineDelegates);
		}
		else
		{
		...
		{
			UE_LOGF(LogConcert, Verbose, "Discovered object %ls for replication", *Object.GetPathName());
		});
		OnObjectRemovedDelegate.AddLambda([](const FSoftObjectPath& ObjectPath)
		{
			UE_LOGF(LogConcert, Verbose, "Removed object %ls from replication", *ObjectPath.ToString());
		});
	}
	...
	void FConcertClientReplicationBridge::PopTrackedObjects(TArrayView<const FSoftObjectPath> InTrackedObjects)
	{
		for (const FSoftObjectPath& TrackedObjectPath : InTrackedObjects)
		{
			...
			if (!TrackedObject)
			{
				UE_LOGF(LogConcert, Warning, "Object %ls was requested to not be tracked but was it already wasn't.", *TrackedObjectPath.ToString());
				continue;
			}
	...
	UObject* FConcertClientReplicationBridge::FindObjectIfAvailable(const FSoftObjectPath& Path)
	{
		...
		if (bObjectWasInCache)
		{
			...
			UE_LOGF(LogConcert, Verbose, "Replicated object %ls was renamed to %ls. Resolving new target object...", *Path.ToString(), *ObjectInfo->ResolvedObject->GetPathName());
		}
		
		// This should be an object in a UWorld. This will resolve if the world is opened.
		UObject* Object = Path.ResolveObject();
		UE_CLOGF(Object && !Object->IsInA(UWorld::StaticClass()), LogConcert, Warning, "Object %ls is not in any UWorld. The replication system was designed to work with objects in UWorlds.", *Path.ToString());
		...
		else
		{
			...
			UE_CLOGF(bObjectWasInCache, LogConcert, Warning, "Replicated object %ls was renamed to %ls. The target object was not replaced by any new object so it will stop being replicated.", *Path.ToString(), *ObjectInfo->ResolvedObject.Get()->GetPathName());
		}
	...
	{
		UE_LOGF(LogConcert, Verbose, "Discovering objects in world %ls for replication", *World.GetPathName());
	...
	{
		UE_LOGF(LogConcert, Verbose, "Removing objects from world %ls for replication", *World.GetPathName());
