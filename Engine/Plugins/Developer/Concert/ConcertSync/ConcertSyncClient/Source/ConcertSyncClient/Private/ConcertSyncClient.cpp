...
void FConcertSyncClient::PersistSpecificChanges(TArrayView<const FName> InPackages)
{
	...
	if (Workspace)
	{
		Workspace->PersistSessionChanges({ InPackages });
	}
#endif
}
...
void FConcertSyncClient::PersistAllSessionChanges()
{
	...
	if (Workspace)
	{
		...
		Workspace->PersistSessionChanges({ SessionChanges });
	}
#endif
}
...
{
	ConcertClient->SetFileSharingService(MoveTemp(InFileSharingService));
}

void FConcertSyncClient::CreateWorkspace(const TSharedRef<FConcertSyncClientLiveSession>& InLiveSession)
{
	DestroyWorkspace();
	Workspace = MakeShared<FConcertClientWorkspace>(UE::ConcertSyncClient::FSessionBindArgs{ InLiveSession, Bridges } , ConcertClient->GetFileSharingService(), this);
