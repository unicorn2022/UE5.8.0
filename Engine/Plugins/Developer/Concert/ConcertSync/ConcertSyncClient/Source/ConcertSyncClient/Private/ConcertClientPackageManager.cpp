...
#include "Sandbox/ConcertPackageSandbox.h"
#include "Sandbox/IConcertSandbox.h"
...
{
	using namespace UE::ConcertSyncClient;
	...
	{
		// Create Sandbox
		SandboxManager = FConcertPackageSandbox::CreateOrLoad(*this, LiveSession.ToSharedRef(), InClientRole);
	}
...
FConcertClientPackageManager::~FConcertClientPackageManager()
{
	...
	if (SandboxManager)
	{
		// Shutdown will cause FConcertPackageReloadHandler to call EnqueueHotReload and EnqueuePurge.
...
{
#if WITH_EDITOR
	check(SandboxManager.IsValid());
	UE::ConcertSyncClient::IConcertSandbox* Sandbox = SandboxManager.Get();
	auto ShouldPersistPackageWithExtension = [Sandbox](const FString& PackageName, const FString& Extension) -> TOptional<FString>
	{
		FString FullPath;
		if (FPackageName::TryConvertLongPackageNameToFilename(PackageName, FullPath, Extension))
		{
			if (Sandbox->DeletedPackageExistsInNonSandbox(FullPath))
			{
...
FPersistResult FConcertClientPackageManager::PersistSessionChanges(FPersistParameters InParam)
{
	...
	if (SandboxManager)
	{
		...
		return SandboxManager->PersistSandbox(FilesToPersist, MoveTemp(InParam));
	}
#endif
	return {};
}
...
void FConcertClientPackageManager::AddPendingReloadForNewExternalMaps(const FConcertPackageInfo& PackageInfo, const FString& PackagePathname)
{
	...
	if (PersistentLevel)
	{
		...
		{
			UE_LOGF(LogConcert, Display, "Scheduling reloading for world partition persistent level %ls.", *PackagePathname);
...
void FConcertClientPackageManager::HandleLocalPackageEvent(const FConcertPackageInfo& PackageInfo, const FString& PackagePathname)
{
	...
	if (PackageInfo.PackageUpdateType == EConcertPackageUpdateType::Added && EnumHasAnyFlags(LiveSession->GetSessionFlags(), EConcertSyncSessionFlags::ShouldUsePackageSandbox))
	{
		...
		if (FPackageName::TryConvertLongPackageNameToFilename(PackageInfo.PackageName.ToString(), DstPackagePathname, PackageInfo.PackageFileExtension))
		{
			...
			{
				UE_LOGF(LogConcert, Error, "Failed to copy package file '%ls' to the sandbox", *PackagePathname);
			}
		}
	}
	...
	if (ApplyPackageFilters(PackageInfo))
	{
		...
		if (PackageFileSize > 0)
		{
			if (CanExchangePackageDataAsByteArray(static_cast<uint64>(PackageFileSize)))
			{
				...
				{
					UE_LOGF(LogConcert, Error, "Failed to load file data '%ls' in memory", *PackagePathname);
					return;
				}
			}
			else if (IsFileSharingEnabled())
			{
				...
				{
					UE_LOGF(LogConcert, Error, "Failed to share a copy of package file '%ls'", *PackagePathname);
					return;
				}
			}
			else
			{
				...
				UE_LOGF(LogConcert, Error, "Failed to handle local package file '%ls'. The file is too big to be sent over the network.", *PackagePathname);
	...
	{
		PersistSessionChanges({MakeArrayView(&PackageInfo.PackageName, 1) });
	}
}
...
bool FConcertClientPackageManager::IsFileSharingEnabled() const
{
