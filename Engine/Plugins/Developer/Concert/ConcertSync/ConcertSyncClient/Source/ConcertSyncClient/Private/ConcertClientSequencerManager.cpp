...
void FConcertClientSequencerManager::OnSequencerCreated(TSharedRef<ISequencer> InSequencer)
{
	...
	UE_LOGF(LogConcertSequencerSync, Verbose, "OnSequencerCreated: %ls", *SequenceObjectPath);
	...
	if (!bRespondingToTransportEvent && IsSequencerRemoteOpenEnabled())
	{
		if (TSharedPtr<IConcertClientSession> Session = WeakSession.Pin())
		{
			if (CanSendSequencerEvent(SequenceObjectPath))
			{
				...
				UE_LOGF(LogConcertSequencerSync, Verbose, "    Sending OpenEvent: %ls", *OpenEvent.SequenceObjectPath);
...
void ApplyPlayRangeToPlayer(ULevelSequencePlayer* Player, const FFrameNumberRange& PlayRange)
{
	...
	{
		UE_LOGF(LogConcertSequencerSync, Warning, "ApplyPlayRangeToPlayer (%ls): Missing sequence or scene",
	...
	{
		UE_LOGF(LogConcertSequencerSync, Verbose, "SetFrameRange (%ls): Start %d, duration %d (was %d, %d)",
...
void FConcertClientSequencerManager::OnSequencerClosed(TSharedRef<ISequencer> InSequencer)
{
	...
	UE_LOGF(LogConcertSequencerSync, Verbose, "OnSequencerClosed: %ls",
	...
	if (Sequence)
	{
		...
		if (TSharedPtr<IConcertClientSession> Session = WeakSession.Pin())
		{
			...
			if (CanSendSequencerEvent(SequenceObjectPath))
			{
				UE_LOGF(LogConcertSequencerSync, Verbose, "    Sending CloseEvent: %ls", *SequenceObjectPath);
		...
		{
			UE_LOGF(LogConcertSequencerSync, VeryVerbose, "    Removing Sequencer state for sequence: %ls", *SequenceObjectPath);
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "Event Received - StateSync: Syncing %d Sequencer states", InEvent.SequencerStates.Num());
	...
	for (const auto& State : InEvent.SequencerStates)
	{
		...
		UE_LOGF(LogConcertSequencerSync, VeryVerbose, "    Syncing %d Sequencers for sequence %ls", OpenSequencersForObject.Num(), *SequencerState.SequenceObjectPath);
		...
		{
			UE_LOGF(LogConcertSequencerSync, VeryVerbose, "    No existing Sequencer with sequence %ls open. Will open and sync a new one at end of frame.", *SequencerState.SequenceObjectPath);
...
void FConcertClientSequencerManager::OnSequencerTimeChanged(TWeakPtr<ISequencer> InSequencer)
{
	...
	if (Session.IsValid() && Sequence && IsSequencerPlaybackSyncEnabled())
	{
		...
		if (OpenSequencer->PlaybackMode == EPlaybackMode::Controller || OpenSequencer->PlaybackMode == EPlaybackMode::Undefined)
		{
			...
			UE_LOGF(LogConcertSequencerSync, Verbose, "Sending StateEvent: %ls, at frame: %d",
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "Event Received - Close: %ls. Deferring until end of frame.", *InEvent.SequenceObjectPath);
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "Event Received - Open: %ls. Deferring until end of frame.", *InEvent.SequenceObjectPath);
	...
	for (TTuple<FName, TObjectPtr<ALevelSequenceActor>>& Item : SequencePlayers)
	{
		...
		if (LevelSequenceActor)
		{
			...
			if (LevelSequence && LevelSequence->GetPackage() == PackageReloaded)
			{
				...
				UE_LOGF(LogConcertSequencerSync, Display, "Rebuild required for LevelSequence %ls", *Item.Key.ToString());
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "    Closing sequence players: %ls, is from controller: %d", *CloseEvent.SequenceObjectPath, CloseEvent.bControllerClose);

	TObjectPtr<ALevelSequenceActor>* Player = SequencePlayers.Find(*CloseEvent.SequenceObjectPath);
	if (!Player)
	{
		UE_LOGF(LogConcertSequencerSync, Verbose, "        No open players to close for sequence %ls", *CloseEvent.SequenceObjectPath);
		return;
	}
	...
	UE_LOGF(LogConcertSequencerSync, VeryVerbose, "    Removing sequence player for sequence: %ls", *CloseEvent.SequenceObjectPath);
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "Handling Event - Close: %ls", *CloseEvent.SequenceObjectPath);
	...
	if (SequencerState)
	{
		...
		{
			UE_LOGF(LogConcertSequencerSync, VeryVerbose, "    Removing Sequencer state for sequence: %ls", *CloseEvent.SequenceObjectPath);
...
void FConcertClientSequencerManager::ApplyOpenEvent(const FConcertSequencerOpenEvent& InOpenEvent)
{
	...
	UE_LOGF(LogConcertSequencerSync, Verbose, "Handling Event - Open: %ls", *SequenceObjectPath);
	...
	if (GIsEditor && bDidOpen && UE::Private::ConcertClientSequencerManager::IsPendingTakePath(SequenceObjectPath))
	{
		if (ULevelSequence* PendingLevelSequence = FindObject<ULevelSequence>(nullptr, *UE::Private::ConcertClientSequencerManager::PendingTakePath))
		{
			...
			MissingObjectDelegate.BindLambda([](const FStringView MissingObject)
				{
					UE_LOGF(LogConcertSequencerSync, Display, "Missing Object %ls when loading PendingTake", *FString(MissingObject));
				});
			...
			{
				UE_LOGF(LogConcertSequencerSync, Display, "Missing take data on pending take.");
			}
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "CreateNewSequencePlayerIfNotExists: %ls", *SequenceObjectPath);

	if (SequencePlayers.Contains(*SequenceObjectPath))
	{
		UE_LOGF(LogConcertSequencerSync, Verbose, "    Sequence player already exists for sequence %ls", *SequenceObjectPath);
		return;
	}
	...
	if (!Sequence)
	{
		UE_LOGF(LogConcertSequencerSync, Warning, "    Failed to load level sequence while creating new sequence player: %ls", *SequenceObjectPath);
		return;
	}
...
void FConcertClientSequencerManager::DestroyPlayer(ALevelSequenceActor* LevelSequenceActor)
{
	...
	{
		UE_LOGF(LogConcertSequencerSync, VeryVerbose, "Destroying LevelSequenceActor: %ls", *LevelSequenceActor->GetPathName());
		LevelSequenceActor->GetSequencePlayer()->Stop();
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "Event Received - TimeAdjustment: %ls. Deferring until end of frame.", *InEvent.SequenceObjectPath);
...
void FConcertClientSequencerManager::ApplyTimeAdjustmentEvent(const FConcertSequencerTimeAdjustmentEvent& TimeAdjustmentEvent)
{
	...
	UE_LOGF(LogConcertSequencerSync, Verbose, "Handling Event - TimeAdjustment: %ls", *TimeAdjustmentEvent.SequenceObjectPath);
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "    TimeAdjustment: Updating Sequencers for sequence %ls", *TimeAdjustmentEvent.SequenceObjectPath);
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "    TimeAdjustment: Updating sequence players for sequence %ls", *TimeAdjustmentEvent.SequenceObjectPath);

	TObjectPtr<ALevelSequenceActor>* SeqPlayer = SequencePlayers.Find(*TimeAdjustmentEvent.SequenceObjectPath);
	if (!SeqPlayer)
	{
		UE_LOGF(LogConcertSequencerSync, Verbose, "        No sequence player for sequence %ls", *TimeAdjustmentEvent.SequenceObjectPath);
		return;
	}
...
void FConcertClientSequencerManager::ApplyTransportEvent(const FConcertSequencerState& EventState)
{
	...
	UE_LOGF(LogConcertSequencerSync, Verbose, "Handling Event - Transport: %ls", *EventState.SequenceObjectPath);
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "    Transport: Update Sequencer for sequence %ls, at frame: %d", *EventState.SequenceObjectPath, EventState.Time.Time.FrameNumber.Value);
	FConcertSequencerState& SequencerState = SequencerStates.FindOrAdd(*EventState.SequenceObjectPath);
	...
	for (FOpenSequencerData* OpenSequencer : GatherRootSequencersByState(EventState.SequenceObjectPath))
	{
		...
		if (EventState.PlayerStatus == EConcertMovieScenePlayerStatus::Playing)
		{
			...
			if (Sequencer->GetPlaybackStatus() != EMovieScenePlayerStatus::Playing)
			{
				...
				// Log time metrics
				UE_LOGF(LogConcertSequencerSync, Display, "Starting multi-user playback for sequence '%ls':\n"
			...
			else
			{
				...
				UE_LOGF(LogConcertSequencerSync, Display, "Incoming update to sequence '%ls':\n"
					"    Current Time       = %d+%fs (%f seconds)\n"
					"    Incoming Time      = %d+%fs (%f seconds)\n"
...
{
	UE_LOGF(LogConcertSequencerSync, Verbose, "    Transport: Update sequence player for sequence %ls, at frame: %d", *EventState.SequenceObjectPath, EventState.Time.Time.FrameNumber.Value);

	TObjectPtr<ALevelSequenceActor>* SeqPlayer = SequencePlayers.Find(*EventState.SequenceObjectPath);
	if (!SeqPlayer)
	{
		UE_LOGF(LogConcertSequencerSync, Verbose, "        No sequence player for sequence %ls", *EventState.SequenceObjectPath);
		return;
	}
...
void FConcertClientSequencerManager::OnWorkspaceEndFrameCompleted()
{
	...
	if (!PendingDestroy.IsEmpty())
	{
		...
		UE_LOGF(LogConcertSequencerSync, VeryVerbose, "Running garbage collection after destroying %d sequence players. Deferring remaining pending events until next frame", PendingDestroy.Num());
