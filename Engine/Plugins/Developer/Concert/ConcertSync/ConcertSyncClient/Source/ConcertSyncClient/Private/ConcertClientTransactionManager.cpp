...
void FConcertClientTransactionManager::ProcessTransactionEvent(const FPendingTransactionToProcessContext& InContext, const FStructOnScope& InEvent)
{
	...
	{
		UE_LOGF(LogConcert, VeryVerbose, "Dropping transaction for '%ls' (index %d) as it arrived out-of-order", *TransactionEvent.TransactionId.ToString(), TransactionEvent.TransactionUpdateIndex);
		return;
	}
...
{
	UE_LOGF(LogConcert, Warning, "Object %ls conflicts with in-bound transaction and has been removed}.",
...
void FConcertConflictDescriptionAggregate::AddObjectRenamed(const FConcertObjectId& OldObjectId, const FConcertObjectId& NewObjectId)
{
	...
	{
		UE_LOGF(LogConcert, Warning, "Object %ls conflicts with in-bound transaction and has been renamed to %ls.",
			   *OldObjectId.ObjectName.ToString(), *NewObjectId.ObjectName.ToString());
