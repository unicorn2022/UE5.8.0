...
namespace UE::ConcertSyncClient::Replication
{
	...
	void FObjectReplicationApplierProcessor::ProcessObject(const FObjectProcessArgs& Args)
	{
		...
		if (!Object)
		{
			UE_LOGF(LogConcert, Error, "Replication: Object %ls is unavailable. The data source should not have reported it.", *Args.ObjectInfo.ObjectId.Object.ToString());
			return;
		}
		...
		Args.Item.ExtractReplicationDataForObject(
			[this, &Args, Object, Actions, bDebugActions, &bAppliedData](const FConcertSessionSerializedPayload& Payload)
			{
				...
				UE_CLOGF(bDebugActions, LogConcert, Log, "===== %ls actions ====", *Object->GetPathName());
		...
		UE_CLOGF(!bAppliedData, LogConcert, Warning, "Replication: Server sent data that could not be applied (likely it was empty) for object %ls from stream %ls", *Args.ObjectInfo.ObjectId.Object.ToString(), *Args.ObjectInfo.ObjectId.StreamId.ToString());
