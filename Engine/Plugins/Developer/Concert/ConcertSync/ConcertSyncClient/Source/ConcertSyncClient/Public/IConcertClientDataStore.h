 ...
 *     UE_LOGF(MyLogCat, Display, "%d", Session->GetDataStore().FetchAs<FPoint2D>(Key).Get().GetValue().x); // Prints 0
 *     GetDataStore().CompareExchange(Key, Position, Point2D{10, 20}).Get();
 *     UE_LOGF(MyLogCat, Display, "%d", Session->GetDataStore().FetchAs<FPoint2D>(Key).Get().GetValue().x); // Prints 10
 *
 *     // Store a shape.
 *     FShape Shape;
