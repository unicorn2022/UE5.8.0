...
class UPackage;
namespace UE::ConcertSyncClient { class FConcertSandboxLock; }
...
public:
	 ...
	 * @param InClientRole Role of the client. Used by sandbox system to generate lock reason text.
	 */
	...
	/** Enqueues a package for hot reload. */
	void EnqueueHotReload(FName InPackageName) { PackagesPendingHotReload.Add(InPackageName); }
	/** Enqueues a package for purge. */
	void EnqueuePurge(FName InPackageName) { PackagesPendingPurge.Add(InPackageName); }

private:
	/**
	 * Is file sharing enabled.
	 */
	bool IsFileSharingEnabled() const;
	...
	TUniquePtr<UE::ConcertSyncClient::FConcertPackageSandbox> SandboxManager; // TODO UE-354294: Will need to ensure the sandbox also works on cooked clients
#endif
