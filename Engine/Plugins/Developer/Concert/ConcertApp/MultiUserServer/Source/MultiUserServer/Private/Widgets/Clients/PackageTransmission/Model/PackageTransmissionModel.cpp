...
namespace UE::MultiUserServer
{
	...
	{
		UE_LOGF(LogConcert, Verbose, "OnLocalBeginSendPackage: %ls", *Args.TransmissionId.ToString(EGuidFormats::DigitsWithHyphens));
	...
	uint64 FPackageTransmissionModel::DeterminePackageRevision(EPackageSendDirection SendDirection, const ConcertSyncCore::ConcertPackageEvents::FConcertBeginSendPackageParams& Args) const
	{
		...
		if (!Session)
		{
			UE_LOGF(LogConcert, Warning, "Processing package event: failed to get head revision because no session was found for client.");
			return ErrorResult;
		}
		const TOptional<FConcertSyncSessionDatabaseNonNullPtr> Database = Server->GetLiveSessionDatabase(Session->GetId());
		if (!Database)
		{
			UE_LOGF(LogConcert, Warning, "Processing package event: failed to get head revision because session database could not be accessed.");
			return ErrorResult;
		}
		...
		UE_LOGF(LogConcert, Error, "Processing package event: failed to get head revision because the package was not saved in the database.");
		return ErrorResult;
	}
