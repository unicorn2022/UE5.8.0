...
{
	UE_LOGF(LogDisasterRecovery, Verbose, "Looking up disaster recovery server...");
}
...
bool FLookupDisasterRecoveryServerTask::Tick()
{
	...
	for (const FConcertServerInfo& ServerInfo : Servers)
	{
		if (ServerInfo.ServerName == ServerName)
		{
			...
			UE_LOGF(LogDisasterRecovery, Verbose, "...Looking up disaster recovery server -> Found");
			return true; // Done
		}
	}
...
{
	UE_LOGF(LogDisasterRecovery, Verbose, "Mounting session repository...");
	...
	MountFn().Next([this, ExecutionToken](const FConcertAdmin_MountSessionRepositoryResponse& Response)
	{
		...
		UE_LOGF(LogDisasterRecovery, Verbose, "...Mounting session repository -> Succeeded");
...
{
	UE_LOGF(LogDisasterRecovery, Verbose, "Discarding recovery sessions...");
	...
	DropRepositoryFn().Next([this, ExecutionToken](const FConcertAdmin_DropSessionRepositoriesResponse& Response)
	{
		...
		{
			OnErrorFn(DisasterRecoveryTasksUtil::GetSystemErrorMessage(Response.ResponseCode));
			UE_LOGF(LogDisasterRecovery, Verbose, "...Discarding recovery sessions -> Failed");
		}
		else
		{
			OnRepositoriesDiscardedFn(Response.DroppedRepositoryIds);
			UE_LOGF(LogDisasterRecovery, Verbose, "...Discarding recovery sessions -> Succeeded");
		}
...
{
	UE_LOGF(LogDisasterRecovery, Verbose, "Looking up recovery session '%ls'...", *SessionName);
	...
	GetSessionsFn().Next([this, ExecutionToken](const FConcertAdmin_GetAllSessionsResponse& Response)
	{
		...
		{
			OnErrorFn(DisasterRecoveryTasksUtil::GetSystemErrorMessage(Response.ResponseCode));
			UE_LOGF(LogDisasterRecovery, Verbose, "...Looking up recovery session -> Failed");
		}
		else if (const FConcertSessionInfo* ArchivedSession = Response.ArchivedSessions.FindByPredicate([this](const FConcertSessionInfo& MatchCandidate){ return MatchCandidate.SessionName == SessionName; }))
		{
			...
			UE_LOGF(LogDisasterRecovery, Verbose, "...Looking up recovery session -> Found archive session ID %ls", *ArchivedSession->SessionId.ToString());
		}
		else if (const FConcertSessionInfo* LiveSession = Response.LiveSessions.FindByPredicate([this](const FConcertSessionInfo& MatchCandidate){ return MatchCandidate.SessionName == SessionName; })) // The user live session?
		{
			...
			UE_LOGF(LogDisasterRecovery, Verbose, "...Looking up recovery session -> Found live session ID %ls", *LiveSession->SessionId.ToString());
		}
		else
		{
			UE_LOGF(LogDisasterRecovery, Verbose, "...Looking up recovery session -> Not found");
...
{
	UE_LOGF(LogDisasterRecovery, Verbose, "Creating/Restoring session '%ls'...", *SessionName);
	...
	CreateAndJoinFn().Next([this, ExecutionToken](EConcertResponseCode Response)
	{
		...
		else if (Response != EConcertResponseCode::Success)
		{
			if (!bDone) // Ensure the task is not already done from a cancellation.
			{
				...
				bDone = true;
				UE_LOGF(LogDisasterRecovery, Warning, "Failed to create or restore session %ls. Disaster recovery will not be enabled for this session.", *SessionName);
			}
		}
	});
}
...
bool FCreateAndJoinDisasterRecoverySessionTask::Tick()
{
	if (bWorkspaceSynchronized)
	{
		if (LoopCountSinceWorkspaceSync++ > 0) // Ensure FConcertClientWorkspace::OnEndFrame() is called once before saying the session is joined. It needs to run in case the session changes needs to be persisted.
		{
			OnSessionJoinedFn();
			bDone = true;
			UE_LOGF(LogDisasterRecovery, Verbose, "...Creating/Restoring session '%ls' -> Succeeded", *SessionName);
		}
	}
...
void FCreateAndJoinDisasterRecoverySessionTask::OnSessionConnectionChanged(IConcertClientSession& Session, EConcertConnectionStatus Status)
{
	...
	else if (Status == EConcertConnectionStatus::Disconnected)
	{
		if (!bDone)
		{
			...
			bDone = true;
			UE_LOGF(LogDisasterRecovery, Verbose, "...Creating/Restoring session '%ls' -> Failed (Disconnected)", *SessionName);
		}
	}
}
...
{
	UE_LOGF(LogDisasterRecovery, Verbose, "Recovery session workspace synched");
...
{
	UE_LOGF(LogDisasterRecovery, Verbose, "Looking up a recoverable session...");
}
...
bool FFindRecoverableDisasterRecoverySessionTask::Tick()
{
	...
	if (!CurrentCandidate) // 1- The 'pick a candidate' state. Mount the next candidate in the list.
	{
		if (!Candidates.Num())
		{
			...
			UE_LOGF(LogDisasterRecovery, Verbose, "...Looking up a recoverable session -> All candidates were eliminated");
			return true; // Done;
		}

		CurrentCandidate = Candidates.Last();
		UE_LOGF(LogDisasterRecovery, Verbose, "Evaluating candidate session '%ls'.", *CurrentCandidate->SessionName);
	...
	else if (MountingTask) // 2- The 'mounting session repository' state.
	{
		if (MountingTask->Tick()) // Done?
		{
			...
			{
				UE_LOGF(LogDisasterRecovery, Verbose, "Eliminated candidate session '%ls'. Failed to mount the session repository.", *CurrentCandidate->SessionName);
	...
	else if (SearchingSessionIdTask) // 3- The 'searching session' state.
	{
		if (SearchingSessionIdTask->Tick()) // Done?
		{
			...
			{
				UE_LOGF(LogDisasterRecovery, Verbose, "Eliminated candidate session '%ls'. Failed to find the session files.", *CurrentCandidate->SessionName);
	...
	else if (ActivityStream) // 4- The 'load activities' state.
	{
		...
		if (ReadCount > 0) // On error, ReadCount is 0.
		{
			for (const TSharedPtr<FConcertSessionActivity>& Activity : Activities)
			{
				if (!Activity->Activity.bIgnored)
				{
					...
					UE_LOGF(LogDisasterRecovery, Verbose, "...Looking up a recoverable session -> Found recoverable session '%ls' among the candidates", *CurrentCandidate->SessionName);
		...
		if (bEndOfStreamOrError)
		{
			...
			{
				UE_LOGF(LogDisasterRecovery, Verbose, "Eliminated candidate session %ls. No recoverable activities available.", *CurrentCandidate->SessionName);
			}
			else
			{
				UE_LOGF(LogDisasterRecovery, Warning, "Eliminated candidate session %ls. Reason: %ls", *CurrentCandidate->SessionName, *ErrorMsg.ToString());
			}
