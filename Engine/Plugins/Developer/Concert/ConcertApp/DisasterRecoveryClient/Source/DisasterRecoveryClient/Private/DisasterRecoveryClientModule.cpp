...
public:
	virtual void StartupModule() override
	{
		...
		{
			UE_LOGF(LogDisasterRecovery, Warning, "The current project doesn't have any name. Recovery Hub will be disabled! Please set your project name.");
...
private:
	...
	bool SpawnDisasterRecoveryServer(const FString& ServerName)
	{
		...
		{
			UE_LOGF(LogDisasterRecovery, Warning, "Unreal Recovery Service application was not found. Recovery Hub will be disabled! Please build '%ls'.", *DisasterRecoveryUtil::GetDisasterRecoveryServiceExeName());
			return false;
		}
		...
		if (!DisasterRecoveryServiceHandle.IsValid())
		{
			UE_LOGF(LogDisasterRecovery, Error, "Failed to launch Recovery Service application. Recovery Hub will be disabled!");
			return false;
		}
	...
	void StartDisasterRecoveryService()
	{
		...
		{
			UE_LOGF(LogDisasterRecovery, Display, "Skipped recovery process. The application was launched in unattended mode. Previous sessions will be available from recovery hub 'recent' list.");
