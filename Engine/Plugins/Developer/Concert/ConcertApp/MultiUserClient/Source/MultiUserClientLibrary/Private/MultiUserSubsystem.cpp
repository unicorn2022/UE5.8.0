...
void UMultiUserSubsystem::SendCustomEvent(const FConcertBlueprintEvent& EventData)
{
	...
	if (!ConcertManager.IsValid())
	{
		UE_LOGF(LogConcert, Error, "No concert manager initialized. Unable to send Blueprint RPC over Multi-user.");
		return;
	}

	if (!ConcertManager->WeakSession.IsValid())
	{
		UE_LOGF(LogConcert, Warning, "Attempting to send a Multi-user custom event when not in a multi-user session. ");
		return;
	}
...
bool UMultiUserSubsystem::RegisterCustomEventHandler(const UStruct* EventType, FCustomEventHandler InEventHandler)
{
	...
	if (Handler != nullptr)
	{
		UE_LOGF(LogConcert, Error, "Multi-user Subsystem: A custom event handler for %ls already exists and a new handler cannot be registered.", *EventTypeName.ToString());
		return false;
	}
	UE_LOGF(LogConcert, Display, "Multi-user Subsystem: Registered custom event handler for %ls messages.", *EventTypeName.ToString());
...
bool UMultiUserSubsystem::UnregisterCustomEventHandler(const UStruct* EventType)
{
	...
	UE_LOGF(LogConcert, Display, "Multi-user Subsystem: Unregistered custom event handler for %ls messages.", *EventTypeName.ToString());
