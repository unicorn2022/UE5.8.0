...
#undef SET_REASON
	...
	void FReassignObjectPropertiesLogic::ReassignAllTo(TArray<FSoftObjectPath> ObjectsToReassign, const FGuid& ClientId)
	{
		...
		{
			UE_LOGF(LogConcert, Error, "Property Reassignment: The target client is not editable.");
			return;
		}
	...
	void FReassignObjectPropertiesLogic::OnRemoteClientsChanged()
	{
		...
		{
			UE_LOGF(LogConcert, Warning,
				"Client %ls disconnected while reassigning objects to it. Cancelling the operation however object may already have been removed on other clients.",
	...
	void FReassignObjectPropertiesLogic::PerformSubmission_GameThread(ISubmissionWorkflow& Workflow)
	{
		...
		{
			UE_LOGF(LogConcert, Error, "Property Reassignment: The target client is no longer editable.");
			return;
		}
		...
		{
			UE_LOGF(LogConcert, Warning, "Property Reassignment: No reassignment will take place because all remote clients failed.");
			InProgressOperation.Reset();
			return;
		}
		...
		{
			UE_LOGF(LogConcert, Error, "Property Reassignment: Failed to give target client properties.");
			return;
		}
