		...
		for (const TPair<const FOnlineClient*, FConcertClientInfo>& ClientData : IncludedClients)
		{
			...
			if (!TargetClientPreset)
			{
				UE_LOGF(LogConcert, Warning,
					"There are multiple clients with display name %ls and device name %ls in the session. Only the 1st encountered will be saved into the preset. Did you perhaps launch 2 editors on the same machine (if so you can use -CONCERTDISPLAYNAME)?",
