...
namespace UE::MultiUserClient::Replication
{
	...
	void FSubmissionWorkflow_RemoteClient::ProcessSubmissionResponse(const FMultiUser_ChangeRemote_Response& Response)
	{
		...
		UE_LOGF(LogConcert, Log, "Remote client %ls accepted change request", *GetRemoteClientName());
	}
	
	void FSubmissionWorkflow_RemoteClient::TimeoutAndClean()
	{
		UE_LOGF(LogConcert, Warning, "Change request to client %ls timed out", *GetRemoteClientName());
	...
	{
		UE_LOGF(LogConcert, Warning, "Change request to client %ls failed (error flags: %d)", *GetRemoteClientName(), static_cast<int32>(ErrorFlags));
	...
	void FSubmissionWorkflow_RemoteClient::OnStreamRemoteChangeEvent(const FConcertSessionContext& Context, const FMultiUser_ChangeRemote_StreamUpdatedEvent& EventData)
	{
		...
		UE_CLOGF(!EventData.bProcessedSuccessfully, LogConcert, Error, "Remote stream change to client %ls failed", *GetRemoteClientName());
