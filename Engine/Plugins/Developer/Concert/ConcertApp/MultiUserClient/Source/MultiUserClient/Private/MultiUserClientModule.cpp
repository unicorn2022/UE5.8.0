...
#include "Sandbox/MultiUserSandboxIntegration.h"
...
public:
	virtual void StartupModule() override
	{
		...
		if (GIsEditor)
		{
			...
			SandboxIntegration = MakeUnique<UE::MultiUserClient::FMultiUserSandboxIntegration>(MultiUserClient.ToSharedRef());
		}
#endif
	...
	void RegisterTabSpawner(const TSharedPtr<FWorkspaceItem>& WorkspaceGroup)
	{
		...
		{
			SandboxIntegration->GetEntryPoint()->OnSummonProviderUI().AddLambda([]
			{
				FGlobalTabmanager::Get()->TryInvokeTab(ConcertBrowserTabName);
			});
		}
#endif
	}
...
private:
	...
	void GetPreConnectionTasks(const IConcertClient& InClient, TArray<TUniquePtr<IConcertClientConnectionTask>>& OutTasks)
	{
		...
		{
			UE_LOGF(LogConcert, Display, "Source control validation task was bypassed.");
		}
	...
	{
		TSharedPtr<UE::FileSandboxUI::IExternalSandboxActiveViewModel> ExternalSandboxViewModel;
#if WITH_EDITOR
		ExternalSandboxViewModel = SandboxIntegration ? SandboxIntegration->GetExternalSandboxViewModel().ToSharedPtr() : nullptr;
#endif
		
		return SNew(SDockTab)
			.TabRole(NomadTab)
			[
				SAssignNew(BrowserTab, SConcertBrowser, 
					MultiUserClient.ToSharedRef(), ReplicationManager.ToSharedRef(), ExternalSandboxViewModel
					)
			];
	}
...
private:
	...
	TUniquePtr<UE::MultiUserClient::FMultiUserSandboxIntegration> SandboxIntegration;
#endif
