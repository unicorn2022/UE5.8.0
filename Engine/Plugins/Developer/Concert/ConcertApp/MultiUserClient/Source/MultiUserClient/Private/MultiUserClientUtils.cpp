...
void LogNoCompatibleCommunicationPluginEnabled()
{
	...
	{
		UE_LOGF(LogConcert, Warning, "The '%ls' plugin is disabled. Multi-User requires '%ls' to function correctly", *CompatibleTransportPlugins[0], *CompatibleTransportPlugins[0]);
	}
	else
	{
		UE_LOGF(LogConcert, Warning, "Multi-User requires one of the following plugins to function correctly: %ls", *FString::Join(CompatibleTransportPlugins, TEXT(", ")));
	}
}
