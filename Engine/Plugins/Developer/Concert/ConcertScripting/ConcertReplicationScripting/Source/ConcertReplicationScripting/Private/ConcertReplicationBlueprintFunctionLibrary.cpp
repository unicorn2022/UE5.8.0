...
bool UConcertReplicationBlueprintFunctionLibrary::MakePropertyChainByLiteralPath(const TSubclassOf<UObject>& Class, const TArray<FName>& PathToProperty, FConcertPropertyChainWrapper& Result)
{
	if (!Class)
	{
		UE_LOGF(LogConcert, Error, "UConcertReplicationBlueprintFunctionLibrary::MakePropertyChainByLiteralPath: Invalid class!")
		return false;
	}
	...
	if (PropertyChain)
	{
		...
		{
			UE_LOGF(LogConcert, Error, "UConcertReplicationBlueprintFunctionLibrary::MakePropertyChainByLiteralPath: Property path [%ls] resolved to a property that is not replicatable!",
	...
	UE_LOGF(LogConcert, Error, "UConcertReplicationBlueprintFunctionLibrary::MakePropertyChainByLiteralPath: Path [%ls] does not point to any property in class %ls or it is not replicatable!",
		*FString::JoinBy(PathToProperty, TEXT(", "), [](const FName& Name){ return TEXT("\"") + Name.ToString() + TEXT("\""); }),
		*Class->GetPathName()
		);
	return false;
...
TArray<FConcertPropertyChainWrapper> UConcertReplicationBlueprintFunctionLibrary::GetPropertiesIn(const TSubclassOf<UObject>& Class, FPropertyChainPredicate Filter)
{
	if (!Class)
	{
		UE_LOGF(LogConcert, Error, "UConcertReplicationBlueprintFunctionLibrary::GetPropertiesIn: No class set!")
		return {};
	}
