...
#define SET_REASON(FailReasonVar, ReasonLocText) if (FailReason) { *FailReasonVar = ReasonLocText; }
	...
	bool FTraceControls::StartSynchronizedTrace(TSharedRef<IConcertSession> Session, const FStartTraceArgs& Args, FText* FailReason)
	{
		...
		auto SendRequest = [this, &Session, &Request](const FGuid& Endpoint)
		{
			...
			{
				UE_LOGF(LogConcert, Verbose, "Sending synchronized trace request to client %ls", *Endpoint.ToString());
				...
				Session->SendCustomRequest<FConcertTrace_StartSyncTrace_Request, FConcertTrace_StartSyncTrace_Response>(Request, Endpoint)
					...
					{
						UE_CLOGF(Response.ErrorCode == EConcertTraceErrorCode::Timeout, LogConcert, Warning, "Client %ls timed out answering synchronized trace request", *Endpoint.ToString());
						UE_CLOGF(Response.ErrorCode == EConcertTraceErrorCode::Joined, LogConcert, Log, "Client %ls accepted synchronized trace request", *Endpoint.ToString());
						UE_CLOGF(Response.ErrorCode == EConcertTraceErrorCode::Rejected, LogConcert, Error, "Client %ls rejected synchronized trace request", *Endpoint.ToString());
					});
			}
		};
	...
	void FTraceControls::StopSynchronizedTrace()
	{
		...
		UE_LOGF(LogConcert, Log, "Stopping synchronized trace...");
	...
	{
		UE_LOGF(LogConcert, Log, "Starting synchronized trace.");
