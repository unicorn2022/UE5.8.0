		...
		Result = Client->ClientAdminEndpoint->SendRequest<FConcertAdmin_FindSessionRequest, FConcertAdmin_SessionInfoResponse>(Request, ServerAdminEndpointId)
			.Next([this, AsyncExecutionToken](const FConcertAdmin_SessionInfoResponse& SessionInfoResponse)
			{
				...
				*ResolvedSessionName = FText::AsCultureInvariant(SessionInfoResponse.SessionInfo.SessionName);
				...
				{
					if (SessionInfoResponse.FileShare.bFileShareEnabled && Client->FileSharingService)
					{
						// Enable the file sharing service.
						Client->FileSharingService->SetEnabled(true);

						// Clean-up the challenge file we originally generated when we joined the session.
						Client->FileSharingService->Delete(SessionInfoResponse.FileShare.Challenge.Path);
					}
...
FConcertSessionFileShare MakeFileSharingChallenge(TSharedPtr<IConcertFileSharingService>& SharingService, const FDirectoryPath& FileShareRoot)
{
	FConcertSessionFileShare FileShare;
	FileShare.bFileShareEnabled = false;
	
	// We should always have a file sharing service to work with.
	if (SharingService)
	{
		FileShare.FileShareRoot = FileShareRoot.Path;
		
		SharingService->SetPath(FileShareRoot.Path);
		
		TSharedPtr<FArchive> Writer = SharingService->CreateWriter(FileShare.Challenge.Path);
		if (Writer)
		{
			FileShare.bFileShareEnabled = true;
			*Writer << FileShare.Challenge.Data.ChallengeKey;
			*Writer << FileShare.Challenge.Data.CreationTimeInTicks;
		}
	}

	if (!FileShare.bFileShareEnabled)
	{
		UE_LOGF(LogConcert, Display, "File sharing is disabled this client.");
	}
...
private:
	...
	void HandleServerDiscoveryEvent(const FConcertMessageContext& Context)
	{
		...
		if (Info == nullptr)
		{
			UE_LOGF(LogConcert, Display, "Server %ls discovered.", *Message->ServerName);
	...
	void TimeoutDiscovery(const FDateTime& UtcNow)
	{
		...
		for (auto It = KnownServers.CreateIterator(); It; ++It)
		{
			if (It->Value.LastDiscoveryTime + DiscoveryTimeoutSpan <= UtcNow)
			{
				...
				UE_LOGF(LogConcert, Display, "Server %ls lost.", *It->Value.ServerInfo.ServerName);
				It.RemoveCurrent();
				continue;
			}
		}
...
void FConcertClient::SetFileSharingService(TSharedPtr<IConcertFileSharingService> InFileSharingService)
{
	FileSharingService = MoveTemp(InFileSharingService);

	if (FileSharingService)
	{
		// Start with the FileSharingService disabled.  Per session, we determine if it should be on.
		FileSharingService->SetEnabled(false);
	}
}

TSharedPtr<IConcertFileSharingService> FConcertClient::GetFileSharingService() const
{
...
TFuture<EConcertResponseCode> FConcertClient::InternalCreateSession(const FGuid& ServerAdminEndpointId, const FConcertCreateSessionArgs& CreateSessionArgs, TUniquePtr<FAsyncTaskNotification> OngoingNotification)
{
	...
	// Create session task
	{
		...
		if (GetConfiguration()->ClientSettings.bEnableFileSharing)
		{
			CreateSessionRequest.FileShare = UE::ConcertClient::Private::MakeFileSharingChallenge(FileSharingService, GetConfiguration()->ClientSettings.CloudFileSharingPath);
		}
...
TFuture<EConcertResponseCode> FConcertClient::InternalJoinSession(const FGuid& ServerAdminEndpointId, const FGuid& SessionId, TUniquePtr<FAsyncTaskNotification> OngoingNotification)
{
	...
	// Find session task
	{
		...
		if (GetConfiguration()->ClientSettings.bEnableFileSharing)
		{
			FindSessionRequest.FileShare = UE::ConcertClient::Private::MakeFileSharingChallenge(FileSharingService, GetConfiguration()->ClientSettings.CloudFileSharingPath);
		}
...
void FConcertClient::InternalDisconnectSession()
{
	if (ClientSession.IsValid())
	{
		...
		// Turn off the file sharing service.
		if (FileSharingService)
		{
			FileSharingService->SetEnabled(false);
		}
