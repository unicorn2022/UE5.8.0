...
void FConcertClientSession::Startup()
{
	...
	if (!SessionTick.IsValid())
	{
		...
		UE_LOGF(LogConcert, Display, "Initialized Concert session '%ls' (Id: %ls, Owner: %ls).", *SessionInfo.SessionName, *SessionIdString, *SessionInfo.OwnerUserName);
...
void FConcertClientSession::Shutdown()
{
	if (SessionTick.IsValid())
	{
		...
		UE_LOGF(LogConcert, Display, "Shutdown Concert session '%ls' (Id: %ls, Owner: %ls).", *SessionInfo.SessionName, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
	}
}
...
void FConcertClientSession::Connect()
{
	if (ConnectionStatus == EConcertConnectionStatus::Disconnected)
	{
		...
		UE_LOGF(LogConcert, Display, "Connecting to Concert session '%ls' (Id: %ls, Owner: %ls).", *SessionInfo.SessionName, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
...
void FConcertClientSession::Disconnect()
{
	if (ConnectionStatus != EConcertConnectionStatus::Disconnected)
	{
		...
		UE_LOGF(LogConcert, Display, "Disconnected from Concert session '%ls' (Id: %ls, Owner: %ls).", *SessionInfo.SessionName, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
	}
}
	...
	EConcertSendReceiveState InState)
{
	...
	UE_LOGF(LogConcert, Display,
		   "%ls Concert session '%ls' (Id: %ls, Owner: %ls).",
		   *State,
...
void FConcertClientSession::HandleJoinSessionResultEvent(const FConcertMessageContext& Context)
{
	...
	switch (Message->ConnectionResult)
	{
		...
		case EConcertConnectionResult::ConnectionRefused:
			ConnectionStatus = EConcertConnectionStatus::Disconnected;
			OnConnectionChangedDelegate.Broadcast(*this, ConnectionStatus);
			UE_LOGF(LogConcert, Display, "Disconnected from Concert session '%ls' (Id: %ls, Owner: %ls): Connection Refused.", *SessionInfo.SessionName, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
			break;
...
void FConcertClientSession::ConnectionAccepted(const TArray<FConcertSessionClientInfo>& InSessionClients)
{
	...
	UE_LOGF(LogConcert, Display, "Connected to Concert session '%ls' (Id: %ls, Owner: %ls).", *SessionInfo.SessionName, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
...
void FConcertClientSession::UpdateSessionClients(const TArray<FConcertSessionClientInfo>& InSessionClients)
{
	...
	for (const FConcertSessionClientInfo& SessionClientInfo : InSessionClients)
	{
		if (ClientSessionEndpoint->GetEndpointContext().EndpointId != SessionClientInfo.ClientEndpointId)
		{
			...
			else
			{
				...
				UE_LOGF(LogConcert, Display, "User '%ls' (Endpoint: %ls) joined Concert session '%ls' (Id: %ls, Owner: %ls).", *SessionClient.ClientInfo.ClientInfo.UserName, *SessionClient.ClientInfo.ClientEndpointId.ToString(), *SessionInfo.SessionName, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
			}
		}
	}
	...
	for (auto SessionClientIt = SessionClients.CreateIterator(); SessionClientIt; ++SessionClientIt)
	{
		if (!AvailableClientIds.Contains(SessionClientIt.Key()))
		{
			...
			UE_LOGF(LogConcert, Display, "User '%ls' (Endpoint: %ls) left Concert session '%ls' (Id: %ls, Owner: %ls).", *SessionClient.ClientInfo.ClientInfo.UserName, *SessionClient.ClientInfo.ClientEndpointId.ToString(), *SessionInfo.SessionName, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
		}
	}
}
