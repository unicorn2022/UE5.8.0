...
public:
	...
	/**
	 * Sets the handle to the file sharing service.  The client needs to prove to the server that they both can see the
	 * service.  This file sharing service will ultimate get promoted to the workspace to be used during a working session.
	 */
	virtual void SetFileSharingService(TSharedPtr<IConcertFileSharingService> InFileSharingService) = 0;

	/**
	 * Get the current file sharing service held by the client. 
	 */
	virtual TSharedPtr<IConcertFileSharingService> GetFileSharingService() const = 0;		
