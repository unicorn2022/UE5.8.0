...
public: 
	...
	virtual TSharedPtr<IConcertFileSharingService> GetFileSharingService() const override;		
...
private:
	...
	/** Validate a file a session join request for file sharing support. */
	EConcertResponseCode ValidateFileSharing(const FConcertSessionFileShare& InRequest, FConcertSessionFileShare& OutFileShare, FText& OutFailureReason);

	/** Helper template method that handles the two different message types that are used for joining a session (create / join existing). */
	template<typename TCreateOrJoinMessage>
	bool FillSessionResponseInfoFromFileShareChallenge(const TCreateOrJoinMessage* Message, FConcertAdmin_SessionInfoResponse& ResponseData, const FConcertSessionInfo& SessionInfo);
	...
	/** File sharing service that we use to verify reachability to client proof files. */
