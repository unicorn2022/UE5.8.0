...
void FConcertServer::Configure(const UConcertServerConfig* InSettings)
{
	...
	if (Settings->bEnableFileSharing && FileSharingService)
	{
		FileSharingService->SetEnabled(true);
		FileSharingService->SetPath(Settings->RootFileSharingPath.Path);
	}
	...
	if (!Settings->SessionRepositoryRootDir.IsEmpty())
	{
		...
		{
			UE_LOGF(LogConcert, Warning, "Invalid session repository root directory. Falling back on %ls default.", *SessionRepositoryRootDir);
		}
	}
}
...
void FConcertServer::SetFileSharingService(TSharedPtr<IConcertFileSharingService> InFileSharingService)
{
	FileSharingService = MoveTemp(InFileSharingService);
	if (FileSharingService && Settings && Settings->bEnableFileSharing)
	{
		FileSharingService->SetEnabled(true);
		FileSharingService->SetPath(Settings->RootFileSharingPath.Path);
	}
...
TSharedPtr<IConcertFileSharingService> FConcertServer::GetFileSharingService() const
{
...
TSharedPtr<IConcertServerSession> FConcertServer::CreateSession(const FConcertSessionInfo& SessionInfo, FText& OutFailureReason)
{
	if (!SessionInfo.SessionId.IsValid() || SessionInfo.SessionName.IsEmpty())
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to create a session was made, but the session info was missing an ID or name!");
		return nullptr;
	}
	...
	if (!Settings->ServerSettings.bIgnoreSessionSettingsRestriction && SessionInfo.VersionInfos.Num() == 0)
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to create a session was made, but the session info was missing version info!");
		return nullptr;
	}
	...
	if (LiveSessions.Contains(SessionInfo.SessionId))
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to create a session with ID '%ls' was made, but that session already exists!", *SessionInfo.SessionId.ToString());
		return nullptr;
	}
	...
	if (GetLiveSessionIdByName(SessionInfo.SessionName).IsValid())
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to create a session with name '%ls' was made, but that session already exists!", *SessionInfo.SessionName);
		return nullptr;
	}
	...
	if (!DefaultSessionRepository && !MountDefaultSessionRepository(Settings.Get()))
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to create a session with name '%ls' was made, but the server did not have any repository mounted to store it! The repository may already be mounted by another process.", *SessionInfo.SessionName);
		return nullptr;
	}
...
TSharedPtr<IConcertServerSession> FConcertServer::RestoreSession(const FGuid& SessionId, const FConcertSessionInfo& SessionInfo, const FConcertSessionFilter& SessionFilter, FText& OutFailureReason)
{
	...
	UE_LOGF(LogConcert, Error, "An attempt to restore session '%ls' was made, but that session could not be found!", *SessionId.ToString());
	return nullptr;
}
...
TSharedPtr<IConcertServerSession> FConcertServer::CopySession(const FGuid& SrcSessionId, const FConcertSessionInfo& NewSessionInfo, const FConcertSessionFilter& SessionFilter, FText& OutFailureReason)
{
	if (!NewSessionInfo.SessionId.IsValid() || NewSessionInfo.SessionName.IsEmpty())
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to copy a session was made, but the session info was missing an ID or name!");
		return nullptr;
	}
	else if (!Settings->ServerSettings.bIgnoreSessionSettingsRestriction && NewSessionInfo.VersionInfos.Num() == 0)
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to copy a session was made, but the session info was missing version info!");
		return nullptr;
	}
	else if (LiveSessions.Contains(NewSessionInfo.SessionId))
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to copy a session with ID '%ls' was made, but that session already exists!", *NewSessionInfo.SessionId.ToString());
		return nullptr;
	}
	else if (GetLiveSessionIdByName(NewSessionInfo.SessionName).IsValid())
	{
		...
		UE_LOGF(LogConcert, Error, "An attempt to copy a session with name '%ls' was made, but that session already exists!", *NewSessionInfo.SessionName);
		return nullptr;
	}
	...
	else if (TSharedPtr<IConcertServerSession> LiveSession = LiveSessions.FindRef(SrcSessionId))
	{
		...
		{
			UE_LOGF(LogConcert, Display, "Live session '%ls' (%ls) was copied as '%ls' (%ls)", *LiveSessionInfo.SessionName, *LiveSessionInfo.SessionId.ToString(), *NewSessionInfo.SessionName, *NewSessionInfo.SessionId.ToString());
			return CreateLiveSession(NewSessionInfo, CopySessionRepository);
		}
		else
		{
			UE_LOGF(LogConcert, Error, "An attempt to copy a session '%ls' was made, but failed!", *SrcSessionId.ToString());
			return nullptr;
		}
	}
	...
	UE_LOGF(LogConcert, Error, "An attempt to copy a session '%ls' was made, but that session could not be found!", *SrcSessionId.ToString());
	return nullptr;
}
...
void FConcertServer::RecoverSessions(const FConcertServerSessionRepository& InRepository, bool bCleanupExpiredSessions)
{
	...
	for (FConcertSessionInfo& LiveSessionInfo : LiveSessionInfos)
	{
		...
		{
			UE_LOGF(LogConcert, Display, "Live session '%ls' (%ls) was recovered.", *LiveSessionInfo.SessionName, *LiveSessionInfo.SessionId.ToString());
		}
	}
	...
	else
	{
		...
		for (FConcertSessionInfo& ArchivedSessionInfo : ArchivedSessionInfos)
		{
			...
			{
				UE_LOGF(LogConcert, Display, "Archived session '%ls' (%ls) was discovered.", *ArchivedSessionInfo.SessionName, *ArchivedSessionInfo.SessionId.ToString());
			}
		}
	}
}
...
void FConcertServer::ArchiveOfflineSessions(const FConcertServerSessionRepository& InRepository)
{
	...
	for (FConcertSessionInfo& LiveSessionInfo : LiveSessionInfos)
	{
		...
		{
			UE_LOGF(LogConcert, Display, "Deleting %ls", *InRepository.GetSessionWorkingDir(LiveSessionInfo.SessionId));
			ConcertUtil::DeleteDirectoryTree(*InRepository.GetSessionWorkingDir(LiveSessionInfo.SessionId));
			UE_LOGF(LogConcert, Display, "Live session '%ls' (%ls) was archived on reboot.", *LiveSessionInfo.SessionName, *LiveSessionInfo.SessionId.ToString());
		}
	}
}
...
EConcertSessionRepositoryMountResponseCode FConcertServer::MountSessionRepository(FConcertServerSessionRepository Repository, bool bCreateIfNotExist, bool bCleanWorkingDir, bool bCleanExpiredSessions, bool bSearchByPaths, bool bAsDefault)
{
	...
	// Exclusive access scope to the session repository db.
	{
			...
			[&Repository, bSearchByPaths](const FConcertServerSessionRepository& Candidate){ return bSearchByPaths ? Candidate.WorkingDir == Repository.WorkingDir && Candidate.SavedDir == Repository.SavedDir : Candidate.RepositoryId == Repository.RepositoryId; }))
		{
			...
			{
				UE_LOGF(LogConcert, Display, "Remounted repository %ls. The repository is already mounted by this process.", *Repository.RepositoryId.ToString());
				bAlreadyMountedByThisProcess = true; // Already mounted by this process, don't process the session files again.
			}
			else
			{
				UE_LOGF(LogConcert, Warning, "Failed to mount repository %ls. The repository is already mounted by another process.", *Repository.RepositoryId.ToString());
		...
		{
			UE_LOGF(LogConcert, Warning, "Failed to mount repository %ls. The repository was not found.", *Repository.RepositoryId.ToString());
	...
	if (bAsDefault)
	{
		if (MountStatus == EConcertSessionRepositoryMountResponseCode::Mounted)
		{
			...
			UE_LOGF(LogConcert, Display, "Default session repository %ls set successfully.", *Repository.RepositoryId.ToString());
		}
		else
		{
			DefaultSessionRepository.Reset();
			UE_LOGF(LogConcert, Warning, "Default session repository %ls failed to mount.", *Repository.RepositoryId.ToString());
		}
...
bool FConcertServer::UnmountSessionRepository(const FGuid& RepositoryId, bool bDropped)
{
	...
	if (DefaultSessionRepository && DefaultSessionRepository->RepositoryId == RepositoryId)
	{
		...
		UE_LOGF(LogConcert, Warning, "Default repository %ls unmounted. No session will be created until a mounted repository is set as default", *RepositoryId.ToString());
	}
	else
	{
		UE_LOGF(LogConcert, Display, "Repository %ls unmounted.", *RepositoryId.ToString())
	}
	...
	if (bDropped && !Repository.RepositoryRootDir.IsEmpty()) // When dropped, the repository can be deleted if it has the standard root structure.
	{
		...
		{
			UE_LOGF(LogConcert, Display, "Repository %ls deleted.", *Repository.RepositoryId.ToString())
		}
	}
...
EConcertResponseCode FConcertServer::ValidateFileSharing(const FConcertSessionFileShare& InRequest, FConcertSessionFileShare& OutFileShare, FText& OutFailureReason)
{
	// Assume that we can't support file sharing and prove otherwise.
	OutFileShare.bFileShareEnabled = false;
	OutFileShare.FileShareRoot = TEXT("");

	/* Supply any file share information back to the client. */
	if (FileSharingService && InRequest.bFileShareEnabled && Settings->bEnableFileSharing)
	{
		// Try to accept the file sharing challenge
		OutFileShare.Challenge = InRequest.Challenge;

		FString BaseDir = FPaths::ConvertRelativePathToFull(Settings->RootFileSharingPath.Path);
		FString Normalized = FPaths::ConvertRelativePathToFull(InRequest.Challenge.Path);

		if (!BaseDir.IsEmpty() && FPaths::IsUnderDirectory(Normalized, BaseDir))
		{
			TSharedPtr<FArchive> Reader = FileSharingService->CreateReader(Normalized);
			if (Reader)
			{
				FConcertSessionChallengeData ChallengeData;
				*Reader << ChallengeData.ChallengeKey;
				*Reader << ChallengeData.CreationTimeInTicks;
	
				if (ChallengeData == InRequest.Challenge.Data)
				{
					// We can accept the challenge.
					OutFileShare.bFileShareEnabled = true;
					OutFileShare.FileShareRoot = Settings->RootFileSharingPath.Path;
				}
			}	
		}

		if (!OutFileShare.bFileShareEnabled)
		{
			OutFailureReason = LOCTEXT("Error_FailedChallenge", "The client failed to prove they could support file sharing.");
			return EConcertResponseCode::Failed;
		}
	}
	else if (FileSharingService && FileSharingService->IsEnabled() && Settings->bEnableFileSharing)
	{
		OutFailureReason = LOCTEXT("Error_NoFileSharing", "The server is setup for file sharing. Clients joining a session must support file sharing.");
		return EConcertResponseCode::Failed;
	}
	else if (InRequest.bFileShareEnabled)
	{
		OutFailureReason = LOCTEXT("Error_FileSharingFromClient", "The client asked for file sharing but the server does not support it.");		
		return EConcertResponseCode::Failed;
	}
...
template<typename TCreateOrJoinMessage>
bool FConcertServer::FillSessionResponseInfoFromFileShareChallenge(const TCreateOrJoinMessage* Message, FConcertAdmin_SessionInfoResponse& ResponseData, const FConcertSessionInfo& SessionInfo)
{
	const TCHAR* ServerSessionNamePtr = *SessionInfo.SessionName;

	if (Message->FileShare.bFileShareEnabled)
	{
		UE_LOGF(LogConcert, Display, "Validating file sharing challenge from user %ls to join session %ls (Id: %ls, Owner: %ls)", *Message->OwnerClientInfo.UserName, ServerSessionNamePtr, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
	}
	
	ResponseData.ResponseCode = ValidateFileSharing(Message->FileShare, ResponseData.FileShare, ResponseData.Reason);

	if (ResponseData.ResponseCode != EConcertResponseCode::Success)
	{
		UE_LOGF(LogConcert, Display, "Refusing user %ls to join session %ls because of failed file share challenge. (Id: %ls, Owner: %ls, Reason: %ls)", *Message->OwnerClientInfo.UserName, ServerSessionNamePtr, *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName, *ResponseData.Reason.ToString());
	}
	else
	{
		// Fill out the session info to return back to the client.
		ResponseData.SessionInfo = SessionInfo;
		UE_LOGF(LogConcert, Display, "Allowing user %ls to join session %ls.  File sharing is %ls. (Id: %ls, Owner: %ls)", *Message->OwnerClientInfo.UserName, ServerSessionNamePtr, ResponseData.FileShare.bFileShareEnabled ? TEXT("enabled") : TEXT("disabled"), *SessionInfo.SessionId.ToString(), *SessionInfo.OwnerUserName);
	}

	return ResponseData.ResponseCode == EConcertResponseCode::Success;  
}
...
TFuture<FConcertAdmin_SessionInfoResponse> FConcertServer::HandleCreateSessionRequest(const FConcertMessageContext& Context)
{
	...
	TSharedPtr<IConcertServerSession> NewServerSession;
	{
		...
		if (FillSessionResponseInfoFromFileShareChallenge(Message, ResponseData, SessionInfo))
		{
			// Copy the session file sharing information to the user. 
			SessionInfo.FileSharing = ResponseData.FileShare;
	...
	if (!NewServerSession)
	{
		...
		UE_LOGF(LogConcert, Display, "Session creation failed. (User: %ls, Reason: %ls)", *Message->OwnerClientInfo.UserName, *ResponseData.Reason.ToString());
	}
...
TFuture<FConcertAdmin_SessionInfoResponse> FConcertServer::HandleFindSessionRequest(const FConcertMessageContext& Context)
{
	...
	{
		FillSessionResponseInfoFromFileShareChallenge(Message, ResponseData, ServerSession->GetSessionInfo());
	}
	else
	{
		const TCHAR* ServerSessionNamePtr = ServerSession ? *ServerSession->GetName() : TEXT("<unknown>");
		ResponseData.ResponseCode = EConcertResponseCode::Failed;
		UE_LOGF(LogConcert, Display, "Refusing user %ls to join session %ls (Id: %ls, Owner: %ls, Reason: %ls)", *Message->OwnerClientInfo.UserName, ServerSessionNamePtr, *Message->SessionId.ToString(), ServerSession ? *ServerSession->GetSessionInfo().OwnerUserName : TEXT("unknown owner"), *ResponseData.Reason.ToString());
	}
...
TFuture<FConcertAdmin_SessionInfoResponse> FConcertServer::HandleCopySessionRequest(const FConcertMessageContext& Context)
{
	...
	else
	{
		...
		UE_LOGF(LogConcert, Display, "Session copy failed. (User: %ls, Reason: %ls)", *Message->OwnerClientInfo.UserName, *ResponseData.Reason.ToString());
	}
...
TFuture<FConcertAdmin_ArchiveSessionResponse> FConcertServer::HandleArchiveSessionRequest(const FConcertMessageContext& Context)
{
	...
	if (ServerSession)
	{
		...
		if (ArchivedSessionId.IsValid())
		{
			...
			UE_LOGF(LogConcert, Display, "User %ls archived session %ls (%ls) as %ls (%ls)", *Message->UserName, *ResponseData.SessionName, *ResponseData.SessionId.ToString(), *ResponseData.ArchiveName, *ResponseData.ArchiveId.ToString());
		}
		else
		{
			ResponseData.ResponseCode = EConcertResponseCode::Failed;
			ResponseData.Reason = FailureReason;
			UE_LOGF(LogConcert, Display, "User %ls failed to archive session %ls (Id: %ls, Reason: %ls)", *Message->UserName, *ResponseData.SessionName, *ResponseData.SessionId.ToString(), *ResponseData.Reason.ToString());
		}
	}
	else
	{
...
FConcertAdmin_RenameSessionResponse FConcertServer::RenameSessionInternal(const FConcertAdmin_RenameSessionRequest& Request, bool bCheckPermission)
{
	...
	if (TSharedPtr<IConcertServerSession> ServerSession = GetLiveSession(Request.SessionId)) // Live session?
	{
		...
		if (bCheckPermission && !IsRequestFromSessionOwner(ServerSession, Request.UserName, Request.DeviceName)) // Not owner?
		{
			...
			UE_LOGF(LogConcert, Error, "User %ls failed to rename live session '%ls' (Id: %ls, Owner: %ls, Reason: %ls)", *Request.UserName, *ServerSession->GetName(), *ResponseData.SessionId.ToString(), *ServerSession->GetSessionInfo().OwnerUserName, *ResponseData.Reason.ToString());
		}
		...
		else
		{
			...
			ResponseData.ResponseCode = EConcertResponseCode::Success;
			UE_LOGF(LogConcert, Display, "User %ls renamed live session %ls from %ls to %ls", *Request.UserName, *ResponseData.SessionId.ToString(), *ResponseData.OldName, *ServerSession->GetName());
		}
	}
	else if (FConcertSessionInfo* ArchivedSessionInfo = ArchivedSessions.Find(Request.SessionId)) // Archive session?
	{
		...
		{
			ResponseData.Reason = LOCTEXT("Error_Rename_InvalidPerms_NotOwner", "Not the session owner.");
			UE_LOGF(LogConcert, Display, "User %ls failed to rename archived session '%ls' (Id: %ls, Owner: %ls, Reason: %ls)", *Request.UserName, *ArchivedSessionInfo->SessionName, *ResponseData.SessionId.ToString(), *ArchivedSessionInfo->OwnerUserName, *ResponseData.Reason.ToString());
		}
		else if (GetArchivedSessionIdByName(Request.NewName).IsValid()) // Name collision?
		{
			...
			UE_LOGF(LogConcert, Error, "User %ls failed to rename archived session '%ls' (Id: %ls, Owner: %ls, Reason: %ls)", *Request.UserName, *ArchivedSessionInfo->SessionName, *ResponseData.SessionId.ToString(), *ArchivedSessionInfo->OwnerUserName, *ResponseData.Reason.ToString());
		}
		else
		{
			...
			ResponseData.ResponseCode = EConcertResponseCode::Success;
			UE_LOGF(LogConcert, Display, "User %ls renamed archived session %ls from %ls to %ls", *Request.UserName, *ResponseData.SessionId.ToString(), *ResponseData.OldName, *Request.NewName);
		}
	}
	else // Not found?
	{
		ResponseData.Reason = LOCTEXT("Error_Rename_DoesNotExist", "Session does not exist.");
		UE_LOGF(LogConcert, Display, "User %ls failed to rename session (Id: %ls, Reason: %ls)", *Request.UserName, *ResponseData.SessionId.ToString(), *ResponseData.Reason.ToString());
	}
	...
	for (const FGuid& SessionToDelete : Request.SessionIds)
	{
		if (TSharedPtr<IConcertServerSession> ServerSession = GetLiveSession(SessionToDelete))
		{
			...
			else if (!bHasPermission)
			{
				...
				UE_LOGF(LogConcert, Display, "User %ls failed to delete live session '%ls' (Id: %ls, Owner: %ls, Reason: %ls)", *Request.UserName, *ServerSession->GetName(), *SessionToDelete.ToString(), *ServerSession->GetSessionInfo().OwnerUserName, *OutResponse.Reason.ToString());
				return false;
			}
		...
		else if (const FConcertSessionInfo* ArchivedSessionInfo = ArchivedSessions.Find(SessionToDelete))
		{
			...
			{
				OutResponse.Reason = LOCTEXT("Error_BatchDelete_InvalidPerms_NotOwner", "Not the session owner.");
				UE_LOGF(LogConcert, Display, "User %ls failed to delete live session '%ls' (Id: %ls, Owner: %ls, Reason: %ls)", *Request.UserName, *ArchivedSessionInfo->SessionName, *SessionToDelete.ToString(), *ArchivedSessionInfo->OwnerUserName, *OutResponse.Reason.ToString());
				return false;
			}
		...
		else
		{
			...
			UE_LOGF(LogConcert, Display, "User %ls failed to delete session (Id: %ls, Reason: %ls)", *Request.UserName, *SessionToDelete.ToString(), *OutResponse.Reason.ToString());
			return false;
		}
	}
...
FConcertAdmin_DeleteSessionResponse FConcertServer::DeleteSessionInternal(const FConcertAdmin_DeleteSessionRequest& Request, bool bCheckPermission)
{
	...
	if (TSharedPtr<IConcertServerSession> ServerSession = GetLiveSession(Request.SessionId)) // Live session?
	{
		...
		if (bCheckPermission && !IsRequestFromSessionOwner(ServerSession, Request.UserName, Request.DeviceName))
		{
			...
			UE_LOGF(LogConcert, Display, "User %ls failed to delete live session '%ls' (Id: %ls, Owner: %ls, Reason: %ls)", *Request.UserName, *ResponseData.SessionName, *ResponseData.SessionId.ToString(), *ServerSession->GetSessionInfo().OwnerUserName, *ResponseData.Reason.ToString());
		}
		...
		{
			ResponseData.ResponseCode = EConcertResponseCode::Success;
			UE_LOGF(LogConcert, Display, "User %ls deleted live session %ls (%ls)", *Request.UserName, *ResponseData.SessionName, *ResponseData.SessionId.ToString());
		}
	}
	else if (const FConcertSessionInfo* ArchivedSessionInfo = ArchivedSessions.Find(Request.SessionId)) // Archived session?
	{
		...
		{
			ResponseData.Reason = LOCTEXT("Error_Delete_InvalidPerms_NotOwner", "Not the session owner.");
			UE_LOGF(LogConcert, Display, "User %ls failed to delete archived session '%ls' (Id: %ls, Owner: %ls, Reason: %ls)", *Request.UserName, *ArchivedSessionInfo->SessionName, *ResponseData.SessionId.ToString(), *ArchivedSessionInfo->OwnerUserName, *ResponseData.Reason.ToString());
		}
		else if (!DestroyArchivedSession(Request.SessionId, /*bDeleteSessionData*/true))
		{
			ResponseData.Reason = LOCTEXT("Error_Delete_SessionFailedToDestroy", "Failed to destroy session.");
			UE_LOGF(LogConcert, Display, "User %ls failed to delete archived session '%ls' (Id: %ls, Reason: %ls)", *Request.UserName, *ResponseData.SessionName, *ResponseData.SessionId.ToString(), *ResponseData.Reason.ToString());
		}
		else // Succeeded to delete the session.
		{
			ResponseData.ResponseCode = EConcertResponseCode::Success;
			UE_LOGF(LogConcert, Display, "User %ls deleted archived session %ls (%ls)", *Request.UserName, *ResponseData.SessionName, *ResponseData.SessionId.ToString());
		}
	}
	else // Not found?
	{
		ResponseData.Reason = LOCTEXT("Error_Delete_SessionDoesNotExist", "Session does not exist.");
		UE_LOGF(LogConcert, Display, "User %ls failed to delete session (Id: %ls, Reason: %ls)", *Request.UserName, *ResponseData.SessionId.ToString(), *ResponseData.Reason.ToString());
	}
...
TFuture<FConcertAdmin_GetSessionActivitiesResponse> FConcertServer::HandleGetSessionActivitiesRequest(const FConcertMessageContext& Context)
{
	...
	else // The only reason to get here is when the session is not found.
	{
		...
		UE_LOGF(LogConcert, Display, "Failed to fetch activities from session (Id: %ls, Reason: %ls)", *Message->SessionId.ToString(), *ResponseData.Reason.ToString());
	}
...
TSharedPtr<IConcertServerSession> FConcertServer::CreateLiveSession(const FConcertSessionInfo& SessionInfo, const FConcertServerSessionRepository& InRepository)
{
	...
	if (Settings->ServerSettings.bIgnoreSessionSettingsRestriction)
	{
		UE_CLOGF(LiveSessionInfo.VersionInfos.Num() > 0, LogConcert, Warning, "Clearing version information when creating session '%ls' due to -CONCERTIGNORE. This session will be unversioned!", *LiveSessionInfo.SessionName);
...
FGuid FConcertServer::ArchiveLiveSession(const FGuid& LiveSessionId, const FString& ArchivedSessionNameOverride, const FConcertSessionFilter& SessionFilter, FGuid ArchiveSessionIdOverride)
{
	...
	if (LiveSession)
	{
		...
		{
			UE_LOGF(LogConcert, Display, "Live session '%ls' (%ls) was archived as '%ls' (%ls)", *LiveSession->GetName(), *LiveSession->GetId().ToString(), *ArchivedSessionInfo.SessionName, *ArchivedSessionInfo.SessionId.ToString());
...
TSharedPtr<IConcertServerSession> FConcertServer::RestoreArchivedSession(const FGuid& ArchivedSessionId, const FConcertSessionInfo& NewSessionInfo, const FConcertSessionFilter& SessionFilter, FText& OutFailureReason)
{
	...
	if (const FConcertSessionInfo* ArchivedSessionInfo = ArchivedSessions.Find(ArchivedSessionId))
	{
		...
		if (Settings->ServerSettings.bIgnoreSessionSettingsRestriction)
		{
			UE_CLOGF(LiveSessionInfo.VersionInfos.Num() > 0, LogConcert, Warning, "Clearing version information when restoring session '%ls' due to -CONCERTIGNORE. This may lead to instability and crashes!", *NewSessionInfo.SessionName);
			LiveSessionInfo.VersionInfos.Reset();
		}
		else if (NewSessionInfo.VersionInfos.Num() > 0)
		{
			...
			if (LiveSessionInfo.VersionInfos.Num() > 0)
			{
				...
				{
					UE_LOGF(LogConcert, Error, "An attempt to restore session '%ls' was rejected due to a versioning incompatibility: %ls", *NewSessionInfo.SessionName, *OutFailureReason.ToString());
					return nullptr;
				}
		...
		{
			UE_LOGF(LogConcert, Display, "Archived session '%ls' (%ls) was restored as '%ls' (%ls)", *ArchivedSessionInfo->SessionName, *ArchivedSessionInfo->SessionId.ToString(), *LiveSessionInfo.SessionName, *LiveSessionInfo.SessionId.ToString());
