...
public:
	...
	/**
	 * Specifies if we should use a file sharing service for larger assets that are saved in a Multi-user session.
	 * Note the specified folder must be visible to all clients.
	 *
	 * This can be set on the command line using -CONCERTFILESHARINGENABLED
	 */
	UPROPERTY(config, EditAnywhere, Category = "File Sharing")
	bool bEnableFileSharing = false;

	/**
	 * The path for that shares files across clients. This path must be visible to all nodes. 
	 *
	 * This can be set on the command line using -CONCERTFILESHAREPATH=<FullPathName>
	 */
	UPROPERTY(config, EditAnywhere, meta=(EditCondition="bEnableFileSharing"), Category = "File Sharing")
	FDirectoryPath RootFileSharingPath;
};
