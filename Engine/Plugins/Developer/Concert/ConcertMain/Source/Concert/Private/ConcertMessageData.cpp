...
bool TryCompressImpl(const UScriptStruct* InEventType, const void* InEventData, FConcertSessionSerializedPayload& InOutPayload, EConcertPayloadCompressionType CompressionType)
{
	...
	if (ShouldCompress(InOutPayload, CompressionType))
	{
		...
		{
			const FNumericUnit<uint64> InBytesDisplay = FUnitConversion::QuantizeUnitsToBestFit(InBytes.Num(), EUnit::Bytes);
			const FNumericUnit<uint64> OutBytesDisplay = FUnitConversion::QuantizeUnitsToBestFit(CompressedSize, EUnit::Bytes);
			UE_LOGF(LogConcert, VeryVerbose, "Compressed payload for %ls from to %lld %ls to %lld %ls. Compression Type: %ls", *InEventType->GetName(), InBytesDisplay.Value,
				   FUnitConversion::GetUnitDisplayString(InBytesDisplay.Units), OutBytesDisplay.Value, FUnitConversion::GetUnitDisplayString(OutBytesDisplay.Units),
				   *NamedCompressionAlgo.ToString());   
			
			OutCompressedData.SetNum(CompressedSize, EAllowShrinking::No);
			InOutPayload.PayloadBytes.Bytes = MoveTemp(OutCompressedData);
			InOutPayload.PayloadCompressionMethod = UE::Concert::Compression::GetCompressionFromNamedType(NamedCompressionAlgo);
			InOutPayload.PayloadCompressionBias = UE::Concert::Compression::GetConcertCompressionBias(CompressFlags);
		}
		else
		{
			UE_LOGF(LogConcert, Warning, "Unable to compress data for %ls!", *InEventType->GetName());
			InOutPayload.PayloadCompressionMethod = EConcertCompressionMethod::Uncompressed;
			InOutPayload.PayloadCompressionBias = EConcertCompressionBias::None;
		}
	}
	else
	{
		const FNumericUnit<uint64> InBytesDisplay = FUnitConversion::QuantizeUnitsToBestFit(InOutPayload.PayloadBytes.Bytes.Num(), EUnit::Bytes);
		UE_LOGF(LogConcert, VeryVerbose, "Not compressing payload for %ls. Size is %lld %ls",
			   *InEventType->GetName(), InBytesDisplay.Value, FUnitConversion::GetUnitDisplayString(InBytesDisplay.Units));
...
{
	EConcertCompressionMethod CompressAlgo = InPayload.PayloadCompressionMethod;
	if (UE::Concert::Compression::DataIsCompressed(CompressAlgo))
	{
		...
		ECompressionFlags CompressFlags = UE::Concert::Compression::GetCoreCompressionFlags(InPayload.PayloadCompressionBias);
		...
		{
			UE_LOGF(LogConcert, Warning, "Unable to uncompress data for %ls!", *InPayload.PayloadTypeName.ToString());
		}
	}
