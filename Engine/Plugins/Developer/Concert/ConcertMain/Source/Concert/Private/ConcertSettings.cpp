...
FText ValidateFileSharingPath(const FString& Path)
{
	if (Path.IsEmpty())
	{
		return LOCTEXT("PathName_Empty", "Path name is empty.");
	}

	FText Reason;
	if (!FPaths::ValidatePath(Path, &Reason))
	{
		return FText::Format(LOCTEXT("IllegalPathName", "{0}"), Reason);
	}

	if (!FPaths::DirectoryExists(Path))
	{
		return FText::Format(LOCTEXT("DirectoryDoesNotExist", "Path {0} does not exist."), FText::FromString(Path));
	}
