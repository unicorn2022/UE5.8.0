...
struct FActiveLoggers
{
	...
	void ChangeVerbosity(const TArray<FString>& Args)
	{
		if ( Args.Num() < 1 )
		{
			UE_LOGF(LogConcert, Log, "Usage: Concert.SetLoggingLevel VerbosityLevel");
			return;
		}
#if !NO_LOGGING
...
{
	UE_LOGF(LogConcertDebug, Warning, "%ls: Message %ls timed-out sending to '%ls'.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(Message), *EndpointId.ToString());
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Acknowledgement '%ls' sent to '%ls' for message '%ls'.", *OwnerContext.ToString(), *AckData.MessageId.ToString(), *DestEndpoint.ToString(), *AckData.SourceMessageId.ToString());
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Endpoint closure '%ls' sent to '%ls'.", *OwnerContext.ToString(), *EndpointClosedData.MessageId.ToString(), *DestEndpoint.ToString());
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Handshake sent to '%ls' (state: %ls, channel: %d, index: %d).", 
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Handshake received from '%ls' (state: %ls, channel: %d, index: %d).", 
		*OwnerContext.ToString(),
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Message %ls published.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(Message));
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Message %ls sent to '%ls'.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(Message), *DestEndpoint.ToString());
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Message %ls received from '%ls'.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext), *ConcertContext.SenderConcertEndpointId.ToString());
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Message %ls queued.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext));
...
void FConcertLogger::LogMessageDiscarded(const FConcertMessageContext& ConcertContext, const FGuid& DestEndpoint, const EMessageDiscardedReason Reason)
{
	...
	case EMessageDiscardedReason::NotRequired:
		UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Message %ls discarded. Message was not required.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext));
		break;

	case EMessageDiscardedReason::AlreadyProcessed:
		UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Message %ls discarded. Message was already processed.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext));
		break;

	case EMessageDiscardedReason::UnknownEndpoint:
		UE_LOGF(LogConcertDebug, Warning, "%ls: Message %ls discarded. Unknown remote endpoint '%ls'.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext), *ConcertContext.Message->ConcertEndpointId.ToString());
		break;
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Event %ls processed.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext));
...
{
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Request %ls processed.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext));
...
void FConcertLogger::LogProcessResponse(const FConcertMessageContext& ConcertContext, const FGuid& DestEndpoint) 
{
	...
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Response %ls processed for request '%ls'.", *OwnerContext.ToString(), *ConcertLoggerUtil::GetMessageString(ConcertContext), *ResponseMessage->RequestMessageId.ToString());
...
void FConcertLogger::LogProcessAck(const FConcertMessageContext& ConcertContext, const FGuid& DestEndpoint)
{
	...
	UE_LOGF(LogConcertDebug, VeryVerbose, "%ls: Acknowledgement '%ls' processed for message '%ls'.", *OwnerContext.ToString(), *AckMessage->MessageId.ToString(), *AckMessage->SourceMessageId.ToString());
...
{
	UE_LOGF(LogConcertDebug, Display, "%ls: Remote endpoint '%ls' discovered.", *OwnerContext.ToString(), *ConcertContext.SenderConcertEndpointId.ToString());
...
{
	UE_LOGF(LogConcertDebug, Display, "%ls: Remote endpoint '%ls' timed-out.", *OwnerContext.ToString(), *EndpointId.ToString());
...
{
	UE_LOGF(LogConcertDebug, Display, "%ls: Remote endpoint '%ls' closed by remote peer.", *OwnerContext.ToString(), *EndpointId.ToString());
