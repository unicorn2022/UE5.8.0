...
UENUM()
enum class UE_DEPRECATED(5.8, "Use EConcertCompressionMethod instead.") EConcertCompressionDetails : uint8
{
...
/** Compression method to use. Note that these are assigned a bit shifted value to match the previous implementation in EConcertCompressionDetails. */
UENUM()
enum class EConcertCompressionMethod : uint8
{
...
UENUM()
enum class EConcertCompressionBias : uint8
{
...
inline bool DataIsCompressed(EConcertCompressionMethod InFormat)
{
	return InFormat == EConcertCompressionMethod::Zlib || InFormat == EConcertCompressionMethod::Oodle;
}

/** Returns true if the compression details is set to uncompressed format. */
inline bool DataIsUncompressed(EConcertCompressionMethod InFormat)
{
	return InFormat == EConcertCompressionMethod::Uncompressed;
}

/** Returns true if we are suppose to use Oodle for compression. */
inline bool DataIsCompressedWithOodle(EConcertCompressionMethod InFormat)
{
	return InFormat == EConcertCompressionMethod::Oodle;
}
...
inline FName GetCompressionAlgorithm(EConcertCompressionMethod InDetails)
{
...
inline ECompressionFlags GetCoreCompressionFlags(EConcertCompressionBias InBias)
{
	if (InBias == EConcertCompressionBias::Size)
	{
		return COMPRESS_BiasSize;
	}
	if (InBias == EConcertCompressionBias::Speed)
	{
		return COMPRESS_BiasSpeed;
	}
	return COMPRESS_NoFlags;
...
/** Get the concert compression bias value given the compressor compression flags. */
inline EConcertCompressionBias GetConcertCompressionBias(ECompressionFlags InFlags)
{
	if (InFlags == COMPRESS_BiasSize)
	{
		return EConcertCompressionBias::Size;
	}
	
	if (InFlags == COMPRESS_BiasSpeed)
	{
		return EConcertCompressionBias::Speed;
	}
	
	return EConcertCompressionBias::None;
}

/** For a given named compression algorithm and compression flags convert it into a EConcertCompressionMethod */
inline EConcertCompressionMethod GetCompressionFromNamedType(FName NamedMethod)
{
	check(NamedMethod == NAME_Oodle || NamedMethod == NAME_Zlib);

	EConcertCompressionMethod Compressor = EConcertCompressionMethod::Zlib;

	if (NamedMethod == NAME_Oodle)
	{
		Compressor = EConcertCompressionMethod::Oodle;
	}
...
template <typename SizeType>
inline EConcertCompressionMethod GetCompressionMethod(SizeType DataSize)
{
	static_assert(std::is_integral<SizeType>::value);
	...
	if (ShouldCompress(DataSize) && CompressionType != 0)
	{
		...
		{
			return EConcertCompressionMethod::Oodle;
		}
		return EConcertCompressionMethod::Zlib;
	}
	return EConcertCompressionMethod::Uncompressed;
}
...
USTRUCT()
struct FConcertSessionChallengeData 
{
	GENERATED_BODY()

	FConcertSessionChallengeData()
	{
		ChallengeKey = FGuid::NewGuid();
		CreationTimeInTicks = FDateTime::Now().GetTicks();
	}

	bool operator==(const FConcertSessionChallengeData& Other) const = default;
	
	UPROPERTY(Meta = (IgnoreForMemberInitializationTest))
	FGuid ChallengeKey;

	UPROPERTY()
	int64 CreationTimeInTicks;
};

USTRUCT()
struct FConcertSessionFileShareChallenge
{
	GENERATED_BODY()
	...
	UPROPERTY()
	FConcertSessionChallengeData Data;
};
...
struct FConcertSessionFileShare
{
	 ...
	 */
	UPROPERTY()
	bool bFileShareEnabled = false;

	/**
	 * Root path for all file shares for this session.
	 */
	UPROPERTY()
	FString FileShareRoot;

	/**
	 * A challenge id that can be shared with the server / client to confirm that
	 * file sharing can be established between a server and a client.
	 */
	UPROPERTY()
	FConcertSessionFileShareChallenge Challenge;
};
...
struct FConcertSessionInfo
{
	...
	UPROPERTY()
	FConcertSessionFileShare FileSharing;
...
struct FConcertSessionSerializedPayload
{
	...
	{
		return UE::Concert::Compression::DataIsCompressed(PayloadCompressionMethod);
	}
	...
	UPROPERTY(VisibleAnywhere, Category = "Payload")
	EConcertCompressionMethod PayloadCompressionMethod = EConcertCompressionMethod::Uncompressed;

	/** Indicates how the serialized payload has been compressed. */
	UPROPERTY(VisibleAnywhere, Category = "Payload")
	EConcertCompressionBias PayloadCompressionBias = EConcertCompressionBias::None;
