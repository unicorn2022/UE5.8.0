...
static void* LoadAndCheckRenderDocLibrary(FRenderDocPluginLoader::RENDERDOC_API_CONTEXT*& RenderDocAPI, const FString& RenderDocPath)
{
	...
	if (!FPaths::FileExists(PathToRenderDocDLL))
	{
		...
		{
			UE_LOGF(RenderDocPlugin, Display, "unable to locate RenderDoc library at: %ls", *PathToRenderDocDLL);
		}
		else
		{
			UE_LOGF(RenderDocPlugin, Warning, "unable to locate RenderDoc library at: %ls", *PathToRenderDocDLL);
		}
		return(nullptr);
	}

	UE_LOGF(RenderDocPlugin, Log, "a RenderDoc library has been located at: %ls", *PathToRenderDocDLL);

	void* RenderDocDLL = FPlatformProcess::GetDllHandle(*PathToRenderDocDLL);
	if (RenderDocDLL == nullptr)
	{
		UE_LOGF(RenderDocPlugin, Warning, "unable to dynamically load RenderDoc library");
		return(nullptr);
	}
	...
	{
		UE_LOGF(RenderDocPlugin, Warning, "unable to obtain 'RENDERDOC_GetAPI' function from '%ls'. You are likely using an incompatible version of RenderDoc.", *RenderDocDllName);
	...
	{
		UE_LOGF(RenderDocPlugin, Warning, "unable to initialize RenderDoc library due to API incompatibility (plugin requires eRENDERDOC_API_Version_1_0_0).");
		FPlatformProcess::FreeDllHandle(RenderDocDLL);
		return(nullptr);
	}
	...
	UE_LOGF(RenderDocPlugin, Log, "RenderDoc library has been loaded (RenderDoc API v%i.%i.%i).", MajorVersion, MinorVersion, PatchVersion);
...
void FRenderDocPluginLoader::Initialize()
{
	...
	if (!bCapture)
	{
		UE_LOGF(RenderDocPlugin, Display, "RenderDoc plugin will not be loaded. Use '-AttachRenderDoc' on the cmd line or enable 'renderdoc.AutoAttach' in the plugin settings.");
		return;
	}
	...
	if (GUsingNullRHI)
	{
		...
		UE_LOGF(RenderDocPlugin, Display, "RenderDoc plugin will not be loaded because a null RHI (Cook Server, perhaps) is being used.");
		return;
	}

	// Look for a renderdoc.dll somewhere in the system:
	UE_LOGF(RenderDocPlugin, Log, "locating RenderDoc library (%ls)...", *RenderDocDllName);
	...
	if (RenderDocDLL == nullptr && DesktopPlatform && !FApp::IsUnattended())
	{
		...
		UE_LOGF(RenderDocPlugin, Log, "RenderDoc library not found; provide a custom installation location...");
	...
	if (RenderDocDLL == nullptr)
	{
		if (FApp::IsUnattended())
		{
			UE_LOGF(RenderDocPlugin, Display, "Unable to initialize the plugin because no RenderDoc libray has been located.");
		}
		else
		{
			UE_LOGF(RenderDocPlugin, Warning, "Unable to initialize the plugin because no RenderDoc libray has been located.");
		}

#if PLATFORM_LINUX
		UE_LOGF(RenderDocPlugin, Log, "Please add qrenderdoc directory to your PATH or set 'renderdoc.BinaryPath' cvar to a valid directory.");
#endif

		return;
	}

	UE_LOGF(RenderDocPlugin, Log, "plugin has been loaded successfully.");
}
...
void FRenderDocPluginLoader::Release()
{
	...
	UE_LOGF(RenderDocPlugin, Log, "plugin has been unloaded.");
}
