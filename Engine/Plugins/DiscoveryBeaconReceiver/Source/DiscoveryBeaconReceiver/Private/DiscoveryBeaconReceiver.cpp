...
void FDiscoveryBeaconReceiver::Startup()
{
	...
	{
		UE_LOGF(LogDiscoveryBeaconReceiver, Warning, "No discovery address provided for %ls", *Description);
		return;
	}
	...
	if (DiscoveryPort < 0)
	{
		UE_LOGF(LogDiscoveryBeaconReceiver, Warning, "No valid discovery port provided for %ls (got %d)", *Description, DiscoveryPort);
		return;
	}
	...
	if (!Socket)
	{
		UE_LOGF(LogDiscoveryBeaconReceiver, Warning, "%ls failed to create multicast socket", *Description);
		return;
	}
	...
	// Note: If a new interface appears later on, the socket will need to join it in order to receive multicast packets from it.
	{
		...
		for (TSharedPtr<FInternetAddr>& LocalIp : LocapIps)
		{
			...
			if (bJoinedGroup)
			{
				UE_LOGF(LogDiscoveryBeaconReceiver, Log,
					"%ls joined multicast group '%ls' on detected local interface '%ls'",
					*Description, *DiscoveryAddress.ToString(), *LocalIp->ToString(false));
			}
			else
			{
				UE_LOGF(LogDiscoveryBeaconReceiver, Warning,
					"%ls failed to join multicast group '%ls' on detected local interface '%ls'",
					*Description, *DiscoveryAddress.ToString(), *LocalIp->ToString(false));
			}
		}
	}
