...
void UInterchangeDatasmithStaticMeshPipeline::ApplyAdditionalData(const UInterchangeBaseNodeContainer* NodeContainer, const UInterchangeStaticMeshFactoryNode* FactoryNode, UStaticMesh* StaticMesh) const
{
	...
	TArray<UDatasmithInterchangeStaticMeshDataNode*> StaticMeshDataNodes;
	NodeContainer->GetNodesOfType<UDatasmithInterchangeStaticMeshDataNode>(StaticMeshDataNodes);
	if (StaticMeshDataNodes.Num() == 0)
	{
		return;
	}

	const UDatasmithInterchangeStaticMeshDataNode* StaticMeshDataNode = StaticMeshDataNodes[0];
