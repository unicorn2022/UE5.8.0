...
namespace JacketingProcessUtils
{
	...
	float ValidateVoxelSize(float InAccuracy, const TArray<UStaticMeshComponent*>& StaticMeshComponents)
	{
		...
		if (MaxGuestimatedVoxelCount > EmpiricalReasonableVoxelCount)
		{
			...
			UE_LOGF(LogJacketingProcess, Warning, "AssemblyJacketing: Voxel precision of %.5f too small. Using %.5f instead.", InAccuracy, OutAccuracy);
		}
...
void FJacketingProcess::FindOverlappingActors(const TArray<AActor*>& InActorsToTest, const TArray<AActor*>& InActorsToTestAgainst, const FJacketingOptions* Options, TArray<AActor*>& OutOverlappingActors, bool bSilent)
{
	...
	{
		UE_LOGF(LogJacketingProcess, Warning, "FindOverlappingActors: No actors to process. Aborting...");
		return;
	}
	...
	{
		UE_LOGF(LogJacketingProcess, Warning, "FindOverlappingActors: No meshes to process. Aborting...");
		return;
	}
	...
	if (Geometry.Num() == 0)
	{
		UE_LOGF(LogJacketingProcess, Warning, "FindOverlappingActors: No geometry to process. Aborting...");
		return;
	}
	...
	if (!Volume.IsValid())
	{
		UE_LOGF(LogJacketingProcess, Error, "FindOverlappingActors: Voxelization of geometry failed. Aborting process...");
		return;
	}
	...
	UE_LOGF(LogJacketingProcess, Log, "FindOverlappingActors: Creation of volume took %.3f s.", IntermediateTime);
	...
	if (HoleRadius > 0.5 * BBoxMinorAxis)
	{
		...
		UE_LOGF(LogJacketingProcess, Warning, "OverlappingActors: Merge distance %f too large, clamped to %f.", Options->MergeDistance, float(2. * HoleRadius));
	}
	...
	UE_LOGF(LogJacketingProcess, Log, "FindOverlappingActors: Closure of gaps took %.3f s.", IntermediateTime);
	LastTime = FPlatformTime::Cycles64();
	...
	UE_LOGF(LogJacketingProcess, Log, "FindOverlappingActors: took %d min %.3f s. %d occluded actors out of %d", ElapsedMin, ElapsedSeconds, OverlappingActorSet.Num(), BuildVolumeComponents.Num());
...
void FJacketingProcess::ApplyJacketingOnMeshActors(const TArray<AActor*>& Actors, const FJacketingOptions* Options, TArray<AActor*>& OccludedActorArray, bool bSilent)
{
	if (Actors.Num() == 0)
	{
		UE_LOGF(LogJacketingProcess, Warning, "AssemblyJacketing: No actors to process. Aborting...");
		return;
	}
	...
	{
		UE_LOGF(LogJacketingProcess, Warning, "AssemblyJacketing: No meshes to process. Aborting...");
		return;
	}
	...
	if (Geometry.Num() == 0)
	{
		UE_LOGF(LogJacketingProcess, Warning, "AssemblyJacketing: No geometry to process. Aborting...");
		return;
	}
	...
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: Processing %i components, %i polygons", StaticMeshComponents.Num(), PolygonCount);

	TUniquePtr<IProxyLODVolume> Volume(IProxyLODVolume::CreateSDFVolumeFromMeshArray(Geometry, Accuracy));
	if (!Volume.IsValid())
	{
		UE_LOGF(LogJacketingProcess, Error, "AssemblyJacketing: Voxelization of geometry failed. Aborting process...");
		return;
	}
	...
	double IntermediateTime = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - LastTime);
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: Creation of volume took %.3f s.", IntermediateTime);
	LastTime = FPlatformTime::Cycles64();
	...
	if (HoleRadius > .5 * BBoxMinorAxis)
	{
		...
		UE_LOGF(LogJacketingProcess, Warning, "AssemblyJacketing: Merge distance %f too large, clamped to %f.", Options->MergeDistance, float(2. * HoleRadius));
	}
	...
	IntermediateTime = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - LastTime);
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: Closure of gaps took %.3f s.", IntermediateTime);
	LastTime = FPlatformTime::Cycles64();
	...
	if (Options->Target == EJacketingTarget::Level)
	{
		...
		int32 ElapsedMin = int32(ElapsedSeconds / 60.0);
		ElapsedSeconds -= 60.0 * (double)ElapsedMin;
		UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: took %d min %.3f s. %d occluded actors out of %d", ElapsedMin, ElapsedSeconds, OccludedActorSet.Num(), StaticMeshComponents.Num());
	...
	IntermediateTime = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - LastTime);
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: Processing of meshes took %.3f s.", IntermediateTime);
	LastTime = FPlatformTime::Cycles64();
	...
	IntermediateTime = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - LastTime);
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: Processing of vertices took %.3f s.", IntermediateTime);
	LastTime = FPlatformTime::Cycles64();
	...
	IntermediateTime = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - LastTime);
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: Deleting triangles took %.3f s.", IntermediateTime);
	LastTime = FPlatformTime::Cycles64();
	...
	IntermediateTime = FPlatformTime::ToSeconds64(FPlatformTime::Cycles64() - LastTime);
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: Rebuilding static meshes took %.3f s.", IntermediateTime);
	LastTime = FPlatformTime::Cycles64();
	...
	if (OccludedActorSet.Num() > 0)
	{
		...
		for (AActor* Actor : OccludedActorSet)
		{
			...
			{
				UE_LOGF(LogJacketingProcess, Error, "AssemblyJacketing: Cannot delete Actor %ls.", *Actor->GetActorLabel());
			}
		}
	...
	int32 ElapsedMin = int32(ElapsedSeconds / 60.0);
	ElapsedSeconds -= 60.0 * (double)ElapsedMin;
	UE_LOGF(LogJacketingProcess, Log, "AssemblyJacketing: took %d min %.3f s. %d occluded components out of %d, %d polygons deleted out of %d",
