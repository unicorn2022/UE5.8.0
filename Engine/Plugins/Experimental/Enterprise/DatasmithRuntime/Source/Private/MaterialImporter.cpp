...
namespace DatasmithRuntime
{
	...
	void FSceneImporter::ProcessMaterialData(FAssetData& MaterialData)
	{
		...
		{
			UE_LOGF(LogDatasmithRuntime, Error, "A material marked for deletion is actually used by the scene");
			return;
		}
	...
	EActionResult::Type FSceneImporter::ProcessMaterial(FSceneGraphId ElementId)
	{
		...
		else
		{
			...
			UE_LOGF(LogDatasmithRuntime, Warning, "Failed to create material %ls", Element->GetName());
