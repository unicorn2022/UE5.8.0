...
namespace DatasmithRuntime
{
	...
	EActionResult::Type FSceneImporter::CreateTexture(FSceneGraphId ElementId)
	{
		...
		if (TextureData.Width == 0 || TextureData.Height == 0 || TextureData.ImageData == nullptr)
		{
			...
			UE_LOGF(LogDatasmithRuntime, Warning, "Failed to create texture %ls", TextureElement->GetName());
	...
	bool FSceneImporter::LoadTexture(FSceneGraphId ElementId)
	{
		...
		if (!bSuccessfulLoad)
		{
			...
			UE_LOGF(LogDatasmithRuntime, Warning, "Cannot load image file %ls for texture %ls", TextureElement->GetFile(), TextureElement->GetLabel());
		}
		else
		{
	...
	void FSceneImporter::ProcessTextureData(FSceneGraphId TextureId)
	{
		...
		{
			UE_LOGF(LogDatasmithRuntime, Error, "A texture marked for deletion is actually used by the scene");
			return;
		}
