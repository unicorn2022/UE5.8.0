...
namespace DatasmithRuntime
{
	...
	Timer::~Timer()
	{
		...
		UE_LOGF(LogDatasmithRuntime, Log, "%ls in [%.3f ms] ( since beginning [%d min %.3f s] )", *Text, ElapsedMilliSeconds, MinSinceOrigin, SecondsSinceOrigin);
	}
#endif
	...
	void FSceneImporter::Tick(float DeltaSeconds)
	{
		...
		if (TasksToComplete == EWorkerTask::NoTask && SceneElement.IsValid())
		{
			...
			UE_LOGF(LogDatasmithRuntime, Log, "Total load time is [%d min %.3f s]", ElapsedMin, ElapsedSeconds);
#endif
		}
	}
	...
	bool FSceneImporter::IncrementalUpdate(TSharedRef< IDatasmithScene > InSceneElement, FUpdateContext& UpdateContext)
	{
		...
		GlobalStartTime = FPlatformTime::Seconds();
#endif
		UE_LOGF(LogDatasmithRuntime, Log, "Incremental update...");
		...
		for (TSharedPtr<IDatasmithElement>& ElementPtr : Modifications)
		{
			if (Elements.Contains(ElementPtr->GetNodeId()))
			{
				...
				if (AssetDataList.Contains(ElementId))
				{
					...
					UE_LOGF(LogDatasmithRuntime, Log, "IncrementalModifications: %ls %ls (%d)", *Prefix, ElementPtr->GetName(), ElementPtr->GetNodeId());
				...
				else if (ActorDataList.Contains(ElementId))
				{
					...
					UE_LOGF(LogDatasmithRuntime, Log, "IncrementalModifications: Actor %ls (%d)", ElementPtr->GetName(), ElementPtr->GetNodeId());
		...
		for (DirectLink::FSceneGraphId& ElementId : Deletions)
		{
			if (Elements.Contains(ElementId))
			{
				if (AssetDataList.Contains(ElementId))
				{
					...
					UE_LOGF(LogDatasmithRuntime, Log, "IncrementalDeletions: %ls %d", *Prefix, ElementId);
				...
				{
					UE_LOGF(LogDatasmithRuntime, Log, "IncrementalDeletions: actor %ls (%d)", Elements[ElementId]->GetLabel(), ElementId);
				...
				{
					UE_LOGF(LogDatasmithRuntime, Error, "Element %d (%ls) was not found", ElementId, Elements[ElementId]->GetName());
					ensure(false);
				}
			}
		}
	...
	void FSceneImporter::IncrementalAdditions(TArray<TSharedPtr<IDatasmithElement>>& Additions, TArray<TSharedPtr<IDatasmithElement>>& Updates)
	{
		...
		LocalAddAsset = [&](TSharedPtr<IDatasmithElement>&& Element, EDataType DataType) -> void
		{
			...
			if (this->AssetElementMapping.Contains(PrefixedName))
			{
				...
				{
					UE_LOGF(LogDatasmithRuntime, Error, "Found a new %ls (%d) with the same name, %ls, as an existing one (%d).", *Prefix, ElementId, this->Elements[ExistingElementId]->GetName(), ExistingElementId);
				}
		...
		for (TSharedPtr<IDatasmithElement>& ElementPtr : Additions)
		{
			if (ElementPtr->IsA(EDatasmithElementType::BaseMaterial))
			{
				UE_LOGF(LogDatasmithRuntime, Log, "IncrementalAdditions: Material %ls (%d)", ElementPtr->GetName(), ElementPtr->GetNodeId());
				LocalAddAsset(MoveTemp(ElementPtr), EDataType::Material);
			}
			else if (ElementPtr->IsA(EDatasmithElementType::StaticMesh))
			{
				UE_LOGF(LogDatasmithRuntime, Log, "IncrementalAdditions: StaticMesh %ls (%d)", ElementPtr->GetName(), ElementPtr->GetNodeId());
			...
			{
				UE_LOGF(LogDatasmithRuntime, Log, "IncrementalAdditions: Texture %ls (%d)", ElementPtr->GetName(), ElementPtr->GetNodeId());
			...
			{
				UE_LOGF(LogDatasmithRuntime, Log, "IncrementalAdditions: Actor %ls (%d)", ElementPtr->GetName(), ElementPtr->GetNodeId());
			}
		}
				...
				for (FReferencer& Referencer : AssetDataList[ElementId].Referencers)
				{
					if (AssetDataList.Contains(Referencer.ElementId))
					{
						...
						if (AssetData.HasState(EAssetState::Processed) && !AssetData.HasState(EAssetState::PendingDelete))
						{
							...
							UE_LOGF(LogDatasmithRuntime, Error, "Element %ls (%d) marked for deletion but referencer %ls (%d) is neither marked for deletion or for update", ElementName, ElementId, ReferencerName, Referencer.ElementId);
