...
namespace DatasmithRuntime
{
	bool FSceneImporter::ProcessMeshData(FAssetData& MeshData)
	{
		...
		{
			UE_LOGF(LogDatasmithRuntime, Error, "A mesh marked for deletion is actually used by the scene");
			return false;
		}
		...
		for (int32 Index = 0; Index < MeshElement->GetMaterialSlotCount(); Index++)
		{
			if (const IDatasmithMaterialIDElement* MaterialIDElement = MeshElement->GetMaterialSlotAt(Index).Get())
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogDatasmithRuntime, Warning, "ProcessMeshData: Cannot find material %ls", *MaterialPathName);
					}
				}
			}
		}
	...
	bool FSceneImporter::CreateStaticMesh(FSceneGraphId ElementId)
	{
		...
		FDatasmithMeshElementPayload MeshPayload;
		{
			if (!Translator->LoadStaticMesh(MeshElement, MeshPayload))
			{
				...
				if (!bSecondTrySucceeded)
				{
					...
					UE_LOGF(LogDatasmithRuntime, Warning, "CreateStaticMesh: Loading file %ls failed. Mesh element %ls has not been imported", MeshElement->GetFile(), MeshElement->GetLabel());
		...
		if (MeshDescriptions.Num() == 0)
		{
			...
			UE_LOGF(LogDatasmithRuntime, Warning, "CreateStaticMesh: %ls does not have a mesh description", MeshElement->GetLabel());
		...
		for (int32 Index = 0; Index < MeshElement->GetMaterialSlotCount(); Index++)
		{
			if (const IDatasmithMaterialIDElement* MaterialIDElement = MeshElement->GetMaterialSlotAt(Index).Get())
			{
				if (MaterialIDElement->GetId() >= 0)
				{
					...
					{
						UE_LOGF(LogDatasmithRuntime, Warning, "CreateStaticMesh: Cannot assign material %ls to any slot", MaterialIDElement->GetName());
						continue;
					}
