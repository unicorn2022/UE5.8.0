...
#include "ControlRigPhysicsEditorModule.h"
...
#define LOCTEXT_NAMESPACE "FControlRigPhysicsEditorModule"

namespace ControlRigPhysicsEditorModulePrivate
{
	static const FName DebugTabID(TEXT("ControlRigPhysicsDebug"));

	// The sibling debug tab from the ControlRigDynamics plugin. We don't depend on that module - we just
	// recognise its tab ID string. If that plugin isn't loaded, this lookup simply returns null.
	static const FName SiblingDebugTabID(TEXT("ControlRigDynamicsDebug"));
	...
	// relocation ticker may still be called after shutdown begins; when it does, this flag tells it
	// to early-return before touching FGlobalTabmanager (which may be mid-teardown) or scheduling
	// further Slate work.
		...
		*HandleHolder = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
			[WeakNewTab, TabLabel, HandleHolder](float /*DeltaTime*/) -> bool
			{
				...
				const TSharedRef<SDockTab> ReplacementTab = SNew(SDockTab)
					...
					.Label(TabLabel)
					[
						SNew(SControlRigPhysicsDebugWidget)
					];
...
//======================================================================================================================
void FControlRigPhysicsEditorModule::StartupModule()
{
	using namespace ControlRigPhysicsEditorModulePrivate;
	...
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
			DebugTabID,
			FOnSpawnTab::CreateRaw(this, &FControlRigPhysicsEditorModule::OnSpawnDebugTab))
		.SetDisplayName(LOCTEXT("DebugTabTitle", "ControlRig Physics Debug"))
		.SetTooltipText(LOCTEXT("DebugTabTooltip",
			"Opens the ControlRig Physics debug panel: toggle visualization, override solver time-step settings, etc."))
		...
		{
			return ControlRigPhysicsEditorModulePrivate::CurrentDebugTab.Pin();
		}));

	// Extend the Control Rig asset editor's toolbar with a button that opens the debug tab.
	UToolMenus::RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateRaw(
		this, &FControlRigPhysicsEditorModule::RegisterMenus));
}

//======================================================================================================================
void FControlRigPhysicsEditorModule::ShutdownModule()
{
	using namespace ControlRigPhysicsEditorModulePrivate;
...
//======================================================================================================================
void FControlRigPhysicsEditorModule::RegisterMenus()
{
	using namespace ControlRigPhysicsEditorModulePrivate;
	...
	FToolMenuSection& Section = ControlRigToolBar->FindOrAddSection("ControlRigPhysicsDebug");

	Section.AddEntry(FToolMenuEntry::InitToolBarButton(
		"OpenControlRigPhysicsDebug",
		FUIAction(FExecuteAction::CreateLambda([TabID = DebugTabID]()
		{
			FGlobalTabmanager::Get()->TryInvokeTab(FTabId(TabID));
		})),
		LOCTEXT("ToolBarButtonLabel", "Physics Debug"),
		LOCTEXT("ToolBarButtonTooltip", "Open the ControlRig Physics debug panel."),
		FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Settings")));
}

//======================================================================================================================
TSharedRef<SDockTab> FControlRigPhysicsEditorModule::OnSpawnDebugTab(const FSpawnTabArgs& /*SpawnTabArgs*/)
{
	const FText TabLabel = LOCTEXT("DebugTabTitle", "ControlRig Physics Debug");
	...
	ControlRigPhysicsEditorModulePrivate::CurrentDebugTab = NewTab;
	ControlRigPhysicsEditorModulePrivate::ScheduleSiblingDockingIfNeeded(NewTab, TabLabel);
...
IMPLEMENT_MODULE(FControlRigPhysicsEditorModule, ControlRigPhysicsEditor)
