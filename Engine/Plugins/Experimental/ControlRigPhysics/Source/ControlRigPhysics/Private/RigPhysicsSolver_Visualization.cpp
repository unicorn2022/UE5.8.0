...
// TODO expose these in the visualization settings
...
TAutoConsoleVariable<int> CVarControlRigPhysicsShowActiveContactsOverride(
	TEXT("ControlRig.Physics.ShowActiveContactsOverride"), -1,
...
TAutoConsoleVariable<int> CVarControlRigPhysicsShowInactiveContactsOverride(
	TEXT("ControlRig.Physics.ShowInactiveContactsOverride"), -1,
...
TAutoConsoleVariable<int> CVarControlRigPhysicsShowBodiesOverride(
	TEXT("ControlRig.Physics.ShowBodiesOverride"), -1,
	TEXT("Whether to draw bodies (requires visualization to be enabled). -1 uses the visualization setting, 0 forces drawing to be disabled, 1 forces it to be enabled."));

TAutoConsoleVariable<int> CVarControlRigPhysicsShowCentreOfMassOverride(
	TEXT("ControlRig.Physics.ShowCentreOfMassOverride"), -1,
	TEXT("Whether to draw centres of mass (requires visualization to be enabled). -1 uses the visualization setting, 0 forces drawing to be disabled, 1 forces it to be enabled."));

TAutoConsoleVariable<int> CVarControlRigPhysicsShowJointsOverride(
	TEXT("ControlRig.Physics.ShowJointsOverride"), -1,
	TEXT("Whether to draw joints (requires visualization to be enabled). -1 uses the visualization setting, 0 forces drawing to be disabled, 1 forces it to be enabled."));

TAutoConsoleVariable<int> CVarControlRigPhysicsShowControlsOverride(
	TEXT("ControlRig.Physics.ShowControlsOverride"), -1,
	TEXT("Whether to draw controls (requires visualization to be enabled). -1 uses the visualization setting, 0 forces drawing to be disabled, 1 forces it to be enabled."));

TAutoConsoleVariable<int> CVarControlRigPhysicsShowWorldObjectsOverride(
	TEXT("ControlRig.Physics.ShowWorldObjectsOverride"), -1,
	TEXT("Whether to draw world objects (requires visualization to be enabled). -1 uses the visualization setting, 0 forces drawing to be disabled, 1 forces it to be enabled."));

TAutoConsoleVariable<int> CVarControlRigPhysicsShowWorldOverlapBoxOverride(
	TEXT("ControlRig.Physics.ShowWorldOverlapBoxOverride"), -1,
	TEXT("Whether to draw the world overlap box (requires visualization to be enabled). -1 uses the visualization setting, 0 forces drawing to be disabled, 1 forces it to be enabled."));

//======================================================================================================================
// Tri-state helper for the Show*Override CVars: -1 defers to the authored setting, 0 forces off, anything else forces on.
inline bool GetControlRigPhysicsShow(const bool SettingsValue, const TAutoConsoleVariable<int>& CVar)
{
...
// per-element Show*Override CVars. Used as the early-out for Draw() in the runtime gate so we don't
// pay the cost of entering Draw (which also enables low-level Chaos debug draw) when nothing visible
...
{
	return GetControlRigPhysicsShow(
		VisualizationSettings.bShowBodies, CVarControlRigPhysicsShowBodiesOverride)
		|| GetControlRigPhysicsShow(
			VisualizationSettings.bShowCentreOfMass, CVarControlRigPhysicsShowCentreOfMassOverride)
		|| GetControlRigPhysicsShow(
			VisualizationSettings.bShowJoints, CVarControlRigPhysicsShowJointsOverride)
		|| GetControlRigPhysicsShow(
			VisualizationSettings.bShowControls, CVarControlRigPhysicsShowControlsOverride)
		|| GetControlRigPhysicsShow(
			VisualizationSettings.bShowWorldObjects, CVarControlRigPhysicsShowWorldObjectsOverride)
		|| GetControlRigPhysicsShow(
			VisualizationSettings.bShowWorldOverlapBox, CVarControlRigPhysicsShowWorldOverlapBoxOverride)
		|| GetControlRigPhysicsShow(
			VisualizationSettings.bShowActiveContacts, CVarControlRigPhysicsShowActiveContactsOverride)
		|| GetControlRigPhysicsShow(
			VisualizationSettings.bShowInactiveContacts, CVarControlRigPhysicsShowInactiveContactsOverride);
}
	...
	const Chaos::FPerShapeData*              Shape)
{
	...
	{
		// Convex shapes arrive wrapped in TImplicitObjectInstanced<FConvex> (Instanced or Scaled) by
		// ChaosInterface::CreateGeometry, so go through the wrapper to reach the hull.
		const FConvex* Convex = nullptr;
		if (IsInstanced(PackedType))
		{
			if (const auto* Wrapped = ImplicitObject->GetObject<TImplicitObjectInstanced<FConvex>>())
			{
				Convex = Wrapped->GetInstancedObject();
			}
		}
		else if (IsScaled(PackedType))
		{
			if (const auto* Wrapped = ImplicitObject->GetObject<TImplicitObjectScaled<FConvex>>())
			{
				Convex = Wrapped->GetUnscaledObject();
			}
		}
		else
		{
			Convex = ImplicitObject->GetObject<FConvex>();
		}
		...
		if (Convex)
		{
			...
			Endpoints.Reserve(NumEdges * 2);
			for (int32 EdgeIndex = 0; EdgeIndex < NumEdges; ++EdgeIndex)
			{
				Endpoints.Add(FVector(Convex->GetVertex(Convex->GetEdgeVertex(EdgeIndex, 0))));
				Endpoints.Add(FVector(Convex->GetVertex(Convex->GetEdgeVertex(EdgeIndex, 1))));
			}
			DI.DrawLines(
				ShapeTransform, Endpoints,
				GetColor(bIsKinematic, bCollides),
				VisualizationSettings.LineThickness);
		}
		break;
	...
	const UWorld*                            DebugWorld) const
{
...
#if CHAOS_DEBUG_DRAW
	// TODO enable/disable chaos debug draw with a cvar or flag
	...
	// Resolve each Show* toggle against its matching override CVar once per Draw call.
	const bool bShowBodies = GetControlRigPhysicsShow(
		VisualizationSettings.bShowBodies, CVarControlRigPhysicsShowBodiesOverride);
	const bool bShowCentreOfMass = GetControlRigPhysicsShow(
		VisualizationSettings.bShowCentreOfMass, CVarControlRigPhysicsShowCentreOfMassOverride);
	const bool bShowJoints = GetControlRigPhysicsShow(
		VisualizationSettings.bShowJoints, CVarControlRigPhysicsShowJointsOverride);
	const bool bShowControls = GetControlRigPhysicsShow(
		VisualizationSettings.bShowControls, CVarControlRigPhysicsShowControlsOverride);
	const bool bShowWorldObjects = GetControlRigPhysicsShow(
		VisualizationSettings.bShowWorldObjects, CVarControlRigPhysicsShowWorldObjectsOverride);
	const bool bShowWorldOverlapBox = GetControlRigPhysicsShow(
		VisualizationSettings.bShowWorldOverlapBox, CVarControlRigPhysicsShowWorldOverlapBoxOverride);
	const bool bShowActiveContacts = GetControlRigPhysicsShow(
		VisualizationSettings.bShowActiveContacts, CVarControlRigPhysicsShowActiveContactsOverride);
	const bool bShowInactiveContacts = GetControlRigPhysicsShow(
		VisualizationSettings.bShowInactiveContacts, CVarControlRigPhysicsShowInactiveContactsOverride);
	...
	if (bShowBodies)
	{
		...
		{
			DrawActor(*DI, VisualizationSettings, bShowCentreOfMass, SpaceTransform, *CollisionActorHandle);
		}
		...
		for (const TPair<FRigComponentKey, FRigBodyRecord>& BodyRecordPair : BodyRecords)
		{
			...
			{
				DrawActor(*DI, VisualizationSettings, bShowCentreOfMass, SpaceTransform, *ActorHandle);
			}
		}
	}
	...
	if (bShowWorldObjects)
	{
		for (TPair<FWorldObjectKey, FWorldObject>& WorldObjectPair : *WorldObjects.Get())
		{
			...
			{
				DrawActor(*DI, VisualizationSettings, bShowCentreOfMass, SpaceTransform, *WorldObject.ActorHandle);
			}
		}
	}
