...
void UPlaneCutTool::GenerateAsset(const TArray<FDynamicMeshOpResult>& Results)
{
	...
	int32 NumSourceMeshes = MeshesToCut.Num();
	TArray<TArray<TUniquePtr<FDynamicMesh3>>> AllSplitMeshes; AllSplitMeshes.SetNum(NumSourceMeshes);
	...
	for (int OrigMeshIdx = 0; OrigMeshIdx < NumSourceMeshes; OrigMeshIdx++)
	{
		...
		if (BasicProperties->bExportSeparatedPiecesAsNewMeshAssets)
		{
			...
			TArray<TUniquePtr<FDynamicMesh3>>& SplitMeshes = AllSplitMeshes[OrigMeshIdx];
			bool bWasSplit = FDynamicMeshEditor::SplitMesh(UseMesh, SplitMeshes, false, [SubMeshIDs](int TID)
			{
	...
	if (bNeedToAdd)
	{
		for (int OrigMeshIdx = 0; OrigMeshIdx < NumSourceMeshes; OrigMeshIdx++)
		{
			...
			for (int AddMeshIdx = 1; AddMeshIdx < SplitMeshes.Num(); AddMeshIdx++)
			{
				...
				NewMeshObjectParams.SetMesh(MoveTemp(*SplitMeshes[AddMeshIdx]));
