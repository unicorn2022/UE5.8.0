...
void SMeshLayersList::DeleteMeshLayer(TSharedPtr<FMeshLayerElement> LayerToDelete)
{
	...
	// Note: The controller will indirectly trigger a RefreshAndRestore() call via RemoveMeshLayer
	// so we don't need to manually update the list post-removal.
}
...
void SMeshLayerItem::Construct(const FArguments& InArgs, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	// Note the code for this checkbox is intentionally left here but disabled, as the underlying feature doesn't exist yet
	// but this has the desired UI look for when it does
#if 0
	BoxArgs
	+ SHorizontalBox::Slot()
	.AutoWidth()
