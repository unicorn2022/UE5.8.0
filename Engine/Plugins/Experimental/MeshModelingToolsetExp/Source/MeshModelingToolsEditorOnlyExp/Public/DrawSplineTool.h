...
public:
	...
	/** Whether to fit the spline to the selected surface. The spline will be subdivided and projected to reduce the distance between the spline and the surface */
	UPROPERTY(EditAnywhere, Category = RaycastTargets, meta = (EditCondition = bMeshSelected, HideEditConditionToggle))
	bool bFitToSurface = false;

	/** The maximum number of times the spline will be subdivided in order to fit the surface */
	UPROPERTY(EditAnywhere, Category = RaycastTargets, meta = (EditCondition = "bFitToSurface && bMeshSelected", ClampMin = 0, UIMax = 100))
	int32 MaxNewPoints = 100;

	/** The threshold for spline subdivision. New points are iteratively added where the distance from the spline to the surface is greater than this value */
	UPROPERTY(EditAnywhere, Category = RaycastTargets, meta = (EditCondition = "bFitToSurface && bMeshSelected", ClampMin = 0, UIMax = 100))
	...
	UPROPERTY()
	bool bMeshSelected = false;
};

UCLASS(MinimalAPI)
class UDrawSplineTool : public USingleSelectionMeshEditingTool
...
private:
	...
	TUniquePtr<UE::Geometry::FDynamicMeshAABBTree3> ProjectionTarget;
	TUniquePtr<UE::Geometry::FDynamicMesh3> ProjectionTargetMesh;
	FTransform MeshTargetTransform;
	TWeakObjectPtr<USplineComponent> ProjectionSpline = nullptr;
...
class UDrawSplineToolBuilder : public USingleSelectionMeshEditingToolBuilder
{
	GENERATED_BODY()
...
public:
	...
	UE_API virtual void InitializeNewTool(USingleSelectionMeshEditingTool* Tool, const FToolBuilderState& SceneState) const override;
};
