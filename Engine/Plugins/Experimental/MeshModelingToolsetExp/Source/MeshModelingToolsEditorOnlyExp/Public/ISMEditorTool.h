...
public:
	...
	/**
	 * Take the last selected element and contract the selection to exclude any instances of that static mesh and materials.
	 */
	UFUNCTION(CallInEditor, Category = SelectionEdits, meta = (DisplayPriority = 1))
	void ContractLast() { PostAction(EISMEditorToolActions::ContractLast); }

	/**
	 * Take the last selected element and expand the selection to include any other instances of the same static mesh and materials.
	 */
	UFUNCTION(CallInEditor, Category = SelectionEdits, meta = (DisplayPriority = 2))
	void ExpandLast() { PostAction(EISMEditorToolActions::ExpandLast); }

	/**
	 * Expand the selection to include any other instances of the same static mesh(es) and materials as selected.
	 */
	UFUNCTION(CallInEditor, Category = SelectionEdits, meta = (DisplayPriority = 3))
	void ExpandAll() { PostAction(EISMEditorToolActions::ExpandAll); }
...
class UISMEditorToolExtractPropertySet : public UISMEditorToolActionPropertySetBase
{
	GENERATED_BODY()
public:
	/** Name to use when duplicating or extracting to a new actor. */
	UPROPERTY(EditAnywhere, Category = Extract)
	FString ActorName;

	/** When true, the components created when extracting to a new actor are HISM's instead of ISM's. */
	UPROPERTY(EditAnywhere, Category = Extract)
	bool bCreateHISMs = false;

	/** Duplicate the selected instances into a new actor. */
	UFUNCTION(CallInEditor, Category = Extract)
	void DuplicateToNew() { PostAction(EISMEditorToolActions::DuplicateToNew); }

	/** Duplicate the selected instances into a new actor and delete them from the current actor(s). */
	UFUNCTION(CallInEditor, Category = Extract)
	void ExtractToNew() { PostAction(EISMEditorToolActions::ExtractToNew); }
};
...
private:
	UPROPERTY()
	TObjectPtr<UISMEditorToolExtractPropertySet> ExtractActions;

	UPROPERTY()
	TObjectPtr<ULocalDoubleClickInputBehavior> DoubleClickBehavior;
protected:
...
private:
	void OnContractLastSelection();
	void OnExpandLastSelection();
	void OnExpandAllSelection();
	void OnDuplicateToNew();
	void OnExtractToNew();

	void ExpandSelectionBy(const FSelectedInstance& SelectedInstance, TArray<FSelectedInstance>& SelectionToModify);
	void ContractSelectionBy(const FSelectedInstance& SelectedInstance, TArray<FSelectedInstance>& SelectionToModify);
	void ToggleSelectionBy(const FSelectedInstance& SelectedInstance, TArray<FSelectedInstance>& SelectionToModify);
	void UpdateSelectionInternal(const TArray<FSelectedInstance>& NewSelection, bool bEmitChange, const FText& TransactionLabel);

	void DuplicateToNew(bool bDeleteFromCurrent);

	void OnDoubleClicked(const FInputDeviceRay& ClickPos);

	// Helpers that use the modifiers to determine what to do with selection
	bool ShouldAddToSelection() { return bShiftModifier && !bCtrlModifier; }
	bool ShouldToggleSelection() { return bShiftModifier && bCtrlModifier; }
	bool ShouldSubtractSelection() { return !bShiftModifier && bCtrlModifier; }
	bool ModifyingSelection() { return bShiftModifier || bCtrlModifier; }
protected:
