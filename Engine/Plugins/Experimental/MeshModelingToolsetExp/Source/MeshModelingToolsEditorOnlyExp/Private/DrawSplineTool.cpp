...
void UDrawSplineTool::Setup()
{
	...
	Settings->WatchProperty(Settings->bFitToSurface, [this](bool) 
	{
		RefitToSurface();
	});

	Settings->WatchProperty(Settings->MaxNewPoints, [this](int32)
	{
		RefitToSurface();
	});

	Settings->WatchProperty(Settings->DistanceThreshold, [this](double) 
	{
		RefitToSurface();
	});
	...
	if (Target)
	{
		if (IPersistentDynamicMeshSource* const DynamicMeshSource = Cast<IPersistentDynamicMeshSource>(Target))
		{
			UDynamicMesh* const MeshContainer = DynamicMeshSource->GetDynamicMeshContainer();
			ProjectionTarget = MakeUnique<UE::Geometry::FDynamicMeshAABBTree3>(MeshContainer->GetMeshPtr(), true);
		}
		else if (IDynamicMeshProvider* const DynamicMeshProvider = Cast<IDynamicMeshProvider>(Target))
		{
			ProjectionTargetMesh = MakeUnique<UE::Geometry::FDynamicMesh3>();
			ProjectionTargetMesh->Copy(DynamicMeshProvider->GetDynamicMesh());
			ProjectionTarget = MakeUnique<UE::Geometry::FDynamicMeshAABBTree3>(ProjectionTargetMesh.Get(), true);
		}

		if (ISceneComponentBackedTarget* const PrimitiveToolTarget = Cast<ISceneComponentBackedTarget>(Target))
		{
			MeshTargetTransform = PrimitiveToolTarget->GetWorldTransform();
		}
		else
		{
			MeshTargetTransform = FTransform();
		}		

		Settings->bMeshSelected = true;
	}
	else
	{
		Settings->bMeshSelected = false;
	}
}
...
void UDrawSplineTool::Shutdown(EToolShutdownType ShutdownType)
{
	...
	{
		if (Settings->bFitToSurface && ProjectionSpline.IsValid())
		{
			WorkingSpline->SetSpline(ProjectionSpline->GetSpline());
		}

		if (WorkingSpline.IsValid() && WorkingSpline->GetNumberOfSplinePoints() > 0)
		{
			GenerateAsset();
		}
	}
	...
	if (ProjectionSpline.IsValid())
	{
		ProjectionSpline->DestroyComponent();
	}
...
void UDrawSplineTool::RefitToSurface()
{
	if (Settings->bFitToSurface && ProjectionTarget)
	{
		const FInterpCurveVector& Curve = WorkingSpline->GetSplinePointsPosition();
		FInterpCurveVector NewCurve;
		SplineUtil::ProjectSplineToSurface(NewCurve, Curve, *ProjectionTarget, WorkingSpline->GetComponentTransform(), MeshTargetTransform, Settings->DistanceThreshold, Settings->MaxNewPoints);

		if (!ProjectionSpline.IsValid())
		{
			ProjectionSpline = DrawSplineToolLocals::CreateNewSplineInActor(PreviewActor);
		}

		TArray<FVector> ProjectedPoints;
		for (FInterpCurvePointVector NewPoint : NewCurve.Points)
		{
			ProjectedPoints.Add(NewPoint.OutVal);
		}

		ProjectionSpline->SetSplinePoints(ProjectedPoints, ESplineCoordinateSpace::World, /*bUpdateSpline = */ true);
	}
	else if (ProjectionSpline.IsValid())
	{
		ProjectionSpline->DestroyComponent();
		ProjectionSpline.Reset();
	}
}
...
void UDrawSplineTool::Render(IToolsContextRenderAPI* RenderAPI)
{
	...
	if (!Settings->bFitToSurface && WorkingSpline.IsValid())
	{
	...
	if (Settings->bFitToSurface && ProjectionSpline.IsValid())
	{
		SplineUtil::FDrawSplineSettings DrawSettings;
		DrawSettings.ScaleVisualizationWidth = Settings->FrameVisualizationWidth;
		DrawSettings.RegularColor = FColor::Cyan;
		SplineUtil::DrawSpline(*ProjectionSpline, *RenderAPI, DrawSettings);
	}
...
USingleSelectionMeshEditingTool* UDrawSplineToolBuilder::CreateNewTool(const FToolBuilderState& SceneState) const
{
	return NewObject<UDrawSplineTool>(SceneState.ToolManager);
}

void UDrawSplineToolBuilder::InitializeNewTool(USingleSelectionMeshEditingTool* Tool, const FToolBuilderState& SceneState) const
{
	// Same as USingleSelectionMeshEditingToolBuilder::InitializeNewTool but allow for null Target
	...
	if (UDrawSplineTool* const SplineTool = Cast<UDrawSplineTool>(Tool))
	{
		// May be null
		SplineTool->SetSelectedActor(ToolBuilderUtil::FindFirstActor(SceneState, [](AActor*) { return true; }));
	}
}
