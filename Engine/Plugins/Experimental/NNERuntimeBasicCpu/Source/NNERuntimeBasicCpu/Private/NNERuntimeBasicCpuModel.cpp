...
namespace UE::NNE::RuntimeBasic
{
	namespace Private
	{
			...
			const uint32 InputStride)
		{
			...
			for (uint32 BatchIdx = 0; BatchIdx < BatchSize; BatchIdx++)
			{
				for (uint32 Repeat = 0; Repeat < Repeats; Repeat++)
				{
					...
					{
						Output[BatchIdx * OutputStride + Repeat * InputSize + Idx] = Input[BatchIdx * InputStride + Idx];
					}
				}
			}
#endif
			...
			const uint32 InputStride)
		{
			...
			for (uint32 BatchIdx = 0; BatchIdx < BatchSize; BatchIdx++)
			{
				...
				for (uint32 RowIdx = 0; RowIdx < InputSize; RowIdx++)
				{
					...
					if (Value != 0.0)
					{
						const float Offset = Value * WeightOffsets[RowIdx];
						const float Scales = Value * WeightScales[RowIdx];

						for (uint32 ColIdx = 0; ColIdx < OutputSize; ColIdx++)
						{
							Output[BatchIdx * OutputStride + ColIdx] += Scales * Weights[RowIdx * OutputSize + ColIdx] + Offset;
						}
					}
				}
			}
		...
		static inline void OperatorSoftmax(
			float* RESTRICT Output,
			const float* RESTRICT Input,
			const uint32 BatchSize,
			const uint32 InputOutputSize,
			...
			const uint32 InputStride)
		{
			NNE_RUNTIME_BASIC_TRACE_SCOPE(NNE::RuntimeBasic::Private::OperatorSoftmax);

			NNE_RUNTIME_BASIC_CHECK_ALIASING_SIZE(Output, Input, InputOutputSize);

#if NNE_RUNTIME_BASIC_ENABLE_ISPC
			ispc::NNERuntimeBasicCPUOperatorSoftmax(
				Output,
				Input,
				BatchSize,
				InputOutputSize,
			...
			for (uint32 BatchIdx = 0; BatchIdx < BatchSize; BatchIdx++)
			{
				...
				for (uint32 Idx = 0; Idx < InputOutputSize; Idx++)
				{
					Max = FMath::Max(Max, Input[BatchIdx * InputStride + Idx]);
				}

				float Denom = 0.0f;
				for (uint32 Idx = 0; Idx < InputOutputSize; Idx++)
				{
					Denom += FMath::Exp(Input[BatchIdx * InputStride + Idx] - Max);
				}
				Denom = FMath::Max(Denom, 1e-8f);

				for (uint32 Idx = 0; Idx < InputOutputSize; Idx++)
				{
					Output[BatchIdx * OutputStride + Idx] = FMath::Exp(Input[BatchIdx * InputStride + Idx] - Max) / Denom;
				}
			}
#endif
		}
		...
		void FAggregateOrInclusiveLayerInstance::SetMaxBatchSize(const uint32 MaxBatchSize)
		{
			...
			TotalElementNum = 0;
			ElementAccum.SetNumUninitialized(MaxBatchSize, EAllowShrinking::No);
			ElementNums.SetNumUninitialized(MaxBatchSize, EAllowShrinking::No);
			ElementOffsets.SetNumUninitialized(MaxBatchSize, EAllowShrinking::No);
		...
		struct FSoftmaxLayer : public ILayer
		{
			virtual ELayerType GetLayerType() const override final { return ELayerType::Softmax; }
			virtual uint32 GetInputSize() const override final { return InputOutputSize; }
			virtual uint32 GetOutputSize() const override final { return InputOutputSize; }
			...
			virtual void Evaluate(
				...
				const uint32 InputBufferStride) override final
			{
				NNE_RUNTIME_BASIC_TRACE_SCOPE(NNE::RuntimeBasic::Private::FSoftmaxLayer::Evaluate);
				check(OutputBufferSize == GetOutputSize() && InputBufferSize == GetInputSize());
				check(OutputBufferStride >= GetOutputSize() && InputBufferStride >= GetInputSize());
				check(Instance == nullptr);
				OperatorNanCheck(InputBuffer, BatchSize, InputBufferSize, InputBufferStride);
		...
		namespace Serialization
		{
			...
			static inline void Load(uint64& InOutOffset, TSharedPtr<ILayer>& OutLayer, TConstArrayView<uint8> Data)
			{
				...
				if (OutLayer == nullptr || OutLayer->GetLayerType() != (ELayerType)LayerTypeId)
				{
					...
					case ELayerType::Softmax: OutLayer = MakeShared<FSoftmaxLayer>(); break;
	...
	bool FModelCPU::SerializationLoad(uint64& InOutOffset, TConstArrayView<uint8> Data)
	{
		...
		{
			UE_LOGF(LogNNERuntimeBasicCPU, Error, "Invalid Magic Number %i", Magic);
			return false;
		}
		...
		{
			UE_LOGF(LogNNERuntimeBasicCPU, Error, "Unsupported Version Number %i", Version);
			return false;
		}
	...
	FModelBuilderElement FModelBuilder::MakeSoftmax(const uint32 InputOutputSize)
	{
		const TSharedPtr<Private::FSoftmaxLayer> Layer = MakeShared<Private::FSoftmaxLayer>();
		Layer->InputOutputSize = InputOutputSize;
		return StaticCastSharedPtr<Private::ILayer>(Layer);
	}
		...
		const float Scale)
	{
		...
		{
			WeightOffsets[RowIdx] = Mins[RowIdx];
			WeightScales[RowIdx] = FMath::Max(Maxs[RowIdx] - Mins[RowIdx], UE_SMALL_NUMBER) / 65535.0f;
		}
		...
		for (uint32 RowIdx = 0; RowIdx < InputSize; RowIdx++)
		{
			for (uint32 ColIdx = 0; ColIdx < OutputSize; ColIdx++)
			{
				Weights[RowIdx * OutputSize + ColIdx] = (uint16)FMath::RoundToFloat(65535.0f *
					FMath::Clamp((Values[RowIdx * OutputSize + ColIdx] - Mins[RowIdx]) / FMath::Max(Maxs[RowIdx] - Mins[RowIdx], UE_SMALL_NUMBER), 0.0f, 1.0f));
			}
		}
