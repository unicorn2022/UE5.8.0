...
{
	// if NoiseOverride is set, will be used instead of default Octave Noise.
	TFunction<FVector3d(const FVector& Pos)> NoiseOverride = nullptr;
	// desired spacing between noise samples, used for both Octave Noise and Noise Override cases
	float PointSpacing = 1;

	// Octave Noise settings, not used if NoiseOverride is specified
	float Amplitude = 2;
...
struct FIslandSplitSettings
{
	...
	FIslandSplitSettings(bool bInSplitIslands = true, double InCloseVertexDistance = 1e-3, double InVertexToSurfaceBridgeDistance = 0)
		: bSplitIslands(bInSplitIslands), CloseVertexDistance(InCloseVertexDistance),
		  VertexToSurfaceBridgeDistance(InVertexToSurfaceBridgeDistance) {}

	/** Whether to detect and split disconnected islands */
 ...
 * @param IslandSplitSettings			Controls detection and splitting of disconnected components after the cut
...
int32 PLANARCUT_API CutWithPlanarCells(
	...
	FIslandSplitSettings IslandSplitSettings = FIslandSplitSettings()
);
