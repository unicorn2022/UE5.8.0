...
struct FCellMeshes
{
	...
	{
		NoiseOffsets.SetOffsets(RandomStream);
		OutsideCellIndex = -1;
	}
...
struct FDynamicMeshCollection
{
	 ...
	 */
	int32 CutWithCellMeshes(const FInternalSurfaceMaterials& InternalSurfaceMaterials, const TArray<TPair<int32, int32>>& CellConnectivity, FCellMeshes& CellMeshes, const FIslandSplitSettings& IslandSplitSettings, FGeometryCollection* Collection, bool bSetDefaultInternalMaterialsFromCollection, double CollisionSampleSpacing);
	 ...
	 */
	int32 SplitAllIslands(FGeometryCollection* Collection, double CollisionSampleSpacing, const FIslandSplitSettings& IslandSplitSettings = FIslandSplitSettings());
	...
	bool SplitIslands(UE::Geometry::FDynamicMesh3& Source, TArray<TUniquePtr<UE::Geometry::FDynamicMesh3>>& SeparatedMeshes, const FIslandSplitSettings& IslandSplitSettings = FIslandSplitSettings());
