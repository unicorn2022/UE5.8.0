...
namespace
{
	...
	void SetUnsetColors(FGeometryCollection* Collection, int32 FirstGeometryIndex, bool bPropagateFromNeighbors = true)
	{
		...
		constexpr float CopyColorDistance = 1e-03f;
...
{
	return FVector3d(Settings.NoiseVector(FVector(Pos), NoiseOffsets));
}
...
double FCellMeshes::GetSafeNoiseSpacing(float SurfaceArea, float TargetSpacing)
{
	...
	{
		UE_LOGF(LogPlanarCut, Warning,
			"Requested spacing of noise points (surface resolution) of %f would require too many added vertices; Using %f instead.",
...
int32 FDynamicMeshCollection::CutWithMultiplePlanes(
	...
	const FIslandSplitSettings& IslandSplitSettings,
	int32 RandomSeed,
...
)
{
	...
	if (bHasGrout)
	{
		...
		FProgressCancel::FProgressScope GroutUnionScope(Progress, .3f);
		...
		return CutWithCellMeshes(InternalSurfaceMaterials, CellConnectivity, GroutCells, IslandSplitSettings, Collection, bSetDefaultInternalMaterialsFromCollection, CollisionSampleSpacing);
	}
	...
	for (int32 PlaneIdx = 0; PlaneIdx < Planes.Num(); PlaneIdx++)
	{
		...
		for (int32 ToCutIdx = 0, ToCutNum = ToCut.Num(); ToCutIdx < ToCutNum; ToCutIdx++)
		{
			...
			if (bKeepResults)
			{
				...
				{
					if (IslandSplitSettings.bSplitIslands && SplitIslands(ToCut[ResultIndices[UnsplitIdx]]->AugMesh, SplitMeshes, IslandSplitSettings))
					{
						// TODO: Add a SetMesh(FDynamicMesh3&&) method so we can MoveTemp here
						...
						for (int32 Idx = 1; Idx < SplitMeshes.Num(); Idx++)
						{
							...
							// TODO: Add a FMeshData constructor to let us MoveTemp the mesh here
...
int32 FDynamicMeshCollection::SplitAllIslands(FGeometryCollection* Collection, double CollisionSampleSpacing, const FIslandSplitSettings& IslandSplitSettings)
{
	int32 FirstIdx = -1;
	TArray<int32> GeometryForRemoval;
	...
	for (FMeshData& Surface : Meshes)
	{
		...
		TArray<TUniquePtr<FDynamicMesh3>> Islands;
		if (SplitIslands(Surface.AugMesh, Islands, IslandSplitSettings))
		{
...
int32 FDynamicMeshCollection::CutWithCellMeshes(const FInternalSurfaceMaterials& InternalSurfaceMaterials, const TArray<TPair<int32, int32>>& CellConnectivity, FCellMeshes& CellMeshes, const FIslandSplitSettings& IslandSplitSettings, FGeometryCollection* Collection, bool bSetDefaultInternalMaterialsFromCollection, double CollisionSampleSpacing)
{
		...
		if (NonEmptyResults > 1) // only write to geometry collection if more than one result was non-empty
		{
			...
			for (int32 CellIdx = 0; CellIdx < CellMeshes.CellMeshes.Num(); CellIdx++)
			{
				if (BooleanResults[CellIdx].IsValid() && BooleanResults[CellIdx]->TriangleCount() > 0)
				{
					...
					int32 CreatedGeometryIdx = -1;
					TArray<TUniquePtr<FDynamicMesh3>> Islands;
					if (IslandSplitSettings.bSplitIslands && SplitIslands(AugBoolResult, Islands, IslandSplitSettings))
					{
						for (int32 i = 0; i < Islands.Num(); i++)
						{
							FDynamicMesh3& Island = *Islands[i];
...
bool FDynamicMeshCollection::SplitIslands(FDynamicMesh3& Source, TArray<TUniquePtr<FDynamicMesh3>>& SeparatedMeshes, const FIslandSplitSettings& Settings)
{
	SeparatedMeshes.Reset();

	FVertexConnectedComponents VertComponents;
	VertComponents.Init(Source);
	VertComponents.ConnectTriangles(Source);

	if (Settings.CloseVertexDistance > 0.)
	{
		VertComponents.ConnectCloseVertices(Source, Settings.CloseVertexDistance);
	}

	if (Settings.VertexToSurfaceBridgeDistance > 0.)
	{
		TMeshAABBTree3<FDynamicMesh3> Spatial(&Source);
		VertComponents.ConnectVerticesToNearestDifferentComponent(Spatial, Settings.VertexToSurfaceBridgeDistance);
	}

	bool bWasSplit = FDynamicMeshEditor::SplitMesh(&Source, SeparatedMeshes, false, [&Source, &VertComponents](int TID)
		{
			return VertComponents.GetComponent(Source.GetTriangle(TID).A);
		});

	if (bWasSplit)
	{
		// disconnected components that are contained inside other components need to be re-merged
		TMeshSpatialSort<FDynamicMesh3> SpatialSort;
		...
		SpatialSort.Compute(SeparatedMeshes);
		...
		{
			FDynamicMeshEditor Editor(SeparatedMeshes[Nest.OuterIndex].Get());
			FMeshIndexMappings Mappings;
			for (int Inner : Nest.InnerIndices)
			{
				Editor.AppendMesh(SeparatedMeshes[Inner].Get(), Mappings);
