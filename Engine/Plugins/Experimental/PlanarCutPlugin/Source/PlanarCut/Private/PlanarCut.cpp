...
)
{
	...
	return CutMultipleWithPlanarCells(Cells, Source, TransformIndices, Grout, CollisionSampleSpacing, RandomSeed, TransformCollection, bIncludeOutsideCellInOutput, bSetDefaultInternalMaterialsFromCollection, Progress, CellsOrigin, IslandSplitSettings);
}
...
{
	FProgressCancel::FProgressScope PrepareScope = FProgressCancel::CreateScopeTo(Progress, .1f, LOCTEXT("CutWithMultiplePlanesInit", "Preparing to cut with planes"));
	...
	FProgressCancel::FProgressScope CutScope = FProgressCancel::CreateScopeTo(Progress, .99f, LOCTEXT("CutWithMultiplePlanesBody", "Cutting with planes"));

	int32 NewGeomStartIdx = -1;
	NewGeomStartIdx = MeshCollection.CutWithMultiplePlanes(CenteredPlanes, Grout, CollisionSampleSpacing, IslandSplitSettings, RandomSeed, &Collection, InternalSurfaceMaterials, bSetDefaultInternalMaterialsFromCollection, Progress);
...
)
{
	...
	FProgressCancel::FProgressScope SurfaceScope = FProgressCancel::CreateScopeTo(Progress, .99f);
...
{
	FProgressCancel::FProgressScope CreateMeshCollectionScope = FProgressCancel::CreateScopeTo(Progress, .1f);
	if (bSetDefaultInternalMaterialsFromCollection)
	{
	...
	FProgressCancel::FProgressScope CellMeshScope = FProgressCancel::CreateScopeTo(Progress, .1f);
	...
	FProgressCancel::FProgressScope CutScope = FProgressCancel::CreateScopeTo(Progress, .99f);
	int32 NewGeomStartIdx = -1;
	NewGeomStartIdx = MeshCollection.CutWithCellMeshes(Cells.InternalSurfaceMaterials, Cells.PlaneCells, CellMeshes, IslandSplitSettings, &Source, bSetDefaultInternalMaterialsFromCollection, CollisionSampleSpacing);
	CutScope.Done();
...
)
{
	...
	FProgressCancel::FProgressScope SplitScope = FProgressCancel::CreateScopeTo(Progress, .99f);
	int32 NewGeomStartIdx = -1;
	NewGeomStartIdx = MeshCollection.SplitAllIslands(&Collection, CollisionSampleSpacing, IslandSplitSettings);
	SplitScope.Done();
...
{
	FProgressCancel::FProgressScope PrepareScope = FProgressCancel::CreateScopeTo(Progress, .1f);
	...
	NewGeomStartIdx = MeshCollection.CutWithCellMeshes(InternalSurfaceMaterials, CellConnectivity, CellMeshes, IslandSplitSettings, &Collection, bSetDefaultInternalMaterialsFromCollection, CollisionSampleSpacing);
...
)
{
	...
	for (int32 Idx = 0; Idx < TransformIndicesArray.Num(); ++Idx)
	{
		...
		{
			double UnscaledVolume = (double)VolumesAttrib[TransformIdx];
...
// mesh adapter for a single geometry in a geometry collection
// Note: implicitly remaps vertices and faces to start at 0, to avoid wasting space in the Mesh AABB tree use case
struct FGeometryCollectionGeometryMeshAdapter
{
	TConstArrayView<FIntVector> FaceView;
	TConstArrayView<FVector3f> VertexView;
	int32 FaceStart, FaceNum, VertexStart, VertexNum;

	FGeometryCollectionGeometryMeshAdapter(const FGeometryCollection& Collection, int32 GeometryIdx)
	{
		FaceStart = Collection.FaceStart[GeometryIdx];
		FaceNum = Collection.FaceCount[GeometryIdx];
		VertexStart = Collection.VertexStart[GeometryIdx];
		VertexNum = Collection.VertexCount[GeometryIdx];
		FaceView = Collection.Indices.GetConstArray();
		VertexView = Collection.Vertex.GetConstArray();
	}

	bool IsTriangle(int32 TID) const
	{
		return TID >= 0 && TID < FaceNum;
	}
	bool IsVertex(int32 VID) const
	{
		return VID >= 0 && VID < VertexNum;
	}
	...
	// note: GetChangeStamp is required by mesh aabb tree interface but not needed/useful otherwise
	...
	{
		FIntVector Tri = FaceView[TID + FaceStart];
		return FIndex3i(Tri.X - VertexStart, Tri.Y - VertexStart, Tri.Z - VertexStart);
	}
	FVector3f GetVertex(int32 VID) const
	{
		return VertexView[VID + VertexStart];
	}

	template<typename VertexType>
	inline void GetTriVertices(int TID, VertexType& V0, VertexType& V1, VertexType& V2) const
	{
		FIntVector Tri = FaceView[TID + FaceStart];
		V0 = (VertexType)VertexView[Tri.X];
		V1 = (VertexType)VertexView[Tri.Y];
		V2 = (VertexType)VertexView[Tri.Z];
	}
};
...
static TMap<UE::Geometry::FIndex2i, float> ComputeGeometryVolumeContactWeightsFromProximity(
	const FGeometryCollection& Collection,
	const TArray<TSet<int32>>& GeomProximity,
	const TSet<int32>& GeomIdxSelection,
	const TMap<int32, double>* OptionalGeomToVol = nullptr,
	float DistanceThreshold = .01, float SamplesPerUnitArea = 10, int32 MaxSamplesPerTri = 100)
{
	TMap<UE::Geometry::FIndex2i, float> ContactWeights;
	TMultiMap<int32, int32> NeedConnections;
	for (int32 GeomIdx : GeomIdxSelection)
	{
		double GeomVol = OptionalGeomToVol ? (*OptionalGeomToVol)[GeomIdx] : 0.;
		for (int32 GeomNbrIdx : GeomProximity[GeomIdx])
		{
			// filter out contacts that we'll see twice
			if (GeomNbrIdx < GeomIdx && GeomIdxSelection.Contains(GeomNbrIdx))
			{
				continue;
			}

			// process in a consistent order. if we have volumes calculated, favor small->large,
			// since that may be a bit faster (typically fewer trianges/less surface area to sample)
			double NbrVol = OptionalGeomToVol ? (*OptionalGeomToVol)[GeomNbrIdx] : 0.;
			int32 GeomB = GeomIdx, GeomA = GeomNbrIdx;
			if (NbrVol < GeomVol || (NbrVol == GeomVol && GeomB < GeomA))
			{
				Swap(GeomA, GeomB);
			}
			NeedConnections.Add(GeomA, GeomB);
		}
	}
	TArray<int32> Keys;
	NeedConnections.GetKeys(Keys);
	for (int32 KeyGeom : Keys)
	{
		FGeometryCollectionGeometryMeshAdapter RefMesh(Collection, KeyGeom);
		TMeshAABBTree3<FGeometryCollectionGeometryMeshAdapter> RefSpatial(&RefMesh, true);
		FTransform3f RefTransform = (FTransform3f)GeometryCollectionAlgo::GlobalMatrix(Collection.Transform, Collection.Parent, Collection.TransformIndex[KeyGeom]);
		for (auto KeyItr = NeedConnections.CreateConstKeyIterator(KeyGeom); KeyItr; ++KeyItr)
		{
			int32 NbrGeom = KeyItr.Value();
			FTransform3f NbrTransform = (FTransform3f)GeometryCollectionAlgo::GlobalMatrix(Collection.Transform, Collection.Parent, Collection.TransformIndex[NbrGeom]);
			FGeometryCollectionGeometryMeshAdapter NbrMesh(Collection, NbrGeom);
			float ContactArea = UE::Geometry::TMeshQueries<FGeometryCollectionGeometryMeshAdapter>::ApproximateContactArea<TMeshAABBTree3<FGeometryCollectionGeometryMeshAdapter>, float>
				(NbrMesh, NbrTransform, RefSpatial, RefTransform, DistanceThreshold, SamplesPerUnitArea, MaxSamplesPerTri);
			FIndex2i OutputKey(KeyGeom, NbrGeom);
			OutputKey.Sort();
			ContactWeights.Add(OutputKey, ContactArea);
		}
	}
...
)
{
	...
	for (int32 Idx = 0; Idx < TransformIndices.Num(); ++Idx)
	{
		...
		if (GeomIdx > -1 && Collection.IsRigid(TransformIdx))
		{
			...
			GeomToParent.Add(GeomIdx, Collection.Parent[TransformIdx]);
		}
	}
	...
	TMap<UE::Geometry::FIndex2i, float> ConnectionWeights;
	if (NeighborSelectionMethod == ENeighborSelectionMethod::LargestContactArea)
	{
		ConnectionWeights = ComputeGeometryVolumeContactWeightsFromProximity(Collection, *UseProximity, TooSmalls, &GeomToVol);
	}
	...
	for (int32 SmallIdx : TooSmalls)
	{
		...
		for (int32 NbrIdx : Prox)
		{
			if (NbrIdx != SmallIdx && CanMerge.Contains(NbrIdx))
			{
				...
				else if (NeighborSelectionMethod == UE::PlanarCut::ENeighborSelectionMethod::NearestCenter)
				{
					Score = 1.0 / (DOUBLE_SMALL_NUMBER + DistanceSquared(GetCenter(NbrIdx), GetCenter(SmallIdx)));
				}
				else // use pre-computed connection weight map
				{
					FIndex2i Key(SmallIdx, NbrIdx);
					Key.Sort();
					float* FoundWt = ConnectionWeights.Find(Key);
					Score = FoundWt ? *FoundWt : 0.f;
				}
				if (Score > BestScore)
				{
					BestScore = Score;
		...
		{
			UE_LOGF(LogPlanarCut, Warning, "Couldn't fix Bone %d: No neighbors found in proximity graph", SmallIdx);
			continue;
		}
...
namespace
{
	...
	static void GetRigidGeometrySelectionFromBoneSelection(const FGeometryCollection& Collection, int32 BoneIdx, TSet<int32>& OutGeometryIndices)
	{
		int32 GeoIdx = Collection.TransformToGeometryIndex[BoneIdx];
		if (Collection.IsRigid(BoneIdx) && GeoIdx != INDEX_NONE)
		{
			OutGeometryIndices.Add(GeoIdx);
		}
		else if (Collection.IsClustered(BoneIdx))
		{
			for (int32 Child : Collection.Children[BoneIdx])
			{
				GetRigidGeometrySelectionFromBoneSelection(Collection, Child, OutGeometryIndices);
			}
		}
	}
}
...
)
{
	...
	TSet<int32> ToProcessGeometry;
	for (int32 BoneIdx : SmallTransformIndices)
	{
		GetRigidGeometrySelectionFromBoneSelection(Collection, BoneIdx, ToProcessGeometry);
	}
	TMap<UE::Geometry::FIndex2i, float> ConnectionWeights;
	if (NeighborSelectionMethod == ENeighborSelectionMethod::LargestContactArea)
	{
		ConnectionWeights = ComputeGeometryVolumeContactWeightsFromProximity(Collection, *UseProximity, ToProcessGeometry);
	}
	...
	for (int32 ProcessIdx = 0; ProcessIdx < ToProcess.Num(); ++ProcessIdx)
	{
		...
		else
		{
			...
			else if (NeighborSelectionMethod == ENeighborSelectionMethod::LargestNeighbor)
			{
				double Volume = GetClusterVolumeFromRigidsHelper(Collection, Volumes, MergeIdx);
			...
			else // ENeighborSelectionMethod::LargestContactArea
			{
				TSet<int32> MergeGeo, NbrGeo;
				GetRigidGeometrySelectionFromBoneSelection(Collection, MergeIdx, MergeGeo);
				double LargestContact = -1;
				for (int32 NeighborIdx : Neighbors)
				{
					double ContactArea = 0.;
					NbrGeo.Reset();
					GetRigidGeometrySelectionFromBoneSelection(Collection, NeighborIdx, NbrGeo);
					// Look for proximity-neighbors of the merge bone's geometry
					// that belong to the current neighbor bone's geometry,
					// and add the corresponding contact area
					for (int32 MergeGeoIdx : MergeGeo)
					{
						for (int32 MergeGeoNbrIdx : (*UseProximity)[MergeGeoIdx])
						{
							if (NbrGeo.Contains(MergeGeoNbrIdx))
							{
								FIndex2i Key(MergeGeoIdx, MergeGeoNbrIdx);
								Key.Sort();
								float* FoundArea = ConnectionWeights.Find(Key);
								if (FoundArea)
								{
									ContactArea += double(*FoundArea);
								}
							}
						}
					}
					if (BestNeighbor == INDEX_NONE || ContactArea > LargestContact)
					{
						BestNeighbor = NeighborIdx;
						LargestContact = ContactArea;
					}
				}
			}
		}
