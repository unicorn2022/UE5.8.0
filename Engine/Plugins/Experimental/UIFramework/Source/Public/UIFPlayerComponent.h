 ...
 */
USTRUCT()
struct FUIFrameworkWidgetToFocus
{
	GENERATED_BODY()

	//~ ID of the widget to focus on.
	UPROPERTY()
	FUIFrameworkWidgetId Id;

	//~ Tracker to ensure widget ID is replicated even if it hasn't changed.
	UPROPERTY()
	int32 ReplicationCounter = 0;
};
...
private:
	...
	UE_API bool TrySetFocus(float DeltaTime, FUIFrameworkWidgetId CurrentWidgetToFocusId);
...
private:
	...
	FUIFrameworkWidgetToFocus WidgetToFocus;

	bool bIsPendingFocusCall = false;
