...
{
	UE_LOGF(LogUIFramework, VeryVerbose, "Constructing UIFrameworkPlayerComponent");
...
void UUIFrameworkPlayerComponent::InitializeComponent()
{
	...
	UE_LOGF(LogUIFramework, VeryVerbose, "Initializing UIFrameworkPlayerComponent");
...
{
	UE_LOGF(LogUIFramework, VeryVerbose, "Uninitializing UIFrameworkPlayerComponent");
	...
	for (const auto& ClassesToLoadPair : ClassesToLoad)
	{
		if (ClassesToLoadPair.Value.StreamableHandle.IsValid())
		{
			ClassesToLoadPair.Value.StreamableHandle->CancelHandle();
		}
	}
	ClassesToLoad.Empty();
...
void UUIFrameworkPlayerComponent::AddWidget(FUIFrameworkGameLayerSlot InEntry)
{
	...
	{
		UE_LOG(LogUIFramework, VeryVerbose, TEXT("Attempting to add widget '%" INT64_FMT "' on the server."), InEntry.AuthorityGetWidget()->GetWidgetId().GetKey());
...
void UUIFrameworkPlayerComponent::RemoveWidget(UUIFrameworkWidget* Widget)
{
	...
	else
	{
		...
		{
			UE_LOG(LogUIFramework, VeryVerbose, TEXT("Attempting to remove widget '%" INT64_FMT "' on the server."), Widget->GetWidgetId().GetKey());
...
void UUIFrameworkPlayerComponent::HandleAddPending()
{
	...
	for (int32 ReplicationId : AddPending)
	{
		...
		{
			UE_LOG(LogUIFramework, VeryVerbose, TEXT("Attempting to add pending widget '%" INT64_FMT "'."), Entry->ChildId.GetKey());
		...
		{
			UE_LOGF(LogUIFramework, Verbose, "A widget would have been added but couldn't be found anymore.");
		}
	}
	...
	if (bIsPendingFocusCall && WidgetToFocus.Id.IsValid())
	{
		if (UUIFrameworkWidget* Widget = GetWidgetTree().FindWidgetById(WidgetToFocus.Id))
		{
			if (UWidget* UMGWidget = Widget->LocalGetUMGWidget())
			{
				if (!PendingWidgetConstructedTickerHandle.IsValid())
				{
					PendingWidgetConstructedTickerHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateUObject(this, &UUIFrameworkPlayerComponent::TrySetFocus, WidgetToFocus.Id));
					bIsPendingFocusCall = false;
				}
			}
		}
	}
...
void UUIFrameworkPlayerComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	{
		UE_LOGF(LogUIFramework, VeryVerbose, "Pending widgets found, attempting to add.");
		HandleAddPending();
	}
...
void UUIFrameworkPlayerComponent::LocalWidgetWasAddedToTree(const FUIFrameworkWidgetTreeEntry& Entry)
{
	...
	if (Entry.Child && Entry.Child->LocalIsReplicationReady())
	{
		...
		{
			UE_LOG(LogUIFramework, VeryVerbose, TEXT("Widget %s with ID  '%" INT64_FMT "' is ready to be created on the client."), *WidgetClass->GetName(), Entry.ChildId.GetKey());
		...
		{
			UE_LOG(LogUIFramework, VeryVerbose, TEXT("The widget '%" INT64_FMT "' needs to load its WidgetClass soft pointer."), Entry.ChildId.GetKey());
	...
	{
		UE_LOG(LogUIFramework, VeryVerbose, TEXT("Widget '%" INT64_FMT "' is not ready for replication yet."), Entry.ChildId.GetKey());
...
void UUIFrameworkPlayerComponent::LocalWidgetRemovedFromTree(const FUIFrameworkWidgetTreeEntry& Entry)
{
	...
	UE_LOG(LogUIFramework, VeryVerbose, TEXT("Removed Widget '%" INT64_FMT "' from the widget tree locally."), Entry.ChildId.GetKey());
...
{
	WidgetToFocus.Id = WidgetId;
	WidgetToFocus.ReplicationCounter++;
	...
	if (ClassesToLoad.RemoveAndCopyValue(WidgetClass, FoundWidgetClassToLoad))
	{
		if (WidgetClass.Get())
		{
			for (int32 ReplicationId : FoundWidgetClassToLoad.EntryReplicationIds)
			{
				...
				{
					UE_LOGF(LogUIFramework, Log, "A widget with class %ls was removed.", *WidgetClass.Get()->GetName());
				}
			}
		}
		else
		{
			UE_LOGF(LogUIFramework, Error, "Failed to load widget class %ls.", *WidgetClass.ToString());
			ensure(false);
		}
	}
	else
	{
		UE_LOGF(LogUIFramework, Log, "A load request for class %ls was not found but could had be removed.", *WidgetClass.Get()->GetName());
	}
		...
		if (Widget)
		{
			...
			if (ensure(UMGWidget))
			{
				...
				if (UUIFrameworkPresenter* PresenterPtr = GetOrCreatePresenter())
				{
					...
					UE_LOGF(LogUIFramework, VeryVerbose, "Widget %ls added to the local viewport via the presenter.", *UMGWidget->GetName());
...
bool UUIFrameworkPlayerComponent::TrySetFocus(float DeltaTime, FUIFrameworkWidgetId CurrentWidgetToFocusId)
{
	if (UUIFrameworkWidget* Widget = GetWidgetTree().FindWidgetById(CurrentWidgetToFocusId))
	{
		if (UWidget* UMGWidget = Widget->LocalGetUMGWidget())
		{
			if (UMGWidget->IsConstructed())
			{
				const FUIFrameworkWidgetTreeEntry* RootEntry = GetWidgetTree().FindRootEntryById(CurrentWidgetToFocusId);
				if (RootEntry)
				{
...
{
	if (WidgetToFocus.Id.IsValid())
	{
		bIsPendingFocusCall = true;
		if (UUIFrameworkWidget* Widget = GetWidgetTree().FindWidgetById(WidgetToFocus.Id))
		{
			if (UWidget* UMGWidget = Widget->LocalGetUMGWidget())
			{
				if (UMGWidget->IsConstructed())
				{
					TrySetFocus(0, WidgetToFocus.Id);
					bIsPendingFocusCall = false;
					return;
				}
			}
