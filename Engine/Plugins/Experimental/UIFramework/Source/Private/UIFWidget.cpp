...
void UUIFrameworkWidget::LocalCreateUMGWidget(IUIFrameworkWidgetTreeOwner* InOwner, bool* bDidCreateWidgetPtr)
{
	...
	if (UClass* Class = WidgetClass.Get())
	{
		...
		if (LocalUMGWidget)
		{
			UE_LOGF(LogUIFramework, VeryVerbose, "UMG Widget object %ls created on the client.", *LocalUMGWidget->GetName());

		}
		else
		{ 
			UE_LOG(LogUIFramework, VeryVerbose, TEXT("No UMG Widget object was created for widget ID  '%" INT64_FMT "'."), Id.GetKey());
		}
