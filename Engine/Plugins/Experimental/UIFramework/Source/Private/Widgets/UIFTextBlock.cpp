...
void UUIFrameworkTextBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(ThisClass, AutoWrap, Params);
	DOREPLIFETIME_WITH_PARAMS_FAST(ThisClass, WrappingPolicy, Params);
	DOREPLIFETIME_WITH_PARAMS_FAST(ThisClass, WrapWidth, Params);
}
...
void UUIFrameworkTextBase::SetAutoWrap(bool InAutoWrap)
{
	if (AutoWrap != InAutoWrap)
	{
		AutoWrap = InAutoWrap;
		MARK_PROPERTY_DIRTY_FROM_NAME(ThisClass, AutoWrap, this);
		ForceNetUpdate();
	}
}

void UUIFrameworkTextBase::SetWrapWidth(float InWrapWidth)
{
	if (WrapWidth != InWrapWidth)
	{
		WrapWidth = InWrapWidth;
		MARK_PROPERTY_DIRTY_FROM_NAME(ThisClass, WrapWidth, this);
		ForceNetUpdate();
	}
}

void UUIFrameworkTextBase::SetWrappingPolicy(ETextWrappingPolicy InWrappingPolicy)
{
	if (WrappingPolicy != InWrappingPolicy)
	{
		WrappingPolicy = InWrappingPolicy;
		MARK_PROPERTY_DIRTY_FROM_NAME(ThisClass, WrappingPolicy, this);
		ForceNetUpdate();
	}
}
...
void UUIFrameworkTextBase::LocalOnUMGWidgetCreated()
{
	...
	SetAutoWrapToWidget(AutoWrap);
	SetWrapWidthToWidget(WrapWidth);
	SetWrappingPolicyToWidget(WrappingPolicy);
...
void UUIFrameworkTextBase::OnRep_AutoWrap()
{
	if (LocalGetUMGWidget())
	{
		SetAutoWrapToWidget(AutoWrap);
...
void UUIFrameworkTextBase::OnRep_WrapWidth()
{
	if (LocalGetUMGWidget())
	{
		SetWrapWidthToWidget(WrapWidth);
...
void UUIFrameworkTextBase::OnRep_WrappingPolicy()
{
	if (LocalGetUMGWidget())
	{
		SetWrappingPolicyToWidget(WrappingPolicy);
...
void UUIFrameworkTextBlock::SetAutoWrapToWidget(bool InValue)
{
	CastChecked<UTextBlock>(LocalGetUMGWidget())->SetAutoWrapText(InValue);
}

void UUIFrameworkTextBlock::SetWrapWidthToWidget(float InValue)
{
	CastChecked<UTextBlock>(LocalGetUMGWidget())->SetWrapTextAt(InValue);
}

void UUIFrameworkTextBlock::SetWrappingPolicyToWidget(ETextWrappingPolicy InValue)
{
	CastChecked<UTextBlock>(LocalGetUMGWidget())->SetWrappingPolicy(InValue);
}
