...
void FRazerChromaDeviceModule::StartupModule()
{
	...
	UE_LOGF(LogRazerChroma, Log, "[%s] Razer Chroma module starting...", __func__);
	...
	{
		UE_LOGF(LogRazerChroma, Log, "[%s] URazerChromaDevicesDeveloperSettings::IsRazerChromaEnabled is false, Razer Chroma will not be available.", __func__);
		return;
	}
	...
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load DLL '%ls'. Razer Chroma will not be available.", __func__, *UE::RazerChroma::GetRazerChromaDLLName());
		return;
	}
	...
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load the Razer Chroma Editor Dynamic Library! Razer Chroma will not be available.", __func__);
		return;
	}
	...
	if (bLoadedDynamicAPISuccessfully)
	{
		...
		UE_LOGF(LogRazerChroma, Log, "[%s] Razer Chroma module has successfully started!", __func__);
		...
		if (const URazerChromaAnimationAsset* NewIdleAnimation = Settings->GetIdleAnimation())
		{
			...
			UE_LOGF(LogRazerChroma, Log, "[%s] Set default Idle Animation to %ls", __func__, *NewIdleAnimation->GetAnimationName());
		}
	}
	...
	{
		UE_LOGF(LogRazerChroma, Log, "[%s] Failed to Init Razer Chroma Editor API. The Razer Synapse client is likely not installed on this machine. Error code %d (%ls)",
			__func__,
			Res,
	...
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to Init Razer Chroma Editor API. Error code %d (%ls)",
			__func__,
			Res,
			*FRazerChromaDeviceModule::RazerErrorToString(Res));
	}	
	...
	UE_LOGF(LogRazerChroma, Log, "[%s] RAZER_CHROMA_SUPPORT=0    No Razer Chroma Animation functionality will be available.", __func__);
...
TSharedPtr<IInputDevice> FRazerChromaDeviceModule::CreateInputDevice(const TSharedRef<FGenericApplicationMessageHandler>& InMessageHandler)
{
	...
	{
		UE_LOGF(LogRazerChroma, 
			Log, 
			"[%s] URazerChromaDevicesDeveloperSettings::ShouldCreateRazerInputDevice is false, we will not create the Razer Chroma input device. Device Properties will not work.", 
			__func__);
	}
...
void FRazerChromaDeviceModule::CleanupSDK()
{
	...
	UE_LOGF(LogRazerChroma,
		Log,
		"[%s] Razer Chroma Editor library cleaned up.",
		__func__);
}
...
void FRazerChromaDeviceModule::ForceReinitalize()
{
	...
	// And re-init
	const RZRESULT Res = UE::RazerChroma::InitChromaSDK();
	UE_CLOGF(Res != RZRESULT_SUCCESS, LogRazerChroma, Error, "[%s] Failed to Init Razer Chroma Editor API. Error code %d", __func__, Res);
