...
void FRazerChromaInputDevice::HandlePlayAnimationFile(const FRazerChromaPlayAnimationFile& Property)
{
	...
	{
		UE_LOGF(LogRazerChroma, Error, "[%s]Invalid animation name!", __func__);
		return;
	}

	if (!Property.AnimationByteBuffer)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] There is no animation data for chroma effect %ls", __func__, *Property.AnimName);
		return;
	}
	...
	if (LoadedAnimId == -1)
	{
		UE_LOGF(LogRazerChroma, Warning, "[%s] Failed to load animation from memory", __func__);
		return;
	}
	
	const int32 ActiveAnimId = FRazerChromaEditorDynamicAPI::PlayAnimationWithId(LoadedAnimId, Property.bLooping);
	UE_CLOGF(ActiveAnimId == -1, LogRazerChroma, Error, "[%s] Failed to play animation!", __func__);
}
