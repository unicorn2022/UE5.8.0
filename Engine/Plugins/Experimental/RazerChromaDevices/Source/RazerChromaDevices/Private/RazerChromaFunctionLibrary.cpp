...
bool URazerChromaFunctionLibrary::PlayChromaAnimation(const URazerChromaAnimationAsset* AnimToPlay, const bool bLooping)
{
	...
	if (!AnimToPlay)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Invalid AnimToPlay!", __func__);
		return false;
	}
	...
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load Chroma Animation %ls", __func__, *GetNameSafe(AnimToPlay));
		return false;
	}

	const int32 ActiveAnimId = FRazerChromaEditorDynamicAPI::PlayAnimationWithId(LoadedAnimId, bLooping);

	UE_CLOGF(ActiveAnimId == INDEX_NONE, LogRazerChroma, Error, "[%s] Failed to play animation!", __func__);
	UE_CLOGF(ActiveAnimId != INDEX_NONE, LogRazerChroma, Verbose, "[%s] Playing Razer Chroma Animation %ls (%d)", __func__, *GetNameSafe(AnimToPlay), ActiveAnimId);
...
bool URazerChromaFunctionLibrary::IsAnimationPlaying(const URazerChromaAnimationAsset* Anim)
{
	...
	if (!Anim)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Invalid AnimToStop!", __func__);
		return false;
	}
	...
	if (LoadedAnimId == INDEX_NONE)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load Chroma Animation %ls", __func__, *GetNameSafe(Anim));
		return false;
	}
...
void URazerChromaFunctionLibrary::StopChromaAnimation(const URazerChromaAnimationAsset* AnimToStop)
{
	...
	if (LoadedAnimId == INDEX_NONE)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load Chroma Animation %ls", __func__, *GetNameSafe(AnimToStop));
		return;
	}

	FRazerChromaEditorDynamicAPI::StopAnimation(LoadedAnimId);

	UE_LOGF(LogRazerChroma, Verbose, "[%s] Stopping Razer Chroma Animation %ls (%d)", __func__, *GetNameSafe(AnimToStop), LoadedAnimId);
#endif	// #if RAZER_CHROMA_SUPPORT
}
...
void URazerChromaFunctionLibrary::PauseChromaAnimation(const URazerChromaAnimationAsset* AnimToPause)
{
	...
	if (!AnimToPause)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Invalid AnimToPause!", __func__);
		return;
	}
	...
	if (LoadedAnimId == INDEX_NONE)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load Chroma Animation %ls", __func__, *GetNameSafe(AnimToPause));
		return;
	}

	FRazerChromaEditorDynamicAPI::PauseAnimation(LoadedAnimId);
	UE_LOGF(LogRazerChroma, Verbose, "[%s] Pausing Razer Chroma Animation %ls (%d)", __func__, *GetNameSafe(AnimToPause), LoadedAnimId);
#endif	// #if RAZER_CHROMA_SUPPORT
}
...
bool URazerChromaFunctionLibrary::IsChromaAnimationPaused(const URazerChromaAnimationAsset* Anim)
{
	...
	if (!Anim)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Invalid Anim!", __func__);
		return false;
	}
...
void URazerChromaFunctionLibrary::ResumeChromaAnimation(const URazerChromaAnimationAsset* AnimToResume, const bool bLoop)
{
	...
	if (!AnimToResume)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Invalid AnimToResume!", __func__);
		return;
	}
	...
	if (LoadedAnimId == INDEX_NONE)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load Chroma Animation %ls", __func__, *GetNameSafe(AnimToResume));
		return;
	}
...
void URazerChromaFunctionLibrary::SetIdleAnimation(const URazerChromaAnimationAsset* NewIdleAnimation)
{
	...
	if (!NewIdleAnimation)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Invalid NewIdleAnimation!", __func__);
		return;
	}
	...
	if (LoadedAnimId == INDEX_NONE)
	{
		UE_LOGF(LogRazerChroma, Error, "[%s] Failed to load Idle Animation!", __func__);
		return;
	}
...
void URazerChromaFunctionLibrary::SetAllDevicesStaticColor(const FColor& ColorToSet, const ERazerChromaDeviceTypes DeviceTypes /* = ERazerChromaDeviceTypes::All */)
{
	...
	UE_LOGF(LogRazerChroma, Verbose, "[%s] Set static light color to %ls on device types %ls",
		__func__,
		*ColorToSet.ToString(),
