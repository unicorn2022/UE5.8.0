...
#define LOCTEXT_NAMESPACE "MotoSynthSourceFactory"
...
UObject* UMotoSynthSourceFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	if (StagedSoundWave.IsValid())
	{
		...
		if (SoundWave)
		{
			...
			{
				UE_LOGF(LogMotoSynthEditor, Warning, "Sound source used as moto synth source has more than one channel. Only using the 0th channel index (left) for moto synth source.");
			}
