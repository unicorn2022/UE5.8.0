...
void UMotoSynthSource::RegisterSourceData()
{
	...
	{
		UE_LOGF(LogMotoSynth, Error, "No RPM curve data loaded for moto synth source %ls", *GetName());
	}
}
...
void UMotoSynthSource::BuildGrainTableByRPMEstimation()
{
	...
	{
		UE_LOGF(LogMotoSynth, Error, "No analysis buffer to build grain table from.");
		return;
	}

	if (SourceSampleRate <= 0)
	{
		UE_LOGF(LogMotoSynth, Error, "Unable to build grain table for moto synth soruce, source sample rate is invalid (%d)", SourceSampleRate);
		return;
	}
	...
	UE_LOGF(LogMotoSynth, Log, "Grain Table Built Using RPM Estimation: %d Grains", GrainTable.Num());
}
