...
void FMotoSynthSourceDataManager::RegisterData(uint32 InSourceID, const FName& InSourceName, TArray<int16>&& InSourceDataPCM, int32 InSourceSampleRate, TArray<FGrainTableEntry>&& InGrainTable, const FRichCurve& InRPMCurve, bool bConvertTo8Bit)
{
	...
	{
		UE_LOGF(LogMotoSynth, Error, "Moto synth source data already registered for source ID %d", InSourceID);
		return;
	}
	...
	if (MemoryLoggingCvar == 1)
	{
		...
		UE_LOGF(LogMotoSynth, Display, "Registering New Moto Synth Source (Name: %ls, Id: %d), Source Size %d MB, Grain Table Size %d MB", *InSourceName.ToString(), InSourceID, SourceDataSizeBytes / (1024 * 1024), GrainTableDataSizeBytes / (1024 * 1024));
		LogMemoryUsage();
	}
}
...
void FMotoSynthSourceDataManager::UnRegisterData(uint32 InSourceID)
{
	...
	if (NumRemoved == 0)
	{
		UE_LOGF(LogMotoSynth, Error, "No entry in moto synth source data entry for moto synth source ID %d", InSourceID);
	}

	if (NumRemoved == 1 && MemoryLoggingCvar == 1)
	{
		UE_LOGF(LogMotoSynth, Display, "Unregistering Moto Synth Source (Id: %d)", InSourceID);
		LogMemoryUsage();
	}
}
...
MotoSynthDataPtr FMotoSynthSourceDataManager::GetMotoSynthData(uint32 InSourceID)
{
	...
	{
		UE_LOGF(LogMotoSynth, Error, "Unable to get moto synth data view for source ID %d", InSourceID);
		return nullptr;
	}
}
...
{	
	...
	UE_LOGF(LogMotoSynth, Display, "MotoSynthSource Data: %d Sources, %.2f MB source, %.2f MB grain table (%.2f MB total)", NumSources, NumMBSource, NumMBGrainTable, NumMBSource + NumMBGrainTable);
}
