...
{
	UE_LOGF(LogInteractions, Log, "[%s] %d targets are now available to be interacted with.", __func__, CurrentAvailableTargets.Num());
}

void UGameplayAbility_Interact::OnTriggerInteraction_Implementation()
{
	UE_LOGF(LogInteractions, Log, "[%s] trigger interactions on %d targets", __func__, CurrentAvailableTargets.Num());
}
