...
{
	CurrentConfig = MakeShared<UE::AIAssistant::FCurrentConfig>();
	...
	auto SetupWhenEditorIsReady = [this]()
		{
			RegisterStatusBarPanelDrawerSummon();
			UE::AIAssistant::FFileLockManager::RegisterWithEditor();
		};
	if (GEditor)
	{
		SetupWhenEditorIsReady();
	}
	else
	{
		FSimpleMulticastDelegate& OnPostEngineInit = FOnPostEngineInit<FCoreDelegates>::Get();
		OnPostEngineInitHandle = UE::ToolsetRegistry::FDelegateHandleRaii::Create(
			OnPostEngineInit, OnPostEngineInit.AddLambda(MoveTemp(SetupWhenEditorIsReady)));
	}
...
/*virtual*/ void FAIAssistantModule::ShutdownModule()
{
	...
	UE::AIAssistant::FFileLockManager::UnregisterFromEditor();
...
void FAIAssistantModule::OnSlateQuery()
{
	...
	if (WidgetPath.IsValid())
	{
		...
		// OnOpenPluginTab can silently fail in contexts that don't support the AI Assistant tab
		// (e.g. the UEFN widget editor). Guard against proceeding with a null browser widget.
		if (!GetAIAssistantWebBrowserWidget().IsValid())
		{
			UE_LOGF(
				LogAIAssistant, Warning, "Cannot query UI: AI Assistant tab failed to open.");
			return;
		}
