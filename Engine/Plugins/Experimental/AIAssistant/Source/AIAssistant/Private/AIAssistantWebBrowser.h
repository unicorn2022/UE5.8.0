...
public:
	SLATE_BEGIN_ARGS(SAIAssistantWebBrowser) {}
		// This *must* be set to construct this widget.
		SLATE_ARGUMENT(TSharedPtr<UE::AIAssistant::FCurrentConfig>, CurrentConfig);
	SLATE_END_ARGS()
...
protected:
	// Create the WebApplication for this browser. Override to provide a custom
	// WebApplication subclass (e.g. for testing).
	virtual TSharedPtr<UE::AIAssistant::FWebApplication> CreateWebApplication(
		UE::AIAssistant::IWebJavaScriptExecutor& Executor,
		UE::AIAssistant::IWebJavaScriptDelegateBinder& Binder,
...
private:
	...
	// If the reference is not valid and the editor is not shutting down,
	// assert / check on if bCheckUsingWebApplicationIsCalled is true.
	// When the editor is shutting down UsingWebApplication will not be called.
	...
	// Configuration.
	TSharedPtr<UE::AIAssistant::FCurrentConfig> CurrentConfig;
