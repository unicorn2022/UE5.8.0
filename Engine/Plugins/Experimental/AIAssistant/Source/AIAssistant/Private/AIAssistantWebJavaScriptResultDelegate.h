...
public:
	...
	// Whether or not the handler should remain registered or be unregistered after running.
	...
	// This functor should return EHandlerStatus::Unregister to unregister the handler or
	// EHandlerStatus::Continue to leave the handler registered for future callbacks.
	using FResultHandler = TFunction<EHandlerStatus(FResultHandlerContext&&)>;
...
public:
	...
	void Bind(
		...
		FSimpleMulticastDelegate& UnbindDelegate = FCoreDelegates::OnPreExit);
	...
	// Register a callback that may be called multiple times. Return EHandlerStatus::Unregister
	// from the callback to unregister the handler, or explicitly call UnregisterResultHandler.
	FString RegisterResultHandlerForCallback(TFunction<EHandlerStatus(FResult)> Callback);

	// Unregister a previously registered handler with the specified ID.
	void UnregisterResultHandler(const FString& HandlerId);
