	...
	for (bool bExpectedJsonResultIsError : TStaticArray<bool, 2>{ true, false })
	{
		...
		FString HandlerId = ResultDelegate->RegisterResultHandler(
			[this, &ExpectedJsonResult, bExpectedJsonResultIsError, &bCalledHandler](
				UAIAssistantWebJavaScriptResultDelegate::FResultHandlerContext&&
				ResultHandlerContext) -> UAIAssistantWebJavaScriptResultDelegate::EHandlerStatus
			{
				(void)TestEqual(
	...
	const FString& UnusedParameters)
{
	...
	FString HandlerId = ResultDelegate->RegisterResultHandler(
		[&bWasCalled](UAIAssistantWebJavaScriptResultDelegate::FResultHandlerContext&&) ->
			UAIAssistantWebJavaScriptResultDelegate::EHandlerStatus
		{
			bWasCalled = true;
			return UAIAssistantWebJavaScriptResultDelegate::EHandlerStatus::Unregister;
	...
	const FString& UnusedParameters)
{
	...
	FString HandlerId = ResultDelegate->RegisterResultHandler(
		[&NumberOfCalls](
			UAIAssistantWebJavaScriptResultDelegate::FResultHandlerContext&&) mutable ->
			UAIAssistantWebJavaScriptResultDelegate::EHandlerStatus
		{
			NumberOfCalls++;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAIAssistantWebJavaScriptResultDelegateTestRegisterResultHandlerForCallback,
	"AI.Assistant.WebJavaScriptResultDelegate.RegisterResultHandlerForCallback",
	AIAssistantTest::Flags);

bool FAIAssistantWebJavaScriptResultDelegateTestRegisterResultHandlerForCallback::RunTest(
	const FString& UnusedParameters)
{
	TStrongObjectPtr<UAIAssistantWebJavaScriptResultDelegate> ResultDelegate(
		NewObject<UAIAssistantWebJavaScriptResultDelegate>());
	int CallCount = 0;
	FString HandlerId = ResultDelegate->RegisterResultHandlerForCallback(
		[&CallCount](UAIAssistantWebJavaScriptResultDelegate::FResult) mutable ->
			UAIAssistantWebJavaScriptResultDelegate::EHandlerStatus
		{
			CallCount++;
			return UAIAssistantWebJavaScriptResultDelegate::EHandlerStatus::Continue;
	...
	for (int i = 0; i < ExpectedCalls; ++i)
	{
		FWebJavaScriptResultDelegateAccessor::CallHandleResult(
			*ResultDelegate, HandlerId, TEXT(""), false);
		(void)TestEqual(
			TEXT("ExpectedNumberOfCalls"), i + 1, CallCount);
	}
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAIAssistantWebJavaScriptResultDelegateTestUnregisterResultHandler,
	"AI.Assistant.WebJavaScriptResultDelegate.UnregisterResultHandler",
	AIAssistantTest::Flags);

bool FAIAssistantWebJavaScriptResultDelegateTestUnregisterResultHandler::RunTest(
	const FString& UnusedParameters)
{
	TStrongObjectPtr<UAIAssistantWebJavaScriptResultDelegate> ResultDelegate(
		NewObject<UAIAssistantWebJavaScriptResultDelegate>());
	...
	ResultDelegate->UnregisterResultHandler(HandlerId);
	(void)TestEqual(
		TEXT("NumberOfHandlers"), 0,
		FWebJavaScriptResultDelegateAccessor::GetResultHandlersById(
			*ResultDelegate).Num());
	FWebJavaScriptResultDelegateAccessor::CallHandleResult(
		*ResultDelegate, HandlerId, TEXT(""), false);
	(void)TestFalse(TEXT("HandlerNotCalled"), bWasCalled);
	return true;
}
...
DECLARE_MULTICAST_DELEGATE(FAIAssistantTestOnPreExit)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAIAssistantWebJavaScriptResultDelegateTestCompleteAllPendingPromisesOnPreExit,
	"AI.Assistant.WebJavaScriptResultDelegate.CompleteAllPendingPromisesOnPreExit",
	AIAssistantTest::Flags);

bool FAIAssistantWebJavaScriptResultDelegateTestCompleteAllPendingPromisesOnPreExit::RunTest(
	const FString& UnusedParameters)
{
	FAIAssistantTestOnPreExit OnPreExit;
	FFakeWebJavaScriptDelegateBinder Binder;
	TStrongObjectPtr<UAIAssistantWebJavaScriptResultDelegate> ResultDelegate(
		NewObject<UAIAssistantWebJavaScriptResultDelegate>());
	ResultDelegate->Bind(Binder, OnPreExit);
	TPair<FString, TFuture<UAIAssistantWebJavaScriptResultDelegate::FResult>> ResultHandle =
		ResultDelegate->RegisterResultHandlerForFuture();

	// Use OnPreExit() to force clean up of pending promises.
	OnPreExit.Broadcast();
