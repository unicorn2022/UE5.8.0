...
{
	const auto DefaultSearchDirectories = FConfig::GetDefaultSearchDirectories();
	(void)TestNotEqual(
...
bool FAIAssistantConfigTestFindConfigFileExists::RunTest(const FString& UnusedParameters)
{
	...
	const FString ConfigFilename =
		FPaths::Combine(*TemporaryDirectoryWithConfig, FConfig::DefaultFilename);
	TestTrue(TEXT("WriteConfigFile"), FFileHelper::SaveStringToFile(TEXT(""), *ConfigFilename));
	(void)TestEqual(
		TEXT("FindConfigFileExists"),
		FConfig::FindConfigFile({ *TemporaryDirectoryUnused, *TemporaryDirectoryWithConfig }),
		ConfigFilename);
	return true;
}
...
bool FAIAssistantConfigTestFindConfigFileMissing::RunTest(const FString& UnusedParameters)
{
	...
	(void)TestEqual(
		...
		FConfig::FindConfigFile({ *TemporaryDirectory }),
		"");
	return true;
}
...
{
	const FConfig Config = FConfig::Load("");
	(void)TestEqual("main_url", Config.MainUrl, FConfig::DefaultMainUrl);
	return true;
}
...
{
	const FTemporaryDirectory TemporaryDirectory;
	const FString ConfigFilename(FPaths::Combine(*TemporaryDirectory, FConfig::DefaultFilename));
	verify(
		FFileHelper::SaveStringToFile(
			FString(TEXT(R"json({"main_url": "https://localhost/assistant"})json")),
			*ConfigFilename));
	FConfig Config = FConfig::Load(ConfigFilename);
...
bool FAIAssistantConfigTestIgnoresAdditionalFields::RunTest(const FString& UnusedParameters)
{
	...
	verify(
		FFileHelper::SaveStringToFile(
			...
			*ConfigFilename));
	const FConfig Config = FConfig::Load(ConfigFilename);
	(void)TestEqual(TEXT("main_url"), Config.MainUrl, FConfig::DefaultMainUrl);
	return true;
}
