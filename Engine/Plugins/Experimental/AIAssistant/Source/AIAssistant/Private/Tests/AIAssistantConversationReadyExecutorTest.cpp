...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FAIAssistantConversationReadyExecutorNotifyAgentEnvironmentReconfiguringTest,
	"AI.Assistant.ConversationReadyExecutor.NotifyAgentEnvironmentReconfiguring",
	AIAssistantTest::Flags);

bool FAIAssistantConversationReadyExecutorNotifyAgentEnvironmentReconfiguringTest::RunTest(
	const FString& UnusedParameters)
{
	FConversationReadyExecutor ConversationReadyExecutor;
	ConversationReadyExecutor.NotifyAgentEnvironmentConfigured();
	...
	// reconfiguring should clear the configured flag
	ConversationReadyExecutor.NotifyAgentEnvironmentReconfiguring();
	(void)TestFalse(TEXT("NotConfiguredAfterReconfiguring"), ConversationReadyExecutor.IsAgentEnvironmentConfigured());

	// executor should block when agent is not configured
	bool bExecuted = false;
	ConversationReadyExecutor.ExecuteWhenReady([&bExecuted]() -> void { bExecuted = true; });
	(void)TestFalse(TEXT("NotExecutedWhileReconfiguring"), bExecuted);

	// executor should unblock when agent is configured
	ConversationReadyExecutor.NotifyAgentEnvironmentConfigured();
	(void)TestTrue(TEXT("ExecutedAfterReconfigured"), bExecuted);
	(void)TestTrue(TEXT("ConfiguredAgain"), ConversationReadyExecutor.IsAgentEnvironmentConfigured());
	return true;
}
