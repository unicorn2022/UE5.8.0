...
namespace UE::AIAssistant
{
		...
		const FString& ResultJson, bool bResultJsonIsError)
	{
		...
		// Remove the completed entry so it no longer appears in FindExecutedAsyncFunctions.
		// This ensures repeated calls to TestExpectAsyncFunctionCallAndComplete on the same
		// function name (e.g. across multiple page loads) each find exactly one match.
		ExecutedAsyncFunctions.RemoveAll(
			[&ExecutedAsyncFunction](const FExecutedAsyncFunction& Other)
			{
				return Other.HandlerId == ExecutedAsyncFunction.HandlerId;
			});
	}

	void FFakeWebApi::InvokeCallback(
		const FString& HandlerId, const FString& ResultJson, bool bResultJsonIsError)
	{
		FWebJavaScriptResultDelegateAccessor::CallHandleResult(
			*WebJavaScriptResultDelegate, HandlerId, ResultJson, bResultJsonIsError);
	}

	FString FFakeWebApi::RegisterCallback(
		TFunction<UAIAssistantWebJavaScriptResultDelegate::EHandlerStatus(
			UAIAssistantWebJavaScriptResultDelegate::FResult)> Callback)
	{
		RegisteredCallbackHandlerIds.Add(FWebApi::RegisterCallback(MoveTemp(Callback)));
		return RegisteredCallbackHandlerIds.Last();
	}
