...
void FDMMaterialValueMediaStreamPropertyRowGenerator::AddCacheCategory(FDMComponentPropertyRowGeneratorParams& InParams)
{
	...
	FDMPropertyHandle& UseLocalCacheHandle = InParams.PropertyRows->Add_GetRef(
		IDMWidgetLibrary::Get().GetPropertyHandle(InParams.CreatePropertyHandleParams(TEXT("bUseLocalCache")))
	);
	UseLocalCacheHandle.CategoryOverrideName = CategoryName;
