...
TSharedPtr<FJsonValue> UDMMaterialValueMediaStream::JsonSerialize() const
{
	...
	Data.Add(FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(FMediaStreamSource, Scheme)), FDMJsonUtils::Serialize(Source.Scheme));
	Data.Add(FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(FMediaStreamSource, Path)), FDMJsonUtils::Serialize(Source.Path));
	...
	if (TScriptInterface<IMediaStreamPlayer> Player = MediaStream->GetPlayer())
	{
		...
		Data.Add(FJsonObject::FStringType(TEXT("PlayerConfig")), FDMJsonUtils::Serialize<FMediaStreamPlayerConfig>(PlayerConfig));

		const FMediaStreamTextureConfig& TextureConfig = Player->GetTextureConfig();
		Data.Add(FJsonObject::FStringType(TEXT("TextureConfig")), FDMJsonUtils::Serialize<FMediaStreamTextureConfig>(TextureConfig));
	}
...
bool UDMMaterialValueMediaStream::JsonDeserialize(const TSharedPtr<FJsonValue>& InJsonValue)
{
	...
	if (const TSharedPtr<FJsonValue>* JsonValue = Data.Find(FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(FMediaStreamSource, Scheme))))
	{
	...
	if (const TSharedPtr<FJsonValue>* JsonValue = Data.Find(FJsonObject::FStringType(GET_MEMBER_NAME_STRING_CHECKED(FMediaStreamSource, Path))))
	{
	...
	if (TScriptInterface<IMediaStreamPlayer> Player = MediaStream->GetPlayer())
	{
		if (const TSharedPtr<FJsonValue>* JsonValue = Data.Find(FJsonObject::FStringType(TEXT("PlayerConfig"))))
		{
		...
		if (const TSharedPtr<FJsonValue>* JsonValue = Data.Find(FJsonObject::FStringType(TEXT("TextureConfig"))))
		{
