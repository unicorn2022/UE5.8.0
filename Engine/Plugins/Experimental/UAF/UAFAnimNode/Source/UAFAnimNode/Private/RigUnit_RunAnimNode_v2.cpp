...
#include "AnimNode/RigUnit_RunAnimNode_v2.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(RigUnit_RunAnimNode_v2)

FRunAnimNodeWorkData_v2::~FRunAnimNodeWorkData_v2()
{
...
FRigUnit_RunAnimNode_v2_Execute()
{
	...
	if (WorkData.DebugInstanceId == 0)
	{
		...
		TRACE_INSTANCE(ModuleContextData.GetObject(), WorkData.DebugInstanceId, ModuleInstance.GetUniqueId(), FRigUnit_RunAnimNode_v2::StaticStruct(), "UAF Anim Graph");
	}
#endif
	...
	const int32 DesiredLOD = RefPose.GetSourceLODLevel();

	FAnimNextGraphLODPose GraphPose;
	// TODO: Currently forcing additive flag to false here
	GraphPose.LODPose.PrepareForLOD(RefPose, DesiredLOD, true, false);

	ensure(GraphPose.LODPose.LODLevel == DesiredLOD);
	...
	if (WorkData.AnimNodeInstance)
	{
		...
		// Use AnimOps to compute our pose
		{
			...
			USkeletalMesh* SkeletalMesh = OutputComponent.GetSkeletalMesh();
			...
			if (Skeleton != nullptr)
			{
				...
				EvaluateValues(EvaluateArgs, GraphPose);
			}
			else
			{
	...
	// Move our output pose into the result
	Result.SetAs<FVirtualValueBundle_ValueBundle>(MoveTemp(GraphPose));
}
