...
void UAnimNextAssetWorkspaceAssetUserData::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	if (!Context.IsSaving())
	{
		...
		{
			UUAFRigVMAsset* Asset = CastChecked<UUAFRigVMAsset>(GetOuter());
			if (const UUAFRigVMAssetEditorData* GraphEditorData = UE::UAF::UncookedOnly::FUtils::GetEditorData<UUAFRigVMAssetEditorData>(Asset))
			{
