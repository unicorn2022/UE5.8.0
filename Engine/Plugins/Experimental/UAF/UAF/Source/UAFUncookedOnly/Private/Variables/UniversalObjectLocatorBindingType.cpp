...
TSharedRef<SWidget> FUniversalObjectLocatorBindingType::CreateEditWidget(const TSharedRef<IPropertyHandle>& InPropertyHandle, const FAnimNextParamType& InType) const
{
	class SEditWidget : public SCompoundWidget
	{
		...
		void RefreshEntries()
		{
			...
			UObject* Context = UUAFComponent::StaticClass()->GetDefaultObject();
			...
			else if(const UClass* Class = Cast<UClass>(Struct))
			{
				...
				for (const UClass* LibraryClass : Classes)
				{
					...
					{
						const UBlueprintGeneratedClass* BPClass = Cast<UBlueprintGeneratedClass>(LibraryClass);
						
						// Make sure we skip any out-of-date or skeleton classes
						if(LibraryClass->HasAnyClassFlags(CLASS_NewerVersionExists) ||
							(BPClass && BPClass->IsInternalClass()))
						{
							continue;
						}
					}
...
FText FUniversalObjectLocatorBindingType::GetDisplayText(TConstStructView<FAnimNextVariableBindingData> InBindingData) const
{
	...
	case FAnimNextUniversalObjectLocatorBindingType::Property:
		...
		else if (!LocatorBinding->Property.IsPathToFieldEmpty())
		{
			StringBuilder.Append(TEXT("???"));
		}
		else
	...
	case FAnimNextUniversalObjectLocatorBindingType::HoistedFunction:
		...
		else if (!LocatorBinding->Function.IsNull())
		{
			StringBuilder.Append(TEXT("???"));
		}
		else
...
FText FUniversalObjectLocatorBindingType::GetTooltipText(TConstStructView<FAnimNextVariableBindingData> InBindingData) const
{
	...
	if (!Validate(InBindingData, ValidationError))
	{
		TextBuilder.AppendLine(FText::Format(LOCTEXT("ValidationErrorFormat", "Error: {0}"), ValidationError));
	}

	return TextBuilder.ToText();
}

bool FUniversalObjectLocatorBindingType::Validate(TConstStructView<FAnimNextVariableBindingData> InBindingData, FText& OutError) const
{
	const FAnimNextUniversalObjectLocatorBindingData& LocatorBinding = InBindingData.Get<FAnimNextUniversalObjectLocatorBindingData>();
	if (LocatorBinding.Locator.IsEmpty())
	{
		return true;
	}
	...
	case FAnimNextUniversalObjectLocatorBindingType::Property:
	{
		if (LocatorBinding.Property.Get() == nullptr)
		{
			OutError = FText::Format(LOCTEXT("PropertyBindingError", "Property binding '{0}' could not be found"), FText::FromString(LocatorBinding.Property.ToString()));
			return false;
		}
		return true;
	}
	case FAnimNextUniversalObjectLocatorBindingType::Function:
	{
		if(LocatorBinding.Function.Get() == nullptr)
		{
			OutError = FText::Format(LOCTEXT("FunctionBindingError", "Function '{0}' could not be found"), FText::FromString(LocatorBinding.Function.ToString()));
			return false;
		}
		return true;
	}
	case FAnimNextUniversalObjectLocatorBindingType::HoistedFunction:
	{
		if(LocatorBinding.Function.Get() == nullptr)
		{
			OutError = FText::Format(LOCTEXT("HoistedFunctionBindingError", "Function '{0}' could not be found"), FText::FromString(LocatorBinding.Function.ToString()));
			return false;
		}
		return true;
	}
...
void FUniversalObjectLocatorBindingType::BuildBindingGraphFragment(const FRigVMCompileSettings& InSettings, const FBindingGraphFragmentArgs& InArgs, URigVMPin*& OutExecTail, FVector2D& OutLocation) const
{
	...
	for(const TPair<FUniversalObjectLocator, FBindingData>& LocatorDataPair : LocatorDataMap)
	{
		...
		{
			FText ValidationError;
			if (!Validate(Input->BindingData, ValidationError))
			{
				FText ValidationErrorReport = FText::Format(LOCTEXT("ValidationErrorReport", "Variable '{0}': {1}"), FText::FromName(Input->VariableName), ValidationError);
				InSettings.ReportError(ValidationErrorReport.ToString());
				continue;
			}
			...
			case FAnimNextUniversalObjectLocatorBindingType::Property:
			{
				if (LocatorBinding.Property.Get() == nullptr)
				{
					InSettings.ReportErrorf(TEXT("Property binding '%s' for variable '%s' could not be found"), *LocatorBinding.Property.ToString(), *Input->VariableName.ToString());
					continue;
				}
				...
				URigVMPin* ValuePin = GetterNode->FindPin(FRigVMDispatch_GetObjectProperty::ValueName.ToString());
				if (ValuePin == nullptr)
				{
					InSettings.ReportError(TEXT("Could not find Value pin"));
					continue;
				}

				if (!InArgs.Controller->ResolveWildCardPin(ValuePin->GetPinPath(), FRigVMRegistry::Get().GetTypeIndex(*Input->CPPType, Input->CPPTypeObject), false))
				{
					InSettings.ReportError(TEXT("Could not resolve wildcard pin to function's return type"));
					continue;
				}
			...
			case FAnimNextUniversalObjectLocatorBindingType::Function:
			{
				...
				URigVMPin* ValuePin = GetterNode->FindPin(FRigVMDispatch_CallObjectAccessorFunctionBase::ValueName.ToString());
				if (ValuePin == nullptr)
				{
					InSettings.ReportError(TEXT("Could not find Value pin"));
					continue;
			...
			case FAnimNextUniversalObjectLocatorBindingType::HoistedFunction:
			{
				...
				URigVMPin* FunctionPin = GetterNode->FindPin(FRigVMDispatch_CallHoistedAccessorFunctionBase::FunctionName.ToString());
				if(FunctionPin == nullptr)
				{
					InSettings.ReportError(TEXT("Could not find Function pin"));
					continue;
				...
				URigVMPin* ValuePin = GetterNode->FindPin(FRigVMDispatch_CallHoistedAccessorFunctionBase::ValueName.ToString());
				if (ValuePin == nullptr)
				{
					InSettings.ReportError(TEXT("Could not find Value pin"));
					continue;
