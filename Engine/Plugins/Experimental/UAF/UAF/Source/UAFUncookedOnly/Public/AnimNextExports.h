...
struct FAnimNextExport
{
	...
	{
		const UScriptStruct* ThisDataType = Data.GetScriptStruct();
		const UScriptStruct* OtherDataType = Other.Data.GetScriptStruct();
		return Identifier == Other.Identifier && ThisDataType == OtherDataType && (ThisDataType == nullptr || ThisDataType->CompareScriptStruct(Data.GetMemory(), Other.Data.GetMemory(), 0));
	}
...
struct FAnimNextVariableDeclarationData : public FAnimNextExportData
{
	...
	FAnimNextVariableDeclarationData(const FAnimNextParamType& InType, const FGuid& InGuid, const EAnimNextExportedVariableFlags& InFlags) : Type(InType), Guid(InGuid), Flags(static_cast<uint32>(InFlags)) {}
