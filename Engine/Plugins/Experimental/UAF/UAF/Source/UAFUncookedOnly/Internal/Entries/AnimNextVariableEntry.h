...
class UAnimNextVariableEntry : public UUAFRigVMAssetEntry, public IUAFRigVMVariableInterface, public IUAFRigVMExportInterface
{
	GENERATED_BODY()

	friend class UUAFRigVMAssetEditorData;
	friend class UUAFSystem_EditorData;
	friend class UUAFSharedVariables_EditorData;
	...
	friend struct FAssetVariableTests;
	...
	virtual const FGuid& GetGuid() const override { return Guid; }
	...
	/** Stable (unique) Guid used to reference variable without requiring ParameterName */
	UPROPERTY()
	FGuid Guid;
};
