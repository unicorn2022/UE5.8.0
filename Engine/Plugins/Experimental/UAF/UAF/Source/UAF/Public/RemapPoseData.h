...
struct FRemappedBone
{
	...
	FBoneIndexType SourceBoneIndex = INVALID_BONE_INDEX;
	FBoneIndexType TargetBoneIndex = INVALID_BONE_INDEX;
};
...
struct FRemapPoseData
{
	 ...
	 * Prepares the target pose internally (allocates, fills ref pose for unmapped bones).
	 */
	UAF_API void RemapPose(const UE::UAF::FLODPoseHeap& SourcePose,
		UE::UAF::FLODPoseHeap& OutTargetPose) const;

	/**
	 * Remap bone transforms from source into an already-prepared target pose.
	 * The target must already have memory allocated and ref pose set for unmapped bones.
	 * Operates on FLODPose base, works with any allocator (heap or stack).
	 */
	UAF_API void RemapBones(const UE::UAF::FLODPose& SourcePose, UE::UAF::FLODPose& OutTargetPose) const;

	/**
	 * Remap attributes from source into an already-prepared target attribute container.
	 * Bone indices are translated from source LOD pose space to target LOD pose space via the skeleton bone name.
	 * Attributes whose source bone has no counterpart in the target skeleton are silently dropped.
	 * Templated to support both heap (FHeapAttributeContainer) and stack (FStackAttributeContainer) output containers.
	 */
	 ...
	 */
	FTransform RecursiveCalcModelspaceTransform(const UE::UAF::FLODPose& Pose, FBoneIndexType BoneIndex) const;
