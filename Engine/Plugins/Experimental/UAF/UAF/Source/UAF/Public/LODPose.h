...
struct FLODPose
{
	...
	const TArrayView<const FBoneIndexType> GetMeshBoneIndexToParentMeshBoneIndexMap() const
	{
		if (RefPose != nullptr)
		{
			return RefPose->GetMeshBoneIndexToParentMeshBoneIndexMap();
		}
		else
		{
			return TArrayView<const FBoneIndexType>();
	...
	// Returns INVALID_BONE_INDEX if missing ReferencePose or if no bone found for a given name
	...
	{
		check(ChildLODBoneIndex != INVALID_BONE_INDEX);
		check(ParentLODBoneIndex != INVALID_BONE_INDEX);
		...
		while (ParentOfChild != INVALID_BONE_INDEX)
		{
	...
	// Returns INVALID_BONE_INDEX if no reference pose or if ChilddLODBoneIndex is the root
...
struct TLODPose : public FLODPose
{
	...
	TLODPose(const TLODPose& Other) : FLODPose(Other), LocalTransforms(Other.LocalTransforms)
	{
	...
	TLODPose& operator=(const TLODPose& Other)
	{
		FLODPose::operator=(Other);
