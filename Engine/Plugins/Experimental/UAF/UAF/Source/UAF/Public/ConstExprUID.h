...
namespace UE::UAF
{
	...
	struct FConstExprUID final
	{
		...
		// Constructs a UID
		[[nodiscard]] explicit constexpr FConstExprUID(FFConstExprUIDRaw InUID, const TCHAR* InDebugName = TEXT("<Unknown ConstExprUID Name>")) noexcept
			: UID(InUID)
#if !UE_BUILD_SHIPPING
		...
		// Constructs a UID
		[[nodiscard]] static constexpr FConstExprUID MakeFromString(const TCHAR* InName) noexcept
		{
		...
		[[nodiscard]] constexpr const TCHAR* GetDebugName() const noexcept { return DebugName; }
#endif

		// Returns the raw UID
		[[nodiscard]] constexpr FFConstExprUIDRaw GetUID() const noexcept { return UID; }

		// Returns whether this UID is valid or not
		[[nodiscard]] constexpr bool IsValid() const noexcept { return UID != INVALID_UID; }

		// Returns the hash of this UID
		[[nodiscard]] constexpr friend uint32 GetTypeHash(FConstExprUID InUID)
		{
			// Return the UID as-is since it is already a hashed value
			return InUID.UID;
		}
