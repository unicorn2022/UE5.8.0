...
namespace UE::UAF
{
	...
	// @TODO: AoS version is untested because AoS currently does not compile in UAF.
	UAF_API void BlendWithIdentityAndAccumulatePerBoneWeight(const FTransformArrayAoSView& Base, const FTransformArrayAoSConstView& Additive,
		const TArrayView<const int32>& TransformIndexToWeightIndexMap, const TArrayView<const float>& BoneWeights, const float DefaultBlendWeight);
	UAF_API void BlendWithIdentityAndAccumulatePerBoneWeight(const FTransformArraySoAView& Base, const FTransformArraySoAConstView& Additive,
		const TArrayView<const int32>& TransformIndexToWeightIndexMap, const TArrayView<const float>& BoneWeights, const float DefaultBlendWeight);

	// Performs a BlendWithIdentityAndAccumulatePerBoneWeight in mesh space, result is in local space
	// @TODO: AoS version is untested because AoS currently does not compile in UAF.
	UAF_API void BlendWithIdentityAndAccumulatePerBoneWeightMesh(
	...
	// Performs a BlendWithIdentityAndAccumulatePerBoneWeight in mesh space, result is in local space
	UAF_API void BlendWithIdentityAndAccumulatePerBoneWeightMesh(
		const FTransformArraySoAView& Base, const FTransformArraySoAConstView& Additive, const TArrayView<const FBoneIndexType>& LODBoneIndexToParentLODBoneIndexMap, 
		const TArrayView<const int32>& TransformIndexToWeightIndexMap, const TArrayView<const float>& BoneWeights, const float DefaultBlendWeight);
	...
	// Convert Input into an Additive Pose, by doing Output = Input - Base 
	// @TODO: AoS version is untested because AoS currently does not compile in UAF.
	UAF_API void ConvertToAdditive(const FTransformArrayAoSConstView& Input, const FTransformArrayAoSConstView& Base, const FTransformArrayAoSView& Output);
	UAF_API void ConvertToAdditive(const FTransformArraySoAConstView& Input, const FTransformArraySoAConstView& Base, const FTransformArraySoAView& Output);
}
