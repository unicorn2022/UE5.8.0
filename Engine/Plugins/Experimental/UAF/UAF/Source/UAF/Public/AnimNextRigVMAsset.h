...
struct FUAFRigVMSystemScriptEventsComponent;
...
protected:
	...
	friend class UAnimNextWorkspaceEditorMode;
	friend UAnimNextAnimGraphSettings;
	friend FUAFRigVMComponent;
	friend FUAFRigVMSystemScriptEventsComponent;
	...
	friend UE::UAF::FSystemBuilderContext;
	...
	UE_API UUAFRigVMAsset(const FObjectInitializer& ObjectInitializer);
...
public:
	...
	// Execute a parameterless function and copy its return value into OutReturnValue.
	// The function must have exactly 1 arg (the return value output). Returns true on success.
	UE_API bool ExecuteParameterlessFunction(UE::UAF::FFunctionHandle InHandle, FRigVMExtendedExecuteContext& InContext, void* OutReturnValue, int32 ReturnValueSize);

	// Get a function handle given its stable GUID
	UE_API UE::UAF::FFunctionHandle GetFunctionHandle(const FGuid& InFunctionGuid) const;
	...
	using FOnCompileJobEvent = TTSMulticastDelegate<void(UUAFRigVMAsset*)>;
	...
	TConstArrayView<TObjectPtr<const UUAFRigVMAsset>> GetReferencedVariableAssets() const { return ReferencedVariableAssets; }
	...
	// Get all the IRigVMRuntimeAssetInterfaces whose variables we will reference/instance at runtime
	TConstArrayView<TScriptInterface<const IRigVMRuntimeAssetInterface>> GetReferencedVariableRigVMAssets() const { return ReferencedVariableRigVMAssets; }
...
protected:
	...
	// Default components that are required for instances of this asset
	UPROPERTY()
	...
	UPROPERTY(transient)
	EAnimNextRigVMAssetState CompilationState = EAnimNextRigVMAssetState::CompiledWithSuccess;
#endif
};
