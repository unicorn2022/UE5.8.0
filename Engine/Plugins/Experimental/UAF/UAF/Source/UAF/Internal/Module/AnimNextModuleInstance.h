...
struct FRigUnit_UAFRunAsset;
class UUAFEngineSubsystem;
struct FRigUnit_AnimNextInitializeEvent;
struct FRigUnit_CopyModuleProxyVariables;
struct FUAFNotifyDispatcherComponent;
...
// Represents an instance of a UAF system.
USTRUCT()
struct FAnimNextModuleInstance : public FUAFAssetInstance
#if CPP
	, TSharedFromThis<FAnimNextModuleInstance>
#endif
{
	GENERATED_BODY()
	...
	UE_API FAnimNextModuleInstance(
		const UUAFSystem* InModule,
		EAnimNextModuleInitMethod InInitMethod);

	// Standalone constructor, UObject host
	UE_API FAnimNextModuleInstance(
		const UUAFSystem* InModule,
		UObject* InObject,
		EAnimNextModuleInitMethod InInitMethod);
	...
	UE_API void QueueInputTraitEvent(FAnimNextTraitEventPtr Event);

	// Queues an output trait event
	// Output events will be processed at the end of the schedule tick
	UE_API void QueueOutputTraitEvent(FAnimNextTraitEventPtr Event);

	// Get the object that this module is bound to
	UObject* GetObject() const { return Object; }

	// Get the system that this instance represents
	UAF_API const UUAFSystem* GetSystem() const;

#if UE_ENABLE_DEBUG_DRAWING
	// Get the debug draw interface
	UE_API FRigVMDrawInterface* GetDebugDrawInterface();

	// Set whether to show the module's debug drawing instructions in the viewport
	UE_API void ShowDebugDrawing(bool bInShowDebugDrawing);
#endif

	// Run a simple task on the GT via FFunctionGraphTask::CreateAndDispatchWhenReady
	UE_API static void RunTaskOnGameThread(TUniqueFunction<void(void)>&& InFunction);
	...
	UE_API UE::UAF::FModuleEventTickFunction* FindFirstUserTickFunction();

	// Find the last 'user' tick function
	UE_API UE::UAF::FModuleEventTickFunction* FindLastUserTickFunction();

	// Run the specified script event using a module context
	UE_API void RunScriptEvent(const UE::UAF::FScriptEvent& InEvent, float InDeltaTime);

	// Run the specified script event using a module context, using name lookup alone (slower)
	UE_API void RunScriptEventByName(FName InEventName, float InDeltaTime);
	...
	UE_API TArrayView<UE::UAF::FModuleEventTickFunction> GetTickFunctions();

	// Get a weak, non-owning reference to this instance
	UE_API FUAFWeakSystemReference GetReference() const;

	// Queue a task to run before the first user event
	// @param	InTaskFunction		The function to run
	void QueueTask(UE::UAF::FUniqueSystemTask&& InTaskFunction)
	{
		QueueTask(FName(), MoveTemp(InTaskFunction), UE::UAF::ETaskRunLocation::Before);
	}

	// Queue a task to run at a particular system event
	// @param	InEventName			The event in the system to run the supplied task relative to. If this is NAME_None or not found then the first 'user' event will be used.
	// @param	InTaskFunction		The function to run
	// @param	InLocation			Where to run the task, before or after
	UE_API void QueueTask(FName InEventName, UE::UAF::FUniqueSystemTask&& InTaskFunction, UE::UAF::ETaskRunLocation InLocation = UE::UAF::ETaskRunLocation::Before);
...
private:
	// Setup the entry
	// Note that we perform initialization in two phases because we may need to access weak references to 'this' during initialization
	UE_API void Initialize(const FUAFSystemFactoryParams& InFactoryParams);
	...
	// Call module components' OnBeginExecution extension point
	void BeginExecution(float InDeltaTime);
	...
	// Access a variable's value given its reference via a raw memory view.
	...
	EPropertyBagResult ReadVariableFromExternal(const FAnimNextVariableReference& InVariable, const FAnimNextParamType& InType, TFunctionRef<void(TConstArrayView<uint8>)> InFunction);

	// Access the supplied system component for reading. Component will not be lazily created.
	// @param	InComponentType		The type of the component we want ot access
	...
	bool ReadComponentFromExternal(UScriptStruct* InComponentType, TFunctionRef<void(FConstStructView InComponentStruct)> InFunction);
...
private:
	...
	friend FRigUnit_UAFRunAsset;
	friend UUAFEngineSubsystem;
	friend FRigUnit_AnimNextInitializeEvent;
	friend FRigUnit_CopyModuleProxyVariables;
	friend FUAFNotifyDispatcherComponent;
	friend FAnimNextGraphInstance;
	friend UE::UAF::Tests::FVariables_Shared;
	friend UE::UAF::Tests::FVariables_UOLBindings;
	friend UE::UAF::FCustomSystemDeleter;
	...
	FTransactionallySafeRWLock EventListLock;

	// Lock to ensure proxy access is thread safe
	FTransactionallySafeRWLock ProxyLock;

	// Lock guarding against gross concurrent access via system references (safety only: should rarely contend if dependencies are set correctly)
	FTransactionallySafeRWLock SystemReferenceLock;
	...
	// Whether the module instance has had its custom destructor called (queueing it up for release at the end of the frame)
	bool bHasBeenQueuedForRelease : 1 = false;
#endif
...
#if UAF_TRACE_ENABLED
#define TRACE_UAF_MODULE(ModuleInstance) ModuleInstance.Trace();
#else
#define TRACE_UAF_MODULE(ModuleInstance)
#endif
