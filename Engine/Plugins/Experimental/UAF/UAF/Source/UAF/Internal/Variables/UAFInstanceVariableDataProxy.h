...
struct FUAFInstanceVariableDataProxy
{
	...
	EPropertyBagResult AccessVariable(const FAnimNextVariableReference& InVariable, const FAnimNextParamType& InType, TArrayView<uint8>& OutValue);
	...
	// Returns whether the variable has been marked as dirty
	bool IsVariableDirty(const FAnimNextVariableReference& InVariable) const;
