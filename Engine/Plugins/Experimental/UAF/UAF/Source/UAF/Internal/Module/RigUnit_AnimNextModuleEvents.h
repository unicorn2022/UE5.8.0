...
struct FRigUnit_AnimNextModuleEventBase : public FRigUnit_AnimNextBase
{
	...
	virtual UE::UAF::EScriptEventPhase GetEventPhase() const { return UE::UAF::EScriptEventPhase::Execute; }

	// Overriden in derived classes to provide binding function
	virtual UE::UAF::FTickFunctionBindingFunction GetBindingFunction() const { return [](const UE::UAF::FTickFunctionBindingContext& InContext, FTickFunction& InTickFunction){}; }

	// Get the tick group for this event
	virtual ETickingGroup GetTickGroup() const { return TG_PrePhysics; }

	// Get the end tick group for this event
	virtual ETickingGroup GetEndTickGroup() const { return TG_PrePhysics; }
...
struct FRigUnit_AnimNextExecuteBindings_GT : public FRigUnit_AnimNextModuleEventBase
{
	...
	virtual UE::UAF::EScriptEventPhase GetEventPhase() const override final { return UE::UAF::EScriptEventPhase::PreExecute; }
	UE_API virtual UE::UAF::FTickFunctionBindingFunction GetBindingFunction() const override final;
};
...
struct FRigUnit_AnimNextUserEvent : public FRigUnit_AnimNextModuleEventBase
{
	...
	virtual UE::UAF::EScriptEventPhase GetEventPhase() const override final { return UE::UAF::EScriptEventPhase::Execute; }
	UE_API virtual UE::UAF::FTickFunctionBindingFunction GetBindingFunction() const override final;
	virtual ETickingGroup GetTickGroup() const override final { return TickGroup; }
	virtual ETickingGroup GetEndTickGroup() const override final { return EndTickGroup; }
	...
	// The tick group the event completes by 
	UPROPERTY(EditAnywhere, Category = "Tick", meta = (Input, Constant, DetailsOnly))
	TEnumAsByte<ETickingGroup> EndTickGroup = ETickingGroup::TG_PrePhysics;
};
