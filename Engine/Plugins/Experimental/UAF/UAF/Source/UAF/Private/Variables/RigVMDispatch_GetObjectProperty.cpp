...
void FRigVMDispatch_GetObjectProperty::Execute(FRigVMExtendedExecuteContext& InContext, FRigVMMemoryHandleArray Handles, FRigVMPredicateBranchArray RigVMBranches)
{
	...
	if (!ensureMsgf(TargetProperty->GetClass() == SourceProperty->GetClass(), TEXT("FRigVMDispatch_GetObjectProperty::Execute - Type mismatch: %s, %s")
		, *TargetProperty->GetClass()->GetFName().ToString()
		, *SourceProperty->GetClass()->GetFName().ToString()))
	{
		return;
	}
