	...
	, FReferencePose& OutAnimationReferencePose)
{
	...
	UE_LOGF(LogAnimGenerationTools, VeryVerbose, "Generating CanonicalBoneSet for SkeletalMesh %ls.", *SkeletalMesh->GetPathName());
	...
	{
		UE_LOGF(LogAnimGenerationTools, Warning, "Error generating CanonicalBoneSet for SkeletalMesh %ls. No SkeletalMeshRenderData.", *SkeletalMesh->GetPathName());
		return false;
	}
	...
	for (int32 LODIndex = NumLODs - 1; LODIndex > 0; --LODIndex)
	{
		...
		for (FBoneIndexType BoneIdx : LODData.RequiredBones)
		{
			if (!PrevLODData.RequiredBones.Contains(BoneIdx))
			{
				...
				UE_LOGF(LogAnimGenerationTools, Warning, "SkeletalMesh LOD %d does not contain bone [%ls] required by LOD %d. Please update the skeletal mesh asset [%ls] or its corresponding LOD settings asset."
					, LODIndex - 1
	...
	, const USkeletalMesh* SkeletalMesh)
{
	...
	for (int32 LODIndex = NumLODs - 1; LODIndex >= 1; --LODIndex)
	{
		...
		if (bPrevSmaller == false)
		{
			...
			UE_LOGF(LogAnimGenerationTools, Warning, "SkeletalMesh %ls canonical ordered bone set can not be stored as single bones list. LOD %d does not contain all the bones of LOD %d", *SkeletalMesh->GetPathName(), LODIndex, LODIndex - 1);
			break;
		}
		...
		for (TArray<FBoneIndexType>::TConstIterator LODExcludedBonesIt(PrevLODData.ExcludedBones); LODExcludedBonesIt; ++LODExcludedBonesIt)
		{
			...
			{
				bCanGenerateSingleBonesList = false;
				UE_LOGF(LogAnimGenerationTools, Warning, "SkeletalMesh %ls canonical ordered bone set can not be stored in LOD order. LOD %d does not contain all the bones of LOD %d, like e.g. '%ls'.", *SkeletalMesh->GetPathName(), LODIndex, LODIndex - 1, *GetBoneNameSafe(SkeletalMesh, *LODExcludedBonesIt));
				break;
			}
		}
	}
	...
	, TArray<FBoneIndexType>& OrderedBoneList)
{
	...
	for (int i = 0; i < NumBones; ++i)
	{
		...
		if (BoneIndexParentIndex >= 0)
		{
			...
			{
				bCanFastPath = false;
				UE_LOGF(LogAnimGenerationTools, Warning, "Warning : SkeletalMesh [%ls] has an invalid LOD setup.", *SkeletalMesh->GetPathName());
				break;
			}
		}
	}
...
/*static*/ bool FGenerationTools::CheckSkeletalAllMeshSocketsAlwaysAnimate(const USkeletalMesh* SkeletalMesh)
{
	...
	for (const USkeletalMeshSocket* Socket : ActiveSocketList)
	{
		...
		if (BoneIndex != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogSkeletalMesh, Warning, "SkeletalMesh %ls canonical ordered bone set can not be stored as single bones list. Socket [%ls] is not set to always animate.", *SkeletalMesh->GetPathName(), *Socket->GetName());
		...
		static void CheckSkeletalMeshesLODs()
		{
			...
			for (int32 Idx = 0; Idx < NumAssets; Idx++)
			{
				...
				if (SkeletalMesh != nullptr)
				{
					...
					{
						UE_LOGF(LogAnimGenerationTools, VeryVerbose, "[%d of %d] SkeletalMesh %ls BoneReferencePose generated.", Idx + 1, NumAssets, *SkeletalMesh->GetPathName());
					}
				}
				else
				{
					UE_LOGF(LogAnimGenerationTools, VeryVerbose, "[%d of %d] SkeletalMesh is null. Asset : %ls could not be loaded.", Idx + 1, NumAssets, *Assets[Idx].AssetName.ToString());
				}
			}
		}
	};
