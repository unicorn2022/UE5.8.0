...
EPropertyBagResult FUAFInstanceVariableDataProxy::AccessVariable(const FAnimNextVariableReference& InVariable, const FAnimNextParamType& InType, TArrayView<uint8>& OutValue)
{
	if (Variables == nullptr)
	{
		// Not yet initialized
	...
	return ProxyVariableSets[FoundMapping->AllSharedVariableContainersIndex].AccessVariable(FoundMapping->VariableIndex, InType, OutValue);
}
...
bool FUAFInstanceVariableDataProxy::IsVariableDirty(const FAnimNextVariableReference& InVariable) const
{
	if (Variables == nullptr)
	{
		return false;
	...
	const FUAFInstanceVariableContainerProxy& VariableSet = ProxyVariableSets[FoundMapping->AllSharedVariableContainersIndex];
	const bool bIsVariableDirty = VariableSet.DirtyFlags[FoundMapping->VariableIndex];

	return bIsVariableDirty;
}