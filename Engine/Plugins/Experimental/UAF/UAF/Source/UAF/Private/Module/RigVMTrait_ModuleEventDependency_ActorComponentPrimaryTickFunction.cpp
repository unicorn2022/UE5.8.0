...
void FRigVMTrait_ModuleEventDependency_ActorComponentPrimaryTickFunction::OnAddDependency(const UE::UAF::FModuleDependencyContext& InContext) const
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "FRigVMTrait_ModuleEventDependency_ActorComponentPrimaryTickFunction: Could not add dependency for component [%ls] - UAF Component is not valid.", *GetNameSafe(Component));
		return;
	}

	if(Component == nullptr)
	{
		UE_LOGF(LogAnimation, Warning, "FRigVMTrait_ModuleEventDependency_ActorComponentPrimaryTickFunction: Could not add dependency - Component is not valid.");
		return;
	}
...
void FRigVMTrait_ModuleEventDependency_ActorComponentPrimaryTickFunction::OnRemoveDependency(const UE::UAF::FModuleDependencyContext& InContext) const
{
	...
	if(AnimNextComponent == nullptr)
	{
		UE_LOGF(LogAnimation, Warning, "FRigVMTrait_ModuleEventDependency_ActorComponentPrimaryTickFunction: Could not remove dependency for component [%ls] - UAF Component is not valid.", *GetNameSafe(Component));
		return;
	}
	
	if(Component == nullptr)
	{
		UE_LOGF(LogAnimation, Warning, "FRigVMTrait_ModuleEventDependency_ActorComponentPrimaryTickFunction: Could not remove dependency - Component is not valid.");
		return;
	}
