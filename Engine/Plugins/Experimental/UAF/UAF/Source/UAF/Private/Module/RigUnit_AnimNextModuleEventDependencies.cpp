...
static void ApplyDependenciesHelper(const FAnimNextExecuteContext& InContext, const PredicateType& InPredicate)
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "ApplyDependenciesHelper: Could not apply dependencies - No dependencies found.");
		return;
	}
	...
	{
		UE_LOGF(LogAnimation, Warning, "ApplyDependenciesHelper: Could not apply dependencies - No valid tick function found.");
		return;
	}
	...
	FAnimNextModuleInstance::RunTaskOnGameThread([WeakObject, TickFunctions = MoveTemp(TickFunctions), Dependencies = MoveTemp(Dependencies), InPredicate]()
	{
		...
		if(Object == nullptr)
		{
			UE_LOGF(LogAnimation, Warning, "ApplyDependenciesHelper: Could not apply dependencies - Module Object is not valid.");
			return;
		}
		...
		for(int32 DependencyIndex = 0; DependencyIndex < Dependencies.Num(); ++DependencyIndex)
		{
			...
			// For Before-ordered dependencies, also apply to module-owned prerequisites
			// (e.g. ExecuteBindings_GT chained before the user event tick function),
			// so that external prerequisites like a pawn primary tick also gate bindings execution.
			if(Dependency.Ordering == EAnimNextModuleEventDependencyOrdering::Before)
			{
				for(FTickPrerequisite& Prereq : TickFunction->GetPrerequisites())
				{
					if(Prereq.PrerequisiteObject == Object)
					{
						if(FTickFunction* PrereqTF = Prereq.Get())
						{
							// Only propagate to game-thread prerequisites. Worker-thread tick functions
							// run in parallel with the game thread, so adding a game-thread actor tick
							// as a prerequisite would incorrectly serialize parallel work.
							if(!PrereqTF->bRunOnAnyThread)
							{
								FModuleDependencyContext PrereqContext(Object, *PrereqTF);
								InPredicate(PrereqContext, Dependency);
							}
						}
					}
				}
