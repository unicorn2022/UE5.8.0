...
{
	if (&InSourceRefPose != SourceRefPose || &InTargetRefPose != TargetRefPose)
	{
		// First time use or dangling pointers.
		return true;
	}
...
FTransform FRemapPoseData::RecursiveCalcModelspaceTransform(const UE::UAF::FLODPose& Pose, FBoneIndexType BoneIndex) const
{
	...
	const FTransform LocalTransform = Pose.LocalTransformsView[BoneIndex];
...
void FRemapPoseData::RemapPose(const UE::UAF::FLODPoseHeap& SourcePose, UE::UAF::FLODPoseHeap& OutTargetPose) const
{
	...
	RemapBones(SourcePose, OutTargetPose);
}

void FRemapPoseData::RemapBones(const UE::UAF::FLODPose& SourcePose, UE::UAF::FLODPose& OutTargetPose) const
{
	SCOPE_CYCLE_COUNTER(STAT_AnimNext_RemapPose_Mesh2Mesh);

	const int32 SourceLODLevel = SourcePose.LODLevel;
	const int32 TargetLODLevel = OutTargetPose.LODLevel;
	...
	for (int32 MappingElementIndex = 0; MappingElementIndex < NumToRemap; ++MappingElementIndex)
	{
		...
		OutTargetPose.LocalTransformsView[TargetBoneIndex] = SourcePose.LocalTransformsView[SourceBoneIndex];
	}
#endif
	...
	if (RootBoneOnSourceMapped != INDEX_NONE)
	{
		...
		OutTargetPose.LocalTransformsView[UE::UAF::FReferencePose::RootBoneIndex] = RootModelTransform;
	}
}
...
void FRemapPoseData::RemapAttributes(const UE::UAF::FLODPose& SourceLODPose,
	...
	OutContainerType& OutAttributes) const
{
...
template UAF_API void FRemapPoseData::RemapAttributes(const UE::UAF::FLODPose&, const UE::Anim::FHeapAttributeContainer&, const UE::UAF::FLODPose&, UE::Anim::FHeapAttributeContainer&) const;
template UAF_API void FRemapPoseData::RemapAttributes(const UE::UAF::FLODPose&, const UE::Anim::FHeapAttributeContainer&, const UE::UAF::FLODPose&, UE::Anim::FStackAttributeContainer&) const;
