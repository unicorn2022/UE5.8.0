...
namespace UE::UAF::Private
{
	...
	{
		TValueOrError<int64, void> Result = UE::StructUtils::GetPropertyValueAsInt64(InProperty, Address);
		if (Result.HasValue())
		{
	...
	{
		TValueOrError<uint64, void> Result = UE::StructUtils::GetPropertyValueAsUInt64(InProperty, Address);
		if (Result.HasValue())
		{
			OutValue = Result.GetValue();
			return EPropertyBagResult::Success;
	...
	{
		TValueOrError<double, void> Result = UE::StructUtils::GetPropertyValueAsDouble(InProperty, Address);
		if (Result.HasValue())
		{
			OutValue = Result.GetValue();
			return EPropertyBagResult::Success;
...
FUAFInstanceVariableContainer::FUAFInstanceVariableContainer(const FUAFAssetInstance& InHostInstance, const UUAFRigVMAsset* InAsset, const TSharedPtr<const UE::UAF::FVariableOverridesCollection>& InOverrides, int32 InOverridesIndex)
	: HostInstance(&InHostInstance)
{
...
FUAFInstanceVariableContainer::FUAFInstanceVariableContainer(const FUAFAssetInstance& InHostInstance, const IRigVMRuntimeAssetInterface* InAsset, const TSharedPtr<const UE::UAF::FVariableOverridesCollection>& InOverrides, int32 InOverridesIndex)
	: HostInstance(&InHostInstance)
{
	TArray<FRigVMExternalVariable> ExternalVariables = const_cast<IRigVMRuntimeAssetInterface*>(InAsset)->GetExternalVariables();
	FInstancedPropertyBag DefaultProperties;
	
	for (const FRigVMExternalVariable& Variable : ExternalVariables)
	{
		// Add to schema
		DefaultProperties.AddProperty(Variable.GetName(), Variable.GetProperty());

		// Copy in the default value
		DefaultProperties.SetValue(Variable.GetName(), Variable.GetProperty(), InAsset->GetVariablesMemory());
	}
	
	NumVariables = DefaultProperties.GetNumPropertiesInBag();

	// Copy the variables into the container
	VariablesContainer.Set<FInstancedPropertyBag>(MoveTemp(DefaultProperties));

	const UObject* InterfaceObject = Cast< const UObject>(InAsset);
	TScriptInterface<const IRigVMRuntimeAssetInterface> Interface = InterfaceObject;
...
FName FUAFInstanceVariableContainer::GetFName() const
{
	switch (AssetOrStructData.GetIndex())
	{
		case FAssetOrStructType::IndexOfType<FAssetType>():
			{
				return AssetOrStructData.Get<FAssetType>().GetFName();
			}
		case FAssetOrStructType::IndexOfType<FRigVMAssetType>():
			{
				return AssetOrStructData.Get<FRigVMAssetType>().GetObject()->GetFName();
			}
	case FAssetOrStructType::IndexOfType<FStructType>():
		{
			return AssetOrStructData.Get<FStructType>().GetFName();
		}
	default:
		checkNoEntry();
		break;
	...
	case FVariablesContainerType::IndexOfType<FInstancedPropertyBag>():
		{
			...
			if (AssetOrStructData.IsType<FAssetType>())
			{
				const FAssetType& Asset = AssetOrStructData.Get<FAssetType>();
				OwnerObject = Asset;
			}
			else if(AssetOrStructData.IsType<FRigVMAssetType>())
			{
				FRigVMAssetType AssetInterface = AssetOrStructData.Get<FRigVMAssetType>();
				check(AssetInterface.GetObject());
				OwnerObject = AssetInterface.GetObject();
			}
			...
			for (const FPropertyBagPropertyDesc& Desc : PropertyBag->GetPropertyDescs())
			{
				...
				{
					OutVariables.Add(FAnimNextVariableReference::FromName(Desc.Name, OwnerObject));
				}
			}
			break;
		}
	case FVariablesContainerType::IndexOfType<FInstancedStruct>():
		{
			...
			for (const FStructDataCache::FPropertyInfo& PropertyInfo : StructDataCache->GetProperties())
			{
				...
				{
					OutVariables.Add(FAnimNextVariableReference::FromName(PropertyInfo.Property->GetFName(), Struct));
				}
			}
			break;
		}
...
bool FUAFInstanceVariableContainer::ForEachVariable(TFunctionRef<bool(FName, const FAnimNextParamType&, int32)> InFunction, int32& InOutVariableIndex) const
{
	...
	case FVariablesContainerType::IndexOfType<FInstancedPropertyBag>():
		{
			const UPropertyBag* PropertyBag = VariablesContainer.Get<FInstancedPropertyBag>().GetPropertyBagStruct();
...
void FUAFInstanceVariableContainer::ResolveOverrides()
{
	...
	case FAssetOrStructType::IndexOfType<FAssetType>():
		{
			const UUAFRigVMAsset* Asset = AssetOrStructData.Get<FAssetType>();
	...
	case FAssetOrStructType::IndexOfType<FStructType>():
		...
		case FAssetOrStructType::IndexOfType<FRigVMAssetType>():
		{
			const IRigVMRuntimeAssetInterface* RigVMAsset = AssetOrStructData.Get<FRigVMAssetType>().GetInterface();
			checkf(RigVMAsset == PinnedOverrides->Collection[OverridesIndex].AssetOrStructData.Get<FVariableOverrides::FRigVMAssetType>().GetInterface(), TEXT("Variable overrides RigVMRuntimeAsset does not match"));
			TConstArrayView<FVariableOverrides::FOverride> OverridesView = PinnedOverrides->Collection[OverridesIndex].Overrides;
			const int32 NumOverrides = OverridesView.Num();
			...
			{
				const FVariableOverrides::FOverride& Override = OverridesView[OverrideIndex];
				int32 VariableIndex = ExternalVariables.IndexOfByPredicate([&Override](const FRigVMExternalVariable& InVariable)
					{
						return InVariable.GetName() == Override.Name;
					});
				...
				const FProperty* Property = ExternalVariables[VariableIndex].GetProperty();
				if (Property == nullptr)
				{
					ensureMsgf(false, TEXT("Property not found"));
					continue;
				}
				
				FPropertyBagPropertyDesc Desc(Override.Name, Property);
				if (Override.Type != FAnimNextParamType::FromPropertyBagPropertyDesc(Desc))
				{
					ensureMsgf(false, TEXT("Variable type does not match"));
					continue;
	...
	case FAssetOrStructType::IndexOfType<FRigVMAssetType>():
		{
			...
			const FRigVMAssetType& RigVMAsset = AssetOrStructData.Get<FRigVMAssetType>();
			TArray<FRigVMExternalVariable> ExternalVariables = const_cast<IRigVMRuntimeAssetInterface*>(RigVMAsset.GetInterface())->GetExternalVariables();
			for (const FRigVMExternalVariable& ExternalVariable : ExternalVariables)
			{
				PropertyBag.AddProperty(ExternalVariable.GetName(), ExternalVariable.GetProperty());
			}
	...
	case FVariablesContainerType::IndexOfType<FInstancedPropertyBag>():
		{
			...
			for (int32 VariableIndex = 0; VariableIndex < Descs.Num(); ++VariableIndex)
			{
				...
				else
				{
					...
					if (!ensure(Desc.CachedProperty->GetClass() == Def.GetProperty()->GetClass()))
					{
						OutRuntimeData.Empty();
						return;
					}
	...
	case FVariablesContainerType::IndexOfType<FInstancedStruct>():
		{
			...
			for (int32 VariableIndex = 0; VariableIndex < Properties.Num(); ++VariableIndex)
			{
				...
				else
				{
					...
					const FRigVMExternalVariableDef& Def = InVariableDefs[VariableIndex];
					if (!ensure(Property.Property->GetClass() == Def.GetProperty()->GetClass()))
					{
						OutRuntimeData.Empty();
						return;
					}
...
void FUAFInstanceVariableContainer::AddStructReferencedObjects(FReferenceCollector& Collector)
{
	...
	case FAssetOrStructType::IndexOfType<FRigVMAssetType>():
		{
			Collector.AddReferencedObject(AssetOrStructData.Get<FRigVMAssetType>().GetObjectRef());
			break;
		}
	default:
		checkNoEntry();	
