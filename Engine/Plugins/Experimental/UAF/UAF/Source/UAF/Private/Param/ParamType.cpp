...
FAnimNextParamType FAnimNextParamType::FromRigVMTemplateArgument(const FRigVMTemplateArgumentType& RigVMType)
{
	...
	const FName CPPType = *RigVMType.GetBaseCPPType();
	...
	{
		ensureMsgf(false, TEXT("Unsupported type : %s"), *RigVMType.CPPType.ToString());
...
FAnimNextParamType FAnimNextParamType::FromProperty(const FProperty* InProperty)
{
	...
	else if (const FByteProperty* ByteProperty = CastField<const FByteProperty>(InProperty))
	{
		if (ByteProperty->Enum != nullptr)
		{
			Type.ValueType = EPropertyBagPropertyType::Enum;
			Type.ValueTypeObject = ByteProperty->Enum;
		}
		else
		{
			Type.ValueType = EPropertyBagPropertyType::Byte;
...
size_t FAnimNextParamType::GetSize() const
{
	...
	checkf(false, TEXT("Error: FParameterType::GetSize: Unknown Type Container %d, Value %d"), EnumToUnderlyingType(ContainerType), EnumToUnderlyingType(ValueType));
	return 0;
}
...
size_t FAnimNextParamType::GetValueTypeSize() const
{
	...
	checkf(false, TEXT("Error: FParameterType::GetValueTypeSize: Unknown Type Container %d, Value %d"), EnumToUnderlyingType(ContainerType), EnumToUnderlyingType(ValueType));
	return 0;
}
...
size_t FAnimNextParamType::GetAlignment() const
{
	...
	checkf(false, TEXT("Error: FParameterType::GetAlignment: Unknown Type: Container %d, Value %d"), EnumToUnderlyingType(ContainerType), EnumToUnderlyingType(ValueType));
	return 0;
}
...
size_t FAnimNextParamType::GetValueTypeAlignment() const
{
	...
	checkf(false, TEXT("Error: FParameterType::GetValueTypeAlignment: Unknown Type: Container %d, Value %d"), EnumToUnderlyingType(ContainerType), EnumToUnderlyingType(ValueType));
	return 0;
}
...
FAnimNextParamType FAnimNextParamType::FromString(const FString& InString)
{
	auto GetInnerType = [](const FString& InTypeString, FAnimNextParamType& OutType)
	{
		...
		else if(InTypeString.StartsWith(TEXT("TEnumAsByte<U"), ESearchCase::CaseSensitive))
		{
			ObjectInnerString = InTypeString.RightChop(13).LeftChop(1).TrimStartAndEnd();
			ObjectValueType = EValueType::Enum;
		}

		if(UObject* ObjectType = FindFirstObjectSafe<UObject>(*ObjectInnerString, EFindFirstObjectOptions::NativeFirst))
		{
