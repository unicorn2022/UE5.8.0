...
namespace UE::UAF
{
	...
	void BlendWithIdentityAndAccumulatePerBoneWeight(const FTransformArraySoAView& Base, const FTransformArraySoAConstView& Additive,
		const TArrayView<const int32>& TransformIndexToWeightIndexMap, const TArrayView<const float>& BoneWeights, const float DefaultBlendWeight)
	{
		...
		for (int32 TransformIndex = 0; TransformIndex < NumTransforms; ++TransformIndex)
		{
			...
			const float ScaleWeight = BoneWeights.IsValidIndex(PerTransformIndex) ? BoneWeights[PerTransformIndex] : DefaultBlendWeight;

			const TransformVectorRegister VBlendWeight(ScalarRegister(ScaleWeight).Value);
			const TransformVectorRegister VOneMinusAlpha = VectorSubtract(VectorOne(), VBlendWeight);
	...
	void ConvertToAdditive(const FTransformArraySoAConstView& Input, const FTransformArraySoAConstView& Base, const FTransformArraySoAView& Output)
	{
		const int32 NumTransforms = Input.Num();
		...
		for (int32 TransformIndex = 0; TransformIndex < NumTransforms; ++TransformIndex)
		{
			Output.Rotations[TransformIndex] = Input[TransformIndex].GetRotation() * Base[TransformIndex].GetRotation().Inverse();
			Output.Translations[TransformIndex] = Input[TransformIndex].GetTranslation() - Base[TransformIndex].GetTranslation();
			...
			Output.Scales3D[TransformIndex] = Input[TransformIndex].GetScale3D() * FTransform::GetSafeScaleReciprocal(Base[TransformIndex].GetScale3D()) -1.0f; 
			
			Output.Rotations[TransformIndex].Normalize();
		}
	}
}
