...
void SVariablePickerCombo::Construct(const FArguments& InArgs)
{
	...
	FOnSubPropertyVariablePicked OnSubPropertyVariablePicked = InArgs._PickerArgs.OnSubPropertyVariablePicked;
	...
	PickerArgs.OnSubPropertyVariablePicked = FOnSubPropertyVariablePicked::CreateSPLambda(this, [this, OnSubPropertyVariablePicked](const FAnimNextSoftVariableReference& InTopVarRef, const TArray<FName>& InSubPropertyPath, const FAnimNextParamType& InLeafType)
	{
		FSlateApplication::Get().DismissAllMenus();

		// Forward to the original delegate
		OnSubPropertyVariablePicked.ExecuteIfBound(InTopVarRef, InSubPropertyPath, InLeafType);
