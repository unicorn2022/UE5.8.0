...
void FAnimNextEdGraphNodeCustomization::PopulateCategory(IDetailLayoutBuilder& DetailBuilder, const TSharedPtr<FTraitStackDetailsData>& TraitData)
{
	...
	VisitChildren(PropertyHandle.ToSharedRef(), [&TraitData](const TSharedRef<IPropertyHandle>& InPropertyHandle)
	{
		...
		InPropertyHandle->SetOnPropertyValueChangedWithData(TDelegate<void(const FPropertyChangedEvent&)>::CreateLambda([WeakTraitData, WeakPropertyHandle](const FPropertyChangedEvent& InPropertyChangedEvent)
		{
			...
			// Early out if the change type is interactive e.g. from dragging of a slier, to avoid recompilation spam
			// This will be followed by a ValueSet when finished
			if ((InPropertyChangedEvent.ChangeType & EPropertyChangeType::Interactive) != 0)
			{
				return;
			}	
			...
			PinnedPropertyHandle->EnumerateConstRawData([&PinnedTraitData, &PinnedPropertyHandle, Controller, &InPropertyChangedEvent](const void* InRawData, const int32 InDataIndex, const int32 InNumDatas)
			{
				...
				auto PropertyPathToPinPath = [](const TSharedPtr<IPropertyHandle>& InPropertyHandle, const TSharedPtr<FTraitStackDetailsData>& InTraitData, const UAnimNextEdGraphNode* InEdGraphNode)
				{
					// Transform property path into pin path
					FString PropertyPath(InPropertyHandle->GetPropertyPath());
					...
					const FString RootPropertyPath(InTraitData->RootPropertyHandle->GetPropertyPath());
					PinPath = PinPath.Replace(*RootPropertyPath, *InTraitData->Name.ToString());
					// Prefix node path
					const FString NodePath = InEdGraphNode->GetModelNodePath();
					const bool bIsTrait = InTraitData->Type == FCategoryDetailsData::EType::TraitStack;
					return bIsTrait ? (NodePath + TEXT(".") + InTraitData->Name.ToString() + TEXT(".") + PinPath) : (NodePath + TEXT(".") + PinPath);
				};

				const FString PinPath = PropertyPathToPinPath(PinnedPropertyHandle, PinnedTraitData, EdGraphNode.Get());
				if ((InPropertyChangedEvent.ChangeType & EPropertyChangeType::ResetToDefault) != 0)
				{
					Controller->ResetPinDefaultValue(PinPath);
				}
				else
				{
