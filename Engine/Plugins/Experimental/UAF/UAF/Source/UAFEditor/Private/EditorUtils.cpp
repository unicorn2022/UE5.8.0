...
void FUtils::GetAllEntryNames(const UUAFRigVMAssetEditorData* InEditorData, TSet<FName>& OutNames)
{
	for(const UUAFRigVMAssetEntry* Entry : InEditorData->GetAllEntries())
	{
...
bool FUtils::IsValidParameterNameForEntry(const UAnimNextVariableEntry* Entry, const FText& InNewName, FText& OutErrorText)
{
	if(Entry == nullptr)
	{
		OutErrorText = LOCTEXT("InvalidVariableError", "Variable is invalid");
		return false;
	}

	const UUAFRigVMAssetEditorData* EditorData = Entry->GetTypedOuter<UUAFRigVMAssetEditorData>();
	if(EditorData == nullptr)
	{
		OutErrorText = LOCTEXT("InvalidEditorDataError", "Variable has invalid editor data");
		return false;
	}

	const FString NewString = InNewName.ToString();
	if (!IsValidParameterNameString(NewString, OutErrorText))
	{
		return false;
	}
	...
	UUAFRigVMAssetEntry* ExistingEntry = EditorData->FindEntry(Name);
	if(ExistingEntry && ExistingEntry != Entry)
	{
		OutErrorText = LOCTEXT("NameExistsError", "Variable name already exists in this asset");
		return false;
	}
