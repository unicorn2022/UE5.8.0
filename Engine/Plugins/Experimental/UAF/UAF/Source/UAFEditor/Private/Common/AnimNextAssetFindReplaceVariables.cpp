...
{
	static UClass* Types[] = { UUAFRigVMAsset::StaticClass() };
	return Types;
}
...
bool UAnimNextAssetFindReplaceVariables::ShouldFilterOutAsset(const FAssetData& InAssetData, bool& bOutIsOldAsset) const
{
	...
	if (SharedWorkspaceEditor.IsValid())
	{
		if (CurrentSearchScope == ESearchScope::Asset)
		{
			...
			{
				if (InAssetData.GetSoftObjectPath() != FSoftObjectPath(FocusedObject->GetTypedOuter<UUAFRigVMAsset>()))
				{
					return true;
				}
			}	
...
{
	if (UUAFRigVMAsset* RigVMAsset = Cast<UUAFRigVMAsset>(InAssetData.GetAsset()))
	{
		if (UUAFRigVMAssetEditorData* EditorData = UE::UAF::UncookedOnly::FUtils::GetEditorData<UUAFRigVMAssetEditorData>(RigVMAsset))
		{
	...
	auto VariableScopeFilter = FOnFilterVariable::CreateLambda([this](const FAnimNextSoftVariableReference& InVariableReference)
	{
      	...
      	if (SharedWorkspaceEditor.IsValid())
      	{
      		...
      		if (CurrentSearchScope == ESearchScope::Asset)
      		{
      			...
      			TArray<const UUAFRigVMAsset*> AssetsInExportChain;
      			Document.Export.GetAssetsOfType<const UUAFRigVMAsset>(AssetsInExportChain, true);
  
      			auto ProcessAsset = [&AssetsInExportChain](const UUAFRigVMAsset* InAsset)
      			{
      				if (InAsset)
      				{
      					const UUAFRigVMAssetEditorData* EditorData = UE::UAF::UncookedOnly::FUtils::GetEditorData<const UUAFRigVMAssetEditorData,const UUAFRigVMAsset>(InAsset);
      					EditorData->ForEachEntryOfType<UUAFSharedVariablesEntry>([&AssetsInExportChain](const UUAFSharedVariablesEntry* VariablesEntry)
      					{
      			...
      			{
      				const UUAFRigVMAsset* Asset = AssetsInExportChain[AssetIndex];
	...
	
	{
		...
		ReplaceArgs.OnFilterVariableType  = FOnFilterVariableType::CreateLambda([this](const FAnimNextParamType& InParamType)-> EFilterVariableResult
		{
			if(SearchType.IsValid())
			{
				...
				if(!UE::UAF::FParamUtils::GetCompatibility(SearchType, InParamType).IsCompatibleWithDataLoss())
				{
...
{
	SearchReference = FAnimNextSoftVariableReference::FromName(InVariableEntry->GetVariableName(), InVariableEntry->GetTypedOuter<UUAFRigVMAsset>());;
