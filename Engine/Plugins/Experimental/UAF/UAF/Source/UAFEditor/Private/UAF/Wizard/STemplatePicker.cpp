...
namespace UE::UAF::Editor
{
								...
								.OnGenerateRow_Lambda([this](TSharedPtr<FAssetMetadata> InAsset, const TSharedRef<STableViewBase>& InTableView)
								{
									...
									[
										...
										[
											...
											[
												...
												[
													...
													.Text(InAsset->AssetData.GetClass()->GetDisplayNameText())
												]
											]
										]
										+ SHorizontalBox::Slot()
										...
										[
											...
											.OnVerifyTextChanged_Lambda([this, InAsset](const FText& InNewText, FText& OutErrorMessage)
											{
												...
												{
													OutErrorMessage = LOCTEXT("AssetAlreadyExists", "Name clashes with an existing asset");
													return false;
												}

												for (TSharedPtr<FAssetMetadata> OtherAsset : IncludedAssets)
												{
													if (InAsset != OtherAsset && OtherAsset->AssetRename.Equals(InNewText.ToString()))
													{
														OutErrorMessage = LOCTEXT("AssetNameClash", "Name clashes with another template asset");
														return false;
													}
												}

												if (TemplateConfig->BlueprintMode == ETemplateBlueprintMode::CreateNewBlueprint && TemplateConfig->BlueprintAssetName.Equals(InNewText.ToString()))
												{
													OutErrorMessage = LOCTEXT("AssetNameClashWithBlueprint", "Name clashes with blueprint asset name");
													return false;
												}
	...
	void STemplatePicker::UpdateUserAssetNames()
	{
		UEditorAssetSubsystem* EditorAssetSubsystem = GEditor->GetEditorSubsystem<UEditorAssetSubsystem>();

		const auto GetNewUserAssetName = [&, this](const FString BaseAssetName)
			{
				if (!EditorAssetSubsystem->DoesAssetExist(FPaths::Combine(TemplateConfig->OutputPath.Path, BaseAssetName)))
				{
					return BaseAssetName;
				}

				FString OutAssetName = BaseAssetName;
				int32 Suffix = 1;

				while (EditorAssetSubsystem->DoesAssetExist(FPaths::Combine(TemplateConfig->OutputPath.Path, OutAssetName)))
				{
					OutAssetName = BaseAssetName + FString::FromInt(Suffix++);
				}
		...
		if (SelectedTemplate.IsValid())
		{
			...
			TemplateConfig->BlueprintAssetName = GetNewUserAssetName(SelectedTemplate->Template->DefaultBlueprintAssetName);
			...
			for (const TObjectPtr<UObject>& Asset : SelectedTemplate->Template->Assets)
			{
				...
				AssetMetadata->AssetRename = GetNewUserAssetName(BaseAssetName);
	...
	bool STemplatePicker::CanCreateAssets() const
	{
		...
		case ETemplateBlueprintMode::CreateNewBlueprint:
			if (TemplateConfig->BlueprintAssetName.IsEmpty())
			{
				ErrorText = LOCTEXT("InvalidBlueprintAssetName", "Invalid blueprint asset name");
				return false;
			}
			if (TemplateConfig->BlueprintClass.Get() == nullptr)
			{
				ErrorText = LOCTEXT("InvalidBlueprintClass", "Invalid blueprint class");
				return false;
			}
			if (TemplateConfig->SkeletalMesh.IsNull())
			{
				ErrorText = LOCTEXT("InvalidSkeletalMesh", "Invalid skeletal mesh");
				return false;
			}
			break;
		case ETemplateBlueprintMode::ModifyExistingBlueprint:
			if (TemplateConfig->BlueprintToModify.IsNull())
			{
				ErrorText = LOCTEXT("InvalidBlueprintToModify", "Invalid blueprint to modify");
				return false;
			}
			break;
		}
		...
		{
			for (TSharedPtr<FAssetMetadata> Asset : IncludedAssets)
			{
				if (Asset->AssetRename.Equals(TemplateConfig->BlueprintAssetName))
				{
					ErrorText = LOCTEXT("InvalidAssetNames", "Blueprint asset name clashes with template asset name");
					return false;
				}
			}

			UEditorAssetSubsystem* EditorAssetSubsystem = GEditor->GetEditorSubsystem<UEditorAssetSubsystem>();
			if (EditorAssetSubsystem->DoesAssetExist(FPaths::Combine(TemplateConfig->OutputPath.Path, TemplateConfig->BlueprintAssetName)))
			{
				ErrorText = LOCTEXT("ClashingBlueprintAssetName", "Blueprint asset name clashes with an existing asset");
				return false;
			}
		}
