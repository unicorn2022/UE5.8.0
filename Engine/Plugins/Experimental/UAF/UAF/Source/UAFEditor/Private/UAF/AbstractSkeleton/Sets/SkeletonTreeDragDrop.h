	...
	public:
		...
		TSharedPtr<SWidget> GetDefaultDecorator() const override
		{
			return SNew(SBorder)
				...
				[
					SNew(STextBlock)
						.Text(Items.Num() == 1 ? FText::FromName(Items[0]->BoneName) :	FText::Format(NSLOCTEXT("UE::UAF::Editor::FSkeletonTreeDragDropOp", "MultipleBonesTooltip", "{0} bones"), FText::AsNumber(Items.Num())))
				];
		}
