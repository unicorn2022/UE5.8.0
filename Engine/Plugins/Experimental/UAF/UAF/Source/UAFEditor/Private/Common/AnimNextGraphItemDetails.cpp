...
{
	TMap<UUAFRigVMAssetEditorData*, TArray<UUAFRigVMAssetEntry*>> EntriesToDelete;
	for(const FWorkspaceOutlinerItemExport& Export : Exports)
	{
		...
		if (Data.IsValid() && Data.GetScriptStruct() == FAnimNextGraphOutlinerData::StaticStruct())
		{
			...
			UUAFRigVMAssetEditorData* EditorData = GraphData.GetEntry()->GetTypedOuter<UUAFRigVMAssetEditorData>();
			if(EditorData == nullptr)
			{
				continue;
			}
			UUAFRigVMAssetEntry* Entry = GraphData.GetEntry();
			if(Entry == nullptr)
			{
				continue;
			}
			TArray<UUAFRigVMAssetEntry*>& Entries = EntriesToDelete.FindOrAdd(EditorData);
			Entries.Add(Entry);
		}
	}
	...
	if(EntriesToDelete.Num() > 0)
	{
		...
		for(TPair<UUAFRigVMAssetEditorData*, TArray<UUAFRigVMAssetEntry*>> EntryToDeletePair : EntriesToDelete)
		{
...
void FAnimNextGraphItemDetails::Rename(const FWorkspaceOutlinerItemExport& Export, const FText& InName) const
{
	...
	if (Data.IsValid() && Data.GetScriptStruct() == FAnimNextGraphOutlinerData::StaticStruct())
	{
		...
		UUAFRigVMAssetEntry* ExistingEntry = EditorData->FindEntry(NewName);
...
bool FAnimNextGraphItemDetails::ValidateName(const FWorkspaceOutlinerItemExport& Export, const FText& InName, FText& OutErrorMessage) const
{
	...
	if (Data.IsValid() && Data.GetScriptStruct() == FAnimNextGraphOutlinerData::StaticStruct())
	{
		const FAnimNextGraphOutlinerData& GraphData = Export.GetData().Get<FAnimNextGraphOutlinerData>();
		UUAFRigVMAssetEditorData* EditorData = GraphData.GetEntry()->GetTypedOuter<UUAFRigVMAssetEditorData>();
		UUAFRigVMAssetEntry* ExistingEntry = EditorData->FindEntry(FName(*InName.ToString()));
