...
#define LOCTEXT_NAMESPACE "UE::UAF::Editor::SSetsTab"
		...
		TabManager->RegisterTabSpawner(Tabs::SkeletonTreeId, FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs& Args)
			{
				return SNew(SDockTab)
					.CanEverClose(false)
					[
						SAssignNew(SkeletonTreeWidget, SSetsSkeletonTree, SetBinding)
					];
			}))
		...
		TabManager->RegisterTabSpawner(Tabs::AttributesId, FOnSpawnTab::CreateLambda([this](const FSpawnTabArgs& Args)
			{
				return SNew(SDockTab)
					.CanEverClose(false)
					[
						SAssignNew(AttributesListWidget, SAttributesList, SetBinding)
					];
			}))
						...
						.OnObjectChanged_Lambda([this](const FAssetData& AssetData)
							{
								if (SetBinding.IsValid())
								{
									const FScopedTransaction Transaction(LOCTEXT("SetSetCollection", "Set Set Collection"));
									SetBinding->Modify();
						...
						.OnObjectChanged_Lambda([this](const FAssetData& AssetData)
							{
								if (SetBinding.IsValid())
								{
									const FScopedTransaction Transaction(LOCTEXT("SetBindingSkeleton", "Set Binding Skeleton"));
									SetBinding->Modify();
