...
#define LOCTEXT_NAMESPACE "UE::UAF::Editor::SSetsSkeletonTree"

namespace UE::UAF::Editor
{
	void SSetsSkeletonTree::Construct(const FArguments& InArgs, TWeakObjectPtr<UAbstractSkeletonSetBinding> InSetBinding)
	{
		SetBinding = InSetBinding;

		if (SetBinding.IsValid())
		{
			OnBindingsChangedHandle = SetBinding->RegisterOnBindingsChanged(FSimpleMulticastDelegate::FDelegate::CreateSP(this, &SSetsSkeletonTree::HandleBindingsChanged));
		}
				...
				SNew(SVerticalBox)
					...
					[
						SAssignNew(TreeView, STreeView<TSharedPtr<FTreeItem>>)
							...
							.OnContextMenuOpening(this, &SSetsSkeletonTree::TreeView_OnContextMenuOpening)
	...
	void SSetsSkeletonTree::Tick(const FGeometry& AllocatedGeometry, const double InCurrentTime, const float InDeltaTime)
	{
		if (bTreeDirty)
		{
			RepopulateTreeData();
	...
	FReply SSetsSkeletonTree::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
	{
	...
	void SSetsSkeletonTree::PostUndo(bool bSuccess)
	{
		RepopulateTreeData();
	}

	void SSetsSkeletonTree::PostRedo(bool bSuccess)
	{
		RepopulateTreeData();
	}
			...
			.OnAcceptDrop_Lambda([this](const FDragDropEvent& DragDropEvent, EItemDropZone DropZone, TSharedPtr<FTreeItem> TargetTreeItem) -> FReply
				{
					...
					if (DragDropOp.IsValid())
					{
						...
						// TODO: Use all dragged sets
						check(!DragDropOp->GetDraggedSets().IsEmpty());
						const bool bSuccess = SetBinding->AddBoneToSet(TargetSetTreeItem->BoneName, DragDropOp->GetDraggedSets()[0].SetName);
						ensure(bSuccess);
	...
	TSharedPtr<SWidget> SSetsSkeletonTree::TreeView_OnContextMenuOpening()
	{
		...
		MenuBuilder.AddMenuEntry(Commands.SelectWithChildren);

		return MenuBuilder.MakeWidget();
	}

	TArray<TSharedPtr<SSetsSkeletonTree::FTreeItem>> SSetsSkeletonTree::GetAllDescendentTreeItems(const TArray<TSharedPtr<FTreeItem>> ParentItems)
	{
		TArray<TSharedPtr<FTreeItem>> AllItems;
		AllItems.Append(ParentItems);
	...
	void SSetsSkeletonTree::RepopulateTreeData()
	{
		...
		for (const TSharedPtr<FTreeItem>& TreeItem : GetAllDescendentTreeItems(RootItems))
		{
		...
		for (TSharedPtr<FTreeItem> TreeItem : GetAllDescendentTreeItems(RootItems))
		{
	...
	void SSetsSkeletonTree::BindCommands()
	{
		...
		CommandList->MapAction(Commands.UnbindSelection,
			FExecuteAction::CreateSP(this, &SSetsSkeletonTree::HandleUnbindSelection),
			FCanExecuteAction::CreateSP(this, &SSetsSkeletonTree::IsSelectionNonEmpty));

		CommandList->MapAction(Commands.SelectWithChildren,
			FExecuteAction::CreateSP(this, &SSetsSkeletonTree::HandleSelectWithChildren),
			FCanExecuteAction::CreateSP(this, &SSetsSkeletonTree::IsSelectionSingle));
	}

	void SSetsSkeletonTree::HandleBindingsChanged()
	{
		bTreeDirty = true;
	}

	void SSetsSkeletonTree::HandleUnbindSelection()
	{
		...
		{
			SetBinding->RemoveBoneFromSet(Selected->BoneName);
		}
	}

	void SSetsSkeletonTree::HandleSelectWithChildren()
	{
		// No transaction as selection is transient

		TArray<TSharedPtr<FTreeItem>> Selection = TreeView->GetSelectedItems();
		check(Selection.Num() == 1);

		TArray<TSharedPtr<FTreeItem>> NewSelection = GetAllDescendentTreeItems(Selection);
		TreeView->SetItemSelection(NewSelection, true);
	}

	bool SSetsSkeletonTree::IsSelectionNonEmpty() const
	{
		return TreeView->GetNumItemsSelected() > 0;
	}

	bool SSetsSkeletonTree::IsSelectionSingle() const
	{
