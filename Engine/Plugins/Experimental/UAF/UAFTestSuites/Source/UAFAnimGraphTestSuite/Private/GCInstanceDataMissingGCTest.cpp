...
namespace UE::UAF
{
	/** A test trait with a UObject instance property that incorrectly does not implement IGarbageCollection */
	struct FTestTrait_InstanceDataGCMissing : FBaseTrait, IUpdate
	{
		DECLARE_ANIM_TRAIT(FTestTrait_InstanceDataGCMissing, FBaseTrait)
		...
		virtual void PostUpdate(FUpdateTraversalContext& Context, const TTraitBinding<IUpdate>& Binding, const FTraitUpdateState& TraitState) const override
		{
			...
			{
				InstanceData->WeakAnimSequence = NewObject<UAnimSequence>(GetTransientPackage(), NAME_None);
			}
	...
	GENERATE_ANIM_TRAIT_IMPLEMENTATION(FTestTrait_InstanceDataGCMissing, TRAIT_INTERFACE_ENUMERATOR, NULL_ANIM_TRAIT_INTERFACE_ENUMERATOR, NULL_ANIM_TRAIT_EVENT_ENUMERATOR)
	#undef TRAIT_INTERFACE_ENUMERATOR
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FAnimationUAFRuntimeTest_InstanceDataGCMissing, "Animation.UAF.Runtime.GCInstanceDataMissingGCInterface", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

/*
 * Tests that a trait that has a UObject instance property that implements IGarbageCollection does not have its UObject GC'd.
 */
bool FAnimationUAFRuntimeTest_InstanceDataGCMissing::RunTest(const FString& InParameters)
{
	using namespace UE::UAF;

	{
		AUTO_REGISTER_ANIM_TRAIT(FTestTrait_InstanceDataGCMissing)
		...
		UE_RETURN_ON_ERROR(AnimationGraph != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to create animation graph");

		UUAFAnimGraph_EditorData* EditorData = UncookedOnly::FUtils::GetEditorData<UUAFAnimGraph_EditorData>(AnimationGraph);
		UE_RETURN_ON_ERROR(EditorData != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to find module editor data");
		...
		UE_RETURN_ON_ERROR(Controller != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to get RigVM controller");
		...
		UE_RETURN_ON_ERROR(MainEntryPointNode != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to find main entry point node");

		URigVMPin* BeginExecutePin = MainEntryPointNode->FindPin(GET_MEMBER_NAME_STRING_CHECKED(FRigUnit_AnimNextGraphRoot, Result));
		UE_RETURN_ON_ERROR(BeginExecutePin != nullptr && BeginExecutePin->GetDirection() == ERigVMPinDirection::Input, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to create entry point");
		...
		FString DisplayName;
		{
			...
			UE_RETURN_ON_ERROR(DecoratorStackNode != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to create trait stack node");
			...
			UE_RETURN_ON_ERROR(CppDecoratorStruct != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to get find Cpp trait static struct");

			const FTrait* Trait = FTraitRegistry::Get().Find(FTestTrait_InstanceDataGCMissing::TraitUID);
			UE_RETURN_ON_ERROR(Trait != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to find test trait");

			UScriptStruct* ScriptStruct = Trait->GetTraitSharedDataStruct();
			UE_RETURN_ON_ERROR(ScriptStruct != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to find trait shared data struct");
			...
			FString DefaultValue;
			{
				...
				UE_RETURN_ON_ERROR(CppDecoratorStructInstance.CanBeAddedToNode(DecoratorStackNode, nullptr), "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Trait cannot be added to trait stack node");
			...
			UE_RETURN_ON_ERROR(DecoratorName == DisplayName, TEXT("FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Unexpected trait name"));
		}

		// Set some values on our trait
		URigVMPin* DecoratorPin = DecoratorStackNode->FindPin(*DisplayName);
		UE_RETURN_ON_ERROR(DecoratorPin != nullptr, TEXT("FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Failed to find trait pin"));

		// For some reason a manual recompile is needed, otherwise the trait interface implementations are never run
		UE::UAF::UncookedOnly::Compilation::RequestAssetCompilation(AnimationGraph);
		...
		// so that FAnimNextGraphInstance.AddReferencedObjects is called. (This isn't strictly needed as it won't traverse
		// our test trait anyway as it hasn't implemented IGarbageCollection) but its to match parity with the other GC test
		...
		// Second update, the anim sequence should have been destroyed now
		UE::UAF::UpdateGraph(UpdateGraphContext);

		UE_RETURN_ON_ERROR(Messages.Num() == 2, "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Not enough messages received");
		AddErrorIfFalse(Messages[0] == TEXT("Valid"), "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Update 1: Test AnimSequence should be valid");
		AddErrorIfFalse(Messages[1] == TEXT("Invalid"), "FAnimationUAFRuntimeTest_InstanceDataGCMissing -> Update 2: Test AnimSequence should be invalid");
