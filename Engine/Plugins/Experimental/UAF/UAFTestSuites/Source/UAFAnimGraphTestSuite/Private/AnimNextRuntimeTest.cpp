...
namespace UE::UAF
{
	...
	{
		FNodeTemplateRegistry& NodeRegistry = FNodeTemplateRegistry::Get();

		OldTemplateBufferSize = NodeRegistry.TemplateBuffer.Num();
		OldTemplateUIDToHandleMap = NodeRegistry.TemplateUIDToHandleMap;
	}
	...
	FScopedClearNodeTemplateRegistry::~FScopedClearNodeTemplateRegistry()
	{
		...
		NodeRegistry.TemplateBuffer.SetNum(OldTemplateBufferSize, EAllowShrinking::No);
		NodeRegistry.TemplateUIDToHandleMap = OldTemplateUIDToHandleMap;
	}

	FScopedNewNodeTemplateRegistry::FScopedNewNodeTemplateRegistry()
	{
	...
	bool FTestUtils::LoadFromArchiveBuffer(UUAFAnimGraph& AnimationGraph, TArray<FNodeHandle>& NodeHandles, const TArray<uint8>& SharedDataArchiveBuffer)
	{
