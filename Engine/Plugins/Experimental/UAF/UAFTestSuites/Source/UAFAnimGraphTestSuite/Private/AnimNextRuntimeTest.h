...
namespace UE::UAF
{
	...
	// Captures the node template registry and restores its state when we go out of scope
	struct FScopedClearNodeTemplateRegistry final
	{
		UE_NONCOPYABLE(FScopedClearNodeTemplateRegistry);	// We disallow copy/move semantics

		UAFANIMGRAPHTESTSUITE_API FScopedClearNodeTemplateRegistry();
		UAFANIMGRAPHTESTSUITE_API ~FScopedClearNodeTemplateRegistry();

		// The size of the TemplateBuffer in the node registry when the scope began
		// We'll restore it to that size (it is append only) when the scope ends
		int32 OldTemplateBufferSize = 0;

		// The TemplateUIDToHandleMap in the node registry when the scope began
		// We'll restore it to this one when the scope ends
		TMap<uint32, FNodeTemplateRegistryHandle> OldTemplateUIDToHandleMap;
	};

	// Creates a temporary empty node template registry instance and swaps it for the current one until the end of the scope
	// Should only be used when testing the node registry itself
	struct FScopedNewNodeTemplateRegistry final
	{
		UE_NONCOPYABLE(FScopedNewNodeTemplateRegistry);	// We disallow copy/move semantics
