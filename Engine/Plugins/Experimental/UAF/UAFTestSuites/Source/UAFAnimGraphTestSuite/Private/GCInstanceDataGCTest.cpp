...
namespace UE::UAF
{
	/** A test trait with a UObject instance property that correctly implements IGarbageCollection */
	struct FTestTrait_InstanceDataGC : FBaseTrait, IUpdate, IGarbageCollection
	{
		DECLARE_ANIM_TRAIT(FTestTrait_InstanceDataGC, FBaseTrait)
		...
		virtual void PostUpdate(FUpdateTraversalContext& Context, const TTraitBinding<IUpdate>& Binding, const FTraitUpdateState& TraitState) const override
		{
			...
			{
				InstanceData->AnimSequence = NewObject<UAnimSequence>(GetTransientPackage(), NAME_None);
				InstanceData->WeakAnimSequence = InstanceData->AnimSequence;
			}
	...
	GENERATE_ANIM_TRAIT_IMPLEMENTATION(FTestTrait_InstanceDataGC, TRAIT_INTERFACE_ENUMERATOR, NULL_ANIM_TRAIT_INTERFACE_ENUMERATOR, NULL_ANIM_TRAIT_EVENT_ENUMERATOR)
	#undef TRAIT_INTERFACE_ENUMERATOR
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FAnimationUAFRuntimeTest_InstanceDataGC, "Animation.UAF.Runtime.GCInstanceDataWithGCInterface", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

/*
 * Tests that a trait that has a UObject instance property that incorrectly does not implement IGarbageCollection results in its UObject being GC'd.
 */
bool FAnimationUAFRuntimeTest_InstanceDataGC::RunTest(const FString& InParameters)
{
	using namespace UE::UAF;

	{
		AUTO_REGISTER_ANIM_TRAIT(FTestTrait_InstanceDataGC)
		...
		UE_RETURN_ON_ERROR(AnimationGraph != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to create animation graph");

		UUAFAnimGraph_EditorData* EditorData = UncookedOnly::FUtils::GetEditorData<UUAFAnimGraph_EditorData>(AnimationGraph);
		UE_RETURN_ON_ERROR(EditorData != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to find module editor data");
		...
		UE_RETURN_ON_ERROR(Controller != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to get RigVM controller");
		...
		UE_RETURN_ON_ERROR(MainEntryPointNode != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to find main entry point node");

		URigVMPin* BeginExecutePin = MainEntryPointNode->FindPin(GET_MEMBER_NAME_STRING_CHECKED(FRigUnit_AnimNextGraphRoot, Result));
		UE_RETURN_ON_ERROR(BeginExecutePin != nullptr && BeginExecutePin->GetDirection() == ERigVMPinDirection::Input, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to create entry point");
		...
		FString DisplayName;
		{
			...
			UE_RETURN_ON_ERROR(DecoratorStackNode != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to create trait stack node");
			...
			UE_RETURN_ON_ERROR(CppDecoratorStruct != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to get find Cpp trait static struct");

			const FTrait* Trait = FTraitRegistry::Get().Find(FTestTrait_InstanceDataGC::TraitUID);
			UE_RETURN_ON_ERROR(Trait != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to find test trait");

			UScriptStruct* ScriptStruct = Trait->GetTraitSharedDataStruct();
			UE_RETURN_ON_ERROR(ScriptStruct != nullptr, "FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to find trait shared data struct");
			...
			FString DefaultValue;
			{
				...
				UE_RETURN_ON_ERROR(CppDecoratorStructInstance.CanBeAddedToNode(DecoratorStackNode, nullptr), "FAnimationUAFRuntimeTest_InstanceDataGC -> Trait cannot be added to trait stack node");
			...
			UE_RETURN_ON_ERROR(DecoratorName == DisplayName, TEXT("FAnimationUAFRuntimeTest_InstanceDataGC -> Unexpected trait name"));
		}

		// Set some values on our trait
		URigVMPin* DecoratorPin = DecoratorStackNode->FindPin(*DisplayName);
		UE_RETURN_ON_ERROR(DecoratorPin != nullptr, TEXT("FAnimationUAFRuntimeTest_InstanceDataGC -> Failed to find trait pin"));
		...
		// so that FAnimNextGraphInstance.AddStructReferencedObjects is called. (This isn't strictly needed as it won't traverse
		...
		UE_RETURN_ON_ERROR(Messages.Num() == 2, "FAnimationUAFRuntimeTest_InstanceDataGC -> Not enough messages received");
		AddErrorIfFalse(Messages[0] == TEXT("Valid"), "FAnimationUAFRuntimeTest_InstanceDataGC -> Update 1: Test AnimSequence should be valid");
		AddErrorIfFalse(Messages[1] == TEXT("Valid"), "FAnimationUAFRuntimeTest_InstanceDataGC -> Update 2: Test AnimSequence should be valid");
