...
bool FAnimationAnimNextRuntimeTest_GraphExecute::RunTest(const FString& InParameters)
{
...

	{
		...
		UE_RETURN_ON_ERROR(Messages.Num() == 9, "FAnimationAnimNextRuntimeTest_GraphExecute - > Unexpected message count");
...
bool FAnimationAnimNextRuntimeTest_GraphExecuteLatent::RunTest(const FString& InParameters)
{
...

	{
		...
		UE_RETURN_ON_ERROR(Messages.Num() == 9, "FAnimationAnimNextRuntimeTest_GraphExecuteLatent - > Unexpected message count");
...
bool FAnimationAnimNextRuntimeTest_Variables::RunTest(const FString& InParameters)
{
...

	{
		...
		auto AddPublicVariable = [this](UUAFAnimGraph* InAnimationGraph, FName InName, const auto& InValue)
		{
			using ValueType = std::remove_reference_t<decltype(InValue)>;
			FAnimNextParamType Type = FAnimNextParamType::GetType<ValueType>();
			UAnimNextVariableEntry* VariableEntry = UUAFRigVMAssetLibrary::AddVariable(InAnimationGraph, InName, Type.GetValueType(), Type.GetContainerType(), Type.GetValueTypeObject(), TEXT(""), false, false);
		...
		AddPublicVariable(AnimationGraph, "Object", UUAFSharedVariables::StaticClass()->GetDefaultObject<UUAFSharedVariables>());
		AddPublicVariable(AnimationGraph, "SoftObject", FSoftObjectPath(UUAFSharedVariables::StaticClass()->GetDefaultObject<UUAFSharedVariables>()));
		AddPublicVariable(AnimationGraph, "Class", UUAFSharedVariables::StaticClass());
		AddPublicVariable(AnimationGraph, "SoftClass", FSoftClassPath(UUAFSharedVariables::StaticClass()));

		TSharedPtr<FAnimNextGraphInstance> GraphInstance = AnimationGraph->AllocateInstance();
		UE_RETURN_ON_ERROR(GraphInstance.IsValid(), "Failed to allocate animation graph instance");

		// Bool/Integers + conversions
		{
			// Gets
			bool TestBool = false;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Bool", AnimationGraph), TestBool) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestBool == true, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			uint8 TestByte = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Byte", AnimationGraph), TestByte) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestByte == 42, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			int32 TestInt32 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int32", AnimationGraph), TestInt32) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestInt32 == -4679222, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			int64 TestInt64 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int64", AnimationGraph), TestInt64) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			...
			// Bool
			TestByte = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Bool", AnimationGraph), TestByte) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestByte == 1, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestInt32 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Bool", AnimationGraph), TestInt32) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestInt32 == 1, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestInt64 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Bool", AnimationGraph), TestInt64) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestInt64 == 1, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			// Byte
			TestBool = false;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Byte", AnimationGraph), TestBool) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestBool == true, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestInt32 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Byte", AnimationGraph), TestInt32) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestInt32 == 42, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestInt64 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Byte", AnimationGraph), TestInt64) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestInt64 == 42, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			// Int32
			TestBool = false;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int32", AnimationGraph), TestBool) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestBool == true, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestByte = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int32", AnimationGraph), TestByte) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestByte == 202/*(uint8)-4679222*/, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestInt64 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int32", AnimationGraph), TestInt64) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestInt64 == -4679222, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			// Int64
			TestBool = false;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int64", AnimationGraph), TestBool) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestBool == true, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestByte = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int64", AnimationGraph), TestByte) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestByte == 255/*(uint8)-3415919105*/, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestInt32 = 0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Int64", AnimationGraph), TestInt32) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
		...
		// Float/double + conversions
		{
			...
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Float", AnimationGraph), TestFloat) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestFloat == 1.0f, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			double TestDouble = 0.0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Double", AnimationGraph), TestDouble) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestDouble == 1.0, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			// Conversions
			TestDouble = 0.0f;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Float", AnimationGraph), TestDouble) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestDouble == 1.0f, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			TestFloat = 0.0;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Double", AnimationGraph), TestFloat) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
		...
		UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Name", AnimationGraph), TestFName) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
		UE_RETURN_ON_ERROR(TestFName == "Test", "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

		FString TestString;
		UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("String", AnimationGraph), TestString) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
		...
		// Enums
		{
			...
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Enum", AnimationGraph), TestEnum) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			...
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Enum", AnimationGraph), MismatchedEnum) != EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable succeeded");
		}

		// Structs
		{
			FVector TestStruct = FVector::ZeroVector;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Struct", AnimationGraph), TestStruct) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			...
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Struct", AnimationGraph), TestDerived) != EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable succeeded");

			// Test conversion from derived -> base
			TestStruct = FVector::ZeroVector;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("DerivedStruct", AnimationGraph), TestStruct) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestStruct == FVector::OneVector, "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");
		}

		// Objects
		{
			UUAFSharedVariables* TestObject = nullptr;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Object", AnimationGraph), TestObject) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestObject == UUAFSharedVariables::StaticClass()->GetDefaultObject<UUAFSharedVariables>(), "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			// Test unrelated object fails
			UAnimSequence* TestAnimSequence = nullptr;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Object", AnimationGraph), TestAnimSequence) != EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable succeeded");

			// Test Derived -> Base succeeds
			UObject* BaseObject = nullptr;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Object", AnimationGraph), BaseObject) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(BaseObject == UUAFSharedVariables::StaticClass()->GetDefaultObject<UUAFSharedVariables>(), "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			// Test Base -> Derived fails
			UUAFAnimGraph* DerivedObject = nullptr;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Object", AnimationGraph), DerivedObject) != EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable succeeded");
		}

		FSoftObjectPath TestSoftObjectPath;
		UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("SoftObject", AnimationGraph), TestSoftObjectPath) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
		UE_RETURN_ON_ERROR(TestSoftObjectPath == FSoftObjectPath(UUAFSharedVariables::StaticClass()->GetDefaultObject<UUAFSharedVariables>()), "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

		// Classes
		{
			UClass* TestClass = nullptr;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Class", AnimationGraph), TestClass) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(TestClass == UUAFSharedVariables::StaticClass(), "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");


			// Test unrelated class fails
			TSubclassOf<UAnimSequence> TestUnrelatedSubclassOf;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Class", AnimationGraph), TestUnrelatedSubclassOf) != EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable succeeded");

			// Test Derived -> Base succeeds
			TSubclassOf<UObject> BaseSubclassOf;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Class", AnimationGraph), BaseSubclassOf) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
			UE_RETURN_ON_ERROR(BaseSubclassOf == UUAFSharedVariables::StaticClass(), "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");

			// Test Base -> Derived fails
			TSubclassOf<UUAFAnimGraph> DerivedSubclassOf;
			UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("Class", AnimationGraph), DerivedSubclassOf) != EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable succeeded");
		}

		FSoftClassPath TestSoftClassPath;
		UE_RETURN_ON_ERROR(GraphInstance->GetVariable(FAnimNextVariableReference::FromName("SoftClass", AnimationGraph), TestSoftClassPath) == EPropertyBagResult::Success, "FAnimationAnimNextRuntimeTest_Variables -> GetVariable failed");
		UE_RETURN_ON_ERROR(TestSoftClassPath == FSoftClassPath(UUAFSharedVariables::StaticClass()), "FAnimationAnimNextRuntimeTest_Variables -> Variable value did not match");
	}
