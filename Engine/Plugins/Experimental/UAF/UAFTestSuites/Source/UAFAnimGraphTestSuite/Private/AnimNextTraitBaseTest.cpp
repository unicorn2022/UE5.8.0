...
bool FAnimationAnimNextRuntimeTest_NodeTemplateRegistry::RunTest(const FString& InParameters)
{
	...
	FScopedNewNodeTemplateRegistry ScopedClearNodeTemplateRegistry;
...
bool FAnimationAnimNextRuntimeTest_NodeLifetime::RunTest(const FString& InParameters)
{
	using namespace UE::UAF;
	{
		...
		UFactory* AnimationGraphFactory = NewObject<UUAFAnimGraphFactory>();
		UUAFAnimGraph* AnimationGraph = CastChecked<UUAFAnimGraph>(AnimationGraphFactory->FactoryCreateNew(UUAFAnimGraph::StaticClass(), GetTransientPackage(), TEXT("TestAnimNextModule"), RF_Transient, nullptr, nullptr, NAME_None));
...
bool FAnimationAnimNextRuntimeTest_TraitSerialization::RunTest(const FString& InParameters)
{
...

	{
		...
		{
			FScopedNewNodeTemplateRegistry ScopedClearNodeTemplateRegistryForLoad;
...

	{
		...
		
		{
			FAnimGraphBuilderContext AnimGraphBuilderContext;
			{
				...
				Builder.AddTraitStructView(0, ETraitVariableMapping::All, TraitBaseRef0);
				Builder.AddTraitStructView(0, ETraitVariableMapping::All, TraitAddRef0);
				Builder.AddTraitStructView(0, ETraitVariableMapping::All, TraitNativeRef0);
			...
			// Clear out the node template registry to test registration on load
			{
				...
				const UUAFAnimGraph* AnimationGraph = AnimGraphBuilderContext.Build();
