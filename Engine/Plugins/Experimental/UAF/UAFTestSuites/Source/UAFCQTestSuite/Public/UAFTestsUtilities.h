...
namespace UAFTestsUtilities
{
	 ...
	 * @param InUnitNodeClassPath:  Path to the unit node class path, optional
	 * @return A pointer to the RigUnit node instance, or nullptr otherwise.
	 */
	UEdGraphNode* AddUnitNode(UEdGraph* InParentGraph, const FString& InScriptStructPath, TArray<UEdGraphPin*>& InFromPins, const FVector2f& InRigUnitLocation, const FString& InUnitNodeClassPath = TEXT(""));
	 ...
	 */
	URigVMLibraryNode* AddFunctionNode(UUAFRigVMAsset* InAnimNextAsset, const FString& InFunctionName = TEXT("NewFunction"));
	 ...
	 */
	UAnimNextVariableEntry* AddVariable(UUAFRigVMAsset* InAnimNextAsset, const FAnimNextParamType& InVariableType, const FString& InVariableName = TEXT("NewVariable"), const FString& InDefaultValue = TEXT(""));

	/**
	 * Adds a new shared variables instance to the AnimNext Asset specified.
	 *
	 * @param InAnimNextAsset  A pointer to the AnimNext asset instance.
	 * @param InSharedVariables:  A pointer to the AnimNext shared variables instance.
	 * @return A pointer to the object instance for the shared variables, or nullptr otherwise.
	 */
	 ...
	 */
	URigVMPin* AddPin(UUAFRigVMAsset* InAnimNextAsset, URigVMLibraryNode* InLibraryNode, ERigVMPinDirection InDirection, const FString& InCPPName = TEXT("Argument"), const FString& InCPPType = TEXT("bool"), const FName& InCPPTypeObjectPath = TEXT("None"), const FString& InDefaultValue = TEXT("False"));
	 ...
	 */
	URigVMCollapseNode* CollapseNodes(UUAFRigVMAsset* InAnimNextAsset, const TArray<FName>& InNodeNames, const FString& InCollapseNodeName = TEXT(""), bool InCollapseToFunction = false);
}
