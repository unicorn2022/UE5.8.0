...
{
	IMPLEMENT_SIMPLE_AUTOMATION_TEST(FTransformerTests_TransformAttribute, "Animation.UAF.ValueRuntime.Transformers.TransformAttribute", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

	bool FTransformerTests_TransformAttribute::RunTest(const FString& InParameters)
	{
		...
		SetBinding->AddAttributeToSet(FAnimationAttributeIdentifier(TEXT("AttrA"), INDEX_NONE, NAME_None, FTransformAnimationAttribute::StaticStruct()), "SetA");
		SetBinding->AddAttributeToSet(FAnimationAttributeIdentifier(TEXT("AttrB"), INDEX_NONE, NAME_None, FTransformAnimationAttribute::StaticStruct()), "SetB");
		SetBinding->AddAttributeToSet(FAnimationAttributeIdentifier(TEXT("AttrC"), INDEX_NONE, NAME_None, FTransformAnimationAttribute::StaticStruct()), "SetC");
		...
		AddInfo("Overwrite");
		{
			// 0.25 weight
			{
				FTransformerUnaryTest<FTransformAnimationAttribute, false> TransformTest;
				...
				TransformTest.IsExpectedResult = [&](TOptional<FTransformAnimationAttribute> Result) -> TOptional<FString>
					{
						if (!Result.IsSet())
						{
		...
		AddInfo("Accumulate");
		{
			// 0.25 weight
			{
				FTransformerBinaryTest<Transformers::FAccumulate, FTransformAnimationAttribute, EInplaceTransformationSupport::Allow> TransformTest;
				TransformTest.AttributeName = "AttrA";
		...
		AddInfo("Interpolate");
		{
			// 0.25 weight
			{
				FTransformerBinaryTest<Transformers::FInterpolate, FTransformAnimationAttribute, EInplaceTransformationSupport::Allow> TransformTest;
				TransformTest.AttributeName = "AttrA";
				TransformTest.InputA.Value = TransformValueA;
				TransformTest.InputB.Value = TransformValueB;
				TransformTest.ApplyTransformer = [](const FValueBundleHeap& InputAValues, const FValueBundleHeap& InputBValues, FValueBundleHeap& OutputValues)
		...
		AddInfo("Layer");
		{
			// Value in layer but not in base
			{
				// Weight 0.25
				{
					FTransformerBinaryTest<Transformers::FLayer, FTransformAnimationAttribute, EInplaceTransformationSupport::Allow> TransformTest;
					TransformTest.AttributeName = "AttrC";
					TransformTest.WeightB = 0.25f;
					TransformTest.InputA.Reset();
					TransformTest.InputB = FTransformAnimationAttribute({ TransformValueB });
					TransformTest.IsExpectedResult = [&](TOptional<FTransformAnimationAttribute> Result) -> TOptional<FString>
						{
							if (Result.IsSet())
							{
			...
			// Value in both base and layer
			{
				// Weight 0.25
				{
					...
					TransformTest.WeightB = 0.25f;
					TransformTest.InputA = FTransformAnimationAttribute({ TransformValueA });
					TransformTest.InputB = FTransformAnimationAttribute({ TransformValueB });
					TransformTest.IsExpectedResult = [&](TOptional<FTransformAnimationAttribute> Result) -> TOptional<FString>
						{
							if (!Result.IsSet())
		...
		{
			{
				FTransformerBinaryTest<Transformers::FMakeAdditiveSpace, FTransformAnimationAttribute, EInplaceTransformationSupport::Disallow> TransformTest;
				TransformTest.AttributeName = "AttrA";
				TransformTest.InputA.Value = TransformValueA;
				TransformTest.InputB.Value = TransformValueB;
				TransformTest.ApplyTransformer = [](const FValueBundleHeap& InputAValues, const FValueBundleHeap& InputBValues, FValueBundleHeap& OutputValues)
		...
		AddInfo("ApplyAdditiveSpace");
		{
			// 0.25 weight
			{
				FTransformerBinaryTest<Transformers::FMakeAdditiveSpace, FTransformAnimationAttribute, EInplaceTransformationSupport::Allow> TransformTest;
				TransformTest.AttributeName = "AttrA";
				TransformTest.InputA.Value = TransformValueA;
				TransformTest.InputB.Value = TransformValueB;
				TransformTest.ApplyTransformer = [](const FValueBundleHeap& InputAValues, const FValueBundleHeap& InputBValues, FValueBundleHeap& OutputValues)
				...
				TransformTest.IsExpectedResult = [&](TOptional<FTransformAnimationAttribute> Result) -> TOptional<FString>
					{
						...
						FTransform ExpectedValue = TransformValueA;
						FTransform::BlendFromIdentityAndAccumulate(ExpectedValue, TransformValueB, ScalarRegister(0.25f));
						if (!ExpectedValue.Equals(Result->Value))
						{
							return FString::Printf(TEXT("ApplyAdditiveSpace - 0.25 weight - expected %s, received %s"), *ExpectedValue.ToHumanReadableString(), *Result->Value.ToHumanReadableString());
						}
		...
		AddInfo("Sanitize");
		{
			FTransformerUnaryTest<FTransformAnimationAttribute, true> TransformTest;
			TransformTest.AttributeName = "AttrA";
			TransformTest.Input.Value = FTransform(FQuat(0.2f, 0.5f, 0.3f, 0.1f), FVector(100.0f, 50.0f, 25.0f), FVector(1.0f));
			TransformTest.ApplyTransformer = [](FValueBundleHeap& InputValues)
				{
					const auto& TransformerMap = FValueRuntimeRegistry::Get().GetTransformerMap();
