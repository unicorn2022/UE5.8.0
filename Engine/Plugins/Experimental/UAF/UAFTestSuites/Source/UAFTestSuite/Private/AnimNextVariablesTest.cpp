...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FVariables, "Animation.AnimNext.Variables.Variables", EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FVariables::RunTest(const FString& InParameters)
{
	...
	UUAFRigVMAsset* Asset = CastChecked<UUAFRigVMAsset>(Factory->FactoryCreateNew(UUAFSystem::StaticClass(), GetTransientPackage(), TEXT("TestAsset"), RF_Transient, nullptr, nullptr, NAME_None));
	...
	// Get event graph
	UAnimNextEventGraphEntry* EventGraph = Cast<UAnimNextEventGraphEntry>(EditorData->FindEntry(UUAFSystem_EditorData::DefaultEventGraphName));
	URigVMGraph* RigVMGraph = nullptr;
	URigVMNode* PrePhysicsEventNode = nullptr;
	
	if(EventGraph == nullptr)
	{
		EventGraph = EditorData->AddEventGraph(UUAFSystem_EditorData::DefaultEventGraphName, FRigUnit_AnimNextPrePhysicsEvent::StaticStruct());
...
bool FVariables_UOLBindings::RunTest(const FString& InParameters)
{
	...
	FAnimNextModuleInstance Instance(Asset, NewObject<UUAFComponent>(), EAnimNextModuleInitMethod::None);
	Instance.Initialize(FUAFSystemFactoryParams());
	FUAFRigVMComponent& RigVMComponent = Instance.GetOrAddComponent<FUAFRigVMComponent>();

	// Execute bindings to fetch data
	RigVMComponent.CallEventByName(FAnimNextModuleContextData(Instance, FRigUnit_AnimNextExecuteBindings_GT::EventName, 0.0f));

	// Run PrePhysics event to print messages
	RigVMComponent.CallEventByName(FAnimNextModuleContextData(Instance, FRigUnit_AnimNextPrePhysicsEvent::DefaultEventName, 0.0f));
...
namespace Private
{
	struct FTestReadWrite
	{
		...
		{
			const FAnimNextVariableReference Reference = FAnimNextVariableReference::FromName(FName(InMemberName), FAnimNextParamTypeTestStruct::StaticStruct());
			...
			if constexpr(std::is_same_v<Type, FTransform>)
			{
...
bool FVariables_Shared::RunTest(const FString& InParameters)
{
	...
	UUAFSharedVariablesEntry* Entry = EditorData->AddSharedVariablesStruct(FAnimNextParamTypeTestStruct::StaticStruct());
	UE_RETURN_ON_ERROR(Entry != nullptr, "FVariables_Shared -> Could not add shared variables struct.");

	FAnimNextModuleInstance Instance(Asset, EAnimNextModuleInitMethod::None);
	Instance.Initialize(FUAFSystemFactoryParams());
	...
	Private::FTestReadWrite::Test<TObjectPtr<UObject>>(*this, Instance, TEXT("Object"), UUAFRigVMAsset::StaticClass()->GetDefaultObject());
	Private::FTestReadWrite::Test<TObjectPtr<UClass>>(*this, Instance, TEXT("Class"), UUAFRigVMAsset::StaticClass());
	Private::FTestReadWrite::Test<TSubclassOf<UObject>>(*this, Instance, TEXT("SubclassOf"), UUAFRigVMAsset::StaticClass());
	// TODO: Soft obj/class ptrs not supported yet in RigVM
...
//	Private::FTestReadWrite::Test<TSoftClassPtr<UObject>>(*this, Instance, TEXT("SoftClassPtr"), UAnimNextRigVMAsset::StaticClass());
	...
	Private::FTestReadWrite::Test<TArray<TObjectPtr<UObject>>>(*this, Instance, TEXT("ObjectArray"), { UUAFRigVMAsset::StaticClass()->GetDefaultObject() });
	Private::FTestReadWrite::Test<TArray<TObjectPtr<UClass>>>(*this, Instance, TEXT("ClassArray"), { UUAFRigVMAsset::StaticClass() });
	Private::FTestReadWrite::Test<TArray<TSubclassOf<UObject>>>(*this, Instance, TEXT("SubclassOfArray"), { UUAFRigVMAsset::StaticClass() });
	// TODO: Soft obj/class ptrs not supported yet in RigVM
...
//	Private::FTestReadWrite::Test<TArray<TSoftClassPtr<UObject>>>(*this, Instance, TEXT("SoftClassPtrArray"), { UAnimNextRigVMAsset::StaticClass() });
	...
	bool bResult = Instance.AccessVariablesStruct<FAnimNextParamTypeTestStruct>([this](FAnimNextParamTypeTestStruct& InStruct)
	{
		...
		{
			// Specialize comparisons for types that dont have operator==
			if constexpr(std::is_same_v<T0, FTransform>)
			{
		...
		TestValue(TEXT("Object"), InStruct.Object, UUAFRigVMAsset::StaticClass()->GetDefaultObject());
		TestValue(TEXT("Class"), InStruct.Class, UUAFRigVMAsset::StaticClass());
		TestValue(TEXT("SubclassOf"), InStruct.SubclassOf, UUAFRigVMAsset::StaticClass());
		// TODO: Soft obj/class ptrs not supported yet in RigVM
