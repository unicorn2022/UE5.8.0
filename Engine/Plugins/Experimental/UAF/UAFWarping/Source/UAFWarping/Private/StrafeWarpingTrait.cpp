...
namespace UE::UAF
{
	...
	void FStrafeWarpingTrait::InitializeSpineData(TArrayView<FSpineBoneData> OutSpineBoneData, const TArray<FName>& SpineBoneNames, const FLODPoseStack& Pose)
	{
		...
		for (int32 i = 0; i < SpineBoneNames.Num(); i++)
		{
			...
			{
				OutSpineBoneData[i].LODBoneIndex = INVALID_BONE_INDEX;
			}
		}
...
void FAnimNextStrafeWarpingTask::DoWarpRootMotion(TUniquePtr<UE::UAF::FKeyframeState>& Keyframe, const FVector& RotationAxisVector, const FVector& TargetMoveDir, float& OutTargetOrientationAngleRad) const
{
	...
	{
		UE_LOGF(LogAnimNextWarping, Error, "FAnimNextStrafeWarpingTask::Execute, missing RootMotionProvider");
		return;
	}
...
void FAnimNextStrafeWarpingTask::DoWarpPose(TUniquePtr<UE::UAF::FKeyframeState>& Keyframe, const FVector& RotationAxisVector) const
{
	...
	{
		{
			...
			if (NumIKFootBones > 0)
			{
				for (int32 ArrayIndex = 0; ArrayIndex < NumIKFootBones; ArrayIndex++)
				{
					...
					if (LegTipIndex == INVALID_BONE_INDEX)
					{
						continue;
					}

					FTransform FootTargetTransform(ComponentSpaceTransforms.Rotations[LegTipIndex], ComponentSpaceTransforms.Translations[LegTipIndex]);
					...
					if (FVisualLogger::IsRecording())
					{
						...
						UE_VLOG_SPHERE(InstanceData->HostObject, "OrientationWarping", Display, InstanceData->RootBoneTransform.TransformPosition(FootTargetTransform.GetLocation()), DebugDrawSpehereRadius, FColor::Green, TEXT(""));
					}
#endif // ENABLE_VISUAL_LOG && ENABLE_ANIM_DEBUG
					...
					if (SharedData->FootMode == EStrafeWarpingFootMode::DoBasicIK)
					{
						...
						// Validate data
						if (LegRootIndex == INVALID_BONE_INDEX || LegMidIndex == INVALID_BONE_INDEX)
						{
							continue;
						}
						...
						if (FVisualLogger::IsRecording())
						{
							...
							FTransform FootWorldTransformTarget = FTransform(ComponentSpaceTransforms.Rotations[LegTipIndex], FootTargetTransform.GetLocation()) * InstanceData->RootBoneTransform;
							...
							UE_VLOG_ARROW(InstanceData->HostObject, "OrientationWarping", Display, LegMidTransformCS.GetLocation() + InstanceData->RootBoneTransform.GetLocation(), PoleTarget + InstanceData->RootBoneTransform.GetLocation(), FColor::Red, TEXT(""));
					...
					FBoneIndexType IKTargetIndex = Pose.FindLODBoneIndexFromBoneName(FootData.IKTargetBone);

					if (IKTargetIndex != INVALID_BONE_INDEX)
					{
						// Set IK target to equal leg tip in component space
						FBoneIndexType IKTargetParent = Pose.GetLODBoneParentIndex(IKTargetIndex);		// Usually the root bone but we can make it work for any parent
						if (IKTargetParent != INVALID_BONE_INDEX)
						{
							FTransform IKTargetCS = FootTargetTransform;
							FTransform IKTargetParentCS = ComponentSpaceTransforms[IKTargetParent];
							FTransform IKTargetLocal = IKTargetCS * IKTargetParentCS.Inverse();

							PoseTransforms[IKTargetIndex] = IKTargetLocal;
						}
					}
	...
	if (bUpdateSpineBones)
	{
		...
		for (int32 ArrayIndex = NumSpineBones - 1; ArrayIndex >= 0; ArrayIndex--)
		{
			...
			if (BoneData.LODBoneIndex == INVALID_BONE_INDEX || BoneData.Weight == 0.0f)
			{
				continue;
			}
