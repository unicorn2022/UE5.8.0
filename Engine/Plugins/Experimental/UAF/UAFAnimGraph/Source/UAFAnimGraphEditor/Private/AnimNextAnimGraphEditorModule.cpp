...
#include "AssetPreview/UAFGraphAssetPreview.h"
...
#include "Traits/UAFCallFunctionInfoDetails.h"
...
class FAnimNextAnimGraphEditorModule : public IModuleInterface
{
	...
	{
		AnimGraphTopLevelAssetPath = UUAFAnimGraph::StaticClass()->GetClassPathName();
		...
		PropertyModule.RegisterCustomPropertyTypeLayout(
			FUAFCallFunctionInfo::StaticStruct()->GetFName(),
			FOnGetPropertyTypeCustomizationInstance::CreateLambda([] { return MakeShared<FCallFunctionInfoDetails>(); }));
		PropertyModule.RegisterCustomPropertyTypeLayout(
		...
		FCoreDelegates::GetOnPostEngineInit().AddLambda([]()
		{
			if (GEditor)
			{
				GEditor->GetEditorSubsystem<UUAFAssetPreviewFactorySubsystem>()->AddAssetPreviewFactory(MakeShared<FUAFGraphAssetPreviewFactory>());
			}
		});
		...
		Workspace::FObjectDocumentArgs AnimNextAnimationGraphDocumentArgs(
			Workspace::FOnRedirectWorkspaceContext::CreateLambda([](const Workspace::FWorkspaceDocument& Document) -> UObject*
			{
				...
				UUAFAnimGraph* AnimationGraph = CastChecked<UUAFAnimGraph>(Document.GetObject());
		...
		WorkspaceEditorModule.RegisterObjectDocumentType(AnimGraphTopLevelAssetPath, AnimNextAnimationGraphDocumentArgs);

		WorkspaceEditorModule.RegisterViewportControllerFactory(UUAFAnimGraph::StaticClass(), []()
			{
...
/*		WorkspaceEditorModule.OnRegisterTabsForEditor().AddLambda([](FWorkflowAllowedTabSet& TabFactories, const TSharedRef<FTabManager>& InTabManager, TSharedPtr<UE::Workspace::IWorkspaceEditor> InEditorPtr)
		...
		AnimNextEditorModule.AddWorkspaceSupportedAssetClass(UUAFAnimGraph::StaticClass()->GetClassPathName());
	...
	virtual void ShutdownModule() override
	{
		...
		if(FModuleManager::Get().IsModuleLoaded("WorkspaceEditor"))
		{
			...
			WorkspaceEditorModule.UnregisterObjectDocumentType(AnimGraphTopLevelAssetPath);
			if(UObjectInitialized())
			{
				...
				WorkspaceEditorModule.UnregisterViewportControllerFactory(UUAFAnimGraph::StaticClass());
			}
		}
	...
	FTopLevelAssetPath AnimGraphTopLevelAssetPath;
};
