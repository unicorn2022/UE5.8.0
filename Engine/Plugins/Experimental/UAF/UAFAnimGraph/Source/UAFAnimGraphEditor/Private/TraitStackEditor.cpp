			...
			[
				...
				[
					...
					.Visibility_Lambda([this]() -> EVisibility
						{
							if (FSourceCodeNavigation::CanNavigateToStruct(SharedDataStruct))
							{
						...
						.OnClicked_Lambda([this]()
							{
								if (SharedDataStruct)
								{
									FSourceCodeNavigation::NavigateToStruct(SharedDataStruct);
								}
								return FReply::Handled();
							})
						.ToolTipText(FText::Format(LOCTEXT("GoToCode_ToolTip", "Click to open the trait shared data source file in {0}"), FSourceCodeNavigation::GetSelectedSourceCodeIDE()))
						.ContentPadding(0.0f)
						[
							SNew(SImage)
				...
				[
					...
					[
						...
						.ToolTipText(LOCTEXT("DeprecatedWarning", "This trait is deprecated. Please remove, replace or upgrade it to remove this warning."))
						.Visibility_Lambda([this]()
						{
							TSharedPtr<IPropertyHandle> PropertyHandle = WeakPropertyHandle.Pin();
							if (!PropertyHandle.IsValid())
							...
							return StructData.IsValid() && StructData->GetStruct() && StructData->GetStruct()->HasMetaData(TEXT("Deprecated")) ? EVisibility::Visible : EVisibility::Collapsed;
						})
					]
					+SHorizontalBox::Slot()
					.AutoWidth()
					...
					[
						...
						.OnGetMenuContent_Lambda([this]()
						{
							...
							if (PropertyHandle.IsValid())
							{
								...
								TSharedPtr<FStructOnScope> StructData = StructHandle->GetStructData();
								if (StructData.IsValid() && StructData->GetStruct() && StructData->GetStruct()->HasMetaData(TEXT("Deprecated")))
								{
									const FTrait* Trait = FTraitRegistry::Get().Find(CastChecked<UScriptStruct>(StructData->GetStruct()));
									check(Trait);

									FRigVMStructUpgradeInfo UpgradeInfo = Trait->GetUpgradeInfo();
									if (UpgradeInfo.IsValid())
									{
										MenuBuilder.AddMenuEntry(
											LOCTEXT("UpgradeLabel", "Upgrade"),
											LOCTEXT("UpgradeTooltip", "Upgrade this trait"),
											FSlateIcon(),
											FExecuteAction::CreateLambda([this, UpgradeInfo = MoveTemp(UpgradeInfo)]()
											{
												FSlateApplication::Get().DismissAllMenus();
												...
												FRigVMControllerCompileBracketScope CompileBracketScope(Controller);
												Controller->OpenUndoBracket(LOCTEXT("UpgradeTraitTransaction", "Upgrade Trait").ToString());

												for (TWeakObjectPtr<UAnimNextEdGraphNode> WeakNode : Nodes)
												{
													if (UAnimNextEdGraphNode* Node = WeakNode.Get())
													{
														Controller->UpgradeTraitByName(Node->GetModelNodeName(), StructData->GetStruct()->GetFName(), true, true);
													}
												}
