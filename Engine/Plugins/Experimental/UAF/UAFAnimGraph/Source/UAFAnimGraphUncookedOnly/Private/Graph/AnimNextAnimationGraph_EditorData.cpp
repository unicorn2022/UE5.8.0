...
namespace UE::UAF::UncookedOnly::Private
{
	...
	struct FTraitGraph
	{
		...
		explicit FTraitGraph(const UUAFAnimGraph* InAnimationGraph, URigVMNode* InRootNode)
			: RootNode(InRootNode)
		{
	...
	{
		URigVMGraph* Graph = TraitStack.DecoratorStackNode->GetGraph();
		check(Graph);
		IRigVMClientHost* ClientHost = Graph->GetImplementingOuter<IRigVMClientHost>();
		check(ClientHost);
		URigVMController* Controller = ClientHost->GetOrCreateController(Graph);
		check(Controller);
		...
		for (URigVMPin* Pin : Pins)
		{
			...
			{
				// If we contain child handles, we need to patch them to the correct source node/trait indices
				// Note patching the pins in this way here is only OK because we operate on a temporary intermediate copy graph, not the authored graph
				auto PatchDefaultValue = [&TraitStackNodes, Controller](URigVMPin* InPin)
				{
					// Trait handle pins don't have a value, just an optional link
					const TArray<URigVMLink*>& PinLinks = InPin->GetLinks();
					if (!PinLinks.IsEmpty())
					{
						...
						if (SourceNodeHandle.IsValid())
						{
							...
							Controller->SetPinDefaultValue(InPin, PropertyValue, false, false, false);
						}
					}
				};
				...
				{
					PatchDefaultValue(Pin);
				}
				else
				{
					const TArray<URigVMPin*>& SubPins = Pin->GetAllSubPinsRecursively();
					for (URigVMPin* SubPin : SubPins)
					{
						if (SubPin->GetCPPTypeObject() == FAnimNextTraitHandle::StaticStruct())
						{
							PatchDefaultValue(SubPin);
						}
					}
				}
	...
	bool ValidateTraitStack(const FRigVMCompileSettings& InSettings, const FTraitStackMapping& InMapping, URigVMUnitNode* InVMUnitNode)
	{
		...
		for (int32 TraitIndex = 0; TraitIndex < InMapping.TraitEntries.Num(); ++TraitIndex)
		{
			...
			const UScriptStruct* TraitStruct = Entry.Trait->GetTraitSharedDataStruct();
			if (InSettings.bWarnAboutDeprecatedNodes && TraitStruct->HasMetaData(TEXT("Deprecated")))
			{
				InSettings.ASTSettings.Report(EMessageSeverity::Warning, InVMUnitNode, FText::Format(LOCTEXT("DeprecatedTraitWarning", "@@: The trait '{0}' is deprecated. Please remove or upgrade the trait."), TraitStruct->GetDisplayNameText()).ToString());
			}
		...
		const FTraitStackMapping& TraitStack = InMapping;
		{
			for (const FTraitEntryMapping& TraitEntry : TraitStack.TraitEntries)
			{
				for (URigVMPin* Pin : TraitEntry.DecoratorEntryPin->GetSubPins())
				{
					...
					const bool bIsProgrammaticPin = Pin->IsProgrammaticPin();
					...
					if (bHasLinks || bIsProgrammaticPin)
					{
						...
						FString PinCPPType = Pin->GetCPPType();
						if (RigVMTypeUtils::IsUObjectType(PinCPPType))
						{
							const bool bImplementsGarbageCollection = TraitEntry.Trait->GetTraitInterface(IGarbageCollection::InterfaceUID) != nullptr;
							if (!bImplementsGarbageCollection)
							{
								const FText MissingInterfaceError = FText::Format(
									LOCTEXT("TraitStatusInStack_DoesNotImplementGarbageCollection", "@@: Trait is connected to a UObject but does not implement IGarbageCollection ({0} - {1})"),
									TraitEntry.Trait->GetTraitSharedDataStruct()->GetDisplayNameText(),
									FText::FromName(Pin->GetDisplayName()));
								InSettings.ASTSettings.Report(EMessageSeverity::Error, InVMUnitNode, MissingInterfaceError.ToString());
								bResult = false;
							}
						}
					}
	...
	FTraitGraph CollectGraphInfo(const FRigVMCompileSettings& InSettings, const UUAFAnimGraph* InAnimationGraph, const URigVMGraph* VMGraph, URigVMController* VMController)
	{
...
UUAFAnimGraph_EditorData::UUAFAnimGraph_EditorData()
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		IAssetRegistry& AssetRegistry = FModuleManager::Get().LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry")).Get();
		AssetRegistry.OnAssetRenamed().AddUObject(this, &UUAFAnimGraph_EditorData::HandleAssetRenamed);
	}
}

void UUAFAnimGraph_EditorData::OnPreCompileAsset(FRigVMCompileSettings& InSettings)
{
	...
	UUAFAnimGraph* AnimationGraph = FUtils::GetAsset<UUAFAnimGraph>(this);
...
void UUAFAnimGraph_EditorData::BuildFunctionHeadersContext(const FRigVMCompileSettings& InSettings, FAnimNextGetFunctionHeaderCompileContext& OutCompileContext) const
{
	using namespace UE::UAF::UncookedOnly;
...
void UUAFAnimGraph_EditorData::OnPreCompileProcessGraphs(const FRigVMCompileSettings& InSettings, FAnimNextProcessGraphCompileContext& OutCompileContext)
{
	using namespace UE::UAF::UncookedOnly;
	using namespace UE::UAF::UncookedOnly::Private;
	...
	for(URigVMGraph* SourceGraph : InOutGraphs)
	{
		...
		if(SourceGraph->GetSchemaClass() == UAnimNextAnimationGraphSchema::StaticClass())
		{
			...
			{
				FCompilerResultsLog& CompileLog = FCompilationScope::GetLogForObject(AnimationGraph);
				CompileLog.NotifyIntermediateObjectCreation(CreatedObjects.FindChecked(SourceNode), SourceNode);
			}
...
void UUAFAnimGraph_EditorData::OnCompileJobStarted()
{
	using namespace UE::UAF::UncookedOnly;
...
void UUAFAnimGraph_EditorData::OnCompileJobFinished()
{
	using namespace UE::UAF::UncookedOnly;
...
TConstArrayView<TSubclassOf<UUAFRigVMAssetEntry>> UUAFAnimGraph_EditorData::GetEntryClasses() const
{
...
bool UUAFAnimGraph_EditorData::CanAddNewEntry(TSubclassOf<UUAFRigVMAssetEntry> InClass) const
{
	// Prevent users adding more than one animation graph
	if(InClass == UAnimNextAnimationGraphEntry::StaticClass())
	{
		auto IsAnimNextGraphEntry = [](const UUAFRigVMAssetEntry* InEntry)
		{
			if (InEntry)
			{
...
UAnimNextAnimationGraphEntry* UAnimNextAnimationGraphLibrary::AddAnimationGraph(UUAFAnimGraph* InAsset, FName InName, bool bSetupUndoRedo, bool bPrintPythonCommand)
{
	return UE::UAF::UncookedOnly::FUtils::GetEditorData<UUAFAnimGraph_EditorData>(InAsset)->AddAnimationGraph(InName, bSetupUndoRedo, bPrintPythonCommand);
}

UAnimNextAnimationGraphEntry* UUAFAnimGraph_EditorData::AddAnimationGraph(FName InName, bool bSetupUndoRedo, bool bPrintPythonCommand)
{
	if(InName == NAME_None)
	{
		ReportError(TEXT("UUAFAnimGraph_EditorData::AddAnimationGraph: Invalid graph name supplied."));
		return nullptr;
	}
	...
	{
		ReportError(TEXT("UUAFAnimGraph_EditorData::AddAnimationGraph: Cannot add an animation graph to this asset - entry is not allowed."));
		return nullptr;
	}
...
TSubclassOf<UAssetUserData> UUAFAnimGraph_EditorData::GetAssetUserDataClass() const
{
	return UAnimNextAnimGraphWorkspaceAssetUserData::StaticClass();
}

void UUAFAnimGraph_EditorData::InitializeAssetUserData()
{
...
UClass* UUAFAnimGraph_EditorData::GetRigVMEdGraphSchemaClass() const
{
	return UUAFAnimGraphEdGraphSchema::StaticClass();
}

void UUAFAnimGraph_EditorData::HandleModifiedEvent(ERigVMGraphNotifType InNotifType, URigVMGraph* InGraph, UObject* InSubject)
{
...
void UUAFAnimGraph_EditorData::HandleAssetRenamed(const FAssetData& InAssetData, const FString& InOldName)
{
	for (URigVMGraph* Graph : GetAllModels())
	{
		if(UAnimNextController* Controller = Cast<UAnimNextController>(GetController(Graph)))
		{
			for (URigVMNode* Node : Graph->GetNodes())
			{
				if (UAnimNextTraitStackUnitNode* TraitStack = Cast<UAnimNextTraitStackUnitNode>(Node))
				{
					TraitStack->HandleAssetRenamed(Controller, InAssetData, InOldName);
				}
			}
		}
	}
