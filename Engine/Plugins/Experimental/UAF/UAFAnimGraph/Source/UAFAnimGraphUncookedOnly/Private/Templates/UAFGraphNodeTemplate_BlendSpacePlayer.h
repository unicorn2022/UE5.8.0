...
class UUAFGraphNodeTemplate_BlendSpacePlayer : public UUAFGraphNodeTemplate
{
	...
	virtual void HandleAssetDropped_Implementation(UAnimNextController* Controller, URigVMUnitNode* Node, UObject* Asset) const
	{
		...
		if (Asset)
		{
			...
			Controller->SetPinDefaultValue(TEXT("AnimNextBlendSpacePlayerTraitSharedData.bLoop"), LexToString(BlendSpace->bLoop), true, bSetupUndoRedo, true, true, true);
	...
	virtual void HandleAssetRenamed_Implementation(UAnimNextController* Controller, URigVMNode* Node, const FAssetData& AssetData, const FString& OldName) const
	{
		...
		URigVMPin* Pin = Node->FindPin(GET_PIN_PATH_STRING_CHECKED(FAnimNextBlendSpacePlayerTraitSharedData, BlendSpace));
