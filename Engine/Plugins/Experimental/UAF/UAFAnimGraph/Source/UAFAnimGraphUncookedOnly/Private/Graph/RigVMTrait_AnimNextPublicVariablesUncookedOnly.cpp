...
void FPublicVariablesImpl::GetProgrammaticPins(const FRigVMTrait_AnimNextPublicVariables& InTrait, URigVMController* InController, int32 InParentPinIndex, const FString& InDefaultValue, FRigVMPinInfoArray& OutPinArray)
{
	...
	UUAFRigVMAssetEditorData* EditorData = UE::UAF::UncookedOnly::FUtils::GetEditorData<UUAFRigVMAssetEditorData>(InTrait.InternalAsset.Get());
	if(EditorData == nullptr)
	{
		return;
	}

	TArray<UUAFRigVMAssetEditorData::FVariableInfo> PublicVariablesInfoArray;
	EditorData->GetAllVariables(PublicVariablesInfoArray, UUAFRigVMAssetEditorData::EVariableRecursion::SelfOnly, UUAFRigVMAssetEditorData::EVariableAccessFilter::PublicOnly);
	...
	{
		auto FindMatchingName = [&VariableName](const UUAFRigVMAssetEditorData::FVariableInfo& InVariableInfo)
		{
			return InVariableInfo.Name == VariableName;
		};

		if (UUAFRigVMAssetEditorData::FVariableInfo* PublicVariablePtr = PublicVariablesInfoArray.FindByPredicate(FindMatchingName))
		{
