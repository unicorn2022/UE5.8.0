...
public:
	...
	// Called in editor when a variable is drag/dropped. Called after ConfigureNewNode.
	UFUNCTION(BlueprintNativeEvent, Category = "UAF|Template")
	UE_API void HandleVariableDropped(UAnimNextController* Controller, URigVMUnitNode* Node, FAnimNextVariableReference Variable) const;
	UE_API virtual void HandleVariableDropped_Implementation(UAnimNextController* Controller, URigVMUnitNode* Node, FAnimNextVariableReference Variable) const;
	...
	// Called in editor when an asset is renamed, e.g. to be used to reconfigure the nodes appearance
	UFUNCTION(BlueprintNativeEvent, Category = "UAF|Template")
	UE_API void HandleAssetRenamed(UAnimNextController* Controller, URigVMNode* Node, const FAssetData& AssetData, const FString& OldName) const;
	...
	// Get all the types of variable this template handles for drag-drop
	TConstArrayView<FAnimNextParamType> GetDragDropVariableTypes() const
	{
		return DragDropVariableTypes;
	}
...
protected:
	...
	// Variable types that we handle for drag-drop
	UPROPERTY(EditAnywhere, BlueprintReadOnly, AssetRegistrySearchable, Category="Template")
