...
namespace UE::UAF
{
	namespace Private
	{
		...
		static FName GetMarkerName(int32 MarkerIndex, const TArrayView<const FTimelineSyncMarker>& SyncMarkers)
		{
		...
		static FSyncGroupPhasePosition CalculateSyncPosition(const FTimelineSyncMarkerArray& SyncMarkers, const FTimelineState& TimelineState, const FMarkerPair& PrevMarker, const FMarkerPair& NextMarker)
		{
			...
			{
				PrevTime = 0.0f;
				PhasePosition.PrevMarkerIndex = MarkerIndexSpecialValues::AnimationBoundary;
				PhasePosition.PrevMarkerName = NAME_None;
			}
			...
			{
				NextTime = Duration;
				PhasePosition.NextMarkerIndex = MarkerIndexSpecialValues::AnimationBoundary;
				PhasePosition.NextMarkerName = NAME_None;
			}

			if (PrevTime > NextTime)
			{
				// We looped, account for it
			...
			{
				// If the two markers overlap, space them apart by the duration to ensure the time between them isn't zero
				PrevTime -= Duration;
			}

			if (PrevTime > CurrentTime)
			{
				// Our current time isn't between the markers, we looped
		...
		static int32 GetNumLoopingMarkers(const FTimelineState& TimelineState, const TArrayView<const FTimelineSyncMarker>& SyncMarkers)
		{
			int32 NumSyncMarkers = SyncMarkers.Num();

			if (NumSyncMarkers != 0)
			{
				// Determine if our first and last markers match and repeat on the first/last keyframes in which case we don't have a phase in between

				const FTimelineSyncMarker& FirstMarker = SyncMarkers[0];
				const FTimelineSyncMarker& LastMarker = SyncMarkers.Last();
				if (FirstMarker.GetName() == LastMarker.GetName())
				{
					if (FirstMarker.GetPosition() == 0.0f && LastMarker.GetPosition() == TimelineState.GetDuration())
					{
						// We perfectly overlap, ignore our last marker
						NumSyncMarkers--;
					}
				}
			}

			return NumSyncMarkers;
		}

		// Returns whether or not two members have phases that can be matched
		static bool CanMatchPhase(
			const FTimelineState& TimelineStateA, const TArrayView<const FTimelineSyncMarker>& SyncMarkersA,
			const FTimelineState& TimelineStateB, const TArrayView<const FTimelineSyncMarker>& SyncMarkersB)
		{
			...
			// Identify the number of markers contained by a phase cycle
			// We only support phase matching if our cycles match
			// Imagine we have a marker sequence with: RLRL
			// Our phase cycle is: RL
			// We only consider simple cycles where the first marker matches the last and isn't present within the cycle
			// e.g. a cycle cannot contain the first marker, RLR would not be valid for example as part of RLRRLR
			// 
			// Once we've identified our cycle, we can determine whether or not two clips can phase match
			// There are 3 cases to consider:
			//   - Neither clip loops
			//   - One clip loops and the other doesn't
			//   - Both clips loop
			// 
			// We also have to consider looping clips where the last keyframe matches the first (we don't interpolate between)
			// This occurs and is necessary when root motion is used which means the first marker can be repeated: RLRLR

			if (!TimelineStateA.IsLooping() && !TimelineStateB.IsLooping())
			{
				// Neither are looping
				// Both must have a matching number of cycles(we don't support partial matching, e.g. RLRL and RLRLRL matching
				// in the first portion since the first would 'freeze' once it reaches the last marker if it doesn't loop)
				// Here we can also support partial cycles as long as both match (e.g. RLR and RLR)

				const int32 NumSyncMarkersA = SyncMarkersA.Num();
				const int32 NumSyncMarkersB = SyncMarkersB.Num();

				if (NumSyncMarkersA != NumSyncMarkersB)
				{
					return false;	// Cannot match if our marker count differs
				}

				if (NumSyncMarkersA == 0)
				{
					return false;	// Cannot match if we have no markers
				}

				for (int32 MarkerIdx = 0; MarkerIdx < NumSyncMarkersA; ++MarkerIdx)
				{
					if (SyncMarkersA[MarkerIdx].GetName() != SyncMarkersB[MarkerIdx].GetName())
					{
						// Markers do not match, we cannot phase match
						return false;
					}
				}

				// All our markers match
				return true;
			}
			else if (TimelineStateA.IsLooping() && TimelineStateB.IsLooping())
			{
				// Both are looping
				// They can phase match as long as the marker sequence loops properly over their combined length
				// If we have RL and RLRL, our combined length is our greatest common divisor: 4 markers
				// We'd get: RL RL and RLRL
				// If we have RLRL and RLRLRL, our combined length is our greatest common divisor: 12 markers
				// We'd get: RLRL RLRL RLRL and RLRLRL RLRLRL

				const int32 NumSyncMarkersA = GetNumLoopingMarkers(TimelineStateA, SyncMarkersA);
				const int32 NumSyncMarkersB = GetNumLoopingMarkers(TimelineStateB, SyncMarkersB);

				if (NumSyncMarkersA == 0 || NumSyncMarkersB == 0)
				{
					return false;	// Cannot match if we have no markers
				}

				const int32 NumTargetMarkers = FMath::GreatestCommonDivisor(NumSyncMarkersA, NumSyncMarkersB);
				for (int32 TargetMarkerIndex = 0; TargetMarkerIndex < NumTargetMarkers; ++TargetMarkerIndex)
				{
					const int32 MarkerIndexA = TargetMarkerIndex % NumSyncMarkersA;
					const int32 MarkerIndexB = TargetMarkerIndex % NumSyncMarkersB;

					if (SyncMarkersA[MarkerIndexA].GetName() != SyncMarkersB[MarkerIndexB].GetName())
					{
						// Markers do not match, we cannot phase match
						return false;
					}
				...
				// Markers are perfectly in sync
				return true;
			}
			else
			{
				// One of them is looping
				// The non-looping clip must have a matching multiple number of cycles meaning the looping
				// clip must fully loop a number of times (not partially) (e.g. RLRL (looping) and RLRLRL cannot phase match but RLRL (looping) and RLRLRLRL can)
				// This is because we do not track how many times we've looped and so if the shorter sequence is leading, we do not know how far into the second
				// sequence we should be which is problematic if the short leading sequence is the one looping

				const FTimelineState& LoopingTimelineState = TimelineStateA.IsLooping() ? TimelineStateA : TimelineStateB;
				const TArrayView<const FTimelineSyncMarker>& LoopingSyncMarkersList = TimelineStateA.IsLooping() ? SyncMarkersA : SyncMarkersB;
				const TArrayView<const FTimelineSyncMarker>& NonLoopingSyncMarkersList = TimelineStateA.IsLooping() ? SyncMarkersB : SyncMarkersA;

				const int32 NumLoopingMarkers = GetNumLoopingMarkers(LoopingTimelineState, LoopingSyncMarkersList);
				const int32 NumNonLoopingMarkers = NonLoopingSyncMarkersList.Num();

				if (NumNonLoopingMarkers == 0 || NumLoopingMarkers == 0)
				{
					return false;	// Cannot match if we have no markers
				}

				if ((NumNonLoopingMarkers % NumLoopingMarkers) != 0)
				{
					// Our looping clip isn't fully looping, we don't handle partial looping
					return false;
				}

				for (int32 NonLoopingMarkerIdx = 0; NonLoopingMarkerIdx < NumNonLoopingMarkers; ++NonLoopingMarkerIdx)
				{
					const int32 LoopingMarkerIndex = NonLoopingMarkerIdx % NumLoopingMarkers;

					if (NonLoopingSyncMarkersList[NonLoopingMarkerIdx].GetName() != LoopingSyncMarkersList[LoopingMarkerIndex].GetName())
					{
						// Markers do not match, we cannot phase match
						return false;
					}
		...
		static FSyncGroupPhasePosition FindMatchingPhaseTime(
			const FTimelineState& TimelineStateA, const TArrayView<const FTimelineSyncMarker>& SyncMarkersA, const FSyncGroupPhasePosition& SyncPositionA,
			const FTimelineState& TimelineStateB, const FTimelineSyncMarkerArray& SyncMarkersB)
		{
			// If we get here, we know we can phase match, @see CanMatchPhase above for details
			...
			{
				// Neither are looping
				check(SyncMarkersA.Num() == SyncMarkersB.Num());	// Invariant enforced in CanMatchPhase

				PrevMarkerIndex = SyncPositionA.PrevMarkerIndex;
				NextMarkerIndex = SyncPositionA.NextMarkerIndex;
			}
			else if (TimelineStateA.IsLooping() && TimelineStateB.IsLooping())
			{
				// Both are looping
				...
				check(NumSyncMarkersA != 0 && NumSyncMarkersB != 0);	// Invariant enforced in CanMatchPhase

				const int32 NumTargetMarkers = FMath::GreatestCommonDivisor(NumSyncMarkersA, NumSyncMarkersB);

				// TODO: Because we don't keep track of how many loop iterations we've performed, this doesn't quite work
				//       If the short loop is leading, when it wraps around we'll cause the longer loop to wrap around as well

				PrevMarkerIndex = SyncPositionA.PrevMarkerIndex % NumTargetMarkers;
				NextMarkerIndex = SyncPositionA.NextMarkerIndex % NumTargetMarkers;
			}
			else
			{
				// One of them is looping
				...
				check(NumNonLoopingMarkers != 0 && NumLoopingMarkers != 0);	// Invariant enforced in CanMatchPhase
				check((NumNonLoopingMarkers % NumLoopingMarkers) == 0);		// Invariant enforced in CanMatchPhase

				// TODO: Because we don't keep track of how many loop iterations we've performed, this doesn't quite work
				//       If the short loop is leading, when it wraps around we'll cause the longer loop to wrap around as well

				const int32 NumTargetMarkers = FMath::GreatestCommonDivisor(NumLoopingMarkers, NumNonLoopingMarkers);
			...
			check(GetMarkerName(SyncPositionA.PrevMarkerIndex, SyncMarkersA) == GetMarkerName(PrevMarkerIndex, SyncMarkersB));
			check(GetMarkerName(SyncPositionA.NextMarkerIndex, SyncMarkersA) == GetMarkerName(NextMarkerIndex, SyncMarkersB));
		...
		static void AdvanceLeaderTimeBased(FExecutionContext& Context, FSyncGroupContext& GroupContext)
		{
			...
			FTimelineState LeaderStartTimelineState;
			ensure(GroupLeaderContext.TimelineTrait.GetState(Context, LeaderStartTimelineState));
			...
			// Get our new state
			FTimelineState LeaderEndTimelineState;
			ensure(GroupLeaderContext.TimelineTrait.GetState(Context, LeaderEndTimelineState));
		...
		static void AdvanceLeaderMarkerBased(FExecutionContext& Context, FSyncGroupContext& GroupContext)
		{
			...
			if (GroupLeaderContext.State->bJustJoined && !GroupContext.State->bJustFormed)
			{
				if (GroupLeaderContext.State->GroupParameters.bMatchSyncPoint)
				{
					...
					const bool bCanMatchPhase = CanMatchPhase(
						GroupContext.State->PreviousLeaderTimelineState, GroupContext.State->PreviousLeaderSyncMarkers,
						LeaderStartTimelineState, GroupLeaderContext.SyncMarkers);
					...
					if (bCanMatchPhase)
					{
						...
						const FSyncGroupPhasePosition LeaderPhasePosition = FindMatchingPhaseTime(
							GroupContext.State->PreviousLeaderTimelineState, GroupContext.State->PreviousLeaderSyncMarkers, GroupContext.State->PreviousLeaderPhasePosition,
							LeaderStartTimelineState, GroupLeaderContext.SyncMarkers);
		...
		static void AdvanceFollowerTimeBased(FExecutionContext& Context, FSyncGroupContext& GroupContext, FSyncGroupMemberContext& GroupMemberContext)
		{
			...
			// Cache our starting timeline state
			FTimelineState FollowerStartTimelineState;
			ensure(GroupMemberContext.TimelineTrait.GetState(Context, FollowerStartTimelineState));
			...
			if (CDebugVarEnableSyncLog.GetValueOnAnyThread())
			{
				...
				FTimelineState FollowerEndTimelineState;
				ensure(GroupMemberContext.TimelineTrait.GetState(Context, FollowerEndTimelineState));
		...
		static void AdvanceFollowerMarkerBased(FExecutionContext& Context, FSyncGroupContext& GroupContext, FSyncGroupMemberContext& GroupMemberContext)
		{
			...
			const bool bCanMatchPhase = CanMatchPhase(
				GroupContext.LeaderTimelineState, GroupLeaderContext.SyncMarkers,
				FollowerStartTimelineState, GroupMemberContext.SyncMarkers);
			...
			if (GroupMemberContext.State->bJustJoined)
			{
				if (GroupMemberContext.State->GroupParameters.bMatchSyncPoint)
				{
					...
					if (bCanMatchPhase)
					{
						...
						const FSyncGroupPhasePosition FollowerPhasePosition = FindMatchingPhaseTime(
							GroupContext.LeaderTimelineState, GroupLeaderContext.SyncMarkers, GroupContext.LeaderPhaseStart,
							FollowerStartTimelineState, GroupMemberContext.SyncMarkers);
