	...
	const UE::UAF::FInjectionBlendSettings& BlendOutSettings)
{
	...
	{
		UE_LOGF(LogAnimation, Warning, "UInjectionCallbackProxy::Inject: Failed to inject - UAFComponent is not valid.");
		return false;
	}
	...
	PlayingRequest = UE::UAF::FInjectionUtils::Inject(AnimNextComponent, AnimNextComponent->GetSystemReference(), MoveTemp(RequestArgs), MoveTemp(LifetimeEvents));
