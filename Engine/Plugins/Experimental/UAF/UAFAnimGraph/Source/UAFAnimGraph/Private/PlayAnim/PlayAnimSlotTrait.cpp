...
namespace UE::UAF
{
	...
	void FPlayAnimSlotTrait::FPlayAnimSlotRequest::Initialize(FInjectionRequestPtr InRequest, const FInjectionBlendSettings& InBlendSettings, const UUAFAnimGraph* InAnimationGraph)
	{
		Request = InRequest;
	...
	void FPlayAnimSlotTrait::PreUpdate(FUpdateTraversalContext& Context, const TTraitBinding<IUpdate>& Binding, const FTraitUpdateState& TraitState) const
	{
		...
		for (int32 RequestIndex = 0; RequestIndex < NumSlotRequests; ++RequestIndex)
		{
			...
			FTimelineState ChildState;
			ensure(ChildTimelineTrait.GetState(Context, ChildState));
