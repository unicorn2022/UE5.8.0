		...
		*this << NumNodes;
		...
		for (uint32 NodeIndex = 0; NodeIndex < NumNodes; ++NodeIndex)
		{
			...
			*this << NodeTemplateIndex;
			...
			if (NodeTemplateIndex >= NumNodeTemplates)
			{
				// Corrupted / malformed data
				return EErrorState::InvalidData;
			}
			...
			// Align this node
			check(NodeTemplate->GetMinAlignment() > 0);
			SharedDataSize = Align(SharedDataSize, NodeTemplate->GetMinAlignment());

			if ((size_t)SharedDataSize > MAXIMUM_GRAPH_SHARED_DATA_SIZE)
			{
			...
			uint32 NodeSharedDataSize = NodeTemplate->GetNodeSharedDataSize();

			if ((size_t)SharedDataSize + (size_t)NodeSharedDataSize > MAX_uint32)
			{
				// Detect overrun of SharedDataSize
				return EErrorState::GraphTooLarge;
			}

			SharedDataSize += NodeSharedDataSize;
		}
		...
		for (uint32 NodeIndex = 0; NodeIndex < NumNodes; ++NodeIndex)
		{
			...
			if (SharedDataOffset >= SharedDataSize)
			{
				// Corrupted / malformed data
				return EErrorState::InvalidData;
			}
