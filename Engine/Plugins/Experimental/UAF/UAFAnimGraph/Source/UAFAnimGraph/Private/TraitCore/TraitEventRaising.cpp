...
namespace UE::UAF
{
	void RaiseTraitEvent(FExecutionContext& Context, const FTraitStackBinding& StackBinding, FAnimNextTraitEvent& Event)
	{
		if (!Event.IsValid())
		{
			return;	// Consumed events are invalid and do not propagate further
		}

		if (!StackBinding.IsValid())
		{
			UE_LOGF(LogAnimation, Warning, "RaiseTraitEvent: Could not raise trait event - The stack binding is invalid.");
			return;
		}
	...
	void RaiseTraitEvents(FExecutionContext& Context, const FTraitStackBinding& StackBinding, const FTraitEventList& EventList)
	{
		if (!StackBinding.IsValid())
		{
			UE_LOGF(LogAnimation, Warning, "RaiseTraitEvents: Could not raise trait events - The stack binding is invalid.");
			return;
		}
