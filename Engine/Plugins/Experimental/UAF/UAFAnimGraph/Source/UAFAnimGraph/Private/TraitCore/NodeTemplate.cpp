...
namespace UE::UAF
{
	...
	void FNodeTemplate::Finalize()
	{
		...
		uint32 MinAlignment32 = alignof(FNodeDescription);
		...
		for (uint32 TraitIndex = 0; TraitIndex < NumTraits_; ++TraitIndex)
		{
			...
			if (const FTrait* Trait = TraitRegistry.Find(TraitUID))
			{
				...
				MinAlignment32 = FMath::Max(MinAlignment32, MemoryLayout.SharedDataAlignment);
				MinAlignment32 = FMath::Max(MinAlignment32, MemoryLayout.InstanceDataAlignment);
			...
			check(MinAlignment32 <= MAX_uint8);
		...
		MinAlignment = static_cast<uint8>(MinAlignment32);
