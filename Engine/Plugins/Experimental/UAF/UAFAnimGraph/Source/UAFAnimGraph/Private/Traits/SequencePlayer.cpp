...
namespace UE::UAF
{
	...
	static bool IsSequenceLoopingHelper(const UAnimSequence* InAnimSequence, EAnimAssetLoopMode InLoopMode)
	{
		switch (InLoopMode)
		{
			case EAnimAssetLoopMode::Auto:
				return InAnimSequence ? InAnimSequence->bLoop : false;
	...
	void FSequencePlayerTrait::PreEvaluate(FEvaluateTraversalContext& Context, const TTraitBinding<IEvaluate>& Binding) const
	{
		...
		Task.bLooping = IsSequenceLoopingHelper(InstanceData->AnimSequence, SharedData->GetLoopMode(Binding));
	...
	FOnExtractRootMotionAttribute FSequencePlayerTrait::GetOnExtractRootMotionAttribute(FExecutionContext& Context, const TTraitBinding<IAttributeProvider>& Binding) const
	{
		...
		if (InstanceData->AnimSequence)
		{
			...
			FTimelineState State;
			ensure(TimelineTrait.GetState(Context, State));
	...
	bool FSequencePlayerTrait::GetState(const FExecutionContext& Context, const TTraitBinding<ITimeline>& Binding, FTimelineState& OutTimelineState) const
	{
		const FInstanceData* InstanceData = Binding.GetInstanceData<FInstanceData>();
		if (const UAnimSequence* AnimSeq = InstanceData->AnimSequence.Get())
		{
			...
			OutTimelineState = FTimelineState(
				InstanceData->InternalTimeAccumulator,
				AnimSeq->GetPlayLength(),
				SharedData->GetPlayRate(Binding) * AnimSeq->RateScale,
				IsSequenceLoopingHelper(AnimSeq, SharedData->GetLoopMode(Binding))).WithDebugName(AnimSeq->GetFName());
			return true;
		}
	...
	bool FSequencePlayerTrait::GetDelta(const FExecutionContext& Context, const TTraitBinding<ITimeline>& Binding, FTimelineDelta& OutTimelineDelta) const
	{
		const FInstanceData* InstanceData = Binding.GetInstanceData<FInstanceData>();
		if (const UAnimSequence* AnimSeq = InstanceData->AnimSequence.Get())
		{
			OutTimelineDelta = FTimelineDelta(
