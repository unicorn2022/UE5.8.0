...
namespace UE::UAF
{
	...
	void FTraitStackBinding::SnapshotLatentProperties(float DeltaTime, bool bIsFrozen, bool bJustBecameRelevant)
	{
		if (!IsValid())
		{
		...
		{
			GraphInstance.ExecuteLatentPins(TConstArrayView<FLatentPropertyHandle>(LatentHandles, NumLatentHandles), NodeInstance, DeltaTime, bIsFrozen, bJustBecameRelevant);
		}
	}
}
