...
namespace UE::UAF
{
	...
	FWeakTraitPtr IHierarchy::GetStackForwardingChild(const FExecutionContext& Context, const TTraitBinding<IHierarchy>& Binding) const
	{
		// By default, the 'forwarding child' is a singular child for the whole stack, and multi child stacks must override this function if they want to forward
		FChildrenArray Children;
		GetChildren(Context, Binding, Children);
