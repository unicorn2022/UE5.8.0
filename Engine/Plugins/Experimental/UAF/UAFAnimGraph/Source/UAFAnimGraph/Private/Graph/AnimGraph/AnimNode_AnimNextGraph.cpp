...
void FAnimNode_AnimNextGraph::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	if (!CVarAnimNextForceAnimBP.GetValueOnAnyThread() && GraphInstance.IsValid())
	{
...

		{
			...
			if (UE::UAF::UseAbstractSkeletonRuntime())
			{
                USkeletalMesh* SkeletalMesh = SkeletalMeshComponent->GetSkeletalMeshAsset();
                check(SkeletalMesh);
                
				// By default, use the everything set
				...
				UAbstractSkeletonSetBinding* Binding = UserData ? UserData->GetSetBinding() : nullptr;
				FName SetName;

				EvaluateGraphContext.SetSkeletonSet(Binding, SkeletalMesh, SetName);
			}
