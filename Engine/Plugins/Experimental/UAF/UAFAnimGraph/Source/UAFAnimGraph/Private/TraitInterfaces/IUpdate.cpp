...
#ifndef DO_UAF_GRAPH_LOOPGUARD
	#if (!(UE_BUILD_SHIPPING || UE_BUILD_TEST) || WITH_EDITOR)
		#define DO_UAF_GRAPH_LOOPGUARD 1
	#else
		#define DO_UAF_GRAPH_LOOPGUARD 0
	#endif
#endif

static int32 GMaxUAFGraphUpdateLoopCount = 1000;
static FAutoConsoleVariableRef CVarMaxUAFGraphUpdateLoopCount(
	TEXT("UAF.MaxGraphUpdateLoopCount"),
	GMaxUAFGraphUpdateLoopCount,
	TEXT("Sets the number of update loops before graph is considered in an infinite loop and will break out of update.\n"),
	ECVF_Default
);
...
namespace UE::UAF
{
	...
	TWeakObjectPtr<const USkeletalMeshComponent> FUpdateGraphContext::GetBindingObject() const
	{
	...
	void UpdateGraph(FUpdateGraphContext& UpdateGraphContext)
	{
...
#if DO_UAF_GRAPH_LOOPGUARD
		int32 ActiveUpdateLoopCount = 0;
#endif		
		while (Private::FUpdateEntry* Entry = TraversalContext.PopUpdateEntry())
		{
			...
			if (!Entry->bHasPreUpdated)
			{
				...
				Entry->TraitStack.SnapshotLatentProperties(DeltaTime, bIsFrozen, bJustBecameRelevant);
...
#if DO_UAF_GRAPH_LOOPGUARD
			// Check for loop count and break out if a potential endless loop is detected
			++ActiveUpdateLoopCount;
			if (!ensureMsgf(ActiveUpdateLoopCount < GMaxUAFGraphUpdateLoopCount, 
				TEXT("Potential endless loop detected in Graph [%s] of System [%s] - will break out of the update loop"),
				 *GraphInstance.GetAssetName().ToString(), GraphInstance.GetModuleInstance() ? *GraphInstance.GetModuleInstance()->GetAssetName().ToString() : TEXT("NONE")))
			{
				break;
			}
#endif	
