	...
	if (VM.GetActiveNamedSet())
	{
		for (int32 PoseIndex = 0; PoseIndex < NumPoses; ++PoseIndex)
		{
			...
			if (Collection == nullptr || !Collection->IsValid())
			{
				...
				UE_LOGF(LogAnimation, Warning, "FAnimNextConvertRotationsToLocalSpaceTask::Execute: Failed to fully execute - No pose data to convert.");
				break;
			}

			Transformers::FBoneSpace::MeshRotationToLocal(FPoseValueBundle::From(**Collection), FPoseValueBundle::From(**Collection));
		}
	}
	else
	{
