...
void FAnimNextBlendTwoKeyframesTask::Execute(UE::UAF::FEvaluationVM& VM) const
{
	...
	if (VM.GetActiveNamedSet())
	{
		...
		TUniquePtr<FValueBundle> CollectionB;
		if (!VM.PopValue(ATTRIBUTE_STACK_NAME, CollectionB))
		{
			// We have no inputs, nothing to do
			return;
		}

		TUniquePtr<FValueBundle> CollectionA;
		if (!VM.PopValue(ATTRIBUTE_STACK_NAME, CollectionA))
		{
			// We have a single input, leave it on top of the stack
			VM.PushValue(ATTRIBUTE_STACK_NAME, MoveTemp(CollectionB));
			return;
		}

		const float WeightOfPoseB = GetInterpolationAlpha(*CollectionA, *CollectionB);

		if (!FAnimWeight::IsRelevant(WeightOfPoseB))
		{
			VM.PushValue(ATTRIBUTE_STACK_NAME, MoveTemp(CollectionA));
		}
		...
		{
			// Interpolate in place the result within A
			// Named sets must match as it ensures that our inputs have the same sizes/shapes
			check(CollectionA->GetNamedSet() == CollectionB->GetNamedSet());

			Transformers::FInterpolate::Apply(VM.GetTransformerMap(), *CollectionA, *CollectionB, WeightOfPoseB, *CollectionA);
			Transformers::FSanitize::Apply(VM.GetTransformerMap(), *CollectionA);
...
float FAnimNextBlendTwoKeyframesTask::GetInterpolationAlpha(const UE::UAF::FValueBundle& CollectionA, const UE::UAF::FValueBundle& CollectionB) const
{
	using namespace UE::UAF;
...
void FAnimNextBlendOverwriteKeyframeWithScaleTask::Execute(UE::UAF::FEvaluationVM& VM) const
{
	...
	if (VM.GetActiveNamedSet())
	{
		...
		if (!VM.PopValue(ATTRIBUTE_STACK_NAME, Collection))
		{
			// We have no inputs, nothing to do
			return;
		}
		...
		Transformers::FOverwrite::Apply(VM.GetTransformerMap(), *Collection, ScaleFactor, *Collection);

		VM.PushValue(ATTRIBUTE_STACK_NAME, MoveTemp(Collection));
	}
	else
	{
...
void FAnimNextBlendAddKeyframeWithScaleTask::Execute(UE::UAF::FEvaluationVM& VM) const
{
	...
	if (VM.GetActiveNamedSet())
	{
		...
		// Accumulate and place the result within B
		// Named sets must match as it ensures that our inputs have the same sizes/shapes
		check(CollectionA->GetNamedSet() == CollectionB->GetNamedSet());

		Transformers::FAccumulate::Apply(VM.GetTransformerMap(), *CollectionB, *CollectionA, ScaleFactor, *CollectionB);
