...
namespace UE::UAF
{
	...
	FTraitPtr FExecutionContext::AllocateNodeInstance(FAnimNextGraphInstance& GraphInstance, FAnimNextTraitHandle ChildTraitHandle) const
	{
		...
		{
			UE_LOGF(LogAnimation, Log, "FExecutionContext::AllocateNodeInstance: Could not allocate node instance - The provided trait handle is not valid.");
			return FTraitPtr();	// Attempting to allocate a node using an invalid trait handle
		}

		if (!ensureMsgf(IsBound(), TEXT("FExecutionContext::AllocateNodeInstance: Could not allocate node instance - The execution context is not bound to a valid graph instance.")))
		{
			return FTraitPtr();	// The execution context must be bound to a valid graph instance
		}

		if (!ensureMsgf(GraphInstance.GetAnimationGraph() != nullptr, TEXT("FExecutionContext::AllocateNodeInstance: Could not allocate node instance - The provided graph instance has no valid animation graph.")))
		{
		...
		if (!ensureMsgf(NodeTemplate != nullptr, TEXT("FExecutionContext::AllocateNodeInstance: Could not allocate node instance - Node template wasn't found, node descriptor might be corrupted")))
		{
		...
		{
			UE_LOGF(LogAnimation, Warning, "FExecutionContext::AllocateNodeInstance: Could not allocate node instance - The requested trait index doesn't exist on that node descriptor.");
		...
		for (const FTraitTemplate* TraitDesc = StartDesc; TraitDesc != EndDesc; ++TraitDesc)
		{
			...
			if (TraitDesc->IsGCAware())
			{
				GraphInstance.RegisterWithGC(Binding.GetTraitPtr());
			}
	...
	FTraitPtr FExecutionContext::AllocateNodeInstance(const FWeakTraitPtr& ParentBinding, FAnimNextTraitHandle ChildTraitHandle) const
	{
		...
		if (!ensureMsgf(ParentBinding.IsValid(), TEXT("FExecutionContext::AllocateNodeInstance: Could not allocate node instance - The provided parent binding was not valid.")))
		{
		...
		for (const FTraitTemplate* TraitDesc = StartDesc; TraitDesc != EndDesc; ++TraitDesc)
		{
			...
			// Auto-register this trait with garbage collection
			if (TraitDesc->IsGCAware())
			{
				const FWeakTraitPtr TraitPtr = Binding.GetTraitPtr();
				FAnimNextGraphInstance& Instance = TraitPtr.GetNodeInstance()->GetOwner();
				Instance.RegisterWithGC(TraitPtr);
			}
	...
	void FExecutionContext::ReleaseNodeInstance(FTraitPtr& NodePtr) const
	{
		...
		if (!ensureMsgf(IsBoundTo(NodeInstance->GetOwner()), TEXT("FExecutionContext::ReleaseNodeInstance: The execution context isn't bound to the right graph instance")))
		{
		...
		{
			return;	// Node instance still has references, we can't release it yet
		}

		const FNodeDescription& NodeDesc = GetNodeDescription(*NodeInstance);
		const FNodeTemplate* NodeTemplate = GetNodeTemplate(NodeDesc);
		if (!ensureMsgf(NodeTemplate != nullptr, TEXT("FExecutionContext::ReleaseNodeInstance: Could not release node instance - Node template wasn't found, node descriptor is perhaps corrupted (we'll leak the node memory)")))
		{
		...
		for (const FTraitTemplate* TraitDesc = StartDesc; TraitDesc != EndDesc; --TraitDesc)
		{
			...
			// Auto-unregister this trait with garbage collection
			if (TraitDesc->IsGCAware())
			{
				const FWeakTraitPtr TraitPtr = Binding.GetTraitPtr();
				FAnimNextGraphInstance& Instance = TraitPtr.GetNodeInstance()->GetOwner();
				Instance.UnregisterWithGC(TraitPtr);
			}
	...
	bool FExecutionContext::GetStack(const FWeakTraitPtr& TraitPtr, FTraitStackBinding& OutStackBinding) const
	{
		if (!TraitPtr.IsValid())
		{
			...
			UE_LOGF(LogAnimation, Warning, "FExecutionContext::GetStack: Provided trait pointer was not valid.");
			return false;
		}

		if (!ensureMsgf(IsBoundTo(TraitPtr.GetNodeInstance()->GetOwner()), TEXT("FExecutionContext::GetStack: The execution context isn't bound to the right graph instance")))
		{
			OutStackBinding.Reset();
	...
	void FExecutionContext::PushScopedInterfaceImpl(FTraitInterfaceUID InterfaceUID, const FTraitBinding& Binding)
	{
		if (!Binding.IsValid())
		{
			UE_LOGF(LogAnimation, Warning, "FExecutionContext::PushScopedInterfaceImpl: Cannot push scoped interface - Provided trait binding is not valid.");
	...
	bool FExecutionContext::PopScopedInterfaceImpl(FTraitInterfaceUID InterfaceUID, const FTraitBinding& Binding)
	{
		if (!Binding.IsValid())
		{
			UE_LOGF(LogAnimation, Warning, "FExecutionContext::PopScopedInterfaceImpl: Cannot pop scoped interface - Provided trait binding is not valid.");
			return false;
		}
	...
	bool FExecutionContext::PopStackScopedInterfaces(const FTraitStackBinding& StackBinding)
	{
		...
		{
			UE_LOGF(LogAnimation, Warning, "FExecutionContext::PopStackScopedInterfaces: Cannot pop scoped interfaces - Provided stack binding is not valid.");
			return false;
		}
	...
	const FNodeDescription& FExecutionContext::GetNodeDescription(const FAnimNextGraphInstance& GraphInstance, FNodeHandle NodeHandle) const
	{
		...
		const UUAFAnimGraph* AnimationGraph = GraphInstance.GetAnimationGraph();
	...
	TWeakObjectPtr<const USkeletalMeshComponent> FExecutionContext::GetBindingObject() const
	{
		// Pull the component from the system's output component
		if (IsBound())
		{
			if (FAnimNextModuleInstance* SystemInstance = RootGraphInstance->GetModuleInstance())
			{
	...
	void FExecutionContext::Log(ELogVerbosity::Type Verbosity, const FString& InMessage, const FString& TraitName) const
	{
		const UObject* HostObject = GetHostObject();
		const UObject* SystemOwner = HostObject ? HostObject->GetOuter() : nullptr;
		const UObject* RootGraphAsset = IsBound() ? RootGraphInstance->GetAsset<UObject>() : nullptr;

		switch (Verbosity)
		{
		case ELogVerbosity::Fatal:
			UE_LOGFMT(LogAnimation, Fatal, "{TraitName}: {Message} - [Root Graph: {GraphPath}] [Owner: {OwnerPath}]", TraitName, InMessage, FSoftObjectPath(RootGraphAsset), FSoftObjectPath(SystemOwner));
			break;
		case ELogVerbosity::Error:
			UE_LOGFMT(LogAnimation, Error, "{TraitName}: {Message} - [Root Graph: {GraphPath}] [Owner: {OwnerPath}]", TraitName, InMessage, FSoftObjectPath(RootGraphAsset), FSoftObjectPath(SystemOwner));
			break;
		case ELogVerbosity::Warning:
			UE_LOGFMT(LogAnimation, Warning, "{TraitName}: {Message} - [Root Graph: {GraphPath}] [Owner: {OwnerPath}]", TraitName, InMessage, FSoftObjectPath(RootGraphAsset), FSoftObjectPath(SystemOwner));
			break;
		case ELogVerbosity::Display:
			UE_LOGFMT(LogAnimation, Display, "{TraitName}: {Message} - [Root Graph: {GraphPath}] [Owner: {OwnerPath}]", TraitName, InMessage, FSoftObjectPath(RootGraphAsset), FSoftObjectPath(SystemOwner));
			break;
		case ELogVerbosity::Log:
			UE_LOGFMT(LogAnimation, Log, "{TraitName}: {Message} - [Root Graph: {GraphPath}] [Owner: {OwnerPath}]", TraitName, InMessage, FSoftObjectPath(RootGraphAsset), FSoftObjectPath(SystemOwner));
			break;
		default:
			UE_LOGFMT(LogAnimation, Error, "Unsupported log category used! Message: {TraitName}: {Message} - [Root Graph: {GraphPath}] [Owner: {OwnerPath}]", TraitName, InMessage, FSoftObjectPath(RootGraphAsset), FSoftObjectPath(SystemOwner));
			break;
		}
	}
