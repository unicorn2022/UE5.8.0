...
namespace UE::UAF
{
	...
	{
		SyncGroupComponent = &Context.GetOrAddComponent<FSyncGroupGraphInstanceComponent>();
	...
	void FSynchronizeUsingGroupsTrait::PreUpdate(FUpdateTraversalContext& Context, const TTraitBinding<IUpdate>& Binding, const FTraitUpdateState& TraitState) const
	{
		...
		if (!InstanceData->bHasTimelinePlayer)
		{
			...
			UAF_TRAIT_LOG(Warning, TEXT("FSynchronizeUsingGroupsTrait::PreUpdate: Could not synchronize - No valid timeline player beneath us on the trait stack."));
	...
	void FSynchronizeUsingGroupsTrait::GetFactoryParams(FExecutionContext& Context, const TTraitBinding<IGraphFactory>& Binding, FGraphAssetHandleConstView InAssetData, FAnimNextFactoryParams& InOutParams) const
	{
		IGraphFactory::GetFactoryParams(Context, Binding, InAssetData, InOutParams);
		...
		if (GroupParameters.SyncMode != EAnimGroupSynchronizationMode::NoSynchronization)
		{
			...
			InOutParams.AddTrait(ETraitVariableMapping::All, 0, SynchronizeUsingGroups);
		}
	}
}
