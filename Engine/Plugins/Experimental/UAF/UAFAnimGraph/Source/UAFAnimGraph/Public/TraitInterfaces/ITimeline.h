...
namespace UE::UAF
{
	...
	struct ITimeline : ITraitInterface
	{
		...
		// It is possible for the timeline to be in an 'unknown' state if this is called before supporting data is copied/read, in which case this 
		// function will return false and the state will not be filled
		UE_API virtual bool GetState(const FExecutionContext& Context, const TTraitBinding<ITimeline>& Binding, FTimelineState& OutTimelineState) const;

		// Returns the last change in state of this timeline
		// It is possible for the timeline to be in an 'unknown' state if this is called before supporting data is copied/read, in which case this 
		// function will return false and the delta will not be filled
		UE_API virtual bool GetDelta(const FExecutionContext& Context, const TTraitBinding<ITimeline>& Binding, FTimelineDelta& OutTimelineDelta) const;
	...
	struct TTraitBinding<ITimeline> : FTraitBinding
	{
		...
		bool GetState(const FExecutionContext& Context, FTimelineState& OutTimelineState) const
		{
			return GetInterface()->GetState(Context, *this, OutTimelineState);
		}

		// @see ITimeline::GetDelta
		bool GetDelta(const FExecutionContext& Context, FTimelineDelta& OutTimelineDelta) const
		{
			return GetInterface()->GetDelta(Context, *this, OutTimelineDelta);
		}
