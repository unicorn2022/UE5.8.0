...
namespace UE::UAF
{
	...
	*/
	struct FEvaluateGraphContext
	{
		...
		// Only set if the new abstract skeleton runtime is enabled
		UAbstractSkeletonSetBinding* GetSkeletonSetBinding() const { return SkeletonSetBinding; }
		FName GetSkeletonSetName() const { return SkeletonSetName; }
		USkeletalMesh* GetSkeletalMesh() const { return SkeletalMesh; }

	protected:
		UE_API TWeakObjectPtr<const USkeletalMeshComponent> GetBindingObject() const;

		void SetSkeletonSet(UAbstractSkeletonSetBinding* InSkeletonSetBinding, USkeletalMesh* InSkeletalMesh, FName InSkeletonSetName)
		{
		...
		friend UE_API FEvaluationProgram EvaluateGraph(const FEvaluateGraphContext& EvaluateGraphContext, const FWeakTraitPtr& GraphRootPtr);
		friend struct ::FRigUnit_AnimNextRunAnimationGraph_v1;
		friend struct ::FRigUnit_AnimNextRunAnimationGraph_v2;
		friend struct ::FRigUnit_UAFRunAsset;
	 ...
	 */
	[[nodiscard]] UE_API FEvaluationProgram EvaluateGraph(const FEvaluateGraphContext& EvaluateGraphContext);
	 ...
	 * @see IEvaluate::PreEvaluate, IEvaluate::PostEvaluate, IHierarchy::GetChildren
	 */
	[[nodiscard]] UE_API FEvaluationProgram EvaluateGraph(const FEvaluateGraphContext& EvaluateGraphContext, const FWeakTraitPtr& GraphRootPtr);

	/**
	 * Evaluates a sub-graph starting at its root and produces an output pose
	 */
	UE_API void EvaluateGraph(const FEvaluateGraphContext& EvaluateGraphContext, FAnimNextGraphLODPose& OutputPose);

	// Returns whether or not we should use the new abstract skeleton runtime
	UE_API bool UseAbstractSkeletonRuntime();
}
