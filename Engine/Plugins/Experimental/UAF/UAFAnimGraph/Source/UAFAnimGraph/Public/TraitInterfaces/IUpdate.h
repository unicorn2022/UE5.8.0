...
namespace UE::UAF
{
	...
	*/
	...
	protected:
		TWeakObjectPtr<const USkeletalMeshComponent> GetBindingObject() const;
	...
	struct FUpdateTraversalContext final : FExecutionContext
	{
		...
		UE_API virtual void RaiseInputTraitEvent(FAnimNextTraitEventPtr Event) override;

		// Raises an output trait event
		// The raised event will be seen by every parent of the currently updating trait stack
		UE_API virtual void RaiseOutputTraitEvent(FAnimNextTraitEventPtr Event) override;
