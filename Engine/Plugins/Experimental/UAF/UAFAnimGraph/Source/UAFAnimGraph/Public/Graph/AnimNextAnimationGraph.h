...
namespace UE::UAF
{
	...
	{
		// The system to use as our outermost host
		FUAFWeakSystemReference SystemReference;
		// The hosting/parent graph instance
		TWeakPtr<FAnimNextGraphInstance> ParentGraphInstance;
...
public:
	UE_API UUAFAnimGraph(const FObjectInitializer& ObjectInitializer);
	...
	// Pre-allocates an instance of the graph
	// @param	InParams		Parameters to pre-allocate the graph with
	// @param	InFactoryParams	Factory parameters to apply to the graph when allocated
	// @return the pre-allocated instance.
	UE_API TSharedPtr<FAnimNextGraphInstance> PreAllocateInstance(const UE::UAF::FGraphAllocationParams& InParams = UE::UAF::FGraphAllocationParams(), FAnimNextFactoryParams&& InFactoryParams = FAnimNextFactoryParams()) const;
	...
	UE_API TSharedPtr<FAnimNextGraphInstance> AllocateInstance(const UE::UAF::FGraphAllocationParams& InParams = UE::UAF::FGraphAllocationParams(), FAnimNextFactoryParams&& InFactoryParams = FAnimNextFactoryParams()) const;
...
private:
	...
	friend class UUAFAnimGraph_EditorData;
