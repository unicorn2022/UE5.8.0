...
namespace UE::UAF
{
	...
	{
		UE_API FEvaluationVM(EEvaluationFlags InEvaluationFlags, const FReferencePose& InReferencePose, int32 InCurrentLOD);
		UE_API FEvaluationVM(EEvaluationFlags EvaluationFlags, TNonNullPtr<const UAbstractSkeletonSetBinding> SetBinding, const USkeletalMesh* SkeletalMesh, FName SetName, int32 CurrentLOD);

		// REMOVE ME: Exclusively for testing that before/after yields the same results
		UE_API FEvaluationVM(EEvaluationFlags InEvaluationFlags, const FReferencePose& IReferencePose, const UAbstractSkeletonSetBinding* SetBinding, const USkeletalMesh* SkeletalMesh, FName SetName, int32 InCurrentLOD);
		...
		// Returns the evaluation context that controls what state we produce
		[[nodiscard]] UE_API const FEvaluationTaskContext& GetActiveEvaluationContext() const;
		...
		[[nodiscard]] int32 GetCurrentLOD() const { return CurrentLOD; }

		// Returns the value transformer map
		[[nodiscard]] UE_API const FValueTransformerMapPtr& GetTransformerMap() const;
	...
	private:
		...
		// Reference pose
		const FReferencePose* ReferencePose = nullptr;

		// The internal context that controls what state we are producing
		TArray<FEvaluationTaskContext> EvaluationContextStack;
	...
	// A stack of FValueBundle instances used when sampling sequences and blending their results
	extern UAFANIMGRAPH_API const FEvaluationVMStackName ATTRIBUTE_STACK_NAME;
	ANIM_NEXT_ENABLE_EVALUATION_STACK_USAGE(TUniquePtr<FValueBundle>)
}
