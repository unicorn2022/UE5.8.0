...
namespace UE::UAF
{
	...
	struct FTraitTemplate
	{
		...
		// Returns whether this trait template needs be registered with the reference collector
		bool IsGCAware() const noexcept { return bGCAware; }
	...
	private:
		...
		FTraitTemplate(FTraitUID InUID, FTraitRegistryHandle InRegistryHandle, ETraitMode InMode, bool bInGCAware, uint32 InTraitIndexOrNumTraits) noexcept
			: UID(InUID.GetUID())
		...
		// Trait mode (we only need 2 bits)
		uint8	Mode : 2;
		static_assert(static_cast<uint8_t>(ETraitMode::Num) <= 2);
		
		// Flag set if a trait implements IGarbageCollection
		uint8   bGCAware : 1;
		
		// (Padding for extra flags)
		uint8   Padding0 : 5;
		...
		uint16	Padding1;						// Unused for now
