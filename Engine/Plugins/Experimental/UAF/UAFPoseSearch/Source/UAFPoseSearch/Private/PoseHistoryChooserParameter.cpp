...
{
	if(FUAFAssetInstance* Instance = GetFirstUAFAssetInstance(Context))
	{
		return GetVariableValue(Instance, Variable, OutResult) == EPropertyBagResult::Success;
	}
	return false;
}

bool FPoseHistoryAnimProperty::GetValue(FChooserEvaluationContext& Context, FPoseSearchHistory& OutResult) const
{
	if(FUAFAssetInstance* Instance = GetFirstUAFAssetInstance(Context))
	{
		return GetVariableValue(Instance, Variable, OutResult) == EPropertyBagResult::Success;
