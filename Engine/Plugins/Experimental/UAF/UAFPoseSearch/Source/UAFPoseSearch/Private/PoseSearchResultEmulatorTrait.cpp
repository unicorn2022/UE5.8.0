...
#undef TRAIT_INTERFACE_ENUMERATOR
	...
	void FPoseSearchResultEmulatorTrait::PreUpdate(FUpdateTraversalContext& Context, const TTraitBinding<IUpdate>& Binding, const FTraitUpdateState& TraitState) const
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FPoseSearchResultEmulatorTrait::PreUpdate, missing IBlendStack");
			return;
		}
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FPoseSearchResultEmulatorTrait::PreUpdate, missing ITimeline");
			return;
		}
		...
		if (AnimObject)
		{
			...
			{
				TInstancedStruct<FUAFGraphFactoryAsset> AssetHandle = FAssetDataFactory::CreateUAFAssetDataFromObject<FUAFGraphFactoryAsset>(AnimationAsset);
				...
				if (bIsBlendSpace)
				{
					...
					const EAnimAssetLoopMode LoopMode = bLoop ? EAnimAssetLoopMode::ForceLoop : EAnimAssetLoopMode::ForceNonLoop;

					FactoryParams.AddInitializeTask([XAxisSamplePoint, YAxisSamplePoint, AnimationAsset, WantedPlayRate, SelectedTime, LoopMode](const FInstanceTaskContext& InInstance)
					{
						InInstance.AccessVariablesStruct<FSynchronizeUsingGroupsData>([](FSynchronizeUsingGroupsData& SynchronizeUsingGroups)
						{
						...
						InInstance.AccessVariablesStruct<FBlendSpacePlayerData>([AnimationAsset, XAxisSamplePoint, YAxisSamplePoint, WantedPlayRate, SelectedTime, LoopMode](FBlendSpacePlayerData& BlendSpacePlayer)
						{
				...
				else
				{
					...
					FactoryParams.AddInitializeTask([SelectedTime, LoopMode, AnimationAsset, WantedPlayRate](const FInstanceTaskContext& InInstance)
					{
						InInstance.AccessVariablesStruct<FSequencePlayerData>([SelectedTime, LoopMode, AnimationAsset, WantedPlayRate](FSequencePlayerData& AnimSequencePlayer)
						{
...
void FAnimNextPoseSearchResultEmulatorTask::Execute(UE::UAF::FEvaluationVM& VM) const
{
	...
	{
		UE_LOGF(LogPoseSearch, Error, "FAnimNextPoseSearchResultEmulatorTask::Execute, missing RootMotionProvider");
		return;
	}
