...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void FMotionMatchingTrait::PreUpdate(FUpdateTraversalContext& Context, const TTraitBinding<IUpdate>& Binding, const FTraitUpdateState& TraitState) const
	{
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FMotionMatchingTrait::PreUpdate, PostEvaluate has not being called last frame! Some trait in the TraitStack didn't propagate correctly the PostEvaluate!");
		}
#endif // WITH_EDITOR
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FMotionMatchingTrait::PreUpdate, missing IBlendStack");
			return;
		}
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FMotionMatchingTrait::PreUpdate, missing UE::UAF::IPoseHistory");
			return;
		}
		...
		{
			UE_LOGF(LogPoseSearch, Error, "FMotionMatchingTrait::PreUpdate, missing ITimeline");
			return;
		}
		...
		if (!PoseHistory)
		{
			UE_LOGF(LogPoseSearch, Error, "FMotionMatchingTrait::PreUpdate, missing UE::PoseSearch::IPoseHistory");
			return;
		}
		...
		UPoseSearchInteractionLibrary::MotionMatchInteraction(MotionMatchingState.SearchResult, Availabilities, AnimContext, FName(), PoseHistory,
			SharedData->GetbValidateResultAgainstAvailabilities(Binding), SharedData->GetbKeepInteractionAlive(Binding), SharedData->GetBlendArguments(Binding).BlendTime, DeltaTime);
		...
		if (!MotionMatchingState.SearchResult.bIsInteraction)
		{
			...
			float TimelineNormalizedTime = 0.0f;
			FTimelineState TimelineState;
			if (TimelineTrait.GetState(Context, TimelineState))
			{
				const float TimelineRealTime = TimelineState.GetPosition();
				TimelineNormalizedTime = TimelineRealTime;
			...
			if (MotionMatchingState.ElapsedPoseSearchTime < SearchThrottleTime)
			{
				SearchResult.InitFrom(MotionMatchingState.SearchResult);
		...
		if (bJumpToPose)
		{
			if (UAnimationAsset* AnimationAsset = MotionMatchingState.SearchResult.GetAnimationAssetForRole())
			{
				...
				if (!bSkipBlend)
				{
					FGraphAssetHandle AssetHandle = FAssetDataFactory::CreateUAFAssetDataFromObject<FUAFGraphFactoryAsset>(AnimationAsset);
					FAnimNextFactoryParams FactoryParams = FAnimGraphFactory::GetDefaultParamsForAsset(AssetHandle);

					// currently the default params for sequences and blend spaces lack the FEvaluationNotifiesTrait, necessary trait to be able to evaluate 
					// FEvaluationNotify_BaseInstance(s) during trait post evaluation task (FAnimNextEvaluationNotifiesTask::Execute).
					// Since it's required for MM interaction warp aligment, and there's no data driven way to do so, we have to hard code it here for now.
					FactoryParams.AddTraitStruct<FEvaluationNotifiesTraitSharedData>(ETraitVariableMapping::None, 0);

					// This code manually creates the trait data required for blend space and anim sequence
					// We could utilize UAFAssetData if we add all the options we need to it
					
					if (bIsBlendSpace)
					{
						FactoryParams.AddTraitStruct<FSynchronizeUsingGroupsData>(ETraitVariableMapping::All, 0);
						FactoryParams.AddTraitStruct<FBlendSpacePlayerData>(ETraitVariableMapping::All, 0);

						const EAnimGroupSynchronizationMode SyncMode = SharedData->GetSyncMode(Binding);
						const FVector BlendParameters = MotionMatchingState.SearchResult.BlendParameters;
						const float SelectedTime = MotionMatchingState.SearchResult.SelectedTime;
						const EAnimAssetLoopMode LoopMode = MotionMatchingState.SearchResult.bLoop ? EAnimAssetLoopMode::ForceLoop : EAnimAssetLoopMode::ForceNonLoop;
						
						FactoryParams.AddInitializeTask([SyncMode, BlendParameters, AnimationAsset, DesiredPlayRate, SelectedTime, LoopMode](const FInstanceTaskContext& InInstance)
						{
							InInstance.AccessVariablesStruct<FSynchronizeUsingGroupsData>([SyncMode, BlendParameters](FSynchronizeUsingGroupsData& SynchronizeUsingGroups)
							{
								...
								SynchronizeUsingGroups.SyncMode = SyncMode;
								SynchronizeUsingGroups.bMatchSyncPoint = true;
							});

							InInstance.AccessVariablesStruct<FBlendSpacePlayerData>([AnimationAsset, BlendParameters, DesiredPlayRate, SelectedTime, LoopMode](FBlendSpacePlayerData& BlendSpacePlayer)
							{
								BlendSpacePlayer.BlendSpace = Cast<UBlendSpace>(AnimationAsset);

								BlendSpacePlayer.XAxisSamplePoint = BlendParameters.X;
								BlendSpacePlayer.YAxisSamplePoint = BlendParameters.Y;

								BlendSpacePlayer.PlayRate = DesiredPlayRate;
								BlendSpacePlayer.StartPosition = SelectedTime;
								BlendSpacePlayer.LoopMode = LoopMode;
							});
						});
					}
					else
					{
						FactoryParams.AddTraitStruct<FSequencePlayerData>(ETraitVariableMapping::All, 0);

						const float SelectedTime = MotionMatchingState.SearchResult.SelectedTime;
						const EAnimAssetLoopMode LoopMode = MotionMatchingState.SearchResult.bLoop ? EAnimAssetLoopMode::ForceLoop : EAnimAssetLoopMode::ForceNonLoop;

						FactoryParams.AddInitializeTask([SelectedTime, LoopMode, AnimationAsset, DesiredPlayRate](const FInstanceTaskContext& InInstance)
						{
							InInstance.AccessVariablesStruct<FSequencePlayerData>([SelectedTime, LoopMode, AnimationAsset, DesiredPlayRate](FSequencePlayerData& AnimSequencePlayer)
							{
					...
					const UUAFAnimGraph* AnimationGraph = IGraphFactory::GetOrBuildGraph(Context, Binding, AssetHandle, FactoryParams);
					if (AnimationGraph != nullptr)
					{
						...
						NewGraphRequest.BlendProfile = SharedData->GetBlendProfile(Binding);
		...
		if (MotionMatchingState.SearchResult.bIsInteraction && SharedData->GetbEnableWarp(Binding))
		{
			if (const UMultiAnimAsset* MultiAnimAsset = Cast<UMultiAnimAsset>(MotionMatchingState.SearchResult.SelectedAnim))
			{
				...
				if (CurrentResultRoleIndex != INDEX_NONE &&
					MotionMatchingState.SearchResult.ActorRootTransforms.Num() == MotionMatchingState.SearchResult.AnimContexts.Num() &&
					MotionMatchingState.SearchResult.ActorRootBoneTransforms.Num() == MotionMatchingState.SearchResult.AnimContexts.Num())
				{
					...
					CalculateFullAlignedTransforms(MotionMatchingState.SearchResult, MultiAnimAsset->GetPlayLength(FVector::ZeroVector), 0.f, SharedData->GetbWarpUsingRootBone(Binding), FullAlignedTransforms);
	...
	void FMotionMatchingTrait::PublishResults(const TTraitBinding<IUpdate>& Binding) const
	{
		...
		{
			UE_LOGF(LogPoseSearch, Warning, "FMotionMatchingTrait::PreUpdate called while '%ls' is indexing. returning an invalid result..", *CurrentResultDatabase->GetName());
		}
#endif // WITH_EDITOR
		
		const auto PublishResult = [](FAnimNextModuleInstance* ModuleInstance, const FUAFWeakSystemReference& SystemReference, const FPoseSearchBlueprintResult& SearchResult, FName ResultOutputVariableName)
			{
				if (ResultOutputVariableName != NAME_None)
				{
					if (SystemReference.IsValid())
					{
						...
						// in case the module containing this trait gets destroyed before the QueueTask lambda execution
		...
		const FUAFWeakSystemReference ResultSystemReference = SharedData->GetMotionMatchingResultSystemReference(Binding);
		PublishResult(ModuleInstance, ResultSystemReference, MotionMatchingState.SearchResult, ResultOutputVariableName);
		
		const FUAFWeakSystemReference ResultSystemReferenceAlt = SharedData->GetMotionMatchingResultSystemReferenceAlt(Binding);
		PublishResult(ModuleInstance, ResultSystemReferenceAlt, MotionMatchingState.SearchResult, ResultOutputVariableNameAlt);
	}
	...
	void FMotionMatchingTrait::PostEvaluate(FEvaluateTraversalContext& Context, const TTraitBinding<IEvaluate>& Binding) const
	{
		...
		if (InstanceData->bIsPostEvaluateBeingCalled)
		{
			UE_LOGF(LogPoseSearch, Error, "FMotionMatchingTrait::PostEvaluate, PostEvaluate called without calling PreUpdate on this frame! Some trait in the TraitStack doesn't propagate correctly the PreUpdate!");
		}
		else
		{
		...
		const FSharedData* SharedData = Binding.GetSharedData<FSharedData>();
		check(SharedData);
		if (SharedData->GetbEnableWarp(Binding))
		{
			...
			if (SearchResult.bIsInteraction)
			{
				if (const UMultiAnimAsset* MultiAnimAsset = Cast<UMultiAnimAsset>(SearchResult.SelectedAnim))
				{
					...
					if (CurrentResultRoleIndex != INDEX_NONE)
					{
						...
						if (const FAnimNextModuleInstance* ModuleInstance = Context.GetRootGraphInstance().GetModuleInstance())
						{
							if (const UUAFComponent* AnimNextComponent = Cast<UUAFComponent>(ModuleInstance->GetObject()))
							{
...
void FAnimNextMotionMatchingTask::Execute(UE::UAF::FEvaluationVM& VM) const
{
	...
	{
		UE_LOGF(LogPoseSearch, Error, "FAnimNextMotionMatchingTask::Execute, missing RootMotionProvider");
		return;
	}
	...
	CalculateFullAlignedTransforms(SearchResult, SearchResult.SelectedTime, 0.f, bWarpUsingRootBone, FullAlignedTransforms);
