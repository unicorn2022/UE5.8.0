...
{
	UUAFAnimGraph_EditorData::RecompileVM();
	...
	UE::UAF::UncookedOnly::FCompilationScope CompileScope(AnimationStateTree);
	...
	UStateTreeEditingSubsystem::CompileStateTree(InnerStateTree, Log);

	FCompilerResultsLog& CompileLog = UE::UAF::UncookedOnly::FCompilationScope::GetLogForObject(AnimationStateTree);
...
TConstArrayView<TSubclassOf<UUAFRigVMAssetEntry>> UAnimNextStateTree_EditorData::GetEntryClasses() const
{
		...
		auto MakeNodeVariables = [&](UStateTreeState& State, UStateTreeState* ParentState)
		{
			for (const FStateTreeEditorNode& Node : State.EnterConditions)
			{
				...
				if (const FAnimNextStateTreeRigVMConditionBase* Condition = Node.Node.GetPtr<FAnimNextStateTreeRigVMConditionBase>())
				{
					...
					const_cast<FAnimNextStateTreeRigVMConditionBase*>(Condition)->GetProgrammaticFunctionHeaders(ProgrammaticFunctionHeaderParams, &State, Node.GetInstance(), Desc);
				}
			}
			...
			for (const FStateTreeEditorNode& Node : State.Tasks)
			{
				...
				if (const FAnimNextStateTreeRigVMTaskBase* Condition = Node.Node.GetPtr<FAnimNextStateTreeRigVMTaskBase>())
				{
					...
					// Graph gen isn't const since we need to cache generated names in the node
					const_cast<FAnimNextStateTreeRigVMTaskBase*>(Condition)->GetProgrammaticFunctionHeaders(ProgrammaticFunctionHeaderParams, &State, Node.GetInstance(), Desc);
				}
			}
...
void UAnimNextStateTree_EditorData::OnPreCompileGetProgrammaticGraphs(const FRigVMCompileSettings& InSettings, FAnimNextGetGraphCompileContext& OutCompileContext)
{
	...
	if (UAnimNextStateTree* AnimStateTree = UE::UAF::UncookedOnly::FUtils::GetAsset<UAnimNextStateTree>(this))
	{
		...
		const FName SmootherPerBoneTraitName = Controller->AddTraitStruct(TraitStackNode, TInstancedStruct<FAnimNextBlendSmootherPerBoneCoreTraitSharedData>::Make(), INDEX_NONE, false, false);

		if (SmootherPerBoneTraitName == NAME_None)
		{
			InSettings.ReportError(TEXT("Failed to add Blend Smoother Per Bone Core trait to node"));
			return;
		}
