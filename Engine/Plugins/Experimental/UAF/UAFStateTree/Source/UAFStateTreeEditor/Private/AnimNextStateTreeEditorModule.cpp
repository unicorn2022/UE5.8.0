...
#include "UAFStateTreeEditorSchema.h"
...
void FAnimNextStateTreeEditorModule::StartupModule()
{
	...
	StateTreeEditorModule.RegisterEditorSchemaClass(UStateTreeAnimNextSchema::StaticClass(), UUAFStateTreeEditorSchema::StaticClass());

	StateTreeClassPath = FUAFAnimNodeDataFactory::RegisterAsset<FUAFStateTreeNodeData, UAnimNextStateTree>(
	[](UAnimNextStateTree* StateTree)
		{
			FUAFStateTreeNodeData StateTreeNode;
			StateTreeNode.SetStateTree(StateTree);
...
void FAnimNextStateTreeEditorModule::ShutdownModule()
{
	if (UObjectInitialized())
	{
		...
		if(FStateTreeEditorModule* StateTreeEditorModule = FStateTreeEditorModule::GetModulePtr())
		{
			...
			StateTreeEditorModule->UnregisterEditorSchemaClass(UStateTreeAnimNextSchema::StaticClass());
		}
    }
    
    FUAFAnimNodeDataFactory::UnregisterAsset(StateTreeClassPath);
}
