	...
	TScriptInterface<IAsyncMessageBindingEndpointInterface> DesiredEndpoint)
{
	...
	if (!World)
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Failed to queue message '%ls' for broadcasting: Unable to find a world.",
	...
	if (!Sys.IsValid())
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Failed to queue message '%ls' for broadcasting: Unable to find a message system for world '%ls'.",
