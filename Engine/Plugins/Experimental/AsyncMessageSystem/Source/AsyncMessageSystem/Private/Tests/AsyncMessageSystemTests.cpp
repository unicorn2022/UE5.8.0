...
namespace UE::AsyncMessageSystem
{
	...
	// This test ensures that if you: 
	// 1. Bind a listener
	// 2. Call "Unbind" on it immediately, before the subsystem can tick
	// 3. An event is queued and fired for the same message you were bound it
	// 4. The listener from 1 does NOT get fired.
	IMPLEMENT_SIMPLE_AUTOMATION_TEST(TMessageSystemUnBindingBeforeBindingFinishes, "AsyncMessagePassing.UnBinding.BeforeBindingFinishes", QuickTestFlags)
	bool TMessageSystemUnBindingBeforeBindingFinishes::RunTest(const FString& InParameters)
	{
		// Startup a new message system...
		TSharedPtr<FTestMessageSystem, ESPMode::ThreadSafe> TestSystem = FAsyncMessageSystemBase::CreateMessageSystem<FTestMessageSystem>();
		TestTrue(TEXT("MessageSystem startup has been called"), TestSystem->HasStarted());

		// Set up a listener which would modify some test data if called.
		int32 TestData = 0;
		const FAsyncMessageHandle ListenerHandle = TestSystem->BindListener(TestMessageId_A, [&TestData, this](const FAsyncMessage& Message)
		{
			TestData++;
			TestTrue(TEXT("Test failed. This delegate should never get called."), false);
		});

		// Note: When you the call Bind/Unbind listener functions, the handles are put a queue, so they will not immediately be made valid or invalid.

		// Ensure that the pre-tick state is correct. The handle shouldn't be bound yet because we have not ticked the subsystem
		TestFalse(TEXT("The handle is in the expected pre-tick state of being unbound"), TestSystem->IsHandleBound(ListenerHandle));
		
		// Unbind the listener now, before we tick and ProcessListenersPendingBinding gets called.
		TestSystem->UnbindListener(ListenerHandle);

		// Tick the subsytem, which should process the pending queue of listeners and bind the delegates internally.
		TestSystem->RunOnce();

		// Since we called Unbind immediately, the handle should never be fully bound.
		TestFalse(TEXT("The handle was correctly not bound during tick."), TestSystem->IsHandleBound(ListenerHandle));

		// Fire a message for the event which was bound/unbound. The test data should not be mutated
		TestSystem->QueueMessageForBroadcast(TestMessageId_A);
		TestSystem->RunOnce();

		// Ensure that our test data has not been modified after firing a delegate
		TestTrue(TEXT("The handle is in the expected post-tick state and is unbound"), TestData == 0);
	...
	bool TMessageSystemMultiThreadedQueueMessagesTest::RunTest(const FString& InParameters)
	{
		...
		FAsyncMessageHandle ListenerHandle = TestSystem->BindListener(TestMessageId_A, [](const FAsyncMessage& Message)
		{
			...
			if (Data)
			{
				UE_LOGF(LogTemp, Log, "DataVal : %d", Data->IncrementAmount);
			}
			else
			{
				UE_LOGF(LogTemp, Error, "Failed to find the payload data, it has expired");
			}
		});
