	...
	TWeakPtr<FAsyncMessageBindingEndpoint> BindingEndpoint /*= nullptr*/)
{
	...
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Failed to queue message: There is no valid DefaultBindingEndpoint on this message system.", __func__);
		return false;
	}
...
void FAsyncMessageSystemBase::Startup()
{
	...
	UE_LOGF(LogAsyncMessageSystem, Verbose, "[%s] Message System Startup...", __func__);
...
void FAsyncMessageSystemBase::Shutdown()
{
	...
	UE_LOGF(LogAsyncMessageSystem, Verbose, "[%s] Message System Shutdown...", __func__);
	...
	const FAsyncMessageBindingOptions& Options)
{
	...
	if (bIsShuttingDown)
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Attempting to bind an message listener to message '%ls' during shutdown.",
	...
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Attempting to bind an message listener to invalid message name '%ls'",
			__func__, *MessageId.ToString());
	...
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Attempting to bind message '%ls' to invalid handle '%ls'",
	...
	TWeakPtr<FAsyncMessageBindingEndpoint> BindingEndpoint)
{
	...
	if (bIsShuttingDown)
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Attempting to queue an message '%ls' during shutdown. The message will not be queued",
			__func__, *MessageId.ToString());
	...
	if (!MessageId.IsValid())
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Attempting to queue an invalid message id  '%ls', the message will not be queued",
			__func__, *MessageId.ToString());
	...
	UE_LOGF(LogAsyncMessageSystem, Verbose, "[%s] message '%ls' was added to %u binding queues", 
...
void FAsyncMessageSystemBase::ProcessMessageQueueForBinding_Impl(const FAsyncMessageBindingOptions& Options)
{
	...
	if (bIsShuttingDown)
	{
		UE_LOGF(LogAsyncMessageSystem, Error, "[%s] Attempting to Process Message Queue during shutdown, exiting", __func__);
		return;
	}
	...
	UE_CLOGF(ProcessedMessages > 0, LogAsyncMessageSystem, VeryVerbose, "[%s] Processed %u messages. Called %u message listeners.", 
...
void FAsyncMessageSystemBase::ProcessUnbindHandleRequests()
{
	...
	while (HandleToRemoveOpt.IsSet())
	{
		...
		if (TSharedPtr<FAsyncMessageBindingEndpoint> Endpoint = HandleToRemove.GetBindingEndpoint())
		{
			...
			{
				// If we get here, then the current handle has been unbound before we have actually had a chance to fully set up its binding yet.
				// For example, if you call Bind/Unbind immediately in a blueprint node. 
				// Keep track of these handles which we couldn't fully unbind yet so that we don't bother to set up their bindings.
				UE_LOGF(LogAsyncMessageSystem, Verbose, "Handle: %ls -- No binding data found. That means this listener has not been fully added yet. We will prevent it from being added instead of removing it.", *HandleToRemove.ToString());				
				IncompleteBoundHandlesToRemove.Add(HandleToRemove);
			}
		}
		else
		{
			UE_LOGF(LogAsyncMessageSystem, Error, "Attempting to unbind from an invalid endpoint. Failed to remove listeners for handle: %ls", *HandleToRemove.ToString());
		}
...
void FAsyncMessageSystemBase::ProcessListenersPendingBinding()
{
	...
	while (ListenerToBind.IsSet())
	{
		...
		// Only finish setting up the bindings for handles which have not been unbound already.
		if (!IncompleteBoundHandlesToRemove.Contains(ListenerToBind->Data.Handle))
		{
		...
		{
			UE_LOGF(LogAsyncMessageSystem, Verbose, "Skipping pending binding to handle %ls because it was already marked for unbinding.", *ListenerToBind->Data.Handle.ToString());
		}

		ListenerToBind = PendingBoundListenerQueue.Dequeue();
	}

	// We have processed all the pending bound listeners so we can now empty this
	IncompleteBoundHandlesToRemove.Empty();

	// After iterating and dequeue'ing off the listener map it should be empty
	check(PendingBoundListenerQueue.IsEmpty());
}
...
FAsyncMessageHandle FAsyncMessageSystemBase::GenerateNextValidMessageHandle(const FAsyncMessageId ForMessageId, TWeakPtr<FAsyncMessageBindingEndpoint> Endpoint)
{
	...
	if (HandleValue == 0)
	{
		...
		UE_LOGF(LogAsyncMessageSystem, Warning, "The Async Message Handle index has been wrapped!");
	}
