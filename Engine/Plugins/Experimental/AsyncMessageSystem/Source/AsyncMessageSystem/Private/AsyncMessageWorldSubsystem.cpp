...
void UAsyncMessageWorldSubsystem::InitMessageSystem()
{
	...
	UE_LOGF(LogAsyncMessageSystem, Verbose, "[%s] Init world message system for world '%ls'", __func__, *GetNameSafe(GetWorld()));
...
void UAsyncMessageWorldSubsystem::ShutdownMessageSystem()
{
	...
	{
		UE_LOGF(LogAsyncMessageSystem, Verbose, "[%s] Shutdown world message system for world '%ls'", __func__, *GetNameSafe(GetWorld()));
