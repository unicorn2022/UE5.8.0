...
bool FPBIKSolver::InitBones()
{
	...
	if (Bones.IsEmpty())
	{
		UE_LOGF(LogPBIKSolver, Warning, "PBIK: no bones added to solver. Cannot initialize.");
		return false;
	}

	if (Effectors.IsEmpty())
	{
		UE_LOGF(LogPBIKSolver, Warning, "PBIK: no effectors added to solver. Cannot initialize.");
		return false;
	}
	...
	if (!SolverRoot)
	{
		UE_LOGF(LogPBIKSolver, Warning, "PBIK: root bone not set or not found. Cannot initialize.");
		return false;
	}

	if (NumSolverRoots > 1)
	{
		UE_LOGF(LogPBIKSolver, Warning, "PBIK: more than 1 bone was marked as solver root. Cannot initialize.");
		return false;
	}
	...
	for (FBone& Bone : Bones)
	{
		...
		if (!bIndexInRange)
		{
			UE_LOGF(LogPBIKSolver, Warning, "PBIK: bone found with invalid parent index. Cannot initialize.");
			return false;
		}
...
bool FPBIKSolver::InitBodies()
{
	...
	for (const FEffector& Effector : Effectors)
	{
		...
		while (true)
		{
			...
			if (!BodyBone)
			{
				UE_LOGF(LogPBIKSolver, Warning, "PBIK: effector is on bone that is not on or below root bone.");
				return false;
			}
...
bool FPBIKSolver::InitConstraints()
{
	...
	for (FEffector& Effector : Effectors)
	{
		...
		if (!BodyBone)
		{
			UE_LOGF(LogPBIKSolver, Warning, "PBIK: effector is on bone that does not have a parent.");
			return false;
		}
