...
void FAnimNode_SequencerMixerTarget::Update_AnyThread(const FAnimationUpdateContext& Context)
{
	...
	{
		FAnimSubsystem_SequencerMixer& MixerSubsystem = AnimInstance->GetSubsystem<FAnimSubsystem_SequencerMixer>();
		for (FSequencerMixerPendingNotifyBatch& Batch : MixerSubsystem.ConsumePendingNotifyBatches(TargetName))
		{
			Context.AnimInstanceProxy->AddAnimNotifies(Batch.Notifies, Batch.Weight);
		}
	}
}
...
void FAnimNode_SequencerMixerTarget::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	ApplySequencerMixedPose(Output, TargetName);
}

void FAnimNode_SequencerMixerTarget::ApplySequencerMixedPose(FPoseContext& Output, FName InTargetName)
{
	...
	const TSharedPtr<FAnimNextEvaluationTask>* MixerTask = MixerSubsystem.GetEvalTask(InTargetName);
