...
void USequencerMixedAnimInstance::StagePendingNotifyBatches(TArray<FSequencerMixerPendingNotifyBatch>&& InBatches)
{
	GetProxyOnGameThread<FSequencerMixedAnimInstanceProxy>().StagePendingNotifyBatches(MoveTemp(InBatches));
}
...
void FSequencerMixedAnimInstanceProxy::UpdateAnimationNode(const FAnimationUpdateContext& InContext)
{
	...
	// PreUpdateAnimation has already cleared NotifyQueue and PostUpdateAnimation has not
	// yet dispatched: the only window where adds will survive to dispatch this frame.
	for (FSequencerMixerPendingNotifyBatch& Batch : PendingNotifyBatches)
	{
		NotifyQueue.AddAnimNotifies(/*bSrcIsLeader=*/true, Batch.Notifies, Batch.Weight);
	}
	PendingNotifyBatches.Reset();
}
...
void FSequencerMixedAnimInstanceProxy::StagePendingNotifyBatches(TArray<FSequencerMixerPendingNotifyBatch>&& InBatches)
{
	PendingNotifyBatches = MoveTemp(InBatches);
}
	...
	struct FPreAnimatedAnimInstanceMixedSkeletalAnimationTraits : FBoundObjectPreAnimatedStateTraits
	{
		...
		void RestorePreAnimatedValue(const KeyType& Object, StorageType& InOutCachedValue, const FRestoreStateParams& Params)
		{
			...
			if (PreviousAnimInstance && IsValid(PreviousAnimInstance.Get()) && PreviousAnimInstance->GetSkelMeshComponent() == Component)
			{
				...
				if (Component->AnimScriptInstance != PreviousAnimInstance.Get())
				{
				...
				if (Component->AnimScriptInstance)
				{
					//the skeleton may have changed so need to recalc required bones as needed.
					if (Component->GetSkeletalMeshAsset() && Component->AnimScriptInstance->CurrentSkeleton != Component->GetSkeletalMeshAsset()->GetSkeleton())
					{
				...
				if (bNeedsReInit)
				{
					// When Changing anim instances or skeletons, RecalcRequiredBones and UpdateMorphTargets need to be called, and InitializeAnimation needs to be called regardless, when changing script instances
	...
	public:
		...
		) const
		{
			...
			if (SequencerInstance)
			{
				...
				SequencerInstance->StagePendingNotifyBatches(
					UMovieSceneAnimMixerSystem::ConsumeMixerNotifyBatches(Linker, SkeletalMeshComponent, Target));
			}
			else if (!AnimInstanceTarget->HasFiredWarningForTarget())
			{
				...
				UE_LOGF(LogMovieSceneAnimMixer, Warning,
				       "Unable to bind anim mixer custom instance to skeletal mesh: %ls on actor: %ls. This is usually caused by a conflict with a control rig track. Currently it is unsupported to use the Sequenccer Anim Mixer with a Custom Anim Instance target and control rig tracks, if the actor has an animation blueprint assigned. Please remove one to resolve this issue.", *SkeletalMeshComponent->GetName(), *SkeletalMeshComponent->GetOwner()->GetName())
