...
struct FAnimNode_SequencerMixerTarget final : public FAnimNode_Base
{
	...
	// We expose this so that the anim slot node can (through IAnimGraph_SequencerMixerTargetConnector) be backwards compatible with the mixer when the mixer plugin is enabled
	static void ApplySequencerMixedPose(FPoseContext& Output, FName InTargetName);
