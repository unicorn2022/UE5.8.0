...
void SImTabGroup::Construct(const FArguments& InArgs)
{
	...
	if (TSharedPtr<SDockTab> ParentTab = SlateIM::FSlateIMManager::Get().FindMostRecentContainer<SImTab>())
	{
		TabManager = FGlobalTabmanager::Get()->NewTabManager(ParentTab.ToSharedRef());
	}
	else if (SlateIM::FSlateIMManager::Get().HasCurrentRoot()
		&& SlateIM::FSlateIMManager::Get().GetCurrentRoot().RootWidget.IsValid())
	{
		TabManager = FGlobalTabmanager::Get()->NewTabManager(SlateIM::FSlateIMManager::Get().GetCurrentRoot().RootWidget->GetRootTab());
	}
	else
	{
		ensureAlwaysMsgf(false, TEXT("Missing root widget when creating a tab. Are you missing a Begin Root?"));
		return;
	}

	const TSharedRef<FTabManager::FArea> PrimaryArea = TabManager->NewPrimaryArea();
...
bool SImTabGroup::IsTabForegrounded(const FName& TabId) const
{
	...
	else
	{
		for (const FImTab& Tab : Tabs)
		{
			...
			{
				if (Tab.TabWidget->IsForeground())
				{
					return true;
				}
				
				return (Tab.TabWidget->IsInSidebar() && Tab.TabWidget->IsDrawerOpen());
			}
		}
	}
