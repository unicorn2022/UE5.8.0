...
void SImContextMenuAnchor::OpenMenu(const FVector2D& InScreenPosition, const FWidgetPath* InWidgetPath)
{
	if (IsMenuOpen())
	{
		return;
	...
	OpenedMenu = FSlateApplication::Get().PushMenu(
		AsShared(), 
		InWidgetPath ? *InWidgetPath : FWidgetPath(), 
...
TSharedRef<SWidget> SImContextMenuAnchor::GetMenuContent() const
{
	return MenuWidget.IsValid() ? MenuWidget.ToSharedRef() : SNullWidget::NullWidget;
}
...
FReply SImContextMenuAnchor::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	{
		OpenMenu(MouseEvent.GetScreenSpacePosition(), MouseEvent.GetEventPath());
...
TSharedRef<SWidget> SImContextMenuAnchor::BuildMenu_Recursive(int32& InOutMenuIndex, int32 MenuLevel)
{
	...
	while(InOutMenuIndex < NumMenuItems)
	{
		...
		// This has already been done in the switch statement for SubMenus.
		if (MenuItemData.Type != EMenuType::SubMenu)
		{
			++InOutMenuIndex;
		}
	}
