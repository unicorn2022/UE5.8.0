...
namespace SlateIM
{
	...
	{
		GraphLine(Points, {.XViewRange = XViewRange, .YViewRange = YViewRange, .LineColor = LineColor, .LineThickness = LineThickness});
	}

	void GraphLine(const TArrayView<FVector2D>& Points, const FGraphLinePointsParams& Params)
	{
		TSharedPtr<SImGraph> GraphWidget = GetCurrentGraphWidget();
		if (ensureMsgf(GraphWidget, TEXT("Calling SlateIM::GraphLine() but the last widget is not a graph widget")))
		{
			GraphWidget->AddLineGraph(Points, Params);
		}
	}
	...
	{
		GraphLine(Values, {.ViewRange = ViewRange, .LineColor = LineColor, .LineThickness = LineThickness});
	}

	void GraphLine(const TArrayView<double>& Values, const FGraphLineValuesParams& Params)
	{
		TSharedPtr<SImGraph> GraphWidget = GetCurrentGraphWidget();
		if (ensureMsgf(GraphWidget, TEXT("Calling SlateIM::GraphLine() but the last widget is not a graph widget")))
		{
			GraphWidget->AddLineGraph(Values, Params);
		}
	}
}
