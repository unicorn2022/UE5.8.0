...
{	
	...
	{
		return BeginTab(TabId, {.TabIcon = TabIcon, .TabTitle = TabTitle});
	}

	bool BeginTab(const FName& TabId, const FTabParams& Params)
	{
		TSharedPtr<SImTabGroup> TabGroup = FSlateIMManager::Get().GetCurrentContainer<SImTabGroup>();
		...
		// Note widget scope must fall out of scope before we push container or else we are pushing this new container into itself instead of into its parent
		{
			...
			if (!ContainerWidget || ContainerWidget->TabId != TabId)
			{
				...
				ContainerWidget->SetOnTabDrawerOpened(FSimpleDelegate::CreateSP(ContainerWidget.Get(), &SImTab::OnDrawerOpened));
				ContainerWidget->SetOnTabDrawerClosed(FSimpleDelegate::CreateSP(ContainerWidget.Get(), &SImTab::OnDrawerClosed));
				Scope.UpdateWidget(ContainerWidget);

				// Params.TabIcon and Params.TabTitle are not updated after construction, this could be supported in the future but is not currently part of the Tab Manager API
				ContainerWidget->TabIcon = Params.TabIcon;
				ContainerWidget->TabTitle = Params.TabTitle;
			}

			if (!FSlateIMManager::Get().GetMutableCurrentRoot().Tabs.Contains(TabId))
			{
				FSlateIMManager::Get().GetMutableCurrentRoot().Tabs.Add(TabId, ContainerWidget);
			}
		}
	...
	void ActivateTab(const FName& TabId)
	{
		if (ensureMsgf(FSlateIMManager::Get().HasCurrentRoot(), TEXT("ActivateTab called without a current root. Have you started a root?!")))
		{
			FSlateIMManager::Get().GetMutableCurrentRoot().TabsToActivate.Add(TabId);
		}
	}
}
