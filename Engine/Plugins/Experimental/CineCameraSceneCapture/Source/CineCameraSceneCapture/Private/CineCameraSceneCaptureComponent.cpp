...
#define CINE_CAMERA_INVALID_PARENT_WARNING_FMT "Cine Capture requires to be parented to Cine Camera Component. Cine Capture %s on Actor \"%s\" will be disabled until it is parented to Cine Camera Actor."
...
#endif
		UE_LOG(LogCineCapture, Warning, TEXT(CINE_CAMERA_INVALID_PARENT_WARNING_FMT), *GetName(), *GetOuter()->GetName());
	}
}
