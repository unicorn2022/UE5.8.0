...
namespace Chaos
{
	...
	void RegisterAdapter(FComponentCacheAdapter* InAdapter)
	{
		...
		for(FComponentCacheAdapter* Adapter : Adapters)
		{
			...
			{
				UE_LOGF(LogCacheAdapter,
					   Error,
					   "Attempted to register a cache adapter with GUID %ls however the desired class %ls is "
							"already registered at the requested priority (%u) by a cache adapter with GUID %ls",
