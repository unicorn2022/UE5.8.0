			...
			for (auto& ChannelData : NewData.PendingChannelsData)
			{
				...
				else
				{
					for (int32 PendingIndex = 0; PendingIndex < NumParticles; ++PendingIndex)
					{
						...
						if (TargetCurve->RichCurves.IsValidIndex(*ChannelIndex))
						{
...
FCacheUserToken UChaosCache::BeginRecord(const UPrimitiveComponent* InComponent, FGuid InAdapterId, const FTransform& SpaceTransform)
{
	...
	if(OtherRecordersCount == 0)
	{
		...
		{
			UE_LOGF(LogChaosCache, Warning, "Failed to open cache %ls for record, it was the only recorder but the cache was open for playback.", *GetPathName());
			CurrentRecordCount--;
		}
	}
	else
	{
		UE_LOGF(LogChaosCache, Warning, "Failed to open cache %ls for record, the cache was already open for record.", *GetPathName());
		CurrentRecordCount--;
	}
...
void UChaosCache::EndRecord(FCacheUserToken& InOutToken)
{
	...
	else
	{
		if(InOutToken.Owner)
		{
			UE_LOGF(LogChaosCache, Warning, "Attempted to close a recording session with a token from an external cache.");
		}
		else
		{
			UE_LOGF(LogChaosCache, Warning, "Attempted to close a recording session with an invalid token");
		}
	}
...
void UChaosCache::EndPlayback(FCacheUserToken& InOutToken) const
{
	...
	else
	{
		if(InOutToken.Owner)
		{
			UE_LOGF(LogChaosCache, Warning, "Attempted to close a playback session with a token from an external cache.");
		}
		else
		{
			UE_LOGF(LogChaosCache, Warning, "Attempted to close a playback session with an invalid token");
		}
	}
}
...
FCacheEvaluationResult UChaosCache::Evaluate(const FCacheEvaluationContext& InContext, const TArray<FTransform>* MassToLocalTransforms) const
{
	...
	if(CurrentPlaybackCount.load() == 0)
	{
		...
		UE_LOGF(LogChaosCache, Warning, "Attempted to evaluate a cache that wasn't opened for playback");
		return Result;
	}
...
void UChaosCache::BuildSpawnableFromComponent(const UPrimitiveComponent* InComponent, const FTransform& SpaceTransform)
{
	...
	// duplication of the component also copy the attach parent reference, but if we keep it this causes crashes because the garbage collection will have a dangling reference
