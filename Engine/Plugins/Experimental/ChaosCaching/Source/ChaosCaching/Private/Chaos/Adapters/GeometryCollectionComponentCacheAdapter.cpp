...
namespace Chaos
{
	...
	namespace GeometryCollectionCacheAdapter::CVars
	{
		bool bUseKinematicTarget = true;
		FAutoConsoleVariableRef CVARUseKinematicTarget(TEXT("p.Chaos.Cache.GeometryCollectionPlayUseKinematicTarget"), bUseKinematicTarget,
			TEXT("When in Play cache mode, use kinematic target to move the geometry collection rigid body instead of setting their position directly [def:true]"));
	}
	...
	void FGeometryCollectionCacheAdapter::Record_PostSolve(UPrimitiveComponent* InComp, const FTransform& InRootTransform, FPendingFrameWrite& OutFrame, Chaos::FReal InTime) const
	{
		...
		if ((Proxy->GetSimParameters().bGenerateTrailingData || Proxy->GetSimParameters().bGenerateGlobalTrailingData) && TrailingDataArray && ProxyCachedEventData.ProxyTrailingDataIndices)
		{
															...
															TArray<TPBDRigidParticleHandle<Chaos::FReal, 3>*>& OutUpdatedRigids) const
	{
		...
		if ((Proxy->GetSimParameters().bGenerateTrailingData || Proxy->GetSimParameters().bGenerateGlobalTrailingData) && TrailingEvents)
		{
			...
			for (const FCacheEventHandle& Handle : *TrailingEvents)
			{
				if (const FTrailingEvent* Event = Handle.GetConst<FTrailingEvent>())
				{
					if (Chaos::FPBDRigidClusteredParticleHandle* Particle = Proxy->GetParticle_Internal(Event->Index))
					{
						...
						CachedTrail.SetEmitterFlag(Proxy->GetSimParameters().bGenerateTrailingData, Proxy->GetSimParameters().bGenerateGlobalTrailingData);
		...
		for(int32 Index = 0; Index < NumTransforms; ++Index)
		{
			...
			if (Chaos::FPBDRigidClusteredParticleHandle* Handle = Proxy->GetParticle_Internal(ParticleIndex))
			{
				...
				if (GeometryCollectionCacheAdapter::CVars::bUseKinematicTarget)
				{
					FKinematicTarget Target = FKinematicTarget::MakePositionTarget(WorldTransform);
					Solver->GetEvolution()->SetParticleKinematicTarget(Handle, Target);
				}
				else
				{
				...
				if(FRigidParticle* ClusterParent = Handle->ClusterIds().Id)
				{
					if(FClusterParticle* Parent = ClusterParent->CastToClustered())
					{
						if(Parent->InternalCluster())
						{
							...
							if (GeometryCollectionCacheAdapter::CVars::bUseKinematicTarget)
							{
								FKinematicTarget Target = FKinematicTarget::MakePositionTarget(Result);
								Solver->GetEvolution()->SetParticleKinematicTarget(Parent, Target);
							}
							else
							{
	...
	void FGeometryCollectionCacheAdapter::SetRestState(UPrimitiveComponent* InComponent, UChaosCache* InCache, const FTransform& InRootTransform, Chaos::FReal InTime) const
	{
		...
		if (UGeometryCollectionComponent* Comp = CastChecked<UGeometryCollectionComponent>(InComponent))
		{
			...
			FTransform WorldToComponent = Comp->GetComponentTransform().Inverse();
			FTransform ActorToComponent = ActorToWorld * WorldToComponent;
			const FTransform CompScaleTransform = FTransform(FQuat::Identity, FVector::ZeroVector, Comp->GetComponentTransform().GetScale3D());
			const bool bIsScaledComponent = !CompScaleTransform.GetScale3D().Equals(FVector::OneVector);
			...
			if (const UGeometryCollection* RestCollection = Comp->GetRestCollection())
			{
				...
				if (bIsScaledComponent)
				{
					const TManagedArray<FTransform>& MassToLocal = GeometryCollection->GetAttribute<FTransform>("MassToLocal", FGeometryCollection::TransformGroup);
					for (int32 CacheIdx = 0; CacheIdx < NumCacheTransforms; ++CacheIdx)
					{
						const int32 TransformIdx = EvaluatedResult.ParticleIndices[CacheIdx];
						FTransform MassToLocalTransform = MassToLocal[TransformIdx];
						RestTransforms[TransformIdx] = MassToLocalTransform.Inverse() * CompScaleTransform * MassToLocalTransform * RestTransforms[TransformIdx];
					}
				}
	...
	void FGeometryCollectionCacheAdapter::EmitEvents(UPrimitiveComponent* InComponent, UChaosCache* InCache, FPlaybackTickRecord& TickRecord) const
	{
		EnsureIsInGameThreadContext();
		...
		if (UGeometryCollectionComponent* Comp = CastChecked<UGeometryCollectionComponent>(InComponent))
		{
			...
			EmitBreakingEvents_GameThread(*Comp, EvaluatedResult);
			EmitCollisionEvents_GameThread(*Comp, EvaluatedResult);
		}
	}

	void FGeometryCollectionCacheAdapter::EmitCollisionEvents_GameThread(UPrimitiveComponent& InComponent, const FCacheEvaluationResult& Results) const
	{
		if (const TArray<FCacheEventHandle>* CollisionEvents = Results.Events.Find(FCollisionEvent::EventName))
		{
			UWorld* ComponentWorld = InComponent.GetWorld();
			if (FPhysScene* PhysicsScene = ComponentWorld ? ComponentWorld->GetPhysicsScene() : nullptr)
			{
				UChaosEventRelay* ChaosEventRelay = PhysicsScene->GetChaosEventRelay();
				if (ChaosEventRelay && ChaosEventRelay->OnCollisionEvent.IsBound())
				{
					const FTransform ComponentToWorld = InComponent.GetComponentToWorld();

					TArray<FCollisionChaosEvent> CollisionsEventsToEmit;
					for (const FCacheEventHandle& Handle : *CollisionEvents)
					{
						if (const FCollisionEvent* CachedEvent = Handle.GetConst<FCollisionEvent>())
						{
							FCollisionChaosEvent& CollisionEvent = CollisionsEventsToEmit.AddDefaulted_GetRef();
							CollisionEvent.Location = ComponentToWorld.TransformPosition(CachedEvent->Location);
							CollisionEvent.AccumulatedImpulse = ComponentToWorld.TransformVectorNoScale(CachedEvent->AccumulatedImpulse);
							CollisionEvent.Normal = ComponentToWorld.TransformVectorNoScale(CachedEvent->Normal);
							CollisionEvent.PenetrationDepth = CachedEvent->PenetrationDepth;

							// since we don't know what component each body comes from, we set both to the same
							// this will allow at least the event handler to be able to query component related information like materials
							CollisionEvent.Body1.Component = &InComponent; 
							CollisionEvent.Body2.Component = &InComponent;

							CollisionEvent.Body1.Velocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->Velocity1);
							CollisionEvent.Body1.DeltaVelocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->DeltaVelocity1);
							CollisionEvent.Body1.AngularVelocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->AngularVelocity1);
							CollisionEvent.Body1.Mass = CachedEvent->Mass1;
							CollisionEvent.Body1.PhysMaterial = nullptr; // non-cached information
							CollisionEvent.Body1.BodyIndex = INDEX_NONE; // non-cached information
							CollisionEvent.Body1.BoneName = NAME_None; // non-cached information

							CollisionEvent.Body2.Velocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->Velocity2);
							CollisionEvent.Body2.DeltaVelocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->DeltaVelocity2);
							CollisionEvent.Body2.AngularVelocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->AngularVelocity2);
							CollisionEvent.Body2.Mass = CachedEvent->Mass2;
							CollisionEvent.Body2.PhysMaterial = nullptr; // non-cached information
							CollisionEvent.Body2.BodyIndex = INDEX_NONE; // non-cached information
							CollisionEvent.Body2.BoneName = NAME_None; // non-cached information
						}
					}
					if (CollisionsEventsToEmit.Num() > 0)
					{
						ChaosEventRelay->DispatchPhysicsCollisionEvents(CollisionsEventsToEmit);
					}
				}
			}
		}
	...
	void FGeometryCollectionCacheAdapter::EmitBreakingEvents_GameThread(UPrimitiveComponent& InComponent, const FCacheEvaluationResult& Results) const
	{
		if (const TArray<FCacheEventHandle>* BreakingEvents = Results.Events.Find(FBreakingEvent::EventName))
		{
			UWorld* ComponentWorld = InComponent.GetWorld();
			if (FPhysScene* PhysicsScene = ComponentWorld ? ComponentWorld->GetPhysicsScene() : nullptr)
			{
				UChaosEventRelay* ChaosEventRelay = PhysicsScene->GetChaosEventRelay();
				if (ChaosEventRelay && ChaosEventRelay->OnBreakEvent.IsBound())
				{
					const FTransform ComponentToWorld = InComponent.GetComponentToWorld();

					TArray<FChaosBreakEvent> BreakEventsToEmit;
					for (const FCacheEventHandle& Handle : *BreakingEvents)
					{
						if (const FBreakingEvent* CachedEvent = Handle.GetConst<FBreakingEvent>())
						{
							FChaosBreakEvent& BreakEvent = BreakEventsToEmit.AddDefaulted_GetRef();
							BreakEvent.Component = &InComponent;
							BreakEvent.Location = ComponentToWorld.TransformPosition(CachedEvent->Location);
							BreakEvent.Orientation = ComponentToWorld.TransformRotation(CachedEvent->Orientation);
							BreakEvent.Velocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->Velocity);
							BreakEvent.AngularVelocity = ComponentToWorld.TransformVectorNoScale(CachedEvent->AngularVelocity);
							const FVector Extents = (CachedEvent->BoundingBoxMax - CachedEvent->BoundingBoxMin) * 0.5;
							const FBox Bounds(CachedEvent->Location - Extents, CachedEvent->Location + Extents);
							const FBox TransformedBounds = Bounds.TransformBy(ComponentToWorld);
							BreakEvent.Extents = TransformedBounds.GetExtent();
							BreakEvent.Mass = CachedEvent->Mass;
							BreakEvent.Index = CachedEvent->Index;
							BreakEvent.bFromCrumble = false; // todo(chaos) : maybe we should cache this informnation too in the future ? 
						}
					}
					if (BreakEventsToEmit.Num() > 0)
					{
						ChaosEventRelay->DispatchPhysicsBreakEvents(BreakEventsToEmit);
					}
				}
			}
		}
	...
	void FGeometryCollectionCacheAdapter::RegisterEventHandler(FGeometryCollectionPhysicsProxy* Proxy)
	{
		...
		if (Chaos::FPhysicsSolver* Solver = Proxy->GetSolver<Chaos::FPhysicsSolver>())
		{
			...
			if (Chaos::FEventManager* EventManager = Solver->GetEventManager())
			{
				...
				if (SimulationParameters.bGenerateTrailingData || SimulationParameters.bGenerateGlobalTrailingData)
				{
			...
			for (const TPair<IPhysicsProxyBase*, FCachedEventData>& CacheDataPair : CachedData)
			{
				...
				if (Proxy != OtherGCProxy && OtherGCProxy)
				{
					...
					if (OtherProxy->GetSolver<Chaos::FPhysicsSolver>() == Solver)
					{
						...
						bListenToTrailingEvents |= (OtherSimParams.bGenerateTrailingData || OtherSimParams.bGenerateGlobalTrailingData);
					}
				}
			}
