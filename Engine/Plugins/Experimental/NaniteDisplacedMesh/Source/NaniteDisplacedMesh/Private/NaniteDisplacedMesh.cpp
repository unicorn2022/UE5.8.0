...
bool FNaniteBuildAsyncCacheTask::BuildData(const UE::FSharedString& Name, const UE::DerivedData::FCacheKey& Key)
{
	...
	if (!IsValid(BaseMesh))
	{
		UE_LOGF(LogNaniteDisplacedMesh, Error, "Cannot find a valid base mesh to build the displaced mesh asset.");
		return false;
	}

	if (!BaseMesh->IsMeshDescriptionValid(0))
	{
		UE_LOGF(LogNaniteDisplacedMesh, Error, "Cannot find a valid mesh description to build the displaced mesh asset.");
		return false;
	}
	...
	if (!UE::NaniteDisplacedMesh::DisplaceNaniteMesh(
			Parameters,
			InputMeshData.Vertices,
			InputMeshData.TriangleIndices,
	...
	{
		UE_LOGF(LogNaniteDisplacedMesh, Error, "Failed to build perform displacement mapping for Nanite displaced mesh asset.");
		return false;
	}
	...
	{
		UE_LOGF(LogNaniteDisplacedMesh, Error, "Failed to build Nanite for displaced mesh asset.");
		return false;
	}
...
FIoHash UNaniteDisplacedMesh::CreateDerivedDataKeyHash(const ITargetPlatform* TargetPlatform)
{
	...
	FGuid DisplacedMeshVersionGuid(0x30E6E166, 0xB41F49F7, 0xBD245D08, 0x62B124DB);
	...
	for( auto& DisplacementMap : Parameters.DisplacementMaps )
	{
		...
		if (DisplacementMap.UVChannel != 0 || DisplacementMap.MaskUVChannel != 1)
		{
			// Serializing a unique guid here to differenciate these optional settings being
			// serialized from potential future bools that could be added to the serialization past this point.
			FGuid CustomUVChannelGuid(0x52fe3faf, 0xff02595b, 0x65b59ac1, 0x458d62f5);
			Writer << CustomUVChannelGuid;
			Writer << DisplacementMap.UVChannel;
			Writer << DisplacementMap.MaskUVChannel;
		}
	}
