...
static FVector3f UserGetDisplacement(
	...
	int32 MaterialIndex,
	TArrayView<UE::Geometry::FDisplacementMap> DisplacementMaps,
	TArrayView<int32> MappedUVs,
	int32 MaskUVChannel,
	UE::NaniteDisplacedMesh::EDisplacementOptions Options)
{
	using namespace UE::NaniteDisplacedMesh;

	const int32 DisplacementIndex = FMath::IsWithin(MaskUVChannel, 0, static_cast<int32>(MAX_STATIC_TEXCOORDS)) ? FMath::FloorToInt(Vert0.UVs[MaskUVChannel].X) : 0;

	if (!DisplacementMaps.IsValidIndex(DisplacementIndex) || !MappedUVs.IsValidIndex(DisplacementIndex))
	{
		return FVector3f::ZeroVector;
	}
	const int32 SamplingUVChannel = MappedUVs[DisplacementIndex];
	if (!FMath::IsWithin(SamplingUVChannel, 0, static_cast<int32>(MAX_STATIC_TEXCOORDS)))
	{
		return FVector3f::ZeroVector;
	}

	FVector2f UV;
	UV  = Vert0.UVs[SamplingUVChannel] * Barycentrics.X;
	UV += Vert1.UVs[SamplingUVChannel] * Barycentrics.Y;
	UV += Vert2.UVs[SamplingUVChannel] * Barycentrics.Z;

	float DisplacementWeight = 1.0f;
	if (EnumHasAnyFlags(Options, EDisplacementOptions::IgnoreNonNormalizedDisplacementUVs))
	{
	...
	const float Displacement = DisplacementMaps[DisplacementIndex].Sample(UV);
	...
	int32 MaskUVChannel)
{
	...
	if (DisplacementMaps.IsValidIndex(DisplacementIndex) && MappedUVs.IsValidIndex(DisplacementIndex))
	{
		...
		{
			FVector2f UV;
			UV  = Vert0.UVs[SamplingUVChannel] * Barycentrics[k].X;
			UV += Vert1.UVs[SamplingUVChannel] * Barycentrics[k].Y;
			UV += Vert2.UVs[SamplingUVChannel] * Barycentrics[k].Z;
	...
	int32 MaskUVChannel)
{
	...
	if( DisplacementMaps.IsValidIndex( DisplacementIndex ) )
	{
		...
		for( int k = 0; k < 3; k++ )
		{
			UVs[k]  = Vert0.UVs[SamplingUVChannel] * Barycentrics[k].X;
			UVs[k] += Vert1.UVs[SamplingUVChannel] * Barycentrics[k].Y;
			UVs[k] += Vert2.UVs[SamplingUVChannel] * Barycentrics[k].Z;

			UVs[k].X *= (float)DisplacementMaps[ DisplacementIndex ].GetSizeX();
			UVs[k].Y *= (float)DisplacementMaps[ DisplacementIndex ].GetSizeY();
		}
...
bool UE::NaniteDisplacedMesh::DisplaceNaniteMesh(
	const FNaniteDisplacedMeshParams& Parameters,
	FMeshBuildVertexData& Verts,
	TArray< uint32 >& Indexes,
	TArray< int32 >& MaterialIndexes,
	FBounds3f& VertexBounds,
	EDisplacementOptions Options
)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UE::NaniteDisplacedMesh::DisplaceNaniteMesh);
	...
	// START - MESH PREPARE
	const int32 Map0MaskUV = Parameters.DisplacementMaps.IsValidIndex(0) ? Parameters.DisplacementMaps[0].MaskUVChannel : 1;
	const int32 SanitizedMaskUV = FMath::IsWithin(Map0MaskUV, static_cast<int32>(INDEX_NONE), Verts.UVs.Num()) ? Map0MaskUV : 1;

	TArray<FVector3f> VertDisplacements;
	if (Verts.UVs.IsValidIndex(SanitizedMaskUV) || SanitizedMaskUV == INDEX_NONE)
	{
		...
		[&](int32 VertIndex)
		{
			...
			{
				if (SanitizedMaskUV != INDEX_NONE && Verts.UVs[SanitizedMaskUV][GuideVertIndex].Y >= 0.0f)
				{
					continue;
				}
	...
	TArray< UE::Geometry::FDisplacementMap > DisplacementMaps;
	for( auto& DisplacementMap : Parameters.DisplacementMaps )
	{
		...
		if (DisplacementMap.Texture)
		{
			if (IsValid(DisplacementMap.Texture) && DisplacementMap.Texture->Source.IsValid())
			{
				...
				{
					DisplacementMaps.Add(UE::Geometry::FDisplacementMap(
						MoveTemp(FirstMipImage.RawData),
						FImageCoreUtils::ConvertToTextureSourceFormat(FirstMipImage.Format),
						FirstMipImage.SizeX,
						FirstMipImage.SizeY, 
				...
				else
				{
					...
					UE_LOGF( LogStaticMesh, Error, "Adaptive tessellate failed because it couldn't read the displacement texture source data");
					return false;
				}
			}
			else
			{
				...
				UE_LOGF( LogStaticMesh, Error, "Adaptive tessellate failed because it couldn't use the displacement texture");
				return false;
			}
		}
		else
	...
	TArray<int32> MappedUVs;
	Algo::Transform(Parameters.DisplacementMaps, MappedUVs,
		[](const FNaniteDisplacedMeshDisplacementMap& DisplacementMap)
		{
			return FMath::IsWithin(DisplacementMap.UVChannel, 0, static_cast<int32>(MAX_STATIC_TEXCOORDS)) ? DisplacementMap.UVChannel : 0;
		});
	...
	// CurveRadius not supported
	check(Verts.CurveRadius.Num() == 0);
	...
	UE_LOGF( LogStaticMesh, Log, "Adaptive tessellate [%.2fs], tris: %i", FPlatformTime::ToMilliseconds( Time1 - Time0 ) / 1000.0f, Indexes.Num() / 3 );
