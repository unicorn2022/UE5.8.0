...
struct FNaniteDisplacedMeshDisplacementMap
{
	...
	/**
	 * UV channel used for sampling the displacement texture.
	 */
	UPROPERTY(EditAnywhere, Category = Texture, AdvancedDisplay, meta = (ClampMin = 0, ClampMax = 7))
	int32 UVChannel = 0;
	
	/**
	 * UV channel used for masking the displacement. If the value is -1, then no masking is applied.
	 */
	UPROPERTY(EditAnywhere, Category = Texture, AdvancedDisplay, meta = (ClampMin = -1, ClampMax = 7))
	int32 MaskUVChannel = 1;

	bool operator==(const FNaniteDisplacedMeshDisplacementMap& Other) const = default;
	bool operator!=(const FNaniteDisplacedMeshDisplacementMap& Other) const = default;
...
struct FNaniteDisplacedMeshParams
{
	...
	/** Controls the precisions of the tesselation, lower values leads to higher precision and denser meshes. */
	UPROPERTY(EditAnywhere, Category = Mesh, meta = (ClampMin = 0.1, ClampMax = 1))
	float RelativeError = 0.03f;
	...
	bool operator==(const FNaniteDisplacedMeshParams& Other) const = default;
	bool operator!=(const FNaniteDisplacedMeshParams& Other) const = default;
