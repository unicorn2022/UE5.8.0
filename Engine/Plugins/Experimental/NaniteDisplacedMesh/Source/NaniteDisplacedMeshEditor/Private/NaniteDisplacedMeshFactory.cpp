		...
		if (UNaniteDisplacedMesh* LoadedDisplacedMesh = LoadObject<UNaniteDisplacedMesh>(nullptr, *DisplacedAssetPath, nullptr, LOAD_Quiet))
		{
			...
			else
			{
				...
				UE_LOGF(
					...
					Error,
					"The NaniteDisplacementMesh parameters doesn't match the guid from its name (Current parameters: %ls). Updating parameters of (%ls). Consider saving the displaced mesh again to remove this error.",
	...
	else if (bCanLinkAgainstNewAsset)
	{
		...
		else
		{
			UE_LOGF(
				LogNaniteDisplacedMesh,
				Error,
				"Failed to create asset for %ls in folder %.*ls. Consult log for more details",
				*DisplacedMeshName,
	...
	if (UE::NaniteDisplacedMesh::Private::Factory::CVarUseNewAggregateID->GetBool())
	{
		...
		for (int32 Index = 0; Index < DisplacedMeshParams.DisplacementMaps.Num(); ++Index)
		{
			...
			if (IsValid(DisplacementMap.Texture) && DisplacementMap.Magnitude > 0.f)
			{
				...
				{
					// Serializing a custom string to make sure the optional UV Channel properties can be
					// differentiated from potential future bool serialized past this point.
					FString CustomUVChannelIdentifier = TEXT("CustomUVChannel");
					IdBuilder << CustomUVChannelIdentifier;
					IdBuilder << DisplacementMap.UVChannel;
					IdBuilder << DisplacementMap.MaskUVChannel;
				}
			}
		}
	}
