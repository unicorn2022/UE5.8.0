...
{
	// Mirrors UFortEditorValidatorSubsystem::GetAllEditorOnlyPlugins: a plugin is treated as
	// editor-only if either (a) its .uplugin sets the non-canonical "EditorOnly": true field,
	// or (b) the project's plugin reference has it enabled but disabled for both Client and
	// Server build targets.
	TArray<FString> GatherEditorOnlyPluginMounts()
	{
		TArray<FString> Mounts;
	#if WITH_EDITOR
		TSet<FString> EditorOnlyPluginNames;

		for (const TSharedRef<IPlugin>& Plugin : IPluginManager::Get().GetEnabledPlugins())
		{
			bool bEditorOnly = false;
			if (const TSharedPtr<FJsonObject>& DescriptorJson = Plugin->GetDescriptorJson())
			{
				DescriptorJson->TryGetBoolField(TEXT("EditorOnly"), bEditorOnly);
			}
			if (bEditorOnly)
			{
				EditorOnlyPluginNames.Add(Plugin->GetName());
			}
		}
		...
		if (const FProjectDescriptor* Project = IProjectManager::Get().GetCurrentProject())
		{
			...
			{
				if (PluginRef.bEnabled
					&& !PluginRef.IsEnabledForTarget(EBuildTargetType::Client)
					&& !PluginRef.IsEnabledForTarget(EBuildTargetType::Server))
				{
					EditorOnlyPluginNames.Add(PluginRef.Name);
				}
			}
		}

		Mounts.Reserve(EditorOnlyPluginNames.Num());
		for (const FString& PluginName : EditorOnlyPluginNames)
		{
			if (TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(PluginName))
			{
				if (Plugin->CanContainContent())
				{
					// GetMountedAssetPath returns "/PluginName/"; strip trailing '/' for
					// directory-boundary matching against package names.
					FString Mount = Plugin->GetMountedAssetPath();
					while (Mount.EndsWith(TEXT("/")))
					{
	...
	// Expands UProjectPackagingSettings::DirectoriesToNeverCook into long-package-name
	// prefixes ready for directory-boundary matching. Entries in that array are either a
	// full package path (/Game/Foo, /Engine/Foo, /PluginName/Foo) or a path relative to
	// /Game (Foo → /Game/Foo). See ProjectPackagingSettings.h for the documented convention.
	TArray<FString> GatherNeverCookPackagePathPrefixes()
	{
		...
		Prefixes.Reserve(PackagingSettings->DirectoriesToNeverCook.Num());
		for (const FDirectoryPath& Dir : PackagingSettings->DirectoriesToNeverCook)
		{
			if (Dir.Path.IsEmpty())
			{
				continue;
			}
			FString LongPackagePath = Dir.Path.StartsWith(TEXT("/"))
				? Dir.Path
				: FString(TEXT("/Game/")) + Dir.Path;
			while (LongPackagePath.EndsWith(TEXT("/")))
			{
				LongPackagePath.LeftChopInline(1, EAllowShrinking::No);
			}
			if (!LongPackagePath.IsEmpty())
			{
				Prefixes.Add(MoveTemp(LongPackagePath));
			}
		}
		return Prefixes;
	}

	// Directory-boundary StartsWith: matches only if PackagePath equals Prefix exactly
	// or is immediately followed by '/'. Prevents /Game/FooBar from matching /Game/Foo.
	bool PackagePathHasPrefix(const FString& PackagePath, const FString& Prefix)
	{
		if (PackagePath.Len() < Prefix.Len() || !PackagePath.StartsWith(Prefix, ESearchCase::IgnoreCase))
		{
			return false;
		}
		return PackagePath.Len() == Prefix.Len() || PackagePath[Prefix.Len()] == TEXT('/');
	}

	// Returns true when the package should be excluded from displaced-mesh scanning:
	//   - Path is under a DirectoriesToNeverCook entry  (project-configured, never cooked)
	//   - Path is under an editor-only plugin's mount   (plugin never cooked)
	bool ShouldSkipPackage(
		FName PackageName,
		const TArray<FString>& EditorOnlyPluginMounts,
		const TArray<FString>& NeverCookPackagePathPrefixes)
	{
		...
		for (const FString& Prefix : NeverCookPackagePathPrefixes)
		{
			if (PackagePathHasPrefix(PackagePath, Prefix))
			{
				return true;
			}
		}
		for (const FString& Mount : EditorOnlyPluginMounts)
		{
			if (PackagePathHasPrefix(PackagePath, Mount))
			{
				return true;
			}
		}
...
int32 UGenerateNaniteDisplacedMeshCommandlet::Main(const FString& CmdLineParams)
{
	...
	UE_LOGF(LogNaniteDisplacedMesh, Display, "Revision control enabled: %ls", SourceControlHelper.UseSourceControl() ? TEXT("true") : TEXT("false"));
	...
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Use collection filter: %ls", *CollectionFilter);
	...
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Use actor class path filter: %ls", *ActorClassPathFilterString);
	...
	if (GConfig->GetInt(TEXT("CookSettings"), TEXT("MemoryMinFreeVirtual"), ValueInMB, GEditorIni))
	{
		...
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Loaded MemoryMinFreeVirtual from CookSettings (%d MiB)", ValueInMB);
	}
	...
	if (GConfig->GetInt(TEXT("CookSettings"), TEXT("MemoryMaxUsedVirtual"), ValueInMB, GEditorIni))
	{
		...
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Loaded MemoryMaxUsedVirtual from CookSettings (%d MiB)", ValueInMB);
	}
	...
	if (GConfig->GetInt(TEXT("CookSettings"), TEXT("MemoryMinFreePhysical"), ValueInMB, GEditorIni))
	{
		...
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Loaded MemoryMinFreePhysical from CookSettings (%d MiB)", ValueInMB);
	}
	...
	if (GConfig->GetInt(TEXT("CookSettings"), TEXT("MemoryMaxUsedPhysical"), ValueInMB, GEditorIni))
	{
		...
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Loaded MemoryMaxUsedPhysical from CookSettings (%d MiB)", ValueInMB);
	}
	...
	//{
	//	UE_LOGF(LogNaniteDisplacedMesh, Display, "Searching the levels that need to be processed and their dependencies (this may take a while)...");
	//}
	//else
	//{
	UE_LOGF(LogNaniteDisplacedMesh, Display, "Searching all assets (this may take a while)...");
	...
	// Filter out content that can never be part of a cook: editor-only plugin mounts and
	// project-configured DirectoriesToNeverCook entries.
	const TArray<FString> EditorOnlyPluginMounts = UE::NaniteDisplacedMesh::Private::GenerateNaniteDisplacedMesh::GatherEditorOnlyPluginMounts();
	const TArray<FString> NeverCookPackagePathPrefixes = UE::NaniteDisplacedMesh::Private::GenerateNaniteDisplacedMesh::GatherNeverCookPackagePathPrefixes();
	if (EditorOnlyPluginMounts.Num() > 0)
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Excluding content from %d editor-only plugin(s):\n\t%ls", EditorOnlyPluginMounts.Num(), *FString::Join(EditorOnlyPluginMounts, TEXT("\n\t")));
	}
	if (NeverCookPackagePathPrefixes.Num() > 0)
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Excluding content from %d DirectoriesToNeverCook entry(ies):\n\t%ls", NeverCookPackagePathPrefixes.Num(), *FString::Join(NeverCookPackagePathPrefixes, TEXT("\n\t")));
	}
	{
		const int32 PreFilterCount = LevelAssets.Num();
		LevelAssets.RemoveAll([&EditorOnlyPluginMounts, &NeverCookPackagePathPrefixes](const FAssetData& LevelAsset)
		{
			if (UE::NaniteDisplacedMesh::Private::GenerateNaniteDisplacedMesh::ShouldSkipPackage(LevelAsset.PackageName, EditorOnlyPluginMounts, NeverCookPackagePathPrefixes))
			{
				UE_LOGF(LogNaniteDisplacedMesh, Verbose, "Skipping editor-only/NeverCook level: %ls", *LevelAsset.PackageName.ToString());
				return true;
			}
			return false;
		});
		if (const int32 RemovedCount = PreFilterCount - LevelAssets.Num(); RemovedCount > 0)
		{
			UE_LOGF(LogNaniteDisplacedMesh, Display, "Excluded %d editor-only/NeverCook level(s) from the primary scan list", RemovedCount);
		}
	}
		...
		for (const FAssetData& LevelAsset : LevelAssets)
		{
			...
			while (!CurrentDependencies.IsEmpty())
			{
				...
				if (!StopSearchAt.Contains(DependendPackageName))
				{
					...
					// Cut the dep walk at editor-only plugin / NeverCook boundaries — those
					// assets can't be part of a cook so there's no point recursing through them.
					if (UE::NaniteDisplacedMesh::Private::GenerateNaniteDisplacedMesh::ShouldSkipPackage(DependendPackageName, EditorOnlyPluginMounts, NeverCookPackagePathPrefixes))
					{
						UE_LOGF(LogNaniteDisplacedMesh, Verbose, "Skipping editor-only/NeverCook dependency: %ls", *DependendPackageName.ToString());
						continue;
					}
	...
	UE_LOGF(LogNaniteDisplacedMesh, Display, "Processing %d level(s)...", LevelCount);

	UE_LOGF(LogNaniteDisplacedMesh, Display, "Processing %d level(s) from the original query...", LevelAssets.Num());
	uint32 LevelIndex = 0;
	for (const FAssetData& LevelAsset : LevelAssets)
	{
		++LevelIndex;
		UE_LOGF(LogNaniteDisplacedMesh, Display, "-------------------------------------------------------------------");
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Process Level: %ls (%d/%d)", *LevelAsset.GetSoftObjectPath().ToString(), LevelIndex, LevelCount);
		LoadLevel(LevelAsset);
	}

	UE_LOGF(LogNaniteDisplacedMesh, Display, "Processing the %d dependencies of the level(s) ...", LevelAssetsAddedByDependencies.Num());
	...
	for (const FAssetData& LevelAsset : LevelAssetsAddedByDependencies)
	{
		...
		UE_LOGF(LogNaniteDisplacedMesh, Display, "-------------------------------------------------------------------");
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Process Level: %ls (%d/%d)", *LevelAsset.GetSoftObjectPath().ToString(), LevelIndex, LevelCount);
		UE_LOGF(LogNaniteDisplacedMesh, Display, "This level was added because it's a dependency of one of the processed level(s)");
		LoadLevel(LevelAsset);
	}

	Module.OverrideNaniteDisplacedMeshLink.Unbind();

	UE_LOGF(LogNaniteDisplacedMesh, Display, "======================= All levels processed ======================");
	UE_LOGF(LogNaniteDisplacedMesh, Display, "Linked %d unique mesh(es)", LinkedPackageNames.Num());

	UE_LOGF(LogNaniteDisplacedMesh, Display, "-------------------------------------------------------------------");

	if (LinkedPackageFolders.Num() > 0)
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Find existing meshes (with prefix %ls) in %d folder(s):\n\t%ls", LinkedDisplacedMeshAssetNamePrefix, LinkedPackageFolders.Num(), *FString::Join(LinkedPackageFolders, TEXT("\n\t")));
		...
		{
			UE_LOGF(LogNaniteDisplacedMesh, Display, "Found %d existing mesh(es):\n\t%ls", ExistingPackageNames.Num(), *FString::Join(ExistingPackageNames, TEXT("\n\t")));
		}
		else
		{
			UE_LOGF(LogNaniteDisplacedMesh, Display, "No existing meshes found");
		}
		...
		if (AddedPackageNames.Num() > 0)
		{
			...
			UE_LOGF(LogNaniteDisplacedMesh, Display, "Found %d added mesh(es):\n\t%ls", AddedPackageNames.Num(), *FString::Join(AddedPackageNames, TEXT("\n\t")));
		}
		else
		{
			UE_LOGF(LogNaniteDisplacedMesh, Display, "No added meshes found");
		}
		...
		if (OtherAddedPackagesAsArray.Num())
		{
			...
			UE_LOGF(LogNaniteDisplacedMesh, Display, "Modified %d Actor(s) or Level(s):\n\t%ls", OtherAddedPackagesAsArray.Num(), *FString::Join(OtherAddedPackagesAsArray, TEXT("\n\t")));
		}
		...
		{
			UE_LOGF(LogNaniteDisplacedMesh, Display, "Found %d unused mesh(es):\n\t%ls", UnusedPackageNames.Num(), *FString::Join(UnusedPackageNames, TEXT("\n\t")));
		}
		else
		{
			UE_LOGF(LogNaniteDisplacedMesh, Display, "No unused meshes found");
		}
		...
		if (bDeleteUnused)
		{
			...
			{
				UE_LOGF(LogNaniteDisplacedMesh, Display, "Deleting %d unused mesh(es)...", PackagesToDelete.Num());
				if (!SourceControlHelper.Delete(PackagesToDelete))
				{
					UE_LOGF(LogNaniteDisplacedMesh, Error, "Failed to delete unused meshes!");
					bSuccess = false;
				}
			}
			else
			{
				UE_LOGF(LogNaniteDisplacedMesh, Display, "No unused meshes to delete");
			}
		...
		{
			UE_LOGF(LogNaniteDisplacedMesh, Display, "Use submit description: %ls", *SubmitWithDescription);
			...
			{
				UE_LOGF(LogNaniteDisplacedMesh, Display, "Submitting %d changed mesh(es)...", SubmitFilenames.Num());
				if (!SourceControlHelpers::CheckInFiles(SubmitFilenames, SubmitWithDescription))
				{
					UE_LOGF(LogNaniteDisplacedMesh, Error, "Failed to submit changed meshes!");
					bSuccess = false;
				}
			}
			else
			{
				UE_LOGF(LogNaniteDisplacedMesh, Display, "No changed meshes to submit");
			}
	...
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "No meshes linked in processed levels");
	}
...
UNaniteDisplacedMesh* UGenerateNaniteDisplacedMeshCommandlet::OnLinkDisplacedMesh(UNaniteDisplacedMesh* ExistingDisplacedMesh, FValidatedNaniteDisplacedMeshParams&& InParameters, const FNaniteDisplacedMeshLinkParameters& InLinkParameters)
{
	...
	if (NaniteDisplacedMesh != nullptr)
	{
		...
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Linked mesh: %ls", *PackageName);
...
void UGenerateNaniteDisplacedMeshCommandlet::LoadLevel(const FAssetData& AssetData)
{
	...
	{
		if (PersistentLevel->bUseExternalActors)
		{
		...
		else
		{
			...
			{
				if (UPackage* LevelPackage = PersistentLevel->GetPackage())
				{
...
bool UGenerateNaniteDisplacedMeshCommandlet::ShouldFreeMemory() const
{
	...
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Low virtual memory available (%lld MiB). kicking GC.", MemoryStats.AvailableVirtual / 1024 / 1024);
		return true;
	}

	if (MemoryMinFreePhysical > 0 && MemoryStats.AvailablePhysical < MemoryMinFreePhysical)
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Low physical memory available (%lld MiB). kicking GC.", MemoryStats.AvailablePhysical / 1024 / 1024);
		return true;
	}

	if (MemoryMaxUsedVirtual > 0 && MemoryStats.UsedVirtual > MemoryMaxUsedVirtual)
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "High virtual memory usage (%lld MiB). kicking GC.", MemoryStats.UsedVirtual / 1024 / 1024);
		return true;
	}

	if (MemoryMaxUsedPhysical > 0 && MemoryStats.UsedPhysical > MemoryMaxUsedPhysical)
	{
		UE_LOGF(LogNaniteDisplacedMesh, Display, "High physical memory usage (%lld MiB). kicking GC.", MemoryStats.UsedPhysical / 1024 / 1024);
		return true;
	}
...
namespace UE::GenerateNaniteDisplacedMesh::Private
{
	void RunCommandlet(const TArray<FString>& Args)
	{
		...
		UE_LOGF(LogNaniteDisplacedMesh, Display, "Run commandlet GenerateNaniteDisplacedMesh: %ls", *CmdLineParams);
