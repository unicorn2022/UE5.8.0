...
void UGPULightmassSubsystem::PostInitialize()
{
	...
	if (SettingsActor == nullptr)
	{
		UE_LOGF(LogGPULightmass, Warning, "Failed to spawn settings actor in World: %ls", *World->GetName());
	}
}
...
void UGPULightmassSubsystem::Launch()
{
	...
	if (!GPULightmassModule.GetStaticLightingSystemForWorld(World))
	{
		...
		
		{
			...
			{
				UE_LOGF(LogTemp, Log, "Static lighting system is created for world %ls.", *World->GetPathName(World->GetOuter()));
			...
			{
				UE_LOGF(LogTemp, Log, "Tried to create static lighting system for world %ls, but failed", *World->GetPathName(World->GetOuter()));
			}
		}
...
void UGPULightmassSubsystem::Stop()
{
	...
	if (GPULightmassModule.GetStaticLightingSystemForWorld(World))
	{
...

		{
			...
			UE_LOGF(LogTemp, Log, "Static lighting system is removed for world %ls.", *World->GetPathName(World->GetOuter()));
		}
...
AGPULightmassSettingsActor* UGPULightmassSubsystem::GetSettingsActor()
{
	...
	if (TActorIterator<AGPULightmassSettingsActor> It(World, AGPULightmassSettingsActor::StaticClass(), EActorIteratorFlags::SkipPendingKill); It)
	{
		SettingsActor = *It;
	}
...
void UGPULightmassSubsystem::SetRealtime(bool bInRealtime)
{
	...
	{
		UE_LOGF(LogGPULightmass, Warning, "CurrentLevelEditingViewportClient is NULL!");
	}
}
