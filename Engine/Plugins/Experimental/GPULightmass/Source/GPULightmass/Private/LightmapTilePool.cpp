...
FLightmapTilePoolGPU::FLightmapTilePoolGPU(FRHICommandListBase& RHICmdList, int32 NumLayers, FIntPoint InSizeInTiles, FIntPoint TileSize)
	...
	, SizeInTiles(InSizeInTiles)
{
	...
	// UE_LOGF(LogGPULightmass, Log, "LightmapTilePool created with %.2fMB", MemorySize / 1024.0f / 1024.0f);
}

void FLightmapTilePoolGPU::Initialize(FRHICommandListBase& RHICmdList, TArray<FLayerFormatAndTileSize> Layers)
{
	ensure(SizeInTiles.X == SizeInTiles.Y);
