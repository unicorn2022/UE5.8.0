	...
	for (int32 LODIndex = 0; LODIndex < MAX_STATIC_MESH_LODS; LODIndex++)
	{
		...
		for (int32 StaticMeshIndex = 0; StaticMeshIndex < Scene.StaticMeshInstanceRenderStates.Elements.Num(); StaticMeshIndex++)
		{
			...
			if (!RayTracingMaskAndStatus.bAllSegmentsUnlit)
			{
				...
				const FRayTracingGeometry* RayTracingGeometry = Instance.RenderData->RayTracingProxy->LODs[LODIndexToUse].RayTracingGeometry;

				FRayTracingGeometryInstance& RayTracingInstance = RayTracingGeometryInstancesPerLOD[LODIndex].AddDefaulted_GetRef();
				RayTracingInstance.GeometryRHI = RayTracingGeometry->GetRHI();
				...
				for (int32 SegmentIndex = 0; SegmentIndex < MeshBatches.Num(); SegmentIndex++)
				{
					FFullyCachedRayTracingMeshCommandContext CommandContext(MeshCommandStorage, ShaderBindingsPerLOD[LODIndex], RayTracingInstance.GeometryRHI, SegmentIndex, RayTracingGeometry->GetInitializer().bAllowUpdate, SBTAllocation);
		...
		for (int32 InstanceGroupIndex = 0; InstanceGroupIndex < Scene.InstanceGroupRenderStates.Elements.Num(); InstanceGroupIndex++)
		{
			...
			if (!RayTracingMaskAndStatus.bAllSegmentsUnlit)
			{
				...
				const FRayTracingGeometry* RayTracingGeometry = InstanceGroup.ComponentUObject->GetStaticMesh()->GetRenderData()->RayTracingProxy->LODs[LODIndexToUse].RayTracingGeometry;
...
FCachedRayTracingSceneData::~FCachedRayTracingSceneData()
{
	...
	RaytracingSBT.FlushAllocationsToClear(FRHICommandListImmediate::Get());

	// Move OwnedRayTracingInstanceTransforms to RHIThread and it will get destroyed when the lambda is released
	FRHICommandListImmediate::Get().EnqueueLambda([OwnedRayTracingInstanceTransforms = MoveTemp(OwnedRayTracingInstanceTransforms)](FRHICommandList&){});
}
...
bool FSceneRenderState::SetupRayTracingScene(FRDGBuilder& GraphBuilder, FSceneUniformBuffer& SceneUniforms, int32 LODIndex)
{
	...
	bool bBuildAll = true;
	GRayTracingGeometryManager->ProcessBuildRequests(GraphBuilder.RHICmdList, bBuildAll);
#endif // RHI_RAYTRACING
...

	{
		...
		FRayTracingSceneBuildPassParams* PassParameters = GraphBuilder.AllocParameters<FRayTracingSceneBuildPassParams>();
		PassParameters->View = ReferenceView->ViewUniformBuffer;
		PassParameters->Scene = SceneUniforms.GetBuffer(GraphBuilder);

		SCOPED_DRAW_EVENTF(GraphBuilder.RHICmdList, GPULightmassUpdateRayTracingScene, TEXT("GPULightmass UpdateRayTracingScene %d Instances"), StaticMeshInstanceRenderStates.Elements.Num());

		TArray<FRayTracingGeometryInstance, SceneRenderingAllocator>& RayTracingGeometryInstances = GraphBuilder.AllocArray<FRayTracingGeometryInstance>();
		if (LODIndex != INDEX_NONE)
		{
				...
				for (int32 SubY = 0; SubY < Landscape.NumSubsections; SubY++)
				{
					for (int32 SubX = 0; SubX < Landscape.NumSubsections; SubX++)
					{
						...
						if (!Landscape.SectionRayTracingStates[SubSectionIdx].IsValid())
						{
							...
							FRayTracingDynamicGeometryUpdateManager& DynamicGeometryUpdateManager = *GraphBuilder.AllocObject<FRayTracingDynamicGeometryUpdateManager>();
							...
							UpdateParams.MeshBatchesView = MeshBatches;
							UpdateParams.NumVertices = (uint32)FMath::Square(Landscape.SubsectionSizeVerts);
							UpdateParams.VertexBufferSize = (uint32)FMath::Square(Landscape.SubsectionSizeVerts) * (uint32)sizeof(FVector3f);
							UpdateParams.NumTriangles = (uint32)FMath::Square(Landscape.SubsectionSizeVerts - 1) * 2;
							UpdateParams.Geometry = &Landscape.SectionRayTracingStates[SubSectionIdx]->Geometry;
							UpdateParams.Buffer = &Landscape.SectionRayTracingStates[SubSectionIdx]->RayTracingDynamicVertexBuffer;
							UpdateParams.bApplyWorldPositionOffset = false;
							...
							DynamicGeometryUpdateManager.ScheduleUpdates(GraphBuilder, /*bUseTracingFeedback*/ false);

							FRDGBufferRef DynamicGeometryScratchBuffer;
							DynamicGeometryUpdateManager.AddDynamicGeometryUpdatePass(
								GraphBuilder,
								ERDGPassFlags::Compute,
								SceneUniforms.GetBuffer(GraphBuilder),
								ERHIPipeline::Graphics,
								DynamicGeometryScratchBuffer);
						...
						const uint32 SegmentCount = Landscape.SectionRayTracingStates[SubSectionIdx]->Geometry.GetInitializer().Segments.Num();
						...
						const FRayTracingGeometry& RayTracingGeometry = Landscape.SectionRayTracingStates[SubSectionIdx]->Geometry;

						FRayTracingGeometryInstance& RayTracingInstance = RayTracingGeometryInstances[InstanceIndex];
						RayTracingInstance.GeometryRHI = RayTracingGeometry.GetRHI();
						RayTracingInstance.Transforms = MakeArrayView(GraphBuilder.AllocObject<FMatrix>(Landscape.LocalToWorld), 1);
						RayTracingInstance.NumTransforms = 1;
						RayTracingInstance.InstanceContributionToHitGroupIndex = SBTAllocation->GetInstanceContributionToHitGroupIndex(ERayTracingShaderBindingLayer::Base);
						...
						{
							FDynamicRayTracingMeshCommandContext CommandContext(DynamicRayTracingMeshCommandStorage, DynamicRTShaderBindings, RayTracingInstance.GeometryRHI, MeshBatch.SegmentIndex, RayTracingGeometry.GetInitializer().bAllowUpdate, SBTAllocation);
							FLightmapRayTracingMeshProcessor RayTracingMeshProcessor(&CommandContext);
		...
		if (IsRayTracingEnabled())
		{
			...
			RayTracingSceneBuffer = GraphBuilder.RHICmdList.CreateBuffer(SceneCreateDesc);
			RayTracingSceneSRV = GraphBuilder.RHICmdList.CreateShaderResourceView(FShaderResourceViewInitializer(RayTracingSceneBuffer, RayTracingScene, 0));
			...
			FBufferRHIRef ScratchBuffer = GraphBuilder.RHICmdList.CreateBuffer(ScratchCreateDesc);

			FRWBufferStructured InstanceBuffer;
			InstanceBuffer.Initialize(GraphBuilder.RHICmdList, TEXT("LightmassRayTracingInstanceBuffer"), GRHIRayTracingInstanceDescriptorSize, SceneInitializer.MaxNumInstances);
			
			FRWBufferStructured HitGroupContributionsBuffer;
			
			if(GRHIGlobals.RayTracing.RequiresSeparateHitGroupContributionsBuffer)
			{
				HitGroupContributionsBuffer.Initialize(GraphBuilder.RHICmdList, TEXT("LightmassRayTracingHitGroupBuffer"), 4, SceneInitializer.MaxNumInstances);
			}

			GraphBuilder.AddPass(
				RDG_EVENT_NAME("GPULM::RayTracingBuildScene"),
				PassParameters,
				ERDGPassFlags::Compute | ERDGPassFlags::NeverCull,
				[
					RayTracingScene = RayTracingScene,
					RayTracingSceneBuffer = RayTracingSceneBuffer,
					ScratchBuffer = ScratchBuffer,
					MaxNumInstances = SceneInitializer.MaxNumInstances,
					InstanceBufferUAV = InstanceBuffer.UAV,
					InstanceBufferRHI = InstanceBuffer.Buffer,
					bRequiresSeparateHitGroupContributionsBuffer = GRHIGlobals.RayTracing.RequiresSeparateHitGroupContributionsBuffer,
					HitGroupContributionsBufferUAV = HitGroupContributionsBuffer.UAV,
					HitGroupContributionsBufferRHI = HitGroupContributionsBuffer.Buffer,
					RayTracingInstanceBufferBuilder = MoveTemp(RayTracingInstanceBufferBuilder)](FRHICommandListImmediate& RHICmdList) mutable
				{
					...
					RayTracingInstanceBufferBuilder.BuildRayTracingInstanceBuffer(
						...
						nullptr,
						InstanceBufferUAV,
						bRequiresSeparateHitGroupContributionsBuffer ? HitGroupContributionsBufferUAV : nullptr,
						MaxNumInstances,
						nullptr,
						false,
						nullptr,
					...
					BuildParams.InstanceBuffer = InstanceBufferRHI;
					BuildParams.InstanceBufferOffset = 0;

					if (bRequiresSeparateHitGroupContributionsBuffer)
					{
						BuildParams.HitGroupContributionsBuffer = HitGroupContributionsBufferRHI;
			...
			GraphBuilder.RHICmdList.EnqueueLambda([LandscapeTransforms = MoveTemp(LandscapeTransforms)](FRHICommandList&){});
			...
			RayTracingPipelineState = PipelineStateCache::GetAndOrCreateRayTracingPipelineState(GraphBuilder.RHICmdList, PSOInitializer);

			RayTracingSBT.ResetStaticAllocationLock();

			SBT = RayTracingSBT.AllocateTransientRHI(GraphBuilder.RHICmdList, ERayTracingShaderBindingMode::RTPSO, ERayTracingHitGroupIndexingMode::Allow, PSOInitializer.GetMaxLocalBindingDataSize());
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
				...
				GraphBuilder.AddPass(
					RDG_EVENT_NAME("GPULM::SetRayTracingBindings"),
					PassParameters,
					ERDGPassFlags::Compute | ERDGPassFlags::NeverCull,
					[BindingWriter = MoveTemp(BindingWriter), RayTracingPipelineState = RayTracingPipelineState, SBT = SBT](FRHICommandListImmediate& RHICmdList) mutable
					{
						...
						FRayTracingLocalShaderBindings* MergedBindings = static_cast<FRayTracingLocalShaderBindings*>(RHICmdList.Bypass()
...
void FLightmapRenderer::Finalize(FRDGBuilder& GraphBuilder)
{
	...
	// Upload & copy converged tiles directly
	{
		...
		if (TileUploadRequests.Num() > 0)
		{
			...
			{
				UploadTilePoolGPU = MakeUnique<FLightmapTilePoolGPU>(GraphBuilder.RHICmdList, 4, FIntPoint(NewSize, NewSize), FIntPoint(GPreviewLightmapPhysicalTileSize, GPreviewLightmapPhysicalTileSize));
				UE_LOGF(LogGPULightmass, Log, "Resizing GPULightmass upload tile pool to (%d, %d) %dx%d", NewSize, NewSize, NewSize * GPreviewLightmapPhysicalTileSize, NewSize * GPreviewLightmapPhysicalTileSize);
			}
...

		{
			...
			while (!bScratchAllocationSucceeded)
			{
				...
				if (!bScratchAllocationSucceeded)
				{
					...
					ScratchTilePoolGPU = MakeUnique<FLightmapTilePoolGPU>(GraphBuilder.RHICmdList, 3, FIntPoint(NewSize, NewSize), FIntPoint(GPreviewLightmapPhysicalTileSize, GPreviewLightmapPhysicalTileSize));
					UE_LOGF(LogGPULightmass, Log, "Resizing GPULightmass scratch tile pool to (%d, %d) %dx%d", NewSize, NewSize, NewSize * GPreviewLightmapPhysicalTileSize, NewSize * GPreviewLightmapPhysicalTileSize);
				}
			}
		}
	...
	// Perform readback on any potential converged tiles
	{
		...
		if (ConvergedTileRequests.Num() > 0)
		{
			...
			for (uint32 GPUIndex = 0; GPUIndex < GNumExplicitGPUsForRendering; GPUIndex++)
			{
				...
				{
					LightmapReadbackGroup.ReadbackTilePoolGPU = MakeUnique<FLightmapTilePoolGPU>(GraphBuilder.RHICmdList, 4, FIntPoint(NewSize, NewSize), FIntPoint(GPreviewLightmapPhysicalTileSize, GPreviewLightmapPhysicalTileSize));
...
void FLightmapRenderer::BackgroundTick(FRHICommandListImmediate& RHICmdList)
{
	{
	...
	if (bIsViewportNonRealtime && !bWasRunningAtFullSpeed)
	{
		...
		UE_LOGF(LogGPULightmass, Log, "GPULightmass is now running at full speed");
	}

	if (!bIsViewportNonRealtime && bWasRunningAtFullSpeed)
	{
		bWasRunningAtFullSpeed = false;
		UE_LOGF(LogGPULightmass, Log, "GPULightmass is now throttled for realtime preview");
	}
	...
	if (!bOnlyBakeWhatYouSee)
	{
		...
		if (Mip0WorkDoneLastFrame < NumWorkPerFrame)
		{
			...
			if (!SelectedLightmapNames.IsEmpty() && bIsViewportNonRealtime && FrameNumber % 100 == 0)
			{
				...
				UE_LOGF(LogGPULightmass, Log, "Working on: %ls", *AllNames);
			}
		}
...
void FLightmapRenderer::DeduplicateRecordedTileRequests()
{
	...
	UE_LOGF(LogGPULightmass, Log, "Tile deduplication removed %d tiles", OldNum - RecordedTileRequests.Num());
}
