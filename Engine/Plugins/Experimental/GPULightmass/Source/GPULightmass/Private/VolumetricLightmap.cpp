...
void InitializeBrickData(FRHICommandListBase& RHICmdList, FIntVector BrickDataDimensions, FVolumetricLightmapBrickData& BrickData, const bool bForAccumulation)
{
	...
	UE_LOGF(LogGPULightmass, Log, "Allocated %.2fMB for volumetric lightmap %ls brick data", TotalSize / 1024.0f / 1024.0f, bForAccumulation ? TEXT("accumulation") : TEXT("display"));
}
...
void FVolumetricLightmapRenderer::VoxelizeScene(FRHICommandListImmediate& RHICmdList)
{
...

	{
...

		{
			...
			UE_LOGF(LogGPULightmass, Log, "Allocated %.2fMB for volumetric lightmap indirection texture",
...

	{
		...
		UE_LOGF(LogGPULightmass, Log, "Volumetric lightmap NumTotalBricks = %d", NumTotalBricks);
...

	{
		...
		UE_LOGF(LogGPULightmass, Log, "Allocated %.2fMB for volumetric lightmap validity mask",
...
void FVolumetricLightmapRenderer::BackgroundTick(FRHICommandListImmediate& RHICmdList)
{
	if (NumTotalBricks == 0)
	{
		return;
