	...
	: FLocalLightBuildInfo(SpotLightComponent)
{
	...
	const FVector2f ClampedConeAngles = SpotLightComponent->GetClampedConeAngles();
	InnerConeAngle = ClampedConeAngles.X;
...
bool FSpotLightBuildInfo::AffectsBounds(const FBoxSphereBounds& InBounds) const
{
	...
	{
		float Sin = FMath::Sin(OuterConeAngle);
		float Cos = FMath::Cos(OuterConeAngle);
	...
	: FLocalLightRenderState(SpotLightComponent)
{
	...
	{
		const FVector2f ClampedConeAngles = SpotLightComponent->GetClampedConeAngles();
		const float CosOuterCone = FMath::Cos(ClampedConeAngles.Y);
		const float CosInnerCone = FMath::Cos(ClampedConeAngles.X);
