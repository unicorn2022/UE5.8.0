...
void FScene::GatherImportanceVolumes()
{
	...
	if (CombinedImportanceVolume.GetExtent().SizeSquared() == 0)
	{
		...
		else
		{
			...
			UE_LOGF(LogGPULightmass, Warning, "No importance volume found, so the scene bounding box was used.  You can optimize your scene's quality and lighting build times by adding importance volumes.");
...
void FScene::AddLight(LightComponentType* LightComponent)
{
	...
	if (bCastStationaryShadows)
	{
		...
		{
			UE_LOGF(LogGPULightmass, Log, "Ignoring light with ShadowMapChannel == -1 (probably in the middle of SpawnActor)");
			return;
		}
	}
...
void FScene::AddLight(USkyLightComponent* SkyLight)
{
	...
	{
		UE_LOGF(LogGPULightmass, Log, "Skipping skylight with empty cubemap");
		return;
	}

	if (LightScene.SkyLight.IsSet())
	{
		UE_LOGF(LogGPULightmass, Log, "Warning: trying to add more than one skylight - removing the old one");
...
void FScene::AddGeometryInstanceFromComponent(UInstancedStaticMeshComponent* InComponent)
{
	...
	if (DoesPlatformUseInstancedStaticMeshVertexFactory(GMaxRHIShaderPlatform))
	{
	...
	{
		if (InstanceRenderState.InstancedRenderData)
		{
...
void FScene::RemoveGeometryInstanceFromComponent(UInstancedStaticMeshComponent* InComponent)
{
	...
	ENQUEUE_RENDER_COMMAND(RenderThreadRemove)(
		[ElementId, &RenderState = RenderState](FRHICommandListImmediate&) mutable
	{
		if (RenderState.InstanceGroupRenderStates.Elements[ElementId].InstancedRenderData)
		{
		...
		if (!bIsCompilingShaders)
		{
			if (bNeedsVoxelization)
			{
				...
				ENQUEUE_RENDER_COMMAND(BackgroundTickRenderThread)(
					[&RenderState = RenderState, TargetDetailCellSize](FRHICommandListImmediate& RHICmdList) mutable
				{
					RenderState.VolumetricLightmapRenderer->TargetDetailCellSize = TargetDetailCellSize;
					RenderState.VolumetricLightmapRenderer->VoxelizeScene(RHICmdList);
					RenderState.VolumetricLightmapRenderer->FrameNumber = 0;
					RenderState.VolumetricLightmapRenderer->SamplesTaken = 0;
				});
...
void FSceneRenderState::BackgroundTick(FRHICommandListImmediate& RHICmdList)
{
	...
	LightmapRenderer->BackgroundTick(RHICmdList);
	...
	{
		VolumetricLightmapRenderer->BackgroundTick(RHICmdList);
	}
}
