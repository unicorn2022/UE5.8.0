...
void FNNERuntimeIREEShaderCompilationManager::ProcessAsyncResults()
{
	...
	for (int32 JobIndex = JobQueue.Num() - 1; JobIndex >= 0; JobIndex--)
	{
		...
		{
			UE_LOGF(LogNNERuntimeIREEShader, Verbose, "GPU shader compile succeeded. Id %d", CurrentJob->Id);
		}
		else
		{
			UE_LOGF(LogNNERuntimeIREEShader, Verbose, "GPU shader compile failed! Id %d", CurrentJob->Id);
		}
...
static void LogShaderCompilerErrors(FNNERuntimeIREEShaderCompileResults const& Results)
{
	for (FNNERuntimeIREEShaderCompileMessage const& Message : Results.Messages)
	{
		...
		{
			UE_LOGF(LogNNERuntimeIREEShader, Warning, "%ls", *MessageText);
		}
		else if (Message.Type == FNNERuntimeIREEShaderCompileMessage::EMessageType::Error)
		{
			UE_LOGF(LogNNERuntimeIREEShader, Error, "%ls", *MessageText);
		}
	}
}
		...
		if (ShaderMap && Kernels)
		{
			...
			for (int32 JobIndex = 0; JobIndex < ResultArray.Num(); JobIndex++)
			{
				...
				if (GShowNNERuntimeIREEShaderWarnings || !CurrentJob.bSucceeded)
				{
					...
					{
						UE_LOGF(LogNNERuntimeIREEShader, Verbose, "There were errors for job \"%ls\"", *CurrentJob.Input.DebugGroupName)
					}
				}
				else
				{
					UE_LOGF(LogNNERuntimeIREEShader, Verbose, "There were NO errors for job \"%ls\"", *CurrentJob.Input.DebugGroupName);
				}
			}
				...
				for (FNNERuntimeIREEResource* Kernel : KernelsArray)
				{
					...
					if (Kernel->IsSame(CompletedShaderMap->GetShaderMapId()))
					{
						...
						else
						{
							...
							{
								UE_LOGF(LogNNERuntimeIREEShader, Warning, "Warnings while compiling NNERuntimeIREE shader %ls for platform %ls:",
