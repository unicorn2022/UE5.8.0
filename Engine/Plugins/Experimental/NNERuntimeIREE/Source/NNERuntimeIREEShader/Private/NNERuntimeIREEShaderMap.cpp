...
uint64 FNNERuntimeIREEShaderMapId::GetKernelHash() const
{
	...
	)
{
	...
	UE_LOGF(LogNNERuntimeIREEShader, Verbose, "%ls", *InKernel->GetHLSLSource());
	
	AddUniformBufferIncludesToEnvironment(NewJob->Input.Environment, InPlatform);

	FShaderCompilerEnvironment& ShaderEnvironment = NewJob->Input.Environment;

	UE_LOGF(LogNNERuntimeIREEShader, Verbose, "			%ls", GetName());
	...
	)
{
	...
	{
		UE_LOGF(LogNNERuntimeIREEShader, Fatal, "Trying to compile NNERuntimeIREE shader %ls at run-time, which is not supported on consoles!", *InKernel->GetFriendlyName() );
	}
	else
	{
		...
		else
		{
			...
			UE_LOGF(LogNNERuntimeIREEShader, Verbose, "CompilingId = %p %d", InKernel, CompilingId);
			...
			for(TLinkedList<FShaderType*>::TIterator ShaderTypeIt(FShaderType::GetTypeList());ShaderTypeIt;ShaderTypeIt.Next())
			{
				...
				else if (ShaderType)
				{
					...
					UE_LOGF(LogNNERuntimeIREEShader, Fatal, "%ls", *Message);
				}
			}
  
			if (!Kernel)
			{
				UE_LOGF(LogNNERuntimeIREEShader, Verbose, "		%u Shaders", NumShaders);
			}
...
FShader* FNNERuntimeIREEShaderMap::ProcessCompilationResultsForSingleJob(FShaderCompileJob& CurrentJob, uint64 InShaderMapHash)
{
...
bool FNNERuntimeIREEShaderMap::ProcessCompilationResults(const TArray<FShaderCommonCompileJobPtr>& InCompilationResults, int32& InOutJobIndex, float& InOutTimeBudget)
{
	...
	uint64 ShaderMapHash = GetContent()->ShaderMapId.GetKernelHash();
	...
	if (ShouldCacheNNERuntimeIREEShader(InShaderType, GetShaderPlatform(), InKernel))
	{
		for (int32 PermutationId = 0; PermutationId < InKernel->GetNumPermutations(); ++PermutationId)
		{
			if (!GetContent()->HasShader((FShaderType*)InShaderType, PermutationId))
			{
				if (!bSilent)
				{
					UE_LOGF(LogNNERuntimeIREEShader, Warning, "Incomplete shader %ls, missing FNNERuntimeIREEShader %ls.", *InKernel->GetFriendlyName(), InShaderType->GetName());
				}
				return false;
			}
		}
