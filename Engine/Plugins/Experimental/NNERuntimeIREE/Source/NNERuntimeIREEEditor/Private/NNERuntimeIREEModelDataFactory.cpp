...
UObject* UNNERuntimeIREEModelDataFactory::FactoryCreateBinary(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, const TCHAR * Type, const uint8 *& Buffer, const uint8 * BufferEnd, FFeedbackContext* Warn)
{
	...
	{
		UE_LOGF(LogNNERuntimeIREE, Error, "UNNERuntimeIREEModelDataFactory failed to parse the models meta data");
		return nullptr;
	}
	if (ModuleMetaData.FunctionMetaData.IsEmpty())
	{
		UE_LOGF(LogNNERuntimeIREE, Error, "UNNERuntimeIREEModelDataFactory no public functions found in models meta data");
		return nullptr;
	}
	...
	if (ModuleMetaData.FunctionMetaData.Num() > 1)
	{
		...
		{
			UE_LOGF(LogNNERuntimeIREE, Error, "UNNERuntimeIREEModelDataFactory could not import the model! Please select the neural network main function in the import dialog!");
			return nullptr;
		}
