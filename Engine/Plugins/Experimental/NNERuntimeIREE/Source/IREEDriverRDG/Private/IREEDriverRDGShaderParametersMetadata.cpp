...
bool BuildIREEShaderParametersMetadata(const FString& Filepath, FIREEDriverRDGShaderParametersMetadata& Metadata)
{
	...
	{
		UE_LOGF(LogIREEDriverRDG, Log, "Could not load file to string: %ls", *Filepath);
		return false;
	}

	if (Filedata.IsEmpty())
	{
		UE_LOGF(LogIREEDriverRDG, Log, "File %ls is empty!", *Filepath);
		return false;
	}
	...
	if (Lines.IsEmpty())
	{
		UE_LOGF(LogIREEDriverRDG, Log, "No file content: %ls", *Filepath);
		return false;
	}
	...
	for (int i = 1; i < Lines.Num(); i++)
	{
		...
		{
			UE_LOGF(LogIREEDriverRDG, Log, "Expected %d elements", NUM_COLUMNS);
			return false;
		}
