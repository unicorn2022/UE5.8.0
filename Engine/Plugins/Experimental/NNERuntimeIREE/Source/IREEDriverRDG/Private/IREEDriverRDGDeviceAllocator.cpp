...
private:
	...
	static iree_hal_buffer_compatibility_t QueryBufferCompatibility(iree_hal_allocator_t* BaseAllocator, iree_hal_buffer_params_t* Params, iree_device_size_t* AllocationSize)
	{
		...
		UE_LOGF(LogIREEDriverRDG, Display, "%ls type %s usage %s access %s allocationSize %lu", StringCast<TCHAR>(__FUNCTION__).Get(), memory_type_str.data, usage_str.data, access_str.data, *AllocationSize);
#endif
		...
		if (iree_all_bits_set(Params->type, IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL))
		{
			...
			{
				UE_LOGF(LogIREEDriverRDG, Display, "Buffer compability for Size %lld: Device local and host visible not supported, falling back to host local and device visible!", (int64)*AllocationSize);
		...
		{
			UE_LOGF(LogIREEDriverRDG, Warning, "Buffer compability for Size %lld: Host local not supported!", (int64)*AllocationSize);
		...
		{
			UE_LOGF(LogIREEDriverRDG, Display, "Buffer compability for Size %lld contains unsupported IREE_HAL_BUFFER_USAGE_MAPPING bit flag set.", (int64)*AllocationSize);
	...
	static iree_status_t AllocateBufferInternal(iree_hal_allocator_t* BaseAllocator, const iree_hal_buffer_params_t* Params, iree_device_size_t AllocationSize, iree_hal_buffer_release_callback_t UserReleaseCallback, iree_hal_buffer_t** OutBuffer)
	{
		...
		UE_LOGF(LogIREEDriverRDG, Display, "%ls type %s usage %s access %s AllocationSize %d", StringCast<TCHAR>(__FUNCTION__).Get(), memory_type_str.data, usage_str.data, access_str.data, AllocationSize);
#endif
...
#if IREE_DRIVER_RDG_VERBOSITY == 1
			UE_LOGF(LogIREEDriverRDG, Display, "Allocate heap buffer of size %d", AllocationSize);
#endif
...
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "--> Allocator allocated buffer 0x%x", (uint64)*OutBuffer); 
#endif
	...
	static iree_status_t AllocateBuffer(iree_hal_allocator_t* BaseAllocator, const iree_hal_buffer_params_t* Params, iree_device_size_t AllocationSize, iree_hal_buffer_t** OutBuffer)
	{
		...
		UE_LOGF(LogIREEDriverRDG, Display, "%ls %ls type %s usage %s access %s AllocationSize %d", StringCast<TCHAR>(__FUNCTION__).Get(), TEXT("PooledBuffer"), memory_type_str.data, usage_str.data, access_str.data, AllocationSize);
#endif
	...
	static iree_status_t ImportHostBuffer(iree_hal_allocator_t* BaseAllocator, const iree_hal_buffer_params_t* Params, iree_hal_external_buffer_t* ExternalBuffer, iree_hal_buffer_release_callback_t ReleaseCallback, iree_hal_buffer_t** OutBuffer)
	{
		...
		UE_LOGF(LogIREEDriverRDG, Display, "%ls type %s usage %s access %s size %d", StringCast<TCHAR>(__FUNCTION__).Get(), memory_type_str.data, usage_str.data, access_str.data, ExternalBuffer->size);
#endif
...
const iree_hal_allocator_vtable_t FDeviceAllocator::VTable =
{
	...
	// Not supported — all entries set to NULL.
	.supports_virtual_memory = NULL,
	.virtual_memory_query_granularity = NULL,
	.virtual_memory_reserve = NULL,
	.virtual_memory_release = NULL,
	.physical_memory_allocate = NULL,
	.physical_memory_free = NULL,
	.virtual_memory_map = NULL,
	.virtual_memory_unmap = NULL,
	.virtual_memory_protect = NULL,
	.virtual_memory_advise = NULL
};
