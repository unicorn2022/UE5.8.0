...
public:
	static iree_status_t BufferWrap(iree_allocator_t HostAllocator, iree_hal_allocator_t* DeviceAllocator, iree_hal_memory_type_t MemoryType, iree_hal_memory_access_t AllowedAccess, iree_hal_buffer_usage_t AllowedUsage, iree_device_size_t AllocationSize, iree_device_size_t ByteOffset, iree_device_size_t ByteLength, const TRefCountPtr<FRDGPooledBuffer>& PooledBuffer, iree_hal_buffer_release_callback_t UserReleaseCallback, iree_hal_buffer_t** OutBuffer)
	{
		...
		UE_LOGF(LogIREEDriverRDG, Display, "%ls PooledBuffer Size %d", StringCast<TCHAR>(__FUNCTION__).Get(), PooledBuffer->Desc.GetSize());
#endif
...
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "--> Created iree_hal_buffer_t 0x%x", (uint64)*OutBuffer);
#endif
	...
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls RDGBuffer Size %d", StringCast<TCHAR>(__FUNCTION__).Get(), RDGBuffer->Desc.GetSize());
#endif
		check(DeviceAllocator);
		check(GraphBuilder);
	...
	static FRDGPooledBuffer* GetPooledBuffer(iree_hal_buffer_t* BaseBuffer)
	{
		if (!BaseBuffer)
		{
			return nullptr;
		...
		iree_hal_buffer_t* AllocatedBuffer = iree_hal_buffer_allocated_buffer(BaseBuffer);

		return Cast(AllocatedBuffer)->PooledBuffer.GetReference();
	}
	...
	static FRDGBufferRef GetRDGBuffer(iree_hal_buffer_t* BaseBuffer, FRDGBuilder* GraphBuilder)
	{
		...
		if (!BaseBuffer)
		{
			return FRDGBufferRef{};
		}

		iree_hal_buffer_t* AllocatedBuffer = iree_hal_buffer_allocated_buffer(BaseBuffer);

		FBuffer* Buffer = Cast(AllocatedBuffer);
		if (Buffer->RDGBuffer)
		{
...
private:
	...
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls Buffer 0x%x %ls Size %d", StringCast<TCHAR>(__FUNCTION__).Get(), BaseBuffer, (IsTransient(BaseBuffer) ? TEXT("RDGBuffer") : TEXT("PooledBuffer")), BaseBuffer->allocation_size);
#endif
	...
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls Buffer 0x%x mm %d ma %d offset %d length %d", StringCast<TCHAR>(__FUNCTION__).Get(), BaseBuffer, (int32)MappingMode, (int32)MemoryAccess, (int32)LocalByteOffset, (int32)LocalByteLength);
#endif
	...
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls Buffer 0x%x", StringCast<TCHAR>(__FUNCTION__).Get(), BaseBuffer);
#endif
		return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "Memory mapping functionality not available since required GPU-CPU synchronisation not supported by NNERuntimeIREERdg");
	}
...
FRDGPooledBuffer* BufferPooledBufferHandle(iree_hal_buffer_t* Buffer)
{
