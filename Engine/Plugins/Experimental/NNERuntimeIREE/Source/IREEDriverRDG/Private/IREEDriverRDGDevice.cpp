...
private:
	...
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls: category='%.*s' key='%.*s'", StringCast<TCHAR>(__FUNCTION__).Get(), (int)Category.size, Category.data, (int)Key.size, Key.data);
#endif
		check(OutValue);
	...
	static iree_status_t QueryCapabilities(iree_hal_device_t* BaseDevice, iree_hal_device_capabilities_t* OutCapabilities)
	{
		memset(OutCapabilities, 0, sizeof(*OutCapabilities));
		return iree_ok_status();
	}

	static const iree_hal_device_topology_info_t* TopologyInfo(iree_hal_device_t* BaseDevice)
	{
		return &Cast(BaseDevice)->TopologyInfoData;
	}

	static iree_status_t RefineTopologyEdge(iree_hal_device_t* SrcDevice, iree_hal_device_t* DstDevice, iree_hal_topology_edge_t* Edge)
	{
		return iree_ok_status();
	}

	static iree_status_t AssignTopologyInfo(iree_hal_device_t* BaseDevice, const iree_hal_device_topology_info_t* TopologyInfo)
	{
		Cast(BaseDevice)->TopologyInfoData = *TopologyInfo;
		return iree_ok_status();
	}
	...
	static iree_status_t CreateSemaphore(iree_hal_device_t* BaseDevice, iree_hal_queue_affinity_t /*QueueAffinity*/, uint64_t InitialValue, iree_hal_semaphore_flags_t Flags, iree_hal_semaphore_t **OutSemaphore)
	{
		FDevice* Device = Cast(BaseDevice);
	...
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls AllocationSize %d", StringCast<TCHAR>(__FUNCTION__).Get(), AllocationSize);
#endif
	...
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls Buffer 0x%x, AllocationSize %d", StringCast<TCHAR>(__FUNCTION__).Get(), Buffer, Buffer->allocation_size);
#endif
...
// #endif
	...
	static iree_status_t QueueHostCall(iree_hal_device_t* /*BaseDevice*/, iree_hal_queue_affinity_t /*QueueAffinity*/, const iree_hal_semaphore_list_t /*WaitSemaphoreList*/, const iree_hal_semaphore_list_t /*SignalSemaphoreList*/, iree_hal_host_call_t /*Call*/, const uint64_t /*Args*/[4], iree_hal_host_call_flags_t /*Flags*/)
	{
		return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "host calls not supported");
	}
	...
	static iree_status_t WaitSemaphore(iree_hal_device_t* BaseDevice, iree_hal_wait_mode_t WaitMode, const iree_hal_semaphore_list_t SemaphoreList, iree_timeout_t Timeout, iree_hal_wait_flags_t /*Flags*/)
	{
#if IREE_DRIVER_RDG_VERBOSITY == 1
		UE_LOGF(LogIREEDriverRDG, Display, "%ls", StringCast<TCHAR>(__FUNCTION__).Get());
#endif
	...
	iree_hal_device_topology_info_t TopologyInfoData;
...
const iree_hal_device_vtable_t FDevice::VTable =
{
	...
	.query_capabilities = FDevice::QueryCapabilities,
	.topology_info = FDevice::TopologyInfo,
	.refine_topology_edge = FDevice::RefineTopologyEdge,
	.assign_topology_info = FDevice::AssignTopologyInfo,
	...
	.queue_host_call = FDevice::QueueHostCall,
