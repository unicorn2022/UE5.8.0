...
void PrintIREEError(const FString& InMessage, iree_status_t InStatus)
{
	...
	UE_LOGF(LogNNERuntimeIREE, Error, "%ls: %ls", *InMessage, *FString(StringCast<TCHAR>(static_cast<const ANSICHAR*>(ErrorString)).Get()));
...
bool CreateTaskTopology(const TArray<FNNERuntimeIREEThreadingAffinity>& TaskTopologyGroups, TArray<iree_task_topology_t>& TaskTopologies)
{
	...
	{
		UE_LOGF(LogNNERuntimeIREE, Log, "Using default task topology...");

		const iree_host_size_t MaxCoreCount = 64;
		iree_task_topology_t TaskTopology;
		iree_status_t Status = iree_task_topology_initialize_from_physical_cores(IREE_TASK_TOPOLOGY_NODE_ID_ANY, IREE_TASK_TOPOLOGY_PERFORMANCE_LEVEL_ANY, IREE_TASK_TOPOLOGY_DISTRIBUTION_SCATTER, MaxCoreCount, &TaskTopology);
	...
	if (Algo::AllOf(TaskTopologyGroups, [](FNNERuntimeIREEThreadingAffinity Group) { return Group.CoreIndex == -1; }))
	{
		...
		{
			UE_LOGF(LogNNERuntimeIREE, Warning, "All task topology groups require the same specifier type");
			return false;
		}
		...
		{
			UE_LOGF(LogNNERuntimeIREE, Warning, "All task topology groups require the same NUMA node index");
			return false;
		}
		...
		iree_task_topology_t TaskTopology;
		iree_status_t Status = iree_task_topology_initialize_from_physical_cores(NodeId, IREE_TASK_TOPOLOGY_PERFORMANCE_LEVEL_ANY, IREE_TASK_TOPOLOGY_DISTRIBUTION_SCATTER, MaxCoreCount, &TaskTopology);
		if (!iree_status_is_ok(Status))
		{
			Private::PrintIREEError("Failed to initialize task topology", Status);
			return false;
	...
	{
		UE_LOGF(LogNNERuntimeIREE, Warning, "Inhomogeneous core assignment not supported yet");
	}
...
{
	UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "# ===--------------------------------------------------------------===");
	UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "# topology[%llu]: %llu worker groups", static_cast<uint64>(TopologyId), static_cast<uint64>(Topology->group_count));
	UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "# ===--------------------------------------------------------------===\n#");
	...
	for (iree_host_size_t j = 0; j < Topology->group_count; ++j)
	{
		...
		UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "# group[%d]: '%ls'", Group->group_index, UTF8_TO_TCHAR(Group->name));
		UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "#      processor: %u", Group->processor_index);

		if (Group->ideal_thread_affinity.id_assigned)
		{
			UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "#       affinity: group=%u, id=%u, smt=%u", Group->ideal_thread_affinity.group, Group->ideal_thread_affinity.id, Group->ideal_thread_affinity.smt);
		}
		else
		{
			UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "#       affinity: (unassigned)");
		}

		UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "#  caches: l1d=%u, l2d=%u", Group->caches.l1_data, Group->caches.l2_data);

		if (Group->constructive_sharing_mask == 0)
		{
			UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "#  last level cache sharing: (none)");
		}
		else if (Group->constructive_sharing_mask == IREE_TASK_TOPOLOGY_GROUP_MASK_ALL)
		{
			UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "#  last level cache sharing: (all/undefined)");
		}
		else
		{
			...
			UE_LOGF(LogNNERuntimeIREE, DUMP_TASK_TOPOLOGY_LOG_CATEGORY, "#  last level cache sharing: %u group(s): %ls", ShareCount, *MaskString);
		}
