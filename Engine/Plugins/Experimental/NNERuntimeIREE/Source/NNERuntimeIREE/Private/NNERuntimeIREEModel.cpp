...
namespace UE::NNERuntimeIREE
{
	namespace Private
	{
		...
		static bool ShapeEqualDims(const UE::NNE::FTensorShape& Shape, const iree_hal_dim_t* Dims, iree_host_size_t Rank)
		{
			if (Shape.Rank() != Rank)
			{
				return false;
		...
		static UE::NNE::FTensorShape DimsToShape(const iree_hal_dim_t* Dims, iree_host_size_t Rank)
		{
			...
			return UE::NNE::FTensorShape::Make(TConstArrayView<uint32>(Shape, ShapeRank));
		}
		...
		public:
			...
			bool CreateModule(TConstArrayView<uint8> InVmfbDataView, iree_vm_module_t** OutModule)
			{
				...
				Status = iree_vm_bytecode_module_create(iree_runtime_instance_vm_instance(Instance), IREE_VM_BYTECODE_MODULE_FLAG_NONE, ModuleData, iree_allocator_null(), GetHostAllocator(), &Module);
				if (!iree_status_is_ok(Status))
				{
		...
		public:
			...
			static TSharedPtr<UE::NNE::FSharedModelData> GetVmfbAsModelData(const FString& InVmfbPath)
			{
				...
				if (!Reader)
				{
					UE_LOGF(LogNNERuntimeIREE, Error, "CPU module: Failed to open the vmfb data file '%ls'", *InVmfbPath);
					return {};
				}
				int64 DataSize = Reader->TotalSize();
				if (DataSize < 1)
				{
					UE_LOGF(LogNNERuntimeIREE, Error, "CPU module: vmfb data file '%ls' is empty", *InVmfbPath);
					return {};
				}
			...
			static TSharedPtr<FModule> Make(const FString& InVmfbPath, TSharedRef<UE::NNE::FSharedModelData> ModelData, const FModuleMetaData& InModuleMetaData)
			{
				check(!InVmfbPath.IsEmpty());
			...
			bool GetFunctionByName(const FString& InFunctionName, iree_vm_function_t* OutFunction)
			{
				...
				if (!bFound)
				{
					UE_LOGF(LogNNERuntimeIREE, Error, "CPU module: Failed to find the module function %ls", *InFunctionName);
					iree_status_free(Status);
					return false;
				}
	...
	namespace CPU
	{
		namespace Private
		{
			...
			public:
				...
				static TSharedPtr<FLibrary> GetLibrary(const FString& InLibraryPath, const FString& InLibraryName)
				{
					...
					if (!Library)
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "CPU library: Failed to load the shared library '%ls'", *CombinedPath);
					...
					if (!Library)
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "CPU library: Failed to load the shared library '%ls' from '%ls'", *InLibraryName, *InLibraryPath);
						return TSharedPtr<FLibrary>();
					}
#endif
				...
				bool GetFunctionPointer(const FString& InFunctionName, void** OutFunctionPointer) const
				{
					...
					UE_LOGF(LogNNERuntimeIREE, Error, "CPU library: Failed to get the function %ls", *InFunctionName);
					return false;
				}
			};
			...
			public:
				...
				static TSharedPtr<FSession> Make(TSharedRef<FDevice> InDevice, TSharedRef<UE::NNERuntimeIREE::Private::FModule> InModule)
				{
					...
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Function signature mismatch in function %ls", *MainFunctionName);
						iree_runtime_session_release(Session);
						iree_status_free(Status);
						return TSharedPtr<FSession>();
					}
				...
				ESetInputTensorShapesStatus SetInputTensorShapes(TConstArrayView<UE::NNE::FTensorShape> InInputShapes)
				{
					...
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Number of input shapes does not match number of input tensors");
					...
					for (int32 i = 0; i < InInputShapes.Num(); ++i)
					{
						...
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Input shape does not match input tensor %ls of index %d", *SymbolicDesc.GetName(), i);
							return ESetInputTensorShapesStatus::Fail;
						}
					}
				...
				ERunSyncStatus RunSyncCPU(TConstArrayView<UE::NNE::FTensorBindingCPU> InInputBindings, TConstArrayView<UE::NNE::FTensorBindingCPU> InOutputBindings)
				{
					...
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Input shapes are not set, please call SetInputTensorShapes.");
						return ERunSyncStatus::Fail;
					}
					if (InInputBindings.Num() != InputTensorShapes.Num())
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Input bindings need to match input tensor descriptor count (got %d, expected %d).", InInputBindings.Num(), InputTensorShapes.Num());
						return ERunSyncStatus::Fail;
					}
					if (!InOutputBindings.IsEmpty() && InOutputBindings.Num() != OutputTensorDescs.Num())
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Output binding can be empty or needs to match output tensor descriptor count (got %d, expected %d).", InOutputBindings.Num(), OutputTensorDescs.Num());
						return ERunSyncStatus::Fail;
					}
					...
					for (int32 i = 0; i < InInputBindings.Num(); i++)
					{
						...
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Binding input tensor %d is not set but given size is non-zero %lld.", i, InInputBindings[i].SizeInBytes);
							return ERunSyncStatus::Fail;
						}
						if (InInputBindings[i].SizeInBytes != (InputTensorShapes[i].Volume() * InputTensorDescs[i].GetElementByteSize()))
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Binding input tensor %d size does not match size given by tensor descriptor (got %lld, expected %lld).", i, InInputBindings[i].SizeInBytes, InputTensorShapes[i].Volume() * InputTensorDescs[i].GetElementByteSize());
							return ERunSyncStatus::Fail;
						}
						if (FMath::Modulo<uint64>((uint64)InInputBindings[i].Data, IREE_HAL_HEAP_BUFFER_ALIGNMENT) != 0)
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Input bindings memory need to be aligned with %d bytes", IREE_HAL_HEAP_BUFFER_ALIGNMENT);
							return ERunSyncStatus::Fail;
						}
					}
					...
					for (int32 i = 0; i < InOutputBindings.Num(); i++)
					{
						...
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "CPU session: Output bindings memory need to be aligned with %d bytes", IREE_HAL_HEAP_BUFFER_ALIGNMENT);
							return ERunSyncStatus::Fail;
						}
					}
			...
			private:
				...
				iree_status_t RunSyncCPUInternal(TConstArrayView<UE::NNE::FTensorBindingCPU> InInputBindings, TConstArrayView<UE::NNE::FTensorBindingCPU> InOutputBindings)
				{
					...
					for (int32 i = 0; i < OutputTensorDescs.Num(); i++)
					{
						...
						if (!UE::NNERuntimeIREE::Private::ShapeEqualDims(OutputTensorShapes[i], Dims, Rank))
						{
							OutputTensorShapes[i] = UE::NNERuntimeIREE::Private::DimsToShape(Dims, Rank);
						}

						const uint64 DataSizeInBytes = iree_hal_buffer_view_byte_length(BufferView);

						// Copy result to output binding if present and buffer big enough
						// note: by design silently skip if output buffer is undersized.
						if (!InOutputBindings.IsEmpty() && InOutputBindings[i].Data && DataSizeInBytes <= InOutputBindings[i].SizeInBytes)
						{
		...
		TSharedPtr<FModel> FModel::Make(UE::NNERuntimeIREE::Private::FEnvironment& InEnvironment, const FString& InDirPath, const FString& InSharedLibraryFileName, const FString& InVmfbFileName, const FString& InLibraryQueryFunctionName, const FModuleMetaData& InModuleMetaData)
		{
	...
	namespace RDG
	{
		namespace Private
		{
			...
			public:
				...
				static TSharedPtr<FDevice> Make(const TMap<FString, TConstArrayView<uint8>>& Executables)
				{
					...
					if (!iree_status_is_ok(Status))
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "Could not create IREE RDG device!");
						iree_status_free(Status);
						return {};
					}
			...
			public:
				...
				static TSharedPtr<FSession> Make(TSharedRef<FDevice> InDevice, TSharedRef<UE::NNERuntimeIREE::Private::FModule> InModule)
				{
					...
					if (!iree_status_is_ok(Status) || NumInputs != InputTensorDescs.Num() || NumOutputs != OutputTensorDescs.Num())
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Function signature mismatch in function %ls", *MainFunctionName);
						iree_runtime_session_release(Session);
						iree_status_free(Status);
						return {};
					}
				...
				ESetInputTensorShapesStatus SetInputTensorShapes(TConstArrayView<UE::NNE::FTensorShape> InInputShapes)
				{
					...
					if (InInputShapes.Num() != InputTensorDescs.Num())
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Number of input shapes does not match number of input tensors");
						return ESetInputTensorShapesStatus::Fail;
					}
					...
					for (int32 i = 0; i < InInputShapes.Num(); ++i)
					{
						...
						if (!InInputShapes[i].IsCompatibleWith(SymbolicDesc.GetShape()))
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Input shape does not match input tensor %ls of index %d", *SymbolicDesc.GetName(), i);
							return ESetInputTensorShapesStatus::Fail;
						}
				...
				EEnqueueRDGStatus EnqueueRDG(FRDGBuilder& GraphBuilder, TConstArrayView<UE::NNE::FTensorBindingRDG> InInputBindings, TConstArrayView<UE::NNE::FTensorBindingRDG> InOutputBindings)
				{
					...
					if (InputTensorShapes.IsEmpty())
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Input shapes are not set, please call SetInputTensorShapes.");
						return EEnqueueRDGStatus::Fail;
					}

					if (InInputBindings.Num() != InputTensorShapes.Num())
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Input bindings need to match input tensor descriptor count (got %d, expected %d).", InInputBindings.Num(), InputTensorShapes.Num());
						return EEnqueueRDGStatus::Fail;
					}

					if (!InOutputBindings.IsEmpty() && InOutputBindings.Num() != OutputTensorDescs.Num())
					{
						UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Output binding can be empty or needs to match output tensor descriptor count (got %d, expected %d).", InOutputBindings.Num(), OutputTensorDescs.Num());
						return EEnqueueRDGStatus::Fail;
					}
					...
					for (int32 i = 0; i < InInputBindings.Num(); i++)
					{
						...
						if (!Binding.Buffer)
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Binding input tensor %d is null.", i);
							return EEnqueueRDGStatus::Fail;
						}

						if (Binding.Buffer->Desc.GetSize() < InputTensors[i].GetDataSize())
						{
							UE_LOGF(LogNNERuntimeIREE, Error, "RDG session: Binding input tensor %d size (%d) is smaller than the size (%lld) given by tensor descriptor.", i, Binding.Buffer->Desc.GetSize(), InputTensorShapes[i].Volume() * InputTensorDescs[i].GetElementByteSize());
							return EEnqueueRDGStatus::Fail;
						}
					}
					...
					if (iree_status_is_ok(Status))
					{
						...
						UE_LOGF(LogNNERuntimeIREE, Verbose, "NNERuntimeIREERDG::FSession::EnqueueRDG: synchronous invocation of IREE call.");
					...
					{
						check(OutputTensorShapes.Num() == 0 || OutputTensorShapes.Num() == OutputTensorDescs.Num());
						bool bSetOutputTensorShapes = false;
						if (OutputTensorShapes.Num() == 0 && OutputTensorDescs.Num() > 0)
						{
							bSetOutputTensorShapes = true;
							OutputTensorShapes.SetNum(OutputTensorDescs.Num());
						}
						check(OutputTensorShapes.Num() == OutputTensorDescs.Num());
						check(InOutputBindings.IsEmpty() || InOutputBindings.Num() == OutputTensorDescs.Num());
						...
						for (int32 i = 0; i < OutputTensorDescs.Num(); i++)
						{
							iree_hal_buffer_view_t* IREEBufferView = nullptr;
							Status = iree_runtime_call_outputs_pop_front_buffer_view(&Call, &IREEBufferView);
							if (!iree_status_is_ok(Status))
							{
								if (bSetOutputTensorShapes)
								{
									OutputTensorShapes.Reset();
								}
								break;
							}
							iree_host_size_t Rank = iree_hal_buffer_view_shape_rank(IREEBufferView);
							const iree_hal_dim_t* Dims = iree_hal_buffer_view_shape_dims(IREEBufferView);
							if (!UE::NNERuntimeIREE::Private::ShapeEqualDims(OutputTensorShapes[i], Dims, Rank))
							{
								OutputTensorShapes[i] = UE::NNERuntimeIREE::Private::DimsToShape(Dims, Rank);
							}

							if (InOutputBindings.IsEmpty() || !InOutputBindings[i].Buffer)
							{
								iree_hal_buffer_view_release(IREEBufferView);
								continue;
							}
							...
							if (!IREEBuffer)
							{
								Status = iree_make_status(IREE_STATUS_UNKNOWN, "Failed to get the result buffer");
								iree_hal_buffer_view_release(IREEBufferView);
								break;
							}

							const uint64 Length = (uint64)iree_hal_buffer_view_byte_length(IREEBufferView);
							const uint64 Offset = (uint64)iree_hal_buffer_byte_offset(IREEBuffer);

							FRDGBufferRef RDGBuffer = IREE::HAL::RDG::BufferRDGBuffer(IREEBuffer, &GraphBuilder);
							check(Length <= RDGBuffer->Desc.GetSize() - Offset);

							// note: by design silently skip if output buffer is undersized.
							if (Length <= (uint64)InOutputBindings[i].Buffer->Desc.GetSize())
							{
		...
		TSharedPtr<FModel> FModel::Make(const FString& InDirPath, TConstArrayView64<uint8> VmfbData, const FModuleMetaData& InModuleMetaData, const TMap<FString, TConstArrayView<uint8>>& Executables)
		{
