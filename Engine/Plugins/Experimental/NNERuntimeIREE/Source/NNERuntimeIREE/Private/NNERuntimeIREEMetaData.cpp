...
#define NNEMLIR_CXX_API_THROW(Message) \
	do { \
		UE_LOGF(LogNNERuntimeIREE, Fatal, "%s", Message.c_str()); \
		abort(); \
	} while (false)
#endif
...
{
	namespace ModuleMetaData::Private
	{
		enum Version : uint32
		{
			V0 = 0, // Initial
	...
	} // ModuleMetaData::Private

	void FModuleMetaData::Serialize(FArchive& Ar)
	{
		...
		else
		{
			...
			case UE::NNERuntimeIREE::ModuleMetaData::Private::Version::V0:
				...
				break;
			default:
				UE_LOGF(LogNNERuntimeIREE, Error, "UNNERuntimeIREEModuleMetaData: Unknown asset version %d: Deserialisation failed, please reimport the original model.", Ar.CustomVer(UE::NNERuntimeIREE::ModuleMetaData::Private::GUID));
				break;
			}
		}
	}

	bool FModuleMetaData::ParseFromBuffer(TConstArrayView64<uint8> Buffer)
	{
		...
		{
			UE_LOGF(LogNNERuntimeIREE, Error, "Parse MLIR exception: %s", Exception.what());
			return false;
		}
		catch (...)
		{
			UE_LOGF(LogNNERuntimeIREE, Error, "Parse MLIR unknown exception.");
			return false;
		}
#endif // WITH_EDITOR
