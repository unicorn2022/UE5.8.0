...
{
	UE_LOGF(LogBackChannel, Verbose, "Destroying OSC Connection to %ls", *GetDescription());
	if (IsRunning)
	{
		Stop();
	}
...
void FBackChannelOSCConnection::ReceiveMessages(const float MaxTime /*= 0*/)
{
    ...
    //UE_LOGF(LogBackChannel, Log, "Starting Packet Check at %.02f", StartTime);
			...
			if (ReceivedDataSize == ExpectedSizeOfNextPacket)
			{
				...
				if (ExpectedSizeOfNextPacket == 4)
				{
					...
					else
					{
						...
						UE_LOGF(LogBackChannel, Error,
						       "Received packet of %d bytes that was out of the range [0,%d]. Assuming data is malformed and disconnecring",
						       Size, kMaxPacketSize);
						HasErrorState = true;
				...
				else
				{
					...
					if (Packet.IsValid())
					{
						...
						if (Packet->GetType() == OSCPacketType::Message)
						{
							...
							UE_LOGF(LogBackChannel, VeryVerbose, "Received message to %ls (tags:%ls, size:%d)", *NewAddress, *MsgPacket->GetTags(), ExpectedSizeOfNextPacket);
							...
							if (CurrentCount > 0)
							{
								UE_CLOGF(GBackChannelLogPackets, LogBackChannel, Log, "%ls has %d unprocessed messages", *NewAddress, CurrentCount + 1);

								if (MaxMessages > 0 && CurrentCount >= MaxMessages)
								{
									UE_LOGF(LogBackChannel, VeryVerbose, "Discarding old messages due to limit of %d", MaxMessages);
						...
						{
							UE_LOGF(LogBackChannel, VeryVerbose, "Received #bundle of %d bytes", ExpectedSizeOfNextPacket);
						}
    ...
    const double ElapsedTime = FPlatformTime::Seconds() - StartTime;
    UE_LOGF(LogBackChannel, VeryVerbose, "Received %d packets in %.03f secs at %.03f", PacketsReceived, ElapsedTime, FPlatformTime::Seconds());
}
...
void FBackChannelOSCConnection::DispatchMessages()
{
	...
	for (auto& Packet : ReceivedPackets)
	{
		if (Packet->GetType() == OSCPacketType::Message)
		{
			...
			UE_LOGF(LogBackChannel, VeryVerbose, "Dispatching %ls to handlers", *Msg->GetPath());

			if (!DispatchMap.DispatchMessage(*Msg.Get()))
			{
				UE_LOGF(LogBackChannel, Log, "Failed to displatch message to %ls. No handler?", *Msg->GetPath());
			}
			Msg->ResetRead();
		}
	}
...
bool FBackChannelOSCConnection::StartReceiveThread()
{
	...
	UE_LOGF(LogBackChannel, Verbose, "Started OSC Connection to %ls", *GetDescription());
...
uint32 FBackChannelOSCConnection::Run()
{
	...
	UE_LOGF(LogBackChannel, Verbose, "OSC Connection to %ls is Running", *Connection->GetDescription());
	...
	UE_LOGF(LogBackChannel, Verbose, "OSC Connection to %ls is exiting.", *Connection->GetDescription());
	IsRunning = false;
	return 0;
}
...
void FBackChannelOSCConnection::Stop()
{
	if (IsRunning)
	{
		UE_LOGF(LogBackChannel, Verbose, "Requesting OSC Connection to stop..");
	...
	UE_LOGF(LogBackChannel, Verbose, "OSC Connection is stopped");
	Connection = nullptr;
}
...
bool FBackChannelOSCConnection::SendPacket(FBackChannelOSCPacket& Packet)
{
	...
	UE_LOGF(LogBackChannel, VeryVerbose, "Sent message to %ls (tags:%ls, size:%d)", *MsgPacket->GetPath(), *MsgPacket->GetTags(), Data.Num());
