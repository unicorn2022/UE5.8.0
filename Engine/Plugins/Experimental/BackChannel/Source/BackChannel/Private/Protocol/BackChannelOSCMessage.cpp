...
int FBackChannelOSCMessage::Read(const TCHAR* InName, FString& Value)
{
	...
	else*/
	{
		...
		{
			UE_LOGF(LogBackChannel, Warning, "OSCMessage(%ls): no more tags to read (expected 's', have %d tags). This can happen when the sender uses an older protocol version.", *Address, TagString.Len());
			return 1;
		}
		...
		if (CurrentTag != 's')
		{
			UE_LOGF(LogBackChannel, Error, "OSCMessage: Requested tag 's' but next tag was %c", CurrentTag);
			return 1;
		}
...
TCHAR FBackChannelOSCMessage::ReadTag(const TCHAR ExpectedTag, bool SuppressWarning /*= false*/)
{
	if (TagIndex == TagString.Len())
	{
		UE_CLOGF(!SuppressWarning, LogBackChannel, Error, "OSCMessage: Cannot read tag %c, no more tags!", ExpectedTag);
		return 0;
	}
	...
	{
		UE_CLOGF(!SuppressWarning, LogBackChannel, Error, "OSCMessage: Requested tag %c but next tag was %c", ExpectedTag, CurrentTag);
		return 0;
	}
	...
	if (ReadTag(TEXT('b')))
	{
		...
		else
		{
			...
			else
			{
				...
				UE_LOGF(LogBackChannel, Error, "OSCMessage: buffer for reading blob was too small. Blob: %d bytes, buffer: %d", OutBlobSize, MaxBlobSize);
			}
		}
