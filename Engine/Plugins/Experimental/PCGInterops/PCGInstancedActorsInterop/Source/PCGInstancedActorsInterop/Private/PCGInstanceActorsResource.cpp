...
bool UPCGInstancedActorsManagedResource::MoveResource(const FPCGMoveResourceParams& Params)
{
	Super::MoveResource(Params);
	...
	for (FInstancedActorsInstanceHandle& Handle : Handles)
	{
		if (AActor* Manager = Handle.GetManager())
		{
			...
			if (!Manager->IsAttachedTo(TargetActor))
			{
				Manager->DetachFromActor(FDetachmentTransformRules::KeepWorldTransform);
				Manager->SetOwner(nullptr);
				Manager->AttachToActor(TargetActor, FAttachmentTransformRules::KeepWorldTransform);
			}
		}
	}
...
{
	UE_LOGF(LogPCGInstancedActorsInterop, Warning, "Instanced actors cannot currently be marked as transient on load.");
...
void UPCGInstancedActorsManagedResource::ChangeTransientState(EPCGEditorDirtyMode NewEditingMode)
{
	...
	{
		UE_LOGF(LogPCGInstancedActorsInterop, Warning, "Instanced actors cannot currently be marked as transient or load as preview. Will flush instances to prevent data corruption.");
