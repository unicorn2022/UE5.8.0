...
bool FChaosPathedMovementState::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	...
	if (Ar.IsSaving())
	{
		...
		// In variable-dt (non-async) mode, MakeChangeTime sets FrameCount = INDEX_NONE, which matches the default,
		// so HasLastChangeTime is always false and nothing is sent. This is intentional: variable-dt implies
		// standalone-only (no networking), so there is nothing to serialize. See note on FChaosPathedMovementInputs.
		HasLastChangeTime = LastChangeTime.FrameCount != Defaults.LastChangeTime.FrameCount;
	}
	else
	{
		// Set to defaults
	...
	Ar.SerializeBits(&HasLastChangeTime, 1);
...
void FChaosPathedMovementState::ToString(FAnsiStringBuilderBase& Out) const
{
	...
	Out.Appendf("Last Change Time: %s | ", *LastChangeTime.ToString());
...
bool FChaosPathedMovementState::ShouldReconcile(const FMoverDataStructBase& AuthorityState) const
{
	...
	if (PropertiesInEffect != TypedAuthority.PropertiesInEffect
		...
		|| LastChangeTime.FrameCount != TypedAuthority.LastChangeTime.FrameCount
...
void FChaosPathedMovementState::Interpolate(const FMoverDataStructBase& From, const FMoverDataStructBase& To, float Pct)
{
	...
	LastChangeTime = (Pct < .5f) ? TypedFrom.LastChangeTime : TypedTo.LastChangeTime;
