...
void UChaosPathedMovementMode::OnRegistered(const FName ModeName, const FMoverSimContext& SimContext)
{
	Chaos::EnsureIsInGameThreadContext();

	Super::OnRegistered(ModeName, SimContext);

	if (const UChaosMoverSimulation* Sim = GetSimulation())
	{
		bIsUsingAsyncPhysics = Sim->IsUsingAsyncPhysics();
	}
...
FMoverTime UChaosPathedMovementMode::MakeChangeTime(const FMoverTimeStep& TimeStep) const
{
	FMoverTime T;
	T.FrameCount = bIsUsingAsyncPhysics ? TimeStep.ServerFrame : INDEX_NONE;
	T.TimeMs = TimeStep.BaseSimTimeMs;
	return T;
}
...
void UChaosPathedMovementMode::GenerateMove_Implementation(const FMoverSimContext& SimContext, const FMoverTickStartData& StartState, const FMoverTimeStep& TimeStep, FProposedMove& OutProposedMove) const
{
}
...
{
	QUICK_SCOPE_CYCLE_COUNTER(ChaosPathedMovementMode_SimulationTick);

	Chaos::EnsureIsInPhysicsThreadContext();

	if (!ensureMsgf(Simulation, TEXT("No Simulation set on ChaosPathedMovementMode")))
	{
		return;
	}
	...
	// Compute the current path progress before we modify movement properties or update LastChangeTime
	...
	if (PreviousProperties.bWantsToPlay && !PreviousMoveState.bHasFinished)
	{
		...
		const double FrameDt = Params.TimeStep.StepMs * 0.001;
		const double Elapsed = PreviousMoveState.LastChangeTime.ElapsedSecondsTo(Params.TimeStep.ToStartTime(), FrameDt);
		CurrentPathProgress += (float)(Elapsed * PreviousDirection * PlaybackSpeed);
	}
			...
			|| (PathedMovementInputs->MovementStartFrame > OutputPathedMovemenState.LatestMovementStartFrame)))
	{
		...
		OutputPathedMovemenState.LastChangeTime = MakeChangeTime(Params.TimeStep);
	...
	{
		const float PlaybackSpeed = (OneWayPlaybackDuration >= 0.0f) ? (1.0f / FMath::Max(OneWayPlaybackDuration, SMALL_NUMBER)) : 0.0f;
		const float Direction = OutputPathedMovemenState.bIsPathProgressionIncreasing ? 1.0f : -1.0f;
		const double FrameDt = Params.TimeStep.StepMs * 0.001;
		const double Elapsed = OutputPathedMovemenState.LastChangeTime.ElapsedSecondsTo(Params.TimeStep.ToNextTime(), FrameDt);
		NewPathProgress = OutputPathedMovemenState.LastChangePathProgress + (float)(Elapsed * Direction * PlaybackSpeed);
	...
	{
		OnInReverseChanging(Params.TimeStep, CurrentProperties.bWantsReversePlayback);
	}
	if (bLoopingJustChanged)
	{
		OnLoopingChanging(Params.TimeStep, CurrentProperties.bWantsLoopingPlayback);
	}
	...
	{
		OnOneWayChanging(Params.TimeStep, CurrentProperties.bWantsOneWayPlayback);
	}

	if (bReceivedNewStartRequest)
	{
		OnPlaybackStarting(Params.TimeStep);
	...
	{
		OnPlaybackStopping(Params.TimeStep, bHasJustFinished);
	}

	if (bHasJustBounced)
	{
		OnPlaybackBounced(Params.TimeStep);
	}
...
void UChaosPathedMovementMode::OnPlaybackStarting(const FMoverTimeStep& SimTime)
{
	// Add a pathed movement started event to the simulation (will be broadcast on GT during post sim)
	if (Simulation)
	{
		Simulation->AddEvent(MakeShared<FPathedMovementStartedEventData>(SimTime));
...
void UChaosPathedMovementMode::OnPlaybackStopping(const FMoverTimeStep& SimTime, bool bReachedEndOfPlayback)
{
	// Add a pathed movement stopped event to the simulation (will be broadcast on GT during post sim)
	if (Simulation)
	{
		Simulation->AddEvent(MakeShared<FPathedMovementStoppedEventData>(SimTime, bReachedEndOfPlayback));
...
void UChaosPathedMovementMode::OnPlaybackBounced(const FMoverTimeStep& SimTime)
{
	// Add a pathed movement bounced event to the simulation (will be broadcast on GT during post sim)
	if (Simulation)
	{
		Simulation->AddEvent(MakeShared<FPathedMovementBouncedEventData>(SimTime));
...
void UChaosPathedMovementMode::OnInReverseChanging(const FMoverTimeStep& SimTime, bool bNewInReverse)
{
	// Add a pathed movement stopped event to the simulation (will be broadcast on GT during post sim)
	if (Simulation)
	{
		Simulation->AddEvent(MakeShared<FPathedMovementInReverseChangedEventData>(SimTime, bNewInReverse));
...
void UChaosPathedMovementMode::OnLoopingChanging(const FMoverTimeStep& SimTime, bool bNewLooping)
{
	// Add a pathed movement stopped event to the simulation (will be broadcast on GT during post sim)
	if (Simulation)
	{
		Simulation->AddEvent(MakeShared<FPathedMovementLoopingChangedEventData>(SimTime, bNewLooping));
...
void UChaosPathedMovementMode::OnOneWayChanging(const FMoverTimeStep& SimTime, bool bNewOneWay)
{
	// Add a pathed movement stopped event to the simulation (will be broadcast on GT during post sim)
	if (Simulation)
	{
		Simulation->AddEvent(MakeShared<FPathedMovementOneWayChangedEventData>(SimTime, bNewOneWay));
...
FTransform UChaosPathedMovementMode::CalcTargetTransform(float OverallPathProgress, const FTransform& BasisTransform) const
{
	...
	const float EasedPathProgress = FAlphaBlend::AlphaToBlendOption(OverallPathProgress, Easing, CustomEasingCurve);
	...
	for (UChaosPathedMovementPatternBase* PathPattern : PathPatterns)
	{
		if (PathPattern)
		{
			const FTransform PatternWorldTransform = PathPattern->CalcTargetTransform(EasedPathProgress, BasisTransformNoScale);
...
void UChaosPathedMovementMode::PostSimulationTick_Async(FChaosMoverPostSimContext& Context)
{
	QUICK_SCOPE_CYCLE_COUNTER(ChaosPathedMovementMode_PostSimulationTick);
	...
	Chaos::FPBDJointConstraintHandle* ActuationConstraintHandle = Simulation->GetActuationConstraintHandle();
	...
	{
		Simulation->EnableActuationConstraint();

		if (!Simulation->IsControlledParticleDynamic())
		{
			Simulation->SetControlledParticleDynamic();
		}

		ActuationConstraintHandle->SetSettings(GetConstraintSettings());
	}
	else
	{
		if (!Simulation->IsControlledParticleKinematic())
		{
			Simulation->SetControlledParticleKinematic();
		}
	}

	const FMoverDefaultSyncState* PostSimDefaultSyncState =
		Context.OutputData.SyncState.SyncStateCollection.FindDataByType<FMoverDefaultSyncState>();
	check(PostSimDefaultSyncState);
	Simulation->SetActuationTargetTransform(PostSimDefaultSyncState->GetTransform_WorldSpace());
}
