...
void UChaosFlyingMode::GenerateMove_Implementation(const FMoverSimContext& SimContext, const FMoverTickStartData& StartState, const FMoverTimeStep& TimeStep, FProposedMove& OutProposedMove) const
{
	QUICK_SCOPE_CYCLE_COUNTER(ChaosFlyingMode_GenerateMove);

	if (!Simulation)
	{
		UE_LOGF(LogChaosMover, Warning, "No Simulation set on UChaosFlyingMode. Check you have a Chaos Backend");
		return;
	}
	...
	{
		UE_LOGF(LogChaosMover, Warning, "ChaosFlyingMode requires FChaosMoverSimulationDefaultInputs and FCharacterDefaultInputs");
		return;
	}
	...
	{
		UE_LOGF(LogChaosMover, Warning, "ChaosFlyingMode unable to read from shared settings (USharedChaosCharacterMovementSettings)");
		return;
	}
...
{
	QUICK_SCOPE_CYCLE_COUNTER(ChaosFlyingMode_SimulationTick);
	...
	if (!DefaultSimInputs)
	{
		UE_LOGF(LogChaosMover, Warning, "ChaosFlyingMode requires FChaosMoverSimulationDefaultInputs");
		return;
	}
...
void UChaosFlyingMode::UpdateCurrentFloor(const FMoverTimeStep& TimeStep) const
{
	if (UMoverBlackboard* SimBlackboard = Simulation->GetBlackboard_Mutable())
	{
		SimBlackboard->Invalidate(CommonBlackboard::LastFloorResult);
