...
{
	UE_LOGF(LogChaosMover, Verbose, "Crouch Activated");
	...
	Simulation->QueueMovementModifier(MoveTemp(StanceModifier));
}

void UChaosCharacterCrouchCheck::Uncrouch(const FSimulationTickParams& TickParams)
{
	UE_LOGF(LogChaosMover, Verbose, "Uncrouch Activated");
...
FTransitionEvalResult UChaosCharacterCrouchCheck::Evaluate_Implementation(const FSimulationTickParams& TickParams) const
{
	...
	if (!Simulation)
	{
		UE_LOGF(LogChaosMover, Warning, "No Simulation set on UChaosCharacterCrouchCheck. Check you have a Chaos Backend");
		return EvalResult;
	}

	if (bTriggerCrouch || bTriggerUncrouch)
	{
		UE_LOGF(LogChaosMover, Warning, "Trying to crouch/uncrouch in the same substep");
		return EvalResult;
	}
	...
	if (CrouchInputs)
	{
		if (Simulation->HasMovementMode(TickParams.StartState.SyncState.MovementMode))
		{
