...
void UChaosSwimmingMode::GenerateMove_Implementation(const FMoverSimContext& SimContext, const FMoverTickStartData& StartState, const FMoverTimeStep& TimeStep, FProposedMove& OutProposedMove) const
{
	QUICK_SCOPE_CYCLE_COUNTER(ChaosSwimmingMode_GenerateMove);

	if (!Simulation)
	{
		UE_LOGF(LogChaosMover, Warning, "No Simulation set on UChaosSwimmingMode. Check you have a Chaos Backend");
		return;
	}
	...
	if (!ensureMsgf(DefaultSimInputs, TEXT("ChaosSwimmingMode requires FChaosMoverSimulationDefaultInputs")))
	{
		return;
	}
	const FCharacterDefaultInputs* CharacterInputs = StartState.InputCmd.InputCollection.FindDataByType<FCharacterDefaultInputs>();
	if (!ensureMsgf(CharacterInputs, TEXT("ChaosSwimmingMode requires FCharacterDefaultInputs")))
	{
		return;
	}	
	const FMoverDefaultSyncState* StartingSyncState = StartState.SyncState.SyncStateCollection.FindDataByType<FMoverDefaultSyncState>();
	if (!ensureMsgf(StartingSyncState, TEXT("ChaosSwimmingMode requires FMoverDefaultSyncState")))
	{
		return;
	}
	...
	{
		UE_LOGF(LogChaosMover, Warning, "ChaosSwimmingMode unable to read from shared settings (USharedChaosCharacterMovementSettings)");
		return;
	}
	...
	if (UE::ChaosMover::CVars::bEnablePreSimGroundCheck)
	{
		...
		SimBlackboard->TryGet(CommonBlackboard::LastWaterResult, WaterResult);
	}
	else
	{
...
{
	QUICK_SCOPE_CYCLE_COUNTER(ChaosSwimmingMode_SimulationTick);
	...
	const FChaosMoverSimulationDefaultInputs* DefaultSimInputs = Simulation->GetLocalSimInput().FindDataByType<FChaosMoverSimulationDefaultInputs>();
	if (!ensureMsgf(DefaultSimInputs != nullptr, TEXT("No FChaosMoverSimulationDefaultInputs in UChaosSwimmingMode::SimulationTick_Implementation, skipping")))
	{
		return;
	}
