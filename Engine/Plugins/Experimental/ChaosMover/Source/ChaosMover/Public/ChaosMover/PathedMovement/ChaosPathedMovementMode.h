...
protected:
	void OnPlaybackStarting(const FMoverTimeStep& SimTime);
	void OnPlaybackStopping(const FMoverTimeStep& SimTime, bool bReachedEndOfPlayback);
	void OnPlaybackBounced(const FMoverTimeStep& SimTime);
	void OnInReverseChanging(const FMoverTimeStep& SimTime, bool bNewInReverse);
	void OnLoopingChanging(const FMoverTimeStep& SimTime, bool bNewLooping);
	void OnOneWayChanging(const FMoverTimeStep& SimTime, bool bNewOneWay);
...
private:
	 ...
	 *
	 * Note that the movement is calculated on the worker thread either way, so it will correctly interact with other bodies in the sim.
	 *
	...
	// Whether the physics simulation runs asynchronously with a fixed timestep.
	// Cached from the simulation at registration time; used only when recording LastChangeTime reference points.
	bool bIsUsingAsyncPhysics = true;

	// Builds the FMoverTime reference point to store when a change occurs (start of the current frame).
	// Sets FrameCount in fixed-dt (async) mode, leaves it INDEX_NONE in variable-dt (non-async) mode.
	FMoverTime MakeChangeTime(const FMoverTimeStep& TimeStep) const;
