...
protected:
	...
	*/
	...
	/** If true, each loop plays forward only (0 → 1). If false, each loop plays there-and-back (0 → 1 → 0). */
	UPROPERTY(EditAnywhere, Category = "Path Pattern")
	bool IsOneWay = true;

	/**
	 * When true, progress bounces at 0 and 1 instead of crossing the seam (wrapping).
	 * Use this for non-looping paths to avoid teleports at the endpoints.
	 * When false (default), progress wraps at the seam — correct for geometrically looping paths.
	 */
	UPROPERTY(EditAnywhere, Category = "Path Pattern")
	bool bBounceAtEnds = false;
