 ...
 *
 * NOTE: MovementStartFrame and LastChangeFrame use integer frame numbers for scheduling and ordering.
 * This is correct in async (fixed-dt) mode, where both server and client share the same synchronized frame counter.
 * In non-async (variable-dt) mode, frame numbers are a local monotonic counter and are NOT synchronized across
 * endpoints. The current implicit assumption is: networked => fixed-dt, variable-dt => standalone only.
 * To support variable-dt networking properly, these fields would need to become FMoverTime.
 */
USTRUCT()
...
struct FChaosPathedMovementState : public FMoverDataStructBase
{
	...
	// The time at which the last change in movement properties occurred (set to the start of that frame).
	// FrameCount is valid in fixed-dt (async) mode; TimeMs is valid in variable-dt (non-async) mode.
	// @see LastChangePathProgress, FMoverTime::ElapsedSecondsTo
	UPROPERTY(VisibleAnywhere, Category = ChaosMover)
	FMoverTime LastChangeTime;

	/** The frame at which we last started playing, or INDEX_NONE if we were never playing.
	 *  Used to order start requests against MovementStartFrame from inputs. See note on FChaosPathedMovementInputs. */
	UPROPERTY(VisibleAnywhere, Category = ChaosMover)
...
struct FPathedMovementStartedEventData : public FMoverSimulationEventData
{
	...
	FPathedMovementStartedEventData(const FMoverTimeStep& InEventTime)
...
struct FPathedMovementStoppedEventData : public FMoverSimulationEventData
{
	...
	FPathedMovementStoppedEventData(const FMoverTimeStep& InEventTime, bool InbReachedEndOfPlayback)
		: FMoverSimulationEventData(InEventTime)
...
struct FPathedMovementBouncedEventData : public FMoverSimulationEventData
{
	...
	FPathedMovementBouncedEventData(const FMoverTimeStep& InEventTime)
		: FMoverSimulationEventData(InEventTime)
	{
	}
...
struct FPathedMovementInReverseChangedEventData : public FMoverSimulationEventData
{
	...
	FPathedMovementInReverseChangedEventData(const FMoverTimeStep& InEventTime, bool InbNewInReverse)
		: FMoverSimulationEventData(InEventTime)
...
struct FPathedMovementLoopingChangedEventData : public FMoverSimulationEventData
{
	...
	FPathedMovementLoopingChangedEventData(const FMoverTimeStep& InEventTime, bool InbNewLooping)
		: FMoverSimulationEventData(InEventTime)
...
struct FPathedMovementOneWayChangedEventData : public FMoverSimulationEventData
{
	...
	FPathedMovementOneWayChangedEventData(const FMoverTimeStep& InEventTime, bool InbNewOneWay)
		: FMoverSimulationEventData(InEventTime)
