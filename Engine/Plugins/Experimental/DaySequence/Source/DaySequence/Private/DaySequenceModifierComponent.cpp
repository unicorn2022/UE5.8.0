...
namespace UE::DaySequence
{
	...
	static TAutoConsoleVariable<bool> CVarModifierBlendPositionValidateTimestamp(
		TEXT("DaySequence.Modifier.BlendPositionValidateTimestamp"),
		true,
		TEXT("When true, day sequence modifier components will check the camera manager's timestamp for validity before returning a position."),
		ECVF_Default
	);
	...
	: Super(Init)
{
	...
	BlendTime = 0.f;
	TimedBlendingLastUpdated = 0.f;
	TimedBlendingStartWeight = 0.f;
	TimedBlendingTargetWeight = 0.f;
	TimedBlendingTargetWeightLastUpdated = 0.f;
...
void UDaySequenceModifierComponent::UpdateEditorPreview(float DeltaTime)
{
	...
	if (bIsComponentEnabled && bPreview && IsRegistered() && !GetWorld()->IsGameWorld())
	{
		...
		if (GetBlendWeight() > UE_SMALL_NUMBER)
		{
			EnableModifier();
...
void UDaySequenceModifierComponent::BindToDaySequenceActor(ADaySequenceActor* DaySequenceActor)
{
	...
	if (ensureMsgf(DaySequenceActor, TEXT("BindToDaySequenceActor called with a null Day Sequence Actor.")))
	{
		...
		DaySequenceActor->GetOnPostInitializeDaySequences().AddUObject(this, static_cast<void (UDaySequenceModifierComponent::*)(ADaySequenceActor::FSubSectionPreserveMap*)>(&UDaySequenceModifierComponent::ReinitializeSubSequence));
		...
		[this, DebugData]()
		{
			(*DebugData).FindOrAdd("Name") = GetPathName(GetOwner() ? GetOwner()->GetOuter() : nullptr);
		...
		case EDayNightCycleMode::LocalFixedTime:
			{
				auto WantsStaticTime = [this]() -> bool
				{
					return IsValidChecked(this) && bIsEnabled && bIsComponentEnabled &&
						(DayNightCycle == EDayNightCycleMode::FixedTime ||
						DayNightCycle == EDayNightCycleMode::RandomFixedTime ||
						DayNightCycle == EDayNightCycleMode::LocalFixedTime);
				};

				auto GetStaticTime = [this, RandomTime, WantsStaticTime](FStaticTimeInfo& OutRequest) -> bool
				{
					if (WantsStaticTime())
					{
				...
				EStaticTimeChannel Channel = DayNightCycle == EDayNightCycleMode::LocalFixedTime ? EStaticTimeChannel::LocalFixedTime : EStaticTimeChannel::FixedTime;
				TargetActor->RegisterStaticTimeContributor({this, Bias, WantsStaticTime, GetStaticTime, Channel});
			}
			break;
...
void UDaySequenceModifierComponent::ReinitializeSubSequences()
{
	ReinitializeSubSequence(nullptr);
}
...
UMovieSceneSubSection* UDaySequenceModifierComponent::InitializeDaySequence(const FDaySequenceCollectionEntry& Entry)
{
	...
	auto CreateSubTrack = [this, MovieScene, bEnableBlending = Entry.bEnableHierarchicalBlending](UDaySequence* Sequence, int BiasOffset, bool bActivate)
	{
		...
		SubSection->Parameters.Flags            = EMovieSceneSubSectionFlags::OverrideRestoreState
												...
												| (bEnableBlending ? EMovieSceneSubSectionFlags::BlendHierarchicalBias : EMovieSceneSubSectionFlags::None);
	...
	UMovieSceneSubSection* SubSection = CreateSubTrack(Entry.Sequence, Entry.BiasOffset, bActivate);
	...
	[SubSection]()
	{
		return IsValid(SubSection) ? SubSection->Parameters.HierarchicalBias : 0;
	}));
#endif
...
void UDaySequenceModifierComponent::SetBlendTime(float TimeInSeconds)
{
	BlendTime = TimeInSeconds;
}

float UDaySequenceModifierComponent::GetBlendTime() const
{
	return BlendTime;
}
...
#endif
	if (const APlayerController* BlendTarget = WeakBlendTarget.Get();
		BlendTarget && BlendTarget->PlayerCameraManager &&
		(!UE::DaySequence::CVarModifierBlendPositionValidateTimestamp.GetValueOnAnyThread() || BlendTarget->PlayerCameraManager->GetCameraCacheTime() != 0.f))
	{
...
float UDaySequenceModifierComponent::UpdateInternalBlendWeight()
{
	...
	if (BlendTime > 0.0f)
	{
		if (const UWorld* World = GetWorld())
		{
			...
			float DeltaWeightRange;
			float DeltaTime;
			if (Mode == EDaySequenceModifierMode::Time)
			{
				TimedBlendingTargetWeight = InternalBlendWeight > 0.f ? 1.f : 0.f;
				TimedBlendingStartWeight = PreviousBlendWeight;
				DeltaTime = CurrentTime - TimedBlendingLastUpdated;
				DeltaWeightRange = InternalBlendWeight > 0.f ? 1.f : -1.f;
			}
			else // Global || Volume
			{
				if (FMath::Abs(InternalBlendWeight - TimedBlendingTargetWeight) > 0.f)
				{
					TimedBlendingTargetWeight = InternalBlendWeight;
					TimedBlendingStartWeight = PreviousBlendWeight;
					TimedBlendingTargetWeightLastUpdated = TimedBlendingLastUpdated == 0.f ? CurrentTime : TimedBlendingLastUpdated;
				}
				DeltaTime = CurrentTime - TimedBlendingTargetWeightLastUpdated;
				DeltaWeightRange = TimedBlendingTargetWeight - TimedBlendingStartWeight;
			}

			TimedBlendingLastUpdated = CurrentTime;
			
			const float DeltaTimeRatio = FMath::Clamp(DeltaTime / FMath::Max(BlendTime, UE_SMALL_NUMBER), 0.f, 1.f);
			const float DeltaWeight = DeltaWeightRange * DeltaTimeRatio;

			const float MinWeight = DeltaWeightRange >= 0.f ? TimedBlendingStartWeight : TimedBlendingTargetWeight;
			const float MaxWeight = DeltaWeightRange >= 0.f ? TimedBlendingTargetWeight : TimedBlendingStartWeight;
			InternalBlendWeight = FMath::Clamp(TimedBlendingStartWeight + DeltaWeight, MinWeight, MaxWeight);
			
			UE_LOGF(LogDaySequence, VeryVerbose, "[BlendWeight]: Start = %f, Target = %f, DeltaTime = %f, Weight = %f", TimedBlendingStartWeight, TimedBlendingTargetWeight, DeltaTime, InternalBlendWeight);
		}
		else
		{
...
FOnPostEnableModifier& UDaySequenceModifierComponent::GetOnPostEnableModifier()
{
	return OnPostEnableModifier;
}
