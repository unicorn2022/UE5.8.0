...
namespace UE::DaySequence
{
	...
	bool FStaticTimeManager::HasStaticTime(EStaticTimeChannel InChannels) const
	{
		for (const FStaticTimeContributor& Contributor : Contributors)
		{
			if (Contributor.UserObject.Get() &&
				EnumHasAnyFlags(InChannels, Contributor.Channels) &&
				Contributor.WantsStaticTime())
			{
				return true;
			}
		}
		...
		ResetBlendState(InChannels);

		return false;
	}
	
	float FStaticTimeManager::GetStaticTime(float InitialTime, float DayLength, const EStaticTimeChannel InChannels) const
	{
		FStaticTimeChannelData& Channel = Channels.FindOrAdd(InChannels); 
		...
		for (int32 PriorityGroupStartIdx = 0; PriorityGroupStartIdx < Contributors.Num();)
		{
			...
			// Process this group 
			const FStaticTimeInfo GroupInfo = ProcessPriorityGroup(InChannels, PriorityGroupStartIdx, PriorityGroupEndIdx);
		...
		if (Channel.LastBlendDelta.IsSet() &&
			Channel.LastBlendDirection.IsSet() &&
			Channel.LastBlendDirection.GetValue() != BlendDirection)
		{
			const float BlendDeltaChange = BlendDelta - Channel.LastBlendDelta.GetValue();
			if (FMath::Abs(BlendDeltaChange) > HalfDayLength)
			{
				...
				Channel.LastBlendWinding += BlendDeltaSign;
				Channel.LastBlendOffset = FMath::WrapExclusive(Channel.LastBlendOffset + BlendDeltaSign, -1, 2);
			}
			else
			{
				// Crossed TargetTime
				if (Channel.LastBlendOffset != 0)
				{
		...
		{
			float BlendOffset = Channel.LastBlendOffset * DayLength;
...
#if !NO_LOGGING
			UE_LOGF(LogDaySequence, VeryVerbose, "BlendDelta: %f | BlendDir: %d | BlendWinding: %d | BlendOffset: %f | StartTime: %f | InitialTime: %f | TargetTime: %f", BlendDelta, BlendDirection, Channel.LastBlendWinding, BlendOffset, StartTime, InitialTime, TargetTime);
			UE_LOGF(LogDaySequence, VeryVerbose, "AccumTime: %f | AccumWeight: %f", AccumulatedTime, AccumulatedWeight);
#endif
		}
		else
		{
		...
		Channel.LastBlendDelta = BlendDelta;
		Channel.LastBlendDirection = BlendDirection;
		
		return AccumulatedTime;
	}

	float FStaticTimeManager::GetTargetStaticTime(const float InitialTime, EStaticTimeChannel InChannels) const
	{
		// Process batches of contributors based on priority
		for (int32 PriorityGroupStartIdx = 0; PriorityGroupStartIdx < Contributors.Num();)
		{
			...
			// Process this group 
			const FStaticTimeInfo GroupInfo = ProcessPriorityGroup(InChannels, PriorityGroupStartIdx, PriorityGroupEndIdx, 1.0f);

			// Return the highest priority group that contains an active contributor
			if (GroupInfo.BlendWeight > 0.f)
			{
				return GroupInfo.StaticTime;
			}
	...
	void FStaticTimeManager::ResetBlendState(const EStaticTimeChannel InChannels) const
	{
		FStaticTimeChannelData& Channel = Channels.FindOrAdd(InChannels);
		Channel.LastBlendDirection.Reset();
		Channel.LastBlendDelta.Reset();
		Channel.LastBlendWinding = 0;
		Channel.LastBlendOffset = 0;
	}


	FStaticTimeInfo FStaticTimeManager::ProcessPriorityGroup(const EStaticTimeChannel InChannels, int32 StartIdx, int32 EndIdx, TOptional<float> OverrideWeight) const
	{
		...
		for (int32 CurrentIdx = StartIdx; CurrentIdx <= EndIdx; ++CurrentIdx)
		{
			...
			if (!CurrentContributor.UserObject.Get() ||
				!EnumHasAnyFlags(InChannels, CurrentContributor.Channels) ||
				!CurrentContributor.WantsStaticTime())
			{
				continue;
			}
			...
			GroupInfo.BlendWeight += OverrideWeight.IsSet() ? OverrideWeight.GetValue() : ContributorInfo.BlendWeight;
