...
{
#if DAY_SEQUENCE_ENABLE_DRAW_DEBUG
	FDaySequenceDebugEntry::FDaySequenceDebugEntry(FShowDebugDataConditionFunction InShowCondition, FGetDebugDataFunction InGetData, FGetNumericValue InGetNumericValue)
	: ShowCondition(InShowCondition)
	, GetData(InGetData)
	, GetNumericValue(InGetNumericValue)
	{}
...
UMovieSceneSubSection* ADaySequenceActor::InitializeDaySequence(const FDaySequenceCollectionEntry& SequenceAsset)
{
	...
	if (SubTrack)
	{
		...
		{
			UE_LOGF(LogDaySequence, Warning, "Failed to create UMovieSceneSubSection in ADaySequenceActor::InitializeDaySequence");
		}
	}
	else
	{
		UE_LOGF(LogDaySequence, Warning, "Failed to create UDaySequenceTrack in ADaySequenceActor::InitializeDaySequence");
	}
...
float ADaySequenceActor::GetApparentTimeOfDay() const
{
	...
	// Query both FixedTime and LocalFixedTime channels for Apparent time of day.
	constexpr EStaticTimeChannel ApparentChannels = EStaticTimeChannel::FixedTime | EStaticTimeChannel::LocalFixedTime;
	float Time;
	if (StaticTimeManager && StaticTimeManager->HasStaticTime(ApparentChannels))
	{
		Time = StaticTimeManager->GetStaticTime(GetTimeOfDay(), DayLength.ToHours(), ApparentChannels);
	}
	else
	{
		Time = GetTimeOfDay();
	}
	return GetInterpCurveTimeOfDay(Time);
}

float ADaySequenceActor::GetTargetApparentTimeOfDay() const
{
	using namespace UE::DaySequence;
	...
	if (StaticTimeManager && StaticTimeManager->HasStaticTime(ApparentChannels))
	{
		Time = StaticTimeManager->GetTargetStaticTime(GetTimeOfDay(), ApparentChannels);
	}
	else
	{
		Time = GetTimeOfDay();
...
float ADaySequenceActor::GetStaticTimeOfDay() const
{
	...
	if (HasStaticTimeOfDay())
	{
		Time = StaticTimeManager->GetStaticTime(GetTimeOfDay(), DayLength.ToHours(), EStaticTimeChannel::FixedTime);
	}
	else
	{
		Time = GetTimeOfDay();
...
float ADaySequenceActor::GetInterpCurveTimeOfDay(float Time) const
{
	const bool bUseCurve = bUseInterpCurve && DayInterpCurve && !bForceDisableDayInterpCurve;
	return bUseCurve ? DayInterpCurve->FloatCurve.Eval(Time, Time) : Time;
}
...
void ADaySequenceActor::Play()
{
	if (!bRunDayCycle)
	{
		UE_LOGF(LogDaySequence, Warning, "Cannot begin/resume playback while bRunDayCycle is false.");
		return;
	}
...
void ADaySequenceActor::OnShowDebugInfo(AHUD* HUD, UCanvas* Canvas, const FDebugDisplayInfo& DisplayInfo, float& YL, float& YPos)
{
	...
	if (!Canvas || !GEngine || GetNetMode() == NM_DedicatedServer || !HUD || GetWorld() != HUD->GetWorld())
	{
		return;
	}
	...
	class FDebugEntriesLessThan
	{
	public:

		bool operator()(const TWeakPtr<FDaySequenceDebugEntry>& LHS, const TWeakPtr<FDaySequenceDebugEntry>& RHS) const
		{
			const TSharedPtr<FDaySequenceDebugEntry> PinnedLHS = LHS.Pin();
			const TSharedPtr<FDaySequenceDebugEntry> PinnedRHS = RHS.Pin();

			if (PinnedLHS && PinnedRHS)
			{
				return (PinnedLHS->GetNumericValue ? PinnedLHS->GetNumericValue() : 0) < (PinnedRHS->GetNumericValue ? PinnedRHS->GetNumericValue() : 0);
			}

			// LHS is expired & RHS is valid, expired elements are always less than valid elements.
			if (PinnedLHS)
			{
				return false;
			}

			// LHS is valid & RHS is expired, expired elements are always less than valid elements.
			if (PinnedRHS)
			{
				return true;
			}

			// If both are expired, then we consider the values equal (so LHS is not less than RHS).
			return false;
		}
	};
	...
	for (TPair<FName, TPair<FDebugEntryArray, FDebugCategoryDrawFunction>>& Category : DebugEntries)
	{
		...
		if (HUD->ShouldDisplayDebug(ShowDebug_GeneralCategory) || HUD->ShouldDisplayDebug(Category.Key))
		{
			...
			Entries.StableSort(FDebugEntriesLessThan());
