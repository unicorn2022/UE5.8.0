...
void UDaySequence::PostLoad()
{
	...
	for (int32 Index = 0; Index < MovieScene->GetSpawnableCount(); ++Index)
	{
		...
		if (!Spawnable.GetObjectTemplate())
		{
			...
			UE_LOGF(LogDaySequence, Warning, "Spawnable '%ls' with ID '%ls' does not have a valid object template", *Spawnable.GetName(), *Spawnable.GetGuid().ToString());
		}
	}
#endif
}
...
FGuid UDaySequence::FindOrAddBinding(UObject* InObject)
{
	...
	if (Actor && Actor->ActorHasTag("SequencerActor"))
	{
		...
		UE_LOGF(LogDaySequence, Error, "Unable to possess object '%ls' since it is, or is part of a spawnable that is not in this sequence.", *InObject->GetName());
		return FGuid();
	}
	...
	{
		UE_LOGF(LogDaySequence, Error, "Unable to possess object '%ls' because it's parent could not be bound.", *InObject->GetName());
		return FGuid();
	}
