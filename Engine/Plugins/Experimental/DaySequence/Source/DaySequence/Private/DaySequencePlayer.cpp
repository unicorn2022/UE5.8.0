...
void UDaySequencePlayer::PlayInternal()
{
	...
	if (!IsPlaying() && Sequence && CanPlay())
	{
		...
		UE_LOGF(LogDaySequence, Verbose, "PlayInternal - %ls (current status: %ls)", *SequenceName, *UEnum::GetValueAsString(Status));
...
void UDaySequencePlayer::Pause()
{
	...
	const FString SequenceName = GetSequenceName(true);
	UE_LOGF(LogDaySequence, Verbose, "Pause - %ls (current status: %ls)", *SequenceName, *UEnum::GetValueAsString(Status));
	...
	if (IsPlaying())
	{
		...
		auto FinishPause = [this]
		{
			...
			UE_LOGF(LogDaySequence, Verbose, "Paused - %ls", *SequenceName);
...
void UDaySequencePlayer::StopInternal(FFrameTime TimeToResetTo)
{
	...
	const FString SequenceName = GetSequenceName(true);
	UE_LOGF(LogDaySequence, Verbose, "StopInternal - %ls (at: %ls, current status: %ls)", *SequenceName, *LexToString(TimeToResetTo), *UEnum::GetValueAsString(Status));
	...
	if (IsPlaying() || IsPaused())
	{
		...
		auto OnFlushed = [this, TimeToResetTo]
		{
			...
			const FString SequenceName = this->GetSequenceName(true);
			UE_LOGF(LogDaySequence, Verbose, "Stopped - %ls", *SequenceName);
...
bool UDaySequencePlayer::HasEverEvaluated() const
{
	return bHasEverEvaluated;
}
...
void UDaySequencePlayer::SetFrameRate(FFrameRate FrameRate)
{
	...
	{
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogDaySequence, Error, "Attempting to set sequence %ls with an invalid frame rate: %ls", *SequenceName, *FrameRate.ToPrettyText().ToString());
		return;
	}
	...
	if (MovieScene)
	{
		...
		{
			UE_LOGF(LogDaySequence, Warning, "Attempting to play back a sequence with tick resolution of %f ticks per second frame locked to %f fps, which is not a multiple of the resolution.", MovieScene->GetTickResolution().AsDecimal(), FrameRate.AsDecimal());
		}
	}
...
void UDaySequencePlayer::SetCompletionModeOverride(EMovieSceneCompletionModeOverride CompletionModeOverride)
{
	...
	{
		UE_LOGF(LogDaySequence, Warning, "Attempting to set completion mode override to force restore state while the sequence is already playing. Force restore state must be set before starting playback.");
	}
...
void UDaySequencePlayer::Initialize(UMovieSceneSequence* InSequence)
{
	...
	if (MovieScene)
	{
		...
		UE_LOGF(LogDaySequence, Verbose, "Initialize - MovieSceneSequence: %ls, TickResolution: %ls, DisplayRate: %ls", *InSequence->GetName(), *TickResolution.ToPrettyText().ToString(), *DisplayRate.ToPrettyText().ToString());

		if (!TickResolution.IsValid() || TickResolution.Numerator <= 0)
		{
			const FString SequenceName = GetSequenceName(true);
			const FFrameRate DefaultTickResolution(60000, 1);
			UE_LOGF(LogDaySequence, Error, "Attempting to set sequence %ls with an invalid tick resolution: %ls, defaulting to: %ls", *SequenceName, *TickResolution.ToPrettyText().ToString(), *DefaultTickResolution.ToPrettyText().ToString());
		...
		if (!DisplayRate.IsValid() || DisplayRate.Numerator <= 0)
		{
			...
			UE_LOGF(LogDaySequence, Error, "Attempting to set sequence %ls with an invalid display rate: %ls, defaulting to: %ls", *SequenceName, *DisplayRate.ToPrettyText().ToString(), *DefaultDisplayRate.ToPrettyText().ToString());
...
void UDaySequencePlayer::UpdateTimeCursorPosition_Internal(FFrameTime NewPosition, EUpdatePositionMethod Method, bool bHasJumpedOverride)
{
	...
	if (Duration == 0 && DurationSubFrames == 0.f)
	{
		if (bWarnZeroDuration)
		{
			...
			const FString SequenceName = GetSequenceName(true);
			UE_LOGF(LogDaySequence, Warning, "Attempting to play back sequence %ls with zero duration", *SequenceName);
		}
		return;
	}
	bWarnZeroDuration = true;
	...
	if (Method == EUpdatePositionMethod::Play && ShouldStopOrLoop(NewPosition))
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogDaySequence, Verbose,
...
void UDaySequencePlayer::UpdateMovieSceneInstance(FMovieSceneEvaluationRange InRange, EMovieScenePlayerStatus::Type PlayerStatus, const FMovieSceneUpdateArgs& Args)
{
	if (Observer && !Observer->CanObserveSequence())
	{
		UE_LOGF(LogDaySequence, Error, "Refusing to update an unobservable sequence! Did it become unobservable during playback?");
		return;
	}
	...
	if (UE_LOG_ACTIVE(LogDaySequence, VeryVerbose))
	{
		...
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogDaySequence, VeryVerbose, "Evaluating sequence %ls at frame %d, subframe %f (%f fps).", *SequenceName, CurrentTime.Time.FrameNumber.Value, CurrentTime.Time.GetSubFrame(), CurrentTime.Rate.AsDecimal());
	}
#endif
...
void UDaySequencePlayer::RPC_ExplicitServerUpdateEvent_Implementation(EUpdatePositionMethod EventMethod, FFrameTime MarkerTime, int32 NewSerialNumber)
{
	...
	if (UE_LOG_ACTIVE(LogDaySequence, Verbose))
	{
		...
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogDaySequence, Verbose, "Explicit update event for sequence %ls %ls @ %ls. Server has moved to %ls @ %ls.",
			*SequenceName,
...
void UDaySequencePlayer::RPC_OnStopEvent_Implementation(FFrameTime StoppedTime, int32 NewSerialNumber)
{
	...
	if (UE_LOG_ACTIVE(LogDaySequencePlayerRepl, Verbose))
	{
		const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogDaySequencePlayerRepl, Verbose, "Explicit Stop() event for sequence %ls %ls @ frame %d, subframe %f. Server has stopped at frame %d, subframe %f.",
	...
	{
		UE_LOGF(LogDaySequencePlayerRepl, Verbose, "Catching up to explicit stop time %ls", *LexToString(StoppedTime));
...
void UDaySequencePlayer::RPC_OnFinishPlaybackEvent_Implementation(FFrameTime StoppedTime, int32 NewSerialNumber)
{
	...
	const FString SequenceName = GetSequenceName(true);
	UE_LOGF(LogDaySequence, Verbose, "Received RPC event to finish sequence %ls at frame %ls.", *SequenceName, *LexToString(StoppedTime));
...
void UDaySequencePlayer::PostNetReceive()
{
	...
	if (NetSyncProps.LastKnownSerialNumber < SerialNumber)
	{
		...
		const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogDaySequencePlayerRepl, Verbose, 
	...
	if (Observer && !Observer->CanObserveSequence())
	{
		...
		if (UE_LOG_ACTIVE(LogDaySequencePlayerRepl, Verbose))
		{
			const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
			const FString SequenceName = GetSequenceName(true);
			UE_LOGF(LogDaySequencePlayerRepl, Verbose, "Ignoring network update for unobservable sequence %ls %ls @ %ls. Server is %ls @ %ls.",
				*SequenceName,
				*UEnum::GetValueAsString(Status.GetValue()), *LexToString(CurrentTime),
				*UEnum::GetValueAsString(NetSyncProps.LastKnownStatus.GetValue()), *LexToString(NetSyncProps.LastKnownPosition));
		}
	...
	if (UE_LOG_ACTIVE(LogDaySequencePlayerRepl, VeryVerbose))
	{
		const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
		const FString SequenceName = GetSequenceName(true);
		UE_LOGF(LogDaySequencePlayerRepl, VeryVerbose, "Network sync for sequence %ls %ls @ %ls. Server is %ls @ %ls.",
			*SequenceName,
			*UEnum::GetValueAsString(Status.GetValue()), *LexToString(CurrentTime),
			*UEnum::GetValueAsString(NetSyncProps.LastKnownStatus.GetValue()), *LexToString(NetSyncProps.LastKnownPosition));
	}
	...
	if (Status == EMovieScenePlayerStatus::Playing)
	{
		...
		if (Difference > LagThreshold + PingLag)
		{
			...
			if (UE_LOG_ACTIVE(LogDaySequencePlayerRepl, Log))
			{
				const FFrameTime CurrentTime = PlayPosition.GetCurrentPosition();
				const FString SequenceName = GetSequenceName(true);
				UE_LOGF(LogDaySequencePlayerRepl, Log, "Correcting de-synced play position for sequence %ls %ls @ %ls. Server is %ls @ %ls, (smoothed: %ls). Client ping is %.2fms.",
					*SequenceName, 
					*UEnum::GetValueAsString(Status.GetValue()), *LexToString(CurrentTime),
					*UEnum::GetValueAsString(NetSyncProps.LastKnownStatus.GetValue()), *LexToString(NetSyncProps.LastKnownPosition),
