...
namespace UE::DaySequence
{
	...
	struct FDaySequenceDebugEntry
	{
		...
		using FGetNumericValue = TFunction<int32()>;

		FDaySequenceDebugEntry(FShowDebugDataConditionFunction InShowCondition, FGetDebugDataFunction InGetData, FGetNumericValue InGetNumericValue);
		
		FShowDebugDataConditionFunction ShowCondition;
		FGetDebugDataFunction GetData;
		FGetNumericValue GetNumericValue;
	};
...
public:
	...
	/**
	 * Get the current target apparent time of day in hours.
	 * This is distinct from apparent time of day in that it is only capturing the target time of static time contributors.
	 * The target time is determined by the highest priority static time contributor independent of blend weight. Multiple
	 * contributors at the same priority will return the average of their target times.
	 * 
	 * @return float, time in hours
	 */
	UE_API float GetTargetApparentTimeOfDay() const;
	...
	/**
	 * Get this day sequence actor's static (fixed) time-of-day, or GetTimeOfDay() if it doesn't have one
	 */
	UFUNCTION(BlueprintCallable, Category="TimeOfDay")
...
protected:
	...
	UE_API float GetInterpCurveTimeOfDay(float Time) const;
