...
public:
	...
	/**
	 * Rebuilds this component's subsections in the root sequence from the current UserDaySequence/DaySequenceCollections.
	 * Call after swapping collection assets at runtime.
	 */
	UFUNCTION(BlueprintCallable, Category="Day Sequence")
	DAYSEQUENCE_API void ReinitializeSubSequences();
	...
	/** Sets the time to blend towards the target weight. */
	DAYSEQUENCE_API void SetBlendTime(float TimeInSeconds);
	DAYSEQUENCE_API float GetBlendTime() const;
	...
	DAYSEQUENCE_API TArray<UShapeComponent*> GetVolumeShapeComponents() const;

	FOnPostEnableModifier& GetOnPostEnableModifier();
...
protected:
	...
	/** The user provided collection. This is an alternative to UserDaySequence. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Day Sequence", meta=(EditCondition="bUseCollection", EditConditionHides, DisplayAfter="bUseCollection"))
	...
	/** Defines the amount of time (in seconds) that it takes for the blend weight to reach the target weight */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Day Sequence", meta=(DisplayAfter="Mode"))
	float BlendTime;
...
private:
	...
	/** Used by UpdateInternalBlendWeight to compute the weight delta for timed blends */
	float TimedBlendingStartWeight;
	float TimedBlendingTargetWeight;
	/** Used by UpdateInternalBlendWeight to compute time delta between TimedBlendingTargetWeight updates for global/volume timed blends. */
	float TimedBlendingTargetWeightLastUpdated;
