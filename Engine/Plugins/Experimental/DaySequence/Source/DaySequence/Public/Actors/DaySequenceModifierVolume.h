...
public:
	...
	UE_API void OnPostEnableModifier();
...
protected:
	...
	UE_DEPRECATED(5.8, "Replaced with CachedPlayer")
	UPROPERTY(Transient, DuplicateTransient, meta = (DeprecatedProperty, DeprecationMessage = "Replaced with CachedPlayer"))
	TObjectPtr<APlayerController> CachedPlayerController;

	UPROPERTY(Transient, DuplicateTransient)
	TObjectPtr<UPlayer> CachedPlayer;
...
private:
	 ...
	 * Key: Local player
	 * Value: Optional transient modifier component
	 */
	UPROPERTY(Transient, DuplicateTransient)
	TMap<TObjectPtr<UPlayer>, TObjectPtr<UDaySequenceModifierComponent>> AdditionalPlayers;
