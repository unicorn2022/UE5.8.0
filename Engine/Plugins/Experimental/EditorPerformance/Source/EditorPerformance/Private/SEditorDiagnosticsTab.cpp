...
{	
	...
	UI_COMMAND(SetShowCrashes, "Crashes", "Displays the crashes log", EUserInterfaceActionType::ToggleButton, FInputChord());
}
...
TSharedRef<SWidget> SEditorDiagnosticsTab::CreateToolBar()
{
	...
	TWeakPtr<FUICommandInfo> ShowCrashesCommand = FEditorDiagnosticsCommands::Get().SetShowCrashes;
	ToolBarCommandList->MapAction(FEditorDiagnosticsCommands::Get().SetShowCrashes,
		FExecuteAction::CreateRaw(this, &SEditorDiagnosticsTab::SetShowCrashes),
		FCanExecuteAction(),
		FGetActionCheckState::CreateLambda([this, ShowCrashesCommand] ()
			{
				return SelectedCommand == ShowCrashesCommand ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
			}));
	...
	ToolBarBuilder.AddToolBarButton(FEditorDiagnosticsCommands::Get().SetShowCrashes);
...
TSharedRef<SWidget> SEditorDiagnosticsTab::CreateCrashesLogPanel()
{
	using namespace UE::Editor::CrashDiagnostics;
	FCrashDiagnosticsModule& CrashDiagnostics = FCrashDiagnosticsModule::GetChecked();
	return CrashDiagnostics.CreateCrashLogPanel();
}
...
void SEditorDiagnosticsTab::SetShowCrashes()
{
	if (TSharedPtr<SBorder> BorderWidget = ContentBorder.Pin())
	{
		BorderWidget->SetContent(CreateCrashesLogPanel());
		SelectedCommand = FEditorDiagnosticsCommands::Get().SetShowCrashes;
	}
}
