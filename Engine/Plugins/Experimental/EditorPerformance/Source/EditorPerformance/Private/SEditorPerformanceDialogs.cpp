...
TSharedRef<SWidget> SEditorPerformanceReportDialog::GetHintGridPanel()
{
	...
	if (KPIHints.Num()>0)
	{
		...
		if (EditorPerfModule.GetKPIRegistry().GetKPIValue(KPIHint.Id, KPIValue) == true)
		{
			...
			if (!KPIHint.DocumentationLink.IsEmpty())
			{
				Panel->AddSlot(0, Row)
					.HAlign(HAlign_Left)
					...
					[
						SNew(SHyperlink)
							...
							.ToolTipText(FText::FromString(*KPIHint.DocumentationLink))
							.OnNavigate_Lambda([KPIHint]() { IDocumentation::Get()->Open(KPIHint.DocumentationLink); })
					];
				...
				[
					...
					[
						...
						.Image_Lambda([KPIId = KPIValue.Id]()
						{
							...
							if (EditorPerfModule.GetKPIRegistry().GetKPIValue(KPIId, KPIValue))
							{
								if (KPIValue.State == FKPIValue::Bad)
								{
									switch (KPIValue.Severity)
									{
									case FKPIValue::ESeverity::Critical:
										return FAppStyle::Get().GetBrush("EditorPerformance.Report.Critical");
									case FKPIValue::ESeverity::Major:
										return FAppStyle::Get().GetBrush("EditorPerformance.Report.Major");
									case FKPIValue::ESeverity::Minor:
									default:
										return FAppStyle::Get().GetBrush("EditorPerformance.Report.Minor");
									}
								}
								else
								{
