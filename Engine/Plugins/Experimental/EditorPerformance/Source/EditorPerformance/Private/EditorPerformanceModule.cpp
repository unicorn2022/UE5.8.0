...
const FName DiskCategoryName = TEXT("Disk");
const FText DiskCategoryText = LOCTEXT("Disk", "Disk");
...
const FName DynamicInvalidationKPIName = TEXT("Dynamic Invalidation");
const FText DynamicInvalidationKPIText = LOCTEXT("DynamicInvalidation", "Dynamic Invalidation");

const FName EngineDirFreeSpaceKPIName = TEXT("Engine Dir Free Space");
const FText EngineDirFreeSpaceKPIText = LOCTEXT("EngineDirFreeSpace", "Engine Dir Free Space");

const FName ProjectDirFreeSpaceKPIName = TEXT("Project Dir Free Space");
const FText ProjectDirFreeSpaceKPIText = LOCTEXT("ProjectDirFreeSpace", "Project Dir Free Space");

const FName UserDirFreeSpaceKPIName = TEXT("User Dir Free Space");
const FText UserDirFreeSpaceKPIText = LOCTEXT("UserDirFreeSpace", "User Dir Free Space");

// Thresholds
TOptional<float> NoSetLimit;
float LocalDDCEfficiencyKPILimit = 85.f;
...
float DynamicInvalidationKPILimit = 1.0f;
...
void FEditorPerformanceModule::InitializeEditor()
{
	...
	FTimerDelegate HighFrequencyUpdateDelegate;
	HighFrequencyUpdateDelegate.BindRaw(this, &FEditorPerformanceModule::UpdateKPIsHighFrequency);
	GEditor->GetTimerManager()->SetTimer(HighFrequencyUpdateTimerHandle, HighFrequencyUpdateDelegate, HighFrequencyUpdateIntervalSeconds, bLoop);
	
	FTimerDelegate LowFrequencyUpdateDelegate;
	LowFrequencyUpdateDelegate.BindRaw(this, &FEditorPerformanceModule::UpdateKPIsLowFrequency);
	GEditor->GetTimerManager()->SetTimer(LowFrequencyUpdateTimerHandle, LowFrequencyUpdateDelegate, LowFrequencyUpdateIntervalSeconds, bLoop);
	...
	if (Menu != nullptr)
	{
		...
		FToolMenuSection& EditorPerfSection = Menu->AddSection("EditorPerf", FText::GetEmpty(), FToolMenuInsert("Insights", EToolMenuInsertType::Before));
	...
	if (EditorPerformanceSettings && EditorPerformanceSettings->bEnableExperimentalFeatures)
	{
	...
	// Update low frequency KPIs right away so that we have initial values for them without having to way for the timer to tick once
	UpdateKPIsLowFrequency();
}
...
void FEditorPerformanceModule::TerminateEditor()
{
	...
	if (GEditor)
	{
		GEditor->GetTimerManager()->ClearTimer(HighFrequencyUpdateTimerHandle);
		GEditor->GetTimerManager()->ClearTimer(LowFrequencyUpdateTimerHandle);
		GEditor->GetTimerManager()->ClearTimer(HitchSamplerTimerHandle);
	}
...
void FEditorPerformanceModule::InitializeKPIs()
{
	// Declare the KPIs 
	{
		...
		FKPIValue DynamicInvalidationKPIValue = FKPIValue(EditorCategoryName, EditorCategoryText, DynamicInvalidationKPIName,
			DynamicInvalidationKPIText, FKPIValue::ESeverity::Major, 0.0, DynamicInvalidationKPILimit, FKPIValue::GreaterThanOrEqual, FKPIValue::Custom);
		DynamicInvalidationKPIValue.CustomDisplayValueGetter.BindRaw(this, &FEditorPerformanceModule::GetDynamicInvalidationDisplayValue);
		DynamicInvalidationKPI		= KPIRegistry.DeclareKPIValue(MoveTemp(DynamicInvalidationKPIValue));
	}
	...
	{
		LocalDDCEfficiencyKPI		= KPIRegistry.DeclareKPIValue(CacheCategoryName, CacheCategoryText, LocalDDCEfficiencyKPIName, LocalDDCEfficiencyKPIText, 100.0, LocalDDCEfficiencyKPILimit, FKPIValue::GreaterThan, FKPIValue::Percent);
	...
	
	{
		...
		FKPIValue MemoryPressureKPIValue = FKPIValue(MemoryCategoryName, MemoryCategoryText, MemoryPressureKPIName, MemoryPressureKPIText, FKPIValue::ESeverity::Critical,
	...
	{
		FKPIValue EngineDirFreeSpaceKPIValue	= FKPIValue(DiskCategoryName, DiskCategoryText, EngineDirFreeSpaceKPIName, EngineDirFreeSpaceKPIText, FKPIValue::ESeverity::Critical,
															0.0, 5.0, FKPIValue::GreaterThanOrEqual, FKPIValue::GigaBytes);
		EngineDirFreeSpaceKPI					= KPIRegistry.DeclareKPIValue(EngineDirFreeSpaceKPIValue);

		FKPIValue ProjectDirFreeSpaceKPIValue	= FKPIValue(DiskCategoryName, DiskCategoryText, ProjectDirFreeSpaceKPIName, ProjectDirFreeSpaceKPIText, FKPIValue::ESeverity::Critical,
															0.0, 5.0, FKPIValue::GreaterThanOrEqual, FKPIValue::GigaBytes);
		ProjectDirFreeSpaceKPI					= KPIRegistry.DeclareKPIValue(ProjectDirFreeSpaceKPIValue);

		FKPIValue UserDirFreeSpaceKPIValue		= FKPIValue(DiskCategoryName, DiskCategoryText, UserDirFreeSpaceKPIName, UserDirFreeSpaceKPIText, FKPIValue::ESeverity::Critical,
															0.0, 1.0, FKPIValue::GreaterThanOrEqual, FKPIValue::GigaBytes);
		UserDirFreeSpaceKPI						= KPIRegistry.DeclareKPIValue(UserDirFreeSpaceKPIValue);

		const FText FreeSpaceHint = LOCTEXT("FreeSpaceHintMessage", "{0} has limited free space.\n\nTo avoid potential problems, please consider freeing up at least {1} GB.");
		KPIRegistry.DeclareKPIHint(EngineDirFreeSpaceKPI, FText::Format(FreeSpaceHint, FText::FromString(FPaths::ConvertRelativePathToFull(FPlatformProcess::BaseDir())), EngineDirFreeSpaceKPIValue.ThresholdValue.Get(5.0)));
		KPIRegistry.DeclareKPIHint(ProjectDirFreeSpaceKPI, FText::Format(FreeSpaceHint, FText::FromString(FPaths::ConvertRelativePathToFull(FPlatformMisc::ProjectDir())), ProjectDirFreeSpaceKPIValue.ThresholdValue.Get(5.0)));
		KPIRegistry.DeclareKPIHint(UserDirFreeSpaceKPI, FText::Format(FreeSpaceHint, FText::FromString(FPaths::ConvertRelativePathToFull(FPlatformProcess::UserDir())), UserDirFreeSpaceKPIValue.ThresholdValue.Get(1.0)));
	}

	// Declare the KPI Hints
	KPIRegistry.DeclareKPIHint(EditorBootKPI, LOCTEXT("EditorBootHintMessage", "The Editor boot time is slow.\nCheck you have enabled a Game Feature Plugin profile for your project and that the expected local cache efficiency is met.\nIf you are booting the Editor in the background then disable the Use Less CPU in Background option in the settings."));
	KPIRegistry.DeclareKPIHint(TotalTimeToEditorKPI, LOCTEXT("EditorStartupHintMessage", "The Editor start-up time is slow.\nCheck you have enabled a Game Feature Plugin profile for your project and that the expected local cache efficiency is met.\nIf you are booting the Editor in the background then disable the Use Less CPU in Background option in the settings."));
	KPIRegistry.DeclareKPIHint(EditorPluginCountKPI, LOCTEXT("EditorPluginHintMessage", "The Editor is loading more plugins than expected and this will affect Editor start-up performance.\nCheck you have enabled a Game Feature Plugin profile for your project."));

	KPIRegistry.DeclareKPIHint(PIETransitionKPI, LOCTEXT("PIETransitionHintMessage", "The Editor transition to PIE is slow.\nCheck that the expected local cache efficiency is met.\nIf you are transitioning to PIE with the Editor in the background then disable the Use Less CPU in Background option in the settings."));
	
	KPIRegistry.DeclareKPIHint(LocalDDCEfficiencyKPI, LOCTEXT("LocalCacheEfficiencyHintMessage", "The Editor will not perform well if the local cache efficiency has not yet met the expected value.\nIf this is the first time you have booted the Editor after a sync then this is to be expected."), TEXT("using-derived-data-cache-in-unreal-engine"));
	
	KPIRegistry.DeclareKPIHint(AvailableMemoryKPI, LOCTEXT("LowAvailableMemoryHintMessage", "Your hardware is running low on memory.\nTry closing applications that are no longer needed to recover available memory."));
	KPIRegistry.DeclareKPIHint(MemoryPressureKPI, LOCTEXT("MemoryPressureHintMessage", "Your system is running low on memory.\nTry closing applications that are no longer needed to recover available memory."));

	KPIRegistry.DeclareKPIHint(DynamicInvalidationKPI, LOCTEXT("DynamicInvalidationHintMessage", "Some Dynamic Invalidation Panels contain widgets that do not support invalidation."));
...
void FEditorPerformanceModule::UpdateKPIsHighFrequency()
{
	const UEditorPerformanceSettings* EditorPerformanceSettings = GetDefault<UEditorPerformanceSettings>();
...
#if WITH_SLATE_DEBUGGING && UE_SLATE_WITH_DYNAMIC_INVALIDATION
	{
		static IConsoleVariable* DynamicInvalidationOptionsVar = IConsoleManager::Get().FindConsoleVariable(TEXT("Slate.DynamicInvalidation.Options"));
		ensure(DynamicInvalidationOptionsVar);
		const int32 DynamicInvalidationOptions = DynamicInvalidationOptionsVar ? DynamicInvalidationOptionsVar->GetInt() : 0;

		const int NoPanels = FSlateDebugging::GetNoInvalidationPanels();
		const int NoDynPanels = FSlateDebugging::GetNoDynamicInvalidationPanels();
		const int NoDynPanelsSupportingInvalidation = FSlateDebugging::GetNoDynamicInvalidationPanelsSupportingInvalidation();

		if (DynamicInvalidationOptions == 0) // Dynamic Invalidation Disabled
		{
			KPIRegistry.InvalidateKPIValue(DynamicInvalidationKPI);
		}
		else if (DynamicInvalidationOptions == 1 && NoPanels > 0) // Dynamic Invalidation Panels acting like Invalidation Panels
		{
			KPIRegistry.SetKPIValue(DynamicInvalidationKPI, 1.0); // all of them are invalidating
		}
		else if (DynamicInvalidationOptions > 1 && NoDynPanels > 0) // Dynamic Invalidation enabled or forced
		{
			KPIRegistry.SetKPIValue(DynamicInvalidationKPI, (float)NoDynPanelsSupportingInvalidation / (float)NoDynPanels);
		}
		else
		{
			KPIRegistry.InvalidateKPIValue(DynamicInvalidationKPI);
	...
	static TArray<FGuid, TInlineAllocator<4>> RecordedKPIEvents;
	
	const TArray<FGuid, TInlineAllocator<4>> RecordedKPIEventsBefore = RecordedKPIEvents;
	...
	for (FKPIValues::TConstIterator It(GetKPIRegistry().GetKPIValues()); It; ++It)
	{
		...
		{
			if (RecordedKPIEvents.Find(KPIValue.Id) == INDEX_NONE)
			{
				...
				if (EditorPerformanceSettings)
				{
					if (EditorPerformanceSettings->bEnableSnapshots)
					{
						...
						RecordInsightsSnapshot(KPIValue);
					}
				...
				RecordedKPIEvents.Emplace(KPIValue.Id);
			}
		}
		else
		{
			// No longer exceeding threshold, so next time this KPI is exceeded we will record the event
			RecordedKPIEvents.Remove(KPIValue.Id);
		}
	}

	// Check TEDS diagnostic columns separately so that additions and removals both trigger a broadcast.
	bool bTedsColumnsChanged;
	{
		...
		static TSet<FGuid> PreviousTedsColumnIds;
		TSet<FGuid> CurrentTedsColumnIds;

		if (Factory)
		{
			DataStorage->RunQuery(Factory->GetDiagnosticCriticalQueryHandle(), CreateDirectQueryCallbackBinding(
				[&CurrentTedsColumnIds](IDirectQueryContext& Context, const FEditorPerformanceCriticalColumn& Column)
				{
					CurrentTedsColumnIds.Add(Column.Id);
				}));

			DataStorage->RunQuery(Factory->GetDiagnosticWarningQueryHandle(), CreateDirectQueryCallbackBinding(
				[&CurrentTedsColumnIds](IDirectQueryContext& Context, const FEditorPerformanceWarningColumn& Column)
				{
					CurrentTedsColumnIds.Add(Column.Id);
				}));
		}

		bTedsColumnsChanged = PreviousTedsColumnIds.Num() != CurrentTedsColumnIds.Num()
			|| CurrentTedsColumnIds.Difference(PreviousTedsColumnIds).Num() > 0;

		PreviousTedsColumnIds = MoveTemp(CurrentTedsColumnIds);
	}

	if (RecordedKPIEventsBefore != RecordedKPIEvents || bTedsColumnsChanged)
	{
		OnPerformanceStateChanged.Broadcast();
	}
}

void FEditorPerformanceModule::UpdateKPIsLowFrequency()
{
	const UEditorPerformanceSettings* EditorPerformanceSettings = GetDefault<UEditorPerformanceSettings>();
	...
	// Gather free disk space
	{
		...
		{
			KPIRegistry.SetKPIValue(EngineDirFreeSpaceKPI, static_cast<float>(FreeDiskSpace) / (1024.f * 1024.f * 1024.f));
		}

		if (FPlatformMisc::GetDiskTotalAndFreeSpace(FPlatformMisc::ProjectDir(), TotalDiskSpace, FreeDiskSpace))
		{
			KPIRegistry.SetKPIValue(ProjectDirFreeSpaceKPI, static_cast<float>(FreeDiskSpace) / (1024.f * 1024.f * 1024.f));
		}

		if (FPlatformMisc::GetDiskTotalAndFreeSpace(FPlatformProcess::UserDir(), TotalDiskSpace, FreeDiskSpace))
		{
			KPIRegistry.SetKPIValue(UserDirFreeSpaceKPI, static_cast<float>(FreeDiskSpace) / (1024.f * 1024.f * 1024.f));
		}
	}
...
FText FEditorPerformanceModule::GetDynamicInvalidationDisplayValue(const float InValue) const
{
	...
	if (DynamicInvalidationOptions == 0) // Dynamic Invalidation Disabled
	{
		return LOCTEXT("DynamicInvalidationDisabled", "Disabled");
	}

	const FText Percent = InValue < 0 ? FText::FromString("...") : FText::AsPercent(InValue);
	if (DynamicInvalidationOptions == 1) // Dynamic Invalidation Panels acting like regular invalidation panels
	{
		return FText::Format(LOCTEXT("DynamicInvalidationForcedStatic", "{0} (Forced Static)"), Percent);
	}
	else if (DynamicInvalidationOptions == 2) // Dynamic Invalidation enabled
	{
		return Percent;
	}
	else if (DynamicInvalidationOptions == 3) // Invalidation Panels acting like regular Dynamic Invalidation Panels
	{
		return FText::Format(LOCTEXT("DynamicInvalidationForcedDynamic", "{0} (Forced Dynamic)"), Percent);
	}

	return LOCTEXT("DynamicInvalidationUnknown", "Unknown");
#else
	return LOCTEXT("DynamicInvalidationDisabled", "Disabled");
#endif
}
...
bool FEditorPerformanceModule::RecordInsightsSnapshot(const FKPIValue& KPIValue)
{
