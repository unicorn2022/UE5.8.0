...
public:
	...
	UE_API bool RecordInsightsSnapshot(const FKPIValue& Value);
	...
	FPerformanceStateChangedEvent& GetOnPerformanceStateChanged() { return OnPerformanceStateChanged; }
...
private:
	...
	FTimerHandle					HighFrequencyUpdateTimerHandle;
	const float						HighFrequencyUpdateIntervalSeconds = 1.0f;
	FTimerHandle					LowFrequencyUpdateTimerHandle;
	const float						LowFrequencyUpdateIntervalSeconds = 60.0f;
	...
	FGuid							DynamicInvalidationKPI;

	FGuid							EngineDirFreeSpaceKPI;
	FGuid							ProjectDirFreeSpaceKPI;
	FGuid							UserDirFreeSpaceKPI;
};
