...
void FVehicleState::CaptureState(const Chaos::FRigidBodyHandle_Internal* Handle, float GravityZ, float DeltaTime)
{
	if (Handle)
	{
		...
		{
			UE_LOGF(LogVehicle, Log, "Vehicle state last velocity = %ls | World Transform = %ls | World Velocity = %ls",
...
void UChaosVehicleSimulation::TickVehicle(UWorld* WorldIn, float DeltaTime, const FChaosVehicleAsyncInput& InputData, FChaosVehicleAsyncOutput& OutputData, Chaos::FRigidBodyHandle_Internal* Handle)
{
	...
	if (World && RigidHandle)
	{
		...
		{
			UE_LOGF(LogTemp, Log, "SERVER | PT | TickVehicle | Async tick vehicle with inputs at frame %d : Throttle = %f Brake = %f Roll = %f Pitch = %f Yaw = %f Steering = %f Handbrake = %f Sleeping = %d",
		...
		{
			UE_LOGF(LogTemp, Log, "CLIENT | PT | TickVehicle | Async tick vehicle with inputs at frame %d : Throttle = %f Brake = %f Roll = %f Pitch = %f Yaw = %f Steering = %f Handbrake = %f Sleeping = %d",
				InputData.PhysicsInputs.NetworkInputs.LocalFrame, VehicleInputs.ThrottleInput, VehicleInputs.BrakeInput, VehicleInputs.RollInput, VehicleInputs.PitchInput,
				VehicleInputs.YawInput, VehicleInputs.SteeringInput, VehicleInputs.HandbrakeInput, VehicleState.bSleeping);
		}
#endif
...
void UChaosVehicleSimulation::ApplyTorqueControl(float DeltaTime, const FChaosVehicleAsyncInput& InputData)
{
	...
	if (!GVehicleDebugParams.DisableTorqueControl && RigidHandle)
	{
		...
		if (PVehicle->HasTorqueControlSetup() && PVehicle->GetTorqueControl().Setup().Enabled)
		{
			...
			{
				if ((PVehicle->Wheels.Num() > 0) && PVehicle->Wheels[0].InContact())
				{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
void UChaosVehicleMovementComponent::MigratePhysicsState(FArchive& Ar)
{
	...
	{
		FPhysicsVehicleOutput::StaticStruct()->SerializeItem(Ar, PVehicleOutput.Get(), nullptr);
	}

	if (Ar.IsLoading())
	{
		bIsVehicleStatesMigrated = true;
		MigratedVehicleStates.NetSerialize(Ar, nullptr, bIsVehicleStatesMigrated);
	}
	else if (Ar.IsSaving())
	{
		...
		if (CanSendMigratedPhysicsState(MigrationContext, ObjMigrationSendType))
		{
			...
			if (bIsVehicleStatesMigrated)
			{
				UE_LOGF(LogVehicle, Verbose, "[%hs] Migrated a pending vehicle state", __func__);
				MigratedVehicleStates.NetSerialize(Ar, nullptr, bOutSuccess);
			}
			else
			{
				FNetworkVehicleStates VehicleStates;
				// BuildData accesses VehicleSimulationPT, which contains Physics Thread data.
				// While FPhysicsCommand is not inherently thread-safe due to PT ownership, 
				// this is currently safe because Dstm enforces synchronous physics execution.
				if (FPhysScene* PhysScene = GetWorld() ? GetWorld()->GetPhysicsScene() : nullptr)
				{
					FPhysicsCommand::ExecuteWrite(PhysScene, [this, &VehicleStates]()
						{
							VehicleStates.BuildData(this);
						});
				}
				else
				{
					UE_LOGF(LogVehicle, Warning, "[%hs] Cannot BuildData - no valid physics scene", __func__);
				}
				VehicleStates.NetSerialize(Ar, nullptr, bOutSuccess);
			}
		}
		else
		{
			UE_LOGF(LogVehicle, Log, "[%hs] Migrated an empty vehicle physics state", __func__);
			FNetworkVehicleStates VehicleStates;
			bool bOutSuccess = true;
			VehicleStates.NetSerialize(Ar, nullptr, bOutSuccess);
		}
...
bool UChaosVehicleMovementComponent::CanSendMigratedPhysicsState(const FUObjectMigrationContext* MigrationContext, EObjectMigrationSendType ObjMigrationSendType) const
{
...
void UChaosVehicleMovementComponent::PostMigrate(const struct FUObjectMigrationContext& MigrationContext)
{
	Super::PostMigrate(MigrationContext);
}

void UChaosVehicleMovementComponent::ApplyMigratedVehicleState()
{
	if (bIsVehicleStatesMigrated)
	{
		MigratedVehicleStates.ApplyData(this);
		bIsVehicleStatesMigrated = false;
	}
}
#endif 
...
bool UChaosVehicleMovementComponent::CanCreateVehicle() const
{
	...
	{
		UE_LOGF(LogVehicle, Warning, "Can't create vehicle %ls (%ls). UpdatedComponent is not set.", *ActorName, *GetPathName());
		return false;
	}

	if (UpdatedPrimitive == NULL)
	{
		UE_LOGF(LogVehicle, Warning, "Can't create vehicle %ls (%ls). UpdatedComponent is not a PrimitiveComponent.", *ActorName, *GetPathName());
		return false;
	}
...
{
	if (GearNum != TargetGear)
	{
		if (bUsingNetworkPhysicsPrediction)
		{
			// With network physics prediction, the gear is set via the input data path
			// (Update -> ProcessInputs_Internal) to stay consistent with the input history.
			// Direct ExecuteWrite would conflict with ApplyData during resimulation.
		}
		else
		{
			// Without network physics prediction, set the gear directly on the PT.
...
void UChaosVehicleMovementComponent::UpdateState(float DeltaTime)
{
	...
	{
		// For automatic transmission with network physics prediction, the physics simulation
		// (TransmissionSystem::Simulate) owns gear changes. Sync the GT's TargetGear from the
		// physics output so FillAsyncInput doesn't send a stale value that overrides the
		// simulation's automatic upshift/downshift decisions each frame.
		// See UE-374761: gear stuck at N because GT kept forcing TargetGear=1 via ProcessInputs_Internal.
		if (bUsingNetworkPhysicsPrediction && TransmissionType == Chaos::ETransmissionType::Automatic)
		{
			TargetGear = PVehicleOutput->TargetGear;
		}

		if (bReverseAsBrake)
		{
			// Automatically shift between reverse and first gear based on input.
			// Throttle takes priority when both inputs are pressed to avoid gear oscillation.
			if (RawThrottleInput > KINDA_SMALL_NUMBER && GetTargetGear() <= 0)
			{
				SetTargetGear(1, true);
			}
			else if (RawBrakeInput > KINDA_SMALL_NUMBER && RawThrottleInput <= KINDA_SMALL_NUMBER && GetTargetGear() >= 0)
			{
...
#if UE_WITH_REMOTE_OBJECT_HANDLE
		// Skip inertia scaling when vehicle is migrated since its scaled inertia is already migrated. 
		if (!bIsMigrated)
#endif
		{
...
{
#if UE_WITH_REMOTE_OBJECT_HANDLE
	// Don't reinitialize PVehicleOutput when migrated because its data is already migrated over. 
	if (!bIsMigrated)
#endif
	{
	...
	if (CurAsyncInput)
	{
		if (const FBodyInstance* BodyInstance = GetBodyInstance())
		{
			if (auto Handle = BodyInstance->GetPhysicsActor())
			{
				...
				// Pass the GT's gear decision through the input data path so it flows into
				// the network physics input history via BuildData/ApplyData consistently
				AsyncInput->PhysicsInputs.NetworkInputs.TransmissionTargetGear = TargetGear;
				AsyncInput->PhysicsInputs.NetworkInputs.TransmissionCurrentGear = TargetGear;
