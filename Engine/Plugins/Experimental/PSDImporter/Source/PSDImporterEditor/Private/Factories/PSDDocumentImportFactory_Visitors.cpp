...
void FPSDDocumentImportFactory_Visitors::OnImportComplete()
{
	...
	auto DeleteLayerTexture = [this, EditorAssetSubsystem](const FPSDFileLayer& InLayer)
	{
		...
		{
			UE_LOGF(LogPSDImporterEditor, Error, "Error deleting texture asset for layer.");
		}
	};
			...
			&& !InLayerImage.RawData.IsEmpty())
		{
			// Texture Asset
			{
					...
					&& !HasMergedParent(Layer))
				{
					...
					if (!LayerTexture)
					{
						UE_LOGF(LogPSDImporterEditor, Error, "Error creating texture for layer");
					}
					else
					{
					...
					if (InReadMaskData)
					{
						...
						if (InMaskImage.GetWidth() > 0 && InMaskImage.GetHeight() > 0)
						{
							...
							if (!MaskTexture)
							{
								UE_LOGF(LogPSDImporterEditor, Error, "Error creating mask texture for layer");
							}
							else
							{
