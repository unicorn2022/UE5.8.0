...
namespace UE::PSDImporter::Private
{
	...
	public:
		...
		bool Open()
		{
			...
			if (!Document)
			{
				UE_LOGF(LogPSDImporterCore, Error, "Cannot open the document for file '%ls'", *FileName);
				Close();
				return false;
			}
			...
			{
				UE_LOGF(LogPSDImporterCore, Warning, "PSD import only supports RGB as a color mode.")
				Close();
				return false;
			}
