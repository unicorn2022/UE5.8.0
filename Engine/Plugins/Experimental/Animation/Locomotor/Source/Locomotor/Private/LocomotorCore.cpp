...
void FLocomotor::UpdateWorldSpeedAndPhase()
{
	...
	// DEBUG OUTPUT
	//UE_LOGF(LogTemp, Warning, "Distance to Goal  : %f", DistanceToGoal);
	//UE_LOGF(LogTemp, Warning, "Stopping Distance : %f", CurrentStoppingDistance);
	//UE_LOGF(LogTemp, Warning, "Current speed     : %f", CurrentSpeed);
	//UE_LOGF(LogTemp, Warning, "Phase   speed     : %f", CurrentPhaseSpeed);
	//UE_LOGF(LogTemp, Warning, "Delta time        : %f", Settings.DeltaTime);
	//UE_LOGF(LogTemp, Warning, "PercentOfMaxSpeed : %f", CurrentPercentOfMaxSpeed);
	//UE_LOGF(LogTemp, Warning, "Current Stride    : %f", CurrentStrideLength);
	//UE_LOGF(LogTemp, Warning, "Accelerating      : %ls", bAccelerating ? TEXT("Accelerating") : TEXT("Decelerating"));
}
...
void FLocomotor::UpdateFeetTargets()
{
	...
	if (bFullyAtRest && bAnyFeetWantToStep)
	{
		...
		for (FLocomotorFoot* FootPtr : AllFeet)
		{
			...
			//UE_LOGF(LogTemp, Warning, "Force updating feet phases: %f", Foot.CurrentPhase);
		}
	}
	...
	for (FLocomotorFoot* FootPtr : AllFeet)
	{
		...
		else
		{
			...
			//UE_LOGF(LogTemp, Warning, "Updating foot phase Current/Target: %f, %f", Foot.CurrentPhase, Foot.TargetPhase);
		}
...
void FLocomotor::AnimateFeet()
{
	for (FLocomotorFoot* FootPtr : AllFeet)
	{
		...
		if (Foot.bInSwingPhase)
		{
			...
			constexpr float PeelDurationAsPercentOfSwing = 0.2f;
	...
	//UE_LOGF(LogTemp, Warning, "CurrentPhase:            %f", Foot.CurrentPhase);
	//UE_LOGF(LogTemp, Warning, "PercentSwing:            %f", PercentSwing);
	//UE_LOGF(LogTemp, Warning, "PercentSwingBasedOnFoot: %f", PercentSwingBasedOnFoot);
	//UE_LOGF(LogTemp, Warning, "PhaseWhenSwingEnds:      %f", Foot.FootPhaseWhenSwingEnds);
	//UE_LOGF(LogTemp, Warning, "InSwingPhase:            %ls", Foot.bInSwingPhase ? TEXT("True") : TEXT("False"));
}
