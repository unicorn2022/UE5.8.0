...
{
	// New functions should only be appended to this list. The function index is serialized.
	FBuiltinFunctions()
	{
		// clamp(value, min, max)
		...
		CE_EXPR(remap, 5, FMath::GetMappedRangeValueClamped(FFloatRange(V[1], V[2]), FFloatRange(V[3], V[4]), V[0]) )
	}
...
void FExpressionObject::Load(FArchive& Ar)
{
	...
	for (int32 OperandIndex = 0; OperandIndex < OperandCount; OperandIndex++)
	{
		...
		case OpElement::IndexOfType<float>():
			...
			break;
		default:
			UE_LOGF(LogCurveExpression, Error, "Invalid element type found in serialized expression");
			Ar.SetError();
			return;
		}
	}
