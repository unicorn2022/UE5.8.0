...
bool FRigMapperDefinitionsSingleton::GetRigMapper(const TObjectPtr<const URigMapperDefinition>& InDefinition, FacialRigMapping::FRigMapper& OutRigMapper)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogRigMapper, Error, "Could not load definition (%ls)", *InDefinition->GetPathName())
			return false;
		}
		if (!InDefinition->WasDefinitionValidated())
		{
			UE_LOGF(LogRigMapper, Warning, "RigMapper definition asset (%ls) was not validated please ensure that all RigMapper node definitions have been set to validated assets.", *InDefinition->GetPathName())
		}
...
FRigMapperProcessor::FRigMapperProcessor(const TArray<URigMapperDefinition*>& InDefinitions)
{
	for (const URigMapperDefinition* Definition : InDefinitions)
	{
		if (Definition)
		{
			...
			{
				UE_LOGF(LogRigMapper, Error, "Could not load definition %ls", *Definition->GetPathName())
					RigMappers.Empty();
				return;
			}
