...
void URelativeBodyAnimModifier::OnApply_Implementation(UAnimSequence* InAnimation)
{
	...
	{
		UE_LOGF(LogAnimation, Error, "Cannot create RelativeBodyAnimNotify. Reason: Must specify valid relative body notify subclass to create");
		return;
	}
	...
	if (!PhysicsAsset)
	{
		UE_LOGF(LogAnimation, Error, "Cannot create RelativeBodyAnimNotify. Reason: Must specify valid physics asset");
		return;
	}
	...
	// Process animation asset
	{
		...
		for (int SampleIndex = 0; SampleIndex < SampleNum; ++SampleIndex)
		{
			...
			TArray<TUniquePtr<UE::Geometry::TPointHashGrid3f<int32>>> BodiesVertHash;
			BodiesVertHash.SetNum(NumBodies);
			size_t count = 0;
			for (int32 BodyIndex = 0; BodyIndex < NumBodies; ++BodyIndex)
			{
				BodiesVertHash[BodyIndex] = MakeUnique<UE::Geometry::TPointHashGrid3f<int32>>(HashGridCellSize, INDEX_NONE);
			...
			for (int32 BodyIndex1 = 0; BodyIndex1 < NumBodies; ++BodyIndex1)
			{
				...
				for (int32 BodyIndex2 = BodyIndex1 + 1; BodyIndex2 < NumBodies; ++BodyIndex2)
				{
					...
					if (NearestQueryVertexIndex != INDEX_NONE)
					{
						...
						{
							FTransform3f SourceBodyRot1(FTransform(BakedAnimNotifyInfo->OffsetTransformsForBones[BoneName1].GetRotation()));
							FTransform3f SourceBodyRot2(FTransform(BakedAnimNotifyInfo->OffsetTransformsForBones[BoneName2].GetRotation()));
							
							size_t offset = SampleIndex*BakedAnimNotifyInfo->BodyPairs.Num();
							BakedAnimNotifyInfo->BodyPairsLocalReference[offset+count*2+0] = SourceBodyRot1.TransformPosition(Loc1);
							BakedAnimNotifyInfo->BodyPairsLocalReference[offset+count*2+1] = SourceBodyRot2.TransformPosition(Loc2);
						}
					}
					count++;
				}
...
void URelativeBodyAnimModifier::CacheBodyDataForSourceMesh(FRelativeBodySourceData& OutSourceData)
{
	...
	for (int32 VertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex)
	{
		...
		for (int32 InfluenceIndex = 0; InfluenceIndex < InfluenceCount; ++InfluenceIndex)
		{
			...
			UE_LOGF(LogAnimation, Log, "Source %d: Bone '%ls'[%f].", VertexIndex, *InfluenceBoneName.ToString(), InfluenceBoneWeight);
	...
	for (int32 idx = 0; idx < NumBodies; ++idx)
	{
		...
		UE_LOGF(LogAnimation, Log, "%d: Bone '%ls' [%d verts].", idx, *BoneName.ToString(), OutSourceData.SourceVertexIndicesInfluencedByBodyIndices[idx].Num());
	}
