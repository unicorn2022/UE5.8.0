...
struct FContextualAnimSceneBinding
{
	...
	inline int8 GetSectionIdx() const { return SectionIdx; }
	inline int8 GetAnimSetIdx() const { return AnimSetIdx; }
	inline int8 GetAnimTrackIdx() const { return AnimTrackIdx; }

	UE_API FTransform GetIKTargetTransform(const FName& TrackName, float Time) const;
	UE_API FTransform GetAlignmentTransform(const FName& TrackName, float Time) const;
	...
	UAnimSequenceBase* GetAnimation() const { return Animation; }

	FContextualAnimAlignmentTrackContainer& GetAlignmentData() { return AlignmentData; }
	const FContextualAnimAlignmentTrackContainer& GetAlignmentData() const { return AlignmentData; }

	FContextualAnimAlignmentTrackContainer& GetIKTargetData() { return IKTargetData; }
	const FContextualAnimAlignmentTrackContainer& GetIKTargetData() const { return IKTargetData; }

	const FTransform& GetMeshToScene() const { return MeshToScene; }
	FTransform& GetMeshToScene() { return MeshToScene; }

	void SetOverrideData(const FContextualAnimationOverrideData& InData);
...
private:
	...
	UPROPERTY()
	int8 SectionIdx = INDEX_NONE;

	UPROPERTY()
	int8 AnimSetIdx = INDEX_NONE;

	UPROPERTY()
	int8 AnimTrackIdx = INDEX_NONE;
	...
	friend FContextualAnimSceneBinding;
};
...
struct FContextualAnimSceneBindings
{
	...
	UE_DEPRECATED(5.7, "This constructor is deprecated. Please use the version with the other signature")
	UE_API FContextualAnimSceneBindings(const UContextualAnimSceneAsset& InSceneAsset, int32 InSectionIdx, int32 InAnimSetIdx);

	UE_API FContextualAnimSceneBindings(const UContextualAnimSceneAsset& InSceneAsset);
	...
	FContextualAnimSceneBinding* FindBindingByActor(const AActor* Actor)
	{
		return Actor ? Data.FindByPredicate([Actor](const FContextualAnimSceneBinding& Item) { return Item.GetActor() == Actor; }) : nullptr;
	}
	...
	UE_DEPRECATED(5.7, "This getter and its corresponding property is deprecated. SectionIdx is now stored per Binding")
	inline int32 GetSectionIdx() const { return SectionIdx_DEPRECATED; }

	UE_DEPRECATED(5.7, "This getter and its corresponding property is deprecated. AnimSetIdx is now stored per Binding")
	inline int32 GetAnimSetIdx() const { return AnimSetIdx_DEPRECATED; }
	...
	UE_API bool BindActorToRole(AActor& ActorRef, FName Role, int32 SectionIdx, int32 AnimSetIdx);
	...
	UE_API void CalculateWarpPoints(TArray<FContextualAnimWarpPoint>& OutWarpPoints, int32 SectionIdx) const;
	...
	UE_DEPRECATED(5.7, "This function is deprecated. Please use ShouldBindingSyncAnimation instead")
	UE_API bool ShouldSyncAnimation() const;

	UE_API bool ShouldBindingSyncAnimation(const FContextualAnimSceneBinding& Binding) const;
	...
	UE_API bool AreAllBindingsInTheSameSection() const;

	UE_API void SetOverrideProvider(const TScriptInterface <IContextualAnimOverrideInterface>& Provider);
	
	const TScriptInterface <IContextualAnimOverrideInterface>& GetOverrideProvider() const { return OverrideProviderInterface;}

	/*
	* Updates Each Bindings Overrides, Including Animation, Alignment IK, ETC 
	* Returns True is there is a valid Scene and Override Provider
	*/
	UE_API bool UpdateBindingOverridesIfNeeded();
...
private:
	...
	// This property is deprecated and will be removed soon. AnimSetIdx is now stored per Binding
	UPROPERTY()
	int8 SectionIdx_DEPRECATED = INDEX_NONE;

	// This property is deprecated and will be removed soon. AnimSetIdx is now stored per Binding
	UPROPERTY()
	int8 AnimSetIdx_DEPRECATED = INDEX_NONE;
	...
	UPROPERTY()
	TScriptInterface<IContextualAnimOverrideInterface> OverrideProviderInterface = nullptr;
};
