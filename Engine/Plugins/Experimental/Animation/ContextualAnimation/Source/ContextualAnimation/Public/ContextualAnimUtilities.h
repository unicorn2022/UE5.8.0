...
class IContextualAnimOverrideInterface;
...
public:
	...
	static UE_API float GetSyncTimeForWarpSection(const UAnimSequenceBase* Animation, const FName& WarpSectionName);
	
	static UE_API void GetStartAndEndTimeForWarpSection(const UAnimSequenceBase* Animation, const FName& WarpSectionName, float& OutStartTime, float& OutEndTime);

	/** Returns the start/end time and warp target name of the Nth distinct warp section found sequentially in the animation's motion warping notifies */
	static UE_API FName GetStartAndEndTimeForWarpSection(const UAnimSequenceBase* Animation, int32 DesiredWarpSectionOrdinal, float& OutStartTime, float& OutEndTime);
	
	static UE_API void GenerateAlignmentTrack(const UContextualAnimSceneAsset& SceneAsset, const FContextualAnimSceneBinding& InBinding, const FContextualAnimSceneBindings& InBindings, FContextualAnimAlignmentTrackContainer& Container);
	static UE_API void GenerateIKTargetTrack(const UContextualAnimSceneAsset& SceneAsset, const FContextualAnimSceneBinding& InBinding, const FContextualAnimSceneBindings& InBindings, FContextualAnimAlignmentTrackContainer& Container);
	...
	UFUNCTION(BlueprintCallable, Category = "Contextual Anim|Scene Asset", meta = (DisplayName = "Add Override Provider to Bindings"))
	static UE_API void BP_AddContextualAnimationOverrideProvider(UPARAM(Ref)FContextualAnimSceneBindings& InBindings, UPARAM(Ref)TScriptInterface<IContextualAnimOverrideInterface>& OverrideProvider, FContextualAnimSceneBindings& OutBindings);
	...
	UE_DEPRECATED(5.7, "This function is deprecated. Please use the version that takes a Section Index")
	...
	UFUNCTION(BlueprintCallable, Category = "Contextual Anim|Scene Bindings", meta = (DisplayName = "Calculate Warp Points For Bindings"))
	static UE_API void BP_SceneBindings_CalculateWarpPointsForSectionAtIndex(const FContextualAnimSceneBindings& Bindings, int32 SectionIdx, TArray<FContextualAnimWarpPoint>& OutWarpPoints);
	...
	UFUNCTION(BlueprintCallable, Category = "Contextual Anim|Scene Bindings", meta = (DisplayName = "Add Or Update Warp Targets For Bindings"))
	static UE_API void BP_SceneBindings_AddOrUpdateWarpTargetsForSectionAtIndex(const FContextualAnimSceneBindings& Bindings, int32 SectionIdx);
	...
	UE_DEPRECATED(5.7, "This function is deprecated. Please use BP_SceneBinding_GetSectionAndAnimSetIndices instead")
	...
	UE_DEPRECATED(5.7, "This function is deprecated. Please use BP_SceneBindings_GetSectionAndAnimSetNamesFromBinding instead")
	...
	UFUNCTION(BlueprintPure, Category = "Contextual Anim|Scene Bindings", meta = (DisplayName = "Get Section And AnimSet Names From Binding"))
	static UE_API void BP_SceneBindings_GetSectionAndAnimSetNamesFromBinding(const FContextualAnimSceneBindings& Bindings, const FContextualAnimSceneBinding& Binding, FName& SectionName, FName& AnimSetName);
	...
	UFUNCTION(BlueprintPure, Category = "Contextual Anim|Scene Binding", meta = (DisplayName = "Get Section And AnimSet Indices"))
	static UE_API void BP_SceneBinding_GetSectionAndAnimSetIndices(const FContextualAnimSceneBinding& Binding, int32& SectionIdx, int32& AnimSetIdx);
	...
	static UE_API void GenerateAlignmentTracksForBindings(UContextualAnimSceneAsset& SceneAsset, FContextualAnimSceneBindings& InBindings);
	static UE_API void GenerateAlignmentTrackForBinding(UContextualAnimSceneAsset& SceneAsset, const FName& Role, FContextualAnimSceneBinding& Binding);
};
