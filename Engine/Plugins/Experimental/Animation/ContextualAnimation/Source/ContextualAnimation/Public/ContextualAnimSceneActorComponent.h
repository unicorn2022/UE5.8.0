...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FContextualAnimOnMontageReachedEndDelegate, class UContextualAnimSceneActorComponent*, SceneComponent, UAnimMontage* const, Montage, int32, LoopCounter);
...
public:
	...
	/** Delegate called when the currently playing montage reaches the end but does not blends out */
	UPROPERTY(BlueprintAssignable, Category = "Events")
	FContextualAnimOnMontageReachedEndDelegate OnMontageReachedEndDelegate;
...
protected:
	...
	struct FCharacterProperties
	{
		...
		TOptional<struct FCollisionResponseContainer> CollisionProfileResponses;
	};
	...
	UE_API void JoinScene(const FContextualAnimSceneBindings& InBindings, const TArray<FContextualAnimWarpPoint> WarpPoints, const TArray<FContextualAnimWarpTarget>& ExternalWarpTargets, float StartTime = 0.f);
	...
	/** 
	 * Finds ideal start time for the interaction based on the distance from the actor to the warp target in the animation and the actual distance in game. 
	 * Effectively skipping the portion of the 'approach-motion' the actor doesn't need.
     * Returns 0 if the actor is far enough to play the full animation from the beginning.
	 */
	float FindIdealStartTime(const FContextualAnimSceneBindings& InBindings, const FContextualAnimSceneBinding& Binding, const TArray<FContextualAnimWarpPoint>& WarpPoints) const;
