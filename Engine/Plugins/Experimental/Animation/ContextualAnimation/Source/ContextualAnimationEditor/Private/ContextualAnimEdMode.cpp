...
void FContextualAnimEdMode::Tick(FEditorViewportClient* ViewportClient, float DeltaTime)
{
	...
	if (ViewModel)
	{
		ViewModel->TickSimulateMode(DeltaTime);
	}
}
...
void FContextualAnimEdMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	if (ViewModel)
	{
		if (const UContextualAnimSceneAsset* SceneAsset = ViewModel->GetSceneAsset())
		{
			...
			// If any bound actor became invalid (e.g., Blueprint recompilation destroying the old instance),
			// refresh the preview scene to re-spawn actors from the current classes.
			...
			if (const FContextualAnimSceneSection* Section = SceneAsset->GetSection(PrimaryBinding->GetSectionIdx()))
			{
				// Draw warp points
				if (const FContextualAnimSet* AnimSet = Section->GetAnimSet(PrimaryBinding->GetAnimSetIdx()))
				{
					for (const FContextualAnimWarpPointDefinition& WarpPointDef : Section->GetWarpPointDefinitions())
					{
						...
						if (Bindings.CalculateWarpPoint(WarpPointDef, WarpPoint))
						{
							...
							for (const FContextualAnimSceneBinding& Binding : Bindings)
							{
								const UAnimSequenceBase* AnimationSequence = Binding.GetAnimation();
								if(AnimationSequence)
								{
									const float Time = UContextualAnimUtilities::GetSyncTimeForWarpSection(AnimationSequence, WarpPointDef.WarpTargetName);
									const FTransform TransformRelativeToWarpPoint = SceneAsset->GetAlignmentTransform(Binding.GetSectionIdx(), Binding.GetAnimSetIdx(), Binding.GetAnimTrackIdx(), WarpPointDef.WarpTargetName, Time);
				...
				if (PlaybackTime <= 0.f && !ViewModel->IsSimulateModePlaying())
				{
					...
					{
						const bool bIsActiveSet = AnimSetIdx == PrimaryBinding->GetAnimSetIdx();
						...
						for (const FContextualAnimTrack& AnimTrack : Section->GetAnimSet(AnimSetIdx)->Tracks)
						{
							...
							{
								const UAnimSequenceBase* TrackAnimationSequence = Binding->GetAnimation();
								if (AnimTrack.Role != PrimaryRole && TrackAnimationSequence)
								{
									...
									UContextualAnimUtilities::DrawPose(PDI, TrackAnimationSequence, Time, Transform, bPassesSelectionCriteria ? FColor::Green : FColor::White, 1.f);
								}
							}
						}
					}
...
void FContextualAnimEdMode::DrawHUD(FEditorViewportClient* ViewportClient, FViewport* Viewport, const FSceneView* View, FCanvas* Canvas)
{
	...
	if (ViewModel)
	{
		const FText DebugText = ViewModel->GetSelectionDebugText();
		const UFont* Font = GEngine->GetSmallFont();
		const FVector2D TextPos(10.f, 10.f);
		const float Padding = 6.f;

		// Measure text size for the background
		int32 TextWidth = 0;
		int32 TextHeight = 0;
		StringSize(Font, TextWidth, TextHeight, *DebugText.ToString());

		// Draw semi-transparent black background
		const FVector2D BackgroundPos(TextPos.X - Padding, TextPos.Y - Padding);
		const FVector2D BackgroundSize(TextWidth + Padding * 2.f, TextHeight + Padding * 2.f);
		FCanvasTileItem BackgroundTile(BackgroundPos, BackgroundSize, FLinearColor(0.f, 0.f, 0.f, 0.5f));
		BackgroundTile.BlendMode = SE_BLEND_Translucent;
		Canvas->DrawItem(BackgroundTile);

		// Draw text
		FCanvasTextItem TextItem(TextPos, DebugText, Font, FLinearColor::White);
