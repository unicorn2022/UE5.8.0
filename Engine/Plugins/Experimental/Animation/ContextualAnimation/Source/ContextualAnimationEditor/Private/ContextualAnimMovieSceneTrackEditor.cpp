...
int32 FContextualAnimSection::OnPaintSection(FSequencerSectionPainter& InPainter) const
{
	...
	{
		const FContextualAnimViewModel& ViewModel = Section->GetOwnerTrack().GetViewModel();

		if (const FContextualAnimTrack* SelectedTrack = ViewModel.GetSelectedAnimTrack())
		{
			if (Section->GetSectionIdx() == SelectedTrack->SectionIdx &&
				Section->GetAnimSetIdx() == SelectedTrack->AnimSetIdx &&
				Section->GetAnimTrackIdx() == SelectedTrack->AnimTrackIdx)
			{
				...
				FSlateDrawElement::MakeBox(
					...
					FAppStyle::GetBrush(TEXT("Sequencer.Section.LockedBorder")),
					DrawEffects,
					FStyleColors::AccentGreen.GetColor(FWidgetStyle())
				);
				++LayerId;
			}
		}
