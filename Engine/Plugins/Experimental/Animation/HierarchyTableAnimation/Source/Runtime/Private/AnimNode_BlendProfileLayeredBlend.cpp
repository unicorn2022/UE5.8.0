...
void FAnimNode_BlendProfileLayeredBlend::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	// Blend poses
	{
		// Root space blending: zero the root bone rotation before converting to mesh space so that
		// child rotations accumulate relative to root's local frame rather than full mesh space.
		// The root bone's own rotation is saved, blended separately in local space, and restored after.
		FQuat BaseRootRot = FQuat::Identity;
		FQuat TargetRootRot = FQuat::Identity;
		const FCompactPoseBoneIndex RootBoneIndex(0);
		if (RotationBlendSpace == EBlendProfileRotationBlendSpace::RootSpace)
		{
			BaseRootRot = BasePoseData.GetPose()[RootBoneIndex].GetRotation();
			TargetRootRot = BlendPoseData.GetPose()[RootBoneIndex].GetRotation();
			BasePoseData.GetPose()[RootBoneIndex].SetRotation(FQuat::Identity);
			BlendPoseData.GetPose()[RootBoneIndex].SetRotation(FQuat::Identity);
		}

		if (RotationBlendSpace != EBlendProfileRotationBlendSpace::LocalSpace)
		{
		...
		if (RotationBlendSpace == EBlendProfileRotationBlendSpace::RootSpace)
		{
			const float RootBlendWeight = FMath::Clamp(CurrentBoneBlendWeights[0], 0.f, 1.f);
			FQuat BlendedRootRot;
			if (!FAnimWeight::IsRelevant(RootBlendWeight))
			{
				BlendedRootRot = BaseRootRot;
			}
			else if (FAnimWeight::IsFullWeight(RootBlendWeight))
			{
				BlendedRootRot = TargetRootRot;
			}
			else
			{
				BlendedRootRot = FQuat::FastLerp(BaseRootRot, TargetRootRot, RootBlendWeight);
				BlendedRootRot.Normalize();
			}
			Output.Pose[RootBoneIndex].SetRotation(BlendedRootRot);
		}
	}
