...
void UBlendProfileStandalone::BeginDestroy()
{
...
void UBlendProfileStandalone::HandleSkeletonHierarchyChanged()
{
	UpdateHierarchy();
	UpdateCachedData();
}
...
void UBlendProfileStandalone::SetSkeleton(TObjectPtr<USkeleton> InSkeleton)
{
	if (Skeleton)
	{
		Skeleton->UnregisterOnSkeletonHierarchyChanged(this);
	...
	if (ensure(InSkeleton))
	{
		...
		Skeleton->RegisterOnSkeletonHierarchyChanged(
			USkeleton::FOnSkeletonHierarchyChanged::CreateUObject(this, &UBlendProfileStandalone::HandleSkeletonHierarchyChanged));
	}
}
