...
public:
	...
	// When enabled, controller yaw rate is computed as rotation around the character's up vector instead of world Z.
	// Required for correct trajectory prediction on non-standard gravity surfaces (walls, ceilings, slopes).
	UPROPERTY(EditDefaultsOnly, Category = "Trajectory Settings")
	bool bEnableNonStandardGravity = false;

	float ControllerYawRate = 0.f;
	float ControllerYawRateClamped = 0.f;
	float DesiredControllerYawLastUpdate = 0.f;
	FQuat DesiredControllerRotationLastUpdate = FQuat::Identity;
