...
bool FTrajectoryExportAssetInfo::IsExportPathValid() const
{
	...
	static const FString PluginsDir = TEXT("/Plugins/");
	...
	if (ObjectPath.StartsWith(GameDir))
	{
		...
		const FString RelativeAssetFilename = FPaths::Combine(FPaths::ProjectContentDir(), ObjectPath.Replace(*GameDir, TEXT("")));
		const FString RelativeAssetPath = FPaths::GetPath(RelativeAssetFilename);
	...
	{
		FString ProjectDir = FPaths::ProjectDir();
		const FString RelativeAssetFilename = FPaths::Combine(FPaths::EngineContentDir(), ObjectPath.Replace(*EngineDir, TEXT("")));
		const FString RelativeAssetPath = FPaths::GetPath(RelativeAssetFilename);
...
{
	return IsExportPathValid() && !DoesAssetExistAtPath();
}
...
void UTrajectoryExportOperation::NotifyUserOfResults(const FTrajectoryExportContext& Context, FScopedSlowTask& Progress) const
{
	...
	if (Progress.ShouldCancel())
	{
		...
		TSharedPtr<SNotificationItem> NotificationWidget = FSlateNotificationManager::Get().AddNotification(Notification);
		NotificationWidget->SetCompletionState(SNotificationItem::CS_Fail);
	}
	else
	{
		...
		{
			UE_LOGF(LogTemp, Display, "Export trajectory - New Asset Created: %ls", *GeneratedAsset->GetName());
		}
		else
		{
			UE_LOGF(LogTemp, Display, "Export trajectory - Asset Modified: %ls", *AssetToProcess->GetName());
		}
		...
		Notification.Hyperlink = FSimpleDelegate::CreateLambda([Asset = GeneratedAsset]()
		{
			// Select all new assets and show them in the content browser.
			const FContentBrowserModule& ContentBrowserModule = FModuleManager::Get().LoadModuleChecked<FContentBrowserModule>("ContentBrowser");
			TArray<FAssetData> NewAssetsData = { Asset.Get() };
			ContentBrowserModule.Get().SyncBrowserToAssets(NewAssetsData);
		});

		Notification.HyperlinkText = LOCTEXT("OpenGeneratedAssetFromTrajectory", "Browse to asset");
		Notification.Text = FText::Format(
			LOCTEXT("CompletedExportingTrajectoryNotification", "{0}'s trajectory data was exported to {1} animation sequence."),
