...
FRewindDebuggerTrajectory::~FRewindDebuggerTrajectory()
{
	if (UToolMenus* Menus = UToolMenus::Get())
	{
		...
		{
			if (UToolMenu* Menu = Menus->FindMenu(RewindDebuggerModule->GetMainToolbarName()))
			{
				Menu->RemoveSection("TrajectoryWorkflows");
			}
			
			if (UToolMenu* PreviewMenu = Menus->FindMenu(RewindDebuggerModule->GetPreviewMenuName()))
			{
				PreviewMenu->RemoveSection("PreviewTrajectories");
			}
		}
	}
}
...
void FRewindDebuggerTrajectory::AnalysisSessionOpened(IRewindDebugger* RewindDebugger)
{
	// Clear up any cached state / variables.
	Reset();
}

void FRewindDebuggerTrajectory::AnalysisSessionClosed(IRewindDebugger* RewindDebugger)
{
...
{
	const IRewindDebuggerModule* RewindDebuggerModule = FModuleManager::GetModulePtr<IRewindDebuggerModule>("RewindDebugger");
	if (!ensure(RewindDebuggerModule))
	{
		return;
	}

	// Export button lives in the main toolbar's TrajectoryWorkflows section
	if (UToolMenu* Menu = UToolMenus::Get()->FindMenu(RewindDebuggerModule->GetMainToolbarName()))
	{
		FToolMenuSection& Section = Menu->FindOrAddSection("TrajectoryWorkflows", LOCTEXT("Trajectory Tools", "Trajectory Tools"));
		Section.AddSeparator("TrajectoryWorkflows");

		static const FName ExportButtonName = "ExportToolbarMenu";
		static const FText ExportLabel = LOCTEXT("ExportTrajectoriesLabel", "Export");
		static const FText ExportTooltip = LOCTEXT("ExportTrajectoriesTooltip", "Export recording/trajectory into a standalone asset");

		const FToolBarStyle& ToolBarStyle = FAppStyle::Get().GetWidgetStyle<FToolBarStyle>("SlimToolBar");

		TSharedRef<SWidget> CustomButton = SNew(SButton)
			.ButtonStyle(&ToolBarStyle.ButtonStyle)
			.ToolTipText(ExportTooltip)
			.IsEnabled_Lambda([]()
				{
					IRewindDebugger* RewindDebugger = IRewindDebugger::Instance();
					return RewindDebugger ? !RewindDebugger->IsPIESimulating() || !RewindDebugger->IsRecording() : false;
				})
			.OnClicked_Lambda([this]()
				{
					ShowBakeOutTrajectoryWindow();
			...
			[
				...
				[
					SNew(SImage)
					.Image(FAppStyle::GetBrush("Icons.Toolbar.Export"))
				]
		...
		Section.AddEntry(FToolMenuEntry::InitWidget(ExportButtonName, CustomButton, ExportLabel, false, true, false, ExportTooltip));
	}

	// Trajectories preview lives under Preview → Trajectories in the named preview menu
	{
		UToolMenu* PreviewMenu = UToolMenus::Get()->ExtendMenu(RewindDebuggerModule->GetPreviewMenuName());
		FToolMenuSection& Section = PreviewMenu->FindOrAddSection("PreviewTrajectories", LOCTEXT("OverlaysSectionLabel", "Overlays"));
		Section.AddSubMenu(
			"Trajectories",
			LOCTEXT("ToggleDebugDrawTrajectoriesLabel", "Trajectories"),
			LOCTEXT("ToggleDebugDrawTrajectoriesTooltip", "Toggle recorded trajectories to display in viewport"),
			FNewToolMenuDelegate::CreateRaw(this, &FRewindDebuggerTrajectory::MakeToggleDebugDrawMenu),
			false,
			FSlateIcon(),
			false
		);
...
{
	// Prepare trajectories to preview.
	...
	for (int i = 0; i < State.Trajectories.Num(); ++i)
	{
		FExtensionState::FDebugInfo& DebugInfoEntry = State.DebugInfos[i];
		...
		FUIAction UIAction;
		{
			...
			UIAction.CanExecuteAction = FCanExecuteAction::CreateLambda([this, i]()
				{
					const bool bHasAnyData = !State.Trajectories[i].Samples.IsEmpty() && !State.Trajectories[i].Poses.IsEmpty();
					return bHasAnyData;
				});
		}

		Section.AddMenuEntry(DebugInfoEntry.Name, FText::FromName(DebugInfoEntry.Name), FText::FromName(DebugInfoEntry.Name), FSlateIconFinder::FindIconForClass(USkeletalMeshComponent::StaticClass(), TEXT("SCS.Component")), UIAction, EUserInterfaceActionType::Check);
	}
}
...
void FRewindDebuggerTrajectory::DebugDraw(UCanvas* InCanvas, APlayerController* InController)
{
	...
	if (WorldToVisualize.IsValid() && !State.Trajectories.IsEmpty())
	{
			...
			if (State.DebugInfos[TrajectoryIndex].bShouldDraw)
			{
				...
				for (int SampleIndex = 0; SampleIndex < DebugTrajectoryPair.Samples.Num(); ++SampleIndex)
				{
					...
					if (SampleIndex + 1 < DebugTrajectoryPair.Samples.Num())
					{
						...
						DrawDebugLine(WorldToVisualize.Get(), Sample.Position, FutureSample.Position, DebugDrawColorForTrajectory);
					}

					// Draw sections at every second
					if (SampleIndex % 20 == 0)
					{
						DrawDebugPoint(WorldToVisualize.Get(), Sample.Position, 5, DebugDrawColorForTrajectory);
					}
				}
			}
		}
...
void FRewindDebuggerTrajectory::ShowBakeOutTrajectoryWindow()
{
	...
	.Title(LOCTEXT("BakeOutTrajectoryWindowTitle", "Export Recording / Trajectories"))
...
void FRewindDebuggerTrajectory::BuildTrajectoryOwnersList(const IRewindDebugger* InRewindDebugger, const IGameplayProvider* InGameplayProvider, double InTraceStartTime, double InTraceEndTime, TArray<FObjectInfo> & OutObjectInfos)
{
	...
	// Not a single actor with a skeletal mesh component was traced, safe to early out.
	if (SkelMeshCmpClassInfo == nullptr)
	{
		return;
	}
	...
	InGameplayProvider->EnumerateObjects(InTraceStartTime, InTraceEndTime, [&OutObjectInfos,InGameplayProvider, SkelMeshCmpClassInfo, InRewindDebugger](const FObjectInfo& InObjectInfo)
	{
		...
		if (!InObjectInfo.GetOuterId().IsSet())
		{
			...
			{
				const bool bIsSubObjectValid = InSubObjectId.IsSet();
				...
				if (bIsSubObjectValid && bIsSubObjectASkelMesh)
				{
		...
		{
			const bool bIsObjectValid = InObjectInfo.GetId().IsSet();
			...
			if (bIsObjectValid && bIsObjectASkelMesh)
			{
...
FName FRewindDebuggerTrajectory::GetFullNameForDebugInfoOwner(const IGameplayProvider* InGameplayProvider, uint64 InOwnerObjectId)
{
	...
	while(true)
	{
		...
		if (InGameplayProvider->IsSubClassOf(ObjectInfo.ClassId, ActorClassInfo->Id))
		{
			...
			if (WorldInfo)
			{
				Result.Append(GetDebugInfoOwnerWorldPostFix(*WorldInfo).ToString());
			}
			else
			{
				Result.Append(TEXTVIEW("Unknown"));
			}
			Result.Append(TEXTVIEW(")"));
...
// copied from FGameplayTrack::GetWorldPostFix()
FText FRewindDebuggerTrajectory::GetDebugInfoOwnerWorldPostFix(const FWorldInfo& InWorldInfo)
{
		...
		case FWorldInfo::EType::PIE:
			{
				...
				default: 
					return LOCTEXT("UnknownNetModePIEPostFix", "PIE");
				}
			}
