...
void SPreviewTrajectoryViewport::ResetPreviewSkeletalMesh() const
{
	...
	if (GameplayProvider && AnimationProvider)
	{
		...
		UpdatePreviewPoseFromScrubTime(0, RewindDebugger->CurrentTraceTime());
	}
}

void SPreviewTrajectoryViewport::UpdatePreviewPoseFromScrubTime(double InTraceStartTime, double InTraceEndTime) const
{
...
void FPreviewTrajectoryViewportClient::Tick(float DeltaTime)
{
	...
	if (PreviewEditorViewport.IsValid())
	{
		if (IRewindDebugger* RewindDebugger = IRewindDebugger::Instance())
		{
			...
			{
				PreviewEditorViewport.Pin().Get()->UpdatePreviewPoseFromScrubTime(0, RewindDebugger->CurrentTraceTime());
			}
		}
	}
...
void SExportTrajectoriesWindow::Construct(const FArguments& InArgs)
{
		...
		[
			...
			[
				...
				[
					...
					[
						...
						[
							...
							[
								SNew(SRichTextBlock)
								.Text(LOCTEXT("TrajectoryPickerLabel", "Source"))
								.TransformPolicy(ETextTransformPolicy::ToUpper)
								.DecoratorStyleSet(&FAppStyle::Get())
								.TextStyle(FAppStyle::Get(), "DetailsView.CategoryTextStyle")
								.ToolTipText(LOCTEXT("TrajectoryPickerTooltip", "Traced Skeletal Mesh Component to use as the export source for a sequence or trajectory."))
							]
				...
				[
					...
					.Visibility_Lambda([this]{ return GetExportValidation() != EExportValidation::Valid ? EVisibility::Visible : EVisibility::Collapsed; })
					.MessageStyle(EMessageStyle::Warning)
					.Message_Lambda([this] { return GetWarningMessage(); })
				]
				...
				[
					...
					[
						...
						{
							return GetExportValidation() == EExportValidation::Valid;
						})
	...
	// Now that everything is constructed properly, default to first trajectory as selected, if we have any.
	if (!Trajectories.IsEmpty())
	{
		// Select first valid trajectory
		...
		if (SelectedTrajectoryIndex != INDEX_NONE)
		{
			UpdateViewportAfterTrajectorySelection();
		}
	}
}
...
void SExportTrajectoriesWindow::UpdateViewportAfterTrajectorySelection()
{
...
TSharedRef<SWidget> SExportTrajectoriesWindow::OnGetTrajectoryPickerMenuContent()
{
	...
	for (int i = 0; i < TrajectoryOwnerNames.Num(); ++i)
	{
		...
		const FText ToolTip = bIsValidTrajectory ? FText::FromName(TrajectoryOwnerNames[i]) : LOCTEXT("InvalidTrajectoryTooltip", "Invalid. Trajectory data is empty or there is a mismatch between poses and samples.");
...
FText SExportTrajectoriesWindow::GetWarningMessage() const
{
	const EExportValidation ExportValidationState = GetExportValidation();
	
	switch (ExportValidationState)
	{
	case EExportValidation::MultipleTraceRanges: return LOCTEXT("MultipleTraceRangeLabel", "Multiple trace ranges");
	case EExportValidation::NoTrajectorySelected: return  LOCTEXT("NoTrajectorySelectedLabel", "No trajectory selected");
	case EExportValidation::MissingAssetInfo: return LOCTEXT("MissingAssetInfoLabel", "Missing asset information");
	case EExportValidation::ExportPathIsInvalid: return LOCTEXT("ExportPathIsInvalidLabel", "Invalid directory");
	case EExportValidation::AssetAlreadyExists: return LOCTEXT("AssetAlreadyExistsLabel", "Asset already exists");
	case EExportValidation::Valid:
	default:
		return FText::GetEmpty();
	}
}

SExportTrajectoriesWindow::EExportValidation SExportTrajectoriesWindow::GetExportValidation() const
{
	...
	if (PreviewData->OutputTrajectory.TraceInfo.Num() > 1)
	{
		...
		{
			return EExportValidation::MultipleTraceRanges;
		}
	}

	if (SelectedTrajectoryIndex == INDEX_NONE)
	{
		return EExportValidation::NoTrajectorySelected;
	}

	if (ExportDetails == nullptr || !ExportDetails->ExportAssetInfo.IsValid())
	{
		return EExportValidation::MissingAssetInfo;	
	}

	if (!ExportDetails->ExportAssetInfo.IsExportPathValid())
	{
		return EExportValidation::ExportPathIsInvalid;
	}

	if (!ExportDetails->ExportSettings.bShouldOverwriteExistingFiles && ExportDetails->ExportAssetInfo.DoesAssetExistAtPath())
	{
		return EExportValidation::AssetAlreadyExists;
	}
	
	return EExportValidation::Valid;
}
