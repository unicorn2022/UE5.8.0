...
void FUAFRigLogicTask::UpdateControlCurves(FRigInstance* InRigInstance,
	...
	const FCachedIndexedCurve& MLMaskCurves,
	...
	const UE::UAF::FLODPoseStack& InputPose) const
{
	...
	if (MLMaskCurves.Num() != 0)
	{
		TArrayView<float> MaskBuffer = InRigInstance->GetMLMaskValues();
		UE::Anim::FNamedValueArrayUtils::Union(Curves, MLMaskCurves,
