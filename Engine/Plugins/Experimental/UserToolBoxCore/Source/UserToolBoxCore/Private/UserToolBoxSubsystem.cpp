...
TArray<FAssetData> UUserToolboxSubsystem::GetAvailableTabList()
{
	...
	for (FAssetData AssetData:AssetDatas)
	{
		...
		if (Tab==nullptr)
		{
			UE_LOGF(LogTemp,Warning,"Unable to load %ls",*AssetData.PackageName.ToString())
			continue;
		}
		...
		if (IsValid(Asset))
		{
			...
			if (!Root.IsEmpty())
			{
				for (const FIconFolderInfo& IconFolderInfo:Asset->IconFolderInfos)
				{
					...
					{
						UE_LOGF(LogUserToolBoxCore,Error," directory doesn't exist: %ls",*DirPath);
						continue;
					}
