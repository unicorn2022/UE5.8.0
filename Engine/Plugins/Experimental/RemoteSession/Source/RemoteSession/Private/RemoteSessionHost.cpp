...
{	
	...
	for (const FRemoteSessionChannelInfo& Channel : SupportedChannels)
	{
		...
		UE_LOGF(LogRemoteSession, Log, "Offering channel %ls with mode %d", *Channel.Type, int(ClientMode));
	}
	...
	if (IsLegacyConnection())
	{
		UE_LOGF(LogRemoteSession, Log, "Pre-creating channels for legacy connection");
