...
FRemoteSessionClient::FRemoteSessionClient(const TCHAR* InHostAddress)
{
	...
	// Detect whether the address already includes a port
	bool bHasPort = false;
	if (HostAddress.StartsWith(TEXT("[")))
	{
		// IPv6 bracket notation — port follows the closing bracket, e.g. [::1]:2049
		int32 CloseBracket;
		if (HostAddress.FindChar(TEXT(']'), CloseBracket))
		{
			bHasPort = HostAddress.Mid(CloseBracket + 1).StartsWith(TEXT(":"));
		}
	}
	else
	{
		// A single colon means IPv4/hostname with port (e.g. "192.168.1.1:2049").
		// Multiple colons means bare IPv6 without port (e.g. "::1", "fd00::1").
		int32 FirstColon = HostAddress.Find(TEXT(":"), ESearchCase::IgnoreCase, ESearchDir::FromStart);
		int32 LastColon = HostAddress.Find(TEXT(":"), ESearchCase::IgnoreCase, ESearchDir::FromEnd);
		bHasPort = (FirstColon != INDEX_NONE && FirstColon == LastColon);
	}
	...
	if (!bHasPort)
	{
		...
		{
			// Bare IPv6 like "::1" -> "[::1]:2049"
			HostAddress = FString::Printf(TEXT("[%s]:%d"), *HostAddress, (int32)IRemoteSessionModule::kDefaultPort);
		}
		else
		{
			// IPv4 or hostname -> "host:2049"
			HostAddress += FString::Printf(TEXT(":%d"), (int32)IRemoteSessionModule::kDefaultPort);
		}
	}

	// OnReceiveHello will enable PixelStreaming if supported by both host and client
	bIsUsingPixelStreaming = false;

	UE_LOGF(LogRemoteSession, Display, "Will attempt to connect to %ls..", *HostAddress);
}
...
void FRemoteSessionClient::CheckConnection()
{
	...
	bool Success = Connection->WaitForConnection(0, [this](auto InConnection)
	{
		...
		UE_LOGF(LogRemoteSession, Log, "Connected to host at %ls", *HostAddress);
	...
	if (IsConnected() == false)
	{
		if (Success == false || TimeSpentConnecting >= ConnectionTimeout)
		{
			...
			UE_LOGF(LogRemoteSession, Log, "%ls", *Msg);
...
void FRemoteSessionClient::OnReceiveHello(IBackChannelPacket& Message)
{
	...
	// Negotiate: use PS if host supports PS2 and we have the PS2 module
	bool bLocalPSCapable = FModuleManager::Get().IsModuleLoaded("PixelStreaming2");
	bIsUsingPixelStreaming = bLocalPSCapable && RemotePixelStreamingVersion == TEXT("PS2");

	UE_LOGF(LogRemoteSession, Log, "Negotiated streaming mode: %ls (host PSVersion: %ls, local PS2 module: %ls)",
		bIsUsingPixelStreaming ? TEXT("PixelStreaming") : TEXT("MJPEG"),
		RemotePixelStreamingVersion.IsEmpty() ? TEXT("none") : *RemotePixelStreamingVersion,
		bLocalPSCapable ? TEXT("loaded") : TEXT("not loaded"));
}
...
void FRemoteSessionClient::OnReceiveChannelList(IBackChannelPacket& Message)
{
	...
	if (!IsLegacyConnection())
	{
		...
		if (NumChannels == 0)
		{
			UE_LOGF(LogRemoteSession, Error, "Received ChannelList messsage with no channels!");
	...
	for (int i = 0; i < NumChannels; i++)
	{
		...
		{
			UE_LOGF(LogRemoteSession, Log, "Remote host supports channel %ls with mode %ls", *ChannelName, ::LexToString((ERemoteSessionChannelMode)ChannelMode));
			AvailableChannels.Emplace(MoveTemp(ChannelName), ChannelMode);
		}
		else
		{
			UE_LOGF(LogRemoteSession, Error, "Failed to read channel from ChannelSelection message!");
		}
	}
	...
	if (IsLegacyConnection())
	{
		...
		UE_LOGF(LogRemoteSession, Log, "Creating all channels for legacy connection");
	}
	else
	{
