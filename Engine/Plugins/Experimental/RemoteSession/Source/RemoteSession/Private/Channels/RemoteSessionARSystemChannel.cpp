...
void FRemoteSessionARSystemChannel::ReceiveARInit(IBackChannelPacket& Message)
{
	...
	UE_LOGF(LogRemoteSession, Log, "Received AR session config (%ls)", *MsgParam.Param1);
...
void FRemoteSessionARSystemChannel::ReceiveAddTrackable_GameThread(FString ClassPathName, TSharedPtr<TArray<uint8>, ESPMode::ThreadSafe> DataCopy)
{
	...
	if (TrackableClass != nullptr)
	{
		...
		UE_LOGF(LogRemoteSession, Log, "Added new trackable (%ls) with class (%ls)", *TrackedGeometry->GetName(), *ClassPathName);
	}
	else
	{
		UE_LOGF(LogRemoteSession, Warning, "Couldn't find class (%ls) for added trackable", *ClassPathName);
	}
}
...
void FRemoteSessionARSystemChannel::ReceiveUpdateTrackable_GameThread(FGuid UniqueId, TSharedPtr<TArray<uint8>, ESPMode::ThreadSafe> DataCopy)
{
	...
	if (Updated != nullptr)
	{
		...
		UE_LOGF(LogRemoteSession, Log, "Updated trackable (%ls) with UniqueId (%ls)", *Updated->GetName(), *UniqueId.ToString());
	}
	else
	{
		UE_LOGF(LogRemoteSession, Warning, "Couldn't find trackable with UniqueId (%ls)", *UniqueId.ToString());
	}
}
...
void FRemoteSessionARSystemChannel::ReceiveRemoveTrackable_GameThread(FGuid UniqueId)
{
	...
	UE_LOGF(LogRemoteSession, Log, "Removed trackable with UniqueId (%ls)", *UniqueId.ToString());
}
...
void FRemoteSessionARSystemChannel::SendARInitMessage()
{
	if (Connection.IsValid())
	{
		...
		if (Config != nullptr)
		{
			...
			UE_LOGF(LogRemoteSession, Log, "Sent AR init message with session config (%ls)", *PathName);
		}
	}
}
...
void FRemoteSessionARSystemChannel::SendAddedMessage(UARTrackedGeometry* Added)
{
	...
	UE_LOGF(LogRemoteSession, Log, "Sent trackable added (%ls)", *Added->GetName());
}
...
void FRemoteSessionARSystemChannel::SendUpdatedMessage(UARTrackedGeometry* Updated)
{
	...
	UE_LOGF(LogRemoteSession, Log, "Sent trackable updated (%ls)", *Updated->GetName());
}
...
void FRemoteSessionARSystemChannel::SendRemovedMessage(UARTrackedGeometry* Removed)
{
	...
	UE_LOGF(LogRemoteSession, Log, "Sent trackable removed (%ls)", *Removed->GetName());
}
...
TSharedPtr<IRemoteSessionChannel> FRemoteSessionARSystemChannelFactoryWorker::Construct(ERemoteSessionChannelMode InMode, TSharedPtr<IBackChannelConnection, ESPMode::ThreadSafe> InConnection) const
{
	...
	{
		UE_LOGF(LogRemoteSession, Warning, "FRemoteSessionARSystemChannel does not support sending on this platform/device");
	}
