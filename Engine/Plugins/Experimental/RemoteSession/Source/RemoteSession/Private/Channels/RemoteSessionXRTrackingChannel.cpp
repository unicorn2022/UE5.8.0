...
public:
	TSharedPtr<IRemoteSessionChannel> Construct(ERemoteSessionChannelMode InMode, TBackChannelSharedPtr<IBackChannelConnection> InConnection) const
	{
		...
		{
			UE_LOGF(LogRemoteSession, Warning, "FRemoteSessionXRTrackingChannel does not support sending on this platform/device");
		}
	...
	if (Connection.IsValid())
    {
        if (XRSystem.IsValid() && XRSystem->IsTracking(IXRTrackingSystem::HMDDeviceId))
        {
            ...
            if (XRSystem->GetCurrentPose(IXRTrackingSystem::HMDDeviceId, Orientation, Location))
            {
                ...
                UE_LOGF(LogRemoteSession, VeryVerbose, "Sent Rotation (%.02f,%.02f,%.02f)", Rotation.Pitch,Rotation.Yaw,Rotation.Roll);
            }
            else
            {
                 UE_LOGF(LogRemoteSession, Warning, "Failed to get XRPose");
            }
        }
        else
        {
            UE_LOGF(LogRemoteSession, Warning, "XR Tracking not available to send");
        }
    }
}
...
void FRemoteSessionXRTrackingChannel::ReceiveXRTracking(IBackChannelPacket& Message)
{
	...
	{
        UE_LOGF(LogRemoteSession, Warning, "XRProxy is invalid. Cannot receive pose");
		return;
	}
    ...
    AsyncTask(ENamedThreads::GameThread, [TaskXRSystem, DataCopy=MoveTemp(DataCopy)]
	{
		...
		if (TaskXRSystemPinned)
		{
			...
			UE_LOGF(LogRemoteSession, VeryVerbose, "Received Rotation (%.02f,%.02f,%.02f)", MsgParam.Param2.Pitch, MsgParam.Param2.Yaw, MsgParam.Param2.Roll);
