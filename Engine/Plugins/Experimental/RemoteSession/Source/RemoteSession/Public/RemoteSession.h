...
public:
	...
	/** Opaque build identifier sent in the Hello handshake; surfaced to the peer as "BuildInfo". */
	static UE_API void			SetClientBuildInfo(const FString& BuildInfo);

	static UE_API FString		GetClientBuildInfo();

	/** Analytics provider for RemoteSession events; falls back to FEngineAnalytics when unset. */
	static UE_API void			SetAnalyticsProvider(TSharedPtr<IAnalyticsProviderET> Provider);
...
public:
	...
	/** Event name for the host connection-established analytics event. Resets on StopHost. */
	virtual void SetHostAnalyticsEventName(const FString& EventName) = 0;

	/** Record mode for the host connection-established analytics event. Resets to Cached on StopHost. */
	virtual void SetHostAnalyticsEventMode(EAnalyticsRecordEventMode Mode) = 0;
};
