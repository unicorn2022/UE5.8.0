...
void FCustomizableObjectPopulationEditor::TestPopulation()
{
	...
	if (Population && Viewport.IsValid())
	{
		...
		{
			UE_LOGF(LogMutable, Warning, "There are one or more unassigned Population Classes. Please review your %ls Population.", *(Population->Name));
			return;
		}

		if (!Population->HasGenerator())
		{
			UE_LOGF(LogMutable, Warning, "Save the population before testing it.");
			return;
		}
		...
		if (ViewportInstances.Num() > 0)
		{
			for (int32 i = 0; i < ViewportInstances.Num(); ++i)
			{
				...
				if (PreviewCustomizableSkeletalComponent && PreviewSkeletalMeshComponent && ColliderComponent)
				{
					...
					PreviewCustomizableSkeletalComponent->SetCustomizableObjectInstance(ViewportInstances[i]);
...
void FCustomizableObjectPopulationEditor::OpenSkeletalMesh()
{
	...
	TMap<FName, TObjectPtr<USkeletalMesh>>::TConstIterator It = ViewportInstances[SelectedInstance]->GetPrivate()->SkeletalMeshes.CreateConstIterator();
	if (!It)
	{
		return;
	}
	
	USkeletalMesh* SkeletalMesh = It->Value;
	if (!SkeletalMesh)
	{
		return;
	}	
...
void FCustomizableObjectPopulationEditor::GeneratePopulationInstances()
{
	if (!Population || !Population->IsValidPopulation())
	{
		UE_LOGF(LogMutable, Warning, "Unable to create customizable object instance assets from invalid population");
		return;
	}

	if (!Population->HasGenerator())
	{
		UE_LOGF(LogMutable, Warning, "Save the population before generating instances");
		return;
	}
