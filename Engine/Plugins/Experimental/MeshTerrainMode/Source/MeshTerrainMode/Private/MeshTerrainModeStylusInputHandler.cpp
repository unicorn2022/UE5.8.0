...
{
	return "MeshTerrainModeStylusInputHandler";
}
...
void FStylusInputHandler::OnPacket(const FStylusInputPacket& Packet, IStylusInputInstance* Instance)
{
	...
	{
		// Force instantiate the tablet context on hover for device supporting pressure detection.
		GetTabletContext(Instance, Packet.TabletContextID);
	}
}
...
bool FStylusInputHandler::AnyContextSupportsPressure() const
{
	for (const TPair<uint32, TSharedPtr<IStylusInputTabletContext>>& Entry : TabletContexts)
	{
		if (!Entry.Value)
		{
			continue;
		}
		const ETabletSupportedProperties SupportedProperties = Entry.Value->GetSupportedProperties();
		if ((SupportedProperties & ETabletSupportedProperties::NormalPressure) != ETabletSupportedProperties::None)
		{
			return true;
		}
	}
