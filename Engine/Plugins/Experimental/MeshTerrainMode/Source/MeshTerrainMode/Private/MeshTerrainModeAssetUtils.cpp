...
#define LOCTEXT_NAMESPACE "MeshTerrainModeAssetUtils"
...
FString UE::MeshTerrain::GetWorldRelativeAssetRootPath(const UWorld* World)
{
	if (World == nullptr)
	{
...
FString UE::MeshTerrain::GetGlobalAssetRootPath()
{
	...
	{
		UE_LOGF(LogTemp, Error, "Could not create global asset root path. Faiure Reason: %ls", *FailureReason);
		return {};
	}
}

FString UE::MeshTerrain::GetNewAssetPathName(const FString& BaseNameIn, const UWorld* TargetWorld, FString SuggestedFolder)
{
	const UMeshTerrainModeSettings* Settings = GetDefault<UMeshTerrainModeSettings>();
	const EMeshTerrainModeAssetGenerationBehavior AutoGenMode = Settings->GetAssetGenerationMode();
	...
	else
	{
		...
		if (Settings->GetAssetGenerationLocation() == EMeshTerrainModeAssetGenerationLocation::AutoGeneratedWorldRelativeAssetPath)
		{
		...
		if (Settings->GetAssetGenerationLocation() == EMeshTerrainModeAssetGenerationLocation::CurrentAssetBrowserPathIfAvailable)
		{
			FString CurrentAssetPath = Local::GetActiveAssetFolderPath();
	...
	if (AutoGenMode == EMeshTerrainModeAssetGenerationBehavior::InteractivePromptToSave || bFallBackToInteractiveSave)
	{
		const FString SelectedPath = Local::InteractiveSelectAssetPath(
...
bool UE::MeshTerrain::AutoSaveAsset(UObject* Asset)
{
...
void UE::MeshTerrain::OnNewAssetCreated(UObject* Asset)
{
	const UMeshTerrainModeSettings* Settings = GetDefault<UMeshTerrainModeSettings>();
	EMeshTerrainModeAssetGenerationBehavior AutoGenMode = Settings->GetAssetGenerationMode();
	...
	if (AutoGenMode == EMeshTerrainModeAssetGenerationBehavior::AutoGenerateAndAutosave
		 || AutoGenMode == EMeshTerrainModeAssetGenerationBehavior::InteractivePromptToSave)
	{
		AutoSaveAsset(Asset);
	}
	else if (AutoGenMode == EMeshTerrainModeAssetGenerationBehavior::AutoGenerateButDoNotAutosave)
	{
		Asset->MarkPackageDirty();
		FAssetRegistryModule::AssetCreated(Asset);
	}
