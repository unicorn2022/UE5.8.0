...
FEnumId FTestBatchBuilder::DeclareEnum(FType Type, EEnumMode Mode, ELeafWidth Width, std::initializer_list<const char*> InNames, std::initializer_list<uint64> Constants)
{
	FEnumId Id = IndexEnum(Type);
	EnumDeclarations.Declare(Id, Type, Mode, Width, MakeEnumerators(InNames, Constants), EEnumAliases::Fail);
	return Id;
}
...
static FSchemaBatchId ParseBatch(TArray64<uint8>& OutData, TArray<FStructView>& OutObjects)
{
...
[[nodiscard]] FSchemaBatchId ParseYamlBatch(TArray64<uint8>& OutData, TArray<FStructView>& OutObjects, FUtf8StringView YamlView)
{
	...
	return ParseBatch(OutData, OutObjects);
};

[[nodiscard]] FSchemaBatchId ParseJsonBatch(TArray64<uint8>& OutData, TArray<FStructView>& OutObjects, FUtf8StringView JsonView)
{
	ParseJsonBatch(OutData, JsonView);
	return ParseBatch(OutData, OutObjects);
}

enum class ERoundtripFormat { Yaml, Json };

static void RoundtripText(
	const FBatchIds& BatchIds,
	TConstArrayView<FStructView> Objects,
	ERoundtripFormat Format=ERoundtripFormat::Yaml)
{
	// Print text
	TUtf8StringBuilder<4096> Text;
	switch (Format)
	{
	case ERoundtripFormat::Yaml: PrintYamlBatch(Text, BatchIds, Objects); break;
	case ERoundtripFormat::Json: PrintJsonBatch(Text, BatchIds, Objects); break;
	}
	FUtf8StringView TextView = Text.ToView();

	// Log text
	auto Wide = StringCast<TCHAR>(TextView.GetData(), TextView.Len());
	UE_LOGF(LogPlainPropsTests, Log, "Schemas with StableNames:\n%.*ls", Wide.Length(), Wide.Get());

	// Parse text
	TArray64<uint8> Data;
	TArray<FStructView> ParsedObjects;
	FSchemaBatchId ParsedBatch;
	switch (Format)
	{
	case ERoundtripFormat::Yaml: ParsedBatch = ParseYamlBatch(Data, ParsedObjects, TextView); break;
	case ERoundtripFormat::Json: ParsedBatch = ParseJsonBatch(Data, ParsedObjects, TextView); break;
	}
...
public:
	...
	void RoundtripText(PlainProps::ERoundtripFormat Format)
	{
		PlainProps::RoundtripText(BatchIds.GetValue(), Objects, Format);
	}
...
static void TestSerialize(void (*BuildObjects)(FTestBatchBuilder&, FScratchAllocator&), void (*CheckObjects)(TConstArrayView<FStructView>, const FTestNameReader&))
{
...

	{
		...
		Batch.RoundtripText(PlainProps::ERoundtripFormat::Yaml);
		CheckObjects(Batch.GetObjects(), Batch.GetNames());
	}

	{
		FTestBatchReader Batch(MakeMemoryView(Data));
		Batch.RoundtripText(PlainProps::ERoundtripFormat::Json);
		CheckObjects(Batch.GetObjects(), Batch.GetNames());
	}
}
...
TEST_CASE_NAMED(FPlainPropsReadWriteTest, "System::Core::Serialization::PlainProps::ReadWrite", "[Core][PlainProps][SmokeFilter]")
{
	...
	SECTION("Hex")
	{
		static constexpr std::initializer_list<const char*> MemberNames =  {"Hex8", "Hex16", "Hex32", "Hex64"};

		TestSerialize([](FTestBatchBuilder& Batch, FScratchAllocator& Scratch)
		{
			FMemberSpec MemberTypes[] = {SpecHex8, SpecHex16, SpecHex32, SpecHex64};
			FDeclId Id = Batch.DeclareStruct("Test", "Hex", MemberNames, MemberTypes, EMemberPresence::AllowSparse);

			FMemberBuilder Misc, Mins, Maxs, Some;

			Misc.AddHex(Batch.NameMember("Hex8"),	uint8(8));
			Misc.AddHex(Batch.NameMember("Hex16"),	uint16(16));
			Misc.AddHex(Batch.NameMember("Hex32"),	uint32(32));
			Misc.AddHex(Batch.NameMember("Hex64"),	uint64(64));
			
			Mins.AddHex(Batch.NameMember("Hex8"),	std::numeric_limits< uint8>::min());
			Mins.AddHex(Batch.NameMember("Hex16"),	std::numeric_limits<uint16>::min());
			Mins.AddHex(Batch.NameMember("Hex32"),	std::numeric_limits<uint32>::min());
			Mins.AddHex(Batch.NameMember("Hex64"),	std::numeric_limits<uint64>::min());

			Maxs.AddHex(Batch.NameMember("Hex8"),	std::numeric_limits< uint8>::max());
			Maxs.AddHex(Batch.NameMember("Hex16"),	std::numeric_limits<uint16>::max());
			Maxs.AddHex(Batch.NameMember("Hex32"),	std::numeric_limits<uint32>::max());
			Maxs.AddHex(Batch.NameMember("Hex64"),	std::numeric_limits<uint64>::max());

			Some.AddHex(Batch.NameMember("Hex32"),	uint32(0));
		...
		[](TConstArrayView<FStructView> Objects, const FTestNameReader& Names)
		{
			...
			FTestMemberReader Misc(Objects[0]);
			CHECK(Misc.GrabLeaf().AsHex8()	== uint8(8));
			CHECK(Misc.GrabLeaf().AsHex16()	== uint16(16));
			CHECK(Misc.GrabLeaf().AsHex32()	== uint32(32));
			CHECK(Misc.GrabLeaf().AsHex64()	== uint64(64));

			FTestMemberReader Mins(Objects[1]);
			CHECK(Mins.GrabLeaf().AsHex8()	== std::numeric_limits< uint8>::min());
			CHECK(Mins.GrabLeaf().AsHex16()	== std::numeric_limits<uint16>::min());
			CHECK(Mins.GrabLeaf().AsHex32()	== std::numeric_limits<uint32>::min());
			CHECK(Mins.GrabLeaf().AsHex64()	== std::numeric_limits<uint64>::min());

			FTestMemberReader Maxs(Objects[2]);
			CHECK(Maxs.GrabLeaf().AsHex8()	== std::numeric_limits< uint8>::max());
			CHECK(Maxs.GrabLeaf().AsHex16()	== std::numeric_limits<uint16>::max());
			CHECK(Maxs.GrabLeaf().AsHex32()	== std::numeric_limits<uint32>::max());
			CHECK(Maxs.GrabLeaf().AsHex64()	== std::numeric_limits<uint64>::max());

			FTestMemberReader Some(Objects[3]);
			CHECK(Names[Some.PeekName()] == "Hex32");
			CHECK(Some.GrabLeaf().AsHex32() == 0);
		});
	}
	...
	SECTION("Enum")
	{
		...
		{ "A2", "A0", "B0", "B4", "B5", "B7", "C3", "D0", "D34", "Max8", "Max16", "Max32", "Max64", "IF" };
		...
		TestSerialize([](FTestBatchBuilder& Batch, FScratchAllocator& Scratch)
		{
			...
			FEnumId D	= Batch.DeclareEnum("Test", "FlagSparse8",	EEnumMode::Flag, ELeafWidth::B8,	{"None", "A", "B", "C"}, {0, 2, 16, 32});
			...
			FMemberSpec MemberTypes[] = {
				...
				Enum8(C), Enum8(D), Enum8(D), Enum8(E), Enum16(F), Enum32(G), Enum64(H), Enum64(I) };
			...
			Members.AddEnum(Batch.NameMember("D0"),				D, (uint8)0);
		...
		[](TConstArrayView<FStructView> Objects, const FTestNameReader& Names)
		{
			...
			CHECK(EqualEnumNames(MakeConstArrayView(FlagSparse8.Footer, FlagSparse8.Num), MakeConstArrayView({"None", "A", "C"})));
			...
			CHECK(EqualItems(GetConstants<uint8>(FlagSparse8), MakeConstArrayView({0, 2, 32})));
	...
	SECTION("LeafRange")
	{
		...
		TestSerialize([](FTestBatchBuilder& Batch, FScratchAllocator& Scratch)
		{
			...
			static constexpr std::initializer_list<const char*> MemberNames = { "B0", "B1", "B8", "B9", "D0", "D3", "H0", "E3", "E0", "Hi" };
			FMemberSpec MemberTypes[] = {	S32Range(SpecBool), S32Range(SpecBool), S64Range(SpecBool), S64Range(SpecBool), 
											S32Range(SpecF64), S32Range(SpecF64),
											S32Range(SpecHex16), S32Range(Enum16(ABCD)), S32Range(Enum8(Unused1)),
											S32Range(SpecUtf8)};
			...
			Members.AddRange(Batch.NameMember("H0"), BuildHexRange(Scratch,  TConstArrayView<uint16>({0x00, 0x01, 0xEF, 0xFF})));
		...
		[](TConstArrayView<FStructView> Objects, const FTestNameReader& Names)
		{
			...
			FLeafRangeView H0 = It.GrabRange().AsLeaves();
			...
			CHECK(EqualItems(H0.AsHex16s(),		TConstArrayView<uint16>({0x00, 0x01, 0xEF, 0xFF})));
	...
	SECTION("NestedRange")
	{
		...
		TestSerialize([](FTestBatchBuilder& Batch, FScratchAllocator& Scratch)
		{
			...
			FMemberSpec MemberTypes[] = { S32Range(S32Range(SpecS32)), S32Range(S32Range(SpecS32)), S64Range(S64Range(SpecF32)),
			...
			FNestedRangeBuilder StructRRs(MakeNestedRangeSchema(Scratch, ESizeType::S16, ZWRangeSchema), 1);
			StructRRs.Add(ZWRs.BuildAndReset(Scratch, ESizeType::S16));
	...
	SECTION("UniRange")
	{
		TestSerialize([](FTestBatchBuilder& Batch, FScratchAllocator& Scratch)
		{
			FMemberSpec StructMemberTypes[] = { UniRange(SpecBool), S32Range(SpecBool), S32Range(UniRange(SpecBool)), SpecBool };
			...
			Structs[7].AddRange(Batch.NameMember("MaybeBs"), MaybeBs.BuildAndReset(Scratch, ESizeType::S32));
			Structs[7].Add(Batch.NameMember("B"), true);
			MaybeBs.Add(BuildLeafRange(Scratch, &True, false));
			Structs[8].AddRange(Batch.NameMember("MaybeBs"), MaybeBs.BuildAndReset(Scratch, ESizeType::S32));
	...
	SECTION("DynamicStructRange")
	{
		...
		{
			FDeclId SA = Batch.DeclareStruct("Test", "SA", {"X"}, {SpecS32}, EMemberPresence::AllowSparse);
			FDeclId Unused = Batch.DeclareStruct("Test", "Unused2", {"X"}, {SpecS32}, EMemberPresence::AllowSparse);
			FDeclId SB = Batch.DeclareStruct("Test", "SB", {"X"}, {SpecF32}, EMemberPresence::AllowSparse);
