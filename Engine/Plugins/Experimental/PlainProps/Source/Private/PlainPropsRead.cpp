...
const FSchemaId*	FMemberReader::GetInnerSchemas() const	{ return FStructSchema::GetInnerSchemas(Footer, NumMembers, NumRangeTypes, NumMembers - UsesSuper); }
...
, UsesSuper(PlainProps::UsesSuper(Schema.Inheritance))
...
{
	int32 MemberNameIdx = MemberIdx - UsesSuper;
	return MemberNameIdx >= 0 ? ToOptional(GetMemberNames()[MemberNameIdx]) : NoId;
}

FMemberId FMemberReader::PeekNameUnchecked() const
{
	int32 MemberNameIdx = MemberIdx - UsesSuper;
...
void FMemberReader::SkipMember()
{
	switch (PeekKind())
	{
		case EMemberKind::Leaf:		(void)GrabLeaf();		break;
		case EMemberKind::Struct:	(void)GrabStruct();		break;
		case EMemberKind::Range:	(void)GrabRange();		break;
	}
}
