...
void FTransformBinding::Save(FMemberBuilder& Dst, const FTransform& Src, const FTransform* Default, const FSaveContext& Ctx) const
{
	...
	// Lazily cache declarations on first save, then hoist them to the stack
	if (!VectorDeclaration)
	{
		VectorDeclaration = &Ctx.Schemas.GetDeclaration(VectorId);
		QuatDeclaration = &Ctx.Schemas.GetDeclaration(QuatId);
	}
	const FStructDeclaration& VectorDecl = *VectorDeclaration;
	const FStructDeclaration& QuatDecl = *QuatDeclaration;
...
inline FDiffNode MakeStructDiff(const void* A, const void* B, FMemberId Name, FBindId Id, FDiffContext& Ctx)
{
	return { DefaultStructBindType, Name, {.Struct = Id}, A, B, Ctx.LastSibling };
}
...
bool FTransformBinding::Diff(const FTransform& A, const FTransform& B, FDiffContext& Ctx) const
{
	...
	{
		Ctx.AddDiff(MakeStructDiff(&A, &B, MemberIds[(uint8)EMember::Translate], VectorId, Ctx));
		return true;
	}
	if (!A.RotationEquals(B, 0.0))
	{
		Ctx.AddDiff(MakeStructDiff(&A, &B, MemberIds[(uint8)EMember::Rotate], QuatId, Ctx));
		return true;
	}
	if (!A.Scale3DEquals(B, 0.0))
	{
		Ctx.AddDiff(MakeStructDiff(&A, &B, MemberIds[(uint8)EMember::Scale], VectorId, Ctx));
		return true;
	}
...
void FGuidBinding::Save(FMemberBuilder& Dst, const FGuid& Src, const FGuid*, const FSaveContext&) const
{
	...
	static_assert(offsetof(FGuid, A) == 0);
	static_assert(offsetof(FGuid, B) == 4);
	static_assert(offsetof(FGuid, C) == 8);
	static_assert(offsetof(FGuid, D) == 12);
}
