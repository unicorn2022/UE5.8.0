...
int32 FDiffContext::AddDiff(FDiffNode&& Node)
{
	int32 Index = Diffs.Add(Node);
...
void FDiffContext::Reset()
{
	Diffs.Reset();
	LastSibling = INDEX_NONE;
}
...
inline bool DiffMember(FLeafMemberBinding Member, const void* OwnerA, const void* OwnerB, FDiffContext& Ctx, FOptionalMemberId Name)
{
	...
	if (DiffLeaf(A, B, Member.Leaf))
	{
		...
		Ctx.AddDiff({Member.Leaf.Pack(), Name, {Member.Enum}, A, B, Ctx.LastSibling});
		return true;
	}
	return false;
}
...
struct TLastSiblingScope
{
	TLastSiblingScope(ContextType& InCtx) : Ctx(InCtx)
	{
		if constexpr (std::is_same_v<ContextType, FDiffContext>)
		{
			OldLastSibling = InCtx.LastSibling;
			InCtx.LastSibling = INDEX_NONE;
		}
	}
	...
	~TLastSiblingScope()
	{
		if constexpr (std::is_same_v<ContextType, FDiffContext>)
		{
			Ctx.LastSibling = OldLastSibling;
		}
	}

	ContextType& Ctx;
	int32 OldLastSibling;
};
...
bool DiffMember(FRangeMemberBinding Member, const void* OwnerA, const void* OwnerB, ContextType& Ctx, FOptionalMemberId Name)
{
	...
	else
	{
		...
		{
			bDiff = DiffItemRange(A, B, ItemBinding, Ctx, Member.InnermostId.Get().AsStructBindId());
		}
	...
	if constexpr (std::is_same_v<ContextType, FDiffContext>)
	{
		...
		{
			// place breakpoint here
			Ctx.AddDiff({FMemberBindType(Binding.GetSizeType()), Name, {.Range = Binding}, A, B, Ctx.LastSibling});
		}
	}
	return bDiff;
}
...
static bool DiffMember(FStructMemberBinding Member, const void* OwnerA, const void* OwnerB, FDiffContext& Ctx, FMemberId Name)
{
	...
	{
		// place breakpoint here
		Ctx.AddDiff({FMemberBindType(Member.Type), Name, {.Struct = Member.Id}, A, B, Ctx.LastSibling});
		return true;
	}
	return false;
}
...
bool DiffMembers(FBindId Id, const void* A, const void* B, ContextType& Ctx)
{
	...
	if (Schema.HasSuper())
	{
		...
		{
			bDiff = true;
			if (StopOnFirstDiff(Ctx)) { return true; }
		}
	}

	TLastSiblingScope ScopeLastSibling(Ctx);
...
static bool DiffLeaves(FUnpackedLeafType Leaf, FLeafRangeView A, FLeafRangeView B, uint64& OutIdx)
{
	...
	case ELeafType::Hex:
		switch (Leaf.Width)
		{
		case ELeafWidth::B8:  return DiffItems(A.AsHex8s(),  B.AsHex8s(),  OutIdx);
		case ELeafWidth::B16: return DiffItems(A.AsHex16s(), B.AsHex16s(), OutIdx);
		case ELeafWidth::B32: return DiffItems(A.AsHex32s(), B.AsHex32s(), OutIdx);
		case ELeafWidth::B64: return DiffItems(A.AsHex64s(), B.AsHex64s(), OutIdx);
		}
	case ELeafType::Enum:
		switch (Leaf.Width)
		{
...
bool operator==(const FDiffPath& A, const FDiffPath& B)
{
	if (A.Num() != B.Num())
	{
		return false;
...
uint32 GetTypeHash(const FDiffPath& Path)
{
	uint32 Hash = 0;

	for (const FDiffNode& Node : Path)
	{
		uint32 NameHash = GetTypeHash(Node.Name);
		Hash = HashCombineFast(Hash, NameHash);
	}
