...
TOptional<T> ParseHex(FUtf8StringView String)
{
	const char* First = reinterpret_cast<const char*>(String.GetData());
	const char* Last = First + String.Len();
	T Out;
	std::from_chars_result Result = std::from_chars(First, Last, Out, 16);
	return Result.ec == std::errc{} ? Out : TOptional<T>{ NullOpt };
}
...
template <UnsignedIntegral T>
static void ParseEnumValue(
	FUtf8StringView String,
	T& OutValue,
	const FParsedEnumSchema& Schema,
	const FTextIndexer& Names)
{
	OutValue = 0;

	if (String == GLiterals.UnsetFlagEnum)
	{
		return;
	}
	...
	{
		static constexpr FAsciiSet Separators(" |");

		String = FAsciiSet::TrimPrefixWith(String, Separators);
		FUtf8StringView Element = FAsciiSet::FindPrefixWithout(String, Separators);
		check(!Element.IsEmpty());

		String = String.Mid(Element.Len());

		bool bEnumeratorFound = false;
		for (const auto& [Name, Constant] : Schema.Enumerators)
		{
			FUtf8StringView NameView = Names.ResolveName(Name);
			if (NameView != Element)
			{
				continue;
			}
			...
			OutValue |= Constant;
			bEnumeratorFound = true;
			break;
		}

		checkf(bEnumeratorFound, TEXT("Unable to match enumerator '%.*hs'"), String.Len(), String.GetData());
	}
}
...
class FMarkupTokenizer
{
public:
	virtual						~FMarkupTokenizer() = default;
	virtual bool				HasMore() const = 0;
	virtual TOptional<FToken>	GrabToken() = 0;

	// PP-TEXT: GetLastError is a very temporary API while errors are unexpected
	FUtf8StringView GetLastError() const;

protected:
							FMarkupTokenizer(FUtf8StringView InStringView);
	FLineInfo				ReadOneLine();
	void					SetLastError(FUtf8StringView Message, FUtf8StringView Context);
...
FMarkupTokenizer::FMarkupTokenizer(FUtf8StringView InStringView)
: ReadView(InStringView)
{
}

FUtf8StringView FMarkupTokenizer::GetLastError() const
{
	return LastError;
}

FLineInfo FMarkupTokenizer::ReadOneLine()
{
	if (ReadView.IsEmpty())
	{
		return {
...
void FMarkupTokenizer::SetLastError(FUtf8StringView Message, FUtf8StringView Context)
{
	LastError.Reset();
	if (Message.Len())
	{
		LastError.Appendf("'%.*s' in line %u: '%.*s'",
...
class FYamlTokenizer
	: public FMarkupTokenizer
{
public:
	FYamlTokenizer(FUtf8StringView InStringView);
	~FYamlTokenizer();

	virtual bool HasMore() const override { return ReadLine.EndOfFile == false || ScopeStack.Num() > 0; }
	virtual TOptional<FToken> GrabToken() override;
...
TOptional<FToken> FYamlTokenizer::GrabTokenInternal()
{
	...
	SetLastError(Error, ReadLine.View);
	return ParsedToken;
}
...
class FJsonTokenizer
	: public FMarkupTokenizer
{
public:
								FJsonTokenizer(FUtf8StringView InData);
								~FJsonTokenizer();
	virtual bool				HasMore() const override;
	virtual TOptional<FToken>	GrabToken() override;

private:
	enum class EScope : uint8 { Document, Struct, Range, };

	enum : uint16
	{
		Flag_AllowStruct	= 1 << 0,
		Flag_AllowRange		= 1 << 1,
		Flag_AllowOpen		= Flag_AllowStruct | Flag_AllowRange,
		Flag_AllowClose		= 1 << 2,
		Flag_ExpectKey		= 1 << 3,
		Flag_ExpectColon	= 1 << 4,
		Flag_ExpectValue	= 1 << 5 | Flag_AllowOpen,
		Flag_ExpectComma	= 1 << 6,
		Flag_HasComma		= 1 << 7,
	};
	...
	TOptional<FToken>	GrabTokenInternal();
	FToken				SetLastError(FUtf8StringView Msg);
	void				AdvanceData(uint32 ChopLength);
	TOptional<FToken>	ParseOpen();
	TOptional<FToken>	ParseClose();
	TOptional<FToken>	ParseKey();
	TOptional<FToken>	ParseColon();
	TOptional<FToken>	ParseValue();
	TOptional<FToken>	ParseComma();
	FUtf8StringView		Data;
	TArray<FScopeInfo>	Scopes;
	TOptional<FToken>	PendingToken;

};

FJsonTokenizer::FJsonTokenizer(FUtf8StringView InData)
: FMarkupTokenizer(InData)
{
	Scopes.Emplace(FScopeInfo{
		.Flags = Flag_AllowStruct,
	});

	AdvanceData(0);
}

FJsonTokenizer::~FJsonTokenizer()
{
	check(!PendingToken.IsSet());
	check(Scopes.IsEmpty());
}

bool FJsonTokenizer::HasMore() const
{
	return !Scopes.IsEmpty();
}

void FJsonTokenizer::AdvanceData(uint32 ChopLength)
{
	Data.RightChopInline(ChopLength);
	FUtf8StringView Trim = FAsciiSet::FindPrefixWith(Data, WhitespaceSet);
	Data.RightChopInline(Trim.Len());
	...
	while (Line.View.IsEmpty());

	Data = Line.View;
}

FToken FJsonTokenizer::SetLastError(FUtf8StringView Msg)
{
	FMarkupTokenizer::SetLastError(Msg, Data);
	Scopes.Reset();
	return FToken{ EToken::Invalid, 0 };
}

TOptional<FToken> FJsonTokenizer::ParseOpen()
{
	FScopeInfo& Last = Scopes.Last();
	...
	if (Char == '[')
	{
		...
		if ((Last.Flags & Flag_AllowRange) == 0)
		{
			return SetLastError("Range not allowed here");
		}

		Last.Flags = Flag_ExpectComma | Flag_AllowClose;

		Scopes.Emplace(FScopeInfo{
			.Scope = EScope::Range,
			.Flags = Flag_ExpectValue | Flag_AllowClose,
		});
		uint16 Depth = uint16(Scopes.Num() - 1);

		return FToken{ EToken::BeginRange, Depth };
	}

	if (Char == '{')
	{
		check(Last.Flags & Flag_AllowStruct);

		AdvanceData(1);

		if (Last.Flags == Flag_AllowStruct)
		{
			check(Scopes.Num() == 1);
			Last.Flags = Flag_ExpectKey | Flag_AllowClose;
			return FToken{ EToken::BeginDocument, 0 };
		}
		...
		Scopes.Emplace(FScopeInfo{
			.Scope = EScope::Struct,
			.Flags = Flag_ExpectKey | Flag_AllowClose,
		});
		uint16 Depth = uint16(Scopes.Num() - 1);

		return FToken{ EToken::BeginStruct, Depth };
	}

	return {};
}

TOptional<FToken> FJsonTokenizer::ParseClose()
{
	int32 Char = Data[0];
	if ((Char != ']') && (Char != '}'))
	{
	...
	const FScopeInfo& Last = Scopes.Last();

	if (Last.Flags & Flag_HasComma)
	{
		return SetLastError("Trailing seperator");
	}

	bool bValid = (Char == '}') & (Last.Scope != EScope::Range);
	bValid |= (Char == ']') & (Last.Scope == EScope::Range);
	if (!bValid)
	{
		return SetLastError("Mismatched scope");
	}
	...
	EToken Token = EToken::EndRange;
	if (Char == '}')
	{
		Token = (Scopes.IsEmpty()) ? EToken::EndDocument : EToken::EndStruct;
	}

	return FToken{ Token, uint16(Scopes.Num()) };
}

TOptional<FToken> FJsonTokenizer::ParseKey()
{
	int32 Char = Data[0];
	if (Char != '\"')
	{
		return SetLastError("Key string expected");
	}

	FLineInfo LineInfo = { .View = Data, };
	FUtf8StringView Key = GrabTokenFromLine(LineInfo);
	if (Key.IsEmpty())
	{
		return SetLastError("Empty key");
	}

	AdvanceData(Key.Len() + 2);

	Scopes.Last().Flags = Flag_ExpectColon;
	return FToken{ EToken::LeafId, uint16(Scopes.Num()), Key };
}

TOptional<FToken> FJsonTokenizer::ParseColon()
{
	int32 Char = Data[0];
	if (Char != ':')
	{
		return SetLastError("Key should be followed by a colon");
	}

	AdvanceData(1);

	Scopes.Last().Flags = Flag_ExpectValue;
	return {};
}

TOptional<FToken> FJsonTokenizer::ParseValue()
{
	int32 Char = Data[0];

	if (TOptional<FToken> Token = ParseOpen(); Token.IsSet())
	{
		return Token;
	}
	...
	if (Char != '\"')
	{
		return SetLastError("Only string-type values are supported");
	}

	FLineInfo LineInfo = { .View = Data, };
	FUtf8StringView Value = GrabTokenFromLine(LineInfo);
	AdvanceData(Value.Len() + 2);

	Scopes.Last().Flags = Flag_ExpectComma | Flag_AllowClose;

	EToken Token = (Scopes.Last().Scope == EScope::Range) ? EToken::Leaf : EToken::LeafValue;
	return FToken{ Token, uint16(Scopes.Num()), Value };
}

TOptional<FToken> FJsonTokenizer::ParseComma()
{
	int32 Char = Data[0];
	if (Char != ',')
	{
		return SetLastError("Comma separator expected");
	}

	AdvanceData(1);

	FScopeInfo& Last = Scopes.Last();
	Last.Flags = (Last.Scope == EScope::Range) ? Flag_ExpectValue : Flag_ExpectKey;
	Last.Flags |= Flag_HasComma;
	return {};
}

TOptional<FToken> FJsonTokenizer::GrabTokenInternal()
{
	if (Data.IsEmpty())
	{
		return SetLastError("Encountered EOF");
	}

	if (Scopes.IsEmpty())
	{
		return SetLastError("Unexpected token");
	}

	static constexpr struct {
		uint32					Flag;
		TOptional<FToken>		(FJsonTokenizer::*Method)(void);
	} Rota[] = {
		{ Flag_AllowOpen,		&FJsonTokenizer::ParseOpen },
		{ Flag_AllowClose,		&FJsonTokenizer::ParseClose },
		{ Flag_ExpectComma, 	&FJsonTokenizer::ParseComma },
		{ Flag_ExpectKey,		&FJsonTokenizer::ParseKey },
		{ Flag_ExpectColon,		&FJsonTokenizer::ParseColon },
		{ Flag_ExpectValue,		&FJsonTokenizer::ParseValue },
	};

	for (const FScopeInfo& Last = Scopes.Last();;)
	{
		for (const auto& [RotaFlag, Method] : Rota)
		{
			if (Last.Flags & RotaFlag)
			{
				if (TOptional<FToken> Token = (this->*Method)(); Token.IsSet())
				{
					return Token;
				}
			}
		...
		if (Last.Flags & (Flag_ExpectKey | Flag_ExpectValue))
		{
			continue;
		}

		SetLastError("Parsing reached the unreachable");
		return {};
	}
}

TOptional<FToken> FJsonTokenizer::GrabToken()
{
	TOptional<FToken> Token;
	if (PendingToken.IsSet())
	{
		Token = PendingToken.GetValue();
		PendingToken.Reset();
	}
	else
	{
		Token = GrabTokenInternal();
	}

	if (!Token.IsSet())						return {};
	if (Token->Token == EToken::Invalid)	return {};
	if (Token->Token != EToken::LeafId)		return Token;

	TOptional<FToken> NextToken = GrabTokenInternal();
	if (NextToken.IsSet())
	{
		if (NextToken->Token == EToken::Invalid)
		{
			return {};
		}

		if (NextToken->Token == EToken::BeginStruct || NextToken->Token == EToken::BeginRange)
		{
			NextToken->Str = Token->Str;
			NextToken->Len = Token->Len;
			return NextToken;
		}
	}
...
bool FEnumSchemaParser::ParseEnumSchema(FParsedEnumSchema& Out)
{
	...
	while (ScopedReader.Peek()->Depth == TokenDepth)
	{
		...
		else if (Token.Token == EToken::BeginStruct)
		{
		...
		{
			checkf(false, TEXT("Invalid token %d '%s' for enum"), EnumToUnderlyingType(Token.Token), *Print(Token));
		}
	}
	...
	while (ScopedReader.Peek()->Depth == TokenDepth)
	{
		...
		{
			checkf(false, TEXT("Invalid token %d '%s' in enum constants"), EnumToUnderlyingType(Token.Token), *Print(Token));
		}
	}
	...
	TConstArrayView<uint64> ConstantValues)
{
	...
	const FEnumDeclaration& Decl = Declarations.Declare(Id, Type, Mode, Width, Enumerators, EEnumAliases::Strip);
...
bool FStructSchemaParser::ParseStructSchema(FParsedStructSchema& Out)
{
	...
	while (ScopedReader.Peek()->Depth == TokenDepth)
	{
		...
		{
			checkf(false, TEXT("Invalid token %d '%s' for struct"), EnumToUnderlyingType(Token.Token), *Print(Token));
		}
	}
	...
	while (ScopedReader.Peek()->Depth == TokenDepth)
	{
		...
		{
			checkf(false, TEXT("Invalid token %d '%s' in struct members"), EnumToUnderlyingType(Token.Token), *Print(Token));
		}
	}
...
bool FMemberParser::ParseAll()
{
	while (HasMore())
	{
		...
		else
		{
			...
			checkf(false, TEXT("Invalid member token %d '%s'"), EnumToUnderlyingType(Token.Token), *Print(Token))
		}
	}
	return true;
}
...
template<UnsignedIntegral T>
void AddEnum(FMemberBuilder& Members, FMemberId Id, const FParsedEnumSchema& Schema, const FTextIndexer& Names, FUtf8StringView String)
{
	T Value;
	ParseEnumValue(String, Value, Schema, Names);
	Members.AddEnum(Id, Schema.Id, Value);
}

template<UnsignedIntegral T>
void AddHex(FMemberBuilder& Members, FMemberId Id, FUtf8StringView String)
{
	if (TOptional<T> Value = ParseHex<T>(String))
	{
		Members.AddHex(Id, Value.GetValue());
	}
}
...
bool FMemberParser::ParseLeaf()
{
	...
	switch (Leaf.Type)
	{
		...
		case ELeafType::Hex:
			switch (Leaf.Width)
			{
				case ELeafWidth::B8:	AddHex<uint8 >(MemberBuilder, MemberId, String);	break;
				case ELeafWidth::B16:	AddHex<uint16>(MemberBuilder, MemberId, String);	break;
				case ELeafWidth::B32:	AddHex<uint32>(MemberBuilder, MemberId, String);	break;
				case ELeafWidth::B64:	AddHex<uint64>(MemberBuilder, MemberId, String);	break;
			}
			break;
		case ELeafType::Enum:
			switch (Leaf.Width)
			{
				case ELeafWidth::B8:	AddEnum<uint8 >(MemberBuilder, MemberId, ResolveEnumSchema(Schemas, Enum.Get()), Names, String); break;
				case ELeafWidth::B16:	AddEnum<uint16>(MemberBuilder, MemberId, ResolveEnumSchema(Schemas, Enum.Get()), Names, String); break;
				case ELeafWidth::B32:	AddEnum<uint32>(MemberBuilder, MemberId, ResolveEnumSchema(Schemas, Enum.Get()), Names, String); break;
				case ELeafWidth::B64:	AddEnum<uint64>(MemberBuilder, MemberId, ResolveEnumSchema(Schemas, Enum.Get()), Names, String); break;
			}
			break;
	...
	FScopedTokenReader& RangeTokens)
{
	...
	return BuildEnumRange(Scratch, Schema.Id, MaxSize, TConstArrayView<T>(Values));
}

template<UnsignedIntegral T>
[[nodiscard]] static FTypedRange ParseHexRange(
	FScratchAllocator& Scratch,
	ESizeType MaxSize,
	FScopedTokenReader& RangeTokens)
{
	...
	{
		FToken Token = RangeTokens.GrabToken();
		if (TOptional<T> Value = ParseHex<T>(Token.Value()))
		{
			Values.Add(Value.GetValue());
		}
	}
	return BuildHexRange(Scratch, MaxSize, TConstArrayView<T>(Values));
}
...
FTypedRange FMemberParser::ParseLeaves(const FMemberSchema& RangeSchema, FScopedTokenReader RangeTokens)
{
	...
	switch (Leaf.Type)
	{
		...
		case ELeafType::Hex:
			switch (Leaf.Width)
			{
				case ELeafWidth::B8:	TypedRange = ParseHexRange<uint8 >(Scratch, NumType, RangeTokens); break;
				case ELeafWidth::B16:	TypedRange = ParseHexRange<uint16>(Scratch, NumType, RangeTokens); break;
				case ELeafWidth::B32:	TypedRange = ParseHexRange<uint32>(Scratch, NumType, RangeTokens); break;
				case ELeafWidth::B64:	TypedRange = ParseHexRange<uint64>(Scratch, NumType, RangeTokens); break;
			}
			break;
		case ELeafType::Enum:
			switch (Leaf.Width)
			{
				case ELeafWidth::B8:	TypedRange = ParseEnumRange<uint8 >(Scratch, ResolveEnumSchema(Schemas, Enum.Get()), Names, NumType, RangeTokens); break;
				case ELeafWidth::B16:	TypedRange = ParseEnumRange<uint16>(Scratch, ResolveEnumSchema(Schemas, Enum.Get()), Names, NumType, RangeTokens); break;
				case ELeafWidth::B32:	TypedRange = ParseEnumRange<uint32>(Scratch, ResolveEnumSchema(Schemas, Enum.Get()), Names, NumType, RangeTokens); break;
				case ELeafWidth::B64:	TypedRange = ParseEnumRange<uint64>(Scratch, ResolveEnumSchema(Schemas, Enum.Get()), Names, NumType, RangeTokens); break;
			}
			break;
		case ELeafType::Unicode:
...
void FBatchParser::Tokenize()
{
	while (Tokenizer.HasMore())
	{
		...
		if (Token->Depth == 1 && Token->Token == EToken::BeginStruct)
		{
		...
		else if (Token->Depth == 1 && Token->Token == EToken::BeginRange)
		{
			if (Token->Value() == GLiterals.Objects)
			{
...
void FBatchParser::ParseObjects(TConstArrayView64<FToken> TokensView)
{
	...
	while (ScopedReader.HasMore())
	{
		// Each item in the "Objects" range is a struct with one leaf, where the
		// leaf's key is the object type and it's value a struct of the object;
		//
		//   [ {"<type>": { ... }}, {"<type>": ...etc
		//
		// So we've two structs and depths to fetch.
		...
		FScopedTokenReader StructReader(TokenIt);
		if (!GrabToNextBeginStruct(Token, StructReader))
		{
			break;
		}
...
void ParseJsonBatch(TArray64<uint8>& OutBinary, FUtf8StringView Json)
{
	FScratchAllocator Scratch;
	FJsonTokenizer JsonScanner(Json);
	FBatchParser BatchParser(JsonScanner, Scratch);
