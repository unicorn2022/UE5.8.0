...
void FStructSchemaBuilder::NoteMembersRecursively(const FBuiltStruct& Struct)
{
	...
	{
		while (NoteIdx < NumNoted && MemberOrder[NoteIdx] != Member.Name)
		{
			++NoteIdx;
		}
		checkf(NoteIdx < NumNoted, TEXT("Member '%s' has not been declared in '%s'"),
...
{
	checkf(NotedWidth == NullOpt || NotedWidth == Width,
		TEXT("Enum '%s' in '%s' first added as '%.*hs' and later as '%.*hs' for value '%llx'."),
		*Debug.Print(Member), *Debug.Print(Struct),
		ToString(*NotedWidth).Len(), ToString(*NotedWidth).GetData(),
		ToString(Width).Len(), ToString(Width).GetData(),
		Value);
	...
	if (Declaration.Mode == EEnumMode::Flag)
	{
		...
		else
		{
			...
			for (int32 Idx = NumValidated, Num = NotedConstants.Num(); Idx < Num; ++Idx)
			{
				...
				checkf(Algo::FindBy(Declaration.GetEnumerators(), Flag, &FEnumerator::Constant),
					TEXT("Enum flag %llx is undeclared in %s, illegal value detected in %s::%s"),
					Flag, *Debug.Print(Id), *Debug.Print(Struct), *Debug.Print(Member));
			}
		}
	}
	else
	{
		...
		if (!bValidated)
		{
			checkf(Algo::FindBy(Declaration.GetEnumerators(), Value, &FEnumerator::Constant),
				TEXT("Enum value %llu is undeclared in %s, illegal value detected in %s::%s"),
				Value, *Debug.Print(Id), *Debug.Print(Struct), *Debug.Print(Member));
		}
	}
}
