...
FBaseline::FBaseline(FDefaultLink* Chain UE_LIFETIMEBOUND, int32 Num)
: Ptr(reinterpret_cast<uint64>(Chain) | TagBit)
{
	check(Num >= 2);
	for (FDefaultLink& Link : MakeArrayView(Chain, Num))
	{
		checkSlow(!(Link.Ptr & TagBit));
	}

	// Tag last link
	Chain[Num - 1].Ptr |= TagBit;
}
...
[[nodiscard]] FBuiltRange* SaveRange(const void* Range, FRangeMemberBinding Member, const FSaveContext& Ctx)
{
	...
	case EMemberKind::Struct:	return SaveStructRange(Range, ItemBinding, Ctx, Member.InnermostId.Get().AsStructBindId());
	}
...
[[nodiscard]] FMemberSchema CreateRangeSchema(FScratchAllocator& Scratch, FRangeMemberBinding Member)
{
	FMemberType* InnerRangeTypes = CreateInnerRangeTypes(Scratch, Member.NumRanges, Member.InnerTypes);
	return { FMemberType(Member.RangeBindings[0].GetSizeType()), ToMemberType(Member.InnerTypes[0]), Member.NumRanges, Member.InnermostId, InnerRangeTypes };
}

void SaveMember(FMemberBuilder& Out, const void* Struct, FMemberId Name, const FSaveContext& Ctx, FLeafMemberBinding Member)
{
...
void SaveMember(FMemberBuilder& Out, const void* Struct, FMemberId Name, const FSaveContext& Ctx, FRangeMemberBinding Member)
{
	Out.AddRange(Name, { CreateRangeSchema(Ctx.Scratch, Member), SaveRange(At(Struct, Member.Offset), Member, Ctx) });
}

void SaveMember(FMemberBuilder& Out, const void* Struct, FMemberId Name, const FSaveContext& Ctx, FStructMemberBinding Member)
{
...
static const FStructDeclaration& SaveStruct(FMemberBuilder& Out, const void* Struct, FBindId Id, const FSaveContext& Ctx)
{
	...
	else
	{
		...
		{
			const FStructDeclaration& SuperDecl = SaveStruct(Out, Struct, It.GrabSuper(), Ctx);
			Out.BuildSuperStruct(Ctx.Scratch, SuperDecl, Ctx.Schemas.GetDebug());
			checkSlow(SuperDecl.Id == ToOptionalStruct(Declaration->Super));
		}
...
{
	FMemberBuilder Out;
	const FStructDeclaration& Declaration = SaveStruct(Out, Struct, Id, Ctx);
...
static bool DiffItem(const uint8* A, const uint8* B, const FSaveContext& Ctx, FRangeMemberBinding Member)
{
	...
	case EMemberKind::Struct:	return DiffItemRange(A, B, ItemBinding, Ctx, Member.InnermostId.Get().AsStructBindId());
	}
...
const FStructDeclaration& SaveStructDelta(FMemberBuilder& Out, const void* Struct, FBaseline Base, FBindId Id, const FSaveContext& Ctx);

static void SaveSchemaBoundStructDelta(FMemberBuilder& Out, const void* Struct, FBaseline Base, const FSchemaBinding& Schema, const FStructDeclaration& Declaration, const FSaveContext& Ctx)
{
	FMemberVisitor It(Schema);
	if (Declaration.Super)
	{
		FBindId SuperId = It.GrabSuper();
		// A schema bound struct may have a custom bound super struct
		check(LowerCast(SuperId) == Declaration.Super);
		const FStructDeclaration& SuperDecl = SaveStructDelta(Out, Struct, Base.Super(), SuperId, Ctx);
		Out.BuildSuperStruct(Ctx.Scratch, SuperDecl, Ctx.Schemas.GetDebug());
	}

	const void* Default = Base.Get();
	if (Declaration.Occupancy == EMemberPresence::AllowSparse && Default)
	{
...
const FStructDeclaration& SaveStructDelta(FMemberBuilder& Out, const void* Struct, FBaseline Base, FBindId Id, const FSaveContext& Ctx)
{
	...
	{
		const void* Default = Base.Get();
		if (!Default || Custom->DiffCustom(Struct, Default, Ctx))
		{
			...
			// It's not important to type-check generate bindings.
		}
	}
	else
	{
		const FSchemaBinding& Schema = Ctx.Schemas.GetStruct(Id, /* out */ Declaration);
		SaveSchemaBoundStructDelta(Out, Struct, Base, Schema, *Declaration, Ctx);
	}
	
	return *Declaration;
}

FBuiltStruct* SaveStructDelta(const void* Struct, FBaseline Base, FBindId Id, const FSaveContext& Ctx)
{
	FMemberBuilder Out;
	const FStructDeclaration& Declaration = SaveStructDelta(Out, Struct, Base, Id, Ctx);
	return Out.BuildAndReset(Ctx.Scratch, Declaration, Ctx.Schemas.GetDebug());
}

FBuiltStruct* SaveStructDeltaIfDiff(const void* Struct, FBaseline Base, FBindId Id, const FSaveContext& Ctx)
{
	FMemberBuilder Out;
	const void* Default = Base.Get();
	...
	{
		const FSchemaBinding& Schema = Ctx.Schemas.GetStruct(Id, /* out */ Declaration);
		if (!Default || Declaration->Occupancy == EMemberPresence::AllowSparse || DiffStructs(Struct, Default, Id, Ctx))
		{
			SaveSchemaBoundStructDelta(Out, Struct, Base, Schema, *Declaration, Ctx);
		}
	}
...
void SaveSuper(FMemberBuilder& Out, const void* Super, FBindId SuperId, const FSaveContext& Ctx)
{
	const FStructDeclaration& Declaration = SaveStruct(Out, Super, SuperId, Ctx);
	Out.BuildSuperStruct(Ctx.Scratch, Declaration, Ctx.Schemas.GetDebug());
}

void SaveSuperDelta(FMemberBuilder& Out, const void* Super, FBaseline SuperBase, FBindId SuperId, const FSaveContext& Ctx)
{
	const FStructDeclaration& Declaration = SaveStructDelta(Out, Super, SuperBase, SuperId, Ctx);
	Out.BuildSuperStruct(Ctx.Scratch, Declaration, Ctx.Schemas.GetDebug());
}
