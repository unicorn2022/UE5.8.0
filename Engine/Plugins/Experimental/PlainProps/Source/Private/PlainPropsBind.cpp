...
FStructMemberBinding FMemberVisitor::GrabStruct()
{
	...
	return { PeekType().AsStruct(), GrabInnerId().AsStructBindId(), GrabMemberOffset() };
}
...
FRangeMemberBinding FMemberVisitor::GrabRange()
{
	...
	FOptionalInnerId InnermostId = HasSchema(InnerTypes.Last()) ? ToOptional(GrabInnerId()) : NoId; 
	uint64 Offset = GrabMemberOffset();
		
	return { &InnerTypes[0], RangeBindings, static_cast<uint16>(InnerTypes.Num()), InnermostId, Offset};
}
...
FInnerId FMemberVisitor::GrabInnerId()
{
...
FRangeMemberBinding GetInnerRange(FRangeMemberBinding In)
{
	...
	return { In.InnerTypes + 1, In.RangeBindings + 1, static_cast<uint16>(In.NumRanges - 1), In.InnermostId };
}
...
{
	for (TSparseSet<FBindId>::TConstIterator It(Keys); It; ++It)
	{
...
static void ValidateStructSpec(const FStructSpec& Spec, const FDebugIds& Ids)
{
#if DO_CHECK
	check(Spec.MemberNames.Num() == Spec.MemberTypes.Num());
	for (int32 Idx = 0, Num = Spec.MemberNames.Num(); Idx < Num; ++Idx)
	{
		checkf(Spec.MemberTypes[Idx], TEXT("The declared member '%s' at member index '%d' in '%s' has not been specified"),
			*Ids.Print(Spec.MemberNames[Idx]), Idx, *Ids.Print(Spec.Id));
	}
#endif
}
...
{
	ValidateStructSpec(Spec, Map.Debug);
...
{
	FType BindType = IndexInnermost(Ids, InnermostType, InnermostId);
...
static uint16 CountInnerIds(TConstArrayView<FMemberBinding> Members)
{
	uint32 Out = 0;
	for (const FMemberBinding& Member : Members)
	{
		Out += !!Member.InnermostId;
	}
...
struct FMemberBinder : FMemberBinderBase
{
	...
	, InnerIdIt(const_cast<FInnerId*>(InSchema.GetInnerIds()))
	{}
	...
	void AddInnerId(FInnerId InnermostId)
	{
...
static void DeclareOrValidate(FStructDeclarationPtr& Out, FStructSpec Info, TConstArrayView<FInnerId> InnerIds, const FSchemaBinding& Binding)
{
	...
	checkSlow(TypeIt + Num == Ptr->GetInnerRangeTypes() && Binding.Members + Num == Binding.GetInnerRangeTypes()); // Loop assumes inner range types follow member types
	for (FMemberBindType BindType : MakeArrayView(Binding.Members, Binding.NumMembers + Binding.NumInnerRanges))
	{
...
FSchemaBinding* FSchemaBindings::Bind(FBindId Id, TConstArrayView<FMemberBinding> Members, FDeclId DeclId) 
{
	...
	FSchemaBinding Header = { DeclId, IntCastChecked<uint16>(Members.Num()), CountInnerIds(Members), CountRanges(Members) };
	...
	for (const FMemberBinding& Member : Members)
	{
		...
		{
			Footer.AddInnerId(Member.InnermostId.Get());
		}
	}
...
TArray<FInnerId> IndexAllRuntimeIds(const FSchemaBatch& Schemas, FIdIndexerBase& Indexer)
{
	const uint8* Base = reinterpret_cast<const uint8*>(&Schemas);
	const uint32* Offsets = Schemas.SchemaOffsets;

	TArray<FInnerId> Out;
	Out.SetNumUninitialized(Schemas.NumSchemas);
	for (uint32 Idx = 0; Idx < Schemas.NumStructSchemas; ++Idx)
	{
		const FStructSchema& Schema = *reinterpret_cast<const FStructSchema*>(Base + Offsets[Idx]);
		Out[Idx] = FInnerId(Indexer.IndexDeclId(Schema.Type));
	}

	for (uint32 Idx = Schemas.NumStructSchemas; Idx < Schemas.NumSchemas; ++Idx)
	{
		const FEnumSchema& Schema = *reinterpret_cast<const FEnumSchema*>(Base + Offsets[Idx]);
		Out[Idx] = FInnerId(Indexer.IndexEnum(Schema.Type));
	}

	return Out;
}

TArray<FStructId> IndexStructRuntimeIds(const FSchemaBatch& Schemas, FIdIndexerBase& Indexer)
{
	const uint8* Base = reinterpret_cast<const uint8*>(&Schemas);
	const uint32* Offsets = Schemas.SchemaOffsets;
