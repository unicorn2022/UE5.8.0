...
struct FEnumerator
{
	...
	friend bool operator==(FEnumerator, FEnumerator) = default;
};

enum class EEnumMode : uint8 { Flat, Flag };
...
struct FEnumDeclaration
{
	...
	TConstArrayView<FEnumerator>	GetEnumerators() const		{ return MakeArrayView(Enumerators, NumEnumerators);	}
};
...
public:
	...
	PLAINPROPS_API const FEnumDeclaration&	Declare(FEnumId Id, FType Type, EEnumMode Mode, ELeafWidth Width, TConstArrayView<FEnumerator> Enumerators, EEnumAliases Policy);
...
struct IDeclarations
{
	...
	PLAINPROPS_API const FEnumDeclaration&		Get(FEnumId Id) const;
	PLAINPROPS_API const FStructDeclaration&	Get(FStructId Id) const;
};
