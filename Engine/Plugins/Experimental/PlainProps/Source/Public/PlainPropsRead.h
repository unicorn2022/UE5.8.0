...
public:
	...
	uint64 CheckableSize() const
	{
#if DO_CHECK
		return static_cast<uint64>(End - It);
#else
		return 0;
#endif
	}
...
struct FLeafView
{
	...
	FORCEINLINE uint8		AsHex8() const		{ return As<uint8,  {ELeafType::Hex, ELeafWidth::B8}>();  }
	FORCEINLINE uint16		AsHex16() const		{ return As<uint16, {ELeafType::Hex, ELeafWidth::B16}>(); }
	FORCEINLINE uint32		AsHex32() const		{ return As<uint32, {ELeafType::Hex, ELeafWidth::B32}>(); }
	FORCEINLINE uint64		AsHex64() const		{ return As<uint64, {ELeafType::Hex, ELeafWidth::B64}>(); }
...
{
public:
	FBoolRangeView(const void* Bits, uint64 Num)
	: Data(static_cast<const uint8*>(Bits))
	, NumBits(Num)
	{}
...
public:
	...
	TRangeView<uint8>		AsHex8s() const		{ return As<uint8,  {ELeafType::Hex, ELeafWidth::B8}>();  }
	TRangeView<uint16>		AsHex16s() const	{ return As<uint16, {ELeafType::Hex, ELeafWidth::B16}>(); }
	TRangeView<uint32>		AsHex32s() const	{ return As<uint32, {ELeafType::Hex, ELeafWidth::B32}>(); }
	TRangeView<uint64>		AsHex64s() const	{ return As<uint64, {ELeafType::Hex, ELeafWidth::B64}>(); }
...
private:
	...
	friend FOptionalEnumSchemaId GetEnumSchemaId(FLeafRangeView Range);
};
...
public:
	...
	PLAINPROPS_API FMemberReader(const FStructSchema& Schema, FByteReader Values, FSchemaBatchId InBatch);

	bool								HasMore() const			{ return MemberIdx < NumMembers; }
	bool								HasSuper() const		{ return UsesSuper && MemberIdx == 0; }		
	
	PLAINPROPS_API FOptionalMemberId	PeekName() const;			// @pre HasMore()
	PLAINPROPS_API FMemberId			PeekNameUnchecked() const;  // @pre HasMore() && !HasSuper()
	...
	PLAINPROPS_API void					SkipMember();				// @pre HasMore()
