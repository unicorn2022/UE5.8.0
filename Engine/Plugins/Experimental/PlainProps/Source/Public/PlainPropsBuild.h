...
public:
	...
	inline T* AllocateArray(uint64 Num, Ts... ConstructorParams)
	{
		T* Out = static_cast<T*>(Allocate(Num * sizeof(T), alignof(T)));
		for (uint64 Idx = 0; Idx < Num; ++Idx)
		{
			new (Out + Idx) T(ConstructorParams...);
		}
		return Out;
	}
	
	template<typename T>
	inline T* CloneArray(uint64 Num, const T* In)
	{
		T* Out = static_cast<T*>(Allocate(Num * sizeof(T), alignof(T)));
		for (uint64 Idx = 0; Idx < Num; ++Idx)
		{
			new (Out + Idx) T(In[Idx]);
		}
		return Out;
	}
...
inline T Normalize(T Value)					{ return Value; }
inline float Normalize(float Value)			{ return Value; } // todo
inline double Normalize(double Value)		{ return Value; } // todo
...
FMemberSchema MakeHexRangeSchema(ESizeType MaxSize)
{
	return { FMemberType(MaxSize), FMemberType(ELeafType::Hex, ReflectArithmetic<T>.Width), 1, NoId, nullptr };
}
...
[[nodiscard]] FTypedRange BuildHexRange(FScratchAllocator& Scratch, TConstArrayView<T, SizeType> Values)
{
	return { MakeHexRangeSchema<T>(RangeSizeOf(SizeType{})), CloneLeaves(Scratch, Values.Num(), Values.GetData(), sizeof(T)) };
}

template<UnsignedIntegral T>
[[nodiscard]] FTypedRange BuildHexRange(FScratchAllocator& Scratch, ESizeType SizeType, TConstArrayView<T> Values)
{
	return { MakeHexRangeSchema<T>(SizeType), CloneLeaves(Scratch, Values.Num(), Values.GetData(), sizeof(T)) };
}
...
{
	return Scratch.Allocate(Size, MIN_ALIGNMENT);
}
