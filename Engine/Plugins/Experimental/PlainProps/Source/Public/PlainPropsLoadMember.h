...
public:
	...
	bool							HasSuper() const			{ return Reader.HasSuper(); }
	
	FOptionalMemberId				PeekName() const			{ return Reader.PeekName(); }
	FMemberId						PeekNameUnchecked() const	{ return Reader.PeekNameUnchecked(); }
...
// Grab and load super if it exists, call before grabbing from Src
inline void	TryLoadSuper(void* Dst, /* in-out */ FMemberLoader& Src)
{
	if (Src.HasSuper())
	{
