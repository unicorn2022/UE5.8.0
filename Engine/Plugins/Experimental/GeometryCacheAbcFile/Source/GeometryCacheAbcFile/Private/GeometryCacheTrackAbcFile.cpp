...
bool UGeometryCacheTrackAbcFile::SetSourceFile(const FString& FilePath, UAbcImportSettings* AbcSettings, float InitialTime, bool bIsLooping)
{
	...
	if (!FilePath.IsEmpty())
	{
		...
		if (Result != EAbcImportError::AbcImportError_NoError)
		{
			...
			UE_LOGF(LogGeometryCacheAbcFile, Warning, "Failed to open %ls: %ls", *Filename, *FailureMessage.ToString());
		...
		if (Result != EAbcImportError::AbcImportError_NoError)
		{
			...
			UE_LOGF(LogGeometryCacheAbcFile, Warning, "Failed to load %ls: %ls", *Filename, *FailureMessage.ToString());
