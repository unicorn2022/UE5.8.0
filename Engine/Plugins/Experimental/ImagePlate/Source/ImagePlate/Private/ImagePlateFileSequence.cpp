...
FImagePlateAsyncCache UImagePlateFileSequence::GetAsyncCache()
{
	...
	{
		UE_LOGF(LogImagePlateFileSequence, Warning, "Sequence path is not a long package name. This path is not portable, and may not work in a packaged build.");
...
bool FImagePlateSourceFrame::EnsureTextureMetrics(UTexture* DestinationTexture) const
{
	...
	{
		UE_LOGF(LogImagePlateFileSequence, Warning, "Unsupported source image bitdepth: %u. Only float 16 and fixed 8 bitdepths are supported", BitDepth);
		return false;
	}
	...
	else if (DestinationTexture->GetResource())
	{
		...
		if (!Texture2DRHI)
		{
			UE_LOGF(LogImagePlateFileSequence, Warning, "Unsupported texture type encountered: Unable to update texture to fit source frame size or bitdepth.");
			return false;
		}
		else
		{
			...
			if (DestBitDepth != BitDepth)
			{
				UE_LOGF(LogImagePlateFileSequence, Warning, "Invalid destination texture bitdepth. Expected %u, encountered %u.", BitDepth, DestBitDepth);
				return false;
			}
		}
	}
...
TFuture<void> FImagePlateSourceFrame::CopyTo(UTexture* DestinationTexture)
{
	...
	ENQUEUE_RENDER_COMMAND(CopySourceBufferToTexture)(
		[CommandData](FRHICommandListImmediate& RHICmdList)
		{
			...
			uint32 DestPitch = 0;
			uint8* DestinationBuffer = (uint8*)RHICmdList.LockTexture2D(Texture2DRHI, 0, RLM_WriteOnly, DestPitch, false);
			if (!DestinationBuffer)
			{
				UE_LOGF(LogImagePlateFileSequence, Warning, "Unable to lock texture for write");
				return;
			}
...
namespace ImagePlateFrameCache
{
		...
		: CurrentFrameNumber(-1), MinCacheRange(-1), MaxCacheRange(-1), Framerate(InFramerate)
	{
		...
		{
			UE_LOGF(LogImagePlateFileSequence, Warning, "The directory '%ls' does not contain any image files that match the wildcard '%ls'", *SequenceFolder, *InWildcard);
		}
		else
		{
			UE_LOGF(LogImagePlateFileSequence, Verbose, "Found %i image files in '%ls' with the wildcard '%ls'", FrameFilenames.Num(), *SequenceFolder, *InWildcard);
			FrameFilenames.Sort();
		}
	...
	FImagePlateSourceFrame LoadFileData(FString FilenameToLoad, int32 FrameNumber)
	{
		...
		{
			UE_LOGF(LogImagePlateFileSequence, Warning, "Failed to load file data from '%ls'", *FilenameToLoad);
