...
UFunction* UK2Node_WebAPIOperation::GetFactoryFunction() const
{
	if (OperationClass == nullptr)
	{
		UE_LOGF(LogWebAPIBlueprintGraph, Error, "OperationClass null in %ls. Was a class deleted or saved on a non promoted build?", *GetFullName());
		return nullptr;
	}
	...
	if (FactoryFunction == nullptr)
	{
		...
		UE_CLOGF(FactoryFunction == nullptr, LogWebAPIBlueprintGraph, Error, "FactoryFunction %ls null in %ls. Was a class deleted or saved on a non promoted build?", *ProxyFunctionName.ToString(), *GetFullName());
	}
