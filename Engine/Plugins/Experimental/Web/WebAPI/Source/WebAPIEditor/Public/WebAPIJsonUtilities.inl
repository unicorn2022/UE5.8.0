...
namespace UE::Json
{
		...
		const TMap<FString, EnumType>& InNameToValue)
	{
		...
		if(InJsonValue->Type != EJson::Boolean)
		{
			...
			else
			{
				...
				{
					// UE_LOGF(LogWebAPIEditor, Error, "Tried to convert enum from string, but no lookup was provided.");
					return bResult;
				}
	...
	TryGet(const TSharedPtr<FJsonValue>& InJsonValue, TMap<KeyType, ValueType>& OutValues)
	{
		...
		if(InJsonValue->TryGetObject(JsonObject))
		{
			...
			for(TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue, ESPMode::ThreadSafe>>& FieldValuePair : JsonObject->Get()->Values)
			{
