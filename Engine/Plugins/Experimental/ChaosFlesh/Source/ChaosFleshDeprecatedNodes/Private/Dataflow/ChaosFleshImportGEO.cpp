...
FImportGEO::ReadGEOFile(const bool PrintStats) const
{
	...
	{
		UE_LOGF(LogImportGEO, Error, "'%ls' cannot find GEO file: '%ls'",
	...
	if (!ChaosFlesh::ReadGEO(filestr, IntVars, IntVectorVars, FloatVectorVars, IndexedStringVars, &out))
	{
		...
		UE_LOGF(LogImportGEO, Error, "'%ls' - Failed to read GEO file '%ls'; log:\n%ls",
...
*/
		UE_LOGF(LogImportGEO, Log, "%ls", *OutStr);
	}
	return true;
#else // WITH_EDITOR
	UE_LOGF(LogImportGEO, Error, "ImportGEO node '%ls' is not available on this platform.", *GetName().ToString());
	return false;
#endif // WITH_EDITOR
}
	...
	const FDataflowOutput* Out) const
{
	if (Out->IsA<FManagedArrayCollection>(&Collection))
	{
		...
		if (bImportTetrahedronMesh)
		{
			...
			UE_LOGF(LogImportGEO, Display, "Got %d points.", Vertices.Num());
			...
			if (IvIt != nullptr)
			{
				...
				UE_LOGF(LogImportGEO, Display, "Tet start index: %d num tets: %d", StartIndex, NumTets);
				...
				UE_LOGF(LogImportGEO, Display, "Got %d tets.", Elements.Num());
			}
			...
			if (Vertices.Num() && Elements.Num())
			{
				...
				UE_LOGF(LogImportGEO, Log,
					"'%ls' - Authored tetrahedral mesh of %d vertices and %d tetrahedra",
