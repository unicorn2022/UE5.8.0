...
TOptional<TArray<int32>> UDeformableTetrahedralComponent::GetMeshImportVertexMap(const USkinnedAsset& SkinnedMeshAsset) const
{
	...
	if (!MLDModel)
	{
		UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning, "MLDModel is empty.");
		return None;
	}
	if (!MLDModel->LODModels.IsValidIndex(LODIndex))
	{
		UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning, "LODIndex %d is not a valid index.", LODIndex);
		return None;
	}
	...
	if (Map.IsEmpty())
	{
		UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning, "MeshToImportVertexMap is empty. MLDeformer Asset should be an imported SkeletalMesh (e.g. from fbx).");
		return None;
	}
	...
	if (SkeletalMeshAsset && RestCollection && RestVertices)
	{
		...
		for (int32 SectionIndex = 0; SectionIndex < NumSections; ++SectionIndex)
		{
			...
			{
				UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning, "SkeletalMeshAsset and FleshAsset have different number of vertices in section %d. Check if the assets have the same mesh.", SectionIndex);
				return None;
			}
		...
		if (NumOffVertices > 0)
		{
			UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning, 
				"%d vertices bound in FleshAsset [%ls] have different vertex positions than SkeletalMeshAsset [%ls]. Check if the assets have the same vertex order or some points are not embedded."
	...
	TArray<bool>* OutInfluence) const
{
	...
	{
		UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning,
			"'%ls' - GetSkeletalMeshEmbeddedPositionsInternal - RestCollection is not set.",
			*GetName());
	...
	{
		UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning,
			"'%ls' - GetSkeletalMeshEmbeddedPositionsInternal - TargetDeformationSkeleton is not set on the flesh asset.",
			*GetName());
		return TArray<FVector>();
	}
	...
	else
	{
		...
		if (TargetDeformationSkeletalMeshComponent)
		{
			...
			{
				UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning, 
					"'%ls' - Failed to find a valid bone index (got %d) for bone name '%ls' in TargetDeformationSkeleton '%ls' "
					"corresponding to SkeletalMeshComponent '%ls', which has %d bones.", 
					*GetName(),
		...
		{
			UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning, 
				"'%ls' - Failed to find SkeletalMeshComponent for TargetDeformationSkeleton '%ls'.",
				*GetName(),
			...
			if (ensure(Verts != nullptr) && TetrahedronStart)
			{
				auto CalculateBindings = [this, TetBindings, TetrahedronStart, SkeletalMeshName, OutInfluence](const TManagedArray<FVector3f>* Verts, TArray<FVector>& OutPositions)
				{
					...
					for (int32 TetMeshIdx = 0; TetMeshIdx < TetrahedronStart->Num(); TetMeshIdx++)
					{
						...
						{
							UE_LOGF(LogDeformableTetrahedralComponentInternal, Warning,
								   "'%ls' - GetEmbeddedPositionsInternal - Failed to find mesh bindings for skeletal mesh '%ls'",
								   *GetName(),
