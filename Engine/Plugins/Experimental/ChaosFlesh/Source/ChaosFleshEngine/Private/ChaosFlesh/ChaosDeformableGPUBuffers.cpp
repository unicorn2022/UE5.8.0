...
void FFloatArrayBufferWithSRV::InitRHI(FRHICommandListBase& RHICmdList)
{
    if (Array.Num() > 0)
    {
		...
		UE_LOGF(LogFleshDeformerBuffers, Log,
			"FFloatArrayBufferWithSRV - '%ls' - Buffer size: %zu, Array num: %d",
...
void FHalfArrayBufferWithSRV::InitRHI(FRHICommandListBase& RHICmdList)
{
    if (Array.Num() > 0)
    {
		...
		UE_LOGF(LogFleshDeformerBuffers, Log,
			"FHalfArrayBufferWithSRV - '%ls' - Buffer size: %d, Array num: %d",
...
void FIndexArrayBufferWithSRV::InitRHI(FRHICommandListBase& RHICmdList)
{
    ...
    if (BufferSize)
    {
		...
		if (Local::CVarParam.bTestUnpacking)
		{
			...
			UE_LOGF(LogFleshDeformerBuffers, Log,
				"FIndexArrayBufferWithSRV - TEST_UNPACKING succeeded for %d values, packed into buffer size %d, %d bits per value.",
		...
		UE_LOGF(LogFleshDeformerBuffers, Log,
			"FIndexArrayBufferWithSRV - '%ls' - Data stride: %d, Buffer size: %d, Input array size: %d",
