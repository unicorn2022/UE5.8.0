...
namespace Chaos
{
			...
			if (FEvolution* Evolution = PhysicsThreadAccess.GetEvolution())
			{
				...
				//We always write a cache even if there are no particles
				{
					...
					if (PrimitiveStage.MonolithStage)
					{
						...
						{
							UE_LOGF(LogChaosFleshCache, Error,
								"Failed to write points '%ls' at time %g to file: '%ls'",
						...
						{
							UE_LOGF(LogChaosFleshCache, Error,
								"Failed to write activations '%ls' at time %g to file: '%ls'",
								*PrimitiveStage.PrimPath, InTime, *PrimitiveStage.MonolithStage.GetRootLayer().GetDisplayName());
							return;
						}
		...
		const TFunction<void(const int32, const int32, const FVector3f&, const FVector3f&, const float Activation)>& LoadFunction) const
	{
		...
		if (PrimitiveStage.MonolithStage)
		{
			...
			{
				UE_LOGF(LogChaosFleshCache, Error,
					"Inconsistent bracketing time samples for attributes '%ls' and '%ls' at frame %g from file: '%ls'",
			...
			{
				UE_LOGF(LogChaosFleshCache, Error,
					"Failed to read points '%ls' at time %g from file: '%ls'",
	...
	void FFleshCacheAdapter::InitializeForLoad(UPrimitiveComponent* InComponent, FObservedComponent& InObserved)
	{
		...
		if(const UFleshComponent* FleshComp = CastChecked<UFleshComponent>(InComponent))
		{
			...
			if (!PrimitiveStage.MonolithStage)
			{
				...
				{
					UE_LOGF(LogChaosFleshCache, Warning, "Read Failure: USD File Path = %ls", *PrimitiveStage.FilePath);
				}
			}
		}
#endif // USE_USD_SDK && DO_USD_CACHING
		...
		if( FDeformableSolver* Solver = GetDeformableSolver(InComponent))
		{
			...
			if (UFleshComponent* FleshComp = CastChecked<UFleshComponent>(InComponent))
			{
				...
				if (!PlatformFile.DirectoryExists(*CacheDir))
				{
					...
					{
						UE_LOGF(LogChaosFleshCache, Error, "Failed to create output directory: '%ls'", *CacheDir);
						return false;
					}
				}
				...
				if (!RestCollection)
				{
					UE_LOGF(LogChaosFleshCache, Error, "Failed to get rest collection from flesh component: '%ls'", *FleshComp->GetName());
					return false;
				}
				...
				if (bUseMonolith)
				{
					...
					if (CVarParams.bNoClobber)
					{
						if (PlatformFile.FileExists(*PrimitiveStage.FilePath))
						{
							...
							if (bRestartSimulation)
							{
								...
								{
									UE_LOGF(LogChaosFleshCache, Error, "Failed to copy file from '%ls' to '%ls'.", *PrimitiveStage.FilePath, *UniqueFilePath);
									return false;
								}
							}
							else
							{
								...
								{
									UE_LOGF(LogChaosFleshCache, Error, "Failed to rename file from '%ls' to '%ls'.", *PrimitiveStage.FilePath, *UniqueFilePath);
									return false;
								}
							}
						}
					...
					else
					{
						...
						{
							UE_LOGF(LogChaosFleshCache, Error, "Failed to remove existing cache file: '%ls'", *PrimitiveStage.FilePath);
							return false;
						}
					}
					...
					if (bRestartSimulation)
					{
						...
						{
							UE_LOGF(LogChaosFleshCache, Error, "Failed to read restart USD file: '%ls'", *PrimitiveStage.FilePath);
							return false;
						}
						...
						{
							UE_LOGF(LogChaosFleshCache, Error, "Failed to delete USD data in file: '%ls', delete range: [%f, %f]", *PrimitiveStage.FilePath, RestartTimeStart, RestartTimeEnd);
							return false;
						}
					}
					else
					{
						...
						{
							UE_LOGF(LogChaosFleshCache, Error, "Failed to create new USD file: '%ls'", *PrimitiveStage.FilePath);
							return false;
						}
						if (!UE::ChaosCachingUSD::WriteTetMesh(PrimitiveStage.MonolithStage, PrimitiveStage.PrimPath, *RestCollection))
						{
							UE_LOGF(LogChaosFleshCache, Error, "Failed to write tetrahedron mesh '%ls' to USD file: '%ls'", *PrimitiveStage.PrimPath, *PrimitiveStage.FilePath);
							return false;
						}
					}
				}
			...
			if (UDeformableTetrahedralComponent* FleshComp = CastChecked<UDeformableTetrahedralComponent>(InComponent))
			{
				...
				if (bUseMonolith)
				{
					...
					if (PlatformFile.FileExists(*PrimitiveStage.FilePath))
					{
						...
						if (!UE::ChaosCachingUSD::OpenStage(PrimitiveStage.FilePath, PrimitiveStage.MonolithStage))
						{
							UE_LOGF(LogChaosFleshCache, Error, "Failed to open USD cache file: '%ls'", *PrimitiveStage.FilePath);
							return false;
						}
					}
					else
					{
						UE_LOGF(LogChaosFleshCache, Error, "USD cache file not found: '%ls'", *PrimitiveStage.FilePath);
						return false;
					}
				}
				if (!bUseMonolith)
				{
					UE_LOGF(LogChaosFleshCache, Warning, "No USD file structure selected (bMonolith).");
				}
#endif // USE_USD_SDK && DO_USD_CACHING
