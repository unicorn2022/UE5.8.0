...
UDIFleshDeformer::CreateDataProvider(TObjectPtr<UObject> InBinding, uint64 InInputMask, uint64 InOutputMask) const
{
	...
	if ((Provider->SkinnedMesh = Cast<USkinnedMeshComponent>(InBinding)))
	{
				...
				if (UDeformableTetrahedralComponent* FleshComponent = const_cast<UDeformableTetrahedralComponent*>(Cast<UDeformableTetrahedralComponent>(Component)))
				{
					if (const UFleshAsset* FleshAsset = FleshComponent->GetRestCollection())
					{
						...
						else if (!FleshAsset->TargetDeformationSkeleton && NumFleshComponents == 1)
						{
							...
							UE_LOGF(LogFleshDeformer, Display,
								"FleshAsset '%ls.%ls.RestCollection' has no value for 'TargetDeformationSkeleton', "
									"using it for skeletal mesh '%ls' anyway as it's the only UFleshComponent available.",
								*Actor->GetName(),
								*FleshAsset->GetName(),
						...
						{
							UE_LOGF(LogFleshDeformer, Warning,
								"FleshAsset '%ls.%ls.RestCollection' has no value for 'TargetDeformationSkeleton'; "
									"not using it for skeletal mesh '%ls' as there are %d UFleshComponent candidates.",
								*Actor->GetName(),
								*FleshAsset->GetName(),
								*Provider->SkinnedMesh->GetName(),
								NumFleshComponents);
		...
		if (!Provider->FleshMesh)
		{
			...
			{
				UE_LOGF(LogFleshDeformer, Error,
					"Found %d UFleshComponents under actor '%ls', none of which were associated with skeletal mesh '%ls'. "
			...
			{
				UE_LOGF(LogFleshDeformer, Error,
					"No UFleshComponent found under actor '%ls' for skeletal mesh '%ls'.",
					*Actor->GetName(),
					*Provider->SkinnedMesh->GetName());
			}
		}
...
{
	if (SkinnedMeshComponentIn && SkinnedMeshComponentIn->IsRegistered())
	{
		SceneInterface = SkinnedMeshComponentIn->GetScene();
		ComponentId = SkinnedMeshComponentIn->GetPrimitiveSceneId();
	}
...
FDIFleshDeformerProviderProxy::IsValid(FValidationData const& InValidationData) const
{
	...
	if (SkinnedMeshComponent == nullptr || !SkeletalMeshObject || FleshComponent == nullptr)
	{
		return false;
	}
	...
	if (!FleshComponent->GetGPUBufferManager().InitGPUBindingsBuffer(SkinnedMeshComponent, MeshName, SkeletalMeshObject->GetLOD()))
	{
		return false;
	}
...
FDIFleshDeformerProviderProxy::GatherDispatchData(FDispatchData const& InDispatchData)
{
	...
	FSkeletalMeshLODRenderData const* LodRenderData = &SkeletalMeshRenderData.LODRenderData[SkeletalMeshObject->GetLOD()];
	const TStridedView<FParameters> ParameterArray = MakeStridedParameterView<FParameters>(InDispatchData);

	FName MeshName(*MeshId, MeshId.Len());
	Chaos::Softs::FChaosFleshDeformableGPUManager& GPUManager = FleshComponent->GetGPUBufferManager();
	Chaos::Softs::FChaosFleshDeformableGPUManager::FBindingsBuffer* Bindings = GPUManager.GetBindingsBuffer(SkinnedMeshComponent, MeshName, SkeletalMeshObject->GetLOD());
	if (!Bindings)
	{
		return;
	}
	...
	for (int32 InvocationIndex = 0; InvocationIndex < ParameterArray.Num(); ++InvocationIndex)
	{
		...
		//SHADER_PARAMETER(uint32, NumMaskBuffer)
		//SHADER_PARAMETER_SRV(Buffer<FFloat16>, MaskBuffer)
		Parameters.NumMaskBuffer = Bindings->GetMaskBuffer().GetNumValues();
