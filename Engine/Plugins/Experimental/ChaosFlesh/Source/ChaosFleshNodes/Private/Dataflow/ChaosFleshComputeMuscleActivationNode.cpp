	...
	const USkeletalMesh& InSkeletalMesh, TArray<float>& MinLengthRatio)
{
	...
	if (!Vertex)
	{
		UE_LOGF(LogChaosFlesh, Error, "FindLowestMuscleLengthRatio: No Vertex attribute in the Collection.");
		return false;
	}
	...
	{
		UE_LOGF(LogChaosFlesh, Error, "FindLowestMuscleLengthRatio: TransformSource group in the Collection is not valid.");
		return false;
	}
	TSet<int32> Roots;
	...
	if (Skeleton)
	{
		...
		if (Roots.IsEmpty())
		{
			UE_LOGF(LogChaosFlesh, Error, "FindLowestMuscleLengthRatio: The root in the TransformSource group is incompatible with the SkeletalMesh.");
			return false;
		}
		else
		{
	...
	{
		UE_LOGF(LogChaosFlesh, Error, "FindLowestMuscleLengthRatio: There's no skeleton in the SkeletalMesh.");
		return false;
	}
	...
	{
		UE_LOGF(LogChaosFlesh, Error, "FindLowestMuscleLengthRatio: No kinematics information in the Collection.");
		return false;
	}
	const FMuscleActivationFacade MuscleActivation(InCollection);
	if (!MuscleActivation.IsValid())
	{
		UE_LOGF(LogChaosFlesh, Error, "FindLowestMuscleLengthRatio: No muscle activation information in the Collection.");
		return false;
	}
						...
						for (int32 Idx = 0; Idx < ParameterArray.Num(); ++Idx)
						{
							...
							if (MinLengthRatio.IsValidIndex(MuscleIndex))
							{
								...
								else
								{
									...
									{
										UE_LOGF(LogChaosFlesh, Error,
											"SetMuscleActivationParameter::ImportLowestMuscleLengthRatio: Muscle [%ls] index [%d] has minimum origin-insertion length ratio %.2f (scaled to %.2f) across the whole animation.",
											*MuscleName, MuscleIndex, MinLengthRatio[MuscleIndex], ScaledRatio);
									}
									else
									{
										UE_LOGF(LogChaosFlesh, Warning,
											"SetMuscleActivationParameter::ImportLowestMuscleLengthRatio: Muscle [%ls] index [%d] has minimum origin-insertion length ratio %.2f across the whole animation.",
							...
							{
								UE_LOGF(LogChaosFlesh, Error,
									"SetMuscleActivationParameter::ImportLowestMuscleLengthRatio: Geometry [%ls] is not a valid muscle.",
									*MuscleName);
	...
	if (Out->IsA(&Collection) || Out->IsA(&GeometrySelection))
	{
		...
		if (MuscleActivationFacade.IsValid())
		{
			for (const FCurveMuscleName& CurveMuscleName : CurveMuscleNameArray)
			{
				...
				{
					UE_LOGF(LogChaosFlesh, Error,
						"ReadSkeletalMeshCurves: Geometry %ls (connecting to curve %ls) is not an active muscle.",
