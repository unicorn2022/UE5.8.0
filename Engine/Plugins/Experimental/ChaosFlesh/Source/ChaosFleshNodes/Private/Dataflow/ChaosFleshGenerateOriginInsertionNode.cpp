	...
	if (Out->IsA<FManagedArrayCollection>(&Collection))
	{
		...
		if (!Elements)
		{
			UE_LOGF(LogChaosFlesh, Warning,
				"GenerateOriginInsertionNode: Failed to find geometry collection attr '%ls' in group '%ls'",
		...
		{
			UE_LOGF(LogChaosFlesh, Warning,
				"GenerateOriginInsertionNode: Failed to find geometry collection attr 'Vertex' in group 'Vertices'");
			Out->SetValue(MoveTemp(InCollection), Context);
			return;
		}
