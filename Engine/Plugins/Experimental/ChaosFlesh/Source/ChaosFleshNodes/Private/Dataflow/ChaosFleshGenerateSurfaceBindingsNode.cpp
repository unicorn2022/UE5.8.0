...
FGenerateSurfaceBindings::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	...
	if (Out->IsA(&Collection) || Out->IsA(&SKMDynamicMesh))
	{
			...
			Vertex)
		{
			...
			if (IsConnected(&TransformSelection))
			{
				...
				{
					UE_LOGF(LogMeshBindings, Error,
						"TransformSelection size: %d is different from Collection Transform group size: %d",
			...
			{
				UE_LOGF(LogMeshBindings, Error,
					"GeometryGroupGuids contains %d guids but none was matched (empty guids are ignored).",
			...
			for (int32 LOD = 0; LOD < RenderMeshVertices.Num(); LOD++)
			{
					...
					for (; j < TetIntersections.Num(); j++)
					{
						...
						if (!Tets[TetIdx].Outside(Pos, 0.f)) // includes boundary
						{
							...
							else
							{
								...
								{
									UE_LOGF(LogMeshBindings, Error,
										"Vertex position does not collocate with interpolated position, for example LOD %d, SKM vertex %d, tetrahedron %d, distance = %.4f)",
										LOD,
										i,
										TetIdx,
					...
					if (j == TetIntersections.Num())
					{
						...
						if (bDoSurfaceProjection)
						{
								...
								Result))
							{
								if (Result.Num() > 0)
								{
									const Chaos::TTriangleCollisionPoint<Chaos::FRealDouble>& CollisionPoint = Result[0];
				...
				{
					UE_LOGF(LogMeshBindings, Error,
						"'%ls' - Generated mesh bindings between tet mesh and %ls mesh of '%ls' LOD %d - stats:\n"
				...
				{
					UE_LOGF(LogMeshBindings, Display,
						"'%ls' - Generated mesh bindings between tet mesh and %ls mesh of '%ls' LOD %d - stats:\n"
							"    Render vertices num: %d\n"
							"    Vertices in tetrahedra: %d\n"
							"    Vertices bound to tet surface: %d\n"
				...
				{
					UE_LOGF(LogMeshBindings, Error,
						"%d vertex positions do not collocate with interpolated position for LOD %d",
						NumTetNotCollocated,
						LOD);
				}
