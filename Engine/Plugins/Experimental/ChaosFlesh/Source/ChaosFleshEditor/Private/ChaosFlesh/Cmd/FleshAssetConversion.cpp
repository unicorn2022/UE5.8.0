...
TUniquePtr<FFleshCollection> ReadTetPBDeformableGeometryCollection(const FString& Filename)
	{
		...
		UE_LOGF(LogChaosFlesh, Display, "Reading Path %ls", *Filename);
		...
		for (auto* It : TetMeshes)
		{
			...
			Vertices = &It->Points;
			UE_LOGF(LogChaosFlesh, Display, "Got %d tets.", Elements->Num());
			UE_LOGF(LogChaosFlesh, Display, "Got %d rest points.", Vertices->Num());
			break;
		}
		...
		if (!Vertices || Vertices->Num() == 0)
		{
			...
			UE_LOGF(LogChaosFlesh, Display, "Got %d points from frame %d.", Vertices->Num(), FrameRange.Key);
		}

		GetSurfaceElements(*Elements, SurfaceElements, false);
		UE_LOGF(LogChaosFlesh, Display, "Got %d tris.", SurfaceElements.Num());
	...
	TUniquePtr<FFleshCollection> ReadGEOFile(const FString& Filename)
	{
		...
		{
			UE_LOGF(LogChaosFlesh, Display, "Failed to open GEO file: '%ls'.", *Filename);
			return Collection;
		}
		...
		UE_LOGF(LogChaosFlesh, Display, "Got %d points.", Vertices.Num());
		...
		if (ivIt != nullptr)
		{
			...
			UE_LOGF(LogChaosFlesh, Display, "Tet start index: %d num tets: %d", startIndex, numTets);
...
TUniquePtr<FFleshCollection> FFleshAssetConversion::ImportTetFromFile(const FString& Filename)
{
	...
	if (FPaths::FileExists(Filename))
	{
		...
		UE_LOGF(LogChaosFlesh, Warning, "Unsupported file type: '%ls'.", *Filename);
	}
	else
	{
		UE_LOGF(LogChaosFlesh, Warning, "Unknown file path: '%ls'.", *Filename);
	}
	*/
	return nullptr;
}
