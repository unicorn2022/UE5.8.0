...
// Convert a TArray<FShere> to a dynamic mesh
USTRUCT(meta = (DataflowGeometryCollection))
struct FSpheresToMeshDataflowNode : public FDataflowNode
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FSpheresToMeshDataflowNode, "SpheresToMesh", "GeometryCollection|Mesh", "")

private:
	// Array of spheres to convert to a mesh/meshes
	UPROPERTY(meta = (DataflowInput, DataflowIntrinsic))
	TArray<FSphere> Spheres;

	// Number of vertices to use along each axis when creating a mesh for each sphere
	UPROPERTY(EditAnywhere, Category = Options, meta = (ClampMin = 2))
	int32 VerticesAlongEachSide = 4;

	/** Single mesh aggregating all the spheres together */
	UPROPERTY(meta = (DataflowOutput, DataflowRenderGroups = ""))
	TObjectPtr<UDynamicMesh> Mesh;

	/** Array of meshes for each spheres found */
	UPROPERTY(meta = (DataflowOutput))
	TArray<TObjectPtr<UDynamicMesh>> Meshes;
...
public:
	FSpheresToMeshDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid());
