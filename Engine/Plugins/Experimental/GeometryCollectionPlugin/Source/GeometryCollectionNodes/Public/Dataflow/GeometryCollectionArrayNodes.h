...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FDataflowMakeArrayNode, "MakeArray", "Utilities|Array", "Element")

public:
	FDataflowMakeArrayNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid());
...
private:
	...
	/** Elements to convert to an array */
	UPROPERTY()
	TArray<FDataflowAllTypes> Elements;
	...
	virtual bool CanRemovePin() const override { return Elements.Num() > NumInitialElements; }
	...
	UE::Dataflow::TConnectionReference<FDataflowAllTypes> GetConnectionReference(int32 Index) const;

	static constexpr int32 NumRequiredInputs = 0;
	static constexpr int32 NumInitialElements = 2; 

	inline static FName ArrayElementDependencyGroup = FName("ArrayElement");
};
