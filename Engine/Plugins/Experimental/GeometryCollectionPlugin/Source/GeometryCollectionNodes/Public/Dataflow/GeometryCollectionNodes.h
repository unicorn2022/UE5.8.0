...
/**
 * Get a collection from a geometry collecion asset
 * DEPRECATED 5.7 : use GeometryCollectionToCollection node instead
 */
USTRUCT(meta = (DataflowGeometryCollection, Deprecated = "5.7"))
 ...
 *
 * Description for this node
 * DEPRECATED 5.5 - use Print node ( core nodes ) 
 */
 ...
 * DEPRECATED 5.8 - use CollectionBounds node instead
 */
...
/**
 * Get the bounding box a collection and compute its box ratio from a reference size
 * The reference size is the length of a perfect cube representing a reference volume
 * 
 */
USTRUCT(meta = (DataflowGeometryCollection))
...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FBoundingBoxRatioDataflowNode, "GetCollectionBoundingBoxRatio", "Utilities|Box", "Collection Reference Resolution Bounds Size Dimensions Extents Center")

public:
	FBoundingBoxRatioDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid());

private:
	/** Collection to compute the bounding box and ratio from */
	UPROPERTY(meta = (DataflowInput))
	FManagedArrayCollection Collection;

	/** Reference box size to use to compute the ratio */
	UPROPERTY(EditAnywhere, Category = "Proximity", meta = (DataflowInput, ClampMin = "0", UIMin = "0", Units = cm))
	float ReferenceBoxSize = 100;
...
/**
 * Compute the boudning sphere of the geometrry of a collection 
 */
USTRUCT(meta = (DataflowGeometryCollection))
...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FBoundingBoxAverageExtentsDataflowNode, "GetBoxAverageExtents", "Utilities|Box", "BoundingBox size extent ratio length min max")

public:
	FBoundingBoxAverageExtentsDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid());

private:
	/** Bounding box to get the information from */
	UPROPERTY(meta = (DataflowInput))
	FBox BoundingBox = FBox(ForceInit);;

	/** Average of the 3 axis extents */
	UPROPERTY(meta = (DataflowOutput))
	float AverageExtents = 0.f;

	/** Average of the 3 axis half extents */
	UPROPERTY(meta = (DataflowOutput))
	float AverageHalfExtents = 0.f;

	/** Minimum edge length of the box */
	UPROPERTY(meta = (DataflowOutput))
	float MinLength = 0.f;

	/** Maximum edge length of the box */
	UPROPERTY(meta = (DataflowOutput))
	float MaxLength = 0.f;

	/** Ratio of the min edge length over the max edge length */
	UPROPERTY(meta = (DataflowOutput))
...
public:
	...
	/** Minimum corner ofthe bounding box */
	UPROPERTY(meta = (DataflowOutput))
	FVector Min = FVector(0.0);

	/** Maximum corner ofthe bounding box */
	UPROPERTY(meta = (DataflowOutput))
	...
	/** Distances from the center to any corner along the three axis */
	UPROPERTY(meta = (DataflowOutput))
...
public:
	UPROPERTY(meta = (DataflowInput, GizmoType = Translate))
...
public:
	...
	/** Translation */
	UPROPERTY(EditAnywhere, Category = "Transform", meta = (DataflowInput, GizmoType = Translate));
	...
	/** Rotation */
	UPROPERTY(EditAnywhere, Category = "Transform", meta = (DataflowInput, GizmoType = Rotate, GizmoPosition = Translate));
	FVector Rotate = FVector(0.0);

	/** Scale */
	UPROPERTY(EditAnywhere, Category = "Transform", meta = (UIMin = 0.001f), meta = (DataflowInput, GizmoType = Scale, GizmoPosition = Translate));
...
/**
 * Transforms a mesh
 * DEPRECATED 5.8 - use FTransformMeshDataflowNode_v2 instead
 */
...
public:
	...
	/** Scale */
	UPROPERTY(EditAnywhere, Category = "Transform", meta = (UIMin = 0.001f, DataflowInput, GizmoType = Scale, GizmoPosition = Translate));
	FVector Scale = FVector(1.0);
 ...
 *
 */
USTRUCT(meta = (DataflowGeometryCollection, Icon = "FractureEditor.Proximity"))
 ...
 */
USTRUCT(meta = (DataflowGeometryCollection))
struct FCollectionInfoDataflowNode : public FDataflowNode
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FCollectionInfoDataflowNode, "CollectionInfo", "GeometryCollection|Utilities", "")
...
public:
	FCollectionInfoDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid())
		: FDataflowNode(InParam, InGuid)
	{
		RegisterInputConnection(&Collection);
		RegisterOutputConnection(&Collection, &Collection);
		RegisterOutputConnection(&MaxLevel);
	}
...
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FTransformMeshDataflowNode_v2, "TransformMesh", "Math|Transform", "")
...
public:
	FTransformMeshDataflowNode_v2(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid());
};
