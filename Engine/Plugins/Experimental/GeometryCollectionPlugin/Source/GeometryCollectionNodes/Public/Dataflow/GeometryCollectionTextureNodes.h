...
private:
	...
	/** Whether to pad out gutter size so that all empty texels are filled */
	UPROPERTY(EditAnywhere, Category = ChannelOptions, meta = (DataflowInput))
	bool bFullyPadGutter = false;

	/** Approximate space to fill in between UV islands, measured in texels. Ignored if Fully Pad Gutter is enabled. */
	UPROPERTY(EditAnywhere, Category = ChannelOptions, meta = (DataflowInput, EditCondition="!bFullyPadGutter", ClampMin = 1, UIMin = 1))
	int32 GutterSize = 1;

	// If true, will clear the alpha channel for gutter pixels, so they are transparent and the UV boundaries are clearer. If false, gutter pixel values will be copied as-is from the nearest interior pixel.
	UPROPERTY(EditAnywhere, Category = ChannelOptions, meta = (DataflowInput))
	bool bClearGutterAlpha = true;

	/** Index of the added UV channel */
	UPROPERTY(EditAnywhere, Category = ChannelOptions, meta = (DataflowInput, DataflowOutput, DataflowPassthrough = "UVChannel", DataflowUVChannel, ClampMin = 0, ClampMax = 7, UIMin = 0, UIMax = 7))
	int32 UVChannel = 0;
