...
namespace UE::Dataflow
{
	namespace Private
	{
		...
		{
			if (FDataflowArrayTypePolicy::SupportsTypeStatic(FromType))
			{
				// Cannot convert arrays to nested arrays.
				return false;
			}
	...
	void GeometryCollectionArrayNodes()
	{
		...
		DATAFLOW_NODE_REGISTER_CREATION_FACTORY(FDataflowMakeArrayNode);
...
void FFloatArrayNormalizeDataflowNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA<TArray<float>>(&OutFloatArray))
	{
		...
		if (IsConnected<FDataflowVertexSelection>(&Selection))
		{
			...
			{
				// Compute Min/Max
				float MinValue = 1e9, MaxValue = 1e-9f;
		...
		{
			// Compute Min/Max
			float MinValue = 1e9f, MaxValue = 1e-9f;
...
FName FDataflowConvertToArrayNode::GetDependentConnectionType(const FDataflowConnection& SourceConnection, const FDataflowConnection& DependentConnection) const
{
	...
	{
		if (FDataflowArrayTypePolicy::SupportsTypeStatic(SourceConnection.GetType()))
		{
			// Special case: Array elements are left unchanged.
...
///////////////////////////////////////////////////////////////////////////////////////////////
FDataflowMakeArrayNode::FDataflowMakeArrayNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
	: FDataflowNode(InParam, InGuid)
{	
	...
	// Add initial pins
	for (int32 Index = 0; Index < NumInitialElements; ++Index)
	{
		AddPins();
	}
	check(GetNumInputs() == NumRequiredInputs + NumInitialElements);  // Update NumRequiredInputs if you add more Inputs. This is used by Serialize.
}

FName FDataflowMakeArrayNode::GetDependentConnectionType(const FDataflowConnection& SourceConnection, const FDataflowConnection& DependentConnection) const
{
	auto IsElementConnection = [this](const FDataflowConnection& Connection)
		{
			for (const FDataflowAllTypes& Element : Elements)
			{
				if (Connection.IsA(&Element))
				{
					return true;
				}
			}
	...
	if (SourceConnection.IsA(&Array) && IsElementConnection(DependentConnection))
	{
		return FDataflowArrayTypePolicy::GetElementType(SourceConnection.GetType());
	}
	else if (DependentConnection.IsA(&Array) && IsElementConnection(SourceConnection))
	{
		if (FDataflowArrayTypePolicy::SupportsTypeStatic(SourceConnection.GetType()))
		{
			// Special case: Array elements are left unchanged.
...
void FDataflowMakeArrayNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA(&Array))
	{
		TArray<const UE::Dataflow::FContextValue> InElements;

		InElements.Reserve(Elements.Num());

		bool bIsAppendingArrays = false;
		for (int32 Index = 0; Index < Elements.Num(); ++Index)
		{
			if (IsConnected(GetConnectionReference(Index)))
			{
				const UE::Dataflow::FContextValue& InElement =  InElements.Emplace_GetRef(GetValue(Context, GetConnectionReference(Index)));
				if (InElement.IsArray())
				{
					bIsAppendingArrays = true;
				}
			}
		}

		if (bIsAppendingArrays)
		{
			UE::Dataflow::FContextValue OutArray = UE::Dataflow::FContextValue::AppendArrays(Context, InElements);
			SetValue(Context, MoveTemp(OutArray), &Array);
		}
		else
		{
			UE::Dataflow::FContextValue OutArray = UE::Dataflow::FContextValue::MakeArray(Context, InElements);
			SetValue(Context, MoveTemp(OutArray), &Array);
		}
	}
}

TArray<UE::Dataflow::FPin> FDataflowMakeArrayNode::AddPins()
{
	const int32 Index = Elements.AddDefaulted();
	FDataflowInput& Input = RegisterInputArrayConnection(GetConnectionReference(Index));
	if (Index > 0)
	{
		if (const FDataflowInput* Input0 = FindInput(GetConnectionReference(0)))
		{
			if (Input0->HasConcreteType())
			{
				SetConnectionConcreteType(&Input, Input0->GetType(), ArrayElementDependencyGroup);
			}
		}
	}
	Input.SetTypeDependencyGroup(ArrayElementDependencyGroup);
	return { { UE::Dataflow::FPin::EDirection::INPUT, Input.GetType(), Input.GetName() } };
}

TArray<UE::Dataflow::FPin> FDataflowMakeArrayNode::GetPinsToRemove() const
{
	const int32 Index = Elements.Num() - 1;
	check(Elements.IsValidIndex(Index));
...
void FDataflowMakeArrayNode::OnPinRemoved(const UE::Dataflow::FPin& Pin)
{
	const int32 Index = Elements.Num() - 1;
	check(Elements.IsValidIndex(Index));
#if DO_CHECK
...
UE::Dataflow::TConnectionReference<FDataflowAllTypes> FDataflowMakeArrayNode::GetConnectionReference(int32 Index) const
{
	return { &Elements[Index], Index, &Elements };
}

void FDataflowMakeArrayNode::PostSerialize(const FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		for (int32 Index = NumInitialElements; Index < Elements.Num(); ++Index)
		{
		...
		if (Ar.IsTransacting())
		{
			...
			check(OrigNumRegisteredInputs >= NumRequiredInputs + NumInitialElements);
			const int32 OrigNumElements = Elements.Num();
			const int32 OrigNumRegisteredElements = (OrigNumRegisteredInputs - NumRequiredInputs);
			if (OrigNumRegisteredElements > OrigNumElements)
			{
				// Inputs have been removed.
				// Temporarily expand Collections so we can get connection references.
				Elements.SetNum(OrigNumRegisteredElements);
				for (int32 Index = OrigNumElements; Index < Elements.Num(); ++Index)
				{
					UnregisterInputConnection(GetConnectionReference(Index));
				}
				Elements.SetNum(OrigNumElements);
			}
		}
		else
		{
			ensureAlways(Elements.Num() + NumRequiredInputs == GetNumInputs());
		}
	}
}
