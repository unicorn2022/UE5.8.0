	...
	: FDataflowNode(InParam, InGuid)
{
	...
	RegisterInputConnection(&bClearGutterAlpha).SetCanHidePin(true).SetPinIsHidden(true);
	RegisterInputConnection(&bFullyPadGutter).SetCanHidePin(true).SetPinIsHidden(true);
...
void FBakeTextureFromCollectionDataflowNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	...
	else if (Out->IsA(&Image))
	{
		...
		const int32 InGutterSize = GetValue(Context, &bFullyPadGutter) ? InResolution : GetValue(Context, &GutterSize);
		...
		if (!bIsValidUVChannel)
		{
			...
			UE_LOGF(LogChaosDataflow, Error, "Dataflow Node [%ls] : Invalid UV channel [%d] - The collection has %d UV channels", *GetName().ToString(), InUVChannel, GeomCollection->NumUVLayers());
		...
		BakeAttributeSettings.ClearGutterChannel = GetValue(Context, &bClearGutterAlpha) ? 3 : INDEX_NONE;
