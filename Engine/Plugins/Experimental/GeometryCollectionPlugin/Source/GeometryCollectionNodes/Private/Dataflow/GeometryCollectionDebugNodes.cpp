...
namespace UE::Dataflow
{
	void GeometryCollectionDebugNodes()
	{
		...
		DATAFLOW_NODE_REGISTER_CREATION_FACTORY(FSpheresToMeshDataflowNode);
	}
}
...
void FWriteStringToFile::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	...
	if (!bSuccess)
	{
		UE_LOGF(LogChaos, Warning, "Failed to write to file %ls:\n\n%ls", *InPath, *InContents);
	}
}

/* --------------------------------------------------------------------------------------------------------------------------- */

FSpheresToMeshDataflowNode::FSpheresToMeshDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
	: FDataflowNode(InParam, InGuid)
{
...
void FSpheresToMeshDataflowNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	using namespace UE::Geometry;

	if (Out->IsA(&Mesh))
	{
		TArray<FSphere> InSphereArr = GetValue(Context, &Spheres);
		...
		FDynamicMeshEditor Editor(&Accum);
		FMeshIndexMappings IndexMaps_Unused;
		for (int32 SphereIdx = 0; SphereIdx < InSphereArr.Num(); ++SphereIdx)
		{
			FBoxSphereGenerator SphereGen;
			SphereGen.EdgeVertices.A = SphereGen.EdgeVertices.B = SphereGen.EdgeVertices.C = FMath::Max(2, VerticesAlongEachSide);
			SphereGen.Radius = InSphereArr[SphereIdx].W;
			FVector Center = InSphereArr[SphereIdx].Center;
	...
	else if (Out->IsA(&Meshes))
	{
		...
		TArray<TObjectPtr<UDynamicMesh>> NewMeshes;
		NewMeshes.SetNum(Spheres.Num());

		for (FSphere& Sphere : InSphereArr)
		{
			FBoxSphereGenerator SphereGen;
			SphereGen.EdgeVertices.A = SphereGen.EdgeVertices.B = SphereGen.EdgeVertices.C = FMath::Max(2, VerticesAlongEachSide);
			SphereGen.Radius = Sphere.W;
			SphereGen.Box.Frame.Origin = Sphere.Center;

			FDynamicMesh3 SphereMesh(&SphereGen.Generate());

			TObjectPtr<UDynamicMesh> NewMesh = NewObject<UDynamicMesh>();
			NewMesh->SetMesh(MoveTemp(SphereMesh));
			NewMeshes.Add(NewMesh);
		}
