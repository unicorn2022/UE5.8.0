...
namespace UE::Dataflow
{
	void GeometryCollectionEngineAssetNodes()
	{
		...
		DATAFLOW_NODE_REGISTER_CREATION_FACTORY(FAssetRandomSeedDataflowNode);
	}
}
...
FGeometryCollectionToCollectionDataflowNode_v2::FGeometryCollectionToCollectionDataflowNode_v2(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
	: FDataflowNode(InParam, InGuid)
{
	RegisterInputConnection(&GeometryCollection);
...
void FGeometryCollectionToCollectionDataflowNode_v2::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	...
	const TObjectPtr<UGeometryCollection> InGeometryCollection = GetValue(Context, &GeometryCollection);
	if (InGeometryCollection)
	{
		FGeometryCollectionEngineConversion::ConvertGeometryCollectionToGeometryCollection(InGeometryCollection, OutCollection, OutMaterials, OutInstancedMeshes);

		FTransform RootTransform = FTransform::Identity;
		if (InGeometryCollection->GetGeometryCollection())
		{
			const GeometryCollection::Facades::FCollectionTransformFacade TransformFacade(*InGeometryCollection->GetGeometryCollection());
		...
		for (int32 MeshIndex = 0; MeshIndex < InGeometryCollection->RootProxyData.ProxyMeshes.Num(); MeshIndex++)
		{
			FDataflowRootProxyMesh RootProxyMesh;
...
FAssetRandomSeedDataflowNode::FAssetRandomSeedDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
	: FDataflowNode(InParam, InGuid)
{
	RegisterInputConnection(&Seed);
	RegisterOutputConnection(&RandomSeed);
}

void FAssetRandomSeedDataflowNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA(&RandomSeed))
	{
		const int32 InSeed = GetValue(Context, &Seed);

		if (const UE::Dataflow::FEngineContext* EngineContext = Context.AsType<UE::Dataflow::FEngineContext>())
		{
			if (EngineContext->Owner)
			{
				const int32 HashVal = (int32)HashCombine(GetTypeHash(EngineContext->Owner->GetPathName()) + GetTypeHash(InSeed));
				SetValue(Context, HashVal, &RandomSeed);

				return;
			}	
		}

		SetValue(Context, InSeed, &RandomSeed);
	}
}
