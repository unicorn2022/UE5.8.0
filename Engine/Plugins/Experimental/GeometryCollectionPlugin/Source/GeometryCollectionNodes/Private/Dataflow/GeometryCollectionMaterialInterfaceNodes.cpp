...
namespace UE::Dataflow
{
	void RegisterGeometryCollectionMaterialInterfaceNodes()
	{
		...
		DATAFLOW_NODE_REGISTER_CREATION_FACTORY(FMaterialInterfaceSetFloatParameterDataflowNode);
		DATAFLOW_NODE_REGISTER_CREATION_FACTORY(FMaterialInterfaceSetVectorParameterDataflowNode);
		DATAFLOW_NODE_REGISTER_CREATION_FACTORY(FMaterialInterfaceSetTexture2DParameterDataflowNode);
		DATAFLOW_NODE_REGISTER_CREATION_FACTORY(FMaterialInterfaceTerminalDataflowNode);
...
FMaterialInterfaceSetFloatParameterDataflowNode::FMaterialInterfaceSetFloatParameterDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
	: FDataflowNode(InParam, InGuid)
{
	RegisterInputConnection(&MaterialParameters);
	RegisterInputConnection(&ParameterName);
	RegisterInputConnection(&ParameterValue);
	RegisterOutputConnection(&MaterialParameters, &MaterialParameters);
}

void FMaterialInterfaceSetFloatParameterDataflowNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA(&MaterialParameters))
	{
		FDataflowMaterialParameters OutMaterialParameters = GetValue(Context, &MaterialParameters);

		FDataflowMaterialFloatParameter FloatParameter;
		FloatParameter.Name = FName(GetValue(Context, &ParameterName));
		FloatParameter.Value = static_cast<float>(GetValue(Context, &ParameterValue));

		if (!FloatParameter.Name.IsNone())
		{
			OutMaterialParameters.FloatParameters.Add(FloatParameter);
		}

		SetValue(Context, MoveTemp(OutMaterialParameters), &MaterialParameters);
	}
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

FMaterialInterfaceSetVectorParameterDataflowNode::FMaterialInterfaceSetVectorParameterDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
	: FDataflowNode(InParam, InGuid)
{
	RegisterInputConnection(&MaterialParameters);
	RegisterInputConnection(&ParameterName);
...
void FMaterialInterfaceSetVectorParameterDataflowNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA(&MaterialParameters))
	{
		FDataflowMaterialParameters OutMaterialParameters = GetValue(Context, &MaterialParameters);

		FDataflowMaterialVectorParameter VectorParameter;
		VectorParameter.Name = FName(GetValue(Context, &ParameterName));
		VectorParameter.Value = GetValue(Context, &ParameterValue);

		if (!VectorParameter.Name.IsNone())
		{
			OutMaterialParameters.VectorParameters.Add(VectorParameter);
		}
...
FMaterialInterfaceSetTexture2DParameterDataflowNode::FMaterialInterfaceSetTexture2DParameterDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
	: FDataflowNode(InParam, InGuid)
{
	RegisterInputConnection(&MaterialParameters);
	RegisterInputConnection(&ParameterName);
...
void FMaterialInterfaceSetTexture2DParameterDataflowNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA(&MaterialParameters))
	{
		FDataflowMaterialParameters OutMaterialParameters = GetValue(Context, &MaterialParameters);

		FDataflowMaterialTextureParameter TextureParameter;
		TextureParameter.Name = FName(GetValue(Context, &ParameterName));
		TextureParameter.Value = GetValue(Context, &ParameterValue);

		if (!TextureParameter.Name.IsNone())
		{
			OutMaterialParameters.TextureParameters.Add(TextureParameter);
		}
...
FMaterialInterfaceTerminalDataflowNode::FMaterialInterfaceTerminalDataflowNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
...
void FMaterialInterfaceTerminalDataflowNode::Evaluate(UE::Dataflow::FContext& Context) const
{
	...
	TObjectPtr<UMaterialInterface> OutMaterial = ::LoadObject<UMaterialInterface>(nullptr, InAssetPath);
	SetValue(Context, OutMaterial, &Material);
}

void FMaterialInterfaceTerminalDataflowNode::SetAssetValue(TObjectPtr<UObject> Asset, UE::Dataflow::FContext& Context) const
{
	TObjectPtr<UMaterialInterface> NullMaterialPtr = nullptr;

#if WITH_EDITOR
	const TObjectPtr<UMaterialInterface> ParentMaterial = GetValue(Context, &Material);
	if (ParentMaterial == nullptr)
	{
		Context.Warning(TEXT("Parent Material is null, material instance cannot be created"), this);
		SetValue(Context, NullMaterialPtr, &Material);
		return;
	}

	const FDataflowMaterialParameters& InMaterialParameters = GetValue(Context, &MaterialParameters);
	const FString& InAssetPath = GetValue(Context, &AssetPath);
	if (!FPackageName::IsValidObjectPath(InAssetPath))
	{
		Context.Warning(FString::Format(TEXT("Asset Path input is not a valid path : {0}"), { InAssetPath }), this);
		SetValue(Context, NullMaterialPtr, &Material);
		return;
	}
	...
	if (Package == nullptr)
	{
		Context.Warning(FString::Format(TEXT("Failed to find or create package {0}"), { PackageName }), this);
		SetValue(Context, NullMaterialPtr, &Material);
		return;
	}

	const FString AssetName = FPackageName::GetLongPackageAssetName(PackageName);
	UMaterialInstanceConstant* MaterialInstance = NewObject<UMaterialInstanceConstant>(Package, UMaterialInstanceConstant::StaticClass(), FName(AssetName), RF_Public | RF_Standalone | RF_Transactional);
	if (!MaterialInstance)
	{
		Context.Warning(FString::Format(TEXT("Material instance {0} with cannot be created"), { InAssetPath }), this);
		SetValue(Context, NullMaterialPtr, &Material);
		return;
	}
	...
	for (const FDataflowMaterialFloatParameter& FloatParameter : InMaterialParameters.FloatParameters)
	{
		MaterialInstance->SetScalarParameterValueEditorOnly(*FloatParameter.Name.ToString(), FloatParameter.Value);
	}

	for (const FDataflowMaterialVectorParameter& VectorParameter : InMaterialParameters.VectorParameters)
	{
		MaterialInstance->SetVectorParameterValueEditorOnly(*VectorParameter.Name.ToString(), FLinearColor(VectorParameter.Value));
	}

	for (const FDataflowMaterialTextureParameter& TextureParameter : InMaterialParameters.TextureParameters)
	{
		MaterialInstance->SetTextureParameterValueEditorOnly(*TextureParameter.Name.ToString(), TextureParameter.Value.Get());
	}
	...
	TObjectPtr<UMaterialInterface> OutMaterialInstance = MaterialInstance;
	SetValue(Context, OutMaterialInstance, &Material);

#else
	Context.Error(TEXT("Creation of material instance only supported in Editor"), this);
	SetValue(Context, NullMaterialPtr, &Material);
#endif
}
