...
TConstArrayView<FAssetCategoryPath> UAssetDefinition_GeometryCollection::GetAssetCategories() const
{
	static const TArray<FAssetCategoryPath> Categories = 
		{
			FAssetCategoryPath(EAssetCategoryPaths::Physics, LOCTEXT("GeometryCollection_SubMenu", "Geometry Collection"), ECategoryMenuType::Section)
		};
	return Categories;
}
			...
			UToolMenus::RegisterStartupCallback(
				FSimpleMulticastDelegate::FDelegate::CreateLambda([]()
				{
					...
					FToolMenuSection& Section = Menu->FindOrAddSection("GetAssetActions");
					{
						...
						Section.AddMenuEntry("GeometryCollection_ConvertToSkeletalMesh", Label, ToolTip, Icon, UIAction);
					}
				}));
		});
}
