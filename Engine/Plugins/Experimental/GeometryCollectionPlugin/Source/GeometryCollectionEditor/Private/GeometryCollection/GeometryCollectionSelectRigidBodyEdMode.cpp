...
void FGeometryCollectionSelectRigidBodyEdMode::ActivateMode(TSharedRef<IPropertyHandle> PropertyHandleId, TSharedRef<IPropertyHandle> PropertyHandleSolver, TFunction<void()> OnEnterMode, TFunction<void()> OnExitMode)
{
	...
	if (CanActivateMode())
	{
		...
		if (!GEditor->bIsSimulatingInEditor)
		{
			...
			// Log/display message
			UE_LOGF(LogGeometryCollectionSelectRigidBodyEdMode, Warning, "Player possession ejected by rigid body picker.");
			if (GEngine)
			{
		...
		if (FEdMode* const EdMode = GLevelEditorModeTools().GetActiveMode(EditorModeID))
		{
			...
			// Log/display message
			UE_LOGF(LogGeometryCollectionSelectRigidBodyEdMode, Log, "Click on the rigid body to select...");
			if (GEngine)
			{
	...
	else
	{
		...
		UE_LOGF(LogGeometryCollectionSelectRigidBodyEdMode, Error, "The rigid body picker is only available in play in editor.");
		if (GEngine)
		{
...
bool FGeometryCollectionSelectRigidBodyEdMode::MouseMove(FEditorViewportClient* /*ViewportClient*/, FViewport* Viewport, int32 x, int32 y)
{
	...
	UE_CLOGF(bIsHoveringGeometryCollection, LogGeometryCollectionSelectRigidBodyEdMode, VeryVerbose, "Hovering GeometryCollectionActor %ls, Transform Index %d.", *static_cast<const HGeometryCollection*>(HitProxy)->Component->GetOwner()->GetName(), static_cast<const HGeometryCollection*>(HitProxy)->BoneIndex);
...
void FGeometryCollectionSelectRigidBodyEdMode::EnableTransformSelectionMode(bool bEnable)
{
	...
	for (TActorIterator<AGeometryCollectionActor> ActorIterator(GEditor->PlayWorld); ActorIterator; ++ActorIterator)
	{
		...
		{
			UE_LOGF(LogGeometryCollectionSelectRigidBodyEdMode, Verbose, "EnableTransformSelectionMode(%d) called for AGeometryCollectionActor %ls.", bEnable, *ActorIterator->GetName());
#if GEOMETRYCOLLECTION_EDITOR_SELECTION
