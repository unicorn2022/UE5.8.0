...
{
	//UE_LOGF(UGeometryCollectionConversionLogging, Log, "FGeometryCollectionConversion::CreateCommand()");
...
void FGeometryCollectionConversion::CreateFromSelectedAssetsCommand(UWorld * World)
{
	...
	for (const FAssetData& AssetData : SelectedAssets)
	{
		...
		{
			UE_LOGF(UGeometryCollectionConversionLogging, Log, "Static Mesh Content Browser : %ls", *AssetData.GetClass()->GetName());
			if (!GeometryCollectionAsset.IsCreated())
			{
				GeometryCollectionAsset.Create();
			}
		...
		{
			UE_LOGF(UGeometryCollectionConversionLogging, Log, "Skeletal Mesh Content Browser : %ls", *AssetData.GetClass()->GetName());
			if (!GeometryCollectionAsset.IsCreated())
			{
				GeometryCollectionAsset.Create();
			}
