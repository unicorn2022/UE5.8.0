...
UObject* UGeometryCollectionFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	UE::DataflowAssetDefinitionHelpers::SetDataflowFromTemplatePicker(NewGeometryCollection);

	// Assign a default dataflow template if available 
	/*if (const UDataflow* DataflowTemplateObject = LoadObject<UDataflow>(nullptr, TEXT("/GeometryCollectionPlugin/DF_GC_Template_StaticMesh.DF_GC_Template_StaticMesh")))
	{
		if (UDataflow* DataflowEmbeddedAsset = DuplicateObject<UDataflow>(DataflowTemplateObject, NewGeometryCollection, TEXT("EmbeddedDataflow")))
		{
			NewGeometryCollection->GetDataflowInstance().SetDataflowAsset(DataflowEmbeddedAsset);
		}
	}*/
