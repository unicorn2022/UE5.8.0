		...
		for (TWeakObjectPtr<> WeakObject : Player.FindBoundObjects(Operand))
		{
			...
			if (GeometryCollectionComponent)
			{
				if (UGeometryCollectionCache* GeometryCollectionCache = Cast<UGeometryCollectionCache>(Params.GeometryCollectionCache.ResolveObject()))
				{
					...
					if (!bIsValidCache)
					{
						UE_LOGF(LogMovieScene, Warning, "Unsupported Geometry Collection Cache (%ls) for Component (%ls), ignoring playback!", *GeometryCollectionCache->GetFullName(), *GeometryCollectionComponent->GetFullName());
						continue;
					}
...
float FMovieSceneGeometryCollectionSectionTemplateParameters::MapTimeToAnimation(FFrameTime InPosition, FFrameRate InTickResolution) const
{
	...
	const float SeqLength = GetSequenceLength() - InTickResolution.AsSeconds(StartFrameOffset + EndFrameOffset);
	...
	AnimPosition += InTickResolution.AsSeconds(StartFrameOffset);
	// if (bReverse)
	// {
