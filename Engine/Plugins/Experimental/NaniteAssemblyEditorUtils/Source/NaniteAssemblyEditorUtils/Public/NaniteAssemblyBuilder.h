...
bool UNaniteAssemblyBuilder::ValidateCandidatePartMesh(const TMesh* PartMesh) const
{
	...
	if (PartMesh == nullptr)
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[AddAssemblyParts] Part mesh is not valid.");
		return false;
	}
	...
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[AddAssemblyParts] Adding target mesh as a part not supported.");
		return false;
	}
	...
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[AddAssemblyParts] Adding Nanite Assembly mesh as a part not currently supported. Part Mesh: %ls",
