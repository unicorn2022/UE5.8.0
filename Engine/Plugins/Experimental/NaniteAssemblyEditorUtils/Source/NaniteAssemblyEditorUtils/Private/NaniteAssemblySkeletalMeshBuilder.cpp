...
static bool ValidateValidMeshAndSkeleton(const TCHAR* Label, const USkeletalMesh* Mesh)
{
	if (Mesh == nullptr)
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[%ls] Target Skeletal Mesh is not valid.",
			Label);
		return false;
	}
	...
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[%ls] Target Skeletal Mesh has no valid skeleton: %ls",
			Label,
			*Mesh->GetName());
		return false;
	}
...
bool UNaniteAssemblySkeletalMeshBuilder::FinishAssemblyBuild(USkeletalMesh*& OutSkeletalMesh)
{
	...
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Display,
			"[FinishSkeletalMeshAssemblyBuild] Assembly build failed. No assembly parts were added.");
		return false;
	}
...
bool UNaniteAssemblySkeletalMeshBuilder::ValidateBindings(TArrayView<const FNaniteAssemblySkeletalMeshPartBinding> Bindings) const
{
	...
	for (const auto& Binding : Bindings)
	{
		...
		{
			UE_LOGF(LogNaniteAssemblyBuilder, Error,
				"[AddAssemblyParts] An invalid binding was encountered with 0 bone influences.");
			return false;
		}
		for (const auto& BoneInfluence : Binding.BoneInfluences)
		{
			...
			{
				UE_LOGF(LogNaniteAssemblyBuilder, Error,
					"[AddAssemblyParts] Binding with invalid bone index %d encountered.",
