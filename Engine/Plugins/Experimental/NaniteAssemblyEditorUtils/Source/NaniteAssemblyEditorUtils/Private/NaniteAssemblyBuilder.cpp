...
bool UNaniteAssemblyBuilder::ValidateBuildStatus(const TCHAR* Label, bool bExpectedIsBuilding) const
{
	...
	if (bIsBuilding != bExpectedIsBuilding)
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[%ls] This builder is %lscurrently building an assembly.",
...
)
{
	...
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[CreateNewMeshForAssemblyBuild] Cannot duplicate %ls as a new %ls asset. Incompatible types.",
	...
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[CreateNewMeshForAssemblyBuild] Failed to create new package for Nanite Assembly asset: %ls",
			*PackageName);
		return nullptr;
	}
	...
	{
		UE_LOGF(LogNaniteAssemblyBuilder, Error,
			"[CreateNewMeshForAssemblyBuild] Failed to create new %ls for Nanite Assembly asset: %ls.%ls",
			*Class->GetName(),
			*PackageName,
			*AssetName);
		return nullptr;
