...
namespace PrivateFliteSpeechStreaming
{
		...
		int bIsLastChunk, cst_audio_streaming_info* AudioStreamingInfo)
	{
		...
		{
			UE_LOGF(LogTextToSpeech, VeryVerbose, "Stopping Flite speech streaming.");
		...
		if (bIsLastChunk == 1)
		{
			UE_LOGF(LogTextToSpeech, VeryVerbose, "Synthesizing last speech chunk of %i bytes.", SpeechData.GetNumSpeechSamples());
...
public:
	...
	void DoWork()
	{
		UE_LOGF(LogTextToSpeech, VeryVerbose, "Sy Background thread synthesizing %ls.", *SynthesisText);
...
{
	UE_LOGF(LogTextToSpeech, VeryVerbose, "Starting speech synthesis in background thread.");
...
{
	UE_LOGF(LogTextToSpeech, VeryVerbose, "Toggling streaming flag on.");
...
{
	UE_LOGF(LogTextToSpeech, VeryVerbose, "Toggling speech streaming flag off.");
