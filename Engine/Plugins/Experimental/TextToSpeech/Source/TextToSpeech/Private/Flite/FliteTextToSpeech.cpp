...
void FFliteTextToSpeech::Speak(const FString& InStringToSpeak)
{
	...
	if (IsActive())
	{
		...
		UE_LOGF(LogTextToSpeech, VeryVerbose, "Starting to synthesize text: %ls", *InStringToSpeak);
...
void FFliteTextToSpeech::StopSpeaking()
{
	...
	if (IsActive())
	{
		UE_LOGF(LogTextToSpeech, VeryVerbose, "Stopping speech synthesis.");
...
{
	check(IsInGameThread());
	UE_LOGF(LogTextToSpeech, Verbose, "Activating speech synthesis");
...
{
	check(IsInGameThread());
	UE_LOGF(LogTextToSpeech, Verbose, "Deactivating text to speech.");
	if (IsSpeaking())
	{
		StopSpeaking();
	}
