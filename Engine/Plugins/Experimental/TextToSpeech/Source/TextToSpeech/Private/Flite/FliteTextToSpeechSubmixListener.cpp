...
void FFliteTextToSpeechSubmixListener::OnNewSubmixBuffer(const USoundSubmix* OwningSubmix, float* AudioData, int32 NumSamples, int32 NumChannels, const int32 SampleRate, double AudioClock)
{
	...
	if (bPlayedLastChunk)
	{
		UE_LOGF(LogTextToSpeech, VeryVerbose, "Finishing playing last chunk of audio.");
...
void FFliteTextToSpeechSubmixListener::QueueSynthesizedSpeechChunk_AnyThread(FFliteSynthesizedSpeechData InSynthesizedSpeechChunk)
{
	...
	if (FAudioDeviceManager* AudioDeviceManager = FAudioDeviceManager::Get())
	{
		if (FAudioDeviceHandle AudioDeviceHandle = AudioDeviceManager->GetMainAudioDeviceHandle())
		{
			...
			UE_LOGF(LogTextToSpeech, VeryVerbose, "Sample rate conversion ratio is : %f.", SampleRateConversionRatio);
			...
			if (ErrorCode != 0)
			{
				UE_LOGF(LogTextToSpeech, Warning, "Problem occured resampling Flite speech data.");
				ensure(false);
			}
			...
			UE_LOGF(LogTextToSpeech, VeryVerbose, "Calculated SRC buffer size: %i. Actual output samples: %i.", NumConvertedSamples, OutputSamples);
...
void FFliteTextToSpeechSubmixListener::StopPlayback_GameThread()
{
	...
	UE_LOGF(LogTextToSpeech, VeryVerbose, "Emptied speech buffer.");
}
...
void FFliteTextToSpeechSubmixListener::FSynthesizedSpeechBuffer::PushData(FFliteSynthesizedSpeechData InSpeechData)
{
	...
	{
		// We ran out of space
		UE_LOGF(LogTextToSpeech, Verbose, "Flite audio buffer ran out of space. ");
		ensure(false);
	}
...
{
	UE_LOGF(LogTextToSpeech, VeryVerbose, "Popping speech data. %i samples requested.", NumRequestedSamples);
	bOutPoppingLastChunk = false;
	if (IsEmpty() || (NumRequestedSamples < 1))
	{
		UE_LOGF(LogTextToSpeech, VeryVerbose, "SPeech buffer is empty. No data popped.");
...
// because of the way flite streaming works
	...
	UE_LOGF(LogTextToSpeech, VeryVerbose, "Number of samples popped: %i", NumPoppedSamples);
	ensure(NumPoppedSamples <= NumRequestedSamples && NumPoppedSamples > 0);
