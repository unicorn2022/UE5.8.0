...
namespace FollowSplineMode::Utils::Private
{
	static float ComputeRangeInputValue(const USplineComponent* ControlSpline, FSplineOffsetRangeInput RangeOffsetInput)
	{
		...
		default:
		{
			UE_LOGF(LogMover, Warning, "Unknown Spline Offset Unit");
			break;
		}
		}
		return 0.0f;
...
void UFollowSplineMode::GenerateMove_Implementation(const FMoverSimContext& SimContext, const FMoverTickStartData& StartState, const FMoverTimeStep& TimeStep, FProposedMove& OutProposedMove) const
{
...
void UFollowSplineMode::OnRegistered(const FName ModeName, const FMoverSimContext& SimContext)
{
