...
void UFollowPathMode::GenerateMove_Implementation(const FMoverSimContext& SimContext, const FMoverTickStartData& StartState, const FMoverTimeStep& TimeStep, FProposedMove& OutProposedMove) const
{
...
void UFollowPathMode::Activate(const FMoverEventContext& Context, FName PrevModeName, const FMoverSimContext& SimContext, const FMoverTickStartData& StartState, FMoverSyncState* OutSyncState, FMoverAuxStateContext* OutAuxState)
{
	Super::Activate(Context, PrevModeName, SimContext, StartState, OutSyncState, OutAuxState);
	
	UpdateControlPoints(true);
	TimeMultiplier = 1.f / (Duration > UE_KINDA_SMALL_NUMBER ? Duration : UE_KINDA_SMALL_NUMBER);
}
...
FRotator UFollowPathMode::ComputeMoveOrientation(const float TargetPathPos, const FVector& BaseLocation, FRotator DefaultOrientation) const
{
	...
	{
		UE_LOGF(LogMover, Warning, "EFollowPathRotationType %i is not supported yet. Using default orietation instead", int(RotationType));
		ReturnOrientation = DefaultOrientation;
	}
