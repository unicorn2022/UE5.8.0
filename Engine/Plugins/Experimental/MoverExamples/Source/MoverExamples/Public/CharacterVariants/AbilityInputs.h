...
struct MOVEREXAMPLES_API FMoverExampleAbilityInputs : public FMoverDataStructBase
{
	...
	{
		const FMoverExampleAbilityInputs& FromInputs = static_cast<const FMoverExampleAbilityInputs&>(From);
		const FMoverExampleAbilityInputs& ToInputs   = static_cast<const FMoverExampleAbilityInputs&>(To);

		// "Just pressed" one-shot bools: OR across both frames so a press is never lost
		// to the interpolation cutoff. A synthesized input covering a frame where the press
		// occurred must preserve it regardless of which side of 0.5 the lerp factor lands on.
		bIsDashJustPressed     = FromInputs.bIsDashJustPressed     | ToInputs.bIsDashJustPressed;
		bIsVaultJustPressed    = FromInputs.bIsVaultJustPressed    | ToInputs.bIsVaultJustPressed;
		bWantsToStartZiplining = FromInputs.bWantsToStartZiplining | ToInputs.bWantsToStartZiplining;

		// Held/sustained bools: stable across frames, threshold selection is fine.
		const FMoverExampleAbilityInputs& SourceInputs = (LerpFactor < 0.5f) ? FromInputs : ToInputs;
		bIsAimPressed      = SourceInputs.bIsAimPressed;
		bWantsToBeCrouched = SourceInputs.bWantsToBeCrouched;
	}
