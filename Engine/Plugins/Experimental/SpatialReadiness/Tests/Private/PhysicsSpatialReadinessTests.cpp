...
namespace
{
	...
	FSingleParticlePhysicsProxy* MakeDynamicBox(FChaosScene& Scene, const FBox& Bounds, const bool bGravityEnabled=true)
	{
		...
		Builder.SetCollisionChannelIndex(ECollisionChannel::ECC_WorldDynamic);
		Builder.SetBlockChannelMask(TNumericLimits<uint64>::Max());
		Builder.SetFilterFlags(Chaos::EFilterFlags::CCD);
		...
		ParticleHandle.SetShapeFilterData(0, ShapeFilterData);
...
TEST_CASE("Sim Callback", "[physics spatial readiness]")
{
	...
	SECTION("Spawn dynamic particle overlapping unready volume, with gravity")
	{
		...
		// Advance ten more ticks to make sure it never starts falling
	...
	SECTION("Dynamic particle with gravity stops falling when it hits an unready volume")
	{
		...
		// Make a particle which is just above the unready volume
		...
		// should be removed from the list of unready particles after 2 ticks
		// because spatial readiness always keeps particles unready for one extra tick.
		Volume.MarkReady();
		AdvanceAndWait(Solver);
		REQUIRE(SimCallback->GetUnreadyRigidParticles_PT().Num() == 1);
		AdvanceAndWait(Solver);
