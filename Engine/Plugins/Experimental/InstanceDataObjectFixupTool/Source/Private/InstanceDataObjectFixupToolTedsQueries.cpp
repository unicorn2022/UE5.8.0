...
void UInstanceDataObjectFixupToolTedsQueryFactory::ShowFixUpTool(UE::Editor::DataStorage::RowHandle Row, bool bRecurseIntoObject)
{
	...
	if (FTypedElementUObjectColumn* ObjectColumn = DataStorage->GetColumn<FTypedElementUObjectColumn>(Row))
	{
		if (bRecurseIntoObject)
		{
			...
			TOptional<FTopLevelAssetPath> ClassPath;

			UE::FPropertyBagRepository::Get().FindNestedInstanceDataObject(Owner, true,
				[Owner, &ClassPath](UObject* NestedObject)
				{
					ClassPath = Owner->GetClass()->GetClassPathName();
				});


			FInstanceDataObjectFixupToolModule::Get().CreateDataRecoveryToolDialog(ClassPath);
			
		}
		else
		{
			UObject* Owner = ObjectColumn->Object.Get();
			if (TObjectPtr<UObject> InstanceDataObject = UE::FPropertyBagRepository::Get().FindInstanceDataObject(Owner))
			{
				FTopLevelAssetPath ClassPath = Owner->GetClass()->GetClassPathName();
				FInstanceDataObjectFixupToolModule::Get().CreateDataRecoveryToolDialog(ClassPath);
			}
		}
	}
