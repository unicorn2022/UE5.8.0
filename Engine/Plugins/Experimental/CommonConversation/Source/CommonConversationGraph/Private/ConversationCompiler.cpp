...
namespace UE::CommonConversationGraph::Private
{
	static int32 GetNodeXLocation(const UEdGraphPin* Pin)
	{
		check(Pin);
...
void FConversationCompiler::RebuildBank(UConversationDatabase* ConversationAsset)
{
	...
	for (UConversationGraphNode* EdNode : AllGraphNodes)
	{
		...
		if (UConversationGraphNode_Task* EdTaskNode = Cast<UConversationGraphNode_Task>(EdNode))
		{
			...
			for (UAIGraphNode* SubNode : EdTaskNode->SubNodes)
			{
				...
				if (ensure(TypedSubNode))
				{
					...
					{
						UE_LOGF(LogCommonConversationGraph, Warning, "Invalid TaskSubNode inside Conversation Asset %ls", *ConversationAsset->GetPrimaryAssetId().ToString());
					}
				}
				else
				{
					UE_LOGF(LogCommonConversationGraph, Warning, "Invalid TypedSubNode inside Conversation Asset %ls", *ConversationAsset->GetPrimaryAssetId().ToString());
				}
			}
		}
		...
		if (UConversationNodeWithLinks* RuntimeNodeWithLinks = Cast<UConversationNodeWithLinks>(EdNode->NodeInstance))
		{
			...
			ForeachConnectedOutgoingConversationNodeSorted(OutputPin, [RuntimeNodeWithLinks](UConversationGraphNode* RemoteNode)
			{
...
void FConversationCompiler::ForeachConnectedOutgoingConversationNodeSorted(UEdGraphPin* Pin, TFunctionRef<void(UConversationGraphNode*)> Predicate)
{
	TArray<UEdGraphPin*> SortedPins = Pin->LinkedTo;
	Algo::SortBy(SortedPins, UE::CommonConversationGraph::Private::GetNodeXLocation);

	for (UEdGraphPin* RemotePin : SortedPins)
	{
		if (UConversationGraphNode_Knot* Knot = Cast<UConversationGraphNode_Knot>(RemotePin->GetOwningNode()))
		{
			ForeachConnectedOutgoingConversationNode(Knot->GetOutputPin(), Predicate);
