...
void UConversationParticipantComponent::ClientUpdateConversation_Implementation(const FClientConversationMessagePayload& Message)
{
	...
	UE_LOGF(LogCommonConversationRuntime, Log, "ClientUpdateConversation: %ls", *Message.ToString());
...
{
	UE_LOGF(LogCommonConversationRuntime, Log, "ClientUpdateConversationTaskChoiceData :%ls", *Handle.ToString());
...
{
	UE_LOGF(LogCommonConversationRuntime, Verbose, "[%ls]: %ls has entered the conversation state.", ANSI_TO_TCHAR(__FUNCTION__), *GetNameSafe(GetOwner()));
}
...
{
	UE_LOGF(LogCommonConversationRuntime, Verbose, "[%ls]: %ls has exited the conversation state.", ANSI_TO_TCHAR(__FUNCTION__), *GetNameSafe(GetOwner()));
}
