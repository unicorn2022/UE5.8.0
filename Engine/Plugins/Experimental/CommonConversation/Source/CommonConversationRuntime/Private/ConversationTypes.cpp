...
bool FConversationChoiceDataHandle::NetSerialize(FArchive& Ar, class UPackageMap* Map, bool& bOutSuccess)
{
	...
	//if (Ar.IsSaving())
	//{
	//	UE_CLOGF(Data.Num() > MAX_uint8, LogCommonConversationRuntime, Warning, "Too many TargetData sources (%d!) to net serialize. Clamping to %d", Data.Num(), MAX_uint8);
	...
	//	if (DataNum > 32)
	//	{
	//		UE_LOGF(LogCommonConversationRuntime, Warning, "FConversationChoiceDataHandle::NetSerialize received with large DataNum: %d", DataNum);
	//	}
	//}
	...
	//			UE_LOGF(LogCommonConversationRuntime, Fatal, "FConversationChoiceDataHandle::NetSerialize called on data struct %ls without a native NetSerialize", *ScriptStruct->GetName());
	...
	//	{
	//		UE_LOGF(LogCommonConversationRuntime, Error, "FConversationChoiceDataHandle::NetSerialize: Bad ScriptStruct serialized, can't recover.");
	//		Ar.SetError();
	//		break;
	//	}
	//}
