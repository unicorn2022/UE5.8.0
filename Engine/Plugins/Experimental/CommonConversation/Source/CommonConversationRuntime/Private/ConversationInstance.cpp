...
namespace UE::ConversationCommon::Private
{
	static EConversationNodeBranchOrder FindBranchOrderFromChoice(const FConversationContext& Context, const FConversationChoiceReference& PreviousNode)
	{
		if (const UConversationTaskNode* ConversationNode = Cast<UConversationTaskNode>(PreviousNode.NodeReference.TryToResolve(Context)))
		{
			return ConversationNode->BranchOrder;
		}

		return EConversationNodeBranchOrder::Random;
	}
}
...
void UConversationInstance::ServerAssignParticipant(const FGameplayTag& ParticipantID, AActor* ParticipantActor)
{
	...
	{
		UE_LOGF(LogCommonConversationRuntime, Error, "AConversationInstance::ServerAssignParticipant(ID=%ls, Actor=%ls) passed bad arguments",
	...
	UE_LOGF(LogCommonConversationRuntime, Verbose, "Conversation %ls assigned participant ID=%ls to Actor=%ls",
...
{
	UE_LOGF(LogCommonConversationRuntime, Verbose, "Conversation %ls starting at %ls with %d participants",
	...
	{
		UE_LOGF(LogCommonConversationRuntime, Warning, "Entry point %ls did not exist or had no destination entries; conversation aborted", *EntryPoint.ToString());
	...
	else
	{
		...
		{
			UE_LOGF(LogCommonConversationRuntime, Verbose, "All branches from entry point %ls are disabled, conversation aborted", *EntryPoint.ToString());
			ServerAbortConversation();
			return;
		}
		else
		{
			const int32 StartingIndex = ChooseBranchIndex(EConversationNodeBranchOrder::Random, LegalStartingPoints.Num());
			...
			UE_LOGF(LogCommonConversationRuntime, Verbose, "Chosing branch index %d to %ls (of %d legal branches) from entry point %ls",
...
void UConversationInstance::ServerAdvanceConversation(const FAdvanceConversationRequest& InChoicePicked)
{
	...
	{
		UE_LOGF(LogCommonConversationRuntime, Verbose, "ServerAdvanceConversation is determining destinations from %ls", *GetCurrentChoiceReference().ToString());
		...
		if (InChoicePicked.Choice != FConversationChoiceReference::Empty)
		{
			...
			{
				UE_LOGF(LogCommonConversationRuntime, Verbose, "User picked option %ls, going to try that", *BranchPoint->ClientChoice.ChoiceReference.ToString());
		...
		{
			UE_LOGF(LogCommonConversationRuntime, Verbose, "No available destinations from %ls, ending the conversation", *GetCurrentChoiceReference().ToString());
			ServerAbortConversation();
			return;
		}
		else
		{
			...
			const EConversationNodeBranchOrder BranchOrder = UE::ConversationCommon::Private::FindBranchOrderFromChoice(ServerContext, PreviousNode);
			const int32 StartingIndex = ChooseBranchIndex(BranchOrder, ValidDestinations.Num());
			const FConversationBranchPoint& TargetChoice = ValidDestinations[StartingIndex];
			
			UE_LOGF(LogCommonConversationRuntime, Verbose, "Chosing destination index %d to %ls (of %d legal branches) from %ls using order %ls",
				StartingIndex, *TargetChoice.ClientChoice.ChoiceReference.ToString(), ValidDestinations.Num(), *PreviousNode.ToString(), *UEnum::GetValueAsString(BranchOrder));
	...
	{
		UE_LOGF(LogCommonConversationRuntime, Error, "ServerAdvanceConversation called when the conversation is not active");
	}
}
...
void UConversationInstance::OnInvalidBranchChoice(const FAdvanceConversationRequest& InChoicePicked)
{
	...
	{
		UE_LOGF(LogCommonConversationRuntime, Error, "User picked option %ls but it's not a legal output, aborting", *InChoicePicked.ToString());
		ServerAbortConversation();
	}
	else
	{
		UE_LOGF(LogCommonConversationRuntime, Warning, "User picked option %ls but it's not a legal output, ignoring", *InChoicePicked.ToString());
...
void UConversationInstance::ServerAbortConversation()
{
	if (bConversationStarted)
	{
		UE_LOGF(LogCommonConversationRuntime, Verbose, "Conversation aborted or finished");
...
{
	UE_LOGF(LogCommonConversationRuntime, Verbose, "Modying Current Node From %ls To %ls",
...
TArray<FGuid> UConversationInstance::DetermineBranches(const TArray<FGuid>& SourceList, EConversationRequirementResult MaximumRequirementResult)
{
	...
	for (const FGuid& TestGUID : SourceList)
	{
		...
		if (UConversationTaskNode* TaskNode = Cast<UConversationTaskNode>(TestNode))
		{
			...
			{
				UE_LOGF(LogCommonConversationRuntime, Verbose, "\t%ls is legal", *TestGUID.ToString());
				EnabledPaths.Add(TestGUID);
			}
		}
	}

	UE_LOGF(LogCommonConversationRuntime, Verbose, "\t%d paths out of %d are legal", EnabledPaths.Num(), SourceList.Num());
	return EnabledPaths;
}
...
void UConversationInstance::ProcessCurrentConversationNode()
{
	...
	{
		UE_LOGF(LogCommonConversationRuntime, Verbose, "Executing task node %ls", *GetCurrentChoiceReference().ToString());
	...
	{
		UE_LOGF(LogCommonConversationRuntime, Error, "Ended up with no task node with ID %ls, aborting conversation", *GetCurrentChoiceReference().ToString());

		ServerAbortConversation();
	}
}

int32 UConversationInstance::ChooseBranchIndex(EConversationNodeBranchOrder BranchOrder, int32 NumBranchIndices)
{
	switch (BranchOrder)
	{
	case EConversationNodeBranchOrder::LeftToRight:
		return 0;
	case EConversationNodeBranchOrder::RightToLeft:
		return FMath::Max(NumBranchIndices - 1, 0);
	case EConversationNodeBranchOrder::Random:
	default:
		return ConversationRNG.RandRange(0, NumBranchIndices - 1);
	}
}
