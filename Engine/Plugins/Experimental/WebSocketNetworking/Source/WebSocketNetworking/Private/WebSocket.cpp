...
{
	UE_LOGF(LogWebSocketNetworking, Log, "client: %ls", ANSI_TO_TCHAR(line));
}
...
:IsServerSide(false)
{
	...
	if (SockFd == -1)
	{
		UE_LOGF(LogWebSocketNetworking, Error, "Socket creation failed ");
	}
	else
	{
		UE_LOGF(LogWebSocketNetworking, Warning, " Socked %d created ", SockFd);
	}
	...
	{
		UE_LOGF(LogWebSocketNetworking, Warning, "WSAStringToAddress failed ");
		return;
	}
#else
	if (inet_pton(AF_INET, TCHAR_TO_ANSI(*ServerAddress.ToString(false)), &RemoteAddr.sin_addr) != 1)
	{
		UE_LOGF(LogWebSocketNetworking, Warning, "inet_pton failed to %ls", *ServerAddress.ToString(false));
		return;
	}
#endif
	...
	UE_LOGF(LogWebSocketNetworking, Warning, " Connect socket returned %d to %ls. Error Code: %d", Ret, *ServerAddress.ToString(false), ((Ret != 0) ? errno : 0));
#endif
}
...
void FWebSocket::HandlePacket()
{
	...
	if (Res == -1) {
		UE_LOGF(LogWebSocketNetworking, Warning, "Select Failed!");
		return;
	}
...
void FWebSocket::Flush()
{
	...
	while (OutgoingBuffer.Num() > 0 && !IsServerSide)
	{
		...
		{
			UE_LOGF(LogWebSocketNetworking, Warning, "Unable to flush all of OutgoingBuffer in FWebSocket.");
			break;
		}
	};
}
...
void FWebSocket::OnRawWebSocketWritable(WebSocketInternal* wsi)
{
	...
	while (DataToSend)
	{
		...
		{
			UE_LOGF(LogWebSocketNetworking, Warning, "Could not write all '%d' bytes to socket", DataToSend);
		}
		DataToSend-=Sent;
	}
	...
	while (DataToSend)
	{
		...
		if (Result == -1)
		{
			...
			UE_LOGF(LogWebSocketNetworking, Error, "Could not write %d bytes", Packet.Num());
			ErrorCallBack.ExecuteIfBound();
			return;
		}
