...
{
	UE_LOGF(LogWebSocketNetworking, Log, "websocket server: %ls", ANSI_TO_TCHAR(line));
}
...
bool FWebSocketServer::Init(uint32 Port, FWebSocketClientConnectedCallBack CallBack, FString InBindAddress)
{
	...
	const bool bUseIPv6 = EnumHasAnyFlags(NetworkProtocol, IWebSocketServer::ENetworkProtocol::IPv6);
	const bool bUseIPv4 = EnumHasAnyFlags(NetworkProtocol, IWebSocketServer::ENetworkProtocol::IPv4);
	...
	{
		// LWS_SERVER_OPTION_IPV6_V6ONLY_MODIFY tells lws to set IPV6_V6ONLY explicitly.
		Info.options |= LWS_SERVER_OPTION_IPV6_V6ONLY_MODIFY;
		if (!bUseIPv4)
		{
			// IPv6-only: set IPV6_V6ONLY=1 to reject IPv4 connections.
			Info.options |= LWS_SERVER_OPTION_IPV6_V6ONLY_VALUE;
		}
		// else: dual-stack (IPV6_V6ONLY=0) — IPv4 connections accepted via ::ffff: mapped addresses.
		// This is required on Windows where the default is IPV6_V6ONLY=1.
	}
	...
	switch (Reason)
	{
		...
		case LWS_CALLBACK_FILTER_PROTOCOL_CONNECTION:
		{
			if (Server && Server->FilterConnectionCallback.IsBound())
			{
				...
				constexpr int32 IPAddressLength = 46; // INET6_ADDRSTRLEN — large enough for IPv6
