...
bool FModelInstanceRDG::LoadModel(TConstArrayView<uint8> ModelData, FNNERuntimeRDGDataModelFormat& Format, int32 GuidAndVersionSize)
{
	...
	for (int32 Idx = 0; Idx < Format.Tensors.Num(); ++Idx)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Tensor %ls has invalid format: Data type None is reserved for empty tensors.", *SymbolicTensor.GetName());
			return false;
		}
		...
		else if (FormatTensorDesc.Type == ENNERuntimeRDGDataTensorType::Initializer)
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Weight tensor %ls should have a concrete shape", *SymbolicTensor.GetName());
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Weight %ls has incorrect size. Expected %lld bytes, got %lld", *SymbolicTensor.GetName(), FormatTensorDesc.DataSize, WeightRDG.GetDataSize());
				return false;
			}
...
FModelInstanceRDG::ESetInputTensorShapesStatus FModelInstanceRDG::SetInputTensorShapes(TConstArrayView<NNE::FTensorShape> InInputShapes)
{
	...
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Number of input shapes does not match number of input tensors");
	...
	for (int32 i = 0; i < InInputShapes.Num(); ++i)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Input shape does not match input tensor %ls of index %d", *SymbolicDesc.GetName(), i);
			return ESetInputTensorShapesStatus::Fail;
		}
	}
...
FModelInstanceRDG::EEnqueueRDGStatus FModelInstanceRDG::EnqueueRDG(FRDGBuilder& RDGBuilder, TConstArrayView<NNE::FTensorBindingRDG> InInputBindings, TConstArrayView<NNE::FTensorBindingRDG> InOutputBindings)
{
	...
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Input shapes are not set, please call SetInputTensorShapes.");
	...
	if (Res != -1)
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Invalid buffer for input tensor binding at index %d", Res);
		return EEnqueueRDGStatus::Fail;
	}
	...
	if (Res != -1)
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Invalid buffer for output tensor binding at index %d", Res);
		return EEnqueueRDGStatus::Fail;
	}
...
int32 FModelInstanceRDG::SetTensors(FRDGBuilder& GraphBuilder, FTensorRDGArray& InTensorRDGs, TConstArrayView<NNE::FTensorBindingRDG> InBindings)
{
	...
	for (int32 Idx = 0; Idx < InBindings.Num(); ++Idx)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "nullptr buffer encountered.");
			return Idx;
		}
		if (Binding.Buffer->GetSize() % NNERUNTIMERDGHLSL_BUFFER_LENGTH_ALIGNMENT != 0)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Buffer has size %i which is not a multiple of %i", Binding.Buffer->GetSize(), NNERUNTIMERDGHLSL_BUFFER_LENGTH_ALIGNMENT);
			return Idx;
		}
