		...
		static FAutoConsoleCommand GetAutomationRuntimeFilterCommand(
			...
			FConsoleCommandWithArgsDelegate::CreateStatic(
				[](const TArray< FString >& Args)
				{
					...
					UE_LOGF(LogNNERuntimeRDGHlsl, Display, "Operators support matrix: \n%ls", *SupportMatrix);
				}
			)
		);
	} // namespace ConsoleCommands
	...
	namespace Details
	{
		UNNERuntimeRDGHlslImpl::ECanCreateModelDataStatus CheckCanCreateModelData(bool bShouldLog, const FString& FileType, TConstArrayView64<uint8> FileData, const TMap<FString, TConstArrayView64<uint8>>& AdditionalFileData, const FGuid& FileId, const ITargetPlatform* TargetPlatform)
		{
			...
			if (FileType.Compare("onnx", ESearchCase::IgnoreCase) != 0)
			{
				if (bShouldLog)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Cannot create the model data with id %ls (Filetype: %ls), Only 'onnx' file type is supported", *FileId.ToString(EGuidFormats::Digits).ToLower(), *FileType);
				}
			...
			if ((TArray<uint8>::SizeType)FileData.Num() != FileData.Num())
			{
				if (bShouldLog)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Cannot create the model data with id %ls (Filetype: %ls), models > 2GBs are not supported", *FileId.ToString(EGuidFormats::Digits).ToLower(), *FileType);
				}
				return UNNERuntimeRDGHlslImpl::ECanCreateModelDataStatus::Fail;
			}
			...
			if (!AdditionalFileData.IsEmpty())
			{
				if (bShouldLog)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Cannot create the model data with id %ls (Filetype: %ls), external data not supported at the moment, please convert the model to internal storage. See https://onnx.ai/onnx/repo-docs/ExternalData.html", *FileId.ToString(EGuidFormats::Digits).ToLower(), *FileType);
				}
				return UNNERuntimeRDGHlslImpl::ECanCreateModelDataStatus::Fail;
			}
			...
			if (bShouldLog)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Cannot create the model data with id %ls (Filetype: %ls), NNERuntimeRDGUtils is not available on this platform", *FileId.ToString(EGuidFormats::Digits).ToLower(), *FileType);
			}
			return UNNERuntimeRDGHlslImpl::ECanCreateModelDataStatus::Fail;
#endif
		}
...
TSharedPtr<UE::NNE::IModelRDG> UNNERuntimeRDGHlslImplRDG::CreateModelRDG(const TObjectPtr<UNNEModelData> ModelData)
{
	...
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Cannot create a model from the model data with id %ls", *ModelData->GetFileId().ToString(EGuidFormats::Digits));
