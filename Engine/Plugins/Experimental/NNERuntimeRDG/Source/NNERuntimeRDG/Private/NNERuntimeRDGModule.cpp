...
namespace UE::NNERuntimeRDG::Private::Details
{
	bool IsInferenceSupported()
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Display, "Minimum feature level required is SM5 for current RHI platform.");
			bResult = false;
		}

		if (!GRHISupportsWaveOperations)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Display, "Current RHI platform doesn't support wave operations.");
			bResult = false;
		}

		if (!GRHIGlobals.SupportsNative16BitOps)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Display, "Current RHI platform doesn't support native 16-bit operations.");
			bResult = false;
		}
#endif
...
void FNNERuntimeRDGModule::StartupModule()
{
	...
	if (FDataDrivenShaderPlatformInfo::GetSupportsNNEShaders(GMaxRHIShaderPlatform))
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Display, "Not registering inference for runtime because current hardware is incompatible, consider bypassing by setting the define NNE_FORCE_HARDWARE_SUPPORTS_HLSL.");
		}
	}
	else
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Display, "Not registering inference for runtime because current RHI shader platform is not enabled, consider setting the flag bSupportsNNEShaders in DataDrivenPlatformInfo.");
	}
