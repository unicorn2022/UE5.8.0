...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			if(CoordTransMode == UE::NNEHlslShaders::Internal::ECoordTransMode::TfCropAndResize)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: `roi` tensor could not be made constant. (name %ls).", *Roi.GetName());
					return -1;
				}
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: `roi` tensor (name %ls) must have 2 * N length.", *Roi.GetName());
					return -1;
				}
			}
			...
			if(InputTensors.Num() == 3)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: `scales` tensor could not be made constant. (name %ls).", *Scales.GetName());
					return -1;
				}
			...
			if(InputTensors.Num() == 4)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: `sizes` tensor could not be made constant. (name %ls).", *Sizes.GetName());
					return -1;
				}
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: antialias not yet supported.");
				return false;
			}
			...
			for (int32& Axis : Axes)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: 'Axes' attribute should contain value be in the range [-r,r] with r being the rank of the input (name: %ls) however got %d while rank is %d.", *InputTensorDescs[0].GetName(), Axis, InputTensorDescs[0].GetShape().Rank());
					return false;
				}
			...
			if(Axes.Num() != 0)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: `axes` attribute not yet supported.");
				return false;
			}
			...
			if(ExcludeOutside == 1)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: `exclude_outside` attribute not yet supported.");
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Cubic interpolation not yet supported.");
				return false;
			}
	...
	bool ValidateResizeOperator(const NNERuntimeRDGData::Internal::FAttributeMap& AttributeMap, TConstArrayView<ENNETensorDataType> InputTypes, TConstArrayView<NNE::FSymbolicTensorShape> InputShapes)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Got a total of '%d' inputs but should be between 3 and 4.", InputTypes.Num());
			return false;
		}

		if(InputShapes[0].Rank() < 1)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Input tensor must have rank >= 1.");
			return false;
		}
		...
		if (InputTypes[1] != ENNETensorDataType::None)
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Roi tensor must be a 1-D tensor.");
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Roi tensor must have dimension 2*N (where N is the input rank) when `coordinate_transformation_mode` is `tf_crop_and_resize`.");
				return false;
			}
		}
		...
		if(InputTypes.Num() == 3)
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Scales tensor must be a 1-D tensor.");
				return false;
			}
			if(InputShapes[2].GetData()[0] != InputShapes[0].Rank())
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Scales tensor must have dimension N (where N is the input rank).");
				return false;
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Scales tensor must be empty (i.e. empty name and data type 'None') when Sizes is specified.");
				return false;
			}
			if(InputShapes[3].Rank() != 1)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Sizes tensor must be a 1-D tensor.");
				return false;
			}
			if(InputShapes[3].GetData()[0] != InputShapes[0].Rank())
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Resize: Sizes tensor must have dimension N (where N is the input rank).");
				return false;
			}
		}
