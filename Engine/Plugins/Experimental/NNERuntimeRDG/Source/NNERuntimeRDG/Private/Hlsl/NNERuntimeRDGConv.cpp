...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Conv: Kernel size, strides, dilations combination is not supported. Kernel tensor: %ls.", *InputTensors[1]->GetName());
				return -1;
			}
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Conv: First input should be at least of rank 2");
				return false;
			}
			if (Weights.GetShape().Rank() != Input.GetShape().Rank())
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Conv: First and second inputs should be of same ranks");
				return false;
			}
			if (Output.GetShape().Rank() != Input.GetShape().Rank())
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Conv: First and output should be of same ranks");
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Conv: Strides count doesn't match the number of spatial dimensions");
				return false;
			}
	...
	bool ValidateConvOperator(const NNERuntimeRDGData::Internal::FAttributeMap& AttributeMap, TConstArrayView<ENNETensorDataType> InputTypes, TConstArrayView<NNE::FSymbolicTensorShape> InputShapes)
	{
		...
		for (ENNETensorDataType InputType : InputTypes)
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Conv: All input tensor data types have to match each other");
				return false;
			}
		}
		return bIsValid;
