...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			for (int32 i = 0; i < OutputRank; ++i)
			{
				...
				for (int32 InputIdx = 0; InputIdx < NumInput; ++InputIdx)
				{
					...
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ElementWiseVariadic: Could not compute shape for element wise variadic op, input shapes are not compatible");
						return -1;
					}
	...
	bool ValidateElementWiseVariadicOperator(const NNERuntimeRDGData::Internal::FAttributeMap& AttributeMap, TConstArrayView<ENNETensorDataType> InputTypes, TConstArrayView<NNE::FSymbolicTensorShape> InputShapes)
	{
		...
		if (InputTypes.Num() == 0)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Error, "ElementWiseVariadic: Operator requires at least 1 input");
			bIsValid = false;
		}
		for (int32 i = 0; i < InputTypes.Num(); ++i)
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ElementWiseVariadic: Operator input '%d' of type '%d' is not supported, should be float at the moment.", i, int(InputTypes[i]));
				bIsValid = false;
			}
		}
