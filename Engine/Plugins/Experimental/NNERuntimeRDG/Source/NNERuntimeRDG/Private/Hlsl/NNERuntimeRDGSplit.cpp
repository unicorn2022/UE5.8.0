	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			if(Version >= 13 && InputTensors.Num() == 2)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Split: split tensor need to be CPU-constant in order to compute output shapes.");
					return -1;
				}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Split: Sum of split values not equal to split axis' dimension.");
				return -1;
			}

			if(Splits.Num() > FSplitConstants::MAX_NUM_SPLITS)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Split: Number of splits (%d) exceeds maximum allowed (%d).", Splits.Num(), FSplitConstants::MAX_NUM_SPLITS);
				return -1;
			}
			
			if(OutputTensors.Num() != Splits.Num())
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Split: Number of output tensors differs from number of splits provided.");
				return -1;
			}
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Split: Attribute 'Axis' should be in the range [-r,r] with r being the rank of the input (name: %ls) however got %d while rank is %d.", *InputTensorDescs[0].GetName(), Axis, InputRank);
				return false;
			}
			// Canonicalize Axis
			if (Axis < 0)
			...
			if(Version >= 18 && Attributes.GetAttributeValue(TEXT("num_outputs")))
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Split: Attribute 'num_outputs' doesn't match number of output tensors. Value: %d. Number of output tensors: %d.", NumOutputs, OutputTensorDescs.Num());
					return false;
				}
			}
