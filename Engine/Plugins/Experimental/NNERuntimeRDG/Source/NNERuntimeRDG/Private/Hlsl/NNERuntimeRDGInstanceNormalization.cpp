...
int ValidateInput(const UE::NNE::FTensorShape& Input, const UE::NNE::FTensorShape& Scale, const UE::NNE::FTensorShape& Bias)
{
	if (Scale.Rank() != 1)
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "InstanceNormalization: Input scale should be of rank 1: %d", Scale.Rank());
		return -1;
	}
	if (Scale.GetData()[0] != Input.GetData()[1])
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "InstanceNormalization: Input scale size should be equal to channel count: %d vs %d", Scale.GetData()[0], Input.GetData()[1]);
		return -1;
	}

	if (Bias.Rank() != 1)
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "InstanceNormalization: Input B should be of rank 1: %d", Bias.Rank());
		return -1;
	}
	if (Bias.GetData()[0] != Input.GetData()[1])
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "InstanceNormalization: Input B size should be equal to channel count: : %d vs %d", Bias.GetData()[0], Input.GetData()[1]);
		return -1;
	}
...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			if (InputRank < 3)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "InstanceNormalization input data should be at least of rank 3 but got: %d", InputRank);
				return false;
			}

			if (InputRank != OutputTensorDescs[0].GetShape().Rank())
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "InstanceNormalization requires the output to have the same rank as the input.");
				return false;
			}
