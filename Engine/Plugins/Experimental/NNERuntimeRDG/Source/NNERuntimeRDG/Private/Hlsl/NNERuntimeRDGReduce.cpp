...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			if constexpr (bAxesAsInput)
			{
				if(InputTensors.Num() == 2)
				{
					...
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Reduce: Tensor `axes` (name: %ls) must be CPU constant.", *InputTensors[1]->GetName());
						return false;
					}
			...
			for (int32& Axis : Axes)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Reduce: 'Axes' attribute should contain value be in the range [-r,r] with r being the rank of the input (name: %ls) however got %d while rank is %d.", *InputTensors[0]->GetName(), Axis, InputRank);
					return false;
				}
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Reduce: Attribute `axes` cannot be empty.");
				return false;
			}
