...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Reshape: Input 'Shape' (name: %ls) should be constant for shape inference to succeed, however it is not.", *ShapeTensor.GetName());
				return -1;
			}
