	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			for (int32 Idx = Axes.Num() - 1; Idx >= 0; --Idx)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Squeeze: Axe %d for 'Data' (name: %ls) should be targeting a dimension of size 1 but it is %d.", Axe, *X.GetName(), OutputShapeData[Axe]);
					return -1;
				}
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			for (int32 Axe : Axes)
			{
				if (Axe < 0)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Squeeze: Does not support negative axes");
					return false;
				}
				if (Axe >= InputTensorDescs[0].GetShape().Rank())
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Squeeze: Does not support axes greater than the number of dimensions of the input tensor shape");
					return false;
				}
			}
