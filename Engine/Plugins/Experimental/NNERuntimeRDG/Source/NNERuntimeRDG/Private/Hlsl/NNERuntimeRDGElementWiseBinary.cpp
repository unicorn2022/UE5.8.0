	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			for (int32 i = 0; i < OutputRank; ++i)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ElementWiseBinary: Could not compute the shape, input shapes are not compatible");
					return -1;
				}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ElementWiseBinary: Output tensor could not be made constant nor it was of float type. Only floats are supported at the moment.");
				return -1;
			}
