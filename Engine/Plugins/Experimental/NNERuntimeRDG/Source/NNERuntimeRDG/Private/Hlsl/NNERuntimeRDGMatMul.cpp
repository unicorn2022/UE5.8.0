...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			if (InputA.Rank() < 2)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Matmul: First input should be at least of rank 2");
				return -1;
			}
			if (InputB.Rank() < 2)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Matmul: Second input should be at least of rank 2");
				return -1;
			}
			if (InputA.GetData()[InputA.Rank() - 1] != InputB.GetData()[InputB.Rank() - 2])
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Matmul: First input last dimension should be equal to second input last dimension");
				return -1;
			}
