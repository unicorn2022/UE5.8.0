...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Softmax: Input tensor should be at least 2-D (but got rank %d)", InputDimensions);
				return false;
			}

			if (InputTensorDescs[0].GetShape().Rank() != OutputTensorDescs[0].GetShape().Rank())
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Softwax: Output should have the same rank as the input.");
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Softmax: Invalid axis (should in the interval [%d, %d], but got %d)", AxisLow, AxisHigh, Axis);
				return false;
			}
			if(Axis < 0)
			{
