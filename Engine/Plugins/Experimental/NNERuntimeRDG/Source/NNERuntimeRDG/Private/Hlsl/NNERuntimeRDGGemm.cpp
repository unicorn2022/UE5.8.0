...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Gemm: First input should be of rank 2");
				return false;
			}
			if (InputB.GetShape().Rank() != 2)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Gemm: Second input should be of rank 2");
				return false;
			}
			if (InputTensorDescs.Num() == 3)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Gemm: Third input should be of rank 2 or less");
					return false;
				}
			}
