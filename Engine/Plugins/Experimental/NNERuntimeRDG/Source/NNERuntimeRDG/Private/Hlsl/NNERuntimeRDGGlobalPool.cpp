...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "GlobalPool: Operators requires the output to have the same rank as the input.");
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "GlobalPool: Operators requires input tensor to be at least 3-D (but got rank %d)", InputRank);
				return false;
			}
