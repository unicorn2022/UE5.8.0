...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Upsample: Input 'Scale' (name: %ls) should be constant for shape inference to succeed, however it is not constant.", *Scales.GetName());
				return -1;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Upsample: Input 'Scale' (name: %ls) have %d elements. While it should be the same as the rank of input 'X' (name : %ls) witch is %d", *Scales.GetName(), ScalesData.Num(), *Input.GetName(), Input.GetShape().Rank());
				return -1;
			}
			...
			for (int32 i = 0; i < ScalesData.Num(); i++)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Upsample: Input 'Scale' takes values greater than or equal to 1.");
					return -1;
				}

				if (Mode.Equals(AttrValue::Linear) && i < ScalesData.Num() - 3 && ScalesData[i] > 1.0f)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Upsample: 'Linear mode' only support up to trilinear interpolation, meaning input 'Scale' values for the outermost dimensions (Rank - 3) are 1.");
					return -1;
				}
			}
	...
	bool ValidateUpsampleOperator(const NNERuntimeRDGData::Internal::FAttributeMap& AttributeMap, TConstArrayView<ENNETensorDataType> InputTypes, TConstArrayView<NNE::FSymbolicTensorShape> InputShapes)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Upsample: Only supports %ls or %ls as value for attribute %ls", AttrValue::Nearest, AttrValue::Linear, AttrName::Mode);
			return false;
		}
