	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			if constexpr (Version >= 11)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Pad: Input 'pads' (name: %ls) should be constant.", *PadsTensor->GetName());
					return -1;
				}
				...
				if (InputTensors.Num() >= 3 && !InputTensors[2]->IsEmpty())
				{
					...
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Pad: Input 'constant_value' (name: %ls) should be constant.", *ValueTensor.GetName());
						return -1;
					}

					if(ValueTensor.GetPreparedData<float>().Num() != 1)
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Pad: Input 'constant_value' (name: %ls) should be scalar, however it is not.", *ValueTensor.GetName());
						return -1;
					}
				...
				if (InputTensors.Num() >= 4 && !InputTensors[3]->IsEmpty())
				{
					...
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Pad: Input 'axes' (name: %ls) should be constant.", *AxesTensor->GetName());
						return -1;
					}
				...
				else
				{
					if (!PadsTensor->HasPreparedData())
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Pad: 'pads' attribute length (%d) should be twice the rank of input X (%d).", 0, X.GetShape().Rank());
						return -1;
					}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Pad: 'pads' attribute length (%d) should be twice the rank of input X (%d).", Pads.Num(), X.GetShape().Rank());
				return false;
			}
			...
			for (int32 i = 0; i < X.GetShape().Rank(); ++i)
			{
				...
				if (OutputDim < 1)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Pad: Cannot reduce dimension below 1, but would for tensor (name:%ls) at rank %d of size %d with prepad %d and postpad %d.", *X.GetName(), i, X.GetShape().GetData()[i], PrePad, PostPad);
					return -1;
				}
