	...
	public:
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			if constexpr (PoolOperatorType == UE::NNEHlslShaders::Internal::EPoolOperatorType::MAX_POOL)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "MaxPool: 2nd optional output 'Indices' is not supported.");
					return false;
				}
			}
			else
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "%ls : Input should be at least of rank 3, to have 1+ spatial dimension(s) but is of rank %d", GetOperatorName(), Input.GetShape().Rank());
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "%ls: KernelShape should have as many elements as the spatial dimensions of the input, got %d while input have %d.", GetOperatorName(), KernelShape.Num(), NumSpatialDimensions);
				return false;
			}
			if (Strides.Num() != NumSpatialDimensions)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "%ls: Strides should have as many elements as the spatial dimensions of the input, got %d while input have %d.", GetOperatorName(), Strides.Num(), NumSpatialDimensions);
				return false;
			}
			if (Dilations.Num() != NumSpatialDimensions)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "%ls: Dilations should have as many elements as the spatial dimensions of the input, got %d while input have %d.", GetOperatorName(), Dilations.Num(), NumSpatialDimensions);
				return false;
			}
			if (Pads.Num() != 2*NumSpatialDimensions)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "%ls: Pads should have twice as many elements as the spatial dimensions of the input, got %d while input have %d.", GetOperatorName(), Pads.Num(), NumSpatialDimensions);
				return false;
			}
