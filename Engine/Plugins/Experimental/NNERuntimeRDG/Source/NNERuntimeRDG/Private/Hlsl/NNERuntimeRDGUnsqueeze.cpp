	...
	public:
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			for (int32 Axe : Axes)
			{
				if (Axe < 0)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Unsqueeze: Does not support negative axes");
					return false;
				}
				if (Axe >= InputTensorDescs[0].GetShape().Rank() + Axes.Num())
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Unsqueeze: Does not support axes greater than the number of dimensions of the resulting tensor shape");
					return false;
				}
			}
