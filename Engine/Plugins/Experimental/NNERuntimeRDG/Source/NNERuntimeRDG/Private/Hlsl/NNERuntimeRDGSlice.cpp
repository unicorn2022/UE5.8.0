	...
	private:
		...
		bool TryGetAttributesFromConstantTensors(TConstArrayView<FTensorRef> InputTensors)
		{
			...
			if (InputTensors.Num() >= 4)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Axes' input tensor (%ls) is only supported as a constant integer tensor but it is not.", *InputTensors[3]->GetName());
					return false;
				}
				if(AxesAttr.Num() > InputTensors[0]->GetShape().Rank() || AxesAttr.Num() < 1)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Axes' input tensor (%ls) contains %d elements but input rank is %d.", *InputTensors[3]->GetName(), AxesAttr.Num(), InputTensors[0]->GetShape().Rank());
					return false;
				}
			}
			else
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Starts' input tensor (%ls) is only supported as a constant integer tensor but it is not.", *InputTensors[1]->GetName());
				return false;
			}
			if(StartsAttr.Num() != NumAxes)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Starts' input tensor (%ls) contains %d elements but number of axes is %d.", *InputTensors[1]->GetName(), StartsAttr.Num(), NumAxes);
				return false;
			}

			if (!OperatorHelper::GetInt32ArrayFromConstTensor(EndsAttr, InputTensors[2]))
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Ends' input tensor (%ls) is only supported as a constant integer tensor but it is not.", *InputTensors[2]->GetName());
				return false;
			}
			if(EndsAttr.Num() != NumAxes)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Ends' input tensor (%ls) contains %d elements but number of axes is %d.", *InputTensors[2]->GetName(), EndsAttr.Num(), NumAxes);
				return false;
			}
			...
			if (InputTensors.Num() == 5)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Steps' input tensor (%ls) is only supported as a constant integer tensor but it is not.", *InputTensors[4]->GetName());
					return false;
				}
				if(StepsAttr.Num() != NumAxes)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Steps' input tensor (%ls) contains %d elements but number of axes is %d.", *InputTensors[4]->GetName(), StepsAttr.Num(), NumAxes);
					return false;
				}
				for(const int32 Value : StepsAttr)
				{
					if(Value == 0)
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: 'Steps' tensor (%ls) can only contain non-0 integers.", *InputTensors[4]->GetName());
						return false;
					}
				}
			}
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			for (int32 Idx = 0; Idx < InputRank; ++Idx)
			{
				...
				if(RangeSize < 1)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: Start (tensor: %ls) and end (tensor: %ls) indices are incompatible with step direction for dimension %d.", *InputTensors[1]->GetName(), *InputTensors[2]->GetName(), Idx);
					return -1;
				}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: Output tensor (%ls) could not be constant-folded from input.", *OutputTensors[0]->GetName());
				return -1;
			}
		...
		virtual bool Initialize(TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& Attributes) override
		{
			...
			if constexpr (OpVersion == 1)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Slice: Starts, Ends and Axes must be of the same size.");
					return false;
				}
