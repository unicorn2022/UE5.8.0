...
namespace UE::NNERuntimeRDG::Private::Hlsl
{
	...
	public:
		...
		virtual int PrepareOutputs(TConstArrayView<FTensorRef> InputTensors, TArrayView<FTensorRef> OutputTensors) override
		{
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ScatterND: Last dimension in the shape of `indices` (%d) must be less than input rank (%d).", IndicesShape.Last(), InputShape.Num());
				return -1;
			}
			if (UpdatesShape.Num() != (IndicesShape.Num() - 1) + (InputShape.Num() - IndicesShape.Last()))
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ScatterND: Rank of `updates` should equal (q - 1) + (r - k), with q rank of `indices`, r rank of `data` and `k` last dimension of `indices`' shape.");
				return -1;
			}
			if (!Algo::Compare( UpdatesShape.Slice(0, IndicesShape.Num() - 1), IndicesShape.Slice(0, IndicesShape.Num() - 1) ))
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ScatterND: Updates.shape[0:q-1] should match indices.shape[0:q-1].");
				return -1;
			}
			if (!Algo::Compare( UpdatesShape.RightChop(IndicesShape.Num() - 1), InputShape.RightChop(IndicesShape.Last()) ))
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ScatterND: Updates.shape[q-1:] should match data.shape[k:].");
				return -1;
			}
			...
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ScatterND: Only supports input tensors up to a volume of %u.", MAX_int32);
				return -1;
			}
