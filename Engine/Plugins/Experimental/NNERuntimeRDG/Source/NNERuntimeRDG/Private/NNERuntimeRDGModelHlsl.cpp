...
FOperatorHlsl* OpCreate(const FOperatorDesc& OpDesc, TConstArrayView<NNE::FTensorDesc> InputTensorDescs, TConstArrayView<NNE::FTensorDesc> OutputTensorDescs, const NNERuntimeRDGData::Internal::FAttributeMap& AttributeMap)
{
	...
	if (!CreateFn)
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Operator registry failed to find operator: %ls", *OpDesc.GetFullName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Error initializing operator: %ls", *OpDesc.GetFullName());
		delete Op;
		return nullptr;
	}
...
bool FModelInstance::Init(TConstArrayView<uint8> ModelData)
{
	...
	for (int32 Idx = 0; Idx < Format.Operators.Num(); ++Idx)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Failed to create operator:%ls", *OperatorDesc.GetFullName());
...
int FModelInstance::PrepareTensorShapesAndData()
{
	...
	if (Operators.Num() == 0)
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "No operators in model");
		return -1;
	}
	...
	for (int32 Idx = 0; Idx < Operators.Num(); ++Idx)
	{
		...
		if (Op->PrepareOutputs(InputTensors, OutputTensors) != 0)
		{
			...
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Could not deduce tensor shapes for this model during shape inference, HLSL runtime wont support the model as it need to precompute all shapes for performance reasons.");
