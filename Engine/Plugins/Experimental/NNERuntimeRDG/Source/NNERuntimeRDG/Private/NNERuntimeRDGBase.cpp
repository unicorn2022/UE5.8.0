...
bool FInputValidator::Validate(TConstArrayView<ENNETensorDataType> InputTypes)
{
	...
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Required '%d' inputs but found '%d'.", NumRequiredInput, InputTypes.Num());
		bAreInputValid = false;
	}
	if (InputTypes.Num() > NumRequiredInput+NumOptionalInput)
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Got a total of '%d' inputs but should have '%d' maximum.", InputTypes.Num(), NumRequiredInput + NumOptionalInput);
		bAreInputValid = false;
	}
	...
	for (int32 Idx = 0; Idx < NumInputsToValidate; ++Idx)
	{
			...
			!(InputTypes[Idx] == ENNETensorDataType::None && Idx >= NumRequiredInput))
		{
			...
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Input at index '%d' (from template T%d) is of type '%ls' witch is not supported for that input.", Idx, TemplateIdx, *TargetType);
			bAreInputValid = false;
		}
	}
...
bool FAttributeValidator::Validate(const NNERuntimeRDGData::Internal::FAttributeMap& AttributesToValidate)
{
	...
	for (int32 Idx = 0; Idx < RequiredAttributes.Num(); ++Idx)
	{
		...
		if (FoundAttribute == nullptr)
		{
			...
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Required attribute '%ls' not found.",
				*RequiredAttributes[Idx].Name);
		}
		else if (RequiredAttributes[Idx].Type != FoundAttribute->GetType())
		{
			bAreAttributesValid = false;
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Required attribute '%ls' type '%d' does not match expected type '%d'.",
				*RequiredAttributes[Idx].Name,
	...
	for (int32 Idx = 0; Idx < OptionalAttributes.Num(); ++Idx)
	{
		...
		{
			bAreAttributesValid = false;
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Optional attribute '%ls' type '%d' does not match expected type '%d'.",
	...
	for (int32 Idx = 0; Idx < AttributesToValidate.Num(); ++Idx)
	{
		...
		{
			bAreAttributesValid = false;
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Found unsupported attribute '%ls'.", *Name);
		}
	}
