...
public:
	...
	OperatorValidateFunc OpFindValidation(const FOperatorDesc& OpDesc)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Operator: %ls is not registered", *OpDesc.GetFullName());
			return nullptr;
		}
	...
	bool OpAdd(const FOperatorDesc& OpDesc, OperatorCreateFunc CreateFunc, OperatorValidateFunc ValidateFunc = AlwaysValidValidationFunction)
	{
		...
		if (VersionToFunctions != nullptr)
		{
			...
			if(OperatorFunctions != nullptr)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Operator is already registered: %ls", *OpDesc.GetFullName());
				return false;
			}
			else if(!OpDesc.Version.IsSet() || VersionToFunctions->Find(TOptional<TOperatorVersionType>()) != nullptr)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Operator %ls is unversioned, can't register a version of it", *FOperatorDesc{{OpDesc}}.GetFullName());
				return false;
			}
...
public:
	...
	virtual bool ValidateModel(TConstArrayView<uint8> InputModel) const override
	{
		...
		for (int32 Idx = 0; Idx < Format.Operators.Num(); ++Idx)
		{
			...
			if (!ValidationFn)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "OperatorRegistry failed to find validation for operator:%ls", *OpType);
				return false;
			}
			
			if (!ValidationFn(AttributeMap, InputTensorTypes, InputTensorShapes))
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "OperatorRegistry failed to validate operator:%ls", *OpType);
				return false;
			}
		}
