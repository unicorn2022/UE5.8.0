...
namespace ModelOptimizerNNEHelper
{
	...
	bool GetTensorInfoFromONNXInitializer(const onnx::TensorProto& Tensor, TArray<int32>& Shape, ENNETensorDataType& DataType, TensorInfoData& OutData)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "External data not supported.");
			return false;
		}
	...
	static bool GetAttributeTensorFromONNXInitializer(const onnx::TensorProto& InTensor, NNERuntimeRDGData::Internal::FAttributeTensor& OutTensor)
	{
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Tensor data could not be loaded");
			return false;
		}
		if (Shape.Num() > FTensorShape::MaxRank)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Tensor shape of rank %i exceeds MaxRank %i", Shape.Num(), FTensorShape::MaxRank);
			return false;
		}
		...
		for (int32 value : Shape)
		{
			if (value < 0)
			{
				UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Tensor shape has negative value");
				return false;
			}
			UIntShape.Add(value);
		}
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Tensor data size %zu doesn't match expected data size %i",
	...
	bool BuildNNEFormatFromONNX(TArray<uint8>& ONNXData, TArray<uint8>& NNEData)
	{
		...
		if (!result)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Could not parse the input model as a ModelProto.");
			return false;
		}

		if (ModelProto.opset_import_size() < 1)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Could not read opset version from ONNX.");
			return false;
		}
		...
		for (const onnx::ValueInfoProto& Output : Graph.output())
		{
			...
			if (Initializer)
			{
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Tensor data could not be loaded for weights of output node '%ls'", ANSI_TO_TCHAR(Output.name().c_str()));
					return false;
				}

				if (DataType != InitializerDataType)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Initializer type does not match output type for output tensor %ls.", ANSI_TO_TCHAR(Output.name().c_str()));
					return false;
				}
				...
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Initializer shape does not match output shape for output tensor %ls.", ANSI_TO_TCHAR(Output.name().c_str()));
					return false;
				}
		...
		for (const onnx::NodeProto& Node : Graph.node())
		{
			...
			for (const onnx::AttributeProto& Attribute : Node.attribute())
			{
				...
				else if (Attribute.type() == onnx::AttributeProto::INTS)
				{
					...
					for (int64 Value64 : Attribute.ints())
					{
						...
						{
							UE_LOGF(LogNNERuntimeRDGHlsl, Display, "Overflow detected when converting to int32 attribute '%ls' in node '%ls'", *AttributeName, *NNEOpType);
						}
				...
				else if (Attribute.type() == onnx::AttributeProto::TENSOR)
				{
					...
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Failed to get data from attribute %ls in operator %ls", *AttributeName, *NNEOpName);
						return false;
					}
				...
				else if (Attribute.type() == onnx::AttributeProto::TENSORS)
				{
					...
					for (const onnx::TensorProto& Initializer : Attribute.tensors())
					{
						...
						{
							UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Failed to get data from tensor at index %i of attribute %ls in operator %ls", TensorIndex, *AttributeName, *NNEOpName);
							return false;
						}
						Values.Add(Tensor);
						TensorIndex++;
				...
				else
				{
					...
					UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Unsupported attribute type for attribute '%ls' in node '%ls' of type '%ls'", *AttributeName, *NNEOpName, *NNEOpType);
				}
			}
			...
			for (const std::string& TensorName : Node.input())
			{
				...
				if (Initializer)
				{
					...
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Tensor data could not be loaded for weight '%ls' in node '%ls' of type '%ls'", ANSI_TO_TCHAR(TensorName.c_str()), *NNEOpName, *NNEOpType);
						return false;
					}
					TConstArrayView<uint8> DataView = Data.GetArrayView();
				...
				else if (!TensorName.empty())
				{
					...
					if (!ValueInfoProto)
					{
						UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Could not find Tensor ValueInfoProto or Initializer in graph for input '%ls' in node '%ls' of type '%ls'", ANSI_TO_TCHAR(TensorName.c_str()), *NNEOpName, *NNEOpType);
						return false;
					}
			...
			for (const std::string& TensorName : Node.output())
			{
				...
				if (!ValueInfoProto)
				{
					UE_LOGF(LogNNERuntimeRDGHlsl, Error, "Could not find Tensor ValueInfoProto in graph for output '%ls' in node '%ls' of type '%ls'", ANSI_TO_TCHAR(TensorName.c_str()), *NNEOpName, *NNEOpType);
					return false;
				}
				ENNETensorDataType DataType;
				TArray<int32> Shape;
...
bool FModelOptimizerONNXToNNERT::Optimize(TConstArrayView<uint8> InputModel, TArray<uint8>& OptimizedModel)
{
	...
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Error while optimizing the ONNX model before convertion to NNERT format.");
		return false;
	}

	if (!ModelOptimizerNNEHelper::BuildNNEFormatFromONNX(OptimizedONNXModel, OptimizedModel))
	{
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Error while building NNERT Model from ONNX.");
		return false;
	}
