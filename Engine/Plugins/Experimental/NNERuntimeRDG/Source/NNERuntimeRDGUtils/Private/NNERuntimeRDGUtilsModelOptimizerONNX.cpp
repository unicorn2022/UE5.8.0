...
public:
	...
	virtual bool ApplyPass(TArray<uint8>& ModelData) const
	{
		...
		if (!result)
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "%ls could not parse the input model as a ModelProto.", *(GetName()));
			return false;
		}
...
public:
	...
	virtual bool ApplyPass(TArray<uint8>& ModelData) const
	{
		...
		UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "ONNX Shape inference can't be run as exception are disabled.");
		return true;
#else
		...
		{
			UE_LOGF(LogNNERuntimeRDGHlsl, Warning, "Shape inference failed with : %ls.", ANSI_TO_TCHAR(e.what()));
		}
#endif
