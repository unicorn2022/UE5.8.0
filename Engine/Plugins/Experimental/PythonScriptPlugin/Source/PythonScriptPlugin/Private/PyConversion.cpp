...
#define NATIVIZE_INLINE_PROPERTY(PROPTYPE)											\
	...
	if (const FOptionalProperty* CastProp = CastField<FOptionalProperty>(Prop))
	{
		FPyConversionResult Result = FPyConversionResult::Failure();

		if (PyObj == Py_None)
		{
			EmitPropertyChangeNotifications(InChangeNotify, !CastProp->IsSet(ValueAddr), [&]()
			{
				for (void* ArchInstValueAddr : InArchetypeInstValueAddrs)
				{
					CastProp->MarkUnset(ArchInstValueAddr);
				}
				CastProp->MarkUnset(ValueAddr);
			});

			Result = FPyConversionResult::Success();
		}
		else
		{
			PyUtil::FPropValueOnScope NewValue(PyUtil::FConstPropOnScope::ExternalReference(CastProp));
			Result = NativizeProperty_Direct(PyObj, CastProp->GetValueProperty(), CastProp->MarkSetAndGetInitializedValuePointerToReplace(NewValue.GetValue()), {}, nullptr, SetErrorState);

			if (Result)
			{
				EmitPropertyChangeNotifications(InChangeNotify, CastProp->Identical(ValueAddr, NewValue.GetValue(), PPF_None), [&]()
				{
					for (void* ArchInstValueAddr : InArchetypeInstValueAddrs)
					{
						CastProp->CopySingleValue(ArchInstValueAddr, NewValue.GetValue());
					}
					CastProp->CopySingleValue(ValueAddr, NewValue.GetValue());
				});
			}
		}
...
#define PYTHONIZE_GETTER_PROPERTY(PROPTYPE)											\
	...
	if (const FOptionalProperty* CastProp = CastField<FOptionalProperty>(Prop))
	{
		if (CastProp->IsSet(ValueAddr))
		{
			PYCONVERSION_PROPERTY_RETURN(PythonizeProperty_Direct(CastProp->GetValueProperty(), CastProp->GetValuePointerForRead(ValueAddr), OutPyObj, ConversionMethod, OwnerPyObj, SetErrorState));
		}
		else
		{
			Py_INCREF(Py_None);
