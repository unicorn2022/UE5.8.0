...
bool FPipInstall::InitPipInstall()
{
	...
	{
		UE_LOGF(LogPython, Display, "No pip-enabled plugins with python dependencies found, skipping");
...
FPipInstall::FPipInstall()
{
	...
	if (!EnvInstallPath.IsEmpty())
	{
		...
		{
			UE_LOGF(LogPython, Warning, "UE_PIPINSTALL_PATH: Invalid path specified: %ls", *ErrReason.ToString());
		}
	}
...
void FPipInstall::CheckRemoveOrphanedPackages(const FString& SitePackagesPath)
{
	...
	if (DirVisit.bOrphan)
	{
		...
		UE_LOGF(LogPython, Log, "PipInstall found orphan plugin site-package directory: %ls (removing)", *SitePackagesPath);
	...
	else
	{
		...
		{
			UE_LOGF(LogPython, Log, "PipInstall found orphan plugin site-package directory: %ls (removing)", *OrphanDir);
...
void FPipInstall::CheckInvalidPipEnv() const
{
	...
	UE_LOGF(LogPython, Display, "Engine python version (%ls) incompatible with venv (%ls), recreating...", TEXT(PY_VERSION), *VenvVersion);
...
bool FPipInstall::SetupPipEnv(FFeedbackContext* Context, bool bForceRebuild /* = false */) const
{
	...
	if (Res != 0)
	{
		UE_LOGF(LogPython, Error, "Unable to create pip install environment (%d)", Res);
		return false;
	}
...
bool FPipInstall::LaunchPipInstallImpl(bool RunAsync, TSharedPtr<ICmdProgressNotifier> CmdProgressNotifier, FFeedbackContext* Context, FSimpleDelegate OnCompleted)
{
	...
	if (!FPaths::FileExists(ParsedReqsFile))
	{
		UE_LOGF(LogPython, Warning, "PipInstaller background task already running");
		return true;
	}
...
int32 FPipInstall::RunPythonCmd(const FString& PythonInterp, const FString& Cmd, FFeedbackContext* Context, TSharedPtr<ICmdProgressParser> CmdParser)
{
	...
	UE_LOGF(LogPython, Log, "Running python command: python %ls", *Cmd);

	int32 OutResult = 0;
	if (!FLoggedSubprocessSync::Run(OutResult, FPaths::ConvertRelativePathToFull(PythonInterp), Cmd, Context, CmdParser))
	{
		UE_LOGF(LogPython, Error, "Unable to create python process");
		return -1;
	}
...
bool FPipInstall::RunPipCmdAsync(const FString& PythonInterp, const FString& Cmd, FFeedbackContext* Context, TSharedPtr<ICmdProgressParser> CmdParser, FSimpleDelegate OnCompleted)
{
	...
	UE_LOGF(LogPython, Log, "Running background pip install: python %ls", *Cmd);
