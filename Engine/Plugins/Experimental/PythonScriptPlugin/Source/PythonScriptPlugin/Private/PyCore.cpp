...
PyObject* Log(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (PyArg_ParseTuple(InArgs, "O:log", &PyObj))
	{
		...
		if (ShouldLog(LogMessage))
		{
			...
			UE_LOGF(LogPython, Log, "%ls", *LogMessage);
...
PyObject* LogWarning(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (PyArg_ParseTuple(InArgs, "O:log_warning", &PyObj))
	{
		...
		{
			Py_BEGIN_ALLOW_THREADS
			UE_LOGF(LogPython, Warning, "%ls", *LogMessage);
			Py_END_ALLOW_THREADS
...
PyObject* LogError(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (PyArg_ParseTuple(InArgs, "O:log_error", &PyObj))
	{
		...
		{
			Py_BEGIN_ALLOW_THREADS
			UE_LOGF(LogPython, Error, "%ls", *LogMessage);
			Py_END_ALLOW_THREADS
...
PyMethodDef PyCoreMethods[] = {
	...
	{ "register_ticker_callback", PyCFunctionCast(&RegisterTickerCallback), METH_VARARGS | METH_KEYWORDS, "x.register_ticker_callback(callable: Union[Callable[[float], bool], DelegateBase], delay: float=0.0) -> object -- register the given callable (taking a single float and returning a bool where true will continue ticking) as a ticker callback in FTSTicker, to run at most every delay seconds" },
...
void ShutdownModule()
{
	...
	if (GetPythonTypeContainerSingleton().IsValid())
	{
		UObject* PythonTypesPackage = GetPythonTypeContainerSingleton().Get();
		PythonTypesPackage->Rename(TEXT("PythonTypes_Garbage"), nullptr, REN_DoNotDirty | REN_DontCreateRedirectors | REN_NonTransactional);
		PythonTypesPackage->MarkAsGarbage();
		if (PythonTypesPackage->IsRooted())
		{
			PythonTypesPackage->RemoveFromRoot();
		}
		PythonTypesPackage->ClearFlags(RF_Public | RF_Standalone);
		GetPythonTypeContainerSingleton().Reset();
	}
}
