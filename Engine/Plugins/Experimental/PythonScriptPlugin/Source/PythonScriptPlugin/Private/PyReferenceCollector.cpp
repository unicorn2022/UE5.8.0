...
{
	if (!PythonWrappedInstances.IsEmpty())
	{
		FPyScopedGIL GIL;
...
{
#if WITH_EDITOR
	// Only do the reference cleaning when the editor is not shuthing down to avoid wasting time.
	if (!IsEngineExitRequested())
	{
		// Preclean to only grab the object that are refered by other systems
		CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS);

		TArray<UObject*> PythonObjectInstances;
		ForEachObjectOfClass(UPythonGeneratedClass::StaticClass(), [&PythonObjectInstances](UObject* InObject)
			{
				UPythonGeneratedClass* PythonGeneratedClass = CastChecked<UPythonGeneratedClass>(InObject);
				ForEachObjectOfClass(PythonGeneratedClass, [&PythonObjectInstances](UObject* InInnerObject)
					{
						PythonObjectInstances.Add(InInnerObject);
					}, false, RF_NoFlags, EInternalObjectFlags::Native);
			}, false, RF_ClassDefaultObject, EInternalObjectFlags::Native);

		ObjectTools::ForceReplaceReferences(nullptr, PythonObjectInstances);
		CollectGarbage(GARBAGE_COLLECTION_KEEPFLAGS);
	}
#endif //WITH_EDITOR
	...
	if (PurgingReferenceCollector.HasObjectToPurge())
	{
		...
		for (const FWeakObjectPtr& WeakReferencesToPurgedObject : WeakReferencesToPurgedObjects)
		{
			if (UObject* FailedToPurgeObject = WeakReferencesToPurgedObject.Get())
			{
				...
				{
					UE_LOGF(LogPython, Display, "Object '%ls' was externally referenced when shutting down Python. Forcibly releasing its Python resources!", *FailedToPurgeObject->GetPathName());
					PythonResourceOwner->ReleasePythonResources();
				}
				else
				{
					UE_LOGF(LogPython, Warning, "Object '%ls' was externally referenced when shutting down Python and could not gracefully cleaned-up. This may lead to crashes!", *FailedToPurgeObject->GetPathName());
				}
			}
		}
	}
...
void FPyReferenceCollector::Shutdown()
{
	PythonWrappedInstances.Reset();
}
...
void FPyReferenceCollector::AddReferencedObjectsFromPropertyInternal(FReferenceCollector& InCollector, const FProperty* InProp, void* InBaseAddr, const EPyReferenceCollectorFlags InFlags, bool& OutValueChanged)
{
	...
	if (const FOptionalProperty* CastProp = CastField<FOptionalProperty>(InProp))
	{
		for (int32 ArrIndex = 0; ArrIndex < InProp->ArrayDim; ++ArrIndex)
		{
			void* ValuePtr = CastProp->ContainerPtrToValuePtr<void>(InBaseAddr, ArrIndex);
			if (CastProp->IsSet(ValuePtr))
			{
				AddReferencedObjectsFromPropertyInternal(InCollector, CastProp->GetValueProperty(), CastProp->GetValuePointerForReadOrReplace(ValuePtr), InFlags, OutValueChanged);
			}
		}
		return;
	}
