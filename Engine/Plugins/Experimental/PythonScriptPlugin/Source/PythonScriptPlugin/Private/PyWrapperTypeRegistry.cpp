...
void FPyWrapperTypeReinstancer::ProcessPending()
{
	if (ClassesToReinstance.Num() > 0 || StructsToReinstance.Num() > 0)
	{
		...
		for (const TTuple<TObjectPtr<UPythonGeneratedClass>, TObjectPtr<UPythonGeneratedClass>>& ClassToReinstancePair : ClassesToReinstance)
		{
			if (ClassToReinstancePair.Key && ClassToReinstancePair.Value)
			{
				// Resolve any CLASS_NewerVersionExists chains, so that any older classes all get reinstanced onto the most recent class
				UPythonGeneratedClass* NewClass = ClassToReinstancePair.Value;
				while (NewClass && NewClass->HasAnyClassFlags(CLASS_NewerVersionExists))
				{
					// We expect to find the replacement for NewClass in ClassesToReinstance, since 
					// that's the only way it should have been marked as CLASS_NewerVersionExists
					NewClass = ClassesToReinstance.FindRef(NewClass);
					ensureAlways(NewClass);
				}

				if (NewClass)
				{
					// Assume the classes have changed
					Reload->NotifyChange(NewClass, ClassToReinstancePair.Key);
				}
				else
				{
					// We failed to resolve the new class; old class won't be purged via Reinstance(),
					// but its property links must still be cleared so that DestroyNonNativeProperties
					// on its CDO doesn't walk FFields that may be freed by the base class FinishDestroy
					ClassToReinstancePair.Key->DestroyChildPropertiesAndResetPropertyLinks();
				}
			}
		}

		// This doesn't do anything ATM
		for (const TTuple<TObjectPtr<UPythonGeneratedStruct>, TObjectPtr<UPythonGeneratedStruct>>& StructToReinstancePair : StructsToReinstance)
		{
		...
		// Skip the GC in case we're actively loading to avoid a full flush especially during engine startup.
		// The next GC will take care of the cleanup.
		if (!IsAsyncLoading())
		{
...
void FPyWrapperTypeReinstancer::Shutdown()
{
...
{
	InCollector.AddReferencedObjects(ClassesToReinstance);
	InCollector.AddReferencedObjects(StructsToReinstance);
}
...
void FPyWrapperTypeRegistry::GenerateWrappedTypes(bool bAllowMultithreadedGeneration)
{
	...
	UE_LOGF(LogPython, Verbose, "Took %f seconds to generate and initialize Python wrapped types for the initial load.", GenerateDuration);
}
...
void FPyWrapperTypeRegistry::GenerateWrappedTypesForModule(const FName ModuleName)
{
	...
	UE_LOGF(LogPython, Verbose, "Took %f seconds to generate and initialize Python wrapped types for '%ls'.", GenerateDuration, *ModuleName.ToString());
}
...
void FPyWrapperTypeRegistry::GenerateStubCodeForWrappedTypes(const EPyOnlineDocsFilterFlags InDocGenFlags) const
{
	...
	UE_LOGF(LogPython, Display, "Generating Python API stub file...");
	...
	TArray<FString> AdditionalPythonCode;
	{
		...
		if (PyGenUtil::IsTypeHintingEnabled())
		{
			PythonScript.WriteLine(TEXT("from typing import Any, Callable, Dict, ItemsView, Iterable, Iterator, KeysView, List, Mapping, MutableMapping, MutableSequence, MutableSet, Optional, Set, Sequence, Tuple, Type, TypeVar, Union, ValuesView"));
	...
	UE_LOGF(LogPython, Display, "  ...generating Python API: glue code (types)");
	...
	auto ProcessWrappedDataArray = [this, &PythonScript, &GeneratedTypesToSkip, InDocGenFlags](const auto& WrappedData, const TSharedPtr<FPyOnlineDocsSection>& OnlineDocsSection)
	{
		...
		{
			UE_LOGF(LogPython, Display, "  ...generating Python API: %ls", *OnlineDocsSection->GetName());
	...
	UE_LOGF(LogPython, Display, "  ...generating Python API: glue code (methods)");
	PythonScript.WriteLine(TEXT("##### Glue Code #####"));
	PythonScript.WriteNewLine();
	...
	UE_LOGF(LogPython, Display, "  ...generating Python API: additional code");
	...
	UE_LOGF(LogPython, Display, "  ...generated: %ls", *PythonSourceFilename);
...
void FPyWrapperTypeRegistry::Shutdown()
{
	PythonWrappedTypeNameToUnrealTypeName.Empty();
	PythonWrappedClasses.Empty();
	PythonWrappedStructs.Empty();
	PythonWrappedEnums.Empty();
	PythonWrappedDelegates.Empty();
	GeneratedWrappedTypes.Empty();
	GeneratedWrappedTypesForModule.Empty();
	UnresolvedMakeFuncs.Empty();
	UnresolvedBreakFuncs.Empty();
	OrphanedWrappedTypes.Empty();
	NativePythonModules.Empty();
	OnModuleDirtiedDelegate.Clear();
	InlineStructFactories.Empty();

	bCanRegisterInlineStructFactories.store(true);
}
...
void FPyWrapperTypeRegistry::GenerateStubCodeForWrappedType(PyTypeObject* PyType, const PyGenUtil::FGeneratedWrappedType* GeneratedTypeData, FPyFileWriter& OutPythonScript, FPyOnlineDocsSection* OutOnlineDocsSection)
{
	...
	if (PyGenUtil::IsTypeHintingEnabled() &&  PyTypeName == TEXT("Array"))
	{
	...
	else if (PyGenUtil::IsTypeHintingEnabled() && PyTypeName == TEXT("FixedArray"))
	{
		// Implies that _ElemType = TypeVar('_ElemType') is already written to the file.
		OutPythonScript.WriteLine(FString::Printf(TEXT("class %s(%s, Sequence[_ElemType]):"), *PyTypeName, UTF8_TO_TCHAR(PyType->tp_base->tp_name)));
	}
	...
	// Export the __init__ function for this type
	{
		...
		if (PyGenUtil::IsTypeHintingEnabled())
		{
			if (PyType == &PyWrapperArrayType || PyType == &PyWrapperFixedArrayType)
			{
				...
				ExportNoBodyMethod(TEXT("def __len__(self) -> int:"));
				if (PyType == &PyWrapperArrayType)
				{
					ExportNoBodyMethod(TEXT("def __delitem__(self, key: Union[int, slice, Any]) -> None:"));
				}
			}
			else if (PyType == &PyWrapperSetType)
			{
				...
				ExportNoBodyMethod(TEXT("def __len__(self) -> int:"));
				ExportNoBodyMethod(TEXT("def __contains__(self, x: object) -> bool:"));
...
void ShutdownPyWrappers()
{
	FPyWrapperObjectFactory::Get().Shutdown();
	FPyWrapperStructFactory::Get().Shutdown();
	FPyWrapperDelegateFactory::Get().Shutdown();
	FPyWrapperMulticastDelegateFactory::Get().Shutdown();
	FPyWrapperNameFactory::Get().Shutdown();
	FPyWrapperTextFactory::Get().Shutdown();
	FPyWrapperArrayFactory::Get().Shutdown();
	FPyWrapperFixedArrayFactory::Get().Shutdown();
	FPyWrapperSetFactory::Get().Shutdown();
	FPyWrapperMapFactory::Get().Shutdown();
	FPyWrapperFieldPathFactory::Get().Shutdown();
	FPyWrapperTypeReinstancer::Get().Shutdown();
	FPyWrapperTypeRegistry::Get().Shutdown();
}
