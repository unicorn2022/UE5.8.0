...
void FPyOnlineDocsWriter::GenerateConfigFile()
{
	...
	{
		UE_LOGF(LogPython, Warning, "Documentation generation template file '%ls' failed to load!", *ConfigTemplatePath);
		return;
	}
	...
	{
		UE_LOGF(LogPython, Warning, "Documentation generation file '%ls' failed to write!", *ConfigPath);
	}
}
...
void FPyOnlineDocsWriter::GenerateIndexFile()
{
	...
	{
		UE_LOGF(LogPython, Warning, "Documentation generation template file '%ls' failed to load!", *IndexTemplatePath);
		return;
	}
	...
	{
		UE_LOGF(LogPython, Warning, "Documentation generation file '%ls' failed to write!", *IndexPath);
	}
}
...
void FPyOnlineDocsWriter::GenerateModuleFiles()
{
	...
	{
		UE_LOGF(LogPython, Warning, "Documentation generation template file '%ls' failed to load!", *ModuleTemplatePath);
		return;
	}
	...
	for (const TSharedRef<FPyOnlineDocsModule>& Module : Modules)
	{
		...
		{
			UE_LOGF(LogPython, Warning, "Documentation generation file '%ls' failed to write!", *ModulePath);
		}
	}
...
void FPyOnlineDocsWriter::GenerateClassFiles()
{
	...
	{
		UE_LOGF(LogPython, Warning, "Documentation generation template file '%ls' failed to load!", *ClassTemplatePath);
		return;
	}
	...
	for (const TSharedRef<FPyOnlineDocsSection>& Section : Sections)
	{
		for (const FString& TypeName : Section->TypeNames)
		{
			...
			{
				UE_LOGF(LogPython, Warning, "Documentation generation file '%ls' failed to write!", *ClassPath);
			}
		}
	}
...
{
	UE_LOGF(LogPython, Display, "Generating Python API online docs used by Sphinx to generate static HTML...");
	...
	// Copy generated unreal module stub file to PythonScriptPlugin/SphinxDocs/modules
	{
		...
		if (FFileHelper::LoadFileToString(SourceFileContents, *InPythonStubPath))
		{
			...
			{
				UE_LOGF(LogPython, Warning, "Documentation generation file '%ls' failed to write!", *PythonStubDestPath);
			}
		}
		else
		{
			UE_LOGF(LogPython, Warning, "Documentation generation file '%ls' failed to load!", *InPythonStubPath);
		}
	}
	...
	UE_LOGF(LogPython, Display, "  ... finished generating Sphinx files.");
	...
	// Validate the unreal generated stub file before we let Sphinx try and process it
	{
		...
		UE_LOGF(LogPython, Display, "Validating the unreal generated stub file...");
		if (!IPythonScriptPlugin::Get()->ExecPythonCommand(*PyCommandStr))
		{
			UE_LOGF(LogPython, Error, "The unreal generated stub file failed to import. Aborting!");
			return;
		}
		UE_LOGF(LogPython, Display, "  ... finished validating file.");
	}
	...
	if (bUseSphinx)
	{
		...
		// Update pip and install Sphinx
		{
			...
			UE_LOGF(LogPython, Display, "Installing Sphinx...");
			IPythonScriptPlugin::Get()->ExecPythonCommand(*PyCommandStr);
			UE_LOGF(LogPython, Display, "  ... finished installing Sphinx.");
		}
		...
		// Run Sphinx to build the docs
		{
			...
			UE_LOGF(LogPython, Display, "Calling Sphinx in PythonPlugin/SphinxDocs to generate the HTML. This can take a long time - 25+ minutes for full build on test system...");
			...
			UE_LOGF(LogPython, Display, "  ... finished generating Python API online docs!\n\n"
	...
	else
	{
		...
		UE_LOGF(LogPython, Display, "To build the Python API online docs manually follow the instructions in:\n"
			"  PythonScriptPlugin/SphinxDocs/PythonAPI_docs_readme.txt\n\n"
			"And then call:"
