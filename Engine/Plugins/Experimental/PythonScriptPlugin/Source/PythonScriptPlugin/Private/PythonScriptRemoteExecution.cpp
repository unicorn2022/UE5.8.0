...
public:
		...
		, BroadcastSocket(nullptr)
	{
		...
		if (!FIPv4Endpoint::Parse(PythonScriptPluginSettings->RemoteExecutionMulticastGroupEndpoint, MulticastGroupEndpoint))
		{
			...
			UE_LOGF(LogPythonRemoteExecution, Error, "Failed to parse multicast group endpoint '%ls'. Remote execution will be disabled!", *PythonScriptPluginSettings->RemoteExecutionMulticastGroupEndpoint);
		}
		...
		{
			bValidEndpointAddresses = false;
			UE_LOGF(LogPythonRemoteExecution, Error, "Failed to parse multicast bind address '%ls'. Remote execution will be disabled!", *PythonScriptPluginSettings->RemoteExecutionMulticastBindAddress);
		}
...
private:
	void HandleMessage(const FPythonScriptRemoteExecutionMessage& InMessage)
	{
		...
		UE_LOGF(LogPythonRemoteExecution, VeryVerbose, "Unhandled remote execution message type '%ls'", *InMessage.Type);
	}
	...
	void HandleOpenConnectionMessage(const FPythonScriptRemoteExecutionMessage& InMessage)
	{
		...
		if (InMessage.Data && InMessage.Data->TryGetObject(ConnectionData) && *ConnectionData)
		{
			FIPv4Address CommandIp;
			{
				...
				{
					UE_LOGF(LogPythonRemoteExecution, Error, "Failed to find required data field 'command_ip' (string) for the 'open_connection' message!");
					return;
				}
				if (!FIPv4Address::Parse(CommandIpString, CommandIp))
				{
					UE_LOGF(LogPythonRemoteExecution, Error, "Failed to parse command address '%ls'!", *CommandIpString);
					return;
				}
			}
			...
			{
				UE_LOGF(LogPythonRemoteExecution, Error, "Failed to find required data field 'command_port' (integer) for the 'open_connection' message!");
				return;
			}
	...
	void BroadcastMessage(const FPythonScriptRemoteExecutionMessage& InMessage)
	{
		...
		{
			UE_LOGF(LogPythonRemoteExecution, Error, "Failed to write remote execution message: %ls", *WriteErrorMsg.ToString());
			return;
		}
...
private:
	...
	void HandleCommandMessage(const FPythonScriptRemoteExecutionMessage& InMessage)
	{
		...
		if (InMessage.Data && InMessage.Data->TryGetObject(CommandData) && *CommandData)
		{
			...
			{
				UE_LOGF(LogPythonRemoteExecution, Error, "Failed to find required data field 'command' (string) for the 'command' message!");
				return;
			}

			{
				...
				{
					UE_LOGF(LogPythonRemoteExecution, Error, "Failed to find required data field 'unattended' (bool) for the 'command' message!");
					return;
				}
...

			{
				...
				{
					UE_LOGF(LogPythonRemoteExecution, Error, "Failed to find required data field 'exec_mode' (string) for the 'command' message!");
					return;
				}
				if (!LexTryParseString(PythonCommand.ExecutionMode, *ExecModeString))
				{
					UE_LOGF(LogPythonRemoteExecution, Error, "Failed to find parse execution mode '%ls'!", *ExecModeString);
					return;
				}
			}
...
bool FPythonScriptRemoteExecution::Start()
{
	...
	{
		UE_LOGF(LogPythonRemoteExecution, Error, "Failed to open remote execution broadcast channel! Check your settings and the output log for more information.");
...
void FPythonScriptRemoteExecution::OpenCommandConnection(const FString& InRemoteNodeId, const FIPv4Endpoint& InCommandEndpoint)
{
	if (!CommandConnection || !CommandConnection->IsConnectedTo(InRemoteNodeId, InCommandEndpoint))
	{
		...
		{
			UE_LOGF(LogPythonRemoteExecution, Error, "Failed to open remote execution command channel to '%ls' (%ls)!", *InRemoteNodeId, *InCommandEndpoint.ToString());
			CommandConnection.Reset();
		}
	}
}
...
namespace PythonScriptRemoteExecutionUtil
{
	...
	bool ParseMessageData(const TCHAR* InMessageData, FPythonScriptRemoteExecutionMessage& OutMessage)
	{
		...
		{
			UE_LOGF(LogPythonRemoteExecution, Error, "Failed to read remote execution message '%ls': %ls", InMessageData, *ReadErrorMsg.ToString());
			return false;
		}
		return true;
	}
