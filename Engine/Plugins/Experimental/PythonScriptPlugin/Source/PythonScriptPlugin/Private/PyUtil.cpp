	...
	, ValueDef()
{
	...
	if (const FOptionalProperty* OptionalProp = CastField<FOptionalProperty>(InProperty))
	{
		ValueDef = MakeShared<FPropertyDef>(OptionalProp->GetValueProperty());
	}
}
...
bool CalculatePropertyDef(PyObject* InPyObj, FPropertyDef& OutPropertyDef)
{
	...
	if (PyObject* PyValueObj = nullptr;
		IsOptionalPropertyDef(InPyObj, &PyValueObj))
	{
		FPropertyDef ValuePropertyDef;
		if (CalculatePropertyDef(PyValueObj, ValuePropertyDef))
		{
			OutPropertyDef.PropertyClass = FOptionalProperty::StaticClass();
			OutPropertyDef.ValueDef = MakeShared<FPropertyDef>(MoveTemp(ValuePropertyDef));
			return true;
		}
	}
...
bool IsOptionalPropertyDef(PyObject* InPyObj, PyObject** OutPyValueObj)
{
	if (OutPyValueObj)
	{
		*OutPyValueObj = nullptr;
	}

	// A tuple of two values, where one is None, is considered an optional property
	if (PyTuple_Check(InPyObj) && PyTuple_GET_SIZE(InPyObj) == 2)
	{
		PyObject* PyValueObj = nullptr;
		int32 NoneCount = 0;

		if (PyObject* PotentialPyValueObj = PyTuple_GET_ITEM(InPyObj, 0);
			PotentialPyValueObj != Py_None)
		{
			PyValueObj = PotentialPyValueObj;
		}
		else
		{
			++NoneCount;
		...
		if (PyObject* PotentialPyValueObj = PyTuple_GET_ITEM(InPyObj, 1);
			PotentialPyValueObj != Py_None)
		{
			PyValueObj = PotentialPyValueObj;
		}
		...
		if (PyValueObj && NoneCount == 1)
		{
			if (OutPyValueObj)
			{
				*OutPyValueObj = PyValueObj;
			}
			return true;
		}
	}
...
FProperty* CreateProperty(const FPropertyDef& InPropertyDef, const int32 InArrayDim, FFieldVariant InOuter, const FName InName)
{
	...
	if (Prop)
	{
		...
		{
			OptionalProp->SetValueProperty(CreateProperty(*InPropertyDef.ValueDef, 1, OptionalProp));
		}
...
bool LogPythonError(FString* OutError, const bool bInteractive)
{
	...
	if (bFetchedError)
	{
		// Log the error
		{
			...
			{
				UE_LOGF(LogPython, Error, "%ls", *ErrorLine);
			}
		}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(PyUtil::CollectGarbage);
	if (Generations == 2)
	{
...
bool CanTickPython()
{
	...
	if (UE::IsSavingPackage() || IsGarbageCollecting() || FUObjectThreadContext::Get().IsRoutingPostLoad)
	{
		return false;
	}
