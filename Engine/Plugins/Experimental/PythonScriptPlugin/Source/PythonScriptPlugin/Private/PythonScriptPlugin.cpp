...
static bool bIsPythonLifeCycleDynamic = false;
static FAutoConsoleVariableRef CVarIsPythonLifeCycleDynamic(TEXT("Engine.Python.IsPythonLifeCycleDynamic"), bIsPythonLifeCycleDynamic, TEXT("If true python will not be initialized on boot in the editor. This allow the project to control when python should be initialized or not."), ECVF_ReadOnly);

static bool bIsPythonInRestrictiveMode = false;
static FAutoConsoleVariableRef CVarIsPythonInRestrictiveMode(TEXT("Engine.Python.IsPythonInRestrictiveMode"), bIsPythonInRestrictiveMode, TEXT("If true python will be initialized in the restrive mode of the editor. This allow the project to control what is acessible to Python and is still a work in progress."));

static bool bIsEnabledByDefault = true;
static FAutoConsoleVariableRef CVarIsPythonEnabledByDefault(TEXT("Engine.Python.IsEnabledByDefault"), bIsEnabledByDefault, TEXT("True if Python is enabled by default (after checking any command line overrides), or False if it should be considered disabled. This must be set prior to OnPostEngineInit."));

bool IsPythonEnabled()
{
	if (FParse::Param(FCommandLine::Get(), TEXT("ForceEnablePython")))
	{
		UE_LOGF(LogPython, Log, "Python enabled via command-line flag '-ForceEnablePython'");
		return true;
	}

	if (FParse::Param(FCommandLine::Get(), TEXT("DisablePython")))
	{
		UE_LOGF(LogPython, Log, "Python disabled via command-line flag '-DisablePython'");
		return false;
	}
	...
	if (IsRunningCommandlet())
	{
		...
		if (!DisablePythonForCommandlets.IsEmpty() && FParse::Value(FCommandLine::Get(), TEXT("-run="), RunningCommandletName))
		{
			...
			for (FString& DisablePythonForCommandlet : DisablePythonForCommandlets)
			{
				...
				{
					UE_LOGF(LogPython, Log, "Python disabled via config setting 'DisablePythonForCommandlet'");
					return false;
				}
			}
		}
	...
	{
		UE_LOGF(LogPython, Log, "Python enabled via command-line flag '-EnablePython'");
		return true;
	}
	...
	switch (GetDefault<UPythonScriptPluginUserSettings>()->EnablePythonOverride)
	{
		case EPythonEnabledOverrideState::Enable:
			UE_LOGF(LogPython, Log, "Python enabled via PythonScriptPluginUserSettings");
			return true;

		case EPythonEnabledOverrideState::Disable:
			UE_LOGF(LogPython, Log, "Python disabled via PythonScriptPluginUserSettings");
			return false;
	...
	{
		UE_LOGF(LogPython, Log, "Python enabled via CVar 'Engine.Python.IsEnabledByDefault'");
		return true;
	}

	UE_LOGF(LogPython, Log, "Python disabled via CVar 'Engine.Python.IsEnabledByDefault'");
	return false;
}
...
bool FPythonCommandExecutor::Exec(const TCHAR* Input)
{
	...
	UE_LOGF(LogPython, Log, "%ls", Input);
...
bool FPythonScriptPlugin::ForceEnablePythonAtRuntime(const FString& Location)
{
	...
	if (!bIsForceEnabledAtRuntime)
	{
		...
		{
			UE_LOGF(LogPython, Warning, "Python enabled via IPythonScriptPlugin::ForceEnablePythonAtRuntime: %ls", *Location);
		}
		else
		{
			UE_LOGF(LogPython, Log, "Python enabled via IPythonScriptPlugin::ForceEnablePythonAtRuntime: %ls", *Location);
		}
...
void FPythonScriptPlugin::ForceDisablePythonAtRuntime(UE::FSourceLocation Location)
{
#if WITH_PYTHON
	if (bIsInterpreterInitialized)
	{
		const FString CallerLocation = UE::SourceLocation::Full(Location).ToString();
		if (!UE::Python::Private::bIsEnabledByDefault || !UE::Python::Private::bIsPythonLifeCycleDynamic)
		{
			UE_LOGF(LogPython, Warning, "Python disabled via IPythonScriptPlugin::DisablePythonAtRuntime: %ls", *CallerLocation);
		}
		else
		{
			UE_LOGF(LogPython, Log, "Python disabled via IPythonScriptPlugin::DisablePythonAtRuntime: %ls", *CallerLocation);
		}
...
bool FPythonScriptPlugin::ExecPythonCommandEx(FPythonCommandEx& InOutPythonCommand)
{
	...
	if (!bIsInterpreterInitialized)
	{
		...
		UE_LOGF(LogPython, Warning, "%ls", *InOutPythonCommand.CommandResult);
		return false;
	}
...
void FPythonScriptPlugin::StartupModule()
{
	...
	FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FPythonScriptPlugin::OnPostEngineInit);
#endif	// WITH_PYTHON
}
...
void FPythonScriptPlugin::ConfigureAndInitializePython()
{
	...
	if (UE::Python::Private::bIsPythonLifeCycleDynamic && !bIsForceEnabledAtRuntime)
	{
		return;
	}
	...
	{
		bIsEnabled = false;
		UE_LOGF(LogPython, Error, "Python could not be initialized and was disabled.")
	...
	if (UToolMenus::IsToolMenuUIEnabled())
	{
		CmdMenu = MakeUnique<FPythonCommandMenuImpl>();
...
void FPythonScriptPlugin::AddStartupScriptAllowPath(const FName Owner, const FString& Path)
{
	StartupScriptAllowedPaths.AddAllowListItem(Owner, Path);
}

void FPythonScriptPlugin::RemoveStartupScriptAllowPath(const FName Owner)
{
	StartupScriptAllowedPaths.UnregisterOwner(Owner);
}
...
bool FPythonScriptPlugin::InitializePython()
{
	...
	// Initialize the Python interpreter
	{
		...
		UE_LOGF(LogPython, Log, "Using Python %d.%d.%d", PY_MAJOR_VERSION, PY_MINOR_VERSION, PY_MICRO_VERSION);
			...
			!ensure(!PyStatus_Exception(Status)))
		{
			...
			UE_LOGF(LogPython, Error, "Py_PreInitialize failed! Error: '%s'. Python will be disabled.", Status.err_msg)
			return false;
		}
		...
		{
			PyUtil::LogPythonError();
			UE_LOGF(LogPython, Error, "Py_InitializeFromConfig failed! Error: '%s'. Python will be disabled.", Status.err_msg)
			return false;
		}
#else
	...
	// Setup UE conventions for the embedded interpreter environment
	{
		...
		{
			PyUtil::LogPythonError();
			UE_LOGF(LogPython, Error, "PySys_SetObject(\"argv\") failed! Python will be disabled.")
			return false;
		}
#else
		PySys_SetArgvEx(1, UE::Python::Private::NullPyArgPtrs, 0);
		...
		{
			PyUtil::LogPythonError();
			UE_LOGF(LogPython, Error, "Failed to initialize '__main__'! This typically means the Python SDK is missing or could not be loaded")
			return false;
		}
	...
	// Initialize the Unreal Python module
	{
		...
		// We don't support pip installs in a restrictive environment at the moment.
		if (!UE::Python::Private::bIsPythonInRestrictiveMode)
		{
#if WITH_EDITOR
...
void FPythonScriptPlugin::ShutdownPython()
{
	...
	ShutdownPyWrappers();
	FPyReferenceCollector::Get().Shutdown();
...
{
	if (bIsFullyInitialized || !bIsInterpreterInitialized)
	{
		return;
	}
	...
	double InitTotalTime = 0.0;

	auto RunTimedStartupScript = [this, &InitTotalTime](const FString& StartupScript, const FText& StartupScriptInfoText, const bool bRunAsFile)
	{
		FScopedDurationTimeCustomLogger TimeLogger(*StartupScriptInfoText.ToString(), InitTotalTime, [](const TCHAR* Msg) { UE_LOGF(LogPython, Display, "%ls", Msg) });
		if (bRunAsFile)
		{
	...
	FPathPermissionList FolderPermissionList;
	FolderPermissionList.Append(StartupScriptAllowedPaths);
	FolderPermissionList.Append(IAssetTools::Get().GetFolderPermissionList().Get());

	auto FilePathPassPermissionList = [FolderPermissionList](const FString& FilenamePath) -> bool
		{
#if WITH_EDITOR
			if (!UE::Python::Private::bIsPythonInRestrictiveMode)
			{
			...
			bool bIsPathPermitted = false;

			FString OutPackagePath;
			if (FPackageName::TryConvertFilenameToLongPackageName(FilenamePath, OutPackagePath))
			{
				// Writable content folders sort of act as a working user space.
				bIsPathPermitted = FolderPermissionList.PassesStartsWithFilter(OutPackagePath);
			}

			return bIsPathPermitted;
	...
	for (const FString& PySysPath : PySysPaths)
	{
		...
		if (FilePathPassPermissionList(PotentialFilePath) && FPaths::FileExists(PotentialFilePath))
		{
	...
	{
		if (FilePathPassPermissionList(StartupScript))
		{
...
bool FPythonScriptPlugin::RunFile(const TCHAR* InFile, const TCHAR* InArgs, FPythonCommandEx& InOutPythonCommand)
{
...
#if WITH_EDITOR
	// We don't want to update the menu entries for the recent files when running the startups scripts
	if (CmdMenu && bIsFullyInitialized)
	{
	...
	if (!bLoaded)
	{
		...
		UE_LOGF(LogPython, Error, "%ls", *InOutPythonCommand.CommandResult);
		return false;
	}
	...
	double ElapsedSeconds = 0.0;
	{
		...
		else if (PyUtil::LogPythonError(&InOutPythonCommand.CommandResult))
		{
			...
			InOutPythonCommand.CommandResult.ParseIntoArrayLines(ErrorLines);

			for (const FString& ErrorLine : ErrorLines)
			{
				InOutPythonCommand.LogOutput.Add(FPythonLogOutputEntry{ EPythonLogOutputType::Error, ErrorLine });
			}
			return false;
		}
	}
