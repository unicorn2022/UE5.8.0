				...
				if (!ScriptAndArgs.IsEmpty())
				{
					...
					if (!IAssetRegistry::GetChecked().IsLoadingAssets())
					{
						...
						if (!IPythonScriptPlugin::Get()->ExecPythonCommandEx(PythonCommand))
						{
							if (bErrorsAreFatal)
							{
								UE_LOGF(LogEditorPythonExecuter, Fatal, "Python script executed with errors");
							}
							else
							{
								UE_LOGF(LogEditorPythonExecuter, Error, "Python script executed with errors");
							}
						}
					}
				}
...
void FEditorPythonExecuter::OnStartupModule()
{
	...
	if (!ScriptAndArgs.IsEmpty())
	{
		if (!GIsEditor)
		{
			UE_LOGF(LogEditorPythonExecuter, Error, "-ExecutePythonScript cannot be used outside of the editor.");
		}
		else if (IsRunningCommandlet())
		{
			UE_LOGF(LogEditorPythonExecuter, Error, "-ExecutePythonScript cannot be used by a commandlet. Use -run=PythonScript instead?");
		}
		else
		{
			...
			IPythonScriptPlugin::Get()->RegisterOnPythonConfigured(FSimpleDelegate::CreateLambda([ScriptAndArgs = MoveTemp(ScriptAndArgs), bScriptErrorsAreFatal = FParse::Param(FCommandLine::Get(), TEXT("ScriptErrorsAreFatal"))]() mutable
			{
				...
				{
					UE_LOGF(LogEditorPythonExecuter, Error, "-ExecutePythonScript cannot be used when Python support is disabled.");
				}
			}));
		}
	}
