...
int FPyWrapperSet::Init(FPyWrapperSet* InSelf, const PyUtil::FPropertyDef& InElementDef)
{
	...
	PyUtil::FSetPropOnScope SetProp = PyUtil::FSetPropOnScope::OwnedReference(new FSetProperty(FFieldVariant(), PyUtil::DefaultPythonPropertyName));
...
int FPyWrapperSet::IntersectionUpdate(FPyWrapperSet* InSelf, PyObject* InOthers)
{
	...
	auto ProcessOtherObject = [&InSelf, &SelfScriptSetHelper, &SelfElementDef](PyObject* InOtherObj, const int32 InObjIndex)
	{
		...
		// We iterate in reverse, as when TSet is TCompactSet (see UE_USE_COMPACT_SET_AS_DEFAULT), 
		// it will perform a RemoveSwap, vs TSparseSet which will leave holes until compaction
		// Reverse iteration works correctly for both implementations, whereas forward iteration 
		// fails for TCompactSet, as it will skip over the element swapped from the end of the set
		const int32 SelfSparseCount = SelfScriptSetHelper.GetMaxIndex();
		for (int32 SelfSparseIndex = SelfSparseCount - 1; SelfSparseIndex >= 0; --SelfSparseIndex)
		{
