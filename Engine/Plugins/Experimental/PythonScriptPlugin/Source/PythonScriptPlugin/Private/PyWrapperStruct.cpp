...
PyObject* FPyWrapperStruct::BreakStruct(FPyWrapperStruct* InSelf)
{
	...
	// Generic implementation just creates a dict with each property
	FPyObjectPtr PyPropDict = FPyObjectPtr::StealReference(PyDict_New());
	for (int32 ParamIndex = 0; ParamIndex < StructMetaData->InitParams.Num(); ++ParamIndex)
	{
		...
		FPyObjectPtr Value = FPyObjectPtr::StealReference(PyValue);
		FPyObjectPtr Key = FPyObjectPtr::StealReference(PyUnicode_FromString(InitParam.ParamName.GetData()));
		PyDict_SetItem(PyPropDict, Key, Value);
	}

	return PyPropDict.Release();
}
...
PyObject* FPyWrapperStruct::CallBreakFunction_Impl(FPyWrapperStruct* InSelf, const PyGenUtil::FGeneratedWrappedFunction& InFuncDef)
{
	if (ensureAlways(InFuncDef.Func))
	{
		...
		for (int32 ParamIndex = 0; ParamIndex < InFuncDef.OutputParams.Num(); ++ParamIndex)
		{
			...
			FPyObjectPtr Key = FPyObjectPtr::StealReference(PyUnicode_FromString(ParamDef.ParamName.GetData()));
			FPyObjectPtr Value = FPyObjectPtr::StealReference(PyValue);
			PyDict_SetItem(PyPropDict, Key, Value);
		}
		return PyPropDict.Release();
...
#undef DEFINE_BINARY_OPERATOR_FUNC
	...
	struct FMethods
	{
		...
		{
			FPyObjectPtr PyDict = FPyObjectPtr::StealReference(FPyWrapperStruct::BreakStruct(InSelf));
			...
			FPyObjectPtr Values = FPyObjectPtr::StealReference(PyDict_Values(PyDict));
			if (!Values)
			{
				return nullptr; // Exception set internally
			}

			// Convert the values (which are iterable) into a new tuple
			return PyList_AsTuple(Values);
		}

		static PyObject* ToDict(FPyWrapperStruct* InSelf)
		{
		...
		static PyGenUtil::FGeneratedWrappedProperty FindEditorPropertyImpl(FPyWrapperStruct* InSelf, const FName InPythonPropName)
		{
			...
			if (!ResolvedProp)
			{
				ResolvedProp = InSelf->ScriptStruct->CustomFindProperty(ResolvedName);
			}
			if (!ResolvedProp)
			{
	...
	static PyMethodDef PyMethods[] = {
		...
		{ "to_dict", PyCFunctionCast(&FMethods::ToDict), METH_NOARGS, "to_dict(self) -> Dict[str, object] -- convert this Unreal struct into a dict of its properties" },
...
void UPythonGeneratedStruct::ReleasePythonResources()
{
	...
	if (Py_IsInitialized())
	{
		...
		FPyWrapperStructMetaData::SetMetaData(PyType.Get(), nullptr);
		PyType.Reset();
