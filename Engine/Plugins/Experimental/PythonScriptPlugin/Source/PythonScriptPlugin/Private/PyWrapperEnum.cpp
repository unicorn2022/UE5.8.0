...
private:
	bool RegisterDescriptors(const TArray<FPyUValueDef*>& InPyValueDefs)
	{
		...
		check(InPyValueDefs.Num() == NewEnum->EnumValueDefs.Num());
		{
			...
			if (!NewEnum->SetEnums(ValueNames, UEnum::ECppForm::Namespaced, UEnum::EUnderlyingType::uint8, EEnumFlags::None, UEnum::EAddMaxKeyIfMissing::Yes))
			{
...
void UPythonGeneratedEnum::ReleasePythonResources()
{
	...
	if (Py_IsInitialized())
	{
		...
		FPyWrapperEnumMetaData::SetMetaData(PyType.Get(), nullptr);
		PyType.Reset();
	}
	else
	{
