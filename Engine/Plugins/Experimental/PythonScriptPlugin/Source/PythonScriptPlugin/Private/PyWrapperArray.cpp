...
int FPyWrapperArray::Init(FPyWrapperArray* InSelf, const PyUtil::FPropertyDef& InElementDef)
{
	...
	PyUtil::FArrayPropOnScope ArrayProp = PyUtil::FArrayPropOnScope::OwnedReference(new FArrayProperty(FFieldVariant(), PyUtil::DefaultPythonPropertyName));
...
int FPyWrapperArray::SetItem(FPyWrapperArray* InSelf, Py_ssize_t InIndex, PyObject* InValue)
{
	...
	if (InValue == nullptr)
	{
		return RemoveAt(InSelf, InIndex);
	}
...
int FPyWrapperArray::RemoveAt(FPyWrapperArray* InSelf, Py_ssize_t InIndex)
{
	if (!ValidateInternalState(InSelf))
	{
		return -1;
