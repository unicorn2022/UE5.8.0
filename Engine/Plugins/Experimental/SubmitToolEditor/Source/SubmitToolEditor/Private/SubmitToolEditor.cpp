...
void FSubmitToolEditorModule::RegisterSubmitOverrideDelegate(const USubmitToolEditorSettings* InSettings)
{
	...
	{
		UE_LOGF(LogSubmitToolEditor, Display, "Registering SubmitTool to handle submissions from the editor");

		if (InSettings->SubmitToolPath.IsEmpty())
		{
			UE_LOGF(LogSubmitToolEditor, Error, "Submit Tool path is empty: '%ls'", *InSettings->SubmitToolPath);
			return;
		}

		if (InSettings->SubmitToolArguments.IsEmpty())
		{
			UE_LOGF(LogSubmitToolEditor, Error, "Submit Tool Args are empty: '%ls'", *InSettings->SubmitToolArguments);
			return;
		}

		UE_LOGF(LogSubmitToolEditor, Display, "Registering Submit Tool with Path:'%ls' and Args: '%ls'", *InSettings->SubmitToolPath, *InSettings->SubmitToolArguments);
...
void FSubmitToolEditorModule::UnregisterSubmitOverrideDelegate()
{
	...
	{
		UE_LOGF(LogSubmitToolEditor, Display, "Unregistering Submit Tool.");
...
FSubmitOverrideReply FSubmitToolEditorModule::OnSubmitToolOverrideCallback(const SSubmitOverrideParameters& InParameters)
{
	...
	{
		UE_LOGF(LogSubmitToolEditor, Error, "The path is invalid: file does not exist - '%ls'", *NormalizedPath);
	...
	UE_LOGF(LogSubmitToolEditor, Error, "The parameter is invalid: it does not contains and identifier nor a list of files");
...
FSubmitOverrideReply FSubmitToolEditorModule::InvokeSubmitTool(ISourceControlProvider& InProvider, const FString& InPath, const FString& InArgs, const FString& InDescription, const FString& InIdentifier)
{
	...
	{
		UE_LOGF(LogSubmitToolEditor, Error, "Identifier was empty.");
		return FSubmitOverrideReply::Error;
	}
	...
	if (Settings->bEnforceDataValidation)
	{
		...
		bool bValidChangelist = GetChangelistValidationResult(InProvider, InIdentifier, ValidationMessages);
		...
		UpdateOptionalValidationArgs(Settings, ValidationMessages, Args);
		UpdateOptionalValidationTags(Settings, ValidationMessages, Description);
	}

	FString Identifier;
	if (EditChangelistDescription(InProvider, Description, InIdentifier, Identifier))
	{
		return InvokeSubmitTool(InProvider, InPath, Args, Identifier);
	}
...
FSubmitOverrideReply FSubmitToolEditorModule::InvokeSubmitTool(ISourceControlProvider& InProvider, const FString& InPath, const FString& InArgs, const FString& InDescription, const TArray<FString>& InFiles)
{
	...
	{
		return InvokeSubmitTool(InProvider, InPath, InArgs, InDescription, Identifier);
	}
...
FSubmitOverrideReply FSubmitToolEditorModule::InvokeSubmitTool(ISourceControlProvider& InProvider, const FString& InPath, const FString& InArgs, const FString& InIdentifier)
{
	...
	UE_LOGF(LogSubmitToolEditor, Display, "Invoking submit tool: '%ls %ls'", *InPath, *SubstitutedArgs);
	...
	{
		UE_LOGF(LogSubmitToolEditor, Error, "Submit Tool could not be launched.");
		return FSubmitOverrideReply::Error;
	}
...
bool FSubmitToolEditorModule::IsPerforceProvider() const
{
	...
	UE_LOGF(LogSubmitToolEditor, Warning, "Current Provider is not supported for Submit Tool: '%ls', Using the regular editor source control flow", *SourceControlProvider.GetName().ToString());
	return false;
}
...
bool FSubmitToolEditorModule::GetPerforceParameter(ISourceControlProvider& InProvider, FString& OutPort, FString& OutUser, FString& OutClient, FString& OutWorkspacePath)
{
	...
	{
		UE_LOGF(LogSubmitToolEditor, Error, "Could not get a Port from ISourceControlProvider's status");
		return false;
	}
	...
	{
		UE_LOGF(LogSubmitToolEditor, Error, "Could not get a User from ISourceControlProvider's status");
		return false;
	}
	...
	{
		UE_LOGF(LogSubmitToolEditor, Error, "Could not get a Client from ISourceControlProvider's status");
		return false;
	}
	...
	{
		UE_LOGF(LogSubmitToolEditor, Error, "Could not get a the Workspace Path ISourceControlProvider's status");
		return false;
	}
...
bool FSubmitToolEditorModule::EditChangelistDescription(ISourceControlProvider& InProvider, const FString& InNewChangelistDescription, const FString& InIdentifier, FString& OutIdentifier)
{
	...
	{
		UE_LOGF(LogSubmitToolEditor, Warning, "Could not find changelist '%ls'.", *InIdentifier);
		return false;
	}
	...
	if (ChangeListState->SupportsPersistentDescription())
	{
		...
		{
			UE_LOGF(LogSubmitToolEditor, Warning, "Could not edit changelist '%ls''s description, this is not critical and shall not prevent running the submit tool.", *InIdentifier);
		}
...
bool FSubmitToolEditorModule::CreateChangelist(ISourceControlProvider & InProvider, const FString & InNewChangelistDescription, const TArray<FString>&InFiles, FString & OutIdentifier)
{
	...
	switch (CreateCLResult)
	{
		...
		default:
			UE_LOGF(LogSubmitToolEditor, Error, "Could not create changelist.");
			break;
	}
...
bool FSubmitToolEditorModule::GetChangelistValidationResult(ISourceControlProvider& InProvider, const FString& InIdentifier, TArray<FText>& OutValidationMessages)
{
	if (InIdentifier.IsEmpty())
	{
		return true;
	...
	{
		FSourceControlPreSubmitChangelistValidationExDelegate ValidationDelegate = ISourceControlModule::Get().GetRegisteredPreSubmitChangelistValidationEx();

		FSourceControlValidationDelegateResult ValidationResult;
		
		ValidationDelegate.ExecuteIfBound(*(ChangelistToProcess), ValidationResult);

		OutValidationMessages = MoveTemp(ValidationResult.AllMessages);
		return (ValidationResult.Result == EDataValidationResult::Valid);
	}
	else
	{
		UE_LOGF(LogSubmitToolEditor, Warning, "Could not find changelist '%ls'.", *InIdentifier);
...
void FSubmitToolEditorModule::UpdateOptionalValidationArgs(USubmitToolEditorSettings const* InSettings, TArrayView<FText> InValidationMessages, FString& InOutArgs)
{
	for (FSubmitToolArgumentTrigger const& Trigger : InSettings->OptionalArgumentTriggers)
	{
		TSharedPtr<FRegexPattern> Pattern = MakeShared<FRegexPattern>(Trigger.RegExMessage, ERegexPatternFlags::CaseInsensitive);

		for (FText const& Message : InValidationMessages)
		{
			FRegexMatcher Regex = FRegexMatcher(*Pattern, Message.ToString());
			if (Regex.FindNext())
			{
				InOutArgs.Appendf(TEXT(" %s"), *Trigger.SubmitToolArgument);
				break;
			}
		}
	}
...
void FSubmitToolEditorModule::UpdateOptionalValidationTags(USubmitToolEditorSettings const* InSettings, TArrayView<FText> InValidationMessages, FString& InOutDescription)
{
	if (InSettings->ChangelistTagTriggers.Num() == 0)
	{
		return;
	}

	TArray<FString> Lines;
	InOutDescription.ParseIntoArrayLines(Lines, /*bCullEmpty*/false);

	for (FSubmitToolChangelistTagTrigger const& Trigger : InSettings->ChangelistTagTriggers)
	{
		// Find matching message.
		TSharedPtr<FRegexPattern> Pattern = MakeShared<FRegexPattern>(Trigger.RegExMessage, ERegexPatternFlags::CaseInsensitive);

		FString TagLine;
		for (int32 MessageIndex = InValidationMessages.Num() - 1; MessageIndex >= 0; --MessageIndex)
		{
			const FString Message = InValidationMessages[MessageIndex].ToString();
			FRegexMatcher Regex = FRegexMatcher(*Pattern, Message);
			if (Regex.FindNext())
			{
				const FString TagValue = Regex.GetCaptureGroup(1);
				if (!TagValue.IsEmpty())
				{
					TagLine = FString::Printf(TEXT("#%s %s"), *Trigger.SubmitToolTag, *TagValue);
					break;
				}
			}
		}

		// Find lines with matching tag.
		const FString TagLineStart = TEXT("#") + Trigger.SubmitToolTag;

		for (int32 LineIndex = Lines.Num() - 1; LineIndex >= 0 ; --LineIndex)
		{
			if (!Lines[LineIndex].StartsWith(TagLineStart, ESearchCase::IgnoreCase))
			{
				continue;
			}
			...
			else
			{
				...
				Lines[LineIndex] = MoveTemp(TagLine);
				// Reset and continue iterating to remove any additional matching tags.
				TagLine.Reset();
			}
		}
		...
		if (!TagLine.IsEmpty())
		{
			// Didn't replace existing tag. So add new tag line.
			Lines.Add(TagLine);
		}
	}

	InOutDescription = FString::Join(Lines, LINE_TERMINATOR);
}
