...
private:
	...
	bool GetChangelistValidationResult(ISourceControlProvider& InProvider, const FString& InIdentifier, TArray<FText>& OutValidationMessages);
	FString UpdateValidationTag(const FString& InDescription, bool bIsvalid);
	void UpdateOptionalValidationArgs(USubmitToolEditorSettings const* InSettings, TArrayView<FText> InValidationMessages, FString& InOutArgs);
	void UpdateOptionalValidationTags(USubmitToolEditorSettings const* InSettings, TArrayView<FText> InValidationMessages, FString& InOutDescription);
