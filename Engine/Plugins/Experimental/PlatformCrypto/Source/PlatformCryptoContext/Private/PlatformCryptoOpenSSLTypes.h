...
/**
 * RAII wrapper for the OpenSSL EVP public key algorithm context object.
 */
class FScopedEvpPKeyContext
{
public:
	/**
	 * Creates OpenSSL EVP public key algorithm context on construction.
	 */
	FScopedEvpPKeyContext(int KeyAlgorithm) :
		Context(EVP_PKEY_CTX_new_id(KeyAlgorithm, nullptr))
	{
	}

	/** Disable copying/assigning */
	FScopedEvpPKeyContext(FScopedEvpPKeyContext&) = delete;
	FScopedEvpPKeyContext& operator=(FScopedEvpPKeyContext&) = delete;

	/**
	 * Releases OpenSSL EVP public key algorithm context.
	 */
	~FScopedEvpPKeyContext()
	{
		EVP_PKEY_CTX_free(Context);
	}

	/**
	 * Returns OpenSSL EVP public key algorithm context.
	 */
	EVP_PKEY_CTX* Get() const { return Context; }
