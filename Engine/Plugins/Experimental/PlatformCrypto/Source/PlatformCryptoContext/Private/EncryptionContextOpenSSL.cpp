...
EPlatformCryptoResult FSHA256HasherOpenSSL::Init()
{
	...
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Warning, "FSHA256Hasher::Init was called while there was still a digest being computed.");
...
EPlatformCryptoResult FSHA256HasherOpenSSL::Update(const TArrayView<const uint8> InDataBuffer)
{
	...
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Warning, "FSHA256Hasher::Update was called after the hasher was finalized without first calling Init");
		return EPlatformCryptoResult::Failure;
	}
...
EPlatformCryptoResult FSHA256HasherOpenSSL::Finalize(const TArrayView<uint8> OutDataBuffer)
{
	if (Inner->State == FImplDetails::EState::NeedsInit)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Warning, "FSHA256Hasher::Finalize was called after the hasher was finalized without first calling Init");
		return EPlatformCryptoResult::Failure;
	}
	if (OutDataBuffer.Num() < (int32)OutputByteLength)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Warning, "FSHA256Hasher::Finalize called with a buffer that is too small");
		return EPlatformCryptoResult::Failure;
	}
...
struct FEd25519CheckContextOpenSSL
{
	void Shutdown()
	{
		if (EvpPkey != nullptr)
	...
	EVP_MD_CTX* EvpMdCtx = nullptr;
	EVP_PKEY* EvpPkey = nullptr;

	enum class EState { NeedsInit, Ready, InitFailed } State = EState::NeedsInit;

protected:
	~FEd25519CheckContextOpenSSL()
	{
		Shutdown();
	}
};

struct FSignerEd25519OpenSSL::FImplDetails : public FEd25519CheckContextOpenSSL
{
};

FSignerEd25519OpenSSL::FSignerEd25519OpenSSL() : Inner(MakePimpl<FSignerEd25519OpenSSL::FImplDetails>())
{
	check(Inner);
}

FSignerEd25519OpenSSL::~FSignerEd25519OpenSSL()
{
}

bool FSignerEd25519OpenSSL::IsReady() const
{
	return Inner->IsReady();
}

bool FSignerEd25519OpenSSL::SetPrivateKey(const TArrayView<const uint8> Key)
{
	Inner->Shutdown();

	Inner->State = FImplDetails::EState::InitFailed;

	Inner->EvpMdCtx = EVP_MD_CTX_new();
	if (Inner->EvpMdCtx)
	{
		Inner->EvpPkey = EVP_PKEY_new_raw_private_key(EVP_PKEY_ED25519, NULL, Key.GetData(), Key.Num());
		if (Inner->EvpPkey)
		{
			if (EVP_DigestSignInit(Inner->EvpMdCtx, NULL, NULL, NULL, Inner->EvpPkey) == 1)
			{
				Inner->State = FImplDetails::EState::Ready;
			}
		}
...
bool FSignerEd25519OpenSSL::Sign(const TArrayView<const uint8> Message, const TArrayView<uint8> Signature)
{
	if (!Inner->IsReady() || Signature.Num() != OutputSignatureLength)
	{
		return false;
	}

	size_t SignatureLength = OutputSignatureLength;

	bool Succeeded = EVP_DigestSign(Inner->EvpMdCtx, Signature.GetData(), &SignatureLength, Message.GetData(), Message.Num()) == 1;
	if (!Succeeded)
	{
		Inner->State = FImplDetails::EState::InitFailed;
	}
...
struct FVerifierEd25519OpenSSL::FImplDetails : public FEd25519CheckContextOpenSSL
{
};

FVerifierEd25519OpenSSL::FVerifierEd25519OpenSSL() : Inner(MakePimpl<FVerifierEd25519OpenSSL::FImplDetails>())
{
	check(Inner);
}

FVerifierEd25519OpenSSL::~FVerifierEd25519OpenSSL()
{
}

bool FVerifierEd25519OpenSSL::IsReady() const
{
	return Inner->IsReady();
}

bool FVerifierEd25519OpenSSL::SetPublicKey(const TArrayView<const uint8> Key)
{
	Inner->Shutdown();
	...
	if (Inner->EvpMdCtx)
	{
		Inner->EvpPkey = EVP_PKEY_new_raw_public_key(EVP_PKEY_ED25519, NULL, Key.GetData(), Key.Num());
		if (Inner->EvpPkey)
		{
			if (EVP_DigestVerifyInit(Inner->EvpMdCtx, NULL, NULL, NULL, Inner->EvpPkey) == 1)
			{
				Inner->State = FImplDetails::EState::Ready;
			}
		}
...
bool FVerifierEd25519OpenSSL::Verify(const TArrayView<const uint8> Message, const TArrayView<const uint8> Signature)
{
	if (!Inner->IsReady())
	{
		return false;
	...
	return EVP_DigestVerify(Inner->EvpMdCtx, Signature.GetData(), Signature.Num(), Message.GetData(), Message.Num()) == 1;
}

TOptional<FCryptoKeyPair> FCryptoKeyGeneratorOpenSSL::GenerateEd25519() const
{
	TOptional<FCryptoKeyPair> Result;

	const FScopedEvpPKeyContext Context(EVP_PKEY_ED25519);
	if (!Context.Get())
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Error, "CryptoKeyGeneratorOpenSSL::GenerateEd25519. Unable to allocates public key algorithm context");
		return Result;
	}

	int ErrCode = EVP_PKEY_keygen_init(Context.Get());
	if (ErrCode != 1)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Error, "CryptoKeyGeneratorOpenSSL::GenerateEd25519. Unable to initialize a public key algorithm context. ErrCode=[%d]", ErrCode);
		return Result;
	}

	EVP_PKEY* PvtKey = nullptr;
	ON_SCOPE_EXIT
	{
		if (PvtKey != nullptr)
		{
	...
	ErrCode = EVP_PKEY_keygen(Context.Get(), &PvtKey);
	if (ErrCode != 1)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Error, "CryptoKeyGeneratorOpenSSL::GenerateEd25519. Unable to generate keys. ErrCode=[%d]", ErrCode);
		return Result;
	}

	FCryptoKeyPair KeyPair;
	KeyPair.PrivateKey.SetNumZeroed(32);
	size_t PrivateKeyLen = KeyPair.PrivateKey.Num();
	ErrCode = EVP_PKEY_get_raw_private_key(PvtKey, KeyPair.PrivateKey.GetData(), &PrivateKeyLen);
	if (ErrCode != 1)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Error, "CryptoKeyGeneratorOpenSSL::GenerateEd25519. Unable to get private key. ErrCode=[%d] PrivateKeyLen=[%llu]", ErrCode, static_cast<uint64_t>(PrivateKeyLen));
		return Result;
	}

	KeyPair.PublicKey.SetNumZeroed(32);
	size_t PublicKeyLen = KeyPair.PublicKey.Num();
	ErrCode = EVP_PKEY_get_raw_public_key(PvtKey, KeyPair.PublicKey.GetData(), &PublicKeyLen);
	if (ErrCode != 1)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Error, "CryptoKeyGeneratorOpenSSL::GenerateEd25519. Unable to get public key. ErrCode=[%d] PublicKeyLen=[%llu]", ErrCode, static_cast<uint64_t>(PublicKeyLen));
		return Result;
	}

	return Result.Emplace(MoveTemp(KeyPair));
}
...
bool FEncryptionContextOpenSSL::DigestVerify_RS256(const TArrayView<const uint8> Message, const TArrayView<const uint8> Signature, FRSAKeyHandle Key)
{
	...
	if (VerifyResult != 1)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Error,
			"[FEncryptionContextOpenSSL::DigestVerify_RS256] Signature is invalid: (%lu): %ls.",
...
bool FEncryptionContextOpenSSL::GenerateKey_RSA(const int32 InNumKeyBits, TArray<uint8>& OutPublicExponent, TArray<uint8>& OutPrivateExponent, TArray<uint8>& OutModulus)
{
	if (InNumKeyBits % 8 != 0)
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Warning, "FEncryptionContextOpenSSL::GenerateKey_RSA failed because InNumKeyBits is not divisible by 8.");
		return false;
	}
...
bool FEncryptionContextOpenSSL::CalcSHA256(const TArrayView<const uint8> Source, TArray<uint8>& OutHash)
{
	...
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Verbose,
			"[FEncryptionContextOpenSSL::CalcSHA256] Failed to create SHA256Hasher.");
	...
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Verbose,
			"[FEncryptionContextOpenSSL::CalcSHA256] Failed to update SHA256Hasher.");
	...
	{
		UE_LOGF(LogPlatformCryptoOpenSSL, Verbose,
			"[FEncryptionContextOpenSSL::CalcSHA256] Failed to finalize SHA256Hasher.");
