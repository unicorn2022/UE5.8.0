...
namespace PluginReferenceViewerUtils
{
	void ExportPlugins(const TArray<FString>& InArgs)
	{
		...
		{
			UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid plugin names argument. Expect plugin names separated by ',' as the first argument. e.g a,b,c");
			return;
		}
	...
	void ExportReference(const TArray<FString>& InArgs)
	{
		if (InArgs.Num() < 2)
		{
			UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid arguments. Expected: [plugin name] [reference name] (optional)[filename.csv]");
			return;
		}
	...
	void ExportAssetReference(const TArray<FString>& InArgs)
    	{
    		if (InArgs.Num() < 2)
    		{
    			UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid arguments. Expected: [plugin name] [reference name] (optional)[filename.csv]");
    		...
    		FPluginReferenceViewerUtils::ExportAssetReference(PluginName, ReferenceName, Filename);
    	}
	...
	void ExportDirectory(const TArray<FString>& InArgs)
	{
		...
		if (InArgs.Num() >= 1)
		{
			...
			{
				UE_LOGF(LogPluginReferenceViewerUtils, Error, "Directory does not exist %ls", *SearchDirectory);
				return;
			}
		}
		else
		{
			UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid arguments. Expected directory path");
			return;
		}
	...
	bool TracePathFromPluginToPlugin(const FString& StartPluginName, const FString& EndPluginName, FString& FoundPathToEndPlugin)
	{
		...
		{
			UE_LOGF(LogPluginReferenceViewerUtils, Error, "Plugin `%ls` could not be found!", *StartPluginName);
			return false;
		}
		...
		{
			UE_LOGF(LogPluginReferenceViewerUtils, Display, "No paths from plugin `%ls` to plugin '%ls' was found!", *StartPluginName, *EndPluginName);
			return false;
		}
		...
		TStringBuilder<256> PathString;
		PathString.Appendf(TEXT("Found dependency path of length %d : "), ReversePath.Num());
		PathString.Join(ReversePath, TEXTVIEW(" -> "));
		FoundPathToEndPlugin = PathString;
		UE_LOGF(LogPluginReferenceViewerUtils, Display, "%ls", *FoundPathToEndPlugin);
		return true;
	}
	...
	void TracePath(const TArray<FString>& InArgs)
	{
		...
		{
			UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid arguments. Expected plugin name as 1st arg");
			return;
		}
		...
		{
			UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid arguments. Expected plugin name as 2nd arg");
			return;
		}
...
namespace PluginReferenceViewerCVars
{
	static FAutoConsoleCommand ExportPlugins(
		...
		TEXT("Exports to .csv the number of references (by type) that a plugin has for each of it's dependencies.\n"
			"1st arg: single plugin name or multiple names separated with ','.\n"
	...
	static FAutoConsoleCommand ExportAssetReference(
    		TEXT("PluginReferenceViewer.ExportAssetReference"),
    		TEXT("Exports to .csv the list of asset references that exist between a plugin and one of it's dependencies, including which asset has the reference.\n"
    				"1st arg: plugin name.\n"
    				"2nd arg: reference name.\n"
    				"3rd arg (optional): output filename.\n"
    				"Example: PluginReferenceViewer.ExportAssetReference PluginName ReferenceName PluginReport.csv"),
    		FConsoleCommandWithArgsDelegate::CreateStatic(PluginReferenceViewerUtils::ExportAssetReference)
    	);
...
/*static*/TMap<FAssetIdentifier, TArray<FAssetIdentifier>> FPluginReferenceViewerUtils::GetAssetDependencyMap(const TSharedRef<IPlugin>& InPlugin)
{
	IAssetRegistry& AssetRegistry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry").Get();

	TMap<FAssetIdentifier, TArray<FAssetIdentifier>> AssetDependencies;
	...
	for (const FAssetData& AssetData : AssetsInPlugin)
	{
		...
		AssetDependencies.Add(FAssetIdentifier(AssetData.PackageName), AssetReferencers);
...
/*static*/ void FPluginReferenceViewerUtils::SplitByReferenceType(const TArray<FAssetIdentifier>& InAssetIdentifiers, TArray<FAssetIdentifier>& OutAssetReferences, TArray<FAssetIdentifier>& OutScriptReferences, TArray<FAssetIdentifier>& OutNameReferences)
{
	...
	for (const FAssetIdentifier& AssetIdentifier : InAssetIdentifiers)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogPluginReferenceViewerUtils, Display, "Skipping package '%ls' due to missing asset data. Package may no longer exist!", *AssetIdentifier.PackageName.ToString());
			}
		}
	}
}
...
/*static*/ void FPluginReferenceViewerUtils::ExportPlugins(const TArray<FString>& InPluginNames, const FString& InFilename)
{
	...
	if (Plugins.IsEmpty())
	{
		UE_LOGF(LogPluginReferenceViewerUtils, Error, "Plugin names array is empty");
		return;
	}

	if (FPaths::GetExtension(InFilename) != TEXT("csv"))
	{
		UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid filename extension '%ls'. Expected .csv", *InFilename);
		return;
	}
	...
	TStringBuilder<256> ConcatenatePluginNames;
	ConcatenatePluginNames.Join(InPluginNames, TEXTVIEW(", "));
	UE_LOGF(LogPluginReferenceViewerUtils, Display, "Exported plugins; '%ls' to '%ls'", *ConcatenatePluginNames, *FPaths::ConvertRelativePathToFull(InFilename));
}

/*static*/ void FPluginReferenceViewerUtils::ExportAssetReference(const FString& InPlugin, const FString& InReference, const FString& InFilename)
{
	const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(InPlugin);
	if (!Plugin.IsValid())
	{
		UE_LOGF(LogPluginReferenceViewerUtils, Error, "Plugin %ls was not found!", *InPlugin);
		return;
	}
	...
	if (Found == nullptr)
	{
		UE_LOGF(LogPluginReferenceViewerUtils, Error, "Plugin reference %ls was not found!", *InReference);
		return;
	}

	if (FPaths::GetExtension(InFilename) != TEXT("csv"))
	{
		UE_LOGF(LogPluginReferenceViewerUtils, Error, "Invalid filename extenstion '%ls'. Expected .csv", *InFilename);
		return;
	}
	...
	const TMap<FAssetIdentifier, TArray<FAssetIdentifier>> AllDependencies = FPluginReferenceViewerUtils::GetAssetDependencyMap(Plugin.ToSharedRef());
	for (const TPair<FAssetIdentifier, TArray<FAssetIdentifier>>& AssetDependency : AllDependencies)
	{
		const TMap<FString, TArray<FAssetIdentifier>> PluginAssetMap = FPluginReferenceViewerUtils::SplitByPlugins(Plugin.ToSharedRef(), AssetDependency.Value);
    	...
    	if (PluginAssets != nullptr)
    	{
    		...
    		for (TArray<FAssetIdentifier>* AssetList : { &AssetReferences, &ScriptReferences, &NameReferences })
    		{
    			...
    			{
    				StringBuilder.Reset();
    				StringBuilder.Append(AssetDependency.Key.ToString());
    				StringBuilder.AppendChar(Separator);
