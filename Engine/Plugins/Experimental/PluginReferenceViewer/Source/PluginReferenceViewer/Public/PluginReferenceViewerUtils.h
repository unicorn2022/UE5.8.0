...
public:
	...
	static UE_API TMap<FAssetIdentifier, TArray<FAssetIdentifier>> GetAssetDependencyMap(const TSharedRef<IPlugin>& InPlugin);
	...
	/* Exports to a .csv file the list of asset references that exist between the plugin and one of it's plugin dependencies by asset */
	static UE_API void ExportAssetReference(const FString& InPlugin, const FString& InReference, const FString& InFilename);
