...
void UNiagaraDataInterfacePhysicsField::SetShaderParameters(const FNiagaraDataInterfaceSetShaderParametersContext& Context) const
{
	...
	if (ProxyData != nullptr && ProxyData->FieldResource)
	{
		...
		ShaderInstanceParameters->NodesParams = FieldResource->NodesParams.SRV;
		ShaderInstanceParameters->NodesOffsets = FieldResource->NodesOffsets.SRV;
		ShaderInstanceParameters->TargetsOffsets = FieldResource->TargetsOffsets.SRV;
		ShaderInstanceParameters->TimeSeconds = ProxyData->TimeSeconds;
	...
	{
		// Instance Parameters
		ShaderInstanceParameters->NodesParams		= FNiagaraRenderer::GetDummyFloatBuffer();
		ShaderInstanceParameters->NodesOffsets	= FNiagaraRenderer::GetDummyIntBuffer();
		ShaderInstanceParameters->TargetsOffsets	= FNiagaraRenderer::GetDummyIntBuffer();
		ShaderInstanceParameters->TimeSeconds		= 0.0f;
