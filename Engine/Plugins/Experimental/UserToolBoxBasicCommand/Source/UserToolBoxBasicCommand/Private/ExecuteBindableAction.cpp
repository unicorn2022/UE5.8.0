...
void UExecuteBindableAction::Execute()
{
	if (CommandInfo. Context.IsEmpty())
	{
		...
		{
			UE_LOGF(LogUserToolBoxBasicCommand,Display,"%ls",*CurContext->GetContextName().ToString());
		}
		return;
	}
	...
	if (!Command.IsValid())
	{
		UE_LOGF(LogUserToolBoxBasicCommand,Warning,"%ls is not available, try these ones:",*CommandInfo.CommandName);
		...
		{
			UE_LOGF(LogUserToolBoxBasicCommand,Display,"%ls",*CurrentCommandInfo->GetCommandName().ToString());
		}
	}
	else
	{
