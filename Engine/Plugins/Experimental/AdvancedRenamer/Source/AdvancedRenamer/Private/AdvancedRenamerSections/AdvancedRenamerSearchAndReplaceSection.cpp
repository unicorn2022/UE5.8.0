		...
		for (int32 CharIndex = 0; CharIndex <= InReplaceString.Len(); ++CharIndex)
		{
			...
			if (bReadingGroupName)
			{
				...
				else if (GroupIndex > 0)
				{
					...
					{
						UE_LOGF(LogARP, Error, "Regex: Capture group does not exist %d.", GroupIndex);
					}
				...
				{
					UE_LOGF(LogARP, Error, "Regex: Unescaped %ls.", *GroupString);
			...
			else
			{
				...
				if (Char == NullChar)
				{
					UE_LOGF(LogARP, Error, "Regex: Unescaped %ls.", *EscapeString);
