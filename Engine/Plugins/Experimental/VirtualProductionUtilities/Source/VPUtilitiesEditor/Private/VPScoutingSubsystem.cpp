...
{
	UE_LOGF(LogVPUtilitiesEditor, Log, "VP Scouting subsystem initialized.");
...
{	
	...
	{
		UE_LOGF(LogVPUtilitiesEditor, Warning, "UVPScoutingSubsystem::ToggleVRScoutingUI - WidgetClass or PanelID can't be null.");
	...
	if (VRMode && VRMode->UISystemIsActive())
	{
		...
		else // Otherwise open a new one - with the user-defined VProd UI being the default
		{
			...
			{
				UE_LOGF(LogVPUtilitiesEditor, Warning, "UVPScoutingSubsystem::ToggleVRScoutingUI - Failed to open widget-based VR window.");
			}
		}
	}
}
