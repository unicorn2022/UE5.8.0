...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		else
		{
			...
			UE_LOGF(LogVPBookmarkEditor, Warning, "FVPBookmarkTypeActions::InitFromViewport has no valid component Bookmark: %ls Index: %d)", *GetPathNameSafe(VPBookmark), BookmarkIndex);
		}
...
AActor* FVPBookmarkTypeActions::SpawnBookmark(FEditorViewportClient& InViewportClient, const TSubclassOf<AActor> InActorClass, const FVPBookmarkCreationContext& InCreationContext, const FVector& InOffset, const bool bInFlattenRotation)
{
	...
	{
		UE_LOGF(LogVPBookmarkEditor, Error, "VPBookmarkTypeActions::SpawnBookmark - Invalid class");
		return nullptr;
	}

	if (!InActorClass.Get()->ImplementsInterface(UVPBookmarkProvider::StaticClass()))
	{
		UE_LOGF(LogVPBookmarkEditor, Warning, "VPBookmarkTypeActions::SpawnBookmark - The class '%ls' doesn't implement IVPBookmarkProvider", *(InActorClass.Get()->GetName()));
		return nullptr;
	}
	...
	if (World == nullptr)
	{
		UE_LOGF(LogVPBookmarkEditor, Error, "VPBookmarkTypeActions::SpawnBookmark - Unable to get world");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogVPBookmarkEditor, Error, "VPBookmarkTypeActions::SpawnBookmark - Unable to get world settings");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogVPBookmarkEditor, Warning, "VPBookmarkTypeActions::SpawnBookmark - Unable to add bookmark");
		return nullptr;
	}
	NewBookmark->Modify();
	...
	{
		UE_LOGF(LogVPBookmarkEditor, Error, "VPBookmarkTypeActions::SpawnBookmark - Unable to spawn the actor");
