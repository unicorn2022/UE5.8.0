...
bool UVPBookmarkBlueprintLibrary::CreateVPBookmarkName(AActor* Bookmark, const FString& FormatString, FString& GeneratedNumber, FString& GeneratedLetter)
{
	if (Bookmark == nullptr)
	{
		UE_LOGF(LogVPBookmark, Error, "UVPBookmarkBlueprintLibrary::CreateVPBookmarkName - No actor provided");
		return false;
	}

	if (!Bookmark->GetClass()->ImplementsInterface(UVPBookmarkProvider::StaticClass()))
	{
		UE_LOGF(LogVPBookmark, Warning, "UVPBookmarkBlueprintLibrary::CreateVPBookmarkName - Actor '%ls' doesn't implement IVPBookmarkProvider", *(Bookmark->GetClass()->GetName()));
		return false;
	}
