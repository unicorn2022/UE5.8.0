...
namespace UE::Learning
{
	FSharedMemoryTrainerServerProcess::FSharedMemoryTrainerServerProcess(
		...
		const FString& NneCpuContentPath,
		...
		const ELogSetting LogSettings)
	{
		...
		const FString CommandLineArguments = FString::Printf(TEXT("\"%s\" \"%s\" -p \"%s\" -m \"%s\" \"%s\" --nne-cpu-path \"%s\" SharedMemory \"%s\" -g \"%s\""),
		...
		const float InTimeout)
	{
		...
		{
			UE_LOGF(LogLearning, Error, "ExternalTrainerProcess was null, but attach to existing trainer process was false. Either pass in external trainer process or set bUseExternalTrainingProcess to true in the settings (along with associated settings).")
			return;
		}
		else
		{
	...
	ETrainerResponse FSharedMemoryTrainer::SendConfigs(
		...
		const TSharedRef<FJsonObject>& ModelConfigObject,
		...
		const ELogSetting LogSettings)
	{
		...
		for(int32 Index = 0; Index < NeuralNetworkSharedMemoryArrayViews.Num(); Index++)
		{
			...
			JsonObject->SetStringField(TEXT("Guid"), *NeuralNetworkSharedMemoryArrayViews[Index].GetPlatformGuidString());
		...
		{
			TSharedPtr<FJsonObject> JsonObject = MakeShared<FJsonObject>();
			JsonObject->SetStringField(TEXT("EpisodeStartsGuid"), *SharedMemoryExperienceContainer.EpisodeStarts.GetPlatformGuidString());
			JsonObject->SetStringField(TEXT("EpisodeLengthsGuid"), *SharedMemoryExperienceContainer.EpisodeLengths.GetPlatformGuidString());
			JsonObject->SetStringField(TEXT("EpisodeCompletionModesGuid"), *SharedMemoryExperienceContainer.EpisodeCompletionModes.GetPlatformGuidString());
			...
			{
				EpisodeFinalObservationsGuidsArray.Add(MakeShared<FJsonValueString>(*EpisodeFinalObservations.GetPlatformGuidString()));
			...
			{
				EpisodeFinalMemoryStatesGuidsArray.Add(MakeShared<FJsonValueString>(*EpisodeFinalMemoryStates.GetPlatformGuidString()));
			...
			{
				ObservationsGuidsArray.Add(MakeShared<FJsonValueString>(*Observations.GetPlatformGuidString()));
			...
			{
				ActionsGuidsArray.Add(MakeShared<FJsonValueString>(*Actions.GetPlatformGuidString()));
			...
			{
				ActionModifiersGuidsArray.Add(MakeShared<FJsonValueString>(*ActionModifiers.GetPlatformGuidString()));
			...
			{
				MemoryStatesGuidsArray.Add(MakeShared<FJsonValueString>(*MemoryStates.GetPlatformGuidString()));
			...
			{
				RewardsGuidsArray.Add(MakeShared<FJsonValueString>(*Rewards.GetPlatformGuidString()));
		...
		FFileHelper::SaveStringToFile(SharedMemoryConfigString, *(ConfigPath / FString::Printf(TEXT("shared-memory-%s.json"), *Controls.GetPlatformGuidString())));

		// Merge model config into data config for shared memory protocol compatibility
		for (const auto& Pair : ModelConfigObject->Values)
		{
			DataConfigObject->SetField(Pair.Key, Pair.Value);
		}
		...
		UE_LOGF(LogLearning, Display, "Wrote Config Files to %ls. Sending Config Signal...", *ConfigPath);
		...
		const ELogSetting LogSettings)
	{
		...
		const FString CommandLineArguments = FString::Printf(TEXT("\"%s\" \"%s\" -p \"%s\" -m \"%s\" \"%s\" --nne-cpu-path \"%s\" Socket \"%s:%i\" \"%s\""),
			*FileManager.ConvertToAbsolutePathForExternalAppForRead(*(PythonContentPath / TEXT("train.py"))),
			*TaskName,
			*FileManager.ConvertToAbsolutePathForExternalAppForRead(*CustomTrainerPath),
			*TrainerFileName,
		...
		const bool IsHostname)
	{
		...
		if (IsHostname)
		{
			...
			while (!ResolveInfo->IsComplete())
			{
				...
				if (WaitTime > Timeout)
				{
					UE_LOGF(LogLearning, Warning, "Timed-out waiting for DNS...");
					break;
				}
			}
			...
			{
				UE_LOGF(LogLearning, Error, "Unable to resolve hostname %ls. Error code %d...", IpAddressOrHostname, ResolveInfo->GetErrorCode());
		...
		else
		{
			...
			if (!bIsValid)
			{
				UE_LOGF(LogLearning, Error, "Invalid Ip Address \"%ls\"...", IpAddressOrHostname);
				OutResponse = ETrainerResponse::Unexpected;
				return;
			}
		}
	...
	ETrainerResponse FSocketTrainer::SendStop()
	{
		if (!Socket)
		{
			UE_LOGF(LogLearning, Error, "Training socket is nullptr");
			return ETrainerResponse::Unexpected;
		}
		...
		const ELogSetting LogSettings)
	{
		...
		// Merge model config into data config for socket protocol compatibility
		for (const auto& Pair : ModelConfigObject->Values)
		{
			DataConfigObject->SetField(Pair.Key, Pair.Value);
		}
		...
		const ELogSetting LogSettings)
	{
		...
		UE_LOGF(LogLearning, Display, "Checking if there is any newer networks... Current version: %d", NetworksVersion);
		...
		for (int32 i = 0; i < NetworkIds.Num(); i++)
		{
			...
			{
				UE_LOGF(LogLearning, Display, "Using the most updated networks version: %d", NetworksVersion);
	...
	FFileTrainer::FFileTrainer(const FString& InIntermediatePath, ETrainerExportFlags InExportFlags)
	{
		IntermediatePath = InIntermediatePath;
		FileSaveDirectory = IntermediatePath / TEXT("TrainingMaterials") / FDateTime::Now().ToString(TEXT("%Y%m%d%H%M%S"));
		ExportFlags = InExportFlags;
	}
		...
		const ELogSetting LogSettings)
	{
		...
		if (EnumHasAnyFlags(ExportFlags, ETrainerExportFlags::ReplayBuffers))
		{
			const FString ReplayBuffersDir = FileSaveDirectory / TEXT("ReplayBuffers");
			PlatformFile.CreateDirectoryTree(*ReplayBuffersDir);
			FString DataConfigString;
			TSharedRef<TJsonWriter<>> DataJsonWriter = TJsonWriterFactory<>::Create(&DataConfigString, 0);
			FJsonSerializer::Serialize(DataConfigObject, DataJsonWriter, true);
			FFileHelper::SaveStringToFile(DataConfigString, *(ReplayBuffersDir / "data-config.json"));
		}

		if (EnumHasAnyFlags(ExportFlags, ETrainerExportFlags::Networks))
		{
			const FString NetworksDir = FileSaveDirectory / TEXT("Networks");
			PlatformFile.CreateDirectoryTree(*NetworksDir);
			FString ModelConfigString;
			TSharedRef<TJsonWriter<>> ModelJsonWriter = TJsonWriterFactory<>::Create(&ModelConfigString, 0);
			FJsonSerializer::Serialize(ModelConfigObject, ModelJsonWriter, true);
			FFileHelper::SaveStringToFile(ModelConfigString, *(NetworksDir / "model-config.json"));
		}

		if (EnumHasAnyFlags(ExportFlags, ETrainerExportFlags::TrainerConfig))
		{
			FString TrainerConfigString;
			TSharedRef<TJsonWriter<>> TrainerJsonWriter = TJsonWriterFactory<>::Create(&TrainerConfigString, 0);
			FJsonSerializer::Serialize(TrainerConfigObject, TrainerJsonWriter, true);
		...
		UE_LOGF(LogLearning, Display, "FileTrainer: wrote config files to %ls.", *IntermediatePath);
		...
		const ELogSetting LogSettings)
	{
		...
		{
			UE_LOGF(LogLearning, Display, "FileTrainer: saved network %d snapshot to %ls", NetworkId, *BinPath);
		}
	...
	ETrainerResponse FFileTrainer::SendReplayBuffer(const int32 ReplayBufferId, const FReplayBuffer& ReplayBuffer, const ELogSetting LogSettings)
	{
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "FileTrainer: wrote replay buffer (%d eps, %d steps) to %ls", ReplayBuffer.GetEpisodeNum(), ReplayBuffer.GetStepNum(), *BinPath);
		}
