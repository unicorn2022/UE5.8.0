...
namespace UE::Learning
{
	...
	void FSubprocess::Terminate()
	{
		if (IsRunning())
		{
			UE_LOGF(LogLearning, Display, "Terminating Subprocess...");
	...
	bool FSubprocess::Update()
	{
		...
		for (int32 Idx = 0; Idx < OutputBuffer.Len(); Idx++)
		{
			...
			{
				UE_LOGF(LogLearning, Display, "Subprocess: %ls", *OutputBuffer.Mid(LineStartIdx, Idx - LineStartIdx));
...
namespace UE::Learning::Trainer
{
		...
		const Observation::FSchemaElement& ObservationSchemaElement)
	{
		...
		case Observation::EType::Continuous:
		{
			...
			Object->SetStringField(TEXT("Normalization"), Observation::GetNormalizationOptionAsString(Parameters.NormalizationOption));
			break;
		}
		...
		case Observation::EType::Sparse:
		{
			const Observation::FSchemaSparseParameters Parameters = ObservationSchema.GetSparse(ObservationSchemaElement);

			Object->SetStringField(TEXT("Type"), TEXT("Sparse"));
			Object->SetNumberField(TEXT("Num"), Parameters.Num);
			break;
		}

		case Observation::EType::NamedSparse:
		{
			const Observation::FSchemaNamedSparseParameters Parameters = ObservationSchema.GetNamedSparse(ObservationSchemaElement);

			Object->SetStringField(TEXT("Type"), TEXT("NamedSparse"));
		...
		const Action::FSchemaElement& ActionSchemaElement)
	{
		...
		case Action::EType::Continuous:
		{
			...
			Object->SetStringField(TEXT("Type"), TEXT("Continuous"));
			Object->SetNumberField(TEXT("Num"), Parameters.Num);
			Object->SetStringField(TEXT("Normalization"), Action::GetNormalizationOptionAsString(Parameters.NormalizationOption));
		...
		case Action::EType::Encoding:
		{
			...
			Object->SetStringField(TEXT("Type"), TEXT("Encoding"));
			Object->SetNumberField(TEXT("EncodingSize"), Parameters.EncodingSize);
			Object->SetNumberField(TEXT("EncodingDepth"), Parameters.LayerNum);
			Object->SetStringField(TEXT("ActivationFunction"), UE::Learning::Action::GetEncodingActivationAsString(Parameters.ActivationFunction));
			Object->SetObjectField(TEXT("Element"), ConvertActionSchemaToJSON(ActionSchema, Parameters.Element));
			break;
		}
	...
	bool IsObservationSchemaSubsetCompatible(const TSharedRef<FJsonObject> Object, const Observation::FSchema& ObservationSchema, const Observation::FSchemaElement& ObservationSchemaElement)
	{
		...
		case Observation::EType::Sparse:
		{
			...
			return
				Object->GetStringField(TEXT("Type")) == TEXT("Sparse") &&
				Object->GetNumberField(TEXT("Num")) == Parameters.Num;
		}
		...
		case Observation::EType::NamedSparse:
		{
			...
			if (Object->GetStringField(TEXT("Type")) != TEXT("NamedSparse"))
			{
				return false;
			}
		...
		const Observation::FSchemaElement& ObservationSchemaElement)
	{
		...
		if ((Object->GetStringField(TEXT("Type")) != TEXT("Null")) && ObservationSchema.GetType(ObservationSchemaElement) == Observation::EType::Null)
		{
			// Skipping element: schema has Null but source JSON has a non null type. This is an acceptable method to skip using certain actions for training
			int32 VectorSize = Object->GetNumberField(TEXT("VectorSize"));
		...
		else if (Object->GetStringField(TEXT("Type")) == TEXT("And"))
		{
			...
			TSharedPtr<FJsonObject> SubObject = Object->GetObjectField(TEXT("Elements"));
			TArray<FJsonObject::FStringType> ElementNames;
			SubObject->Values.GetKeys(ElementNames);

			for (const FJsonObject::FStringType& ElementName : ElementNames)
			{
				...
				for (int32 ElementIdx = 0; ElementIdx < Parameters.ElementNames.Num(); ElementIdx++)
				{
					if (ElementName == Parameters.ElementNames[ElementIdx])
					{
						bFound = true;
		...
		else if (Object->GetStringField(TEXT("Type")) == TEXT("Sparse"))
		{
			const Observation::FSchemaSparseParameters Parameters = ObservationSchema.GetSparse(ObservationSchemaElement);
		...
		else if (Object->GetStringField(TEXT("Type")) == TEXT("NamedSparse"))
		{
			const Observation::FSchemaNamedSparseParameters Parameters = ObservationSchema.GetNamedSparse(ObservationSchemaElement);
	...
	bool IsActionSchemaSubsetCompatible(const TSharedRef<FJsonObject> Object, const Action::FSchema& ActionSchema, const Action::FSchemaElement& ActionSchemaElement)
	{
		...
		case Action::EType::Encoding:
		{
			...
			if (Object->GetStringField(TEXT("Type")) != TEXT("Encoding") ||
				...
				Object->GetNumberField(TEXT("EncodingDepth")) != Parameters.LayerNum ||
				Object->GetStringField(TEXT("ActivationFunction")) != UE::Learning::Action::GetEncodingActivationAsString(Parameters.ActivationFunction))
			{
				return false;
			}
	...
	void ComputeActionSchemaSubsetIndices(int32& NextIndex, TArray<int32>& Indices,	const TSharedRef<FJsonObject> Object, const Action::FSchema& ActionSchema, const Action::FSchemaElement& ActionSchemaElement)
	{
		...
		if ((Object->GetStringField(TEXT("Type")) != TEXT("Null")) && ActionSchema.GetType(ActionSchemaElement) == Action::EType::Null)
		{
			// Skipping element: schema has Null but source JSON has a non null type. This is an acceptable method to skip using certain actions for training
			int32 VectorSize = Object->GetNumberField(TEXT("VectorSize"));
			NextIndex += VectorSize;
	...
	{
		return EngineDir / TEXT("Plugins/Experimental/LearningCore/Content/Python/");
	}
	...
	FString GetNneCpuContentPath()
	{
		TSharedPtr<IPlugin> NNERuntimeBasicCpuPlugin = IPluginManager::Get().FindEnabledPlugin(TEXT("NNERuntimeBasicCpu"));
		if (NNERuntimeBasicCpuPlugin.IsValid())
		{
			return FPaths::Combine(NNERuntimeBasicCpuPlugin->GetContentDir(), TEXT("Python"));
		}

		UE_LOGF(LogLearning, Error, "GetNneCpuContentPath: NNERuntimeBasicCpu plugin not found");
		return FString();
	}

	FString GetIntermediatePath(const FString& IntermediateDir)
	{
		return IntermediateDir / TEXT("LearningCore");
	}
