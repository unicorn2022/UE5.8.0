...
namespace UE::Learning::SocketTraining
{
		...
		const float SignalTimeout)
	{
		...
		bool bSuccess = false;
		{
			...
			{
				UE_LOGF(LogLearning, Error, "Error receiving network. Incorrect buffer size. Buffer is %lli bytes, expected %i.", OutNetworkBuffer.Num(), OutNetwork.GetSnapshotByteNum());
				bSuccess = false;
			}
			else
			{
		...
		const ELogSetting LogSettings)
	{
		...
		{
			UE_LOGF(LogLearning, Display, "Sending config...");
		}
		...
		const ELogSetting LogSettings)
	{
		...
		bool bSuccess = false;
		{
			...
			{
				UE_LOGF(LogLearning, Error, "Error sending network. Incorrect buffer size. Buffer is %lli bytes, expected %i.", NetworkBuffer.Num(), Network.GetSnapshotByteNum());
				bSuccess = false;
			}
			else
			{
