...
namespace UE::Learning::SharedMemoryTraining
{
		...
		const ELogSetting LogSettings)
	{
		...
		{
			UE_LOGF(LogLearning, Display, "Sending config signal...");
		}
		...
		const ELogSetting LogSettings)
	{
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "Pulling network...");
		}
		...
		bool bSuccess = false;
		{
			...
			{
				UE_LOGF(LogLearning, Error, "Error receiving network. Incorrect buffer size. Buffer is %lli bytes, expected %i.", NetworkData.Num(), OutNetwork.GetSnapshotByteNum());
				bSuccess = false;
			}
			else
			{
				...
				{
					UE_LOGF(LogLearning, Error, "Error receiving network. Invalid Format.");
					bSuccess = false;
				}
				else
				{
		...
		const ELogSetting LogSettings)
	{
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "Pushing network...");
		}
		...
		bool bSuccess = false;
		{
			...
			{
				UE_LOGF(LogLearning, Error, "Error sending network. Incorrect buffer size. Buffer is %lli bytes, expected %i.", NetworkData.Num(), Network.GetSnapshotByteNum());
				bSuccess = false;
			}
			else
			{
		...
		const ELogSetting LogSettings)
	{
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "Pushing Experience...");
		}
