...
{
	UE_LOGF(LogLearning, Display, "Running PPO Training Server Commandlet...");
	...
	const FString* NneCpuPathParam = Params.Find(TEXT("NneCpuPath"));
	const FString NneCpuContentPath = NneCpuPathParam ? *NneCpuPathParam : UE::Learning::Trainer::GetNneCpuContentPath();

	UE_LOGF(LogLearning, Display, "---  PPO Training Server Arguments ---");
	UE_LOGF(LogLearning, Display, "PythonExecutiblePath: %ls", *PythonExecutiblePath);
	UE_LOGF(LogLearning, Display, "PythonContentPath: %ls", *PythonContentPath);
	UE_LOGF(LogLearning, Display, "IntermediatePath: %ls", *IntermediatePath);
	UE_LOGF(LogLearning, Display, "NneCpuContentPath: %ls", *NneCpuContentPath);
	UE_LOGF(LogLearning, Display, "IpAddress: %ls", IpAddress);
	UE_LOGF(LogLearning, Display, "Port: %i", Port);
	UE_LOGF(LogLearning, Display, "LogSettings: %ls", LogSettings == UE::Learning::ELogSetting::Normal ? TEXT("Normal") : TEXT("Silent"));

	UE::Learning::FSocketTrainerServerProcess ServerProcess(
		TEXT("Training"),
		UE::Learning::Trainer::GetProjectPythonContentPath(),
		TEXT("learning_core.train_ppo"),
...
namespace UE::Learning::PPOTrainer
{
		...
		const ELogSetting LogSettings)
	{
		...
		{
			UE_LOGF(LogLearning, Display, "Sending initial Policy...");
		}
		...
		// Write Model Config
		TSharedRef<FJsonObject> ModelConfigObject = MakeShared<FJsonObject>();
		...
		ModelConfigObject->SetArrayField(TEXT("Networks"), NetworkObjects);
		...
		ExternalTrainer->SendConfigs(DataConfigObject, ModelConfigObject, TrainingConfigObject, LogSettings);
		...
		if (Response != ETrainerResponse::Success)
		{
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOGF(LogLearning, Error, "Error sending initial policy to trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
			}
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "Sending initial Critic...");
		}
		...
		if (Response != ETrainerResponse::Success)
		{
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOGF(LogLearning, Error, "Error sending initial critic to trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
			}
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "Sending initial Encoder...");
		}
		...
		if (Response != ETrainerResponse::Success)
		{
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOGF(LogLearning, Error, "Error sending initial encoder to trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
			}
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "Sending initial Decoder...");
		}
		...
		if (Response != ETrainerResponse::Success)
		{
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOGF(LogLearning, Error, "Error sending initial decoder to trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
			}
		...
		while (true)
		{
			if (bRequestTrainingStopSignal && (*bRequestTrainingStopSignal))
			{
				*bRequestTrainingStopSignal = false;
				...
				if (LogSettings != ELogSetting::Silent)
				{
					UE_LOGF(LogLearning, Display, "Stopping Training...");
				}
				...
				if (Response != ETrainerResponse::Success)
				{
					if (LogSettings != ELogSetting::Silent)
					{
						UE_LOGF(LogLearning, Error, "Error sending stop signal to trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
					}
			...
			else
			{
				...
				if (Response != ETrainerResponse::Success)
				{
					if (LogSettings != ELogSetting::Silent)
					{
						UE_LOGF(LogLearning, Error, "Error sending replay buffer to trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
					}
			...
			if (Response == ETrainerResponse::Completed)
			{
				if (LogSettings != ELogSetting::Silent)
				{
					UE_LOGF(LogLearning, Display, "Trainer completed training.");
				}
				break;
			}
			else if (Response != ETrainerResponse::Success)
			{
				if (LogSettings != ELogSetting::Silent)
				{
					UE_LOGF(LogLearning, Error, "Error receiving policy from trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
				}
				break;
			}
			...
			if (Response != ETrainerResponse::Success)
			{
				if (LogSettings != ELogSetting::Silent)
				{
					UE_LOGF(LogLearning, Error, "Error receiving critic from trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
				}
				break;
			}
			...
			if (Response != ETrainerResponse::Success)
			{
				if (LogSettings != ELogSetting::Silent)
				{
					UE_LOGF(LogLearning, Error, "Error receiving encoder from trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
				}
				break;
			}
			...
			if (Response != ETrainerResponse::Success)
			{
				if (LogSettings != ELogSetting::Silent)
				{
					UE_LOGF(LogLearning, Error, "Error receiving decoder from trainer: %ls. Check log for errors.", Trainer::GetResponseString(Response));
				}
				break;
			}
		...
		if (Response != ETrainerResponse::Success)
		{
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOGF(LogLearning, Error, "Error waiting for trainer to exit: %ls. Check log for errors.", Trainer::GetResponseString(Response));
			}
		}
		...
		if (LogSettings != ELogSetting::Silent)
		{
			UE_LOGF(LogLearning, Display, "Training Task Done!");
		}
