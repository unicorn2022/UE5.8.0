...
namespace UE::Learning
{
	...
	{
		virtual ~IExternalTrainer() = default;
		...
		*
		* @param DataConfigObject		The config to send with data-related settings (replay buffers, schemas)
		* @param ModelConfigObject		The config to send with model-related settings (networks, architecture)
	...
	*/
	...
	struct FFileTrainer : public IExternalTrainer
	{
		...
		*
		* @param InIntermediatePath				The Intermediate Path - for writing temporary files.
		* @param InExportFlags					Which sections to export (defaults to All).
		*/
		UE_API FFileTrainer(const FString& InIntermediatePath = FString(), ETrainerExportFlags InExportFlags = ETrainerExportFlags::All);
	...
	private:
		...
		ETrainerExportFlags ExportFlags = ETrainerExportFlags::All;
