...
namespace UE::Learning
{
	...
	*/
	...
	/** Flags controlling which sections a trainer exports. */
	enum class ETrainerExportFlags : uint8
	{
		None           = 0,
		ReplayBuffers  = 1 << 0,	// ReplayBuffers/ dir: replay binaries + data-config.json
		Networks       = 1 << 1,	// Networks/ dir: network binaries + model-config.json
		TrainerConfig  = 1 << 2,	// trainer-config.json
		All            = ReplayBuffers | Networks | TrainerConfig,
	};
	ENUM_CLASS_FLAGS(ETrainerExportFlags)
	...
	*/
	...
	namespace Trainer
	{
		...
		*/
		...
		/**
		* Gets the NNE Runtime Basic CPU Python content path from the plugin.
		*/
		UE_API FString GetNneCpuContentPath();
