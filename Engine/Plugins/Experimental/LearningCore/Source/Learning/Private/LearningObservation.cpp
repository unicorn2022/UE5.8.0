...
namespace UE::Learning::Observation
{
	...
	{
		template<typename T>
		static inline void ArraySerializeRaw(FArchive& Ar, TArray<T>& Array)
		{
			if (Ar.IsSaving())
			{
				int32 Num = Array.Num();
		...
		struct FContinuousDataDeprecated
		{
	...
	FSchemaElement FSchema::CreateContinuous(const FSchemaContinuousParameters Parameters, const FName Tag)
	{
		...
		ElementData.NormalizationOption = Parameters.NormalizationOption;

		if (Parameters.NormalizationOption == ENormalization::Manual)
		{
			check(Parameters.Scales.Num() == Parameters.Num or Parameters.Scales.IsEmpty());
			check(Parameters.Offsets.Num() == Parameters.Num or Parameters.Offsets.IsEmpty());

			ElementData.NormalizationIndexOffset = NormalizationScales.Num();

			// Use provided arrays or identity (std=1, mean=0)
			if (Parameters.Scales.Num() == Parameters.Num)
			{
				NormalizationScales.Append(Parameters.Scales.GetData(), Parameters.Scales.Num());
			}
			else
			{
				for (int32 i = 0; i < Parameters.Num; i++)
				{
					NormalizationScales.Add(1.0f);
				}
			}

			if (Parameters.Offsets.Num() == Parameters.Num)
			{
				NormalizationOffsets.Append(Parameters.Offsets.GetData(), Parameters.Offsets.Num());
			}
			else
			{
				for (int32 i = 0; i < Parameters.Num; i++)
				{
					NormalizationOffsets.Add(0.0f);
				}
			}
		}
	...
	FSchemaElement FSchema::CreateSparse(const FSchemaSparseParameters Parameters, const FName Tag)
	{
		FSparseData ElementData;
		ElementData.Num = Parameters.Num;

		const int32 Index = Types.Add(EType::Sparse);
		Tags.Add(Tag);
		ObservationVectorSizes.Add(Parameters.Num);
		EncodedVectorSizes.Add(Parameters.Num);
		ObservationDistributionVectorSizes.Add(Parameters.Num);
		TypeDataIndices.Add(SparseData.Add(ElementData));

		return { Index, Generation };
	}

	FSchemaElement FSchema::CreateNamedSparse(const FSchemaNamedSparseParameters Parameters, const FName Tag)
	{
		check(!Private::ContainsDuplicates(Parameters.ElementNames));

		FNamedSparseData ElementData;
		ElementData.Num = Parameters.ElementNames.Num();
		ElementData.ElementsOffset = SubElementObjects.Num();
		...
		const int32 Index = Types.Add(EType::NamedSparse);
		Tags.Add(Tag);
		ObservationVectorSizes.Add(Parameters.ElementNames.Num());
		EncodedVectorSizes.Add(Parameters.ElementNames.Num());
		ObservationDistributionVectorSizes.Add(Parameters.ElementNames.Num());
		TypeDataIndices.Add(NamedSparseData.Add(ElementData));
	...
	FSchemaContinuousParameters FSchema::GetContinuous(const FSchemaElement Element) const
	{
		...
		Parameters.NormalizationOption = ElementData.NormalizationOption;
		
		if (Parameters.NormalizationOption == ENormalization::Manual)
		{
			Parameters.Scales = TArrayView<const float>(NormalizationScales.GetData() + ElementData.NormalizationIndexOffset, ElementData.Num);
			Parameters.Offsets = TArrayView<const float>(NormalizationOffsets.GetData() + ElementData.NormalizationIndexOffset, ElementData.Num);
		}
	...
	FSchemaSparseParameters FSchema::GetSparse(const FSchemaElement Element) const
	{
		check(IsValid(Element) && GetType(Element) == EType::Sparse);
		const FSparseData& ElementData = SparseData[TypeDataIndices[Element.Index]];

		FSchemaSparseParameters Parameters;
		Parameters.Num = ElementData.Num;
		return Parameters;
	}

	FSchemaNamedSparseParameters FSchema::GetNamedSparse(const FSchemaElement Element) const
	{
		check(IsValid(Element) && GetType(Element) == EType::NamedSparse);
		const FNamedSparseData& ElementData = NamedSparseData[TypeDataIndices[Element.Index]];

		FSchemaNamedSparseParameters Parameters;
		Parameters.ElementNames = TArrayView<const FName>(SubElementNames.GetData() + ElementData.ElementsOffset, ElementData.Num);
		return Parameters;
	}

	void FSchema::SetGeneration(const uint32 InGeneration)
	{
	...
	void FSchema::Empty()
	{
		...
		Conv2dData.Empty();
		SparseData.Empty();
		NamedSparseData.Empty();

		SubElementNames.Empty();
		SubElementObjects.Empty();

		NormalizationScales.Empty();
		NormalizationOffsets.Empty();
	...
	void FSchema::Reset()
	{
		...
		Conv2dData.Reset();
		SparseData.Reset();
		NamedSparseData.Reset();

		SubElementNames.Reset();
		SubElementObjects.Reset();

		NormalizationScales.Reset();
		NormalizationOffsets.Reset();

		Generation++;
	}

	bool FSchema::Serialize(FArchive& Ar)
	{
		static constexpr int32 Magic = 0x7db41964;
		...
		if (Ar.IsLoading())
		{
			...
			Ar << LoadMagic;
			if (!ensure(LoadMagic == Magic)) { Ar.Seek(Offset); Empty(); return false; }
			Ar << LoadVersion;

			if (LoadVersion > Version) { Ar.Seek(Offset); Empty(); return false; }
		}
		else
		{
		...
		Ar << Types;
		Ar << Tags;
		Ar << ObservationVectorSizes;
		Ar << EncodedVectorSizes;
		Ar << ObservationDistributionVectorSizes;
		Ar << TypeDataIndices;

		if (LoadVersion == 0)
		{
			TArray<Private::FContinuousDataDeprecated> DeprecatedContinuousData;
			Private::ArraySerializeRaw(Ar, DeprecatedContinuousData);

			int32 TotalSize = 0;
			for (const Private::FContinuousDataDeprecated& DeprecatedData : DeprecatedContinuousData)
			{
				TotalSize += DeprecatedData.Num;
			}

			ContinuousData.Reserve(DeprecatedContinuousData.Num());
			NormalizationScales.Reserve(TotalSize);
			NormalizationOffsets.Reserve(TotalSize);

			for (const Private::FContinuousDataDeprecated& DeprecatedData : DeprecatedContinuousData)
			{
				FContinuousData CurrentData;
				CurrentData.Num = DeprecatedData.Num;
				CurrentData.NormalizationOption = ENormalization::Manual;
				CurrentData.NormalizationIndexOffset = NormalizationScales.Num();
				ContinuousData.Add(CurrentData);

				for (int32 Idx = 0; Idx < DeprecatedData.Num; Idx++)
				{
					NormalizationScales.Add(DeprecatedData.Scale);
					NormalizationOffsets.Add(0.0f);
				}
			}
		}
		...
		{
			Private::ArraySerializeRaw(Ar, ContinuousData);
		}

		Private::ArraySerializeRaw(Ar, DiscreteExclusiveData);
		Private::ArraySerializeRaw(Ar, DiscreteInclusiveData);
		Private::ArraySerializeRaw(Ar, NamedDiscreteExclusiveData);
		Private::ArraySerializeRaw(Ar, NamedDiscreteInclusiveData);
		Private::ArraySerializeRaw(Ar, AndData);
		Private::ArraySerializeRaw(Ar, OrExclusiveData);
		Private::ArraySerializeRaw(Ar, OrInclusiveData);
		Private::ArraySerializeRaw(Ar, ArrayData);
		Private::ArraySerializeRaw(Ar, SetData);
		Private::ArraySerializeRaw(Ar, EncodingData);
		Private::ArraySerializeRaw(Ar, Conv1dData);
		Private::ArraySerializeRaw(Ar, Conv2dData);

		if (LoadVersion > 1)
		{
			Private::ArraySerializeRaw(Ar, SparseData);
			Private::ArraySerializeRaw(Ar, NamedSparseData);
		}

		Ar << SubElementNames;
		Ar << SubElementObjects;

		if (LoadVersion > 0)
		{
			Private::ArraySerializeRaw(Ar, NormalizationScales);
			Private::ArraySerializeRaw(Ar, NormalizationOffsets);
		}
	...
	FObjectElement FObject::CreateSparse(const FObjectSparseParameters Parameters, const FName Tag)
	{
		check(Parameters.DiscreteIndices.Num() == Parameters.Values.Num());
	...
	FObjectElement FObject::CreateNamedSparse(const FObjectNamedSparseParameters Parameters, const FName Tag)
	{
		check(Parameters.ElementNames.Num() == Parameters.Values.Num());
		check(!Private::ContainsDuplicates(Parameters.ElementNames));
	...
	FObjectSparseParameters FObject::GetSparse(const FObjectElement Element) const
	{
		check(IsValid(Element) && GetType(Element) == EType::Sparse);

		FObjectSparseParameters Parameters;
		Parameters.DiscreteIndices = TArrayView<const int32>(DiscreteValues.GetData() + DiscreteDataOffsets[Element.Index], DiscreteDataNums[Element.Index]);
		Parameters.Values = TArrayView<const float>(ContinuousValues.GetData() + ContinuousDataOffsets[Element.Index], ContinuousDataNums[Element.Index]);
		return Parameters;
	}

	FObjectNamedSparseParameters FObject::GetNamedSparse(const FObjectElement Element) const
	{
		check(IsValid(Element) && GetType(Element) == EType::NamedSparse);

		FObjectNamedSparseParameters Parameters;
		Parameters.ElementNames = TArrayView<const FName>(SubElementNames.GetData() + SubElementDataOffsets[Element.Index], SubElementDataNums[Element.Index]);
		Parameters.Values = TArrayView<const float>(ContinuousValues.GetData() + ContinuousDataOffsets[Element.Index], ContinuousDataNums[Element.Index]);
		return Parameters;
	}

	void FObject::SetGeneration(const uint32 InGeneration)
	{
		Generation = InGeneration;
	}
	...
	bool FObject::Serialize(FArchive& Ar)
	{
		static constexpr int32 Magic = 0xf71282d9;
		...
		if (Ar.IsLoading())
		{
			...
			if (!ensure(LoadMagic == Magic)) { Ar.Seek(Offset); Empty(); return false; }
			Ar << LoadVersion;
			if (!ensure(LoadVersion == Version)) { Ar.Seek(Offset); Empty(); return false; }
		}
		else
		{
			int32 SaveMagic = Magic;
		...
		Ar << ContinuousDataOffsets;
		Ar << ContinuousDataNums;

		Ar << DiscreteDataOffsets;
		Ar << DiscreteDataNums;

		Ar << SubElementDataOffsets;
		Ar << SubElementDataNums;
		...
		const int32 Salt)
	{
		...
		case EType::Sparse:
		{
			return Private::HashCombine({ Hash, Private::HashInt(Schema.GetSparse(SchemaElement).Num) });
		}

		case EType::NamedSparse:
		{
			const FSchemaNamedSparseParameters Parameters = Schema.GetNamedSparse(SchemaElement);
			return Private::HashCombine({ Hash, Private::HashElements(Schema, Parameters.ElementNames, Salt) });
		}
		...
		const FSchemaElement SchemaElementB)
	{
		...
		case EType::Sparse: return SchemaA.GetSparse(SchemaElementA).Num == SchemaB.GetSparse(SchemaElementB).Num;

		case EType::NamedSparse:
		{
			const FSchemaNamedSparseParameters ParametersA = SchemaA.GetNamedSparse(SchemaElementA);
			const FSchemaNamedSparseParameters ParametersB = SchemaB.GetNamedSparse(SchemaElementB);
		...
		const FNetworkSettings& NetworkSettings)
	{
		...
		case EType::Sparse:
		{
			const int32 ValueNum = Schema.GetSparse(SchemaElement).Num;
		...
		case EType::NamedSparse:
		{
			const int32 ValueNum = Schema.GetNamedSparse(SchemaElement).ElementNames.Num();
		...
		const FObjectElement ObjectElement)
	{
		...
		case EType::Continuous:
		{
			...
			const FObjectContinuousParameters ObjectParameters = Object.GetContinuous(ObjectElement);

			const TArrayView<const float> ObservationValues = ObjectParameters.Values;
			...
			{
				float Value = ObservationValues[ValueIdx];

				if (SchemaParameters.NormalizationOption == ENormalization::Manual)
				{
					Value -= SchemaParameters.Offsets[ValueIdx];
					const float StdDev = FMath::Max(SchemaParameters.Scales[ValueIdx], UE_SMALL_NUMBER);
					Value /= StdDev;
				}
				
				OutObservationVector[ValueIdx] = Value;
			}
			return;
		}
		...
		case EType::Sparse:
		{
			const TArrayView<const int32> ObservationValues = Object.GetSparse(ObjectElement).DiscreteIndices;
			const TArrayView<const float> ObservationWeights = Object.GetSparse(ObjectElement).Values;
			check(ObservationValues.Num() == ObservationWeights.Num());
			check(Schema.GetObservationVectorSize(SchemaElement) >= ObservationValues.Num());
			check(Schema.GetObservationVectorSize(SchemaElement) >= ObservationWeights.Num());
			check(Schema.GetObservationVectorSize(SchemaElement) == OutObservationVector.Num());
			...
			{
				check(Schema.GetObservationVectorSize(SchemaElement) > ObservationValues[ObservationValueIdx] && ObservationValues[ObservationValueIdx] >= 0);
				OutObservationVector[ObservationValues[ObservationValueIdx]] = ObservationWeights[ObservationValueIdx];
			}
		...
		case EType::NamedSparse:
		{
			const TArrayView<const FName> SchemaNames = Schema.GetNamedSparse(SchemaElement).ElementNames;
			const TArrayView<const FName> ObservationValues = Object.GetNamedSparse(ObjectElement).ElementNames;
			const TArrayView<const float> ObservationWeights = Object.GetNamedSparse(ObjectElement).Values;
			check(ObservationValues.Num() == ObservationWeights.Num());
			check(Schema.GetObservationVectorSize(SchemaElement) >= ObservationValues.Num());
			check(Schema.GetObservationVectorSize(SchemaElement) == OutObservationVector.Num());
			...
			for (int32 ObservationValueIdx = 0; ObservationValueIdx < ObservationValues.Num(); ObservationValueIdx++)
			{
				const int32 ObservationIndex = SchemaNames.Find(ObservationValues[ObservationValueIdx]);
				check(ObservationIndex != INDEX_NONE);
				OutObservationVector[ObservationIndex] = ObservationWeights[ObservationValueIdx];
			}
			return;
		}
		...
		const TLearningArrayView<1, const float> ObservationVector)
	{
		...
		case EType::Continuous:
		{
			...
			for (int32 ValueIdx = 0; ValueIdx < ValueNum; ValueIdx++)
			{
				float Value = ObservationVector[ValueIdx];
				...
				ObservationValues[ValueIdx] = Value;
			}
		...
		case EType::Sparse:
		{
			check(ObservationVectorSize == Schema.GetSparse(SchemaElement).Num);

			// Find Indices
			TArray<int32, TInlineAllocator<8>> InclusiveIndices;
			TArray<float, TInlineAllocator<8>> InclusiveValues;
			InclusiveIndices.Reserve(ObservationVectorSize);
			InclusiveValues.Reserve(ObservationVectorSize);
			for (int32 Idx = 0; Idx < ObservationVectorSize; Idx++)
			{
				if (ObservationVector[Idx] != 0.0f)
				{
					InclusiveIndices.Add(Idx);
					InclusiveValues.Add(ObservationVector[Idx]);
				}
			}

			OutObjectElement = OutObject.CreateSparse({ InclusiveIndices, InclusiveValues }, SchemaElementTag);
			return;
		}
		...
		{
			const TArrayView<const FName> SchemaNames = Schema.GetNamedSparse(SchemaElement).ElementNames;
			check(ObservationVectorSize == Schema.GetNamedSparse(SchemaElement).ElementNames.Num());
			...
			OutObjectElement = OutObject.CreateNamedSparse({ InclusiveNames, InclusiveValues }, SchemaElementTag);
			return;
		}
	...
	void GetContinuousObservationVectorMask(
		...
		const FSchemaElement SchemaElement)
	{
		...
		case EType::And:
		{
			...
			for (int32 SchemaElementIdx = 0; SchemaElementIdx < Parameters.Elements.Num(); SchemaElementIdx++)
			{
				...
				GetContinuousObservationVectorMask(
...
FArchive& operator<<(FArchive& Ar, UE::Learning::Observation::FSchemaElement& SchemaElement)
{
	Ar << SchemaElement.Generation;
	Ar << SchemaElement.Index;
	return Ar;
}

FArchive& operator<<(FArchive& Ar, UE::Learning::Observation::FObjectElement& ObjectElement)
{
	Ar << ObjectElement.Generation;
	Ar << ObjectElement.Index;
	return Ar;
}
bool operator==(const UE::Learning::Observation::FSchemaElement& Lhs, const UE::Learning::Observation::FSchemaElement& Rhs)
{
	return Lhs.Generation == Rhs.Generation && Lhs.Index == Rhs.Index;
}

bool operator==(const UE::Learning::Observation::FObjectElement& Lhs, const UE::Learning::Observation::FObjectElement& Rhs)
{
	return Lhs.Generation == Rhs.Generation && Lhs.Index == Rhs.Index;
}

uint32 GetTypeHash(const UE::Learning::Observation::FSchemaElement& SchemaElement)
{
	return GetTypeHash(TPair<int32, int32>{ SchemaElement.Generation, SchemaElement.Index });
}

uint32 GetTypeHash(const UE::Learning::Observation::FObjectElement& ObjectElement)
{
	return GetTypeHash(TPair<int32, int32>{ ObjectElement.Generation, ObjectElement.Index });
}