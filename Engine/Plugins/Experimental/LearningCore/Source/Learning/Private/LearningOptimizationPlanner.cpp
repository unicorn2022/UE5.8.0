	...
	namespace OptimizationPlanner
	{
			...
			TAtomic<bool>* bActionVectorsUpdatedFlag)
		{
			...
			checkf(OptimizationPlannerBuffer.Samples.Num<0>() == SampleInstances.Num(),
				TEXT("Number of samples in OptimizationPlannerBuffer (%lli) must match the number of SampleInstances (%i)."),
			...
			checkf(OptimizationPlannerBuffer.Samples.Num<1>() == InOutActionVectors.Num<0>(),
				TEXT("Number of steps in OptimizationPlannerBuffer (%lli) must match the number of steps in InOutActionVectors (%lli)."),
			...
			TAtomic<bool>* bActionVectorsUpdatedFlag)
		{
			...
			checkf(WindowStepNum <= InOutActionVectors.Num<0>(),
				TEXT("WindowStepNum (%i) must be less than or equal to the number of steps in the action vectors (%lli)"),
			...
			checkf(OptimizationPlannerBuffer.Samples.Num<1>() == WindowStepNum,
				TEXT("Number of steps in OptimizationPlannerBuffer (%lli) must match WindowStepNum (%i)."),
