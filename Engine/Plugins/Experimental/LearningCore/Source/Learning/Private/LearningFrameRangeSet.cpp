...
namespace UE::Learning
{
	...
	{
		static inline bool ContainsSorted(const TLearningArrayView<1, const int32> Elements, const int32 Element)
		{
			const int32 Num = Elements.Num();

#if UE_LEARNING_ISPC
			return ispc::LearningContainsSorted(Elements.GetData(), Element, Num);
#else
			if (Num <= 512)
			{
		...
		static inline int32 FindSorted(const TLearningArrayView<1, const int32> Elements, const int32 Element)
		{
			const int32 Num = Elements.Num();

#if UE_LEARNING_ISPC
			return ispc::LearningFindSorted(Elements.GetData(), Element, Num);
#else
			if (Num <= 512)
			{
				for (int32 Idx = 0; Idx < Num; Idx++)
...
#if UE_LEARNING_ISPC
			return ispc::LearningComputeRangeOffsets(
				Offsets.GetData(),
				Lengths.GetData(),
				Lengths.Num(),
				InitialOffset);
		...
		static inline void FramesCopy(
			const TLearningArrayView<1, int32> DstFrames,
			const TLearningArrayView<1, const int32> SrcFrames)
		{
			check(DstFrames.Num() == SrcFrames.Num());

#if UE_LEARNING_ISPC
			return ispc::LearningFramesCopy(
				DstFrames.GetData(),
				SrcFrames.GetData(),
				DstFrames.Num());
#else

			const int32 FrameNum = DstFrames.Num();
		...
		static inline void RangesCopy(
			const TLearningArrayView<1, int32> DstStarts,
			const TLearningArrayView<1, int32> DstLengths,
			const TLearningArrayView<1, const int32> SrcStarts,
			const TLearningArrayView<1, const int32> SrcLengths)
		{
			check(DstStarts.Num() == DstLengths.Num());
			check(SrcStarts.Num() == SrcLengths.Num());
			check(DstStarts.Num() == SrcStarts.Num());

#if UE_LEARNING_ISPC
			return ispc::LearningRangesCopy(
				DstStarts.GetData(),
				DstLengths.GetData(),
				SrcStarts.GetData(),
				SrcLengths.GetData(),
			...
			const int32 RangeNum = DstStarts.Num();
			for (int32 RangeIdx = 0; RangeIdx < RangeNum; RangeIdx++)
			{
		...
		static inline void RangesCopyFrames(
			const TLearningArrayView<1, int32> DstStarts,
			const TLearningArrayView<1, int32> DstLengths,
			const TLearningArrayView<1, const int32> SrcFrames)
		{
			check(DstStarts.Num() == DstLengths.Num());
			check(DstStarts.Num() == SrcFrames.Num());

#if UE_LEARNING_ISPC
			return ispc::LearningRangesCopyFrames(
				DstStarts.GetData(),
				DstLengths.GetData(),
				SrcFrames.GetData(),
				DstStarts.Num());
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesEqual(
				LhsStarts.GetData(),
				LhsLengths.GetData(),
				RhsStarts.GetData(),
				RhsLengths.GetData(),
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesContains(
				Starts.GetData(),
				Lengths.GetData(),
				Frame,
				Starts.Num());
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesIntersectsRange(
				Starts.GetData(),
				Lengths.GetData(),
				Start,
				Length,
		...
		static inline bool RangesContainsRange(
			const TLearningArrayView<1, const int32> Starts,
			const TLearningArrayView<1, const int32> Lengths,
			const int32 Start,
			const int32 Length)
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesContainsRange(
				Starts.GetData(),
				Lengths.GetData(),
				Start,
				Length,
		...
		static inline bool RangesContainsTime(
			...
			const float Tolerance = UE_KINDA_SMALL_NUMBER)
		{
			check(Starts.Num() == Lengths.Num());

#if UE_LEARNING_ISPC
			return ispc::LearningRangesContainsTime(
				Starts.GetData(),
				Lengths.GetData(),
				Starts.Num(),
				Time,
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesFind(
				OutRangeIdx,
				OutRangeFrame,
				Starts.GetData(),
				Lengths.GetData(),
			...
			const float Tolerance = UE_KINDA_SMALL_NUMBER)
		{
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesFindTime(
				OutRangeIdx,
				OutRangeTime,
				Starts.GetData(),
				Lengths.GetData(),
			...
			for (int32 RangeIdx = 0; RangeIdx < RangeNum; RangeIdx++)
			{
				const float StartTime = Starts[RangeIdx] * FrameDeltaTime;
				const float EndTime = (Starts[RangeIdx] + Lengths[RangeIdx] - 1) * FrameDeltaTime;
				const float Duration = (Lengths[RangeIdx] - 1) * FrameDeltaTime;

				if (Time >= StartTime - Tolerance && Time < EndTime + Tolerance)
				{
					OutRangeIdx = RangeIdx;
					OutRangeTime = FMath::Clamp(Time - StartTime, 0.0f, Duration);
					return true;
				}
			}
			...
			const TLearningArrayView<1, const int32> RhsLengths)
		{
...
#if UE_LEARNING_ISPC
			return ispc::LearningFramesRangesUnion(
				OutStarts.GetData(),
				OutLengths.GetData(),
				LhsFrames.GetData(),
				RhsStarts.GetData(),
			...
			{
				RangesCopy(
					OutStarts.Slice(0, RhsStarts.Num()), 
					OutLengths.Slice(0, RhsLengths.Num()), 
					RhsStarts,
					RhsLengths);
			...
			{
				RangesCopyFrames(
					OutStarts.Slice(0, LhsFrames.Num()),
					OutLengths.Slice(0, LhsFrames.Num()),
					LhsFrames);
...
#if UE_LEARNING_ISPC
			return ispc::LearningFramesRangesIntersection(
				OutFrames.GetData(),
				LhsFrames.GetData(),
				RhsStarts.GetData(),
				RhsLengths.GetData(),
			...
			const TLearningArrayView<1, const int32> RhsLengths)
		{
...
#if UE_LEARNING_ISPC
			return ispc::LearningFramesRangesDifference(
				OutFrames.GetData(),
				LhsFrames.GetData(),
				RhsStarts.GetData(),
				RhsLengths.GetData(),
			...
			if (RhsStarts.IsEmpty())
			{
				FramesCopy(OutFrames.Slice(0, LhsFrames.Num()), LhsFrames);
				return LhsFrames.Num();
			}
			...
			const TLearningArrayView<1, const int32> RhsFrames)
		{
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesFramesDifference(
				OutStarts.GetData(),
				OutLengths.GetData(),
				LhsStarts.GetData(),
				LhsLengths.GetData(),
			...
			{
				RangesCopy(
					OutStarts.Slice(0, LhsStarts.Num()), 
					OutLengths.Slice(0, LhsLengths.Num()),
					LhsStarts,
					LhsLengths);
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesUnion(
				OutStarts.GetData(),
				OutLengths.GetData(),
				LhsStarts.GetData(),
				LhsLengths.GetData(),
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesIntersection(
				OutStarts.GetData(),
				OutLengths.GetData(),
				LhsStarts.GetData(),
				LhsLengths.GetData(),
		...
		{
#if UE_LEARNING_ISPC
			return ispc::LearningRangesTotalFrameNum(RangeLengths.GetData(), RangeLengths.Num());
#else
			int32 Total = 0;
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesIntersectionWithOffsets(
				OutStarts.GetData(),
				OutLengths.GetData(),
				OutLhsOffsets.GetData(),
				OutRhsOffsets.GetData(),
				LhsStarts.GetData(),
				LhsLengths.GetData(),
				RhsStarts.GetData(),
				RhsLengths.GetData(),
...
#if UE_LEARNING_ISPC
			return ispc::LearningRangesDifference(
				OutStarts.GetData(),
				OutLengths.GetData(),
				LhsStarts.GetData(),
				LhsLengths.GetData(),
		...
		static inline int32 RangesIntersects(
			TLearningArrayView<1, int32> OutStarts,
			TLearningArrayView<1, int32> OutLengths,
			const TLearningArrayView<1, const int32> LhsStarts,
			const TLearningArrayView<1, const int32> LhsLengths,
			...
			const TLearningArrayView<1, const int32> RhsLengths)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FrameRangeSet::Private::RangesIntersects);

#if UE_LEARNING_ISPC
			return ispc::LearningRangesIntersects(
				OutStarts.GetData(),
				OutLengths.GetData(),
				LhsStarts.GetData(),
				LhsLengths.GetData(),
		...
		static inline int32 RangesFramesIntersects(
			TLearningArrayView<1, int32> OutStarts,
			TLearningArrayView<1, int32> OutLengths,
			const TLearningArrayView<1, const int32> LhsStarts,
			const TLearningArrayView<1, const int32> LhsLengths,
			const TLearningArrayView<1, const int32> RhsFrames)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FrameRangeSet::Private::RangesFramesIntersects);

#if UE_LEARNING_ISPC
			return ispc::LearningRangesFramesIntersects(
				OutStarts.GetData(),
				OutLengths.GetData(),
				LhsStarts.GetData(),
				LhsLengths.GetData(),
	...
	int32 FFrameRangeSet::GetEntryRangeStop(const int32 EntryIdx, const int32 RangeIdx) const { return RangeStarts[EntryRangeOffsets[EntryIdx] + RangeIdx] + RangeLengths[EntryRangeOffsets[EntryIdx] + RangeIdx]; }
	...
	float FFrameRangeSet::GetAverageFrameNum() const
	{
		return GetTotalRangeNum() == 0 ? 0 : (float)GetTotalFrameNum() / (float)GetTotalRangeNum();
	}

	int32 FFrameRangeSet::GetMinFrameNum() const
	{
		const int32 RangeNum = GetTotalRangeNum();
		if (RangeNum == 0) { return 0; }
		int32 Minimum = INT32_MAX;
		for (int32 RangeIdx = 0; RangeIdx < RangeNum; RangeIdx++)
		{
			Minimum = FMath::Min(Minimum, RangeLengths[RangeIdx]);
		}
		return Minimum;
	}

	int32 FFrameRangeSet::GetMaxFrameNum() const
	{
		const int32 RangeNum = GetTotalRangeNum();
		if (RangeNum == 0) { return 0; }
		int32 Maximum = INT32_MIN;
		for (int32 RangeIdx = 0; RangeIdx < RangeNum; RangeIdx++)
		{
			Maximum = FMath::Max(Maximum, RangeLengths[RangeIdx]);
		}
		return Maximum;
	}
	...
	{
		return FrameRangeSet::Private::ContainsSorted(EntrySequences, Sequence);
	}
	...
	bool FFrameRangeSet::EntryContains(const int32 EntryIdx, const int32 Frame) const
	{
		return FrameRangeSet::Private::RangesContains(
			GetEntryRangeStarts(EntryIdx),
			GetEntryRangeLengths(EntryIdx),
			Frame);
	}
	...
	bool FFrameRangeSet::EntryContainsRange(const int32 EntryIdx, const int32 Start, const int32 Length) const
	{
		return FrameRangeSet::Private::RangesContainsRange(
			GetEntryRangeStarts(EntryIdx),
			GetEntryRangeLengths(EntryIdx),
			Start, Length);
	}

	bool FFrameRangeSet::EntryIntersectsRange(const int32 EntryIdx, const int32 Start, const int32 Length) const
	{
		return FrameRangeSet::Private::RangesIntersectsRange(
			GetEntryRangeStarts(EntryIdx),
			GetEntryRangeLengths(EntryIdx),
			Start,
			Length);
	...
	bool FFrameRangeSet::ContainsTime(const int32 Sequence, const float Time, const float FrameDeltaTime, const float Tolerance) const
	{
		const int32 EntryIdx = FindSequenceEntry(Sequence);
	...
	{
		return FrameRangeSet::Private::FindSorted(EntrySequences, Sequence);
	}
	...
	bool FFrameRangeSet::FindTime(int32& OutEntryIdx, int32& OutRangeIdx, float& OutRangeTime, const int32 Sequence, const float Time, const float FrameDeltaTime, const float Tolerance) const
	{
		const int32 EntryIdx = FindSequenceEntry(Sequence);
		...
		const TLearningArrayView<1, const int32> InLengths)
	{
		...
		FrameRangeSet::Private::RangesCopy(
			RangeStarts.Slice(CurrRangeNum, AddRangeNum),
			RangeLengths.Slice(CurrRangeNum, AddRangeNum),
			InStarts,
			InLengths);
	...
	namespace FrameRangeSet
	{
		bool Equal(const FFrameRangeSet& Lhs, const FFrameRangeSet& Rhs)
		{
			...
			return
				...
				ispc::LearningInt32ArraysEqual(Lhs.EntrySequences.GetData(), Rhs.EntrySequences.GetData(), Lhs.EntrySequences.Num()) &&
				ispc::LearningInt32ArraysEqual(Lhs.EntryRangeOffsets.GetData(), Rhs.EntryRangeOffsets.GetData(), Lhs.EntryRangeOffsets.Num()) &&
				ispc::LearningInt32ArraysEqual(Lhs.EntryRangeNums.GetData(), Rhs.EntryRangeNums.GetData(), Lhs.EntryRangeNums.Num()) &&
				ispc::LearningInt32ArraysEqual(Lhs.RangeStarts.GetData(), Rhs.RangeStarts.GetData(), Lhs.RangeStarts.Num()) &&
				ispc::LearningInt32ArraysEqual(Lhs.RangeLengths.GetData(), Rhs.RangeLengths.GetData(), Lhs.RangeLengths.Num()) &&
				ispc::LearningInt32ArraysEqual(Lhs.RangeOffsets.GetData(), Rhs.RangeOffsets.GetData(), Lhs.RangeOffsets.Num());
#else
			return
				Lhs.EntrySequences.Num() == Rhs.EntrySequences.Num() &&
				Lhs.RangeStarts.Num() == Rhs.RangeStarts.Num() &&
		...
		void Union(FFrameRangeSet& OutFrameRangeSet, const FFrameSet& FrameSet, const FFrameRangeSet& FrameRangeSet)
		{
			...
			while (LhsIndex < LhsEntryNum && RhsIndex < RhsEntryNum)
			{
				...
				if (FrameSet.GetEntrySequence(LhsIndex) < FrameRangeSet.GetEntrySequence(RhsIndex))
				{
					...
					FrameRangeSet::Private::RangesCopyFrames(
			...
			while (LhsIndex < LhsEntryNum && RhsIndex < RhsEntryNum)
			{
				...
				if (FrameSet.GetEntrySequence(LhsIndex) < FrameRangeSet.GetEntrySequence(RhsIndex))
				{
					...
					if (FrameSet.GetEntryFrameNum(LhsIndex) > 0)
					{
						...
						FrameRangeSet::Private::FramesCopy(
		...
		void Bounds(FFrameRangeSet& Out, const FFrameRangeSet& FrameRangeSet)
		{
			const int32 EntryNum = FrameRangeSet.GetEntryNum();

			Out.EntrySequences.SetNumUninitialized({ EntryNum });
			Out.EntryRangeOffsets.SetNumUninitialized({ EntryNum });
			Out.EntryRangeNums.SetNumUninitialized({ EntryNum });
			Out.RangeStarts.SetNumUninitialized({ EntryNum });
			Out.RangeLengths.SetNumUninitialized({ EntryNum });
			
			for (int32 EntryIdx = 0; EntryIdx < EntryNum; EntryIdx++)
			{
				Out.EntrySequences[EntryIdx] = FrameRangeSet.GetEntrySequence(EntryIdx);
				Out.EntryRangeOffsets[EntryIdx] = EntryIdx;
				Out.EntryRangeNums[EntryIdx] = 1;
				Out.RangeStarts[EntryIdx] = FrameRangeSet.GetEntryRangeStart(EntryIdx, 0);
				Out.RangeLengths[EntryIdx] = 
					FrameRangeSet.GetEntryRangeStop(EntryIdx, FrameRangeSet.GetEntryRangeNum(EntryIdx) - 1) - 
					FrameRangeSet.GetEntryRangeStart(EntryIdx, 0);
			}
			
			Out.RangeOffsets.SetNumUninitialized({ EntryNum });
			Private::ComputeRangeOffsets(Out.RangeOffsets, Out.RangeLengths);
			Out.Check();
		}

		void Bounds(FFrameRangeSet& Out, const FFrameSet& FrameSet)
		{
			...
			for (int32 EntryIdx = 0; EntryIdx < EntryNum; EntryIdx++)
			{
				Out.EntrySequences[EntryIdx] = FrameSet.GetEntrySequence(EntryIdx);
				Out.EntryRangeOffsets[EntryIdx] = EntryIdx;
				Out.EntryRangeNums[EntryIdx] = 1;
				Out.RangeStarts[EntryIdx] = FrameSet.GetEntryFrame(EntryIdx, 0);
				Out.RangeLengths[EntryIdx] =
					FrameSet.GetEntryFrame(EntryIdx, FrameSet.GetEntryFrameNum(EntryIdx) - 1) + 1 -
					FrameSet.GetEntryFrame(EntryIdx, 0);
			}
		...
		void Intersects(FFrameRangeSet& Out, const FFrameRangeSet& Lhs, const FFrameRangeSet& Rhs)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FrameRangeSet::Intersects);
			...
			Out.EntryRangeOffsets.SetNumUninitialized({ LhsEntryNum });
			Out.EntryRangeNums.SetNumUninitialized({ LhsEntryNum });
			Out.RangeStarts.SetNumUninitialized({ LhsRangeNum });
			Out.RangeLengths.SetNumUninitialized({ LhsRangeNum });
			...
			while (LhsIndex < LhsEntryNum && RhsIndex < RhsEntryNum)
			{
				...
				else
				{
					...
					// Append intersecting frames to output
					const int32 RangeNum = FrameRangeSet::Private::RangesIntersects(
						Out.RangeStarts.Slice(RangeIndex, Out.RangeStarts.Num() - RangeIndex),
						Out.RangeLengths.Slice(RangeIndex, Out.RangeLengths.Num() - RangeIndex),
						Lhs.GetEntryRangeStarts(LhsIndex),
						Lhs.GetEntryRangeLengths(LhsIndex),
					...
					check(RangeNum <= Lhs.GetEntryRangeNum(LhsIndex));
		...
		void Intersects(FFrameRangeSet& Out, const FFrameRangeSet& Lhs, const FFrameSet& Rhs)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(Learning::FrameRangeSet::Intersects);
			...
			while (LhsIndex < LhsEntryNum && RhsIndex < RhsEntryNum)
			{
				...
				{
					check(Lhs.GetEntryRangeNum(LhsIndex) > 0);
					check(Rhs.GetEntryFrameNum(RhsIndex) > 0);

					// Append intersecting frames to output
					const int32 RangeNum = FrameRangeSet::Private::RangesFramesIntersects(
						Out.RangeStarts.Slice(RangeIndex, Out.RangeStarts.Num() - RangeIndex),
						Out.RangeLengths.Slice(RangeIndex, Out.RangeLengths.Num() - RangeIndex),
						Lhs.GetEntryRangeStarts(LhsIndex),
						Lhs.GetEntryRangeLengths(LhsIndex),
		...
		void MakeFrameSetFromRangeInterval(FFrameSet& OutFrameSet, const FFrameRangeSet& FrameRangeSet, const int32 IntervalFrames)
		{
			check(IntervalFrames > 0);

			const int32 EntryNum = FrameRangeSet.GetEntryNum();

			OutFrameSet.EntrySequences = FrameRangeSet.EntrySequences;
			OutFrameSet.EntryFrameOffsets.SetNumUninitialized({ EntryNum });
			OutFrameSet.EntryFrameNums.SetNumUninitialized({ EntryNum });
			Array::Zero(OutFrameSet.EntryFrameOffsets);
			Array::Zero(OutFrameSet.EntryFrameNums);
			...
			for (int32 EntryIdx = 0; EntryIdx < EntryNum; EntryIdx++)
			{
				...
				for (int32 RangeIdx = 0; RangeIdx < RangeNum; RangeIdx++)
				{
					check(FrameRangeSet.GetEntryRangeLength(EntryIdx, RangeIdx) > 0);
					TotalFrameNum += (FrameRangeSet.GetEntryRangeLength(EntryIdx, RangeIdx) - 1) / IntervalFrames + 1;
				}
			}

			OutFrameSet.Frames.SetNumUninitialized({ TotalFrameNum });

			int32 FrameOffset = 0;
			for (int32 EntryIdx = 0; EntryIdx < EntryNum; EntryIdx++)
			{
				OutFrameSet.EntryFrameOffsets[EntryIdx] = FrameOffset;
				...
				for (int32 RangeIdx = 0; RangeIdx < RangeNum; RangeIdx++)
				{
					...
					for (int32 FrameIdx = RangeStart; FrameIdx < RangeStart + RangeLength; FrameIdx += IntervalFrames)
					{
						OutFrameSet.EntryFrameNums[EntryIdx]++;
						OutFrameSet.Frames[FrameOffset] = FrameIdx;
						FrameOffset++;
					}
				}
			}
