...
namespace UE::Learning::Action
{
	...
	void FSchema::Empty()
	{
		...
		ActionModifierVectorSizes.Empty();
	...
	void FSchema::Reset()
	{
		...
		ActionModifierVectorSizes.Reset();
	...
	{
		static constexpr int32 Magic = 0x25344a5c;
		static constexpr int32 Version = 1;

		bool bSerializeDeprecatedContinuousData = false;
		...
		if (Ar.IsLoading())
		{
			...
			if (LoadVersion == 0) { bSerializeDeprecatedContinuousData = true; }
			else if (!ensure(LoadVersion == Version)) { Ar.Seek(Offset); Empty(); return false; }
		}
		else
		{
		...
		Ar << ActionVectorSizes;
		Ar << ActionDistributionVectorSizes;
		Ar << ActionModifierVectorSizes;
		Ar << TypeDataIndices;

		if (bSerializeDeprecatedContinuousData)
		{
			...
			for (const Private::FContinuousDataDeprecated& DeprecatedData : DeprecatedContinuousData)
			{
				...
				for (int32 i = 0; i < DeprecatedData.Num; ++i)
				{
	...
	{
		static constexpr int32 Magic = 0x535a63fb;
	...
	void FModifier::SetGeneration(const uint32 InGeneration)
	{
		Generation = InGeneration;
	}
	...
	bool FModifier::Serialize(FArchive& Ar)
	{
		static constexpr int32 Magic = 0x5342479b;
		static constexpr int32 Version = 0;
		...
		Ar << ElementDataOffsets;
		Ar << ElementDataNums;
		Ar << MaskedDataOffsets;
		Ar << MaskedDataNums;

		Ar << ContinuousMaskeds;
		Ar << ContinuousMaskedValues;
		Ar << DiscreteValues;
		Ar << SubElementModifiers;
		Ar << SubElementNames;
		Ar << MaskedElementNames;
		...
		const FObjectElement ObjectElement)
	{
		...
		case EType::Continuous:
		{
			...
			for (int32 ValueIdx = 0; ValueIdx < ValueNum; ValueIdx++)
			{
				float Value = ActionValues[ValueIdx];				
				...
				OutActionVector[ValueIdx] = Value;
			}
		...
		const TLearningArrayView<1, const float> ActionVector)
	{
		...
		case EType::Continuous:
		{
			...
			for (int32 ValueIdx = 0; ValueIdx < ValueNum; ValueIdx++)
			{
				float Value = ActionVector[ValueIdx];
...
FArchive& operator<<(FArchive& Ar, UE::Learning::Action::FSchemaElement& SchemaElement)
{
...
FArchive& operator<<(FArchive& Ar, UE::Learning::Action::FObjectElement& ObjectElement)
{
...
FArchive& operator<<(FArchive& Ar, UE::Learning::Action::FModifierElement& ModifierElement)
{
	Ar << ModifierElement.Generation;
	Ar << ModifierElement.Index;
	return Ar;
}

bool operator==(const UE::Learning::Action::FSchemaElement& Lhs, const UE::Learning::Action::FSchemaElement& Rhs)
{
	return Lhs.Generation == Rhs.Generation && Lhs.Index == Rhs.Index;
}

bool operator==(const UE::Learning::Action::FObjectElement& Lhs, const UE::Learning::Action::FObjectElement& Rhs)
{
	return Lhs.Generation == Rhs.Generation && Lhs.Index == Rhs.Index;
}

bool operator==(const UE::Learning::Action::FModifierElement& Lhs, const UE::Learning::Action::FModifierElement& Rhs)
{
	return Lhs.Generation == Rhs.Generation && Lhs.Index == Rhs.Index;
}

uint32 GetTypeHash(const UE::Learning::Action::FSchemaElement& SchemaElement)
{
	return GetTypeHash(TPair<int32, int32>{ SchemaElement.Generation, SchemaElement.Index });
}

uint32 GetTypeHash(const UE::Learning::Action::FObjectElement& ObjectElement)
{
	return GetTypeHash(TPair<int32, int32>{ ObjectElement.Generation, ObjectElement.Index });
}

uint32 GetTypeHash(const UE::Learning::Action::FModifierElement& ModifierElement)
{
	return GetTypeHash(TPair<int32, int32>{ ModifierElement.Generation, ModifierElement.Index });
}