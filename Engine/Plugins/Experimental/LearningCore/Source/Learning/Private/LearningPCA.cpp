...
namespace UE::Learning
{
		...
		FProgress* Progress)
	{
		...
		{
			UE_LOGF(LogLearning, Display, "Computing PCA...");
		}
		...
		if (ensure(Info == Eigen::ComputationInfo::Success))
		{
			...
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOGF(LogLearning, Display, "--- PCA Cumulative Variance ---");
			}
			...
			for (int32 ValueIndex = 0; ValueIndex < FMath::Min((int32)VarianceRatio.size(), Settings.MaximumDimensions); ValueIndex++)
			{
				...
				if (LogSettings != ELogSetting::Silent)
				{
					UE_LOGF(LogLearning, Display, "%4i: %6.4f", ValueIndex, KeptVarianceRatio);
				}
			...
			if (LogSettings != ELogSetting::Silent)
			{
				UE_LOGF(LogLearning, Display, "Kept %i of %i Components", ComponentsToKeepNum, DimensionNum);
			}
		...
		{
			UE_LOGF(LogLearning, Warning, "PCA Computation Failed: %ls", Eigen::ComputationInfoString(Info));
			Empty();
		}
