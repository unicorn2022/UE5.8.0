...
namespace UE::Learning
{
	...
	namespace Array
	{
		void FloatArrayZero32(float* Data, const int32 Num)
		{
#if UE_LEARNING_ISPC
			ispc::LearningFrameAttributeZero(Data, Num);
#else
		...
		void FloatArraySet32(float* Data, const int32 Num, const float Value)
		{
#if UE_LEARNING_ISPC
			ispc::LearningFrameAttributeSet(Data, Num, Value);
#else
			for (int32 Idx = 0; Idx < Num; Idx++)
			{
				Data[Idx] = Value;
		...
		void FloatArrayCopy32(float* Dst, float* Src, const int32 Num)
		{
#if UE_LEARNING_ISPC
			ispc::LearningFrameAttributeCopy(Dst, Src, Num);
#else
			for (int32 Idx = 0; Idx < Num; Idx++)
			{
				Dst[Idx] = Src[Idx];
