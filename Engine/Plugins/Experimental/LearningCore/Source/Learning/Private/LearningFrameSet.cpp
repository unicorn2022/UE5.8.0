...
namespace UE::Learning
{
	...
	TLearningArrayView<1, const int32> FFrameSet::GetAllFrames() const { return Frames; }

	int32 FFrameSet::GetTotalFrameNum() const { return Frames.Num(); }
	
	bool FFrameSet::ContainsSequence(const int32 Sequence) const
	{
		return FrameSet::Private::ContainsSorted(EntrySequences, Sequence);
	}
	...
	{
		return FrameSet::Private::FindSorted(EntrySequences, Sequence);
	}
	...
	bool FFrameSet::Find(int32& OutEntryIdx, int32& OutFrameIdx, const int32 Sequence, const int32 Frame) const
	{
		...
		{
			const int32 FoundFrameIdx = FrameSet::Private::FindSorted(GetEntryFrames(EntryIdx), Frame);
	...
	bool FFrameSet::FindFramesInRange(int32& OutStartFrameIdx, int32& OutFrameNum, const int32 EntryIdx, const int32 RangeStart, const int32 RangeLength) const
	{	
		OutStartFrameIdx = INDEX_NONE;
		OutFrameNum = 0;
		...
		for (int32 FrameIdx = 0; FrameIdx < FrameNum; FrameIdx++)
		{
			const int32 Frame = GetEntryFrame(EntryIdx, FrameIdx);

			if (Frame >= RangeStart && Frame < RangeStart + RangeLength)
			{
				if (OutStartFrameIdx == INDEX_NONE)
				{
					OutStartFrameIdx = FrameIdx;
				}
				OutFrameNum++;
			}
		}

		return OutStartFrameIdx != INDEX_NONE;
	}
		...
		const TLearningArrayView<1, const int32> InFrames)
	{
		...
		FrameSet::Private::FramesCheck(InFrames);
