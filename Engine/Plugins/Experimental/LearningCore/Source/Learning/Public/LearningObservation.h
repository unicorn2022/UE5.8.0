...
namespace UE::Learning::Observation
{
	...
	enum class EType : uint8
	{
		...
		// Set of exclusive discrete observations
		DiscreteExclusive = 2,

		// Set of inclusive discrete observations
		DiscreteInclusive = 3,

		// Set of named exclusive discrete observations
		NamedDiscreteExclusive = 4,

		// Set of named inclusive discrete observations
		...
		// Sparse continuous observation indexed by integers
		Sparse = 14,

		// Sparse continuous observation indexed by names
		NamedSparse = 15,
	};

	enum class ENormalization : uint8
	{
		Manual = 0,
		AutoShared = 1,
		AutoPerDimension = 2,
	...
	static inline FString GetNormalizationOptionAsString(const ENormalization NormalizationOption)
	{
		switch (NormalizationOption)
		{
		case UE::Learning::Observation::ENormalization::Manual: return TEXT("Manual");
		case UE::Learning::Observation::ENormalization::AutoShared: return TEXT("AutoShared");
		case UE::Learning::Observation::ENormalization::AutoPerDimension: return TEXT("AutoPerDimension");
		case UE::Learning::Observation::ENormalization::AutoDimensionAverage: return TEXT("AutoDimensionAverage");
	...
	struct FSchemaContinuousParameters
	{
		...
		TArrayView<const float> Offsets;

		// The normalization option to use
		ENormalization NormalizationOption = ENormalization::Manual;
	};
	...
	struct FSchemaSparseParameters
	{
		// Number of values in the sparse observation
		int32 Num = 0;
	};

	struct FSchemaNamedSparseParameters
	{
		// Names of the sparse observation elements.
		TArrayView<const FName> ElementNames;
	};
	...
	struct FSchema
	{
		...
		UE_API FSchemaElement CreateSparse(const FSchemaSparseParameters Parameters, const FName Tag = NAME_None);
		UE_API FSchemaElement CreateNamedSparse(const FSchemaNamedSparseParameters Parameters, const FName Tag = NAME_None);
		...
		UE_API FSchemaSparseParameters GetSparse(const FSchemaElement Element) const;
		UE_API FSchemaNamedSparseParameters GetNamedSparse(const FSchemaElement Element) const;
		...
		UE_API void SetGeneration(const uint32 InGeneration);
		...
		// Serializes an observation schema. Returns false if serialization fails.
	...
	private:
		...
		struct FContinuousData
		{
			...
			int32 NormalizationIndexOffset = INDEX_NONE;
			ENormalization NormalizationOption = ENormalization::Manual;
		};
		...
		TArray<FSparseData> SparseData;
		TArray<FNamedSparseData> NamedSparseData;
		...
		/** Normalization arrays for continuous observations, indexed by NormalizationOffset in FContinuousData */
		TArray<float> NormalizationScales;
		TArray<float> NormalizationOffsets;
	};
	...
	struct FObjectNamedSparseParameters
	{
		// Names of the sparse observations.
		TArrayView<const FName> ElementNames;
	...
	struct FObject
	{
		...
		UE_API FObjectElement CreateSparse(const FObjectSparseParameters Parameters, const FName Tag = NAME_None);
		UE_API FObjectElement CreateNamedSparse(const FObjectNamedSparseParameters Parameters, const FName Tag = NAME_None);
		...
		UE_API FObjectSparseParameters GetSparse(const FObjectElement Element) const;
		UE_API FObjectNamedSparseParameters GetNamedSparse(const FObjectElement Element) const;
		...
		// Serializes an observation object. Returns false if serialization fails.
		UE_API bool Serialize(FArchive& Ar);
	 ...
	 * @param SchemaElement				Observation Schema Element
	 */
	UE_API void GetContinuousObservationVectorMask(
...
UE_API FArchive& operator<<(FArchive& Ar, UE::Learning::Observation::FSchemaElement& SchemaElement);
UE_API FArchive& operator<<(FArchive& Ar, UE::Learning::Observation::FObjectElement& ObjectElement);
UE_API bool operator==(const UE::Learning::Observation::FSchemaElement& Lhs, const UE::Learning::Observation::FSchemaElement& Rhs);
UE_API bool operator==(const UE::Learning::Observation::FObjectElement& Lhs, const UE::Learning::Observation::FObjectElement& Rhs);
UE_API uint32 GetTypeHash(const UE::Learning::Observation::FSchemaElement& SchemaElement);
UE_API uint32 GetTypeHash(const UE::Learning::Observation::FObjectElement& ObjectElement);
