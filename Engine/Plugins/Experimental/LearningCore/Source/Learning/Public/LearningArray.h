	...
	*/
	namespace Array
	{
			...
			else
			{
				for (const int32 Idx : Indices)
				{
					checkf(
						...
						TEXT("Invalid value %f found at flat array index %d"), View.GetData()[Idx], Idx);
				}
			}
		}
		...
		*/
		...
		// Specializations for when array data is float and size can be 32-bit

		UE_API void FloatArrayZero32(float* Data, const int32 Num);
		UE_API void FloatArraySet32(float* Data, const int32 Num, const float Value);
		UE_API void FloatArrayCopy32(float* Dst, float* Src, const int32 Num);
		...
		*/
		template<>
		inline void Zero<1, float>(TLearningArrayView<1, float> View)
		{
			if (View.NumBytes() < MAX_int32)
			{
				FloatArrayZero32(View.GetData(), View.Num());
			}
			else
			{
				FMemory::Memzero((void*)View.GetData(), View.Num() * sizeof(float));
			}
		}
		...
		*/
		template<>
		inline void Zero<2, float>(TLearningArrayView<2, float> View)
		{
			if (View.NumBytes() < MAX_int32)
			{
				FloatArrayZero32(View.GetData(), View.Num());
		...
		*/
		template<>
		inline void Set<1, float>(TLearningArrayView<1, float> View, const float& Element)
		{
			if (View.NumBytes() < MAX_int32)
			{
				FloatArraySet32(View.GetData(), View.Num(), Element);
			}
			else
			{
				for (int64 Idx = 0; Idx < View.Num(); Idx++)
		...
		*/
		template<>
		inline void Set<2, float>(TLearningArrayView<2, float> View, const float& Element)
		{
			if (View.NumBytes() < MAX_int32)
			{
				FloatArraySet32(View.GetData(), View.Num(), Element);
		...
		*/
		template<>
		inline void Copy<1, float>(
			TLearningArrayView<1, float> Dst,
			TLearningArrayView<1, float> Src)
		{
			CheckShapesEqual<1>(Dst.Shape(), Src.Shape());

			if (Src.NumBytes() < MAX_int32)
			{
				FloatArrayCopy32(Dst.GetData(), Src.GetData(), Dst.Num());
			}
			else
			{
				for (int64 Idx = 0; Idx < Dst.Num(); Idx++)
		...
		*/
		template<>
		inline void Copy<2, float>(
			TLearningArrayView<2, float> Dst,
			TLearningArrayView<2, float> Src)
		{
			CheckShapesEqual<2>(Dst.Shape(), Src.Shape());
		...
		/**
		* Check if the shapes of two arrays are equal
		*/
		template<uint8 InDimNum>
		inline bool ShapeEqual(
			const TLearningArrayShape<InDimNum>& Lhs,
			const TLearningArrayShape<InDimNum>& Rhs)
		{
			for (uint32 Idx = 0; Idx < InDimNum; Idx++)
			{
		...
		*/
		...
		{
			return Format<uint64>(Array, [](const uint64& Value) { return FString::Printf(TEXT("%llu"), Value); }, MaxItemNum);
		}
	}
}
