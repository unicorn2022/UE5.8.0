...
void FFastGeoInstancedSkinnedMeshComponent::Serialize(FArchive& Ar)
{
	...
	Ar << SceneProxyDesc.TransformProvider;
	Ar << SceneProxyDesc.bUseGpuLodSelection;
	Ar << SceneProxyDesc.bForceAnimateSockets;
	FArchive_Serialize_BitfieldBool(Ar, SceneProxyDesc.bAllowAlwaysVisible);
}
...
void FFastGeoInstancedSkinnedMeshComponent::UpdateSkinning()
{
	// FInstancedSkeletalMeshObject::Update consumes only LODIndex, but DynamicData must still be
	// seeded or UpdateSceneExtensionHeader asserts on check(DynamicData) for the GPU-skin path.
