...
const FFastGeoElementType FFastGeoSkinnedMeshComponentBase::Type(&FFastGeoMeshComponent::Type);
...
void FFastGeoSkinnedMeshComponentBase::Serialize(FArchive& Ar)
{
	...
	Ar << SceneProxyDesc.ForceStreamedLOD;
...
void FFastGeoSkinnedMeshComponentBase::InitializeSceneProxyDescDynamicProperties()
{
	...
	SceneProxyDesc.MinLODLevel = FSkinnedMeshComponentHelper::ComputeMinLOD(*this);
}
...
FPrimitiveSceneProxy* FFastGeoSkinnedMeshComponentBase::CreateSceneProxy(ESceneProxyCreationError* OutError)
{
	...
	{
		UE_LOGF(LogFastGeoStreaming, Verbose, "Skipping CreateSceneProxy for FastGeoSkinnedMeshComponent of %ls (component PSOs are still compiling)", *GetOwnerComponentCluster()->GetName());
		if (OutError)
		{
...
void FFastGeoSkinnedMeshComponentBase::DestroyMeshObject()
{
	if (MeshObject)
	{
		// ReleaseResources is safe from any thread (MPSC queues, ENQUEUE_RENDER_COMMAND).
		MeshObject->ReleaseResources();

		// BeginCleanup must be called from the game thread or parallel game thread.
		...
		{
			FSkeletalMeshObject* MeshObjectToCleanup = MeshObject;
			AsyncTask(ENamedThreads::GameThread, [MeshObjectToCleanup]()
			{
				BeginCleanup(MeshObjectToCleanup);
			});
		}
...
void FFastGeoSkinnedMeshComponentBase::ForEachMaterial(TFunctionRef<void(UMaterialInterface*, bool bIsNaniteOverride)> Func) const
{
	...
	{
		for (const FSkeletalMaterial& SlotMat : Asset->GetMaterials())
		{
	...
	const FSkinnedMeshSceneProxyDesc& Desc = GetSkinnedMeshSceneProxyDesc();
...
void FFastGeoSkinnedMeshComponentBase::UpdateSkinning()
{
	// Non-instanced FSkeletalMeshObject paths (GPUSkin, Static, CPU skin) consume the bone arrays
	// in Update - e.g. GPU skin uploads them as the per-frame pose. FastGeo doesn't tick animation,
	// so feed the asset's ref pose. The instanced subclass overrides this method to skip the fill.
	...
	RunMeshObjectUpdate(DynamicData);
}

void FFastGeoSkinnedMeshComponentBase::RunMeshObjectUpdate(FSkinnedMeshSceneProxyDynamicData& InOutDynamicData)
{
	...
	// Guard against asset/render-data inconsistency. Mirrors USkinnedMeshComponent::SendRenderDynamicData_Concurrent.
...
{
	return FSkinnedMeshSceneProxyDesc::CreateSceneProxy(SceneProxyDesc, bHideSkin);
}
