...
{
	if (const TViewModelPtr<ITrackExtension> TrackExtension = InSequencerOutlinerViewModel.ImplicitCast())
	{
		return FindItemsFromMovieSceneObject(TrackExtension->GetTrack());
	}

	if (const TViewModelPtr<ITrackRowExtension> TrackRowExtension = InSequencerOutlinerViewModel.ImplicitCast())
	{
		return FindItemsFromMovieSceneObject(TrackRowExtension->GetParentTrack());
	}
...
void FNavigationTool::BindEvents()
{
	if (const TSharedPtr<ISequencer> Sequencer = WeakSequencer.Pin())
	{
		Sequencer->OnChannelChangedWithTime().AddSPLambda(this, [this](const TArray<FFrameNumber>&,const FMovieSceneChannelMetaData*, UMovieSceneSection*)
			{
				OnTreeViewChanged();
			});
