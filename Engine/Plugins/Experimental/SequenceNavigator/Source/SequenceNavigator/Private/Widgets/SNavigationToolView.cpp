...
void SNavigationToolView::Construct(const FArguments& InArgs, const TSharedRef<FNavigationToolView>& InToolView)
{
	...
	{
		FilterBar->OnActivationStateChanged().AddSP(this, &SNavigationToolView::OnFilterActivationChanged);
		FilterBar->OnVisibilityChanged().AddSP(this, &SNavigationToolView::OnFilterVisibilityChanged);
		FilterBar->OnLayoutChanged().AddSP(this, &SNavigationToolView::OnFilterLayoutChanged);
		...
		.OnSlotResized_Lambda([this](const float InNewCoefficient)
			{
				if (UNavigationToolSettings* const ToolSettings = GetMutableDefault<UNavigationToolSettings>())
				{
					ToolSettings->SetLastFilterBarSizeCoefficient(InNewCoefficient, false);
				}
			})
		[
...
void SNavigationToolView::OnFilterActivationChanged()
{
	RebuildWidget();
}

void SNavigationToolView::OnFilterVisibilityChanged(bool bInIsVisible)
{
	RebuildWidget();
}

void SNavigationToolView::OnFilterLayoutChanged(EFilterBarLayout InLayout)
{
	RebuildWidget();
}
