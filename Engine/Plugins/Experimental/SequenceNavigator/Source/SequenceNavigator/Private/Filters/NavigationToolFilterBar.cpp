...
void FNavigationToolFilterBar::Init()
{
	...
	TextFilter->OnChanged().AddSP(this, &FNavigationToolFilterBar::HandleTextFilterChanged);
...
void FNavigationToolFilterBar::MuteFilters(const bool bInMute)
{
	...
	MuteFiltersChangedEvent.Broadcast(bFiltersMuted);
	RequestFilterUpdate();
}
...
void FNavigationToolFilterBar::GetCommonFiltersImpl(TArray<TSharedPtr<void>>& OutTypeErasedFilters) const
{
	for (const TSharedRef<FNavigationToolFilter>& Filter : CommonFilters->GetAllFilters())
	{
...
void FNavigationToolFilterBar::ActivateAllEnabledFilters(const bool bInActivate, const TArray<FString> InExceptionFilterNames)
{
	...
	// Broadcast state change if any filter is active, or all filters are being deactivated.
	if (!bInActivate || HasAnyFilterActive(false, false, true, true, true))
	{
		ActivationStateChangedEvent.Broadcast();
	}
}
...
void FNavigationToolFilterBar::ToggleFilterBarVisibility()
{
	...
	VisibilityChangedEvent.Broadcast(bNewVisibility);
}
...
void FNavigationToolFilterBar::SetToVerticalLayout()
{
	...
	LayoutChangedEvent.Broadcast(ToolSettings->GetFilterBarLayout());
}
...
void FNavigationToolFilterBar::HandleTextFilterChanged()
{
	TextFilterTextChangedEvent.Broadcast();
	RequestFilterUpdate();
}
...
void FNavigationToolFilterBar::OpenTextExpressionHelp()
{
	...
	Config.AddCommonFilterHelpEntries<FNavigationToolFilter>(SharedThis(this));
...
bool FNavigationToolFilterBar::DoesCommonFilterPass(const TViewModelPtr<FViewModel>& InViewModel, const FString& InFilterName) const
{
...
void FNavigationToolFilterBar::GetCommonFilterKeySuggestions(TArray<FSequencerFilterSuggestion>& OutSuggestions) const
{
	for (const TSharedRef<FNavigationToolFilter>& CommonFilter : CommonFilters->GetAllFilters())
	{
...
void FNavigationToolFilterBar::GetCommonFilterValueSuggestions(const FString& InKey, TArray<FSequencerFilterSuggestion>& OutSuggestions) const
{
	for (const TSharedRef<FNavigationToolFilter>& CommonFilter : CommonFilters->GetAllFilters())
	{
		if (InKey.Equals(CommonFilter->GetName(), ESearchCase::IgnoreCase))
		{
			// Suggestions for common filters text expressions will always be boolean for turning on/off
...
bool FNavigationToolFilterBar::IsCommonFilterName(const FString& InFilterName) const
{
	bool bFoundFilter = false;

	CommonFilters->ForEachFilter([&InFilterName, &bFoundFilter](const TSharedRef<FNavigationToolFilter>& InFilter)
		{
