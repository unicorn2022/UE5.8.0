...
struct OIDNState
{
	...
	void UpdateFilter(FIntPoint Size, FDenoiseSettings Settings)
	{
		...
		{
#if WITH_EDITOR
			UE_LOGF(LogOpenImageDenoise, Log, "Updating filter configuration for (%d x %d) with denoise_alpha=%s, denoise_aux=%s, use_aux=%s)", Size.X, Size.Y,
				   Settings.DenoiseAlpha ? "on" : "off",
				   Settings.DenoiseAux   ? "on" : "off",
				   Settings.UseAux       ? "on" : "off");
#endif
...
static void Denoise(OIDNState& DenoiserState, FRHICommandListImmediate& RHICmdList, FRHITexture* ColorTex, FRHITexture* AlbedoTex, FRHITexture* NormalTex, FRHITexture* OutputTex, FRHIGPUMask GPUMask)
{
	...
	{
		UE_LOGF(LogOpenImageDenoise, Warning, "Denoiser failed: %s", errorMessage);
		return;
	}

	FilterExecuteTime += FPlatformTime::Cycles64();
	const double FilterExecuteTimeMS = 1000.0 * FPlatformTime::ToSeconds64(FilterExecuteTime);
	UE_LOGF(LogOpenImageDenoise, Log, "Denoised %d x %d pixels in %.2f ms", Size.X, Size.Y, FilterExecuteTimeMS);
#endif
}
...
{
#if WITH_EDITOR
	UE_LOGF(LogOpenImageDenoise, Log, "OIDN starting up (built with OIDN %s", OIDN_VERSION_STRING);
#endif
...
{
#if WITH_EDITOR
	UE_LOGF(LogOpenImageDenoise, Log, "OIDN shutting down");
#endif
