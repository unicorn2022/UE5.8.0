...
struct FLayeredMoveBase
{
	...
	virtual ~FLayeredMoveBase() = default;
	...
	/** Adds all gameplay tags currently active on this layered move to InOutTags. */
	virtual void GetGameplayTags(FGameplayTagContainer& InOutTags) const {}
	...
	UE_API void StartMove_Async(UMoverBlackboard* SimBlackboard, const FMoverTime& SimTime);

	// Called when this layered move starts.
	virtual void OnStart(const UMoverComponent* MoverComp, UMoverBlackboard* SimBlackboard) {}
	// Async version of the above
	virtual void OnStart_Async(UMoverBlackboard* SimBlackboard, const FMoverTime& SimTime) {}
	...
	// Returns true if this move supports async (worker thread) execution via GenerateMove_Async.
	// Moves that return false will only have GenerateMove called on them, never GenerateMove_Async.
	...
	// Returns true if this move is considered nearly equal to Other for comparison purposes.
	...
	virtual bool IsNearlyEqualTo(const FLayeredMoveBase& Other) const
	{
...
struct FLayeredMoveGroup
{
	...
	// Populate Debug_ActiveMoves from ActiveLayeredMoves so CVD can display them via reflection.
	// Call just before tracing.
	UE_API void PrepareCVDMirror() const;
#endif
	...
	template <typename MoveType UE_REQUIRES(std::is_base_of_v<FLayeredMoveBase, MoveType>)>
	void ForEachActiveMoveOfType(TFunctionRef<void(MoveType&)> Func)
	{
		for (const TSharedPtr<FLayeredMoveBase>& Move : ActiveLayeredMoves)
		{
			if (Move.IsValid() && Move->GetScriptStruct()->IsChildOf(MoveType::StaticStruct()))
			{
				Func(*static_cast<MoveType*>(Move.Get()));
			}
		}
	}
	...
	template <typename MoveType UE_REQUIRES(std::is_base_of_v<FLayeredMoveBase, MoveType>)>
	void ForEachActiveMoveOfType(TFunctionRef<void(const MoveType&)> Func) const
	{
		for (const TSharedPtr<FLayeredMoveBase>& Move : ActiveLayeredMoves)
		{
			if (Move.IsValid() && Move->GetScriptStruct()->IsChildOf(MoveType::StaticStruct()))
			{
				Func(*static_cast<const MoveType*>(Move.Get()));
			}
		}
	}
...
protected:
	...
	UE_API void FlushMoveArrays(const UMoverComponent* MoverComp, UMoverBlackboard* SimBlackboard, const FMoverTime& SimTime, bool bIsAsync);
	...
	// Reflected snapshot of ActiveLayeredMoves, populated just before CVD tracing in
	// UChaosMoverSimulation::TraceMoverData. TSharedPtr arrays cannot be UPROPERTY so
	// this mirror is the only way for CVD's reflection-based display to see active moves.
	// mutable so it can be filled from a const reference during tracing.
	UPROPERTY(VisibleAnywhere, Transient, Category = Mover, DisplayName = "ActiveMoves")
	mutable TArray<FInstancedStruct> Debug_ActiveMoves;
