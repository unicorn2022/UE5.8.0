	...
	UE::Editor::DataStorage::RowHandle TargetRow, UE::Editor::DataStorage::RowHandle WidgetRow, const UE::Editor::DataStorage::FMetaDataView& Arguments)
{
	...
	UE::Editor::DataStorage::FMetaDataEntryView MetaDataEntryView = Arguments.FindForColumn<FSlateColorColumn>(UE::Editor::DataStorage::IsEditableName);

	if (MetaDataEntryView.IsSet())
	{
		const bool* IsEditable = MetaDataEntryView.TryGetExact<bool>();
