...
thread_local UE::Editor::DataStorage::FObjectCompatibilityImmediateScope* UEditorDataStorageCompatibility::ImmediateModeTracker = nullptr;

UEditorDataStorageCompatibility::~UEditorDataStorageCompatibility()
{
	UE_LOGF(LogEditorDataStorage, Log, "Destroyed TEDS Compatibility.");
}
...
void UEditorDataStorageCompatibility::Initialize(UEditorDataStorage* InStorage)
{
	...
	UE_LOGF(LogEditorDataStorage, Log, "Initializing TEDS Compatibility.");
	...
	Storage = InStorage;
	Environment = InStorage->GetEnvironmentPtr();
	...
	UE_LOGF(LogEditorDataStorage, Log, "Initialized TEDS Compatibility.");
}
...
void UEditorDataStorageCompatibility::Deinitialize()
{
	...
	UE_LOGF(LogEditorDataStorage, Log, "De-initializing TEDS Compatibility.");
	...
	ObjectRegistrationFilters.Reset();
	ObjectToRowDialiasers.Reset();
	TypeToTableMap.Reset();
	ObjectAddedCallbackList.Reset();
	PreObjectRemovedCallbackList.Reset();

	StandardUObjectTable = InvalidTableHandle;
	StandardExternalObjectTable = InvalidTableHandle;

	UE_LOGF(LogEditorDataStorage, Log, "De-initialized TEDS Compatibility.");
}
	...
	TWeakObjectPtr<UStruct> TypeInfo, UE::Editor::DataStorage::TableHandle Table)
{
	...
	if (Storage)
	{
		checkf(Storage->IsTableValid(Table), TEXT("Attempting to associate invalid table %" PRIu64 " with object type %s"),
			Table, TypeInfo.IsValid() ? *TypeInfo->GetName() : TEXT("<unknown>"));
...
UE::Editor::DataStorage::RowHandle UEditorDataStorageCompatibility::AddCompatibleObjectExplicit(UObject* Object)
{
	...
	bool bCanAddObject =
		ensureMsgf(Storage, TEXT("Trying to add a UObject to the editor's Data Storage before it's available.")) &&
		Object != nullptr &&
...
UE::Editor::DataStorage::RowHandle UEditorDataStorageCompatibility::AddCompatibleObjectExplicit(void* Object, TWeakObjectPtr<const UScriptStruct> TypeInfo)
{
	...
	checkf(Storage, TEXT("Trying to add an object to the editor's Data Storage before it's available."));
	FScopedExclusiveLock Lock(EGlobalLockScope::Public);
	
	RowHandle Result = FindRowWithCompatibleObjectExplicit(Object);
	if (!Storage->IsRowAvailable(Result))
	{
		if (const UScriptStruct* TypeInfoPtr = TypeInfo.Get(true))
		{
			Result = Storage->ReserveRow();
			Storage->MapRow(ICompatibilityProvider::ObjectMappingDomain, FMapKey(Object), Result);
			if (ImmediateModeTracker)
			{
				FObjectCompatibilityImmediateScopeWriteAccess::RegisterObject(*ImmediateModeTracker, Object, TypeInfo.Get(true), Result);
			}
			else if (Private::bUseCommandBuffer)
			{
...
void UEditorDataStorageCompatibility::EnterImmediateMode(UE::Editor::DataStorage::FObjectCompatibilityImmediateScope& Scope)
{
	FObjectCompatibilityImmediateScopeWriteAccess::SetPrevious(Scope, ImmediateModeTracker);
	ImmediateModeTracker = &Scope;
}

void UEditorDataStorageCompatibility::LeaveImmediateMode(UE::Editor::DataStorage::FObjectCompatibilityImmediateScope& Scope)
{
	using namespace UE::Editor::DataStorage;

	FObjectCompatibilityImmediateScope* CurrentTracker = ImmediateModeTracker;
	checkf(CurrentTracker, 
		TEXT("Attempting to leave immediate mode in the editor data storage's compatibility layer, but immediate was never entered."));
	checkf(CurrentTracker == &Scope, TEXT("Attempting to leave immediate mode in the editor data storage's compatibility ")
		TEXT("layer, but the active scope doesn't match the provided scope."));

	if (Scope.GetObjectCount() > 0)
	{
		FScopedExclusiveLock Lock(EGlobalLockScope::Public);

		FObjectCompatibilityImmediateScopeWriteAccess::ListUObjects(*CurrentTracker,
			[this](RowHandle Row, UObject* Object, const UClass* TypeInfo)
			{
				TableHandle Table = FindBestMatchingTable(TypeInfo);
				checkf(Table != InvalidTableHandle,
					TEXT("Unable to find a matching table in the compatibility layer for the editor's data storage for type '%s'"),
					*Object->GetClass()->GetName());

				Storage->AddRow(Row, Table, [this, Object](RowHandle Row)
					{
						FCommandProcessor::SetupRow(*Storage, *this, Row, Object);
					});
			});

		FObjectCompatibilityImmediateScopeWriteAccess::ListExternalObjects(*CurrentTracker,
			[this](RowHandle Row, void* Object, const UScriptStruct* TypeInfo)
			{
				TableHandle Table = FindBestMatchingTable(TypeInfo);
				Table = (Table != InvalidTableHandle) ? Table : StandardExternalObjectTable;
				Storage->AddRow(Row, Table, [this, Object, TypeInfo](RowHandle Row)
					{
						FCommandProcessor::SetupRow(*Storage, *this, Row, Object, TypeInfo);
					});
			});
	}

	ImmediateModeTracker = FObjectCompatibilityImmediateScopeWriteAccess::GetPrevious(*CurrentTracker);
}
...
void UEditorDataStorageCompatibility::CreateStandardArchetypes()
{
	...
	// Thread-safe as this is only called from a function that has an exclusive lock.
	using namespace UE::Editor::DataStorage;
	StandardUObjectTable = Storage->RegisterTable<
	...
	Storage->RegisterTableForeignKey(StandardUObjectTable, FName("Serialization"), 
			[](TConstQueryContext<RowBatchInfo> Context, TResult<FForeignKey>& Result, TConstBatch<FTypedElementUObjectColumn> Objects)
			{
				Context.ForEachRow([&Result](RowHandle Row, const FTypedElementUObjectColumn& Object)
					{
						const UObject* ObjectPtr = Object.Object.Get(true);
						Result.Add(Row, ObjectPtr
							? FForeignKey(ObjectPtr->GetPathName())
							: FForeignKey(FKeyIssue(TEXT("Row doesn't contain a valid UObject.")))
						);
					}, Objects);
			});
	Storage->RegisterTableForeignKey(StandardUObjectTable, FName("Memento"),
			[](TConstQueryContext<RowBatchInfo> Context, TResult<FForeignKey>& Result, TConstBatch<FTypedElementUObjectColumn> Objects)
			{
				Context.ForEachRow([&Result](RowHandle Row, const FTypedElementUObjectColumn& Object)
					{
	...
	StandardExternalObjectTable = Storage->RegisterTable<
...
UE::Editor::DataStorage::TableHandle UEditorDataStorageCompatibility::FindBestMatchingTable(const UStruct* TypeInfo) const
{
	...
	while (TypeInfo)
	{
		...
		{
			checkf(Storage && Storage->IsTableValid(*Table), TEXT("A previously registered table (%" PRIu64 ") has become invalid."), *Table);
			return *Table;
		}
...
UE::Editor::DataStorage::RowHandle UEditorDataStorageCompatibility::AddCompatibleObjectExplicitTransactionable(UObject* Object)
{
	...
	{	
		...
		{
			checkf(
				!(Object->GetInternalFlags() & EInternalObjectFlags_AsyncLoading),
				TEXT("Cannot add object '%s' as it's being asynchronously loaded to the editor's data storage while in immediate mode."),
				*Object->GetName());

			Result = Storage->ReserveRow();
			Storage->MapRow(ICompatibilityProvider::ObjectMappingDomain, FMapKey(Object), Result);
			FObjectCompatibilityImmediateScopeWriteAccess::RegisterObject(*ImmediateModeTracker, Object, nullptr, Result);
		}
		else
		{
	...
	UEditorDataStorageCompatibility& Compatibility, const TFunctionRef<void(UE::Editor::DataStorage::RowHandle, const AddressType&)>& SetupRowCallback)
{
	...
	// Double buffer because new entries can be added during processing of the queue. This can happen if there are observers that add
	// new UObjects or external objects in response to one being registered.
	Swap(Entries, ProcessingEntries);

	// Start by removing any entries that are no longer valid.
	for (auto It = ProcessingEntries.CreateIterator(); It; ++It)
	{
	...
	if (!ProcessingEntries.IsEmpty())
	{
		...
		for (FEntry& Entry : ProcessingEntries)
		{
			if constexpr (std::is_same_v<AddressType, TWeakObjectPtr<UObject>>)
			{
				switch (Private::GetAsyncLoadStrategy())
				{
					case Private::EAsyncUObjectLoadStrategy::Assert:
					{
						...
						checkf(StorageInterface.IsTableValid(Entry.Table),
							TEXT("Retrieved table (%" PRIu64 ") for object %s of type %s is invalid."), Entry.Table,
							*Entry.Address->GetName(), *Entry.Address->GetClass()->GetFName().ToString());
						break;
					}
		...
		FEntry* Current = ProcessingEntries.GetData();
		FEntry* End = Current + ProcessingEntries.Num();
		
		FEntry* TableFront = Current;

		// It is expected that InvalidTableHandle should always be sorted at the end
		// This assertion ensures that we catch a change in this behavior should the constant
		// be modified. 
		static_assert(InvalidTableHandle == TNumericLimits<TableHandle>::Max()); 
		if (ProcessingEntries[0].Table == InvalidTableHandle)
		{
			Entries.Append(ProcessingEntries);
			ProcessingEntries.Reset();
			return;
		}
		TableHandle CurrentTable = ProcessingEntries[0].Table;
		checkf(CurrentTable == InvalidTableHandle || StorageInterface.IsTableValid(CurrentTable),
			TEXT("The table (%" PRIu64 ") starting the processing queue has an unsupported value."), CurrentTable);
		...
		for (; Current != End; ++Current)
		{
			if (Current->Table != CurrentTable)
			{
				...
				checkf(CurrentTable == InvalidTableHandle || StorageInterface.IsTableValid(CurrentTable),
					TEXT("The next table (%" PRIu64 ") in the processing queue is invalid."), CurrentTable);
				Compatibility.RowScratchBuffer.Reset();
		...
		{
			// There are some objects that couldn't be added this frame. Take that list
			// and add it to the active entries list so they get picked up next frame.
			const int32 RemainingEntries = static_cast<int32>(End - Current);
			Entries.Reserve(Entries.Num() + RemainingEntries);
			for (; Current != End; ++Current)
			{
				Entries.Add(*Current);
			}
...
void UEditorDataStorageCompatibility::TickPendingCommands()
{
	...
	if (CommandCount > 0)
	{
		...
		if (Private::PrintCompatCommandBuffer > 0 )
		{
			...
			UE_LOGF(LogEditorDataStorage, Log, "Pending Commands:\n%ls%u Nops",
...
void UEditorDataStorageCompatibility::TickPendingUObjectRegistration()
{
	...
	if (Storage && !UObjectsPendingRegistration.IsEmpty())
	{
		checkf(Storage->IsTableValid(StandardUObjectTable), TEXT("Default UObject table has become invalid."));
		UObjectsPendingRegistration.ProcessEntries(*Storage, *this,
			...
			{
				FCommandProcessor::SetupRow(*Storage, *this, Row, Object.Get());
			});
	}
}
...
void UEditorDataStorageCompatibility::TickPendingExternalObjectRegistration()
{
	...
	if (Storage && !ExternalObjectsPendingRegistration.IsEmpty())
	{
		checkf(Storage->IsTableValid(StandardExternalObjectTable), TEXT("Default external object table has become invalid."));
		ExternalObjectsPendingRegistration.ProcessEntries(*Storage, *this,
			...
			{
				FCommandProcessor::SetupRow(*Storage, *this, Row, Object.Object, Object.TypeInfo);
			});
	}
}
