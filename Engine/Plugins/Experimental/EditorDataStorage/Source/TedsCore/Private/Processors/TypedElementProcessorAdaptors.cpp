...
namespace UE::Editor::DataStorage
{
	namespace Private
	{
		/** Check whether a query has any relation or hierarchy conditions that need per-entity filtering. */
		static bool HasRelationConditions(const FQueryDescription& Description)
		{
			return !Description.RelationConditions.IsEmpty()
				|| !Description.DescendantOfConditions.IsEmpty()
				|| !Description.AncestorOfConditions.IsEmpty();
		}

		/** Evaluate all relation/hierarchy conditions for a single row. Returns true if the row passes all conditions. */
		static bool PassesRelationConditions(RowHandle Row, const FQueryDescription& Description,
			FMassEntityManager& EntityManager, const FTedsRelationAdapter& RelationAdapter,
			const FTedsHierarchicalRelationManager* HierarchyManager)
		{
			// Check relation conditions (IsSubjectOf / IsObjectOf)
			for (const FQueryDescription::FRelationCondition& Condition : Description.RelationConditions)
			{
				if (Condition.RelationType == InvalidRelationTypeHandle)
				{
					continue;
				}

				if (Condition.Role == FQueryDescription::ERelationRole::Subject)
				{
					if (Condition.TargetRow != InvalidRowHandle)
					{
						// IsSubjectOf(Type, TargetObject) — check if this row is subject with that specific object
						if (!RelationAdapter.HasMassRelationData(Condition.RelationType) ||
								!EntityManager.GetRelationManager().IsSubjectOfRelation(
									RelationAdapter.GetMassHandle(Condition.RelationType),
									FMassEntityHandle::FromNumber(Row),
									FMassEntityHandle::FromNumber(Condition.TargetRow)))
						{
							return false;
						}
					}
					...
					{
						// IsSubjectOf(Type) — check if this row is a subject of any relation of this type
						if (!RelationAdapter.HasMassRelationData(Condition.RelationType) ||
							!EntityManager.GetRelationManager().HasRelationObjects(
								RelationAdapter.GetMassHandle(Condition.RelationType),
								FMassEntityHandle::FromNumber(Row)))
						{
							return false;
				...
				else // ERelationRole::Object
				{
					if (Condition.TargetRow != InvalidRowHandle)
					{
						// IsObjectOf(Type, TargetSubject) — check if this row is object with that specific subject
						if (!RelationAdapter.HasMassRelationData(Condition.RelationType) ||
								!EntityManager.GetRelationManager().IsSubjectOfRelation(
									RelationAdapter.GetMassHandle(Condition.RelationType),
									FMassEntityHandle::FromNumber(Condition.TargetRow),
					...
					{
						// IsObjectOf(Type) — check if this row is an object of any relation of this type
						if (!RelationAdapter.HasMassRelationData(Condition.RelationType) ||
							!EntityManager.GetRelationManager().HasRelationSubjects(
								RelationAdapter.GetMassHandle(Condition.RelationType),
								FMassEntityHandle::FromNumber(Row)))
						{
							return false;
			...
			// Check descendant-of conditions
			if (!Description.DescendantOfConditions.IsEmpty())
			{
				if (!HierarchyManager)
				{
					return false;
				}
				for (const FQueryDescription::FHierarchyCondition& Condition : Description.DescendantOfConditions)
				{
					if (Condition.RelationType == InvalidRelationTypeHandle || Condition.ReferenceRow == InvalidRowHandle)
					{
						continue;
					}
					if (Condition.bIncludeSelf && Row == Condition.ReferenceRow)
					{
						continue; // Passes — self-inclusion
					}
					if (!HierarchyManager->IsDescendantOf(EntityManager, RelationAdapter, Condition.RelationType, Row, Condition.ReferenceRow))
					{
						return false;
					}
				}
			...
			// Check ancestor-of conditions
			if (!Description.AncestorOfConditions.IsEmpty())
			{
				if (!HierarchyManager)
				{
					return false;
				}
				for (const FQueryDescription::FHierarchyCondition& Condition : Description.AncestorOfConditions)
				{
					if (Condition.RelationType == InvalidRelationTypeHandle || Condition.ReferenceRow == InvalidRowHandle)
					{
						continue;
					...
					// IsAncestorOf(Type, Descendant) means: is Descendant a descendant of Row?
					if (!HierarchyManager->IsDescendantOf(EntityManager, RelationAdapter, Condition.RelationType, Condition.ReferenceRow, Row))
					{
						return false;
					}
				}
		...
		/**
		 * Evaluate relation conditions for all entities in the current chunk and invoke the callback
		 * once per contiguous sub-range of passing entities. This performs entity-level relation filtering
		 * entirely within the TEDS adaptor layer, without requiring Mass-level EntityCondition support.
		 */
		static void ForEachRelationFilteredSubrange(
			FMassExecutionContext& Context,
			const FQueryDescription& Description,
			FMassEntityManager& EntityManager,
			const FTedsRelationAdapter& RelationAdapter,
			const FTedsHierarchicalRelationManager* HierarchyManager,
			TFunctionRef<void(int32 SubRangeStart, int32 SubRangeLength)> Callback)
		{
			TConstArrayView<FMassEntityHandle> Entities = Context.GetEntities();
			const int32 Num = Entities.Num();

			FMemMark Mark(FMemStack::Get());
			TArray<bool, TMemStackAllocator<>> Passes;
			Passes.SetNumUninitialized(Num);
			for (int32 i = 0; i < Num; ++i)
			{
				Passes[i] = PassesRelationConditions(
					Entities[i].AsNumber(), Description, EntityManager, RelationAdapter, HierarchyManager);
			}

			int32 RangeStart = INDEX_NONE;
			for (int32 i = 0; i <= Num; ++i)
			{
				const bool bPasses = (i < Num) && Passes[i];
				if (bPasses && RangeStart == INDEX_NONE)
				{
					RangeStart = i;
				}
				else if (!bPasses && RangeStart != INDEX_NONE)
				{
					Callback(RangeStart, i - RangeStart);
	...
	{
		/**
		 * Deferred command to change a row's relation object.
		 * Removes the existing relation (if any) then optionally creates a new one.
		 * Executes after the processor chunk loop, under ICoreProvider's normal write path.
		 */
		struct FSetRelationObjectCommand
		{
			RelationTypeHandle Type = InvalidRelationTypeHandle;
			RowHandle Target        = InvalidRowHandle;
			RowHandle NewObject     = InvalidRowHandle;   // InvalidRowHandle = remove only
			ICoreProvider* CoreProvider = nullptr;

			void operator()()
			{
				if (!CoreProvider || !CoreProvider->IsRowAssigned(Target))
				{
					return;
				}

				// Remove existing relation, if any
				const RowHandle OldObject = CoreProvider->GetRelationObject(Type, Target);
				if (OldObject != InvalidRowHandle)
				{
					CoreProvider->DestroyRelation(Type, Target, OldObject);
				}

				// Create new relation, if requested
				if (NewObject != InvalidRowHandle && CoreProvider->IsRowAssigned(NewObject))
				{
					CoreProvider->CreateRelation(Type, Target, NewObject);
				}
			}
		};
		...
		struct FMassContextCommon
		{
			...
			void SetSubRange(int32 InStart, int32 InLength)
			{
				SubRangeStart = InStart;
				SubRangeLength = InLength;
			}
			...
			{
				return HasSubRange() ? static_cast<uint32>(SubRangeLength) : Context.GetNumEntities();
			}

			TConstArrayView<RowHandle> GetRowHandles() const
			{
				TConstArrayView<RowHandle> All = MassEntitiesToRowsConversion(Context.GetEntities());
				return HasSubRange() ? All.Mid(SubRangeStart, SubRangeLength) : All;
			}

			const void* GetColumn(const UScriptStruct* ColumnType) const
			{
				const uint8* Base = reinterpret_cast<const uint8*>(Context.GetFragmentView(ColumnType).GetData());
				return HasSubRange() ? Base + SubRangeStart * ColumnType->GetStructureSize() : Base;
			}

			void* GetMutableColumn(const UScriptStruct* ColumnType)
			{
				uint8* Base = reinterpret_cast<uint8*>(Context.GetMutableFragmentView(ColumnType).GetData());
				return HasSubRange() ? Base + SubRangeStart * ColumnType->GetStructureSize() : Base;
			}
				...
				const EQueryAccessType* AccessTypes)
			{
				for (int32 Index = 0; Index < TypeCount; ++Index)
				{
					...
					if (*AccessTypes == EQueryAccessType::ReadWrite)
					{
						uint8* Base = reinterpret_cast<uint8*>(Context.GetMutableFragmentView(ColumnTypes->Get()).GetData());
						*RetrievedAddresses = reinterpret_cast<char*>(HasSubRange() ? Base + SubRangeStart * ColumnTypes->Get()->GetStructureSize() : Base);
					}
					else
					{
						const uint8* Base = reinterpret_cast<const uint8*>(Context.GetFragmentView(ColumnTypes->Get()).GetData());
						*RetrievedAddresses = const_cast<char*>(reinterpret_cast<const char*>(HasSubRange() ? Base + SubRangeStart * ColumnTypes->Get()->GetStructureSize() : Base));
					}
			...
			bool HasColumn(RowHandle Row, const UScriptStruct* ColumnType) const
			{
				...
				// Entity was invalid (either never was or was destroyed).  Invalid entities have no columns
				if (!Archetype.IsValid())
				{
					return false;
				}
				
				if (UE::Mass::IsA<FMassTag>(ColumnType) || UE::Mass::IsA<FMassFragment>(ColumnType))
				{
					const FMassArchetypeCompositionDescriptor& Composition = Manager.GetArchetypeComposition(Archetype);
					return Composition.GetElementsBitSet().Contains(ColumnType);
				}
				const bool bIsTagOrFragment = false;
				checkf(bIsTagOrFragment, TEXT("Attempting to check for a column type that is not a column or tag."));
				return false;
		...
		public:
			...
			~FMassWithEnvironmentContextCommon()
			{
				...
				if (bBatchDeferredCommands)
				{
					...
					// Process the per row operations
					{
						...
						{
							UniqueAdd.Add(TPair<FMassTagBitSet, FMassFragmentBitSet>(AddColumns->AddDescriptor.Get<FMassTagBitSet>(), AddColumns->AddDescriptor.Get<FMassFragmentBitSet>()));
						}
						...
						for (FAddedColumns* const AddColumns : AddedColumnsQueue)
						{
							...
							Operations.ColumnsChange.AddedTags = AddColumns->AddDescriptor.Get<FMassTagBitSet>();
							Operations.ColumnsChange.AddedFragments = AddColumns->AddDescriptor.Get<FMassFragmentBitSet>();
						}
						...
						for (FRemovedColumns* const RemovedColumns : RemovedColumnsQueue)
						{
							...
							Operations.ColumnsChange.RemovedTags += RemovedColumns->RemovedColumns.Get<FMassTagBitSet>();
							Operations.ColumnsChange.RemovedFragments += RemovedColumns->RemovedColumns.Get<FMassFragmentBitSet>();
						}
				...
				{ 
					this->Context.Defer().template PushCommand<FMassDeferredAddCommand>(
						[CommandData, this](FMassEntityManager& System)
						{
							...
							{
								FMassEntityHandle Entity = FMassEntityHandle::FromNumber(Row);
								if (System.IsEntityActive(Entity))
								{
			...
			void AddColumns(RowHandle Row, TConstArrayView<const UScriptStruct*> ColumnTypes) 
			{
				...
				{ 
					this->Context.Defer().template PushCommand<FMassDeferredAddCommand>(
						...
						{
							if (System.IsEntityActive(AddedColumns->Entity))
							{
			...
			void RemoveColumns(RowHandle Row, TConstArrayView<const UScriptStruct*> ColumnTypes) 
			{
				...
				RemovedColumns->RemovedColumns = FMassElementBitSet(ColumnTypes);
				...
				else
				{
					this->Context.Defer().template PushCommand<FMassDeferredRemoveCommand>(
						...
						{
							if (FMassArchetypeHandle Archetype = System.GetArchetypeForEntity(RemovedColumns->Entity); Archetype.IsValid())
							{
								FMassArchetypeEntityCollection Collection(Archetype, RemovedColumns->Entity);
								System.BatchChangeCompositionForEntities({ Collection }, {}, RemovedColumns->RemovedColumns);
							}
						});
				}
			}
			...
			void RemoveColumns(TConstArrayView<RowHandle> Rows, TConstArrayView<const UScriptStruct*> ColumnTypes) 
			{
				...
				case 0:
					return;
				case 1:
					RemoveColumns(*Rows.GetData(), ColumnTypes);
					return;
				default:
				{
					FScratchBuffer& ScratchBuffer = Environment.GetScratchBuffer();
					...
					else
					{
						this->Context.Defer().template PushCommand<FMassDeferredRemoveCommand>(
							[RemovedColumns](FMassEntityManager& System)
							{
								using EntityHandleArray = TArray<FMassEntityHandle, TInlineAllocator<32>>;
								using EntityArchetypeLookup = TMap<FMassArchetypeHandle, EntityHandleArray, TInlineSetAllocator<32>>;
								using EntityCollectionArray = TArray<FMassArchetypeEntityCollection, TInlineAllocator<32>>;
								...
								{
									if (FMassArchetypeHandle Archetype = System.GetArchetypeForEntity(Entity); Archetype.IsValid())
									{
										LookupTable.FindOrAdd(Archetype).Add(Entity);
									}
								}

								EntityCollectionArray EntityCollections;
								EntityCollections.Reserve(LookupTable.Num());
								for (EntityArchetypeLookup::TIterator It = LookupTable.CreateIterator(); It; ++It)
								{
									EntityCollections.Emplace(It.Key(), TConstArrayView<FMassEntityHandle>(It.Value()),
										FMassArchetypeEntityCollection::EDuplicatesHandling::FoldDuplicates);
								}

								System.BatchChangeCompositionForEntities(EntityCollections, {}, RemovedColumns->RemovedColumns);
							});
					}
				}}
			}
			...
			RowHandle AddRow(TableHandle Table) 
			{
				...
				FMassArchetypeHandle ArchetypeHandle = Environment.GetTableManager().GetArchetype(Table);
			...
			{
				FMassElementBitSet RemovedColumns;
				FMassEntityHandle Entity;
			};
		...
		using FMassDirectContextForwarderBase = Queries::Private::TImplementationForwarder<
			Queries::FProcessorContextEnvironment,
			IDirectQueryContext,
			Queries::FTableInfoContextCapability,
			Queries::FDynamicColumnInfoContextCapability>;
		struct FMassDirectContextForwarder final : FMassDirectContextForwarderBase
		{
			FMassDirectContextForwarder(FMassExecutionContext& InContext, FEnvironment& InEnvironment, FQueryDescription& Description)
				: FMassDirectContextForwarderBase(InEnvironment, InContext)
			...
			virtual void SetRelation(RelationTypeHandle Type, RowHandle Target, RowHandle NewObject) override
			{
				FSetRelationObjectCommand Command;
				Command.Type = Type;
				Command.Target = Target;
				Command.NewObject = NewObject;
				Command.CoreProvider = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName);
				ICommonQueryContext::PushCommand(MoveTemp(Command));
			}
		...
		using FMassSubqueryContextForwarderBase = Queries::Private::TImplementationForwarder<
			Queries::FProcessorContextEnvironment,
			ISubqueryContext,
			Queries::FTableInfoContextCapability,
			Queries::FDynamicColumnInfoContextCapability>;
		struct FMassSubqueryContextForwarder : FMassSubqueryContextForwarderBase
		{
			FMassSubqueryContextForwarder(FMassExecutionContext& InContext, FEnvironment& InEnvironment, FQueryDescription& Description)
				: FMassSubqueryContextForwarderBase(InEnvironment, InContext)
		...
		using FMassContextForwarderBase = Queries::Private::TImplementationForwarder<
			Queries::FProcessorContextEnvironment,
			IQueryContext,
			Queries::FTableInfoContextCapability,
			Queries::FDynamicColumnInfoContextCapability>;
		struct FMassContextForwarder final : FMassContextForwarderBase
		{
			FMassContextForwarder(
				...
				FExtendedQueryStore& InQueryStore,
				FEnvironment& InEnvironment)
					: FMassContextForwarderBase(InEnvironment, InContext)
			...
			void SetSubRange(int32 InStart, int32 InLength) { Implementation.SetSubRange(InStart, InLength); }
			void ClearSubRange() { Implementation.ClearSubRange(); }
	...
	UE::Editor::DataStorage::EDirectQueryExecutionFlags ExecutionFlags)
{
	...
	{
		const bool bHasRelationConditions = UE::Editor::DataStorage::Private::HasRelationConditions(Description);
		const FTedsRelationAdapter* RelationAdapter = bHasRelationConditions ? &Environment.GetRelationAdapter() : nullptr;
		const FTedsHierarchicalRelationManager* HierarchyManager = bHasRelationConditions ? &Environment.GetHierarchicalRelationManager() : nullptr;

		auto ExecuteFunction = [&Result, &Callback, &Description, &Environment, &EntityManager,
			bHasRelationConditions, RelationAdapter, HierarchyManager](FMassExecutionContext& Context)
			{
				using namespace Processors::Private;
				FMassDirectContextForwarder QueryContext(Context, Environment, Description);

				if (bHasRelationConditions)
				{
					UE::Editor::DataStorage::Private::ForEachRelationFilteredSubrange(
						Context, Description, EntityManager, *RelationAdapter, HierarchyManager,
						[&](int32 SubStart, int32 SubLength)
						{
							QueryContext.Implementation.SetSubRange(SubStart, SubLength);
							Callback(Description, QueryContext);
							Result.Count += SubLength;
						});
					QueryContext.Implementation.ClearSubRange();
				}
				else
				{
					Callback(Description, QueryContext);
		...
		Context.SetDeferredCommandBuffer(Environment.GetSharedMassCommandBuffer());
		Context.SetFlushDeferredCommands(false);

		if (EnumHasAnyFlags(ExecutionFlags, EDirectQueryExecutionFlags::ParallelizeChunks))
		{
			const FMassEntityQuery::EParallelExecutionFlags Mode =
...
UE::Editor::DataStorage::FQueryResult FTypedElementQueryProcessorData::Execute(
	...
	FMassEntityManager& EntityManager,
	UE::Editor::DataStorage::FEnvironment& Environment,
	UE::Editor::DataStorage::ERunQueryFlags ExecutionFlags)
{
	using namespace UE::Editor::DataStorage;
	...
	// Check compatibility here once instead of on the call itself to avoid doing the same thing for every chunk.
	if (!QueryContext_ProcessorApi::SupportsCapabilities(Callback.CapabilityMask))
	{
		QueryContext_ProcessorApi::StaticReportCompatibility(Callback.CapabilityMask);
		FQueryResult UnsupportedResult;
		UnsupportedResult.Completed = FQueryResult::ECompletion::Unsupported;
		return UnsupportedResult;
	}

	if (EnumHasAnyFlags(ExecutionFlags, ERunQueryFlags::AllowBoundQueries) || !Description.Callback.Function)
	{
		FMassExecutionContext Context(EntityManager);
		Context.SetDeferredCommandBuffer(Environment.GetSharedMassCommandBuffer());
		Context.SetFlushDeferredCommands(false);
		auto ExecuteFunction = [&Result, &Callback, &Environment](FMassExecutionContext& Context)
			{
				using namespace Processors::Private;
				FProcessorContextEnvironment MassContextEnvironment(Environment, Context);
				QueryContext_ProcessorApi QueryContext(MassContextEnvironment);
				// This assumes that there already has been a check that the provided function is compatible with the query.
				Callback.Call(QueryContext, MassContextEnvironment);
				Result.Count += Context.GetNumEntities();
			};
		if (EnumHasAnyFlags(ExecutionFlags, ERunQueryFlags::ParallelizeChunks))
		{
			FMassEntityQuery::EParallelExecutionFlags Mode =
				EnumHasAnyFlags(ExecutionFlags, ERunQueryFlags::AutoBalanceParallelChunkProcessing)
				? FMassEntityQuery::EParallelExecutionFlags::AutoBalance
				: FMassEntityQuery::EParallelExecutionFlags::Default;
			NativeQuery.ParallelForEachEntityChunk(Context, ExecuteFunction, Mode);
		}
...
UE::Editor::DataStorage::FQueryResult FTypedElementQueryProcessorData::Execute(
	const UE::Editor::DataStorage::Queries::TConstQueryFunction<void>& Callback,
	...
	UE::Editor::DataStorage::ERunQueryFlags ExecutionFlags)
{
	...
	if (EnumHasAnyFlags(ExecutionFlags, ERunQueryFlags::AllowBoundQueries) || !Description.Callback.Function)
	{
		...
		auto ExecuteFunction = [&Result, &Callback, &Environment, &Description, &EntityManager,
			bHasRelationConditions, RelationAdapter, HierarchyManager](FMassExecutionContext& Context)
			{
				using namespace Processors::Private;
				FConstProcessorContextEnvironment MassContextEnvironment(Environment, Context);
				QueryContext_ConstProcessorApi QueryContext(MassContextEnvironment);
				...
				if (bHasRelationConditions)
				{
					UE::Editor::DataStorage::Private::ForEachRelationFilteredSubrange(
						...
						[&](int32 SubStart, int32 SubLength)
						{
							MassContextEnvironment.SetSubRange(SubStart, SubLength);
							Callback.Call(QueryContext, MassContextEnvironment);
							Result.Count += SubLength;
						});
					MassContextEnvironment.ClearSubRange();
				}
				else
				{
					Callback.Call(QueryContext, MassContextEnvironment);
...
void FTypedElementQueryProcessorData::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	auto ExecuteFunction = [this, &Description, &EntityManager,
		bHasRelationConditions, RelationAdapter, HierarchyManager](FMassExecutionContext& Context)
		{
			if (PrepareCachedDependenciesOnQuery(Description, Context))
			{
				...
				if (bHasRelationConditions)
				{
					UE::Editor::DataStorage::Private::ForEachRelationFilteredSubrange(
						...
						[&](int32 SubStart, int32 SubLength)
						{
							QueryContext.SetSubRange(SubStart, SubLength);
							Description.Callback.Function(Description, QueryContext);
						});
					QueryContext.ClearSubRange();
				}
				else
				{
					Description.Callback.Function(Description, QueryContext);
	...
	UE::Editor::DataStorage::FEnvironment& Environment, TArrayView<FMassEntityQuery> Subqueries)
{
	...
	ObservedTypes.Reset();
	ObservedTypes.Add(Query.Description.Callback.MonitoredType);
