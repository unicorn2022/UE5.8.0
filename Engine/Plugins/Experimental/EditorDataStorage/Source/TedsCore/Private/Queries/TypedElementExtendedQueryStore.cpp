...
namespace UE::Editor::DataStorage
{
	...
	static void FORCENOINLINE PrintQueryCompositionToLog(const FMassArchetypeCompositionDescriptor& CompositionDescriptor)
	{
		...
		StringBuilder.Append( TEXT("Composition Descriptor:\nColumns: {"));
		{
			...
			const FMassFragmentBitSet FragmentBitSet = CompositionDescriptor.Get<FMassFragmentBitSet>();
			const int32 FragmentCount = FragmentBitSet.CountStoredTypes();
			for(auto IndexIterator = FragmentBitSet.GetIndexIterator(); IndexIterator; ++IndexIterator)
			{
				StringBuilder.Append(FragmentBitSet.GetTypeAtIndex(*IndexIterator)->GetName());
		...
		}
		{
			...
			const FMassTagBitSet TagBitSet = CompositionDescriptor.Get<FMassTagBitSet>();
			const int32 TagsCount = TagBitSet.CountStoredTypes();
			StringBuilder.Append(TEXT("Tags: {"));
			for(auto IndexIterator = TagBitSet.GetIndexIterator(); IndexIterator; ++IndexIterator)
			{
				StringBuilder.Append(TagBitSet.GetTypeAtIndex(*IndexIterator)->GetName());
		...
		UE_LOGF(LogEditorDataStorage, Error, "%ls", StringBuilder.ToString());
	}
	...
	bool FTedsQueryArchetypeMatchOverride::Match(const FMassArchetypeCompositionDescriptor& CompositionDescriptor) const
	{
		...
		if (Query->Description.Conditions)
		{
			...
			FMassFragmentBitSet FragmentBitSet = CompositionDescriptor.Get<FMassFragmentBitSet>();
			auto FragmentIterator = FragmentBitSet.GetIndexIterator(true);
			while (FragmentIterator)
			{
				const UScriptStruct* FragmentStruct = FragmentBitSet.GetTypeAtIndex(*FragmentIterator);
			...
			const FMassTagBitSet TagBitSet = CompositionDescriptor.Get<FMassTagBitSet>();
			auto TagIterator = TagBitSet.GetIndexIterator(true);
			while (TagIterator)
			{
				const UScriptStruct* TagStruct = TagBitSet.GetTypeAtIndex(*TagIterator);
	...
	uint64 FExtendedQueryStore::CalculateQueryTablesTopologyHash(Handle Query) const
	{
		const FExtendedQuery* QueryData = Get(Query);
		// Const cast used because CalculateArchetypeVersionHash is not const as it may need to redetermine what
		// archetypes match with the native query. This is however conceptually const from TEDS' point of view.
		return
			QueryData
			? const_cast<FExtendedQuery*>(QueryData)->NativeQuery.CalculateArchetypeVersionHash()
			: 0;
	}
	...
	template<typename CallbackReference, typename ExecutionFlagsType>
	FQueryResult FExtendedQueryStore::RunQueryCallbackCommon(
		FMassEntityManager& EntityManager,
		FEnvironment& Environment,
		FMassExecutionContext* ParentContext,
		Handle Query,
		ExecutionFlagsType ExecutionFlags,
		...
		if (FExtendedQuery* QueryData = Get(Query))
		{
			...
			case FQueryDescription::EActionType::Select:
				if (!QueryData->Processor.IsValid())
				{
					...
					{
						static_assert(std::is_same_v<ExecutionFlagsType, EDirectQueryExecutionFlags>, 
							"Running a direct query requires 'EDirectQueryExecutionFlags' flags.");
						Result = FTypedElementQueryProcessorData::Execute(
							Callback, QueryData->Description, QueryData->NativeQuery, EntityManager, Environment, ExecutionFlags);
					}
					else if constexpr (std::is_same_v<CallbackReference, Queries::TQueryFunction<void>>)
					{
						...
						{
							static_assert(std::is_same_v<ExecutionFlagsType, ERunQueryFlags>, 
								"Calling run query requires 'ERunQueryFlags' flags.");
							Result = FTypedElementQueryProcessorData::Execute(
								Callback, QueryData->Description, QueryData->NativeQuery, EntityManager, Environment, ExecutionFlags);
						}
						else
	...
	FQueryResult FExtendedQueryStore::RunQueryCallbackCommon(
		...
		FMassExecutionContext* ParentContext,
		Handle Query,
		ERunQueryFlags ExecutionFlags,
		const Queries::TConstQueryFunction<void>& Callback) const
	{
		FQueryResult Result;
		if (const FExtendedQuery* QueryData = Get(Query))
		{
			switch (QueryData->Description.Action)
			{
			case FQueryDescription::EActionType::None:
	...
	{
		return RunQueryCallbackCommon(EntityManager, Environment, nullptr, Query, Flags, Callback);
	}
	...
	bool FExtendedQueryStore::SetupHierarchyDependencies(FQueryDescription& Query, FEnvironment& Environment)
	{
		if (!Query.Hierarchy.IsNone())
		{
			...
			if (HierarchyAccessInterface)
			{
				...
				// Relations-backed hierarchies store data on relation rows rather than
				// participant rows, so GetHierarchyDataColumnType() returns null for them.
				// Skip adding a null type -- the query still works for those backends.
	...
	bool FExtendedQueryStore::CheckCompatibility(const FQueryDescription& QueryDescription, const Queries::TConstQueryFunction<void> QueryCallback) const
	{
		for (const UScriptStruct* Column : QueryCallback.ConstColumnTypes)
		{
			if (int32 Index = QueryDescription.SelectionTypes.Find(Column);
