...
namespace UE::Editor::DataStorage
{
	...
	FDynamicColumnGeneratorInfo FDynamicColumnGenerator::GenerateColumn(const UScriptStruct& Template, const FName& Identifier)
	{
	...
	
		{
			...
			// Per-column transient package. Isolates this struct's metadata map from any
			// shared package whose metadata other systems may write to concurrently.
			TStringBuilder<256> PackagePathBuilder;
			PackagePathBuilder.Append(TEXT("/Engine/TedsDynamicColumns/"));
			PackagePathBuilder.Append(Template.GetName());
			PackagePathBuilder.Append(TEXT("_"));
			Identifier.AppendString(PackagePathBuilder);
			UPackage* ColumnPackage = NewObject<UPackage>(nullptr, *FString(PackagePathBuilder), RF_Transient);

			UScriptStruct* NewScriptStruct = NewObject<UScriptStruct>(ColumnPackage, ObjectName);
			// Ensure it is not garbage collected
			// FDynamicColumnGenerator is not a UObject and thus does not participate in GC.
			// The column's package is kept alive via the struct's Outer reference.
			...
			ColumnPackage->GetMetaData().SetValue(NewScriptStruct, TEXT("EditorDataStorage_DerivedFromDynamicTemplate"), IdentifierStringBuilder.ToString());
	...
	FValueTagManager::FValueTagManager(FMassEntityManager& InEntityManager, FDynamicColumnGenerator& InColumnGenerator)
	...
	FConstSharedStruct FValueTagManager::GenerateValueTag(const FValueTag& InTag, const FName& InValue)
	{
		...
		const FConstSharedStruct& SharedStruct = EntityManager.GetOrCreateConstSharedFragment(*ColumnType, reinterpret_cast<const uint8*>(&Overlay));
		return SharedStruct;
	}
