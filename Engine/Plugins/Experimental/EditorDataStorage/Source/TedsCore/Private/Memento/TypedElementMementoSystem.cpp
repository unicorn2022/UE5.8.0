...
{
	FMementoSystem::FMementoSystem(ICoreProvider& InDataStorage, FTableManager& TableManager)
		: DataStorage(InDataStorage)
	{
		...
		MementoRowBaseTable = TableManager.Register({ FTypedElementMementoTag::StaticStruct() }, TEXT("MementoRowBaseTable"));
	...
	{	
		for (const UTedsMementoTranslatorBase* Translator : MementoTranslators)
		{
			if (void* SourceColumn = DataStorage.GetColumnData(SourceRow, Translator->GetColumnType()))
			{
				...
				UE_LOGF(LogEditorDataStorage, VeryVerbose,
					"Column->Memento: %llu -> %llu", SourceRow, MementoRow);
			}
		}
	}
	...
	void FMementoSystem::RestoreMemento(RowHandle MementoRow, RowHandle TargetRow)
	{
		...
		for (const UTedsMementoTranslatorBase* Translator : MementoTranslators)
		{
			if (void* MementoColumn = DataStorage.GetColumnData(MementoRow, Translator->GetMementoType()))
			{
				...
				if (const UScriptStruct* Tag = Translator->GetColumnToAddOnRestore())
				{
					DataStorage.AddColumn(TargetRow, Tag);
				}

				UE_LOGF(LogEditorDataStorage, VeryVerbose, 
					"Memento->Column: %llu -> %llu", MementoRow, TargetRow);
			}
		}
	}
