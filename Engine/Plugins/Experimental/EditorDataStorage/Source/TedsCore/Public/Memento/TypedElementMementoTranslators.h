...
public:
	...
	/**
	 * This column (if specified) will be added to the object row after the memento is restored
	 */
	const UScriptStruct* GetColumnToAddOnRestore() const;
	
protected:
	
	// Optional column that will be added to the object row after the memento is restored
	UPROPERTY()
	TObjectPtr<const UScriptStruct> ColumnToAddOnRestore = nullptr;
};
