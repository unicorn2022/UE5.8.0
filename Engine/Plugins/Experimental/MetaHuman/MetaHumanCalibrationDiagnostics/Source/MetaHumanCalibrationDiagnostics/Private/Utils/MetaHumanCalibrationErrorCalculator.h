...
public:
	...
	TArray<double> GetErrorArrayForBlock(const FString& InCameraName, int32 InBlockIndex, int32 InFrameIndex) const;
	...
	FVector2D GetBlockSize(const FString& InCameraName) const;
	FBox2D GetBlock(const FString& InCameraName, int32 InBlockIndex) const;
	int32 GetBlockNum() const;

	TArray<FBox2D> GetAreaOfInterest() const { return AreaOfInterest; }
	TArray<FIntVector2> GetImageSizes() const { return ImageSizes; }
	TArray<FString> GetCameraNames() const { return CameraNames; }
