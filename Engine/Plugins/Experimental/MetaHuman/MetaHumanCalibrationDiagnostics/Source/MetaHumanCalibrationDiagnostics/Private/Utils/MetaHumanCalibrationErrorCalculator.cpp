	...
	, ImageSizes(InImageSizes)
{
	...
	check(CoverageMapSize != FVector2D::ZeroVector);
...
TArray<double> FMetaHumanCalibrationErrorCalculator::GetErrorArrayForBlock(const FString& InCameraName, int32 InBlockIndex, int32 InFrameIndex) const
{
	check(ErrorsPerCameraPerFramePerBlock.Contains(InCameraName));
	...
	{
		check(ErrorsPerCameraPerFramePerBlock[InCameraName][InFrameIndex].IsValidIndex(InBlockIndex));
		return ErrorsPerCameraPerFramePerBlock[InCameraName][InFrameIndex][InBlockIndex].Errors;
	}
...
FMetaHumanCalibrationErrorCalculator::FErrors FMetaHumanCalibrationErrorCalculator::GetErrorsForBlock(const FString& InCameraName, int32 InBlockIndex, int32 InFrameIndex) const
{
	...
	Errors.Errors = GetErrorArrayForBlock(InCameraName, InBlockIndex, InFrameIndex);
...
FVector2D FMetaHumanCalibrationErrorCalculator::GetBlockSize(const FString& InCameraName) const
{
	int32 Index = CameraNames.IndexOfByKey(InCameraName);
	check(AreaOfInterest.IsValidIndex(Index));

	return AreaOfInterest[Index].GetSize() / CoverageMapSize;
}
