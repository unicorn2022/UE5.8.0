															  ...
															  FSlateWindowElementList& OutDrawElements, int32& OutLayerId, int32 InCameraIndex)
{
	...
	for (int32 PointIndex = 0; PointIndex < CameraPoints.Num(); ++PointIndex)
	{
		...
		static constexpr float MinReprojectionError = FMetaHumanCalibrationErrorAnalysis::MinRMS;
		FLinearColor HeatColor = HandleErrorColor(Error, MinReprojectionError, Options->ReprojectionErrorThreshold).GetSpecifiedColor();
													 ...
													 FSlateWindowElementList& OutDrawElements, int32& OutLayerId, int32 InCameraIndex)
{
	...
	for (int32 BlockIndex : BlockIndices)
	{
		...
		FLinearColor Color = HandleErrorColor(Errors.RMSError, FMetaHumanCalibrationErrorAnalysis::MinRMS, Options->RMSErrorThreshold).GetSpecifiedColor();
		Color.A = 0.2f;
...
FSlateColor SCalibrationDiagnosticsImageViewer::HandleRMSErrorColor() const
{
	...
	return HandleErrorColor(TotalRMSError, FMetaHumanCalibrationErrorAnalysis::MinRMS, Options->RMSErrorThreshold);
}
...
FSlateColor SCalibrationDiagnosticsImageViewer::HandleErrorColor(double InError, double InMinValue, double InThreshold) const
{
	float Normalized = FMath::Clamp(FMath::GetRangePct(InMinValue, InThreshold, InError), 0.0, 1.0);
