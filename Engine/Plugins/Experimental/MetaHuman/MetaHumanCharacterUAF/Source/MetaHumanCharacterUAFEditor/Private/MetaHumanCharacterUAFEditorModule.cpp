...
public:
	...
	static bool ContainsUAFComponent(TNotNull<UBlueprint*> InBlueprint, const TArray<FSubobjectDataHandle>& SubobjectDataHandles)
	{
		for (const FSubobjectDataHandle& Handle : SubobjectDataHandles)
		{
			...
			{
				if (Cast<UUAFComponent>(ActorComponent))
				{
					return true;
				}
			}
	...
	virtual void ModifyBlueprint(TNotNull<UBlueprint*> InBlueprint) override
	{
		...
		for (const FSubobjectDataHandle& Handle : SubobjectDataHandles)
		{
			if (UActorComponent* ActorComponent = const_cast<UActorComponent*>(Handle.GetData()->GetObjectForBlueprint<UActorComponent>(InBlueprint)))
			{
				...
				if (UUAFComponent* UAFComponent = Cast<UUAFComponent>(ActorComponent))
				{
					// Recompile UAF module.
					UE::UAF::FSystemAssetHandleConstView UAFAsset = UAFComponent->GetAssetData();
					if (const FUAFSystemFactoryAsset_System* SystemAsset = UAFAsset.GetPtr<FUAFSystemFactoryAsset_System>())
					{
						if (UUAFSystem* MutableSystem = const_cast<UUAFSystem*>(SystemAsset->System.Get()))
						{
							if (UUAFRigVMAsset* UAFRigVMAsset = CastChecked<UUAFRigVMAsset>(MutableSystem))
							{
								if (UUAFRigVMAssetEditorData* EditorData = UE::UAF::UncookedOnly::FUtils::GetEditorData<UUAFRigVMAssetEditorData>(UAFRigVMAsset))
								{
