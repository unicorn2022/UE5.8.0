	...
	)
{
	if (!TreeStaticMesh)
	{
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "No static mesh given");
		return false;
	}
	if (!TreePivotPosTexture)
	{
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "No pivot pos texture given");
		return false;
	}
	if (!TargetSkeletalMesh)
	{
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "No target skeletal mesh given");
		return false;
	}
	if (!TargetSkeleton)
	{
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "No target skeleton given");
		return false;
	}
	if (TargetSkeletalMesh->GetLODNum() != 0 || TargetSkeletalMesh->GetSkeleton() || TargetSkeletalMesh->GetRefSkeleton().GetNum() != 0)
	{
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "Target skeletal mesh is not empty");
		return false;
	}
	if (TargetSkeleton->GetReferenceSkeleton().GetNum() != 0)
	{
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "Target skeleton is not empty");
		return false;
	}
	...
	if (!TreeStaticMesh->CloneHiResMeshDescription(MeshDescription))
	{
		...
		{
			UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "No valid geometry found on tree mesh");
			return false;
		}
	}
	...
	{
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error, "Invalid Pivot UV layer %d (Max layers: %d)", TreePivotUVIndex, UVCoords.GetNumChannels());
	}
...
bool UDynamicWindBlueprintLibrary::ImportDynamicWindSkeletalDataFromFile(USkeletalMesh* TargetSkeletalMesh)
{
		...
		FileNames))
	{
		...
		UE_LOGF(LogDynamicWindBlueprintLibrary, Error,
			"ImportDynamicWindSkeletalDataFromFile: Failed to load dynamic wind skeletal data from %ls for SkeletalMesh %ls.",
