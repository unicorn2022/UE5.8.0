...
TMap<const FScene*, FDynamicWindTransformProvider*> FDynamicWindTransformProvider::SceneProviderMap;

FDynamicWindTransformProvider* FDynamicWindTransformProvider::FindForScene(const FSceneInterface* InScene)
{
	if (const FScene* RenderScene = InScene ? InScene->GetRenderScene() : nullptr)
	{
		if (auto* Found = SceneProviderMap.Find(RenderScene))
		{
			return *Found;
		}
	}
...
bool FDynamicWindTransformProvider::Register()
{
	...
	if (auto TransformProvider = Scene.GetExtensionPtr<FSkinningTransformProvider>())
	{
		...
		SceneProviderMap.Add(&Scene, this);
...
void FDynamicWindTransformProvider::RegisterProxy(const FSkinningSceneExtensionProxy* ExtensionProxy, const UDynamicWindSkeletalData* InSkeletalData)
{
	if (!bIsRegistered)
	{
		UE_LOGF(LogDynamicWind, Verbose, "Trying to register proxy (SkinnedAsset=%ls), when the provider is not registered.", *GetNameSafe(ExtensionProxy->GetSkinnedAsset()));
		return;
	}

	const USkeletalMesh* SkeletalMesh = Cast<USkeletalMesh>(ExtensionProxy->GetSkinnedAsset());
	...
	const UDynamicWindSkeletalData* Data = InSkeletalData;
	if (Data == nullptr)
	{
	...
	const bool bUsesBoneMap = ExtensionProxy->GetBoneTransformStorageMode() == EBoneTransformStorageMode::BoneMap;
	FSkeletonEntry& SkeletonEntry = SkeletonLookup.FindOrAdd(FSkeletonKey{ Skeleton->GetGuid(), bUsesBoneMap });
	...
	SkeletonEntry.Data.PackedSimGroupAndGroundCover = 0;
	SetMaxSimulationGroupIndex(SkeletonEntry.Data, uint32(FMath::Max(Data->SimulationGroups.Num() - 1, 0)));
	SetIsGroundCover(SkeletonEntry.Data, Data->bIsGroundCover ? 1u : 0u);
...
void FDynamicWindTransformProvider::UnregisterProxy(const FSkinningSceneExtensionProxy* ExtensionProxy)
{
	if (!bIsRegistered)
	{
		UE_LOGF(LogDynamicWind, Verbose, "Trying to unregister proxy (SkinnedAsset=%ls), when the provider is not registered.", *GetNameSafe(ExtensionProxy->GetSkinnedAsset()));
		return;
	}
	...
	const USkeleton* Skeleton = SkeletalMesh->GetSkeleton();
	const bool bUsesBoneMap = ExtensionProxy->GetBoneTransformStorageMode() == EBoneTransformStorageMode::BoneMap;
	auto EntryID = SkeletonLookup.FindId(FSkeletonKey{ Skeleton->GetGuid(), bUsesBoneMap });
...
void FDynamicWindTransformProvider::ProvideTransforms(FSkinningTransformProvider::FProviderContext& Context)
{
	...
	GraphBuilder.AddSetupTask(
		[
			BlockHeaders,
			SkeletonData,
			SkeletonLookup = TMap<FSkeletonKey, FSkeletonEntry, SceneRenderingSetAllocator>(SkeletonLookup), // Copy for task
		...
		]
		{
			...
			for (const FSkinningTransformProvider::FProviderIndirection Indirection : Indirections)
			{
				...
				uint32 DstTransformOffset = Indirection.CurrentTransformOffset;

				const bool bUsesBoneMap = Indirection.BoneMapOffset != INDEX_NONE;
				FSetElementId SkeletonId = SkeletonLookup.FindId(FSkeletonKey{ SkinnedAnimationBatch.SkeletonGuid, bUsesBoneMap });

				const uint32 BoneMapBufferOffset = Indirection.BoneMapOffset;
				...
				for (uint32 DirectionalityIndex = 0; DirectionalityIndex < UniqueAnimationCount; ++DirectionalityIndex)
				{
					...
					{
						FDynamicWindBlockHeader& Header = BlockHeaders[BlockWrite];
						Header.SkeletonDataIndex = SkeletonId.AsInteger(); // NOTE: Can be INDEX_NONE
						Header.BlockDstTransformOffset = DstTransformOffset;
						Header.PackedNumBonesAndOffset = 0;
						SetBlockNumBones(Header, BonesPerGroup);
						SetBlockBoneOffset(Header, BoneOffset);
						Header.PackedDirectionalityAndTransformCount = 0;
						SetBlockDirectionalityIndex(Header, DirectionalityIndex);
						SetTotalTransformCount(Header, TransformCount);
						Header.BoneMapBufferOffset = BoneMapBufferOffset;
					...
					if (PartialTransformCount > 0)
					{
						...
						Header.BlockDstTransformOffset = DstTransformOffset;
						Header.PackedNumBonesAndOffset = 0;
						SetBlockNumBones(Header, PartialTransformCount);
						SetBlockBoneOffset(Header, BoneOffset);
						Header.PackedDirectionalityAndTransformCount = 0;
						SetBlockDirectionalityIndex(Header, DirectionalityIndex);
						SetTotalTransformCount(Header, TransformCount);
