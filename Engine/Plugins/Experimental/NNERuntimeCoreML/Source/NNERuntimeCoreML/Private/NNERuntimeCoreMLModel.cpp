...
namespace Detail
{
	...
	FString GenerateUniqueTempDirPath(const FString& BasePath)
	{
		FString UniqueDirPath;
		do
		{
			UniqueDirPath = FPaths::Combine(BasePath, *FString::Printf(TEXT("CoreMLModel_%s"), *FGuid::NewGuid().ToString()));
		} while (IFileManager::Get().DirectoryExists(*UniqueDirPath));
										...
										const FString& Context)
	{
		...
		{
			UE_LOGF(LogNNERuntimeCoreML, Error, "Invalid feature dictionary for model %ls. Was nullptr expecting a valid dictionary.", *Context);
			return false;
		}
		...
		for(NSString* Key in FeatureDictionary)
		{
			...
			{
				UE_LOGF(LogNNERuntimeCoreML, Error, "Optional feature are not supported but %ls feature %ls is optional.", *Context, *FeatureName);
				TensorDescs.Reset();
				FeatureNames.Reset();
				return false;
			}
			...
			{
				UE_LOGF(LogNNERuntimeCoreML, Error, "Plugin is experimental: Only MultiArray are supported at the moment but %ls feature %ls is not", *Context, *FeatureName);
				TensorDescs.Reset();
				FeatureNames.Reset();
				return false;
			}
			...
			else // undefined MultiArray shape
			{
				...
				{
					UE_LOGF(LogNNERuntimeCoreML, Error, "MultiArray features need to define a shape but %ls feature %ls does not", *Context, *FeatureName);
					TensorDescs.Reset();
					FeatureNames.Reset();
					return false;
				}
...
FModelInstanceCoreMLCpu::ESetInputTensorShapesStatus FModelInstanceCoreMLBase<InstanceType>::SetInputTensorShapes(TConstArrayView<NNE::FTensorShape> InInputShapes)
{
	...
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "Number of input shapes does not match number of input tensors");
	...
	for (int32 i = 0; i < InInputShapes.Num(); ++i)
	{
		...
		{
			UE_LOGF(LogNNERuntimeCoreML, Error, "Input shape does not match input tensor %ls of index %d", *SymbolicDesc.GetName(), i);
			return FModelInstanceCoreMLCpu::ESetInputTensorShapesStatus::Fail;
		}
	}
...
FModelInstanceCoreMLCpu::ERunSyncStatus FModelInstanceCoreMLBase<InstanceType>::RunSync(TConstArrayView<NNE::FTensorBindingCPU> InInputBindings, TConstArrayView<NNE::FTensorBindingCPU> InOutputBindings)
{
	...
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "Input shapes are not set, please call SetInputTensorShapes.");
	...
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "Input bindings need to match input tensor descriptor count (got %d, expected %d).", InInputBindings.Num(), InputTensorShapes.Num());
		return FModelInstanceCoreMLCpu::ERunSyncStatus::Fail;
	}

	if (!InOutputBindings.IsEmpty() && InOutputBindings.Num() != OutputFeatureNames.Num())
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "Output binding can be empty or needs to match output feature count (got %d, expected %d).", InOutputBindings.Num(), OutputFeatureNames.Num());
		return FModelInstanceCoreMLCpu::ERunSyncStatus::Fail;
	}
	...
	for(int i = 0; i < InputFeatureNames.Num(); ++i)
	{
		...
		if (ErrorCreatingInput != nullptr)
		{
			...
			UE_LOGF(LogNNERuntimeCoreML, Error, "Failed to create the input (%ls)", *Error);
			return FModelInstanceCoreMLCpu::ERunSyncStatus::Fail;
		}
	...
	if (ErrorRunningInference != nullptr)
	{
		...
		UE_LOGF(LogNNERuntimeCoreML, Error, "Failed to run inference (%ls)", *Error);
		return FModelInstanceCoreMLCpu::ERunSyncStatus::Fail;
	}
	if (!OutFeatureProvider) 
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "No output feature provider was returned");
		return FModelInstanceCoreMLCpu::ERunSyncStatus::Fail;
	}
	...
	for(int i = 0; i < OutputFeatureNames.Num(); ++i)
	{
		...
		{
			const int32 ElementSize = NNE::GetTensorDataTypeSizeInBytes(Detail::GetTypeFromMultiArrayDataType(OutputMultiArray.dataType));
			
			// OutputMultiArray.count is the product of shape dimensions (logical element count, excludes stride padding)
			const uint64 LogicalSizeInBytes = static_cast<uint64>(OutputMultiArray.count) * ElementSize;

			if (InOutputBindings[i].SizeInBytes >= LogicalSizeInBytes)
			{
				[OutputMultiArray getBytesWithHandler:^(const void* bytes, NSInteger size) {
					FMemory::Memcpy(InOutputBindings[i].Data, bytes, LogicalSizeInBytes);
				}];
			}
		}
	}
...
{
	SCOPED_NAMED_EVENT_TEXT("FModelInstanceCoreMLBase::Init", FColor::Magenta);
	...
	if (!CoreMLModelData.Load(ModelData))
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "Failed to read model data.");
		return false;
	}

	// Save file data to disk
	FString ProjIntermediateDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectIntermediateDir());
	FString TempDirForModel = Detail::GenerateUniqueTempDirPath(ProjIntermediateDir);
	FString Filepath;
	if (CoreMLModelData.ModelType == EModelType::MLModel)
	{
		Filepath = FPaths::Combine(TempDirForModel, TEXT("CoreMLModel.mlmodel"));
		if (!FFileHelper::SaveArrayToFile(CoreMLModelData.FileDataView, *Filepath))
		{
			IFileManager::Get().DeleteDirectory(*TempDirForModel, false, true);
			UE_LOGF(LogNNERuntimeCoreML, Error, "Could not write model to disk at path %ls.", *Filepath);
			return false;
		}
	}
	else if (CoreMLModelData.ModelType == EModelType::MLPackage)
	{
		Filepath = FPaths::Combine(TempDirForModel, TEXT("CoreMLModel.mlpackage"));
		if (FPaths::DirectoryExists(Filepath))
		{
			IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
			if (!PlatformFile.DeleteDirectoryRecursively(*Filepath))
			{
				UE_LOGF(LogNNERuntimeCoreML, Error, "Failed to delete directory %ls", *Filepath);
				return false;
			}
		}

		if (!SaveArrayToDirectory(CoreMLModelData.FileDataView, *Filepath))
		{
			IFileManager::Get().DeleteDirectory(*TempDirForModel, false, true);
			UE_LOGF(LogNNERuntimeCoreML, Error, "Could not write model to disk at path %ls.", *Filepath);
			return false;
	...
	{
		checkf(false, TEXT("Unexpected ModelType %i."), static_cast<int32>(CoreMLModelData.ModelType));
		return false;
	}
	...
	if (ErrorCompilationObj != nullptr)
	{
		...
		UE_LOGF(LogNNERuntimeCoreML, Error, "Failed to compile model (%ls)", *Error);
		return false;
	}
	if (!IFileManager::Get().DeleteDirectory(*TempDirForModel, false, true))
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "Could not cleanup model temp directory %ls.", *TempDirForModel);
	}
	...
	if (ErrorCreationObj != nullptr)
	{
		...
		UE_LOGF(LogNNERuntimeCoreML, Error, "Failed to make an instance from compiled model (%ls)", *Error);
		return false;
	}
...
TSharedPtr<NNE::IModelInstanceCPU> FModelCoreMLCpu::CreateModelInstanceCPU()
{
	...
	{
		UE_LOGF(LogNNERuntimeCoreML, Error, "Cannot initialize model instance");
