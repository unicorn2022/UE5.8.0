	...
	bool Pop(const TCHAR* FuncName, const TCHAR* TagName, bool bForce = true)
	{
		...
		if (!Utils::IsElementA(XmlElement, TagName))
		{
			UE_LOGF(LogXmlSerialization, Error,
				"FXmlArchiveOutputFormatter::%ls: Requested to leave \"%ls\", but it is \"%ls\".",
...
FXmlArchiveOutputFormatter::~FXmlArchiveOutputFormatter()
{
	if (!Impl->bDocumentSaveAttempted)
	{
		...
		{
			UE_LOGF(LogXmlSerialization, Error,
				"~FXmlArchiveOutputFormatter: Failed to save XML document to inner archive. Archive might be already destroyed, use SaveDocumentToInnerArchive() to ensure proper order.");
		}
	}
	delete Impl;
}
...
bool FXmlArchiveOutputFormatter::SaveDocumentToInnerArchive(EXmlSerializationEncoding InEncoding)
{
	...
	if (Impl->bDocumentSaveAttempted)
	{
		UE_LOGF(LogXmlSerialization, Warning,
			"FXmlArchiveOutputFormatter: Document has already been saved to archive. Prepare for unforeseen consequences.");
	}
