...
void UPixelStreamingSignallingComponent::Connect(const FString& Url)
{
	...
	if(Components.Num() < 2)
	{
		UE_LOGF(LogPixelStreamingPlayer, Error, "Incorrectly formated connection URL. Ensure URL is in the format (protocol)://(ip):(port)");
		return;
	}
...
void UPixelStreamingSignallingComponent::SendOffer(const FPixelStreamingSessionDescriptionWrapper& Offer)
{
	...
	{
		UE_LOGF(LogPixelStreamingPlayer, Error, "Send Offer failed: Offer was null");
	}
	else
	{
...
void UPixelStreamingSignallingComponent::SendAnswer(const FPixelStreamingSessionDescriptionWrapper& Answer)
{
	...
	{
		UE_LOGF(LogPixelStreamingPlayer, Error, "Send Answer failed: Answer was null");
	}
	else
	{
...
void UPixelStreamingSignallingComponent::FSignallingObserver::OnSignallingPlayerConnected(FPixelStreamingPlayerId PlayerId, const FPixelStreamingPlayerConfig& PlayerConfig, bool bSendOffer)
{
	...
	UE_LOGF(LogPixelStreamingPlayer, Warning, "Warning: Received message about player connected from SS, this very likely means you have misconfigured the Pixel Streaming player connection and entered the streamer port. Please check your connect node and ensure it using the player port, e.g. 80 (or no port).");
}
...
void UPixelStreamingSignallingComponent::FSignallingObserver::OnSignallingPlayerDisconnected(FPixelStreamingPlayerId PlayerId)
{
	...
	UE_LOGF(LogPixelStreamingPlayer, Warning, "Warning: Received message about player disconnected from SS, this very likely means you have misconfigured the Pixel Streaming player connection and entered the streamer port. Please check your connect node and ensure it using the player port, e.g. 80 (or no port).");
}
