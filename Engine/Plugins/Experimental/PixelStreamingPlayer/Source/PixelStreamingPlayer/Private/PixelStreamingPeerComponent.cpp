...
void UPixelStreamingPeerComponent::SetConfig(const FPixelStreamingRTCConfigWrapper& Config)
{
	...
	{
		UE_LOGF(LogPixelStreamingPlayer, Error, "SetConfig Failed.");
	}
}
...
FPixelStreamingSessionDescriptionWrapper UPixelStreamingPeerComponent::CreateOffer()
{
	if (PeerConnection)
	{
		...
		const auto								 OnGeneralFailure = [&TaskEvent](const FString& ErrorMsg) {
			  UE_LOGF(LogPixelStreamingPlayer, Error, "CreateOffer Failed: %ls", *ErrorMsg);
			  TaskEvent->Trigger();
		};
		...
		{
			UE_LOGF(LogPixelStreamingPlayer, Error, "CreateOffer Failed: Timeout");
		}
	...
	{
		UE_LOGF(LogPixelStreamingPlayer, Error, "Failed to create offer. Call SetConfig first.");
		return {};
	}
}
...
FPixelStreamingSessionDescriptionWrapper UPixelStreamingPeerComponent::CreateAnswer(const FString& Sdp)
{
	if (PeerConnection)
	{
		...
		FEvent*									 TaskEvent = FPlatformProcess::GetSynchEventFromPool();
		const auto								 OnGeneralFailure = [&TaskEvent](const FString& ErrorMsg) {
			  UE_LOGF(LogPixelStreamingPlayer, Error, "CreateAnswer Failed: %ls", *ErrorMsg);
			  TaskEvent->Trigger();
		};
		...
		if (Wrapper.SDP == nullptr)
		{
			UE_LOGF(LogPixelStreamingPlayer, Error, "CreateAnswer Failed: Timeout");
		}
	...
	{
		UE_LOGF(LogPixelStreamingPlayer, Error, "Failed to create answer. Call SetConfig first.");
		return {};
	}
}
...
void UPixelStreamingPeerComponent::ReceiveAnswer(const FString& Answer)
{
	if (PeerConnection)
	{
		FEvent*	   TaskEvent = FPlatformProcess::GetSynchEventFromPool();
		const auto OnGeneralFailure = [&TaskEvent](const FString& ErrorMsg) {
			UE_LOGF(LogPixelStreamingPlayer, Error, "ReceiveAnswer Failed: %ls", *ErrorMsg);
			TaskEvent->Trigger();
		};
	...
	{
		UE_LOGF(LogPixelStreamingPlayer, Error, "Failed to receive answer. Call SetConfig first.");
	}
}
