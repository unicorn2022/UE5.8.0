...
void UAutomationUtilsBlueprintLibrary::AddAutomationMountPoint(const FString& RootPath)
{
	FPackageName::RegisterMountPoint(RootPath, FPaths::AutomationTransientDir());
}

void UAutomationUtilsBlueprintLibrary::RemoveAutomationMountPoint(const FString& RootPath)
{
	FPackageName::UnRegisterMountPoint(RootPath, FPaths::AutomationTransientDir());
}

void UAutomationUtilsBlueprintLibrary::FinishAllAssetCompilation()
{
	// FinishAllCompilation drains the compile workers; FlushRenderingCommands then
	// drains the completion render commands those workers posted, so callers can
	// safely destroy compiled assets without racing against ~FMaterial.
	FAssetCompilingManager::Get().FinishAllCompilation();
	FlushRenderingCommands();
}
