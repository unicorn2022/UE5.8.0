...
void UXRCreativeVREditorMode::EnableStereo()
{
	...
	if (Contexts.IsEmpty())
	{
		UE_LOGF(LogXRCreativeEditor, Warning, "No UInputMappingContexts provided in the UXRCreativeToolset. Action bindings will not be visible to the OpenXR runtime.");
	}
...
void UXRCreativeVREditorMode::InvalidSettingNotification(const FText& ErrorDetails)
{
	...
	UE_LOGF(LogXRCreativeEditor, Error, "%ls: %ls", *NotificationHeading.ToString(), *ErrorDetails.ToString());
