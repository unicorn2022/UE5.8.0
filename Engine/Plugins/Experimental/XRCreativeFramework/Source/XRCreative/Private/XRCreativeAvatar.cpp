...
void AXRCreativeAvatar::OnConstruction(const FTransform& InTransform)
{
	...
	UE_LOGF(LogXRCreative, Log, "Handedness: %ls", *UEnum::GetValueAsString(Settings->Handedness) );
...
void AXRCreativeAvatar::PlayHapticEffect(UHapticFeedbackEffect_Base* HapticEffect, const int ControllerID, const EControllerHand Hand, float Scale, bool bLoop)
{
	if (HapticEffect)
	{
		...
		case EControllerHand::Right:
			...
			break;
		default:
			UE_LOGF(LogXRCreative, Warning, "invalid hand specified (%d) for feedback effect %ls. Only left and right hand controllers supported", (int32)Hand, *HapticEffect->GetName());
		}
	}
}
