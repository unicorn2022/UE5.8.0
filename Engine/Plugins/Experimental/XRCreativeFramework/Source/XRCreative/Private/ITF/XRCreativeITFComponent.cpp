...
public:
	...
	virtual void DisplayMessage(const FText& Message, EToolMessageLevel Level) override
	{
		...
		switch (Level)
		{
			...
			default:
				ensureMsgf(false, TEXT("Unhandled EToolMessageLevel: %X"), EnumToUnderlyingType(Level));
...
void UXRCreativeITFComponent::ToolsTick(float InDeltaTime)
{
	...
	const FMatrix InvViewMatrix = SceneView->ViewMatrices.GetViewToWorld();
	const FMatrix InvProjMatrix = SceneView->ViewMatrices.GetClipToView();
...
void UXRCreativeITFComponent::RecreateActorPhysicsState(AActor* Actor)
{
#if WITH_EDITOR
	if (!Actor)
	{
