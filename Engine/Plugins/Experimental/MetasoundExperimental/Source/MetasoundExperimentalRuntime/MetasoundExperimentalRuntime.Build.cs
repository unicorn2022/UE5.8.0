...
public class MetasoundExperimentalRuntime : ModuleRules
{
	public MetasoundExperimentalRuntime(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		// These should probably move back to the experimental plugin again.
		RuntimeDependencies.Add("$(PluginDir)/Content/Schemas/ChannelAgnostic/Composite/*.json", StagedFileType.UFS);
		RuntimeDependencies.Add("$(PluginDir)/Content/Schemas/ChannelAgnostic/Discrete/*.json", StagedFileType.UFS);
		RuntimeDependencies.Add("$(PluginDir)/Content/Schemas/ChannelAgnostic/Soundfield/*.json", StagedFileType.UFS);
	}
}
