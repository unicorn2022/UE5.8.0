...
void UMetaSoundEditorViewModel::OnGraphInputDefaultChanged(const FGraphInputDefaultChangedArgs& Args)
{
	if (UMetaSoundInputViewModel* InputViewModel = InputViewModels.FindRef(Args.InputName))
	{
		InputViewModel->OnInputDefaultChanged(Args.Literal, Args.PageName, Args.ChangeType);
	}
}

void UMetaSoundEditorViewModel::OnInputDisplayNameChanged(const FName InputName, const FText InDisplayName)
{
	if (UMetaSoundInputEditorViewModel* InputEditorViewModel = Cast<UMetaSoundInputEditorViewModel>(InputViewModels.FindRef(InputName)))
	{
...
{
	if (UMetaSoundOutputEditorViewModel* OutputEditorViewModel = Cast<UMetaSoundOutputEditorViewModel>(OutputViewModels.FindRef(OutputName)))
	{
...
void UMetaSoundEditorViewModel::BindDelegates()
{
	...
	if (BuilderListener)
	{
		...
		BuilderListener->OnGraphInputDefaultChangedWithArgsDelegate.AddDynamic(this, &ThisClass::OnGraphInputDefaultChanged);
