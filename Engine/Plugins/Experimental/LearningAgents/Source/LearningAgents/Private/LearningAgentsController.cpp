...
{
	UE_LOGF(LogLearning, Error, "%ls: EvaluateAgentController function must be overridden!", *GetName());
	...
	const TArray<int32>& AgentIds)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Not enough Observation Objects. Expected %i, Got %i.", *GetName(), AgentNum, InObservationObjectElements.Num());
		return;
	}
	...
	const FName Name)
{
	if (!InManager)
	{
		UE_LOGF(LogLearning, Error, "MakeController: InManager is nullptr.");
		return nullptr;
	}

	if (!Class)
	{
		UE_LOGF(LogLearning, Error, "MakeController: Class is nullptr.");
		return nullptr;
	}
...
void ULearningAgentsController::EvaluateController()
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Not enough Observation Objects added by GatherAgentObservations. Expected %i, Got %i.", *GetName(), Manager->GetAgentNum(), Interactor->GetObservationObjectElements().Num());
		return;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Not enough Action Objects added by EvaluateAgentControllers. Expected %i, Got %i.", *GetName(), Manager->GetAgentNum(), Interactor->GetActionObjectElements().Num());
		return;
	}
