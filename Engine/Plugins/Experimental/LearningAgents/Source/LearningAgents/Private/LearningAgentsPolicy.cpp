	...
	const int32 Seed)
{
	if (!InManager)
	{
		UE_LOGF(LogLearning, Error, "MakePolicy: InManager is nullptr.");
		return nullptr;
	}

	if (!Class)
	{
		UE_LOGF(LogLearning, Error, "MakePolicy: Class is nullptr.");
		return nullptr;
	}
	...
	const int32 Seed)
{
	if (IsSetup())
	{
		UE_LOGF(LogLearning, Error, "%ls: Setup already run!", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogLearning, Warning, "%ls: Identical Network Assets given as both Policy and Encoder: %ls.", *GetName(), *PolicyNeuralNetworkAsset->GetName());
	}


	if (PolicyNeuralNetworkAsset && PolicyNeuralNetworkAsset == DecoderNeuralNetworkAsset)
	{
		UE_LOGF(LogLearning, Warning, "%ls: Identical Network Assets given as both Policy and Decoder: %ls.", *GetName(), *PolicyNeuralNetworkAsset->GetName());
	}

	if (EncoderNeuralNetworkAsset && EncoderNeuralNetworkAsset == DecoderNeuralNetworkAsset)
	{
		UE_LOGF(LogLearning, Warning, "%ls: Identical Network Assets given as both Encoder and Decoder: %ls.", *GetName(), *EncoderNeuralNetworkAsset->GetName());
	}
	...
	if (EncoderNeuralNetworkAsset)
	{
		...
		if (EncoderNeuralNetworkAsset->NeuralNetworkData && !bReinitializeEncoderNetwork)
		{
			...
			{
				UE_LOGF(LogLearning, Error, "%ls: Encoder Network Asset provided during Setup is incompatible with Schema. Network hash is %i vs Schema hash %i.", *GetName(),
			...
			{
				UE_LOGF(LogLearning, Error, "%ls: Encoder Network Asset provided during Setup is incorrect size: Got inputs of size %i, expected %i. Got outputs of size %i, expected %i.", *GetName(),
	...
	if (PolicyNeuralNetworkAsset)
	{
		...
		if (PolicyNeuralNetworkAsset->NeuralNetworkData && !bReinitializePolicyNetwork)
		{
			...
			{
				UE_LOGF(LogLearning, Error, "%ls: Policy Network Asset provided during Setup is incompatible with Schema. Network hash is %i vs Schema hash %i.", *GetName(),
			...
			{
				UE_LOGF(LogLearning, Error, "%ls: Policy Network Asset provided during Setup is incorrect size: Got inputs of size %i, expected %i. Got outputs of size %i, expected %i.", *GetName(),
			...
			Builder.WriteFileDataAndReset(FileData, PolicyInputSize, PolicyOutputSize,
				Builder.MakeSequence({
					Builder.MakeMLP(
						...
						UE::Learning::Agents::NeuralNetwork::GetBuilderActivationFunction(PolicyActivationFunction)),
	...
	if (DecoderNeuralNetworkAsset)
	{
		...
		if (DecoderNeuralNetworkAsset->NeuralNetworkData && !bReinitializeDecoderNetwork)
		{
			...
			{
				UE_LOGF(LogLearning, Error, "%ls: Decoder Network Asset provided during Setup is incompatible with Schema. Network hash is %i vs Schema hash %i.", *GetName(),
			...
			{
				UE_LOGF(LogLearning, Error, "%ls: Decoder Network Asset provided during Setup is incorrect size: Got inputs of size %i, expected %i. Got outputs of size %i, expected %i.", *GetName(),
...
void ULearningAgentsPolicy::EncodeObservations()
{
	...
	{
		UE_LOGF(LogLearning, Warning, "%ls: No agents added to Manager.", *GetName());
	}
	...
	for (const int32 AgentId : Manager->GetAllAgentSet())
	{
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i does not have an observation vector ready be encoded. Was GatherObservations run without error?", *GetName(), AgentId);
			continue;
		}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Encoder Network Input size doesn't match. Network input size is %i but Encoder expects %i.", *GetName(),
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Encoder Network Output size don't match. Network output size is %i but Encoder expects %lli.", *GetName(),
...
void ULearningAgentsPolicy::EvaluatePolicy()
{
	...
	for (const int32 AgentId : Manager->GetAllAgentSet())
	{
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i has not encoded observations so policy will not be evaluated for it. Was EncodeObservations run without error?", *GetName(), AgentId);
			continue;
		}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Policy Network Input size doesn't match. Network input size is %i but Policy expects %lli.", *GetName(),
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Policy Network Output size don't match. Network output size is %i but Policy expects %lli.", *GetName(),
...
void ULearningAgentsPolicy::DecodeAndSampleActions(const float ActionNoiseScale)
{
	...
	for (const int32 AgentId : Manager->GetAllAgentSet())
	{
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i does not have an encoded action so actions will not be decoded for it. Was EvaluatePolicy run without error?", *GetName(), AgentId);
			continue;
		}

		if (Interactor->GetActionModifierIteration(AgentId) == 0)
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i does not have an action modifier made so actions will not be decoded for it. Was MakeActionModifiers run without error?", *GetName(), AgentId);
			continue;
		}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Decoder Network Input size doesn't match. Network input size is %i but Decoder expects %lli.", *GetName(),
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Decoder Network Output size don't match. Network output size is %i but Decoder expects %lli.", *GetName(),
...
void ULearningAgentsPolicy::GetMemoryState(TArray<float>& OutMemoryState, const int32 AgentId) const
{
	...
	if (!HasAgent(AgentId))
	{
		UE_LOGF(LogLearning, Error, "%ls: AgentId %d not found in the agents set.", *GetName(), AgentId);
		OutMemoryState.Empty();
		return;
	}
...
void ULearningAgentsPolicy::SetMemoryState(const int32 AgentId, const TArray<float>& InMemoryState)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Memory State is incorrect size. Expected %lli, got %i.", *GetName(), MemoryState.Num<1>(), InMemoryState.Num());
		return;
	}
