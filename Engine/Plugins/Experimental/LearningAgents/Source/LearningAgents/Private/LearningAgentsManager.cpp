...
void ULearningAgentsManager::SetMaxAgentNum(const int32 InMaxAgentNum)
{
	if (InMaxAgentNum <= 0)
	{
		UE_LOGF(LogLearning, Error, "%ls: Attempted to SetMaxAgentNum but the input is less than or equal to 0.", *GetName());
		return;
	}
	
	if (GetAgentNum() > 0)
	{
		UE_LOGF(LogLearning, Error, "%ls: Cannot SetMaxAgentNum because agents have already been added.", *GetName());
		return;
	}
...
int32 ULearningAgentsManager::AddAgent(UObject* Agent)
{
	if (Agent == nullptr)
	{
		UE_LOGF(LogLearning, Error, "%ls: Attempted to add an agent but agent is nullptr.", *GetName());
		return INDEX_NONE;
	}

	if (VacantAgentIds.IsEmpty())
	{
		UE_LOGF(LogLearning, Error, "%ls: Attempting to add an agent but we have no more vacant ids. Increase MaxAgentNum (%d) or remove unused agents.", *GetName(), MaxAgentNum);
		return INDEX_NONE;
	}

	if (Agents.Contains(Agent))
	{
		UE_LOGF(LogLearning, Error, "%ls: Agent %ls already added to this manager.", *GetName(), *Agent->GetName());
		return INDEX_NONE;
	}
	...
	UE_LOGF(LogLearning, Display, "%ls: Adding Agent %ls with id %i.", *GetName(), *Agent->GetName(), AgentId);
...
void ULearningAgentsManager::AddAgents(TArray<int32>& OutAgentIds, const TArray<UObject*>& InAgents)
{
	...
	for (int32 AgentIdx = 0; AgentIdx < InAgents.Num(); AgentIdx++)
	{
		...
		if (Agents.Contains(InAgents[AgentIdx]))
		{
			UE_LOGF(LogLearning, Error, "%ls: Agent %ls already added to this manager.", *GetName(), *InAgents[AgentIdx]->GetName());
			continue;
		}
	...
	UE_LOGF(LogLearning, Display, "%ls: Adding Agents %ls.", *GetName(), *UE::Learning::Array::FormatInt32(OnEventAgentIds));
...
void ULearningAgentsManager::RemoveAgent(const int32 AgentId)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Attempting to remove an agent with id of -1.", *GetName());
		return;
	}
	...
	if (RemovedCount == 0)
	{
		UE_LOGF(LogLearning, Error, "%ls: Trying to remove an agent with id of %i but it was not found.", *GetName(), AgentId);
		return;
	}
	...
	UE_LOGF(LogLearning, Display, "%ls: Removing Agent %i.", *GetName(), AgentId);
...
void ULearningAgentsManager::RemoveAgents(const TArray<int32>& AgentIds)
{
	...
	for (int32 AgentIdx = 0; AgentIdx < AgentIds.Num(); AgentIdx++)
	{
		...
		if (RemovedCount == 0)
		{
			UE_LOGF(LogLearning, Error, "%ls: Trying to remove an agent with id of %i but it was not found.", *GetName(), AgentIds[AgentIdx]);
			continue;
		}
	...
	UE_LOGF(LogLearning, Display, "%ls: Removing Agents %ls.", *GetName(), *UE::Learning::Array::FormatInt32(OnEventAgentIds));
...
void ULearningAgentsManager::ResetAgent(const int32 AgentId)
{
	if (AgentId == INDEX_NONE)
	{
		UE_LOGF(LogLearning, Error, "%ls: Attempting to reset an agent with id of -1.", *GetName());
		return;
	}

	if (!HasAgent(AgentId))
	{
		UE_LOGF(LogLearning, Error, "%ls: Trying to reset an agent with id of %i but it was not found.", *GetName(), AgentId);
		return;
	}
	...
	UE_LOGF(LogLearning, Display, "%ls: Resetting Agent %i.", *GetName(), AgentId);
...
void ULearningAgentsManager::ResetAgents(const TArray<int32>& AgentIds)
{
	...
	for (int32 AgentIdx = 0; AgentIdx < AgentIds.Num(); AgentIdx++)
	{
		...
		{
			UE_LOGF(LogLearning, Error, "%ls: Trying to reset an agent with id of %i but it was not found.", *GetName(), AgentIds[AgentIdx]);
			continue;
		}
	...
	UE_LOGF(LogLearning, Display, "%ls: Resetting Agents %ls.", *GetName(), *UE::Learning::Array::FormatInt32(OnEventAgentIds));
...
UObject* ULearningAgentsManager::GetAgent(const int32 AgentId, const TSubclassOf<UObject> AgentClass) const
{
	if (!HasAgent(AgentId))
	{
		UE_LOGF(LogLearning, Error, "%ls: AgentId %d not found. Be sure to only use AgentIds returned by AddAgent and check that the agent has not been removed.", *GetName(), AgentId);
		return nullptr;
	}
	
	if (Agents[AgentId] == nullptr)
	{
		UE_LOGF(LogLearning, Warning, "%ls: AgentId %d object is nullptr.", *GetName(), AgentId);
		return nullptr;
	}
	
	if (!Agents[AgentId].IsA(AgentClass))
	{
		UE_LOGF(LogLearning, Error, "%ls: AgentId %d cast invalid. Agent was of class '%ls', not '%ls'.", *GetName(), AgentId, *Agents[AgentId]->GetClass()->GetName(), *AgentClass->GetName());
		return nullptr;
	}
...
void ULearningAgentsManager::GetAgents(TArray<UObject*>& OutAgents, const TArray<int32>& AgentIds, const TSubclassOf<UObject> AgentClass) const
{
	...
	for (int32 AgentIdIdx = 0; AgentIdIdx < AgentIds.Num(); AgentIdIdx++)
	{
		...
		{
			UE_LOGF(LogLearning, Error, "%ls: AgentId %d not found. Be sure to only use AgentIds returned by AddAgent and check that the agent has not been removed.", *GetName(), AgentIds[AgentIdIdx]);
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: AgentId %d object is nullptr.", *GetName(), AgentIds[AgentIdIdx]);
			OutAgents[AgentIdIdx] = nullptr;
			continue;
		}
		...
		{
			UE_LOGF(LogLearning, Error, "%ls: AgentId %d cast invalid. Agent was of class '%ls', not '%ls'.", *GetName(), AgentIds[AgentIdIdx], *Agents[AgentIds[AgentIdIdx]]->GetClass()->GetName(), *AgentClass->GetName());
			OutAgents[AgentIdIdx] = nullptr;
			continue;
		}
...
int32 ULearningAgentsManager::GetAgentId(UObject* Agent) const
{
	...
	if (AgentId == INDEX_NONE)
	{
		UE_LOGF(LogLearning, Error, "%ls: Cannot get id for Agent %ls, as it was not found in added agent set.", *GetName(), *Agent->GetName());
		return INDEX_NONE;
	}
	else
	{
...
void ULearningAgentsManager::GetAgentIds(TArray<int32>& OutAgentIds, const TArray<UObject*>& InAgents) const
{
	...
	for (int32 AgentIdx = 0; AgentIdx < InAgents.Num(); AgentIdx++)
	{
		...
		if (AgentId == INDEX_NONE)
		{
			UE_LOGF(LogLearning, Error, "%ls: Cannot get id for Agent %ls, as it was not found in added agent set.", *GetName(), *InAgents[AgentIdx]->GetName());
			OutAgentIds[AgentIdx] = INDEX_NONE;
		}
		else
		{
