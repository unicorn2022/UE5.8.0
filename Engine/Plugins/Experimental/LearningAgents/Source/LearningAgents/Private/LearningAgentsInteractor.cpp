	...
	const FName Name)
{
	if (!InManager)
	{
		UE_LOGF(LogLearning, Error, "MakeInteractor: InManager is nullptr.");
		return nullptr;
	}

	if (!Class)
	{
		UE_LOGF(LogLearning, Error, "MakeInteractor: Class is nullptr.");
		return nullptr;
	}
...
void ULearningAgentsInteractor::SetupInteractor(ULearningAgentsManager*& InManager)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Invalid observation provided to Interactor during SpecifyObservations.", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Observation vector is zero-sized - specified observations have no size.", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Observation encoded vector is zero-sized - observations map to empty encoding.", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Invalid action provided to Interactor during SpecifyActions.", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Action vector is zero-sized - specified actions have no size.", *GetName());
		return;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Action encoded vector is zero-sized - actions map to empty encoding.", *GetName());
		return;
	}
...
{
	UE_LOGF(LogLearning, Error, "%ls: SpecifyAgentObservation function must be overridden!", *GetName());
...
{
	UE_LOGF(LogLearning, Error, "%ls: GatherAgentObservation function must be overridden!", *GetName());
...
{
	UE_LOGF(LogLearning, Error, "%ls: SpecifyAgentAction function must be overridden!", *GetName());
...
{
	UE_LOGF(LogLearning, Error, "%ls: PerformAgentAction function must be overridden!", *GetName());
}
...
void ULearningAgentsInteractor::PerformAgentActions_Implementation(const ULearningAgentsActionObject* InActionObject, const TArray<FLearningAgentsActionObjectElement>& InActionObjectElements, const TArray<int32>& AgentIds)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: PerformAgentActions: Not enough Action Objects. Expected %i, Got %i.", *GetName(), AgentNum, InActionObjectElements.Num());
		return;
	}
	...
	const TArray<int32>& AgentIds)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: MakeAgentActionModifiers: Not enough Observation Objects. Expected %i, Got %i.", *GetName(), AgentNum, InObservationObjectElements.Num());
		return;
	}
...
void ULearningAgentsInteractor::GatherObservations(const UE::Learning::FIndexSet AgentSet, bool bIncrementIteration)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Not enough Observation Objects added by GatherAgentObservations. Expected %i, Got %i.", *GetName(), ValidAgentIds.Num(), ObservationObjectElements.Num());
		return;
	}
...
void ULearningAgentsInteractor::MakeActionModifiers(const UE::Learning::FIndexSet AgentSet, bool bIncrementIteration)
{
	...
	for (const int32 AgentId : AgentSet)
	{
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i does not have an observation vector so action modifiers will not be created for it. Was GatherObservations run without error?", *GetName(), AgentId);
			continue;
		}
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Not enough Action Modifiers added by MakeAgentActionModifiers. Expected %i, Got %i.", *GetName(), ValidAgentIds.Num(), ActionModifierElements.Num());
		return;
	}
...
void ULearningAgentsInteractor::PerformActions(const UE::Learning::FIndexSet AgentSet)
{
	...
	for (const int32 AgentId : AgentSet)
	{
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i does not have an action vector so actions will not be scattered for it. Was DecodeAndSampleActions run without error?", *GetName(), AgentId);
			continue;
		}
...
void ULearningAgentsInteractor::GetObservationVector(TArray<float>& OutObservationVector, int32& OutObservationCompatibilityHash, const int32 AgentId)
{
	...
	if (ObservationVectorIteration[AgentId] == 0)
	{
		UE_LOGF(LogLearning, Error, "%ls: Observation vector not set for agent %i.", *GetName(), AgentId);
		OutObservationVector.Empty();
		OutObservationCompatibilityHash = 0;
		return;
	}
...
void ULearningAgentsInteractor::GetActionModifierVector(TArray<float>& OutActionModifierVector, int32& OutActionCompatibilityHash, const int32 AgentId)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Action Modifier vector not set for agent %i.", *GetName(), AgentId);
		OutActionModifierVector.Empty();
		OutActionCompatibilityHash = 0;
		return;
	}
...
void ULearningAgentsInteractor::GetActionVector(TArray<float>& OutActionVector, int32& OutActionCompatibilityHash, const int32 AgentId)
{
	...
	if (ActionVectorIteration[AgentId] == 0)
	{
		UE_LOGF(LogLearning, Error, "%ls: Action vector not set for agent %i.", *GetName(), AgentId);
		OutActionVector.Empty();
		OutActionCompatibilityHash = 0;
		return;
	}
...
void ULearningAgentsInteractor::SetObservationVector(const TArray<float>& ObservationVector, const int32 InObservationCompatibilityHash, const int32 AgentId, bool bIncrementIteration)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Observation Compatibility hash incompatible. Got %i, expected %i.", *GetName(), InObservationCompatibilityHash, ObservationCompatibilityHash);
		return;
	}

	if (ObservationVector.Num() != GetObservationVectorSize())
	{
		UE_LOGF(LogLearning, Error, "%ls: Observation Vector size incompatible. Got %i, expected %i.", *GetName(), ObservationVector.Num(), GetObservationVectorSize());
		return;
	}
...
void ULearningAgentsInteractor::SetActionModifierVector(const TArray<float>& ActionModifierVector, const int32 InActionCompatibilityHash, const int32 AgentId, bool bIncrementIteration)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Action Compatibility hash incompatible. Got %i, expected %i.", *GetName(), InActionCompatibilityHash, ActionCompatibilityHash);
		return;
	}

	if (ActionModifierVector.Num() != GetActionModifierVectorSize())
	{
		UE_LOGF(LogLearning, Error, "%ls: Action Modifier Vector size incompatible. Got %i, expected %i.", *GetName(), ActionModifierVector.Num(), GetActionModifierVectorSize());
		return;
	}
...
void ULearningAgentsInteractor::SetActionVector(const TArray<float>& ActionVector, const int32 InActionCompatibilityHash, const int32 AgentId, bool bIncrementIteration)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Action Vector size incompatible. Got %i, expected %i.", *GetName(), ActionVector.Num(), GetActionVectorSize());
		return;
	}
