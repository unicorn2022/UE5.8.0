...
namespace UE::Learning::Agents::Recording
{
	UE_API int32 GetMagicNumber();
	UE_API int32 GetVersionNumber();
}
...
/** Container for the record's binary data. Kept separate to keep Slate from loading it. */
UCLASS()
class ULearningAgentsRecordBlob : public UObject
{
	GENERATED_BODY()
...
public:
	...
	/** The binary data. */
	UPROPERTY()
	TObjectPtr<ULearningAgentsRecordBlob> Blob;
};
...
public:
	...
	UE_API virtual void ResetRecording();
	...
	UFUNCTION(BlueprintCallable, Category = "LearningAgents", meta = (RelativePath))
	UE_API virtual void LoadRecordingFromFile(const FFilePath& File, const FName& SchemaName, const FGameplayTag& Tag, TArray<FLearningAgentsRecord>& OutRecords);
...
public:
	...
	/**
	 * Builds a replay buffer from this recording, filtering by tag and validating schema compatibility.
	 * Returns nullptr if no valid data is found.
	 *
	 * @param Interactor							The interactor whose observation/action schemas define the target layout.
	 * @param RecordingFilterTag					Tag used to filter records. If invalid, no filtering is applied.
	 * @param bCheckObservationSchemaCompatibility	Whether to validate observation schema subset compatibility.
	 * @param bCheckActionSchemaCompatibility		Whether to validate action schema subset compatibility.
	 * @return A replay buffer containing the copied data, or nullptr if no valid records were found.
	 */
	UE_API TUniquePtr<UE::Learning::FReplayBuffer> BuildReplayBuffer(
	...
	UE_API void AddSchema(const FName& SchemaName, FLearningAgentsSchema NewSchema);
