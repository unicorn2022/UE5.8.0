...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnRecordingBeginSignature);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnRecordingEndSignature);
...
public:
	...
	/** Called after the recording has started */
	UPROPERTY(BlueprintAssignable, Category = "LearningAgents")
	FOnRecordingBeginSignature OnRecordingBegin;

	/** Called after the recording has stopped */
	UPROPERTY(BlueprintAssignable, Category = "LearningAgents")
	FOnRecordingEndSignature OnRecordingEnd;
...
protected:
	...
	/**
	 * Saves the schema config JSON to the recording directory with the given filename.
	 * Defaults to "schema-config.json".
	 */
	UFUNCTION(BlueprintCallable, Category = "LearningAgents")
	UE_API FLearningAgentsSchema SaveSchemaConfig(const FString& FileName = TEXT("schema-config.json"));
