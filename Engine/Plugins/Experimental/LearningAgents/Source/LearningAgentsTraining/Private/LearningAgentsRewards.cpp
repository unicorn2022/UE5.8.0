	...
	const FLinearColor VisualLoggerColor)
{
	if (!SplineComponent)
	{
		UE_LOGF(LogLearning, Error, "MakeRewardFromVelocityAlongSpline: SplineComponent is nullptr.");
		return 0.0f;
	}
	...
	{
		UE_LOGF(LogLearning, Warning, "MakeRewardFromVelocityAlongSpline: FiniteDifferenceDelta is too small (%6.4f). Clamping to %6.4f.", FiniteDifferenceDelta, UE_KINDA_SMALL_NUMBER);
