	...
	const FLearningAgentsPPOTrainerSettings& TrainerSettings)
{
	if (!InManager)
	{
		UE_LOGF(LogLearning, Error, "MakePPOTrainer: InManager is nullptr.");
		return nullptr;
	}

	if (!Class)
	{
		UE_LOGF(LogLearning, Error, "MakePPOTrainer: Class is nullptr.");
		return nullptr;
	}
	...
	const FLearningAgentsPPOTrainerSettings& TrainerSettings)
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: InTrainingEnvironment is nullptr.", *GetName());
		return;
	}

	if (!InTrainingEnvironment->IsSetup())
	{
		UE_LOGF(LogLearning, Error, "%ls: %ls's Setup must be run before it can be used.", *GetName(), *InTrainingEnvironment->GetName());
		return;
	}
	...
	if (!InCritic)
	{
		UE_LOGF(LogLearning, Error, "%ls: InCritic is nullptr.", *GetName());
		return;
	}

	if (!InCritic->IsSetup())
	{
		UE_LOGF(LogLearning, Error, "%ls: %ls's Setup must be run before it can be used.", *GetName(), *InCritic->GetName());
		return;
	}
	...
	const bool bResetAgentsOnBegin)
{
	...
	if (IsTraining())
	{
		UE_LOGF(LogLearning, Error, "%ls: Already Training!", *GetName());
		return;
	}
	...
	UE_LOGF(LogLearning, Display, "%ls: Sending initial policy...", *GetName());
	...
	if (Response != UE::Learning::ETrainerResponse::Success)
	{
		UE_LOGF(LogLearning, Error, "%ls: Error sending critic to trainer: %ls. Check log for additional errors.", *GetName(), UE::Learning::Trainer::GetResponseString(Response));
		bHasTrainingFailed = true;
		Trainer->Terminate();
		return;
	}
...
void ULearningAgentsPPOTrainer::ProcessExperience(const bool bResetAgentsOnUpdate)
{
	...
	for (const int32 AgentId : Manager->GetAllAgentSet())
	{
		...
		{
			UE_LOGF(LogLearning, Display, "%ls: Agent with id %i has not completed a full step of observations, action modifiers, actions, rewards, completions and so experience will not be processed for it.", *GetName(), AgentId);
			continue;
		}
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i has non-matching iteration numbers (observation: %lli, action: %lli, action modifiers: %lli, reward: %lli, completion: %lli). Experience will not be processed for it.", *GetName(), AgentId,
	...
	for (const int32 AgentId : ValidAgentSet)
	{
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Agent with id %i has completed episode and will be reset but has not generated any experience.", *GetName(), AgentId);
		}
	}
	...
	if (bReplayBufferFull)
	{
		...
		if (Response != UE::Learning::ETrainerResponse::Success)
		{
			UE_LOGF(LogLearning, Error, "%ls: Error waiting to push experience to trainer: %ls. Check log for additional errors.", *GetName(), UE::Learning::Trainer::GetResponseString(Response));
			bHasTrainingFailed = true;
			EndTraining();
			return;
		}
		...
		for (int32 i = 0; i < NetworkIds.Num(); i++)
		{
			...
			{
				UE_LOGF(LogLearning, Error, "Error receiving network (id=%d) from trainer: %ls. Check log for errors.", NetworkIds[i], UE::Learning::Trainer::GetResponseString(NetworkResponses[i]));
				bHasTrainingFailed = true;
				EndTraining();
				return;
			}
