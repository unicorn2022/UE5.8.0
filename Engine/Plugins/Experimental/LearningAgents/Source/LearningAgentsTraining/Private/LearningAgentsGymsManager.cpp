...
void ALearningAgentsGymsManager::SpawnGyms()
{
	...
	for (FSpawnGymInfo& GymTemplate : GymTemplates)
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("GymsCountOverride"), SpawnCountInput))
		{
			...
			if (SpawnCountOverride > 0)
			{
				...
				UE_LOGF(LogLearning, Log, "Commandline flag -GymsCountOverride is active! Generating %d Gyms for the template %ls.", GymTemplate.GetCount(), *GymTemplate.GymClass->GetName());
			}
			else
			{
				UE_LOGF(LogLearning, Log, "Commandline flag -GymsCountOverride was passed an invalid input. Generating %d Gyms for the template %ls.", GymTemplate.GetCount(), *GymTemplate.GymClass->GetName());
			}
		}
	...
	{
		UE_LOGF(LogLearning, Log, "Unable to build navigation system due to invalid navigation system reference!");
	}
}
...
void ALearningAgentsGymsManager::InitializeRandomStream()
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("GymsManagerRandomSeed"), RandomSeedInput))
	{
		if (RandomSeedInput.StartsWith(TEXT("=")))
		{
			...
			UE_LOGF(LogLearning, Log, "Removed leading '=' from RandomSeedInput: %ls", *RandomSeedInput);
		}
	...
	if (FParse::Param(FCommandLine::Get(), TEXT("GymsManagerRandomizeNoSeed")))
	{
		...
		UE_LOGF(LogLearning, Log, "Commandline flag -GymsManagerRandomizeNoSeed is active! Using randomly generated seed from timestamp: %d.", RandomSeed);
	}
	RandomStream = MakeShareable(new FRandomStream);
	RandomStream->Initialize(RandomSeed);
	UE_LOGF(LogLearning, Log, "GymsManager initialized with seed %d!", RandomSeed);
}
