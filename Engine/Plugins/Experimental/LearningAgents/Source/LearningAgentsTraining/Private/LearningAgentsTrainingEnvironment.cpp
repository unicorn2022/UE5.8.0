	...
	const FName Name)
{
	if (!InManager)
	{
		UE_LOGF(LogLearning, Error, "MakeTrainer: InManager is nullptr.");
		return nullptr;
	}

	if (!Class)
	{
		UE_LOGF(LogLearning, Error, "MakeTrainer: Class is nullptr.");
		return nullptr;
	}
...
{
	UE_LOGF(LogLearning, Error, "%ls: GatherAgentReward function must be overridden!", *GetName());
	OutReward = 0.0f;
}
...
{
	UE_LOGF(LogLearning, Error, "%ls: GatherAgentCompletion function must be overridden!", *GetName());
...
{
	UE_LOGF(LogLearning, Error, "%ls: ResetAgentEpisode function must be overridden!", *GetName());
}
...
void ULearningAgentsTrainingEnvironment::GatherRewards()
{
	...
	{
		UE_LOGF(LogLearning, Warning, "%ls: Not enough rewards added by GetAgentRewards. Expected %i, Got %i.", *GetName(), ValidAgentSet.Num(), RewardBuffer.Num());
		return;
	}
	...
	for (int32 AgentIdx = 0; AgentIdx < RewardBuffer.Num(); AgentIdx++)
	{
		...
		{
			UE_LOGF(LogLearning, Warning, "%ls: Got invalid reward for agent %i: %f.", *GetName(), ValidAgentSet[AgentIdx], RewardValue);
			continue;
		}
	}
}
...
void ULearningAgentsTrainingEnvironment::GatherCompletions()
{
	...
	{
		UE_LOGF(LogLearning, Warning, "%ls: Not enough completions added by GetAgentCompletions. Expected %i, Got %i.", *GetName(), ValidAgentSet.Num(), CompletionBuffer.Num());
		return;
	}
...
float ULearningAgentsTrainingEnvironment::GetReward(const int32 AgentId) const
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Agent with id %d has not evaluated rewards. Did you run EvaluateRewards?", *GetName(), AgentId);
		return 0.0f;
	}
...
ELearningAgentsCompletion ULearningAgentsTrainingEnvironment::GetCompletion(const int32 AgentId) const
{
	...
	{
		UE_LOGF(LogLearning, Error, "%ls: Agent with id %d has not evaluated completions. Did you run EvaluateCompletions?", *GetName(), AgentId);
		return ELearningAgentsCompletion::Running;
	}
