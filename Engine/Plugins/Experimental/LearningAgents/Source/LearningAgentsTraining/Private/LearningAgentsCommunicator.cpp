	...
	const FLearningAgentsSharedMemoryCommunicatorSettings& SharedMemorySettings)
{
	...
	{
		UE_LOGF(LogLearning, Display, "SpawnSharedMemoryTrainingProcess: Skipping spawning because bUseExternalTrainingProcess is True.");
	...
	{
		UE_LOGF(LogLearning, Error, "SpawnSharedMemoryTrainingProcess: Can't find Python executable \"%ls\".", *PythonExecutablePath);
		return TrainerProcess;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "SpawnSharedMemoryTrainingProcess: Can't find LearningAgents plugin Content \"%ls\".", *PythonContentPath);
		return TrainerProcess;
	}
	...
	{
		UE_LOGF(LogLearning, Error, "SpawnSharedMemoryTrainingProcess: Can't find custom trainer module \"%ls\".", *CustomTrainerModulePath);
		return TrainerProcess;
	}

	const FString NneCpuContentPath = TrainerProcessSettings.GetNneCpuContentPath();
	...
	const FLearningAgentsSharedMemoryCommunicatorSettings& SharedMemorySettings)
{
	...
	{
		UE_LOGF(LogLearning, Display, "Overriding Task Name from the cmdline: %ls", *CmdLineTaskName);
	}
	...
	{
		UE_LOGF(LogLearning, Display, "Overriding Timeout with value from the cmdline: %f", CmdLineTimeout);
	}
	...
	{
		UE_LOGF(LogLearning, Display, "Overriding Controls Guid with value from the cmdline: %ls", *CmdLineControlsGuid.ToString(PLATFORM_MAC ? EGuidFormats::Short : EGuidFormats::DigitsWithHyphensInBraces));
	}
	...
	{
		UE_LOGF(LogLearning, Display, "Overriding Config Path with value from the cmdline: %ls", *CmdLineConfigPath);
	}
	...
	const FLearningAgentsSocketCommunicatorSettings& SocketSettings)
{
	...
	{
		UE_LOGF(LogLearning, Display, "SpawnSocketTrainingProcess: Skipping spawning because bUseExternalTrainingProcess is True.");
		return TrainerProcess;
	}
	...
	if (!FPaths::FileExists(PythonExecutablePath))
	{
		UE_LOGF(LogLearning, Error, "SpawnSocketTrainingProcess: Can't find Python executable \"%ls\".", *PythonExecutablePath);
		return TrainerProcess;
	}
	...
	if (!FPaths::DirectoryExists(PythonContentPath))
	{
		UE_LOGF(LogLearning, Error, "SpawnSocketTrainingProcess: Can't find LearningAgents plugin Content \"%ls\".", *PythonContentPath);
		return TrainerProcess;
	}
	...
	if (!CustomTrainerModulePath.IsEmpty() and !FPaths::DirectoryExists(CustomTrainerModulePath))
	{
		UE_LOGF(LogLearning, Error, "SpawnSocketTrainingProcess: Can't find custom trainer module \"%ls\".", *CustomTrainerModulePath);
		return TrainerProcess;
	}
	...
	const FLearningAgentsSocketCommunicatorSettings& SocketSettings)
{
	...
	{
		UE_LOGF(LogLearning, Display, "Using Hostname override from the cmdline: %ls", *CmdLineHostname);
	}
	else if (bIpAddressOverridden)
	{
		UE_LOGF(LogLearning, Display, "Using Ip Address override from the cmdline: %ls", *CmdLineIpAddress);
	}
	...
	if (bPortOverridden)
	{
		UE_LOGF(LogLearning, Display, "Using Port override from the cmdline: %u", CmdLinePort);
	}
	...
	{
		UE_LOGF(LogLearning, Error, "MakeSocketCommunicator: Failed to connect to training process: %ls. Check log for additional errors.", UE::Learning::Trainer::GetResponseString(Response));
