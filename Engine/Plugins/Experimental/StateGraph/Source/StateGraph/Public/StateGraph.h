...
public:
		...
		CreateNode(FName NodeName, ObjectType* Object, FunctionType Function, ArgsTypes&&... Args)
	{
		...
		else if constexpr (std::is_member_function_pointer_v<FunctionType>)
		{
...
TSharedPtr<NodeType, ESPMode::ThreadSafe> FStateGraphNode::Next(ArgsTypes&&... Args) const
{
	...
	if (!StateGraphPtr)
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls] Node attempted to be created with invalid state graph", *LogName);
