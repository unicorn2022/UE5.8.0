...
{
	UE_LOGF(LogStateGraph, VeryVerbose, "[%ls] Created node", *LogName);
}

FStateGraphNode::~FStateGraphNode()
{
	UE_LOGF(LogStateGraph, VeryVerbose, "[%ls] Destroyed node", *LogName);
}
...
bool FStateGraphNode::CheckDependencies() const
{
	...
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls] Node checked with invalid state graph", *LogName);
		return false;
	}
...
void FStateGraphNode::Complete()
{
	...
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls] Node already completed", *LogName);
		return;
	}
	...
	UE_LOGF(LogStateGraph, Log, "[%ls] Completed node (Duration=%.3f Timeout=%.3f)", *LogName, GetDuration(), Timeout);
	...
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls] Node completed with invalid state graph", *LogName);
	}
}

void FStateGraphNode::Reset()
{
	UE_LOGF(LogStateGraph, Verbose, "[%ls] Resetting node", *LogName);
	StartTime = 0.f;
	CompletedTime = 0.f;
...
bool FStateGraphNodeFunction::CheckDependencies() const
{
	...
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls] Function node start not bound", *GetLogName());
		return false;
	}
...
void FStateGraphNodeFunction::Start()
{
	...
	StartFunction.Execute(*StateGraph,
		[NodeWeakPtr, LogName = GetLogName()]()
		{
			...
			{
				UE_LOGF(LogStateGraph, Log, "[%ls] Function node completed after node was destroyed", *LogName);
			}
		});
}
	...
	ConfigSectionName(FString::Printf(TEXT("StateGraph.%s"), *InName.ToString()))
{
	...
	UE_LOGF(LogStateGraph, VeryVerbose, "[%ls] Created state graph", *LogName);
}

FStateGraph::~FStateGraph()
{
	UE_LOGF(LogStateGraph, VeryVerbose, "[%ls] Destroyed state graph", *LogName);
...
bool FStateGraph::AddNode(const FStateGraphNodeRef& Node)
{
	...
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls.%ls] Node already associated with a state graph %ls", *LogName, *Node->GetName().ToString(), *Node->LogName);
		return false;
	}

	if (Nodes.Contains(Node->GetName()))
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls.%ls] Node with the same name already exists", *LogName, *Node->GetName().ToString());
		return false;
	}

	Node->LogName = FString::Printf(TEXT("%s.%s"), *LogName, *Node->GetName().ToString());
	UE_LOGF(LogStateGraph, Verbose, "[%ls] Adding node", *Node->LogName);
...
bool FStateGraph::RemoveNode(FName NodeName)
{
	...
	if (!Node)
	{
		UE_LOGF(LogStateGraph, Warning, "[%ls.%ls] Failed to remove node", *LogName, *NodeName.ToString());
		return false;
	}

	UE_LOGF(LogStateGraph, Verbose, "[%ls] Removing node", *(*Node)->LogName);
...
void FStateGraph::Run()
{
	...
	if (Timeout > 0.f)
	{
		...
		if (NextTimeout <= 0.f)
		{
			UE_LOGF(LogStateGraph, Log, "[%ls] State graph timed out (Duration=%.3f Timeout=%.3f)", *LogName, Duration, Timeout);
	...
	UE_LOGF(LogStateGraph, Verbose, "[%ls] Starting run loop", *LogName);
	...
	for (FName NodeName : NodeNames)
	{
		...
		case FStateGraphNode::EStatus::Blocked:
			...
			UE_LOGF(LogStateGraph, Log, "[%ls] Starting node", *(*Node)->LogName);
		...
		case FStateGraphNode::EStatus::Started:
			if ((*Node)->Timeout > 0.f)
			{
				...
				{
					++TimedOut;
					UE_LOGF(LogStateGraph, Log, "[%ls] Node timed out (Duration=%.3f Timeout=%.3f)", *(*Node)->LogName, NodeDuration, (*Node)->Timeout);
		...
		default:
			UE_LOGF(LogStateGraph, Error, "[%ls] Unknown state", *(*Node)->LogName);
			checkNoEntry();
			break;
		}
	}
	...
	bRunning = false;
	UE_LOGF(LogStateGraph, Verbose, "[%ls] Duration=%.3f Timeout=%.3f Blocked=%d Started=%d Running=%d Completed=%d Removed=%d TimedOut=%d",
	...
	else if (Started == 0 && Running == 0)
	{
		...
		{
			CompletedTime = Now;
			UE_LOGF(LogStateGraph, Log, "[%ls] Completed (Duration=%.3f Timeout=%.3f)", *LogName, GetDuration(), Timeout);
			SetStatus(EStatus::Completed);
		}
		else
		{
			UE_LOGF(LogStateGraph, Warning, "[%ls] Blocked on %d nodes, timed out %d nodes", *LogName, Blocked, TimedOut);
	...
	{
		UE_LOGF(LogStateGraph, VeryVerbose, "[%ls] Waiting on %d nodes", *LogName, Started + Running);
	...
	{
		UE_LOGF(LogStateGraph, Verbose, "[%ls] Setting timer for %.3f", *LogName, NextTimeout);
...
void FStateGraph::Reset()
{
	...
	UE_LOGF(LogStateGraph, Verbose, "[%ls] Resetting state graph", *LogName);
	StartTime = 0.f;
	CompletedTime = 0.f;
	bRunAgain = false;
	FTSTicker::RemoveTicker(TimeoutTicker);
...
{
	UE_LOGF(LogStateGraph, Verbose, "[%ls] Pausing state graph", *LogName);
	bRunAgain = false;
	FTSTicker::RemoveTicker(TimeoutTicker);
...
void FStateGraph::LogDebugInfo(ELogVerbosity::Type Verbosity)
{
	...
	{
		UE_LOGF(LogStateGraph, Warning, "%ls", *Message);
	}
	else
	{
		UE_LOGF(LogStateGraph, Log, "%ls", *Message);
	}
