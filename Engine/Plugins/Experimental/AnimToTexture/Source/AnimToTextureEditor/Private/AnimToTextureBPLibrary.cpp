...
bool UAnimToTextureBPLibrary::AnimationToTexture(UAnimToTextureDataAsset* DataAsset)
{
	...
	if (DataAsset->Mode == EAnimToTextureMode::Vertex)
	{
		...
		{
			UE_LOGF(LogAnimToTextureEditor, Warning, "Vertex Animation data cannot be fit in a %ix%i texture.", DataAsset->MaxHeight, DataAsset->MaxWidth);
			return false;
		}
	...
	if (DataAsset->Mode == EAnimToTextureMode::Bone)
	{
		...
		// Write Bone Position and Rotation Textures
		{
			...
			if (!FindBestResolution(DataAsset->NumFrames + 1, DataAsset->NumBones,
				...
				DataAsset->MaxHeight, DataAsset->MaxWidth, DataAsset->bEnforcePowerOfTwo))
			{
				UE_LOGF(LogAnimToTextureEditor, Warning, "Bone Animation data cannot be fit in a %ix%i texture.", DataAsset->MaxHeight, DataAsset->MaxWidth);
				return false;
			}
		...
		// Write Weights Texture
		{
			...
			if (!FindBestResolution(2, NumVertices,
				...
				DataAsset->MaxHeight, DataAsset->MaxWidth, DataAsset->bEnforcePowerOfTwo))
			{
				UE_LOGF(LogAnimToTextureEditor, Warning, "Weights Data cannot be fit in a %ix%i texture.", DataAsset->MaxHeight, DataAsset->MaxWidth);
				return false;
			}
	...
	int32& OutSocketIndex, TArray<FAnimToTextureAnimSequenceInfo>& OutAnimSequences)
{
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid StaticMesh");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid SkeletalMesh");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid SkeletalMesh. No valid Skeleton found");
		return false;
	}
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid StaticMesh LOD Index: %i", DataAsset->StaticLODIndex);
		return false;
	}
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid SkeletalMesh LOD Index: %i", DataAsset->SkeletalLODIndex);
		return false;
	}
	...
	if (DataAsset->AttachToSocket.IsValid() && !DataAsset->AttachToSocket.IsNone())
	{
		...
		{
			UE_LOGF(LogAnimToTextureEditor, Warning, "Socket: %ls not found in Raw Bone List", *DataAsset->AttachToSocket.ToString());
			return false;
		}
		else
		{
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid StaticMesh UVChannel: %i. Already used by LightMap", DataAsset->UVChannel);
		return false;
	}
	...
	if (DataAsset->Precision == EAnimToTexturePrecision::EightBits &&
		NumBones > 256)
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Too many Bones: %i. There is a maximum of 256 bones for 8bit Precision", NumBones);
		return false;
	}
	...
	for (const FAnimToTextureAnimSequenceInfo& AnimSequenceInfo : DataAsset->AnimSequences)
	{
		...
		if (AnimSequenceInfo.bEnabled && AnimSequence)
		{
			...
			{
				UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid AnimSequence: %ls for given SkeletalMesh: %ls", *AnimSequence->GetFName().ToString(), *DataAsset->GetSkeletalMesh()->GetFName().ToString());
				return false;
			}
			...
			{
				UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid CustomRange for AnimSequence: %ls", *AnimSequence->GetName());
				return false;
			}
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "No valid AnimSequences found");
		return false;
	}
	...
	const EMaterialParameterAssociation MaterialParameterAssociation)
{
	...
	if (DataAsset->bAutoPlay)
	{
		...
		{
			UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid AnimationIndex: %i", DataAsset->AnimationIndex);
		}
	}
	else
	{
		...
		{
			UE_LOGF(LogAnimToTextureEditor, Warning, "Frame out of range: %i", DataAsset->Frame);
		}
	}
...
UStaticMesh* UAnimToTextureBPLibrary::ConvertSkeletalMeshToStaticMesh(USkeletalMesh* SkeletalMesh, const FString PackageName, const int32 LODIndex)
{
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid LODIndex: %i", LODIndex);
		return nullptr;
	}
	...
	if (OutStaticMesh)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAnimToTextureEditor, Warning, "Invalid Source Model Index: %i", LODIndex);
	...
	const int32 Height, const int32 Width)
{
	...
	if (UVChannelIndex == StaticMesh->GetNumUVChannels(LODIndex))
	{
		...
		{
			UE_LOGF(LogAnimToTextureEditor, Warning, "Unable to Add UVChannel");
			return false;
		}
	}
	else if (UVChannelIndex > StaticMesh->GetNumUVChannels(LODIndex))
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "UVChannel: %i Out of Range. Number of existing UVChannels: %i", UVChannelIndex, StaticMesh->GetNumUVChannels(LODIndex));
		return false;
	}
	...
	{
		UE_LOGF(LogAnimToTextureEditor, Warning, "Unable to Set UVChannel: %i. TexCoords: %i", UVChannelIndex, TexCoords.Num());
		return false;
	}
}
