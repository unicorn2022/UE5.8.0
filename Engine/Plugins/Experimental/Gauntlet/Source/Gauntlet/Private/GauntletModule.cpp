...
void FGauntletModuleImpl::StartupModule()
{
	...
	{
		UE_LOGF(LogGauntlet, Display, "Gauntlet Initialized");
		FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FGauntletModuleImpl::PerformInitialization);
	}
#if WITH_EDITOR
	else
	{
...
void FGauntletModuleImpl::ShutdownModule()
{
	...
	UE_LOGF(LogGauntlet, Log, "Gauntlet Shutdown");
...
void FGauntletModuleImpl::LoadControllers()
{
	...
	if (ControllerString.Len())
	{
		...
		for (const FString& Name : ControllerNames)
		{
			...
			UE_LOGF(LogGauntlet, Display, "Added Gauntlet controller %ls", *Name);
...
void FGauntletModuleImpl::BroadcastStateChange(FName NewState)
{
	...
	UE_LOGF(LogGauntlet, Display, "Changed state to %ls", *NewState.ToString());
}
...
void FGauntletModuleImpl::LogHeartbeat()
{
	...
	UE_LOGF(LogGauntlet, Display, "%ls %ls", *HeartbeatMessage, *HeartbeatStatus);
	bHeartbeatActive = false;
