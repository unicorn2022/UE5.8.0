...
{
public:
	using InnerArrayType = TArray<T>;
	using AllocatorType = InnerArrayType::AllocatorType;
	using SizeType = InnerArrayType::SizeType;
	...
	void SetNum(SizeType NewNum, EAllowShrinking AllowShrink = UE::Core::Private::AllowShrinkingByDefault<AllocatorType>())
	{
		FWriteScopeLock ScopeLock(Lock);

		CheckCopyOnWrite();
		State->Arr.SetNum(NewNum, AllowShrink);
	}

	void SetNumZeroed(SizeType NewNum, EAllowShrinking AllowShrink = UE::Core::Private::AllowShrinkingByDefault<AllocatorType>())
	{
		FWriteScopeLock ScopeLock(Lock);

		CheckCopyOnWrite();
		State->Arr.SetNumZeroed(NewNum, AllowShrink);
	}

	SizeType Num() const
	{
		return State ? State->Arr.Num() : 0;
	}

	const T& Last(SizeType IndexFromEnd = 0) const
	{
		checkf(State, TEXT("Array index out of bounds: Last() accessing empty array"));
		return State->Arr.Last(IndexFromEnd);
	}
...
private:
	...
	struct FState : public UE::Private::FQueryableRefCountedObject
	{
		TArray<T> Arr;
	};
