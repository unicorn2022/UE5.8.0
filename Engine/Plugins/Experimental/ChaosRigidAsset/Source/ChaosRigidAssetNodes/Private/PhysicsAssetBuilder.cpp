...
namespace UE::Chaos::RigidAsset
{
	...
	FPhysicsAssetBuilder& FPhysicsAssetBuilder::JoinLast(TObjectPtr<UPhysicsConstraintTemplate> NewJoint)
	{
		...
		{
			UE_LOGF(LogTemp, Error, "JoinLast called with less than two available bodies in the builder - cannot join these bodies");
		}
	...
	TObjectPtr<UPhysicsAsset> FPhysicsAssetBuilder::Build()
	{
		...
		// Refresh components using this physics asset
		for(FThreadSafeObjectIterator Iter(USkeletalMeshComponent::StaticClass()); Iter; ++Iter)
		{
			...
			if(!SkeletalMeshComponent || !SkeletalMeshComponent->IsRegistered())
			{
				// If the component is invalid, or it's not registered then it can't have physics state.
				continue;
			}

			if(SkeletalMeshComponent->GetPhysicsAsset() == TargetAsset)
			{
