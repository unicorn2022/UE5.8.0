...
/**
 * Merge a bone selection using the specified merge profile
 */
USTRUCT()
struct FDataflowMergeBoneSelection : public FRigidDataflowNode
{
	GENERATED_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FDataflowMergeBoneSelection, "Merge Bones in Selection", "PhysicsAsset", "")
	DATAFLOW_NODE_RENDER_TYPE("GeomRender", FName("FBoneSelection"), "Selection")

public:

	FDataflowMergeBoneSelection(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid())
		: FRigidDataflowNode(InParam, InGuid)
	{
		Register(InParam);
	}
...
private:
	...
	// Resulting bone selection
	UPROPERTY(meta = (DataflowInput, DataflowOutput, DataflowPassthrough = "Selection"))
	FRigidAssetBoneSelection Selection;

	// Merge settings to use to decide how to merge the bones
	UPROPERTY(EditAnywhere, Category = Settings)
	FBaseGenerationSettings MergeProfile;
...
/**
* Run a rigid simulation of a Physics asset state and bake it into an animation file 
*/
USTRUCT(meta = (Experimental))
struct FDataflowRigidSimulationCachingNode : public FRigidDataflowNode
{
	GENERATED_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FDataflowRigidSimulationCachingNode, "Rigid Simulation Caching", "PhysicsAsset", "")

public:

	FDataflowRigidSimulationCachingNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid())
		: FRigidDataflowNode(InParam, InGuid)
	{
		Register(InParam);
	}
...
private:
	...
	/** Input physics asset state to use for the simulation */
	UPROPERTY(meta = (DataflowInput))
	FPhysicsAssetDataflowState State;

	/** Output animation asset  */
	UPROPERTY(meta = (DataflowOutput))
	TObjectPtr<UAnimSequence> Animation = nullptr;

	/** Duration in seconds of the simulation */
	UPROPERTY(EditAnywhere, Category = Caching, meta = (DataflowInput, ClampMin=0))
	float Duration = 5.0;

	/** Number of frames per second */
	UPROPERTY(EditAnywhere, Category = Caching, meta = (DataflowInput, ClampMin = 1))
	int32 FrameRate = 30;

	/** Gravity vector in cm/s^2 */
	UPROPERTY(EditAnywhere, Category = Simulation, meta = (DataflowInput))
	FVector Gravity = FVector(0, 0, 980);

	/** Simulation space velocity in cm/s */
	UPROPERTY(EditAnywhere, Category = Simulation, meta = (DataflowInput))
	FVector Velocity = FVector(0, 0, 0);

	/** Affector used to manipulate objects in the simulation */
	UPROPERTY(EditAnywhere, Category = Simulation, meta = (DataflowInput))
	FDataflowSimulationAffectorTypes Affector;

	void Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const override;
	void Register(const UE::Dataflow::FNodeParameters& InParam);

	TUniquePtr<FBodyInstance> CreateBodyInstance(USkeletalBodySetup* BodySetup) const;

	struct FSimulationState
	{
		float FixedTimeStep = 0;
		TUniquePtr<ImmediatePhysics::FSimulation> Simulation;
		TMap<FName, ImmediatePhysics::FActorHandle*> ActorsByName;
		TArray<ImmediatePhysics::FActorHandle*> Actors;
	};

	void BuildSimulationFromState(const FPhysicsAssetDataflowState& InState, FSimulationState& OutSimState) const;
	void RunSimulation(FSimulationState& Simulation, const FVector& InGravity) const;
};
