...
EAngularConstraintMotion GetMotionType(float InLimit)
{
	if(InLimit == 0)
	{
		return ACM_Locked;
...
TArray<TObjectPtr<UPhysicsConstraintTemplate>> UConstraintGenerator_SwingTwist::Build(TObjectPtr<UPhysicsConstraintTemplate> ConstraintTemplate, FRigidAssetBoneSelection Bones) const
{
	...
	for(const FRigidAssetBoneInfo& Bone : Bones.SelectedBones)
	{
		...
		while(CurrParent != INDEX_NONE)
		{
			...
			Defaults.SetAngularSwing1Limit(GetMotionType(Swing1Limit), Swing1Limit);
			Defaults.SetAngularSwing2Limit(GetMotionType(Swing2Limit), Swing2Limit);
			Defaults.SetAngularTwistLimit(GetMotionType(TwistLimit), TwistLimit);
