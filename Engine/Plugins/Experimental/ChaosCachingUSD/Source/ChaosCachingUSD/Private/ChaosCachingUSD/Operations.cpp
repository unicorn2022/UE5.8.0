...
UE::ChaosCachingUSD::NewStage(const FString& StageName, UE::FUsdStage& UsdStage)
{
	...
	{
		UE_LOGF(LogUsd, Error, "Cannot clobber existing file: '%ls'", *StageName);
		return false;
	}
	...
	if (!bFound)
	{
		...
		UE_LOGF(LogUsd, Error, 
			"Failed to create new USD stage: '%ls', file format '%ls' is not native to USD. Supported formats: '%ls'",
	...
	if (!UsdStage)
	{
		UE_LOGF(LogUsd, Error, "Failed to create new USD stage: '%ls'", *StageName);
		return false;
	}
...
UE::ChaosCachingUSD::OpenStage(const FString& StageName, UE::FUsdStage& UsdStage)
{
	if (!FPaths::FileExists(StageName))
	{
		...
		{
			UE_LOGF(LogUsd, Error, "File not found: '%ls'", *StageName);
			return false;
		}
	}
	...
	if (!UsdStage)
	{
		UE_LOGF(LogUsd, Error, "Failed to open USD stage: '%ls'", *StageName);
		return false;
	}
	return true;
}
...
UE::ChaosCachingUSD::SaveStage(UE::FUsdStage& Stage, const double FirstFrame, const double LastFrame)
{
	...
	{
		UE_LOGF(LogUsd, Log, "Saving USD file: '%ls'", *RootLayer.GetRealPath());
		if (!RootLayer.Save())
		{
			UE_LOGF(LogUsd, Error, "Failed to save USD file: '%ls'", *RootLayer.GetRealPath());
			return false;
		}
		return true;
	}
	...
	{
		UE_LOGF(LogUsd, Error, "USD Stage has no root layer.");
		return false;
	}
}
	...
	const int32 StructureIndex)
{
	...
	if (StructureIndex != INDEX_NONE)
	{
		...
		{
			UE_LOGF(LogUsd, Error, "Invalid vertex index: %d, valid range [0, %d).", StructureIndex, VertexStart->Num());
			return false;
		}
		...
		{
			UE_LOGF(LogUsd, Error, "Invalid tet mesh index: %d, valid range [0, %d).", StructureIndex, TetrahedronStart->Num());
			return false;
		}
			...
			!FacesCount->IsValidIndex(StructureIndex))
		{
			...
			UE_LOGF(LogUsd, Warning, "Invalid tet mesh faces index: %d, valid range [0, %d),", StructureIndex, FacesStart->Num());
		}
		else
		{
		...
		UE_LOGF(LogUsd, Log, "Extracting tetrahedral mesh %d of %d from collection; vertex range [%d, %d), tet [%d, %d), tri [%d, %d).",
	...
	{
		UE_LOGF(LogUsd, Error, "Failed to create UsdGeomPointBased at path: '%ls' on USD stage: '%ls'.",
	...
	{
		UE_LOGF(LogUsd, Warning, "Failed to create UsdGeomMesh at path: '%ls' on USD stage: '%ls'.",
			*PrimPath, *Stage.GetRootLayer().GetRealPath());
	}
	...
	{
		UE_LOGF(LogUsd, Warning, "Failed to create UEUsdGeomTetMesh at path: '%ls' on USD stage: '%ls'.",
			*PrimPath, *Stage.GetRootLayer().GetRealPath());
	}
	...
	TArray<double>& TimeSamples)
{
	...
	if (!Prim)
	{
		UE_LOGF(LogUsd, Error, "No prim found at path '%ls'.", *PrimPath);
		return false;
	}
	UE::FUsdAttribute Attr = Prim.GetAttribute(*AttrName);
	if (!Attr)
	{
		UE_LOGF(LogUsd, Error, "No attribute '%ls' found on prim '%ls'.", *AttrName, *PrimPath);
		return false;
	}
