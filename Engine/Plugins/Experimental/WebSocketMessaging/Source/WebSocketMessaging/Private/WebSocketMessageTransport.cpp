	...
	if (ServerPort > 0)
	{
		...
		if (WebSocketNetworkingModule)
		{
			...
			if (Server)
			{
				...
				{
					Server.Reset();
					UE_LOGF(LogWebSocketMessaging, Error, "Unable to start WebSocketMessaging Server on port %d", ServerPort);
				}
				else
				{
					...
					UE_LOGF(LogWebSocketMessaging, Log, "WebSocketMessaging Server started on port %d", ServerPort);
				}
			}
		}
		else
		{
			UE_LOGF(LogWebSocketMessaging, Log, "Unable to load WebSocketNetworking module, ensure to enable it");
		}
	}
...
{
	UE_LOGF(LogWebSocketMessaging, Log, "Connection to %ls closed, Code: %d Reason: \"%ls\" UserClose: %ls, retrying...", *InWebSocketMessageConnection->Url, InCode, *InReason, bInUserClose ? TEXT("true") : TEXT("false"));
...
void FWebSocketMessageTransport::OnConnectionError(const FString& InMessage, FWebSocketMessageConnectionRef InWebSocketMessageConnection)
{
	...
	{
		UE_LOGF(LogWebSocketMessaging, Log, "Connection to %ls error: %ls, retrying...", *InWebSocketMessageConnection->Url, *InMessage);
	}
	ForgetTransportNode(InWebSocketMessageConnection);
	InWebSocketMessageConnection->bIsConnecting = false;
	RetryConnection(InWebSocketMessageConnection);
...
void FWebSocketMessageTransport::OnJsonMessage(const FString& InMessage, FWebSocketMessageConnectionRef InWebSocketMessageConnection)
{
	...
	{
		UE_LOGF(LogWebSocketMessaging, Log, "Invalid Json Message received on %ls: %ls", *InWebSocketMessageConnection->Url, *ParseError);
	}
}
...
{
	UE_LOGF(LogWebSocketMessaging, Log, "Connected to %ls", *InWebSocketMessageConnection->Url);
	InWebSocketMessageConnection->bIsConnecting = false;
}

void FWebSocketMessageTransport::OnServerConnectionClosed(FWebSocketMessageConnectionRef InWebSocketMessageConnection)
{
	UE_LOGF(LogWebSocketMessaging, Log, "%ls disconnected", *InWebSocketMessageConnection->Url);
	ForgetTransportNode(InWebSocketMessageConnection);
...
void FWebSocketMessageTransport::ClientConnected(INetworkingWebSocket* InNetworkingWebSocket)
{
	...
	UE_LOGF(LogWebSocketMessaging, Log, "New WebSocket Server connection: %ls", *RemoteEndPoint);
