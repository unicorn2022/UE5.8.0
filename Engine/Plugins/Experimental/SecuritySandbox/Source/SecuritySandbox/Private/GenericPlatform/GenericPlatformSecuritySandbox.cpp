...
void FGenericPlatformSecuritySandbox::RestrictSelf()
{
	...
	if (!IsEnabled())
	{
		UE_LOGF(LogSecuritySandbox, Warning, "SecuritySandbox is disabled by config or command line param. Skipping self restrictions.");
		return;
	}

	if (ensureMsgf(!bHasRestrictedSelf, TEXT("RestrictSelf must be called exactly once.")))
	{
		UE_LOGF(LogSecuritySandbox, Log, "Restricting self.");
...
void FGenericPlatformSecuritySandbox::OnEngineInitComplete()
{
	...
	{
		UE_LOGF(LogSecuritySandbox, Verbose, "Engine init complete - triggering RestrictSelf.");
		RestrictSelf();
	}
}
