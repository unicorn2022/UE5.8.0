...
void FWindowsPlatformSecuritySandbox::PlatformRestrictSelf()
{
	...
	if (bNeedsCustomHandlerForURL)
	{
		if (ensure(!FCoreDelegates::LaunchCustomHandlerForURL.IsBound()))
		{
			...
			UE_LOGF(LogSecuritySandbox, Verbose, "Registered FCoreDelegates::LaunchCustomHandlerForURL");
		}
		else
		{
			UE_LOGF(LogSecuritySandbox, Warning, "FCoreDelegates::LaunchCustomHandlerForURL is already bound. Skipping.");
		}
	}
}
	...
	if (Settings)
	{
		if (Settings->bUseLowIntegrityLevel)
		{
			if (FPaths::ShouldSaveToUserDir())
			{
				...
				if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_DEFAULT, &hProcessToken))
				{
					...
					if (CreateWellKnownSid(WinLowLabelSid, nullptr, Sid.Sid, &SidSize))
					{
						...
						{
							UE_LOGF(LogSecuritySandbox, Log, "Set low integrity level");
			...
			else
			{
				...
				UE_LOGF(LogSecuritySandbox, Error, "Low integrity level is configured but ShouldSaveToUserDir returned false. Low integrity level will not be set.");
#else
				UE_LOGF(LogSecuritySandbox, Log, "Low integrity level is configured but ShouldSaveToUserDir returned false. Low integrity level will not be set. "
		...
		{
			UE_LOGF(LogSecuritySandbox, Log, "Low integrity level disabled by config, skipping");
		}
	}
	else 
	{
		UE_LOGF(LogSecuritySandbox, Warning, "Config is not available, skipping integrity level change");
	}
}
	...
	if (Settings)
	{
		if (Settings->bDisallowLowIntegrityLibraries)
		{
			...
			if (GetModuleHandleEx(0, TEXT("kernel32.dll"), &hKernel32))
			{
				...
				if (fnGetProcessMitigationPolicy && fnSetProcessMitigationPolicy)
				{
					...
					if (fnGetProcessMitigationPolicy(GetCurrentProcess(), ProcessImageLoadPolicy, &ImageLoadPolicy, sizeof(ImageLoadPolicy)))
					{
						if (ImageLoadPolicy.NoLowMandatoryLabelImages != 1)
						{
							...
							{
								UE_LOGF(LogSecuritySandbox, Log, "Set process mitigations");
							}
							else
							{
						...
						{
							UE_LOGF(LogSecuritySandbox, Verbose, "NoLowMandatoryLabelImages already set, skipping");
						}
					}
					else
					{
				...
				{
					UE_LOGF(LogSecuritySandbox, Error, "Failed to resolve process mitigation exports");
				}
				FreeLibrary(hKernel32);
			}
			else
			{
				UE_LOGF(LogSecuritySandbox, Error, "Failed to find kernel32.dll");
			}
		}
		else
		{
			UE_LOGF(LogSecuritySandbox, Verbose, "Process mitigations disabled by config, skipping");
		}
	}
	else
	{
		UE_LOGF(LogSecuritySandbox, Warning, "Config is not available, skipping process mitigations");
	}
}
...
void FWindowsPlatformSecuritySandbox::ApplyJobRestrictions()
{
	...
	if (Settings)
	{
		if (Settings->bDisallowChildProcesses || Settings->bDisallowSystemOperations)
		{
			...
			if (hJob)
			{
				...
				if (Settings->bDisallowChildProcesses)
				{
					...
					{
						UE_LOGF(LogSecuritySandbox, Log, "Set job basic limits: 0x%X", BasicInfo.LimitFlags);
				...
				if (Settings->bDisallowSystemOperations)
				{
					...
					{
						UE_LOGF(LogSecuritySandbox, Log, "Set job UI limits: 0x%X", BasicUi.UIRestrictionsClass);
					}
					else
					{
		...
		{
			UE_LOGF(LogSecuritySandbox, Verbose, "Job restrictions disabled by config, skipping");
		}
	}
	else
	{
		UE_LOGF(LogSecuritySandbox, Warning, "Config is not available, skipping job restrictions");
	}
}
...
void FWindowsPlatformSecuritySandbox::CustomHandlerForURL(const FString& URL, FString* ErrorMsg)
{
	...
	UE_LOGF(LogSecuritySandbox, Log, "CustomHandlerForURL %ls", *URL);
	...
	{
		UE_LOGF(LogSecuritySandbox, Warning, "CustomHandlerForURL failed: %ls", *LocalError);
	}
