...
bool UFractureToolMeshCut::IsCuttingActorValid()
{
	...
	AStaticMeshActor* CuttingActor = LocalCutSettings->CuttingActor.Get();
	if (CuttingActor == nullptr)
	{
		return false;
	}
	const UStaticMeshComponent* Component = CuttingActor->GetStaticMeshComponent();
...
void UFractureToolMeshCut::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	{
		FBox Box = MeshCutSettings->CuttingActor.Get()->GetStaticMeshComponent()->GetStaticMesh()->GetBoundingBox();
...
public:
	...
	double CloseVertexDistance = 1e-3;
	double VertexToSurfaceBridgeDistance = 0;
	...
	virtual void CalculateResult(FProgressCancel* Progress) override
	{
		...
		for (const FTransform& ScatterTransform : MeshTransforms)
		{
			...
			int32 Index = CutWithMesh(CuttingMesh, ScatterTransform, InternalSurfaceMaterials, *CollectionCopy, BonesToCut, PointSpacing, Transform, bSetDefaultInternalMaterialsFromCollection, Progress, FIslandSplitSettings(bSplitIslands, CloseVertexDistance, VertexToSurfaceBridgeDistance));
			if (Progress && Progress->Cancelled())
			{
				return;
			}
...
int32 UFractureToolMeshCut::ExecuteFracture(const FFractureToolContext& FractureContext)
{
	if (FractureContext.IsValid())
	{
		...
		MeshCutOp->CloseVertexDistance = CutterSettings->CloseVertexDistance;
		MeshCutOp->VertexToSurfaceBridgeDistance = CutterSettings->VertexToSurfaceBridgeDistance;
		if (LocalCutSettings->CutDistribution == EMeshCutDistribution::SingleCut)
		{
			MeshCutOp->MeshTransforms.Add(LocalCutSettings->CuttingActor.Get()->GetTransform());
		}
		else
		{
			GenerateMeshTransforms(FractureContext, MeshCutOp->MeshTransforms);
		}
		FMeshDescription* MeshDescription = LocalCutSettings->CuttingActor.Get()->GetStaticMeshComponent()->GetStaticMesh()->GetMeshDescription(0);
