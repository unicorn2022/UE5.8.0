...
int32 UFractureToolSlice::ExecuteFracture(const FFractureToolContext& FractureContext)
{
	if(FractureContext.IsValid())
	{
		...
		return CutMultipleWithMultiplePlanes(CuttingPlanes, InternalSurfaceMaterials, *FractureContext.GetGeometryCollection(), FractureContext.GetSelection(), CutterSettings->Grout, CollisionSettings->GetPointSpacing(), FractureContext.GetSeed(), FractureContext.GetTransform(), true, nullptr, FIslandSplitSettings(CutterSettings->bSplitIslands, CutterSettings->CloseVertexDistance, CutterSettings->VertexToSurfaceBridgeDistance));
	}
