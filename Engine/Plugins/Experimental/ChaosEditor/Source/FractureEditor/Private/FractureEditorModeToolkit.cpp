...
namespace FractureEditorModeToolkit::Private
{
	bool bUseNewDataflowEvaluator = true;
	FAutoConsoleVariableRef CVARDataflowEnableGraphEval(TEXT("p.FractureEditor.UseNewDataflowEvaluator"), bUseNewDataflowEvaluator,
		TEXT("Enable new dataflow evaluator for the fracture editor. [def:true]"));
}
...
FFractureEditorModeToolkit::~FFractureEditorModeToolkit()
{
	...
	if (UFractureModeSettings* Settings = GetMutableDefault<UFractureModeSettings>())
	{
		Settings->OnModified.Remove(ProjectSettingsModifiedHandle);
	}
	if (OverridesDetailsView)
	{
		OverridesDetailsView->GetOnFinishedChangingPropertiesDelegate().RemoveAll(this);
	}
}
...
void FFractureEditorModeToolkit::CreateVariableOverrideDetailView()
{
	...
	OnFinishedChangingVariableOverridesDelegateHandle = OverridesDetailsView->GetOnFinishedChangingPropertiesDelegate().AddSP(this, &FFractureEditorModeToolkit::OnPropertyValueChanged);

	RefreshVariableOverrideDetailView(nullptr);
}

void FFractureEditorModeToolkit::OnPropertyValueChanged(const FPropertyChangedEvent& PropertyChangedEvent)
{
	if (UFractureSettings* FractureSettings = GetMutableDefault<UFractureSettings>())
	{
		if (TStrongObjectPtr<const UGeometryCollection> RestCollection = FractureSettings->RestCollection.Pin())
		{
			// we need a modifiable object
			if (UGeometryCollection* EditableRestCollection = const_cast<UGeometryCollection*>(RestCollection.Get()))
			{
				// Recreate the context for now
 				DataflowContext = MakeShared<UE::Dataflow::FEngineContext>(EditableRestCollection);
			}
		}
	}
}
...
void FFractureEditorModeToolkit::RefreshVariableOverrideDetailView(const UGeometryCollection* RestCollection)
{
	...
	if (const IDataflowInstanceInterface* Interface = EditableRestCollection)
	{
		...
		if (!DataflowContext.IsValid() || !IsValid(DataflowContext->Owner) || DataflowContext->Owner != EditableRestCollection)
		{
			DataflowContext = MakeShared<UE::Dataflow::FEngineContext>(EditableRestCollection);
		}
	}
...
FReply FFractureEditorModeToolkit::OnDataflowOverridesUpdateAsset()
{
	...
	{
		if (FractureEditorModeToolkit::Private::bUseNewDataflowEvaluator)
		{
			/*auto OnEvaluationCompleted = [DataflowContext = DataflowContext](UE::Dataflow::FContext&)
				{
					if (DataflowContext)
					{
						DataflowContext->UpdateAssetFromTerminalNodes(true);
					}
				};*/
			DataflowEvaluator.Reset();
			if (DataflowContext)
			{
				DataflowEvaluator = DataflowContext->EvaluateGraph<UE::Dataflow::FDataflowTaskGraphEvaluator>(/*OnEvaluationCompleted*/{});
				if (!DataflowEvaluator.IsValid())
				{
					UE_LOGF(LogFractureTool, Error, "FractureMode: Failed to start evaluating Dataflow graph for geometry collection %ls", *FractureSettings->RestCollection->GetFName().ToString());
				}
			}
		}
		else
...
void FFractureEditorModeToolkit::Init(const TSharedPtr<IToolkitHost>& InitToolkitHost, TWeakObjectPtr<UEdMode> InOwningMode)
{
	...
	TSharedPtr<SVerticalBox> DataflowOverrideWidget =
		...
		.AutoHeight()
		[
			DataflowProgressBar.ToSharedRef()
		]
		+ SVerticalBox::Slot()
		.AutoHeight()
		[
...
void FFractureEditorModeToolkit::BuildToolPalette(FName PaletteIndex, class FToolBarBuilder& ToolbarBuilder) 
{
	...
	if (PaletteIndex == FractureEditorModeToolkit::FavoritesTools)
	{
		...
		for (FString ToolName : UISettings->ToolFavorites)
		{
			...
			{
				UE_LOGF(LogFractureTool, Display, "FractureMode: could not find Favorited Tool %ls", *ToolName);
			}
		}
	}
	...
	else if (PaletteIndex == FractureEditorModeToolkit::UtilitiesTools)
	{
		...
		ToolbarBuilder.AddToolBarButton(Commands.SplitIslands);
	...
	if (Component)
	{
		...
		if (RestCollection && IsValidChecked(RestCollection))
		{
			...
			if (!GeometryCollection->HasAttribute("Volume", FTransformCollection::TransformGroup))
			{
				...
				UE_LOGF(LogFractureTool, Warning, "Added Volume attribute to GeometryCollection.");
			}
...
void FFractureEditorModeToolkit::CreateDataflowProgressBar()
{
	const FSlateBrush* OverlayBrush = FAppStyle::Get().GetBrush("EditorViewport.OverlayBrush");

	auto GetBarVisibility = [this]() ->EVisibility
		{
			const bool bVisible = DataflowEvaluator ? DataflowEvaluator->IsRunning() : false;
	...
	auto GetProgressText = [this]() -> FText
		{
			...
			{
				return FText::Format(
					LOCTEXT("DataflowGraphEditor_ProgressText", "Total Progress : {0}% - {1}")
					, DataflowEvaluator->GetProgress() * 100.f
					, DataflowEvaluator->GetProgressMessage()
				);
			}
	...
	auto GetProgressPercent = [this]() -> float
		{
			return DataflowEvaluator ? DataflowEvaluator->GetProgress() : 0.f;
		};
	...
	SAssignNew(DataflowProgressBar, SHorizontalBox)
