...
namespace FractureToolBrickLocals
{
	...
	static FVector GetBrickDimensions(const UFractureBrickSettings* BrickSettings, const FVector& Extents)
	{
		...
		if (NumBricks > NumBricksLimit)
		{
			...
			UE_LOGF(LogFractureTool, Warning, "Brick Fracture: Current brick dimensions of %f x %f x %f would result in %lld bricks. "
				"Reduced brick dimensions to %f x %f x %f resulting in %lld bricks to stay within maximum number of %lld bricks.",
...
public:
	...
	virtual void CalculateResult(FProgressCancel* Progress) override
	{
		...
		ResultGeometryIndex = CutMultipleWithPlanarCells(Cells, *CollectionCopy, Selection, Grout, PointSpacing, Seed, Transform, true, true, Progress, CellsOrigin, FIslandSplitSettings(bSplitIslands, CloseVertexDistance, VertexToSurfaceBridgeDistance));
...
int32 UFractureToolBrick::ExecuteFracture(const FFractureToolContext& FractureContext)
{
	if (FractureContext.IsValid())
	{
		...
		BrickOp->CloseVertexDistance = CutterSettings->CloseVertexDistance;
		BrickOp->VertexToSurfaceBridgeDistance = CutterSettings->VertexToSurfaceBridgeDistance;
