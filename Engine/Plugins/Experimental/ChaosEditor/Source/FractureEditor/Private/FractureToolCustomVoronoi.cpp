...
void UFractureToolCustomVoronoi::GenerateLivePattern(const TArray<FFractureToolContext>& FractureContexts, int32 RandomSeed)
{
	...
	{
		auto HasMesh = [](AStaticMeshActor* ReferenceMesh) -> bool
		{
			if (ReferenceMesh == nullptr)
			{
				return false;
			}
			const UStaticMeshComponent* Component = ReferenceMesh->GetStaticMeshComponent();
		...
		AStaticMeshActor* ReferenceMesh = CustomVoronoiSettings->ReferenceMesh.Get();
		if (HasMesh(ReferenceMesh))
		{
			FTransform VerticesTransform = ReferenceMesh->GetStaticMeshComponent()->GetComponentTransform();
			if (!CustomVoronoiSettings->bStartAtActor)
			{
				VerticesTransform.SetLocation(GizmoTransform.GetLocation());
			}

			FMeshDescription* MeshDescription = ReferenceMesh->GetStaticMeshComponent()->GetStaticMesh()->GetMeshDescription(0);
