...
void UFractureToolVoronoiCutterBase::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	if (CutterSettings->bDrawSites)
	{
		...
		{
			PDI->DrawPoint(VoronoiSites[Idx] + ExplodedVector, FLinearColor(.62f, .94f, .84f), 4.f, SDPG_Foreground);
		});
	}
...
void UFractureToolVoronoiCutterBase::AddLineVisualizations(TArray<FCellDiagramResult>& DiagramResults)
{
	...
	{
		UE_LOGF(LogFractureTool, Warning, "Voronoi diagram(s) number of sites too large; will not display Voronoi diagram edges");
		RestoreEditorViewFlags();
		return;
	}
...
void UFractureToolVoronoiCutterBase::UpdateVisualizations(TArray<FFractureToolContext>& FractureContexts)
{
	...
	for (FFractureToolContext& FractureContext : FractureContexts)
	{
		...
		{
			UE_LOGF(LogFractureTool, Warning, "Voronoi diagram(s) number of sites too large; will not display Voronoi diagram sites");
			ClearVisualizations();
			break;
		}
...
public:
	...
	virtual void CalculateResult(FProgressCancel* Progress) override
	{
		...
		FProgressCancel::FProgressScope VoronoiDiagramProgress =
			FProgressCancel::CreateScopeTo(Progress, .01f, LOCTEXT("ComputingVoronoiDiagramMessage", "Computing Voronoi Diagram"));
		...
		ResultGeometryIndex = CutMultipleWithPlanarCells(VoronoiPlanarCells, *CollectionCopy, Selection, Grout, PointSpacing, Seed, Transform, true, true, Progress, Origin, FIslandSplitSettings(bSplitIslands, CloseVertexDistance, VertexToSurfaceBridgeDistance));
...
int32 UFractureToolVoronoiCutterBase::ExecuteFracture(const FFractureToolContext& FractureContext)
{
	if (FractureContext.IsValid())
	{
		...
		VoronoiOp->CloseVertexDistance = CutterSettings->CloseVertexDistance;
		VoronoiOp->VertexToSurfaceBridgeDistance = CutterSettings->VertexToSurfaceBridgeDistance;
