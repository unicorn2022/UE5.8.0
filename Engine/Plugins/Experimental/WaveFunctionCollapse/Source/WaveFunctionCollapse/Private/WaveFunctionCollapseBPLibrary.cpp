...
float UWaveFunctionCollapseBPLibrary::CalculateShannonEntropy(const TArray<FWaveFunctionCollapseOption>& Options, UWaveFunctionCollapseModel* WFCModel)
{
	if (Options.IsEmpty())
	{
		UE_LOGF(LogWFC, Display, "Cannot calculate shannon entropy because the options are empty.");
		return -1;
	}
	...
	if (SumWeights == 0)
	{
		UE_LOGF(LogWFC, Display, "Cannot calculate shannon entropy because the sum of weights equals zero.");
		return -1;
	}
	...
	const TArray<FSoftObjectPath>& IgnoreRotationAssets)
{
	...
	for (int32 InputActorIndex = 0; InputActorIndex < InputActors.Num(); InputActorIndex++)
	{
		...
		if (InputStaticMeshActor)
		{
			...
			if (!InputStaticMesh)
			{
				UE_LOGF(LogWFC, Display, "%ls contains invalid StaticMesh, skipping.", *InputStaticMeshActor->GetFName().ToString());
				continue;
			}
			...
			{
				UE_LOGF(LogWFC, Display, "%ls is in an overlapping position, skipping.", *InputStaticMeshActor->GetFName().ToString());
				continue;
			}
...
bool UWaveFunctionCollapseBPLibrary::GetPositionToOptionMapFromActor(AActor* Actor, float TileSize, UPARAM(ref) TMap<FIntVector, FWaveFunctionCollapseOption>& PositionToOptionMap)
{
	...
	if (!Actor)
	{
		UE_LOGF(LogWFC, Display, "GetPositionToOptionMapFromActor called with a null Actor");
		return false;
	}
	...
	for (int32 ISMComponentIndex = 0; ISMComponentIndex < ISMComponents.Num(); ISMComponentIndex++)
	{
		...
		if (!InputStaticMesh)
		{
			UE_LOGF(LogWFC, Display, "%ls.%ls contains invalid StaticMesh, skipping.", *Actor->GetFName().ToString(), *ISMComponent->GetFName().ToString());
			continue;
		}
		else
		{
			for (int32 InstanceIndex = 0; InstanceIndex < ISMComponent->GetInstanceCount(); InstanceIndex++)
			{
				...
				if (TmpPositionToOptionMap.Contains(CurrentGridPosition))
				{
					UE_LOGF(LogWFC, Display, "%ls.%ls Index %d is in an overlapping position, skipping.", *Actor->GetFName().ToString(), *ISMComponent->GetFName().ToString(), InstanceIndex);
					continue;
				}
