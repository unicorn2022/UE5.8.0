...
AActor* UWaveFunctionCollapseSubsystem::Collapse(int32 TryCount /* = 1 */, int32 RandomSeed /* = 0 */)
{
	if (!WFCModel)
	{
		UE_LOGF(LogWFC, Error, "Invalid WFC Model");
		return nullptr;
	}
	
	UE_LOGF(LogWFC, Display, "Starting WFC - Model: %ls, Resolution %dx%dx%d", *WFCModel->GetFName().ToString(), Resolution.X, Resolution.Y, Resolution.Z);
	...
	if (TryCount > 1)
	{
		...
		{
			CurrentTry += 1;
			UE_LOGF(LogWFC, Warning, "Failed with Seed Value: %d. Trying again.  Attempt number: %d", ChosenRandomSeed, CurrentTry);
	...
	{
		UE_LOGF(LogWFC, Error, "Invalid TryCount on Collapse: %d", TryCount);
		return nullptr;
	}
	...
	if (bSuccessfulSolve)
	{
		...
		UE_LOGF(LogWFC, Display, "Success! Seed Value: %d. Spawned Actor: %ls", ChosenRandomSeed, *SpawnedActor->GetActorLabel());
		return SpawnedActor;
	}
	else
	{
		UE_LOGF(LogWFC, Error, "Failed after %d tries.", TryCount);
		return nullptr;
	}
}
...
void UWaveFunctionCollapseSubsystem::InitializeWFC(TArray<FWaveFunctionCollapseTile>& Tiles, TArray<int32>& RemainingTiles)
{
	...
	{
		UE_LOGF(LogWFC, Error, "Could not create Initial Tile from Model");
	}
		...
		for (TPair<int32, FWaveFunctionCollapseQueueElement>& ObservationAdjacenctElement : ObservationQueue)
		{
			...
			if (bAddToPropagationQueue)
			{
				...
				else
				{
					...
					UE_LOGF(LogWFC, Error, "Encountered Contradiction on Index %d", ObservationAdjacenctElement.Key);
					return false;
				}
			}
		}
...
AActor* UWaveFunctionCollapseSubsystem::SpawnActorFromTiles(const TArray<FWaveFunctionCollapseTile>& Tiles)
{
	...
	for (int32 index = 0; index < Tiles.Num(); index++)
	{
		...
		if (LoadedObject)
		{
			...
			{
				UE_LOGF(LogWFC, Warning, "Invalid Type, skipping: %ls", *BaseObject.ToString());
			}
		}
		else
		{
			UE_LOGF(LogWFC, Warning, "Unable to load object, skipping: %ls", *BaseObject.ToString());
		}
	}
...
void UWaveFunctionCollapseSubsystem::DeriveGridFromTransformBounds(const TArray<FTransform>& Transforms)
{
	...
	{
		UE_LOGF(LogWFC, Error, "Empty Transform Array.");
		return;
	}
...
void UWaveFunctionCollapseSubsystem::DeriveGridFromTransforms(const TArray<FTransform>& Transforms)
{
	...
	{
		UE_LOGF(LogWFC, Display, "StartOption at: %d,%d,%d", StarterOption.Key.X, StarterOption.Key.Y, StarterOption.Key.Z);
	}
}
