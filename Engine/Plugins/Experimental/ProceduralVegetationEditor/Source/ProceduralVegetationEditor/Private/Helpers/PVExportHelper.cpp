...
#include "ProceduralVegetation/Private/Helpers/PVDistributionHelper.h"
...
namespace PV::Export::Internal
{
	...
	UPackage* CreateAssetPackage(const FString& LongPackageName)
	{
		...
		// LoadPackage can return an in-memory UPackage stub even when no file exists on disk
		// (e.g. left behind by a prior failed LoadObject on the same path), so it isn't a
		// reliable signal that the asset is already saved. DoesPackageExist consults disk truth.
	...
	bool IsEngineContentPath(const FString& InPackageOrObjectPath)
	{
		return InPackageOrObjectPath.StartsWith(TEXT("/Engine/"));
	}

	// Resolves the parent package path under which a generated companion mesh
	// for FoliageInstancePackagePath should live.
	//   - Non-engine source: original long package path (sibling to the source asset).
	//   - Engine source:     "<ExportMeshParent>/<ExportMeshShortName>_NaniteParts/Engine/<rest of source dir>".
	//                        Engine content is read-only on disk under Perforce, so the companion
	//                        mesh must be rebased into the user's project. The "Engine/" segment is
	//                        preserved so the origin is visible in the Content Browser and so
	//                        same-named assets in different engine folders do not collide.
	FString ResolveGeneratedMeshParentPath(
		const FString& FoliageInstancePackagePath,
		const FString& ExportMeshPackageName)
	{
		const FString LongFoliagePackagePath = FPackageName::GetLongPackagePath(FoliageInstancePackagePath);

		if (!IsEngineContentPath(FoliageInstancePackagePath))
		{
			return LongFoliagePackagePath;
		}

		const FString ExportParent    = FPackageName::GetLongPackagePath(ExportMeshPackageName);
		const FString ExportShortName = FPackageName::GetShortName(ExportMeshPackageName);

		FString RelativeEngineDir = LongFoliagePackagePath;
		RelativeEngineDir.RemoveFromStart(TEXT("/"));

		return FPaths::Combine(ExportParent, ExportShortName + TEXT("_NaniteParts"), RelativeEngineDir);
	}

	void DeletePackage(UPackage* InPackage)
	{
		TArray<UObject*> PackageObjects;
		GetObjectsWithPackage(InPackage, PackageObjects, EGetObjectsFlags::None);
		for (UObject* ExistingObject : PackageObjects)
		{
			ExistingObject->MarkAsGarbage();
			IAssetRegistry::Get()->AssetDeleted(ExistingObject);
		}
	}
 
	void ReplaceAssetInPackage(const TObjectPtr<UObject>& NewObject)
	{
		UPackage* Package = NewObject->GetPackage();
		
		const FString ObjectName = FPackageName::GetShortName(Package->GetName());
		TArray<UObject*> PackageObjects;
		GetObjectsWithPackage(Package, PackageObjects, EGetObjectsFlags::None);
		PackageObjects = PackageObjects.FilterByPredicate(
			[&](UObject* Object)
			{
		...
		for (UObject* ExistingObject : PackageObjects)
		{
			// Mimic ObjectTools::DeleteSingleObject. 
			// Deselect the old asset, close any open editors for the asset,
			// notify the asset registry, and clear relevant flags on object.
			//
			// Additionally call ObjectTools::ForceReplaceReferences to ensure
			// references to ExistingObject gets updated in the engine, and finally
			// rename the object so that we can rename the NewObject to the same location
			// as the old asset.

			if (GEditor)
			{
				GEditor->GetSelectedObjects()->Deselect(ExistingObject);

				if (UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
				{
					AssetEditorSubsystem->CloseAllEditorsForAsset(ExistingObject);
				}
			}

			// Technically shouldn't be possible for ExistingObject class to differ from NewObject class.
			// However, we check here as ObjectTools::ForceReplaceReferences only makes sense for objects of the same type.
			// We want to delete all other objects regardless.
			if (ExistingObject->IsA(NewObject->GetClass())) 
			{
				TArray<UObject*> ObjectsToReplace = { ExistingObject };
				ObjectTools::ForceReplaceReferences(NewObject, ObjectsToReplace);
			}
			...
			// Remove the deleted object out of the package and mark as garbage
			const FString NewName = MakeUniqueObjectName(GetTransientPackage(), ExistingObject->GetClass(), *(ExistingObject->GetName() + "_DEADOBJECT")).ToString();
			ExistingObject->Rename(*NewName, GetTransientPackage(), REN_DontCreateRedirectors | REN_DoNotDirty | REN_NonTransactional);
			ExistingObject->MarkAsGarbage();
		}

		NewObject->Rename(*ObjectName, Package, REN_DontCreateRedirectors | REN_DoNotDirty | REN_NonTransactional);

		// If this is a newly created asset then the asset registry will be notified by ExportCollectionAsMesh.
		// If not then we notify asset registry manually here to ensure it's aware of the swapped in asset.
		if (!Package->HasAnyPackageFlags(PKG_NewlyCreated)) 
		{
			IAssetRegistry::Get()->AssetCreated(NewObject);
		}
	}
	...
	TObjectPtr<UDynamicMesh> CollectionToDynamicMesh(const FManagedArrayCollection& Collection)
	{
		...
		ConvertGeometryCollectionToDynamicMesh(CombinedMesh, TransformOut, bCenterPivot, *GeomCollection, false, BoneTransforms.GetConstArray(), true, TransformIndices);
	...
	TArray<TObjectPtr<UMaterialInterface>> GetMaterialsFromCollection(const FManagedArrayCollection& Collection)
	{
		...
		if (Collection.HasAttribute("MaterialPath", FGeometryCollection::MaterialGroup))
		{
			...
			{
				Materials.Add(LoadObject<UMaterialInterface>(nullptr, *Path, {}, LOAD_NoWarn | LOAD_Quiet));
			}
		}
		return Materials;
	}
	...
	)
	{
		...
		TMap<FString, int32> FoliageNameToDynamicMeshResult;
		{
			...
			for (int32 FoliageNameIndex = 0; FoliageNameIndex < NumFoliageNames; ++FoliageNameIndex)
			{
				...
				{
					UE_LOGF(LogProceduralVegetationEditor, Warning, "Duplicate foliage name %ls in foliage facade", *FoliageName);
					continue;
				}
			...
			ParallelFor(ConvertFoliageToDynamicMeshResults.Num(), [&ConvertFoliageToDynamicMeshResults](int32 Index)
			{
				...
				if (const TObjectPtr<UStaticMesh> StaticMesh = LoadObject<UStaticMesh>(nullptr, *FoliageName, {}, LOAD_NoWarn | LOAD_Quiet))
				{
				...
				else if (const TObjectPtr<USkeletalMesh> SkeletalMesh = LoadObject<USkeletalMesh>(nullptr, *FoliageName, {}, LOAD_NoWarn | LOAD_Quiet))
				{
	...
	)
	{
		...
		for (const FString& FoliageInstanceObjectPath : UsedFoliage)
		{
			...
			TObjectPtr<UStaticMesh> PartStaticMesh = LoadObject<UStaticMesh>(nullptr, *FoliageInstanceObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
			if (PartStaticMesh)
			{
			...
			const FString LongFoliagePackagePath = Internal::ResolveGeneratedMeshParentPath(
				FoliageInstancePackagePath, StaticMesh->GetOutermost()->GetName());
			...
			PartStaticMesh = LoadObject<UStaticMesh>(nullptr, StaticMeshObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
			if (PartStaticMesh)
			{
				const TObjectPtr<USkeletalMesh> SourceSkeletalMesh = LoadObject<USkeletalMesh>(nullptr, *FoliageInstanceObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
				if (SourceSkeletalMesh)
				{
					const FString SkeletalMeshFile = FPackageName::LongPackageNameToFilename(SourceSkeletalMesh->GetPackage()->GetName(), FPackageName::GetAssetPackageExtension());
					const FString StaticMeshFile = FPackageName::LongPackageNameToFilename(PartStaticMesh->GetPackage()->GetName(), FPackageName::GetAssetPackageExtension());
					const FDateTime SkeletalMeshTime = IFileManager::Get().GetTimeStamp(*SkeletalMeshFile);
					const FDateTime StaticMeshTime = IFileManager::Get().GetTimeStamp(*StaticMeshFile);
					if (SkeletalMeshTime > StaticMeshTime)
					{
						// Source skeletal mesh was modified after the static mesh was generated; fall through to regenerate
						PartStaticMesh = nullptr;
					}
				}
			...
			const TObjectPtr<USkeletalMesh> PartSkeletalMesh = LoadObject<USkeletalMesh>(nullptr, *FoliageInstanceObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
			if (!PartSkeletalMesh)
			{
				UE_LOGF(LogProceduralVegetationEditor, Warning, "Could not find foliage instance %ls", *FoliageInstanceObjectPath);
				continue;
			}
			...
			{
				UE_LOGF(LogProceduralVegetationEditor, Warning, "Failed to convert foliage instance \"%ls\" to dynamic mesh", *FoliageInstancePackagePath);
				continue;
			}
			...
			const FGeometryScriptCopyMeshToAssetOptions Options = GetCopyMeshToAssetOptions(
				PartMaterials, 
				PartStaticMesh->GetNaniteSettings(), 
				NaniteVoxelsSupported() ? ENaniteShapePreservation::Voxelize : ENaniteShapePreservation::PreserveArea
			);
			...
			{
				OutModifiedPackages.Remove(StaticMeshPackage);
				PartStaticMesh->MarkAsGarbage();
				StaticMeshPackage->MarkAsGarbage();
				continue;
			}
	...
	)
	{
		...
		for (const FString& FoliageInstanceObjectPath : UsedFoliage)
		{
			...
			TObjectPtr<USkeletalMesh> PartSkeletalMesh = LoadObject<USkeletalMesh>(nullptr, *FoliageInstanceObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
			if (PartSkeletalMesh)
			{
			...
			const FString FoliageInstancePackagePath = FPackageName::ObjectPathToPackageName(FoliageInstanceObjectPath);
			const FString LongFoliagePackagePath = Internal::ResolveGeneratedMeshParentPath(
				FoliageInstancePackagePath, SkeletalMesh->GetOutermost()->GetName());
			const FString ShortFoliagePackageName = FPackageName::GetShortName(FoliageInstancePackagePath);
			...
			PartSkeletalMesh = LoadObject<USkeletalMesh>(nullptr, SkeletalMeshObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
			TObjectPtr<USkeleton> Skeleton = LoadObject<USkeleton>(nullptr, SkeletonObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
			if (PartSkeletalMesh && Skeleton)
			{
				const TObjectPtr<UStaticMesh> SourceStaticMesh = LoadObject<UStaticMesh>(nullptr, *FoliageInstanceObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
				if (SourceStaticMesh)
				{
					const FString StaticMeshFile = FPackageName::LongPackageNameToFilename(SourceStaticMesh->GetPackage()->GetName(), FPackageName::GetAssetPackageExtension());
					const FString SkeletalMeshFile = FPackageName::LongPackageNameToFilename(PartSkeletalMesh->GetPackage()->GetName(), FPackageName::GetAssetPackageExtension());
					const FDateTime StaticMeshTime = IFileManager::Get().GetTimeStamp(*StaticMeshFile);
					const FDateTime SkeletalMeshTime = IFileManager::Get().GetTimeStamp(*SkeletalMeshFile);
					if (StaticMeshTime > SkeletalMeshTime)
					{
						// Source static mesh was modified after the skeletal mesh was generated; fall through to regenerate
						PartSkeletalMesh = nullptr;
					}
				}
			}

			const auto DeleteNewlyCreatedPackage = [&](UPackage* InPackage)
			{
				if (InPackage && OutModifiedPackages.Remove(InPackage) > 0)
				{
					DeletePackage(InPackage);
				}
			};
 
			UPackage* SkeletonPackage = nullptr;
			if (!Skeleton)
			{
				SkeletonPackage = Internal::CreateAssetPackage(*SkeletonPackagePath);
			...
			if (!PartSkeletalMesh)
			{
				const TObjectPtr<UStaticMesh> PartStaticMesh = LoadObject<UStaticMesh>(nullptr, *FoliageInstanceObjectPath, {}, LOAD_NoWarn | LOAD_Quiet);
				if (!PartStaticMesh)
				{
					DeleteNewlyCreatedPackage(SkeletonPackage);
					UE_LOGF(LogProceduralVegetationEditor, Warning, "Could not find foliage instance %ls", *FoliageInstanceObjectPath);
					continue;
				}
				...
				if (!ConvertToDefaultSkeletalMesh(PartSkeletalMesh, PartDynamicMesh, PartMaterials))
				{
					UE_LOGF(LogProceduralVegetationEditor, Warning, "Failed to create part skeletal mesh %ls.", *PartSkeletalMesh->GetFullName());
					DeleteNewlyCreatedPackage(SkeletalMeshPackage);
					DeleteNewlyCreatedPackage(SkeletonPackage);
					continue;
				}
			}
	...
	bool ConvertToDefaultSkeletalMesh(USkeletalMesh* SkeletalMesh, FDynamicMesh3& Mesh, const TArray<TObjectPtr<UMaterialInterface>>& Materials)
	{
	...
	)
	{
		...
		if (bBuildNaniteAssemblies && bShouldExportFoliage)
		{
			...
			{
				UE_LOGF(LogProceduralVegetationEditor, Warning, "Failed to build Nanite Assemblies, because neither Nanite Foliage nor Nanite Assemblies are enabled for the project.");
			}
		}
	...
	)
	{
		...
		int32 FoliageVertexStart = 0;
		{
			...
			for (const int32& PointID : VertexPointIDs)
			{
				...
				else
				{
					...
					UE_LOGF(LogProceduralVegetationEditor, Log, "Invalid Bone Assigned to PointID %i", PointID);
				}
			}
 ...
 
		{
			...
			{
				UE_LOGF(LogProceduralVegetationEditor, Warning, "Failed to build wind data");
			}
		}
	...
	void AssignBoneIDsToFoliage(const TArray<Facades::FBoneNode>& Bones, FManagedArrayCollection& Collection)
	{
		...
		for(int32 Id = 0; Id < NumInstances; Id++)
		{
			...
			if (!bBoneIDAssigned)
			{
				UE_LOGF(LogProceduralVegetationEditor, Warning, "No bone assigned for foliage Id {%i} , Branch Bone count {%i}", Id , BoneIDs.Num());
			}
		}
	}
	...
	void RemoveUnwantedSkinWeights(TObjectPtr<UDynamicMesh> DynamicMesh, const FName ProfileName, const TArray<int32>& VertexBoneIDs, int32 FoliageVertexStart, const TArray<Facades::FBoneNode>& BoneNodes)
	{
		...
		DynamicMesh->EditMesh([&](FDynamicMesh3& EditMesh)
		{
			...
			ParallelFor(NumVertices, [&](const int32 VertexIdx)
			{
				...
				{
					UE_LOGF(LogProceduralVegetationEditor, Log, "Invalid bone index found while removing bones for vertex %i", VertexIdx);
				}
			});
	...
	void SetupPhysicAsset(const FManagedArrayCollection& Collection, const EPVCollisionGeneration& CollisionGeneration,UPhysicsAsset* PhysicsAsset, TArray<Facades::FBoneNode> InBones)
	{
		if (!PhysicsAsset)
		{
			UE_LOGF(LogProceduralVegetationEditor, Warning, "No physics asset specified to create physics bodies");
			return;
		}
		...
		for (const Facades::FBoneNode&  Node : ReducedBones)
		{
			...
			if (!MappedBone)
			{
				UE_LOGF(LogProceduralVegetationEditor, Log, "MappedBone not found for BodySetup");
				continue;
			}
 
			if (!InBones.IsValidIndex(MappedBone->ParentBoneIndex))
			{
				UE_LOGF(LogProceduralVegetationEditor, Log, "MapedBone not found for BodySetup");
				continue;
			}
...
namespace PV::Export
{
	...
	)
	{
		...
		Internal::EGetValidAssetPathResult AssetPathResult;
		
		FString MeshName = ExportParams.MeshName.ToString();
		FString MeshPackageName = ExportParams.GetOutputMeshPackagePath();
		AssetPathResult = Internal::GetValidAssetPath(ExportParams.ReplacePolicy, ExportParams.GetMeshClass(), MeshPackageName, MeshName);
		...
		const bool bShouldExportFoliage = ExportEntry->bExportFoliage;
 
		if (ExportParams.ExportMeshType == EPVExportMeshType::StaticMesh)
		{
			const TObjectPtr<UStaticMesh> ExportMesh = NewObject<UStaticMesh>(Internal::CreateAssetPackage(MeshPackageName), NAME_None, RF_Public | RF_Standalone);
			...
			{
				ExportMesh->MarkAsGarbage();
				return ExitWithResult(EExportResult::Cancelled);
			}
 
			Internal::ReplaceAssetInPackage(ExportMesh);
		...
		else if (ExportParams.ExportMeshType == EPVExportMeshType::SkeletalMesh)
		{
			...
			const TObjectPtr<USkeletalMesh> ExportMesh = NewObject<USkeletalMesh>(Internal::CreateAssetPackage(MeshPackageName), NAME_None, RF_Standalone | RF_Public);
			const TObjectPtr<USkeleton> ExportSkeleton = NewObject<USkeleton>(Internal::CreateAssetPackage(SkeletonPackageName), NAME_None, RF_Standalone | RF_Public);
			...
			AssetPathResult = Internal::GetValidAssetPath(ExportParams.ReplacePolicy, UPhysicsAsset::StaticClass(), PhysicsPackageName, PhysicsAssetName);
			if (AssetPathResult != Internal::EGetValidAssetPathResult::Success)
			{
				return ExitWithAssetPathResult(AssetPathResult);
			}
			...
			{
				PhysicsAsset = NewObject<UPhysicsAsset>(Internal::CreateAssetPackage(PhysicsPackageName), NAME_None, RF_Standalone | RF_Public);
				ExportMesh->SetPhysicsAsset(PhysicsAsset);
				PhysicsAsset->SetPreviewMesh(ExportMesh, false);
			}
			...
			{
				ExportMesh->SetSkeleton(nullptr);
				if (PhysicsAsset)
				{
					ExportMesh->SetPhysicsAsset(nullptr);
					PhysicsAsset->MarkAsGarbage();
				}
				
				ExportSkeleton->MarkAsGarbage();
				ExportMesh->MarkAsGarbage();
			...
			if (PhysicsAsset)
			{
				Internal::ReplaceAssetInPackage(PhysicsAsset);
				ModifiedPackages.Add(PhysicsAsset->GetOutermost());
 
				OutCreatedAssets.Add(PhysicsPackageName);
				Internal::AttachProceduralVegetationLink(PhysicsAsset, InProceduralVegetation);
			}
			
			Internal::ReplaceAssetInPackage(ExportSkeleton);
			Internal::ReplaceAssetInPackage(ExportMesh);
			ModifiedPackages.Add(ExportMesh->GetOutermost());
