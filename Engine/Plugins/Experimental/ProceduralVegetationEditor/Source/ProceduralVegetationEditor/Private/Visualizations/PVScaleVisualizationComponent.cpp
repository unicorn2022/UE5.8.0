...
void UPVScaleVisualizationComponent::UpdateScaleVisualizations()
{
	...
	
	{
		...
		TextRenderComponent->SetMaterial(0, LoadObject<UMaterialInterface>(nullptr, TEXT("/ProceduralVegetationEditor/Materials/ScaleTextMaterial.ScaleTextMaterial")));
		TextRenderComponent->SetText(FText::FromString(FString::Printf(TEXT("%.2fm"), Length)));

		AddLine(StartPosition, EndPosition, FLinearColor::Red, ESceneDepthPriorityGroup::SDPG_World);
	}

	{
		...
		AddLine(StartPosition, EndPosition, FLinearColor::Green, ESceneDepthPriorityGroup::SDPG_World);
	}

	{
		...
		AddLine(StartPosition, EndPosition, FLinearColor::Blue, ESceneDepthPriorityGroup::SDPG_World);
	}
}
