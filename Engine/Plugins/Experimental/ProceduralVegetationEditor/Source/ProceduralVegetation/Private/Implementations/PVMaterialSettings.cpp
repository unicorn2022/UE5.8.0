...
void FPVMaterialSettings::SetMaterial(UMaterialInterface* Material, FVector2f UMinMax, int32 Index)
{
	FTrunkGenerationMaterialSetup MaterialSetup;
	MaterialSetup.Material = Material;
...
{
	UMaterialInterface* Material = LoadObject<UMaterialInterface>(nullptr, TrunkMaterial, {}, LOAD_NoWarn | LOAD_Quiet);
	SetMaterial(Material, UMinMax, Index);
}
...
void FPVMaterialSettings::SetBranchUVMaterial(FManagedArrayCollection& Collection, TArray<int32>& UsedMaterials, TArray<FBranchUVData>& BranchUVData) const
{
	...
	for (int32 PointIndex = 0; PointIndex < PointFacade.GetElementCount(); PointIndex++)
	{
		...
		const PV::FBudDevelopmentView BudDevelopmentView(BudDevelopment);
		
		float PScale = PointFacade.GetPointScale(PointIndex);

		MinAge = FMath::Min(MinAge, BudDevelopmentView.BranchAge);
		MinPScale = FMath::Min(MinPScale, PScale);

		MaxAge = FMath::Max(MaxAge, BudDevelopmentView.BranchAge);
		MaxPScale = FMath::Max(MaxPScale, PScale);
	}
	
	for (int32 BranchIndex = 0; BranchIndex < BranchFacade.GetElementCount(); BranchIndex++)
	{
		const int32 GenerationNumber = PV::Facades::FTreeFacade::GetBranchGenerationNumber(Collection, BranchIndex);
	...
	for (int32 BranchIndex = 0; BranchIndex < BranchFacade.GetElementCount(); BranchIndex++)
	{
		...
		TArray<int32> BudDevelopment = PV::AttributesHelper::GetBudDevelopment(
			BudDevelopmentAttribute, BranchPointsAttribute, BranchParentNumberAttribute, BranchIndex, 0);
		
		const PV::FBudDevelopmentView BudDevelopmentView(BudDevelopment);

		float PScale = PV::AttributesHelper::GetBranchPointScale(PointScaleAttribute, BranchPointsAttribute, BranchParentNumberAttribute, BranchIndex, 0);
		...
		{
			BranchMaterialVal = GetMaterialValue(BudDevelopmentView.BranchAge, MinAge, MaxAge, MaxGeneration, MinGeneration);
		}
...
void FPVMaterialSettings::SetBaseUVs(FManagedArrayCollection& Collection, TArray<FPointUVData>& PointUVData) const
{
	...
	{
		UE_LOGF(LogProceduralVegetation, Warning, "PointScales Array is empty when setting Base UVs.");
		return;
	}
	
	const float MaxPointScale = *Algo::MaxElement(PointScales);

	if(MaxPointScale <= 0.0)
	{
		UE_LOGF(LogProceduralVegetation, Warning, "MaxPointScale is zero when setting Base UVs.");
		return;
	}
...
void FPVMaterialSettings::SetOutputUVs(FManagedArrayCollection& Collection, TArray<FPointUVData>& PointUVData, TArray<FBranchUVData>& BranchUVData) const
{
	PV::Facades::FBranchFacade BranchFacade = PV::Facades::FBranchFacade(Collection);
	PV::Facades::FPointFacade PointFacade = PV::Facades::FPointFacade(Collection);
	PV::Facades::FPlantFacade PlantFacade = PV::Facades::FPlantFacade(Collection);
	...
	for (int BranchIndex = 0; BranchIndex < BranchFacade.GetElementCount(); BranchIndex++ )
	{
		...
		const bool IsTrunk = PlantFacade.IsTrunkIndex(BranchIndex);
		const int StartingIndex = IsTrunk ? 0 : 1;
		
		for (int j= StartingIndex; j < BranchPoints.Num(); j++)
		{
			...
			PointFacade.SetTextureCoordV(BranchPoints[j], UV.Y);
