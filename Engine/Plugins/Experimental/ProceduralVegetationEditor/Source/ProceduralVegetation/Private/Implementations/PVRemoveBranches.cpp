...
namespace PV::Facades
{
	...
	void GatherBranchRemovalData(const FManagedArrayCollection& Collection, const FBranchFacade& InBranchFacade, const FPointFacade& InPointFacade, const FPlantFacade& InPlantFacade,
	                             const ERemoveBranchesBasis InBasis, TArray<float>& OutRemovalData)
	{
		     ...
		     const TPair<int32, int32> Pair : PlantNumbersToTrunkIDs)
		{
			...
			for (const int32 BranchIndex : BranchIndices)
			{
				...
				{
					OutRemovalData[BranchIndex] = PV::AttributesHelper::GetBranchPointScale(PointScaleAttribute, BranchPointsAttribute, BranchParentNumberAttribute, BranchIndex, 0);
				}
...
void FPVRemoveBranches::ApplyRemoveBranches(const ERemoveBranchesBasis InBasis, const float InThreshold, FManagedArrayCollection& OutCollection)
{
	...
	GatherBranchRemovalData(OutCollection, BranchFacade, PointFacade, PlantFacade, InBasis, OutRemovalData);
