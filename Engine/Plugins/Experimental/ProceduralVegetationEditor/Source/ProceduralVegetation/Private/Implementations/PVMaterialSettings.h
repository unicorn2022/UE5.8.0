...
struct FTrunkGenerationMaterialSetup
{
	...
	UPROPERTY(EditAnywhere, Category="Materials", meta=(DisplayName="Use Trunk Texture Setup", Tooltip="When enabled, the Material and X Range for this generation are sourced automatically from the connected Trunk Texture Setup pin, matched by generation index."))
	bool bDeriveFromTrunkTextureSetup = false;

	UPROPERTY(EditAnywhere, Category="Materials", meta=(EditCondition="!bDeriveFromTrunkTextureSetup", EditConditionHides, Tooltip="Material to apply to this generation."))
	TObjectPtr<UMaterialInterface> Material;

	UPROPERTY(EditAnywhere, Category="Materials" , meta=(DisplayName = "Y Texture Mode", Tooltip="How textures align vertically (Default uses native tiling; Fit0_1 stretches once)."))
	...
	UPROPERTY(EditAnywhere, Category="Materials", meta=(DisplayName = "X Range",
		EditCondition="!bDeriveFromTrunkTextureSetup", EditConditionHides,
		Tooltip="Limits horizontal coverage range.\n\nSets the portion of the mesh affected along the X axis, useful for restricting materials (e.g., moss/lichen) to a subsection."))
...
struct FPVMaterialSettings
{
	...
	UPROPERTY(EditAnywhere, Category="Materials", meta=(Tooltip="Repeat or Fit materials across generations.\n\nRepeat: cycle materials when generations exceed material count. Fit: stretch N materials across all generations."))
	...
	UPROPERTY(EditAnywhere, Category="Materials", meta=(Tooltip="Driver for material selection: Generation, Age, or Radius.\n\nGeneration: by branch hierarchy. Age: by segment age. Radius: by branch thickness."))
	...
	void SetMaterial(UMaterialInterface* Material, FVector2f UMinMax, int32 Index);
