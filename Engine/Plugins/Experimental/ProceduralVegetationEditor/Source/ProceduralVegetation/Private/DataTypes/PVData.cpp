...
{
	Collection = MakeShared<FManagedArrayCollection>();
}

void UPVData::Initialize(FManagedArrayCollection&& InCollection)
{
	Collection = MakeShared<FManagedArrayCollection>(MoveTemp(InCollection));
}
...
UPCGSpatialData* UPVData::CopyInternal(FPCGContext* Context) const
{
	...
	NewProceduralVegetationData->Initialize(MoveTemp(NewCollection));
...
void UPVData::AddVisualizationCollection(FManagedArrayCollection InCollection, TArray<FPVVisualizationSettings> InVisualizationSettings)
{
	VisualizationCollections.Emplace(MoveTemp(InCollection), MoveTemp(InVisualizationSettings));
}

void UPVData::SetDebugSettings(const FPVDebugSettings& InSettings) const
{
	const FScopeLock Lock(&DebugSettingsMutex);
	DebugSettings = InSettings;
}

void UPVData::SetDebugSettings(FPVDebugSettings&& InSettings) const
{ 
	const FScopeLock Lock(&DebugSettingsMutex);
	DebugSettings = MoveTemp(InSettings);
}

FPVDebugSettings UPVData::GetDebugSettings() const 
{ 
	const FScopeLock Lock(&DebugSettingsMutex);
