...
namespace PV::Facades
{
	...
	FBoneNode* FindParentBoneForBranch(TArray<FBoneNode>& BoneNodes, FVector BonePosition, float BoneScale,
	                            FString& OutMessage, bool bFallbackToClosestBone = false)
	{
		...
		if (!FoundBone && bFallbackToClosestBone && ClosestBone)
		{
			OutMessage = FString::Printf(TEXT("FindParentBoneForBranch fallback to closest bone Index : %i closest bone Branch %i"), ClosestBone->BoneIndex, ClosestBone->BranchIndex);
			return ClosestBone;
		}
	...
	void SetBranchSimulationGroupIndex(const FManagedArrayCollection& Collection, const FPointFacade& PointFacade, FBranchFacade& BranchFacade)
	{
		...
		for (int32 BranchIdx = 0; BranchIdx < NumBranches; ++BranchIdx)
		{
			...
			float Pscale = PV::AttributesHelper::GetBranchPointScale(PointScaleAttribute, BranchPointsAttribute, BranchParentNumberAttribute, BranchIdx, 0);
	...
	void FBoneFacade::SetWindSimulationData(FManagedArrayCollection& Collection)
	{
		...
		SetBranchSimulationGroupIndex(Collection, PointFacade, BranchFacade);
	}
	...
	bool FBoneFacade::CreateBoneData(TArray<FBoneNode>& Bones,const float ReductionStrength,bool bSetWindData /*= true*/)
	{
		...
		const auto PointScaleAttribute = FPointScaleAttribute::GetAttribute(VertexPointIds.GetConstCollection());
		const auto BranchPointsAttribute = FBranchPointsAttribute::GetAttribute(VertexPointIds.GetConstCollection());
		const auto BranchParentNumberAttribute = FBranchParentNumberAttribute::GetAttribute(VertexPointIds.GetConstCollection());
		const auto NjordPixelIndexAttribute = PV::FPointNjordPixelIndexAttribute::FindAttribute(VertexPointIds.GetConstCollection());

		if (!(Utilities::IsValidGrowthData(VertexPointIds.GetConstCollection()) && PlantFacade.IsValid()))
		{
			return false;	
		}
		 ...
		 const TPair<int32, int32> Pair : PlantNumbersToTrunkIDs)
		{
			...
			const int32 TrunkBoneIndex = CurrentBoneIndex;
			FBoneNode TrunkBoneNode{ "Root", TrunkBoneIndex, RootBoneIndex, RootPointIndex, TrunkIndex, 1.0, FTransform{RootPointPosition}};
			CurrentBoneIndex++;
			Bones.Emplace(MoveTemp(TrunkBoneNode));

			int ParentBoneIndex = TrunkBoneIndex;
			...
			{
				UE_LOGF(LogProceduralVegetation, Log, "Cannot create bones, no branches found for Plant Number {%i}", PlantNumber);
				continue;
			}
			for (int32 i = 0; i < SortedBranchIndices.Num(); ++i)
			{
				...
				FString Message;
				const FVector BranchRootPosition = FVector(Positions[BranchPoints[0]]);
				const float BranchRootScale = PV::AttributesHelper::GetBranchPointScale(PointScaleAttribute, BranchPointsAttribute, BranchParentNumberAttribute, BranchIndex, 0);
				FBoneNode* FoundBoneNode = FindParentBoneForBranch(ParentBoneNodes, BranchRootPosition, BranchRootScale, Message, true);

				if (FoundBoneNode == nullptr && i != 0)
				{
					UE_LOGF(LogProceduralVegetation, Log, "Parent bone not found for branch (%i) ParentIndex {%i}", BranchIndex, ParentIndex);
				}
				
				if (!Message.IsEmpty())
				{
					UE_LOGF(LogProceduralVegetation, Log, "%ls Branch (%i) ParentIndex {%i}", *Message, BranchIndex, ParentIndex);
				}
			
				ParentBoneIndex = FoundBoneNode ? FoundBoneNode->BoneIndex : TrunkBoneIndex;
				...
				for (int Idx = 0; Idx < BranchPoints.Num(); Idx++)
				{
					...
					const float PScale = PV::AttributesHelper::GetBranchPointScale(PointScaleAttribute, BranchPointsAttribute, BranchParentNumberAttribute, BranchIndex, Idx);
					...
					float NjordPixelID = NjordPixelIndexAttribute.IsValidIndex(PointIdx) ? NjordPixelIndexAttribute[PointIdx] : -1;
					...
					for (auto ChildBranch : BranchFacade.GetChildren(BranchIndex))
					{
						...
						{
							UE_LOGF(LogProceduralVegetation, Warning, "Child branch number {%i} not found in facade, for branch {%i}", ChildBranch, BranchFacade.GetBranchNumber(BranchIndex));
						}
					}

					bool IsLastPoint = Idx == BranchPoints.Num() - 1;
				
					//UE_LOGF(LogProceduralVegetation, Warning, "Iterating Branch Points NjordPixelId, Id{%f} PointIndex{%i} PointFacade.GetBudNumber(BranchPoints[PointIndex]){%i} BranchID{%i} ParentBoneIndex{%i} bFound{%i} FMath::Frac(NjordPixelID){%f}",
					...
					if ((FMath::Frac(NjordPixelID) <= 0.0001f || IsConnectingPoint || IsLastPoint))
					{
						...
						//UE_LOGF(LogProceduralVegetation, Warning, "Creating Bone NjordPixelId, Id{%f} BoneId{%i} PointIndex{%i} ParentBoneIndex{%i} BranchID{%i}",
	...
	FBoneNode* FBoneFacade::FindClosestBone(const FManagedArrayCollection& Collection, TArray<FBoneNode>& Bones, const int32 PointID)
	{
		...
		const auto NjordPixelIndexAttribute = PV::FPointNjordPixelIndexAttribute::FindAttribute(Collection);
		
		if (!NjordPixelIndexAttribute.IsValid())
		{
			UE_LOGF(LogProceduralVegetation, Warning, "BoneFacade::FindClosestBone called with invalid NjordPixelIndexAttribute.");
			return nullptr;
		}

		const float NjordValue = NjordPixelIndexAttribute.IsValidIndex(PointID) ? NjordPixelIndexAttribute[PointID] : -1;
		...
		if (BranchIndex == -1)
		{
			UE_LOGF(LogProceduralVegetation, Warning, "Unable to find the branch for point index %i", PointID);
			return nullptr;
		}
	...
	TArray<FBoneNode> FBoneFacade::GetBoneDataFromCollection() const
	{
		...
		if (!IsValid())
		{
			UE_LOGF(LogProceduralVegetation, Warning, "Trying to get bones with invalid BoneFacade.");
			return Bones;
		}

		const auto NjordPixelIndexAttribute = PV::FPointNjordPixelIndexAttribute::FindAttribute(BoneName.GetConstCollection());
		
		if (!NjordPixelIndexAttribute.IsValid())
		{
			UE_LOGF(LogProceduralVegetation, Warning, "BoneFacade::GetBoneDataFromCollection() called with invalid NjordPixelIndexAttribute.");
			return Bones;
		}
		...
		for (int32 i = 0; i < BoneName.Num(); i++)
		{
			...
			BoneNode.NjordPixelID = NjordPixelIndexAttribute.IsValidIndex(BoneNode.PointIndex) ? NjordPixelIndexAttribute[BoneNode.PointIndex] : -1;
	...
	void FBoneFacade::SetBoneDataToCollection(TArray<FBoneNode>& Bones)
	{
		if (!IsValid())
		{
			UE_LOGF(LogProceduralVegetation, Warning, "Cannot Set Bones, FBoneFacade is not valid, Schema not defined.");
			return;
		}
