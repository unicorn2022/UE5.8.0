...
static FAutoConsoleCommand RequestReleaseBundleCommand(
	TEXT("RequestReleaseBundle"),
	TEXT("Request a bundle to be uninstalled"),
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
{
	TSharedPtr<IInstallBundleManager> InstallBundleManager = IInstallBundleManager::GetPlatformInstallBundleManager();
	if (InstallBundleManager == nullptr)
	{
		LOG_INSTALL_BUNDLE_MAN(Error, TEXT("RequestReleaseBundle: InstallBundleManager not found!"));
		return;
	}
	if (InstallBundleManager->GetInitState() != EInstallBundleManagerInitState::Succeeded)
	{
		LOG_INSTALL_BUNDLE_MAN(Error, TEXT("RequestReleaseBundle: InstallBundleManager not initialized!"));
		return;
	}
	if (Args.Num() != 1)
	{
		LOG_INSTALL_BUNDLE_MAN(Display, TEXT("RequestReleaseBundle: Usage:\n")
							   TEXT("RequestReleaseBundle <BundleName>"));
		return;
	}
	TValueOrError<FInstallBundleCombinedInstallState, EInstallBundleResult> InstallState = InstallBundleManager->GetInstallStateSynchronous(FName(*Args[0]), false);
	if (InstallState.HasValue())
	{
		LOG_INSTALL_BUNDLE_MAN(Warning, TEXT("Requesting to uninstall bundle %s"), *Args[0]);
		InstallBundleManager->RequestReleaseContent(FName(*Args[0]), EInstallBundleReleaseRequestFlags::ExplicitRemoveList | EInstallBundleReleaseRequestFlags::RemoveFilesIfPossible);
	}
	else
	{
		LOG_INSTALL_BUNDLE_MAN(Error, TEXT("RequestBundle: InstallBundleManager not found!"));
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
{
	...
	else if (Args.Num() == 0)
	{
		LOG_INSTALL_BUNDLE_MAN(Display, TEXT("SetInstallBundleErrorSimulationCommands: Usage:\n")
			TEXT("SetInstallBundleErrorSimulationCommands [-SimulateClientNotLatest, -SimulateContentNotLatest, -SimulateOnCellularNetwork, -SimulateOnLimitedDataNetwork, -SimulateLowDiskSpace_NoRoomForInstall, -SimulateLowDiskSpace_InstallFailure, -SimulateBuildMetaDataDownloadError, -SimulateRemoteBuildMetaDataNotFoundError, -SimulateBuildVerificationError]"));
	}
	else
	{
	...
	, StatsMap(MakeShared<InstallBundleUtil::FContentRequestStatsMap>())
{
	...
	CSV_METADATA(TEXT("BundleManager_HasDownloadedContent"), TEXT("0"));
}
...
FInstallBundleCombinedInstallState FDefaultInstallBundleManager::GetInstallStateInternal(TArrayView<const FName> BundleNames) const
{
	...
	for (const FName& BundleName : BundleNames)
	{
		...
		case EBundleState::Mounted:
			InstallState = EInstallBundleInstallState::UpToDate;
			RetVal.MountedBundles.Add(BundleName);
			break;
		default:
...
void FDefaultInstallBundleManager::UpdateBundleSourceComplete(TSharedRef<IInstallBundleSource> Source, FInstallBundleSourceUpdateContentResultInfo InResultInfo, FContentRequestRef Request)
{
	...
	for (TPair<FInstallBundleSourceType, FInstallBundleSourceUpdateContentResultInfo>& Pair : Request->SourceRequestResults)
	{
		...
		{
			TArray<TUniquePtr<UE::IoStore::FOnDemandMountArgs>> CreatedOnDemandMountArgs;
			for (FString& ContentPath : ResultInfo.ContentPaths)
			{
				...
				// If the bundle source did not supply OnDemandMountArgs, create them now.
				// This assumes that the bundle source has already correctly setup ondemand hostgroups.
				if (ResultInfo.OnDemandMountArgs.IsEmpty() && ContentPath.EndsWith(TEXTVIEW(".uondemandtoc")))
				{
					TUniquePtr<UE::IoStore::FOnDemandMountArgs> MountArgs = MakeUnique<UE::IoStore::FOnDemandMountArgs>();
					MountArgs->MountId = Request->BundleName.ToString();
					MountArgs->FilePath = ContentPath;

					if (EnumHasAnyFlags(ResultInfo.MountOptions.MountFlags, FPakMountOptions::EMountFlags::WithSoftReferences))
					{
						MountArgs->Options |= UE::IoStore::EOnDemandMountOptions::WithSoftReferences;
					}

					CreatedOnDemandMountArgs.Add(MoveTemp(MountArgs));
				}
			}

			Request->OnDemandMountArgs.Append(MoveTemp(CreatedOnDemandMountArgs));
			...
			BundleInfo.ContentPaths.PakOrder = FMath::Max(BundleInfo.ContentPaths.PakOrder, ResultInfo.PakOrder);
		}
...
void FDefaultInstallBundleManager::MountPaks(FContentRequestRef Request)
{
	...
	TRACE_BEGIN_REGION(*WriteToString<256>(TEXT("Mount "), FNameBuilder(Request->BundleName)), TEXT("InstallBundles"));
	...
	auto MountPaksWorkFunc =
		...
		, PakOrder = BundleInfo.ContentPaths.PakOrder
		, LogVerbosityOverride = Request->LogVerbosityOverride](TValueOrError<void, UE::UnifiedError::FError>& OutResult) mutable -> bool
	{
		return MountPaksInList(ContentPaths, OutResult, LogVerbosityOverride, PakOrder);
	};
	...
	auto OnPakMountCompleteFunc = [this, WeakThis = AsWeak(), Request](bool bMountedPaks, const TValueOrError<void, UE::UnifiedError::FError>& MountResult) mutable
	{
		...
		{
			// TODO: We should have a function to remove these so they don't leak at metaverse scale
		...
		StatsEnd(Request->GetStatsKey(), EContentRequestState::Mounting);
		TRACE_END_REGION(*WriteToString<256>(TEXT("Mount "), FNameBuilder(Request->BundleName)));
		Request->StepResult = EContentRequestStepResult::Done;
	};
...
bool FDefaultInstallBundleManager::MountPaksInList(TArrayView<TPair<FString, FPakMountOptions>> Paths, TValueOrError<void, UE::UnifiedError::FError>& OutResult, ELogVerbosity::Type LogVerbosityOverride, const int32 PakOrder)
{
	OutResult = MakeValue();
	...
	{
		ensureMsgf(false, TEXT("Pak files have not been correctly initialized. Use -UsePaks on the cmdline if you are using the UnrealEditor.exe"));
		return false;
	}
...
{
	if (EnumHasAnyFlags(Request->Flags, EInstallBundleRequestFlags::SkipOnDemandTocMount))
	{
		return false;
	}
...
bool FDefaultInstallBundleManager::TickReporting(float dt)
{
	...
	if (bPersistentStateDirty)
	{
		...
		{
			CSV_METADATA(TEXT("BundleManager_HasDownloadedContent"), TEXT("1"));
...
EInstallBundleResult FDefaultInstallBundleManager::GetAllMountedBundles(TArray<FName>& OutBundleNames) const
{
	CSV_SCOPED_TIMING_STAT(InstallBundleManager, InstallBundleManager_GetAllMountedBundles);
	...
	OutBundleNames.Reserve(BundleInfoMap.Num());
	for (const TPair<FName, FBundleInfo>& BundleInfoPair : BundleInfoMap)
	{
		if (BundleInfoPair.Value.GetBundleStatus(*this) == EBundleState::Mounted)
		{
			OutBundleNames.Add(BundleInfoPair.Key);
		}
	}
	return EInstallBundleResult::OK;
}

EInstallBundleResult FDefaultInstallBundleManager::GetAllBundlesWithState(EInstallBundleInstallState InstallState, TArray<FName>& OutBundleNames) const
{
	CSV_SCOPED_TIMING_STAT(InstallBundleManager, InstallBundleManager_GetAllBundlesWithState);
	...
	for (const TPair<FName, FBundleInfo>& BundleInfoPair : BundleInfoMap)
	{
		EInstallBundleInstallState CurrentBundleState = EInstallBundleInstallState::Count;
		switch (BundleInfoPair.Value.GetBundleStatus(*this))
		{
		case EBundleState::NotInstalled:
			CurrentBundleState = EInstallBundleInstallState::NotInstalled;
			break;
		case EBundleState::NeedsUpdate:
			CurrentBundleState = EInstallBundleInstallState::NeedsUpdate;
			break;
		case EBundleState::NeedsMount:
			CurrentBundleState = EInstallBundleInstallState::UpToDate;
			break;
		case EBundleState::Mounted:
			CurrentBundleState = EInstallBundleInstallState::UpToDate;
			break;
		case EBundleState::Count:
		default:
			break;
		}
		if (CurrentBundleState == InstallState)
		{
			OutBundleNames.Add(BundleInfoPair.Key);
		}
	}
...
void FDefaultInstallBundleManager::SetLimitedDataPreference(int32 Value)
{
	for (const TPair<FInstallBundleSourceType, TSharedPtr<IInstallBundleSource>>& Pair : BundleSources)
	{
		Pair.Value->SetLimitedDataPreference(Value);
	}
}
...
void FDefaultInstallBundleManager::SetAnalyticsProvider(TSharedPtr<IAnalyticsProviderET> NewAnalyticsProvider)
{
	check(IsInGameThread());
	if (AnalyticsProvider == NewAnalyticsProvider)
	{
		return;
	}

	AnalyticsProvider = NewAnalyticsProvider;

	for (const TPair<FInstallBundleSourceType, TSharedPtr<IInstallBundleSource>>& Pair : BundleSources)
	{
		if (Pair.Value.IsValid())
		{
			Pair.Value->SetAnalyticsProvider(AnalyticsProvider);
		}
	}
}
