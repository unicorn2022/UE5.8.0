...
void FInstallBundleSourcePlatformChunkInstall::AsyncInit(FInstallBundleSourceInitDelegate Callback)
{
	...
	struct AsyncInitContext
	{
		...
		TArray<TPair<FString, TArray<FRegexPattern>>> BundleSubsetRegexList;
	...
	Context->BundleSubsetRegexList = MoveTemp(BundleSubsetRegexList);
	Context->PlatformChunkInstall = PlatformChunkInstall;
	Context->BundleManager = IInstallBundleManager::GetPlatformInstallBundleManager();
	...
	auto AsyncInit = [Context]() mutable
	{
		...
		for (TPair<FName, TArray<FString>>& FilePathPair : NamedChunkFilePaths)
		{
			for (FString& Path : FilePathPair.Value)
			{
				...
				{
					FName BundleFName(*BundleName);
					FBundleInfo& BundleInfo = Context->BundleInfoMap.FindChecked(BundleFName);
					TArray<FString>* BundleSubsets = Context->BundleToSubsetMap.Find(BundleFName);

					const int32 iSubset = (!BundleSubsets) ? INDEX_NONE : Context->BundleManager->SelectBundleSubset(BundleName, *BundleSubsets);

					if (!BundleSubsets || !BundleSubsets->IsValidIndex(iSubset) ||
						InstallBundleUtil::MatchBundleSubsetRegex((*BundleSubsets)[iSubset], Context->BundleSubsetRegexList, Path))
					{
	...
	for (const FName& BundleName : BundleNames)
	{
		...
		if (BundleInfo != nullptr)
		{
			...
			if (bHasChunkStatus)
			{
				...
				State.ContentSize.CurrentSizeOnDiskOtherDirs += ChunkStatusDetail.CurrentInstallSize;
...
EInstallBundleSourceBundleSkipReason FInstallBundleSourcePlatformChunkInstall::GetBundleSkipReason(FName BundleName) const
{
	...
	if (BundleInfo == nullptr)
	{
		LOG_SOURCE_CHUNKINSTALL(Verbose, TEXT("Skipping Bundle %s - no equivalent platform named chunk - allowing anyway"), *BundleName.ToString());
		return SkipReason;
	}
...
void FInstallBundleSourcePlatformChunkInstall::CancelBundles(TArrayView<const FName> BundleNames)
{
	...
	for ( FName BundleName : BundleNames)
	{
		...
		if (ContentRequestPtr != nullptr)
		{
			...
			if (BundleInfo != nullptr)
			{
				bool bCancelled = PlatformChunkInstall->CancelNamedChunkInstall(BundleInfo->NamedChunk);
				LOG_SOURCE_CHUNKINSTALL(Display, TEXT("Named chunk %s cancellation for bundle %s returned %s"), *BundleInfo->NamedChunk.ToString(), *BundleName.ToString(), *LexToString(bCancelled) );
			}
		}
	}
}
