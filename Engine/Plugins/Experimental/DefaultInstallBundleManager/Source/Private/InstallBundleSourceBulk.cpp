...
void FInstallBundleSourceBulk::AsyncInit_MakeBundlesForBulkBuild()
{
	...
	[this, FoundFiles]()
	{
		...
		// TODO: this cache is incompatible with bundle subsets. Its also unclear if its still needed, so I'm disabling it for now
		// bool bDidLoadAllFilesFromMetadata = bHasAnyFilesToParse ? TryLoadBulkBuildBundleMetadata(*FoundFiles, BulkBuildBundles) : false;
		bool bDidLoadAllFilesFromMetadata = false;
		...
		if (InstallBundleConfig)
		{
			...
			for (const FString& Section : SectionNames)
			{
				...
				BulkBuildBundles.FindOrAdd(FName(*BundleName));
			}

			TSharedPtr<IInstallBundleManager> BundleManger = IInstallBundleManager::GetPlatformInstallBundleManager();

			//If we had files remaining to manually parse, now sort the remaining files into bundles
			if (!bDidLoadAllFilesFromMetadata)
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(FInstallBundleSourceBulk_MatchFiles);
				
				struct FSectionParallelData
				{
					FString BundleName;
					TBitArray<> MatchedFiles;
					bool bAnyMatches;
				};
				TArray<FSectionParallelData> ParallelData;
				ParallelData.AddDefaulted(SectionNames.Num());
				
				// If the file was matched by a bundle with higher priority we can skip it
				// Used as an optimization - don't trust the value in here!
				TArray<std::atomic<int32>> MatchedBy;
				MatchedBy.AddDefaulted(FoundFiles->Num());
				for (int32 I = 0; I < MatchedBy.Num(); ++I)
				{
					MatchedBy[I] = INT32_MAX;
				}
				
				EParallelForFlags ParallelFlags = EParallelForFlags::Unbalanced;
				
				bool bParallelRegexMatching = true;
				
				// Search configs if we should MT
				{
					GConfig->GetBool(TEXT("InstallBundleSource.Bulk.MiscSettings"), TEXT("bParallelRegexMatching"), bParallelRegexMatching, GInstallBundleIni);
	
					if (const FString* ParallelMatchingVar = FPlatformMisc::GetConfigRulesVariable(TEXT("bParallelRegexMatching")))
					{
						bParallelRegexMatching = FCString::ToBool(**ParallelMatchingVar);
					}
					
#if !UE_BUILD_SHIPPING
					if (FParse::Param(FCommandLine::Get(), TEXT("NoParallelBundleRegexMatching")))
					{
						bParallelRegexMatching = false;
					}
#endif // !UE_BUILD_SHIPPING
				}
				
				if (!bParallelRegexMatching)
				{
					ParallelFlags |= EParallelForFlags::ForceSingleThread;
				}
				
				// Match found files for each section in parallel
				ParallelFor(SectionNames.Num(), [&](int32 SectionIndex)
				{
					const FString& Section = SectionNames[SectionIndex];
					FString BundleName = Section.RightChop(InstallBundleUtil::GetInstallBundleSectionPrefix().Len());
					FSectionParallelData& Data = ParallelData[SectionIndex];
					Data.BundleName = BundleName;
					Data.MatchedFiles.Init(false, FoundFiles->Num());
					Data.bAnyMatches = false;

					TArray<FString> StrSearchRegexPatterns;
					InstallBundleConfig->GetArray(*Section, TEXT("FileRegex"), StrSearchRegexPatterns);

					if (StrSearchRegexPatterns.IsEmpty())
					{
						return;
					}
					...
					const int32 iSubset = BundleSubsets.IsEmpty() ? INDEX_NONE : BundleManger->SelectBundleSubset(BundleName, BundleSubsets);

					TArray<FString> StrSubsetSearchRegexPatterns;
					if (BundleSubsets.IsValidIndex(iSubset))
					{
						const FString SubsetSection = InstallBundleUtil::GetInstallBundleSubsetSectionPrefix() + BundleSubsets[iSubset];
						InstallBundleConfig->GetArray(*SubsetSection, TEXT("FileRegex"), StrSubsetSearchRegexPatterns);
					}
					...
					TArray<FRegexPattern> SubsetSearchRegexPatterns;
					SubsetSearchRegexPatterns.Reserve(StrSubsetSearchRegexPatterns.Num());
					for (const FString& Str : StrSubsetSearchRegexPatterns)
					{
						SubsetSearchRegexPatterns.Emplace(Str, ERegexPatternFlags::CaseInsensitive);
					}
					
					for (int32 I = 0; I < FoundFiles->Num(); ++I)
					{
						// Skip if file was already matched by a bundle with higher order
						if (MatchedBy[I].load(std::memory_order_relaxed) <= SectionIndex)
						{
							continue;
						}
						
						FString& File = (*FoundFiles)[I];
						...
						bMatches = SubsetSearchRegexPatterns.IsEmpty();
						for (const FRegexPattern& Pattern : SubsetSearchRegexPatterns)
						{
							if (FRegexMatcher(Pattern, File).FindNext())
							{
								bMatches = true;
						...
						// This should really be a locked min - which we don't have.
						// Would have to be a CAS loop but from my testing that was slower than doing this.
						// This will give us some false positives but it's "good enough" and saves quite a bit of time.
						if (MatchedBy[I].load(std::memory_order_relaxed) > SectionIndex)
						{
							MatchedBy[I].store(SectionIndex, std::memory_order_relaxed);
						}

						Data.MatchedFiles[I] = bMatches;
						Data.bAnyMatches |= bMatches;
						if (!bMatches)
						{
							LOG_SOURCE_BULK(Verbose, TEXT("Excluded %s from Bundle %s with subset %s"), *File, *BundleName, *BundleSubsets[iSubset]);
						}
					}
				}, ParallelFlags);
				
				// We want files to be added to the BulkBuildBundles in the order that they are defined in SectionNames
				TBitArray<> FoundFilesBits;
				FoundFilesBits.Init(false, FoundFiles->Num());
				for (int32 SectionIndex = 0; SectionIndex < SectionNames.Num(); ++SectionIndex)
				{
					FSectionParallelData& Data = ParallelData[SectionIndex];
					if (Data.bAnyMatches)
					{
						TArray<FString>& BundleFileList = BulkBuildBundles.FindChecked(FName(*Data.BundleName));
						for (int32 I = 0; I < FoundFiles->Num(); ++I)
						{
							FString& File = (*FoundFiles)[I];
							if (Data.MatchedFiles[I] && !FoundFilesBits[I])
							{
								FoundFilesBits[I] = true;
								BundleFileList.AddUnique(MoveTemp(File));
							}
						}
					}
				}
