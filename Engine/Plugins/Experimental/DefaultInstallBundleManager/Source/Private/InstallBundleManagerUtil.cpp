...
namespace InstallBundleManagerUtil
{
	TSharedPtr<IInstallBundleSource> MakeBundleSource(FInstallBundleSourceType Type)
	{
		...
		{
			IPlatformChunkInstall* PlatformChunkInstall = InstallBundleManagerUtil::GetPlatformChunkInstall();
			if (PlatformChunkInstall)
			{
				return MakeShared<FInstallBundleSourcePlatformChunkInstall>(PlatformChunkInstall);
			}
		}
#endif // WITH_PLATFORM_INSTALL_BUNDLE_SOURCE

#if WITH_PLATFORM_DLC_INSTALL_BUNDLE_SOURCE
		if (Type.GetName() == TEXTVIEW("PlatformDLC"))
		{
			TSharedPtr<IPlatformDLC> PlatformDLC = InstallBundleManagerUtil::GetPlatformDLC();
			if (PlatformDLC.IsValid())
			{
				return MakeShared<FInstallBundleSourcePlatformDLC>(PlatformDLC);
			}
		}
...
namespace InstallBundleManagerAnalytics
{
	...
	void FireEvent_InitBundleSourcePlatformDLCComplete(IAnalyticsProviderET* AnalyticsProvider, const FString InitResultString)
	{
		if (AnalyticsProvider == nullptr || InstallBundleUtil::FInstallBundleSuppressAnalytics::IsEnabled())
		{
			return;
		...
		AnalyticsProvider->RecordEvent(TEXT("InstallBundleManager.InitBundleSourcePlatformDLCComplete"), MakeAnalyticsEventAttributeArray(
			TEXT("InitResultString"), InitResultString));
	}
		...
		const TArray<FInstallBundleChunkDownloadInfo>* ChunkDownloadInfos)
	{
		...
		Attributes.Add(FAnalyticsEventAttribute(TEXT("DownloadType"), RequestCompleteInfo.DownloadType));

		// Only add attributes if they're not all 0, and 'TotalCompressedBytes' is a good indicator.
		if (RequestCompleteInfo.BundleDecompressionInfo.TotalCompressedBytes > 0)
		{
			Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalCompressedBytes"), RequestCompleteInfo.BundleDecompressionInfo.TotalCompressedBytes));
			Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalDecompressedBytes"), RequestCompleteInfo.BundleDecompressionInfo.TotalDecompressedBytes));
			Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalActuallyReadBytes"), RequestCompleteInfo.BundleDecompressionInfo.TotalActuallyReadBytes));
			Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalActuallyWrittenBytes"), RequestCompleteInfo.BundleDecompressionInfo.TotalActuallyWrittenBytes));
			Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalDecompressionReadTime"), RequestCompleteInfo.BundleDecompressionInfo.TotalDecompressionReadTime));
			Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalDecompressionDecompressTime"), RequestCompleteInfo.BundleDecompressionInfo.TotalDecompressionDecompressTime));
			Attributes.Add(FAnalyticsEventAttribute(TEXT("TotalDecompressionWriteTime"), RequestCompleteInfo.BundleDecompressionInfo.TotalDecompressionWriteTime));
		}

		Attributes.Add(FAnalyticsEventAttribute(TEXT("AsyncStartupDelay"), RequestCompleteInfo.AsyncStartupDelay));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("AsyncExitDelay"), RequestCompleteInfo.AsyncExitDelay));
		Attributes.Add(FAnalyticsEventAttribute(TEXT("AsyncPlacedInBackground"), RequestCompleteInfo.AsyncPlacedInBackground));
