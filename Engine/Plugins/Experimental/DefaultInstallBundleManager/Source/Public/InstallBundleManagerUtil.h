...
#if !defined(WITH_PLATFORM_DLC_INSTALL_BUNDLE_SOURCE)
	#define WITH_PLATFORM_DLC_INSTALL_BUNDLE_SOURCE 0
#endif
...
namespace InstallBundleManagerUtil
{
	...
	DEFAULTINSTALLBUNDLEMANAGER_API IPlatformChunkInstall* GetPlatformChunkInstall();

	UE_DEPRECATED(5.8, "Implement InstallBundleManagerUtil::GetPlatformChunkInstall instead.")
	...
	DEFAULTINSTALLBUNDLEMANAGER_API TSharedPtr<IPlatformDLC> GetPlatformDLC();
#endif
...
namespace InstallBundleManagerAnalytics
{
	...
	*/
	DEFAULTINSTALLBUNDLEMANAGER_API void FireEvent_InitBundleSourcePlatformChunkInstallComplete(IAnalyticsProviderET* AnalyticsProvider,
		const FString InitResultString);
	/**
	* @EventName InstallBundleManager.FireEvent_InitBundleSourcePlatformDLCComplete
	* @Trigger Bundle Manager finished async initialization
	* @Type Client
	* @EventParam InitResultString String Result code
	* @Comments
	*/
	DEFAULTINSTALLBUNDLEMANAGER_API void FireEvent_InitBundleSourcePlatformDLCComplete(IAnalyticsProviderET* AnalyticsProvider,
		const FString InitResultString);
	 ...
	 * @EventParam DownloadType (string) Type of download
	 * @EventParam AsyncStartupDelay (double) How long (in seconds) before the async install task starts
	 * @EventParam AsyncExitDelay (double) How long (in seconds) before the async OnComplete is processed
	 * @EventParam AsyncPlacedInBackground (bool) True if app was placed in background during install step
