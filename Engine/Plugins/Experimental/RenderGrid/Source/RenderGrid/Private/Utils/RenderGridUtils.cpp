...
TArray<uint8> UE::RenderGrid::Private::FRenderGridUtils::GetRemoteControlValueBytesFromJson(const TArray<uint8>& OldValueBytes, const FString& NewValueJson)
{
	...
	if (!Value.IsValid())
	{
		UE_LOGF(LogRenderGrid, Warning, "Unable to retrieve remote control value from provided json. The json is invalid: '%ls'", *NewValueJson);
		return TArray<uint8>();
	}
