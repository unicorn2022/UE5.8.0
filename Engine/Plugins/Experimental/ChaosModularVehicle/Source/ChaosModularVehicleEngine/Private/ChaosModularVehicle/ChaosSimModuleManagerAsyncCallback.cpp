...
{
	QUICK_SCOPE_CYCLE_COUNTER(Stat_FModularVehicleAsyncInput_Simulate);
...
{
	static FNetworkModularVehicleInputs LocalDefaultDeltaSource;
	...
	if (ChaosModularVehicleCVars::bEnableInputReducedBandwidth && bIsUsingDeltaSerialization && DeltaSource)
	{
...
{
	if (ChaosModularVehicleCVars::bEnableStateReducedBandwidth && bIsUsingDeltaSerialization)
	{
...
{
	static FNetworkModularVehicleStates LocalDefaultDeltaSource;
	...
	UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "====DeltaNetSerialize Saving: %d. ServerFrame: %d. DeltaSource_ServerFrame: %d Starting Bit: %lld", Ar.IsSaving(), ServerFrame, DeltaSource->ServerFrame, Ar.IsSaving()?BitWriter->GetNumBits():BitReader->GetPosBits());
	...
	auto GetDeltaDataHelper = [&](int32 InIdx, uint32 InModuleTypeHash, int32 InSimArrayIndex)
	{
		...
		{
			UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "==DeltaNetSerialize Generating Default Data for DeltaData Module %u", InModuleTypeHash);
			DeltaData = Chaos::FModuleFactoryRegister::Get().GenerateNetData(InModuleTypeHash, InSimArrayIndex);
			if (DeltaData == nullptr)
			{
				UE_LOGF(LogModularVehicleSim, Error, "Unable to generate net data for delta source when delta is invalid");
				bOutSuccess = false;
			}
			else
			{
	...
	if (Ar.IsLoading())
	{
		...
		if(bPrintDebugInfo)
		{
			...
			UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "==DeltaNetSerialize LOADING. Using ModuleShouldSerialize: %ls",*BitString);
		}
		for (uint32 I = 0; I < NumNetModules; I++)
		{
			...
			if (TSharedPtr<Chaos::FModuleNetData> Data = Chaos::FModuleFactoryRegister::Get().GenerateNetData(ModuleTypeHash, SimArrayIndex))
			{
				...
				UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "==DeltaNetSerialize LOADING. ModuleData: %d STA. Bit: %lld",I,Ar.IsSaving()?BitWriter->GetNumBits():BitReader->GetPosBits());
				ModuleData[I] = Data;
				check(ModuleTypeHash == Chaos::FModuleFactoryRegister::GetModuleHash(Data->GetSimType()));
				...
				UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "==DeltaNetSerialize LOADING. ModuleData: %d END. Bit: %lld Total: %d Error: %d",I,Ar.IsSaving()?BitWriter->GetNumBits():BitReader->GetPosBits(), EndBit-StartBit,Ar.IsError());
			}
		}
	}
	else
	{
		if(bPrintDebugInfo)
		{
			...
			UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "==DeltaNetSerialize SAVING. Using ModuleShouldSerialize: %ls",*BitString);
		}
		for (uint32 I = 0; I < NumNetModules; I++)
		{
			int32 StartBit = Ar.IsSaving()?BitWriter->GetNumBits():BitReader->GetPosBits();
			UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "==DeltaNetSerialize SAVING. ModuleData: %d STA. Bit: %lld - %ls",I, Ar.IsSaving()?BitWriter->GetNumBits():BitReader->GetPosBits(), *ModuleData[I]->GetSimType().ToString());
			if (VehicleStateData.ModuleShouldSerialize[I])
			{
				const int32 SimArrayIndex = VehicleStateData.Indexes[I];
				const uint32 ModuleTypeHash = VehicleStateData.Hashes[I];
			...
			int32 EndBit = Ar.IsSaving()?BitWriter->GetNumBits():BitReader->GetPosBits();
			UE_CLOGF(bPrintDebugInfo,LogModularVehicleSim, Warning, "==DeltaNetSerialize SAVING. ModuleData: %d END. Bit: %lld Size: %d - %ls",I, Ar.IsSaving()?BitWriter->GetNumBits():BitReader->GetPosBits(), EndBit-StartBit, *ModuleData[I]->GetSimType().ToString());
		}
	}
...
void FNetworkModularVehicleStates::InterpolateData(const FNetworkPhysicsData& MinData, const FNetworkPhysicsData& MaxData)
{
	...
	{
		UE_LOGF(LogModularVehicleSim, Error, "Mismatch in module data num when interpolating between min and max states!");
		return;
	}
