...
DECLARE_MULTICAST_DELEGATE_EightParams(FModularVehicleSimulationCallback, FModularVehicleSimulation*, UWorld*, float, const FModularVehicleAsyncInput&, Chaos::FAllInputs&, IPhysicsProxyBase*, Chaos::FPBDRigidParticleHandle*, Chaos::FSimModuleTree*);
...
public:
	...
	void SetTestInputBuffer(TArray<FModuleInputContainer>& InTestInputBuffer, bool bInIsLoopBuffer, int32 InTestInputBufferStartFrame)
	{
		UE::TWriteScopeLock InputConfigLock(InputConfigurationLock);
	...
	UE_API void PerformAdditionalSimWork(float DeltaSeconds, UWorld* InWorld, const FModularVehicleAsyncInput& InputData, IPhysicsProxyBase* Proxy, Chaos::FAllInputs& AllInputs);
	...
	FModularVehicleSimulationCallback VehicleSimulationCallback;
};
