	...
	{       
        TFunction<bool(TSharedRef<FAVDevice> const&, const TCHAR*)> CheckCodecSupport = [](TSharedRef<FAVDevice> const& Device, const TCHAR* Codec) {
            ...
            for (CFIndex i = 0; i < EncoderCount; i++) 
            {
                ...
                if (EncoderDict)
                {
					...
					CFComparisonResult CompareResult = CFStringCompare(EncoderCodec, TargetCodec, 0);
        ...
        FVideoEncoder::RegisterPermutationsOf<TVideoEncoderVT<FVideoResourceMetal>>
            ...
            ::And<FVideoEncoderConfigVT, FVideoEncoderConfigH264>(
                ...
                {
					static bool bSupportsCodec = FAPI::Get<FVT>().IsValid() && (CheckCodecSupport(NewDevice, TEXT("H.264")) || CheckCodecSupport(NewDevice, TEXT("avc1")));
...
#if PLATFORM_MAC
		// TODO(Eden.Harris) VTRegisterSupplementalVideoDecoderIfAvailable is only available on mac
