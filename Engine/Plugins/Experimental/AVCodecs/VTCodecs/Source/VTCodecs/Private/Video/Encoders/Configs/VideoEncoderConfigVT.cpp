...
TAVResult<OSType> FVideoEncoderConfigVT::ConvertFormat(EVideoFormat const& Format)
{
	...
	default:
		return FAVResult(EAVResult::ErrorUnsupported, FString::Printf(TEXT("Pixel format %d is not supported"), EnumToUnderlyingType(Format)), TEXT("VT"));
	}
}
	...
	static auto const ConvertEntropyCodingMode = [](EH264EntropyCodingMode EntropyCodingMode) -> TAVResult<CFStringRef>
		{
			...
			default:
				return FAVResult(EAVResult::ErrorUnsupported, FString::Printf(TEXT("H264 entropy coding mode %d is not supported"), EnumToUnderlyingType(EntropyCodingMode)), TEXT("VT"));
			}
		};
