...
TAVResult<AMF_VIDEO_ENCODER_RATE_CONTROL_METHOD_ENUM> FVideoEncoderConfigAMF::ConvertRateControlMode(ERateControlMode Mode)
{
	...
	default:
		return FAVResult(EAVResult::ErrorUnsupported, FString::Printf(TEXT("Rate control mode %d is not supported"), EnumToUnderlyingType(Mode)), TEXT("AMF"));
	}
}
	...
	static auto const ConvertProfile = [](EH264Profile Profile) -> TAVResult<AMF_VIDEO_ENCODER_PROFILE_ENUM>
		{
			...
			default:
				return FAVResult(EAVResult::ErrorUnsupported, FString::Printf(TEXT("H264 profile %d is not supported"), EnumToUnderlyingType(Profile)), TEXT("AMF"));
			}
		};
