...
TUniquePtr<FScalableVideoController> TVideoEncoderLibVpxVP9<TResource>::CreateScalabilityStructureFromConfig(FVideoEncoderConfigLibVpx const& Config)
{
	...
	TOptional<EScalabilityMode> ScalabilityMode = ScalabilityModeFromString(FString(Builder));
...
void TVideoEncoderLibVpxVP9<TResource>::FillReferenceIndices(const vpx_codec_cx_pkt& Packet, const size_t PicNum, const bool bInterLayerPredicted, FCodecSpecificInfoVP9* Info)
{
	...
	if (bIsSvc)
	{
		...
		FAVResult::Log(EAVResult::Success, FString::Printf(TEXT("Frame %zu S%dT%d referenced buffers %hs"), PicNum, LayerId.spatial_layer_id, LayerId.temporal_layer_id, RefBufFlags), TEXT("LibVpx"));
	}
	else if (!bIsKeyFrame)
	{
...
void TVideoEncoderLibVpxVP9<TResource>::UpdateReferenceBuffers(const vpx_codec_cx_pkt& Packet, size_t PicNum)
{
	...
	if (bIsSvc)
	{
		...
		FAVResult::Log(EAVResult::Success, FString::Printf(TEXT("Frame %zu S%dT%d updated buffers %s"), PicNum, LayerId.spatial_layer_id, LayerId.temporal_layer_id, *BitArray.ToString()), TEXT("LibVpx"));
	}
	else
	{
		check(NumSpatialLayers == 1);
