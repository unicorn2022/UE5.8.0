...
namespace UE::OptiXDenoiser
{
	...
	static void ContextLogCallback(uint32_t Level, const char* Tag, const char* Message, void* /*cbdata*/)
	{
		...
		{
			UE_LOGF(LogOptiXDenoise, Log, "[%d][%ls]: %ls", Level, *FString(ANSI_TO_TCHAR(Tag)), *FString(ANSI_TO_TCHAR(Message)));
		...
		FReleaseCUDATextureCallback InOnReleaseCUDATextureCallback, FCUDASurfaceTextureCopyCallback InCopyCallback)
	{
		if (Format == EOptiXImageFormat::CUDA_A32B32G32R32_F || Format == EOptiXImageFormat::CUDA_FloatRGBA)
		{
			...
			{
				UE_LOGF(LogOptiXDenoise, Warning, "SetTexture | Cuda surface object is null");
			}
		}
	}
	...
	void FOptiXImageFactory::SetTexture(TSharedPtr<FOptiXImage2D> FOptiXImageData, const FTextureRHIRef& Texture)
	{
		...
		{
			UE_LOGF(LogOptiXDenoise, Error, "OptiX Denoiser does not support D3D11. Please use other RHI.");
		}
		else if (RHIType == ERHIInterfaceType::D3D12)
		{
			...
			{
				UE_LOGF(LogOptiXDenoise, Error, "OptiX Denoiser is only supported on NVIDIA device. Current RHI - %ls", GDynamicRHI->GetName());
			}
		}
#endif
		else
	...
	{
		UE_LOGF(LogOptiXDenoise, Error, "OptiX Denoiser is not supported on Vulkan Yet.");
	}
	...
	void FOptiXImageFactory::SetTextureCUDAD3D12(TSharedPtr<FOptiXImage2D> FOptiXImageData, const FTextureRHIRef& Texture)
	{
		...
		{
			UE_LOGF(LogOptiXDenoise, Error, "Failed to get DX texture handle for importing memory to CUDA: %d (Get Device)", QueryResult);
		}
		...
		{
			UE_LOGF(LogOptiXDenoise, Error, "Failed to get DX texture handle for importing memory to CUDA: %d", QueryResult);
		}
...

		{
			...
			{
				UE_LOGF(LogOptiXDenoise, Error, "Failed to import external memory error: %d", Result);
			}
		}
...

		{
			...
			if (Result != CUDA_SUCCESS)
			{
				UE_LOGF(LogOptiXDenoise, Error, "Failed to bind mipmappedArray error: %d", Result);
			}
		}
		...
		{
			UE_LOGF(LogOptiXDenoise, Error, "Failed to bind to mip 0.");
		}
		...
		CUsurfObject CuSurfaceObject;
		{
			...
			if (Result != CUDA_SUCCESS)
			{
				UE_LOGF(LogOptiXDenoise, Error, "Failed to create surface object error: %d", Result);
			}
		}
		...
		FOptiXImageData->SetTexture(CuSurfaceObject, FOptiXImage2D::EUnderlyingRHI::D3D12, D3D12TextureHandle,
			...
			if (CuSurfaceObject)
			{
				...
				if (Result != CUDA_SUCCESS)
				{
					UE_LOGF(LogOptiXDenoise, Error, "Failed to destroy Surface Object: %d", Result);
				}
			}
			...
			if (MappedArray)
			{
				...
				if (Result != CUDA_SUCCESS)
				{
					UE_LOGF(LogOptiXDenoise, Error, "Failed to destroy MappedArray: %d", Result);
				}
			}
			...
			if (MappedMipArray)
			{
				...
				if (Result != CUDA_SUCCESS)
				{
					UE_LOGF(LogOptiXDenoise, Error, "Failed to destroy MappedMipArray: %d", Result);
				}
			}
			...
			if (MappedExternalMemory)
			{
				...
				if (Result != CUDA_SUCCESS)
				{
					UE_LOGF(LogOptiXDenoise, Error, "Failed to destroy MappedExternalMemoryArray: %d", Result);
				}
			}
