...
#define OPTIX_CHECK(Call)																		\
	...
	{																							\
		...
		if(Result != 0)																\
		{																						\
			UE_LOGF(LogOptiXDenoise, Error, "Optix call (%ls) failed with code [%d] (%ls: %d", ANSI_TO_TCHAR(#Call), Result, ANSI_TO_TCHAR(__FILE__), (__LINE__));	\
		}																						\
	} while (false);																			\
...
#define CUDA_CHECK(Call)																		\
	...
	{																							\
		...
		if (Error != 0)																			\
		{																						\
			UE_LOGF(LogOptiXDenoise, Error, "Cuda call (%ls) failed with code [%d] (%ls: %d", ANSI_TO_TCHAR(#Call), Error, ANSI_TO_TCHAR(__FILE__), (__LINE__));	\
		}																						\
	} while (false);																			\
