	...
	cv::Mat& OutMat)
{
	...
	if (!CalibrationPoint)
	{
		UE_LOGF(LogLedWallCalibration, Error, "Invalid CalibrationPointComponent.");
		return false;
	}
	...
	{
		UE_LOGF(LogLedWallCalibration, Error, "Calibration point did not have a static mesh component parent.  "
	...
	if (!StaticMesh)
	{
		UE_LOGF(LogLedWallCalibration, Error, "ParentMeshComponent did not have a StaticMesh in it.");
		return false;
	}
	...
	if (!MeshDescription)
	{
		UE_LOGF(LogLedWallCalibration, Error, "Mesh did not have a valid Mesh Description");
		return false;
	}
	...
	{
		UE_LOGF(LogLedWallCalibration, Error, "Odd. The number of triangles (%d) was expected to be an even number.",
	...
	{
		UE_LOGF(LogLedWallCalibration, Error, "Panel detection failed because its detected amount (%d) does not equal half the number of triangles (%d)",
			WallPanels.Num(),
			ExpectedNumPanels
		);
	...
	{
		UE_LOGF(LogLedWallCalibration, Error, "Invalid texture dimensions of %d x %d",
	...
	{
		UE_LOGF(LogLedWallCalibration, Error, "Invalid Aruco dictionary");
		return false;
	}
	...
	if (MarkerId < 1)
	{
		UE_LOGF(LogLedWallCalibration, Error, "Invalid starting Marker Id (%d)",
			MarkerId
		);
	...
	// check that the dictionary will have enough symbols
	{
		...
		{
			UE_LOGF(LogLedWallCalibration, Error, "Not enough symbols in Aruco dictionary. The number of detected panels is %d, "
	...
	for (FWallPanel& WallPanel : WallPanels)
	{
		...
		{
			UE_LOGF(LogLedWallCalibration, Warning, "Skipping invalid panel dimensions of %d x %d",
				PanelWidthPixels,
				PanelHeightPixels
			);
		...
		if (MarkerBits % 2)
		{
			UE_LOGF(LogLedWallCalibration, Warning, "MarkerBits (%d) was not an even number, so the marker won't be perfectly centered",
				MarkerBits
			);
		}
	...
	UE_LOGF(LogLedWallCalibration, Error, "OpenCV is required for Aruco support");
	return false;
