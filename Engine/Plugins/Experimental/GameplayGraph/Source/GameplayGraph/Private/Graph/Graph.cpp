...
namespace Graph
{
	...
	// Return a rough estimate of the total number of edges in the graph. This can be used to reserve memory for each edge.
	// The return type is int32 to match IGraphSerialization::Initialize().
	static int32 EstimateNumEdges(const UGraph& Graph)
	{
		const TMap<FGraphVertexHandle, TObjectPtr<UGraphVertex>>& Vertices = Graph.GetVertices();
		if (Vertices.IsEmpty())
		{
			return 0;
		}

		const TObjectPtr<UGraphVertex>& FirstVertex = Vertices.CreateConstIterator()->Value;
		if (UNLIKELY(!FirstVertex))
		{
			return 0;
		}

		// Clamp estimate to reduce the chance of a specially crafted graph or atypical sample vertex leading to excessive overallocation.
		uint64 Estimate = static_cast<uint64>(Vertices.Num()) * FMath::Clamp(FirstVertex->NumEdges(), 2, 10);
		return IntCastChecked<int32>(FMath::Min<uint64>(Estimate, std::numeric_limits<int32>::max()));
	}
}
...
bool UGraph::CreateEdge(FGraphVertexHandle Node1, FGraphVertexHandle Node2, bool bMergeIslands)
{
	...
	if (Properties.bGenerateIslands)
	{
		...
		if (!IslandHandle1.IsValid() || !IslandHandle2.IsValid())
		{
			...
			else if (UGraphIsland* Island2 = IslandHandle2.GetIsland())
			{
...
void operator<<(IGraphSerialization& Output, const UGraph& Graph)
{
	...
	Output.Initialize(Graph.NumVertices(), Graph::EstimateNumEdges(Graph), Graph.NumIslands());
...

	{
		...
		{
			// Vertices are directly from the graph, keys should always be complete. Checking IsComplete() looks up the key in the map, which is
			// expensive when doing it for every vertex in the graph.
			checkSlow(Kvp.Key.IsComplete());
			...
			Kvp.Value->ForEachAdjacentVertex(
				[&Kvp, &Output](const FGraphVertexHandle& AdjacentVertexHandle)
				{
					// Only serialize the vertex in one direction, the reverse is implicit and re-created during deserialization.
					// Note that this permits edges from one vertex back to itself (potentially several such edges), which shall be serialized.
					if (AdjacentVertexHandle < Kvp.Key)
					{
						// This edge is already serialized in the reverse direction as {AdjacentVertexHandle, Kvp.Key}
						return;
					}

					Output.WriteGraphEdge(Kvp.Key, AdjacentVertexHandle);
				}
			);
		}
	}
