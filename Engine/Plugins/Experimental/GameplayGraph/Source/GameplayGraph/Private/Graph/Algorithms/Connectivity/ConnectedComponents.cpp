...
namespace Graph::Algorithms
{
	...
	{
		UE_LOGF(LogGameplayGraph, Verbose, "FindConnectedComponents Starting Set Size: %d", StartingSet.Num());
		...
		while (!WorkQueue.IsEmpty())
		{
			...
			if (CurrentComponent.IsEmpty())
			{
				...
				UE_LOGF(LogGameplayGraph, Warning, "FindConnectedComponents found an empty component with a non-empty work queue [probably due to an incomplete handle?]");
				break;
			}
