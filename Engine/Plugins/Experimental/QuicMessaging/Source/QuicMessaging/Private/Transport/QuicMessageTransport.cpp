...
{
	UE_LOGF(LogQuicMessaging, Verbose, "[MessageTransport] Restarting transport ...");
...
bool FQuicMessageTransport::StartTransport(IMessageTransportHandler& Handler)
{
    ...
    UE_LOGF(LogQuicMessaging, Verbose, "[MessageTransport] Started transport.");
...
{
	UE_LOGF(LogQuicMessaging, Verbose, "[MessageTransport] Stopping transport ...");
    ...
    UE_LOGF(LogQuicMessaging, Verbose, "[MessageTransport] Stopped transport.");
}
	...
	const FGuid NodeId) const
{
	if (!DeserializedMessage.IsValid())
	{
		UE_LOGF(LogQuicMessaging, Error,
			"[MessageTransport] Deserialized message pointer invalid.");
...
void FQuicMessageTransport::HandleManagerNodeDiscovered(const FGuid& DiscoveredNodeId)
{
	UE_LOGF(LogQuicMessaging, Display,
		"[MessageProcessor] Discovered node %ls.", *DiscoveredNodeId.ToString());
...
{
	UE_LOGF(LogQuicMessaging, Display,
		"[MessageProcessor] Lost node %ls.", *LostNodeId.ToString());
