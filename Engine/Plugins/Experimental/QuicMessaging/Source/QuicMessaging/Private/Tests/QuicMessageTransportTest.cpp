...
private:
		...
		const FGuid LocalNodeId, const TSharedRef<IMessageContext>& Context)
	{
		...
		if (!AuthMessage)
		{
			UE_LOGF(LogTemp, Error,
				"[QuicMessageTest::OnQuicMetaMessageReceived] "
		...
		const FGuid LocalNodeId, const TSharedRef<IMessageContext>& Context)
	{
		...
		{
			UE_LOGF(LogTemp, Error,
				"[QuicMessageTest::OnQuicAuthResponseMessageReceived] "
					"Authentication failed. Reason: %ls", *AuthResponseMessage->Reason);
		}
	}
...
public:
	...
	TOptional<FGuid> GetNodeId(const FString& RemoteEndpoint) const
    {
		...
		{
			UE_LOGF(LogTemp, Error,
				"[QuicMessageTest::GetNodeId] Endpoint string could not be parsed.");
