...
void FQuicServer::StartEndpoint()
{
		...
		ServerListenerCallback, this, &ServerListener)))
	{
		...
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Could not open listener: %ls.",
	...
	if (QUIC_FAILED(Status = MsQuic->ListenerStart(ServerListener, &Alpn, 1, &Address)))
	{
		...
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Could not start listener: %ls.",
			*QuicUtils::GetEndpointErrorString(ListenerError));
	...
	UE_LOGF(LogQuicMessagingTransport, Display,
		"[QuicServer] Started server. Listening on %ls.",
...
void FQuicServer::StopEndpoint()
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose, 
		"[QuicServer] Stopping server ...");
...
void FQuicServer::ConsumeOutboundMessages()
{
	...
	while (CurrentTime < ConsumeLimit && OutboundMessages.Dequeue(OutboundMessage))
	{
...

		{
			...
			{
				UE_LOGF(LogQuicMessagingTransport, Error,
					"[QuicServer] Could not find server handler when preparing outbound message: %ls",
...
void FQuicServer::AddConnectionAttempt(const FIPv4Endpoint& ConnectingEndpoint)
{
	...
	UE_LOGF(LogQuicMessagingTransport, Warning,
		"Connection cooldown of %llu seconds for IP %ls.",
...
void FQuicServer::CollectStatistics()
{
	...
	for (FIPv4Endpoint& UpdateEndpoint : UpdateHandlers)
	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[QuicServer] Could not collect statistics: %ls.",
...
void FQuicServer::DisconnectHandler(const FIPv4Endpoint& HandlerEndpoint)
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Could not find handler to disconnect: %ls.",
...
void FQuicServer::SendHello(FIPv4Endpoint RemoteEndpoint)
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[QuicServer] Sent hello to handler %ls.", *RemoteEndpoint.ToString());
}
...
void FQuicServer::SendBye(const FIPv4Endpoint& HandlerEndpoint)
{
...

	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[QuicServer] Could not find server handler when sending bye message: %ls",
				*OutboundMessage.Recipient.ToString());
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[QuicServer] Sent bye to handler %ls.", *HandlerEndpoint.ToString());
}
	...
	void* Context, QUIC_CONNECTION_EVENT* Event)
{
	...
	default:
		...
		UE_LOGF(LogQuicMessagingTransport, Warning,
			"[QuicServer] Received unhandled connection event: %d", Event->Type);
	...
	void* Context, QUIC_STREAM_EVENT* Event)
{
	...
	default:
		...
		UE_LOGF(LogQuicMessagingTransport, Warning,
			"[QuicServer] Received unhandled stream event: %d", Event->Type);
...
QUIC_STATUS FQuicServer::OnListenerNewConnection(QUIC_LISTENER_EVENT* Event)
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[QuicServer] Connection attempt by client: %ls.",
		*HandlerEndpoint.ToString());
...
void FQuicServer::OnConnectionConnected(HQUIC Connection)
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[QuicServer] Handshake completed. Handler: %ls.", *HandlerEndpoint.ToString());
}
...
void FQuicServer::OnConnectionShutdownByTransport(HQUIC Connection, QuicUtils::HRESULT Status)
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[QuicServer] Connection end (by transport). Handler: %ls.",
		*HandlerEndpoint.ToString());

	if (HandlerError != EQuicEndpointError::Normal)
	{
		UE_LOGF(LogQuicMessagingTransport, Verbose,
			"[QuicServer] Handler: %ls / shutdown reason: %ls.",
...
void FQuicServer::OnConnectionShutdownByPeer(HQUIC Connection, QUIC_UINT62 ErrorCode)
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[QuicServer] Connection end (by peer). Handler: %ls / Error code: %lld.",
...
void FQuicServer::OnConnectionShutdownComplete(HQUIC Connection, BOOLEAN AppCloseInProgress)
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[QuicServer] Connection shutdown complete. Handler: %ls.",
		*HandlerEndpoint.ToString());
}
...
void FQuicServer::OnConnectionPeerStreamStarted(HQUIC Connection, HQUIC Stream)
{
	...
	UE_LOGF(LogQuicMessagingTransport, VeryVerbose,
		"[QuicServer] Peer stream started. Handler: %ls / StreamId: %lld.",
	...
	const HQUIC Connection, const FOutboundMessage OutboundMessage)
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Could not open stream: %d.", EnumToUnderlyingType(QuicUtils::ConvertQuicStatus(Status)));
		return;
	}
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Could not start stream: %d.", EnumToUnderlyingType(QuicUtils::ConvertQuicStatus(Status)));
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Could not send meta on stream: %d.",
			EnumToUnderlyingType(QuicUtils::ConvertQuicStatus(Status)));
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Could not send message on stream: %d.",
			EnumToUnderlyingType(QuicUtils::ConvertQuicStatus(Status)));
...
bool FQuicServer::ReceiveFromStream(const FInboundQuicBuffer& QuicBuffer)
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Invalid inbound buffer in ReceiveFromStream %lld.",
			QuicBuffer.StreamId);
		...
		{
			{
				...
				{
					UE_LOGF(LogQuicMessagingTransport, Error,
						"[QuicServer] Cannot check authentication status of invalid handler %ls.",
				...
				{
					UE_LOGF(LogQuicMessagingTransport, Error,
						"[QuicServer] Unauthenticated handler sending data.");
	...
	if (Buffer->bHeaderDeserialized)
	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[QuicServer] Authentication message size too large.");
...
void FQuicServer::ReachedEndOfStream(const FInboundQuicBuffer& QuicBuffer)
{
...

	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[QuicServer] Invalid inbound buffer in ReachedEndOfStream.");
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[QuicServer] Transmitted message size smaller than expected %lld / %d / %d.",
...
QUIC_ADDR FQuicServer::GetRemoteAddress(HQUIC Connection) const
{
	...
	if (MsQuic)
	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[QuicServer] Could not get QUIC address from connection: %ls.",
				*QuicUtils::ConvertResult(Status));
		}
	}
