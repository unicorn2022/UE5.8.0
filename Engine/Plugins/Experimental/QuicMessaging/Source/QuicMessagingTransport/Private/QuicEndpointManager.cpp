	...
	, KnownNodes(TMap<FGuid, FQuicNodeInfo>())
{
	...
	if (QUIC_FAILED(Status = MsQuicOpen2(&MsQuic)))
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not open MsQuic: %ls.",
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not open registration: %ls.",
			*QuicUtils::ConvertResult(Status));
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose, 
		"[EndpointManager] Started endpoint manager.");
}
...
FQuicEndpointManager::~FQuicEndpointManager()
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose, 
		"[EndpointManager] Stopped endpoint manager.");
}
...
void FQuicEndpointManager::Shutdown()
{
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose, 
		"[EndpointManager] Shutdown completed.");
}
...
HQUIC FQuicEndpointManager::LoadClientConfiguration() const
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not open client configuration: %ls.",
			*QuicUtils::ConvertResult(Status));
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not load client config credentials: %ls.",
			*QuicUtils::ConvertResult(Status));
	...
	FString CertificatePath, FString PrivateKeyPath) const
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not open server configuration: %ls.",
			*QuicUtils::ConvertResult(Status));
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not load server config credentials: %ls.",
			*QuicUtils::ConvertResult(Status));
...
void FQuicEndpointManager::InitializeServer()
{
	...
	if (ServerEndpointConfig->AuthenticationMode == EAuthenticationMode::Disabled)
	{
		UE_LOGF(LogQuicMessagingTransport, Warning, 
			"[EndpointManager] Authentication disabled.");
	}
	else
	{
		UE_LOGF(LogQuicMessagingTransport, Verbose, 
			"[EndpointManager] Authentication enabled.");
	}

	if (ServerEndpointConfig->ConnCooldownMode == EConnectionCooldownMode::Enabled)
	{
		UE_LOGF(LogQuicMessagingTransport, Verbose,
			"[EndpointManager] Connection cooldown active: "
	...
	if (PrivateKey == "" || Certificate == "")
	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[EndpointManager] Could not generate self-signed certificate.");
		...
		UE_LOGF(LogQuicMessagingTransport, Verbose, 
			"[EndpointManager] Generated self-signed certificate.");
	}
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Server could not be started.");
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[EndpointManager] Configured endpoint manager to run in server mode.");
}
...
void FQuicEndpointManager::AddClient(const FIPv4Endpoint& ClientEndpoint)
{
...

    {
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Verbose,
				"[EndpointManager] AddClient: endpoint %ls still connected, removing it.",
	...
	if (!CurrentConfiguration)
	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[EndpointManager] Client could not be started.");
	...
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[EndpointManager] Added client endpoint %ls.",
		*ClientEndpoint.ToString());
}
...
{
	{
		...
		if (const FQuicEndpointPtr* Endpoint = Endpoints.Find(ClientEndpoint))
		{
			...
			UE_LOGF(LogQuicMessagingTransport, Verbose,
				"[EndpointManager] Removed client endpoint %ls.",
				*ClientEndpoint.ToString());
		}
	}
...
{
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[EndpointManager] Reconnecting client %ls.",
		*ClientEndpoint.ToString());
...
void FQuicEndpointManager::SetEndpointAuthenticated(const FGuid& NodeId)
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not find node to set as authenticated: %ls",
			*NodeId.ToString());
    ...
    if (EndpointMode == EEndpointMode::Server)
    {
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[EndpointManager] Could not find server endpoint to set handler as authenticated.");
...
void FQuicEndpointManager::DisconnectNode(const FGuid& NodeId)
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not find node to disconnect: %ls",
			*NodeId.ToString());
...
void FQuicEndpointManager::DisconnectEndpoint(const FIPv4Endpoint& Endpoint)
{
	...
	{
		UE_LOGF(LogQuicMessagingTransport, Error,
			"[EndpointManager] Could not find server endpoint to disconnect handler.");
	...
	const TArray<TTuple<FGuid, uint32>>& MessageMetas, const EQuicMessageType MessageType)
{
	...
	for (const TTuple<FGuid, uint32>& MessageMeta : MessageMetas)
	{
...

		{
			...
			{
				UE_LOGF(LogQuicMessagingTransport, Error,
					"[EndpointManager] Could not find message recipient node %ls",
					*Recipient.ToString());
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Error,
				"[EndpointManager] Could not find sending endpoint to enqueue message on: %ls",
...
{
	{
		...
		if (FQuicNodeInfo* ExistingNode = KnownNodes.Find(NodeId))
		{
			UE_LOGF(LogQuicMessagingTransport, Display,
				"[EndpointManager] Discovered node already known.");
...
{
	{
		...
		{
			UE_LOGF(LogQuicMessagingTransport, Warning,
				"[EndpointManager] Node to forget not found.");
...
{
	UE_LOGF(LogQuicMessagingTransport, Verbose,
		"[EndpointManager] Endpoint disconnected: %ls / Reason: %d.",
		*DisconnectedEndpoint.ToString(), EnumToUnderlyingType(Reason));
