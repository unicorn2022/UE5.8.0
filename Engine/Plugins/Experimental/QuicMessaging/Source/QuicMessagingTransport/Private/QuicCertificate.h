...
namespace QuicCertificateUtils
{
	 ...
	 */
	inline bool GeneratePrivateKey(EVP_PKEY* PrivateKey)
	{
		if (!PrivateKey)
		{
			return false;
	 ...
	 * @param PrivateKey The private key struct
	 */
	inline char* PrivateKeyToString(EVP_PKEY* PrivateKey)
	{
	 ...
	 */
	inline bool GenerateCertificate(EVP_PKEY* PrivateKey, X509* x509)
	{
		if (!x509)
		{
			return false;
	 ...
	 * @param x509 The certificate struct
	 */
	inline char* X509ToString(X509* x509)
	{
		BIO* Bio = BIO_new(BIO_s_mem());
	 ...
	 */
	inline TTuple<FString, FString> GetSelfSignedPaths()
	{
