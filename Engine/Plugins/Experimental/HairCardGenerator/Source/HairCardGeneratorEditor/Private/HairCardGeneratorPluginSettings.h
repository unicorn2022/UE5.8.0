...
namespace HairCardSettings_Helpers
{
	HAIRCARDGENERATOREDITOR_API FString GetGroomDirectoryForHairCards(TObjectPtr<const UGroomAsset> GroomAsset);
	HAIRCARDGENERATOREDITOR_API FString GetGroomNameForHairCards(TObjectPtr<const UGroomAsset> GroomAsset);
	HAIRCARDGENERATOREDITOR_API FString GetDefaultBaseNameForHairCards(TObjectPtr<const UGroomAsset> GroomAsset, int LODIndex, const FString& DefaultName);
	HAIRCARDGENERATOREDITOR_API FString GetDefaultDestinationPathForHairCards(TObjectPtr<const UGroomAsset> GroomAsset, int LODIndex, 
...
public:
	...
	void ResetFromSourceDescription(const FHairGroupsCardsSourceDescription& SourceDesc, const FString& DefaultDirectory, const FString& DefaultName);
	// Load settings from Json file if one exists
	bool ResetFromSettingsJson(const FString& DefaultDirectory, const FString& DefaultName);
	// Load default values for all options
	void ResetToDefault(const FString& DefaultDirectory, const FString& DefaultName);
