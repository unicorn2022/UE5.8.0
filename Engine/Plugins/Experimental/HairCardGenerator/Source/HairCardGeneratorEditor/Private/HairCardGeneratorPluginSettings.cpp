...
TSharedPtr<FJsonObject> FJsonHelper::ReadJsonSettings(const FString& SettingsFilename)
{
	...
	{
		// UE_LOGF(LogHairCardGenerator, Warning, "Unable to open json settings file: %ls", *SettingsFilename);
		return nullptr;
	}
	...
	{
		// UE_LOGF(LogHairCardGenerator, Warning, "Unable to parse json settings file: %ls", *SettingsFilename);
		return nullptr;
	}
...
FString HairCardSettings_Helpers::GetDefaultBaseNameForHairCards(TObjectPtr<const UGroomAsset> GroomAsset, int LODIndex, const FString& DefaultName)
{
	...
	{
		FormatArgs.Add(TEXT("groom_name"), FText::FromString(DefaultName));
	}
	else if (GroomAsset)
    {
        FormatArgs.Add(TEXT("groom_name"), FText::FromString(GetGroomNameForHairCards(GroomAsset)));
    }
...
FString HairCardSettings_Helpers::GetDefaultDestinationPathForHairCards(TObjectPtr<const UGroomAsset> GroomAsset, int LODIndex, const FString& DefaultDirectory, const FString& DefaultName)
{
	const UHairCardGeneratorEditorSettings* ConfigSettings = GetDefault<UHairCardGeneratorEditorSettings>();
	...
	{
		FormatArgs.Add(TEXT("groom_dir"), FText::FromString(DefaultDirectory));
	}
	else
	{
		FormatArgs.Add(TEXT("groom_dir"), FText::FromString(GetGroomDirectoryForHairCards(GroomAsset)));
	}
...
FString HairCardSettings_Helpers::GetGroomDirectoryForHairCards(TObjectPtr<const UGroomAsset> GroomAsset)
{
	if (GroomAsset)
	{
...
FString HairCardSettings_Helpers::GetGroomNameForHairCards(TObjectPtr<const UGroomAsset> GroomAsset)
{
	return GroomAsset ? GroomAsset->GetName() : FString();
}
...
void HairCardSettings_Helpers::ResetSettingsFromTemplate(TObjectPtr<UObject> SettingsObject, TObjectPtr<const UObject> TemplateObject)
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Warning, "Invalid template object for copying settings (%ls)", *SettingsObject->GetName());
		return;
	}
...
uint8 HairCardSettings_Helpers::PipelineSettingsDiff(TObjectPtr<const UObject> SettingsObject, TObjectPtr<UObject> DiffObject)
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Warning, "Comparing settings objects of incompatible types (%ls,%ls)", *SettingsObject->GetClass()->GetName(), *DiffObject->GetClass()->GetName());
	...
	for (TFieldIterator<FProperty> PropertyIt(SettingsObject->GetClass(), EFieldIterationFlags::IncludeDeprecated); PropertyIt; ++PropertyIt)
	{
		...
		{
			UE_LOGF(LogHairCardGenerator, Warning, "Hair Card Settings field: %ls has no category metadata (required for change detection)!", *PropertyIt->GetName());
			continue;
		}
		...
		{
			UE_LOGF(LogHairCardGenerator, Warning, "Hair Card Settings field: %ls has unexpected category \"%ls\" (cannot detect pipeline change effects)!", *PropertyIt->GetName(), *BaseCategoryName);
			continue;
		}
...
void HairCardSettings_Helpers::GetStrandCardGroupInfo(TObjectPtr<const UGroomAsset> GroomAsset, TArray<FName>& OutGrpIds, TArray<int32>& OutGrpCount)
{
	...
	if ( IdNames.Contains(NAME_None) )
	{
		...
		UE_LOGF(LogHairCardGenerator, Warning, "Groom has groom_group_cards_id attribute, but some strands are empty (will be ignored)");
	}
...
bool HairCardSettings_Converter::ResetSettingsFromJson(UObject* SettingsObject, const TSharedPtr<FJsonObject>& JsonObject)
{
	for (TFieldIterator<FProperty> PropertyIt(SettingsObject->GetClass(), EFieldIterationFlags::IncludeDeprecated); PropertyIt; ++PropertyIt)
	{
		...
		if ( !PropValue )
		{
			UE_LOGF(LogHairCardGenerator, Warning, "Json Field Not Found: %ls", *PropertyIt->GetName());
			continue;
		}
...
void UHairCardGeneratorPluginSettings::ResetFromSourceDescription(const FHairGroupsCardsSourceDescription& SourceDesc, const FString& DefaultDirectory, const FString& DefaultName)
{
	...
	{
		BaseFilename = HairCardSettings_Helpers::GetDefaultBaseNameForHairCards(GroomAsset, LODIndex, DefaultName);
		DestinationPath.Path = HairCardSettings_Helpers::GetDefaultDestinationPathForHairCards(GroomAsset, LODIndex, DefaultDirectory, DefaultName);
	}
...
bool UHairCardGeneratorPluginSettings::ResetFromSettingsJson(const FString& DefaultDirectory, const FString& DefaultName)
{
...
void UHairCardGeneratorPluginSettings::ResetToDefault(const FString& DefaultDirectory, const FString& DefaultName)
{
...
void UHairCardGeneratorPluginSettings::ResetFromJson()
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Warning, "No haircard group settings found in file: %ls", *SettingsFile);
		return;
	}
	...
	for ( int Index = 0; Index < JsonGroups.Num(); ++Index )
	{
		...
		{
			UE_LOGF(LogHairCardGenerator, Warning, "Invalid settings at index %d: %ls", Index, *SettingsFile);
			continue;
		}
...
void UHairCardGeneratorPluginSettings::WriteToJson()
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Warning, "Unable to write json settings string for object");
		return;
	}

	if ( !FFileHelper::SaveStringToFile(JsonText, *SettingsFile) )
	{
		UE_LOGF(LogHairCardGenerator, Warning, "Unable to write json settings file: %ls", *SettingsFile);
		return;
	}
}
...
void UHairCardGeneratorPluginSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UHairCardGeneratorPluginSettings, BaseFilename))
	{
		if (BaseFilename.IsEmpty())
		{
			BaseFilename = HairCardSettings_Helpers::GetDefaultBaseNameForHairCards(GroomAsset, LODIndex, FString());
		}
		else
		{
		...
		if ( !ResetFromSettingsJson(FString(), FString()) )
		{
			UpdateOutputPaths();
		}
	...
	else if (PropertyName == GET_MEMBER_NAME_CHECKED(UHairCardGeneratorPluginSettings, DestinationPath))
	{
		...
		{
			DestinationPath.Path = HairCardSettings_Helpers::GetDefaultDestinationPathForHairCards(GroomAsset, LODIndex, FString(), FString());
		}
	}
