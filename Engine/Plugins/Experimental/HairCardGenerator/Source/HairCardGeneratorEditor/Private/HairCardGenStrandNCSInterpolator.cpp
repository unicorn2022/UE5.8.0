...
FHairCardGenStrandNSCInterpolator::FHairCardGenStrandNSCInterpolator(const TArray<FVector>& Positions, const TArray<float>& Widths, const TArray<float>& AOs)
{
	...
	if (AOs.Num() > 0)
	{
		Splines.Add(FHairCardGenNaturalCubicSplines(DistancesFromStart, AOs));
	}
}
...
FHairCardGenStrandNSCInterpolator::StrandInterpolationResult FHairCardGenStrandNSCInterpolator::GetInterpolatedStrand(const int NumInterpolatedPoints)
{
	...
	if (Splines.Num() == 5)
	{
		Result.AOs.SetNum(NumInterpolatedPoints);
	}
