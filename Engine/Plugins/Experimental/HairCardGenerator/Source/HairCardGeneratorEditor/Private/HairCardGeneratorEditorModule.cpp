...
TObjectPtr<UHairCardGenControllerBase> FHairCardGenController::GetHairCardGenerator()
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Error, "Failed to find an active hair card controller.");
		return nullptr;
	}

	if ( HairCardGenControllers.Num() != 1 )
		UE_LOGF(LogHairCardGenerator, Warning, "More than one active hair-card controller (using first controller found).")
...
namespace HairCardGeneratorEditor_Impl
{
	...
	static TObjectPtr<UHairCardGeneratorPluginSettings> CreateGroomSettingsLast(TObjectPtr<UGroomAsset> Groom, const FHairGroupsCardsSourceDescription& SourceDesc, 
		const FString& DefaultDirectory = "", const FString& DefaultName = "");
	...
	static bool GenerateCardsGeometry(TObjectPtr<const UHairCardGeneratorPluginSettings> Settings, int index, FHairCardGeomData& OutGeomData, FHairCardMeshData& OutMeshData, int32& OutCardCount);
	...
	TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings, uint8& AllGenFlags, uint8& PipelineGenFlags, const FCardsGenerationAdvancedOptions& AdvancedOptions)
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Error, "No hair card generator controller found, cannot generate hair cards!");
		return false;
	}

	UE_CLOGF(CardsDesc.GenerationSettings && !UHairCardGeneratorPluginSettings::IsCompatibleSettings(CardsDesc.GenerationSettings), LogHairCardGenerator, Warning, "Old hair-card generation settings (from a different generator) will be discarded when you run this generator.");

	GenerationSettings = CardGenController->GetGroomSettings(NewGroomAsset, CardsDesc.LODIndex);
	if ( !GenerationSettings )
	{
		GenerationSettings = HairCardGeneratorEditor_Impl::CreateGroomSettingsLast(NewGroomAsset, CardsDesc, AdvancedOptions.DefaultDirectory, AdvancedOptions.DefaultName);
	}
	else
	{
	...
	{
		UE_LOGF(LogHairCardGenerator, Display, "No settings changes, skipping hair card run");
...
bool FHairCardGeneratorUtils::LoadGenerationSettings(const TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings)
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Error, "Failed to load hair card generation settings. See log for details.");
...
bool FHairCardGeneratorUtils::GenerateCardsClumps(const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const int32 FilterIndex, const uint8 GenFlags, FHairCardClumpData& OutClumpData)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	{
		if (!CardGenController->GenerateClumps(FilterIndex, OutClumpData))
		{
			UE_LOGF(LogHairCardGenerator, Error, "Strand clustering failed for settings group %d. Check the log for details.", FilterIndex);
			return false;
		}
		if (!CardGenController->SaveClumpData(FilterIndex, OutClumpData))
		{
			UE_LOG(LogHairCardGenerator, Warning, TEXT("Failed to save clump data for settings group %d."), FilterIndex);
		}
	}
	return true;
}

bool FHairCardGeneratorUtils::GenerateCardsGeometry(const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const int32 FilterIndex, const uint8 GenFlags, FHairCardGeomData& OutGeomData, FHairCardMeshData& OutMeshData, int32& OutCardCount)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	{
		if ( !HairCardGeneratorEditor_Impl::GenerateCardsGeometry(GenerationSettings, FilterIndex, OutGeomData, OutMeshData, OutCardCount) )
		{
			UE_LOGF(LogHairCardGenerator, Error, "Cards geometry failed for settings group %d. Check the log for details.", FilterIndex)
			return false;
		}
		if (!CardGenController->SaveCardData(FilterIndex, OutGeomData, OutMeshData, OutCardCount))
		{
			UE_LOG(LogHairCardGenerator, Warning, TEXT("Failed to save card data for settings group %d."), FilterIndex);
		}
	}
	return true;
}
	...
	FHairCardTextureClusterData& OutClusterData)
{
	...
	if ( UHairCardGeneratorPluginSettings::CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::TextureClustering) && !GenerationSettings->bReduceCardsFromPreviousLOD )
	{
		...
		int32 CardCount = 0;
		if (!CardGenController->LoadCardData(FilterIndex, GeomData, MeshData, CardCount))
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Failed to load card data for settings group %d."), FilterIndex);
			return false;
		}

		if (!CardGenController->ClusterTextures(FilterIndex, GeomData, MeshData, OutClusterData))
		{
			UE_LOGF(LogHairCardGenerator, Error, "Texture clustering failed for settings group %d. Check the log for details.", FilterIndex)
			return false;
		}
		if (!CardGenController->SaveTextureClusterData(FilterIndex, OutClusterData))
		{
			UE_LOG(LogHairCardGenerator, Warning, TEXT("Failed to save texture cluster data for settings group %d."), FilterIndex);
		}
	}
	return true;
}

bool FHairCardGeneratorUtils::GenerateTexturesLayoutAndAtlases(const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const uint8 GenFlags, FHairCardAtlasLayoutData& OutLayoutData, FHairCardAtlasUVData& OutAtlasUVData)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	const int32 NumGroups = GenerationSettings->GetFilterGroupSettings().Num();
	TArray<FHairCardGeomData> AllGeom;
	TArray<FHairCardMeshData> AllMesh;
	TArray<FHairCardTextureClusterData> AllCluster;
	TArray<int32> AllCardCount;
	AllGeom.SetNum(NumGroups);
	AllMesh.SetNum(NumGroups);
	AllCluster.SetNum(NumGroups);
	AllCardCount.SetNum(NumGroups);
	for (int32 i = 0; i < NumGroups; ++i)
	{
		if (!CardGenController->LoadCardData(i, AllGeom[i], AllMesh[i], AllCardCount[i]))
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Failed to load card data for group %d in texture layout step."), i);
			return false;
		}
		if (!CardGenController->LoadTextureClusterData(i, AllCluster[i]))
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Failed to load texture cluster data for group %d in texture layout step."), i);
			return false;
		}
	}

	// Layout is fast (bin-packing only) — always run it so OutLayoutData and OutAtlasUVData are
	// reliably populated for callers (e.g. the Dataflow node repopulates VertexTextureUVs each eval).
	// Skip only when deriving geometry from a previous LOD, where the parent layout is reused.
	const bool bRunLayout    = !GenerationSettings->bReduceCardsFromPreviousLOD;
	const bool bRunRendering = UHairCardGeneratorPluginSettings::CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::TextureRendering) && !GenerationSettings->bReduceCardsFromPreviousLOD;

	if (bRunLayout)
	{
		FHairCardAtlasBinpackData BinpackData;
		if (!CardGenController->GenerateTextureLayout(AllGeom, AllMesh, AllCluster, OutLayoutData, BinpackData, OutAtlasUVData))
		{
			UE_LOGF(LogHairCardGenerator, Error, "Texture layout failed. See log for details.");
			return false;
		}
		CardGenController->SaveAtlasLayoutData(OutLayoutData);
		CardGenController->SaveAtlasBinpackData(BinpackData);
	}

	if (bRunRendering)
	{
		FHairCardAtlasBinpackData BinpackData;
		if (!CardGenController->LoadAtlasBinpackData(BinpackData))
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Failed to load atlas binpack data for texture rendering step."));
			return false;
		}
		if (!CardGenController->GenerateTextureAtlases(AllGeom, AllMesh, AllCluster, OutLayoutData, BinpackData, -1.0f))
		{
			UE_LOGF(LogHairCardGenerator, Error, "Texture atlas creation failed. See log for details.");
			return false;
		}
	}
...
bool FHairCardGeneratorUtils::RunCardsGeneration(const TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings, const uint8 PipelineFlags, const FGeneratorFunction& PipelineFunction, const bool bCheckFlags)
{
	...
	for ( int32 FilterIndex = 0; FilterIndex < GenerationSettings->GetFilterGroupSettings().Num(); ++FilterIndex )
	{
		...
		{
			UE_LOGF(LogHairCardGenerator, Display, "Setttings group %d already generated. Skipping.", FilterIndex);
			continue;
		}
	...
	if ( !bAllGenerated )
	{
		UE_LOGF(LogHairCardGenerator, Error, "Failed to generate cards for all groups. See log for details.");
...
static bool GenerateHairCardGenGroomData(const int PointsPerStrand, const FHairDescription& InGroomData, FHairCardGen_GroomData& Out, const FHairCardGeneratorUtils::FBuilderFunction& BuilderFunction)
{
	...
	UE_CLOGF(!TraverseStrands.First().HasGroupID(), LogHairCardGenerator, Warning, "Groom description has no physics group info, may not generate correctly");

	const FHairStrandAttribsRefProxy& FirstStrand = TraverseStrands.First();
	UE_CLOGF((!FirstStrand.HasCurveType() || !FirstStrand.HasBasisType() || FirstStrand.HasKnots()), LogHairCardGenerator, Warning, "Invalid strand curve specification, treating as linear");
	...
	{
		UE_LOGF(LogHairCardGenerator, Warning, "VariableOrder curves not supported, treating as linear");
	...
	bool bHasAO = FirstStrand.HasVertexAO();
	if (bHasAO)
	{
		Out.VertexOcclusions.Reserve(NumStrands * PointsPerStrand);
	}
	...
	for ( const FHairStrandAttribsRefProxy& StrandAttribs : TraverseStrands )
	{
		...
		TArray<float> VertexAOs;
		VertexAOs.Reserve(StrandAttribs.NumVerts());
		...
		for (int StrandVertIndex = 0; StrandVertIndex < StrandAttribs.NumVerts(); ++StrandVertIndex)
		{
			...
			if (bHasAO)
			{
				VertexAOs.Add(StrandAttribs.GetVertexAO(StrandVertIndex));
			}
		}

		FHairCardGenStrandNSCInterpolator::StrandInterpolationResult InterpolatedStrand = FHairCardGenStrandNSCInterpolator(VertexPositions, VertexWidths, VertexAOs).GetInterpolatedStrand(PointsPerStrand);

		Out.VertexPositions.Append(InterpolatedStrand.Positions);
		Out.VertexWidths.Append(InterpolatedStrand.Widths);
		Out.VertexOcclusions.Append(InterpolatedStrand.AOs);
		++StrandIndex;
	}
	...
	const TObjectPtr<UHairCardGeneratorPluginSettings>& GenerationSettings, const uint8 GenFlags)
{
	...
	if ( GenerationSettings->CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::ImportTextures) )
	{
		...
		UE_LOGF(LogHairCardGenerator, Display, "Importing generated cards from: %ls", *TexturePath);
		...
		{
			UE_LOGF(LogHairCardGenerator, Error, "Failed to import textures. Check '%ls' for the files.", *TexturePath);
		...
		if ( NumAssigned == 0 )
		{
			UE_LOGF(LogHairCardGenerator, Error, "Unable to assign any of the imported assets to the groom. Verify texture file name format.");
	...
	if ( GenerationSettings->CheckGenerationFlags(GenFlags, EHairCardGenerationPipeline::GenerateMesh) )
	{
		...
		if ( !NewMesh )
		{
			...
			UE_LOGF(LogHairCardGenerator, Error, "Unable to create mesh in destination package.");
		...
		// Load all per-group and layout data from disk for mesh generation
		const int32 NumGroupsForMesh = GenerationSettings->GetFilterGroupSettings().Num();
		TArray<FHairCardGeomData> MeshAllGeom;
		TArray<FHairCardMeshData> MeshAllMesh;
		TArray<FHairCardTextureClusterData> MeshAllCluster;
		TArray<int32> MeshAllCardCount;
		MeshAllGeom.SetNum(NumGroupsForMesh);
		MeshAllMesh.SetNum(NumGroupsForMesh);
		MeshAllCluster.SetNum(NumGroupsForMesh);
		MeshAllCardCount.SetNum(NumGroupsForMesh);
		for (int32 i = 0; i < NumGroupsForMesh; ++i)
		{
			if (!CardGenController->LoadCardData(i, MeshAllGeom[i], MeshAllMesh[i], MeshAllCardCount[i]) ||
			    !CardGenController->LoadTextureClusterData(i, MeshAllCluster[i]))
			{
				UE_LOG(LogHairCardGenerator, Error, TEXT("Failed to load stage data for group %d in mesh generation step."), i);
		...
		FHairCardAtlasLayoutData MeshLayoutData;
		if (!CardGenController->LoadAtlasLayoutData(MeshLayoutData))
		{
			UE_LOG(LogHairCardGenerator, Error, TEXT("Failed to load atlas layout data in mesh generation step."));
			HairCardGeneratorEditor_Impl::CreateFinishNotification(false, LOCTEXT("HairCardGen.FailedMeshGen", "Mesh generation failed. See log for details."));
			return false;
		}

		if ( !CardGenController->GenerateMesh(NewMesh, MeshAllGeom, MeshAllMesh, MeshAllCluster, MeshLayoutData) )
		{
			UE_LOGF(LogHairCardGenerator, Error, "Mesh generation failed. See log for details.");
			HairCardGeneratorEditor_Impl::CreateFinishNotification(false, LOCTEXT("HairCardGen.FailedMeshGen", "Mesh generation failed. See log for details."));
			return false;
		}
...
static TObjectPtr<UGroomAsset> HairCardGeneratorEditor_Impl::ParseGroomAssetArg(const TArray<FString>& Args, bool bOptional)
{
	...
	{
		UE_CLOGF(!bOptional, LogHairCardGenerator, Error, "The path is not a valid asset path.");
		return nullptr;
	}
	...
	{
		UE_CLOGF(!bOptional, LogHairCardGenerator, Error, "The asset path cannot be found in the registry.");
		return nullptr;
	}
	...
	{
		UE_CLOGF(!bOptional, LogHairCardGenerator, Error, "The specified asset is not a groom asset.");
		return nullptr;
	}
...
static TObjectPtr<UHairCardGeneratorPluginSettings> HairCardGeneratorEditor_Impl::CreateGroomSettingsLast(TObjectPtr<UGroomAsset> Groom, 
	const FHairGroupsCardsSourceDescription& SourceDesc, const FString& DefaultDirectory, const FString& DefaultName)
{
	...
	NewSettings->ResetToDefault(DefaultDirectory, DefaultName);

	// First try to reset from Json file (then fallback to previous settings if they exist)
	if ( NewSettings->ResetFromSettingsJson(DefaultDirectory, DefaultName) )
		return NewSettings;

	NewSettings->ResetFromSourceDescription(SourceDesc, DefaultDirectory, DefaultName);
	return NewSettings;
}
...
static void HairCardGeneratorEditor_Impl::DbgCmdCacheGroom(const TArray<FString>& Args)
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Error, "Failed to cache groom asset.");
	}
}
...
static bool HairCardGeneratorEditor_Impl::GenerateCardsGeometry(TObjectPtr<const UHairCardGeneratorPluginSettings> Settings, int index, FHairCardGeomData& OutGeomData, FHairCardMeshData& OutMeshData, int32& OutCardCount)
{
	TObjectPtr<UHairCardGenControllerBase> CardGenController = FHairCardGenController::GetHairCardGenerator();
	if ( !CardGenController )
	{
	...
	// Load clump data that was written by the clumps stage
	SlowTask.EnterProgressFrame(1);
	FHairCardClumpData ClumpData;
	if (!CardGenController->LoadClumpData(index, ClumpData))
	{
		UE_LOG(LogHairCardGenerator, Error, TEXT("Failed to load clump data for settings group %d. Check the log for details."), index);
		SlowTask.EnterProgressFrame(9);
		return false;
	}

	if (!CardGenController->SetOptimizations(index, ClumpData))
	{
		UE_LOGF(LogHairCardGenerator, Error, "Card geometry generation failed for settings group %d. Check the log for details.", index);
		SlowTask.EnterProgressFrame(9);
		return false;
	}

	TArray<int32> NumCards = CardGenController->GetNumCardsPerClump();

	int TotalNumCards = 0;
	for (int CurrentNumCards : NumCards) TotalNumCards += CurrentNumCards;

	// Empty group: no cards to subdivide. Leave OutGeomData/OutMeshData default-constructed
	// (all arrays empty) and OutCardCount = 0. SaveCardData will persist empty files so
	// downstream LoadCardData succeeds unconditionally for all groups.
	if (NumCards.IsEmpty() || TotalNumCards == 0)
	{
		UE_LOG(LogHairCardGenerator, Warning, TEXT("Settings group %d produced no cards — saving empty geometry data and continuing."), index);
		SlowTask.EnterProgressFrame(9);
		OutCardCount = 0;
		return true;
	}
	...
	SlowTask.EnterProgressFrame(9);
	if (!CardGenController->GenerateCardsGeometry(OutGeomData, OutMeshData, OutCardCount))
	{
		UE_LOGF(LogHairCardGenerator, Error, "Card geometry generation failed for settings group %d. Check the log for details.", index);
		return false;
	}
...
static bool HairCardGeneratorEditor_Impl::GenerateCardsForCardGroup(const TObjectPtr<const UHairCardGeneratorPluginSettings> GenerationSettings, const int32 FilterIndex, const uint8 GenFlags)
{
	...
	SlowTask.EnterProgressFrame(1);
	FHairCardClumpData ClumpData;
	if(!FHairCardGeneratorUtils::GenerateCardsClumps(GenerationSettings, FilterIndex, GenFlags, ClumpData))
	{
		SlowTask.EnterProgressFrame(9);
		return false;
	}
...
static UStaticMesh* HairCardGeneratorEditor_Impl::CreateMeshInPackage(const FString& DestinationPackage, const FString& BaseFilename, int GroupID)
{
	...
	UE_LOGF(LogHairCardGenerator, Error, "Non-mesh asset ('%ls') already exists in package: '%ls'", *ObjectName, *PackageDest);
...
static int32 HairCardGeneratorEditor_Impl::AssignImportedTextures(const TArray<UObject*>& ImportedAssets, const FHairCardImportSettings& ImportSettings, FHairGroupsCardsSourceDescription& Assignee)
{
	...
	for (UObject* ImportedObject : ImportedAssets)
	{
		...
		if ( TextureImport )
		{
			...
			UE_CLOGF(!bFound, LogHairCardGenerator, Warning, "Unable to determine which slot imported texture ('%ls') is for. Check your config.", *TextureImport->GetName());
		}
		else
		{
			UE_LOGF(LogHairCardGenerator, Warning, "Unused asset imported along with hair card mesh: '%ls'", *ImportedObject->GetName());
		}
	}
...
bool FHairCardGeneratorEditorModule::GenerateHairCardsForLOD(UGroomAsset* NewGroomAsset, FHairGroupsCardsSourceDescription& CardsDesc)
{
	...
	{
		UE_LOGF(LogHairCardGenerator, Error, "Failed to load groom asset. See log for details.");
	...
	if(!FHairCardGeneratorUtils::GenerateTexturesLayoutAndAtlases(GenerationSettings, AllGenFlags, LayoutData, AtlasUVData))
	{
		return false;
	}
