...
{
	static void BuildStrandsPositions(const FManagedArrayCollection& GroomCollection, TArray<TArray<FVector>>& StrandsPositions, const FHairDescription& HairDescription)
	{
		...
		if (CurvesFacade.IsValid())
		{
			...
			StrandsPositions.SetNum(HairDescription.GetNumStrands());

			int32 PointOffset = 0;
			const int32 NumCurves = CurvesFacade.GetNumCurves();
			for(int32 CurveIndex = 0; CurveIndex < NumCurves; ++CurveIndex)
			{
				const int32 GroupIndex = CurvesFacade.GetCurveGeometryIndices()[CurveIndex];
				const int32 SourceCurve = CurvesFacade.GetCurveSourceIndices()[CurveIndex] >> 1;
				
				if (HairDescriptionGroups.HairGroups.IsValidIndex(GroupIndex) && HairDescriptionGroups.HairGroups[GroupIndex].Strands.StrandsCurves.CurvesMapping.IsValidIndex(SourceCurve))
				{
					...
					if (StrandsPositions.IsValidIndex(StrandID))
					{
						StrandsPositions[StrandID].SetNum(NumPoints);
				
						for(int32 PointIndex = 0; PointIndex < NumPoints; ++PointIndex)
						{
							StrandsPositions[StrandID][PointIndex] = FVector(CurvesFacade.GetPointRestPositions()[PointIndex + PointOffset]);
						}
					}
				}
...
void FBuildCardsSettingsNode::OnPropertyChanged(UE::Dataflow::FContext& Context, const FPropertyChangedEvent& InPropertyChangedEvent)
{
	if (InPropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(FBuildCardsSettingsNode, GenerationSettings) || 
		InPropertyChangedEvent.GetPropertyName() == GET_MEMBER_NAME_CHECKED(FBuildCardsSettingsNode, GroomAsset))
	{
		if (UGroomAsset* LocalAsset = GetValue<TObjectPtr<UGroomAsset>>(Context, &GroomAsset))
		{
			if (AssetsDirectoryPath.IsEmpty())
			{
				AssetsDirectoryPath = HairCardSettings_Helpers::GetGroomDirectoryForHairCards(LocalAsset);
			}
			if (AssetsBaseName.IsEmpty())
			{
				AssetsBaseName = HairCardSettings_Helpers::GetGroomNameForHairCards(LocalAsset);
			}
		}
	}
}
		...
		if (LocalAsset)
		{
			for(const FGroomGenerationSettings& LocalSettings : GenerationSettings)
			{
				...
				if (CardsDescriptionPtr)
				{
					...
					AdvancedOptions.DefaultDirectory = AssetsDirectoryPath;
					AdvancedOptions.DefaultName= AssetsBaseName;
					...
					if(!LocalSettings.FilterSettings.IsEmpty())
					{
						...
						FHairCardGeneratorUtils::LoadGroomStrands(LocalAsset, [&GeometryCollection,LocalAsset](TArray<TArray<FVector>>& CurvesPositions)
						{
							UE::CardGen::Private::BuildStrandsPositions(GeometryCollection, CurvesPositions, LocalAsset->GetHairDescription());
						});
