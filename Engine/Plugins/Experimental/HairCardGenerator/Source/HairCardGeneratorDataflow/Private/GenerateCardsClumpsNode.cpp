				...
				[&StrandsClumps, &NumClumps, &StrandsFilter](const TObjectPtr<const UHairCardGeneratorPluginSettings>& GenerationSettings, const int32 FilterIndex, const uint8 GenFlags)
						{
							...
							{
								for (int32 CurveIndex = 0; CurveIndex < ClumpData.StrandLabels.Num(); ++CurveIndex)
								{
									if (StrandsClumps.IsValidIndex(CurveIndex) && ClumpData.StrandLabels[CurveIndex] != INDEX_NONE)
									{
										StrandsClumps[CurveIndex] = ClumpData.StrandLabels[CurveIndex] + NumClumps;
										StrandsFilter[CurveIndex] = FilterIndex;
									}
								}
								NumClumps += ClumpData.MaxMainClump + 1;
								return true;
							}
							return false;
						}, false);
