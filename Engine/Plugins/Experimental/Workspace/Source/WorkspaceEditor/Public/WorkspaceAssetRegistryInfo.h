...
public:
	...
	FWorkspaceOutlinerItemExport GetRelativeAssetExport(const FSoftObjectPath& InAssetPath) const
	{
		FWorkspaceOutlinerItemExport Export;
		...
		if(Path.PathSegments.Num() > 0)
		{
			...
			{
				FSoftObjectPath ObjectPath = Path.PathSegments[SegmentIndex].ToString();
				if (ObjectPath.IsValid() && ObjectPath == InAssetPath && SegmentIndex != 0)
				{
					Export.Path.PathSegments.Append(&Path.PathSegments[0], SegmentIndex);
					break;
				}
			}
		}
