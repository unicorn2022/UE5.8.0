...
public:
	...
	// Whether workspaces using this schema can add more assets to the workspace besides the root-level asset
	virtual bool SupportsMultipleAssets() const { return false; }
	
	// Whether workspaces using this schema has a root-level asset that always exists in the workspace
	virtual bool HasPersistentRootAsset() const { return false; }
	
	// Whether workspaces using this schema can be saved as an asset
	virtual bool CanSaveWorkspace() const { return false; }
};
