...
FObjectDocumentArgs FWorkspaceEditorModule::CreateGraphDocumentArgs(const FGraphDocumentWidgetArgs& InArgs)
{
	...
	Args.OnMakeDocumentWidget = FOnMakeDocumentWidget::CreateLambda([InArgs](const FWorkspaceEditorContext& InContext)
	{
		...
		TSharedRef<SWidget> Widget = SNew(SGraphDocument, StaticCastSharedRef<FWorkspaceEditor>(InContext.WorkspaceEditor), InContext.Document)
			...
			.OnCreateComment(InArgs.OnCreateComment);
...
{
	UWorkspaceFactory* DefaultWorkspaceFactory = WorkSpaceFactoryClass->GetDefaultObject<UWorkspaceFactory>();
	UWorkspaceSchema* DefaultWorkspaceSchema = DefaultWorkspaceFactory->SchemaClass ? DefaultWorkspaceFactory->SchemaClass->GetDefaultObject<UWorkspaceSchema>() : nullptr;

	if (DefaultWorkspaceSchema && DefaultWorkspaceSchema->CanSaveWorkspace())
	{
	...
	else
	{
		...
		{
			if (!ensureMsgf(Factory->ConfigureProperties(), TEXT("Can not create a workspace without a valid Schema")))
			{
				return nullptr;
			}
		}

		TObjectPtr<UWorkspace> NewWorkspace = CastChecked<UWorkspace>(Factory->FactoryCreateNew(UWorkspace::StaticClass(), GetTransientPackage(), NAME_None, RF_Public | RF_Standalone, nullptr, GWarn));
		
		NewWorkspace->AddAsset(InObject, false);
		(void)NewWorkspace->MarkPackageDirty();
		...
		IWorkspaceEditor* WorkspaceEditor = static_cast<IWorkspaceEditor*>(GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->FindEditorForAsset(InObject, true));
		if (WorkspaceEditor)
		{
			WorkspaceEditor->OpenAssets({InObject});
		}
