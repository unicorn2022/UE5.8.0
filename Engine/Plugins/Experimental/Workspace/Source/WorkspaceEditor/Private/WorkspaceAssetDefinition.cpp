...
EAssetCommandResult UAssetDefinition_Workspace::OpenAssets(const FAssetOpenArgs& OpenArgs) const
{
	...
	{
		if (!Asset->GetSchema()->CanSaveWorkspace())
		{
			// Attempting to open a workspace asset that is no longer savable as an asset
			FNotificationInfo ErrorNotification(LOCTEXT("CannotOpenWorkspaceAsset", "Workspace assets of this schema can no longer be saved as assets. Please delete this asset."));
			ErrorNotification.bFireAndForget = true;
			ErrorNotification.bUseLargeFont = false;
			ErrorNotification.ExpireDuration = 3.0f;
			
            FSlateNotificationManager::Get().AddNotification(ErrorNotification)->SetCompletionState(SNotificationItem::CS_Fail);
			UE_LOGF(LogAnimation, Error, "Workspace assets of this schema can no longer be saved as assets. Please delete this asset.");
