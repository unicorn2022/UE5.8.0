...
void UCineSplineMetadata::InsertPoint(int32 Index, float Alpha, bool bClosedLoop)
{
	if (Index < 0)
	{
		UE_LOGF(LogCineSpline, Error, "InsertPoint - invalid Index: %d", Index);
		return;
	}
...
void UCineSplineMetadata::UpdatePoint(int32 Index, float Alpha, bool bClosedLoop)
{
	...
	{
		UE_LOGF(LogCineSpline, Error, "UpdatePoint - invalid Index: %d", Index);
		return;
	}
	Alpha = FMath::Clamp(Alpha, 0.f, 1.f);
...
void UCineSplineMetadata::AutoSetTangents()
{
	Modify();
	FocalLength.AutoSetTangents(0.0f, false);
	Aperture.AutoSetTangents(0.0f, false);
	FocusDistance.AutoSetTangents(0.0f, false);
	PointRotation.AutoSetTangents(0.0f, false);
}

void UCineSplineMetadata::SetTangentType(int32 Index, ECineSplineMetadataTangentType TangentType)
{
	check(Index < FocalLength.Points.Num());
	check(Index >= 0);
	...
	{
		Modify();
		FocalLength.Points[Index].InterpMode = Mode;
		Aperture.Points[Index].InterpMode = Mode;
		FocusDistance.Points[Index].InterpMode = Mode;
		PointRotation.Points[Index].InterpMode = Mode;
	}
}

EInterpCurveMode ConvertCineSplineMetadataTangentTypeToInterpCurveMode(ECineSplineMetadataTangentType MetadataTangentType)
{
	switch (MetadataTangentType)
	{
		case ECineSplineMetadataTangentType::Linear:
			return CIM_Linear;
		case ECineSplineMetadataTangentType::Curve:
			return CIM_CurveAuto;
		case ECineSplineMetadataTangentType::Constant:
			return CIM_Constant;
		case ECineSplineMetadataTangentType::CurveClamped:
...
ECineSplineMetadataTangentType ConvertInterpCurveModeToCineSplineMetadataTangentType(EInterpCurveMode InterpCurveMode)
{
	switch (InterpCurveMode)
	{
		case CIM_Linear:
			return ECineSplineMetadataTangentType::Linear;
		case CIM_CurveAuto:
			return ECineSplineMetadataTangentType::Curve;
		case CIM_Constant:
			return ECineSplineMetadataTangentType::Constant;
		case CIM_CurveAutoClamped:
			return ECineSplineMetadataTangentType::CurveClamped;
...
#if WITH_EDITOR
void UCineSplineMetadata::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	const FProperty* TouchedProperty = PropertyChangedEvent.Property;
	const FName TouchedName = TouchedProperty ? TouchedProperty->GetFName() : NAME_None;

	static const FName AbsolutePositionName = GET_MEMBER_NAME_CHECKED(UCineSplineMetadata, AbsolutePosition);
	static const FName FocalLengthName      = GET_MEMBER_NAME_CHECKED(UCineSplineMetadata, FocalLength);
	static const FName ApertureName         = GET_MEMBER_NAME_CHECKED(UCineSplineMetadata, Aperture);
	static const FName FocusDistanceName    = GET_MEMBER_NAME_CHECKED(UCineSplineMetadata, FocusDistance);
	static const FName PointRotationName    = GET_MEMBER_NAME_CHECKED(UCineSplineMetadata, PointRotation);
	static const FName SplineCurvesName     = GET_MEMBER_NAME_CHECKED(USplineComponent, SplineCurves);

	const bool bNeedsFixup =
		TouchedProperty == nullptr ||
		TouchedName == AbsolutePositionName ||
		TouchedName == FocalLengthName ||
		TouchedName == ApertureName ||
		TouchedName == FocusDistanceName ||
		TouchedName == PointRotationName ||
		TouchedName == SplineCurvesName;

	if (bNeedsFixup)
	{
		if (UCineSplineComponent* OwningSpline = GetTypedOuter<UCineSplineComponent>())
		{
			const int32 NumOfPoints = OwningSpline->GetNumberOfSplinePoints();
			if (NumOfPoints > 0)
			{
				Fixup(NumOfPoints, OwningSpline);
			}
		}
	}
}
