...
public:
	...
	virtual void RegisterCommands() override
	{
		...
		UI_COMMAND(SetMetadataToLinear, "Linear", "Set spline point to Linear type", EUserInterfaceActionType::RadioButton, FInputChord());
		UI_COMMAND(SetMetadataToCurve, "Curve", "Set spline point to Curve type", EUserInterfaceActionType::RadioButton, FInputChord());
		UI_COMMAND(SetMetadataToConstant, "Constant", "Set spline point to Constant type", EUserInterfaceActionType::RadioButton, FInputChord());
		UI_COMMAND(SetMetadataToCurveClamped, "CurveClamped", "Set spline point to CurveClamped type", EUserInterfaceActionType::RadioButton, FInputChord());
	}
...
public:
	...
	TSharedPtr<FUICommandInfo> SetMetadataToLinear;
	TSharedPtr<FUICommandInfo> SetMetadataToCurve;
	TSharedPtr<FUICommandInfo> SetMetadataToConstant;
	TSharedPtr<FUICommandInfo> SetMetadataToCurveClamped;
};
...
void FCineSplineComponentVisualizer::OnRegister()
{
	...
	SplineComponentVisualizerActions->MapAction(
		Commands.SetMetadataToLinear,
		FExecuteAction::CreateSP(this, &FCineSplineComponentVisualizer::OnSetMetadataTangent, ECineSplineMetadataTangentType::Linear),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FCineSplineComponentVisualizer::IsMetadataTangent, ECineSplineMetadataTangentType::Linear)
	);
	SplineComponentVisualizerActions->MapAction(
		Commands.SetMetadataToCurve,
		FExecuteAction::CreateSP(this, &FCineSplineComponentVisualizer::OnSetMetadataTangent, ECineSplineMetadataTangentType::Curve),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FCineSplineComponentVisualizer::IsMetadataTangent, ECineSplineMetadataTangentType::Curve)
	);
	SplineComponentVisualizerActions->MapAction(
		Commands.SetMetadataToConstant,
		FExecuteAction::CreateSP(this, &FCineSplineComponentVisualizer::OnSetMetadataTangent, ECineSplineMetadataTangentType::Constant),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FCineSplineComponentVisualizer::IsMetadataTangent, ECineSplineMetadataTangentType::Constant)
	);
	SplineComponentVisualizerActions->MapAction(
		Commands.SetMetadataToCurveClamped,
		FExecuteAction::CreateSP(this, &FCineSplineComponentVisualizer::OnSetMetadataTangent, ECineSplineMetadataTangentType::CurveClamped),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FCineSplineComponentVisualizer::IsMetadataTangent, ECineSplineMetadataTangentType::CurveClamped)
	);
}
...
void FCineSplineComponentVisualizer::GenerateContextMenuSections(FMenuBuilder& InMenuBuilder) const
{
	...
	if (GetEditedSplineComponent() != nullptr)
	{
		InMenuBuilder.BeginSection("CineSpline", LOCTEXT("CineSpline", "CineSpline"));
		{
			...
			InMenuBuilder.AddSubMenu(
				LOCTEXT("CineSplineMetadataTangentType", "Metadata Tangent Type"),
				LOCTEXT("CineSplineMetadataTangentTypeTooltip", "Define the type of the metadata tangent"),
				FNewMenuDelegate::CreateSP(this, &FCineSplineComponentVisualizer::GenerateMetadataTangentTypeSubMenu)
			);
		}
...
void FCineSplineComponentVisualizer::GenerateMetadataTangentTypeSubMenu(FMenuBuilder& MenuBuilder) const
{
	if (const UCineSplineComponent* SplineComp = Cast<UCineSplineComponent>(GetEditedSplineComponent()))
	{
		MenuBuilder.AddMenuEntry(FCineSplineComponentVisualizerCommands::Get().SetMetadataToLinear);
		MenuBuilder.AddMenuEntry(FCineSplineComponentVisualizerCommands::Get().SetMetadataToCurve);
		MenuBuilder.AddMenuEntry(FCineSplineComponentVisualizerCommands::Get().SetMetadataToConstant);
		MenuBuilder.AddMenuEntry(FCineSplineComponentVisualizerCommands::Get().SetMetadataToCurveClamped);
	}
}

void FCineSplineComponentVisualizer::OnSetMetadataTangent(ECineSplineMetadataTangentType TangentType)
{
	if (UCineSplineComponent* SplineComp = Cast<UCineSplineComponent>(GetEditedSplineComponent()))
	{
		check(SelectionState);
		...
		for (int32 SelectedKeyIndex : SelectedKeys)
		{
			...
			SplineComp->CineSplineMetadata->SetTangentType(SelectedKeyIndex, TangentType);
		}
		SplineComp->CineSplineMetadata->AutoSetTangents();
	}
}

bool FCineSplineComponentVisualizer::IsMetadataTangent(ECineSplineMetadataTangentType TangentType)
{
	if (IsKeySelectionValid())
	{
		UCineSplineComponent* SplineComp = Cast<UCineSplineComponent>(GetEditedSplineComponent());
		check(SplineComp != nullptr);
		check(SelectionState);

		EInterpCurveMode Mode = ConvertCineSplineMetadataTangentTypeToInterpCurveMode(TangentType);
		if(Mode != CIM_Unknown)
		{
			const TSet<int32>& SelectedKeys = SelectionState->GetSelectedKeys();
			for (int32 SelectedKeyIndex : SelectedKeys)
			{
				check(SelectedKeyIndex >= 0);
				check(SelectedKeyIndex < SplineComp->GetNumberOfSplinePoints());
				const auto& SelectedPoint = SplineComp->CineSplineMetadata->FocalLength.Points[SelectedKeyIndex];
