...
void FCineSplineMetadataDetails::Update(USplineComponent* InSplineComponent, const TSet<int32>& InSelectedKeys)
{
	...
	for (const int32 Index : InSelectedKeys)
	{
		...
		bUpdateInterpType = bUpdateInterpType ? UpdateMultipleValue(InterpTypeValue, ConvertInterpCurveModeToCineSplineMetadataTangentType(Metadata->PointRotation.Points[Index].InterpMode)) : false;
	}
}
...
FText FCineSplineMetadataDetails::GetInterpType() const
{
	if (InterpTypeValue.IsSet())
	{
		const UEnum* InterpTypeEnum = StaticEnum<ECineSplineMetadataTangentType>();
		return InterpTypeEnum->GetDisplayNameTextByValue((int64)InterpTypeValue.GetValue());
	}
	return LOCTEXT("MultipleTypes", "Multiple Types");
}

void FCineSplineMetadataDetails::OnInterpTypeChanged(TSharedPtr<FString> NewValue, ESelectInfo::Type SelectInfo)
{
	...
	{
		const UEnum* InterpTypeEnum = StaticEnum<ECineSplineMetadataTangentType>();
		const int64 InterpType =InterpTypeEnum->GetValueByNameString(*NewValue);
		if (InterpType != INDEX_NONE)
		{
			EInterpCurveMode Mode = ConvertCineSplineMetadataTangentTypeToInterpCurveMode(static_cast<ECineSplineMetadataTangentType>(InterpType));

			const FScopedTransaction Transaction(LOCTEXT("SetSplineMetadataInterpType", "Set spline metadata interp type"));
			Metadata->Modify();
			for (int32 Index : SelectedKeys)
			{
				Metadata->SetTangentType(Index, static_cast<ECineSplineMetadataTangentType>(InterpType));
			}
			Metadata->AutoSetTangents();
...
void FCineSplineMetadataDetails::UpdateInterpTypes()
{
	const UEnum* InterpTypeEnum = StaticEnum<ECineSplineMetadataTangentType>();
	InterpTypes.Reset();
	for (int32 Index = 0; Index < InterpTypeEnum->NumEnums() - 1; ++Index)
	{
		InterpTypes.Add(MakeShareable(new FString(InterpTypeEnum->GetNameStringByIndex(Index))));
	}
}

TSharedRef<SWidget> FCineSplineMetadataDetails::OnGenerateInterpTypeComboWidget(TSharedPtr<FString> InComboString)
{
...
void FCineSplineMetadataDetails::OnInterpTypeComboWidgetOpened()
{
	TSharedPtr<FString> SelectedItem;
	for (TSharedPtr<FString> Item : InterpTypes)
	{
		if (Item->Equals(GetInterpType().ToString()))
		{
			SelectedItem = Item;
		}
	}
	InterpTypeComboBox->SetSelectedItem(SelectedItem);
}
...
void FCineSplineMetadataDetails::GenerateChildContent(IDetailGroup& DetailGroup)
{
	...
	DetailGroup.AddWidgetRow()
		...
		[
			SNew(STextBlock)
				.Text(LOCTEXT("InterpType", "Interp Type"))
				.Font(IDetailLayoutBuilder::GetDetailFont())
		]
		.ValueContent()
		.MinDesiredWidth(125.0f)
		.MaxDesiredWidth(125.0f)
		[
			SAssignNew(InterpTypeComboBox, SComboBox<TSharedPtr<FString>>)
				.OptionsSource(&InterpTypes)
				.OnGenerateWidget(this, &FCineSplineMetadataDetails::OnGenerateInterpTypeComboWidget)
				.OnSelectionChanged(this, &FCineSplineMetadataDetails::OnInterpTypeChanged)
				.OnComboBoxOpening(this, &FCineSplineMetadataDetails::OnInterpTypeComboWidgetOpened)
				[
					SNew(STextBlock)
						.Font(IDetailLayoutBuilder::GetDetailFont())
						.Text(this, &FCineSplineMetadataDetails::GetInterpType)
				]
		];
}
