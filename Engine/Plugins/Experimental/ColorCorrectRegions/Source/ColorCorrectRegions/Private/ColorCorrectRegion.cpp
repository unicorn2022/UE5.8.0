...
void AColorCorrectRegion::HandleAffectedActorsPropertyChange(uint32 ActorListChangeType)
{
	...
	if (ActorListChangeType == EPropertyChangeType::ArrayClear
		...
		|| ActorListChangeType == EPropertyChangeType::Unspecified)
	{
		...
		|| ActorListChangeType == EPropertyChangeType::Unspecified)
	{
		// In case user assigns Color Correct Region or Window, we should remove it as it is invalid operation.
		{
			...
			{
				UE_LOGF(ColorCorrectRegions, Warning, "Color Correct Region or Window assignment to Per Actor CC is not supported.");
			}
	...
	// Notify connected multi-user clients of the updated AffectedActors list via a Concert custom event
	if (ColorCorrectRegionsSubsystem.IsValid())
	{
		ColorCorrectRegionsSubsystem->NotifyPerActorCCChanged(this);
	}
}
...
void AColorCorrectRegion::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	// Use MemberProperty (the class-level field) rather than the leaf property name.
	// When resetting an array element to default, the leaf property is the inner element type,
	// not "AffectedActors" itself, so GetPropertyName() would miss it.
	const FName MemberPropertyName = PropertyChangedEvent.MemberProperty
		? PropertyChangedEvent.MemberProperty->GetFName()
		: PropertyName;

	if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(AColorCorrectRegion, AffectedActors))
	{
		HandleAffectedActorsPropertyChange(PropertyChangedEvent.ChangeType);
	}
...
void AColorCorrectRegion::PostTransacted(const FTransactionObjectEvent& TransactionEvent)
{
	...
	// PerAffectedActorStencilData is non-UPROPERTY so the transaction system never restores it.
	// Clear it on undo/redo so it stays consistent with the transaction-restored AffectedActors.
	// The next stencil assignment will repopulate it.
	if (TransactionEvent.GetEventType() == ETransactionObjectEventType::UndoRedo)
	{
		PerAffectedActorStencilData.Empty();
	}
}
#endif //WITH_EDITOR
