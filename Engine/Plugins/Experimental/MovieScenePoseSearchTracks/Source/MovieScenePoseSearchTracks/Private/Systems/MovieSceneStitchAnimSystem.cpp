...
namespace UE::MovieScene
{
	...
	struct FUpdateStitchAnimTask
	{
		...
		const UObject* GetContextObject(TInstancedStruct<FMovieSceneMixedAnimationTarget> Target, const UObject* BoundObject) const
		{
			...
			{
				ContextObject = TryFindComponent.template operator()<UUAFComponent>(BoundObject);
			}
			else if (!Target.IsValid() || Target.GetScriptStruct() == FMovieSceneMixedAnimationTarget::StaticStruct())
			{
				...
				const UUAFComponent* AnimNextComponent = TryFindComponent.template operator()<UUAFComponent>(BoundObject);
			...
			if (!ContextObject)
			{
				if (const USkeletalMeshComponent* MeshComponent = TryFindComponent.template operator()<USkeletalMeshComponent>(BoundObject))
				{
					ContextObject = MeshComponent->GetAnimInstance();
				}
			}
