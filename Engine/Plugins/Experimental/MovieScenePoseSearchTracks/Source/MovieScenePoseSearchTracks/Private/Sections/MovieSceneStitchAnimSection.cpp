...
void FMovieSceneStitchAnimEvaluationTask::Execute(UE::UAF::FEvaluationVM& VM) const
{
	...
	const UAnimInstance* AnimInstance = Cast<UAnimInstance>(AnimContext);
	if (PoseHistory == nullptr && AnimInstance != nullptr && !StitchData.OverridePoseHistoryTag.IsNone())
	{
		if (const FAnimNode_PoseSearchHistoryCollector_Base* PoseHistoryNode = UPoseSearchLibrary::FindPoseHistoryNode(StitchData.OverridePoseHistoryTag, AnimInstance))
		{
	...
	// CurrentMeshTransform converted back to world actor space, used only for VLOG debugging below.
	...
	const bool bUseRawData = FDecompressionTools::ShouldUseRawData(AnimSequence, ExtractionContext);
	...
	if (EnumHasAnyFlags(VM.GetFlags(), EEvaluationFlags::Bones))
	{
		...
		// Stash the warped world-space transform on the root bone so the standard
		// FAnimNextExtractRootMotionTask will lift it onto RootTransformAttribute
		// (and zero the bone) the same way it does for every other section.
		const int32 RootIndex = Keyframe.Pose.GetRefPose().GetLODBoneIndexFromSkeletonBoneIndex(0);
		if (RootIndex != INDEX_NONE)
		{
			Keyframe.Pose.LocalTransformsView[RootIndex] = CurrentMeshTransform;
		}
	}
...
void UMovieSceneStitchAnimSection::ImportEntityImpl(UMovieSceneEntitySystemLinker* EntityLinker, const FEntityImportParams& InParams, FImportedEntity* OutImportedEntity)
{
	...
	StitchData.OverridePoseHistoryTag = OverridePoseHistoryTag;
	...
	OutImportedEntity->AddBuilder(
		...
		.Add(AnimMixerComponents->Target, TInstancedStruct<FMovieSceneMixedAnimationTarget>::Make())
		.Add(AnimMixerComponents->MixerEntry, nullptr)
		.Add(AnimMixerComponents->Priority, 0)
	);
}
