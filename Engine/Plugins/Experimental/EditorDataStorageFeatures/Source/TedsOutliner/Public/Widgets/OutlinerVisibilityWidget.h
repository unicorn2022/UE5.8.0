...
protected:
	...
	/** Returns whether the item on the given row can commit visibility */
	virtual bool CanCommitVisibility(UE::Editor::DataStorage::ICoreProvider& DataStorage, RowHandle Row) const;
	...
	UE_API void SetIsVisible(RowHandle InRow, const bool bVisible, 
		UE::Editor::DataStorage::FRowHandleArray* ChangedRowsToAppendTo = nullptr);
...
private:
	...
	UE_API void CommitVisibility(UE::Editor::DataStorage::ICoreProvider& DataStorage, RowHandle Row, bool bVisible, 
		UE::Editor::DataStorage::FRowHandleArray* ChangedRowsToAppendTo);

	// Recursive internal method to set visibility on a row and all its children.
	UE_API void SetVisibility_Recursive(UE::Editor::DataStorage::ICoreProvider& DataStorage, RowHandle Row,
		TSharedPtr<UE::Editor::DataStorage::IHierarchyViewerDataInterface> HierarchyData, bool bVisible,
		UE::Editor::DataStorage::FRowHandleArray* ChangedRowsToAppendTo);

	// Legacy method to work on Outliner tree items since they don't use the new hierarchy interface yet.
	UE_API void SetVisibility_Recursive(UE::Editor::DataStorage::ICoreProvider& DataStorage, FSceneOutlinerTreeItemPtr TreeItem, bool bVisible,
		UE::Editor::DataStorage::FRowHandleArray* ChangedRowsToAppendTo);
	...
	TWeakObjectPtr<const UScriptStruct> EditorObjectHierarchyParentTag;
};
