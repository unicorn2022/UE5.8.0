...
FText GetComponentMobility(const EComponentMobility::Type InMobility)
{
	switch(InMobility)
	{
...
{
	void FMobilityWidgetSearcher::GetSearchableString(FString& SearchableString, const ICoreProvider& Storage, const FTedsActorMobilityColumn& Column) const
	{
		SearchableString = GetComponentMobility(Column.Mobility).ToString();
	}
}
			...
			[
				SNew(STextBlock)
					...
					.Text(Binder.BindData(&FTedsActorMobilityColumn::Mobility, [](EComponentMobility::Type InMobility)
						{
							return GetComponentMobility(InMobility);
						}))
					.HighlightText(UE::Editor::Outliner::Helpers::GetHighlightTextAttribute(DataStorage, WidgetRow))
			];
}

TArray<TSharedPtr<const UE::Editor::DataStorage::FColumnSearcherInterface>> FOutlinerMobilityWidgetConstructor::ConstructColumnSearchers(
	UE::Editor::DataStorage::ICoreProvider* DataStorage,
	UE::Editor::DataStorage::IUiProvider* DataStorageUi,
	const UE::Editor::DataStorage::FMetaDataView& Arguments)
{
	...
	return TArray<TSharedPtr<const FColumnSearcherInterface>>(
		{
			MakeShared<FMobilityWidgetSearcher>()
		});
}
