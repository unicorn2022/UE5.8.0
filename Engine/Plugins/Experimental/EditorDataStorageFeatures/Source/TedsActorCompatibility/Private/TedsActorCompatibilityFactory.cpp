...
void UTedsActorCompatibilityFactory::RegisterTable(UE::Editor::DataStorage::ICoreProvider& DataStorage)
{
	...
	TableHandle UObjectTable = DataStorage.FindTable("Editor_StandardUObjectTable");
	checkf(DataStorage.IsTableValid(UObjectTable), TEXT("Actor factor is unable to retrieve the UObject table, "
		"which is a required template for the actor table."))

	StandardActorTable = DataStorage.RegisterTable<
		FTypedElementLabelColumn, FTypedElementLabelHashColumn, FTedsActorMobilityColumn, FTypedElementActorTag, FLevelColumn, FGuidColumn>(
		FName("Editor_StandardActorTable"), FTableRegistrationOptions{ .SourceTable = UObjectTable });
