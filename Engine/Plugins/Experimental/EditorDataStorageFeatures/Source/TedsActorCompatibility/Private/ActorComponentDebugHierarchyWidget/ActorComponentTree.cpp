	...
	void RegisterActionComponentDebugHierarchyWidget()
	{
		...
		FTabSpawnerEntry& TabEntry = FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
			...
			FOnSpawnTab::CreateLambda([](const FSpawnTabArgs& Args)
			{
				...
				FHierarchyHandle HierarchyHandle = DataStorageInterface->FindHierarchyByName(UTedsEditorHierarchyFactory::EditorObjectHierarchyName);

				if (DataStorageInterface->IsValidHierarchyHandle(HierarchyHandle))
				{
					...
					TSharedPtr<QueryStack::IRowNode> TopLevelRowNode = MakeShared<QueryStack::FRowQueryCallbackResultsNode>(
						*DataStorageInterface,
						QueryNode,
						QueryStack::FRowQueryResultsNode::ESyncActions::RefreshOnUpdate,
						FilterRowsBySelectionSet
						);

					// Use the actor-component hierarchy to enumerate the components of all the selected actors
					TSharedPtr<QueryStack::IRowNode> AllRowNode = MakeShared<QueryStack::FHierarchyRowNode>(
						*DataStorageInterface,
						HierarchyHandle,
						TopLevelRowNode,
						QueryStack::FHierarchyRowNode::ESyncFlags::Always);
