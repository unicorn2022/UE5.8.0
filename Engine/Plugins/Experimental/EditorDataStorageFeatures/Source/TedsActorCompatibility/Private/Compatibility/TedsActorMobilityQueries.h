...
private:
	...
	/**
	 * Takes each actor's IsLockLocation status, and adds/removes a FTypedElementLockTransformTag
	 *  in the Data Storage if FTypedElementSyncFromWorldTag is set.
	 */
	void RegisterLockTransformActorToTagQuery(UE::Editor::DataStorage::ICoreProvider& DataStorage) const;

	/**
	 * Takes the FTypedElementLockTransformTag presence/absence in the Data Storage and calls
	 *  SetLockLocation on the actor if the FTypedElementSyncBackToWorldTag has been set and
	 *  the lock status differs.
	 */
	void RegisterLockTransformTagToActorQuery(UE::Editor::DataStorage::ICoreProvider& DataStorage) const;
};
