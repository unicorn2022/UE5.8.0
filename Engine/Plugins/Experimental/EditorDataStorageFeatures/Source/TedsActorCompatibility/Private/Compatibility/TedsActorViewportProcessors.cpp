		...
		FConsoleCommandWithArgsAndOutputDeviceDelegate::CreateLambda([](const TArray<FString>& Args, FOutputDevice& Output)
			{
				...
				if (ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName))
				{
					...
					for (RowHandle Row : RowHandles)
					{
						...
						DataStorage->AddColumns<FTypedElementViewportOverlayColorDirtyFlag>(Row);
					}
				}
			}));
...
void UActorViewportDataStorageFactory::RegisterOverlayColorColumnToActor(UE::Editor::DataStorage::ICoreProvider& DataStorage)
{
	...
	auto RefreshActorOverlayColor = [](FTypedElementUObjectColumn& Actor, const FTypedElementViewportOverlayColorColumn& ViewportColor)
		{
			if (AActor* ActorInstance = Cast<AActor>(Actor.Object); ActorInstance != nullptr)
			{
	...
	DataStorage.RegisterQuery(
		Select(
			TEXT("Refresh viewport overlay color column to actor"),
			FProcessor(EQueryTickPhase::PostPhysics, DataStorage.GetQueryTickGroupName(EQueryTickGroups::Update))
				.SetExecutionMode(EExecutionMode::GameThread),
				RefreshActorOverlayColor
			)
		.Where()
			.All<FTypedElementActorTag, FTypedElementViewportOverlayColorDirtyFlag>()
		.Compile());

	DataStorage.RegisterQuery(
		Select(
			TEXT("Remove viewport overlay dirty flag to actor"),
			FProcessor(EQueryTickPhase::FrameEnd, DataStorage.GetQueryTickGroupName(EQueryTickGroups::Update))
				.SetExecutionMode(EExecutionMode::GameThread),
				[](IQueryContext& ActorQueryContext, RowHandle ObjectRow)
				{
					ActorQueryContext.RemoveColumns<FTypedElementViewportOverlayColorDirtyFlag>(ObjectRow);
				}
			)
		.Where()
			.All<FTypedElementActorTag, FTypedElementViewportOverlayColorDirtyFlag>()
