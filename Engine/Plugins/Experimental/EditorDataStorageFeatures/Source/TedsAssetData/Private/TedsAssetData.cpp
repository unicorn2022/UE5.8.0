	...
	: Database(InDatabase)
{
	...
	AssetRegistry.OnScanStarted().AddRaw(this, &FTedsAssetData::OnARScanStarted);
	AssetRegistry.OnScanEnded().AddRaw(this, &FTedsAssetData::OnARScanEnded);

	// Start processing immediately if Asset Registry is not gathering and has not completed its initial scan
	// Otherwise wait for OnScanEnded
	if (AssetRegistry.IsSearchAllAssets() && !AssetRegistry.IsGathering())
	{
		StartProcessing();
	}
}
...
FTedsAssetData::~FTedsAssetData()
{
	...
	if (!IsEngineExitRequested())
	{
		...
		if (IAssetRegistry* AssetRegistry = IAssetRegistry::Get())
		{
			...
			AssetRegistry->OnScanStarted().RemoveAll(this);
			AssetRegistry->OnScanEnded().RemoveAll(this);
...
void FTedsAssetData::Initialize()
{
	using namespace UE::Editor::DataStorage::Queries;
	TRACE_CPUPROFILER_EVENT_SCOPE(FTedsAssetData::Initialize);
	...
	TQueue<FAssetData, EQueueMode::Mpsc> CachedAssets;
	std::atomic<int32> CachedAssetsCount = 0;
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FTedsAssetData::Initialize_EnumerateAllAssets);
		AssetRegistry.EnumerateAllAssets([&CachedAssets, &CachedAssetsCount](const FAssetData& AssetData)
			{
				...
				{
					CachedAssets.Enqueue(AssetData);
					++CachedAssetsCount;
				}
				return true;
			}, UE::AssetRegistry::EEnumerateAssetsFlags::Parallel);
	}


	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FTedsAssetData::Initialize_DrainQueue);
		AddedAssets.Reserve(AddedAssets.Num() + CachedAssetsCount);
		FAssetData AssetDataTemp;
		while (CachedAssets.Dequeue(AssetDataTemp))
		{
			AddedAssets.Add(MoveTemp(AssetDataTemp));
		}
	}
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FTedsAssetData::Initialize_EnumerateAllCachedPaths);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FTedsAssetData::Initialize_CachedPaths.MoveToLinearArray);
...
void FTedsAssetData::StartProcessing()
{
	if (!bIsInitialized)
	{
		// Do initial bulk asset grab and register to Asset Registry events
		Initialize();
	}
	bIsPaused = false;
}

void FTedsAssetData::PauseProcessing()
{
	bIsPaused = true;
}
...
void FTedsAssetData::OnARScanStarted()
{
	PauseProcessing();
}

void FTedsAssetData::OnARScanEnded()
{
	StartProcessing();
}
...
void FTedsAssetData::OnDatabaseUpdate()
{
	...
	// Asset Registry is currently gathering. Don't process batches at this time to avoid competing on the Game Thread
	if (bIsPaused)
	{
		return;
	}
...
void FTedsAssetData::RecordAssetDataStats()
{
	...
	SET_DWORD_STAT(STAT_TedsAssetData_AssetAddBatchSize, LastAddedAssetBatchSize);
	SET_DWORD_STAT(STAT_TedsAssetData_AssetUpdateBatchSize, LastUpdatedAssetBatchSize);
