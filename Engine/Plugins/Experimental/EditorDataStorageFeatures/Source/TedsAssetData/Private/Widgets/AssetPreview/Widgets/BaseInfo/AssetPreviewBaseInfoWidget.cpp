	...
	const UE::Editor::DataStorage::FMetaDataView& Arguments)
{
	...
	if (DataStorage->HasColumns<FAssetNameColumn>(TargetRow))
	{
		...
		IUiProvider::FWidgetConstructorPtr OutNameWidgetConstructorPtr;
		auto AssignNameWidgetToColumn = [&OutNameWidgetConstructorPtr] (IUiProvider::FWidgetConstructorPtr WidgetConstructor, TConstArrayView<TWeakObjectPtr<const UScriptStruct>>)
		{
			OutNameWidgetConstructorPtr = MoveTemp(WidgetConstructor);;
			return false;
		};
	...
	{
		if (FAssetDataColumn_Experimental* AssetDataColumn = DataStorage->GetColumn<FAssetDataColumn_Experimental>(TargetRow))
		{
			FAssetData AssetData = AssetDataColumn->AssetData;
