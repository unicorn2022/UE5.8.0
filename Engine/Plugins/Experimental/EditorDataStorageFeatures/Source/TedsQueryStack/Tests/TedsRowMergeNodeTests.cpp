...
TEST_CASE_NAMED(TEDS_QueryStack_RowMergeNode_Tests, "Editor::DataStorage::QueryStack::FRowMergeNode", "[ApplicationContextMask][EngineFilter]")
{
	...
	SECTION("No parents")
	{
		TSharedPtr<FRowMergeNode> Node = MakeShared<FRowMergeNode>(TConstArrayView<TSharedPtr<IRowNode>>(), FRowMergeNode::EMergeApproach::Append);
		...
		FExplicitUpdateExecutor Executor("NoParentsTestExecutor", Node);
		Executor.Update();
	}
