...
namespace UE::Editor::DataStorage::Testing
{
	...
	void UpdateNode(
		UE::Editor::DataStorage::ICoreProvider& DataStorage, const TSharedPtr<UE::Editor::DataStorage::QueryStack::FRowSortNode>& Node)
	{
		using namespace UE::Editor::DataStorage::QueryStack;
		FExplicitCooperativeExecutor Executor("RowSortNodeTestExecutor", Node);
		
		uint64 StartTime = FPlatformTime::Cycles64();
		do
		{
			Executor.Update(FTimespan::FromMilliseconds(10));
