...
namespace UE::Editor::DataStorage::QueryStack
{
	...
	void FRowSortNode::Update(FTimespan AllottedTime)
	{
		...
		if (SortContext)
		{
			...
			{
				// If the update is uncapped then limit to the requested upper limit as otherwise the editor can lock up
				// for a long time. If there's explicit time assigned, use as much of it as needed, even if it exceeds
				// the maximum duration that's specified because TEDS is telling this function that's the amount of
				// time that's available.
				SortContext->Tasks.Update(AllottedTime == INode::UnlimitedTime ? MaxFrameDuration : AllottedTime);
			}
	...
	void FRowSortNode::VisitParents(ParentListCallback Callback)
	{
		Callback(Parent);
	}
