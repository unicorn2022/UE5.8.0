...
namespace UE::Editor::DataStorage::QueryStack
{
	FRowMonitorNode::FRowMonitorNode(
		...
		, ParentNode(MoveTemp(InParentNode))
		, Storage(InStorage)
	{
		checkf(ParentNode, TEXT("FRowMonitorNode requires a valid ParentNode."));
	...
	FRowMonitorNode::FRowMonitorNode(
		...
		: MonitoredColumns(MoveTemp(InColumns))
		, ParentNode(MoveTemp(InParentNode))
		, Storage(InStorage)
	{
		checkf(ParentNode, TEXT("FRowMonitorNode requires a valid ParentNode."));
	...
	FRowMonitorNode::FRowMonitorNode(
		...
		: MonitoredColumns(MoveTemp(InMonitoredColumns))
		, QueryNode(MoveTemp(InQueryNode))
		, ParentNode(MoveTemp(InParentNode))
		, Storage(InStorage)
		, bFixedColumns(true)
	...
	void FRowMonitorNode::Update(FTimespan AllottedTime)
	{
		if (QueryNode && QueryNode->GetRevision() != QueryRevision)
		{
			UpdateColumnsFromQuery();
			UpdateMonitoredColumns();
			Revision++;
		...
		if (ParentNode && ParentNode->GetRevision() != ParentRevision)
		{
	...
	void FRowMonitorNode::VisitParents(ParentListCallback Callback)
	{
		if (QueryNode)
		{
			Callback(QueryNode);
	...
	void FRowMonitorNode::UpdateColumnsFromQuery()
	{
		...
		if (!bFixedColumns)
		{
			...
			{
				MonitoredColumns.Empty();
				QueryRevision = UninitializedRevisionId;
			}
		}
	}
	...
	void FRowMonitorNode::UpdateMonitoredColumns()
	{
		...
		Observers.Reset(MonitoredColumns.Num() * 2); // Add double, one for the OnAdd and one for the OnRemove.
