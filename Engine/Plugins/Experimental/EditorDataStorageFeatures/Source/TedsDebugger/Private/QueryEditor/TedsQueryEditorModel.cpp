...
namespace UE::Editor::DataStorage::Debug::QueryEditor
{
	...
	void FTedsQueryEditorModel::SetHierarchy(const FName& InHierarchy)
	{
		...
		CurrentRelationType.Reset();
		TableViewerRefreshRequiredDelegate.Broadcast();
	}

	void FTedsQueryEditorModel::SetShowHiddenRows(bool bInShowHiddenRows)
	{
		bShowHiddenRows = bInShowHiddenRows;
		TableViewerRefreshRequiredDelegate.Broadcast();
	}

	bool FTedsQueryEditorModel::GetShowHiddenRows() const
	{
		return bShowHiddenRows;
	}

	void FTedsQueryEditorModel::SetRelationType(RelationTypeHandle InRelationType)
	{
		CurrentRelationType = InRelationType;
		CurrentHierarchy = NAME_None;
		TableViewerRefreshRequiredDelegate.Broadcast();
	}

	bool FTedsQueryEditorModel::HasRelationType() const
	{
		return CurrentRelationType.IsSet();
	}

	RelationTypeHandle FTedsQueryEditorModel::GetRelationType() const
	{
		return CurrentRelationType.Get(InvalidRelationTypeHandle);
	}
