	...
	const UE::Editor::DataStorage::FMetaDataView& Arguments)
{
	...
	if (DataStorage->IsRowAvailable(TargetRow))
	{
		...
		const RowHandle ParentWidgetRowHandle = TableViewerUtils::GetTableViewerUiRow(DataStorage, WidgetRow);
...
TSharedPtr<SWidget> FContentBrowserTileViewWidgetConstructor::CreateThumbnailWidget(FTedsTileViewCommonArguments InTedsTileViewCommonArguments, UE::Editor::DataStorage::RowHandle& OutThumbnailWidgetRowHandle) const
{
	...
	{
		OutThumbnailWidgetConstructorPtr = WidgetConstructor;
		return false;
	};
	...
	if (OutThumbnailWidgetConstructorPtr)
	{
		...
		if (OutThumbnailWidgetRowHandle != InvalidRowHandle)
		{
			...
			{
				.OnGetWidgetSlateBrush = MakeShared<FOnGetWidgetSlateBrush>(FOnGetWidgetSlateBrush::CreateLambda([this, InTedsTileViewCommonArguments] () { return GetAssetThumbnailBorderOverride(InTedsTileViewCommonArguments); }))
			});
...
TSharedPtr<SWidget> FContentBrowserTileViewWidgetConstructor::CreateItemTypeWidget(FTedsTileViewCommonArguments InTedsTileViewCommonArguments, UE::Editor::DataStorage::RowHandle& OutItemTypeWidgetRowHandle) const
{
	...
	IUiProvider::FWidgetConstructorPtr OutItemTypeWidgetConstructorPtr;

	auto AssignItemTypeWidgetToColumn = [&OutItemTypeWidgetConstructorPtr] (IUiProvider::FWidgetConstructorPtr WidgetConstructor, TConstArrayView<TWeakObjectPtr<const UScriptStruct>>)
	{
		OutItemTypeWidgetConstructorPtr = MoveTemp(WidgetConstructor);;
		return false;
	};
	...
	if (OutItemTypeWidgetConstructorPtr)
	{
		...
		if (OutItemTypeWidgetRowHandle != InvalidRowHandle)
		{
			...
			InTedsTileViewCommonArguments.DataStorage->AddColumn(OutItemTypeWidgetRowHandle, FOnGetWidgetColorAndOpacityColumn_Experimental
				{
					.OnGetWidgetColorAndOpacity = MakeShared<FOnGetWidgetColorAndOpacity>(FOnGetWidgetColorAndOpacity::CreateLambda([this, InTedsTileViewCommonArguments] { return GetAssetClassLabelTextColor(InTedsTileViewCommonArguments); }))
				});
