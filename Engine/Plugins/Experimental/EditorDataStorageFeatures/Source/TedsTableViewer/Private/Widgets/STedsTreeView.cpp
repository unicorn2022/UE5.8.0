...
namespace UE::Editor::DataStorage
{
	...
	void STedsTreeView::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
	{
		...
		{
			UpdateTreeMap();
			// The list refresh will invalidate the layout in STableViewBase::RequestLayoutRefresh()
			// and we want to defer that until after we update the tree map.
			// We are still marking the base class for refresh here prior to it's next tick which we call below.
