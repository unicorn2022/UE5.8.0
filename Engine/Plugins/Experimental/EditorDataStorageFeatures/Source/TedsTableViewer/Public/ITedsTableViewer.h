...
namespace UE::Editor::DataStorage
{
    ...
    public:
    	...
    	virtual void SetQueryStack(TSharedPtr<QueryStack::IRowNode> InRowQueryStack) = 0;

    	// Clear the current list of columns being displayed and set it to the given list
    	virtual void SetColumns(const TArray<TWeakObjectPtr<const UScriptStruct>>& InColumns) = 0;
    	
    	// Sets the Table Identifier (Can be used in case data is dynamic and created post widget init)
        virtual void SetTableViewerIdentifier(const FName& NewIdentifier) = 0;
