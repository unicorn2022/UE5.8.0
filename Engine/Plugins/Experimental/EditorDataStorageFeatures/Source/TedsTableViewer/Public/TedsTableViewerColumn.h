...
namespace UE::Editor::DataStorage
{
	...
	public:
		...
		TEDSTABLEVIEWER_API FTedsTableViewerColumn(
			...
			const IUiProvider::FWidgetConstructorPtr& InCellWidgetConstructor, // The widget constructor to use for this column
			const TArray<TWeakObjectPtr<const UScriptStruct>>& InMatchedColumns = {}, // An optional list of matched Teds columns 
			const IUiProvider::FWidgetConstructorPtr& InHeaderWidgetConstructor = nullptr, // Optional constructor to use for the header widget
			const FMetaData& MetaData = FMetaData(), // Optional generic metadata to use when constructing widgets
			const TArray<TSharedPtr<FColumnMetaData>>& ColumnMetaData = {});  // Optional metadata specific to the matched columns
		...
		TEDSTABLEVIEWER_API void OnGetSearchStringCallback(const RowHandle& InRowHandle, TArray< FString >& OutSearchStrings);
		TEDSTABLEVIEWER_API void SetInitialSortMode(EColumnSortMode::Type InSortMode);
		...
		TEDSTABLEVIEWER_API FComboMetaDataView<FGenericMetaDataView, FColumnsMetaDataView> GetMetaData() const;
	...
	private:
		...
		// Widget Constructors
		IUiProvider::FWidgetConstructorPtr CellWidgetConstructor;
		IUiProvider::FWidgetConstructorPtr HeaderWidgetConstructor;
		...
		const TArray<TSharedPtr<FColumnMetaData>> ColumnMetaData;
		...
		TArray<TSharedPtr<const FColumnSearcherInterface>> ColumnSearchers;
