...
struct FTedsChildAlertColumn final : public FEditorDataStorageColumn
{
	...
	/** How many direct contributing children (leaf alerts or intermediate nodes) have propagated to this row. */
	uint16 ProcessedChildCount = 0;

	/** Expected number of direct contributing children. Set at reset time by the "Count alert children" processor. */
	uint16 NumAlertChildren = 0;
};

/**
 * Column that holds info about the hierarchy an alert is participating in
 */
USTRUCT(meta = (DisplayName = "Alert Hierarchy Column"))
struct FTedsAlertHierarchyColumn final : public FEditorDataStorageColumn
{
	GENERATED_BODY()
	
	UE::Editor::DataStorage::FHierarchyHandle HierarchyHandle;
};
...
namespace UE::Editor::DataStorage::Columns
{
	...
	using FAlertHierarchyColumn = FTedsAlertHierarchyColumn;
