...
{
	static TAutoConsoleVariable<bool> CVarEnableTedsAlerts(
		TEXT("Teds.Feature.Alerts"),
		true,
		TEXT("Enable adding alerts to rows in TEDS"),
		ECVF_Default
	);

	void AddAlert(ICoreProvider& DataStorage, RowHandle TargetRow, const FName& Name, FText Message, 
		Columns::FAlertColumnType Type, uint8 Priority, Columns::FAlertActionCallback Action, const FName& HierarchyName)
	{
		using namespace UE::Editor::DataStorage::Columns;

		if (!CVarEnableTedsAlerts.GetValueOnGameThread())
		{
			return;
		}

		// Add the new alert to the unsorted alert chain table and let TEDS sort is out at an appropriate time.
		if (UTedsAlertsFactory* AlertsFactory = DataStorage.FindFactory<UTedsAlertsFactory>())
		{
			// To prevent duplicates, if there is already an alert with this name on the row remove and re-add it, which acts like an overwrite.
			...
			FHierarchyHandle HierarchyHandle = DataStorage.FindHierarchyByName(HierarchyName);

			if (DataStorage.IsValidHierarchyHandle(HierarchyHandle))
			{
				checkf(DataStorage.GetParentChangedColumnType(HierarchyHandle),
						TEXT("Alerts with hierarchies require a parent changed column to properly detect changes. This can be specified on hierarchy registration."));
				
				AlertsFactory->RegisterAlertHierarchy(DataStorage, HierarchyName);
			}

			DataStorage.AddRow(AlertsFactory->GetUnsortedAlertChainTable(),
				[&DataStorage, TargetRow, Name, Message = MoveTemp(Message), Type, Priority, Action = MoveTemp(Action), HierarchyHandle]
				(RowHandle Row) mutable
				{
					...
					DataStorage.AddColumn(Row, FAlertHierarchyColumn{.HierarchyHandle =  HierarchyHandle});
		...
		Columns::FAlertColumnType Type, uint8 Priority, Columns::FAlertActionCallback Action, const FName& HierarchyName)
	{
		...
		Context.PushCommand([TargetRow, Name, Message = MoveTemp(Message), Type, Priority, Action = MoveTemp(Action), HierarchyName]() mutable
			{
				if (ICoreProvider* DataStorage = GetMutableDataStorageFeature<ICoreProvider>(StorageFeatureName))
				{
					AddAlert(*DataStorage, TargetRow, Name, MoveTemp(Message), Type, Priority, MoveTemp(Action), HierarchyName);
				}
			});
	}
