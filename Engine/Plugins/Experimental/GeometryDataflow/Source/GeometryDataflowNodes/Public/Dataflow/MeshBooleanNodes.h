...
// Enum to select among the input meshes for a Mesh Boolean operation
UENUM(BlueprintType)
enum class EDataflowMeshBooleanInputMeshEnum : uint8
{
	None,
	First,
	Second,
...
private:
	...
	/** Whether to weld common edges at the joins of the mesh boolean */
	UPROPERTY(EditAnywhere, Category = "Boolean", AdvancedDisplay)
	bool bWeldSharedEdges = false;

	/** Whether to simplify the result along new edges */
	UPROPERTY(EditAnywhere, Category = "Simplification")
	bool bSimplifyAlongNewEdges = true;
	
	UPROPERTY(EditAnywhere, Category = "Simplification", AdvancedDisplay, DisplayName="Preserve UV Quality for Mesh", meta=(EditCondition="bSimplifyAlongNewEdges"))
	EDataflowMeshBooleanInputMeshEnum PreserveUVQualityForMesh = EDataflowMeshBooleanInputMeshEnum::First;
