...
void FMediaStreamCustomization::AddCacheCategory(IDetailLayoutBuilder& InDetailBuilder)
{
	...
	TSharedPtr<IPropertyHandle> UseLocalCacheHandle = PlayerConfigHandle->GetChildHandle("bUseLocalCache");
	...
	if (!UseLocalCacheHandle.IsValid() && !CacheAheadHandle.IsValid() && !CacheBehindHandle.IsValid() && !CacheBehindGameHandle.IsValid())
	{
		return;
	}
	...
	if (UseLocalCacheHandle.IsValid())
	{
		UseLocalCacheHandle->MarkHiddenByCustomization();
		MediaCacheCategory.AddProperty(UseLocalCacheHandle);
	}
