...
struct FMediaStreamPlayerConfig
{
	...
	/**
	 * Enables a per-player local sample cache override instead of the default global cache settings.
	 * Currently maps to the ImgMediaPlayer smart cache, with CacheAhead reused as the look-ahead time.
	 */
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category = "Media Stream|Player")
	bool bUseLocalCache = false;
