...
public:
	...
	/**
	 * Fires after the player has been created (so GetPlayer()->GetMediaTexture() is valid) but
	 * before its IMediaStreamPlayer::OnSourceChanged runs. Skipped when the new source is invalid.
	 * Handlers must not re-enter SetSource synchronously.
	 */
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnPreSourceChange, UMediaStream* /*InMediaStream*/);
	MEDIASTREAM_API FOnPreSourceChange& GetOnPreSourceChange();
...
protected:
	...
	/** Fired by ApplySource between player creation and IMediaStreamPlayer::OnSourceChanged. See GetOnPreSourceChange. */
	FOnPreSourceChange OnPreSourceChange;

	/** True while ApplySource is in progress; blocks recursive SetSource calls from delegate handlers. */
	bool bIsApplyingSource = false;
