...
{
	if (bIsApplyingSource)
	{
		// The outer ApplySource owns the Source field and player lifecycle; nested mutation would leave them inconsistent.
		UE_LOGF(LogMediaStream, Warning,
			"UMediaStream::SetSource ignored: called recursively from a delegate handler running inside another SetSource. "
			"Stream=\"%ls\", AttemptedSource={Scheme=%ls, Path=\"%ls\", Object=\"%ls\"}.",
			*GetPathName(),
			*InSource.Scheme.ToString(),
			*InSource.Path,
			*GetNameSafe(InSource.Object));
		return false;
	}
...
UMediaStream::FOnPreSourceChange& UMediaStream::GetOnPreSourceChange()
{
	return OnPreSourceChange;
}
...
{
	TGuardValue<bool> ApplyingGuard(bIsApplyingSource, true);
	...
	if (IMediaStreamPlayer* Player = Cast<IMediaStreamPlayer>(PlayerObject))
	{
		OnPreSourceChange.Broadcast(this);
