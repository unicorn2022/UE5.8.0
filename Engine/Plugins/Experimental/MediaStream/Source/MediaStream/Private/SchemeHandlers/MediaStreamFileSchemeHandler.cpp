...
FMediaStreamSource FMediaStreamFileSchemeHandler::CreateSource(UObject* InOuter, const FString& InPath)
{
	...
	if (!IsValid(InOuter))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Media Stream in FMediaStreamFileSchemeHandler::CreateSource");
		return Source;
	}
...
UMediaPlayer* FMediaStreamFileSchemeHandler::CreateOrUpdatePlayer(const FMediaStreamSchemeHandlerCreatePlayerParams& InParams)
{
	...
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Media Stream in FMediaStreamFileSchemeHandler::CreateOrUpdatePlayer");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Media Path in FMediaStreamFileSchemeHandler::CreateOrUpdatePlayer [%ls]", *MediaObject->GetClass()->GetName());
		return nullptr;
	}
...
FString FMediaStreamFileSchemeHandler::ResolveFilePath(const FString& InPath)
{
	...
	// Shouldn't generate an error since it's default behavior and common code path. Otherwise it breaks tests.
	UE_LOGF(LogMediaStream, Display, "Invalid File Path in FMediaStreamFileSchemeHandler::ResolveFilePath [%ls]", *InPath);
...
UMediaSource* FMediaStreamFileSchemeHandler::CreateMediaSource(UObject* InOuter, const FString& InPath)
{
	...
	if (!MediaSource)
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Media Path in FMediaStreamFileSchemeHandler::CreateMediaSource [%ls]", *FilePath);
		return nullptr;
	}
