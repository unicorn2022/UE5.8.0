...
UMediaStreamObjectHandlerSubsystem* UMediaStreamObjectHandlerSubsystem::Get()
{
	...
	{
		UE_LOGF(LogMediaStream, Error, "UObject system not initialized in UMediaStreamObjectHandlerSubsystem::Get");
		return nullptr;
	}

	if (!GEngine)
	{
		UE_LOGF(LogMediaStream, Error, "Invalid GEngine in UMediaStreamObjectHandlerSubsystem::Get");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogMediaStream, Error, "Unable to find Media Source Object Handler Subsystem in UMediaStreamObjectHandlerSubsystem::Get");
		return nullptr;
	}
