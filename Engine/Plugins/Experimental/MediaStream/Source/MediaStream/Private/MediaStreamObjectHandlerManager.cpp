...
bool FMediaStreamObjectHandlerManager::CanHandleObject(const UClass* InClass) const
{
	if (!IsValid(InClass))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Class in FMediaStreamObjectHandlerManager::CanHandleObject");
		return false;
	}
...
bool FMediaStreamObjectHandlerManager::CanHandleObject(const UObject* InObject) const
{
	if (!IsValid(InObject))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Object in FMediaStreamObjectHandlerManager::CanHandleObject");
		return false;
	}
...
UMediaPlayer* FMediaStreamObjectHandlerManager::CreateOrUpdatePlayer(const FMediaStreamObjectHandlerCreatePlayerParams& InParams) const
{
	...
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Media Stream in FMediaStreamObjectHandlerManager::CreateOrUpdatePlayer");
		return nullptr;
	}

	if (!IsValid(InParams.Source))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Object in FMediaStreamObjectHandlerManager::CreateOrUpdatePlayer");
		return nullptr;
	}
	...
	if (TSharedPtr<IMediaStreamObjectHandler> Handler = GetHandler(InParams.Source->GetClass()))
	{
		...
		if (!MediaPlayer)
		{
			UE_LOGF(LogMediaStream, Error, "Failed to create Media Player in FMediaStreamObjectHandlerManager::CreateOrUpdatePlayer [%ls] [%ls]", *InParams.Source->GetClass()->GetName(), *InParams.Source->GetName());
			return nullptr;
		}
	...
	if (!InParams.bCanOpenSource)
	{
		UE_LOGF(LogMediaStream, Error, "Cannot use Media Assets Module to create player at the moment in FMediaStreamObjectHandlerManager::CreateOrUpdatePlayer [%ls] [%ls]", *InParams.Source->GetClass()->GetName(), *InParams.Source->GetName());
		return nullptr;
	}
	...
	if (IMediaAssetsModule* MediaAssetsModule = FModuleManager::LoadModulePtr<IMediaAssetsModule>("MediaAssets"))
	{
		...
		if (!MediaPlayer)
		{
			UE_LOGF(LogMediaStream, Error, "No player for Object in FMediaStreamObjectHandlerManager::CreateOrUpdatePlayer [%ls] [%ls]", *InParams.Source->GetClass()->GetName(), *InParams.Source->GetName());
			return nullptr;
		}
	...
	UE_LOGF(LogMediaStream, Error, "Unable to get Media Assets Module in FMediaStreamObjectHandlerManager::CreateOrUpdatePlayer [%ls] [%ls]", *InParams.Source->GetClass()->GetName(), *InParams.Source->GetName());
	return nullptr;
}
...
bool FMediaStreamObjectHandlerManager::HasObjectHandler(const UClass* InClass) const
{
	if (!IsValid(InClass))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Class in FMediaStreamObjectHandlerManager::HasObjectHandler");
		return false;
	}
...
bool FMediaStreamObjectHandlerManager::HasObjectHandler(const UObject* InObject) const
{
	if (!IsValid(InObject))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Object in FMediaStreamObjectHandlerManager::HasObjectHandler");
		return false;
	}
...
bool FMediaStreamObjectHandlerManager::RegisterObjectHandler(const UClass* InClass, const TSharedRef<IMediaStreamObjectHandler>& InHandler)
{
	if (!IsValid(InClass))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Class in FMediaStreamObjectHandlerManager::RegisterObjectHandler");
		return false;
	}
...
TSharedPtr<IMediaStreamObjectHandler> FMediaStreamObjectHandlerManager::UnregisterObjectHandler(const UClass* InClass)
{
	if (!IsValid(InClass))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Class in FMediaStreamObjectHandlerManager::UnregisterObjectHandler");
		return nullptr;
	}
