...
TSharedPtr<IMediaStreamSchemeHandler> FMediaStreamSchemeHandlerManager::GetHandlerTypeForScheme(FName InScheme) const
{
	if (InScheme.IsNone())
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Scheme in FMediaStreamSchemeHandlerManager::GetHandlerTypeForScheme");
		return nullptr;
	}
...
UMediaPlayer* FMediaStreamSchemeHandlerManager::CreateOrUpdatePlayer(const FMediaStreamSchemeHandlerCreatePlayerParams& InParams) const
{
	...
	if (!HandlerPtr)
	{
		UE_LOGF(LogMediaStream, Error, "No handler for scheme in FMediaStreamSchemeHandlerManager::CreateOrUpdatePlayer [%ls]", *Source.Scheme.ToString());
		return nullptr;
	}
...
FMediaStreamSource FMediaStreamSchemeHandlerManager::CreateSource(UObject* InOuter, FName InScheme, const FString& InPath) const
{
	if (!IsValid(InOuter))
	{
		UE_LOGF(LogMediaStream, Error, "Invalid Media Stream in FMediaStreamSchemeHandlerManager::GetSourceFromSchemePath");
		return {};
	}
	...
	if (!HandlerPtr)
	{
		UE_LOGF(LogMediaStream, Error, "Missing handler for Scheme in FMediaStreamSchemeHandlerManager::GetMediaSourceFromSchemePath [%ls://%ls]", *InScheme.ToString(), *InPath);
		return {};
	}
