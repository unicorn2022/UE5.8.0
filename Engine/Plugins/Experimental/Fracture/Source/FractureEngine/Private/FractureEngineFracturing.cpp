		...
		const FUniformFractureProcSettings& InUniformFractureProcSettings)
	{
		...
		int32 FirstNewGeometryIndex = CutMultipleWithPlanarCells(VoronoiPlanarCells, 
			*OutGeomCollection, 
			...
			nullptr, 
			Origin, 
			FIslandSplitSettings(InUniformFractureProcSettings.SplitIslands, InUniformFractureProcSettings.CloseVertexDistance, InUniformFractureProcSettings.VertexToSurfaceBridgeDistance));
	...
	if (TUniquePtr<FGeometryCollection> GeomCollection = TUniquePtr<FGeometryCollection>(InOutCollection.NewCopy<FGeometryCollection>()))
	{
		if (InSites.Num() > 0)
		{
				...
				InOutCollection.HasAttribute(FGeometryCollection::BoundingBoxAttribute, FGeometryCollection::GeometryGroup))
			{
				...
				int ResultGeometryIndex = CutMultipleWithPlanarCells(VoronoiPlanarCells, *GeomCollection, TransformSelectionArr, InGrout, InCollisionSampleSpacing, InRandomSeed, InTransform, true, true, nullptr, Origin, IslandSplitSettings);
	...
	TConstArrayView<FTransform> InCutPlaneTransforms)
{
	...
	if (TUniquePtr<FGeometryCollection> GeomCollection = TUniquePtr<FGeometryCollection>(InOutCollection.NewCopy<FGeometryCollection>()))
	{
		...
		int ResultGeometryIndex = CutMultipleWithMultiplePlanes(CuttingPlanes, InternalSurfaceMaterials, *GeomCollection, TransformSelectionArr, GroutVal, CollisionSampleSpacingVal, InRandomSeed, InTransform, true, nullptr, IslandSplitSettings);
	...
	float InCollisionSampleSpacing)
{
	if (TUniquePtr<FGeometryCollection> GeomCollection = TUniquePtr<FGeometryCollection>(InOutCollection.NewCopy<FGeometryCollection>()))
	{
		...
		int ResultGeometryIndex = CutMultipleWithMultiplePlanes(CuttingPlanes, InternalSurfaceMaterials, *GeomCollection, TransformSelectionArr, GroutVal, CollisionSampleSpacingVal, InRandomSeed, FTransform::Identity, true, nullptr, IslandSplitSettings);
	...
	float InCollisionSampleSpacing)
{
	if (TUniquePtr<FGeometryCollection> GeomCollection = TUniquePtr<FGeometryCollection>(InOutCollection.NewCopy<FGeometryCollection>()))
	{
		...
		int32 ResultGeometryIndex = CutMultipleWithPlanarCells(Cells, *GeomCollection, TransformSelectionArr, GroutVal, InPointSpacing, InRandomSeed, InTransform, true, true, nullptr, Origin, IslandSplitSettings);
	...
	const float InCollisionSampleSpacing)
{
	...
	if (TUniquePtr<FGeometryCollection> GeomCollection = TUniquePtr<FGeometryCollection>(InOutCollection.NewCopy<FGeometryCollection>()))
	{
		...
		{
			auto ApplyCut = 
				[&ScatterTransform, &InternalSurfaceMaterials, &GeomCollection, &TransformSelectionArr, InCollisionSampleSpacing, IslandSplitSettings, &ResultGeometryIndex]
				(const UE::Geometry::FDynamicMesh3& CuttingMesh)
			{
				constexpr bool bSetDefaultInternalMaterialsFromCollection = true;
				int32 Index = CutWithMesh(CuttingMesh, ScatterTransform, InternalSurfaceMaterials, *GeomCollection, TransformSelectionArr, InCollisionSampleSpacing, FTransform::Identity, bSetDefaultInternalMaterialsFromCollection, nullptr, IslandSplitSettings);
	...
	const float InCollisionSampleSpacing)
{
	...
	return MeshArrayCutter(MeshTransforms, InOutCollection, InTransformSelection, 
		...
		InRandomSeed, InChanceToFracture, IslandSplitSettings, InCollisionSampleSpacing);
}
	...
	const FUniformFractureSettings& InUniformFractureSettings)
{
	if (TUniquePtr<FGeometryCollection> GeomCollection = TUniquePtr<FGeometryCollection>(InOutCollection.NewCopy<FGeometryCollection>()))
	{
		...
		if (InUniformFractureSettings.GroupFracture)
		{
			...
			UniformFractureProcSettings.CloseVertexDistance = InUniformFractureSettings.CloseVertexDistance;
			UniformFractureProcSettings.VertexToSurfaceBridgeDistance = InUniformFractureSettings.VertexToSurfaceBridgeDistance;
