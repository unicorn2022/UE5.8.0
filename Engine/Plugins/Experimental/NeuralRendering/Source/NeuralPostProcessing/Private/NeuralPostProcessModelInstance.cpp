...
TSharedPtr<UE::NNE::IModelInstanceRDG> CreateNNEModelInstance(UNNEModelData* NNEModelData, FString RuntimeName)
{
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogNeuralPostProcessing, Error, "Can't get %ls runtime.", *RuntimeName);
#endif
		return nullptr;
	}
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogNeuralPostProcessing, Log, "No RDG runtime '%ls' found", *RuntimeName);
#endif
		return nullptr;
	}
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogNeuralPostProcessing, Error, "Cannot create model RDG for Model = %ls, Runtime = %ls", *NNEModelData->GetName(), *RuntimeName);
#endif
		return nullptr;
	}
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogNeuralPostProcessing, Error, "CreateModelRDG failed for Model = %ls, Runtime = %ls", *NNEModelData->GetName(), *RuntimeName);
#endif
		return nullptr;
	}
...
bool UNeuralPostProcessModelInstance::ModifyInputShape(int Dim, int Size)
{
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogNeuralPostProcessing, Error, "Cannot set dimension %d to %d. It is not dynamic. revert back to %d",Dim, Size, NewResolvedInputShape[Dim]);
#endif
		return false;
	}
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogNeuralPostProcessing, Error, "Cannot set dimension %d to %d due to NNE error. Revert back to %d", Dim, Size, CurrentResolvedInputTensorShape.GetData()[Dim]);
#endif
...
void UNeuralPostProcessModelInstance::CreateDefaultNNEModel(UNNEModelData* NNEModelData, FString RuntimeName)
{
...
#if WITH_EDITOR
			UE_LOGF(LogNeuralPostProcessing, Warning, "SetInputTensorShapes Failed for NNE RDG Model = %ls, Runtime = %ls", *NNEModelData->GetName(), *RuntimeName);
#endif
		}
	...
	{
#if WITH_EDITOR
		UE_LOGF(LogNeuralPostProcessing, Log, "Failed to create NNE RDG Model.");
#endif
	}
}
