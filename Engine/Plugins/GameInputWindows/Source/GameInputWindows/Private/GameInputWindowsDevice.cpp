...
GameInputKind FGameInputWindowsInputDevice::GetCurrentGameInputKindSupport() const
{
	...
	if (!bLogOnce)
	{
		...
		{
			UE_LOGF(LogGameInput, Log, "[FGameInputWindowsInputDevice::GetCurrentGameInputKindSupport] Keyboard support was requested, but is not currently supported via the GameInput plugin on Windows.");
		}

		if ((RegisterInputKindMask & GameInputKindMouse) != 0)
		{
			UE_LOGF(LogGameInput, Log, "[FGameInputWindowsInputDevice::GetCurrentGameInputKindSupport] Mouse support was requested, but is not currently supported via the GameInput plugin on Windows.");
		}
		bLogOnce = true;
	}
...
{	
	if (Device)
	{
		if (FGameInputDeviceContainer* Data = GetDeviceData(Device))
		{
			...
			UE_LOGF(LogGameInput, Log, "Game Input Device '%ls' Disconnected Successfully at Input Device ID '%d'", *UE::GameInput::LexToString(Device), Data->GetDeviceId().GetId());
		...
		{
			UE_LOGF(LogGameInput, Error, "Game Input failed to disconnect a device. The Device '%ls' did not have an associated FGameInputWindowsInputDevice!", *UE::GameInput::LexToString(Device));
		}
	}
	else
	{
		UE_LOGF(LogGameInput, Warning, "Game Input failed to disconnect a device. The Device was null! %ls", *UE::GameInput::LexToString(Device));
	}
...
void FGameInputWindowsInputDevice::HandleDeviceConnected(IGameInputDevice* Device, uint64 Timestamp)
{
	...
	UE_LOGF(LogGameInput, Log, "Game Input Device Connected: %ls  of kind: %ls", *UE::GameInput::LexToString(Device), *UE::GameInput::LexToString(Info->supportedInput));
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		FInputDeviceScope InputScope(
			...
			DeviceMapper.GetUserIndexForPlatformUser(UserToAssign),
			FString(UE::GameInput::GetHardwareDeviceIdentifierName(Device)));
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		
		{
			FInputDeviceDescriptor Desc;
			Desc.HardwareDeviceHandle     = DeviceId;
			Desc.InputDeviceName          = UE::GameInput::GetDeviceScopeClassName();
			Desc.HardwareDeviceIdentifier = FName(UE::GameInput::GetHardwareDeviceIdentifierName(Device));
		...
		UE_LOGF(LogGameInput, Log, "Using PlatformUserId %d and InputDeviceId %d for device %ls",
	...
	{
		UE_LOGF(LogGameInput, Log, "Game Input Device %ls had no processors, so it will not be assiged to a platform user. You may need to configure it in the project settings.",
			*UE::GameInput::LexToString(Device));
	}
...
void FGameInputWindowsInputDevice::HandleEditorSettingChanged()
{
	...
	{
		GameInputKind SupportedKind = GetCurrentGameInputKindSupport();

		// Make sure that this device is still supported. If you are in the editor then you may have changed
		// this value and a connected device is no longer supported. If that is the case, then make sure we don't create
		// any new processors for it at all.
		IGameInputDevice* GameInputDevice = Device->GetGameInputDevice();
		if (!IsSupportedDevice(GameInputDevice))
		{
			UE_LOGF(LogGameInput, Log, "'%ls' is not supported (the settings have changed) and input will no longer be processed for it.", *UE::GameInput::LexToString(GameInputDevice));
			SupportedKind = GameInputKind::GameInputKindUnknown;
		}
		
		// Recreate the device processors for every device
		Device->RecreateDeviceProcessors(SupportedKind);
