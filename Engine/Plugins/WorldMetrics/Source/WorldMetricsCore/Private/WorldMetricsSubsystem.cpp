...
{
	UE_LOGF(LogWorldMetrics, Verbose, "[%s]", __FUNCTION__);
...
void UWorldMetricsSubsystem::BeginDestroy()
{
	if (!ensure(!IsEnabled()))
	{
		UE_LOGF(LogWorldMetrics, Error, "[%s] Unexpected subsystem state: IsEnabled", __FUNCTION__);
		Clear();
	}
	Super::BeginDestroy();
}
...
void UWorldMetricsSubsystem::InitializeMetrics()
{
	...
	{
		UE_LOGF(
			LogWorldMetrics, Warning, "[%s] Invalid warm-up frames value: %d, reset to zero.", __FUNCTION__,
			PendingWarmUpFrames);
...
void UWorldMetricsSubsystem::Enable(bool bEnable)
{
	if (bEnable)
	{
		...
		{
			UE_LOGF(
				LogWorldMetrics, Warning, "[%s] Invalid update rate value: %.02fs, reset to zero.", __FUNCTION__,
				UpdateRateInSeconds);
...
void UWorldMetricsSubsystem::SetUpdateRateInSeconds(float InSeconds)
{
	...
	{
		UE_LOGF(
			LogWorldMetrics, Warning, "[%s] Invalid UpdateRateInSeconds input value %.04f: ignored.",
...
UWorldMetricInterface* UWorldMetricsSubsystem::CreateMetric(const TSubclassOf<UWorldMetricInterface>& InMetricClass)
{
	...
	{
		UE_LOGF(LogWorldMetrics, Warning, "[%s] Unexpected null metric class", __FUNCTION__);
		return nullptr;
	}
	...
	{
		UE_LOGF(
			LogWorldMetrics, Warning, "[%s] Parameter metric class is abstract: %ls", __FUNCTION__,
	...
	{
		UE_LOGF(
			LogWorldMetrics, Error, "[%s] Failed to create metric of class: %ls", __FUNCTION__,
			*InMetricClass->GetFName().ToString());
	}
	return Metric;
}
...
bool UWorldMetricsSubsystem::ContainsMetric(UWorldMetricInterface* InMetric) const
{
	...
	{
		UE_LOGF(LogWorldMetrics, Warning, "[%s] Unexpected null metric instance", __FUNCTION__);
		return false;
	}
...
bool UWorldMetricsSubsystem::AddMetric(UWorldMetricInterface* InMetric)
{
	...
	UE_LOGF(
		LogWorldMetrics, Verbose, "[%s] Added metric of class %ls.", __FUNCTION__,
...
bool UWorldMetricsSubsystem::RemoveMetric(UWorldMetricInterface* InMetric)
{
	...
	UE_LOGF(
		LogWorldMetrics, Verbose, "[%s] Removed metric of class %ls", __FUNCTION__,
		*InMetric->GetClass()->GetFName().ToString());
	...
	const TSubclassOf<UWorldMetricsExtension>& InExtensionClass)
{
	if (UNLIKELY(!InOwner))
	{
		UE_LOGF(LogWorldMetrics, Warning, "[%s] Unexpected invalid owner", __FUNCTION__);
		return nullptr;
	}

	if (UNLIKELY(!InExtensionClass))
	{
		UE_LOGF(LogWorldMetrics, Warning, "[%s] Unexpected null extension class", __FUNCTION__);
		return nullptr;
	}
	...
	{
		UE_LOGF(
			LogWorldMetrics, Warning, "[%s] Parameter extension class is abstract: %ls", __FUNCTION__,
	...
	const TSubclassOf<UWorldMetricsExtension>& InExtensionClass)
{
	...
	{
		UE_LOGF(
			LogWorldMetrics, Error, "[%s] Failed to create extension of class: %ls", __FUNCTION__,
			*InExtensionClass->GetFName().ToString());
		return nullptr;
	}
	...
	UE_LOGF(
		LogWorldMetrics, Verbose, "[%s] Added extension of class: %ls", __FUNCTION__,
		*InExtensionClass->GetFName().ToString());
	...
	const TSubclassOf<UWorldMetricsExtension>& InExtensionClass)
{
	...
	{
		UE_LOGF(
			LogWorldMetrics, Error, "[%s] Parameter object doesn't own this extension: %ls", __FUNCTION__,
			*InExtensionClass->GetFName().ToString());
	}
...
bool UWorldMetricsSubsystem::TryRemoveExtension(const TSubclassOf<UWorldMetricsExtension>& InExtensionClass)
{
	...
	if (Extension && Extension->Owners.IsEmpty())
	{
		...
		UE_LOGF(
			LogWorldMetrics, Verbose, "[%s] Removed extension of class %ls", __FUNCTION__,
...
void UWorldMetricsSubsystem::VerifyMetricReleasedAllExtensions(UWorldMetricInterface* InMetric)
{
	...
	for (TPair<TSubclassOf<UWorldMetricsExtension>, FExtension>& ExtensionPair : Extensions)
	{
		if (UNLIKELY(ExtensionPair.Value.Owners.Remove(InMetric)))
		{
			...
			UE_LOGF(
				LogWorldMetrics, Warning, "[%s] World metric %ls did not release extension %ls in Deinitialize",
			...
			for (const TSubclassOf<UWorldMetricsExtension>& ExtensionClass : ExtensionsToRemove)
			{
				if (const FExtension* Extension = Extensions.Find(ExtensionClass))
				{
					...
					UE_LOGF(
						...
						"[%s] World extension %ls was acquired by another extension but never released.",
