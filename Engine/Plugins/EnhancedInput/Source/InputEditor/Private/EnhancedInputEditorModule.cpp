...
#include "EnhancedInputCustomizations.h"
...
namespace UE::Input
{
	static void ShowToast(const FText& ClassBeingChanged, const FString& NewClassName, const FString& OldClassName)
	{
		...
		UE_LOGF(LogEnhancedInputEditor, Log, "Upgrading Default %ls class from '%ls' to '%ls'", *ClassBeingChanged.ToString(), *OldClassName, *NewClassName);
	}
}
