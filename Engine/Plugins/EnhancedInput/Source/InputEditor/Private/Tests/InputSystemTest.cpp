...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FInputSystemContextChangeIgnoreHeldKeysTest, "Input.System.ContextChange.IgnoreHeldKeys", BasicSystemTestFlags)

bool FInputSystemContextChangeIgnoreHeldKeysTest::RunTest(const FString& Parameters)
{
	// Test that a Pressed trigger does not re-fire when a different input context is added while
	// the key is still held. The UE-270589 fix replaces triggers with fresh instances during
	// rebuild, but runtime state (LastValue) is transferred from old to new triggers so that
	// Pressed correctly sees "already actuated" rather than a false edge. (UE-372464)
	...
	AND(AnActionIsMappedToAKey(Data, TestContext, TestAction, TestKey));
	AND(ATriggerIsAppliedToAnAction(Data, NewObject<UInputTriggerPressed>(), TestAction));

	// 1 - Press key, Pressed trigger fires once
	WHEN(AKeyIsActuated(Data, TestKey));
	AND(InputIsTicked(Data));
	THEN(PressingKeyTriggersAction(Data, TestAction));

	// 2 - Hold tick, trigger transitions to Completed
	WHEN(InputIsTicked(Data));
	THEN(HoldingKeyTriggersCompleted(Data, TestAction));

	// 3 - Hold tick, action is quiet (Pressed fires only once)
	WHEN(InputIsTicked(Data));
	THEN(HoldingKeyDoesNotTrigger(Data, TestAction));

	// 4 - Add a second context while key is held, forcing a mapping rebuild.
	// Using bForceImmediately to rebuild synchronously, like a runtime context change.
	// NOT using ControlMappingsAreRebuilt() — that clears ActionInstanceData (trigger state),
	// which is the data we are testing here.
	// bIgnoreAllPressedKeysUntilRelease defaults to true in FModifyContextOptions.
	{
		UInputMappingContext* SecondContext = NewObject<UInputMappingContext>();
		FModifyContextOptions Options;
		Options.bForceImmediately = true;
		Data.Subsystem->AddMappingContext(SecondContext, 10, Options);
	}

	// 5 - Tick after context change, Pressed must NOT re-fire (MAIN TEST)
	// Without the fix: fresh triggers have LastValue=0, so Pressed sees
	// IsActuated(current)=true AND !IsActuated(LastValue=0)=true -> re-fires.
	// With the fix: LastValue is transferred from old triggers, so Pressed correctly
	// sees !IsActuated(LastValue)=false and does not re-fire.
	WHEN(InputIsTicked(Data));
	THEN(HoldingKeyDoesNotTrigger(Data, TestAction));
	...
	// 7 - Re-press key, Pressed trigger fires again normally.
	WHEN(AKeyIsActuated(Data, TestKey));
	AND(InputIsTicked(Data));
	THEN(PressingKeyTriggersAction(Data, TestAction));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FInputSystemContextChangeNoHeldKeysTest, "Input.System.ContextChange.NoHeldKeys", BasicSystemTestFlags)

bool FInputSystemContextChangeNoHeldKeysTest::RunTest(const FString& Parameters)
{
	// Regression test: adding a context while no keys are held must not suppress subsequent
	// trigger firing. bShouldBeIgnored should only be set when a key is actually down.
	...
	// 1 - Add a second context with no keys held
	{
		UInputMappingContext* SecondContext = NewObject<UInputMappingContext>();
		FModifyContextOptions Options;
		Options.bForceImmediately = true;
	...
	// 2 - Press key, Pressed trigger fires normally (not incorrectly suppressed)
	WHEN(AKeyIsActuated(Data, TestKey));
	AND(InputIsTicked(Data));
	THEN(PressingKeyTriggersAction(Data, TestAction));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FInputSystemContextChangeDownTriggerTest, "Input.System.ContextChange.DownTriggerContinuesAfterRebuild", BasicSystemTestFlags)

bool FInputSystemContextChangeDownTriggerTest::RunTest(const FString& Parameters)
{
	// Test that a Down trigger continues to fire every tick after a mapping rebuild caused by
	// adding a second IMC while the key is held. The previous bShouldBeIgnored-based fix for
	// UE-372464 would skip the entire mapping, causing the Down action to fire Completed and
	// go dead. The correct fix transfers trigger runtime state so no mapping skip is needed.
	...
	// 1 - Press key, Down trigger fires Triggered
	WHEN(AKeyIsActuated(Data, TestKey));
	AND(InputIsTicked(Data));
	THEN(PressingKeyTriggersAction(Data, TestAction));

	// 2 - Hold tick, Down continues to fire Triggered
	WHEN(InputIsTicked(Data));
	THEN(HoldingKeyTriggersAction(Data, TestAction));

	// 3 - Add a second context while key is held, forcing a mapping rebuild
	{
		UInputMappingContext* SecondContext = NewObject<UInputMappingContext>();
		FModifyContextOptions Options;
		Options.bForceImmediately = true;
	...
	// 4 - Tick after context change, Down must STILL fire Triggered (MAIN TEST)
	// Without the fix: bShouldBeIgnored skips the mapping -> Triggered->None -> Completed -> dead
	// With the fix: trigger state is transferred, no mapping skip, Down continues normally
	WHEN(InputIsTicked(Data));
	THEN(HoldingKeyTriggersAction(Data, TestAction));

	// 5 - Another hold tick, still firing
	WHEN(InputIsTicked(Data));
	THEN(HoldingKeyTriggersAction(Data, TestAction));

	// 6 - Release key, Down transitions to Completed
	WHEN(AKeyIsReleased(Data, TestKey));
	AND(InputIsTicked(Data));
	THEN(ReleasingKeyTriggersCompleted(Data, TestAction));
