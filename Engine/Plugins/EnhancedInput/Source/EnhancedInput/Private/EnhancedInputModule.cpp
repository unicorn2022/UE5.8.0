...
public:
	...
	{
		// Input can only possibly exist on the client, so never attempt to tick it on the server.
...
void FEnhancedInputModule::StartupModule()
{
	...
	EKeys::AddKey(FKeyDetails(FEnhancedInputKeys::ComboKey, LOCTEXT("ComboKey", "Combo Key (Deprecated)"), FKeyDetails::NotActionBindableKey, NAME_EnhancedInput));
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
