...
void UInputTrigger::OnTriggerReinstanced(const UInputTrigger* OldTrigger)
{
	if (!OldTrigger)
	{
		return;
	...
	// The old trigger must be guaranteed to be the same class
	check(OldTrigger->GetClass() == GetClass());
	
	LastValue = OldTrigger->LastValue;
	ReceiveTriggerReinstanced(OldTrigger);
}
...
void UInputTriggerTimedBase::ReceiveTriggerReinstanced_Implementation(const UInputTrigger* OldTrigger)
{
	if (const UInputTriggerTimedBase* OldTimed = CastChecked<UInputTriggerTimedBase>(OldTrigger))
	{
		HeldDuration = OldTimed->HeldDuration;
	}
}
...
void UInputTriggerHold::ReceiveTriggerReinstanced_Implementation(const UInputTrigger* OldTrigger)
{
	Super::ReceiveTriggerReinstanced_Implementation(OldTrigger);
	
	if (const UInputTriggerHold* OldHold = CastChecked<UInputTriggerHold>(OldTrigger))
	{
		bTriggered = OldHold->bTriggered;
	}
}
...
UInputTriggerRepeatedTap::UInputTriggerRepeatedTap()
{
	// The time window between taps is the whole point of this trigger. Without always-tick,
	// UEnhancedPlayerInput::ProcessActionMappingEvent skips evaluation on frames where the key
	// has no event, and the action state collapses from Ongoing to None between taps — firing
	// a spurious Canceled and resetting NumberOfTapsSinceLastTrigger. See UE-226494.
...
ETriggerState UInputTriggerRepeatedTap::UpdateState_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue ModifiedValue, float DeltaTime)
{
	...
	// Preserve Ongoing between taps so a pending repeat doesn't bounce through None
	// (which would emit a spurious Canceled). Requires bShouldAlwaysTick set in the
	// constructor so the pipeline keeps evaluating us on frames where the key is idle.
	// bAwaitingRepeatTap covers the window after the first tap — NumberOfTapsSinceLastTrigger
	// is only incremented from the second tap onward, so the counter alone can't carry
	// Ongoing across the gap before the first increment.
	State = (NumberOfTapsSinceLastTrigger > 0 || bAwaitingRepeatTap || (bHasSingleTap && IsActuated(LastValue))) ? ETriggerState::Ongoing : State;

	// If too much time has passed to be within a valid repeat range, and a repeat sequence is pending,
	// then we need to cancel this trigger.
	if (!bIsWithinValidRepeatTimeRange && (NumberOfTapsSinceLastTrigger > 0 || bAwaitingRepeatTap))
	{
		State = ETriggerState::None;
	}
	...
	// Keep track of the next time range which is acceptable for another repeat, and mark that a
	// repeat sequence is now in progress so idle frames between taps stay Ongoing.
	if (bHasSingleTap)
	{
		RepeatTime = CurrentTime + RepeatDelay;
		bAwaitingRepeatTap = true;
	}
	...
	if (State == ETriggerState::None || State == ETriggerState::Triggered)
	{
		...
		bAwaitingRepeatTap = false;
	}
	
	UE_LOGF(LogEnhancedInput, Verbose, "Repeated Tap InputState:  %d / %d taps :: %ls", NumberOfTapsSinceLastTrigger, NumberOfTapsWhichTriggerRepeat, *UE::Input::LexToString(State));
...
void UInputTriggerPulse::ReceiveTriggerReinstanced_Implementation(const UInputTrigger* OldTrigger)
{
	Super::ReceiveTriggerReinstanced_Implementation(OldTrigger);
	
	if (const UInputTriggerPulse* OldPulse = CastChecked<UInputTriggerPulse>(OldTrigger))
	{
		TriggerCount = OldPulse->TriggerCount;
	}
}
...
ETriggerState UInputTriggerCombo::UpdateState_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue ModifiedValue, float DeltaTime)
{
	...
	{
		UE_LOGF(LogEnhancedInput, Warning, "A Combo Trigger has no combo actions and will not work properly!");
		return ETriggerState::None;
	}
...
EDataValidationResult UInputTriggerCombo::IsDataValid(FDataValidationContext& Context) const
{
	...
	Context.AddWarning(LOCTEXT("ComboTriggerDeprecated", "The input combo trigger is deprecated and will be removed in a future release"));
