...
void UInputModifier::OnModifierReinstanced(const UInputModifier* OldModifier)
{
	if (!OldModifier)
	{
		return;
	...
	check(OldModifier->GetClass() == GetClass());

	ReceiveModifierReinstanced(OldModifier);
}
...
void UInputModifierSmoothDelta::ReceiveModifierReinstanced_Implementation(const UInputModifier* OldModifier)
{
	if (const UInputModifierSmoothDelta* OldSmooth = CastChecked<UInputModifierSmoothDelta>(OldModifier))
	{
		OldValue = OldSmooth->OldValue;
		Delta = OldSmooth->Delta;
	}
}
...
EDataValidationResult UInputModifierDeadZone::IsDataValid(class FDataValidationContext& Context) const
{
	...
	{
		Result = EDataValidationResult::Invalid;
		Context.AddError(LOCTEXT("InputModifierDeadZone", "The 'Lower Threshold' cannot be greater than or equal to the 'Upper Threshold' of a deadzone."));
	}
...
void UInputModifierDeadZone::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
		...
		MemberPropertyName == GET_MEMBER_NAME_CHECKED(UInputModifierDeadZone, UpperThreshold))
	{
		...
		if (LowerThreshold > UpperThreshold)
		{
			LowerThreshold = UpperThreshold - 0.001f;
		}
	}
}
#endif	// WITH_EDITOR
...
void UInputModifierSmooth::ReceiveModifierReinstanced_Implementation(const UInputModifier* OldModifier)
{
	if (const UInputModifierSmooth* OldSmooth = CastChecked<UInputModifierSmooth>(OldModifier))
	{
		ZeroTime = OldSmooth->ZeroTime;
		AverageValue = OldSmooth->AverageValue;
		Samples = OldSmooth->Samples;
		TotalSampleTime = OldSmooth->TotalSampleTime;
	}
}
