...
void UEnhancedInputLocalPlayerSubsystem::InitalizeUserSettings()
{
	...
	if (!LP)
	{
		UE_LOGF(LogEnhancedInput, Error, "Failed to initialize user settings, there is no valid local player!");
		return;
	}
	...
	if (!GetDefault<UEnhancedInputDeveloperSettings>()->bEnableUserSettings)
	{
		UE_LOGF(LogEnhancedInput, Verbose, "bEnableUserSettings is set to false, skipping creation of UEnhancedInputUserSettings!");
		return;
	}
	...
	if (!GetPlayerInput())
	{
		UE_LOGF(LogEnhancedInput, Warning, "UEnhancedInputLocalPlayerSubsystem for Local Player '%ls' does not have a valid PlayerInput object. Failed to load user settings.", *GetNameSafe(LP));
		return;
	}
	...
	if (ensure(UserSettings))
	{
		...
		UE_LOGF(LogEnhancedInput, Log, "Enhanced Input local player subsystem has initialized the user settings!");
	}
}
...
bool UEnhancedInputWorldSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
	...
	if (!bShouldCreate)
	{
		UE_LOGF(LogWorldSubsystemInput, Verbose, "UEnhancedInputDeveloperSettings::bEnableWorldSubsystem is false, the world subsystem will not be created!");
	}
...
void UEnhancedInputWorldSubsystem::AddActorInputComponent(AActor* Actor)
{
	if (Actor)
	{
		...
		{
			UE_LOGF(LogWorldSubsystemInput, Error, "Attempted to push a null Input Component to the Enhanced Input World Subsystem!");
		}
	}
}
...
bool UEnhancedInputWorldSubsystem::InputKey(const FInputKeyEventArgs& Params)
{
	if (PlayerInput)
	{
		UE_LOGF(LogWorldSubsystemInput, VeryVerbose, "EI %ls World Subsystem InputKey : [%ls]", LexToString(GetWorld()->WorldType), *Params.Key.ToString());
