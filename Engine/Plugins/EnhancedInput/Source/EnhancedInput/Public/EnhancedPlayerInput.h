...
public:
	...
	*/
	 ...
	 * 
	 * Note: Do NOT hold onto any references to these mappings or their input triggers/modifiers, as they will get
	 * re-instanced every time the control mappings are rebuilt. This is for read-only access to view the current mappings
	 * which may be useful for systems which want to query specific data about the current trigger states of specific mappings.
	 */
	UE_API TConstArrayView<const FEnhancedActionKeyMapping> GetEnhancedActionMappingsView() const;
