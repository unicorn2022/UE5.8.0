...
public:
	...
	*/
	...
	/**
	 * Called during RebuildControlMappings when this trigger instance replaces an older instance
	 * of the same class. Transfers runtime state from the old trigger so that state-dependent
	 * triggers like Pressed don't see false transitions, and timed triggers don't restart.
	 *
	 * The old trigger is guaranteed to be the same class (verified by FEnhancedActionKeyMapping::Equals).
	 *
	 * This function copies LastValue, then dispatches to ReceiveTriggerReinstanced for subclass state.
	 */
	UE_API void OnTriggerReinstanced(const UInputTrigger* OldTrigger);

protected:

	/**
	 * Override to transfer custom runtime state from a previous trigger instance of the same class.
	 * Called by OnTriggerReinstanced after LastValue has already been copied.
	 * Subclasses should call Super to ensure parent state is transferred.
	 */
	UFUNCTION(BlueprintNativeEvent, Category = "Trigger")
	UE_API void ReceiveTriggerReinstanced(const UInputTrigger* OldTrigger);
	virtual void ReceiveTriggerReinstanced_Implementation(const UInputTrigger* OldTrigger) {}
...
protected:
	...
	UE_API virtual void ReceiveTriggerReinstanced_Implementation(const UInputTrigger* OldTrigger) override;
...
public:
	UE_API UInputTriggerRepeatedTap();
...
protected:
	...
	/**
	 * True once we've detected a single tap and are still within the repeat window, waiting for
	 * the next tap. Keeps the trigger state Ongoing on idle frames between taps (the first tap
	 * doesn't increment NumberOfTapsSinceLastTrigger, so we need a separate flag to represent
	 * "a repeat sequence is in progress"). Cleared when the sequence triggers or is cancelled.
	 */
	UPROPERTY()
	bool bAwaitingRepeatTap = false;
};
...
struct UE_DEPRECATED(5.8, "The Input combo trigger and related types are deprecated") FInputComboStepData
{
	GENERATED_BODY()
...
USTRUCT(BlueprintType)
struct UE_DEPRECATED(5.8, "The Input combo trigger and related types are deprecated") FInputCancelAction
{
	GENERATED_BODY()
...
class UE_DEPRECATED(5.8, "UInputTriggerCombo has been deprecated.") UInputTriggerCombo final : public UInputTrigger
{
	GENERATED_BODY()
