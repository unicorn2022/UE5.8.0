...
{
	// UE_LOGF(LogTemp, Error, "On Drop Action");
	// UE_LOGF(LogTemp, Error, "bWasSwitchDragOp: %ls", FBridgeUIManager::BrowserBinding->bWasSwitchDragOperation ? TEXT("true") : TEXT("false"));
	...
	if (!bDropWasHandled)
	{
		...
		// UE_LOGF(LogTemp, Error, "Drag discarded");
...
bool IsAssetFoundInDragOperation(FString AssetId, TArray<FString> IDs)
{
	...
	UE_LOGF(LogTemp, Error, "AssetId not found in Drag operation: %ls", *AssetId);
...
void UBrowserBinding::DragStarted(TArray<FString> ImageUrls, TArray<FString> IDs, TArray<FString> Types)
{
	...
	AsyncTask(ENamedThreads::Type::GameThread, [this, ImageUrls, IDs, ActorFactory] () 
	{
		...
		FBridgeDragDropHelper::Instance->SetOnAddProgressiveStageData(FOnAddProgressiveStageDataCallbackInternal::CreateLambda(
			...
			{
				// UE_LOGF(LogTemp, Error, "Processing AssetId: %ls", *AssetId);
				...
				if (FBridgeUIManager::BrowserBinding->bIsDragging)
				{
					...
					else
					{
						...
						{
							// UE_LOGF(LogTemp, Error, "FoundSphereActor is null");
							return;
						}
				...
				else
				{
					...
					if (SpawnedActor == nullptr)
					{
						// UE_LOGF(LogTemp, Error, "SpawnedActor is null");
