...
TSharedRef<SDockTab> FBridgeUIManagerImpl::CreateBridgeTab(const FSpawnTabArgs& Args)
{
	...
	{
		UE_LOGF(LogTemp, Error, "WebBrowserModule isn't available! Can't display Quixel Bridge tab contents.");
		return SAssignNew(LocalBrowserDock, SDockTab)
			...
			[
				SNew(SVerticalBox)
					...
					[
						SNew(STextBlock)
							.Text(FText::FromString("Unable to display the Quixel Bridge Browser"))
	...
	TSharedPtr<IWebBrowserWindow> Browser = WebBrowserSingleton->CreateBrowserWindow(WindowSettings);
	if (!Browser.IsValid())
	{
		UE_LOGF(LogTemp, Error, "WebBrowserWindow is invalid! Can't display Quixel Bridge tab contents.");
		return SAssignNew(LocalBrowserDock, SDockTab)
			.TabRole(ETabRole::NomadTab)
			[
				SNew(SVerticalBox)
