...
namespace ClothingSimulationClothConsoleVariables
{
	...
	static float GlobalMaxLinearVelocity = 0.f;
	FAutoConsoleVariableRef CVarMaxLinearVelocity(
		TEXT("p.ChaosCloth.MaxLinearVelocity"),
		GlobalMaxLinearVelocity,
		TEXT("The maximum amount of linear velocity sent to the local cloth space from the reference bone. "
			 "The console variable value will take precedence when set smaller to the equivalent Velocity Scale config properties."
			 "Use 0 to disable."));

	static float GlobalMaxAngularVelocity = 0.f;
	FAutoConsoleVariableRef CVarMaxAngularVelocity(
		TEXT("p.ChaosCloth.MaxAngularVelocity"),
		GlobalMaxAngularVelocity,
		TEXT("The maximum amount of angular velocity sent to the local cloth space from the reference bone. "
			 "The console variable value will take precedence when set smaller to the equivalent Velocity Scale config properties."
			 "Use 0 to disable."));

	static float GlobalMaxLinearAcceleration = 0.f;
	FAutoConsoleVariableRef CVarMaxLinearAcceleration(
		TEXT("p.ChaosCloth.MaxLinearAcceleration"),
		GlobalMaxLinearAcceleration,
		TEXT("The maximum amount of linear acceleration sent to the local cloth space from the reference bone. "
			 "The console variable value will take precedence when set smaller to the equivalent Velocity Scale config properties."
			 "Use 0 to disable."));

	static float GlobalMaxAngularAcceleration = 0.f;
	FAutoConsoleVariableRef CVarMaxAngularAcceleration(
		TEXT("p.ChaosCloth.MaxAngularAcceleration"),
		GlobalMaxAngularAcceleration,
		TEXT("The maximum amount of angular acceleration sent to the local cloth space from the reference bone. "
			 "The console variable value will take precedence when set smaller to the equivalent Velocity Scale config properties."
			 "Use 0 to disable."));
}
...
void FClothingSimulationCloth::FLODData::Add(FClothingSimulationSolver* Solver, FClothingSimulationCloth* Cloth, int32 InLODIndex)
{
	...
	ClothConstraints.AddRules(ConfigProperties, TriangleMesh, &PatternData, WeightMaps, VertexSets, FaceSets, FaceIntMaps, Tethers, MeshScale, bEnabled, ComplexColliders, ManagedArrayCollection, &SolverDatum.AccessoryMeshData);
...
void FClothingSimulationCloth::FLODData::Update(FClothingSimulationSolver* Solver, FClothingSimulationCloth* Cloth)
{
	...
	if (ParticleRangeId != INDEX_NONE)
	{
		...
		ClothConstraints.Update(Cloth->Config->GetProperties(SolverDatum.LODIndex), WeightMaps, VertexSets, FaceSets, FaceIntMaps, MeshScale, MaxDistancesScale, Cloth->ReferenceSpaceTransform.GetRotation(), Solver->GetLocalSpaceRotation(), LocalSpaceScale, Solver->GetLocalSpaceLocation());
...
void FClothingSimulationCloth::Add(FClothingSimulationSolver* Solver)
{
	...
	// Add all particles first and in reverse order. (was doing this for multi-res constraints, but now just leaving it)
	for (int32 Index = LODData.Num() - 1; Index >= 0; --Index)
	{
		LODData[Index]->AddParticles(Solver, this, Index);
	}
	// Now add the LODs themselves.
...
TSet<FString> FClothingSimulationCloth::GetAllVertexSetNames() const
{
	TSet<FString> Names;
	for (const TUniquePtr<FLODData>& Data : LODData)
	{
		TSet<FString> LODNames;
		Data->VertexSets.GetKeys(LODNames);
		Names.Append(MoveTemp(LODNames));
	}
	return Names;
}

const TSet<int32>* FClothingSimulationCloth::GetVertexSetByName(const FClothingSimulationSolver* Solver, const FString& Name) const
{
	const int32 LODIndex = LODIndices.FindChecked(Solver);
	return LODData.IsValidIndex(LODIndex) ? LODData[LODIndex]->VertexSets.FindRef(Name) : nullptr;
}

const TSet<int32>* FClothingSimulationCloth::GetVertexSetByProperty(const FClothingSimulationSolver* Solver, const FName& Property) const
{
	check(Config);
	const FString PropertyString = Config->GetProperties(GetLODIndex(Solver)).GetStringValue(Property);
	return GetVertexSetByName(Solver, PropertyString);
}
...
void FClothingSimulationCloth::Update(FClothingSimulationSolver* Solver)
{
	...
	if (LODIndex != INDEX_NONE && ParticleRangeId != INDEX_NONE)
	{
		...
		if (bNeedsReset)
		{
			...
			UE_LOGF(LogChaosCloth, Verbose, "Cloth in group Id %d Needs reset.", GroupId);
		}
		else if (bNeedsTeleport)
		{
			...
			AppliedReferenceSpaceVelocity = FVec3(0.);
			bDisableFictitiousForces = true;
			UE_LOGF(LogChaosCloth, Verbose, "Cloth in group Id %d Needs teleport.", GroupId);
		}
		else
		{
			...
			// Apply the cvars clamp, if any are active
			using namespace ClothingSimulationClothConsoleVariables;
			MaxLinearVelocity = (GlobalMaxLinearVelocity > 0.f) ? MaxLinearVelocity.ComponentwiseMin(FVec3f(GlobalMaxLinearVelocity)) : MaxLinearVelocity;
			MaxLinearAcceleration = (GlobalMaxLinearAcceleration > 0.f) ? MaxLinearAcceleration.ComponentwiseMin(FVec3f(GlobalMaxLinearAcceleration)) : MaxLinearAcceleration;
			MaxAngularVelocity = (GlobalMaxAngularVelocity > 0.f) ? FMath::Min(MaxAngularVelocity, (FRealSingle)GlobalMaxAngularVelocity) : MaxAngularVelocity;
			MaxAngularAcceleration = (GlobalMaxAngularAcceleration > 0.f) ? FMath::Min(MaxAngularAcceleration, (FRealSingle)GlobalMaxAngularAcceleration) : MaxAngularAcceleration;
		}
		using EFictitiousForcesModel = FClothingSimulationSolver::EFictitiousForcesModel;
		const EFictitiousForcesModel FictitiousForcesModel = bDisableFictitiousForces ?
			EFictitiousForcesModel::None :
			(EFictitiousForcesModel)GetFictitiousForcesModel(*ConfigProperties, (int32)EFictitiousForcesModel::Legacy);
