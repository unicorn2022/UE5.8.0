		...
		if(InComponent)
		{
			if( USkeletalMeshComponent* MeshComp = CastChecked<USkeletalMeshComponent>(InComponent) )
			{
				if (MeshComp->GetClothingSimulationInstances().Num() > 1)
				{
					...
					UE_CLOGF(!bErroredOnce, LogChaosCloth, Error,
						"SkeletalMeshComponent [%ls] has more than one clothing simulation running, but cloth caching can only deal with a single one.", *MeshComp->GetName());
					bErroredOnce = true;
				}
