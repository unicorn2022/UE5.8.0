...
#if CHAOS_DEBUG_DRAW
		Evolution->AddGroupArray(&OriginalReferenceSpaceVelocities);
		Evolution->AddGroupArray(&OriginalReferenceSpaceAngularVelocities);
#endif
...
#if CHAOS_DEBUG_DRAW
		PBDEvolution->AddArray(&OriginalReferenceSpaceVelocities);
		PBDEvolution->AddArray(&OriginalReferenceSpaceAngularVelocities);
#endif
...
void FClothingSimulationSolver::SetSolverLOD(int32 LODIndex)
{
	checkfSlow(Config && Config->GetNumLODs(), TEXT("Config must be initialized first."));
	SolverLOD = FMath::Min(LODIndex, Config->GetNumLODs() - 1);
}
...
void FClothingSimulationSolver::Reset()
{
	if (Evolution)
	{
		constexpr bool bResetGlobalRules = false;
		Evolution->Reset(bResetGlobalRules);
...
void FClothingSimulationSolver::SetReferenceVelocityScale(uint32 GroupId,
	...
	EFictitiousForcesModel FictitiousForcesModel
)
{
	...
	const FReal AppliedFictitiousAngularScale = (FictitiousForcesModel == EFictitiousForcesModel::None) ? (FReal)0.f : (PBDEvolution ? FMath::Min((FReal)2., (FReal)FictitiousAngularScale) : (FReal)1.);

	FVec3 FictitiousAngularDisplacement;
	switch (FictitiousForcesModel)
	{
	default:
	case EFictitiousForcesModel::None:
		FictitiousAngularDisplacement = FVec3(0);
		break;
	case EFictitiousForcesModel::Legacy:
		FictitiousAngularDisplacement = ReferenceSpaceTransform.TransformVector(DeltaRotation) * AppliedFictitiousAngularScale;
		break;
	case EFictitiousForcesModel::AppliedScale:
		FictitiousAngularDisplacement = ReferenceSpaceTransform.TransformVector(AppliedDeltaRotation) * AppliedFictitiousAngularScale;
		break;
	case EFictitiousForcesModel::FullScale:
		FictitiousAngularDisplacement = ReferenceSpaceTransform.TransformVector(FullDeltaRotation) * AppliedFictitiousAngularScale;
		break;
	}

	// Save the reference bone relative angular velocity for calculating the fictitious forces
	const bool bHasNonZeroDeltaTime = (DeltaTime > (Softs::FSolverReal)0.f);
	FictitiousAngularVelocities[GroupId] = bHasNonZeroDeltaTime ? Softs::FSolverVec3(FictitiousAngularDisplacement) / DeltaTime : Softs::FSolverVec3(0.f);
	ReferenceSpaceLocations[GroupId] = LocalSpaceScaleInv * (ReferenceSpaceTransform.GetLocation() - LocalSpaceLocation);
	AppliedReferenceSpaceVelocities[GroupId] = bHasNonZeroDeltaTime ? FSolverVec3(LocalSpaceScaleInv * InOutAppliedReferenceVelocity) : Softs::FSolverVec3(0.f);
	AppliedReferenceSpaceAngularVelocities[GroupId] = bHasNonZeroDeltaTime ? FSolverVec3(InOutAppliedReferenceAngularVelocity) : Softs::FSolverVec3(0.f);
#if CHAOS_DEBUG_DRAW
	OriginalReferenceSpaceVelocities[GroupId] = bHasNonZeroDeltaTime ? FSolverVec3(LocalSpaceScaleInv * WorldDeltaTranslation / DeltaTime) : Softs::FSolverVec3(0.f);
	OriginalReferenceSpaceAngularVelocities[GroupId] = bHasNonZeroDeltaTime ? FSolverVec3(FullDeltaRotation / DeltaTime) : Softs::FSolverVec3(0.f);
#endif
}
...
Softs::FSolverReal FClothingSimulationSolver::SetParticleMassPerArea(Softs::FSolverParticlesRange& Particles, const FTriangleMesh& Mesh)
{
	...
	UE_LOGF(LogChaosCloth, Verbose, "Total area: %f, SI total area: %f", TotalArea, TotalArea / FMath::Square(ClothingSimulationSolverConstant::WorldScale));
	return TotalArea;
...
void FClothingSimulationSolver::ParticleMassUpdateDensity(Softs::FSolverParticlesRange& Particles, const FTriangleMesh& Mesh, const Softs::FSolverVec2& Density, const TConstArrayView<float>& DensityMultipliers)
{
	...
	UE_LOGF(LogChaosCloth, Verbose, "Total mass: %f, ", TotalMass);
}
...
void FClothingSimulationSolver::Update(Softs::FSolverReal InDeltaTime)
{
	...
	if (ClothSolverDebugHitchLength && ClothSolverDebugHitchInterval)
	{
		...
		if (--HitchCounter < 0)
		{
			UE_LOGF(LogChaosCloth, Warning, "Hitching for %dms", ClothSolverDebugHitchLength);
	...
	if (bAdvanceTimeStep)
	{
		...
		if (bEnableSolver)
		{
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "DeltaTime: %.6f, Time = %.6f", DeltaTime, Time);
		}
	}
...
bool FClothingSimulationSolver::UsesBuoyancy()
{
	static IConsoleVariable* const UsesBuoyancy = IConsoleManager::Get().FindConsoleVariable(TEXT("p.Chaos.Cloth.EnableBuoyancy"));
	return UsesBuoyancy ? UsesBuoyancy->GetBool() && !IsLegacySolver() : false;
}

void FClothingSimulationSolver::PreSimulateBuoyancyField_GameThread(float dt, const UWorld* World, const FBox BoundingBox)
{
	auto& AllConstraints = ClothsConstraints;

	for (auto& Pair : AllConstraints)
	{
		int32 Key = Pair.Key;
		::Chaos::FClothConstraints* Constraints = Pair.Value.Get();

		if (Buoyancy::FBuoyancyField *BuoyancyField = Constraints->GetBuoyancyField().Get())
		{
			BuoyancyField->PreSimulate_GameThread(dt, World, BoundingBox);
		}
	}
}
