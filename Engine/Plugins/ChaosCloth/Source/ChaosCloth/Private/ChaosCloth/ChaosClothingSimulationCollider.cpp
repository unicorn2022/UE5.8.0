	...
	const TArray<int32>& UsedSubBoneIndices)
{
	...
	const int32 NumExtrudedCapsules = ClothCollisionData.ExtrudedTaperedCapsules.Num();
	...
	const int32 NumSimpleColliders = NumSpheres + NumCapsules + NumConvexes + NumBoxes + NumExtrudedCapsules + NumLevelSets;
	...
	if (NumCapsules)
	{
		for (int32 Index = CapsuleIndexOffset; Index < CapsuleIndexOffset + NumCapsules; ++Index)
		{
			...
			UE_CLOGF(Sphere0.BoneIndex != Sphere1.BoneIndex,
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision capsule on bone index %d.", BoneIndices[Index]);
	...
	if (NumSpheres != 0)
	{
		for (int32 Index = SphereIndexOffset, SphereIndex = 0; SphereIndex < ClothCollisionData.Spheres.Num(); ++SphereIndex)
		{
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision sphere on bone index %d.", BoneIndices[Index]);
	...
	if (NumConvexes != 0)
	{
		for (int32 Index = ConvexIndexOffset, ConvexIndex = 0; ConvexIndex < NumConvexes; ++Index, ++ConvexIndex)
		{
			...
			BaseTransforms[Index] = Softs::FSolverRigidTransform3(Convex.LocalPosition, Convex.LocalRotation);

			BoneIndices[Index] = GetMappedBoneIndex(UsedBoneIndices, Convex.BoneIndex);
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision convex on bone index %d.", BoneIndices[Index]);
			...
			{
				UE_LOGF(LogChaosCloth, Warning, "Invalid convex collision: not enough surface points.");
			}
			else if (NumFaces < 4)
			{
				UE_LOGF(LogChaosCloth, Warning, "Invalid convex collision: not enough faces.");
			}
			else
			{
				...
				for (const FClothCollisionPrim_ConvexFace& Face : Convex.Faces)
				{
					...
					{
						UE_LOGF(LogChaosCloth, Warning, "Invalid convex collision: bad plane normal.");
						break;
					}
					if (!Face.Indices.Num())
					{
						UE_LOGF(LogChaosCloth, Warning, "Invalid convex collision: no face indices, this asset might be using a deprecated Apex collision shape.");
						break;
					}
			...
			{
				UE_LOGF(LogChaosCloth, Warning, "Replacing invalid convex collision by a default unit sphere.");
	...
	{		
		for (int32 Index = BoxIndexOffset, BoxIndex = 0; BoxIndex < NumBoxes; ++Index, ++BoxIndex)
		{
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision box on bone index %d.", BoneIndices[Index]);
	...
	// Extruded Tapered Capsules
	const int32 ExtrudedCapsuleIndexOffset = BoxIndexOffset + NumBoxes;
	if (NumExtrudedCapsules != 0)
	{
		for (int32 Index = ExtrudedCapsuleIndexOffset, ExtrudedCapsuleIndex = 0; ExtrudedCapsuleIndex < NumExtrudedCapsules; ++Index, ++ExtrudedCapsuleIndex)
		{
			const FClothCollisionPrim_ExtrudedTaperedCapsule ExtrudedCapsule = ClothCollisionData.ExtrudedTaperedCapsules[ExtrudedCapsuleIndex];
			BaseTransforms[Index] = Softs::FSolverRigidTransform3::Identity;

			BoneIndices[Index] = GetMappedBoneIndex(UsedBoneIndices, ExtrudedCapsule.BoneIndex);
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision ExtrudedCapsule on bone index %d.", BoneIndices[Index]);

			const FVec3f MajorAxis = FVec3f(ExtrudedCapsule.LocalRotation.RotateVector(FVector::UpVector));
			const FVec3f MinorAxis = FVec3f(ExtrudedCapsule.LocalRotation.RotateVector(FVector::ForwardVector));
			Solver->SetCollisionGeometry(CollisionRangeId, Index, MakeImplicitObjectPtr<FExtrudedTaperedCapsule>(FVec3f(ExtrudedCapsule.LocalPosition * InScale), MajorAxis, MinorAxis, ExtrudedCapsule.Length * InScale * .5f, ExtrudedCapsule.Width * InScale * .5f, ExtrudedCapsule.Radius1 * InScale, ExtrudedCapsule.Radius2 * InScale));
		}
	}
	
	// Level sets
	const int32 LevelSetIndexOffset = ExtrudedCapsuleIndexOffset + NumExtrudedCapsules;
	if (NumLevelSets != 0)
	{
		for (int32 Index = LevelSetIndexOffset, LevelSetIndex = 0; LevelSetIndex < NumLevelSets; ++Index, ++LevelSetIndex)
		{
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision level set on bone index %d.", BoneIndices[Index]);
	...
	UE_LOGF(LogChaosCloth, VeryVerbose, "Added collisions: %d spheres, %d capsules, %d convexes, %d boxes, %d extruded tapered capsules, %d level sets.", NumSpheres, NumCapsules, NumConvexes, NumBoxes, NumExtrudedCapsules, NumLevelSets);
	...
	if (bSupportsComplexColliders)
	{
		...
		for (int32 Index = SkinnedLevelSetIndexOffset, SkinnedLevelSetIndex = 0; SkinnedLevelSetIndex < NumSkinnedLevelSets; ++Index, ++SkinnedLevelSetIndex)
		{
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision skinned level set on bone index %d.", BoneIndices[Index]);
		...
		for (int32 Index = MLLevelSetIndexOffset, MLLevelsetIndex = 0; MLLevelsetIndex < NumMLLevelSets; ++Index, ++MLLevelsetIndex)
		{
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision ml level set on bone index %d.", BoneIndices[Index]);
		...
		for (int32 Index = SkinnedTriMeshIndexOffset, SkinnedTriMeshIndex = 0; SkinnedTriMeshIndex < NumSkinnedTriangleMeshes; ++Index, ++SkinnedTriMeshIndex)
		{
			...
			UE_LOGF(LogChaosCloth, VeryVerbose, "Found collision skinned triangle mesh on bone index %d.", BoneIndices[Index]);
	...
	bool bSkipMissingBones)
{
	...
	if (PhysicsAsset)
	{
		...
		for (const USkeletalBodySetup* BodySetup : PhysicsAsset->SkeletalBodySetups)
		{
			...
			{
				if (TaperedCapsuleElem.IsExtruded())
				{
					FClothCollisionPrim_ExtrudedTaperedCapsule Capsule;
					Capsule.LocalPosition = TaperedCapsuleElem.Center;
					Capsule.LocalRotation = TaperedCapsuleElem.Rotation.Quaternion();
					Capsule.Length = TaperedCapsuleElem.Length;
					Capsule.Width = TaperedCapsuleElem.Width;
					Capsule.Radius1 = TaperedCapsuleElem.Radius0;
					Capsule.Radius2 = TaperedCapsuleElem.Radius1;
					Capsule.BoneIndex = MappedBoneIndex;
					ClothCollisionData.ExtrudedTaperedCapsules.Add(MoveTemp(Capsule));
				}
				else if (TaperedCapsuleElem.Length == 0 && !TaperedCapsuleElem.IsOneSidedCollision())
				{
					// Add extracted sphere collision data
					FClothCollisionPrim_Sphere Sphere;
				...
				else
				{
					...
					SphereConnection.SphereIndices[0] = ClothCollisionData.Spheres.Add(Sphere0);
					SphereConnection.SphereIndices[1] = ClothCollisionData.Spheres.Add(Sphere1);
					if (TaperedCapsuleElem.IsOneSidedCollision())
					{
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	UE_CLOGF(FMath::Abs(Scale3D.X - Scale3D.Y) > KINDA_SMALL_NUMBER || FMath::Abs(Scale3D.X - Scale3D.Z) > KINDA_SMALL_NUMBER,
		LogChaosCloth, Warning, "Object '%ls' has a non uniform scale, and has a cloth simulation attached. "
