...
#include "ChaosClothingSimulationSolver.generated.h"
...
/**
 * Fictitious forces model.
 * Decide how fictitious forces (e.g. centrifugal force) are applied to the simulation.
 */
UENUM()
enum class EChaosClothingSimulationSolverFictitiousForcesModel : uint8
{
	// Don't apply fictitious forces.
	None,
	// Use partial scaled angular velocity, fictitious forces are calculated from the portion of the scaled character velocities. Legacy behavior, use the FullScale model instead.
	Legacy,
	// Use applied angular velocity (velocities after clamping), fictitious forces are calculated from the applied scale character velocities.
	AppliedScale,
	// Use full angular velocity, fictitious forces are calculated from the full scale character velocities.
	FullScale,
};
...
namespace Chaos
{
	...
	{
	public:
		using EFictitiousForcesModel = EChaosClothingSimulationSolverFictitiousForcesModel;
		...
		CHAOSCLOTH_API void SetSolverLOD(int32 LODIndex);
		...
		CHAOSCLOTH_API void SetReferenceVelocityScale(
			uint32 GroupId,
		...
		UE_DEPRECATED(5.8, "Use the version that takes a EFictitiousForcesModel argument instead.")
			...
			bool bDisableFictitiousForces)
		{
			SetReferenceVelocityScale(
				...
				InOutAppliedReferenceVelocity,
				InOutAppliedReferenceAngularVelocity,
				VelocityScaleSpace,
				LinearVelocityScale,
				MaxLinearVelocity,
				...
				MaxVelocityScale,
				bDisableFictitiousForces ? EFictitiousForcesModel::None : EFictitiousForcesModel::Legacy);
		}
			...
			bool bDisableFictitiousForces = false)
		{
			...
			SetReferenceVelocityScale(
				...
				LinearVelocityScale,
				TVec3<FRealSingle>(TNumericLimits<FRealSingle>::Max()),
				TVec3<FRealSingle>(TNumericLimits<FRealSingle>::Max()),
				AngularVelocityScale,
				TNumericLimits<FRealSingle>::Max(),
				TNumericLimits<FRealSingle>::Max(),
				FictitiousAngularScale,
				MaxVelocityScale,
				bDisableFictitiousForces ? EFictitiousForcesModel::None : EFictitiousForcesModel::Legacy);
		...
		CHAOSCLOTH_API bool UsesBuoyancy();
		CHAOSCLOTH_API void PreSimulateBuoyancyField_GameThread(float dt, const UWorld* World, const FBox BoundingBox);		
		...
		const Softs::FSolverVec3& GetAppliedReferenceSpaceVelocity(uint32 GroupId) const { return AppliedReferenceSpaceVelocities[GroupId]; }
		const Softs::FSolverVec3& GetAppliedReferenceSpaceAngularVelocity(uint32 GroupId) const { return AppliedReferenceSpaceAngularVelocities[GroupId]; }
#if CHAOS_DEBUG_DRAW
		const Softs::FSolverVec3& GetOriginalReferenceSpaceVelocity(uint32 GroupId) const { return OriginalReferenceSpaceVelocities[GroupId]; }
		const Softs::FSolverVec3& GetOriginalReferenceSpaceAngularVelocity(uint32 GroupId) const { return OriginalReferenceSpaceAngularVelocities[GroupId]; }
#endif
...
#if CHAOS_DEBUG_DRAW
		TArrayCollectionArray<Softs::FSolverVec3> OriginalReferenceSpaceVelocities; // Reference space velocity before any scaling or clamping. Used for debug draw.
		TArrayCollectionArray<Softs::FSolverVec3> OriginalReferenceSpaceAngularVelocities; // Reference space velocity before any scaling or clamping. Used for debug draw.
#endif
