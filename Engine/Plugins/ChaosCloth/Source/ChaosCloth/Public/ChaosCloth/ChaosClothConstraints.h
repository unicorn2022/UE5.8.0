...
namespace Chaos
{
	...
	public:
		...
		UE_DEPRECATED(5.8, "MultiRes constraint is no longer supported. Use other version of AddRules instead.")
		void AddRules(
			const Softs::FCollectionPropertyConstFacade& ConfigProperties,
			const FTriangleMesh& TriangleMesh,
			const FClothingPatternData* PatternData,
			...
			const TMap<FName, FClothingAccessoryMeshData>* AccessoryMeshes = nullptr)
		{
			AddRules(ConfigProperties, TriangleMesh, PatternData, WeightMaps, VertexSets, FaceSets, FaceIntMaps, Tethers, MeshScale, bEnabled, ComplexColliders, ManagedArrayCollection, AccessoryMeshes);
		}
		...
		void Update(
			...
			const FReal LocalSpaceScale = (Softs::FSolverReal)1.,
			const FSolverVec3 LocalSpaceLocation = FSolverVec3::ZeroVector);
		...
		const TSharedPtr<Buoyancy::FBuoyancyField>& GetBuoyancyField() const { return VelocityPressureAndBuoyancyField; }
		...
		const TSharedPtr<Softs::FMultiResConstraints>& GetMultiResConstraints() const { static TSharedPtr<Softs::FMultiResConstraints> Dummy; return Dummy; }
	...
	private:
		...
		void CreateVelocityPressureAndBuoyancyField(
			const Softs::FCollectionPropertyConstFacade& ConfigProperties,
			const TMap<FString, TConstArrayView<FRealSingle>>& WeightMaps,
			const FTriangleMesh& TriangleMesh
		);
		...
		TSharedPtr<Buoyancy::FBuoyancyField> VelocityPressureAndBuoyancyField;
