...
namespace Chaos
{
	...
	public:
		 ...
		 * @param InPropertyCollections The cloth source property collections to initialize from.
		 * @param bForceDirty Whether to force the initialization even if the source property collections haven't changed.
		 * @param LODIndex The index of the LOD to replace, or INDEX_NONE to replace all LODs.
		 */
		CHAOSCLOTH_API void Initialize(const TArray<TSharedPtr<const FManagedArrayCollection>>& InPropertyCollections, bool bForceDirty = false, int32 LODIndex = INDEX_NONE);
