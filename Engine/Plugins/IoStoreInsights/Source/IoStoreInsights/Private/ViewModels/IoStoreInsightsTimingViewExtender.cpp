...
namespace UE::IoStoreInsights
{
	...
	void FIoStoreInsightsViewSharedState::Tick(const TraceServices::IAnalysisSession& InAnalysisSession)
	{
		...
		if (bForceIoEventsUpdate)
		{
			...
			{
				UE_LOGF(LogTimingProfiler, Verbose, "[IO] Enumerated IoStore activities (%d request states, %d events) in %ls.", RequestStates.Num(), AllIoEvents.Num(), *Insights::FormatTimeAuto(EnumeratedTime));
			}
			...
			if (SortTime > 0.01)
			{
				UE_LOGF(LogTimingProfiler, Verbose, "[IO] Sorted IoStore activities (%d request states, %d events) in %ls.", RequestStates.Num(), AllIoEvents.Num(), *Insights::FormatTimeAuto(SortTime));
			}
			...
			if (RequestStates.Num() > 0)
			{
				...
				Stopwatch.Stop();
				UE_LOGF(LogTimingProfiler, Verbose, "[IO] Computed layout for IoStore activities in %ls.", *Insights::FormatTimeAuto(Stopwatch.GetAccumulatedTime()));
				...
				Stopwatch.Stop();
				UE_LOGF(LogTimingProfiler, Verbose, "[IO] Updated depth for events in %ls.", *Insights::FormatTimeAuto(Stopwatch.GetAccumulatedTime()));
			}
		}
	}
