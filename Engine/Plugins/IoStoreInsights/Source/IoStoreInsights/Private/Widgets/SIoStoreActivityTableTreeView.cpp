			...
			if (StartTime < EndTime && AnalysisSession != nullptr)
			{
				...
				if (Provider)
				{
					...
					Provider->EnumerateIoStoreRequests(
						[this, &ActivityTable, &Activities]
						(const FIoStoreRequest& IoStoreRequest, const IIoStoreInsightsProvider::Timeline& Timeline)
						{
							Timeline.EnumerateEvents(StartTime, EndTime,
								[this, &ActivityTable, &Activities]
								(double EventStartTime, double EventEndTime, uint32 EventDepth, const FIoStoreActivity* IoStoreActivity)
								{
									if (IoStoreActivity->ActivityType == EIoStoreActivityType::Request_Read && IoStoreActivity->EndTime > 0 && !IoStoreActivity->Failed)
									{
										...
										const uint32 TableRowIndex = Activities.Num() - 1;
										const uint32 RequestIndex = IoStoreActivity->IoStoreRequest->IoStoreRequestIndex;
										FActivityNodePtr NodePtr = MakeShared<FIoStoreActivityNode>(ActivityTable, TableRowIndex, RequestIndex);
