	...
	const FCollectionAttributeKey& AttributeKey, TArray<TArray<ScalarType>>& AttributeValues)
{
	if (!AttributeKey.Attribute.IsEmpty() && !AttributeKey.Group.IsEmpty())
	{
		...
		{	
			AttributeValues.SetNum(SelectedCollection.NumElements(*AttributeKey.Group));
		}
		return true;
	}
	return false;
						...
						if (InSkeletalMesh)
						{
							...
							if (NumBones)
							{
								...
								if (FDynamicMeshAttributeSet* Attributes = DynamicMesh.Attributes())
								{
									...
									if(VertexSkinWeights.Num() > 0)
									{ 
										FDynamicMeshVertexSkinWeightsAttribute* SkinWeights = Attributes->GetSkinWeightsAttribute(FSkeletalMeshAttributesShared::DefaultSkinWeightProfileName);

										TArray<int32> BoneIndices;
										TArray<float> BoneWeights;

										// Per-geometry vertex offset into the collection-global VertexSkinWeights array.
										const int32 GeometryVertexOffset = RenderingFacade.GetVertexStart()[GeometryIndex];

										FNonManifoldMappingSupport NonManifoldMappingSupport(DynamicMesh);
										for (int32 MeshVertexIndex : DynamicMesh.VertexIndicesItr())
										{
											BoneIndices.Reset();
											BoneWeights.Reset();

											// Translate to the collection-global index used to key VertexSkinWeights.
											const int32 GlobalVertexIndex = GeometryVertexOffset + NonManifoldMappingSupport.GetOriginalNonManifoldVertexID(MeshVertexIndex);
											VertexSkinWeights.ToArrays(GlobalVertexIndex, BoneIndices, BoneWeights);
											check(BoneIndices.Num() == BoneWeights.Num());

											UE::AnimationCore::FBoneWeights WeightsData;
											const int32 NumWeights = BoneIndices.Num();
											for (int32 WeightIndex = 0; WeightIndex < NumWeights; ++WeightIndex)
											{
												WeightsData.SetBoneWeight(BoneIndices[WeightIndex], BoneWeights[WeightIndex]);
											}
											SkinWeights->SetValue(MeshVertexIndex, WeightsData);
										}
									}
								}
							}
...
void FDataflowCollectionEditSkinWeightsNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	...
	if (Out->IsA<FManagedArrayCollection>(&Collection))
	{
		...
		if (!BoneIndicesKeyValue.Attribute.IsEmpty() && !BoneWeightsKeyValue.Attribute.IsEmpty() )
		{
			...
			{
				Context.Error(LOCTEXT("EmptyAttributes", "Invalid attributes names, enter valid attribute name or connect valid attribute keys to inputs"), this, Out);
			}
		}
		else
		{
			if (BoneIndicesKeyValue.Attribute.IsEmpty())
			{
				Context.Error(LOCTEXT("EmptyBoneIndicesAttributes", "Bone indices attribute name is empty, enter a valid attribute name or connect valid attribute key to inputs"), this, Out);
			}

			if (BoneWeightsKeyValue.Attribute.IsEmpty())
			{
				Context.Error(LOCTEXT("EmptyBoneWeightsAttributes", "Bone weights attribute name is empty, enter a valid attribute name or connect valid attribute key to inputs"), this, Out);
			}
		}
