...
FBox FDataflowNegateFloatSampler::GetRenderBounds() const
{
	if (Sampler.IsValid())
	{
...
void FDataflowNegateFloatSampler::Sample(TArrayView<const FVector3f> Positions, TArrayView<float> OutValues) const
{
...
FBox FDataflowNegateVectorSampler::GetRenderBounds() const
{
	if (Sampler.IsValid())
	{
		return Sampler->GetRenderBounds();
...
void FDataflowNegateVectorSampler::Sample(TArrayView<const FVector3f> Positions, TArrayView<FVector3f> OutValues) const
{
	if (Sampler.IsValid() && Positions.Num() == OutValues.Num())
	{
		...
		for (int32 Idx = 0; Idx < OutValues.Num(); ++Idx)
		{
			OutValues[Idx] = FVector3f(-1.f * OutValueFromInputSampler[Idx]);
		}
	}
	else
	{
...
FDataflowNegateSamplerNode::FDataflowNegateSamplerNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid)
...
void FDataflowNegateSamplerNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA(&Sampler))
	{
		...
		if (FloatSampler)
		{
			TSharedRef<FDataflowNegateFloatSampler> Impl = MakeShared<FDataflowNegateFloatSampler>();
		...
		if (VectorSampler)
		{
			TSharedRef<FDataflowNegateVectorSampler> Impl = MakeShared<FDataflowNegateVectorSampler>();
