	...
	public:
		...
		*/
		...
		T* AddNewComponent(FName ComponentName = NAME_None, UObject* ComponentParent = nullptr)
		{
			...
			if (NewComponent)
			{
				if (USceneComponent* ParentObjectAsComp = Cast<USceneComponent>(ParentObject))
				{
					NewComponent->SetupAttachment(ParentObjectAsComp);
				}
		...
		T* MakeNewObject(FName ObjectName = NAME_None)
		{
			return NewObject<T>(RootActor);
		}
