...
class UDataflowPropertyGizmoManager;
...
namespace UE::Dataflow
{
	...
	namespace Private
	{
		...
		{
		public:
			void Add(const UE::Dataflow::FContext::FLogMessage& Message);
			void RemoveForNode(const FDataflowNode& Node, const FTimestamp& Timestamp = FTimestamp::Invalid);
			void GetMessageForNode(const FDataflowNode& Node, TArray<UE::Dataflow::FContext::FLogMessage>& OutMessages) const;
			EMessageSeverity::Type GetMaxSeverityForNode(const FDataflowNode& Node) const;
			int32 Num() const { return Entries.Num(); }
			void GetMessage(int32 Index, UE::Dataflow::FContext::FLogMessage& OutMessage) const;
			EMessageSeverity::Type GetMaxSeverity();

		private:
			struct FEntry
			{
				UE::Dataflow::FTimestamp Timestamp;
...
public:
	...
	UE_API void LogMessage(const EMessageSeverity::Type Severity, const FDataflowNode* Node, const FDataflowOutput* Output, const FText& Message) const;
	...
	UE_API void DrawPropertyGizmoLabels(FCanvas* Canvas, const FSceneView* SceneView) const;

	UE_API void OnConstructionViewModeChanged(const UE::Dataflow::IDataflowConstructionViewMode* NewConstructionViewMode);
...
protected:
	...
	UE_API void OnContextLog(const UE::Dataflow::FContext::FLogMessage& InLogMessage);
	UE_API void RefreshLogMessageView();
...
private:
	...
	/** reset the editor. Used when the dataflow graph as changed */
	void ResetEditor();

	/** This adds the widget to switch from embdedded or external dataflow as well as read - only lock when applicable */
	void AddDataflowActionWidget(FToolMenuSection& Section);

	/** return true if the Dataflow is not an asset and is an object embedded into the owner asset */
	bool IsEmbeddedDataflow() const;

	/** return true if the Dataflow is a separate asset or no dataflow is assigned to the owner asset */
	bool IsExternalDataflow() const;

	/** return true if the bound asset is the reference asset or if the reference asset is null */
	bool IsReferenceAssetNullOrSameAsBoundAsset() const;

	/** Get Reference asset from the dataflow graph */
	UObject* GetReferenceAsset() const;

	/** return true the current dataflow can be edited - condition are must be External and match reference asset (if reference asset is defined)*/
	bool AllowExternalDataflowEditing() const;

	/** whether the Lock/Unlock editing button is visible or not */
	bool IsEditingLockButtonVisible() const;

	bool IsEditable() const;

	/** Duplicate the current external dataflow and embed it into the owner asset */
	void ConvertToEmbeddedDataflow();

	/** 
	* Duplicate the embedded dataflow object and store it into a separate asset  
	* this methid is interactive and will prompt a dialog for the user to choose a path to save the duplicated Dataflow to 
	*/
	void ConvertToExternalDataflow();

	void ToggleDataflowEditingLock();
	...
	FDelegateHandle OnContextLogDelegateHandle;

	// The currently selected set of dataflow nodes.
	TSet<TObjectPtr<UDataflowEdNode>> SelectedDataflowNodes;
	TSet<TObjectPtr<UEdGraphNode_Comment>> SelectedComments;

	/** Manages viewport gizmos for GizmoType-annotated properties on the currently selected node. */
	TObjectPtr<UDataflowPropertyGizmoManager> PropertyGizmoManager;
	...
	/** Allow editing of external dataflow */
	bool bAllowExternalDataflowEditing = false;
	...
	/** relevant message to display */
	TSharedRef<UE::Dataflow::Private::FLogMessageStorage> LogMessages;
