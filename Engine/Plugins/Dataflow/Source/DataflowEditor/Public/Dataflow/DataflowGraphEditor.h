...
public:
	...
	using FGraphEvaluationCallback = TFunction<void(const FDataflowNode*, const FDataflowOutput*)>;
	using FOnDragDropEventCallback = TFunction<void(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)>;
	using FIsEditableCallback = TFunction<bool()>;
	...
	SLATE_ARGUMENT(FGraphEvaluationCallback, EvaluateGraph)
	SLATE_ARGUMENT(FOnDragDropEventCallback, OnDragDropEvent)
	SLATE_ARGUMENT(FIsEditableCallback, IsEditable)
	...
	/** return whether or not the graph is editable */
	...
	//~ Begin FDataflowSEditorInterface 
	...
	UE_API virtual FText GetMessageText(const UDataflowEdNode& Node) const override;
	UE_API virtual EMessageSeverity::Type GetMessageMaxSeverity(const UDataflowEdNode& Node) const override;
	//~ End FDataflowSEditorInterface 
	...
	void SetLogMessageStorage(TSharedPtr<UE::Dataflow::Private::FLogMessageStorage> InLogMessages);
...
private:
	...
	FOnDragDropEventCallback OnDragDropEventCallback;
	FGraphEvaluationCallback EvaluateGraphCallback;
	FIsEditableCallback IsEditableCallback;
	...
	TSharedPtr<UE::Dataflow::Private::FLogMessageStorage> LogMessages;
};
