...
void SDataflowSimulationTimeline::Construct(const FArguments& InArgs, const TSharedRef<FDataflowSimulationBinding>& InSimulationBinding)
{
	...
	auto GetCacheStatusBarColor = [WeakSimulationBinding]() -> FLinearColor
		{
			bool bCacheActive = false;
			if (TSharedPtr<FDataflowSimulationBinding> SimulationBinding = WeakSimulationBinding.Pin())
			{
				bCacheActive = SimulationBinding->IsSimulationLocked();
			}
			return bCacheActive
				? FLinearColor(0.0f, 4.0f, 0.0f, 1.0f)
				: FLinearColor(.50f, .50f, .50f, 1.0f);
		};
						...
						[
							...
							[
								...
								//.BorderBackgroundColor(FLinearColor(.50f, .50f, .50f, 1.0f))
									//FLinearColor(0.0f, 4.0f, 0.0f, 1.0f)
								.BorderBackgroundColor_Lambda(GetCacheStatusBarColor)
								.Padding(0.f)
							]
							+ SVerticalBox::Slot()
							.AutoHeight()
