...
FCursorReply FDataflowTimeSliderController::OnCursorQuery(const SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
...
{
	// Clamp the scrub position to the visible view range
	const TRange<double> ViewRange = TimeSliderArgs.ViewRange.Get();
	if (ViewRange.HasLowerBound() && ViewRange.HasUpperBound())
	{
		const FFrameRate TickResolution = GetTickResolution();
		const FFrameTime LowerBound = ViewRange.GetLowerBoundValue() * TickResolution;
		const FFrameTime UpperBound = ViewRange.GetUpperBoundValue() * TickResolution;
