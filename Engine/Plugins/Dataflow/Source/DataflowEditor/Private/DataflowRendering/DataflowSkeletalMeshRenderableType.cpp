...
namespace UE::Dataflow::Private
{
	...
	class FSkeletalMeshSurfaceRenderableType : public IRenderableType
	{
		...
		virtual void GetPrimitiveComponents(const FRenderableTypeInstance& Instance, FRenderableComponents& OutComponents) const override
		{
			...
			{
				const FName ComponentName = Instance.GetComponentName(SkeletalMesh->GetFName());

				if (USkeletalMeshComponent* Component = OutComponents.AddNewComponent<USkeletalMeshComponent>(ComponentName))
				{
		...
		virtual void GetPrimitiveComponents(const FRenderableTypeInstance& Instance, FRenderableComponents& OutComponents) const override
		{
			...
			if (const TObjectPtr<USkeletalMesh> SkeletalMesh = GetSkeletalMesh(Instance, nullptr))
			{
				const int32 UVChannel = Settings
					...
					: 0;
				Rendering::AddUvDynamicMeshComponent(*SkeletalMesh, UVChannel, OutComponents);
			}
		}
	};
