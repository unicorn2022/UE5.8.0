...
void FDataflowEditorModeToolkit::Init(const TSharedPtr<IToolkitHost>& InitToolkitHost, TWeakObjectPtr<UEdMode> InOwningMode)
{
	...
	ModeDetailsView->OnFinishedChangingProperties().AddSP(this, &FDataflowEditorModeToolkit::OnSettingsPropertiesChanged);

	// add viewport overlay widget to accept / cancel tool
	MakeToolAcceptCancelWidget();
}

void FDataflowEditorModeToolkit::OnSettingsPropertiesChanged(const FPropertyChangedEvent& PropertyChangedEvent)
{
	if (ActiveToolName.IsEmpty())
	{
		RefreshConstructionScene();
	}
}
...
void FDataflowEditorModeToolkit::CustomizeModeDetailsViewArgs(FDetailsViewArgs& ArgsInOut)
{
	FBaseCharacterFXEditorModeToolkit::CustomizeModeDetailsViewArgs(ArgsInOut);
	ArgsInOut.bAllowSearch = true;
	ArgsInOut.bAllowFavoriteSystem = true;
}

void FDataflowEditorModeToolkit::RefreshConstructionScene()
{
...
void FDataflowEditorModeToolkit::OnRenderSettingsUpdated(const TArray<UDataflowRenderableTypeSettings*>& InRenderSettings)
{
	RenderSettings = InRenderSettings; 
	UpdateModeDetailsView();
}

void FDataflowEditorModeToolkit::UpdateModeDetailsView()
{
	if (ModeDetailsView)
	{
		...
		{
			ObjectsToSet.Append(CurTool->GetToolProperties(true));
		}
		else
		{
			// if a tool is up , it has its own settings, so we only display the selection render settings when the tool is not up 
			ObjectsToSet.Append(RenderSettings);
		}

		ModeDetailsView->SetObjects(ObjectsToSet);
	}
}
