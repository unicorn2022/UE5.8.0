...
void SDataflowConstructionViewport::Construct(const FArguments& InArgs, const FAssetEditorViewportConstructionArgs& InViewportConstructionArgs)
{
	...
	DataflowClient = InArgs._ViewportClient;
			...
			FNewMenuDelegate::CreateLambda([WeakViewport = Context->Viewport](FMenuBuilder& Menu)
				{
					...
					{
						if (const TSharedPtr<FDataflowConstructionViewportClient> DataflowClient = Viewport->GetDataflowClient())
						{
							...
							{
								Visualization.Value->ExtendViewportShowMenu(DataflowClient, Menu);
							}
						}
					}
				}),
...
{
	FocusViewportToSelection(/*bInstant*/false);
}

void SDataflowConstructionViewport::FocusViewportToSelection(bool bInstant)
{
	const UDataflowEditorMode* const DataflowEdMode = GetEdMode();

	if (DataflowEdMode && DataflowClient)
	{
		...
		if (BoundingBox.IsValid && !(BoundingBox.Min == FVector::Zero() && BoundingBox.Max == FVector::Zero()))
		{
			...
			DataflowClient->FocusViewportOnBoundingBox(BoundingBox, bInstant);
		}
	}
}
...
void SDataflowConstructionViewport::PopulateViewportOverlays(TSharedRef<SOverlay> Overlay)
{
	...
	Overlay->AddSlot()
		...
		[
			...
			.Padding(FMargin(8, 8))
			.Visibility(this, &SDataflowConstructionViewport::GetEditorModeMessageVisibility)
			[
				SNew(STextBlock)
				.ColorAndOpacity(FColor::Black)
				.Text(this, &SDataflowConstructionViewport::GetEditorModeMessage)
			]
		];
...
bool SDataflowConstructionViewport::HasEditorModeMessage() const
{
	if (const UDataflowEditorMode* const DataflowEdMode = GetEdMode())
	{
		return DataflowEdMode->HasUserMessage();
	}
	return false;
}

EVisibility SDataflowConstructionViewport::GetEditorModeMessageVisibility() const
{
	return HasEditorModeMessage()
		? EVisibility::Visible
		: EVisibility::Collapsed;
}

FText SDataflowConstructionViewport::GetEditorModeMessage() const
{
	if (const UDataflowEditorMode* const DataflowEdMode = GetEdMode())
	{
		return DataflowEdMode->GetUserMessage();
	}
	return {};
}
