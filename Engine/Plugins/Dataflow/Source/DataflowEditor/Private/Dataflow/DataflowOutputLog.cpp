...
void FDataflowOutputLog::CreateMessageLog()
{
	...
	const FString NameString = FString::Printf(TEXT("LogDataflow%lld"), UE::Dataflow::FTimestamp::Current());
...
{
	TSharedRef<FTokenizedMessage> BuildTokenizedMessage(const EMessageSeverity::Type InSeverity, const FText& InMessage, const FDataflowPath& InPath)
	{
		// Token0 - Severity
		// Token1 - Time code
		// Token2 - Path ()
		...
		TSharedRef<IMessageToken> TokenMessage = FTextToken::Create(InMessage);
...
{
	const FText TextMessage = FText::FromString(InMessage);
	AddMessage(InSeverity, TextMessage, InPath);
}

void FDataflowOutputLog::AddMessage(const EMessageSeverity::Type InSeverity, const FText& InMessage, const FDataflowPath& InPath)
{
	if (MessageLogListing.IsValid())
	{
	...
	UE_LOGF(LogDataflowContextOutput, Verbose, "[%ls][%ls]", *InPath.ToString(), *InMessage.ToString());
}
