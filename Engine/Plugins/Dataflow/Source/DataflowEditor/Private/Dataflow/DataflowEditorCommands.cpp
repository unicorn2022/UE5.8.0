...
void FDataflowEditorCommandsImpl::RegisterCommands()
{
	...
	UI_COMMAND(EvaluateGraph, "Evaluate Dataflow Graph", "Trigger an evaluation of the Dataflow graph terminal nodes.", EUserInterfaceActionType::Button, FInputChord(EKeys::F5));
	...
	UI_COMMAND(StartSimulation, "Start Dataflow Simulation", "Start the simulation of the asset", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(StopSimulation, "Stop Dataflow Simulation", "Stop the simulation of the asset.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(PauseSimulation, "Pause Dataflow Simulation", "Pause the simulation of the asset.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(StepSimulation, "Step Dataflow Simulation", "Advance the simulation of the asset by one frame.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ResetSimulation, "Reset Dataflow Simulation", "Reset the simulation of the asset.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(EvaluateGraphAutomatic, "Automatic Graph Evaluation", "Set the evaluation mode of the Dataflow graph to Automatic.", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(EvaluateGraphManual, "Manual Graph Evaluation", "Set the evaluation mode of the Dataflow graph to Manual.", EUserInterfaceActionType::RadioButton, FInputChord());
	UI_COMMAND(ClearGraphCache, "Force Graph Re-evaluation", "Force the entire Dataflow graph to re-evaluate by deleting the evaluation cache and re-evaluating the terminal nodes.", EUserInterfaceActionType::Button, FInputChord());
	...
	UI_COMMAND(ToggleAsyncEvaluation, "Asynchronous Evaluation (Experimental)", "Toggle asynchronous evaluation of the Dataflow graph (This is an experimental feature).", EUserInterfaceActionType::ToggleButton, FInputChord());
	...
	UI_COMMAND(ZoomToFitGraph, "ZoomToFitGraph", "Fit the Dataflow graph in the graph editor viewport.", EUserInterfaceActionType::None, FInputChord(EKeys::F));
	...
	UI_COMMAND(ConvertToEmbeddedDataflow, "Embed Dataflow", "Embed the Dataflow graph in the current asset", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ConvertToExternalDataflow, "Extract and Reference Dataflow...", "Convert this Dataflow graph to be an external Dataflow asset that can be shared by other asset and assign it to this asset", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ToggleDataflowEditingLock, "Lock/Unlock Editing", "Toggle editing for the current Dataflow graph.", EUserInterfaceActionType::ToggleButton, FInputChord());
	...
	for (const FName& NodeName : NodeNames)
	{
		...
		FUICommandInfo::MakeCommandInfo(
			...
			FText::Format(LOCTEXT("AddDataflowNodeTooltip", "Add a \"{0}\" node to the Dataflow graph"), FText::FromString(NodeName.ToString())),
			ToolRegistry.GetAddNodeButtonIcon(NodeName),
...
{
	UE_LOGF(LogChaosDataflow, VeryVerbose, "FDataflowEditorCommands::EvaluateNode(): Node [%ls], NodeName [%ls] Output [%ls]", Node ? *Node->GetName().ToString() : TEXT("nullptr"), *NodeName, Output ? *Output->GetName().ToString() : TEXT("nullptr"));
			...
			[WeakNode, WeakAsset](UE::Dataflow::FContext& Context)
			{
				if (TSharedPtr<const FDataflowNode> Node = WeakNode.Pin())
				{
					if (const FDataflowTerminalNode* const TerminalNode = Node->AsType<const FDataflowTerminalNode>())
					{
						...
						{
							TStrongObjectPtr<UObject> Asset = WeakAsset.Pin();
							UE_LOGF(LogChaosDataflow, Verbose, "FDataflowTerminalNode::SetAssetValue(): TerminalNode [%ls], Asset [%ls]", *TerminalNode->GetName().ToString(), Asset? *Asset->GetName(): TEXT("<none>"));
...
{
	UE_LOGF(LogChaosDataflow, VeryVerbose, "FDataflowEditorCommands::EvaluateNode() : Node [%ls], Output [%ls]", *Node.GetName().ToString(), Output ? *Output->GetName().ToString() : TEXT("nullptr"));
	...
	if (InOutLastNodeTimestamp < Node.GetTimestamp())
	{
		...
		{
			Node.EvaluateNode(Context);
			OnPostEvaluation(Context);
		}
...
void FDataflowEditorCommands::OnNotifyPropertyPreChange(TSharedPtr<IStructureDetailsView> PropertiesEditor, UDataflow* Graph, class FEditPropertyChain* PropertyAboutToChange)
{
	...
	if (PropertiesEditor && Graph)
	{
		...
		if (TSharedPtr<const IStructureDataProvider> StructProvider = PropertiesEditor->GetStructureProvider())
		{
			...
			else if (BaseStruct && BaseStruct->IsChildOf(UEdGraphNode_Comment::StaticClass()))
			{
				// Manually capture UObject state for undo/redo; FStructOnScope won't auto-Modify it.
				TArray<TSharedPtr<FStructOnScope>> StructData;
				StructProvider->GetInstances(StructData, UEdGraphNode_Comment::StaticClass());
				for (const TSharedPtr<FStructOnScope>& StructOnScope : StructData)
				{
					if (StructOnScope.IsValid() && StructOnScope->IsValid())
					{
						if (UEdGraphNode_Comment* const CommentNode = reinterpret_cast<UEdGraphNode_Comment*>(StructOnScope->GetStructMemory()))
						{
							CommentNode->Modify();
						}
					}
...
void FDataflowEditorCommands::OnCommentPropertyValueChanged(const FPropertyChangedEvent& InPropertyChangedEvent, const TSet<TObjectPtr<UEdGraphNode_Comment>>& SelectedComments)
{
	if (InPropertyChangedEvent.ChangeType != EPropertyChangeType::Interactive)
	{
		for (UEdGraphNode_Comment* const SelectedComment : SelectedComments)
		{
			if (SelectedComment)
			{
				FPropertyChangedEvent NonConstEvent = InPropertyChangedEvent;
				SelectedComment->PostEditChangeProperty(NonConstEvent);
			}
		}
	}
}
					...
					if (UDataflowEdNode* EdNode = Cast<UDataflowEdNode>(SelectedObject))
					{
						if (TSharedPtr<FDataflowNode> DataflowNode = DataflowGraph->FindBaseNode(EdNode->GetDataflowNodeGuid()))
						{
							FIsPropertyReadOnly IsPropertyReadOnlyLambda = FIsPropertyReadOnly::CreateLambda(
								[WeakNode = DataflowNode.ToWeakPtr()](const FPropertyAndParent& PropertyAndParent)
								{
