						...
						if (const FProperty* const Property = Struct->FindPropertyByName(CollectionPropertyName))
						{
							if (const FStructProperty* const StructProperty = CastField<FStructProperty>(Property))
							{
								if (StructProperty->GetCPPType(nullptr, CPPF_None) == TEXT("FManagedArrayCollection"))
								{
									...
									{
										// Read-only: do not force evaluation here. Evaluation is the toolkit's responsibility (on selection / on invalidation).
										UE::Dataflow::FContextThreaded EmptyContext;
										UE::Dataflow::FContext& EffectiveContext = Context.IsValid() ? *Context : EmptyContext;
										if (const FDataflowOutput* const ConnectedOutput = DataflowInput->GetConnection())
										{
											return ConnectedOutput->ReadValue(EffectiveContext, EmptyCollection);
										}
