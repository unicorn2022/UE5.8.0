...
bool FDataflowTaskManager::AllocateSimulationResource(const FVector2f& TimeRange, const int32 FrameRate,
		...
		const FInstancedPropertyBag& BlueprintVariables, const bool bSkeletalMeshVisibility, float DeltaTime)
{
	...
	PreviewActor = UE::Dataflow::SpawnSimulatedActor(ActorClass, CacheManager, CacheAsset, true, DataflowContent, BlueprintTransform, BlueprintVariables);
	...
	if (PreviewActor)
	{
		// Prevent the component from having a rendering proxy that can cause unecessary render state invalidation 
		// and potential issue when calling those from a worker thread
		PreviewActor->ForEachComponent<UPrimitiveComponent>(true, [](UPrimitiveComponent* Component)
			{
...
bool FDataflowTaskManager::AllocateSimulationResource(const FVector2f& TimeRange, const int32 FrameRate,
	const TObjectPtr<UChaosCacheCollection>& CacheAsset, const TSubclassOf<AActor>& ActorClass,
	const TObjectPtr<UDataflowBaseContent>& DataflowContent, const FTransform& BlueprintTransform,
	const bool bSkeletalMeshVisibility, float DeltaTime)
{
	return AllocateSimulationResource(TimeRange, FrameRate,
		CacheAsset, ActorClass,
		DataflowContent, BlueprintTransform,
		FInstancedPropertyBag(), bSkeletalMeshVisibility, DeltaTime);
}
...
void FDataflowSimulationGenerator::StartGenerateSimulation()
{
	...
	if (TaskManager != nullptr)
	{
		UE_LOGF(LogDataflowSimulationGenerator, Error, "Previous generation is still running.");
	...
	TaskManager->AllocateSimulationResource(TimeRange, CacheParams.FrameRate,
		CacheAsset, BlueprintClass,
		DataflowContent, BlueprintTransform,
		BlueprintVariables, bSkeletalMeshVisibility, GeneratorDeltaTime);
...
void FDataflowSimulationGenerator::SetBlueprintVariables(const FInstancedPropertyBag& InBlueprintVariables)
{
	BlueprintVariables = InBlueprintVariables;
}
...
void FDataflowSimulationGenerator::FreeTaskResource(bool bCancelled)
{
	...
	TaskManager->FreeSimulationResource();
	const FDateTime CurrentTime = FDateTime::UtcNow();
	UE_LOGF(LogDataflowSimulationGenerator, Log, "Simulation finished in %f seconds", (CurrentTime - TaskManager->StartTime).GetTotalSeconds());
