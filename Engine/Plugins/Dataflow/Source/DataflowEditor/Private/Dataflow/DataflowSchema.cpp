...
int32 UDataflowSchema::CurrentCacheRefreshID = 0;
...
void UDataflowSchema::GetContextMenuActions(class UToolMenu* Menu, class UGraphNodeContextMenuContext* Context) const
{
	...
	if (const UDataflowEdNode* EdNode = Cast<UDataflowEdNode>(Context->Node))
	{
		if (Context->Pin && Context->Pin->Direction == EGPD_Output)
		{
			...
			{
				if (Connection->IsAnyType() && !Connection->IsTypeLocked())
				{
					TArray<FName> SupportedTypes;
					Connection->GetSupportedTypes(SupportedTypes);
					if (SupportedTypes.Num() > 0)
					{
						FToolMenuSection& Section = Menu->AddSection("DataflowSchema_PinContextMenu_SectionTypes", LOCTEXT("DataflowSchema_PinContextMenu_SectionTypes_Text", "Compatible Types"));
						for (FName SupportedType : SupportedTypes)
						{
							Section.AddMenuEntry(
								SupportedType,
								FText::Format(LOCTEXT("PinConvertType_Label", "Convert to {0}"), FText::FromName(SupportedType)),
								FText::Format(LOCTEXT("PinConvertType_Tooltip", "Convert the output type {0}"), FText::FromName(SupportedType)),
								FSlateIcon(),
								FUIAction(
									// Fine to capture raw pointers because the lifetime of teh menu is shorter than the connection and node lifetime
									FExecuteAction::CreateLambda([SupportedType, Connection, EdNode]()
										{
											if (FDataflowNode* Node = Connection->GetOwningNode())
											{
												Node->SetOutputConnectionConcreteType(Connection, SupportedType);
												Node->NotifyConnectionTypeChanged(Connection);

												// teh context only gives a const node, but we need to update the pins 
												if (UDataflowEdNode* MutableEdNode = const_cast<UDataflowEdNode*>(EdNode))
												{
													MutableEdNode->UpdatePinsFromDataflowNode();
												}
												Node->Invalidate();
											}
										}),
									// Fine to capture raw pointers because the lifetime of teh menu is shorter than the connection and node lifetime
									FCanExecuteAction::CreateLambda([SupportedType, Connection]()
										{
											return (Connection 
												&& !Connection->IsTypeLocked() 
												&& !Connection->IsConnected() 
												&& Connection->GetType() != SupportedType
												&& !Connection->IsAnytypeDependencyConnected()
												);
										})
								)
							);
		...
		else if (Context->Pin && Context->Pin->Direction == EGPD_Input)
		{
			if (FDataflowConnection* Connection = UDataflowEdNode::GetConnectionFromPin(Context->Pin))
			{
				...
				if (TSharedPtr<const FDataflowNode> DataflowNode = UDataflowEdNode::GetDataflowNodeFromEdNode(EdNode))
				{
					if (DataflowNode->CanAddPin())
					{
						Section.AddSeparator(TEXT("PinActionSeperator_OptionPins"));
						Section.AddMenuEntry(FDataflowEditorCommands::Get().AddOptionPin, LOCTEXT("DataflowContextPinMenu_AddOptionPin", "Add Option Pin"));
						Section.AddMenuEntry(FDataflowEditorCommands::Get().RemoveOptionPin, LOCTEXT("DataflowContextPinMenu_RemoveOptionPin", "Remove Last Option Pin"));
					}
				}
			}
		}
...
bool UDataflowSchema::IsCacheVisualizationOutOfDate(int32 InVisualizationCacheID) const
{
	return CurrentCacheRefreshID != InVisualizationCacheID;
}

int32 UDataflowSchema::GetCurrentVisualizationCacheID() const
{
	return CurrentCacheRefreshID;
}

void UDataflowSchema::ForceVisualizationCacheClear() const
{
