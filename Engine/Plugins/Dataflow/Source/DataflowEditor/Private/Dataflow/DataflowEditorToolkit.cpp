...
bool bDataflowForceGraphEditable = false;
FAutoConsoleVariableRef CVARbDataflowForceGraphEditable(TEXT("p.Dataflow.Editor.ForceGraphEditable"), bDataflowForceGraphEditable,
	TEXT("when enabled all graph will be editable and the Unlock/lock editing buttons will not be visible.[def:false]"));
...
namespace UE::Dataflow::Private
{
	...
	static const float DataflowLayoutTimelineMaxHeight = 75.f;
	...
	void FLogMessageStorage::GetMessage(int32 Index, UE::Dataflow::FContext::FLogMessage& OutMessage) const
	{
		if (Entries.IsValidIndex(Index))
		{
			OutMessage = Entries[Index].Message;
		}
		else
		{
			OutMessage = UE::Dataflow::FContext::FLogMessage();
		}

	}

	void FLogMessageStorage::Add(const UE::Dataflow::FContext::FLogMessage& Message)
	{
	...
	void FLogMessageStorage::GetMessageForNode(const FDataflowNode& Node, TArray<UE::Dataflow::FContext::FLogMessage>& OutMessages) const
	{
		for (const FEntry& Entry : Entries)
		{
			if (Entry.Message.Node == Node.AsWeak())
			{
				OutMessages.Add(Entry.Message);
			}
		}
	}

	EMessageSeverity::Type FLogMessageStorage::GetMaxSeverityForNode(const FDataflowNode& Node) const
	{
		EMessageSeverity::Type OutSeverity = EMessageSeverity::Info;
		for (const FEntry& Entry : Entries)
		{
			if (Entry.Message.Node == Node.AsWeak())
			{
				if (Entry.Message.Severity < OutSeverity)
				{
					OutSeverity = Entry.Message.Severity;
				}
			}
		}
		return OutSeverity;
	...
	void FLogMessageStorage::RemoveForNode(const FDataflowNode& Node, const FTimestamp& Timestamp)
	{
		TArray<FEntry> NewEntries;
		...
		for (const FEntry& Entry : Entries)
		{
			...
			if (Entry.Message.Node == Node.AsWeak())
			{
				if (Timestamp == FTimestamp::Invalid || Entry.Timestamp < Timestamp)
				{
					bKeep = false;
				}
			}
			...
			{
				// invalid node , remove the message even if the node does not match
				bKeep = false;
			}
			if (bKeep)
			{
				NewEntries.Emplace(Entry);
			}
		}
		Entries = MoveTemp(NewEntries);
	}

	EMessageSeverity::Type FLogMessageStorage::GetMaxSeverity()
	{
		EMessageSeverity::Type OutSeverity = EMessageSeverity::Info;
		for (const FEntry& Entry : Entries)
		{
...
FDataflowEditorToolkit::FDataflowEditorToolkit(UAssetEditor* InOwningAssetEditor)
	...
	, LogMessages(MakeShared<UE::Dataflow::Private::FLogMessageStorage>())
{
			...
			(
				...
				FTabManager::NewSplitter()->SetOrientation(Orient_Vertical)
					...
					FTabManager::NewStack()
							...
							->SetMinSize(UE::Dataflow::Private::DataflowLayoutTimelineMaxHeight)
...
void FDataflowEditorToolkit::RegisterContextHandlers()
{
	...
	if (TSharedPtr<UE::Dataflow::FEngineContext> DataflowContext = EditorContent->GetDataflowContext())
	{
		...
		OnContextLogDelegateHandle = DataflowContext->GetOnContextLogMulticast().AddSP(this, &FDataflowEditorToolkit::OnContextLog);
	}

	FDataflowAssetDelegates::OnNodeInvalidated.AddSP(this, &FDataflowEditorToolkit::OnNodeInvalidated);
	FDataflowAssetDelegates::OnDataflowPostEditUndo.AddSPLambda(this, [this, EditorContent](UDataflow& DataflowAsset)
		{
			// An undo/redo may have changed which Dataflow the owning asset references
			// (e.g. undoing the creation of a new embedded Dataflow on a Groom). If the
			// panel is no longer in sync with the owner, rebind the graph editor.
			if (EditorContent->GetDataflowAsset() != GetDataflowAsset(EditorContent->GetDataflowOwner()))
			{
				ResetEditor();
				
			}

			if (NodeDetailsEditor.IsValid() && IsValid(&DataflowAsset))
			{
				// Bug in SStructureDetailsView (UE-349909) causes the UI to get out of sync and requires the view to be recreated
				TSet<TObjectPtr<UObject>> SelectionSet;
				Algo::Transform(SelectedDataflowNodes, SelectionSet, [](const TObjectPtr<UDataflowEdNode>& EdNode) { return Cast<UObject>(EdNode); });
				FDataflowEditorCommands::OnSelectedNodesChanged(NodeDetailsEditor, &DataflowAsset, &DataflowAsset, SelectionSet);
			}
		});
}
...
void FDataflowEditorToolkit::UnregisterContextHandlers()
{
	...
	if (TSharedPtr<UE::Dataflow::FEngineContext> DataflowContext = EditorContent->GetDataflowContext())
	{
		...
		DataflowContext->GetOnContextLogMulticast().Remove(OnContextLogDelegateHandle);
	}

	FDataflowAssetDelegates::OnNodeInvalidated.RemoveAll(this);
	FDataflowAssetDelegates::OnDataflowPostEditUndo.RemoveAll(this);
}
...
{
	// Tear down property gizmos before the mode's tools context is destroyed.
	if (PropertyGizmoManager && PropertyGizmoManager->HasActiveGizmos())
	{
		if (UDataflowEditorMode* const DataflowMode = Cast<UDataflowEditorMode>(EditorModeManager->GetActiveScriptableMode(UDataflowEditorMode::EM_DataflowEditorModeId)))
		{
			PropertyGizmoManager->Teardown(DataflowMode->GetInteractiveToolsContext()->ToolManager);
		}
	}
...
void FDataflowEditorToolkit::PostInitAssetEditor()
{
	...
	PropertyGizmoManager = NewObject<UDataflowPropertyGizmoManager>();
	...
	auto SetCommonViewportClientOptions = [](FEditorViewportClient* Client)
	{
		...
		// Enable the dithering of occluded selection and portions of gizmos.
		Client->EngineShowFlags.SetOpaqueCompositeEditorPrimitives(false);
	...
	AddDataflowActionWidget(Section);
	...
	// Enforce the timeline min size even when restoring an older saved layout that predates the MinSize field.
	// The SSplitter slot binds GetNodeMinSize() as a TAttribute, so setting it here takes effect immediately.
	if (TSharedPtr<SDockTab> TimelineTab = TabManager->FindExistingLiveTab(SimulationTimelineTabId))
	{
		if (TSharedPtr<SDockingTabStack> TimelineStack = TimelineTab->GetParentDockTabStack())
		{
			TimelineStack->SetNodeMinSize(UE::Dataflow::Private::DataflowLayoutTimelineMaxHeight);
		}
	}
}
...
{
	if (UDataflowEditorUserSettings* Settings = GetMutableDefault<UDataflowEditorUserSettings>())
	{
		Settings->bShowPerformanceData = !Settings->bShowPerformanceData;
...
bool FDataflowEditorToolkit::IsPerfDataEnabled() const
{
	if (UDataflowEditorUserSettings* Settings = GetMutableDefault<UDataflowEditorUserSettings>())
	{
		return Settings->bShowPerformanceData;
	}
	return false;
}
...
void FDataflowEditorToolkit::ToggleAsyncEvaluation()
{
	if (UDataflowEditorUserSettings* Settings = GetMutableDefault<UDataflowEditorUserSettings>())
	{
		Settings->bUseAsyncEvaluation = !Settings->bUseAsyncEvaluation;
		Settings->PostEditChange();
		Settings->SaveConfig();
	}
}
...
bool FDataflowEditorToolkit::IsAsyncEvaluationEnabled() const
{
	if (UDataflowEditorUserSettings* Settings = GetMutableDefault<UDataflowEditorUserSettings>())
	{
		return Settings->bUseAsyncEvaluation;
	}
	return false;
}
...
FSlateIcon FDataflowEditorToolkit::GetEvaluationStatusImage() const
{
	...
	{
		const EMessageSeverity::Type Severity = LogMessages->GetMaxSeverity();
		switch (Severity)
		{
	...
	InSection.AddDynamicEntry("DataflowEvaluationOptionsDynamic", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& DynamicSection)
		{
			...
			if (const TSharedPtr<FDataflowEditorToolkit> Toolkit = GetAssetEditorToolkitFromMenuContext(DynamicSection.FindContext<UAssetEditorToolkitMenuContext>()))
			{
				FToolMenuEntry EvaluationOptions = FToolMenuEntry::InitComboButton(
					...
					LOCTEXT("DataflowEvaluationOptions_ToolbarTooltip", "Options to customize how the Dataflow Graph evaluates"),
...
void FDataflowEditorToolkit::AddDataflowActionWidget(FToolMenuSection& InSection)
{
	InSection.AddSeparator(TEXT("Dataflow Actions"));

	// Convert to embedded Dataflow button
	ToolkitCommands->MapAction(FDataflowEditorCommands::Get().ConvertToEmbeddedDataflow,
		FExecuteAction::CreateSP(this, &FDataflowEditorToolkit::ConvertToEmbeddedDataflow),
		FCanExecuteAction::CreateSP(this, &FDataflowEditorToolkit::IsExternalDataflow),
		FIsActionChecked(),
		FIsActionButtonVisible::CreateSP(this, &FDataflowEditorToolkit::IsExternalDataflow)
	);

	InSection.AddDynamicEntry("DataflowActionConvertToEmbed", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& DynamicSection)
		{
			DynamicSection.AddEntry(FToolMenuEntry::InitToolBarButton(
				FDataflowEditorCommands::Get().ConvertToEmbeddedDataflow,
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon()
			));
	...
	// Convert to external Dataflow button
	ToolkitCommands->MapAction(FDataflowEditorCommands::Get().ConvertToExternalDataflow,
		FExecuteAction::CreateSP(this, &FDataflowEditorToolkit::ConvertToExternalDataflow),
		FCanExecuteAction::CreateSP(this, &FDataflowEditorToolkit::IsEmbeddedDataflow),
		FIsActionChecked(),
		FIsActionButtonVisible::CreateSP(this, &FDataflowEditorToolkit::IsEmbeddedDataflow)
	);

	InSection.AddDynamicEntry("DataflowActionConvertToExternal", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& DynamicSection)
		{
			DynamicSection.AddEntry(FToolMenuEntry::InitToolBarButton(
				FDataflowEditorCommands::Get().ConvertToExternalDataflow,
				TAttribute<FText>(),
				TAttribute<FText>(),
				FSlateIcon()
			));
	...
	ToolkitCommands->MapAction(FDataflowEditorCommands::Get().ToggleDataflowEditingLock,
		FExecuteAction::CreateSP(this, &FDataflowEditorToolkit::ToggleDataflowEditingLock),
		FCanExecuteAction::CreateSP(this, &FDataflowEditorToolkit::IsExternalDataflow),
		FIsActionChecked::CreateSP(this, &FDataflowEditorToolkit::IsEditable),
		FIsActionButtonVisible::CreateSP(this, &FDataflowEditorToolkit::IsEditingLockButtonVisible)
	);

	InSection.AddDynamicEntry("DataflowActionLockUnlockGraph", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& DynamicSection)
		{
			using namespace UE::Dataflow::Private;
			if (const TSharedPtr<FDataflowEditorToolkit> Toolkit = GetAssetEditorToolkitFromMenuContext(DynamicSection.FindContext<UAssetEditorToolkitMenuContext>()))
			{
				const FSlateIcon UnlockedIcon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Unlock");
				const FSlateIcon LockedIcon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "Icons.Lock");
				const FText UnlockedText = LOCTEXT("LockEditing_Text", "Lock Editing");
				const FText UnlockedTooltip = LOCTEXT("lockEditing_Tooltip", "Lock Editing for this external Dataflow asset so that other assets using will not be affected by any potential changes (Default behavior)");
				const FText LockedText = LOCTEXT("UnlockEditing_Text", "Unlock Editing");
				const FText LockedTooltip = LOCTEXT("UnlockEditing_Tooltip", "Unlock Editing for this external Dataflow asset\nWarning : this asset may be used by other asset and changes made will affect them");

				TWeakPtr<FDataflowEditorToolkit> WeakToolkit = Toolkit;
				DynamicSection.AddEntry(FToolMenuEntry::InitToolBarButton(
					FDataflowEditorCommands::Get().ToggleDataflowEditingLock,
					TAttribute<FText>::CreateLambda([WeakToolkit, UnlockedText, LockedText]() 
						{ 
							if (TSharedPtr<FDataflowEditorToolkit> ToolkitPtr = WeakToolkit.Pin())
							{
								return ToolkitPtr->AllowExternalDataflowEditing() ? UnlockedText : LockedText;
							}
							return UnlockedText;
						}),
					TAttribute<FText>::CreateLambda([WeakToolkit, UnlockedTooltip, LockedTooltip]() 
						{ 
							if (TSharedPtr<FDataflowEditorToolkit> ToolkitPtr = WeakToolkit.Pin())
							{
								return ToolkitPtr->AllowExternalDataflowEditing() ? UnlockedTooltip : LockedTooltip;
							}
							return UnlockedTooltip;
						}),
					TAttribute<FSlateIcon>::CreateLambda([WeakToolkit, UnlockedIcon, LockedIcon]()
						{ 
							if (TSharedPtr<FDataflowEditorToolkit> ToolkitPtr = WeakToolkit.Pin())
							{
								return ToolkitPtr->AllowExternalDataflowEditing() ? UnlockedIcon : LockedIcon;
							}
							return UnlockedIcon;
						})
				));
	...
	// Open reference asset button 
	InSection.AddDynamicEntry("DataflowActionOpenReferenceAsset", FNewToolMenuSectionDelegate::CreateLambda([](FToolMenuSection& DynamicSection)
		{
			using namespace UE::Dataflow::Private;
			if (const TSharedPtr<FDataflowEditorToolkit> Toolkit = GetAssetEditorToolkitFromMenuContext(DynamicSection.FindContext<UAssetEditorToolkitMenuContext>()))
			{
				...
				DynamicSection.AddEntry(FToolMenuEntry::InitToolBarButton(
					TEXT("DataflowOpenReferenceAsset"),
					FUIAction(
						FExecuteAction::CreateLambda([WeakToolkit]()
							{
								if (TSharedPtr<FDataflowEditorToolkit> ToolkitPtr = WeakToolkit.Pin())
								{
									if (UObject* ReferenceAsset = ToolkitPtr->GetReferenceAsset())
									{
										GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OpenEditorForAsset(ReferenceAsset);
									}
								}
							}
						),
						FCanExecuteAction::CreateLambda([WeakToolkit]()
							{
								if (TSharedPtr<FDataflowEditorToolkit> ToolkitPtr = WeakToolkit.Pin())
								{
									return !ToolkitPtr->IsReferenceAssetNullOrSameAsBoundAsset();
								}
								return false;
							}
						)
					),
					TAttribute<FText>(LOCTEXT("OpenReferenceAssetLabel", "Open Reference Asset")),
					LOCTEXT("OpenReferenceAssetTooltip", "Open reference asset in an editor"),
...
bool FDataflowEditorToolkit::IsEmbeddedDataflow() const
{
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		if (const UDataflow* DataflowAsset = EditorContent->GetDataflowAsset())
		{
			return !DataflowAsset->IsAsset();
		}
	}
	return false;
}

bool FDataflowEditorToolkit::IsExternalDataflow() const
{
	return !IsEmbeddedDataflow();
}

UObject* FDataflowEditorToolkit::GetReferenceAsset() const
{
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		if (const UDataflow* DataflowAsset = EditorContent->GetDataflowAsset())
		{
			return DataflowAsset->ReferenceAsset;
		}
	}
	return nullptr;
}

bool FDataflowEditorToolkit::IsReferenceAssetNullOrSameAsBoundAsset() const
{
	UObject* ReferenceAsset = nullptr;
	bool bIsReferenceAssetNullOrSameAsBoundAsset = true;
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		if (const UDataflow* DataflowAsset = EditorContent->GetDataflowAsset())
		{
			ReferenceAsset = DataflowAsset->ReferenceAsset;
		}
		TObjectPtr<UObject> BoundAsset = EditorContent->GetDataflowOwner();
		bIsReferenceAssetNullOrSameAsBoundAsset = (ReferenceAsset == nullptr) || (BoundAsset == ReferenceAsset);
	}
	return bIsReferenceAssetNullOrSameAsBoundAsset;
}

bool FDataflowEditorToolkit::AllowExternalDataflowEditing() const
{
	return IsExternalDataflow() && bAllowExternalDataflowEditing && IsReferenceAssetNullOrSameAsBoundAsset();
}

bool FDataflowEditorToolkit::IsEditable() const
{
	return (IsEmbeddedDataflow() || AllowExternalDataflowEditing() || bDataflowForceGraphEditable);
}

bool FDataflowEditorToolkit::IsEditingLockButtonVisible() const
{
	return IsExternalDataflow() && IsReferenceAssetNullOrSameAsBoundAsset() && !bDataflowForceGraphEditable;
}

void FDataflowEditorToolkit::ConvertToEmbeddedDataflow()
{
	ensure(IsExternalDataflow());

	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		if (TObjectPtr<UObject> BoundAsset = EditorContent->GetDataflowOwner())
		{
			UDataflow* NewDataflowAsset = nullptr;
			if (UE::DataflowAssetDefinitionHelpers::EmbedDataflowAsset(BoundAsset, NewDataflowAsset))
			{
				BoundAsset->Modify();
				ResetEditor();
			}
...
void FDataflowEditorToolkit::ConvertToExternalDataflow()
{
	ensure(IsEmbeddedDataflow());
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		if (TObjectPtr<UObject> BoundAsset = EditorContent->GetDataflowOwner())
		{
			UDataflow* NewDataflowAsset = nullptr;
			if (UE::DataflowAssetDefinitionHelpers::MakeShareableDataflowAsset(BoundAsset, NewDataflowAsset))
			{
				ResetEditor();
			}
		}
...
void FDataflowEditorToolkit::ToggleDataflowEditingLock()
{
	if (IsExternalDataflow() && IsReferenceAssetNullOrSameAsBoundAsset())
	{
		bAllowExternalDataflowEditing = !bAllowExternalDataflowEditing;
	}
}
...
void FDataflowEditorToolkit::OnAssetsSavedAs(const TArray<UObject*>& SavedObjects)
{
	...
	for (UObject* const SavedObj : SavedObjects)
	{
		...
		else if (IDataflowInstanceInterface* Interface = UE::Dataflow::InstanceUtils::GetDataflowInterfaceFromObject(SavedObj))
		{
			NewAssetWithInterface = SavedObj;
			NewDataflowInterface = Interface;
		}
...
void FDataflowEditorToolkit::CreateWidgets()
{
	...
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		if (UDataflow* DataflowAsset = EditorContent->GetDataflowAsset())
		{
			NodeDetailsEditor = CreateNodeDetailsEditorWidget(EditorContent->GetDataflowAsset());
...
void FDataflowEditorToolkit::OnPropertyValueChanged(const FPropertyChangedEvent& PropertyChangedEvent)
{
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		...
		if (UDataflow* DataflowAsset = EditorContent->GetDataflowAsset())
		{
			...
			{
				FDataflowEditorCommands::OnCommentPropertyValueChanged(PropertyChangedEvent, SelectedComments);
			}
	...
	// Keep viewport gizmos in sync with the updated property values.
	if (PropertyGizmoManager)
	{
		UpdateGizmos();
	}
}
...
void FDataflowEditorToolkit::ResetEditor()
{
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		if (EditorContent)
		{
			if (IDataflowInstanceInterface* DataflowInstanceInterface = UE::Dataflow::InstanceUtils::GetDataflowInterfaceFromObject(EditorContent->GetDataflowOwner()))
			{
	...
	// Reset the editing state for external dataflows
	bAllowExternalDataflowEditing = false;
}
...
void FDataflowEditorToolkit::OnAssetPropertyValueChanged(const FPropertyChangedEvent& PropertyChangedEvent)
{
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		...
		{
			FDataflowEditorCommands::OnAssetPropertyValueChanged(EditorContent, PropertyChangedEvent);
		}
	}
}
...
void FDataflowEditorToolkit::OnNodeSelectionChanged(const TSet<UObject*>& InNewSelection)
{
	...
	SelectedComments.Reset();
	auto IsACommentNode = [](UObject* Obj) { return Cast<UEdGraphNode_Comment>(Obj) != nullptr; };
	auto CastToCommentNode = [](UObject* Obj) { return Cast<UEdGraphNode_Comment>(Obj); };
	Algo::TransformIf(InNewSelection, SelectedComments, IsACommentNode, CastToCommentNode);
...
bool FDataflowEditorToolkit::IsGraphDirty() const
{
	...
	// we need to check the specific terminal for this asset and see if it's up to date 
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		const FTimestamp LastModifiedTimestamp = EditorContent->GetLastModifiedTimestamp();
		const FName TerminalNodeName(EditorContent->GetDataflowTerminal());

		if (UDataflow* DataflowAsset = EditorContent->GetDataflowAsset())
		{
			if (TSharedPtr<FGraph> DataflowGraph = DataflowAsset->GetDataflow())
			{
				if (FGraph::FConstNodeSharedPtr Node = DataflowGraph->FindBaseNode(TerminalNodeName))
				{
					if (const FDataflowTerminalNode* TerminalNode = Node->AsType<FDataflowTerminalNode>())
					{
					...
					{
						// no main terminal means always dirty
						return true;
					}
				}
			}
...
void FDataflowEditorToolkit::OnNodeInvalidated(UDataflow& DataflowAsset, FDataflowNode& Node)
{
	if (EvaluationMode == EDataflowEditorEvaluationMode::Automatic)
	{
		if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
		{
			if (EditorContent->GetDataflowAsset() == &DataflowAsset)
			{
				...
				{
					// also add "terminal" normal nodes ( no output connections )
					bool bHasOutputConnection = false;
					Node.ForEachConnection([&bHasOutputConnection](const FDataflowConnection* Connection)
						{
							if (Connection->GetDirection() == UE::Dataflow::FPin::EDirection::OUTPUT)
							{
								bHasOutputConnection |= Connection->IsConnected();
							}
							return !bHasOutputConnection;
						});
					if (!bHasOutputConnection)
					{
						NodesToEvaluateOnTick.Add(Node.GetGuid());
					}
				}
			}
		}
	...
	LogMessages->RemoveForNode(Node, Node.GetTimestamp());
}
...
void FDataflowEditorToolkit::OnNodeDeleted(const TSet<UObject*>& NewSelection)
{
	...
	FGraphPanelSelectionSet DeletedNodesSet;
	for (UDataflowEdNode* Node : DeletedNodes)
	{
		if (SelectedDataflowNodes.Contains(Node))
		{
			SelectedDataflowNodes.Remove(Node);
		}
		DeletedNodesSet.Add(Node);
	}

	// remove from the logs
	for (UDataflowEdNode* Node : DeletedNodes)
	{
		if (TSharedPtr<const FDataflowNode> DataflowNode = Node->GetDataflowNode())
		{
			LogMessages->RemoveForNode(*DataflowNode);
		}
	}

	if (UDataflowEditorMode* DataflowEdMode = Cast<UDataflowEditorMode>(EditorModeManager->GetActiveScriptableMode(UDataflowEditorMode::EM_DataflowEditorModeId)))
	{
		DataflowEdMode->OnDataflowNodeDeleted(DeletedNodesSet);
	}
}
...
void FDataflowEditorToolkit::LogMessage(const EMessageSeverity::Type Severity, const FDataflowNode* Node, const FDataflowOutput* Output, const FString& Message) const
{
	...
	LogMessage(Severity, Node, Output, TextMessage);
}

void FDataflowEditorToolkit::LogMessage(const EMessageSeverity::Type Severity, const FDataflowNode* Node, const FDataflowOutput* Output, const FText& Message) const
{
	if (DataflowOutputLog)
	{
		FDataflowPath Path;
...
void FDataflowEditorToolkit::RefreshLogMessageView()
{
	if (DataflowOutputLog)
	{
		DataflowOutputLog->ClearMessageLog();

		UE::Dataflow::FContext::FLogMessage Message;
		const int32 NumMessages = LogMessages->Num();
		for (int32 Index = 0; Index < NumMessages; ++Index)
		{
			LogMessages->GetMessage(Index, Message);
			TSharedPtr<const FDataflowNode> Node = Message.Node.Pin();
			const FDataflowOutput* Output = Node ? Node->FindOutput(Message.OutputGuid) : nullptr;
			LogMessage(Message.Severity, Node.Get(), Output, Message.Text);
		}
	}
}

void FDataflowEditorToolkit::OnContextLog(const UE::Dataflow::FContext::FLogMessage& InLogMessage)
{
	LogMessages->Add(InLogMessage);
}
...
void FDataflowEditorToolkit::Tick(float DeltaTime)
{
	if (const TObjectPtr<UDataflowBaseContent> EditorContent = GetEditorContent())
	{
		if (EditorContent->GetDataflowAsset())
		{
			...
			if (UDataflowEditorUserSettings* Settings = GetMutableDefault<UDataflowEditorUserSettings>())
			{
				if (TSharedPtr<UE::Dataflow::FEngineContext> Context = EditorContent->GetDataflowContext())
				{
					Context->EnablePerfData(Settings->bShowPerformanceData);
				}
			}
			...
			// Evaluate the nodes that are requested from NodesToEvaluateOnTick
			const TSet<FGuid, DefaultKeyFuncs<FGuid>, TInlineSetAllocator<4>> NodesToEvaluateThisTick(NodesToEvaluateOnTick); // EvaluateNode can re-enter and Add to the set
			NodesToEvaluateOnTick.Reset();
			for (const FGuid& NodeToEvaluateGuid : NodesToEvaluateThisTick)
			{
				if (UDataflow* DataflowAsset = EditorContent->GetDataflowAsset())
				{
					if (TSharedPtr<UE::Dataflow::FGraph> DataflowGraph = DataflowAsset->GetDataflow())
					{
						if (TSharedPtr<const FDataflowNode> Node = DataflowGraph->FindBaseNode(NodeToEvaluateGuid))
						{
							UE::Dataflow::FTimestamp DummyNodeTimeStamp = UE::Dataflow::FTimestamp::Invalid;
							EvaluateNode(Node.Get(), nullptr, DummyNodeTimeStamp);
							bViewsNeedRefresh = true;
						}
					}
				}
...
void FDataflowEditorToolkit::EvaluateTerminalNode(const FDataflowTerminalNode& TerminalNode)
{
	...
	UE_LOGF(LogChaosDataflow, VeryVerbose, "FDataflowEditorToolkit::EvaluateTerminalNode(): Node [%ls]", *TerminalNode.GetName().ToString());
...
{
	UE_LOGF(LogChaosDataflow, VeryVerbose, "FDataflowEditorToolkit::EvaluateNode(): Node [%ls], Output [%ls]", Node ? *Node->GetName().ToString() : TEXT("nullptr"), Output ? *Output->GetName().ToString() : TEXT("nullptr"));

	if (!Node)
	{
		UE_LOGF(LogChaosDataflow, VeryVerbose, "FDataflowEditorToolkit::EvaluateNode(): Node is null, skipping evaluation");
		return;
	}
	...
	if (!Node->IsActive())
	{
		UE_LOGF(LogChaosDataflow, VeryVerbose, "FDataflowEditorToolkit::EvaluateNode(): Node [%ls] is inactive skipping evaluation", *Node->GetName().ToString());
		return;
	}
	...
	if (bDataflowEnableGraphEval)
	{
		...
		if (bCanEvaluate)
		{
			if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
			{
				const FDateTime StartEvaluationTime = FDateTime::Now();

				// when evaluation completes, refresh the views
				TWeakObjectPtr<UDataflowBaseContent> WeakEditorContent = EditorContent;
				TWeakPtr<FDataflowEditorToolkit> WeakToolkit = SharedThis(this);
				TWeakPtr<const FDataflowNode> WeakNode = Node->AsWeak();
				auto OnEvaluationCompleted = [WeakEditorContent, WeakToolkit, WeakNode, StartEvaluationTime](UE::Dataflow::FContext& Context)
					{
						check(IsInGameThread());

						const FDateTime EndEvaluationTime = FDateTime::Now();
						// refresh views 
						if (TSharedPtr<FDataflowEditorToolkit> ToolKit = WeakToolkit.Pin())
						{
							ToolKit->bViewsNeedRefresh = true;
							ToolKit->GraphEditor->NotifyGraphChanged();
						}
						if (TSharedPtr<const FDataflowNode> Node = WeakNode.Pin())
						{
							const FText ElapsedTimeText = FText::FromString(UE::Dataflow::Private::GetElapsedTimeString(EndEvaluationTime, StartEvaluationTime));
							const FText NodeNameText = FText::FromName(Node->GetName());
							const FText MessageText = FText::Format(LOCTEXT("EvalElapsedLog", "Graph evaluation from node [{0}] time : {1}"), NodeNameText, ElapsedTimeText);
							Context.Info(MessageText, Node.Get());

							if (TStrongObjectPtr<UDataflowBaseContent> EditorContent = WeakEditorContent.Pin())
							{
								if (Node->GetName().ToString() == EditorContent->GetDataflowTerminal())
								{
									EditorContent->SetLastModifiedTimestamp(Node->GetTimestamp());
								}
								EditorContent->SetConstructionDirty(true);
							}
						}
					};
				bool bUseAsyncEvaluation = false;
				if (UDataflowEditorUserSettings* Settings = GetMutableDefault<UDataflowEditorUserSettings>())
				{
					bUseAsyncEvaluation = Settings->bUseAsyncEvaluation;
				}
				if (bUseAsyncEvaluation)
				{
					// TODO : maybe this should be in the commands EvaluateNode as there's logic with the timestamp
					EditorContent->EvaluateGraph(OnEvaluationCompleted);
				}
				else
				{
...
TSharedRef<SDataflowGraphEditor> FDataflowEditorToolkit::CreateGraphEditorWidget(UEdGraph* GraphToEdit, TSharedPtr<IStructureDetailsView> InNodeDetailsEditor)
{
	...
	const SDataflowGraphEditor::FGraphEvaluationCallback Evaluate =
		[this](const FDataflowNode* Node, const FDataflowOutput* Output)
		{
			...
			if (!bIsTerminalNode || EvaluationMode == EDataflowEditorEvaluationMode::Manual)
			{
				if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
				{
					FTimestamp NodeTimeStamp = FTimestamp::Invalid;
					EvaluateNode(Node, Output, NodeTimeStamp);
				}
			}
		};
	...
	TSharedRef<SDataflowGraphEditor> NewGraphEditor = SNew(SDataflowGraphEditor, DataflowAsset)
		...
		.IsEditable([this]() { return IsEditable(); })
	...
	NewGraphEditor->SetLogMessageStorage(LogMessages);
...
TSharedPtr<IStructureDetailsView> FDataflowEditorToolkit::CreateNodeDetailsEditorWidget(UObject* ObjectToEdit)
{
	...
	LocalDetailsView->GetDetailsView()->SetIsPropertyEditingEnabledDelegate(FIsPropertyEditingEnabled::CreateRaw(this, &FDataflowEditorToolkit::IsEditable));
...
void FDataflowEditorToolkit::OnTabClosed(TSharedRef<SDockTab> Tab)
{
	...
	else if (Tab->GetTabLabel().EqualTo(FText::FromString("Scene Outliner")))
	{
		ViewListeners.Remove(DataflowOutlinerView.Get());
	}
}
...
FText FDataflowEditorToolkit::GetToolkitName() const
{
	if (const TObjectPtr<UDataflowBaseContent>& EditorContent = GetEditorContent())
	{
		...
		{
			// If we have an asset userdata type it is potentially a wrapper around a dataflow asset.
			// In this case, check for an underlying asset and prefer to use the asset name rather
			// than the name of the wrapper for the tab label
			const UAssetUserData* AsUserData = Cast<UAssetUserData>(EditorContent->GetDataflowOwner());

			if(!AsUserData || !EditorContent->GetDataflowAsset())
			{
...
void FDataflowEditorToolkit::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	Collector.AddReferencedObject(PropertyGizmoManager);
...
void FDataflowEditorToolkit::DrawPropertyGizmoLabels(FCanvas* Canvas, const FSceneView* SceneView) const
{
	if (PropertyGizmoManager)
	{
		PropertyGizmoManager->DrawPropertyLabels(Canvas, SceneView);
	}
}

void FDataflowEditorToolkit::OnConstructionViewModeChanged(const UE::Dataflow::IDataflowConstructionViewMode* NewConstructionViewMode)
{
	UpdateGizmos();
}

void FDataflowEditorToolkit::UpdateGizmos()
{
	// Update property gizmos whenever the view is updated
	if (PropertyGizmoManager)
	{
		bool bCanDisplayGizmo = false;
		if (UDataflowEditorMode* const DataflowMode = Cast<UDataflowEditorMode>(EditorModeManager->GetActiveScriptableMode(UDataflowEditorMode::EM_DataflowEditorModeId)))
		{
			if (const UE::Dataflow::IDataflowConstructionViewMode* const ConstructionViewMode = DataflowMode->GetConstructionViewMode())
			{
				bCanDisplayGizmo = ConstructionViewMode->CanDisplayGizmo();
			}

			UInteractiveToolManager* ToolManager = DataflowMode->GetInteractiveToolsContext()->ToolManager;
			PropertyGizmoManager->Teardown(ToolManager);
			if (bCanDisplayGizmo)
			{
				if (SelectedDataflowNodes.Num() == 1)
				{
					PropertyGizmoManager->Setup(ToolManager, *SelectedDataflowNodes.begin());
				}
			}
		}
	}
