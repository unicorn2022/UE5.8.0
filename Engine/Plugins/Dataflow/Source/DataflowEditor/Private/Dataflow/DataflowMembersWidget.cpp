...
bool SDataflowMembersWidget::IsDataflowAssetEditable() const
{
	if (TSharedPtr<SDataflowGraphEditor> GraphEditor = GetGraphEditor())
	{
...
void SDataflowMembersWidget::InitializeCommands()
{
	...
	CommandList->MapAction(FDataflowEditorCommands::Get().ConvertToBasicSubGraph,
		...
		FCanExecuteAction::CreateSP(this, &SDataflowMembersWidget::IsDataflowAssetEditable));
...
IDataflowInstanceInterface* SDataflowMembersWidget::GetDataflowInstanceInterface() const
{
	...
	{
		return UE::Dataflow::InstanceUtils::GetDataflowInterfaceFromObject(EditedAssetWeakPtr.Get());
	}
	return nullptr;
}
...
TSharedRef<SWidget> SDataflowMembersWidget::OnCreateWidgetForAction(FCreateWidgetForActionData* const InCreateData)
{
	if (InCreateData && InCreateData->Action)
	{
		...
		{
			TSharedRef<SWidget> Widget = Section->CreateWidgetForAction(InCreateData, GetGraphEditor());
			TAttribute<bool> SetEnabledAttribute(this, &SDataflowMembersWidget::IsDataflowAssetEditable);
			Widget->SetEnabled(SetEnabledAttribute);
			return Widget;
		}
	}
...
{
	return IsDataflowAssetEditable();
}
...
{
	if (IsDataflowAssetEditable())
	{
		if (TSharedPtr<FGraphActionNode> ActionNode = InSelectedNode.Pin())
		{
			if (TSharedPtr<FEdGraphSchemaAction> Action = ActionNode->Action)
			{
				if (const TSharedPtr<ISection> Section = GetSectionById(Action->GetSectionID()))
				{
