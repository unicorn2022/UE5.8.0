		...
		if (UE::Dataflow::FNodeFactory* Factory = UE::Dataflow::FNodeFactory::GetInstance())
		{
			...
			if (Param.IsValid())
			{
				...
				if ((bIsSimulationGraph && bIsSimulationNode) || (!bIsSimulationGraph && !bIsSimulationNode))
				{
					...
					FSlateIcon Icon;
					if (Param.DefaultNodeObject)
					{
						Icon = UDataflowEdNode::GetIconFromNode(*Param.DefaultNodeObject);
					}

					TSharedPtr<FAssetSchemaAction_Dataflow_CreateNode_DataflowEdNode> NewNodeAction(
						new FAssetSchemaAction_Dataflow_CreateNode_DataflowEdNode(NodeName, InNodeTypeName, Category, MenuDesc, ToolTip, Tags, Icon));
...
FAssetSchemaAction_Dataflow_CreateNode_DataflowEdNode::FAssetSchemaAction_Dataflow_CreateNode_DataflowEdNode(const FName& InName, const FName& InType, FText InNodeCategory, FText InMenuDesc, FText InToolTip, FText InKeywords, FName InIconName)
	: FEdGraphSchemaAction(InNodeCategory, InMenuDesc, InToolTip, 0, InKeywords)
	, NodeName(InName)
	, NodeTypeName(InType)
{
	Icon = FSlateIcon(FAppStyle::GetAppStyleSetName(), InIconName);
}

FAssetSchemaAction_Dataflow_CreateNode_DataflowEdNode::FAssetSchemaAction_Dataflow_CreateNode_DataflowEdNode(const FName& InName, const FName& InType, FText InNodeCategory, FText InMenuDesc, FText InToolTip, FText InKeywords, const FSlateIcon& InIcon)
	: FEdGraphSchemaAction(InNodeCategory, InMenuDesc, InToolTip, 0, InKeywords)
	, NodeName(InName)
	, NodeTypeName(InType)
	, Icon(InIcon)
...
FSlateBrush const* FAssetSchemaAction_Dataflow_CreateNode_DataflowEdNode::GetPaletteIcon() const
{
	return Icon.GetIcon();
}
...
bool FEdGraphSchemaAction_DataflowVariable::CanRenameItem(FText NewName) const
{
	...
	{
		if (VariableName == NewName.ToString())
		{
			return true; // same value allowed
		}
		const FName NewVariableName(NewName.ToString());
		const FName SanitizedName = FInstancedPropertyBag::SanitizePropertyName(NewVariableName);
		if (SanitizedName != NewVariableName)
		{
			return false;
		}
		const bool bVariableAlreadyExists = (nullptr != DataflowAsset->Variables.FindPropertyDescByName(NewVariableName));
