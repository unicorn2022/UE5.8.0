...
namespace UE::Dataflow
{
	...
	bool IsViewModeSupported(const UDataflowEdNode& EdNode, const UDataflowBaseContent& EditorContent, const IDataflowConstructionViewMode& ViewMode, bool bCheckDebugDraw)
	{
		if (const TSharedPtr<FEngineContext> Context = EditorContent.GetDataflowContext())
		{
			...
			{
				// first check the rendering collection rendering system (legacy)
				if (const FRenderingFactory* const Factory = FRenderingFactory::GetInstance())
				{
					...
					{
						if (Factory->CanRenderNodeOutput(FGraphRenderingState{ EdNode.GetDataflowNodeGuid(), DataflowNode.Get(), Parameter, *Context, ViewMode }))
						{
							return true;
						}
					}
				...
				// check if the node needs debug draw
				if (bCheckDebugDraw && DataflowNode->CanDebugDrawViewMode(ViewMode.GetName()))
				{
					return true;
				}

				// check the component based rendering system (new)
				TArray<FRenderableTypeInstance> RenderableInstances;
				FRenderableTypeInstance::GetRenderableInstancesForNode(*Context, ViewMode, *DataflowNode, /*bEvaluateOutputs*/false, RenderableInstances);
				if (RenderableInstances.Num() > 0)
				{
					return true;
				}
			}
	...
	bool CanRenderNodeOutput(const UDataflowEdNode& EdNode, const UDataflowBaseContent& EditorContent, const IDataflowConstructionViewMode& ViewMode, bool bCheckDebugDraw)
	{
		if (const TSharedPtr<FEngineContext> Context = EditorContent.GetDataflowContext())
		{
			if (TSharedPtr<const FDataflowNode> NodeTarget = EdNode.GetDataflowGraph()->FindBaseNode(FName(EdNode.GetName())))
			{
				...
				if (bCheckDebugDraw && NodeTarget->CanDebugDrawViewMode(ViewMode.GetName()))
				{
					return true;
				}
			}
	...
	void GetViewModesForNode(const UDataflowEdNode& EdNode, const UDataflowBaseContent& EditorContent, TArray<FName>& OutValidViewModeNames)
	{
		...
		for (const FName& ViewModeName : AllViewModeNames)
		{
			...
			{
				if (IsViewModeSupported(EdNode, EditorContent, *ViewMode))
				{
