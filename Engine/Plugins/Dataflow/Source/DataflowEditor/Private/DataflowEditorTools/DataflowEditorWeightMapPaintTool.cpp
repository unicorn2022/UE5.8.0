...
void UDataflowEditorWeightMapPaintToolBuilder::SetEditorMode(UDataflowEditorMode* InMode)
{
			...
			if (bHaveDynamicMeshToWeightConversion)
			{
				...
				for (int32 DynamicMeshVert = 0; DynamicMeshVert < Mesh->VertexCount(); ++DynamicMeshVert)
				{
					...
					else
					{
						...
						UE_LOGF(LogTemp, Warning, "Weight map misalignment.");
						break;
					}
				}
			}
...
void UDataflowEditorWeightMapPaintTool::ComputeGradient()
{
	...
	{
		UE_LOGF(LogTemp, Warning, "No active weight map");
		return;
	}
