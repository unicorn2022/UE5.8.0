...
{
	/**
	 * Concrete ILegacyClothingConverterProvider implementation that lets ChaosClothAssetTools
	 * (which sits below us in the dependency graph) drive the new Dataflow-graph-based converter
	 * via IModularFeatures. Registered in StartupModule, unregistered in ShutdownModule.
	 */
	class FLegacyClothingConverterProvider final : public ILegacyClothingConverterProvider
	{
	public:
		virtual bool ConvertInto(const UClothingAssetCommon* Source, UChaosClothAsset* Target) const override
		{
			const FLegacyClothingConverterResult Result = FLegacyClothingConverter::ConvertInto(Source, Target);
			return Result.CreatedAsset != nullptr;
		}
	};
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS  // UE_DEPRECATED(5.8, "The Cloth Panel Editor is now deprecated, use the Dataflow Editor instead.")
		if (UAssetDefinition_ClothAsset::AllowClothPanelEditor())
		{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	void FChaosClothAssetEditorModule::StartupModule()
	{
		...
		DataflowMenusHandle = UE::DataflowAssetDefinitionHelpers::RegisterDataflowAssetMenus(UChaosClothAsset::StaticClass());

		// Register the legacy -> ChaosClothAsset converter as a modular feature so the right-click
		// "Export to Chaos Cloth Asset" exporter in ChaosClothAssetTools (which sits below this
		// module in the dependency graph) can call into the new Dataflow-graph-based converter.
		LegacyClothingConverterProvider = MakeUnique<FLegacyClothingConverterProvider>();
		IModularFeatures::Get().RegisterModularFeature(
			ILegacyClothingConverterProvider::FeatureName,
			LegacyClothingConverterProvider.Get());
	}

	void FChaosClothAssetEditorModule::ShutdownModule()
	{
		// Unregister the legacy conversion provider so the exporter falls back to its in-tree
		// flat-mesh path if it is invoked after we unload.
		if (LegacyClothingConverterProvider)
		{
			IModularFeatures::Get().UnregisterModularFeature(
				ILegacyClothingConverterProvider::FeatureName,
				LegacyClothingConverterProvider.Get());
			LegacyClothingConverterProvider.Reset();
		}
