...
void UPaperTileMap::Serialize(FArchive& Ar)
{
	...
	{
		UE_LOGF(LogPaper2D, Warning, "PaperTileMap '%ls' was using 2D physics which has been removed, it has been switched to 3D physics.", *GetPathName());
		SpriteCollisionDomain = ESpriteCollisionMode::Use3DPhysics;
	}
}
...
void UPaperTileMap::ValidateSelectedLayerIndex()
{
	if (!TileLayers.IsValidIndex(SelectedLayerIndex))
	{
		...
		{
			if (TileLayers[LayerIndex] && TileLayers[LayerIndex]->ShouldRenderInEditor())
			{
