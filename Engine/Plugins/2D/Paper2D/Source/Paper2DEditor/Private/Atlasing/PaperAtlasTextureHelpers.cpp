...
bool FPaperAtlasTextureHelpers::ReadSpriteTexture(UTexture* SourceTexture, const FIntPoint& SourceXY, const FIntPoint& SourceSize, TArray<uint8>& TargetBuffer)
{
	...
	{
		UE_LOGF(LogPaper2DEditor, Error, "Couldn't retrive the source texture of the Sprite texture %ls while building the atlas for the sprite.", *SourceTexture->GetName());
		return false;
	}
