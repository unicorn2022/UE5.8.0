...
void FPaperAtlasGenerator::HandleAssetChangedEvent(UPaperSpriteAtlas* Atlas)
{
	...
	for (UPaperSprite* Sprite : SpritesInNewAtlas)
	{
		...
		{
			UE_LOGF(LogPaper2DEditor, Error, "Sprite %ls has no source texture and cannot be packed", *(Sprite->GetPathName()));
			continue;
		}
		...
		if ((PaddedSpriteSize.X > Atlas->MaxWidth) || (PaddedSpriteSize.Y > Atlas->MaxHeight))
		{
			...
			UE_LOGF(LogPaper2DEditor, Error, "Sprite %ls (%d x %d) can never fit into atlas %ls (%d x %d) due to maximum page size restrictions",
				*(Sprite->GetPathName()),
				SpriteSize.X,
				SpriteSize.Y,
				*(Atlas->GetPathName()),
	...
	for (int32 AtlasIndex = 0; AtlasIndex < RemappedAtlasTextures.Num(); ++AtlasIndex)
	{
		...
		AtlasTexture->PreEditChange(nullptr);
