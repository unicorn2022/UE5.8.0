...
void FPaperGroupedSpriteUtilities::SplitSprites(const TArray<UObject*>& InObjectList)
{
	...
	if (ComponentsToHarvest.Num() > 0)
	{
		...
		else
		{
			...
			UE_LOGF(LogPaper2DEditor, Warning, "Splitting sprites in the Blueprint editor is not currently supported");
		}
	}
	...
	if (ComponentsToHarvest.Num() > 0)
	{
		if (UWorld* World = ComponentsToHarvest[0]->GetWorld())
		{
			...
			else
			{
				...
				UE_LOGF(LogPaper2DEditor, Warning, "Merging sprites in the Blueprint editor is not currently supported");
			}
		}
	}
