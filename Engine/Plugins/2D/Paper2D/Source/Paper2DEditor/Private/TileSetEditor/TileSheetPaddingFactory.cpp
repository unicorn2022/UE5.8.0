...
UObject* UTileSheetPaddingFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	{
		UE_LOGF(LogPaper2DEditor, Error, "Tile sheet texture '%ls' is not BGRA8, cannot create a padded texture from it", *SourceTexture->GetName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogPaper2DEditor, Error, "Tile sheet texture '%ls' is too small to contain any tiles, cannot create a padded texture from it", *SourceTexture->GetName());
		return nullptr;
	}
