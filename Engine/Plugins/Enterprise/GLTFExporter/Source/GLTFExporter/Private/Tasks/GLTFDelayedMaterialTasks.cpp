...
void FGLTFDelayedMaterialTask::Process()
{
	...
	if (!BaseMaterial)
	{
		Builder.LogWarning(FString::Printf(
			TEXT("Material %s does not have a base material. Aborting."),
			*Material->GetName()));
		return;
	}
	...
	{
		Builder.LogWarning(FString::Printf(
			TEXT("Non-surface materials not supported, Material %s will be treated as surface."),
			*Material->GetName()));
	}

	// Abort process if the material has no valid resource because no proxy can be created without it
	// See FExportMaterialProxy constructor which requires a valid resource to proceed
	if (!Material->GetMaterialResource(GMaxRHIShaderPlatform))
	{
		Builder.LogWarning(FString::Printf(
			TEXT("Cannot bake material %s because it has no valid resource."),
			*Material->GetName()));
		return;
	}
