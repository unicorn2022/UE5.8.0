...
FGLTFJsonMesh* FGLTFStaticMeshConverter::Convert(const UStaticMesh* StaticMesh,  const UStaticMeshComponent* StaticMeshComponent, FGLTFMaterialArray Materials, int32 LODIndex)
{
	...
	if (StaticMesh)
	{
		...
		{
			MaterialCount = UE::MeshParser::Private::GetNumberOfValidPolygons(StaticMesh, LODIndex);
		}
		else
#endif
		{
			MaterialCount = FGLTFMeshUtilities::GetMaterials(StaticMesh).Num();
		}
	}
...
{
#if !WITH_EDITOR
	if (SkeletalMesh && !SkeletalMesh->GetLODInfo(LODIndex)->bAllowCPUAccess)
	{
		Builder.LogSuggestion(FString::Printf(
	...
	if (SkeletalMesh)
	{
		...
		if (Builder.ExportOptions->bExportSourceModel)
		{
			MaterialCount = UE::MeshParser::Private::GetNumberOfValidPolygons(SkeletalMesh, LODIndex);
		}
		else
#endif
		{
			MaterialCount = FGLTFMeshUtilities::GetMaterials(SkeletalMesh).Num();
		}
	}
