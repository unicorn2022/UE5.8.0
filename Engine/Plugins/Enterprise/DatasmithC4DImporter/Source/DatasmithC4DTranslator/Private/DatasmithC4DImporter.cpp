...
bool FDatasmithC4DImporter::AddChildActor(cineware::BaseObject* Object, TSharedPtr<IDatasmithActorElement> ParentActor, cineware::Matrix WorldTransformMatrix, const TSharedPtr<IDatasmithActorElement>& Actor)
{
	...
	if (!bExtracted)
	{
		UE_LOGF(LogDatasmithC4DImport, Warning, "Actor %ls (%ls) has some zero scaling", Actor->GetName(), Actor->GetLabel());
...
TSharedPtr<IDatasmithLightActorElement> FDatasmithC4DImporter::ImportLight(cineware::BaseObject* InC4DLightPtr, const FString& DatasmithName, const FString& DatasmithLabel)
{
	...
	{
		UE_LOGF(LogDatasmithC4DImport, Warning, "Failed to create DatasmithLightActorElement for light '%ls'", *MelangeObjectName(InC4DLightPtr));
		return nullptr;
	}
	...
	if (bUseIES)
	{
		...
		{
			bUseIES = false;
			UE_LOGF(LogDatasmithC4DImport, Warning, "Could not find IES file '%ls' used by light '%ls'", *IESFilename, *MelangeObjectName(InC4DLightPtr));
		}
		else
		{
			// Create IES texture
			...
			for (int32 LayerIndex = ReflectionLayerCount - 1; LayerIndex >= 0; --LayerIndex)
			{
				...
				if (ReflectionLayerFlags & REFLECTION_FLAG_ACTIVE)
				{
					...
					case REFLECTION_FRESNEL_CONDUCTOR:
						Opacity *= 0.4f;  // The reflectance layer looks like a transparent coat or overlay
						break;
					default:
						break;
					}
...
void FDatasmithC4DImporter::ImportAnimations(TSharedPtr<IDatasmithActorElement> ActorElement)
{
	...
	for(cineware::BaseTag* Tag = Object->GetFirstTag(); Tag; Tag=Tag->GetNext())
	{
		...
		if (TagType == Tcrane && ObjectType == Ocamera)
		{
			...
			if (!FoundAttributes)
			{
				UE_LOGF(LogDatasmithC4DImport, Warning, "Trying to parse animations for crane camera '%ls', but it doesn't have crane camera attributes!", *MelangeObjectName(Object));
				continue;
			}
		...
		else if (TagType == Taligntospline)
		{
			...
			if (!FoundSpline)
			{
				UE_LOGF(LogDatasmithC4DImport, Error, "Did not find target spline object '%ls' for %ls's AlignToSpline animation!", *MelangeObjectName(SplineObj), *MelangeObjectName(Object));
				continue;
			}
...
TSharedPtr<IDatasmithActorElement> FDatasmithC4DImporter::ImportObjectAndChildren(cineware::BaseObject* ActorObject, cineware::BaseObject* DataObject, TSharedPtr<IDatasmithActorElement> ParentActor, const cineware::Matrix& WorldTransformMatrix, const FString& InstancePath, const FString& DatasmithLabel, const TArray<cineware::TextureTag*>& TextureTags)
{
	...
	if (!DatasmithName)
	{
		UE_LOGF(LogDatasmithC4DImport, Error, "Could not get the ID of object \"%ls\"", *MelangeObjectName(ActorObject));
	...
	if (bActorVisible)
	{
		...
		case Oarray:
			...
			if (ObjectType == Ocloner && MelangeGetInt32(ActorObject, cineware::MGCLONER_VOLUMEINSTANCES_MODE) != 0)
			{
				...
				UE_LOGF(LogDatasmithC4DImport, Warning, "Render-instance or multi-instance Cloners are not supported. Actor '%ls' will be ignored", *MelangeObjectName(ActorObject));
			}
			else if (ObjectType == Oarray && MelangeGetInt32(ActorObject, cineware::ARRAYOBJECT_RENDERINSTANCES) != 0)
			{
				...
				UE_LOGF(LogDatasmithC4DImport, Warning, "Render-instance Arrays are not supported. Actor '%ls' will be ignored", *MelangeObjectName(ActorObject));
			}
			else
			{
		...
		if (!bSuccess)
		{
			UE_LOGF(LogDatasmithC4DImport, Error, "Could not import the object \"%ls\"", *MelangeObjectName(ActorObject));
		}
	...
	if (ParentActor)
	{
		if (!AddChildActor(ActorObject, ParentActor, NewWorldTransformMatrix, ActorElement))
		{
			...
			UE_LOGF(LogDatasmithC4DImport, Error, "Could not create the actor for the object \"%ls\"", *MelangeObjectName(ActorObject));
		}
	}
...
TSharedPtr<IDatasmithMeshElement> FDatasmithC4DImporter::ImportMesh(cineware::PolygonObject* PolyObject, const FString& DatasmithMeshName, const FString& DatasmithLabel)
{
	...
	{
		UE_LOGF(LogDatasmithC4DImport, Error, "Mesh '%ls' has %d UV channels! Only the first %d will be used", *DatasmithLabel, MAX_STATIC_TEXCOORDS-1, UVWTagsData.Num());
...
bool FDatasmithC4DImporter::OpenFile(const FString& InFilename)
{
	...
	if (C4Dfile->Open(DOC_IDENT, TCHAR_TO_ANSI(*InFilename), FILEOPEN_READ))
	{
		...
		if (bSuccess)
		{
			UE_LOGF(LogDatasmithC4DImport, Log, "Melange SDK successfully read the file '%ls' (read %lld out of %lld bytes, version %lld)", *InFilename, LastPos, Length, Version);
		}
		else
		{
			UE_LOGF(LogDatasmithC4DImport, Warning, "Melange SDK did not read the entire file '%ls' (read %lld out of %lld bytes, version %lld, error code: %d). Imported scene may contain errors or missing data.", *InFilename, LastPos, Length, Version, Error);
		}
	}
	else
	{
		UE_LOGF(LogDatasmithC4DImport, Error, "Cannot open file '%ls'", *InFilename);
		DeleteObj(C4Dfile);
		DeleteObj(C4dDocument);
		return false;
	}
	...
	if (C4dDocument != nullptr && !C4dDocument->HasCaches())
	{
		...
		UE_LOGF(LogDatasmithC4DImport, Warning, "%ls", *ErrorMsg.ToString());
...
bool FDatasmithC4DImporter::ProcessScene()
{
	...
	if(MelangeFPS == 0.0f)
	{
		UE_LOGF(LogDatasmithC4DImport, Error, "DOCUMENT_FPS not found");
		return false;
	}
	...
	if (!RenderData)
	{
		UE_LOGF(LogDatasmithC4DImport, Error, "Active Render Data not found");
		return false;
	}
