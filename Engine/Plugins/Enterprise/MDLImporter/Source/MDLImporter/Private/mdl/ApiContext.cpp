...
namespace Mdl
{
		...
		virtual void message(mi::base::Message_severity Level, const char* ModuleCategory, const mi::base::Message_details&, const char* Message) override
		{
			switch (Level)
			{
				case mi::base::MESSAGE_SEVERITY_FATAL:
					...
					UE_LOGF(LogMDLImporter, Fatal, "topic: %ls, %ls", UTF8_TO_TCHAR(ModuleCategory), UTF8_TO_TCHAR(Message));
					break;
				case mi::base::MESSAGE_SEVERITY_ERROR:
					Messages.Emplace(MDLImporterLogging::EMessageSeverity::Error, FString(UTF8_TO_TCHAR(Message)));
					UE_LOGF(LogMDLImporter, Error, "topic: %ls, %ls", UTF8_TO_TCHAR(ModuleCategory), UTF8_TO_TCHAR(Message));
					break;
				case mi::base::MESSAGE_SEVERITY_WARNING:
					Messages.Emplace(MDLImporterLogging::EMessageSeverity::Warning, FString(UTF8_TO_TCHAR(Message)));
					UE_LOGF(LogMDLImporter, Warning, "topic: %ls, %ls", UTF8_TO_TCHAR(ModuleCategory), UTF8_TO_TCHAR(Message));
					break;
				case mi::base::MESSAGE_SEVERITY_INFO:
					UE_LOGF(LogMDLImporter, Log, "topic: %ls, %ls", UTF8_TO_TCHAR(ModuleCategory), UTF8_TO_TCHAR(Message));
					break;
				case mi::base::MESSAGE_SEVERITY_VERBOSE:
				case mi::base::MESSAGE_SEVERITY_DEBUG:
					UE_LOGF(LogMDLImporter, Verbose, "topic: %ls, %ls", UTF8_TO_TCHAR(ModuleCategory), UTF8_TO_TCHAR(Message));
					break;
				default:
					break;
			}
	...
	bool FApiContext::Load(const FString& LibrariesPath, const FString& ModulesPath)
	{
		...
		if (!DsoHandle)
		{
			UE_LOGF(LogMDLImporter, Error, "Error: Library %ls does not exist in the path %ls", TEXT("libmdl_sdk" MI_BASE_DLL_FILE_EXT), *LibrariesPath);
			return false;
		}
		...
		if (!SymbolPtr)
		{
			UE_LOGF(LogMDLImporter, Error, "Error: Symbol %ls does not exist in the library %ls", TEXT("mi_factory"), TEXT("libmdl_sdk" MI_BASE_DLL_FILE_EXT));
			return false;
		}
		...
		if (!Neuray)
		{
			...
			if (!Version)
			{
				UE_LOGF(LogMDLImporter, Error, "Error: Incompatible library.");
			}
			else
			{
				UE_LOGF(LogMDLImporter, Error, "Error: Library version %ls does not match header version %ls.",
		...
		if ( Result != 0)
		{
			UE_LOGF(LogMDLImporter, Error, "mi::neuraylib::IPlugin_configuration::load_plugin_library() failed with return code %d.", Result);
			return false;
		}
		...
		if (Result)
		{
			UE_LOGF(LogMDLImporter, Error, "mi::neuraylib::INeuray::start() failed with return code %d.", Result);
			return false;
		}
	...
	bool FApiContext::LoadModule(const FString& InModuleName, FMaterialCollection& OutMaterials)
	{
		...
		if (Result < 0)
		{
			UE_LOGF(LogMDLImporter, Error, "Failed to load MDL module (%u): %ls", Result, *InModuleName);
		...
		const FString ElementName = TEXT("mdl") + InModuleName;
		{
			...
			{
				UE_LOGF(LogMDLImporter, Error, "Invalid Module interface: %ls", *ElementName);
				MDL_CHECK_RESULT() = Transaction->commit();
				return false;
			}
	...
	void FApiContext::LogInfo()
	{
		...
		UE_LOGF(LogMDLImporter, Log, "MDL SDK header version          = %ls", TEXT(MI_NEURAYLIB_PRODUCT_VERSION_STRING));
		UE_LOGF(LogMDLImporter, Log, "MDL SDK library product name    = %ls", UTF8_TO_TCHAR(Version->get_product_name()));
		UE_LOGF(LogMDLImporter, Log, "MDL SDK library product version = %ls", UTF8_TO_TCHAR(Version->get_product_version()));
		UE_LOGF(LogMDLImporter, Log, "MDL SDK library build number    = %ls", UTF8_TO_TCHAR(Version->get_build_number()));
		UE_LOGF(LogMDLImporter, Log, "MDL SDK library build date      = %ls", UTF8_TO_TCHAR(Version->get_build_date()));
		UE_LOGF(LogMDLImporter, Log, "MDL SDK library build platform  = %ls", UTF8_TO_TCHAR(Version->get_build_platform()));
		UE_LOGF(LogMDLImporter, Log, "MDL SDK library version string  = \"%ls\"", UTF8_TO_TCHAR(Version->get_string()));

		mi::base::Uuid NeurayIdLibArray  = Version->get_neuray_iid();
		mi::base::Uuid NeurayIdInterface = mi::neuraylib::INeuray::IID();

		UE_LOGF(LogMDLImporter, Log, "MDL SDK header interface ID           = <%2x, %2x, %2x, %2x>", NeurayIdInterface.m_id1,
		       NeurayIdInterface.m_id2, NeurayIdInterface.m_id3, NeurayIdInterface.m_id4);
		UE_LOGF(LogMDLImporter, Log, "MDL SDK library interface ID          = <%2x, %2x, %2x, %2x>\n", NeurayIdLibArray.m_id1,
