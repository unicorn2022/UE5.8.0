...
namespace Mdl
{
	namespace
	{
		                ...
		                mi::neuraylib::ITransaction* Transaction)
		{
			...
			UE_LOGF(LogMDLImporter, Log, "Compiled:\n%ls", *Printer.Print(*CompiledMaterial, Transaction));
#endif
#if MDL_DEBUG_PRINT_MATERIAL & 2
			UE_LOGF(LogMDLImporter, Log, "Distilled:\n%ls", *Printer.Print(*DistilledMaterial, Transaction));
#endif
#endif
		}
	                                           ...
	                                           FBakeParam&                  MapBakeParam) const
	{
		...
		{
			UE_LOGF(LogMDLImporter, Log, "Bake value was null for: %ls", Mdl::ToString(MapType));
			return;
		}
		...
		switch (MapBakeParam.ValueType)
		{
			...
			default:
			{
				UE_LOGF(LogMDLImporter, Log, "Ignoring unsupported value type : %d", (int)MapBakeParam.ValueType);
				return;
			}
		}
		...
		if (MapBakeParam.SetBakedValue(*ValueData.get()))
		{
			...
			switch (MapBakeParam.ValueType)
			{
				case Mdl::EValueType::Float:
				{
					...
					UE_LOGF(LogMDLImporter, Log, "Baked value: %f for: %ls from: %ls", Value, Mdl::ToString(MapType), *MapBakeParam.InputBakePath);
					break;
				}
				case Mdl::EValueType::Float3:
				{
					...
					UE_LOGF(LogMDLImporter, Log, "Baked float3: %f %f %f for: %ls from: %ls", Value.x, Value.y, Value.z, Mdl::ToString(MapType),
				...
				case Mdl::EValueType::ColorRGB:
				{
					...
					UE_LOGF(LogMDLImporter, Log, "Baked color: %f %f %f %f for: %ls from: %ls", Value.r, Value.g, Value.b, Value.a,
	                                     ...
	                                     FBakeParam& MapBakeParam) const
	{
		...
		{
			UE_LOGF(LogMDLImporter, Log, "Bake texture path is null : %d", MapTypeInt);
			return;
		}
		...
		{
			UE_LOGF(LogMDLImporter, Log, "Bake texture was empty: %d", MapTypeInt);
			return;
		}
		...
		{
			UE_LOGF(LogMDLImporter, Log, "Baked texture %ls for: %ls from: %ls with gamma: %f", *FPaths::GetBaseFilename(TextureName),
			       Mdl::ToString(MapType), *MapBakeParam.InputBakePath, Canvas->get_gamma());
		}
		else
		{
			UE_LOGF(LogMDLImporter, Warning, "Failed to bake texture %ls for: %ls from: %ls", *FPaths::GetBaseFilename(TextureName),
			       Mdl::ToString(MapType), *MapBakeParam.InputBakePath);
		}
	}
