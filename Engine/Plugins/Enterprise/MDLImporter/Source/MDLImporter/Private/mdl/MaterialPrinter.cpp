	                                                  ...
	                                                  mi::neuraylib::ITransaction*             Transaction)
	{
		switch (Element.Expression->get_kind())
		{
			...
			case mi::neuraylib::IExpression::EK_DIRECT_CALL:
			{
				...
				UE_LOGF(LogMDLImporter, Error, "[MaterialPrinter] ran into unhandled Semantic: '%ls' Semantic: '%d'", *FunctionName, Semantic);
				return;
			}
			...
			case mi::neuraylib::IValue::VK_TEXTURE:
			{
				...
				if (MdlPath)
				{
					...
					else
					{
						...
						UE_LOGF(LogMDLImporter, Error, "[MaterialPrinter] unresolved texture: '%s'", MdlPath);
					}
				}
				TraverseResult << TEXT(")");
				return;
	...
	void FMaterialPrinter::HandleVisitEndExpression(const FTraversalElement& Element, mi::neuraylib::ITransaction* Transaction)
	{
		switch (Element.Expression->get_kind())
		{
			case mi::neuraylib::IExpression::EK_DIRECT_CALL:
			{
				...
				UE_LOGF(LogMDLImporter, Error, "[MaterialPrinter] ran into unhandled Semantic: '%ls' Semantic: '%d'", *Name, Semantic);
				break;
			}
			default:
				break;
