...
namespace Mdl
{
	namespace
	{
		const char* GetPixelType(int Channels)
		{
			...
			UE_LOGF(LogMDLImporter, Error, "Unsupported channel count %d", Channels);
			return nullptr;
		}
	}
	namespace Util
	{
		...
		bool CreateMaterialInstance(mi::neuraylib::ITransaction* Transaction, const FString& MaterialDbName, const FString& MaterialInstanceDbName)
		{
			...
			if (MaterialCheck)
			{
				UE_LOGF(LogMDLImporter, Log, "Material instance was already created %ls", *MaterialInstanceDbName);
				// already created
				return false;
			}
		                             ...
		                             float                        MetersPerSceneUnit)
		{
			...
			if (ErrorCount != 0)
			{
				UE_LOGF(LogMDLImporter, Error, "There were errors creating compiled material '%ls':", *MaterialInstanceDbName);
				for (uint64 Index = 0; Index < ErrorCount; Index++)
				{
					const mi::neuraylib::IMessage* Message = Context->get_error_message(Index);
					if (Message)
					{
						UE_LOGF(LogMDLImporter, Error, "Error %llu: '%ls'", Index, ANSI_TO_TCHAR(Message->get_string()));
					}
				}
				return false;
			}
