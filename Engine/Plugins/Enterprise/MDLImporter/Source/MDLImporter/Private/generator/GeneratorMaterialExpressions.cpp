...
namespace Generator
{
	                                                                           ...
	                                                                           const TArray<FMaterialExpressionConnection>& Inputs)
	{
		...
		{
			UE_LOGF(LogMDLImporter, Warning, "Function <%ls> received too many inputs - expected %d, but got %d!\n", *(Function->GetName()), Expression->FunctionInputs.Num(), Inputs.Num());
		}
		else
		{
	...
	T* NewMaterialExpressionTextureType(UObject* Parent, UTexture* Texture)
	{
		...
		if (Texture)
		{
			...
			Expression->SamplerType = MaterialExpressionUtils::GetSamplerTypeForTexture(Texture);
		}
