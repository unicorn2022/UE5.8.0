...
void FDataprepEditor::TakeSnapshot()
{
	...
	UE_LOGF( LogDataprepEditor, Verbose, "Taking snapshot..." );
	...
	TAtomic<bool> bGlobalIsValid(true);
	{
					...
					[this, AssetObjectPtr, &bGlobalIsValid]()
					{
						if(UObject* AssetObject = AssetObjectPtr.Get())
						{
							if ( bGlobalIsValid.Load(EMemoryOrder::Relaxed) )
							{
								...
								UE_LOGF( LogDataprepEditor, Verbose, "Saving asset %ls", *AssetPathString );
		...
		for (int32 Index = 0; Index < AsyncTasks.Num(); ++Index)
		{
			if(UObject* AssetObject = Assets[Index].Get())
			{
				...
				{
					UE_LOGF( LogDataprepEditor, Log, "Failed to save %ls", *AssetPathString );
				...
				{
					UE_LOGF( LogDataprepEditor, Verbose, "Asset %ls successfully saved", *AssetPathString );
				}
			}
		}
	}
	...
	if(ContentSnapshot.bIsValid)
	{
		...
		UE_LOGF( LogDataprepEditor, Verbose, "Saving preview world" );
		...
		if(ContentSnapshot.bIsValid)
		{
			UE_LOGF( LogDataprepEditor, Verbose, "Level successfully saved" );
		}
		else
		{
			UE_LOGF( LogDataprepEditor, Warning, "Failed to save level" );
		}
	}
	...
	UE_LOGF( LogDataprepEditor, Verbose, "Snapshot taken in [%d min %.3f s]", ElapsedMin, ElapsedSeconds );
}
...
void FDataprepEditor::RestoreFromSnapshot(bool bUpdateViewport)
{
	...
	if ( !ContentSnapshot.bIsValid )
	{
		UE_LOGF( LogDataprepEditor, Log, "Snapshot is invalid. Running the producers..." );
		OnBuildWorld();
		return;
	}
	...
	uint64 StartTime = FPlatformTime::Cycles64();
	UE_LOGF( LogDataprepEditor, Verbose, "Restoring snapshot..." );
	...
	SlowTask.EnterProgressFrame( 40.0f, LOCTEXT("RestoreFromSnapshot_Assets", "Restoring assets ...") );
	{
		...
		for (FSnapshotDataEntry& DataEntry : ContentSnapshot.DataEntries)
		{
			...
			UE_LOGF( LogDataprepEditor, Verbose, "Loading asset %ls", *ObjectPath.GetAssetPathString() );
...

			{
				...
				{
					UE_LOGF( LogDataprepEditor, Error, "Failed to restore asset %ls", *ObjectPath.GetAssetPathString() );
				}
			}

			Assets.Add( Asset );

			UE_LOGF( LogDataprepEditor, Verbose, "Asset %ls loaded", *ObjectPath.GetAssetPathString() );
		}
	...
	UE_LOGF( LogDataprepEditor, Verbose, "Level loaded" );
	...
	int ElapsedMin = int(ElapsedSeconds / 60.0);
	ElapsedSeconds -= 60.0 * (double)ElapsedMin;
	UE_LOGF( LogDataprepEditor, Verbose, "Preview world restored in [%d min %.3f s]", ElapsedMin, ElapsedSeconds );
