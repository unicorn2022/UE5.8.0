...
namespace ViewportDebug
{
	...
	public:
			...
			, Text( InText )
		{
			if( bLogTiming )
			{
				UE_LOGF( LogDataprepEditor, Log, "%ls ...", *Text );
			}
		}
		...
		~FTimeLogger()
		{
			if( bLogTiming )
			{
				...
				UE_LOGF( LogDataprepEditor, Log, "%ls took [%d min %.3f s]", *Text, ElapsedMin, ElapsedSeconds );
			}
		}
...
void SDataprepEditorViewport::Construct(const FArguments& InArgs, TSharedPtr<FDataprepEditor> InDataprepEditor)
{
	...
	if (ensure(PreviewSceneWorld) && GWorld != nullptr && GWorld->Scene != nullptr)
	{
		PreviewSceneWorld->ShaderPlatformChanged(GWorld->Scene->GetShaderPlatform());
	}

	// Listen to and act on changes in preview platform
	UEditorEngine* Editor = CastChecked<UEditorEngine>(GEngine);
	PreviewFeatureLevelChangedHandle = Editor->OnPreviewShaderPlatformChanged().AddLambda([&PreviewSceneWorld](EShaderPlatform NewShaderPlatform)
	{
		PreviewSceneWorld->ShaderPlatformChanged(NewShaderPlatform);
	});
...
FPrimitiveSceneProxy* UCustomStaticMeshComponent::CreateSceneProxy()
{
	...
	{
		UE_LOGF(LogStaticMesh, Warning, "Unable to create a proxy for StaticMeshComponent [%ls] because it doesn't have a fallback mesh.", *GetFullName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogStaticMesh, Verbose, "Skipping CreateSceneProxy for StaticMeshComponent %ls (LOD problems)", *GetFullName());
		return nullptr;
	}
