...
{
	UE_LOGF( LogDataprepCore, Log, "%ls : %ls", *InObject.GetName(), *InLogText.ToString() );
}

void FDataprepCoreUtils::FDataprepLogger::LogWarning(const FText& InLogText, const UObject& InObject)
{
	UE_LOGF( LogDataprepCore, Warning, "%ls : %ls", *InObject.GetName(), *InLogText.ToString() );
}

void FDataprepCoreUtils::FDataprepLogger::LogError(const FText& InLogText,  const UObject& InObject)
{
	UE_LOGF( LogDataprepCore, Error, "%ls : %ls", *InObject.GetName(), *InLogText.ToString() );
}
...
{
	UE_LOGF( LogDataprepCore, Log, "Start: %ls ...", *InTitle.ToString() );
	++TaskDepth;
}
...
void FDataprepCoreUtils::FDataprepProgressTextReporter::ReportProgress( float Progress, const FText& InMessage )
{
	if( TaskDepth > 0 )
	{
		UE_LOGF( LogDataprepCore, Log, "Doing %ls ...", *InMessage.ToString() );
	}
}
