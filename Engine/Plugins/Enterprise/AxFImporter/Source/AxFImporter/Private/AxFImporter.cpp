...
// 
		...
		void FitBRDF()
		{
			...
			const float IterationRangeReduction = 0.2f; // After finding best fit search restarts around found value in a smaller area
			const int32 IterationCount = 5;
			const float RayAngleExtent = HALF_PI * 0.8f; // don't take into account angles too close to grazing - spikes are too big there 
