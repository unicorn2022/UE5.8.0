...
bool FOpenNurbsTranslatorImpl::TranslateBRep(ON_Brep* Brep, const ON_3dmObjectAttributes& Attributes, FMeshDescription& OutMesh, TSharedRef< IDatasmithMeshElement >& MeshElement, const FString& Name, bool& bHasNormal)
{
	...
	if (OpenNurbsOptions.Geometry == EDatasmithOpenNurbsBrepTessellatedSource::UseUnrealNurbsTessellation)
	{
		...
		if (!bRet)
		{
			...
			UE_LOGF(LogDatasmithOpenNurbsTranslator, Warning, "Failed to generate the mesh of \"%ls\" (%ls) StaticMesh.", *StaticMeshLable, *Name);
		}
		return bRet;
	}
	else
...
bool FDatasmithOpenNurbsTranslator::LoadScene(TSharedRef<IDatasmithScene> OutScene)
{
...

	{
		...
		UE_LOGF(LogDatasmithOpenNurbsTranslator, Display, "CAD translation [%ls].", *Filename);
		UE_LOGF(LogDatasmithOpenNurbsTranslator, Display, " - Parsing Library:      %ls", TEXT("OpenNurbs"));
		UE_LOGF(LogDatasmithOpenNurbsTranslator, Display, " - Tessellation Library: %ls", *TesselationLibrary);
	}
