...
void UDataprepTessellationOperation::OnExecution_Implementation(const FDataprepContext& InContext)
{
	...
	if(	!IsCancelled() && SelectedMeshes.Num() > 0)
	{
		...
		UE_LOGF( LogCADLibrary, Log, "Tessellation of %d out of %d static mesh(es) in [%d min %.3f s]", ModifiedStaticMeshes.Num(), SelectedMeshes.Num(), ElapsedMin, ElapsedSeconds );
