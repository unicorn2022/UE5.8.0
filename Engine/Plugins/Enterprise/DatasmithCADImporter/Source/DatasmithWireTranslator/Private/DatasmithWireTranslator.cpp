...
uint64 IWireInterface::AliasVersion = 0;

static TAutoConsoleVariable<FString> CVarAliasDllDirectory(
	TEXT("WireTranslator.Binary.Directory"),
	TEXT(""),
	TEXT("Controls which Alias API dll to load.\n"),
	ECVF_Default);

uint64 IWireInterface::GetRequiredAliasVersion(uint64 AliasMaxVersion)
{
#if PLATFORM_WINDOWS
	static bool bInitialized = [&AliasMaxVersion]() -> bool
		{
			const TCHAR* AliasDllName = TEXT("libalias_api.dll");

			const FString AliasDllDirectory = CVarAliasDllDirectory->GetString();
			if (!AliasDllDirectory.IsEmpty())
			{
				FString AliasDllPath = AliasDllDirectory / AliasDllName;
				FPaths::MakePlatformFilename(AliasDllPath);
				if (FPaths::FileExists(AliasDllPath))
				{
					uint64 AliasDllVersion = FPlatformMisc::GetFileVersion(*AliasDllPath);
					if (AliasDllVersion < AliasMaxVersion && AliasDllVersion > 0)
					{
						if (void* DllHandle = ::LoadLibraryExW(*AliasDllPath, nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32))
						{
							AliasVersion = AliasDllVersion;
							return true;
						}
					}
				}
			...
			FString BestAliasDllPath;
			uint64 AliasBestVersion = 0;
			...
			{
				const FString AliasDllPath = Path / AliasDllName;
				if (FPaths::FileExists(AliasDllPath))
				{
					uint64 AliasDllVersion = FPlatformMisc::GetFileVersion(*AliasDllPath);
					if (AliasDllVersion < AliasMaxVersion && AliasDllVersion > AliasBestVersion)
					{
						AliasBestVersion = AliasDllVersion;
						BestAliasDllPath = Path;
					}
				}
			}

			if (!BestAliasDllPath.IsEmpty())
			{
				FString AliasDllPath = BestAliasDllPath / AliasDllName;
				FPaths::MakePlatformFilename(AliasDllPath);
				if(void* DllHandle = ::LoadLibraryExW(*AliasDllPath, nullptr, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_SYSTEM32))
				{
					AliasVersion = AliasBestVersion;
				}
			}
...
bool FDatasmithWireTranslator::CanTranslate()
{
	static const bool bCanTranslate = []() -> bool
		{
			if (IWireInterface::AliasVersion == 0)
			{
				return false;
			}
...
void FDatasmithWireTranslator::Initialize(FDatasmithTranslatorCapabilities& OutCapabilities)
{
	...
	const int32 MajorVersion = (IWireInterface::AliasVersion & 0xffff0000) >> 16;
	const int32 MinorVersion = IWireInterface::AliasVersion & 0x0000ffff;
