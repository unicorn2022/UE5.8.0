...
namespace UE_DATASMITHWIRETRANSLATOR_NAMESPACE
{
	...
	const uint64 LibAlias2027_0_0_Version = 9288674231451972;
	...
	const uint64 LibAliasVersionMax = LibAlias2027_0_0_Version;
	...
	bool FWireTranslatorImpl::Initialize(const TCHAR* InSceneFullName)
	{
		if (InSceneFullName)
		{
			...
			{
				UE_LOGF(LogWireInterface, Error, "Cannot initialize OpenModel SDK. Import is aborted.");
				return false;
			}
			...
			{
				UE_LOGF(LogWireInterface, Error, "Cannot load %ls with the selected OpenModel SDK.", InSceneFullName);
				return false;
			}
		}
	...
	{
		UE_LOGF(LogWireInterface, Display, "CAD translation [%ls].", *SceneFullPath);
		UE_LOGF(LogWireInterface, Display, " - File version:         Alias %ls", *SceneVersion);
		UE_LOGF(LogWireInterface, Display, " - Parsing Library:      Alias %ls", *AliasSdkVersion);
		UE_LOGF(LogWireInterface, Display, " - Tessellation Library: %ls", WireSettings.bUseCADKernel ? TEXT("CADKernel") : TEXT("TechSoft"));

		UE_LOGF(LogWireInterface, Display, " - Translation parameters:");
		UE_LOGF(LogWireInterface, Display, "     - Merge By Group:      %ls", WireSettings.bMergeGeometryByGroup ? TEXT("True") : TEXT("False"));
		UE_LOGF(LogWireInterface, Display, "     - Layer As actor:      %ls", WireSettings.bUseLayerAsActor ? TEXT("True") : TEXT("False"));

		UE_LOGF(LogWireInterface, Display, " - Tessellation parameters:");
		UE_LOGF(LogWireInterface, Display, "     - ChordTolerance:      %lf", WireSettings.ChordTolerance);
		UE_LOGF(LogWireInterface, Display, "     - MaxEdgeLength:       %lf", WireSettings.MaxEdgeLength);
		UE_LOGF(LogWireInterface, Display, "     - MaxNormalAngle:      %lf", WireSettings.NormalTolerance);
		...
		UE_LOGF(LogWireInterface, Display, "     - StitchingTechnique:  %ls", *StitchingTechnique);
		UE_LOGF(LogWireInterface, Display, "     - GeometricTolerance:  %lf", WireSettings.GetGeometricTolerance());
		UE_LOGF(LogWireInterface, Display, "     - Stitching Tolerance: %lf", WireSettings.GetStitchingTolerance());

		// #wire_import: TODO - Revisit stitching extension when using CADKernel
		//if (WireSettings.bUseCADKernel)
		//{
		//	UE_LOGF(LogWireInterface, Display, "     - Stitching Options:");
		//	UE_LOGF(LogWireInterface, Display, "         - ForceSew:              %ls", CADLibrary::FImportParameters::bGStitchingForceSew ? TEXT("True") : TEXT("False"));
		//	UE_LOGF(LogWireInterface, Display, "         - RemoveThinFaces:       %ls", CADLibrary::FImportParameters::bGStitchingRemoveThinFaces ? TEXT("True") : TEXT("False"));
		//	UE_LOGF(LogWireInterface, Display, "         - RemoveDuplicatedFaces: %ls", CADLibrary::FImportParameters::bGStitchingRemoveDuplicatedFaces ? TEXT("True") : TEXT("False"));
		//	UE_LOGF(LogWireInterface, Display, "         - ForceFactor:           %f", CADLibrary::FImportParameters::GStitchingForceFactor);
		//}
	...
	TOptional<FMeshDescription> FWireTranslatorImpl::GetMeshDescriptionFromBodyNode(TSharedPtr<FBodyNode>& BodyNode, TSharedPtr<IDatasmithMeshElement> MeshElement, CADLibrary::FMeshParameters& OutMeshParameters)
	{
		...
		UE_LOGF(LogWireInterface, Warning, "Failed to generate the mesh of \"%ls\" (%ls) StaticMesh.", StaticMeshLabel, StaticMeshName);
	...
	public:
		...
		{
			// AliasStudio 2027 and beyond is not supported
			uint64 AliasVersion = IWireInterface::GetRequiredAliasVersion(LibAlias2027_0_0_Version);
...
#pragma optimize("", off)
	void MakeMeshVisible(FMeshDescription& MeshDescription)
	{
		...
		UE_LOGF(LogWireInterface, Warning, "Scaling factor: %.3f %.3f %.3f", ScaleX, ScaleY, ScaleZ);
