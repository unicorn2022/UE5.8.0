...
bool FNetworkNode::SendMessage(const TArray<uint8>& Message, double Timeout_s)
{
	if (bWriteError)
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "bWriteError flag raised, can't write");
		return false;
	}

	if (ConnectedSocket == nullptr)
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "node not connected, can't write");
		return false;
	}
	...
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "connection state error");
		return false;
	}
	...
	if (!bCanWrite)
	{
		UE_LOGF(LogDatasmithDispatcher, Warning, "can't write on socket");
		return false;
	}
	...
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "can't write header on socket");
		bWriteError = true;
		return false;
	}
	...
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "can't write content on socket");
		bWriteError = true;
		return false;
	}
...
bool FNetworkNode::ReceiveMessage(TArray<uint8>& OutMessage, double Timeout_s)
{
	if (bReadError)
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "ReadError flag raised, can't read");
		return false;
	}

	if (ConnectedSocket == nullptr)
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "node not connected, can't read");
		return false;
	}
	...
	if (IncommingMessage.Header.ByteSize < 0)
	{
		...
		{
			UE_LOGF(LogDatasmithDispatcher, Error, "Parsed header failed");
			bReadError = true;
			return false;
		}
		...
		if (IncommingMessage.Header.ByteSize < 0)
		{
			UE_LOGF(LogDatasmithDispatcher, Error, "Parsed header failed: bad message size %d", IncommingMessage.Header.ByteSize);
			bReadError = true;
			return false;
		}
	...
	while (IncommingMessage.Header.ByteSize >= 0 && ConnectedSocket->HasPendingData(PendingByteCount))
	{
		...
		{
			bReadError = true;
			UE_LOGF(LogDatasmithDispatcher, Error, "Recv issue");
			return false;
		}
...
FSocket* FNetworkNode::CreateInternalSocket(const FString& Description)
{
	...
	UE_CLOGF(!NewSocket, LogDatasmithDispatcher, Error, "Socket creation failure");
	return NewSocket;
}
...
FNetworkServerNode::FNetworkServerNode()
{
	...
	if (BoundPort == 0)
	{
		...
		UE_LOGF(LogDatasmithDispatcher, Error, "Socket binding failure");
		return;
	}
	...
	if (!ListeningSocket->Listen(0))
	{
		...
		UE_LOGF(LogDatasmithDispatcher, Error, "Socket listen failure");
		return;
	}
}
...
bool FNetworkClientNode::Connect(const FString& Description, int32 ServerPort, double Timeout_s)
{
	...
	do
	{
		...
		{
			UE_LOGF(LogDatasmithDispatcher, Verbose, "Client Node is connected");
			return true;
		}
	}
	while(FPlatformTime::Seconds() < ConnectTimeout_s);

	UE_LOGF(LogDatasmithDispatcher, Error, "Client socket failed to connect");
	CloseSocket(ConnectedSocket);
	return false;
}
