...
const EAppReturnType::Type OpenMessageDialog(uint64 AvailableRam, uint64 EstimationOfMemory, bool bWorkerWarning)
{
	...
	{
		UE_LOGF(LogDatasmithDispatcher, Error, "Estimated memory used by static meshes is greater than available RAM (%ls available vs %ls estimated). Import process is interrupted.", *RamSizeTxt.ToString(), *StaticMeshSizeTxt.ToString());
		UE_LOGF(LogDatasmithDispatcher, Error, "Import of the files below have been aborted:");
	}
	return Choice;
}
...
void FDatasmithDispatcher::LogMessages(const TArray<TPair<uint8, FString>>& Messages) const
{
	for (const TPair<uint8, FString>& Message : Messages)
	{
		...
		case ELogVerbosity::Fatal:
			UE_LOGF(LogDatasmithDispatcher, Fatal, "%ls", *Message.Value);
			break;
		case ELogVerbosity::Error:
			UE_LOGF(LogDatasmithDispatcher, Error, "%ls", *Message.Value);
			break;
		case ELogVerbosity::Warning:
			UE_LOGF(LogDatasmithDispatcher, Warning, "%ls", *Message.Value);
			break;
		case ELogVerbosity::Display:
			UE_LOGF(LogDatasmithDispatcher, Display, "%ls", *Message.Value);
			break;
		case ELogVerbosity::Log:
			UE_LOGF(LogDatasmithDispatcher, Log, "%ls", *Message.Value);
			break;
		case ELogVerbosity::Verbose:
			UE_LOGF(LogDatasmithDispatcher, Verbose, "%ls", *Message.Value);
			break;
		case ELogVerbosity::VeryVerbose:
		default:
			UE_LOGF(LogDatasmithDispatcher, VeryVerbose, "%ls", *Message.Value);
			break;
		}
	}
}
...
TOptional<FTask> FDatasmithDispatcher::GetNextTask()
{
	...
	UE_LOGF(LogDatasmithDispatcher, Display, "Launch %ls (%d / %d)", *TaskPool[NextTaskIndex].FileDescription.GetSourcePath(), NextTaskIndex, TaskPool.Num());
...
void FDatasmithDispatcher::SetTaskState(int32 TaskIndex, ETaskState TaskState)
{
	CADLibrary::FFileDescriptor FileDescriptor;
	{
		...
		case ETaskState::ProcessFailed:
		{
			...
			{
				UE_LOGF(LogDatasmithDispatcher, Warning, "   - Task failed with CADKernel: %ls", *Task.FileDescription.GetFileName());
				UE_LOGF(LogDatasmithDispatcher, Warning, "      => Add task to process with Techsoft");
	...
	UE_CLOGF(TaskState == ETaskState::ProcessOk, LogDatasmithDispatcher, Verbose, "File processed: %ls", *FileDescriptor.GetFileName());
	UE_CLOGF(TaskState == ETaskState::UnTreated, LogDatasmithDispatcher, Warning, "File resubmitted: %ls", *FileDescriptor.GetFileName());
	UE_CLOGF(TaskState == ETaskState::ProcessFailed, LogDatasmithDispatcher, Error, "File processing failure: %ls", *FileDescriptor.GetFileName());
	UE_CLOGF(TaskState == ETaskState::FileNotFound, LogDatasmithDispatcher, Warning, "file not found: %ls", *FileDescriptor.GetSourcePath());
}
...
void FDatasmithDispatcher::Process(bool bWithProcessor)
{
	...
	TFunction<bool()> CanImportContinue = [&]() -> bool
	{
		...
		if (AvailableMemory < EstimationOfMemoryUsedByStaticMeshes)
		{
			...
			if (Choice == EAppReturnType::Cancel)
			{
				...
				{
					UE_LOGF(LogDatasmithDispatcher, Error, "   - %ls", *Task->FileDescription.GetFileName());
				}
			...
			for (FDatasmithWorkerHandler& Handler : WorkerHandlers)
			{
				...
				if (Handler.IsRestartable())
				{
					...
					if (WorkerId < NumberOfWorkers + Config::MaxRestartAllowed)
					{
						...
						UE_LOGF(LogDatasmithDispatcher, Warning, "Restarting worker (new worker: %d)", WorkerId);
					}
					else if (bLogRestartError)
					{
						bLogRestartError = false;
						UE_LOGF(LogDatasmithDispatcher, Warning, "Worker not restarted (Limit reached)");
					}
				}
	...
	if (!IsOver())
	{
		...
		if (bWithProcessor)
		{
			UE_LOGF(LogDatasmithDispatcher, Warning,
				"Begin local processing. (Multi Process failed to consume all the tasks)\n" "See workers logs: %lsPrograms/DatasmithCADWorker/Saved/Logs", *FPaths::ConvertRelativePathToFull(FPaths::EngineDir()));
		}
	...
	{
		UE_LOGF(LogDatasmithDispatcher, Display, "Multi Process ended and consumed all the tasks");
	}
}
