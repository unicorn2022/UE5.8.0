...
namespace DatasmithSceneElementUtil
{
	...
	bool ImportDatasmithSceneFromCADFiles(const FString& DestinationFolder, const TArray<FString>& FilePaths, FDatasmithImportFactoryCreateFileResult& Result, TArray<FString>& FilesNotProcessed)
	{
		...
		if (!ExternalSource)
		{
			UE_LOGF(LogDatasmithImport, Warning, "Datasmith import error: cannot resolve the external source from Source file or URI. Abort import");
			return false;
		}
		...
		{
			UE_LOGF(LogDatasmithImport, Error, "Invalid Destination '%ls': %ls", *DestinationFolder, OutFailureReason);
			return false;
		}
...
UDatasmithSceneElement* UDatasmithSceneElement::ConstructDatasmithSceneFromSourceUri(const FString& SourceUriString)
{
	...
	if (!ExternalSource)
	{
		UE_LOGF(LogDatasmithImport, Error, "Datasmith import error: source uri could not be resolved to an external source. Abort import.");
		return nullptr;
	}
...
UDatasmithSceneElement* UDatasmithSceneElement::GetExistingDatasmithScene(const FString& AssetPath)
{
	...
	if (UDatasmithScene* SceneAsset = Cast<UDatasmithScene>(UEditorAssetLibrary::LoadAsset(AssetPath)))
	{
		...
		{
			UE_LOGF(LogDatasmithImport, Warning, "Datasmith ReimportScene error: no import data.");
			return nullptr;
		}
		...
		if (!ExternalSource)
		{
			UE_LOGF(LogDatasmithImport, Warning, "Datasmith import error: file name could not be resolved to an external source. Abort import.");
			return nullptr;
		}
...
bool UDatasmithSceneElement::TranslateScene()
{
	...
	{
		UE_LOGF(LogDatasmithImport, Error, "Invalid State. Ensure ConstructDatasmithSceneFromFile has been called.");
		return false;
	}
	...
	{
		UE_LOGF(LogDatasmithImport, Error, "Datasmith import error: Scene translation failure. Abort import.");
		return false;
	}
...
FDatasmithImportFactoryCreateFileResult UDatasmithSceneElement::ImportScene(const FString& DestinationFolder)
{
	...
	if (bMultifile)
	{
		...
		{
			UE_LOGF(LogDatasmithImport, Warning, "ImportScene - not all files were imported into the scene(try construct_datasmith_scene_from_file to import each file separately):");
			for (FString FileName : FilesNotProcessed)
			{
				UE_LOGF(LogDatasmithImport, Warning, "  '%ls' wasn't imported", *FileName);
			}
		}
...
FDatasmithImportFactoryCreateFileResult UDatasmithSceneElement::ReimportScene()
{
	...
	{
		UE_LOGF(LogDatasmithImport, Error, "Invalid State. Ensure GetExistingDatasmithScene has been called.");
		return Result;
	}
	...
	{
		UE_LOGF(LogDatasmithImport, Error, "Invalid Destination '%ls': %ls", *ImportPath, OutFailureReason);
		return Result;
	}
		...
		for (int32 LODIndex = 0; LODIndex < StaticMesh->GetNumSourceModels(); ++LODIndex)
		{
			...
			if (LODIndex == 0)
			{
				...
				if (bGenerateLightmapUVs)
				{
					...
					{
						UE_LOGF(LogSetupStaticLighting, Warning, "Could not complete the static lighting setup for static mesh %ls as the generated lightmap UV is set to be in channel #%i while the maximum lightmap channel is %i", *StaticMesh->GetName(), GeneratedLightmapChannel, Lightmass::MAX_TEXCOORDS);
						break;
					}
				}
