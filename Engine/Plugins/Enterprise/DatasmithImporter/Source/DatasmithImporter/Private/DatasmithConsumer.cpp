...
void UDatasmithConsumer::PostLoad()
{
	...
	if(HasAnyFlags(RF_WasLoaded))
	{
		...
		if(bMarkDirty)
		{
			UE_LOGF(LogDatasmithImport, Warning, "%ls is from an old version and has been updated. Please save asset to complete update.", *GetOuter()->GetName());
		}
	}
}
...
namespace DatasmithConsumerUtils
{
	...
	TArray<AActor*> MoveActorsToLevel(const TArray<AActor*>& ActorsToMove, ULevel* DestLevel, const TArray<UPackage*>& PackagesToCheck, bool bDuplicate)
	{
		...
		for (FSelectionIterator It(GEditor->GetSelectedActorIterator()); It; ++It)
		{
			...
			if (!bFoundMatch)
			{
				UE_LOGF(LogDatasmithImport, Error, "Cannot find remapping for moved actor ID %ls, any soft references pointing to it will be broken!", *Actor->GetPathName());
			}
		}
