...
int32 GetUsableMaxAssetPathLength()
{
	...
	static bool RunOnce = [&]()
	{
		...
		{
			UE_LOGF(LogDatasmithImport, Error,
				"Datasmith can encounter import issues due to a Content path too long, and an OS limitation on path length.\n" "Content path: '%ls'\n" "System max path length: %d.\n"
				, *FullPathProjectContentDir, FPlatformMisc::GetMaxPathLength());
		}

		if (GDatasmithMaxAssetPathLength > ProjectConstraint)
		{
			UE_LOGF(LogDatasmithImport, Warning,
				"The Datasmith.MaxAssetPathLength value (%d) is too high for the current setup.\n" "Content path: '%ls'\n" "System max path length: %d.\n"
...
void FDatasmithImporterUtils::DeleteActor( AActor& Actor )
{
	...
	if ( ActorWorld )
	{
		...
		// #ueent_remark: Underlying question why the actor and its components are still reachable after being 'deleted'
		{
			...
			GetObjectsWithOuter( &Actor, SubObjectsArray, EGetObjectsFlags::IncludeNestedObjects);
...
namespace FDatasmithImporterUtilsHelper
{
	...
	TSharedPtr< IDatasmithActorElement > ConvertLightActorToActorElement(const ALight* LightActor)
	{
		...
		{
			UE_LOGF(LogDatasmithImport, Warning, "Import of RectLight is not yet supported");
...
bool FDatasmithImporterUtils::CreatePlmXmlSceneFromCADFiles(FString PlmXmlFileName, const TSet<FString>& FilesToProcess, TArray<FString>& FilesNotProcessed)
{
	...
	{
		UE_LOGF(LogDatasmithImport, Error, "Datasmith import error: no translator found for CAD files. Abort import.");
		return false;
	}
	...
	for (const FString& FileName : FilesToProcess)
	{
		...
		else
		{
			...
			// UE_LOGF(LogDatasmithImport, Warning, "Datasmith import error: '%ls' is not a CAD file, skipping.", *FileName);
		}
	}
	...
	{
		UE_LOGF(LogDatasmithImport, Error, "Datasmith import error: Failed to create PlmXml file '%ls' for parallel loading ...", *PlmXmlFileName);
		return false;
	}
	return true;
}
