...
void FDatasmithImporter::ImportClothes(FDatasmithImportContext& ImportContext)  // UE_DEPRECATED(5.5, "The experimental Cloth importer is no longer supported.")
{
	...
	{
		UE_LOGF(LogDatasmithImport, Warning, "No cloth factory class provider found, cloth import skipped.");
		return;
	}
	...
	{
		UE_LOGF(LogDatasmithImport, Warning, "Invalid (null) cloth asset factory, cloth import skipped.");
		return;
	}
	if (ClothFactoryClassesProviders.Num() > 1)
	{
		UE_LOGF(LogDatasmithImport, Warning, "Multiple cloth asset providers found, only the cloth asset type from provider %ls will be imported.", *ClothFactoryClassesProviders[0]->GetName().ToString());
	}
	...
	for (int32 ClothIndex = 0; ClothIndex < ClothesCount && !ImportContext.bUserCancelled; ++ClothIndex)
	{
		...
		// #ue_ds_cloth_todo: async parallel support
		{
			...
			{
				UE_LOGF(LogDatasmithImport, Warning, "Cloth %ls: load failed, translator issue.", ClothElement->GetName());
				return;
			}

			if (Payload.Cloth.Patterns.IsEmpty())
			{
				UE_LOGF(LogDatasmithImport, Warning, "Cloth %ls: no patterns, import skipped.", ClothElement->GetName());
				return;
			}
		}
