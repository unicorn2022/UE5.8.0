...
bool FDatasmithImportContext::InitOptions(const TSharedPtr<FJsonObject>& ImportSettingsJson, const TOptional<FString>& InImportPath, bool bSilent)
{
	...
	{
		UE_LOGF(LogDatasmithImport, Warning, "Import failed. The AssetTools module can't be loaded.");
		return false;
	}
	...
	else
	{
		...
		{
			UE_LOGF(LogDatasmithImport, Display, "Can't call FDatasmithImportContext::InitOptions(), with bSilent=false if InImportPath is not set.");
			return false;
		}
		...
		if (!bShouldImport)
		{
			...
			UE_LOGF(LogDatasmithImport, Display, "Import canceled.");
			return false;
		}
...
bool FDatasmithImportContext::SetupDestination(const FString& InImportPath, EObjectFlags InFlags, FFeedbackContext* InWarn, bool bSilent)
{
	...
	if (!ActorsContext.ImportWorld)
	{
		...
		else if (Options->BaseOptions.SceneHandling == EDatasmithImportScene::CurrentLevel)
		{
			...
			{
				UE_LOGF(LogDatasmithImport, Warning, "Import failed. There is no World/Map open in the Editor.");
				return false;
			}
		}
		else
