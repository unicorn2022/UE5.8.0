...
UK2Node_FunctionEntry* FVariantManager::CreateDirectorFunction(ULevelVariantSets* InLevelVariantSets, FName InFunctionName, UClass* PinClassType, bool bCreateWithArguments)
{
	...
	constexpr bool bIsUserCreated = false;
	...
	if (ensure(EntryNodes.Num() == 1 && EntryNodes[0]))
	{
		...
		if (bCreateWithArguments)
		{
			FEdGraphPinType ArgsPinType;
			ArgsPinType.PinCategory = UEdGraphSchema_K2::PC_Name;
			ArgsPinType.ContainerType = EPinContainerType::Map;
			ArgsPinType.PinSubCategory = UEdGraphSchema_K2::PC_Name;
			FEdGraphPinType ValuePinType;
			ValuePinType.PinCategory = UEdGraphSchema_K2::PC_String;
			ArgsPinType.PinValueType = FEdGraphTerminalType::FromPinType(ValuePinType);
			EntryNode->CreateUserDefinedPin(ARGUMENTS_PIN_NAME, ArgsPinType, EGPD_Output, true);
		}
...
UK2Node_FunctionEntry* FVariantManager::CreateDirectorFunctionFromFunction(UFunction* QuickBindFunction, UClass* PinClassType, bool bCreateWithArguments)
{
	...
	UK2Node_FunctionEntry* NewFunctionEntry = CreateDirectorFunction(GetCurrentLevelVariantSets(), FName(*DesiredNewEventName), PinClassType, bCreateWithArguments);
	...
	for (UVariantObjectBinding* Binding : InTheseBindings)
	{
		...
		for (const TSharedPtr<FCapturableProperty>& PropAndDisplay : OfTheseProperties)
		{
			if (ExistingPropertyNames.Contains(PropAndDisplay->DisplayName))
			{
				if (!bSilenceWarnings)
				{
					UE_LOGF(LogVariantManager, Log, "Ignoring attempt to capture property '%ls' as it is already captured for actor binding '%ls'", *PropAndDisplay->DisplayName, *Binding->GetObject()->GetName());
				}
				continue;
			}
			...
			default: // Generic
			{
				...
				else
				{
					...
					UE_LOGF(LogVariantManager, Error, "Failed to capture property with path '%ls' for actor '%ls'", *PropAndDisplay->DisplayName, *ObjectName);
				}
				break;
			}
			}
			...
			{
				UE_LOGF(LogVariantManager, Log, "Actor '%ls' does not have a property with path '%ls', so it will not be captured for this actor", *ObjectName, *NewPropVal->GetFullDisplayString());
			}
		}
	...
	for (UVariant* Variant : InTheseVariants)
	{
		...
		for (UObject* SelectedActor : OfTheseObjects)
		{
			...
			else
			{
				...
				{
					UE_LOGF(LogVariantManager, Error, "When creating object bindings, variant '%ls' seems to have a binding, but we can't retrieve it!", *SelectedActor->GetName());
				}
			}
		}
