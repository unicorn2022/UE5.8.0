...
TSharedRef<SWidget> SVariantManager::GetMenuContextForLevelVariantSets()
{
	FMenuBuilder Builder(true, MakeShared<FUICommandList>());

	FAssetData AssetData;
	if (TSharedPtr<FVariantManager> VariantManager = VariantManagerPtr.Pin())
	{
		AssetData = FAssetData(VariantManager->GetCurrentLevelVariantSets());	
	}

	Builder.AddWidget(PropertyCustomizationHelpers::MakeAssetPickerWithMenu(
		AssetData,
		false,
		...
		FOnAssetSelected::CreateLambda([this](const FAssetData& AssetData)
			{
				...
				{
					AssetViewUtils::OpenEditorForAsset(AssetData.GetAsset());
				}
			}),
		FSimpleDelegate()),
		FText::GetEmpty());
...
TSharedRef<SWidget> SVariantManager::MakeLevelVariantSetsComboButton()
{
	return SNew(SHorizontalBox)
		...
		[
			SNew(STextBlock)
			.Text(NSLOCTEXT("SVariantManager", "Asset", "Asset:"))
		]
		...
		[
			...
			.OnAreAssetsAcceptableForDropWithReason_Static(&SVariantManager::OnAreAssetsAcceptableForVariantManager)
			.OnAssetsDropped(this, &SVariantManager::OnAssetDroppedForVariantManager)
			[
				SNew(SComboButton)
				.OnGetMenuContent(this, &SVariantManager::GetMenuContextForLevelVariantSets)
				.ButtonContent()
				[
					SNew(SBorder)
					...
					[
						...
						[
							SNew(SImage)
							.Image(FVariantManagerStyle::Get().GetBrush("VariantManager.Icon.Small"))
						...
						[
							...
							.Text_Lambda([this]() -> FText
							{
								...
								if (VarMan.IsValid() && VarMan->GetCurrentLevelVariantSets())
								{
									return FText::FromName(VarMan->GetCurrentLevelVariantSets()->GetFName());
								}

								return NSLOCTEXT("SVariantManager", "NoAssetSelected", "No asset selected");
							})
			                .OverflowPolicy(GetDefault<UEditorStyleSettings>()->bEnableMiddleEllipsis ? TOptional(ETextOverflowPolicy::MiddleEllipsis) : NullOpt)
						]
					]
				]
			]
		...
		[
			...
			[
				...
				.OnClicked_Static(&SVariantManager::OnUseSelectedAssetFromContentBrowserForCurrentLevelVariantSet)
				.ContentPadding(0.0f)
				.IsFocusable(true)
				[ 
					SNew( SImage )
		...
		[
			...
			[
				SNew(SButton)
				.ButtonStyle( FAppStyle::Get(), "SimpleButton" )
				.OnClicked( this, &SVariantManager::OnBrowseToCurrentLevelVariantSet)
				.ContentPadding(0.0f)
				.IsFocusable(true)
				[ 
					SNew( SImage )
...
TSharedRef<ITableRow> SVariantManager::MakeCapturedPropertyRowWithColumns(
	TSharedPtr<FVariantManagerPropertyNode> Item,
	const TSharedRef<STableViewBase>& OwnerTable)
{
	return SNew(SVariantManagerMultiColumnPropertyTableRow, OwnerTable, Item.ToSharedRef());
}

TSharedPtr<SWidget> SVariantManager::OnPropertyListContextMenuOpening() const
{
...
void SVariantManager::Construct(const FArguments& InArgs, TSharedRef<FVariantManager> InVariantManager)
{
	...
	const bool bUseNewAssetPicker = UE::VariantManager::Get_CVar_VariantManager_AssetPicker().GetValueOnGameThread();
	TSharedRef<SWidget> VariantSetsComboBox = bUseNewAssetPicker ? MakeLevelVariantSetsComboButton() : SNullWidget::NullWidget;
	TSharedRef<SWidget> OldSearchBox = bUseNewAssetPicker ? SNullWidget::NullWidget : SNew(SSearchBox)
		.HintText(LOCTEXT("VariantManagerFilterText", "Search Variant"))
		.OnTextChanged(this, &SVariantManager::OnOutlinerSearchChanged)
		.MinDesiredWidth(200);
	TSharedRef<SWidget> NewSearchBox = !bUseNewAssetPicker ? SNullWidget::NullWidget : SNew(SSearchBox)
		.HintText(LOCTEXT("VariantManagerFilterText", "Search Variant"))
		.OnTextChanged(this, &SVariantManager::OnOutlinerSearchChanged)
		.MinDesiredWidth(200);
...
void SVariantManager::CreateCommandBindings()
{
	...
	VariantTreeCommandBindings->MapAction(
		FVariantManagerEditorCommands::Get().SelectThumbnailAssetCommand,
		FExecuteAction::CreateSP(this, &SVariantManager::SelectThumbnailAsset),
...
void SVariantManager::SelectThumbnailAsset()
{
	TSharedPtr<FVariantManager> VariantManager = VariantManagerPtr.Pin();
	if (!VariantManager.IsValid())
	{
	...
	Config.DialogTitleOverride = NSLOCTEXT("SVariantManager", "SelectAnAssetForThumbnail", "Select an Asset for Thumbnail");
	TArray<FAssetData> AssetDatas = ContentBrowser.CreateModalOpenAssetDialog(Config);
	if (AssetDatas.Num() == 1)
	{
		if (UObject* ObjectThumbnail = AssetDatas[0].GetAsset())
		{
			FScopedTransaction Transaction(LOCTEXT("SelectThumbnailAssetTransaction", "Select asset as a thumbnail for a variant or variant set"));

			for (UVariant* Var : Vars)
			{
				Var->SetThumbnailFromObject(ObjectThumbnail);
			...
			for (UVariantSet* VarSet : VarSets)
			{
				VarSet->SetThumbnailFromObject(ObjectThumbnail);
			}
		}
	}
}
...
void SVariantManager::OnAssetDroppedForVariantManager(const FDragDropEvent& DragDropEvent, TArrayView<FAssetData> AssetDatas)
{
	if (AssetDatas.IsValidIndex(0))
	{
		const FAssetData& AssetData = AssetDatas[0];
		if (AssetData.AssetClassPath != FTopLevelAssetPath(ULevelVariantSets::StaticClass()))
		{
			return;
		}
		UObject* Asset = AssetData.GetAsset();
		ULevelVariantSets* LevelVariantSets = Cast<ULevelVariantSets>(Asset);
		if (!LevelVariantSets)
		{
			return;
		}
		AssetViewUtils::OpenEditorForAsset(LevelVariantSets);
	}
}

bool SVariantManager::OnAreAssetsAcceptableForVariantManager(TArrayView<FAssetData> Assets, FText& OutReason)
{
	if (Assets.Num() != 1)
	{
		OutReason = LOCTEXT("MultipleAssetsNotAllowed", "Please select only one asset.");
		return false;
	}
	...
	if (AssetData.AssetClassPath != FTopLevelAssetPath(ULevelVariantSets::StaticClass()))
	{
		OutReason = LOCTEXT("LevelVariantSetsNotAllowed", "Only Level Variant Sets assets can be opened by Variant Manager.");
		return false;
	}
...
FReply SVariantManager::OnUseSelectedAssetFromContentBrowserForCurrentLevelVariantSet()
{
	IContentBrowserSingleton& Singleton = FModuleManager::LoadModuleChecked<FContentBrowserModule>("ContentBrowser").Get();;
	TArray<FAssetData> ContentBrowserSelectedAssets;
	Singleton.GetSelectedAssets(ContentBrowserSelectedAssets);

	if (ContentBrowserSelectedAssets.Num() == 1)
	{
		FAssetData SelectedAsset = ContentBrowserSelectedAssets[0];

		if (SelectedAsset.IsInstanceOf(ULevelVariantSets::StaticClass()))
		{
			if (ULevelVariantSets* LevelVariantSets = Cast<ULevelVariantSets>(SelectedAsset.GetAsset()))
			{
				AssetViewUtils::OpenEditorForAsset(LevelVariantSets);
			}
		}
...
FReply SVariantManager::OnBrowseToCurrentLevelVariantSet() const
{
	if (TSharedPtr<FVariantManager> VariantManger = VariantManagerPtr.Pin())
	{
		if (ULevelVariantSets* LevelVariantSets = VariantManger->GetCurrentLevelVariantSets())
		{
			TArray<FAssetData> AssetDataArray;
			AssetDataArray.Add(FAssetData(LevelVariantSets));
...
void SVariantManager::OnObjectPropertyChanged(UObject* Object, struct FPropertyChangedEvent& Event)
{
	...
	if (PropertyPath.IsEmpty())
	{
		...
		{
			UE_LOGF(LogVariantManager, Warning, "Failed to auto-expose property '%ls' for object '%ls'", Event.MemberProperty? *Event.MemberProperty->GetDisplayNameText().ToString() : TEXT("nullptr"), Object ? *Object->GetName() : TEXT("nullptr"));
			return;
		}
...
TSharedRef<SWidget> SVariantManager::GenerateRightTreePropertiesRowContent()
{
	...
	const bool bShowColumns = UE::VariantManager::Get_CVar_VariantManager_AssetPicker().GetValueOnAnyThread();

	TSharedPtr<SWidget> PropertyListWidget;

	float ActorsSplitterPrecent = 0.5f;
	float PropertiesSplitterPercent = 0.5f;

	if (bShowColumns)
	{
		ActorsSplitterPrecent = 0.2f;
		PropertiesSplitterPercent = 0.8f;

		PropertyListWidget = SNew(SVerticalBox)
			...
			[
				SAssignNew(CapturedPropertyListView, SListView<TSharedPtr<FVariantManagerPropertyNode>>)
					...
					.OnGenerateRow_Static(&SVariantManager::MakeCapturedPropertyRowWithColumns)
					.Visibility(EVisibility::Visible)
					.HeaderRow(
						SNew(SHeaderRow)

						+ SHeaderRow::Column(SVariantManagerMultiColumnPropertyTableRow::ColumnPath)
						.DefaultLabel(LOCTEXT("PathColumnHeader", "Path"))
						.SortMode(this, &SVariantManager::GetSortMode, SVariantManagerMultiColumnPropertyTableRow::ColumnPath)
						.SortPriority(this,  &SVariantManager::GetSortPriority, SVariantManagerMultiColumnPropertyTableRow::ColumnPath)
						.OnSort(this, &SVariantManager::HandleSort)
						.FillWidth(0.2f)

						+ SHeaderRow::Column(SVariantManagerMultiColumnPropertyTableRow::ColumnProperty)
						.DefaultLabel(LOCTEXT("PropertyColumnHeader", "Property"))
						.SortMode(this, &SVariantManager::GetSortMode, SVariantManagerMultiColumnPropertyTableRow::ColumnProperty)
						.SortPriority(this,  &SVariantManager::GetSortPriority, SVariantManagerMultiColumnPropertyTableRow::ColumnProperty)
						.OnSort(this, &SVariantManager::HandleSort)
						.FillWidth(0.3f)

						+ SHeaderRow::Column(SVariantManagerMultiColumnPropertyTableRow::ColumnValue)
	...
	{
		PropertyListWidget = SAssignNew(CapturedPropertyListView, SListView<TSharedPtr<FVariantManagerPropertyNode>>)
			.SelectionMode(ESelectionMode::Single)
			.ListItemsSource(&DisplayedPropertyNodes)
			.OnContextMenuOpening(this, &SVariantManager::OnPropertyListContextMenuOpening)
			.OnGenerateRow_Static(&SVariantManager::MakeCapturedPropertyRow)
			.Visibility(EVisibility::Visible);
	}
	...
	[
		...
		// Properties column
		+ SSplitter::Slot()
		.Value(PropertiesSplitterPercent)
		[
			SNew(SVerticalBox)
			+ SVerticalBox::Slot()
			.AutoHeight()
			...
			.FillHeight(1.0f)
			[
				PropertyListWidget.ToSharedRef()
			]
		]
	];
}
...
EColumnSortMode::Type SVariantManager::GetSortMode(const FName ColumnName) const
{
	return ColumnName == SortColumn ? SortMode : EColumnSortMode::None;
}

EColumnSortPriority::Type SVariantManager::GetSortPriority(const FName ColumnName) const
{
	return ColumnName == SortColumn ? SortPriority : EColumnSortPriority::None;
}

void SVariantManager::HandleSort(
	...
	const EColumnSortMode::Type NewSortMode)
{
	...
	if (CapturedPropertyListView)
	{
		CapturedPropertyListView->RequestListRefresh();
	}
}

void SVariantManager::SortItems()
{
	auto SortFn = [this](const TSharedPtr<FVariantManagerPropertyNode>& A, const TSharedPtr<FVariantManagerPropertyNode>& B)
		{
			if (SortColumn == SVariantManagerMultiColumnPropertyTableRow::ColumnPath)
			{
				if (A->GetSubPath() != B->GetSubPath())
				{
					const bool bRet = A->GetSubPath() < B->GetSubPath();

					return SortMode == EColumnSortMode::Ascending ? bRet : !bRet;
				}
			}
			else if (SortColumn == SVariantManagerMultiColumnPropertyTableRow::ColumnProperty)
			{
				if (A->GetPropertyName() != B->GetPropertyName())
				{
					const bool bRet = A->GetPropertyName() < B->GetPropertyName();
	...
	DisplayedPropertyNodes.Sort(SortFn);
}
