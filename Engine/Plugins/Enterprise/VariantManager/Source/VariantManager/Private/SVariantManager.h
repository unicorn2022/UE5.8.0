...
public:
	...
	TSharedRef<SWidget> GetMenuContextForLevelVariantSets();
	TSharedRef<SWidget> MakeLevelVariantSetsComboButton();
	...
	static TSharedRef<ITableRow> MakeCapturedPropertyRow(TSharedPtr<FVariantManagerPropertyNode> Item, const TSharedRef<STableViewBase>& OwnerTable);
	static TSharedRef<ITableRow> MakeCapturedPropertyRowWithColumns(TSharedPtr<FVariantManagerPropertyNode> Item, const TSharedRef<STableViewBase>& OwnerTable);
	TSharedPtr<SWidget> OnPropertyListContextMenuOpening() const;
...
private:
	...
	void OnAssetDroppedForVariantManager(const FDragDropEvent& DragDropEvent, TArrayView<FAssetData> AssetDatas);
	static bool OnAreAssetsAcceptableForVariantManager(TArrayView<FAssetData> Assets, FText& OutReason);

	static FReply OnUseSelectedAssetFromContentBrowserForCurrentLevelVariantSet();
	FReply OnBrowseToCurrentLevelVariantSet() const;

	EColumnSortMode::Type GetSortMode(const FName ColumnName) const;
	EColumnSortPriority::Type GetSortPriority(const FName ColumnName) const;
...
private:
	...
	float PropertiesNameColumnWidth = 0;
	float PropertiesValueColumnWidth = 0;
