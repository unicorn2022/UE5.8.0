...
public:
	...
	/**
	 * Full outliner content for this property node. This includes name, splitter and value content.
	 */
	virtual TSharedRef<SWidget> GetCustomOutlinerContent(TSharedPtr<SVariantManagerTableRow> InTableRow) override;
	
	/**
	 * Just value content for this property node.
	 */
	virtual TSharedRef<SWidget> GetCustomValueContent();
	...
	/**
	 * Flag if this is an Actor or Actor Component property.
	 */
	bool IsActorProperty() const;

	/**
	 * Path without the property name
	 */
	FString GetSubPath() const;

	/**
	 * Name of the property (last part of the path)
	 */
