...
UENUM(BlueprintType)
enum class EVariantFunctionCallerSignature : uint8
{
	Static,
	NoParameters,
	OneParameter,
	OneParameterWithArgs,
	FourParameters,
	FourParametersWithArgs
};
...
class UVariantManagerBlueprintLibrary : public UBlueprintFunctionLibrary
{
	...
	// Gets or initializes and returns Variant Manager
	static FVariantManager& GetVariantManager();
...
public:
	...
	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category="VariantManager")
	static TArray<FVariantDependency> GetDependencies(UVariant* Variant);
	...
	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category = "VariantManager|FunctionCallers")
	static UBlueprint* GetOrCreateDirectorBlueprint(UVariant* Variant);

	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category = "VariantManager|FunctionCallers")
	static TArray<FName> GetFunctionCallerNames(UVariant* Variant, AActor* Actor);

	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category = "VariantManager|FunctionCallers")
	static bool GetFunctionCallerArguments(UVariant* Variant, AActor* Actor, FName FunctionName, TMap<FName, FString>& Arguments);

	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category = "VariantManager|FunctionCallers")
	static bool CreateFunctionCaller(UVariant* Variant, AActor* Actor, FName FunctionName, EVariantFunctionCallerSignature Signature);

	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category = "VariantManager|FunctionCallers")
	static bool AddFunctionCaller(UVariant* Variant, AActor* Actor, FName FunctionName);

	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category = "VariantManager|FunctionCallers")
	static bool UpdateFunctionCallerArguments(UVariant* Variant, AActor* Actor, FName FunctionName, const TMap<FName, FString>& Arguments);

	UFUNCTION(BlueprintCallable, meta=(ScriptMethod), Category = "VariantManager|FunctionCallers")
	static bool RemoveFunctionCaller(UVariant* Variant, AActor* Actor, FName FunctionName);

private:
	static bool CheckTheFunctionSignature(UVariant* Variant, AActor* BoundActor, FName FunctionName, EVariantFunctionCallerSignature Signature);
