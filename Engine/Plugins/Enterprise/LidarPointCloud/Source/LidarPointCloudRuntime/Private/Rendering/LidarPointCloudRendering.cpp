			...
			for (const FLidarPointCloudProxyUpdateDataNode& Node : RenderData.SelectedNodes)
			{
				if (Node.RayTracingGeometry.IsValid() && Node.RayTracingGeometry->IsInitialized())
				{
...

					{
						...
						UpdateParams.MeshBatchesView = NodeRayTracingMaterials;
						UpdateParams.NumVertices = Geometry->GetNumVertices();
						UpdateParams.VertexBufferSize = Geometry->GetBufferSize();
						UpdateParams.NumTriangles = Geometry->GetNumPrimitives();
