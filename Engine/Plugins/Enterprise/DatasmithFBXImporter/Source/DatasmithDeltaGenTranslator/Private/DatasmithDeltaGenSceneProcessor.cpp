...
namespace DeltaGenProcessorImpl
{
	...
	TSharedPtr<FDatasmithFBXSceneMaterial> FetchAOTexture(const FString& MeshName, TSharedPtr<FDatasmithFBXSceneMaterial>& Material, const TArray<FDirectoryPath>& TextureFolders)
	{
		...
		{
			UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Found more than one candidate for an AO texture matching mesh name '%ls', so texture '%ls' will be used. Better matching between texture and mesh name would help.", *MeshName, *AOTexPath);
		}
