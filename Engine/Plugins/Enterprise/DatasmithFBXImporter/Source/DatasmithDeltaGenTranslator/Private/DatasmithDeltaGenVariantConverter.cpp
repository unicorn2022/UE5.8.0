...
TSharedPtr<IDatasmithLevelVariantSetsElement> FDeltaGenVariantConverter::ConvertVariants(TArray<FDeltaGenVarDataVariantSwitch>& Vars, TArray<FDeltaGenPosDataState>& PosStates, FActorMap& ActorsByOriginalName, FMaterialMap& MaterialsByName)
{
	...
	for (const TPair<FString, FDeltaGenVarDataVariantSwitch*>& Pair : NonPackageVariantsByName)
	{
		...
		case EDeltaGenVarDataVariantSwitchType::Camera:
		{
			...
			{
				UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Did not spawn a camera actor, so camera variants were discarded!");
			}
			break;
		}
		case EDeltaGenVarDataVariantSwitchType::Geometry:
		{
			...
			for (const FName& NodeName : Var->Geometry.TargetNodes)
			{
				...
				{
					UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Did not find actor '%ls' used by variant '%ls'", *NodeName.ToString(), *VarName);
				}
			}
				...
				for (TPair<FName, TArray<int32>>& ValuePair : ValueIndicesByTargetActor)
				{
					...
					for (int32 ValueIndex : Indices)
					{
						...
						case EObjectSetDataType::Center:
							break;
						default:
							UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Unsupported object set data type captured for actor '%ls' used by variant '%ls'", *TargetActor.ToString(), *VarName);
							continue;
						}
		...
		case EDeltaGenVarDataVariantSwitchType::SwitchObject:
		{
			...
			{
				UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Did not find actor '%ls' used by switch object variant '%ls'", *SwitchObjectName.ToString(), *VarName);
				break;
			}
		...
		default:
			UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Unsupported variant type for variant '%ls'", *VarName);
			continue;
		}
	}
	...
	while (PackageVariantsCopy.Num() > 0)
	{
		...
		for (const FString& TargetVarSetName : PackageVarSwitch->Package.TargetVariantSets)
		{
			...
			if (!TargetVarPtr)
			{
				UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Did not find the target DeltaGen variant '%ls' for parsed package variant '%ls'", *TargetVarSetName, *PackageVarSwitch->Name);
				// Can't step out/return as we need to keep the same number of targets as selected options
			}

			if (!TargetVarSet)
			{
				UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Did not find the target variant set '%ls' for parsed package variant '%ls'", *TargetVarSetName, *PackageVarSwitch->Name);
			}
			...
			for (const FDeltaGenVarDataPackageVariant& Option : PackageVarSwitch->Package.Variants)
			{
				...
				for (int32 TargetIndex = 0; TargetIndex < NumTargetVarSets; TargetIndex++)
				{
					...
					if (int32* TargetDGVarIndex = TargetDGVar->VariantIDToVariantIndex.Find(TargetVariantID))
					{
						...
						{
							UE_LOGF(LogDatasmithDeltaGenImport, Warning, "When parsing package variant '%ls', did not find variant with index %d for variant set '%ls'", *PackageVarSwitch->Name, Option.SelectedVariants[TargetIndex], TargetVarSet->GetName());
							continue;
						}
		...
		if (PackageVarIndex < 0 && RemainingPackageVariants > 0)
		{
			...
			{
				UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Failed to parse %d package variants due to too complex nested structure:", PackageVariantsCopy.Num());
				for (int32 Ind = 0; Ind < PackageVariantsCopy.Num(); Ind++)
				{
					UE_LOGF(LogDatasmithDeltaGenImport, Warning, "\t%ls", *PackageVariantsCopy[Ind]->Name);
				}
				break;
			}
	...
	if (PosStates.Num() > 0)
	{
		...
		for (const FDeltaGenPosDataState& State : PosStates)
		{
			...
			for (const TPair<FString, bool>& StatePair : State.States)
			{
				...
				if (!FoundActors)
				{
					UE_LOGF(LogDatasmithDeltaGenImport, Error, "Did not find actor '%ls' for POS state '%ls'", *ActorName, *State.Name);
					continue;
				}
			...
			for (const TPair<FString, FString>& MaterialPair : State.Materials)
			{
				...
				{
					UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Failed to find material '%ls' for variant set '%ls'", *MaterialName, NewVarSet->GetName());
					continue;
				}
