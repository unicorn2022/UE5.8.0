...
bool FDatasmithDeltaGenImporter::OpenFile(const FString& FilePath)
{
	...
	UE_LOGF(LogDatasmithDeltaGenImport, Log, "Scene %ls has %d nodes, %d geometries, %d meshes, %d materials",
		*FilePath, Stats.NodeCount, Stats.GeometryCount, Stats.MeshCount, Stats.MaterialCount);

	ProcessScene();

	Stats = IntermediateScene->GetStats();
	UE_LOGF(LogDatasmithDeltaGenImport, Log, "Processed scene %ls has %d nodes, %d geometries, %d meshes, %d materials",
		*FilePath, Stats.NodeCount, Stats.GeometryCount, Stats.MeshCount, Stats.MaterialCount);
...
bool FDatasmithDeltaGenImporter::ParseFbxFile(const FString& FBXPath)
{
	...
	{
		UE_LOGF(LogDatasmithDeltaGenImport, Error, "Error parsing FBX file: %ls", FbxImporter->GetErrorMessage());
...
bool FDatasmithDeltaGenImporter::CheckNodeType(const TSharedPtr<FDatasmithFBXSceneNode>& Node)
{
	...
	{
		UE_LOGF(LogDatasmithDeltaGenImport, Error, "Node '%ls' can't have a mesh and a camera at the same time!", *Node->Name);
		return false;
	}
	else if (Node->Mesh.IsValid() && Node->Light.IsValid())
	{
		UE_LOGF(LogDatasmithDeltaGenImport, Error, "Node '%ls' can't have a mesh and a light at the same time!", *Node->Name);
		return false;
	}
	else if (Node->Light.IsValid() && Node->Camera.IsValid())
	{
		UE_LOGF(LogDatasmithDeltaGenImport, Error, "Node '%ls' can't have a light and a camera at the same time!", *Node->Name);
		return false;
	}
	...
	void PopulateTransformAnimation(IDatasmithTransformAnimationElement& TransformAnimation, const FDeltaGenTmlDataAnimationTrack& Track, float InFramerate, float TimelineDelayMs)
	{
		...
		switch(Track.Type)
		{
			...
			case EDeltaGenTmlDataAnimationTrackType::Center:
				UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Center animations are currently not supported!");
				return;
			default:
				return;
		}
		...
		{
			UE_LOGF(LogDatasmithDeltaGenImport, Error, "Unsupported transform animation interpolation type for animation '%ls'!", TransformAnimation.GetLabel());
			return;
		}
	...
	void ResolveDuplicatedObjectNamesRecursive(TSharedPtr<FDatasmithFBXSceneNode>& Node)
	{
		...
		if (Mesh.IsValid() && !ProcessedMeshes.Contains(Mesh))
		{
			if (Mesh->Name.Len() > MAX_MESH_NAME_LENGTH)
			{
				...
				UE_LOGF(LogDatasmithDeltaGenImport, Warning, "Mesh name '%ls' is too long, renaming to '%ls'", *Mesh->Name, *NewName);
				Mesh->Name = NewName;
			}
...
bool FDatasmithDeltaGenImporter::SendSceneToDatasmith()
{
	...
	{
		UE_LOGF(LogDatasmithDeltaGenImport, Error, "FBX Scene root is invalid!");
		return false;
	}
	...
	{
		UE_LOGF(LogDatasmithDeltaGenImport, Error, "Root node '%ls' failed to convert!", *IntermediateScene->RootNode->Name);
		return false;
	}
	return true;
}
