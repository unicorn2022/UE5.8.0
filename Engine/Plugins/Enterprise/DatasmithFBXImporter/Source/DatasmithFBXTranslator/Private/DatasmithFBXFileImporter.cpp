...
void FDatasmithFBXFileImporter::TraverseHierarchyNodeRecursively(FbxNode* ParentNode, TSharedPtr<FDatasmithFBXSceneNode>& ParentInfo)
{
	...
	for (int32 NodeIndex = 0; NodeIndex < NodeCount; NodeIndex++)
	{
		...
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "Node: %ls", *ChildInfo->Name);
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "\tTranslation: [%f, %f, %f]", Trans.X, Trans.Y, Trans.Z);
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "\tScale: [%f, %f, %f]", Scale.X, Scale.Y, Scale.Z);
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "\tRotation: [%f, %f, %f, %f]", Rotation.X, Rotation.Y, Rotation.Z, Rotation.W);
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "\tRotationPivot: [%f, %f, %f]", RotPivot.X, RotPivot.Y, RotPivot.Z);
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "\tScalingPivot: [%f, %f, %f]", ScalePivot.X, ScalePivot.Y, ScalePivot.Z);
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "\tRotationOffset: [%f, %f, %f]", RotOffset.X, RotOffset.Y, RotOffset.Z);
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "\tScalingOffset: [%f, %f, %f]", ScaleOffset.X, ScaleOffset.Y, ScaleOffset.Z);
		...
		if (FbxAttr != nullptr)
		{
			...
			else if (FbxAttrType == FbxNodeAttribute::eLight && BaseOptions->bIncludeLight)
			{
				...
				switch (Light->DecayType.Get())
				{
					...
					case FbxLight::EDecayType::eCubic: //Fall through since there is no cubic decay in VRED
						...
						SceneLight->Intensity /= 318309.875f;
						break;
				}
			...
			else if (FbxAttrType == FbxNodeAttribute::eMesh && BaseOptions->bIncludeGeometry)
			{
				...
				{
					// Triangulation stuff
					UE_LOGF(LogDatasmithFBXImport, Warning, "Triangulating static mesh %ls", *ChildInfo->Name);
					...
					{
						UE_LOGF(LogDatasmithFBXImport, Error, "Unable to triangulate mesh '%ls'", *ChildInfo->Name);
						Mesh = nullptr;
					}
				}
...
void ProcessCurve(FbxAnimCurve* InCurve, FDatasmithFBXSceneAnimCurve* OutCurve, EDatasmithFBXSceneAnimationCurveType InType, EDatasmithFBXSceneAnimationCurveComponent InComponent)
{
	if (InCurve == nullptr)
	{
		UE_LOGF(LogDatasmithFBXImport, Warning, "InCurve was nullptr!");
		return;
	}
	if (OutCurve == nullptr)
	{
		UE_LOGF(LogDatasmithFBXImport, Warning, "OutCurve was nullptr!");
		return;
	}
	...
	for (int32 KeyIndex = 0; KeyIndex < KeyCount; KeyIndex++)
	{
		...
		UE_LOGF(LogDatasmithFBXImport, VeryVerbose, "Time: %f, Value: %f", KeyTime, Key.GetValue());
		...
		{
			UE_LOGF(LogDatasmithFBXImport, Warning, "Key %d of animation track type %d, component %d will have its ArriveTangent clamped to the value %f", KeyIndex, (int32)InType, (int32)InComponent, ArriveTangent > 0 ? SAFE_TANGENT_THRESHOLD : -SAFE_TANGENT_THRESHOLD);
		}

		if (FMath::Abs(LeaveTangent) > SAFE_TANGENT_THRESHOLD)
		{
			UE_LOGF(LogDatasmithFBXImport, Warning, "Key %d of animation track type %d, component %d will have its LeaveTangent clamped to the value %f", KeyIndex, (int32)InType, (int32)InComponent, LeaveTangent > 0 ? SAFE_TANGENT_THRESHOLD : -SAFE_TANGENT_THRESHOLD);
		}
...
void FDatasmithFBXFileImporter::FillCurveFromClipsFile(FbxAnimCurve* InCurve, EDatasmithFBXSceneAnimationCurveType InType, EDatasmithFBXSceneAnimationCurveComponent InComponent)
{
	...
	{
		UE_LOGF(LogDatasmithFBXImport, Warning, "Could not find AnimCurve for DSID '%d' parsed at '%f'", DSID, FirstKeyTime);
		return;
	}
	...
	if (TArray<FDatasmithFBXSceneAnimCurve*>* FoundCurves = ImportedAnimationCurves.Find(DSID))
	{
		for (FDatasmithFBXSceneAnimCurve* FoundCurve : *FoundCurves)
		{
			...
			{
				UE_LOGF(LogDatasmithFBXImport, Verbose, "Matched to curve with DSID %d", DSID);
				Curve = FoundCurve;
				break;
			}
		}
	...
	if (Curve == nullptr)
	{
		UE_LOGF(LogDatasmithFBXImport, Warning, "Failed to match AnimCurve of type %u and component %u to the imported data from .clips file", (uint8)InType, (uint8)InComponent);
		return;
	}
...
FDatasmithFBXSceneAnimCurve FDatasmithFBXFileImporter::CreateNewCurve(FbxAnimCurve* InCurve, EDatasmithFBXSceneAnimationCurveType InType, EDatasmithFBXSceneAnimationCurveComponent InComponent)
{
	...
	if (KeyCount == 1)
	{
		...
		if (FMath::IsNearlyEqual(Val, 0.0f))
		{
			...
			{
				UE_LOGF(LogDatasmithFBXImport, Warning, "Discarding animation curves for having a single key with value zero, as they are likely artifacts of not having .clips data. You can provide two keys with value zero if you wish to prevent this behaviour.");
	...
	if (InProperty.IsValid())
	{
		...
		if (CurveNode != nullptr)
		{
			...
			for (uint8 Channel = 0; Channel < CurveNode->GetChannelsCount(); Channel++)
			{
				...
				UE_LOGF(LogDatasmithFBXImport, Verbose, "\tFound %d curves for property %s, channel %d", CurveCount, PropertyName.Get(), Channel);
...
void FDatasmithFBXFileImporter::ExtractAnimations(FbxNode* Node)
{
	...
	UE_LOGF(LogDatasmithFBXImport, Verbose, "Extracting animation curves for node %ls", UTF8_TO_TCHAR(Node->GetName()));
	...
	UE_LOGF(LogDatasmithFBXImport, Verbose, "Need %d animation blocks to fit %d curves", NumNeededBlocks, AllCurves->Curves.Num());
...
void FDatasmithFBXFileImporter::DoImportMesh(FbxMesh* InMesh, FDatasmithFBXSceneMesh* Mesh)
{
	...
	{
		UE_LOGF(LogDatasmithFBXImport, Error, "There is no geometry information in mesh '%ls'", *Mesh->Name);
		return;
	}
	...
	if (PolygonCount == 0)
	{
		UE_LOGF(LogDatasmithFBXImport, Warning, "No Polygon in mesh '%ls'", *Mesh->Name);
		return;
	}
	...
	{
		UE_LOGF(LogDatasmithFBXImport, Warning, "Unsupported Smoothing group mapping mode on mesh '%ls'", *Mesh->Name);
	}
