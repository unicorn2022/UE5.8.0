...
bool FDatasmithVREDTranslator::LoadScene(TSharedRef<IDatasmithScene> OutScene)
{
	...
	{
		UE_LOGF(LogDatasmithVREDImport, Log, "Failed to open file '%ls'!", *FilePath);
		return false;
	}

	if (!Importer->SendSceneToDatasmith())
	{
		UE_LOGF(LogDatasmithVREDImport, Log, "Failed to convert the VRED FBX scene '%ls' to Datasmith!", OutScene->GetName());
		return false;
	}
