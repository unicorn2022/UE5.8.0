...
void FDatasmithVREDClipProcessor::BuildMaps()
{
    ...
    for (FDatasmithFBXSceneAnimClip& Clip : Clips)
    {
		...
		if(bAlreadyExisted)
        {
            UE_LOGF(LogDatasmithVREDImport, Warning, "Found more than one AnimClip/AnimNode named '%ls'. Imported animations may be innacurate!", *Clip.Name);
        }
    ...
    for (FDatasmithFBXSceneAnimNode& AnimNode : AnimNodes)
    {
        for (FDatasmithFBXSceneAnimBlock& Block : AnimNode.Blocks)
        {
			...
			if(bAlreadyExisted)
            {
                UE_LOGF(LogDatasmithVREDImport, Warning, "Found more than one AnimClip/AnimNode named '%ls'. Imported animations may be innacurate!", *Block.Name);
            }
        }
    }
}
...
void FDatasmithVREDClipProcessor::RecursivelyPropagateFlippedToBlockUsages(FDatasmithFBXSceneAnimClip* Clip, bool bFlipped)
{
    for (FDatasmithFBXSceneAnimUsage& Usage : Clip->AnimUsages)
    {
        ...
        {
            UE_LOGF(LogDatasmithVREDImport, Error, "Did not find target of AnimUsage '%ls'!", *Usage.AnimName);
        }
    }
}
