...
bool FDatasmithVREDImporter::OpenFile(const FString& FilePath)
{
	...
	UE_LOGF(LogDatasmithVREDImport, Log, "Scene %ls has %d nodes, %d geometries, %d meshes, %d materials",
		*FilePath, Stats.NodeCount, Stats.GeometryCount, Stats.MeshCount, Stats.MaterialCount);

	ProcessScene();

	Stats = IntermediateScene->GetStats();
	UE_LOGF(LogDatasmithVREDImport, Log, "Processed scene %ls has %d nodes, %d geometries, %d meshes, %d materials",
		*FilePath, Stats.NodeCount, Stats.GeometryCount, Stats.MeshCount, Stats.MaterialCount);
...
bool inline CheckFbxScene(fbxsdk::FbxScene* Scene)
{
	...
	*/
	...
	UE_LOGF(LogDatasmithVREDImport, Log, "Scene was exported from VRED version '%ls', with DatasmithExporter plugin version '%ls'", *VREDVersion, *ExporterVersion);

	if(ExporterVersion != MATCHING_EXPORTER_VERSION)
	{
		UE_LOGF(LogDatasmithVREDImport, Warning, "Imported data may be innacurate! For the best results, please re-export the scene from VRED with version '%ls' of the DatasmithExporter plugin!", MATCHING_EXPORTER_VERSION);
	}
...
void CleanImportedVariants(TArray<FVREDCppVariant>& VariantSwitches, const FDatasmithFBXScene* VREDScene)
{
	...
	UE_LOGF(LogDatasmithVREDImport, Log, "Attempting to remove invalid or empty variants, variant sets and variant set references. This step can be disabled on the import options menu.");
	...
	for (int32 VarIndex = VariantSwitches.Num() - 1; VarIndex >= 0; VarIndex--)
	{
		...
		{
			UE_LOGF(LogDatasmithVREDImport, Log, "Removed camera variant %ls", *Var.Name);
		...
		case EVREDCppVariantType::Geometry:
		{
			...
			for (int32 OptionIndex = Geom.Options.Num() - 1; OptionIndex >= 0; OptionIndex--)
			{
				...
				{
					UE_LOGF(LogDatasmithVREDImport, Log, "Removed option %ls from geometry variant %ls", *Option.Name, *VarName);
			...
			{
				UE_LOGF(LogDatasmithVREDImport, Log, "Removed geometry variant %ls", *VarName);
				VariantSwitches.RemoveAt(VarIndex);
			}
			break;
		}
		case EVREDCppVariantType::Light:
		{
			...
			{
				UE_LOGF(LogDatasmithVREDImport, Log, "Removed light variant %ls", *VarName);
				VariantSwitches.RemoveAt(VarIndex);
			}
			break;
		}
		case EVREDCppVariantType::Material:
		{
			...
			Mat.Options.RemoveAll([&MaterialNames, &VarName](FVREDCppVariantMaterialOption& Option)
			{
				...
				{
					UE_LOGF(LogDatasmithVREDImport, Log, "Removed option %ls from material variant %ls", *Option.Name, *VarName);
					return true;
				}
				return false;
			});
			...
			{
				UE_LOGF(LogDatasmithVREDImport, Log, "Removed material variant %ls", *VarName);
				VariantSwitches.RemoveAt(VarIndex);
			}
			break;
		}
		case EVREDCppVariantType::Transform:
		{
			...
			{
				UE_LOGF(LogDatasmithVREDImport, Log, "Removed transform variant %ls", *VarName);
				VariantSwitches.RemoveAt(VarIndex);
			}
			break;
		}
	...
	for (int32 VarIndex = VariantSwitches.Num() - 1; VarIndex >= 0; VarIndex--)
	{
		...
		for (int32 OptionIndex = VarSet.TargetVariantNames.Num() - 1; OptionIndex >= 0; OptionIndex--)
		{
			...
			if (!bValidOption)
			{
				UE_LOGF(LogDatasmithVREDImport, Log, "Removed reference to variant %ls with chosen option %ls from variant set %ls", *TargetVariantName, *ChosenOptionName, *Var.Name);
...
bool FDatasmithVREDImporter::ParseFbxFile(const FString& FBXPath)
{
	...
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "Error parsing FBX file: %ls", FbxImporter->GetErrorMessage());
	...
	for (FDatasmithFBXSceneAnimNode& AnimNode : Src)
	{
		for (FDatasmithFBXSceneAnimBlock& Block : AnimNode.Blocks)
		{
			for (FDatasmithFBXSceneAnimCurve& Curve : Block.Curves)
			{
				...
				{
					UE_LOGF(LogDatasmithVREDImport, Warning, "Warning: More than one curve with DSID %d", Curve.DSID);
				}
		...
		for (FDatasmithFBXSceneAnimBlock& Block : AnimNode.Blocks)
		{
			for (FDatasmithFBXSceneAnimCurve& Curve : Block.Curves)
			{
				...
				else
				{
					...
					//UE_LOGF(LogDatasmithVREDImport, Warning, "Warning: Clips file had no description of curve with DSID %d", Curve.DSID);
				}
			}
		}
	}
...
bool FDatasmithVREDImporter::CheckNodeType(const TSharedPtr<FDatasmithFBXSceneNode>& Node)
{
	...
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "Node '%ls' can't have a mesh and a camera at the same time!", *Node->Name);
		return false;
	}
	else if (Node->Mesh.IsValid() && Node->Light.IsValid())
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "Node '%ls' can't have a mesh and a light at the same time!", *Node->Name);
		return false;
	}
	else if (Node->Light.IsValid() && Node->Camera.IsValid())
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "Node '%ls' can't have a light and a camera at the same time!", *Node->Name);
		return false;
	}
...
void AddColorProperty(IDatasmithMaterialInstanceElement* Element, const FString& PropertyName, const FVector4& Value)
{
	...
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "Ignored vector parameter '%ls' of material '%ls' for being a NaN/infinite", *PropertyName, Element->GetName());
		return;
	}
...
void AddTextureMappingProperties(IDatasmithMaterialInstanceElement* Element, const FString& TexHandle, const FDatasmithFBXSceneMaterial::FTextureParams& Tex)
{
	...
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "Displacement textures are only supported with UV mapping. Texture '%ls' will use UV mapping instead", *Tex.Path);
...
void PopulateTransformAnimation(IDatasmithTransformAnimationElement& TransformAnimation, EDatasmithTransformType AnimationType, const FDatasmithFBXSceneAnimCurve* CurveX, const FDatasmithFBXSceneAnimCurve* CurveY, const FDatasmithFBXSceneAnimCurve* CurveZ, double ScaleFactor, float Framerate)
{
	...
	for (int32 Frame = StartFrame.Value; Frame <= EndFrame.Value + 1; ++Frame)
	{
		...
		//UE_LOGF(LogTemp, Warning, "Adding frame %d of type %u at %f, vec: (%f, %f, %f)", Frame, (uint8)AnimationType, TimeSeconds, Val.X, Val.Y, Val.Z);
	}
}
...
TSharedPtr<IDatasmithLevelSequenceElement> FDatasmithVREDImporter::ConvertAnimBlock(const FCombinedAnimBlock& CombinedBlock)
{
	...
	for (const auto& Pair : CombinedBlock.NodeNameToBlock)
	{
		...
		if (CurvesPerType.Contains(EDatasmithFBXSceneAnimationCurveType::Visible))
		{
			...
			if (Curves.Num() > 1)
			{
				UE_LOGF(LogDatasmithVREDImport, Error, "AnimBlock for node '%ls' has more than one Visibility curve. The first one will be used.", *NodeName);
			}
...
void ConvertAnimClips(TArray<FDatasmithFBXSceneAnimClip>& AnimClips, TMap<FString, FImportedAnim>& ImportedAnims, TArray<TSharedPtr<IDatasmithLevelSequenceElement>>& OutCreatedClips, float Framerate)
{
	...
	while (ClipsRemaining.Num() > 0)
	{
		...
		for (int32 ClipIndex = ClipsRemaining.Num()-1; ClipIndex >= 0; ClipIndex--)
		{
			...
			if (!bCanParse)
			{
				UE_LOGF(LogDatasmithVREDImport, Verbose, "Postponed parsing AnimClip '%ls'. It still needs these usages:", *Clip.Name);
				for (const FString& Usage : MissingParsedUsages)
				{
					UE_LOGF(LogDatasmithVREDImport, VeryVerbose, "\t%ls", *Usage);
				}

				UE_LOGF(LogDatasmithVREDImport, VeryVerbose, "This is what has been imported so far:");
				for (const auto& Pair : ImportedAnims)
				{
					UE_LOGF(LogDatasmithVREDImport, VeryVerbose, "\t%ls", *Pair.Key);
				}
				continue;
			}

			UE_LOGF(LogDatasmithVREDImport, Verbose, "Parsing AnimClip '%ls'", *Clip.Name);
			...
			for (FDatasmithFBXSceneAnimUsage& Usage : Clip.AnimUsages)
			{
				if (FImportedAnim* FoundAnim = ImportedAnims.Find(Usage.AnimName))
				{
					...
					UE_LOGF(LogDatasmithVREDImport, VeryVerbose, "\tParsed subsequence animation. TargetClip: '%ls', StartTime: %d, TimeScale: %f, OrigStart: %f, OrigEnd: %f, UsageStart: %f, UsageEnd: %f", *Usage.AnimName, StartFrame.Value, TimeScale, FoundAnim->OriginalStartSeconds, FoundAnim->OriginalEndSeconds, Usage.StartTime, Usage.EndTime);
					SequenceElement->AddAnimation(SubsequenceAnimation.ToSharedRef());
				}
			}

			UE_LOGF(LogDatasmithVREDImport, Verbose, "Parsed AnimClip '%ls', which depends on the following clips:", *Clip.Name);
			for (FDatasmithFBXSceneAnimUsage& Usage : Clip.AnimUsages)
			{
				UE_LOGF(LogDatasmithVREDImport, Verbose, "\t%ls", *Usage.AnimName);
			}
		...
		if (NumClipsParsedThisLoop == 0)
		{
			...
			UE_LOGF(LogDatasmithVREDImport, Error, "%ls", *ErrorMessage);
			break;
		}
	}
}
	...
	void ResolveDuplicatedObjectNamesRecursive(TSharedPtr<FDatasmithFBXSceneNode>& Node)
	{
		...
		if (Mesh.IsValid() && !ProcessedMeshes.Contains(Mesh))
		{
			if (Mesh->Name.Len() > MAX_MESH_NAME_LENGTH)
			{
				...
				UE_LOGF(LogDatasmithVREDImport, Warning, "Mesh name '%ls' is too long, renaming to '%ls'", *Mesh->Name, *NewName);
				Mesh->Name = NewName;
			}
...
bool FDatasmithVREDImporter::SendSceneToDatasmith()
{
	...
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "FBX Scene root is invalid!");
		return false;
	}
	...
	if (NodeActor.IsValid())
	{
		...
		for (const TSharedPtr<IDatasmithLevelSequenceElement>& CreatedClip : CreatedClips)
		{
			...
			{
				UE_LOGF(LogDatasmithVREDImport, Verbose, "Added clip level sequence '%ls' to the scene", CreatedClip->GetName());
	...
	{
		UE_LOGF(LogDatasmithVREDImport, Error, "Root node '%ls' failed to convert!", *IntermediateScene->RootNode->Name);
		return false;
	}
	return true;
}
