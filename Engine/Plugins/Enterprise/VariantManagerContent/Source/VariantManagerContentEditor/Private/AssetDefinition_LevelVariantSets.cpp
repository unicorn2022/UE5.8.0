...
TConstArrayView<FAssetCategoryPath> UAssetDefinition_LevelVariantSets::GetAssetCategories() const
{
	static const TArray<FAssetCategoryPath> Categories = 
		{
			FAssetCategoryPath(EAssetCategoryPaths::Data, LOCTEXT("LevelVariantSets", "Other"), ECategoryMenuType::Section)
		};
	return Categories;
}
...
const FSlateBrush* UAssetDefinition_LevelVariantSets::GetThumbnailBrush(const FAssetData& InAssetData, const FName InClassName) const
{
	return FVariantManagerContentStyle::Get().GetBrush("VariantManager.Icon");
}
