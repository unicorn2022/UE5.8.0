...
{
	if (WorldContext == nullptr || !IsValidChecked(WorldContext) || WorldContext->IsUnreachable() || !WorldContext->GetWorld())
	{
		return nullptr;
	}
...
UVariantSet* ULevelVariantSets::GetVariantSetByName(FString VariantSetName)
{
	...
	{
		return VarSet && VarSet->GetDisplayText().ToString() == VariantSetName;
	});
...
int32 ULevelVariantSets::GetVariantSetIndex(FString VariantSetName) const
{
	return VariantSets.IndexOfByPredicate([VariantSetName](const UVariantSet* VarSet)
	{
		return VarSet && VarSet->GetDisplayText().ToString() == VariantSetName;
	});
}
