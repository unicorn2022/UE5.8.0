...
void UPropertyValueSoftObject::SetRecordedData(const uint8* NewDataBytes, int32 NumBytes, int32 Offset /*= 0*/)
{
	...
	if (ULevelVariantSets* LVS = GetTypedOuter<ULevelVariantSets>())
	{
		...
		if (PIEInstanceID != INDEX_NONE)
		{
			...
			{
				UE_LOGF(LogVariantContent, Error, "Failed to record Actor '%ls' into property '%ls': The actor could not be found in the Editor world. Note that it is not possible to record references to temporary actors!", *NewObj->GetName(), *GetFullDisplayString());
			}
