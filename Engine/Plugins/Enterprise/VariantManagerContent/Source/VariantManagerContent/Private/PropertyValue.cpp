...
int32 EnumValueBytesToIndex(UEnum* Enum, const TArray<uint8>& ValueBytes, bool bEnumIsSignedInt)
{
	...
	if (bEnumIsSignedInt)
	{
		...
		default:
			UE_LOGF(LogVariantContent, Error, "Invalid size for signed int value: %d", Size);
			return INDEX_NONE;
		}
	}
	else
	{
		...
		default:
			UE_LOGF(LogVariantContent, Error, "Invalid size for unsigned int value: %d", Size);
			return INDEX_NONE;
		}
	}
...
void UPropertyValue::Serialize(FArchive& Ar)
{
	...
	else if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogVariantContent, Warning, "Captured property '%ls' was created with an older Unreal Studio version (4.21 or less). A conversion to the new storage format is required and will be attempted. There may be some data loss.", *FullDisplayString);
			...
			if (NumDeprecatedProps > 0)
			{
				...
				{
					UE_LOGF(LogVariantContent, Warning, "Failed to convert property '%ls'! Captured data will be ignored and property will fail to resolve.", *FullDisplayString);
		...
		if (CustomVersion >= FVariantManagerObjectVersion::CorrectSerializationOfFStringBytes)
		{
			...
			{
				//UE_LOGF(LogVariantContent, Error, "Failed to retrieve property class for property '%ls'", *GetFullDisplayString());
	...
	if (ParentContainerClass)
	{
		if (const UClass* Class = Cast<const UClass>(ParentContainerClass))
		{
			...
			if (PropertySetter)
			{
				...
				{
					UE_LOGF(LogVariantContent, Error, "Property setter does not have a parameter that can receive an object of the property type (%ls)!", *ThisClass->GetName());
...
int32 UPropertyValue::GetRecordedDataAsEnumIndex()
{
	...
	if (!Enum)
	{
		UE_LOGF(LogVariantContent, Error, "Invalid enum for enum property '%ls'", *GetFullDisplayString());
		return INDEX_NONE;
	}

	if (!HasRecordedData())
	{
		UE_LOGF(LogVariantContent, Error, "Enum property '%ls' has no recorded data!", *GetFullDisplayString());
		return INDEX_NONE;
	}
	...
	{
		UE_LOGF(LogVariantContent, Error, "Invalid underlying format for enum property!");
		return INDEX_NONE;
	}
...
void UPropertyValue::SanitizeRecordedEnumData()
{
	...
	if (!IsEnumIndexValid(Enum, CurrentIndex))
	{
		...
		{
			UE_LOGF(LogVariantContent, Error, "Enum property '%ls' points to an enum that doesn't have any valid values!", *GetFullDisplayString());
			return;
		}
	}
}
...
void UPropertyValue::SetRecordedDataFromEnumIndex(int32 Index)
{
	...
	if(!IsEnumIndexValid(Enum, Index))
	{
		UE_LOGF(LogVariantContent, Error, "Invalid index for enum '%ls'!", *Enum->GetName());
		return;
	}
...
FString UPropertyValue::GetBaseDisplayString() const
{
	FString LeftString;
	FString RightString;
...
int32 UPropertyValue::GetValueSizeInBytes() const
{
	...
	UE_LOGF(LogVariantContent, Warning, "Returning size zero for PropertyValue '%ls'", *GetFullDisplayString());
	return 0;
}
...
int32 UPropertyValue::GetPropertyOffsetInBytes() const
{
	...
	// Dangerous
	UE_LOGF(LogVariantContent, Warning, "Returning offset zero for PropertyValue '%ls'", *GetFullDisplayString());
	return 0;
}
...
const TArray<uint8>& UPropertyValue::GetRecordedData()
{
	...
	if (bHasRecordedData && PropClass && PropClass->IsChildOf(FObjectPropertyBase::StaticClass()) && !TempObjPtr.IsNull())
	{
		...
		{
			UE_LOGF(LogVariantContent, Warning, "Failed to find object with the correct class at path '%ls' for property '%ls'. Value will be ignored.",
				*TempObjPtr.ToString(),
				*FullDisplayString
			);
...
void UPropertyValue::ApplyViaFunctionSetter(UObject* TargetObject)
{
	...
	if (!TargetObject)
	{
		UE_LOGF(LogVariantContent, Error, "Trying to apply via function setter with a nullptr target object! (UPropertyValue: %ls)", *GetFullDisplayString());
		return;
	}
	if (!PropertySetter)
	{
		UE_LOGF(LogVariantContent, Error, "Trying to apply via function setter with a nullptr function setter! (UPropertyValue: %ls)", *GetFullDisplayString());
		return;
	}
	...
	for( TFieldIterator<FProperty> It(PropertySetter); It && It->HasAnyPropertyFlags(CPF_Parm) && !It->HasAnyPropertyFlags(CPF_OutParm|CPF_ReturnParm); ++It, NumParamsEvaluated++ )
	{
		...
		if (Defaults)
		{
			...
			if (!Result)
			{
				UE_LOGF(LogVariantContent, Error, "Failed at applying the default value for parameter '%ls' of PropertyValue '%ls'", *PropertyParam->GetName(), *GetFullDisplayString());
			}
		}
	...
	{
		UE_LOGF(LogVariantContent, Error, "Did not find a parameter that could receive our value of class %ls", *GetPropertyClass()->GetName());
	}
...
void UPropertyValueTransform::Serialize(FArchive& Ar)
{
	...
	case EPropertyValueCategory::RelativeScale3D:
		...
		break;
	default:
		UE_LOGF(LogVariantContent, Error, "Problem serializing old PropertyValueTransform '%ls'", *GetFullDisplayString());
		break;
	}
}
...
void UPropertyValueVisibility::Serialize(FArchive& Ar)
{
	...
	case EPropertyValueCategory::Visibility:
		...
		break;
	default:
		UE_LOGF(LogVariantContent, Error, "Problem serializing old PropertyValueVisibility '%ls'", *GetFullDisplayString());
		break;
	}
}
