...
TArray<uint8> UPropertyValueMaterial::GetDataFromResolvedObject() const
{
	...
	if (!ContainerObject)
	{
		UE_LOGF(LogVariantContent, Error, "UPropertyValueMaterial '%ls' does not have a UMeshComponent as parent address!", *GetFullDisplayString());
		return CurrentData;
	}
...
void UPropertyValueMaterial::ApplyDataToResolvedObject()
{
	...
	if ( !Mat )
	{
		UE_LOGF( LogVariantContent, Error, "Failed to apply recorded data from UPropertyValueMaterial '%ls'!", *GetFullDisplayString() );
		return;
	}
				...
				if (Resolve(Object->GetClass()->GetDefaultObject()))
				{
					if (UMeshComponent* ContainerObject = (UMeshComponent*) ParentContainerAddress)
					{
						...
						if (NumSegs > 0)
						{
							...
							if (!ContainerObject->GetMaterialSlotNames().IsValidIndex(MatIndex))
							{
								UE_LOGF(LogVariantContent, Warning, "Tried to fetch non-existent material with index %d for object '%ls'! Setting default as nullptr instead", MatIndex, *Object->GetName());
							}
