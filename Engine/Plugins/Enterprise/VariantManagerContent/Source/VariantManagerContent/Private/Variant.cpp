...
void UVariant::SetThumbnailFromObject(UObject* ObjectThumbnail)
{
	...
	{
		if (UTexture2D* NewThumbnail = ThumbnailGenerator::GenerateThumbnailFromObjectThumbnail(ObjectThumbnail))
		{
...
int32 UVariant::AddDependency(FVariantDependency& Dependency)
{
	...
	{
		UE_LOGF(LogVariantContent, Error, "Cannot add variant '%ls' as a dependency of '%ls'! Cycles were detected!", *Variant->GetDisplayText().ToString(), *GetDisplayText().ToString());
		return INDEX_NONE;
	}
...
void UVariant::SetDependency(int32 Index, FVariantDependency& Dependency)
{
	if (Dependencies.IsValidIndex(Index))
	{
		...
		if (Variant && !IsValidDependency(Variant))
		{
			UE_LOGF(LogVariantContent, Error, "Cannot set variant '%ls' as a dependency of '%ls'! Cycles were detected!",
	...
	{
		UE_LOGF(LogVariantContent, Error, "Invalid dependency index '%d'! Note: Variant '%ls' has '%d' dependencies", Index, *GetDisplayText().ToString(), GetNumDependencies());
	}
}
