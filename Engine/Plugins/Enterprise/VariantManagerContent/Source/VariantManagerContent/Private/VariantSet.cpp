...
UVariant* UVariantSet::GetVariantByName(FString VariantName)
{
	...
	{
		return Var && Var->GetDisplayText().ToString() == VariantName;
	});
...
int32 UVariantSet::GetVariantIndexByName(FString VariantName) const
{
	return Variants.IndexOfByPredicate([VariantName](const UVariant* Var)
	{
		return Var && Var->GetDisplayText().ToString() == VariantName;
	});
}

void UVariantSet::SetThumbnailFromObject(UObject* ObjectThumbnail)
{
