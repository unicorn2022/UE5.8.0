...
UTexture2D* ThumbnailGenerator::GenerateThumbnailFromTexture(UTexture2D* Texture)
{
	...
	{
		UE_LOGF(LogVariantContent, Error, "Failed create a thumbnail from texture '%ls'", *Texture->GetName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogVariantContent, Warning, "Failed to generate thumbnail from texture '%ls'", *Texture->GetName());
	}
...
UTexture2D* ThumbnailGenerator::GenerateThumbnailFromCamera(UObject* WorldContextObject, const FTransform& CameraTransform, float FOVDegrees, float MinZ, float Gamma)
{
    ...
    if (Thumbnail == nullptr)
	{
		UE_LOGF(LogVariantContent, Warning, "Failed to resize texture thumbnail texture from camera!");
	}
...
UTexture2D* ThumbnailGenerator::GenerateThumbnailFromEditorViewport()
{
    ...
    if (Thumbnail == nullptr)
	{
		UE_LOGF(LogVariantContent, Warning, "Failed to create thumbnail texture from viewport!");
	}
...
UTexture2D* ThumbnailGenerator::GenerateThumbnailFromObjectThumbnail(UObject* Object)
{
    ...
    if (OldThumbnail && !OldThumbnail->IsEmpty())
    {
		...
		if (Thumbnail == nullptr)
		{
			UE_LOGF(LogVariantContent, Warning, "Failed to create thumbnail texture from object '%ls'!", *Object->GetName());
		}
