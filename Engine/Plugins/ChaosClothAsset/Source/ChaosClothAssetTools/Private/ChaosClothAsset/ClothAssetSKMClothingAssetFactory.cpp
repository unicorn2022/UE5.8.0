...
TArray<UClothingAssetBase*> UChaosClothAssetSKMClothingAssetFactory::CreateFromSourceAsset(USkeletalMesh* TargetMesh, const UObject* SourceAsset) const
{
	...
	{
		const FName AssetName = StaticFindObject(UClothingAssetBase::StaticClass(), TargetMesh, *ClothAsset->GetName()) ?
			MakeUniqueObjectName(TargetMesh, UClothingAssetBase::StaticClass(), ClothAsset->GetFName()) :
			ClothAsset->GetFName();
