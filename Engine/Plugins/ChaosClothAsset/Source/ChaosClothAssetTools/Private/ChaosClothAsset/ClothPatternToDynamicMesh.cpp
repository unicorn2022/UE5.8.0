...
namespace UE::Chaos::ClothAsset
{
	namespace Private
	{
		...
		public:
			...
			FClothPatternWrapper(
				const FCollectionClothConstFacade& ClothFacade,
				int32 PatternIndex,
				EClothPatternVertexType VertexDataType,
				bool bInFlip2DSimFaces = false,
				bool bInConvertWeightMaps = true)
				: VertexDataType(VertexDataType)
				, Cloth(ClothFacade)
				, bFlip2DSimFaces(bInFlip2DSimFaces)
				, bConvertWeightMaps(bInConvertWeightMaps)
			{
				NumTexCoords = 0;
			...
			{
				checkSlow(WeightMapNames.IsValidIndex(LayerIndex));
		...
		private:
			...
			int32 NumTexCoords;
			bool bFlip2DSimFaces = false;
			bool bConvertWeightMaps = true;
	...
	void FClothPatternToDynamicMesh::Convert(
		...
		bool bFlip2DSimFaces,
		bool bConvertWeightMaps)
	{
		const FCollectionClothConstFacade ClothFacade(ClothCollection);

		// Actual conversion
		UE::Geometry::TToDynamicMesh<Private::FClothPatternWrapper> PatternToDynamicMesh;
		Private::FClothPatternWrapper PatternWrapper(ClothFacade, PatternIndex, VertexDataType, bFlip2DSimFaces, bConvertWeightMaps);

		auto TriangleToGroupFunction = [](Private::FClothPatternWrapper::TriIDType) { return 0; };
		...
		else
		{
			...
			auto TriangleToMaterialFunction = [PatternIndex, bIsRenderType, &ClothFacade](Private::FClothPatternWrapper::TriIDType TriID)->int32
			{
				if (bIsRenderType)
				{
					if (PatternIndex != INDEX_NONE)
		...
		bool bConvertWeightMaps)
	{
		...
		Convert(ClothCollections[LODIndex], PatternIndex, VertexDataType, MeshOut, bDisableAttributes, MaterialOffset, bFlip2DSimFaces, bConvertWeightMaps);
	}
}	// namespace UE::Chaos::ClothAsset
