...
namespace UE::Chaos::ClothAsset
{
	...
	void FClothGeometryTools::StripUnusedUserAttributes(const TSharedRef<FManagedArrayCollection>& ClothCollection, bool bStripUnusedSimWeightMaps, const TArray<FName>& ExtraSimWeightMaps, bool bStripUnusedSets, const TArray<FName>& ExtraSets, bool bStripUnusedFaceIntMaps, const TArray<FName>& ExtraFaceIntMaps, bool bStripUnusedRenderWeightMaps, const TArray<FName>& ExtraRenderWeightMaps)
	{
		const ::Chaos::Softs::FCollectionPropertyConstFacade Properties(ClothCollection);
		FCollectionClothFacade Cloth(ClothCollection);
		...
		if (bStripUnusedSimWeightMaps)
		{
			const TArray<FName> WeightMapNames = Cloth.GetWeightMapNames();
			for (const FName& WeightMapName : WeightMapNames)
			{
				if (!(IsNameUsedInProperties(WeightMapName) || IsNameReserved(WeightMapName) || ExtraSimWeightMaps.Contains(WeightMapName)))
				{
					Cloth.RemoveWeightMap(WeightMapName);
				}
			}
		}
		...
		if (bStripUnusedSets)
		{
			...
			{
				if (!(IsNameUsedInProperties(SelectionName) || IsNameReserved(SelectionName) || ExtraSets.Contains(SelectionName)))
				{
					Selection.RemoveSelectionSet(SelectionName);
				}
			}
		}

		if (bStripUnusedFaceIntMaps)
		{
			...
			{
				if (!(IsNameUsedInProperties(FaceIntMapName) || IsNameReserved(FaceIntMapName) || ExtraFaceIntMaps.Contains(FaceIntMapName)))
				{
					Cloth.RemoveUserDefinedAttribute(FaceIntMapName, ClothCollectionGroup::SimFaces);
				}
			}
		}

		if (bStripUnusedRenderWeightMaps)
		{
			const TArray<FName> WeightMapNames = Cloth.GetUserDefinedAttributeNames<float>(ClothCollectionGroup::RenderVertices);
			for (const FName& WeightMapName : WeightMapNames)
			{
				if (!(IsNameUsedInProperties(WeightMapName) || IsNameReserved(WeightMapName) || ExtraRenderWeightMaps.Contains(WeightMapName)))
				{
					Cloth.RemoveUserDefinedAttribute(WeightMapName, ClothCollectionGroup::RenderVertices);
				}
			}
		}
	}
