...
namespace UE::Chaos::ClothAsset
{
	...
	uint32 FCollectionClothConstFacade::CalculateTypeHash(bool bIncludeWeightMaps, uint32 PreviousHash) const
	{
		...
		ResultHash = HashCombineFast(ResultHash, GetTypeHash(GetCopyAllUserAttributesToSimModel()));
	...
	bool FCollectionClothConstFacade::GetCopyAllUserAttributesToSimModel() const
	{
		return ClothCollection->GetCopyAllUserAttributesToSimModel() && ClothCollection->GetNumElements(ClothCollectionGroup::Lods) > 0 ? (*ClothCollection->GetCopyAllUserAttributesToSimModel())[0] : false;
	}
	...
	void FCollectionClothFacade::Append(const FCollectionClothConstFacade& Other)
	{
		...
		if (GetCopyAllUserAttributesToSimModel() == false)
		{
			SetCopyAllUserAttributesToSimModel(Other.GetCopyAllUserAttributesToSimModel());
		}
	...
	void FCollectionClothFacade::InitializeCookedOnly(const FCollectionClothConstFacade& Other)
	{
		...
		// Sim AccessoryMeshes
		SetNumSimAccessoryMeshes(0, bCookedOnly);
		const int32 OtherNumSimAccessoryMeshes = Other.GetNumSimAccessoryMeshes();
		SetNumSimAccessoryMeshes(OtherNumSimAccessoryMeshes, bCookedOnly);
	...
	void FCollectionClothFacade::SetCopyAllUserAttributesToSimModel(bool bValue)
	{
		if (ClothCollection->GetNumElements(ClothCollectionGroup::Lods) && GetClothCollection()->GetCopyAllUserAttributesToSimModel())
		{
			(*GetClothCollection()->GetCopyAllUserAttributesToSimModel())[0] = bValue;
		}
	}
