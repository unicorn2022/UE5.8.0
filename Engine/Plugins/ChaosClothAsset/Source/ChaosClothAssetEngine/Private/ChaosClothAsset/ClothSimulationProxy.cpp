...
namespace UE::Chaos::ClothAsset
{
	...
	void FClothSimulationProxy::BeginInitialization_GameThread()
	{
		...
		for (const UChaosClothAssetBase* const Asset : ClothComponentAdapter.GetAssets())
		{
			...
			for (int32 ModelIndex = 0; ModelIndex < NumClothSimulationModels; ++ModelIndex)
			{
				...
				{
					checkf(Algo::NoneOf(Cloths, [&SimulationGroupId](const TUniquePtr<::Chaos::FClothingSimulationCloth>& Cloth)
						{
							return Cloth && Cloth->GetGroupId() == SimulationGroupId;
						}), TEXT("The ClothComponentAdapter cannot return the same SimulationGroupId for two different Asset/ModelIndex pairs (unless INDEX_NONE)."));

					const TSharedPtr<const FChaosClothSimulationModel> ClothSimulationModel = ClothComponentAdapter.GetActiveClothSimulationModel(Asset, ModelIndex);
	...
	bool FClothSimulationProxy::PreSimulate_GameThread(float DeltaTime)
	{
		...
		{			
			const bool bUsesBuoyancy = Solver->UsesBuoyancy();

			FBox BoundingBox;
			if (World->PhysicsField || bUsesBuoyancy)
			{
				BoundingBox = CalculateBounds_AnyThread().GetBox().TransformBy(ClothComponentAdapter.GetOwnerComponent().GetComponentTransform());
			}
			...
			// broadphase against water bodies			
			if (bUsesBuoyancy)
			{				
				// find all buoyancy fields and compute collisions				
				Solver->PreSimulateBuoyancyField_GameThread(DeltaTime, World, BoundingBox);
			}
		}
	...
	void FClothSimulationProxy::PostSimulate_GameThread(TMap<int32, FClothSimulData>* ExternalClothSimulData)
	{
		if (bIsSimulating)
		{
			WriteSimulationData(ExternalClothSimulData);

			// Clean properties dirty flags to avoid re-updating the constraints at the next tick when properties have not changed
			// Note this does not clear the source collection whose dirty flags are cleared after the copy in InitializeConfigs()
			for (TUniquePtr<::Chaos::FClothingSimulationConfig>& Config : Configs)
			{
				const int32 LODIndex = Solver->GetSolverLOD();
				if (LODIndex < Config->GetNumLODs())
				{
					Config->GetProperties(LODIndex).ClearDirtyFlags();
				}
			}
		}
	}

	void FClothSimulationProxy::PostProcess_GameThread(TMap<int32, FClothSimulData>* ExternalClothSimulData)
	{
		if (!bIsPreProcessed)
		{
	...
	void FClothSimulationProxy::Tick()
	{
		...
		{
			static const TConsoleVariableData<int32>* const DebugDrawSingleVertexCVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("p.ChaosCloth.DebugDrawSingleVertex"));
			static const TConsoleVariableData<FString>* const DebugDrawFilterSetCVar = IConsoleManager::Get().FindTConsoleVariableDataString(TEXT("p.ChaosCloth.DebugDrawFilterSet"));
			...
			static const TConsoleVariableData<bool>* const DebugDrawSelfCollisionThicknessCVar = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("p.ChaosCloth.DebugDrawSelfCollisionThickness"));
			static const TConsoleVariableData<bool>* const DebugDrawClothCollisionThicknessCVar = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("p.ChaosCloth.DebugDrawClothCollisionThickness"));
			static const TConsoleVariableData<bool>* const DebugDrawInnerCollisionThicknessCVar = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("p.ChaosCloth.DebugDrawInnerCollisionThickness"));
			...
			static const TConsoleVariableData<bool>* const DebugDrawVelocityScaleCVar = IConsoleManager::Get().FindTConsoleVariableDataBool(TEXT("p.ChaosCloth.DebugDrawVelocityScale"));

			FClothVisualizationNoGC::FDrawContext DrawContext;
			DrawContext.Filter.SingleVertex = DebugDrawSingleVertexCVar ? DebugDrawSingleVertexCVar->GetValueOnAnyThread() : INDEX_NONE;
			DrawContext.Filter.VertexSet = DebugDrawFilterSetCVar ? DebugDrawFilterSetCVar->GetValueOnAnyThread() : FString();
			FClothVisualizationNoGC::FDrawTextsContext DrawTextsContext;
			DrawTextsContext.Filter = DrawContext.Filter;

			if (DebugDrawParticleIndicesCVar && DebugDrawParticleIndicesCVar->GetValueOnAnyThread()) { Visualization->DrawParticleIndices(DrawTextsContext); }
			if (DebugDrawElementIndicesCVar && DebugDrawElementIndicesCVar->GetValueOnAnyThread()) { Visualization->DrawElementIndices(DrawTextsContext); }
			if (DebugDrawLocalSpaceCVar && DebugDrawLocalSpaceCVar->GetValueOnAnyThread()) { Visualization->DrawLocalSpace(DrawContext); }
			if (DebugDrawBoundsCVar && DebugDrawBoundsCVar->GetValueOnAnyThread()) { Visualization->DrawBounds(DrawContext); }
			if (DebugDrawGravityCVar && DebugDrawGravityCVar->GetValueOnAnyThread()) { Visualization->DrawGravity(DrawContext); }
			if (DebugDrawPhysMeshWiredCVar && DebugDrawPhysMeshWiredCVar->GetValueOnAnyThread()) { Visualization->DrawPhysMeshWired(DrawContext); }
			if (DebugDrawAnimMeshWiredCVar && DebugDrawAnimMeshWiredCVar->GetValueOnAnyThread()) { Visualization->DrawAnimMeshWired(DrawContext); }
			if (DebugDrawPointVelocitiesCVar && DebugDrawPointVelocitiesCVar->GetValueOnAnyThread()) { Visualization->DrawPointVelocities(DrawContext); }
			if (DebugDrawAnimNormalsCVar && DebugDrawAnimNormalsCVar->GetValueOnAnyThread()) { Visualization->DrawAnimNormals(DrawContext); }
			if (DebugDrawPointNormalsCVar && DebugDrawPointNormalsCVar->GetValueOnAnyThread()) { Visualization->DrawPointNormals(DrawContext); }
			if (DebugDrawCollisionCVar && DebugDrawCollisionCVar->GetValueOnAnyThread()) { Visualization->DrawCollision(DrawContext); }
			if (DebugDrawBackstopsCVar && DebugDrawBackstopsCVar->GetValueOnAnyThread()) { Visualization->DrawBackstops(DrawContext); }
			if (DebugDrawBackstopDistancesCVar && DebugDrawBackstopDistancesCVar->GetValueOnAnyThread()) { Visualization->DrawBackstopDistances(DrawContext); }
			if (DebugDrawMaxDistancesCVar && DebugDrawMaxDistancesCVar->GetValueOnAnyThread()) { Visualization->DrawMaxDistances(DrawContext); }
			if (DebugDrawMaxDistanceValuesCVar && DebugDrawMaxDistanceValuesCVar->GetValueOnAnyThread()) { Visualization->DrawMaxDistanceValues(DrawTextsContext); }
			if (DebugDrawAnimDriveCVar && DebugDrawAnimDriveCVar->GetValueOnAnyThread()) { Visualization->DrawAnimDrive(DrawContext); }
			if (DebugDrawEdgeConstraintCVar && DebugDrawEdgeConstraintCVar->GetValueOnAnyThread()) { Visualization->DrawEdgeConstraint(DrawContext); }
			if (DebugDrawBendingConstraintCVar && DebugDrawBendingConstraintCVar->GetValueOnAnyThread()) { Visualization->DrawBendingConstraint(DrawContext); }
			if (DebugDrawLongRangeConstraintCVar && DebugDrawLongRangeConstraintCVar->GetValueOnAnyThread()) { Visualization->DrawLongRangeConstraint(DrawContext); }
			if (DebugDrawWindForcesCVar && DebugDrawWindForcesCVar->GetValueOnAnyThread()) { Visualization->DrawWindAndPressureForces(DrawContext); }
			if (DebugDrawWindVelocityCVar && DebugDrawWindVelocityCVar->GetValueOnAnyThread()) { Visualization->DrawWindVelocity(DrawContext); }
			if (DebugDrawSelfCollisionCVar && DebugDrawSelfCollisionCVar->GetValueOnAnyThread()) { Visualization->DrawSelfCollision(DrawContext); }
			if (DebugDrawSelfIntersectionCVar && DebugDrawSelfIntersectionCVar->GetValueOnAnyThread()) { Visualization->DrawSelfIntersection(DrawContext); }
			if (DebugDrawSelfCollisionThicknessCVar && DebugDrawSelfCollisionThicknessCVar->GetValueOnAnyThread()) { Visualization->DrawSelfCollisionThickness(DrawContext); }
			if (DebugDrawClothCollisionThicknessCVar && DebugDrawClothCollisionThicknessCVar->GetValueOnAnyThread()) { Visualization->DrawClothCollisionThickness(DrawContext); }
			if (DebugDrawInnerCollisionThicknessCVar && DebugDrawInnerCollisionThicknessCVar->GetValueOnAnyThread()) { Visualization->DrawInnerCollisionThickness(DrawContext); }
			if (DebugDrawParticleIndicesCVar && DebugDrawParticleIndicesCVar->GetValueOnAnyThread()) { Visualization->DrawParticleIndices(DrawTextsContext); }
			if (DebugDrawElementIndicesCVar && DebugDrawElementIndicesCVar->GetValueOnAnyThread()) { Visualization->DrawElementIndices(DrawTextsContext); }
			if (DebugDrawClothClothConstraintsCVar && DebugDrawClothClothConstraintsCVar->GetValueOnAnyThread()) { Visualization->DrawClothClothConstraints(DrawContext); }
			if (DebugDrawTeleportResetCVar && DebugDrawTeleportResetCVar->GetValueOnAnyThread()) { Visualization->DrawTeleportReset(DrawContext); }
			if (DebugDrawExtremlyDeformedEdgesCVar && DebugDrawExtremlyDeformedEdgesCVar->GetValueOnAnyThread()) { Visualization->DrawExtremlyDeformedEdges(DrawContext); }
			if (DebugDrawVelocityScaleCVar && DebugDrawVelocityScaleCVar->GetValueOnAnyThread()) { Visualization->DrawVelocityScale(DrawContext); }
		}
	...
	void FClothSimulationProxy::WriteSimulationData(TMap<int32, FClothSimulData>* OutClothSimulDataOverride)
	{
		using namespace ::Chaos;
		check(!IsValidRef(ParallelInitializationTask));
		check(bIsPreProcessed);
		...
		// Use either the proxy owned or overridden data
		if (!OutClothSimulDataOverride)
		{
			OutClothSimulDataOverride = &CurrentSimulationData;
		}

		// Cleanup the unused simulation data map on exit so that the render vertex factory won't run unnecessarily
		TBitArray<TInlineAllocator<16>> SimulationGroupIdsToRemove;
		auto FlagForRemovalOnExit = [&SimulationGroupIdsToRemove](const int32 SimulationGroupId, bool bRemove)
			{
				constexpr bool bRemoveDefault = false;  // Only remove owned data
				SimulationGroupIdsToRemove.SetNum(FMath::Max(SimulationGroupIdsToRemove.Num(), SimulationGroupId + 1), bRemoveDefault);
				SimulationGroupIdsToRemove[SimulationGroupId] = bRemove;
			};

		for (const TUniquePtr<FClothingSimulationCloth>& Cloth : Cloths)
		{
			for (int32 SimulationGroupId = Cloth ? Cloth->GetGroupId() : INDEX_NONE;
				SimulationGroupId != INDEX_NONE;
				SimulationGroupId = ClothComponentAdapter.GetNextLinkedSimulationGroupId(SimulationGroupId))
			{
				FlagForRemovalOnExit(SimulationGroupId, true);  // Flag for removal all owned data until it has been successfully copied
			}
		}

		ON_SCOPE_EXIT
			{
				for (TMap<int32, FClothSimulData>::TIterator It = OutClothSimulDataOverride->CreateIterator(); It; ++It)
				{
					const int32 SimulationGroupId = It.Key();
					if (SimulationGroupIdsToRemove.IsValidIndex(SimulationGroupId) && SimulationGroupIdsToRemove[SimulationGroupId])
					{
						It.RemoveCurrent();
					}
				}
		...
		for (const TUniquePtr<FClothingSimulationCloth>& Cloth : Cloths)
		{
			...
			const int32 ReferenceBoneIndex = LeaderPoseComponent ? OwnerComponent.GetLeaderBoneMap()[Cloth->GetReferenceBoneIndex()] : Cloth->GetReferenceBoneIndex();

			// Get the reference bone transform for this cloth
			if (!ComponentSpaceTransforms.IsValidIndex(ReferenceBoneIndex))
			{
				UE_CLOGF(!bHasInvalidReferenceBoneTransforms, LogChaosClothAsset, Warning, "Failed to write back clothing simulation data for component %ls as bone transforms are invalid.",
			...
			const int32 SimulationGroupId = Cloth->GetGroupId();
			FClothSimulData& Data = OutClothSimulDataOverride->FindOrAdd(SimulationGroupId);
			...
			if (bSimulationDataCheckFinite)
			{
				UE_CLOGF(!bSimDataIsFinite, LogChaosClothAsset, Warning, "Failed to write back clothing simulation data for component %ls as simulation contains non-finite values.", *OwnerComponent.GetName());
			...
			if (bSimDataIsFinite)
			{
				// Mark the sim data as valid
				FlagForRemovalOnExit(SimulationGroupId, false);

				// Copy linked data (for when a single cloth asset is used through several Clothing Data in multiple render sections)
				for (int32 NextLinkedSimulationGroupId = ClothComponentAdapter.GetNextLinkedSimulationGroupId(SimulationGroupId);
					NextLinkedSimulationGroupId != INDEX_NONE;
					NextLinkedSimulationGroupId = ClothComponentAdapter.GetNextLinkedSimulationGroupId(NextLinkedSimulationGroupId))
				{
					FClothSimulData& NextData = OutClothSimulDataOverride->FindOrAdd(NextLinkedSimulationGroupId);
					NextData = Data;  // TODO: Deprecate the current map type to use shared pointers to avoid Data duplication if ever possible (the editor warns for shared cloth asset setup to avoid this).

					FlagForRemovalOnExit(NextLinkedSimulationGroupId, false);
				}
			}
		}
	}
	...
	void FClothSimulationProxy::InitializeConfigs()
	{
		...
		Solver->SetSolverLOD(LodIndex);

		if (Configs.Num())
		{
			// Replace physics thread's configs with the game thread's configs
			constexpr bool bForceDirty = false;  // Do not force the copy of the collection if it isn't dirty
			int32 ConfigIndex = 0;
			Configs[ConfigIndex++]->Initialize(ClothComponentAdapter.GetSolverPropertyCollections(), bForceDirty, LodIndex);
			...
			for (const UChaosClothAssetBase* const Asset : ClothComponentAdapter.GetAssets())
			{
				if (Asset)
				{
					for (int32 ModelIndex = 0; ModelIndex < Asset->GetNumClothSimulationModels(); ++ModelIndex)
					{
						...
						{
							Configs[ConfigIndex++]->Initialize(ClothComponentAdapter.GetPropertyCollections(Asset, ModelIndex), bForceDirty, LodIndex);
						}
					}
				}
			}
			ClothComponentAdapter.ClearPropertyCollectionsDirtyFlags(LodIndex);
		}
	}
