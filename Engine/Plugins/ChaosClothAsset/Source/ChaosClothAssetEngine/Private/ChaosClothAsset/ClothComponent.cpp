	...
	, CollisionSources(MakeUnique<UE::Chaos::ClothAsset::FCollisionSources>(this, (bool)bCollideWithEnvironment))
{
	...
	EndTickFunction.TickGroup = TG_LastDemotable;
	EndTickFunction.EndTickGroup = TG_LastDemotable;
	EndTickFunction.bCanEverTick = true;
	EndTickFunction.bStartWithTickEnabled = true;
...
void UChaosClothComponent::ResetConfigProperties()
{
	...
	{
		UE_LOGF(LogChaosClothAsset, Warning, "Chaos Cloth Component [%ls]: Trying to reset runtime config properties without being registered.", *GetName());
	}
}
...
void UChaosClothComponent::RecreateClothSimulationProxy()
{
	...
	{
		UE_LOGF(LogChaosClothAsset, Warning, "Chaos Cloth Component [%ls]: Trying to recreate the simulation proxy without being registered.", *GetName());
	}
}
...
void UChaosClothComponent::PostDuplicate(bool bDuplicateForPIE)
{
...
void UChaosClothComponent::RefreshAsset()
{
#if WITH_EDITORONLY_DATA
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	Asset = GetAsset();
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		{
			SetAsset(Asset);  // Do not remove, ImportText_Direct skips the UPROPERTY Setter
		}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
void UChaosClothComponent::PostEditImport()
{
	Super::PostEditImport();

	// Sync the alias after a duplicate or paste (PostDuplicate is not called on actor components)
	RefreshAsset();
}
...
TStructOnScope<FActorComponentInstanceData> UChaosClothComponent::GetComponentInstanceData() const
{
	return MakeStructOnScope<FActorComponentInstanceData, FChaosClothComponentInstanceData>(this);
}
#endif // WITH_EDITOR
...
void UChaosClothComponent::OnRegister()
{
	...
	// Register the TG_LastDemotable wait tick when neither the in-tick wait nor the EOF-sync fallback is active.
	RegisterEndTick(ShouldUseEndTickWait() && !IsSimulationSuspended() && !ShouldWaitForParallelSimulationInTickComponent());
}

void UChaosClothComponent::SetComponentTickEnabled(bool bEnabled)
{
	Super::SetComponentTickEnabled(bEnabled);

	// Unregister the end tick when the component's tick is being disabled (it will be re-enabled on the next TickComponent / OnRegister once the component ticks again).
	if (!bEnabled)
	{
		RegisterEndTick(false);
	}
}
...
void UChaosClothComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	// Re-evaluate the end-tick registration so p.ClothPhysics.WaitForParallelClothTask and p.ClothPhysics.UseEndTickWait toggles take effect at runtime (no-op when state matches).
	RegisterEndTick(ShouldUseEndTickWait() && !IsSimulationSuspended() && !ShouldWaitForParallelSimulationInTickComponent());
...
{
	if (!ShouldUseEndTickWait() && !IsSimulationSuspended() && !ShouldWaitForParallelSimulationInTickComponent())
	{
		return true;
	}
	if (bBindToLeaderComponent)
...
void UChaosClothComponent::OnPreEndOfFrameSync()
{
	...
	if (!ShouldUseEndTickWait())
	{
		HandleExistingParallelSimulation();
	}
}

bool UChaosClothComponent::ShouldUseEndTickWait() const
{
	// Not cached as a function-local static: FindConsoleVariable can return null if it runs before the engine CVar registers (different module init order), and a static would lock that null in. Defaults to true so the end-tick path stays active when the CVar can't be located.
	if (const IConsoleVariable* const CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("p.ClothPhysics.UseEndTickWait")))
	{
		return CVar->GetBool();
	}
	return true;
...
void FChaosClothComponentEndTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	QUICK_SCOPE_CYCLE_COUNTER(FChaosClothComponentEndTickFunction_ExecuteTick);
	if (Target)
	{
		Target->HandleExistingParallelSimulation();
	}
}

FString FChaosClothComponentEndTickFunction::DiagnosticMessage()
{
	if (Target)
	{
...
FName FChaosClothComponentEndTickFunction::DiagnosticContext(bool bDetailed)
{
	return FName("ChaosClothComponentEndTick");
}

void UChaosClothComponent::RegisterEndTick(bool bRegister)
{
	if (bRegister != EndTickFunction.IsTickFunctionRegistered())
	{
		if (bRegister)
		{
			if (SetupActorComponentTickFunction(&EndTickFunction))
			{
				EndTickFunction.Target = this;
				// Cloth simulation is dispatched from TickComponent, so PrimaryComponentTick is the correct prerequisite. USkeletalMeshComponent has a separate ClothTickFunction and pairs its end tick with that one.
				EndTickFunction.AddPrerequisite(this, PrimaryComponentTick);
			}
		}
		else
		{
...
void UChaosClothComponent::GetAdditionalRequiredBonesForLeader(int32 LeaderLODIndex, TArray<FBoneIndexType>& InOutRequiredBones) const
{
	...
	if (const FSkeletalMeshRenderData* const SkeletalMeshRenderData = GetSkeletalMeshRenderData())
	{
		...
		if (SkeletalMeshRenderData->LODRenderData.IsValidIndex(LODIndex))
		{
			...
			{
				if (GetLeaderBoneMap().IsValidIndex(RequiredBone))
				{
					const int32 FollowerRequiredLeaderBone = GetLeaderBoneMap()[RequiredBone];
...
void UChaosClothComponent::SetSimulateInEditor(const bool bInSimulateInEditor)
{
#if WITH_EDITOR
	bSimulateInEditor = bInSimulateInEditor;
	// Editor-only tick gate must follow bSimulateInEditor so the simulation actually runs when toggled on.
	bTickInEditor = bInSimulateInEditor;
#endif
}
...
void UChaosClothComponent::ClearPropertyCollectionsDirtyFlags(int32 LODIndex) const
{
	using namespace ::Chaos::Softs;
	for (int32 ModelIndex = 0; ModelIndex < ClothSimulationProperties.Num(); ++ModelIndex)
	{
		const TArray<TSharedPtr<FCollectionPropertyFacade>>& CollectionPropertyFacades = ClothSimulationProperties[ModelIndex].CollectionPropertyFacades;
		if (CollectionPropertyFacades.IsValidIndex(LODIndex) && CollectionPropertyFacades[LODIndex].IsValid())
		{
			CollectionPropertyFacades[LODIndex]->ClearDirtyFlags();
		}
	}
}
...
void UChaosClothComponent::WriteToSimulation(const float DeltaTime, const bool bAsyncTask)
{
	...
	if (ClothSimulationProxy.IsValid())
	{
		ClothSimulationProxy->PreSimulate_GameThread(DeltaTime);
	}
}
