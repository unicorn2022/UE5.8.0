	...
	for (int32 SectionIndex = 0; SectionIndex < NumSections; ++SectionIndex)
	{
			...
			&ClothFacade, MaterialOffset, &ClothAsset, &RecomputeTangent, &MeshUtilities, bIsValidSourceMesh, &SourceMesh, &SourceMeshMutex, TargetPlatform, LodIndex, MaxDistances]() 
		{
			...
			if (!ClothAsset.GetMaterials().IsValidIndex(MaterialIndex))
			{
				...
				UE_LOGF(LogChaosClothAsset, Warning, "Cloth Asset BuildLod: Pattern %d in LOD %d has invalid material index. Material index is %d, asset has %d materials. Expected material path for this pattern: %ls",
