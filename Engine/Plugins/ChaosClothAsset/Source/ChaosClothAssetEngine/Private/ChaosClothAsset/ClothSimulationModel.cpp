...
bool FChaosClothSimulationLodModel::Serialize(FArchive& Ar)
{
	...
	if (bCooked && Ar.IsSaving())
	{
		...
		for (const FName Key : WeightMapsToRemove)
		{
			...
			UE_LOGF(LogChaosClothAsset, Display, "TrimOnCook %ls WeightMap[%ls] [%llu -> 0, saved %llu bytes]",
				*AssetPath, *Key.ToString(), Size, Size);
		}
		...
		for (const FName Key : VertexSetsToRemove)
		{
			...
			UE_LOGF(LogChaosClothAsset, Display, "TrimOnCook %ls VertexSet[%ls] [%llu -> 0, saved %llu bytes]",
				*AssetPath, *Key.ToString(), Size, Size);
		}
		...
		for (const FName Key : FaceSetsToRemove)
		{
			...
			const uint64 Size = (uint64)Value.GetAllocatedSize();
			FaceSets.Remove(Key);
			UE_LOGF(LogChaosClothAsset, Display, "TrimOnCook %ls FaceSet[%ls] [%llu -> 0, saved %llu bytes]",
				*AssetPath, *Key.ToString(), Size, Size);
		}
	}
#endif
...
int32 FChaosClothSimulationLodModel::CalcMaxBoneInfluences() const
{
	int32 MaxBoneInfluences = 0;
	for (const FClothVertBoneData& ClothVertBoneData : BoneData)
	{
		MaxBoneInfluences = FMath::Max(MaxBoneInfluences, ClothVertBoneData.NumInfluences);
	}
	...
	TArray<FChaosClothAssetLodTransitionDataCache>* InOutTransitionCache)
{
	...
	for (int32 LodIndex = 0; LodIndex < NumLods; ++LodIndex)
	{
		...
		const bool bCopyAllUserAttributes = Cloth.GetCopyAllUserAttributesToSimModel();

		// Copy weight maps
		LodModel.WeightMaps.Reserve(WeightMapNames.Num());
		for (const FName& WeightMapName : WeightMapNames)
		{
			// Keep all weight maps to allow runtime switching. This is legacy behavior even regardless of value of bCopyAllUserAttributes
		...
		{
			if (bCopyAllUserAttributes || IsNameUsedInProperties(SelectionName) || IsNameReserved(SelectionName))
			{
		...
		{
			if (bCopyAllUserAttributes || IsNameUsedInProperties(FaceIntMapName) || IsNameReserved(FaceIntMapName))
			{
