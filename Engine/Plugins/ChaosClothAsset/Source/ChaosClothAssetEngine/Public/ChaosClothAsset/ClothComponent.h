...
/** TG_LastDemotable wait on the parallel cloth task. Mirror of FSkeletalMeshComponentClothEndTickFunction. */
USTRUCT()
struct FChaosClothComponentEndTickFunction : public FTickFunction
{
	GENERATED_USTRUCT_BODY()

	UChaosClothComponent* Target = nullptr;
...
template<>
struct TStructOpsTypeTraits<FChaosClothComponentEndTickFunction> : public TStructOpsTypeTraitsBase2<FChaosClothComponentEndTickFunction>
{
	enum
	{
		WithCopy = false
...
public:
	...
	*/
	...
	UE_API void SetSimulateInEditor(const bool bInSimulateInEditor);

#if WITH_EDITORONLY_DATA
	/** Return whether the cloth is set to simulate in the editor. */
	bool GetSimulateInEditor() const { return bSimulateInEditor; }
#endif
...
protected:
	...
	UE_API virtual void ClearPropertyCollectionsDirtyFlags(int32 LODIndex) const override;
...
private:
	...
	bool ShouldUseEndTickWait() const;
	UE_API void UpdateComponentSpaceTransforms();
	UE_API void UpdateVisibility();
	UE_API void UpdateClothTeleport();

	/** Sync the editor-only Asset alias from the underlying SkinnedAsset. */
	void RefreshAsset();

	/** Register/unregister the TG_LastDemotable wait tick. Drives registration from the in-tick wait CVar gate. */
	void RegisterEndTick(bool bRegister);

	/** Tick function that waits on the parallel cloth simulation task in TG_LastDemotable. */
	FChaosClothComponentEndTickFunction EndTickFunction;

	friend struct FChaosClothComponentEndTickFunction;
	friend UE::Chaos::ClothAsset::FClothComponentCacheAdapter;
	friend struct FChaosClothComponentInstanceData;
	...
	*/
	...
	UPROPERTY(EditInstanceOnly, Transient, Setter = SetSimulateInEditor, Getter = GetSimulateInEditor, Category = ClothComponent)
