...
public:
	...
	UE_API virtual void PrepareMeshModel() override;
#endif
	 ...
	 * @param InClothCollections The source managed array collections to set to the asset.
	 * @param bReregisterComponents Whether to reregister all components using this asset. This is the safe way to hard reset all simulations.
	 */
	UE_API void SetClothCollections(TArray<TSharedRef<const FManagedArrayCollection>>&& InClothCollections, bool bReregisterComponents = false);

	/**
	 * Rebuild this asset from the specified managed array collection data.
	 * @param InClothCollections The source managed array collections to build/rebuild the asset with.
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	UE_API virtual UChaosClothAssetBase* CreatePreviewAssetCopy(UObject* Outer, EObjectFlags Flags) const override;
#endif
	UE_DEPRECATED(5.6, "SetReferenceSkeleton is deprecated. Skeletons must only been set through the cloth collections.")
...
private:
	...
	virtual void ExecutePostLoadAssetImpl(FSkinnedAssetPostLoadContext& Context) override;
#endif
	 ...
	 */
	UPROPERTY(EditAnywhere, Setter = SetSkeleton, Category = Skeleton, Meta = (EditCondition = "!HasDataflow()"))
	TObjectPtr<USkeleton> Skeleton;

	/** Physics asset used for collision. Set by the Dataflow evaluation. */
	UPROPERTY(EditAnywhere, Category = Collision, Meta = (EditCondition = "!HasDataflow()"))
	TObjectPtr<UPhysicsAsset> PhysicsAsset;

	/** Whether to blend positions between the skinned/simulated transitions of the cloth render mesh. Only used when not overridden in Dataflow by the ProxyDeformer node. */
	UE_DEPRECATED(5.4, "Superseded by the ProxyDeformer node.")
	UPROPERTY()
	bool bSmoothTransition_DEPRECATED = true;

	/** Whether to use multiple triangle influences on the proxy wrap deformer to help smoothe deformations. Only used when not overridden in Dataflow by the ProxyDeformer node. */
	UE_DEPRECATED(5.4, "Superseded by the ProxyDeformer node.")
	UPROPERTY()
	bool bUseMultipleInfluences_DEPRECATED = false;

	/** The radius from which to get the multiple triangle influences from the simulated proxy mesh. Only used when not overridden in Dataflow with the ProxyDeformer node. */
	UE_DEPRECATED(5.4, "Superseded by the ProxyDeformer node.")
	UPROPERTY()
