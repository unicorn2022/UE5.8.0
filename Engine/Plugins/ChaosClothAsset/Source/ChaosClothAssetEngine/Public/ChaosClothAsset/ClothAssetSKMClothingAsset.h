...
public:
	...
	const TSharedPtr<const FChaosClothSimulationModel>& GetTransientSimulationModel() const;
...
private:
	...
	virtual void UnbindFromSkeletalMesh(USkeletalMesh* SkeletalMesh, const int32 MeshLodIndexx, const int32 SectionIndex) override;
	virtual void UpdateAllLODBiasMappings(USkeletalMesh* SkeletalMesh) override;
	virtual void GetSimulationMesh(const int32 InMeshLodIndex, TArray<FVector3f>& OutPositions, TArray<uint32>& OutIndices, TArray<float>& OutMaxDistances) const override;
	virtual TArray<TPair<FText, FText>> GetStats() const override;
	virtual USkinnedAsset* GetSkinnedAssetDependency() const override;
	...
	TSharedPtr<const FChaosClothSimulationModel> TransientSimulationModel;
