...
namespace UE::Chaos::ClothAsset
{
	...
	public:
		...
		/**
		 * Write data back onto the game thread once the simulation is done.
		 * For specialized usage when not calling Tick_GameThread.
		 * @param OutClothSimulDataOverride An optional override to store the resulting simulation data, or nullptr to use the internally owned map insted.
		 */
		UE_API void PostSimulate_GameThread(TMap<int32, FClothSimulData>* OutClothSimulDataOverride = nullptr);
		/**
		 * Post setup, required for when the simulation didn't run.
		 * For specialized usage when not calling Tick_GameThread.
		 * @param OutClothSimulDataOverride An optional override to store the resulting simulation data, or nullptr to use the internally owned map insted.
		 */
		UE_API void PostProcess_GameThread(TMap<int32, FClothSimulData>* OutClothSimulDataOverride = nullptr);
	...
	protected:
		...
		UE_API void Tick();
		UE_API void WriteSimulationData(TMap<int32, FClothSimulData>* OutClothSimulDataOverride = nullptr);
