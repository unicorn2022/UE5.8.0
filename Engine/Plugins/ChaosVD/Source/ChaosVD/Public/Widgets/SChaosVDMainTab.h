...
class SButton;
class FChaosVDRecordingControls;
class SDockTab;
class FChaosVDScene;
...
public:
	...
	/** Connects to a live session using via the Unreal Trace Server and its Trace Store. This is a legacy option */
	bool ConnectToLiveSession(int32 SessionID, const FString& InSessionAddress, EChaosVDLoadRecordedDataMode LoadingMode) const;

	UE_DEPRECATED(5.8, "Use the version that takes a Remote session ID")
	bool ConnectToLiveSession_Direct(EChaosVDLoadRecordedDataMode LoadingMode) const;

	bool ConnectToLiveSession_Direct(FGuid RemoteSessionID, EChaosVDLoadRecordedDataMode LoadingMode) const;
	...
	TSharedPtr<FChaosVDRecordingControls> GetRecordingControls()
	{
		return RecordingControls;
	}

	CHAOSVD_API virtual void AddViewportOverlayWidget(TSharedRef<SWidget> InOverlaidWidget, int32 ZOrder, TSharedPtr<IAssetViewport> InViewport = nullptr) override;

private:

	void ExtendMainToolbar();
	void ExtendStatusToolbar();
	...
	void GenerateRecentFilesMenu(FMenuBuilder& MenuBuilder, UToolMenu* Menu);
	...
	void HandleDisconnectSessionClicked();
	...
	TSharedPtr<FChaosVDRecordingControls> RecordingControls;
