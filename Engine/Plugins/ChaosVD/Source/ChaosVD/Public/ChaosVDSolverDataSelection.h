 ...
 * Entries may optionally carry a group name; entries sharing the same name are displayed under
 * a single collapsible category with that name. Entries with no group go into "Recorded Data".
 */
USTRUCT()
struct FChaosVDSelectionMultipleView
{
	...
	void AddData(StructType* Struct, FName GroupName = NAME_None);

	void AddData(TSharedPtr<FStructOnScope> StructOnScope, FName GroupName = NAME_None)
	{
		DataInstances.Add({ StructOnScope, GroupName });
	}
...
protected:
	TArray<FDataEntry> DataInstances;

	friend class FChaosVDSelectionMultipleViewCustomization;
};

template <typename StructType>
void FChaosVDSelectionMultipleView::AddData(StructType* Struct, FName GroupName)
{
	if (!Struct)
	{
		return;
	...
	DataInstances.Add({ MakeShared<FStructOnScope>(StructType::StaticStruct(), reinterpret_cast<uint8*>(Struct)), GroupName });
}
