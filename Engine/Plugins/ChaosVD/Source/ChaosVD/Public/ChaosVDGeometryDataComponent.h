...
namespace Chaos::VD
{
	...
	{
	public:
		Chaos::EImplicitObjectType ImplicitObjectType;
		FChaosVDImplicitObjectBasicView ImplicitObjectInfo;

		void AddChild(TSharedPtr<FChaosVDImplicitObjectView> Child)
		{
			Children->Add(Child);
		}
		...
		void SetMeshData(TSharedPtr<FChaosVDInstancedMeshData> InMeshData)
		{
			MeshData = InMeshData;
		}
	...
	private:
		TSharedPtr<TArray<TSharedPtr<FChaosVDImplicitObjectView>>> Children = MakeShared<TArray<TSharedPtr<FChaosVDImplicitObjectView>>>();
		TSharedPtr<FChaosVDInstancedMeshData> MeshData;
	};
}
