...
public:
	 ...
	 *	@param OutMeshDataHandles Array containing all the generated components paired with their corresponding implicit objects
	 *  @param Root ImplicitObjectView to attach children to
...
private:
	...
	void CreateMeshesFromImplicit_Internal(const Chaos::FImplicitObject* InRootImplicitObject,const Chaos::FImplicitObject* InLeafImplicitObject, TArray<TPair<TSharedPtr<FChaosVDExtractedGeometryDataHandle>, TSharedPtr<Chaos::VD::FChaosVDImplicitObjectView>>>& OutMeshDataHandles, Chaos::VD::FChaosVDImplicitObjectView* Root, const int32 DesiredLODCount = 0, const Chaos::FRigidTransform3& InTransform = Chaos::FRigidTransform3(), const int32 ParentShapeInstanceIndex = 0, int32 AvailableShapeDataNum = 0);
...
bool FChaosVDGeometryBuilder::InitializeMeshComponent(AActor* Owner, ComponentType* MeshComponent)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Failed To Create mesh component | Component Is Null. ", ANSI_TO_TCHAR(__FUNCTION__));
		return false;
	}
	...
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Failed To Register Component | Owner Is Null. ", ANSI_TO_TCHAR(__FUNCTION__));
		return false;
	}
