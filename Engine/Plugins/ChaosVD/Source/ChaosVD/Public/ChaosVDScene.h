...
/** Fired by FChaosVDSceneParticleCustomization when building details for a selected particle.
 *  Subscribers (e.g. UChaosVDParticleExtraDataComponent) can append additional categories and
 *  struct entries to the details panel via the provided IDetailLayoutBuilder. */
DECLARE_MULTICAST_DELEGATE_ThreeParams(FChaosVDParticleDetailsExtraDataDelegate, int32 /*SolverID*/, int32 /*ParticleID*/, IDetailLayoutBuilder& /*DetailBuilder*/)

/** Lightweight delegate broadcast on every frame change for an already-selected particle.
 *  Subscribers update their cached struct scope memory in-place; they set bOutNeedsFullRebuild=true
 *  if the data layout has changed and a full IDetailLayoutBuilder refresh is required. */
DECLARE_MULTICAST_DELEGATE_ThreeParams(FChaosVDParticleDetailsExtraDataRefreshDelegate, int32 /*SolverID*/, int32 /*ParticleID*/, bool& /*bOutNeedsFullRebuild*/)
...
public:
	...
	/** Fired when the particle details panel is being built for a selected particle.
	 *  Subscribers can append additional categories/structs to the panel via IDetailLayoutBuilder. */
	FChaosVDParticleDetailsExtraDataDelegate& OnParticleDetailsExtraDataRequested()
	{
		return ParticleDetailsExtraDataDelegate;
	}

	/** Fired on every frame change while a particle remains selected.
	 *  Subscribers should update their cached struct scope memory in-place (no layout rebuild). */
	FChaosVDParticleDetailsExtraDataRefreshDelegate& OnParticleDetailsExtraDataRefreshRequested()
	{
		return ParticleDetailsExtraDataRefreshDelegate;
	}
	...
	CHAOSVD_API const Chaos::FImplicitObject* UnpackGeometry(const Chaos::FImplicitObject* InImplicitObject, Chaos::FRigidTransform3& InOutTransform) const;
	CHAOSVD_API bool DoesGeometryNeedUnpacking(const Chaos::FImplicitObject* InImplicitObject) const;
	...
	CHAOSVD_API void SetSelected(const FTypedElementHandle& InElementHandle);
	 ...
	 */
	CHAOSVD_API TSharedPtr<FChaosVDSceneParticle> GetParticleInstance(int32 SolverID, int32 ParticleID);
	 ...
	 */
	CHAOSVD_API AChaosVDSolverInfoActor* GetSolverInfoActor(int32 SolverID);
...
private:
	...
	FChaosVDParticleDetailsExtraDataDelegate ParticleDetailsExtraDataDelegate;
	FChaosVDParticleDetailsExtraDataRefreshDelegate ParticleDetailsExtraDataRefreshDelegate;
