...
private:
	...
	/** Shows a Slate dialog listing data types skipped because Safe Loading Mode was active. */
	void ShowSafeModeSkippedDialog();

	/** Shows a Slate dialog warning that non-backwards-compatible types were loaded with Safe Loading Mode disabled. */
	void ShowUnsafeDataDialog();

	/** Shows a Slate dialog warning that natively-serialized particle extra data entries are present,
	 *  and lets the user choose whether to display them or skip them. */
	void ShowNativeSerializationDialog();

	/** Shows a Slate dialog informing the user that natively-serialized particle extra data entries
	 *  were stripped because Particle Extra Data Loading Mode is set to SerializeBin Only. */
	void ShowSerializeBinOnlyStrippedDialog();
...
public:
	...
	/** Types whose data was skipped entirely because Safe Loading Mode was enabled and they are not backwards compatible. */
	const TSet<FString>& GetTypesSkippedInSafeMode() const
	{
		return TypesSkippedInSafeMode;
	}

	/** Types that were processed despite not being backwards compatible (Safe Loading Mode was off). */
	const TSet<FString>& GetUnsafeTypesProcessed() const
	{
		return UnsafeTypesProcessed;
	}
	...
	void CreateGeometryArchiveContext();

	TSharedPtr<Chaos::FChaosArchiveContext> GetGeometryArchiveContext()
	{
		return GeometryArchiveContext;
	}
...
private:
	...
	TSet<FString> TypesSkippedInSafeMode;
	TSet<FString> UnsafeTypesProcessed;
	...
	TSharedPtr<Chaos::FChaosArchiveContext> GeometryArchiveContext;
