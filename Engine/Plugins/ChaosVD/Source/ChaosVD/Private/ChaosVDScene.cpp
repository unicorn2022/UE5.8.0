...
void FChaosVDScene::UpdateFromRecordedSolverStageData(const int32 SolverID, const FChaosVDFrameStageData& InRecordedStepData, const FChaosVDSolverFrameData& InFrameData)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Attempted to playback a solver frame from an invalid solver container", ANSI_TO_TCHAR(__FUNCTION__));	
	}
...
void FChaosVDScene::HandleEnterNewGameFrame(int32 FrameNumber, const TArray<int32, TInlineAllocator<16>>& AvailableSolversIds, const FChaosVDGameFrameData& InNewGameFrameData, TArray<int32, TInlineAllocator<16>>& OutRemovedSolversIds)
{
	...
	for (int32 SolverID : AvailableSolversIds)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Error, "[%s] Failed to create solver data actor for id [%d]", __func__, SolverID);
		}
	}
	...
	for (TMap<int32, AChaosVDSolverInfoActor*>::TIterator RemoveIterator = SolverDataContainerBySolverID.CreateIterator(); RemoveIterator; ++RemoveIterator)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Log, "[%ls] Removing Solver [%d] as it is no longer present in the recording", ANSI_TO_TCHAR(__FUNCTION__), RemoveIterator.Key());
...
Chaos::FConstImplicitObjectPtr FChaosVDScene::GetUpdatedGeometry(int32 GeometryID) const
{
	if (ensure(LoadedRecording.IsValid()))
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Warning, "Geometry for key [%d] is not loaded in the recording yet", GeometryID);
		}
	}
...
const Chaos::FImplicitObject* FChaosVDScene::UnpackGeometry(const Chaos::FImplicitObject* InImplicitObject, Chaos::FRigidTransform3& InOutTransform) const
{
	return GeometryGenerator->UnpackImplicitObject(InImplicitObject, InOutTransform);
}

bool FChaosVDScene::DoesGeometryNeedUnpacking(const Chaos::FImplicitObject* InImplicitObject) const
{
	return GeometryGenerator->ImplicitObjectNeedsUnpacking(InImplicitObject);
}
