...
namespace Chaos::VisualDebugger::CombinedTraceFile
{
	...
	bool CombineFiles(const TArray<TUniquePtr<IFileHandle>>& InFileHandlesToCombine, const FString& InCombinedFilePathName)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Error, "Failed to combine files because an empty file handles array was provided.");
			return false;
		}
		...
		{
			UE_LOGF(LogChaosVDEditor, Error, "Failed to combine files because an the target file name already exists.");
			return false;
		}
		...
		if (!FileWriter)
		{
			UE_LOGF(LogChaosVDEditor, Error, "Failed to combine files because there was an error while creating the target file.")
			return false;
		}
	...
	TArray<TUniquePtr<IFileHandle>> GetInnerFileHandles(const FString& InContainerFilePath)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Error, "Failed to read combined trace file. There was an error trying to open the file for read | File path [%ls].", *InContainerFilePath);
			return FileHandles;
		}
		...
		{
			UE_LOGF(LogChaosVDEditor, Error, "Failed to read combined trace file. The serialized file table header is not valid | File path [%ls].", *InContainerFilePath);
			return FileHandles;
		}
		...
		for (const FFileEntry& FileEntry : FileTable.Files)
		{
			...
			{
				UE_LOGF(LogChaosVDEditor, Error, "Failed to create a inner file handle for. there was an error trying to open the file for read | Combined file path [%ls].", *InContainerFilePath);
			}
		}
