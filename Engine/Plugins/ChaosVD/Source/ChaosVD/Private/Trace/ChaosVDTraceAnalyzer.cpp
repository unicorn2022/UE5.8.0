...
void FChaosVDTraceAnalyzer::OnAnalysisBegin(const FOnAnalysisContext& Context)
{
	...
	Builder.RouteEvent(RouteId_ChaosVDMiscHeaderData, "ChaosVDLogger", "ChaosVDMiscHeaderData");
...
bool FChaosVDTraceAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	case RouteId_ChaosVDBinaryDataEnd:
		{
			...
			if (!ChaosVDTraceProvider->ProcessBinaryData(DataID))
			{
				...
				UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] FailedToProcess Binary Data with ID [%d]", ANSI_TO_TCHAR(__FUNCTION__), DataID);
			}
	...
	case RouteId_ChaosVDUsesAutoRTFM:
		{
			if (EventData.GetValue<bool>("bUsingAutoRTFM"))
			{
				UE_LOGF(LogChaosVDEditor, Log, "Processing recording was made with AutoRTFM enabled. ");
			}

			break;
		}
	case RouteId_ChaosVDMiscHeaderData:
		{
			if (EventData.GetValue<bool>("bUsesSharedGeometryContextArchive"))
			{
				ChaosVDTraceProvider->CreateGeometryArchiveContext();
			}
			break;
		}
	default:
