...
bool FChaosVDArchiveHeaderProcessor::ProcessRawData(const TArray<uint8>& InData)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Processed an archive header while the recording already had data loaded. That initially loaded data used the default header and serialization errors might have occured | This should not happen...", ANSI_TO_TCHAR(__FUNCTION__));
	}
