...
void UChaosVDInstancedStaticMeshComponent::UpdateVisibilityForInstance(const TSharedRef<FChaosVDInstancedMeshData>& InInstanceHandle)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Attempted to update a mesh instance using a handle from another component. No instances were updated | Handle Component [%ls] | Current Component [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *GetNameSafe(InInstanceHandle->GetMeshComponent()), *GetNameSafe(this));
		return;
	}
...
void UChaosVDInstancedStaticMeshComponent::UpdateColorForInstance(const TSharedRef<FChaosVDInstancedMeshData>& InInstanceHandle)
{
	...
	if (bIsSolidColor)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Desired Color is not supported in this mesh component [%ls]...", ANSI_TO_TCHAR(__FUNCTION__), *NewColor.ToString());
		}
	}
	else
	{
...
void UChaosVDInstancedStaticMeshComponent::Initialize()
{
	...
	if (ensure(GeometryBuilder))
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Error, "[%ls] | Component [%ls] already had a material, which is not expected!. It likely it was modified after being disposed.", ANSI_TO_TCHAR(__FUNCTION__), *GetNameSafe(this));
...
bool UChaosVDInstancedStaticMeshComponent::UpdateGeometryKey(const uint32 NewHandleGeometryKey)
{
	if (CurrentGeometryKey != 0 && CurrentGeometryKey != NewHandleGeometryKey)
	{
		...
		UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Attempted to add a mesh instance belonging to another geometry key. No instance was added | CurrentKey [%u] | New Key [%u]", ANSI_TO_TCHAR(__FUNCTION__), CurrentGeometryKey, NewHandleGeometryKey);
		return false;
	}
	else
	{
...
void UChaosVDInstancedStaticMeshComponent::RemoveMeshInstance(const TSharedRef<FChaosVDInstancedMeshData>& InHandleToRemove, ERemovalMode Mode)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Attempted to remove a mesh instace using a handle from another component. No instanced were removed | Handle Component [%ls] | Current Component [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *GetNameSafe(InHandleToRemove->GetMeshComponent()), *GetNameSafe(this));
		return;
	}
	...
	else
	{
		...
		
		{
			...
			{
				UE_LOGF(LogChaosVDEditor, Error, "[%ls] Mesh Data Handle has an invalid instance index. No instanced were removed | Handle Instance Index [%d] | Current Instance Count [%d]", ANSI_TO_TCHAR(__FUNCTION__), InHandleToRemove->GetMeshInstanceIndex(), CurrentInstanceCount);
				return;
			}
		}
...
void UChaosVDInstancedStaticMeshComponent::UpdateInstanceHandle(int32 OldIndex, int32 NewIndex)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Trying to update an instance from another component | Handle Component [%ls] | This component [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *GetNameSafe(RelocatedHandle->GetMeshComponent()), *GetNameSafe(this));
	}
	...
	if (bIsMeshReady)
	{
		...
		for (TMap<TSharedPtr<FChaosVDInstancedMeshData>, EChaosVDMeshInstanceOperationsFlags>::TIterator HandleOperationsIterator = PendingOperationsByInstance.CreateIterator(); HandleOperationsIterator; ++HandleOperationsIterator)
		{
			...
			if (CanExecuteOperation(EChaosVDMeshInstanceOperationsFlags::SelectionUpdate, OperationHandle.Value))
			{
				...
				if (!ensure(IsValidInstance(InstanceIndex)))
				{
					UE_LOGF(LogChaosVDEditor, Error, "[%ls] Attempted to update a mesh instance using a handle with an invalid instance index | Handle Instance Index [%d] | Current Instance Conut [%d]", ANSI_TO_TCHAR(__FUNCTION__), InstanceIndex, GetInstanceCount());
			...
			if (CanExecuteOperation(EChaosVDMeshInstanceOperationsFlags::Remove, OperationHandle.Value))
			{
				...
				if (!ensure(IsValidInstance(InstanceIndex)))
				{
					UE_LOGF(LogChaosVDEditor, Error, "[%ls] Mesh Data Handle has an invalid instance index. No instanced were removed | Handle Instance Index [%d] | Current Instance Count [%d]", ANSI_TO_TCHAR(__FUNCTION__), OperationHandle.Key->GetMeshInstanceIndex(), CurrentInstanceCount);
				}
				else
				{
					RemoveInstance(InstanceIndex);
