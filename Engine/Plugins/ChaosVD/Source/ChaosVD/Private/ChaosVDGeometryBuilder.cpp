...
namespace Chaos::VisualDebugger
{
	...
	void HandleTriangleAddedToDynamicMesh(const UE::Geometry::FMeshShapeGenerator& Generator, FDynamicMesh3& OutDynamicMesh, int32 TriangleIDResult, int32 GroupID, int32 GeneratorTriangleIndex, int32& OutSkippedTriangles, bool bAttemptToFixNoManifoldError = true)
	{
		...
		if (TriangleIDResult == FDynamicMesh3::NonManifoldID && bAttemptToFixNoManifoldError)
		{
			...
			UE_LOGF(LogChaosVDEditor, Verbose, "Failed to add triangle | [%d] but expected [%d] | Attempting to fix it ... Repaired triangle ID [%d]", TriangleIDResult, GeneratorTriangleIndex, RepairedTriangleID);
		...
		{
			OutSkippedTriangles++;
			UE_LOGF(LogTemp, Verbose, "Failed to add triangle | [%d] but expected [%d] | Ignoring Duplicated triangle.", TriangleIDResult, GeneratorTriangleIndex);
			return;
		}

		OutSkippedTriangles++;
		UE_LOGF(LogTemp, Error, "Failed to add triangle | [%d] but expected [%d]. This geometry will have missing triangles.", TriangleIDResult, GeneratorTriangleIndex);
	...
	void GenerateDynamicMeshFromGenerator(const UE::Geometry::FMeshShapeGenerator& Generator, FDynamicMesh3& OutDynamicMesh)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Attempted to created a mesh using a generator without attributes. CVD Meshes requiere attributes, this should have not happened.", ANSI_TO_TCHAR(__FUNCTION__));
			return;
		}
...
void FChaosVDGeometryBuilder::Initialize(const TWeakPtr<FChaosVDScene>& ChaosVDScene)
{
	...
	auto ProcessMeshComponent = [WeakThis = AsWeak()](uint32 GeometryKey, const TWeakObjectPtr<UMeshComponent> Object)
	{
		...
		if (!GeometryBuilder)
		{
			UE_LOGF(LogChaosVDEditor, Verbose, " [%ls] Failed to update mesh for Handle | Geometry Key [%u] | Geometry Builder is invalid", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
	...
	auto UpdateMeshMaterialForComponent = [WeakThis = AsWeak()](const TWeakObjectPtr<UMeshComponent> Object)
	{
		...
		if (!GeometryBuilder)
		{
			UE_LOGF(LogChaosVDEditor, Verbose, " [%ls] Failed to Create Material for Mesh | Geometry builder is no longer valid ", ANSI_TO_TCHAR(__FUNCTION__));
...
void FChaosVDGeometryBuilder::DeInitialize()
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Failed to cancel [%d] tasks", ANSI_TO_TCHAR(__FUNCTION__), TasksFailedToCancelNum);
	}
...
void FChaosVDGeometryBuilder::CreateMeshesFromImplicitObject(const Chaos::FImplicitObject* InImplicitObject, TArray<TPair<TSharedPtr<FChaosVDExtractedGeometryDataHandle>, TSharedPtr<Chaos::VD::FChaosVDImplicitObjectView>>>& OutMeshDataHandles, Chaos::VD::FChaosVDImplicitObjectView* Root, int32 AvailableShapeDataNum , const int32 DesiredLODCount, const Chaos::FRigidTransform3& InTransform, const int32 MeshIndex)
{
	...
	CreateMeshesFromImplicit_Internal(InImplicitObject, InImplicitObject, OutMeshDataHandles, Root, DesiredLODCount, InTransform, MeshIndex, AvailableShapeDataNum);
}
...
void FChaosVDGeometryBuilder::CreateMeshesFromImplicit_Internal(const Chaos::FImplicitObject* InRootImplicitObject, const Chaos::FImplicitObject* InLeafImplicitObject, TArray<TPair<TSharedPtr<FChaosVDExtractedGeometryDataHandle>, TSharedPtr<Chaos::VD::FChaosVDImplicitObjectView>>>& OutMeshDataHandles, Chaos::VD::FChaosVDImplicitObjectView* Root, const int32 DesiredLODCount, const Chaos::FRigidTransform3& InTransform, const int32 ParentShapeInstanceIndex, int32 AvailableShapeDataNum)
{
	...
	TSharedPtr<Chaos::VD::FChaosVDImplicitObjectView> ImplicitView = MakeShared<Chaos::VD::FChaosVDImplicitObjectView>();
	ImplicitView->ImplicitObjectType = InnerType;

	Root->AddChild(ImplicitView);
	...
	if (InnerType == ImplicitObjectType::Union || InnerType == ImplicitObjectType::UnionClustered)
	{
		if (const FImplicitObjectUnion* Union = InLeafImplicitObject->template AsA<FImplicitObjectUnion>())
		{
			...
			for (int32 ObjectIndex = 0; ObjectIndex < Union->GetObjects().Num(); ++ObjectIndex)
			{
				...
				CreateMeshesFromImplicit_Internal(InRootImplicitObject, UnionImplicit.GetReference(), OutMeshDataHandles, ImplicitView.Get(), DesiredLODCount, InTransform, CurrentShapeInstanceIndex, AvailableShapeDataNum);
			}
		}
	...
	if (InnerType == ImplicitObjectType::Transformed)
	{
		if (const TImplicitObjectTransformed<FReal, 3>* Transformed = InLeafImplicitObject->template GetObject<TImplicitObjectTransformed<FReal, 3>>())
		{
			...
			CreateMeshesFromImplicit_Internal(InRootImplicitObject, Transformed->GetTransformedObject(), OutMeshDataHandles, ImplicitView.Get(), DesiredLODCount, Transformed->GetTransform(), ParentShapeInstanceIndex, AvailableShapeDataNum);
		}
	...
	if (const TSharedPtr<FChaosVDExtractedGeometryDataHandle> MeshDataHandle = ExtractGeometryDataForImplicit(InLeafImplicitObject, InTransform))
	{
		...
		OutMeshDataHandles.Add(TPair<TSharedPtr<FChaosVDExtractedGeometryDataHandle>, TSharedPtr<Chaos::VD::FChaosVDImplicitObjectView>>(MeshDataHandle, ImplicitView));
	}
}
...
bool FChaosVDGeometryBuilder::ApplyMeshToComponentFromKey(TWeakObjectPtr<UMeshComponent> MeshComponent, const uint32 GeometryKey)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Failed to apply geometry with key [%d] | Mesh Component is invalid", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
	...
	if (!DataComponent)
	{
		...
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Failed to apply geometry with key [%d] | Mesh component is not a ChaosVDGeometryDataComponent", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
	...
	{
		UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] Failed to apply geometry with key [%u] | Geometry was not ready", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
	}
...
TSharedPtr<FChaosVDExtractedGeometryDataHandle> FChaosVDGeometryBuilder::ExtractGeometryDataForImplicit(const Chaos::FImplicitObject* InImplicitObject, const Chaos::FRigidTransform3& InTransform)
{
	...
	if (const Chaos::FImplicitObject* ImplicitObjectToProcess = bNeedsUnpack ? UnpackImplicitObject(InImplicitObject, ExtractedTransform) : InImplicitObject)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogChaosVDEditor, Verbose, "Geometry key [%u] | Implicit Object Type [%ls]", GeometryKey, *Chaos::GetImplicitObjectTypeName(ImplicitObjectType).ToString())
			}
...
void FChaosVDGeometryBuilder::AddMeshComponentWaitingForGeometry(uint32 GeometryKey, TWeakObjectPtr<UMeshComponent> MeshComponent) const
{
	if (!MeshComponent.IsValid())
	{
		UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] Failed to add mesh component update for geometry key [%d] | Mesh component is invalid", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
		return;
	}

	if (!ensure(MeshComponentsWaitingForGeometry.IsValid()))
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Failed to add mesh component update for geometry key [%d] | WaitingListObject is invalid", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
		return;
	}
...
void FChaosVDGeometryBuilder::RemoveMeshComponentWaitingForGeometry(uint32 GeometryKey, TWeakObjectPtr<UMeshComponent> MeshComponent) const
{
	if (!MeshComponent.IsValid())
    {
    	UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] Failed to remove mesh component update for geometry key [%d] | Mesh component is invalid", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
    	return;
    }

	if (!ensure(MeshComponentsWaitingForGeometry.IsValid()))
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Failed to remove mesh component update for geometry key [%d] | WaitingListObject is invalid", ANSI_TO_TCHAR(__FUNCTION__), GeometryKey);
		return;
	}
