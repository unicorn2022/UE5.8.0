...
void SChaosVDSolverTracks::Construct(const FArguments& InArgs, TWeakPtr<FChaosVDPlaybackController> InPlaybackController, TWeakPtr<SChaosVDMainTab> MainTab)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Solver tracks constructed with an invalid player controller. The solver tracks widget will not be functional", ANSI_TO_TCHAR(__FUNCTION__))	
	}
