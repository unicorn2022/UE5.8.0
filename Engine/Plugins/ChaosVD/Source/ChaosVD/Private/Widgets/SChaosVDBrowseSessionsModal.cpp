...
FChaosVDTraceSessionInfo SChaosVDBrowseSessionsModal::GetSelectedTraceInfo()
{
	if (ensure(CurrentTraceSessionSelected.IsValid()))
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Error, "Selected session [%ls] is no longer available", *CurrentTraceSessionSelected.ToString())
		}
	}
	else
	{
		UE_LOGF(LogChaosVDEditor, Error, "Selected session [%ls] is invalid", *CurrentTraceSessionSelected.ToString())
	}
...
void SChaosVDBrowseSessionsModal::UpdateCurrentSessionInfoMap()
{
	...
	if (!StoreClient)
	{
		UE_LOGF(LogChaosVDEditor, Error, "Failed to connect to trace store at [%ls]", *CurrentTraceStoreAddress)
		return;
	}
