...
SChaosVDRecordedLogInstances::~SChaosVDRecordedLogInstances()
{
	...
	{
		EngineInstance->OnTraceSessionOpened().RemoveAll(this);
		EngineInstance->OnTraceSessionClosed().RemoveAll(this);
	}
}
...
void SChaosVDRecordedLogInstances::Construct(const FArguments& InArgs, const TSharedRef<SDockTab>& InOwnerTab, const TSharedRef<FChaosVDEngine>& InEngineInstance)
{
	...
	InEngineInstance->OnTraceSessionClosed().AddSP(this, &SChaosVDRecordedLogInstances::HandleSessionClosed);
	InEngineInstance->OnTraceSessionOpened().AddSP(this, &SChaosVDRecordedLogInstances::HandleSessionOpened);

	using namespace UE::TraceBasedDebuggers;
	TArrayView<FTraceSessionDescriptor> SessionDescriptors = InEngineInstance->GetTraceSessionDescriptors();
	if (!SessionDescriptors.IsEmpty())
	{
		for (const FTraceSessionDescriptor& SessionDescriptor : SessionDescriptors)
		{
...
void SChaosVDRecordedLogInstances::HandleSessionOpened(const UE::TraceBasedDebuggers::FTraceSessionDescriptor& InSessionDescriptor)
{
...
void SChaosVDRecordedLogInstances::HandleSessionClosed(const UE::TraceBasedDebuggers::FTraceSessionDescriptor& InSessionDescriptor)
{
...
TSharedRef<FTabManager::FLayout> SChaosVDRecordedLogInstances::GenerateMainLayout()
{
	...
	for (int32 DefaultTabsCounter = 0; DefaultTabsCounter < MaxDefaultTabs; ++DefaultTabsCounter)
	{
		...
		UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] Generated default log browser tab with ID [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *GeneratedTabID.ToString());
	}
