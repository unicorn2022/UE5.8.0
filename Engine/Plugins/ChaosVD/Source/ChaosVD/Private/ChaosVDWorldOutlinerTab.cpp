...
void FChaosVDWorldOutlinerTab::CreateWorldOutlinerWidget()
{
	...
	Params.ColumnDescription = FTedsOutlinerColumnDescription({FVisibleInEditorColumn::StaticStruct()}, ColumnParams);
	...
	Params.HierarchyData = MakeShared<FTedsOutlinerLegacyHierarchyInterface>(FTedsOutlinerHierarchyData(FChaosVDTableRowParentColumn::StaticStruct(), RowHandleGetter, RowHandleSetter, ChildrenRowHandlesGetter));
	
	if (TSharedPtr<FChaosVDScene> CvdScene = GetChaosVDScene().Pin())
	{
		Params.SelectionSet = CvdScene->GetElementSelectionSet();
	}
