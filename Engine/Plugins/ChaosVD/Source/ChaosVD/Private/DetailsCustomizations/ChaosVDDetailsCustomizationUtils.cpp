...
TSharedPtr<FChaosVDCollisionChannelsInfoContainer> FChaosVDDetailsCustomizationUtils::BuildDefaultCollisionChannelInfo()
{
	...
	if (!ensure(Enum))
	{
		UE_LOGF(LogChaosVDEditor, Error, "[%ls] Failed to generate fallback collision channels entry ", ANSI_TO_TCHAR(__FUNCTION__));
	...
	NewCollisionChannelsInfoContainer->CollisionChannelInfos.SetNum(ExpectedChannels);
	...
	for (int32 ChannelIndex = 0; ChannelIndex < ExpectedChannels; ++ChannelIndex)
	{
		...
		NewCollisionChannelsInfoContainer->CollisionChannelInfos[ChannelIndex] = Info;
	}
...
void FChaosVDDetailsCustomizationUtils::BuildCollisionChannelMatrix(const FChaosVDCollisionChannelStateGetter& InCollisionChannelStateGetter, TConstArrayView<FChaosVDCollisionChannelInfo> CollisionChannelsInfo, IDetailGroup& ParentCategoryGroup)
{
	...
	for (int32 ChannelIndex = 0; ChannelIndex < CollisionChannelsInfo.Num(); ++ChannelIndex)
	{
...
void FChaosVDDetailsCustomizationUtils::ExtractResponseContainer(const uint64 BlockMask, const uint64 OverlapMask, FCollisionResponseContainer& OutResponses)
{
	constexpr uint32 ChannelCount = UE_ARRAY_COUNT(FCollisionResponseContainer::EnumArray);
	static_assert(ChannelCount == 64);

	for (uint32 ChannelIndex = 0; ChannelIndex < ChannelCount; ++ChannelIndex)
	{
		const uint64 ChannelBit = ECC_TO_BITFIELD(ChannelIndex);
		uint8 Response = ECR_Ignore;
		if (BlockMask & ChannelBit)
		{
			Response = ECR_Block;
		}
		else if (OverlapMask & ChannelBit)
		{
			Response = ECR_Overlap;
		}
		OutResponses.EnumArray[ChannelIndex] = Response;
	}
}
