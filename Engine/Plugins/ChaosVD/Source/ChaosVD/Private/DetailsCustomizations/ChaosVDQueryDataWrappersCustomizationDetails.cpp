...
void FChaosVDQueryVisitDataCustomization::ReplaceParticleIndexWithParticleName(IDetailLayoutBuilder& DetailBuilder)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%s] Multiple struct selection is not supported yet. The first one will be customized", __func__);
		return;
	}
	...
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%s] attempted to customize an invalid struct ", __func__);
	}
...
ECollisionResponse FChaosVDCollisionResponseParamsCustomization::GetCurrentCollisionResponseForChannel(int32 ChannelIndex) const
{
	...
	{
		if (ensure(ChannelIndex < Data->FlagsPerChannel.Num()))
		{
...
void FChaosVDCollisionObjectParamsCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> StructPropertyHandle, IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	for (int32 ChannelIndex = 0 ; ChannelIndex < CachedCollisionChannelInfos->CollisionChannelInfos.Num(); ChannelIndex++)
	{
		FChaosVDCollisionChannelInfo& ChannelInfo = CachedCollisionChannelInfos->CollisionChannelInfos[ChannelIndex];
		...
		[
			SNew(STextBlock)
			.Text(FText::FromString(ChannelInfo.DisplayName))
			.Font(IDetailLayoutBuilder::GetDetailFont())
		]
		.ValueContent()
		[
...
ECheckBoxState FChaosVDCollisionObjectParamsCustomization::GetCurrentObjectFlag(int32 ChannelIndex) const
{
	...
	return (CollisionObjectResponseParams->ObjectTypesToQuery64 & ECC_TO_BITFIELD(ChannelIndex)) != 0 ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}
