...
void FChaosVDGeometryComponentUtils::UpdateCollisionDataFromShapeArray(TArrayView<FChaosVDShapeCollisionData> InShapeArray, const TSharedRef<FChaosVDInstancedMeshData>& InInstanceHandle)
{
	...
	if (UNLIKELY(!InShapeArray.IsValidIndex(InstanceState.ImplicitObjectInfo.ShapeInstanceIndex)))
	{
		...
		UE_LOGF(LogChaosVDEditor, Verbose, "[%ls]", *ErrorMessage);
	...
	else if (InInstanceHandle->bFailedToUpdateShapeInstanceData)
	{
		...
		UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] Recovered from failing to find shape instance data at Index [%d] | Particle ID[%d] | Available Shape instance Data Num [%d] | This geometry will be shown again...", ANSI_TO_TCHAR(__FUNCTION__), InstanceState.ImplicitObjectInfo.ShapeInstanceIndex, InInstanceHandle->GetOwningParticleID(), InShapeArray.Num());
	}
