...
void FChaosVDPlaybackController::Initialize()
{
	if (bInitialized)
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%s] Playback controller already initialized", __func__);
		return;
	}

	if (UChaosVDGeneralSettings* Settings = FChaosVDSettingsManager::Get().GetSettingsObject<UChaosVDGeneralSettings>())
	{
		HandleGeneralSettingsUpdated(Settings);
		Settings->OnSettingsChanged().AddSP(this, &FChaosVDPlaybackController::HandleGeneralSettingsUpdated);
	}

	bInitialized = true;
}

void FChaosVDPlaybackController::DeInitialize()
{
	if (!bInitialized)
	{
		UE_LOGF(LogChaosVDEditor, Verbose, "[%s] Playback controller not initialized", __func__);
		return;
	}
...
bool FChaosVDPlaybackController::LoadChaosVDRecordingFromTraceSession(const UE::TraceBasedDebuggers::FTraceSessionDescriptor& InSessionDescriptor)
{
	...
	if (const TSharedPtr<const TraceServices::IAnalysisSession> TraceSession = FChaosVDModule::Get().GetTraceSessionsManager()->GetSession(InSessionDescriptor.SessionName))
	{
...
void FChaosVDPlaybackController::PlayFromClosestKeyFrame_AssumesLocked(const int32 InTrackID, const int32 FrameNumber, FChaosVDScene& InSceneToControl)
{
	...
	if (KeyFrameNumber < 0)
	{
		...
		UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Failed to find a keyframe close to frame [%d] of track [%d]", ANSI_TO_TCHAR(__FUNCTION__), FrameNumber, InTrackID);
		return;
	}
	...
	if (KeyFrameNumber == FrameNumber)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Failed to find a keyframe data for frame [%d] of track [%d]. The visualization might be out of sync until a new keyframe is played.", ANSI_TO_TCHAR(__FUNCTION__), FrameNumber, InTrackID);
		}
...
void FChaosVDPlaybackController::PlaySolverStageData(int32 TrackID, const TSharedRef<FChaosVDScene>& InSceneToControlSharedPtr, const FChaosVDSolverFrameData& InSolverFrameData, int32 StageIndex)
{
	...
	else
	{
		...
		UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] Tried to scrub to an invalid step | Step Number [%d] ...", ANSI_TO_TCHAR(__FUNCTION__), StageIndex);
	}
}
...
void FChaosVDPlaybackController::HandleGeneralSettingsUpdated(UObject* SettingsObject)
{
	if (UChaosVDGeneralSettings* GeneralSettings = Cast<UChaosVDGeneralSettings>(SettingsObject))
	{
		bStartLiveSessionPlaybackAutomatically = GeneralSettings->bAutoStartLiveSessionsPlayback;
	}
}
...
void FChaosVDPlaybackController::GoToRecordedSolverStage_AssumesLocked(const int32 InTrackID, const int32 FrameNumber, const int32 StageNumber, FGuid InstigatorID)
{
	if (const TSharedPtr<FChaosVDScene> SceneToControlSharedPtr = SceneToControl.Pin())
	{
		if (ensure(LoadedRecording.IsValid()))
		{
			...
			{
				UE_LOGF(LogChaosVDEditor, Error, "[%ls] Track info for track ID [%d]. We can't continue...", ANSI_TO_TCHAR(__FUNCTION__), InTrackID);
				return;
			}
			...
			if (FChaosVDSolverFrameData* SolverFrameData = LoadedRecording->GetSolverFrameData_AssumesLocked(InTrackID, FrameNumber))
			{
				...
				if (!SolverFrameData->SolverSteps.IsValidIndex(TargetStageIndex))
				{
					...
					{
						UE_LOGF(LogChaosVDEditor, Verbose, "[%ls] Invalid solver stage index [%d] at frame [%d] for Solver ID [%d]. We can't continue...", ANSI_TO_TCHAR(__FUNCTION__), TargetStageIndex, FrameNumber, InTrackID);
						return;
					}
						...
						case EChaosVDSyncTimelinesMode::NetworkTick:
							{
								...
								{
									UE_LOGF(LogChaosVDEditor, Verbose, "[%s] Failed to map frame number from recorded network tick offsets. Falling back to timestamp mode | From Track [%ls] To Track [%ls]", __func__, *InFromTrack->TrackName.ToString(), *InToTrack->TrackName.ToString());
									return LoadedRecording->GetLowestSolverFrameNumberAtCycle_AssumesLocked(InToTrack->TrackID, FromSolverFrameData->FrameCycle);
								}
							}
						default:
...
bool FChaosVDPlaybackController::Tick(float DeltaTime)
{
	...
	if (bIsRecordingLoaded)
	{
		...
		if (!bPlayedFirstFrame)
		{
			...
			{
				// If this a live session and automatic playback is enabled, start playing
				if (IsPlayingLiveSession() && bStartLiveSessionPlaybackAutomatically)
				{
					constexpr int32 GameFrameToLoad = 0;
					constexpr int32 SolverStageToLoad = 0;
					//Ensure Data Container Actors are created in time before the first frame callback.
				...
				if (bPlayedFirstFrame)
				{
					const TWeakPtr<FChaosVDPlaybackController> InControllerWeakPtr = DoesSharedInstanceExist() ? AsWeak() : nullptr;

					FChaosVDExtensionsManager::Get().EnumerateExtensions([InControllerWeakPtr](const TSharedRef<FChaosVDExtension>& Extension)
					{
						Extension->HandleRecordingFirstFrameLoaded(InControllerWeakPtr);
						return true;
					});
				}
			}
		...
		// If we are live and playing, make sure we don't lag too much behind
		if (!bPauseRequested && IsPlayingLiveSession() && ActiveTrack->bIsPlaying)
		{
...
void FChaosVDPlaybackController::HandleFramePlaybackControlInput(EChaosVDPlaybackButtonsID ButtonID, const TSharedRef<const FChaosVDTrackInfo>& InTrackInfoRef, FGuid Instigator)
{
	...
	{
		UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Attempted to perform a playback action with an incompatible sync mode | Ignoring... ", ANSI_TO_TCHAR(__FUNCTION__));
		return;
	}
	...
	if (InTrackInfoRef != ActiveTrack)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Attempted to play when there was another track playing. This should not happen, this is probably a widget with controls not properly disabled.", ANSI_TO_TCHAR(__FUNCTION__));
	...
	case EChaosVDPlaybackButtonsID::Pause:
		{
			...
			{
				UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Attempted to pause but there was not a track currently playing. This should not happen.", ANSI_TO_TCHAR(__FUNCTION__));
			}
	...
	default:
		{
			...
			UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Unsupported Input type | [%ls]", ANSI_TO_TCHAR(__FUNCTION__), *(UEnum::GetDisplayValueAsText(ButtonID).ToString()));
			break;
		}
	}
}
...
void FChaosVDPlaybackController::SetTimelineSyncMode(EChaosVDSyncTimelinesMode SyncMode)
{
	if (CurrentSyncMode != SyncMode)
	{
		...
		{
			UE_LOGF(LogChaosVDEditor, Warning, "[%ls] Failed to re-sync tracks after sync mode was changed.", ANSI_TO_TCHAR(__FUNCTION__));
		}
	}
}
