...
void FChaosVDWorldOutlinerMode::HandlePostSelectionChange(const UTypedElementSelectionSet* ChangesSelectionSet)
{
	...
	if (SelectedParticlesHandles.Num() > 0)
	{
		...
		if (TTypedElement<ITedsTypedElementBridgeInterface> BridgeInterface =  UTypedElementRegistry::GetInstance()->GetElement<ITedsTypedElementBridgeInterface>(SelectedParticlesHandles[0]))
		{
			UE::Editor::DataStorage::RowHandle RowHandle = BridgeInterface.GetRowHandle();
...
FText FChaosVDWorldOutlinerMode::GetStatusText() const
{
	if (CanPopulate())
	{
		const int32 PendingUpdated = RowsPendingAddition.Num() + RowsPendingRemoval.Num();
		if (PendingUpdated > 0)
		{
			return FText::Format(LOCTEXT("SceneOutlinerUpdatingMessage", "Processing {0} changes..."), FText::AsNumber(PendingUpdated));
		}

		return FText::GetEmpty();
	}
	
	return LOCTEXT("SceneOutlinerDisabledWarning", "Outliner updates are disabled during playback or scrubbing");
}

FSlateColor FChaosVDWorldOutlinerMode::GetStatusTextColor() const
{
	return CanPopulate() ? FStyleColors::Foreground : FStyleColors::Warning;
}
