...
TChaosVDObjectPool<ObjectType>::~TChaosVDObjectPool()
{
	...
	UE_LOGF(LogChaosVDEditor, Log, "Object pooling Statics for pool [%ls] | Hits [%d] | Total Acquire requests [%d] | [%f] percent hit/miss ratio", *PoolName, PoolHits,  PoolRequests, HitMissRatio);
...
ObjectType* TChaosVDObjectPool<ObjectType>::AcquireObject(UObject* Outer, FName Name)
{
	...
	if (FChaosVDObjectPoolCVars::bUseObjectPool && PoolSize > 0)
	{
		...
		if (Object)
		{
			...
			Object->Rename(*NewName.ToString() , Outer, REN_NonTransactional | REN_DoNotDirty | REN_SkipGeneratedClasses | REN_DontCreateRedirectors | REN_AllowPackageLinkerMismatch);
