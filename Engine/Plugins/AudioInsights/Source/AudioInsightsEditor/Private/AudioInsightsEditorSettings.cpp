...
{
	return LOCTEXT("AudioInsightsEditorSettings_SectionText", "Audio Insights (Editor)");
}
...
void UAudioInsightsEditorSettings::PostInitProperties()
{
	...
	OnRequestReadSignalFlowSettingsHandle = FSignalFlowSettings::OnRequestReadSettings.AddLambda([this]()
	{
		FSignalFlowSettings::OnReadSettings.Broadcast(SignalFlowSettings);
	});

	OnRequestWriteSignalFlowSettingsHandle = FSignalFlowSettings::OnRequestWriteSettings.AddLambda([this]()
	{
		FSignalFlowSettings::OnWriteSettings.Broadcast(SignalFlowSettings);
		SaveConfig();
	});
}
...
void UAudioInsightsEditorSettings::BeginDestroy()
{
	...
	FSignalFlowSettings::OnRequestReadSettings.Remove(OnRequestReadSignalFlowSettingsHandle);
	FSignalFlowSettings::OnRequestWriteSettings.Remove(OnRequestWriteSignalFlowSettingsHandle);
}
...
void UAudioInsightsEditorSettings::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	...
	if (ActiveMemberNode->GetValue()->GetFName() == GET_MEMBER_NAME_CHECKED(UAudioInsightsEditorSettings, bOnlyTraceAudioChannels))
	{
		OnOnlyTraceAudioChannelsPropertyChanged.Broadcast(bOnlyTraceAudioChannels);
	}
	...
	if (ActiveMemberNode->GetValue()->GetFName() == GET_MEMBER_NAME_CHECKED(UAudioInsightsEditorSettings, SignalFlowSettings))
	{
		FSignalFlowSettings::OnReadSettings.Broadcast(SignalFlowSettings);
	}
}
...
bool UAudioInsightsEditorSettings::VerifyCustomEventNameEdit(const FPropertyChangedChainEvent& PropertyChangedEvent, const FMapProperty* CategoryToEventMapProperty, const FSetProperty* EventNamesSetProperty)
{
	...
	if (Events == nullptr)
	{
		UE_LOGF(LogAudioInsights, Warning, "Could not find category with name %ls in EventLogSettings.CustomCategoriesToEvents", **EditedCategoryName);
		return false;
	}
...
FString* UAudioInsightsEditorSettings::GetEditedCategoryKey(const FPropertyChangedChainEvent& PropertyChangedEvent, const FMapProperty* CategoryToEventMapProperty) const
{
	...
	{
		UE_LOGF(LogAudioInsights, Warning, "Detected invalid index in CustomCategoriesToEvents TMap when trying to validate custom event log edit");
		return nullptr;
	}
...
UAudioInsightsEditorSettings::EditedEvent UAudioInsightsEditorSettings::GetEditedEvent(const FPropertyChangedChainEvent& PropertyChangedEvent, const FSetProperty* EditedEventsSetProperty, const FAudioEventLogCustomEvents& Events) const
{
	...
	if (PropertyChangedEvent.ChangeType & EPropertyChangeType::ArrayAdd)
	{
		...
		UE_LOGF(LogAudioInsights, Warning, "Could not locate newly added event name element in EventNames set");
		return { nullptr };
	}
	else
	{
		...
		{
			UE_LOGF(LogAudioInsights, Warning, "Detected invalid index in EventNames TSet when trying to validate custom event log edit");
			return { nullptr };
		}
