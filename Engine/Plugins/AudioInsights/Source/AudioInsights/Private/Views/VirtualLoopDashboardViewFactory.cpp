...
namespace UE::Audio::Insights
{
	...
	TSharedRef<SWidget> FVirtualLoopDashboardViewFactory::MakeWidget(TSharedRef<SDockTab> InOwnerTab, const FSpawnTabArgs& InSpawnTabArgs)
	{
		if (!DashboardWidget.IsValid())
		{
			DashboardWidget = FTraceTableDashboardViewFactory::MakeWidget(InOwnerTab, InSpawnTabArgs);
...
#if WITH_EDITOR
	FReply FVirtualLoopDashboardViewFactory::OnDataRowKeyInput(const FGeometry& Geometry, const FKeyEvent& KeyEvent) const
	{
		if (AssetContextMenuHelper.ProcessCommandBindings(KeyEvent))
		{
	...
	TSharedPtr<SWidget> FVirtualLoopDashboardViewFactory::OnConstructContextMenu()
	{
		return AssetContextMenuHelper.ContructContextMenuOptions();
	}

	void FVirtualLoopDashboardViewFactory::OnSelectionChanged(TSharedPtr<IDashboardDataViewEntry> SelectedItem, ESelectInfo::Type SelectInfo)
	{
		FTraceObjectTableDashboardViewFactory::OnSelectionChanged(SelectedItem, SelectInfo);

		if (SelectedItem.IsValid())
		{
			TSharedPtr<IObjectDashboardEntry> ObjectEntry = StaticCastSharedPtr<IObjectDashboardEntry>(SelectedItem);
			AssetContextMenuHelper.SetAssetEntry(ObjectEntry);
		}
		else
		{
