...
namespace UE::Audio::Insights
{
	...
	TSharedRef<SHeaderRow> FTraceTreeDashboardViewFactory::MakeHeaderRowWidget()
	{
		...
		{
			SHeaderRow::FColumn::FArguments ColumnArgs = SHeaderRow::Column(ColumnName)
			...
			if (ColumnData.FixedWidth.IsSet())
			{
				ColumnArgs.FixedWidth(ColumnData.FixedWidth.GetValue());
			}
			else
			{
				ColumnArgs.FillWidth(ColumnData.FillWidth);
			}
...
#if WITH_EDITOR
	void FTraceTreeDashboardViewFactory::UpdateDebugDraw(const float DeltaTime)
	{
