...
namespace UE::Audio::Insights
{
	...
	namespace FSoundDashboardViewFactoryPrivate
	{
		...
		bool IsMuteSolo(::Audio::FAudioDebugger& InAudioDebugger, const IDashboardDataTreeViewEntry& InEntry, const bool bInCheckChildren, const EMuteSoloMode InMuteSoloMode)
		{
			...
			if (!bIsSoundCueType && !SoundEntry.bIsCategory)
			{
				bool bIsMuted = false;
				bool bIsSoloed = false;
				FString Reason;
				InAudioDebugger.QuerySoloMuteSoundWave(SoundEntry.GetDisplayNameStr(), bIsSoloed, bIsMuted, Reason);
		...
		const FName AudioComponentNameColumnName = "AudioComponentName";
		const FText AudioComponentNameColumnDisplayName = LOCTEXT("SoundDashboard_AudioComponentNameColumnDisplayName", "Audio Component");
		const FText AudioComponentNameColumnTooltip = LOCTEXT("SoundDashboard_AudioComponentNameColumnTooltip", "The name of the Audio Component playing this sound (if applicable).");
	...
	TSharedRef<SWidget> FSoundDashboardViewFactory::MakeMuteSoloWidget()
	{
		...
		TFunction<FText()> MuteTextFunc = [bNotMuteSolo = bIsNotFilteredMuteOrSolo, &FilteredList = FilteredEntriesListView]()
			{
				return FilteredList.IsValid() && bNotMuteSolo && FilteredList->GetNumItemsSelected() > 0 ?
					...
					LOCTEXT("SoundDashboard_MuteButtonTextFilter", "Mute by Text");
			};
		...
		TFunction<FText()> SoloTextFunc = [bNotMuteSolo = bIsNotFilteredMuteOrSolo, &FilteredList = FilteredEntriesListView]()
			{
				return FilteredList.IsValid() && bNotMuteSolo && FilteredList->GetNumItemsSelected() > 0 ?
					...
					LOCTEXT("SoundDashboard_SoloButtonTextFilter", "Solo by Text");
			};
		...
		return SNew(SHorizontalBox)
			...
			[
				...
				[
					FSlateStyle::Get().CreateToggleButtonContent("AudioInsights.Icon.SoundDashboard.Mute", "SmallButtonText", MuteColorFunc, MuteTextFunc)
				]
			]
			// Solo button
			+ SHorizontalBox::Slot()
			...
			[
				...
				[
					FSlateStyle::Get().CreateToggleButtonContent("AudioInsights.Icon.SoundDashboard.Solo", "SmallButtonText", SoloColorFunc, SoloTextFunc)
				]
			]
			...
			[
				...
				[
					FSlateStyle::Get().FSlateStyle::Get().CreateButtonContentWidget("AudioInsights.Icon.SoundDashboard.Reset", LOCTEXT("SoundsDashboard_ClearMutesAndSolosButtonText", "Clear All Mutes/Solos"), "SmallButtonText")
				]
			];
	}
#endif // WITH_EDITOR
	...
	void FSoundDashboardViewFactory::SetSearchBoxFilterText(const FText& NewText)
	{
		FTraceTreeDashboardViewFactory::SetSearchBoxFilterText(NewText);
		...
		if (bIsPlotFilteredMode)
		{
			FilterText(); // Sets the entry visibility, required for use within PlotFilteredEntries
			ResetPlots();
	...
	TSharedRef<SWidget> FSoundDashboardViewFactory::MakeWidget(TSharedRef<SDockTab> OwnerTab, const FSpawnTabArgs& SpawnTabArgs)
	{
		...
		AudioInsightsModule.GetTimingViewExtender().OnTimeControlMethodReset.AddSP(this, &FSoundDashboardViewFactory::HandleOnTimeControlMethodReset);
		...
		else
		{
			...
			return SNew(SHorizontalBox)
				...
				[
					...
					.ToolTipText_Lambda([InRowData, ColumnData]()
					{
						return ColumnData.GetDisplayValue(InRowData.Get());
					})
				];
	...
	void FSoundDashboardViewFactory::OnSelectionChanged(TSharedPtr<IDashboardDataTreeViewEntry> SelectedItem, ESelectInfo::Type SelectInfo)
	{
		...
		const TArray<TObjectPtr<UObject>> Assets = GetSelectedEditableAssets();
		if (Assets.Num() > 0)
		{
			SelectionManager.SetSelectedAsset(Assets[0]);
		}
	...
	const TMap<FName, FTraceTreeDashboardViewFactory::FHeaderRowColumnData>& FSoundDashboardViewFactory::GetHeaderRowColumns() const
	{
		...
		static const TMap<FName, FTraceTreeDashboardViewFactory::FHeaderRowColumnData> HeaderRowColumnData =
		{
			...
			{
				AudioComponentNameColumnName,
				{
					.DisplayName	  = AudioComponentNameColumnDisplayName,
					.IconName		  = NAME_None,
					.TooltipText	  = AudioComponentNameColumnTooltip,
					.bShowDisplayName = true,
					.bDefaultHidden   = !VisibleColumns.GetIsVisible(AudioComponentNameColumnName),
					.FillWidth		  = 0.1f,
					.Alignment		  = EHorizontalAlignment::HAlign_Left
				}
			},
			...
			},
			{
				PriorityColumnName,
				{
					.GetDisplayValue = [](const IDashboardDataTreeViewEntry& InData)
					{
						...
						return SoundMessageUtils::IsPriorityValueSetToMax(PriorityValue)
			...
			},
			{
				DistanceColumnName,
				{
					.GetDisplayValue = [](const IDashboardDataTreeViewEntry& InData)
					{
						...
						return FText::AsNumber(SoundDashboardEntry.DistanceDataPoint, FSlateStyle::Get().GetDistanceFloatFormat());
					}
				}
			},
			{
			...
			},
			{
				AudioComponentNameColumnName,
				{
					.GetDisplayValue = [](const IDashboardDataTreeViewEntry& InData)
					{
						...
						return SoundDashboardEntry.AudioComponentName;
					}
				}
			},
			{
	...
	void FSoundDashboardViewFactory::SortTable()
	{
		...
		auto SortByAudioComponentName = [&SortByPlayOrder](const FSoundDashboardEntry& First, const FSoundDashboardEntry& Second)
		{
			const int32 Comparison = First.AudioComponentName.CompareToCaseIgnored(Second.AudioComponentName);
			if (Comparison == 0)
			{
				return SortByPlayOrder(First, Second);
			}
		...
		else if (SortByColumn == AudioComponentNameColumnName)
		{
			SortByPredicate(SortByAudioComponentName);
		}
...

		{
			...
			MenuBuilder.AddMenuEntry(
				...
				[
					...
					{
						// Persist the in-memory value (already updated per keystroke) to .ini on commit.
						FSoundDashboardSettings::OnRequestWriteSettings.Broadcast();
					})
#endif // WITH_EDITOR
				],
	...
	void FSoundDashboardViewFactory::MuteSoloFilteredEntries()
	{
		...
		const FText& FilterText = GetSearchFilterText();
		if (!FilterText.IsEmpty())
		{
			const EMuteSoloMode Mode = bIsSoloFilteredMode ? EMuteSoloMode::Solo : EMuteSoloMode::Mute;
			::Audio::FAudioDebugger& AudioDebugger = AudioDeviceManager->GetDebugger();

			if (Mode == EMuteSoloMode::Solo)
			{
				AudioDebugger.SetSoloSoundWave(FName(FilterText.ToString()), true);
			}
			else
			{
				AudioDebugger.SetMuteSoundWave(FName(FilterText.ToString()), true);
			}
		}
#endif //ENABLE_AUDIO_DEBUG
	}
	...
	void FSoundDashboardViewFactory::HandleOnTimeControlMethodReset()
	{
		bPinnedSectionNeedsRebuild = true;
	}

	void FSoundDashboardViewFactory::PlotFilteredEntries()
	{
		if (!bIsPlotFilteredMode)
		{
			return;
		...
		if (FilterText.IsEmpty())
		{
			return; // Intentionally don't plot everything when the filter is empty
		}

		for (const TSharedPtr<IDashboardDataTreeViewEntry>& Entry : DataViewEntries)
		{
			SetShowPlotVisibleRecursive(Entry); // Plot what is visible via filter
		}

		BroadcastPlotUpdate();
	}

	void FSoundDashboardViewFactory::ResetPlots()
	{
		for (const TSharedPtr<IDashboardDataTreeViewEntry>& Entry : DataViewEntries)
		{
	...
	void FSoundDashboardViewFactory::BroadcastPlotUpdate()
	{
		if (FilteredEntriesListView.IsValid())
		{
	...
	TSharedRef<SWidget> FSoundDashboardViewFactory::MakeShowPlotWidget()
	{
		...
		return SNew(SHorizontalBox)
			...
			[
				...
				[
					FSlateStyle::Get().CreateButtonContentWidget("AudioInsights.Icon.SoundDashboard.Plots", LOCTEXT("SoundDashboard_PlotsButtonText", "Plot Selected"), "SmallButtonText")
				]
			]
			// Plot by Text button
			+ SHorizontalBox::Slot()
			...
			[
				SNew(SButton)
				.ButtonStyle(&FAppStyle::Get().GetWidgetStyle<FButtonStyle>("SimpleButton"))
				.ToolTipText(LOCTEXT("SoundDashboard_PlotByTextTooltipText", "Toggles plotting on entries matching the search text."))
				.OnClicked_Lambda([this]()
				{
					bIsPlotFilteredMode = !bIsPlotFilteredMode;

					ResetPlots(); // Clear plots when we change modes

					if (bIsPlotFilteredMode)
					{
						PlotFilteredEntries(); // Since we might be paused, also plot filtered entries here
					}

					return FReply::Handled();
				})
				[
					FSlateStyle::Get().CreateToggleButtonContent("AudioInsights.Icon.SoundDashboard.Plots",
						"SmallButtonText",
						[this]() { return bIsPlotFilteredMode ? FSlateColor(FColor::Green) : FSlateColor::UseForeground(); },
						[]() { return LOCTEXT("SoundDashboard_PlotByTextButtonText", "Plot by Text"); })
				]
			]
			...
			[
				...
				.OnClicked_Lambda([this]()
				{
					bIsPlotFilteredMode = false;
					ResetPlots();

					return FReply::Handled();
				})
				[
					FSlateStyle::Get().CreateButtonContentWidget("AudioInsights.Icon.SoundDashboard.Reset", LOCTEXT("SoundsDashboard_ClearAllPlotsButtonText", "Clear All Plots"), "SmallButtonText")
				]
			];
	}
	...
	void FSoundDashboardViewFactory::SetShowPlotVisibleRecursive(const TSharedPtr<IDashboardDataTreeViewEntry>& Entry)
	{
		using namespace FSoundDashboardViewFactoryPrivate;
		...
		SetShowPlot(Entry, SoundEntry.bIsVisible);

		for (const TSharedPtr<IDashboardDataTreeViewEntry>& ChildEntry : Entry->Children)
		{
			SetShowPlotVisibleRecursive(ChildEntry);
		}
	}
	...
	void FSoundDashboardViewFactory::UpdatePinnedSection()
	{
		if (bPinnedSectionNeedsRebuild)
		{
			...
			const int32 RebuiltChildren = PinnedItemEntries.IsValid() ? PinnedItemEntries->PinnedWrapperChildren.Num() : 0;

			// After OnTimeControlMethodReset, the provider re-creates entries asynchronously from the realtime trace stream.
			// The first UpdatePinnedSection ticks may run with DataViewEntries containing only the parent category (no children yet), so RebuildPinnedSection finds nothing to pin.
			// Keep the latch set across ticks until the rebuild actually produces PinnedWrapperChildren > 0, i.e. the restored entry has finally arrived in DataViewEntries.
			// Also clear the latch if the provider has nothing cached to restore (e.g. after un-pin), so we don't loop on empty rebuilds.
			if (RebuiltChildren > 0)
			{
				bPinnedSectionNeedsRebuild = false;
			}
			else if (const TSharedPtr<const FSoundTraceProvider> Provider = FindProvider<const FSoundTraceProvider>(false /*bEnsureIfMissing*/))
			{
				if (!Provider->HasCachedPinnedEntries())
				{
					bPinnedSectionNeedsRebuild = false;
				}
			}
		...
		if (FPinnedSoundEntryWrapperPrivate::CanBeDeleted(PinnedItemEntries))
		{
			...
			// Scrub may have temporarily destroyed pinned entries that will reappear when the time marker moves back.
			// Re-trigger the latch only if the provider still has cached pins to restore; otherwise (e.g. the user just un-pinned everything) leave it cleared.
			if (const TSharedPtr<const FSoundTraceProvider> Provider = FindProvider<const FSoundTraceProvider>(false /*bEnsureIfMissing*/))
			{
				if (Provider->HasCachedPinnedEntries())
				{
					bPinnedSectionNeedsRebuild = true;
