...
namespace UE::Audio::Insights
{
	...
	void FSoundPlotsWidgetView::ProcessPlotData(const TArray<TSharedPtr<IDashboardDataTreeViewEntry>>& DataViewEntries, const TArray<TSharedPtr<IDashboardDataTreeViewEntry>>& SelectedEntries, const bool bForceUpdate /* = false */)
	{
		...
		const TSharedPtr<FPointDataPerCurveMap>* const FoundPointDataPerCurveElement = PlotWidgetCurveIdToPointDataMapPerColumn.Find(SelectedPlotColumnName);

		if (FoundPointDataPerCurveElement && FoundPointDataPerCurveElement->IsValid())
		{
			// Remove points that are older than max history limit from the most recent timestamp
			for (auto& [CurveId, CurvePoints] : **FoundPointDataPerCurveElement)
			{
	...
	void FSoundPlotsWidgetView::UpdatePlot()
	{
		const TSharedPtr<FPointDataPerCurveMap>* const DataMap = PlotWidgetCurveIdToPointDataMapPerColumn.Find(SelectedPlotColumnName);
		if (!DataMap || !DataMap->IsValid())
		{
			ensureMsgf(false, TEXT("Invalid FPointDataPerCurveMap in FSoundPlotsWidgetView::UpdatePlot"));
			return;
		}

		const FNumberFormattingOptions* const PlotColumnNumberFormat = GetPlotColumnNumberFormat(SelectedPlotColumnName);
		if (!PlotColumnNumberFormat)
		{
			ensureMsgf(false, TEXT("Invalid FNumberFormattingOptions in FSoundPlotsWidgetView::UpdatePlot"));
			return;
		}

		if (!PlotWidget.IsValid())
		{
			ensureMsgf(false, TEXT("Invalid SAudioCurveView in FSoundPlotsWidgetView::UpdatePlot"));
			return;
		}

		PlotWidget->SetCurvesPointData(*DataMap);
		PlotWidget->SetYValueFormattingOptions(*PlotColumnNumberFormat);
	...
	void FSoundPlotsWidgetView::SetTimestampFromCurveView(const double Timestamp)
	{
		...
		FAudioInsightsModule& AudioInsightsModule = UE::Audio::Insights::FAudioInsightsModule::GetChecked();
		AudioInsightsModule.GetTimingViewExtender().PauseTimeMarker(AbsoluteTimestamp, ESystemControllingTimeMarker::PlotsWidget, AbsolutePlottingRange);
	}
	...
	TRange<double> FSoundPlotsWidgetView::GetViewRange()
	{
...
#else
		if (GameState == EGameState::ControlledByTimemarker || !IAudioInsightsModule::IsLiveSession())
		{
		...
		else
		{
			...
			const double TraceCurrentDurationSeconds = FAudioInsightsModule::GetChecked().GetTimingViewExtender().GetCurrentTraceDurationSeconds();
