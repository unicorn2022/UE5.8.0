...
namespace UE::Audio::Insights
{
	...
	TSharedRef<SWidget> FSubmixDashboardViewFactory::GenerateWidgetForColumn(TSharedRef<IDashboardDataViewEntry> InRowData, const FName& InColumnName)
	{
		...
		else if (InColumnName == "Name")
		{
			...
			if (ShouldUpdateAudioMeter(SubmixDashboardEntry) && !SubmixDashboardEntry.AudioMeterInfo->EnvelopeValues.IsEmpty())
			{
			...
			return SNew(SHorizontalBox)
				...
				[
					...
					.IsChecked_Lambda([this, InRowData]()
					{
						...
						if (CheckedSubmixes.Contains(SubmixDashboardEntry.SubmixId))
						{
					...
					.OnCheckStateChanged_Lambda([this, InRowData](ECheckBoxState NewState)
					{
						...
						for (const TSharedPtr<IDashboardDataViewEntry>& Entry : EntriesToToggle)
						{
							if (Entry.IsValid())
							{
								...
								SetSubmixEnvelopeFollowerEnabled(SubmixDashboardEntry, bIsChecked);
							}
						}
					})
				]
	...
	void FSubmixDashboardViewFactory::SetSubmixEnvelopeFollowerEnabled(const FSubmixDashboardEntry& InSubmixDashboardEntry, const bool bInEnabled)
	{
		using namespace FSubmixDashboardViewFactoryPrivate;

		SendSubmixEnvelopeFollowerCVar(bInEnabled, InSubmixDashboardEntry);

		if (bInEnabled)
		{
			CheckedSubmixes.Emplace(InSubmixDashboardEntry.SubmixId);
			FAudioMeterProvider::OnAddAudioMeter.ExecuteIfBound(InSubmixDashboardEntry.SubmixId, InSubmixDashboardEntry.AudioMeterInfo, InSubmixDashboardEntry.GetDisplayName(), SubmixMeterTextColor);
		}
		else
		{
			CheckedSubmixes.Remove(InSubmixDashboardEntry.SubmixId);
			FAudioMeterProvider::OnRemoveAudioMeter.ExecuteIfBound(InSubmixDashboardEntry.SubmixId);
		}
	}
	...
	void FSubmixDashboardViewFactory::RebuildAudioMeters() const
	{
		...
		for (const TSharedPtr<IDashboardDataViewEntry>& Entry : DataViewEntries)
		{
			...
			if (ShouldUpdateAudioMeter(SubmixDashboardEntry))
			{
				...
				if (!SubmixDashboardEntry.AudioMeterInfo->EnvelopeValues.IsEmpty())
				{
					FAudioMeterProvider::OnUpdateAudioMeterInfo.ExecuteIfBound(SubmixDashboardEntry.SubmixId, SubmixDashboardEntry.AudioMeterInfo, SubmixDashboardEntry.GetDisplayName());
				}
			}
	...
	bool FSubmixDashboardViewFactory::ShouldUpdateAudioMeter(const FSubmixDashboardEntry& InSubmixDashboardEntry) const
	{
		...
		if (CheckedSubmixes.Contains(InSubmixDashboardEntry.SubmixId))
		{
			return true;
		}
		...
		if (!IAudioInsightsModule::IsLiveSession())
		{
			return InSubmixDashboardEntry.bEnvelopeFollowerEnabled;
		}
#endif // !WITH_EDITOR
