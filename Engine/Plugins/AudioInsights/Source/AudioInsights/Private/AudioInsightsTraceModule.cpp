...
namespace UE::Audio::Insights
{
	namespace FTraceModulePrivate
	{
		...
		FString GetTraceStoreFilePath(const FString& FileName)
		{
			auto GetTraceHostAddress = []() -> FString
			{
				TSharedPtr<FInternetAddr> RecorderAddress;

				if (ISocketSubsystem* const Sockets = ISocketSubsystem::Get())
				{
			...
			const TUniquePtr<UE::Trace::FStoreClient> StoreClient = UE::Trace::FStoreClient::Create(*GetTraceHostAddress());
			...
			if (StoreClient.IsValid())
			{
				if (const UE::Trace::FStoreClient::FStatus* StoreClientStatus = StoreClient->GetStatus())
				{
					...
					FString FilePath = FPaths::Combine(TraceStorePath, FileName);
			...
			UE_LOGF(LogAudioInsights, Warning, "Failed to connect to the Trace Store Client, returning an empty path.");
	...
	void FTraceModule::StartTraceAnalysis(const bool bInOnlyTraceAudioChannels, const ETraceMode InTraceMode)
	{
...
#if WITH_EDITOR
		// A trace already being connected means we are latching onto one started elsewhere
		// (e.g. the regular Insights record button). In that case the user picked their own
		// channels and we must not override them - just additively enable the audio channels.
		const bool bIsExternalTrace = FTraceAuxiliary::IsConnected();

		CacheCurrentlyEnabledTraceChannels();

		if ((bInOnlyTraceAudioChannels) && (!bIsExternalTrace))
		{
		...
		else
		{
			...
			{
				bTraceStarted = FTraceAuxiliary::Start(FTraceAuxiliary::EConnectionType::Network, TEXT("localhost"), TEXT(""), &Options);
			...
			else if (CurrentTraceMode == ETraceMode::Monitoring)
			{
				...
				bTraceStarted = FTraceAuxiliary::Start(FTraceAuxiliary::EConnectionType::Network, *DirectTraceHost, TEXT(""), &Options);
			}

			if (!bTraceStarted)
			{
				OnTraceStopped(FTraceAuxiliary::EConnectionType::Network, FString());
				return;
			}
		}
#else
	...
	void FTraceModule::OnOnlyTraceAudioChannelsStateChanged(const bool bOnlyTraceAudioChannels)
	{
		...
		if (bOnlyTraceAudioChannels)
		{
			...
			// Don't disable other channels when we're piggybacking on an externally-started trace -
			// the user picked those channels for their own profiling and we'd silently break it.
			if (bIsAudioInsightsTrace)
			{
				DisableAllTraceChannels();
			}
	...
	void FTraceModule::SaveCacheSnapshot()
	{
		using namespace FTraceModulePrivate;

		const FAudioInsightsCacheManager& CacheManager = FAudioInsightsModule::GetChecked().GetCacheManager();

		if (CacheManager.GetNumUsedChunks() == 0)
		{
			UE_LOGF(LogAudioInsights, Warning, "Cache is empty, nothing to export.");
			return;
		}

		const FString FileName = FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S_AudioInsightsCache.utrace"));
		const FString FilePath = GetTraceStoreFilePath(FileName);

		if (FilePath.IsEmpty())
		{
			UE_LOGF(LogAudioInsights, Warning, "Failed to determine trace store path for cache snapshot.");
			return;
		}

		if (!FAudioInsightsCacheTraceWriter::WriteCacheToFile(CacheManager, FilePath))
		{
			UE_LOGF(LogAudioInsights, Warning, "Failed to write cache snapshot to %ls", *FilePath);
		}
	}
