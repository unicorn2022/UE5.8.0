...
namespace UE::Audio::Insights
{
	...
	namespace SoundMessageNames
	{
		...
		extern const FName SoundWaveStop;
	...
	struct FSoundWaveStartMessage : public FSoundStartMessage
	{
			...
			: FSoundStartMessage(InContext)
		{
			WaveInstancePlayOrder = InContext.EventData.GetValue<uint32>("PlayOrder");
			ActiveSoundPlayOrder = InContext.EventData.GetValue<uint32>("ActiveSoundPlayOrder");
			SourceBusID = InContext.EventData.GetValue<int32>("SourceBusID", INDEX_NONE);

			InContext.EventData.GetString("ActorIconName", ActorIconName);
		}
	...
	struct FSoundWaveStopMessage : public FSoundMessageBase
	{
		FSoundWaveStopMessage() = default;

		FSoundWaveStopMessage(const Trace::IAnalyzer::FOnEventContext& InContext)
			: FSoundMessageBase(InContext)
		{
			WaveInstancePlayOrder = InContext.EventData.GetValue<uint32>("PlayOrder");
			ActiveSoundPlayOrder = InContext.EventData.GetValue<uint32>("ActiveSoundPlayOrder");
		...
		virtual uint32 GetSizeOf() const override;
		virtual const FName GetMessageName() const override { return SoundMessageNames::SoundWaveStop; }
		virtual uint64 GetID() const override { return SoundMessageUtils::GeneratePlayOrderUniqueID(ActiveSoundPlayOrder, WaveInstancePlayOrder); }
		virtual FCacheWriteHandler GetCacheWriteHandler() const override;
