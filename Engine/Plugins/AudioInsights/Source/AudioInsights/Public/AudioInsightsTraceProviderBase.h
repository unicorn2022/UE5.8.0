...
namespace UE::Audio::Insights
{
	...
	protected:
		...
		protected:
			...
			virtual bool OnHandleEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context) = 0;
			...
			// Will cache a message, and add it to the provided OutMessageQueue for processing
			...
			void CacheMessage(const FOnEventContext& Context, TAnalyzerMessageQueue<T>& OutMessageQueue)
			{
				...
				AddMessageToCache(MoveTemp(Message), OutMessageQueue);
			}


			// If processing is enabled, will add a message to the provided OutMessageQueue for processing, but will not cache it.
			template <TIsCacheableMessage T>
			void AddMessageToQueue(const FOnEventContext& Context, TAnalyzerMessageQueue<T>& OutMessageQueue)
			{
				if (ShouldProcessNewEvents())
				{
					OutMessageQueue.Enqueue(T(Context));
				}
			}
		...
		private:
			...
			template <TIsCacheableMessage T>
			void AddMessageToCache(TSharedPtr<T>&& Message, TAnalyzerMessageQueue<T>& OutMessageQueue)
			{
			...
			FAudioInsightsCacheManager& CacheManager;
	...
	public:
...
#if WITH_EDITOR
			, bIsTraceActive(FTraceAuxiliary::IsConnected())
#endif // WITH_EDITOR
		{
		...
		::Audio::FDeviceId GetFilteredAudioDeviceID() const
		{
#if WITH_EDITOR
			const IAudioInsightsModule& AudioInsightsModule = IAudioInsightsModule::GetEditorChecked();
			return AudioInsightsModule.GetDeviceId();
#else
			return GetAudioDeviceIdFromDeviceDataMap();
#endif // WITH_EDITOR
		}

		const FDeviceData* FindFilteredDeviceData() const
		{
			const ::Audio::FDeviceId AudioDeviceId = GetFilteredAudioDeviceID();
	...
	protected:
		...
		EntryType& FindOrAddDeviceEntry(::Audio::FDeviceId InDeviceId, const EntryKey& InKey)
		{
			ensure(InDeviceId != INDEX_NONE);
			FDeviceData& DeviceData = DeviceDataMap.FindOrAdd(InDeviceId);
			return DeviceData.FindOrAdd(InKey);
		}
		...
		{
			EntryType& Entry = FindOrAddDeviceEntry(InDeviceId, InKey);
