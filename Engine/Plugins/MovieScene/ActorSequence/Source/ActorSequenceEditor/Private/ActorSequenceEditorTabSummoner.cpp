...
namespace UE::ActorSequenceEditor::Private
{
	const FName ViewportToolbarOwnerName = "ActorSequenceEditorViewportToolbar";
	...
	TSharedRef<ITableRow> GenerateRow(TSharedPtr<FSCSEditorTreeNode> InNodePtr, const TSharedRef<STableViewBase>& OwnerTable)
	{
		...
		Row->SetContent(
			...
			[
				SNew(SImage)
				.Image(ComponentIcon)
				.ColorAndOpacity(FSCSEditorTreeNode::GetColorTintForIcon(InNodePtr))
			]
...
public:
	...
	void Close()
	{
		...
		if (const TSharedPtr<FBlueprintEditor> BlueprintEditor = WeakBlueprintEditor.Pin())
		{
			...
			UToolMenus::Get()->UnregisterOwnerByName(UE::ActorSequenceEditor::Private::ViewportToolbarOwnerName);
		}
	...
	void SetActorSequence(UActorSequence* NewSequence)
	{
		...
		// Extend the SCSEditor viewport toolbar.
		{
			FToolMenuOwnerScoped ScopeOwner(UE::ActorSequenceEditor::Private::ViewportToolbarOwnerName);

			const FName TransformMenu = "SCSEditor.ViewportToolbar.Transform";
			...
			{
				FToolMenuEntry Entry = UE::ActorSequenceEditor::Private::CreateToggleViewportSelectionEntry(Sequencer.ToWeakPtr());
				Entry.InsertPosition.Position = EToolMenuInsertType::Last;
