...
TMap<FGuid,TFunction<bool(UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType, TArray<UMovieSceneScriptingChannel*>& OutChannels)>>  UMovieSceneSectionExtensions::CustomChannelsCallbacks;

FGuid UMovieSceneSectionExtensions::SetGetCustomChannelsFunction(TFunction<bool(UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType, TArray<UMovieSceneScriptingChannel*>& OutChannels) > CustommFunc)
{
	FGuid NewGuid = FGuid::NewGuid();
	CustomChannelsCallbacks.Add(NewGuid,CustommFunc);
	return NewGuid;
}
void UMovieSceneSectionExtensions::RemoveGetCustomChannelsFunction(const FGuid& InGuid)
{
	CustomChannelsCallbacks.Remove(InGuid);
}
...
TArray<UMovieSceneScriptingChannel*> UMovieSceneSectionExtensions::GetAllChannels(UMovieSceneSection* Section)
{
	...
	GetScriptingChannelsForChannel<FMovieSceneActorReferenceChannel, UMovieSceneScriptingActorReferenceChannel>(ChannelProxy, Sequence, Section, Channels);
	...
	for (const TPair<FGuid,TFunction<bool(UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType, TArray<UMovieSceneScriptingChannel*>& OutChannels)>>&
		Function : CustomChannelsCallbacks)
	{
		Function.Value(Section, UMovieSceneScriptingChannel::StaticClass(), Channels);
	}
	return Channels;
}
...
TArray<UMovieSceneScriptingChannel*> UMovieSceneSectionExtensions::GetChannelsByType(UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType)
{
	...
	//But we added this to be able to specify custom UMoiveSceneScriptingChanngels
	for (const TPair<FGuid, TFunction<bool(UMovieSceneSection* Section, TSubclassOf<UMovieSceneScriptingChannel> ChannelType, TArray<UMovieSceneScriptingChannel*>& OutChannels)>>&
		Function : CustomChannelsCallbacks)
	{
		if (Function.Value(Section, ChannelType, Channels))
		{
			return Channels;
		}
	}
	...
	else if (ChannelType == UMovieSceneScriptingActorReferenceChannel::StaticClass()) { GetScriptingChannelsForChannel<FMovieSceneActorReferenceChannel, UMovieSceneScriptingActorReferenceChannel>(ChannelProxy, Sequence, Section, Channels); }
...
UMovieSceneScriptingChannel* UMovieSceneSectionExtensions::GetChannel(UMovieSceneSection* Section, const FName& ChannelName)
{
	...
	else if (ChanneHandle.GetChannelTypeName() == FMovieSceneActorReferenceChannel::StaticStruct()->GetFName())
	{
		return GetScriptingChannelForChannel<FMovieSceneActorReferenceChannel, UMovieSceneScriptingActorReferenceChannel>(MovieSceneChannel, ChannelProxy, ChanneHandle, Sequence, Section, ChannelName);
	}
...
int32 UMovieSceneSectionExtensions::GetParentSequenceFrame(UMovieSceneSubSection* InSubSection, int32 InFrame, UMovieSceneSequence* ParentSequence)
{
	...
	{
		UE_LOGF(LogMovieScene, Error, "UMovieSceneSectionExtension::GetParentSequenceFrame failed because either sub section or parent sequence is null! SubSection: %ls ParentSequence: %ls", InSubSection ? *InSubSection->GetFullName() : TEXT("None"), ParentSequence ? *ParentSequence->GetFullName() : TEXT("None"));
		return InFrame;
	}
