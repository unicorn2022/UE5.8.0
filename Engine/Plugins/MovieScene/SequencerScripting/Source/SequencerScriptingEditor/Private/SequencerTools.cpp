...
DECLARE_LOG_CATEGORY_CLASS(LogSequencerTools, Log, All);
...
namespace SequencerToolsPrivate
{
	...
	void WaitUntil(const FCheckCanCompleteDelegate Condition, const TFunction<void()> Callback, const UWorld* World, const float Interval = 0.1f)
	{
		...
		{
			UE_LOGF(LogSequencerTools, Error, "WaitUntil() - One of the parameters is invalid.");
			return;
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (LevelSequenceCapture)
	{
		...
		{
			// UE_LOGF(LogSequencerTools, Warning, "No Level Sequence Asset specified in UAutomatedLevelSequenceCapture.");
...
bool USequencerToolsFunctionLibrary::ExportAnimSequenceWaitForDelegate(UWorld* World, ULevelSequence* Sequence, UAnimSequence* AnimSequence, UAnimSeqExportOption* ExportOptions, const FMovieSceneBindingProxy& Binding, bool bCreateLink, const FCheckCanCompleteDelegate Delegate)
{
	...
	auto CompleteExport = [Player, Binding, AnimSequence, RootToLocalTransform, Sequence, ExportOptions, World, OutActor, bCreateLink]() 
	{
		...
		if (Player && AnimSequence && Sequence && ExportOptions && World)
		{
			...
			{
				UE_LOGF(LogSequencerTools, Warning, "USequencerToolsFunctionLibrary::ExportAnimSequence: No skel mesh found in Sequencer");
				bResult = false;
			}
		...
		{
			UE_LOGF(LogSequencerTools, Warning, "USequencerToolsFunctionLibrary::ExportAnimSequence: One or more of the completion lambda's captured parameters are invalid.");
			bResult = false;
		}
			...
			if (LevelAnimLink)
			{
				for (FLevelSequenceAnimSequenceLinkItem& LevelAnimLinkItem : LevelAnimLink->AnimSequenceLinks)
				{
						...
						ExportOptions->bUseCustomFrameRate, ExportOptions->CustomFrameRate))
					{
						...
						LevelAnimLinkItem.bSetRetargetSourceAsset = ExportOptions->bSetRetargetSourceAsset;
						...
						LevelAnimLinkItem.bSkipCurvesWithZeroValue = ExportOptions->bSkipCurvesWithZeroValue;
						...
						LevelAnimLinkItem.bRemoveExcludedCurves = ExportOptions->bRemoveExcludedCurves;
