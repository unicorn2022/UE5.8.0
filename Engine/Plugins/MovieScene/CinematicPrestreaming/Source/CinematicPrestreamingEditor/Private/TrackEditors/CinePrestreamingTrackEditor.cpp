	...
	, public TSharedFromThis<FCinePrestreamingSection>
{
	...
	FText GetVisibilityText() const
	{
		...
		if (Section && Section->GetPrestreamingAsset())
		{
			return FText::FromString(FString::Printf(TEXT("Asset: %s"), *Section->GetPrestreamingAsset()->GetName()));
		}
	...
	FText GetLayerBarText() const
	{
		...
		if (Section)
		{
			if (const UCinePrestreamingData* PrestreamingData = Section->GetPrestreamingAsset())
			{
...
TSharedPtr<SWidget> FCinePrestreamingTrackEditor::BuildOutlinerEditWidget(const FGuid& ObjectBinding, UMovieSceneTrack* Track, const FBuildEditWidgetParams& Params)
{
	return UE::Sequencer::MakeAddButton(
		...
		FOnGetContent::CreateSP(this, &FCinePrestreamingTrackEditor::BuildAddPrestreamingMenu, Track),
		Params.ViewModel);
}
...
void FCinePrestreamingTrackEditor::HandleAddTrack()
{
	...
	const FScopedTransaction Transaction(LOCTEXT("AddCinePrestreamingTrack_Transaction", "Add Cinematic Prestreaming Track"));
...
TSharedRef<SWidget> FCinePrestreamingTrackEditor::BuildAddPrestreamingMenu(UMovieSceneTrack* PrestreamingTrack)
{
