...
#include "MovieRenderPipelineCoreModule.generated.h"
...
/**
 * Naming Tokens for Movie Render Pipeline.
 */
UCLASS(NotBlueprintable)
class UMovieRenderPipelineNamingTokens final: public UNamingTokens
{
	GENERATED_BODY()

public:
	/** The namespace that all MRG tokens are included under within the Naming Tokens system. */
	static inline const FString TokenNamespace = TEXT("mrg");
	
	UMovieRenderPipelineNamingTokens()
	{
		Namespace = TokenNamespace;
		NamespaceDisplayName = FText::FromString(TEXT("Movie Render Graph"));
		bIsPrivateNamespace = true;
	}
	
	virtual ~UMovieRenderPipelineNamingTokens() override
	{
	}
};
...
private:
	...
	/** Setup bindings to MRQ Naming Tokens. */
	void BindToNamingTokenEvents();

	/** Executes every time Naming Tokens are fetched. */
	void OnMovieRenderPipelineNamingTokensPreEvaluate(const FNamingTokensEvaluationData& InEvaluationData);

private:
	/** The GUID that the Naming Tokens system has assigned to the MRQ naming tokens. */
	static FGuid NamingTokensGuid;

	/** Naming Tokens object for MRQ. */
	static TWeakObjectPtr<UNamingTokens> NamingTokens;
...
{
public:
	virtual ~IMoviePipelineBurnInExtension() = default;
