	...
	: public TSharedFromThis<FMoviePipelineSurfaceReader, ESPMode::ThreadSafe>
{
	...
	*/
	UE_API void ResolveSampleToReadbackTexture_RenderThread(FRHICommandListImmediate& RHICmdList, const FTextureRHIRef& SourceSurfaceSample);

	UE_DEPRECATED(5.8, "ResolveSampleToReadbackTexture_RenderThread now requires the immediate RHI command list.")
	void ResolveSampleToReadbackTexture_RenderThread(const FTextureRHIRef& SourceSurfaceSample)
	{
		ResolveSampleToReadbackTexture_RenderThread(FRHICommandListImmediate::Get(), SourceSurfaceSample);
	}

	/**
	* Maps the ReadbackTexture to the CPU (which should have been resolved to before this point) and copies the data over.
	*/
	UE_API void CopyReadbackTexture_RenderThread(FRHICommandListImmediate& RHICmdList, TUniqueFunction<void(TUniquePtr<FImagePixelData>&&)>&& InFunctionCallback, TSharedPtr<FImagePixelDataPayload, ESPMode::ThreadSafe> InFramePayload);

	UE_DEPRECATED(5.8, "CopyReadbackTexture_RenderThread now requires the immediate RHI command list.")
	...
	{
		CopyReadbackTexture_RenderThread(FRHICommandListImmediate::Get(), MoveTemp(InFunctionCallback), InFramePayload);
	}
