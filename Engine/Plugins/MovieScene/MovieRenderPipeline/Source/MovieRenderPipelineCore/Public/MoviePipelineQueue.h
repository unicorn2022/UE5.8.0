...
class UMoviePipelineQueueProjectSettings : public UObject
{
	GENERATED_BODY()
...
public:
	/** When a queue finishes rendering, this determines the action that should be taken (if any). */
	UPROPERTY(Config, BlueprintReadWrite, EditAnywhere, Category = "Movie Render Pipeline")
	EMoviePipelinePostRenderActionType PostRenderActionType = EMoviePipelinePostRenderActionType::DoNothing;
};

/** The active configuration mode for a Movie Render Queue job. */
UENUM(BlueprintType)
enum class EMoviePipelineConfigMode : uint8
{
	/** Legacy UMoviePipelinePrimaryConfig preset. */
	Preset,

	/** Graph-based UMovieGraphConfig configuration. */
	Graph,

	/** Simplified UMoviePipelineBasicConfig configuration. */
	Basic
};
...
public:
	...
	*/
	...
	/**
	 * Gets the graph-style preset that this shot is using.
	 * A non-null graph config does not imply the graph config is the config that's in use;
	 * reference GetConfigMode() on the job for the config type that's active.
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	...
	* See UMoviePipelineExecutorJob's version of this function for more details.
	* 
	...
	*/
	...
	/** Returns true if the parent job is using basic configuration mode. */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	UE_API bool IsUsingBasicConfiguration() const;

	/**
	 * Gets the basic shot config overrides. Returns nullptr if no shot overrides are set.
	 * A non-null Basic config override does not imply the Basic config is the config that's in use;
	 * reference the job's GetConfigMode() for the config type that's active.
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	UMoviePipelineBasicConfig* GetBasicShotConfig() const { return BasicShotConfig; }

	/** Creates and returns basic shot overrides for this shot. */
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API UMoviePipelineBasicConfig* SetupBasicShotOverrides();

	/** Returns true if console variable overrides can be used in the active configuration. */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	UE_API bool CanUseConsoleVariableOverrides() const;

	/** Returns whether this should be rendered. */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
...
public:
	...
	/**
	 * (Optional) Console variable overrides which are applied after cvars set via nodes. Applies to graph-based and Basic configurations.
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Console Variable Overrides", meta = (DisplayName = "Console Variables", EditCondition="CanUseConsoleVariableOverrides()", EditConditionHides))
...
private:
	...
	/** The basic configuration overrides for this shot. Non-null indicates shot-level overrides in Basic configuration mode. */
	UPROPERTY(Instanced)
	TObjectPtr<UMoviePipelineBasicConfig> BasicShotConfig;
};

DECLARE_MULTICAST_DELEGATE_TwoParams(FOnMoviePipelineJobGraphPresetChanged, UMoviePipelineExecutorJob*, UMovieGraphConfig*);
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnMoviePipelineJobConfigModeChanged, UMoviePipelineExecutorJob*, EMoviePipelineConfigMode);
...
public:	
	...
	*/
	 ...
	 * 
	 * Note that a non-null configuration does not imply that this Preset is the active configuration;
	 * reference GetConfigMode() for the config type that's active.
	 * 
	 * @see GetPresetOrigin()
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	...
	/** Returns the active configuration mode for this job. */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	EMoviePipelineConfigMode GetConfigMode() const { return ConfigMode; }

	/**
	 * Switches the active configuration mode on the job.
	 * For Basic mode, lazily initializes the BasicConfig if it doesn't exist yet.
	 * For Graph mode, requires a graph preset to already be assigned (returns false otherwise). See SetGraphPreset().
	 * @return true if the mode was switched (or was already the requested mode), false if the switch was not possible.
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API bool SetConfigMode(EMoviePipelineConfigMode InConfigMode);

	/**
	 * Returns true if this job is using graph-style configuration, else false.
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	bool IsUsingGraphConfiguration() const { return ConfigMode == EMoviePipelineConfigMode::Graph; }

	/** Returns true if this job is using basic configuration mode. */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	bool IsUsingBasicConfiguration() const { return ConfigMode == EMoviePipelineConfigMode::Basic; }

	/**
	 * Gets the basic configuration object, or nullptr if basic configuration has not been set up.
	 * A non-null Basic config does not imply the Basic config is the config that's in use;
	 * reference GetConfigMode() for the config type that's active.
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
	UMoviePipelineBasicConfig* GetBasicConfig() const { return BasicConfig; }

	/**
	 * Sets up basic configuration mode for this job. Note that this will cause the job to switch over to
	 * using the Basic configuration. Returns the created basic config object.
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Render Pipeline")
	UE_API UMoviePipelineBasicConfig* SetupBasicConfiguration();
	...
	/**
	 * Returns true if this job's legacy preset config has been seeded with the project's default
	 * settings. Typically used to gate one-time defaults seeding when switching into Preset mode,
	 * so settings the user has deliberately removed aren't re-added on subsequent mode switches.
	 */
	bool HasInitializedLegacyDefaults() const { return bHasInitializedLegacyPresetDefaults; }

	/**
	 * Marks this job's legacy preset config as having been seeded with the project's default settings,
	 * which disables the one-time defaults seeding that would otherwise run on the next Basic/Graph ->
	 * Preset switch. Typically only called by EnsureJobHasDefaultSettings (which manages the flag as
	 * part of its idempotency gate), but may be useful in niche cases where external code manages the
	 * configuration lifecycle directly and needs to coordinate with that gate.
	 */
	void MarkLegacyPresetDefaultsAsInitialized() { bHasInitializedLegacyPresetDefaults = true; }

	/**
	 * Gets the graph-style preset assigned to this job.
	 * A non-null graph config does not imply the graph config is the config that's in use;
	 * reference GetConfigMode() for the config type that's active.
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Render Pipeline")
...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, BlueprintSetter = "SetSequence", Category = "Movie Render Pipeline", meta = (AllowedClasses = "/Script/LevelSequence.LevelSequence", DisplayThumbnail = false))
	FSoftObjectPath Sequence;

	/** Which map should this job render on */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Movie Render Pipeline", meta = (AllowedClasses = "/Script/Engine.World", DisplayThumbnail = false))
	FSoftObjectPath Map;
	...
	*/
	...
	/** Called when the job's configuration mode changes (Basic, Graph, Preset). */
	FOnMoviePipelineJobConfigModeChanged OnJobConfigModeChanged;
...
private:
	...
	*/
	...
	/**
	 * The graph-based configuration preset that this job is using. Can be nullptr. Typically, this should be set with
	 * SetGraphPreset() in order to keep the ConfigMode in sync. May be non-null even when not in Graph mode
	 * (it's preserved across mode switches). Check ConfigMode to determine the active configuration.
	 */
	UPROPERTY()
	TSoftObjectPtr<UMovieGraphConfig> GraphPreset;
	...
	/** The basic configuration for this job. May be non-null even when not in Basic mode (preserved across mode switches). */
	UPROPERTY(Instanced)
	TObjectPtr<UMoviePipelineBasicConfig> BasicConfig;

	/** The active configuration mode for this job. Determines which config object is in use. */
	UPROPERTY()
	EMoviePipelineConfigMode ConfigMode = EMoviePipelineConfigMode::Preset;

	/**
	 * Tracks whether defaults have been applied to this job's legacy configuration. Set when the
	 * defaults are seeded at creation, or on first Basic/Graph -> Preset switch. Prevents re-adding
	 * settings the user has removed on subsequent mode switches. By convention this is written via
	 * MarkLegacyPresetDefaultsAsInitialized().
	 */
	UPROPERTY()
	bool bHasInitializedLegacyPresetDefaults = false;
};
