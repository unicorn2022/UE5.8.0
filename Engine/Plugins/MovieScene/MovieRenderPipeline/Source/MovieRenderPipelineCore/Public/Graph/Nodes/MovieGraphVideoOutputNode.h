...
namespace MoviePipeline { struct FAudioState; }
...
namespace MovieRenderGraph
{
	...
	{
		/**
		 * Resolves per-shot audio recordings from the audio renderer into shot-aligned int16 buffers and invokes
		 * InWriteSampleCallback once per shot, in increasing shot-index order.
		 *
		 * The main submix has ~24 ms of recording latency, so each shot's recording is offset: shot 0 is short by L,
		 * and shot N>0's first L samples are actually shot (N-1)'s tail. This helper redistributes samples from
		 * shot[N+1]'s head onto shot[N]'s tail until each shot matches its expected length, then truncates each shot
		 * to its expected length before passing it to InWriteSampleCallback. The deficit accumulates onto the last
		 * shot, which ends ~24 ms early.
		 *
		 * @param InAudioData              Audio state captured by the audio renderer (provides per-shot FinishedSegments).
		 * @param InLightweightSourceData  Per-shot writer source data (provides SubmittedFrameCount).
		 * @param InOutputFrameRate        Video frame rate used to size each shot's expected sample count.
		 * @param InWriteSampleCallback    Invoked once per shot with the channel-interleaved int16 buffer.
		 */
		UE_API void WriteShotAudioSamples(
