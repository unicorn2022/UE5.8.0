...
namespace UE::MovieGraph::DefaultRenderer
{
	struct FMovieGraphTilingParams
	{
		...
		FVector2f OverlapPad = FVector2f::ZeroVector;

		// The size of the backbuffer in pixels.
		FIntPoint TileSize = FIntPoint(0, 0);

		// The index of this tile.
		FIntPoint TileIndexes = FIntPoint(0, 0);
	...
	struct FSurfaceAccumulatorPool : public TSharedFromThis<FSurfaceAccumulatorPool>
	{
		...
		FSurfaceAccumulatorPool::FInstancePtr GetAccumulatorInstance_GameThread(int32 InFrameNumber, const FMovieGraphRenderDataIdentifier& InPassIdentifier)
		{
			...
			if (AvailableIndex == INDEX_NONE)
			{
				...
				UE_LOGF(LogMovieRenderPipeline, Log, "Allocated a Accumulator for Pool %ls, New Pool Count: %d", *AccumulatorType::GetName().ToString(), Accumulators.Num());
			}
...
public:
	...
	/**
	 * Fetches information for the given camera index.
	 * First, if InCameraSource is provided with a non-negative camera index, its information is used.
	 * Otherwise, it is resolved against default sequence camera or the shot's sidecar camera data.
	 * @param InCameraIndex  Pass INDEX_NONE when using default sequence camera.
	 *                       Pass [0..N) for multi-camera rendering (shot's sidecar or custom source).
	 * @param InCameraSource Optional camera source. If provided but the camera index cannot be
	 *                       resolved, falls back to the default sequence camera.
	 */
	UE_API UE::MovieGraph::DefaultRenderer::FCameraInfo GetCameraInfo(const int32 InCameraIndex, const FMovieGraphRenderCameraSource* InCameraSource = nullptr) const;
	 ...
	 *                       resolved, falls back to the default sequence camera.
	 */
	UE_API float GetCameraOverscan(int32 InCameraIndex, const FMovieGraphRenderCameraSource* InCameraSource = nullptr);
...
public:
	...
	FMoviePipelineAccumulatorPoolPtr GetOrCreateAccumulatorPool()
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "Allocated a Accumulator Pool for AccumulatorType: %ls", *AccumulatorType::GetName().ToString());
	...
	/** Updates telemetry as needed. Generally should only be called by the pipeline. */
	UE_API void UpdateRenderPassTelemetry(FMoviePipelineEndShotRenderTelemetryContext& InOutTelemetry) const;
...
protected:
	...
	*/
	...
	UE_API UE::MovieGraph::DefaultRenderer::FCameraInfo GetCameraInfoImpl(const int32 InCameraIndex) const;
	/**
	 ...
	 */
	UE_API float GetCameraOverscanImpl(int32 InCameraIndex);
...
protected:
	...
	/** Telemetry gathered from live render pass instances before they are torn down. */
	FMoviePipelineEndShotRenderTelemetryContext CachedRenderPassTelemetry;
