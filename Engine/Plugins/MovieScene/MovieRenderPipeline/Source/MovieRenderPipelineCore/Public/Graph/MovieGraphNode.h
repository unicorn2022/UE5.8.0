...
public:
	...
	/**
	 * Gets the dynamic properties as a view. The view provides a scripting-friendly way of getting and modifying the values
	 * within the dynamic properties.
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	MOVIERENDERPIPELINECORE_API UMovieGraphMutableValueView* GetDynamicPropertiesView() const;
	...
	*/
	...
	/**
	 * Allows subclasses to provide a custom serialized default value when a property is promoted
	 * to a variable. Called during "Promote to Variable" before writing the variable's default.
	 * Return true and set OutSerializedValue to override; return false for normal behavior.
	 */
	UFUNCTION(BlueprintCallable, Category = "Movie Graph")
	MOVIERENDERPIPELINECORE_API virtual bool GetCustomPromotionDefaultValue(const FMovieGraphPropertyInfo& InPropertyInfo, FString& OutSerializedValue) const;
...
protected:
	...
	/** A property view which points to the dynamic properties on this node. */
	UPROPERTY(Transient)
	mutable TObjectPtr<UMovieGraphMutableValueView> DynamicPropertiesView = nullptr;
...
public:
	...
	*/
	 ...
	 *
	 * If the properties resolved here should use the token autocomplete widget in the details panel, include the MrgTokenAutocomplete
	 * UPROPERTY metadata on them (eg, `meta=(MrgTokenAutocomplete)`). Note that this metadata will only take effect for top-level properties
	 * (ie, properties that are directly declared on the node); properties that are embedded in structs, for example, will not be detected.
	 */
