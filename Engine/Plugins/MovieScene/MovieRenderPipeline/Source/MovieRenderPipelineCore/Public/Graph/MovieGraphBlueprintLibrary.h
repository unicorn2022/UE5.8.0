...
public:
	...
	*/
	...
	/**
	 * Returns the current play world (PIE or Game). Useful when a Blueprint does not have a world context object,
	 * such as in MovieGraphScriptBase callbacks. Returns nullptr if no play world is active.
	 */
	UFUNCTION(BlueprintPure, Category = "Movie Graph", meta = (DisplayName = "Get Current Play World"))
	static UE_API UWorld* GetCurrentPlayWorld();
