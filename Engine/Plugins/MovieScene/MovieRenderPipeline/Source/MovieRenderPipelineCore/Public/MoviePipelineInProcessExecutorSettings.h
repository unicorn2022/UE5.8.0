...
public:
	...
	{
		bCloseEditor = false;
		// -AudioCallbackBufferFrameSize/-AudioNumBuffersToEnqueue: match the buffer config that PIE applies to its
		// per-world NRT device (LevelEditorPlaySettings::bUseNonRealtimeAudioDevice path passes
		// BufferSizeOverride=32, NumBuffersOverride=2 in PlayLevel.cpp).
		AdditionalCommandLineArguments = TEXT("-NoLoadingScreen -FixedSeed -log -Unattended -MRQInstance -deterministicaudio -audiomixer -AudioCallbackBufferFrameSize=32 -AudioNumBuffersToEnqueue=2");
