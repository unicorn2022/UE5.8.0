		...
		OnDebugSampleAvailable = MoveTemp(OnDebugSampleAvailable)](uint32 Slot)
		{
...

			{
				...
				for (TUniquePtr<FPoolEntry>& Item : TempBufferPool)
				{
					...
					{
						FoundPool = Item.Get();
						//UE_LOGF(LogTemp, Log, "Reused pool entry, size: %d,%d newSize: %d oldSize:%d", 
				...
				if (!FoundPool)
				{
					...
					// UE_LOGF(LogTemp, Log, "Allocated new pool entry, size: %d,%d NewPoolCount: %d", Resolution.X, Resolution.Y, TempBufferPool.Num());
				}
