...
#define MRG_LOG_ONCE_PER_RENDER(Identifier, LogCategory, LogVerbosity, LogFormat, ...)									\
	static UWorld* UE_JOIN(__LastRenderWorld__, Identifier) = nullptr;													\
	static TArray<FString> UE_JOIN(__LoggedMessages__, Identifier);														\
																														\
	...
	if (UE_JOIN(__LastRenderWorld__, Identifier) != MoviePipeline::FindCurrentWorld())									\
	{																													\
		UE_JOIN(__LastRenderWorld__, Identifier) = MoviePipeline::FindCurrentWorld();									\
		UE_JOIN(__LoggedMessages__, Identifier).Empty();																\
	}																													\
																														\
	/* If this message hasn't been logged, record that it has been logged so we can skip it for other frames. */		\
	const FString UE_JOIN(__FinalLogMsg__, Identifier) = FString::Printf(LogFormat, ##__VA_ARGS__);						\
	if (!UE_JOIN(__LoggedMessages__, Identifier).Contains(UE_JOIN(__FinalLogMsg__, Identifier)))						\
	{																													\
		UE_LOGF(LogCategory, LogVerbosity, "%ls", *UE_JOIN(__FinalLogMsg__, Identifier));							\
		UE_JOIN(__LoggedMessages__, Identifier).Add(UE_JOIN(__FinalLogMsg__, Identifier));								\
	}																													\
...
namespace MoviePipeline
{
	...
	/**
	 * Given a non-templated file name/path, returns the "frame-templated" file path. A frame-templated file path is one that has had its
	 * {frame_*} token, if any, replaced with the {frame_placeholder} token. Returns the original file path if no modifications were done.
	 */
	MOVIERENDERPIPELINECORE_API FString GetFrameTemplatedFilePath(const FString& InFilePath);
