...
void UMoviePipelineAntiAliasingSetting::UpdateTelemetry(FMoviePipelineShotRenderTelemetry* InTelemetry) const
{
	...
	InTelemetry->WarmUpFrames = FMath::Max(InTelemetry->WarmUpFrames, FMath::Max(EngineWarmUpCount, 0));
	InTelemetry->AntiAliasingType = UEnum::GetValueAsString(UE::MovieRenderPipeline::GetEffectiveAntiAliasingMethod(this));
}
