	...
	if (ProcessHandle.IsValid())
	{
		...
		for (const FString& Line : SplitStrings)
		{
			...
			if (bCorrectStart && LineContents[1] != TEXT("="))
			{
				...
				UE_LOGF(LogMovieRenderPipeline, Log, "%ls CodecName: %-20ls Codec Description: %ls", *CodecType, *LineContents[1], *CombinedDesc);
				NumCodecsDumped++;
			}
		}

		UE_LOGF(LogMovieRenderPipeline, Log, "Finished dumping audio and video codecs. Please select a video and audio codec from the list above for the settings.");
