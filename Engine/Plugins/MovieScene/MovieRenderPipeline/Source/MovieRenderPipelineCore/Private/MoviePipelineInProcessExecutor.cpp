...
void UMoviePipelineInProcessExecutor::Start(const UMoviePipelineExecutorJob* InJob)
{
	...
	{		
		if (!World)
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Unable to start movie pipeline job. No current map.");
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Current map '%ls' does not match job's map: '%ls'. Rendering may happen on a different map than expected and produce incorrect results.", *GetNameSafe(World), *InJob->Map.GetAssetPath().ToString());
		}
		
		UE_LOGF(LogMovieRenderPipeline, Log, "Starting %ls", *GetNameSafe(World));
	}
	...
	if (!LevelSequence)
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Failed to load Sequence specified by job: %ls", *InJob->Sequence.ToString());
	...
	else
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "About to load target map %ls", *LevelPath);
...
{
	UE_LOGF(LogMovieRenderPipeline, Log, "Finished map load %ls", *GetNameSafe(NewWorld));
	...
	{
		FCoreDelegates::OnBeginFrame.RemoveAll(this);
		UE_LOGF(LogMovieRenderPipeline, Error, "Out of bounds Job Index (%d) in Queue (Length: %d)!", CurrentPipelineIndex, Queue->GetJobs().Num());
	...
	// Force the pipeline to the default graph pipeline if we're using a graph or basic configuration and the pipeline isn't graph-compatible
	if ((CurrentJob->IsUsingGraphConfiguration() || CurrentJob->IsUsingBasicConfiguration())
		&& !MoviePipelineClass->IsChildOf(UMovieGraphPipeline::StaticClass()))
	{
	...
	// Wait until we actually received the right map and created the pipeline before saying that we're actively rendering
	bIsRendering = true;
	
	if (ExecutorSettings->InitialDelayFrameCount == 0)
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Zero Initial Delay, initializing...");
		if (PipelineAsLegacy)
		{
...
void UMoviePipelineInProcessExecutor::OnTick()
{
	if (RemainingInitializationFrames >= 0)
	{
		if (RemainingInitializationFrames == 0)
		{
			UE_LOGF(LogMovieRenderPipeline, Log, "Delay finished, initializing...");
			UMoviePipeline* PipelineAsLegacy = Cast<UMoviePipeline>(ActiveMoviePipeline);
			if (PipelineAsLegacy)
			{
				PipelineAsLegacy->Initialize(Queue->GetJobs()[CurrentPipelineIndex]);
...
void UMoviePipelineInProcessExecutor::OnApplicationQuit()
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "MoviePipelineInProcessExecutor: Application quit while Movie Pipeline was still active. Stalling to do full shutdown.");
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "MoviePipelineInProcessExecutor: Stalling finished, pipeline has shut down.");
	}
}
