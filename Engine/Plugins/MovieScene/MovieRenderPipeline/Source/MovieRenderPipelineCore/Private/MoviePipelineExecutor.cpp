...
bool UMoviePipelineExecutorBase::ConnectSocket(const FString& InHostName, const int32 InPort)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "ConnectSocket called while existing socket connected, call DisconnectSocket first if you wish to change socket hosts. Ignoring request.");
		return false;
	}
	...
	if (!bIsValid)
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Could not parse IP address for socket.");
		return false;
	}
...
bool UMoviePipelineExecutorBase::SendSocketMessage(const FString& InMessage)
{
	if (!IsSocketConnected())
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Attempted to send a socket message but no socket connected, ignoring...");
		return false;
	}
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Verbose, "SendSocketMessage size write failed with code %d", (int32)ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetLastErrorCode());
		return false;
	}
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Verbose, "SendSocketMessage write failed with code %d", (int32)ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetLastErrorCode());
		return false;
	}
...
bool UMoviePipelineExecutorBase::ProcessIncomingSocketData()
{
	...
	while(true)
	{
		...
		if (RecvMessageDataRemaining == 0)
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Verbose, "In progress read failed with code %ls", *GetReadableErrorCode());
				return false;
			}
			...
			if (RecvMessageDataRemaining <= 0)
			{
				UE_LOGF(LogMovieRenderPipeline, Error, "Read failed due to invalid Message Size: %d", RecvMessageDataRemaining);
				return false;
			}
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Verbose, "Read failed with code %ls", *GetReadableErrorCode());
			return false;
		}
...
void UMoviePipelineExecutorBase::SetPostRenderAction_Implementation(const EMoviePipelinePostRenderActionType PostRenderAction)
{
	if (GetSupportsPostRenderActions())
	{
		PostRenderActionType = PostRenderAction;
	}
	else
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Attempting to set a post-render action on an executor that does not support them.");
	}
}
