...
void UMoviePipelineGameOverrideSetting::SetupForPipelineImpl(UMoviePipeline* InPipeline)
{
	...
	UE_LOGF(LogMovieRenderPipeline, Log, "Applying Game Override quality settings and cvars.");
...
void UMoviePipelineGameOverrideSetting::TeardownForPipelineImpl(UMoviePipeline* InPipeline)
{
	...
	UE_LOGF(LogMovieRenderPipeline, Log, "Restoring Game Override quality settings and cvars.");
	ApplyCVarSettings(false);
}

void UMoviePipelineGameOverrideSetting::UpdateTelemetry(FMoviePipelineShotRenderTelemetry* InTelemetry) const
{
	InTelemetry->ScalabilityQualityLevel = bCinematicQualitySettings ? TEXT("Cinematic") : TEXT("Default");
}
...
void UMoviePipelineGameOverrideSetting::ApplyCVarSettings(const bool bOverrideValues)
{
	...
	// Ensures that submixes will always render silence instead of disabling to maintain timing in the sequence
	MOVIEPIPELINE_STORE_AND_OVERRIDE_CVAR_BOOL(PreviousNeverDisableSubmixes, TEXT("au.NeverDisableSubmixes"), 1, bOverrideValues);
	...
	// Enable the alpha channel throughout the renderer
	...
	// Improve (combined) LUT precision to avoid quantization errors, especially with multi-sample accumulation
	MOVIEPIPELINE_STORE_AND_OVERRIDE_CVAR_INT(PreviousLUTSize, TEXT("r.LUT.Size"), 64, bOverrideValues);
...
void UMoviePipelineGameOverrideSetting::BuildNewProcessCommandLineArgsImpl(TArray<FString>& InOutUnrealURLParams, TArray<FString>& InOutCommandLineArgs, TArray<FString>& InOutDeviceProfileCvars, TArray<FString>& InOutExecCmds) const
{
	...
	InOutDeviceProfileCvars.AddUnique(FString::Printf(TEXT("au.NeverDisableSubmixes=%d"), 1));
	...
	InOutDeviceProfileCvars.AddUnique(FString::Printf(TEXT("r.LUT.Size=%d"), 64));
}
