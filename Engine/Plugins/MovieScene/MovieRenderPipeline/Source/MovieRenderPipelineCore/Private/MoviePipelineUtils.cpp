...
namespace MoviePipeline
{
	...
	FString GetFrameTemplatedFilePath(const FString& InFilePath)
	{
		static const FString FramePlaceholder = TEXT("{frame_placeholder}");

		FString FrameTemplatedFormatString = InFilePath;
		if (FrameTemplatedFormatString.Contains(TEXT("{frame_number}")))
		{
			FrameTemplatedFormatString = InFilePath.Replace(TEXT("{frame_number}"), *FramePlaceholder);
		}
		else if (FrameTemplatedFormatString.Contains(TEXT("{frame_number_rel}")))
		{
			FrameTemplatedFormatString = InFilePath.Replace(TEXT("{frame_number_rel}"), *FramePlaceholder);
		}
		else if (FrameTemplatedFormatString.Contains(TEXT("{frame_number_shot}")))
		{
			FrameTemplatedFormatString = InFilePath.Replace(TEXT("{frame_number_shot}"), *FramePlaceholder);
		}
		else if (FrameTemplatedFormatString.Contains(TEXT("{frame_number_shot_rel}")))
		{
			FrameTemplatedFormatString = InFilePath.Replace(TEXT("{frame_number_shot_rel}"), *FramePlaceholder);
		}

		return FrameTemplatedFormatString;
	}
...
namespace MoviePipeline
{
	...
	void SaveOrRestoreSubSectionHierarchy(TSharedPtr<FCameraCutSubSectionHierarchyNode> InLeaf, const bool bInSave)
	{
		...
		while (Node)
		{
			// Use the playback range as a sentinel: if it has open bounds, save was never called
			// for this node (e.g. the pipeline shut down before hierarchy was saved). Skip the
			// entire restoration to avoid writing default/uninitialized values back.
			if (!bInSave && Node->MovieScene.IsValid()
				&& !ensureMsgf(Node->OriginalMovieScenePlaybackRange.GetLowerBound().IsClosed() && Node->OriginalMovieScenePlaybackRange.GetUpperBound().IsClosed(),
					TEXT("Skipping restoration for MovieScene '%s' — cached range has open bounds (pipeline shut down before hierarchy was saved?)."), *GetNameSafe(Node->MovieScene.Get())))
			{
				Node = Node->GetParent();
				continue;
			}
	...
	void SetSubSectionHierarchyActive(TSharedPtr<FCameraCutSubSectionHierarchyNode> InRoot, bool bInActive)
	{
		...
		while (Node)
		{
			...
			if (Node->CameraCutSection.IsValid())
			{
				...
				UE_LOGF(LogMovieRenderPipeline, Verbose, "Disabled CameraCutSection: %ls while soloing shot.", *GetNameSafe(Node->CameraCutSection.Get()));
			}
			...
			if (Node->Section.IsValid())
			{
				...
				UE_LOGF(LogMovieRenderPipeline, Verbose, "Disabled SubSequenceSection: %ls while soloing shot.", *GetNameSafe(Node->Section.Get()));
			}
			...
			for (const UMovieSceneSection* Section : Node->MovieScene->GetAllSections())
			{
				...
				if (Section->GetRange().HasLowerBound())
				{
					...
					if (bOverlaps)
					{
						...
						UE_LOGF(LogMovieRenderPipeline, Warning, "[%ls %ls] Due to Temporal sub-sampling or handle frames, evaluation will occur outside of shot boundaries (from frame %d to %d). Section %ls (Binding: %ls) starts during this time period and cannot be auto-expanded. Please extend this section to start on frame %d. (All times listed are relative to the root sequence)",
...
namespace UE
{
	namespace MoviePipeline
	{
		...
		void ConformOutputFormatStringToken(FString& InOutFilenameFormatString, const FStringView InToken, const FName& InNodeName, const FName& InBranchName)
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Warning, "Missing expected %ls format token on node '%ls' in branch '%ls'. Automatically adding!", InToken.GetData(), *InNodeName.ToString(), *InBranchName.ToString());
		...
		void ValidateOutputFormatString(FString& InOutFilenameFormatString, const bool bTestRenderPass, const bool bTestFrameNumber, const bool bTestCameraName)
		{
			...
			if (bTestRenderPass)
			{
				...
				{
					UE_LOGF(LogMovieRenderPipeline, Warning, "Multiple render passes exported but no {render_pass} format found. Automatically adding!");
			...
			if (bTestCameraName)
			{
				...
				{
					UE_LOGF(LogMovieRenderPipeline, Warning, "Multiple cameras exported but no {camera_name} format found. Automatically adding!");
			...
			if (bTestFrameNumber)
			{
				...
				if (FrameNumberIndex == INDEX_NONE)
				{
					...
					UE_LOGF(LogMovieRenderPipeline, Warning, "Frame number identifier not found. Files may be overwritten.");
				}
				else
				{
		...
		void GetHardwareUsageMetadata(TMap<FString, FString>& InFileMetadata, const FString& InOutputDir)
		{
			...
			InFileMetadata.Add(TEXT("unreal/system/deviceTemp"), LexToString(FPlatformMisc::GetDeviceTemperatureLevel())); // Deprecated in 5.8 in favor of the two separate stats below.
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			InFileMetadata.Add(TEXT("unreal/system/deviceTemperature"), LexToString(FPlatformMisc::GetDeviceTemperature()));
			InFileMetadata.Add(TEXT("unreal/system/deviceThermalState"), LexToString(static_cast<int>(FPlatformMisc::GetDeviceThermalState())));
		...
		void GetMetadataFromCineCamera(UCineCameraComponent* InComponent, const FString& InPrefix, TMap<FString, FString>& InOutMetadata)
		{
			...
			InOutMetadata.Add(FString::Printf(TEXT("%s/squeezeFactor"), *InPrefix), FString::SanitizeFloat(InComponent->LensSettings.SqueezeFactor));
		}
		...
		{
			for (IInterface_PostProcessVolume& PPVolume : InWorld->GetPostProcessVolumeIterator())
			{
