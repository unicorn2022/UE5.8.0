...
void UMoviePipelineBlueprintLibrary::UpdateJobShotListFromSequence(ULevelSequence* InSequence, UMoviePipelineExecutorJob* InJob, bool& bShotsChanged)
{
	...
	struct FSequenceRangeVisitor : ISequenceVisitor
	{
		// Add one root-space section range to the evaluation tree. All callers funnel through
		// this helper so normal inverse-transform results and fallback ranges preserve the
		// same section, sequence-id, and hierarchical-bias semantics.
		void AddRangeToTree(UMovieSceneSection* InSection, const TRange<FFrameNumber>& InRootRange, const FSubSequenceSpace& LocalSpace, TMovieSceneEvaluationTree<FSequenceRangeInfo>* InTree)
		{
			// Empty intersections are valid outcomes when a parent sequence clips a child section
			// completely out of the root evaluation range; those sections should not create shots.
			if (InRootRange.IsEmpty())
			{
				return;
			}

			if (const UMovieSceneCinematicShotSection* ShotSection = Cast<UMovieSceneCinematicShotSection>(InSection))
			{
				// Shot sections represent their child sequence, while camera cuts belong to the
				// currently visited sequence. Store the child sequence ID so the later hierarchy
				// walk can resolve the correct shot/subsequence instead of the parent container.
				InTree->Add(InRootRange, MakeTuple(InSection, ShotSection->GetSequenceID(), LocalSpace.HierarchicalBias));
			}
			else
			{
				// Non-shot sections, currently camera cuts for this visitor, are already authored
				// in the visited sequence's space, so LocalSpace.SequenceID is the owning sequence.
				InTree->Add(InRootRange, MakeTuple(InSection, LocalSpace.SequenceID, LocalSpace.HierarchicalBias));
			}
		}
		...
		void GatherFromSection(UMovieSceneTrack* InTrack, const UE::MovieScene::FSubSequenceSpace& LocalSpace, TMovieSceneEvaluationTree<FSequenceRangeInfo>* InTree)
		{
			...
			for (UMovieSceneSection* Section : InTrack->GetAllSections())
			{
				...
				bool bVisitedRange = false;
				auto Visit = [this, &LocalSpace, Section, InTree, &bVisitedRange](TRange<FFrameTime> InRange)
				{
					bVisitedRange = true;
					...
					{
						AddRangeToTree(Section, RootCameraRange, LocalSpace, InTree);
					}
					return true;
				};
				...
				// Time-warp curves can make inverse range solving fail even though the section is still editorially active.
				// Fall back to the visited root clamp range so the shot remains visible in MRQ/MRG.
				if (!bVisitedRange)
				{
					AddRangeToTree(Section, LocalSpace.RootClampRange, LocalSpace, InTree);
				}
			}
		}
...

	{
		...
		for (FLinearizedEntity& Entity : Entities)
		{
			...
			if (LeafNode->CameraCutSection.IsValid() && LeafNode->MovieScene.IsValid())
			{
				...
				for (int32 Index = 0; Index < LeafNode->MovieScene->GetPossessableCount(); Index++)
				{
					...
					if (ValidBinding.IsValid())
					{
						...
						{
							// Bypass if the binding is muted
							if (FMovieSceneBinding* Binding = LeafNode->MovieScene->FindBinding(ParentGuid))
							{
								if (UMovieSceneMuteSoloDecoration* MuteSoloDecoration = Cast<UMovieSceneMuteSoloDecoration>(Binding->FindDecoration<UMovieSceneMuteSoloDecoration>()))
								{
									if (MuteSoloDecoration->IsMuted())
									{
										bAddCamera = false;
										UE_LOGF(LogMovieRenderPipeline, Log, "Skipping muted camera: %ls", *Possessable.GetName());
										break;
									}
								}
							}
							...
							if (FMovieScenePossessable* ParentAsPossessable = LeafNode->MovieScene->FindPossessable(ParentGuid))
							{
								...
								if (MovieSceneHelpers::IsBoundToAnySpawnable(LeafNode->MovieScene->GetTypedOuter<UMovieSceneSequence>(), ParentAsPossessable->GetGuid(), TransientPlaybackState))
								{
									...
									if (Binding)
									{
										...
										{
											// Bypass if the spawn track is disabled
											if (Track->IsEvalDisabled())
											{
												bAddCamera = false;
												UE_LOGF(LogMovieRenderPipeline, Log, "Skipping disabled camera: %ls", *ParentAsPossessable->GetName());
												continue;
											}
							...
							else
							{
								...
								UE_LOGF(LogMovieRenderPipeline, Error, "A camera binding was found while calculating multi-camera support, but the binding had an invalid parent (ParentGuid %ls), skipping...", *ParentGuid.ToString());
								break;
							}
						}
			...
			if (LeafNode->CameraCutSection.IsValid())
			{
				...
				bool bConvertedCameraCutRange = false;
				auto Visit = [&CameraCutRangeInRoot, &bConvertedCameraCutRange](TRange<FFrameTime> InRange)
				{
					CameraCutRangeInRoot = ConvertToDiscreteRange(InRange);
					bConvertedCameraCutRange = true;
				...
				// If a time-warped camera cut cannot be mapped back to root space, use the
				// already-root-space entity range so warmup math does not mix coordinate spaces.
				if (!bConvertedCameraCutRange)
				{
					CameraCutRangeInRoot = EntityRangeInRoot;
				}
			}
		...
		for (FLinearizedEntity& Entity : Entities)
		{
			...
			for (UMoviePipelineExecutorShot* Shot : InJob->ShotInfo)
			{
				if (!Shot)
				{
					UE_LOGF(LogMovieRenderPipeline, Warning, "Null ShotInfo in job, ignoring...");
					continue;
				}
			...
			if (!NewShot)
			{
				...
				UE_LOGF(LogMovieRenderPipeline, Log, "Generated new ShotInfo for Inner: %ls Outer: %ls (No existing shot found in the job).", *Entity.Name.Get<0>(), *Entity.Name.Get<1>());
			}
			else
			{
				UE_LOGF(LogMovieRenderPipeline, Log, "Reusing existing ShotInfo for Inner: %ls Outer: %ls.", *Entity.Name.Get<0>(), *Entity.Name.Get<1>());
			...
			UE_LOGF(LogMovieRenderPipeline, Log, "Registering range: %ls (InnerName: %ls OuterName: %ls)", *LexToString(NewShot->ShotInfo.TotalOutputRangeRoot), *NewShot->InnerName, *NewShot->OuterName);
		}
...
void UMoviePipelineBlueprintLibrary::ResolveFilenameFormatArguments(const FString& InFormatString, const FMoviePipelineFilenameResolveParams& InParams, FString& OutFinalPath, FMoviePipelineFormatArgs& OutMergedFormatArgs)
{
	...
	if (InParams.ShotOverride && InParams.ShotOverride->GetShotOverrideConfiguration())
	{
		...
		for (UMoviePipelineSetting* Setting : ShotConfig->GetUserSettings())
		{
			if (!Setting)
			{
				UE_LOGF(LogMovieRenderPipeline, Error, "Null setting found in config: %ls - Did you disable a plugin that contained this setting?", *GetNameSafe(ShotConfig));
				continue;
			}
