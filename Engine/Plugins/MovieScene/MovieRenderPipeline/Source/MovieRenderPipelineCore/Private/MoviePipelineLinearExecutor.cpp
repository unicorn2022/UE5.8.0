...
void UMoviePipelineLinearExecutorBase::Execute_Implementation(UMoviePipelineQueue* InPipelineQueue)
{
	if (!InPipelineQueue)
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Executor asked to execute a null queue, not supported!");
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Executor asked to execute on empty list of pipelines.");
	...
	UE_LOGF(LogMovieRenderPipeline, Log, "MoviePipelineLinearExecutorBase starting %d jobs.", InPipelineQueue->GetJobs().Num());
...
void UMoviePipelineLinearExecutorBase::StartPipelineByIndex(int32 InPipelineIndex)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Found null config in list of configs to render. Aborting the pipeline processing!");
	...
	UE_LOGF(LogMovieRenderPipeline, Log, "MoviePipelineLinearExecutorBase starting job [%d/%d]", CurrentPipelineIndex + 1, Queue->GetJobs().Num());
...
{
	UE_LOGF(LogMovieRenderPipeline, Log, "MoviePipelineLinearExecutorBase finished %d jobs in %ls.", JobsStarted, *(FDateTime::UtcNow() - InitializationTime).ToString());
