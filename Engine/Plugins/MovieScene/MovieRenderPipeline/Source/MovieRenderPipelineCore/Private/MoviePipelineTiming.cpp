...
void UMoviePipeline::ProcessOutstandingFutures()
{
	...
	for (int32 Index = 0; Index < OutputFutures.Num(); ++Index)
	{
		...
		FMoviePipelineRenderPassOutputData& OutputData = ShotOutputData->RenderPassData.FindOrAdd(FutureData.PassIdentifier);
		...
		FMoviePipelineStringArray& FilesFromSettingClass = OutputData.SettingTypeToFilePaths.FindOrAdd(FutureData.OriginSettingClass);
		FilesFromSettingClass.Array.Add(FutureData.FilePath);
...
void UMoviePipeline::TickProducingFrames()
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "[GFrameCounter: %lld] Async Shutdown Requested, abandoning remaining work and moving to Finalize.", GFrameCounter);
	...
	if (CurrentCameraCut->ShotInfo.State == EMovieRenderShotState::Uninitialized)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "[%lld] Initializing Camera Cut [%d/%d] in [%ls] %ls.", GFrameCounter,
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "[%lld] Finished initializing Camera Cut [%d/%d] in [%ls] %ls.", GFrameCounter,
			CurrentShotIndex + 1, ActiveShotList.Num(), *CurrentCameraCut->OuterName, *CurrentCameraCut->InnerName);
	}
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Verbose, "[%lld] Evaluating WarmUp frame %d. Remaining: %d", GFrameCounter, CurrentCameraCut->ShotInfo.NumEngineWarmUpFramesRemaining, CurrentCameraCut->ShotInfo.NumEngineWarmUpFramesRemaining - 1);
		...
		// On the last non-emulated warm-up frame, flag the shot so the renderer
		// issues a sequencer jump to reset MetaSounds, whose internal state advanced 
		// while the sequence was playing during warm-up.
		// The emulated path already triggers a jump via the existing !IsPlaying() branch.
		if (CurrentCameraCut->ShotInfo.NumEngineWarmUpFramesRemaining == 0
			&& !CurrentCameraCut->ShotInfo.bEmulateFirstFrameMotionBlur)
		{
			CurrentCameraCut->ShotInfo.bNeedsAudioResetOnRenderStart = true;
		}
		...
		UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "[%lld] Shot WarmUp set engine DeltaTime to %f seconds.", GFrameCounter, FrameDeltaTime);
	...
	if (CurrentCameraCut->ShotInfo.State == EMovieRenderShotState::MotionBlur)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Verbose, "[%lld] Shot MotionBlur set engine DeltaTime to %f seconds.", GFrameCounter, FrameDeltaTime);
	...
	if (CurrentCameraCut->ShotInfo.State == EMovieRenderShotState::Rendering)
	{
		...
		if (!FMath::IsNearlyEqual(WorldTimeDilation, 1.f))
		{
			UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "[%lld] Modified FrameDeltaTime by a factor of %f to account for World Time Dilation.", GFrameCounter, WorldTimeDilation);
		...
		*/
		...
		FFrameTime FinalEvalTime = FrameMetrics.GetFinalEvalTime(CurrentCameraCut->ShotInfo.CurrentTickInRoot);
		UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "[%lld] FinalEvalTime: %ls (Tick: %d SOffset: %d MOffset: %d)", 
		...
		if (!LevelSequenceActor->GetSequencePlayer()->IsPlaying()
			|| CurrentCameraCut->ShotInfo.bNeedsAudioResetOnRenderStart)
		{
			...
			//
			// bNeedsAudioResetOnRenderStart covers the non-emulated warm-up path where the sequence was already playing:
			// the jump causes the audio system to issue StopAndPlay for all sounds, resetting procedural generators
			// (e.g. MetaSound) whose state advanced during warm-up, before recording opens.
			CurrentCameraCut->ShotInfo.bNeedsAudioResetOnRenderStart = false;
		...
		if (DeltaFrameTime.GetFrame() == FFrameNumber(0))
		{
			...
			UE_LOGF(LogMovieRenderPipeline, Error, "Too many temporal samples for the given shutter angle/tick rate combination. Temporal Samples: %d Shutter Angle: %f TicksPerOutputFrame: %ls TicksPerSample: %ls. Consider converting to Spatial Samples instead!",
		...
		{
			// If this output frame started but cannot reach its final temporal sample, don't let it consume an output frame number.
			if (CachedOutputState.TemporalSampleIndex > 0 && CachedOutputState.ShotOutputFrameNumber >= 0)
			{
