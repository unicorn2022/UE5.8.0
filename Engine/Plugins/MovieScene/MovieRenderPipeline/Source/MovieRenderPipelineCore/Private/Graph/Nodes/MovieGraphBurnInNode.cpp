...
TObjectPtr<UMovieGraphBurnInWidget> UMovieGraphBurnInNode::FMovieGraphBurnInPass::GetBurnInWidget(UMovieGraphWidgetRendererBaseNode* InNodeThisFrame) const
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "The burn-in widget provided in layer '%ls' for renderer '%ls' is not valid.", *LayerData.BranchName.ToString(), *Renderer->GetClass()->GetName());
		return nullptr;
	}
	...
	if (!BurnInWidget)
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "Unable to load burn-in widget at path: %ls", *BurnInWidgetClass->GetPathName());
		}
		else
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "Unable to load burn-in widget (nullptr)");
		}
		return nullptr;
	}
