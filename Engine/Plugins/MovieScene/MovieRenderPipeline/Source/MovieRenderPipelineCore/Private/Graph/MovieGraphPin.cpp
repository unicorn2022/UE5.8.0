...
bool UMovieGraphPin::AddEdgeTo(UMovieGraphPin* InOtherPin)
{
	if (!InOtherPin)
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "AddEdgeTo: Invalid InOtherPin");
		return false;
	}
...
bool UMovieGraphPin::BreakEdgeTo(UMovieGraphPin* InOtherPin)
{
	if (!InOtherPin)
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "BreakEdgeTo: Invalid InOtherPin");
		return false;
	}
...
TArray<UMovieGraphPin*> UMovieGraphPin::GetAllConnectedPins() const
{
	...
	for (UMovieGraphEdge* Edge : Edges)
	{
		if (!IsValid(Edge))
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "GetAllConnectedPins() - Found an invalid edge. This is typically caused by a node that is from a plugin that is not currently loaded.");
			continue;
		}
