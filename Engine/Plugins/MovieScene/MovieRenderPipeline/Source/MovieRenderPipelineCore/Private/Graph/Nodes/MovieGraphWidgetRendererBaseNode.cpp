...
void UMovieGraphWidgetRendererBaseNode::FMovieGraphWidgetPass::Render(const FMovieGraphTraversalContext& InFrameTraversalContext, const FMovieGraphTimeStepData& InTimeData)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Resolution %dx%d exceeds maximum allowed by GPU. Widget renders (burn-ins, etc) do not support high-resolution tiling and thus can't exceed %dx%d.", OutputResolution.X, OutputResolution.Y, MaxResolution, MaxResolution);
		return;
	}
	if (OutputResolution.X == 0 || OutputResolution.Y == 0)
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Resolution %dx%d must be greater than zero in both dimensions.", OutputResolution.X, OutputResolution.Y);
		return;
	}
