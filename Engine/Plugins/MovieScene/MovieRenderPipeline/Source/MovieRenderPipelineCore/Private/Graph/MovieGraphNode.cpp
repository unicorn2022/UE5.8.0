...
bool UMovieGraphNode::GetCustomPromotionDefaultValue(const FMovieGraphPropertyInfo& InPropertyInfo, FString& OutSerializedValue) const
{
	return false;
}
...
UMovieGraphMutableValueView* UMovieGraphNode::GetDynamicPropertiesView() const
{
	// Lazily instantiate the view
	if (!DynamicPropertiesView)
	{
		UMovieGraphNode* MutableThis = const_cast<UMovieGraphNode*>(this);
		DynamicPropertiesView = NewObject<UMovieGraphMutableValueView>(MutableThis, TEXT("DynamicPropertiesView"));
		DynamicPropertiesView->ViewProperties(&MutableThis->DynamicProperties);
	}

	return DynamicPropertiesView.Get();
}
