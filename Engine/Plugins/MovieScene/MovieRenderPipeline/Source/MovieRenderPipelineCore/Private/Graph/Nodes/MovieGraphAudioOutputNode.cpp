...
FText UMovieGraphAudioOutputNode::GetBasicConfigShortDisplayName() const
{
	return NSLOCTEXT("MovieGraphNodes", "BasicConfigShortName_WAV", "WAV");
}
#endif // WITH_EDITOR
...
bool UMovieGraphAudioOutputNode::NeedsPerShotFlushing() const
{
	...
	const UMovieGraphAudioOutputNode* AudioNode =
		EvaluatedGraph->GetSettingForBranch<UMovieGraphAudioOutputNode>(GlobalsPinName, bIncludeCDOs, bExactMatch);
	
	const FString FullPath = OutputSettingNode->OutputDirectory.Path / AudioNode->FileNameFormat;
