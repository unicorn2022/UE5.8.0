...
void UMovieGraphGlobalGameOverridesNode::UpdateTelemetry(FMoviePipelineShotRenderTelemetry* InTelemetry) const
{
	InTelemetry->ScalabilityQualityLevel = StaticEnum<EMovieGraphScalabilityQualityLevel>()->GetNameStringByValue(static_cast<int64>(ScalabilityQualityLevel));
}
...
void UMovieGraphGlobalGameOverridesNode::ApplySettings(const bool bOverrideValues, UWorld* InWorld)
{
	...
	// user needing to customize these (or be aware of them) is very low.
	{
		...
		MOVIEPIPELINE_STORE_AND_OVERRIDE_CVAR_INT(NodeCDO->PreviousLUTSize, TEXT("r.LUT.Size"), 64, bOverrideValues);
	}
...
TSubclassOf<AGameModeBase> UMovieGraphGlobalGameOverridesNode::GetGameModeOverride(const UMoviePipelineExecutorJob* InJob)
{
	...
	{
		// Basic configuration does not support game mode overrides.
		return nullptr;
	}
	else
	{
