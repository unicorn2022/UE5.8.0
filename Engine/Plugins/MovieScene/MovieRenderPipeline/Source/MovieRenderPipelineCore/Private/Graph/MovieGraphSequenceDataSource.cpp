...
void UMovieGraphSequenceDataSource::OnSequenceEvaluated(const UMovieSceneSequencePlayer& Player, FFrameTime CurrentTime, FFrameTime PreviousTime)
{
	...
	UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "Sequence Evaluated. CurrentTime: %ls PreviousTime: %ls", *LexToString(CurrentTime), *LexToString(PreviousTime));
}
...
TArray<UE::MovieGraph::FMinimalCameraInfo> UMovieGraphSequenceDataSource::GetCameraInformation(UMoviePipelineExecutorShot* InShot, bool bIncludeSidecar) const
{
	...
	{
		// Keep one result per sidecar so caller indices match SidecarCameras.
		OutInfo.SetNum(InShot->SidecarCameras.Num());
...
UMovieSceneSequencePlayer* UMovieGraphSequenceDataSource::GetSequencePlayer() const
{
	if (LevelSequenceActor)
	{
