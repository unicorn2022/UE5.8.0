...
namespace
{
	...
	/** Removes all "mrg:" namespacing from {tokens} in the format string. For example, replaces {mrg:branch_name} with {branch_name}. */
	void RemoveMrgTokenNamespacing(FString& InOutFormatString)
	{
		static const FString MrgNamespace = FString::Printf(TEXT("%s:"), *UMovieRenderPipelineNamingTokens::TokenNamespace);
		
		for (const FString& IncludedTokenName : GetTokensFromString(InOutFormatString))
		{
			if (IncludedTokenName.StartsWith(MrgNamespace))
			{
				const FString TokenNameWithoutNamespace = IncludedTokenName.Replace(*MrgNamespace, TEXT(""));
				const FString SourceToken = FString::Printf(TEXT("{%s}"), *IncludedTokenName);
				const FString DestToken = FString::Printf(TEXT("{%s}"), *TokenNameWithoutNamespace);
				
				InOutFormatString.ReplaceInline(*SourceToken, *DestToken, ESearchCase::CaseSensitive);
			}
		}
	}
	...
	FString ResolveFormatArgumentsHelper(const FString& InFormatString, const FMovieGraphFilenameResolveParams& InParams, const bool bResolveToPath, FMovieGraphResolveArgs& OutMergedFormatArgs)
	{
		...
		// Data from the UMovieGraphPipeline comes from the resolve params themselves to allow resolving without a runtime instance.
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Warning, "Time Dilation was used but output format does not use relative time, forcing relative numbers. Change {frame_number} to {frame_number_rel} (or the shot version) to remove this message.");
			}
		...
		// The "mrg:" namespace is technically allowed. However, for the purposes of resolving, we need to strip off this namespace because the token
		// names discovered above do not include this namespace, and the Naming Tokens system will not know how to resolve these either.
		RemoveMrgTokenNamespacing(FormatStringCopy);
		...
		const FString ResolvedExtension = (Extension == ExtToken || Extension == TEXT(".")) ? FString() : Extension;
		FString ThisTry = FormattedString + ResolvedExtension;
...
UWorld* UMovieGraphBlueprintLibrary::GetCurrentPlayWorld()
{
	if (!GEngine)
	{
		return nullptr;
	...
	// Fallback: search engine world contexts for a PIE or Game world.
	// This covers early MRQ callbacks (e.g., OnJobStart) where
	// GetCurrentPlayWorld() may return null because the world context
