...
void FMovieGraphCVarManager::ApplyAllCVars()
{
	...
	for (int32 CVarIndex = 0; CVarIndex < CVars.Num(); ++CVarIndex)
	{
		...
		if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(*TrimmedCvar))
		{
			...
			UE_LOGF(LogMovieRenderPipeline, Log, "Applying CVar \"%ls\", PreviousValue: %f NewValue: %f", *CVarEntry.Name, PreviousConsoleVariableValues[CVarIndex], CVarEntry.Value);
		}
		else
		{
...
void FMovieGraphCVarManager::RevertAllCVars()
{
	for (int32 CVarIndex = 0; CVarIndex < CVars.Num(); ++CVarIndex)
	{
		...
		if (IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(*TrimmedCvar))
		{
			...
			UE_LOGF(LogMovieRenderPipeline, Log, "Restoring CVar \"%ls\", PreviousValue: %f NewValue: %f", *CVarEntry.Name, CVarEntry.Value, PreviousConsoleVariableValues[CVarIndex]);
		}
		else
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to revert CVar \"%ls\" due to no cvar by that name. Ignoring.", *CVarEntry.Name);
		}
	}
...
void FMovieGraphCVarManager::RunStartConsoleCommands()
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Executing Console Command \"%ls\" before shot starts.", *StartCommand);
...
void FMovieGraphCVarManager::RunEndConsoleCommands()
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Executing Console Command \"%ls\" after shot ends.", *EndCommand);
