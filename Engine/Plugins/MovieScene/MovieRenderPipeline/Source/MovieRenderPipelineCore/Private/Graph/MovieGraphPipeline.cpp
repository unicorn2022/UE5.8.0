...
{
	UE_LOGF(LogMovieRenderPipeline, Log, "Initializing MovieGraph Render");
	...
	if (!ensureAlwaysMsgf(InJob->GetGraphPreset() || InJob->IsUsingBasicConfiguration(), TEXT("MovieGraphPipeline cannot be initialized with a null job configuration. Make sure you've assigned a Graph or Basic config to this job. Aborting.")))
	{
		//Shutdown(true);
		return;
	}
	...
	CurrentJob = InJob;
	bWasInitializedWithBasicConfig = CurrentJob->IsUsingBasicConfiguration();
	CurrentShotIndex = 0;
	...
	FString OutError;
	UMovieGraphConfig* GraphPreset = GetCurrentJob()->IsUsingGraphConfiguration() ? GetCurrentJob()->GetGraphPreset() : nullptr;
	UMovieGraphEvaluatedConfig* FlattenedConfig = GraphPreset ? GraphPreset->CreateFlattenedGraph(Context, OutError) : nullptr;

	// Lay groundwork for Unreal Insights; this is not supported with Basic configs.
	if (FlattenedConfig)
	{
	...
	else
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Error, "Unable to initialize Movie Pipeline. The following error(s) were encountered: %ls", *OutError);
	}
	...
	//		UE_LOGF(LogMovieRenderPipeline, Warning, "Sub-level '%ls' is set to blueprint streaming and will not be visible during a render unless a Sequencer Visibility Track controls its visibility or you have written other code to handle loading it.",
	//			*NonPrefixedLevelName);
	//	}
	//}

	// ToDo: Print some information, Job Name, Level Sequence, etc.
	UE_LOGF(LogMovieRenderPipeline, Log, "Finished initializing MovieGraph Render");

	const bool bNoShotsToRender = ActiveShotList.Num() == 0;
	if (bNoShotsToRender)
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "No shots detected to render. Either all outside playback range, or disabled via shot mask. Shutting down.");
	}
...
void UMovieGraphPipeline::DuplicateJobAndConfiguration()
{
	...
	// Convert Basic config to Graph on the transient duplicate (before the existing duplication logic processes it)
	if (CurrentJobDuplicate->IsUsingBasicConfiguration())
	{
		// Generate job-level graph from the basic config
		UMovieGraphConfig* GeneratedGraph = UMoviePipelineBasicConfig::GenerateGraph(
			CurrentJobDuplicate->GetBasicConfig(), CurrentJobDuplicate);

		if (!GeneratedGraph)
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "DuplicateJobAndConfiguration: Failed to generate graph from Basic config. The job will have no graph configuration.");
		}

		// Pre-populate the duplication map so DuplicateConfigRecursive skips these already-transient graphs
		if (GeneratedGraph)
		{
			DuplicatedGraphs.Add(GeneratedGraph, GeneratedGraph);
		}

		// Generate per-shot graphs (each shot gets a merged config of job + shot overrides)
		for (UMoviePipelineExecutorShot* Shot : CurrentJobDuplicate->ShotInfo)
		{
			if (const UMoviePipelineBasicConfig* ShotOverrides = Shot->GetBasicShotConfig())
			{
				UMoviePipelineBasicConfig* MergedConfig = NewObject<UMoviePipelineBasicConfig>(Shot);
				CurrentJobDuplicate->GetBasicConfig()->MergeOverrides(ShotOverrides, *MergedConfig);
				UMovieGraphConfig* ShotGraph = UMoviePipelineBasicConfig::GenerateGraph(MergedConfig, Shot);
				Shot->SetGraphPreset(ShotGraph, false);

				if (ShotGraph)
				{
					DuplicatedGraphs.Add(ShotGraph, ShotGraph);
				}
			}
		}

		// Set the job graph after shot graphs are set
		CurrentJobDuplicate->SetGraphPreset(GeneratedGraph, false);
	}
	...
	// Note: We use CurrentJobDuplicate here (not CurrentJob) so that Basic-config-generated graphs are also processed.
	UMovieGraphConfig* DuplicatePrimaryConfig = MoviePipeline::DuplicateConfigRecursive(CurrentJobDuplicate->GetGraphPreset(), DuplicatedGraphs);
	...
	for (int32 Index = 0; Index < CurrentJobDuplicate->ShotInfo.Num(); Index++)
	{
		// Now for each shot we need to duplicate its config (if any)
		// Note: We use CurrentJobDuplicate here (not CurrentJob) so that Basic-config-generated graphs are also processed.
		if (UMovieGraphConfig* ShotGraphPreset = CurrentJobDuplicate->ShotInfo[Index]->GetGraphPreset())
		{
...
void UMovieGraphPipeline::LoadPreviewWidget()
{
	...
	if (PreviewWidgetClassToUse.Get() != nullptr)
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to create Preview Screen UMG Widget. No in-game overlay available.");
		}
	}
	else
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to find Preview Screen UMG Widget class. No in-game overlay available.");
	}
}
		...
		for (const IMovieGraphModifierNodeInterface* ModifierNode : ModifierNodes)
		{
			...
			for (UMovieGraphModifierBase* ModifierInstance : ModifierNode->GetAllModifiers())
			{
				...
				for (const FName& ModifiedCollectionName : ModifierNode->GetAllCollections())
				{
					...
					{
						UE_LOGF(LogMovieRenderPipeline, Warning, "The modifier '%ls' specified a collection '%ls', but the collection couldn't be found.",
...
void UMovieGraphPipeline::BuildShotListFromDataSource()
{
	...
	for (int32 ShotIndex = 0; ShotIndex < GetCurrentJob()->ShotInfo.Num(); ShotIndex++)
	{
		...
		else
		{
			...
			UE_LOGF(LogMovieRenderPipeline, Warning, "The shot '%ls' specified a time step of type '%ls', but it could not be loaded. Defaulting to linear.",
...
void UMovieGraphPipeline::OnEngineTickBeginFrame()
{
	...
	UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "MovieGraph OnEngineTickBeginFrame");
...
void UMovieGraphPipeline::TickProducingFrames()
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "MovieGraph TickProducingFrames: Async Shutdown Requested, abandoning remaining work and moving to Finalize.");
...
void UMovieGraphPipeline::TickPostFinalizeExport(const bool bInForceFinish)
{
	...
	UE_LOGF(LogMovieRenderPipeline, Verbose, "[%lld] PostFinalize Export (Start).", GFrameCounter);
	...
	UE_LOGF(LogMovieRenderPipeline, Verbose, "[%lld] PostFinalize Export (End).", GFrameCounter);
...
void UMovieGraphPipeline::StartUnrealInsightsCapture(UMovieGraphEvaluatedConfig* EvaluatedConfig)
{
	...
	if (!ensureAlwaysMsgf(DebugSetting, TEXT("Failed to find UMovieGraphDebugSettingNode. Aborting.")))
	{
		return;
	}

	const FString FinalFilePath = UE::MovieGraph::Capture::PrepareCapturePath(this, EvaluatedConfig, DebugSetting->UnrealInsightsTraceFileNameFormat, TEXT("utrace"), false);
	...
	if (bTraceStarted)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "Started capturing UnrealInsights trace file to %ls", *FinalFilePath);
	}
	else
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to start capturing UnrealInsights trace. Is there already a trace session in progress?");
	}
}
...
{
	BeginShotSetup(InShot);
	FinishShotSetup(InShot);
}

void UMovieGraphPipeline::BeginShotSetup(const TObjectPtr<UMoviePipelineExecutorShot>& InShot)
{
	CurrentShotStartOutputFrameNumber = GetCustomEngineTimeStep()->SharedTimeStepData.OutputFrameNumber;
...
void UMovieGraphPipeline::FinishShotSetup(const TObjectPtr<UMoviePipelineExecutorShot>& InShot)
{
	const FMovieGraphTimeStepData& TimeStepData = GetTimeStepInstance()->GetCalculatedTimeData();
	UMovieGraphEvaluatedConfig* EvaluatedConfig = TimeStepData.EvaluatedConfig;
	...
	FMoviePipelineBeginShotRenderTelemetryContext BeginShotTelemetry;
	BeginShotTelemetry.Shot = InShot;
	BeginShotTelemetry.EvaluatedConfig = EvaluatedConfig;
	BeginShotTelemetry.bIsBasicConfig = bWasInitializedWithBasicConfig;
	FMoviePipelineTelemetry::SendBeginShotRenderTelemetry(BeginShotTelemetry);
}
...
void UMovieGraphPipeline::TeardownShot(const TObjectPtr<UMoviePipelineExecutorShot>& InShot)
{
	...
	const int32 CurrentShotEndOutputFrameNumber = GetCustomEngineTimeStep()->SharedTimeStepData.OutputFrameNumber;
	FMoviePipelineEndShotRenderTelemetryContext EndShotTelemetry;
	EndShotTelemetry.bIsGraph = true;
	EndShotTelemetry.bIsBasicConfig = bWasInitializedWithBasicConfig;
	EndShotTelemetry.bWasSuccessful = !bShutdownSetErrorFlag;
	EndShotTelemetry.bWasCanceled = bShutdownRequested;
	EndShotTelemetry.TotalRenderedFrameCount = FMath::Max(0, CurrentShotEndOutputFrameNumber - CurrentShotStartOutputFrameNumber);

	if (const UMovieGraphDefaultRenderer* DefaultRenderer = Cast<UMovieGraphDefaultRenderer>(GraphRendererInstance))
	{
		DefaultRenderer->UpdateRenderPassTelemetry(EndShotTelemetry);
	}

	FMoviePipelineTelemetry::SendEndShotRenderTelemetry(EndShotTelemetry);
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "MovieGraph Finished rendering last shot. Moving to Finalize to finish writing items to disk.");
		TransitionToState(EMovieRenderPipelineState::Finalize);
	}
...
void UMovieGraphPipeline::SetSoloShot(const TObjectPtr<UMoviePipelineExecutorShot>& InShot)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Expanding Shot %d/%d (Shot: %ls Camera: %ls)", CurrentShotIndex + 1, ActiveShotList.Num(), *InShot->OuterName, *InShot->InnerName);
		...
		ExpandShot(InShot, OutputNode->HandleFrameCount, bExpandForTemporalSubSample, bIsPrePass, FinalFrameRate, TickResolution, InShot->ShotInfo.NumEngineWarmUpFramesRemaining);
	}
}
	...
	const FFrameRate& InDisplayRate, const FFrameRate& InTickResolution, const int32 InWarmUpFrames)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Shot/Camera \"%ls [%ls]\" starts on a sub-frame. Rendered range has been rounded to the previous frame to make output numbers align with Sequencer.", 
...
void UMovieGraphPipeline::OnEngineTickEndFrame()
{
	...
	UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "MovieGraph OnEngineTickEndFrame (Start)");

	GraphAudioRendererInstance->ProcessAudioTick();
	
	RenderFrame();

	UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "MovieGraph OnEngineTickEndFrame (End)");
}
...
void UMovieGraphPipeline::RequestShutdownImpl(bool bIsError)
{
	...
	case EMovieRenderPipelineState::Finalize:
	case EMovieRenderPipelineState::Export:
		UE_LOGF(LogMovieRenderPipeline, Log, "MovieGraph Async Shutdown Requested, ignoring due to already being on the way to shutdown.");
		break;
	}
}
...
void UMovieGraphPipeline::TransitionToState(const EMovieRenderPipelineState InNewState)
{
	...
	case EMovieRenderPipelineState::ProducingFrames:
		if (InNewState == EMovieRenderPipelineState::Finalize)
		{
			...
			// We also check that the shot was actually initialized to avoid restoring hierarchy state
			// that was never saved. This can happen when PIE ends on the same frame as pipeline
			// initialization, before the first tick runs InitializeShot/SetSoloShot.
			if (bShutdownRequested && (CurrentShotIndex < ActiveShotList.Num())
				&& ActiveShotList[CurrentShotIndex]->ShotInfo.IsInitialized())
			{
			...
			// during ProducingFrames. The instance may be null if the pipeline was shut down
			// before the first TickProducingFrames assigned it.
			if (GetTimeStepInstance())
			{
			...
			if (!PostRenderEvaluatedGraph)
			{
				UE_LOGF(LogMovieRenderPipeline, Warning, "Generating a post-render evaluated graph failed for job '%ls'. Post-render exports will not run. Evaluation error: %ls", *GetCurrentJob()->JobName, *FlattenError);
			}
	...
	case EMovieRenderPipelineState::Export:
		if (InNewState == EMovieRenderPipelineState::Finished)
		{
			...
			//UE_LOGF(LogMovieRenderPipeline, Log, "Movie Pipeline completed. Duration: %ls", *(FDateTime::UtcNow() - InitializationTime).ToString());
			...
			UE_LOGF(LogMovieRenderPipeline, Log, "Movie Graph Render completed. Duration: %ls", *(FDateTime::UtcNow() - GraphInitializationTime).ToString());
		}
		break;
	}
	...
	if (!ensureAlwaysMsgf(!bInvalidTransition, TEXT("MovieGraph An invalid transition was requested (from: %d to: %d), ignoring transition request."),
		EnumToUnderlyingType(PipelineState), EnumToUnderlyingType(InNewState)))
	{
		return;
	}
}
...
void UMovieGraphPipeline::OnMoviePipelineFinishedImpl()
{
	...
	Params.GraphData = GeneratedOutputData;
	//
	//UE_LOGF(LogMovieRenderPipelineIO, Verbose, "Files written to disk for entire sequence:");
	//PrintVerboseLogForFiles(GeneratedShotOutputData);
	//UE_LOGF(LogMovieRenderPipelineIO, Verbose, "Completed outputting files written to disk.");
	//
...
//		if (Shot)
//		{
//			UE_LOGF(LogMovieRenderPipelineIO, Verbose, "Shot: %ls [%ls]", *Shot->OuterName, *Shot->InnerName);
//		}
//		for (const TPair<FMoviePipelinePassIdentifier, FMoviePipelineRenderPassOutputData>& Pair : OutputData.RenderPassData)
//		{
//			UE_LOGF(LogMovieRenderPipelineIO, Verbose, "Render Pass: %ls", *Pair.Key.Name);
//			for (const FString& FilePath : Pair.Value.FilePaths)
//			{
//				UE_LOGF(LogMovieRenderPipelineIO, Verbose, "\t\t%ls", *FilePath);
//			}
//		}
//	}
//}
...
void UMovieGraphPipeline::ProcessOutstandingFinishedFrames()
{
	while (!OutputMerger->GetFinishedFrames().IsEmpty())
	{
		...
		if (ensure(TimeStats))
		{
			...
			// UE_LOGF(LogTemp, Log, "Frame: %d Duration: %ls", FrameNumber, *DurationTimeStr);
		}
...
void UMovieGraphPipeline::ProcessOutstandingFutures(const bool bStallAndProcessAllFutures)
{
	...
	for (int32 Index = 0; Index < OutstandingOutputFutures.Num(); ++Index)
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "Error exporting frame, canceling movie export.");
...
void UMovieGraphPipeline::BeginDestroy()
{
	...
	if (OutputMerger.IsValid())
	{
		OutputMerger->AbandonOutstandingWork();
	}
