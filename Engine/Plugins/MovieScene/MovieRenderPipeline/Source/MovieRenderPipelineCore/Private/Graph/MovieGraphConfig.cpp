...
namespace
{
	void RecurseUpGlobalsBranchToFindOutputDirectory_Internal(const UMovieGraphConfig* InGraph, const UMovieGraphNode* InNode, FString& OutOutputDirectory, TArray<const UMovieGraphConfig*>& VisitedGraphStack, TSet<const UMovieGraphNode*>& VisitedNodes)
	{
		// If there's no Node, or we've already visited this node, there is no way to continue so we early out.
		if (!InGraph || !InNode || VisitedNodes.Contains(InNode))
		{
			return;
		}
		...
		if (const UMovieGraphGlobalOutputSettingNode* SettingsNode = Cast<UMovieGraphGlobalOutputSettingNode>(InNode))
		{
			if (!SettingsNode->IsDisabled() && SettingsNode->bOverride_OutputDirectory)
			{
		...
		if (!DownstreamGlobalsPin)
		{
			return;
		}

		const UMovieGraphPin* UpstreamGlobalsPin = DownstreamGlobalsPin->GetFirstConnectedPin();
		if (!UpstreamGlobalsPin)
		{
			return;
		}

		const UMovieGraphNode* ConnectedNode = UpstreamGlobalsPin->Node;

		// Overrides can be set within Subgraphs.
		if (const UMovieGraphSubgraphNode* SubgraphNode = Cast<UMovieGraphSubgraphNode>(ConnectedNode); SubgraphNode && !SubgraphNode->IsDisabled())
		{
			const UMovieGraphConfig* SubgraphConfig = SubgraphNode->GetSubgraphAsset();

			// Stop recursing if circular references are found.
			if (SubgraphConfig && !VisitedGraphStack.Contains(SubgraphConfig) && OutOutputDirectory.IsEmpty())
			{
				VisitedGraphStack.Push(SubgraphConfig);
				...
				{
					RecurseUpGlobalsBranchToFindOutputDirectory_Internal(SubgraphConfig, SubgraphGlobalsPin->Node, OutOutputDirectory, VisitedGraphStack, VisitedNodes);
				}

				VisitedGraphStack.Pop(EAllowShrinking::No);
			}
		}
		...
		{
			RecurseUpGlobalsBranchToFindOutputDirectory_Internal(InGraph, UpstreamGlobalsPin->Node, OutOutputDirectory, VisitedGraphStack, VisitedNodes);
		}
	}
}
...
UMovieGraphGlobalVariable_ShotName::UMovieGraphGlobalVariable_ShotName()
{
	...
	// SetValueType() will result in a PostEditChangeProperty call; ensure it's only called on the game thread
	AsyncTask(ENamedThreads::GameThread, [WeakThis = TWeakObjectPtr(this)]()
	{
		if (UMovieGraphGlobalVariable_ShotName* ShotVariable = WeakThis.Get())
		{
			ShotVariable->SetValueType(EMovieGraphValueType::String);
		}
	});
}
...
UMovieGraphGlobalVariable_SequenceName::UMovieGraphGlobalVariable_SequenceName()
{
	...
	AsyncTask(ENamedThreads::GameThread, [WeakThis = TWeakObjectPtr(this)]()
	{
		if (UMovieGraphGlobalVariable_SequenceName* SequenceVariable = WeakThis.Get())
		{
			SequenceVariable->SetValueType(EMovieGraphValueType::String);
		}
	});
}
...
UMovieGraphGlobalVariable_FrameNumber::UMovieGraphGlobalVariable_FrameNumber()
{
	...
	AsyncTask(ENamedThreads::GameThread, [WeakThis = TWeakObjectPtr(this)]()
	{
		if (UMovieGraphGlobalVariable_FrameNumber* FrameVariable = WeakThis.Get())
		{
			FrameVariable->SetValueType(EMovieGraphValueType::Int32);
		}
	});
}
...
UMovieGraphGlobalVariable_CameraName::UMovieGraphGlobalVariable_CameraName()
{
	...
	AsyncTask(ENamedThreads::GameThread, [WeakThis = TWeakObjectPtr(this)]()
	{
		if (UMovieGraphGlobalVariable_CameraName* CameraVariable = WeakThis.Get())
		{
			CameraVariable->SetValueType(EMovieGraphValueType::String);
		}
	});
}
	...
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		AllNodes.RemoveAll(
			[](UMovieGraphNode* Node)
			{
				if (!Node)
				{
					UE_LOGF(LogMovieRenderPipeline, Warning, "Encountered invalid source node (nullptr) when building Movie Pipeline Editor graph, skipping creating an editor graph node for the invalid source node.")
					return true;
				}
				return false;
			});
...
UMovieGraphNode* UMovieGraphConfig::InsertBefore(UMovieGraphNode* InTargetNode, const TSubclassOf<UMovieGraphNode> InNodeClass, const FName& InTargetPinLabel)
{
	if (!IsValid(InTargetNode))
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "[%s]: InTargetNode is not valid.", __FUNCTION__);
		return nullptr;
	}

	if (!InNodeClass)
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "[%s]: InNodeClass is null.", __FUNCTION__);
		return nullptr;
	}

	if (!InTargetNode->GetInputPin(InTargetPinLabel))
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "[%s]: Target node '%ls' (%ls) has no input pin '%ls'.",
			__FUNCTION__, *InTargetNode->GetName(), *InTargetNode->GetClass()->GetName(), *InTargetPinLabel.ToString());
		return nullptr;
	}

	// Save info about the node currently upstream of the target before the connection is broken by AddLabeledEdge
	UMovieGraphNode* UpstreamNode = nullptr;
	FName UpstreamOutputPinLabel;
	if (const UMovieGraphPin* ConnectedPin = InTargetNode->GetInputPin(InTargetPinLabel)->GetFirstConnectedPin())
	{
		UpstreamNode = ConnectedPin->Node;
		UpstreamOutputPinLabel = ConnectedPin->Properties.Label;
	}

	Modify();

	UMovieGraphNode* NewNode = CreateNodeByClass(InNodeClass);
	if (!NewNode)
	{
		return nullptr;
	}

	// Connect the new node to the target; input pins only allow one connection, so this auto-breaks
	// the existing UpstreamNode → InTargetNode edge
	AddLabeledEdge(NewNode, FName(), InTargetNode, InTargetPinLabel);

	// Reconnect the previously upstream node to the new node's input
	if (UpstreamNode)
	{
		AddLabeledEdge(UpstreamNode, UpstreamOutputPinLabel, NewNode, FName());
	}

	return NewNode;
}

UMovieGraphNode* UMovieGraphConfig::InsertAfter(UMovieGraphNode* InTargetNode, const TSubclassOf<UMovieGraphNode> InNodeClass, const FName& InTargetPinLabel)
{
	if (!IsValid(InTargetNode))
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "[%s]: InTargetNode is not valid.", __FUNCTION__);
	...
	if (!InTargetNode->GetOutputPin(InTargetPinLabel))
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "[%s]: Target node '%ls' (%ls) has no output pin '%ls'.",
			__FUNCTION__, *InTargetNode->GetName(), *InTargetNode->GetClass()->GetName(), *InTargetPinLabel.ToString());
		return nullptr;
	}

	// Save info about the node currently downstream of the target so it can be reconnected after insertion.
	// The downstream pin's label is preserved so named branch pins (e.g. on the output node) are handled correctly.
	UMovieGraphNode* DownstreamNode = nullptr;
	FName DownstreamInputPinLabel;
	if (const UMovieGraphPin* ConnectedPin = InTargetNode->GetOutputPin(InTargetPinLabel)->GetFirstConnectedPin())
	{
		DownstreamNode = ConnectedPin->Node;
		DownstreamInputPinLabel = ConnectedPin->Properties.Label;
	}
	...
	// Explicitly break the existing InTargetNode → DownstreamNode edge before reconnecting; output pins allow
	// multiple connections so AddLabeledEdge alone will not remove the old edge
	if (DownstreamNode)
	{
		RemoveLabeledEdge(InTargetNode, InTargetPinLabel, DownstreamNode, DownstreamInputPinLabel);
	}

	// Connect the target to the new node, then reconnect the new node to what was previously downstream
	AddLabeledEdge(InTargetNode, InTargetPinLabel, NewNode, FName());
	if (DownstreamNode)
	{
		AddLabeledEdge(NewNode, FName(), DownstreamNode, DownstreamInputPinLabel);
	}
...
{
	TSet<const UMovieGraphNode*> VisitedNodes;
	const bool bPushedThisGraph = !VisitedGraphStack.Contains(this);
	if (bPushedThisGraph)
	{
		VisitedGraphStack.Push(this);
	}

	RecurseUpGlobalsBranchToFindOutputDirectory_Internal(this, InNode, OutOutputDirectory, VisitedGraphStack, VisitedNodes);
...
UMovieGraphEvaluatedConfig* UMovieGraphConfig::CreateFlattenedGraph(const FMovieGraphTraversalContext& InContext, FString& OutError)
{
		...
		for (UMovieGraphPin* InputPin : OutputNode->GetInputPins())
		{
			...
			for (UMovieGraphEdge* Edge : InputPin->Edges)
			{
				if (!IsValid(Edge))
				{
					...
					UE_LOGF(LogMovieRenderPipeline, Error, "%ls", *InvalidNodeError.ToString());
				...
				if (OtherPin)
				{
					...
					{
						UE_LOGF(LogMovieRenderPipeline, Error, "%ls", *StackContext.TraversalError.ToString());
		...
		for (UMovieGraphSettingNode* InjectedNode : InjectedNodes)
		{
			...
			for (const TObjectPtr<UMovieGraphNode>& ExistingNode : NodesWithInstanceName)
			{
				if (ExistingNode && (ExistingNode->GetClass() == InjectedNode->GetClass()))
				{
					...
					UE_LOGF(LogMovieRenderPipeline, Error, "%ls", *InvalidNodeTypesError.ToString());
	...
	if (!bHasRenderLayerNode)
	{
		...
		UE_CALL_ONCE([] { UE_LOGF(LogMovieRenderPipeline, Error, "For render jobs to succeed, one or more render layer node(s) must be present."); });
	}
