...
TArray<FMovieGraphPinProperties> UMovieGraphSelectNode::GetInputPinProperties() const
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "When generating pins on the Select node, found an invalid options array. Select node will not work as intended.");
		return Properties;
	}
	...
	for (int32 Index = 0; Index < SelectOptionsArray.Num(); ++Index)
	{
		...
		default:
			...
			UE_LOGF(LogMovieRenderPipeline, Warning, "Encountered an unexpected type in the Select node when generating pins: %ls", *EnumType->GetNameStringByValue(static_cast<uint8>(ValueType)));
			break;
		}
	}
...
TArray<UMovieGraphPin*> UMovieGraphSelectNode::EvaluatePinsToFollow(FMovieGraphEvaluationContext& InContext) const
{
	...
	if (!SelectOptionsArrayResult.HasValue() || SelectOptionsArrayResult.HasError())
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "When evaluating pins on the Select node, found an invalid options array. Select node will not work as intended.");
		return PinsToFollow;
	}
	...
	for (int32 Index = 0; Index < SelectOptionsArray.Num(); ++Index)
	{
		...
		default:
			const UEnum* EnumType = StaticEnum<EMovieGraphValueType>();
			UE_LOGF(LogMovieRenderPipeline, Warning, "Encountered an unexpected type in the Select node when following pins: %ls", *EnumType->GetNameStringByValue(static_cast<uint8>(SelectOptions->GetValueType())));
			break;
		}
	}
