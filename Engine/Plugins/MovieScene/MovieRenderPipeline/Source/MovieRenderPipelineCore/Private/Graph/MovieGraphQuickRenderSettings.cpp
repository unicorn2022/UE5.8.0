...
UMovieGraphQuickRenderModeSettings* UMovieGraphQuickRenderSettings::GetSavedQuickRenderModeSettings(const EMovieGraphQuickRenderMode SettingsMode)
{
	...
	const UPackage* QuickRenderSettingsPackage = FindPackage(nullptr, *QuickRenderSettingsPackagePath);
	if (!QuickRenderSettingsPackage)
	{
		QuickRenderSettingsPackage = LoadPackage(nullptr, *QuickRenderSettingsPackagePath, LOAD_None);
	}
...
void UMovieGraphQuickRenderSettings::SaveSettings(const UMovieGraphQuickRenderSettings* InSettings)
{
	...
	if (!bSuccess)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Warning, "Unable to save Quick Render settings. Could not save to destination file [%ls].", *PackageFileName);
	}
}
