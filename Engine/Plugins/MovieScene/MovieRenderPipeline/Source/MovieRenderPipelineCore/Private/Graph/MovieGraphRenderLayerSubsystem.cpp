...
void UMovieGraphRenderPropertyModifier::ApplyModifier(const UWorld* World)
{
	auto CacheComponentState = [this](USceneComponent* InComponent)
	{
		...
		if (const UPrimitiveComponent* AsPrimitiveComponent = Cast<UPrimitiveComponent>(InComponent))
		{
			...
			CachedState.bVisibleInReflections = AsPrimitiveComponent->bVisibleInReflections;
		...
		{
			// General fallback for other USceneComponent types (e.g., lights, decals) that don't inherit
			// from UPrimitiveComponent. Only bIsHidden is applicable for these component types.
			CachedState.bIsHidden = InComponent->bHiddenInGame;
		}
	};
...
void UMovieGraphRenderPropertyModifier::SetActorAndComponentState(const TArray<FActorState>& InActorState, const TArray<FComponentState>& InComponentState, const bool bUseStateFromNode)
{
	...
	auto SetStateForComponent = [this, &SetStateForVolumetrics](UActorComponent* InComponent, const FComponentState& InNewComponentState)
	{
		...
		if (UPrimitiveComponent* AsPrimitiveComponent = Cast<UPrimitiveComponent>(InComponent))
		{
			...
			if (bOverride_bVisibleInReflections)
			{
				// There's no Set*() method for Visible in Reflections, so MarkRenderStateDirty() needs to be called manually
				if (AsPrimitiveComponent->bVisibleInReflections != InNewComponentState.bVisibleInReflections)
				{
					AsPrimitiveComponent->bVisibleInReflections = InNewComponentState.bVisibleInReflections;
					AsPrimitiveComponent->MarkRenderStateDirty();
				}
			}
		...
		else if (USceneComponent* AsSceneComponent = Cast<USceneComponent>(InComponent))
		{
			// General fallback for other USceneComponent types (e.g., lights, decals) that don't inherit
			// from UPrimitiveComponent. Only bIsHidden is applicable for these component types.
			if (bOverride_bIsHidden)
			{
				AsSceneComponent->SetHiddenInGame(InNewComponentState.bIsHidden);
			}
		}
	};
	...
	if (bUseStateFromNode)
	{
		...
		NewComponentState.bVisibleInReflections = bVisibleInReflections;
...
#else
		UE_LOGF(LogMovieRenderPipeline, Warning, "Both \"Alpha Output\" and \"Support Primitive Alpha Holdout\" project settings must be enabled, otherwise holdout modifiers will not work properly.");
#endif
	}
}
...
/**
 * Finds the counterpart of InSourceActor in InTargetWorld by matching the correct sublevel via
 * outermost package name, then looking up the actor by name within that level.
 *
 * This works more reliably than EditorUtilities::GetSimWorldCounterpartActor / GetEditorWorldCounterpartActor
 * which can return actors from the wrong sublevel when actor names collide across sublevels.
 */
static AActor* FindCounterpartActorInWorld(const AActor* InSourceActor, const UWorld* InTargetWorld)
{
	if (!InSourceActor || !InTargetWorld)
	{
		return nullptr;
	}

	const ULevel* SourceLevel = InSourceActor->GetLevel();
	if (!SourceLevel || !SourceLevel->GetOutermost())
	{
		return nullptr;
	}

	// RemovePIEPrefix is a no-op if the name has no PIE prefix, so this works for both directions.
	const FString SourceLevelPackageName = UWorld::RemovePIEPrefix(SourceLevel->GetOutermost()->GetName());

	for (FConstLevelIterator LevelIt(InTargetWorld->GetLevelIterator()); LevelIt; ++LevelIt)
	{
		if (ULevel* TargetLevel = *LevelIt)
		{
			...
			const FString TargetLevelPackageName = UWorld::RemovePIEPrefix(TargetLevel->GetOutermost()->GetName());
			if (TargetLevelPackageName == SourceLevelPackageName)
			{
				return FindObject<AActor>(TargetLevel, *InSourceActor->GetFName().ToString());
			}
		}
	}
...
AActor* UMovieGraphConditionGroupQueryBase::GetActorForCurrentWorld(AActor* InActorToConvert)
{
...
#if WITH_EDITOR
	// Legacy API: subclasses may still call this without an evaluation world.
	// Guard editor globals because remote MRQ runs editor code without always having an interactive GEditor context.
	if (!GEditor)
	{
	...
	if (!bIsPIE)
	{
		...
		if (!bIsEditorActor)
		{
			return GetActorForCurrentWorld(InActorToConvert, GEditor->EditorWorld);
		}
	...
	else
	{
		...
		if (bIsEditorActor)
		{
			return GetActorForCurrentWorld(InActorToConvert, GEditor->PlayWorld);
		}
...
AActor* UMovieGraphConditionGroupQueryBase::GetActorForCurrentWorld(AActor* InActorToConvert, const UWorld* InTargetWorld)
{
	if (!InActorToConvert || !InTargetWorld)
	{
		return nullptr;
	}

	if (InActorToConvert->GetWorld() == InTargetWorld)
	{
		return InActorToConvert;
	}

	// Prefer the explicit evaluation/render world over global PIE/editor state. Remote MRQ may not have usable editor globals.
	return FindCounterpartActorInWorld(InActorToConvert, InTargetWorld);
}
...
void UMovieGraphConditionGroupQuery_Actor::EvaluateActorsAndComponents(const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors, TSet<UActorComponent*>& OutMatchingComponents) const
{
	...
	{
		if (AActor* ConvertedActor = GetActorForCurrentWorld(Entry.ActorToMatch.Get(), InWorld))
		{
...
UMovieGraphConditionGroupQuery_LightActorType::UMovieGraphConditionGroupQuery_LightActorType()
{
	// All light actor types are enabled by default.
	EnabledLightActorTypes = *GetSupportedLightActorTypes();
}

void UMovieGraphConditionGroupQuery_LightActorType::Evaluate(const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UMovieGraphConditionGroupQuery_LightActorType::Evaluate);

	for (AActor* Actor : InActorsToQuery)
	{
		if (EnabledLightActorTypes.Contains(Actor->GetClass()))
		{
			OutMatchingActors.Add(Actor);
		}
	}
...
const FSlateIcon& UMovieGraphConditionGroupQuery_LightActorType::GetIcon() const
{
	static const FSlateIcon LightActorTypeIcon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "ClassIcon.PointLightComponent");
	return LightActorTypeIcon;
}

const FText& UMovieGraphConditionGroupQuery_LightActorType::GetDisplayName() const
{
	static const FText DisplayName = LOCTEXT("ConditionGroupQueryDisplayName_LightActorType", "Light Actor Type");
	return DisplayName;
}

#if WITH_EDITOR
TArray<TSharedRef<SWidget>> UMovieGraphConditionGroupQuery_LightActorType::GetWidgets()
{
	TArray<TSharedRef<SWidget>> Widgets;

	Widgets.Add(
		SAssignNew(LightActorTypesList, SMovieGraphSimpleList<UClass*>)
		.DataSource(GetSupportedLightActorTypes())
		.CustomSummaryWidget(SNullWidget::NullWidget.ToSharedPtr())
		.OnGetRowText_Static(&GetRowText)
		.OnGetRowIcon_Static(&GetRowIcon)
		.ShowEnableDisable(true)
		.OnGetRowEnableState_Lambda([this](UClass* InLightActorType)
		{
			return GetEnableStateForLightType(InLightActorType);
		})
		.OnSetRowEnableState_Lambda([this](UClass* InLightActorType, bool bNewEnableState)
		{
			SetEnableStateForLightType(InLightActorType, bNewEnableState);
		})
	);
...
void UMovieGraphConditionGroupQuery_LightActorType::SetEnableStateForLightType(UClass* InLightActorType, const bool bNewEnableState)
{
	if (GetSupportedLightActorTypes()->Contains(InLightActorType))
	{
#if WITH_EDITOR
		const FScopedTransaction Transaction(LOCTEXT("UpdateLightActorTypesInCollection", "Update Light Actor Types in Collection"));
		Modify();
#endif

		if (bNewEnableState)
		{
			EnabledLightActorTypes.AddUnique(InLightActorType);
		}
		else
		{
			EnabledLightActorTypes.Remove(InLightActorType);
		}
	}
}

bool UMovieGraphConditionGroupQuery_LightActorType::GetEnableStateForLightType(UClass* InLightActorType)
{
	return EnabledLightActorTypes.Contains(InLightActorType);
}

TArray<UClass*>* UMovieGraphConditionGroupQuery_LightActorType::GetSupportedLightActorTypes()
{
	// Unfortunately this cannot be const because the list widget doesn't expect a const array
	static TArray<UClass*> SupportedLightActorTypes = {
	...
	return &SupportedLightActorTypes;
}

#if WITH_EDITOR
const FSlateBrush* UMovieGraphConditionGroupQuery_LightActorType::GetRowIcon(UClass* InLightActorType)
{
	return FSlateIconFinder::FindIconForClass(InLightActorType).GetIcon();
}

FText UMovieGraphConditionGroupQuery_LightActorType::GetRowText(UClass* InLightActorType)
{
	if (InLightActorType)
	{
		return InLightActorType->GetDisplayNameText();
	}

	return LOCTEXT("MovieGraphLightActorTypeConditionGroupQuery_Invalid", "(invalid light type)");
}
#endif	// WITH_EDITOR
...
void UMovieGraphConditionGroupQuery_ComponentTagName::Evaluate(const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors) const
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Evaluate() called on the Component Tag Name condition, but the condition only matches components. No results will be returned; EvaluateActorsAndComponents() should be used instead.");
	}
...
TArray<TSharedRef<SWidget>> UMovieGraphConditionGroupQuery_ComponentTagName::GetWidgets()
{
	...
	Widgets.Add(
		...
		[
			...
			.ToolTipText(LOCTEXT("ConditionGroupQueryComponentTagName_OnlyMatchComponentsTooltip", "If checked, only components with the specified tag(s) will be matched. If unchecked, actors that contain components with the specified tag(s) will be matched."))
		]
	);
...
void UMovieGraphConditionGroupQuery_ComponentType::Evaluate(const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors) const
{
	if (bOnlyMatchComponents)
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Evaluate() called on the Component Type condition, but the condition only matches components. No results will be returned; EvaluateActorsAndComponents() should be used instead.");
	}
...
UMovieGraphConditionGroupQuery_ComponentName::UMovieGraphConditionGroupQuery_ComponentName()
	: bOnlyMatchComponents(true)
{
	
}

void UMovieGraphConditionGroupQuery_ComponentName::Evaluate(
	const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors) const
{
	if (bOnlyMatchComponents)
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Evaluate() called on the Component Name condition, but the condition only matches components. No results will be returned; EvaluateActorsAndComponents() should be used instead.");
	}
...
void UMovieGraphConditionGroupQuery_ComponentName::EvaluateActorsAndComponents(
	const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors, TSet<UActorComponent*>& OutMatchingComponents) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UMovieGraphConditionGroupQuery_ComponentName::EvaluateActorsAndComponents);

	// Quick early-out if "*" is used as the wildcard. Faster than doing the wildcard matching.
	if (ComponentNames == TEXT("*"))
	{
		if (bOnlyMatchComponents)
		{
			TInlineComponentArray<UActorComponent*> MatchingComponents;
	...
	// Component names can be specified on multiple lines
	TArray<FString> AllComponentNames;
	ComponentNames.ParseIntoArrayLines(AllComponentNames);
	...
	for (AActor* Actor : InActorsToQuery)
	{
		...
		for (UActorComponent* Component : ActorComponents)
		{
			for (const FString& ComponentName : AllComponentNames)
			{
				if (Component->GetName().MatchesWildcard(ComponentName))
				{
					if (bOnlyMatchComponents)
					{
						OutMatchingComponents.Add(Component);
...
bool UMovieGraphConditionGroupQuery_ComponentName::ShouldEvaluateComponents() const
{
	return bOnlyMatchComponents;
}

const FSlateIcon& UMovieGraphConditionGroupQuery_ComponentName::GetIcon() const
{
	static const FSlateIcon ComponentNameIcon = FSlateIcon(FAppStyle::GetAppStyleSetName(), "ClassIcon.TextRenderActor");
	return ComponentNameIcon;
}

const FText& UMovieGraphConditionGroupQuery_ComponentName::GetDisplayName() const
{
	static const FText DisplayName = LOCTEXT("ConditionGroupQueryDisplayName_ComponentName", "Component Name");
	return DisplayName;
}

#if WITH_EDITOR
TArray<TSharedRef<SWidget>> UMovieGraphConditionGroupQuery_ComponentName::GetWidgets()
{
	TArray<TSharedRef<SWidget>> Widgets;
	...
	Widgets.Add(
		...
		[
			SNew(SMultiLineEditableTextBox)
			.Text_Lambda([this]() { return FText::FromString(ComponentNames); })
			.OnTextCommitted_Lambda([this](const FText& InText, ETextCommit::Type TextCommitType)
			{
				const FScopedTransaction Transaction(LOCTEXT("UpdateComponentNamesInCollection", "Update Component Names in Collection"));
				Modify();

				ComponentNames = InText.ToString();
			})
			.HintText(LOCTEXT("MovieGraphComponentNameQueryHintText", "A component on the actor must match one or more component names.\nWildcards allowed. Enter each component name on a separate line."))
		]
		...
		[
			...
			.OnCheckStateChanged_Lambda([this](ECheckBoxState NewState)
			{
				const FScopedTransaction Transaction(LOCTEXT("ConditionGroupQueryComponentName_ToggleOnlyMatchComponents", "Toggle Only Match Components"));
				Modify();
			...
			[
				SNew(STextBlock)
				.Font(FAppStyle::GetFontStyle(TEXT("PropertyWindow.NormalFont")))
				.Text(LOCTEXT("ConditionGroupQueryComponentName_OnlyMatchComponents", "Only Match Components"))
			]
			.ToolTipText(LOCTEXT("ConditionGroupQueryComponentName_OnlyMatchComponentsTooltip", "If checked, only components with the specified name(s) will be matched. If unchecked, actors that contain components with the specified name(s) will be matched."))
		]
	);
...
void UMovieGraphConditionGroupQuery_Sublevel::Evaluate(const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors) const
{
	for (const TSoftObjectPtr<UWorld>& World : Sublevels)
	{
		...
		if (!LoadedWorld)
		{
			...
			UE_LOGF(LogMovieRenderPipeline, Warning, "Sublevel query in collection '%ls' is excluding level (%ls) because it is not loaded.", *CollectionName, *World.ToString())
			continue;
		}
		...
		for (const TObjectPtr<AActor>& LevelActor : CurrentLevel->Actors)
		{
			...
			if (AActor* ConvertedActor = GetActorForCurrentWorld(LevelActor.Get(), InWorld))
			{
				OutMatchingActors.Add(ConvertedActor);
			}
		}
...
TSharedRef<SWidget> UMovieGraphConditionGroupQuery_Sublevel::GetAddMenuContents(const FMovieGraphConditionGroupQueryContentsChanged& OnAddFinished)
{
	FAssetPickerConfig SublevelPickerConfig;
	{
		...
		{
			if (bOnlyShowSublevelsFromActivePersistentLevel && !IsSublevelInActivePersistentLevel(InLevelAsset.PackageName))
			{
				return true;
			}
	...
	return
		...
		[
			...
			[
				SNew(SCheckBox)
				.ToolTipText(LOCTEXT("ConditionGroupQuerySublevel_OnlyShowActivePersistentLevelSublevelsTooltip", "Filter out sublevels not from the active persistent level."))
				.OnCheckStateChanged_Lambda([this](ECheckBoxState NewState)
				{
					const FScopedTransaction Transaction(LOCTEXT("ConditionGroupQuerySublevel_ToggleOnlyShowActivePersistentLevelSublevels", "Toggle Only Show Active Persistent Level Sublevels"));
					Modify();

					bOnlyShowSublevelsFromActivePersistentLevel = (NewState == ECheckBoxState::Checked);
				...
				{
					return bOnlyShowSublevelsFromActivePersistentLevel ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
				})
				[
					SNew(STextBlock)
					.ColorAndOpacity(FSlateColor::UseSubduedForeground())
					.Text(LOCTEXT("ConditionGroupQuerySublevel_OnlyShowActivePersistentLevelSublevels", "Only show sublevels from the active persistent level"))
				]
			]
...
bool UMovieGraphConditionGroupQuery_Sublevel::IsSublevelInActivePersistentLevel(FName InSublevelPackageName)
{
	if (InSublevelPackageName.IsNone() || !GEditor)
	{
		return false;
	}

	const UWorld* ActiveWorld = GEditor->GetEditorWorldContext().World();
	if (!ActiveWorld)
	{
		return false;
	}

	for (const ULevelStreaming* StreamingLevel : ActiveWorld->GetStreamingLevels())
	{
		if (StreamingLevel && StreamingLevel->GetWorldAssetPackageFName() == InSublevelPackageName)
		{
			return true;
		}
	}
...
void UMovieGraphConditionGroupQuery_IsSpawnable::Evaluate(const TArray<AActor*>& InActorsToQuery, const UWorld* InWorld, TSet<AActor*>& OutMatchingActors) const
{
	for (AActor* InActor : InActorsToQuery)
	{
		const TOptional<FMovieSceneSpawnableAnnotation> Spawnable = FMovieSceneSpawnableAnnotation::Find(InActor);
		if (Spawnable.IsSet() == bIsSpawnable)
		{
			OutMatchingActors.Add(InActor);
		}
	}
...
UMovieGraphConditionGroupQueryBase* UMovieGraphConditionGroup::DuplicateQuery(const int32 QueryIndex)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Invalid query index provided to DuplicateQuery().");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to duplicate condition group query.");
	}
...
bool UMovieGraphRenderLayerSubsystem::AddRenderLayer(UMovieGraphRenderLayer* RenderLayer)
{
	if (!RenderLayer)
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Invalid render layer provided to AddRenderLayer().");
		return false;
	}
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Render layer '%ls' already exists in the render layer subsystem; it will not be added again.", *RenderLayer->GetRenderLayerName().ToString());
		return false;
	}
