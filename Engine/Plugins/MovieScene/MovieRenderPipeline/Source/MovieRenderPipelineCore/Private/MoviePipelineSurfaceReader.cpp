...
void FMoviePipelineSurfaceReader::ResolveSampleToReadbackTexture_RenderThread(FRHICommandListImmediate& RHICmdList, const FTextureRHIRef& SourceSurfaceSample)
{
...
void FMoviePipelineSurfaceReader::CopyReadbackTexture_RenderThread(FRHICommandListImmediate& RHICmdList, TUniqueFunction<void(TUniquePtr<FImagePixelData>&&)>&& InFunctionCallback, TSharedPtr<FImagePixelDataPayload, ESPMode::ThreadSafe> InFramePayload)
{
	// We use GetModuleChecked here to avoid accidentally loading the module from the non-main thread.
	static const FName RendererModuleName("Renderer");
	IRendererModule* RendererModule = &FModuleManager::GetModuleChecked<IRendererModule>(RendererModuleName);
	...
	for (int32 Index = 0; Index < Surfaces.Num(); Index++)
	{
		...
		ENQUEUE_RENDER_COMMAND(PerformReadback)(
			[ResolveSurface](FRHICommandListImmediate& RHICmdList)
			{
				...
				{
					ResolveSurface->Surface->CopyReadbackTexture_RenderThread(RHICmdList, MoveTemp(ResolveSurface->FunctionCallback), ResolveSurface->FunctionPayload);
				}
			});
	}
...
void FMoviePipelineSurfaceQueue::OnRenderTargetReady_RenderThread(const FTextureRHIRef InRenderTarget, TSharedRef<FImagePixelDataPayload, ESPMode::ThreadSafe> InFramePayload, TUniqueFunction<void(TUniquePtr<FImagePixelData>&&)>&& InFunctionCallback)
{
	...
	NextResolveTarget->Surface->ResolveSampleToReadbackTexture_RenderThread(RHICmdList, InRenderTarget);
	...
	// We can now safely map the surface and copy the data out of it without causing a GPU stall.
	{
		...
		if (OldestResolveTarget->Surface->WasEverQueued())
		{
			...
			OldestResolveTarget->Surface->CopyReadbackTexture_RenderThread(RHICmdList, MoveTemp(OldestResolveTarget->FunctionCallback), OldestResolveTarget->FunctionPayload);
		}
	}
