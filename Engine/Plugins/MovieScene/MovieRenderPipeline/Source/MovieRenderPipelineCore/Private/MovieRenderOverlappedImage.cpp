...
void FImageOverlappedAccumulator::AccumulatePixelData(const FImagePixelData& InPixelData, FIntPoint InTileOffset, FVector2D InSubpixelOffset, const MoviePipeline::FTileWeight1D & WeightX, const MoviePipeline::FTileWeight1D & WeightY)
{
	...
	if (IsFetchOk)
	{
		...
		if(bLogTiming)
		{
			...
			UE_LOGF(LogTemp, Log, "    [%8.2f] Unpack time.", ElapsedMs);
		}
		...
		if (bLogTiming)
		{
			...
			UE_LOGF(LogTemp, Log, "    [%8.2f] Gamma time.", ElapsedMs);
		}
		...
		if (bLogTiming)
		{
			...
			UE_LOGF(LogTemp, Log, "    [%8.2f] Tile time.", ElapsedMs);
		}
		...
		if (bLogTiming)
		{
			...
			UE_LOGF(LogTemp, Log, "    [%8.2f] Plane time.", ElapsedMs);
		}
	}
}
