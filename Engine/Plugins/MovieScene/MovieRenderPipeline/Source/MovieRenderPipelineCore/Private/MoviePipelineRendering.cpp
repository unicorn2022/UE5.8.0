...
void UMoviePipeline::SetupRenderingPipelineForShot(UMoviePipelineExecutorShot* InShot)
{
	...
	*/
...

	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "Resolution %dx%d must be greater than zero in both dimensions.", BackbufferResolution.X, BackbufferResolution.Y);
			Shutdown(true);
			return;
		}
	}
...
void UMoviePipeline::TeardownRenderingPipelineForShot(UMoviePipelineExecutorShot* InShot)
{
	...
	if (OutputBuilder->GetNumOutstandingFrames() > 1)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Error, "Not all frames were fully submitted by the time rendering was torn down! Frames will be missing from output!");
	}
}
...
void UMoviePipeline::RenderFrame()
{
	...
	// If this is the first sample for a new frame, update per-output-frame bookkeeping.
	if (CachedOutputState.IsFirstTemporalSample())
	{
	...
	// Only register expected output once the frame can submit completed data.
	if (CachedOutputState.IsLastTemporalSample())
	{
		CurrentShotRenderedFrameCount++;
		...
		for (int32 TileX = 0; TileX < TileCount.X; TileX++)
		{
			...
			for (int32 RenderSampleIndex = 0; RenderSampleIndex < NumSamplesToRender; RenderSampleIndex++)
			{
				...
				if (bMultipleTiles && IsTemporalAccumulationBasedMethod(AntiAliasingMethod))
				{
					...
					UE_LOGF(LogMovieRenderPipeline, Warning, "Temporal AntiAliasing is not supported when using tiling!");
...
void UMoviePipeline::AddFrameToOutputMetadata(const FString& ClipName, const FString& ImageSequenceFileName, const FMoviePipelineFrameOutputState& FrameOutputState, const FString& Extension, const bool bHasAlpha)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "ShotIndex %d out of range", FrameOutputState.ShotIndex);
		return;
	}
