...
void UMoviePipelineWidgetRenderer::UpdateTelemetry(FMoviePipelineShotRenderTelemetry* InTelemetry) const
{
	InTelemetry->bUsesUIRenderer = true;
}
...
void UMoviePipelineWidgetRenderer::SetupImpl(const MoviePipeline::FMoviePipelineRenderPassInitSettings& InPassInitSettings)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Resolution %dx%d exceeds maximum allowed by GPU. Widget Renderer does not support high-resolution tiling and thus can't exceed %dx%d.", OutputResolution.X, OutputResolution.Y, MaxResolution, MaxResolution);
