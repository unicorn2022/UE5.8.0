...
void UMoviePipelineConsoleVariableSetting::UpdateTelemetry(FMoviePipelineShotRenderTelemetry* InTelemetry) const
{
	InTelemetry->bSetsConsoleVariables |= CVars.ContainsByPredicate([](const FMoviePipelineConsoleVariableEntry& Entry)
	{
		return Entry.bIsEnabled;
	});
	InTelemetry->bUsesConsoleVariablePreset |= !ConsoleVariablePresets.IsEmpty();
}
...
void UMoviePipelineConsoleVariableSetting::ApplyCVarSettings(const bool bOverrideValues)
{
	...
	for(const FMoviePipelineConsoleVariableEntry& CVarEntry : MergedConsoleVariables)
	{
		...
		if (CVar)
		{
			if (bOverrideValues)
			{
				...
				UE_LOGF(LogMovieRenderPipeline, Log, "Applying CVar \"%ls\" PreviousValue: %f NewValue: %f", *CVarEntry.Name, PreviousConsoleVariableValues[Index], CVarEntry.Value);
			}
			else
			{
				UE::MoviePipeline::SetValue(CVar, PreviousConsoleVariableValues[Index]);
				UE_LOGF(LogMovieRenderPipeline, Log, "Restoring CVar \"%ls\" PreviousValue: %f NewValue: %f", *CVarEntry.Name, CVarEntry.Value, PreviousConsoleVariableValues[Index]);
			}
		}
		else
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to apply CVar \"%ls\" due to no cvar by that name. Ignoring.", *CVarEntry.Name);
		}
	...
	if (bOverrideValues)
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Log, "Executing Console Command \"%ls\" before shot starts.", *Command);
	...
	else
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Log, "Executing Console Command \"%ls\" after shot ends.", *Command);
			UKismetSystemLibrary::ExecuteConsoleCommand(GetWorld(), Command, nullptr);
		}
	}
}
...
void UMoviePipelineConsoleVariableSetting::MergeInPresetConsoleVariables()
{
	...
	for (const TScriptInterface<IMovieSceneConsoleVariableTrackInterface>& Preset : ConsoleVariablePresets)
	{
		if (!Preset)
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Invalid CVar preset specified. Ignoring.");
			continue;
		}
		...
		for (const TTuple<FString, FString>& CVarPair : PresetCVars)
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to apply CVar \"%ls\" (from preset \"%ls\") because value could not be parsed into a float. Ignoring.",
