...
FMoviePipelinePostRenderFileDisplayProcessor::FFilesToOpen FMoviePipelinePostRenderFileDisplayProcessor::FindFilesToOpen(FMoviePipelineOutputData& InPipelineOutputData) const
{
	...
	// Nothing to do if the graph (GraphData) and preset (ShotData) output is empty.
	if (InPipelineOutputData.GraphData.IsEmpty() && InPipelineOutputData.ShotData.IsEmpty())
	{
		return FilesToOpen;
	}
	...
	TArray<TMap<FString, TArray<FString>>>& InVideosGroupedByShot) const
{
	...
	{
		GroupFilesByShot_Graph(InPipelineOutputData, InImagesGroupedByShot, InFrameTemplateImagesGroupedByShot, InVideosGroupedByShot);
	}
	else
	{
		GroupFilesByShot_Preset(InPipelineOutputData, InImagesGroupedByShot, InFrameTemplateImagesGroupedByShot, InVideosGroupedByShot);
	}
}

void FMoviePipelinePostRenderFileDisplayProcessor::GroupFilesByShot_Graph(
	FMoviePipelineOutputData& InPipelineOutputData,
	TArray<TMap<FString, TArray<FString>>>& InImagesGroupedByShot,
	TArray<TMap<FString, TArray<FString>>>& InFrameTemplateImagesGroupedByShot,
	TArray<TMap<FString, TArray<FString>>>& InVideosGroupedByShot) const
	...
	for (FMovieGraphRenderOutputData& OutputData : InPipelineOutputData.GraphData)
	{
		...
		for (const TPair<FMovieGraphRenderDataIdentifier, FMovieGraphRenderLayerOutputData>& PassData : OutputData.RenderLayerData)
		{
			...
			for (const TPair<FSoftClassPath, FMovieGraphStringArray>& NodeToFiles : PassData.Value.NodeTypeToFilePaths)
			{
				...
				if (!NodeClass)
				{
					UE_LOGF(LogMovieRenderPipeline, Warning, "Found an invalid node type while determining the media to display post-render. This node's media will be skipped.");
					continue;
				}
...
void FMoviePipelinePostRenderFileDisplayProcessor::GroupFilesByShot_Preset(
	FMoviePipelineOutputData& InPipelineOutputData,
	TArray<TMap<FString, TArray<FString>>>& InImagesGroupedByShot,
	TArray<TMap<FString, TArray<FString>>>& InFrameTemplateImagesGroupedByShot,
	TArray<TMap<FString, TArray<FString>>>& InVideosGroupedByShot) const
{
	// OutputData is an individual shot's collection of rendered output (images + movies)
	for (FMoviePipelineShotOutputData& OutputData : InPipelineOutputData.ShotData)
	{
		TMap<FString, TArray<FString>>& ImagesByExtension = InImagesGroupedByShot.AddDefaulted_GetRef();
		TMap<FString, TArray<FString>>& ImagesByFrameTemplate = InFrameTemplateImagesGroupedByShot.AddDefaulted_GetRef();
		TMap<FString, TArray<FString>>& VideosByExtension = InVideosGroupedByShot.AddDefaulted_GetRef();

		// Add the images/videos for each render pass within the shot
		for (const TPair<FMoviePipelinePassIdentifier, FMoviePipelineRenderPassOutputData>& PassData : OutputData.RenderPassData)
		{
			// For each setting type that generated files, add its files
			for (const TPair<FSoftClassPath, FMoviePipelineStringArray>& FilesFromSetting : PassData.Value.SettingTypeToFilePaths)
			{
				const UClass* SettingClass = FilesFromSetting.Key.ResolveClass();
				if (!SettingClass)
				{
					UE_LOGF(LogMovieRenderPipeline, Warning, "Found an invalid setting type while determining the media to display post-render. This setting's media will be skipped.");
					continue;
				}
				
				const bool bIsImageNode = SettingClass->IsChildOf(UMoviePipelineImageSequenceOutputBase::StaticClass());

				// For all files generated by the setting, categorize it either by extension and/or frame template (for images)
				for (const FString& SettingFile : FilesFromSetting.Value.Array)
				{
					const FString Extension = FPaths::GetExtension(SettingFile).ToUpper();
					if (Extension.IsEmpty())
					{
						continue;
					}
					...
					if (bIsImageNode)
					{
						ImagesByExtension.FindOrAdd(Extension).Add(SettingFile);

						// The render pass will contain a set of frame templated file paths, in addition to normal file paths. Determine if this
						// file should be associated with any of these frame templated paths (matched by extension).
						for (const FString& FrameTemplatedFilePath : PassData.Value.FrameTemplatedFilePaths)
						{
							if (FPaths::GetExtension(FrameTemplatedFilePath).ToUpper() == Extension)
							{
								ImagesByFrameTemplate.FindOrAdd(FrameTemplatedFilePath).Add(SettingFile);
								break;
							}
						}
					}
					...
					{
						VideosByExtension.FindOrAdd(Extension).Add(SettingFile);
					}
				}
			}
		}
...
void FMoviePipelinePostRenderFileDisplayProcessor::LaunchFilesWithSettings(const TArray<FString>& InFilesToOpen, const FMovieGraphPostRenderVideoPlayOptions* InPlayOptions, const TTuple<FString, FString>& InFrameRangeToOpen) const
{
	...
	auto ReplaceFramePlaceholderInFilePaths = [this, &StartFrame](TArray<FString>& InFilePaths)
	{
		FString FramePlaceholder;
		
		const EMovieGraphFrameRangeNotation FrameRangeNotationType = PostRenderSettings.PostRenderImageSequencePlayOptions.FrameRangeNotation;
	...
	else
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Display, "Quick Render: Opening external viewer with command: %ls", *FinalCommandString);
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Unable to open the post-render media viewer. Ensure the executable and its command line arguments have been specified correctly in Editor Preferences. The command that was run: [%ls]", *FinalCommandString);
		}
	}
}
...
void FMoviePipelinePostRenderFileDisplayProcessor::PersistOutputData(const TArray<FMoviePipelineOutputData>& InOutputData)
{
	...
	if (!bSuccess)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Warning, "Unable to save post-render play data. Could not save to destination file [%ls].", *PackageFileName);
	}
}
...
UMoviePipelineOutputDataContainer* FMoviePipelinePostRenderFileDisplayProcessor::GetPersistedOutputData()
{
	...
	// Use FindPackage first to avoid triggering the async loading path on packages that are already in memory.
	const UPackage* OutputDataPackage = FindPackage(nullptr, *PersistedOutputDataPackagePath);
	if (!OutputDataPackage)
	{
		OutputDataPackage = LoadPackage(nullptr, *PersistedOutputDataPackagePath, LOAD_None);
	}
