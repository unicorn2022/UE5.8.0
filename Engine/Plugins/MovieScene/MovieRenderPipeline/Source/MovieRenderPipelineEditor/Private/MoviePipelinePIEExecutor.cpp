...
{
	UE_LOGF(LogMovieRenderPipeline, Log, "Updating the PIE Executor to allow using unsaved levels.");
...
void UMoviePipelinePIEExecutor::Start(const UMoviePipelineExecutorJob* InJob)
{
	...
	if (ExecutorSettings->bResizePIEWindowToOutputResolution)
	{
		...
		if (InJob->IsUsingGraphConfiguration() && GraphConfig)
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to evaluate graph to determine output resolution. Traversal error: %ls", *OutTraversalError);
			}
		}
		else if (InJob->IsUsingBasicConfiguration())
		{
			// Basic-mode jobs generate their graph later in DuplicateJobAndConfiguration().
			// Use the BasicConfig's output resolution directly for the PIE window size.
			if (const UMoviePipelineBasicConfig* BasicCfg = InJob->GetBasicConfig())
			{
				const FMovieGraphNamedResolution& Resolution = BasicCfg->OutputResolution;
				WindowSize = FVector2D(Resolution.Resolution.X, Resolution.Resolution.Y);
			}
		}
		else
		{
...
void UMoviePipelinePIEExecutor::OnPIEStartupFinished(bool)
{
	...
	if(!ExecutingWorld)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Error, "Failed to find a PIE UWorld after OnPIEStartupFinished!");
		OnExecutorFinishedImpl();
		return;
	}
	...
	if (!PipelineClass)
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Failed to load project specified MoviePipeline class type!");
		OnExecutorFinishedImpl();
		return;
	}

	if (!Queue || !Queue->GetJobs().IsValidIndex(CurrentPipelineIndex))
	{
		const FText FailureReason = FText::Format(LOCTEXT("OutOfBoundsStartupFailure", "The job index is out of bounds while starting PIE render, did you modify the Queue while rendering was happening? Index: {0}, Length: {1}."),
			FText::AsNumber(CurrentPipelineIndex), FText::AsNumber(Queue ? Queue->GetJobs().Num() : 0));
		UE_LOGF(LogMovieRenderPipeline, Error, "%ls", *FailureReason.ToString());
		OnPipelineErrored(nullptr, true, FailureReason);

		// Route through OnPIEEnded so fixed timestep/log gathering/editor state are restored.
	...
	const bool bNeedsGraphPipeline = CurrentJob->IsUsingGraphConfiguration() || CurrentJob->IsUsingBasicConfiguration();
	if (bNeedsGraphPipeline && !PipelineClass->IsChildOf(UMovieGraphPipeline::StaticClass()))
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Using the default MovieGraphPipeline; the pipeline specified in Project Settings (%ls) is not compatible with graphs.", *PipelineClass->GetName());
		PipelineClass = UMovieGraphPipeline::StaticClass();
	}
	else if (!bNeedsGraphPipeline && !PipelineClass->IsChildOf(UMoviePipeline::StaticClass()))
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Using the default MoviePipeline; the pipeline specified in Project Settings (%ls) is not compatible with normal (non-graph) presets.", *PipelineClass->GetName());
	...
	{
		OnIndividualJobStartedImpl(CurrentJob);
		if (PipelineAsLegacy)
		{
			PipelineAsLegacy->Initialize(CurrentJob);
		...
		{
			PipelineAsGraph->Initialize(CurrentJob, FMovieGraphInitConfig());
			if (CustomInitializationTime.IsSet())
			{
...
{
	// Defensive: a misbehaving delegate listener (BP/Python) could in principle have cleared the
	// queue's jobs out from under us, so abort safely rather than crash on bad indexing.
	if (!Queue || !Queue->GetJobs().IsValidIndex(CurrentPipelineIndex))
	{
		const FText FailureReason = FText::Format(LOCTEXT("OutOfBoundsTickFailure", "The job index is out of bounds while ticking PIE render, did you modify the Queue while rendering was happening? Index: {0}, Length: {1}."),
			FText::AsNumber(CurrentPipelineIndex), FText::AsNumber(Queue ? Queue->GetJobs().Num() : 0));
		UE_LOGF(LogMovieRenderPipeline, Error, "%ls", *FailureReason.ToString());
	...
	if (RemainingInitializationFrames <= 0)
	{
		CurrentJob->SetStatusProgress(GetCompletionPercentageFromActivePipeline());
	}
...
void UMoviePipelinePIEExecutor::OnPIEMoviePipelineFinished(FMoviePipelineOutputData InOutputData)
{
	...
	// Reset progress for UI (regardless of success). Guard against the queue having been mutated
	// by listeners notified above (e.g., via OnPipelineErrored) before indexing back into it.
	if (Queue && Queue->GetJobs().IsValidIndex(CurrentPipelineIndex))
	{
...
void UMoviePipelinePIEExecutor::OnExecutorFinishedImpl()
{
	...
	// Now that the queue has finished, play the rendered frames in an external application, open the output directory,
	// or do nothing.
	if (GetPostRenderAction() == EMoviePipelinePostRenderActionType::PlayRenderOutput)
	{
		FMoviePipelinePostRenderFileDisplayProcessor PostRenderFileDisplayProcessor(EditorSettings->PostRenderSettings);

		for (const FMoviePipelineOutputData& OutputData : AllJobOutputData)
		{
			PostRenderFileDisplayProcessor.AddFiles(OutputData);
		}
		
		PostRenderFileDisplayProcessor.OpenFiles();
	}
	else if (GetPostRenderAction() == EMoviePipelinePostRenderActionType::OpenOutputDirectory)
	{
		while (!ResolvedOutputDirectories.IsEmpty())
		{
			const FString ResolvedOutputDirectory = ResolvedOutputDirectories[0];
			ResolvedOutputDirectories.RemoveAt(0);
			...
			// Depending on the settings, we might only need to open one directory.
			if (EditorSettings->PostRenderSettings.DirectoryOpenBehavior == EMovieGraphDirectoryOpenBehavior::OpenDirectoryForFirstJob)
			{
				break;
			}
		}
	...
	// Reset all cached data tracking rendered output
	ResolvedOutputDirectories.Empty();
	AllJobOutputData.Empty();
}

bool UMoviePipelinePIEExecutor::GetSupportsPostRenderActions_Implementation()
{
	return true;
}
...
void UMoviePipelinePIEExecutor::OnPIEEnded(bool)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "PIE Ended while Movie Pipeline was still active. Stalling to do full shutdown.");

		// This will flush any outstanding work on the movie pipeline (file writes) immediately
		ActiveMoviePipeline->Shutdown(true);
		UE_LOGF(LogMovieRenderPipeline, Log, "MoviePipelinePIEExecutor: Stalling finished, pipeline has shut down.");
	}
...
void UMoviePipelinePIEExecutor::OnIndividualJobStartedImpl(UMoviePipelineExecutorJob* InJob)
{
	...
	// By the time the job finishes, it will have most likely been garbage collected (at least when using the graph, since jobs that use the graph
	// are duplicated). Instead of caching the resolved output directories when the job finishes, cache it when starting while the job is still valid.
	ResolvedOutputDirectories.AddUnique(UMoviePipelineEditorBlueprintLibrary::ResolveOutputDirectoryFromJob(InJob));
}
...
void UMoviePipelinePIEExecutor::OnIndividualJobFinishedImpl(FMoviePipelineOutputData InOutputData)
{
	...
	// Null out the pipeline and job pointers in the output data before caching (so they are GC'd correctly after the pipeline finishes)
	InOutputData.Pipeline = nullptr;
	InOutputData.Job = nullptr;
	AllJobOutputData.Add(InOutputData);
}
