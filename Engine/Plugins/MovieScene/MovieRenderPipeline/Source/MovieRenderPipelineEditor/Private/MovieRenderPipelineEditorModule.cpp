...
#include "Customizations/MoviePipelineNameEditSinks.h"
...
class FMovieRenderPipelineRenderer : public IMovieRendererInterface
{
	virtual void RenderMovie(UMovieSceneSequence* InSequence, const TArray<UMovieSceneCinematicShotSection*>& InSections) override
	{
		...
		if (!ActiveJob)
		{
			...
			UMoviePipelineEditorBlueprintLibrary::ApplyDefaultConfigurationTypeToJob(ActiveJob);
		}
...
void FMovieRenderPipelineEditorModule::RegisterTypeCustomizations()
{
	...
	PropertyModule.RegisterCustomPropertyTypeLayout(FMovieGraphNamedResolution::StaticStruct()->GetFName(),
...
void FMovieRenderPipelineEditorModule::UnregisterTypeCustomizations()
{
	if (UObjectInitialized())
	{
		...
		if (PropertyModule)
		{
			...
			PropertyModule->UnregisterCustomPropertyTypeLayout(FMovieGraphNamedResolution::StaticStruct()->GetFName());
		}
	}
}
...
void FMovieRenderPipelineEditorModule::StartupModule()
{
	...
	// Register Object Name sinks for the job/shot so their title appearance is customized in the details panel.
	const FEditorWidgetsModule& EditorWidgetsModule = FModuleManager::LoadModuleChecked<FEditorWidgetsModule>("EditorWidgets");
	JobNameEditSink = MakeShared<FMoviePipelineJobNameEditSink>();
	ShotNameEditSink = MakeShared<FMoviePipelineShotNameEditSink>();
	EditorWidgetsModule.GetObjectNameEditSinkRegistry()->RegisterObjectNameEditSink(JobNameEditSink.ToSharedRef());
	EditorWidgetsModule.GetObjectNameEditSinkRegistry()->RegisterObjectNameEditSink(ShotNameEditSink.ToSharedRef());
}
...
void FMovieRenderPipelineEditorModule::ShutdownModule()
{
	...
	if (FModuleManager::Get().IsModuleLoaded("EditorWidgets"))
	{
		const FEditorWidgetsModule& EditorWidgetsModule = FModuleManager::GetModuleChecked<FEditorWidgetsModule>("EditorWidgets");
		if (JobNameEditSink.IsValid())
		{
			EditorWidgetsModule.GetObjectNameEditSinkRegistry()->UnregisterObjectNameEditSink(JobNameEditSink.ToSharedRef());
		}
		if (ShotNameEditSink.IsValid())
		{
			EditorWidgetsModule.GetObjectNameEditSinkRegistry()->UnregisterObjectNameEditSink(ShotNameEditSink.ToSharedRef());
		}
	}
}
