	...
	: FSlateStyleSet(StyleName)
{
	...
	const FTextBlockStyle NormalTextBlockStyle = FAppStyle::GetWidgetStyle<FTextBlockStyle>("NormalText");
	
	FEditableTextBoxStyle AutocompleteEditableText = FEditableTextBoxStyle(NormalEditableTextBoxStyle)
		.SetFont(FAppStyle::GetFontStyle(TEXT("PropertyWindow.NormalFont")));
	Set("MovieRenderPipeline.AutocompleteTextStyle", AutocompleteEditableText);
	
	FTextBlockStyle AutocompleteArgumentEditableText = FTextBlockStyle(NormalTextBlockStyle)
		.SetFont(FAppStyle::GetFontStyle(TEXT("PropertyWindow.BoldFont")));
	Set("MovieRenderPipeline.AutocompleteArgumentTextStyle", AutocompleteArgumentEditableText);

	Set("MovieRenderPipeline.TabIcon", new FSlateImageBrush(RootToContentDir(TEXT("TabIcon_24x.png")), Icon16x16));

	// Class icons so that SDetailNameArea shows the correct icon for jobs and shots
	Set("ClassIcon.MoviePipelineExecutorJob", new FSlateBrush(*FAppStyle::Get().GetBrush("ClassIcon.CameraComponent")));
	Set("ClassIcon.MoviePipelineExecutorShot", new FSlateBrush(*FAppStyle::Get().GetBrush("Sequencer.Tracks.CinematicShot")));
	...
	// Toolbar style for the queue
	{
		FToolBarStyle MRQToolBar = FAppStyle::Get().GetWidgetStyle<FToolBarStyle>("AssetEditorToolbar");
		MRQToolBar.SetBackgroundPadding(FMargin(4.0f, 0.0f));
		MRQToolBar.SetSeparatorPadding(FMargin(2.0f, 3.0f));
		MRQToolBar.SetComboButtonPadding(FMargin(4.0f, 0.0f));

		FTextBlockStyle MRQLabelStyle = MRQToolBar.LabelStyle;
		MRQLabelStyle.SetColorAndOpacity(FStyleColors::ForegroundHover);
		MRQToolBar.SetLabelStyle(MRQLabelStyle);

		FButtonStyle MRQSettingsButtonStyle = MRQToolBar.SettingsButtonStyle;
		MRQSettingsButtonStyle
			.SetNormal(FSlateRoundedBoxBrush(FStyleColors::Primary, FVector4(4.0f, 0.0f, 0.0f, 4.0f)))
			.SetHovered(FSlateRoundedBoxBrush(FStyleColors::PrimaryHover, FVector4(4.0f, 0.0f, 0.0f, 4.0f)))
			.SetPressed(FSlateRoundedBoxBrush(FStyleColors::PrimaryPress, FVector4(4.0f, 0.0f, 0.0f, 4.0f)))
			.SetNormalForeground(FStyleColors::ForegroundHover)
			.SetHoveredForeground(FStyleColors::ForegroundHover)
			.SetPressedForeground(FStyleColors::ForegroundHover);
		MRQToolBar.SetSettingsButtonStyle(MRQSettingsButtonStyle);

		FComboButtonStyle MRQSettingsComboButtonStyle = MRQToolBar.SettingsComboButton;
		FButtonStyle MRQSettingsComboButtonButtonStyle = MRQSettingsComboButtonStyle.ButtonStyle;
		MRQSettingsComboButtonButtonStyle.SetNormal(FSlateRoundedBoxBrush(FStyleColors::Dropdown, FVector4(0.0f, 4.0f, 4.0f, 0.0f), FStyleColors::Header, 1.0f));
		MRQSettingsComboButtonStyle.SetButtonStyle(MRQSettingsComboButtonButtonStyle);
		MRQToolBar.SetSettingsComboButtonStyle(MRQSettingsComboButtonStyle);

		Set("MovieRenderPipeline.ToolBar", MRQToolBar);
	}

	// Quick Render icons
	Set("MovieRenderPipeline.QuickRender.Icon.MovieRenderQueueMode", new IMAGE_BRUSH_SVG("Icons/MovieRenderQueueMode_16", Icon16x16));
	Set("MovieRenderPipeline.QuickRender.Icon.QuickRenderMode", new IMAGE_BRUSH_SVG("Icons/QuickRenderMode_16", Icon16x16));
	Set("MovieRenderPipeline.QuickRender.Icon.CurrentSequenceMode", new IMAGE_BRUSH_SVG("Icons/CurrentSequenceMode_16", Icon16x16));
	Set("MovieRenderPipeline.QuickRender.Icon.ViewportCameraInSequenceMode", new IMAGE_BRUSH_SVG("Icons/ViewportCameraInSequenceMode_16", Icon16x16));
	Set("MovieRenderPipeline.QuickRender.Icon.CurrentViewportMode", new IMAGE_BRUSH_SVG("Icons/CurrentViewportMode_16", Icon16x16));
	Set("MovieRenderPipeline.QuickRender.Icon.SelectedCamerasMode", new IMAGE_BRUSH_SVG("Icons/SelectedCamerasMode_16", Icon16x16));
	Set("MovieRenderPipeline.QuickRender.Icon.PlayLastRender", new IMAGE_BRUSH_SVG("Icons/PlayLastRender_16", Icon16x16));

	// Graph icons
	Set("MovieRenderPipeline.Graph.Icon.RenderLayer", new IMAGE_BRUSH_SVG("Icons/RenderLayer_16", Icon16x16));
	Set("MovieRenderPipeline.Graph.Icon.RenderMovieFile", new IMAGE_BRUSH_SVG("Icons/RenderMovieFile_16", Icon16x16));
	Set("MovieRenderPipeline.Graph.Icon.RenderSequenceOutput", new IMAGE_BRUSH_SVG("Icons/RenderSequenceOutput_16", Icon16x16));
	Set("MovieRenderPipeline.Graph.Icon.WarmUpSettings", new IMAGE_BRUSH_SVG("Icons/WarmUpSettings_16", Icon16x16));

	// Pipeline / General
	Set("MovieRenderPipeline.Icon.RenderRemote", new IMAGE_BRUSH_SVG("Icons/RenderRemote_16", Icon16x16));
