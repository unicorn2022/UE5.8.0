...
void UMoviePipelineNewProcessExecutor::Execute_Implementation(UMoviePipelineQueue* InPipelineQueue)
{
	...
	if (!DuplicatedQueue)
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Could not save manifest package to disk. Path: %ls", *ManifestFilePath);
		OnExecutorFinishedImpl();
		return;
	}
	...
	for (UMoviePipelineExecutorJob* Job : DuplicatedQueue->GetJobs())
	{
		...
		// Determine the graph to evaluate. For Basic mode, generate a transient graph from the Basic config.
		UMovieGraphConfig* GraphConfig = nullptr;
		if (Job->IsUsingGraphConfiguration())
		{
			GraphConfig = Job->GetGraphPreset();
		}
		else if (Job->IsUsingBasicConfiguration())
		{
			GraphConfig = UMoviePipelineBasicConfig::GenerateGraph(Job->GetBasicConfig(), GetTransientPackage());
		}
	...
	UE_LOGF(LogMovieRenderPipeline, Log, "Launching a new process to render with the following command line:");
	UE_LOGF(LogMovieRenderPipeline, Log, "%ls %ls", *ExecutablePath, *FinalCommandLine);
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Failed to launch executable for new process render. Executable Path: \"%ls\" Command Line: \"%ls\"", FPlatformProcess::ExecutablePath(), *FinalCommandLine);
...
void UMoviePipelineNewProcessExecutor::CheckForProcessFinished()
{
	...
	if (FPlatformProcess::GetProcReturnCode(ProcessHandle, &ReturnCode))
	{
		...
		if (ReturnCode != 0)
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Process exited with non-success return code. Return Code; %d", ReturnCode);
...
void UMoviePipelineNewProcessExecutor::CancelAllJobs_Implementation()
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Attempting to cancel UMoviePipelineNewProcessExecutor job but process has already exited.");
	}
}
