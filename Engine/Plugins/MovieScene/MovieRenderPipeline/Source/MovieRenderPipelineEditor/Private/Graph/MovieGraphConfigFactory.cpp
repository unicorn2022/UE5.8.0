...
UObject* UMovieGraphConfigFactory::FactoryCreateNew(UClass* Class, UObject* InParent, FName Name, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Could not load the default graph [%ls] specified in Project Settings. Falling back to MRQ-supplied default.", *DefaultGraphPath.GetAssetPathString());
	...
	UE_LOGF(LogMovieRenderPipeline, Warning, "Could not load the default graph [%ls] supplied by MRQ. Falling back to an empty graph.", *DefaultGraphPath.GetAssetPathString());
