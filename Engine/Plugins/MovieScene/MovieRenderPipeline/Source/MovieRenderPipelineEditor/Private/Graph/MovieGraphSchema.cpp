...
namespace UE::MovieGraph::Private
{
	UMovieGraphNode* GetGraphNodeFromEdPin(const UEdGraphPin* InPin)
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "Cannot find the runtime node associated with the editor node (the node may be from a plugin that's not currently loaded).");
			return nullptr;
		}
...
bool UMovieGraphSchema::IsCacheVisualizationOutOfDate(int32 InVisualizationCacheID) const
{
	return CurrentCacheRefreshID != InVisualizationCacheID;
}

int32 UMovieGraphSchema::GetCurrentVisualizationCacheID() const
{
	return CurrentCacheRefreshID;
}

void UMovieGraphSchema::ForceVisualizationCacheClear() const
{
...
bool UMovieGraphSchema::TryCreateConnection(UEdGraphPin* InA, UEdGraphPin* InB) const
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Cannot create a connection to/from a node which is not currently valid (it may be from a plugin that's not currently loaded).");
		return false;
	}
...
void UMovieGraphSchema::BreakPinLinks(UEdGraphPin& TargetPin, bool bSendsNodeNotification) const
{
	...
	if (!IsValid(RuntimeNode))
	{
		UE_LOGF(LogMovieRenderPipeline, Warning, "Cannot remove connection from node which is not currently valid (it may be from a plugin that's not currently loaded).");
		return;
	}
...
{
	// GetTypeColor only considers pin category/subcategory names, so struct types like Vector/Rotator/Transform
	// all resolve to the generic struct color. Check PinSubCategoryObject for well-known struct types first.
	// These types are probably never used in MRG, but this helps UI consistency.
	if (PinType.PinCategory == PC_Struct && PinType.PinSubCategoryObject.IsValid())
	{
		const UGraphEditorSettings* Settings = GetDefault<UGraphEditorSettings>();
