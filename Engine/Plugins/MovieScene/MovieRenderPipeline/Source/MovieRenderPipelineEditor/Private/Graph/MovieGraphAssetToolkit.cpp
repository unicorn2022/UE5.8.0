...
#include "Customizations/Graph/MovieGraphExecuteScriptNodeCustomization.h"
#include "Customizations/Graph/MovieGraphLightModifierCustomization.h"
...
#include "Customizations/Graph/MovieGraphPostProcessPassCustomization.h"
...
TSharedRef<SDockTab> FMovieGraphAssetToolkit::SpawnTab_RenderGraphDetails(const FSpawnTabArgs& Args)
{
	...
	SelectedGraphObjectsDetailsWidget->RegisterInstancedCustomPropertyLayout(
		UMovieGraphLightModifierNode::StaticClass(),
		FOnGetDetailCustomizationInstance::CreateStatic(&FMovieGraphLightModifierCustomization::MakeInstance));

	SelectedGraphObjectsDetailsWidget->RegisterInstancedCustomPropertyTypeLayout(
		FMoviePipelinePostProcessPass::StaticStruct()->GetFName(),
		FOnGetPropertyTypeCustomizationInstance::CreateStatic(&FMovieGraphPostProcessPassCustomization::MakeInstance));
	...
	SelectedGraphObjectsDetailsWidget->RegisterInstancedCustomPropertyLayout(
		UMovieGraphExecuteScriptNode::StaticClass(),
		FOnGetDetailCustomizationInstance::CreateStatic(&FMovieGraphExecuteScriptNodeCustomization::MakeInstance));
...
void FMovieGraphAssetToolkit::RegisterDetailsViewSections()
{
	...
	for (const UClass* NodeClass : UMovieGraphSchema::GetNodeClasses())
	{
		for (TFieldIterator<FProperty> PropertyIt(NodeClass); PropertyIt; ++PropertyIt)
		{
			...
			{
				// Only top-level categories can be added as sections.
				if (CategoryMetadata->Contains(TEXT("|")))
				{
					continue;
				}
