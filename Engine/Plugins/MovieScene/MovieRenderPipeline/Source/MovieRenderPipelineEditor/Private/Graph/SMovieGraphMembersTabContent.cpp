...
namespace UE::MovieGraph::Private
{
	...
	static bool GetIconAndColorFromDataType(const UMovieGraphMember* InGraphMember, const FSlateBrush*& OutPrimaryBrush, FSlateColor& OutIconColor, const FSlateBrush*& OutSecondaryBrush, FSlateColor& OutSecondaryColor)
	{
		if (!InGraphMember)
		{
			return false;
		...
		const UMovieGraphInterfaceBase* InterfaceBase = Cast<UMovieGraphInterfaceBase>(InGraphMember);
		const bool bIsBranch = InterfaceBase ? InterfaceBase->bIsBranch : false;
		constexpr bool bIsWildcard = false;
		const FEdGraphPinType PinType = UMoviePipelineEdGraphNodeBase::GetPinType(InGraphMember->GetValueType(), bIsBranch, bIsWildcard, InGraphMember->GetValueTypeObject());

		OutPrimaryBrush = FAppStyle::GetBrush("Kismet.AllClasses.VariableIcon");
		OutIconColor = GetDefault<UMovieGraphSchema>()->GetPinTypeColor(PinType);
		OutSecondaryBrush = nullptr;
		OutSecondaryColor = FSlateColor(FLinearColor::Transparent);
...
{
	const uint32 SectionID = InCreateData->Action->GetSectionID();

	// For variables, inputs, and outputs, show an icon w/ the color representing the member's type (in addition to the member name)
	if (SectionID == static_cast<uint32>(EActionSection::Variables)
		|| SectionID == static_cast<uint32>(EActionSection::Inputs)
		|| SectionID == static_cast<uint32>(EActionSection::Outputs))
	{
		const FMovieGraphSchemaAction* GraphAction = static_cast<FMovieGraphSchemaAction*>(InCreateData->Action.Get());
		const UMovieGraphMember* Member = Cast<UMovieGraphMember>(GraphAction->ActionTarget);

		const FSlateBrush* PrimaryBrush = FAppStyle::GetBrush("Kismet.AllClasses.VariableIcon");
		FSlateColor IconColor = FSlateColor(FLinearColor::White);
		const FSlateBrush* SecondaryBrush = nullptr;
		FSlateColor SecondaryColor;
		UE::MovieGraph::Private::GetIconAndColorFromDataType(Member, PrimaryBrush, IconColor, SecondaryBrush, SecondaryColor);
