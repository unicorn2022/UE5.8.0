...
bool SaveOutputToGroundTruth(const FString& GroundTruthDirectory, FMoviePipelineOutputData OutputData)
{
	...
	for (FString* FilePath : GeneratedFilePaths)
	{
		...
		{
			UE_LOGF(LogTemp, Error, "Could not generate ground truth, unable to generate relative paths.");
			return false;
		}
		...
		{
			UE_LOGF(LogTemp, Error, "Could not generate ground truth, unable to copy existing image to Automation directory.");
			return false;
		}
...
void AMoviePipelineFunctionalTestBase::CompareRenderOutputToGroundTruth(FMoviePipelineOutputData InOutputData)
{
	...
	if (GroundTruthFilenames.Num() == 0)
	{
		...
		UE_LOGF(LogTemp, Error, "Failed to find a GroundTruth file at %ls, generating one now. Rerun the test after verifying them!", *IdealReportDirectory);
	...
	UE_LOGF(LogTemp, Log, "GroundTruth file located at %ls", *GroundTruthFilename);
	...
	UE_LOGF(LogTemp, Display, "All image sequences from %ls are similar to the Ground Truth.", *MetaData.ScreenShotName);
