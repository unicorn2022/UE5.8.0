...
void FMovieGraphConditionGroupQueryBuilder::AddQueryTypeWidgets(TWeakObjectPtr<UMovieGraphConditionGroupQueryBase> WeakQuery, IDetailChildrenBuilder& ChildrenBuilder) const
{
	...
	for (TFieldIterator<FProperty> PropIt(WeakQuery->GetClass()); PropIt; ++PropIt)
	{
		...
		TSharedPtr<IPropertyHandle> QueryPropertyHandle =
			DetailLayoutBuilder->AddObjectPropertyData({WeakQuery.Get()}, QueryProperty->GetFName());
...
{
	// NAME_None prevents the details panel from restoring (absent) saved expansion state, which would collapse new nodes.
	// This ensures InitiallyCollapsed() == false is respected and condition groups start expanded.
	return NAME_None;
}
...
void SMovieGraphCollectionTreeQueryTypeSelectorWidget::SetQueryType(UClass* InNewQueryType, ESelectInfo::Type SelectInfo)
{
	...
	// Preserve the operation type from the old query so it doesn't reset to Add.
	const EMovieGraphConditionGroupQueryOpType PreviousOpType = WeakQuery->GetOperationType();
	...
	NewQuery->SetOperationType(PreviousOpType);
	WeakConditionGroup->RemoveQuery(WeakQuery.Get());
