...
bool SMoviePipelineGraphPanel::CanCopySelectedNodes() const
{
	...
	if (GraphEditorWidget.IsValid())
	{
		for (UObject* Object : GraphEditorWidget->GetSelectedNodes())
		{
			if (const UMoviePipelineEdGraphNodeBase* GraphNode = Cast<UMoviePipelineEdGraphNodeBase>(Object))
			{
			...
			else if (const UEdGraphNode* EdGraphNode = Cast<UEdGraphNode>(Object))
			{
				// Covers non-MRG nodes, but is likely a Comment node.
				if (EdGraphNode->CanDuplicateNode())
				{
					bAtLeastOneCopyableNode = true;
					break;
				}
...
void SMoviePipelineGraphPanel::PostUndo(bool bSuccess)
{
	if (bSuccess)
	{
		...
		if (GraphEditorWidget.IsValid())
		{
			// Undo has the potential to leave behind an invalid graph or schema. To be on the safe side, close out
			// the editor if this is the case.
			if (!GraphEditorWidget->GetCurrentGraph() || !GraphEditorWidget->GetCurrentGraph()->GetSchema())
			{
				GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->CloseAllEditorsForAsset(CurrentGraph);
				return;
			}
...
void SMoviePipelineGraphPanel::OnSelectedNodesChanged(const TSet<UObject*>& NewSelection)
{
	...
	for (UObject* Obj : NewSelection)
	{
		...
		else if (UEdGraphNode_Comment* CommentNode = Cast<UEdGraphNode_Comment>(Obj))
		{
			// Comment nodes are not MRQ-specific nodes, so they don't have a runtime node.
			// Add the comment node directly so its properties are displayed in the details panel.
			SelectedObjects.Add(CommentNode);
		}
	}
