...
public:
	...
	FText GetPrimaryConfigLabel() const
	{
		...
		if (Job)
		{
			if (Job->IsUsingBasicConfiguration())
			{
				return LOCTEXT("BasicConfigLabel", "Basic");
			}
			...
			if (JobTreeItem.IsValid())
			{
				...
				if (Job)
				{
					...
					if (!Job->IsUsingGraphConfiguration())
					{
						...
						UMoviePipelineEditorBlueprintLibrary::EnsureJobHasDefaultSettings(Job, /*bForce=*/true);
					}
				}
			}
	...
	{
		FScopedTransaction Transaction(LOCTEXT("SetGraphConfig_Transaction", "Set Graph Configuration"));
		...
		for (TSharedPtr<IMoviePipelineQueueTreeItem> Item : AffectedItems)
		{
			...
			{
				UMoviePipelineExecutorJob* Job = JobTreeItem->WeakJob.Get();
				if (Job && !Job->SetConfigMode(EMoviePipelineConfigMode::Graph))
				{
	...
	void OnSwitchToPreset()
	{
		FScopedTransaction Transaction(LOCTEXT("SetPresetConfig_Transaction", "Set Preset Configuration"));
		...
		for (TSharedPtr<IMoviePipelineQueueTreeItem> Item : AffectedItems)
		{
			...
			if (JobTreeItem.IsValid())
			{
				...
				if (Job)
				{
					Job->SetConfigMode(EMoviePipelineConfigMode::Preset);
	...
	EVisibility GetPrimaryConfigModifiedVisibility() const
	{
		...
		if (Job)
		{
			// Basic-mode configs are always saved directly on the job, so there's no external asset to be dirty.
			if (Job->IsUsingBasicConfiguration())
			{
	...
	int32 GetConfigLabelWidgetIndex() const
	{
		UMoviePipelineExecutorJob* Job = WeakJob.Get();
		return (Job && Job->IsUsingBasicConfiguration()) ? 0 : 1;
	}

	FText GetPrimaryConfigLabelWithModifiedIndicator() const
	{
		if (GetPrimaryConfigModifiedVisibility() == EVisibility::Visible)
		{
			return FText::Format(INVTEXT("{0} *"), GetPrimaryConfigLabel());
		}
		return GetPrimaryConfigLabel();
	}
	...
	void BrowseToOutputFolder()
	{
		...
		{
			FText ResolveError;
			const FString ResolvedOutputDir = UMoviePipelineEditorBlueprintLibrary::TryResolveOutputDirectoryFromJob(Job, ResolveError);
			...
			{
				FNotificationInfo Info(LOCTEXT("BrowseToOutputFolder_FailedNotification", "Unable to Open Output Folder"));
				Info.SubText = ResolveError;
	...
	TSharedRef<SWidget> OnGenerateSequencePickerMenu()
	{
		...
		FAssetPickerConfig AssetPickerConfig;
		{
			...
			AssetPickerConfig.SaveSettingsName = TEXT("MoviePipelineSequenceAsset");

			AssetPickerConfig.AssetShowWarningText = LOCTEXT("NoSequencesInPicker_Warning", "No Level Sequences Found");
			AssetPickerConfig.Filter.ClassPaths.Add(ULevelSequence::StaticClass()->GetClassPathName());
			AssetPickerConfig.Filter.bRecursiveClasses = true;
			AssetPickerConfig.OnAssetSelected = FOnAssetSelected::CreateRaw(this, &FMoviePipelineQueueJobTreeItem::OnPickSequenceFromAsset);
		}

		MenuBuilder.BeginSection(NAME_None, LOCTEXT("PickSequence_MenuSection", "Level Sequence"));
		{
	...
	void OnPickSequenceFromAsset(const FAssetData& AssetData)
	{
		FSlateApplication::Get().DismissAllMenus();
		...
		FScopedTransaction Transaction(LOCTEXT("PickJobSequence_Transaction", "Set Job Sequence"));
		...
		for (TSharedPtr<IMoviePipelineQueueTreeItem> Item : AffectedItems)
		{
			...
			if (JobTreeItem.IsValid())
			{
				...
				{
					Job->Modify();
					Job->SetSequence(FSoftObjectPath(LevelSequence));
	...
	TSharedRef<SWidget> OnGenerateConfigPresetPickerMenu()
	{
		...
		return OnGenerateConfigPresetPickerMenuFromClass(
			...
			FExecuteAction::CreateRaw(this, &FMoviePipelineQueueJobTreeItem::OnSwitchToGraph),
			FExecuteAction::CreateRaw(this, &FMoviePipelineQueueJobTreeItem::OnCreateNewGraphAndAssign),
			FExecuteAction::CreateRaw(this, &FMoviePipelineQueueJobTreeItem::OnSwitchToPreset)
			);
	}
	...
	static TSharedRef<SWidget> OnGenerateConfigPresetPickerMenuFromClass(UClass* InClass,
		...
		FExecuteAction InSwitchToGraph, FExecuteAction InCreateNewGraphAndAssign, FExecuteAction InSwitchToPreset)
	{
		if (!TargetJob.IsValid())
		{
			return SNullWidget::NullWidget;
		}
		...
		MenuBuilder.BeginSection(NAME_None, LOCTEXT("CurrentConfig_MenuSection", "Current Configuration"));
		{
			if (!TargetJob->IsUsingGraphConfiguration())
			{
				MenuBuilder.AddMenuEntry(
					LOCTEXT("ResetConfig_Label", "Reset Configuration"),
					LOCTEXT("ResetConfig_Tooltip", "Resets the changes to the config and goes back to the defaults."),
					FSlateIcon(),
			...
			else
			{
				if (bIsShot)
				{
					// Shots can clear their graph, meaning they will inherit from the parent graph.
					MenuBuilder.AddMenuEntry(
				...
				{
					// Primary jobs in graph mode: "Reset Configuration" reassigns the default graph from project settings.
					MenuBuilder.AddMenuEntry(
						LOCTEXT("ResetGraphConfig_Label", "Reset Configuration"),
						LOCTEXT("ResetGraphConfig_Tooltip", "Resets the graph to the default render graph specified in project settings."),
						FSlateIcon(),
						FUIAction(FExecuteAction::CreateLambda([TargetJob]()
						{
							if (TargetJob.IsValid())
							{
								SMoviePipelineQueueEditor::AssignDefaultGraphPresetToJob(TargetJob.Get());
								FSlateApplication::Get().DismissAllMenus();
							}
						})),
						NAME_None,
		...
		if (!bIsShot)
		{
			...
			{
				// Basic radio button
				FUIAction SwitchToBasicAction;
				SwitchToBasicAction.ExecuteAction = FExecuteAction::CreateLambda([TargetJob]()
				{
					if (TargetJob.IsValid())
					{
						FScopedTransaction Transaction(LOCTEXT("SetBasicConfig", "Set Basic Configuration"));
						TargetJob->SetConfigMode(EMoviePipelineConfigMode::Basic);
						FSlateApplication::Get().DismissAllMenus();
					}
				});
				SwitchToBasicAction.GetActionCheckState = FGetActionCheckState::CreateLambda([TargetJob]() -> ECheckBoxState
				{
					return (TargetJob.IsValid() && TargetJob->IsUsingBasicConfiguration()) ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
				});

				MenuBuilder.AddMenuEntry(
					LOCTEXT("ConfigurationType_BasicLabel", "Basic"),
					LOCTEXT("ConfigurationType_BasicTooltip", "Use a simplified set of rendering settings."),
					FSlateIcon(),
					SwitchToBasicAction,
					NAME_None,
				...
				SwitchToPresetAction.ExecuteAction = InSwitchToPreset;
				SwitchToPresetAction.GetActionCheckState = FGetActionCheckState::CreateLambda([TargetJob]() -> ECheckBoxState
				{
					return (TargetJob.IsValid() && !TargetJob->IsUsingGraphConfiguration() && !TargetJob->IsUsingBasicConfiguration())
						? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
				});

				// Graph radio button
				FUIAction SwitchToGraphAction;
				SwitchToGraphAction.ExecuteAction = InSwitchToGraph;
				...
				MenuBuilder.AddMenuEntry(
					LOCTEXT("ConfigurationType_GraphLabel", "Movie Render Graph"),
				...
				// Preset radio button
				MenuBuilder.AddMenuEntry(
					LOCTEXT("ConfigurationType_PresetLabel", "Legacy Preset"),
		...
		// "Create New Asset" is only relevant in Graph mode. Shots additionally require the parent job to be in Graph mode.
		if (TargetJob->IsUsingGraphConfiguration())
		{
		...
		// Asset picker is not applicable in Basic mode (Basic config is edited directly in the details panel).
		if (!TargetJob->IsUsingBasicConfiguration())
		{
...
TSharedRef<SWidget> SQueueJobListRow::GenerateWidgetForColumn(const FName& ColumnName)
{
		...
		return SNew(SBox)
			...
			[
				...
				[
					...
					.OnGetMenuContent(Item.Get(), &FMoviePipelineQueueJobTreeItem::OnGenerateSequencePickerMenu)
					.HasDownArrow(false)
					.ButtonContent()
					[
						SNew(SBox)
	...
	else if (ColumnName == NAME_Settings)
	{
		...
		[
			...
			.WidgetIndex(Item.Get(), &FMoviePipelineQueueJobTreeItem::GetConfigLabelWidgetIndex)

			// Index 0: Basic config — plain text, no hyperlink
			...
			// Index 1: Preset/graph config — hyperlink to open preset picker
			+ SWidgetSwitcher::Slot()
			[
				SNew(SHyperlink)
				.Text(TAttribute<FText>::Create(TAttribute<FText>::FGetter::CreateSP(Item.Get(), &FMoviePipelineQueueJobTreeItem::GetPrimaryConfigLabelWithModifiedIndicator)))
...
struct FMoviePipelineShotItem : IMoviePipelineQueueTreeItem
{
	...
	FText GetShotConfigLabel() const
	{
		...
		if (Shot)
		{
			if (Shot->IsUsingBasicConfiguration())
			{
				const UMoviePipelineBasicConfig* ShotConfig = Shot->GetBasicShotConfig();
				if (ShotConfig && ShotConfig->HasAnyOverrides())
				{
					return LOCTEXT("BasicShotOverrideLabel", "Basic (Override)");
				}

				return LOCTEXT("BasicShotInheritLabel", "Basic");
			}
	...
	FText GetShotConfigLabelWithModifiedIndicator() const
	{
		if (GetShotConfigModifiedVisibility() == EVisibility::Visible)
		{
			return FText::Format(INVTEXT("{0} *"), GetShotConfigLabel());
		}
		return GetShotConfigLabel();
	}

	int32 GetShotConfigLabelWidgetIndex() const
	{
		UMoviePipelineExecutorShot* Shot = WeakShot.Get();
		return (Shot && Shot->IsUsingBasicConfiguration()) ? 0 : 1;
	}
	...
	TSharedRef<SWidget> OnGenerateShotConfigPresetPickerMenu()
	{
		...
		const FExecuteAction SwitchToGraph = nullptr;
...
{
	if (ColumnName == SQueueJobListRow::NAME_Enabled)
	{
		return SNew(SBox)
			.HAlign(HAlign_Center)
			.VAlign(VAlign_Center)
	...
	else if (ColumnName == SQueueJobListRow::NAME_JobName)
	{
		return SNew(SBox)
		.Padding(2.0f)
		[
			...
			[
				SNew(SImage)
				.Image(FAppStyle::Get().GetBrush("Sequencer.Tracks.CinematicShot"))
				.DesiredSizeOverride(FVector2D(16.f, 16.f))
				.ColorAndOpacity(FSlateColor::UseForeground())
			]
	...
	else if (ColumnName == SQueueJobListRow::NAME_Settings)
	{
		...
		[
			SNew(SWidgetSwitcher)
			.WidgetIndex(Item.Get(), &FMoviePipelineShotItem::GetShotConfigLabelWidgetIndex)
			...
			[
				SNew(SHyperlink)
				.Text(TAttribute<FText>::Create(TAttribute<FText>::FGetter::CreateSP(Item.Get(), &FMoviePipelineShotItem::GetShotConfigLabelWithModifiedIndicator)))
...
void SMoviePipelineQueueEditor::Construct(const FArguments& InArgs)
{
	...
	TreeView = SNew(STreeView<TSharedPtr<IMoviePipelineQueueTreeItem>>)
		...
		(
			...
			.FillWidth(0.25f)
			.MinSize(80.f)
			.DefaultLabel(LOCTEXT("QueueHeaderSettings_Text", "Configuration"))
...
{
	UMoviePipelineEditorBlueprintLibrary::AssignDefaultGraphPresetToJob(InJob);
}
...
TArray<UMoviePipelineExecutorJob*> SMoviePipelineQueueEditor::GetSelectedJobs() const
{
	TArray<UMoviePipelineExecutorJob*> Jobs;
	for (const TSharedPtr<IMoviePipelineQueueTreeItem>& Item : TreeView->GetSelectedItems())
	{
		if (UMoviePipelineExecutorJob* Job = Item->GetOwningJob())
		{
			Jobs.AddUnique(Job);
