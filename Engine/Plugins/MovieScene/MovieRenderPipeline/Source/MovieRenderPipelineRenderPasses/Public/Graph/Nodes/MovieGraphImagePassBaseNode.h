...
public:
	...
	/** DEPRECATED - How many spatial samples should be used during warm-up frames? */
	UE_DEPRECATED(5.8, "Override the version that takes arguments instead, this version is no longer called.")
	virtual int32 GetNumSpatialSamplesDuringWarmUp() const { return GetNumSpatialSamples(); }

	/**
	 * The number of spatial samples that are performed during warm-up frames. This is called per-frame, so the value returned here can vary
	 * during the warm-up process.
	 */
	UE_API virtual int32 GetNumSpatialSamplesDuringWarmUp(const FMovieGraphTraversalContext& InFrameTraversalContext, const FMovieGraphTimeStepData& InTimeData) const;
...
protected:
	...
	UE_API virtual void UpdateRenderPassTelemetryImpl(FMoviePipelineEndShotRenderTelemetryContext& InOutTelemetry) const override;
	UE_API virtual void GatherRenderCameraSourcesImpl(TArray<TSharedRef<FMovieGraphRenderCameraSource>>& OutSources) const override;
	virtual int32 GetNumSceneViewsRenderedImpl() const { return CurrentInstances.Num(); }
	UE_API virtual void GetPreviewBannerMessagesImpl(const FMovieGraphRenderDataIdentifier& InIdentifier, TArray<FMovieGraphPreviewBannerMessage>& OutMessages) const override;
    // ~UMovieGraphRenderPassNode Interface

	virtual TUniquePtr<UE::MovieGraph::Rendering::FMovieGraphImagePassBase> CreateInstance() const { return nullptr; }

	/**
	 * Called once per pass instance, immediately after the render layer modifiers are activated and
	 * immediately before the instance's Render() is invoked. Override to insert per-layer setup.
	...
	/**
	 * Called once per pass instance, immediately after the instance's Render() returns and before
	 * the render-layer modifiers are cleared. Override to insert per-layer teardown. Does nothing
	 * by default.
	 */
