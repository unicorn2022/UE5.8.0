...
void UMoviePipelineDeferredPassBase::SetupImpl(const MoviePipeline::FMoviePipelineRenderPassInitSettings& InPassInitSettings)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "The 'Add Default Layer' deferred rendering option requires at least one Actor or Data Layer to be specified.");
...

	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "Failed to load Stencil Mask material, stencil layers will be incorrect. Path: %ls", *StencilMatRef.ToString());
		}
	}
	...
	if (bEnableStencilPass)
	{
		...
		if (CVar)
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Log, "Overriding project custom depth/stencil value to support a stencil pass.");
...
void UMoviePipelineDeferredPassBase::TeardownImpl()
{
	...
	if (PreviousCustomDepthValue.IsSet())
	{
		...
		if (CVar)
		{
			...
			{
				UE_LOGF(LogMovieRenderPipeline, Log, "Restoring custom depth/stencil value to: %d", PreviousCustomDepthValue.GetValue());
...
UE::MoviePipeline::FImagePassCameraViewData UMoviePipelineDeferredPassBase::GetCameraInfo(FMoviePipelineRenderPassMetrics& InOutSampleState, IViewCalcPayload* OptPayload) const
{
	...
	else
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Error, "Failed to find Camera Component for Shot: %d CameraIndex: %d", GetPipeline()->GetCurrentShotIndex(), InOutSampleState.OutputState.CameraIndex);
		}
...
void UMoviePipelineDeferredPass_PathTracer::SetupImpl(const MoviePipeline::FMoviePipelineRenderPassInitSettings& InPassInitSettings)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Error, "Cannot render a Path Tracer pass, Path Tracer is not enabled by this project.");
		GetPipeline()->Shutdown(true);
		return;
	}
