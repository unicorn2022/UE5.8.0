...
void UMoviePipelineImageSequenceOutputBase::OnShotFinishedImpl(const UMoviePipelineExecutorShot* InShot, const bool bFlushToDisk)
{
	if (bFlushToDisk)
	{
		UE_LOGF(LogMovieRenderPipelineIO, Log, "ImageSequenceOutputBase flushing %d tasks to disk, inserting a fence in the queue and then waiting...", ImageWriteQueue->GetNumPendingTasks());
	...
	for (TPair<FMoviePipelinePassIdentifier, TUniquePtr<FImagePixelData>>& RenderPassData : InMergedOutputFrame->ImageOutputData)
	{
		...
		FXMLData XMLData;
		{
			...
			// Resolve the final absolute file path to write this to
			{
				...
				const FString FrameTemplatedFormatString = MoviePipeline::GetFrameTemplatedFilePath(FileNameFormatString);	
				GetPipeline()->ResolveFilenameFormatArguments(FrameTemplatedFormatString, FormatOverrides, OutputData.FrameTemplatedFilePath, FinalFormatArgs, &Payload->SampleState.OutputState);

				if (FPaths::IsRelative(OutputData.FrameTemplatedFilePath))
				{
					OutputData.FrameTemplatedFilePath = FPaths::ConvertRelativePathToFull(OutputData.FrameTemplatedFilePath);
				}
			}
		...
		if (RenderPassData.Key.Name == TEXT("FinalImage") || RenderPassData.Key.Name == TEXT("PathTracer")) 
		{
			for (const MoviePipeline::FCompositePassInfo& CompositePass : CompositedPasses)
			{
				...
				{
					UE_LOGF(LogMovieRenderPipeline, Warning, "Composite resolution %dx%d does not match output resolution %dx%d, skipping composite for %ls on camera %ls",
