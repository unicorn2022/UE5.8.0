	...
	else
	{
		...
		if (CameraInfo.ViewInfo.GetOverscan() != CachedOverscan && InOutSampleState.OutputState.IsFirstTemporalSample())
		{
			UE_LOGF(
				...
				Warning,
				"Overscan on camera %ls changed since start of frame %d in shot %ls, scaling resolution by cached overscan value of %f instead to keep frame resolution consistent",
	...
	while (AvailableIndex == INDEX_NONE)
	{
		...
		if (AvailableIndex == INDEX_NONE)
		{
			if (TSharedPtr<FAccumulatorInstance, ESPMode::ThreadSafe> NewAccumulatorInstance = CreateNewAccumulatorInstance())
			{
				...
				UE_LOGF(LogMovieRenderPipeline, Log, "Allocated a Accumulator for Pool %ls, New Pool Count: %d", *GetPoolName().ToString(), Accumulators.Num());
			}
		}
	}
...
namespace MoviePipeline
{
	...
	void AccumulateSample_TaskThread(TUniquePtr<FImagePixelData>&& InPixelData, const MoviePipeline::FImageSampleAccumulationArgs& InParams)
	{
		...
		if (!bIsWellFormed)
		{
			...
			UE_LOGF(LogMovieRenderPipeline, Log, "AccumulateSample_RenderThread: Data is not well formed.");
		...
		// Accumulate the new sample to our target
		{
			...
			UE_LOGF(LogMovieRenderPipeline, VeryVerbose, "Accumulation time: %8.2fms", ElapsedMs);
