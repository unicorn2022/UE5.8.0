...
void UMoviePipelineWaveOutput::BeginFinalizeImpl()
{
	...
	for (int32 SegmentIndex = 0; SegmentIndex < GetPipeline()->GetAudioState().FinishedSegments.Num(); SegmentIndex++)
	{
		...
		// Append the submix name to have a different .wav output per submix
		FString FileNameWithSubmix = FileNameFormatString + Segment.SubmixName;
		FMoviePipelineFormatArgs FinalFormatArgs;
		FString FinalFilePath;
		GetPipeline()->ResolveFilenameFormatArguments(FileNameWithSubmix, FormatOverrides, FinalFilePath, FinalFormatArgs, &Segment.OutputState);
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "Audio Segment took %f seconds to convert to a sample buffer.", (FPlatformTime::Seconds() - StartTime));
	}
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Delaying main thread for %f seconds while audio writes to disk.", CVar->GetValueOnGameThread());
...
void UMoviePipelineWaveOutput::OnShotFinishedImpl(const UMoviePipelineExecutorShot* InShot, const bool bFlushToDisk)
{
	if (bFlushToDisk)
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "UMoviePipelineWaveOutput flushing disk...");
		...
		UE_LOGF(LogMovieRenderPipeline, Log, "Finished flushing tasks to disk after %2.2fs!", ElapsedS);
	}
}
