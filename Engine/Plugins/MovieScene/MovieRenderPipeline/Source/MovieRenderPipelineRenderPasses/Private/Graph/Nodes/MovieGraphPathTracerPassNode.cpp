...
void UMovieGraphPathTracerRenderPassNode::SetupImpl(const FMovieGraphRenderPassSetupData& InSetupData)
{
	...
	if (!HasTemporalDenoiser())
	{
		...
		{
			UE_LOGF(LogMovieRenderPipeline, Warning, "The Path Traced Renderer node of Layer `%ls` enables temporal denoising but no temporal denoiser plugin is enabled." 
	...
	if (!bSupportsPathTracing)
	{
		...
		UE_LOGF(LogMovieRenderPipeline, Warning, "An active Path Traced Renderer node was found, but path tracing support is not enabled. To get "
...
int32 UMovieGraphPathTracerRenderPassNode::GetNumSpatialSamplesDuringWarmUp(const FMovieGraphTraversalContext& InFrameTraversalContext, const FMovieGraphTimeStepData& InTimeData) const
{
