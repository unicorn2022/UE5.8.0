	...
	public:
		...
		virtual void OnCompleteRenderPassDataAvailable_AnyThread(TUniquePtr<FImagePixelData>&& InData)
		{
			...
			FPoolEntry* TargetBlender = nullptr;
			{
				...
				if (!TargetBlender)
				{
					// UE_LOGF(LogMovieRenderPipeline, Log, "Starting a new Output Frame in the Panoramic Blender for frame: %d", DataPayload->SampleState.OutputState.OutputFrameNumber);
...
FString FMovieGraphDeferredPanoramicPass::GetCameraName() const
{
	return LayerData.CameraName;
}

int32 FMovieGraphDeferredPanoramicPass::GetRenderCameraIndex() const
{
...
void FMovieGraphDeferredPanoramicPass::Render(const FMovieGraphTraversalContext& InFrameTraversalContext, const FMovieGraphTimeStepData& InTimeData)
{
	...
	{
		UE_LOGF(LogMovieRenderPipeline, Log, "Set-up Deferred Panoramic Renderer: %ls Layer: %ls OutputRes: [%d, %d] PaneRes: [%d, %d] PaneCount: [%d, %d] bPageToSystemMemory: %d bAutoExposurePass: %d",
