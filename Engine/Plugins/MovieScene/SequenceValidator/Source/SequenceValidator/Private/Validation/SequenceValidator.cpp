...
{
	UE_LOGF(LogSequenceValidator, Log, "Starting sequence validator runner %d", RunnerIndex);
	bIsRunning = true;
...
void FSequenceValidatorTaskRunner::ThreadedRun(FSequenceValidatorTaskScheduler& InScheduler)
{
	...
	UE_LOGF(LogSequenceValidator, Log, "Stopping sequence validator runner %d", RunnerIndex);
	bIsRunning = false;
...
void FSequenceValidatorTaskScheduler::RunQueue(TArrayView<UMovieSceneSequence*> InQueue, FSimpleDelegate&& InOnFinished)
{
	...
	UE_LOGF(LogSequenceValidator, Log, "Preparing %d sequence validator runners", NumConcurrentTasks);
	...
	UE_LOGF(LogSequenceValidator, Log, "All sequence validator runners finished!");
...
FSequenceValidatorTask* FSequenceValidatorTaskScheduler::AcquireNextTask(int32 InRunnerIndex)
{
	...
	{
		UE_LOGF(LogSequenceValidator, Log, "Dispatching sequence validation task %d to runner %d", CurrentTaskIndex, InRunnerIndex);
...
void FSequenceValidatorTaskScheduler::OnRunnerFinished(int32 InRunnerIndex)
{
	...
	UE_LOGF(LogSequenceValidator, Log, "Sequence validator runner %d finished, %d runners remaining", InRunnerIndex, NewNumRemainingRunners);
