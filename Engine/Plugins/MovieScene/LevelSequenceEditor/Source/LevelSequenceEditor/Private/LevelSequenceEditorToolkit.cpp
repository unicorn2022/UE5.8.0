...
void FLevelSequenceEditorToolkit::AddDefaultTracksForActor(AActor& Actor, const FGuid Binding)
{
	...
	{
		Subsystem->AddDefaultTracksForActor(Sequencer.ToSharedRef(), Actor, Binding);
	}
}
...
void FLevelSequenceEditorToolkit::HandleLevelSequenceWithShotsCreated(UObject* LevelSequenceWithShotsAsset)
{
	...
	if (ProjectSettings->DefaultDuration * NumShots > TickResolution.MaxSeconds())
	{
		...
		UE_LOGF(LogLevelSequenceEditor, Warning, "Default shot duration: %f is too large for creating %d shots. Clamping shot duration to: %f", ProjectSettings->DefaultDuration, NumShots, TickResolution.MaxSeconds() / NumShots);
	}
