...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnLevelSequenceGlobalTimeChanged);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnLevelSequencePlayEvent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnLevelSequenceStopEvent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnLevelSequenceRecordEvent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnLevelSequenceBeginScrubbingEvent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnLevelSequenceEndScrubbingEvent);
...
public:
	...
	// The Sequencer that the Bindings are sourced from
...
public:
	...
	/** Called when the global time changes in the active sequencer */
	UPROPERTY(BlueprintAssignable, Category = "Level Sequence Editor")
	FOnLevelSequenceGlobalTimeChanged OnGlobalTimeChanged;

	/** Called when the active sequencer begins playing */
	UPROPERTY(BlueprintAssignable, Category = "Level Sequence Editor")
	FOnLevelSequencePlayEvent OnPlayEvent;

	/** Called when the active sequencer stops playing */
	UPROPERTY(BlueprintAssignable, Category = "Level Sequence Editor")
	FOnLevelSequenceStopEvent OnStopEvent;

	/** Called when the active sequencer toggles recording */
	UPROPERTY(BlueprintAssignable, Category = "Level Sequence Editor")
	FOnLevelSequenceRecordEvent OnRecordEvent;

	/** Called when the user begins scrubbing in the active sequencer */
	UPROPERTY(BlueprintAssignable, Category = "Level Sequence Editor")
	FOnLevelSequenceBeginScrubbingEvent OnBeginScrubbingEvent;

	/** Called when the user ends scrubbing in the active sequencer */
	UPROPERTY(BlueprintAssignable, Category = "Level Sequence Editor")
	FOnLevelSequenceEndScrubbingEvent OnEndScrubbingEvent;
...
private:
	...
	UPROPERTY()
	TArray<TObjectPtr<UMovieSceneBindingPropertyInfoList>> BindingPropertyInfoLists;
...
public:
	...
	UE_API TSharedRef<SWidget> BuildBindingPropertiesDetailsWidget(TSharedRef<ISequencer> Sequencer);
	UE_API void RefreshBindingDetails(TSharedRef<ISequencer> Sequencer, IDetailsView* DetailsView, FGuid ObjectBindingID);
	...
	/** Add the default movie scene tracks for the given actor to its binding in the sequencer's focused sequence. */
	UE_API void AddDefaultTracksForActor(TSharedRef<ISequencer> InSequencer, AActor& Actor, const FGuid Binding);

private:
	void OnFinishedChangingLocators(const FPropertyChangedEvent& PropertyChangedEvent, TWeakPtr<ISequencer> WeakSequencer, TSharedRef<IDetailsView> DetailsView, FGuid ObjectBindingID);
	...
	UMovieSceneBindingPropertyInfoList* GetBindingPropertyInfoList(TSharedRef<ISequencer> Sequencer);

	UE_API void CreatePlaybackDelegates(TSharedRef<ISequencer> InSequencer);

	void HandleGlobalTimeChanged();
	void HandlePlayEvent();
	void HandleStopEvent();
	void HandleRecordEvent();
	void HandleBeginScrubbingEvent();
	void HandleEndScrubbingEvent();

	struct FSequencerPlaybackDelegateHandles
	{
		FDelegateHandle GlobalTimeChanged;
		FDelegateHandle PlayEvent;
		FDelegateHandle StopEvent;
		FDelegateHandle RecordEvent;
		FDelegateHandle BeginScrubbing;
		FDelegateHandle EndScrubbing;
	};
	TMap<TWeakPtr<ISequencer>, FSequencerPlaybackDelegateHandles> SequencerPlaybackDelegateHandles;
