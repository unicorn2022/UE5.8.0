...
public:
	...
	/** Returns true and fills the message if there is the current selection is not in an expectd state for the current painting mode. */
	bool GetSelectionWarning(FText& OutMessage) const;
...
protected:
	...
	void UpdateOnPostCompile(const TArray<struct FAssetCompileData>& Data);
