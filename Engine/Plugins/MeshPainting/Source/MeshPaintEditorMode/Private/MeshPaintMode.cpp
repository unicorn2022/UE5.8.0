...
void UMeshPaintMode::UpdateOnPostCompile(const TArray<FAssetCompileData>& Data)
{
	// Can happen after add/remove nanite data.
	bRecacheValidForPaint = true;
}
...
TArray<ComponentClass*> UMeshPaintMode::GetSelectedComponents() const
{
	...
	if (Components.Num() == 0)
	{
		for (AActor* SelectedActor : SelectionState.SelectedActors)
		{
			if (SelectedActor)
			{
				// Code inside branch is only useful and compiles when ULandscapeNaniteComponent is derived from ComponentClass.
				if constexpr (TIsDerivedFrom<ULandscapeNaniteComponent, ComponentClass>::Value)
				{
					// Selected landscape actors may contain many mesh components that we are not interested in (landscape grass, spline etc).
					// To avoid performance/selection issues we assume that we only want to paint on the Nanite Landscape Components.
					if (SelectedActor->IsA<ALandscapeProxy>())
					{
						TInlineComponentArray<ULandscapeNaniteComponent*> ActorComponents;
						SelectedActor->GetComponents(ActorComponents);
						for (ULandscapeNaniteComponent* Component : ActorComponents)
						{
...
bool UMeshPaintMode::GetSelectionWarning(FText& OutMessage) const
{
	FToolBuilderState SelectionState;
	GetToolManager()->GetContextQueriesAPI()->GetCurrentSelectionState(SelectionState);

	FString ActiveTool = GetToolManager()->GetActiveToolName(EToolSide::Mouse);
	if (ActiveTool == VertexSelectToolName || ActiveTool == VertexColorPaintToolName || ActiveTool == VertexWeightPaintToolName)
	{
		for (UStaticMeshComponent* StaticMeshComponent : GetSelectedComponents<UStaticMeshComponent>())
		{
			if (!StaticMeshComponent->CanMeshPaintVertexColors())
			{
				OutMessage = LOCTEXT("LevelMeshPainter_NaniteVertexPaintWarning", "Part of the selection cannot be vertex painted because it has disabled vertex paint or because it is currently rendered with nanite.");
				return true;
			}
		}
	}
	if (ActiveTool == TextureColorSelectToolName || ActiveTool == TextureColorPaintToolName)
	{
		if (CanAddMeshPaintTextures())
		{
			OutMessage = LOCTEXT("LevelMeshPainter_AddMeshPaintTextureWarning", "Part of the selection has no Mesh Paint Texture and cannot be painted. Use the Add button to fix.");
			return true;
		}
	}
	...
	for (AActor* SelectedActor : SelectionState.SelectedActors)
	{
		if (SelectedActor)
		{
			if (SelectedActor->IsA<ALandscapeProxy>())
			{
				TInlineComponentArray<ULandscapeNaniteComponent*> NaniteComponents;
				SelectedActor->GetComponents<ULandscapeNaniteComponent>(NaniteComponents);
				if (NaniteComponents.Num() == 0)
				{
					OutMessage = LOCTEXT("LevelMeshPainter_NaniteLandscapeWarning", "Part of the selected landscape has no nanite mesh and cannot be painted.");
					return true;
				}
			}
		}
