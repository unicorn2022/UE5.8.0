...
void FMeshPaintModeSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	ResourceCategory.AddCustomRow(LOCTEXT("SelectionStateRow", "Selection State"))
		.NameContent()
		[
			SNew(STextBlock)
				.Font(IDetailLayoutBuilder::GetDetailFont())
				.Text(LOCTEXT("SelectedComponentsName", "Selected Components"))
		]
		.ValueContent()
		[
			SNew(SHorizontalBox)
				...
				[
					SNew(STextBlock)
						...
						.Text_Lambda([]() -> FText
						{
							const int32 NumSelectedComponents = UMeshPaintMode::GetMeshPaintMode()->GetSelectedComponents<UMeshComponent>().Num();
							return FText::Format(LOCTEXT("SelectionCount", "{0}"), FText::AsNumber(NumSelectedComponents));
						})
				]
				+ SHorizontalBox::Slot()
				.HAlign(HAlign_Left)
				...
				[
					SNew(SImage)
						...
						{
							FText Message;
							return UMeshPaintMode::GetMeshPaintMode()->GetSelectionWarning(Message) ? EVisibility::Visible : EVisibility::Hidden;
						})
						.ToolTipText_Lambda([this]()
						{
							FText Message;
							UMeshPaintMode::GetMeshPaintMode()->GetSelectionWarning(Message);
							return Message;
						})
				]
		];
...
void FTexturePaintingSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	TextureCategory.SetSortOrder(4);
...
void FTextureAssetPaintingSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayout)
{
	...
	IDetailCategoryBuilder& WorldChannelCategory = DetailLayout.EditCategory(TEXT("WorldChannels"));
	WorldChannelCategory.SetSortOrder(3);
	IDetailCategoryBuilder& TextureCategory = DetailLayout.EditCategory(TEXT("TexturePainting"));
	TextureCategory.SetSortOrder(4);
	...
	DetailLayout.EditDefaultProperty(UVChannel)->CustomWidget()
	.NameContent()
	[
