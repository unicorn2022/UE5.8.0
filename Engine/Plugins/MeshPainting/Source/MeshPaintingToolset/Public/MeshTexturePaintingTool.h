...
public:
	...
	/** Get all of the write mask properties. */
	virtual void GetWriteMask(bool& bOutWriteRed, bool& bOutWriteGreen, bool& bOutWriteBlue, bool& bOutWriteAlpha) const
	{
		bOutWriteRed = bWriteRed;
		bOutWriteGreen = bWriteGreen;
		bOutWriteBlue = bWriteBlue;
		bOutWriteAlpha = bWriteAlpha;
	}
};
...
public:
	/** The MeshPaintChannelSet asset that defines how a group of chanels map onto texture assets. */
	UPROPERTY(EditAnywhere, Category = WorldChannels, meta = (DisplayThumbnail = "false"))
	TObjectPtr<UMeshPaintChannelSet> ChannelSet;

	/** The currently selected Channel from the ChannelSet. */
	UPROPERTY(EditAnywhere, Category = WorldChannels, meta = (GetOptions = "GetChannelNames", editcondition = "ChannelSet != nullptr"))
	FName ChannelName;

	/** UV channel which should be used for painting textures. */
	UPROPERTY(EditAnywhere, Category = TexturePainting, meta = (TransientToolProperty, editcondition = "ChannelSet == nullptr"))
	int32 UVChannel = 0;

	/** Texture to which painting should be applied. */
	UPROPERTY(EditAnywhere, Category = TexturePainting, meta = (TransientToolProperty, DisplayThumbnail = "true", editcondition = "ChannelSet == nullptr"))
	TObjectPtr<UTexture2D> PaintTexture;

protected:
	/** Helper function to get the ChannelNames from the currently selected ChannelSet. */
	UFUNCTION()
	...
	// Begin UMeshTexturePaintingToolProperties Interface.
	virtual void GetWriteMask(bool& bOutWriteRed, bool& bOutWriteGreen, bool& bOutWriteBlue, bool& bOutWriteAlpha) const override;
	// End UMeshTexturePaintingToolProperties Interface.
};
...
protected:
	...
	UE_API void SetAllTextureOverrides(FMaterialUpdateContext& MaterialUpdateContext);
	UE_API void ClearAllTextureOverrides(FMaterialUpdateContext& MaterialUpdateContext);
	...
	virtual void ApplyWorldChannelUVs(TArrayView<FTexturePaintTriangleInfo> TriangleInfo) const {}
...
protected:
	...
	UE_API virtual void CacheTexturePaintData() override;
	UE_API virtual bool CanPaintTextureToComponent(UTexture* InTexture, UMeshComponent const* InMeshComponent) const override;
	UE_API virtual void ApplyWorldChannelUVs(TArrayView<FTexturePaintTriangleInfo> TriangleInfo) const override;
	// End UMeshTexturePaintingTool Interface.
