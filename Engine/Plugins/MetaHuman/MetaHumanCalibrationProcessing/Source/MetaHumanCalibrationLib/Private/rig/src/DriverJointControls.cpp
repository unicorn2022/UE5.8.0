...
DiffData<T> DriverJointControls::EvaluateRawControlsFromEuler(const DiffData<T>& eulerControls) const
{
    ...
    for (const auto& [jIndex, mapping] : m->mappings)
    {
        ...
        auto quaternion = tdm::quat<float>::from_euler<tdm::rot_seq::xyz>(euler, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});

        if (mapping.rawX >= static_cast<std::uint32_t>(rawControls.size()) ||
            mapping.rawY >= static_cast<std::uint32_t>(rawControls.size()) ||
            mapping.rawZ >= static_cast<std::uint32_t>(rawControls.size()) ||
            mapping.rawW >= static_cast<std::uint32_t>(rawControls.size()))
        {
            CARBON_CRITICAL("DriverJointControls::EvaluateRawControlsFromEuler(): raw control index out of bounds");
        }
        else
        {
...
DiffData<T> DriverJointControls::EvaluateRawControlsFromJoints(const DiffData<T>& jointDiff) const
{
    ...
    for (const auto& [jointIndex, mapping] : m->mappings)
    {
        ...
        if (mapping.rawX >= static_cast<std::uint32_t>(rawControls.size()) ||
            ...
            mapping.rawW >= static_cast<std::uint32_t>(rawControls.size()))
        {
            CARBON_CRITICAL("DriverJointControls::EvaluateRawControlsFromJoints(): raw control index out of bounds");
        }
        else
        {
            rawControls[mapping.rawX] = quaternion.x;
