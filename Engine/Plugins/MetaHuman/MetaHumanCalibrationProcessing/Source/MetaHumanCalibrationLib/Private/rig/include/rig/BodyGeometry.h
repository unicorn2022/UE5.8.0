...
public:
    ...
    State& EvaluateBodyGeometryWithOffset(const int lod, const DiffDataMatrix<T, 3, -1>& offset, const DiffData<T>& diffJoints, const DiffData<T>& diffPsd, State& state) const;
...
private:
    ...
    //! offset Jacobian info (for differentiable offset in EvaluateBodyGeometryWithOffset)
    int offsetJacobianColOffset = -1;
    std::shared_ptr<Eigen::Matrix<T, -1, -1, Eigen::RowMajor>> offsetJacobianRM = std::make_shared<Eigen::Matrix<T, -1, -1, Eigen::RowMajor>>();
