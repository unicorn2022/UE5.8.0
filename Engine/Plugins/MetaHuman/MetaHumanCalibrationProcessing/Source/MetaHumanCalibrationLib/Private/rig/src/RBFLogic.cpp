...
DiffData<T> RBFLogic<T>::EvaluatePoseControlsFromJoints(const DiffData<T>& joints, bool withJointScaling) const
{
    ...
    auto getJointQuaternion = [&](int index) -> tdm::quat<T> {
                const tdm::rad<T> x(joints.Value()[index * (withJointScaling ? 9 : 6) + 3u]);
                const tdm::rad<T> y(joints.Value()[index * (withJointScaling ? 9 : 6) + 4u]);
                const tdm::rad<T> z(joints.Value()[index * (withJointScaling ? 9 : 6) + 5u]);
    ...
    for (std::uint16_t si = 0u; si < m->solvers.size(); ++si)
    {
        ...
        {
            tdm::quat<T> q = getJointQuaternion(m->solverDriverJointIndices[si][i]);
