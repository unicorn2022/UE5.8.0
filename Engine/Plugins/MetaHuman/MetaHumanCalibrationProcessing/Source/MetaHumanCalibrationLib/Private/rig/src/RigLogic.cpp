...
template <class T>
RigLogic<T>::~RigLogic() = default;
template <class T>
RigLogic<T>::RigLogic(RigLogic&&) = default;
template <class T>
...
DiffData<T> RigLogic<T>::EvaluatePSD(const DiffData<T>& rawControls, const Eigen::VectorX<T>& maskWeights) const
{
    ...
    if (hasJacobian)
    {
        ...
        int rowIndex = 0;
        const int mlControlStart = m->rawControlCount + m->psdControlCount;

        // Process raw and PSD controls
        // ML control rows are handled by the neural net jacobian insertion below
        for (int k = 0; k < mlControlStart; ++k)
        {
        ...
        // Insert neural net jacobian values for ML controls
        // Neural net outputs are sorted by outputIndex to ensure correct sparse matrix insertion order
        for (const auto& [outputIndex, netIndex, indexOfNetOutput] : m->neuralNetOutputOrderedByOutputIndex)
        {
            if (outputIndex < rowIndex)
            {
                CARBON_CRITICAL("invalid output index: {} < rowIndex {}", outputIndex, rowIndex);
            }
        ...
        // Process RBF controls
        // These have identity entries in psdToRawMap but the Jacobian w.r.t. raw controls
        // is computed via RBF logic (handled separately)
        while (rowIndex < m->totalControlCount)
        {
            localJacobian.startVec(rowIndex);
            rowIndex++;
        }
...
DiffData<T> RigLogic<T>::EvaluateJoints(const DiffData<T>& psdControls, int lod) const
{
    ...
    {
        const int rotBase = drivingJointIndex * stride + 3;
        if (rotBase + 2 >= output.size())
        {
            CARBON_CRITICAL("RigLogic::EvaluateJoints(): driver joint index {} out of bounds (output size {})", drivingJointIndex, output.size());
        }
        ...
        const auto euler = tdm::quat<float>(x, y, z, w).normalize().euler<tdm::rot_seq::xyz>(tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive}); // -V503
        output[rotBase + 0] = euler[0].value;
        output[rotBase + 1] = euler[1].value;
        output[rotBase + 2] = euler[2].value;
    }
    ...
    if (psdControls.HasJacobian())
    {
        ...
        for (const auto& [drivingJointIndex, mapping] : m->driverJointControls.Mappings())
        {
            ...
            for (auto i = 0; i < 3; i++)
            {
                for (auto j = 0; j < 4; j++)
                {
                    auto row = drivingJointIndex * stride + 3 + i;
                    auto col = mappingQ[j];
...
std::vector<std::tuple<int, int, Eigen::VectorX<T>>> RigLogic<T>::GetAllExpressions() const
{
    ...
    {
        // Allocate at totalControlCount size so EvaluateJoints receives a correctly-sized vector.
        // Only the raw-control portion [0, rawControlCount) is populated; the rest stays zero.
        Eigen::VectorX<T> rawControls = Eigen::Vector<T, -1>::Zero(m->totalControlCount);
        int controlCount = 0;
        bool valid = true;
        for (typename SparseMatrix<T>::InnerIterator it(m->psdToRawMap, k); it; ++it)
        {
    ...
    std::vector<bool> psdsUsed(m->psdToRawMap.outerSize(), false);
    {
        ...
        for (int k = 0; k < int(m->psdToRawMap.outerSize()); ++k)
        {
            ...
            for (typename SparseMatrix<T>::InnerIterator it(m->psdToRawMap, k); it; ++it)
            {
                ...
                if ((it.col() >= (int)rawControlUseCount.size()) || !rawControlUseCount[it.col()])
                {
                    psdUsed = false;
                }
            }
...
template <typename T>
const std::vector<Eigen::VectorX<std::uint16_t>>& RigLogic<T>::GetJointGroupLODs() const
{
    return m->jointGroupOutputRowsPerLOD;
}
...
void RigLogic<T>::SaveJointDeltas(dna::Writer* writer) const
{
    ...
    for (int lod = m->numLODs - 1; lod >= 0; --lod)
    {
        ...
        {
            if (m->jointMatrixPerLOD[lod].row(jointIndexAndDof).nonZeros() == 0)
            {
                continue;
            }
    ...
    {
        if (m->jointMatrixPerLOD[0].row(jointIndexAndDof).nonZeros() == 0)
        {
            continue;
        }
