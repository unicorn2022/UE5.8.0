...
bool BindPoseJointsCalculation::LoadVolumetricJointsJson(const JsonElement& json)
{
    m->jointsToCalculate.clear();
    ...
    if (json.Contains("joint_correspondence"))
    {
        for (const auto& jointData : json["joint_correspondence"].Array())
        {
            ...
            m->jointsToCalculate.push_back(volJointData);
        }
    }
...
bool BindPoseJointsCalculation::LoadVolumetricConfig(const std::string& filepath)
{
	...
	return LoadVolumetricJointsJson(json);
}

bool BindPoseJointsCalculation::LoadSurfaceConfig(const std::string& filepath)
{
	if (std::filesystem::exists(filepath))
	{
		const TITAN_NAMESPACE::JsonElement json = TITAN_NAMESPACE::ReadJson(TITAN_NAMESPACE::ReadFile(filepath));
		return LoadSurfaceJointsMapFromJson(json);
	}
	else
	{
...
bool BindPoseJointsCalculation::LoadSurfaceJointsMapFromJson(const JsonElement& json)
{
	m->surfaceJointsMapping.clear();
	...
	if (json.Contains("joint_correspondence"))
	{
		for (const auto& element : json["joint_correspondence"].Array())
		{
			...
			m->surfaceJointsMapping[jointName] = vID;
		}
	}
