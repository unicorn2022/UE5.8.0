...
{
    Eigen::Matrix<T, 4, 4> J = Eigen::Matrix<T, 4, 4>::Zero();
...
DiffData<T> TwistSwingLogic<T>::EvaluateJointsFromJoints(const DiffData<T>& jointDiff) const
{
    ...
    auto getJointQuaternion = [&](int index) -> tdm::quat<T>
    {
        ...
        return tdm::quat<T>(tdm::rad3<T> { x, y, z }, tdm::rot_seq::xyz, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});
    };
    ...
    for (const auto& [jointIndex, setup] : m->setups)
    {
        ...
        if (setup.twistOutputJointIndices.size() > 0)
        {
            ...
            for (std::size_t i = 0; i < setup.twistOutputJointIndices.size(); ++i)
            {
                ...
                auto euler = tdm::slerp(q, tdm::quat<T> {}, static_cast<T>(setup.twistWeights[i])).euler(tdm::rot_seq::xyz, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});
        ...
        {
            // Read Euler angles using XYZ convention (matching how they were written by RigLogic)
            const tdm::rad<T> swing_x(jointDiff.Value()[jointIndex * (m->withJointScaling ? 9 : 6) + 3u]);
            const tdm::rad<T> swing_y(jointDiff.Value()[jointIndex * (m->withJointScaling ? 9 : 6) + 4u]);
            const tdm::rad<T> swing_z(jointDiff.Value()[jointIndex * (m->withJointScaling ? 9 : 6) + 5u]);
            tdm::quat<T> swing_tdm(tdm::rad3<T> { swing_x, swing_y, swing_z }, tdm::rot_seq::xyz, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});

            // Convert to Eigen quaternion for getSwing operation
            Eigen::Quaternion<T> swing(swing_tdm.w, swing_tdm.x, swing_tdm.y, swing_tdm.z);
            ...
            for (std::uint16_t i = 0u; i < setup.swingOutputJointIndices.size(); i++)
            {
                ...
                // Perform slerp in Eigen
                Eigen::Quaternion<T> result_quat = swing.inverse().slerp(setup.swingWeights[i], identity);

                // Convert to tdm quaternion and output as XYZ Euler
                tdm::quat<T> result_tdm(result_quat.x(), result_quat.y(), result_quat.z(), result_quat.w());
                auto euler = result_tdm.euler(tdm::rot_seq::xyz, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});
    ...
    if (jointDiff.HasJacobian())
    {
        ...
        for (const auto& [jointIndex, setup] : m->setups)
        {
            ...
            const tdm::quat<T> qSrc(eIn, tdm::rot_seq::xyz, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});
    ...
    for (const auto& [jointIndex, setup] : m->setups)
    {
        ...
        if (setup.swingOutputJointIndices.size() > 0)
        {
            ...
            for (std::uint16_t i = 0u; i < setup.swingOutputJointIndices.size(); i++)
            {
                ...
                // Perform slerp in Eigen
                Eigen::Quaternion<T> result_quat = identity.slerp(setup.swingWeights[i], swing);

                // Convert to tdm quaternion and output as XYZ Euler (matching RigLogic convention)
                tdm::quat<T> result_tdm(result_quat.x(), result_quat.y(), result_quat.z(), result_quat.w());
                auto euler = result_tdm.euler(tdm::rot_seq::xyz, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});
        ...
        if (setup.twistOutputJointIndices.size() > 0)
        {
            ...
            {
                const auto outputJointIndex = setup.twistOutputJointIndices[i];
                auto euler = tdm::slerp(twist, tdm::quat<T> {}, static_cast<T>(setup.twistWeights[i])).euler(tdm::rot_seq::xyz, tdm::rot_sign{tdm::rot_dir::positive, tdm::rot_dir::positive, tdm::rot_dir::positive});
                output[outputJointIndex * (m->withJointScaling ? 9 : 6) + 3u] = euler[0].value;
                output[outputJointIndex * (m->withJointScaling ? 9 : 6) + 4u] = euler[1].value;
                output[outputJointIndex * (m->withJointScaling ? 9 : 6) + 5u] = euler[2].value;
            }
