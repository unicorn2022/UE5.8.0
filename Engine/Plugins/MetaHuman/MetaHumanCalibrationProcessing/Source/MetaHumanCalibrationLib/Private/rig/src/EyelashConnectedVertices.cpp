...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashConnectedVertices<T>& eyelashConnectedVertices)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 1)
    {
        success &= io::FromBinaryFile(stream, eyelashConnectedVertices.valid);
        success &= io::FromBinaryFile(stream, eyelashConnectedVertices.indices);
        success &= FromBinaryFile(stream, eyelashConnectedVertices.affine);
        success &= io::FromBinaryFile(stream, eyelashConnectedVertices.headvIDs);
    }
    else
    {
        success = false;
...
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashConnectedVertices<float>& eyelashConnectedVertices);
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashConnectedVertices<double>& eyelashConnectedVertices);
