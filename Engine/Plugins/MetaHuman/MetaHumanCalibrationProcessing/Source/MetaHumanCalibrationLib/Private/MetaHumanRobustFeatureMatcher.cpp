...
static TArray<FVector2D> PointsVectorToTArrayPoints(const std::vector<double>& InPoints)
{
...
static TArray<FVector> PointsVectorToTArray3dPoints(const std::vector<double>& InPoints)
{
	TArray<FVector> Points;

	std::size_t PointsSize = InPoints.size() / 3;
	Points.Reserve(PointsSize);

	for (std::size_t PointIndex = 0; PointIndex < PointsSize; ++PointIndex)
	{
		double X = InPoints[PointIndex * 3];
		double Y = InPoints[PointIndex * 3 + 1];
		double Z = InPoints[PointIndex * 3 + 2];
...
{
	std::map<std::string, TITAN_API_NAMESPACE::OpenCVCameraD> CameraCalibrations;
												 ...
												 TArray<TArray<FVector2D>>& OutPoints3dReprojected)
{
	...
	std::vector<std::vector<double>> Points3dReprojected;
	std::vector<std::vector<bool>> Visibility;

	bool bFoundFeatures = ImplPtr->API.GetDetectedFeatures(InFrame, Points3d, CameraPoints, Points3dReprojected, Visibility);
	...
	OutPoints3d = Private::PointsVectorToTArray3dPoints(Points3d);

	for (const std::vector<double>& Points : CameraPoints)
	{
		OutCameraPoints.Add(Private::PointsVectorToTArrayPoints(Points));
	}

	for (const std::vector<double>& Points : Points3dReprojected)
	{
