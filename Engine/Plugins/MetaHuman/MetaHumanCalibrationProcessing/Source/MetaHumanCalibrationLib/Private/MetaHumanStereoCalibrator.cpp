...
namespace Wrappers
{
	void PointsVectorToTArrayPoints(const std::vector<double>& InPoints, TArray<FVector2D>& OutPoints)
	{
	...
	void TArrayToPointsVector(const TArray<FVector2D>& InPoints, std::vector<double>& OutPoints)
	{
	...
	{
		std::vector<double> DetectedPoints;
	...
	{
		std::vector<std::map<std::string, std::vector<double>>> PointsVectorPerCameraPerFrame;
		std::map<std::string, TITAN_API_NAMESPACE::OpenCVCameraD> Calibrations;
			
		for (const TMap<FString, TArray<FVector2D>>& PointsPerCamera : InPointsPerCameraPerFrame)
		{
			std::map<std::string, std::vector<double>>& PointsVectorPerCamera = PointsVectorPerCameraPerFrame.emplace_back();
			for (const TPair<FString, TArray<FVector2D>>& Points: PointsPerCamera)
			{
				PointsVectorPerCamera[TCHAR_TO_ANSI(*Points.Key)] = std::vector<double>();
	...
	{
		std::map<std::string, TITAN_API_NAMESPACE::OpenCVCameraD> OpenCVCameras;
