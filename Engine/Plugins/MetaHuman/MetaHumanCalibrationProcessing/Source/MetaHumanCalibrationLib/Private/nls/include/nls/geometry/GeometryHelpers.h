 ...
 */
template <class T>
static TITAN_NAMESPACE::Mesh<T> CreatePyramid(const Eigen::Matrix<T, 3, 1>& origin, const Eigen::Matrix<T, 3, 1>& node, T baseDistance)
{
    const Eigen::Matrix<T, 3, 1> direction = node - origin;
    const Eigen::Matrix<T, 3, 1> normal = direction.normalized();

    const Eigen::Matrix<T, 3, 1> basisVector1 = Eigen::Matrix<T, 3, 1>(-normal[1], normal[0], T(0)).normalized();

    const Eigen::Matrix<T, 3, 1> basisVector2 = Eigen::Matrix<T, 3, 1>(normal[1] * basisVector1[2] - normal[2] * basisVector1[1],
                                                             normal[2] * basisVector1[0] - normal[0] * basisVector1[2],
                                                             normal[0] * basisVector1[1] - normal[1] *
                                                             basisVector1[0]).normalized();


    const Eigen::Matrix<T, 3, 1> p1 = origin + baseDistance * basisVector1;
    const Eigen::Matrix<T, 3, 1> p2 = origin - baseDistance * basisVector1;
    const Eigen::Matrix<T, 3, 1> p3 = origin + baseDistance * basisVector2;
    const Eigen::Matrix<T, 3, 1> p4 = origin - baseDistance * basisVector2;
...
static TITAN_NAMESPACE::Mesh<T> CreateSphere(int subdivLevels)
{
    ...
    vertices.col(0) = Eigen::Matrix<T, 3, 1>(0, 0, 1);
    vertices.col(1) = Eigen::Matrix<T, 3, 1>(1, 0, 0);
    vertices.col(2) = Eigen::Matrix<T, 3, 1>(0, 0, -1);
    vertices.col(3) = Eigen::Matrix<T, 3, 1>(-1, 0, 0);
    vertices.col(4) = Eigen::Matrix<T, 3, 1>(0, 1, 0);
    vertices.col(5) = Eigen::Matrix<T, 3, 1>(0, -1, 0);
...
static const Eigen::VectorX<T> CalculateMaskBasedOnMeshTopology(const Mesh<T>& inputMesh, bool &bInvalidMeshTopology, T edgeRatioThreshold = T(10))
{
    ...
    for (int i = 0; i < inputMesh.NumTriangles(); ++i)
    {
        ...
        const Eigen::Matrix<T, 3, 1> v1 = inputMesh.Vertices().col(vID1);
        const Eigen::Matrix<T, 3, 1> v2 = inputMesh.Vertices().col(vID2);
        const Eigen::Matrix<T, 3, 1> v3 = inputMesh.Vertices().col(vID3);

        const Eigen::Matrix<T, 3, 1> a = v1 - v2;
        const Eigen::Matrix<T, 3, 1> b = v2 - v3;
        const Eigen::Matrix<T, 3, 1> c = v3 - v1;
...
template <class T>
static Eigen::VectorX<T> ComputeTemplateCoverageMask(
    const Mesh<T>& templateMesh,
    const Mesh<T>& scanMesh,
    const Eigen::VectorX<T>& scanQualityMask,
    T maxCoverageDistance)
{
    const int numTemplateVerts = templateMesh.NumVertices();
    const int numScanVerts = scanMesh.NumVertices();

    if (scanQualityMask.size() != numScanVerts)
    {
        return Eigen::VectorX<T>::Ones(numTemplateVerts);
    }

    Eigen::VectorX<T> coverageMask(numTemplateVerts);
    const T maxDistSq = maxCoverageDistance * maxCoverageDistance;

    for (int ti = 0; ti < numTemplateVerts; ++ti)
    {
        const Eigen::Matrix<T, 3, 1> tv = templateMesh.Vertices().col(ti);
        T bestDistSq = std::numeric_limits<T>::max();
        int bestIdx = -1;
        for (int si = 0; si < numScanVerts; ++si)
        {
            if (scanQualityMask[si] < T(0.5))
                continue;
            const T dSq = (tv - scanMesh.Vertices().col(si)).squaredNorm();
            if (dSq < bestDistSq)
            {
                bestDistSq = dSq;
                bestIdx = si;
...
static TITAN_NAMESPACE::Mesh<T> CreateTrapezoidalPrism(T start, T end, T startSize, T endSize)
{
    ...
    int count = 0;
    vertices.col(count++) = Eigen::Matrix<T, 3, 1>(-startSize,  startSize, start);
    vertices.col(count++) = Eigen::Matrix<T, 3, 1>( startSize,  startSize, start);
    vertices.col(count++) = Eigen::Matrix<T, 3, 1>( startSize, -startSize, start);
    vertices.col(count++) = Eigen::Matrix<T, 3, 1>(-startSize, -startSize, start);

    vertices.col(count++) = Eigen::Matrix<T, 3, 1>(-endSize,  -endSize, end);
    vertices.col(count++) = Eigen::Matrix<T, 3, 1>( endSize,  -endSize, end);
    vertices.col(count++) = Eigen::Matrix<T, 3, 1>( endSize,   endSize, end);
    vertices.col(count++) = Eigen::Matrix<T, 3, 1>(-endSize,   endSize, end);
...
// ---------------------------------------------------------------------------
// Boundary loop extraction and mesh merging
// ---------------------------------------------------------------------------

/**
 * Extract ordered boundary loops from a mesh.
 * Boundary edges belong to exactly one triangle. Each loop is a closed
 * sequence of vertex indices ordered along the boundary.
 */
template <class T>
static std::vector<std::vector<int>> ExtractBoundaryLoops(const Mesh<T>& mesh)
{
    const std::vector<int> borderVerts = mesh.CalculateBorderVertices();
    if (borderVerts.empty()) return {};

    const std::vector<std::pair<int, int>> borderEdges = mesh.GetEdges(borderVerts);
    if (borderEdges.empty()) return {};

    // Build adjacency: for each border vertex, which other border vertices connect to it
    std::unordered_map<int, std::vector<int>> adj;
    for (const auto& [a, b] : borderEdges)
    {
        adj[a].push_back(b);
        adj[b].push_back(a);
    }
    ...
    for (int startVert : borderVerts)
    {
        if (visited.count(startVert)) continue;
        ...
        while (true)
        {
            ...
            if (next < 0)
            {
                ...
                for (int n : neighbors)
                {
                    if (n == startVert && loop.size() > 2)
                    {
                        next = -2; // signal: loop closed
                        break;
                    }
                }
                break;
...
/**
 * Find pairs of boundary loops from two meshes that are spatially aligned.
 * Returns pairs of (loopIndexA, loopIndexB).
 *
 * @param tolerance      Maximum per-vertex distance to consider aligned (default 0.01)
 * @param allowDiffCount If false (default), only match loops with identical vertex counts
 */
template <class T>
static std::vector<std::pair<int, int>> FindAlignedBoundaryLoops(
    const Mesh<T>& meshA,
    const Mesh<T>& meshB,
    T tolerance = T(0.01),
    bool allowDiffCount = false)
{
    const auto loopsA = ExtractBoundaryLoops(meshA);
    const auto loopsB = ExtractBoundaryLoops(meshB);

    const T tolSq = tolerance * tolerance;
    std::vector<std::pair<int, int>> aligned;

    for (int ia = 0; ia < (int)loopsA.size(); ++ia)
    {
        for (int ib = 0; ib < (int)loopsB.size(); ++ib)
        {
            const auto& la = loopsA[ia];
            const auto& lb = loopsB[ib];

            if (!allowDiffCount && la.size() != lb.size())
                continue;

            // For each vertex in loopA, find nearest in loopB and check distance
            bool allWithinTol = true;
            T maxDist = T(0);
            for (int vi : la)
            {
                const Eigen::Matrix<T, 3, 1> posA = meshA.Vertices().col(vi);
                T bestDistSq = std::numeric_limits<T>::max();
                for (int vj : lb)
                {
                    const T dSq = (posA - meshB.Vertices().col(vj)).squaredNorm();
                ...
                maxDist = std::max(maxDist, std::sqrt(bestDistSq));
            }

            if (allWithinTol)
                aligned.emplace_back(ia, ib);
        }
    }
...
/**
 * Merge two meshes along aligned boundary loops by welding matching vertices.
 * loopA and loopB are ordered vertex index sequences from meshA and meshB respectively.
 * Matched boundary vertices from meshB are remapped to use meshA's vertices.
 *
 * @returns Combined mesh with welded boundary
 */
template <class T>
static Mesh<T> MergeMeshesAlongBoundary(
    const Mesh<T>& meshA,
    const Mesh<T>& meshB,
    const std::vector<int>& loopA,
    const std::vector<int>& loopB)
{
    const int numVertsA = meshA.NumVertices();
    const int numVertsB = meshB.NumVertices();

    // Build correspondence: for each loopB vertex, find nearest loopA vertex
    std::unordered_map<int, int> bToA; // meshB vertex → meshA vertex
    for (int vb : loopB)
    {
        const Eigen::Matrix<T, 3, 1> posB = meshB.Vertices().col(vb);
        T bestDistSq = std::numeric_limits<T>::max();
        int bestA = -1;
        for (int va : loopA)
        {
            const T dSq = (posB - meshA.Vertices().col(va)).squaredNorm();
            if (dSq < bestDistSq)
            {
                bestDistSq = dSq;
                bestA = va;
    ...
    // Build remapping for meshB vertices
    // Non-boundary vertices get new indices after meshA's vertices
    // Boundary vertices that matched get remapped to meshA's vertex index
    std::vector<int> bRemap(numVertsB, -1);
    int nextIdx = numVertsA;
    for (int i = 0; i < numVertsB; ++i)
    {
        auto it = bToA.find(i);
        if (it != bToA.end())
            bRemap[i] = it->second; // welded to meshA vertex
        else
            bRemap[i] = nextIdx++;
    }

    const int totalVerts = nextIdx;

    // Build combined vertices
    Eigen::Matrix<T, 3, -1> vertices(3, totalVerts);
    vertices.leftCols(numVertsA) = meshA.Vertices();
    for (int i = 0; i < numVertsB; ++i)
    {
        if (bRemap[i] >= numVertsA) // non-welded vertex
            vertices.col(bRemap[i]) = meshB.Vertices().col(i);
    }

    // Build combined triangles
    const int numTrisA = meshA.NumTriangles();
    const int numTrisB = meshB.NumTriangles();
    Eigen::Matrix<int, 3, -1> tris(3, numTrisA + numTrisB);
    tris.leftCols(numTrisA) = meshA.Triangles();
    for (int t = 0; t < numTrisB; ++t)
    {
        for (int k = 0; k < 3; ++k)
            tris(k, numTrisA + t) = bRemap[meshB.Triangles()(k, t)];
    }

    // Build combined quads if any
    Eigen::Matrix<int, 4, -1> quads(4, meshA.NumQuads() + meshB.NumQuads());
    if (meshA.NumQuads() > 0)
        quads.leftCols(meshA.NumQuads()) = meshA.Quads();
    for (int q = 0; q < meshB.NumQuads(); ++q)
    {
        for (int k = 0; k < 4; ++k)
            quads(k, meshA.NumQuads() + q) = bRemap[meshB.Quads()(k, q)];
    }

    Mesh<T> result;
    result.SetVertices(vertices);
    result.SetTriangles(tris);
    if (quads.cols() > 0)
