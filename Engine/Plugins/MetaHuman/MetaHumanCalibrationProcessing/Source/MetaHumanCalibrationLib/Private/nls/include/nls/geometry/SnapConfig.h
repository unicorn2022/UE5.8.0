...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, SnapConfig<T>& snapConfig)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 1)
    {
        success &= io::FromBinaryFile(stream, snapConfig.sourceMesh);
        success &= io::FromBinaryFile(stream, snapConfig.sourceVertexIndices);
        success &= io::FromBinaryFile(stream, snapConfig.targetVertexIndices);
    }
    else
    {
        success = false;
