...
Eigen::Vector3<T> RotationMatrixToEulerXYZ(const Eigen::Matrix<T, 3, 3>& R)
{
    ...
    auto wrapToPi = [](T angle) { return std::remainder(angle, T(2 * CARBON_PI)); };

    // Try alternative Euler representation and pick the one with smaller total magnitude
    if (angles[1] != T(0))
    {
        const T sign = angles[1] > T(0) ? T(1) : T(-1);
        Eigen::Vector3<T> alt;
        alt[0] = wrapToPi(angles[0] - T(CARBON_PI));
        alt[1] = sign * T(CARBON_PI) - angles[1];
        alt[2] = wrapToPi(angles[2] - T(CARBON_PI));
        if (alt.cwiseAbs().sum() < angles.cwiseAbs().sum())
        {
            angles = alt;
        }
    }

    // Eigen returns ZYX order, reorder to XYZ
