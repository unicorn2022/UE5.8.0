...
public:
    ...
    template <class T2, int C2>
    friend bool FromBinaryFile(trio::BoundedIOStream* stream, BarycentricCoordinates<T2, C2>& bcs);
...
{
    bool success = true;
    success &= io::ToBinaryFile<int32_t>(pFile, bcs.m_version);
...
template <typename T, int C = 3>
bool FromBinaryFile(trio::BoundedIOStream* stream, BarycentricCoordinates<T, C>& bcs)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 1)
    {
        success &= io::FromBinaryFile(stream, bcs.m_indices);
        success &= io::FromBinaryFile(stream, bcs.m_weights);
    }
    else
    {
        success = false;
