...
void EigenToPointsVector(const Eigen::MatrixX<double>& InPoints, std::vector<double>& OutPoints)
{
...
void PointsVectorToEigen(const std::vector<double>& InPoints, Eigen::MatrixX<double>& OutPoints)
{
...
void PointsVectorToCvPoints(const std::vector<double>& InPoints, std::vector<cv::Point2d>& OutPoints)
{
    int NumPoints = static_cast<int>(InPoints.size()) / 2;
    for (int i = 0; i < NumPoints; i++)
    {
        double x = InPoints[(i * 2)];
        double y = InPoints[(i * 2) + 1];
        OutPoints.push_back(cv::Point2d(x, y));
    }
}

template<typename T>
std::map<std::string, OpenCVCameraTemplate<T>> GetOpenCvCameras(const std::vector<TITAN_NAMESPACE::calib::Camera*>& InCameras)
{
    std::map<std::string, OpenCVCameraTemplate<T>> OpenCVCameras;
    ...
    for (const auto* calibCamera : InCameras)
    {
        ...
        OpenCVCameraTemplate<T> openCVCamera;
        openCVCamera.width = cameraWidth;
        openCVCamera.height = cameraHeight;
        openCVCamera.fx = (T)K(0, 0);
        openCVCamera.fy = (T)K(1, 1);
        openCVCamera.cx = (T)K(0, 2);
        openCVCamera.cy = (T)K(1, 2);
        openCVCamera.k1 = (T)D(0);
        openCVCamera.k2 = (T)D(1);
        openCVCamera.p1 = (T)D(2);
        openCVCamera.p2 = (T)D(3);
        openCVCamera.k3 = (T)D(4);
        ...
        for (int i = 0, row = 0; row < 4; ++row)
        {
            ...
            {
                openCVCamera.Extrinsics[i++] = (T)aff(row, column);
            }
        }
...
bool MultiCameraCalibration::Calibrate(const std::vector<std::map<std::string, std::vector<double>>>& InCornerPointsPerCameraPerFrame,
                                       std::map<std::string, OpenCVCameraD>& OutCalibrations, double& OutMSE)
{
    try
    {
        TITAN_RESET_ERROR;
        ...
        if (calibSucess)
        {
            OutCalibrations = GetOpenCvCameras<real_t>(m->calibContext->getCameras());
...
bool MultiCameraCalibration::ExportCalibrations(const std::map<std::string, OpenCVCameraD>& InCalibrations, const std::string& InExportFilepath) const
{
    try
    {
        TITAN_RESET_ERROR;
