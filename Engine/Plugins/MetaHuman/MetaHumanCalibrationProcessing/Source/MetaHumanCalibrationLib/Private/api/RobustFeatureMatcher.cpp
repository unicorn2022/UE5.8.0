...
static std::vector<double> EigenToPointsVector(const Eigen::MatrixX<double>& InPoints)
{
...
static std::vector<double> EigenTo3dPointsVector(const Eigen::MatrixX<double>& InPoints)
{
    std::vector<double> Points;
    for (Eigen::MatrixX<double>::Index Row = 0; Row < InPoints.rows(); ++Row)
    {
...
bool RobustFeatureMatcher::Init(const std::string& InCalibrationFilePath,
                               double InReprojectionThreshold,
                               double InRatioThreshold,
                               FeatureDetectorType InFeatureDetectorType)
{
    ...
    pImpl->Params.setFeatureDetectorType(static_cast<int>(InFeatureDetectorType));
...
bool RobustFeatureMatcher::Init(const std::map<std::string, OpenCVCameraD>& InCalibrationInformation,
                                double InReprojectionThreshold,
                                double InRatioThreshold,
                                FeatureDetectorType InFeatureDetectorType)
{
    ...
    std::transform(InCalibrationInformation.begin(), InCalibrationInformation.end(),
                   std::back_inserter(MetaShapeCalibrationInformation), [](const std::pair<std::string, OpenCVCameraD>& InElem)
    {
                                               ...
                                               std::vector<std::vector<bool>>& OutVisibility)
{
    try
    {
        ...
        Eigen::MatrixX<double> Points3d;
        std::vector<std::vector<bool>> Visibility;
        std::vector<Eigen::MatrixX<double>> CameraPoints;
        std::vector<Eigen::MatrixX<double>> Points3dReprojected;
        pImpl->Context.getScene(InFrame, Points3d, Visibility, CameraPoints, Points3dReprojected);

        OutPoints3d = EigenTo3dPointsVector(Points3d);

        auto TransformLambda = [](const Eigen::MatrixX<double>& InElem)
        {
        ...
        OutVisibility = std::move(Visibility);
                                               ...
                                               std::map<size_t, std::vector<std::vector<bool>>>& OutVisibility)
{
    ...
    {
        bool bSuccess = GetDetectedFeatures(Frame, OutPoints3d[Frame], OutCameraPoints[Frame], OutPoints3dReprojected[Frame], OutVisibility[Frame]);
