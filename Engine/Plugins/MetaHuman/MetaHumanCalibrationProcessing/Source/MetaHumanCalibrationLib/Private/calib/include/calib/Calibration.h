...
struct FBBAParams : public BAParams
{
    bool fixedScale { true };

    // Two points per camera for every frame (using vector instead of pair)
    std::vector<std::vector<std::vector<Eigen::Vector2<real_t>>>> referencePoints;

    real_t referenceDistance = 0.0;
};
