...
struct ObjectDetectorSingle : ObjectDetectorImpl
{
    ...
    std::optional<std::vector<ObjectPlaneProjection*>> tryDetect() override
    {
        ...
        for (size_t i = 0; i < obj_planes_count; i++)
        {
            ...
            {
                CARBON_ASSERT(object->getObjectPlane(i) && object->getObjectPlane(i)->isInternalType(), "Invalid conversion between ObjectPlane* and ObjectPlaneInternal*.");
                auto objectPlane = static_cast<ObjectPlaneInternal*>(object->getObjectPlane(i));
