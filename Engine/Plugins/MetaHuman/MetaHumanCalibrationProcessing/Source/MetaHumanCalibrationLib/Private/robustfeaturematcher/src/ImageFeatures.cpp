...
void featureDetection(const Eigen::MatrixX<real_t>& img,
    ...
    if (featureDetectorType == 1) {
        detector = cv::AKAZE::create();
    } else {
        detector = cv::SIFT::create(static_cast<int>(ptsNumThreshold));
    }

    detector->detect(cvImage, kpts);
    ...
    detector->compute(cvImage, kpts, cvdesc);
...
std::vector<Match> featureMatching(const Eigen::MatrixX<real_t>& descriptor1,
    ...
    if (featureDetectorType == 1) {
        ...
        descriptor1cv.convertTo(descriptor1cv, CV_32F);
        descriptor2cv.convertTo(descriptor2cv, CV_32F);
    ...
    matcher->knnMatch(descriptor1cv, descriptor2cv, knnMatches12, 2);
    matcher->knnMatch(descriptor2cv, descriptor1cv, knnMatches21, 2);
...
void repackVisibility(const std::vector<std::vector<bool>>& inputVis,
                      size_t numCameras,
                      std::vector<std::vector<bool>>& outputVis)
{
    for (size_t i = 0; i < numCameras; ++i)
    {
        std::vector<bool> cameraVis;
        for (size_t j = 0; j < inputVis.size(); ++j)
        {
            cameraVis.push_back(inputVis[j][i]);
        }

        outputVis.push_back(std::move(cameraVis));
    }
}
...
void createMultiViewScene(const std::vector<calib::rfm::Match>& matches, const std::vector<MetaShapeCamera<real_t>>& cameras,
    ...
    repackVisibility(visibility, cameras.size(), vis);
}
