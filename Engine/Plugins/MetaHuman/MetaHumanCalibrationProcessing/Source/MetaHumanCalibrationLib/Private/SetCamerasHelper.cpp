...
namespace UE
{
	namespace Wrappers
	{
		void SetCamerasHelper(const TArray<FCameraCalibration>& InCalibrations, std::map<std::string, TITAN_API_NAMESPACE::OpenCVCameraD>& OutCameras)
		{
			OutCameras.clear();

			for (const FCameraCalibration& Calibration : InCalibrations)
			{
				TITAN_API_NAMESPACE::OpenCVCameraD Camera;
		...
		void GetCalibrationsHelper(const std::map<std::string, TITAN_API_NAMESPACE::OpenCVCameraD>& InCameras, TArray<FCameraCalibration>& OutCalibrations)
		{
			OutCalibrations.Empty();
			for (const std::pair<const std::string, TITAN_API_NAMESPACE::OpenCVCameraD>& InCamera : InCameras)
			{
