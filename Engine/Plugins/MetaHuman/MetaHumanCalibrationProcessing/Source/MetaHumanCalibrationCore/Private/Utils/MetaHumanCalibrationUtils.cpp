...
{
	TOptional<FImage> ImageOpt = GetImage(InFullImagePath);
	...
	FImage Image = MoveTemp(ImageOpt.GetValue());
...
TArray64<uint8> GetImageData(const FString& InFullImagePath)
{
	TOptional<FImage> MaybeImage = GetImage(InFullImagePath);
	if (MaybeImage.IsSet())
	{
		return MaybeImage.GetValue().RawData;
	}
...
FString GetCameraId(TObjectPtr<const UObject> InObject)
{
	check(InObject);
	...
	if (Metadata.HasValue(InObject, CameraIdTag))
	{
		return Metadata.GetValue(InObject, CameraIdTag);
	}
