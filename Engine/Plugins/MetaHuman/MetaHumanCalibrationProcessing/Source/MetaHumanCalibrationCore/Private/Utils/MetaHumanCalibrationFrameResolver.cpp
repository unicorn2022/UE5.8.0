...
static TArray<TPair<FString, FString>> GetTimecodedFrames(const TArray<FString>& InCameraPaths,
												 const FTimecode& InTimecode,
												 const FFrameRate& InFrameRate,
												 const FFrameRate& InTargetFrameRate)
{
	TArray<TPair<FString, FString>> CameraTimecodedFrame;
	TArray<FString> SeenTimecode;
	...
	for (int32 Index = 0; Index < InCameraPaths.Num(); ++Index)
	{
		...
		if (!SeenTimecode.Contains(NewTimecode.ToString()))
		{
			SeenTimecode.Add(NewTimecode.ToString());
			CameraTimecodedFrame.Add(TPair<FString, FString>(NewTimecode.ToString(), InCameraPaths[Index]));
		}
	}
																   ...
																   const FMetaHumanCalibrationFrameResolver::FCameraDescriptor& InSecondCamera)
{
	...
	TArray<TPair<FString, FString>> FirstCameraTimecodedFrames = GetTimecodedFrames(InFirstCamera.GetImagePaths(),
																		   InFirstCamera.GetTimecode(),
																		   FirstFrameRate,
																		   TargetFrameRate);

	TArray<TPair<FString, FString>> SecondCameraTimecodedFrames = GetTimecodedFrames(InSecondCamera.GetImagePaths(),
