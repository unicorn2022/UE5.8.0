...
private:
	...
	void SaveConfig();
	void RunCalibration();
	bool CanRunCalibration() const;
	void ShowCalibrationRMS(bool bInSuccess);
	void RunAutomaticFrameSelection();
	void OpenSettings();

	FMetaHumanCalibrationPatternDetector::FDetectedFrame DetectForFrame(int32 InFrame);
	FMetaHumanCalibrationPatternDetector::FDetectedFrame DetectForFrame(const TArray<FMetaHumanCalibrationPatternDetector::FCameraInfo>& InCameras,
