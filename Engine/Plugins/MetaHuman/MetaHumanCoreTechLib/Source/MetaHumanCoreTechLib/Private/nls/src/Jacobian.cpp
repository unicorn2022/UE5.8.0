...
{
    const Eigen::Matrix<T, -1, -1> block = scale * (m_denseMatrix->transpose() * (*m_denseMatrix));
    for (int r = 0; r < (int)block.rows(); ++r)
    {
        for (int c = 0; c <= r; ++c)
        {
            JtJ.push_back(Eigen::Triplet<T>(this->StartCol() + r, this->StartCol() + c, block(r, c)));
        }
    }
}
