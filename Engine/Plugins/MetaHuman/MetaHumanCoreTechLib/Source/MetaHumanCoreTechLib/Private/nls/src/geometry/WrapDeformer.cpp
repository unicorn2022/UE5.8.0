...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, WrapDeformerParams<T>& params)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 2)
    {
        success &= io::FromBinaryFile(stream, params.bExclusiveBind);
        int falloffType;
        success &= io::FromBinaryFile(stream, falloffType);
        params.falloffType = static_cast<typename WrapDeformerParams<T>::FalloffType>(falloffType);
        success &= io::FromBinaryFile(stream, params.maxDistance);
        success &= io::FromBinaryFile(stream, params.weightThreshold);
        success &= io::FromBinaryFile(stream, params.bAutoWeightThreshold);
        success &= io::FromBinaryFile(stream, params.normalOffset);
        success &= io::FromBinaryFile(stream, params.wrappedMeshVertexIndicesToApplyTo);
    }
    else if (success && version == 1)
    {
        success &= io::FromBinaryFile(stream, params.bExclusiveBind);
...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, WrapDeformer<T>& wrapDeformer)
{
    if (!stream)
    {
        return false;
    ...
    {
        std::shared_ptr<Mesh<T>> driverMesh, wrappedMesh;
        success &= io::FromBinaryFile(stream, driverMesh);
        success &= io::FromBinaryFile(stream, wrappedMesh);
        wrapDeformer.SetMeshes(driverMesh, wrappedMesh);
        success &= io::FromBinaryFile(stream, wrapDeformer.m_driverMeshCorrespondenceClosestPointData);
        success &= FromBinaryFile(stream, wrapDeformer.m_wrappingParams);
    }
    else if (success && version == 1)
    {
        std::shared_ptr<Mesh<T>> driverMesh = std::make_shared<Mesh<T>>();
...
template bool FromBinaryFile(trio::BoundedIOStream* stream, WrapDeformerParams<float>& params);
template bool FromBinaryFile(trio::BoundedIOStream* stream, WrapDeformerParams<double>& params);
template bool FromBinaryFile(trio::BoundedIOStream* stream, WrapDeformer<float>& wrapDeformer);
template bool FromBinaryFile(trio::BoundedIOStream* stream, WrapDeformer<double>& wrapDeformer);
