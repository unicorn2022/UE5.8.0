...
public:
    ...
    * @returns the intersection points as pairs, which contain alpha and control point index below, where alpha values define the intersection point: (1 - alpha) A + alpha B
    */
    template <typename U = T>
    std::vector<std::pair<T, int>> FindIntersections(const Eigen::Vector2<T>& A,
        const Eigen::Vector2<T>& B,
        typename std::enable_if<std::is_same<U, T>::value&& D == 2>::type* = 0) const
    {
        std::vector<std::pair<T, int>> intersections;
        const int numControlPoints = int(m_controlPoints.cols());
        ...
        for (int i = 0; i < numControlPoints - 1; ++i)
        {
            ...
            if (fabs(v2) > T(1e-9))
            {
                ...
                {
                    intersections.push_back(std::pair<T, int>{ alpha, i });
                }
            }
        }
