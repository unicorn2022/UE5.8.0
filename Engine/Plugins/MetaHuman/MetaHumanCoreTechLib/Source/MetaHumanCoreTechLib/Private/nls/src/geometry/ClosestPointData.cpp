...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, ClosestPointData<T>& closestPointData)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 1)
    {
        success &= FromBinaryFile(stream, closestPointData.bcs);
        success &= io::FromBinaryFile(stream, closestPointData.weight);
        success &= io::FromBinaryFile(stream, closestPointData.deltaInLocalCoordSystem);
    }
    else
    {
        success = false;
...
template bool FromBinaryFile(trio::BoundedIOStream* stream, ClosestPointData<float>& closestPointData);
template bool FromBinaryFile(trio::BoundedIOStream* stream, ClosestPointData<double>& closestPointData);
