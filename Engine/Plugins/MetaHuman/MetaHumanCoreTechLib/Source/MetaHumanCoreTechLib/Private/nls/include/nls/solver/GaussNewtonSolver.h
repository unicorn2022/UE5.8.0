...
public:
    struct Settings
    {
        ...
        bool useDenseJtJAccumulation = true;
    };
    ...
    bool Solve(const std::function<Cost<T>(Context<T>*)>& evaluationFunction, int iterations = 1, T reg = DiagonalRegularization<T>()) const
    {
        Context<T> context;
        return Solve(evaluationFunction, context, iterations, reg);
    }

    bool Solve(const std::function<Cost<T>(Context<T>*)>& evaluationFunction, const Settings& settings) const
    {
        Context<T> context;
        return Solve(evaluationFunction, context, settings);
    }

    bool Solve(const std::function<Cost<T>(Context<T>*)>& evaluationFunction, Context<T>& context, int iterations = 1,
               T reg = DiagonalRegularization<T>()) const
    {
        Settings settings;
        settings.iterations = iterations;
    ...
    bool Solve(const std::function<Cost<T>(Context<T>*)>& evaluationFunction, Context<T>& context, const Settings& settings) const;
