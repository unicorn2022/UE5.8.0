...
template <class T>
bool GaussNewtonSolver<T>::Solve(const std::function<Cost<T>(Context<T>*)>& evaluationFunction, Context<T>& context, const Settings& settings) const
{
    const T residualErrorStoppingCriterion = ResidualErrorStoppingCriterion<T>();
    const T predictionReductionStoppingCriterion = PredictionReductionErrorStoppingCriterion<T>();
    ...
    for (int iter = 0; iter < settings.iterations; ++iter)
    {
        ...
        const int numTotalVariables = context.UpdateSize();
        const int numCols = cost.Cols();

        // Jtb = -J^T * f(x) via per-term accumulation
        Vector<T> Jtb = Vector<T>::Zero(numCols);
        cost.AddJtx(Jtb, fx, T(-1));

        Eigen::SparseMatrix<T, Eigen::RowMajor> JtJ;
        if (settings.useDenseJtJAccumulation)
        {
            Eigen::Matrix<T, -1, -1> JtJDense = Eigen::Matrix<T, -1, -1>::Zero(numCols, numCols);
            cost.AddDenseJtJLower(JtJDense, T(1), TaskThreadPool::GlobalInstance(true).get());
            Eigen::Matrix<T, -1, -1, Eigen::RowMajor> JtJFull = JtJDense.template selfadjointView<Eigen::Lower>();
            JtJ = JtJFull.sparseView();
        }
        else
        {
            // Sparse JtJ for large systems (e.g. vertex refinement with thousands of variables)
            std::vector<Eigen::Triplet<T>> triplets;
            cost.AddSparseJtJLower(triplets, T(1));
            SparseMatrix<T> JtJLower(numCols, numCols);
            JtJLower.setFromTriplets(triplets.begin(), triplets.end());
            JtJ = SparseMatrix<T>(JtJLower.template selfadjointView<Eigen::Lower>());
        }
        ...
        Vector<T> finalDx;
        if (numCols < numTotalVariables)
        {
            finalDx = Vector<T>::Zero(numTotalVariables);
            finalDx.head(numCols) = dx;
        }
        else
        {
            finalDx = dx;
