...
bool FMetaHumanFaceTextureAttributeMap::Init(const FString& InTextureSynthesisFolderPath, int32 NumTextures)
{
	...
	if (FFileHelper::LoadFileToString(JsonString, *JsonFilePath))
	{
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonObject))
		{
			...
			{
				UE_LOGF(LogMetaHumanCoreTechLib, Error, "Attribute order is missing.");
				return false;
			}
			const TSharedPtr<FJsonObject>* AttributesJson;
			if (!JsonObject->TryGetObjectField(TEXT("attributes"), AttributesJson))
			{
				UE_LOGF(LogMetaHumanCoreTechLib, Error, "Attributes are missing from json file.");
				return false;
			}
			for (int32 Idx = 0; Idx < AttributeOrderJsonArray->Num(); ++Idx)
			{
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLib, Error, "Expected attribute names.");
					return false;
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLib, Error, "Expected attribute value names.");
					return false;
				}
				for (int32 NameIdx = 0; NameIdx < AttributeValueNamesJsonArray->Num(); ++NameIdx)
				{
					...
					{
						UE_LOGF(LogMetaHumanCoreTechLib, Error, "Expected attribute value name.");
						return false;
					}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLib, Error, "Expected attribute values.");
					return false;
				}
				if (AttributeValuesJsonArray->Num() != NumTextures)
				{
					UE_LOGF(LogMetaHumanCoreTechLib, Error, "Invalid number of attribute values: %d, expected %d.", AttributeValuesJsonArray->Num(), NumTextures);
					return false;
				}
				for (int32 ValueIdx = 0; ValueIdx < AttributeValuesJsonArray->Num(); ++ValueIdx)
				{
					...
					{
						UE_LOGF(LogMetaHumanCoreTechLib, Error, "Expected attribute value.");
						return false;
					}
					if (Value < 0 || Value >= AttributeValueNames.Last().Num())
					{
						UE_LOGF(LogMetaHumanCoreTechLib, Error, "Invalid attribute value: %d not in range [0, %d) for %ls", Value, AttributeValueNames.Last().Num(), *AttributeNames.Last());
						return false;
					}
		...
		{
			UE_LOGF(LogMetaHumanCoreTechLib, Error, "Cannot parse json file %ls", *JsonFilePath);
			return false;
		}
	}
	return true;
