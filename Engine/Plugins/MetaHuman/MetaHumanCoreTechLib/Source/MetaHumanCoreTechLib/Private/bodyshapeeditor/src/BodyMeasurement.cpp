    ...
    else
    {
        ...
        if (GetType() == BodyMeasurement::Circumference && resampling > 0)
        {
            ...
            auto projectToPlane = [&](const Eigen::Vector3f& point) {
                ...
                // .eval() materialises the result; without it the returned Eigen expression
                // holds a reference to local `n` (stack-use-after-scope, ASan-flagged).
                return (point - d * n).eval();
            };
