    ...
    std::vector<std::string>& regionsIncluded)
{
    ...
    auto ModelData = FMetaHumanFileResourceLoader::GetBoundedIOStreamFromFile(filename);
    ...
    for (int r = 0; r < readNumOfRegions; r++)
    {
        ...
        success &= io::FromBinaryFile(ModelData.get(), regionData.name);
        success &= io::FromBinaryFile(ModelData.get(), regionData.mean);
        success &= io::FromBinaryFile(ModelData.get(), regionData.modes);
        success &= io::FromBinaryFile(ModelData.get(), regionData.weights);
        success &= io::FromBinaryFile(ModelData.get(), regionData.combinedVertexMapping);
        success &= io::FromBinaryFile(ModelData.get(), regionData.jointsMapping);
