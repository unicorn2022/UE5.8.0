...
{
    // TS parameters
	JsonElement ts_params_json;
	if (FMetaHumanFileResourceLoader::ResourceExists(ts_params_path))
	{
		ts_params_json = FMetaHumanFileResourceLoader::GetJsonElementForFile(ts_params_path.c_str());
	}
	
    if (!ts::detail::LoadTSParams(ts_params_json, m_params, m_characterParams))
    {
        return false;
    };

    // If the local model has an incorrect version, fail initialization
    if (!VersionCheck())
    {
        return false;
    }
    ...
    TITAN_NAMESPACE::TaskThreadPool* task_thread_pool) const
{
    ...
    const Scalar div3 = static_cast<Scalar>(1.0 / 3.0);
