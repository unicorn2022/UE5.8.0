...
void AutoriggerLogFunction(TITAN_NAMESPACE::LogLevel logLevel, const char* format, ...)
{
	...
	case LogLevel::ERR: UE_LOGF(Autorigger, Error, "%ls", *MessageW); break;
	case LogLevel::FATAL: UE_LOGF(Autorigger, Error, "%ls", *MessageW); break;
	case LogLevel::CRITICAL: UE_LOGF(Autorigger, Error, "%ls", *MessageW); break;
	case LogLevel::WARNING: UE_LOGF(Autorigger, Warning, "%ls", *MessageW); break;
	case LogLevel::VERBOSE: break; // we won't display Verbose messages from titan
	default: UE_LOGF(Autorigger, Error, "%ls", *MessageW);
	}
