...
const coretechlib::titan::api::MetaHumanCreatorAPI* FMetaHumanCharacterIdentity::GetMHCAPIPtr() const
{
	check(Impl->MHCAPI);
	return Impl->MHCAPI.get();
}
...
bool FMetaHumanCharacterIdentity::Init(const FString& InMHCDataPath, const FString& InBodyMHCDataPath, UDNAAsset* InDNAAsset, EMetaHumanCharacterOrientation InDNAAssetOrient)
{
	...
	InDNAAsset->RestoreLegacyUEMHCCompatibility();
	dna::Reader* BinaryDnaReader = InDNAAsset->GetDNAReader()->Unwrap();
	
	return Init(InMHCDataPath, InBodyMHCDataPath, BinaryDnaReader, InDNAAssetOrient);
}

bool FMetaHumanCharacterIdentity::Init(const FString& InMHCDataPath, const FString& InBodyMHCDataPath, TNotNull<dna::Reader*> InDNAReader, EMetaHumanCharacterOrientation InDNAAssetOrient)
{
	...
	Impl->InternalDnaReader = InDNAReader;	
	...
	if (!FFileHelper::LoadFileToArray(DNADataAsBuffer, *CombinedDnaPath))
	{
		UE_LOGFMT(LogMetaHumanCoreTechLib, Error, "failed to load the combined DNA data");
		return false;
		
	}
	TSharedPtr<IDNAReader> CombinedDnaReader = ReadDNAFromBuffer(&DNADataAsBuffer, EDNADataLayer::Geometry);

	int MaxThreads = -1;
	Impl->MHCAPI = TITAN_API_NAMESPACE::MetaHumanCreatorAPI::CreateMHCApi(InDNAReader, TCHAR_TO_UTF8(*InMHCDataPath), MaxThreads, (CombinedDnaReader != nullptr) ? CombinedDnaReader->Unwrap() : nullptr);
	if (!Impl->MHCAPI)
	{
		UE_LOGFMT(LogMetaHumanCoreTechLib, Error, "failed to initialize MHC API");
		return false;
	}

	return true;
#else
	UE_LOGFMT(LogMetaHumanCoreTechLib, Error, "MHC API only works with EditorOnly Data");
	return false;
#endif
}
...
bool FMetaHumanCharacterIdentity::FState::FitToFaceDna(TSharedRef<class IDNAReader> InFaceDna, const FFitToTargetOptions& InFitToTargetOptions)
{
	...
	NewMHCState->SetFaceScale(1.0f);
	NewMHCState->SetEvaluateBodyState(InFitToTargetOptions.bAdaptNeck);
...
bool FMetaHumanCharacterIdentity::FState::FitFromBodyVertices(const TArray<FVector3f>& InBodyStateVertices, const FFitToTargetOptions& InFitToTargetOptions)
{
	check(Impl->MHCState);

	const Eigen::Map<const Eigen::Matrix<float, 3, -1>> VerticesEigen((const float*)InBodyStateVertices.GetData(), 3, InBodyStateVertices.Num());
	...
	if (NewMHCState->FitToTarget(VerticesEigen, FitToTargetOptions))
	{
		std::shared_ptr<TITAN_API_NAMESPACE::MetaHumanCreatorAPI::Settings> NewSettings = NewMHCState->GetSettings()->Clone();
		NewSettings->SetGlobalVertexDeltaScale(1.0f);
		if (InFitToTargetOptions.bDisableHighFrequencyDelta)
...
TArray<FVector3f> FMetaHumanCharacterIdentity::FState::GetHeadPartVerticesFromBody(const TArray<FVector3f>& InBodyStateVertices) const
{
	check(Impl->MHCState);

	const Eigen::Map<const Eigen::Matrix<float, 3, -1>> VerticesEigen(
		(const float*)InBodyStateVertices.GetData(), 3, InBodyStateVertices.Num());

	Eigen::Matrix<float, 3, -1> HeadVerticesEigen;
	if (!Impl->MHCState->GetHeadPartVerticesFromBody(VerticesEigen, HeadVerticesEigen) || HeadVerticesEigen.cols() == 0)
	{
		return {};
	}

	TArray<FVector3f> OutVertices;
	OutVertices.SetNumUninitialized(static_cast<int32>(HeadVerticesEigen.cols()));
	for (int32 I = 0; I < OutVertices.Num(); ++I)
	{
		// DNA space is Y-up: {dna.X, dna.Y, dna.Z} → UE space: {dna.X, dna.Z, dna.Y}
		OutVertices[I] = FVector3f{ HeadVerticesEigen(0, I), HeadVerticesEigen(2, I), HeadVerticesEigen(1, I) };
	}
	return OutVertices;
}

bool FMetaHumanCharacterIdentity::FState::FitWithVertexDeltasFromBody(
	const TArray<FMatrix44f>& InBodyJoints,
	const TArray<FVector3f>& InBodyStateVerticesNoDelta,
	const TArray<FVector3f>& InBodyStateVerticesWithDelta,
	const TArray<FVector3f>& InBodyVertexNormals,
	const TArray<int32>& InNumVerticesPerLod)
{
	check(Impl->MHCState);

	SetBodyJointsAndBodyFaceVertices(InBodyJoints, InBodyStateVerticesWithDelta);
	SetBodyVertexNormals(InBodyVertexNormals, InNumVerticesPerLod);

	const Eigen::Map<const Eigen::Matrix<float, 3, -1>> NoDeltaVerticesEigen((const float*)InBodyStateVerticesNoDelta.GetData(), 3, InBodyStateVerticesNoDelta.Num());
	const Eigen::Map<const Eigen::Matrix<float, 3, -1>> WithDeltaVerticesEigen((const float*)InBodyStateVerticesWithDelta.GetData(), 3, InBodyStateVerticesWithDelta.Num());

	std::shared_ptr<TITAN_API_NAMESPACE::MetaHumanCreatorAPI::State> NewMHCState = Impl->MHCState->Clone();
	NewMHCState->SetFaceScale(1.0f);

	TITAN_API_NAMESPACE::MetaHumanCreatorAPI::State::FitToTargetOptions FitToTargetOptions;
	FitToTargetOptions.alignmentOptions = static_cast<TITAN_API_NAMESPACE::AlignmentOptions>(static_cast<int32>(EAlignmentOptions::None));
	if (NewMHCState->FitToTarget(NoDeltaVerticesEigen, FitToTargetOptions))
	{
		NewMHCState->CalculateM2MHDeltaFromBodyVertices(WithDeltaVerticesEigen);
		Impl->MHCState = NewMHCState;
		Impl->CachedEvaluatedState.Reset();
		return true;
	}
...
void FMetaHumanCharacterIdentity::FState::GetTrianglesIndices(TArray<int32>& OutTriangleIndices) const
{
	check(Impl->MHCAPI);
	Eigen::Matrix3Xi Triangles = Impl->MHCAPI->GetLod0TriangleIndices();
	OutTriangleIndices.SetNumUninitialized(static_cast<int32>(Triangles.size()));
	Eigen::Map<Eigen::Matrix3Xi> OutTriangleIndicesMap(OutTriangleIndices.GetData(), Triangles.rows(), Triangles.cols());
	OutTriangleIndicesMap = Triangles;
}

void FMetaHumanCharacterIdentity::FState::GetFaceModelCoefficients(TArray<float>& OutCoefficients) const
{
	check(Impl->MHCAPI);
...
void FMetaHumanCharacterIdentity::FState::SetFaceModelCoefficients(const TArray<float>& InCoefficients)
{
	check(Impl->MHCAPI && Impl->MHCState);
	std::shared_ptr<TITAN_API_NAMESPACE::MetaHumanCreatorAPI::State> NewMHCState = Impl->MHCState->Clone();
	Eigen::Map<const Eigen::VectorXf> InCoefficientsMap(InCoefficients.GetData(), InCoefficients.Num());
	ensure(Impl->MHCAPI->SetParameters(*NewMHCState, InCoefficientsMap));
	Impl->MHCState = NewMHCState;
	Impl->CachedEvaluatedState.Reset();
}
...
FArchive& operator<<(FArchive& Ar, std::string& InStdString)
{
	...
	else if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogMetaHumanCoreTechLib, Error, "failed to deserialize string");
			Ar.SetError();
			return Ar;
		}
...
void FMetaHumanCharacterIdentity::FState::SetFromBodyVertexDeltaScale(float InFromBodyVertexDeltaScale)
{
	check(Impl->MHCState);
	std::shared_ptr<TITAN_API_NAMESPACE::MetaHumanCreatorAPI::State> NewMHCState = Impl->MHCState->Clone();
	NewMHCState->SetM2MHDeltaScale(InFromBodyVertexDeltaScale);
	Impl->MHCState = NewMHCState;
	Impl->CachedEvaluatedState.Reset();
}

void FMetaHumanCharacterIdentity::FState::ClearFromBodyVertexDeltas()
{
	check(Impl->MHCState);		
	std::shared_ptr<TITAN_API_NAMESPACE::MetaHumanCreatorAPI::State> NewMHCState = Impl->MHCState->Clone();
	NewMHCState->ResetM2MHDelta();
	Impl->MHCState = NewMHCState;
	Impl->CachedEvaluatedState.Reset();
}

float FMetaHumanCharacterIdentity::FState::GetFromBodyVertexDeltaScale() const
{
	check(Impl->MHCState);
	return Impl->MHCState->GetM2MHDeltaScale();	
}
...
TSharedRef<IDNAReader> FMetaHumanCharacterIdentity::FState::StateToDna(TNotNull<dna::Reader*> InDnaReader) const
{
	check(Impl->MHCState);
...
TSharedRef<class IDNAReader> FMetaHumanCharacterIdentity::FState::StateToDna(class UDNAAsset* InFaceDNA) const
{
	...
	DnaWriter->setFrom(InFaceDNA->GetDNAReader()->Unwrap(), dna::DataLayer::All);
	DnaWriter->write();
