...
private:
    ...
    static bool CheckLandmarkInBrowsMask(const Eigen::Matrix<T, 3, -1>& vertices, const Camera<T>& camera, const Affine<T, 3, 3>& mesh2scanTransform, const T mesh2ScanScale,
        const Mesh<T>& browMesh, const Eigen::Matrix<T, 2, -1>& curvePts, int ptIndex, BarycentricCoordinates<T>& landmarkBarycentricCoordinates);
