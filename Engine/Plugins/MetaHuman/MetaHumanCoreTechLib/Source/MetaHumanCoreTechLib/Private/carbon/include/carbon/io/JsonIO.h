...
class JsonElement {
    ...
    public:
        ...
        static JsonElement Obj() { return JsonElement(std::map<std::string, JsonElement>{}); }
        static JsonElement Arr() { return JsonElement(std::vector<JsonElement>{}); }

        template<typename T>
        JsonElement& Set(const std::string& key, T value) & {
            Insert(key, JsonElement(std::move(value)));
            return *this;
        }
        template<typename T>
        JsonElement&& Set(const std::string& key, T value) && {
        ...
        JsonElement& Set(const std::string& key, JsonElement element) & {
        ...
        template<typename T>
        JsonElement& Add(T value) & {
            Append(JsonElement(std::move(value)));
            return *this;
        }
        template<typename T>
        JsonElement&& Add(T value) && {
        ...
        JsonElement& Add(JsonElement element) & {
...
inline JsonElement DeepCopy(const JsonElement& src) { return ReadJson(WriteJson(src)); }
