...
typename BodyGeometry<T>::State& BodyGeometry<T>::EvaluateBodyGeometryWithOffset(const int lod,
                                                                                 const DiffDataMatrix<T, 3, -1>& offset,
                                                                                 const DiffData<T>& diffJoints,
                                                                                 const DiffData<T>& diffPsd,
                                                                                 State& state) const
{
    const bool requiresJacobians = diffJoints.HasJacobian() || diffPsd.HasJacobian() || offset.HasJacobian();

    EvaluateBlendshapes(lod, diffPsd, state);

    state.blendshapeVertices += offset.Matrix();

    // Store offset Jacobian as dense matrix for propagation through skinning
    if (offset.HasJacobian())
    {
        state.offsetJacobianColOffset = offset.Jacobian().StartCol();
        const int numActiveCols = offset.Jacobian().Cols() - offset.Jacobian().StartCol();
        state.offsetJacobianRM->resize(offset.Jacobian().Rows(), numActiveCols);
        offset.Jacobian().CopyToDenseMatrix(*state.offsetJacobianRM);
    }
    else
    {
        state.offsetJacobianColOffset = -1;
        state.offsetJacobianRM->resize(0, 0);
    }
...
void BodyGeometry<T>::EvaluateSkinningWithJacobians(const int lod, State& state) const
{
    ...
    if (state.offsetJacobianColOffset >= 0 && state.offsetJacobianRM->size() > 0)
    {
        maxCols = std::max<int>(int(state.offsetJacobianRM->cols()) + state.offsetJacobianColOffset, maxCols);
        startCol = std::min<int>(state.offsetJacobianColOffset, startCol);
    }
    ...
    auto evaluateVertexSkinning = [&](int start, int end) {
            for (int vID = start; vID < end; ++vID)
            {
                ...
                for (typename SparseMatrix<T>::InnerIterator it(vertexInfluenceWeights[lod], vID); it; ++it)
                {
                    ...
                    // Propagate offset Jacobian through skinning: d(skinned_v)/d(offset) = skinning_linear * d(offset)/d(params)
                    if (state.offsetJacobianColOffset >= 0 && state.offsetJacobianRM->size() > 0)
                    {
                        const int colOffset = state.offsetJacobianColOffset - startCol;
                        denseJacobian.block(3 * vID, colOffset, 3,
                                            state.offsetJacobianRM->cols()) += (weight * state.skinningMatrices[jointIndex].linear()) *
                            state.offsetJacobianRM->block(3 * vID,
                                                          0,
                                                          3,
                                                          state.offsetJacobianRM->cols());
                    }
                }
