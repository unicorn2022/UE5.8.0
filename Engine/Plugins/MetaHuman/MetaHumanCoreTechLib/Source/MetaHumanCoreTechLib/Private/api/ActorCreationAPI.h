...
public:
    ...
    * Set the number of processing threads for the API. Recreates the global thread pool with the specified number of threads
     ...
     * @param[out] OutFractionLandmarksInBrowRegion  The fraction of brow landmarks which were inside the brow region.
     ...
     */
    bool GenerateBrowMeshLandmarks(const std::string& InCameraName, std::string& OutJsonStream, float & OutFractionLandmarksInBrowRegion, bool InConcatenate = false) const;
