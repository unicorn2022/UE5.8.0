...
template <class T> class VertexWeights;
}
...
public:
    ...
    //! @returns LOD 0 triangle indices
    Eigen::Matrix<int, 3, -1> TITAN_API GetLod0TriangleIndices() const;

    //! @returns the face-only PatchBlendModel (reduced to face mesh vertices only, no teeth/eyes)
    std::shared_ptr<TITAN_NAMESPACE::PatchBlendModel<float>> TITAN_API GetFacePatchBlendModel() const;
	
	//! @returns the landmarks used for face landmark tracking 
    std::shared_ptr<TITAN_NAMESPACE::MeshLandmarks<float>> TITAN_API GetFaceTrackingLandmarks() const;

    //! @returns the named vertex-weight mask loaded from the MHC config (e.g. "body_blend"),
    //! or nullptr if not present. Indexed against the face patch blend model's vertex layout
    //! (shares indices with the combined BSE mesh).
    TITAN_API const TITAN_NAMESPACE::VertexWeights<float>* GetMask(const std::string& name) const;
    ...
    //! Sets the state parameters @p InParameters into @p State
    bool TITAN_API SetParameters(State& State, const Eigen::VectorXf& InParameters) const;
...
#pragma warning(pop)
    ...
    bool FitToTarget(const Eigen::Matrix<float, 3, -1>& InBodyVertices, const FitToTargetOptions& options);

    //! Extract the head-mesh vertex block from a combined body vertex array.
    bool GetHeadPartVerticesFromBody(const Eigen::Matrix<float, 3, -1>& InBodyVertices, Eigen::Matrix<float, 3, -1>& OutHeadVertices) const;
    ...
    /**
     * Capture a runtime residual delta between @p targetFaceVertices and the face mesh
     * the current state evaluates to, so subsequent Evaluate() calls land on the target.
     *
     * @param targetFaceVertices  3 x numFaceVertices, in canonical face DNA topology, in
     *                            the same scaled output space that Evaluate produces.
     *                            Caller is responsible for slicing combined-body topology
     *                            down to the face range before calling.
     *
     * Subsequent Evaluate() calls add @c scale * delta on top of the regular pipeline.
     * Calling Calculate again replaces (does not accumulate) the delta. The delta is
     * captured against the *current* face state - if the caller mutates face state
     * afterwards the delta becomes stale until Reset / Calculate is called again.
     * Runtime-only - not serialized.
     */
    bool CalculateM2MHDelta(const Eigen::Matrix<float, 3, -1>& targetFaceVertices);
	
	//! Calculate targetFaceVertices from body vertices and calculates M2MH Delta
	bool CalculateM2MHDeltaFromBodyVertices(const Eigen::Matrix<float, 3, -1>& InBodyVertices);

    //! Clear the M2MH residual delta. Does not change face PCA / per-patch state.
    void ResetM2MHDelta();

    //! Runtime-only scale applied to the M2MH delta during Evaluate. Not serialized.
    void SetM2MHDeltaScale(float scale);
	
	//! Get scale applied to the M2MH delta.
	float GetM2MHDeltaScale() const;

    /**
     * Runtime toggle: when @p enabled is false, UpdateCombinedState skips the bodyState
     * merge so combinedState mirrors faceState (face-only render). Use during M2MH /
     * preview flows where the body-side contribution should be temporarily ignored.
     * Defaults to true; not serialized.
     */
    void SetEvaluateBodyState(bool enabled);
