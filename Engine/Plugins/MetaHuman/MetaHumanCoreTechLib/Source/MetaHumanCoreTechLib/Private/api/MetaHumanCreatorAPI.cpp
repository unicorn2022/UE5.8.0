...
namespace
{
    ...
    {
        if (!FMetaHumanFileResourceLoader::ResourceExists(filename))
        {
    	...
    	auto json = FMetaHumanFileResourceLoader::GetJsonElementForFile(filename);
        meshLandmarks->DeserializeJson(json, rigGeometry.GetMesh(meshName), meshName);
...
public:
    ...
    //! M2MH residual delta - face mesh only (3 x numFaceVertices). Runtime-only,
    //! not serialized. Captures target_face - current_evaluated_face so subsequent
    //! Evaluate calls land on the target. Cleared by ResetM2MHDelta.
    std::shared_ptr<const Eigen::Matrix<float, 3, -1>> m2mhDelta;

    //! Runtime-only multiplier applied to m2mhDelta during Evaluate. Mirrors body
    //! VertexDeltaScale lifecycle - not persisted across save/load.
    float m2mhDeltaScale { 1.0f };

    //! Runtime-only switch - when false, UpdateCombinedState skips the bodyState
    //! merge so combinedState mirrors faceState. Used by m2mh / preview flows where
    //! the body-side contribution should be temporarily ignored. Not persisted.
    bool evaluateBodyState { true };
...
{
    if (bodyState && evaluateBodyState)
    {
    ...
    {
        // No bodyState (or body eval disabled) - clear combinedState so Evaluate
        // falls back to faceState directly. combinedScale tracks face only.
        combinedState.reset();
    ...
    dna::Reader* InBodyDnaReader)
{
    try
    {
    	...
    	const std::string PCAModelDescriptionPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "uemhc_rig_calibration_data.json");
    	const std::string PresetsPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "presets.json");
    	const std::string LandmarksPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "landmarks_config.json");
    	const std::string SymmetryPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "symmetry.json");
    	const std::string MasksPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "masks_face.json");
    	const std::string FaceSurfaceJointsMappingPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "surface_joints_face.json");
    	const std::string BodyJointsMappingPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "surface_joints_body.json");
    	const std::string EyelashesVariantsPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "eyelashes_variants.binary");
    	const std::string TeethVariantsPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "teeth_variants.binary");
    	const std::string SpeciesVariantsPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "species_variants.binary");
    	const std::string HFVariantsPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "hf_variants.binary");
    	const std::string FaceLodGenerationConfigPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "face_lod_generation.binary");
    	const std::string VolumetricJointsConfigPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "volumetric_joints.json");
    	const std::string AssetGenerationConfigPath = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "asset_generation.binary");
    	const std::string inputSkinningWeightsConfigJsonFilename = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "skinningWeightsConfig.json");
    	const std::string boundsConfigFilename = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "bounds_config.json");
    	const std::string faceTrackingLandmarksFilename = FMetaHumanFileResourceLoader::GetResolvedPathForFile(InMhcDataPath, "face_landmarks.json");
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(PCAModelDescriptionPath))
        {
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(inputSkinningWeightsConfigJsonFilename))
        {
            JsonElement skinningWeightsConfigJson;
            try
            {
            	skinningWeightsConfigJson = FMetaHumanFileResourceLoader::GetJsonElementForFile(inputSkinningWeightsConfigJsonFilename);
            }
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(MasksPath))
        {
        	const JsonElement vertexWeightsJson = FMetaHumanFileResourceLoader::GetJsonElementForFile(MasksPath);
        	mhcApi->m->masks = VertexWeights<float>::LoadAllVertexWeights(vertexWeightsJson, rigGeometry->GetMesh(faceMeshIndex).NumVertices());
        }
        {
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(VolumetricJointsConfigPath) || FMetaHumanFileResourceLoader::ResourceExists(FaceSurfaceJointsMappingPath))
        {
            mhcApi->m->bindPoseJointsCalculation = std::make_shared<BindPoseJointsCalculation>();
        }

        if (FMetaHumanFileResourceLoader::ResourceExists(VolumetricJointsConfigPath))
        {
        	const JsonElement json = FMetaHumanFileResourceLoader::GetJsonElementForFile(VolumetricJointsConfigPath);
            mhcApi->m->bindPoseJointsCalculation->LoadVolumetricJointsJson(json);
        }
        if (FMetaHumanFileResourceLoader::ResourceExists(FaceSurfaceJointsMappingPath))
        {
        	const JsonElement json = FMetaHumanFileResourceLoader::GetJsonElementForFile(FaceSurfaceJointsMappingPath);
            mhcApi->m->bindPoseJointsCalculation->LoadSurfaceJointsMapFromJson(json);
        }
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(PresetsPath))
        {
        	const JsonElement json = FMetaHumanFileResourceLoader::GetJsonElementForFile(PresetsPath);
        ...
        const JsonElement symmetryJson = FMetaHumanFileResourceLoader::GetJsonElementForFile(SymmetryPath);
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(BodyJointsMappingPath))
        {
            const JsonElement json = FMetaHumanFileResourceLoader::GetJsonElementForFile(BodyJointsMappingPath);
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(FaceLodGenerationConfigPath))
        {
        ...
        if (FMetaHumanFileResourceLoader::ResourceExists(boundsConfigFilename))
        {
            const JsonElement json = FMetaHumanFileResourceLoader::GetJsonElementForFile(boundsConfigFilename);
    	...
    	// Load face tracking landmarks
        mhcApi->m->faceTrackingLandmarks = std::make_shared<MeshLandmarks<float>>();
        if (FMetaHumanFileResourceLoader::ResourceExists(faceTrackingLandmarksFilename))
        {
            mhcApi->m->faceTrackingLandmarks->Load(faceTrackingLandmarksFilename, rigGeometry->GetMesh(faceMeshIndex));
        }
...
Eigen::Matrix<int, 3, -1> MetaHumanCreatorAPI::GetLod0TriangleIndices() const
{
    constexpr int LodIndex = 0;
    return m->archetypeTriangulatedMeshes[LodIndex].Triangles();
}

std::shared_ptr<PatchBlendModel<float>> MetaHumanCreatorAPI::GetFacePatchBlendModel() const
{
    return m->facePatchBlendModel;
}

std::shared_ptr<MeshLandmarks<float>> MetaHumanCreatorAPI::GetFaceTrackingLandmarks() const
{
	return m->faceTrackingLandmarks;
}

const VertexWeights<float>* MetaHumanCreatorAPI::GetMask(const std::string& name) const
{
    auto it = m->masks.find(name);
    return (it != m->masks.end()) ? &it->second : nullptr;
}
...
bool MetaHumanCreatorAPI::Evaluate(const State& State, float* OutVertices) const
{
    ...
    try
    {
        ...
        // update joints and meshes based on the body joints delta — gated on the runtime
        // evaluateBodyState toggle so face-only render doesn't get warped toward body joints.
        if (State.m->evaluateBodyState && State.m->settings->m->evaluationSettings.updateBodyJoints && State.m->bodyJointPositions)
        {
        ...
        // Apply runtime M2MH residual delta into the face mesh range BEFORE the body
        // delta seam-closure runs. This way the seam blends against the m2mh-corrected
        // face surface (not raw PCA), and the delta is visible whether body state is
        // in play or not. Face mesh only; eyes/teeth keep PCA + face-joint output.
        // Captured by CalculateM2MHDelta in the post-scaling output space, so applied
        // here after scaling + body joint warp.
        if (State.m->m2mhDelta && State.m->m2mhDeltaScale != 0.0f)
        {
            ...
            if ((int)State.m->m2mhDelta->cols() == numFaceVertices)
            {
                Vertices.block(0, faceRange.first, 3, numFaceVertices) +=
                    State.m->m2mhDeltaScale * (*State.m->m2mhDelta);
            }
            else
            {
                LOG_ERROR("M2MH delta size mismatch: {} vs face vertices {}",
                          State.m->m2mhDelta->cols(), numFaceVertices);
            }
        }

        // add body delta — gated solely on evaluateBodyState. Either body is in play
        // (full body integration: merge + joint warp + seam closure) or it isn't.
        if (State.m->evaluateBodyState)
        {
            const int offset = m->patchBlendModelDataManipulator->NumJoints();
            auto it = m->masks.find(BodyBlendMaskName);
            if (it != m->masks.end() && State.m->bodyDeltas)
            {
                for (const auto& [vID, alpha] : it->second.NonzeroVerticesAndWeights())
                {
...
bool MetaHumanCreatorAPI::SetParameters(State& state, const Eigen::VectorXf& inParameters) const
{
    try
    {
        TITAN_RESET_ERROR;
        auto pbmStateConst = state.m->State();
        auto pbmState = std::const_pointer_cast<PatchBlendModel<float>::State>(pbmStateConst);
        pbmState->DeserializeFromVector(inParameters);
        state.m->UpdateCombinedState();
        return true;
    }
    catch (const std::exception& e)
    {
...
bool TITAN_API MetaHumanCreatorAPI::CopyBodyJointsToFace(const dna::Reader* InBodyDnaReader, const dna::Reader* InFaceDnaReader, dna::Writer* InOutDnaWriter, bool bUpdateDescendentJoints) const
{
    try
    {
        ...
        if (bUpdateDescendentJoints) 
        {
            ...
            const float convertToDegrees = static_cast<float>(180 / CARBON_PI);
    ...
    bool useStabModel)
{
    try
    {
        ...
        auto headIt = ValidTargetVertices.find(headLod0MeshIndex);
        const bool hasHead = (headIt != ValidTargetVertices.end());
        ...
        // Eye/teeth-only fit (no head target): the user-picked alignmentOptions
        // is irrelevant for this path. We do a *per-mesh* Procrustes — each
        // input mesh (teeth, left eye, right eye) is aligned independently to
        // the same-meshIndex slice of the current model state with rigid +
        // uniform scale. A joint Procrustes over concat(eye+teeth) baked all
        // the per-cluster offsets into one big rigid transform that didn't
        // actually land each piece in its own slot. Per-mesh fixes that.
        if (!hasHead)
        {
            // `faceTeethEyesPatchBlendModel` is a Reduce() of the full PBM
            // built from a CONTIGUOUS index list starting at the head's
            // full-PBM offset (see CreateMHCApi). So its DeformedVertices
            // output is laid out as [head|teeth|leftEye|rightEye] in
            // modelMeshIds order with NO joint prefix. We have to slice it
            // with cumulative offsets, NOT with full-PBM ranges from
            // patchBlendModelDataManipulator — those would index off into
            // the wrong region of the reduced matrix.
            const Eigen::Matrix<float, 3, -1> currentAllVerts =
                m->mhcApi->m->faceTeethEyesPatchBlendModel->DeformedVertices(*m->faceState);

            std::map<int, std::pair<int, int>> reducedRange; // meshIndex → (startCol, count) in reduced model
            int cumStart = 0;
            for (int mi : m->mhcApi->m->modelMeshIds)
            {
                const auto fullRange = m->mhcApi->m->patchBlendModelDataManipulator->GetRangeForMeshIndex(mi);
                const int nv = fullRange.second - fullRange.first;
                reducedRange[mi] = { cumStart, nv };
                cumStart += nv;
            }

            for (auto&& [meshIndex, vertices] : canonicalMeshVertices)
            {
                auto rit = reducedRange.find(meshIndex);
                if (rit == reducedRange.end())
                {
                    LOG_ERROR("FitToTarget(no-head): mesh {} not in modelMeshIds — skipping", meshIndex);
                    continue;
                }
                const int start = rit->second.first;
                const int nv = rit->second.second;
                if ((int)vertices.cols() != nv)
                {
                    LOG_ERROR("FitToTarget(no-head): mesh {} vert count mismatch ({} vs model {})",
                              meshIndex, (int)vertices.cols(), nv);
                    continue;
                }
                Eigen::Matrix<float, 3, -1> currentMeshVerts = currentAllVerts.block(0, start, 3, nv);

                const Eigen::Vector3f srcCenter = vertices.rowwise().mean();
                const Eigen::Vector3f refCenter = currentMeshVerts.rowwise().mean();
                const Eigen::Vector3f srcMin = vertices.rowwise().minCoeff();
                const Eigen::Vector3f srcMax = vertices.rowwise().maxCoeff();
                const Eigen::Vector3f refMin = currentMeshVerts.rowwise().minCoeff();
                const Eigen::Vector3f refMax = currentMeshVerts.rowwise().maxCoeff();

                float meshScale = 1.0f;
                Affine<float, 3, 3> meshTransform;
                std::tie(meshScale, meshTransform) = Procrustes<float, 3>::AlignRigidAndScale(vertices, currentMeshVerts, /*withRotation=*/true);

                vertices = meshTransform.Transform(meshScale * vertices);

                const Eigen::Vector3f dstMin = vertices.rowwise().minCoeff();
                const Eigen::Vector3f dstMax = vertices.rowwise().maxCoeff();
                const Eigen::Vector3f dstCenter = vertices.rowwise().mean();

                LOG_INFO("FitToTarget(no-head) per-mesh Procrustes: meshIndex={}, verts={}, reducedStart={}, scale={}",
                         meshIndex, nv, start, meshScale);
                LOG_INFO("  src   centroid=({} {} {}) bbox=[{}..{}, {}..{}, {}..{}]",
                         srcCenter.x(), srcCenter.y(), srcCenter.z(),
                         srcMin.x(), srcMax.x(), srcMin.y(), srcMax.y(), srcMin.z(), srcMax.z());
                LOG_INFO("  ref   centroid=({} {} {}) bbox=[{}..{}, {}..{}, {}..{}]",
                         refCenter.x(), refCenter.y(), refCenter.z(),
                         refMin.x(), refMax.x(), refMin.y(), refMax.y(), refMin.z(), refMax.z());
                LOG_INFO("  post  centroid=({} {} {}) bbox=[{}..{}, {}..{}, {}..{}]",
                         dstCenter.x(), dstCenter.y(), dstCenter.z(),
                         dstMin.x(), dstMax.x(), dstMin.y(), dstMax.y(), dstMin.z(), dstMax.z());
            }
        }
        else
        {
            // Head-target path — bit-identical to UE5.8_Release. The
            // `headLod0Vertices` / `numHeadLod0Vertices` / `targetMask` setup
            // is hoisted inside this branch because the no-head path above
            // doesn't use any of them.
            Eigen::Ref<const Eigen::Matrix<float, 3, -1>> headLod0Vertices = headIt->second;
            ...
            if (options.alignmentOptions == AlignmentOptions::None)
            {
                ...
                // Gated on evaluateBodyState so face-only flows skip the body-driven pre-transform — the
                // input target is then taken as-is in canonical space.
                if (m->evaluateBodyState && m->bodyJointPositions && m->mhcApi->m->faceToBodySkinning)
                {
        ...
        auto faceFitMaskIt = m->mhcApi->m->masks.find(FaceFitMaskName);
        if (faceFitMaskIt != m->mhcApi->m->masks.end())
        {
            const auto& mask = faceFitMaskIt->second;
        ...
        // Eye/teeth-only fit (no head target): skip the unified PBM solve
        // entirely. The PBM is identity-coupled across head ↔ eye/teeth, so
        // running RegisterPose with only eye/teeth targets either deforms the
        // head shape (regulariser pulls face patches) or, when we tried
        // pinning the head as targets, collapsed the mesh. Instead, keep the
        // PBM state as it is — head shape exactly preserved — and capture the
        // eye/teeth change as per-vertex deltas. `UpdateVertexDeltas` only
        // writes deltas for meshes present in canonicalMeshVertices, so head
        // verts stay at zero delta.
        ...
        if (hasHead)
        {
            // Existing behaviour: full rewrite — clears every patch's per-vertex
            // delta and writes new ones for the meshes that participated in the
            // fit. Preserved as-is for the head-target path.
            m->UpdateVertexDeltas(newState, canonicalMeshVertices);
        }
        else
        {
            // No-head fit (eye/teeth only): surgical update. Anything not in
            // canonicalMeshVertices keeps its existing per-vertex delta, so a
            // prior head fit's residual layer + any other already-fitted slot
            // remain untouched. Only the targeted eye/teeth slots get rewritten.
            //
            // currentVerts  = patchBlendModel(state)  (with existing deltas)
            // existingDelta = EvaluateVertexDeltas(state)
            // baseEval      = currentVerts − existingDelta  (model without deltas, per-mesh)
            //
            // For targeted meshes:          newDelta[mesh] = target − baseEval[mesh]
            // For un-targeted meshes:       newDelta[mesh] = existingDelta[mesh]  (unchanged)
            const auto& patchBlendModel = m->mhcApi->m->patchBlendModel;
            const Eigen::Matrix<float, 3, -1> currentVerts = patchBlendModel->DeformedVertices(*newState);
            Eigen::Matrix<float, 3, -1> mergedDeltas = newState->EvaluateVertexDeltas(*patchBlendModel);

            int currentPos = m->mhcApi->m->patchBlendModelDataManipulator->NumJoints();
            for (int meshIndex : m->mhcApi->m->modelMeshIds)
            {
                const auto range = m->mhcApi->m->patchBlendModelDataManipulator->GetRangeForMeshIndex(meshIndex);
                const int size = range.second - range.first;
                auto meshIt = canonicalMeshVertices.find(meshIndex);
                if (meshIt != canonicalMeshVertices.end())
                {
                    const auto baseEvalSlice = currentVerts.block(0, currentPos, 3, size)
                                             - mergedDeltas.block(0, currentPos, 3, size);
                    mergedDeltas.block(0, currentPos, 3, size) = meshIt->second - baseEvalSlice;
                }
                currentPos += size;
            }

            // Re-bake from scratch so the patch-level deltas reflect mergedDeltas.
            for (int id = 0; id < patchBlendModel->NumPatches(); ++id)
                newState->SetPatchVertexDeltas(id, Eigen::Matrix<float, 3, -1>());
            newState->BakeVertexDeltas(mergedDeltas, *patchBlendModel);
        }
...
bool MetaHumanCreatorAPI::State::FitToTarget(const Eigen::Matrix<float, 3, -1>& InBodyVertices, const FitToTargetOptions& options)
{
    try
    {
        TITAN_RESET_ERROR;
        constexpr int faceMeshIndex = 0;
        std::pair<int, int> faceRange = m->mhcApi->m->patchBlendModelDataManipulator->GetRangeForMeshIndex(faceMeshIndex);
        const int numFaceVertices = faceRange.second - faceRange.first;
        if (numFaceVertices > InBodyVertices.cols())
        {
            return false;
        }

        std::map<int, Eigen::Ref<const Eigen::Matrix<float, 3, -1>>> HeadPartsVertices;
        HeadPartsVertices.insert(std::pair<int, Eigen::Ref<const Eigen::Matrix<float, 3, -1>>>(faceMeshIndex, InBodyVertices.leftCols(numFaceVertices)));

        return FitToTarget(HeadPartsVertices, options);
    }
    catch (const std::exception& e)
    {
        TITAN_HANDLE_EXCEPTION("failure to fit to target: {}", e.what());
...
bool MetaHumanCreatorAPI::State::GetHeadPartVerticesFromBody(const Eigen::Matrix<float, 3, -1>& InBodyVertices, Eigen::Matrix<float, 3, -1>& OutHeadVertices) const
{
    try
    {
        TITAN_RESET_ERROR;
        constexpr int faceMeshIndex = 0;
        const std::pair<int, int> faceRange = m->mhcApi->m->patchBlendModelDataManipulator->GetRangeForMeshIndex(faceMeshIndex);
        const int numFaceVertices = faceRange.second - faceRange.first;
        if (numFaceVertices > InBodyVertices.cols())
        {
            return false;
        }
        OutHeadVertices =InBodyVertices.leftCols(numFaceVertices);
    	return true;
    }
    catch (const std::exception& e)
    {
        TITAN_HANDLE_EXCEPTION("failure to get head part vertices from body: {}", e.what());
    }
}
...
bool MetaHumanCreatorAPI::State::CalculateM2MHDelta(const Eigen::Matrix<float, 3, -1>& targetFaceVertices)
{
    try
    {
        TITAN_RESET_ERROR;

        const auto faceRange = m->mhcApi->m->patchBlendModelDataManipulator->GetRangeForMeshIndex(0);
        const int numFaceVertices = faceRange.second - faceRange.first;

        if ((int)targetFaceVertices.cols() != numFaceVertices)
        {
            CARBON_CRITICAL("CalculateM2MHDelta: targetFaceVertices has {} cols, expected {}",
                            targetFaceVertices.cols(), numFaceVertices);
        }

        // Evaluate without prior m2mhDelta contribution so the residual is captured
        // against the current PCA + per-patch + body-delta baseline only.
        m->m2mhDelta.reset();

        const int totalSize = m->mhcApi->m->patchBlendModelDataManipulator->Size();
        Eigen::Matrix<float, 3, -1> evaluated(3, totalSize);
        m->mhcApi->Evaluate(*this, evaluated.data());

        m->m2mhDelta = std::make_shared<Eigen::Matrix<float, 3, -1>>(
            targetFaceVertices - evaluated.block(0, faceRange.first, 3, numFaceVertices));
    	return true;
    }
    catch (const std::exception& e)
    {
    	TITAN_HANDLE_EXCEPTION("failure to calculate M2MH delta: {}", e.what());
    }
}
	
bool MetaHumanCreatorAPI::State::CalculateM2MHDeltaFromBodyVertices(const Eigen::Matrix<float, 3, -1>& InBodyVertices)
{
	try
	{
		TITAN_RESET_ERROR;
		...
		Eigen::Ref<const Eigen::Matrix<float, 3, -1>> targetFaceVertices(InBodyVertices.leftCols(numFaceVertices));
		return CalculateM2MHDelta(targetFaceVertices);
	}
	catch (const std::exception& e)
	{
		TITAN_HANDLE_EXCEPTION("failure to calculate M2MH delta from body vertices: {}", e.what());
	}
}

void MetaHumanCreatorAPI::State::ResetM2MHDelta()
{
    m->m2mhDelta.reset();
    m->m2mhDeltaScale = 1.0f;
}

void MetaHumanCreatorAPI::State::SetM2MHDeltaScale(float scale)
{
    m->m2mhDeltaScale = scale;
}

float MetaHumanCreatorAPI::State::GetM2MHDeltaScale() const
{
	return m->m2mhDeltaScale;
}

void MetaHumanCreatorAPI::State::SetEvaluateBodyState(bool enabled)
{
    m->evaluateBodyState = enabled;
    m->UpdateCombinedState();
}
...
bool MetaHumanCreatorAPI::State::Reset(bool resetBody)
{
    try
    {
        ...
        // Drop the runtime M2MH residual delta channel - Reset is supposed to bring
        // the render back to the default face state, leaving the delta in place
        // would silently re-shift subsequent Evaluate calls.
        m->m2mhDelta.reset();
        m->m2mhDeltaScale = 1.0f;
        if (resetBody)
        {
...
bool MetaHumanCreatorAPI::State::Serialize(std::string& OutArchive) const
{
    try
    {
        ...
        StateJson.Insert("EvaluateBodyState", JsonElement(m->evaluateBodyState));
...
bool MetaHumanCreatorAPI::State::Deserialize(const std::string& InArchive)
{
    try
    {
        ...
        // EvaluateBodyState added in Version 3 - older blobs default to true.
        if (Version >= 3 && StateJson.Contains("EvaluateBodyState"))
        {
            m->evaluateBodyState = StateJson["EvaluateBodyState"].Boolean();
        }
        else
        {
            m->evaluateBodyState = true;
        }
...
bool MetaHumanCreatorAPI::State::Serialize(trio::BoundedIOStream* OutputStream) const
{
    try
    {
        ...
        // Added in Version 3 - runtime body-state-evaluation toggle.
        archive(m->evaluateBodyState);
...
bool MetaHumanCreatorAPI::State::Deserialize(trio::BoundedIOStream* InputStream)
{
    try
    {
        ...
        if (m->serializationVersionOnLoad > 3)
        {
