...
struct EyeAssetGeneratorParams
{
    ...
    T eyeballNormalDisplacement = static_cast<T>(0.01);
    T faceLockDistanceThreshold = static_cast<T>(0.1);
    T eyeDistanceThreshold = static_cast<T>(0.2);
...
public:
    ...
    template <class U>
    friend bool FromBinaryFile(trio::BoundedIOStream* stream, EyeAssetGenerator<U>& eyeAssetGenerator);
