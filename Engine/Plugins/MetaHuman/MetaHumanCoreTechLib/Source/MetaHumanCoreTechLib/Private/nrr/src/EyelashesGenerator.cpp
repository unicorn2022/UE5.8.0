...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashesGeneratorParams<T>& params)
{
    if (!stream)
    {
        return false;
    ...
    {
        size_t numEyelashesRoots;
        success &= io::FromBinaryFile(stream, numEyelashesRoots);
        if (success)
        {
            params.eyelashesRoots.resize(numEyelashesRoots);
            for (size_t i = 0; i < params.eyelashesRoots.size(); ++i)
            {
                success &= io::FromBinaryFile(stream, params.eyelashesRoots[i]);
            }
        }
    }
    else
...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashesGenerator<T>& eyelashesGenerator)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 1)
    {
        ...
        eyelashesGenerator.m_driverMesh = driverMesh;
        success &= io::FromBinaryFile(stream, eyelashesMesh);
        eyelashesGenerator.m_eyelashesMesh = eyelashesMesh;

        size_t numEyelashConnectedVertices;
        success &= io::FromBinaryFile(stream, numEyelashConnectedVertices);
        if (success)
        {
            eyelashesGenerator.m_eyelashesConnectedVertices.resize(numEyelashConnectedVertices);
            for (size_t i = 0; i < eyelashesGenerator.m_eyelashesConnectedVertices.size(); ++i)
            {
                eyelashesGenerator.m_eyelashesConnectedVertices[i] = std::make_shared<EyelashConnectedVertices<T>>();
                success &= FromBinaryFile(stream, *eyelashesGenerator.m_eyelashesConnectedVertices[i]);
            }
        }
...
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashesGeneratorParams<float>& params);
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashesGeneratorParams<double>& params);
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashesGenerator<float>& eyelashesGenerator);
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyelashesGenerator<double>& eyelashesGenerator);
