...
struct DeformationModelRigLogic<T>::Private
{
	...
	//! user-defined mutual exclusivity pairs of controls which can be used to add a model constraint favoring activations where two controls cannot be both be activated at the same time
	std::vector<std::tuple<int, int, T>> mutuallyExclusiveControlPairs;

	//! user-defined mutual exclusivity triples of controls which can be used to add a model constraint favoring activations where all three controls cannot be activated at the same time
	//! the three ints are the ids of the 3 controls; the three T's are i) weighting for the constraint, ii) sharpness: how fast the control is activated, and iii) startVal: the starting value for the activation
	std::vector<std::tuple<int, int, int, T, T, T>> mutuallyExclusiveControlTriples;
...
template <class T>
void DeformationModelRigLogic<T>::SetMutuallyExclusiveControlPairs(const std::vector<std::tuple<std::string, std::string, T>>& controlNamesAndWeights)
{
    m->mutuallyExclusiveControlPairs.clear();
    const std::vector<std::string>& guiControlNames = m->rig->GetRigLogic()->GuiControlNames();

    for (const auto& controlTuple : controlNamesAndWeights)
    {
        const auto name1 = std::get<0>(controlTuple);
        const auto name2 = std::get<1>(controlTuple);
        ...
        if ((it1 != guiControlNames.end()) && (it2 != guiControlNames.end()))
        {
            ...
            m->mutuallyExclusiveControlPairs.emplace_back(std::tuple<int, int, T>(id1, id2, std::get<2>(controlTuple)));
        }
        else
        {
...
template <class T>
void DeformationModelRigLogic<T>::SetMutuallyExclusiveControlTriples(const std::vector<std::tuple<std::string, std::string, std::string, T, T, T>>& controlNamesAndWeights)
{
    m->mutuallyExclusiveControlTriples.clear();
    const std::vector<std::string>& guiControlNames = m->rig->GetRigLogic()->GuiControlNames();
    ...
    for (const auto& controlTuple : controlNamesAndWeights)
    {
        const auto name1 = std::get<0>(controlTuple);
        const auto name2 = std::get<1>(controlTuple);
        const auto name3 = std::get<2>(controlTuple);
        auto it1 = std::find(guiControlNames.begin(), guiControlNames.end(), name1);
        auto it2 = std::find(guiControlNames.begin(), guiControlNames.end(), name2);
        auto it3 = std::find(guiControlNames.begin(), guiControlNames.end(), name3);
        if ((it1 != guiControlNames.end()) && (it2 != guiControlNames.end()) && (it3 != guiControlNames.end()))
        {
            const int id1 = static_cast<int>(std::distance(guiControlNames.begin(), it1));
            const int id2 = static_cast<int>(std::distance(guiControlNames.begin(), it2));
            const int id3 = static_cast<int>(std::distance(guiControlNames.begin(), it3));
            m->mutuallyExclusiveControlTriples.emplace_back(std::tuple<int, int, int, T, T, T>(id1, id2, id3, std::get<3>(controlTuple), std::get<4>(controlTuple), std::get<5>(controlTuple)));
        }
        else
        {
            if (it1 == guiControlNames.end())
            ...
            if (it3 == guiControlNames.end())
            {
                LOG_WARNING("{} is not part of the rig", name3);
            }
        }
    }
}
...
template <class T>
Cost<T> DeformationModelRigLogic<T>::EvaluateMutuallyExclusivePairConstraints(const DiffData<T>& guiControls)
{
    Cost<T> cost;
    unsigned count = 0;
    for (const auto& tuple : m->mutuallyExclusiveControlPairs)
    {
        const int id1 = std::get<0>(tuple);
        const int id2 = std::get<1>(tuple);
        const T mutualExclusionWeight = std::get<2>(tuple);
        const T c1 = guiControls.Value()[id1];
        const T c2 = guiControls.Value()[id2];
        ...
        {
            // jacobian for the residual above
            SparseMatrix<T> jacobian(1, guiControls.Size());
            std::array<Eigen::Triplet<T>, 2> trips = {
                Eigen::Triplet<T>(0, id1, c2),
                Eigen::Triplet<T>(0, id2, c1)
            };
            jacobian.setFromTriplets(trips.begin(), trips.end());

            // combine with gui controls jacobian
            cost.Add(DiffData<T>(std::move(residual), guiControls.Jacobian().Premultiply(jacobian)),
                mutualExclusionWeight,
                "mutualExclusionPair_" + std::to_string(count));
        }
        else
        {
            cost.Add(DiffData<T>(residual), mutualExclusionWeight, "mutualExclusionPair_" + std::to_string(count));
        }
        count++;
    }
    return cost;
...
template <class T>
Cost<T> DeformationModelRigLogic<T>::EvaluateMutuallyExclusiveTripleConstraints(const DiffData<T>& guiControls)
{
    Cost<T> cost;
    unsigned count = 0;

    auto sigmoid = [](T c, T sharpness, T startVal)
    {
        T x = sharpness * (c - startVal);
        if (x >= T(0))
        {
            T z = std::exp(-x);
            return T(1) / (T(1) + z);
    ...
    auto sigmoidDerivative = [&](T s)
    {
        return s * (T(1) - s);
    };

    for (const auto& tuple : m->mutuallyExclusiveControlTriples)
    {
        const int id1 = std::get<0>(tuple);
        const int id2 = std::get<1>(tuple);
        const int id3 = std::get<2>(tuple);
        const T mutualExclusionWeight = std::get<3>(tuple);
        const T c1 = guiControls.Value()[id1];
        const T c2 = guiControls.Value()[id2];
        const T c3 = guiControls.Value()[id3];

        // we penalize the case where the three controls are all on at the same time.
        // residual is s1 * s2 * s3
        // where s = sigmoid(sharpness(c - startVal))
        // sharpness is how sharp the switch between control on and control off is
        // startVal is where "on" starts for the control
        const T sharpness = std::get<4>(tuple);
        const T startVal = std::get<5>(tuple);
        Vector<T> residual(1);
        const T s1 = sigmoid(c1, sharpness, startVal);
        const T s2 = sigmoid(c2, sharpness, startVal);
        const T s3 = sigmoid(c3, sharpness, startVal);
        residual[0] = s1 * s2 * s3;
        ...
        if (guiControls.HasJacobian())
        {
            // jacobian for the residual above
            SparseMatrix<T> jacobian(1, guiControls.Size());
            std::array<Eigen::Triplet<T>, 3> trips = {
                Eigen::Triplet<T>(0, id1, s2 * s3 * sharpness * sigmoidDerivative(s1)),
                Eigen::Triplet<T>(0, id2, s1 * s3 * sharpness * sigmoidDerivative(s2)),
                Eigen::Triplet<T>(0, id3, s1 * s2 * sharpness * sigmoidDerivative(s3))
            };
            jacobian.setFromTriplets(trips.begin(), trips.end());
            ...
            cost.Add(DiffData<T>(std::move(residual), guiControls.Jacobian().Premultiply(jacobian)),
                mutualExclusionWeight,
                "mutualExclusionTriple_" + std::to_string(count));
        }
        else
        {
            cost.Add(DiffData<T>(residual), mutualExclusionWeight, "mutualExclusionTriple_" + std::to_string(count));
        }
        count++;
    }
    return cost;
...
Cost<T> DeformationModelRigLogic<T>::EvaluateModelConstraints(Context<T>* context)
{
    ...
    cost.Add(EvaluateMutuallyExclusivePairConstraints(guiControls));
    cost.Add(EvaluateMutuallyExclusiveTripleConstraints(guiControls));
