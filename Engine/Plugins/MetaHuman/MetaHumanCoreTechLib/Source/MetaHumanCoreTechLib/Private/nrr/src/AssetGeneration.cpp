...
{
    const bool isValidFile = FileExists(filenameOrData);
    if (isValidFile)
    {
...
{
    auto ModelData = FMetaHumanFileResourceLoader::GetBoundedIOStreamFromFile(AssetGenerationModelFile);
    ...
    bool bSuccess = true;
    int32_t version = -1;
    bSuccess &= io::FromBinaryFile(ModelData.get(), version);

    if (bSuccess && version == 1)
    {
        bSuccess &= FromBinaryFile(ModelData.get(), m_config);
        bSuccess &= FromBinaryFile(ModelData.get(), m_salivaWrapDeformer);
        bSuccess &= FromBinaryFile(ModelData.get(), m_cartilageWrapDeformer);
        bSuccess &= FromBinaryFile(ModelData.get(), m_eyelashesGenerator);
        bSuccess &= FromBinaryFile(ModelData.get(), m_eyeEdgeGenerator);
        bSuccess &= FromBinaryFile(ModelData.get(), m_eyeshellGenerator);
        std::map<std::string, std::shared_ptr<Mesh<T>>> allMeshes;
        bSuccess &= io::FromBinaryFile(ModelData.get(), allMeshes);
        m_allMeshes.clear();
...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, AssetGenerationConfiguration<T>& config)
{
    bool success = true;
    int32_t version;
    ...
    if (success && version == 1)
    {
        success &= io::FromBinaryFile(stream, config.m_archetypePath);
        success &= io::FromBinaryFile(stream, config.m_salivaDriverMeshName);
        success &= io::FromBinaryFile(stream, config.m_salivaMeshName);
        success &= FromBinaryFile(stream, config.m_salivaWrapDeformerParams);
        success &= io::FromBinaryFile(stream, config.m_cartilageDriverMeshName);
        success &= io::FromBinaryFile(stream, config.m_cartilageMeshName);
        success &= FromBinaryFile(stream, config.m_cartilageWrapDeformerParams);
        success &= io::FromBinaryFile(stream, config.m_eyelashesDriverMeshName);
        success &= io::FromBinaryFile(stream, config.m_eyelashesMeshName);
        success &= FromBinaryFile(stream, config.m_eyelashesGeneratorParams);
        success &= io::FromBinaryFile(stream, config.m_headMeshName);
        success &= io::FromBinaryFile(stream, config.m_eyeLeftMeshName);
        success &= io::FromBinaryFile(stream, config.m_eyeRightMeshName);
        success &= io::FromBinaryFile(stream, config.m_eyeEdgeMeshName);
        success &= io::FromBinaryFile(stream, config.m_eyeshellMeshName);
        success &= FromBinaryFile(stream, config.m_eyeEdgeGeneratorParams);
        success &= FromBinaryFile(stream, config.m_eyeshellGeneratorParams);
    }
    else
    {
        success = false;
...
template bool FromBinaryFile(trio::BoundedIOStream* stream, AssetGenerationConfiguration<float>& config);
template bool FromBinaryFile(trio::BoundedIOStream* stream, AssetGenerationConfiguration<double>& config);
