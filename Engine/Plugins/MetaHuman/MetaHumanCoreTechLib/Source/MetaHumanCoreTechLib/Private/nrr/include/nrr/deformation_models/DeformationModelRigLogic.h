...
public:
    ...
    //! Evaluate the constraint for mutually exclusive pairs of GUI control activations
    Cost<T> EvaluateMutuallyExclusivePairConstraints(const DiffData<T>& guiControls);

    //! Evaluate the constraint for mutually exclusive triples of GUI control activations ie not all three controls can be activated at the same time
    Cost<T> EvaluateMutuallyExclusiveTripleConstraints(const DiffData<T>& guiControls);
    ...
    //! Sets mutual exclusivity pairs of controls.
    void SetMutuallyExclusiveControlPairs(const std::vector< std::tuple<std::string, std::string, T>>& controlNamesAndWeights);

    //! Sets mutual exclusivity triples of controls. The three weights are i) weighting for the constraint, ii) sharpness: how fast the control is activated, and iii) startVal: the starting value for the activation
    void SetMutuallyExclusiveControlTriples(const std::vector< std::tuple<std::string, std::string, std::string, T, T, T>>& controlNamesAndWeights);
