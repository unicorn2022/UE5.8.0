...
template <class T, int D>
void IdentityBlendModel<T, D>::LoadModelJson(const std::string& identityModelFile)
{
	LoadModelJson(FMetaHumanFileResourceLoader::GetJsonElementForFile(identityModelFile));
}
...
bool IdentityBlendModel<T, D>::LoadModelBinary(const std::string& identityModelFile)
{
    try
    {
        auto ModelData = FMetaHumanFileResourceLoader::GetBoundedIOStreamFromFile(identityModelFile);
        ...
        bool hasVersion = io::ReadAndCheckOrRevertFromBoundedIOStream(ModelData.get(), IdentityBlendModelMagicNumber);
        if (hasVersion)
        {
            success &= io::FromBinaryFile(ModelData.get(), currVersion);
            if (currVersion != IdentityBlendModelVersion)
            {
                CARBON_CRITICAL("Version not supported {}", currVersion);
            }
        }
        if (!io::FromBinaryFile(ModelData.get(), readNumOfRegions))
        {
            CARBON_CRITICAL("Failed to read binary file {}", identityModelFile);
        }
        Eigen::Matrix<T, D, -1> mean;
        if (!io::FromBinaryFile(ModelData.get(), mean))
        {
            CARBON_CRITICAL("Failed to read binary file {}", identityModelFile);
        }
        ...
        for (int i = 0; i < readNumOfRegions; i++)
        {
            ...
            success &= io::FromBinaryFile(ModelData.get(), regionData.regionName);
            success &= io::FromBinaryFile(ModelData.get(), regionData.modes);
            success &= io::FromBinaryFile(ModelData.get(), regionData.vertexIDs);
            success &= io::FromBinaryFile(ModelData.get(), regionData.weights);
            if (currVersion >= 1)
            {
                io::FromBinaryFile(ModelData.get(), regionData.modeNames);
            }
