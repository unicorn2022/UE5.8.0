...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, EyeAssetGeneratorParams<T>& params)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 2)
    {
        success &= FromBinaryFile(stream, params.wrapDeformerParams);
        success &= io::FromBinaryFile(stream, params.eyeballNormalDisplacement);
        success &= io::FromBinaryFile(stream, params.faceLockDistanceThreshold);
        success &= io::FromBinaryFile(stream, params.faceLockDistanceThreshold);
        success &= io::FromBinaryFile(stream, params.eyeDistanceThreshold);
        success &= io::FromBinaryFile(stream, params.numSolverIterations);
        success &= io::FromBinaryFile(stream, params.numSolverCGIterations);
        success &= io::FromBinaryFile(stream, params.leftRightSplitIndex);
        success &= io::FromBinaryFile(stream, params.optimizePose);
        success &= io::FromBinaryFile(stream, params.wrapDeformOnlyVertexIndices);
        success &= io::FromBinaryFile(stream, params.deformationModelVertexWeight);
        success &= io::FromBinaryFile(stream, params.pointToPointConstraintWeight);
        success &= io::FromBinaryFile(stream, params.eyeConstraintWeight);
        success &= io::FromBinaryFile(stream, params.caruncleVertexIndices);
        success &= io::FromBinaryFile(stream, params.caruncleMultiplier);
    }
    else if (success && version == 1)
    {
        success &= FromBinaryFile(stream, params.wrapDeformerParams);
...
template <class T>
bool FromBinaryFile(trio::BoundedIOStream* stream, EyeAssetGenerator<T>& eyeAssetGenerator)
{
    if (!stream)
    {
        return false;
    ...
    if (success && version == 1)
    {
        ...
        success &= io::FromBinaryFile(stream, headMesh);
        eyeAssetGenerator.m_headMesh = headMesh;
        success &= io::FromBinaryFile(stream, eyeLeftMesh);
        eyeAssetGenerator.m_eyeLeftMesh = eyeLeftMesh;
        success &= io::FromBinaryFile(stream, eyeRightMesh);
        eyeAssetGenerator.m_eyeRightMesh = eyeRightMesh;
        success &= io::FromBinaryFile(stream, eyeAssetMesh);
        eyeAssetGenerator.m_eyeAssetMesh = eyeAssetMesh;
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_eyeLeftWeights);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_eyeRightWeights);
        success &= FromBinaryFile(stream, eyeAssetGenerator.m_wrapDeformer);
        // set the meshes in the wrapDeformer
        eyeAssetGenerator.m_wrapDeformer.SetMeshes(eyeAssetGenerator.m_headMesh, eyeAssetGenerator.m_eyeAssetMesh);
        success &= FromBinaryFile(stream, eyeAssetGenerator.m_params);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_rightAssetVertexIndices);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_rightAssetVertices);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_rightAssetTriangles);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_rightFaceCorrespondenceIndices);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_leftAssetVertexIndices);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_leftAssetVertices);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_leftAssetTriangles);
        success &= io::FromBinaryFile(stream, eyeAssetGenerator.m_leftFaceCorrespondenceIndices);
    }
    else
    {
        success = false;
...
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyeAssetGeneratorParams<float>& params);
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyeAssetGeneratorParams<double>& params);

template bool FromBinaryFile(trio::BoundedIOStream* stream, EyeAssetGenerator<float>& eyeAssetGenerator);
template bool FromBinaryFile(trio::BoundedIOStream* stream, EyeAssetGenerator<double>& eyeAssetGenerator);
