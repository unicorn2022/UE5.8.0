...
namespace Wrappers
{
	...
	bool FMetaHumanConformer::GenerateBrowMeshLandmarks(const FString& InCameraName, TArray<uint8>& OutJsonStream, float& OutFractionLandmarksInBrowRegion, bool bInConcatenate) const
	{
		std::string JsonString;
		OutFractionLandmarksInBrowRegion = 1.0f;
		bool bIsOK = Impl->CreationAPI.GenerateBrowMeshLandmarks(TCHAR_TO_UTF8(*InCameraName), JsonString, OutFractionLandmarksInBrowRegion, bInConcatenate);
	...
	bool FMetaHumanConformer::CheckPcaModelFromDnaRigConfig(const FString& InConfigurationJson, UDNAAsset* InDNAAsset)
	{
		...
		return CheckPcaModelFromDnaRigConfig(InConfigurationJson, InDNAAsset->GetDNAReader());
	}

	bool FMetaHumanConformer::CheckPcaModelFromDnaRigConfig(const FString& InConfigurationJson, TSharedPtr<IDNAReader> InDNAReader)
	{
		TArray<uint8> DNA = DNAToBuffer(InDNAReader);
	...
	TArray<uint8> FMetaHumanConformer::DNAToBuffer(UDNAAsset* InDNAAsset)
	{
		...
		return DNAToBuffer(InDNAAsset->GetDNAReader());
	}

	TArray<uint8> FMetaHumanConformer::DNAToBuffer(TSharedPtr<IDNAReader> InDNAReader)
	{
		TArray<uint8> Buffer;
		...
		Writer->setFrom(InDNAReader->Unwrap());
		Writer->write();
