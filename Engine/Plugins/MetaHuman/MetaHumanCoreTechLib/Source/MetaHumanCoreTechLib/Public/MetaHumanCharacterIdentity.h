...
namespace coretechlib { namespace titan {namespace api {class MetaHumanCreatorAPI; } }}
...
{
	Proportions UMETA(ToolTip = "Blends shape-based characteristics while maintaining the core skeletal proportions."),
	Features UMETA(ToolTip = "Only skeletal proportions are blended, while maintaining shaping."),
...
public:
	...
	UE_DEPRECATED(5.8, "This function is deprecated, use Init function that accepts DNAReader instead.")
	...
	UE_API bool Init(const FString& InMHCDataPath, const FString& InBodyMHCDataPath, TNotNull<dna::Reader*> InDNAReader, EMetaHumanCharacterOrientation InDNAAssetOrient);
...
private:
	...
	const coretechlib::titan::api::MetaHumanCreatorAPI* GetMHCAPIPtr() const;	
...
public:
	...
	UE_API void GetTrianglesIndices(TArray<int32>& OutTriangleIndices) const;

	/** Get the coefficients of the underlying model */
	UE_API void GetFaceModelCoefficients(TArray<float>& OutCoefficients) const;

	/** Set the coefficients of the underlying model */
	UE_API void SetFaceModelCoefficients(const TArray<float>& InCoefficients);
	...
	/* Fits the face state to the supplied body joints and vertices, calculates the M2MH delta from NoDelta to WithDelta vertices */
	UE_API bool FitWithVertexDeltasFromBody(const TArray<FMatrix44f>& InBodyJoints,
	...
	/** 
	 * Fit the Character Identity to the body state vertices, using the supplied options.
	 * Note that this leaves the Identity in a state where it needs autorigging. Returns true if successful, false if not
	 */
	UE_API bool FitFromBodyVertices(const TArray<FVector3f>& InBodyStateVertices, const FFitToTargetOptions& InFitToTargetOptions);

	/** Extract the head-mesh vertices from a combined body vertex array.
	 *  Returns the leading face-vertex-count entries of InBodyStateVertices, matching
	 *  the slice that FitFromBodyVertices feeds to the underlying MHC state.
	 *  Returns an empty array if InBodyStateVertices has too few vertices. */
	UE_API TArray<FVector3f> GetHeadPartVerticesFromBody(const TArray<FVector3f>& InBodyStateVertices) const;


	/** Sets the runtime scale applied to the M2MH vertex delta during Evaluate */
	UE_API void SetFromBodyVertexDeltaScale(float InFromBodyVertexDeltaScale);
	
	/** Clears the M2MH vertex delta applied during Evaluate */
	UE_API void ClearFromBodyVertexDeltas();
	
	/** Gets the runtime scale applied to the M2MH vertex delta during Evaluate */
	UE_API float GetFromBodyVertexDeltaScale() const;
	...
	UE_API TSharedRef<class IDNAReader> StateToDna(TNotNull<dna::Reader*> InDnaReader) const;
