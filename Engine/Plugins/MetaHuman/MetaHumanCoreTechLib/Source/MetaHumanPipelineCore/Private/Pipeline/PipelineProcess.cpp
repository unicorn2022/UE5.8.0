	...
	const UE::MetaHuman::Pipeline::FPipelineRunParameters& InPipelineRunParameters)
{
	...
	if (GPU.IsSet())
	{
		...
		{
			UE_LOGF(LogMetaHumanPipeline, Display, "Pipeline using unspecified GPU");
		}
		else
		{
			UE_LOGF(LogMetaHumanPipeline, Display, "Pipeline using GPU '%ls'", *GPU.GetValue());
		}
	}
...
void FPipelineProcess::StartNode(const TSharedPtr<FNode>& InNode)
{
	...
	if (!InNode->Start(PipelineData))
	{
		...
		if (!ExitPipelineData)
		{
			UE_LOGF(LogMetaHumanPipeline, Error, "   Start error in node \"%ls\" - Code %i, Message \"%ls\"", *InNode->Name, PipelineData->GetErrorNodeCode(), *PipelineData->GetErrorNodeMessage());
	...
	if (bIsInputReady && bIsOutputReady) // Process a frame
	{
		if (InNode == InternalSourceNode) // The internal source node create the new frame
		{
			if (bIsProducing)
			{
				...
				UE_LOGF(LogMetaHumanPipeline, VeryVerbose, "   %i:%ls", PipelineData->GetFrameNumber(), *InNode->Name);
		...
		else
		{
			...
			if (InNode == InternalSyncNode) // The internal sync node signifies the complete processing of a frame - send notification
			{
				...
				else
				{
					...
					UE_LOGF(LogMetaHumanPipeline, Verbose, "Processed Frame %d", PipelineData->GetFrameNumber());
	...
	if (PipelineData) // Data to pass down pipeline
	{
		if (!bPipelineDataOk) // Process/Idle failed
		{
			...
			if (ExitFrame == FPipelineProcess::NotExited)
			{
				...
				if (PipelineData->GetErrorNodeCode() == -1)
				{
					UE_LOGF(LogMetaHumanPipeline, VeryVerbose, "    Terminate issued by node \"%ls\" on frame %i", *InNode->Name, PipelineData->GetFrameNumber());
				}
				else
				{
					UE_LOGF(LogMetaHumanPipeline, Error, "    Process error in node \"%ls\" on frame %i - Code %i, Message \"%ls\"", *InNode->Name, PipelineData->GetFrameNumber(), PipelineData->GetErrorNodeCode(), *PipelineData->GetErrorNodeMessage());
...
void FPipelineProcess::EndNode(const TSharedPtr<FNode>& InNode)
{
	...
	if (!InNode->End(PipelineData))
	{
		...
		if (!ExitPipelineData)
		{
			UE_LOGF(LogMetaHumanPipeline, Error, "   End error in node \"%ls\" - Code %i, Message \"%ls\"", *InNode->Name, PipelineData->GetErrorNodeCode(), *PipelineData->GetErrorNodeMessage());
...
void FPipelineProcess::ConnectPins()
{
	...
	for (const FConnection& Connection : Connections)
	{
		...
		{
			if (InputPin.Address.IsEmpty() && InputPin.Direction == EPinDirection::Input && InputPin.Group == Connection.ToGroup)
			{
				// Get an ordered list of upstream nodes. In general there will be multiple lists of upstream nodes, 
				// each called a path, due to the node potentially having more than one input 
				...
				for (const TArray<TPair<TSharedPtr<FNode>, int32>>& Path : NodePaths)
				{
					...
					{
						const TPair<TSharedPtr<FNode>, int32>& NodeAndPinGroup = Path[Index];

						for (const FPin& OutputPin : NodeAndPinGroup.Key->Pins)
						{
							if (OutputPin.Direction != EPinDirection::Output)
							{
								continue;
							}

							if (!OutputPin.bIsPassthrough && Index != 0)
							{
								continue;
							}

							if (OutputPin.Type == InputPin.Type && OutputPin.Group == NodeAndPinGroup.Value)
							{
	...
	for (const TSharedPtr<FNode>& Node : Nodes)
	{
		...
		{
			if (Pin.Direction == EPinDirection::Input && Pin.Address.IsEmpty() && !Pin.bIsOptional)
			{
				ExitPipelineData = MakeShared<FPipelineData>();
...
void FPipelineProcess::PushSingleThreaded()
{
	...
	UE_LOGF(LogMetaHumanPipeline, Display, "");
	UE_LOGF(LogMetaHumanPipeline, Display, "Run start, single thread");

	// Run node "Start" functions
	{
		MHA_CPUPROFILER_EVENT_SCOPE_STR("Start");

		UE_LOGF(LogMetaHumanPipeline, VeryVerbose, " Start");

		for (const TSharedPtr<FNode>& Node : Nodes)
		{
			UE_LOGF(LogMetaHumanPipeline, VeryVerbose, "  %ls", *Node->Name);
	...
	if (bIsRunning)
	{
		...
		UE_LOGF(LogMetaHumanPipeline, VeryVerbose, " Process");

		int32 LoopCounter = 0;

		while (bIsRunning && !IsAborted())
		{
			UE_LOGF(LogMetaHumanPipeline, VeryVerbose, "  Loop %i - Producing %ls", LoopCounter++, bIsProducing ? TEXT("true") : TEXT("false"));
	...
	// Run node "End" functions
	{
		...
		UE_LOGF(LogMetaHumanPipeline, VeryVerbose, " End");
	...
	UE_LOGF(LogMetaHumanPipeline, Display, "Run end");
}
...
void FPipelineProcess::PushThreadPerNode()
{
	...
	UE_LOGF(LogMetaHumanPipeline, Display, "");
	UE_LOGF(LogMetaHumanPipeline, Display, "Run start, thread per node");
	...
	if (InternalSyncNodeProcess)
	{
		delete InternalSyncNodeProcess;
	}
