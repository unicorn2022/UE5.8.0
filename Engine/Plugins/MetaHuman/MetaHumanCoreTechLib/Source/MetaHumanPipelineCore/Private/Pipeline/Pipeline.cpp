...
namespace UE::MetaHuman::Pipeline
{
	TAutoConsoleVariable<bool> CVarBalancedGPUSelection
	{
		...
		false,
		TEXT("The depth processing pipeline will try to balance out the workload on multiple GPUs."),
		ECVF_Default
	};
...
void FPipeline::Run(const FPipelineRunParameters &InPipelineRunParameters)
{
	...
	if (Mode == EPipelineMode::PushSyncNodes || Mode == EPipelineMode::PushAsyncNodes)
	{
		...
		if (FTaskGraphInterface::Get().GetNumBackgroundThreads() < NumOfRequiredThreads)
		{
			...
			{
				UE_LOGF(LogMetaHumanPipeline, Warning, "Not enough background threads available: required %i, available %i. The MetaHuman pipeline is going to run on a single thread.", NumOfRequiredThreads, FTaskGraphInterface::Get().GetNumBackgroundThreads());
