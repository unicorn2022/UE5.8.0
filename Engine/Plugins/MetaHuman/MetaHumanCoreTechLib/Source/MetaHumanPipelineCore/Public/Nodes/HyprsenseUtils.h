...
namespace UE::MetaHuman::Pipeline
{
	...
	protected:
		...
		UE_API TArray<float, TAlignedHeapAllocator<64>> WarpAffineBilinear(const uint8* InSrcImage, int32 InSrcWidth, int32 InSrcHeight, const Matrix23f& InTransform, int32 InTargetWidth, int32 InTargetHeight, bool bInIsDetector) const;

		UE_API TArray<Bbox> HardNMS(const TArray<float, TAlignedHeapAllocator<64>>& InScores, const TArray<float, TAlignedHeapAllocator<64>>& InBoxes, float InIouThreshold, float InProbThreshold, int32 InTotalSize, int32 InTopK = -1) const;
