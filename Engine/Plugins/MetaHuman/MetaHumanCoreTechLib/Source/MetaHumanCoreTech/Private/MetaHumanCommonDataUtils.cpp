...
{
	static const FString CachedContentDir = IPluginManager::Get().FindPlugin(UE_PLUGIN_NAME)->GetContentDir();
...
TSoftObjectPtr<UObject> FMetaHumanCommonDataUtils::GetDefaultControlRigFromRegistry(const FStringView InAssetPath)
{
	FSoftObjectPath SoftPath(InAssetPath);
	const FName RigLongPackageName = FName(*SoftPath.GetLongPackageName());
	
	FAssetData RigData = GetFirstAssetData(RigLongPackageName);

	if (RigData.IsValid())
	{
		return TSoftObjectPtr<UObject>(RigData.ToSoftObjectPath());
	}
...
{
	return  GetDefaultControlRigFromRegistry(InAssetPath);
}
