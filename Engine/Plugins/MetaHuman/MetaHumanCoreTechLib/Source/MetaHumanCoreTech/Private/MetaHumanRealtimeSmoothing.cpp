...
bool FMetaHumanRealtimeSmoothing::ProcessFrame(const TArray<FName>& InPropertyNames, TArray<float>& InOutFrame, double InDeltaTime)
{
	...
	{
		UE_LOGF(LogMetaHumanRealtimeSmoothing, Warning, "Invalid max buffer size. All values are unmodified.");
		return false;
	}
	...
	for (const TPair<FName, FMetaHumanRealtimeSmoothingParam>& SmoothingParamPair : SmoothingParams)
	{
		...
		if (bIsOrientation)
		{
			...
			{
				UE_LOGF(LogMetaHumanRealtimeSmoothing, Warning, "Specified property name HeadRoll/HeadPitch/HeadYaw not found. Check pre processor configuration.");
				continue;
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogMetaHumanRealtimeSmoothing, Warning, "Specified property name %ls not found. Check pre processor configuration.", *PropertyName.ToString());
				continue;
			}
		}
		...
		if (SmoothingParam.Method == EMetaHumanRealtimeSmoothingParamMethod::RollingAverage)
		{
			...
			if (FrameCount <= 0)
			{
				UE_LOGF(LogMetaHumanRealtimeSmoothing, Error, "Encountered invalid frame count %d for property name %ls. Skipping.", FrameCount, *PropertyName.ToString());
				continue;
			}
...
UE_LOGF(LogTemp, Warning, "%ls", *Rolling);
UE_LOGF(LogTemp, Warning, "%ls", *Euro);
