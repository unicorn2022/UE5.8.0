...
void FDiscoveryCommunication::SendMessage(FDiscoveryPacket InMessage, const FString& InEndpoint)
{
    ...
    {
        UE_LOGF(LogCPSDiscoveryCommunication, Error, "Invalid message: %ls", *(SerializeResult.ClaimError().GetMessage()));
        return;
    }
...
void FDiscoveryCommunication::OnPacketArrived(const FArrayReaderPtr& InPayload, const FIPv4Endpoint& InEndpoint)
{
    ...
    {
        UE_LOGF(LogCPSDiscoveryCommunication, Error, "Invalid message: %ls", *(DeserializeResult.ClaimError().GetMessage()));
        return;
    }
