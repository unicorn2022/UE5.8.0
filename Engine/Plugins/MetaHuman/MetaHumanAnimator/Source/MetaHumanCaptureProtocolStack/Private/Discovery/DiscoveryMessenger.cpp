...
void FDiscoveryMessenger::OnPacketArrived(FDiscoveryPacket InPacket)
{
    ...
    {
        UE_LOGF(LogCPSDiscoveryMessenger, Error, "Client currently doesn't support requests.");
        return;
    }
    else if (InPacket.GetMessageType() == FDiscoveryPacket::EMessageType::Response)
    {
        ...
        {
            UE_LOGF(LogCPSDiscoveryMessenger, Error, "Failed to parse the response message.");
            return;
        }
    ...
    else if (InPacket.GetMessageType() == FDiscoveryPacket::EMessageType::Notify)
    {
        ...
        if (DeserializeResult.IsError())
        {
            UE_LOGF(LogCPSDiscoveryMessenger, Error, "Failed to parse the notify message.");
            return;
        }
    ...
    {
        UE_LOGF(LogCPSDiscoveryMessenger, Error, "Invalid message arrived.");
        return;
    }
}
