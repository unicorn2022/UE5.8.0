...
bool FMetaHumanMinSpec::IsSupported()
{
	if (!bIsInitialized && GDynamicRHI) // Dont initialize too early, ensure a RHI is set
	{
		...
		{
			UE_LOGF(LogMetaHumanPlatform, Display, "Min spec check disabled");
		}
	}
	}
...
{
	return FText::FromString(FString::Printf(TEXT("8 CPU threads, 32Gb memory, 8Gb VRAM, %s RHI and Vulkan"), *FMetaHumanSupportedRHI::GetSupportedRHINames().ToString()));
}

void FMetaHumanMinSpec::Reset()
{
