...
void FPipelineTestAdvancedNodes::GetTests(TArray<FString>& OutBeautifiedNames, TArray<FString>& OutTestCommands) const
{
	...
	Tests.Add("RealtimeMono-None-NNERuntimeORTDml-Default");
	Tests.Add("RealtimeMono-None-NNERuntimeORTDml-StereoHMC");
	Tests.Add("RealtimeMono-Input-NNERuntimeORTDml-Default");
	Tests.Add("RealtimeMono-FaceDetect-NNERuntimeORTDml-Default");
	Tests.Add("RealtimeMono-Headpose-NNERuntimeORTDml-Default");
	Tests.Add("RealtimeMono-Trackers-NNERuntimeORTDml-Default");
	Tests.Add("RealtimeMono-Trackers-NNERuntimeORTCpu-Default"); // Add IREE when models support it
	Tests.Add("RealtimeMono-Solver-NNERuntimeORTDml-Default");
	Tests.Add("RealtimeMonoSmoothing");
	Tests.Add("Audio");
	Tests.Add("RealtimeAudio-NNERuntimeORTDml");
	Tests.Add("RealtimeAudio-NNERuntimeORTCpu");
	Tests.Add("RealtimeAudio-NNERuntimeIREECpu");
		...
		if (Stage == "Stage1")
		{
			...
			if (bIsOK)
			{
				...
				else if (Test == "RealtimeMono")
				{
					...
					Realtime->SetModels(MonocularAnimationPipelineModels);
					Realtime->LoadModels();
					...
					if (Params[0] != "None")
					{
						...
						Save->FilePath = OutputDir + "/Realtime/" + Params[0] + "/" + Params[1] + "/%04d.png";
						Save->FrameNumberOffset = 0;
				...
				else if (Test == "RealtimeAudio")
				{
					...
					Realtime->SetNNEBackend(Params[0]);
					Realtime->LoadModels();
				...
				else if (Test == "FaceTrackerMonoPass2")
				{
					...
					PostProcessing->HierarchicalDefinitionsPlusChinCompressData = DeviceConfig->GetSolverHierarchicalDefinitionsPlusChinCompressData();
				...
				else if (Test == "FaceTrackerMonoPass3")
				{
					...
					PostProcessingFilter->HierarchicalDefinitionsPlusChinCompressData = DeviceConfig->GetSolverHierarchicalDefinitionsPlusChinCompressData();
		...
		else if (Stage == "Stage3")
		{
			...
			if (bIsOK)
			{
				...
				if (Test == "RealtimeMono")
				{
					...
					if (bIsOK)
					{
						UE_LOGF(LogMHABenchmark, Display, "%ls: Average = %.2fms, SD = %.2fms", *Test, AvgTime, StdDev);
					}
					else
					{
						UE_LOGF(LogMHABenchmark, Warning, "Failed");
					}
				}
				else
				{
