...
bool FFaceTrackerPostProcessingFilterNode::Start(const TSharedPtr<FPipelineData>& InPipelineData)
{
	...
	if (!Filter->Init(TemplateData, ConfigData))
	{
		...
		InPipelineData->SetErrorNodeMessage("Failed to load initialize the post processing filter");
		return false;
	}
	...
	switch (MeshSolverDefinitionsType)
	{
	case EMeshSolverDefinitionsType::Standard:
	{
		SolverConfigData = DefinitionsData;
		break;
	}

	case EMeshSolverDefinitionsType::Hierarchical:
	{
		SolverConfigData = HierarchicalDefinitionsData;
		break;
	}

	case EMeshSolverDefinitionsType::HierarchicalPlusChinCompress:
	{
		SolverConfigData = HierarchicalDefinitionsPlusChinCompressData;
		break;
	}

	default:
	{
		UE_LOGF(LogMetaHumanPipeline, Error, "Unhandled EMeshSolverDefinitionsType value");
		bValidConfig = false;
		break;
	}
	}
	...
	{
		InPipelineData->SetErrorNodeCode(ErrorCode::FailedToInitialize);
		InPipelineData->SetErrorNodeMessage("Unhandled solver config type");
		return false;
	}

	if (DNAReader.IsValid())
	{
		bLoadedDNA = Filter->LoadDNA(DNAReader, SolverConfigData);
	}
	else if (!DNAFile.IsEmpty())
	{
		bLoadedDNA = Filter->LoadDNA(DNAFile, SolverConfigData);
	}
	else
	{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		if (!DNAAsset.IsExplicitlyNull())
		{
			if (DNAAsset.IsValid())
			{
				bLoadedDNA = Filter->LoadDNA(DNAAsset.Get(), SolverConfigData);
			}
		}
