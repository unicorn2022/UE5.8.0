...
// Macros for handling back-compatibility of GPU models

#define EFFECTIVE_TRACKER(TRACKER) \
    PRAGMA_DISABLE_DEPRECATION_WARNINGS \
	TSharedPtr<IModelInstanceRunSync> Effective##TRACKER = TRACKER.IsValid() ? TRACKER : TRACKER##Model; \
	PRAGMA_ENABLE_DEPRECATION_WARNINGS

#define EFFECTIVE_NNE_MODEL(INDEX) \
    PRAGMA_DISABLE_DEPRECATION_WARNINGS \
	TSharedPtr<IModelInstanceRunSync> EffectiveNNEModel = (NNEModels[INDEX] && NNEModels[INDEX].IsValid()) ? NNEModels[INDEX] : NNERunSyncModels[INDEX]; \
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
namespace UE::MetaHuman::Pipeline
{
	...
	bool FHyprsenseNodeBase::CheckTrackers(const TMap<ETrackerType, UE::NNE::FTensorShape>& InputValidationMap, const TMap<ETrackerType, TArray<UE::NNE::FTensorShape>>& OutputValidationMap, const TMap<TSharedPtr<UE::NNE::IModelInstanceRunSync>, ETrackerType>& TrackerTypeMap)
	{
		...
		{
			TSharedPtr<IModelInstanceRunSync>& Model = Tuple.Key;
			...
			if (InputTensorDesc.Num() == 1)
			{
				...
				if (ShapeData.Num() == 4)
				{
					...
					if (bFlag)
					{
						...
						UE_LOGF(LogHyprsenseNodeBase, Warning, "%ls", *FString::Printf(TEXT("Using custom tracker model of input resolution %d x %d for part: %s"), ShapeData[2], ShapeData[3], *TrackerName));
					}
		...
		for (auto Tuple : TrackerTypeMap)
		{
			...
			if (Model->SetInputTensorShapes({ ModelInfo }) != IModelInstanceRunSync::ESetInputTensorShapesStatus::Ok)
			{
				EErrorCode = ErrorCode::InvalidTracker;
				ErrorMessage = TrackerName + " is invalid";
				return false;
	...
	bool FHyprsenseNodeBase::ProcessLandmarks(const FUEImageDataType& Input, bool isRealtime, TArray<FHyprsenseNodeBase::PartPoints>& OutDenseTrackerPointsPerModelInversed, FHyprsenseNodeBase::PartPoints& OutSparseTrackerPointsInversed, bool bInRunSparseTrackerOnly)
	{
		...
		if (!bIsFaceDetected)
		{
			...
			if (EffectiveFaceDetector.IsValid())
			{
				...
				if (EffectiveFaceDetector->RunSync(Inputs, Outputs) != IModelInstanceRunSync::ERunSyncStatus::Ok)
				{
		...
		if (bIsFaceTracked)
		{
			...
			TArray<float, TAlignedHeapAllocator<64>> ResizedNNInput = WarpAffineBilinear(OrgImg, Input.Width, Input.Height, LastTransform, TrackerInputSizeX, TrackerInputSizeY, false);

			const int32 Landmarks = 131;
			TArray<float> OutputArrayCropped{}, Score{};

			EFFECTIVE_TRACKER(FaceTracker);
			if (EffectiveFaceTracker.IsValid())
			{
				...
				if (EffectiveFaceTracker->RunSync(Inputs, Outputs) != IModelInstanceRunSync::ERunSyncStatus::Ok)
				{
					EErrorCode = ErrorCode::FailedToTrack;
			...
			else
			{
				...
				if (!bInRunSparseTrackerOnly)
				{
					...
					for (int32 I = 0; I < FacePart::Num; ++I)
					{
						if (ProcessPart[I])
						{
							EFFECTIVE_NNE_MODEL(I);
							if (EffectiveNNEModel != nullptr)
							{
								if (!EffectiveNNEModel.IsValid())
								{
									EErrorCode = ErrorCode::InvalidTracker;
					...
					auto ProcessParts = [&](const int32 InModelIndex, const TArray<int32>& InModels) -> bool
					{
						...
						EFFECTIVE_NNE_MODEL(InModelIndex);
						TConstArrayView<FTensorDesc> OutputDescs = EffectiveNNEModel->GetOutputTensorDescs();
						...
						if (EffectiveNNEModel->RunSync(Inputs, Outputs) != IModelInstanceRunSync::ERunSyncStatus::Ok)
						{
							return false;
						}
					...
					ParallelFor(FacePart::Num, [&](int32 InModelIdx)
						{
							if (ProcessPart[InModelIdx])
							{
								...
								EFFECTIVE_NNE_MODEL(InModelIdx);
								if (EffectiveNNEModel != nullptr) // discard threads so that modelIdx is only models not face parts
								{
	...
	FString FHyprsenseNodeBase::GetNNEBackend(void) const
	{
		if (NNEBackend.IsEmpty())
		{
			const IConsoleVariable* ConsoleVariable = IConsoleManager::Get().FindConsoleVariable(TEXT("mh.FaceTracker.Backend"));
			if (ConsoleVariable)
			{
				return ConsoleVariable->GetString();
			}

			UE_LOGF(LogHyprsenseNodeBase, Warning, "Failed to find mh.FaceTracker.Backend console variable");
			return "";
		}
		else
		{
