...
bool FFaceTrackerPostProcessingNode::Start(const TSharedPtr<FPipelineData>& InPipelineData)
{
	...
	{
		bLoadedDNA = Tracker->LoadDNA(DNAReader, SolverConfigData);
	}
	else if (!DNAFile.IsEmpty())
	{
		bLoadedDNA = Tracker->LoadDNA(DNAFile, SolverConfigData);
	}
	else
	{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		if (!DNAAsset.IsExplicitlyNull())
		{
			if (DNAAsset.IsValid())
			{
				bLoadedDNA = Tracker->LoadDNA(DNAAsset.Get(), SolverConfigData);
			}
		}
