...
bool FFaceTrackerStereoNode::Start(const TSharedPtr<FPipelineData>& InPipelineData)
{
	...
	{
		bLoadedDNA = Tracker->LoadDNA(DNAReader);
	}
	else if(!DNAFile.IsEmpty())
	{
...
FFaceTrackerIPhoneNode::FFaceTrackerIPhoneNode(const FString& InName) : FNode("FaceTrackerIPhone", InName)
{
	...
	Pins.Add(FPin("Depth Calib In", EPinDirection::Input, EPinType::Calibration, 0, true /* bPassthrough */, true /* bOptional */));
	Pins.Add(FPin("Animation Out", EPinDirection::Output, EPinType::Animation));
...
bool FFaceTrackerIPhoneNode::Process(const TSharedPtr<FPipelineData>& InPipelineData)
{
	if (bIsFirstPass)
	{
		if (!Pins[4].Address.IsEmpty() && InPipelineData->HasData<FCalibrationDataType>(Pins[4].Address))
		{
			if (!Calibrations.ContainsByPredicate([](const FCameraCalibration& InCalib)
												  {
													  return InCalib.CameraType == FCameraCalibration::Depth;
												  }))
			{
				FCalibrationDataType CalibrationDataType = InPipelineData->GetData<FCalibrationDataType>(Pins[4].Address);

				if (CalibrationDataType.CameraType == FCalibrationDataType::Depth)
				{
					Calibrations.Add(FDepthGenerateNode::ConvertCalibrationDataType(CalibrationDataType));
				}
				else
				{
					InPipelineData->SetErrorNodeCode(ErrorCode::FailedToInitialize);
					InPipelineData->SetErrorNodeMessage("Provided calibration must be a depth calibration");
					return false;
				}
			}
		}
	...
	if (Tracker->SetInputData(ImageDataMap, LandmarkMap, DepthDataMap))
	{
		...
		if (bIsFirstPass && !bSkipDiagnostics)
		{
			...
			if (!bEstimatedScale)
			{
				...
				UE_LOGF(LogMetaHumanPipeline, Warning, "Failed to calculate head scale diagnostics");
			}
			InPipelineData->SetData<float>(Pins[6], Scale);
		}
		...
		if (Tracker->Track(FrameNumber, FlowInfo, false, DebuggingFolder, bSkipPredictiveSolver, bSkipPerVertexSolve))
		{
			...
			Animation.MeshData = { FaceMeshVertData, TeethMeshVertData, LeftEyeMeshVertData, RightEyeMeshVertData };
			Animation.AnimationQuality = EFrameAnimationQuality::Preview;
			InPipelineData->SetData<FFrameAnimationData>(Pins[5], MoveTemp(Animation));
		}
		else
		{
...
FDepthGenerateNode::FDepthGenerateNode(const FString& InName) : FNode("DepthGenerate", InName)
{
	...
	Pins.Add(FPin("Depth Calib Out", EPinDirection::Output, EPinType::Calibration));
}
...
bool FDepthGenerateNode::Start(const TSharedPtr<FPipelineData>& InPipelineData)
{
	...
	FCameraCalibration DepthCalibration;
	DepthCalibration.CameraId = TEXT("Depth");
	DepthCalibration.CameraType = FCameraCalibration::Depth;

	Calibrations.Add(MoveTemp(DepthCalibration));
...
bool FDepthGenerateNode::Process(const TSharedPtr<FPipelineData>& InPipelineData)
{
	...
	if (Reconstructer->SetInputData(ImageDataMap))
	{
		...
		{
			FCameraCalibration& DepthCalibration = Calibrations[2];
			DepthCalibration.ImageSize.X = Width;
			DepthCalibration.ImageSize.Y = Height;
			DepthCalibration.FocalLength = FVector2D(Intrinsics[0], Intrinsics[4]);
			DepthCalibration.PrincipalPoint = FVector2D(Intrinsics[6], Intrinsics[7]);

			for (int32 I = 0; I < 4; ++I)
			{
				for (int32 J = 0; J < 4; ++J)
				{
					DepthCalibration.Transform.M[I][J] = Extrinsics[I * 4 + J];
				}
			}

			// depth map (rectified) camera has no distortion
			DepthCalibration.P1 = 0.0;
			DepthCalibration.P2 = 0.0;
			DepthCalibration.K1 = 0.0;
			DepthCalibration.K2 = 0.0;
			DepthCalibration.K3 = 0.0;
			...
			InPipelineData->SetData<FCalibrationDataType>(Pins[3], ConvertCameraCalibration(DepthCalibration));
		}
		else
		{
...
concept CIsOneOf = (CSameAs<T, U> || ...);

template<typename From, typename To>
	requires CIsOneOf<From, FCalibrationDataType, FCameraCalibration> && CIsOneOf<To, FCalibrationDataType, FCameraCalibration>
To::EType ConvertCameraType(typename From::EType InType)
{
	switch (InType)
	{
	case From::EType::Video:
		return To::EType::Video;
	case From::EType::Depth:
		return To::EType::Depth;
	case From::EType::Unknown:
	default:
			return To::EType::Unknown;
	}
}

template<typename From, typename To>
	requires CIsOneOf<From, FCalibrationDataType, FCameraCalibration> && CIsOneOf<To, FCalibrationDataType, FCameraCalibration>
To ConvertCalibrationType(From InType)
{
	To ToType;
	ToType.CameraId = InType.CameraId;
	ToType.CameraType = ConvertCameraType<From, To>(InType.CameraType);
	ToType.ImageSize = InType.ImageSize;
	ToType.FocalLength = InType.FocalLength;
	ToType.PrincipalPoint = InType.PrincipalPoint;
	...
	ToType.K5 = InType.K5;
	ToType.K6 = InType.K6;
	ToType.Transform = InType.Transform;

	return ToType;
}

FCalibrationDataType FDepthGenerateNode::ConvertCameraCalibration(const FCameraCalibration& InCameraCalibration)
{
	return ConvertCalibrationType<FCameraCalibration, FCalibrationDataType>(InCameraCalibration);
}

FCameraCalibration FDepthGenerateNode::ConvertCalibrationDataType(const FCalibrationDataType& InCalibrationDataType)
{
	return ConvertCalibrationType<FCalibrationDataType, FCameraCalibration>(InCalibrationDataType);
}
...
bool FFlowNode::Start(const TSharedPtr<FPipelineData>& InPipelineData)
{
	...
	if (UE::MetaHuman::Pipeline::CVarBalancedGPUSelection.GetValueOnAnyThread())
	{
		...
		if (bFoundGPU)
		{
			UE_LOGF(LogMetaHumanPipeline, Display, "Flow node is using GPU '%ls'", *UseGPU);
		}
		else
		{
			UE_LOGF(LogMetaHumanPipeline, Warning, "Failed to find GPU not used by UE, falling back to default behavior (GPU='%ls')", *UseGPU);
		}
	}
