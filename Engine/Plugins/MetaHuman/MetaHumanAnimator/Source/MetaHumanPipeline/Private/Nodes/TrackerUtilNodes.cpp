				...
				if (JsonParsed->TryGetArrayField(TEXT("frames"), Frames))
				{
					...
					for (TSharedPtr<FJsonValue> Frame : *Frames)
					{
						...
						if (bIsOK)
						{
							...
							{
								Contour.TrackingContours.Add(FString(Curve.Key));
								...
								if (Curve.Value->TryGetArray(Indices))
								{
									for (const TSharedPtr<FJsonValue>& Index : *Indices)
									{
										...
										Contour.TrackingContours[FString(Curve.Key)].DensePoints.Add(PointList[IndexInt]);
									}
								}
							}
							...
							{
								Contour.TrackingContours.Add(FString(Landmark.Key));
								...
								if (Landmark.Value->TryGetArray(Indices))
								{
									...
									{
										int32 IndexInt = Index->AsNumber();
										Contour.TrackingContours[FString(Landmark.Key)].DensePoints.Add(PointList[IndexInt]);
									}
								}
							}
	...
	if (FFileHelper::LoadFileToString(FileData, *JsonFile))
	{
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			if (JsonParsed->TryGetObjectField(TEXT("Frames"), Frames))
			{
				...
				for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& JFrame : Frames->Get()->Values)
				{
					FFrameTrackingContourData FrameContours;
					auto FrameJObject = JFrame.Value.Get()->AsObject();

					// Values are the content of each frame
					for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& JFrameContent : FrameJObject->Values)
					{
						const FJsonObject::FStringType& CurveName = JFrameContent.Key;
