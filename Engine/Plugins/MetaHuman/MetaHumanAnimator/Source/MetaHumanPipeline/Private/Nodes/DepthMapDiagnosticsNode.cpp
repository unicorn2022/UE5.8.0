...
{
	if (!Pins[3].Address.IsEmpty() && InPipelineData->HasData<FCalibrationDataType>(Pins[3].Address))
	{
											  ...
											  }))
		{
			...
			FCalibrationDataType CalibrationDataType = InPipelineData->GetData<FCalibrationDataType>(Pins[3].Address);
	...
	if (!Diagnostics->CalcDiagnostics(ImageDataMap, LandmarkMap, DepthDataMap, OutputDiagnostics))
	{
		...
		UE_LOGF(LogMetaHumanPipeline, Warning, "Failed to calculate depthmap diagnostics for frame %d", FrameNumber);
		OutputDiagnostics.Add(DepthCalibration.CameraId, {});
	}

	InPipelineData->SetData<TMap<FString, FDepthMapDiagnosticsResult>>(Pins[4], MoveTemp(OutputDiagnostics));
