...
static void UpgradeEditorBulkData(const UE::Serialization::FEditorBulkData& InEditorData, FByteBulkData& OutBulkData)
{
	if (InEditorData.HasPayloadData())
	{
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Payload size too large");
			return;
		}
...
bool FMetaHumanConfig::GetInfo(UCaptureData* InCaptureData, const FString& InComponent, FString& OutDisplayName, UMetaHumanConfig*& OutConfig)
{
	...
	if (InCaptureData)
	{
		...
		else if (InCaptureData->IsA<UFootageCaptureData>())
		{
			...
			default:
				UE_LOGF(LogMetaHumanConfig, Warning, "Unspecified device class, assuming iPhone 13");
				ConfigAsset = TEXT("iphone13");
				break;
			};
...
bool UMetaHumanConfig::VerifySolverConfig(const FString& InSolverTemplateDataJson, const FString& InSolverConfigDataJson, const FString& InSolverDefinitionsDataJson,
	const FString& InSolverHierarchicalDefinitionsDataJson, const FString& InSolverHierarchicalDefinitionsPlusChinCompressDataJson, const FString& InSolverPCAFromDNADataJson, FString& OutErrorString) const
{
	...
	TObjectPtr<UDNA> ArchetypeDnaAsset = ReadDNAAssetFromFile(PathToDNA, GetTransientPackage());
	...
	if (!UE::Wrappers::FMetaHumanConformer::CheckPcaModelFromDnaRigConfig(InSolverPCAFromDNADataJson, ArchetypeDnaAsset->GetDNAReader()))
	{
	...
	if (!FaceTrackerPostProcessing->LoadDNA(ArchetypeDnaAsset->GetDNAReader(), InSolverDefinitionsDataJson))
	{
	...
	// check the solver definitions; note this functionality is only available with editor
	if (!FaceTrackerPostProcessing->LoadDNA(ArchetypeDnaAsset->GetDNAReader(), InSolverHierarchicalDefinitionsDataJson))
	{
	...
	// check the solver definitions; note this functionality is only available with editor
	// only check this config if it is not empty
	if (InSolverHierarchicalDefinitionsPlusChinCompressDataJson.Len() > 0 && !FaceTrackerPostProcessing->LoadDNA(ArchetypeDnaAsset->GetDNAReader(), InSolverHierarchicalDefinitionsPlusChinCompressDataJson))
	{
		OutErrorString = TEXT("face tracking hierarchical solver plus chin compress definitions contains invalid data.");
		return false;
	}
...
bool UMetaHumanConfig::ReadFromDirectory(const FString& InPath)
{
	...
	FString SolverHierarchicalDefinitionsPlusChinCompressDataFile = InPath + TEXT("/hierarchical_solver_plus_chin_compress_definitions.json");
	...
	FString SolverTemplateDataJson, SolverConfigDataJson, SolverDefinitionsDataJson, SolverHierarchicalDefinitionsDataJson, SolverHierarchicalDefinitionsPlusChinCompressDataJson, SolverPCAFromDNADataJson;
	...
	if (ConfigType == EMetaHumanConfigType::Solver)
	{
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *SolverTemplateDataFile);
			return false;
		}

		if (!FaceTracker->CreateFlattenedJsonStringWrapper(SolverConfigDataFile, SolverConfigDataJson))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *SolverConfigDataFile);
			return false;
		}

		if (!FaceTracker->CreateFlattenedJsonStringWrapper(SolverDefinitionsDataFile, SolverDefinitionsDataJson))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *SolverDefinitionsDataFile);
			return false;
		}
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *SolverHierarchicalDefinitionsDataFile);
			return false;
		}

		if (FPaths::FileExists(SolverHierarchicalDefinitionsPlusChinCompressDataFile))
		{
			if (!FaceTracker->CreateFlattenedJsonStringWrapper(SolverHierarchicalDefinitionsPlusChinCompressDataFile, SolverHierarchicalDefinitionsPlusChinCompressDataJson))
			{
				UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *SolverHierarchicalDefinitionsPlusChinCompressDataFile);
				return false;
			}
		}
		else
		{
			SolverHierarchicalDefinitionsPlusChinCompressDataJson = {};
			UE_LOGF(LogMetaHumanConfig, Warning, "Hierarchical solver definitions plus chinCompress file %ls does not exist; this is a legacy config file", *SolverHierarchicalDefinitionsPlusChinCompressDataFile);
			bV2Config = true;
		}

		if (!FaceTracker->CreateFlattenedJsonStringWrapper(SolverPCAFromDNADataFile, SolverPCAFromDNADataJson))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *SolverPCAFromDNADataFile);
			return false;
		}
		...
		FString ErrorString;
		if (!VerifySolverConfig(SolverTemplateDataJson, SolverConfigDataJson, SolverDefinitionsDataJson, SolverHierarchicalDefinitionsDataJson, SolverHierarchicalDefinitionsPlusChinCompressDataJson, SolverPCAFromDNADataJson, ErrorString))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Solving Config validation error: %ls", *ErrorString);
			return false;
		}
	}
	else if (ConfigType == EMetaHumanConfigType::Fitting)
	{
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *FittingTemplateDataFile);
			return false;
		}

		if (!FaceTracker->CreateFlattenedJsonStringWrapper(FittingConfigDataFile, FittingConfigDataJson))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *FittingConfigDataFile);
			return false;
		}

		if (!FaceTracker->CreateFlattenedJsonStringWrapper(FittingConfigTeethDataFile, FittingConfigTeethDataJson))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *FittingConfigTeethDataFile);
			return false;
		}

		if (!FaceTracker->CreateFlattenedJsonStringWrapper(FittingIdentityModelDataFile, FittingIdentityModelDataJson))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *FittingIdentityModelDataFile);
			return false;
		}

		if (!FaceTracker->CreateFlattenedJsonStringWrapper(FittingControlsDataFile, FittingControlsDataJson))
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load file %ls", *FittingControlsDataFile);
			return false;
		}
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Fitting Config validation error: %ls", *ErrorString);
			return false;
		}
	}
	else if (ConfigType == EMetaHumanConfigType::PredictiveSolver) //-V547
	{
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to load predictive solver files");
			return false;
		}
	...
	if (ParentDirectoryName != TEXT("iphone12") &&
		...
		ParentDirectoryName != TEXT("stereo_hmc") && // note that this is intentionally stereo_hmc and not hmc, as the configs are in a folder stereo_hmc and NOT hmc
		ParentDirectoryName != TEXT("predictivesolvers") &&
		ParentDirectoryName != TEXT("Mesh2MetaHuman"))
	{
		UE_LOGF(LogMetaHumanConfig, Warning, "Unknown directory name %ls", *ParentDirectoryName);
	}
	...
	if (FPaths::FileExists(VersionFilename))
	{
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to read versions file %ls", *VersionFilename);
			return false;
		}
	}
	else
	{
		UE_LOGF(LogMetaHumanConfig, Warning, "Missing version file %ls", *VersionFilename);
	}
	...
	// 3 = compressed, editor bulk data + new chin compress solver definitions
	// 1 | BulkDataMask = 65 = no compression, bulk data
	// 2 | BulkDataMask = 66 = compressed, bulk data
	// 3 | BulkDataMask = 67 = compressed, bulk data + new chin compress solver definitions

	if (bV2Config)
	{
		InternalVersion = (2 | BulkDataMask); // old config with no chin compress solver definitions
	}
	else
	{
		InternalVersion = (3 | BulkDataMask);
	}
	...
	ResetBulkData(SolverHierarchicalDefinitionsPlusChinCompressCipherText);
	...
	if (ConfigType == EMetaHumanConfigType::Solver || ConfigType == EMetaHumanConfigType::Fitting)
	{
		...
		if (BaseConfig)
		{
			if (ConfigType == EMetaHumanConfigType::Solver)
			{
				...
				if (SolverHierarchicalDefinitionsPlusChinCompressDataFile == BaseConfig->GetSolverHierarchicalDefinitionsPlusChinCompressData())
				{
					SolverHierarchicalDefinitionsPlusChinCompressDataFile = USE_BASE_CONFIG_DATA;
				}
	...
	// back-compatibility with old configs
	if (bV2Config)
	{
		SolverHierarchicalDefinitionsPlusChinCompressCipherText = {};
	}
	else
	{
		Encrypt(SolverHierarchicalDefinitionsPlusChinCompressDataJson, SolverHierarchicalDefinitionsPlusChinCompressCipherText);
	}
...
FString UMetaHumanConfig::GetSolverHierarchicalDefinitionsPlusChinCompressData() const
{
	FString Data = DecryptToString(SolverHierarchicalDefinitionsPlusChinCompressCipherText);

	if (Data == USE_BASE_CONFIG_DATA)
	{
		Data = GetBaseConfig()->GetSolverHierarchicalDefinitionsPlusChinCompressData();
	}
...
TArray<uint8> UMetaHumanConfig::GetPredictiveGlobalTeethTrainingData() const
{
	...
	{
		UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to get PredictiveGlobalTeethTraining payload");
	}
...
TArray<uint8> UMetaHumanConfig::GetPredictiveTrainingData() const
{
	...
	{
		UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to get PredictiveTrainingData payload");
	}
...
void UMetaHumanConfig::Serialize(FArchive& Ar)
{
	...
	if ((InternalVersion & BulkDataMask) == 0) // Back compatibility case where data is stored in FEditorBulkData - need to move it to FByteBulkData
	{
        ...
        if (Ar.IsLoading())
        {
            // Older assets do not contain this; default to empty.
            SolverHierarchicalDefinitionsPlusChinCompressCipherText = {};
        }

		// GUARD: don't mutate InternalVersion during harvesting  
		if (Ar.IsSaving() && !Ar.IsObjectReferenceCollector())
		{
			InternalVersion = (InternalVersion | BulkDataMask);
		}
	}
	else if ((InternalVersion & ~BulkDataMask) == 1 || (InternalVersion & ~BulkDataMask) == 2)
	{
	...
	else if ((InternalVersion & ~BulkDataMask) == 3) // version 3, add in the chin compress data
	{
		SolverTemplateDataCipherText.Serialize(Ar, this);
		SolverConfigDataCipherText.Serialize(Ar, this);
		SolverDefinitionsCipherText.Serialize(Ar, this);
		SolverHierarchicalDefinitionsCipherText.Serialize(Ar, this);
		SolverHierarchicalDefinitionsPlusChinCompressCipherText.Serialize(Ar, this);
		SolverPCAFromDNACipherText.Serialize(Ar, this);
		FittingTemplateDataCipherText.Serialize(Ar, this);
		FittingConfigDataCipherText.Serialize(Ar, this);
		FittingConfigTeethDataCipherText.Serialize(Ar, this);
	...
	{
		UE_LOGF(LogMetaHumanConfig, Fatal, "Unknown config version");
	}
}
...
bool UMetaHumanConfig::Encrypt(const FString& InPlainText, FByteBulkData& OutCipherText) const
{
	...
	if (Encryptor->Update(CompressedPlainText, PartialCipherText, PartialCipherTextSize) == EPlatformCryptoResult::Success)
	{
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to finalize config encrypt");
		}
	}
	else
	{
		UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to update config encrypt");
	}
...
bool UMetaHumanConfig::Decrypt(const FByteBulkData& InCipherText, FString& OutPlainText) const
{
	...
	{
		UE_LOGF(LogMetaHumanConfig, Warning, "Failed to get payload");
		OutPlainText = FString();
		return true;
	}
	...
	if (Decryptor->Update(CipherText, PartialPlainText, PartialPlainTextSize) == EPlatformCryptoResult::Success)
	{
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "PartialPlainTextSize too large");
			return false;
		}
		...
		if (Decryptor->Finalize(PartialPlainText, PartialPlainTextSize) == EPlatformCryptoResult::Success)
		{
			...
			if ((InternalVersion & ~BulkDataMask) >= 2)
			{
				...
				{
					UE_LOGF(LogMetaHumanConfig, Fatal, "PlainText too small");
					return false;
				}
				...
				{
					UE_LOGF(LogMetaHumanConfig, Fatal, "UncompressedPlainTextSize bad size");
					return false;
				}
				...
				{
					UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to decompress config");
					return false;
				}
		...
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to finalize config decrypt");
		}
	}
	else
	{
		UE_LOGF(LogMetaHumanConfig, Fatal, "Failed to update config decrypt");
	}
...
UMetaHumanConfig* UMetaHumanConfig::GetBaseConfig() const
{
	...
	else
	{
		...
		if (!BaseConfig)
		{
			UE_LOGF(LogMetaHumanConfig, Fatal, "Failed load base config");
		}
