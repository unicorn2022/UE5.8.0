...
)
{
	...
	for (int32 DirectoryConfigIndex = 0; DirectoryConfigIndex < DirectoryConfigs.Num(); ++DirectoryConfigIndex)
	{
		...
		for (const int32 IndexToRemove : WildcardIndicesToRemove)
		{
			...
			UE_LOGF(LogCaptureSourceAutoReimport, Display, "Removed redundant auto reimport wildcard \"%ls\" from source directory %ls", *RemovedWildcard, *DirectoryConfig.SourceDirectory);
		}
	...
	for (const int32 IndexToRemove : DirectoryConfigIndicesToRemove)
	{
		...
		UE_LOGF(LogCaptureSourceAutoReimport, Display, "Removed empty auto reimport directory config: %ls", *RemovedSourceDirectory);
	}
