...
bool FMetaHumanMediaAudioSourceReader::Open(const FString& URL)
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Cannot open the audio file %ls: %d", *URL, Result);
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to retrieve the video file duration: %d", Result);
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to retrieve the video file duration value: %d", Result);
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Stream Selection Failed: %d", Result);
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Media Type Configuration Failed: %d", Result);
		return false;
	}
...
IMediaAudioSample* FMetaHumanMediaAudioSourceReader::Next()
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to read the next sample: %d", Result);
		return nullptr;
	}

	if (Flags & (MF_SOURCE_READERF_ERROR | MF_SOURCE_READERF_CURRENTMEDIATYPECHANGED))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to read the next sample: Flags(%d)", Flags);
		return nullptr;
	}
	...
	if (!MFSample)
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to read the next sample");
		return nullptr;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to read the audio sample time: %d", Result);
		return nullptr;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to read the audio sample duration: %d", Result);
		return nullptr;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to retrieve the audio sample information: %d", Result);
		return nullptr;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to retrieve the audio data: %d", Result);
		return nullptr;
	}
...
bool FMetaHumanMediaVideoSourceReader::Open(const FString& URL)
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Reader configuration failed: %d", Result);
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Cannot open the video file %ls: %d", *URL, Result);
		return false;
	}
	...
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Only MJPG video format is currently supported");
		return false;
	}
...
IMediaTextureSample* FMetaHumanMediaVideoSourceReader::Next()
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to read the video sample time: %d", Result);
		return nullptr;
	}
	Sample.Time = FMediaTimeStamp(FTimespan(TimeStamp));
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to read the video sample duration: %d", Result);
		return nullptr;
	}
	Sample.Duration = FTimespan(Duration);
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to retrieve the video sample buffer: %d", Result);
		return nullptr;
	}
	...
	if (FAILED(Result))
	{
		...
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to retrieve the video data: %d", Result);
		return nullptr;
	}
