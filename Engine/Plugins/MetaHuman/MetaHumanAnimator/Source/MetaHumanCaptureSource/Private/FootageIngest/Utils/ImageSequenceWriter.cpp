...
bool FImageSequenceWriter::Open(const FString& InDirPath)
{
	...
	{
		UE_LOGF(LogMetaHumanImageSequenceWriter, Error, "Failed to create the image sequence directory: %ls", *InDirPath);
		return false;
	}
...
bool FImageSequenceWriter::Append(IMediaTextureSample* InTexture)
{
	...
	if (!Handle)
	{
		UE_LOGF(LogMetaHumanImageSequenceWriter, Error, "Cannot open the file: %ls", *JPEGFileName);
		return false;
	}
	...
	if (InTexture->GetOrientation() == EMediaOrientation::Original)
	{
		...
		{
			UE_LOGF(LogMetaHumanImageSequenceWriter, Error, "Cannot write the content of the file: %ls", *JPEGFileName);
			return false;
		}
	}
	else {
							 ...
							 InTexture->GetStride(), 1, &OutBuffer, &OutBufferSize, &Transform, TJFLAG_NOREALLOC))
		{
			...
			UE_LOGF(LogMetaHumanImageSequenceWriter, Error, "Failed to apply transform to image: %d. Error: %ls", SequenceIndex, *ErrorString);
			return false;
		}
