                                                                     ...
                                                                     FCameraCalibration& OutDepthCameraCalibration)
{
	...
	for (const TPair<FString, FCubicCameraInfo>& Elem : InTakeCameraCalibrations)
	{
		...
		if (ImageRes > MaxStandardHMCImageSize)
		{
			...
			UE_LOGF(LogMetaHumanCaptureSource, Warning, "Image resolution of %i is larger than the expected maximum size for the MetaHuman plugin (1536 x 2048). Image sequence ingest will be slow and may run out of graphics memory.",
				ImageRes);
			break;
		}
		if (CVarForceSingleThreadedStereoReconstruction.GetValueOnAnyThread())
		{
			...
			UE_LOGF(LogMetaHumanCaptureSource, Warning, "CVar mh.CaptureSource.ForceSingleThreadedStereoReconstruction is set; stereo HMC reconstruction will be run in a single thread of execution.");
			break;
		}
	}
	...
	{
		UE_LOGF(LogMetaHumanCaptureSource, Warning, "Not enough background threads available: required %i, available %i. The HMC ingest pipeline is going to run on a single thread",
	...
	OutDepthCameraCalibration = GenerateDepths->GetNodes()[0]->Calibrations[DepthCalibrationIndex];

	OutDepthCameraCalibration.CameraId = TEXT("Depth");
	...
	if (ResizeDepth.IsValid())
	{
		...
		if (OutDepthCameraCalibration.ImageSize.X < 640 || OutDepthCameraCalibration.ImageSize.Y < 360) // depth image is orientated on its side 
		{
			...
			UE_LOGF(LogMetaHumanCaptureSource, Warning, "%ls", *Warning.ToString());
