...
void FLiveLinkFaceConnectionIngest::FetchThumbnails(TArray<TakeId> InTakeIdList)
{
		...
		[this](const FString& InTakeName, TProtocolResult<FDataStream::FData> InData)
	{
		if (InData.IsError())
		{
			UE_LOGF(LogMetaHumanCaptureSource, Warning, "Failed to fetch thumbnail for take: %ls", *InTakeName);
			return;
		}
...
void FLiveLinkFaceConnectionIngest::OnConnectTimer()
{
	if (!bIsConnected.load())
	{
		UE_LOGF(LogMetaHumanCaptureSource, Verbose, "Connecting to the server: %ls:%d.", *DeviceIPAddress, DeviceControlPort);
...
void FLiveLinkFaceConnectionIngest::ConnectControlClient(FCommsRequestParams InParams)
{
	...
	if (!bIsConnectedLocal)
	{
		...
		{
			Result = FMetaHumanCaptureError{ EMetaHumanCaptureError::CommunicationError };
			UE_LOGF(LogMetaHumanCaptureSource, Verbose, "Failed to connect to %ls.", *DeviceIPAddress);
			return;
		}
		...
		UE_LOGF(LogMetaHumanCaptureSource, Display, "Connected to the server: %ls:%d.", *DeviceIPAddress, DeviceControlPort);
	}
...
bool FLiveLinkFaceConnectionIngest::StartCaptureHandler(TSharedPtr<FBaseCommandArgs> InCommand)
{
	if (bIsConnected.load())
	{
		...
		if (Response.IsError())
		{
			UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to start recording for slate %ls, take number %d", *StartCapture.SlateName, StartCapture.TakeNumber);
			return false;
		}

		UE_LOGF(LogMetaHumanCaptureSource, Display, "Recording started for slate %ls, take number %d", *StartCapture.SlateName, StartCapture.TakeNumber);
		return true;
	}
...
bool FLiveLinkFaceConnectionIngest::StopCaptureHandler(TSharedPtr<FBaseCommandArgs> InCommand)
{
	if (bIsConnected.load())
	{
		...
		if (Response.IsError())
		{
			UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to stop recording.");
			return false;
		}
		...
		UE_LOGF(LogMetaHumanCaptureSource, Display, "Recording stopped, resulting take name: %ls", *Response.GetResult().GetTakeName());
