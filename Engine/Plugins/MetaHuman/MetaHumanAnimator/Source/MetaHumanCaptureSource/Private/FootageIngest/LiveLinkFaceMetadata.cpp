...
bool FLiveLinkFaceMetadataParser::ParseTakeInfo(const FString& InTakeDirectory, FLiveLinkFaceTakeInfo& OutTakeInfo)
{
	...
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Failed to parse JSON object from take metadata file");
		return false;
	}
	...
	if (ParseResult.HasError())
	{
		...
		UE_LOGF(LogMetaHumanCaptureSource, Warning, "Failed to parse take metadata: %ls", *Error.GetMessage());
		return false;
	}
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	{
		UE_LOGF(LogMetaHumanCaptureSource, Error, "Take metadata parse result did not contain an error but no value was provided");
		return false;
	}
