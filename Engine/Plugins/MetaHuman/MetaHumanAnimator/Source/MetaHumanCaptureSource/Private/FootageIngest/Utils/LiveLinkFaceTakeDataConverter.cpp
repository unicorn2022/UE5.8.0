...
void FLiveLinkFaceTakeDataConverter::ConvertMovToWav(const FIngestTask& InTask, const FStopToken& InStopToken)
{
	...
	UE_LOGF(LogMetaHumanCaptureSource, Display, "Convert Mov To Wav: %ls", *MOVFilePath);
...
bool FLiveLinkFaceTakeDataConverter::ConvertVideoToImageSequenceImplementation(const FIngestTask& InTask, const FStopToken& InStopToken)
{
	...
	UE_LOGF(LogMetaHumanCaptureSource, Display, "Convert Mov To Image Sequence Start: %ls", *MOVFilePath);
	UE_LOGF(LogMetaHumanCaptureSource, Display, "Writing the video frames into %ls", *TargetVideoSequenceDirectory);
	...
	UE_LOGF(LogTemp, Display, "Convert Mov To Image Sequence Finished: %ls", *MOVFilePath);
	return true;
}
...
void FLiveLinkFaceTakeDataConverter::ConvertVideoToDepth(const FIngestTask& InTask, const FStopToken& InStopToken)
{
	...
	UE_LOGF(LogMetaHumanCaptureSource, Display, "Convert Mov To Depth Sequence Start: %ls", *MOVFilePath);
	UE_LOGF(LogMetaHumanCaptureSource, Display, "Writing the depth frames into %ls", *TargetDepthSequenceDirectory);
	...
	UE_LOGF(LogMetaHumanCaptureSource, Display, "Convert Mov To Depth Sequence Finished: %ls", *MOVFilePath);
}
