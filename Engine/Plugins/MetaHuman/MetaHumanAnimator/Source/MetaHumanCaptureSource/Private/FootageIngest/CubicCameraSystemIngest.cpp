...
FMetaHumanTakeInfo FCubicCameraSystemIngest::ReadTake(const FString& InFilePath, const FStopToken& InStopToken, const TakeId InNewTakeId)
{
	...
	if (!CubicTakeInfo)
	{
		...
		UE_LOGF(LogMetaHumanCaptureSource, Warning, "%ls", *Message.ToString());
	...
	if (Result.IsSet())
	{
		...
		UE_LOGF(LogMetaHumanCaptureSource, Warning, "The maximum audio duration can be increased by setting the \"au.SoundWaveImportLengthLimitInSeconds\" CVar.");
	}
	...
	for (TPair<FString, FCameraContext>& CameraContext : CameraContextMap)
	{
		...
		if (CameraContext.Key != MaxFrameNumberCameraName)
		{
			if (MaxStartFrameNumber != CurrentStartFrameNumber)
			{
				...
				UE_LOGF(LogMetaHumanCaptureSource, Warning, "Mismatch detected: Start timecode in \"%ls\"(%ls) differs from start timecode in \"%ls\"(%ls).",
		...
		if (CameraContext.Key != MinFrameNumberCameraName)
		{
			if (MinEndFrameNumber < CurrentEndFrameNumber)
			{
				...
				UE_LOGF(LogMetaHumanCaptureSource, Warning, "Mismatch detected: %d trailing frames in \"%ls\" do not have corresponding frames in \"%ls\". They will be ignored.",
					DiffFromEnd, *CameraContext.Key, *MinFrameNumberCameraName);
			}
		}
	}

	UE_LOGF(LogMetaHumanCaptureSource, Display, "Number of frames that will be used for depth reconstruction %d", (MinEndFrameNumber - MaxStartFrameNumber).Value);
