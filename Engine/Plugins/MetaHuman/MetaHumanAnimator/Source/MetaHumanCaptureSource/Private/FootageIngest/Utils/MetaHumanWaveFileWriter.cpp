											...
											const int32 InBitsPerSample)
{
	...
	if (!FileHandle)
	{
		UE_LOGF(LogMetaHumanWaveFileWriter, Error, "Failed to create the wave file: %ls", *InWavFilename);
		return false;
	}

	if (!FileHandle->Seek(WaveFileHeaderSize))
	{
		UE_LOGF(LogMetaHumanWaveFileWriter, Error, "Failed to access the wave file: %ls", *InWavFilename);
		return false;
	}
	return true;
}
...
bool FMetaHumanWaveFileWriter::Append(IMediaAudioSample* InSample)
{
	...
	{
		UE_LOGF(LogMetaHumanWaveFileWriter, Error, "Failed to write onto the wave file");
		return false;
	}
...
bool FMetaHumanWaveFileWriter::Close()
{
	if (!FileHandle)
	{
		UE_LOGF(LogMetaHumanWaveFileWriter, Error, "Failed to close the wave file");
		return false;
	}
