...
bool FMetaHumanCoreTest::RunTest(const FString& InTestCommand)
{
	...
	if (InTestCommand == TEXT("AssetVersions"))
	{
		...
		// This is the latest 5.8.0 release CL
		FEngineVersionBase EngineVersionPluginTargets = FEngineVersionBase(5, 8, 0, 52308457); // TODO this is not a product version of UE 5.8.0 yet as still in development
		...
		for (const FAssetData& AssetData : AssetDataList)
		{
			...
			if (!Object)
			{
				UE_LOGF(LogMetaHumanCoreTest, Error, "Failed to load asset '%ls'", *AssetData.PackageName.ToString());
				continue;
			}
			...
			if (FLinkerLoad* LinkerLoad = Object->GetLinker())
			{
				...
				{
					UE_LOGF(LogMetaHumanCoreTest, Error, "Asset '%ls' was serialized by engine %i.%i.%i (CL=%i). This stream of the plugin is currently set to target %i.%i.%i (CL=%i) and so Marketplace releases will be broken.", *AssetData.PackageName.ToString(), EngineVersionOfAsset.GetMajor(), EngineVersionOfAsset.GetMinor(), EngineVersionOfAsset.GetPatch(), EngineVersionOfAsset.GetChangelist(), EngineVersionPluginTargets.GetMajor(), EngineVersionPluginTargets.GetMinor(), EngineVersionPluginTargets.GetPatch(), EngineVersionPluginTargets.GetChangelist());
					continue;
				}
			}
			...
			for (const FAssetIdentifier& Dependency : Dependencies)
			{
				...
				{
					UE_LOGF(LogMetaHumanCoreTest, Error, "'%ls' depends on '%ls' which is not a plugin or engine dependency", *AssetData.PackageName.ToString(), *DependencyPackageName);
				}
			}
			...
			{
				UE_LOGF(LogMetaHumanCoreTest, Error, "Asset '%ls' has AssetImportData tag set with value '%ls'. This needs to be removed", *AssetData.PackageName.ToString(), *AssetImportTagValue.AsString());
				continue;
			}
			...
			if (USkeletalMesh* SkeletalMesh = Cast<USkeletalMesh>(Object))
			{
				for (int32 LODIndex = 1; LODIndex < SkeletalMesh->GetLODNum(); ++LODIndex)
				{
					if (FSkeletalMeshLODInfo* LODInfo = SkeletalMesh->GetLODInfo(LODIndex))
					{
						...
						{
							UE_LOGF(LogMetaHumanCoreTest, Error, "Skeletal Mesh '%ls' has SourceImportFilename set for LOD %d with value '%ls'. This needs to be removed", *AssetData.PackageName.ToString(), LODIndex, *LODInfo->SourceImportFilename);
						}
					}
				}

				if (UDNA* DNAAsset = USkelMeshDNAUtils::GetMeshDNAAsset(SkeletalMesh))
				{
					if (UAssetImportData* AssetImportData = DNAAsset->AssetImportData)
					{
						...
						{
							UE_LOGF(LogMetaHumanCoreTest, Error, "Asset '%ls' has %d source files in its DNAAsset user data. This needs to be removed", *AssetData.PackageName.ToString(), SourceFileCount);
						}
					}
				}
			}
	...
	else if (InTestCommand == TEXT("AssetSize"))
	{
		...
		for (const FAssetData& AssetData : AssetDataList)
		{
			if (AssetData.IsTopLevelAsset())
			{
				...
				UE_LOGF(LogMetaHumanCoreTest, Display, "Asset '%ls' has package file size %.2f MB", *AssetData.PackageName.ToString(), PackageSizeMegabytes);
		...
		{
			UE_LOGF(LogMetaHumanCoreTest, Error, "Asset packages have a total file size of %.2f MB. This is higher than the current allowed total of %.2f MB. A review will be needed to keep plugin size acceptable.", TotalAssetSizeMegabytes, AllowedTotalAssetSizeMegabytes);
			bIsOK = false;
		}
		else 
		{
			UE_LOGF(LogMetaHumanCoreTest, Display, "Asset packages have a total file size of %.2f MB. This is below the current allowed total of %.2f MB.", TotalAssetSizeMegabytes, AllowedTotalAssetSizeMegabytes);
		}
	}
