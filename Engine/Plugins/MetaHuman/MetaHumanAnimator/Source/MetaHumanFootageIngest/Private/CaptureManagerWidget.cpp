...
void SCaptureManagerWidget::HandleStartStopCaptureToggle()
{
	...
	{
		UE_LOGF(LogCaptureManager, Error, "Failed to start/stop capture: current capture source is invalid");
		return;
	}
	...
	if (!CurrentCaptureSource->bIsRecording)
	{
		...
		if (!Result)
		{
			UE_LOGF(LogCaptureManager, Error, "Failed to start capture");
			return;
		}
	...
	else
	{
		...
		if (!Result)
		{
			UE_LOGF(LogCaptureManager, Error, "Failed to stop capture");
			return;
		}
...
void SCaptureManagerWidget::StartCaptureConsoleHandler(const TArray<FString>& InArguments)
{
	...
	{
		UE_LOGF(LogCaptureManager, Error, "Failed to start capture: Invalid number of arguments");
		return;
	}

	if (!InArguments[1].IsNumeric())
	{
		UE_LOGF(LogCaptureManager, Error, "Failed to start capture: TakeNumber argument must a number");
		return;
	}
	...
	{
		UE_LOGF(LogCaptureManager, Error, "Failed to start capture: Capture Source is not selected");
	}
}
...
void SCaptureManagerWidget::StopCaptureHandler(FFootageCaptureSource* InSource, bool bInShouldFetchTake)
{
	...
	{
		UE_LOGF(LogCaptureManager, Error, "Failed to stop capture: Capture Source doesn't exist");
	}
}
