...
void SFootageIngestWidget::OnGetTakeImported(FMetaHumanCapturePerTakeVoidResult InResult)
{
	...
	for (TSharedPtr<FFootageTakeItem>& Item : QueuedTakes)
	{
			...
			Status != EFootageTakeItemStatus::Ingest_Succeeded)
		{
			...
			{
				UE_LOGF(LogCaptureManager, Log, "Ingest for take %ls was aborted by user", *Item->Name.ToString());
			}
			else if (Status == EFootageTakeItemStatus::Ingest_Succeeded_with_Warnings)
			{
				UE_LOGF(LogCaptureManager, Warning, "Ingest for take %ls produced warnings: '%ls'", *Item->Name.ToString(), *StatusMessage);
			}
			else
			{
				UE_LOGF(LogCaptureManager, Error, "Ingest for take %ls failed: '%ls'", *Item->Name.ToString(), *StatusMessage);
			}
			Item->Status = Status;
