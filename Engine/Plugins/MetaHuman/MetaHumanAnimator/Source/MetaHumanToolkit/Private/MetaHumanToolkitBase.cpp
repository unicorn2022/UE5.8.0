...
class STimeDisplayCombo : public SCompoundWidget
{
	...
	const TPair<FComboItemType, FText> DisplayFormatLocTextMap[4]
	{
		{ MakeShared<FString>(TEXT("Frames")), LOCTEXT("TimelineSequencerDisplayFormat_Frames", "Frames") },  // Do not change order unless you also change OnSelectionChanged
		{ MakeShared<FString>(TEXT("Seconds")), LOCTEXT("TimelineSequencerDisplayFormat_Seconds", "Seconds") },
		{ MakeShared<FString>(TEXT("Timecode (NDF)")), LOCTEXT("TimelineSequencerDisplayFormat_Timecode_NDF", "Timecode (NDF)") },
		{ MakeShared<FString>(TEXT("Timecode (DF)")), LOCTEXT("TimelineSequencerDisplayFormat_Timecode_DF", "Timecode (DF)") }
	};
...
public:
	...
	void Construct(const FArguments& InArgs)
	{
		...
		Options.Reserve(UE_ARRAY_COUNT(DisplayFormatLocTextMap));
		for (const TPair<FComboItemType, FText>& SequencerDisplayOption : DisplayFormatLocTextMap)
		{
			Options.Add(SequencerDisplayOption.Key);
		}

		const FSlateFontInfo Font(FCoreStyle::GetDefaultFont(), 10, "Regular");

		for (const FComboItemType& Option : Options)
		{
			const float Width = FSlateApplication::Get().GetRenderer()->GetFontMeasureService()->Measure(*Option, Font, 1.0).X + 10;
			if (Width > MinWidth)
			{
				MinWidth = Width;
			}
	...
	{
		const TPair<FComboItemType, FText>* OptionLocTextMapping = Algo::FindBy(DisplayFormatLocTextMap, InOption, [](const auto& Item) -> const FComboItemType& { return Item.Key; });
		return SNew(STextBlock).Text(OptionLocTextMapping ? OptionLocTextMapping->Value : FText::FromString(*InOption));
	}
	...
	FText GetCurrentItemLabel() const
	{
		...
		{
			const TPair<FComboItemType, FText>* CurrentItemLocTextMapping = Algo::FindBy(DisplayFormatLocTextMap, CurrentItem, [](const auto& Item) -> const FComboItemType& { return Item.Key; });
			return CurrentItemLocTextMapping ? CurrentItemLocTextMapping->Value : FText::FromString(*CurrentItem);
		}
...
{
	// Unbind sequencer delegates to prevent callbacks on a partially-destroyed toolkit.
	// These were bound via AddRaw in CreateSequencerTimeline(), which has no prevent-after-destroy protection.
	if (TimelineSequencer)
	{
		TimelineSequencer->OnGlobalTimeChanged().RemoveAll(this);
...
void FMetaHumanToolkitBase::SetMediaTrack(TSubclassOf<class UMovieSceneAudioTrack> InTrackClass, class USoundBase* InAudio, FTimecode InTimecode, FFrameNumber InStartFrameOffset)
{
	if (InAudio != nullptr)
	{
		...
		// When a video track is present, its SetMediaTrack sets the display rate
		// to the footage source rate. The audio track only needs to set a display
		// rate in audio-only mode (no video track) to avoid the default 1fps.
		if (ColourMediaTrack == nullptr)
		{
			// 30fps matches the processing rate used by the speech-to-animation
			// solver in audio-only mode (UMetaHumanPerformance::GetFrameRate()).
			const FFrameRate DefaultAudioDisplayRate(30, 1);
			RatchetMovieSceneDisplayRate(DefaultAudioDisplayRate);
		}
	}
}
