...
UENUM()
enum class EPerformanceExportSkeleton
{
	PerformerSkeleton      UMETA(DisplayName = "Performer Skeleton", Tooltip = "Output a copy of the skeleton the body was solved against, preserving the performer's proportions"),
	ExistingSkeleton	   UMETA(DisplayName = "Existing Skeleton", Tooltip = "Retarget onto a user-selected target skeleton via IK retargeting"),
	Raw					   UMETA(Hidden, DisplayName = "Raw")
};

/** How to handle frames without solved body data during Animation Sequence export. */
UENUM()
enum class EBodyUnsolvedFrameBehavior : uint8
{
	/** Repeat the most recent successfully solved frame's bone transforms. */
	LastValidFrame    UMETA(DisplayName = "Last Valid Frame"),
...
public:
	...
	UMetaHumanPerformanceExportAnimationSettings()
		...
		, bExportFace(true)
		, bRemoveRedundantCurveKeys(true)
	{}
	...
	// Whether to export face animation (blendshape curves) into the animation sequence.
	// Disabled when the performance does not contain face animation data.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings",
			  meta = (EditCondition = "IsBodyTrackerEnabled()", EditConditionHides))
	uint8 bExportFace : 1;

	// Whether to export body animation (bone tracks) into the animation sequence.
	// Disabled when the performance does not contain body animation data. 
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings", 
			  meta = (EditCondition = "IsBodyTrackerEnabled()", EditConditionHides))
	uint8 bExportBody : 1;

	// Which skeleton the body animation is baked onto.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings",
			  meta = (EditCondition = "IsBodyTrackerEnabled() && bExportBody", EditConditionHides))
	EPerformanceExportSkeleton ExportSkeleton = EPerformanceExportSkeleton::PerformerSkeleton;

	// The Skeleton or Skeletal Mesh to be used when recording the Animation Sequence
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings", 
			  meta = (AllowedClasses = "/Script/Engine.Skeleton, /Script/Engine.SkeletalMesh", GetAssetFilter = "ShouldFilterTargetAsset", EditCondition = "(bExportFace && !bExportBody) || (bExportBody && ExportSkeleton == EPerformanceExportSkeleton::ExistingSkeleton)", EditConditionHides))
	TObjectPtr<UObject> TargetSkeletonOrSkeletalMesh;

	// The retargeter to use when exporting body anim onto an existing skeleton
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings",
			  meta = (EditCondition = "bExportBody && ExportSkeleton == EPerformanceExportSkeleton::ExistingSkeleton", EditConditionHides))
	TObjectPtr<class UIKRetargeter> BodyRetargeter;

	// How to handle frames without solved body data during export (excluded, unprocessed, dropped).
	// Hold Last Frame repeats the most recent solved frame; Reference Pose uses the skeleton rest pose.
	// Leading unsolved frames (before the first solved frame) always use Reference Pose.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings",
			  meta = (DisplayName = "Unsolved Frame Behavior", EditCondition = "IsBodyTrackerEnabled() && bExportBody", EditConditionHides))
	EBodyUnsolvedFrameBehavior BodyUnsolvedBehavior = EBodyUnsolvedFrameBehavior::LastValidFrame;
	...
	// Whether or not to remove redundant curve keys, default to true
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings",
			  meta = (ScriptName = "remove_redundant_curve_keys;remove_redundant_keys"))
	uint8 bRemoveRedundantCurveKeys : 1;
	...
	// The source skeletal mesh the body animation was solved against (populated by the body tracker actor).
	// Required for body export; set automatically by the editor toolkit.Scripted callers must set it manually.
	UPROPERTY(Transient)
	TObjectPtr<class USkeletalMesh> SourceSkeletalMesh;

	// The SMPL skeletal mesh the raw body animation was solved against (populated by the body tracker actor).
	// Required for SMPL body export; set automatically by the editor toolkit.Scripted callers must set it manually.
	UPROPERTY(Transient)
	TObjectPtr<class USkeletalMesh> SMPLSourceSkeletalMesh;
...
public:
	...
	UFUNCTION()
	bool IsBodyTrackerEnabled() const;

	/** Asset picker filter for TargetSkeletonOrSkeletalMesh. When exporting body, only Skeletal Meshes are valid targets. */
	UFUNCTION()
	bool ShouldFilterTargetAsset(const FAssetData& InAssetData) const;
...
public:
	...
	UMetaHumanPerformanceExportLevelSequenceSettings()
		...
		, bMatchCameraAspectRatioToOriginalFootage(false)
	...
	// Whether or not to use the aspect ratio of the original camera footage for the exported camera. Default to false
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Export Settings", meta = (EditCondition = "bExportCamera"))
	uint8 bMatchCameraAspectRatioToOriginalFootage : 1;
...
public:
	...
	static UE_API bool CanExportFaceAnimation(const class UMetaHumanPerformance* InPerformance);
	static UE_API bool CanExportBodyAnimation(const class UMetaHumanPerformance* InPerformance);
...
private:
	...
	static UE_API void PromptBodyCompatibilitySkeleton(USkeleton* InBodySkeleton);
