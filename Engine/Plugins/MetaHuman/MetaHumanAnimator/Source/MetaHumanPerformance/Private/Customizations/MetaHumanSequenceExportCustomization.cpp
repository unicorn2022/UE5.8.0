...
{
	FString ParentPath;
	if (!InAssetData.GetTagValue(FBlueprintTags::NativeParentClassPath, ParentPath) || ParentPath.IsEmpty())
	{
		return true; // reject as no parent info
	}

	// Native classes are always loaded, so TryFindTypeSlow will find them without triggering any loads.
	UClass* NativeParentClass = UClass::TryFindTypeSlow<UClass>(FPackageName::ExportTextPathToObjectPath(ParentPath));
	if (!NativeParentClass || !NativeParentClass->IsChildOf<AActor>())
	{
		return true; // not an Actor subclass or native class not found
	}
