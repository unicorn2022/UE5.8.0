...
float UContourDataComparisonHelper::ContourComparisonTolerance = 2.4f;
...
bool UContourDataComparisonHelper::ComparePerformanceContourData(const UMetaHumanPerformance* InOriginal, const UMetaHumanPerformance* InNew)
{
	...
	if (!InOriginal || !InNew)
	{
		UE_LOGF(LogMetaHumanPerformance, Error, "Invalid performance asset was specified for contour data comparison");
		return false;
	}

	if (InOriginal->ContourTrackingResults.Num() != InNew->ContourTrackingResults.Num())
	{
		UE_LOGF(LogMetaHumanPerformance, Error, "Mismatch in number of frames for contour data");
		return false;
	}
	...
	for (const FFrameTrackingContourData& OriginalFrameContourData : InOriginal->ContourTrackingResults)
	{
		...
		if (bGoldDataFrameProcessed && bGeneratedDataFrameProcessed)
		{
			...
			if (!bSchemesMatch)
			{
				UE_LOGF(LogTemp, Error, "A mismatch for following contour names:%ls", *MismatchedCurves);
				return false;
			}
		...
		{
			UE_LOGF(LogTemp, Error, "A mismatch in contour data presence between gold and test data for frame %d", FirstValidFrame);
			return false;
		}
	...
	for (const FFrameTrackingContourData& OriginalFrameContourData : InOriginal->ContourTrackingResults)
	{
		...
		{
			UE_LOGF(LogTemp, Error, "Contour data presence mismatch for frame %i", FrameNum);
			bMatch = false;
		}
		...
		for (const TPair<FString, FTrackingContour>& PerCurveData : OriginalFrameContourData.TrackingContours)
		{
			...
			{
				UE_LOGF(LogTemp, Error, "Contour data mismatch for frame %i curve %ls. Maximum point delta for this curve was %f", FrameNum, *PerCurveData.Key, MaximumCurveDelta);
				bMatch = false;
			}
		}
...
bool FMetaHumanPerformanceExportAnimationSequenceTest::RunTest(const FString& InParameters)
{
	...
	ExportAnimSettings->bRemoveRedundantCurveKeys = false;
	...
	ON_SCOPE_EXIT
	{
		// Wait for any in-flight async compression tasks to complete before cleanup,
		// otherwise GC can collect the AnimSequence while compression is still running
		UE::Anim::IAnimSequenceCompilingManager::FinishCompilation(MakeArrayView(&ExportedAnimSequence, 1));
	...
	ExportAnimSettings->bRemoveRedundantCurveKeys = true;
	...
	ON_SCOPE_EXIT
	{
		// Wait for any in-flight async compression tasks to complete before cleanup,
		// otherwise GC can collect the AnimSequence while compression is still running
		UE::Anim::IAnimSequenceCompilingManager::FinishCompilation(MakeArrayView(&ExportedWithKeyReductionAnimSequence, 1));
