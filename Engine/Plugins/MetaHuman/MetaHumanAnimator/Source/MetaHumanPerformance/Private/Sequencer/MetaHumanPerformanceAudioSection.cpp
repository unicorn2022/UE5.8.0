...
int32 FMetaHumanPerformanceAudioSection::OnPaintSection(FSequencerSectionPainter& InPainter) const
{
	...
	if (ISequencer.IsValid() && Performance)
	{
		...
		if (Performance->IsProcessing() && Performance->InputType == EDataInputType::Audio)
		{
			LayerId = MetaHumanPerformanceSectionPainterHelper::PaintCurrentProcessingStage(InPainter, LayerId, ISequencer.Get(), Section, Performance);
		}
	}
