...
static void OnEnableRawBodyExport(IConsoleVariable* InConsoleVariable)
{
	const bool bEnableRawBodyExport = InConsoleVariable->GetBool();

	UEnum* EnumType = StaticEnum<EPerformanceExportSkeleton>();
	const int32 TargetIndex = EnumType->GetIndexByValue(static_cast<int64>(EPerformanceExportSkeleton::Raw));

	if (bEnableRawBodyExport)
	{
		EnumType->RemoveMetaData(TEXT("Hidden"), TargetIndex);
	}
	else
	{
		EnumType->SetMetaData(TEXT("Hidden"), TEXT(""), TargetIndex);
	}
}

static USkeleton* FindMetaHumanBaseSkeleton()
{
	const FSoftObjectPath MetaHumanBaseSkeletonPath(TEXT("/Game/MetaHumans/Common/Female/Medium/NormalWeight/Body/metahuman_base_skel.metahuman_base_skel"));
	...
	FAssetData AssetData = AssetRegistry.GetAssetByObjectPath(MetaHumanBaseSkeletonPath);
...
/**
 * Collects bone transforms for every frame in the export range, filling unsolved frames.
 * Discovers the tracked bone set from the first solved frame's data, then ensures every
 * tracked bone gets exactly one key per frame. Reference pose is used for gap filling only.
 */
static TMap<FString, TArray<FTransform>> CollectBodyFrames(
	const UMetaHumanPerformance* InPerformance,
	const TRange<FFrameNumber>& InExportFrameRange,
	const TMap<FString, FTransform>& InRefPoseMap,
	bool bInUseRefPoseForGaps,
	TFunctionRef<const TMap<FString, FTransform>*(const FFrameAnimationData&)> GetBoneTransforms,
	TArray<bool>& OutSolvedFlags)
{
	const int32 NumExportFrames = InExportFrameRange.GetUpperBoundValue().Value - InExportFrameRange.GetLowerBoundValue().Value;
	const int32 ProcessingLimitStart = InPerformance->GetProcessingLimitFrameRange().GetLowerBoundValue().Value;

	// First pass: find the bone set from solved data and collect per-frame solved/unsolved status.
	// We only write tracks for bones that the body tracker actually produces — not every bone
	// in the skeleton. Bones the tracker doesn't touch keep their ref pose implicitly (no track).
	TSet<FString> TrackedBones;
	OutSolvedFlags.Reserve(NumExportFrames);

	for (int32 FrameIndex = InExportFrameRange.GetLowerBoundValue().Value; FrameIndex < InExportFrameRange.GetUpperBoundValue().Value; ++FrameIndex)
	{
		const FFrameAnimationData& FrameAnimData = InPerformance->AnimationData[FrameIndex - ProcessingLimitStart];
		const TMap<FString, FTransform>* FrameTransforms = GetBoneTransforms(FrameAnimData);
		const bool bIsSolved = FrameTransforms && !FrameTransforms->IsEmpty();

		OutSolvedFlags.Add(bIsSolved);

		if (bIsSolved && TrackedBones.IsEmpty())
		{
			FrameTransforms->GetKeys(TrackedBones);
		}
	}

	// If no solved frames exist in the export range, there are no bones to write.
	if (TrackedBones.IsEmpty())
	{
		return {};
	}

	// Second pass: collect transforms for tracked bones only, filling unsolved frames.
	TMap<FString, TArray<FTransform>> Result;
	for (const FString& BoneName : TrackedBones)
	{
		Result.Add(BoneName).Reserve(NumExportFrames);
	}

	TMap<FString, FTransform> LastSolvedFrame;
	bool bHasSolvedFrame = false;
	int32 FrameIdx = 0;

	for (int32 FrameIndex = InExportFrameRange.GetLowerBoundValue().Value; FrameIndex < InExportFrameRange.GetUpperBoundValue().Value; ++FrameIndex, ++FrameIdx)
	{
		const FFrameAnimationData& FrameAnimData = InPerformance->AnimationData[FrameIndex - ProcessingLimitStart];
		const TMap<FString, FTransform>* FrameTransforms = GetBoneTransforms(FrameAnimData);
		const bool bIsSolved = OutSolvedFlags[FrameIdx];

		if (bIsSolved)
		{
			LastSolvedFrame = *FrameTransforms;
			bHasSolvedFrame = true;
		}
		...
		for (const FString& BoneName : TrackedBones)
		{
			...
			if (bIsSolved)
			{
				Transform = FrameTransforms->Find(BoneName);
			}
			else if (bHasSolvedFrame && !bInUseRefPoseForGaps)
			{
				Transform = LastSolvedFrame.Find(BoneName);
			}

			// Fall back to ref pose for unsolved frames, or identity if bone not in ref pose
			const FTransform* RefPose = InRefPoseMap.Find(BoneName);
			Result[BoneName].Add(Transform ? *Transform : (RefPose ? *RefPose : FTransform::Identity));
		}
	}
...
static TAutoConsoleVariable<bool> CVarEnableRawBodyExport(
	TEXT("mh.BodyTracker.EnableRawExport"),
	false,
	TEXT("When true, exposes the Raw body animation export option in the export dialog."),
	FConsoleVariableDelegate::CreateStatic(&OnEnableRawBodyExport),
	ECVF_Default
);
...
public:
	...
	bool CanExport() const
	{
		...
		{
			if (!ExportAnimationSettings->bExportBody && !ExportAnimationSettings->bExportFace)
			{
				return false;
			}

			if (ExportAnimationSettings->bExportBody && 
				ExportAnimationSettings->ExportSkeleton != EPerformanceExportSkeleton::ExistingSkeleton)
			{
				return true;
			}
		}
	...
	FReply ExportClicked()
	{
		if (UMetaHumanPerformanceExportAnimationSettings* ExportAnimationSettings = Cast<UMetaHumanPerformanceExportAnimationSettings>(SettingsObject))
		{
			...
			if (!ExportAnimationSettings->IsTargetSkeletonCompatible(AnimationCurves, MissingCurves))
			{
				if (ExportAnimationSettings->bExportBody && ExportAnimationSettings->bExportFace)
				{
					MissingCurves.RemoveAll([](const FString& InMissingCurveName)
					{
						return InMissingCurveName.Contains(TEXT("CTRL_expressions_"));
					});
				}
				...
				if (!MissingCurves.IsEmpty())
				{
					...
					FSuppressableWarningDialog::FSetupInfo Info(
						...
						TEXT("SuppressIncompatibleSkeletons"));
			...
			// Warn if the selected target skeleton is missing body bones that the body tracker produces
			if (bExportClicked && ExportAnimationSettings->bExportBody &&
				ExportAnimationSettings->ExportSkeleton == EPerformanceExportSkeleton::ExistingSkeleton)
			{
				USkeleton* TargetSkeleton = ExportAnimationSettings->GetTargetSkeleton();
				if (TargetSkeleton)
				{
					// Find the bone names from the first solved body frame
					TArray<FString> BodyBoneNames;
					for (const FFrameAnimationData& FrameData : Performance->AnimationData)
					{
						if (!FrameData.BodyAnimationData.IsEmpty())
						{
							FrameData.BodyAnimationData.GetKeys(BodyBoneNames);
							break;
						}
					}
					...
					if (!BodyBoneNames.IsEmpty())
					{
						...
						const bool bHasMissingBones = BodyBoneNames.ContainsByPredicate([&RefSkel](const FString& BoneName)
						{
							return RefSkel.FindBoneIndex(FName(*BoneName)) == INDEX_NONE;
						});

						if (bHasMissingBones)
						{
							FSuppressableWarningDialog::FSetupInfo Info(
								LOCTEXT("IncompatibleBodySkeletonMessage",
									"The selected target Skeleton is missing body bones expected by the body tracker.\n\n"
									"The exported body animation may not play correctly on this skeleton.\n\n"
									"Do you want to continue?"),
								LOCTEXT("IncompatibleBodySkeletonTitle", "Target Skeleton may be incompatible with body animation"),
								TEXT("SuppressIncompatibleBodySkeleton"));
							Info.ConfirmText = LOCTEXT("BodyCompat_Yes", "Yes");
							Info.CancelText = LOCTEXT("BodyCompat_No", "No");

							FSuppressableWarningDialog Dialog { Info };
							if (Dialog.ShowModal() == FSuppressableWarningDialog::Cancel)
							{
								bExportClicked = false;
							}
						}
					}
				}
	...
	bool PropertyIsReadOnly(const FPropertyAndParent& InPropertyAndParent) const
	{
		...
		else if (PropertyName == GET_MEMBER_NAME_CHECKED(UMetaHumanPerformanceExportAnimationSettings, bExportFace))
		{
			return !UMetaHumanPerformanceExportUtils::CanExportFaceAnimation(Performance);
		} 
		else if (PropertyName == GET_MEMBER_NAME_CHECKED(UMetaHumanPerformanceExportAnimationSettings, bExportBody))
		{
			return !UMetaHumanPerformanceExportUtils::CanExportBodyAnimation(Performance);
		}
...
bool UMetaHumanPerformanceExportAnimationSettings::IsBodyTrackerEnabled() const
{
	return IModularFeatures::Get().IsModularFeatureAvailable(IMetaHumanBodyTrackerInterface::GetModularFeatureName());
}

bool UMetaHumanPerformanceExportAnimationSettings::ShouldFilterTargetAsset(const FAssetData& InAssetData) const
{
	const bool bIsSkeleton = (InAssetData.AssetClassPath == USkeleton::StaticClass()->GetClassPathName());
	const bool bFiltered = (bExportBody && bIsSkeleton);

	// When exporting body animation, only Skeletal Meshes are valid targets (the retargeter needs a mesh, not a skeleton).
	return bFiltered;
}
...
UMetaHumanPerformanceExportAnimationSettings* UMetaHumanPerformanceExportUtils::GetExportAnimationSequenceSettings(const UMetaHumanPerformance* InPerformance)
{
	...
	ExportAnimationSettings->bExportFace = CanExportFaceAnimation(InPerformance);
	ExportAnimationSettings->bExportBody = CanExportBodyAnimation(InPerformance);
	...
	if (IModularFeatures::Get().IsModularFeatureAvailable(IMetaHumanBodyTrackerInterface::GetModularFeatureName()))
	{
		...
		ExportAnimationSettings->BodyRetargeter = LoadObject<UIKRetargeter>(nullptr, BodyTracker.GetMetaHumanRetargeterAssetPath());
	}

	// Try to use a sensible default target skeleton for the animation sequence.
	// When body export is enabled, default to the body tracker's MetaHuman skeleton.
	// Otherwise, default to the Face Archetype skeleton for face-only export.
	if (ExportAnimationSettings->bExportBody && IModularFeatures::Get().IsModularFeatureAvailable(IMetaHumanBodyTrackerInterface::GetModularFeatureName()))
	{
		const FString BodyMeshPath = TEXT("/MetaHumanBodyTracker/SKM_Body.SKM_Body");

		if (USkeletalMesh* BodyMesh = LoadObject<USkeletalMesh>(nullptr, *BodyMeshPath, nullptr, LOAD_NoWarn))
		{
			ExportAnimationSettings->TargetSkeletonOrSkeletalMesh = BodyMesh;
		}
	}
	else
	{
		...
		if (USkeleton* ArchetypeSkeleton = LoadObject<USkeleton>(nullptr, *MetaHumanPath, nullptr, LOAD_NoWarn))
		{
...
UMetaHumanPerformanceExportLevelSequenceSettings* UMetaHumanPerformanceExportUtils::GetExportLevelSequenceSettings(const UMetaHumanPerformance* InPerformance)
{
	...
	ExportLevelSequenceSettings->bMatchCameraAspectRatioToOriginalFootage = false;
	...
	{
		ExportLevelSequenceSettings->bExportControlRigTrack = InPerformance->ControlRigAssetReference.IsValid();
...
UAnimSequence* UMetaHumanPerformanceExportUtils::ExportAnimationSequence(UMetaHumanPerformance* InPerformance, UMetaHumanPerformanceExportAnimationSettings* InExportSettings)
{
	...
	{
		UE_LOGF(LogMetaHumanPerformance, Error, "Failed to export Animation Sequence. Performance is not valid");
		return nullptr;
	}
	...
	if (InExportSettings->bShowExportDialog)
	{
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Error, "Export of Animation Sequence '%ls' has been cancelled", *NewAnimSequence->GetName());
			if (bUseExisting)
			{
	...
	{
		InExportSettings->PackagePath = FPackageName::GetLongPackagePath(NewAnimSequence->GetOutermost()->GetName());
		InExportSettings->AssetName = NewAnimSequence->GetName();

		if (InExportSettings->bExportBody)
		{
			USkeletalMesh* SkeletalMesh = nullptr;
			if (InExportSettings->ExportSkeleton == EPerformanceExportSkeleton::PerformerSkeleton)
			{
				if (!InExportSettings->SourceSkeletalMesh && IModularFeatures::Get().IsModularFeatureAvailable(IMetaHumanBodyTrackerInterface::GetModularFeatureName()))
				{
					IMetaHumanBodyTrackerInterface& BodyTracker = IModularFeatures::Get().GetModularFeature<IMetaHumanBodyTrackerInterface>(IMetaHumanBodyTrackerInterface::GetModularFeatureName());

					for (const FFrameAnimationData& FrameAnimationData : InPerformance->AnimationData)
					{
						if (!FrameAnimationData.RawBodyAnimationSMPLXShape.IsEmpty())
						{
							InExportSettings->SourceSkeletalMesh = BodyTracker.MakeMHSkeletalMesh(GetTransientPackage(), FrameAnimationData.RawBodyAnimationSMPLXShape);
							break;
						}
					}
				...
				if (!InExportSettings->SourceSkeletalMesh)
				{
					UE_LOGF(LogMetaHumanPerformance, Error,
							"Cannot export body animation to the Performer Skeleton: SourceSkeletalMesh is null. "
							"Populate InExportSettings->SourceSkeletalMesh before calling ExportAnimationSequence "
							"(the editor toolkit does this automatically from the body tracker actor).");
					return nullptr;
				}

				SkeletalMesh = InExportSettings->SourceSkeletalMesh;
			}
			else if (InExportSettings->ExportSkeleton == EPerformanceExportSkeleton::Raw)
			{
				if (!InExportSettings->SMPLSourceSkeletalMesh && IModularFeatures::Get().IsModularFeatureAvailable(IMetaHumanBodyTrackerInterface::GetModularFeatureName()))
				{
					IMetaHumanBodyTrackerInterface& BodyTracker = IModularFeatures::Get().GetModularFeature<IMetaHumanBodyTrackerInterface>(IMetaHumanBodyTrackerInterface::GetModularFeatureName());
					...
					for (const FFrameAnimationData& FrameAnimationData : InPerformance->AnimationData)
					{
						if (!FrameAnimationData.RawBodyAnimationSMPLXShape.IsEmpty())
						{
							InExportSettings->SMPLSourceSkeletalMesh = BodyTracker.MakeSMPLSkeletalMesh(GetTransientPackage(), FrameAnimationData.RawBodyAnimationSMPLXShape);
							break;
						}
					}
				}

				if (!InExportSettings->SMPLSourceSkeletalMesh)
				{
					UE_LOGF(LogMetaHumanPerformance, Error,
							"Cannot export Raw body animation: SMPLSourceSkeletalMesh is null. "
							"Populate InExportSettings->SMPLSourceSkeletalMesh before calling ExportAnimationSequence "
							"(the editor toolkit does this automatically from the body tracker actor).");
					return nullptr;
				}

				SkeletalMesh = InExportSettings->SMPLSourceSkeletalMesh;
			}
			
			if (SkeletalMesh)
			{
				FString SourceSkelMeshName = InExportSettings->AssetName;
				SourceSkelMeshName.RemoveFromStart(TEXT("AS_"));
				SourceSkelMeshName = TEXT("SK_") + SourceSkelMeshName;

				UPackage* Package = CreatePackage(*(InExportSettings->PackagePath + TEXT("/") + SourceSkelMeshName));

				USkeletalMesh* SourceSkeletalMeshCopy = DuplicateObject<USkeletalMesh>(SkeletalMesh, Package, *SourceSkelMeshName);
				SourceSkeletalMeshCopy->SetFlags(RF_Public | RF_Standalone);
				SourceSkeletalMeshCopy->Modify();

				SourceSkeletalMeshCopy->MarkPackageDirty();
				...
				InExportSettings->TargetSkeletonOrSkeletalMesh = SourceSkeletalMeshCopy;
			}
		}
		...
		if (TargetSkeleton != nullptr)
		{
			...
			// For a scripted import the head reference frame could have never been calculated 
			InPerformance->CalculateReferenceFramePose();

			if (InExportSettings->bExportBody && 
				InExportSettings->ExportSkeleton != EPerformanceExportSkeleton::Raw)
			{
				PromptBodyCompatibilitySkeleton(TargetSkeleton);
			}
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Error, "Failed to create Animation Sequence '%ls/%ls due to invalid target Skeleton'", *PackagePath, *AssetName);
		}
	}
	else
	{
		UE_LOGF(LogMetaHumanPerformance, Error, "Failed to create Animation Sequence '%ls/%ls'", *PackagePath, *AssetName);
	}
...
void UMetaHumanPerformanceExportUtils::BakeControlRigTrack(const FBakeControlRigTrackParams& InParams)
{
	...
	if (InParams.ControlRigClass)
	{
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Error, "Failed to add new Control Rig track in the Level Sequence '%ls'", *(InParams.LevelSequence->GetName()));
		}
	}
	else
	{
		UE_LOGF(LogMetaHumanPerformance, Error, "Failed to obtain Control Rig Blueprint class '%ls'", *InParams.Performance->ControlRigAssetReference.GetName());
	}
}
...
ULevelSequence* UMetaHumanPerformanceExportUtils::ExportLevelSequence(UMetaHumanPerformance* InPerformance, UMetaHumanPerformanceExportLevelSequenceSettings* InExportSettings)
{
	...
	if (InPerformance == nullptr)
	{
		UE_LOGF(LogMetaHumanPerformance, Error, "Invalid Performance passed to ExportLevelSequence");
		return nullptr;
	}
	...
	if (InExportSettings->bShowExportDialog)
	{
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Error, "Failed to create new Level Sequence named '%ls'", *(InExportSettings->PackagePath / InExportSettings->AssetName))
			return nullptr;
		}

		if (!SMetaHumanPerformanceExportSettings::ShowSettingsDialog(InExportSettings, InPerformance))
		{
			UE_LOGF(LogMetaHumanPerformance, Warning, "Export of Level Sequence '%ls' has been cancelled", *NewLevelSequence->GetName());
			if (bUseExisting)
			{
	...
	else
	{
		...
		if (NewLevelSequence == nullptr)
		{
			UE_LOGF(LogMetaHumanPerformance, Error, "Failed to create new Level Sequence asset named '%ls'", *(InExportSettings->PackagePath / InExportSettings->AssetName))
			return nullptr;
		}
	}
	...
	const float DefaultNoseHeight = 145.98f;
	float IdentityNosePositionZ = DefaultNoseHeight; // default nose height, since if there is no Identity, this is also used for MetaHuman export ie mono and audio cases
	float IdentityNosePositionY = 0.0;
	if (CanExportIdentity(InPerformance))
	{
		...
		IdentityNosePositionZ = IdentityNosePosition.Z;
		IdentityNosePositionY = IdentityNosePosition.Y;
		// set the offset transform to the identity nose position z, in the local frame of reference
		const FVector LocalOffset(0.0, 0.0, IdentityNosePositionZ);
		OffsetTransform.SetTranslation(
			ReferenceFramePose.TransformVectorNoScale(LocalOffset)
		);
	}
	else
	{
		OffsetTransform = FTransform{ FVector{ 0.0, 0.0, DefaultNoseHeight } }; // default nose height
	}
	...
	if (InExportSettings->TargetMetaHumanClass != nullptr)
	{
		...
		if(MetaHumanActor)
		{
			...
			if (AActor* SpawnedMetaHuman = TempWorld->SpawnActor<AActor>(MetaHumanActor->GetClass(), SpawnInfo))
			{
				...
				{
					// Get the difference between the MetaHuman nose bone position and the current offset transform, in the local frame of reference
					FVector MHNosePosition = UMetaHumanPerformance::GetSkelMeshReferenceBoneLocation(FaceComponent, NoseBoneName);
					const FVector Offset(0.0, MHNosePosition.Y - IdentityNosePositionY, MHNosePosition.Z - IdentityNosePositionZ);
					FTransform MHNoseOffset;
					MHNoseOffset.SetTranslation(
						ReferenceFramePose.TransformVectorNoScale(Offset)
					);
				...
				{
					UE_LOGF(LogMetaHumanPerformance, Warning, "Unable to update offset transform. Failed to find '%ls' component in spawned class '%ls'", *MetaHumanFaceComponentName.ToString(), *InExportSettings->TargetMetaHumanClass->GetName());
				}
				...
				{
					UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to find '%ls' component in spawned class '%ls'", *MetaHumanFaceComponentName.ToString(), *InExportSettings->TargetMetaHumanClass->GetName());
				}
				...
				{
					UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to find '%ls' component in spawned class '%ls'", *MetaHumanBodyComponentName.ToString(), *InExportSettings->TargetMetaHumanClass->GetName());
				}
			}
			else
			{
				UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to spawn Actor of class '%ls'", *InExportSettings->TargetMetaHumanClass->GetName());
			}
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to spawn Actor of class '%ls' in the Level", *InExportSettings->TargetMetaHumanClass->GetName());
		}
	}
	...
	if (InExportSettings->bExportIdentity)
	{
		if (CanExportIdentity(InPerformance))
		{
			...
			if (InExportSettings->bExportControlRigTrack)
			{
				...
				if (UClass* IdentityControlRigClass = InPerformance->ControlRigAssetReference.GetRigClass())
				{
					BakeControlRigTrack({
						.Performance = InPerformance,
						.ExportSettings = InExportSettings,
						.ProcessingRange = ProcessingRange,
						.LevelSequence = NewLevelSequence,
						.ControlRigClass = IdentityControlRigClass,
			...
			if (InExportSettings->bExportTransformTrack)
			{
				...
				{
					UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to add new Transform track in the Level Sequence '%ls'", *InExportSettings->AssetName);
				}
			}
		}
		else
		{
			UE_LOGF(LogMetaHumanPerformance, Warning, "Export identity was set no valid identity was found in '%ls'. Skipping identity export", *InExportSettings->AssetName);
		}
	}
	...
	if (InExportSettings->bExportVideoTrack)
	{
		if (CanExportVideoTrack(InPerformance))
		{
			if (UMovieSceneMediaTrack* NewVideoTrack = NewMovieScene->AddTrack<UMovieSceneMediaTrack>())
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to determine frame range for the video media track");
					}
				}
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Warning, "Export video track was set no video sequence was found in '%ls'. Skipping video track export", (FootageCaptureData != nullptr) ? *FootageCaptureData->GetName() : TEXT("No Footage Capture Data"));
		}
	}
		...
		if (CanExportDepthTrack(InPerformance))
		{
			if (UMovieSceneMediaTrack* NewDepthTrack = NewMovieScene->AddTrack<UMovieSceneMediaTrack>())
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to determine frame range for depth media track");
					}
				}
			...
			{
				UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to add depth track in the exported Level Sequence '%ls'", *InExportSettings->AssetName);
			}
		}
		else
		{
			UE_LOGF(LogMetaHumanPerformance, Warning, "Export depth track was set but no depth sequence was found in '%ls' Capture Data. Skipping depth track export", (FootageCaptureData != nullptr) ? *FootageCaptureData->GetName() : TEXT("No Footage Capture Data"));
		}
	}
		...
		if (CanExportAudioTrack(InPerformance))
		{
			if (UMovieSceneAudioTrack* NewAudioTrack = NewMovieScene->AddTrack<UMovieSceneAudioTrack>())
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to determine frame range for audio track");
					}
				}
			}
			else
			{
				UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to add audio track in the exported Level Sequence '%ls'", *InExportSettings->AssetName);
			}
		}
		else
		{
	...
	if (InExportSettings->bExportCamera)
	{
		...
		if (FootageCaptureData && TempFootageComponent)
		{
			...
			if (InExportSettings->bMatchCameraAspectRatioToOriginalFootage)
			{
				int32 CameraViewIndex = FootageCaptureData->CameraCalibrations[0]->GetCalibrationIndexByName(InPerformance->Camera);
				const FCameraCalibration& DistortionVals = Calibrations[CameraViewIndex];

				float ImageSizeY = DistortionVals.ImageSize.Y;
				float ImageSizeX = DistortionVals.ImageSize.X;
				float SensorHeight = CameraComponent->Filmback.SensorWidth * ImageSizeY / ImageSizeX;
				CameraComponent->Filmback.SensorHeight = SensorHeight;
			}
		...
		// Shift the principal point on the camera component if applicable
		FVector2D PrincipalPointNormalized{ 0.5f, 0.5f };
		if (FootageCaptureData && !FootageCaptureData->CameraCalibrations.IsEmpty())
		{
			int32 CameraViewIndex = FootageCaptureData->CameraCalibrations[0]->GetCalibrationIndexByName(InPerformance->Camera);
			const FCameraCalibration& DistortionVals = Calibrations[CameraViewIndex];
			PrincipalPointNormalized = DistortionVals.PrincipalPoint / DistortionVals.ImageSize;
			CameraComponent->Filmback.SensorHorizontalOffset = (0.5f - PrincipalPointNormalized.X) * CameraComponent->Filmback.SensorWidth;
			CameraComponent->Filmback.SensorVerticalOffset = (PrincipalPointNormalized.Y - 0.5f) * CameraComponent->Filmback.SensorHeight; // OpenCV image Y axis is down
		}
		...
		if (InExportSettings->bApplyLensDistortion && CanExportLensDistortion(InPerformance))
		{
			...
			FLensDistortionState ManualDistortion;
 			...
 			ManualDistortion.DistortionInfo.Parameters.Add(DistortionVals.K3); // TODO should be K1 K2 P1 P2 K3
			...
			ManualDistortion.DistortionInfo.Parameters.Add(DistortionVals.P2);
 			...
 			ManualDistortion.ImageCenter.PrincipalPoint = PrincipalPointNormalized;
		...
		UControlRig* InRecordControlRig, const FVector& InVisualizeMeshHeightOffset)
{
	...
	{
		UE_LOGF(LogMetaHumanPerformance, Warning, "ControlRig '%ls' doesn't support the Backwards Solve event. No keys will be recorded.", *RecordControlRig->GetName());
		return;
	}
	...
	if (AnimationFrame.Pose.IsValid())
	{
		...
		{
			USkeletalMesh* PreviewSkelMesh = InPerformance->GetHeadMesh();
			FTransform HeadBoneInitialTransform;

			if (!PreviewSkelMesh)
			{
				UE_LOGF(LogMetaHumanPerformance, Error, "Could not find Skeleton. Head Movement export will be disabled.");
			}
			...
			{
				UE_LOGF(LogMetaHumanPerformance, Error, "Could not find head bone in Skeleton '%ls'. Head Movement export will be disabled.", *PreviewSkelMesh->GetSkeleton()->GetName());
			}
		}
	}
	...
	const TArray<FRigElementKey> BoneKeys = RigHierarchy->GetBoneKeys();
	for (const FRigElementKey& Bone : BoneKeys)
	{
		if (const FTransform* Value = AnimationFrame.BodyAnimationData.Find(Bone.Name.ToString()))
		{
			RigHierarchy->SetLocalTransform(Bone, *Value);
		}
	}
	...
	// Flush any stale dirty flags before each Execute call.
	// After the previous frame's BackwardsSolve, some elements can be left with CurrentGlobal dirty.
	// ResetPoseToInitial (called inside Execute's initialization path) then copies that dirty Initial
	// state verbatim to Current. If the rig subsequently marks CurrentLocal dirty on the same element
	// via PropagateDirtyFlags or SetGlobalTransform, both Local and Global end up dirty simultaneously,
	// which violates the hierarchy invariant and fires the ensure in MarkDirty / GetTransform.
	// Touching all transform types here forces the lazy evaluation to resolve every dirty element,
	// leaving the hierarchy in a clean and consistent state before the next Execute.
#if WITH_EDITOR
	{
		URigHierarchy* RigHierarchyToFlush = RecordControlRig->GetHierarchy();
		const int32 NumElements = RigHierarchyToFlush->Num();
		for (int32 ElementIdx = 0; ElementIdx < NumElements; ++ElementIdx)
		{
			RigHierarchyToFlush->GetLocalTransformByIndex(ElementIdx, /*bInitial=*/false);
			RigHierarchyToFlush->GetLocalTransformByIndex(ElementIdx, /*bInitial=*/true);
			RigHierarchyToFlush->GetGlobalTransformByIndex(ElementIdx, /*bInitial=*/false);
			RigHierarchyToFlush->GetGlobalTransformByIndex(ElementIdx, /*bInitial=*/true);
		}
	}
#endif
...
void UMetaHumanPerformanceExportUtils::SetHeadControlSwitchEnabled(class UMovieSceneControlRigParameterTrack* InControlRigTrack, bool bInEnableHeadControl)
{
	if (InControlRigTrack != nullptr)
	{
		...
		if (UMovieSceneControlRigParameterSection* ControlRigSection = Cast<UMovieSceneControlRigParameterSection>(InControlRigTrack->GetAllSections()[0]))
		{
			...
			{
				UE_LOGF(LogMetaHumanPerformance, Error, "Could not find switch for head control '%ls' for Control Rig '%ls'", *UMetaHumanPerformanceControlRigComponent::HeadIKSwitchControlName.ToString(), *InControlRigTrack->GetControlRig()->GetName());
			}
		}
	}
}
	...
	if (UAnimSequence* AnimSequence = Cast<UAnimSequence>(InNewAssets[0]))
	{
		...
		if (InPerformance->InputType == EDataInputType::Audio)
		{
			if (TObjectPtr<USoundWave> Audio = InPerformance->GetAudioForProcessing())
			{
				...
				{
					UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to determine frame range for soundwave asset. Timecode information will be incorrect.");
				}
			}
		}
		else if (InPerformance->FootageCaptureData && !InPerformance->FootageCaptureData->ImageSequences.IsEmpty())
		{
			...
			{
				UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to determine frame range for the image sequence. Timecode information will be incorrect.");
			}
		}
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Error, "Could not find head bone in Skeleton '%ls'. Head Movement export will be disabled.", *AnimSequence->GetSkeleton()->GetName());
		...
		for (int32 FrameIndex = ExportFrameRange.GetLowerBoundValue().Value; FrameIndex < ExportFrameRange.GetUpperBoundValue().Value; ++FrameIndex, ++TimecodeFrame)
		{
			...
			if (!InExportSettings->bExportFace)
			{
				continue;
			}
			...
			if (!FrameAnimData.ContainsData(EFrameAnimationDataType::Face))
			{
				continue;
			}
			...
			for (const TPair<FString, float>& Sample : FrameAnimData.AnimationData)
			{
				...
				if (!AddedCurves.Contains(Sample.Key))
				{
					...
					{
						UE_LOGF(LogMetaHumanPerformance, Warning, "Failed to add animation curve '%ls' into '%ls'", *Sample.Key, *AnimSequence->GetName());
						continue;
					}
		...
		if (InExportSettings->bExportBody)
		{
			// Determine which skeleton provides the reference pose for gap filling.
			// The AnimSequence is bound to the target skeleton — we need the same skeleton's ref pose.
			USkeleton* BodySkeleton = nullptr;
			if (InExportSettings->ExportSkeleton == EPerformanceExportSkeleton::ExistingSkeleton)
			{
				if (USkeletalMesh* SkelMesh = Cast<USkeletalMesh>(InExportSettings->TargetSkeletonOrSkeletalMesh))
				{
					BodySkeleton = SkelMesh->GetSkeleton();
				}
				else if (USkeleton* Skeleton = Cast<USkeleton>(InExportSettings->TargetSkeletonOrSkeletalMesh))
				{
			...
			{
				BodySkeleton = InExportSettings->SMPLSourceSkeletalMesh ? InExportSettings->SMPLSourceSkeletalMesh->GetSkeleton() : nullptr;
			}
			else
			{
				BodySkeleton = InExportSettings->SourceSkeletalMesh ? InExportSettings->SourceSkeletalMesh->GetSkeleton() : nullptr;
			}

			// Build reference pose map (bone name -> local transform) for gap filling
			TMap<FString, FTransform> RefPoseMap;
			if (BodySkeleton)
			{
				const FReferenceSkeleton& RefSkeleton = BodySkeleton->GetReferenceSkeleton();
				const TArray<FTransform>& RefPoses = BodySkeleton->GetRefLocalPoses();
				for (int32 BoneIdx = 0; BoneIdx < RefSkeleton.GetNum(); ++BoneIdx)
				{
					RefPoseMap.Add(RefSkeleton.GetBoneName(BoneIdx).ToString(), RefPoses[BoneIdx]);
				}
			}

			if (RefPoseMap.IsEmpty())
			{
				UE_LOGF(LogMetaHumanPerformance, Error, "Cannot export body animation: failed to build reference pose (skeleton is null or has no bones).");
				AnimationController.CloseBracket(bShouldTransact);
				return false;
			}

			const bool bUseRefPoseForUnsolved = (InExportSettings->BodyUnsolvedBehavior == EBodyUnsolvedFrameBehavior::NeutralPose);

			TMap<FString, TArray<FTransform>> BoneTransform;
			TArray<bool> SolvedFlags;

			if (InExportSettings->SourceSkeletalMesh && 
				InExportSettings->ExportSkeleton == EPerformanceExportSkeleton::ExistingSkeleton)
			{
				USkeletalMesh* TargetBodyMesh = Cast<USkeletalMesh>(InExportSettings->TargetSkeletonOrSkeletalMesh);

				FMetaHumanBodyAnimRetargeter Retargeter;

				if (InExportSettings->BodyRetargeter && TargetBodyMesh &&
					Retargeter.Initialize(InExportSettings->SourceSkeletalMesh, TargetBodyMesh, InExportSettings->BodyRetargeter))
				{
					TMap<FString, FTransform> RetargetedFrame;
					const float RetargetDeltaTime = static_cast<float>(1.0 / FrameRate.AsDecimal());

					BoneTransform = CollectBodyFrames(InPerformance, ExportFrameRange, RefPoseMap, bUseRefPoseForUnsolved,
						[&Retargeter, &RetargetedFrame, RetargetDeltaTime](const FFrameAnimationData& FrameAnimData) -> const TMap<FString, FTransform>*
						{
							if (FrameAnimData.RawBodyAnimationSMPLXShape.IsEmpty())
							{
								return nullptr;
							}
							RetargetedFrame.Reset();
							Retargeter.RetargetFrame(FrameAnimData.BodyAnimationData, RetargetedFrame, RetargetDeltaTime);
							return &RetargetedFrame;
						}, SolvedFlags);
				}
				else
				{
					UE_LOGF(LogMetaHumanPerformance, Warning, "Body retargeter failed to initialize.");
					AnimationController.CloseBracket(bShouldTransact);

					return false;
				}
			}
			else if (InExportSettings->SourceSkeletalMesh && 
					 InExportSettings->ExportSkeleton == EPerformanceExportSkeleton::PerformerSkeleton)
			{
				BoneTransform = CollectBodyFrames(InPerformance, ExportFrameRange, RefPoseMap, bUseRefPoseForUnsolved,
					[](const FFrameAnimationData& FrameAnimData) -> const TMap<FString, FTransform>*
					{
						return !FrameAnimData.RawBodyAnimationSMPLXShape.IsEmpty() ? &FrameAnimData.BodyAnimationData : nullptr;
					}, SolvedFlags);
			}
			else if (InExportSettings->SMPLSourceSkeletalMesh &&
					 InExportSettings->ExportSkeleton == EPerformanceExportSkeleton::Raw)
			{
				BoneTransform = CollectBodyFrames(InPerformance, ExportFrameRange, RefPoseMap, bUseRefPoseForUnsolved,
					[](const FFrameAnimationData& FrameAnimData) -> const TMap<FString, FTransform>*
					{
						return !FrameAnimData.RawBodyAnimationSMPLXShape.IsEmpty() ? &FrameAnimData.RawBodyAnimationSMPLXData : nullptr;
					}, SolvedFlags);
			}

			// Write bone tracks (quaternion-safe, no gimbal lock)
			for (const TPair<FString, TArray<FTransform>>& BoneTransformPair : BoneTransform)
			{
				TArray<FVector3f> Position;
				TArray<FQuat4f> Rotation;
				TArray<FVector3f> Scale;

				for (const FTransform& Transform : BoneTransformPair.Value)
				{
					Position.Add(FVector3f(Transform.GetLocation()));
					Rotation.Add(FQuat4f(Transform.GetRotation()));
					Scale.Add(FVector3f::OneVector);
				}

				AnimationController.AddBoneCurve(FName(BoneTransformPair.Key), bShouldTransact);
				AnimationController.SetBoneTrackKeys(FName(BoneTransformPair.Key), Position, Rotation, Scale, bShouldTransact);
			}
			// Write BodyFrameSolved metadata curve 1.0 for solved, 0.0 for filled
			{
				const FName SolvedCurveName(TEXT("BodyFrameSolved"));
				const FAnimationCurveIdentifier SolvedCurveId(SolvedCurveName, ERawCurveTrackTypes::RCT_Float);
				AnimationController.AddCurve(SolvedCurveId, AACF_Metadata, bShouldTransact);

				TArray<FRichCurveKey> SolvedKeys;
				SolvedKeys.Reserve(SolvedFlags.Num());
				for (int32 Idx = 0; Idx < SolvedFlags.Num(); ++Idx)
				{
					const float Time = static_cast<float>(Idx) / FrameRate.AsDecimal();
					SolvedKeys.Emplace(Time, SolvedFlags[Idx] ? 1.0f : 0.0f, 0.f, 0.f, ERichCurveInterpMode::RCIM_Constant);
				}

				AnimationController.SetCurveKeys(SolvedCurveId, SolvedKeys, bShouldTransact);
			}
		}
		...
		if (InExportSettings->bRemoveRedundantCurveKeys)
		{
		...
		if (InExportSettings->bExportBody)
		{
			...
			AnimSequence->SetRetargetSourceAsset(TargetBodyMesh);
		}
...
void UMetaHumanPerformanceExportUtils::PromptBodyCompatibilitySkeleton(USkeleton* InBodySkeleton)
{
	USkeleton* MetaHumanBaseSkeleton = FindMetaHumanBaseSkeleton();
	if (!MetaHumanBaseSkeleton)
	{
		return;
	}

	if (MetaHumanBaseSkeleton == InBodySkeleton)
	{
		return;
	}

	const FSoftObjectPath ExportedPath = FSoftObjectPath(InBodySkeleton);

	const bool bAlreadyCompatible = MetaHumanBaseSkeleton->GetCompatibleSkeletons().ContainsByPredicate(
		[&ExportedPath](const TSoftObjectPtr<USkeleton>& Entry)
		{
			return Entry.ToSoftObjectPath() == ExportedPath;
		});
	...
	const FText DialogText =
		FText::Format(LOCTEXT("AddMetaHumanCompatibilityMessage",
							  "A MetaHuman base skeleton was detected in this project in '{0}'.\n\n"
							  "Would you like to register the exported skeleton as compatible with it, "
							  "so the resulting animation can be played on MetaHuman characters?"), FText::FromString(FSoftObjectPath(MetaHumanBaseSkeleton).ToString()));

	FSuppressableWarningDialog::FSetupInfo Info(
		DialogText,
		LOCTEXT("AddMetaHumanCompatibilityTitle", "Add MetaHuman Skeleton Compatibility?"),
		TEXT("SuppressAddMetaHumanCompatibility"));

	Info.ConfirmText = LOCTEXT("AddCompat_Yes", "Yes, add compatibility");
	Info.CancelText = LOCTEXT("AddCompat_No", "No");
	Info.CheckBoxText = LOCTEXT("AddCompat_Suppress", "Don't ask again");
	Info.DialogMode = FSuppressableWarningDialog::EMode::PersistUserResponse;
	Info.WrapMessageAt = 0.0f; // Avoid text wrapping

	FSuppressableWarningDialog Dialog { Info };
	const FSuppressableWarningDialog::EResult Result = Dialog.ShowModal();

	const bool bAddCompatibility =
		(Result == FSuppressableWarningDialog::Confirm);

	if (bAddCompatibility)
	{
		ISkeletonEditorModule& SkeletonEditorModule =
			FModuleManager::LoadModuleChecked<ISkeletonEditorModule>("SkeletonEditor");

		TSharedRef<IEditableSkeleton> EditableSkeleton =
			SkeletonEditorModule.CreateEditableSkeleton(MetaHumanBaseSkeleton);

		EditableSkeleton->AddCompatibleSkeleton(InBodySkeleton);

		UEditorAssetLibrary::SaveLoadedAsset(MetaHumanBaseSkeleton);
	}
}

bool UMetaHumanPerformanceExportUtils::CanExportFaceAnimation(const class UMetaHumanPerformance* InPerformance)
{
	return InPerformance->ContainsAnimationDataType(EFrameAnimationDataType::Face);
}

bool UMetaHumanPerformanceExportUtils::CanExportBodyAnimation(const class UMetaHumanPerformance* InPerformance)
{
	return InPerformance->ContainsAnimationDataType(EFrameAnimationDataType::Body);
}
...
bool UMetaHumanPerformanceExportUtils::ApplyNeutralPoseCalibration(const UMetaHumanPerformance* InPerformance, const int32 InFrameNumber, FFrameAnimationData& InOutAnimationFrame)
{
	...
	if (InPerformance->bNeutralPoseCalibrationEnabled && !InPerformance->IsProcessing())
	{
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Warning, "Neutral pose calibration frame %i is not a solved frame", InPerformance->NeutralPoseCalibrationFrame);
			return false;
		}
		...
		for (const FName& Curve : InPerformance->NeutralPoseCalibrationCurves)
		{
			...
			{
				UE_LOGF(LogMetaHumanPerformance, Warning, "Neutral pose calibration curve '%ls' not found in neutral pose calibration frame %i", *Curve.ToString(), InPerformance->NeutralPoseCalibrationFrame);
				return false;
			}
		}
		...
		for (const FName& Curve : InPerformance->NeutralPoseCalibrationCurves)
		{
			...
			{
				UE_LOGF(LogMetaHumanPerformance, Warning, "Neutral pose calibration curve '%ls' not found in frame %i", *Curve.ToString(), InFrameNumber);
				return false;
			}
		}
		...
		{
			UE_LOGF(LogMetaHumanPerformance, Warning, "Neutral pose calibration failed for frame %i", InFrameNumber);
			return false;
		}
