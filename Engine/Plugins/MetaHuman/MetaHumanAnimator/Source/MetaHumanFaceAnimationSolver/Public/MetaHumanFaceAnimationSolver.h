...
public:
	...
	UE_API FString GetSolverHierarchicalDefinitionsPlusChinCompressData(class UCaptureData* InCaptureData = nullptr) const;
	...
	static UE_API FString SetEasyToEditControlConstraints(const FString& InSolverConfigData);

private:

	UE_API class UMetaHumanConfig* GetEffectiveConfig(class UCaptureData* InCaptureData) const;

	static UE_API FString JsonObjectAsString(TSharedPtr<class FJsonObject> InJsonObject);
