...
TSharedPtr<UE::NNE::IModelInstanceCPU> FSpeech2FaceInternal::TryLoadModelData(const FSoftObjectPath& InModelAssetPath)
{
	...
	{
		check(false);
		UE_LOGF(LogSpeech2FaceSolver, Error, "Failed to load model, it is invalid (nullptr)");
		return nullptr;
	}

	if (!FModuleManager::Get().LoadModule(TEXT("NNERuntimeORT")))
	{
		UE_LOGF(LogSpeech2FaceSolver, Error, "Failed to load model, could not load NNE Runtime module (NNERuntimeORT): %ls", *ModelData->GetPathName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogSpeech2FaceSolver, Error, "Failed to load model, could not load NNE Runtime: %ls", *ModelData->GetPathName());
		return nullptr;
	}
	...
	if (!ModelCpu.IsValid())
	{
		UE_LOGF(LogSpeech2FaceSolver, Error, "Failed to load model, could not create model CPU: %ls", *ModelData->GetPathName());
		return nullptr;
	}
	...
	{
		UE_LOGF(LogSpeech2FaceSolver, Display, "Loaded model: %ls", *ModelData->GetPathName());
	}
	else
	{
		UE_LOGF(LogSpeech2FaceSolver, Error, "Failed to load model, could not create model instance: %ls", *ModelData->GetPathName());
	}
	...
	TArray<FSpeech2Face::FAnimationFrame>& OutHeadAnimation) const
{
	...
	{
		UE_LOGF(LogSpeech2FaceSolver, Error, "Could not get imported PCM data for SoundWave %ls", *InAudioParams.SpeechRecording->GetName());
		return false;
	}
	...
	// Prepare audio
	UE_LOGF(LogSpeech2FaceSolver, Log, "Preparing samples for solve");
	...
	UE_LOGF(LogSpeech2FaceSolver, Log, "Extracting audio features");
	...
	UE_LOGF(LogSpeech2FaceSolver, Log, "Running predictor");
	...
	{
		UE_LOGF(LogSpeech2FaceSolver, Log, "Copying samples");
	...
	{
		UE_LOGF(LogSpeech2FaceSolver, Log, "Resampling");
	...
	UE_LOGF(LogSpeech2FaceSolver, Log, "Sound Wave Processing Complete");
	return true;
}
...
bool FSpeech2FaceInternal::ExtractAudioFeatures(const FloatSamples& Samples, const TSharedPtr<UE::NNE::IModelInstanceCPU>& AudioExtractor, TArray<float>& OutAudioData)
{
	...
	for (int32 SampleIndex = 0; SampleIndex < Samples.Num(); SampleIndex += RigLogicPredictorMaxAudioSamples)
	{
		...
		{
			UE_LOGF(LogSpeech2FaceSolver, Error, "Could not set the audio extractor input tensor shapes");
			return false;
		}
		...
		{
			UE_LOGF(LogSpeech2FaceSolver, Error, "The audio extractor NNE model failed to execute");
			return false;
		}
...
) const
{
	...
	{
		UE_LOGF(LogSpeech2FaceSolver, Error, "The rig logic model failed to execute");
		return false;
	}
...
bool FSpeech2FaceInternal::GetFloatSamples(const TWeakObjectPtr<const USoundWave>& SoundWave, const TArray<uint8>& PcmData, uint32 SampleRate, bool bDownmixChannels, uint32 ChannelToUse, float SecondsToSkip, FloatSamples& OutSamples)
{
	...
	{
		UE_LOGF(LogSpeech2FaceSolver, Error, "Could not get float samples with %d skipped samples from %d samples for SoundWave %ls", TotalSamplesToSkip, TotalSampleCount, *SoundWave->GetName());
		return false;
	}
	...
	if (SampleRate != AudioEncoderSampleRateHz)
	{
		...
		{
			UE_LOGF(LogSpeech2FaceSolver, Error, "Could not resample audio from %d to %d for SoundWave %ls", SampleRate, AudioEncoderSampleRateHz, *SoundWave->GetName());
			return false;
		}
