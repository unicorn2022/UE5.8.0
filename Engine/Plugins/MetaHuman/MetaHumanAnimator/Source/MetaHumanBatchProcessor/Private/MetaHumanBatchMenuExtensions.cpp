...
namespace MetaHumanBatchMenuExtension
{
	...
	/** Can export from multiple performances if they are all the same time and processed */
	bool CanExportFromPerformance(const FToolMenuContext& InContext)
	{
		...
		if (const UContentBrowserAssetContextMenuContext* Context = UContentBrowserAssetContextMenuContext::FindContextWithAssets(InContext))
		{
			...
			bCanExport = !SelectedAssets.IsEmpty();

			EDataInputType InputType = EDataInputType::DepthFootage; // These need defaults, although never actually used, to satisfy compiler warning!
			bool bFaceTracking = false;
			bool bBodyTracking = false;

			for (int32 Index = 0; Index < SelectedAssets.Num() && bCanExport; ++Index)
			{
				if (UMetaHumanPerformance* Performance = Cast<UMetaHumanPerformance>(SelectedAssets[Index].GetAsset()))
				{
					if (Index == 0)
					{
						InputType = Performance->InputType;
						bFaceTracking = Performance->bFaceTracking;
						bBodyTracking = Performance->bBodyTracking;
					}

					bCanExport = Performance->CanExportAnimation() && Performance->InputType == InputType && Performance->bFaceTracking == bFaceTracking && Performance->bBodyTracking == bBodyTracking;

					bCanExport = Performance->InputType == EDataInputType::Audio; // Only allow audio performances for now. Allowing other types will require an overhaul of the export settings used
				}
				else
				{
					bCanExport = false;
	...
	void FillPerformanceExportSubMenu(UToolMenu* Menu)
	{
		...
		Section.AddMenuEntry("MetaHumanPerformance_ExportAS", 
			...
			LOCTEXT("MetaHumanPerformance_ExportASTooltip", "Export animation sequences from processed audio performance"),
			FSlateIcon(TEXT("MetaHumanPerformanceStyle"), TEXT("Performance.ExportAnimation"), TEXT("Performance.ExportAnimation")),
			ExportASAction);
		...
		Section.AddMenuEntry("MetaHumanPerformance_ExportLS", 
			...
			LOCTEXT("MetaHumanPerformance_ExportLSTooltip", "Export level sequences from processed audio performance"),
			FSlateIcon(TEXT("MetaHumanPerformanceStyle"), TEXT("Performance.ExportLevelSequence"), TEXT("Performance.ExportLevelSequence")),
			ExportLSAction);
	}
}
