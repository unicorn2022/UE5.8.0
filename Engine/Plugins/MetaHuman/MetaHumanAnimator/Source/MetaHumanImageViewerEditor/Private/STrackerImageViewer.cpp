...
void STrackerImageViewer::Construct(const FArguments& InArgs)
{
	...
	DefaultCurvesColor = InArgs._DefaultCurvesColor;
	DefaultPointsColor = InArgs._DefaultPointsColor;
	HighlightedColor = InArgs._HighlightedColor;
	SelectedColor = InArgs._SelectedColor;
	DeactivatedColor = InArgs._DeactivatedColor;
...
bool STrackerImageViewer::ContainsData() const
{
	bool bContainsData = false;

	const UMetaHumanContourData* ContourData = CurveDataController.IsValid() ? CurveDataController->GetContourData() : nullptr;
	if (ContourData)
	{
		bContainsData = ContourData->FrameTrackingContourData.ContainsData();
	}
...
void STrackerImageViewer::SetCurvesColor(FLinearColor InColor)
{
	DefaultCurvesColor = InColor;
}

void STrackerImageViewer::SetPointsColor(FLinearColor InColor)
{
	DefaultPointsColor = InColor;
}

void STrackerImageViewer::SetPointsSize(float InSize)
{
	PointSize = InSize;
}
...
FLinearColor STrackerImageViewer::GetPointColor(const int32 InPointID) const
{
	...
	FLinearColor Color = DefaultPointsColor;
...
{
	FLinearColor Color = DefaultCurvesColor;
...
void STrackerImageViewer::DrawControlVertices(FSlateWindowElementList& OutDrawElements, int32 InLayerId, const FGeometry& AllottedGeometry) const
{
	...
	for (const FControlVertex& Point : ViewState.ControlVerticesForDraw)
	{
		...
		FSlateDrawElement::MakeLines(OutDrawElements, InLayerId, MyGeometry, Point.LinePoints, ESlateDrawEffect::None, Color, true, 2.f);
	}
}
...
void STrackerImageViewer::DrawTrackingCurves(FSlateWindowElementList& OutDrawElements, int32 InLayerId, const FGeometry& AllottedGeometry) const
{
	...
	for(const TPair<FString, TArray<FVector2D>>& Points : ViewState.SplineDensePoints)
	{
		if(Points.Value.Num() > 1)
		{
			...
			FSlateDrawElement::MakeLines(OutDrawElements, InLayerId, MyGeometry, Points.Value, ESlateDrawEffect::None, Color, true, 1.5f);
		}
	}
}
...
bool STrackerImageViewer::SetManipulationStateForMouseClick(const FPointerEvent& InMouseEvent, const FVector2D& InMousePos)
{
	...
	bool bMovingPoints = bClickedOnSelectedPoint && !InMouseEvent.IsControlDown() && !InMouseEvent.GetModifierKeys().AnyModifiersDown();
	bool bMovingCurves = bClickedOnSelectedCurve && !InMouseEvent.IsControlDown() && !InMouseEvent.GetModifierKeys().AnyModifiersDown();
	bool bMovingSelectionMarque = (!bMovingPoints && !bMovingCurves) && InMouseEvent.IsShiftDown();
	...
	return bMovingPoints || bMovingCurves || bMovingSelectionMarque;
}
