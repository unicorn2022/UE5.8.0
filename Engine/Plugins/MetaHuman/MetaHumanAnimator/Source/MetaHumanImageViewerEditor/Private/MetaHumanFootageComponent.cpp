...
UMetaHumanFootageComponent::UMetaHumanFootageComponent()
{
	...
	// Material creation requires the game thread (creates UMaterial via NewObject).
	// During async loading of old identity assets, this constructor runs on the
	// async loading thread. PostLoad handles material creation for that case.
	if (IsInGameThread())
	{
...
void UMetaHumanFootageComponent::PostLoad()
{
	...
	// CreateMovieContourDepthMaterial returns a transient material, so when loading
	// this component check to see if the materials are valid. Material creation
	// requires the game thread, so defer if we're on the async loading thread.
	if (IsInGameThread())
	{
		CreateFootageMaterialInstances();
	}
	else
	{
		...
		{
			if (UMetaHumanFootageComponent* StrongThis = WeakThis.Get())
			{
				StrongThis->CreateFootageMaterialInstances();
			}
		});
	}
