...
void FIdentityTreeNode::UpdateSceneComponentInstanceProperty(FProperty* InProperty)
{
	if (InProperty != nullptr && PreviewSceneComponent.IsValid() && PreviewSceneComponentInstance.IsValid())
	{
		...
		{
			UE_LOGF(LogMetaHumanIdentity, Error, "UpdateSceneComponentInstanceProperty called with a property named '%ls' that doesn't exist in class %ls", *InProperty->GetFName().ToString(), *PreviewSceneComponent->GetClass()->GetName());
		}
	}
}
...
void SMetaHumanIdentityPartsEditor::AddPartsFromAsset(UObject* InAsset, bool bInIsInputConformed)
{
	if (InAsset != nullptr)
	{
		...
		if (bInIsInputConformed)
		{
			...
			if (ETargetTemplateCompatibility::PotentialDegenerateTriangles == Compatibility)
			{
				FSuppressableWarningDialog::FSetupInfo Info(
					LOCTEXT("DegenerateTrianglesHintMessage", "The mesh contains potential degenerate (zero-area) triangles which may cause autorig to fail."),
					LOCTEXT("DegenerateTrianglesHintTitle", "Add Components From Conformed Mesh Warning"),
					TEXT("SuppressDegenerateTrianglesHint"));
				Info.ConfirmText = FDialogButtonTexts::Get().Ok;

				FSuppressableWarningDialog DegenerateTrianglesDialog{ Info };
				DegenerateTrianglesDialog.ShowModal();
			}
			else if (ETargetTemplateCompatibility::Valid != Compatibility)
			{
		...
		if (InAsset->IsA<UStaticMesh>() || InAsset->IsA<USkeletalMesh>())
		{
			...
			{
				UE_LOGF(LogMetaHumanIdentity, Error, "Unable to create a CaptureData from mesh of type '%ls'. It should be either a Static or Skeletal mesh", *InAsset->GetClass()->GetName());
			}
		}
		...
		if (CaptureData != nullptr)
		{
			...
			if (bInIsInputConformed)
			{
				...
				{			
					const bool bIsError = UMetaHumanIdentity::HandleError(Conformed);
					if (bIsError)
					{
						return;
					}
		...
		{
			UE_LOGF(LogMetaHumanIdentity, Error, "Unable to create a Face from mesh of type '%ls'. It should be either a Static or Skeletal mesh", *InAsset->GetClass()->GetName());
		}
	}
	else
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Error creating Components from asset. Asset is not valid");
	}
}
...
void SMetaHumanIdentityPartsEditor::HandleAddIdentityPartOfClass(TSubclassOf<UMetaHumanIdentityPart> InIdentityPartClass)
{
	...
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Trying to add a Part that the MetaHuman Identity already has: '%ls'", *InIdentityPartClass->GetName());
	}
}
...
void SMetaHumanIdentityPartsEditor::HandleAddIdentityPoseOfClass(TSubclassOf<UMetaHumanIdentityPose> InIdentityPose, EIdentityPoseType InPoseType)
{
	...
	if (Identity->CanAddPoseOfClass(InIdentityPose, InPoseType))
	{
		if (UMetaHumanIdentityFace* FacePart = Identity->FindPartOfClass<UMetaHumanIdentityFace>())
		{
			...
			{
				UE_LOGF(LogMetaHumanIdentity, Error, "Failed to find the Face node to add the new Pose to.");
			}
		}
		else
		{
			UE_LOGF(LogMetaHumanIdentity, Error, "Trying to add a Pose that the MetaHuman Identity already has: '%ls' of type '%ls'", *InIdentityPose->GetName(), *UMetaHumanIdentityPose::PoseTypeAsString(InPoseType));
		}
	}
}
...
void SMetaHumanIdentityPartsEditor::HandleIdentityTreeDeleteSelectedNode()
{
	...
	if (SelectedItems.Num() == 1)
	{
		...
		if (Node->IdentityPart.IsValid())
		{
			...
			{
				UE_LOGF(LogMetaHumanIdentity, Error, "Failed to remove MetaHuman Identity Part '%ls'", *IdentityPart->GetPartName().ToString())
			}
		}
		else if (Node->IdentityPose.IsValid())
		{
			if (TSharedPtr<FIdentityTreeNode> FaceNode = FindIdentityPartNodeByClass(UMetaHumanIdentityFace::StaticClass(), GetIdentityRootNode()))
			{
				if (UMetaHumanIdentityFace* FacePart = Cast<UMetaHumanIdentityFace>(FaceNode->IdentityPart.Get()))
				{
					...
					{
						UE_LOGF(LogMetaHumanIdentity, Error, "Failed to remove MetaHuman Identity Pose '%ls'", *Pose->PoseName.ToString())
					}
				}
			}
		}
