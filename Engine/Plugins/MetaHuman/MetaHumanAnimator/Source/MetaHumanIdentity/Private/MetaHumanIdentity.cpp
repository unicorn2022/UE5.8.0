...
void UMetaHumanIdentity::PostInitProperties()
{
...
EIdentityErrorCode UMetaHumanIdentity::ImportDNA(TSharedPtr<IDNAReader> InDNAReader, const TArray<uint8>& InBrowsBuffer)
{
	if (UMetaHumanIdentityFace* Face = FindPartOfClass<UMetaHumanIdentityFace>())
	{
		...
		{
			UE_LOGF(LogMetaHumanIdentity, Error, "Provided DNA is incompatible with the Face archetype:\n%ls", *CompatibilityMsg);

			return EIdentityErrorCode::InCompatibleDNA;
		}

		const EIdentityErrorCode AppliedDNA = Face->ApplyDNAToRig(InDNAReader);
		if (AppliedDNA != EIdentityErrorCode::None && !IsIdentityErrorCodeWarningOnly(AppliedDNA))
		{
			return AppliedDNA;
		}
...
EIdentityErrorCode UMetaHumanIdentity::ImportDNAFile(const FString& InDNAFilePath, EDNADataLayer InDnaDataLayer, const FString& InBrowsFilePath)
{
	if (TSharedPtr<IDNAReader> DNAReader = ReadDNAFromFile(*InDNAFilePath, InDnaDataLayer))
	{
		...
		{
			UE_LOGF(LogMetaHumanIdentity, Display, "Applying DNA from files '%ls' '%ls'", *InDNAFilePath, *InBrowsFilePath);
	...
	UMetaHumanIdentityPose* InPose, UMetaHumanIdentityPromotedFrame* InPromotedFrame, bool bInShowProgress)
{
	...
	if (bBlockingProcessing)
	{
		...
		if (!bTrackersLoaded)
		{
			UE_LOGF(LogMetaHumanIdentity, Warning, "Failed to load trackers");
		}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (!bSetTrackersSuccessfully)
	{
		...
		UE_LOGF(LogMetaHumanIdentity, Error, "%ls", *GenericTracker->GetErrorMessage());
	}
	...
	TSharedPtr<bool> bDepthDiagnosticsPopulated = MakeShared<bool>(false);

	OnFrameComplete.AddLambda([this, InPromotedFrame, TrackingResultsPinName, bInSkipDiagnostics, InDepthFramePath, DepthMapDiagnosticsResultsPinName, bDepthDiagnosticsPopulated](const TSharedPtr<UE::MetaHuman::Pipeline::FPipelineData> InPipelineData)
	{
		...
		if (!bInSkipDiagnostics && !InDepthFramePath.IsEmpty())
		{
			if (InPipelineData->HasData<TMap<FString, FDepthMapDiagnosticsResult>>(DepthMapDiagnosticsResultsPinName))
			{
				...
				*bDepthDiagnosticsPopulated = true;
			}
			else
			{
				UE_LOGF(LogMetaHumanIdentity, Warning, "Depth diagnostics data missing — depth calibration may be absent from capture data");
			}
		}
	});

	OnProcessComplete.AddLambda([this, bInSkipDiagnostics, InDepthFramePath, InPromotedFrame, bDepthDiagnosticsPopulated](TSharedPtr<UE::MetaHuman::Pipeline::FPipelineData> InPipelineData)
	{

		if (InPipelineData->GetExitStatus() != UE::MetaHuman::Pipeline::EPipelineExitStatus::Ok)
		{
			const FString& ErrorNodeMessage = InPipelineData->GetErrorNodeMessage();
			UE_LOGF(LogMetaHumanIdentity, Warning, "Tracking process failed: %ls", *ErrorNodeMessage);
			FText DialogMessage = ErrorNodeMessage.IsEmpty()
				? LOCTEXT("PromotedFrameProcessingFailed", "The promoted frame processing failed.")
				: FText::Format(LOCTEXT("PromotedFrameProcessingFailedDetailed", "The promoted frame processing failed.\n\n{0}"), FText::FromString(ErrorNodeMessage));
			FMessageDialog::Open(EAppMsgType::Ok, DialogMessage, LOCTEXT("PromotedFrameProcessingFailedTitle", "Promoted frame processing failed"));
		}
		else if (!bInSkipDiagnostics && !InDepthFramePath.IsEmpty()) 
		{
			if (*bDepthDiagnosticsPopulated)
			{
				UMetaHumanIdentityFace* Face = FindPartOfClass<UMetaHumanIdentityFace>();
				FText DiagnosticsWarningMessage;
				if (Face && InPromotedFrame->DiagnosticsIndicatesProcessingIssue(Face->MinimumDepthMapFaceCoverage, Face->MinimumDepthMapFaceWidth,  DiagnosticsWarningMessage))
				{
					...
					UE_LOGF(LogMetaHumanIdentity, Warning, "The frame contour tracking diagnostics check found a potential issue with the data: %ls", *DiagnosticsWarningMessage.ToString());
				}
			}
			else
			{
				UE_LOGF(LogMetaHumanIdentity, Warning, "Depth diagnostics did not produce results. The camera calibration may be missing a depth calibration entry.");
				FMessageDialog::Open(EAppMsgType::Ok,
					LOCTEXT("DepthCalibrationMissing", "Depth footage is present but depth diagnostics could not be performed.\n\nThe camera calibration asset may be missing the depth calibration entry. Depth-related quality checks have been skipped."),
					LOCTEXT("DepthCalibrationMissingTitle", "Depth Diagnostics Unavailable"));
			}
		}
...
void UMetaHumanIdentity::CreateDNAForIdentity(bool bInLogOnly)
{
		...
		bInLogOnly))
	{
		...
		// TODO
		//Params.RigType = UE::MetaHuman::ERigType::JointsAndBlendshapes;
		//Params.RigRefinementLevel = UE::MetaHuman::ERigRefinementLevel::Medium;
		...
		Request->AutorigRequestCompleteDelegate.BindLambda([this, bInLogOnly](const UE::MetaHuman::FAutorigResponse& InResponse)
		{
			...
			UE_LOGF(LogMetaHumanIdentity, Display, "Autorigging service executed");
...
void UMetaHumanIdentity::AutoRigSolveFinished(bool bSuccess, bool bInLogOnly)
{
	...
	if (bSuccess) 
	{
		...
		{
			UE_LOGF(LogMetaHumanIdentity, Warning, "The Identity creation diagnostics check found a potential issue with the data: %ls", *DiagnosticsWarningMessage.ToString());
		}
	}
	...
	else if (!bSuccess)
	{
		...
		UE_LOGF(LogMetaHumanIdentity, Error, "Autorigging service parse bytes error: '%ls'", *ErrorText.ToString());
...
bool UMetaHumanIdentity::HandleError(EIdentityErrorCode InErrorCode, bool bInLogOnly)
{
	...
	case EIdentityErrorCode::BrowsProjectionWarning:
		Message = LOCTEXT("BrowsProjectionWarning", "Some brow annotations project to outside the brows region on the conformed mesh."
			"\nWe recommend that you edit the brow landmarks and re-solve to correct this.");
		break;
	...
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "%ls", *Message.ToString());

		if (!bInLogOnly)
		{
			if (!IsIdentityErrorCodeWarningOnly(InErrorCode))
			{
			...
			{
				const FText ErrorDialogTitle = LOCTEXT("IdentityWarning", "MetaHuman Identity Warning");
				FMessageDialog::Open(EAppMsgType::Ok, Message, ErrorDialogTitle);
			}
		}
...
void UMetaHumanIdentity::HandleAutoRigServiceError(EMetaHumanServiceRequestResult InServiceError, bool bInLogOnly)
{
	...
	UE_LOGF(LogMetaHumanIdentity, Error, "Autorigging service returned an error: '%ls'", *ErrorText.ToString());
	if (!bInLogOnly)
	{
		FMessageDialog::Open(EAppMsgType::Ok, ErrorText, UMetaHumanIdentity::AutoRigServiceTitleError);
	}
