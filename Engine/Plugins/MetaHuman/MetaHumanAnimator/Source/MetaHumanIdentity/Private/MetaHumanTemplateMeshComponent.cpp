...
void UMetaHumanTemplateMeshComponent::LoadMeshAssets()
{
	...
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Failed to create a template mesh from the dna file:  %ls", *PathToDNA);
	}
}
...
void UMetaHumanTemplateMeshComponent::SetPoseHeadMeshVertices(EIdentityPoseType InPoseType, TConstArrayView<FVector3f> InNewVertices, ETemplateVertexConversion InConversionType) const
{
	...
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Mismatch in number of vertices when setting mesh for %ls pose. %d vertices provided but %d are expected", *UMetaHumanIdentityPose::PoseTypeAsString(InPoseType), InNewVertices.Num(), PoseHeadMesh->GetMeshRef().VertexCount());
	}
}
...
void UMetaHumanTemplateMeshComponent::SetEyeMeshesVertices(TConstArrayView<FVector3f> InLeftEyeVertices, TConstArrayView<FVector3f> InRightEyeVertices, ETemplateVertexConversion InConversionType)
{
	...
	{
		UE_LOGF(LogMetaHumanIdentity,
			   Error,
			   "Mismatch in number of vertices when setting mesh for eyes. Expected %d for left eye but %d provided. Expected %d for right eye but %d provided",
...
void UMetaHumanTemplateMeshComponent::SetTeethMeshVertices(TConstArrayView<FVector3f> InNewVertices, ETemplateVertexConversion InConversionType) const
{
	...
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Mismatch in number of vertices when setting mesh for teeth. %d vertices provided but %d are expected", InNewVertices.Num(), FittedTeethMesh->GetMeshRef().VertexCount());
	}
}
