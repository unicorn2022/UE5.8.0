...
FLandmarkConfigIdentityHelper::FLandmarkConfigIdentityHelper()
{
	...
	NeutralNoBrowsIrisPoseCurveExclusionList = NeutralPoseCurveExclusionList;
	NeutralNoBrowsIrisPoseCurveExclusionList.Append({"crv_brow_upper_l", "crv_brow_lower_l", "crv_brow_intermediate_l", "crv_brow_upper_r", "crv_brow_lower_r", "crv_brow_intermediate_r", "crv_iris_l", "crv_iris_r",});
	NeutralNoBrowsPoseGroupExclusionList = NeutralPoseGroupExclusionList;;
	NeutralNoBrowsPoseGroupExclusionList.Append({"brow_l", "brow_r"});

	if (!LoadCurvesAndLandmarksFromJson(TEXT("curves_config.json")) || !LoadGroupsFromJson(TEXT("curve_groups.json")))
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Failed to load curve data for the outliner");
	}
}
...
bool FLandmarkConfigIdentityHelper::LoadCurvesAndLandmarksFromJson(const FString& InFileName)
{
	...
	{
		if (const TSharedPtr<FJsonValue> ConfigDataEntry = JsonObject->GetFieldUntyped(TEXT("data")); ConfigDataEntry.IsValid())
		{
			const TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& ConfigCurveDataMap = ConfigDataEntry->AsObject()->Values;
			PopulateMarkerDataFromConfig(ConfigCurveDataMap);
		}
		else
		{
			UE_LOGF(LogMetaHumanIdentity, Error, "Unable to parse curves_config.json. The config does not contain data field");
		}
	}
	else
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "%ls", *JsonReader->GetErrorMessage());
		return false;
	}
...
bool FLandmarkConfigIdentityHelper::LoadGroupsFromJson(const FString& InFileName) const
{
	...
	{
		const TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Values = RootJsonObject->Values;
		MarkerDefs->GroupNames.Empty();
		for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Pair : Values)
		{
			const FString GroupName(Pair.Key);
...
void FLandmarkConfigIdentityHelper::PopulateMarkerDataFromConfig(const TMap<FJsonObject::FStringType, TSharedPtr<class FJsonValue>>& InConfigContourData)
{
	MarkerDefs = MakeShared<FMarkerDefs>();
	for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Pair : InConfigContourData)
	{
...
TArray<FMarkerCurveDef> FLandmarkConfigIdentityHelper::GetCurvesForPreset(const ECurvePresetType& InCurvePreset) const
{
	...
	{
		PresetCurves.RemoveAll([this](const FMarkerCurveDef& CurveDef)
		{
			return NeutralPoseCurveExclusionList.Contains(CurveDef.Name);
		});
	}
	else if (InCurvePreset == ECurvePresetType::Identity_TeethPose)
	{
		PresetCurves.RemoveAll([this](const FMarkerCurveDef& CurveDef)
		{
			return TeethPoseCurveExclusionList.Contains(CurveDef.Name);
		});
	}
	...
	else if (InCurvePreset == ECurvePresetType::NeutralPose_NoBrowsNoIris)
	{
		PresetCurves.RemoveAll([this](const FMarkerCurveDef& CurveDef)
		{
			return NeutralNoBrowsIrisPoseCurveExclusionList.Contains(CurveDef.Name);
		});
	}
...
TArray<FString> FLandmarkConfigIdentityHelper::GetGroupListForSelectedPreset(const ECurvePresetType& InSelectedPreset) const
{
	...
	else if (InSelectedPreset == ECurvePresetType::NeutralPose_NoBrowsNoIris)
	{
		for (const FString& Curve : NeutralNoBrowsPoseGroupExclusionList)
		{
			if (GroupsForPose.Contains(Curve))
			{
				GroupsForPose.Remove(Curve);
