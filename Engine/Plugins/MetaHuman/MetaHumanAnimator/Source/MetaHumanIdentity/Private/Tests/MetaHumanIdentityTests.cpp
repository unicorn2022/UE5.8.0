...
bool FMetaHumanIdentityTest::RunTest(const FString& InTestCommand)
{
	...
	if (InTestCommand == "Pose")
	{
		...
		UMetaHumanFaceContourTrackerAsset* DefaultTracker = UMetaHumanFaceContourTrackerAsset::LoadDefaultTracker();
	...
	else if (InTestCommand == "Face")
	{
		...
		UTEST_FALSE("CaptureData is not initialized by default", CaptureData->IsInitialized(ECaptureDataInitializedCheck::Full));
		...
		UTEST_TRUE("CaptureData should be valid with a TargetMesh set", CaptureData->IsInitialized(ECaptureDataInitializedCheck::Full));
