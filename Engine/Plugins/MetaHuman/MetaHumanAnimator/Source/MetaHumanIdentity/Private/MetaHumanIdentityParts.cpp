	...
	if (FProperty* Property = InPropertyChangedEvent.Property)
	{
		...
		if (bDefaultSolverChanged)
		{
			...
			if (DefaultSolver)
			{
				...
				// update the predictive solvers and face fitting solver
				LoadDefaultFaceFittingSolvers();
			}
		}
	}
...
EIdentityErrorCode UMetaHumanIdentityFace::Conform(EConformType InConformType)
{
	...
	if (UMetaHumanIdentityPose* NeutralPose = FindPoseByType(EIdentityPoseType::Neutral))
	{
		...
		if (InConformType != EConformType::Copy)
		{
			...
			bIsConformed = false;
			float FractionLandmarksInBrowRegion = 0;
			...
			for (int32 FrameIndex = 0; FrameIndex < PromotedFrames.Num(); ++FrameIndex)
			{
				...
				if (PromotedFrame->bIsFrontView)
				{
					...
					bIsConformed = Conformer.GenerateBrowMeshLandmarks(CombineFrameNameAndCameraViewName(GetFrameNameForConforming(PromotedFrame, FrameIndex), NeutralPose->Camera), BrowsBuffer, FractionLandmarksInBrowRegion);
			...
			if (FractionLandmarksInBrowRegion < (1.0f - MaximumInvalidBrowAnnotations / 100.0f) && ! bSkipDiagnostics)
			{
				UE_LOGF(LogMetaHumanIdentity, Warning, "%f%% of tracked brow landmarks were not projected onto the face mesh, which is above the allowed threshold of %f%%; please consider editing the brow landmarks to correct this.", 
					100.0f - FractionLandmarksInBrowRegion * 100.0f, MaximumInvalidBrowAnnotations);
				return EIdentityErrorCode::BrowsProjectionWarning;
			}
		}

		UE_LOGF(LogMetaHumanIdentity, Display, "Conforming took %lf seconds", ConformDuration);
...
EIdentityErrorCode UMetaHumanIdentityFace::ApplyCombinedDNAToRig(TSharedPtr<class IDNAReader> InDNAReader)
{
	...
	SaveDNAToBuffer(InDNAReader.Get(), EDNADataLayer::All, RawDNABuffer);
...
bool UMetaHumanIdentityFace::CheckRigCompatible(FString& OutCompatibilityMsg) const
{
	...
	{	
		if(TSharedPtr<IDNAReader> DNAReader = USkelMeshDNAUtils::GetDNAReader(RigComponent->GetSkeletalMeshAsset()))
		{
			return CheckDNACompatible(DNAReader.Get(), OutCompatibilityMsg);
		}
	}
...
static TArray<uint8> GetBulkDataPayload(const UE::Serialization::FEditorBulkData& InBulkData)
{
	...
	if (InBulkData.HasPayloadData())
	{
		...
		{
			UE_LOGF(LogMetaHumanIdentity, Fatal, "Payload size too large");
			return PayloadData;
		}
...
void UMetaHumanIdentityFace::ApplyDNAToRigComponent(TSharedPtr<class IDNAReader>  InDNAReader, bool bInUpdateBlendShapes, bool bInUpdateSkinWeights)
{
	if (RigComponent != nullptr && RigComponent->GetSkeletalMeshAsset() != nullptr && RigComponent->GetSkeletalMeshAsset()->GetSkeleton() != nullptr)
	{
		...
		FInterchangeDnaModule::GetModule().CreateAndAttachDNAToSkeletalMesh(
			RigComponent->GetSkeletalMeshAsset(),
			InDNAReader);
		...
		Timer.Stop();
		UE_LOGF(LogMetaHumanIdentity, Display, "Apply DNA To Rig took %lf seconds", ApplyDNADuration);
	}
}
...
EIdentityErrorCode UMetaHumanIdentityFace::ApplyDNAToRig(TSharedPtr<class IDNAReader> InDNAReader, bool bInUpdateBlendShapes, bool bInUpdateSkinWeights)
{
	...
	TArray<uint8> RawDNABuffer;
	SaveDNAToBuffer(InDNAReader.Get(), EDNADataLayer::All, RawDNABuffer);
	SetDNABuffer(MoveTemp(RawDNABuffer));
...
bool UMetaHumanIdentityFace::GetPredictiveSolversTaskConfig(FPredictiveSolversTaskConfig& OutConfig) const
{
	...
	TSharedPtr<IDNAReader> DNAReader = USkelMeshDNAUtils::GetDNAReader(RigComponent->GetSkeletalMeshAsset());

	if (!DNAReader.IsValid())
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Face Skeletal Mesh doesn't have DNA asset attached.");
		return false;
	}
	...
	{
		UE_LOG(LogMetaHumanIdentity, Warning, TEXT("PredictiveSolver not available. Skipping predictive solver training data."));
		return false;
	}
...
bool UMetaHumanIdentityFace::RunPredictiveSolverTraining()
{
	...
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Unable to prepare Identity for Performance. Please make sure Depth Processing plugin is enabled. (Available on Fab)");
		return false;
	}
...
void UMetaHumanIdentityFace::LoadDefaultFaceFittingSolvers()
{
	...
	if (IModularFeatures::Get().IsModularFeatureAvailable(IPredictiveSolverInterface::GetModularFeatureName()))
	{
		...
		// PredictiveSolver is optional - it is only required for "Prepare for Performance" solver training,
		// and may not be available in commandlet or headless contexts. Log a warning rather than check()
		// so that DDC fill and other non-interactive workflows can proceed.
		if (!DefaultSolver->PredictiveSolver)
		{
			UE_LOG(LogMetaHumanIdentity, Warning, TEXT("PredictiveSolver not available. Predictive solver training will be unavailable."));
		}
	}
...
void UMetaHumanIdentityFace::CopyMeshVerticesFromExistingMesh(UCaptureData* CaptureData)
{
	if (UMeshCaptureData* MeshCaptureData = Cast<UMeshCaptureData>(CaptureData))
	{
		...
		else if (UStaticMesh* StaticMesh = Cast<UStaticMesh>(MeshCaptureData->TargetMesh))
		{
			...
			{
				UE_LOGF(LogMetaHumanIdentity, Error, "Mismatch in number of vertices when setting mesh for neutral pose. %d vertices provided but %d are expected", NewMesh.VertexCount(), ConformalMeshRef.VertexCount());
			}
		}
	}
}
...
ETargetTemplateCompatibility UMetaHumanIdentityFace::CheckTargetTemplateMesh(const UObject* InAsset)
{
	...
	auto HasDegenerateTriangles = [](const FMeshDescription* InMeshDescription) -> bool
	{
		if (!InMeshDescription)
		{
			return false;
		}
		const FStaticMeshConstAttributes Attributes(*InMeshDescription);
		TVertexAttributesConstRef<FVector3f> VertexPositions = Attributes.GetVertexPositions();
		for (const FTriangleID TriangleID : InMeshDescription->Triangles().GetElementIDs())
		{
			TConstArrayView<FVertexID> TriVerts = InMeshDescription->GetTriangleVertices(TriangleID);
			const float Area = UE::Geometry::VectorUtil::Area<float>(
				VertexPositions[TriVerts[0]],
				VertexPositions[TriVerts[1]],
				VertexPositions[TriVerts[2]]);
			if (FMath::IsNearlyZero(Area, FMathf::ZeroTolerance))
			{
				return true;
			}
		}
	...
	if (const USkeletalMesh* SkelMesh = Cast<const USkeletalMesh>(InAsset))
	{
		...
		bHasDegenerateTriangles = HasDegenerateTriangles(MeshDescription);
	}
	else if (const UStaticMesh* StaticMesh = Cast<const UStaticMesh>(InAsset))
	{
		const FMeshDescription* MeshDescription = StaticMesh->GetMeshDescription(Template2MHHeadMeshIndex);

		FDynamicMesh3 ImportedMesh;
		FMeshDescriptionToDynamicMesh DynamicMeshConverter;
		DynamicMeshConverter.Convert(MeshDescription, ImportedMesh);
	...
	{
		return ETargetTemplateCompatibility::PotentialDegenerateTriangles;
	}
...
USkeletalMesh* UMetaHumanIdentityFace::CreateFaceArchetypeSkelmesh(const FString& InNewRigAssetName, const FString& InNewRigPath)
{
	...
	if (FFileHelper::LoadFileToArray(DNADataAsBuffer, *FaceDNAPath))
	{
		...
		if (USkeletalMesh* SkelMeshAsset = DNAImportModule.ImportSync(InNewRigAssetName, InNewRigPath, FaceDnaReader, FaceSkel, DNAConfig))
		{
			DNAImportModule.CreateAndAttachDNAToSkeletalMesh(SkelMeshAsset, FaceDnaReader);
...
EIdentityErrorCode UMetaHumanIdentityFace::InitializeRig()
{
	...
	if (!IsConformalRigValid())
	{
		...
		{
			TSoftObjectPtr<UObject> FaceboardControlRigAsset = FMetaHumanCommonDataUtils::GetDefaultControlRigFromRegistry(FMetaHumanCommonDataUtils::GetAnimatorPluginFaceControlRigPath());
		...
		if (UDNA* DNAAsset = USkelMeshDNAUtils::GetMeshDNAAsset(RigSkeletalMesh))
		{
			DNAAsset->RestoreLegacyUEMHCCompatibility();
		}
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		else if (UDNAAsset* OldDNAAsset = USkelMeshDNAUtils::GetMeshDNA(RigSkeletalMesh))
		{
		...
		if (DNAReader)
		{
			SetDNABuffer(UE::Wrappers::FMetaHumanConformer::DNAToBuffer(DNAReader));
			...
			for (UMetaHumanIdentityPromotedFrame* PromotedFrame : ContourDataFramesFrontFirst)
			{
				if (UMetaHumanIdentityFootageFrame* FootageFrame = Cast<UMetaHumanIdentityFootageFrame>(PromotedFrame))
				{
					...
					if (FramePathResolver)
					{
						...
						UE_LOGF(LogMetaHumanIdentity, Log, "Resolved promoted frame (%d) to depth path %ls", FootageFrame->FrameNumber, *DepthImagePath);
						...
						{				
							UE_LOGF(LogMetaHumanIdentity, Warning, "Could not resolve depth frame path for promoted frame (%ls) during MetaHuman Identity Solve", *DepthImagePath);
						}
					}
					else
					{
						UE_LOGF(LogMetaHumanIdentity, Warning, "Could not resolve depth frame path for promoted frame number (%d) during MetaHuman Identity Solve", FootageFrame->FrameNumber);
					}
				}
			}
		}
...
bool UMetaHumanIdentityFace::SaveDebuggingData(UMetaHumanIdentityPose* InPose, UE::Wrappers::FMetaHumanConformer& OutConformer, const FString& InAssetSavedFolder) const
{
	...
	if (!PlatformFile.DirectoryExists(*DebuggingFolderBase))
	{
		...
		if (!bCreatedFolder)
		{
			UE_LOGF(LogMetaHumanIdentity, Warning, "Failed to create folder to save debugging data during mesh fitting");
			return false;
		}
	}
	...
	if (UFootageCaptureData* FootageCaptureData = Cast<UFootageCaptureData>(InPose->GetCaptureData()))
	{
		...
		if (!PlatformFile.DirectoryExists(*ImageFolder))
		{
			...
			if (!bCreatedFolder)
			{
				UE_LOGF(LogMetaHumanIdentity, Warning, "Failed to save debugging data (images) during mesh fitting");
				return false;
			}
		}
		...
		if (!PlatformFile.DirectoryExists(*DepthMapFolder))
		{
			...
			if (!bCreatedFolder)
			{
				UE_LOGF(LogMetaHumanIdentity, Warning, "Failed to save debugging data (depth-maps) during mesh fitting");
				return false;
			}
		}
...
EIdentityErrorCode UMetaHumanIdentityFace::RunMeshConformer(UMetaHumanIdentityPose* InPose, UE::Wrappers::FMetaHumanConformer& OutConformer)
{
	...
	{
		UE_LOGF(LogMetaHumanIdentity, Error, "Unable to fit the mesh");
		bIsConformed = false;
	}
