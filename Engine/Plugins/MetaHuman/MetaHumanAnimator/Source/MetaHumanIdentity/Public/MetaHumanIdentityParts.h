...
enum class ETargetTemplateCompatibility : uint8
{
	...
	PotentialDegenerateTriangles,	// Mesh has zero-area triangles which may fail to autorig (hint only)
...
public:
	...
	/* The maximum percentage of the brow annotations which may be outside the brows region of the head mesh. Above this value a diagnostic warning will be flagged. */
	UPROPERTY(EditAnywhere, Category = "Processing Diagnostics", meta = (ClampMin = 0.0, ClampMax = 100.0))
	float MaximumInvalidBrowAnnotations = 10.0f;
