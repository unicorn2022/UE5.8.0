...
FString FMetaHumanContourDataVersion::GetContourDataVersionString()
{
	...
	{
		if (const TSharedPtr<FJsonValue> ConfigDataEntry = JsonObject->GetFieldUntyped(TEXT("version")); ConfigDataEntry.IsValid())
		{
		...
		{
			UE_LOGF(LogMetaHumanCore, Error, "Unable to retrieve a version from curves_config.json. The config does not contain the version filed");
		}
	}
