...
static FTimecode ConvertLatestTimecodeToTargetRate(const TArray<TObjectPtr<UImgMediaSource>>& InImageSequences, const FFrameRate& InTargetRate)
{
	FFrameNumber LatestStartAtOutput(MIN_int32);

	for (const TObjectPtr<UImgMediaSource>& Cam : InImageSequences)
	{
		const FFrameNumber FrameAtCamRate = Cam->StartTimecode.ToFrameNumber(Cam->FrameRateOverride);
		const FFrameTime FrameAtOutput = FFrameRate::TransformTime(FFrameTime(FrameAtCamRate), Cam->FrameRateOverride, InTargetRate);

		if (FrameAtOutput.FrameNumber > LatestStartAtOutput)
		{
			LatestStartAtOutput = FrameAtOutput.FrameNumber;
		}
	}

	const bool bUseDropFrame = FTimecode::IsDropFormatTimecodeSupported(InTargetRate) && FTimecode::UseDropFormatTimecode(InTargetRate);
	return FTimecode::FromFrameNumber(LatestStartAtOutput, InTargetRate, bUseDropFrame);
}

static TValueOrError<TArray<int32>, FString> CalculateFrameOffset(const UFootageCaptureData* InFootageCaptureData)
{
	TArray<FString> DetectedFrameRates;

	TArray<DepthGenerator::FCameraTimecodeInfo> CameraTimecodeInfos;
	for (int32 CameraIndex = 0; CameraIndex < InFootageCaptureData->ImageSequences.Num(); ++CameraIndex)
	{
		UImgMediaSource* Camera = InFootageCaptureData->ImageSequences[CameraIndex];

		if (!Camera->FrameRateOverride.IsValid() || FMath::IsNearlyZero(Camera->FrameRateOverride.AsDecimal()))
		{
			FString Error = FString::Format(TEXT("Invalid frame rate detected for the camera {0}."), { Camera->GetName() });
			return MakeError(MoveTemp(Error));
		}

		DepthGenerator::FCameraTimecodeInfo TimecodeInfo;
		TimecodeInfo.Timecode = Camera->StartTimecode;
		TimecodeInfo.FrameRate = Camera->FrameRateOverride;

		FString FrameRate = Camera->FrameRateOverride.ToPrettyText().ToString();
		DetectedFrameRates.Add(FString::Format(TEXT("{0}: {1}"), { Camera->GetName(), FrameRate }));

		CameraTimecodeInfos.Add(MoveTemp(TimecodeInfo));
	}

	TArray<int32> Offsets = DepthGenerator::CalculateFrameOffset(CameraTimecodeInfos);
	if (Offsets.IsEmpty())
	{
		FString DetectedFrameRatesLog = FString::Join(DetectedFrameRates, TEXT(", "));

		FString Error = FString::Format(TEXT("Incompatible frame rates detected for the cameras - {0}"), { DetectedFrameRatesLog });
		return MakeError(MoveTemp(Error));
	}

	return MakeValue(MoveTemp(Offsets));
}

static int32 GetResizeDepthFactor(EMetaHumanCaptureDepthResolutionType InDepthResolution)
{
...
static void ShowErrorMessageDialog(const FString& InMessage)
{
	FTextBuilder TextBuilder;
	TextBuilder.AppendLineFormat(
		LOCTEXT("FailedDepthGeneration", "Depth generation process has failed to complete.\n\nReason: {0}"), FText::FromString(InMessage));

	FMessageDialog::Open(EAppMsgCategory::Error, EAppMsgType::Ok, TextBuilder.ToText());

	UE_LOGF(LogMetaHumanDepthGeneration, Error, "%ls", *InMessage);
}
...
static void ShowSuccessMessageDialog()
{
	...
	TextBuilder.AppendLine(
		LOCTEXT("SuccessfullDepthGeneration", "Depth process has completed successfully."));
...
public:
	...
	TValueOrError<FCameraCalibration, FString>
...
FDepthGenerator::RunGenerateDepthImagesPipeline(const UFootageCaptureData* InFootageCaptureData, const UCameraCalibration* InCameraCalibration)
{
	...
	{
		UE_LOGF(LogMetaHumanDepthGeneration, Display, "Detected image resolution %i x %i for first image found at %ls", ImageDimensions.X, ImageDimensions.Y, *FullSequencePath);
	}
	else
	{
		FString Message = FString::Printf(TEXT("Could not determine image resolution. Failed to load first image file found at %s."), *FullSequencePath);
	...
	if (ImageResolution > MaxStandardHMCImageSize)
	{
		...
		UE_LOGF(LogMetaHumanDepthGeneration, Warning, "Image resolution of %i is larger than the expected maximum size for the MetaHuman plugin (1536 x 2048). Image sequence ingest will be slow and may run out of graphics memory.", ImageResolution);
	}
	...
	TValueOrError<TArray<int32>, FString> OffsetsResult = CalculateFrameOffset(InFootageCaptureData);
	if (OffsetsResult.HasError())
	{
		FString Message = FString::Printf(TEXT("Could not determine alignment of image sequences: %s"), *OffsetsResult.GetError());
		return MakeError(MoveTemp(Message));
	}

	TArray<int32> Offsets = OffsetsResult.StealValue();
	...
	for (int32 ViewIndex = 0; ViewIndex < InFootageCaptureData->ImageSequences.Num(); ViewIndex++)
	{
		...
		TObjectPtr<UImgMediaSource> ImageSequence = InFootageCaptureData->ImageSequences[ViewIndex];
		FTrackingPathUtils::GetTrackingFilePathAndInfo(ImageSequence, ImageFilePath, FrameOffset, NumFrames);

		FrameOffset += Offsets[ViewIndex];
		...
		{
			TOptional<FCameraCalibration> CameraCalibration = GetCalibrationForView(CameraCalibrations, ImageSequence, ViewIndex);

			if (!CameraCalibration.IsSet())
			{
				FString Message = FString::Printf(TEXT("Could not determine calibration for the camera %s."), *ImageSequence->GetName());
				return MakeError(MoveTemp(Message));
			}

			GenerateDepth->Calibrations.Add(MoveTemp(CameraCalibration.GetValue()));
		}
	}
	...
	{
		UE_LOGF(LogMetaHumanDepthGeneration, Warning, "Not enough background threads available: required %i, available %i. The HMC ingest pipeline is going to run on a single thread",
	...
	if (PipelineOutput->GetExitStatus() != EPipelineExitStatus::Ok)
	{
		...
		FString Message = FString::Printf(TEXT("Pipeline error occurred: %s"), *ErrorMessage);
		return MakeError(MoveTemp(Message));
	}

	static constexpr int32 DepthCalibrationIndex = 2;
	if (!GenerateDepths->GetNodes()[0]->Calibrations.IsValidIndex(DepthCalibrationIndex))
	{
		FString Message = TEXT("Node failed to provide calibration for the depth camera.");
		return MakeError(MoveTemp(Message));
	}

	FCameraCalibration DepthCameraCalibration = GenerateDepths->GetNodes()[0]->Calibrations[DepthCalibrationIndex];
	...
	if (ResizeDepth.IsValid())
	{
		...
		if (DepthCameraCalibration.ImageSize.X < 640 || DepthCameraCalibration.ImageSize.Y < 360) // depth image is orientated on its side 
		{
			...
			UE_LOGF(LogMetaHumanDepthGeneration, Warning, "%ls", *Warning.ToString());
		}
	}
...
bool CreateCalibrationAsset(const FCameraCalibration& InCameraCalibration, const FString& InPackagePath, TObjectPtr<UCameraCalibration> OutCalibrationAsset)
{
	...
	{
		UE_LOGF(LogMetaHumanDepthGeneration, Warning, "Removed the previous depth camera calibration data");
	}
...
bool UMetaHumanDepthGenerator::Process(UFootageCaptureData* InFootageCaptureData)
{
	...
	{
		FString Message = FString::Printf(TEXT("Generating depth images is not possible on this footage. Expecting 2 image sequences, found %d"), InFootageCaptureData->ImageSequences.Num());
		UE::MetaHuman::Private::ShowErrorMessageDialog(Message);
	...
	{
		FString Message = TEXT("Image Sequences in the Footage Capture Data cannot be null.");
		UE::MetaHuman::Private::ShowErrorMessageDialog(Message);
...
bool UMetaHumanDepthGenerator::Process(UFootageCaptureData* InFootageCaptureData, UMetaHumanGenerateDepthWindowOptions* InOptions)
{
	...
	{
		FString Message = TEXT("Unable to generate depth images. Please make sure Depth Processing plugin is enabled. (Available on Fab)");
	...
	if (!InOptions->ReferenceCameraCalibration)
	{
		...
		{
			FString Message = TEXT("Generating depth images is not possible without a Camera Calibration");
			ShowErrorMessageDialog(Message);
	...
	{
		FString Message = TEXT("Provided calibration is invalid");
		ShowErrorMessageDialog(Message);
	...
	{
		FString Message = FString::Printf(TEXT("Depth generation requires one Camera Calibration per camera in the stereo pair. Expecting 2, found %d"), CameraCalibrationCount);
		ShowErrorMessageDialog(Message);
	...
	TValueOrError<FCameraCalibration, FString> Result =
	...
	if (Result.HasError())
	{
		...
		ShowErrorMessageDialog(Result.StealError());
	...
	{
		FString Message = TEXT("Unable to create image sequence asset for depth images");
		ShowErrorMessageDialog(Message);
	...
	UImgMediaSource* FrameRateMin = *Algo::MinElement(InFootageCaptureData->ImageSequences, [](const UImgMediaSource* InLeft, const UImgMediaSource* InRight)
	{
		TLess<double> Less;
		return Less(InLeft->FrameRateOverride.AsDecimal(), InRight->FrameRateOverride.AsDecimal());
	});

	DepthImageSequence->FrameRateOverride = FrameRateMin->FrameRateOverride;
	DepthImageSequence->StartTimecode = ConvertLatestTimecodeToTargetRate(InFootageCaptureData->ImageSequences, FrameRateMin->FrameRateOverride);

	InFootageCaptureData->DepthSequences.Add(DepthImageSequence);
	// Adding twice to match the image sequences
	InFootageCaptureData->DepthSequences.Add(DepthImageSequence);
