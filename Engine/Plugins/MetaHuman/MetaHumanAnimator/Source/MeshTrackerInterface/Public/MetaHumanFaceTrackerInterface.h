...
public:
	...
	UE_DEPRECATED(5.8, "Use LoadDNA function that accepts the reader instead, UDNAAsset has been deprecated.")
	virtual bool LoadDNA(class UDNAAsset* InDNAAsset) = 0;

	/** Load the DNA from a IDNAReader.@returns True if the DNA could be loaded. */
	virtual bool LoadDNA(TSharedPtr<class IDNAReader> InDNAReader) = 0;
...
public:
	...
	/**
	 * Load the DNA from a IDNAReader.
	 *
	 * @param[in] InDNAReader the DNA reader
	 * @param[in] InSolverDefinitionsJson: the string containing the solver definitions as json; a different config will be used for the standard solve than for the hierarchical solve
	 * @returns True if the DNA could be loaded.
	 */
	virtual bool LoadDNA(TSharedPtr<IDNAReader> InDNAReader, const FString& InSolverDefinitionsJson) = 0;
...
public:
	 ...
	 * @returns True if the DNA could be loaded.
	 */
	virtual bool LoadDNA(TSharedPtr<IDNAReader> InDNAReader, const FString& InSolverDefinitions) = 0;
