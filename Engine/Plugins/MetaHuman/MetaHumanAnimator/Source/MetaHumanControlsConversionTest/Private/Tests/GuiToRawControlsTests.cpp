...
bool FGuiToRawControlsConversionTest::RunTest(const FString& InParameters)
{
	...
	if (ConversionType == "FaceTracker")
	{
		...
		TObjectPtr<UDNA> DNAAsset = ReadDNAAssetFromFile(PathToDNA, GetTransientPackage());
		...
		UTEST_TRUE("Check dna load post process tracker", Tracker->LoadDNA(DNAAsset->GetDNAReader(), DeviceConfig->GetSolverHierarchicalDefinitionsData()));
