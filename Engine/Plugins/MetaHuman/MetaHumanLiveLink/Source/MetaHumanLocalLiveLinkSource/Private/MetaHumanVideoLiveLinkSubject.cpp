...
#else
	"UE",
	TEXT("Controls which media player is used. Options are \"UE\""),
#endif
	ECVF_Default
};
...
FMetaHumanVideoLiveLinkSubject::FMetaHumanVideoLiveLinkSubject(ILiveLinkClient* InLiveLinkClient, const FGuid& InSourceGuid, const FName& InSubjectName, UMetaHumanVideoLiveLinkSubjectSettings* InSettings) : FMetaHumanVideoBaseLiveLinkSubject(InLiveLinkClient, InSourceGuid, InSubjectName, InSettings)
{
	...
	else if (InSettings->MediaSourceCreateParams.VideoURL.StartsWith(UE::MetaHuman::Pipeline::FMediaPlayerNode::MediaSourceURL))
	{
		AnalyticsItems.Add(TEXT("DeviceModel"), TEXT("MediaSource"));

		MediaPlayer = MakeShared<UE::MetaHuman::Pipeline::FMediaPlayerUENode>("MediaPlayerUE");
	}
	else if (InSettings->MediaSourceCreateParams.VideoURL.StartsWith(UE::MetaHuman::Pipeline::FMediaPlayerNode::MediaProfileURL))
	{
		AnalyticsItems.Add(TEXT("DeviceModel"), TEXT("MediaProfile"));
	...
	else
	{
		...
		if (MediaPlayerType == "UE")
		{
			MediaPlayer = MakeShared<UE::MetaHuman::Pipeline::FMediaPlayerUENode>("MediaPlayerUE");
		}
#if PLATFORM_WINDOWS
		else if (MediaPlayerType == "WMF")
		{
		...
		{
			UE_LOGF(LogMetaHumanLocalLiveLinkSubject, Warning, "Unknown media player option: %ls", *MediaPlayerType);
	...
	UE_LOGF(LogMetaHumanLocalLiveLinkSubject, Display, "Using media player: %ls", *MediaPlayer->Name);
...
FMetaHumanVideoLiveLinkSubject::~FMetaHumanVideoLiveLinkSubject()
{
	...
	if (IsInGameThread())
	{
		...
		{
			UE_LOGF(LogMetaHumanLocalLiveLinkSubject, Warning, "Failed to close media player");
		}
	}
	else
	{ 
