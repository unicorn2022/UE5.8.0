...
void UMetaHumanAudioBaseLiveLinkSubjectSettings::OnUpdate(TSharedPtr<UE::MetaHuman::Pipeline::FPipelineData> InPipelineData)
{
	...
	if (Monitoring[EMetaHumanLocalLiveLinkSubjectMonitoring::Basic] > 0)
	{
		...
		if (ExitStatus == UE::MetaHuman::Pipeline::EPipelineExitStatus::Unknown)
		{
			...
			FTimecode PipelineTimecode = InPipelineData->GetData<FQualifiedFrameTime>(AudioSampleTimePin).ToTimecode();
	...
	if (Monitoring[EMetaHumanLocalLiveLinkSubjectMonitoring::Advanced] > 0)
	{
		const UE::MetaHuman::Pipeline::EPipelineExitStatus ExitStatus = InPipelineData->GetExitStatus();
		if (ExitStatus != UE::MetaHuman::Pipeline::EPipelineExitStatus::Unknown)
		{
