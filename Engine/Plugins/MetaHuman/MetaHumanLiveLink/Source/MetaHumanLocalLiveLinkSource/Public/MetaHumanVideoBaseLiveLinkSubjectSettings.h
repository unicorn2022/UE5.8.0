...
UENUM(BlueprintType)
enum class EMetaHumanVideoBaseLiveLinkSubjectState : uint8
{
	Unknown = 0,
	NoFaceDetected,
	SubjectTooFar,
...
public:
	...
	/* The video specific state of the processing. */
	/* Base class StateEnum holds states applicable to all processing types, eg ok, error. */
	/* The below holds states specific to video, eg no face detected, subject too far. */
	/* This is only set when the base class StateEnum == EMetaHumanLocalLiveLinkSubjectState::DeviceSpecific */
	UPROPERTY(Transient, BlueprintReadOnly, Category = "Information")
	EMetaHumanVideoBaseLiveLinkSubjectState VideoStateEnum = EMetaHumanVideoBaseLiveLinkSubjectState::Unknown;
	...
	UPROPERTY(VisibleAnywhere, AdvancedDisplay, Category = "Information", meta = (DisplayName = "Models"), meta = (EditCondition = "bIsLiveProcessing", HideEditConditionToggle, EditConditionHides))
	...
	UPROPERTY(Transient, BlueprintReadOnly, Category = "Information")
	float ConfidenceValue = -1;

	/* The resolution of the video being processed. */
	UPROPERTY(Transient, VisibleAnywhere, BlueprintReadOnly, Category = "Information", meta = (EditCondition = "bIsLiveProcessing", HideEditConditionToggle, EditConditionHides, DisplayPriority = "60"))
	FString Resolution;

	UPROPERTY(Transient, BlueprintReadOnly, Category = "Information")
	FIntPoint ImageResolution = FIntPoint(-1, -1);

	/* Whether video frames are being dropped because they can not be processed fast enough. */
	UPROPERTY(Transient, VisibleAnywhere, BlueprintReadOnly, Category = "Information", meta = (EditCondition = "bIsLiveProcessing", HideEditConditionToggle, EditConditionHides, DisplayPriority = "70"))
	FString Dropping;
...
protected:
	...
	virtual void OnUpdate(TSharedPtr<UE::MetaHuman::Pipeline::FPipelineData> InPipelineData) override;
