...
void ULiveLinkFaceSourceSettings::Init(FLiveLinkFaceSource* InSource, const FString& InConnectionString)
{
	...
	if (!bSplitSuccess)
	{
		UE_LOGF(LogLiveLinkFaceSource, Error, "Failed to extract address and port from connection string: '%ls'", *InConnectionString)
		return;
	}
	...
	{
		UE_LOGF(LogLiveLinkFaceSource, Error, "Port string '%ls' contains non-integer characters.", *PortString)
		return;
	}
