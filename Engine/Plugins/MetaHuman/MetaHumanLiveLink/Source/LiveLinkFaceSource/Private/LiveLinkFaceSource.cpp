...
FLiveLinkFaceSource::~FLiveLinkFaceSource()
{
	...
	UE_LOGF(LogLiveLinkFaceSource, Verbose, "Destroying Source")
	Stop();
}
...
void FLiveLinkFaceSource::Connect(const ULiveLinkFaceSourceSettings* InSettings)
{
	...
	UE_LOGF(LogLiveLinkFaceSource, Display, "Connecting to Host %ls Port: %d", *Host, Port)
...
{
	UE_LOGF(LogLiveLinkFaceSource, Verbose, "Stopping");
...
bool FLiveLinkFaceSource::InitUdpReceiver()
{
	...
	if (!Socket)
	{
		UE_LOGF(LogLiveLinkFaceSource, Error, "Failed to create UDP socket");
		return false;
	}
...
{
	UE_LOGF(LogLiveLinkFaceSource, VeryVerbose, "Read %lld bytes from %ls", InPayload->TotalSize(), *InEndpoint.ToString());
	...
	{
		UE_LOGF(LogLiveLinkFaceSource, Error, "Error reading payload");
		return;
	}

	if (Status != Connected)
	{
		UE_LOGF(LogLiveLinkFaceSource, Verbose, "Received valid data but the source is not yet in a connected state");
		return;
	}
...
void FLiveLinkFaceSource::OnStreamingStarted(const FLiveLinkFaceControl::FRemoteSubject& InRemoteSubject)
{
	...
	AsyncTask(ENamedThreads::GameThread, [this, InRemoteSubject]()
	{
		...
		UE_LOGF(LogLiveLinkFaceSource, Verbose, "Streaming started for subject '%ls' with %d control values.", *LiveLinkSubjectKey.SubjectName.ToString(), InRemoteSubject.PropertyNames.Num())
		...
		if (!bLiveLinkSubjectExists)
		{
			...
			{
				UE_LOGF(LogLiveLinkFaceSource, Warning, "Failed to create subject")
			}
		}
		else
		{
...
void FLiveLinkFaceSource::ProcessPacket(const FLiveLinkFacePacket& InPacket)
{
	...
	{
		UE_LOGF(LogLiveLinkFaceSource, Warning, "Received packet but no remote subject is set");
		return;
	}

	if (RemoteSubject->Id != InPacket.GetSubjectId())
	{
		UE_LOGF(LogLiveLinkFaceSource, Warning, "Received packet for unknown subject id: %ls", *SubjectId);
		return;
	}
	...
	{
		UE_LOGF(LogLiveLinkFaceSource, Warning, "Received an unexpected number of control values for subject '%ls'. Received %d Expected %d", *SubjectName, ControlValueCount, ExpectedControlValueCount);
	...
	{
		UE_LOGF(LogLiveLinkFaceSource, Verbose, "Packet received for subject '%ls' but static data has not yet been set.", *SubjectName);
		return;
	}
	...
	{
		UE_LOGF(LogLiveLinkFaceSource, Error, "Expected %d head pose values in packet but received %d.", HeadPoseValueCount, HeadPose.Num())
		return;
	}
	...
	FrameData.MetaData.StringMetaData.Add("HeadPoseMode", FString::Printf(TEXT("%i"), EnumToUnderlyingType(HeadPoseMode)));
...
void FLiveLinkFaceSource::SendAnalytics()
{
	if (bSendAnalytics && GEngine->AreEditorAnalyticsEnabled() && FEngineAnalytics::IsAvailable())
	{
		...
		{
			UE_LOGF(LogTemp, Warning, "[%ls] [%ls]", *Attr.GetName(), *Attr.GetValue());
		}
#else
