...
{
public:
	virtual ~FDnaPayloadContextBase() = default;
...
{
public:
	virtual ~FDnaMeshPayloadContext() = default;
...
{
public:
	virtual ~FDnaMorphTargetPayloadContext() = default;
...
private:
	mutable TSharedPtr<IDNAReader> DNAReader;
	mutable TMap<FString, TSharedPtr<FDnaPayloadContextBase>> PayloadContexts;

	/** Returns a string containing joint heirarchy as expected by the Interchange system */
	UE_API FString GetJointHierarchyName(TSharedPtr<IDNAReader> InDNAReader, int32 JointIndex) const;

	/** Add the hard-coded joints that are missing in the DNA file but is expected by the import */
	UE_API FString AddDNAMissingJoints(UInterchangeBaseNodeContainer& NodeContainer, const FString& InLastNodeId, FTransform& OutCombinedTransform, const FDNAConfig& DNAConfig) const;

	/** A helper function to do sanity checks and further call Fetch payload data on PayloadContexts */
