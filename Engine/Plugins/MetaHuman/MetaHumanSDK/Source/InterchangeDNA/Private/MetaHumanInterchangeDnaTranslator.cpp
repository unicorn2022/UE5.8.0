...
namespace UE::MetaHuman
{
	...
	static const TMap<FString, FString> MaterialSlotsMapping = {
		...
		{"eyelashes_lod1_mesh", "eyelashes_shader_shader"},
...
struct NeutralJointTransformer
{
	virtual ~NeutralJointTransformer() = default;
	// InverseHierarchyTransform parameter is used for root joint in DNA (spine04), which has to be attached to existing joint hierarchy in UE.
	virtual FTransform Transform(TSharedPtr<IDNAReader> DNAReader, uint16 JointIndex, const FTransform& InverseHierarchyTransform) = 0;
};

struct LegacyJointTransformer : NeutralJointTransformer
{
	FTransform Transform(TSharedPtr<IDNAReader> DNAReader, uint16 JointIndex, const FTransform& InverseHierarchyTransform) override
	{
		const int32 ParentIndex = DNAReader->GetJointParentIndex(JointIndex);
		...
		FVector JointRotation = DNAReader->GetNeutralJointRotation(JointIndex);
		FRotator Rotation(JointRotation.X, JointRotation.Y, JointRotation.Z);
		LocalTransform.SetTranslation(JointTranslation);
		LocalTransform.SetRotation(Rotation.Quaternion());

		if (bIsRootNode)
		{
			const FCoordinateSystem Coordinates = DNAReader->GetCoordinateSystem();
			FTransform RootTransform;
			if (Coordinates.YAxis == EDirection::Up)
			{
				// Original MetaHuman DNA
				RootTransform = FTransform(FRotator(0.0f, 0.0f, 90.0f));  // Y-Up to Z-Up
			}
			else if (Coordinates.ZAxis == EDirection::Up)
			{
				// FN style character DNA
				RootTransform = FTransform::Identity;
			}
			else
			{
				UE_LOGFMT(InterchangeDNATranslator, Error, "Trying to import a DNA file with an unsupported Up orientation");
			}
			LocalTransform = LocalTransform * RootTransform * InverseHierarchyTransform;
		}
...
struct AdaptiveJointTransformer : NeutralJointTransformer
{
	FTransform Transform(TSharedPtr<IDNAReader> DNAReader, uint16 JointIndex, const FTransform& InverseHierarchyTransform) override
	{
		const int32 ParentIndex = DNAReader->GetJointParentIndex(JointIndex);
		...
		FTransform LocalTransform = FTransform::Identity;
		const FVector JointTranslation = DNAReader->GetNeutralJointTranslation(JointIndex);
		const FVector JointRotation = DNAReader->GetNeutralJointRotation(JointIndex);
		const dna::Reader* LLDNAReader = DNAReader->Unwrap();
		const dna::RotationUnit RotationUnit = LLDNAReader->getRotationUnit();
		const dna::RotationSequence RotationSequence = LLDNAReader->getRotationSequence();
		const dna::RotationSign RotationSign = LLDNAReader->getRotationSign();
		const auto Angle2Rad = [RotationUnit](float Value) { return (RotationUnit == dna::RotationUnit::degrees) ? tdm::frad(tdm::fdeg(Value)) : tdm::frad(Value); };
		const tdm::fquat Rotation(tdm::frad3(Angle2Rad(JointRotation.X), Angle2Rad(JointRotation.Y), Angle2Rad(JointRotation.Z)), RotationSequence, RotationSign);
		LocalTransform.SetRotation(FQuat(Rotation.x, Rotation.y, Rotation.z, Rotation.w));
		LocalTransform.SetTranslation(JointTranslation);
		if (bIsRootNode)
		{
			LocalTransform = LocalTransform * InverseHierarchyTransform;
		}
		return LocalTransform;
	}
...
bool UMetaHumanInterchangeDnaTranslator::CanImportSourceData(const UInterchangeSourceData* InSourceData) const
{
	const FString Extension = FPaths::GetExtension(InSourceData->GetFilename());
	return Extension.Equals(TEXT("mhdna"), ESearchCase::IgnoreCase);
}
...
bool UMetaHumanInterchangeDnaTranslator::Translate(UInterchangeBaseNodeContainer& NodeContainer) const
{
	...
	TUniquePtr<NeutralJointTransformer> JointTransformer;
	UDNAConfigHolder* DNAConfigHolder = Cast<UDNAConfigHolder>(GetSourceData()->GetContextObjectByTag(TEXT("DNAConfig")));
	if (DNAConfigHolder == nullptr)
	{
		UE_LOGFMT(InterchangeDNATranslator, Error, "DNAConfigHolder was not set on SourceData before invoking MetaHuman DNA translator.");
		return false;
	}

	FDNAConfig DstDNAConfig = DNAConfigHolder->Config;
	TArray<uint8> DNADataAsBuffer;
	if (FFileHelper::LoadFileToArray(DNADataAsBuffer, *FilePath))
	{
		const FDNAConfig LegacyDNAConfig = FDNAConfig::Legacy();
		if (DstDNAConfig.CoordinateSystem == LegacyDNAConfig.CoordinateSystem &&
			DstDNAConfig.RotationSequence == LegacyDNAConfig.RotationSequence &&
			DstDNAConfig.RotationSign == LegacyDNAConfig.RotationSign &&
			DstDNAConfig.FaceWindingOrder == LegacyDNAConfig.FaceWindingOrder)
		{
			DNAReader = ReadDNAFromBuffer(&DNADataAsBuffer);
			JointTransformer = MakeUnique<LegacyJointTransformer>();
		}
		else
		{
			DNAReader = LoadDNAFromBuffer(&DNADataAsBuffer, DstDNAConfig);
			JointTransformer = MakeUnique<AdaptiveJointTransformer>();
		}
	}
	...
	UInterchangeMeshLODContainerNode* LODContainerNode = NewObject<UInterchangeMeshLODContainerNode>(&NodeContainer, NAME_None);
	const FString LODGroupUid = MeshNodeUid + TEXT("_LODGroup");
	const FString LODGroupName = MeshNodeName + TEXT("_LODGroup");
	NodeContainer.SetupNode(LODContainerNode, LODGroupUid, LODGroupName, EInterchangeNodeContainerType::TranslatedAsset);
	CurrentMeshNode->SetCustomAssetInstanceUid(LODGroupUid);

	TSet<FString> UniqueLODMeshEntryTracker;
	for (int32 LODIndex = 0; LODIndex < LODCount; LODIndex++)
	{
		...
		for (int16 LODMeshIndex = 0; LODMeshIndex < LODMeshIndices.Num(); LODMeshIndex++)
		{
			...
			const FString UniqueEntryTrackerUid = LODGroupUid + "." + NodeName;

			if (UniqueLODMeshEntryTracker.Contains(UniqueEntryTrackerUid))
			{
				//This scene node was already created.
				continue;
			}

			UniqueLODMeshEntryTracker.Add(UniqueEntryTrackerUid);
			...
			if (MeshNode)
			{
				LODContainerNode->AddMeshForLODIndex(LODIndex, MeshUniqueID, FTransform::Identity);
			}

			// Assign material dependency.
			if (MaterialSlots.IsValidIndex(MeshIndex)) // Material slot names are corresponding to mesh indices in the same order.
			{
				CurrentMeshNode->SetSlotMaterialDependencyUid(MaterialSlots[MeshIndex].Key, MaterialSlots[MeshIndex].Value);
			}
		}
	}
	...
	FTransform CombinedMissingJointTransform = FTransform::Identity;
	...
	{
		JointRoot = AddDNAMissingJoints(NodeContainer, MeshNodeUid, CombinedMissingJointTransform, DstDNAConfig);
	}
	const FTransform AbsoluteSpine3Inverse = CombinedMissingJointTransform.Inverse();
	...
	for (int32 JointIndex = 0; JointIndex < JointCount; JointIndex++)
	{
		...
		FTransform LocalTransform = JointTransformer->Transform(DNAReader, JointIndex, AbsoluteSpine3Inverse);
		...
		JointNode->SetBindPoseLocalTransform(&NodeContainer, LocalTransform);
...
FString UMetaHumanInterchangeDnaTranslator::AddDNAMissingJoints(UInterchangeBaseNodeContainer& NodeContainer, const FString& InLastNodeId, FTransform& OutCombinedTransform, const FDNAConfig& DNAConfig) const
{
	FString Hierarchy = "";
	...
	// Convert the hard coded parent bones to the new destination coordinate system that was set on the DNA asset.
	const FDNAConfig UEDNAConfig;
	const FDNAConfig LegacyDNAConfig = FDNAConfig::Legacy();
	const bool IsUEConfig = (DNAConfig.CoordinateSystem == UEDNAConfig.CoordinateSystem &&
		DNAConfig.RotationSequence == UEDNAConfig.RotationSequence &&
		DNAConfig.RotationSign == UEDNAConfig.RotationSign &&
		DNAConfig.FaceWindingOrder == UEDNAConfig.FaceWindingOrder);
	const bool IsLegacyConfig = (DNAConfig.CoordinateSystem == LegacyDNAConfig.CoordinateSystem &&
		DNAConfig.RotationSequence == LegacyDNAConfig.RotationSequence &&
		DNAConfig.RotationSign == LegacyDNAConfig.RotationSign &&
		DNAConfig.FaceWindingOrder == LegacyDNAConfig.FaceWindingOrder);

	if (!IsUEConfig && !IsLegacyConfig)
	{
		const FDNAConfig SrcDNAConfig = UEDNAConfig;
		const auto AxisDir = [](EDirection Axis)
			{
				return static_cast<tdm::axis_dir>(Axis);
			};
		const auto CoordSys = [AxisDir](FCoordinateSystem InSys)
			{
				tdm::coord_sys OutSys = {AxisDir(InSys.XAxis), AxisDir(InSys.YAxis), AxisDir(InSys.ZAxis)};
				return OutSys;
			};
		const auto RotSeq = [](ERotationSequence Seq)
			{
				return static_cast<tdm::rot_seq>(Seq);
			};
		const auto RotDir = [](ERotationDirection Dir)
			{
				return Dir == ERotationDirection::Negative ? tdm::rot_dir::negative : tdm::rot_dir::positive;
			};
		const auto RotSign = [RotDir](FRotationSign Sign)
			{
				return tdm::rot_sign{ RotDir(Sign.XAxis), RotDir(Sign.YAxis), RotDir(Sign.ZAxis) };
			};

		const auto ConvertBone = [CoordSys, RotSeq, RotSign, SrcDNAConfig, DNAConfig](FTransform& BoneTransform)
			{
				const tdm::coord_sys SrcSys = CoordSys(SrcDNAConfig.CoordinateSystem);
				const tdm::coord_sys DstSys = CoordSys(DNAConfig.CoordinateSystem);
				// Convert translation
				const FVector BoneSrcTranslation = BoneTransform.GetTranslation();
				const tdm::fvec3 DstTranslation = tdm::convert_position(tdm::fvec3{static_cast<float>(BoneSrcTranslation[0]), static_cast<float>(BoneSrcTranslation[1]), static_cast<float>(BoneSrcTranslation[2])}, SrcSys, DstSys);
				const FVector BoneDstTranslation = {DstTranslation[0], DstTranslation[1], DstTranslation[2]};
				BoneTransform.SetTranslation(BoneDstTranslation);
				// Convert rotation
				const tdm::rot_seq SrcRotSeq = RotSeq(SrcDNAConfig.RotationSequence);
				const tdm::rot_sign SrcRotSigns = RotSign(SrcDNAConfig.RotationSign);
				const FQuat BoneSrcRotation = BoneTransform.GetRotation();
				const tdm::frad3 SrcRotation = tdm::fquat(BoneSrcRotation.X, BoneSrcRotation.Y, BoneSrcRotation.Z, BoneSrcRotation.W).euler(SrcRotSeq, SrcRotSigns);

				const tdm::rot_seq DstRotSeq = RotSeq(DNAConfig.RotationSequence);
				const tdm::rot_sign DstRotSigns = RotSign(DNAConfig.RotationSign);
				const tdm::frad3 DstRotation = tdm::convert_rotation(SrcRotation, SrcSys, SrcRotSeq, SrcRotSigns, DstSys, DstRotSeq, DstRotSigns);
				const tdm::fquat DstRotationQuat(DstRotation, DstRotSeq, DstRotSigns);
				BoneTransform.SetRotation(FQuat(DstRotationQuat.x, DstRotationQuat.y, DstRotationQuat.z, DstRotationQuat.w));
			};
		ConvertBone(Pelvis);
		ConvertBone(Spine01);
		ConvertBone(Spine02);
	...
	{
		Hierarchy = Hierarchy.IsEmpty() ? MissingJoint : Hierarchy + "." + MissingJoint;
		UInterchangeJointNode* JointNode = NewObject<UInterchangeJointNode>(&NodeContainer, NAME_None);
		NodeContainer.SetupNode(JointNode, Hierarchy, MissingJoint, EInterchangeNodeContainerType::TranslatedScene, LastNodeId);

		JointNode->SetDisplayLabel(MissingJoint);
		JointNode->SetCustomLocalTransform(&NodeContainer, FTransform::Identity);

		LastNodeId = Hierarchy;
		...
		JointNode->SetCustomLocalTransform(&NodeContainer, DNATransform);
		JointNode->SetTimeZeroLocalTransform(&NodeContainer, DNATransform);
		JointNode->SetBindPoseLocalTransform(&NodeContainer, DNATransform);
	}
...
bool FDnaMorphTargetPayloadContext::FetchMeshPayloadInternal(TSharedPtr<IDNAReader> InDNAReader, const FTransform& MeshGlobalTransform, FMeshDescription& OutMorphTargetMeshDescription) const
{
	...
	OutMorphTargetMeshDescription.Empty();
	...
	//Extract the points into a simplified MeshDescription
	{
		...
		for (int32 VertexIndex = 0; VertexIndex < NumOfVerts; ++VertexIndex)
		{
			...
			VertexPositions[AddedVertexId] = FVector3f(InDNAReader->GetVertexPosition(DnaMeshIndex, VertexIndex));
