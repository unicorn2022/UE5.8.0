...
namespace UE::MetaHuman::Authentication
{
		...
		if (ClientState && ClientState->OuterClient.IsValid())
		{
			...
			case EOS_EResult::EOS_Success:
			{
				...
				if (EOS_Auth_CopyUserAuthToken(ClientState->AuthHandle, &CopyTokenOptions, ClientState->EpicAccountId, &UserAuthTokenPtr) == EOS_EResult::EOS_Success)
				{
					...
					const auto Utf8UserAuthToken = StringCast<UTF8CHAR>(*UserAuthToken);
					EOS_Auth_Token_Release(UserAuthTokenPtr);

					EOS_Connect_Credentials Credentials = {};
					Credentials.ApiVersion = EOS_CONNECT_CREDENTIALS_API_LATEST;
					Credentials.Token = reinterpret_cast<const char*>(Utf8UserAuthToken.Get());
	...
	void FClientState::LoginUsingPersist(FCallbackContext* CallbackContext)
	{
		...
		const auto Utf8UserEmail = StringCast<UTF8CHAR>(*UserEmail);
		const auto Utf8Password = StringCast<UTF8CHAR>(*Password);
		const auto Utf8ExchangeCode = StringCast<UTF8CHAR>(*ExchangeCode);
		...
		if (!UserEmail.IsEmpty() && !Password.IsEmpty())
		{
			...
			Credentials.Id = reinterpret_cast<const char*>(Utf8UserEmail.Get());
			Credentials.Token = reinterpret_cast<const char*>(Utf8Password.Get());
		}
		else if (!ExchangeCode.IsEmpty())
		{
			...
			Credentials.Token = reinterpret_cast<const char*>(Utf8ExchangeCode.Get());
		}
		else
		{
	...
	void FClientState::Init(TSharedRef<FClient> Outer, EEosEnvironmentType EosEnvironmentType, void* InReservedData)
	{
		...
		if (SDKManager && SDKManager->IsInitialized())
		{
			...
			const auto Utf8ProductId = StringCast<UTF8CHAR>(*ProductId);
			const auto Utf8SandboxId = StringCast<UTF8CHAR>(*SandboxId);
			const auto Utf8ClientId = StringCast<UTF8CHAR>(*ClientId);
			const auto Utf8ClientSecret = StringCast<UTF8CHAR>(*ClientSecret);
			const auto Utf8DeploymentId = StringCast<UTF8CHAR>(*DeploymentId);

			PlatformOptions.ClientCredentials.ClientId = reinterpret_cast<const char*>(Utf8ClientId.Get());
			PlatformOptions.ClientCredentials.ClientSecret = reinterpret_cast<const char*>(Utf8ClientSecret.Get());
			PlatformOptions.ProductId = reinterpret_cast<const char*>(Utf8ProductId.Get());
			PlatformOptions.SandboxId = reinterpret_cast<const char*>(Utf8SandboxId.Get());
			PlatformOptions.DeploymentId = reinterpret_cast<const char*>(Utf8DeploymentId.Get());
