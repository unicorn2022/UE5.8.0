...
public:
	...
	bool AddSourceFileEntry(const FString& Path, const FMetaHumanAssetVersion& Version,  TMap<FString, FMetaHumanAssetVersion>& VersionInfo) const
	{
		FString SanitizedPath = Path;
		// Convert any `\` to `/`
		FPaths::NormalizeFilename(SanitizedPath);
		// Returns false if there are any uncollapsed ".." which guarantees that there can be no leading ".."
		if (FPaths::CollapseRelativeDirectories(SanitizedPath))
		{
			// Returns false if the path starts with "/", "\" or "C:" etc. This would fail or be silently converted
			// to a relative path later, but means that we abort early on installing anything that looks potentially
			// harmful
			if (FPaths::IsRelative(SanitizedPath) && !SanitizedPath.IsEmpty() && !SanitizedPath.Contains(TEXT(":")))
			{
				// This is now a safe path fragment. To work correctly this must both resolve to a file in the zip archive or source folder
				// and a target filename when combined with a destination filepath.
				VersionInfo.Add(SanitizedPath, FMetaHumanAssetVersion(Version));
				return true;
			}
		}

		AddMessage(ELogVerbosity::Error, FText::Format(LOCTEXT("InvalidPathInPackage", "The archive contains a file path that would import to an invalid location: '{0}'"), FText::FromString(Path)));
		return false;
	}

	TMap<FString, FMetaHumanAssetVersion> GetSourceFiles() const
	{
		...
		if (TSharedPtr<FJsonObject> SourceData = FileSource.ReadJson(TEXT("FileList.json")))
		{
			...
			{
				if (!AddSourceFileEntry(Entry.Path, FMetaHumanAssetVersion(Entry.Version), VersionInfo))
				{
					return {};
				}
			}
		...
		else if ((SourceData = FileSource.ReadJson(TEXT("MHAssetVersions.txt"))))
		{
			...
			{
				if (const TSharedPtr<FJsonObject>& AssetVersionInfo = AssetVersionInfoObject->AsObject())
				{
					FString AssetPath = AssetVersionInfo->GetStringField(TEXT("path"));
					...
					FMetaHumanAssetVersion AssetVersion(AssetVersionInfo->GetStringField(TEXT("version")));
					if (!AddSourceFileEntry(AssetPath, AssetVersion, VersionInfo))
					{
						return {};
					}
				}
...
public:
	...
	bool Import()
	{
		...
		for (const TTuple<FString, FMetaHumanAssetVersion>& SourceAssetInfo : SourceFiles)
		{
			...
			// If this file has a package redirect (it was remapped from an external location),
			// use the original source path so that FCoreRedirects correctly maps old -> new.
			const FString PackageRelativePath = FPaths::GetBaseFilename(SourceAssetInfo.Key, false);
			if (const FString* OriginalPath = ImportDescription.PackageRedirects.Find(PackageRelativePath))
			{
				Paths.SourcePackage = *OriginalPath;
			}
...
public:
	...
	TOptional<UObject*> Import()
	{
		...
		const TMap<FString, FMetaHumanAssetVersion> SourceFiles = Context.GetSourceFiles();
		if (SourceFiles.IsEmpty())
		{
			return {};
		}

		const FAssetOperations AssetOperations = DetermineAssetOperations(SourceFiles, ImportPaths);
