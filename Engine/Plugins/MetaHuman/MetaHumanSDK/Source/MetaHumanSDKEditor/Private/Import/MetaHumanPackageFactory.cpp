...
UObject* UMetaHumanPackageFactory::ImportObject(UClass* InClass, UObject* InOuter, FName InName, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, bool& OutCanceled)
{
	...
	{
		FString ReadData;
		FFileHelper::BufferToString(ReadData, FileContents.GetData(), FileContents.Num());
	...
	{
		// Validate that FileRoot is a simple folder name, not a path
		if (!FileRoot.IsEmpty() && FPaths::GetCleanFilename(FileRoot) != FileRoot)
		{
			TStrongObjectPtr<UMetaHumanAssetReport> Report(NewObject<UMetaHumanAssetReport>());
			Args.Add(TEXT("FileRoot"), FText::FromString(FileRoot));
			Report->AddError({FText::Format(LOCTEXT("InvalidArchiveSubFolder", "The archive contains an invalid subfolder name: '{FileRoot}'"), Args)});
			ImportResults.Add({MakeShared<FImportResult>(Report, TStrongObjectPtr<UObject>(nullptr))});
			UnProcessedFiles.Empty();
			break;
		}
		...
		FString ReadData;
		FFileHelper::BufferToString(ReadData, ManifestFileContents.GetData(), ManifestFileContents.Num());

		// A compliment to EmbedVerificationReport in MetaHumanAssetManager.cpp. Required to avoid serialization errors.
		FJsonObjectConverter::CustomImportCallback IgnoreVerificationReport = FJsonObjectConverter::CustomImportCallback::CreateLambda([](const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* Value) -> bool
		{
			if (Property->GetName() == TEXT("VerificationReport"))
			{
				return true;
			}
			return false;
		...
		if (!FJsonObjectConverter::JsonObjectStringToUStruct(ReadData, &SourceDescription,0, 0, false, nullptr, &IgnoreVerificationReport))
		{
			Report->AddError({FText::Format(LOCTEXT("InvalidManifest", "The manifest at {ManifestPath} can not be parsed"), Args)});
			continue;
		}

		// Validate that the asset name is a simple name, not a path
		const FString NameStr = SourceDescription.Name.ToString();
		if (!NameStr.IsEmpty() && FPaths::GetCleanFilename(NameStr) != NameStr)
		{
			Args.Add(TEXT("AssetName"), FText::FromString(NameStr));
			Report->AddError({FText::Format(LOCTEXT("InvalidAssetName", "The archive contains an invalid asset name: '{AssetName}'"), Args)});
			UnProcessedFiles.Empty();
			break;
		}
		...
		FString FileListReadData;
		FFileHelper::BufferToString(FileListReadData, FileListFileContents.GetData(), FileListFileContents.Num());
