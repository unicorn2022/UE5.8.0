...
namespace UE::MetaHuman
{
	namespace Private
	{
		...
		private:
			...
			{
				ReleaseNotes.Reset();
				for (const TTuple<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& ReleaseNoteEntry : Data->AsObject()->Values)
				{
			...
			FHttpRequestPtr InitiateRequest(const FString& RequestUrl, const FString& RequestName, const TFunction<void(const FJsonValue*)>& OnComplete) const
			{
				...
				HttpRequest->OnProcessRequestComplete().BindLambda(
					[this, OnComplete, RequestName](FHttpRequestPtr Unused, FHttpResponsePtr HttpResponse, bool bSucceeded)
					{
						...
						{
							UE_LOGF(LogMetaHumanVersionService, Warning, "%ls: No response", *RequestName);
							return;
						}
						...
						{
							UE_LOGF(LogMetaHumanVersionService, Warning, "%ls", *FString::Format(TEXT("{0}: Invalid response. code={1} response={2}"), {RequestName, HttpResponse->GetResponseCode(), ResponseStr}));
							return;
						}
