...
USkeletalMesh* VerifyGroomingMesh(const UGroomBindingAsset* GroomBindingAsset, FFormatNamedArguments Args, UMetaHumanAssetReport* Report, bool bDetailedVerification)
{
	...
	// The remaining checks compare the source mesh geometry against the template MetaHuman head
	// topology in UV space, which is expensive. Skip them when only basic verification was requested.
	if (!bDetailedVerification)
	{
...
{
	// GetHairDescription() rebuilds the description each call and is expensive, so cache it once
	// and reuse it for all attribute checks below.
	const FHairDescription HairDescription = GroomAsset->GetHairDescription();

	// 3001 groom_width attribute missing from alembic file
	if (!HairDescription.HasAttribute(EHairAttribute::Width))
	{
		Report->AddInfo({
	...
	if (!HairDescription.HasAttribute(EHairAttribute::RootUV))
	{
		Report->AddWarning({
...
void VerifyGroupsStrandsInfoValid(TNotNull<const UGroomAsset*> GroomAsset, const FHairGroupInfoWithVisibility& GroupInfo, FFormatNamedArguments Args, TNotNull<UMetaHumanAssetReport*> Report, TNotNull<const USkeletalMesh*> GroomMesh, bool bVerifyGroomToMeshAlignment)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyGroupsStrandsInfoValid);

	Args.Add(TEXT("GroupIndex"), GroupInfo.GroupIndex);

	// 3008 Follicle verts not aligned with grooming mesh data
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyGroupsStrandsInfoValid_StrandsBoundsIntersect);
	...
	if (bVerifyGroomToMeshAlignment)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyGroupsStrandsInfoValid_VerifyGroomToMeshAlignment);
...
void VerifyCardsInfoValid(const UGroomAsset* GroomAsset, const FHairGroupsCardsSourceDescription& CardsDescription, FFormatNamedArguments Args, UMetaHumanAssetReport* Report, bool bVerifyStrandsAlignment)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyCardsInfoValid);
	...
	if (bVerifyStrandsAlignment)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyCardsInfoValid_StrandsAlignment);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyCardsInfoValid_VerifyMeshUVs);
...
void VerifyMeshesInfoValid(const UGroomAsset* GroomAsset, const FHairGroupsMeshesSourceDescription& MeshesDescription, FFormatNamedArguments Args, UMetaHumanAssetReport* Report, bool bVerifyStrandsAlignment)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyMeshesInfoValid);
	...
	if (bVerifyStrandsAlignment)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyMeshesInfoValid_StrandsAlignment);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyMeshesInfoValid_VerifyMeshUVs);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::Verify_Implementation);
	...
	// Check that Grooming mesh is present and correct. Basic checks (presence and spatial bounds)
	// always run; the detailed UV-topology comparison against the template head is gated by
	// bDetailedGroomingMeshVerification.
	USkeletalMesh* GroomMesh = nullptr;
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyGroomingMesh);
		GroomMesh = VerifyGroomingMesh(GroomBindingAsset, Args, Report, bDetailedGroomingMeshVerification);
	}
	if (!GroomMesh)
	{
		return;
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyWardrobeItem);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyGlobalStrandsInfoValid);
	...
	uint32 TotalGuideCount = 0;
	{
		...
		for (const FHairGroupInfoWithVisibility& GroupInfo : GroomAsset->GetHairGroupsInfo())
		{
			...
			VerifyGroupsStrandsInfoValid(GroomAsset, GroupInfo, Args, Report, GroomMesh, bVerifyGroomToMeshAlignment);
	...
	}
	{
		...
		for (const FHairGroupsCardsSourceDescription& Cards : GroomAsset->GetHairGroupsCards())
		{
			...
			VerifyCardsInfoValid(GroomAsset, Cards, Args, Report, bVerifyCardsStrandsAlignment);
		}
	}
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UVerifyMetaHumanGroom::VerifyMeshesInfoValid);
		for (const FHairGroupsMeshesSourceDescription& Meshes : GroomAsset->GetHairGroupsMeshes())
		{
			...
			VerifyMeshesInfoValid(GroomAsset, Meshes, Args, Report, bVerifyMeshesStrandsAlignment);
		}
	}
