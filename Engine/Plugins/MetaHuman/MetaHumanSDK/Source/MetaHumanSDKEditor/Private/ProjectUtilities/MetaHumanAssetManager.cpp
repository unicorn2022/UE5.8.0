...
// Writes a uasset to the archive at an explicit archive-relative path (for remapped external dependencies).
void WriteUAssetToArchiveAt(const FString& Package, const FString& ArchiveRelativePath, FZipArchiveWriter& ArchiveWriter, FMetaHumanArchiveContents& Contents, const FString& ArchiveSubFolder)
{
	FString Version = TEXT("0.0");
	FString MainAssetName = FPaths::GetPath(Package) / FString::Format(TEXT("{0}.{0}"), {FPaths::GetBaseFilename(Package)});
	if (const UObject* Asset = LoadObject<UObject>(nullptr, *MainAssetName))
	...
	{
		TArray<uint8> Data;
		FFileHelper::LoadFileToArray(Data, *PackageFilename);
		ArchiveWriter.AddFile(ArchiveSubFolder / ArchiveRelativePath, Data, FDateTime::Now());
		Contents.Files.Add({ArchiveRelativePath, Version});
	}
}
...
bool UMetaHumanAssetManager::CreateArchive(const TArray<FMetaHumanAssetDescription>& Assets, const FString& ArchivePath)
{
	...
	for (const FMetaHumanAssetDescription& Asset : Assets)
	{
		...
		// Determine the package-space root path for detecting external dependencies.
		FString RootPackageName = Asset.AssetData.PackagePath.ToString();
		if (Asset.AssetType == EMetaHumanAssetType::CharacterAssembly)
		{
			RootPackageName = FPaths::GetPath(RootPackageName);
		}

		// We need a mutable copy of the asset to record redirects into the manifest.
		FMetaHumanAssetDescription MutableAsset = Asset;

		for (const FName& Dependency : Asset.DependentPackages)
		{
			const FString DependencyStr = Dependency.ToString();
			if (FPaths::IsUnderDirectory(DependencyStr, RootPackageName))
			{
				// Internal dependency — write at its normal relative path.
				Private::WriteUAssetToArchive(DependencyStr, RootPackagePath, ArchiveWriter, Contents, ArchiveSubFolder);
			}
			else
			{
				// External dependency — remap into the archive under the asset's name folder.
				// e.g. /Game/Outfits/CaptainOutfit/OA_Captain -> TestCharacter/Outfits/CaptainOutfit/OA_Captain.uasset
				FString ExternalRelativePath = DependencyStr;
				ExternalRelativePath.RemoveFromStart(TEXT("/Game/"));
				const FString ArchiveRelativePath = Asset.Name.ToString() / ExternalRelativePath + FPackageName::GetAssetPackageExtension();

				Private::WriteUAssetToArchiveAt(DependencyStr, ArchiveRelativePath, ArchiveWriter, Contents, ArchiveSubFolder);

				// Record redirect: archive-relative package path -> original source path.
				// On import, this tells the importer to register a redirect from the original
				// path so that existing references in other assets resolve to the imported copy.
				const FString ArchiveRelativePackage = Asset.Name.ToString() / ExternalRelativePath;
				MutableAsset.PackageRedirects.Add(ArchiveRelativePackage, DependencyStr);
			}
		}
		...
		Private::AddJsonToArchive(MutableAsset, ArchiveSubFolder / TEXT("Manifest.json"), ArchiveWriter);
		Private::AddJsonToArchive(Contents, ArchiveSubFolder / TEXT("FileList.json"), ArchiveWriter);
		if (bMultiAssetArchive && bFirstAsset)
		{
			Private::AddJsonToArchive(MutableAsset, TEXT("Manifest.json"), ArchiveWriter);
		}
...
{
	using namespace UE::MetaHuman::DependencyWalker;

	// Find all dependent packages
	Asset.DependentPackages.Reset();

	// Build the seed list separately from DependentPackages so the visitor can grow
	// DependentPackages freely without aliasing the walker's seed view.
	TArray<FName> Seeds;
	Seeds.Add(Asset.AssetData.PackageName);
	...
	{
		const FName WardrobeItem = GetWardrobeItemPackage(Asset.AssetData.PackageName);
		if (!WardrobeItem.IsNone())
		{
			Seeds.Add(WardrobeItem);
	...
	if (Asset.AssetType == EMetaHumanAssetType::CharacterAssembly)
	{
		// CharacterAssembly goes one level up to include Common/ sibling folder.
		RootPath = FPaths::GetPath(RootPath);
	}
	else
	{
		// Non-assembly types can reference dependencies anywhere under /Game/.
		RootPath = TEXT("/Game");
	}

	WalkDependencies(Seeds, [&Asset, &RootPath](const FName& /*Source*/, const FName& Dependency) -> EVisitResult
	{
		// Skip excluded sub-asset roots and their entire dependency subtrees.
		if (Asset.ExcludedSubAssetRoots.Contains(Dependency))
		{
			return EVisitResult::Skip;
		}
		// Exclude dependencies outside the allowed root. For CharacterAssembly this is
		// the parent folder (to include Common/); for other types it is /Game/.
...
TArray<FMetaHumanAssetDescription> UMetaHumanAssetManager::FindAssetsForPackaging(EMetaHumanAssetType AssetType)
{
	...
	{
		// Find CharacterAssembly assets: query for UBlueprint assets under /Game/ that follow
		// the MetaHuman naming convention (BP_<Name>).
		FARFilter Filter;
		...
		for (const FAssetData& AssetData : AllBlueprints)
		{
			...
			// Verify this is a MetaHuman by checking for the correct folder structure.
			const FString CharacterName = AssetName.Mid(3); // Strip "BP_" prefix
			const FString FolderName = FPaths::GetBaseFilename(AssetData.PackagePath.ToString());
			const FString RootPath = FPaths::GetPath(AssetData.PackagePath.ToString());
			if (FolderName != CharacterName || !AssetRegistry.PathExists(RootPath / TEXT("Common")))
			{
				continue;
			}

			FoundAssets.Emplace(AssetData, EMetaHumanAssetType::CharacterAssembly, FName(CharacterName));
		}
	}
	else
	{
		// For non-assembly types, find all assets of the matching class under /Game/.
		FARFilter Filter;
		Filter.ClassPaths.Add(GetMainAssetClassPathForAssetType(AssetType));
		Filter.PackagePaths.Add(FName(TEXT("/Game")));
		Filter.bRecursivePaths = true;

		TArray<FAssetData> PotentialAssets;
		AssetRegistry.GetAssets(Filter, PotentialAssets);
...
{
	// Ensure the asset is the correct type.
...
TFuture<bool> UMetaHumanAssetManager::ImportArchive(const FString& ArchivePath, const FMetaHumanImportOptions& ImportOptions, UMetaHumanAssetReport* Report)
{
	...
	if (SourceDescription.AssetType == EMetaHumanAssetType::CharacterAssembly)
	{
		...
		FMetaHumanImportDescription ImportParams{
			...
			ZipReader,
			Report,
			SourceDescription.PackageRedirects
		};
...
TArray<FMetaHumanExcludableSubAsset> UMetaHumanAssetManager::FindExcludableSubAssets(const FMetaHumanAssetDescription& Asset)
{
	TArray<FMetaHumanExcludableSubAsset> ExcludableSubAssets;

	// Only CharacterAssembly and Character types have meaningful sub-assets to exclude.
	if (Asset.AssetType != EMetaHumanAssetType::CharacterAssembly && Asset.AssetType != EMetaHumanAssetType::Character)
	{
		return ExcludableSubAssets;
	}

	// Find WardrobeItem packages (WI_*) in the dependency tree.
	// These represent discrete sub-assets (Grooms, Clothing) that can be individually excluded.
	const IAssetRegistry& AssetRegistry = IAssetRegistry::GetChecked();
	for (const FName& Package : Asset.DependentPackages)
	{
		const FString PackageLeaf = FPackageName::GetShortName(Package);
		if (!PackageLeaf.StartsWith(TEXT("WI_")))
		{
			continue;
		}

		// Determine the sub-asset type from the WardrobeItem's dependencies.
		EMetaHumanAssetType SubAssetType = EMetaHumanAssetType::None;
		FName DisplayName = Package;
		TArray<FName> WIDeps;
		AssetRegistry.GetDependencies(Package, WIDeps);
		for (const FName& WIDep : WIDeps)
		{
			TArray<FAssetData> DepAssets;
			AssetRegistry.GetAssetsByPackageName(WIDep, DepAssets);
			for (const FAssetData& DepAsset : DepAssets)
			{
				if (DepAsset.AssetClassPath == GetMainAssetClassPathForAssetType(EMetaHumanAssetType::Groom))
				{
					SubAssetType = EMetaHumanAssetType::Groom;
					DisplayName = DepAsset.AssetName;
				}
				else if (DepAsset.AssetClassPath == GetMainAssetClassPathForAssetType(EMetaHumanAssetType::SkeletalClothing))
				{
					SubAssetType = EMetaHumanAssetType::SkeletalClothing;
					DisplayName = DepAsset.AssetName;
				}
				else if (DepAsset.AssetClassPath == GetMainAssetClassPathForAssetType(EMetaHumanAssetType::OutfitClothing))
				{
					SubAssetType = EMetaHumanAssetType::OutfitClothing;
					DisplayName = DepAsset.AssetName;
				}
			}
		}

		FMetaHumanExcludableSubAsset SubAsset;
		SubAsset.Name = DisplayName;
		SubAsset.AssetType = SubAssetType;
		SubAsset.RootPackage = Package;
		SubAsset.bIncluded = !Asset.ExcludedSubAssetRoots.Contains(Package);
		ExcludableSubAssets.Add(MoveTemp(SubAsset));
	}
