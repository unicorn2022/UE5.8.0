...
#define TEST_INVALID(What, Value) \
	...
	if (Params.StartsWith("MetaHuman."))
	{
		...
		if (BaseTestName == TEXT("MetaHuman.Face"))
		{
			...
			UDNA* DNAAsset = USkelMeshDNAUtils::GetMeshDNAAsset(FaceSkelMesh);
			...
			TEST_EQUAL(GetTestName(TEXT("DNA Asset Descriptor Name")), DNAReader->GetName(), MetaHumanName);
			TEST_EQUAL(GetTestName(TEXT("DNA Asset Descriptor Age")), DNAReader->GetAge(), 0);
			TEST_EQUAL(GetTestName(TEXT("DNA Asset Descriptor Archetype")), DNAReader->GetArchetype(), EArchetype::Other);
			TEST_EQUAL(GetTestName(TEXT("DNA Asset Descriptor Gender")), DNAReader->GetGender(), TestUtils::GetGenderFromIndex(BodyTypeIndex));
			TEST_EQUAL(GetTestName(TEXT("DNA Asset Descriptor")), DNAReader->GetMetaDataCount(), 0);

			// Both readers should be valid, but the Geometry part should be mostly empty
			TEST_NOT_NULL(GetTestName(TEXT("DNA Asset")), DNAReader.Get());

			TEST_EQUAL(GetTestName(TEXT("DNA Asset Num LODs")), DNAReader->GetLODCount(), FaceSkelMesh->GetLODNum());

			TEST_EQUAL(GetTestName(TEXT("DNA Asset File Generation")), DNAReader->GetFileFormatGeneration(), 2);
			TEST_EQUAL(GetTestName(TEXT("DNA Asset File Version")), DNAReader->GetFileFormatVersion(), 1);
			...
			TEST_GREATER_EQUAL(GetTestName(TEXT("DNA Asset File Blend Shapes")), DNAReader->GetMeshBlendShapeChannelMappingCount(), FaceSkelMeshMorphTargets.Num());
			...
			if (!FaceSkelMeshMorphTargets.IsEmpty())
			{
				...
				for (uint16 BlendShapeMappingIndex = 0; BlendShapeMappingIndex < DNAReader->GetMeshBlendShapeChannelMappingCount(); ++BlendShapeMappingIndex)
				{
					const FMeshBlendShapeChannelMapping BlendShapeChannelMapping = DNAReader->GetMeshBlendShapeChannelMapping(BlendShapeMappingIndex);
					const FString MeshName = DNAReader->GetMeshName(BlendShapeChannelMapping.MeshIndex);
					const FString BlendShapeName = DNAReader->GetBlendShapeChannelName(BlendShapeChannelMapping.BlendShapeChannelIndex);
			...
			TEST_GREATER(GetTestName(TEXT("DNA has GUI Controls")), DNAReader->GetGUIControlCount(), 0);
			TEST_GREATER(GetTestName(TEXT("DNA has Raw Controls")), DNAReader->GetRawControlCount(), 0);

			for (uint16 RawControlIndex = 0; RawControlIndex < DNAReader->GetRawControlCount(); ++RawControlIndex)
			{
				FString RawControlName = DNAReader->GetRawControlName(RawControlIndex);
			...
			for (uint16 GUIControlIndex = 0; GUIControlIndex < DNAReader->GetGUIControlCount(); ++GUIControlIndex)
			{
				GUIControls.Add(DNAReader->GetGUIControlName(GUIControlIndex));
			}

			for (uint16 GUIControlIndex = 0; GUIControlIndex < DNAReader->GetGUIControlCount(); ++GUIControlIndex)
			{
				FString GUIControlFullName = DNAReader->GetGUIControlName(GUIControlIndex);
			...
			for (uint16 AnimatedMapIndex = 0; AnimatedMapIndex < DNAReader->GetAnimatedMapCount(); ++AnimatedMapIndex)
			{
				FString AnimatedMapName = DNAReader->GetAnimatedMapName(AnimatedMapIndex);
			...
			{
				const uint16 ParentJointIndex = DNAReader->GetJointParentIndex(JointIndex);

				const FString JointName = DNAReader->GetJointName(JointIndex);
				const FString ParentJointName = DNAReader->GetJointName(ParentJointIndex);
