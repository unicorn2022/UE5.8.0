...
struct FMetaHumanAssetDescription
{
	...
	/**
	 * Root packages of sub-assets excluded by the user (e.g. specific Grooms or Clothing).
	 * These are skipped during dependency walking for both verification and packaging.
	 */
	TSet<FName> ExcludedSubAssetRoots;

	/**
	 * Package path redirects for assets that were remapped into the archive.
	 * Key: archive-relative package path (e.g. "Ada/CaptainOutfit/OA_CaptainOutfit")
	 * Value: original source package path (e.g. "/Game/Outfits/CaptainOutfit/OA_CaptainOutfit")
	 * On import, these are registered as scoped FCoreRedirects so existing references resolve correctly.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="AssetDescription")
	TMap<FString, FString> PackageRedirects;
};
...
/**
 * Describes an excludable sub-asset (e.g. a Groom or Clothing item) discovered
 * within the dependency tree of a parent asset.
 */
struct FMetaHumanExcludableSubAsset
{
	...
	/** The asset type of this sub-asset. */
	EMetaHumanAssetType AssetType = EMetaHumanAssetType::None;

	/** The root package of this sub-asset. */
	FName RootPackage;

	/** Whether this sub-asset should be included in the package (default true). */
	bool bIncluded = true;
};
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "Per-type packaging folders are no longer honored; packageable assets are discovered across all of /Game/.")
	...
	/**
	 * Identifies excludable sub-assets (Grooms, Clothing) within the dependency tree of a parent asset.
	 * Must be called after UpdateAssetDependencies.
	 *
	 * @param Asset The parent asset whose dependencies have been computed
	 * @return Array of excludable sub-assets found in the dependency tree
	 */
	static UE_API TArray<FMetaHumanExcludableSubAsset> FindExcludableSubAssets(const FMetaHumanAssetDescription& Asset);
