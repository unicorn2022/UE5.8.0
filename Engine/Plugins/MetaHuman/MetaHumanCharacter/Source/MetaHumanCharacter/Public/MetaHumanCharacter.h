...
struct FMetaHumanCharacterAssetsSection
{
	...
	METAHUMANCHARACTER_API bool operator==(const FMetaHumanCharacterAssetsSection& Other) const;
};

USTRUCT()
struct FMetaHumanCharacterWardrobeIndividualAssets
{
	GENERATED_BODY()
...
USTRUCT()
struct FMetaHumanCharacterIndividualAssets
{
	GENERATED_BODY()
...
struct FMetaHumanCharacterFaceEvaluationSettings
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Face", meta = (UIMin = "0", UIMax = "1", ClampMin = "0", ClampMax = "1"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Face", meta = (UIMin = "0.8", UIMax = "1.3", ClampMin = "0.8", ClampMax = "1.3"))
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Eyelashes", meta = (ShowOnlyInnerProperties))
	FMetaHumanCharacterEyelashesProperties Eyelashes;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Teeth", meta = (ShowOnlyInnerProperties))
...
public:
	 ...
	 * Stores the Body State data in a compressed buffer
	 */
	void SetBodyTargetPoseStateData(const FMetaHumanCharacterTargetMeshKey& InTargetMeshKey, const FSharedBuffer& InBodyStateData);

	/**
	 * Stores the face model coefficients corresponding to when the target pose state was serialized
	 */
	void SetTargetFaceModelCoefficients(const FMetaHumanCharacterTargetMeshKey& InTargetMeshKey, const TArray<float>& InCoefficients);

	/**
	 * Retrieves the face model coefficients corresponding to when the target pose state was serialized
	 */
	[[nodiscard]] TArray<float> GetTargetFaceModelCoefficients(const FMetaHumanCharacterTargetMeshKey& InTargetMeshKey) const;
	 ...
	 * Retrieves the Body State data from the internal bulk data
	 */
	[[nodiscard]] FSharedBuffer GetBodyTargetPoseStateData(const FMetaHumanCharacterTargetMeshKey& InTargetMeshKey) const;
...
public:
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Skin")
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Textures", AssetRegistrySearchable)
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Body", AssetRegistrySearchable)
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Textures|Face")
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Textures|Face", Transient)
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Textures|Body")
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Textures|Body", Transient)
	...
	UPROPERTY(VisibleAnywhere, Category = "Target Mesh")
	FMetaHumanCharacterTargetMeshKey LastTargetMeshKey;
	
	UPROPERTY(VisibleAnywhere, Category = "Target Mesh")
	FMetaHumanCharacterTargetKeyPointCollection TargetMeshKeyPointsCollection;

	UPROPERTY(VisibleAnywhere, Category = "Target Mesh")
	FMetaHumanCharacterTargetTrackingResultsCollection TargetMeshTrackingResultsCollection;
#endif
...
private:
	...
	UPROPERTY(VisibleAnywhere, Category = "DNA", AssetRegistrySearchable)
	bool bHasFaceDNABlendshapes = false;

	// Stores the Character Body State
	UE::Serialization::FEditorBulkData BodyStateBulkData;

	// Stores posed body states for target meshes
	TMap<FMetaHumanCharacterTargetMeshKey, TUniquePtr<UE::Serialization::FEditorBulkData>> BodyTargetPoseStateBulkData;
	TMap<FMetaHumanCharacterTargetMeshKey, TArray<float>> FaceModelCoefficientsAtTargetPoseMap;
