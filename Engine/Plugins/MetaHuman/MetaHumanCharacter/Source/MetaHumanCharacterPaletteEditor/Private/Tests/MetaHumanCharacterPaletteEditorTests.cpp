...
#include "Tests/MetaHumanCharacterTestEditorPipeline.h"
...
namespace
{
struct FTestSpecPair
{
	UMetaHumanCharacterPipelineSpecification* RuntimeSpec = nullptr;
	UMetaHumanCharacterEditorPipelineSpecification* EditorSpec = nullptr;

	bool IsValid() const
	{
		return EditorSpec->IsValid(RuntimeSpec);
	}

	FMetaHumanCharacterPipelineSlot& AddSlot(FName SlotName)
	{
		EditorSpec->SlotEditorData.Add(SlotName);
		return RuntimeSpec->Slots.Add(SlotName);
	}

	FMetaHumanCharacterPipelineSlotEditorData& EditorData(FName SlotName)
	{
		return EditorSpec->SlotEditorData.FindChecked(SlotName);
	}
};

FTestSpecPair MakeSpecPair()
{
	return FTestSpecPair
	{
		.RuntimeSpec = NewObject<UMetaHumanCharacterPipelineSpecification>(),
		.EditorSpec = NewObject<UMetaHumanCharacterEditorPipelineSpecification>()
	};
}
}
...
bool FMetaHumanCharacterPipelineSpecificationTest::RunTest(const FString& InParams)
{
	...
	{
		FTestSpecPair Pair = MakeSpecPair();

		{
			FMetaHumanCharacterPipelineSlot& Slot = Pair.AddSlot("VirtualA");
			// This slot can support a different type from A, as long as it inherits from one of 
			// A's supported types.
			Pair.EditorData("VirtualA").SupportedPrincipalAssetTypes.Add(USkeletalMesh::StaticClass());
			Slot.TargetSlot = "A";

			UTEST_TRUE("A slot with a target slot is virtual", Slot.IsVirtual());
		}

		UTEST_FALSE("The slot targeted by a virtual slot must exist", Pair.IsValid());

		{
			FMetaHumanCharacterPipelineSlot& Slot = Pair.AddSlot("A");
			Pair.EditorData("A").SupportedPrincipalAssetTypes.Add(USkinnedAsset::StaticClass());
			Pair.EditorData("A").SupportedPrincipalAssetTypes.Add(UStaticMesh::StaticClass());

			UTEST_FALSE("A slot without a target slot is real", Slot.IsVirtual());
		}
		
		UTEST_TRUE("A virtual slot can support a principal asset of a derived type of the target slot", Pair.IsValid());

		{
			FMetaHumanCharacterPipelineSlot& Slot = Pair.AddSlot("VirtualVirtualA");
			Pair.EditorData("VirtualVirtualA").SupportedPrincipalAssetTypes.Add(USkeletalMesh::StaticClass());
			Slot.TargetSlot = "VirtualA";

			UTEST_TRUE("A virtual slot can target a virtual slot", Pair.IsValid());

			Pair.EditorData("VirtualVirtualA").SupportedPrincipalAssetTypes.Reset();
			Pair.EditorData("VirtualVirtualA").SupportedPrincipalAssetTypes.Add(USkinnedAsset::StaticClass());

			UTEST_FALSE("A virtual slot may only support principal assets that are supported by the target slot, "
				"even if the underlying real slot could support other types", Pair.IsValid());
		}
	}
	...
	// Test multiple selection rules
	{
		...
		{
			FMetaHumanCharacterPipelineSlot& SlotA = Pair.AddSlot("A");
			Pair.EditorData("A").SupportedPrincipalAssetTypes.Add(USkinnedAsset::StaticClass());
			SlotA.bAllowsMultipleSelection = false;
		}

		{
			FMetaHumanCharacterPipelineSlot& SlotVirtualA = Pair.AddSlot("VirtualA");
			Pair.EditorData("VirtualA").SupportedPrincipalAssetTypes.Add(USkinnedAsset::StaticClass());
			SlotVirtualA.TargetSlot = "A";
			SlotVirtualA.bAllowsMultipleSelection = false;

			UTEST_TRUE("A virtual slot may target a slot if neither of them allow multiple selection", Pair.IsValid());

			SlotVirtualA.bAllowsMultipleSelection = true;

			UTEST_FALSE("A virtual slot may not allow multiple selection if its target doesn't", Pair.IsValid());
			Pair.RuntimeSpec->Slots["A"].bAllowsMultipleSelection = true;

			UTEST_TRUE("A virtual slot may target a slot if both of them allow multiple selection", Pair.IsValid());
						
			SlotVirtualA.bAllowsMultipleSelection = false;

			UTEST_TRUE("A virtual slot that doesn't allow multiple selection may target a slot that does", Pair.IsValid());
		}
	}
	...
	// Test cycle detection
	{
		...
		
		{
			...
			UTEST_FALSE("A slot may not target itself", Pair.IsValid());
...

		{
			...
			Pair.EditorData("VirtualA").SupportedPrincipalAssetTypes.Add(USkinnedAsset::StaticClass());
			SlotVirtualA.TargetSlot = "A";
			Pair.RuntimeSpec->Slots["A"].TargetSlot = "VirtualA";

			UTEST_FALSE("A chain of virtual slots must terminate in a real slot", Pair.IsValid());
			Pair.RuntimeSpec->Slots["A"].TargetSlot = NAME_None;
		}

		{
			FMetaHumanCharacterPipelineSlot& SlotB = Pair.AddSlot("B");
			Pair.EditorData("B").SupportedPrincipalAssetTypes.Add(USkeletalMesh::StaticClass());
			SlotB.TargetSlot = "A";
		}

		UTEST_FALSE("Multiple virtual slots may not target the same real slot if it doesn't allow multiple selection", Pair.IsValid());
		Pair.RuntimeSpec->Slots["A"].bAllowsMultipleSelection = true;

		UTEST_TRUE("Multiple virtual slots may target the same real slot", Pair.IsValid());

		{
			FMetaHumanCharacterPipelineSlot& SlotC = Pair.AddSlot("C");
			Pair.EditorData("C").SupportedPrincipalAssetTypes.Add(USkeletalMesh::StaticClass());
			SlotC.TargetSlot = "B";
		}

		{
			FMetaHumanCharacterPipelineSlot& SlotD = Pair.AddSlot("D");
			Pair.EditorData("D").SupportedPrincipalAssetTypes.Add(USkeletalMesh::StaticClass());
			SlotD.TargetSlot = "C";
		}

		UTEST_TRUE("Longer virtual slot chains are allowed", Pair.IsValid());
		Pair.RuntimeSpec->Slots["A"].TargetSlot = "D";

		UTEST_FALSE("Cycles are detected in longer virtual slot chains", Pair.IsValid());
		Pair.RuntimeSpec->Slots["A"].TargetSlot = "C";

		UTEST_FALSE("Cycles are detected in part of a longer virtual slot chain", Pair.IsValid());
		
		Pair.RuntimeSpec->Slots.KeySort(FNameLexicalLess());

		UTEST_FALSE("Cycles are detected when slots are in ascending alphabetical order", Pair.IsValid());
		Pair.RuntimeSpec->Slots.KeySort(FNameLexicalNotLess());

		UTEST_FALSE("Cycles are detected when slots are in descending alphabetical order", Pair.IsValid());
	}
...
bool FMetaHumanCharacterCollectionTest::RunTest(const FString& InParams)
{
	...
	UMetaHumanCharacterEditorPipelineSpecification* EditorSpec = NewObject<UMetaHumanCharacterEditorPipelineSpecification>();
	{
		Spec->Slots.Add(SlotA);
		FMetaHumanCharacterPipelineSlotEditorData& SlotEditorData = EditorSpec->SlotEditorData.Add(SlotA);
		SlotEditorData.SupportedPrincipalAssetTypes.Add(USkeletalMesh::StaticClass());
	}
	...
	UMetaHumanCharacterTestEditorPipeline* EditorPipeline = CastChecked<UMetaHumanCharacterTestEditorPipeline>(Pipeline->GetMutableEditorPipeline());
	EditorPipeline->SetSpecification(EditorSpec);
