...
bool UMetaHumanDefaultEditorPipelineLegacy::UpdateActorBlueprint(const UMetaHumanInstance* InCharacterInstance, UBlueprint* InBlueprint) const
{
	using namespace UE::MetaHuman::Private;

	if (!InCharacterInstance || !InCharacterInstance->GetExistingAssemblyOutput().IsValid())
	{
		return false;
	}

	const FMetaHumanDefaultAssemblyOutput* AssemblyStruct = InCharacterInstance->GetExistingAssemblyOutput().GetPtr<FMetaHumanDefaultAssemblyOutput>();
	...
	// Optimize bone counts for all skeletal meshes except the driving skeletal mesh (Body). This removes all bones from all
	// LODs that do not have any vertices weighted against them. This will be done for all following skeletal meshes.
	if (bOptimizeBoneCounts)
	{
		OptimizeBoneCounts(InBlueprint, ActorCDO);
	}
...
void UMetaHumanDefaultEditorPipelineLegacy::OptimizeBoneCounts(UBlueprint* InBlueprint, AActor* ActorCDO)
{
	if (!InBlueprint || !ActorCDO)
	{
		return;
	}
	...
	for (const FSubobjectDataHandle& Handle : SubobjectDataHandles)
	{
		if (!Handle.IsValid() || !Handle.GetData())
		{
			continue;
		}
		...
		if (UActorComponent* ActorComponent = const_cast<UActorComponent*>(Handle.GetData()->GetObjectForBlueprint<UActorComponent>(InBlueprint)))
		{
			...
			// Skip the body for bone count reduction. 
			// Body skeletons often only hold hands and feet meshes and do contain a lot of bones that do not have vertices weighted against them, like e.g.
			// the corrective bones in the shoulder area. The body is running rig logic To drive these. We need to keep them present for our driving skeletal mesh,
			// so that following skeletal meshes can copy the bone transforms from it. 
			...
			if (USkeletalMeshComponent* SkelMeshComponent = Cast<USkeletalMeshComponent>(ActorComponent))
			{
				...
				{
						OptimizeBoneCount(SkeletalMesh);
				}
			}
		}
	}
