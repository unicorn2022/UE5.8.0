...
bool UMetaHumanDefaultEditorPipeline::UpdateActorBlueprint(const UMetaHumanInstance* InCharacterInstance, UBlueprint* InBlueprint) const
{
	...
	if (FObjectProperty* CharacterProperty = CastField<FObjectProperty>(InBlueprint->GeneratedClass->FindPropertyByName(FName(TEXT("CharacterInstance")))))
	{
		...
		{
			CharacterProperty->SetObjectPropertyValue(PropertyAddress, const_cast<UMetaHumanInstance*>(InCharacterInstance));
