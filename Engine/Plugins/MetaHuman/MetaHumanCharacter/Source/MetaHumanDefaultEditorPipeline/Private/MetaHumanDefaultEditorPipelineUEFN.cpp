...
namespace UE::MetaHuman::Private
{
		...
		int32 BodyLODThreshold)
	{
		...
		FIntProperty* BodyLODThresholdProperty = new FIntProperty(MetaHumanComponentUEFNClass, TEXT("BodyLODThreshold"));
...
void UMetaHumanDefaultEditorPipelineUEFN::ResizeTexture(TNotNull<UTexture2D*> InTexture)
{
...
TArray<UMaterialExpression*> UMetaHumanDefaultEditorPipelineUEFN::ReconnectPathTracingQualitySwitches(TNotNull<UObject*> InMaterialOrMaterialFunction)
{
...
void UMetaHumanDefaultEditorPipelineUEFN::RemovePathTracingQualitySwitches(TNotNull<UObject*> InMaterialOrMaterialFunction)
{
...
void UMetaHumanDefaultEditorPipelineUEFN::RemoveAssetGuidelines(TNotNull<UObject*> InObject)
{
...
void UMetaHumanDefaultEditorPipelineUEFN::SaveObjectForUEFNProject(TNotNull<UObject*> TargetObject)
{
	...
	{
		// AnimBPs are not supported in UEFN
	...
	RemoveAssetGuidelines(TargetObject);
	RemovePathTracingQualitySwitches(TargetObject);
}

void UMetaHumanDefaultEditorPipelineUEFN::RemoveAllGraphsFromBlueprint(UBlueprint* Blueprint)
{
...
bool UMetaHumanDefaultEditorPipelineUEFN::BeginCharacterEditorAssemble(TNotNull<UMetaHumanCollection*> InCollection, const FString& InCharacterName)
{
	if (!Super::BeginCharacterEditorAssemble(InCollection, InCharacterName))
	{
		return false;
	}
...
void UMetaHumanDefaultEditorPipelineUEFN::EndCharacterEditorAssemble(TNotNull<class UMetaHumanCollection*> InCollection)
{
	Super::EndCharacterEditorAssemble(InCollection);

	// Unload the UEFN plugin
	if (UEFNPlugin.IsValid())
	{
...
UBlueprint* UMetaHumanDefaultEditorPipelineUEFN::WriteActorBlueprint(const FWriteBlueprintSettings& InWriteBlueprintSettings) const
{
	...
	if (FoundHandle != nullptr)
	{
		...
		RemoveAllGraphsFromBlueprint(Blueprint);
...
bool UMetaHumanDefaultEditorPipelineUEFN::UpdateActorBlueprint(const UMetaHumanInstance* InCharacterInstance, UBlueprint* InBlueprint) const
{
	...
	// See FMetaHumanCharacterEditorBuild::BuildMetaHumanCharacter() for the UE reference implementation
	{
		...
		const FInstancedStruct& AssemblyOutput = InCharacterInstance->GetExistingAssemblyOutput();
...
void UMetaHumanDefaultEditorPipelineUEFN::OnCommonDependenciesUnpacked(const TMap<UObject*, UObject*>& InDuplicatedDependencies) const
{
	...
	for (const TPair<UObject*, UObject*>& It : InDuplicatedDependencies)
	{
		...
		{
			SaveObjectForUEFNProject(TargetObject);
...
void UMetaHumanDefaultEditorPipelineUEFN::UnpackCommonDependencies(TArray<UObject*> InRootObjects, TNotNull<const UMetaHumanCollection*> InCollection, TMap<UObject*, UObject*>& OutDuplicatedRootObjects) const
{
	...
	if (!FMetaHumanCharacterEditorBuild::DuplicateDepedenciesToNewRoot(PluginDependencies, CommonFolderPath, UnpackedDependencies, DuplicatedDependencies,
	...
	{
		UE_LOGFMT(LogMetaHumanDefaultEditorPipeline, Error, "Failed to unpack Common dependencies due to conflicting assets. Check the Message Log for details.");
		return;
	}
	...
	for (UObject* Object : UnpackedDependencies)
	{
		if (Object)
		{
			SaveObjectForUEFNProject(Object);
			UE::MetaHuman::Private::SavePackageDirect(Object);
		}
	}
}
