...
namespace UE::MetaHuman
{
	...
	static void OptimizeGroom(TNotNull<UGroomAsset*> GroomAsset, EMetaHumanQualityLevel InQuality)
	{
		...
		for (int32 GroupIndex = 0; GroupIndex < GroomAsset->GetNumHairGroups(); ++GroupIndex)
		{
			...
			if (!HairGroupLOD.LODs.IsValidIndex(1))
			{
				continue;
			}
	...
	static void DownsizeGroomTextures(
		...
		TNotNull<const ITargetPlatform*> TargetPlatform, 
	...
	if (Quality == EMetaHumanCharacterPaletteBuildQuality::Production)
	{
		...
		for (const TObjectPtr<USkeletalMesh>& Mesh : GroomBuildInput.BindingMeshes)
		{
			if (Mesh)
			{
				...
				CharacterBinding->ClearFlags(RF_Standalone);
	...
	else
	{
		for (const TObjectPtr<USkeletalMesh>& Mesh : GroomBuildInput.BindingMeshes)
		{
			if (Mesh)
			{
				...
				// Reset MatchingSection to 0 for preview builds if the target mesh doesn't have the requested material section at all LODs.
				// This is only applicable to Eyelashes which are bound to section 8 (eyes) while all other grooms are bound to secition 0 (face).
				// This avoids spurious LogHairStrands warnings during the binding build.
				// The builder already falls back to section 0, so the visual result is not changed at all.
				if (const int32 MatchingSection = CharacterBinding->GetMatchingSection();
					MatchingSection > 0 && Mesh->GetLODNum() > 0)
				{
					const FSkeletalMeshRenderData* RenderData = Mesh->GetResourceForRendering();
					if (RenderData)
					{
						bool bSectionMissingInSomeLOD = false;
						for (int32 LODIndex = 0; LODIndex < RenderData->LODRenderData.Num(); ++LODIndex)
						{
							...
							{
								if (Section.MaterialIndex == static_cast<uint16>(MatchingSection))
								{
									bFound = true;
									break;
								}
							...
							if (!bFound)
							{
								bSectionMissingInSomeLOD = true;
								break;
							}
						}

						if (bSectionMissingInSomeLOD)
						{
							// Set the default section to avoid warning.
							// This only affects the transient duplicated binding used for preview. The original shipped binding asset is untouched.
							CharacterBinding->SetMatchingSection(0);
						}
					}
				}
...
UE::Tasks::TTask<FMetaHumanPaletteBuiltData> UMetaHumanGroomEditorPipeline::BuildItem(const FBuildItemParams& Params) const
{
	const UMetaHumanGroomPipeline* RuntimePipeline = Cast<UMetaHumanGroomPipeline>(GetRuntimePipeline());
	if (!RuntimePipeline)
	{
		UE_LOGFMT(LogMetaHumanDefaultEditorPipeline, Error, "Runtime pipeline for item {ItemPath} must be a UMetaHumanGroomPipeline", Params.ItemPath.ToDebugString());
		
		return UE::Tasks::MakeCompletedTask<FMetaHumanPaletteBuiltData>();
	}

	if (!Params.BuildInput.GetPtr<FMetaHumanGroomPipelineBuildInput>())
	{
	...
	if (!GroomBinding)
	{
		UE_LOGFMT(LogMetaHumanDefaultEditorPipeline, Error, "Groom pipeline failed to load groom binding {Binding} during build", Params.WardrobeItem->PrincipalAsset.ToString());
	...
	{
		UE_LOGFMT(LogMetaHumanDefaultEditorPipeline, Error, "No Groom asset assigned to binding {Binding} for item {ItemPath}", GroomBinding->GetName(), Params.ItemPath.ToDebugString());
		
		return UE::Tasks::MakeCompletedTask<FMetaHumanPaletteBuiltData>();
	}

	const FMetaHumanGroomPipelineBuildInput& GroomBuildInput = Params.BuildInput.Get<FMetaHumanGroomPipelineBuildInput>();
	...
	{
		const TArray<FHairGroupsMaterial>& HairGroupMaterials = GroomBinding->GetGroom()->GetHairGroupsMaterials();
		...
		UE::MetaHuman::MaterialUtils::GenerateAssemblyParameters(
			...
			UMetaHumanGroomPipeline::MakeFetchSlotNameDelegate(HairGroupMaterials),
			UMetaHumanGroomPipeline::MakeFetchSlotMaterialDelegate(HairGroupMaterials),
