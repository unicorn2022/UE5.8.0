...
USTRUCT()
struct FMetaHumanOutfitPipelineBuildCharacterData
{
	GENERATED_BODY()
	...
	UPROPERTY()
	bool bGenerateLOD0Only = false;

	UPROPERTY()
	bool bStripSimMesh = false;

	UPROPERTY()
	TObjectPtr<USkeleton> SkeletonForOutputMesh;
};
...
struct FMetaHumanOutfitPipelineBuildInput
{
	...
	UPROPERTY()
	FName TargetBodyPropertyName;

	UPROPERTY()
	FName ResizableOutfitPropertyName;

	UPROPERTY()
	TMap<FMetaHumanPaletteItemKey, FMetaHumanOutfitPipelineBuildCharacterData> CharacterData;
};
