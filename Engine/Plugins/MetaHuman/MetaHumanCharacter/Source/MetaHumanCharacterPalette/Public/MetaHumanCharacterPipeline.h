 ...
 * 
 * See the Instance Parameter functions on UMetaHumanInstance for more info.
 */
USTRUCT()
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	FMetaHumanAssemblyOutput() = default;
	FMetaHumanAssemblyOutput(const FMetaHumanAssemblyOutput& Other) = default;
	FMetaHumanAssemblyOutput(FMetaHumanAssemblyOutput&& Other) = default;
	FMetaHumanAssemblyOutput& operator=(const FMetaHumanAssemblyOutput& Other) = default;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	 ...
	 */
	UE_DEPRECATED(5.8, "There are now two types of Instance Parameters: Assembly Parameters and Post-Assembly Parameters. Only Post-Assembly Parameters are used in this context.")
	UPROPERTY()
	TMap<FMetaHumanPaletteItemPath, FMetaHumanInstanceParameterOutput> InstanceParameters;

	/**
	 * The Post-Assembly Parameters that are available to set on this assembly and their default 
	 * values.
	 * 
	 * Some items may not provide Post-Assembly Parameters, so there may not be an entry for 
	 * every assembled item.
	 * 
	 * The Collection's own Post-Assembly Parameters are stored using the empty item path as the 
	 * key.
	 */
	UPROPERTY()
	FMetaHumanPostAssemblyParameterOutputCollection PostAssemblyParameters;
};

/**
 * A Pipeline contains the functionality for building a Palette and assembling Instances
 * from it.
 * 
 * Each Pipeline owns an Editor Pipeline that provides its editor-only functionality, such as 
 * building the Palette.
 * 
 * The Pipeline itself is responsible for assembling Instances, so this can be done
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "There are now two types of Instance Parameters: Assembly Parameters and Post-Assembly Parameters. This function has been replaced with SetPostAssemblyParameters.")
	UE_API virtual void SetInstanceParameters(const FInstancedStruct& ParameterContext, const FInstancedPropertyBag& Parameters) const;
	 ...
	 */
	UE_API static TArrayView<const FMetaHumanPinnedSlotSelection> FilterPinnedSlotSelectionsToItem(
	...
	UE_API static TArrayView<const FMetaHumanPaletteItemPath> FilterItemPaths(
