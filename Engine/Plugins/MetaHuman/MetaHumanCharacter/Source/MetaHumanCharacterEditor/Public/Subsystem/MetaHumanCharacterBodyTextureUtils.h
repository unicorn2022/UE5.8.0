...
public:
	...
	static void UpdateBodyTextureSet(const TOptional<FMetaHumanCharacterSkinSettings>& InCharacterSkinSettings,
	...
	static UTexture2D* CreateBodyTextureFromSource(EBodyTextureType InTextureType, FImageView InTextureImage, bool bInMakeVirtualIfSupported);

	static void UpdateBodySkinTone(
		const FMetaHumanCharacterSkinProperties& InSkinProperties,
		const FMetaHumanFaceTextureSynthesizer& InFaceTextureSynthesizer,
		const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet,
		TNotNull<UMaterialInstanceDynamic*> InBodyMID);

	UE_API static void UpdateBodyMaterialFromSkinProperties(
		const FMetaHumanCharacterSkinProperties& InSkinProperties,
