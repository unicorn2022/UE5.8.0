...
struct UE_API FMetaHumanCharacterEditorBuildParameters
{
	GENERATED_BODY()
...
{
public:
	UE_API virtual ~IMetaHumanCharacterBuildExtender();
	UE_API static const FName FeatureName;

	// Additional mount points so assets get considered by assembly.
	UE_API virtual TSet<FString> GetMountPoints() const;
	...
	UE_API virtual TSet<FName> GetPackageRootsToCopyFrom() const;

	/** Get the additional animation system names the extender will add. */
	UE_API virtual TArray<FName> GetAnimationSystemOptions() const;

	UE_API virtual bool IsUEFN() const;
	UE_API virtual const TSoftClassPtr<UMetaHumanCollectionPipeline> GetOverwritePipeline(EMetaHumanDefaultPipelineType InPipelineType, EMetaHumanQualityLevel InQualityLevel);

	/** Inform the extender about the Common folder path. */
	UE_API virtual void SetCommonPath(const FString& InCommonFolderPath);
	...
	UE_API virtual TArray<UObject*> GetRootObjects(const FMetaHumanCharacterEditorBuildParameters& BuildParameters) const;

	/** Called after dependencies are duplicated, allowing extenders to perform custom modifications. */
	UE_API virtual void PostDuplicateDependencies(const FString& AssetBuildPath, TNotNull<UMetaHumanInstance*> CharacterInstance);
};
...
struct FMetaHumanCharacterEditorBuild
{
	 ...
	 */
	UE_API static void BuildMetaHumanCharacter(
	 ...
	 */
	UE_API static void StripLODsFromMesh(TNotNull<class USkeletalMesh*> InSkeletalMesh, const TArray<int32>& InLODsToKeep);

	/**
	 * Downsize a texture it is larger than the target resolution
	 */
	UE_API static void DownsizeTexture(TNotNull<class UTexture*> InTexture, int32 InTargetResolution, TNotNull<const ITargetPlatform*> InTargetPlatform);
	 ...
	 */
	UE_API static USkeletalMesh* MergeHeadAndBody_CreateAsset(
	 ...
	 */
	UE_API static USkeletalMesh* MergeHeadAndBody_CreateTransient(
	 ...
	 * @return true if all dependencies were duplicated successfully, false if a fatal conflict was detected
	 */
	UE_API static bool DuplicateDepedenciesToNewRoot(
	 ...
	 *
	 * @param InParentMI         Parent material whose loaded child MIDs/MICs should be marked garbage.
	 * @param InProtectedAssets  Optional set of objects to skip even if they would otherwise match
	 *                           IsChildOf(InParentMI). Used by DuplicateDepedenciesToNewRoot to keep
	 *                           the canonical targets it just produced or reused alive across the sweep.
	 */
	UE_API static void DeleteMaterialInstanceChildren(TNotNull<UMaterialInterface*> InParentMI, const TSet<UObject*>* InProtectedAssets = nullptr);
	...
	*/
	UE_API static void CollectDependencies(const TArray<UObject*>& InRootObjects, const TSet<FString>& InAllowedMountPoints, TSet<UObject*>& OutDependencies);
	...
	*/
	UE_API static void CollectUObjectReferencesFromStruct(const UStruct* StructType, const void* StructPtr, TArray<UObject*>& OutObjects);

	/**
	 * Helper returning the version stored in the metadata. Version of 0.0 is returned if metadata is not found
	 */
	UE_API static UE::MetaHuman::FMetaHumanAssetVersion GetMetaHumanAssetVersion(TNotNull<const UObject*> InAsset);

	UE_DEPRECATED(5.7, "Please use GetCurrentMetaHumanAssetVersion") 
	UE_API static UE::MetaHuman::FMetaHumanAssetVersion GetMetaHumanAssetVersion();

	/**
	 * Helper returning the Actor BP version used in the plugin as the current engine version
	 */
	UE_API static UE::MetaHuman::FMetaHumanAssetVersion GetCurrentMetaHumanAssetVersion();

	/**
	 * Helper returning the earliest compatible version of the engine for MetaHuman Character
	 */
	UE_API static UE::MetaHuman::FMetaHumanAssetVersion GetFirstMetaHumanCompatibleVersion();

	/**
	 * Helper to check if provided asset has matching or higher version in metadata.
	 */
	UE_API static bool MetaHumanAssetMetadataVersionIsCompatible(TNotNull<const UObject*> InAsset);

	/**
	 * Helper to set the latest MetaHuman Asset Version used in the plugin
	 */
	UE_API static void SetMetaHumanVersionMetadata(TNotNull<UObject*> InObject);
