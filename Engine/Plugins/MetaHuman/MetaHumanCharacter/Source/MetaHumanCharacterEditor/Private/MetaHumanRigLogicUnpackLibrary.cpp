...
static TSharedPtr<IDNAReader> ReloadDNAInUESpace(TSharedPtr<IDNAReader> InDNAReader)
{
	auto MemoryStream = dna::makeScoped<dna::MemoryStream>();
	auto DNAWriter = dna::makeScoped<dna::BinaryStreamWriter>(MemoryStream.get());
	DNAWriter->setFrom(InDNAReader->Unwrap());
	DNAWriter->write();
	MemoryStream->seek(0);
	FDNAConfig Config = FDNAConfig::Legacy(ECoordinateSystemTransformPolicy::Transform);
	return LoadDNAFromStream(MemoryStream.get(), Config);
}
...
{
	const TSharedPtr<IDNAReader> BehaviorReader = USkelMeshDNAUtils::GetDNAReader(SkeletalMesh);
	if (!BehaviorReader.IsValid())
	{
		return false;
	}

	const TSharedPtr<IDNAReader> DNAReader = ReloadDNAInUESpace(BehaviorReader);
	...
	for (uint16 i = 0; i < DNAReader->GetJointCount(); i++)
	{
		const FVector Translation = DNAReader->GetNeutralJointTranslation(i);
		NeutralJointTranslations.Add(DNAReader->GetJointName(i), UE::Math::TVector<float>(Translation.X, Translation.Y, Translation.Z));

		const FVector Rotation = DNAReader->GetNeutralJointRotation(i);
		const ERotationUnit RotationUnit = DNAReader->GetRotationUnit();
		const ERotationSequence RotationSequence = DNAReader->GetRotationSequence();
		const FRotationSign RotationSign = DNAReader->GetRotationSign();
		...
		const auto Angle2Rad = [RotationUnit](float Value) { return (RotationUnit == ERotationUnit::Degrees) ? tdm::frad(tdm::fdeg(Value)) : tdm::frad(Value); };
		const dna::RotationSign DNARotationSign{ConvertRotationDir(RotationSign.XAxis), ConvertRotationDir(RotationSign.YAxis), ConvertRotationDir(RotationSign.ZAxis)};
		const tdm::fquat q(tdm::frad3(Angle2Rad(Rotation.X), Angle2Rad(Rotation.Y), Angle2Rad(Rotation.Z)), static_cast<dna::RotationSequence>(RotationSequence), DNARotationSign);
		NeutralJointRotations.Add(DNAReader->GetJointName(i), UE::Math::TQuat(q.x, q.y, q.z, q.w));
	}

	uint16 SolverCount = DNAReader->GetRBFSolverCount();

	OutGeneratedAssets.Reserve(SolverCount);

	for (uint16 i = 0; i < DNAReader->GetRBFSolverCount(); i++)
	{
		// Get the index for each driven joint
		TArrayView<const uint16> JointGroupIndices = DNAReader->GetJointGroupJointIndices(i);

		const FString SolverName = DNAReader->GetRBFSolverName(i);
...

		{
			...
			TArrayView<const uint16> PoseIndices = DNAReader->GetRBFSolverPoseIndices(i);
			...
			TArrayView<const uint16> RawControlIndices = DNAReader->GetRBFSolverRawControlIndices(i);
			TArrayView<const float> RawControlValues = DNAReader->GetRBFSolverRawControlValues(i);
			
			for (const uint16 RawControlIndex : RawControlIndices)
			{
				const FString RawControlName = DNAReader->GetRawControlName(RawControlIndex);
			...
			{
				PoseJointOutputIndicesMap.Add(PoseIndex, DNAReader->GetRBFPoseJointOutputIndices(PoseIndex));
				PoseJointOutputValuesMap.Add(PoseIndex, DNAReader->GetRBFPoseJointOutputValues(PoseIndex));
			}
			...
			for (const uint16 JointIndex : JointGroupIndices)
			{
				...
				const FString BoneName = DNAReader->GetJointName(JointIndex);
				...
				for (const uint16 PoseIndex : PoseIndices)
				{
					const FString PoseName = DNAReader->GetRBFPoseName(PoseIndex);
					PoseNames.AddUnique(FName(PoseName));
					// Get the indices for the driven joints
					TArrayView<const uint16> PoseJointOutputIndices = DNAReader->GetRBFPoseJointOutputIndices(PoseIndex);
					// Get the values for each driven joint
					TArrayView<const float> PoseJointOutputValues = DNAReader->GetRBFPoseJointOutputValues(PoseIndex);
					...
					const auto Angle2Rad = [RotationUnit](float Value) { return (RotationUnit == ERotationUnit::Degrees) ? tdm::frad(tdm::fdeg(Value)) : tdm::frad(Value); };
					const dna::RotationSign DNARotationSign{ ConvertRotationDir(RotationSign.XAxis), ConvertRotationDir(RotationSign.YAxis), ConvertRotationDir(RotationSign.ZAxis) };
					const tdm::fquat q(tdm::frad3(Angle2Rad(CurveOutputMap.FindChecked(StartIndex + 3)), Angle2Rad(CurveOutputMap.FindChecked(StartIndex + 4)), Angle2Rad(CurveOutputMap.FindChecked(StartIndex + 5))), static_cast<dna::RotationSequence>(RotationSequence), DNARotationSign);
		...
		{
			UE_LOGF(LogTemp, Error, "Unable to create PoseAsset for solver: %ls", *SolverName);
			continue;
		}
		...
		if (PoseAsset && IsValid(AnimBlueprint))
		{
			...
			if (!IsValid(PoseDriverNode))
			{
				...
				if (!IsValid(PoseDriverNode))
				{
					UE_LOGF(LogTemp, Error, "Unable to create a pose driver node for %ls", *SolverName);
					continue;
				}
			}
			if (IsValid(PoseDriverNode))
			{
				...
				EAutomaticRadius SolverAutomaticRadius = DNAReader->GetRBFSolverAutomaticRadius(i);
				RBFParams.bAutomaticRadius = false;
				if (SolverAutomaticRadius == EAutomaticRadius::On)
				{
					RBFParams.bAutomaticRadius = true;
				}
				RBFParams.SolverType = DNAReader->GetRBFSolverType(i);
				RBFParams.DistanceMethod = DNAReader->GetRBFSolverDistanceMethod(i);
				RBFParams.Function = DNAReader->GetRBFSolverFunctionType(i);
				RBFParams.NormalizeMethod = DNAReader->GetRBFSolverNormalizeMethod(i);
				ETwistAxis TwistAxis = DNAReader->GetRBFSolverTwistAxis(i);
				...
				RBFParams.Radius = DNAReader->GetRBFSolverRadius(i);
				RBFParams.WeightThreshold = DNAReader->GetRBFSolverWeightThreshold(i);
	...
	TArray<uint16>& HalfRotationSolvers)
{
	...
	TSharedPtr<IDNAReader> BehaviorReader = USkelMeshDNAUtils::GetDNAReader(SkeletalMesh);
	if (!BehaviorReader.IsValid())
	{
	...
	TSharedPtr<IDNAReader> DNAReader = ReloadDNAInUESpace(BehaviorReader);
	...
	if (UnpackSwingTwistEvaluation)
	{
		...
		for (uint16 i = 0; i < DNAReader->GetTwistCount(); i++)
		{
			TArrayView<const uint16> TwistControlIndices = DNAReader->GetTwistInputControlIndices(i);
			if (TwistControlIndices.IsEmpty())
			{
				continue;
			}
			// Get the input joint name
			const FString RawControlName = DNAReader->GetRawControlName(TwistControlIndices[0]);
			...
			TArrayView<const uint16> OutputJointIndices = DNAReader->GetTwistOutputJointIndices(i);
			TArray<FString> OutputJointNames;
			for (const uint16 JointIndex : OutputJointIndices)
			{
				OutputJointNames.AddUnique(DNAReader->GetJointName(JointIndex));
			}

			// Get the blend values
			TArrayView<const float> BlendValues = DNAReader->GetTwistBlendWeights(i);
			const ETwistAxis TwistAxis = DNAReader->GetTwistSetupTwistAxis(i);
			...
			if (!IsValid(TwistGraphNode))
			{
				UE_LOGF(LogTemp, Error, "Unable to create Twist setup for %ls", *InputJointName);
				continue;
			}
		...
		for (uint16 i = 0; i < DNAReader->GetSwingCount(); i++)
		{
			TArrayView<const uint16> SwingControlIndices = DNAReader->GetSwingInputControlIndices(i);
			if (SwingControlIndices.IsEmpty())
			{
				continue;
			}
			// Get the input joint name
			const FString RawControlName = DNAReader->GetRawControlName(SwingControlIndices[0]);
			TArray<FString> SplitName;
			RawControlName.ParseIntoArray(SplitName, TEXT("."), true);
			if (SplitName.IsEmpty())
			{
			...
			// Get the output joint names
			TArrayView<const uint16> OutputJointIndices = DNAReader->GetSwingOutputJointIndices(i);
			TArray<FString> OutputJointNames;
			for (const uint16 JointIndex : OutputJointIndices)
			{
				OutputJointNames.AddUnique(DNAReader->GetJointName(JointIndex));
			...
			// Get the blend values
			TArrayView<const float> BlendValues = DNAReader->GetSwingBlendWeights(i);
			ETwistAxis TwistAxis = DNAReader->GetSwingSetupTwistAxis(i);
			...
			if (!IsValid(SwingGraphNode))
			{
				UE_LOGF(LogTemp, Error, "Unable to create Swing setup for %ls", *InputJointName);
				continue;
			}
	...
	if (!HalfRotationSolvers.IsEmpty())
	{
		...
		{
			// Get the index for each driven joint
			TArrayView<const uint16> JointGroupIndices = DNAReader->GetJointGroupJointIndices(SolverIndex);
			
			for (const uint16 JointIndex : JointGroupIndices)
			{
				FString JointName = DNAReader->GetRawControlName(JointIndex);
				// Get the bone name from the dna and map to the bone index on the skeleton
				FString BoneName = DNAReader->GetJointName(JointIndex);
				DriverJointNames.AddUnique(BoneName);
			}
		}
		...
		for (UEdGraph* Graph : AnimBlueprint->FunctionGraphs)
		{
			if (UAnimationGraph* AnimGraph = Cast<UAnimationGraph>(Graph))
			{
				...
				for (UAnimGraphNode_ControlRig* Node : ControlRigNodes)
				{
					...
					else if (Node->Node.GetControlRigAssetReference().GetPathName() == ControlRig->GetPathName())
					{
						ControlRigNode = Node;
					}
				}
