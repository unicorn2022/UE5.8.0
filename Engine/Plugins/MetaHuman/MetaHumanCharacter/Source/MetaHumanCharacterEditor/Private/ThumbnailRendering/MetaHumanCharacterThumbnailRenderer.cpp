...
void FMetaHumanCharacterThumbnailScene::CreatePreview(UMetaHumanCharacter* InCharacter, EMetaHumanCharacterThumbnailCameraPosition InCameraPosition)
{
	...
	FMetaHumanCharacterFaceMaterialSet HeadMaterialSet = FMetaHumanCharacterSkinMaterials::GetHeadPreviewMaterialInstance(PreviewMode, /* bInWithVTSupport */ false);

	UMaterialInstanceDynamic* BodyPreviewMaterialInstance = FMetaHumanCharacterSkinMaterials::GetBodyPreviewMaterialInstance(PreviewMode, /* bInWithVTSupport */ false);
	...
	// Wait for any async-built skel mesh render data. Save Preset duplicates the character which
	// triggers a fresh build of FaceMesh/BodyMesh; thumbnail draw can otherwise race the build
	// and the scene proxy renders the world grid fallback.
	{
		TArray<UObject*> ObjectsToFinish;
		if (USkeletalMesh* FaceMesh = FaceComp->GetSkeletalMeshAsset())
		{
			ObjectsToFinish.Add(FaceMesh);
		}
		if (USkeletalMesh* BodyMesh = BodyComp->GetSkeletalMeshAsset())
		{
			ObjectsToFinish.Add(BodyMesh);
		}
		FAssetCompilingManager::Get().FinishCompilationForObjects(ObjectsToFinish);
	}

	// Wait for parent material shaders. Fresh MIDs from GetHead/BodyPreviewMaterialInstance
	// may have parents whose shaders are still compiling, which renders as world grid.
	if (GShaderCompilingManager)
	{
		GShaderCompilingManager->FinishAllCompilation();
	}

	// Mirror UpdateActorsSkinPreviewMaterial behavior: mark dirty + bounds update so the
	// proxy is recreated with the new override materials.
	FaceComp->MarkRenderStateDirty();
	FaceComp->UpdateBounds();
	BodyComp->MarkRenderStateDirty();
	...
	// Block until enqueued render commands flush so the thumbnail readback sees the
	// updated proxies, not the previous-frame state.
...
void FMetaHumanCharacterThumbnailScene::DestroyPreview()
{
	if (PreviewActor)
	{
		UMetaHumanCharacterEditorSubsystem::Get()->DestroyMetaHumanCharacterEditorActor(PreviewActor);
	}
