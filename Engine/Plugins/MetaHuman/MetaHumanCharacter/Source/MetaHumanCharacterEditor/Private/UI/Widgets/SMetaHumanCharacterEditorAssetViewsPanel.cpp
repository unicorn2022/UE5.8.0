...
namespace UE::MetaHuman::Private
{
	...
	static const FText MultiFolderText = LOCTEXT("MetaHumanAssetViewsPanel_MultiFolder_Label", "My Presets");
...
{
	bShowThumbnailType = InArgs._ShowThumbnailType;
	...
	OnApplyHeadOnlyDelegate = InArgs._OnApplyHeadOnly;
	OnApplyBodyOnlyDelegate = InArgs._OnApplyBodyOnly;
					...
					[
						...
						[
							...
							[
								...
								// Thumbnail type check box
								+ SOverlay::Slot()
								.VAlign(VAlign_Top)
								.HAlign(HAlign_Right)
								[
									SNew(SOverlay)
									.Visibility(this, &SMetaHumanCharacterAssetViewItem::GetThumbnailTypeCheckBoxVisibility)
									...
									[
										...
										[
											SNew(SCheckBox)
											.IsChecked(this, &SMetaHumanCharacterAssetViewItem::GetThumbnailTypeCheckState)
											.OnCheckStateChanged(this, &SMetaHumanCharacterAssetViewItem::OnThumbnailTypeCheckStateChanged)
											.Style(&FAppStyle::Get().GetWidgetStyle<FCheckBoxStyle>("ToggleButtonCheckBox"))
											.ToolTipText(LOCTEXT("MetaHumanAssetViewsPanel_ThumbnailTypeCheckBox_Tooltip", "Click on the head icon to toggle visualization of the head on the thumbnail on or off."))
											.UncheckedImage(FMetaHumanCharacterEditorStyle::Get().GetBrush("MetaHumanCharacterEditorTools.Rounded.DefaultBrush"))
									...
									.HAlign(HAlign_Center)
									.VAlign(VAlign_Center)
									.Padding(0.f, 1.f, .5f, 0.f)
									[
										SNew(SImage)
										.Image(FMetaHumanCharacterEditorStyle::Get().GetBrush("MetaHumanCharacterEditorTools.ThumbnailType"))
										.DesiredSizeOverride(FVector2D(16.f, 18.f))
	...
	if (AssetItem->ThumbnailImageOverride.IsValid() && AssetItem->AssetData.IsValid())
	{
		ThumbnailWidget =
			...
			[
				...
				[
					SNew(SImage)
					.Image(this, &SMetaHumanCharacterAssetViewItem::GetThumbnailImageOverrideBrush)
				]
			];
	}
	else
...
TSharedRef<SWidget> SMetaHumanCharacterAssetViewItem::GenerateAssetItemContextMenu()
{
	...
	MenuBuilder.BeginSection("OptionsSection", LOCTEXT("OptionsSection", "Options"));
	{
		...
		MenuBuilder.AddMenuEntry(
			LOCTEXT("AssetViewItem_ApplyHeadOnly_Label", "Apply Head Only"),
			LOCTEXT("AssetViewItem_ApplyHeadOnly_Tooltip", "Apply only the head part of this preset"),
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SMetaHumanCharacterAssetViewItem::OnApplyHeadOnlyClicked))
		);

		MenuBuilder.AddMenuEntry(
			LOCTEXT("AssetViewItem_ApplyBodyOnly_Label", "Apply Body Only"),
			LOCTEXT("AssetViewItem_ApplyBodyOnly_Tooltip", "Apply only the body part of this preset"),
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SMetaHumanCharacterAssetViewItem::OnApplyBodyOnlyClicked))
		);
	}
...
void SMetaHumanCharacterAssetViewItem::OnApplyHeadOnlyClicked()
{
	OnApplyHeadOnlyDelegate.ExecuteIfBound(AssetItem);
}

void SMetaHumanCharacterAssetViewItem::OnApplyBodyOnlyClicked()
{
	OnApplyBodyOnlyDelegate.ExecuteIfBound(AssetItem);
}
...
const FSlateBrush* SMetaHumanCharacterAssetViewItem::GetThumbnailImageOverrideBrush() const
{
	if (AssetItem.IsValid() && AssetItem->ThumbnailImageOverride.IsValid() && AssetItem->AssetData.IsValid())
	{
		return AssetItem->ThumbnailImageOverride->GetSlateBrush();
	}
...
ECheckBoxState SMetaHumanCharacterAssetViewItem::GetThumbnailTypeCheckState() const
{
	return AssetItem.IsValid() && AssetItem->ThumbnailType == EMetaHumanCharacterThumbnailCameraPosition::Face ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}

void SMetaHumanCharacterAssetViewItem::OnThumbnailTypeCheckStateChanged(ECheckBoxState NewState)
{
	if (AssetItem.IsValid() && AssetItem->AssetData.IsValid())
	{
		AssetItem->ThumbnailType = NewState == ECheckBoxState::Checked ? EMetaHumanCharacterThumbnailCameraPosition::Face : EMetaHumanCharacterThumbnailCameraPosition::Body;
		OnOverrideThumbnailDelegate.ExecuteIfBound(AssetItem);
	}
}
...
EVisibility SMetaHumanCharacterAssetViewItem::GetThumbnailTypeCheckBoxVisibility() const
{
	return bShowThumbnailType ? EVisibility::Visible : EVisibility::Collapsed;
}
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.AutoWrapText(true)
					.Text(LOCTEXT("DragAssetHint", "Drag a Folder or compatible Asset here."))
...
TSharedRef<ITableRow> SMetaHumanCharacterEditorAssetsView::OnGenerateTile(TSharedPtr<FMetaHumanCharacterAssetViewItem> InItem, const TSharedRef<STableViewBase>& OwnerTable) const
{
	...
	{
		InItem->ThumbnailType = MetaHumanEditorSettings->PresetsThumbnailType;
	}
	...
	TSharedRef<SMetaHumanCharacterAssetViewItem> Item =
		...
		.ShowThumbnailType(bShowThumbnailType)
		.AssetItem(InItem)
		...
		.OnApplyHeadOnly(OnItemApplyHeadOnlyDelegate)
		.OnApplyBodyOnly(OnItemApplyBodyOnlyDelegate)
		.ToolTipText(LOCTEXT("AssetViewItem_Tooltip", "Double click to apply to MetaHuman."));
...
void SMetaHumanCharacterEditorAssetsView::HandleVirtualFolderAsset(const FAssetData& AssetData)
{
	...
	{
		PaletteKey = FMetaHumanPaletteItemKey::FromPrincipalAsset(TSoftObjectPtr<UObject>(AssetData.GetAsset()), NAME_None);
	}
...
void SMetaHumanCharacterEditorAssetViewsPanel::Construct(const FArguments& InArgs)
{
	...
	OnGetCustomMenuOptionsDelegate = InArgs._OnGetCustomMenuOptions;
		...
		[
			...
			[
				...
				[
					...
					.ToolTipText(LOCTEXT("AssetViewSettingsButtonTooltip", "Assets management options"))
					.ButtonContent()
					[
						SNew(SImage)
	...
	// Register to thumbnail type delegate to update the asset views when it is changed from the editor settings
	UMetaHumanCharacterEditorSettings* MetaHumanEditorSettings = GetMutableDefault<UMetaHumanCharacterEditorSettings>();
	if (bShowThumbnailType && !MetaHumanEditorSettings->GetOnThumbnailTypeChanged().IsBoundToObject(this))
	{
		MetaHumanEditorSettings->GetOnThumbnailTypeChanged().AddSP(this, &SMetaHumanCharacterEditorAssetViewsPanel::OnThumbnailTypeChanged);
	}
}

TArray<TSharedPtr<FMetaHumanCharacterAssetViewItem>> SMetaHumanCharacterEditorAssetViewsPanel::GetAllItems() const
{
	TArray<TSharedPtr<FMetaHumanCharacterAssetViewItem>> AllItems;
	for (const TSharedPtr<SMetaHumanCharacterEditorAssetsView>& AssetView : AssetViews)
	{
		...
		TArray<TSharedPtr<FMetaHumanCharacterAssetViewItem>> Items = AssetView->GetItems();
		if (Items.IsEmpty())
		{
			continue;
		}
...
UTexture2D* SMetaHumanCharacterEditorAssetViewsPanel::ThumbnailToTexture(const FObjectThumbnail* InThumbnailObject)
{
...
UTexture2D* SMetaHumanCharacterEditorAssetViewsPanel::LoadThumbnailAsTextureFromAssetData(const FAssetData& InAssetData, EMetaHumanCharacterThumbnailCameraPosition InCameraPosition)
{
...
TSharedRef<SWidget> SMetaHumanCharacterEditorAssetViewsPanel::GenerateAssetViewsSlot(const FName& SlotName)
{
	...
	// Add individual assets section at the top
	...
	Sections.Insert(IndividualAssetsSection, 0);
...
TSharedRef<SWidget> SMetaHumanCharacterEditorAssetViewsPanel::GenerateSettingsMenuWidget()
{
	...
	if (!ToolMenus->IsMenuRegistered(MenuName))
	{
		...
		if (bShowThumbnailType)
		{
			Section.AddSubMenu(
				TEXT("ThumbnailType"),
				LOCTEXT("BlendToolPanelMenuOptions_ThumbnailTypeLabel", "Thumbnail Type"),
				LOCTEXT("BlendToolPanelMenuOptions_ThumbnailTypeToolTip", "Set the default thumbnail type for all the assets"),
				FNewToolMenuDelegate::CreateLambda([this](UToolMenu* SubMenu)
					{
						FToolMenuSection& SubSection = SubMenu->FindOrAddSection("ThumbailType");

						auto AddRadioButtonEntryLambda = [this, &SubSection](FName Name, FText Label, EMetaHumanCharacterThumbnailCameraPosition Value)
							{
								SubSection.AddMenuEntry(
									...
									(
										FExecuteAction::CreateSP(this, &SMetaHumanCharacterEditorAssetViewsPanel::OnSetCharacterThumbnailType, Value),
										FCanExecuteAction(),
										FGetActionCheckState::CreateSP(this, &SMetaHumanCharacterEditorAssetViewsPanel::IsCharacterThumbnailTypeChecked, Value)
									)),
									EUserInterfaceActionType::RadioButton
								);
							};

						AddRadioButtonEntryLambda(TEXT("Face"), LOCTEXT("BlendToolPanelMenuOptions_ThumbnailTypeFace", "Face"), EMetaHumanCharacterThumbnailCameraPosition::Face);
						AddRadioButtonEntryLambda(TEXT("Body"), LOCTEXT("BlendToolPanelMenuOptions_ThumbnailTypeBody", "Body"), EMetaHumanCharacterThumbnailCameraPosition::Body);
					})
			);
		}

		if (OnGetCustomMenuOptionsDelegate.IsBound())
		{
			Section.AddSeparator(NAME_None);
			OnGetCustomMenuOptionsDelegate.Execute(Section);
		}
...
void SMetaHumanCharacterEditorAssetViewsPanel::OnThumbnailTypeChanged()
{
	const UMetaHumanCharacterEditorSettings* MetaHumanEditorSettings = GetDefault<UMetaHumanCharacterEditorSettings>();
	if (!MetaHumanEditorSettings)
	{
	...
	const TArray<TSharedPtr<FMetaHumanCharacterAssetViewItem>> Items = GetAllItems();
	for (TSharedPtr<FMetaHumanCharacterAssetViewItem> Item : Items)
	{
		if (Item.IsValid())
		{
			Item->ThumbnailType = MetaHumanEditorSettings->PresetsThumbnailType;
			OnOverrideThumbnailDelegate.ExecuteIfBound(Item);
		}
	}
}

void SMetaHumanCharacterEditorAssetViewsPanel::OnSetCharacterThumbnailType(EMetaHumanCharacterThumbnailCameraPosition NewThumbnailType)
{
	UMetaHumanCharacterEditorSettings* MetaHumanEditorSettings = GetMutableDefault<UMetaHumanCharacterEditorSettings>();
	if (MetaHumanEditorSettings)
	{
		MetaHumanEditorSettings->PresetsThumbnailType = NewThumbnailType;
		MetaHumanEditorSettings->GetOnThumbnailTypeChanged().Broadcast();
	}
}

ECheckBoxState SMetaHumanCharacterEditorAssetViewsPanel::IsCharacterThumbnailTypeChecked(EMetaHumanCharacterThumbnailCameraPosition ThumbnailType) const
{
	const UMetaHumanCharacterEditorSettings* MetaHumanEditorSettings = GetDefault<UMetaHumanCharacterEditorSettings>();
	return MetaHumanEditorSettings && MetaHumanEditorSettings->PresetsThumbnailType == ThumbnailType ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}
