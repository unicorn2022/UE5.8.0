...
protected:
	...
	TSharedRef<SWidget> CreatePropertySpinBoxWidget(const FText& LabelText, FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr, const int32 FractionalDigits = 2);
	...
	TSharedRef<SWidget> CreatePropertySpinBoxWidgetNormalized(FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr, float InMinValue = -1.0f, float InMaxValue = -1.0f, const FText& InLabelOverride = FText::GetEmpty());

	/** Creates a check box widget for the given Bool Property, if valid. */
	TSharedRef<SWidget> CreatePropertyCheckBoxWidget(const FText& LabelText, FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr);

	/** Creates a combo box widget for the given Enum Property, if valid. */
	template<typename TEnum>
	TSharedRef<SWidget> CreatePropertyComboBoxWidget(const FText& LabelText, TEnum SelectedItem, FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr)
	{
			...
			[
				...
				[
					...
					[
						...
						.CurrentValue_Lambda([this, Property, PropertyContainerPtr]()
						{
							return GetEnumPropertyValue<TEnum>(Property, PropertyContainerPtr);
						})
	...
	TSharedRef<SWidget> CreatePropertyColorPickerWidget(const FText& LabelText, FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr);
	...
	TSharedRef<SWidget> CreatePropertyUVColorPickerWidget(FProperty* PropertyU,
														  FProperty* PropertyV,
														  void* PropertyContainerPtr,
														  const void* DefaultContainerPtr,
	...
	/** Creates the default widget for the given Property, if valid. */
	TSharedRef<SWidget> CreatePropertyDefaultWidget(const FText& LabelText, FProperty* Property, UObject* PropertyContainerObject, FNotifyHook* NotifyHook = nullptr);

	/** Creates a reset to default button for the given Property, if valid. */
	TSharedRef<SWidget> CreatePropertyResetButton(FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr);
	...
	template<typename TEnum>
	TEnum GetEnumPropertyValue(FProperty* Property, void* PropertyContainerPtr) const
	{
		static_assert(std::is_enum_v<TEnum>, "TEnum must be an enum type");
		using UnderT = std::underlying_type_t<TEnum>;
		...
		if (const FEnumProperty* EnumProperty = CastField<FEnumProperty>(Property))
		{
			...
			const void* ValuePtr = EnumProperty->ContainerPtrToValuePtr<void>(PropertyContainerPtr);
			PropertyValue = UnderlyingProp->GetUnsignedIntPropertyValue(ValuePtr);
		}
		else if (const FByteProperty* ByteProperty = CastField<FByteProperty>(Property))
		{
			const void* ValuePtr = ByteProperty->ContainerPtrToValuePtr<void>(PropertyContainerPtr);
			PropertyValue = ByteProperty->GetPropertyValue(ValuePtr);
		}

		return static_cast<TEnum>(static_cast<UnderT>(PropertyValue));
	}
	...
	virtual void OnPostEditChangeProperty(FProperty* Property, bool bIsInteractive);
	...
	/** Called when the reset to default button is clicked for a property. */
	FReply OnPropertyResetToDefaultClicked(FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr);

	/** Gets the visibility of the reset to default button for a property. */
	EVisibility GetPropertyResetVisibility(FProperty* Property, void* PropertyContainerPtr, const void* DefaultContainerPtr) const;
	...
	/** Cancels any outstanding property change transaction. */
	void CancelOutstandingTransaction();
