...
private:
	...
	TArray<FMetaHumanCharacterAssetsSection> GetWardrobeAssetViewsSections(const UMetaHumanCharacter* InCharacter, const UMetaHumanCharacterEditorPipelineSpecification* InSpec) const;
	...
	/** Updates character model settings */
	void UpdateAdditionalCharacterSettings(const FName& SlotName, TSharedPtr<FMetaHumanCharacterAssetViewItem> Item, bool bUnwear = false);

	/** 
	 * Must be called when the collection or its default instance have been modified.
	 * 
	 * If the modifications are part of a transaction, this function must be called within the 
	 * scope of the transaction, so that the edits to the Character's collection are also captured
	 * (not just the preview collection).
	 */
	void OnCollectionModified();

	/** Rebuild the preview collection to update the preview viewport */
	void RebuildPreviewCollection();
	...
	/** Extracts the SlotSelectionItem for a give ItemPath regardless of SlotName */
	void GetSlotSelectionItemForGivenItemPath(const FSoftObjectPath& ItemPath, const TArray<FMetaHumanPipelineSlotSelectionData>& InSlotSelectionData, FMetaHumanPipelineSlotSelection& InOutSlotSelectionItem) const;
