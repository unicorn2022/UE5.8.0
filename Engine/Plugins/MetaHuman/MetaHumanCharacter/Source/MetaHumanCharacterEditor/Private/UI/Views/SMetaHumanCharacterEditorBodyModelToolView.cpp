...
void SMetaHumanCharacterEditorBodyModelToolView::Construct(const FArguments& InArgs, UMetaHumanCharacterEditorToolWithSubTools* InTool)
{
	...
	bOptionalMetaHumanContentInstalled = FMetaHumanCharacterEditorModule::IsOptionalMetaHumanContentInstalled();
}
...
{
	const UMetaHumanCharacterEditorBodyModelTool* BodyModelTool = Cast<UMetaHumanCharacterEditorBodyModelTool>(Tool);
	UMetaHumanCharacterEditorHeadAndBodyParameterProperties* BodyParameterProperties = BodyModelTool ? BodyModelTool->BodyParameterProperties.Get() : nullptr;
	...
	if (!BodyParameterProperties || !ParametricBodyProperties)
	{
	...
	FProperty* ScaleMeasurementRangesWithHeightProperty = UMetaHumanCharacterParametricBodyProperties::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UMetaHumanCharacterParametricBodyProperties, bScaleRangesByHeight));
	FProperty* UnlockBodyRangesProperty = UMetaHumanCharacterParametricBodyProperties::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UMetaHumanCharacterParametricBodyProperties, bUnlockBodyRanges));
	FProperty* ParametersRangeMultiplierProperty = UMetaHumanCharacterParametricBodyProperties::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UMetaHumanCharacterParametricBodyProperties, ParametersRangeMultiplier));
	FProperty* DeltaProperty = UMetaHumanCharacterEditorHeadAndBodyParameterProperties::StaticClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UMetaHumanCharacterEditorHeadAndBodyParameterProperties, BodyDelta));

	const void* ParametricBodyPropertiesDefault = ParametricBodyProperties->GetClass()->GetDefaultObject<UMetaHumanCharacterParametricBodyProperties>();
	...
	TSharedRef<SWidget> ParametersRangeMultiplierWidget = CreatePropertySpinBoxWidget(ParametersRangeMultiplierProperty->GetDisplayNameText(), ParametersRangeMultiplierProperty, ParametricBodyProperties, ParametricBodyPropertiesDefault);
	ParametersRangeMultiplierWidget->SetEnabled(TAttribute<bool>::CreateLambda([ParametricBodyProperties]()
		{
			return ParametricBodyProperties && ParametricBodyProperties->bUnlockBodyRanges;
		}));
	...
	const TSharedRef<SWidget> ParametricConstraintsWidget =
		...
		[
			...
			[
				...
				+ SVerticalBox::Slot()
				[
					CreatePropertyCheckBoxWidget(ShowMeasurementsProperty->GetDisplayNameText(), ShowMeasurementsProperty, ParametricBodyProperties, ParametricBodyPropertiesDefault)
				]

				+ SVerticalBox::Slot()
				[
					CreatePropertyCheckBoxWidget(ScaleMeasurementRangesWithHeightProperty->GetDisplayNameText(), ScaleMeasurementRangesWithHeightProperty, ParametricBodyProperties, ParametricBodyPropertiesDefault)
				]

				+ SVerticalBox::Slot()
				[
					CreatePropertyCheckBoxWidget(UnlockBodyRangesProperty->GetDisplayNameText(), UnlockBodyRangesProperty, ParametricBodyProperties, ParametricBodyPropertiesDefault)
				]

				+ SVerticalBox::Slot()
				[
					ParametersRangeMultiplierWidget
				]

				+ SVerticalBox::Slot()
				.AutoHeight()
				[
					CreatePropertySpinBoxWidget(DeltaProperty->GetDisplayNameText(), DeltaProperty, BodyParameterProperties, BodyParameterPropertiesDefault)
				]
...
EVisibility SMetaHumanCharacterEditorBodyModelToolView::GetFixedCompatibilitySubToolViewVisibility() const
{
	...
	return (bIsVisible && bOptionalMetaHumanContentInstalled) ? EVisibility::Visible : EVisibility::Collapsed;
}
