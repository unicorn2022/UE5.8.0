	...
	const TSharedRef<SWidget> PresetsViewSectionWidget =
		...
		[
			...
			[
				...
				.OnOverrideThumbnail(this, &SMetaHumanCharacterEditorPresetsToolView::OnOverrideItemThumbnailBrush)
...
void SMetaHumanCharacterEditorPresetsToolView::OnOverrideItemThumbnailBrush(TSharedPtr<FMetaHumanCharacterAssetViewItem> Item) const
{
	if (!Item.IsValid())
	{
		return;
	...
	EMetaHumanCharacterThumbnailCameraPosition ThumbnailType = EMetaHumanCharacterThumbnailCameraPosition::Character_Face;
	if (Item->ThumbnailType == EMetaHumanCharacterThumbnailCameraPosition::Body)
	{
		ThumbnailType = EMetaHumanCharacterThumbnailCameraPosition::Character_Body;
	}

	if (UTexture2D* Texture = SMetaHumanCharacterEditorAssetViewsPanel::LoadThumbnailAsTextureFromAssetData(Item->AssetData, ThumbnailType))
	{
