...
TSharedRef<SWidget> SMetaHumanCharacterEditorWardrobeToolView::CreateWardrobeToolViewAssetViewsPanelSection()
{
	...
	const UMetaHumanCollectionEditorPipeline* EditorPipeline = WardrobeToolProperties->Collection->GetEditorPipeline();
	const UMetaHumanCharacterEditorPipelineSpecification* Specification = EditorPipeline ? &*EditorPipeline->GetSpecification() : nullptr;
...
void SMetaHumanCharacterEditorWardrobeToolView::CreateWardrobeItemsForCompatibleAssets(const FMetaHumanCharacterAssetsSection& Section)
{
	...
	const UMetaHumanCollectionEditorPipeline* EditorPipeline = Collection->GetEditorPipeline();
	const FMetaHumanCharacterPipelineSlotEditorData* SlotEditorEntry = EditorPipeline ? EditorPipeline->GetSpecification()->SlotEditorData.Find(Section.SlotName) : nullptr;
	if (!SlotEditorEntry)
	{
		return;
	}
...
TArray<FMetaHumanCharacterAssetViewItem> SMetaHumanCharacterEditorWardrobeToolView::GetWardrobeIndividualAssets(const FName& SlotName) const
{
	...
	for (TSoftObjectPtr<UMetaHumanWardrobeItem> Item : IndividualAssets->Items)
	{
		...
		const bool bIsItemValid = Collection->GetEditorPipeline()->TestWardrobeItemCompatibilityWithSlot(SlotName, Item.Get()) == EMetaHumanWardrobeItemCompatibility::Add;
...
TArray<FMetaHumanCharacterAssetsSection> SMetaHumanCharacterEditorWardrobeToolView::GetWardrobeAssetViewsSections(const UMetaHumanCharacter* InCharacter, const UMetaHumanCharacterEditorPipelineSpecification* InSpec) const
{
	if (!InSpec)
	{
		return TArray<FMetaHumanCharacterAssetsSection>();
	}
	...
	InSpec->SlotEditorData.GetKeys(SpecificationSlotNames);
	return Sections.FilterByPredicate([InSpec, SpecificationSlotNames](const FMetaHumanCharacterAssetsSection& Section)
		{
			...
			const FMetaHumanCharacterPipelineSlotEditorData* SlotEditorEntry = InSpec->SlotEditorData.Find(Section.SlotName);
			if (!SlotEditorEntry)
			{
				return false;
			}
			...
			{
				if (!FilterClass || !SlotEditorEntry->SupportsAssetType(FilterClass))
				{
					return false;
				}
			}
...
bool SMetaHumanCharacterEditorWardrobeToolView::IsItemCompatible(TSharedPtr<FMetaHumanCharacterAssetViewItem> Item, const FMetaHumanCharacterAssetsSection& Section) const
{
	...
	{
		if (const FMetaHumanCharacterPipelineSlotEditorData* SlotEditorEntry = EditorPipeline->GetSpecification()->SlotEditorData.Find(Item->SlotName))
		{
			...
			{
				return SlotEditorEntry->SupportsAsset(PrincipalAssetData);
			}
		}
	}
...
UObject* SMetaHumanCharacterEditorWardrobeToolView::OnProcessDroppedItem(const FAssetData& AssetData)
{
	...
	UMetaHumanCharacterEditorWardrobeToolProperties* WardrobeToolProperties = Cast<UMetaHumanCharacterEditorWardrobeToolProperties>(GetToolProperties());
	const UMetaHumanCollectionPipeline* CollectionPipeline = WardrobeToolProperties->Collection->GetPipeline();
	const UMetaHumanCollectionEditorPipeline* EditorPipeline = WardrobeToolProperties->Collection->GetEditorPipeline();
	if (!EditorPipeline)
	{
		return nullptr;
	...
	for (const TPair<FName, FMetaHumanCharacterPipelineSlotEditorData>& SlotIt : EditorPipeline->GetSpecification()->SlotEditorData)
	{
		const FMetaHumanCharacterPipelineSlotEditorData& Slot = SlotIt.Value;

		if (Slot.SupportsAssetType(AssetObject->GetClass()))
		{
			...
			const TSoftClassPtr<UObject>* MatchedType = Slot.SupportedPrincipalAssetTypes.FindByPredicate(
				[AssetObject](const TSoftClassPtr<UObject>& SupportedType)
				{
					const UClass* SupportedClass = SupportedType.LoadSynchronous();
					return SupportedClass && AssetObject->GetClass()->IsChildOf(SupportedClass);
				});

			if (MatchedType)
			{
				if (const UMetaHumanItemPipeline* FallbackPipeline = CollectionPipeline->GetFallbackItemPipelineForAssetType(*MatchedType))
				{
					UMetaHumanItemPipeline* NewPipeline = NewObject<UMetaHumanItemPipeline>(NewWardrobeItem, FallbackPipeline->GetClass());
...
void SMetaHumanCharacterEditorWardrobeToolView::OnProcessDroppedFolders(const TArray<FContentBrowserItem> Items, const FMetaHumanCharacterAssetsSection& InSection) const
{
	...
	const UMetaHumanCollectionEditorPipeline* EditorPipeline = Collection->GetEditorPipeline();
	const FMetaHumanCharacterPipelineSlotEditorData* SlotEditorEntry = EditorPipeline ? EditorPipeline->GetSpecification()->SlotEditorData.Find(InSection.SlotName) : nullptr;
	if (!SlotEditorEntry)
	{
		return;
	}
	
	const TArray<TSoftClassPtr<UObject>> SupportedTypes = SlotEditorEntry->SupportedPrincipalAssetTypes;
...
TArray<FMetaHumanCharacterAssetViewItem> SMetaHumanCharacterEditorWardrobeToolView::OnPopulateAssetViewsItems(const FMetaHumanCharacterAssetsSection& InSection, const FMetaHumanObserverChanges& InChanges)
{
	...
	{
		bool bWasCollectionEdited = false;
		...
		for (int32 Index = 0; Index < DeletedAssets->Num(); Index++)
		{
			...
			for (int32 ItemIndex = PaletteItems.Num() - 1; ItemIndex >= 0; ItemIndex--)
			{
					...
					Item.WardrobeItem->PrincipalAsset == Asset)
				{
					...
					bWasCollectionEdited = true;
				}
			}
		}

		if (bWasCollectionEdited)
		{
	...
	for (const FAssetData& Asset : FoundAssets)
	{
		...
		if (Asset.IsInstanceOf<UMetaHumanWardrobeItem>() && Asset.IsAssetLoaded())
		{
			...
			if (WardrobeItem->PrincipalAsset.IsValid() || FoundItem)
			{
				...
				bIsItemValid = Collection->GetEditorPipeline()->TestWardrobeItemCompatibilityWithSlot(SlotName, WardrobeItem) == EMetaHumanWardrobeItemCompatibility::Add;
			}
		}
...
void SMetaHumanCharacterEditorWardrobeToolView::OnWardrobeToolVirtualItemDeleted(TSharedPtr<FMetaHumanCharacterAssetViewItem> Item)
{
	...
	{
		const FScopedTransaction DeleteWardrobeItemTransaction(LOCTEXT("DeleteWardrobeItem", "Delete Wardrobe Item"));

		// 1. If the character is currently wearing this item, unwear it first.
		//    ApplyWearRequest is a no-op when the item isn't selected on the character.
		ApplyWearRequest({ Item }, EWearRequest::Unwear);

		// 2. If the item is prepared (in the palette), unprepare it now that it's no longer worn.
		if (!Item->PaletteItemKey.IsNull())
		{
			UnprepareAsset(Item);
		}

		// 3. Remove the asset from the character's individual wardrobe assets.
		Character->Modify();
...
bool SMetaHumanCharacterEditorWardrobeToolView::ApplyWearRequest(const TArray<TSharedPtr<FMetaHumanCharacterAssetViewItem>>& Items, EWearRequest WearRequest)
{
	...
	TNotNull<UMetaHumanInstance*> Instance = Collection->GetMutableDefaultInstance();
	...
	for (const TSharedPtr<FMetaHumanCharacterAssetViewItem>& Item : Items)
	{
		...
		const bool bIsWardrobeItemItemCompatible = Collection->GetEditorPipeline()->TestWardrobeItemCompatibilityWithSlot(Item->SlotName, WardrobeItem) == EMetaHumanWardrobeItemCompatibility::Add;
			...
			if (SlotSpec->bAllowsMultipleSelection)
			{
				for (const TSharedRef<FMetaHumanCharacterAssetViewItem>& Item : Kvp.Value)
				{
					switch (WearRequest)
					{
						...
						{
							FMetaHumanPipelineSlotSelection SlotSelectionItem(SlotName, Item->PaletteItemKey);
							GetSlotSelectionItemForGivenItemPath(Item->AssetData.GetSoftObjectPath(), Instance->GetSlotSelectionData(), SlotSelectionItem);
			...
			else
			{
				switch (WearRequest)
				{
					case EWearRequest::Wear:
					{
						...
						bCollectionUpdated = true;
						// Update aditional settings
						UpdateAdditionalCharacterSettings(SlotName, Item);
						RECORD_WARDROBE_ITEM_WORN_EVENT();
						break;
					}
					case EWearRequest::Unwear:
					{
						...
						if (Instance->TryGetAnySlotSelection(SlotName, CurrentSelectionItemKey))
						{
							...
							if (bHasItem)
							{
								...
								bCollectionUpdated = true;
								UpdateAdditionalCharacterSettings(SlotName, nullptr, /*bUnwear=*/true);
								break;
							}
						}
						break;
...
void SMetaHumanCharacterEditorWardrobeToolView::UpdateAdditionalCharacterSettings(const FName& SlotName, TSharedPtr<FMetaHumanCharacterAssetViewItem> Item, bool bUnwear)
{
	if (SlotName == "Eyelashes")
	{
		UMetaHumanCharacterEditorWardrobeTool* WardrobeTool = Cast<UMetaHumanCharacterEditorWardrobeTool>(Tool);
		UMetaHumanCharacter* Character = WardrobeTool ? UE::ToolTarget::GetTargetMetaHumanCharacter(WardrobeTool->GetTarget()) : nullptr;
		if (!Character)
		...
		if (!bUnwear)
		{
			...
			const UMetaHumanCharacterEditorWardrobeSettings* WardrobeSettings = GetDefault<UMetaHumanCharacterEditorWardrobeSettings>();
			for (const TPair<EMetaHumanCharacterEyelashesType, FSoftObjectPath>& EyelashesItem : WardrobeSettings->EyelashesTypeToAssetPath)
			{
				if (Item->AssetData.GetSoftObjectPath() == EyelashesItem.Value)
				{
					UMetaHumanCharacterEditorSubsystem* Subsystem = UMetaHumanCharacterEditorSubsystem::Get();
					Character->HeadModelSettings.Eyelashes.Type = EyelashesItem.Key;
					Character->HeadModelSettings.Eyelashes.bEnableGrooms = true;
					Subsystem->ApplyHeadModelSettings(Character, Character->HeadModelSettings, /*bIgnoreGrooms=*/true);
					break;
				}
			}
		}
		...
		{
			UMetaHumanCharacterEditorSubsystem* Subsystem = UMetaHumanCharacterEditorSubsystem::Get();
			Character->HeadModelSettings.Eyelashes.Type = EMetaHumanCharacterEyelashesType::None;
			Subsystem->ApplyHeadModelSettings(Character, Character->HeadModelSettings, /*bIgnoreGrooms=*/true);
		}
	}
}

void SMetaHumanCharacterEditorWardrobeToolView::OnCollectionModified()
{
	UMetaHumanCharacterEditorWardrobeToolProperties* WardrobeToolProperties = Cast<UMetaHumanCharacterEditorWardrobeToolProperties>(GetToolProperties());
	UMetaHumanCharacter* Character = WardrobeToolProperties ? WardrobeToolProperties->Character.Get() : nullptr;

	if (Character)
	{
		UMetaHumanCharacterEditorSubsystem* Subsystem = UMetaHumanCharacterEditorSubsystem::Get();

		Subsystem->OnEditPreviewCollection(Character);
	}
}

void SMetaHumanCharacterEditorWardrobeToolView::RebuildPreviewCollection()
{
	UMetaHumanCharacterEditorWardrobeToolProperties* WardrobeToolProperties = Cast<UMetaHumanCharacterEditorWardrobeToolProperties>(GetToolProperties());
	UMetaHumanCharacter* Character = WardrobeToolProperties ? WardrobeToolProperties->Character.Get() : nullptr;
...
void SMetaHumanCharacterEditorWardrobeToolView::UnprepareAsset(TSharedPtr<FMetaHumanCharacterAssetViewItem> Item)
{
	...
	FMetaHumanPipelineSlotSelection SlotSelectionItem(Item->SlotName, Item->PaletteItemKey);
	GetSlotSelectionItemForGivenItemPath(Item->AssetData.GetSoftObjectPath(), Instance->GetSlotSelectionData(), SlotSelectionItem);
	Instance->TryRemoveSlotSelection(SlotSelectionItem);
...
void SMetaHumanCharacterEditorWardrobeToolView::GetSlotSelectionItemForGivenItemPath(const FSoftObjectPath& ItemPath, const TArray<FMetaHumanPipelineSlotSelectionData>& InSlotSelectionData, FMetaHumanPipelineSlotSelection& InOutSlotSelectionItem) const
{
	for (const FMetaHumanPipelineSlotSelectionData& SlotSelectionData : InSlotSelectionData)
	{
		FEditorOnlyAssetReference OutWardrobeItem;
		if (SlotSelectionData.Selection.SelectedItem.TryGetExternalWardrobeItem(OutWardrobeItem) && OutWardrobeItem.IsValid() && OutWardrobeItem.ToSoftObjectPath() == ItemPath)
		{
			InOutSlotSelectionItem = SlotSelectionData.Selection;
			break;
		}
	}
}
