...
public:
	...
	TSharedPtr<STileView<TSharedPtr<TEnum>>> GetTileView() const
	{
		return TileView;
	}

	const TArray<TSharedPtr<TEnum>>& GetTileViewItems() const
	{
		return TileViewItems;
	}
...
private:
	...
	TSharedRef<ITableRow> OnGenerateTile(TSharedPtr<TEnum>InItem, const TSharedRef<class STableViewBase>& InOwnerTable) const
	{
		...
		FText LabelText = UEnum::GetDisplayValueAsText(*InItem);
		if (const UEnum* Enum = StaticEnum<TEnum>())
		{
			LabelText = Enum->GetDisplayNameTextByValue(static_cast<int64>(static_cast<FEnumType>(*InItem)));
		}
