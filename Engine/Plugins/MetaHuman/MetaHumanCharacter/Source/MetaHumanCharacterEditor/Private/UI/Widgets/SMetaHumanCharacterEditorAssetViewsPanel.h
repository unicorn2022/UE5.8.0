...
struct FMetaHumanCharacterAssetViewItem
{
	...
	EMetaHumanCharacterThumbnailCameraPosition ThumbnailType = EMetaHumanCharacterThumbnailCameraPosition::Face;
	bool bIsValid = true;
};
...
public:
	SLATE_BEGIN_ARGS(SMetaHumanCharacterAssetViewItem) 
		: _ShowThumbnailType(false)
		{}

		/** True if the thumbnail type toggle should be shown in the item. */
		SLATE_ARGUMENT(bool, ShowThumbnailType)
		...
		/** Called when "Apply Head Only" is selected from the context menu. */
		SLATE_EVENT(FMetaHumanCharacterEditorAssetViewItemDelegate, OnApplyHeadOnly)

		/** Called when "Apply Body Only" is selected from the context menu. */
		SLATE_EVENT(FMetaHumanCharacterEditorAssetViewItemDelegate, OnApplyBodyOnly)
...
private:
	...
	/** Executes the OnApplyHeadOnly delegate for this item. */
	void OnApplyHeadOnlyClicked();

	/** Executes the OnApplyBodyOnly delegate for this item. */
	void OnApplyBodyOnlyClicked();
	...
	/** Gets the thumbnail image brush. */
	const FSlateBrush* GetThumbnailImageOverrideBrush() const;
	...
	/** Gets the thumbnail type check state. */
	ECheckBoxState GetThumbnailTypeCheckState() const;
	
	/** Called when the thumbnail type check state is changed. */
	void OnThumbnailTypeCheckStateChanged(ECheckBoxState NewState);
	...
	/** Gets the visibility of the thumbnail type check box. */
	EVisibility GetThumbnailTypeCheckBoxVisibility() const;
	...
	/** The delegate to execute when "Apply Head Only" is selected from the context menu. */
	FMetaHumanCharacterEditorAssetViewItemDelegate OnApplyHeadOnlyDelegate;

	/** The delegate to execute when "Apply Body Only" is selected from the context menu. */
	FMetaHumanCharacterEditorAssetViewItemDelegate OnApplyBodyOnlyDelegate;
...
public:
	SLATE_BEGIN_ARGS(SMetaHumanCharacterEditorAssetsView)
		...
		/** True if the thumbnail type toggle should be shown in the asset view items. */
		SLATE_ARGUMENT(bool, ShowThumbnailType)
...
public:
	SLATE_BEGIN_ARGS(SMetaHumanCharacterEditorAssetViewsPanel)
		...
		/** Called to add custom menu options. */
		SLATE_EVENT(FNewToolMenuSectionDelegate, OnGetCustomMenuOptions)
	...
	/** Gets the array of all asset view items of this panel. */
	TArray<TSharedPtr<FMetaHumanCharacterAssetViewItem>> GetAllItems() const;
	...
	/** Loads the thumbnail of an asset as a texture, using the given camera position. */
	static UTexture2D* LoadThumbnailAsTextureFromAssetData(const FAssetData& InAssetData, EMetaHumanCharacterThumbnailCameraPosition InCameraPosition);
...
private:
	...
	/** Called when the thumbnail type has changed. */
	void OnThumbnailTypeChanged();

	/** Called when the user changes the thumbnail type to be displayed for the character. */
	void OnSetCharacterThumbnailType(EMetaHumanCharacterThumbnailCameraPosition NewThumbnailType);

	/** Gets the check state of the given thumbnail type. */
	ECheckBoxState IsCharacterThumbnailTypeChecked(EMetaHumanCharacterThumbnailCameraPosition ThumbnailType) const;
	...
	/** Gets the thumbnail texture from a given thumbnail object. */
	static UTexture2D* ThumbnailToTexture(const class FObjectThumbnail* InThumbnailObject);

	/** The delegate to execute to add custom menu options. */
	FNewToolMenuSectionDelegate OnGetCustomMenuOptionsDelegate;
