...
private:
	...
	/** Creates the subsection widget for showing the Hands & Feet Skin properties. */
	TSharedRef<SWidget> CreateSkinToolViewHandsAndFeetSkinSubSection();
	...
	/** Gets the default accent region Property Container pointer from the current panel selection. */
	const void* GetAccentRegionDefaultPropertyContainerFromSelection() const;
	...
	/** Called when the reset to default button is clicked for an accent region related Property. */
	FReply OnAccentRegionPropertyResetToDefaultClicked(FProperty* Property);

	/** Gets the visibility of the reset to default button for an accent region related Property. */
	EVisibility GetAccentRegionPropertyResetVisibility(FProperty* Property) const;
