...
float SampleHiddenFaceMap(const FImage& Image, int32 PixelX, int32 PixelY)
{
	const FLinearColor Color = Image.GetOnePixelLinear(PixelX, PixelY);
	// Note that the alpha value is not used
	return Color.R;
}

FIntVector2 GetHiddenFaceMapPixelForUV(const FVector2f& UV, float ResX, float ResY)
{
	// Add a small number that acts as a threshold when converting the UV into a pixel coordinate.
	const FVector2f OffsetUV = UV + KINDA_SMALL_NUMBER;
	return FIntVector2(
		FMath::FloorToInt32(FMath::Wrap(OffsetUV.X * ResX, 0.0f, ResX - 1.0f)),
		FMath::FloorToInt32(FMath::Wrap(OffsetUV.Y * ResY, 0.0f, ResY - 1.0f)));
}
...
bool ShouldHideTriangle(
	...
	float MaxCullValue)
{
	const float ResX = static_cast<float>(Image.SizeX);
	const float ResY = static_cast<float>(Image.SizeY);
	...
	{
		return SampleHiddenFaceMap(Image, MinU, MinV) <= MaxCullValue;
	}
	...
	for (int32 PosY = MinV; PosY <= MaxV; ++PosY)
	{
		...
		{
			const bool bStripPixel = SampleHiddenFaceMap(Image, PosX, PosY) <= MaxCullValue;
...
float ComputeShrinkWeight(float PixelValue, float MaxCullValue, float MinKeepValue)
{
	...
	return 1.0f - FMath::Clamp((PixelValue - MaxCullValue) / (MinKeepValue - MaxCullValue), 0.0f, 1.0f);
}
	...
	TConstArrayView<FName> MaterialSlotsToProcess)
{
	...
	// Post edit change scope
	{
		...
		for (int32 TriangleIndex = NumTriangleIndex - 1; TriangleIndex >= 0; TriangleIndex -= 3)
		{
			...
			if (ShouldHideTriangle(UvA, UvB, UvC, HiddenFaceMap.Image, MaxCullValue))
			{
		...
		if (MaxShrinkDistance > 0.0f && MinKeepValue > MaxCullValue)
		{
			...
			for (int32 VertexIndex = 0; VertexIndex < (int32)LODModel.NumVertices; VertexIndex++)
			{
				...
				const FIntVector2 PixelUV = GetHiddenFaceMapPixelForUV(Vertex.UVs[0], ResX, ResY);

				const float PixelValue = SampleHiddenFaceMap(HiddenFaceMap.Image, PixelUV.X, PixelUV.Y);
				const float ShrinkWeight = ComputeShrinkWeight(PixelValue, MaxCullValue, MinKeepValue);
				...
				if (ShrinkWeight > 0.0f)
				{
					...
					// used on.
					Point.bShouldBeCulled = IsPixelCulled(PixelValue, MaxCullValue);
