...
{
	static TAutoConsoleVariable<bool> CVarUseDefaultEditor(
		TEXT("mh.Character.UseDefaultEditor"),
		false,
		TEXT("When true, opens MetaHuman Characters in the default details panel editor instead of the custom asset editor."),
		ECVF_Default);
}
...
namespace MenuExtension_MetaHumanCharacter
{
			...
			for (UMetaHumanCharacter* Character : Characters)
			{
				...
				if (UMetaHumanCollection* Collection = Character->GetMutableInternalCollection())
				{
					...
					for (const FMetaHumanCharacterPaletteItem& Item : Collection->GetItems())
					{
						...
						TNotNull<const UMetaHumanInstance*> Instance = Collection->GetDefaultInstance();
					...
					// Build the set of wardrobe-item soft paths still referenced by surviving palette items, per slot.
					TMap<FName, TSet<FSoftObjectPath>> KeptPathsPerSlot;
					for (const FMetaHumanCharacterPaletteItem& Item : Collection->GetItems())
					{
						FEditorOnlyAssetReference ExternalWardrobeItem;
						if (Item.GetItemKey().TryGetExternalWardrobeItem(ExternalWardrobeItem))
						{
							KeptPathsPerSlot.FindOrAdd(Item.SlotName).Add(ExternalWardrobeItem.ToSoftObjectPath());
						}
					}

					// Prune WardrobeIndividualAssets: drop entries no longer referenced by any surviving palette item,
					// then drop slot entries left empty.
					for (auto SlotIt = Character->WardrobeIndividualAssets.CreateIterator(); SlotIt; ++SlotIt)
					{
						const TSet<FSoftObjectPath>* KeptPaths = KeptPathsPerSlot.Find(SlotIt.Key());

						const int32 RemovedCount = SlotIt.Value().Items.RemoveAll(
							[KeptPaths](const TSoftObjectPtr<UMetaHumanWardrobeItem>& WardrobeItem)
							{
								return !KeptPaths || !KeptPaths->Contains(WardrobeItem.ToSoftObjectPath());
							});
	...
	static void ExecuteSpawnActor(const UContentBrowserAssetContextMenuContext* InCBContext)
	{
		const TArray<UMetaHumanCharacter*> Characters = InCBContext->LoadSelectedObjects<UMetaHumanCharacter>();
		...
		{
			UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>();
			AssetEditorSubsystem->OpenEditorForAsset(Character);

			UMetaHumanCharacterEditorSubsystem::Get()->SpawnMetaHumanActor(Character);
		}
	}
							...
							if (Context && Context->SelectedAssets.Num() > 0)
							{
								FToolMenuSection& Section = InMenu->FindOrAddSection(TEXT("GetAssetActions"));
								{
									...
									}
									{
										...
										const FText Tooltip = LOCTEXT("MetaHumanCharacter_RemoveUnusedWardrobeItemsTooltip", "Removes references to unused wardrobe items from this character."
									...
									{
										const FText Label = LOCTEXT("MetaHumanCharacter_SpawnActor", "Spawn Preview Actor");
										const FSlateIcon Icon{ FAppStyle::GetAppStyleSetName(), "ClassIcon.MetaHumanCharacter" };
										const FUIAction UIAction = FUIAction(
											FExecuteAction::CreateStatic(&ExecuteSpawnActor, Context)
										);
										Section.AddMenuEntry(TEXT("MetaHumanCharacter_SpawnActor"), Label, FText::GetEmpty(), Icon, UIAction);
									}
								}
							}
						}
...
{
	if (UE::MetaHuman::CVarUseDefaultEditor.GetValueOnGameThread())
	{
	...
	for (UMetaHumanCharacter* MetaHumanCharacter : LoadedCharacters)
	{
		...
		if (const TObjectPtr<UMetaHumanCollection> Collection = MetaHumanCharacter->GetInternalCollection())
		{
			if (!IsValid(Collection->GetPipeline()))
			{
				const FText NoPipelineTitle = LOCTEXT("NoPipelineDialogTitle", "No Pipeline set on MetaHuman Character");
				const FText NoPipelineMessage = LOCTEXT("NoPipelineDialogMessage", "This MetaHuman Character does not have a Pipeline set on its Collection "
																				   "which means it can't be loaded. \n"
																				   "Would you like to reset it to the default Pipeline?");

				FSuppressableWarningDialog::FSetupInfo SetupInfo(NoPipelineMessage, NoPipelineTitle, TEXT("MetaHumanCharacterSuppressNoPipeline"));
				SetupInfo.ConfirmText = LOCTEXT("NoPipelineDialogConfirmText", "Yes");
				SetupInfo.CancelText = LOCTEXT("NoPipelineDialogCancelText", "Cancel");

				FSuppressableWarningDialog NoPipelineDialog{ SetupInfo };

				const FSuppressableWarningDialog::EResult Result = NoPipelineDialog.ShowModal();
				if (Result == FSuppressableWarningDialog::EResult::Cancel)
				{
					FMessageLog(UE::MetaHuman::MessageLogName)
						.Error(FailedToOpenCharacterBaseMessage)
						->AddToken(FUObjectToken::Create(MetaHumanCharacter))
						->AddText(NoPipelineTitle);
				...
				MetaHumanCharacter->GetMutableInternalCollection()->SetDefaultPipeline();
