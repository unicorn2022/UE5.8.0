...
void AMetaHumanInvisibleDrivingActor::SetDefaultBodySkeletalMesh()
{
	...
	LiveLinkAnimInstanceClass = TSoftClassPtr<UAnimInstance>(FSoftObjectPath(TEXT("/" UE_PLUGIN_NAME "/Animation/ABP_MH_LiveLink.ABP_MH_LiveLink_C")));
	PreviewAnimInstanceClass = TSoftClassPtr<UAnimInstance>(FSoftObjectPath(TEXT("/" UE_PLUGIN_NAME "/Animation/ABP_AnimationPreview.ABP_AnimationPreview_C")));
}
...
{
	if (USkeletalMeshComponent* SkelMeshComponent = GetSkeletalMeshComponent())
	{
		if (UClass* AnimInstanceClass = LiveLinkAnimInstanceClass.LoadSynchronous())
		{
			SkelMeshComponent->SetAnimInstanceClass(AnimInstanceClass);
...
void AMetaHumanInvisibleDrivingActor::InitPreviewAnimInstance()
{
	...
	if (ensure(SkelMeshComponent))
	{
		if (UClass* AnimInstanceClass = PreviewAnimInstanceClass.LoadSynchronous())
		{
			SkelMeshComponent->SetAnimInstanceClass(AnimInstanceClass);
		}
	}
