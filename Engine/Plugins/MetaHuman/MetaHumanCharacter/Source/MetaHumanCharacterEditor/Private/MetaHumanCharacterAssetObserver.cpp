...
namespace UE::MetaHuman::Private
{
	...
	TArray<FAssetData> GetAssetsInDirectory(const FName& InDirectory, const TSet<TSubclassOf<UObject>>& InClassesToFilter)
	{
		...
		const FARCompiledFilter FilterToUse = GetAssetFilter(InDirectory, InClassesToFilter);

		if (!FilterToUse.IsEmpty() && FAssetRegistryState::IsFilterValid(FilterToUse))
		{
			IAssetRegistry& AssetRegistry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry")).Get();
			AssetRegistry.GetAssets(FilterToUse, Assets);
		}
	...
	TArray<FAssetData> GetWardrobeItemsInDirectory(const FName& InDirectory, const TSet<TSubclassOf<UObject>>& InClassesToFilter)
	{
		...
		IAssetRegistry& AssetRegistry = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry")).Get();
		TArray<FAssetData> Assets = GetAssetsInDirectory(InDirectory, { UMetaHumanWardrobeItem::StaticClass() });
