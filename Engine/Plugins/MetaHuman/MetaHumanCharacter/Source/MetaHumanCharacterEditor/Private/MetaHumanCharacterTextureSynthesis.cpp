...
namespace UE::MetaHuman
{
	...
	// this updates PlatformData NOT Source
	static void CopySynthesizedDataToTexture2D(TConstArrayView<uint8> InSynthesizedRawData, UTexture2D* InOutTexture2D)
	{
		...
		InOutTexture2D->BlockOnAnyAsyncBuild();
...
void FMetaHumanCharacterTextureSynthesis::InitFaceTextureSynthesizer(FMetaHumanFaceTextureSynthesizer& OutFaceTextureSynthesizer)
{
	...
	if (!TextureSynthesisModelPath.IsEmpty())
	{
		...
		{
			UE_LOGFMT(LogMetaHumanCharacterEditor, Log, "Texture synthesis model loaded from settings override: {TextureSynthesisModelPath}", TextureSynthesisModelPath);
			return;
		}
		else
		{
	...
	{
		Plugin->GetContentDir() / TEXT("Optional/TextureSynthesis/TS-1.3-F_UE_res-1024_nchr-153"),
	};
...
UTexture2D* FMetaHumanCharacterTextureSynthesis::CreateFaceTextureFromSource(EFaceTextureType InTextureType, FImageView InTextureImage, bool bInMakeVirtualIfSupported)
{
	...
	const bool bIsVTSupported =
		InTextureType == EFaceTextureType::Basecolor ||
		InTextureType == EFaceTextureType::Normal ||
		InTextureType == EFaceTextureType::Cavity;

	const bool bShouldEnableVTStreaming = bIsVTSupported && bInMakeVirtualIfSupported;

	const FString VTSuffix = bShouldEnableVTStreaming ? TEXT("_VT") : TEXT("");

	// Create a sensible unique name for the texture to allow easy identification when debugging
	const FString TextureName = StaticEnum<EFaceTextureType>()->GetAuthoredNameStringByValue((int64)InTextureType);
	const FString CandidateName = FString::Format(TEXT("T_Face_{0}{1}"), { TextureName, VTSuffix });
	const FName AssetName = MakeUniqueObjectName(GetTransientPackage(), UTexture2D::StaticClass(), FName{ CandidateName }, EUniqueObjectNameOptions::GloballyUnique);
	...
	if (Texture)
	{
		...
		// Enable virtual texture streaming for texture if needed
		Texture->VirtualTextureStreaming = bShouldEnableVTStreaming;

		// no need to UpdateResource
		Texture->PostEditChange();

	
		// to wait on the texture being done use Texture->BlockOnAnyAsyncBuild() at the consumer site
	}
