...
bool CheckNumVertices(TSharedPtr<FMetaHumanCharacterIdentity::FState> InState, int32 InExpectedNumVertices)
{
	...
	{
		UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of vertices in FState::Evaluate. Expected %d, got %d.", InExpectedNumVertices, Vertices.Num());
		bResult = false;
	}
...
bool CheckValues(const TArray<FVector3f> & InNewPositions, const TArray<FVector3f>& InOldPositions, const FString & InLabel, float InMaxTolerance = 0.000001f, float InMedianTolerance = 0.000001f)
{
	...
	{
		UE_LOGF(LogMetaHumanCoreTechLibTest, Display, "Unexpected number of %ls. Expected %d, got %d.", *InLabel, InOldPositions.Num(), InNewPositions.Num());
		bResult = false;
	}
	else
	{
		...
		{
			UE_LOGF(LogMetaHumanCoreTechLibTest, Display, "Max difference of %f between point in %ls exceeds tolerance of %f for vertex %d", MaxDiff, *InLabel, InMaxTolerance, MaxDiffInd);
			bResult = false;
		}
		...
		{
			UE_LOGF(LogMetaHumanCoreTechLibTest, Display, "Median difference of %f between point in %ls exceeds tolerance of %f", MedianDiff, *InLabel, InMedianTolerance);
			bResult = false;
		}
	}
...
TArray<FVector3f> GetHeadMeshVertices(TSharedPtr<IDNAReader> InDNAReader)
{

#if WITH_EDITORONLY_DATA // can only use InDNAAsset->GetGeometryReader() in editor
	TArray<FVector3f> HeadMeshVertices;
	TSharedPtr<IDNAReader> GeometryReader = InDNAReader;
...
#else
	UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "MHC API only works with EditorOnly Data ");
	{
		return {};
	}
#endif
...
bool FTestMetaHumanCharacterIdentityTest::RunTest(const FString& InParameters)
{
	...
	if (InParameters == "MetaHumanCharacterIdentity_MH") // potentially extend this to other supported variants of MH going forward
	{
		...
		if (!ExampleSkelMesh)
		{
			UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to load SkeletalMesh asset from path: %ls", *ExampleSkelMeshPath);
			bResult = false;
		}
		else
		{
			// extract the DNA
			if (TSharedPtr<IDNAReader> DNAReader = USkelMeshDNAUtils::GetDNAReader(ExampleSkelMesh))
			{
				...
				if (!PluginDir.IsEmpty())
				{
					...
					TArray<FVector3f> OrigHeadMeshVertices = GetHeadMeshVertices(DNAReader);

					bool bInitializedAPI = MetaHumanCharacterIdentity.Init(MHCDataPath, MHCBodyDataPath, DNAReader->Unwrap(), HeadOrientation);
					...
					if (bInitializedAPI)
					{
						...
						{
							UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Incorrect number of presets. Found %d, expected %d", PresetNames.Num(), ExpectedNumberOfPresets);
							bResult = false;
						}
						...
						if (!State)
						{
							UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to create FMetaHumaneCharacterIdentity state");
							bResult = false;
						}
						else
						{
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of vertices in FState::Evaluate. Expected %d, got %d.", ExpectedNumVertices, OrigVertices.Num());
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of gizmo positions in FState::EvaluateGizmos. Expected %d, got %d.", ExpectedNumGizmos, OrigGizmoPositions.Num());
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of landmarks in FState::EvaluateLandmarks. Expected %d, got %d.", ExpectedNumLandmarks, OrigLandmarks.Num());
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of landmarks in FState::EvaluateLandmarks. Expected %d, got %d.", ExpectedNumLandmarks + 2, NewLandmarks.Num());
								bResult = false;
							}
							...
							UDNA* FaceDNAAsset = FMetaHumanCharacterSkelMeshUtils::GetArchetypeDNAAsset(EMetaHumanImportDNAType::Face, GetTransientPackage());
							check(FaceDNAAsset);
							TSharedRef<IDNAReader> FaceDNAReader = FaceDNAAsset->GetDNAReader().ToSharedRef();
							...
							if (bDeserialized)
							{
								...
								if (!bCompare)
								{
									UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Deserialized face state vertices does not match original face state");
								}
								...
								if (!bCompare)
								{
									UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Deserialized face state vertex normals does not match original face state");
								}
							}
							else
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to deserialize the face state");
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected GizmoScale does not match actual GizmoScale");
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected FaceScale does not match actual FaceScale");
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to get GlobalScale which matches expected GlobalScale");
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected a non-empty ModelIdentifier");
								bResult = false;
							}
							...
							if (VertexID == -1)
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected a value for selected face vertex not of -1");
								bResult = false;
							}
							...
							if (VertexID != -1)
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected a value for selected face vertex of -1");
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Number of HF varients is incorrect");
								bResult = false;
							}
							State->SetHighFrequencyVariant(4);
							if (State->GetHighFrequencyVariant() != 4)
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "High Frequency variant is incorrect");
								bResult = false;
							}
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of eyelash variants");
								bResult = false;
							}
							...
							// simple test of GetFaceModelCoefficients
							TArray<float> Coefficients;
							State->GetFaceModelCoefficients(Coefficients);
							const int32 ExpectedNumCoefficients = 1397;
							if (Coefficients.Num() != ExpectedNumCoefficients)
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of Coefficients");
								bResult = false;
							}
							...
							// smoke test of StateToDna
							TSharedRef<IDNAReader> FaceDNAReader3 = State->StateToDna(FaceDNAReader->Unwrap());

							// smoke test CopyBodyJointsToFace
							UDNA* BodyDNAAsset = FMetaHumanCharacterSkelMeshUtils::GetArchetypeDNAAsset(EMetaHumanImportDNAType::Body, GetTransientPackage());
							check(BodyDNAAsset);
							TSharedRef<IDNAReader> BodyDNAReader = BodyDNAAsset->GetDNAReader().ToSharedRef();
							...
							{
								UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Get/SetSettings gives unexpected result");
								bResult = false;
							}
							...
							for (int32 Index = 0; Index < RequiredFiles.Num(); ++Index)
							{
								...
								{
									UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "In WriteDebugAutoriggingData function, expected file %ls does not exist", *FullPath);
									bResult = false;
								}
							}
					...
					{
						UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to initialize MetaHumanCharacterIdentity");
						bResult = false;
					}
				}
				else
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to find plugin directory");
					bResult = false;
				}
			}
			else
			{
				UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to extract UDNAAsset data from SkeletalMesh asset from path: %ls", *ExampleSkelMeshPath);
				bResult = false;
			}
		}
	}
	...
	{
		UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected test: %ls", *InParameters);
		bResult = false;
	}
...
bool FTestMetaHumanCharacterBodyIdentityTest::RunTest(const FString& InParameters)
{
	...
	if (InParameters == "MetaHumanCharacterBodyIdentity_MH") // potentially extend this to other supported variants of MH going forward
	{
		...
		UDNA* FaceDNAAsset = FMetaHumanCharacterSkelMeshUtils::GetArchetypeDNAAsset(EMetaHumanImportDNAType::Face, GetTransientPackage());
		if (!PluginDir.IsEmpty() && FaceDNAAsset)
		{
			FMetaHumanCharacterBodyIdentity MetaHumanCharacterBodyIdentity;
			FString BodyPCAModelPath = PluginDir / TEXT("Content/Body/IdentityTemplate");
			FString LegacyBodiesPath = PluginDir / TEXT("Content/Optional/Body/FixedCompatibility");
			FString FaceMHCDataPath = PluginDir / TEXT("Content/Face/IdentityTemplate/");;

			TSharedPtr<FMetaHumanCharacterIdentity> MetaHumanCharacterIdentity = MakeShared<FMetaHumanCharacterIdentity>();
			bool bInitializedFaceAPI = MetaHumanCharacterIdentity->Init(FaceMHCDataPath, BodyPCAModelPath, FaceDNAAsset->GetDNAReader()->Unwrap(), EMetaHumanCharacterOrientation::Y_UP);

			bool bInitializedAPI = MetaHumanCharacterBodyIdentity.Init(BodyPCAModelPath, LegacyBodiesPath, MetaHumanCharacterIdentity);

			if (bInitializedFaceAPI && bInitializedAPI)
			{ 
				...
				if (Gizmos.Num() != 27)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of gizmos");
					bResult = false;
			 	}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of physics volumes for thigh_l");
					bResult = false;
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of physics volumes for spine_04");
					bResult = false;
				}

				// test getting and setting constraints
				const int32 ExpectedNumConstraints = 30;
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of body constraints");
					bResult = false;
				}
				...
				for (int32 Constraint = 0; Constraint < Constraints.Num(); ++Constraint)
				{
					...
					{
						UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expect all constraints to be inactive initially");
						bResult = false;
					}
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of vertices or vertex normals from evaluated state");
					bResult = false;
				}
				...
				if (bVerticesSame)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected vertices to be different and they are the same");
					bResult = false;
				}				
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected vertex normals to be different and they are the same");
					bResult = false;
				}
				...
				if (!bVerticesSame)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected vertices to be the same and they are different");
					bResult = false;
				}
				...
				if (!bVertexNormalsSame)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected vertex normals to be the same and they are different");
					bResult = false;
				}
				...
				if (FMath::Abs(TargetMeasurement - ActualMeasurement) > 0.5f) // seem to have to use quite a large tolerance here
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected calculated measurement %f to be similar to target measurement %f and it is not", ActualMeasurement, TargetMeasurement);
					bResult = false;
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected measurement contour vertices to contain values and it does not");
					bResult = false;
				}
				...
				if (!bArchived)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to archive state");
					bResult = false;
				}
				...
				if (!bRestored)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to restore state");
					bResult = false;
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected global delta scale different from actual global delta scale");
					bResult = false;
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of vertices per LOD result");
					bResult = false;
				}

				const TArray<int32> TriangleIndices = State->GetTrianglesIndices();
				if (TriangleIndices.Num() != 326460)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of triangle indices");
					bResult = false;
				}
				...
				const FVector3f ExpectedVertex = { -20.9420f, 13.3903f, 1.4788f };
				if (FMath::Abs((Vertex - ExpectedVertex).Length()) > 0.001f)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "GetVertex function returns unexpected value");
					bResult = false;
				}
				...
				if (NumJoints != 342)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "GetNumberOfJoints function returns unexpected value");
					bResult = false;
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "CopyBindPose returned array of unexpected size");
					bResult = false;
				}
				
				// copy the component pose
				const TArray<FMatrix44f> ComponentPose = State->CopyComponentPose();
				if (ComponentPose.Num() != NumJoints)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "CopyComponentPose returned array of unexpected size");
					bResult = false;
				}
				
				// test SetEvaluatePose
				State->SetEvaluatePose(true);
				State->SetEvaluatePose(false);
				...
				// test GetNeutralJointTransforms
				TArray<FVector3f> JointTranslations;
				TArray<FRotator3f> JointRotations;
				State->GetNeutralJointTransforms( BindPose, JointTranslations, JointRotations);
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Unexpected number of vertex influence weights");
					bResult = false;
				}
				...
				for (int32 Index = 0; Index < VertexInfluenceWeights.Num(); ++Index)
				{
					for (int32 Weight = 0; Weight < VertexInfluenceWeights[Index].Value.Num(); ++Weight)
					{
						...
						{
							UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "VertexInfluenceWeight is out of range");
							bResult = false;
						}
					}
				}
				...
				bVertexNormalsSame = CheckValues(NewStateVerticesAndNormals.VertexNormals, State2VerticesAndNormals.VertexNormals, FString(TEXT("body state vertex normals")), 0.002f, 0.001f);
				if (!bVertexNormalsSame)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected vertex normals to be the same and they are different");
					bResult = false;
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expecting BlendableBody type");
					bResult = false;
				}
				...
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expecting f_tal_nrw type");
					bResult = false;
				}
				...
				if (BodyDNAAsset && FaceDNAAsset)
				{
					...
					const int32 ExpectedNumMeasurements = 27;

					if (Measurements.Num() != ExpectedNumMeasurements)
					{
						UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Number of measurements does not match expected number");
						bResult = false;
					}
				...
				if (BodyDNAAsset)
				{
					...
					{
						const int32 NumVertices = BodyDNAReader->GetVertexPositionCount(MeshId);					
						Vertices.SetNumUninitialized(NumVertices);
						for (int32 i = 0; i < NumVertices; ++i)
						{
							Vertices[i] = FVector3f(BodyDNAReader->GetVertexPosition(MeshId, i));
						}
					}
					
					TArray<int32> VertexIndices;
					{
						const int32 NumFaces = BodyDNAReader->GetFaceCount(MeshId); 					
						for (int32 FaceIndex = 0; FaceIndex < NumFaces; ++FaceIndex)
						{
							TArrayView<const uint32> LayoutIndices = BodyDNAReader->GetFaceVertexLayoutIndices(MeshId, FaceIndex);
							for (const uint32 LayoutIndex : LayoutIndices)
							{
								int32 PositionIndex = BodyDNAReader->GetVertexLayout(MeshId, LayoutIndex).Position;
								VertexIndices.Add(PositionIndex);
							}
						}
					}
					
					FConformTargetParams TargetParams;
					TargetParams.ConformTargetMesh.BodyVertices = Vertices;
					TargetParams.ConformTargetMesh.BodyVertexIndices = VertexIndices;
					
					bool bIterationCalled = false;
					State->OnMeshConformIteration().BindLambda([&](const FMetaHumanRigEvaluatedState& InRigEvaluatedState, const TArray<FMatrix44f>& InBindPoseMatrices, int32 InIterationCount, ESolveStepType InSolveStepType)
					{
					...
					// Run conform
					if (!State->ConformTarget(TargetParams))
					{
						UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Conform target failed");
						bResult = false;
					}
					
					// Check iteration callback
					if (!bIterationCalled)
					{
						UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Conform iteration callback failed");
						bResult = false;
					}
					
					FRefinementTargetParams Params;
					Params.ConformTargetMesh.BodyVertices = Vertices;
					Params.ConformTargetMesh.BodyVertexIndices = VertexIndices;
					if (!State->RefineVerticesToTarget(Params))
					{
						UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Refine vertices failed");
						bResult = false;
					}
				}

				// SelectVertex, checking a case where we expect an intersection and one where we don't
				const FVector3f Origin = { 0, 0, 0 };
				FVector3f Direction = { 0, 0, 1 };
				FVector3f BodyVertex;
				FVector3f Normal;

				int32 VertexID = State->SelectVertex(Origin, Direction, BodyVertex, Normal);
				if (VertexID == -1)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected a value for selected body vertex not of -1");
					bResult = false;
				}

				Direction = { 0, 0, -1 };
				VertexID = State->SelectVertex(Origin, Direction, BodyVertex, Normal);
				if (VertexID != -1)
				{
					UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Expected a value for selected body vertex of -1");
					bResult = false;
				}
			}
			else
			{
				UE_LOGF(LogMetaHumanCoreTechLibTest, Error, "Failed to initialize MetaHumanCharacterBodyIdentity");
				bResult = false;
			}
		}
