...
public:
	...
	/** Get the Character which we are editing */
	TObjectPtr<UMetaHumanCharacter> GetCharacter() const;
...
private:
	...
	/** Calls FMetaHumanCharacterEditorModeToolkit::UpdateWarningText to update warning text dependent on rigging state and if downloading textures*/
	void UpdateWarningText() const;

	/** Context object registered in the tools context store so the mesh import tool can cache and re-use built dynamic meshes across restarts. */
	UPROPERTY()
	TObjectPtr<UMetaHumanCharacterEditorMeshImportContextObject> MeshImportContextObject;
};