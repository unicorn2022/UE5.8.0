...
AMetaHumanCharacterEditorActor::AMetaHumanCharacterEditorActor()
{
	...
	// Add the IK retargeter component
...
void AMetaHumanCharacterEditorActor::ReinitAnimation()
{
	...
	if (!IKRetargeter)
	{
		IKRetargeter = LoadObject<UIKRetargeter>(nullptr, TEXT("/" UE_PLUGIN_NAME "/Animation/Retargeting/RTG_MH_IKRig"));
	}
...
void AMetaHumanCharacterEditorActor::SetShowNormalsOnFace(const bool InShowNormals)
{
	...
	FaceComponent->MarkRenderStateDirty();
	FlushPendingRenderStateUpdates();
}
...
void AMetaHumanCharacterEditorActor::FlushPendingRenderStateUpdates() const
{
