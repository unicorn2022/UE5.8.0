...
namespace UE::MetaHuman
{
	...
	static FVector3f GetOrientatedPositionOrNormal(const FVector& InVector, EMetaHumanCharacterOrientation InCharacterOrientation)
	{
		FVector3f OutVector;
		if (InCharacterOrientation == EMetaHumanCharacterOrientation::Y_UP)
		{
			OutVector = FVector3f{ InVector };
		}
		else if (InCharacterOrientation == EMetaHumanCharacterOrientation::Z_UP)
		{
			OutVector[0] = InVector.X;
			OutVector[1] = -InVector.Z;
			OutVector[2] = InVector.Y;
		}
		else
		{
			check(false);
	...
	static void UpdateBaseMesh(USkeletalMesh* InSkelMesh, IDNAReader* InDNAReader, FDNAToSkelMeshMap* InDNAToSkelMeshMap, ELodUpdateOption InUpdateOption, EMetaHumanCharacterOrientation InCharacterOrientation)
	{
		...
		for (int32 LODIndex = LODStart; LODIndex < LODRangeSize; LODIndex++)
		{
			...
			for (FSkelMeshSection& Section : LODModel.Sections)
			{
				...
				// Build a mapping from DNA position index to DNA normal index for this mesh,
				// using the vertex layouts (each layout entry binds a position index to a normal index).
				TArray<int32> DNAPositionToNormalIndex;
				if (DNAMeshIndex >= 0)
				{
					const int32 NumPositions = static_cast<int32>(InDNAReader->GetVertexPositionCount(DNAMeshIndex));
					DNAPositionToNormalIndex.Init(INDEX_NONE, NumPositions);
					const int32 NumLayouts = static_cast<int32>(InDNAReader->GetVertexLayoutCount(DNAMeshIndex));
					for (int32 LayoutIndex = 0; LayoutIndex < NumLayouts; ++LayoutIndex)
					{
						const FVertexLayout Layout = InDNAReader->GetVertexLayout(DNAMeshIndex, LayoutIndex);
				...
				for (int32 VertexIndex = 0; VertexIndex < NumSoftVertices; VertexIndex++)
				{
					...
					if (DNAVertexIndex >= 0)
					{
						...
						Vertex.Position = GetOrientatedPositionOrNormal(Position, InCharacterOrientation);

						if (DNAPositionToNormalIndex.IsValidIndex(DNAVertexIndex))
						{
							const int32 NormalIndex = DNAPositionToNormalIndex[DNAVertexIndex];
							if (NormalIndex != INDEX_NONE)
							{
								const FVector Normal = InDNAReader->GetVertexNormal(DNAMeshIndex, NormalIndex);
								const FVector3f NormalUE = GetOrientatedPositionOrNormal(Normal, InCharacterOrientation);
								Vertex.TangentZ = FVector4f(NormalUE.GetSafeNormal(), /*Handedness*/-1.0f);
							}
						}
						...
						for (int32 OverlappingIndex = 0; OverlappingIndex < OverlappingCount; ++OverlappingIndex)
						{
							...
							OverlappingVertex.Position = Vertex.Position;
							OverlappingVertex.TangentZ = Vertex.TangentZ;
						}
					}
					VertexBufferIndex++;
				}
		...
		for (const TPair<FVector2f, TArray<int32>>& DuplicateUVGroup : DuplicateUVIndices)
		{
			...
			for (int32 UVIndex : DuplicateUVGroup.Value)
			{
				...
				for (const FEdgeID EdgeID : InTemplateMeshDescription.GetVertexConnectedEdgeIDs(VertexID))
				{
					...
					if (const TArray<int32>* NeighbourUVIndicesPtr = VertexIdToUVIndex.Find(NeighbourVertexID))
					{
						for (int32 NeighbourUVIndex : *NeighbourUVIndicesPtr)
						{
		...
		TArray<FVector3f>& OutVertices)
	{
		...
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "DNA archetype does not contain mesh for index %d", InDNAMeshIndex);
			return false;
		}
		...
		if (NumVertices == 0)
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "DNA archetype contains no vertices for mesh index %d", InDNAMeshIndex);
			return false;
		}
		...
		if (NumUVs == 0)
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "DNA archetype contains no UVs for mesh index %d", InDNAMeshIndex);
			return false;
		}
		...
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "DNA archetype has invalid vertex layout for mesh index %d. Number of vertex positions does not match number referenced by vertex layout.", InDNAMeshIndex);
			return false;
		}
		...
		if (NumSetUVs != NumUVs)
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "DNA archetype has invalid vertex layout for mesh index %d. Number of texture coordinates does not match number referenced by vertex layout.", InDNAMeshIndex);
			return false;
		}
		...
		for (uint32 DNAPositionIndex = 0; DNAPositionIndex < NumVertices; DNAPositionIndex++)
		{
			...
			{
				UE_LOGF(LogMetaHumanCharacterEditor, Warning, "UV mismatch. Closest vertex to DNA Position Index %d is above UV distance threshold: %f", DNAPositionIndex, Dist);
				OutVertices.Empty();
				return false;
			}
...
void FMetaHumanCharacterSkelMeshUtils::UpdateSkelMeshFromDNA(TSharedRef<IDNAReader> InDNAReader, EUpdateFlags InUpdateFlags, TSharedRef<FDNAToSkelMeshMap>& InOutDNAToSkelMeshMap, EMetaHumanCharacterOrientation InCharacterOrientation, TNotNull<USkeletalMesh*> OutSkeletalMesh)
{
	...
	if (EnumHasAnyFlags(InUpdateFlags, EUpdateFlags::DNA))
	{
		// Set the Reader of DNA in skeletal mesh AssetUserData
		USkelMeshDNAUtils::SetDNAReader(OutSkeletalMesh, InDNAReader, EDNACopyPolicy::Alias, ERigLogicInitPolicy::Defer);
		UDNA* DNAAsset = USkelMeshDNAUtils::GetMeshDNAAsset(OutSkeletalMesh);
		if (DNAAsset)
		{
		...
		{
			UDNAAsset* LegacyDNAAsset = USkelMeshDNAUtils::GetMeshDNA(OutSkeletalMesh);
			if (LegacyDNAAsset)
			{
...
void FMetaHumanCharacterSkelMeshUtils::UpdateMeshDescriptionFromLODModelVerticesNormalsAndTangents(TNotNull<USkeletalMesh*> InSkeletalMesh)
{
	...
	ParallelFor(InSkeletalMesh->GetImportedModel()->LODModels.Num(), [&](int32 LODIndex)
	{
		...
		if (!MeshDescription)
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Warning, "No mesh description for LOD %d", LODIndex);
			return;
		}
		...
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Warning, "Mesh Description does not match Skeletal Mesh model for LOD %d", LODIndex);
			return;
		}
	...
	for (int32 LODIndex = 0; LODIndex < InSkeletalMesh->GetImportedModel()->LODModels.Num(); ++LODIndex)
	{
		...
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Warning, "Full mesh description update for lod %d", LODIndex);
			const FSkeletalMeshLODModel& LODModel = InSkeletalMesh->GetImportedModel()->LODModels[LODIndex];
			FMeshDescription MeshDescription;
			LODModel.GetMeshDescription(InSkeletalMesh, LODIndex, MeshDescription);
			InSkeletalMesh->CreateMeshDescription(LODIndex, MoveTemp(MeshDescription));
...
bool FMetaHumanCharacterSkelMeshUtils::CompareDnaToStateVerticesAndNormals(
	...
	TConstArrayView<int32> InMeshIndicesToCheck /*= {}*/)
{
	const int32 MeshCount = InDNAReader->GetMeshCount();
	for (int32 MeshIndex = 0; MeshIndex < MeshCount; MeshIndex++)
	{
		// Skip meshes not in the input list (when one was provided).
		if (!InMeshIndicesToCheck.IsEmpty() && !InMeshIndicesToCheck.Contains(MeshIndex))
		{
			continue;
		}
		...
		for (int32 DNAVertexIndex = 0; DNAVertexIndex < VertexCount; DNAVertexIndex++)
		{
			...
			if (!bPositionsEqual)
			{
				UE_LOGF(LogMetaHumanCharacterEditor, Warning, "Vertex position mismatch at mesh %d (%ls) and index %d, DNA: %f,%f,%f, State: %f,%f,%f",MeshIndex, *InDNAReader->GetMeshName(MeshIndex), DNAVertexIndex,
			...
			if (!bNormalsEqual)
			{
				UE_LOGF(LogMetaHumanCharacterEditor, Warning, "Vertex normal mismatch at mesh %d (%ls) and index %d, DNA: %f,%f,%f, State: %f,%f,%f", MeshIndex, *InDNAReader->GetMeshName(MeshIndex), DNAVertexIndex,
...
bool FMetaHumanCharacterSkelMeshUtils::CompareDnaToStateVertices(
	TSharedPtr<const IDNAReader> InDNAReader, 
	const TArray<FVector3f>& InStateVertices, 
	TSharedPtr<const FMetaHumanCharacterIdentity::FState> InState, 
	float Tolerance /*= UE_KINDA_SMALL_NUMBER*/,
...
bool FMetaHumanCharacterSkelMeshUtils::CheckDNACompatibility(IDNAReader* InDnaReaderA, IDNAReader* InDnaReaderB)
{
	...
	// Joints
	{
		...
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Warning, "Joint count mismatch: %d vs %d", JointCountA, JointCountB);
			return false;
		}

		bool bJointsOk = true;
		TStringBuilder<0> ResultMsg;
		...
		if (!bJointsOk)
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Warning, "%ls", *ResultMsg);
			return false;
		}
	}
	...
	// Meshes
	{
		...
		for (uint16 MeshIndex = 0; MeshIndex < MeshCount; MeshIndex++)
		{
			if (MeshIndex < MeshCountA && MeshIndex < MeshCountB)
			{
				...
				{
					ResultMsg.Appendf(TEXT("Vertex count mismatch on mesh '%s' (mesh index: %u): %u vs %u\n"), *InDnaReaderA->GetMeshName(MeshIndex), MeshIndex, VertexCountA, VertexCountB);
					bMeshesOk = false;
				}
			}
			else
...
TArray<FTransform> FMetaHumanCharacterSkelMeshUtils::GetUECoordinateComponentTransforms(USkeletalMesh* InSkelMesh, 
	   const TSharedPtr<IDNAReader>& InArchetypeDNAReader, 
...
{
	if (InJointPositions.Num() != InJointRotations.Num() || 
		InJointPositions.Num() != InArchetypeDNAReader->GetJointCount())
	{
		return {};
	}
	
	TArray<FTransform> ComponentTransforms;
	FAnimationRuntime::FillUpComponentSpaceTransforms(InSkelMesh->GetRefSkeleton(), InSkelMesh->GetRefSkeleton().GetRawRefBonePose(), ComponentTransforms);
	
	const TArray<FMeshBoneInfo>& RawBoneInfo = InSkelMesh->GetRefSkeleton().GetRawRefBoneInfo();
	for (uint16 JointIndex = 0; JointIndex < InJointPositions.Num(); JointIndex++)
	{
		int32 BoneIndex = InDNAToSkelMeshMap->GetUEBoneIndex(JointIndex);
		if (BoneIndex < 0 || BoneIndex >= ComponentTransforms.Num())
		{
			continue;
		}
		...
		// Convert to UE coordinates in component space here
		FVector Translation(InJointPositions[JointIndex].X, -InJointPositions[JointIndex].Y, InJointPositions[JointIndex].Z);
		FRotator Rotation(-InJointRotations[JointIndex].Yaw, -InJointRotations[JointIndex].Roll, InJointRotations[JointIndex].Pitch);
			
		if (InArchetypeDNAReader->GetJointParentIndex(JointIndex) == JointIndex) // This is the highest joint of the dna - not necessarily the UE root bone  
		{
			if (InCharacterOrientation == EMetaHumanCharacterOrientation::Y_UP)
			{
				FQuat YUpToZUpRotation = FQuat(FRotator(0, 0, 90));
...
{
	FInterchangeDnaModule::GetModule().CreateAndAttachDNAToSkeletalMesh(InSkelMesh, InDNAReader);
...
bool FMetaHumanCharacterSkelMeshUtils::GetStaticMeshVertices(const TNotNull<UStaticMesh*> InTemplateStaticMesh, int32 InLodIndex, TArray<FVector3f>& OutVertices)
{
	...
	if (!MeshDescription)
	{
		UE_LOGF(LogMetaHumanCharacterEditor, Error, "Template mesh does not contain mesh description for Lod %d", InLodIndex);
		return false;
	}
	...
	TArray<FVector3f>& OutVertices)
{
	...
	{
		UE_LOGF(LogMetaHumanCharacterEditor, Error, "Template mesh does not contain LOD %d", InLodIndex);
		return false;
	}
										   ...
										   const EMetaHumanImportDNAType InImportDNAType)
{
	...
	if (USkeletalMesh* SkelMeshAsset = DNAImportModule.ImportSync(InAssetName, InAssetPath, InDNAReader, Skeleton, DNAConfig))
	{
...
UDNA* FMetaHumanCharacterSkelMeshUtils::GetArchetypeDNAAsset(const EMetaHumanImportDNAType InImportDNAType, UObject* InOuter)
{
	const FString DNAPath = FMetaHumanCommonDataUtils::GetArchetypeDNAPath(InImportDNAType);
	return ReadDNAAssetFromFile(DNAPath, InOuter);
}
...
TSoftObjectPtr<UObject> FMetaHumanCharacterSkelMeshUtils::GetFaceArchetypeDefaultAnimatingRig(EMetaHumanCharacterTemplateType InTemplateType)
{
	TSoftObjectPtr<UObject> FaceRigAsset = FMetaHumanCommonDataUtils::GetDefaultControlRigFromRegistry(FMetaHumanCommonDataUtils::GetCharacterPluginFaceControlRigPath());
...
TSoftObjectPtr<UObject> FMetaHumanCharacterSkelMeshUtils::GetBodyArchetypeDefaultAnimatingRig(EMetaHumanCharacterTemplateType InTemplateType)
{
	const FString RigBlueprintObjectPath = TEXT("/" UE_PLUGIN_NAME "/Common/MetaHuman_ControlRig.MetaHuman_ControlRig");
	TSoftObjectPtr<UObject> BodyRigAsset = FMetaHumanCommonDataUtils::GetDefaultControlRigFromRegistry(FStringView(RigBlueprintObjectPath));
