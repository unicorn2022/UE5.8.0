...
protected:
	...
	/**
	 * Returns the MetaHuman Character being edited for the given scene state, or nullptr if the
	 * scene state has no component whose owner implements UMetaHumanCharacterEditorActorInterface.
	 */
	static class UMetaHumanCharacter* GetMetaHumanCharacter(const FToolBuilderState& InSceneState);

	/**
	 * Returns true if the character being edited is rigged or rig-pending. Returns false when
	 * there is no character in the scene state.
	 */
	static bool IsCharacterRigged(const FToolBuilderState& InSceneState);

	/**
	 * Returns true if the character being edited is currently requesting high-resolution source
	 * textures. Returns false when there is no character in the scene state.
	 */
	static bool IsCharacterRequestingHighResTextures(const FToolBuilderState& InSceneState);
};