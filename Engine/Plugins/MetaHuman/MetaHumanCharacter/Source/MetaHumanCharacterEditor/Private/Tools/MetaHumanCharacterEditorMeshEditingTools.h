...
public:
	...
	/** Scales the size of the on-screen blend handle in the viewport. Useful on high-DPI displays or when precision clicking is difficult. */
	...
	/** When enabled, any edits made to one side of the face are automatically mirrored to the other.. Disable to sculpt asymmetric features. */
...
UCLASS()
class UMetaHumanCharacterEditorHeadAndBodyParameterProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
...
public:
	...
	/**
	 * Delegate that executes on EPropertyChangeType::ValueSet property change event for face evaluation settings
	 * (FaceDelta, HeadScale). Mirrors the pattern from the former UMetaHumanCharacterEditorFaceEditingToolHeadParameterProperties.
	 */
	...
	void CopyTo(FMetaHumanCharacterFaceEvaluationSettings& OutFaceEvaluationSettings) const;
	...
	// ---- Head parameters (merged from UMetaHumanCharacterEditorFaceEditingToolHeadParameterProperties) ----
	...
	UPROPERTY(EditAnywhere, Category = "Head Parameters", meta = (UIMin = "0.0", UIMax = "1.0", ClampMin = "0.0", ClampMax = "1.0", DisplayName = "Global Delta"))
	float FaceDelta = 1.0f;

	/** Changes the global scale of the head relative to the body. */
...
public:
	UMetaHumanCharacterEditorHeadAndBodyParameterProperties* GetHeadAndBodyParameterProperties() const { return HeadAndBodyParameterProperties; }
...
protected:
	UPROPERTY()
	TObjectPtr<UMetaHumanCharacterEditorHeadAndBodyParameterProperties> HeadAndBodyParameterProperties;

	void UpdateHeadAndBodyParameterProperties(TWeakObjectPtr<UInteractiveToolManager> ToolManager, const FMetaHumanCharacterFaceEvaluationSettings& FaceEvaluationSettings);
};
...
public:
	...
	virtual void BlendToMetaHumanCharacterPresetHeadOnly(UMetaHumanCharacter* InCharacterPreset) PURE_VIRTUAL(UMetaHumanCharacterEditorMeshBlendTool::BlendToMetaHumanCharacterPresetHeadOnly);
	virtual void BlendToMetaHumanCharacterPresetBodyOnly(UMetaHumanCharacter* InCharacterPreset) PURE_VIRTUAL(UMetaHumanCharacterEditorMeshBlendTool::BlendToMetaHumanCharacterPresetBodyOnly);
