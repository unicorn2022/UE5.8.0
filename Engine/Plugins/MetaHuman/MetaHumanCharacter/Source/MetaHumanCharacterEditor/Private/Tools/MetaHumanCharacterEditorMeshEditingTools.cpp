...
UStaticMeshComponent* UMetaHumanCharacterEditorMeshEditingTool::CreateManipulator(const FVector3f& InPosition)
{
	...
	ManipulatorComponent->SetVisibility(!InPosition.IsZero());
...
void UMetaHumanCharacterEditorMeshEditingTool::UpdateManipulatorPositions(const TArray<FVector3f>& InPositions)
{
	...
	for (int32 ManipulatorIndex = 0; ManipulatorIndex < InPositions.Num(); ++ManipulatorIndex)
	{
		...
		// Pending a correction of the landmark dataset, hide manipulators that evaluate to world origin.
		ManipulatorComponents[ManipulatorIndex]->SetVisibility(!InPositions[ManipulatorIndex].IsZero());
	}
}
...
void UMetaHumanCharacterEditorMeshEditingTool::OnEndDrag(const FRay& InRay)
{
	...
	{
		// Pending a correction of the landmark dataset, hide manipulators that evaluate to world origin.
		ManipulatorComponents[ManipulatorIndex]->SetVisibility(!ManipulatorComponents[ManipulatorIndex]->GetComponentLocation().IsZero());
	}
...
// -----------------------------------------------------
// HeadAndBodyParameterProperties implementation -------
// -----------------------------------------------------

void UMetaHumanCharacterEditorHeadAndBodyParameterProperties::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UMetaHumanCharacterEditorHeadAndBodyParameterProperties::CopyFrom(const FMetaHumanCharacterFaceEvaluationSettings& InFaceEvaluationSettings)
{
	FaceDelta = InFaceEvaluationSettings.GlobalDelta;
	HeadScale = InFaceEvaluationSettings.HeadScale;
}

void UMetaHumanCharacterEditorHeadAndBodyParameterProperties::CopyTo(FMetaHumanCharacterFaceEvaluationSettings& OutFaceEvaluationSettings) const
{
	OutFaceEvaluationSettings.GlobalDelta = FaceDelta;
	OutFaceEvaluationSettings.HeadScale = HeadScale;
}

void UMetaHumanCharacterEditorHeadAndBodyParameterProperties::OnPostEditChangeProperty(const FPropertyChangedEvent& PropertyChangedEvent)
{
	if (PreviousBodyState.IsValid())
	{
		...
		if (PreviousBodyState->GetGlobalDeltaScale() != BodyDelta)
		{
			Subsystem->SetBodyGlobalDeltaScale(MetaHumanCharacter, BodyDelta);
			...
			if ((PropertyChangedEvent.ChangeType & (EPropertyChangeType::ValueSet | EPropertyChangeType::ResetToDefault)) != 0u && ((PropertyChangedEvent.ChangeType & EPropertyChangeType::Interactive) == 0u))
			{
				...
				const FText CommandChangeDescription = LOCTEXT("BodyGlobalDeltaCommandChange", "Change Body Delta");
...
void UMetaHumanCharacterEditorHeadAndBodyParameterProperties::OnBodyStateChanged()
{
	UMetaHumanCharacterEditorSubsystem* Subsystem = UMetaHumanCharacterEditorSubsystem::Get();
	USingleSelectionTool* OwnerTool = GetTypedOuter<USingleSelectionTool>();
	UMetaHumanCharacter* MetaHumanCharacter = UE::ToolTarget::GetTargetMetaHumanCharacter(OwnerTool->GetTarget());
	BodyDelta = Subsystem->GetBodyGlobalDeltaScale(MetaHumanCharacter);
	CopyFrom(MetaHumanCharacter->FaceEvaluationSettings);
}

void UMetaHumanCharacterEditorHeadAndBodyParameterProperties::ResetBody()
{
...
void UMetaHumanCharacterEditorFaceTool::Setup()
{
	...
	HeadAndBodyParameterProperties = NewObject<UMetaHumanCharacterEditorHeadAndBodyParameterProperties>(this);
	AddToolPropertySource(HeadAndBodyParameterProperties);
	HeadAndBodyParameterProperties->RestoreProperties(this, GetCommandChangeDescription().ToString());

	UMetaHumanCharacter* Character = UE::ToolTarget::GetTargetMetaHumanCharacter(Target);
	check(Character);
	HeadAndBodyParameterProperties->CopyFrom(Character->FaceEvaluationSettings);
	HeadAndBodyParameterProperties->PreviousFaceEvaluationSettings = Character->FaceEvaluationSettings;

	// Bind to the ValueSet event to fill in the undo stack for face evaluation settings
	GetHeadAndBodyParameterProperties()->OnPropertyValueSetDelegate.BindWeakLambda(this, [this](const FPropertyChangedEvent& PropertyChangedEvent)
		{
			if (UMetaHumanCharacter* Character = UE::ToolTarget::GetTargetMetaHumanCharacter(Target))
			{
				...
				GetHeadAndBodyParameterProperties()->CopyTo(NewFaceEvaluationSettings);
				...
				if ((PropertyChangedEvent.ChangeType & (EPropertyChangeType::ValueSet | EPropertyChangeType::ResetToDefault)) != 0u && ((PropertyChangedEvent.ChangeType & EPropertyChangeType::Interactive) == 0u))
				{
					...
					OnSettingsUpdateDelegate.BindWeakLambda(this, [this](TWeakObjectPtr<UInteractiveToolManager> ToolManager, const FMetaHumanCharacterFaceEvaluationSettings& FaceEvaluationSettings)
						{
							UpdateHeadAndBodyParameterProperties(ToolManager, FaceEvaluationSettings);
							UpdateManipulatorPositions();
						});

					TUniquePtr<FMetaHumanCharacterEditorFaceEvaluationSettingsCommandChange> CommandChange = MakeUnique<FMetaHumanCharacterEditorFaceEvaluationSettingsCommandChange>(Character, GetHeadAndBodyParameterProperties()->PreviousFaceEvaluationSettings, OnSettingsUpdateDelegate, GetToolManager());
					...
					GetHeadAndBodyParameterProperties()->PreviousFaceEvaluationSettings = NewFaceEvaluationSettings;
				}
				else
				{
...
void UMetaHumanCharacterEditorFaceTool::Shutdown(EToolShutdownType InShutdownType)
{
	...
	HeadAndBodyParameterProperties->SaveProperties(this, GetCommandChangeDescription().ToString());
}

void UMetaHumanCharacterEditorFaceTool::UpdateHeadAndBodyParameterProperties(TWeakObjectPtr<UInteractiveToolManager> ToolManager, const FMetaHumanCharacterFaceEvaluationSettings& FaceEvaluationSettings)
{
	if (ToolManager.IsValid())
	{
		...
		{
			UMetaHumanCharacterEditorHeadAndBodyParameterProperties* Params = nullptr;
			if (FaceTool->GetToolProperties().FindItemByClass<UMetaHumanCharacterEditorHeadAndBodyParameterProperties>(&Params))
			{
				Params->CopyFrom(FaceEvaluationSettings);
				Params->SilentUpdateWatched();

				// Restore the PreviousFaceEvaluationSettings so that new commands are created
				// with the correct previous settings
				Params->PreviousFaceEvaluationSettings = FaceEvaluationSettings;
			}
		}
	}
}
...
void UMetaHumanCharacterEditorMeshBlendTool::OnBeginDrag(const FRay& InRay)
{
	...
	// Derive the item placement radius from the actual circle mesh world scale so items always
	// sit on the visual ring edge. SM_BlendTool_AncestryCircle has a natural radius of 150 units,
	// which at the base scale (GizmoScale * 10) produces the calibrated 3.f world-unit radius.
	constexpr float CircleMeshNaturalRadius = 150.f;
	const float ItemPlacementRadius = CircleMeshNaturalRadius * GizmoScale * MeshEditingToolProperties->Size * 10.f;
	...
	for(uint16 i = 0 ; i < NumOfPresets ; i++)
	{
		...
		FVector OffsetForMesh = (CameraRight * FMath::Cos(AngleRad) + CameraUp * FMath::Sin(AngleRad)) * ItemPlacementRadius;
...
void UMetaHumanCharacterEditorMeshBlendTool::OnTick(float InDeltaTime)
{
	...
	if (!PendingMoveDelta.IsZero() && SelectedManipulator != INDEX_NONE)
	{
		...
		// Clamp if needed
		const float ClampRadius = GetAncestryCircleRadius() * MeshEditingToolProperties->Size;
		if(LocalOffset2D.Size() > ClampRadius)
		{
			LocalOffset2D = LocalOffset2D.GetSafeNormal() * ClampRadius;
		}
