...
bool UMetaHumanCharacterEditorHeadModelToolBuilder::CanBuildTool(const FToolBuilderState& InSceneState) const
{
	if (!Super::CanBuildTool(InSceneState))
	{
		return false;
	}

	// The Materials sub-tool only edits head materials, which is safe to do on a rigged character.
	// All other sub-tools require the character to not be rigged.
	if (ToolType == EMetaHumanCharacterHeadModelTool::Materials)
	{
		return true;
	}

	return !IsCharacterRigged(InSceneState);
}
...
void UMetaHumanCharacterEditorHeadModelTool::ProcessPending()
{
	...
	if (UMetaHumanCharacter* Character = UE::ToolTarget::GetTargetMetaHumanCharacter(Target))
	{
		if (bTeethVariantWasCommitted && TeethProperties->Teeth != PreviousHeadModelSettings.Teeth)
		{
			...
			// When applying the first prop change after opening teeth tool, prev show teeth would be false and new show teeth would be true.
			// To avoid show teeth becoming part of transaction & ending up closing mouth on undo, set prev show teeth to new show teeth.
			// New show teeth will always be true for teeth tool.
			PreviousHeadModelSettings.Teeth.EnableShowTeethExpression = NewHeadModelSettings.Teeth.EnableShowTeethExpression;
			TUniquePtr<FMetaHumanCharacterEditorHeadModelToolCommandChange> CommandChange =
				MakeUnique<FMetaHumanCharacterEditorHeadModelToolCommandChange>(PreviousHeadModelSettings, NewHeadModelSettings, GetToolManager());
			GetToolManager()->GetContextTransactionsAPI()->AppendChange(Character, MoveTemp(CommandChange), LOCTEXT("HeadModelToolCommandChange", "Edit Head Model"));
			PreviousHeadModelSettings = NewHeadModelSettings;
