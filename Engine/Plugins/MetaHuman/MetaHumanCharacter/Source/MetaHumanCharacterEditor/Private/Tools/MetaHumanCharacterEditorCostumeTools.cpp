...
void UMetaHumanCharacterEditorCostumeTool::UpdateCostumeItems()
{
	...
	TNotNull<const UMetaHumanInstance*> MetaHumanInstance = PropertyObject->Collection->GetDefaultInstance();
	...
	for (const FMetaHumanPipelineSlotSelectionData& SlotSelection : SlotSelections)
	{
		...
		if (NewItem)
		{
			const FMetaHumanPaletteItemPath ItemPath = SlotSelection.Selection.GetSelectedItemPath();

			NewItem->SlotName = SlotSelection.Selection.SlotName;
			NewItem->ItemPath = ItemPath;
			NewItem->WardrobeItem = Item.WardrobeItem;
			NewItem->InstanceParameters = MetaHumanInstance->GetCurrentInstanceParametersForItem(ItemPath);

			TMap<FMetaHumanPaletteItemPath, FInstancedPropertyBag> AssemblyParams = MetaHumanInstance->GetAssemblyParameters();
			if (const FInstancedPropertyBag* AssemblyParameters = AssemblyParams.Find(ItemPath))
			{
				NewItem->DefaultValues = *AssemblyParameters;
			}
