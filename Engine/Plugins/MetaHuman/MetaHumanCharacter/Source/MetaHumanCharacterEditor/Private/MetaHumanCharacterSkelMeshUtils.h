...
struct FMetaHumanCharacterSkelMeshUtils
{
	...
	/**
	 * Compares the vertex positions and normals of each LOD in the input DNA and the Skeletal Mesh
	 * Note that the face state is evaluated internally.
	 *
	 * @param InMeshIndicesToCheck Optional DNA mesh indices to check. When empty, every mesh in the DNA is checked.
	 */
	 ...
	 * @param InMeshIndicesToCheck Optional DNA mesh indices to check. When empty, every mesh in the DNA is checked.
	 */
	static bool CompareDnaToStateVertices(
		TSharedPtr<const IDNAReader> InDNAReader, 
		const TArray<FVector3f>& InStateVertices,
		TSharedPtr<const FMetaHumanCharacterIdentity::FState> InState,
		float Tolerance = UE_KINDA_SMALL_NUMBER,
	...
	/** Converts component joint positions and rotations in DNA space and converts them to UE coordinates */
	static TArray<FTransform> GetUECoordinateComponentTransforms(USkeletalMesh* InSkelMesh, 
	...
	/** Checks for shared joints between source and target meshes and updates target ref pose shared joints from the source ref pose */
	...
	*/
	...
	static UDNA* GetArchetypeDNAAsset(const EMetaHumanImportDNAType InImportDNAType, UObject* InOuter);
	...
	static TSoftObjectPtr<UObject> GetFaceArchetypeDefaultAnimatingRig(EMetaHumanCharacterTemplateType InTemplateType);
	...
	static TSoftObjectPtr<UObject> GetBodyArchetypeDefaultAnimatingRig(EMetaHumanCharacterTemplateType InTemplateType);
};
