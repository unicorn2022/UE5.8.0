...
bool WardrobeItemHasClothingMask(TNotNull<const UMetaHumanWardrobeItem*> WardrobeItem)
{
	if(const UMetaHumanItemEditorPipeline* EditorPipeline = WardrobeItem->GetEditorPipeline())
	{
		...
		if (const FStructProperty* BodyHiddenFaceMapTextureProp = CastField<FStructProperty>(EditorPipeline->GetClass()->FindPropertyByName(TEXT("BodyHiddenFaceMapTexture"))))
		{
			if (FProperty* TextureProp = BodyHiddenFaceMapTextureProp->Struct->FindPropertyByName(TEXT("Texture")))
			{
				const void* StructValuePtr = BodyHiddenFaceMapTextureProp->ContainerPtrToValuePtr<void>(EditorPipeline);
				UTexture2D* Texture = nullptr;
				TextureProp->GetValue_InContainer(StructValuePtr, &Texture);
				if (Texture)
				{
					return true;
				}
