...
void FMetaHumanCharacterEditorCloudRequests::InitFaceAutoRigParams(const FMetaHumanCharacterIdentity::FState& InFaceState, TSharedRef<const IDNAReader> InFaceDNAReader, UE::MetaHuman::FTargetSolveParameters& OutAutoRigParameters)
{
	...
	InFaceState.GetFaceModelCoefficients(OutAutoRigParameters.Coefficients);
			...
			if (FImageUtils::DecompressImage(PngData.GetData(), PngData.Num(), TextureImage))
			{
				...
				if (TObjectPtr<UTexture2D>* FoundTexture = InMetaHumanCharacter->SynthesizedFaceTextures.Find(TextureType))
				{
					...
					if (Texture->HasAnyFlags(RF_Transient))
					{
						...
						// wipe out the PlatformData (why? if you mean to delete the whole texture, that will do this)
						Texture->SetPlatformData(new FTexturePlatformData());
					}

					// note "FoundTexture" variable is overwritten below, 
					//	we make a new one and replace it in the map
					//  did we mean to delete the UOBject ?  Mark it for GC ?
					//Texture->MarkAsGarbage();

					// Create a new texture from the image
					Texture = FMetaHumanCharacterTextureSynthesis::CreateFaceTextureFromSource(TextureType, TextureImage, InCharacterData->bFaceVirtualTextures);

					if (Texture != nullptr)
					{
						// we're deleting the source and just keeping the platformdata
						Texture->BlockOnAnyAsyncBuild();
					...
					{
						UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Failed to create face texture '{TextureTypeName}'", *TextureTypeName);
					}
				}
			}
			else
	...
	for (EBodyTextureType TextureType : TEnumRange<EBodyTextureType>())
	{
		...
		if (!PngData.IsEmpty())
		{
			...
			if (FImageUtils::DecompressImage(PngData.GetData(), PngData.Num(), TextureImage))
			{
				...
				{
					// code dupe of similar logic earlier ; keep them in sync
					...
					Texture = FMetaHumanCharacterBodyTextureUtils::CreateBodyTextureFromSource(TextureType, TextureImage, InCharacterData->bBodyVirtualTextures);
					...
					{
						UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Failed to create body texture '{TextureTypeName}'", *TextureTypeName);
					}
				}
			}
			else
			{
				UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Failed to decompress body texture '{TextureTypeName}' from service", *TextureTypeName);
			}
		}
	}
