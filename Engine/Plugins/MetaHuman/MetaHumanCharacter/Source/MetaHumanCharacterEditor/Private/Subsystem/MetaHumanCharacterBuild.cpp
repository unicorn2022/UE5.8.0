...
namespace UE::MetaHuman::Build
{
	...
	static bool ConfirmUpdateCommonAssets(const FString& InCommonAssetsPath)
	{
		const FText Message = FText::Format(
			LOCTEXT("MetaHumanCommonFolderOverwriteWarning",
				"The assembly is about to write over assets in the MetaHuman Common folder.\n"
				"This may affect MetaHumans that depend on these assets:\n\n{0}\n\nDo you want to continue?"),
			FText::FromString(InCommonAssetsPath));
	...
	{
		const FString DefaultCommonDirectory = TEXT("/Game/MetaHumans/Common");
		const FString PathToMHCommonSkeleton = InCommonAssetsPath / TEXT("Female/Medium/NormalWeight/Body/metahuman_base_skel.metahuman_base_skel");
		...
		// If MHAssetVersion is not compatible and InCommonAssetsPath is not default as well, prompt user before updating non-default common assets
		else if (InCommonAssetsPath != DefaultCommonDirectory)
		{
			return ConfirmUpdateCommonAssets(InCommonAssetsPath);
		}
	...
	// MH-16474: Validate the target package handed back by UPackageTools::FindOrCreatePackageForAssetType
	// before duplicating into it. Two things can go wrong:
	//
	//   1. ON-DISK CLASS MISMATCH (fatal). When an on-disk asset of a different class already
	//      occupies the requested path, FindOrCreatePackageForAssetType silently falls back to a
	//      unique-name package (e.g. /Common/.../Foo_1). That breaks downstream references which
	//      expect the asset at the original path, so we log an error and tell the caller to bail
	//      (same behaviour as the in-memory class-mismatch guard).
	//
	//   2. ORPHAN IN PACKAGE (warning). A top-level asset whose name doesn't match the package
	//      path — typically left behind by a prior rename done with REN_DontCreateRedirectors.
	//      DuplicateObject won't conflict on a differently-named object, so we proceed; but we
	//      surface a warning so the user can clean up manually before the package ends up with
	//      two same-class assets, one of which is no longer referenced.
	//
	//      Filters: IsAsset() + RF_Standalone exclude CDOs, generated classes, ubergraph methods
	//      and other inner sub-objects that legitimately share the package with their primary
	//      asset (especially Blueprints).
	//
	// Returns true if the target package is valid to duplicate into, false if the caller must
	// bail out.
	static bool CheckValidDependencyPackage(const UObject* DependencyAsset, const UPackage* TargetPackage, const FString& AssetPackagePath)
	{
		// (1) On-disk class mismatch — FindOrCreatePackageForAssetType returned a different
		// package than we asked for. Look up the conflicting class for the error message.
		if (TargetPackage->GetName() != AssetPackagePath)
		{
			FString ExistingClassName = TEXT("Unknown");
			IAssetRegistry& AssetRegistry = IAssetRegistry::GetChecked();
			TArray<FAssetData> ExistingAssets;
			AssetRegistry.GetAssetsByPackageName(FName{ AssetPackagePath }, ExistingAssets, /*bIncludeOnlyOnDiskAssets=*/true);
			if (ExistingAssets.Num() > 0)
			{
				ExistingClassName = ExistingAssets[0].AssetClassPath.ToString();
			}

			FMessageLog(UE::MetaHuman::MessageLogName).Error(
				FText::Format(
					LOCTEXT("DuplicateDependencyClassMismatchOnDisk", "Cannot copy Common asset '{0}' an incompatible asset of type '{1}' already exists at the target location. Remove or rename the conflicting asset and try again."),
					FText::FromString(DependencyAsset->GetName()),
					FText::FromString(ExistingClassName)));

			return false;
		}

		// (2) Orphan asset already in the package — warn and continue.
		const FName ExpectedName = DependencyAsset->GetFName();
		ForEachObjectWithOuter(const_cast<UPackage*>(TargetPackage),
			[TargetPackage, ExpectedName](UObject* InObject)
			{
				if (InObject->IsAsset()
					&& InObject->HasAnyFlags(RF_Standalone)
					&& !InObject->IsA<UObjectRedirector>()
					&& InObject->GetFName() != ExpectedName)
				{
					FMessageLog(UE::MetaHuman::MessageLogName).Warning(
						FText::Format(
							LOCTEXT("DuplicateDependencyOrphanInPackage", "Common asset target package '{0}' contains an orphan asset '{1}' of type '{2}' that does not match the package path. Continuing assembly may leave the package in an inconsistent state. Consider removing or renaming the orphan asset."),
							FText::FromString(TargetPackage->GetName()),
							FText::FromString(InObject->GetName()),
							FText::FromString(InObject->GetClass()->GetName())));
				}
			},
...
IMetaHumanCharacterBuildExtender::~IMetaHumanCharacterBuildExtender() {}
const FName IMetaHumanCharacterBuildExtender::FeatureName(TEXT("MetaHumanCharacterBuildExtender"));
TSet<FString> IMetaHumanCharacterBuildExtender::GetMountPoints() const { return {}; }
TSet<FName> IMetaHumanCharacterBuildExtender::GetPackageRootsToCopyFrom() const { return {}; }
TArray<FName> IMetaHumanCharacterBuildExtender::GetAnimationSystemOptions() const { return {}; }
bool IMetaHumanCharacterBuildExtender::IsUEFN() const { return false; }
const TSoftClassPtr<UMetaHumanCollectionPipeline> IMetaHumanCharacterBuildExtender::GetOverwritePipeline(EMetaHumanDefaultPipelineType InPipelineType, EMetaHumanQualityLevel InQualityLevel) { return {}; }
void IMetaHumanCharacterBuildExtender::SetCommonPath(const FString& InCommonFolderPath) {}
TArray<UObject*>IMetaHumanCharacterBuildExtender:: GetRootObjects(const FMetaHumanCharacterEditorBuildParameters& BuildParameters) const { return {}; }
void IMetaHumanCharacterBuildExtender::PostDuplicateDependencies(const FString& AssetBuildPath, TNotNull<UMetaHumanInstance*> CharacterInstance) {}
...
bool FMetaHumanCharacterEditorBuild::DuplicateDepedenciesToNewRoot(
	...
	TFunction<bool(const UObject*)> InIsAssetSupported)
{
	...
	// Collect parent materials whose child MIs need cleanup
	TArray<UMaterialInterface*> MaterialParentsForChildCleanup;
	...
	for (UObject* DependencyAsset : InDependencies)
	{
		...
		if (!UE::MetaHuman::Build::CheckValidDependencyPackage(DependencyAsset, TargetPackage, AssetPackagePath))
		{
			return false;
		}
		...
		{
			// MH-16474: Bail out if the existing asset is a fundamentally incompatible class.
			if (!FoundObject->IsA(DependencyAsset->GetClass()) && !DependencyAsset->IsA(FoundObject->GetClass()))
			{
				FMessageLog(UE::MetaHuman::MessageLogName).Error(
					FText::Format(
						LOCTEXT("DuplicateDependencyClassMismatch", "Cannot copy Common asset '{0}' an incompatible asset of type '{1}' "
							"already exists at the target location. Remove or rename the conflicting asset and try again."),
						FText::FromString(DependencyAsset->GetName()),
						FText::FromString(FoundObject->GetClass()->GetName())));
			...
			if (const TMap<FName, FString>* Metadata = FMetaData::GetMapForObject(FoundObject))
			{
				if (const FString* AssetMetaHumanVersionStr = Metadata->Find(VersionTag))
				{
					...
					{
						// This should never occur, dependencies that are up to date and about to be used should not be marked for delete
						if (!FoundObject->IsValidLowLevel()
							|| FoundObject->HasAnyFlags(RF_BeginDestroyed | RF_FinishDestroyed)
							|| FoundObject->HasAnyInternalFlags(EInternalObjectFlags::Garbage))
						{
							UE_LOGFMT(LogMetaHumanCharacterEditor, Verbose,
								"DuplicateDepedenciesToNewRoot: cannot reuse Common asset {0} — existing up-to-date target is pending destruction; aborting Assemble.",
								*DependencyAsset->GetName());
							return false;
						}
			...
			// Defer to after the duplication loop so we can pass the full set of just-produced/just-reused dependency targets as a protected set 
			if (UMaterialInterface* ParentMI = Cast<UMaterialInterface>(FoundObject))
			{
				MaterialParentsForChildCleanup.AddUnique(ParentMI);
			}
		}
	...
	// Remove MIs & MICs that have an out of date parent MI, but ensure this does not touch any of the dependency MIs
	{
		TSet<UObject*> NewDependencies;
		NewDependencies.Reserve(OutDuplicatedDependencies.Num());
		Algo::Transform(OutDuplicatedDependencies, NewDependencies, &TPair<UObject*, UObject*>::Value);

		for (UMaterialInterface* ParentMI : MaterialParentsForChildCleanup)
		{
			if (ParentMI && ParentMI->IsValidLowLevel())
			{
				DeleteMaterialInstanceChildren(ParentMI, &NewDependencies);
			}
		}

		// MH-18820: RecacheUniformExpressions calls in DeleteMaterialInstanceChildren() only enqueue the proxy 
		// onto FMaterialRenderProxy::DeferredUniformExpressionCacheRequests. Run an explicit flush here to
		// ensure that all null'ed resources are updated
		ENQUEUE_RENDER_COMMAND(FMetaHumanCharacterEditorBuild_UpdateDeferredCachedUniformExpressions)(
	...
	// Ensure compilation has finished for duplicated objects since replacing refs
	// can potentially trigger further compilation requests. Done BEFORE building
	// ReplaceRequests so each Request.New is read from post-flush state and never
	// captures a UBlueprint::GeneratedClass that the reinstancer subsequently
	// renames to REINST_* and the internal CollectGarbage reclaims.
	FAssetCompilingManager::Get().FinishCompilationForObjects(DuplicatedDependenciesArray);

	// Hold strong refs to every Request.New across ForceReplaceReferences.
	TArray<TStrongObjectPtr<UObject>> RequestNewRoots;
	RequestNewRoots.Reserve(OutDuplicatedDependencies.Num() * 2);
	...
	for (const TPair<UObject*, UObject*>& DependencyPair : OutDuplicatedDependencies)
	{
		...
		if (ObjectToReplaceWith != nullptr)
		{
			RequestNewRoots.Emplace(ObjectToReplaceWith);
		}
		...
		{
			UBlueprint* NewBlueprint = Cast<UBlueprint>(ObjectToReplaceWith);
			UClass* OldClass = OldBlueprint->GeneratedClass;

			if (OldClass != nullptr && NewBlueprint != nullptr && NewBlueprint->GeneratedClass != nullptr)
			{
				UClass* ClassToReplaceWith = NewBlueprint->GeneratedClass;
			
				TArray<UObject*>& OldClassArray = Storage.AddDefaulted_GetRef();
				OldClassArray.Add(OldClass);

				RequestNewRoots.Emplace(ClassToReplaceWith);
	...
	// MH-18820: Force GC to actually destroy the MarkAsGarbage'd MICs left over
	// from the earlier DeleteMaterialInstanceChildren cleanup.
	// Note that this explicit GC call is not blocked by the FGCScopeGuard in FMetaHumanCharacterEditorBuild::BuildMetaHumanCharacter()
...
void FMetaHumanCharacterEditorBuild::DeleteMaterialInstanceChildren(TNotNull<UMaterialInterface*> InParentMI, const TSet<UObject*>* InProtectedAssets)
{
	...
	ForEachObjectOfClasses(
		...
		[InParentMI, InProtectedAssets, &ChildMIs](UObject* Obj)
		{
			if (UMaterialInstance* MI = Cast<UMaterialInstance>(Obj))
			{
				...
				if (InProtectedAssets && InProtectedAssets->Contains(MI))
				{
					return;
				}
				ChildMIs.Add(MI);
	...
	{
		// MH-18820: Two-track treatment depending on whether this is a persistent asset or a transient runtime MI.
		if (MI->HasAnyFlags(RF_Standalone))
		{
			// Invalidate any render proxy state of this MI
			MI->RecacheUniformExpressions(/*bRecreateUniformBuffer=*/true);
		}
		else
		{
			// Destroy transient MIs. Null its texture parameter values first so
			// the proxy's last in-flight evaluation (before destruction completes) reads nullptr instead
			// of a UTexture that may itself be about to be garbage-collected
...
TSoftClassPtr<UMetaHumanCollectionPipeline> FMetaHumanCharacterEditorBuild::GetDefaultPipelineClass(EMetaHumanDefaultPipelineType InPipelineType, EMetaHumanQualityLevel InQualityLevel)
{
	...
	if (IModularFeatures::Get().IsModularFeatureAvailable(IMetaHumanCharacterBuildExtender::FeatureName))
	{
		...
		for (IMetaHumanCharacterBuildExtender* Extender : Extenders)
		{
			const TSoftClassPtr<UMetaHumanCollectionPipeline> OverwritePipeline = Extender->GetOverwritePipeline(InPipelineType, InQualityLevel);
			if (!OverwritePipeline.IsNull())
			{
				OutPipelineClass = OverwritePipeline;
			}
		}
	}
...
void FMetaHumanCharacterEditorBuild::BuildMetaHumanCharacter(TNotNull<UMetaHumanCharacter*> InMetaHumanCharacter, const FMetaHumanCharacterEditorBuildParameters& InParams)
{
	...
	// Collection won't be kept after assembly is done
	const bool bMakePackageTransient = true;
	...
	// Force use of absolute path when building from MHC
	...
	if (Collection->GetEditorPipeline()->GetEditorActorClass())
	{
		...
		UMetaHumanInstance* Instance = Cast<UMetaHumanInstance>(CreateNewGeneratedAsset(
			AbsBuildPath,
			InstanceName,
			UMetaHumanInstance::StaticClass(),
			Collection->GetDefaultInstance(),
			bMakePackageTransient));

		if (!Instance)
		{
			ShowNotification(LOCTEXT("ErrorCreatingInstance", "Failed to create MetaHuman Instance asset"), SNotificationItem::ECompletionState::CS_Fail);
			return;
		}
		...
		// Call BeginCharacterEditorAssemble for the pipeline to prepare the unpack operation	
		const FString CharacterNameToUse = InParams.NameOverride.IsEmpty() ? CharacterName : InParams.NameOverride;
		if (!Collection->GetMutablePipeline()->GetMutableEditorPipeline()->BeginCharacterEditorAssemble(Collection, CharacterNameToUse))
		{
			const FText SuccessMessageText = FText::GetEmpty();
			const FText FailureMessageText = LOCTEXT("BeginCharacterEditorAssembleFailed", "Failed to initialize MetaHuman Character assembly");
			ReportMessageLogErrors(false, SuccessMessageText, FailureMessageText);
			return;
		}
		...
		ON_SCOPE_EXIT
		{
			...
			Collection->GetMutablePipeline()->GetMutableEditorPipeline()->EndCharacterEditorAssemble(Collection);
		};
		...
		// Build collection
		{
			...
			Collection->Build(
				...
				Instance->ToPinnedSlotSelections(EMetaHumanUnusedSlotBehavior::PinnedToEmpty));
			...
			{
				UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Collection build failed");

				const FText FailureMessageText = LOCTEXT("CharacterAssemblyFailed", "MetaHuman Character assembly failed");
				ReportMessageLogErrors(false, FText::GetEmpty(), FailureMessageText);

				return;
			}
		}

		// Unpack collection assets
		{
			TOptional<EMetaHumanCharacterAssetsUnpackResult> UnpackResult;

			Collection->UnpackAssets(FOnMetaHumanCharacterAssetsUnpacked::CreateLambda(
				[&UnpackResult](EMetaHumanCharacterAssetsUnpackResult Result) { UnpackResult = Result; }));

			// For now, we assume unpack completes synchronously
			check(UnpackResult.IsSet());

			if (UnpackResult.GetValue() == EMetaHumanCharacterAssetsUnpackResult::Failed)
			{
				UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Collection unpack failed");
		...
		// Assemble instance
		{
			...
			{
				UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Instance assembly failed");
		...
		// Unpack instance
		if (!Instance->TryUnpack(Collection->GetUnpackFolder()))
		{
			UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Instance unpack failed");
		...
		// Copy dependencies and fix up the actor blueprint
		{
			...
			if (!InParams.CommonFolderPath.IsEmpty())
			{
				...
				if (!DuplicateDepedenciesToNewRoot(PluginDependencies, InParams.CommonFolderPath, UnpackedDependencies, DuplicatedDependencies,
					[](const UObject* Obj) -> bool
					{
						return true;
					}))
				{
					const FText FailureMessageText = LOCTEXT("CommonAssetConflict",
						"MetaHuman assembly failed due to conflicting assets in the Common folder. Check the Message Log for details.");
					ReportMessageLogErrors(false, FText::GetEmpty(), FailureMessageText);
					return;
				}
				...
				if (IModularFeatures::Get().IsModularFeatureAvailable(IMetaHumanCharacterBuildExtender::FeatureName))
				{
					...
					for (IMetaHumanCharacterBuildExtender* Extender : Extenders)
					{
						Extender->PostDuplicateDependencies(AbsBuildPath, Instance);
					}
				}
			}
			...
			for (const TPair<EMetaHumanQualityLevel, TSoftClassPtr<UMetaHumanCollectionPipeline>>& LegacyPipelinePair : BuildSettings->DefaultCharacterLegacyPipelines)
			{
				...
				if (Collection->GetPipeline()->GetClass() == LegacyPipeline.Get())
				{
			...
			UE::MetaHuman::Analytics::RecordBuildPipelineCharacterEvent(InMetaHumanCharacter, Collection->GetPipeline()->GetClass());
		}

		const FText SuccessMessageText = LOCTEXT("CharacterAssemblySucceeded", "MetaHuman Character assembly succeeded");
		ReportMessageLogErrors(true, SuccessMessageText, FText::GetEmpty());
	}
	else
	{
		const FText SuccessMessageText = FText::GetEmpty();
...
void FMetaHumanCharacterEditorBuild::StripLODsFromMesh(TNotNull<USkeletalMesh*> InSkeletalMesh, const TArray<int32>& InLODsToKeep)
{
	...
	InSkeletalMesh->Modify();
	{
		...
		if (TSharedPtr<IDNAReader> DNAReader = USkelMeshDNAUtils::GetDNAReader(InSkeletalMesh))
		{
			...
			USkelMeshDNAUtils::SetDNAReader(InSkeletalMesh, OutputDNAReader, EDNACopyPolicy::Alias, ERigLogicInitPolicy::Defer);
			UDNA* DNAAsset = USkelMeshDNAUtils::GetMeshDNAAsset(InSkeletalMesh);
			if (DNAAsset)
			{
			...
			{
				UDNAAsset* LegacyDNAAsset = USkelMeshDNAUtils::GetMeshDNA(InSkeletalMesh);
				if (LegacyDNAAsset)
				{
					LegacyDNAAsset->RestoreLegacyUEMHCCompatibility();
				}
			}
		}
