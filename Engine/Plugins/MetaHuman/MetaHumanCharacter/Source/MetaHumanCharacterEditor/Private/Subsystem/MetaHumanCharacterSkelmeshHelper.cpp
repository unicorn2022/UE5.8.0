...
namespace UE::MetaHuman::Build
{
	...
	)
	{
		...
		if (Profiles.IsEmpty())
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "Mesh description doesn't have a skin weight attribute.");
			return false;
		}
		...
		for (const FVertexID VertexID : InMeshDescription.Vertices().GetElementIDs())
		{
			for (FBoneWeight BoneWeight : VertexSkinWeights.Get(VertexID))
			{
				...
				{
					UE_LOGF(LogMetaHumanCharacterEditor, Error, "Mesh description's skin weight refers to a non-existent bone (%d of %d).", BoneWeight.GetBoneIndex(), BoneIndexMax);
					return false;
				}
			}
		}
	...
	)
	{
		...
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "Skeletal mesh '%ls' is not empty", *InSkeletalMesh->GetPathName());
			return false;
		}

		if (InMeshDescriptions.IsEmpty())
		{
			UE_LOGF(LogMetaHumanCharacterEditor, Error, "No mesh descriptions given");
			return false;
		}
