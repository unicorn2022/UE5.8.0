...
namespace UE::MetaHuman
{
	static int32 GetMappedBodyTextureId(int32 InBodyTextureParam)
	{
		...
		if (InBodyTextureParam < 0 || InBodyTextureParam >= BodyTextureMapping.Num())
		{
			UE_LOGFMT(LogMetaHumanCharacterEditor, Error, "Body Texture Param outside valid range");
			return BodyTextureMapping[0]; // Return the first texture id
		}
	...
	static TObjectPtr<UTexture2D> GetBodyTexture(EBodyTextureType InTextureType, int32 InSkinToneIndex, int32 InSurfaceMapId, bool bInVirtualTexture)
	{
		FString TexturePath;
...
void FMetaHumanCharacterBodyTextureUtils::InitBodyTextureData(const FMetaHumanCharacterSkinProperties& InSkinProperties,
															  const TMap<EBodyTextureType, FMetaHumanCharacterTextureInfo>& InTextureInfo,
															  bool bInUseVirtualTextures,
	...
	if (OutBodyTextures.IsEmpty())
	{
		...
		if (bMetaHumanContentAvailable)
		{
			for (EBodyTextureType TextureType : TEnumRange<EBodyTextureType>())
			{
				...
				{
					OutBodyTextures.FindOrAdd(TextureType) = UE::MetaHuman::GetBodyTexture(TextureType, GetSkinToneIndex(InSkinProperties), GetBodySurfaceMapId(InSkinProperties), bInUseVirtualTextures);
				}
			}
		}
	}
...
void FMetaHumanCharacterBodyTextureUtils::UpdateBodyTextureSet(const TOptional<FMetaHumanCharacterSkinSettings>& InCharacterSkinSettings,
															   const FMetaHumanCharacterSkinProperties& InSkinProperties,
															   bool bInUseVirtualTextures,
															   TMap<EBodyTextureType, FMetaHumanCharacterTextureInfo>& InOutTextureInfo,
															   TMap<EBodyTextureType, TObjectPtr<class UTexture2D>>& InOutBodyTextures)
{
	if (InCharacterSkinSettings.IsSet()) 
	{
		...
		if (OldSkinProperties.BodyTextureIndex != InSkinProperties.BodyTextureIndex)
		{
			...
			for (EBodyTextureType TextureType : TextureIndexDependentBodyTextures)
			{
				...
				InOutBodyTextures.FindOrAdd(TextureType) = UE::MetaHuman::GetBodyTexture(TextureType,
																			   GetSkinToneIndex(InSkinProperties),
																			   GetBodySurfaceMapId(InSkinProperties),
																			   bInUseVirtualTextures);
			}
		}
...
{
	UpdateBodySkinTone(InSkinProperties, InFaceTextureSynthesizer, InFaceMaterialSet, InBodyMID);
	
	UpdateBodyMaterialFromSkinProperties(InSkinProperties, InBodyTextures, InFaceMaterialSet, InBodyMID);
}
...
UTexture2D* FMetaHumanCharacterBodyTextureUtils::CreateBodyTextureFromSource(EBodyTextureType InTextureType, FImageView InTextureImage, bool bInMakeVirtualIfSupported)
{
	...
	const bool bIsVTSupported =
		InTextureType == EBodyTextureType::Body_Basecolor ||
		InTextureType == EBodyTextureType::Body_Normal ||
		InTextureType == EBodyTextureType::Body_Cavity;

	const bool bEnableVTStreaming = bIsVTSupported && bInMakeVirtualIfSupported;

	const FString VTSuffix = bEnableVTStreaming ? TEXT("_VT") : TEXT("");
	...
	const FString CandidateName = FString::Format(TEXT("T_Body_{0}{1}"), { TextureName, VTSuffix });
	...
	if (Texture)
	{
		...
		// Enable virtual texture streaming if supported for this texture type
		Texture->VirtualTextureStreaming = bEnableVTStreaming;
...
void FMetaHumanCharacterBodyTextureUtils::UpdateBodySkinTone(
	const FMetaHumanCharacterSkinProperties& InSkinProperties,
	const FMetaHumanFaceTextureSynthesizer& InFaceTextureSynthesizer,
	const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet,
	TNotNull<UMaterialInstanceDynamic*> InBodyMID)
...
void FMetaHumanCharacterBodyTextureUtils::UpdateBodyMaterialFromSkinProperties(
	const FMetaHumanCharacterSkinProperties& InSkinProperties, 
	const TMap<EBodyTextureType, TObjectPtr<UTexture2D>>& InBodyTextures, 
	const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet, 
	TNotNull<UMaterialInstanceDynamic*> InBodyMID)
