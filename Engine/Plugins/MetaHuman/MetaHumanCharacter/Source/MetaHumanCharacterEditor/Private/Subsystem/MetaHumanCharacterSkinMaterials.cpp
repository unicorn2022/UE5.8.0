...
const FName FMetaHumanCharacterSkinMaterials::BodySlotName = TEXT("body_shader_shader");
...
void FMetaHumanCharacterSkinMaterials::ForEachFaceMaterialSlot(const FMetaHumanCharacterFaceMaterialSet& InMaterialSet, TFunction<void(FName, const class UMaterialInterface*)> InCallback)
{
	for (EMetaHumanCharacterSkinMaterialSlot SkinMaterialSlot : TEnumRange<EMetaHumanCharacterSkinMaterialSlot>())
	{
		if (InMaterialSet.Skin.Contains(SkinMaterialSlot))
		{
			InCallback(FMetaHumanCharacterSkinMaterials::GetSkinMaterialSlotName(SkinMaterialSlot), Cast<const UMaterialInterface>(InMaterialSet.Skin[SkinMaterialSlot]));
		}
		else
		{
			InCallback(FMetaHumanCharacterSkinMaterials::GetSkinMaterialSlotName(SkinMaterialSlot), nullptr);
		}
	}

	InCallback(FMetaHumanCharacterSkinMaterials::EyeLeftSlotName, InMaterialSet.EyeLeft);
	InCallback(FMetaHumanCharacterSkinMaterials::EyeRightSlotName, InMaterialSet.EyeRight);
	InCallback(FMetaHumanCharacterSkinMaterials::EyeShellSlotName, InMaterialSet.EyeShell);
	InCallback(FMetaHumanCharacterSkinMaterials::EyeEdgeSlotName, InMaterialSet.LacrimalFluid);
	InCallback(FMetaHumanCharacterSkinMaterials::TeethSlotName, InMaterialSet.Teeth);
	InCallback(FMetaHumanCharacterSkinMaterials::EyelashesSlotName, InMaterialSet.Eyelashes);
	InCallback(FMetaHumanCharacterSkinMaterials::EyelashesHiLodSlotName, InMaterialSet.EyelashesHiLods);
}
...
FName FMetaHumanCharacterSkinMaterials::GetFaceTextureParameterName(EFaceTextureType InTextureType, bool bInWithVTSuport)
{
	...
	const bool bSupportsVT = InTextureType == EFaceTextureType::Basecolor ||
							 ...
							 InTextureType == EFaceTextureType::Cavity;
...
FMetaHumanCharacterFaceMaterialSet FMetaHumanCharacterSkinMaterials::GetHeadMaterialsFromMesh(TNotNull<class USkeletalMesh*> InFaceMesh)
{
	...
	FMetaHumanCharacterFaceMaterialSet FaceMaterialSet =
	{
		...
		.LacrimalFluid = GetMaterialByName(EyeEdgeSlotName),
...
void FMetaHumanCharacterSkinMaterials::ApplySkinParametersToMaterials(const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet, TNotNull<UMaterialInstanceDynamic*> InBodyMID, const FMetaHumanCharacterSkinSettings& InSkinSettings)
{
	...
	ApplyPalmParametersToMaterial(InBodyMID, InSkinSettings.Skin);
	ApplyFingernailParametersToMaterial(InBodyMID, InSkinSettings.Skin);
	ApplyToenailParametersToMaterial(InBodyMID, InSkinSettings.Skin);
}
...
void FMetaHumanCharacterSkinMaterials::ApplyPalmParametersToMaterial(TNotNull<class UMaterialInstanceDynamic*> InBodyMaterial, const FMetaHumanCharacterSkinProperties& InSkinProps)
{
	for (EMetaHumanCharacterPalmParameters PalmParams : TEnumRange<EMetaHumanCharacterPalmParameters>())
	{
		const FString PalmParamName = FString::Format(TEXT("Palm{0}"), 
			{ StaticEnum<EMetaHumanCharacterPalmParameters>()->GetAuthoredNameStringByValue(static_cast<int64>(PalmParams)) });

		if (const FFloatProperty* PalmParamProperty = FindFProperty<FFloatProperty>(FMetaHumanCharacterSkinProperties::StaticStruct(), *PalmParamName))
		{
			const float& ParamValue = PalmParamProperty->GetPropertyValue_InContainer(&InSkinProps);
			const FString PalmParamNameWithSpaces = FName::NameToDisplayString(PalmParamName, /* bIsBool */ false);
			InBodyMaterial->SetScalarParameterValue(*PalmParamNameWithSpaces, ParamValue);
		}
	}
}

void FMetaHumanCharacterSkinMaterials::ApplyNailParametersToMaterial(TNotNull<class UMaterialInstanceDynamic*> InBodyMaterial, const FMetaHumanCharacterSkinProperties& InSkinProps, const FString& InNailPrefix)
{
	for (EMetaHumanCharacterNailParameters PalmParams : TEnumRange<EMetaHumanCharacterNailParameters>())
	{
		const FString NailParamName = FString::Format(TEXT("{0}{1}"),
			{ InNailPrefix, StaticEnum<EMetaHumanCharacterNailParameters>()->GetAuthoredNameStringByValue(static_cast<int64>(PalmParams)) });

		if (const FFloatProperty* PalmParamProperty = FindFProperty<FFloatProperty>(FMetaHumanCharacterSkinProperties::StaticStruct(), *NailParamName))
		{
			const float& ParamValue = PalmParamProperty->GetPropertyValue_InContainer(&InSkinProps);
			const FString NailParamNameWithSpaces = FName::NameToDisplayString(NailParamName, /* bIsBool */ false);
			InBodyMaterial->SetScalarParameterValue(*NailParamNameWithSpaces, ParamValue);
		}
		// NailTintColor
		else if (const FStructProperty* PalmStructParamProperty = FindFProperty<FStructProperty>(FMetaHumanCharacterSkinProperties::StaticStruct(), *NailParamName))
		{
			const FLinearColor& ParamValue = *PalmStructParamProperty->ContainerPtrToValuePtr<FLinearColor>(&InSkinProps);
			const FString NailParamNameWithSpaces = FName::NameToDisplayString(NailParamName, /* bIsBool */ false);
			InBodyMaterial->SetVectorParameterValue(*NailParamNameWithSpaces, ParamValue);
		}
	}
}

void FMetaHumanCharacterSkinMaterials::ApplyFingernailParametersToMaterial(TNotNull<class UMaterialInstanceDynamic*> InBodyMaterial, const FMetaHumanCharacterSkinProperties& InSkinProps)
{
	ApplyNailParametersToMaterial(InBodyMaterial, InSkinProps, TEXT("Fingernail"));
}

void FMetaHumanCharacterSkinMaterials::ApplyToenailParametersToMaterial(TNotNull<class UMaterialInstanceDynamic*> InBodyMaterial, const FMetaHumanCharacterSkinProperties& InSkinProps)
{
	ApplyNailParametersToMaterial(InBodyMaterial, InSkinProps, TEXT("Toenail"));
}
...
void FMetaHumanCharacterSkinMaterials::ApplyTextureOverrideParameterToMaterials(const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet, TNotNull<UMaterialInstanceDynamic*> InBodyMaterial, const FMetaHumanCharacterSkinSettings& InSkinSettings)
{
	...
	if (InSkinSettings.TextureMaterialOverrides.bEnableTextureOverrides)
	{
		if (const TSoftObjectPtr<UTexture2D>* FoundBodyBaseColorOverride = InSkinSettings.TextureMaterialOverrides.TextureOverrides.Body.Find(EBodyTextureType::Body_Basecolor))
		{
...
void FMetaHumanCharacterSkinMaterials::ApplyMaterialOverrides(
	...
	const EMetaHumanCharacterSkinPreviewMaterial InPreviewMaterialType)
{
	if (!InSkinSettings.TextureMaterialOverrides.bEnableMaterialOverrides)
	{
		return;
	}

	const FMetaHumanCharacterMaterialOverrideSet& Overrides = InSkinSettings.TextureMaterialOverrides.MaterialOverrides;
	const bool bIsEditable = (InPreviewMaterialType == EMetaHumanCharacterSkinPreviewMaterial::Editable);

	// Helper: load a soft MIC ref, create a MID from it, and replace the target slot
	auto TryOverrideMaterial = [](const TSoftObjectPtr<UMaterialInstanceConstant>& InSoftRef, TObjectPtr<UMaterialInstance>& InOutSlot, const TCHAR* InName)
	{
		if (InSoftRef.IsNull())
		{
			return;
		}
		UMaterialInstanceConstant* OverrideMIC = InSoftRef.LoadSynchronous();
		if (!OverrideMIC)
		{
			return;
		}
		const FName MIDName = MakeUniqueObjectName(GetTransientPackage(), UMaterialInstanceDynamic::StaticClass(), FName(InName));
		UMaterialInstanceDynamic* NewMID = UMaterialInstanceDynamic::Create(OverrideMIC, GetTransientPackage(), MIDName);
		InOutSlot = NewMID;
	};

	// Face LOD materials and body - only in Editable preview mode
	if (bIsEditable)
	{
		for (const TPair<EMetaHumanCharacterSkinMaterialSlot, TSoftObjectPtr<UMaterialInstanceConstant>>& FaceLOD : Overrides.Skin)
		{
			const FString Name = FString::Printf(TEXT("MID_Override_Face_%s"),
				*StaticEnum<EMetaHumanCharacterSkinMaterialSlot>()->GetAuthoredNameStringByValue(static_cast<int64>(FaceLOD.Key)));
			TryOverrideMaterial(FaceLOD.Value, InOutFaceMaterialSet.Skin[FaceLOD.Key], *Name);
		}

		if (!Overrides.Body.IsNull())
		{
			if (UMaterialInstanceConstant* BodyMIC = Overrides.Body.LoadSynchronous())
			{
				const FName MIDName = MakeUniqueObjectName(GetTransientPackage(), UMaterialInstanceDynamic::StaticClass(), TEXT("MID_Override_Body"));
				InOutBodyMaterial = UMaterialInstanceDynamic::Create(BodyMIC, GetTransientPackage(), MIDName);
			}
		}
	}

	// Teeth and eyes overrides - apply in all preview modes
	auto GetFaceFeatureSlot = [&InOutFaceMaterialSet](const EMetaHumanCharacterTeethAndEyesSlot InSlot) -> TObjectPtr<UMaterialInstance>*
	{
		switch (InSlot)
		{
			case EMetaHumanCharacterTeethAndEyesSlot::EyeLeft:
				return &InOutFaceMaterialSet.EyeLeft;
			case EMetaHumanCharacterTeethAndEyesSlot::EyeRight:
				return &InOutFaceMaterialSet.EyeRight;
			case EMetaHumanCharacterTeethAndEyesSlot::EyeShell:
				return &InOutFaceMaterialSet.EyeShell;
			case EMetaHumanCharacterTeethAndEyesSlot::LacrimalFluid:
				return &InOutFaceMaterialSet.LacrimalFluid;
			case EMetaHumanCharacterTeethAndEyesSlot::Teeth:
				return &InOutFaceMaterialSet.Teeth;
			case EMetaHumanCharacterTeethAndEyesSlot::Eyelashes:
				return &InOutFaceMaterialSet.Eyelashes;
			case EMetaHumanCharacterTeethAndEyesSlot::EyelashesHiLods:
				return &InOutFaceMaterialSet.EyelashesHiLods;
			default:
				return nullptr;
		}
	};

	for (const auto& Entry : Overrides.TeethAndEyes)
	{
		if (TObjectPtr<UMaterialInstance>* Slot = GetFaceFeatureSlot(Entry.Key))
		{
			const FString Name = FString::Printf(TEXT("MID_Override_%s"),
				*StaticEnum<EMetaHumanCharacterTeethAndEyesSlot>()->GetAuthoredNameStringByValue(static_cast<int64>(Entry.Key)));
			TryOverrideMaterial(Entry.Value, *Slot, *Name);
		}
	}
}
...
void FMetaHumanCharacterSkinMaterials::ApplySynthesizedTexturesToFaceMaterial(const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet, const TMap<EFaceTextureType, TObjectPtr<UTexture2D>>& InSynthesizedFaceTextures)
{
	for (const TPair<EFaceTextureType, TObjectPtr<UTexture2D>>& FaceTexturePair : InSynthesizedFaceTextures)
	{
		...
		if (Texture->VirtualTextureStreaming && !Texture->IsCurrentlyVirtualTextured())
		{
...
FLinearColor FMetaHumanCharacterSkinMaterials::GetScleraTintBasedOnSkinTone(const FMetaHumanCharacterSkinSettings& InSkinSettings)
{
	return FMath::Lerp(FLinearColor::White, FLinearColor{ 0.77f, 0.71f, 0.68f }, InSkinSettings.Skin.U);
}
...
void FMetaHumanCharacterSkinMaterials::ApplyEyelashesPropertiesToMaterial(const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet, const FMetaHumanCharacterEyelashesProperties& InEyelashesProperties, const bool bAlwaysUseCards)
{
	...
	const float Opacity = InEyelashesProperties.bEnableGrooms && !bAlwaysUseCards ? 0.0f : 1.0f;
	UMaterialInstanceDynamic* EyelashesLowerLods = Cast<UMaterialInstanceDynamic>(InFaceMaterialSet.Eyelashes);
	if (EyelashesLowerLods)
	{
		EyelashesLowerLods->SetScalarParameterValue(TEXT("Opacity"), Opacity);
		EyelashesLowerLods->SetScalarParameterValue(TEXT("OpacityRT"), Opacity);
	}
}

void FMetaHumanCharacterSkinMaterials::ToggleEyelashesMaterialOpacity(const FMetaHumanCharacterFaceMaterialSet& InFaceMaterialSet, bool bEyelashesGroomEnabled, bool bAlwaysUseHairCards)
{
	const float Opacity = bEyelashesGroomEnabled && !bAlwaysUseHairCards ? 0.0f : 1.0f;
	UMaterialInstanceDynamic* EyelashesLowerLods = Cast<UMaterialInstanceDynamic>(InFaceMaterialSet.Eyelashes);
	if (EyelashesLowerLods)
	{
		EyelashesLowerLods->SetScalarParameterValue(TEXT("Opacity"), Opacity);
...
UTexture2D* FMetaHumanCharacterSkinMaterials::GetEyelashesMask(const FMetaHumanCharacterEyelashesProperties& InEyelashesProperties)
{
	...
	{
	case EMetaHumanCharacterEyelashesType::ShortSparse:
	...
	case EMetaHumanCharacterEyelashesType::ShortThin:
	...
	case EMetaHumanCharacterEyelashesType::LongSlightCurl:
	...
	case EMetaHumanCharacterEyelashesType::LongThickCurl:
...
FMetaHumanCharacterFaceMaterialSet FMetaHumanCharacterSkinMaterials::GetHeadPreviewMaterialInstance(EMetaHumanCharacterSkinPreviewMaterial InPreviewMaterialType, bool bInWithVTSupport)
{
	...
	switch (InPreviewMaterialType)
	{
		...
		case EMetaHumanCharacterSkinPreviewMaterial::Gray:
			BaseHeadMaterialLOD0 = LoadObject<UMaterialInterface>(nullptr, TEXT("/Script/Engine.MaterialInstanceConstant'/" UE_PLUGIN_NAME "/Materials/MI_M2M_Head.MI_M2M_Head'"));
			BaseHeadMaterialLOD1 = BaseHeadMaterialLOD0;
			BaseHeadMaterialLOD2 = BaseHeadMaterialLOD0;
			BaseHeadMaterialLOD3 = BaseHeadMaterialLOD0;
			BaseHeadMaterialLOD4 = BaseHeadMaterialLOD0;
			BaseHeadMaterialLOD57 = BaseHeadMaterialLOD0;
			BaseLeftEyeMaterial = UE::MetaHuman::GetEmptyMaterial();
			BaseRightEyeMaterial = UE::MetaHuman::GetEmptyMaterial();
			BaseTeethMaterial = UE::MetaHuman::GetEmptyMaterial();
			BaseLacrimalFluidMaterial = UE::MetaHuman::GetEmptyMaterial();
			BaseEyeOcclusionMaterial = UE::MetaHuman::GetEmptyMaterial();
			BaseEyelashesMaterialLOD0 = UE::MetaHuman::GetEmptyMaterial();
			BaseEyelashesMaterialHiLODs = UE::MetaHuman::GetEmptyMaterial();
...
UMaterialInstanceDynamic* FMetaHumanCharacterSkinMaterials::GetBodyPreviewMaterialInstance(EMetaHumanCharacterSkinPreviewMaterial InPreviewMaterialType, bool bInWithVTSupport)
{
	...
	switch (InPreviewMaterialType)
	{
		...
		case EMetaHumanCharacterSkinPreviewMaterial::Gray:
			BaseBodyMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Script/Engine.MaterialInstanceConstant'/" UE_PLUGIN_NAME "/Materials/MI_M2M_Body.MI_M2M_Body'"));
			break;
		default:
			break;
	};
