...
public:
	...
	virtual void StartupModule() override
	{
		...
		{
			UE_LOGF(LogTcpMessaging, Error, "The required module 'Networking' failed to load. Plug-in 'Tcp Messaging' cannot be used.");
...
protected:
	...
	void InitializeBridge()
	{
		...
		if (!FIPv4Endpoint::Parse(ListenEndpointString, ListenEndpoint))
		{
			...
			{
				UE_LOGF(LogTcpMessaging, Warning, "Invalid setting for ListenEndpoint '%ls', listening disabled", *ListenEndpointString);
			}
		...
		for (FString& ConnectToEndpointString : ConnectToEndpointStrings)
		{
			...
			{
				UE_LOGF(LogTcpMessaging, Warning, "Invalid entry for ConnectToEndpoint '%ls', ignoring", *ConnectToEndpointString);
			}
		}
		...
		UE_LOGF(LogTcpMessaging, Log, "%ls", *Status);
