...
void FTcpMessageTransport::AddOutgoingConnection(const FIPv4Endpoint& Endpoint)
{
	...
	if (Socket == nullptr)
	{
		UE_LOGF(LogTcpMessaging, Error, "Failed to create outgoing socket for %ls", *Endpoint.ToString());
		return;
	}

	if (!Socket->Connect(*Endpoint.ToInternetAddr()))
	{
		UE_LOGF(LogTcpMessaging, Error, "Connect failed on outgoing socket for %ls", *Endpoint.ToString());
...
bool FTcpMessageTransport::TransportMessage(const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context, const TArray<FGuid>& Recipients)
{
	...
	{
		UE_LOGF(LogTcpMessaging, Verbose, "No recipients for message %ls from %ls", *Context->GetMessageTypePathName().ToString(), *Context->GetSender().ToString());
		return false;
	}

	UE_LOGF(LogTcpMessaging, Verbose, "Transporting message '%ls' to %d connections", *Context->GetMessageTypePathName().ToString(), RecipientConnections.Num());
	...
	while (!bStopping)
	{
		...
		// incoming messages
		{
			for (auto& Connection : Connections)
			{
				...
				{
					UE_LOGF(LogTcpMessaging, Verbose, "Received message '%ls'", *Message->GetMessageTypePathName().ToString());
...
void FTcpMessageTransport::HandleConnectionStateChanged(TSharedPtr<FTcpMessageTransportConnection> Connection)
{
	...
	if (State == FTcpMessageTransportConnection::STATE_Connected)
	{
		...
		UE_LOGF(LogTcpMessaging, Log, "Discovered node '%ls' on connection '%ls'...", *NodeId.ToString(), *RemoteEndpoint.ToString());
	}
	else if (NodeId.IsValid())
	{
		UE_LOGF(LogTcpMessaging, Log, "Lost node '%ls' on connection '%ls'...", *NodeId.ToString(), *RemoteEndpoint.ToString());
