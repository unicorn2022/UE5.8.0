...
bool FTcpMessageTransportConnection::Send(FTcpSerializedMessagePtr Message)
{
	...
	if (GetConnectionState() == STATE_Connected && bSentHeader)
	{
		...
		{
			UE_LOGF(LogTcpMessaging, Verbose, "Payload size write failed with code %d", (int32)ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetLastErrorCode());
			return false;
		}
		...
		{
			UE_LOGF(LogTcpMessaging, Verbose, "Payload write failed with code %d", (int32)ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetLastErrorCode());
			return false;
		}
...
{
	UE_LOGF(LogTcpMessaging, Log, "Started Connection to '%ls'", *RemoteEndpoint.ToString());
		...
		if ((!SendHeader() || !ReceiveMessages() || Socket->GetConnectionState() == SCS_ConnectionError) && bRun)
		{
			...
			if (Delay > 0)
			{
				bool bReconnectPending = false;
				{
					...
					{
						UE_LOGF(LogTcpMessaging, Verbose, "Connection to '%ls' failed, retrying...", *RemoteEndpoint.ToString());
				...
				{
					UE_LOGF(LogTcpMessaging, Error, "Reconnection to '%ls' failed.", *RemoteEndpoint.ToString());
				}
			}
			else
			{
...
bool FTcpMessageTransportConnection::ReceiveMessages()
{
	...
	// check if the socket has closed
	{
		...
		{
			UE_LOGF(LogTcpMessaging, Verbose, "Dummy read failed with code %ls", *GetReadableErrorCode());
			return false;
		}
	}
	...
	if (!bReceivedHeader)
	{
		if (Socket->HasPendingData(PendingDataSize) && PendingDataSize >= sizeof(FTcpMessageHeader))
		{
			...
			{
				UE_LOGF(LogTcpMessaging, Verbose, "Header read failed with code %ls", *GetReadableErrorCode());
				return false;
			}
			...
			{
				UE_LOGF(LogTcpMessaging, Verbose, "Header read failed with invalid header");
				return false;
			}
			else
			{
	...
	for(;;)
	{
		...
		if (RecvMessageDataRemaining == 0)
		{
			...
			{
				UE_LOGF(LogTcpMessaging, Verbose, "In progress read failed with code %ls", *GetReadableErrorCode());
				return false;
			}
			...
			if (RecvMessageDataRemaining <= 0)
			{
				UE_LOGF(LogTcpMessaging, Error, "Read failed due to invalid Message Size: %d", RecvMessageDataRemaining);
				return false;
			}
		...
		{
			UE_LOGF(LogTcpMessaging, Verbose, "Read failed with code %ls", *GetReadableErrorCode());
			return false;
		}
...
bool FTcpMessageTransportConnection::ChunkedBlockingSend(const TArray<uint8>& Payload)
{
	...
	if (!BlockingSend(Payload.GetData(), Payload.Num()))
	{
		...
		UE_LOGF(LogTcpMessaging, Verbose, "Payload write failed with code %d", (int32)LastError);

		if (LastError == SE_ENOBUFS)
		{
			UE_LOGF(LogTcpMessaging, Verbose, "Could not send %d bytes in one blocking send. Trying really small chunks.", Payload.Num());
			...
			while (BufSz > 0)
			{
				...
				else if (NumRetries < MaxRetries)
				{
					FPlatformProcess::Sleep(1);
					NumRetries++;
					UE_LOGF(LogTcpMessaging, Verbose, "%d retries sending small chunks", NumRetries);
				}
				else
				{
...
bool FTcpMessageTransportConnection::SendHeader()
{
	...
	{
		UE_LOGF(LogTcpMessaging, Verbose, "Header write failed with code %d", (int32)SocketSubsystem->GetLastErrorCode());
		return false;
	}
