...
public:
	 ...
	 */
	UPROPERTY(config, EditAnywhere, Category=Transport, meta = (ClampMin="0.01", ClampMax="100.0"), AdvancedDisplay)
	float MaxSendRate = 1.0f;

	static constexpr float MaxSendRate_Min = 0.01f;
	static constexpr float MaxSendRate_Max = 100.0f;

	/** Accessor that ensures MaxSendRate is clamped to the expected bounds. */
	float GetMaxSendRate() const
	{
		return FMath::Clamp<float>(MaxSendRate, MaxSendRate_Min, MaxSendRate_Max);
	}
	...
	UE_DEPRECATED(5.8, "UDP Messaging work queues are no longer fixed size.")
	UPROPERTY(meta=(DeprecatedProperty))
	uint16 WorkQueueSize = 1024;

	/**
	 * The maximum packet size to use for message segments (including presumed 20 byte IPv4 + 8 byte UDP headers).
	 * This should be less than or equal to your path maximum transmission unit (PMTU) to avoid fragmentation.
	 * Previous versions of UDP Messaging were hard-coded to 1130, which is a conservative, internet-safe value.
	 *
	 * If your traffic is restricted to the local network, support for an MTU of 1500 is common.
	 * This can be increased further by enabling jumbo frames on supported hardware.
	 */
	UPROPERTY(config, EditAnywhere, Category = Transport, meta = (ClampMin="576", ClampMax="65535"), AdvancedDisplay)
	uint16 MaxPacketSize = 1130;

	static constexpr uint16 MaxPacketSize_Min = 576;
	static constexpr uint16 MaxPacketSize_Max = 65535;

	/** Accessor that ensures MaxPacketSize is clamped to the expected bounds. */
	uint16 GetMaxPacketSize() const
	{
		return FMath::Clamp<uint16>(MaxPacketSize, MaxPacketSize_Min, MaxPacketSize_Max);
	}

	/** Enable UDP segmentation offload. */
	UPROPERTY(config, EditAnywhere, Category = Transport, AdvancedDisplay)
	bool bSegmentationOffload = false;
