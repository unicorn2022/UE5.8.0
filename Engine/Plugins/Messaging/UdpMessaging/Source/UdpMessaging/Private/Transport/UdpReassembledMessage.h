...
public:
		...
		, Sequence(InSequence)
	{
		...
		{
			UE_LOGF(LogUdpMessaging, Warning, "FUdpReassembledMessage: Invalid MessageSize, marking message malformed. Message=%ls, MessageSize=%lli",
				*Describe(), MessageSize);
			bIsMalformed = true;
		}
		if (SegmentCount > static_cast<uint32>(TNumericLimits<int32>::Max()) || SegmentCount == 0)
		{
			UE_LOGF(LogUdpMessaging, Warning, "FUdpReassembledMessage: Invalid SegmentCount, marking message malformed. Message=%ls, SegmentCount=%u",
...
public:
	 ...
	 */
	FUdpMessagingTime GetLastSegmentTime() const
	{
	...
	/**
	 * @return How many acknowledgments are currently pending send.
	 */
	int32 NumPendingAcknowledgements() const
	{
		return PendingAcknowledgments.Num();
	}

	/**
	 * @return The time at which any acknowledgments were last transmitted for this message.
	 */
	FUdpMessagingTime GetLastAckTime() const
	{
		return LastAckTime;
	}

	/**
	 * Gets at most the specified number of pending acknowledgments and clears them, updating LastAckTime.
	 *
	 * @return The array of pending acknowledgments removed.
	 */
	TArray<uint32> TakePendingAcknowledgments(int32 MaxAckNum, FUdpMessagingTime CurrentTime)
	{
		check(MaxAckNum > 0);
	 ...
	 */
	void Reassemble(int32 SegmentNumber, int32 SegmentOffset, const TArray<uint8>& SegmentData, const FUdpMessagingTime CurrentTime)
	{
		...
		{
			UE_LOGF(LogUdpMessaging, Warning, "FUdpReassembledMessage::Reassemble: Invalid Segment Number, ignoring segment. Message=%ls, Segment=%d",
		...
		if (PendingSegments[SegmentNumber])
		{
			...
			{
				UE_LOGF(LogUdpMessaging, Warning, "FUdpReassembledMessage::Reassemble: SegmentOffset is invalid or would overflow Data. Message=%ls, Segment=%d, SegmentOffset=%d, SegmentSize=%d, DataSize=%d",
	...
	{
		uint16 SenderShortId = 0;
		uint16 RecipientShortId = 0;
		int32 MessageId = 0;
	};

	const FTraceMetadata& GetTraceMetadata() const
	{
		return TraceMetadata;
	}

	FTraceMetadata& GetTraceMetadata()
	{
		return TraceMetadata;
	}
#endif
...
private:
	...
	FUdpMessagingTime LastSegmentTime;
	...
	/** Holds the time at which any pending acknowledgments were last transmitted. */
	FUdpMessagingTime LastAckTime;
