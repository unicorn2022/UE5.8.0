...
void FUdpMessageTransport::AddStaticEndpoint(const FIPv4Endpoint& InEndpoint)
{
	...
	UE_LOGF(LogUdpMessaging, Verbose, "Added StaticEndpoint at %ls", *InEndpoint.ToString());
...
void FUdpMessageTransport::RemoveStaticEndpoint(const FIPv4Endpoint& InEndpoint)
{
	...
	UE_LOGF(LogUdpMessaging, Verbose, "Removed StaticEndpoint at %ls", *InEndpoint.ToString());
}
...
void DoJoinMulticastGroup(const TSharedRef<FInternetAddr>& MulticastAddr, const TSharedPtr<FInternetAddr>& IpAddr, FSocket* MulticastSocket)
{
	...
	if (bJoinedGroup)
	{
		UE_LOGF(LogUdpMessaging, Display, "Added local interface '%ls' to multicast group '%ls'",
			   *IpAddr->ToString(false), *MulticastAddr->ToString(true));
	}
	else
	{
		UE_LOGF(LogUdpMessaging, Warning, "Failed to join multicast group '%ls' on detected local interface '%ls'",
...
{
	const UUdpMessagingSettings* CurrentSettings = GetDefault<UUdpMessagingSettings>();
	...
	{
		UE_LOGF(LogUdpMessaging, Error, "StartTransport failed to create unicast socket on %ls", *UnicastEndpoint.ToString());
		return false;
	}
	else
	{
		int32 PortNo = UnicastSocket->GetPortNo();
		UE_LOGF(LogUdpMessaging, Display, "Unicast socket bound to '%ls:%d'.", *UnicastEndpoint.Address.ToString(), PortNo);
	}
#endif
	...
	{
		UE_LOGF(LogUdpMessaging, Warning, "StartTransport failed to create multicast socket on %ls, joined to %ls with TTL %i", *UnicastEndpoint.ToString(), *MulticastEndpoint.ToString(), MulticastTtl);
...
#endif
	{
		...
		{
			UE_LOGF(LogUdpMessaging, Display, "Using asynchronous task graph for message deserialization.");
	...
	if (MulticastSocket != nullptr)
	{
		...
		MulticastReceiver->SetMaxReadBufferSize(65535);
		MulticastReceiver->Start();
	}
	else
	{
		UE_LOGF(LogUdpMessaging, Warning, "Multicast socket was null so there is no multicast receiver.");
	}
	...
	UnicastReceiver = new FUdpSocketReceiver(UnicastSocket, ThreadWaitTime, TEXT("UdpMessageUnicastReceiver"));
	{
		...
		UnicastReceiver->SetMaxReadBufferSize(65535);
	...
	UE_LOGF(LogUdpMessaging, Verbose, "Started Transport");
	return true;
}
...
void FUdpMessageTransport::StopTransport()
{
	...
	TransportHandler = nullptr;
	ErrorFuture.Reset();
	UE_LOGF(LogUdpMessaging, Verbose, "Stopped Transport");
}
...
bool FUdpMessageTransport::TransportMessage(const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context, const TArray<FGuid>& Recipients)
{
	...
	if (UE_GET_LOG_VERBOSITY(LogUdpMessaging) >= ELogVerbosity::Verbose)
	{
		...
		UE_LOGF(LogUdpMessaging, Verbose, "TransportMessage %ls from %ls to %ls", *Context->GetMessageTypePathName().ToString(), *Context->GetSender().ToString(), *RecipientStr);
	}
#endif
...
void FUdpMessageTransport::HandleProcessorMessageReassembled(FUdpReassembledMessageSharedPtr ReassembledMessagePtr, const TSharedPtr<IMessageAttachment, ESPMode::ThreadSafe>& Attachment, const FGuid& NodeId)
{
	...
	auto DeserializeReassembledMessage = [DeserializedMessage = MoveTemp(DeserializedMessage), TransportHandler = this->TransportHandler, ReassembledMessagePtr = MoveTemp(ReassembledMessagePtr), NodeId](uint32 Slot)
	{
		...
		TRACE_UDPMESSAGING_MESSAGE_LIFECYCLE(
			...
			UE::UdpMessaging::Trace::EUdpMessageLifecycleEvent::BeginDeserialize
		);
		...
		else
		{
			...
			FString TypeInfoStr = TypeInfo.IsValid() ? TypeInfo->GetName() : TEXT("UNKNOWN TYPE");
			UE_LOGF(LogUdpMessaging, Verbose, "Failed to deserialize message of type %ls from %ls", *TypeInfoStr, *NodeId.ToString());
		}
		...
		TRACE_UDPMESSAGING_MESSAGE_LIFECYCLE(
			...
			ReassembledMessage.GetTraceMetadata().RecipientShortId,
			ReassembledMessage.GetTraceMetadata().MessageId,
			UE::UdpMessaging::Trace::EUdpMessageLifecycleEvent::Delivered
		);
	};
	...
	if (!ErrorFuture.IsValid()) {
		...
		ErrorFuture = Async(EAsyncExecution::TaskGraphMainThread, [WeakTransport = WeakTransportPtr]()
		{
			...
			if (TSharedPtr<FUdpMessageTransport, ESPMode::ThreadSafe> Transport = WeakTransport.Pin())
			{
				...
				{
					UE_LOGF(LogUdpMessaging, Error, "UDP messaging encountered an error. Please restart the service for proper functionality");
				}
			}
		});
	}
...
void FUdpMessageTransport::StartAutoRepairRoutine(uint32 MaxRetryAttempt)
{
	...
	UE_LOGF(LogUdpMessaging, Warning, "UDP messaging encountered an error. Auto repair routine started for reinitialization");
}
