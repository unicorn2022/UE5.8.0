...
namespace FUdpMessageSegment
{
	...
	struct FHeader
	{
		...
		static constexpr uint32 SerializedBytes = (
			sizeof(ProtocolVersion) +
			sizeof(RecipientNodeId) +
			sizeof(SenderNodeId) +
			sizeof(SegmentType)
		);
	...
	public:
		...
		static constexpr uint32 FixedSerializedBytes = (
			sizeof(MessageId) +
			sizeof(decltype(Segments)::SizeType)
		);
	...
	public:
		...
		static constexpr uint32 FixedSerializedBytes = (
			...
			sizeof(TotalSegments) +
			sizeof(Sequence) +
			sizeof(decltype(Data)::SizeType)
		);
