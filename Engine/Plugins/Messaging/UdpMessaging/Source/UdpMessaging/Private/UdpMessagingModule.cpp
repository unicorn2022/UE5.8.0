...
public:
	...
	virtual void StartupModule() override
	{
		...
		{
			UE_LOGF(LogUdpMessaging, Error, "The required module 'Networking' failed to load. Plug-in 'UDP Messaging' cannot be used.");
...
protected:
	...
	bool ParseEndpoint(const FString& InEndpointString, FIPv4Endpoint& OutEndpoint, bool bSupportHostname = true)
	{
		...
		if (bParsedAddr)
		{
			...
			{
				UE_LOGF(LogUdpMessaging, Warning, "Detected IPv4 address in the form of 169.254.x.x. This is a link assigned address and may prevent you from reach external endpoints. ");
			}
		}
		return bParsedAddr;
	}
	...
	TArray<FIPv4Endpoint> ParseStringArrayAddresses(const TArray<FString>& StringAddresses)
	{
		...
		for (const FString& EndpointAsString : StringAddresses)
		{
			...
			{
				UE_LOGF(LogUdpMessaging, Warning, "Invalid UDP Messaging Endpoint '%ls'", *EndpointAsString);
			}
		}
		return Endpoints;
	}
	...
	void InitializeBridge()
	{
		...
		if (!ParseEndpoint(Settings->UnicastEndpoint, UnicastEndpoint))
		{
			if (!Settings->UnicastEndpoint.IsEmpty())
			{
				UE_LOGF(LogUdpMessaging, Warning, "Invalid setting for UnicastEndpoint '%ls' - binding to all local network adapters instead", *Settings->UnicastEndpoint);
			}
		...
		if (!ParseEndpoint(Settings->MulticastEndpoint, MulticastEndpoint, false))
		{
			if (!Settings->MulticastEndpoint.IsEmpty())
			{
				UE_LOGF(LogUdpMessaging, Warning, "Invalid setting for MulticastEndpoint '%ls' - using default endpoint '%ls' instead", *Settings->MulticastEndpoint, *UDP_MESSAGING_DEFAULT_MULTICAST_ENDPOINT.ToText().ToString());
			}
		...
		UE_LOGF(LogUdpMessaging, Log, "Initializing bridge on interface %ls to multicast group %ls.", *UnicastEndpoint.ToString(), *MulticastEndpoint.ToText().ToString());
	...
	void InitializeTunnel()
	{
		...
		UE_LOGF(LogUdpMessaging, Log, "Initializing tunnel on interface %ls to multicast group %ls.", *UnicastEndpoint.ToString(), *MulticastEndpoint.ToText().ToString());
		...
		for (int32 EndpointIndex = 0; EndpointIndex < Settings->RemoteTunnelEndpoints.Num(); ++EndpointIndex)
		{
			...
			{
				UE_LOGF(LogUdpMessaging, Warning, "Invalid UDP RemoteTunnelEndpoint '%ls' - skipping", *Settings->RemoteTunnelEndpoints[EndpointIndex]);
			}
		}
	}
#endif
	...
	void ParseCommandLine(UUdpMessagingSettings* Settings, const TCHAR* CommandLine)
	{
		if (Settings && CommandLine)
		{
			...
			{
				UE_LOGF(LogUdpMessaging, Display, "Sharing known nodes with endpoints is enabled.");
			}
		}
	}
