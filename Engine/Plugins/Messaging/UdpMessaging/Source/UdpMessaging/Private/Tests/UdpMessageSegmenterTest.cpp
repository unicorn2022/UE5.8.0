...
void RunSegmentationTest(FAutomationTestBase& Test, uint32 MessageSize, uint16 SegmentPayloadSize, bool WithReliableMessages)
{
	check(MessageSize < (uint32)255 * SegmentPayloadSize);

	uint16 NumSegments = (MessageSize + SegmentPayloadSize - 1) / SegmentPayloadSize;
	Test.AddInfo(FString::Printf(TEXT("Segmenting message of size %i with %i payloads of size %i (Reliable=%d)..."),
		MessageSize, NumSegments, SegmentPayloadSize, WithReliableMessages));

	// create a large message to segment
	TSharedRef<FUdpSerializedMessage> Message = MakeShared<FUdpSerializedMessage>(
		EUdpMessageFormat::CborPlatformEndianness, UDP_MESSAGING_TRANSPORT_PROTOCOL_VERSION,
		WithReliableMessages ? EMessageFlags::Reliable : EMessageFlags::None);

	for (uint8 SegmentIndex = 0; SegmentIndex < NumSegments; ++SegmentIndex)
	{
		// write segment index into each byte of the current segment
		for (uint16 Offset = 0; (Offset < SegmentPayloadSize) && (Message->TotalSize() < MessageSize); ++Offset)
		{
	...
	const uint16 SegmentSize = IntCastChecked<uint16>((uint32)SegmentPayloadSize + FUdpMessageSegmenter::GetDataSegmentHeaderSize());
	FUdpMessageSegmenter Segmenter = FUdpMessageSegmenter(Message, SegmentSize);

	const bool bInitializeResult = Segmenter.TryInitialize();
	Test.TestTrue(TEXT("Attempting to initialize the segmenter succeeds"), bInitializeResult);
	...
	// do the segmentation
	{
		...
		while (Segmenter.GetNextPendingSegment(OutData, OutSegmentNumber))
		{
			...
			int32 ExpectedSegmentSize = (OutSegmentNumber == (NumSegments - 1))
				? MessageSize % SegmentPayloadSize
				: SegmentPayloadSize;

			if (ExpectedSegmentSize == 0)
			{
				ExpectedSegmentSize = SegmentPayloadSize;
			}
