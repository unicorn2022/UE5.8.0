...
public:
	...
	virtual void RegisterCommands() override
	{
		...
		UI_COMMAND(Command_TraceToggle, "Toggle Trace", "Toggles trace capture.", EUserInterfaceActionType::Button, FInputChord());
	}
	UE_ENABLE_OPTIMIZATION_SHIP

	TSharedPtr<FUICommandInfo> Command_TraceScreenshot;
	TSharedPtr<FUICommandInfo> Command_TraceBookmark;
	TSharedPtr<FUICommandInfo> Command_TraceToggle;
};
...
void SInsightsStatusBarWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	{
		constexpr double Delay = 1.0;
		OpenLiveSessionScheduledTime = InCurrentTime + Delay;
		bShouldTryOpenLiveSession = false;
	}

	if (OpenLiveSessionScheduledTime > 0.0 && OpenLiveSessionScheduledTime <= InCurrentTime)
	{
		OpenLiveSessionScheduledTime = 0.0;
...
TSharedRef<SWidget> SInsightsStatusBarWidget::MakeTraceMenu()
{
	...
	MenuBuilder.BeginSection("TraceData", LOCTEXT("TraceMenu_Section_Data", "Trace Data"));
	{
		...
		MenuBuilder.AddEditableText(LOCTEXT("TraceRegionNameLabel", "Region Name:"),
			...
			FOnTextCommitted::CreateLambda(
				[](const FText& NewRegionName, ETextCommit::Type)
				{
		...
		MenuBuilder.AddMenuEntry(
			...
			LOCTEXT("StatNamedEventsDesc", "Significantly increases the number of CPU profiler events by activating tracing for the named events system."),
			FSlateIcon(),
	...
	{
		MenuBuilder.AddMenuEntry(
			FInsightsStatusBarWidgetCommands::Get().Command_TraceToggle,
			NAME_None,
	...
	{
		MenuBuilder.AddMenuEntry(
			LOCTEXT("OpenLiveSessionOnTraceStart", "Open Live Session on Trace Start"),
			LOCTEXT("OpenLiveSessionOnTraceStartDesc", "When set, the live session will be automatically opened in Unreal Insights when tracing is started.\nThis option will only apply when tracing to the trace store."),
			FSlateIcon(),
	...
	MenuBuilder.BeginSection("Insights", LOCTEXT("TraceMenu_Section_Insights", "Insights"));
	{
		...
		(
			...
			LOCTEXT("RecentTracesTooltip", "Open the latest traces recorded to the trace store or as a file."),
...
void SInsightsStatusBarWidget::Channels_BuildMenu(FMenuBuilder& MenuBuilder)
{
	...
	MenuBuilder.BeginSection("Channels", LOCTEXT("Channels", "Channels"));
	{
		for (int32 Index = 0; Index < ChannelsInfo.Num(); ++Index)
		{
			...
			if (Data.bIsReadOnly)
			{
				ChannelTooltip = FText::Format(LOCTEXT("ChannelTooltipFmt", "{0}{1} This channel is readonly and can only be enabled from the command line."),
					ChannelTooltip, Data.Desc.EndsWith(".") ? FText() : FText::FromString("."));
			}
...
void SInsightsStatusBarWidget::InitCommandList()
{
	...
	CommandList->MapAction(FInsightsStatusBarWidgetCommands::Get().Command_TraceScreenshot,
		FExecuteAction::CreateSP(this, &SInsightsStatusBarWidget::TraceScreenshot_Execute),
		FCanExecuteAction::CreateSP(this, &SInsightsStatusBarWidget::TraceScreenshot_CanExecute));

	CommandList->MapAction(FInsightsStatusBarWidgetCommands::Get().Command_TraceBookmark,
		FExecuteAction::CreateSP(this, &SInsightsStatusBarWidget::TraceBookmark_Execute),
		FCanExecuteAction::CreateSP(this, &SInsightsStatusBarWidget::TraceBookmark_CanExecute));

	CommandList->MapAction(FInsightsStatusBarWidgetCommands::Get().Command_TraceToggle,
		FExecuteAction::CreateSP(this, &SInsightsStatusBarWidget::ToggleTrace_OnClicked),
		FCanExecuteAction::CreateSP(this, &SInsightsStatusBarWidget::ToggleTrace_CanExecute));

	// Ensures the actions are globally accessible.
...
#else
	return LOCTEXT("TraceStatusDisabled", "Trace system is disabled at compile time. Check the UE_TRACE_MINIMAL_ENABLED define.");
#endif
}
...
#else
	return LOCTEXT("StartTraceDisabledButtonTooltip", "Trace system is disabled at compile time. Check the UE_TRACE_MINIMAL_ENABLED define.");
#endif
}
...
void SInsightsStatusBarWidget::ToggleTrace_OnClicked()
{
	if (UE::Trace::IsTracing())
	{
		...
		if (bResult)
		{
			UE_LOGF(LogTraceUtilities, Log, "Stopped tracing to %ls.", *TraceDestinationStr);
		}
		else
		{
			LogMessage(LOCTEXT("TraceStopFailedMsg", "Failed to stop tracing!"));
		}
	}
	else
	{
		...
		if (bResult)
		{
			...
			UE_LOGF(LogTraceUtilities, Log, "Trace started. Trace is now active and saving to the following location (file or trace store): %ls", *TraceDestinationStr);
		}
		else
		{
			if (StatusBarTraceSettings.TraceDestination == ETraceDestination::TraceStore)
			{
				LogMessage(LOCTEXT("TraceFailedToStartTraceStoreMsg", "Trace failed to start. The Unreal Trace Server may not be running. Restart it from the Trace status bar menu > Unreal Trace Server > Start."));
			}
			else
			{
				LogMessage(LOCTEXT("TraceFailedToStartFileMsg", "Trace failed to start. Verify the profiling directory is accessible and writable."));
			}
		}
	}
}
...
void SInsightsStatusBarWidget::CreateChannelsInfo()
{
	...
	Algo::SortBy(ChannelsInfo,
		[](const FChannelData& Entry)
		{
			return Entry.Name;
		},
		[](const FString& Rhs, const FString& Lhs)
		{
...
void SInsightsStatusBarWidget::UpdateChannelsInfo()
{
	...
	UE::Trace::EnumerateChannels([](const UE::Trace::FChannelInfo& Info, void* User)
	{
		...
		int32 Index = Algo::BinarySearchBy(*Channels, Name,
			[](const FChannelData& Entry)
			{
				return Entry.Name;
			},
...
{
	return RegionIsActive() ?
		LOCTEXT("TraceRegionNameDesc_Active", "The name of the region to start or stop. Cannot be edited while a region is active.") :
		LOCTEXT("TraceRegionNameDesc_Inactive", "The name of the region to start or stop.");
}
...
void SInsightsStatusBarWidget::PopulateRecentTracesList()
{
	...
	Algo::SortBy(Traces, [](const TSharedPtr<FTraceFileInfo>& TraceInfo) { return *TraceInfo; });

	constexpr int32 MaxRecentTraces = 15;
	if (Traces.Num() > MaxRecentTraces)
	{
		Traces.RemoveAt(MaxRecentTraces, Traces.Num() - MaxRecentTraces);
	}
}
