...
void FUnrealInsightsLauncher::StartUnrealInsights(const FString& Path, const FString& Parameters, StartUnrealInsightsCallback Callback)
{
	...
	{
		UE_LOGF(LogTraceUtilities, Log, "Launched Unreal Insights executable: %ls %ls", *Path, *Parameters);
...
void FUnrealInsightsLauncher::CloseUnrealInsights()
{
	...
	{
		UE_LOGF(LogTraceUtilities, Log, "Could not find the Unreal Insights process handler");
	}
}
...
{
	UE_LOGF(LogTraceUtilities, Log, "Could not find the Unreal Insights executable: %ls. Attempting to build UnrealInsights.", *Path);
...
bool FUnrealInsightsLauncher::OpenActiveTraceFromStore(const FString& TraceHostAddress)
{
	...
	TUniquePtr<UE::Trace::FStoreClient> StoreClient = UE::Trace::FStoreClient::Create(*TraceHostAddress);
	...
	if (!SessionInfo)
	{
		UE_LOGF(LogTraceUtilities, Warning, "Unable to find session using guid. Possibly running with older server version. Falling back to last active session.");
