...
#define REGISTER_MODELING_ACTION_COMMAND_WITH_SHORTNAME(ToolCommandInfo, ToolName, ShortNameFText, ToolTip) \
	...
	REGISTER_MODELING_TOOL_COMMAND(BeginSkinWeightsPaintTool, "Edit Skin Weights", "Tune the per-vertex skin weights.");
	...
	UI_COMMAND(SelectionEnableSoftSelection, "Soft Selection", "Apply a falloff effect to nearby vertices when transforming the selected elements", EUserInterfaceActionType::ToggleButton, FInputChord(EKeys::B));
