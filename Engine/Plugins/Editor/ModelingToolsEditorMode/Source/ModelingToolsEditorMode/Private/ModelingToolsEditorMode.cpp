...
bool bUseExperimentalMeshAttributePaintTool = true;
static FAutoConsoleVariableRef CVarUseExperimentalMeshAttributePaintTool(
	TEXT("ModelingTools.UseExperimentalMeshAttributePaintTool"),
	bUseExperimentalMeshAttributePaintTool,
	TEXT("Determines whether to use the new experimental mesh attribute paint tool."));
}
...
void UModelingToolsEditorMode::Enter()
{
	...
	if (bUseExperimentalMeshAttributePaintTool)
	{
		RegisterTool(ToolManagerCommands.BeginMeshAttributePaintTool, TEXT("BeginMeshAttributePaintTool"), NewObject<UMeshAttributePaintToolV2Builder>());
	}
	else
	{
	...
	FUIAction ToggleSoftSelectionAction = FUIAction(
		FExecuteAction::CreateLambda([this]
		{
			bool bIsSoftSelectionEnabled = SelectionManager->IsSoftSelectionEnabled();
			SelectionManager->SetSoftSelectionEnabled(!bIsSoftSelectionEnabled);
		}),
		FCanExecuteAction(),
		FIsActionChecked::CreateLambda([this]()
		{
			return SelectionManager->IsSoftSelectionEnabled();
		}));
	...
	if (Extensions.Num() > 0)
	{
		...
		UE_LOGF(LogTemp, Log, "ModelingMode: Found %d Tool Extension Modules", Extensions.Num());
	...
	FWorldDelegates::PreLevelRemovedFromWorld.AddWeakLambda(this, [this](ULevel*, UWorld*) {
		...
		// Ensures selections are cleaned up to avoid crashing with editing level instances
		if (SelectionManager)
		{
 ...
 void UModelingToolsEditorMode::PopulateModelingModeViewportToolbar(const FName InMenuName, const TSharedPtr<const FUICommandList>& InCommandList)
{
	...
	TWeakObjectPtr<UModelingToolsEditorMode> WeakThis(this);
	...
	FToolMenuEntry Entry = FToolMenuEntry::InitSubMenu(
		...
		FNewToolMenuDelegate::CreateLambda([WeakThis, WeakCommandList = InCommandList.ToWeakPtr()](UToolMenu* Submenu) -> void
		{
			...
			if (const TSharedPtr<const FUICommandList>& CommandList = WeakCommandList.Pin())
			{
				...
				// Section for SoftSelection
				{
					...
					TSharedRef<SWidget> SoftSelectionRadiusWidget =
						SNew(SHorizontalBox)
						+ SHorizontalBox::Slot()
						.FillWidth(0.9f)
						[
							SNew(SSpinBox<double>)
								...
								.Value_Lambda(
									[WeakThis]() -> double 
									{
										...
										return WeakThis->SelectionManager->GetSoftSelectionRadius();
									}
								)
								.OnValueChanged_Lambda(
									[WeakThis](double InValue)
									{
										if (WeakThis.IsValid() && WeakThis->SelectionManager)
										{
											WeakThis->SelectionManager->SetSoftSelectionRadius(InValue);
										}
									}
								)
						]
						...
						SoftSelectionSection.AddEntry(FToolMenuEntry::InitWidget(
							"SoftSelectionRadius", SoftSelectionRadiusWidget, LOCTEXT("SoftSelectionRadiusLabel", "Radius")));
					...
					TSharedRef<SWidget> SoftSelectionDistanceModeWidget =
						SNew(SHorizontalBox)
						+ SHorizontalBox::Slot()
						.FillWidth(0.9f)
						[
							SNew(SComboButton)
							.OnGetMenuContent_Lambda( [WeakThis, GetSoftSelectionDistanceModeText]() -> TSharedRef<SWidget>
							{
								FMenuBuilder MenuBuilder(true, nullptr);

								PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
								auto AddModeEntry = [&MenuBuilder, WeakThis, GetSoftSelectionDistanceModeText](
									IGeometrySelector::ESoftSelectionDistanceMode InMode)
									{
										MenuBuilder.AddMenuEntry(
											GetSoftSelectionDistanceModeText(InMode),
											FText::GetEmpty(),
											FSlateIcon(),
											FUIAction(FExecuteAction::CreateLambda([WeakThis, InMode]()
												{
													if (WeakThis.IsValid() && WeakThis->SelectionManager)
													{
														WeakThis->SelectionManager->SetSoftSelectionDistanceMode(InMode);
													}
												})),
											NAME_None,
							...
							[
								SNew(STextBlock)
								.Text_Lambda([WeakThis, GetSoftSelectionDistanceModeText]() -> FText
								{
									PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
									if (!WeakThis.IsValid() || !WeakThis->SelectionManager)
									{
										return GetSoftSelectionDistanceModeText(
											IGeometrySelector::ESoftSelectionDistanceMode::Surface);
									}
									return GetSoftSelectionDistanceModeText(WeakThis->SelectionManager->GetSoftSelectionDistanceMode());
									PRAGMA_ENABLE_EXPERIMENTAL_WARNINGS
								})
							]
						]
					...
					SoftSelectionSection.AddEntry(FToolMenuEntry::InitWidget(
						"SoftSelectionDistanceMode", SoftSelectionDistanceModeWidget, LOCTEXT("SoftSelectionDistanceModeLabel", "Mode")));
				}
