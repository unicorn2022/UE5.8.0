...
FMeshSculptLayerPropertiesDetails::~FMeshSculptLayerPropertiesDetails()
{
	if (SculptProperties.IsValid())
	{
		SculptProperties->OnMeshLayersPanelRequestRebuild.Remove(SculptUpdatesHandle);
	}
}
...
void FMeshSculptLayerPropertiesDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	SculptProperties = SculptLayersProperties;
	...
	// verify invariant that this is always a fresh details object (if this were not the case, would need to unbind delegate handle above.)
	ensureMsgf(!SculptUpdatesHandle.IsValid(), TEXT("Unexpected re-use of detail customization object"));
	// ensure this stack is rebuilt when Mesh Layers are updated elsewhere in the editor
	SculptUpdatesHandle = SculptLayersProperties->OnMeshLayersPanelRequestRebuild.AddLambda([this]() {
