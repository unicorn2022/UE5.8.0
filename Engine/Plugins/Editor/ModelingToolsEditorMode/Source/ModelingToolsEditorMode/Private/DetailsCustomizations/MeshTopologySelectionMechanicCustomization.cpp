...
void FMeshTopologySelectionMechanicPropertiesDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (Properties->bDisplaySelectionDragToolsControls)
	{
		auto AddToggleButtonForSelectionDragToolEnum = [&ToolbarBuilder, Properties](TSharedPtr<IPropertyHandle> EnumPropertyHandle, EMeshTopologySelectionMechanicDragTool EnumValue, FSlateIcon Icon)
			{
				ToolbarBuilder.AddToolBarButton(FUIAction(
					FExecuteAction::CreateWeakLambda(Properties, [EnumPropertyHandle, EnumValue]()
						{
							EnumPropertyHandle->SetValue((uint8)EnumValue);
						}),
					FCanExecuteAction::CreateWeakLambda(Properties, [EnumPropertyHandle]() {
						return EnumPropertyHandle->IsEditable();
						}),
					FIsActionChecked::CreateWeakLambda(Properties, [EnumPropertyHandle, EnumValue]()
						{
							uint8 Value = 0;
							EnumPropertyHandle->GetValue(Value);
							return (Value == (uint8)EnumValue);
						})),
					NAME_None, // Extension hook
					TAttribute<FText>(EnumPropertyHandle->GetPropertyDisplayName()), // Label
					TAttribute<FText>(EnumPropertyHandle->GetToolTipText()), // Tooltip
					TAttribute<FSlateIcon>(Icon),
					EUserInterfaceActionType::ToggleButton);
			};

		TSharedPtr<IPropertyHandle> SelectionDragToolHandle = DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UMeshTopologySelectionMechanicProperties, SelectionDragTool),
			UMeshTopologySelectionMechanicProperties::StaticClass());
		SelectionDragToolHandle->MarkHiddenByCustomization();

		ToolbarBuilder.BeginSection("SelectionDragTools");
		ToolbarBuilder.BeginBlockGroup();

		AddToggleButtonForSelectionDragToolEnum(SelectionDragToolHandle, EMeshTopologySelectionMechanicDragTool::Marquee, FSlateIcon(FAppStyle::GetAppStyleSetName(), "LevelEditor.StrictBoxSelect"));
		AddToggleButtonForSelectionDragToolEnum(SelectionDragToolHandle, EMeshTopologySelectionMechanicDragTool::Lasso, FSlateIcon(FModelingToolsEditorModeStyle::Get()->GetStyleSetName(), "PolyEd.SelectByLasso"));
