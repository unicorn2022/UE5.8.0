...
namespace UE::Landscape
{
	...
	public:
		...
		BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
			...
			SHADER_PARAMETER(FMatrix44f, InHeightmapToPatchUV)
			// InHeightmapToPatchUV matrix with the source texture transform (world space or relative) applied
			SHADER_PARAMETER(FMatrix44f, InHeightmapToPatchUVTransform)
			...
			// Height source texture color channel
			SHADER_PARAMETER(uint32, InTextureChannel)
			// Alpha source mode, override texture (White system texture if undefined), alpha texture color channel, and texture transform
			SHADER_PARAMETER(uint32, InAlphaMode)
			SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D<float4>, InHeightPatchAlphaTexture)
			SHADER_PARAMETER(uint32, InAlphaTextureChannel)
			// InHeightmapToPatchUV matrix with the alpha source texture transform (world space or relative) applied
			SHADER_PARAMETER(FMatrix44f, InHeightmapAlphaToPatchUVTransform)
			// Assigned edit layer's heightmap alpha range [-1, 1]
			SHADER_PARAMETER(float, InEditLayerHeightmapAlpha)
	...
	public:
		...
		BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
			...
			SHADER_PARAMETER(FMatrix44f, InWeightmapToPatchUV)
			// InWeightmapToPatchUV matrix with the source texture transform (world space or relative) applied
			SHADER_PARAMETER(FMatrix44f, InWeightmapToPatchUVTransform)
			// Amount of the patch edge to not apply in UV space. Generally set to 0.5/Dimensions to avoid applying
			// the edge half-pixels.
			SHADER_PARAMETER(FVector2f, InEdgeUVDeadBorder)
			// In world units, the size of the margin across which the alpha falls from 1 to 0
			...
			// Some combination of the flags (see constants above).
			SHADER_PARAMETER(uint32, InFlags)
			// Weight source texture color channel
			SHADER_PARAMETER(uint32, InTextureChannel)
			// Alpha source mode, override texture (White system texture if undefined), alpha texture color channel, and texture transform
			SHADER_PARAMETER(uint32, InAlphaMode)
			SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D<float4>, InWeightPatchAlphaTexture)
			SHADER_PARAMETER(uint32, InAlphaTextureChannel)
			// InWeightmapToPatchUV matrix with the alpha source texture transform (world space or relative) applied
			SHADER_PARAMETER(FMatrix44f, InWeightmapAlphaToPatchUVTransform)
			// Assigned edit layer's weightmap alpha range [0, 1]
			SHADER_PARAMETER(float, InEditLayerWeightmapAlpha)
