...
void ULandscapeTextureBackedRenderTargetBase::PostLoad()
{
	...
	if (!bIsTemporarilyNonTransactional)
	{
	...
	if (IsValid(RenderTarget) && !bIsTemporarilyNonTransactional)
	{
...
// This function is to support a workaround for UE-225445, where we need to temporarily mark internal objects non - transactional
//  It does so in a way that avoids them being re-marked transactional in PostLoad.
//  See comment in ULandscapeTexturePatch::GetComponentInstanceData() for details
void ULandscapeTextureBackedRenderTargetBase::SetTemporarilyNonTransactional(bool bInIsTemporarilyNonTransactional)
{
	if (bInIsTemporarilyNonTransactional)
	{
		ClearFlags(RF_Transactional);
		if (IsValid(InternalTexture))
		{
			InternalTexture->ClearFlags(RF_Transactional);
		}
		if (IsValid(RenderTarget))
		{
			RenderTarget->ClearFlags(RF_Transactional);
		}
	}
	else
	{
	...
	bIsTemporarilyNonTransactional = bInIsTemporarilyNonTransactional;
}
...
void ULandscapeTextureBackedRenderTargetBase::Initialize()
{
	...
	{
		InternalTexture = CreateInternalTexture();
	}
...
void ULandscapeWeightTextureBackedRenderTarget::CopyToInternalTexture()
{
	...
	{
		UE_LOGF(LogLandscapePatch, Error, "Couldn't copy render target to internal texture: %ls", *ErrorMessage.ToString());
	}
#endif // WITH_EDITOR
}
...
void ULandscapeWeightTextureBackedRenderTarget::CopyBackFromInternalTexture()
{
	...
	if (!ensure(IsValid(RenderTarget)))
	{
		...
		{
			check(!bIsTemporarilyNonTransactional);
			RenderTarget->SetFlags(RF_Transactional);
		}
...
#if WITH_EDITOR
UTexture2D* ULandscapeTextureBackedRenderTargetBase::CreateInternalTexture()
{
