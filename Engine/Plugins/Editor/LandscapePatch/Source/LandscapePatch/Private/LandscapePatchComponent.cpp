...
void ULandscapePatchComponent::SetLandscape(ALandscape* NewLandscape)
{
	...
	{
		UE_LOGF(LogLandscapePatch, Warning, "Unable to bind to given landscape (does it have edit layers enabled?).");
	}
#endif // WITH_EDITOR
}
...
void ULandscapePatchComponent::RequestLandscapeUpdate(bool bInUserTriggeredUpdate /* = false */)
{
	...
	else if (EditLayerGuid.IsValid())
	{
		...
		{
			UE_LOGF(LogLandscapePatch, Warning, "Could not find an edit layer with the given guid for the patch. "
				"(Package: %ls",
	...
	else
	{
		...
		{
			UE_LOGF(LogLandscapePatch, Warning, "Patch is not bound to an edit layer. Set the landscape and edit layer guid or run LandscapePatch.FixPatchBindings."
				"on the patch. (Package: %ls)",
				*GetNameSafe(GetPackage()));
...
bool ULandscapePatchComponent::BindToEditLayer(const FGuid& Guid)
{
	...
	if (Landscape.Get())
	{
		...
		{
			UE_LOGF(LogLandscapePatch, Warning, "Mismatch between landscape and layer Guid in patch.");
...
void ULandscapePatchComponent::FixBindings()
{
	...
	if (EditLayerGuid.IsValid())
	{
		...
		UE_LOGF(LogLandscapePatch, Warning, "Could not find patch edit layer with guid: %ls", *EditLayerGuid.ToString());
		ResetEditLayer();
	}
	...
	{
		UE_LOGF(LogLandscapePatch, Warning, "Unable to find landscape with edit layers enabled.");
	}
}
...
bool ULandscapePatchComponent::AssignToLandscape(ALandscape* InLandscape, const FName& InEditLayerName)
{
	bool bSuccess = false;

#if WITH_EDITOR
	Modify(false);
	bSuccess = BindToLandscape(InLandscape);
	if (bSuccess && (InLandscape != nullptr))
	{
		// Passing an empty name is the equivalent of un-assigning the edit layer, it's considered valid : 
		if (InEditLayerName.IsNone())
		{
			bSuccess = BindToEditLayer(FGuid());
		}
		else
		{
			// Validate that the edit layer name corresponds to a valid patch edit layer and spew errors if not : 
			const ULandscapeEditLayerBase* LocalEditLayer = InLandscape->GetEditLayerConst(InEditLayerName);
			if (LocalEditLayer != nullptr)
			{
				const ULandscapePatchEditLayer* PatchEditLayer = Cast<const ULandscapePatchEditLayer>(LocalEditLayer);
				if (PatchEditLayer != nullptr)
				{
					// All good, we can try to attach to that edit layer : 
					bSuccess = BindToEditLayer(LocalEditLayer->GetGuid());
				}
				else
				{
					UE_LOGF(LogLandscapePatch, Warning, "Edit layer with the given name (%ls) on landscape %ls is not a patch edit layer (%ls). "
						"(Package: %ls",
						*InEditLayerName.ToString(),
						*InLandscape->GetActorLabel(),
						*LocalEditLayer->GetClass()->GetName(),
						*GetNameSafe(GetPackage()));

					// Un-assign the edit layer so that the function call effectively changes the edit layer according to the error :
					BindToEditLayer(FGuid());
					bSuccess = false;
				}
			}
			else
			{
				UE_LOGF(LogLandscapePatch, Warning, "Could not find an edit layer with the given name (%ls) on landscape %ls for the patch. "
					"(Package: %ls",
					*InEditLayerName.ToString(),
					*InLandscape->GetActorLabel(),
					*GetNameSafe(GetPackage()));
