...
void ADEPRECATED_LandscapePatchManager::SetTargetLandscape(ALandscape* InTargetLandscape)
{
	...
	if (OwningLandscape != InTargetLandscape && !bDead)
	{
		...
		{
			UE_LOGF(LogLandscapePatch, Warning, "Landscape target for patch manager is a template. Unable to attach manager.");
			SetOwningLandscape(nullptr);
		}
	}
#endif
...
void ADEPRECATED_LandscapePatchManager::MigrateToPrioritySystemAndDeleteInternal(bool bAllowUI)
{
	...
	if (OwningLandscape && LandscapeInfo && !bDead && IsValid(this) && !PatchComponents.IsEmpty())
	{
		...
		UE_LOGF(LogLandscapePatch, Warning, "ALandscapePatchManager: %d landscape patches have been migrated from the legacy patch manager \"%ls\" to be bound directly to a ULandscapePatchEditLayer",
			PatchListCopy.Num(), *GetActorLabel(false));
		if (PatchErrors > 0)
		{
			UE_LOGF(LogLandscapePatch, Error, "ALandscapePatchManager: %d landscape patches failed to migrate successfully.  They may be restorable by running the command LandscapePatch.FixPatchBindings", PatchErrors);
		}
