		...
		for (const TPair<FGuid, TUniquePtr<FTrail>>& GuidTrailPair : OwningHierarchy->GetAllTrails())
		{
			if (OwningHierarchy->GetVisibilityManager().IsTrailVisible(GuidTrailPair.Key, GuidTrailPair.Value.Get(), CachedOptions->bShowSelectedTrails))
			{
				...
				if (Marks.Num() > 0)
				{
					...
					const FLinearColor CurrentFrameMarkColor = UMotionTrailToolOptions::GetTrailOptions()->CurrentFrameMarkColor;
					for (int32 Idx = 0; Idx < Marks.Num(); Idx++)
					{
						const FFrameNumber Frame = CurrentFramesInfo->CurrentFrameTimes.CalculateFrame(Idx);
						...
						if (ColorState.SequencerTime != Frame)
						{
							LineItem.SetColor(Color);
						}
						else
						{
							LineItem.SetColor(CurrentFrameMarkColor);
						}
...
bool FTrailHierarchy::IsAnythingSelected(FVector& OutVectorPosition, FQuat& OutRotation) const
{
	...
	{
		FVector Location;
		bool bHandled = GuidTrailPair.Value->IsAnythingSelected(Location,LocalRotation);
		if (bHandled)
		{
	...
	if (NumSelected > 0)
	{
		...
		OutRotation = NewRotation.GetValue();
...
bool FTrailHierarchy::IsAnythingSelected(TArray<FVector>& OutVectorPositions, bool bAllPositions) const
{
	...
	for (const TPair<FGuid, TUniquePtr<FTrail>>& GuidTrailPair : GetAllTrails())
	{
		if (bAllPositions == false)
		{
			FVector Location;
			FQuat Rotation;
			bool bHandled = GuidTrailPair.Value->IsAnythingSelected(Location,Rotation);
			if (bHandled)
			{
