...
public:
	...
	virtual void ClearOffsetTransform() override;
	virtual void SetOffsetMode() override;
	virtual bool GetOffsetMode() const override;

	virtual FTransform GetParentSpaceTransform() const override;
	...
	virtual void ActorChanged(AActor* InActor) override;
...
protected:
	...
	void CalculateUpdatedOffset();
protected:
	//some interactive states/modes
	bool bIsSelected;
	bool bIsOffsetMode;
	FVector SelectedPos;
	FQuat SelectedRotation;
	bool bIsTracking; 
...
protected: 
	...
	//transforms calculated from evaluating control rigs and actors
	TSharedPtr<UE::AIE::FArrayOfTransforms> CalculatedArrayOfTransforms;
	//calculated transforms plus any offset + parent
	TSharedPtr<UE::AIE::FArrayOfTransforms> ArrayOfTransforms;
	//parent space transforms (optional)
