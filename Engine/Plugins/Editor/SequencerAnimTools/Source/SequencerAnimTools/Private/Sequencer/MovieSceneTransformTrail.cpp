	...
	else
	{
		...
		for (int32 Count = 0; Count < IndicesToCalcluate.Num(); ++Count)
		{
			...
			if (CalculatedIndices.Contains(Index) == false)
			{
				...
				ArrayOfTransforms->Transforms[Index] = OffsetTransform * CalculatedArrayOfTransforms->Transforms[Index];
			}
		}
	}
...
{
	if (bTrailIsEvaluating == false || bIsOffsetMode == true) // if evaluating we now just draw betweeen the keys
	{
...
void FMovieSceneTransformTrail::Render(const FGuid& Guid, const FSceneView* View, FPrimitiveDrawInterface* PDI, bool bTrailIsEvaluating)
{
	...
	else if (bIsOffsetMode && bTrailIsEvaluating)
	{
		RenderEvaluating(Guid, View, PDI);
	}
	//render key tool
...
bool FMovieSceneTransformTrail::HandleClick(const FGuid& Guid, FEditorViewportClient* InViewportClient, HHitProxy* InHitProxy, FInputClick Click)
{
	...
	if (HNewMotionTrailProxy* HitProxy = HitProxyCast<HNewMotionTrailProxy>(InHitProxy))
	{
		if (HitProxy->Guid == Guid)
		{
			...
			SelectedRotation = FQuat::Identity;
			return true;
		}
	}
...
bool FMovieSceneTransformTrail::IsAnythingSelected(FVector& OutVectorPosition, FQuat& OutRotation) const
{
	if (bIsSelected)
	{
		OutVectorPosition = SelectedPos;
		OutRotation = SelectedRotation;
		return true;
	}
...
bool FMovieSceneTransformTrail::GetOffsetMode() const
{
	return bIsOffsetMode;
}
...
void FMovieSceneTransformTrail::CalculateUpdatedOffset()
{
	if (ArrayOfTransforms->Num() == CalculatedArrayOfTransforms->Num())
	{
		ClearCachedData();  
		CacheState = ETrailCacheState::UpToDate;
		for (int32 Index = 0; Index < ArrayOfTransforms->Num(); ++Index)
		{
			FVector InOffsetSpace = CalculatedArrayOfTransforms->Transforms[Index].GetRotation().RotateVector(OffsetTransform.GetLocation());
			InOffsetSpace += CalculatedArrayOfTransforms->Transforms[Index].GetLocation();
			FTransform NewOffset(CalculatedArrayOfTransforms->Transforms[Index].GetRotation().Rotator(),
				InOffsetSpace, CalculatedArrayOfTransforms->Transforms[Index].GetScale3D());
			ArrayOfTransforms->Transforms[Index] = OffsetTransform * CalculatedArrayOfTransforms->Transforms[Index];
		}
		if (ParentSpace.bIsValid && ParentArrayOfTransforms->Num() == ArrayOfTransforms->Num())
		{
			TSet<int32> CalculatedIndices;
			for (int32 Index = 0; Index < ArrayOfTransforms->Num(); ++Index)
			{
...
bool FMovieSceneTransformTrail::ApplyDelta(const FVector& Pos, const FRotator& Rot, const FVector& WidgetLocation, bool bApplyToOffset)
{
	if (bApplyToOffset || bIsOffsetMode)
	{
		...
		{
			FQuat Rotation(Rot);
			FVector UnRotatedVector = Rotation.UnrotateVector(Pos);
			OffsetTransform.SetLocation(OffsetTransform.GetLocation() + UnRotatedVector);
			if (bIsOffsetMode == false)
			{
...
void FMovieSceneComponentTransformTrail::UpdateNeedsEvaluation(const FGuid& InTrailGuid, FSequencerTrailHierarchy* SequencerHierarchy)
{
	if (Component.IsValid())
	{
		...
		SequencerHierarchy->EvaluateActor(InTrailGuid, Actors, CalculatedArrayOfTransforms, ParentArrayOfTransforms);
		ArrayOfTransforms->Transforms.SetNum(CalculatedArrayOfTransforms->Transforms.Num());
...
bool FMovieSceneSocketTransformTrail::IsAnythingSelected(FVector& OutVectorPosition, FQuat& OutRotation) const
{
	if (bIsSelected)
	{
		OutVectorPosition = SelectedPos;
...
void FMovieSceneControlRigTransformTrail::UpdateNeedsEvaluation(const FGuid& InTrailGuid, FSequencerTrailHierarchy* SequencerHierarchy)
{
	...
	SequencerHierarchy->EvaluateControlRig(InTrailGuid, ControlRig, ControlName, CalculatedArrayOfTransforms);
	ArrayOfTransforms->Transforms.SetNum(CalculatedArrayOfTransforms->Transforms.Num());
}
...
bool FMovieSceneControlRigTransformTrail::ApplyDelta(const FVector& Pos, const FRotator& Rot, const FVector& WidgetLocation, bool bApplyToOffset)
{
	...
	if (IsAnythingSelected())
	{
		...
		auto EvalControlRig = [&Context,&RootToLocalTransform, Pos, TickResolution, ControlRig,Player, ControlElement, this](FTrailKeyInfo* KeyInfo)
		{
			if (KeyInfo)
			{
				//the keys are in warped time which is what we want to set the control rig values with, but we want to evaluate in unwarped time
				FFrameTime GlobalTime(KeyInfo->FrameNumber);
				GlobalTime = RootToLocalTransform.TransformTime(GlobalTime);
				Context.LocalTime = TickResolution.AsSeconds(GlobalTime);
				Context.KeyMask = (uint32)EControlRigContextChannelToKey::Translation;
				GlobalTime = RootToLocalTransform.Inverse().TryTransformTime(GlobalTime).Get(GlobalTime);//back to unwarped
