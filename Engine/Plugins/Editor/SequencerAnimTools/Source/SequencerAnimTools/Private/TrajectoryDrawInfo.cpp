...
bool FFrameCalculator::CalculateIndices()
{
	...
	if (CurrentBucket == -1 && MustHaveIndices.Num() > 0)
	{
		IndicesToCalculate.Append(MustHaveIndices);
		++CurrentBucket;
	}
...
void FCurrentFramesInfo::SetViewRange(const TRange<FFrameNumber>& InViewRange, bool bInViewRangeIsEvalRange)
{
	ViewRange = InViewRange;
	bViewRangeIsEvalRange = bInViewRangeIsEvalRange;
}
