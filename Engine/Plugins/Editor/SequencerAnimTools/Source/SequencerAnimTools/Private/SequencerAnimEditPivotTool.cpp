...
namespace UE::AIE::PivotTool
{

	TArray<FName> CurrentControlSelection(UControlRig* InControlRig) 
	{
		TArray<FName> Names;
		if (InControlRig)
		{
			// use this version after 5.7.1 is done, expose FControlRigEditMode::GetRigElementsForSettingTransforms
			//TArray<FRigElementKey> SelectedRigElements = FControlRigEditMode::GetRigElementsForSettingTransforms(InControlRig, true /*bSelectionOnly*/, false /*bIncludeChannels*/);
			...
			*/
			...
			{
				if (FRigControlElement* Element = InControlRig->FindControl(Name))
				{
					if (Element->IsAnimationChannel() == false)
					{
						Names.Add(Name);
					}
				}
	...
	USceneComponent* GetBoundComponent(const UControlRig* InControlRig)
	{
		const TSharedPtr<IControlRigObjectBinding>& ObjectBinding = InControlRig ? InControlRig->GetObjectBinding() : nullptr;
		return ObjectBinding.IsValid() ? Cast<USceneComponent>(ObjectBinding->GetBoundObject()) : nullptr;
	}
	
	FTransform GetBoundComponentTransform(const UControlRig* InControlRig)
	{
		USceneComponent* HostingComponent = GetBoundComponent(InControlRig);
		return HostingComponent ? HostingComponent->GetComponentTransform() : FTransform::Identity;
	}
	
	//includes global + hosting transform
	FTransform GetControlWorldSpaceTransform(UControlRig* ControlRig, const FName& Control)
	{
		FTransform WorldTransform = FTransform::Identity;
		if (ControlRig && ControlRig->GetHierarchy())
		{
			if (FRigTransformElement* ControlElement = ControlRig->FindControl(Control))
			{
				const FTransform ParentTransform = GetBoundComponentTransform(ControlRig);
				//Use GetTransform() not GetControlGlobalTransform
				WorldTransform = ControlRig->GetHierarchy()->GetTransform(ControlElement, ERigTransformType::CurrentGlobal);
				WorldTransform = WorldTransform * ParentTransform;
			}
		}
...
static void GetControlRigsAndSequencer(TArray<TWeakObjectPtr<UControlRig>>& ControlRigs, TWeakPtr<ISequencer>& SequencerPtr, ULevelSequence** LevelSequence)
{
	*LevelSequence = ULevelSequenceEditorBlueprintLibrary::GetFocusedLevelSequence();
	...
	if (*LevelSequence && SequencerLevelSequence)
	{
		...
		{
			//use editt mode (which must be active for pivot tool) to get 
			TArray<UControlRig*> TempControlRigs;
			FControlRigEditMode* ControlRigEditMode = static_cast<FControlRigEditMode*>(GLevelEditorModeTools().GetActiveMode(FControlRigEditMode::ModeName));
			if (ControlRigEditMode)
			{
				TempControlRigs = ControlRigEditMode->GetControlRigsArray(false /*bIsVisible*/); //get all may want to pivot non-visible
			}
...
TOptional<FTransform> FControlRigMappings::GetWorldTransform(const FName& Name) const
{
	...
	if (ControlRig.IsValid() && ControlRig->GetHierarchy())
	{
		if (FRigTransformElement* ControlElement = ControlRig->FindControl(Name))
		{
			const FTransform* LocalTransform = PivotTransforms.Find(Name);
			if (LocalTransform)
			{
				const FTransform ToWorldTransform = UE::AIE::PivotTool::GetBoundComponentTransform(ControlRig.Get());
				//Use GetTransform() not GetControlGlobalTransform
				FTransform CurrentTransform = ControlRig->GetHierarchy()->GetTransform(ControlElement, ERigTransformType::CurrentGlobal);
...
void FControlRigMappings::SetFromWorldTransform(const FName& Name, const FTransform& WorldTransform)
{
	if (ControlRig.IsValid() && ControlRig->GetHierarchy())
	{
		if (FRigTransformElement* ControlElement = ControlRig->FindControl(Name))
		{
			FTransform ToWorldTransform = UE::AIE::PivotTool::GetBoundComponentTransform(ControlRig.Get());
			FTransform GlobalTransform = ControlRig->GetHierarchy()->GetTransform(ControlElement, ERigTransformType::CurrentGlobal) * ToWorldTransform;
...
void USequencerPivotTool::SaveLastSelected()
{
	...
	for (TWeakObjectPtr<UControlRig>& ControlRig : ControlRigs)
	{
		if (ControlRig.IsValid())
		{
			TArray<FName> SelectedControls = UE::AIE::PivotTool::CurrentControlSelection(ControlRig.Get());
			for (const FName& Name : SelectedControls)
			{
	...
	for (TWeakObjectPtr<UControlRig>& ControlRig : ControlRigs)
	{
		...
		for (const FName& Name : SelectedControls)
		{
			...
			if (bHasSavedPivot == false)
			{
				...
				GizmoTransform  = UE::AIE::PivotTool::GetControlWorldSpaceTransform(RealControlRig, Name);
...
{
	using namespace UE::AIE::PivotTool;
	
	Keyframer.Enable(bInPivotMode);
	InteractionDependencies.Reset();
	TransformContext = FControlRigInteractionTransformContext();
	...
	{
		const bool bHandleTransaction = GEditor && !GIsTransacting && GEditor->CanTransact() && !GEditor->IsTransactionActive();
		if (bHandleTransaction)
		{
			Transaction = MakeUnique<FScopedTransaction>(LOCTEXT("USequencerPivotToolTransaction", "Transform"));
		}
		
		for (TWeakObjectPtr<UControlRig>& WeakControlRig : ControlRigs)
		{
			PrepareRigForManipulation(WeakControlRig.Get());
		}

		for (TWeakObjectPtr<AActor>& Actor : Actors)
		{
			PrepareActorForManipulation(Actor.Get());
		}
	}
...
void USequencerPivotTool::SetControlTransform(const FControlRigSelectionDuringDrag& InControlDrag)
{
	UControlRig* ControlRig = InControlDrag.ControlRig;
	if (!ControlRig || InControlDrag.ControlName == NAME_None)
	{
		return;
	}
	
	const bool bFixEulerFlips = (!ControlRig->IsAdditive() || ControlRig->IsA<UFKControlRig>()) && TransformContext.bRotation;
	UE::ControlRigEditMode::FInteractionDependencyCache* Dependencies = InteractionDependencies.Find(ControlRig);
	auto EvaluateRigIfNeeded = [Dependencies, &InControlDrag]()
	{
		...
		if (InControlDrag.ControlRig->IsAdditive())
		{
			InControlDrag.ControlRig->Evaluate_AnyThread();
		}
		else if (Dependencies)
		{
			if (Dependencies->HasDownwardDependencies({InControlDrag.ControlName, ERigElementType::Control}) ||
	...
	const uint32 ControlHash = UTransformableControlHandle::ComputeHash(ControlRig, InControlDrag.ControlName);
	const bool bNeedsConstraintPostProcess = ConstraintsCache.HasAnyActiveConstraint(ControlHash, TargetWorld);
	static const TArray< TWeakObjectPtr<UTickableConstraint> > EmptyConstraints;
	const TArray< TWeakObjectPtr<UTickableConstraint> >& Constraints =
		bNeedsConstraintPostProcess ? ConstraintsCache.Get(ControlHash, TargetWorld) : EmptyConstraints;

	FRigControlModifiedContext Context;
	Context.bConstraintUpdate = true;
	Context.SetKey = bDeferAutokey ? EControlRigSetKey::Never : EControlRigSetKey::DoNotCare;

	USceneComponent* Component = UE::AIE::PivotTool::GetBoundComponent(ControlRig);
	const FTransform& ComponentTransform = Component ? Component->GetComponentTransform() : FTransform::Identity;
	
	const FTransform Global = InControlDrag.CurrentTransform.GetRelativeTransform(ComponentTransform);
	
	constexpr bool bNotify = true, bSetupUndo = true, bPrintPythonCommands = false;
	ControlRig->SetControlGlobalTransform(
		InControlDrag.ControlName, Global, bNotify, Context, bSetupUndo, bPrintPythonCommands, bFixEulerFlips);
	EvaluateRigIfNeeded();
				
	UE::AnimationEditMode::FControlKeyframeData KeyframeData;
	KeyframeData.LocalTransform = ControlRig->GetControlLocalTransform(InControlDrag.ControlName);
	
	if (bNeedsConstraintPostProcess)
	{
		// switch to constraint space
		const TOptional<FTransform> RelativeTransform =
			UE::TransformConstraintUtil::GetConstraintsRelativeTransform(Constraints, KeyframeData.LocalTransform, InControlDrag.CurrentTransform);
		if (RelativeTransform)
		{
			KeyframeData.LocalTransform = *RelativeTransform;
			KeyframeData.bConstraintSpace = true;
		}

		FRigControlModifiedContext LocalContext = Context;
		LocalContext.bConstraintUpdate = false;

		ControlRig->SetControlLocalTransform(InControlDrag.ControlName, KeyframeData.LocalTransform, bNotify, LocalContext, bSetupUndo, bFixEulerFlips);
		EvaluateRigIfNeeded();
	}

	if (bNeedsConstraintPostProcess || ConstraintsCache.HasAnyActiveConstraint(Component, TargetWorld))
	{
		TGuardValue<bool> CompensateGuard(FMovieSceneConstraintChannelHelper::bDoNotCompensate, true);
		const FConstraintsManagerController& Controller = FConstraintsManagerController::Get(TargetWorld);
...
void USequencerPivotTool::GizmoTransformChanged(UTransformProxy* Proxy, FTransform Transform)
{
	...
	TransformContext.bRotation = (Diff.GetRotation().IsIdentity(1e-4f) == false);
	TransformContext.bTranslation = (Diff.GetTranslation().IsNearlyZero() == false);
	
	if (TransformContext.bRotation || TransformContext.bTranslation)
	{
		int32 Index = 0;
		for (FControlRigSelectionDuringDrag& ControlDrag : ControlRigDrags)
		{
			...
			else if (!bShiftPressedWhenStarted || Index != (ControlRigDrags.Num() - 1))
			{
				if (TransformContext.bRotation)
				{
					...
					SetControlTransform(ControlDrag);
				}
				else if (TransformContext.bTranslation)
				{
...
{
	const bool bFinalizeTransaction = Transaction && bManipulatorMadeChange;
	...
	// autokey on release
	{
		for (const TSharedPtr<FControlRigInteractionScope>& InteractionScope: InteractionScopes)
		{
			Keyframer.Apply(*InteractionScope, TransformContext);
		}
		Keyframer.Finalize(TargetWorld);
		Keyframer.Reset();
	}
	
	if (Transaction)
	{
		if (!bFinalizeTransaction)
		{
			Transaction->Cancel();
		}
		Transaction.Reset();
...
void USequencerPivotTool::Render(IToolsContextRenderAPI* RenderAPI)
{
	...
	if (bPickingPivotLocation == false )
	{
		...
		{
			const bool bIsPerspective = (RenderAPI->GetSceneView()->ViewMatrices.GetViewToClip().M[3][3] < 1.0f) ? true : false;
			if (bIsPerspective)
			{
				const float ZoomFactor = FMath::Min<float>(RenderAPI->GetSceneView()->ViewMatrices.GetViewToClip().M[0][0], RenderAPI->GetSceneView()->ViewMatrices.GetViewToClip().M[1][1]);
...
void USequencerPivotTool::PrepareRigForManipulation(UControlRig* InControlRig)
{
	using namespace UE::AIE::PivotTool;
	...
	const TArray<FName> SelectedControls = CurrentControlSelection(InControlRig);
	if (SelectedControls.IsEmpty())
	{
		return;
	}

	// store controls to transform
	TArray<FRigControlElement*> Controls;
	Controls.Reserve(SelectedControls.Num());
	for (const FName& Name : SelectedControls)
	{
		if (FRigControlElement* ControlElement = InControlRig->FindControl(Name))
		{
			...
			{
				if (InControlRig->IsControlSelected(ParentControlElement->GetFName()) && (
	...
	// keep last selected control if bShiftPressedWhenStarted
	FRigControlElement* LastControl = nullptr;
	if (bShiftPressedWhenStarted && !Controls.IsEmpty())
	{
		LastControl = Controls.Last();
		Controls.RemoveAt(Controls.Num() - 1);
	}

	// sort control from depth to avoid setting a child before it's parent
	Algo::Sort(Controls, [Hierarchy](const FRigControlElement* Control0, const FRigControlElement* Control1)
	{
		constexpr bool bRecursive = true;	
		const FRigBaseElementParentArray& Parents = Hierarchy->GetParents(Control0, bRecursive);
		const FRigBaseElementParentArray& OtherParents = Hierarchy->GetParents(Control1, bRecursive);

		int32 Index = Parents.Num(), OtherIndex = OtherParents.Num();
		auto HasCommonAncestor = [&Index, &Parents, &OtherIndex, &OtherParents, Control1, Control0]()
		{
			if (!Parents.IsEmpty() && !OtherParents.IsEmpty())
			{
				FRigBaseElement* CommonAncestor = nullptr;
				while (Index > 0 && OtherIndex > 0 && Parents[Index-1] == OtherParents[OtherIndex-1])
				{
					CommonAncestor = Parents[Index-1];
					--Index;
					--OtherIndex;
				}
				return CommonAncestor != nullptr;
			}
			return false;
		};

		// if common ancestor, compare depths
		if (HasCommonAncestor() && Index != OtherIndex)
		{
			return OtherIndex > Index; 
		}

		// otherwise, compare indices
		if (Control1->GetIndex() > Control0->GetIndex())
		{
			return true;
		}
	...
	// re-add last selected control stored if bShiftPressedWhenStarted
	if (LastControl)
	{
		Controls.Add(LastControl);
	}
			
	UE::ControlRigEditMode::FInteractionDependencyCache& NewDependency = InteractionDependencies.Add(InControlRig);
	...
	{
		FRigControlElement* Control = Controls[Index];
		
		FControlRigSelectionDuringDrag ControlDrag;
		ControlDrag.LevelSequence = LevelSequence;
		ControlDrag.ControlName = Control->GetFName();
		ControlDrag.ControlRig = InControlRig;
		ControlDrag.CurrentTransform = GetControlWorldSpaceTransform(ControlDrag.ControlRig, ControlDrag.ControlName);
		ControlRigDrags.Add(ControlDrag);
				
		for (int32 NextIndex = Index+1; NextIndex < Controls.Num(); ++NextIndex)
		{
			FRigControlElement* OtherControl = Controls[NextIndex];
...
void USequencerPivotTool::PrepareActorForManipulation(AActor* InActor)
{
	if (!InActor)
	{
		return;
	...
	ActorDrag.Actor = InActor;
	ActorDrag.CurrentTransform = InActor->GetTransform();
	ActorDrags.Add(ActorDrag);
}

void USequencerPivotTool::UpdateBoundComponents()
{
	...
	for (const TSharedPtr<FControlRigInteractionScope>& InteractionScope: InteractionScopes)
	{
		if (UControlRig* ControlRig = InteractionScope->GetControlRig())
		{
			USceneComponent* SceneComponent = UE::AIE::PivotTool::GetBoundComponent(ControlRig);
			...
			else if (USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(SceneComponent))
			{
				if (!ControlRig->ContainsSimulation())
				{
					...
					constexpr bool bIncludeFromChildActors = true, bDoNotNeedValidRootMotion = false;
					const AActor* Parent = SkeletalMeshComponent->GetOwner();
					while (Parent)
					{
						Parent->ForEachComponent<USkeletalMeshComponent>(bIncludeFromChildActors, [](USkeletalMeshComponent* SkeletalMeshToUpdate)
						{
							if (SkeletalMeshToUpdate)
							{
								SkeletalMeshToUpdate->TickAnimation(DeltaTime, bDoNotNeedValidRootMotion);
