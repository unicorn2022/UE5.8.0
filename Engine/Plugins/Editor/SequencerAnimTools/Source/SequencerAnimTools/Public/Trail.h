...
struct FColorState
{
	...
	SEQUENCERANIMTOOLS_API void Setup(FTrailHierarchy* TrailHierarchy);
	SEQUENCERANIMTOOLS_API void ReadyForTrail(bool bInIsPinned, EMotionTrailTrailStyle InPinnedStyle);
	SEQUENCERANIMTOOLS_API EMotionTrailTrailStyle GetStyle() const;
};
...
public:
	...
	virtual bool IsAnythingSelected(FVector& OutVectorPosition, FQuat& OutRotation)const { return false;}
	...
	virtual bool GetOffsetMode() const {return false;}
	...
	SEQUENCERANIMTOOLS_API virtual bool HandleObjectsChanged(const TMap<UObject*, UObject*>& ReplacementMap);
	...
	SEQUENCERANIMTOOLS_API virtual void GetColor(const FFrameNumber& CurrentTime, FColorState& InOutColorState);
	//get and restore trail from it's data
	class FMotionTrailState
	{
	public:
		virtual ~FMotionTrailState() = default;
