...
void STutorialOverlay::FocusOnAnyBlueprintNodes(const FTutorialWidgetContent &WidgetContent)
{
	...
	else if ( !ObjectPath.IsEmpty() )
	{
		...
		UE_CLOGF(FPackageName::IsShortPackageName(ObjectPath), LogClass, Warning, "FocusOnAnyBlueprintNodes expects path names for WrapperIdentifier, got short name instead: \"%ls\"", *ObjectPath);
