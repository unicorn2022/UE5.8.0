...
public:
		...
		TArray<FObjectMixerOutlinerMode::FFilterClassSelectionInfo>& FilterClassSelectionInfos, const FText TooltipText)
	{
		...
		if (bHasValidAssetData)
		{
			...
			ToolTipText = FindInContentBrowserCommand->GetDescription();
		}
		else
		{
			ToolTipText = NSLOCTEXT("SFilterClassMenuItem", "NoBlueprintFilterFound", "This filter class is not a Blueprint class.");
		}
