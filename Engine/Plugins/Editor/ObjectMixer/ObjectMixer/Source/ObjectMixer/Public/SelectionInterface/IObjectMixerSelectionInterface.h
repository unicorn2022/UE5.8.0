...
public:
	...
	/**
	 * Select or deselect a list of actor sub-objects.
	 * @param	InSelectedSubObjects	The sub-objects to select.
	 * @param	bShouldSelect			If true, select the sub-objects and deselect all other sub-objects. If false, only deselect the listed sub-objects.
	 * @param	bSelectEvenIfHidden		If false, only select sub-objects that are visible (e.g. not filtered out) in the editor.
	 */
	virtual void SelectSubObjects(const TArray<UObject*>& InSelectedSubObjects, bool bShouldSelect, bool bSelectEvenIfHidden) = 0;
	...
	/** Get the list of selected sub-objects */
	virtual TArray<UObject*> GetSelectedSubObjects() const = 0;
