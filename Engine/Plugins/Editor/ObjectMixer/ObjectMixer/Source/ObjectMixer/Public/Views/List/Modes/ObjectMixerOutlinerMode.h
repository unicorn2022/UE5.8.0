...
namespace ObjectMixerOutliner
{
	...
	{
		bool operator()(TWeakPtr<ISceneOutlinerTreeItem> Item, UObject*& OutObject);
	};
}
...
protected:
	...
	UE_API void SelectSubObjectsInEditor(const TArray<UObject*>& InSelectedSubObjects, bool bShouldSelect, bool bSelectEvenIfHidden);
	...
	/** Gets the list of actor sub-objects that are selected in the editor. */
	UE_API TArray<UObject*> GetSelectedSubObjectsInEditor() const;
	...
	/** Synchronizes the object mixer's selection with any sub-objects selected elsewhere in the editor */
	UE_API void SynchronizeSubObjectSelection();
