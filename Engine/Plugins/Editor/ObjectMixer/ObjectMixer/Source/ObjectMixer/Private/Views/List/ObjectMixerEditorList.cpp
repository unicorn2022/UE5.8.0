...
void FObjectMixerEditorList::BuildPerformanceCache()
{
	...
	for (const TObjectPtr<UObjectMixerObjectFilter>& FilterInstance : GetObjectFilterInstances())
	{
		if (!FilterInstance)
		{
			UE_LOGF(LogObjectMixerEditor, Display, "%s: UObjectMixerObjectFilter instance not valid.", __FUNCTION__);
			continue;
		}
	...
	{
		UE_LOGF(LogObjectMixerEditor, Display, "%s: No Main UObjectMixerObjectFilter instance found.", __FUNCTION__);
	}
}
