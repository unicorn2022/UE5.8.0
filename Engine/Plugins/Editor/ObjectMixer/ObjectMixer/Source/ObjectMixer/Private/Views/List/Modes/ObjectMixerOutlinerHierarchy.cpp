	...
	const ISceneOutlinerTreeItem& Item, const TMap<FSceneOutlinerTreeItemID, FSceneOutlinerTreeItemPtr>& Items, bool bCreate)
{
	...
	else if (const FObjectMixerEditorListRowUObject* ObjectItem = Item.CastTo<FObjectMixerEditorListRowUObject>())
	{
		if (UObject* Object = ObjectItem->ObjectSoftPtr.Get())
		{
			UObject* ParentObject = ObjectItem->bIncludeObjectStack ? Object->GetOuter() : Object->GetTypedOuter<AActor>();
			if (ParentObject)
			{
				if (ObjectItem->ContainerProperty != NAME_None)
				{
					FSceneOutlinerTreeItemID ContainerRowId = HashCombineFast(GetTypeHash(ParentObject), GetTypeHash(ObjectItem->ContainerProperty));
					if (const FSceneOutlinerTreeItemPtr* ParentItem = Items.Find(ContainerRowId))
					{
						return *ParentItem;
					}
					else if (bCreate)
					{
						if (FSceneOutlinerTreeItemPtr ContainerItem =
							Mode->CreateItemFor<FObjectMixerEditorListRowContainer>(
								FObjectMixerEditorListRowContainer(ParentObject, ObjectItem->ContainerProperty, GetCastedMode()->GetSceneOutliner()), true))
						{
				...
				if (const FSceneOutlinerTreeItemPtr* ParentItem = Items.Find(ParentObject))
				{
					return *ParentItem;
				}
				else if (bCreate)
				{
					if (ParentObject->IsA<AActor>())
					{
						if (FSceneOutlinerTreeItemPtr ActorItem =
								Mode->CreateItemFor<FObjectMixerEditorListRowActor>(
									FObjectMixerEditorListRowActor(Cast<AActor>(ParentObject), GetCastedMode()->GetSceneOutliner()), true))
						{
							return ActorItem;
						}
					}
					...
					{
						if (TSharedPtr<FObjectMixerEditorListRowUObject> ParentObjectItem =
							StaticCastSharedPtr<FObjectMixerEditorListRowUObject>(Mode->CreateItemFor<FObjectMixerEditorListRowUObject>(
								FObjectMixerEditorListRowUObject(ParentObject, GetCastedMode()->GetSceneOutliner()), true)))
						{
							ParentObjectItem->bIncludeObjectStack = ObjectItem->bIncludeObjectStack;
	...
	else if (const FObjectMixerEditorListRowContainer* ContainerItem = Item.CastTo<FObjectMixerEditorListRowContainer>())
	{
		if (UObject* PropertyOwner = ContainerItem->GetPropertyOwner())
		{
			if (const FSceneOutlinerTreeItemPtr* ParentItem = Items.Find(PropertyOwner))
			{
				return *ParentItem;
			}
			else if (bCreate)
			{
				if (PropertyOwner->IsA<AActor>())
				{
					if (FSceneOutlinerTreeItemPtr ActorItem =
							Mode->CreateItemFor<FObjectMixerEditorListRowActor>(
								FObjectMixerEditorListRowActor(Cast<AActor>(PropertyOwner), GetCastedMode()->GetSceneOutliner()), true))
					{
						return ActorItem;
					}
				}
				else
				{
					if (TSharedPtr<FObjectMixerEditorListRowUObject> ParentObjectItem =
						StaticCastSharedPtr<FObjectMixerEditorListRowUObject>(Mode->CreateItemFor<FObjectMixerEditorListRowUObject>(
							FObjectMixerEditorListRowUObject(PropertyOwner, GetCastedMode()->GetSceneOutliner()), true)))
					{
						// Assume that, since a container item exists, that it is part of a full sub-object stack
						ParentObjectItem->bIncludeObjectStack = true;
						return ParentObjectItem;
					}
				}
			}
...
bool FObjectMixerOutlinerHierarchy::CreateSubObjectItems(AActor* Actor, TArray<FSceneOutlinerTreeItemPtr>& OutItems) const
{
	bool bAnyItemsBeforeFiltering = false;
	...
	if (const UObjectMixerObjectFilter* Filter = GetObjectFilter())
	{
		const TArray<FObjectMixerEditorActorSubObject> SubObjects = Filter->GetActorSubObjects(Actor);

		for (const FObjectMixerEditorActorSubObject& SubObject : SubObjects)
		{
			if (TStrongObjectPtr<UObject> Object = SubObject.Object.Pin())
			{
				bAnyItemsBeforeFiltering = true;

				if (SubObject.bIncludeObjectStack)
				{
					bool bIsValidSubObjectStack = false;
					TArray<UObject*> SubObjectStack;
					UObject* Outer = Object.Get()->GetOuter();
					while (Outer != Actor && Outer != nullptr)
					{
						SubObjectStack.Add(Outer);
						Outer = Outer->GetOuter();
						if (Outer == Actor)
						{
							bIsValidSubObjectStack = true;
						}
					}

					if (bIsValidSubObjectStack)
					{
						for (int32 Index = 0; Index < SubObjectStack.Num(); ++Index)
						{
							if (FSceneOutlinerTreeItemPtr ObjectItem =
								Mode->CreateItemFor<FObjectMixerEditorListRowUObject>(
									FObjectMixerEditorListRowUObject(SubObjectStack[Index], GetCastedMode()->GetSceneOutliner(), FText::GetEmpty())))
							{
								OutItems.Add(ObjectItem);
							}
						}
				...
				if (SubObject.ContainerProperty.PropertyName != NAME_None)
				{
					if (TSharedPtr<FObjectMixerEditorListRowContainer> ContainerItem =
						StaticCastSharedPtr<FObjectMixerEditorListRowContainer>(Mode->CreateItemFor<FObjectMixerEditorListRowContainer>(
							FObjectMixerEditorListRowContainer(Object->GetOuter(), SubObject.ContainerProperty.PropertyName, GetCastedMode()->GetSceneOutliner()))))
					{
						ContainerItem->PropertyIconOverride = SubObject.ContainerProperty.PropertyIconOverride;
						OutItems.Add(ContainerItem);
					}				
				}
				
				if (TSharedPtr<FObjectMixerEditorListRowUObject> ObjectItem =
					StaticCastSharedPtr<FObjectMixerEditorListRowUObject>(Mode->CreateItemFor<FObjectMixerEditorListRowUObject>(
						FObjectMixerEditorListRowUObject(Object.Get(), GetCastedMode()->GetSceneOutliner(), SubObject.DisplayName))))
				{
					ObjectItem->ContainerProperty = SubObject.ContainerProperty.PropertyName;
					ObjectItem->bIncludeObjectStack = SubObject.bIncludeObjectStack;
					OutItems.Add(ObjectItem);
				}
			}
		}
...
TArray<FSceneOutlinerTreeItemPtr> FObjectMixerOutlinerHierarchy::ConditionallyCreateActorAndComponentItems(AActor* Actor) const
{
	...
	TArray<FSceneOutlinerTreeItemPtr> SubObjectRows;
	bWouldReturnAnyChildItemsBeforeFiltering |= CreateSubObjectItems(Actor, SubObjectRows);
	...
	{
		// Force the actor creation regardless of whether it passes the current filters, as one of its children did
		const bool bForce = true;
		if (const FSceneOutlinerTreeItemPtr ActorItem =
			Mode->CreateItemFor<FObjectMixerEditorListRowActor>(
				FObjectMixerEditorListRowActor(Actor, GetCastedMode()->GetSceneOutliner()), bForce))
		{
			...
			else
			{
				...
				ReturnValue.Append(SubObjectRows);
			}
...
void FObjectMixerOutlinerHierarchy::CreateChildren(const FSceneOutlinerTreeItemPtr& Item, TArray<FSceneOutlinerTreeItemPtr>& OutChildren) const
{
	...
	else if (const FActorTreeItem* ParentActorItem = Item->CastTo<FActorTreeItem>())
	{
		...
		CreateSubObjectItems(ParentActor, OutChildren);
