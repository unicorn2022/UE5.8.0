...
struct FValidatorStatistics
{
	...
	/** Total time (in seconds) that this validator spent validating assets */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Validation")
...
struct FValidateAssetsResults
{
	...
	/** Additional validator tokenized messages not specific to a single asset. */
	TArray<TSharedRef<FTokenizedMessage>> ValidatorMessages;
};
...
public:
	...
	*/
	...
	/**
	 * Gets the full class object path of the UDataValidationCommandlet class to use when the Editor needs to instantiate
	 * the commandlet to run validation on the entire project. If empty, UDataValidationCommandlet will be used.
	 */
	const FString& GetDataValidationCommandletClassName() const { return DataValidationCommandletClassName; }
...
protected:
	 ...
	 */
	UE_API void ValidateChangelistPreSubmit(FSourceControlChangelistPtr Changelist, FSourceControlValidationDelegateResult& OutResult) const;

	/**
	 * Validates files before SCC submit operation
	 */
	UE_API void ValidateFilesPreSubmit(TArray<FSourceControlStateRef>& States, FSourceControlValidationDelegateResult& OutResult) const;
...
protected:
	friend class FDataValidationModule;
...
protected:
	...
	/**
	 * The full class object path of the UDataValidationCommandlet class to use when the Editor needs to instantiate
	 * the commandlet to run validation on the entire project. If empty, UDataValidationCommandlet will be used.
	 */
	UPROPERTY(config)
	FString DataValidationCommandletClassName;
	...
	mutable TSet<FName> SavedPackagesToValidate;
	 ...
	 */
	uint32 DisableValidateOnSaveCount = 0;
};
