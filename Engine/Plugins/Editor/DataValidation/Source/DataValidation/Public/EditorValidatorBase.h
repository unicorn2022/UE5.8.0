...
public:
	...
	/** Override this to do any state cleanup or add additional validation messages after all per asset validation is complete. */
	virtual void PostAssetValidation(TArray<TSharedRef<FTokenizedMessage>>& OutMessages) {}
...
protected:
	...
	UE_API virtual bool K2_CanValidateAsset_Implementation(UObject* InAsset) const;
...
private:
	...
	UPROPERTY(Transient)
	TObjectPtr<UObject> CurrentObjectBeingValidated = nullptr;
	const FAssetData* CurrentAssetBeingValidated = nullptr;
};
