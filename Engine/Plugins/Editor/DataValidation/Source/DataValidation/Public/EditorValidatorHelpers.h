...
{    
	...
	/** Prepend the given UTC timestamp (or UtcNow if none provided) to the given base title to provide a pseudo-unique title, eg) "[Timestamp] BaseTitle" */
	UE_API FText MakeTimestampedMessageLogPageTitle(const FText& BaseTitle);
	UE_API FText MakeTimestampedMessageLogPageTitle(const FText& BaseTitle, const FDateTime& UtcTimestamp);

	UE_API void ValidationStatsToAnalyticEventData(const TMap<FTopLevelAssetPath, FValidatorStatistics>& ValidationStats, TArray<FAnalyticsEventAttribute>& OutEventAttributes);
#if ENABLE_COOK_STATS
	UE_API void ValidationStatsToAnalyticEventData(const TMap<FTopLevelAssetPath, FValidatorStatistics>& ValidationStats, FCookStatsManager::AddStatFuncRef AddStat);
#endif
}
