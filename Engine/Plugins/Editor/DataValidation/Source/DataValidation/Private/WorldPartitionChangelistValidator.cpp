...
void UWorldPartitionChangelistValidator::ValidateActorsAndDataLayersFromChangeList(UDataValidationChangelist* Changelist)
{
	...
	auto TryAssociateActorToMap = [&MapToActorsFiles](const FAssetData& AssetData) -> UClass*
	{
		...
		if (FWorldPartitionActorDescUtils::IsValidActorDescriptorFromAssetData(AssetData))
		{
			...
			if (!ActorFiles)
			{
				...
				{
					UE_LOGF(LogContentValidation, Log, "ValidateActorsAndDataLayersFromChangeList found a map with invalid path '%ls'. It may be in an umounted content folder. Referencing actor package: %ls", *PackageName.ToString(), *AssetData.PackageName.ToString());
				}
			}
	...
	for (TTuple<FTopLevelAssetPath, TSet<FAssetData>>& It : MapToActorsFiles)
	{
		...
		for (const FAssetData& ActorData : ActorsData)
		{
			...
			if (const FWorldPartitionActorDescInstance* ActorDescInstance = ContainersToValidate.GetActorDescInstanceByName(ActorData.AssetName))
			{
...
void UWorldPartitionChangelistValidator::OnInvalidReferenceDataLayerAsset(const UDataLayerInstanceWithAsset* DataLayerInstance)
{
	if (bSubmittingWorldDataLayers)
	{
		FText CurrentError = FText::Format(LOCTEXT("DataValidation.Changelist.WorldPartition.InvalidDataLayerAsset", "Data layer {0} has no data layer asset."),
			FText::FromName(DataLayerInstance->GetFName()));
...
void UWorldPartitionChangelistValidator::OnInvalidDataLayerAssetType(const UDataLayerInstanceWithAsset* DataLayerInstance, const UDataLayerAsset* DataLayerAsset)
{
	if (bSubmittingWorldDataLayers)
	{
		FText CurrentError = FText::Format(LOCTEXT("DataValidation.Changelist.WorldPartition.InvalidDataLayerAssetAsset", "Data layer {0} is not compatible with Data Layer Asset {1} type {2}."),
			FText::FromName(DataLayerInstance->GetFName()), FText::FromName(DataLayerAsset->GetFName()), FText::FromName(DataLayerAsset->GetClass()->GetFName()));
...
void UWorldPartitionChangelistValidator::OnDataLayerAssetConflict(const UDataLayerInstanceWithAsset* DataLayerInstance, const UDataLayerInstanceWithAsset* ConflictingDataLayerInstance)
{
		...
		|| bSubmittingWorldDataLayers)
	{
		FText CurrentError = FText::Format(LOCTEXT("DataValidation.Changelist.WorldPartition.DataLayerAssetConflict", "Data layer instance {0} and data layer instance {1} are both referencing data layer asset {2}."),
			FText::FromName(DataLayerInstance->GetFName()),
			FText::FromName(ConflictingDataLayerInstance->GetFName()),
...
void UWorldPartitionChangelistValidator::OnRejectedActorDescViewMutator(const IWorldPartitionActorDescInstanceView& ActorDescView, const FActorDescViewMutator& RejectedOverrides, EActorDescViewMutatorRejectedReason Reason)
{
	if (Filter(ActorDescView))
	{
		FText CurrentWarning;
		switch (Reason)
		{
		case EActorDescViewMutatorRejectedReason::DivergesClusterRuntimeGrid:
			CurrentWarning = FText::Format(LOCTEXT("DataValidation.Changelist.WorldPartition.MutatorRejectedRuntimeGrid", "Mutator override rejected: cannot set {0} RuntimeGrid to {1} because it would split a reference-connected cluster"),
				FText::FromString(GetFullActorName(ActorDescView)),
				FText::FromName(RejectedOverrides.RuntimeGrid.GetValue()));
			break;
		case EActorDescViewMutatorRejectedReason::DivergesClusterIsSpatiallyLoaded:
			CurrentWarning = FText::Format(LOCTEXT("DataValidation.Changelist.WorldPartition.MutatorRejectedIsSpatiallyLoaded", "Mutator override rejected: cannot set {0} IsSpatiallyLoaded to {1} because it would split a reference-connected cluster"),
				FText::FromString(GetFullActorName(ActorDescView)),
				FText::FromString(RejectedOverrides.bIsSpatiallyLoaded.GetValue() ? TEXT("true") : TEXT("false")));
			break;
		}
