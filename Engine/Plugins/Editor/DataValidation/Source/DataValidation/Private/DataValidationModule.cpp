...
namespace DataValidationOptions
{
	static bool bReportCookValidationStats = true;
	static FAutoConsoleVariableRef CVarReportCookValidationStats(
		TEXT("DataValidation.ReportCookValidationStats"),
		bReportCookValidationStats,
		TEXT("True to report post-cook statistics for each validator"));
}
...
private:
	...
	void OnCookFinished(UE::Cook::ICookInfo& CookInfo);
	...
	struct FCookValidationStats
	{
		TMap<FTopLevelAssetPath, FValidatorStatistics> ValidatorStats;
		double ExternalObjectLoadTotalDurationSeconds = 0.0;
		double ExternalObjectUnloadTotalDurationSeconds = 0.0;
		int32 ExternalObjectTotalCount = 0;
	};
	TOptional<FCookValidationStats> CookValidationStats;
};
...
{	
	UE::Cook::FDelegates::CookStarted.AddRaw(this, &FDataValidationModule::OnCookStarted);
	UE::Cook::FDelegates::CookFinished.AddRaw(this, &FDataValidationModule::OnCookFinished);
#if ENABLE_COOK_STATS
	FCookStatsManager::CookStatsCallbacks.AddRaw(this, &FDataValidationModule::LogCookStats);
#endif
...
{
	UE::Cook::FDelegates::CookStarted.RemoveAll(this);
	UE::Cook::FDelegates::CookFinished.RemoveAll(this);
#if ENABLE_COOK_STATS
...
void FDataValidationModule::ValidateAssets(const TArray<FAssetData>& SelectedAssets, bool bValidateDependencies, const EDataValidationUsecase InValidationUsecase)
{
	...
	if (EditorValidationSubsystem)
	{
		...
		Settings.MessageLogPageTitle = UE::DataValidation::MakeTimestampedMessageLogPageTitle(FText::Format(LOCTEXT("MessageLogPageTitle.ValidateSelectedAssets", "Asset Validation: {0}"), SelectedAssets.Num() == 1 ? FText::FromName(SelectedAssets[0].AssetName) : LOCTEXT("MultipleAssets", "multiple assets")));
...
void FDataValidationModule::OnCookStarted(UE::Cook::ICookInfo& CookInfo)
{
	using namespace UE::Cook;

	if (DataValidationOptions::bReportCookValidationStats)
	{
		CookValidationStats.Emplace();
	}
	else
	{
		CookValidationStats.Reset();
	}

	if (EnumHasAnyFlags(CookInfo.GetCookValidationOptions(), ECookValidationOptions::RunAssetValidation))
	{
		// Log the enabled validators
		// This will also preload any BP-based validators before we start the main cook
		if (UEditorValidatorSubsystem* EditorValidationSubsystem = GEditor->GetEditorSubsystem<UEditorValidatorSubsystem>())
		{
			int32 NumEnabledValidators = 0;

			UE_LOGF(LogContentValidation, Display, "Enabled validators:");
			EditorValidationSubsystem->ForEachEnabledValidator([&NumEnabledValidators](UEditorValidatorBase* Validator)
			{
				++NumEnabledValidators;
				UE_LOGF(LogContentValidation, Display, "\t%ls", *Validator->GetClass()->GetClassPathName().ToString());
				return true;
			});

			if (CookValidationStats)
			{
				CookValidationStats->ValidatorStats.Reserve(NumEnabledValidators);
			}
		}
	}
}

void FDataValidationModule::OnCookFinished(UE::Cook::ICookInfo& CookInfo)
{
	using namespace UE::Cook;

	if (CookValidationStats && EnumHasAnyFlags(CookInfo.GetCookValidationOptions(), ECookValidationOptions::RunAssetValidation))
	{
		// Report the validators, slowest to fastest
		CookValidationStats->ValidatorStats.ValueSort(
			[](const FValidatorStatistics& One, const FValidatorStatistics& Two)
			{
				return One.TotalDurationSeconds > Two.TotalDurationSeconds;
			});

		UE_LOGF(LogContentValidation, Display, "Validation stats: (%d external objects took %f seconds to load and %f seconds to unload)", CookValidationStats->ExternalObjectTotalCount, CookValidationStats->ExternalObjectLoadTotalDurationSeconds, CookValidationStats->ExternalObjectUnloadTotalDurationSeconds);
		for (const TTuple<FTopLevelAssetPath, FValidatorStatistics>& ValidatorStatsPair : CookValidationStats->ValidatorStats)
		{
			UE_LOGF(LogContentValidation, Display, "\t%ls validated %d assets in %f seconds", *ValidatorStatsPair.Key.ToString(), ValidatorStatsPair.Value.AssetsValidated, ValidatorStatsPair.Value.TotalDurationSeconds);
		}
	}
}

#if ENABLE_COOK_STATS
void FDataValidationModule::LogCookStats(FCookStatsManager::AddStatFuncRef AddStat)
{
	if (CookValidationStats)
	{
		UE::DataValidation::ValidationStatsToAnalyticEventData(CookValidationStats->ValidatorStats, AddStat);

		if (CookValidationStats->ExternalObjectTotalCount > 0)
		{
			AddStat(
				TEXT("Validation_ExternalObjectLoad"),
				FCookStatsManager::CreateKeyValueArray(TEXT("TotalDurationSecs"), CookValidationStats->ExternalObjectLoadTotalDurationSeconds, TEXT("AssetsValidated"), CookValidationStats->ExternalObjectTotalCount)
				);

			AddStat(
				TEXT("Validation_ExternalObjectUnload"),
				FCookStatsManager::CreateKeyValueArray(TEXT("TotalDurationSecs"), CookValidationStats->ExternalObjectUnloadTotalDurationSeconds, TEXT("AssetsValidated"), CookValidationStats->ExternalObjectTotalCount)
				);
		}
	}
}
...
EDataValidationResult FDataValidationModule::OnValidateSourcePackageDuringCook(UPackage* Package, FDataValidationContext& ValidationContext)
{
	...
	{
		TArray<FAssetData> AssetList;
		IAssetRegistry::GetChecked().GetAssetsByPackageName(Package->GetFName(), AssetList, /*bIncludeOnlyOnDiskAssets*/true);
		...
		if (AssetList.Num() > 0)
		{
			...
			for (const FAssetData& AssetData : AssetList)
			{
				if (UObject* Asset = AssetData.FastGetAsset(/*bLoad*/false))
				{
					...
					TOptional<UE::DataValidation::FScopedLoadExternalObjects> ExternalObjectsLoader;
					if (!ValidationContext.GetAssociatedExternalObjects().IsEmpty())
					{
						if (CookValidationStats)
						{
							double LoadDuration = 0.0;
							{
								FScopedDurationTimer ValidationTimer(LoadDuration);
								ExternalObjectsLoader.Emplace(Asset, ValidationContext, true);
							}
							CookValidationStats->ExternalObjectLoadTotalDurationSeconds += LoadDuration;
							CookValidationStats->ExternalObjectTotalCount += ValidationContext.GetAssociatedExternalObjects().Num();
						}
						else
						{
							ExternalObjectsLoader.Emplace(Asset, ValidationContext, true);
					...
					EDataValidationResult ValidationResult = EditorValidationSubsystem->ValidateObjectInternal(AssetData, Asset, ValidationContext, CookValidationStats ? &CookValidationStats->ValidatorStats : nullptr);
					...
					if (ExternalObjectsLoader)
					{
						if (CookValidationStats)
						{
							double UnloadDuration = 0.0;
							{
								FScopedDurationTimer ValidationTimer(UnloadDuration);
								ExternalObjectsLoader.Reset();
							}
							CookValidationStats->ExternalObjectUnloadTotalDurationSeconds += UnloadDuration;
						}
						else
						{
							ExternalObjectsLoader.Reset();
