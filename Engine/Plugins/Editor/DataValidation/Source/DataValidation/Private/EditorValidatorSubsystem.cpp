...
{
	static int32 AssetValidationBatchSize = 50;
	static FAutoConsoleVariableRef CVarAssetValidationBatchSize(
		TEXT("EditorValidator.AssetValidationBatchSize"),
		AssetValidationBatchSize,
		TEXT("Controls number of assets per batch. Affects number of assets preloaded at a time, and number of assets unloaded at a time"));

	static bool bEnablePreloadingAssets = false;
	static FAutoConsoleVariableRef CVarEnablePreloadingAssets(
		TEXT("EditorValidator.EnablePreloadingAssets"),
		bEnablePreloadingAssets,
		TEXT("Enables preloading assets during validation to improve performance"));

	static int32 MaxAssetBatchPreloads = 20;
	static FAutoConsoleVariableRef CVarMaxAssetBatchPreloads(
		TEXT("EditorValidator.MaxAssetBatchPreloads"),
		MaxAssetBatchPreloads,
		TEXT("Number of asset batches to preload at a time"));

	void LogLoadingAsset(const FAssetData& AssetData, bool bUnloadAssetsLoadedForValidation)
	{
		UE_LOGFMT(LogContentValidation, Log, "Loading asset {Asset} for validation. After the validation we will {Action}unload it",
			("Asset", AssetData.ToSoftObjectPath()), ("Action", bUnloadAssetsLoadedForValidation ? TEXT("") : TEXT("not ")));
	};

	struct FAssetToValidate
	{
		const FAssetData* Data = nullptr;
		bool bWasAlreadyLoaded = false;
	};

	struct FValidationAssetRange
	{
		FValidationAssetRange(const TSet<FAssetData>& InAllAssets, const FValidateAssetsSettings& InSettings,
			TFunctionRef<bool(const FAssetData& Data)> InShouldSkipAsset, TFunction<void()> InOnAssetsLoaded)
			: OnAssetsLoaded(InOnAssetsLoaded)
			, BatchSize(FMath::Max(1, AssetValidationBatchSize))
			, bPreloadAssets(bEnablePreloadingAssets && !InSettings.bCaptureAssetLoadLogs)
		{
			Assets.Reserve(FMath::Clamp(InAllAssets.Num(), 0, InSettings.MaxAssetsToValidate));
			for (const FAssetData& Data : InAllAssets)
			{
				if (Assets.Num() >= InSettings.MaxAssetsToValidate)
				{
					break;
				}
				...
				bool bAlreadyLoaded = Data.FastGetAsset(false) != nullptr;

				if (InShouldSkipAsset(Data) || (!InSettings.bLoadAssetsForValidation && !bAlreadyLoaded))
				{
					++NumSkipped;
					continue;
				}

				Assets.Add({ &Data, bAlreadyLoaded });
			}

			// Kick off the first set of preloads
			if (bPreloadAssets)
			{
				int32 NumBatches = FMath::CeilToInt32(static_cast<float>(Assets.Num()) / BatchSize);
				int32 InitialPreloads = FMath::Clamp(MaxAssetBatchPreloads, 1, NumBatches);
				for (int32 i = 0; i < InitialPreloads; ++i)
				{
					StartNextPreloadBatch();
				}
			}
		}

		~FValidationAssetRange()
		{
			// Flush any remaining async loads before unloading
			for (const TArray<int32>& Batch : BatchRequestIds)
			{
				FlushAsyncLoading(Batch);
			}

			UnloadTrackedPackages();
		}

		int32 GetNumSkipped() const { return NumSkipped; }

		void TrackPackageForUnload(UPackage* Package)
		{
			if (Package)
			{
				PackagesToUnload.Emplace(Package);
			}
		}

		struct FIterator
		{
			FValidationAssetRange& Owner;
			int32 Index = 0;

			FAssetToValidate operator*() const { return Owner.Assets[Index]; }

			FIterator& operator++()
			{
				++Index;
				Owner.OnAssetConsumed();
				return *this;
			}
		...
		FIterator begin()
		{
			ensureMsgf(!bBeginCalled, TEXT("FValidationAssetRange does not support multiple iterations"));
			bBeginCalled = true;

			EnsureCurrentBatchReady();

			return { *this, 0 };
		}

		FIterator end() { return { *this, Assets.Num() }; }

	private:
		void UnloadTrackedPackages()
		{
			...
			// Remove entries that were successfully unloaded
			// Keep entries still held alive by other preload handles and we'll try
			// again in the next batch boundary
			PackagesToUnload.RemoveAll([](const TWeakObjectPtr<UPackage>& Ptr) { return !Ptr.IsValid(); });
		}

		void StartNextPreloadBatch()
		{
			TArray<int32> RequestIds;
			RequestIds.Reserve(BatchSize);
			while (PrefetchIdx < Assets.Num() && RequestIds.Num() < BatchSize)
			{
				const FAssetToValidate& Asset = Assets[PrefetchIdx++];
				if (Asset.bWasAlreadyLoaded)
				{
					continue;
				}

				int32 RequestId = LoadPackageAsync(Asset.Data->PackageName.ToString());
			...
			BatchRequestIds.Add(MoveTemp(RequestIds));
		}

		void EnsureCurrentBatchReady()
		{
			if (!bPreloadAssets || bWaitedForCurrentBatch)
			{
				return;
			}

			if (CurrentBatch < BatchRequestIds.Num())
			{
				FlushAsyncLoading(BatchRequestIds[CurrentBatch]);
				OnAssetsLoaded();
				bWaitedForCurrentBatch = true;
			}
		}

		void OnAssetConsumed()
		{
			if (++AssetsConsumedInBatch < BatchSize)
			{
				return;
			}
			...
			{
				++CurrentBatch;
				bWaitedForCurrentBatch = false;

				// Start the next batch in line
				StartNextPreloadBatch();
				EnsureCurrentBatchReady();
			}

			UnloadTrackedPackages();
			AssetsConsumedInBatch = 0;
		}

		TFunction<void()> OnAssetsLoaded;
		TArray<FAssetToValidate> Assets;
		TArray<TArray<int32>> BatchRequestIds;
		TArray<TWeakObjectPtr<UPackage>> PackagesToUnload;		
		int32 NumSkipped = 0;
		int32 BatchSize = 1;
		int32 PrefetchIdx = 0;
		int32 CurrentBatch = 0;
		int32 AssetsConsumedInBatch = 0;
		bool bPreloadAssets;
		bool bBeginCalled = false;
		bool bWaitedForCurrentBatch = false;
	};
} // namespace UE::DataValidation
...
{
	// C++ registration
	FCoreDelegates::GetOnPostEngineInit().AddUObject(this, &UEditorValidatorSubsystem::RegisterNativeValidators);
	...
	ISourceControlModule::Get().RegisterPreSubmitChangelistValidationEx(FSourceControlPreSubmitChangelistValidationExDelegate::CreateUObject(this, &UEditorValidatorSubsystem::ValidateChangelistPreSubmit));
	ISourceControlModule::Get().RegisterPreSubmitFilesValidationEx(FSourceControlPreSubmitFilesValidationExDelegate::CreateUObject(this, &UEditorValidatorSubsystem::ValidateFilesPreSubmit));
}
...
{
	EDataValidationResult Result = EDataValidationResult::NotValidated;
	UE_LOG_CONTEXT("AssetBeingValidated", InAssetData.GetSoftObjectPath());
	...
	if (ensure(InObject) && ensure(InAssetData.IsValid()))
	{
		...
		{
			UE_LOGF(LogContentValidation, Verbose, "Validating '%ls' with '%ls'...", *InObject->GetPathName(), *Validator->GetClass()->GetName());
			...
			EDataValidationResult NewResult = EDataValidationResult::NotValidated;
			if (ValidatorStatistics)
			{
				double ValidationDuration = 0.0;
				{
					FScopedDurationTimer ValidationTimer(ValidationDuration);
					NewResult = Validator->ValidateLoadedAsset(InAssetData, InObject, InContext);
				}

				FValidatorStatistics& ThisValidatorStats = ValidatorStatistics->FindOrAdd(Validator->GetClass()->GetClassPathName());
				if (NewResult != EDataValidationResult::NotValidated)
				{
					ThisValidatorStats.AssetsValidated++;
				}
				ThisValidatorStats.TotalDurationSeconds += ValidationDuration;
			}
			else
			{
				NewResult = Validator->ValidateLoadedAsset(InAssetData, InObject, InContext);
	...
	FValidateAssetsResults& OutResults) const
{
	...
	EDataValidationResult Result = EDataValidationResult::NotValidated;
	UE::Telemetry::DataValidation::FValidateAssets Telemetry(InSettings);

	ON_SCOPE_EXIT
	{
		Telemetry.SetResults(Result, OutResults);
		Telemetry.Duration = FPlatformTime::Seconds() - StartTime;
	...
	Result = ValidateAssetsInternal(DataValidationLog, TSet<FAssetData>{AssetDataList}, InSettings, OutResults);
	...
	FValidateAssetsResults& 		OutResults) const
{
	...
	UE_LOGF(LogContentValidation, Log, "Enabled validators:");
	ForEachEnabledValidator([](UEditorValidatorBase* Validator)
	{
		UE_LOGF(LogContentValidation, Log, "\t%ls", *Validator->GetClass()->GetClassPathName().ToString());
		return true;
	});
	...
	{
		FDataValidationContext ValidationContext(false, InSettings.ValidationUsecase, {}, InSettings.MessageLogName);
		...
		for (auto It = AssetsToExternalObjects.CreateIterator(); It; ++It)
		{
			...
			{
				UE_LOGF(LogContentValidation, Display, "Package %ls (owner of some external objects) being skipped for validation.", 
	...
	UE_LOGF(LogContentValidation, Display, "Starting to validate %d assets (%d associated objects such as actors)", 
	...
	auto ShouldSkipAsset = [&InSettings, this](const FAssetData& Data) -> bool
	{
	...
	auto OnAssetsLoaded = [&InSettings, this]()
	{
		WaitForAssetCompilationIfNecessary(InSettings.ValidationUsecase, !InSettings.bSilent);
	};

	UE::DataValidation::FValidationAssetRange ValidationRange(AssetDataList, InSettings, ShouldSkipAsset, OnAssetsLoaded);

	OutResults.NumSkipped += ValidationRange.GetNumSkipped();

	// Now add to map or update as needed
	for (UE::DataValidation::FAssetToValidate Asset : ValidationRange)
	{
		const FAssetData& Data = *Asset.Data;
		...
		// This asset is about to be validated, so remove it from the latent save validation list
		if (SavedPackagesToValidate.Num() > 0)
		{
			SavedPackagesToValidate.Remove(Data.PackageName);
			for (const FAssetData& ExternalObject : ValidationExternalObjects)
			{
				SavedPackagesToValidate.Remove(ExternalObject.PackageName);
			}
		}

		bool bWasLoadedForValidation = !Asset.bWasAlreadyLoaded;
		FDataValidationContext ValidationContext(bWasLoadedForValidation, InSettings.ValidationUsecase, ValidationExternalObjects, InSettings.MessageLogName);
		EDataValidationResult AssetResult = EDataValidationResult::NotValidated;
		if (!LoadedAsset)
		{
			UE::DataValidation::LogLoadingAsset(Data, InSettings.bUnloadAssetsLoadedForValidation);
			UE::DataValidation::FScopedLogMessageGatherer LogGatherer(InSettings.bCaptureAssetLoadLogs);
			UE_LOG_CONTEXT("AssetBeingValidated", Data.GetSoftObjectPath());
		...
		if (bWasLoadedForValidation)
		{
			ValidationContext.MarkAssetLoadedForValidation();

			if (InSettings.bUnloadAssetsLoadedForValidation)
			{
				ValidationRange.TrackPackageForUnload(LoadedAsset ? LoadedAsset->GetPackage() : nullptr);
			}
		}
	...
	int32 NumProcessed = ValidationRange.GetNumSkipped() + OutResults.NumChecked;
	if (OutResults.bAssetLimitReached)
	{
		ensureMsgf(NumProcessed <= AssetDataList.Num(),
			TEXT("Processed more assets than expected: %d skipped + %d checked > %d total"),
			ValidationRange.GetNumSkipped(), OutResults.NumChecked, AssetDataList.Num());
	}
	else
	{
		ensureMsgf(NumProcessed == AssetDataList.Num(),
			TEXT("Not all assets were processed: %d skipped + %d checked != %d total"),
			ValidationRange.GetNumSkipped(), OutResults.NumChecked, AssetDataList.Num());
	}

	ForEachEnabledValidator([&](UEditorValidatorBase* Validator)
	{
		Validator->PostAssetValidation(OutResults.ValidatorMessages);
		return true;
	});

	for (TSharedRef<FTokenizedMessage> const& Message : OutResults.ValidatorMessages)
	{
		DataValidationLog.AddMessage(Message);
	}
...
void UEditorValidatorSubsystem::ValidateOnSave(TArray<FAssetData> AssetDataList, bool bProceduralSave) const
{
	...
	Settings.MessageLogPageTitle = UE::DataValidation::MakeTimestampedMessageLogPageTitle(FText::Format(LOCTEXT("MessageLogPageTitle.ValidateSavedAssets", "Asset Save: {0}"), SavedAsset));

	const int32 NumAssetsWithIssues = ValidateAssetsWithSettings(AssetDataList, Settings, Results);

	if (NumAssetsWithIssues > 0 && !Settings.ShowMessageLogSeverity.IsSet()) // Only show the toast if ValidateAssetsWithSettings didn't already open the message log
	{
...
void UEditorValidatorSubsystem::ValidateSavedPackage(FName PackageName, bool bProceduralSave)
{
	...
	SavedPackagesToValidate.Add(PackageName);
}
...
void UEditorValidatorSubsystem::ValidateAllSavedPackages()
{
	...
	FARFilter Filter;
	Filter.PackageNames = SavedPackagesToValidate.Array();
...
void UEditorValidatorSubsystem::ValidateChangelistPreSubmit(
	...
	FSourceControlValidationDelegateResult& OutResult) const
{
	...
	Settings.MessageLogPageTitle = UE::DataValidation::MakeTimestampedMessageLogPageTitle(FText::Format(LOCTEXT("MessageLogPageTitle.ValidateChangelist", "Changelist Validation: {0}"), FText::FromString(InChangelist->GetIdentifier())));
	Settings.ShowMessageLogSeverity = { EMessageSeverity::Warning };
	Settings.bCollectPerAssetDetails = true;
	Settings.bShowIfNoFailures = false;

	FValidateAssetsResults Results;
	OutResult.Result = ValidateChangelist(Changelist, Settings, Results);
	
	for (const TPair<FString, FValidateAssetsDetails>& Pair : Results.AssetsDetails)
	{
		const FValidateAssetsDetails& Details = Pair.Value;

		OutResult.Warnings.Append(Details.ValidationWarnings);
		OutResult.Errors.Append(Details.ValidationErrors);
		
		for (TSharedRef<FTokenizedMessage> const& Message : Details.ValidationMessages)
		{
			OutResult.AllMessages.Add(Message->ToText());
		}
	}

	for (TSharedRef<FTokenizedMessage> const& Message : Results.ValidatorMessages)
	{
		OutResult.AllMessages.Add(Message->ToText());
	}
}
	...
	FSourceControlValidationDelegateResult& OutResult) const
{
	...
	Settings.ValidationUsecase = EDataValidationUsecase::PreSubmit;
	Settings.bLoadAssetsForValidation = GetDefault<UDataValidationSettings>()->bLoadAssetsWhenValidatingChangelists;
	Settings.MessageLogPageTitle = UE::DataValidation::MakeTimestampedMessageLogPageTitle(LOCTEXT("MessageLogPageTitle.ValidateFiles", "Files Validation"));
	Settings.ShowMessageLogSeverity = { EMessageSeverity::Warning };
	Settings.bCollectPerAssetDetails = true;
	Settings.bShowIfNoFailures = false;
	...
	FValidateAssetsResults& 					OutResults) const
{
	...
	{
		UE_CLOGF(FApp::IsUnattended(), LogContentValidation, Fatal, "Unable to perform unattended content validation while asset registry scan is in progress. Callers just wait for asset registry scan to complete.");
	...
	EDataValidationResult Result = EDataValidationResult::NotValidated;
	UE::Telemetry::DataValidation::FValidateAssets Telemetry(Settings);
	...
	for (UDataValidationChangelist* Changelist : Changelists)
	{
		...
		{
			FDataValidationContext ValidationContext(false, Settings.ValidationUsecase, {}, Settings.MessageLogName); // No associated objects for changelist
	...
	{
		FDataValidationContext ValidationContext(false, Settings.ValidationUsecase, {}, Settings.MessageLogName);
		for (auto It = AssetsToValidate.CreateIterator(); It; ++It)
		{
			if (!ShouldValidateAsset(*It, Settings, ValidationContext))
			{
				UE_LOGF(LogContentValidation, Display, "Excluding asset %ls from validation", *It->GetSoftObjectPath().ToString());
				It.RemoveCurrent();
			}
		}
		UE::DataValidation::AddAssetValidationMessages(DataValidationLog, ValidationContext);
	...
	FValidateAssetsResults& 		OutResults) const
{
	...
	ForEachEnabledValidator([this, InChangelist, &Settings, &InContext, &OutAssets, &OutResults](UEditorValidatorBase* Validator)
	{
		...
		for (const FAssetData& Asset : NewAssets)
		{
			...
			if (ShouldValidateAsset(Asset, Settings, InContext))
			{
				...
				UE_CLOGF(!bExisted, LogContentValidation, Display, "Asset validator %ls adding %ls to be validated from changelist %ls.", *Validator->GetPathName(), *Asset.GetSoftObjectPath().ToString(), *InChangelist->Description.ToString());
			}
		}
	...
	if (Settings.bValidateReferencersOfDeletedAssets)
	{
		...
		for (FName DeletedPackageName : InChangelist->DeletedPackageNames)
		{
			...
			{
				UE_LOGF(LogContentValidation, Display, "Adding %ls to to validated as it is a referencer of deleted asset %ls", *Referencer.ToString(), *DeletedPackageName.ToString());
				TArray<FAssetData> NewAssets;
...
{
	UE_CLOGF(!bHasRegisteredNativeValidators, LogContentValidation, Warning, "UpdateValidators request made before RegisterNativeValidators. Native validators may be missing!");
	UE_CLOGF(!bHasRegisteredBlueprintValidators, LogContentValidation, Warning, "UpdateValidators request made before RegisterBlueprintValidators. Blueprint validators may be missing!");
