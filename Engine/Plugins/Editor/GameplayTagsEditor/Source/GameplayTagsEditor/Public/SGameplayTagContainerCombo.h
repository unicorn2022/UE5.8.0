...
public:
	...
	// If bound returns optional color to use for this tag text. 
    DECLARE_DELEGATE_RetVal_OneParam(TOptional<FSlateColor>, FOnGetTagTextColor, const FGameplayTag& /*Tag*/);
	...
	{}
		...
		// Called to request option tag text color
		SLATE_EVENT(FOnGetTagTextColor, OnGetTagTextColor)
	SLATE_END_ARGS();
	...
	/** Returns true if the expanded edit UI is open */
	GAMEPLAYTAGSEDITOR_API bool IsOpen() const;

	/** Opens the expanded edit UI, optionally focusing on a specific tag */
	GAMEPLAYTAGSEDITOR_API void Open(FGameplayTag TagToFocus = { });
	
	GAMEPLAYTAGSEDITOR_API void RefreshTagContainers();
...
private:
	...
	FOnGetTagTextColor OnGetTagTextColor;
