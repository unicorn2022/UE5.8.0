...
void SGameplayTagContainerCombo::Construct(const FArguments& InArgs)
{
	...
	OnGetTagTextColor = InArgs._OnGetTagTextColor;
...
TSharedRef<ITableRow> SGameplayTagContainerCombo::MakeTagListViewRow(TSharedPtr<FEditableItem> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	if (OnGetTagTextColor.IsBound())
	{
		TextColor = OnGetTagTextColor.Execute(Item->Tag);
	}
...
bool SGameplayTagContainerCombo::IsOpen() const
{
	return ComboButton.IsValid() && ComboButton->IsOpen();
}

void SGameplayTagContainerCombo::Open(const FGameplayTag TagToFocus)
{
	TagToHilight = TagToFocus;
