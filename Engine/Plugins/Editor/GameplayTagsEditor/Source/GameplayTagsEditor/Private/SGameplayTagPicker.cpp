...
void SGameplayTagPicker::Construct(const FArguments& InArgs)
{
	...
	IGameplayTagsModule::OnGameplayTagTreeChanged.AddSP(this, &SGameplayTagPicker::RefreshOnNextTick);
		...
		[
			...
			[
				...
				[
					...
					.HintText(LOCTEXT("GameplayTagPicker_SearchBoxHint", "Search tags matching ALL keywords"))
...
{
	InFilterText.ToString().ParseIntoArrayWS(FilterStringTokens);
	FilterTagTree();
}
...
bool SGameplayTagPicker::FilterChildrenCheck(TSharedPtr<FGameplayTagNode>& InItem) const
{
	...
	if (!FilterStringTokens.IsEmpty())
	{
		const FString CompleteTagString = InItem->GetCompleteTagString();
		for (const FString& Token : FilterStringTokens)
		{
			if (!CompleteTagString.Contains(Token))
			{
				return FilterChildrenCheckRecursive(InItem);
			}
		}
	...
	// All tokens matched (or there are no search tokens)
	return true;
}

FText SGameplayTagPicker::GetHighlightText() const
{
	return FilterStringTokens.IsEmpty() ? FText::GetEmpty() : FText::FromString(FilterStringTokens.Last());
}
