...
public:
	...
	virtual void StartupModule() override
	{
		...
		FCoreDelegates::GetOnPostEngineInit().AddRaw(this, &FGameplayTagsEditorModule::OnPostEngineInit);
	...
	void ShowNotification(const FText& TextToDisplay, float TimeToDisplay, bool bLogError = false, bool bOnlyLog = false)
	{
		if (bOnlyLog)
		{
			if (bLogError)
			{
				UE_LOGF(LogGameplayTags, Error, "%ls", *TextToDisplay.ToString())
			}
			else
			{
				UE_LOGF(LogGameplayTags, Display, "%ls", *TextToDisplay.ToString())
			}
		}
		else
		{
	...
	virtual bool MoveTagsBetweenINI(const TArray<FString>& TagsToMove, const FName& TargetTagSource, TArray<FString>& OutTagsMoved, TArray<FString>& OutFailedToMoveTags) override
	{
		...
		for (const FString& TagToMove : TagsToMove)
		{
			...
			{
				UE_LOGF(LogGameplayTags, Display, "%d tag sources found for tag %ls. Moving first found ini source only! (DefaultTagList or TagList)", TagSourceNames.Num(), *TagToMove);
			}
