...
public:
	...
	// Sets snapping on/off, rather than the explicit type to enable/disable
	void SetSnappingActive(bool bInEnabled)
	{
		bSnappingActive = bInEnabled;
	}

	bool IsSnappingActive() const
	{
		return bSnappingActive;
	}
	...
	// Whether to perform the snapping, not just get it's checkbox state
	bool IsSnapActive(ESnapTypeFlag SnapMode) const
	{
		return bSnappingActive && GetSnapEnabled(SnapMode);
	}
...
protected:
	...
	// Toggle for all snapping, can be toggled so that whatever options were stored aren't cleared on enable (vs. ESnapTypeFlag::NoSnap)
