	...
	const TWeakPtr<FConsoleVariablesEditorListRow> InRow)
{
	...
	PinnedItem->OnFlashRequested.AddSP(this, &SConsoleVariablesEditorListRow::FlashRow);
}

FReply SConsoleVariablesEditorListRow::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (MouseEvent.GetModifierKeys().IsShiftDown())
	{
		const FConsoleVariablesEditorListRowPtr PinnedItem = Item.Pin();
		const TSharedPtr<SConsoleVariablesEditorList> ListView =
			PinnedItem.IsValid() ? PinnedItem->GetListViewPtr().Pin() : nullptr;
		if (ListView.IsValid())
		{
			if (MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
			{
				ListView->CopySelectedValues();
				FlashRow();
				return FReply::Handled();
			}
			if (MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
			{
				ListView->PasteValueToSelectedVariable();
				FlashRow();
				return FReply::Handled();
			}
		}
	...
	return SMultiColumnTableRow<FConsoleVariablesEditorListRowPtr>::OnMouseButtonUp(MyGeometry, MouseEvent);
}
