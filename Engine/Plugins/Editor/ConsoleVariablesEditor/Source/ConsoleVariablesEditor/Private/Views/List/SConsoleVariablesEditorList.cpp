...
void SConsoleVariablesEditorList::Construct(const FArguments& InArgs, TSharedRef<FConsoleVariablesEditorList> ListModel)
{
	...
	[
		...
		[
			...
			[
				...
				.OnContextMenuOpening(this, &SConsoleVariablesEditorList::OnTreeViewContextMenuOpening)
			...
			[
				SAssignNew(EmptyAreaBorder, SBorder)
	...
	if (EmptyAreaBorder.IsValid())
	{
		EmptyAreaBorder->SetOnMouseButtonUp(
			FPointerEventHandler::CreateSP(this, &SConsoleVariablesEditorList::OnEmptyAreaMouseButtonUp));
	}
...
FReply SConsoleVariablesEditorList::TryEnterGlobalSearch(const FString& SearchString)
{
	...
	{
		UE_LOGF(LogConsoleVariablesEditor, Verbose,
			"%s: Global search request is empty. Exiting Global Search.",
			__FUNCTION__);
	...
	if (!bFoundMatches)
	{
		UE_LOGF(LogConsoleVariablesEditor, Warning,
			"%s: Failed to find console variable objects with names containing search string %ls",
				...
				.OnCheckStateChanged_Lambda([this] (const ECheckBoxState NewState)
				{
					...
					for (const FConsoleVariablesEditorListRowPtr& Object : TreeViewRootObjects)
					{
							...
							FConsoleVariablesEditorCommandInfo::EConsoleObjectType::Variable)
						{
							...
							{
								Object->ResetToStartupValueAndSource();
							}
							else if (NewState == ECheckBoxState::Checked)
							{
								Object->ResetToPresetValue();
							}
						}
					}
				})
...
{	
	...
	{
		UE_LOGF(LogConsoleVariablesEditor, VeryVerbose, "%s: Considering %ls for the list",
		...
		if (CommandInfo.IsValid())
		{
			...
			{
				UE_LOGF(LogConsoleVariablesEditor, VeryVerbose, "%s: Not Adding %ls to the list because it's already in the list",
					__FUNCTION__, *SavedCommand.CommandName);
				continue;
			}
			...
			{
				UE_LOGF(LogConsoleVariablesEditor, VeryVerbose, "%s: Adding %ls to the list from cache",
					__FUNCTION__, *SavedCommand.CommandName);
			...
			if (RowToAdd)
			{
				...
				UE_LOGF(LogConsoleVariablesEditor, VeryVerbose, "%s: Adding %ls to the list with a new row item",
					__FUNCTION__, *SavedCommand.CommandName);
...
FReply SConsoleVariablesEditorList::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	if (InKeyEvent.IsControlDown())
	{
		if (InKeyEvent.GetKey() == EKeys::C)
		{
			CopySelectedNamesAndValues();
			FlashSelectedRows();
...
void SConsoleVariablesEditorList::FlashSelectedRows()
{
	for (const FConsoleVariablesEditorListRowPtr& Row : TreeViewPtr->GetSelectedItems())
	{
		if (Row.IsValid())
		{
			Row->RequestFlash();
...
void SConsoleVariablesEditorList::FlashRowByCommand(const FString& InCommandName)
{
	for (const FConsoleVariablesEditorListRowPtr& Row : TreeViewRootObjects)
	{
		if (!Row.IsValid())
		...
		const TSharedPtr<FConsoleVariablesEditorCommandInfo> RowCommand = Row->GetCommandInfo().Pin();
		if (RowCommand.IsValid() && RowCommand->Command.Equals(InCommandName, ESearchCase::IgnoreCase))
		{
			Row->RequestFlash();
			break;
		}
...
void SConsoleVariablesEditorList::CopySelectedVariableNames()
{
	const TArray<FConsoleVariablesEditorListRowPtr> SelectedItems = TreeViewPtr->GetSelectedItems();
	...
	for (const FConsoleVariablesEditorListRowPtr& Row : SelectedItems)
	{
		if (!Row.IsValid())
		{
			continue;
		...
		const TSharedPtr<FConsoleVariablesEditorCommandInfo> CommandInfo = Row->GetCommandInfo().Pin();
		if (!CommandInfo.IsValid())
		{
			continue;
		}
		...
		ClipboardText += CommandInfo->Command;
	}
...
void SConsoleVariablesEditorList::CopySelectedNamesAndValues()
{
	const TArray<FConsoleVariablesEditorListRowPtr> SelectedItems = TreeViewPtr->GetSelectedItems();
	if (SelectedItems.Num() == 0)
	{
	...
	for (const FConsoleVariablesEditorListRowPtr& Row : SelectedItems)
	{
		...
		FString Value;
		if (!CommandInfo->GetCurrentValueAsString(Value))
		{
			Value = Row->GetCachedValue();
		}
		...
		ClipboardText += FString::Printf(TEXT("%s %s"), *CommandInfo->Command, *Value);
	}
...
void SConsoleVariablesEditorList::CopySelectedValues()
{
	const TArray<FConsoleVariablesEditorListRowPtr> SelectedItems = TreeViewPtr->GetSelectedItems();
	if (SelectedItems.Num() == 0)
	{
...
void SConsoleVariablesEditorList::PasteValueToSelectedVariable()
{
	const TArray<FConsoleVariablesEditorListRowPtr> SelectedItems = TreeViewPtr->GetSelectedItems();
	...
	const FConsoleVariablesEditorListRowPtr& Row = SelectedItems[0];
	if (!Row.IsValid() || !Row->IsRowChecked())
	{
		return;
	}
	...
	ClipboardText = ClipboardText.TrimStartAndEnd();
	...
	CommandInfo->ExecuteCommand(ClipboardText);
	Row->SetCachedValue(ClipboardText);
	Row->RequestFlash();
	FindVisibleObjectsAndRequestTreeRefresh();
}

void SConsoleVariablesEditorList::PasteNameAndValue()
{
	FString ClipboardText;
	FPlatformApplicationMisc::ClipboardPaste(ClipboardText);
	...
	for (const FString& Line : Lines)
	{
		...
		FString CommandName;
		FString Value;
		if (!TrimmedLine.Split(TEXT(" "), &CommandName, &Value))
		{
			UE_LOG(LogConsoleVariablesEditor, Error,
				TEXT("Paste Name and Value: Failed to parse '%s' as a console variable command. Expected format: 'VariableName Value'"),
				*TrimmedLine);
			continue;
		}
		...
		if (CommandName.IsEmpty() || Value.IsEmpty())
		{
			UE_LOG(LogConsoleVariablesEditor, Error,
				TEXT("Paste Name and Value: Failed to parse '%s' as a console variable command. Expected format: 'VariableName Value'"),
				*TrimmedLine);
		...
		TSharedPtr<FConsoleVariablesEditorCommandInfo> CommandInfo =
			FConsoleVariablesEditorModule::Get().FindCommandInfoByName(CommandName).Pin();

		if (!CommandInfo.IsValid())
		{
			UE_LOG(LogConsoleVariablesEditor, Error,
				TEXT("Paste Name and Value: Console variable '%s' not found."),
				*CommandName);
			continue;
		}

		CommandInfo->ExecuteCommand(Value);
		bAppliedAny = true;

		bool bFoundExistingRow = false;
		for (const FConsoleVariablesEditorListRowPtr& Row : TreeViewRootObjects)
		{
			if (Row.IsValid() && Row->GetCommandInfo().IsValid())
			{
				const TSharedPtr<FConsoleVariablesEditorCommandInfo> RowCommand = Row->GetCommandInfo().Pin();
				if (RowCommand.IsValid() && RowCommand->Command.Equals(CommandName, ESearchCase::IgnoreCase))
				{
					Row->SetCachedValue(Value);
					Row->RequestFlash();
					bFoundExistingRow = true;
					break;
				}
			}
		}

		// If the variable isn't already in the preset list, add it. The list will rebuild and
		// the new row will flash on scroll-into-view via SetShouldFlashOnScrollIntoView.
		if (!bFoundExistingRow)
		{
			const FString CommandLine = FString::Printf(TEXT("%s %s"), *CommandName, *Value);
			FConsoleVariablesEditorModule::Get().ValidateConsoleInputAndAddToCurrentPreset(
				FText::FromString(CommandLine));
		}
	}
...
TSharedPtr<SWidget> SConsoleVariablesEditorList::OnTreeViewContextMenuOpening()
{
	return BuildContextMenuWidget();
}

TSharedRef<SWidget> SConsoleVariablesEditorList::BuildContextMenuWidget()
{
	const TArray<FConsoleVariablesEditorListRowPtr> SelectedItems = TreeViewPtr->GetSelectedItems();
	const bool bHasSelection = SelectedItems.Num() > 0;

	bool bCanPasteValue = false;
	if (SelectedItems.Num() == 1)
	{
		const FConsoleVariablesEditorListRowPtr& Row = SelectedItems[0];
		if (Row.IsValid() && Row->IsRowChecked())
		{
			const TSharedPtr<FConsoleVariablesEditorCommandInfo> CommandInfo = Row->GetCommandInfo().Pin();
			bCanPasteValue = CommandInfo.IsValid() &&
				CommandInfo->ObjectType == FConsoleVariablesEditorCommandInfo::EConsoleObjectType::Variable;
		}
	}

	const TAttribute<FText> CopyValueChord =
		FInputChord(EModifierKey::Shift, EKeys::RightMouseButton).GetInputText(false);
	const TAttribute<FText> PasteValueChord =
		FInputChord(EModifierKey::Shift, EKeys::LeftMouseButton).GetInputText(false);
	const TAttribute<FText> CopyNameAndValueChord =
		FInputChord(EModifierKey::Control, EKeys::C).GetInputText(false);
	const TAttribute<FText> PasteNameAndValueChord =
		FInputChord(EModifierKey::Control, EKeys::V).GetInputText(false);

	FMenuBuilder MenuBuilder(true, nullptr);

	MenuBuilder.BeginSection("Edit", LOCTEXT("EditSectionHeading", "Edit"));
	{
		MenuBuilder.AddMenuEntry(
			LOCTEXT("CopyValue", "Copy Value"),
			LOCTEXT("CopyValueTooltip", "Copy the selected console variable value(s) to the clipboard"),
		...
		MenuBuilder.AddMenuEntry(
			...
			LOCTEXT("PasteValueTooltip", "Paste the clipboard value into the selected console variable"),
			FSlateIcon(FAppStyle::GetAppStyleSetName(), "GenericCommands.Paste"),
			FUIAction(
				FExecuteAction::CreateSP(this, &SConsoleVariablesEditorList::PasteValueToSelectedVariable),
				FCanExecuteAction::CreateLambda([bCanPasteValue]() { return bCanPasteValue; })
			),
			NAME_None,
			EUserInterfaceActionType::Button,
			NAME_None,
		...
		MenuBuilder.AddMenuEntry(
			LOCTEXT("CopyName", "Copy Console Variable Name"),
			LOCTEXT("CopyNameTooltip", "Copy the selected console variable name(s) to the clipboard"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this]()
				{
					CopySelectedVariableNames();
					FlashSelectedRows();
				}),
				FCanExecuteAction::CreateLambda([bHasSelection]() { return bHasSelection; })
			)
		...
		MenuBuilder.AddMenuEntry(
			LOCTEXT("CopyNameAndValue", "Copy Console Variable Name And Value"),
			LOCTEXT("CopyNameAndValueTooltip", "Copy the selected console variable(s) as 'Name Value' to the clipboard"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([this]()
				{
			...
			EUserInterfaceActionType::Button,
			NAME_None,
			CopyNameAndValueChord
		);

		// Paste Name and Value is always enabled — it can target an existing row OR add a new
		// variable to the preset, including when the list is empty or the user right-clicks
		// the table background.
		MenuBuilder.AddMenuEntry(
			LOCTEXT("PasteNameAndValue", "Paste Name and Value"),
			LOCTEXT("PasteNameAndValueTooltip", "Paste 'Name Value' from the clipboard and apply, adding the variable to the preset if it isn't already present"),
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &SConsoleVariablesEditorList::PasteNameAndValue)),
			NAME_None,
			EUserInterfaceActionType::Button,
			NAME_None,
			PasteNameAndValueChord
		);
	}
...
FReply SConsoleVariablesEditorList::OnEmptyAreaMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	const TSharedRef<SWidget> MenuWidget = BuildContextMenuWidget();
