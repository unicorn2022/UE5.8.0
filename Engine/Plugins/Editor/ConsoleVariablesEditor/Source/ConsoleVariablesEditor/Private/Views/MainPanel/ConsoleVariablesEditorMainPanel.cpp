...
FReply FConsoleVariablesEditorMainPanel::ValidateConsoleInputAndAddToCurrentPreset(const FText& CommittedText) const
{
	...
	{
		UE_LOGF(LogConsoleVariablesEditor, Warning, "%s: Input is blank.", __FUNCTION__);
	...
	else
	{
		UE_LOGF(LogConsoleVariablesEditor, Warning,
			"%s: Input '%ls' is not a recognized console variable or command.",
...
void FConsoleVariablesEditorMainPanel::OnConnectionChanged(EConcertConnectionStatus Status)
{
	switch (Status)
	{
		case EConcertConnectionStatus::Connected:
			UE_LOGF(LogConsoleVariablesEditor, VeryVerbose, "Multi-user has connected to a session.");
			break;
		case EConcertConnectionStatus::Disconnected:
			UE_LOGF(LogConsoleVariablesEditor, VeryVerbose, "Multi-user has disconnected from session.");
			break;
		default:
			break;
	}
...
void FConsoleVariablesEditorMainPanel::OnRemoteListItemCheckStateChange(const FString InName, ECheckBoxState InCheckedState)
{
	...
	UE_LOGF(LogConsoleVariablesEditor, Display,
		"%s: Remote check state change for list item with name '%ls' set to '%ls'", __FUNCTION__, *InName, *CheckStateAsString);
}
