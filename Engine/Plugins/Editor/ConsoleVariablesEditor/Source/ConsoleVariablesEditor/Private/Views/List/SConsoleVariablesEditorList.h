...
public:
	...
	// Copy/Paste — public so row widgets can invoke on Shift+RMB / Shift+LMB

	void CopySelectedVariableNames();
	void CopySelectedNamesAndValues();
	void CopySelectedValues();
	void PasteValueToSelectedVariable();
	void PasteNameAndValue();
...
private:
	...
	TSharedPtr<SWidget> OnTreeViewContextMenuOpening();
	TSharedRef<SWidget> BuildContextMenuWidget();
	FReply OnEmptyAreaMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent);

	/** Flash all currently selected rows, e.g. after a Ctrl+C / Ctrl+V hotkey. */
	void FlashSelectedRows();
	/** Flash a row whose command matches InCommandName (case-insensitive). */
	void FlashRowByCommand(const FString& InCommandName);

	TSharedPtr<SBorder> EmptyAreaBorder;
