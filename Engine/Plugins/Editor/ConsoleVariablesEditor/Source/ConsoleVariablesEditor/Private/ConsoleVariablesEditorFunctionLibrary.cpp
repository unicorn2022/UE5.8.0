...
void UConsoleVariablesEditorFunctionLibrary::LoadPresetIntoConsoleVariablesEditor(const UConsoleVariablesAsset* InAsset, const EConsoleVariablesEditorPresetImportMode InImportMode)
{
	if (!InAsset)
	{
		UE_LOGF(LogConsoleVariablesEditor, Error,
			"%s: InAsset was nullptr, please verify that InAsset is valid.",
			__FUNCTION__);
		return;
	}
 	...
 	const UConsoleVariablesAsset* InAsset, TArray<FString>& OutCommandList)
{
	...
	{
		UE_LOGF(LogConsoleVariablesEditor, Error,
			"%s: InAsset has no saved commands. Please verify you're using the right asset.",
			__FUNCTION__);
		return false;
	}
	...
	const FString InValue)
{
		...
		ConsoleVariablesEditorModule.FindCommandInfoByName(InCommandName); CommandInfo.IsValid())
	{
		...
		UE_LOGF(LogConsoleVariablesEditor, Error,
			"%s: Command %ls is not a variable type. Please do not enter console commands, only console variables.",
			__FUNCTION__, *InCommandName);
	}
	else
	{
		UE_LOGF(LogConsoleVariablesEditor, Error,
			"%s: FConsoleVariablesEditorCommandInfo was not foundwith given name: %ls.",
			__FUNCTION__, *InCommandName);
	}
	return false;
}
...
bool UConsoleVariablesEditorFunctionLibrary::GetConsoleVariableSourceByName(const FString InCommandName, FString& OutValue)
{
	...
	UE_LOGF(LogConsoleVariablesEditor, Error,
		"%s: FConsoleVariablesEditorCommandInfo was not found with given name: %ls.",
		__FUNCTION__, *InCommandName);
	return false;
}
...
bool UConsoleVariablesEditorFunctionLibrary::GetEnableMultiUserCVarSync()
{
	...
	UE_LOGF(LogConsoleVariablesEditor, Error,
		"%s: UConcertCVarSynchronization was not found, please verify the Multi-user plugin is enabled and this object class is compiled.",
		__FUNCTION__);
	return false;
}
