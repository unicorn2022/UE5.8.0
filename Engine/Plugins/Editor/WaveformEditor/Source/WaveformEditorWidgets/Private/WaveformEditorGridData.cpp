...
const int64 FWaveformEditorGridData::GetFramePositionFromRatio(double Ratio) const
{
	const int64 DisplayedFrames = DisplayRange.GetUpperBoundValue() - DisplayRange.GetLowerBoundValue();

	const int64 FramePosition = FMath::RoundToInt64(Ratio * DisplayedFrames) + DisplayRange.GetLowerBoundValue();

	return FramePosition;
}

const double FWaveformEditorGridData::GetRatioFromFramePosition(int64 FramePosition) const
{
	if (!IsFramePositionInDisplayRange(FramePosition))
	{
		return -1; // Invalid ratio as we are now offscreen
	}

	const int64 PositionInDisplay = FramePosition - DisplayRange.GetLowerBoundValue();
	const double Range = DisplayRange.Size<double>();

	// A display range of 0 should not be possible as you would not be able to see anything in waveform editor
	check(Range > 0);

	return PositionInDisplay / Range;
}

const bool FWaveformEditorGridData::IsFramePositionInDisplayRange(int64 FramePosition) const
{
	return FramePosition >= DisplayRange.GetLowerBoundValue() && FramePosition <= DisplayRange.GetUpperBoundValue();
}
