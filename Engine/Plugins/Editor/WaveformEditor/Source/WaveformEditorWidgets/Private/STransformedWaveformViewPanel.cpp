...
void STransformedWaveformViewPanel::SetPlayheadRatio(const double InRatio)
{
	check(InRatio >= 0.0 && InRatio <= 1.0);
	CachedPlayheadRatio = InRatio;
	check(GridData);
	CachedPlayheadFramePosition = GridData->GetFramePositionFromRatio(InRatio);
}

void STransformedWaveformViewPanel::SetPlayheadFramePosition(const int64 FramePosition)
{
	CachedPlayheadFramePosition = FramePosition;
}

void STransformedWaveformViewPanel::ResolveFromFramePosition()
{
	check(GridData);
	CachedPlayheadRatio = GridData->GetRatioFromFramePosition(CachedPlayheadFramePosition);
}
...
void STransformedWaveformViewPanel::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (PlayheadOverlay && GridData)
	{
		PlayheadOverlay->SetPlayheadVisible(GridData->IsFramePositionInDisplayRange(CachedPlayheadFramePosition));
	}

	if (TimeRuler && GridData)
	{
		TimeRuler->SetPlayheadVisible(GridData->IsFramePositionInDisplayRange(CachedPlayheadFramePosition));
	}
...
{
	// If the frame position is not visible then the overlays will not render and do not need to update
	if (GridData != nullptr && !GridData->IsFramePositionInDisplayRange(CachedPlayheadFramePosition))
	{
		return;
	}
