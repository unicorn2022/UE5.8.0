...
public:
	...
	UE_API virtual bool CloseWindow(EAssetEditorCloseReason InCloseReason) override;
	...
	UE_API void ExportWaveform(bool bIsAsync);

	// Skips the playhead to the next marker
	UE_API void SkipToNextMarker();
	UE_API void ResetPlaybackToStart();

	TSharedPtr<class FSparseSampledSequenceTransportCoordinator> GetTransportCoordinator() { return TransportCoordinator; }
	const int64 GetPlayheadFramePosition() const { return WaveformView.ViewWidget.IsValid() ? WaveformView.ViewWidget->GetPlayheadFramePosition() : 0; }
...
private:
	...
	UE_API void SkipToNextZeroCrossing();
	...
	// SoundWave->PostEditChange() calls UpdateAsset() which kills all playback of the sound asset.
	// Calling SoundWave->PostEditChangeProperty(PropertyChangedEvent) with a valid property event avoids this.
	UE_API void NotifySoundWavePostEditChangeProperty(const FName& PropertyName, const EPropertyChangeType::Type PropertyChangeType = EPropertyChangeType::ValueSet);
	...
	// Defer UpdateRenderElements onto a single ticker fire so heavy regen doesn't run every mouse-move.
	bool bDeferredRenderUpdatePending = false;
	// Handle to the in-flight one-shot ticker, kept so a non-interactive supersede or the destructor can cancel the pending fire.
	FTSTicker::FDelegateHandle DeferredRenderTickerHandle;
	...
	TSubclassOf<UTransformationFadeFunction> FadeInType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Exponential];

	static constexpr float DefaultFadeOutAmount = 0.5f;
	float CachedFadeOutAmount = 0.f;
	TSubclassOf<UTransformationFadeFunction> FadeOutType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Exponential];
	...
	/** Set to true when the editor is being closed because the SoundWave asset is being deleted or invalidated.
	 *  Guards against unsafe SoundWave modification in the destructor cleanup path. */
	bool bSoundWaveBeingDeleted = false;
};
