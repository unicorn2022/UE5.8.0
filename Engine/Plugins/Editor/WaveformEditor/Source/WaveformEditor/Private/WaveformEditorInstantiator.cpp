...
{
	UE_LOGF(LogWaveformEditor, Warning, "%ls", *ErrorMessage.ToString())
...
void FWaveformEditorInstantiator::CreateWaveformEditor(TArray<USoundWave*> SoundWavesToEdit)
{
	for (USoundWave* SoundWavePtr : SoundWavesToEdit)
	{
		if (CanSoundWaveBeOpenedInEditor(SoundWavePtr))
		{
			...
			{
				UE_LOGF(LogWaveformEditor, Warning, "Could not open waveform editor for soundwave %ls, initialization failed", *(SoundWavePtr->GetName()));
