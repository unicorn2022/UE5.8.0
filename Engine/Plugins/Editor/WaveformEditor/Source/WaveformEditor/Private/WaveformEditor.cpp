...
bool FWaveformEditor::Init(const EToolkitMode::Type Mode, const TSharedPtr<IToolkitHost>& InitToolkitHost, USoundWave* SoundWaveToEdit)
{
	...
	// If either condition is zero it is an indication of a zero length or corrupt file.
	// FWaveformEditorInstantiator::CanSoundWaveBeOpenedInEditor already prevents zero-duration waves from opening through the normal route, added guard here to block in other scenarios (e.g. automated testing)
	if (FMath::IsNearlyZero(SoundWave->GetDuration()) || FMath::IsNearlyZero(SoundWave->TotalSamples))
	{
		UE_LOGF(LogWaveformEditor, Warning, "Waveform Editor attempted to open Zero-Length SoundWave: %ls", *SoundWave->GetName());
	...
	// Must be initialized whether init succeeds or fails for proper destruction
	...
	if (bIsInitialized)
	{
		...
		{
			// Update the properties without marking the SoundWave dirty to prevent checkouts unintended by the user
			const bool bWasAlreadyDirty = SoundWave->GetPackage()->IsDirty();
...
bool FWaveformEditor::CloseWindow(EAssetEditorCloseReason InCloseReason)
{
	if (InCloseReason == EAssetEditorCloseReason::CloseAllEditorsForAsset
		|| InCloseReason == EAssetEditorCloseReason::AssetUnloadingOrInvalid
		|| InCloseReason == EAssetEditorCloseReason::AssetForceDeleted)
	{
		bSoundWaveBeingDeleted = true;
	}
...
{
	// Cancel any in-flight deferred render tick — its lambda holds a weak ptr to this editor so it would otherwise still fire and operate on an object that's mid-destruction.
	bDeferredRenderUpdatePending = false;
	FTSTicker::GetCoreTicker().RemoveTicker(DeferredRenderTickerHandle);
	DeferredRenderTickerHandle.Reset();
	...
	if (MarkerTransformation)
	{
		...
		// Skip modifying the SoundWave if it is being deleted — modifications are invalid during asset deletion
		if (!bSoundWaveBeingDeleted && TransformationChainConfig.bIsPreviewingLoopRegion)
		{
			...
			NotifySoundWavePostEditChangeProperty(GET_MEMBER_NAME_CHECKED(USoundWave, bLooping));
		}
	}

	TObjectPtr<UWaveformTransformationTrimFade> TrimFadeTransformation = bSoundWaveBeingDeleted ? nullptr : GetTrimFadeTransformation();
	...
	if (TrimFadeTransformation)
	{
		...
		check(TrimFadeTransformation != nullptr);
		check(TrimFadeTransformation->FadeTransformation != nullptr);
		...
		{
			if (TrimFadeTransformation->FadeTransformation->GetFadeRegionsNum() != 1
				|| (TrimFadeTransformation->GetFadeRegion().FadeIn && FMath::IsNearlyEqual(TrimFadeTransformation->GetFadeRegion().FadeIn->Duration, 0, ErrorTolerance)
					&& TrimFadeTransformation->GetFadeRegion().FadeOut && FMath::IsNearlyEqual(TrimFadeTransformation->GetFadeRegion().FadeOut->Duration, 0, ErrorTolerance))
				|| (TrimFadeTransformation->GetFadeRegion().FadeIn == nullptr && TrimFadeTransformation->GetFadeRegion().FadeOut == nullptr))
			{
				// If the number of Fade Regions is invalid
				// Or if a Fade Region is present and both fades have a zero duration or if both fades are null, remove the transformation
	...
	if (SoundWave->Transformations.Num() == 0)
	{
		...
		for (const TSubclassOf<UWaveformTransformationBase>& TransformationClass : TransformationsSettings->LaunchTransformations)
		{
			...
			if (TransformationClass)
			{
				...
				{
					UE_LOGF(LogWaveformEditor, Warning, "UWaveformTransformationTrimFade is already added automatically and removed if unused. Remove it from LaunchTransformations TSet.");
			...
			NotifySoundWavePostEditChangeProperty(GET_MEMBER_NAME_CHECKED(USoundWave, Transformations), EPropertyChangeType::ArrayAdd);
		}
...
void FWaveformEditor::NotifyPostTransformationChange(const EPropertyChangeType::Type& PropertyChangeType)
{
	...
	//Update the content browser asset state to dirty
	NotifySoundWavePostEditChangeProperty(GET_MEMBER_NAME_CHECKED(USoundWave, Transformations), PropertyChangeType);
...
TObjectPtr<UWaveformTransformationTrimFade> FWaveformEditor::GetOrAddTrimFadeTransformation(const bool bMarkModify)
{
	...
	if (TrimFadeTransformation == nullptr)
	{
		...
		check(TrimFadeTransformation->FadeTransformation);
		check(TrimFadeTransformation->FadeTransformation->GetFadeRegionsNum() == 1);
		// Add default fade curves
		TrimFadeTransformation->GetMutableFadeRegion().FadeIn = NewObject<UTransformationFadeFunction>(TrimFadeTransformation->FadeTransformation.Get(), FadeInType.Get(), NAME_None, RF_Transactional);
		check(TrimFadeTransformation->GetFadeRegion().FadeIn);
		TrimFadeTransformation->GetMutableFadeRegion().FadeOut = NewObject<UTransformationFadeFunction>(TrimFadeTransformation->FadeTransformation.Get(), FadeOutType.Get(), NAME_None, RF_Transactional);
		check(TrimFadeTransformation->GetFadeRegion().FadeOut);

		TrimFadeTransformation->GetFadeRegion().FadeIn->Duration = 0.f;
		TrimFadeTransformation->GetFadeRegion().FadeOut->Duration = 0.f;
...
void FWaveformEditor::ToggleFadeIn()
{
	...
	const FTransformationFadeFunctionData& ConstFadeRegion = TrimFadeTransformation->GetFadeRegion();

	if (ConstFadeRegion.FadeIn)
	{
		...
		ConstFadeRegion.FadeIn->Modify();
		
		if (ConstFadeRegion.FadeIn->Duration == 0.0f)
		{
			...
			if (!ConstFadeRegion.FadeIn.IsA(FadeInType.Get()))
			{
				FTransformationFadeFunctionData& MutableFadeRegion = TrimFadeTransformation->GetMutableFadeRegion();
				MutableFadeRegion.FadeIn = NewObject<UTransformationFadeFunction>(TrimFadeTransformation->FadeTransformation.Get(), FadeInType.Get(), NAME_None, RF_Transactional);
				MutableFadeRegion.FadeIn->Modify();

				ensure(&ConstFadeRegion == &MutableFadeRegion);
			}

			ConstFadeRegion.FadeIn->Duration = CachedFadeInAmount;
		}
		else //if fade duration is not 0, assume there is a fade applied and toggle fade "off" by setting fade time to 0
		{
			...
			CachedFadeInAmount = ConstFadeRegion.FadeIn->Duration;
			...
			ConstFadeRegion.FadeIn->Duration = 0.0f;
		}
...
void FWaveformEditor::ToggleFadeOut()
{
	...
	if (ConstFadeRegion.FadeOut)
	{
		...
		if (ConstFadeRegion.FadeOut->Duration == 0.0f)
		{
			...
			// Ensures the selected subtype (linear, exponential, logarithmic, or sigmoid)
			if (!ConstFadeRegion.FadeOut.IsA(FadeOutType.Get()))
			{
				FTransformationFadeFunctionData& MutableFadeRegion = TrimFadeTransformation->GetMutableFadeRegion();
				MutableFadeRegion.FadeOut = NewObject<UTransformationFadeFunction>(TrimFadeTransformation->FadeTransformation.Get(), FadeOutType.Get(), NAME_None, RF_Transactional);
				MutableFadeRegion.FadeOut->Modify();

				ensure(&ConstFadeRegion == &MutableFadeRegion);
			}

			ConstFadeRegion.FadeOut->Duration = CachedFadeOutAmount;
		}
		else //if fade duration is not 0, assume there is a fade applied and toggle fade "off" by setting fade time to 0
		{
			// No need to check for fade type since we are toggling it "off". Type check is done during toggle "on"
			CachedFadeOutAmount = ConstFadeRegion.FadeOut->Duration;

			//No need to set FadeCurve when toggling off. This will preserve user curve data if they later manually change the StartFadeTime.
			ConstFadeRegion.FadeOut->Duration = 0.0f;
		}
...
void FWaveformEditor::CreateMarker(bool bIsLoopRegion)
{
	...
	NewCuePoint.FramePosition = WaveformView.ViewWidget->GetPlayheadFramePosition();
...
void FWaveformEditor::SkipToNextMarker()
{
	...
	if (TObjectPtr<UWaveformTransformationMarkers> MarkerTransformation = GetOrAddMarkerTransformation())
	{
		...
		const int64 PlaybackFramePosition = WaveformView.ViewWidget->GetPlayheadFramePosition();
		...
		if (NewPlaybackFramePosition != MAX_int64)
		{
			...
			if (ActiveDuration > 0)
			{
				WaveformView.ViewWidget->SetPlayheadFramePosition(NewPlaybackFramePosition);
				WaveformView.ViewWidget->ResolveFromFramePosition();
			}
		}
	}
}

void FWaveformEditor::SkipToNextZeroCrossing()
{
	check(SoundWave != nullptr);

	const TArray<float>& PCMArray = WaveformView.DataProvider->GetTransformedPCMData();
	const int64 StartFrame = WaveformView.ViewWidget->GetPlayheadFramePosition();

	float EndTime = TransformationChainConfig.EndTime;
	if (EndTime < 0)
	{
		EndTime = SoundWave->Duration;
	}

	check(TransformationChainConfig.SampleRate > 0);
	const int64 EndFrame = FMath::Max(EndTime * TransformationChainConfig.SampleRate - 1, 0);
	...
	check(SoundWave->NumChannels > 0);

	const int64 FramesToMove = UWaveformTransformationMarkers::GetFramesToNextZeroCrossing(StartFrame, EndFrame, PCMArray, true, NoiseThresholdValue, SoundWave->NumChannels);

	WaveformView.ViewWidget->SetPlayheadFramePosition(StartFrame + FramesToMove);
	WaveformView.ViewWidget->ResolveFromFramePosition();
}
...
void FWaveformEditor::UpdateTransformations(bool bMarkDirty)
{
	...
	//Update the content browser asset state to dirty
	const EPropertyChangeType::Type ChangeType = bMarkDirty ? EPropertyChangeType::ValueSet : EPropertyChangeType::Interactive;
	NotifySoundWavePostEditChangeProperty(GET_MEMBER_NAME_CHECKED(USoundWave, Transformations), ChangeType);
	...
	FPropertyChangedEvent PropertyChangedEvent(TransformationsProperty, ChangeType);

	NotifyPostChange(PropertyChangedEvent, &PropertyChain);

	// Check if we do want to mark the package dirty or not. The user would not expect to modify data when doing things like selecting a loop region
	if (bMarkDirty)
	{
#if WITH_EDITOR
		// Only update SoundWave properties when a change that requires marking it dirty occurs
		UpdateSoundWaveProperties();
#endif //WITH_EDITOR
	}
	else if (!bWasAlreadyDirty)
	{
		SoundWave->GetPackage()->ClearDirtyFlag();
	}
...
void FWaveformEditor::UpdateRenderElements()
{
	...
	// During interactive drag, coalesce heavy PCM regens onto a deferred ticker so GenerateSequenceDataInternal doesn't block every mouse-move event.
	// The ticker reads the latest property values when it fires, so visual feedback remains correct.
	if (!bDeferredRenderUpdatePending)
	{
		bDeferredRenderUpdatePending = true;
		TWeakPtr<FWaveformEditor> WeakSelf = SharedThis(this);
		constexpr float DeferredUpdateDelaySeconds = 0.f; // Wait a single tick, then execute

		DeferredRenderTickerHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda([WeakSelf](float /*DeltaTime*/) -> bool
			{
				if (TSharedPtr<FWaveformEditor> PinnedSelf = WeakSelf.Pin())
				{
					PinnedSelf->DeferredRenderTickerHandle = FTSTicker::FDelegateHandle();

					// Bail out if the editor is tearing down its SoundWave (asset-delete path) or the view has gone away, the asset-teardown window can fire between schedule and tick.
					if (!PinnedSelf->bDeferredRenderUpdatePending || PinnedSelf->bSoundWaveBeingDeleted || !IsValid(PinnedSelf->SoundWave) || !PinnedSelf->WaveformView.DataProvider.IsValid())
					{
						PinnedSelf->bDeferredRenderUpdatePending = false;
						
						return false;
					}

					PinnedSelf->WaveformView.DataProvider->UpdateRenderElements();
					PinnedSelf->TransformationChainConfig = PinnedSelf->SoundWave->UpdateTransformations();
					PinnedSelf->bDeferredRenderUpdatePending = false;
				}

				return false; // one-shot
			}), DeferredUpdateDelaySeconds);
	}
}

void FWaveformEditor::NotifySoundWavePostEditChangeProperty(const FName& PropertyName, const EPropertyChangeType::Type PropertyChangeType)
{
	check(SoundWave);

	FProperty* Property = USoundWave::StaticClass()->FindPropertyByName(PropertyName);
	check(Property);

	FPropertyChangedEvent PropertyChangedEvent(Property, PropertyChangeType);
	SoundWave->PostEditChangeProperty(PropertyChangedEvent);

	if (PropertyChangeType != EPropertyChangeType::Interactive)
	{
...
void FWaveformEditor::SetCuePointOrigin(TObjectPtr<UWaveformTransformationMarkers> MarkerTransformation)
{
	if (GEditor)
	{
		if (MarkerTransformation && SoundWave->GetCuePointOrigin() == ESoundWaveCuePointOrigin::WaveFile)
		{
			...
			NotifySoundWavePostEditChangeProperty(USoundWave::GetCuePointOriginPropertyName());
			GEditor->EndTransaction();
		}
...
bool FWaveformEditor::CreateTransportController()
{
	...
	{
		UE_LOGF(LogWaveformEditor, Warning, "Trying to setup transport controls with a null audio component");
		return false;
	}
...
bool FWaveformEditor::BindDelegates()
{
	if (!ensure(AudioComponent))
	{
		UE_LOGF(LogWaveformEditor, Warning, "Failed to bind to playback percentage change, audio component is null");
		return false;
	}
...
void FWaveformEditor::ResetPlaybackToStart()
{
	...
	WaveformView.ViewWidget->SetPlayheadFramePosition(TransformationChainConfig.GetStartFrameOffset());
	WaveformView.ViewWidget->ResolveFromFramePosition();
	// Reset the cached start so the next Play() begins at the active region start, not a stale offset. Zero-based within the post-trim region.
	TransportController->CacheStartTime(0.0f);
}
...
bool FWaveformEditor::BindCommands()
{
	...
	constexpr bool bIsAsync = true;
	
	ToolkitCommands->MapAction(
		Commands.ExportWaveform,
		FExecuteAction::CreateSP(this, &FWaveformEditor::ExportWaveform, bIsAsync),
	...
	ToolkitCommands->MapAction(
		...
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeInType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Linear];
			}),
		FCanExecuteAction::CreateLambda([this] { return true; }),
		FIsActionChecked::CreateLambda([this] { return FadeInType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Linear]; }));

	ToolkitCommands->MapAction(
		Commands.FadeInExponential,
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeInType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Exponential];
			}),
		FCanExecuteAction::CreateLambda([this] { return true; }),
		FIsActionChecked::CreateLambda([this] { return FadeInType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Exponential]; }));

	ToolkitCommands->MapAction(
		Commands.FadeInLogarithmic,
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeInType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Logarithmic];
			}),
		FCanExecuteAction::CreateLambda([this] { return true; }),
		FIsActionChecked::CreateLambda([this] { return FadeInType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Logarithmic]; }));

	ToolkitCommands->MapAction(
		Commands.FadeInSigmoid,
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeInType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Sigmoid];
			}),
		FCanExecuteAction::CreateLambda([this] { return true; }),
		FIsActionChecked::CreateLambda([this] { return FadeInType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Sigmoid]; }));
	...
	ToolkitCommands->MapAction(
		...
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeOutType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Linear];
			}),
		FCanExecuteAction::CreateLambda([this] {return true; }),
		FIsActionChecked::CreateLambda([this] {return FadeOutType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Linear]; }));

	ToolkitCommands->MapAction(
		Commands.FadeOutExponential,
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeOutType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Exponential];
			}),
		FCanExecuteAction::CreateLambda([this] {return true; }),
		FIsActionChecked::CreateLambda([this] {return FadeOutType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Exponential]; }));

	ToolkitCommands->MapAction(
		Commands.FadeOutLogarithmic,
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeOutType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Logarithmic];
			}),
		FCanExecuteAction::CreateLambda([this] {return true; }),
		FIsActionChecked::CreateLambda([this] {return FadeOutType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Logarithmic]; }));

	ToolkitCommands->MapAction(
		Commands.FadeOutSigmoid,
		FExecuteAction::CreateLambda([this] 
			{ 
				FadeOutType = UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Sigmoid];
			}),
		FCanExecuteAction::CreateLambda([this] {return true; }),
		FIsActionChecked::CreateLambda([this] {return FadeOutType == UWaveformTransformationFade::FadeModeToFadeFunctionMap[EWaveEditorTransformationFadeMode::Sigmoid]; }));
	...
	ToolkitCommands->MapAction(
		Commands.SkipToNextZeroCrossing,
		FExecuteAction::CreateSP(this, &FWaveformEditor::SkipToNextZeroCrossing));
...
void FWaveformEditor::NotifyPostChange(const FPropertyChangedEvent& PropertyChangedEvent, class FEditPropertyChain* PropertyThatChanged)
{
	...
	// Non-interactive changes (mouse-release ValueSet, programmatic edits) run immediately and supersede any pending deferred update.
	if (bIsInteractive)
	{
	...
	{
		// Immediate update supersedes any pending deferred regen, cancel the in-flight ticker so it doesn't fire a stale no-op next frame.
		bDeferredRenderUpdatePending = false;
		FTSTicker::GetCoreTicker().RemoveTicker(DeferredRenderTickerHandle);
		DeferredRenderTickerHandle = FTSTicker::FDelegateHandle();
		WaveformView.DataProvider->UpdateRenderElements();
	}
...
void FWaveformEditor::UpdateTransportState()
{
	if (!TransportController->IsPlaying())
	{
		...
		case EAudioComponentPlayState::Stopped:
			...
			if (!TransportCoordinator->IsScrubbing() && !TransportCoordinator->IsScrolling())
			{
				check(SoundWave->GetImportedSampleRate() > 0);
				const float PlayheadTime = static_cast<float>(WaveformView.ViewWidget->GetPlayheadFramePosition()) / static_cast<float>(SoundWave->GetImportedSampleRate());

				// If the playhead goes out of bounds, snap it to the closest trim edge
				if (PlayheadTime > TransformationChainConfig.EndTime)
				{
					TransportController->CacheStartTime(TransformationChainConfig.EndTime - TransformationChainConfig.StartTime);
					TransportCoordinator->SetProgressRatio(1.0f);

					// Cache a play time at the end if the end time trim is what caused the playhead to move
					StartTimeBeforeTransformInteraction = TransformationChainConfig.EndTime;
				}
				else if (PlayheadTime < TransformationChainConfig.StartTime)
				{
...
void FWaveformEditor::ExportWaveform(bool bIsAsync)
{
	check(WaveWriter);
	WaveWriter->ExportTransformedWaveform(bIsAsync);
}
...
bool FWaveformEditor::CreateDetailsViews()
{
	if (!ensure(SoundWave)) 
	{
		UE_LOGF(LogWaveformEditor, Warning, "Trying to setup wav editor properties view from a null SoundWave");
		return false;
	}
...
bool FWaveformEditor::CreateWaveformView()
{
	if (!ensure(SoundWave))
	{
		UE_LOGF(LogWaveformEditor, Warning, "Trying to setup waveform panel from a null SoundWave");
		return false;
	}
