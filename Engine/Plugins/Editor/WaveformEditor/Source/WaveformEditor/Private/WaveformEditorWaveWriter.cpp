...
void FWaveformEditorWaveWriter::ExportTransformedWaveform(bool bIsAsync)
{
	...
	TFunction<void(const USoundWave*)> OnSoundWaveWritten = [AssetName, AssetPath](const USoundWave* ResultingWave) {
		UE_LOGF(LogWaveformEditor, Log, "Finished Exporting edited soundwave %ls/%ls", *AssetPath, *AssetName);
		TArray<UPackage*> PackagesToSave;

		if (ResultingWave == nullptr)
		{
			UE_LOGF(LogWaveformEditor, Warning, "Resulting Exported soundwave is null: %ls/%ls", *AssetPath, *AssetName);

			return;
		}

		if (!ResultingWave->IsValidLowLevel())
		{
			UE_LOGF(LogWaveformEditor, Warning, "Resulting Exported soundwave is invalid: %ls/%ls", *AssetPath, *AssetName);
		...
		if (UPackage* SoundWavePackage = ResultingWave->GetPackage())
		{
			...
			if (bSuccess)
			{
				UE_LOGF(LogWaveformEditor, Log, "Exported soundwave saving succeeded: % ls / % ls", *AssetPath, *AssetName);
			}
			else
			{
				UE_LOGF(LogWaveformEditor, Warning, "Exported soundwave saving failed: %ls/%ls", *AssetPath, *AssetName);
			}
		}
	};
	...
	if (bIsAsync)
	{
		...
		{
			UE_LOGF(LogWaveformEditor, Log, "Exporting async edited soundwave to %ls/%ls failed", *AssetPath, *AssetName);
		}
	}
	else
	{
		if (!WaveWriter->SynchronouslyWriteSoundWave(BufferToWrite, &AssetName, &AssetPath, SourceSoundWave))
		{
			UE_LOGF(LogWaveformEditor, Log, "Exporting edited soundwave to %ls/%ls failed", *AssetPath, *AssetName);
		}
	}
}
...
Audio::TSampleBuffer<int16> FWaveformEditorWaveWriter::GenerateSampleBuffer() const
{
	...
	{
		UE_LOGF(LogInit, Warning, "Failed to get imported soundwave data for file: %ls. Edited waveform will not be rendered.", *SourceSoundWave->GetPathName());
