...
void FColorGradingEditorDataModel::SetSelectedColorGradingGroup(int32 InColorGradingGroupIndex)
{
	...
	SelectedColorGradingGroupIndex = ColorGradingGroups.IsValidIndex(InColorGradingGroupIndex) ? InColorGradingGroupIndex : INDEX_NONE;

	// When the color grading group has changed, reset the selected color grading element as well
	const bool bHasColorGradingElements = ColorGradingGroups.IsValidIndex(SelectedColorGradingGroupIndex) && ColorGradingGroups[SelectedColorGradingGroupIndex].ColorGradingElements.Num() > 0;
