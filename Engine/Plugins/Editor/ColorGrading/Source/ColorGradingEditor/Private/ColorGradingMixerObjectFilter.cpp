...
TArray<FObjectMixerEditorActorSubObject> UColorGradingMixerObjectFilter::GetActorSubObjects(AActor* InActor) const
{
	if (InActor)
	{
		if (const IColorGradingMixerObjectHierarchyConfig* Config = FColorGradingMixerObjectFilterRegistry::GetClassHierarchyConfig(InActor->GetClass()))
		{
			return Config->GetActorSubObjects(InActor);
		}
	}
