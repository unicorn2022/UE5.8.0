...
EAssetCommandResult UAssetDefinition_AnimBlueprint::OpenAssets(const FAssetOpenArgs& OpenArgs) const
{
	...
	for (UAnimBlueprint* AnimBlueprint : OpenArgs.LoadObjects<UAnimBlueprint>({}, &OutAssetsThatFailedToLoad))
	{
		if (AnimBlueprint->SkeletonGeneratedClass && AnimBlueprint->GeneratedClass)
		{
			...
			{
				// For derived anim blueprints, the skeleton is determined by the root anim BP. If we don't have a skeleton it shouldn't either.
				...
				if (RootAnimBlueprint && RootAnimBlueprint->TargetSkeleton)
				{
					// If our base has a new skeleton, but we haven't updated to it yet. Ask for confirmation to update as we may reference assets exclusive to the old skeleton (sequences, etc).
					ShouldRetargetMessage = LOCTEXT("ShouldRetargetToParent_Message", "Anim Blueprint '{BlueprintName}' does not have a skeleton. But parent '{RootAnimBlueprint}' does. Would you like to update to the parent skeleton?");
				}
				else if (RootAnimBlueprint)
				{
					// If our base does not have a skeleton, offer to update the base
					ShouldRetargetMessage = LOCTEXT("ShouldRetargetParent_Message", "Could not find the skeleton for Anim Blueprint '{BlueprintName}' in parent '{RootAnimBlueprint}' Would you like to choose a new parent skeleton?");
				}
				...
				{
					Arguments.Add(TEXT("RootAnimBlueprint"), FText::FromString(RootAnimBlueprint->GetName()));
				}

				if (FMessageDialog::Open(EAppMsgType::YesNo, FText::Format(ShouldRetargetMessage, Arguments)) == EAppReturnType::Yes)
				{
					bool bSkeletonReplaced = false;
					if (RootAnimBlueprint && RootAnimBlueprint->TargetSkeleton)
					{
						// Update to parent skeleton
						constexpr bool bRetargetReferredAssets = true;
						constexpr bool bConvertSpaces = false;

						FAnimationRetargetContext RetargetContext({ TWeakObjectPtr<UObject>(AnimBlueprint) }, bRetargetReferredAssets, bConvertSpaces);
						const FNameDuplicationRule* NameRule = nullptr;

						EditorAnimUtils::RetargetAnimations(nullptr, RootAnimBlueprint->TargetSkeleton, RetargetContext, bRetargetReferredAssets, NameRule);
						bSkeletonReplaced = true;
					}
					else
					{
						...
						if (RootAnimBlueprint && !RootAnimBlueprint->TargetSkeleton)
						{
							AssetsToRetarget.Add(RootAnimBlueprint);
						}
						AssetsToRetarget.Add(AnimBlueprint);

						// Spawn skeleton selector modal
						bSkeletonReplaced = UE::AnimBlueprint::ReplaceMissingSkeleton(AssetsToRetarget);
					}
