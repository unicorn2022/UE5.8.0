...
void SSourceControlReview::PostComment(FReviewComment& Comment)
{
	...
	CommentsAPI->PostComment(Comment, IReviewCommentAPI::OnPostCommentComplete::CreateLambda(
		[](const FReviewComment& Comment, const FString& ErrorMessage)
		{
			...
			{
				UE_LOGF(LogSourceControl, Error, "IReviewCommentsAPI::PostComment Error: %ls", *ErrorMessage)
				return;
			}
...
void SSourceControlReview::EditComment(FReviewComment& Comment)
{
	...
	CommentsAPI->EditComment(Comment, IReviewCommentAPI::OnEditCommentComplete::CreateLambda(
		[](const FReviewComment& Comment, const FString& ErrorMessage)
		{
			if (!ErrorMessage.IsEmpty())
			{
				UE_LOGF(LogSourceControl, Error, "IReviewCommentsAPI::EditComment Error: %ls", *ErrorMessage)
			}
		}));
}
...
void SSourceControlReview::OnGetReviewTopic(const FReviewTopic& Topic, const FString& ErrorMessage)
{
	if(!ErrorMessage.IsEmpty())
	{
		...
		{
			UE_LOGF(LogSourceControl, Error, "IReviewCommentsAPI::GetReviewTopicForCL Error: %ls", *ErrorMessage);
		}
...
void SSourceControlReview::OnGetReviewComments(const TArray<FReviewComment>& Comments, const FString& ErrorMessage)
{
	...
	if (!ErrorMessage.IsEmpty())
	{
		UE_LOGF(LogSourceControl, Error, "IReviewCommentsAPI::GetComments Error: %ls", *ErrorMessage)
		return;
	}
...
void SSourceControlReview::SaveCLHistory()
{
	...
	GConfig->SetArray(TEXT("SourceControlReview"), TEXT("CLHistory.Numbers"), Numbers, GEditorPerProjectIni);
	GConfig->SetArray(TEXT("SourceControlReview"), TEXT("CLHistory.Authors"), Authors, GEditorPerProjectIni);
	GConfig->SetArray(TEXT("SourceControlReview"), TEXT("CLHistory.Descriptions"), Descriptions, GEditorPerProjectIni);
...
void SSourceControlReview::LoadCLHistory()
{
	...
	TArray<FString> Authors;
	TArray<FString> Descriptions;
	GConfig->GetArray(TEXT("SourceControlReview"), TEXT("CLHistory.Numbers"), Numbers, GEditorPerProjectIni);
	GConfig->GetArray(TEXT("SourceControlReview"), TEXT("CLHistory.Authors"), Authors, GEditorPerProjectIni);
	GConfig->GetArray(TEXT("SourceControlReview"), TEXT("CLHistory.Descriptions"), Descriptions, GEditorPerProjectIni);
	CLHistory.Empty();
	if (Numbers.Num() == Authors.Num() && Numbers.Num() == Descriptions.Num())
	{
...
void SSourceControlReview::SetFileSourceControlAction(TSharedPtr<FChangelistFileData> ChangelistFileData, const FString& SourceControlAction)
{
	...
	{
		UE_LOGF(LogSourceControl, Error, "Unable to set revision control action information. No changelist file data");
		return;
	}
	...
	{
		UE_LOGF(LogSourceControl, Error, "Unable to parse revision control action information. '%ls' diff will not be shown", *ChangelistFileData->AssetName);
