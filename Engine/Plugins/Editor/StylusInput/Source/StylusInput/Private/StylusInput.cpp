...
namespace UE::StylusInput
{
	namespace Private
	{
		...
		public:
			IStylusInputInstance* CreateInstance(SWindow& Window, const FName* InterfaceName, const bool bRequestedInterfaceOnly) const
			{
				...
				auto CreateInstanceForInterface = [&Window](IStylusInputInterface& Interface) -> IStylusInputInstance*
				{
					if (IStylusInputInstance* Instance = Interface.CreateInstance(Window))
					{
						...
						UE_LOGF(LogStylusInput, Error, "Stylus input instance for interface '%ls' was not initialized successfully.",
					...
					{
						UE_LOGF(LogStylusInput, Error, "Failed to create stylus input instance for interface '%ls'.",
							   *Interface.GetName().ToString());
					}
				...
				if (InterfaceName)
				{
					...
					{
						UE_LOGF(LogStylusInput, Warning, "Requested stylus input interface '%ls' is not available.", *InterfaceName->ToString());
					}
				}
				...
				UE_LOGF(LogStylusInput, Warning, "No valid stylus input interface and/or instance available.");
				return nullptr;
			}
			...
			bool RegisterInterface(IStylusInputInterface* Interface)
			{
				if (!Interface)
				{
					UE_LOGF(Private::LogStylusInput, Warning, "Nullptr passed into StylusInput::RegisterInterface().");
					return false;
				}
				...
				{
					UE_LOGF(Private::LogStylusInput, Error, "Interface with name '%ls' has already been registered via StylusInput::RegisterInterface().",
			...
			bool UnregisterInterface(IStylusInputInterface* Interface)
			{
				if (!Interface)
				{
					UE_LOGF(Private::LogStylusInput, Warning, "Nullptr passed into StylusInput::UnregisterInterface().");
					return false;
				}
				...
				{
					UE_LOGF(Private::LogStylusInput, Error, "StylusInput::UnregisterInterface: Interface with name '%ls' has not been previously registered.",
						   *InterfaceName.ToString())
					return false;
				}
	...
	bool ReleaseInstance(IStylusInputInstance* Instance)
	{
		if (!Instance)
		{
			UE_LOGF(Private::LogStylusInput, Warning, "Nullptr passed into StylusInput::ReleaseInstance().");
			return false;
		}
