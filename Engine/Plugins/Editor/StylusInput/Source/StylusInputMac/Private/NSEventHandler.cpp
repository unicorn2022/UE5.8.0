...
namespace UE::StylusInput::Mac
{
	...
	NSEvent* FNSEventHandler::HandleNSEvent(NSEvent* Event)
	{
		...
		if (Event.pressure == 0)
		{
			...
			if (CurrentPacketType == EPacketType::OnDigitizer || CurrentPacketType == EPacketType::StylusDown)
				CurrentPacketType = EPacketType::StylusUp;
			else
				CurrentPacketType = EPacketType::AboveDigitizer;
		}
		...
		if (Event.buttonMask & NSEventButtonMaskPenTip || Event.pressure != 0)
		{
			CurrentPenStatus = CurrentPenStatus | EPenStatus::CursorIsTouching;
			if (CurrentPacketType != EPacketType::OnDigitizer && CurrentPacketType != EPacketType::StylusDown)
