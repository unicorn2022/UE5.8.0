...
namespace UE::StylusInput::Wintab
{
	...
	void FStylusInputWintabModule::StartupModule()
	{
		if (FWintabAPI::DriverIsAvailable())
		{
			...
			{
				UE_LOGF(Private::LogStylusInput, Warning, "Wintab interface initialization failed.");
			}
		}
		else
		{
			UE_LOGF(Private::LogStylusInput, Log, "Wintab driver is not available; Wintab interface will be not be initialized.");
		}
	}
