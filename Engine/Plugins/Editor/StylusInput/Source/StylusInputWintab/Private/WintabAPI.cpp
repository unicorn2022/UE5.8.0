...
namespace UE::StylusInput::Wintab
{
	FWintabAPI::FWintabAPI()
	{
		...
		if (void* Wintab32DllHandle = GetDllHandle(Wintab32DllName))
		{
			...
			GetDllExport(Wintab32DllName, Wintab32DllHandle, TEXT("WTOverlap"), WtOverlap);
			GetDllExport(Wintab32DllName, Wintab32DllHandle, TEXT("WTPacket"), WtPacket);
		}

		LogValidityDiagnostics();
	...
	bool FWintabAPI::IsValid() const
	{
		...
		// WtOverlap is optional; missing-export diagnostics handled in LogValidityDiagnostics.

		return bSuccess;
	}

	void FWintabAPI::LogValidityDiagnostics() const
	{
		if (!WtOverlap)
		{
			LogWarning(LOG_PREAMBLE,
				"WTOverlap export is not available; multi-window stylus context promotion is disabled. "
				"Only the most recently opened tablet context will receive packets across windows.");
		}

		if (!IsValid())
		{
			FString MissingExports;
			auto Append = [&MissingExports](const TCHAR* Name, const void* Ptr)
			{
				if (!Ptr)
				{
			...
			Append(TEXT("GetWindowRect"), reinterpret_cast<const void*>(GetWindowRect));
			Append(TEXT("GetSystemMetrics"), reinterpret_cast<const void*>(GetSystemMetrics));
			Append(TEXT("WTInfoW"), reinterpret_cast<const void*>(WtInfo));
			Append(TEXT("WTOpenW"), reinterpret_cast<const void*>(WtOpen));
			Append(TEXT("WTClose"), reinterpret_cast<const void*>(WtClose));
			Append(TEXT("WTPacket"), reinterpret_cast<const void*>(WtPacket));

			LogError(LOG_PREAMBLE, FString::Format(
				TEXT("Wintab API initialization is incomplete; missing required DLL exports: {0}. Stylus input via Wintab will not be available."),
				{MissingExports}));
		}
	}
