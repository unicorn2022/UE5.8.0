...
#undef PREPARE_STATEMENT
	...
	public: bool AddSearchRecord(const FAssetData& InAssetData, const FString& IndexedJson, const FString& IndexedJsonHash)
	{
		...
		if (Statement_AddAssetToAssetTable.BindAndExecute(InAssetData.AssetName.ToString(), InAssetData.AssetClassPath.ToString(), AssetObjectPath, IndexedJsonHash))
		{
						...
						if (Notation == EJsonNotation::ObjectStart)
						{
							...
							while (JsonReader->ReadNext(Notation) && Notation != EJsonNotation::ArrayEnd)
							{
								if (Notation == EJsonNotation::ObjectStart)
								{
									...
									{
										UE_LOGF(LogAssetSearch, Error, "Failed to add entry for asset '%ls': %ls", *AssetObjectPath, *Database.GetLastError());
									}
									else
									{
										EntriesAdded++;
			...
			{
				UE_LOGF(LogAssetSearch, Error, "Failed commit entries for asset '%ls': %ls", *AssetObjectPath, *Database.GetLastError());
				EntriesAdded = 0;
			}

			if (EntriesAdded > 0)
			{
				//UE_LOGF(LogAssetSearch, Log, "Adding '%ls': %ls", *AssetObjectPath, *Database.GetLastError());
			}
...
bool FAssetSearchDatabase::Open(const FString& InSessionPath, const ESQLiteDatabaseOpenMode InOpenMode)
{
	...
	{
		UE_LOGF(LogAssetSearch, Error, "Failed to open database for '%ls': %ls", *InSessionPath, *GetLastError());
		return false;
	}

	if (bEnableIntegrityChecks && !Database->PerformQuickIntegrityCheck())
	{
		UE_LOGF(LogAssetSearch, Error, "Database failed integrity check, deleting.");
	...
	if (LoadedDatabaseVersion != (int32)EAssetSearchDatabaseVersion::Empty)
	{
		...
		{
			Close();
			UE_LOGF(LogAssetSearch, Error, "Failed to open database for '%ls': Database is too new (version %d, expected = %d)", *InSessionPath, LoadedDatabaseVersion, (int32)EAssetSearchDatabaseVersion::LatestVersion);
			return false;
		}
		else if (LoadedDatabaseVersion < (int32)EAssetSearchDatabaseVersion::LatestVersion)
		{
			Close(true);
			UE_LOGF(LogAssetSearch, Log, "Opened database '%ls': Database is too old (version %d, expected = %d), creating new database", *InSessionPath, LoadedDatabaseVersion, (int32)EAssetSearchDatabaseVersion::LatestVersion);
...
bool FAssetSearchDatabase::Close(const bool InDeleteDatabase)
{
	...
	if (!Database->Close())
	{
		UE_LOGF(LogAssetSearch, Error, "Failed to close database for '%ls': %ls", *SessionPath, *GetLastError());
		return false;
	}
...
{
	UE_LOGF(LogAssetSearch, Error, "Database '%ls' Error: %ls", *SessionPath, *GetLastError());
}
