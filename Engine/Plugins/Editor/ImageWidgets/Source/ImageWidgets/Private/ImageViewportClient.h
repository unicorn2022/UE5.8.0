...
namespace UE::ImageWidgets
{
	...
	DECLARE_DELEGATE_RetVal(float, FGetLayoutAndDPIScaleFactor)
	DECLARE_DELEGATE_RetVal(float, FGetLayoutScaleFactor)
	...
	public:
		...
		// FEditorViewportClient overrides - begin
		...
		// FEditorViewportClient overrides - end
	...
	private:
		...
		IImageViewer::FDrawProperties::FPlacement GetPlacementProperties(FIntPoint ImageSize, FVector2d ScaledViewportSize) const;
		...
		FVector2d GetScaledViewportSize() const;

		FGetImageSize GetImageSize;
		FDrawImage DrawImage;
		FGetDrawSettings GetDrawSettings;
		FGetLayoutAndDPIScaleFactor GetLayoutAndDPIScaleFactor;
		FGetLayoutScaleFactor GetLayoutScaleFactor;
		...
		FVector2d CachedScaledViewportSize = FVector2d::ZeroVector;
