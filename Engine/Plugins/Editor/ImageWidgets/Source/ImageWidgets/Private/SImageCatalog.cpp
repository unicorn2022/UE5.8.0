...
TOptional<TArray<FGuid>> FImpl::RemoveGroup(const FName Name, const FName* GroupToMoveItemsInto)
{
	...
	if (GroupToMoveItemsInto && Name == *GroupToMoveItemsInto)
	{
		return {};
	}

	if (const int32* GroupIndexPtr = GroupMapping.Find(Name))
	{
		const int32 GroupIndex = *GroupIndexPtr;
		...
		if (GroupToMoveItemsInto && !NewGroupIndex)
		{
			NewGroupIndex = GroupMapping.Find(DefaultGroupName);
		}
		if (NewGroupIndex)
		{
...
bool FImpl::AddGroup(FName Name, const FText& Heading, const int32 Index)
{
	...
	if (!GroupMapping.Contains(Name))
	{
		...
		TWeakPtr<SWidget> WeakGroupHeader = GroupHeader;
		GroupHeader->SetOnMouseButtonUp(FPointerEventHandler::CreateLambda([this, Name, WeakGroupHeader](const FGeometry& Geometry, const FPointerEvent& Event)
		{
			if (Event.GetEffectingButton() == EKeys::RightMouseButton)
			{
				if (const TSharedPtr<SWidget> PinnedHeader = WeakGroupHeader.Pin())
				{
					const TSharedPtr<SWidget> MenuContent = OnGroupContextMenuOpening(Name);
					if (MenuContent.IsValid())
					{
						const FWidgetPath WidgetPath = Event.GetEventPath() != nullptr ? *Event.GetEventPath() : FWidgetPath();
						FSlateApplication::Get().PushMenu(PinnedHeader->AsShared(), WidgetPath, MenuContent.ToSharedRef(), Event.GetScreenSpacePosition(),
