...
namespace UE::ImageWidgets
{
	                                      ...
	                                      const TSharedPtr<FUICommandList>& InCommandList, FConstructParameters Parameters)
	{
		...
		GetLayoutAndDPIScaleFactor = MoveTemp(Parameters.GetLayoutAndDPIScaleFactor);
		...
		ToolbarExtenderStyle = Parameters.ToolbarExtenderStyle;
		ToolbarExtenderStyleName = Parameters.ToolbarExtenderStyleName;
	...
	FSlimHorizontalToolBarBuilder SImageViewportToolbar::GetToolbarBuilder(const TSharedPtr<FExtender>& Extender) const
	{
		FSlimHorizontalToolBarBuilder ToolbarBuilder(CommandList, FMultiBoxCustomization::None, Extender, true);

		const ISlateStyle* Style = ToolbarExtenderStyle ? ToolbarExtenderStyle : &FAppStyle::Get();
		const FName StyleName = ToolbarExtenderStyle && !ToolbarExtenderStyleName.IsNone() ? ToolbarExtenderStyleName : FName("EditorViewportToolBar");
		ToolbarBuilder.SetStyle(Style, StyleName);
	...
	{
		FSlimHorizontalToolBarBuilder ToolbarBuilder = GetToolbarBuilder(Extender);
			...
			if (bEnableABComparison && ABComparison)
			{
				auto GetTextButton = [this](const FString& Label, const FCheckBoxStyle* ButtonStyle, FImageABComparison::EAorB AorB)
				{
					return SNew(SCheckBox)
						...
						.IsChecked_Lambda([this, AorB]
							{
								return ABComparison->ABComparisonIsSet(AorB) ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
							})
	...
	FText SImageViewportToolbar::GetZoomMenuLabel() const
	{
		...
		const float LayoutAndDPIScaleFactor = GetLayoutAndDPIScaleFactor.Execute();

		const double Zoom = ZoomSettings.Zoom / LayoutAndDPIScaleFactor;
	...
	FText SImageViewportToolbar::GetMipMenuLabel() const
	{
		...
		return FText::Format(LOCTEXT("Mip", "Mip {0}"), Mip == INDEX_NONE ? ImageViewportToolbar_Local::Auto : FText::AsNumber(Mip));
	}
	...
	TSharedRef<SWidget> SImageViewportToolbar::MakeMipMenu() const
	{
		...
		AddMenuEntry(ImageViewportToolbar_Local::Auto, LOCTEXT("MipLevelAuto_Tooltip", "Choose Mip Level automatically"), INDEX_NONE);
