...
namespace UE::ImageWidgets
{
	...
	public:
		 ...
		 * can currently not be used with status bars.
		 */
		...
		struct FControllerSettings
		{
			...
			/** Adjusts zoom to be proportional to viewport resizes, e.g. doubling the viewport size will double the zoom. */
			bool bZoomOnResize = false;
		};
		...
		struct FOverlaySettings
		{
			...
			/** Do not show the bottom right status bar. */
			bool bDisableStatusBarRight = false;
		};

		SLATE_BEGIN_ARGS(SImageViewport)
			: _ToolbarExtenderStyle(nullptr)
			...
			/** Style override for viewport toolbar extensions. */
			SLATE_ARGUMENT(const ISlateStyle*, ToolbarExtenderStyle)

			/**
			 * Name used for the viewport toolbar extensions style override.
			 * Only used if ToolbarExtenderStyle is set. If this is not set, "EditorViewportToolBar" will be used.
			 */
			SLATE_ARGUMENT(FName, ToolbarExtenderStyleName)
	...
	protected:
		...
		// SCompoundWidget overrides - begin
	...
	private:
		...
		/** Provides the window DPI scale factor. */
		float GetDPIScaleFactor() const;

		/** Provides the external layout factor (ratio of accumulated scale to window DPI). */
		float GetLayoutScaleFactor() const;

		/** Provides the layout and DPI scale factor, i.e. the full accumulated geometry scale. */
		float GetLayoutAndDPIScaleFactor() const;
		...
		/** Flag that determines if AB comparison widgets are enabled or not. The value does not change after the call to @see Construct(). */
		...
		/** Style override for toolbar extender. Only used during construction, then reset. */
		const ISlateStyle* ToolbarExtenderStyle = nullptr;
		FName ToolbarExtenderStyleName;
		...
		/** Cached scale for the allotted geometry; used to correctly incorporate the overall scale of the layout when drawing the image. */
		mutable float AllottedGeometryScale = 1.0f;
	};
}
