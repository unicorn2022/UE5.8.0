				...
				if (Toolkit.IsValid())
				{
					...
					FExecuteAction::CreateLambda([this, ToolClass, ToolIdentifier]()
					{
						if (GetToolManager()->CanActivateTool(EToolSide::Mouse, ToolIdentifier))
						{
							...
							{
								UE_LOGF(LogTemp, Warning, "FAILED TO SET ACTIVE TOOL TYPE!");
							}
						}
					}),
...
#if WITH_UNREAL_DEVELOPER_TOOLS
	ViewportWidgetAPI->MakeDraggableWidgetFunc = [](TSharedRef<SWidget> Content)
		{
			return SNew(UE::ToolWidgets::SDraggableBoxOverlay)
				[
					Content
				];
		};
