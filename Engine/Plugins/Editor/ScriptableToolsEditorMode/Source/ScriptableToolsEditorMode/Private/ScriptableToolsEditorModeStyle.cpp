...
bool FScriptableToolsEditorModeStyle::TryRegisterCustomIcon(FName StyleIdentifier, FString FileNameWithPath, FString ExternalRelativePath)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "Custom ScriptableTool Icon file not found at path %ls (on-disk path is %ls)", *ExternalRelativePath, *FileNameWithPath);
		return false;
	}
	...
	{
		UE_LOGF(LogTemp, Warning, "Custom ScriptableTool Icon at path %ls has unsupported type, must be svg or png", *ExternalRelativePath);
		return false;
	}
