...
int32 UAssetRegistryExportCommandlet::Main(const FString& CmdLineParams)
{
	...
	{
		UE_LOGF(LogAssetRegistryExport, Display, "");
		UE_LOGF(LogAssetRegistryExport, Display, "Asset Registry Exporter");
		UE_LOGF(LogAssetRegistryExport, Display, "");
		UE_LOGF(LogAssetRegistryExport, Display, "Converts a given asset registry to a sqlite/csv database. Each AssetClass gets its own");
		UE_LOGF(LogAssetRegistryExport, Display, "table with the corresponding tags for the class as columns. Assets are added to their");
		UE_LOGF(LogAssetRegistryExport, Display, "class's table, as well as a global Assets table.");
		UE_LOGF(LogAssetRegistryExport, Display, "");
		UE_LOGF(LogAssetRegistryExport, Display, "If the CompressedSize DB columns for all assets are NULL then the registry didn't have metadata ");
		UE_LOGF(LogAssetRegistryExport, Display, "written back (ProjectsSettings/Packaging/WriteBackMetadataToAssetRegistry)");
		UE_LOGF(LogAssetRegistryExport, Display, "");
		UE_LOGF(LogAssetRegistryExport, Display, "Parameters:");
		UE_LOGF(LogAssetRegistryExport, Display, "");
		UE_LOGF(LogAssetRegistryExport, Display, "    -Output=<path/to/file>              Output Sqlite DB file (Required Or CSV)");
		UE_LOGF(LogAssetRegistryExport, Display, "    -CSV=<path>                         Directory for CSV output files (Required Or Sqlite) MAKES A TON OF FILES.");
		UE_LOGF(LogAssetRegistryExport, Display, "    -Input=<path/to/file>               Input Asset Registry file (Required)");
		UE_LOGF(LogAssetRegistryExport, Display, "    -ListDependencies=typea,typeb       Comma separated list of primary asset types to output dependency");
		UE_LOGF(LogAssetRegistryExport, Display, "                                        information. Requires asset registry metadata and to be running as");
		UE_LOGF(LogAssetRegistryExport, Display, "                                        a commandlet for the project that generated the asset registry.");
		UE_LOGF(LogAssetRegistryExport, Display, "    -IncludeSharedWithDependencies      When listing dependencies, this will also add shared dependencies");
		UE_LOGF(LogAssetRegistryExport, Display, "                                        to a \"shared\" root.");
		UE_LOGF(LogAssetRegistryExport, Display, "    -FilterToClass=<classname>          Only add the table for the given class (and global assets)");
		UE_LOGF(LogAssetRegistryExport, Display, "                                        Example: -FilterToClass=\"/Script/Engine.Texture2D\"");
		return 1;
	}
	...
	if (bCSV == false)
	{
		// We get a mountain of errors if we append to a database.
		if (!IFileManager::Get().Delete(*OutputDatabaseName, false))
		{
			UE_LOGF(LogAssetRegistryExport, Error, "Failed to delete sqlite database before opening %ls", *OutputDatabaseName);
			return 1;
		}
		if (OutputDatabase.Open(*OutputDatabaseName) == false)
		{
			UE_LOGF(LogAssetRegistryExport, Error, "Failed to open sqlite database %ls", *OutputDatabaseName);
			return 1;
		}
		UE_LOGF(LogAssetRegistryExport, Display, "Creating Sqlite DB: %ls", *OutputDatabaseName);
	}
	else
	{
	...
	FAssetRegistryState AssetRegistry;
	{
		...
		if (FFileHelper::LoadFileToArray(SerializedAssetData, *AssetRegistryFileName))
		{
			...
			{
				UE_LOGF(LogAssetRegistryExport, Error, "Failed to serialize asset registry %ls", *AssetRegistryFileName);
				return 1;
			}
		}
		else
		{
			UE_LOGF(LogAssetRegistryExport, Error, "Failed to read asset registry %ls", *AssetRegistryFileName);
			return 1;
		}
	}
	UE_LOGF(LogAssetRegistryExport, Display, "Opened asset registry: %ls", *AssetRegistryFileName);
	...
	if (DependencyListAssetTypeNames.Num())
	{
		...
		if (CheckDeps.Num() == 0)
		{
			UE_LOGF(LogAssetRegistryExport, Error, "Can't list dependencies without running as a commandlet for the same project that generated the DevelopmentAssetRegistry");
			return 1;
		}
	}
	...
	{
		UE_LOGF(LogAssetRegistryExport, Display, "Got option: FilterToClass=%ls", *FilterToClass);
	}
	...
	if (DependencyListAssetTypeNames.Num())
	{
		...
		UE_LOGF(LogAssetRegistryExport, Display, "ListDependencies matching PrimaryAssets discovered %ls", *NumberString(PrimaryAssets.Num()));
					...
					if (bIsUnique)
					{
						...
						if (UseAsClassData)
						{
							...
							if (UseAsClassData->GetTagValue(UE::AssetRegistry::Stage_ChunkCompressedSizeFName, CompressedSize))
							{
								...
								{
									UE_LOGF(LogAssetRegistryExport, Warning, "Already seen %ls in %ls, we are %ls", *PackageDependency.ToString(), *AccountedFor[UseAsClassData].ToString(), *Pair.Key.ToString());
								}
				...
				if (UnionedTotalDependencies.Contains(Package.Key) == false)
				{
					...
					if (UseAsClassData)
					{
						...
						if (UseAsClassData->GetTagValue(UE::AssetRegistry::Stage_ChunkCompressedSizeFName, CompressedSize))
						{
							...
							if (AccountedFor.Contains(UseAsClassData))
							{
								UE_LOGF(LogAssetRegistryExport, Warning, "Already seen %ls in %ls, we are %ls", *Package.Key.ToString(), *AccountedFor[UseAsClassData].ToString(), *Package.Key.ToString());
							}
		...
		UE_LOGF(LogAssetRegistryExport, Display, "TotalUnique: %ls, Shared: %ls Unassigned: %ls, Total %ls, Orphaned (%ls assets) %ls",
		...
		{
			UE_LOGF(LogAssetRegistryExport, Display, "%ls orphaned assets found - this is due to differences in assigning the \"most important\" asset in a package between UnrealPak and here.", *NumberString(RemainingAssetsWithSize.Num()));
		}
		for (const FAssetData* LD : RemainingAssetsWithSize)
		{
			...
			if (LD->GetTagValue(UE::AssetRegistry::Stage_ChunkCompressedSizeFName, value))
			{
				...
				UE_LOGF(LogAssetRegistryExport, Warning, "...Orphaned: %ls (%ls bytes, in packagemap: %ls", *LD->PackageName.ToString(), *NumberString(value), *LexToString(AllPackages.Contains(LD->PackageName)));
				...
				for (const FAssetData* AD : PackageAssets)
				{
					...
					UE_LOGF(LogAssetRegistryExport, Warning, "......Package has %ls %lld", *AD->AssetName.ToString(), blah);
				...
				UE_LOGF(LogAssetRegistryExport, Warning, "......MostImportant: %ls", MostImportant ? *MostImportant->AssetName.ToString() : TEXT("null"));
			}
		}
	...
	for (TPair<FTopLevelAssetPath, FClassInfo>& Pair : ClassInfos)
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogAssetRegistryExport, Error, "Table Create Error: %ls", *OutputDatabase.GetLastError());
			...
			{
				UE_LOGF(LogAssetRegistryExport, Error, "Insert Statement Create Error: %ls", *OutputDatabase.GetLastError());
				OutputDatabase.Close();
				return 1;
			}
		}
	...
	else
	{
		...
		{
			UE_LOGF(LogAssetRegistryExport, Error, "Error: %ls", *OutputDatabase.GetLastError());
			OutputDatabase.Close();
			return 1;
		}
	...
	UE_LOGF(LogAssetRegistryExport, Display, "Adding %d assets to the database...", AssetCount);
	...
	AssetRegistry.EnumerateAllAssets(TSet<FName>(), [&](const FAssetData& AssetData)
	{
		...
		else
		{
			// Add to the 'Assets' table.
			{
				...
				{
					UE_LOGF(LogAssetRegistryExport, Error, "Insertion Error: %ls", *OutputDatabase.GetLastError());
				}
			}
			...
			if (ClassPtr)
			{
				...
				{
					UE_LOGF(LogAssetRegistryExport, Error, "Class Insert Error: %ls", *OutputDatabase.GetLastError());
				}
			}
	...
	UE_LOGF(LogAssetRegistryExport, Display, "Done in %f seconds per 1000 assets, total of %f seconds.", (EndTime - StartTime) / (AssetCount / 1000.0), EndTime - StartTime);

	if (bGotCompressedSize == false)
	{
		UE_LOGF(LogAssetRegistryExport, Display, "Asset registry didn't have size metadata written back, CompressedSize DB column will be NULL.");
		UE_LOGF(LogAssetRegistryExport, Display, "Metadata can be written back via ProjectSettings/Packaging/WriteBackMetadataToAssetRegistry");
	}
	...
	if (bCSV == false)
	{
		...
		{
			UE_LOGF(LogAssetRegistryExport, Error, "Close Error: %ls", *OutputDatabase.GetLastError());
		}
	}

	UE_LOGF(LogAssetRegistryExport, Display, "Done.");
	return 0;
}
