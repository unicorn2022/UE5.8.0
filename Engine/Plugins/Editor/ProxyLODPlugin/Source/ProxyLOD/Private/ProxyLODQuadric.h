...
namespace ProxyLOD
{
	...
	inline float FQuadric::Evaluate(const FVector& Point) const
	{
		...
		if (!FMath::IsFinite(Q))
		{
			UE_LOGF(LogProxyQuadric, Warning, "Quadric point evaluate detected possible degenerate. Returning 0.");
			Q = 0;
		}
	...
	inline float TQuadricAttr< NumAttributes >::Evaluate(const FVector& Point, const float* Attributes, const float* AttributeWeights) const
	{
		...
		if (!FMath::IsFinite(Q))
		{
			UE_LOGF(LogProxyQuadric, Warning, "Quadric point+attributes evaluate detected possible degenerate. Returning 0.");
			Q = 0;
		}
