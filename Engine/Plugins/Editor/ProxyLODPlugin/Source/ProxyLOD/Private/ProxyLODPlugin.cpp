...
FVoxelizeMeshMerging::ComputeResolution(const FMeshProxySettings& InProxySettings, float ObjectSize)
{
	...
	UE_LOGF(LogProxyLODMeshReduction, Log, "Spatial Sampling Distance Scale used %f, and the major axis for the object bbox was %f", VoxelSize, ObjectSize);
...
void FVoxelizeMeshMerging::ProxyLOD(FMeshDescriptionArrayAdapter& InSrcGeometryAdapter, FMeshDescriptionArrayAdapter& InClippingGeometryAdapter, const FMeshProxySettings& InProxySettings, const FFlattenMaterialArray& InputMaterials, const FGuid InJobGUID)
{
	...
	{
		UE_LOGF(LogProxyLODMeshReduction, Warning, "No static meshes input, no output mesh will be generated.");
	...
	// Compute the simplified mesh and related materials.
	{
...

		{
			...
			if (!BBox.hasVolume())
			{
				UE_LOGF(LogProxyLODMeshReduction, Log, "Empty bounding box for all static meshes input, no output mesh will be generated.");
			...
			TRACE_CPUPROFILER_EVENT_SCOPE(CreateHighPolyGeometry)
			{
				...
				if (bSuccess)
				{
					...
					if (HoleRadius > .5 * BBoxMinorAxis )
					{
						...
						UE_LOGF(LogProxyLODMeshReduction, Display, "Merge distance %f too large, clamped to %f.", InProxySettings.MergeDistance, float(2. * HoleRadius));
					}
				...
				{
					UE_LOGF(LogProxyLODMeshReduction, Warning, "Allocation Error: The objects were too large for the selected Spatial Sampling Distance");
		...
		if (bUVGenerationSuccess && SrcGeometryPolyField)
		{
			...
			// --- By mapping the texels to the source geometry.
			{
				...
				UE_LOGF(LogProxyLODMeshReduction, Log, "Material Distance %f used to discover textures.", float(MaxRayLength));
		...
		else  // UV-generation failed! 
		{
			...
			UE_LOGF(LogProxyLODMeshReduction, Warning, "UV Generation failed.");
