...
)
{
	...
	OutMidPointHeight = bDraggedIsTopEdge
		? ChooseConsistentEdgeVert(ELatticeEdgeType::Bottom, ControlPoints[BottomStartVertIndex], ControlPoints[BottomEndVertIndex])
		: ChooseConsistentEdgeVert(ELatticeEdgeType::Top, ControlPoints[TopStartVertIndex], ControlPoints[TopEndVertIndex]);
}
}

TOptional<FLatticePointTangentsMirrorOp> FLatticePointTangentsMirrorOp::MakeMirrorOpForDragLatticeControlPoint(
	int32 InControlPointIndex, const FCurveModelID& InCurveId, const FPerCurveDeformer2D& InDeformer, const FCurveEditor& InCurveEditor)
{
...
TOptional<FLatticePointTangentsMirrorOp::FCurveMirrorData> FLatticePointTangentsMirrorOp::ComputeTangentMirrorData(
	int32 InControlPointIndex, const FCurveEditor& InCurveEditor, const FCurveModelID& InCurveId, const FPerCurveDeformer2D& InDeformer
	)
{
	...
	Algo::Transform(CellMetaData, KeyHandles, [](const FLatticePointMetaData& MetaData) { return MetaData.KeyHandle;} );
	
	CurveEditor::FTangentMirrorSolver MirrorSolver(DragStartHeight, MidPointHeight);
	const bool bHadTangentsToMirror = MirrorSolver.AddTangents(InCurveEditor, InCurveId, KeyHandles, ComputeFalloff);
	// Only user-specified tangents need mirroring
	return bHadTangentsToMirror ? FCurveMirrorData(InCurveId, MoveTemp(MirrorSolver), MoveTemp(FalloffValues)) : TOptional<FCurveMirrorData>();
}
}
