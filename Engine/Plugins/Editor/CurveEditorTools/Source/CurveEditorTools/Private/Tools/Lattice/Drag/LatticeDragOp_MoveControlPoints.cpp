...
void FLatticeDragOp_MoveControlPoints::OnBeginDrag(const FGeometry& InGeometry, const FVector2D& InInitialMousePosition, EKeyMoveMode InSnapMode)
{
	OnBeginDragControlPointsDelegate.ExecuteIfBound();
}

void FLatticeDragOp_MoveControlPoints::OnMoveMouse(const FGeometry& InGeometry, const FVector2D& InScreenPosition, EKeyMoveMode InSnapMode)
{
	UpdateControlPoints(InGeometry, InScreenPosition);
	OnInteractiveDragControlPointsDelegate.Execute(UpdatedControlPointPositions, InSnapMode);
}

void FLatticeDragOp_MoveControlPoints::OnEndDrag(const FGeometry& InGeometry, const FVector2D& InMousePosition, EKeyMoveMode InSnapMode)
{
	UpdateControlPoints(InGeometry, InMousePosition);
	OnFinishDragControlPointsDelegate.Execute(UpdatedControlPointPositions, InSnapMode);
}
