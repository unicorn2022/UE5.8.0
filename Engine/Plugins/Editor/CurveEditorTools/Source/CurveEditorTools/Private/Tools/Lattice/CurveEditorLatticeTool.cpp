...
#include "Editor.h"
#include "LatticePointMetaData.h"
...
TOptional<FDragOpData> FCurveEditorLatticeTool::CreateDragDrop(
...
TOptional<FDragOpData> FCurveEditorLatticeTool::CreateDragDrop_Point(
	...
	)
{
	...
	const auto MakeDelegate = [this, InControlPointIndex](EPropertyChangeType::Type ChangeType)
	{
		return FHandleMovedPoints::CreateLambda(
			[this, InControlPointIndex, ChangeType](const TConstArrayView<FVector2D>& InNewControlPointPositions, EKeyMoveMode InSnapMode)
			{
				const int32 Indices[1] { InControlPointIndex };
				HandleDragPoints(&FCurveEditorLatticeTool::OnMoveControlPoint,Indices, InNewControlPointPositions, ChangeType, InSnapMode);
...
TOptional<FDragOpData> FCurveEditorLatticeTool::CreateDragDrop_Edge(
	...
	)
{
	...
	{
		return FHandleMovedPoints::CreateLambda(
			[this, EdgeIndices, ChangeType](const TConstArrayView<FVector2D>& InNewControlPointPositions, EKeyMoveMode InSnapMode)
			{
				const int32 Indices[2]{ EdgeIndices.Start, EdgeIndices.End };
				HandleDragPoints(&FCurveEditorLatticeTool::OnMoveEdge, Indices, InNewControlPointPositions, ChangeType, InSnapMode);
				GetGlobalDeformer().UpdateControlPoints(Indices, InNewControlPointPositions, [](auto, auto){});
...
TOptional<FDragOpData> FCurveEditorLatticeTool::CreateDragDrop_Cell(
	...
	)
{
	...
	{
		return FHandleMovedPoints::CreateLambda(
			[this, CellIndices, ChangeType](const TConstArrayView<FVector2D>& InNewControlPointPositions, EKeyMoveMode InSnapMode)
			{
				const int32 Indices[4]{ CellIndices.TopLeft, CellIndices.TopRight, CellIndices.BottomRight, CellIndices.BottomLeft };
				HandleDragPoints(&FCurveEditorLatticeTool::OnSetControlPoints, Indices, InNewControlPointPositions, ChangeType, InSnapMode);
				GetGlobalDeformer().UpdateControlPoints(Indices, InNewControlPointPositions, [](auto, auto){});
...
void FCurveEditorLatticeTool::HandleDragPoints(
	FMovePointsSignature InMovePointsCallback,
	TConstArrayView<int32> InVertexIndices, TConstArrayView<FVector2D> InNewControlPoints,
	EPropertyChangeType::Type InChangeType, EKeyMoveMode InSnapMode
	)
{
	const TSharedPtr<FCurveEditor> CurveEditorPin = WeakCurveEditor.Pin();
	if (!CurveEditorPin)
	...
	for (TPair<FCurveModelID, FPerCurveLatticeData>& Pair : LatticeDeformerState->PerCurveData)
	{
		...
		(this->*InMovePointsCallback)(PerCurveDeformer, *CurveModel, InVertexIndices, CurveRelativeControlPoints, InChangeType, InSnapMode);
	}
}
	...
	)
{
	...
	OnSetControlPoints(InDeformer, InCurveModel, InVertexIndices, InNewControlPointPositions, InChangeType, InSnapMode);
}
	...
	)
{
	...
	const TSharedPtr<FCurveEditor> CurveEditor = WeakCurveEditor.Pin();
	const TOptional<FCurvePointSnapper> Snapper = CurveEditor ? FCurvePointSnapper::MakeSnapper(*CurveEditor, InCurveModel.GetOrInitId()) : TOptional<FCurvePointSnapper>{};
	...
	InDeformer.UpdateControlPoints(InControlPointsToUpdate, InNewControlPoints,
		[InSnapMode, &Snapper, &Handles, &Keys](const FLatticePointMetaData& MetaData, const FVector2D& KeyPosition)
		{
			Handles.Add(MetaData.KeyHandle);
			const FKeyPosition& Original = MetaData.OriginalPosition;
			const FKeyPosition Position
			{
				EnumHasAnyFlags(InSnapMode, EKeyMoveMode::IgnoreX) ? Original.InputValue : KeyPosition.X,
				EnumHasAnyFlags(InSnapMode, EKeyMoveMode::IgnoreY) ? Original.OutputValue : KeyPosition.Y,
			};
			Keys.Add(Snapper->SnapKey(Position, InSnapMode));
		});
...
void FCurveEditorLatticeTool::MirrorGrid(FPerCurveDeformer2D& InDeformer, FCurveModel& InCurveModel, FCurveEditor& InCurveEditor)
{
	...
	const TConstArrayView<FLatticePointMetaData> CellMetaData = InDeformer.GetCellMetaData(0);
	Algo::Transform(CellMetaData, Keys, [](const FLatticePointMetaData& MetaData){ return MetaData.KeyHandle; });
