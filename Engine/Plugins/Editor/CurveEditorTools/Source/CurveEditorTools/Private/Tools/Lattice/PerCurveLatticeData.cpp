...
static void AddPointsToDeformer(TConstArrayView<FKeyHandle> InKeys, const FCurveModel* CurveModel, FPerCurveDeformer2D& LocalDeformer)
{
	...
	for (int32 Index = 0; Index < InKeys.Num(); ++Index)
	{
		...
		const FLatticePointMetaData MetaData{ KeyId, Position };
		LocalDeformer.AddPoints_BeforeLatticeMoved(MakeConstArrayView(&MetaData, 1), MakeConstArrayView(&AsVector, 1));
	}
}
}
