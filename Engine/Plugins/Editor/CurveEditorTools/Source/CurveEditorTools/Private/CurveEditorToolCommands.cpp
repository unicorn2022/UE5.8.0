...
void FCurveEditorToolCommands::RegisterCommands()
{
	...
	UI_COMMAND(SetFocusSelectionRange, "Focus Selection Range", "Focuses the Curve Editor on the current Selection Range with zoom based on visible curves.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(SetKeysInSelectionRange, "Select Keys in Selection Range",
		"Selects all keys within the current Selection Range (or Playback Range if no selection range is set).",
