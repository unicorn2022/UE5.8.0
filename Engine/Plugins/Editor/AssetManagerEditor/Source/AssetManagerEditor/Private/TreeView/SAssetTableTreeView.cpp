...
void SAssetTableTreeView::Construct(const FArguments& InArgs, TSharedPtr<FAssetTable> InTablePtr)
{
	...
	CommandList->MapAction(
		...
		FExecuteAction::CreateSP(this, &SAssetTableTreeView::ExecuteFindInContentBrowserAction),
		FCanExecuteAction::CreateSP(this, &SAssetTableTreeView::CanExecuteFindInContentBrowserAction)
	);
...
void SAssetTableTreeView::ExecuteFindInContentBrowserAction()
{
	const FContentBrowserModule& ContentBrowserModule = FModuleManager::Get().LoadModuleChecked<FContentBrowserModule>("ContentBrowser");
	ContentBrowserModule.Get().SyncBrowserToAssets(GetAssetDataForSelection());
}

bool SAssetTableTreeView::CanExecuteFindInContentBrowserAction()
{
...
TSharedRef<SWidget> SAssetTableTreeView::ConstructSearchBox()
{
	SAssignNew(SearchBox, SSearchBox)
		...
		.OnTextChanged(this, &SAssetTableTreeView::SearchBox_OnTextChanged);

	SearchBoxWidget = SearchBox;

	return SearchBoxWidget.ToSharedRef();
}
...
void SAssetTableTreeView::RefreshAssets()
{
	...
	UE_LOGF(LogInsights, Log, "[AssetTree] Build asset table...");
		...
		for (int32 SourceAssetIndex = 0; SourceAssetIndex < SourceAssets.Num(); SourceAssetIndex++)
		{
			...
			if (IndicesOfDependenciesInRowTableToAddToCurrentSourceAssetRow.Num())
			{
				...
				for (const FAssetData* const SourceAssetData : AssetsInSourcePackage)
				{
					...
					if (RowIndex == nullptr)
					{
						UE_LOGF(LogInsights, Warning, "Failed to find asset %ls in package %ls, source asset index %d.", *SourceAssetData->AssetName.ToString(), *SourceAssetData->PackageName.ToString(), SourceAssetIndex);
					}
	...
	if (CookMetadata.IsValid())
	{
		...
		// Plugin Data
		{
			...
			for (const UE::Cook::FCookMetadataPluginEntry& PluginEntry : PluginHierarchy.PluginsEnabledAtCook)
			{
				...
				if (const int64* SizePtr = PluginToSizeMap.Find(PluginEntry.Name))
				{
					if (CookMetadata.GetSizesPresent() != UE::Cook::ECookMetadataSizesPresent::NotPresent)
					{
						...
						{
							UE_LOGF(LogInsights, Warning, "Plugin %ls found with mismatched ucookmetadata and internal asset size calculation. Metadata size: %lld Calculated size: %lld",
			...
			if (TotalMismatch != 0)
			{
				UE_LOGF(LogInsights, Warning, "Total Size of Plugins from Metadata: %lld // Total Discrepancy (vs calculated size from assets): %lld", TotalSize, TotalMismatch);
			}
			...
			for (uint16 RootPluginIndex : PluginHierarchy.RootPlugins)
			{
				...
				const TCHAR* StoredPluginName = AssetTable->StoreStr(PluginEntry.Name);
				FAssetTablePluginInfo& PluginInfo = AssetTable->GetOrCreatePluginInfo(StoredPluginName);
				UE_LOGF(LogInsights, Display, "Found root plugin %ls", StoredPluginName);
	...
	{
		UE_LOGF(LogInsights, Warning, "CookMetadata not available, deriving plugin dependencies and sizes from loaded asset registry instead.");
		...
		for (TPair<FString, int64>& PluginEntry : PluginToSizeMap)
		{
			...
			{
				UE_LOGF(LogInsights, Warning, "Could not find plugin %ls in plugin manager.", PluginInfo.PluginName);
			}
		}
	}
	...
	for (int32 PluginIndex = 0; PluginIndex < AssetTable->GetNumPlugins(); PluginIndex++)
	{
		...
		{
			UE_LOGF(LogInsights, Warning, "Found orphaned plugin %ls", PluginInfo.GetName());
		}
	}
	...
	UE_LOGF(LogInsights, Log, "[AssetTree] Asset table rebuilt in %.4fs (%d visible + %d hidden = %d assets)",
	...
	UE_LOGF(LogInsights, Log, "[AssetTree] String store: %d strings (%lld bytes) --> %d strings (%lld bytes | %lld bytes) %.0f%%",
...
void SAssetTableTreeView::DumpDifferencesBetweenDiscoveredDataAndLoadedMetadata(TMap<const TCHAR*, DeprecatedTCharSetType, FDefaultSetAllocator, TStringPointerMapKeyFuncs_DEPRECATED<const TCHAR*, DeprecatedTCharSetType>>& DiscoveredPluginDependencyEdges) const
{
	...
	if (CookMetadata.IsValid())
	{
		...
		for (const TPair<const TCHAR*, DeprecatedTCharSetType>& DiscoveredDependencies : DiscoveredPluginDependencyEdges)
		{
			...
			{
				UE_LOGF(LogInsights, Warning, "Unable to find plugin %ls, known from asset traversal, in cook metadata.", DiscoveredDependencies.Key);
			}
			else
			{
				for (const TCHAR* DiscoveredDependency : DiscoveredDependencies.Value)
				{
					...
					{
						UE_LOGF(LogInsights, Warning, "Unable to find plugin %ls, known from asset traversal, in cook metadata.", DiscoveredDependency);
					}
				}
		...
		for (int32 PluginIndex = 0; PluginIndex < AssetTable->GetNumPlugins(); PluginIndex++)
		{
			...
			if (IndexInMetadata == nullptr)
			{
				UE_LOGF(LogInsights, Warning, "Unable to find plugin %ls in metadata.", PluginInfoInTable.GetName());
			}
			else
			{
				...
				{
					UE_LOGF(LogInsights, Warning, "Unable to find plugin %ls, known from metadata, in asset traversal data", PluginInfoInTable.GetName());
				}
			}
		}
		...
		{
			UE_LOGF(LogInsights, Warning, "Dependencies in assets but not in cook metadata:");
			for (const TPair<FString, FString>& Edge : DependenciesInAssetsNotMetadata)
			{
				UE_LOGF(LogInsights, Warning, "%ls-->%ls", *Edge.Key, *Edge.Value);
			}
		}

		if (DependenciesInMetadataNotAssets.Num() > 0)
		{
			UE_LOGF(LogInsights, Warning, "Dependencies in cook metadata but not in assets:");
...
void SAssetTableTreeView::RebuildTree(bool bResync)
{
	...
	{
		UE_LOGF(LogInsights, Log, "[AssetTree] Creating %d asset nodes (previously: %d nodes)...", VisibleAssetCount, PreviousNodeCount);
	...
	{
		UE_LOGF(LogInsights, Log, "[AssetTree] Resetting tree (previously: %d nodes)...", PreviousNodeCount);
	}

	SyncStopwatch.Stop();

	UE_LOGF(LogInsights, Log, "[AssetTree] Update tree...");
	UpdateTree();
	...
	UE_LOGF(LogInsights, Log, "[AssetTree] Tree view rebuilt in %.4fs (sync: %.4fs + update: %.4fs) --> %d asset nodes",
		TotalTime, SyncTime, TotalTime - SyncTime, TableRowNodes.Num());
}

void SAssetTableTreeView::SetFilterToName(const FString& Name)
{
	if (SearchBoxWidget.IsValid())
	{
		StaticCastSharedPtr<SSearchBox>(SearchBoxWidget)->SetText(FText::FromString(Name));
	}
}
