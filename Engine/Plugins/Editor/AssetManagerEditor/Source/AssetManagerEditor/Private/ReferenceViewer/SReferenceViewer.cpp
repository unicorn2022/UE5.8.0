...
void SReferenceViewer::Construct(const FArguments& InArgs)
{
	...
	// Build the status bar widget if a parent tab was provided
	TSharedRef<SWidget> StatusBarWidget = SNullWidget::NullWidget;
	if (InArgs._ParentTab.IsValid())
	{
		const FName StatusBarName(FString::Printf(TEXT("ReferenceViewer_0x%llx"), reinterpret_cast<uint64>(this)));
		if (UStatusBarSubsystem* StatusBarSubsystem = GEditor->GetEditorSubsystem<UStatusBarSubsystem>())
		{
			StatusBarWidget = StatusBarSubsystem->MakeStatusBarWidget(StatusBarName, InArgs._ParentTab.ToSharedRef());
			StatusBarWidget->SetVisibility(TAttribute<EVisibility>::CreateLambda([ParentTab = TWeakPtr<SDockTab>(InArgs._ParentTab)]() { return (ParentTab.IsValid() && ParentTab.Pin()->GetVisualTabRole() == ETabRole::MajorTab) ? EVisibility::Visible : EVisibility::Collapsed; }));
		}
	}
...
void SReferenceViewer::OnAddressBarTextCommitted(const FText& NewText, ETextCommit::Type CommitInfo)
{
	if (CommitInfo == ETextCommit::OnEnter)
	{
		...
		// Whether or not the path actually changed, the user has finished editing —
		// fall back to displaying the current graph root rather than stale typed text.
...
void SReferenceViewer::ReCenterGraphOnNodes(const TSet<UObject*>& Nodes)
{
	...
	if ( NewGraphRootNames.Num() > 0 )
	{
		...
		// The graph root has changed; drop any in-progress address bar edit so the
		// displayed path reflects the new root instead of stale typed text.
		TemporaryPathBeingEdited = FText();
