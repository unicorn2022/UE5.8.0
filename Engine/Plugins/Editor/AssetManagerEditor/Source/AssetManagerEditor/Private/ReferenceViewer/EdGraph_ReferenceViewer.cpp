...
UEdGraph_ReferenceViewer::RecursivelyPopulateNodeInfos(bool bInReferencers, const TArray<FAssetIdentifier>& Identifiers, TMap<FAssetIdentifier, FReferenceNodeInfo>& InNodeInfos, int32 InCurrentDepth, int32 InMaxDepth)
{
	...
	if (!bInReferencers && AssetData && AssetData->IsRedirector())
	{
		...
		FAssetData DestinationObjectAssetData = *AssetData;
		TSet<FString> AllVisitedObjectPaths;
		while (DestinationObjectAssetData.IsRedirector())
		{
			FString DestinationObject = FPackageName::ExportTextPathToObjectPath(DestinationObjectAssetData.GetTagValueRef<FString>(GET_MEMBER_NAME_CHECKED(UObjectRedirector, DestinationObject)));
			if (DestinationObject.IsEmpty() || AllVisitedObjectPaths.Contains(DestinationObject))
			{
				// No destination or a loop
				DestinationObjectAssetData = FAssetData();
				break;
			}

			AllVisitedObjectPaths.Add(DestinationObject);
			DestinationObjectAssetData = AssetRegistry->GetAssetByObjectPath(DestinationObject, true);
		}

		if (DestinationObjectAssetData.IsValid())
		{
			const FName DestinationPackageName = DestinationObjectAssetData.PackageName;
