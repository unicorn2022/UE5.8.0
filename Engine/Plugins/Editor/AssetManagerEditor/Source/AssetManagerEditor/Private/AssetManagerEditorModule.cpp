...
TSharedRef<SWidget> IAssetManagerEditorModule::MakePrimaryAssetIdSelector(FPrimaryAssetIdSelectorArgs Args)
{
	FOnGetContent OnCreateMenuContent = FOnGetContent::CreateLambda([
		OnSetId = Args.OnSetId,
		bAllowClear = Args.bAllowClear,
		AllowedTypes = Args.AllowedTypes,
		AllowedClasses = Args.AllowedClasses,
		DisallowedClasses = Args.DisallowedClasses]()
	{
		...
		FOnSetObject OnSetObject = FOnSetObject::CreateLambda([OnSetId](const FAssetData& AssetData)
		{
			...
			OnSetId.ExecuteIfBound(AssetId);
		});
	...
	TAttribute<FText> OnGetObjectText = TAttribute<FText>::Create(Args.OnGetDisplayText);
	TAttribute<FText> OnGetObjectTooltipText = TAttribute<FText>::Create(Args.OnGetTooltipText);
...
{
	return MakePrimaryAssetIdSelector(
		FPrimaryAssetIdSelectorArgs {
			.OnGetDisplayText = OnGetDisplayText,
			.OnGetTooltipText = OnGetDisplayText,
			.OnSetId = OnSetId,
...
public:
	...
	virtual void OpenAssetSizeTreeViewUI(const FString& SelectedPlugin) override;
...
TSharedRef<SDockTab> FAssetManagerEditorModule::SpawnReferenceViewerTab(const FSpawnTabArgs& Args)
{
	...
	TSharedRef<SReferenceViewer> ReferenceViewerWidget = SNew(SReferenceViewer).ParentTab(NewTab);
	ReferenceViewerUI = ReferenceViewerWidget;
	NewTab->SetContent(ReferenceViewerWidget);
...
void FAssetManagerEditorModule::OpenReferenceViewerTab(const TArray<FAssetIdentifier>& InAssetIdentifiers, const FReferenceViewerParams& ReferenceViewerParams)
{
	...
	{
		// Initialize Tab
		TSharedRef<SReferenceViewer> ReferenceViewerWidget = SNew(SReferenceViewer).ParentTab(CurrentAssetTab.ToSharedRef());
		ReferenceViewerUI = ReferenceViewerWidget;
		CurrentAssetTab->SetContent(ReferenceViewerWidget);
...
void FAssetManagerEditorModule::OpenAssetSizeTreeViewUI(const FString& SearchTerm)
{
	if (TSharedPtr<SDockTab> NewTab = FGlobalTabmanager::Get()->TryInvokeTab(AssetDiskSizeTabName))
	{
		TSharedRef<SAssetTableTreeView> AssetTableTreeView = StaticCastSharedRef<SAssetTableTreeView>(NewTab->GetContent());

		AssetTableTreeView->SetFilterToName(SearchTerm);
	}
}
...
void FAssetManagerEditorModule::PerformDependencyChainConsoleCommand(const TArray<FString>& Args)
{
	if (Args.Num() < 2)
	{
		UE_LOGF(LogAssetManagerEditor, Display, "FindDepChain given incorrect number of arguments.  Usage: %ls", FindDepChainHelpText);
		return;
	}
...
void FAssetManagerEditorModule::PerformDependencyClassConsoleCommand(const TArray<FString>& Args)
{
	if (Args.Num() < 2)
	{
		UE_LOGF(LogAssetManagerEditor, Display, "FindDepClasses given incorrect number of arguments.  Usage: %ls", FindClassDepHelpText);
		return;
	}
	...
	for (int32 i = 1; i < Args.Num(); ++i)
	{
		if (!GetDependencyTypeArg(Args[i], RequiredDependencyFlags))
		{
			...
			{
				UE_LOGF(LogClass, Warning, "Short class names are not supported: %ls", *Args[i]);
			}
		}
	}
	...
	{
		UE_LOGF(LogAssetManagerEditor, Verbose, "FindDepClasses for: %ls", *SourcePackage.ToString());
...
void FAssetManagerEditorModule::FindReferenceChains(FName TargetPackageName, FName RootSearchPath, UE::AssetRegistry::EDependencyQuery RequiredDependencyFlags)
{
	...
	UE_LOGF(LogAssetManagerEditor, Log, "Found %i, Dependency Chains to %ls from directory %ls", CurrentFoundChain, *TargetPackageName.ToString(), *RootSearchPath.ToString());
	for (int32 ChainIndex = 0; ChainIndex < CurrentFoundChain; ++ChainIndex)
	{
		TArray<FName>& FoundChain = FoundChains[ChainIndex];
		UE_LOGF(LogAssetManagerEditor, Log, "Chain %i", ChainIndex);

		for (FName& Name : FoundChain)
		{
			UE_LOGF(LogAssetManagerEditor, Log, "\t%ls", *Name.ToString());
		}
	}
}
...
void FAssetManagerEditorModule::FindClassDependencies(FName SourcePackageName, const TArray<FTopLevelAssetPath>& TargetClasses, UE::AssetRegistry::EDependencyQuery RequiredDependencyFlags)
{
	...
	{
		UE_LOGF(LogAssetManagerEditor, Log, "Couldn't find source package %ls. Abandoning class dep search.  ", *SourcePackageName.ToString());
		return;
	}
	...
	{
		UE_LOGF(LogAssetManagerEditor, Log, "Found %i: dependencies for %ls of the target classes", DependencyPackages.Num(), *SourcePackageName.ToString());
		for (FName DependencyPackage : DependencyPackages)
		{
			UE_LOGF(LogAssetManagerEditor, Log, "\t%ls", *DependencyPackage.ToString());
		}
		...
		for (FName DependencyPackage : DependencyPackages)
		{
			...
			UE_LOGF(LogAssetManagerEditor, Log, "Chain to package: %ls", *DependencyPackage.ToString());
			...
			if (AssetRegistry->GetAssets(Filter, DepAssets))
			{
				for (const FAssetData& DepAsset : DepAssets)
				{
					...
					{
						UE_LOGF(LogAssetManagerEditor, Log, "Asset: %ls class: %ls", *DepAsset.AssetName.ToString(), *DepAsset.AssetClassPath.ToString());
					}
				}
			}
			...
			{
				UE_LOGF(LogAssetManagerEditor, Log, "\t%ls", *DepChainEntry.ToString());
			}
		}
	}
}
...
void FAssetManagerEditorModule::WriteProfileFile(const FString& Extension, const FString& FileContents)
{
	...
	UE_LOGF(LogAssetManagerEditor, Log, "Saving %ls", *FPaths::ConvertRelativePathToFull(FilenameFull));
...
void FAssetManagerEditorModule::LogAssetsWithMultipleLabels()
{
	...
	for (TPair<FName, TArray<FPrimaryAssetId>> Pair : PackageToLabelMap)
	{
		if (Pair.Value.Num() > 1)
		{
			...
			UE_LOGF(LogAssetManagerEditor, Log, "%ls has %ls", *Pair.Key.ToString(), *TagString);
		}		
	}
}
...
void FAssetManagerEditorModule::DumpAssetDependencies(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogAssetManagerEditor, Log, "=========== Asset Manager Dependencies ===========");
	...
	for (const FPrimaryAssetTypeInfo& TypeInfo : TypeInfos)
	{
		...
		{
			UE_LOGF(LogAssetManagerEditor, Log, "  Type %ls:", *TypeInfo.PrimaryAssetType.ToString());
			...
			{
				UE_LOGF(LogAssetManagerEditor, Log, "    %ls: depends on %ls", *DependencyInfo.AssetName.ToString(), *DependencyInfo.AssetListString);
			}
		}
	}
...
bool FAssetManagerEditorModule::WriteCollection(ICollectionContainer& CollectionContainer, FName CollectionName, ECollectionShareType::Type ShareType, const TArray<FName>& PackageNames, bool bShowFeedback)
{
	...
	{
		UE_LOGF(LogAssetManagerEditor, Log, "Nothing to add to collection %ls", *CollectionName.ToString());
	...
	else if (CreateOrEmptyCollection(CollectionContainer, CollectionName, ShareType, Error))
	{
		...
		{
			UE_LOGF(LogAssetManagerEditor, Log, "Updated collection %ls", *CollectionName.ToString());
			ResultsMessage = FText::Format(LOCTEXT("CreateCollectionSucceeded", "Updated collection {0}"), FText::FromName(CollectionName));
			bSuccess = true;
		}
		else
		{
			UE_LOGF(LogAssetManagerEditor, Warning, "Failed to update collection %ls. %ls", *CollectionName.ToString(), *Error.ToString());
	...
	{
		UE_LOGF(LogAssetManagerEditor, Warning, "Failed to create collection %ls. %ls", *CollectionName.ToString(), *Error.ToString());
