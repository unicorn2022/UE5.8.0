...
TSharedRef<SWidget> SSizeMap::GenerateSourceComboItem(TSharedPtr<FString> InItem) const
{
	return SNew(STextBlock).Text(FText::FromString(*InItem));
}

void SSizeMap::HandleSourceComboChanged(TSharedPtr<FString> Item, ESelectInfo::Type SelectInfo)
{
	if (Item.IsValid())
	{
...
FText SSizeMap::GetSourceComboText() const
{
...
TSharedRef<SWidget> SSizeMap::GeneratePluginComboItem(TSharedPtr<FString> InItem) const
{
	const FText DisplayText = InItem->IsEmpty()
		? LOCTEXT("PluginFilter_None", "None")
		: FText::FromString(*InItem);
	return SNew(STextBlock).Text(DisplayText);
}

void SSizeMap::HandlePluginComboChanged(TSharedPtr<FString> Item, ESelectInfo::Type SelectInfo)
{
	if (Item.IsValid())
	{
		CurrentPluginSelection = *Item;
		if (!CurrentPluginSelection.IsEmpty())
		{
			SetRootAssetIdentifiersFromPlugin(CurrentPluginSelection);
		}
	}
}

FText SSizeMap::GetPluginComboText() const
{
	return CurrentPluginSelection.IsEmpty()
		? LOCTEXT("PluginFilter_None", "None")
		: FText::FromString(CurrentPluginSelection);
}

void SSizeMap::SetRootAssetIdentifiersFromPlugin(const FString& PluginName)
{
	TSet<FString> AllPluginNames;
	// Only support a single plugin being collected right now. Too many will overload the display.
	AllPluginNames.Add(PluginName);

	// Gather all asset package names from those plugins via the editor asset registry
	IPluginManager& PluginManager = IPluginManager::Get();
	FARFilter Filter;
	Filter.bRecursivePaths = true;
	for (const FString& Name : AllPluginNames)
	{
	...
	CurrentRegistrySource->GetAssets(Filter, FoundAssets);
	TSet<FAssetIdentifier> AssetIdentifiers;
	AssetIdentifiers.Reserve(FoundAssets.Num());
	for (const FAssetData& Asset : FoundAssets)
	{
		AssetIdentifiers.Add(FAssetIdentifier(Asset.PackageName));
	}

	// Replace existing roots — SetRootAssetIdentifiers clears before setting
	SetRootAssetIdentifiers(AssetIdentifiers.Array());
}
...
void SSizeMap::Construct(const FArguments& InArgs)
{
	...
	// Populate plugin picker list — all enabled plugins that can contain content, sorted by name
	PluginComboList.Add(MakeShared<FString>(TEXT(""))); // "None" sentinel (empty string)
	{
		IPluginManager& PluginManager = IPluginManager::Get();
		TArray<TSharedRef<IPlugin>> EnabledPlugins = PluginManager.GetEnabledPluginsWithContent();
		...
		{
			PluginComboList.Add(MakeShared<FString>(Plugin->GetName()));
		}
	}
	...
	Commands->MapAction(
		...
		FExecuteAction::CreateSP(this, &SSizeMap::CopyPackageName),
		FCanExecuteAction::CreateSP(this, &SSizeMap::IsAnythingSelected));
	...
	[
		...
		[
			...
			[
				SNew(STextBlock)
				.ToolTipText(LOCTEXT("Platform_Tooltip", "Select which platform to display size data for. Platforms are only available if a cooked DevelopmentAssetRegistry.bin exists in Saved/Cooked/[Platform]/Metadata."))
			...
			.AutoWidth()
			[
				SAssignNew(SourceComboBoxWidget, SComboBox<TSharedPtr<FString>>)
				.OptionsSource(&SourceComboList)
				.OnGenerateWidget(this, &SSizeMap::GenerateSourceComboItem)
				.OnSelectionChanged(this, &SSizeMap::HandleSourceComboChanged)
				[
					SNew(STextBlock)
					.Text(this, &SSizeMap::GetSourceComboText)
				]
			]
			+SHorizontalBox::Slot()
			.HAlign(HAlign_Right)
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(LOCTEXT("PluginFilter_Label", "Plugin Filter:"))
				.ToolTipText(LOCTEXT("PluginFilter_Tooltip", "Select a plugin to use as the size map root. All assets from the selected plugin will be shown."))
			]
			+SHorizontalBox::Slot()
			.AutoWidth()
			.VAlign(VAlign_Center)
			.Padding(2.f)
			[
				SAssignNew(PluginComboBoxWidget, SComboBox<TSharedPtr<FString>>)
				.OptionsSource(&PluginComboList)
				.OnGenerateWidget(this, &SSizeMap::GeneratePluginComboItem)
				.OnSelectionChanged(this, &SSizeMap::HandlePluginComboChanged)
				.ToolTipText(LOCTEXT("PluginFilter_Combo_Tooltip", "Select a plugin. All assets from this plugin will replace the current size map roots."))
				[
					SNew(STextBlock)
					.Text(this, &SSizeMap::GetPluginComboText)
				]
			]
		]
		+SVerticalBox::Slot()
	...
	// Refresh registry data to discover any available platform registries (e.g. DevelopmentAssetRegistry.bin)
	EditorModule->RefreshRegistryData();
}
...
{
	bMemorySizeCached = false;
	RootAssetIdentifiers.Reset();
	RootAssetIdentifiers.Append(NewRootAssetIdentifiers);
...
void SSizeMap::SetCurrentRegistrySource(const FAssetManagerEditorRegistrySource* RegistrySource)
{
	...
	// Refresh the platform combo list to reflect current available registry sources
	TArray<const FAssetManagerEditorRegistrySource*> AvailableSources;
	EditorModule->GetAvailableRegistrySources(AvailableSources);
	...
	if (SourceComboBoxWidget.IsValid())
	{
		SourceComboBoxWidget->RefreshOptions();
	}
	...
	for (const FAssetIdentifier& AssetIdentifier : AssetIdentifiers)
	{
		...
		else
		{
			...
			if (AssetPackageName != NAME_None)
			{
				...
				FAssetData FoundData = CurrentRegistrySource->GetAssetByPackageName(AssetPackageName);
			...
			NodeSizeMapData.ExternalPackageCount = 0;
			...
			if (NodeSizeMapData.AssetData.IsValid())
			{
				...
				TSet<FAssetIdentifier> ReferencedAssetIdentifiers;
				...
				if (AssetPackageName != NAME_None)
				{
					if (EditorModule->GetIntegerValueForCustomColumn(NodeSizeMapData.AssetData, GetDefault<USizeMapSettings>()->SizeType, FoundSize, CurrentRegistrySource))
					{
				...
				// External / generated packages discovery
				GatherExternalPackages(NodeSizeMapData.AssetData, AssetPackageName, ChildTreeMapNode, VisitedAssetIdentifiers);
...
void SSizeMap::GatherExternalPackages(const FAssetData CurrentAssetData, FName AssetPackageName, TSharedRef<FTreeMapNodeData> ChildTreeMapNode, TMap<FAssetIdentifier, TSharedPtr<FTreeMapNodeData>>& VisitedAssetIdentifiers)
{
	auto GetExternalPackageSize = [this](const FAssetData& ExternalPackageAsset) -> int64
	{
		int64 Size = 0;
		if (EditorModule->GetIntegerValueForCustomColumn(ExternalPackageAsset, GetDefault<USizeMapSettings>()->SizeType, Size, CurrentRegistrySource))
		{
			return Size;
		}
		return 0;
	...
	auto AddExternalPackageNode = [&](const FAssetData& ExternalPackageAsset, const TSharedPtr<FTreeMapNodeData>& ParentNode)
	{
		FAssetIdentifier AssetIdentifier(ExternalPackageAsset.PackageName);
		if (VisitedAssetIdentifiers.Contains(AssetIdentifier))
		{
			return;
		}

		FTreeMapNodeDataRef TreeNode = MakeShareable(new FTreeMapNodeData());
		ParentNode->Children.Add(TreeNode);
		TreeNode->Parent = ParentNode.Get();

		VisitedAssetIdentifiers.Add(AssetIdentifier, TreeNode);

		FNodeSizeMapData& ExternalPackageNode = NodeSizeMapDataMap.Add(TreeNode);
		ExternalPackageNode.AssetData = ExternalPackageAsset;
		ExternalPackageNode.AssetSize = GetExternalPackageSize(ExternalPackageAsset);
		ExternalPackageNode.bHasKnownSize = true;
	};

	if (CurrentRegistrySource->bIsEditor)
	{
		// Scan external actor and object paths
		TArray<FAssetData> ExternalPackagesAssets;
		{
			FARFilter QueryFilter = FExternalObjectAndActorDependencyGatherer::GetQueryFilter(AssetPackageName);
			CurrentRegistrySource->GetAssets(QueryFilter, ExternalPackagesAssets);
		}

		// Group external packages by class
		if (ExternalPackagesAssets.Num() > 0)
		{
			TMap<FTopLevelAssetPath, TArray<const FAssetData*>> ClassGroups;
			for (const FAssetData& ExternalPackageAsset : ExternalPackagesAssets)
			{
				ClassGroups.FindOrAdd(ExternalPackageAsset.AssetClassPath).Add(&ExternalPackageAsset);
			}

			for (const TPair<FTopLevelAssetPath, TArray<const FAssetData*>>& ClassGroup : ClassGroups)
			{
				const FTopLevelAssetPath ClassPath = ClassGroup.Key;
				const TArray<const FAssetData*> Assets = ClassGroup.Value;

				int64 GroupTotalSize = 0;
				for (const FAssetData* Asset : Assets)
				{
					GroupTotalSize += GetExternalPackageSize(*Asset);
				}

				FTreeMapNodeDataRef GroupNode = MakeShareable(new FTreeMapNodeData());
				ChildTreeMapNode->Children.Add(GroupNode);
				GroupNode->Parent = &ChildTreeMapNode.Get();

				const FString ClassName = ClassPath.GetAssetName().ToString();
				GroupNode->Name = FString::Printf(TEXT("%s  (%d %s, %s)"), *ClassName, Assets.Num(), Assets.Num() == 1 ? TEXT("package") : TEXT("packages"), *SizeMapInternals::MakeBestSizeString(GroupTotalSize, true));
				GroupNode->Size = 0.0f;

				for (const FAssetData* Asset : Assets)
				{
					AddExternalPackageNode(*Asset, GroupNode);
				}
			}

			// Store count for display in FinalizeNodesRecursively
			NodeSizeMapDataMap.FindChecked(ChildTreeMapNode).ExternalPackageCount = ExternalPackagesAssets.Num();
		}
	}
	else
	{
		// Cooked: scan generated packages
		TArray<FAssetData> GeneratedAssets;
		{
			const FString GeneratedPath = AssetPackageName.ToString() / FPackageName::GetGeneratedPackageSubPath();

			FARFilter GeneratedFilter;
			GeneratedFilter.bRecursivePaths = true;
			GeneratedFilter.bIncludeOnlyOnDiskAssets = true;
			GeneratedFilter.PackagePaths.Add(FName(*GeneratedPath));

			CurrentRegistrySource->GetAssets(GeneratedFilter, GeneratedAssets);
		}

		if (GeneratedAssets.Num() > 0)
		{
			// Compute children's total size before adding nodes
			SIZE_T ChildrenTotalSize = 0;
			for (const FAssetData& GenAsset : GeneratedAssets)
			{
				ChildrenTotalSize += GetExternalPackageSize(GenAsset);
			}

			// Put all the generated packages in a single "Generated Packages" container
			FTreeMapNodeDataRef GeneratedGroupNode = MakeShareable(new FTreeMapNodeData());
			ChildTreeMapNode->Children.Add(GeneratedGroupNode);
			GeneratedGroupNode->Parent = &ChildTreeMapNode.Get();
			GeneratedGroupNode->Name = FString::Printf(TEXT("Generated Packages  (%d %s, %s)"), GeneratedAssets.Num(), GeneratedAssets.Num() == 1 ? TEXT("package") : TEXT("packages"), *SizeMapInternals::MakeBestSizeString(ChildrenTotalSize, true));
			GeneratedGroupNode->Size = 0.0f;

			for (const FAssetData& GenAsset : GeneratedAssets)
			{
				AddExternalPackageNode(GenAsset, GeneratedGroupNode);
			}

			// Substract the generated packages size from the generator package size
			{
				FNodeSizeMapData& GeneratorPackageEntry = NodeSizeMapDataMap.FindChecked(ChildTreeMapNode);
				GeneratorPackageEntry.ExternalPackageCount = GeneratedAssets.Num();
				GeneratorPackageEntry.AssetSize -= FMath::Min(GeneratorPackageEntry.AssetSize, ChildrenTotalSize);
			}
		}
	}
}
...
void SSizeMap::FinalizeNodesRecursively(TSharedPtr<FTreeMapNodeData>& Node, const TSharedPtr<FTreeMapNodeData>& SharedRootNode, int32& TotalAssetCount, SIZE_T& TotalSize, bool& bAnyUnknownSizes)
{
	...
	else if (!NodeSizeMapDataMap.Contains(Node.ToSharedRef()))
	{
		// Container node without asset data
		Node->Size = 0.0f;
	}
	else
	{
		...
		// For generated packages, use the package short name instead of the asset name
		FString DisplayName;
		{
			const FString PackageNameStr = NodeSizeMapData.AssetData.PackageName.ToString();
			if (FPackageName::IsUnderGeneratedPackageSubPath(PackageNameStr))
			{
				DisplayName = FPackageName::GetShortName(PackageNameStr);
			}
			else
			{
				DisplayName = NodeSizeMapData.AssetData.AssetName.ToString();
			}
		}
		...
		else
		{
			...
			const FString SizeString = SizeMapInternals::MakeBestSizeString(SubtreeSize + NodeSizeMapData.AssetSize, !bAnyUnknownSizesInSubtree && NodeSizeMapData.bHasKnownSize);

			// Append external/generated package count when present
			FString ExternalPackageSuffix;
			if (NodeSizeMapData.ExternalPackageCount > 0)
			{
				const bool bIsCooked = CurrentRegistrySource && !CurrentRegistrySource->bIsEditor;
				ExternalPackageSuffix = FString::Printf(TEXT(", %d %s %s"), NodeSizeMapData.ExternalPackageCount, bIsCooked ? TEXT("generated") : TEXT("external"), NodeSizeMapData.ExternalPackageCount == 1 ? TEXT("package") : TEXT("packages"));
			}
			...
			if (PrimaryAssetId.IsValid())
			{
				Node->Name = FString::Printf(TEXT("%s  (%s%s)"),
					...
					*SizeString,
					*ExternalPackageSuffix);
			}
			else
			{
				// "Asset name (asset type, size, N external/Generated Package(s))"
				Node->Name = FString::Printf(TEXT("%s  (%s, %s%s)"),
					*DisplayName,
...
void SSizeMap::RefreshMap()
{
	...
	{
		FString OnlyAssetName = RootAssetIdentifiers.FindArbitraryElement()->ToString();
...
void SSizeMap::CopyPackageName() const
{
	const FNodeSizeMapData* NodeSizeMapData = GetCurrentSizeMapData(true);
	if (NodeSizeMapData)
	{
		FPlatformApplicationMisc::ClipboardCopy(*NodeSizeMapData->AssetData.PackageName.ToString());
	}
}
...
void SSizeMap::OnTreeMapNodeRightClicked(FTreeMapNodeData& TreeMapNodeData, const FPointerEvent& MouseEvent)
{
	...
	// Determine the node type to show appropriate context menu entries
	const FNodeSizeMapData* NodeData = NodeSizeMapDataMap.Find(TreeMapNodeData.AsShared());
	const bool bIsContainerNode = (NodeData == nullptr);
	bool bIsExternalPackage = false;
	bool bIsGeneratedPackage = false;
	if (NodeData)
	{
		const FString PackageNameStr = NodeData->AssetData.PackageName.ToString();
		bIsGeneratedPackage = FPackageName::IsUnderGeneratedPackageSubPath(PackageNameStr);
		bIsExternalPackage = !bIsGeneratedPackage &&
			(PackageNameStr.Contains(FPackagePath::GetExternalActorsFolderName()) || 
			 PackageNameStr.Contains(FPackagePath::GetExternalObjectsFolderName()));
	}
	const bool bIsRegularAsset = NodeData && !bIsExternalPackage && !bIsGeneratedPackage;
	...
	MenuBuilder.BeginSection(TEXT("Asset"), LOCTEXT("AssetSectionLabel", "Asset"));
	{
		...
		if (!bIsContainerNode)
		{
			MenuBuilder.AddMenuEntry(FAssetManagerEditorCommands::Get().CopyPaths);
		}
	}
	...
	{
		if (bIsRegularAsset || bIsExternalPackage)
		{
