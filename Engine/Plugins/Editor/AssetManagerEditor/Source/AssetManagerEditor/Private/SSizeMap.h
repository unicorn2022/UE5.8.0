...
protected:
	...
	struct FNodeSizeMapData
	{
		...
		/** Number of external/generated packages discovered for this asset */
	...
	void GatherDependenciesRecursively(TSharedPtr<class FAssetThumbnailPool>& AssetThumbnailPool, TMap<FAssetIdentifier, TSharedPtr<class FTreeMapNodeData>>& VisitedAssetIdentifiers, const TSet<FAssetIdentifier>& AssetIdentifiers, const FPrimaryAssetId& FilterPrimaryAsset, const TSharedPtr<FTreeMapNodeData>& ParentTreeMapNode, TSharedPtr<FTreeMapNodeData>& SharedRootNode, int32& NumAssetsWhichFailedToLoad);

	/** Gather all external/generated packages tied to the provided asset */
	void GatherExternalPackages(const FAssetData CurrentAssetData, FName AssetPackageName, TSharedRef<FTreeMapNodeData> ChildTreeMapNode, TMap<FAssetIdentifier, TSharedPtr<FTreeMapNodeData>>& VisitedAssetIdentifiers);
	...
	/** Platform / Registry Source Combo Box */
	TSharedRef<SWidget> GenerateSourceComboItem(TSharedPtr<FString> InItem) const;
	...
	TSharedPtr<SComboBox<TSharedPtr<FString>>> SourceComboBoxWidget;

	/** Plugin Filter Combo Box — sets plugin assets as root identifiers */
	TSharedRef<SWidget> GeneratePluginComboItem(TSharedPtr<FString> InItem) const;
	void HandlePluginComboChanged(TSharedPtr<FString> Item, ESelectInfo::Type SelectInfo);
	FText GetPluginComboText() const;

	/** Clears root identifiers and replaces them with all assets from PluginName */
	void SetRootAssetIdentifiersFromPlugin(const FString& PluginName);

	TArray<TSharedPtr<FString>> PluginComboList;
	TSharedPtr<SComboBox<TSharedPtr<FString>>> PluginComboBoxWidget;
	FString CurrentPluginSelection; // Empty string = no plugin filter (None)

	/** Our tree map widget */
	TSharedPtr<class STreeMap> TreeMapWidget;

	/** The assets we were asked to look at */
	TSet<FAssetIdentifier> RootAssetIdentifiers;
