...
namespace InternalEditorLevelLibrary
{
	...
	bool IsAssetRegistryModuleLoading()
	{
		...
		{
			UE_LOGF(LogEditorScripting, Error, "The AssetRegistry is currently loading.");
			return false;
		}
		return true;
	}
	...
	bool ValidateSourceAndDestinationForOperation(const FString& SourceDirectoryPath, const FString& DestinationDirectoryPath, bool bValidIfOnlyAllAssetCanBeOperatedOn, const TCHAR* CommandName, FValidatedPaths& OutValidatedPaths, FValidatedObjectInfos& OutObjectInfos)
	{
		...
		{
			UE_LOGF(LogEditorScripting, Error, "%ls. Failed to convert the source path. %ls", CommandName, *FailureReason);
			return false;
		}
		...
		{
			UE_LOGF(LogEditorScripting, Error, "%ls. Failed to convert the source path '%ls' to a full path. Was it rooted?", CommandName, *OutValidatedPaths.SourceValidDirectoryPath);
			return false;
		}
		...
		{
			UE_LOGF(LogEditorScripting, Error, "%ls. Failed to convert the destination path. %ls", CommandName, *FailureReason);
			return false;
		}
		...
		{
			UE_LOGF(LogEditorScripting, Error, "%ls. Failed to convert the destination path '%ls' to a full path. Was it rooted?", CommandName, *OutValidatedPaths.DestinationFilePath);
			return false;
		}
		...
		{
			UE_LOGF(LogEditorScripting, Error, "%ls. The source directory do not exist.", CommandName);
			return false;
		}
		...
		if (!IFileManager::Get().DirectoryExists(*OutValidatedPaths.DestinationFilePath))
		{
			...
			{
				UE_LOGF(LogEditorScripting, Error, "%ls. The destination directory could not be created.", CommandName);
				return false;
			}
		}
		...
		{
			UE_LOGF(LogEditorScripting, Error, "%ls. Failed to validate all assets.", CommandName);
			for (const FString& Reason : FailureReasons)
			{
				UE_LOGF(LogEditorScripting, Error, "  %ls", *Reason);
			}
			return false;
		}
		...
		if (OutObjectInfos.PreviousLoadedAssets.Num())
		{
			for (UObject* Object : OutObjectInfos.PreviousLoadedAssets)
			{
				...
				{
					UE_LOGF(LogEditorScripting, Error, "%ls. Failed to validate the destination for asset '%ls'. %ls", CommandName, *Object->GetName(), *FailureReason);
				...
				{
					UE_LOGF(LogEditorScripting, Error, "%ls. Failed to validate the destination for asset '%ls'. There's alreay an asset at the destination.", CommandName, *NewAssetPackageName);
					OutObjectInfos.Reset();
					return false;
				}
...
void UEditorAssetLibrary::SyncBrowserToObjects(const TArray<FString>& AssetPaths)
{
	...
	if (GEditor)
	{
		...
		for (const FString& AssetPath : AssetPaths)
		{
			...
			{
				UE_LOGF(LogEditorScripting, Warning, "SyncBrowserToObjects. Cannot sync %ls : %ls", *AssetPath, *FailureReason);
			}
		}
		if (Assets.Num() > 0)
		{
