...
namespace EditorScriptingUtils
{
	bool IsInEditorAndNotPlaying()
	{
		if (!IsInGameThread())
		{
			UE_LOGF(LogEditorScripting, Error, "You are not on the main thread.");
			return false;
		}
		if (!GIsEditor)
		{
			UE_LOGF(LogEditorScripting, Error, "You are not in the Editor.");
			return false;
		}
		if (GEditor->PlayWorld || GIsPlayInEditorWorld)
		{
			UE_LOGF(LogEditorScripting, Error, "The Editor is currently in a play mode.");
			return false;
		}
		return true;
	}
