...
void UEditorLevelLibrary::ReplaceSelectedActors(const FString& InAssetPath)
{
	...
	{
		UE_LOGF(LogEditorScripting, Error, "ReplaceSelectedActors: Asset path is empty.");
		return;
	}
	...
	{
		UE_LOGF(LogEditorScripting, Error, "ReplaceSelectedActors: Asset path not found: %ls. %ls", *InAssetPath, *OutFailureReason);
		return;
	}
