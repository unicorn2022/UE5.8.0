...
class UNewPluginPythonOptionsData;
...
private:
	...
	/**
	 * Get the actual name of the plugin that will be created, derived from the selected template's
	 * .uplugin filename (replacing the PLUGIN_NAME token with the user-entered name). Falls back to
	 * the user-entered name if the template does not use the token.
	 */
	FString GetExpandedPluginName() const;
...
private:
	...
	/** Additional user-defined descriptor data */
	TWeakObjectPtr<UNewPluginPythonOptionsData> PythonOptionsData;
