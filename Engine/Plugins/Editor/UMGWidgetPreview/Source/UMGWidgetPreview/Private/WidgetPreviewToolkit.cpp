...
namespace UE::UMGWidgetPreview::Private
{
	...
	void FWidgetPreviewToolkit::SaveAsset_Execute()
	{
		...
		for (UObject* Object : ObjectsToSave)
		{
			if (Object == nullptr)
			{
				...
				UE_LOGF(LogWidgetPreview, Log, "Invalid preview to save: %ls", (Object != nullptr) ? *Object->GetFullName() : TEXT("Null Object"));
			}
			else
			{
	...
	FSimpleMulticastDelegate& FWidgetPreviewToolkit::OnMessageChanged()
	{
		return OnMessageChangedDelegate;
	}
	...
	bool FWidgetPreviewToolkit::SetState(FWidgetPreviewToolkitStateBase* InNewState)
	{
	...
	void FWidgetPreviewToolkit::ResolveState()
	{
		...
		bool bForceMessageUpdate = false;
		...
		const bool bStateChanged = SetState(NewState);

		if (!bStateChanged && bForceMessageUpdate)
		{
			OnMessageChanged().Broadcast();
		}
	}
