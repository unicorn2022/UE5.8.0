...
void FAssetFileContextMenu::MakeAssetActionsSubMenu(UToolMenu* Menu)
{
	...
	// ADVANCED ACTIONS
	{
		...
		if (bCanUsePropertyMatrix)
		{
			...
			{
				if (Asset.AssetClassPath == UMaterialInstanceConstant::StaticClass()->GetClassPathName() || Asset.AssetClassPath == UMaterialFunction::StaticClass()->GetClassPathName() || Asset.AssetClassPath == UMaterialFunctionInstance::StaticClass()->GetClassPathName())
				{
...
void FAssetFileContextMenu::ExecuteResetLocalizationId()
{
	...
	for (const FAssetData& AssetData : SelectedAssets)
	{
		...
		if (Asset)
		{
			if (!TextNamespaceUtil::HasDeterministicPackageNamespace(Asset))
			{
				Asset->Modify();
				TextNamespaceUtil::ResetPackageNamespace(Asset);
			}
		}
	}
}
