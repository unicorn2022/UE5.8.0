...
{
	return ContentBrowserDataUtils::IsPrimaryAsset(InAssetData);
}

bool IsPrimaryAsset(UObject* InObject)
{
	return ContentBrowserDataUtils::IsPrimaryAsset(InObject);
}
...
bool CanRenameAssetFileItem(IAssetTools* InAssetTools, const FContentBrowserAssetFileItemDataPayload& InAssetPayload, const bool bCheckUniqueName, const FString* InNewName, const bool InIsTempoarary, FText* OutErrorMsg)
{
	...
	if (InNewName && (InIsTempoarary || InNewName->Len() >= NAME_SIZE || InAssetPayload.GetAssetData().AssetName != FName(**InNewName))) // Deliberately ignore case here to allow case-only renames of existing assets
	{
...
bool RenameAssetFileItem(IAssetTools* InAssetTools, const FContentBrowserAssetFileItemDataPayload& InAssetPayload, const FString& InNewName)
{
	...
	if (UObject* Asset = InAssetPayload.LoadAsset({ ULevel::LoadAllExternalObjectsTag }))
	{
		TArray<UObject*> AssetsToRename = {Asset};
		FOnPreDestructiveAssetActionArgs PreDestructiveArgs(AssetsToRename, EDestructiveAssetActions::AssetRename);
		FEditorDelegates::OnPreDestructiveAssetActionWithArgs.Broadcast(PreDestructiveArgs);
		if (!PreDestructiveArgs.Result.bSuccess)
		{
			UE_LOGF(LogContentBrowserAssetDataSource, Warning, "%ls", *PreDestructiveArgs.Result.ErrorMessage);
			return false;
		}
...
bool MoveAssetFileItems(TArrayView<const TSharedRef<const FContentBrowserAssetFileItemDataPayload>> InAssetPayloads, const FName InDestPath)
{
	...
	AssetsToMove.Reserve(InAssetPayloads.Num());

	TArray<FTopLevelAssetPath> Destinations;
	Destinations.Reserve(InAssetPayloads.Num());

	const FString DestPath = InDestPath.ToString();
	for (const TSharedRef<const FContentBrowserAssetFileItemDataPayload>& AssetPayload : InAssetPayloads)
	{
		// We need to potentially load the asset in order to duplicate it
		if (UObject* Asset = AssetPayload->LoadAsset({ ULevel::LoadAllExternalObjectsTag }))
		{
			...
			FSoftObjectPath SourceAssetPath(Asset);
			FNameBuilder SourcePackagePath(SourceAssetPath.GetLongPackageFName());
			FNameBuilder DestPackagePath;
			DestPackagePath.Append(DestPath);
			FPathViews::AppendPath(DestPackagePath, FPathViews::GetBaseFilename(SourcePackagePath.ToView()));
			FTopLevelAssetPath DestTopLevelAssetPath(FName(DestPackagePath.ToView()), SourceAssetPath.GetAssetFName());
			Destinations.Add(DestTopLevelAssetPath);
		}
	}
	...
	{
		AssetViewUtils::MoveAssets(AssetsToMove, DestPath);
		return true;
	}
	...
	static const FName ParentClassTag = FName("ParentClass");
	{
		...
		if (InAssetData.AssetClassPath == BlueprintAssetClass)
		{
			...
			if (ParentClassRef.IsSet())
			{
				...
				{
					UE_LOGF(LogContentBrowserAssetDataSource, Warning, "Unable to convert short ParentClass name \"%ls\" to path name", *ParentClassName);
				}
			}
		}
...
bool CanModifyAssetAccessSpecifier(IAssetTools* InAssetTools, const UContentBrowserDataSource* InOwnerDataSource, const FContentBrowserItemData& InItem)
{
	...
	if (TSharedPtr<const FContentBrowserAssetFileItemDataPayload> AssetPayload = GetAssetFileItemPayload(InOwnerDataSource, InItem))
	{
		...
		return InAssetTools->CanAssetBePublic(AssetPath, nullptr);
	}
