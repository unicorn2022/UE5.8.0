...
void UContentBrowserAssetDataSource::Initialize(const bool InAutoRegister)
{
	...
	// Extend content browser commands
	TArray<FContentBrowserCommandExtender>& CommandExtenders = ContentBrowserModule->GetAllContentBrowserCommandExtenders();
	CommandExtenders.Add(FContentBrowserCommandExtender::CreateUObject(this, &UContentBrowserAssetDataSource::OnExtendContentBrowserCommands));
...
void UContentBrowserAssetDataSource::Shutdown()
{
	...
	if (FContentBrowserModule* ContentBrowserModulePtr = FModuleManager::GetModulePtr<FContentBrowserModule>(TEXT("ContentBrowser")))
	{
		TArray<FContentBrowserCommandExtender>& CommandExtenders = ContentBrowserModulePtr->GetAllContentBrowserCommandExtenders();
		CommandExtenders.RemoveAll([this](const FContentBrowserCommandExtender& Delegate) { return Delegate.GetUObject() == this; });
	}
...
bool UContentBrowserAssetDataSource::CanRenameItem(const FContentBrowserItemData& InItem, const FString* InNewName, const IContentBrowserHideFolderIfEmptyFilter* HideFolderIfEmptyFilter, FText* OutErrorMsg)
{
	...
	if (InNewName != nullptr && HideFolderIfEmptyFilter != nullptr)
	{
		if (TSharedPtr<const FContentBrowserAssetFolderItemDataPayload> FolderPayload = GetAssetFolderItemPayload(InItem))
		{
			...
			if (NewInternalPathString.Len() < NAME_SIZE) // If the path is too long, CanRenameItem will fail anyways, let it produce the error message.
			{
...
bool UContentBrowserAssetDataSource::Legacy_TryConvertAssetDataToVirtualPath(const FAssetData& InAssetData, const bool InUseFolderPaths, FName& OutPath)
{
	return InAssetData.AssetClassPath != FTopLevelAssetPath(TEXT("/Script/CoreUObject"), TEXT("Class")) // Ignore legacy class items
		&& ContentBrowserAssetData::IsPrimaryAsset(InAssetData)
PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
void UContentBrowserAssetDataSource::OnAssetsAdded(TConstArrayView<FAssetData> InAssets)
{
	...
	ParallelFor(InAssets.Num(),
		[this, &Cache, &InAssets, &Updates](int32 Index)
		{
			...
			UE_LOGF(LogContentBrowserAssetDataSource, VeryVerbose, "OnAssetsAdded: %ls", *WriteToString<256>(InAssetData.GetSoftObjectPath()));
...
void UContentBrowserAssetDataSource::OnAssetRemoved(const FAssetData& InAssetData)
{
	...
	if (ContentBrowserAssetData::IsPrimaryAsset(InAssetData))
	{
		UE_LOGF(LogContentBrowserAssetDataSource, VeryVerbose, "OnAssetRemoved: %ls", *WriteToString<256>(InAssetData.GetSoftObjectPath()));
...
void UContentBrowserAssetDataSource::OnAssetRenamed(const FAssetData& InAssetData, const FString& InOldObjectPath)
{
	...
	if (ContentBrowserAssetData::IsPrimaryAsset(InAssetData))
	{
		UE_LOGF(LogContentBrowserAssetDataSource, VeryVerbose, "OnAssetRenamed: %ls", *WriteToString<256>(InAssetData.GetSoftObjectPath()));
...
void UContentBrowserAssetDataSource::OnAssetUpdated(const FAssetData& InAssetData)
{
	...
	if (ContentBrowserAssetData::IsPrimaryAsset(InAssetData))
	{
		UE_LOGF(LogContentBrowserAssetDataSource, VeryVerbose, "OnAssetUpdated: %ls", *WriteToString<256>(InAssetData.GetSoftObjectPath()));
...
void UContentBrowserAssetDataSource::OnAssetUpdatedOnDisk(const FAssetData& InAssetData)
{
	...
	if (ContentBrowserAssetData::IsPrimaryAsset(InAssetData))
	{
		UE_LOGF(LogContentBrowserAssetDataSource, VeryVerbose, "OnAssetUpdatedOnDisk: %ls", *WriteToString<256>(InAssetData.GetSoftObjectPath()));
...
void UContentBrowserAssetDataSource::OnPathsRemoved(TConstArrayView<FStringView> Paths)
{
	...
	for (FStringView InPath : Paths)
	{
		...
		const FStringView MountPointWithLeadingSlash = FPathViews::GetMountPointNameFromPath(InPath, nullptr, false);
		if (FPackageName::MountPointExists(MountPointWithLeadingSlash))
		{
			QueueItemDataUpdate(FContentBrowserItemDataUpdate::MakeItemRemovedUpdate(CreateAssetFolderItem(PathName)));
		}
		else
		{
			// Required to avoid loss of favorites for plugins being unmounted
			QueueItemDataUpdate(FContentBrowserItemDataUpdate::MakeItemUnmountedUpdate(CreateAssetFolderItem(PathName)));
		}
...
bool UContentBrowserAssetDataSource::SetAssetFolderAttributes(const FName InPath, const EContentBrowserFolderAttributes InAttributesToSet)
{
	if (InAttributesToSet != EContentBrowserFolderAttributes::None)
	{
		...
		if (bHasChanged)
		{
			...
			UE_LOGF(LogContentBrowserAssetDataSource, Verbose, "Updated folder attributes: %ls %ls", *WriteToString<256>(InPath), *WriteToString<256>(NewAttributes));
		}
		return bHasChanged;
	}
...
void UContentBrowserAssetDataSource::OnNewAssetRequested(const FName InPath, TWeakObjectPtr<UClass> InFactoryClass, UContentBrowserDataMenuContext_AddNewMenu::FOnBeginItemCreation InOnBeginItemCreation)
{
	...
	if (ensure(!InPath.IsNone()) && ensure(FactoryClass) && ensure(InOnBeginItemCreation.IsBound()))
	{
		...
		if (NewFactory->ConfigureProperties())
		{
			...
			const TOptional<FString> DefaultAssetName = AssetTools->GetDefaultAssetNameForClass(NewFactory->GetSupportedClass(), NewFactory);
			const FString BaseAssetName = DefaultAssetName.Get(NewFactory->GetDefaultNewAssetName());
...
void UContentBrowserAssetDataSource::OnBeginCreateAsset(const FName InDefaultAssetName, const FName InPackagePath, UClass* InAssetClass, UFactory* InFactory, UContentBrowserDataMenuContext_AddNewMenu::FOnBeginItemCreation InOnBeginItemCreation)
{
	...
	// This will create a content browser item, which will trigger the factory to create the actual object upon rename completion
	auto CreateContentBrowserItemWithCreationPayload = [this, InDefaultAssetName, InPackagePath, InAssetClass, InFactory, ClassToUse, InOnBeginItemCreation]()
	{
	...
	{
		FOnFactoryConfigurePropertiesAsyncComplete OnConfigurePropertiesComplete = FOnFactoryConfigurePropertiesAsyncComplete::CreateLambda([CreateContentBrowserItemWithCreationPayload](UFactory* Factory)
		{
			CreateContentBrowserItemWithCreationPayload();
		});

		// At this point, the configuration process is completely handed off to the factory. It's the factory's responsibility to call Complete or Cancel.
		// Complete will call the regular content browser creation code path.
		InFactory->ConfigurePropertiesAsync(OnConfigurePropertiesComplete, FOnFactoryConfigurePropertiesAsyncCancelled());
	}
	else
	{
		CreateContentBrowserItemWithCreationPayload();
...
void UContentBrowserAssetDataSource::OnExtendContentBrowserCommands(TSharedRef<FUICommandList> InCommandList, FOnContentBrowserGetSelection InGetSelectionDelegate)
{
	const FContentBrowserAssetDataSourceCommands& AssetDataSourceCommands = FContentBrowserAssetDataSourceCommands::Get();

	using namespace UE::Editor::ContentBrowserAssetDataSource;

	InCommandList->MapAction(
		AssetDataSourceCommands.CaptureThumbnail,
		FExecuteAction::CreateWeakLambda(this, [InGetSelectionDelegate]()
			{
				TArray<FAssetData> SelectedAssets;
				TArray<FString> SelectedFolders;
				InGetSelectionDelegate.ExecuteIfBound(SelectedAssets, SelectedFolders);
				const int32 Result = CaptureThumbnail(SelectedAssets);

				// Notify user
				FSlateNotificationManager& NotificationManager = FSlateNotificationManager::Get();
				FNotificationInfo Info(
					FText::FormatOrdered(
						LOCTEXT("CaptureThumbnailNotification", "Capturing thumbnail(s) for {0} asset(s)"), 
		...
		FCanExecuteAction::CreateWeakLambda(this, [InGetSelectionDelegate]()->bool
			{
				TArray<FAssetData> SelectedAssets;
				TArray<FString> SelectedFolders;
				InGetSelectionDelegate.ExecuteIfBound(SelectedAssets, SelectedFolders);
				return CanCaptureThumbnail(SelectedAssets);
			}
		)
	);
}
