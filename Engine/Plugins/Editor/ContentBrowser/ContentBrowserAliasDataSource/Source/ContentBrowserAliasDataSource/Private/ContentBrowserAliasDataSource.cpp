...
bool UContentBrowserAliasDataSource::GetItemAliasedPath(const FContentBrowserItemData& InItemData, FSoftObjectPath& OutObjectPath) const
{
	if (InItemData.GetOwnerDataSource() == this && InItemData.IsFile())
	{
		if (const TSharedPtr<const FContentBrowserAliasItemDataPayload> AliasPayload = StaticCastSharedPtr<const FContentBrowserAliasItemDataPayload>(InItemData.GetPayload()))
		{
			if (AliasPayload->Alias.Key.IsValid())
			{
				OutObjectPath = AliasPayload->Alias.Key;
				return true;
			}
		}
	}
	return Super::GetItemAliasedPath(InItemData, OutObjectPath);
}
...
void UContentBrowserAliasDataSource::AddAliasImpl(const FAssetData& Asset, const AliasType& AliasToAdd, const bool bInIsFromMetaData, const bool bSkipPrimaryAssetValidation)
{
	...
	{
		UE_LOGF(LogContentBrowserAliasDataSource, Warning, "Cannot add alias %ls for %ls because: %ls", *Alias.ToString(), *Asset.GetObjectPathString(), Reason);
	};
	...
	//UE_LOGF(LogContentBrowserAliasDataSource, VeryVerbose, "Adding alias %ls for %ls", *AliasString, *Asset.ObjectPath.ToString());
}
...
void UContentBrowserAliasDataSource::LogAliases() const
{
	...
	for (const FContentBrowserUniqueAlias& Alias : Aliases)
	{
		...
		UE_LOGF(LogContentBrowserAliasDataSource, Log, "%ls", StringBuilder.ToString());
	}
}
