...
public:
	...
	void AddDomain(TSharedPtr<FDomainData> Domain, FStringView RemainingPath)
	{
		...
		{
			UE_LOGF(LogAssetReferenceRestrictions, Verbose, "Assigning path %ls to domain %ls", *GetPath(), *Domain->UserFacingDomainName.ToString());
	...
	void DebugPrint(const FString& PreceedingPath, int32 Depth) const
	{
		...
		UE_LOGF(LogAssetReferenceRestrictions, Log, "[%d] Path %ls%ls%ls", Depth, *PreceedingPath, *ChildrenSuffix, *DomainSuffix);
...
void FDomainDatabase::RebuildFromScratch()
{
	...
	for (const FARPDomainDefinitionByContentRoot& PathRule : Settings->AdditionalDomains)
	{
		if (PathRule.IsValid())
		{
			...
			for(FName PackageName : PathRule.SpecificAssets)
			{
				...
				{
					UE_LOGF(LogAssetReferenceRestrictions, Error, "Invalid specific asset path '%ls' for domain '%ls'", *PackageName.ToString(), *PathRule.DomainName);
				}
			}
	...
	for (const TSharedRef<IPlugin>& Plugin : EnabledPlugins)
	{
		...
		else if (Plugin->CanContainContent() || Plugin->CanContainVerse())
		{
	...
	for (const auto& KVP : DomainNameMap)
	{
		...
		for (FName PackageName : Domain->SpecificAssetPackages)
		{
			...
			{
				UE_LOGF(LogAssetReferenceRestrictions, Warning, "Overriding existing specific domain '%ls' of package '%ls' with new doman '%ls'", *(*ExistingDomain)->UserFacingDomainName.ToString(), *PackageName.ToString(), *Domain->UserFacingDomainName.ToString());
			}
...
void FDomainDatabase::Init()
{
	...
	UE_LOGF(LogAssetReferenceRestrictions, Log, "Finished building domain database");
#endif
...
void FDomainDatabase::UpdateIfNecessary()
{
	if (bDatabaseOutOfDate)
	{
		UE_LOGF(LogAssetReferenceRestrictions, Display, "Updating asset referencing domain DB due to plugin or rule changes");
...
void FDomainDatabase::BuildDomainFromPlugin(TSharedRef<IPlugin> Plugin)
{
	...
	for (const FPluginReferenceDescriptor& Dependencies : Plugin->GetDescriptor().Plugins)
	{
		...
		{
			TSharedPtr<IPlugin> PluginWeDependOn = IPluginManager::Get().FindEnabledPlugin(Dependencies.Name);
			if (PluginWeDependOn.IsValid() && (PluginWeDependOn->CanContainContent() || PluginWeDependOn->CanContainVerse()))
			{
...
void FDomainDatabase::ValidateAllDomains()
{
	for (const auto& KVP : DomainNameMap)
	{
		...
		if (!TestDomain->IsValid())
		{
			...
			UE_LOGF(LogAssetReferenceRestrictions, Error, "Asset domain %ls was referenced by [%ls] but wasn't found (perhaps it is a plugin that was disabled?)", *KVP.Key, *FString::Join(DomainsThatReferenceMe, TEXT(", ")));
		}
	}
}
...
void FDomainDatabase::DebugPrintAllDomains()
{
	...
	{
		TSharedPtr<FDomainData> Domain = KVP.Value;
		UE_LOGF(LogAssetReferenceRestrictions, Log, "Domain: %ls (%ls)", *KVP.Key, *Domain->UserFacingDomainName.ToString());
		UE_LOGF(LogAssetReferenceRestrictions, Log, "Roots: %ls", *FString::Join(Domain->DomainRootPaths, TEXT(", ")));
		UE_LOGF(LogAssetReferenceRestrictions, Log, "CanSee: %ls", *FString::JoinBy(Domain->DomainsVisibleFromHere, TEXT(", "), [](TSharedPtr<FDomainData> VisibleDomain) { return VisibleDomain->UserFacingDomainName.ToString(); }));
		UE_LOGF(LogAssetReferenceRestrictions, Log, "");
	}
...
#if UE_ASSET_DOMAIN_FILTERING_DEBUG_LOGGING
	UE_LOGF(LogAssetReferenceRestrictions, Verbose, "Asset %ls belongs to domain %ls",
