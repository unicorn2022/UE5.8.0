		...
		EnumIndex != INDEX_NONE)
	{
		...
		else
		{
			...
			for (int32 Index{2}; Index < Pins.Num(); ++Index)
			{
				...
				{
					UE_LOGF(LogHttpBlueprintEditor, Warning, "Pin was invalid");
					break;
				}
	...
	TUniqueFunction<void(UEdGraphPin*, int32)>&& PinFunc)
{
	...
	for (UEdGraphPin*& Pin : InPins)
	{
		...
		{
			UE_LOGF(LogHttpBlueprintEditor, Warning, "Pin %ls wasn't valid.", *Pin->GetName());
			continue;
		}
		...
		{
			UE_LOGF(LogHttpBlueprintEditor, Warning, "Pin %ls wasn't affected.", *Pin->GetName());
		}
	}
}
