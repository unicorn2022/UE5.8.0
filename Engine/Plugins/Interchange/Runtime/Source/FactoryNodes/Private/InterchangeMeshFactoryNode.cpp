...
FString UInterchangeMeshFactoryNode::GetKeyDisplayName(const UE::Interchange::FAttributeKey& NodeAttributeKey) const
{
	...
	{
		return KeyDisplayName = TEXT("LOD Dependencies Count");
	}
	else if (NodeAttributeKeyString.StartsWith(UE::Interchange::FMeshFactoryNodeStaticData::GetLodDependenciesBaseKey().ToString()))
	{
		return KeyDisplayName = UE::Interchange::BuildDisplayNameForArrayKey(TEXT("LOD Dependencies Index "), NodeAttributeKeyString);
	}
	...
	{
		return KeyDisplayName = TEXT("Nanite Assembly Part Dependencies Count");
	}
	...
	{
		return KeyDisplayName = UE::Interchange::BuildDisplayNameForArrayKey(TEXT("Nanite Assembly Part Dependencies Index "), NodeAttributeKeyString);
	}
...
void UInterchangeMeshFactoryNode::GetSlotMaterialDependencies(TMap<FString, FString>& OutMaterialDependencies) const
{
	...
	else
	{
		...
		{
			if (!MaterialUids.Contains(MaterialDependency.Value))
			{
