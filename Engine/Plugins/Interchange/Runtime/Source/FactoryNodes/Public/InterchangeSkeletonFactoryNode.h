...
public:
	...
	{
		SkeletalMeshFactoryNodeUids.Initialize(Attributes.ToSharedRef(), "__SkeletalMeshFactoryNodeUids__");
	}
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Skeleton",
		meta = (DeprecatedFunction, DeprecationMessage = "Use GetCustomSkeletalMeshFactoryNodeUidAtIndex (or GetCustomSkeletalMeshFactoryNodeUids) instead."))
	bool GetCustomSkeletalMeshFactoryNodeUid(FString& AttributeValue) const
	{
		if (GetCustomSkeletalMeshFactoryNodeCount() > 0)
		{
			GetCustomSkeletalMeshFactoryNodeUidAtIndex(0, AttributeValue);
			return true;
		}
	...
	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Skeleton")
	int32 GetCustomSkeletalMeshFactoryNodeCount() const
	{
		return SkeletalMeshFactoryNodeUids.GetCount();
	}

	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Skeleton")
	void GetCustomSkeletalMeshFactoryNodeUids(TArray<FString>& OutDependencies) const
	{
		SkeletalMeshFactoryNodeUids.GetItems(OutDependencies);
	}

	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Skeleton")
	void GetCustomSkeletalMeshFactoryNodeUidAtIndex(const int32 Index, FString& OutDependency) const
	{
		SkeletalMeshFactoryNodeUids.GetItem(Index, OutDependency);
	}

	UFUNCTION(BlueprintCallable, Category = "Interchange | Node | Skeleton")
	bool AddCustomSkeletalMeshFactoryNodeUid(const FString& InDependency)
	{
		return SkeletalMeshFactoryNodeUids.AddItem(InDependency);
	}
...
private:
	...
	const UE::Interchange::FAttributeKey Macro_CustomSkeletalMeshFactoryNodeUidKey = UE::Interchange::FAttributeKey(TEXT("SkeletalMeshFactoryNodeUids"));
	
	UE::Interchange::TArrayAttributeHelper<FString> SkeletalMeshFactoryNodeUids;
