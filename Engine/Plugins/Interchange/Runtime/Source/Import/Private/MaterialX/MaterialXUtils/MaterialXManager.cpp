...
#include "MaterialX/MaterialXUtils/MaterialXGltfPbr.h"
...
FMaterialXManager::FMaterialXManager()
	: MatchingInputNames{
	   ...
	   {{TEXT(""),                            TEXT("lumacoeffs")},  ExpressionInput(UE::Expressions::Inputs::LuminanceFactors)},
	   ...
	   {{mx::Category::Fractal2D,             TEXT("texcoord")},    ExpressionInput(UE::Expressions::Inputs::Position)},
	...
	, MaterialXContainerDelegates {
		...
		{mx::Category::GltfPbr, FMaterialXManager::FOnGetMaterialXInstance::CreateStatic(&FMaterialXGltfPbr::MakeInstance)},
	...
	, MatchingMaterialFunctions {
		...
		{mx::Category::Rotate3D,						FMaterialXMaterialFunction{TInPlaceType<FString>{}, MaterialFunctionPackage(UE::MaterialFunctions::Path::MxRotate3D)}},
		// ColorTransform																					
...
namespace
{
	bool ValidateDocument(MaterialX::DocumentPtr Document, const UInterchangeTranslatorBase* Translator = nullptr)
	{
		...
		if (std::string MaterialXMessage; !Document->validate(&MaterialXMessage))
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Error, "%ls", UTF8_TO_TCHAR(MaterialXMessage.c_str()));
			}
			return false;
		}
		...
		for (mx::ElementPtr Elem : Document->traverseTree())
		{
			...
			if (Node)
			{
				...
				if (!Node->getNodeDef())
				{
					...
					{
						UE_LOGF(LogInterchangeImport, Error, "<%ls> has no matching NodeDef, aborting import...", UTF8_TO_TCHAR(Node->getName().c_str()));
					}
					return false;
				}
				...
				if (!Node->getTypeDef() && Node->getType() != "multioutput")
				{
					...
					{
						UE_LOGF(LogInterchangeImport, Error, "<%ls> has no matching TypeDef, aborting import...", UTF8_TO_TCHAR(Node->getName().c_str()));
					}
					return false;
				}
			}
...
bool FMaterialXManager::Translate(const FString& Filename, UInterchangeBaseNodeContainer& BaseNodeContainer, const UInterchangeTranslatorBase* Translator)
{
	...
	try
	{
		...
		if (LoadedLibs.empty())
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Error, "Couldn't load MaterialX libraries from %ls", UTF8_TO_TCHAR(MaterialXFolder.asString().c_str()));
			}
			return false;
		}
	...
	catch (std::exception& Exception)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Error, "%ls", UTF8_TO_TCHAR(Exception.what()));
		}
	}
...
bool FMaterialXManager::Translate(MaterialX::DocumentPtr Document, UInterchangeBaseNodeContainer& BaseNodeContainer, const UInterchangeTranslatorBase* Translator)
{
			...
			if (Node)
			{
				...
				if (bIsMaterialShader || bIsLightShader)
				{
					...
					if (ShaderTranslator)
					{
						...
						ShaderTranslator->CollectAnalytics(AnalyticsAttributes);
					}
				}
			}
		}
	...
	catch (std::exception& Exception)
	{
		...
		if (Translator)
		{
			...
			Message->Text = FText::Format(LOCTEXT("MaterialXException", "{0}"),
										  FText::FromString(Exception.what()));
			AnalyticsAttributes.Emplace(TEXT("MaterialXFail"), TEXT("[FAILED] MaterialX Document invalid"));
		}
		else
		{
			UE_LOGF(LogInterchangeImport, Error, "%ls", UTF8_TO_TCHAR(Exception.what()));
	...
	if (Translator && Translator->AnalyticsHandler)
	{
		Translator->AnalyticsHandler->Append(TEXT("Interchange.Usage.Import.MaterialX"), AnalyticsAttributes);
	}
		...
		auto ArePackagesLoaded = [](const TArray<FString>& TextPaths) -> bool
		{
			...
			for(const FString& TextPath : TextPaths)
			{
				...
				if(FPackageName::DoesPackageExist(FunctionPath))
				{
					...
					{
						UE_LOGF(LogInterchangeImport, Warning, "Couldn't load %ls", *FunctionPath);
						bAllLoaded = false;
					}
				}
				else
				{
					UE_LOGF(LogInterchangeImport, Warning, "Couldn't find %ls", *FunctionPath);
					bAllLoaded = false;
				}
			}
