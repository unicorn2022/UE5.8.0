...
{
	return GetTexturePayloadData(PayloadKey, AlternateTexturePath, Results, AnalyticsHandler);
}

TOptional<UE::Interchange::FImportImage> UInterchangeMaterialXTranslator::GetTexturePayloadData(const FString& PayloadKey, TOptional<FString>& AlternateTexturePath, UInterchangeResultsContainer* InResults, TObjectPtr<UInterchangeAnalyticsHandlerBase> InAnalyticsHandler)
{
	...
	UE::Interchange::Private::FScopedTranslator ScopedTranslator(Filename, InResults, InAnalyticsHandler);
...
{
	return GetBlockedTexturePayloadData(PayloadKey, AlternateTexturePath, Results, AnalyticsHandler);
}

TOptional<UE::Interchange::FImportBlockedImage> UInterchangeMaterialXTranslator::GetBlockedTexturePayloadData(const FString& PayloadKey, TOptional<FString>& AlternateTexturePath, UInterchangeResultsContainer* InResults, TObjectPtr<UInterchangeAnalyticsHandlerBase> InAnalyticsHandler)
{
	...
	for (const TPair<int32, FString>& Pair : UDIMBlocks)
	{
		...
		UE::Interchange::Private::FScopedTranslator ScopedTranslator(FilenameUDIM, InResults, InAnalyticsHandler);
		const IInterchangeTexturePayloadInterface* TextureTranslator = ScopedTranslator.GetPayLoadInterface<IInterchangeTexturePayloadInterface>();
