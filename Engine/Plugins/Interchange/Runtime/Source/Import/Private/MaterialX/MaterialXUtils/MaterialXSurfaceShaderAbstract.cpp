...
bool FMaterialXSurfaceShaderAbstract::AddLinearColorAttribute(MaterialX::InputPtr Input, const FString& InputChannelName, UInterchangeShaderNode* ShaderNode, const FLinearColor& DefaultValue, int32 OutputIndex)
{
	...
	if(Input && Input->hasValue())
	{
		...
		if(!Value.Equals(DefaultValue))
		{
			...
			if (Input->getType() == mx::Type::Color4)
			{
				OutputIndex = 5; // RGBA output on a VectorParameter is the 6th output
			}
...
bool FMaterialXSurfaceShaderAbstract::AddVectorAttribute(MaterialX::InputPtr Input, const FString& InputChannelName, UInterchangeShaderNode* ShaderNode, const FVector4f& DefaultValue, int32 OutputIndex)
{
	...
	if(Input && Input->hasValue())
	{
		...
		if(!Value.Equals(DefaultValue))
		{
			...
			if (Input->getType() == mx::Type::Vector4)
			{
				OutputIndex = 5; // RGBA output on a VectorParameter is the 6th output
			}
...
bool FMaterialXSurfaceShaderAbstract::ConnectNodeNameOutputToInput(MaterialX::InputPtr InputToConnectedNode, UInterchangeShaderNode* ShaderNode, const FString& ParentInputName)
{
	...
	// for a direct node connection without nodegraph, we need the output of the first node connection
	// for example connecting directly a separate2/3/4 node on a surfaceshader input might miss an entire shader graph
	FString OutputString = !InputToConnectedNode->getOutputString().empty() ? ANSI_TO_TCHAR(InputToConnectedNode->getOutputString().c_str()) : TEXT("out");
...
{
	// We could always use the material function MX_HeightToNormalMap, 
	// though it has been proven that sampling textures to compute a normal map is cheaper and gives better results
	...
	if(mx::InputPtr Input = Connect.UpstreamNode->getInput("in"))
	{
		...
		{
			if(UInterchangeTextureNode* TextureNode = CreateTextureNode<UInterchangeTexture2DNode>(ConnectedNode))
			{
				...
				const FString TextureObjectInputKey = UInterchangeShaderPortsAPI::MakeInputParameterKey(TextureObject::Inputs::Texture.ToString());
				TextureShaderNode->AddStringAttribute(TextureObjectInputKey, TextureNode->GetUniqueID());
				...
				// Create a Texture Property with texel size to have a better quality when sampling the textures for the normalmap
				const FString TexturePropertyNodeName = Connect.UpstreamNode->getName().c_str() + FString{ "_textureproperty" };
				UInterchangeShaderNode* TexturePropertyNode = CreateShaderNode(Connect.UpstreamNode, TexturePropertyNodeName, TextureProperty::Name.ToString());
				TexturePropertyNode->AddInt32Attribute(TextureProperty::Attributes::Property.ToString(), EMaterialExposedTextureProperty::TMTM_TexelSize);
				UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(TexturePropertyNode, TextureProperty::Inputs::Texture.ToString(), TextureShaderNode->GetUniqueID());
				UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(HeightMapNode, NormalFromHeightMap::Inputs::Offset.ToString(), TexturePropertyNode->GetUniqueID());
				
				AddAttributeFromValueOrInterface(Connect.UpstreamNode->getInput("texcoord"), NormalFromHeightMap::Inputs::Coordinates.ToString(), HeightMapNode);

				// The default value in the material function is 8, but the default value for the node is 1
				if (!AddAttributeFromValueOrInterface(Connect.UpstreamNode->getInput("scale"), NormalFromHeightMap::Inputs::Intensity.ToString(), HeightMapNode))
				{
					const FString IntensityInputKey = UInterchangeShaderPortsAPI::MakeInputValueKey(NormalFromHeightMap::Inputs::Intensity.ToString());
					HeightMapNode->AddFloatAttribute(IntensityInputKey, 1.f);
				}
			}
			else
			{
		...
		{
			UInterchangeFunctionCallShaderNode* HeightToNormalMapNode = CreateFunctionCallShaderNode(Connect.UpstreamNode, Connect.UpstreamNode->getName().c_str(), UE::MaterialFunctions::Path::MxHeightToNormalMap);
			
			for(mx::InputPtr InputHeightmap : Connect.UpstreamNode->getInputs())
			{
				AddAttributeFromValueOrInterface(InputHeightmap, GetInputName(InputHeightmap), HeightToNormalMapNode);
			}

			UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(Connect.ParentShaderNode, Connect.InputChannelName, HeightToNormalMapNode->GetUniqueID());
		}
	}
}
...
void FMaterialXSurfaceShaderAbstract::ConnectLuminanceInputToOutput(const FConnectNode& Connect)
{
	using namespace UE::Interchange::Materials::Standard::Nodes;
	UInterchangeShaderNode* LuminanceNode = CreateShaderNode(Connect.UpstreamNode, Connect.UpstreamNode->getName().c_str(), Luminance::Name.ToString());
	AddAttributeFromValueOrInterface(Connect.UpstreamNode->getInput("in"), Luminance::Inputs::Input.ToString(), LuminanceNode);

	if (mx::InputPtr InputCoeffs = Connect.UpstreamNode->getInput("lumacoeffs"); InputCoeffs && InputCoeffs->hasValue())
	{
		mx::Color3 LumaCoeffs = InputCoeffs->getValue()->asA<mx::Color3>();
		LuminanceNode->AddLinearColorAttribute(Luminance::Attributes::LuminanceFactors.ToString(), FLinearColor{ LumaCoeffs[0], LumaCoeffs[1] ,LumaCoeffs[2] });
	}

	UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(Connect.ParentShaderNode, Connect.InputChannelName, LuminanceNode->GetUniqueID());
}

void FMaterialXSurfaceShaderAbstract::ConnectHexTiledInputToOutput(const FConnectNode& Connect)
{
	using namespace UE::Interchange::Materials::Standard::Nodes;

	const bool bNormalTexture = Connect.UpstreamNode->getCategory() == mx::Category::HexTiledNormalMap ? true : false;
	TGuardValue<bool>InputTypeBeingProcessedGuard(bTangentSpaceInput, bNormalTexture);

	if (UInterchangeTextureNode* TextureNode = CreateTextureNode<UInterchangeTexture2DNode>(Connect.UpstreamNode))
	{
		const FString HexTileFunctionPath = Connect.UpstreamNode->getCategory() == mx::Category::HexTiledImage ? UE::MaterialFunctions::Path::MxHexTiledImage : UE::MaterialFunctions::Path::MxHexTiledNormalMap;
		UInterchangeShaderNode* HexTileNode = CreateFunctionCallShaderNode(Connect.UpstreamNode, Connect.UpstreamNode->getName().c_str(), HexTileFunctionPath);
		...
		for (mx::InputPtr Input : Connect.UpstreamNode->getInputs())
		{
			AddAttributeFromValueOrInterface(Input, GetInputName(Input), HexTileNode);
		}

		UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(HexTileNode, TEXT("Texture"), TextureShaderNode->GetUniqueID());
		UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(Connect.ParentShaderNode, Connect.InputChannelName, HexTileNode->GetUniqueID());
	}
	else
	{
		AddAttributeFromValueOrInterface(Connect.UpstreamNode->getInput("default"), Connect.InputChannelName, Connect.ParentShaderNode);
	}
}

void FMaterialXSurfaceShaderAbstract::ConnectLatLongImageInputToOutput(const FConnectNode& Connect)
{
	using namespace UE::Interchange::Materials::Standard::Nodes;

	if (UInterchangeTexture2DNode* TextureNode = Cast<UInterchangeTexture2DNode>(CreateTextureNode<UInterchangeTexture2DNode>(Connect.UpstreamNode)))
	{
		TextureNode->SetForceLongLatCubemap(true);
		UInterchangeShaderNode* LatLongNode = CreateFunctionCallShaderNode(Connect.UpstreamNode, Connect.UpstreamNode->getName().c_str(), UE::MaterialFunctions::Path::MxLatLongImage);
		...
		if (mx::InputPtr Input = Connect.UpstreamNode->getInput("viewdir"))
		{
			AddAttributeFromValueOrInterface(Input, GetInputName(Input), LatLongNode);
		}

		if (mx::InputPtr Input = Connect.UpstreamNode->getInput("rotation"))
		{
			AddAttributeFromValueOrInterface(Input, GetInputName(Input), LatLongNode);
		}

		UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(LatLongNode, TEXT("Texture"), TextureShaderNode->GetUniqueID());
		UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(Connect.ParentShaderNode, Connect.InputChannelName, LatLongNode->GetUniqueID());
	}
	else
	{
		AddAttributeFromValueOrInterface(Connect.UpstreamNode->getInput("default"), Connect.InputChannelName, Connect.ParentShaderNode);
...
void FMaterialXSurfaceShaderAbstract::ConnectFractalInputToOutput(const FConnectNode& Connect)
{
	UInterchangeShaderNode* FractalNode = CreateShaderNode(Connect.UpstreamNode, Connect.UpstreamNode->getName().c_str(), UE::Expressions::Names::MaterialXFractal);
	for (mx::InputPtr Input : Connect.UpstreamNode->getInputs())
	{
		AddAttributeFromValueOrInterface(Input, GetInputName(Input), FractalNode);
	}

	const int32 OutputIndex =  Connect.UpstreamNode->getType() == mx::Type::Vector2 ? 1 
		: Connect.UpstreamNode->getType() == mx::Type::Vector3 || Connect.UpstreamNode->getType() == mx::Type::Color3 ? 2
		: Connect.UpstreamNode->getType() == mx::Type::Vector4 || Connect.UpstreamNode->getType() == mx::Type::Color4 ? 3
		: 0;

	UInterchangeShaderPortsAPI::ConnectOuputToInputByIndex(Connect.ParentShaderNode, Connect.InputChannelName, FractalNode->GetUniqueID(), OutputIndex);
}

bool FMaterialXSurfaceShaderAbstract::ConnectSubsurfaceTransmissionToSurfaceShader(UInterchangeShaderNode* OpenPbrStandarSurfaceShaderNode, EInterchangeMaterialXShaders SurfaceShaderType
																				   , const FName& BaseWeight, const FName& BaseColor
																				   , const FName& TransmissionWeight, const FName& TransmissionColor, const FName& TransmissionDepth, const FName& TransmissionDispersionAbbeNumber, const FName& TransmissionDispersionScale, const FName& TransmissionScatter, const FName& TransmissionScatterAnisotropy
																				   , const FName& SubsurfaceWeight, const FName& SubsurfaceColor, const FName& SubsurfaceRadiusScale
																				   , const FName& FrontMaterialOpaque, const FName& FrontMaterialTranslucent)
{
	using namespace UE::Interchange::Materials;
	using namespace UE::Interchange::MaterialX;
	using namespace UE::Interchange::Materials::Standard::Nodes;

	bool bSubsurfaceTransmission = false;

	UInterchangeShaderPortsAPI::ConnectOuputToInputByName(ShaderGraphNode, Common::Parameters::Refraction.ToString(), OpenPbrStandarSurfaceShaderNode->GetUniqueID(), TEXT("Refraction (IOR)"));

	if (UInterchangeShaderPortsAPI::HasInput(OpenPbrStandarSurfaceShaderNode, TransmissionWeight) &&
		UInterchangeShaderPortsAPI::HasInput(OpenPbrStandarSurfaceShaderNode, SubsurfaceWeight) &&
		UInterchangeShaderPortsAPI::HasInput(OpenPbrStandarSurfaceShaderNode, BaseColor))
	{
		FString SurfaceType = TEXT("Standard Surface");
		if (SurfaceShaderType == EInterchangeMaterialXShaders::OpenPBRSurface)
		{
			SurfaceType = TEXT("OpenPBR");
		}
		AnalyticsAttributes.Emplace(SurfaceType, "Opaque");

		bSubsurfaceTransmission = true;
		// 1. Set the Material as Opaque with Mask Mode
		OpenPbrStandarSurfaceShaderNode->AddInt32Attribute(Attributes::EnumType, IndexSurfaceShaders);
		OpenPbrStandarSurfaceShaderNode->AddInt32Attribute(Attributes::EnumValue, int32(SurfaceShaderType));
		...
		UInterchangeShaderPortsAPI::DisconnectInputFromOutputNode(OpenPbrStandarSurfaceShaderNode, TransmissionWeight.ToString(), TransmissionNodeUid, TransmissionOutputName);
		UInterchangeShaderPortsAPI::DisconnectInput(OpenPbrStandarSurfaceShaderNode, TransmissionColor.ToString());
		UInterchangeShaderPortsAPI::DisconnectInput(OpenPbrStandarSurfaceShaderNode, TransmissionDepth.ToString());
		UInterchangeShaderPortsAPI::DisconnectInput(OpenPbrStandarSurfaceShaderNode, TransmissionDispersionAbbeNumber.ToString());
		UInterchangeShaderPortsAPI::DisconnectInput(OpenPbrStandarSurfaceShaderNode, TransmissionDispersionScale.ToString());
		UInterchangeShaderPortsAPI::DisconnectInput(OpenPbrStandarSurfaceShaderNode, TransmissionScatterAnisotropy.ToString());
		UInterchangeShaderPortsAPI::DisconnectInput(OpenPbrStandarSurfaceShaderNode, TransmissionScatter.ToString());

		//3. Create DitherMask node			
		UInterchangeShaderNode* DitherMaskNode = CreateFunctionCallShaderNode(SurfaceShaderNode, TEXT("DitherMaskNode"), UE::MaterialFunctions::Path::DitherMask);
		...
		{
			UInterchangeShaderPortsAPI::ConnectOuputToInputByName(DitherMaskNode, TEXT("BSDF"), OpenPbrStandarSurfaceShaderNode->GetUniqueID(), FrontMaterialOpaque.ToString());
	...
	else if (UInterchangeShaderPortsAPI::HasInput(OpenPbrStandarSurfaceShaderNode, TransmissionWeight) &&
			 UInterchangeShaderPortsAPI::HasInput(OpenPbrStandarSurfaceShaderNode, SubsurfaceWeight))
	{
		bSubsurfaceTransmission = true;
		ShaderGraphNode->SetCustomBlendMode(EBlendMode::BLEND_TranslucentColoredTransmittance);
		auto ReplaceInputConnection = [this, OpenPbrStandarSurfaceShaderNode](const FString& InputToRemove, const FString& InputName)
			{
				UInterchangeShaderPortsAPI::DisconnectInput(OpenPbrStandarSurfaceShaderNode, InputToRemove);

				FString NodeUid, NodeOutputName;
				UInterchangeShaderPortsAPI::GetInputConnection(OpenPbrStandarSurfaceShaderNode, InputName, NodeUid, NodeOutputName);
				if (const UInterchangeShaderNode* ShaderNode = Cast<UInterchangeShaderNode>(NodeContainer.GetNode(NodeUid)))
				{
					UInterchangeShaderPortsAPI::ConnectOuputToInputByName(OpenPbrStandarSurfaceShaderNode, InputToRemove, ShaderNode->GetUniqueID(), NodeOutputName);
				}
			};

		//1. subsurface_color input now replaces the base_color
		if (UInterchangeShaderPortsAPI::HasInput(OpenPbrStandarSurfaceShaderNode, SubsurfaceColor))
		{
			ReplaceInputConnection(BaseColor.ToString(), SubsurfaceColor.ToString());
		}

		//2. replace base by subsurface
		ReplaceInputConnection(BaseWeight.ToString(), SubsurfaceWeight.ToString());

		//3. set an empirical value to the TransmissionDepth if there isn't any input (otherwise just keep it as is).
		//   see egg_jade in https://github.com/stehrani3d/MaterialEggs
		//   and masonJarWater in https://github.com/DigitalProductionExampleLibrary/OpenPBRShaderPlayground
		if (!UInterchangeShaderPortsAPI::HasInput(OpenPbrStandarSurfaceShaderNode, TransmissionDepth))
		{
			constexpr float TransmissionDepthValue = 3.f;
			OpenPbrStandarSurfaceShaderNode->AddFloatAttribute(UInterchangeShaderPortsAPI::MakeInputValueKey(TransmissionDepth.ToString()), TransmissionDepthValue);
		}

		//4. Connect the SubsurfaceColor to the TransmissionScatter
		FString SubsurfaceColorNodeUid, SubsurfaceColorNodeOutputName;
		if (UInterchangeShaderPortsAPI::GetInputConnection(OpenPbrStandarSurfaceShaderNode, SubsurfaceColor.ToString(), SubsurfaceColorNodeUid, SubsurfaceColorNodeOutputName))
		{
			UInterchangeShaderPortsAPI::ConnectOuputToInputByName(OpenPbrStandarSurfaceShaderNode, TransmissionScatter.ToString(), SubsurfaceColorNodeUid, SubsurfaceColorNodeOutputName);
		}

		//5. Connect the SubsurfaceRadius for Standard Surface, SubsurfaceRadiusScale for OpenPBR to the TransmissionColor
		FString SubsurfaceRadiusNodeUid, SubsurfaceRadiusNodeOutputName;
		if (UInterchangeShaderPortsAPI::GetInputConnection(OpenPbrStandarSurfaceShaderNode, SubsurfaceRadiusScale.ToString(), SubsurfaceRadiusNodeUid, SubsurfaceRadiusNodeOutputName))
		{
			// 6. Multiply the SubsurfaceRadiusScale by the TransmissionColor if any otherwise just plug it to the TransmissionColor
			FString TransmissionColorNodeUid, TransmissionColorNodeOutputName;
			if (UInterchangeShaderPortsAPI::DisconnectInputFromOutputNode(OpenPbrStandarSurfaceShaderNode, TransmissionColor.ToString(), TransmissionColorNodeUid, TransmissionColorNodeOutputName))
			{
				UInterchangeShaderNode* SubsurfaceTransmissionColorNode = CreateShaderNode(SurfaceShaderNode, TEXT("SubsurfaceRadiusTransmissionColorMult"), Multiply::Name.ToString());
				...
				UInterchangeShaderPortsAPI::ConnectOuputToInputByName(SubsurfaceTransmissionColorNode, Multiply::Inputs::B.ToString(), SubsurfaceRadiusNodeUid, SubsurfaceRadiusNodeOutputName);
				UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(OpenPbrStandarSurfaceShaderNode, TransmissionColor.ToString(), SubsurfaceTransmissionColorNode->GetUniqueID());
			}
			else
			{
				UInterchangeShaderPortsAPI::ConnectOuputToInputByName(OpenPbrStandarSurfaceShaderNode, TransmissionColor.ToString(), SubsurfaceRadiusNodeUid, SubsurfaceRadiusNodeOutputName);
			}
		}

		UInterchangeShaderPortsAPI::ConnectOuputToInputByName(ShaderGraphNode, SubstrateMaterial::Parameters::FrontMaterial.ToString(), OpenPbrStandarSurfaceShaderNode->GetUniqueID(), FrontMaterialTranslucent.ToString());
	}

	return bSubsurfaceTransmission;
}
...
{
	return UTF8_TO_TCHAR(Element->getActiveFilePrefix().c_str());
}
...
void FMaterialXSurfaceShaderAbstract::RegisterConnectNodeOutputToInputDelegates()
{
	...
	MatchingConnectNodeDelegates.Add(mx::Category::Luminance,		  FOnConnectNodeOutputToInput::CreateSP(this, &FMaterialXSurfaceShaderAbstract::ConnectLuminanceInputToOutput));
	MatchingConnectNodeDelegates.Add(mx::Category::HexTiledImage,	  FOnConnectNodeOutputToInput::CreateSP(this, &FMaterialXSurfaceShaderAbstract::ConnectHexTiledInputToOutput));
	MatchingConnectNodeDelegates.Add(mx::Category::HexTiledNormalMap, FOnConnectNodeOutputToInput::CreateSP(this, &FMaterialXSurfaceShaderAbstract::ConnectHexTiledInputToOutput));
	MatchingConnectNodeDelegates.Add(mx::Category::LatLongImage,	  FOnConnectNodeOutputToInput::CreateSP(this, &FMaterialXSurfaceShaderAbstract::ConnectLatLongImageInputToOutput));
	MatchingConnectNodeDelegates.Add(mx::Category::Fractal2D,		  FOnConnectNodeOutputToInput::CreateSP(this, &FMaterialXSurfaceShaderAbstract::ConnectFractalInputToOutput));
	MatchingConnectNodeDelegates.Add(mx::Category::Fractal3D,		  FOnConnectNodeOutputToInput::CreateSP(this, &FMaterialXSurfaceShaderAbstract::ConnectFractalInputToOutput));
}
