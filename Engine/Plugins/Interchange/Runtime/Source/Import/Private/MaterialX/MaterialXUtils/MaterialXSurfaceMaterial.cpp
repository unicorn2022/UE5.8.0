...
UInterchangeBaseNode* FMaterialXSurfaceMaterial::Translate(MaterialX::NodePtr SurfaceMaterialNode)
{
		...
		if (Input)
		{
			...
			if (ShaderTranslator.IsValid())
			{
				...
				if (!ShaderGraphNode)
				{
					...
					NodeContainer.SetupNode(ShaderGraphNode, ShaderGraphNodeUID, SurfaceMaterialNode->getName().c_str(), EInterchangeNodeContainerType::TranslatedAsset);
	...
	AnalyticsAttributes.Emplace(TEXT("MaterialType"), TEXT("Surface Material"));
	if(!bHasSurfaceShader)
	{
		...
		AnalyticsAttributes.Emplace(TEXT("ShaderType"), TEXT("Surface Shader unsupported"));
	}
