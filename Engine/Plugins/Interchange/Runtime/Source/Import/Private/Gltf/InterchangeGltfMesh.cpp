...
namespace UE::Interchange::Gltf::Private
{
	...
	*/
	...
	void PrintGltfMessages(GLTF::FMeshFactory& MeshFactory)
	{
		// Add glTF errors and warnings to the Interchange results
		for (const GLTF::FLogMessage& LogMessage : MeshFactory.GetLogMessages())
		{
			switch (LogMessage.Get<0>())
			{
			case GLTF::EMessageSeverity::Error:
			{
				UE_LOGF(LogInterchangeImport, Error, "glTF Mesh Import Error message: %ls", *LogMessage.Get<1>().ToString());
			}
			break;
			case GLTF::EMessageSeverity::Warning:
			{
				UE_LOGF(LogInterchangeImport, Warning, "glTF Mesh Import Warning message: %ls", *LogMessage.Get<1>().ToString());
			}
			break;
			case GLTF::EMessageSeverity::Display:
			{
				UE_LOGF(LogInterchangeImport, Display, "glTF Mesh Import Display message: %ls", *LogMessage.Get<1>().ToString());
			}
			break;
			default:
				break;
