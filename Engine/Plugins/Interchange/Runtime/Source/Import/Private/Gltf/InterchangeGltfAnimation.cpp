...
namespace UE::Interchange::Gltf::Private
{
	...
	*/
	...
	T InterpolateValue(TArray<float>& Seconds /*input*/, TArray<T>& SegmentValues /*output*/, int32 kIndex /*lower*/, int32 kp1Index /*higher*/, double CurrentRequestedTimeStamp, GLTF::FAnimation::EInterpolation Interpolation, bool IsRotation)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "Interpolation Failed due to invalid kIndices");
			return T();
		}
	...
	bool GetTransformAnimationPayloadData(const FString& PayLoadKey, const GLTF::FAsset& GltfAsset, FAnimationPayloadData& OutPayloadData)
	{
		...
		for (const int32 ChannelIndex : ChannelIndices)
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Warning, "Animation[%d]:Channel[%d]: Morph Animation (weight path) type not supported for transform-like animations. Morph animations should be handled through the \"GetMorphTargetAnimationPayloadData\" function.", AnimationIndex, ChannelIndex);
				break;
			}
			default:
				UE_LOGF(LogInterchangeImport, Warning, "Animation type not supported");
				break;
			}
		}
		return true;
	...
	namespace AnimationHelpers
	{
		struct FAnimationHandler
		{
			...
			{
				inline static const double DefaultFrameRate = 30.0;
				inline static const TArray<FFrameRate> UsualFrameRates = { 
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(DefaultFrameRate), //30
					
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(24.),
					FFrameRate(24000, 1001),								//23.976
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(25),
					FFrameRate(30000, 1001),								//29.97
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(48),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(50),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(60),
					FFrameRate(60000, 1001),								//59.94
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(72),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(90),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(100),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(120),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(144),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(165),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(240),
					UE::Interchange::Animation::ConvertSampleRateToFrameRate(480)
				};
			...
			UInterchangeSkeletalAnimationTrackNode* AcquireTrackNode(const FString& SkeletonNodeUid, FAnimationTimes& AnimationTimes)
			{
				...
				if (RootJointIndexToTrackNodeMap.Contains(SkeletonNodeUid))
				{
					...
					TrackNode->GetCustomAnimationStopTime(AnimationTimes.StopTime);
				}
				else
				{
			...
			void UpdateAnimationTimes(const GLTF::FAnimation::FSampler& Sampler, FAnimationTimes& AnimationTimes)
			{
				...
				if (Seconds.Num() > 0)
				{
					...
					if (AnimationTimes.StopTime < CurrentStopTime)
					{
						AnimationTimes.StopTime = CurrentStopTime;
					}
				}
			};

			void ValidateAnimationStopAndFrameRate(const FString& AnimationName, double& AnimationStopTime, double& FrameRate)
			{
				auto AtLeast1FrameLong = [&AnimationStopTime, &FrameRate]()
					{
						double FrameLength = 1. / FrameRate;
						if (AnimationStopTime < FrameLength)
						{
							AnimationStopTime = FrameLength;
						}
					};

				for (const FFrameRate& FrameRateCandidate : FAnimationTimes::UsualFrameRates)
				{
					const FFrameTime FrameTime = FrameRateCandidate.AsFrameTime(AnimationStopTime);
					const FFrameTime RoundedFrameTime = FrameTime.RoundToFrame();
					float DeltaTiming = FMath::Abs(RoundedFrameTime.AsDecimal() * FrameRateCandidate.AsInterval() - AnimationStopTime);
					//0.001 represents 1ms.
					if (FMath::IsNearlyZero(DeltaTiming, 1e-3) || FMath::IsNearlyEqual(DeltaTiming, FrameRateCandidate.AsInterval(), 1e-3))
					{
						FrameRate = FrameRateCandidate.AsDecimal();

						const float SubFrame = FrameTime.GetSubFrame();
						if (!FMath::IsNearlyZero(SubFrame, KINDA_SMALL_NUMBER) && !FMath::IsNearlyEqual(SubFrame, 1.0f, KINDA_SMALL_NUMBER))
						{
							//Update timing to align with FrameTimings for UE this might be needed due to:
							//Exporters sometimes round the timings to certain decimals (for example to ms)
							// For example 20 frames on 30 FPS becomes 0.66666666666666666666666666666667
							//	Which then might be truncated to 0.666 or rounded to 0.667.
							// To check for this scenario we actually do comparison on the SubFrame in seconds with 0.001 delta evaluation.
							// To fix this we re-align the AnimationStopTime here:
							AnimationStopTime = FrameRateCandidate.AsSeconds(FrameRateCandidate.AsFrameTime(AnimationStopTime).RoundToFrame());
						}

						AtLeast1FrameLong();

						if (FrameRate != FAnimationTimes::DefaultFrameRate)
						{
							UE_LOG(LogInterchangeImport, Display, TEXT("Animation [%s] FrameRate identified as %.3f"), *AnimationName, FrameRate);
						}
						return;
					}
				}
				...
				FrameRate = FAnimationTimes::DefaultFrameRate;
				AtLeast1FrameLong();
				UE_LOG(LogInterchangeImport, Display, TEXT("Animation [%s] FrameRate could not be identified. Using default %f."), *AnimationName, FrameRate);
			}
			...
			void ProcessRiggedAnimations(const TMap<int32, TMap<int32, TSet<int32>>>& RiggedAnimations)
			{
				for (const TPair<int32, TMap<int32, TSet<int32>>>& RiggedAnimation : RiggedAnimations)
				{
					...
					for (const TTuple<int32, TSet<int32>>& AnimatedNodeIndexToChannelIndices : RiggedAnimation.Value)
					{
						...
						for (int32 ChannelIndex : AnimatedNodeIndexToChannelIndices.Value)
						{
							...
							if (!ValidateChannelSampler(Channel, Sampler))
							{
								...
								UE_LOGF(LogInterchangeImport, Warning, "glTF sampler corrupt. Input and Output do not meet expectations.");
								break;
							}
					...
					double FrameRate = FAnimationTimes::DefaultFrameRate;
					double AnimationStopTime = AnimationTimes.StopTime;
					ValidateAnimationStopAndFrameRate(TrackNode->GetDisplayLabel(), AnimationStopTime, FrameRate);
					TrackNode->SetCustomAnimationSampleRate(FrameRate);
					TrackNode->SetCustomAnimationStopTime(AnimationStopTime);
				}
			}
