...
static const TArray<FString> ImporterSupportedExtensions = {
	...
	GLTF::ToString(GLTF::EExtension::KHR_MaterialsVolume),
...
namespace UE::Interchange::Gltf::Private
{
	...
	void AddAnalytics(const TranslationResult& TranslationResult,
		TObjectPtr<UInterchangeAnalyticsHandlerBase> AnalyticsHandler,
		...
		const FText& GLTFReaderLogMessage = FText::GetEmpty())
	{
		...
		AnalyticsHandler->Append(GLTFAnalyticsIdentifier, GLTFAnalytics);
	};
}
	...
	const TMap<int32, FTransform>& T0Transforms, const FString& SceneNodeUid) const
{
	...
	UInterchangeSceneNode* InterchangeSceneNode;
	if (GltfNode.Type == GLTF::FNode::EType::Joint)
	{
		InterchangeSceneNode = NewObject< UInterchangeJointNode >(&NodeContainer);
	}
	else
	{
		InterchangeSceneNode = NewObject< UInterchangeSceneNode >(&NodeContainer);
	}
	...
	switch ( GltfNode.Type )
	{
		case GLTF::FNode::EType::MeshSkinned:
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Warning, "Node [%ls] with a skinned mesh is not root. Parent transforms will not affect a skinned mesh.", *GltfNode.Name);
			}
			...
			{//Set Morph Target Curve Weights
				...
				{
					UE_LOGF(LogInterchangeImport, Warning, "glTF Node [%ls] Import Warning: The glTF node's MorphTargetNames count does not match its MorphTargetWeights count.", *GltfNode.UniqueId);
				}
			}
		...
		{
			UInterchangeJointNode* JointNode = CastChecked<UInterchangeJointNode>(InterchangeSceneNode);

			if (GltfNode.bHasLocalBindPose)
			{
				JointNode->SetBindPoseLocalTransform(&NodeContainer, GltfNode.LocalBindPose);
			}

			if (GltfAsset.Animations.Num() == 0 || !T0Transforms.Contains(GltfNode.Index))
			{
				//If no animations present, use Local Transform for T0
				JointNode->SetTimeZeroLocalTransform(&NodeContainer, GltfNode.Transform);
			}
			else
			{
				JointNode->SetTimeZeroLocalTransform(&NodeContainer, T0Transforms[GltfNode.Index]);
			}
			break;
		}
		...
		static bool bRequiredPackageLoaded = []() -> bool
			{
				...
				{
					UE_LOGF(LogInterchangeImport, Warning, "UInterchangeGLTFPipeline: Some required packages are missing. Material import might be wrong.");
					return false;
				}
...
bool UInterchangeGLTFTranslator::Translate( UInterchangeBaseNodeContainer& NodeContainer ) const
{
	...
	{
		AddAnalytics(TranslationResult::INPUT_FILE_NOTFOUND, AnalyticsHandler);
		return false;
	}
	...
	for (GLTF::FLogMessage& LogMessage : GLTFReadFileLogMessages)
	{
		if (LogMessage.Key == GLTF::EMessageSeverity::Error)
		{
			...
			AddAnalytics(TranslationResult::GLTFREADER_FAILED, AnalyticsHandler, GltfAsset, LogMessage.Value);
			return false;
		}
	}
	...
	if (NotSupportedRequiredExtensions.Num() > 0)
	{
		...
		AddAnalytics(TranslationResult::NOTSUPPORTED_EXTENSION_FOUND, AnalyticsHandler, GltfAsset);
		return false;
	}
	...
	// Textures
	{
		...
		for ( const GLTF::FTexture& GltfTexture : GltfAsset.Textures )
		{
			...
			const FString TextureNodeUid = UInterchangeTextureNode::MakeNodeUid(GltfTexture.UniqueId);
			UInterchangeTexture2DNode* TextureNode = UInterchangeTexture2DNode::Create(&NodeContainer, TextureNodeUid, GltfTexture.Name);
	...
	// Materials
	{
		...
		for ( const GLTF::FMaterial& GltfMaterial : GltfAsset.Materials )
		{
			...
			// TODO: Spec specifies linear space, but it seems to give better results in sRGB especially on the Dragon
			SetTextureSRGB(NodeContainer, GltfMaterial.Volume.ThicknessTexture, true);
	...
	{
		UE_LOGF(LogInterchangeImport, Warning, "glTF Mesh Import Warning: glTF mesh usage expectations are not met.");
	}

	AddAnalytics(TranslationResult::SUCCESSFULL, AnalyticsHandler, GltfAsset);
	return true;
}
...
TOptional< UE::Interchange::FImportImage > UInterchangeGLTFTranslator::GetTexturePayloadData(const FString& PayloadKey, TOptional<FString>& AlternateTexturePath) const
{
	...
	else
	{
		...
		UE::Interchange::Private::FScopedTranslator ScopedTranslator(TextureFilePath, Results, AnalyticsHandler);
	...
	if (GltfAsset.Textures.IsValidIndex(TextureMap.TextureIndex))
	{
		...
		if (UInterchangeTextureNode* TextureNode = const_cast<UInterchangeTextureNode*>(Cast<UInterchangeTextureNode>(NodeContainer.GetNode(TextureUid))))
		{
			...
			if (TextureNode->GetCustomSRGB(bExistingSRGBSetting))
			{
				...
				{
					UE_LOGF(LogInterchangeImport, Warning, "UInterchangeGLTFPipeline: Unexpected SRGB/Linear setting on Texture: %ls.", *TextureNode->GetDisplayLabel());
				}
			}
