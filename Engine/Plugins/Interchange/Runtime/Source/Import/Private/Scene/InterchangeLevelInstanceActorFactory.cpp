...
UObject* UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread(const UInterchangeFactoryBase::FImportSceneObjectsParams& CreateSceneObjectsParams)
{
	...
	{
		UE_LOGF(LogInterchangeImport, Warning, "UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread: Actor level instance do not reference any level factory node.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogInterchangeImport, Warning, "UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread: Reference Level factory node is invalid.");
		return nullptr;
	}
	...
	{
		UE_LOGF(LogInterchangeImport, Warning, "UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread: Reference Level factory node do not point on a valid UWorld.");
		return nullptr;
	}
	...
	if (!ReferenceWorld)
	{
		UE_LOGF(LogInterchangeImport, Warning, "UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread: Cannot load a valid Reference UWorld.");
		return nullptr;
	}
	...
	if (bIsPackedActor)
	{
		...
		if (!PackedActorBP)
		{
			...
			if (UInterchangeEditorUtilitiesBase* EditorUtilities = UInterchangeManager::GetInterchangeManager().GetEditorUtilities())
			{
				...
				{
					UE_LOGF(LogInterchangeImport, Warning, "UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread: Cannot save the packed actor's (%ls) referenced world (%ls)", *FactoryNode->GetDisplayLabel(), *ReferenceWorld->GetName());
				}
			}
		...
		if (ensure(PackedActorBP))
		{
			...
			if (AsyncHelper.IsValid())
			{
				...
				AssetInfo.SetImportedObject(PackedActorBP);
		...
		if (!LevelInstanceActor)
		{
			UE_LOGF(LogInterchangeImport, Warning, "UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread: Failed to create packed level blueprint.");
			return nullptr;
		}
	}
	else
	...
	if (!LevelInstanceActor)
	{
		UE_LOGF(LogInterchangeImport, Warning, "UInterchangeLevelInstanceActorFactory::ImportSceneObject_GameThread: Cannot create/Find a level instance actor in the target world.");
		return nullptr;
	}
