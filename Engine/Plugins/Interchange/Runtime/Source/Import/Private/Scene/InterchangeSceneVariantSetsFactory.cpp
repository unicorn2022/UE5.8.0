...
namespace UE::Interchange::Private
{
	...
	void FLevelVariantSetsHelper::AddVariantSet(const UInterchangeVariantSetNode& VariantSetNode)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "No payload for variant set %ls.", *DisplayText);
			return;
		}
	...
	void FLevelVariantSetsHelper::AddBinding(const FVariantBinding& Binding, UVariant& Variant)
	{
		...
		if (!Actor)
		{
			UE_LOGF(LogInterchangeImport, Warning, "Cannot find actor for variant %ls.", *Variant.GetDisplayText().ToString());
			return;
		}
		...
		else if (PropertyCapture.ObjectUid.IsSet())
		{
			...
			if (EnumHasAnyFlags(PropertyCapture.Category, EVariantPropertyCaptureCategory::Material))
			{
				if (const UInterchangeBaseMaterialFactoryNode* TargetFactoryNode = Cast<UInterchangeBaseMaterialFactoryNode>(NodeContainer.GetNode(TargetFactoryNodeUid)))
				{
					...
					{
						UE_LOGF(LogInterchangeImport, Warning, "Could not find material '%ls' when creating variant asset.", *TargetFactoryNode->GetDisplayLabel());
					}
				}
				else
				{
					UE_LOGF(LogInterchangeImport, Warning, "Could not find factory when creating variant assets.");
				}
			}
		}
	}
...
UInterchangeFactoryBase::FImportAssetResult UInterchangeSceneVariantSetsFactory::BeginImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Cannot import LevelVariantSets. The translator does not implement the IInterchangeVariantSetPayloadInterface interface.");
		return ImportAssetResult;
	}
	...
	if (!LevelVariantSets)
	{
		UE_LOGF(LogInterchangeImport, Warning, "Could not create LevelVariantSets asset %ls.", *Arguments.AssetName);
		return ImportAssetResult;
	}
