...
{
	FScopedTranslator::FScopedTranslator(const FString& PayLoadKey, UInterchangeResultsContainer* Results, TObjectPtr<UInterchangeAnalyticsHandlerBase> AnalyticsHandler)
	{
		...
		if (PayloadSourceData)
		{
			...
			if (SourceTranslator)
			{
				SourceTranslator->AnalyticsHandler = AnalyticsHandler;
