...
namespace UE::Interchange::Private::InterchangeDDSTranslator
{
	...
	TUniquePtr<UE::DDS::FDDSFile> CreateDDSFileReader(const FString& Filename, bool bHeaderOnly)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Error, "Failed to import DDS, cannot open file. [%ls]", *Filename);
			return {};
		}
		...
		{
			UE_LOGF(LogInterchangeImport, Error, "Failed to import DDS, cannot load file content into an array. [%ls]", *Filename);
			return {};
		}
		...
		if (!DDS.IsValid())
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Warning, "Failed to load DDS (Error=%d) [%ls]", (int)Error, *Filename);
			}
			return {};
		}
		...
		if (!bHeaderOnly)
		{
			...
			if (DDS->MipCount > 1)
			{
				...
				{
					UE_LOGF(LogInterchangeImport, Warning, "Imported DDS files must either be a single base mip, or a full mip chain, not a partial mip chain (found %d expected %d) [%ls]", DDS->MipCount, FImageCoreUtils::GetMipCountFromDimensions(DDS->Width, DDS->Height, DDS->Depth, DDS->IsValidTextureVolume()), *Filename);
				}
			}
	...
	uint32 GetDDSMipCount(const TUniquePtr<UE::DDS::FDDSFile>& DDS)
	{
		...
		if (MipCount > MAX_TEXTURE_MIP_COUNT)
		{
			...
			UE_LOGF(LogInterchangeImport, Warning, "DDS exceeds MAX_TEXTURE_MIP_COUNT");
...
TOptional<UE::Interchange::FImportSlicedImage> UInterchangeDDSTranslator::GetSlicedTexturePayloadData(const FString& /*PayloadKey*/, TOptional<FString>& /*AlternateTexturePath*/) const
{
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Failed to import DDS, unsupported format. [%ls]", *Filename);
