...
namespace UE::Interchange::Private
{
	...
	{
		const UInterchangeJointNode* JointNode = Cast<UInterchangeJointNode>(NodeContainer->GetNode(JointUid));
		if (!JointNode)
		{
			return;
		}
		constexpr bool bAddSourceNodeName = true;
		UInterchangeUserDefinedAttributesAPI::DuplicateAllUserDefinedAttribute(JointNode, DestinationNode, bAddSourceNodeName);
	...
	void FSkeletonHelper::RecursiveBoneHasBindPose(const UInterchangeBaseNodeContainer* NodeContainer, const FString& JointNodeId, bool& bHasBoneWithoutBindPose)
	{
		...
		//In case of rigid meshes, a regular SceneNode can still be considered a Joint.
		const UInterchangeSceneNode* SceneNode = Cast<UInterchangeSceneNode>(NodeContainer->GetNode(JointNodeId));
		if (!SceneNode)
		{
			UE_LOGF(LogInterchangeImport, Warning, "Invalid Skeleton Joint");
			return;
		}
		
		// Only UInterchangeJointNode can have bind pose
		if (const UInterchangeJointNode* JointNode = Cast<UInterchangeJointNode>(SceneNode))
		{
			bool bHasInvalidBindPose;

			// Only consider joint nodes that has been set explicitly as not having a bind pose
			if (JointNode->GetHasInvalidBindPose(bHasInvalidBindPose) && bHasInvalidBindPose)
			{
		...
		, bool bImportSockets)
	{
		...
		const UInterchangeSceneNode* SceneNode = Cast<UInterchangeSceneNode>(BaseNode);
		if (!SceneNode)
		{
		...
		FJointInfo& Info = JointInfos.AddZeroed_GetRef();
		Info.Name = GetBoneNameForNode(SceneNode);
		...
		SceneNode->GetCustomLocalTransform(LocalTransform);

		const UInterchangeJointNode* JointNode = Cast<UInterchangeJointNode>(SceneNode);
		if (JointNode)
		{
			bHasTimeZeroTransform = JointNode->GetTimeZeroLocalTransform(TimeZeroLocalTransform);
			bHasBindPoseTransform = JointNode->GetBindPoseLocalTransform(BindPoseLocalTransform);
		}
		...
		if (ParentIndex == INDEX_NONE)
		{
			...
			if (bBakeMeshes)
			{
				...
				ensure(SceneNode->GetCustomGlobalTransform(NodeContainer, GlobalOffsetTransform, LocalTransform));

				if (JointNode)
				{
					bHasTimeZeroTransform = JointNode->GetTimeZeroGlobalTransform(NodeContainer, GlobalOffsetTransform, TimeZeroLocalTransform);
					bHasBindPoseTransform = JointNode->GetBindPoseGlobalTransform(NodeContainer, GlobalOffsetTransform, BindPoseLocalTransform);
				}
			}
		}
		...
		else if (JointNode && !GIsAutomationTesting && !bHasBindPoseTransform && !bUseTimeZeroAsBindPose)
		{
			//StaticMeshes converted to SkeletalMeshes are not expected to have BindPoses
		...
		for (int32 ChildIndex = 0; ChildIndex < ChildrenIds.Num(); ++ChildIndex)
		{
			...
			if(const UInterchangeSceneNode* ChildJointNode = Cast<UInterchangeSceneNode>(NodeContainer->GetNode(ChildUid)))
			{
				...
				{
					//Sockets will be add by the skeletal mesh factory
					//We don't want a socket to be a skeleton joint
					continue;
				}
