...
namespace UE
{
	namespace Interchange
	{
		...
		FInterchangeMeshPayLoadKey FMeshNodeContext::GetMorphTargetAndTransformPayloadKey(const FInterchangeMeshPayLoadKey& MorphTargetKey) const
		{
			...
			GlobalPayloadKey.UniqueId += FInterchangeMeshPayLoadKey::GetTransformString(SceneGlobalTransform);
			return GlobalPayloadKey;
		}
		...
		FString FMeshNodeContext::GetUniqueId() const
		{
			...
			UniqueId += FInterchangeMeshPayLoadKey::GetTransformString(SceneGlobalTransform);
			return UniqueId;
		}

		namespace Private
		{
			void FindAllSockets(const UInterchangeBaseNodeContainer* NodeContainer, const UInterchangeBaseNode* BaseNode, TMap<FString, TSet<const UInterchangeSceneNode*>>& OutSocketsPerBoneMap)
			{
				const TArray<FString> ChildrenIds = NodeContainer->GetNodeChildrenUids(BaseNode->GetUniqueID());
				for (int32 ChildIndex = 0; ChildIndex < ChildrenIds.Num(); ++ChildIndex)
				{
					const FString& ChildUid = ChildrenIds[ChildIndex];
					if (const UInterchangeSceneNode* ChildNode = Cast<UInterchangeSceneNode>(NodeContainer->GetNode(ChildUid)))
					{
						if (FSkeletonHelper::IsValidSocket(NodeContainer, ChildNode))
						{
							//We found a socket add it to the map
							TSet<const UInterchangeSceneNode*>& Sockets = OutSocketsPerBoneMap.FindOrAdd(BaseNode->GetDisplayLabel());
							Sockets.Add(ChildNode);
							//Socket cannot have children
							continue;
						}
						FindAllSockets(NodeContainer, ChildNode, OutSocketsPerBoneMap);
					}
				}
			}
																 ...
																 , FString AssetName)
			{
				...
				for (const FString& MorphTargetUid : MorphTargetUids)
				{
					...
					{
						const TOptional<FInterchangeMeshPayLoadKey>& OptionalPayLoadKey = MorphTargetMeshNode->GetPayLoadKey();
						if (!OptionalPayLoadKey.IsSet())
						{
							UE_LOGF(LogInterchangeImport, Warning, "Empty LOD morph target mesh reference payload when importing SkeletalMesh asset %ls.", *AssetName);
							continue;
						}
						const FInterchangeMeshPayLoadKey& PayLoadKey = OptionalPayLoadKey.GetValue();
						const FInterchangeMeshPayLoadKey& GlobalMorphPayLoadKey = MeshNodeContext.GetMorphTargetAndTransformPayloadKey(PayLoadKey);
				...
				for (const FString& MorphTargetUid : MorphTargetUids)
				{
					if (const UInterchangeMeshNode* MorphTargetMeshNode = Cast<UInterchangeMeshNode>(NodeContainer->GetNode(MorphTargetUid)))
					{
						...
						{
							UE_LOGF(LogInterchangeImport, Warning, "Invalid skeletal mesh morph target payload key [%ls] for SkeletalMesh asset %ls.", *MorphTargetPayloadKeyString, *AssetName);
							continue;
						}
						...
						MorphTargetMeshDescriptionsPerMorphTargetName.Add(MorphTargetPayloadKeyString, MorphTargetMeshPayload); //Note: this is adding a copy of MorphTargetMeshPayload to the TMap.
					}
				}
			}

			const UInterchangeJointNode* RecursiveFindJointByName(const UInterchangeBaseNodeContainer* NodeContainer, const FString& JointNodeId, const FString& JointName)
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(RecursiveFindJointByName)
				if (const UInterchangeJointNode* JointNode = Cast<UInterchangeJointNode>(NodeContainer->GetNode(JointNodeId)))
				{
				...
				TArray<FString> NodeChildrenUids = NodeContainer->GetNodeChildrenUids(JointNodeId);
				for (int32 ChildIndex = 0; ChildIndex < NodeChildrenUids.Num(); ++ChildIndex)
				{
					if (const UInterchangeJointNode* JointNode = RecursiveFindJointByName(NodeContainer, NodeChildrenUids[ChildIndex], JointName))
					{
						return JointNode;
					}
				}
			...
			void SkinVertexPositionToTimeZero(UE::Interchange::FMeshPayloadData& LodMeshPayload
				...
				, const FTransform GeometricTransform
				, const FTransform& SceneNodeTransform)
			{
				...
				FMatrix SceneNodeMatrixInverse = SceneNodeTransform.ToInverseMatrixWithScale();
				FMatrix SceneNodeMatrixInverseWithoutGlobalOffset = SceneNodeMatrixInverse;
				...
				if (UInterchangeCommonPipelineDataFactoryNode* CommonPipelineDataFactoryNode = UInterchangeCommonPipelineDataFactoryNode::GetUniqueInstance(NodeContainer))
				{
					...
					if (CommonPipelineDataFactoryNode->GetCustomGlobalOffsetTransform(GlobalOffsetTransform))
					{
						bool bBakeMeshes = false;
						if (CommonPipelineDataFactoryNode->GetBakeMeshes(bBakeMeshes) && bBakeMeshes)
						{
							SceneNodeMatrixInverseWithoutGlobalOffset = (SceneNodeTransform * GlobalOffsetTransform.Inverse()).ToInverseMatrixWithScale();
						}
					}
				}
				...
				for (int32 JointIndex = 0; JointIndex < JointCount; ++JointIndex)
				{
					...
					const UInterchangeJointNode* JointNode = RecursiveFindJointByName(NodeContainer, RootJointNodeId, JointName);
					if (!JointNode)
					{
						continue;
					}
					...
					//Scope any transform variable to be sure they are not use to create the vertex transformation matrix
					{
						...
						if (!JointNode->GetAttribute<FMatrix>(AttributeKey, JointBindPoseMatrix))
						{
							...
							if (!ensure(JointNode->GetBindPoseGlobalTransform(NodeContainer, FTransform::Identity, JointBindPose)))
							{
						...
						if (!JointNode->GetGlobalMatrixForT0Rebinding(JointT0Matrix))
						{
							FTransform JointT0;
							if (!JointNode->GetTimeZeroGlobalTransform(NodeContainer, FTransform::Identity, JointT0))
							{
						...
						if (!JointNode->GetGlobalBindPoseReferenceFromMeshUID(MeshNode->GetUniqueID(), JointReferenceMatrix))
						{
							//Skip this joint
							continue;
						}
						...
						JointReferenceMatrix = GeometricTransform.ToMatrixWithScale() * JointReferenceMatrix;
					}

					FMatrix VertexTransformMatrix;
					{
						FMatrix ReferenceMatrixRelativeToBindPose = JointReferenceMatrix * JointBindPoseMatrix.Inverse();
						FMatrix JointTimeZeroRelativeToMeshNodeMatrix = JointT0Matrix * SceneNodeMatrixInverseWithoutGlobalOffset;
						//Compute the transform to apply to vertex to change the bind pose from the existing one to use time zero pose instead
						VertexTransformMatrix = SceneNodeMatrixInverse * (ReferenceMatrixRelativeToBindPose * JointTimeZeroRelativeToMeshNodeMatrix);
					}
													...
													, FMeshDescription& DestinationMeshDescription)
			{
				...
				for(TPair<const FMeshNodeContext*, TOptional<UE::Interchange::FMeshPayloadData>>& MeshNodeContextAndFuture: LodMeshPayloadPerTranslatorPayloadKey)
				{
					...
					{
						UE_LOGF(LogInterchangeImport, Warning, "Invalid skeletal mesh payload key [%ls] for SkeletalMesh asset %ls.", *MeshNodeContext.TranslatorPayloadKey.UniqueId, *Arguments.AssetName);
						continue;
					}
					
					const int32 VertexOffset = DestinationMeshDescription.Vertices().Num();

					const bool bIsRiggedMesh = LodMeshPayload->JointNames.Num() > 0;
					if (MeshNodeContext.MeshNode && bIsRiggedMesh)
					{
						FSourceGeometryPartID PartID = DestinationMeshAttributes.CreateSourceGeometryPart();

						NameAttribute.Set(PartID, FName(MeshNodeContext.SceneNodeName));
						VertexAndCountAttribute.Set(PartID, { DestinationMeshDescription.Vertices().Num(), LodMeshPayload->MeshDescription.Vertices().Num()});
					}

					FString AssetName = SkeletalMeshFactoryNode->GetAssetName();
					...
					if (bSkinControlPointToTimeZero && bIsRiggedMesh)
					{
						...
						SkinVertexPositionToTimeZero(LodMeshPayload.GetValue(), NodeContainer, RootJointNodeId, MeshNodeContext.MeshNode, MeshNodeContext.GeometricTransform, MeshNodeContext.SceneGlobalTransform);
					}
					...
					else if(!bIsRiggedMesh)
					{
						...
						const FString ToSkinBoneName = MeshNodeContext.SceneNodeName;
						for (int32 RefBoneIndex = 0; RefBoneIndex < RefBoneCount; ++RefBoneIndex)
						{
							const SkeletalMeshImportData::FBone& Bone = RefBonesBinary[RefBoneIndex];
					...
					AppendSettings.MeshTransform = InstanceData.ShouldFetchWithTransform() ? FTransform::Identity : MeshNodeContext.SceneGlobalTransform;
					...
					if (MeshNodeContext.MeshNode->IsSkinnedMesh() || !bIsRiggedMesh)
					{
				...
				, bool bImportSockets)
			{
				...
				//Scope to query the mesh node
				{
					...
					ImportAssetObjectLODDataRef.MeshNodeContexts.Reserve(LODMeshDataArray.Num());

					for (const FInterchangeLODMeshData& LODMeshData : LODMeshDataArray)
					{
						FMeshNodeContext MeshReference;
						const UInterchangeBaseNode* MeshBaseNode = Arguments.NodeContainer->GetNode(LODMeshData.MeshUid);
						MeshReference.MeshNode = Cast<UInterchangeMeshNode>(MeshBaseNode);
						MeshReference.SceneGlobalTransform = LODMeshData.Transform;
						MeshReference.GeometricTransform = LODMeshData.GeometricTransform;
						MeshReference.SceneNodeName = LODMeshData.SceneNodeName;
			    ...
			    , const FString& AssetName)
			{
				...
				if (bNumRemapped != InOutDescription.BoneInfluences.Num())
				{
					UE_LOGF(LogInterchangeImport, Warning
						, "Failed to remap one or more Nanite Assembly Bone Influence's while processing skeletal mesh '%ls'"
						, *AssetName
					);
					return false;
				}
...
void UInterchangeSkeletalMeshFactory::CreatePayloadTasks(const FImportAssetObjectParams& Arguments, bool bAsync, TArray<TSharedPtr<UE::Interchange::FInterchangeTaskBase>>& PayloadTasks)
{
	...
	if (!MeshTranslatorPayloadInterface)
	{
		...
		Message->Text = FText::Format(NSLOCTEXT("InterchangeSkeletalMeshFactory", "CreatePayloadTasks_TranslatorInterfaceMissing", "Cannot import skeletalMesh {0}, the translator {1} does not implement the IInterchangeSkeletalMeshPayloadInterface.")
			...
			, FText::FromString(Arguments.Translator ? Arguments.Translator->GetName() : TEXT("UnknownTranslator")));
		return;
	}
	...
	for (int32 LodIndex = 0; LodIndex < LodCount; ++LodIndex)
	{
		...
		{
			const FInternalInstanceData& InstanceData = MeshInstancesDatas.FindChecked(MeshNodeContext.TranslatorPayloadKey.UniqueId);
			FTransform ApplyTransformWhenFetchPayload = InstanceData.ShouldFetchWithTransform() ? MeshNodeContext.SceneGlobalTransform : FTransform::Identity;
			...
			if (bImportMorphTarget)
			{
				...
				for (const FString& MorphTargetUid : MorphTargetUids)
				{
					if (const UInterchangeMeshNode* MorphTargetMeshNode = Cast<UInterchangeMeshNode>(Arguments.NodeContainer->GetNode(MorphTargetUid)))
					{
						...
						if (!OptionalPayLoadKey.IsSet())
						{
							UE_LOGF(LogInterchangeImport, Warning, "Empty LOD morph target mesh reference payload when importing SkeletalMesh asset %ls.", *Arguments.AssetName);
							continue;
						}
...
UInterchangeFactoryBase::FImportAssetResult UInterchangeSkeletalMeshFactory::BeginImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	...
	if (!ensure(SkeletalMesh))
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "Could not create or find a SkeletalMesh asset named %ls.", *Arguments.AssetName);
		}
		return ImportAssetResult;
	}
...
UInterchangeFactoryBase::FImportAssetResult UInterchangeSkeletalMeshFactory::ImportAsset_Async(const FImportAssetObjectParams& Arguments)
{
	...
	for (int32 LodIndex = 0; LodIndex < LodCount; ++LodIndex)
	{
		...
		if (ImportAssetObjectData.bIsReImport)
		{
			...
			// The LOD slot may have survived from a previous import with stale
			// RequiredBones / ActiveBoneIndices / MeshToImportVertexMap / IndexBuffer
			// /etc. The downstream build only overwrites the fields it produces, so
			// wipe the slot back to a default-constructed state before refilling it.
			ImportedResource->LODModels[CurrentLodIndex].Empty();
		}
		else
		{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "Asset [%ls] contains too many influences.", *Arguments.AssetName);
		}
...
UInterchangeFactoryBase::FImportAssetResult UInterchangeSkeletalMeshFactory::EndImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	...
	if (!ensure(SkeletalMesh))
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Error, "Could not create SkeletalMesh asset %ls.", *Arguments.AssetName);
		}
		else
		{
			UE_LOGF(LogInterchangeImport, Error, "Could not find reimported SkeletalMesh asset %ls.", *Arguments.AssetName);
		}
		return ImportAssetResult;
	}
	...
	if (USkeleton* SkeletonReference = ImportAssetObjectData.SkeletonReference)
	{
		...
		if (bImportSockets && !ContentInfo.bApplySkinningOnly)
		{
			...
			if(!RootSkeletonNodeUid.IsEmpty())
			{
				...
				TMap<FString, TSet<const UInterchangeSceneNode*>> SocketsPerBoneMap;

				if (const UInterchangeBaseNode* RootSkeletonNode = Arguments.NodeContainer->GetNode(RootSkeletonNodeUid))
				{
					UE::Interchange::Private::FindAllSockets(Arguments.NodeContainer, RootSkeletonNode, SocketsPerBoneMap);
				}

				for (const TPair<FString, TSet<const UInterchangeSceneNode*>>& BoneAndSockets : SocketsPerBoneMap)
				{
					...
					if (RefSkeleton.IsValidIndex(BoneIndex))
					{
						...
						for (const UInterchangeSceneNode* SocketSceneNode : BoneAndSockets.Value)
						{
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Interchange Import UInterchangeSkeletalMeshFactory::EndImportAssetObject_GameThread, USkeleton* SkeletonReference is nullptr.");
	}
