...
UInterchangeFactoryBase::FImportAssetResult UInterchangeSkeletonFactory::BeginImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	...
	if (!Skeleton)
	{
		UE_LOGF(LogInterchangeImport, Warning, "Could not create Skeleton asset %ls", *Arguments.AssetName);
		return ImportAssetResult;
	}
	...
	{
		UE_LOGF(LogInterchangeImport, Warning, "Could not create Skeleton asset %ls, because there is no valid root joint node", *Arguments.AssetName);
		return ImportAssetResult;
	}
	if (bIsReImport)
	{
