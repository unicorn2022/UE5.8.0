...
UInterchangeFactoryBase::FImportAssetResult UInterchangeGeometryCacheFactory::BeginImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Cannot import geometry cache. The translator does not implement IInterchangeMeshPayloadInterface.");
	...
	for (int32 MeshIndex = 0; MeshIndex < MeshUids.Num(); ++MeshIndex)
	{
		...
		if (GeometryCacheNode == nullptr)
		{
			...
			if (!ensure(SceneNode))
			{
				UE_LOGF(LogInterchangeImport, Warning, "Invalid mesh reference when importing GeometryCache asset %ls.", *AssetName);
				continue;
			}
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Cannot import geometry cache. No valid mesh to import.");
		return ImportAssetResult;
	}
	...
	if (!GeometryCache)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "Could not create GeometryCache asset %ls.", *AssetName);
		}
		return ImportAssetResult;
	}
		...
		auto ReadFrame = [&ValidatedNodes, &AssetName, MeshTranslatorPayloadInterface, &PayloadAttributes](int32 FrameIndex)
		{
			...
			for (const FNodeInfo& NodeInfo : ValidatedNodes)
			{
				...
				if (!ensure(OptionalPayLoadKey.IsSet()))
				{
					...
					UE_LOGF(LogInterchangeImport, Warning, "Empty mesh reference payload when importing GeometryCache asset %ls.", *AssetName);
					continue;
				}
...
UInterchangeFactoryBase::FImportAssetResult UInterchangeGeometryCacheFactory::ImportAsset_Async(const FImportAssetObjectParams& Arguments)
{
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Could not import the GeometryCache asset %ls because the asset does not exist.", *Arguments.AssetName);
		return ImportAssetResult;
	}
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Could not cast to GeometryCache asset %ls.", *Arguments.AssetName);
		return ImportAssetResult;
	}
...
UInterchangeFactoryBase::FImportAssetResult UInterchangeGeometryCacheFactory::EndImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	...
	if (!ensure(GeometryCache))
	{
		UE_LOGF(LogInterchangeImport, Error, "Could not create GeometryCache asset %ls.", *Arguments.AssetName);
		return ImportAssetResult;
	}
