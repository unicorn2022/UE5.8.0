...
namespace UE::Interchange::Private::MeshHelper
{
	...
	)
	{
		...
		const UInterchangeSceneNode* MeshSceneNode = nullptr;
		...
		for (const FString& SocketUid : SocketUids)
		{
			if (const UInterchangeSceneNode* SceneNode = Cast<UInterchangeSceneNode>(Arguments.NodeContainer->GetNode(SocketUid)))
			{
				...
				{
					// In case we are not baking, we need to find the relative transform of the socket from the mesh non-baked origin.
					if (!MeshSceneNode)
					{
						// For this we need the current global transform for the mesh holding those sockets.
						// Since all sockets share the same mesh, we can do a search once and use the computed MeshGlobalTransform for every other sockets.
						TArray<FString> MeshNodeUids;
						FactoryNode->GetTargetNodeUids(MeshNodeUids);

						if (ensure(MeshNodeUids.Num() > 0))
						{
							const FString MeshNodeUid = MeshNodeUids[0];

							MeshSceneNode = Cast<UInterchangeSceneNode>(Arguments.NodeContainer->GetNode(SceneNode->GetParentUid()));
							while (MeshSceneNode)
							{
								FString MeshUid;
								if (MeshSceneNode->GetCustomAssetInstanceUid(MeshUid) && MeshUid == MeshNodeUid)
								{
									break;
								}

								MeshSceneNode = Cast<UInterchangeSceneNode>(Arguments.NodeContainer->GetNode(MeshSceneNode->GetParentUid()));
							}

							if (ensure(MeshSceneNode))
							{
								MeshSceneNode->GetCustomGlobalTransform(Arguments.NodeContainer, FTransform::Identity, MeshGlobalTransform);
							}
						}
					}
					...
					if (MeshSceneNode)
					{
						FTransform SocketGlobalTransform;
						SceneNode->GetCustomGlobalTransform(Arguments.NodeContainer, FTransform::Identity, SocketGlobalTransform);
						Transform = SocketGlobalTransform.GetRelativeTransform(MeshGlobalTransform);
					}
				}
	...
	void AddSceneNodeGeometricAndPivotToGlobalTransform(FTransform& GlobalTransform, const UInterchangeSceneNode* SceneNode, const bool bBakeMeshes, const bool bBakePivotMeshes)
	{
		...
		GlobalTransform = SceneNodeGeometricTransform * GlobalTransform;
	}

	//Resolves the optional RootJointParentGlobalInverseMatrix, or in case it's not set, 
	// it traverses the parent hierarchy of the SceneNode until it finds the RootJoint's Parent (RootJoint's Parent, is the first SceneNode that is not a JointNode) and acquire the GlobalInverseMatrix from found node.
	FMatrix ResolveRootJointParentGlobalInverseMatrix(UInterchangeBaseNodeContainer& BaseNodeContainer, const UInterchangeSceneNode* SceneNode, const FTransform& GlobalOffsetTransform, const TOptional<TPair<FString, FMatrix>>& RootJointParentUidAndGlobalInverseMatrixOptional)
	{
		FMatrix RootJointParentGlobalInverseMatrix = FMatrix::Identity;
		if (RootJointParentUidAndGlobalInverseMatrixOptional.IsSet())
		{
			RootJointParentGlobalInverseMatrix = RootJointParentUidAndGlobalInverseMatrixOptional.GetValue().Value;
		}
		else
		{
			const UInterchangeSceneNode* Current = SceneNode;
			const UInterchangeSceneNode* Candidate = Cast<UInterchangeSceneNode>(BaseNodeContainer.GetNode(Current->GetParentUid()));

			//In this case we will try to identify the root joint node via iterating through Parents until we hit the top or a non-UInterchangeJointNode.
			static const uint32 MaxDepthCheckLimit = UINT16_MAX;
			uint32 MaxDepthChecker = 0;
			while (Candidate && Candidate->IsA<UInterchangeJointNode>())
			{
				Current = Candidate;
				Candidate = Cast<UInterchangeSceneNode>(BaseNodeContainer.GetNode(Current->GetParentUid()));
				MaxDepthChecker++;
				if (MaxDepthChecker > MaxDepthCheckLimit)
				{
					Candidate = nullptr;
					break;
				}
			...
			if (Candidate)
			{
				FTransform RootJointParentGlobalTransform = FTransform::Identity;
				Candidate->GetCustomGlobalTransform(&BaseNodeContainer, GlobalOffsetTransform, RootJointParentGlobalTransform);

				RootJointParentGlobalInverseMatrix = RootJointParentGlobalTransform.ToMatrixWithScale().Inverse();
			}
		}

		return RootJointParentGlobalInverseMatrix;
	}

	/**
	* When we are trying to acquire the wanted/needed transform for a MeshNode we have to combine/calculate with the following transforms:
	*		- GlobalTransform: only if bBakeMeshes is true. (Global transform of the instantiating SceneNode)
	*		- GeometricTransform
	*		- PivotTransform: only if !bBakeMeshes && bBakePivotMeshes.
	* To do this we use the following function: UE::Interchange::Private::MeshHelper::AddSceneNodeGeometricAndPivotToGlobalTransform
	* The complete setup should be as follows:
	*		Transform:= MeshTransform * PivotNodeTransform * GeometricTransform * GlobalSceneNodeTransform
	*			MeshTransform is specific to LODMeshes (UInterchangeMeshLODContainerNode->FInterchangeLODMeshData.Transform)
	*				and it is not handled directly within this function.
	* In case the input SceneNode is a JointNode we are going to calculate the SkeletonTransform of the given Node (for bBakeMeshes=false)
	*/
	FTransform CalculateCompleteSceneNodeTransform(
		UInterchangeBaseNodeContainer* BaseNodeContainer, const UInterchangeSceneNode* SceneNode, 
		const bool bBakeMeshes, const bool bBakePivotMeshes, 
		const FTransform& GlobalOffsetTransform,
		const bool bUseTimeZeroAsBindPose,
		const TOptional<TPair<FString, FMatrix>>& RootJointParentUidAndGlobalInverseMatrixOptional,
		const bool bProcessingSkeletalMesh,
		const bool bAlignSkeletalMeshInScene)
	{
		...
		FTransform OutGlobalTransform = FTransform::Identity;
		
		if (const UInterchangeJointNode* JointNode = Cast<UInterchangeJointNode>(SceneNode))
		{
			FTransform JointNodeSkeletonTransform = FTransform::Identity;

			if (!bUseTimeZeroAsBindPose || !JointNode->GetTimeZeroGlobalTransform(BaseNodeContainer, GlobalOffsetTransform, JointNodeSkeletonTransform))
			{
				//Even if BindPose transform is not present it will fallback to Local Transform for the GlobalTransform generation
				//in case that fails, there is no other fallback mechanism to be applied.
				//Hence the reason why we can ignore the success check returned from GetBindPoseGlobalTransform.
				JointNode->GetBindPoseGlobalTransform(BaseNodeContainer, GlobalOffsetTransform, JointNodeSkeletonTransform);
			}

			if (bBakeMeshes)
			{
				OutGlobalTransform = JointNodeSkeletonTransform;
			}
			else
			{
				FMatrix RootJointParentGlobalInverseMatrix = ResolveRootJointParentGlobalInverseMatrix(*BaseNodeContainer, SceneNode, GlobalOffsetTransform, RootJointParentUidAndGlobalInverseMatrixOptional);

				//We calculate the 'Skeleton Transform' here:
				// In other words the Global/Relative transform to the RootJointNode's Parent.

				FMatrix JointNodeGlobalMatrix = JointNodeSkeletonTransform.ToMatrixWithScale();
				FTransform SkeletonTransform(JointNodeGlobalMatrix * RootJointParentGlobalInverseMatrix);

				OutGlobalTransform = SkeletonTransform;
			}
		}
		else if (bBakeMeshes)
		{
			FTransform SceneNodeGlobalTransform;
			if (SceneNode->GetCustomGlobalTransform(BaseNodeContainer, GlobalOffsetTransform, SceneNodeGlobalTransform))
			{
				OutGlobalTransform = SceneNodeGlobalTransform;
			}

			if (!bAlignSkeletalMeshInScene && 
				bProcessingSkeletalMesh && 
				RootJointParentUidAndGlobalInverseMatrixOptional.IsSet())
			{
				//**bAlignSkeletalMeshInScene==FALSE**
				//If the SK and SKM was aligned prior to the Scene Composition,
				//in that case we have to make sure we are baking the final SKM Instantiating Node's transform.
				// Which is the SkeletonRoot's Parent Global transform.
				//This is already part of the Mesh's SceneNode's GlobalTransform, IF the SceneNode is a child of the RootJoint's ParentNode,
				if (!BaseNodeContainer->GetIsAncestor(SceneNode->GetUniqueID(), RootJointParentUidAndGlobalInverseMatrixOptional.GetValue().Key))
				{
					//But if the SceneNode is not part of the RootJointParent children tree (aka GetIsAncestor returns false), 
					// then we need to apply the RootJointParentGlobalMatrix:
					//So for example, this scenario:
					//		SceneRoot
					//			RootJointParent
					//				RootJoint (aka Skeleton)
					//					BoneXYZ
					//					...
					//			TransformNodeABC
					//				Mesh (Mesh referenced)
					//	As the bAlignSkeletalMeshInScene==false, which means the alignment is happening outside of the Scene Composition (prior to it).
					//
					//In this case the RootJointParent's Global Matrix needs to be additionally baked.
					// Test file for this: glTF 2.0 example file RiggedFigure.
					FMatrix RootJointParentGlobalMatrix = ResolveRootJointParentGlobalInverseMatrix(*BaseNodeContainer, SceneNode, GlobalOffsetTransform, RootJointParentUidAndGlobalInverseMatrixOptional).Inverse();
					FTransform SkeletonTransform(OutGlobalTransform.ToMatrixWithScale() * RootJointParentGlobalMatrix);

					OutGlobalTransform = SkeletonTransform;
				}
			}
		}
		else if (bAlignSkeletalMeshInScene && bProcessingSkeletalMesh && RootJointParentUidAndGlobalInverseMatrixOptional.IsSet())
		{
			//**bAlignSkeletalMeshInScene==TRUE**
			//If it is expected that SKM and SK is aligned in the Scene (via the Scene Composition).
			//Then we have to bake the complete transform for the mesh (SceneNode), regardless of the bBakeMesh setting.
			// However we do have to remove the final SKM Instantiating Node's global transform 
			// (aka the RootJointParentGlobalTransfrom needs to be inverse applied, so that we keep SKM and SK allignment.)
			// So for example, for this scenario:
			//		SceneRoot
			//			RootJointParent
			//				RootJoint (aka Skeleton)
			//					BoneXYZ
			...
			//				TransformNodeABC
			//					Mesh (Mesh referenced)
			//	We have to remove the RootJointParent Global Transform in case bBakeMesh==false (which is this else if branch) 
			// And we also have to remove the RootJointParent Global Transform (aka apply the RootJointParentGlobalTransform.Inverse())
			//		for this scenario:
			//			SceneRoot
			//				RootJointParent
			//					RootJoint (aka Skeleton)
			...
			FMatrix RootJointParentGlobalInverseMatrix = ResolveRootJointParentGlobalInverseMatrix(*BaseNodeContainer, SceneNode, GlobalOffsetTransform, RootJointParentUidAndGlobalInverseMatrixOptional);
			FTransform SkeletonTransform(OutGlobalTransform.ToMatrixWithScale() * RootJointParentGlobalInverseMatrix);
		...
		, const bool bIsReimport)
	{
		...
		for (TPair<FString, FString>& SlotMaterialDependency : ImportedSlotMaterialDependencies)
		{
			...
			{
				// Fallback: the dependency value may be a content path to an existing material rather than a node UID. 
				// Use ResolveObject instead of TryLoad as this can run on a background thread
				FSoftObjectPath ContentPath(SlotMaterialDependency.Value);
				if (ContentPath.IsValid())
				{
					if (UMaterialInterface* MaterialInterface = Cast<UMaterialInterface>(ContentPath.ResolveObject()))
					{
						UpdateOrAddMaterial(MaterialSlotName, MaterialInterface, ImportedMaterialIndex);
						continue;
					}
				}
		...
		, const bool bMergeMorphTargetWithSameName)
	{
		...
		for (const TPair<FString, TOptional<UE::Interchange::FMeshPayloadData>>& Pair : LodMorphTargetMeshDescriptions)
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Error, "Empty morph target optional payload data [%ls].", *MorphTargetUniqueId);
				continue;
			}
			...
			{
				UE_LOGF(LogInterchangeImport, Error, "Corrupted morph target optional payload data [%ls].", *MorphTargetUniqueId);
				continue;
			}
		...
		for (TPair<FString, TArray<FString>>& NamePerUIDs : KeysPerName)
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Warning, "Duplicate morph target '%ls' found, renamed to '%ls'.", *MorphTargetNameBase, *MorphTargetName.ToString());
			}

			//Systemic morph target normals registration can be memory expensive for large mesh description (UE-348667).
			//If they are needed, they will be registered later when processing NDeltas, but for now, do not register them.
			DestinationMeshAttributes.RegisterMorphTargetAttribute(MorphTargetName, false);
			...
			TOptional<TVertexInstanceAttributesRef<FVector3f>> DestinationMeshMorphNormals;
			ensure(!DestinationMeshMorphNormals.IsSet());
			...
			for (const FString& MorphTargetKey : NamePerUIDs.Value)
			{
				...
				const FMeshDescription& MorphTargetMeshDescription = MorphTargetPayloadData->MeshDescription;
				const int32 VertexOffset = MorphTargetPayloadData->VertexOffset;
				...
				const bool bHasVertexInstanceNormals = MorphTargetMeshVertexInstanceNormals.IsValid() && (MorphTargetMeshVertexInstanceNormals.GetNumElements() > 0);
				...
				// and it will have the elements inside it due to VertexInstance creation automatically adds empty elements. (only exception if we don't have VertexInstances set to begin with)
				...
				for (FVertexID MorphTargetVertexID : MorphTargetMeshDescription.Vertices().GetElementIDs())
				{
					...
					{
						const TArrayView<const FVertexInstanceID> DestinationVertexInstanceIDs = DestinationMeshDescription.GetVertexVertexInstanceIDs(DestinationVertexID);
						const TArrayView<const FVertexInstanceID> MorphTargetVertexInstanceIDs = MorphTargetMeshDescription.GetVertexVertexInstanceIDs(MorphTargetVertexID);
						...
						if (DestinationVertexInstanceIDs.Num() == MorphTargetVertexInstanceIDs.Num())
						{
							for (size_t VertexInstanceIndex = 0; VertexInstanceIndex < DestinationVertexInstanceIDs.Num(); VertexInstanceIndex++)
							{
								...
								if (!NDelta.IsNearlyZero())
								{
									if (!DestinationMeshMorphNormals.IsSet())
									{
										//We only register morph target normals when finding at least one non-zero NDelta.
										DestinationMeshAttributes.RegisterMorphTargetAttribute(MorphTargetName, true);
										DestinationMeshMorphNormals = DestinationMeshAttributes.GetVertexInstanceMorphNormalDelta(MorphTargetName);
									}
	...
	EInterchangeMeshCollision GetMeshCollisionFromName(const FString& Name)
	{
		EInterchangeMeshCollision CollisionType = EInterchangeMeshCollision::None;

		if (Name.StartsWith(TEXT("UBX_")))
		{
			CollisionType = EInterchangeMeshCollision::Box;
		}
		else if (Name.StartsWith(TEXT("UCX_")) || Name.StartsWith(TEXT("MCDCX_")))
		{
			CollisionType = EInterchangeMeshCollision::Convex18DOP;
		}
		else if (Name.StartsWith(TEXT("USP_")))
		{
			CollisionType = EInterchangeMeshCollision::Sphere;
		}
		else if (Name.StartsWith(TEXT("UCP_")))
		{
	...
	bool HasPotentialMeshCollisionPrefix(const FString& Name)
	{
		return Name.StartsWith(TEXT("UBX")) ||
			Name.StartsWith(TEXT("UCX")) ||
			Name.StartsWith(TEXT("USP")) ||
			Name.StartsWith(TEXT("UCP")) ||
			Name.StartsWith(TEXT("MCDCX"));
	}
	...
	class FInternalNaniteAssemblyBuilder : public FInterchangeNaniteAssemblyBuilder::ImplBase
	{
		...
		void AddDescription(const TOptional<FNaniteAssemblyDescription>& InDescription) override
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Warning 
					,"Invalid description while building nanite assembly data for mesh '%ls' - %ls"
			...
			{
				UE_LOGF(LogInterchangeImport, Warning
					,"Invalid description while building nanite assembly data for mesh '%ls' - Skeletal "
			...
			{
				UE_LOGF(LogInterchangeImport, Warning
					, "Nanite assembly description part index data for mesh '%ls' contains one or more invalid indices."
					, *MeshFactoryNode->GetDisplayLabel()
				);
			}
		}
		...
		void EnableNaniteAndFinalize()
		{
			...
			{
				UE_LOGF(LogInterchangeImport, Warning
					,"Nanite assembly builder data for mesh '%ls' is invalid, skipping."
					, *MeshFactoryNode->GetDisplayLabel()
				);
				return;
			}
			...
			if (!ApplyResult)
			{
				...
				UE_LOGF(LogInterchangeImport, Warning
					, "Failed to apply assembly builder data to mesh '%ls', skipping."
					, * MeshFactoryNode->GetDisplayLabel()
				);
				return;
			}
		...
		bool GetPartObjects(const UE::Interchange::FNaniteAssemblyDescription& Description, FPartObjectList& OutPartObjects)
		{
			...
			if (!MissingPartUids.IsEmpty())
			{
				...
				UE_LOGF(LogInterchangeImport, Warning, "%ls", *Message);
			}
