...
namespace UE::Interchange::Private
{
	...
	void FLevelSequenceHelper::PopulateTransformTrack(const UInterchangeTransformAnimationTrackNode& TransformTrackNode)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "Cannot find actor for animation track %ls", *TransformTrackNode.GetDisplayLabel());
			return;
		}
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "No payload key for animation track %ls on actor %ls", *TransformTrackNode.GetDisplayLabel(), *Actor->GetActorLabel());
			return;
		}
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "No payload for animation track %ls on actor %ls", *TransformTrackNode.GetDisplayLabel(), *Actor->GetActorLabel());
			return;
		}
	...
	void FLevelSequenceHelper::PopulateSubsequenceTrack(const UInterchangeAnimationTrackSetInstanceNode& InstanceNode)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Error, "No unique id specified for the animation track set referenced by animation track %ls.", *InstanceNode.GetDisplayLabel());
			return;
		}
		...
		{
			UE_LOGF(LogInterchangeImport, Error, "Cannot find factory of animation track set referenced by animation track %ls.", *InstanceNodeDisplayLabel);
		};
	...
	void FLevelSequenceHelper::PopulateAnimationTrack(const UInterchangeAnimationTrackNode& AnimationTrackNode)
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "Cannot find actor for animation track %ls", *AnimationTrackNode.GetDisplayLabel());
			return;
		}
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "No payload key for animation track %ls on actor %ls", *AnimationTrackNode.GetDisplayLabel(), *Actor->GetActorLabel());
			return;
		}
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "No payload for animation track %ls on actor %ls", *AnimationTrackNode.GetDisplayLabel(), *Actor->GetActorLabel());
			return;
		}
...
UInterchangeFactoryBase::FImportAssetResult UInterchangeLevelSequenceFactory::BeginImportAsset_GameThread(const FImportAssetObjectParams& Arguments)
{
	...
	UE_LOGF(LogInterchangeImport, Error, "Cannot import levelsequence asset in runtime, this is an editor only feature.");
	...
	{
		UE_LOGF(LogInterchangeImport, Warning, "Level sequence asset, %ls, not imported, because all referenced actors are missing.", *FactoryNode->GetDisplayLabel());
		return ImportAssetResult;
	}
	...
	if (!LevelSequence)
	{
		UE_LOGF(LogInterchangeImport, Warning, "Could not create LevelSequence asset %ls", *Arguments.AssetName);
		return ImportAssetResult;
	}
...
UObject* UInterchangeLevelSequenceFactory::ImportObjectSourceData(const FImportAssetObjectParams& Arguments)
{
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Cannot import LevelSequence, the translator do not implement the IInterchangeAnimationPayloadInterface.");
		return nullptr;
	}
	...
	if (!ExistingAsset)
	{
		UE_LOGF(LogInterchangeImport, Error, "Could not import the LevelSequence asset %ls, because the asset do not exist.", *Arguments.AssetName);
		return nullptr;
	}
