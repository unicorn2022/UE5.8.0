...
void UInterchangeGroomFactory::CreatePayloadTasks(const FImportAssetObjectParams& Arguments, bool bAsync, TArray<TSharedPtr<UE::Interchange::FInterchangeTaskBase>>& PayloadTasks)
{
	...
	{
		UE_LOGF(LogInterchangeImport, Error, "Cannot import groom asset. The translator does not implement IInterchangeGroomPayloadInterface.");
		return;
	}
	...
	if (const UInterchangeGroomNode* GroomNode = Cast<const UInterchangeGroomNode>(Node))
	{
		...
		{
			UE_LOGF(LogInterchangeImport, Warning, "Empty groom reference payload when importing Groom asset %ls.", *Arguments.AssetName);
			return;
		}
