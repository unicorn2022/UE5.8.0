...
public:
	...
	/** Whether to first consider Fbx Clusters instead of FbxPoses for Bind Pose Calculations. */
	UPROPERTY(EditAnywhere, Category = "FBX Translator")
	bool bConsiderClusterBeforePoseForMeshBindPose = false;

	/** Whether to use the uFBX parser. */
	UPROPERTY(EditAnywhere, Category = "FBX Translator", meta = (EditCondition = "bDisplayUfbxParser", EditConditionHides, DisplayName="Use uFBX Translator (Experimental)", Tooltip = "If enabled, the ufbx SDK is used instead of the FBX SDK."))
...
private:
	INTERCHANGEIMPORT_API FString CreateLoadFbxFileCommand(const FString& FbxFilePath, const bool bConvertScene, const bool bForceFrontXAxis, const bool bConvertSceneUnit, const bool bKeepFbxNamespace, const bool bConsiderClusterBeforePoseForMeshBindPose) const;
