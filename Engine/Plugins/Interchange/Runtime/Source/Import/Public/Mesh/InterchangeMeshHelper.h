...
namespace UE::Interchange::Private::MeshHelper
{
	...
	* RootJointParentUidAndGlobalInverseMatrixOptional - Specifically used for SkeletalMesh's transform generations.
	* bProcessingSkeletalMesh - Changes transform generation behavior for Non Joint SceneNode transforms 
	* 							(we always bake to RootJoint or SceneRoot for SkinnedMeshes).
	*/
	INTERCHANGEIMPORT_API FTransform CalculateCompleteSceneNodeTransform(
		...
		const bool bUseTimeZeroAsBindPose = false,
		const TOptional<TPair<FString, FMatrix>>& RootJointParentUidAndGlobalInverseMatrixOptional = TOptional<TPair<FString, FMatrix>>(),
		const bool bProcessingSkeletalMesh = false,
		const bool bAlignSkeletalMeshInScene = false);
	...
	/* Checks name for UBX_, UCX_, MCDCX_, USP_, UCP_ prefixes.*/
	INTERCHANGEIMPORT_API EInterchangeMeshCollision GetMeshCollisionFromName(const FString& Name);

	/* Checks if Name has any of the prefixes mentioned in GetMeshCollisionFromName, without the '_', this primarily used for checking for typos. */
	INTERCHANGEIMPORT_API bool HasPotentialMeshCollisionPrefix(const FString& Name);
