...
namespace UE::Interchange::ActorHelper
{
	 ...
	 * In case of a reimport, PreviousActorFactoryNode is used to determine which slots should be updated
	 * according to the requested import strategy associated with ActorFactoryNode, i.e. do nothing, apply imported slots,
	 * apply imported slots while preserving changes made after the last import or reimport.
	 */
	UE_API void ApplySlotMaterialDependencies(const UInterchangeBaseNodeContainer& NodeContainer, const UInterchangeMeshActorFactoryNode& ActorFactoryNode, UMeshComponent& MeshComponent, const UInterchangeMeshActorFactoryNode* PreviousActorFactoryNode = nullptr);
