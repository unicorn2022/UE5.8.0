...
namespace GLTF
{
	...
	struct FBufferView
	{
		...
		bool HasCapacityFor(const uint32 InCount, const uint64 InByteOffset, const uint32 InElementSize, const uint32 InByteStride) const
		{
			if (!IsValid())
			{
				return false;
			...
			if (InElementSize == 0 || InByteStride == 0)
			{
				return false;
			}

			if (InByteOffset >= ByteLength)
			{
				return false;
			}
			...
			if (InCount > 0)
			{
				const uint64 AvailableBytes = ByteLength - InByteOffset;

				if (InElementSize > AvailableBytes)
				{
					return false;
				}
				if (InCount - 1 > (AvailableBytes - InElementSize) / InByteStride)
				{
					return false;
				}
			}
