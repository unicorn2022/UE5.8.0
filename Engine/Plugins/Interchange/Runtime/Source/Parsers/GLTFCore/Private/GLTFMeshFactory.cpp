...
namespace GLTF
{
	...
	private:
		...
		template <typename T>
		void ReIndex(const TArray<T>& Source, const TArray<uint32>& Indices, TArray<T>& Dst, const FString& AttributeName, const FString& MeshName);
		
		void GenerateFlatNormals(const TArray<FVector3f>& Positions, const TArray<uint32>& Indices, TArray<FVector3f>& Normals, const FString& MeshName);
		...
		template <typename T>
		void FMeshFactoryImpl::ReIndex(const TArray<T>& Source, const TArray<uint32>& Indices, TArray<T>& Dst, const FString& AttributeName, const FString& MeshName)
		{
			...
			bool bHasOOBIndex = false;
			for (uint32 Index : Indices)
			{
				if (Index >= NumOfSource)
				{
					bHasOOBIndex = true;
					Index = NumOfSource - 1;
				}
			...
			if (bHasOOBIndex)
			{
				FString WarningMsg = TEXT("Out of Bounds indices found in ") + AttributeName + TEXT(" for: '") + MeshName + TEXT("'. Clamping given indices.");
				Messages.Emplace(EMessageSeverity::Warning, FText::FromString(WarningMsg));
			}
		}

		void FMeshFactoryImpl::GenerateFlatNormals(const TArray<FVector3f>& Positions, const TArray<uint32>& Indices, TArray<FVector3f>& Normals, const FString& MeshName)
		{
			Normals.Empty();
			...
			Normals.AddDefaulted(N);

			uint32 NumOfPositions = Positions.Num();
			...
			bool bHasOOBIndex = false;
			auto ValidateIndex = [&NumOfPositions, &bHasOOBIndex](uint32& Index)
				{
					if (Index >= NumOfPositions)
					{
						bHasOOBIndex = true;
						Index = NumOfPositions - 1;
					}
				};
			...
			for (uint32 i = 0; i < N; i += 3)
			{
				...
				ValidateIndex(BIndex);
				ValidateIndex(CIndex);

				const FVector3f& A = Positions[AIndex];
				const FVector3f& B = Positions[BIndex];
				const FVector3f& C = Positions[CIndex];
			...
			if (bHasOOBIndex)
			{
				FString WarningMsg = TEXT("Out of Bounds indices found while generating Flat Normals for: '") + MeshName + TEXT("'. Clamping given indices.");
				Messages.Emplace(EMessageSeverity::Warning, FText::FromString(WarningMsg));
			}
		}
	...
	void FMeshFactoryImpl::FillMeshDescription(const FMesh &Mesh, const FTransform& MeshGlobalTransform, FMeshDescription* MeshDescription, const TArray<float>& MorphTargetWeights)
	{
		...
		for (int32 Index = 0; Index < Mesh.Primitives.Num(); ++Index)
		{
			...
			const bool        bHasDegenerateTriangles =
				ImportPrimitive(Primitive, Mesh.Name, Index, NumUVs, Mesh.HasTangents(), Mesh.HasColors(),  //
		...
		const TArray<float>& MorphTargetWeights)
	{
		...
		if (Primitive.HasNormals())
		{
			...
			ReIndex(Normals, Indices, ReindexBuffer, TEXT("Normals"), MeshName);
		...
		if (Normals.IsEmpty())
		{
			if (Primitive.HasNormals())
			{
				Messages.Emplace(EMessageSeverity::Warning,
					FText::Format(LOCTEXT("InvalidNormalsAccessor",
						"[{0}] Normal accessor declared but produced no data (invalid or empty); falling back to flat normals. Tangents discarded."),
			...
			GenerateFlatNormals(Positions, Indices, Normals, MeshName);
		...
		if (!bIgnoreTangents && Primitive.HasTangents())
		{
			...
			ReIndex(Tangents, Indices, ReindexBuffer, TEXT("Tangents"), MeshName);
			Swap(Tangents, ReindexBuffer);
		}
		
		if (bMeshHasTangents && Tangents.IsEmpty())
		{
		...
		if (Primitive.HasColors())
		{
			...
			ReIndex(Colors, Indices, ReindexBuffer, TEXT("Colors"), MeshName);
			Swap(Colors, ReindexBuffer);
		}
		
		if (bMeshHasColors && Colors.IsEmpty())
		{
		...
		for (int32 UVIndex = 0; UVIndex < NumUVs; ++UVIndex)
		{
			FString UVNameSpace = TEXT("UVs[") + FString::FromInt(UVIndex) + TEXT("]");
			...
			if (Primitive.HasTexCoords(UVIndex))
			{
				...
				ReIndex(*UVs[UVIndex], Indices, ReindexBuffer, UVNameSpace, MeshName);
				Swap(*UVs[UVIndex], ReindexBuffer);
			}
			
			if ((*UVs[UVIndex]).IsEmpty())
			{
		...
		else
		{
			for (uint32 TriangleIndex = 0; TriangleIndex < TriCount; ++TriangleIndex)
			{
				...
				PolygonObjectNames.Set(NewPolygonID, MeshFName);
			}
		}
