...
namespace GLTF
{
	...
	void FExtensionsHandler::SetupMaterialExtensions(const FJsonObject& Object, FMaterial& Material) const
	{
		...
		static const TArray<EExtension> Extensions = { 
			...
			EExtension::KHR_MaterialsVolume,
		...
		for (int32 Index = 0; Index < Extensions.Num(); ++Index)
		{
			...
			switch (Extension)
			{
				...
				case EExtension::KHR_MaterialsSpecular:
				{
					...
					Material.Specular.SpecularColorFactor = GetVec3(Specular, TEXT("specularColorFactor"), FVector::OneVector);
				...
				case EExtension::KHR_MaterialsIridescence:
				{
					...
					Asset->ProcessedExtensions.Add(EExtension::KHR_MaterialsIridescence);
				}
				break;
				case EExtension::KHR_MaterialsAnisotropy:
				{
					...
					Asset->ProcessedExtensions.Add(EExtension::KHR_MaterialsAnisotropy);
				}
				break;
				case EExtension::KHR_MaterialsVolume:
				{
					const FJsonObject& Volume = ExtObj;
					Material.Volume.bHasVolume = true;

					Material.Volume.Thickness = GetScalar(Volume, TEXT("thicknessFactor"));
					Material.Volume.AttenuationDistance = GetScalar(Volume, TEXT("attenuationDistance"), 1.f);
					Material.Volume.AttenuationColor = GetVec3(Volume, TEXT("attenuationColor"), FVector::OneVector);
					GLTF::SetTextureMap(Volume, TEXT("thicknessTexture"), nullptr, Asset->Textures, Material.Volume.ThicknessTexture, Messages);
					Asset->ProcessedExtensions.Add(EExtension::KHR_MaterialsVolume);
				}
				break;
				default:
					if (!ensure(false))
	...
	void FExtensionsHandler::CheckExtensions(const FJsonObject& Object, const TArray<FString>& ExtensionsSupported) const
	{
		...
		{
			if (ExtensionsSupported.Find(*StrValuePair.Key) == INDEX_NONE)
			{
				Messages.Emplace(RuntimeWarningSeverity(), FText::Format(LOCTEXT("UnsupportedExtension", "Extension is not supported: {0}"), FText::FromString(*StrValuePair.Key)));
			}
		}
	}
