...
namespace GLTF
{
	namespace
	{
		...
		void ProcessExtras(const FJsonObject& Object, TMap<FString, FString>& StorageForExtras, TSet<FString> KeyExceptions = {})
		{
			if (Object.HasField(TEXT("extras")))
			{
				...
				{
					for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Pair : Extras->Values)
					{
						FString KeyStr(Pair.Key);
						if (KeyExceptions.Contains(KeyStr))
						{
							continue;
						}
						...
						{
							StorageForExtras.Add(MoveTemp(KeyStr), ExtraString);
						}
					}
				}
				else
				{
					for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& Pair : Extras->Values)
					{
						...
						if (Pair.Value->TryGetString(ExtraString))
						{
							StorageForExtras.Add(FString(Pair.Key), ExtraString);
						}
					}
				}
			}
	...
	void FFileReader::LoadMetadata(GLTF::FAsset& OutAsset)
	{
		...
		for (const auto& ValuePair : Extras->Values)
		{
			...
			OutAsset.Metadata.Extras.Emplace(FMetadata::FExtraData {FString(ValuePair.Key), JsonValue->AsString()});
		}
	}
