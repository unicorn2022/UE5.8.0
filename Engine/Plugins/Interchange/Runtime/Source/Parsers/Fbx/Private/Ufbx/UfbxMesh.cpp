...
namespace UE::Interchange::Private
{
	...
	public:
		...
		void AddFace(FPolygonGroupID GroupID, const ufbx_face& Face, uint32 FaceIndex)
		{
			...
			for (int32 TriangleIndex = 0; TriangleIndex < FaceTriCount; ++TriangleIndex, TriangulatedIndices+=3)
			{
				...
				PolygonToFaceIndex.Add(FaceIndex);
		...
		// Maps FPolygonID->GetValue() for triangle to it's ufbx face index in ufbx
		TArray<FPolygonID> PolygonToFaceIndex;
	...
	void FUfbxMesh::AddAllMeshes(UInterchangeBaseNodeContainer& NodeContainer)
	{
		for (ufbx_mesh* Mesh : Parser.Scene->meshes)
		{
			...
			if (Cast<UInterchangeMeshNode>(NodeContainer.GetNode(MeshNameAndUid.UniqueID)))
			{
				continue;
			}
			...
			bool bMeshHasSmoothGroup = Mesh->face_smoothing.count > 0 || Mesh->edge_smoothing.count > 0;
			...
			{
				const FString MorphTargetName = Parser.GetElementName(Deformer->element);
				for (int32 ChannelIndex = 0; ChannelIndex < Deformer->channels.count; ++ChannelIndex)
				{
					...
					FString ChannelName = Parser.GetElementName(Channel->element);
					...
					for (int32 ChannelMorphTargetIndex = 0; ChannelMorphTargetIndex < Channel->keyframes.count; ++ChannelMorphTargetIndex)
					{
						...
						FString ShapeName = CurrentChannelMorphTargetCount > 1
							? Parser.GetElementName(Shape->element)
						...
						if (!ExistingMorphTargetNode)
						{
							...
							if (!Parser.PayloadContexts.Contains(MorphTargetPayLoadKey))
							{
						...
						if (!SkeletonRootPerBlendShape.Contains(Shape))
						{
	...
	bool FUfbxMesh::FetchBlendShape(FUfbxParser& Parser, FMeshDescription& MeshDescription, const ufbx_mesh& Mesh, const ufbx_blend_shape& BlendShape, const FTransform& MeshGlobalTransform)
	{
		...
		TArray<ufbx_vec3> Vertices(Mesh.vertices.data, Mesh.vertices.count);
	...
	bool FetchMesh(FUfbxParser& Parser, FMeshDescription& MeshDescription, ufbx_element* Element, const FTransform& MeshGlobalTransform, TFunctionRef<void (FFaceCollector& FaceCollector)> HandleMesh=[](FFaceCollector&){})
	{
		...
		bool bAppliedSmoothingFromData = false;
#ifdef WITH_MESH_DESCRIPTION_BUILDER
		FMeshDescriptionBuilder Builder;
		...
		if (bMakeMeshGroupsPerMaterial)
		{
			...
			for (int32 MaterialIndex = 0; MaterialIndex < Mesh.material_parts.count; ++MaterialIndex)
			{
				...
				if (MeshPart.num_faces > 0)
				{
					...
					{
						const uint32 UfbxFaceIndex = MeshPart.face_indices.data[FaceIndex];
						FaceCollector.AddFace(GroupID, Mesh.faces[UfbxFaceIndex], UfbxFaceIndex);
					}
				}
			}
		}
		else
		{
			...
			for (int32 FaceIndex = 0; FaceIndex < FaceCount; ++FaceIndex)
			{
				FaceCollector.AddFace(GroupID, Mesh.faces[FaceIndex], (uint32)FaceIndex);
			}
		}
		...
		// Apply edge hardness from ufbx smoothing data if available.
		// ByEdge: direct 1:1 mapping from ufbx edge flags to MeshDescription edges via vertex-pair lookup.
		// ByPolygon (face_smoothing bool): edge is hard when ALL faces have face_smoothing==false. This handles the common all-zero case correctly. 
		// But:
		// #ufbx_todo: face_smoothing is bool in ufbx and this is wrong since in source data in fbx it is group BIT MASK(32 bit for 32 groups, why 3ds Max has 32 group limit btw) 
		// and ufbx converts it to bool. So different smoothing groups both appear true and get soft edges here 
		// Correct fix requires changing ufbx.c and storing the raw int32 bitmask. 
		if (Mesh.edge_smoothing.count > 0)
		{
			bAppliedSmoothingFromData = true;
			TEdgeAttributesRef<bool> EdgeHardnesses = Attributes.GetEdgeHardnesses();

			for (int32 EdgeIdx = 0; EdgeIdx < (int32)Mesh.edge_smoothing.count; ++EdgeIdx)
			{
				const ufbx_edge& Edge = Mesh.edges[EdgeIdx];
				const FVertexID VertexA = FaceCollector.VertexMapping[Mesh.vertex_position.indices[Edge.a]];
				const FVertexID VertexB = FaceCollector.VertexMapping[Mesh.vertex_position.indices[Edge.b]];
				FEdgeID EdgeID = MeshDescription.GetVertexPairEdge(VertexA, VertexB);
				if (EdgeID == INDEX_NONE)
				{
					EdgeID = MeshDescription.CreateEdge(VertexA, VertexB);
				}
				// ufbx softness to UE hardness
				EdgeHardnesses[EdgeID] = !Mesh.edge_smoothing.data[EdgeIdx];
			}
		}
		else if (Mesh.face_smoothing_groups.count > 0)
		{
			// Raw int32 ByPolygon bitmask smoothing groups. Two faces share a smooth edge iff
			// (group_a & group_b) != 0. 
			// All-zero groups are handled naturally. Zero means polygon doesn't belong to any group. Or doesn't share a group with another polygon. (0 & 0) == 0 -> hard.
			// Exception: skinned mesh with all-zero groups uses normals-based detection (FBX SDK parity - see FMeshDescriptionImporter::FillMeshDescriptionFromFbxMesh).
			bool bSkinnedAllZero = (Mesh.skin_deformers.count > 0);
			if (bSkinnedAllZero)
			{
				for (uint32 FaceSmoothingMask: Mesh.face_smoothing_groups)
				{
					if (FaceSmoothingMask)
					{
						bSkinnedAllZero = false;
						break;
					}
				}
			}
			...
			if (!bSkinnedAllZero)
			{
				...
				{
					TArray<FPolygonID, TInlineAllocator<2>> ConnectedPolygons;
					MeshDescription.GetEdgeConnectedPolygons(EdgeID, ConnectedPolygons);

					uint32 GroupA = 0;
					uint32 GroupB = 0;
					if (ConnectedPolygons.Num() > 1)
					{
						GroupA = Mesh.face_smoothing_groups.data[FaceCollector.PolygonToFaceIndex[ConnectedPolygons[0].GetValue()]];
						GroupB = Mesh.face_smoothing_groups.data[FaceCollector.PolygonToFaceIndex[ConnectedPolygons[1].GetValue()]];
					}
					EdgeHardnesses[EdgeID] = (GroupA & GroupB) == 0;
				}
			}
		}
#endif

		if (!bAppliedSmoothingFromData)
		{
			...
			for (int32 VertexIndex = 0; VertexIndex < Skin.vertices.count; ++VertexIndex)
			{
				...
				for (uint32 WeightIndex = 0; WeightIndex < SkinVertex.num_weights; ++WeightIndex)
				{
					...
					if (BoneNode && !SortedJoints.Find(BoneNode, BoneIndex))
					{
						...
						FString JointNodeUniqueID = Parser.GetBoneNodeName(*BoneNode);
