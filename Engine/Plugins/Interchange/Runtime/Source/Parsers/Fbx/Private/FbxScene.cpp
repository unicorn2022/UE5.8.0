...
namespace UE
{
	namespace Interchange
	{
		namespace Private
		{
			...
			FTransform GetConvertedTransform(FbxNode* Node, const FbxAMatrix& NewFbxMatrix)
			{
			...
			void FFbxScene::ParseMesh(UInterchangeBaseNodeContainer& NodeContainer
				...
				, FbxNodeAttribute* NodeAttribute)
			{
				...
				FTransform GeometricTransform = GetConvertedTransform(Node, Geometry);
				...
				FTransform PivotNodeTransform = GetConvertedTransform(Node, PivotGeometry);

				CreateMeshNodeReference(UnrealNode, NodeAttribute, NodeContainer, GeometricTransform, PivotNodeTransform);
			}

			void FFbxScene::ParseMeshLODs(UInterchangeMeshLODContainerNode& LODContainerNode
				, int32 LODIndex
				, FbxNode* Node
				, FbxScene* SDKScene
				, const FbxAMatrix& InvTransformMatrix /*Global Inverse Transform Matrix Of LODContainer*/)
			{
				//#interchange_LODRefactor_Note: we could report warnings, if unexpected NodeAttribute is found (for ep eSkeleton etc)
					...
					switch (NodeAttribute->GetAttributeType())
					{
						case FbxNodeAttribute::eMesh:
						{
							...
							if (ensure(Mesh))
							{
								...
								//Get relative transform to LODContainerNode:
								FbxAMatrix RelativeMatrix = InvTransformMatrix * SDKScene->GetAnimationEvaluator()->GetNodeGlobalTransform(Node, 0);
								FTransform RelativeTransform = GetConvertedTransform(Node, RelativeMatrix);
								...
								Geometry.SetR(Rotation);
								Geometry.SetS(Scaling);
								FTransform GeometryTransform = GetConvertedTransform(Node, Geometry);
								...
								FullPivot[2] = -RotationPivot[2];
								PivotGeometry.SetT(FullPivot);
								FTransform PivotTransform = GetConvertedTransform(Node, PivotGeometry);
								
								//Combine relative geometirc and pivot transform matrices.
								FTransform CombinedTransform = PivotTransform * GeometryTransform * RelativeTransform;

								LODContainerNode.AddMeshForLODIndex(LODIndex, MeshRefString, CombinedTransform);
							}
							break;
						}
						default:
				...
				for (int32 ChildIndex = 0; ChildIndex < ChildCount; ++ChildIndex)
				{
					...
					ParseMeshLODs(LODContainerNode, LODIndex, ChildNode, SDKScene, InvTransformMatrix);
				}
			}
			...
			{	
				UInterchangeSceneNode* UnrealNode = CreateUnrealSceneNode(UnrealParentNode, Node, SDKScene, NodeContainer,
					ForceJointNodes, bBadBindPoseMessageDisplay, FbxJointMeshBindPoseGenerator);

				bool bAllowChildrenParsing = true;
				...
				for (int32 AttributeIndex = 0; AttributeIndex < AttributeCount; ++AttributeIndex)
				{
					...
					switch (NodeAttribute->GetAttributeType())
					{
						...
						case FbxNodeAttribute::eNull:
							// eNull were already process at this point.
							break;

						case FbxNodeAttribute::eSkeleton:
							// Joint were already process at this point.
							break;

						case FbxNodeAttribute::eMesh:
						{
							ParseMesh(NodeContainer, UnrealNode, Node, NodeAttribute);
							break;
						}
						case FbxNodeAttribute::eLODGroup:
						{
							//From here on out everything should be procesed into a MeshLODContainer.
							FString LODContainerUid = UInterchangeMeshLODContainerNode::MakeMeshLODContainerUid(UnrealNode->GetUniqueID());
							...
							NodeContainer.SetupNode(
								LODContainerNode,
								LODContainerUid,
								UnrealNode->GetDisplayLabel(),
								EInterchangeNodeContainerType::TranslatedAsset
							);

							UnrealNode->SetCustomAssetInstanceUid(LODContainerUid);
							
							FbxAMatrix InverseGlobalTransformMatrixOfLODContainer = SDKScene->GetAnimationEvaluator()->GetNodeGlobalTransform(Node, 0).Inverse();

							const int32 ChildCount = Node->GetChildCount();
							for (int32 ChildIndex = 0; ChildIndex < ChildCount; ++ChildIndex)
							{
								FbxNode* ChildNode = Node->GetChild(ChildIndex);
								ParseMeshLODs(*LODContainerNode, ChildIndex, ChildNode, SDKScene, InverseGlobalTransformMatrixOfLODContainer);
							}

							bAllowChildrenParsing = false;
				...
				if (!UnrealNode->IsA(UInterchangeJointNode::StaticClass()))
				{
					//Set the node default transform
					FbxAMatrix GlobalFbxMatrix = Node->EvaluateGlobalTransform();
					FTransform GlobalTransform = GetConvertedTransform(Node, GlobalFbxMatrix);
					if (FbxNode* ParentNode = Node->GetParent())
					{
						...
						FTransform LocalTransform = GetConvertedTransform(Node, LocalFbxMatrix);
						UnrealNode->SetCustomLocalTransform(&NodeContainer, LocalTransform);
					}
					else
					{
						//No parent, set the same matrix as the global
				...
				for(int32 i = 0, Count = Node->GetNodeAttributeCount(); i < Count; ++i)
				{
					...
					while(Property.IsValid())
					{
						...
						if(CurveNode && CurveNode->IsAnimated() && FFbxAnimation::IsFbxPropertyTypeSupported(PropertyType))
						{
							...
							FFbxAnimation::AddNodeAttributeCurvesAnimation(Parser, Node, Property, CurveNode, PayloadContexts, PropertyType, PayloadKey, bIsStepCurve);
			...
			UInterchangeSceneNode* FFbxScene::CreateUnrealSceneNode(UInterchangeSceneNode* UnrealParentNode
				, FbxNode* Node
				, FbxScene* SDKScene
				, UInterchangeBaseNodeContainer& NodeContainer
				, const TArray<FbxNode*>& ForceJointNodes
				...
				, FFbxJointMeshBindPoseGenerator& FbxJointMeshBindPoseGenerator)
			{
				auto ApplySkeletonAttribute = [this, &SDKScene, &Node, &NodeContainer, &bBadBindPoseMessageDisplay, &FbxJointMeshBindPoseGenerator](UInterchangeJointNode* JointNode, bool bIsOriginalJoint)
					{
						if (FRootJointInfo* RootJointInfo = CommonJointRootNodes.Find(Node))
						{
							if (!RootJointInfo->bValidBindPose)
							{
								JointNode->SetHasInvalidBindPose(true);
							}
						}

						bool bIsRootJoint = false;
						if (const UInterchangeSceneNode* UEParentNode = Cast< UInterchangeSceneNode >(NodeContainer.GetNode(JointNode->GetParentUid())))
						{
							if (!UEParentNode->IsA(UInterchangeJointNode::StaticClass()))
							{
								bIsRootJoint = true;
							}
						}
...

						{
							...
							FTransform GlobalBindPoseJointTransform = GetConvertedTransform(Node, GlobalBindPoseJointMatrix);
							...
							if (ParentNode != nullptr)
							{
								...
								FTransform LocalBindPoseJointTransform = GetConvertedTransform(Node, LocalFbxMatrix);
						...
						//Get time Zero transform for this joint
						{
							...
							JointNode->SetGlobalMatrixForT0Rebinding(FFbxConvert::ConvertMatrix<FMatrix>(GlobalFbxMatrix));
				...
				const FString NodeName = Parser.GetFbxHelper()->GetFbxObjectName(Node);
				const FString NodeUniqueID = Parser.GetFbxHelper()->GetFbxNodeHierarchyName(Node);
				const FString ParentNodeUid = UnrealParentNode ? UnrealParentNode->GetUniqueID() : TEXT("");

				const bool bNodeUnderCommonRootJoint = IsNodeUnderCommonJointRootNode(Node, CommonJointRootNodes);

				const int32 AttributeCount = Node->GetNodeAttributeCount();

				// We need to know in advance if we have to create a UInterchangeSceneNode or UInterchangeJointNode.
				for (int32 AttributeIndex = 0; AttributeIndex < AttributeCount; ++AttributeIndex)
				{
					FbxNodeAttribute* NodeAttribute = Node->GetNodeAttributeByIndex(AttributeIndex);
					...
					{
						// All eSkeleton/bones are converted to an Interchange Joint node
						bIsJoint = true;
						bIsNodeContainJointAttribute = true;
					}
					else
					{
						if (NodeAttribute->GetAttributeType() == FbxNodeAttribute::eNull && bNodeUnderCommonRootJoint)
						{
							// Any eNull under an eSkeleton or equivalent is considered an eSkeleton too
							bIsJoint = true;
							bIsNodeContainJointAttribute = true;
						}
					}
				...
				if (!bIsJoint) //Can happen when AttributeCount is 0.
				{
					if (ForceJointNodes.Contains(Node) || (!bIsRootNode && bNodeUnderCommonRootJoint))
					{
						// Finally, any node that is forced as a bone or that is under an eSkeleton or equivalent (except the RootNode) is considered non-native bones.
						bIsJoint = true;
						bIsNodeContainJointAttribute = false;
					}
				}

				if (bIsJoint)
				{
					UnrealNode = CreateUnrealSceneNodeInternal<UInterchangeJointNode>(NodeContainer, NodeName, NodeUniqueID, ParentNodeUid);
					ApplySkeletonAttribute(CastChecked<UInterchangeJointNode>(UnrealNode), bIsNodeContainJointAttribute);
				}
				else
				{
					UnrealNode = CreateUnrealSceneNodeInternal<UInterchangeSceneNode>(NodeContainer, NodeName, NodeUniqueID, ParentNodeUid);
				}
				...
				UnrealNode->SetCustomActorVisibility(Node->GetVisibility());

				return UnrealNode;
			}

			template<typename T>
			T* FFbxScene::CreateUnrealSceneNodeInternal(UInterchangeBaseNodeContainer& NodeContainer, const FString& NodeName, const FString& NodeUID, const FString& ParentNodeUID)
			{
			...
			bool FFbxScene::IsValidBindPose(FbxScene* SDKScene, FbxNode* RootJoint)
			{
			...
			void FFbxScene::AddHierarchy(FbxScene* SDKScene, UInterchangeBaseNodeContainer& NodeContainer, TMap<FString, TSharedPtr<FPayloadContextBase, ESPMode::ThreadSafe>>& PayloadContexts)
			{
				...
				TArray<FbxNode*> ForceJointNodes = FindForceJointNode(SDKScene);

				//Cache the common root joint
				FillCommonJointRootNode(SDKScene, ForceJointNodes);

				FFbxJointMeshBindPoseGenerator FbxJointMeshBindPoseGenerator(SDKScene, Parser, Parser.GetUseFbxClustersForBindPoseMatrices());
				...
				, TMap<FString, TSharedPtr<FPayloadContextBase, ESPMode::ThreadSafe>>& PayloadContexts) const
			{
				...
				FbxAnimStack* AnimationStack = (FbxAnimStack*)SDKScene->GetSrcObject<FbxAnimStack>(AnimationIndex);
				...
				while (Property.IsValid())
				{
					...
					{
						FbxAnimCurveNode* CurveNode = Property.GetCurveNode(AnimationStack);
				...
				if (UsedChannels)
				{
					...
					FFbxAnimation::AddRigidTransformAnimation(Parser, Node, AnimationIndex, TranlsationCurveNode, RotationCurveNode, ScaleCurveNode, PayloadContexts, PayloadKey);
			...
			FbxNode* FFbxScene::Internal_GetRootSkeleton(FbxScene* SDKScene, FbxNode* Link, bool bIsCreatorBlender)
			{
			...
			void FFbxScene::FillCommonJointRootNode(FbxScene* SDKScene, const TArray<FbxNode*>& ForceJointNodes)
			{
				...
				for (int32 NodeIndex = 0; NodeIndex < NodeCount; ++NodeIndex)
				{
					if (FbxNode* Node = SDKScene->GetNode(NodeIndex))
					{
						...
						if (bProcessNode)
						{
							if (FbxNode* Root = Internal_GetRootSkeleton(SDKScene, Node, Parser.IsCreatorBlender()))
							{
								if (!CommonJointRootNodes.Contains(Root))
								{
									CommonJointRootNodes.Add(Root, FFbxScene::FRootJointInfo(IsValidBindPose(SDKScene, Root)));
								}
							}
						}
					}
			...
			TArray<FbxNode*> FFbxScene::FindForceJointNode(FbxScene* SDKScene)
			{
				...
				for (int32 GeometryIndex = 0; GeometryIndex < GeometryCount; ++GeometryIndex)
				{
					...
					const FbxMesh* Mesh = static_cast<FbxMesh*>(Geometry);
					if (!Mesh)
					{
						continue;
					}
				...
				, const TArray<FbxNode*>& ForceJointNodes)
			{
				...
				if (UInterchangeSceneNode* UnrealNode = const_cast<UInterchangeSceneNode*>(Cast< UInterchangeSceneNode >(NodeContainer.GetNode(NodeUniqueID))))
				{
					...
					bool bNewSkeletalAnimationStarted = false;
					...
					for (int32 AttributeIndex = 0; AttributeIndex < AttributeCount && !HasSkeletonAttribute; ++AttributeIndex)
					{
						...
						case FbxNodeAttribute::eSkeleton:
							bIsNodeContainJointAttribute = true;
							bNewSkeletalAnimationStarted = ApplySkeletonAttribute() || bNewSkeletalAnimationStarted;
							break;
						default:
							break;
						}
					...
					if (bNewSkeletalAnimationStarted)
					{
					...
					if (!HasSkeletonAttribute)
					{
						...
						//as on the next occurrence of a Joint node a New skeleton will start:
					...
					else if (SkeletalAnimationTrackNode)
					{
						...
						if (FFbxAnimation::AddSkeletalTransformAnimation(NodeContainer, SDKScene, Parser, Node, UnrealNode, PayloadContexts, SkeletalAnimationTrackNode, AnimationIndex, bNewSkeletalAnimationStarted)
					...
					AddRigidAnimation(Node, SDKScene, AnimationIndex, UnrealNode, NodeContainer, PayloadContexts);
				}
				...
				NodeContainer.IterateNodesOfType<UInterchangeMeshNode>([&](const FString& NodeUid, UInterchangeMeshNode* MeshNode)
					{
						...
						while (!JointNodeUid.Equals(UInterchangeBaseNode::InvalidNodeUid()))
						{
							...
							{
								if (Node->IsA(UInterchangeJointNode::StaticClass()))
								{
