			...
			for (int32 PolygonIndex = 0; PolygonIndex < PolygonCount; PolygonIndex++)
			{
				...
				for (int32 CornerIndex = 0; CornerIndex < PolygonVertexCount; CornerIndex++)
				{
					...
					if (LayerElementVertexColor)
					{
						...
						int32 VectorColorIndex = (VertexColorReferenceMode == FbxLayerElement::eDirect) ? //-V516 PVS 7.41 bug
					...
					if (LayerElementNormal)
					{
						...
						int NormalValueIndex = (NormalReferenceMode == FbxLayerElement::eDirect) ? //-V516 PVS 7.41 bug
						...
						if (bHasNTBInformation)
						{
							int TangentMapIndex = (TangentMappingMode == FbxLayerElement::eByControlPoint) ?
								ControlPointIndex : RealFbxVertexIndex;
							int TangentValueIndex = (TangentReferenceMode == FbxLayerElement::eDirect) ? //-V516 PVS 7.41 bug
							...
							int BinormalMapIndex = (BinormalMappingMode == FbxLayerElement::eByControlPoint) ?
								ControlPointIndex : RealFbxVertexIndex;
							int BinormalValueIndex = (BinormalReferenceMode == FbxLayerElement::eDirect) ? //-V516 PVS 7.41 bug
				...
				// Create polygon edges
				{
					...
					for (uint32 PolygonEdgeNumber = 0; PolygonEdgeNumber < (uint32)PolygonVertexCount; ++PolygonEdgeNumber)
					{
						...
						if (!EdgeHardnesses[MatchEdgeId])
						{
							...
							{
								int32 lSmoothingIndex = (SmoothingReferenceMode == FbxLayerElement::eDirect) ? EdgeIndex : SmoothingInfo->GetIndexArray().GetAt(EdgeIndex); //-V516 PVS 7.41 bug
								//Set the hard edges
					...
					if (FBXUVs.LayerElementUV[UVLayerIndex] != NULL)
					{
						for (int32 VertexIndex = 0; VertexIndex < 3; ++VertexIndex)
						{
							int UVMapIndex = (FBXUVs.UVMappingMode[UVLayerIndex] == FbxLayerElement::eByControlPoint)
								...
								: CurrentVertexInstanceIndex + VertexIndex;
							int32 UVIndex = (FBXUVs.UVReferenceMode[UVLayerIndex] == FbxLayerElement::eDirect)
								? UVMapIndex
		...
		for (int32 MorphTargetIndex = 0; MorphTargetIndex < MorphTargetCount; ++MorphTargetIndex)
		{
			...
			for (int32 ChannelIndex = 0; ChannelIndex < MorphTargetChannelCount; ++ChannelIndex)
			{
				...
				for (int32 ChannelMorphTargetIndex = 0; ChannelMorphTargetIndex < CurrentChannelMorphTargetCount; ++ChannelMorphTargetIndex)
				{
					...
					static const FString NameClash = "_ncl_";

					if (ShapeNameToFbxShape.Contains(ShapeName))
					{
						int32 UniqueId = 1;
						FString UniqueShapeName;
						do
						{
							UniqueShapeName = ShapeName + NameClash + FString::FromInt(UniqueId++);
						}
						while (ShapeNameToFbxShape.Contains(UniqueShapeName));

						UE_LOGF(LogInterchangeFbxParser, Warning,
							"Duplicate morph target channel name '%ls' found in FBX file. It will be imported as '%ls'.",
							*ShapeName, *UniqueShapeName);
...
FFbxJointMeshBindPoseGenerator::FFbxJointMeshBindPoseGenerator(FbxScene* SDKScene, FFbxParser& Parser, const bool bConsiderClusterBeforePoseForMeshBindPose)
{
	...
	TSet<const FbxNode*> ValidationTracker; //Gets reset everytime when a BindPose gets recreated.
	TMap<const FbxNode*, FString> JointToGeometryMap; //Maps Joint to the Geometry that the Bind Pose is being used from.
	...
	for (int32 GeometryIndex = 0; GeometryIndex < GeometryCount; ++GeometryIndex)
	{
		...
		FString GeometryName = Parser.GetFbxHelper()->GetFbxObjectName(Geometry);
		...
		for (int32 GeometryDeformerIndex = 0; GeometryDeformerIndex < GeometryDeformerCount; ++GeometryDeformerIndex)
		{
			...
			for (int32 ClusterIndex = 0; ClusterIndex < ClusterCount; ClusterIndex++)
			{
				...
				//Acquire Cluster transforms:
				FbxAMatrix BoneBindMatrix_Cluster;
				FbxAMatrix MeshBindMatrix_Cluster;
				Cluster->GetTransformMatrix(MeshBindMatrix_Cluster);
				Cluster->GetTransformLinkMatrix(BoneBindMatrix_Cluster);

				//Acquire BindPose transforms, reasoning:
				...
				FbxAMatrix BoneBindMatrix_Pose;
				FbxAMatrix MeshBindMatrix_Pose;
				bool bRetrieveBindPoseResult = RetrievePoseFromBindPose(Joint, BoneBindMatrix_Pose);
				if (!bRetrieveBindPoseResult)
				{
					// if failed, delete bind pose, and retry.
					...
					bRetrieveBindPoseResult = RetrievePoseFromBindPose(Joint, BoneBindMatrix_Pose);

					if (!bRetrieveBindPoseResult)
					{
						Cluster->GetTransformLinkMatrix(BoneBindMatrix_Pose);
					}
				}
				if (!RetrievePoseFromBindPose(Geometry->GetNode(), MeshBindMatrix_Pose))
				{
					Cluster->GetTransformMatrix(MeshBindMatrix_Pose);
				}

				if (bRetrieveBindPoseResult && !bClusterVsPoseMissmatch)
				{
					// Convert to FMatrix to check equality with tolerance. 
					const FMatrix ClusterMatrix = FFbxConvert::ConvertMatrix<FMatrix>(BoneBindMatrix_Cluster);
					const FMatrix PoseMatrix = FFbxConvert::ConvertMatrix<FMatrix>(BoneBindMatrix_Pose);

					if (!ClusterMatrix.Equals(PoseMatrix, KINDA_SMALL_NUMBER))
					{
				...
				if (bConsiderClusterBeforePoseForMeshBindPose)
				{
					MeshBindMatrix = MeshBindMatrix_Cluster;
					BoneBindMatrix = BoneBindMatrix_Cluster;
				}
				else
				{
					MeshBindMatrix = MeshBindMatrix_Pose;
					BoneBindMatrix = BoneBindMatrix_Pose;
				}

				TMap<FString, FMatrix>& MeshIdToGlobalBindPoseJointMap = JointToMeshIdToGlobalBindPoseJointMap.FindOrAdd(Joint);
				MeshIdToGlobalBindPoseJointMap.Add(MeshUniqueID, FFbxConvert::ConvertMatrix<FMatrix>(BoneBindMatrix));

				const FbxAMatrix BindPoseFbxAMatrix = BoneBindMatrix * Parser.JointOrientationMatrix;
				if (!JointToBindPoseMap.Contains(Joint))
				{
					JointToBindPoseMap.Add(Joint, BindPoseFbxAMatrix);
					ValidationTracker.Add(Joint);
					JointToGeometryMap.Add(Joint, GeometryName);
				}
				else
				{
					if (ValidationTracker.Contains(Joint))
					{
						// Convert to FMatrix to check equality with tolerance. 
						const FMatrix ExistingMatrix = FFbxConvert::ConvertMatrix<FMatrix>(JointToBindPoseMap[Joint]);
						const FMatrix NewMatrix = FFbxConvert::ConvertMatrix<FMatrix>(BindPoseFbxAMatrix);

						if (!NewMatrix.Equals(ExistingMatrix, KINDA_SMALL_NUMBER))
						{
							FText JointName = FText::FromString(Parser.GetFbxHelper()->GetFbxObjectName(Joint));
							bool bWarningAdded = false;
							if (JointToGeometryMap.Contains(Joint))
							{
								if (GeometryName != JointToGeometryMap[Joint])
								{
									//Conflicting bind Poses.
									//#interchange_todo: Perhaps in the future we could introduce a Pose selector for Bind Pose calculations. Currently out of scope.
									...
									Message->Text = FText::Format(LOCTEXT("FFbxJointMeshBindPoseGenerator_ConflictingBindPoseMatrices", 
										...
										JointName, 
										FText::FromString(JointToGeometryMap[Joint]), FText::FromString(ExistingMatrix.ToString()),
										FText::FromString(GeometryName), FText::FromString(NewMatrix.ToString()));
							...
							{
								UInterchangeResultWarning_Generic* Message = Parser.AddMessage<UInterchangeResultWarning_Generic>();
								Message->Text = FText::Format(LOCTEXT("FFbxJointMeshBindPoseGenerator_UnexpectedBindPoseMatrices", "Imported SkeletalMesh and/or Skeleton might be incorrect.\nExpected to have same joint [{0}] node in different clusters to have same Bind Pose matrix!"), FText::FromString(Parser.GetFbxHelper()->GetFbxObjectName(Joint)));
							}
						}
					}
					else
					{
						JointToBindPoseMap[Joint] = BindPoseFbxAMatrix;
						JointToGeometryMap.Add(Joint, GeometryName);
					}
				}

				TMap<FString, FMatrix>& MeshIdToGlobalBindPoseReferenceMap = JointToMeshIdToGlobalBindPoseReferenceMap.FindOrAdd(Joint);
				MeshIdToGlobalBindPoseReferenceMap.Add(MeshUniqueID, FFbxConvert::ConvertMatrix<FMatrix>(MeshBindMatrix));
			}
		}
	}
