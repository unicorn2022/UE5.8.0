...
namespace UE
{
	namespace Interchange
	{
		...
		void FInterchangeDispatcher::SetTaskState(int32 TaskIndex, ETaskState TaskState, const FString& JsonResult, const TArray<FString>& JSonMessages)
		{
			...
			UE_CLOGF(TaskState == ETaskState::ProcessOk, LogInterchangeDispatcher, Verbose, "Json processed: %ls", *JsonDescription);
			UE_CLOGF(TaskState == ETaskState::UnTreated, LogInterchangeDispatcher, Warning, "Json resubmitted: %ls", *JsonDescription);
			UE_CLOGF(TaskState == ETaskState::ProcessFailed, LogInterchangeDispatcher, Error, "Json processing failure: %ls", *JsonDescription);
		}
		...
		void FInterchangeDispatcher::WaitAllTaskToCompleteExecution()
		{
			if (!IsValid())
			{
				UE_LOGF(LogInterchangeDispatcher, Error, "Cannot execute tasks before starting the process");
				return;
			}
			...
			if (!IsOver())
			{
				UE_LOGF(LogInterchangeDispatcher, Warning,
					   "Begin local processing. (Multi Process failed to consume all the tasks)\n" "See workers logs: %lsPrograms/InterchangeWorker/Saved/Logs", *FPaths::ConvertRelativePathToFull(FPaths::EngineDir()));
			}
			else
			{
				UE_LOGF(LogInterchangeDispatcher, Verbose, "Multi Process ended and consumed all the tasks");
			}
		}
				...
				WorkerApplicationPath = [&]()
					{
						...
						if (!FPaths::FileExists(Path))
						{
							UE_LOGF(LogInterchangeDispatcher, Display, "InterchangeWorker executable not found. Expected location: %ls", *FPaths::ConvertRelativePathToFull(Path));
						}
						return Path;
					}();
			}
