	...
	namespace Interchange
	{
		...
		void FInterchangeWorkerHandler::StartWorkerProcess()
		{
			...
			if (FPaths::FileExists(ProcessorPath))
			{
				...
				UE_LOGF(LogInterchangeDispatcher, Verbose, "CommandToProcess: %ls", *CommandToProcess);
		...
		void FInterchangeWorkerHandler::ValidateConnection()
		{
			...
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "NetworkInterface lost");
			...
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "Worker lost");
				WorkerState = EWorkerState::Closing;
			...
			if(CurrentTime - LastProgressMessageTime > ProgressTickTime)
			{
				...
				{
					UE_LOGF(LogInterchangeDispatcher, Verbose, "Query progress command sent");
				}
				else
				{
		...
		void FInterchangeWorkerHandler::Run()
		{
			...
			UE_CLOGF(ErrorState != EWorkerErrorState::Ok, LogInterchangeDispatcher, Display, "Handler ended with fault: %ls", EWorkerErrorStateAsString(ErrorState));
		}
		...
		void FInterchangeWorkerHandler::RunInternal()
		{
			auto StartNewTask = [this]()->bool
			{
				...
				if (CurrentTask.IsSet())
				{
					...
					{
						UE_LOGF(LogInterchangeDispatcher, Verbose, "New task command sent");
						bSucceed = true;
					}
					else
					{
				...
				switch (WorkerState)
				{
					...
					case EWorkerState::Idle:
					{
						...
						else
						{
							...
							if (bShouldTerminate)
							{
								UE_LOGF(LogInterchangeDispatcher, Verbose, "Exit loop gracefully");
								WorkerState = EWorkerState::Closing;
							}
							// No task to execute, wait a little
							else if (WorkerState == EWorkerState::Idle)
							{
								FPlatformProcess::Sleep(static_cast<float>(Config::IdleLoopDelay));
							}
						}
						break;
					}
		...
		{
			UE_LOGF(LogInterchangeDispatcher, Display, "%ls", *ErrorCommand.ErrorMessage);
