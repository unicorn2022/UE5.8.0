...
namespace UE
{
	namespace Interchange
	{
		...
		bool FNetworkNode::SendMessage(const TArray<uint8>& Message, double Timeout_s)
		{
			...
			if (bWriteError)
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "bWriteError flag raised, can't write");
				return false;
			}

			if (ConnectedSocket == nullptr)
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "node not connected, can't write");
				return false;
			}
			...
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "connection state error");
				return false;
			}
			...
			if (!bCanWrite)
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "can't write on socket");
				return false;
			}
			...
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "can't write header on socket");
				bWriteError = true;
				return false;
			}
			...
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "can't write content on socket");
				bWriteError = true;
				return false;
			}
		...
		bool FNetworkNode::ReceiveMessage(TArray<uint8>& OutMessage, double Timeout_s)
		{
			...
			if (bReadError)
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "ReadError flag raised, can't read");
				return false;
			}

			if (ConnectedSocket == nullptr)
			{
				UE_LOGF(LogInterchangeDispatcher, Display, "node not connected, can't read");
				return false;
			}
			...
			if (IncommingMessage.Header.ByteSize < 0)
			{
				...
				{
					UE_LOGF(LogInterchangeDispatcher, Display, "Parsed header failed");
					bReadError = true;
					return false;
				}
				...
				{
					UE_LOGF(LogInterchangeDispatcher, Display, "Parsed header failed: bad message size %d", IncommingMessage.Header.ByteSize);
					bReadError = true;
					return false;
				}
			...
			if (IncommingMessage.Header.ByteSize >= 0 && ConnectedSocket->HasPendingData(PendingByteCount))
			{
				...
				{
					bReadError = true;
					UE_LOGF(LogInterchangeDispatcher, Display, "Recv issue");
					return false;
				}
		...
		FSocket* FNetworkNode::CreateInternalSocket(const FString& Description)
		{
			...
			UE_CLOGF(!NewSocket, LogInterchangeDispatcher, Display, "Socket creation failure");
			return NewSocket;
		}
		...
		FNetworkServerNode::FNetworkServerNode()
		{
			...
			if (BoundPort == 0)
			{
				...
				UE_LOGF(LogInterchangeDispatcher, Display, "Socket binding failure");
				return;
			}
			...
			if (!ListeningSocket->Listen(0))
			{
				...
				UE_LOGF(LogInterchangeDispatcher, Display, "Socket listen failure");
				return;
			}
		}
		...
		bool FNetworkClientNode::Connect(const FString& Description, int32 ServerPort, double Timeout_s)
		{
			...
			do
			{
				...
				{
					UE_LOGF(LogInterchangeDispatcher, Verbose, "Client Node is connected");
					return true;
				}
			} while (FPlatformTime::Seconds() < ConnectTimeout_s);

			UE_LOGF(LogInterchangeDispatcher, Display, "Client socket failed to connect");
			CloseSocket(ConnectedSocket);
			return false;
		}
