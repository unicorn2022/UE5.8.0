...
void UInterchangeGenericAssetsPipeline::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
	Ar.UsingCustomVersion(FInterchangeCustomVersion::GUID);
}

void UInterchangeGenericAssetsPipeline::PostLoad()
{
	Super::PostLoad();

	if (GetLinkerCustomVersion(FInterchangeCustomVersion::GUID) < FInterchangeCustomVersion::SkeletalAndStaticMeshOptionsCleanUp)
	{
		if (CommonSkeletalMeshesAndAnimationsProperties && CommonMeshesProperties)
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			CommonMeshesProperties->bConvertStaticsWithAnimatedTransformToSkeletals = CommonMeshesProperties->bAutoDetectMeshType_DEPRECATED;
			CommonMeshesProperties->bConvertStaticsInBoneHierarchyToSkeletals = CommonSkeletalMeshesAndAnimationsProperties->bImportMeshesInBoneHierarchy_DEPRECATED;
			CommonMeshesProperties->bConvertStaticsWithMorphTargetsToSkeletals = CommonSkeletalMeshesAndAnimationsProperties->bConvertStaticsWithMorphTargetsToSkeletals_DEPRECATED;
...
void UInterchangeGenericAssetsPipeline::AdjustSettingsForContext(const FInterchangePipelineContextParams& ContextParams)
{
	...
	if (ContextParams.ContextType == EInterchangePipelineContext::AssetImport)
	{
		if (CommonSkeletalMeshesAndAnimationsProperties
			&& CommonSkeletalMeshesAndAnimationsProperties->bTryAutoSelectSkeleton
			&& !CommonSkeletalMeshesAndAnimationsProperties->IsSkeletonValid())
		{
...
void UInterchangeGenericAssetsPipeline::FilterPropertiesFromTranslatedData(UInterchangeBaseNodeContainer* InBaseNodeContainer)
{
	...
	if (CommonMeshesProperties && CommonSkeletalMeshesAndAnimationsProperties && MeshPipeline && AnimationPipeline)
	{
		...
		if (SkeletalMeshes.Num() > 0)
		{
			...
			{
				HideProperty(OuterMostPipeline, CommonMeshesProperties, GET_MEMBER_NAME_CHECKED(UInterchangeGenericCommonMeshesProperties, VertexOverrideColor));
				HideProperty(OuterMostPipeline, CommonMeshesProperties, GET_MEMBER_NAME_CHECKED(UInterchangeGenericCommonMeshesProperties, VertexColorImportOption));
				HideProperty(OuterMostPipeline, MeshPipeline, GET_MEMBER_NAME_CHECKED(UInterchangeGenericMeshPipeline, bImportMorphTargets));
				HideProperty(OuterMostPipeline, MeshPipeline, GET_MEMBER_NAME_CHECKED(UInterchangeGenericMeshPipeline, ThresholdPosition));
				HideProperty(OuterMostPipeline, MeshPipeline, GET_MEMBER_NAME_CHECKED(UInterchangeGenericMeshPipeline, ThresholdTangentNormal));
				HideProperty(OuterMostPipeline, MeshPipeline, GET_MEMBER_NAME_CHECKED(UInterchangeGenericMeshPipeline, ThresholdUV));
				HideProperty(OuterMostPipeline, MeshPipeline, GET_MEMBER_NAME_CHECKED(UInterchangeGenericMeshPipeline, MorphThresholdPosition));
			}
			else if (MeshPipeline->SkeletalMeshImportContentType == EInterchangeSkeletalMeshContentType::Geometry)
			{
				HideProperty(OuterMostPipeline, MeshPipeline, GET_MEMBER_NAME_CHECKED(UInterchangeGenericMeshPipeline, bUpdateSkeletonReferencePose));
			}
		}
		...
		if (CommonSkeletalMeshesAndAnimationsProperties->bImportOnlyAnimations)
		{
			if (MaterialPipeline)
			{
				HidePropertiesOfCategory(OuterMostPipeline, MaterialPipeline, UInterchangeGenericMaterialPipeline::GetPipelineCategory(nullptr));
				if (MaterialPipeline->TexturePipeline)
				{
					HidePropertiesOfCategory(OuterMostPipeline, MaterialPipeline->TexturePipeline, UInterchangeGenericTexturePipeline::GetPipelineCategory(nullptr));
				}
			}
		}
		...
		{
			HidePropertiesOfCategory(OuterMostPipeline, MeshPipeline, UInterchangeGenericMeshPipeline::GetPipelineCategory(UStaticMesh::StaticClass()));
		}
		if (bHideSkeletalMeshes)
		{
			HidePropertiesOfCategory(OuterMostPipeline, MeshPipeline, UInterchangeGenericMeshPipeline::GetPipelineCategory(USkeletalMesh::StaticClass()));
		}
		if (bHideGeometryCaches)
		{
			HidePropertiesOfCategory(OuterMostPipeline, MeshPipeline, UInterchangeGenericMeshPipeline::GetPipelineCategory(UGeometryCache::StaticClass()));
		}
		if (bHideCommonMeshes)
		{
			HidePropertiesOfCategory(OuterMostPipeline, CommonMeshesProperties, UInterchangeGenericCommonMeshesProperties::GetPipelineCategory(nullptr));
		}
		if (bHideCommonSkeletalMeshesAndAnimations)
		{
			...
			if (!CommonSkeletalMeshesAndAnimationsProperties->bImportOnlyAnimations)
			{
				HidePropertiesOfCategory(OuterMostPipeline, CommonSkeletalMeshesAndAnimationsProperties, UInterchangeGenericCommonSkeletalMeshesAndAnimationsProperties::GetPipelineCategory(nullptr));
			}
		}
		if (bHideCommonSkeletalMeshesAndAnimations_StaticMesh)
		{
			HidePropertiesOfCategory(OuterMostPipeline, CommonSkeletalMeshesAndAnimationsProperties, UInterchangeGenericMeshPipeline::GetPipelineCategory(UStaticMesh::StaticClass()));
		}
		if (bHideAnimations)
		{
			HidePropertiesOfCategory(OuterMostPipeline, AnimationPipeline, UInterchangeGenericAnimationPipeline::GetPipelineCategory(nullptr));
		}
	}
...
void UInterchangeGenericAssetsPipeline::InternalRecursiveFillJointsFromNodeContainer(TSharedPtr<FSkeletonJoint> ParentJoint, TMap<FString, TSharedPtr<FSkeletonJoint>>& Joints, const FString& JoinUid, const UInterchangeBaseNodeContainer* BaseNodeContainer, const bool bConvertStaticToSkeletalActive)
{
	...
	//If we are not converting static to skeleton then we should only accept UInterchangeJointNodes
	if (!bConvertStaticToSkeletalActive && !SceneNode->IsA(UInterchangeJointNode::StaticClass()))
	{
		return;
	}
	...
	//We should not have any name collision
	if (Joints.Contains(ImportBoneName))
	{
		ensure(false);
		return;
	}
...
void UInterchangeGenericAssetsPipeline::CreateSkeletonConflict(USkeleton* SpecifiedSkeleton, USkeletalMesh* SkeletalMesh, UInterchangeBaseNodeContainer* TransientBaseNodeContainer)
{
	...
	constexpr int32 LodIndex = 0;
	{
		...
		const bool bConvertStaticToSkeletalActive = CommonMeshesProperties->bConvertStaticsWithMorphTargetsToSkeletals || CommonMeshesProperties->ForceAllMeshAsType == EInterchangeForceMeshType::IFMT_SkeletalMesh;
...
#if WITH_EDITOR
	USkeleton* SpecifiedSkeleton = CommonSkeletalMeshesAndAnimationsProperties->IsSkeletonValid() 
		...
		if (ConflictInfo.UniqueId == ConflictUniqueId)
		{
			if (ensure(ConflictInfo.Pipeline == this))
			{
				...
				else if (SkeletonConflictData.ConflictUniqueId == ConflictUniqueId)
				{
					...
					USkeleton* Skeleton = [this, ReimportSkeletalMesh]() -> USkeleton* {
						if (CommonSkeletalMeshesAndAnimationsProperties->IsSkeletonValid())
						{
							return CommonSkeletalMeshesAndAnimationsProperties->Skeleton.LoadSynchronous();
						}

						if (ReimportSkeletalMesh)
						{
							return ReimportSkeletalMesh->GetSkeleton();
						}
...
void UInterchangeGenericAssetsPipeline::ExecutePipeline(UInterchangeBaseNodeContainer* InBaseNodeContainer, const TArray<UInterchangeSourceData*>& InSourceDatas, const FString& ContentBasePath)
{
	...
	{
		UE_LOGF(LogInterchangePipeline, Warning, "UInterchangeGenericAssetsPipeline: Cannot execute pre-import pipeline because InBaseNodeContrainer is null");
		return;
	}
...
namespace UE::Interchange::Private
{
	static bool ExpansionFilter_ConflictFilter(TSharedPtr<UInterchangeGenericAssetsPipeline::FSkeletonJoint> Joint)
	{
		return Joint.IsValid() && Joint->bMatch && Joint->bChildConflict;
	}
}
...
FReply SInterchangeGenericAssetSkeletonConflictWidget::OnExpandToConflict()
{
	...
	{
		OnSetExpansionRecursive(Joint, false);
	}

	for (TSharedPtr<UInterchangeGenericAssetsPipeline::FSkeletonJoint> Joint : Joints)
	{
		OnSetExpansionRecursive(Joint, true, FExpansionPredicate::CreateStatic(&ExpansionFilter_ConflictFilter));
	}
	return FReply::Handled();
}
...
void SInterchangeGenericAssetSkeletonConflictWidget::OnSetExpansionRecursive(TSharedPtr<UInterchangeGenericAssetsPipeline::FSkeletonJoint> SkeletonJoint, bool bShouldBeExpanded, const FExpansionPredicate Predicate)
{
	if (!SkeletonJoint.IsValid())
	{
		return;
	}
	...
	{
		if (!Predicate.Execute(SkeletonJoint))
		{
			return;
		}
	}

	CompareTree->SetItemExpansion(SkeletonJoint, bShouldBeExpanded);
	for (TSharedPtr<UInterchangeGenericAssetsPipeline::FSkeletonJoint> ChildJoint : SkeletonJoint->Children)
	{
		OnSetExpansionRecursive(ChildJoint, bShouldBeExpanded, Predicate);
	}
}
...
TSharedPtr<SWidget> SInterchangeGenericAssetSkeletonConflictWidget::ConstructSkeletonComparison()
{
	...
	CompareTree = SNew(STreeView<TSharedPtr<UInterchangeGenericAssetsPipeline::FSkeletonJoint>>)
		...
		.OnSetExpansionRecursive(
			this,
			&SInterchangeGenericAssetSkeletonConflictWidget::OnSetExpansionRecursive, 
			FExpansionPredicate());
					...
					[
						...
						[
							...
							[
								SNew(SButton)
								.HAlign(HAlign_Center)
								.Text(NSLOCTEXT("SInterchangeGenericAssetSkeletonConflictWidget", "SInterchangeGenericAssetSkeletonConflic_ExpandToConflict", "Expand To Conflict"))
