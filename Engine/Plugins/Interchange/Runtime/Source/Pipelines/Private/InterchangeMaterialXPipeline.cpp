...
namespace
{
	...
	constexpr const TCHAR* GltfPBRFunctionsPath = TEXT("/InterchangeAssets/gltf/Substrate/MF_GLTFPBR.MF_GLTFPBR");
	constexpr const TCHAR* GltfPBRSubstratePath = TEXT("/InterchangeAssets/gltf/Substrate/MF_GLTFPBR.MF_GLTFPBR");
...
{	
	...
	if(HasAnyFlags(EObjectFlags::RF_ClassDefaultObject))
	{
		...
		SettingsInputsOutputs = {
			...
			{
				UMaterialXPipelineSettings::ToEnumKey(EInterchangeMaterialXShaders::GltfPBR),
				{
					// GltfPBR Inputs
					TSet<FName>{
						mx::GltfPbr::Input::Alpha,
						mx::GltfPbr::Input::AlphaCutoff,
						mx::GltfPbr::Input::AlphaMode,
						mx::GltfPbr::Input::AnisotropyRotation,
						mx::GltfPbr::Input::AnisotropyStrength,
						mx::GltfPbr::Input::AttenuationColor,
						mx::GltfPbr::Input::AttenuationDistance,
						mx::GltfPbr::Input::BaseColor,
						mx::GltfPbr::Input::Clearcoat,
						mx::GltfPbr::Input::ClearcoatNormal,
						mx::GltfPbr::Input::ClearcoatRoughness,
						mx::GltfPbr::Input::Dispersion,
						mx::GltfPbr::Input::Emissive,
						mx::GltfPbr::Input::EmissiveStrength,
						mx::GltfPbr::Input::Ior,
						mx::GltfPbr::Input::Iridescence,
						mx::GltfPbr::Input::IridescenceIor,
						mx::GltfPbr::Input::IridescenceThickness,
						mx::GltfPbr::Input::Metallic,
						mx::GltfPbr::Input::Normal,
						mx::GltfPbr::Input::Occlusion,
						mx::GltfPbr::Input::Roughness,
						mx::GltfPbr::Input::SheenColor,
						mx::GltfPbr::Input::SheenRoughness,
						mx::GltfPbr::Input::Specular,
						mx::GltfPbr::Input::SpecularColor,
						mx::GltfPbr::Input::Tangent,
						mx::GltfPbr::Input::Thickness,
						mx::GltfPbr::Input::Transmission
					},
					// GltfPbr Outputs					
					TSet<FName>{
						SubstrateMaterial::Parameters::FrontMaterial,
	...
	: MaterialXSettings(UMaterialXPipelineSettings::StaticClass()->GetDefaultObject<UMaterialXPipelineSettings>())
{
	if (HasAnyFlags(EObjectFlags::RF_ClassDefaultObject))
	{
		...
		PathToEnumMapping = {
			...
			{MATERIALX_MATERIALFUNCTION_PATH(GltfPBR),					  UMaterialXPipelineSettings::ToEnumKey(EInterchangeMaterialXShaders::GltfPBR)},
...
void UInterchangeMaterialXPipeline::ExecutePipeline(UInterchangeBaseNodeContainer* NodeContainer, const TArray<UInterchangeSourceData*>& InSourceDatas, const FString& ContentBasePath)
{
	...
	TArray<UInterchangeMaterialFunctionCallExpressionFactoryNode*> DisplacementNodes;
	// only add the StaticBoolParameter if the MX_Displacement hasn't been overridden
	const bool bRecomputeNormalsStaticBool = DisplacementFunctionsPath ==  MaterialXSettings->PredefinedSurfaceShaders.FindChecked(EInterchangeMaterialXShaders::Displacement).ToString();

	auto UpdateMaterialXNodes = [this, NodeContainer, &DisplacementNodes, bRecomputeNormalsStaticBool](const FString& NodeUid, UInterchangeMaterialFunctionCallExpressionFactoryNode* FactoryNode)
	{
		...
		FString FunctionShaderNodeUID = FactoryNode->GetUniqueID();
		...
		{
			if (int32 EnumValue; FunctionCallShaderNode->GetInt32Attribute(UE::Interchange::MaterialX::Attributes::EnumValue, EnumValue))
			{
				FactoryNode->AddStringAttribute(MaterialFunctionMemberName, MaterialXSettings->GetAssetPathString(MaterialXSettings->ToEnumKey(EnumType, EnumValue)));

				if (bRecomputeNormalsStaticBool &&
					EnumType == UE::Interchange::MaterialX::IndexSurfaceShaders &&
					EnumValue == int32(EInterchangeMaterialXShaders::Displacement))
				{
					DisplacementNodes.Emplace(FactoryNode);
				}
			}
		}
		if(FString MaterialFunctionPath; FactoryNode->GetStringAttribute(MaterialFunctionMemberName, MaterialFunctionPath))
		{
			if (const EInterchangeMaterialXSettings* EnumPtr = PathToEnumMapping.Find(MaterialFunctionPath))
			{
				FactoryNode->AddStringAttribute(MaterialFunctionMemberName, MaterialXSettings->GetAssetPathString(*EnumPtr));
			}
		}
	};
	...
	// Create StaticBool expression Factory node
	for(UInterchangeMaterialFunctionCallExpressionFactoryNode* DisplacementExpressionFactoryNode : DisplacementNodes)
	{
		const FString ParentUid = DisplacementExpressionFactoryNode->GetUniqueID();
		const FString InputName = TEXT("bRecomputeNormals");
		const FString MaterialExpressionUid = ParentUid + TEXT("\\") + InputName;

		UInterchangeMaterialExpressionFactoryNode* StaticBoolExpressionFactoryNode = NewObject<UInterchangeMaterialExpressionFactoryNode>(NodeContainer);
		StaticBoolExpressionFactoryNode->SetCustomExpressionClassName(UMaterialExpressionStaticBoolParameter::StaticClass()->GetName());
		NodeContainer->SetupNode(StaticBoolExpressionFactoryNode, MaterialExpressionUid, InputName, EInterchangeNodeContainerType::FactoryData, ParentUid);

		const FName DefaultValueMemberName = GET_MEMBER_NAME_CHECKED(UMaterialExpressionStaticBoolParameter, DefaultValue);
		StaticBoolExpressionFactoryNode->AddBooleanAttribute(DefaultValueMemberName.ToString(), bRecomputeNormals);
		StaticBoolExpressionFactoryNode->AddApplyAndFillDelegates<bool>(DefaultValueMemberName.ToString(), UMaterialExpressionStaticBoolParameter::StaticClass(), DefaultValueMemberName);

		UInterchangeShaderPortsAPI::ConnectDefaultOuputToInput(DisplacementExpressionFactoryNode, InputName, StaticBoolExpressionFactoryNode->GetUniqueID());
	}
#endif
}
...
void UMaterialXPipelineSettings::InitPredefinedAssets()
{
	if(bIsSubstrateEnabled)
	{
		TArray<TTuple<EInterchangeMaterialXSettings, FString, FString>> MappingToSubstrate
		{
			...
			{UMaterialXPipelineSettings::ToEnumKey(EInterchangeMaterialXShaders::GltfPBR), GltfPBRFunctionsPath, GltfPBRSubstratePath},
