...
void UInterchangeGenericMeshPipeline::ExecutePreImportPipelineGeometryCache()
{
	...
	{
		// The bConvertStaticsWithAnimatedTransformToSkeletals setting converts static meshes with animated transforms into skeletal meshes with AnimSequence
		// For the purpose of geometry cache, both are considered as just meshes
		TArray<FString> AnimMeshInstanceUids;
		if (CommonMeshesProperties->bConvertStaticsWithAnimatedTransformToSkeletals && CommonMeshesProperties->ForceAllMeshAsType == EInterchangeForceMeshType::IFMT_None)
		{
	...
	for (const FString& MeshUid : MeshInstanceUids)
	{
		...
		//UInterchangeInstancedStaticMeshComponentNode is not supported and hence will be skipped.
		if (const UInterchangeSceneNode* SceneNode = Cast<const UInterchangeSceneNode>(MeshInstance.InstancingNode))
		{
			...
			else
			{
				...
				FString AssetUid;
				if (SceneNode->GetCustomAssetInstanceUid(AssetUid) && !VisitedMeshAssetUids.Contains(AssetUid))
				{
...
UInterchangeGeometryCacheFactoryNode* UInterchangeGenericMeshPipeline::CreateGeometryCacheFactoryNode(const TArray<FString>& MeshUids)
{
	...
	FString DisplayLabel;
	{
		...
		if (const UInterchangeSceneNode* SceneNode = Cast<const UInterchangeSceneNode>(Node))
		{
			...
			if (!ParentSceneNode || ParentSceneNode->GetParentUid() == UInterchangeBaseNode::InvalidNodeUid())
			{
				...
				SceneNode->GetCustomAssetInstanceUid(GeometryCacheUid);
			}
			else
			{
	...
	const FString StaticMeshUid = TEXT("Factory_GeometryCache_") + GeometryCacheUid;
