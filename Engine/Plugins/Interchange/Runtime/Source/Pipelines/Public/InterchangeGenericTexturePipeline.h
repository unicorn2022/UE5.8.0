...
public:
	...
	/** Detect if a texture should be imported as long/lat cubemap. The minimum size to detect a cubemap texture is a 512 pixels height. Use the CVar Interchange.Textures.MinCubeMapDetection if you want to control that setting.*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Textures", Meta = (EditCondition = "bImportTextures && (bIsReimport == false)"))
	bool bDetectLongLatCubemap = false;

	/** Specify the file types that should be imported as long/lat cubemaps. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Textures", Meta=(EditCondition="bImportTextures && (bDetectLongLatCubemap == false)"))
