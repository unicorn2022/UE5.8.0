...
enum class EInterchangeMaterialSearchLocation : uint8
{
	...
	AllAssets,
	/** Do not search for existing existing materials. Deprecated replaced with boolean bReuseExistingMaterials */
	DoNotSearch  UE_DEPRECATED(5.8, "Moved as a boolean, bReuseExistingMaterials in UInterchangeGenericMaterialPipeline") UMETA(Hidden, Deprecated),
};
...
public:
	...
	/** If enabled, and there is only one asset and one source, the imported asset will be given this name. */
	...
	/**
	 * When enabled, if the name of a material in the imported source matches a material
	 * that already exists in the project, the imported assets will reuse that existing material.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta = (EditCondition = "bImportMaterials"))
	bool bReuseExistingMaterials = true;

	/** Determines where to look for matching materials.*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta = (EditCondition = "bImportMaterials && bReuseExistingMaterials"))
	EInterchangeMaterialSearchLocation SearchLocation = EInterchangeMaterialSearchLocation::Local;

	/**
	 * When enabled, a new material asset will be created for each material
	 * in the imported source that has not been matched to an existing material.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta = (EditCondition = "bImportMaterials"))
	bool bCreateNewMaterials = true;

	/** Determines which kind of material asset to create for all new materials. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta=(DisplayName="New Material Asset Type", EditCondition = "bImportMaterials && bCreateNewMaterials"))
	EInterchangeMaterialImportOption MaterialImport = EInterchangeMaterialImportOption::ImportAsMaterials;

	/** If enabled, material instances are created from a specified parent material. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta=(DisplayName = "Create MaterialInstance From Parent", EditCondition="bImportMaterials && bCreateNewMaterials && MaterialImport==EInterchangeMaterialImportOption::ImportAsMaterialInstances"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta= (EditCondition="bImportMaterials && bCreateNewMaterials && MaterialImport==EInterchangeMaterialImportOption::ImportAsMaterialInstances && bCreateMaterialInstanceForParent", AllowedClasses="/Script/Engine.MaterialInterface"))
	FSoftObjectPath ParentMaterial;

	/**
	 * If enabled and duplicated materials are detected, for each set of duplicated materials,
	 * a reference material along with respective material instances are created.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta = (EditCondition = "bImportMaterials"))
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Instanced, Category = "Volumes")
	 ...
	 * Note: Some MaterialX Surface may have their preferred displacement center specified.
	 *       In that case, enabling this option may affect the rendering result. 
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta = (DisplayName = "Override Displacement Center", EditCondition = "bImportMaterials"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Materials", Meta = (EditCondition = "bOverrideDisplacement && bImportMaterials", ClampMin = "0", DisplayName = "Displacement Center"))
...
private:
    ...
    UE_API void HandleLuminanceNode(const UInterchangeShaderNode* ShaderNode, UInterchangeBaseMaterialFactoryNode* MaterialFactoryNode, UInterchangeMaterialExpressionFactoryNode* LuminanceFactoryNode);
    UE_API void HandleTexturePropertyNode(const UInterchangeShaderNode* ShaderNode, UInterchangeBaseMaterialFactoryNode* MaterialFactoryNode, UInterchangeMaterialExpressionFactoryNode* TexturePropertyFactoryNode);
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	void UpdateSearchLocationProperty()
	{
		// Only process DoNotSearch case since bReuseExistingMaterials is set to true by default
