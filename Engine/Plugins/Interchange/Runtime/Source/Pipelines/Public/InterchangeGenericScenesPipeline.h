...
protected:
	 ...
	 */
	UE_API virtual UInterchangeActorFactoryNode* CreateActorFactoryNode(const UInterchangeBaseNode* TranslatedAssetNode) const;

	UE_DEPRECATED(5.8, "Use CreateActorFactoryNode(const UInterchangeBaseNode* TranslatedAssetNode) instead")
	...
	{
		return CreateActorFactoryNode(TranslatedAssetNode);
	}
	 ...
	 */
	UE_API virtual void SetUpFactoryNode(UInterchangeActorFactoryNode* ActorFactoryNode, const UInterchangeSceneNode* SceneNode, const UInterchangeBaseNode* TranslatedAssetNode, bool bIsRootJoint = false) const;
...
protected:
	...
	TSet<FString> Cached_ActiveJointUids;

	FPreviewScene* PreviewScene = nullptr;

private:
	void CreateActorFactoryNode(const FTransform& GlobalOffsetTransform, const UInterchangeSceneNode* SceneNode, const FString& SkeletalMeshFactoryNodeUid);
};
