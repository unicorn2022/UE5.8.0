...
public:
	 ...
	 */
 	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Common Meshes", meta = (SubCategory = "Conversion"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Common Meshes", meta = (SubCategory = "Conversion", editcondition = "ForceAllMeshAsType == EInterchangeForceMeshType::IFMT_None"))
	bool bConvertStaticsWithAnimatedTransformToSkeletals = true;
	...
	*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Common Meshes", meta = (SubCategory = "Conversion", editcondition = "ForceAllMeshAsType == EInterchangeForceMeshType::IFMT_SkeletalMesh"))
	...
	UE_DEPRECATED(5.8, "Use bConvertStaticsWithAnimatedTransformToSkeletals instead")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "bAutoDetectMeshType is deprecated. Use bConvertStaticsWithAnimatedTransformToSkeletals instead."))
	bool bAutoDetectMeshType_DEPRECATED = true;
	...
	virtual bool IsPropertyChangeNeedRefresh(const FPropertyChangedEvent& PropertyChangedEvent) const override
	{
		static const TSet<FName> NeedRefreshProperties =
		{
			...
			GET_MEMBER_NAME_CHECKED(UInterchangeGenericCommonMeshesProperties, bConvertStaticsWithAnimatedTransformToSkeletals),
			GET_MEMBER_NAME_CHECKED(UInterchangeGenericCommonMeshesProperties, bConvertStaticsWithMorphTargetsToSkeletals),
			GET_MEMBER_NAME_CHECKED(UInterchangeGenericCommonMeshesProperties, bConvertStaticsInBoneHierarchyToSkeletals),
...
public:
	...
	/** If enabled, the pipeline will try to find a skeleton asset at the import location if available and set the skeleton property to use that asset. A new skeleton will be created if no skeleton asset is found. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Common Skeletal Meshes and Animations")
	bool bTryAutoSelectSkeleton = false;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Common Skeletal Meshes and Animations", meta = (EditCondition = "!bTryAutoSelectSkeleton"))
	...
	UE_DEPRECATED(5.8, "Use UInterchangeGenericCommonMeshesProperties::bConvertStaticsInBoneHierarchyToSkeletals instead")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "bImportMeshesInBoneHierarchy is deprecated. Use UInterchangeGenericCommonMeshesProperties::bConvertStaticsInBoneHierarchyToSkeletals instead."))
	bool bImportMeshesInBoneHierarchy_DEPRECATED = true;

	UE_DEPRECATED(5.8, "Use UInterchangeGenericCommonMeshesProperties::bConvertStaticsWithMorphTargetsToSkeletals instead")
	UPROPERTY(meta = (DeprecatedProperty, DeprecationMessage = "bConvertStaticsWithMorphTargetsToSkeletals is deprecated. Use UInterchangeGenericCommonMeshesProperties::bConvertStaticsWithMorphTargetsToSkeletals instead."))
	bool bConvertStaticsWithMorphTargetsToSkeletals_DEPRECATED = false;
	...
	virtual bool IsPropertyChangeNeedRefresh(const FPropertyChangedEvent& PropertyChangedEvent) const override
	{
		static const TSet<FName> NeedRefreshProperties =
		{
			...
			GET_MEMBER_NAME_CHECKED(UInterchangeGenericCommonSkeletalMeshesAndAnimationsProperties, bTryAutoSelectSkeleton),
		};
	...
	{
		if (bImportOnlyAnimations && !IsSkeletonValid())
		{
	...
	UE_API UInterchangeSkeletonFactoryNode* CreateSkeletonFactoryNode(UInterchangeBaseNodeContainer* BaseNodeContainer, const FString& RootJointUid, const FString& SourceFileName/*Used for Warning generation*/);

	UE_API bool IsSkeletonValid() const;
};
