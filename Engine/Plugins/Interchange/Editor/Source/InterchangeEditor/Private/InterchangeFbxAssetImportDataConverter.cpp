...
namespace UE::Interchange::Private
{
	...
	void FillFbxStaticMeshImportData(const UInterchangeGenericAssetsPipeline* GenericAssetPipeline, UFbxStaticMeshImportData* DestinationStaticMeshImportData)
	{
		...
		DestinationStaticMeshImportData->bCombineMeshes = GenericAssetPipeline->MeshPipeline->CombineStaticMeshesBehavior == EInterchangeCombineStaticMeshesBehavior::All;
	...
	{
		DestinationSkeletalMeshImportData->bImportMeshesInBoneHierarchy = GenericAssetPipeline->CommonMeshesProperties->bConvertStaticsInBoneHierarchyToSkeletals;
	...
	void FillFbxAnimSequenceImportData(const UInterchangeGenericAssetsPipeline* GenericAssetPipeline, UFbxAnimSequenceImportData* DestinationAnimSequenceImportData)
	{
		...
		DestinationAnimSequenceImportData->bImportMeshesInBoneHierarchy = GenericAssetPipeline->CommonMeshesProperties->bConvertStaticsInBoneHierarchyToSkeletals;
		...
		DestinationAnimSequenceImportData->bSnapToClosestFrameBoundary = GenericAssetPipeline->AnimationPipeline->FrameAlignment == EInterchangeFrameAlignment::SnapToClosest ? true : false;
		...
		, bool bFillBaseClass = true)
	{
		...
		GenericAssetPipeline->MeshPipeline->CombineStaticMeshesBehavior = StaticMeshImportData->bCombineMeshes ? EInterchangeCombineStaticMeshesBehavior::All : EInterchangeCombineStaticMeshesBehavior::DoNotCombine;
		...
		, bool bFillBaseClass = true)
	{
		...
		GenericAssetPipeline->CommonMeshesProperties->bConvertStaticsInBoneHierarchyToSkeletals = SkeletalMeshImportData->bImportMeshesInBoneHierarchy;
		...
		, const UFbxAnimSequenceImportData* AnimSequenceImportData)
	{
		...
		GenericAssetPipeline->CommonMeshesProperties->bConvertStaticsInBoneHierarchyToSkeletals = AnimSequenceImportData->bImportMeshesInBoneHierarchy;
		...
		GenericAssetPipeline->AnimationPipeline->FrameAlignment = AnimSequenceImportData->bSnapToClosestFrameBoundary ? EInterchangeFrameAlignment::SnapToClosest : EInterchangeFrameAlignment::None;
	...
	UFbxImportUI* ConvertToLegacyFbx(UObject* Owner, const UInterchangeAssetImportData* InterchangeSourceData)
	{
		...
		if (!GenericAssetPipeline->MaterialPipeline->bReuseExistingMaterials)
		{
		...
		if (GenericAssetPipeline->CommonSkeletalMeshesAndAnimationsProperties->IsSkeletonValid())
		{
			DestinationData->Skeleton = GenericAssetPipeline->CommonSkeletalMeshesAndAnimationsProperties->Skeleton.LoadSynchronous();
		}
	...
	UAssetImportData* ConvertToInterchange(UObject* Owner, const UFbxImportUI* FbxImportUI)
	{
		...
		GenericAssetPipeline->MaterialPipeline->bReuseExistingMaterials = true;
		switch (FbxImportUI->TextureImportData->MaterialSearchLocation)
		{
			...
			case EMaterialSearchLocation::DoNotSearch:
				{
					GenericAssetPipeline->MaterialPipeline->bReuseExistingMaterials = false;
				}
				break;
			default:
				ensure(false);
...
bool UInterchangeFbxAssetImportDataConverter::ConvertImportData(const UObject* SourceImportData, const UClass* DestinationClass, UObject** DestinationImportData) const
{
	...
	{
		// UE-359435: Make sure UFbxImportUI is created using Transient package to prevent Editor-only dependencies.
		...
		{
			UObject* DestinationPtr = nullptr;

			if (DestinationClass->IsChildOf<UFbxImportUI>())
			{
				DestinationPtr = FbxImportUI;
			}
			else
			{
				if (DestinationClass->IsChildOf<UFbxStaticMeshImportData>())
				{
					DestinationPtr = FbxImportUI->StaticMeshImportData;
				}
				else if (DestinationClass->IsChildOf<UFbxSkeletalMeshImportData>())
				{
					DestinationPtr = FbxImportUI->SkeletalMeshImportData;
				}
				else if (DestinationClass->IsChildOf<UFbxAnimSequenceImportData>())
				{
					DestinationPtr = FbxImportUI->AnimSequenceImportData;
				}

				if (DestinationPtr)
				{
					//Don't forget to set proper outer for supported classes.
					DestinationPtr->Rename(nullptr, SourceImportData->GetOuter(), REN_DontCreateRedirectors | REN_NonTransactional);
				}
			}

			if (DestinationPtr)
			{
				(*DestinationImportData) = DestinationPtr;
				bResult = true;
			}
			else
			{
