...
TSharedRef<SBox> SInterchangeGraphInspectorWindow::SpawnGraphInspector()
{
	...
	// TODO: @vjavdekar - hidden as this doesn't currently provide any value and existing open modal windows block the property matrix window from opening.
	// Reconsider this when the Preview Panel gets a UI Pass.
...
void SInterchangeGraphInspectorWindow::Construct(const FArguments& InArgs)
{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(SInterchangeGraphInspectorWindow::SortingNodes)

		InterchangeBaseNodeContainer->SortNodes(
			[](const UInterchangeBaseNode& LHS, const UInterchangeBaseNode& RHS)
			{
				const bool bLeftIsFactory = LHS.IsA<UInterchangeFactoryBaseNode>();
				const bool bRightIsFactory = RHS.IsA<UInterchangeFactoryBaseNode>();
				const bool bLeftIsScene = LHS.IsA<UInterchangeSceneNode>();
				const bool bRightIsScene = RHS.IsA<UInterchangeSceneNode>();

				// Always put scene nodes above other nodes
				if (bLeftIsScene != bRightIsScene)
				{
					return bLeftIsScene > bRightIsScene;
				}
				// Then prefer other nodes above factory nodes
				else if (bLeftIsFactory != bRightIsFactory)
				{
					return bLeftIsFactory < bRightIsFactory;
				}
				else
				{
					return LHS.GetDisplayLabel() < RHS.GetDisplayLabel();
				}
			}
		);
	}
