...
void FInterchangeBaseNodeDetailsCustomization::CustomizeDetails( IDetailLayoutBuilder& DetailBuilder )
{
	...
	//Add a row to describe the node type and class
	{
		...
		FText NodeTypeValueText = FText::GetEmpty();
		FText NodeClassValueText = FText::GetEmpty();
		FText ObjectClassNameValueText = FText::GetEmpty();

		NodeClassValueText = FText::FromString(InterchangeBaseNode->GetClass()->GetName());

		if (UInterchangeFactoryBaseNode* FactoryNode = Cast<UInterchangeFactoryBaseNode>(InterchangeBaseNode))
		{
			const FString ClassName = FactoryNode->GetObjectClass() 
										? FactoryNode->GetObjectClass()->GetName() 
										: "None - Unprocessed";

			NodeTypeValueText = LOCTEXT("NodeFactoryInformation_FactoryNodeValueText", "Factory Node");
			ObjectClassNameValueText = FText::FromString(ClassName);
		}
		else
		{
			NodeTypeValueText = LOCTEXT("NodeFactoryInformation_TranslatedNodeValueText", "Translated Node");
		}

		auto AddNodeInformationRow = [&AttributeCategoryBuilder, &NodeInformationCategoryText](const FText& Name, const FText& Value)
			{
				AttributeCategoryBuilder.AddCustomRow(NodeInformationCategoryText)
					.FilterString(Name)
					.NameContent()
					[
						SNew(STextBlock)
		...
		const FText NodeTypeNameText = LOCTEXT("NodeFactoryInformationNodeTypeNameText", "Node Type");
		AddNodeInformationRow(NodeTypeNameText, NodeTypeValueText);

		const FText NodeClassNameText = LOCTEXT("NodeFactoryInformationNodeClassNameText", "Node Class");
		AddNodeInformationRow(NodeClassNameText, NodeClassValueText);

		if (!ObjectClassNameValueText.IsEmpty())
		{
			const FText ObjectClassNameNameText = LOCTEXT("NodeFactoryInformationObjectClassNameNameText", "Object Class Name");
			AddNodeInformationRow(ObjectClassNameNameText, ObjectClassNameValueText);
		}
	}
...
void FInterchangeBaseNodeDetailsCustomization::AddAttributeRow(UE::Interchange::FAttributeKey& AttributeKey, IDetailCategoryBuilder& AttributeCategory)
{
	...
	switch (AttributeType)
	{
		...
		case UE::Interchange::EAttributeTypes::InterchangeLODMeshData:
		{
			BuildStringInterchangeMeshLODDataValueContent(AttributeCategory, AttributeKey);
		}
		break;

		case UE::Interchange::EAttributeTypes::InterchangeLODMeshDataArray:
		{
			BuildStringInterchangeMeshLODDataArrayValueContent(AttributeCategory, AttributeKey);
		}
		break;
...
void FInterchangeBaseNodeDetailsCustomization::BuildStringInterchangeMeshLODDataArrayValueContent(IDetailCategoryBuilder& AttributeCategory, UE::Interchange::FAttributeKey& AttributeKey)
{
	UE::Interchange::EAttributeTypes AttributeType = InterchangeBaseNode->GetAttributeType(AttributeKey);

	const UE::Interchange::FAttributeStorage::TAttributeHandle<TArray<FInterchangeLODMeshData>> AttributeHandle = InterchangeBaseNode->GetAttributeHandle<TArray<FInterchangeLODMeshData>>(AttributeKey);
	if (!AttributeHandle.IsValid())
	{
		CreateInvalidHandleRow(AttributeCategory, AttributeKey);
		return;
	}
	TArray<FInterchangeLODMeshData> LODMeshDataArrayValue;
	if (AttributeHandle.IsValid())
	{
		AttributeHandle.Get(LODMeshDataArrayValue);
	}

	const FString KeyDisplayName = InterchangeBaseNode->GetKeyDisplayName(AttributeKey);

	for (int32 LODMeshCounter = 0; LODMeshCounter < LODMeshDataArrayValue.Num(); LODMeshCounter++)
	{
		const FText AttributeNameMeshUid = FText::FromString(KeyDisplayName + TEXT("[") + FString::FromInt(LODMeshCounter) + TEXT("]") + TEXT(".MeshUid"));
		const FText AttributeTooltipMeshUid = FText::FromString(TEXT("Attribute Type: ") + UE::Interchange::AttributeTypeToString(AttributeType) + TEXT(" (MeshUid)"));

		const FText AttributeNameTransform = FText::FromString(KeyDisplayName + TEXT("[") + FString::FromInt(LODMeshCounter) + TEXT("]") + TEXT(".Transform"));
		const FText AttributeTooltipTransform = FText::FromString(TEXT("Attribute Type: ") + UE::Interchange::AttributeTypeToString(AttributeType) + TEXT(" (Transform)"));

		//MeshUid
		{
			FDetailWidgetRow& CustomRow = AttributeCategory.AddCustomRow(AttributeNameMeshUid)
				.NameContent()
				[
					SNew(STextBlock)
						.Text(AttributeNameMeshUid)
						.Font(IDetailLayoutBuilder::GetDetailFont())
						.ToolTipText(AttributeTooltipMeshUid)
				]
				.ValueContent()
				[
					SNew(SHorizontalBox)
						...
						[
							SNew(STextBlock)
								.Text(FText::FromString(LODMeshDataArrayValue[LODMeshCounter].MeshUid))
						]
				];
		}
		...
		//Transform
		{
			auto AddNumericWidget = [](double X, double Y, double Z, double W = 0, bool bUseW = false) -> TSharedRef<SHorizontalBox>
				{
			...
			const bool bAdvancedProperty = false;
			IDetailGroup& Group = AttributeCategory.AddGroup(FName(*AttributeNameTransform.ToString()), AttributeNameTransform, bAdvancedProperty);
			FDetailWidgetRow& GroupHeaderRow = Group.HeaderRow();
			GroupHeaderRow.NameContent().Widget = SNew(SBox)
				[
					SNew(STextBlock)
						.Text(AttributeNameTransform)
						.Font(IDetailLayoutBuilder::GetDetailFont())
						.ToolTipText(AttributeTooltipTransform)
				];

			FQuat4d Rotation = LODMeshDataArrayValue[LODMeshCounter].Transform.GetRotation();
			FVector3d Translation = LODMeshDataArrayValue[LODMeshCounter].Transform.GetTranslation();
			FVector3d Scale3D = LODMeshDataArrayValue[LODMeshCounter].Transform.GetScale3D();
			...
			Group.AddWidgetRow()
				...
				.ValueContent()
				[
					AddNumericWidget(Translation.X, Translation.Y, Translation.Z)
				];
			...
			Group.AddWidgetRow()
				...
				.ValueContent()
				[
					AddNumericWidget(Rotation.X, Rotation.Y, Rotation.Z, Rotation.W, true)
				];
			...
			Group.AddWidgetRow()
				...
				.ValueContent()
				[
					AddNumericWidget(Scale3D.X, Scale3D.Y, Scale3D.Z)
				];
		}
	}
}

void FInterchangeBaseNodeDetailsCustomization::BuildStringInterchangeMeshLODDataValueContent(IDetailCategoryBuilder& AttributeCategory, UE::Interchange::FAttributeKey& AttributeKey)
{
	UE::Interchange::EAttributeTypes AttributeType = InterchangeBaseNode->GetAttributeType(AttributeKey);

	{
		const UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = InterchangeBaseNode->GetAttributeHandle<FInterchangeLODMeshData>(AttributeKey);
		if (!AttributeHandle.IsValid())
		{
			CreateInvalidHandleRow(AttributeCategory, AttributeKey);
			return;
	...
	const FText AttributeNameMeshUid = FText::FromString(KeyDisplayName + (TEXT(".MeshUid")));
	const FText AttributeNameSceneNodeName = FText::FromString(KeyDisplayName + (TEXT(".SceneNodeName")));
	const FText AttributeNameTransform = FText::FromString(KeyDisplayName + (TEXT(".Transform")));
	const FText AttributeNameGeometricTransform = FText::FromString(KeyDisplayName + (TEXT(".GeometricTransform")));

	const FString AttributeTooltipStringMeshUid = TEXT("Attribute Type: ") + UE::Interchange::AttributeTypeToString(AttributeType) + TEXT(" (MeshUid)");
	const FString AttributeTooltipStringSceneNodeName = TEXT("Attribute Type: ") + UE::Interchange::AttributeTypeToString(AttributeType) + TEXT(" (SceneNodeName)");
	const FString AttributeTooltipStringTransform = TEXT("Attribute Type: ") + UE::Interchange::AttributeTypeToString(AttributeType) + TEXT(" (Transform)");
	const FString AttributeTooltipStringGeometricTransform = TEXT("Attribute Type: ") + UE::Interchange::AttributeTypeToString(AttributeType) + TEXT(" (GeometricTransform)");

	const FText AttributeTooltipMeshUid = FText::FromString(AttributeTooltipStringMeshUid);
	const FText AttributeTooltipSceneNodeName = FText::FromString(AttributeTooltipStringSceneNodeName);
	const FText AttributeTooltipTransform = FText::FromString(AttributeTooltipStringTransform);
	const FText AttributeTooltipGeometricTransform = FText::FromString(AttributeTooltipStringGeometricTransform);

	//MeshUid
	{
		const FText AttributeName = FText::FromString(TEXT("MeshUid"));
		FDetailWidgetRow& CustomRow = AttributeCategory.AddCustomRow(AttributeName)
			.NameContent()
			[
				SNew(STextBlock)
							...
							.Text_Lambda([this, AttributeKey]()->FText
								{
									UE::Interchange::EAttributeTypes AttributeType = InterchangeBaseNode->GetAttributeType(AttributeKey);
									FText ReturnText;
									if (AttributeType == UE::Interchange::EAttributeTypes::InterchangeLODMeshData)
									{
										UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = InterchangeBaseNode->GetAttributeHandle<FInterchangeLODMeshData>(AttributeKey);
										FInterchangeLODMeshData Value;
										if (AttributeHandle.IsValid())
										{
											AttributeHandle.Get(Value);
										}
										ReturnText = FText::FromString(Value.MeshUid);
									}
									return ReturnText;
								})
							.Font(IDetailLayoutBuilder::GetDetailFont())
	...
	//SceneNodeName
	{
		const FText AttributeName = FText::FromString(TEXT("SceneNodeName"));
		FDetailWidgetRow& CustomRow = AttributeCategory.AddCustomRow(AttributeName)
			.NameContent()
			[
				SNew(STextBlock)
					.Text(AttributeNameSceneNodeName)
					.Font(IDetailLayoutBuilder::GetDetailFont())
					.ToolTipText(AttributeTooltipSceneNodeName)
			]
			.ValueContent()
			[
				SNew(SHorizontalBox)
							...
							.Text_Lambda([this, AttributeKey]()->FText
								{
									...
									if (AttributeType == UE::Interchange::EAttributeTypes::InterchangeLODMeshData)
									{
										...
										ReturnText = FText::FromString(Value.SceneNodeName);
									}
									return ReturnText;
								})
							.Font(IDetailLayoutBuilder::GetDetailFont())
	...
	{
		const bool bAdvancedProperty = false;
		const FString GroupName = InterchangeBaseNode->GetKeyDisplayName(AttributeKey) + TEXT(".Transform");
		IDetailGroup& Group = AttributeCategory.AddGroup(FName(*GroupName), FText::FromString(GroupName), bAdvancedProperty);
		FDetailWidgetRow& GroupHeaderRow = Group.HeaderRow();
		GroupHeaderRow.NameContent().Widget = SNew(SBox)
			[
		...
		auto GetRotationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key)->FQuat4d
			{
				FInterchangeLODMeshData NodeUidTransformsPair;
				const UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = BaseNode->GetAttributeHandle<FInterchangeLODMeshData>(Key);
				if (AttributeHandle.IsValid())
				{
					AttributeHandle.Get(NodeUidTransformsPair);
				}
				return NodeUidTransformsPair.Transform.GetRotation();
			};

		auto SetRotationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key, const FQuat4d& Value)
			{
				FInterchangeLODMeshData NodeUidTransformsPair;
				UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = BaseNode->GetAttributeHandle<FInterchangeLODMeshData>(Key);
				if (AttributeHandle.IsValid())
				{
					AttributeHandle.Get(NodeUidTransformsPair);
					NodeUidTransformsPair.Transform.SetRotation(Value);
					AttributeHandle.Set(NodeUidTransformsPair);
				}
			};

		auto GetTranslationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key)->FVector3d
			{
				FInterchangeLODMeshData NodeUidTransformsPair;
				const UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = BaseNode->GetAttributeHandle<FInterchangeLODMeshData>(Key);
				if (AttributeHandle.IsValid())
				...
				return NodeUidTransformsPair.Transform.GetTranslation();
			};

		auto SetTranslationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key, const FVector3d& Value)
			{
				FInterchangeLODMeshData NodeUidTransformsPair;
				UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = BaseNode->GetAttributeHandle<FInterchangeLODMeshData>(Key);
				if (AttributeHandle.IsValid())
				{
					AttributeHandle.Get(NodeUidTransformsPair);
					NodeUidTransformsPair.Transform.SetTranslation(Value);
					AttributeHandle.Set(NodeUidTransformsPair);
				}
			};

		auto GetScale3DValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key)->FVector3d
			{
				FInterchangeLODMeshData NodeUidTransformsPair;
				const UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = BaseNode->GetAttributeHandle<FInterchangeLODMeshData>(Key);
				if (AttributeHandle.IsValid())
				...
				return NodeUidTransformsPair.Transform.GetScale3D();
			};

		auto SetScale3DValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key, const FVector3d& Value)
			{
				FInterchangeLODMeshData NodeUidTransformsPair;
				UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = BaseNode->GetAttributeHandle<FInterchangeLODMeshData>(Key);
				if (AttributeHandle.IsValid())
				{
					AttributeHandle.Get(NodeUidTransformsPair);
					NodeUidTransformsPair.Transform.SetScale3D(Value);
					AttributeHandle.Set(NodeUidTransformsPair);
				}
			};
		...
		Group.AddWidgetRow()
			...
			.ValueContent()
			[
				CreateVectorWidget<FVector3d, double>(GetTranslationValue, SetTranslationValue, AttributeKey)
			];
		...
		Group.AddWidgetRow()
			...
			.ValueContent()
			[
				CreateQuaternionWidget<FQuat4d, double>(GetRotationValue, SetRotationValue, AttributeKey)
			];
		...
		Group.AddWidgetRow()
			...
			.ValueContent()
			[
				CreateVectorWidget<FVector3d, double>(GetScale3DValue, SetScale3DValue, AttributeKey)
			];
	}
	...
	{
		const bool bAdvancedProperty = false;
		const FString GroupName = InterchangeBaseNode->GetKeyDisplayName(AttributeKey) + TEXT(".GeometricTransform");
		IDetailGroup& Group = AttributeCategory.AddGroup(FName(*GroupName), FText::FromString(GroupName), bAdvancedProperty);
		FDetailWidgetRow& GroupHeaderRow = Group.HeaderRow();
		GroupHeaderRow.NameContent().Widget = SNew(SBox)
			[
				SNew(STextBlock)
					.Text(AttributeNameGeometricTransform)
					.Font(IDetailLayoutBuilder::GetDetailFont())
					.ToolTipText(AttributeTooltipGeometricTransform)
			];

		auto GetRotationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key)->FQuat4d
			{
				FInterchangeLODMeshData LODMeshData;
				const UE::Interchange::FAttributeStorage::TAttributeHandle<FInterchangeLODMeshData> AttributeHandle = BaseNode->GetAttributeHandle<FInterchangeLODMeshData>(Key);
				if (AttributeHandle.IsValid())
				{
					AttributeHandle.Get(LODMeshData);
				}
				return LODMeshData.GeometricTransform.GetRotation();
			};
		...
		auto SetRotationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key, const FQuat4d& Value)
			{
				...
				{
					AttributeHandle.Get(LODMeshData);
					LODMeshData.GeometricTransform.SetRotation(Value);
					AttributeHandle.Set(LODMeshData);
				}
			};
		...
		auto GetTranslationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key)->FVector3d
			{
				...
				return LODMeshData.GeometricTransform.GetTranslation();
			};
		...
		auto SetTranslationValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key, const FVector3d& Value)
			{
				...
				{
					AttributeHandle.Get(LODMeshData);
					LODMeshData.GeometricTransform.SetTranslation(Value);
					AttributeHandle.Set(LODMeshData);
				}
			};
		...
		auto GetScale3DValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key)->FVector3d
			{
				...
				return LODMeshData.GeometricTransform.GetScale3D();
			};
		...
		auto SetScale3DValue = [](UInterchangeBaseNode* BaseNode, UE::Interchange::FAttributeKey& Key, const FVector3d& Value)
			{
				...
				{
					AttributeHandle.Get(LODMeshData);
					LODMeshData.GeometricTransform.SetScale3D(Value);
					AttributeHandle.Set(LODMeshData);
				}
			};
