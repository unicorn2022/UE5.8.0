...
void FInterchangeUsdTranslatorSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailLayoutBuilder)
{
	...
	if (TSharedPtr<IPropertyHandle> CustomHandlerProperty = DetailLayoutBuilder.GetProperty(
			GET_MEMBER_NAME_CHECKED(UInterchangeUsdTranslatorSettings, CustomHandlerEntries)
		))
	{
		DetailLayoutBuilder.HideProperty(CustomHandlerProperty);

		UInterchangeUsdTranslatorSettings* Settings = CurrentOptions;

		CatBuilder.AddCustomRow(FText::FromString(TEXT("ConfigureHandlersButton")))
		.WholeRowContent()
		[
			SNew(SBox)
			...
			[
				SNew(SButton)
				.HAlign(HAlign_Center)
				.Text(LOCTEXT("EditSchemaHandlerButton", "Configure schema handlers"))
				.OnClicked_Lambda([Settings, CustomHandlerProperty]()
				{
					FScopedTransaction Transaction(LOCTEXT("CustomHandlerTransaction", "Edit Custom Handlers"));
					CustomHandlerProperty->NotifyPreChange();

					bool bNeedNewTranslation = SInterchangeUsdHandlerOrderWindow::ShowWindow(Settings);

					if (bNeedNewTranslation)
					{
						// These events are important as they're the thing that eventually causes
						// SInterchangeTranslatorSettingsDialog to trigger OnTranslatorSettingsDialogClosed, and
						// ultimately cause the code in SInterchangePipelineConfigurationDialog::OnEditTranslatorSettings()
						// to trigger a new translation of the source file with the new translator (and schema handler)
						// settings when bTranslatorSettingsChanged is true
						CustomHandlerProperty->NotifyPostChange(EPropertyChangeType::ValueSet);
						CustomHandlerProperty->NotifyFinishedChangingProperties();
					}
