		...
		const FOpenVDBFileInfo* GetOrLoadFileInfo(const FString& Filename)
		{
			...
			// we don't need to use read locks everywhere
			{
				...
				if (!Info->bFailed && Info->FileBytes.Num() == 0)
				{
					...
					{
						UE_LOGF(LogInterchangeOpenVDBImport, Error, "OpenVDB file could not be found: %ls", *Filename);
						Info->bFailed = true;
						return nullptr;
					}
					...
					{
						UE_LOGF(LogInterchangeOpenVDBImport, Error, "OpenVDB file could not be loaded: %ls", *Filename);
						Info->bFailed = true;
						return nullptr;
					}
					...
					{
						UE_LOGF(LogInterchangeOpenVDBImport, Error, "Failed to read OpenVDB file: %ls", *Filename);
						Info->bFailed = true;
						return nullptr;
					}
				}
