...
void AAudioGameplayVolume::AddProxy() const
{
	if (AGVComponent)
	{
		AGVComponent->OnProxyEnter.AddUniqueDynamic(const_cast<AAudioGameplayVolume*>(this), &AAudioGameplayVolume::OnListenerEnter);
		AGVComponent->OnProxyExit.AddUniqueDynamic(const_cast<AAudioGameplayVolume*>(this), &AAudioGameplayVolume::OnListenerExit);
