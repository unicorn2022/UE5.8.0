...
namespace AudioGameplayVolumeUtils
{
	bool NeedsPhysicsQuery(UPrimitiveComponent* PrimitiveComponent, const FVector& Position)
	{
		...
		return PrimitiveComponent->Bounds.GetBox().IsInsideOrOn(Position);
	}
...
void UAudioGameplayVolumeProxy::InitFromComponent(const UAudioGameplayVolumeComponent* Component)
{
	...
	{
		UE_LOGF(AudioGameplayVolumeLog, Verbose, "AudioGameplayVolumeProxy - Attempted Init from invalid volume component!");
		return;
	}
	
#if !UE_BUILD_SHIPPING
	AGVOwnerKey = Component->GetOwner();
#endif
...
void UAudioGameplayVolumeProxy::CollectMutators(ProxyMutatorList& OutMutators) const
{
	check(IsInAudioThread());
	OutMutators.Append(ProxyVolumeMutators);
}
...
#if !UE_BUILD_SHIPPING
FName UAudioGameplayVolumeProxy::GetOwnerName() const
{
	return GetFNameSafe(AGVOwnerKey.ResolveObjectPtr());
}
#endif
