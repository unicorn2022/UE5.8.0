...
namespace UnrealBuildTool
{
	class UEDeployVisionOS : UEDeployIOS
	{
		...
		public static bool GenerateVisionOSPList(string ProjectDirectory, bool bIsUnrealGame, string GameName, bool bIsClient, string ProjectName, string InEngineDir, string AppDirectory, UnrealPluginLanguage? UPL, string? BundleID)
		{
			// @todo visionos: THIS!
			...
			// @todo Handle visionOS minimum supported version properly.
			...
			// @todo Handle visionOS additional plist data properly.
			...
			Text.AppendLine($"\t<string>{SanitizedBundleIdentifier}</string>");
		...
		{
			return GenerateVisionOSPList(ProjectDirectory, bIsUnrealGame, GameName, bIsClient, ProjectName, InEngineDir, AppDirectory, null, BundleID);
		}
	}
}
