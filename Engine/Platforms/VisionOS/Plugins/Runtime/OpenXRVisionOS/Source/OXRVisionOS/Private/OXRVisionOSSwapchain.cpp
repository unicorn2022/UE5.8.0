...
FOXRVisionOSSwapchain::~FOXRVisionOSSwapchain()
{
	...
	{
		UE_LOGF(LogOXRVisionOS, Warning, "Destructing FOXRVisionOSSwapchain because create failed.");
	}
...
const FOXRVisionOSSwapchain::FSwapchainImage& FOXRVisionOSSwapchain::GetLastWaitedImage() const
{
	...
	UE_LOGF(LogOXRVisionOS, VeryVerbose, "FOXRVisionOSSwapchain::GetLastWaitedImage() NextImageToWait=%i Metal texture = 0x%x", NextImageToWait, WaitedImage.Image.GetReference()->GetNativeResource());
...
const FOXRVisionOSSwapchain::FSwapchainImage& FOXRVisionOSSwapchain::GetLastReleasedImage() const
{
	...
	UE_LOGF(LogOXRVisionOS, VeryVerbose, "FOXRVisionOSSwapchain::GetLastReleasedImage() NextImageToRelease=%i Metal texture = 0x%x", NextImageToRelease, ReleasedImage.Image.GetReference()->GetNativeResource());
	...
	uint32_t*									index)
{
	...
	if (NextImage.ImageState == EImageState::Released)
	{
		...
		*index = NextImageToAcquire;
		...
		UE_LOGF(LogOXRVisionOS, VeryVerbose, "FOXRVisionOSSwapchain::XrAcquireSwapchainImage() Acquired %i metal texture 0x%x", NextImageToAcquire, NextImage.Image.GetReference()->GetNativeResource());
	...
	const XrSwapchainImageWaitInfo*				waitInfo)
{
	...
	{
		UE_LOGF(LogOXRVisionOS, VeryVerbose, "FOXRVisionOSSwapchain::XrWaitSwapchainImage() Waiting on NextImageToWait %i ", NextImageToWait);
		...
		UE_LOGF(LogOXRVisionOS, VeryVerbose, "FOXRVisionOSSwapchain::XrWaitSwapchainImage() WaitComplete on NextImageToWait %i  metal texture 0x%x", NextImageToWait, NextImage.Image.GetReference()->GetNativeResource());
		return XrResult::XR_SUCCESS;
	}
	else
	{
	...
	const XrSwapchainImageReleaseInfo*			releaseInfo)
{
	...
	{
		UE_LOGF(LogOXRVisionOS, VeryVerbose, "FOXRVisionOSSwapchain::XrReleaseSwapchainImage() Releasing %i metal texture 0x%x", NextImageToRelease, NextImage.Image.GetReference()->GetNativeResource());
