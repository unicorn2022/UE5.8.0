...
XrResult FOXRVisionOSInstance::Create(TSharedPtr<FOXRVisionOSInstance, ESPMode::ThreadSafe>& OutInstance, const XrInstanceCreateInfo* CreateInfo, FOXRVisionOS* InModule)
{
	...
	for (int i = 0; i < CreateInfo->enabledExtensionCount; i++)
	{
		...
		{
			UE_LOGF(LogOXRVisionOS, Error, "FOXRVisionOSInstance::Create required extension: %s but it is not supported.  Instance creation failed.", ExtensionName);
...
FOXRVisionOSInstance::~FOXRVisionOSInstance()
{
	...
	if (bCreateFailed)
	{
		UE_LOGF(LogOXRVisionOS, Warning, "Destructing FOXRVisionOSInstance because instance create failed.");
	}
	else
	{
	...
	const XrInteractionProfileSuggestedBinding* SuggestedBindings)
{
	...
	UE_LOGF(LogHMD, Verbose, "XrSuggestInteractionProfileBindings profile: %ls", ANSI_TO_TCHAR(PathToString(SuggestedBindings->interactionProfile)));
	...
	if (InteractionProfilePairs == nullptr)
	{
		...
		UE_LOGF(LogOXRVisionOS, Verbose, "  Ignoring %ls because OXRVisionOS does not support it", ANSI_TO_TCHAR(PathToString(SuggestedBindings->interactionProfile)));
		return XrResult::XR_SUCCESS;
	}
	...
	for (uint32_t i = 0; i < SuggestedBindingCount; i++)
	{
		...
		if (Source == nullptr)
		{
			UE_LOGF(LogOXRVisionOS, Warning, "Unsupported path %ls in profile %ls", ANSI_TO_TCHAR(PathToString(Binding.binding)), ANSI_TO_TCHAR(PathToString(SuggestedBindings->interactionProfile)));
		...
		else
		{
			...
			{
				UE_LOGF(LogOXRVisionOS, Log, "TopLevelPath %ls set to InteractionProfile %ls", ANSI_TO_TCHAR(PathToString(TopLevelPath)), ANSI_TO_TCHAR(PathToString(SuggestedBindings->interactionProfile)));
