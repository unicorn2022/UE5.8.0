...
FXRSwapChainPtr CreateSwapchain_OXRVisionOS(XrSession InSession, uint8 Format, uint8& OutActualFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, const FClearValueBinding& ClearValueBinding)
{
	if (Dimension != ETextureDimension::Texture2D)
	{
		UE_LOGF(LogHMD, Error, "Unsupported texture dimension for VisionOS");
		ensure(false);
		return nullptr;
	}
	...
	XrSwapchain Swapchain = FOpenXRSwapchain::CreateSwapchain(InSession, Format, SizeX, SizeY, ArraySize, NumMips, NumSamples, Dimension, CreateFlags);
	if (!Swapchain)
	{
		return nullptr;
	}
