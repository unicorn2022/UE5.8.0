...
public:
	...
	virtual FXRSwapChainPtr CreateSwapchain(XrSession InSession, uint8 Format, uint8& OutFormat, uint32 SizeX, uint32 SizeY, uint32 ArraySize, uint32 NumMips, uint32 NumSamples, ETextureDimension Dimension, ETextureCreateFlags CreateFlags, const FClearValueBinding& ClearValueBinding, ETextureCreateFlags AuxiliaryCreateFlags) override final
	{
		return CreateSwapchain_OXRVisionOS(InSession, Format, OutFormat, SizeX, SizeY, ArraySize, NumMips, NumSamples, Dimension, CreateFlags, ClearValueBinding);
	}
