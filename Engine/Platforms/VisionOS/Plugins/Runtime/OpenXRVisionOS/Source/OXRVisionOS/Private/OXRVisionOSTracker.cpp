...
//	bGazeEnabled(bEnableGaze)
{
	...
	//if (Ret != OK)
	//{
	//	UE_LOGF(LogOXRVisionOSTracker, Warning, "Initialize failed: 0x%08X", Ret);
	//	bCreateFailed = true;
	//	return;
	//}
}
...
FOXRVisionOSTracker::~FOXRVisionOSTracker()
{
	// if (Result != OK)
	// {
	// 	UE_LOGF(LogOXRVisionOSTracker, Warning, "Term failed: 0x%08X", Result);
	// }
}
...
void FOXRVisionOSTracker::SyncActions(int32 FrameCounter, XrTime PredictedDisplayTime)
{
	...
	// 		if (Result.Failed)
	// 		{
	// 			UE_LOGF(LogOXRVisionOSTracker, Warning, "SyncActions GazeGetResult failed");
	// 		}
	// 	}
	// }
}
...
void FOXRVisionOSTracker::TryRegisterDevice(TrackerDeviceType DeviceType, FDeviceInfo& Info)
{
	...
	// if (Result != RESULT_OK)
	// {
	// 	UE_LOGF(LogOXRVisionOSTracker, Warning, "RegisterDevice failed code 0x%x. \n", Result);
	// }
	// else
	// {
	// 	Info.bIsActive = true;
...
bool FOXRVisionOSTracker::GetResult(int32 DeviceHandle, XrTime DisplayTime, OXRVisionOS::TrackerResultData& ResultData)
{
	...
	if (bFound == false)
	{
		UE_LOGF(LogOXRVisionOSTracker, Warning, "FOXRVisionOSTracker::GetResult called with unrecognized DeviceHandle %i", DeviceHandle);
		return false;
	}
...
// 	if (Result != RESULT_OK)
// 	{
// 		UE_LOGF(LogOXRVisionOSTracker, Warning, "GetResult failed: %d (%u, %08X)", Result, Result, Result);
// 		return false;
// 	}
...
// 	{
// 		UE_LOGF(LogOXRVisionOSTracker, Warning, "FOXRVisionOSTracker::GetGazeResult called without gaze tracking enabled");
// 		return false;
// 	}
...
bool FOXRVisionOSTracker::UpdateReferenceSpaces(class FOXRVisionOSInstance* Instance, FOXRVisionOSSession* Session, int32 DeviceHandle, XrTime DisplayTime, int32 FrameNumber)
{
	...
	// UE_LOGF(LogOXRVisionOSTracker, Log, "FOXRVisionOSTracker::UpdateReferenceSpaces update happening. %i", ResultData.playAreaUpdateCounter);
	// PlayAreaData.PlayAreaUpdateCounter = ResultData.playAreaUpdateCounter;

	// if (PlayAreaData.PlayAreaMode != ResultData.playAreaMode)
	// {
	// 	UE_LOGF(LogOXRVisionOSTracker, Log, "FOXRVisionOSTracker::UpdateReferenceSpaces playAreaMode changing from %i to %i", PlayAreaData.PlayAreaMode, ResultData.playAreaMode);
