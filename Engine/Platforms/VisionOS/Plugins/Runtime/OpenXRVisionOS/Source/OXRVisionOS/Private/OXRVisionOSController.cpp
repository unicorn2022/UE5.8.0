...
{
	UE_LOGF(LogOXRVisionOS, Warning, "VrControllerAPI::ControllerOpen() : %d", static_cast<int32>(ControllerHand));
...
bool FOXRVisionOSController::GetActionStateBoolean(EOXRVisionOSControllerButton Button, bool& OutValue, bool &OutActive, XrTime& OutTimeStamp)
{
	...
	case EOXRVisionOSControllerButton::PushR:
		...
		break;
	default:
		UE_LOGF(LogOXRVisionOS, Error, "GetActionStateBoolean unknown button %ls", OXRVisionOS::OXRVisionOSControllerButtonToTCHAR(Button));
		OutActive = false;
