...
FSubstrateSubsurfaceHeader SubstrateLoadSubsurfaceHeader(inout FSubstrateAddressing SubstrateAddressing)
{
	...
	else if (GBufferData.ShadingModelID == SHADINGMODELID_SUBSURFACE_PROFILE ||
		GBufferData.ShadingModelID == SHADINGMODELID_EYE)
	{
	...
	{
		Mode = SSS_TYPE_TRANSMITTANCE_SHADOW;
		...
		SubstrateSubSurfaceHeaderSetTransmittanceShadowMFPInMeters(Out, GetToonProfileShadowSettings(ToonProfileId).ShadowMFPInMeters.xxx);
	}
...
#endif
{
	...
	#endif
	{
		...
		{
			const float3 DiffuseColor = ComputeDiffuseAlbedo(TOON_BASECOLOR(BSDF), TOON_METALLIC(BSDF));
			const float3 SpecularColor = ComputeF0(TOON_SPECULAR(BSDF), TOON_BASECOLOR(BSDF), TOON_METALLIC(BSDF));

			Data.Closures[i].DiffuseR		= DiffuseColor.r;
			Data.Closures[i].DiffuseG		= DiffuseColor.g;
			Data.Closures[i].DiffuseB		= DiffuseColor.b;
			Data.Closures[i].F0R			= SpecularColor.r;
			Data.Closures[i].F0G			= SpecularColor.g;
			Data.Closures[i].F0B			= SpecularColor.b;
			Data.Closures[i].Roughness		= TOON_ROUGHNESS(BSDF);
			Data.Closures[i].SSSPRofileID	= TOON_PROFILEID(BSDF);

			Data.Closures[i].bHasAnisotropy = BSDF_GETHASANISOTROPY(BSDF);
			if (Data.Closures[i].bHasAnisotropy)
			{
				Data.Closures[i].Anisotropy = TOON_ANISOTROPY(BSDF);
			}
			
			Data.Closures[i].GlintUVDDXx	= TOON_PATTERNUVS(BSDF).x;
			Data.Closures[i].GlintUVDDXy	= TOON_PATTERNUVS(BSDF).y;

			if (Data.Closures[i].SSSType == SSS_TYPE_TRANSMITTANCE_SHADOW)
			{
			#if SUBSTRATE_MATERIALCONTAINER_IS_VIEWRESOURCE
				const float ShadowMFPInMeters = GetToonProfileShadowSettings(TOON_PROFILEID(BSDF)).ShadowMFPInMeters;
				Data.Closures[i].SSSMFPR = ShadowMFPInMeters;
				Data.Closures[i].SSSMFPG = ShadowMFPInMeters;
				Data.Closures[i].SSSMFPB = ShadowMFPInMeters;
			#endif
			}
		}
		break;
