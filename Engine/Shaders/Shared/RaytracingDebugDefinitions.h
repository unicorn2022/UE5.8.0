...
#define RAY_TRACING_DEBUG_VIZ_CLUSTERS							27
...
INLINE_ATTR bool RequiresRayTracingDebugCHS(UINT_TYPE DebugVisualizationMode)
{
	return DebugVisualizationMode == RAY_TRACING_DEBUG_VIZ_INSTANCES
		...
		|| DebugVisualizationMode == RAY_TRACING_DEBUG_VIZ_CLUSTERS
...
INLINE_ATTR bool RayTracingDebugModeSupportsInline(UINT_TYPE DebugVisualizationMode)
{
	...
	const bool bIsHitKind = DebugVisualizationMode == RAY_TRACING_DEBUG_VIZ_HITKIND;

	return bIsBarycentricsMode || bIsWorldNormalMode || bIsPickingMode || bUsesDebugCHSAndNotHitCount || bIsTraversalMode || bIsHitKind;
}
