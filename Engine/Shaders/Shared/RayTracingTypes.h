...
struct FPathTracingLight
{
	...
	float3  CameraColorScale; // Tinting factor for camera rays only
	uint    Padding;
};
HLSL_STATIC_ASSERT(sizeof(FPathTracingLight) == 128, "Path tracing light structure should be kept as small as possible");
...
struct FPathTracingDebugPackedPathState
{
	uint      RandSeqSampleIndex;
	uint      RandSeqSampleSeed;
	float3    Radiance;
	float     Alpha;
	uint3     PackedDiffSpecColor;
	uint2     PackedNormalRoughness;
	float3    RayOrigin;
	float3    RayDirection;
	float3    PathThroughput;
	uint2     PackedRoughnessSigma;
...
HLSL_STATIC_ASSERT(sizeof(FPathTracingDebugPackedPathState) == 96, "Packed Path State size should be minimized");
...
using FPathTracingDebugPackedPathState = UE::HLSL::FPathTracingDebugPackedPathState;
