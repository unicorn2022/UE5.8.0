...
namespace UE { 
	namespace Jni {
		namespace PsoServices {
			...
			{
				{"NativeSetThreadPriority", "(J)V"          , (void*)FPsoProgramService::NativeSetThreadPriority},
				{"InitVKDevice"           , "()V"           , (void*)FPsoProgramService::InitVKDevice},
				{"ShutdownVKDevice"       , "()V"           , (void*)FPsoProgramService::ShutdownVKDevice},
				{"CompileVKGFXPSO"        , "([B[B[B[B[F)[B", (void*)FPsoProgramService::CompileVKGFXPSO},
				{"CompileVKGFXPSOSHM"     , "(IJJJJ[F)I"    , (void*)FPsoProgramService::CompileVKGFXPSOSHM}
			};
		}
	}
}
...
JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	jint result = env->RegisterNatives(clazz, PsoServices::NativeMethods, sizeof(PsoServices::NativeMethods) / sizeof(JNINativeMethod));
...
#define COPY_FROM_BUFFER(Dst, Src, Offset, Size) \
	...
	std::string CompileGFXPSO(const uint8_t* VS, uint64_t VSSize, const uint8_t* PS, uint64_t PSSize, const uint8_t* PSO, uint64_t PSOSize, const uint8_t* PSOCacheDataSource, uint64_t PSOCacheDataSourceSize)
	{
		...
		std::vector<VkPipelineShaderStageCreateInfo> ShaderStages(PipelineCreateInfo.StageCount);
		...
		for (int32_t Idx = 0; Idx < PipelineCreateInfo.StageCount; ++Idx)
		{
			...
			bool bHasSpecializationInfo;
			COPY_FROM_BUFFER(&bHasSpecializationInfo, PSO, MemoryOffset, sizeof(bHasSpecializationInfo));
			if (bHasSpecializationInfo)
			{
				ShaderStages[Idx].pSpecializationInfo = (VkSpecializationInfo*)&PSO[MemoryOffset];
				MemoryOffset += sizeof(VkSpecializationInfo);

				((VkSpecializationInfo*)ShaderStages[Idx].pSpecializationInfo)->pMapEntries = (VkSpecializationMapEntry*)&PSO[MemoryOffset];
				MemoryOffset += ShaderStages[Idx].pSpecializationInfo->mapEntryCount * sizeof(VkSpecializationMapEntry);

				((VkSpecializationInfo*)ShaderStages[Idx].pSpecializationInfo)->pData = (void*)&PSO[MemoryOffset];
				MemoryOffset += ShaderStages[Idx].pSpecializationInfo->dataSize;
			}
		}
		...
		if (bUseRenderPass2)
		{
			...
			if (RenderPassCreateInfo.attachmentCount > 0)
			{
				...
				for (uint32_t AttachmentIndex = 0; AttachmentIndex < RenderPassCreateInfo.attachmentCount; ++AttachmentIndex)
				{
					...
					{
						const_cast<VkAttachmentDescription2*>(&RenderPassCreateInfo.pAttachments[AttachmentIndex])->pNext = &PSO[MemoryOffset];
						MemoryOffset += sizeof(VkAttachmentDescriptionStencilLayout);
					}
				}
			}
...
jbyteArray JNICALL UE::Jni::PsoServices::FPsoProgramService::CompileVKGFXPSO(JNIEnv* jenv, jobject thiz, jbyteArray jVS, jbyteArray jPS, jbyteArray jPSO, jbyteArray jPSOCacheDataSource, jfloatArray jCompilationDuration)
{
	...
	{
		jenv->ReleaseByteArrayElements(jPSOCacheDataSource, (int8_t*)PSOCacheDataSource, JNI_ABORT);
	}
	if (PSO)
	{
		jenv->ReleaseByteArrayElements(jPSO, (int8_t*)PSO, JNI_ABORT);
	}
	if (PS)
	{
		jenv->ReleaseByteArrayElements(jPS, (int8_t*)PS, JNI_ABORT);
	}
	if (VS)
	{
		jenv->ReleaseByteArrayElements(jVS, (int8_t*)VS, JNI_ABORT);
	}
