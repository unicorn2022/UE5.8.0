...
# for dotnet applications that require older dotnet runtimes, this will allow them to use our bundled dotnet runtime
...
if [ $IS_DOTNET_INSTALLED -eq 0 ]; then
	...
	UE_DOTNET_DIR=$CUR_DIR/../../../Binaries/ThirdParty/DotNet/10.0/mac-$ARCH
	if [ $(which realpath) ]; then
		UE_DOTNET_DIR=$(realpath "$UE_DOTNET_DIR")
	fi
