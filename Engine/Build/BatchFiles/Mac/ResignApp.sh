...
echo "iOS App Resigning Tool: Version 2"
...
DoWork()
{
	...
	if [[ ! -z "$MARKETING_VERSION" ]]; then
		...
		/usr/libexec/PlistBuddy -c "Set:CFBundleShortVersionString $MARKETING_VERSION" "$TARGET/Info.plist"
	fi
	...
	if [[ ! -z "$DEVELOPER" ]]; then
		...
		while IFS='' read -r line || [[ -n "$line" ]]; do
			...
			if (( ${#FRAMEWORKS_TO_REMOVE[@]} > 0 )); then
				if [[ "$line" == *".app" ]]; then
					EXECUTABLE=$(/usr/libexec/PlistBuddy -c "Print:CFBundleExecutable" "$line/Info.plist")
					if [[ -z "$EXECUTABLE" ]]; then
						echo "Error: Could not read CFBundleExecutable from Info.plist."
						exit 1
					fi
					
					DYLIBS=$(otool -L "$line/$EXECUTABLE" 2>/dev/null)
					if [[ -z "$DYLIBS" ]]; then
						echo "Error: otool produced no output. File may not be a Mach-O binary."
						exit 1
					fi
					
					OTOOL_OUTPUT=("${(f)DYLIBS}")
					for OTOOL_LINE in "${OTOOL_OUTPUT[@]}"; do
						for FRAMEWORK in "${FRAMEWORKS_TO_REMOVE[@]}"
						do
							if [[ "$OTOOL_LINE" == *"$FRAMEWORK"* && "$OTOOL_LINE" == *" weak"* ]]; then
								FRAMEWORK="${OTOOL_LINE%% *}"
								FRAMEWORK="${FRAMEWORK##[[:space:]]}"
								FRAMEWORK="${FRAMEWORK%%[[:space:]]}"
								REPLACEMENT_FRAMEWORK="/${FRAMEWORK_COUNTER:#1}"
								echo "Replacing weak framework "${OTOOL_LINE##[[:space:]]}" with $REPLACEMENT_FRAMEWORK"
								install_name_tool -change "$FRAMEWORK" "$REPLACEMENT_FRAMEWORK" "$TARGET/$EXECUTABLE" 2>/dev/null
								((FRAMEWORK_COUNTER++))
							fi
						done
					done
			...
			if [[ -f "$line/embedded.mobileprovision" ]]; then
				# find the correct mobileprovision file
				for profile in "${(@k)EXT_PROFILES}"; do
					if [[ ! -z "$BUNDLE" ]] && [[ "$line" == *".appex"* ]] && [[ "$line" == *"$profile"* ]]; then
						PROFILE_PATH="${EXT_PROFILES[$profile]}"
						echo "Copying appex $PROFILE_PATH profile into $line.."
						cp "$PROFILE_PATH" "$line/embedded.mobileprovision"
					fi
				done
			...
			echo ""
			echo "Codesigning [/usr/bin/codesign --sign \"$DEVELOPER\" --continue --force $EXTRAOPTIONS \"$line\"]"
			/usr/bin/codesign --sign "$DEVELOPER" --continue --force $EXTRAOPTIONS "$line"
...
Help()
{
	...
	echo "     Path to the .mobileprovision file for the app extension to sign"
	echo "  -px <ExtBundleName>=<path to .mobileprovision>"
	echo "     Path to the .mobileprovision file for the app extension to sign to specified extension name under Build/IOS/Extensions"
	echo "     Can be repeated for each extension within the app"
	echo "       ie: '-px NotifService=/Users/auser/notifservice.mobileprovision'"
	echo "  -b | --bundleid"
    ...
    echo "  -rm | --removeframework <Framework Name>"
    echo "     Remove the specified weak framework from the app bundle and from the executable load commands."
    echo "     Must be used with signing. Can be used multiple times."
	echo "  -h | --help"
...
while [[ $# -gt 0 ]]; do
  case $1 in
	...
	-pe|--provisionappex)
	  MOBILEAPPEXPROV="$2"
	  echo "App extension param '-pe' is deprecated. Please use '-px'"
	  shift # past argument
	  shift # past value
	  ;;
	-px)
	  EXT_TO_PROFILE="$2"
	  if [[ $EXT_TO_PROFILE != *=* ]]; then
		  echo "Invalid extension mobileprovision param. Aborting..."
		  exit 1
	  fi
	  key=${EXT_TO_PROFILE%%=*}
	  val=${EXT_TO_PROFILE#*=}
	  EXT_PROFILES[$key]=$val
	  printf 'key=[%s] val=[%s]\n' "${key}" "${val}"
	  shift # past argument
	  shift # past value
	  ;;
