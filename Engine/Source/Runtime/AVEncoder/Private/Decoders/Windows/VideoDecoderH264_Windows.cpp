...
bool FF5PlayerVideoDecoderOutputDX::PreInitForSoftwareDecode(FIntPoint OutputDim, ID3D11Texture2D* TextureBuffer)
{
	...
	if (bNeedNew)
	{
		...
		if (TextureBuffer)
		{
			...
			{
				UE_LOGF(LogVideoDecoder, Error, "Failed to create decoder surface buffer from texture");
				return false;
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogVideoDecoder, Error, "Failed to create a decoder memory buffer");
				return false;
			}
		}
		...
		if (MFCreateSample(MFSample.GetInitReference()) == S_OK)
		{
			...
			{
				UE_LOGF(LogVideoDecoder, Error, "Failed to add buffer to decoder output media sample");
				return false;
			}
		}
		else
		{
			UE_LOGF(LogVideoDecoder, Error, "Failed to create a decoder media sample");
			return false;
		}
	}
	return true;
...
bool FF5PlayerVideoDecoderOutputDX::InitializeWithSharedTexture(const TRefCountPtr<ID3D11Device>& InD3D11Device, const TRefCountPtr<ID3D11DeviceContext> InDeviceContext, const TRefCountPtr<IMFSample>& InMFSample, const FIntPoint& OutputDim)
{
	...
	if (bNeedsNew)
	{
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "ID3D11Device::CreateTexture2D() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
	...
	if (SharedTexture)
	{
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFSample::GetBufferByIndex() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFMediaBuffer::QueryInterface(IMFDXGIBuffer) failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFDXGIBuffer::GetResource() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		uint32 ViewIndex = 0;
		if (FAILED(Result = DXGIBuffer->GetSubresourceIndex(&ViewIndex)))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFDXGIBuffer::GetSubresourceIndex() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
	...
	if (GlobalInitCount++ == 0)
	{
		...
		if (FWindowsPlatformMisc::VerifyWindowsVersion(6, 2))
		{
			...
			if (bOk)
			{
				...
				{
					UE_LOGF(LogVideoDecoder, Error, "MFStartup() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
				}
			}
			else
			{
				UE_LOGF(LogVideoDecoder, Error, "Not all required DLLs were loaded");
			}
		}
		else
		{
			UE_LOGF(LogVideoDecoder, Error, "Windows versions less than Windows 8 are not supported");
		}
	}
...
bool FVideoDecoderH264_WindowsImpl::CreateD3DResources()
{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			if (AppResult == 0)
			{
				...
				if (ApplicationD3DDeviceVersion >= 11000 && ApplicationD3DDeviceVersion <= 12999)
				{
					...
					{
						UE_LOGF(LogVideoDecoder, Error, "No application D3D device returned by video decoder allocation interface!");
					}
				}
				else
				{
					UE_LOGF(LogVideoDecoder, Error, "Unsupported D3D version returned (%d) which is neither DX11 or DX12.", ApplicationD3DDeviceVersion);
				}
			}
			else
			{
				UE_LOGF(LogVideoDecoder, Error, "Application video decoder allocation interface returned %d for GetD3DDevice()", AppResult);
			}
		}
		else
		{
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			UE_LOGF(LogVideoDecoder, Error, "Incorrect video decoder allocation interface is set. Is %08x, should be %08x", AllocationMethods->MagicCookie, 0x57696e58);
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
	}
	else
	{
		UE_LOGF(LogVideoDecoder, Error, "No video decoder allocation interface is set");
	}
	...
	if (SUCCEEDED(Result))
	{
		...
		if (ApplicationD3DDeviceVersion / 1000 == 11)
		{
			...
			{
				UE_LOGF(LogVideoDecoder, Error, "ID3D11Device::GetAdapter() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			}
		}
		// D3D12
		else
		{
			...
			if (SUCCEEDED(Result))
			{
				...
				{
					UE_LOGF(LogVideoDecoder, Error, "EnumAdapterByLuid() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
				}
			}
			else
			{
				UE_LOGF(LogVideoDecoder, Error, "CreateDXGIFactory() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			}
		}
			...
			if (SUCCEEDED(Result))
			{
				if (FeatureLevel >= D3D_FEATURE_LEVEL_9_3)
				{
					...
					if (SUCCEEDED(Result))
					{
						...
						UE_LOGF(LogVideoDecoder, Display, "Created D3D11 device for H.264 decoding on %ls.", *FString(AdapterDesc.Description));

						return true;
					}
					else
					{
						UE_LOGF(LogVideoDecoder, Error, "ResetDevice() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
					}
				}
				else
				{
					DXDeviceInfo.DxVersion = FDXDeviceInfo::ED3DVersion::VersionUnknown;
					UE_LOGF(LogVideoDecoder, Error, "Unable to Create D3D11 Device with feature level 9.3 or above");
				}
			}
			else
			{
				UE_LOGF(LogVideoDecoder, Error, "D3D11CreateDevice() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			}
		}
	}
	else
	{
		UE_LOGF(LogVideoDecoder, Error, "MFCreateDXGIDeviceManager() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
	}
...
bool FVideoDecoderH264_WindowsImpl::FallbackToSwDecoding(FString Reason)
{
	...
	UE_LOGF(LogVideoDecoder, Display, "FallbackToSwDecoding: %ls", *Reason);
	...
	if (IsWindows8Plus())
	{
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "Failed to set video decoder into software mode (main D3D device multithread protect). No application D3D device set");
			return false;
		}
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "Failed to set video decoder into software mode (main D3D device multithread protect) with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
	}
#endif
...
bool FVideoDecoderH264_WindowsImpl::ReconfigureForSwDecoding(FString Reason)
{
	...
	if (DecoderTransform.GetReference())
	{
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "Failed to set video decoder into software mode (unset D3D manager) with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		}
	}
	return false;
}
...
bool FVideoDecoderH264_WindowsImpl::InternalDecoderCreate()
{
	...
	if (Electra::IsWindows8Plus())
	{
		...
		if (Result != S_OK)
		{
			UE_LOGF(LogVideoDecoder, Error, "D3D device loss detected. Reason 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "CoCreateInstance() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	Result = Decoder->GetAttributes(Attributes.GetInitReference());
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "GetAttributes() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
...
bool FVideoDecoderH264_WindowsImpl::StartStreaming()
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "MFT_MESSAGE_NOTIFY_BEGIN_STREAMING failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	Result = DecoderTransform->ProcessMessage(MFT_MESSAGE_NOTIFY_START_OF_STREAM, 0);
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "MFT_MESSAGE_NOTIFY_START_OF_STREAM failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	return true;
}
...
bool FVideoDecoderH264_WindowsImpl::DecoderSetInputType()
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "MFCreateMediaType() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "Failed to set input media type with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "Failed to set video decoder input media type resolution of %d*%d with 0x%08x (%ls)", configW, configH, Result, *GetComErrorDescription(Result));
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "SetInputType() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
...
bool FVideoDecoderH264_WindowsImpl::DecoderSetOutputType()
{
	...
	for(int32 TypeIndex=0; ; ++TypeIndex)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "GetOutputAvailableType() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "Failed to get video decoder available output media (sub-)type with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		if (OutputMediaMajorType == MFMediaType_Video && OutputMediaSubtype == MFVideoFormat_NV12)
		{
			...
			{
				UE_LOGF(LogVideoDecoder, Error, "SetOutputType() failed  with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
				return false;
			}
		}
	}
...
bool FVideoDecoderH264_WindowsImpl::DecoderVerifyStatus()
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "GetStreamCount() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	if (NumInputStreams != 1 || NumOutputStreams != 1)
	{
		UE_LOGF(LogVideoDecoder, Error, "Unexpected number of streams: input %d, output %d", (int32)NumInputStreams, (int32)NumOutputStreams);
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "GetInputStatus() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	if (MFT_INPUT_STATUS_ACCEPT_DATA != DecoderStatus)
	{
		UE_LOGF(LogVideoDecoder, Error, "Decoder doesn't accept data, status %d", (int32)DecoderStatus);
		return false;
	}
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "GetOutputStreamInfo() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	...
	if (!bIsHardwareAccelerated && (DecoderOutputStreamInfo.dwFlags & MFT_OUTPUT_STREAM_PROVIDES_SAMPLES))
	{
		UE_LOGF(LogVideoDecoder, Error, "Incompatible H.264 decoder: s/w decoder is expected to require preallocated output samples");
		return false;
	}
	return true;
}
...
bool FVideoDecoderH264_WindowsImpl::CreateDecoderInstance()
{
	...
	if (InternalDecoderCreate())
	{
		...
		if (bRequiresReconfigurationForSW)
		{
			...
			if (bRequiresReconfigurationForSW)
			{
				UE_LOGF(LogVideoDecoder, Error, "Failed to switch H.264 decoder transform to software decoding");
				bOk = false;
			}
		}
		...
		switch(DXDeviceInfo.DxVersion)
		{
			default:
			{
				UE_LOGF(LogVideoDecoder, Error, "Unsupported version of DirectX");
				DecodeMode = EDecodeMode::Undefined;
				bOk = false;
				break;
			}
			...
			{
				UE_LOGF(LogVideoDecoder, Error, "DirectX9 is not supported!");
				DecodeMode = EDecodeMode::Undefined;
				bOk = false;
				break;
			}
...
bool FVideoDecoderH264_WindowsImpl::AllocateApplicationOutputSampleBuffer(int32 Width, int32 Height)
{
	...
	{
		UE_LOGF(LogVideoDecoder, Error, "Unsupported decoding mode");
		return false;
	}
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		UE_LOGF(LogVideoDecoder, Error, "Unsupported allocation return value");
		return false;
	}
	else
	{
	...
	if (DecoderTransform.GetReference() && InInput)
	{
		...
		while(!bTerminateWorkerThread && InputSample.IsValid())
		{
			...
			else
			{
				if (FAILED(Result))
				{
					UE_LOGF(LogVideoDecoder, Error, "Failed to process video decoder output with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
				}
				return false;
			}
		}
...
bool FVideoDecoderH264_WindowsImpl::CopyTexture(const TRefCountPtr<IMFSample>& DecodedOutputSample, FF5PlayerVideoDecoderOutputDX* DecoderOutput, FIntPoint OutputDim, FIntPoint NativeDim)
{
	...
	if (FAILED(Result))
	{
		UE_LOGF(LogVideoDecoder, Error, "IMFSample::GetBufferCount() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
		return false;
	}
	else if (BuffersNum != 1)
	{
		UE_LOGF(LogVideoDecoder, Error, "IMFSample::GetBufferCount() returned %d buffers instead of 1", (int32)BuffersNum);
		return false;
	}
	...
	if (DecodeMode == EDecodeMode::Win8HW)
	{
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "Failed to get a decode output buffer from the application!");
			return false;
		}
	...
	else if (DecodeMode == EDecodeMode::Win10SW)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFMediaBuffer::GetCurrentLength() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFMediaBuffer::Lock() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFMediaBuffer::Unlock() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
	...
	else if (DecodeMode == EDecodeMode::Win10HW)
	{
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFDXGIBuffer::QueryInterface(ID3D11Texture2D) failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		D3D11_TEXTURE2D_DESC TextureDesc;
		...
		{
			UE_LOGF(LogVideoDecoder, Error, "FVideoDecoderH264_WindowsImpl::CopyTexture(): Decoded texture is not in NV12 format");
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFDXGIBuffer::QueryInterface(IMF2DBuffer) failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFMediaBuffer::Lock2D() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogVideoDecoder, Error, "IMFMediaBuffer::Unlock2D() failed with 0x%08x (%ls)", Result, *GetComErrorDescription(Result));
			return false;
		}
	...
	{
		UE_LOGF(LogVideoDecoder, Error, "FVideoDecoderH264::CopyTexture(): Unhandled D3D version");
		return false;
	}
#endif
