...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
{
	...
	if (CodecType == ECodecType::AAC)
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			UE_LOGF(LogAVEncoder, Error, "%ls", *ErrorStr);
			return false;
		}
	}
	else
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_LOGF(LogAVEncoder, Log, "AudioEncoder config: %d channels, %d Hz, %.2f Kbps", InConfig.NumChannels, InConfig.Samplerate, InConfig.Bitrate / 1000.0f);
...
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_LOGF(LogAVEncoder, Verbose, "Audio input: time %.3f, duration %.3f, %d samples", Frame.Timestamp.GetTotalSeconds(), Frame.Duration.GetTotalSeconds(), Frame.Data.GetNumSamples());
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	while (true)
	{
		...
		UE_LOGF(LogAVEncoder, Verbose, "Audio encoded: time %.3f, duration %.3f, size %d", OutputSample.GetTime().GetTotalSeconds(), OutputSample.GetDuration().GetTotalSeconds(), OutputSize);
...
TUniquePtr<FAudioEncoder> FWmfAudioEncoderFactory::CreateEncoder(const FString& Codec)
{
	...
	{
		UE_LOGF(LogAVEncoder, Error, "FWmfAudioEncoderFactory doesn't support the %ls codec", *Codec);
		return nullptr;
	}
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS
