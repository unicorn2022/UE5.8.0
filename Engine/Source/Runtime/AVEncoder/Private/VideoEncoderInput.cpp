...
namespace AVEncoder
{
	...
	{
		{
			...
			{
				UE_LOGF(LogVideoEncoder, Error, "There are still %d active input frames.", ActiveFrames.Num());
			}
	...
	bool FVideoEncoderInputImpl::SetupForD3D11(void* InApplicationD3DDevice)
	{
		...
		if (Result != S_OK)
		{
			UE_LOGF(LogVideoEncoder, Error, "ID3D11Device::QueryInterface() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
			return false;
		}
		else if ((Result = DXGIDevice->GetAdapter(Adapter.GetInitReference())) != S_OK)
		{
			UE_LOGF(LogVideoEncoder, Error, "DXGIDevice::GetAdapter() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		{
			UE_LOGF(LogVideoEncoder, Error, "D3D11CreateDevice() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
			return false;
		}
	...
	bool FVideoEncoderInputImpl::SetupForD3D12(void* InApplicationD3DDevice)
	{
		...
		{
			UE_LOGF(LogVideoEncoder, Error, "CreateDXGIFactory() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
			return false;
		}
		...
		{
			UE_LOGF(LogVideoEncoder, Error, "DXGIFactory::EnumAdapterByLuid() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
			return false;
		}
	...
	bool FVideoEncoderInputImpl::SetupForD3D12Shared(void* InApplicationD3DDevice)
	{
		...
		{
			UE_LOGF(LogVideoEncoder, Error, "D3D11CreateDevice() - failed to create device w/ feature level 11.1 - needed to encode textures from D3D12.");
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		else
		{
			...
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			UE_LOGF(LogVideoEncoder, Verbose, "Created new frame total frames: %d", NumBuffers);
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
	...
	FVideoEncoderInputFrameImpl* FVideoEncoderInputImpl::CreateFrame()
	{
		...
		case EVideoFrameFormat::Undefined:
			UE_LOGF(LogVideoEncoder, Error, "Got undefined frame format!");
			break;
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		if (NumRemoved > 0)
		{
			...
			if (InFrame->GetFormat() != FrameFormat)
			{
				...
				NumBuffers--;
				UE_LOGF(LogVideoEncoder, Verbose, "Deleted buffer (format mismatch) total remaining: %d", NumBuffers);
				return;
			}
			...
			if (!AvailableFrames.IsEmpty() && NumBuffers > MaxNumBuffers)
			{
				ProtectFrames.Unlock();
				NumBuffers--;
				UE_LOGF(LogVideoEncoder, Verbose, "Deleted buffer (too many) total frames: %d", NumBuffers);
				return;
			}
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	FVideoEncoderInputFrame::~FVideoEncoderInputFrame()
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			if (NumRef > 2)
			{
				UE_LOGF(LogVideoEncoder, Warning, "VideoEncoderFame - D3D11 input texture still holds %d references.", NumRef);
			}
		...
		if (D3D12.EncoderTexture)
		{
			...
			if (NumRef > 2)
			{
				UE_LOGF(LogVideoEncoder, Warning, "VideoEncoderFame - D3D12 input texture still holds %d references.", NumRef);
			}
			// Need to call release twice as we have added an extra reference just above (only way to count how many references we have)
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				...
				{
					UE_LOGF(LogVideoEncoder, Error, "IDXGIResource::GetSharedHandle() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
				}
				else if (SharedHandle == nullptr)
				{
					UE_LOGF(LogVideoEncoder, Error, "IDXGIResource::GetSharedHandle() failed to return a shared texture resource no created as shared? (D3D11_RESOURCE_MISC_SHARED).");
				}
				PRAGMA_DISABLE_DEPRECATION_WARNINGS
				...
				PRAGMA_ENABLE_DEPRECATION_WARNINGS
				{
					UE_LOGF(LogVideoEncoder, Error, "ID3D11Device::OpenSharedResource() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
				}
				else
				{
					PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				UE_LOGF(LogVideoEncoder, Error, "ID3D12Device::CreateSharedHandle() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
			}
			PRAGMA_DISABLE_DEPRECATION_WARNINGS
			else if (D3D11.SharedHandle == nullptr)
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
			{
				UE_LOGF(LogVideoEncoder, Error, "ID3D12Device::CreateSharedHandle() failed to return a shared texture resource no created as shared? (D3D12_RESOURCE_FLAG_ALLOW_SIMULTANEOUS_ACCESS).");
			}
			else
			{
				...
				PRAGMA_ENABLE_DEPRECATION_WARNINGS
				{
					UE_LOGF(LogVideoEncoder, Error, "ID3D11Device::OpenSharedResource1() failed 0x%X - %ls.", Result, *GetComErrorDescription(Result));
				}
				else
				{
					PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			if (!CUDA.EncoderTexture)
			{
				UE_LOGF(LogVideoEncoder, Warning, "SetTexture | Cuda device pointer is null");
			}
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
	}
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			{
				UE_LOGF(LogVideoEncoder, Warning, "SetTexture | Vulkan VkImage is null");
			}
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
	}
	...
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	{
		...
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
		{
			...
			{
				UE_LOGF(LogVideoEncoder, Warning, "SetTexture | Vulkan VkImage or VkTextureDeviceMemory is null");
			}
			PRAGMA_ENABLE_DEPRECATION_WARNINGS
		}
	}
