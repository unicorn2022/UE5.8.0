...
class FLogSuppressionImplementation: public FLogSuppressionInterface, private FSelfRegisteringExec
{
	...
	void ProcessCmdString(const FString& CmdString, bool FromBoot = false)
	{
		...
		static FName NAME_BootGlobal("BootGlobal");
		static FName NAME_Reset("Reset");
		...
		for (int32 Index = 0; Index < SubCmds.Num(); Index++)
		{
			...
			else
			{
				...
				for (int32 PartIndex = 1; PartIndex < CommandParts.Num(); PartIndex++)
				{
					...
					static FName NAME_Verbose("Verbose");
					static FName NAME_VeryVerbose("VeryVerbose");
					static FName NAME_On("On");
					static FName NAME_Off("Off");
					static FName NAME_Break("Break");
					static FName NAME_Fatal("Fatal");
					static FName NAME_Log("Log");
					static FName NAME_Display("Display");
			...
			if (Category != NAME_Reset)
			{
				...
				if (VerbosityLevel)
				{
					...
					{
						UE_LOGF(LogHAL, Log, "Log category %ls verbosity has been raised to %ls.", *Category.ToString(), ToString(static_cast<ELogVerbosity::Type>(VerbosityLevel)));
					}
				}
			}
		}
	...
	void SetupSuppress(FLogCategoryBase* Destination, FName NameFName)
	{
		...
		else
		{
			...
			if (Boot)
			{
				Destination->DefaultVerbosity = FMath::Min<uint8>(*Boot, Destination->GetCompileTimeVerbosity());
				Destination->ResetFromDefault();
			}
		}
		// store off the last non-zero one for toggle
	...
	void AssociateSuppressImpl(FLogCategoryBase* Destination)
	{
		...
		for (TMultiMap<FName, FLogCategoryBase*>::TKeyIterator It(ReverseAssociations, NameFName); It; ++It)
		{
			if (It.Value() == Destination)
			{
				if (!IsReloadActive())
				{
					UE_LOGF(LogHAL, Fatal,"Log suppression category %ls was somehow declared twice with the same data.", *NameFName.ToString());
				}
			}
			// if it is registered, it better be the same
			if (It.Value()->CompileTimeVerbosity != Destination->CompileTimeVerbosity)
			{
				UE_LOGF(LogHAL, Fatal,"Log suppression category %ls is defined multiple times with different compile time verbosity.", *NameFName.ToString());
			}
