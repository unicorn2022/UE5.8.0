...
struct FOodleDataCompressionFormat : ICompressionFormat
{
	...
	virtual bool Compress(void* OutCompressedBuffer, int64& OutCompressedSize, const void* InUncompressedBuffer, int64 InUncompressedSize, uintptr_t InCompressionData, ECompressionFlags Flags) override
	{
		...
		checkf(CompressedBufferSize >= CheckCompressedBufferSize, TEXT("The compressed buffer size (%lld) must be larger than %lld"), CompressedBufferSize, CheckCompressedBufferSize);
		...
		if ( Flags & COMPRESS_ForPackaging )
		{
			...
			//UE_LOGF(OodleDataCompression, Display, "Oodle Compress : %d -> %d", UncompressedSize, Result);
...
ICompressionFormat * CreateOodleDataCompressionFormat()
{
	...
	if ( IsProgram || IsIOStore )
	{
		...
		{
			UE_LOGF(OodleDataCompression, Display, "Oodle v%ls format for pak/iostore with method=%ls, level=%d=%ls", TEXT(OodleVersion), **MethodMap.FindKey(UsedCompressor), (int)UsedLevel, **LevelMap.FindKey(UsedLevel) );
			...
			if ( ! OodleDLL.IsEmpty() )
			{
				UE_LOGF(OodleDataCompression, Display, "OodleCompressDLL=%ls", *OodleDLL);
				...
				{
					UE_LOGF(OodleDataCompression, Warning, "OodleCompressDLL %ls requested but could not be loaded", *OodleDLL);
				}
				else
				{
					...
					{
						UE_LOGF(OodleDataCompression, Warning, "OodleCompressDLL %ls loaded but no OodleLZ_Compress !?", *OodleDLL);
					}
