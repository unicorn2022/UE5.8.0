...
static OodleLZ_Compressor CompressorToOodleLZ_Compressor(ECompressor Compressor)
{
	...
	default:
		UE_LOGF(OodleDataCompression,Error,"Invalid Compressor: %d\n",(int)Compressor);
		return OodleLZ_Compressor_Invalid;
	}
}
...
static OodleLZ_CompressionLevel CompressionLevelToOodleLZ_CompressionLevel(ECompressionLevel Level)
{
	...
	{
		UE_LOGF(OodleDataCompression,Error,"Invalid Level: %d\n",IntLevel);
...
ECompressionCommonUsage CORE_API GetCommonUsageFromLegacyCompressionFlags(ECompressionFlags Flags)
{
	switch(Flags)
	{
		...
		default:
			UE_LOGF(OodleDataCompression,Error,"Invalid ECompressionFlags : %04X\n",Flags);
			return ECompressionCommonUsage::Default;
	}
}
...
void CORE_API GetCompressorAndLevelForCommonUsage(ECompressionCommonUsage Usage,ECompressor & OutCompressor,ECompressionLevel & OutLevel)
{
	switch(Usage)
	{
		...
		case ECompressionCommonUsage::SlowestOfflineDistributionEncode:
			...
			break;
		default:
			UE_LOGF(OodleDataCompression,Error,"Invalid ECompressionFlags : %d\n",(int)Usage);
...
struct OodleScratchBuffers
{
	...
	OodleScratchBuffers()
	{
		...
		UE_LOGF(OodleDataCompression, Verbose, "OodleScratchMemorySize=%d x %d",(int)OodleScratchMemorySize,OodleScratchBufferCount);
	}
	...
	~OodleScratchBuffers()
	{
		...
		if (OodleScratchBufferCount)
		{
			for (int i = 0; i < OodleScratchBufferCount; ++i)
			{
				...
				{
					UE_LOGF(OodleDataCompression, Error, "OodleDataCompression - shutting down while in use?");
				}
			}
	...
	int64 OodleDecode(const void * InCompBuf, int64 InCompBufSize64, void * OutRawBuf, int64 InRawLen64) 
	{
		...
		UE_LOGF(OodleDataCompression, VeryVerbose, "OodleDecode: %lld -> %lld",InCompBufSize64,InRawLen64);
		...
		if ( InCompBufSize <= 0 )
		{
			UE_LOGF(OodleDataCompression, Display, "OodleDecode: no compressed buffer size to decode!");
			return OODLELZ_FAILED;
		}
		else if ( static_cast<const OO_U8 *>(InCompBuf)[0] == 0 )
		{
			UE_LOGF(OodleDataCompression, Display, "OodleDecode: compressed buffer starts with zero byte; invalid or corrupt compressed stream.");
			return OODLELZ_FAILED;
		}
		...
		//UE_LOGF(OodleDataCompression, Display, "Decode with malloc : %d -> %d",InCompBufSize,InRawLen );
		...
		{
			UE_LOGF(OodleDataCompression, Display, "OodleDecode : no Oodle compressor found!; likely an invalid or corrupt compressed stream.");
			return OODLELZ_FAILED;
		}
		...
		{
			UE_LOGF(OodleDataCompression, Display, "OodleDecode : MemorySizeNeeded invalid; likely an invalid or corrupt compressed stream.");
			return OODLELZ_FAILED;
		}
		...
		{
			UE_LOGF(OodleDataCompression, Error, "OodleDecode - Failed to allocate %d!", (int)DecoderMemorySize);
			return OODLELZ_FAILED;
		}
								...
								int64 DictionaryBackup)
	{
		...
		{
			UE_LOGF(OodleDataCompression,Error,"OodleEncode: Compressor or Level Invalid\n");		
			return OODLELZ_FAILED;
		}
		...
		{
			UE_LOGF(OodleDataCompression,Error,"OodleEncode: OutCompressedSize too small\n");		
			return OODLELZ_FAILED;
		}
		...
		for (int i = 0; i < OodleScratchBufferCount; ++i)
		{
			if (OodleScratches[i].OodleScratchMemoryMutex.TryLock()) 
			{
				...
				UE_LOGF(OodleDataCompression, VeryVerbose, "OodleEncode: %ls (%ls): %lld -> %lld", \
...
static void OODLE_CALLBACK OodlePrintf(int verboseLevel, const char* file, int line, const char* fmt, ...)
{
	...
	UE_LOGF(OodleDataCompression, Display, "Oodle: %s", *OodleOutput);
}
							...
							int64 MinChunkSize)
{
	...
	// if CompressIndependentChunks is set we always do OODLELZ_BLOCK_LEN cuts and MinChunkSize is ignored
	if ( !CompressIndependentChunks && ChunkLen < MinChunkSize )
	{
		constexpr uint64 BlockLenMask = OODLELZ_BLOCK_LEN-1;
		ChunkLen = (MinChunkSize + BlockLenMask) & (~BlockLenMask);
	}
...
int64 CompressParallel(
							...
							int64 MinChunkSize)
{
	if ( UncompressedSize <= FMath::Max(OODLELZ_BLOCK_LEN,MinChunkSize) )
	{
	...
	int64 TotalCompLen = CompressParallelSub(Chunks,InUncompressedData,UncompressedSize,Compressor,Level,CompressIndependentChunks,MinChunkSize);
	if ( TotalCompLen <= 0 )
	{
		UE_LOGF(OodleDataCompression,Error,"CompressParallelSub failed\n");
	...
	{
		UE_LOGF(OodleDataCompression,Error,"CompressParallel buffer size insufficient\n");
		return OODLELZ_FAILED;
	}
						...
						)
{
	...
	// -> we could improve this if there's a use case for it
	...
	{
		UE_LOGF(OodleDataCompression,Display,"OodleLZ_FillSeekTable failed\n");
		return false;
	}
