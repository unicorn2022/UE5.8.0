...
namespace
{
	...
	void RunPadToNumTests(const typename ArrayType::ElementType& PaddingValue1, const typename ArrayType::ElementType& PaddingValue2)
	{
		using ElementType = typename ArrayType::ElementType;

		ArrayType Array;

		// Check padding an empty array
		{
			auto Result = Array.PadToNumDefaulted(0);
			check(Result == 0);
			check(Array.IsEmpty());
		}
		{
			auto Result = Array.PadToNum(0, PaddingValue1);
			check(Result == 0);
			check(Array.IsEmpty());
		}

		// Check padding with a value
		{
			auto Result = Array.PadToNumDefaulted(5);
			check(Result == 5);
			check(Array.Num() == 5);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{} }));
		}

		// Check underpadding
		{
			auto Result = Array.PadToNumDefaulted(4);
			check(Result == 0);
			check(Array.Num() == 5);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{} }));
		}
		{
			auto Result = Array.PadToNum(4, PaddingValue1);
			check(Result == 0);
			check(Array.Num() == 5);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{} }));
		}

		// Check adding exact padding
		{
			auto Result = Array.PadToNumDefaulted(5);
			check(Result == 0);
			check(Array.Num() == 5);
		...
		{
			auto Result = Array.PadToNum(5, PaddingValue1);
			check(Result == 0);
			check(Array.Num() == 5);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{} }));
		}

		// Check adding 1 more padding value to non-empty arrays
		{
			auto Result = Array.PadToNumDefaulted(6);
			check(Result == 1);
			check(Array.Num() == 6);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{} }));
		}
		{
			auto Result = Array.PadToNum(7, PaddingValue1);
			check(Result == 1);
			check(Array.Num() == 7);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{}, PaddingValue1 }));
		}

		// Check adding multiple padding to non-empty arrays
		{
			auto Result = Array.PadToNumDefaulted(10);
			check(Result == 3);
			check(Array.Num() == 10);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{}, PaddingValue1, ElementType{}, ElementType{}, ElementType{} }));
		}
		{
			auto Result = Array.PadToNum(14, PaddingValue2);
			check(Result == 4);
			check(Array.Num() == 14);
			check(Array == (ArrayType{ ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{}, ElementType{}, PaddingValue1, ElementType{}, ElementType{}, ElementType{}, PaddingValue2, PaddingValue2, PaddingValue2, PaddingValue2 }));
		}
	}
	...
	void RunMapConsistencyTests()
	{
...

		{
			...
			if constexpr (requires { Map.SortFreeList(); })
			{
	...
	void RunMapPerformanceTest(const FString& Description, int32 NumObjects, int32 NumOperations)
	{
		...
		for (int32 i = 0; i < NumObjects; i++)
		{
			Map.Add(KeyArray[i], FString("New Value"));
		}
...
{
	RunPadToNumTests<TArray<int32>>(5, 7);
	RunPadToNumTests<TArray<FString>>("Hello", "World");
...
{
	using namespace ContainersTSetTest;
...
{
	struct FHashTableHelper
	{
		// Given a hash table, a key and a list of values - test that the linked list of values for this key matches the passed in values
		// Also tests that the list is correctly terminated to ~0u
		// e.g.: 
		//		FHashTable Table;
		//		Table.Add(1, 1);
		//		Table.Add(1, 2);
		//		UE::Private::FHashTableHelper::TestHashList(TEXT("Description"), TestPtr, Table, 1, 1, 2);
		template<typename... Values>
		static void TestHashList(const TCHAR* TestString, FAutomationTestBase* Test, const FHashTable& InTable, uint32 First, uint32 Second, Values&&... Rest)
		{
			Test->TestEqual(TestString, InTable.First(First), Second);

			if constexpr(sizeof...(Rest) > 0)
			{
				TestHashListInner(TestString, Test, InTable, Second, Rest...);
			}
			else
			{
				// Test end of list is correctly terminated
				Test->TestEqual(TestString, InTable.Next(Second), ~0u);
			}
		}

		// Secondary test as the first value requires a call to FHashTable::First and all subsequents require FHashTable::Next
		template<typename... Values>
		static void TestHashListInner(const TCHAR* TestString, FAutomationTestBase* Test, const FHashTable& InTable, uint32 First, uint32 Second, Values&&... Rest)
		{
			Test->TestEqual(TestString, InTable.Next(First), Second);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FContainersFHashTableTest, "System.Core.Containers.FHashTable", EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::SmokeFilter)
bool FContainersFHashTableTest::RunTest(const FString& Parameters)
{
	FHashTable HashTable(1024, 16);

	// Resizing < size
	{
		// Key list for key == 1 -> [3, 10, 13, 2, 1]
		HashTable.Add(1, 1);
		HashTable.Add(1, 2);
		HashTable.Add(1, 13);
		HashTable.Add(1, 10);
		...
		UE::Private::FHashTableHelper::TestHashList(TEXT("Middle gap initial list is correct"), this, HashTable, 1, 3, 10, 13, 2, 1);
		
		// Key list for key == 1 -> [3, 2, 1]
		HashTable.Resize(8);

		UE::Private::FHashTableHelper::TestHashList(TEXT("Middle gap resized list is correct"), this, HashTable, 1, 3, 2, 1);

		HashTable.Clear(1024, 16);

		// Key list for key == 1 -> [1, 13, 12, 11, 10]
		HashTable.Add(1, 10);
		HashTable.Add(1, 11);
		HashTable.Add(1, 12);
		HashTable.Add(1, 13);
		...
		UE::Private::FHashTableHelper::TestHashList(TEXT("Open end list is correct"), this, HashTable, 1, 1, 13, 12, 11, 10);

		// Key list for key == 1 -> [1]
		HashTable.Resize(8);

		UE::Private::FHashTableHelper::TestHashList(TEXT("Open end list is correct after resize"), this, HashTable, 1, 1);

		HashTable.Clear(1024, 16);

		// Key list for key == 1 -> [13, 12, 11, 10, 1]
		HashTable.Add(1, 1);
		HashTable.Add(1, 10);
		HashTable.Add(1, 11);
		HashTable.Add(1, 12);
		...
		UE::Private::FHashTableHelper::TestHashList(TEXT("Open begin list is correct"), this, HashTable, 1, 13, 12, 11, 10, 1);

		// Key list for key == 1 -> [1]
		HashTable.Resize(8);

		UE::Private::FHashTableHelper::TestHashList(TEXT("Open begin list is correct after resize"), this, HashTable, 1, 1);
	}
...
bool FContainersNatvisTest::RunTest(const FString& Parameters)
{
	...
	{
		// TSet
		TSetNatvisTest<FSetResolver> SetNatvisTest;
		TSetNatvisTest<FSparseSetResolver> SparseSetNatvisTest;
		TSetNatvisTest<FCompactSetResolver> CompactNatvisTest;

		// TMap
		TMapNatvisTest<FSetResolver> MapNatvisTest;
		TMapNatvisTest<FSparseSetResolver> SparseMapNatvisTest;
		TMapNatvisTest<FCompactSetResolver> CompactMapNatvisTest;
