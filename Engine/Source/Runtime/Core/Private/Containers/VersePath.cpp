...
namespace UE::VersePath::Private
{
	...
	template <typename EndType>
	TSet<TCHAR> GetDomainLabelInvalidChars(const TCHAR* Ptr, EndType End)
	{
	...
	template <typename EndType>
	FText MakeDomainLabelInvalidCharsMessage(const TCHAR* Ptr, EndType End)
	{
		const TSet<TCHAR> InvalidChars = GetDomainLabelInvalidChars(Ptr, End);
		return MakeInvalidCharsMessage(InvalidChars, LOCTEXT("DomainLabel", "Domain label"));
	}

	template <typename EndType>
	bool ParseDomainLabel(const TCHAR*& Ptr, EndType End, FText* OutErrorMessage = nullptr)
	{
		const TCHAR* LocalPtr = Ptr;
		if (LocalPtr == End)
		{
		...
		if (!IsAlphaNum(*LocalPtr))
		{
			if (OutErrorMessage)
			{
				...
				{
					*OutErrorMessage = MakeDomainLabelInvalidCharsMessage(LocalPtr, End);
				}
			}
			return false;
		}
	...
	template <typename EndType>
	TSet<TCHAR> GetIdentInvalidChars(const TCHAR* Ptr, EndType End)
	{
		TSet<TCHAR> InvalidChars;
		while (Ptr != End)
		{
	...
	template <typename EndType>
	FText MakeIdentInvalidCharsMessage(const TCHAR* Ptr, EndType End, const FText& IdentTerm)
	{
		const TSet<TCHAR> InvalidChars = GetIdentInvalidChars(Ptr, End);
		return MakeInvalidCharsMessage(InvalidChars, IdentTerm);
	}

	template <typename EndType>
	bool ParseIdent(const TCHAR*& Ptr, EndType End, FText* OutErrorMessage = nullptr, const FText* IdentTermReplacement = nullptr)
	{
		...
		if (!IsAlpha(*LocalPtr))
		{
			if (OutErrorMessage)
			{
				...
				{
					*OutErrorMessage = MakeIdentInvalidCharsMessage(LocalPtr, End, IdentTermToUse);
				}
			}
			return false;
		}
	...
	template <typename EndType>
	bool ParseDomain(const TCHAR*& Ptr, EndType End, FText* OutErrorMessage = nullptr)
	{
		const TCHAR* DomainStart = Ptr;

		// Determine end of the first label (up to '@' or End)
		const TCHAR* FirstLabelEnd = Ptr;
		while (FirstLabelEnd != End && *FirstLabelEnd != TEXT('@'))
		{
			++FirstLabelEnd;
		}

		if (!ParseDomainLabel(Ptr, FirstLabelEnd, OutErrorMessage))
		{
			if (OutErrorMessage)
			{
		...
		// If there is an '@', consume it and parse the rest as a full label to End
		if (ParseChar('@', Ptr, End))
		{
			if (!ParseDomainLabel(Ptr, End, OutErrorMessage))
			{
				if (OutErrorMessage)
				{
					const FStringView Domain = GetPathSection(DomainStart, End);
					if (ensure(!Domain.IsEmpty()))
					{
						MakeInvalidDomainErrorMessage(Domain, *OutErrorMessage);
					}
				}
	...
	bool ParseSubpath(const TCHAR*& Ptr, EndType End, FText* OutErrorMessage = nullptr)
	{
		for (;;)
		{
			...
			const FStringView PathSection = GetPathSection(IdentStart, End);
			if (!ParseIdent(Ptr, IdentStart + PathSection.Len(), OutErrorMessage))
			{
				if (OutErrorMessage)
				{
					if (!PathSection.IsEmpty())
					{
						*OutErrorMessage = FText::Format(LOCTEXT("InvalidIdentifierInSubPath", "Invalid subpath \"{0}\" : {1}"), FText::FromStringView(PathSection), *OutErrorMessage);
					}
	...
	bool ParsePath(const TCHAR*& Ptr, EndType End, FText* OutErrorMessage = nullptr)
	{
		...
		const FStringView Domain = GetPathSection(Ptr, End);
		if (!ParseDomain(Ptr, Ptr + Domain.Len(), OutErrorMessage))
		{
			return false;
		}
	...
	{
		const bool bResult = UE::VersePath::Private::ParseDomain(Ptr, End, OutErrorMessage);
		// Make sure the entire string was parsed.
		return bResult && Ptr == End;
	}
	...
	{
		const bool bResult = UE::VersePath::Private::ParseIdent(Ptr, End, OutErrorMessage, IdentTermReplacement);
		// Make sure the entire string was parsed.
		return bResult && Ptr == End;
	}
