...
void FileCache_PostIoStoreCompressionBlockSize(uint32 InCompressionBlockSize, FString const& InContainerFilePath)
{
	if (bFileCacheInitialized.load())
	{
		...
		if (InCompressionBlockSize != CacheSlotID::GetSize())
		{
			...
			UE_LOGF(LogStreamingFileCache, Warning, "	Check your IoStore compression block size (Project Settings -> 'Package Compression Commandline Options'");
			UE_LOGF(LogStreamingFileCache, Warning, "	and your File Cache block size (fc.BlockSize cvar). They should match!");
		}
		return;
	}
	...
	, MinNumFreeSlots(NumSlots)
{
	...
	{
		UE_LOGF(LogStreamingFileCache, Warning, "IoStore containers have multiple compression block sizes! This means the FileCache block size must be misaligned with at least one!");
		UE_LOGF(LogStreamingFileCache, Warning, "	Check your IoStore compression block size (Project Settings -> 'Package Compression Commandline Options'");
		UE_LOGF(LogStreamingFileCache, Warning, "	and your File Cache block size (fc.BlockSize cvar). They should match!");
	}
