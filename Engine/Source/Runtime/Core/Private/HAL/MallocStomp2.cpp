...
void* FMallocStomp2::TryMalloc(SIZE_T Size, uint32 Alignment)
{
	...
	if (Size < MinSize || Size >= MaxSize || Alignment > PageSize || FPlatformTLS::GetTlsValue(FMallocStomp2::DisableTlsSlot) != 0)
	{
		return UsedMalloc->Malloc(Size, Alignment);
	}

	const SIZE_T AlignedSize = Align(Size, Alignment);
	const SIZE_T AllocFullPageSize = Align(AlignedSize + sizeof(FAllocationData), PageSize);
	...
	if(bUseUnderrunMode)
	{
		const SIZE_T AlignedAllocationData = Align(AllocationDataSize, Alignment);
...
FMalloc* FMallocStomp2::OverrideIfEnabled(FMalloc* InUsedAlloc)
{
	if (GMallocStomp2Enabled)
	{
		...
		ensureMsgf(MaxSize > 0, TEXT("MallocStomp2 has been enabled but MallocStomp2MaxSize is set to 0 - this will not produce meaningful results."));
		ensureMsgf(MaxSize >= MinSize, TEXT("MallocStomp2 has been enabled but MallocStomp2MaxSize is smaller than MallocStomp2MinSize - this will not produce meaningful results."));
#else
