...
#ifdef CACHE_FREED_OS_ALLOCS
		{
			...
			for (uint32 i=0; i < Allocator.FreedPageBlocksNum; ++i)
			{
				...
				if (Allocator.FreedPageBlocks[i].ByteSize == NewSize)
				{
					...
					UE_CLOGF(!Ret, LogMemory, Fatal, "OS memory allocation cache has been corrupted!");
...
/*			for (uint32 i=0; i < Allocator.FreedPageBlocksNum; ++i)
	...
	static FORCEINLINE void* SmallOSAlloc(FMallocBinned& Allocator, SIZE_T NewSize, SIZE_T& OutActualSize)
	{
		...
		if (Ptr == nullptr)
		{
			UE_LOGF(LogTemp, Warning, "malloc failure allocating %zu, error code: %d", NewSize, errno);
		}
	...
	static void UpdateSlackStat(FMallocBinned& Allocator)
	{
		...
		Allocator.SlackCurrent = Allocator.OsCurrent - LocalWaste - Allocator.UsedCurrent; //-V567
	#endif // STATS
	}
};
...
//need a lock to cover the SetAllocationSizes()
					...
					BINNED_PEAK_STATCOUNTER(UsedPeak, UsedCurrent); //-V567
...
void FMallocBinned::DumpAllocatorStats( class FOutputDevice& Ar )
{
	FBufferedOutputDevice BufferedOutput;
	{
		...
		BufferedOutput.CategorizedLogf( LogMemory.GetCategoryName(), ELogVerbosity::Log, TEXT( "Current Memory %.2f MB used, plus %.2f MB waste" ), UsedCurrent / (1024.0f * 1024.0f), (OsCurrent - UsedCurrent) / (1024.0f * 1024.0f) );  //-V567
		BufferedOutput.CategorizedLogf( LogMemory.GetCategoryName(), ELogVerbosity::Log, TEXT( "Peak Memory %.2f MB used, plus %.2f MB waste" ), UsedPeak / (1024.0f * 1024.0f), (OsPeak - UsedPeak) / (1024.0f * 1024.0f) ); //-V567

		BufferedOutput.CategorizedLogf( LogMemory.GetCategoryName(), ELogVerbosity::Log, TEXT( "Current OS Memory %.2f MB, peak %.2f MB" ), OsCurrent / (1024.0f * 1024.0f), OsPeak / (1024.0f * 1024.0f) ); //-V567
		BufferedOutput.CategorizedLogf( LogMemory.GetCategoryName(), ELogVerbosity::Log, TEXT( "Current Waste %.2f MB, peak %.2f MB" ), WasteCurrent / (1024.0f * 1024.0f), WastePeak / (1024.0f * 1024.0f) ); //-V567
		BufferedOutput.CategorizedLogf( LogMemory.GetCategoryName(), ELogVerbosity::Log, TEXT( "Current Used %.2f MB, peak %.2f MB" ), UsedCurrent / (1024.0f * 1024.0f), UsedPeak / (1024.0f * 1024.0f) ); //-V567
		...
		BufferedOutput.CategorizedLogf( LogMemory.GetCategoryName(), ELogVerbosity::Log, TEXT( "Allocs      % 6i Current / % 6i Total" ), CurrentAllocs, TotalAllocs ); //-V567
