...
//		ensure(IsGoodHelpString(Value));
	...
	/** Remove a single delegate to the new multicast delegate. */
	virtual void ClearOnChangedCallback() override
	{
		OnChangedCallback.Remove(LegacyDelegateHandle);
	}
	...
	bool CanChange(EConsoleVariableFlags SetBy) const
	{
		...
		if(!bRet)
		{
			...
			if (OldPri == ECVF_SetByConsoleVariablesIni || OldPri == ECVF_SetByCommandline || OldPri == ECVF_SetBySystemSettingsIni || OldPri == ECVF_SetByHotfix)
			{
				...
				UE_LOGF(LogConsoleManager, Verbose, "%ls", *Message);
			}
			else if (NewPri == ECVF_SetByScalability && OldPri == ECVF_SetByDeviceProfile)
			{
				...
				UE_LOGF(LogConsoleManager, Log, "%ls", *Message);
			}
			else
			{
				// Treat as a warning unless we're intentionally ignoring this priority interaction		
				UE_LOGF(LogConsoleManager, Warning, "%ls", *Message);
			}
		}
		...
		else
		{
			...
			if(ConsoleManager.IsThreadPropagationThread() && FPlatformProcess::SupportsMultithreading())
			{
				if(!bWarnedAboutThreadSafety)
				{
					...
					UE_LOGF(LogConsoleManager, Warning,
						"Console variable '%ls' used in the render thread. Rendering artifacts could happen. Use ECVF_RenderThreadSafe or don't use in render thread.",
						CVarName.IsEmpty() ? TEXT("unknown?") : *CVarName
						);
...
bool IConsoleManager::VisitPlatformCVarsForEmulation(FName PlatformName, const FString& DeviceProfileName, TFunctionRef<void(const FString& CVarName, const FString& CVarValue, EConsoleVariableFlags SetBy)> Visit)
{
	...
	} Sections[] =
	{
		...
		// Preview active GameFeaturePlugins - 
		// NOTE: THIS ONLY WORKS FOR ENGINE.INI GFP CONFIG FILES!
		//       because the code below only looks in the Engine branch, the GFPs don't run their Dynamic Modification code on other platforms configs
		{ TEXT("PluginCVars_LowPriority"), ECVF_SetByPluginLowPriority },
		{ TEXT("PluginCVars_HighPriority"), ECVF_SetByPluginHighPriority },
	...
	auto VisitIfAllowed = [&CVarSetByMap, &Visit](const FString& InName, const FString& Value, EConsoleVariableFlags SetBy)
	{
		...
		{
			UE_LOGF(LogConsoleManager, Log, "Skipping CVar %ls=%ls while visiting another platform, because it was already visited with a higher priority", *Name, *Value);
			return;
		}
	...
	for (const FSectionPair& SectionPair : Sections)
	{
		...
		if (bIsDeviceProfile)
		{
			...
			{
				UE_LOGF(LogConsoleManager, Warning, "Attempted to get CVars for another platform before FCoreDelegates::GatherDeviceProfileCVars was bound to a callback. CVar values are likely incorrect.");
				continue;
			}
			...
			UE_LOGF(LogConsoleManager, Verbose, "Gathering device profile cvars for %ls, platform config %ls", *DeviceProfileName, *PlatformName.ToString());
			...
			{
				NewSection->Add(Pair);
				UE_LOGF(LogConsoleManager, Verbose, "   %ls = %ls", *Pair.Key.ToString(), *Pair.Value);
			}
...
public:
	...
	virtual FResolvedContext ResolveContext(const FSetContext& Context) override
	{
		...
		if (Context.PriorityMode == FSetContext::EPriorityMode::ReplaceCurrent)
		{
			...
			{
				UE_LOGF(LogConsoleManager, Display, 
					"Trying to Replace Cvar=%ls Set By Constructor Value implicitly by calling SetWithCurrentPriority or ReplaceCurrentPriorityAndTag.\n" "This is not allowed, possible solutions:\n" "1. Use SetWithCurrentPriority or ReplaceCurrentPriorityAndTag with explicit MinPriority and MaxPriority values, with MinPriority >= ECVF_SetByScalability.\n" "2. Use explicit Set, Cvar->Set(<Value>, <SetByPriority>)",
...
protected:
	...
	virtual void Unset(EConsoleVariableFlags SetBy, FName Tag) override
	{
		...
		uint32 UnsetFlags = (uint32)SetBy & ECVF_SetFlagMask;

		// if we don't know the SetBy, unset from all
		// this isn't ideal because it could call SetInternal multiple times
		if ((SetBy & ECVF_SetByMask) == ECVF_SetByMask)
		{
			#define RECURSE(x) Unset(EConsoleVariableFlags(ECVF_SetBy##x | UnsetFlags), Tag);
		...
		if (UnsetPri >= CurrentPri)
		{
			...
			{
				UnsetFlags |= ECVF_Set_SetOnly_Unsafe;
			}

			NewSetBy = (EConsoleVariableFlags)(NewSetBy | UnsetFlags);

			UE_LOGF(LogConsoleManager, Verbose, " |-> Unsetting %ls, now %ls", *IConsoleManager::Get().FindConsoleObjectName(this),
...
/**
 * A thin console variable that delegates all reads to a Getter and all writes to a Setter.
 * No history, no priority tracking, no platform value caching - the external source is authoritative.
 * Inherits from FConsoleVariableBase for flags, help text, and change callbacks only.
 */
template <class T>
class FDelegatedConsoleVariable : public FConsoleVariableBase
{
public:
	FDelegatedConsoleVariable(
		T InDefaultValue,
		TFunction<T()> InGetter,
		TFunction<void(const T&)> InSetter,
		const TCHAR* Help,
		EConsoleVariableFlags InFlags)
		: FConsoleVariableBase(Help, InFlags)
		...
		, Setter(MoveTemp(InSetter))
	{
		check(Getter);
		check(Setter);
		check(!(InFlags & ECVF_RenderThreadSafe)); // Not supported - no shadow data
	}
	...
	virtual FResolvedContext ResolveContext(const FSetContext& Context) override
	{
		// No history system - just pass through the flags and tag as-is
		return FResolvedContext{ Context.Flags, Context.Tag };
	}

	virtual void Set(const TCHAR* Value, const FResolvedContext& Context) override
	{
		if (CanChange(Context.Flags))
		{
			T ParsedValue;
			UE::ConsoleManager::Private::GetValueFromString<T>(ParsedValue, Value);
			Setter(ParsedValue);
			OnChanged(Context.Flags, false);
		}
	}
	...
	virtual void Unset(EConsoleVariableFlags SetBy, FName Tag) override
	{
		// The getter is always authoritative, Unset has no effect
	}

	virtual bool GetBool() const override   { return FConsoleVariableConversionHelper<T>::GetBool(Getter()); }
	virtual int32 GetInt() const override   { return FConsoleVariableConversionHelper<T>::GetInt(Getter()); }
	virtual float GetFloat() const override { return FConsoleVariableConversionHelper<T>::GetFloat(Getter()); }
	virtual FString GetString() const override { return FConsoleVariableConversionHelper<T>::GetString(Getter()); }

	virtual FString GetDefaultValue() override
	{
		return TTypeToString<T>::ToString(DefaultValue);
	}
...
#if ALLOW_OTHER_PLATFORM_CONFIG
	virtual TSharedPtr<IConsoleVariable> GetPlatformValueVariable(FName PlatformName, const FString& DeviceProfileName) override { return nullptr; }
	virtual bool HasPlatformValueVariable(FName PlatformName, const FString& DeviceProfileName) override { return false; }
	virtual void SetOtherPlatformValue(const TCHAR* InValue, EConsoleVariableFlags SetBy, FName Tag) override {}
#endif
...
private:
	T DefaultValue;
	TFunction<T()> Getter;
	TFunction<void(const T&)> Setter;
};

template<>
void FDelegatedConsoleVariable<FString>::Set(
	const TCHAR* Value,
	const FResolvedContext& Context)
{
	if (CanChange(Context.Flags))
	{
		Setter(FString(Value));
		OnChanged(Context.Flags, false);
	}
}

template <class T>
IConsoleVariable* FConsoleManager::RegisterDelegatedConsoleVariableImpl(
	const TCHAR* Name, T DefaultValue,
	TFunction<T()> Getter, TFunction<void(const T&)> Setter,
	const TCHAR* Help, uint32 Flags)
{
	return AddConsoleObject(
		Name,
		new FDelegatedConsoleVariable<T>(
			MoveTemp(DefaultValue),
			MoveTemp(Getter),
			MoveTemp(Setter),
			Help,
			(EConsoleVariableFlags)Flags))->AsVariable();
}

IConsoleVariable* FConsoleManager::RegisterDelegatedConsoleVariable(const TCHAR* Name, int32 DefaultValue, TFunction<int32()> Getter, TFunction<void(const int32&)> Setter, const TCHAR* Help, uint32 Flags)
{
	return RegisterDelegatedConsoleVariableImpl(Name, DefaultValue, MoveTemp(Getter), MoveTemp(Setter), Help, Flags);
}

IConsoleVariable* FConsoleManager::RegisterDelegatedConsoleVariable(const TCHAR* Name, float DefaultValue, TFunction<float()> Getter, TFunction<void(const float&)> Setter, const TCHAR* Help, uint32 Flags)
{
	return RegisterDelegatedConsoleVariableImpl(Name, DefaultValue, MoveTemp(Getter), MoveTemp(Setter), Help, Flags);
}

IConsoleVariable* FConsoleManager::RegisterDelegatedConsoleVariable(const TCHAR* Name, bool DefaultValue, TFunction<bool()> Getter, TFunction<void(const bool&)> Setter, const TCHAR* Help, uint32 Flags)
{
	return RegisterDelegatedConsoleVariableImpl(Name, DefaultValue, MoveTemp(Getter), MoveTemp(Setter), Help, Flags);
}

IConsoleVariable* FConsoleManager::RegisterDelegatedConsoleVariable(const TCHAR* Name, const FString& DefaultValue, TFunction<FString()> Getter, TFunction<void(const FString&)> Setter, const TCHAR* Help, uint32 Flags)
{
	return RegisterDelegatedConsoleVariableImpl(Name, DefaultValue, MoveTemp(Getter), MoveTemp(Setter), Help, Flags);
}
...
FConsoleVariable<T>* FindOrCreateTypedPlatformCVar(FConsoleVariableExtendedData<T>* CVar, FName PlatformKey)
{
	...
	{
		PlatformCVar = TSharedPtr<IConsoleVariable>(new FConsoleVariable(CVar->GetDefaultTypedValue(), TEXT("Platform CVar copy"), (EConsoleVariableFlags)(CVar->GetFlags() & ~ECVF_SetByMask), CVar));
...
static const FString GSpecialDPNameForPremadePlatformKey("/");
/**
...
public:
	...
	{
		return FMath::Clamp(Base::GetFloat(), Min->GetFloat(), Max->GetFloat());
	}
...
template<>
TAutoRangedConsoleVariable<int32>::TAutoRangedConsoleVariable(IConsoleVariable* Parent, const TCHAR* Name, int32 DefaultValue, int32 MinValue, int32 MaxValue, const TCHAR* Help, uint32 Flags)
...
template<>
TAutoRangedConsoleVariable<float>::TAutoRangedConsoleVariable(IConsoleVariable* Parent, const TCHAR* Name, float DefaultValue, float MinValue, float MaxValue, const TCHAR* Help, uint32 Flags)
	: TAutoConsoleVariable(GetManager().AddConsoleObject(Name, new FRangedConsoleVariable(Name, DefaultValue, MinValue, MaxValue, Help, (EConsoleVariableFlags)Flags, Parent))->AsVariable())
{
}
...
IConsoleObject* FConsoleManager::FindConsoleObject(const TCHAR* Name, bool bTrackFrequentCalls) const
{
	...
	if (bTrackFrequentCalls)
	{
		...
		if(CVar)
		{
			...
			{
				UE_LOGF(LogConsoleManager, Warning, "Performance warning: Console object named '%ls' shows many (%d) FindConsoleObject() calls (consider caching e.g. using static)", Name, CVar->FindCallCount);
			}
		}
		else// if (bEarlyAppPhase || GFrameCounter == 1000)
		{
			...
			{
				UE_LOGF(LogConsoleManager, Warning, "Performance warning: Many (%d) failed FindConsoleObject() for '%ls'. ", PerNameNullFindCallCount[Name], Name);
			}

			if(NullFindCallCount == 500)
			{
				UE_LOGF(LogConsoleManager, Warning, "Performance warning: Many (%d) failed FindConsoleObject() across all CVars. Fail counts per name:", NullFindCallCount);
				for (TPair<FName, uint32> Entry : PerNameNullFindCallCount)
				{
					UE_LOGF(LogConsoleManager, Warning, "   %ls : %d", *Entry.Key.ToString(), Entry.Value);
				}
			}
		}
	}
...
void FConsoleManager::LoadHistoryIfNeeded()
{
	...
	const FString SectionName = "ConsoleHistory";
	const FName KeyName = "History";
	...
	for (const FString& Key : SortedKeys)
	{
		if (Prefix.IsEmpty() || Key.StartsWith(Prefix))
		{
			...
			if (CVar != nullptr)
			{
				...
				FString Value = Obj->IsDeprecated() ? TEXT("<deprecated, can't show>") : CVar->GetString();
...
void FConsoleManager::OnCreatedPlatformCVar(IConsoleVariable* MainVariable, TSharedPtr<IConsoleVariable> PlatformVariable, FName PlatformKey)
{
	...
	if (LoadedValues.Num() > 0)
	{
		for (TTuple<FString, EConsoleVariableFlags>* Value : LoadedValues)
		{
			...
			UE_LOGF(LogConsoleManager, Verbose, "Setting delayed platform cvar value, %ls = %ls, at priority %ls", *UnknownCVarKey, *Value->Key, GetConsoleVariableSetByName(Value->Value));
		}
...
bool FConsoleManager::ProcessUserConsoleInput(const TCHAR* InInput, FOutputDevice& Ar, UWorld* InWorld)
{
	...
	if( CCmd )
	{
		...
		TArray<FString> Args;
		FString Arg;
		while (FParse::Token(It, Arg, false))
		{
			Args.Add(Arg);
		}
...
IConsoleObject* FConsoleManager::AddConsoleObject(const TCHAR* Name, IConsoleObject* Obj)
{
	...
	if(ExistingObj)
	{
		...
		if( !bCanUpdateOrReplaceObj )
		{
			...
			UE_LOGF(LogConsoleManager, Warning, "Console object named '%ls' already exists but is being registered again, but we weren't expected it to be! (FConsoleManager::AddConsoleObject)", Name );
		}
		...
		{
			UE_LOGF(LogConsoleManager, Fatal, "Console object named '%ls' can't be replaced with the new one of different type!", Name );
		}
...
void FConsoleManager::LoadAllPlatformCVars(FName PlatformName, const FString& DeviceProfileName)
{
		...
		[PlatformName, PlatformKey, this](const FString& CVarName, const FString& CVarValue, EConsoleVariableFlags SetByAndPreview)
	{
		...
		IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(*CVarName, false /* bTrackFrequentCalls */);
		if (CVar == nullptr)
		{
			...
			UE_LOGF(LogConsoleManager, Verbose, "Loaded %ls@%ls = %ls, but the main CVar doesn't exist!", *CVarName, *PlatformKey.ToString(), *CVarValue);
			return;
		}
		...
		UE_LOGF(LogConsoleManager, Verbose, "Loading %ls@%ls = %ls [get = %ls]", *CVarName, *PlatformKey.ToString(),
...
{
	UE_LOGF(LogConsoleManager, Display, "Previewing/Stomping CVars for Platform '%ls', DeviceProfile '%ls', ModeTag '%ls'", *PlatformName.ToString(), *DeviceProfileName, *Tag.ToString());
		...
		if (IConsoleVariable* CVar = Pair.Value->AsVariable())
		{
			...
			if ((CVar->GetFlags() & (RequiredFlags | DisallowedFlags)) == RequiredFlags)
			{
				...
				if (CVar->HasPlatformValueVariable(PlatformKey, GSpecialDPNameForPremadePlatformKey))
				{
					...
					UE_LOGF(LogConsoleManager, Display, "  |-> Setting %ls = %ls", *Pair.Key, *PlatformCVar->GetString());
				}
			}
		}
	}
...
static TAutoConsoleVariable<int32> CVarMobileExtendedGBuffer(
	TEXT("r.Mobile.ExtendedGBuffer"),
	0,
	TEXT("0: 128bit GBuffer (default)\n"
		 "1: Enable the 160-bit GBuffer, which supports ClearCoat bottom normals and improved normal encoding with decal normal blending (requires static lighting to be disabled)."),
	ECVF_RenderThreadSafe | ECVF_ReadOnly);
...
static TAutoConsoleVariable<int32> CVarMobileFloatPrecisionMode(
	...
	0,
	TEXT("0: Use Half-precision ALU & some uniform buffers (default)\n"
		 "1: Half precision ALU, except Full precision for material expressions\n"
		 "2: Force use of high precision ALU & uniform buffers in pixel shaders.\n"),
	ECVF_ReadOnly);
...
static TAutoConsoleVariable<bool> CVarMaterialSimulateMobileForLowQuality(
	TEXT("r.Material.SimulateMobileForLowQuality"),
	false,
	TEXT("If true, use the ES3_1 pin on Feature Level Switch nodes and Mobile pin on Shading Path nodes for higher feature levels using low material quality"),
	ECVF_ReadOnly
);
...
static TAutoConsoleVariable<int32> CVarSkipPresentOnCameraCut(
	TEXT("r.SkipPresentOnCameraCut"),
	0,
	TEXT("0: Do not skip the Present call on a Camera Cut. (default)\n"
		"1: Skip the Present call on a Camera Cut."),
	ECVF_Scalability | ECVF_RenderThreadSafe);

static TAutoConsoleVariable<int32> CVarFinishCurrentFrame(
	TEXT("r.FinishCurrentFrame"),
	0,
	TEXT(" 0: Wait for previous frame to be Finished on the GPU.(default)\n"
		"-1: Do no wait for GPU.\n"
		" 1: Wait for current frame to be Finished on the GPU."),
	ECVF_Scalability | ECVF_RenderThreadSafe);
...
protected:
	...
	void LogOrEditorMessage(const FText& Msg, bool bIsError) const
	{
		...
		if (bIsError)
		{
			UE_LOGF(LogConsoleManager, Error, "%ls", *Msg.ToString());
		}
		else
		{
			UE_LOGF(LogConsoleManager, Warning, "%ls", *Msg.ToString());
		}
	}
			...
			if (Object == nullptr)
			{
				if (LookupBehavior != EShadowCVarBehavior::NoMessaging)
				{
					...
					case EShadowCVarBehavior::Assert:
						UE_LOGF(LogConsoleManager, Fatal, "%ls", *Message.ToString());
						break;
					}
				}
				return false;
	...
	bool BindForUsage() const
	{
		...
		if (!bHasMessagedForUsage && !FCVarDisableDeprecationWarningScope::IsDisabled() )
		{
			if (UsageBehavior != EShadowCVarBehavior::NoMessaging)
			{
				...
				if (DeprecationMessage.IsEmpty())
				{
					if (ShadowName.IsEmpty() || !!(GetRealObject()->GetFlags() & ECVF_Unregistered))
					{
						// in-place deprecation requires a version number
						Message = FText::Format(LOCTEXT("ShadowCVarUsage_DeprecatedInPlace", "Using a deprecated (as of UE {DeprecatedVersion}) CVar/Command: '{ThisName}'. It will be removed in the future, and may not function as expected. Please review your usage of {ThisName}."), Arguments);
					}
					else
					{
						if (DeprecatedVersion != TEXT(""))
						{
							Message = FText::Format(LOCTEXT("ShadowCVarUsage_Deprecated", "Using a deprecated (as of UE {DeprecatedVersion}) CVar/Command: '{ThisName}'. It will be removed in the future. Change all uses to '{ShadowName}' instead."), Arguments);
						}
						else
						{
...
public:
	...
	virtual void ClearOnChangedCallback() override
	{
		if (Bind())
		{
			RealVariable->ClearOnChangedCallback();
		}
	}
...
// helper class for in-place deprecation, which has a backing cvar unregistered cvar created with a _dep nam, but since 
// it's ECVF_Unregistered, it doesn't show up in auto-complete etc
class FConsoleVariableDeprecatedNoReplace : public FConsoleVariableShadow
{
public:
	FConsoleVariableDeprecatedNoReplace(const TCHAR* CVarToShadow, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
	: FConsoleVariableShadow(CVarToShadow, EShadowCVarBehavior::NoMessaging, UsageBehavior, true, DeprecatedAtVersion, DeprecationMessage)
	{
	...
	virtual EConsoleVariableFlags GetFlags() const override
	{
		if (Bind())
		{
			// we need to remove the ECVF_Unregistered flag when returning the info about the "good" cvar
			return (EConsoleVariableFlags)(GetRealObject()->GetFlags() & ~ECVF_Unregistered);
		}
		return (EConsoleVariableFlags)0;
...
class FConsoleCommandDeprecatedNoReplace : public FConsoleCommandShadow
{
public:
	FConsoleCommandDeprecatedNoReplace(const TCHAR* CVarToShadow, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
		: FConsoleCommandShadow(CVarToShadow, EShadowCVarBehavior::NoMessaging, UsageBehavior, true, DeprecatedAtVersion, DeprecationMessage)
	{
...
static FString MakeDeprecatedHelp(const TCHAR* Name, const TCHAR* DeprecatedAtVersion, const TCHAR* DeprecationMessage)
{
	FString Message = DeprecationMessage;
	if (Message.IsEmpty())
	{
		Message = FString::Printf(TEXT("'%s' has been deprecated as of UE %s. It should no longer be used."), Name, DeprecatedAtVersion);
	}

	return Message;
}

static thread_local int32 GDisabledCount = 0;

FCVarDisableDeprecationWarningScope::FCVarDisableDeprecationWarningScope(const bool bInCondition) 
...
FCVarDisableDeprecationWarningScope::~FCVarDisableDeprecationWarningScope() 
{
	if (bCondition)
	{
		--GDisabledCount;
...
bool FCVarDisableDeprecationWarningScope::IsDisabled()
{ 
	return GDisabledCount > 0; 
}

template<typename T, typename CreateVarFuncType>
IConsoleVariable* CreateInPlaceDeprecatedVar(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, uint32 Flags, CreateVarFuncType CreateVarFunc)
{
#if !NO_CVARS
	// create a backing unregistered cvar with a dummy name, but this name won't show up since Unregistered
	FString BackingName = FString(Name) + "_dep";
	IConsoleVariable* BackingCVar = CreateVarFunc(*MakeDeprecatedHelp(Name, DeprecationArgs.Version, DeprecationArgs.Message), (EConsoleVariableFlags)(Flags| ECVF_Unregistered));
	GetManager().AddConsoleObject(*BackingName, BackingCVar);
	
	// create the shadow cvar that shadows the backing cvar, so we can use all the existing logic
	FConsoleVariableDeprecatedNoReplace* DeprecatedCVar = new FConsoleVariableDeprecatedNoReplace(*BackingName, DeprecationArgs.Version, DeprecationArgs.UsageBehavior, DeprecationArgs.Message);
	// don't let the AddCosoleObject throw an error by calling GetFlags()
	GetManager().AddShadowConsoleObject(Name, DeprecatedCVar);
...
template<typename T>
IConsoleVariable* CreateInPlaceDeprecatedVar(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const T& DefaultValue, uint32 Flags)
{
	return CreateInPlaceDeprecatedVar<T>(DeprecationArgs, Name, Flags, 
		[&](const FString& DeprecationHelp, EConsoleVariableFlags ConsoleVariableFlags)
	{ 
		return new FConsoleVariable<T>(DefaultValue, *DeprecationHelp, ConsoleVariableFlags);
	});
}

template<typename T>
TAutoConsoleVariable<T>::TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const T& DefaultValue, uint32 Flags)
	: TAutoConsoleVariable(CreateInPlaceDeprecatedVar(DeprecationArgs, Name, DefaultValue, Flags))
{
}

template CORE_API TAutoConsoleVariable<int32>::TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const int32& DefaultValue, uint32 Flags);
template CORE_API TAutoConsoleVariable<float>::TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const float& DefaultValue, uint32 Flags);
template CORE_API TAutoConsoleVariable<bool>::TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const bool& DefaultValue, uint32 Flags);
template CORE_API TAutoConsoleVariable<FString>::TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const FString& DefaultValue, uint32 Flags);

template<typename T, typename CmdType>
void CreateInPlaceDeprecatedCommand(const TCHAR* Name, const CmdType& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
#if !NO_CVARS
	check(Name != nullptr && FCString::Strlen(Name) > 0);
	check(DeprecatedAtVersion != nullptr && FCString::Strlen(DeprecatedAtVersion) > 0);

	// create a backing unregistered cvar with a dummy name, but this name won't show up since Unregistered
	FString BackingName = FString(Name) + "_dep";
	T* BackingCommand = new T(Command, *MakeDeprecatedHelp(Name, DeprecatedAtVersion, DeprecationMessage), (EConsoleVariableFlags)(ECVF_Default | ECVF_Unregistered));
	GetManager().AddConsoleObject(*BackingName, BackingCommand);

	// create the shadow cvar that shadows the backing cvar, so we can use all the existing logic
	FConsoleCommandDeprecatedNoReplace* DeprecatedCVar = new FConsoleCommandDeprecatedNoReplace(*BackingName, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
	// don't let the AddCosoleObject throw an error by calling GetFlags()
	GetManager().AddShadowConsoleObject(Name, DeprecatedCVar);
...
void CreateInPlaceDeprecatedCommandExec(const TCHAR* Name, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
#if !NO_CVARS
	check(Name != nullptr && FCString::Strlen(Name) > 0);
	check(DeprecatedAtVersion != nullptr && FCString::Strlen(DeprecatedAtVersion) > 0);
	...
	// create a backing unregistered cvar with a dummy name, but this name won't show up since Unregistered
	FString BackingName = FString(Name) + "_dep";
	FConsoleCommandExec* BackingCommand = new FConsoleCommandExec(*MakeDeprecatedHelp(Name, DeprecatedAtVersion, DeprecationMessage), (EConsoleVariableFlags)(ECVF_Default | ECVF_Unregistered));
	GetManager().AddConsoleObject(*BackingName, BackingCommand);
...
template<typename T>
TAutoConsoleVariableDeprecated_NoReplace<T>::TAutoConsoleVariableDeprecated_NoReplace(const TCHAR* Name, const T& DefaultValue, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedVar(FCVarDeprecationArgs{ DeprecatedAtVersion, UsageBehavior, DeprecationMessage }, Name, DefaultValue, ECVF_Default);
}

// because we create a variable whose type is not in the header, we have to instantiate each possible type here
template CORE_API TAutoConsoleVariableDeprecated_NoReplace<int32>::TAutoConsoleVariableDeprecated_NoReplace(const TCHAR* Name, const int32& DefaultValue, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage);
template CORE_API TAutoConsoleVariableDeprecated_NoReplace<float>::TAutoConsoleVariableDeprecated_NoReplace(const TCHAR* Name, const float& DefaultValue, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage);
template CORE_API TAutoConsoleVariableDeprecated_NoReplace<bool>::TAutoConsoleVariableDeprecated_NoReplace(const TCHAR* Name, const bool& DefaultValue, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage);
template CORE_API TAutoConsoleVariableDeprecated_NoReplace<FString>::TAutoConsoleVariableDeprecated_NoReplace(const TCHAR* Name, const FString& DefaultValue, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage);

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommandExec(Name, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommand<FConsoleCommand>(Name, Command, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithArgsDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommand<FConsoleCommandWithArgs>(Name, Command, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithOutputDeviceDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommand<FConsoleCommandWithOutputDevice>(Name, Command, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommand<FConsoleCommanWithArgsAndOutputDevice>(Name, Command, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithWorldDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommand<FConsoleCommandWithWorld>(Name, Command, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommand<FConsoleCommandWithWorldArgsAndOutputDevice>(Name, Command, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}

FAutoConsoleCommandDeprecated_NoReplace::FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithWorldAndArgsDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior, const TCHAR* DeprecationMessage)
{
	CreateInPlaceDeprecatedCommand<FConsoleCommandWithWorldAndArgs>(Name, Command, DeprecatedAtVersion, UsageBehavior, DeprecationMessage);
}
...
template<typename T>
IConsoleVariable* CreateInPlaceDeprecatedVarRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, T& RefValue, uint32 Flags)
{
	return CreateInPlaceDeprecatedVar<T>(DeprecationArgs, Name, Flags, 
		[&](const FString& DeprecationHelp, EConsoleVariableFlags ConsoleVariableFlags)
	{ 
		return IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, *DeprecationHelp, ConsoleVariableFlags);
	});
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, bool DefaultValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, int32 DefaultValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, float DefaultValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, const TCHAR* DefaultValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, bool DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, int32 DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, float DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariable::FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, const TCHAR* DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, int32& RefValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, int32& RefValue, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, float& RefValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, float& RefValue, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, bool& RefValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, bool& RefValue, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, FString& RefValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FString& RefValue, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, FName& RefValue, const TCHAR* Help)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FName& RefValue, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, int32& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, int32& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, float& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, float& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, bool& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, bool& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, FString& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleVariableRef(Name, RefValue, Help, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleVariableRef::FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FString& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags)
	: FAutoConsoleObject(CreateInPlaceDeprecatedVarRef(DeprecationArgs, Name, RefValue, Flags))
{
	AsVariable()->SetOnChangedCallback(Callback);
}

FAutoConsoleCommand::FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommand::FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommand::FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithOutputDeviceDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommand::FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommand::FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommand::FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommandWithWorld::FAutoConsoleCommandWithWorld(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommandWithWorldAndArgs::FAutoConsoleCommandWithWorldAndArgs(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldAndArgsDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommandWithArgsAndOutputDevice::FAutoConsoleCommandWithArgsAndOutputDevice(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommandWithOutputDevice::FAutoConsoleCommandWithOutputDevice(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithOutputDeviceDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}

FAutoConsoleCommandWithWorldArgsAndOutputDevice::FAutoConsoleCommandWithWorldArgsAndOutputDevice(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command)
	: FAutoConsoleObject(IConsoleManager::Get().RegisterConsoleCommand(Name, Help, Command, Flags))
{
}
