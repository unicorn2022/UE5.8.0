	...
	FConsoleCommandDelegate::CreateStatic([]()
{
	...
	{
		UE_LOGF(LogPlatformFileManagedStorage, Display, "%ls", *CategoryStat.Value.Print());
	}
})
);
	...
	FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
{
	if (Args.Num() < 2)
	{
		UE_LOGF(LogPlatformFileManagedStorage, Error, "Not enough parameters to run console command CreateDummyFileInPersistentStorage");
		return;
	}
	...
	{
		UE_LOGF(LogPlatformFileManagedStorage, Error, "Failed to write dummy file %ls.  File path is not under /download0", *DummyFilePath);
		return;
	}
	...
	if (!Ar)
	{
		UE_LOGF(LogPlatformFileManagedStorage, Error, "Failed to write dummy file %ls.", *DummyFilePath);
		return;
	}
	...
	if(!Ar->Close())
	{
		UE_LOGF(LogPlatformFileManagedStorage, Error, "There was an error writing to file %ls.", *DummyFilePath);
	}
}),
ECVF_Default);
...
FPersistentStorageManager::FCategoryInfo FPersistentStorageManager::InitCategories()
{
	...
	for (const FString& Category : CategoryConfigs)
	{
			...
			FParse::Value(*TrimmedCategory, PropertyDirectories, DirectoryNames, false))
		{
			...
			{
				UE_LOGF(LogPlatformFileManagedStorage, Error, "Found empty category in [PersistentStorageManager]:Categories");
				continue;
			}
	...
	{
		UE_LOGF(LogPlatformFileManagedStorage, Error, "Default category %ls doesn't exist", *DefaultCategoryName);
	}
