...
struct FMallocBinnedGPU::FPoolInfoSmall
{
	...
	void CheckCanary(ECanary ShouldBe) const
	{
		if (Canary != ShouldBe)
		{
			UE_LOGF(LogMemory, Fatal, "MallocBinnedGPU Corruption Canary was 0x%x, should be 0x%x", int32(Canary), int32(ShouldBe));
		}
	}
	void SetCanary(ECanary ShouldBe, bool bPreexisting, bool bGuarnteedToBeNew)
	{
		if (bPreexisting)
		{
			...
			{
				UE_LOGF(LogMemory, Fatal, "MallocBinnedGPU Corruption Canary was 0x%x, should be 0x%x. This block is both preexisting and guaranteed to be new; which makes no sense.", int32(Canary), int32(ShouldBe));
			}
			if (ShouldBe == ECanary::SmallUnassigned)
			{
				if (Canary != ECanary::SmallAssigned)
				{
					UE_LOGF(LogMemory, Fatal, "MallocBinnedGPU Corruption Canary was 0x%x, will be 0x%x because this block should be preexisting and in use.", int32(Canary), int32(ShouldBe));
				}
			}
			else if (Canary != ShouldBe)
			{
				UE_LOGF(LogMemory, Fatal, "MallocBinnedGPU Corruption Canary was 0x%x, should be 0x%x because this block should be preexisting.", int32(Canary), int32(ShouldBe));
			}
		}
		else
		{
			if (bGuarnteedToBeNew)
			{
				...
				{
					UE_LOGF(LogMemory, Fatal, "MallocBinnedGPU Corruption Canary was 0x%x, will be 0x%x. This block is guaranteed to be new yet is it already assigned.", int32(Canary), int32(ShouldBe));
				}
			}
			else if (Canary != ShouldBe && Canary != ECanary::SmallUnassigned)
			{
				UE_LOGF(LogMemory, Fatal, "MallocBinnedGPU Corruption Canary was 0x%x, will be 0x%x does not have an expected value.", int32(Canary), int32(ShouldBe));
			}
		}
		Canary = ShouldBe;
	}
	...
	static void FreeBundles(FMallocBinnedGPU& Allocator, FBundleNode* BundlesToRecycle, uint32 InBlockSize, uint32 InPoolIndex)
	{
		...
		while (Bundle)
		{
			...
			do
			{
				...
				if (!NodePoolBlock)
				{
					UE_LOGF(LogMemory, Fatal, "FMallocBinnedGPU Attempt to free an unrecognized small block %p", GPUNode);
				}
...
FMallocBinnedGPU::~FMallocBinnedGPU()
{
	...
	MBG_STAT(
		UE_CLOGF(BinnedGPUAllocatedOSSmallPoolMemory > 0, LogCore, Error, "FMallocBinnedGPU leaked small block memory: %fmb", ((double)BinnedGPUAllocatedOSSmallPoolMemory) / (1024.0f * 1024.0f));
		UE_CLOGF(BinnedGPUAllocatedLargePoolMemoryWAlignment > 0, LogCore, Error, "FMallocBinnedGPU leaked large block memory: %fmb", ((double)BinnedGPUAllocatedLargePoolMemoryWAlignment) / (1024.0f * 1024.0f));
	)
...
void* FMallocBinnedGPU::MallocExternal(SIZE_T Size, uint32 Alignment)
{
	...
	UE_CLOGF(!IsAligned(Result, Alignment), LogMemory, Fatal, "FMallocBinnedGPU alignment was too large for OS. Alignment=%d   Ptr=%p", Alignment, Result);
...
void FMallocBinnedGPU::FreeExternal(void* Ptr)
{
	...
	else if (Ptr)
	{
...

		{
			...
			if (!Pool)
			{
				UE_LOGF(LogMemory, Fatal, "FMallocBinnedGPU Attempt to free an unrecognized block %p", Ptr);
			}
...
bool FMallocBinnedGPU::GetAllocationSizeExternal(void* Ptr, SIZE_T& SizeOut)
{
	...
	if (!Pool)
	{
		UE_LOGF(LogMemory, Fatal, "FMallocBinnedGPU Attempt to GetAllocationSizeExternal an unrecognized block %p", Ptr);
	}
...
void FMallocBinnedGPU::FlushCurrentThreadCache()
{
	...
	{
		UE_LOGF(LogMemory, Warning, "FMallocBinnedGPU took %6.2fms to wait for mutex for trim.", WaitForMutexTime * 1000.0f);
	}
	if (WaitForMutexAndTrimTime > 0.02f)
	{
		UE_LOGF(LogMemory, Warning, "FMallocBinnedGPU took %6.2fms to wait for mutex AND trim.", WaitForMutexAndTrimTime * 1000.0f);
	}
}
...
void FMallocBinnedGPU::Trim(bool bTrimThreadCaches)
{
	if (bTrimThreadCaches && ArenaParams.bPerThreadCaches)
	{
		...
		//UE_LOGF(LogTemp, Display, "Trim Broadcast = %6.2fms", 1000.0f * float(FPlatformTime::Seconds() - StartTime));
	}
}
...
{
	UE_LOGF(LogMemory, Fatal, "FMallocBinnedGPU Attempt to realloc an unrecognized block %p   canary == 0x%x != 0x%x", (void*)this, (int32)Canary, (int32)FMallocBinnedGPU::FFreeBlock::CANARY_VALUE);
}
