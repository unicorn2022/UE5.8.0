...
namespace MemoryMiscInternal
{
	...
	{
		UE_LOGF(LogMemory, Log, "ScopedMemoryStat[%ls] UsedPhysical %.02fMB (%+.02fMB), PeakPhysical: %.02fMB (%+.02fMB), UsedVirtual: %.02fMB (%+.02fMB) PeakVirtual: %.02fMB (%+.02fMB)",
			Context,
	...
	void LogSharedMemoryDiff(const TCHAR* Context, const FExtendedPlatformMemoryStats& StartStats, const FExtendedPlatformMemoryStats& EndStats)
	{
		...
		{
			UE_LOGF(LogMemory, Log, "SharedMemoryTracker[%ls] Unique: %.02fMB (%+.02fMB), Shared: %.02fMB (%+.02fMB), KSM Profit: %.02fMB (%+.02fMB), Shared To Unique: %.02fMB (%+.02fMB)",
				   Context,
				   ...
				   BytesToMB(EndStats.Shared_Memory_COW_Bytes),
				   MemDeltaInMB(EndStats.Shared_Memory_COW_Bytes, StartStats.Shared_Memory_COW_Bytes));
		}
		else
		{
			UE_LOGF(LogMemory, Log, "SharedMemoryTracker[%ls] Unique: %.02fMB (%+.02fMB), Shared: %.02fMB (%+.02fMB), Shared To Unique: %.02fMB (%+.02fMB)",
				   Context,
				   BytesToMB(EndStats.Private_Clean + EndStats.Private_Dirty),
				   MemDeltaInMB(EndStats.Private_Clean + EndStats.Private_Dirty, StartStats.Private_Clean + StartStats.Private_Dirty),
				   BytesToMB(EndStats.Shared_Clean + EndStats.Shared_Dirty),
