...
static FAutoConsoleCommand GDumpShortTermIOStats(
	...
	{
		UE_LOGF(LogDiskIO, Display, "Disk I/O short term stats:");
...
{
	UE_LOGF(LogDiskIO, Display, "Total Reads: %llu Total Bytes Read: %llu", TotalReads, TotalBytesRead);
	UE_LOGF(LogDiskIO, Display, "Total IO Time: %fs Total Idle Time: %fs", TotalIOTime, TotalIdleTime);
	UE_LOGF(LogDiskIO, Display, "Read Throughput: %fMB/s Pct Time Idle: %f%%", GetReadThrougputMBS(), GetPercentTimeIdle());
}
...
float FDiskUtilizationTracker::GetThrottleRateMBS()
{
	...
	{
		UE_LOGF(LogDiskIO, Warning, "Disk IO will be throttled to %fMB/s", ThrottledThroughputMBs);
	}
...
void FDiskUtilizationTracker::StartRead(uint64 InReadBytes, uint64 InSeekDistance)
{
	...
	if (bReset)
	{
		...
		UE_LOGF(LogDiskIO, Display, "Short term stats reset");
		bBreak = true;
	}
