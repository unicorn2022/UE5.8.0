...
uint32 FBitTree::CountOnes(uint32 UpTo) const
{
	...
	while (UpTo >= 64)
	{
		Result += FMath::CountBits64(*At);
		At++;
		UpTo -= 64;
	}
	if (UpTo)
	{
		Result += FMath::CountBits64((*At) << (64 - UpTo));
	}
	return Result;
}
...
#if UE_MBC_SUPPORT_RUNTIME_TLC_TOGGLE
...
{
	UE_LOGF(LogMemory, Fatal, "FMallocBinnedCommon attempted to GetAllocationSizeExternal on an unrecognized pointer %p", Ptr);
...
void FMallocBinnedCommonBase::LogLargeAllocation(SIZE_T Size) const
{
	...
	UE_LOGF(LogMemory, Log, "Allocating %d MB - 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx 0x%08llx",
		   AllocationSizeInMB,
		   CallStack[0],
		   CallStack[1],
		   CallStack[2],
...
void FBinCallStackTracker::ForceCrash()
{
	...
	UE_LOGF(LogMemory, Fatal, "OOM Force crash requested by MallocBinned Light Weight CallStack tracker, Frame %d", CallStackFrame.load(std::memory_order_relaxed));
}
