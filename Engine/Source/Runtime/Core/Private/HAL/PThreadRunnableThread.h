...
protected:
	...
	virtual bool SetThreadAttr(pthread_attr_t *ThreadAttr, uint32 InStackSize, EThreadCreateFlags InCreateFlags)
	{
		...
		if (InStackSize != 0)
		{
			if (pthread_attr_init(ThreadAttr) == 0)
			{
				...
				{
					UE_LOGF(LogHAL, Log, "Failed to change pthread stack size to %d bytes", (int)InStackSize);
				}
				return true;
			}
			else
			{
				UE_LOGF(LogHAL, Log, "Failed to init thread attr");
			}
		}
	...
	bool SpinPthread(pthread_t* HandlePtr, bool* OutThreadCreated, PthreadEntryPoint Proc, uint32 InStackSize, EThreadCreateFlags InCreateFlags, void *Arg)
	{
		...
		*OutThreadCreated = (ThreadErrno == 0);
		...
		if (!*OutThreadCreated)
		{
			UE_LOGF(LogHAL, Log, "Failed to create thread! (err=%d, %ls)", ThreadErrno, UTF8_TO_TCHAR(strerror(ThreadErrno)));
		}
	...
	static void *STDCALL _ThreadProc(void *pThis)
	{
		...
		if (ThisThread->ThreadID == ~uint32(0)) [[unlikely]]
		{
			LowLevelFatalError(TEXT("Thread ID is -1. Reconsider implementation of FThread::InvalidThreadId"));
		}
...
public:
	...
	virtual bool Kill(bool bShouldWait) override
	{
		...
		if (bShouldWait && bThreadStartedAndNotCleanedUp)
		{
			...
			UE_CLOGF(JoinResult != 0, LogHAL, Warning, "Failed to join thread %u (%ls). (err=%d, %ls)", ThreadID, *ThreadName, JoinResult, UTF8_TO_TCHAR(strerror(JoinResult)));
