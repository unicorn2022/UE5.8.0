...
FTaskTagScope::FTaskTagScope(ETaskTag InTag) : Tag(InTag)
{
	...
	{
		// The static thread ID must be initialized from the first call to this constructor.
		// Otherwise, there is a possibility that it will be initialized on a different thread.
		GetStaticThreadId();
...
#	endif
	...
	if (!EnumHasAllFlags(Tag, ETaskTag::EParallelThread))
	{
		...
		checkf(IsOK || IsEngineExitRequested(), TEXT("Only Scopes tagged with ETaskTag::EParallelThread can be tagged multiple times. ActiveNamedThreads(%x) cannot be tagged multiple times in the same callstack with the current Tag(%x)"), int32{EnumToUnderlyingType(OldTag)}, int32{EnumToUnderlyingType(Tag)});
	}
	...
	else if (TagOnlyIfNone && ActiveTaskTag != Tag)
	{
		...
		{
			checkf(IsInRenderingThread(), TEXT("ETaskTag::EParallelRenderingThread can only be retagged if they are in a parallel for on the RenderingThread or not tagged check the ActiveNamedThreads(%x)"), int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())});
		}

		if (EnumHasAllFlags(Tag, ETaskTag::EParallelGameThread))
		{
			checkf(IsInGameThread(), TEXT("ETaskTag::EParallelGameThread can only be retagged if they are in a parallel for on the GameThread or not tagged check the ActiveNamedThreads(%x)"), int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())});
		}
	}
}
...
{
	checkf(TagOnlyIfNone || ActiveTaskTag == Tag, TEXT("ActiveTaskTag(%x) corrupted needs to be Tag(%x)"), int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())}, int32{EnumToUnderlyingType(Tag)});
	...
	if (!EnumHasAllFlags(Tag, ETaskTag::EParallelThread))
	{
		...
		checkf((OldTag & NamedThreadBits) == (NamedThreadBits) || IsEngineExitRequested(), TEXT("Currently active Threads(%x) got corrupted check the ActiveNamedThreads(%x)"), int32{EnumToUnderlyingType(OldTag)}, int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())});
	}
...
#if !UE_BUILD_SHIPPING && !UE_BUILD_TEST && !USING_INSTRUMENTATION
		if (!UE::Tasks::Private::IsThreadRetractingTask())
		{
			const uint32 CurrentThreadId = FPlatformTLS::GetCurrentThreadId();
			bool oldValue = CurrentThreadId == GGameThreadId;
			ensureMsgf(oldValue == newValue, TEXT("oldValue(%i) newValue(%i) If this check fails make sure that there is a FTaskTagScope(ETaskTag::EGameThread) as deep as possible on the current callstack, you can see the current value in ActiveNamedThreads(%x)"), oldValue, newValue, int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())});
			newValue = oldValue;
		}
#endif
		return newValue;
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	if (!UE::Tasks::Private::IsThreadRetractingTask())
	{
		...
		ensureMsgf(oldValue == newValue, TEXT("oldValue(%i) newValue(%i) If this check fails make sure that there is a FTaskTagScope(ETaskTag::ESlateThread) as as deep as possible on the current callstack, you can see the current value in ActiveNamedThreads(%x)"), oldValue, newValue, int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())});
		newValue = oldValue;
	}
#endif
	return newValue;
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
#if !UE_BUILD_SHIPPING && !UE_BUILD_TEST
	{
		...
		ensureMsgf(oldValue == newValue, TEXT("oldValue(%i) newValue(%i) If this check fails make sure that there is a FTaskTagScope(ETaskTag::ERenderingThread) as deep as possible on the current callstack, you can see the current value in ActiveNamedThreads(%x)"), oldValue, newValue, int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())});
		newValue = oldValue;
	}
#endif
	return newValue;
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	...
	if (!UE::Tasks::Private::IsThreadRetractingTask())
	{
		...
		ensureMsgf(oldValue == newValue, TEXT("oldValue(%i) newValue(%i) If this check fails make sure that there is a FTaskTagScope(ETaskTag::EParallelRenderingThread) as deep as possible on the current callstack, you can see the current value in ActiveNamedThreads(%x)"), oldValue, newValue, int32{EnumToUnderlyingType(FTaskTagScope::GetCurrentTag())});
		newValue = oldValue;
	}
#endif
	return newValue;
...
bool FThreadManager::CheckThreadListSafeToContinueIteration()
{
	...
	{
		UE_LOGF(LogCore, Error, "FThreadManager::Threads was modified during unsafe iteration. Iteration will be aborted.");
		return false;
	}
...
void FThreadManager::AddThread(uint32 ThreadId, FRunnableThread* Thread)
{
	...
	bool bAlreadyInThreadPool = false;
	{
		const FScopeLock ThreadsLock(&ThreadsCritical);
		// Some platforms do not support TLS
		bAlreadyInThreadPool = Threads.Contains(ThreadId);
		if (!bAlreadyInThreadPool)
		{
			Threads.Add(ThreadId, Thread);
			OnThreadListModified();
		}
	}
	// notify observers only about new threads
	if (!bAlreadyInThreadPool)
	{
		ThreadEvent.Broadcast(ThreadId, Thread, EThreadEventType::Added);
	}
}

void FThreadManager::RemoveThread(FRunnableThread* Thread)
{
	TOptional<uint32> RemovedThreadId;
	{
		FScopeLock ThreadsLock(&ThreadsCritical);
		const uint32* ThreadId = Threads.FindKey(Thread);
		if (ThreadId)
		{
			RemovedThreadId.Emplace(*ThreadId);
			// `ThreadId` can be invalidated after it
			Threads.Remove(*ThreadId);
			OnThreadListModified();
		}
	}
	// notify observers only about existing threads
	if (RemovedThreadId)
	{
		ThreadEvent.Broadcast(RemovedThreadId.GetValue(), Thread, EThreadEventType::Removed);
	}
}

FThreadManager::FThreadEvent& FThreadManager::OnThreadEvent()
{
	return ThreadEvent;
}
...
FSharedEventRef::FSharedEventRef(EEventMode Mode  /* = EEventMode::AutoReset */)
	: Ptr(Mode == EEventMode::ManualReset
			? TLazySingleton<TEventPool<EEventMode::ManualReset>>::Get().GetRawEvent()
			: TLazySingleton<TEventPool<EEventMode::AutoReset>>::Get().GetRawEvent(),
		[](FEvent* Event)
		{
			if (Event->IsManualReset())
			{
...
FTlsAutoCleanup* FThreadSingletonInitializer::Get( TFunctionRef<FTlsAutoCleanup*()> CreateInstance, uint32& InOutTlsSlot )
{
	...
	// This open block is a) hit frequently and b) constrained in the scope of
	// open code that is touched.
	// When running with the AutoRTFM sanitizer on this becomes a performance
	// bottleneck even though there is no additional safety achieved. Thus we
	// run this open with the AutoRTFM sanitizer disabled.
...
{
	// Destruction of TLS values need to occur in the reverse order of initialization, as thread_local does.
	struct FTlsContainer : public TArray<TUniquePtr<FTlsAutoCleanup>>
	{
		~FTlsContainer()
		{
	...
	static thread_local FTlsContainer TlsInstances;
