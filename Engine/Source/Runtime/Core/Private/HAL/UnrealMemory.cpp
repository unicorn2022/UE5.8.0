...
class FMallocPurgatoryProxy : public FMalloc
{
	...
	enum
	{
		...
		PURGATORY_STOMP_MAX_PURGATORY_MEM_KB = 100000, // for loading and whatnot, we really can't keep this stuff forever
...
public:
	...
	*/
	virtual void Free(void* Ptr) override
	{
		...
		bool bFlushAnyway = OutstandingSizeInKB.GetValue() > PURGATORY_STOMP_MAX_PURGATORY_MEM_KB;
...
void FMemory::EnablePurgatoryTests()
{
	...
	{
		UE_LOGF(LogMemory, Error, "Purgatory proxy cannot be turned on because we are using PLATFORM_USES_FIXED_GMalloc_CLASS");
		return;
	}
	static bool bOnce = false;
	if (bOnce)
	{
		UE_LOGF(LogMemory, Error, "Purgatory proxy was already turned on.");
		return;
	}
	bOnce = true;
	while (true)
	{
		...
		{
			UE_LOGF(LogConsoleResponse, Display, "Purgatory proxy is now on.");
			return;
		}
		delete Proxy;
	}
...
void FMemory::EnablePoisonTests()
{
	...
	{
		UE_LOGF(LogConsoleResponse, Display, "SKIPPING Poison proxy - platform does not support multithreads");
		return;
	}
	if (PLATFORM_USES_FIXED_GMalloc_CLASS)
	{
		UE_LOGF(LogMemory, Error, "Poison proxy cannot be turned on because we are using PLATFORM_USES_FIXED_GMalloc_CLASS");
		return;
	}
	static bool bOnce = false;
	if (bOnce)
	{
		UE_LOGF(LogMemory, Error, "Poison proxy was already turned on.");
		return;
	}
	bOnce = true;
	while (true)
	{
		...
		if (FPlatformAtomics::InterlockedCompareExchangePointer((void**)&UE::Private::GMalloc, Proxy, LocalGMalloc) == LocalGMalloc)
		{
			UE_LOGF(LogConsoleResponse, Display, "Poison proxy is now on.");
			return;
		}
		delete Proxy;
	}
...
#error "Turn off PLATFORM_USES_FIXED_GMalloc_CLASS in order to use special allocator proxies"
	...
	{
		UE_LOGF(LogMemory, Fatal, "PLATFORM_USES_FIXED_GMalloc_CLASS only makes sense for allocators that are internally threadsafe.");
	}
#else
...
// On Mac it's too early to log here in some cases. For example GMalloc may be created during initialization of a third party dylib on load, before CoreFoundation is initialized
	...
	UE_LOGF(LogMemory, Display, "Used memory before allocating anything was %.2fMB", SizeInMb);
#endif
...
void FScopedMallocTimer::Spew()
{
	...
	if (Frames)
	{
		...
		auto PrintIt = [&TotalCycles, &TotalCount, &TotalMisses, &Frames](const TCHAR * Op, int32 InIndex)
		{
			...
			{
				UE_LOGF(LogMemory, Display, "FMemory %8ls  %5d count/frame   %6.2fms / frame (all threads)  %6.2fns / op    inline miss rate %5.2f%%",
					Op,
