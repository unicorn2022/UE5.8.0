...
namespace UE::Tasks
{
	namespace Private
	{
		...
		bool FTaskBase::TryRetractAndExecute(FTimeout Timeout, uint32 RecursionDepth/* = 0*/)
		{
			...
			if (!IsAwaitable())
			{
				UE_LOGF(LogTemp, Fatal, "Deadlock detected! A task can't be waited here, e.g. because it's being executed by the current thread");
				return false;
			}

			if (IsNamedThreadTask())
			{
				if (ExtendedPriority >= EExtendedTaskPriority::GameThreadNormalPri &&
					ExtendedPriority <= EExtendedTaskPriority::GameThreadHiPriLocalQueue &&
					IsInGameThread())
				{
					// Allow task retraction of game thread tasks on the game thread
				}
				else if (ExtendedPriority >= EExtendedTaskPriority::RenderThreadNormalPri &&
					ExtendedPriority <= EExtendedTaskPriority::RenderThreadHiPriLocalQueue &&
					IsInRenderingThread())
				{
					// Allow task retraction of rendering tasks on the rendering thread
				}
				else if (ExtendedPriority >= EExtendedTaskPriority::RHIThreadNormalPri &&
					ExtendedPriority <= EExtendedTaskPriority::RHIThreadHiPriLocalQueue &&
					IsInRHIThread())
				{
					// Allow task retraction of RHI tasks on the RHI thread
				}
				else
				{
					return false;
		...
		// Stack overflow prevention for deep Close -> TryUnlock -> Close chains.
		// Track recursion depth per-thread; beyond MaxCloseRecursionDepth, defer
		// subsequent unlocks into a worklist drained iteratively at the outermost level.
		static constexpr int32 MaxCloseRecursionDepth = 128;

		struct FCloseOverflowState
		{
			int32 Depth = 0;
			TArray<FTaskBase*> Pending;
		};
		static thread_local FCloseOverflowState CloseOverflow;
		...
		void FTaskBase::Close()
		{
			...
			if (--CloseOverflow.Depth == 0 && CloseOverflow.Pending.Num() > 0)
			{
				// Always wake up workers, at this point we're long past the first element anyway
				bWakeUpWorker = true;

				TArray<FTaskBase*> Pending;

				// Bump Depth back to 1 during drain so recursive Close calls don't also try to drain.
				++CloseOverflow.Depth;
				while (CloseOverflow.Pending.Num())
				{
					// Backbuffer to maintain ordering without having to allocate for each iteration
					Swap(Pending, CloseOverflow.Pending);

					for (FTaskBase* Task : Pending)
					{
						// TryUnlock may recurse into Close, appending more items
						Task->TryUnlock(bWakeUpWorker);
					}

					Pending.Reset(); 
				}
				CloseOverflow.Pending.Empty(); // Don't leave an allocation when done
				--CloseOverflow.Depth;
			}
		}
...
#undef CONVERT_EXTENDED_TASK_PRIORITY
	...
	namespace Private
	{
		...
		ENamedThreads::Type TranslatePriority(EExtendedTaskPriority Priority)
		{
			...
			default:
				checkf(Priority >= EExtendedTaskPriority::GameThreadNormalPri && Priority < EExtendedTaskPriority::Count, TEXT("only named threads can call this method: %d"), int32{EnumToUnderlyingType(Priority)});
