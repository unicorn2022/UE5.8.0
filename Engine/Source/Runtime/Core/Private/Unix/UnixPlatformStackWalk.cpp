...
// When true, thread stack capture timeout should not cause a deliberate crash.
// Set during crash handling to prevent recursive crashes when gathering all thread stacks.
bool GUnixStackWalkNoCrashOnTimeout = false;

// Set while a stack walk is in progress on the current thread. Without this guard,
// a failure inside the stack walker can recurse until the stack is exhausted.
static thread_local bool GStackWalkInProgress = false;

// Returns true if it is safe to start a stack walk on the current thread. Returns false
// if one is already in progress (the caller should bail out); in that case, an optional
// caller-supplied buffer is populated with a short marker so its output path is not
// left with stale data.
static bool CanProcessStackWalk(ANSICHAR* HumanReadableString = nullptr, SIZE_T HumanReadableStringSize = 0)
{
	if (GStackWalkInProgress)
	{
		if (HumanReadableString && HumanReadableStringSize > 0)
		{
			FCStringAnsi::Strncpy(HumanReadableString, "[stack walker re-entered; skipping]\n", HumanReadableStringSize);
		}
		return false;
	}
	return true;
...
void CORE_API UnixPlatformStackWalk_PreloadModuleSymbolFile()
{
	if (GModuleSymbolFileMemory == nullptr)
	{
		...
		{
			int ErrNo = errno;
			UE_LOGF(LogHAL, Warning, "UnixPlatformStackWalk_PreloadedModuleSymbol: open() failed on path %ls errno=%d (%ls)",
				*ModuleSymbolPath,
				ErrNo,
		...
		else
		{
			...
			UE_LOGF(LogHAL, Warning, "UnixPlatformStackWalk_PreloadModuleSymbolFile: GModuleSymbolFileMemory = 0x%llx, GModuleSymbolFileMemorySize = %zu bytes", GModuleSymbolFileMemory, GModuleSymbolFileMemorySize);

			GModuleSymbolFileMemory += FPlatformMemory::GetConstants().PageSize;
			UE_LOGF(LogHAL, Warning, "UnixPlatformStackWalk_PreloadModuleSymbolFile: GModuleSymbolFileMemory = 0x%llx (After adding additional memory page", GModuleSymbolFileMemory);
			...
			{
				UE_LOGF(LogHAL, Warning, "UnixPlatformStackWalk_PreloadedModuleSymbol: BytesRead %zd Expected %zu", BytesRead, GModuleSymbolFileMemorySize);
				UnixPlatformStackWalk_UnloadPreloadedModuleSymbol();

				if (BytesRead == -1)
				{
					int ErrNo = errno;
					UE_LOGF(LogHAL, Warning, "UnixPlatformStackWalk_PreloadedModuleSymbol: read() failed, errno=%d (%ls)",
						ErrNo,
						UTF8_TO_TCHAR(strerror(ErrNo)));
				}
			}
...
#pragma pack(pop)
	...
	public:
		...
		{
			// We need at least a single byte to record an empty string into `Buffer`
			check(MaxSize > 0);

			// Offset unsigned -1 (0xffffffff) is invalid
			if (LIKELY(Offset != static_cast<uint32_t>(-1)))
			{
	...
	bool PopulateProgramCounterSymbolInfoFromSymbolFile(uint64 ProgramCounter, FProgramCounterSymbolInfo& out_SymbolInfo)
	{
		...
		if (bDladdrRet != 0)
		{
				...
				UNLIKELY(info.dli_fbase == nullptr))
			{
				...
				{
					UE_LOGF(LogCore, VeryVerbose, "Symbol lookup time for 0x%016llx Dladdr: %lfms", reinterpret_cast<void*>(ProgramCounter), (DladdrEndTime - StartTime) * 1000);
				}
		...
		if (CheckingEnsureTime)
		{
			UE_LOGF(LogCore, VeryVerbose, "Symbol lookup time for 0x%016llx Dladdr: %lfms Open: %lfms Search: %lfms",
					ProgramCounter,
...
{
	if (!CanProcessStackWalk(HumanReadableString, HumanReadableStringSize))
	{
		return;
	}
	TGuardValue<bool> ReentryGuard(GStackWalkInProgress, true);
...
UE_AUTORTFM_ALWAYS_OPEN uint32 FUnixPlatformStackWalk::CaptureStackBackTraceViaFramePointerWalking(uint64* BackTrace, uint32 MaxDepth)
{
#if defined(__aarch64__) || defined(__x86_64__)
	uint64 StackTopPtr = 0;

	// Cache the main thread's stack top to avoid repeated pthread_getattr_np calls.
	const bool bIsMainThread = syscall(SYS_gettid) == getpid();
	static std::atomic<uint64> StackTopMainThread{0};
	uint64 CachedStackTop = StackTopMainThread.load(std::memory_order_relaxed);
	if (bIsMainThread && CachedStackTop != 0)
	{
		StackTopPtr = CachedStackTop;
	}
	else
	{
		...
		void* StackBase;
		size_t StackSize;
		if (pthread_attr_getstack(&ThreadAttr, &StackBase, &StackSize) != 0)
		{
		...
		if (bIsMainThread)
		{
			StackTopMainThread.store(StackTopPtr, std::memory_order_relaxed);
		}
	}
	...
	struct StackFrame
	{
		...
		inline UPTRINT GetReturnPtr()
		{
			...
			uintptr_t Ptr = ReturnPtr;
			// xpaclri operates on the link register (x30), so move value there and back.
			// This instruction is HINT #7, mapped to NOP on pre-PAC architectures.
			asm volatile(
				"mov x30, %0\n"
				"xpaclri\n"
				"mov %0, x30\n"
...
namespace
{
	// Shorter timeout for crash handling — the default CVar is 60s which would cause
	// multi-minute waits if several threads are unresponsive.
	constexpr double CrashHandlingThreadStackMaxWait = 2.0;

	void WaitForSignalHandlerToFinishOrCrash(ThreadStackUserData& ThreadStack)
	{
		const double MaxWait = GUnixStackWalkNoCrashOnTimeout
			? CrashHandlingThreadStackMaxWait
			: CVarUnixPlatformThreadCallStackMaxWait.AsVariable()->GetFloat();
		double EndWaitTimestamp = FPlatformTime::Seconds() + MaxWait;
		...
		while (!ThreadStack.bDone)
		{
			...
			{
				if (GUnixStackWalkNoCrashOnTimeout)
				{
					// During crash handling, skip unresponsive threads rather than causing a recursive crash
					return;
				}
	...
	void GatherCallstackFromThread(ThreadStackUserData& ThreadStack, uint64 ThreadId)
	{
		...
		if (syscall(SYS_rt_tgsigqueueinfo, info.si_pid, ThreadId, THREAD_CALLSTACK_GENERATOR, &info) == 0)
		{
			...
			if (!ThreadStack.bDone && GUnixStackWalkNoCrashOnTimeout)
			{
				// Timed out waiting for signal handler. Explicitly mark as failed so
				// callers see a deterministic 0 count rather than stale/partial data.
				ThreadStack.BackTraceCount = 0;
			}
		}
	}
}
...
{
	// During crash handling (GUnixStackWalkNoCrashOnTimeout == true), use a ring buffer of
	// static entries so that if the signal times out and we return early, the in-flight signal
	// handler on the target thread still has valid memory to write to (avoiding use-after-scope
	// on a stack local). The ring buffer ensures each timed-out capture gets its own slot,
	// preventing a late-arriving handler from corrupting the current capture's data.
	static constexpr uint32 RingBufferSize = 8;
	static ThreadStackUserData StaticRingBuffer[RingBufferSize];
	static uint32 RingBufferIndex = 0;
	ThreadStackUserData StackThreadBackTrace;

	ThreadStackUserData& ThreadBackTrace = GUnixStackWalkNoCrashOnTimeout
		? StaticRingBuffer[RingBufferIndex++ % RingBufferSize]
		: StackThreadBackTrace;
