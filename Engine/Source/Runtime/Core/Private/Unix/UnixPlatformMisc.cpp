...
static void UnixPlatForm_CheckIfKSMUsable()
{
	...
	if (GUseKSM)
	{
		...
		{
			GUseKSM = 0;
			UE_LOGF(LogInit, Error, "Cannot run ksm when its disabled in the kernel. Please check /sys/kernel/mm/ksm/run");
		}
		else
		{
			if (PagesToScan <= 0)
			{
				GUseKSM = 0;
				UE_LOGF(LogInit, Error, "KSM enabled but number of pages to be scanned is 0 which will implicitly disable KSM. Please check /sys/kernel/mm/ksm/pages_to_scan");
			}
			else
			{
				UE_LOGF(LogInit, Log, "KSM enabled. Number of pages to be scanned before ksmd goes to sleep: %u", PagesToScan);
			}
		}
	}
...
void FUnixPlatformMisc::PlatformInit()
{
	...
	UE_LOGF(LogInit, Log, "Unix hardware info:");
	UE_LOGF(LogInit, Log, " - we are %lsthe first instance of this executable", bFirstInstance ? TEXT("") : TEXT("not "));
	UE_LOGF(LogInit, Log, " - this process' id (pid) is %d, parent process' id (ppid) is %d", static_cast< int32 >(getpid()), static_cast< int32 >(getppid()));
	UE_LOGF(LogInit, Log, " - we are %lsrunning under debugger", IsDebuggerPresent() ? TEXT("") : TEXT("not "));
	UE_LOGF(LogInit, Log, " - machine network name is '%ls'", FPlatformProcess::ComputerName());
	UE_LOGF(LogInit, Log, " - user name is '%ls' (%ls)", FPlatformProcess::UserName(), FPlatformProcess::UserName(false));
	UE_LOGF(LogInit, Log, " - we're logged in %ls", FPlatformMisc::HasBeenStartedRemotely() ? TEXT("remotely") : TEXT("locally"));
	UE_LOGF(LogInit, Log, " - we're running %ls rendering", bIsNullRHI ? TEXT("without") : TEXT("with"));
	UE_LOGF(LogInit, Log, " - CPU: %ls '%ls' (signature: 0x%X)", *FPlatformMisc::GetCPUVendor(), *FPlatformMisc::GetCPUBrand(), FPlatformMisc::GetCPUInfo());
	UE_LOGF(LogInit, Log, " - Number of physical cores available for the process: %d", FPlatformMisc::NumberOfCores());
	UE_LOGF(LogInit, Log, " - Number of logical cores available for the process: %d", FPlatformMisc::NumberOfCoresIncludingHyperthreads());

	if (!GPUBrandInfo.IsEmpty())
	{
		UE_LOGF(LogInit, Log, " - GPU Brand Info: %ls", *GPUBrandInfo);
	}

	UE_LOGF(LogInit, Log, " - Memory allocator used: %ls", UE::Private::GMalloc->GetDescriptiveName());
	UE_LOGF(LogInit, Log, " - This is %ls build.", BuildSettings::IsLicenseeVersion() ? TEXT("a licensee") : TEXT("an internal"));

	FPlatformTime::PrintCalibrationLog();

	UE_LOGF(LogInit, Log, "Unix-specific commandline switches:");
	UE_LOGF(LogInit, Log, " -ansimalloc - use malloc()/free() from libc (useful for tools like valgrind and electric fence)");
	UE_LOGF(LogInit, Log, " -jemalloc - use jemalloc for all memory allocation");
	UE_LOGF(LogInit, Log, " -binnedmalloc - use binned malloc  for all memory allocation");
	UE_LOGF(LogInit, Log, " -filemapcachesize=NUMBER - set the size for case-sensitive file mapping cache");
	UE_LOGF(LogInit, Log, " -useksm - uses kernel same-page mapping (KSM) for mapped memory (%ls)", GUseKSM ? TEXT("ON") : TEXT("OFF"));
	UE_LOGF(LogInit, Log, " -ksmmergeall - marks all mmap'd memory pages suitable for KSM (%ls)", GKSMMergeAllPages ? TEXT("ON") : TEXT("OFF"));
	UE_LOGF(LogInit, Log, " -preloadmodulesymbols - Loads the main module symbols file into memory (%ls)", bPreloadedModuleSymbolFile ? TEXT("ON") : TEXT("OFF"));
	UE_LOGF(LogInit, Log, " -sigdfl=SIGNAL - Allows a specific signal to be set to its default handler rather then ignoring the signal");
	UE_LOGF(LogInit, Log, " -crashhandlerstacksize - Allows setting crash handler stack sizes (%lu)", GCrashHandlerStackSize);
	UE_LOGF(LogInit, Log, " -noexclusivelockonwrite - disables marking files created by the engine as exclusive locked while the engine has them opened");

	// [RCL] FIXME: this should be printed in specific modules, if at all
	UE_LOGF(LogInit, Log, " -httpproxy=ADDRESS:PORT - redirects HTTP requests to a proxy (only supported if compiled with libcurl)");
	UE_LOGF(LogInit, Log, " -reuseconn - allow libcurl to reuse HTTP connections (only matters if compiled with libcurl)");
	UE_LOGF(LogInit, Log, " -virtmemkb=NUMBER - sets process virtual memory (address space) limit (overrides VirtualMemoryLimitInKB value from .ini)");
	UE_LOGF(LogInit, Log, " -allowsyscallfilterfile=PATH_TO_FILE - sets up a system call filter allow list. any invalid syscall in this list *will* cause a crash");
	...
	if (GNameBlocksDebug)
	{
		UE_LOGF(LogInit, VeryVerbose, "GNameBlocksDebug Valid - %i", !!GNameBlocksDebug);
	}
}
...
void FUnixPlatformMisc::PlatformTearDown()
{
	...
	if (GDeferedExitLogging)
	{
		...
		{
			UE_LOGF(LogCore, Log, "FUnixPlatformMisc::RequestExit(bForce=false, ReturnCode=%d)", OverriddenErrorLevel);
		}
		else
		{
			UE_LOGF(LogCore, Log, "FUnixPlatformMisc::RequestExit(false)");
		}
	}
...
void FUnixPlatformMisc::RequestExit(bool Force, const TCHAR* CallSite)
{
	...
	{
		UE_LOGF(LogCore, Log,  "FUnixPlatformMisc::RequestExit(%i, %ls)", Force,
...
void FUnixPlatformMisc::RequestExitWithStatus(bool Force, uint8 ReturnCode, const TCHAR* CallSite)
{
	...
	{
		UE_LOGF(LogCore, Log, "FUnixPlatformMisc::RequestExit(bForce=%ls, ReturnCode=%d, CallSite=%ls)",
...
void FUnixPlatformMisc::GetOSVersions(FString& out_OSVersionLabel, FString& out_OSSubVersionLabel)
{
	...
	out_OSVersionLabel = FString("GenericLinuxVersion");
	// Get Kernel Version
...
#undef _XTAG
	...
	{
		int ErrNo = errno;
		UE_LOGF(LogCore, Warning, "Unable to fopen('%ls'): errno=%d (%ls)",
...
{
#if PLATFORM_HAS_CPUID
	static FString Result = "NonX86CPUBrand";
	static bool bHaveResult = false;
...
bool FUnixPlatformMisc::GetDiskTotalAndFreeSpace(const FString& InPath, uint64& TotalNumberOfBytes, uint64& NumberOfFreeBytes)
{
	...
	{
		int ErrNo = errno;
		UE_LOGF(LogCore, Warning, "Unable to statfs('%ls'): errno=%d (%ls)", *InPath, ErrNo, UTF8_TO_TCHAR(strerror(ErrNo)));
	}
	return (Err == 0);
}
...
{
	Result = FUnixPlatformGuid::CreateGuid();
}
...
namespace
{
	...
	public:
		...
		bool FinalizeSyscallFilter()
		{
			...
			{
				UE_LOGF(LogHAL, Warning, "To many filters have been added, max is %i", TNumericLimits<unsigned short>::Max());
			...
			{
				int ErrNo = errno;
				UE_LOGF(LogHAL, Warning, "prctl failed to set PR_SET_NO_NEW_PRIVS '%ls'", ANSI_TO_TCHAR(strerror(ErrNo)));
			...
			{
				int ErrNo = errno;
				UE_LOGF(LogHAL, Warning, "prctl failed to set SECCOMP_MODE_FILTER '%ls'", ANSI_TO_TCHAR(strerror(ErrNo)));
...
bool FUnixPlatformMisc::SetupSyscallFilters()
{
	...
	if (bHasBeenSetup)
	{
		UE_LOGF(LogHAL, Warning, "Syscall Filters have already been setup");
	...
	UE_LOGF(LogHAL, Display, "Setting up System call filters.");
	...
	if (FFileHelper::LoadFileToStringArray(Syscalls, *SyscallFilterPath))
	{
		for (const FString& SyscallRule : Syscalls)
		{
			...
			if (SyscallRule.FindChar(TEXT(' '), Index))
			{
				...
				if (ArgList.Num() != 5)
				{
					UE_LOGF(LogHAL, Warning, "Failed to parse syscall rule '%ls', invalid number of arguments", *SyscallRule);
				}
				else
				{
					...
					if (SyscallNumber >= 0)
					{
						...
						UE_LOGF(LogHAL, Verbose, "Adding syscall rule '%ls'('%i') arg number %i value %i allow %i check bit only %i", *ArgList[0], SyscallNumber, SyscallArgNumber, Value, Allow, BitCheck);
					}
					else
					{
						UE_LOGF(LogHAL, Warning, "Failed to find the syscall number in the look-up table for '%ls'", *SyscallRule);
					}
				}
			}
			else
			{
				...
				if (SyscallNumber >= 0)
				{
					...
					UE_LOGF(LogHAL, Verbose, "Adding syscall '%ls' mapped to syscall number '%i' to filter", *SyscallRule, SyscallNumber);
				}
				else
				{
					UE_LOGF(LogHAL, Warning, "Failed to find the syscall number in the look-up table for '%ls'", *SyscallRule);
...
{ 
	...
	{
		UE_LOGF(LogHAL, Error, "Profiling data collection is already under way! (file being written is '%ls'", *FPlatformMisc::GetEnvironmentVariable(ProfFileEnvVar));
		return false;
	}

	UnixPlatformMisc::GPGICollectionUnderway = true;
	FPlatformMisc::SetEnvironmentVar(ProfFileEnvVar, *AbsoluteFileName);
	UE_LOGF(LogHAL, Log, "Starting PGI data collection to file '%ls'", *AbsoluteFileName);
...
bool FUnixPlatformMisc::StopPGOCollectionAndCloseFile()
{
	if (!FPlatformMisc::IsPGIActive())
	{
		UE_LOGF(LogHAL, Warning, "Cannot stop PGO data collection, it was not started.");
		return false;
	}

	UE_LOGF(LogHAL, Log, "Stopping PGO data collection.");

	if (__llvm_profile_write_file() != 0)
	{
		UE_LOGF(LogHAL, Error, "Error writing out PGO file.");
	}
	...
	FString DummyProfileFileName = "/tmp/";
