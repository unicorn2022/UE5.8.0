...
static TAutoConsoleVariable<bool> CVarUnixCaptureAllThreadStacks(
	TEXT("unix.CaptureAllThreadStacksOnCrash"),
	true,
	TEXT("When true, capture callstacks from all threads during crash reporting.\n")
	TEXT("Set to false to disable if this causes issues such as hangs or recursive crashes."));
...
{
	FString ErrorString = "Unhandled Exception: ";
...
// Defined in UnixPlatformStackWalk.cpp - prevents deliberate crash on timeout during crash handling
extern bool GUnixStackWalkNoCrashOnTimeout;

void FUnixCrashContext::CaptureAllThreadContexts()
{
#if PLATFORM_SUPPORTS_ALL_THREAD_BACKTRACES
	if (!CVarUnixCaptureAllThreadStacks.AsVariable()->GetBool())
	{
		return;
	}

	const uint32 CrashThreadId = FPlatformTLS::GetCurrentThreadId();

	// Reserve upfront to minimize allocations during crash handling.
	// ~1KB per thread XML entry, 128 threads max = ~128KB.
	AllThreadContextsString.Reserve(128 * 1024);

	// Prevent recursive crash if a thread is unresponsive during stack capture
	GUnixStackWalkNoCrashOnTimeout = true;

	FThreadManager::Get().ForEachThreadStackBackTrace(
		[this, CrashThreadId](uint32 ThreadId, const TCHAR* ThreadName, const TConstArrayView<uint64>& StackTrace)
		{
			TArray<FCrashStackFrame> PortableStack;
			if (ThreadId == CrashThreadId && CallStack.Num() > 0)
			{
				// For the crashing thread, use the already-captured CallStack which has
				// better accuracy from the signal context than a re-capture would
				PortableStack = CallStack;
			}
			else
			{
				GetPortableCallStack(StackTrace.GetData(), StackTrace.Num(), PortableStack);
			}

			// Skip threads where we failed to capture a callstack (e.g. thread already
			// exited, or the RT signal could not be delivered). This matches Windows
			// behavior where such threads don't appear with empty stacks.
			if (PortableStack.Num() == 0 && ThreadId != CrashThreadId)
			{
				return true;
			}

			// Format as XML and append
			int32 MaxModuleNameLen = 0;
			for (const FCrashStackFrame& StFrame : PortableStack)
			{
				MaxModuleNameLen = FMath::Max(MaxModuleNameLen, StFrame.ModuleName.Len());
			}

			AllThreadContextsString += TEXT("<Thread>");
			{
				AllThreadContextsString += TEXT("<CallStack>");
				...
				AppendEscapedXMLString(AllThreadContextsString, *CallstackStr);
				AllThreadContextsString += TEXT("</CallStack>");
				AllThreadContextsString += LINE_TERMINATOR;
			}
			AllThreadContextsString += FString::Printf(TEXT("<IsCrashed>%s</IsCrashed>"), ThreadId == CrashThreadId ? TEXT("true") : TEXT("false"));
			AllThreadContextsString += LINE_TERMINATOR;
			AllThreadContextsString += TEXT("<Registers></Registers>");
			AllThreadContextsString += LINE_TERMINATOR;
			AllThreadContextsString += FString::Printf(TEXT("<ThreadID>%d</ThreadID>"), ThreadId);
			AllThreadContextsString += LINE_TERMINATOR;
			AllThreadContextsString += TEXT("<ThreadName>");
			AppendEscapedXMLString(AllThreadContextsString, ThreadName);
			AllThreadContextsString += TEXT("</ThreadName>");
			AllThreadContextsString += LINE_TERMINATOR;
			AllThreadContextsString += TEXT("</Thread>");
			AllThreadContextsString += LINE_TERMINATOR;
	...
	GUnixStackWalkNoCrashOnTimeout = false;
#endif
}

bool FUnixCrashContext::GetPlatformAllThreadContextsString(FString& OutStr) const
{
	OutStr = AllThreadContextsString;
	AllThreadContextsString.Empty();
...
void FUnixCrashContext::GenerateCrashInfoAndLaunchReporter() const
{
	...
	if (!bSkipCRC)
	{
		...
		if (IFileManager::Get().MakeDirectory(*CrashInfoAbsolute, true))
		{
			...
			DumpAdditionalContext(*CrashInfoFolder, Params);
			...
			if (!bLowDriveSpace)
			{
				...
				else
				{
					...
					{
						FFileHelper::SaveStringToFile(FString("Log not available, too large for submission to crash reporting backend"), *LogDstAbsolute);
					}
				}
			}
			else if (TotalDiskFreeSpace >= MaxFileSizeForCrashLog)
			{
				FFileHelper::SaveStringToFile(FString("Log not available, server has low available disk space"), *LogDstAbsolute);
			}
#endif // !NO_LOGGING
			...
			FString CrashReportClientArguments = " -Abslog=";
			...
			if (bReportingNonCrash && bStartCRCFromEngineHandler)
			{
				...
				while (!bFoundEmptySlot)
				{
					...
					if (!bFoundEmptySlot)
					{
						...
						if (IsRunningDedicatedServer())
						{
							UE_LOGF(LogCore, Warning, "Too many reports already in progress, skipping upload of this one.");
							bFoundEmptySlot = true;
						}
						else
						{
							...
							if (kTimeOutTimer >= kEnsureTimeOut)
							{
								...
								{
									UE_LOGF(LogCore, Warning, "Terminated CrashReport process[0]");
		...
		{
			UE_LOGF(LogCore, Warning, "MakeDirectory %ls failed", *CrashInfoAbsolute);
		}
	}
...
void DefaultCrashHandler(const FUnixCrashContext & Context)
{
	...
	const_cast<FUnixCrashContext&>(Context).CaptureAllThreadContexts();
	if (GLog)
	{
		GLog->Panic();
	}
