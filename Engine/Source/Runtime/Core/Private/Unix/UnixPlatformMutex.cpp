...
FUnixSystemWideMutex::FUnixSystemWideMutex(const FString& InName, FTimespan InTimeout)
{
	...
	FGenericPlatformMisc::SanitizeSystemWideMutexName(SanitizedName);
	const FString LockPath = FString(FPlatformProcess::ApplicationSettingsDir()) / SanitizedName;
	FileHandle = -1;
	...
	while (true)
	{
		if (FileHandle == -1)
		{
			...
			FileHandle = open(TCHAR_TO_UTF8(*LockPath), O_CREAT | O_WRONLY | O_NONBLOCK, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
		}
