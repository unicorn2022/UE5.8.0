...
namespace
{
	...
	// Wrapper around UE_LOG to avoid infinite logging if the act of logging causes an error
	#define UE_LOGF_UNIX_FILE(Verbosity, Format, ...) \
	{ \
		if (!bLoggingError) \
		{ \
			bLoggingError = true;  \
			UE_LOGF(LogUnixPlatformFile, Verbosity, Format, ##__VA_ARGS__); \
			bLoggingError = false; \
		} \
	}
}
...
public:
	...
	virtual bool ReadAt(uint8* Destination, int64 BytesToRead, int64 Offset) override
	{
		...
		do
		{
			...
			ssize_t BytesRead = pread(FileHandle, Destination, BytesToRead32, Offset);

			if (BytesRead == -1 && errno == EFAULT)
			{
				// Hacky workaround @see ReadInternal
				void* TempDest = FMemory::Malloc(BytesToRead32);
				if (TempDest)
				{
					BytesRead = pread(FileHandle, TempDest, BytesToRead32, Offset);
					if (BytesRead > 0 && BytesRead <= BytesToRead32)
					{
...
private:
	...
	int64 ReadInternal(uint8* Destination, int64 BytesToRead)
	{
		...
		while (BytesToRead)
		{
			...
			if (ThisRead == -1 && errno == EINTR)   // need to retry
			{
				continue;
			}			
			
			if (ThisRead == -1)  // some other error just bail
			{
				return BytesRead;
			}
 ...
 */
 ...
 TRACE_DECLARE_INT_COUNTER(FUnixFileMapper_UnnecessaryFileRemapping, TEXT("FUnixFileMapper/Unnecessary File Remappings"));
 TRACE_DECLARE_INT_COUNTER(FUnixFileMapper_NecessaryFileRemapping, TEXT("FUnixFileMapper/Necessary File Remappings"));
...
public:
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FUnixFileMapper::GetPathComponent);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FUnixFileMapper::MapFileRecursively);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(FUnixFileMapper::MapCaseInsensitiveFile);

		// Have to be careful logging in this function since this is called for setting up the log file
		...
		ExistingFilename = AutoRTFM::Open([this, &PossiblyWrongFilename, &bFound]
		{
			...
			FString FoundFilename("/");	// Start from the root.
		...
		if (bFound && !PossiblyWrongFilename.Equals(ExistingFilename, ESearchCase::CaseSensitive) && !ExistingFilename.IsEmpty())
		{
			// Can't just log directly here.  Use LowLevelOutputDebugString if logging isn't hooked up.
			if (GLog && FPlatformOutputDevices::GetLog() && GLog->IsRedirectingTo(FPlatformOutputDevices::GetLog()))
			{
				UE_LOGF(LogUnixPlatformFile, Log, "FUnixFileMapper::MapCaseInsensitiveFile: Lookup:%ls Found:%ls\n", *PossiblyWrongFilename, *ExistingFilename);
			}
			else
			{
				FPlatformMisc::LowLevelOutputDebugStringf(TEXT("FUnixFileMapper::MapCaseInsensitiveFile: Lookup:%s Found:%s\n"), *PossiblyWrongFilename, *ExistingFilename);
			}
			TRACE_COUNTER_ADD(FUnixFileMapper_NecessaryFileRemapping, 1);
		}
		else
		{
			TRACE_COUNTER_ADD(FUnixFileMapper_UnnecessaryFileRemapping, 1);
		}
		return bFound;
	}
	...
	int32 OpenCaseInsensitiveRead(const FString & Filename, FString & MappedToFilename)
	{
		...
		else
		{
			...
			if (ENOENT != ErrNo)
			{
				UE_LOGF(LogUnixPlatformFile, Warning, "open('%ls', O_RDONLY | O_CLOEXEC) failed: errno=%d (%ls)", *Filename, ErrNo, UTF8_TO_TCHAR(strerror(ErrNo)));
			}
			else
			{
...
#endif
				TRACE_CPUPROFILER_EVENT_SCOPE(FUnixFileMapper::OpenCaseInsensitiveRead);
				...
				if (MaxPathComponents > 0)
				{
					FString FoundFilename("/");	// start with root
					if (MapFileRecursively(Filename, 0, MaxPathComponents, FoundFilename))
					{
						...
						if (Handle != -1)
						{
							...
							if (!Filename.Equals(MappedToFilename, ESearchCase::CaseSensitive))
							{
								UE_LOGF(LogUnixPlatformFile, Log, "FUnixFileMapper::OpenCaseInsensitiveRead: '%ls' to '%ls'", *Filename, *MappedToFilename);
								TRACE_COUNTER_ADD(FUnixFileMapper_NecessaryFileRemapping, 1);
							}
						}
					}
...
public:
	...
	virtual IMappedFileRegion* MapRegion(int64 Offset = 0, int64 BytesToMap = MAX_int64, FFileMappingFlags Flags = EMappedFileFlags::ENone) override
	{
		...
		{
			UE_LOGF(LogUnixPlatformFile, Warning, "Failed to map memory %ls, error is %d", *Filename, errno);
			return nullptr;
		}
	...
	void UnMap(const FUnixMappedFileRegion* Region)
	{
		...
		checkf(Res == 0, TEXT("Failed to unmap, error is %d, errno is %d [params: %llx, %lld]"), Res, errno, MappedPtr, GetFileSize());
	}
...
FOpenMappedResult FUnixPlatformFile::OpenMappedEx2(const TCHAR* Filename, EOpenMappedFlags OpenOptions, int64 MaximumSize)
{
	...
	if (EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::OpenForWrite))
	{
		Handle = open(TCHAR_TO_UTF8(*NormalizedFilename), Flags, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
	}
	else
	{
		Handle = open(TCHAR_TO_UTF8(*NormalizedFilename), Flags);
	}
	...
	if (EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::OpenForWrite))
	{
		...
		{
			int TruncResult = 0;
			do { TruncResult = ftruncate(Handle, MaximumSize); } while (TruncResult < 0 && errno == EINTR);

			if (TruncResult == -1)
			{
				int ErrNo = errno;
				FString ErrorStr = FString::Printf(TEXT("ftruncate() failed for '%s': errno=%d (%s)"), *NormalizedFilename, ErrNo, UTF8_TO_TCHAR(strerror(ErrNo)));
				UE_LOG_UNIX_FILE(Warning, TEXT("%s"), *ErrorStr);
	...
	return MakeValue(new FUnixMappedFileHandle(Handle, MaximumSize, NormalizedFilename));
}
...
bool FUnixPlatformFile::DeleteDirectory(const TCHAR* Directory)
{
	...
	if (IntendedFilename != CaseSensitiveFilename)
	{
		UE_LOGF(LogUnixPlatformFile, Warning, "Could not find directory '%ls', deleting '%ls' instead (for consistency with the rest of file ops)", *IntendedFilename, *CaseSensitiveFilename);
	}
...
bool FUnixPlatformFile::IterateDirectory(const TCHAR* Directory, FDirectoryVisitor& Visitor)
{
	...
	return IterateDirectoryCommon(Directory, [&](struct dirent* InEntry) -> bool
	{
		...
		else
		{
			...
			{
				int ErrNo = errno;
				UE_LOGF(LogUnixPlatformFile, Warning, "Cannot determine whether '%ls' is a directory - d_type not supported and stat() failed with errno=%d (%ls)", *AbsoluteUnicodeName, ErrNo, UTF8_TO_TCHAR(strerror(ErrNo)));
			}
		}
	...
	for (int32 i=0; i<Len; ++i)
	{
		...
		if (SubPath[i] == '/')
		{
			...
			if (mkdir(SubPath, 0775) == -1)
			{
				...
				UE_LOGF_UNIX_FILE(Warning, "create dir('%s') failed: errno=%d (%s)", SubPath, ErrNo, strerror(ErrNo));
...
bool FUnixFileRegistry::PlatformReopenFile(FRegisteredFileHandle* Handle)
{
	...
	if(UnixHandle->FileHandle != -1)
	{
		...
		{
			UE_LOGF(LogUnixPlatformFile, Warning, "Could not seek to the previous position on handle for file '%ls'", *(UnixHandle->Filename));
			bSuccess = false;
		}
	}
	else
	{
		UE_LOGF(LogUnixPlatformFile, Warning, "Could not reopen handle for file '%ls'", *(UnixHandle->Filename));
		bSuccess = false;
	}
