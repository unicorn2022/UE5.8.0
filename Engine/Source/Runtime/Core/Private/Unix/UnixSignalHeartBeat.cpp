...
namespace
{
	...
	void SignalHitchHandler(int Signal, siginfo_t* info, void* context)
	{
		...
		UE_LOGF(LogUnixHeartBeat, Verbose, "SignalHitchHandler");
		GHitchDetected = true;
#endif
	}
	...
	timespec TimerGetTime(timer_t TimerId)
	{
		...
		{
			int Errno = errno;
			UE_LOGF(LogUnixHeartBeat, Warning, "Failed to timer_gettime() errno=%d (%ls)", Errno, UTF8_TO_TCHAR(strerror(Errno)));
		}
...
void FUnixSignalGameHitchHeartBeat::Init()
{
	...
	if (!bActionCreated)
	{
		...
		UE_LOGF(LogUnixHeartBeat, Warning, "Failed to sigaction() errno=%d (%ls)", Errno, UTF8_TO_TCHAR(strerror(Errno)));
	}
	...
	{
		const int Errno = errno;
		UE_LOGF(LogUnixHeartBeat, Warning, "Failed to timer_create() errno=%d (%ls)", Errno, UTF8_TO_TCHAR(strerror(Errno)));
	}
...
void FUnixSignalGameHitchHeartBeat::InitSettings()
{
	...
	UE_LOGF(LogUnixHeartBeat, Verbose, "HitchThresholdS:%f", HitchThresholdS);
}
...
void FUnixSignalGameHitchHeartBeat::FrameStart(bool bSkipThisFrame)
{
	...
	UE_LOGF(LogUnixHeartBeat, VeryVerbose, "bDisabled:%ls SuspendCount:%d bTimerCreated:%ls TimerId:%d timer:%d",
	...
	{
		UE_LOGF(LogUnixHeartBeat, VeryVerbose, "HitchThresholdS:%f MinimalHitchThreashold:%f",
		...
		if (HitchThresholdS > MinimalHitchThreashold)
		{
			...
			if (GHitchDetected)
			{
				UE_LOGF(LogCore, Error, "Hitch detected on previous gamethread frame (%8.2fms since last frame)", float(FPlatformTime::Seconds() - StartTime) * 1000.0f);
			}
			...
			{
				int Errno = errno;
				UE_LOGF(LogUnixHeartBeat, Warning, "Failed to timer_settime() errno=%d (%ls)", Errno, UTF8_TO_TCHAR(strerror(Errno)));
			}
		}
	}
...
void FUnixSignalGameHitchHeartBeat::SuspendHeartBeat()
{
	...
	SuspendCount++;
	UE_LOGF(LogUnixHeartBeat, Verbose, "SuspendCount:%d", SuspendCount);
...
{
	UE_LOGF(LogUnixHeartBeat, Verbose, "Restart");
	bDisabled = false;
...
{
	UE_LOGF(LogUnixHeartBeat, Verbose, "Stop");
	SuspendHeartBeat();
	bDisabled = true;
}
...
{
	UE_LOGF(LogUnixHeartBeat, Verbose, "PostFork");
