...
void FRunnableThreadUnix::PreRun()
{
	...
	if (ErrCode != 0)
	{
		UE_LOGF(LogHAL, Warning, "pthread_setname_np(, '%ls') failed with error %d (%ls).", *ThreadName, ErrCode, ANSI_TO_TCHAR(strerror(ErrCode)));
	}
...
void FRunnableThreadUnix::PostRun()
{
	if (StackGuardPageAddress != nullptr)
	{
		...
		{
			UE_LOGF(LogCore, Error, "Unable to remove a guard page from the alt stack");
		}
