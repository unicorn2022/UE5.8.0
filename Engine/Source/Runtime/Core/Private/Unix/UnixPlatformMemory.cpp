...
#ifdef UE4_DO_ROOT_PRIVILEGE_CHECK
	// UE_DEPRECATED(5.8)
	#warning "UE4_DO_ROOT_PRIVILEGE_CHECK has been renamed UE_DO_ROOT_PRIVILEGE_CHECK"
	#define UE_DO_ROOT_PRIVILEGE_CHECK UE4_DO_ROOT_PRIVILEGE_CHECK
#endif
// do not do a root privilege check on non-x86-64 platforms (assume an embedded device)
#ifndef UE_DO_ROOT_PRIVILEGE_CHECK
  #if defined(_M_X64) || defined(__x86_64__) || defined (__amd64__)
#define UE_DO_ROOT_PRIVILEGE_CHECK	 1
  #else
    #define UE_DO_ROOT_PRIVILEGE_CHECK	 0
  #endif // defined(_M_X64) || defined(__x86_64__) || defined (__amd64__) 
#endif // ifndef UE_DO_ROOT_PRIVILEGE_CHECK
...
namespace
{
	...
	std::atomic<uint64> GSharedMemoryCOWBytes = 0;
}
...
void FUnixPlatformMemory::Init()
{
	...
	UE_LOGF(LogInit, Log, " - Physical RAM available (not considering process quota): %d GB (%lu MB, %lu KB, %lu bytes)", 
	...
	UE_LOGF(LogInit, Log, " - VirtualMemoryAllocator pools will grow at scale %g", GVMAPoolScale);
	UE_LOGF(LogInit, Log, " - MemoryRangeDecommit() will %ls", 
	...
	UE_LOGF(LogInit, Log, " - PageSize %zu", MemoryConstants.PageSize);
	UE_LOGF(LogInit, Log, " - BinnedPageSize %zu", MemoryConstants.BinnedPageSize);
}
...
class FMalloc* FUnixPlatformMemory::BaseAllocator()
{
...
#if UE_DO_ROOT_PRIVILEGE_CHECK && !IS_PROGRAM
...
#endif // UE_DO_ROOT_PRIVILEGE_CHECK && !IS_PROGRAM
	...
	if (FILE* CmdLineFile = fopen("/proc/self/cmdline", "r"))
	{
		...
		while(getdelim(&Arg, &Size, 0, CmdLineFile) != -1)
		{
			...
			if (FCStringAnsi::Stricmp(Arg, "-binnedmalloc3") == 0)
			{
				AllocatorToUse = EMemoryAllocatorToUse::Binned3;
				break;
			}

			if (FCStringAnsi::Stricmp(Arg, "-binnedmalloc4") == 0)
			{
				AllocatorToUse = EMemoryAllocatorToUse::Binned4;
				break;
			}
	...
	case EMemoryAllocatorToUse::Binned3:
		Allocator = new FMallocBinned3();
		break;

	case EMemoryAllocatorToUse::Binned4:
		Allocator = new FMallocBinned4();
		break;
...
static void MarkMappedMemoryMergable(void* Pointer, SIZE_T Size)
{
	...
	if (GUseKSM && (GKSMMergeAllPages || Size > BinnedPageSize))
	{
		...
		{
			int ErrNo = errno;
			UE_LOGF(LogHAL, Fatal, "madvise(addr=%p, length=%zu, advice=MADV_MERGEABLE) failed with errno = %d (%ls)",
...
#if (UE_PLATFORM_SANITY_CHECK_OS_ALLOCATIONS != 0)
...
FUnixPlatformMemory::FPlatformVirtualMemoryBlock FUnixPlatformMemory::FPlatformVirtualMemoryBlock::AllocateVirtual(size_t InSize, size_t InAlignment)
{
	...
	Result.Ptr = mmap(nullptr, Result.GetActualSize(), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_NORESERVE, -1, 0);
...
static uint32 ReadProcFields(const char *FileName, FProcField *ProcFields, uint32 NumFields)
{
	...
	if (Fd >= 0)
	{
		...
		for (;;)
		{
			...
			checkf(BytesParsed <= BytesAvailableInChunk, TEXT("BytesParsed more than BytesAvailableInChunk %llu %llu"), BytesParsed, BytesAvailableInChunk);
...
bool FUnixPlatformMemory::GetForkedPageAllocationInfo(TArray<FForkedPageAllocation>& OutPages)
{
	...
	if (Fd < 0)
	{
		UE_LOGF(LogHAL, Warning, "Failed to open /proc/self/smaps for pages");
		return false;
	}
...
FExtendedPlatformMemoryStats FUnixPlatformMemory::GetExtendedStats()
{
...

	{
		...
		int Fd = open("/sys/kernel/mm/ksm/run", O_RDONLY);
		if (Fd != -1)
		{
			char Value = 0;
			read(Fd, &Value, 1);
	...
	MemoryStats.Shared_Memory_COW_Bytes = GSharedMemoryCOWBytes;
...
const FPlatformMemoryConstants& FUnixPlatformMemory::GetConstants()
{
	...
	if( MemoryConstants.TotalPhysical == 0 )
	{
		...
		if (0 == sysinfo(&SysInfo))
		{
			...
			MaxVirtualRAMBytes
				= MaxPhysicalRAMBytes
				+ static_cast< unsigned long long >( SysInfo.mem_unit ) * static_cast< unsigned long long >( SysInfo.totalswap );
		}
...
FPlatformMemory::FSharedMemoryRegion* FUnixPlatformMemory::MapNamedSharedMemoryRegion(const FString& InName, bool bCreate, uint32 AccessMode, SIZE_T Size)
{
	...
	{
		int ErrNo = errno;
		UE_LOGF(LogHAL, Warning, "shm_open(name='%ls', flags=0x%x, mode=0x%x) failed with errno = %d (%ls)", *Name, ShmOpenFlags, ShmOpenMode, ErrNo, 
	...
	if (bCreate)
	{
		...
		{
			int ErrNo = errno;
			UE_LOGF(LogHAL, Warning, "ftruncate(fd=%d, size=%zu) failed with errno = %d (%ls)", SharedMemoryFd, Size, ErrNo,
				StringCast< TCHAR >(strerror(ErrNo)).Get());
	...
	{
		int ErrNo = errno;
		UE_LOGF(LogHAL, Warning, "mmap(addr=NULL, length=%zu, prot=0x%x, flags=MAP_SHARED, fd=%d, 0) failed with errno = %d (%ls)", Size, MmapProtFlags, SharedMemoryFd, ErrNo,
			StringCast< TCHAR >(strerror(ErrNo)).Get());
...
bool FUnixPlatformMemory::UnmapNamedSharedMemoryRegion(FSharedMemoryRegion * MemoryRegion)
{
	...
	if (MemoryRegion)
	{
		...
		if (munmap(UnixRegion->GetAddress(), UnixRegion->GetSize()) == -1) 
		{
			...
			int ErrNo = errno;
			UE_LOGF(LogHAL, Warning, "munmap(addr=%p, len=%zu) failed with errno = %d (%ls)", UnixRegion->GetAddress(), UnixRegion->GetSize(), ErrNo,
				StringCast< TCHAR >(strerror(ErrNo)).Get());
		}
		...
		if (close(UnixRegion->GetFileDescriptor()) == -1)
		{
			...
			int ErrNo = errno;
			UE_LOGF(LogHAL, Warning, "close(fd=%d) failed with errno = %d (%ls)", UnixRegion->GetFileDescriptor(), ErrNo, 
				StringCast< TCHAR >(strerror(ErrNo)).Get());
		}
		...
		if (UnixRegion->NeedsToUnlinkRegion())
		{
			...
			if (shm_unlink(NameUTF8.Get()) == -1)
			{
				...
				int ErrNo = errno;
				UE_LOGF(LogHAL, Warning, "shm_unlink(name='%ls') failed with errno = %d (%ls)", UnixRegion->GetName(), ErrNo, 
					StringCast< TCHAR >(strerror(ErrNo)).Get());
			}
		}
...
void FUnixPlatformMemory::OnOutOfMemory(uint64 Size, uint32 Alignment)
{
	auto HandleOOM = [&]()
	{
		...
		UE_LOGF(LogMemory, Fatal, "Ran out of memory allocating %llu bytes with alignment %u. Last error msg: %ls.", Size, Alignment, ErrorMsg);
	};
...
void FUnixPlatformMemory::IncSharedMemoryCOWBytes(size_t Bytes)
{
	std::atomic_fetch_add(&GSharedMemoryCOWBytes, Bytes);
}
