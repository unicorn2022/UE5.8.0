...
#define UE_WITH_CONTAINER_META !UE_BUILD_SHIPPING

#if UE_WITH_CONTAINER_META
#include "HAL/PlatformFileManager.h"
#include "IO/IoContainerMeta.h"
#include "Misc/Paths.h"
#else
#include "Containers/Utf8String.h"
#endif // UE_WITH_CONTAINER_META
...
static const TCHAR* const GetIoErrorText_ErrorCodeTextArray[] =
{
	...
	TEXT("File Corrupt"),
	TEXT("Http Read Error"),
	TEXT("File Handle Open"),
	TEXT("Pending Delete")
};
...
FStringBuilderBase& operator<<(FStringBuilderBase& Sb, EIoContainerFlags Flags)
{
	...
	constexpr uint32 BitCount = 1 + FMath::CountTrailingZeros(
		static_cast<std::underlying_type_t<EIoContainerFlags>>(EIoContainerFlags::Last));
	static_assert(UE_ARRAY_COUNT(Names) == BitCount + 1, "Please update names list");

	for (int32 Idx = 0; Idx < BitCount; ++Idx)
	{
		const EIoContainerFlags FlagToTest = static_cast<EIoContainerFlags>(1 << Idx);
...
FString LexToString(EIoContainerFlags Flags)
{
	TStringBuilder<128> Sb;
	return (Sb << Flags).ToString();
}
...
#if UE_WITH_CONTAINER_META
class FIoMountedContainerMeta
{
	struct FContainerMeta
	{
		FContainerMeta(FIoContainerMetaReader&& InReader, FString&& InFilePath)
			: Reader(MoveTemp(InReader))
			, FilePath(MoveTemp(InFilePath))
		{ }

		FIoContainerMetaReader	Reader;
		FString					FilePath;
	};

public:
	void			Mount(const FString& FilePath);
	void			Unmount(const FString& FilePath);
	FUtf8StringView	GetFilename(const FIoChunkId& ChunkId, FUtf8StringBuilderBase& OutFilename, FUtf8StringView& OutContainerName) const;

private:
	mutable UE::FMutex					Mutex;
	TArray<TUniquePtr<FContainerMeta>>	Meta;
};

void FIoMountedContainerMeta::Mount(const FString& FilePath)
{
	LLM_SCOPE_BYNAME(TEXT("FileSystem/Debug"));

	IPlatformFile& Ipf = FPlatformFileManager::Get().GetPlatformFile();

	FString MetaPath = FPaths::ChangeExtension(FilePath, FIoContainerMetaHeader::FileExtension);
	if (Ipf.FileExists(*MetaPath) == false)
	{
		return;
	}
	...
	{
		UE_LOGF(LogIoDispatcher, Warning, "Failed to load container metadata '%ls', reason: %ls", *MetaPath, *MaybeReader.Status().ToString());
		return;
	}

	UE_LOGF(LogIoDispatcher, Log, "Mounted container metadata '%ls'", *MetaPath);
	{
		UE::TUniqueLock Lock(Mutex);
		Meta.Add(MakeUnique<FContainerMeta>(MaybeReader.ConsumeValueOrDie(), MoveTemp(MetaPath)));
	}
}

FUtf8StringView FIoMountedContainerMeta::GetFilename(const FIoChunkId& ChunkId, FUtf8StringBuilderBase& OutFilename, FUtf8StringView& OutContainerName) const
{
	FUtf8StringView Result;
	UE::TUniqueLock Lock(Mutex);
	for (const TUniquePtr<FContainerMeta>& ContainerMeta : Meta)
	{
		if (Result = ContainerMeta->Reader.GetFilename(ChunkId, OutFilename, OutContainerName); !Result.IsEmpty())
		{
			return Result;
		}
	}
...
public:
	void			Mount(const FString& FilePath) { }
	void			Unmount(const FString& FilePath) { }
	FUtf8StringView	GetFilename(const FIoChunkId& ChunkId, FUtf8StringBuilderBase& OutFilename, FUtf8StringView& OutContainerName) const { return FUtf8StringView(); }
};
#endif // UE_WITH_CONTAINER_META
...
public:
	...
	{
		TIoStatusOr<FIoMappedRegion> OutResult{FIoStatus(EIoErrorCode::NotFound)};
		...
		if (ChunkId.IsValid())
		{
			...
			for (const FBackendAndPriority& Backend : Backends)
			{
				...
				if (Result.Status().GetErrorCode() != EIoErrorCode::NotFound)
				{
					if (OutResult.Status().GetErrorCode() == EIoErrorCode::NotFound ||
						OutResult.Status().GetErrorCode() == EIoErrorCode::UnknownChunkID)
					{
						OutResult = MoveTemp(Result);
					}
				}
		...
		{
			OutResult = FIoStatus(EIoErrorCode::InvalidParameter, TEXT("FIoChunkId is not valid"));
		}
	...
	void MountContainerMeta(const FString& FilePath)
	{
		ContainerMeta.Mount(FilePath);
	}

	FUtf8StringView	GetFilename(const FIoChunkId& ChunkId, FUtf8StringBuilderBase& OutFilename, FUtf8StringView& OutContainerName) const
	{
		return ContainerMeta.GetFilename(ChunkId, OutFilename, OutContainerName);
	}
	...
	{
		return TotalLoaded.load(std::memory_order_relaxed);
	}
...
private:
	...
	void ProcessCompletedRequests()
	{
		...
		for (const FBackendAndPriority& Backend : Backends)
		{
			...
			while (CompletedRequestsHead)
			{
				...
				{
					UE_CLOGF(!CompletedRequestsHead->HasBuffer(), LogStreaming, Fatal, "Backend provided a completed request without an IoBuiffer. Requests that are not failed or cancelled must have an IoBuffer");
					TotalLoaded.fetch_add(CompletedRequestsHead->GetBuffer().DataSize(), std::memory_order_relaxed);
	...
	FIoMountedContainerMeta ContainerMeta;
	...
	std::atomic<int64> TotalLoaded = 0;
...
void FIoDispatcher::MountContainerMeta(const FString& FilePath)
{
	Impl->MountContainerMeta(FilePath);
}

FUtf8StringView	FIoDispatcher::GetFilename(const FIoChunkId& ChunkId, FUtf8StringBuilderBase& OutFilename, FUtf8StringView& OutContainerName) const
{
	return Impl->GetFilename(ChunkId, OutFilename, OutContainerName);
}
...
FIoRequest::GetResultOrDie() const
{
	...
	if (!Result)
	{
		if (Impl)
		{
			UE_LOGF(LogIoDispatcher, Fatal, "I/O Error '%ls' (IoChunkId='%ls', Type=%ls)", *Status().ToString(), *LexToString(Impl->ChunkId), *LexToString(Impl->ChunkId.GetChunkType()));
		}
		else
		{
			UE_LOGF(LogIoDispatcher, Fatal, "I/O Error '%ls'", *Status().ToString());
		}
	}
	return *Result;
}
...
bool
FIoDispatcherInternal::HasPackageData()
{
...
void
FIoDispatcherInternal::MountContainerMeta(const FString& FilePath)
{
	FIoDispatcher::Get().MountContainerMeta(FilePath);
}

FUtf8StringView
FIoDispatcherInternal::GetFilename(const FIoChunkId& ChunkId, FUtf8StringBuilderBase& OutFilename, FUtf8StringView& OutContainerName)
{
	return FIoDispatcher::Get().GetFilename(ChunkId, OutFilename, OutContainerName);
}

void
FIoDispatcherInternal::GetDebugInfo(const FIoRequest& IoRequest, FIoRequestDebugInfo& OutDebugInfo)
{
	if (IoRequest.Impl && IoRequest.Impl->Backend)
	{
		OutDebugInfo.BackendName = IoRequest.Impl->Backend->GetName();
	}
}
