...
class FIoStoreTocReader : public IIoStoreTocReader
{
public:
	FIoStoreTocReader()
	{
	...
	virtual ~FIoStoreTocReader() = default;
...
IIoStoreTocReader::~IIoStoreTocReader() = default;

/* static */ TIoStatusOr<TUniquePtr<IIoStoreTocReader>> IIoStoreTocReader::ReadFromDisk(const TCHAR* TocFilePath, const TMap<FGuid, FAES::FAESKey>& DecryptionKeys)
{
	TUniquePtr<IIoStoreTocReader> TocReader = MakeUnique<FIoStoreTocReader>();
	FIoStatus Result = ((FIoStoreTocReader*)TocReader.Get())->Read(TocFilePath, DecryptionKeys);
	if (Result != FIoStatus::Ok)
	{
		return Result;
	}
	return TocReader;
...
static FIoStoreTocChunkInfo GetTocChunkInfoInternal(const T& Toc, int32 TocEntryIndex)
{
	...
	for (int32 BlockIndex = FirstBlockIndex; BlockIndex <= LastBlockIndex; ++BlockIndex)
	{
		...
		ChunkInfo.SizeOnDisk += Align(CompressionBlock.GetCompressedSize(), FAES::AESBlockSize);
...
public:
	...
	[[nodiscard]] FIoStatus Initialize(FStringView InContainerPath, const TMap<FGuid, FAES::FAESKey>& InDecryptionKeys)
	{
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(FIoStoreReader::ReadToc);
		...
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(FIoStoreReader::GenerateContainerFileAccessors);
	...
	{
		return TocReader.GetTocResource().Header.PartitionSize;
	}
...
uint64 FIoStoreReader::GetPartitionSize() const
{
	return Impl->GetPartitionSize();
}
...
FIoStoreTocResourceView FIoStoreTocResource::AsView() const
{
	FIoStoreTocResourceView Result = {};
	Result.Header = Header;
	Result.ChunkIds = ChunkIds;
	Result.ChunkOffsetLengths = ChunkOffsetLengths;
	Result.ChunkPerfectHashSeeds = ChunkPerfectHashSeeds;
	Result.ChunkIndicesWithoutPerfectHash = ChunkIndicesWithoutPerfectHash;
	Result.CompressionBlocks = CompressionBlocks;
	Result.CompressionMethods = CompressionMethods;
	Result.SignatureHash = SignatureHash;
	Result.ChunkBlockSignatures = ChunkBlockSignatures;
	Result.DirectoryIndexBuffer = DirectoryIndexBuffer;
	Result.ChunkMetas = ChunkMetas;
	Result.LegacyChunkMetas = {};
	return Result;
}
