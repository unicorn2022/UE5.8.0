...
namespace UE::Sanitizer::RaceDetector {
	...
	extern TRefCountPtr<FContext> GetContextByThreadId(uint32 InThreadId);
	extern volatile bool bIsResettingShadow;
	extern thread_local bool bIsResettingShadowThread;
	extern void WriteToLog(FString&& Message);
	extern bool bRuntimeInitialized;
	constexpr int32 MinStackSize = 256 * 1024;
}

// Sentinel used to validate our TLS walking assumptions at runtime.
static __declspec(thread) volatile uint8 TlsValidationSentinel = 0;

// Tri-state: 0 = not yet checked, 1 = validated OK, -1 = validation failed.
static volatile int8 TlsCleanupState = 0;

// Walks every loaded module's PE TLS directory and invokes the callback with the
// base address and size of each per-thread TLS block. No API calls are made —
// everything is read directly from TEB/PEB/PE structures to avoid reentrancy
// through detoured functions.
// Returns false if the TEB or PEB structures couldn't be read.
template<typename CallbackType>
INSTRUMENTATION_FUNCTION_ATTRIBUTES static bool ForEachTlsRegion(CallbackType&& Callback)
{
	// TEB offset 0x58 (x64) is ThreadLocalStoragePointer
	void** TlsArray = *(void***)((BYTE*)NtCurrentTeb() + 0x58);
	if (!TlsArray)
	{
		return false;
	}
	...
	for (LIST_ENTRY* Entry = Head->Flink; Entry != Head; Entry = Entry->Flink)
	{
		...
		BYTE* DllBase = (BYTE*)LdrEntry->DllBase;
		if (!DllBase) 
		{
			continue;
		}
		...
		IMAGE_NT_HEADERS64* NtHeaders = (IMAGE_NT_HEADERS64*)(DllBase + DosHeader->e_lfanew);
		if (NtHeaders->Signature != IMAGE_NT_SIGNATURE) 
		{
			continue;
		}

		IMAGE_DATA_DIRECTORY& TlsDataDir = NtHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS];
		if (TlsDataDir.VirtualAddress == 0) 
		{
			continue;
		}

		IMAGE_TLS_DIRECTORY* TlsDir = (IMAGE_TLS_DIRECTORY*)(DllBase + TlsDataDir.VirtualAddress);
		DWORD TlsIndex = *(DWORD*)TlsDir->AddressOfIndex;
		SIZE_T TlsSize = TlsDir->EndAddressOfRawData - TlsDir->StartAddressOfRawData;
		void* TlsBase = TlsArray[TlsIndex];

		if (TlsBase && TlsSize > 0)
		{
			Callback(TlsBase, TlsSize);
		}
	}
...
// Validates that ForEachTlsRegion produces correct results by checking that
// a known __declspec(thread) variable falls within one of the discovered regions.
INSTRUMENTATION_FUNCTION_ATTRIBUTES static bool ValidateTlsCleanupAssumptions()
{
	const BYTE* SentinelAddr = (const BYTE*)&TlsValidationSentinel;
	bool bFound = false;

	bool bSuccess = ForEachTlsRegion([&](void* TlsBase, SIZE_T TlsSize)
	{
		const BYTE* Base = (const BYTE*)TlsBase;
		if (SentinelAddr >= Base && SentinelAddr < Base + TlsSize)
		{
			bFound = true;
		}
	});

	return bSuccess && bFound;
}

// Ensures TLS cleanup has been validated. Called lazily on the first thread
// shutdown so that all modules are loaded and TLS is fully initialized.
INSTRUMENTATION_FUNCTION_ATTRIBUTES static bool EnsureTlsCleanupValidated()
{
	if (TlsCleanupState == 0)
	{
		if (ValidateTlsCleanupAssumptions())
		{
			TlsCleanupState = 1;
		}
		else
		{
			TlsCleanupState = -1;
			UE_LOGF(LogRaceDetector, Warning,
				"TLS shadow cleanup disabled: could not validate TEB/PE TLS assumptions on this OS version. " "TLS-related false positives may occur.");
		}
	}
	return TlsCleanupState > 0;
}
...
INSTRUMENTATION_FUNCTION_ATTRIBUTES void InstrumentEndThread()
{
	...
	// Clean TLS to avoid false positives when memory gets reused for another thread.
	// Validated lazily on the first thread shutdown to ensure all modules are loaded.
	if (EnsureTlsCleanupValidated())
	{
		ForEachTlsRegion([](void* TlsBase, SIZE_T TlsSize)
		{
			UE::Sanitizer::RaceDetector::FreeMemoryRange(TlsBase, TlsSize);
		});
	}
...
INSTRUMENTATION_FUNCTION_ATTRIBUTES DWORD WINAPI InstrumentedStartThread(LPVOID Param)
{
	...
	Context.ThreadArgs = ThreadArgs;
	{
		...
		// Do not try to sync if we reach this before the handle is set from the creating thread
		// it means that CREATE_SUSPENDED was not passed as a flag anyway so we don't need any barrier in that case.
		if (ThreadArgs->ThreadHandle && bRuntimeInitialized)
		{
			// This allows us to sync with ResumeThread if the thread was created in the suspended state.
			FSyncObjectRef Sync = GetSyncObject(Context, ThreadArgs->ThreadHandle);
			Sync->SyncAcquireAsSoleOwnerOrReadOwner(Context, _ReturnAddress(), ThreadArgs->ThreadHandle, TEXT("InstrumentedStartThread"));
		}
...
// NtUnmapViewOfSectionEx is the low-level function used by both UnmapViewOfFile and the DLL
// loader (FreeLibrary -> LdrUnloadDll). When virtual memory is unmapped, the address range
// returns to the OS and may be reused for new allocations. Without shadow cleanup, stale
// entries from the old mapping cause false positive race reports.
// See UE-368317: ReadLibraryImportsFromMemory maps DLLs via MapViewOfFile to parse imports,
// then unmaps them. The shadow entries from those reads trip when the VA is reused.
typedef NTSTATUS(NTAPI NTUNMAPVIEWOFSECTIONEX)(HANDLE ProcessHandle, PVOID BaseAddress, ULONG Flags);
NTUNMAPVIEWOFSECTIONEX* TrueNtUnmapViewOfSectionEx = nullptr;
INSTRUMENTATION_FUNCTION_ATTRIBUTES NTSTATUS NTAPI DetouredNtUnmapViewOfSectionEx(HANDLE ProcessHandle, PVOID BaseAddress, ULONG Flags)
{
	// Only clean shadow when unmapping from the current process.
	if (BaseAddress && ProcessHandle == GetCurrentProcess())
	{
		START_WINAPI_INSTRUMENTATION(return TrueNtUnmapViewOfSectionEx(ProcessHandle, BaseAddress, Flags));

		MEMORY_BASIC_INFORMATION MemInfo;
		const uint8* Addr = (const uint8*)BaseAddress;
		while (VirtualQuery(Addr, &MemInfo, sizeof(MemInfo)) && MemInfo.AllocationBase == BaseAddress)
		{
			if (MemInfo.State == MEM_COMMIT)
			{
				FreeMemoryRange((void*)Addr, MemInfo.RegionSize);
			}
			Addr += MemInfo.RegionSize;
		}

		NTSTATUS Result = TrueNtUnmapViewOfSectionEx(ProcessHandle, BaseAddress, Flags);

		FINISH_WINAPI_INSTRUMENTATION();
		return Result;
	}

	return TrueNtUnmapViewOfSectionEx(ProcessHandle, BaseAddress, Flags);
}
...
typedef NTSYSAPI DWORD(NTAPI SUSPENDTHREAD)(IN HANDLE ThreadHandle);

SUSPENDTHREAD* TrueSuspendThread = nullptr;
INSTRUMENTATION_FUNCTION_ATTRIBUTES DWORD NTAPI DetouredSuspendThread(HANDLE ThreadHandle)
{
	START_WINAPI_INSTRUMENTATION(return TrueSuspendThread(ThreadHandle));

	// Suspend the thread first, then acquire its clock state. Once SuspendThread returns
	// successfully, the target thread is stopped and all its prior memory accesses are
	// visible. This is needed for callstack capture patterns where thread A suspends
	// thread B, reads B's context/stack, then resumes B.
	DWORD result = TrueSuspendThread(ThreadHandle);
	if (result != (DWORD)-1)
	{
		DWORD ThreadId = ::GetThreadId(ThreadHandle);
		TRefCountPtr<FContext> SuspendedThreadContext = GetContextByThreadId(ThreadId);
		if (SuspendedThreadContext)
		{
			Context.ClockBank.Acquire(SuspendedThreadContext->ClockBank, _ReturnAddress());
		}
	}
...
typedef NTSYSAPI DWORD(NTAPI RESUMETHREAD)(IN HANDLE ThreadHandle);

RESUMETHREAD* TrueResumeThread = nullptr;
INSTRUMENTATION_FUNCTION_ATTRIBUTES DWORD NTAPI DetouredResumeThread(HANDLE ThreadHandle)
{
	START_WINAPI_INSTRUMENTATION(return TrueResumeThread(ThreadHandle));

	// Add a release barrier when thread is resumed to avoid false positive that could happen
	// if something is written between the createthread and the resumethread and then 
	// read by the thread once it is resumed.
	FSyncObjectRef Sync = GetSyncObject(Context, ThreadHandle);
	Sync->SyncReleaseAsSoleOwner(Context, _ReturnAddress(), ThreadHandle, TEXT("ResumeThread"));

	// This handle synchronization of SuspendThread/ResumeThread pairs (e.g. callstack capture)
	// once the thread is already started.
	DWORD ThreadId = ::GetThreadId(ThreadHandle);
	TRefCountPtr<FContext> SuspendedThreadContext = GetContextByThreadId(ThreadId);
	if (SuspendedThreadContext)
	{
		SuspendedThreadContext->ClockBank.Acquire(Context.ClockBank, _ReturnAddress());
	}
	DWORD result = TrueResumeThread(ThreadHandle);
...
INSTRUMENTATION_FUNCTION_ATTRIBUTES void PopulateDetouredFunctions()
{
	...
	TrueSuspendThread = ::SuspendThread;
	DetouredFunctions.Emplace(&(PVOID&)TrueSuspendThread, DetouredSuspendThread);
	TrueResumeThread = ::ResumeThread;
	DetouredFunctions.Emplace(&(PVOID&)TrueResumeThread, DetouredResumeThread);
	...
	// Hook NtUnmapViewOfSectionEx to clean up shadow state when memory-mapped views or
	// DLL image sections are unmapped. Covers both UnmapViewOfFile and FreeLibrary paths.
	TrueNtUnmapViewOfSectionEx = (NTUNMAPVIEWOFSECTIONEX*)GetProcAddress(GetModuleHandleA("ntdll"), "NtUnmapViewOfSectionEx");
	if (TrueNtUnmapViewOfSectionEx)
	{
		DetouredFunctions.Emplace(&(PVOID&)TrueNtUnmapViewOfSectionEx, DetouredNtUnmapViewOfSectionEx);
	}
}
...
namespace UE::Sanitizer::RaceDetector::Platform {
	...
	INSTRUMENTATION_FUNCTION_ATTRIBUTES void PrepareTrampoline(void* ThunkFunctionAddress, void* DestFunctionAddress, bool bUseRETBypass)
	{
		...
		for (int32 Index = 0; Index < INSTRUMENTATION_HOTPATCH_PREFIX_NOPS; ++Index)
		{
			...
			if (PrefixStart[Index] != 0x90)
			{
				...
				UE_LOGF(LogRaceDetector, Fatal, "The function at address %p doesn't have a patchable prefix or was already patched by another system", ThunkFunctionAddress);
				return;
			}
		}
		...
		{
			UE_LOGF(LogRaceDetector, Fatal, "The function at address %p doesn't have a patchable entry or was already patched by another system", ThunkFunctionAddress);
			return;
		}
		...
		{
			UE_LOGF(LogRaceDetector, Fatal, "Unable to change page protection for hotpatching at %p (error %d)", ThunkFunctionAddress, GetLastError());
		}
		...
		*(uint16*)(Trampoline + 10) = 0xE0FF; /* JMP RAX */
		...
		{
			UE_LOGF(LogRaceDetector, Fatal, "Unable to restore page protection for hotpatching at %p (error %d)", ThunkFunctionAddress, GetLastError());
		}
	}
	...
	INSTRUMENTATION_FUNCTION_ATTRIBUTES void CleanupTrampoline(void* ThunkFunctionAddress)
	{
		...
		// Make sure there is no jmp left, equivalent to deactivation.
	...
	// ------------------------------------------------------------------------------
	// Linear allocator for FSyncObject and FSyncObjectBank.
	// Reserves a large virtual address range upfront, commits pages in chunks on demand,
	// and decommits the entire region in a single syscall on shadow reset.
	// This replaces per-object VirtualAlloc/VirtualFree which is extremely slow at scale.
	// ------------------------------------------------------------------------------
	static uint8*    SyncAllocBase = nullptr;
	static SIZE_T    SyncAllocReservedSize = 0;
	static volatile int64 SyncAllocCommittedSize = 0;
	static volatile int64 SyncAllocOffset = 0;
	static SIZE_T    SyncAllocCommitGranularity = 0;
	static volatile int32 SyncAllocGlobalRefCount = 0;

	INSTRUMENTATION_FUNCTION_ATTRIBUTES static void SyncAllocEnsureCommitted(SIZE_T RequiredSize)
	{
		while ((SIZE_T)SyncAllocCommittedSize < RequiredSize)
		{
			int64 OldCommitted = SyncAllocCommittedSize;
			if ((SIZE_T)OldCommitted >= RequiredSize)
			{
				break;
			}

			int64 NewCommitted = (int64)Align(RequiredSize, SyncAllocCommitGranularity);
			NewCommitted = FMath::Min(NewCommitted, (int64)SyncAllocReservedSize);

			// Commit the pages BEFORE advancing the high-water mark.
			// VirtualAlloc(MEM_COMMIT) is idempotent on already-committed pages,
			// so overlapping commits from concurrent threads are safe.
			void* Result = VirtualAlloc(SyncAllocBase + OldCommitted, (SIZE_T)(NewCommitted - OldCommitted), MEM_COMMIT, PAGE_READWRITE);
			checkf(Result != nullptr, TEXT("SyncObjectAllocator: Failed to commit %llu bytes"), NewCommitted - OldCommitted);

			// Advance the high-water mark. If another thread already advanced past us, that's fine —
			// the pages are committed either way. We loop and re-check.
			FPlatformAtomics::InterlockedCompareExchange(&SyncAllocCommittedSize, NewCommitted, OldCommitted);
		}
	}

	INSTRUMENTATION_FUNCTION_ATTRIBUTES void InitSyncObjectAllocator(SIZE_T ReservedSize, SIZE_T CommitGranularity)
	{
		SyncAllocReservedSize = ReservedSize;
		SyncAllocCommitGranularity = CommitGranularity;
		SyncAllocBase = (uint8*)VirtualAlloc(NULL, ReservedSize, MEM_RESERVE, PAGE_NOACCESS);
		checkf(SyncAllocBase != nullptr, TEXT("SyncObjectAllocator: Failed to reserve %llu bytes of virtual address space"), ReservedSize);
	}
	...
	INSTRUMENTATION_FUNCTION_ATTRIBUTES void ShutdownSyncObjectAllocator()
	{
		if (SyncAllocBase)
		{
			VirtualFree(SyncAllocBase, 0, MEM_RELEASE);
			SyncAllocBase = nullptr;
			SyncAllocCommittedSize = 0;
			SyncAllocOffset = 0;
		}
	}
	...
	INSTRUMENTATION_FUNCTION_ATTRIBUTES void* SyncObjectAllocatorAlloc(SIZE_T Size)
	{
		...
		const SIZE_T AlignedSize = Align(Size, CacheLineSize);
		const int64 OldOffset = FPlatformAtomics::InterlockedAdd(&SyncAllocOffset, (int64)AlignedSize);

		checkf((SIZE_T)OldOffset + AlignedSize <= SyncAllocReservedSize, TEXT("SyncObjectAllocator: Exceeded reserved size of %llu bytes"), SyncAllocReservedSize);

		SyncAllocEnsureCommitted((SIZE_T)OldOffset + AlignedSize);

		return SyncAllocBase + OldOffset;
	}

	INSTRUMENTATION_FUNCTION_ATTRIBUTES void SyncObjectAllocatorReset()
	{
		if (SyncAllocCommittedSize > 0)
		{
			VirtualFree(SyncAllocBase, (SIZE_T)SyncAllocCommittedSize, MEM_DECOMMIT);
		}
		SyncAllocOffset = 0;
		SyncAllocCommittedSize = 0;
	}
	...
	INSTRUMENTATION_FUNCTION_ATTRIBUTES void InitShadowMemory()
	{
		...
		if (!ShadowBitmapBase)
		{
			UE_LOGF(LogRaceDetector, Fatal, "Failed to reserve shadow memory (err: %d)", GetLastError());
		}

		if (!VirtualAlloc((LPVOID)ShadowBitmapBase, ShadowBitmapSize + DirtyShadowBitmapSize, MEM_COMMIT, PAGE_READWRITE))
		{
			UE_LOGF(LogRaceDetector, Fatal, "Failed to commit shadow memory bitmap (err: %d)", GetLastError());
		}
	...
	template <typename T>
	FORCEINLINE INSTRUMENTATION_FUNCTION_ATTRIBUTES constexpr T AlignDown(T Val, uint64 Alignment)
	{
	...
	template <typename T>
	FORCEINLINE INSTRUMENTATION_FUNCTION_ATTRIBUTES constexpr T Align(T Val, uint64 Alignment)
	{
	...
	INSTRUMENTATION_FUNCTION_ATTRIBUTES void ThrottleMemoryAccessesDuringShadowReset()
	{
		if (bIsResettingShadow && !bIsResettingShadowThread)
		{
				...
				do
				{
					if (DirtyBits & 1)
					{
						...
						for (UPTRINT Addr = 0; Addr < PageSize; Addr += 8)
						{
							...
							ShadowPageCount += FMath::CountBits64(*Bits);
						}
					}
...
typedef NTSTATUS(NTAPI DBGUIREMOTEBREAKIN)
(
	IN HANDLE hProcess
);

NTSTATUS(NTAPI* TrueDbgUiRemoteBreakin)(HANDLE hProcess);
INSTRUMENTATION_FUNCTION_ATTRIBUTES NTSTATUS NTAPI DetouredDbgUiRemoteBreakin(HANDLE hProcess)
{
        // Notify visual studio as soon as it attaches itself to avoid seeing unhandled exceptions
	// for late attachments.
	Platform::HideFirstChanceExceptionInVisualStudio();

	return TrueDbgUiRemoteBreakin(hProcess);
}
...
INSTRUMENTATION_FUNCTION_ATTRIBUTES int DetourBootFunctions()
{
	...
	TrueDbgUiRemoteBreakin = (DBGUIREMOTEBREAKIN*)GetProcAddress(GetModuleHandleA("ntdll"), "DbgUiRemoteBreakin");
	DetourAttach(&TrueDbgUiRemoteBreakin, DetouredDbgUiRemoteBreakin);
	...
	if (Result != NO_ERROR)
	{
		UE_LOGF(LogRaceDetector, Fatal, "Could not install detoured boot functions (error %d)", Result);
		return Result;
	}
