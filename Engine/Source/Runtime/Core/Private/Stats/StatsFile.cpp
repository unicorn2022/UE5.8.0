...
void IStatsWriteFile::Start( const FString& InFilename )
{
	...
	UE_LOGF( LogStats, Log, "Opening stats file: %ls", *TempFilename );

	File = IFileManager::Get().CreateFileWriter( *TempFilename );
	if (!File)
	{
		UE_LOGF( LogStats, Error, "Could not open: %ls", *TempFilename );
	}
	else
	{
...
void IStatsWriteFile::Stop()
{
	if (IsValid())
	{
		...
		{
			UE_LOGF( LogStats, Warning, "Could not rename stats file: %ls to final name %ls", *TempFilename, *ArchiveFilename );
		}

		UE_LOGF( LogStats, Log, "Wrote stats file: %ls", *ArchiveFilename );
...
bool FStatsReadFile::PrepareLoading()
{
...

	{
		...
		if (Size < 4)
		{
			UE_LOGF( LogStats, Error, "Could not open: %ls", *Filename );
			bResult = false;
			return false;
		}
		...
		{
			UE_LOGF( LogStats, Error, "Could not read, header is invalid: %ls", *Filename );
			bResult = false;
			return false;
		}
		...
		if (!bValidStatType)
		{
			UE_LOGF( LogStats, Error, "Could not read, invalid stats type: %ls", *Filename );
			bResult = false;
			return false;
		}
		...
		if (!bIsFinalized)
		{
			UE_LOGF( LogStats, Error, "Could not read, file is not finalized: %ls", *Filename );
			bResult = false;
			return false;
		}
		...
		{
			UE_LOGF( LogStats, Error, "Could not read, invalid version: %ls, expected %u, was %u", *Filename, (uint32)EStatMagicWithHeader::VERSION_6, Stream.Header.Version );
			bResult = false;
			return false;
		}
		...
		{
			UE_LOGF( LogStats, Error, "Could not read, required compressed data: %ls", *Filename );
			bResult = false;
			return false;
		}
	}
	...
	UE_LOGF( LogStats, Log, "Prepare loading took %.2f sec(s)", TotalTime );
...
void FStatsReadFile::ReadStats()
{
	...
	{
		const double TotalTime = FPlatformTime::Seconds() - StartTime;
		UE_LOGF( LogStats, Log, "Reading took %.2f sec(s)", TotalTime );
	...
	{
		UE_LOGF( LogStats, Warning, "Reading stopped, abandoning" );
	...
	if (bRawStatsFile)
	{
		if (!IsProcessingStopped())
		{
					...
					for (int32 Index = 0; Index < NumStatMessages; Index++)
					{
						...
						else if (Op == EStatOperation::CycleScopeEnd)
						{
							...
							else
							{
								...
								UE_LOGF( LogStats, Warning, "Broken cycle scope end %ls/%ls, current %ls",
			...
			if (!IsProcessingStopped())
			{
				...
				const double TotalTime = FPlatformTime::Seconds() - StartTime;
				UE_LOGF( LogStats, Log, "Processing took %.2f sec(s)", TotalTime );
			}
			else
			{
				UE_LOGF( LogStats, Warning, "Processing stopped, abandoning" );
			}
...
void FStatsReadFile::UpdateReadStageProgress()
{
	...
	if (CurrentSeconds > LastUpdateTime + NumSecondsBetweenUpdates)
	{
		...
		UE_LOGF( LogStats, Verbose, "UpdateReadStageProgress: %3i%%", PercentagePos );
...
void FStatsReadFile::UpdateCombinedHistoryStats()
{
	...
	for (const auto& It : CombinedHistory)
	{
		...
		UE_LOGF( LogStats, Verbose, "Frame: %4i/%2i Size: %5.1f MB / %10i",
				FrameNum,
				FramePackets,
	...
	UE_LOGF( LogStats, Warning, "Total PacketSize: %6.1f MB, Max: %2f MB, PacketsNum: %i, StatMessagesNum: %i, Frames: %i",
...
void FStatsReadFile::UpdateProcessStageProgress( const int32 CurrentStatMessageIndex, const int32 FrameIndex, const int32 PacketIndex )
{
	...
	if (CurrentSeconds > LastUpdateTime + NumSecondsBetweenUpdates)
	{
		...
		StageProgress.Set( PercentagePos );
		UE_LOGF( LogStats, Verbose, "Processing %3i%% (%10i/%10i) stat messages [Frame: %3i, Packet: %2i]", PercentagePos, CurrentStatMessageIndex, FileInfo.TotalStatMessagesNum, FrameIndex, PacketIndex );
		LastUpdateTime = CurrentSeconds;
	}
...
void FCommandStatsFile::TestLastSaved()
{
	...
	if (Instance)
	{
		...
		{
			UE_LOGF( LogStats, Log, "Failed to stats file: %ls", *FilePath );
			return;
		}
		UE_LOGF( LogStats, Log, "Loaded stats file: %ls, %lld frame", *FilePath, 1 + Loaded.GetLatestValidFrame() - Loaded.GetOldestValidFrame() );
		{
			int64 TestFrame = Loaded.GetOldestValidFrame();
			UE_LOGF( LogStats, Log, "**************************** Test Frame %lld", TestFrame );
