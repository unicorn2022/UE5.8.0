...
FConditionalScopeLogTime::~FConditionalScopeLogTime()
{
	if (Units != ScopeLog_DontLog)
	{
		...
		if( Cumulative )
		{
			...
			UE_LOGF( LogStats, Log, "%32ls - %6.3f %ls - Total %6.2f s / %5u / %6.3f %ls", *Name, GetDisplayScopedTime(ScopedTime), *DisplayUnitsString, Cumulative->Key, Cumulative->Value, GetDisplayScopedTime(Average), *DisplayUnitsString );
		}
		else
		{
			UE_LOGF( LogStats, Log, "%32ls - %6.3f %ls", *Name, GetDisplayScopedTime(ScopedTime), *DisplayUnitsString );
		}
	}
}
