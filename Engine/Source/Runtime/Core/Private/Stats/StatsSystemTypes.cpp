...
public:
	...
	virtual TStatId GetHighPerformanceEnableForStat(FName StatShortName, const char* InGroup, const char* InCategory, bool bDefaultEnable, bool bShouldClearEveryFrame, EStatDataType::Type InStatType, TCHAR const* InDescription, bool bCycleStat, bool bSortByName, FPlatformMemory::EMemoryCounterRegion MemoryRegion = FPlatformMemory::MCR_Invalid) override
	{
		...
		FString StatDescription;
		{
			...
			if (Found)
			{
				...
				{
					UE_LOGF(LogStatGroupEnableManager, Fatal, "Stat group %ls was was defined both on and off by default.", *Group.ToString());
				}
				...
				if (StatFound)
				{
					if (StatFoundAlways)
					{
						UE_LOGF(LogStatGroupEnableManager, Fatal, "Stat %ls is both always enabled and not always enabled, so it was used for two different things.", *Stat.ToString());
					}
	...
	void ListGroup(FName Group)
	{
		...
		if (Found)
		{
			UE_LOGF(LogStatGroupEnableManager, Display, "  %d  default %d %ls", !!Found->CurrentEnable, !!Found->DefaultEnable, *Group.ToString());
		}
	}
	...
	void ListGroups(bool bDetailed = false)
	{
		...
		{
			UE_LOGF(LogStatGroupEnableManager, Display, "  %d  default %d %ls", !!It.Value().CurrentEnable, !!It.Value().DefaultEnable, *(It.Key().ToString()));
			if (bDetailed)
			{
				for (auto ItInner = It.Value().NamesInThisGroup.CreateConstIterator(); ItInner; ++ItInner)
				{
					UE_LOGF(LogStatGroupEnableManager, Display, "      %d %ls", !ItInner.Value()->IsNone(), *ItInner.Key().ToString());
				}
				for (auto ItInner = It.Value().AlwaysEnabledNamesInThisGroup.CreateConstIterator(); ItInner; ++ItInner)
				{
					UE_LOGF(LogStatGroupEnableManager, Display, "      (always enabled) %ls", *ItInner.Key().ToString());
				}
			}
		}
	}
	...
	FName CheckGroup(TCHAR const*& Cmd, bool Enable)
	{
		...
		MaybeGroup = FString("STATGROUP_") + MaybeGroup;
		...
		if (!Found)
		{
			...
			ListGroups();
			UE_LOGF(LogStatGroupEnableManager, Display, "Group Not Found %ls", *MaybeGroupFName.ToString());
			return NAME_None;
		}
...
public:
	...
	void StatMessage(FStatPacket* Packet)
	{
		...
		{
			UE_LOGF(LogStats, Log, "Packet from %x with %d messages", Packet->ThreadId, Packet->StatMessages.Num());
		}
...
void FThreadStats::FlushRegularStats(bool bHasBrokenCallstacks, bool bForceFlush)
{
	...
	if ((!ScopeCount || bForceFlush) && Packet.StatMessages.Num())
	{
		...
		{
			UE_LOGF(LogStats, Verbose, "StatMessage Packet has more than %i messages.  Ignoring for the presize history.", (int32)PRESIZE_MAX_SIZE);
		}
...
void FThreadStats::FlushRawStats(bool bHasBrokenCallstacks /*= false*/, bool bForceFlush /*= false*/)
{
	...
	if (NumMessages > MAX_RAW_MESSAGES_IN_PACKET || bFrameHasChanged || bForceFlush || bSawExplicitFlush)
	{
		...
		{
			UE_LOGF(LogStats, Warning, "FlushRawStats NumMessages: %i (%.2f MB), Thread: %u", NumMessages, NumMessagesAsMB, Packet.ThreadId);
		}

		UE_LOGF(LogStats, Verbose, "FlushRawStats NumMessages: %i (%.2f MB), Thread: %u", NumMessages, NumMessagesAsMB, Packet.ThreadId);
	}
}
...
void FThreadStats::CheckForCollectingStartupStats()
{
	...
	while (1)
	{
		...
		for (int32 Index = 0; Index < CmdsArray.Num(); Index++)
		{
			...
			UE_LOGF(LogStatGroupEnableManager, Log, "Sending Stat Command '%ls'", *StatCmd);
...
void FThreadStats::StartThread()
{
	...
	UE_LOGF(LogStats, Log, "Stats thread started at %f", FPlatformTime::Seconds() - GStartTime);
}
