...
FMalloc* FMacPlatformMemory::BaseAllocator()
{
	...
	{
		// Make mimalloc a default allocator on Editor as it has a significantly better perf
...
FPlatformMemoryStats FMacPlatformMemory::GetStats()
{
	...
	if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info_t)&Stats, &StatsSize) != KERN_SUCCESS)
	{
	...
	if (task_info( mach_task_self(), TASK_VM_INFO, (task_info_t)&TaskInfo, &TaskInfoCount ) != KERN_SUCCESS)
	{
		UE_LOGF(LogTemp, Warning, "Failed to fetch vm statistics");
		return MemoryStats;
	}
	
	// we want to use phys_footprint instead of resident size as phys_footprint 
	// represents our application's total memory footprint rather than the memory
	// that's just in RAM right now (which can vary drastically depending on memory pressure)
	MemoryStats.UsedPhysical = TaskInfo.phys_footprint;
...
bool FMacPlatformMemory::UnmapNamedSharedMemoryRegion(FSharedMemoryRegion * MemoryRegion)
{
	...
	if (MemoryRegion)
	{
		...
		if (munmap(MacRegion->GetAddress(), MacRegion->GetSize()) == -1) 
		{
			...
			int ErrNo = errno;
			UE_LOGF(LogHAL, Warning, "munmap(addr=%p, len=%zu) failed with errno = %d (%ls)", MacRegion->GetAddress(), MacRegion->GetSize(), ErrNo,
				StringCast< TCHAR >(strerror(ErrNo)).Get());
		}
		...
		if (close(MacRegion->GetFileDescriptor()) == -1)
		{
			...
			int ErrNo = errno;
			UE_LOGF(LogHAL, Warning, "close(fd=%d) failed with errno = %d (%ls)", MacRegion->GetFileDescriptor(), ErrNo, 
				StringCast< TCHAR >(strerror(ErrNo)).Get());
		}
		...
		if (MacRegion->NeedsToUnlinkRegion())
		{
			...
			if (shm_unlink(NameUTF8.Get()) == -1)
			{
				...
				int ErrNo = errno;
				UE_LOGF(LogHAL, Warning, "shm_unlink(name='%ls') failed with errno = %d (%ls)", MacRegion->GetName(), ErrNo, 
					StringCast< TCHAR >(strerror(ErrNo)).Get());
			}
		}
