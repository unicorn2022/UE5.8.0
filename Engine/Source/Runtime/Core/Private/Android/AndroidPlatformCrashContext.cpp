...
static void SetCrashContextOnGameThread(const FString KeyIn, const FString ValueIn)
{
	...
	{
		UE_LOGF(LogAndroid, Log, "Failed to set crash context `%ls` = '%ls'", *KeyIn, *ValueIn);
	}
}
...
bool FAndroidCrashContext::ShouldIncludeInPortableCallStackHash(const FCrashStackFrame& Frame) const
{
	// Match against libUnreal since GetPortableCallStack puts the .so name in the callstack
	return Frame.ModuleName == TEXT("libUnreal");
}
