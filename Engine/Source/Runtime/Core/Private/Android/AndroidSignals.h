...
protected:
	...
	static void Init(int ForwardingSignal)
	{
		...
		UE_CLOGF(!bStackSizeSuccess, LogAndroid, Error, "Failed to set signal thread stack size.");

		bool bThreadCreated = pthread_create(&SignalHandlerThread, &otherAttr, ThreadFunc, nullptr) == 0;
		UE_CLOGF(!bThreadCreated, LogAndroid, Fatal, "Failed to create signal handler");
