...
void FAndroidPlatformMemory::Init()
{
	...
	UE_LOGF(LogInit, Log, "Memory total: Physical=%.2fMB (%dGB approx) Available=%.2fMB PageSize=%.1fKB", 
	...
	*/
	...
	static void SendMemoryWarningContext()
	{
		...
		else
		{
			...
			UE_LOGF(LogAndroid, Warning, "Not calling memory warning handler, received too early. Last Trim Memory State: %d", (int)Context.LastTrimMemoryState);
		}
	}
}
...
// Called from JNI when trim messages are received.
...
FMalloc* FAndroidPlatformMemory::BaseAllocator()
{
	...
	const bool bUseMimalloc = FPlatformMisc::GetConfigRulesVariableAsUtf8("malloctype").Equals("mimalloc");
	if (bUseMimalloc)
	{
		return (Instance = new FMallocMimalloc());
	}
#endif
	...
	if (USE_MALLOC_BINNED2)
	{
		...
		if (End - Base < Size)
		{
			if (Base)
			{
				if (Base < End)
				{
					if (munmap(Base, End - Base) != 0)
					{
						...
						UE_LOGF(LogHAL, Fatal, "munmap (for trim) (addr=%p, len=%llu) failed with errno = %d (%ls)", Base, End - Base,
			...
			if (Base > UnalignedBase)
			{
				...
				{
					const int ErrNo = errno;
					UE_LOGF(LogHAL, Fatal, "munmap (for align) (addr=%p, len=%llu) failed with errno = %d (%ls)", UnalignedBase, Base - UnalignedBase,
						ErrNo, StringCast< TCHAR >(strerror(ErrNo)).Get());
				}
			}
		...
		if (Base > UnalignedBase)
		{
			...
			{
				const int ErrNo = errno;
				UE_LOGF(LogHAL, Fatal, "munmap (for tail align) (addr=%p, len=%llu) failed with errno = %d (%ls)", UnalignedBase, Base - UnalignedBase,
					ErrNo, StringCast< TCHAR >(strerror(ErrNo)).Get());
			}
		}
	}
