...
FString PGO_GetOutputDirectory()
{
	...
	UE_LOGF(LogAndroid, Warning, "No PGO output destination path specifed, defaulting to %ls", *DefaultPGODir);
...
static void PGO_ResetCounters()
{
	...
	UE_LOGF(LogAndroid, Log, "Resetting PGO counters.");
...
void PGO_WriteFile()
{
	...
	UE_LOGF(LogAndroid, Log, "Writing out PGO results file: \"%ls\".", *OutputFileName);
	__llvm_profile_set_filename(TCHAR_TO_ANSI(*OutputFileName));

	if (__llvm_profile_write_file() != 0)
	{
		UE_LOGF(LogAndroid, Error, "Failed to write PGO output file.");
	}
	else
	{
		UE_LOGF(LogAndroid, Log, "PGO results file written successfully.");
	}
