...
void* FAndroidPlatformProcess::GetDllHandle(const TCHAR* Filename)
{
	...
	if (!Handle)
	{
		UE_LOGF(LogAndroid, Warning, "dlopen failed: %ls", ANSI_TO_TCHAR(dlerror()));
	}
	return Handle;
}
...
#else
	UE_LOGF(LogAndroid, Fatal, "A sub-platform that doesn't use USE_ANDROID_FILE must implement PlatformProcess::ExecutableName");
	return TEXT("");
#endif
}
...
static void ApplyDefaultThreadAffinity(IConsoleVariable* Var)
{
	...
	if (AffinityCmd.ParseIntoArrayWS(Args) > 0)
	{
		for (int32 Index = 0; Index + 1 < Args.Num(); Index += 2)
		{
			...
			if (!Aff)
			{
				UE_LOGF(LogAndroid, Display, "Parsed 0 for affinity, using 0xFFFFFFFFFFFFFFFF instead");
...
void FRunnableThreadAndroid::SetThreadPriority(pthread_t InThread, EThreadPriority NewPriority)
{
	...
	if (InitialPolicy != NewPolicy && (sched_setscheduler(ThreadID, NewPolicy, &Sched)!=0) && bCanLog)
	{
		bErrorEncountered = true;
		UE_LOGF(LogHAL, Error, "Failed to set %ls thread scheduler, tid %d from %d to %d (errno %d)", *GetThreadName(), ThreadID, InitialPolicy, NewPolicy, errno);
	}
	...
	{
		bErrorEncountered = true;
		UE_LOGF(LogHAL, Error, "Failed to set %ls thread priority, tid %d from %d to %d (errno %d)", *GetThreadName(), ThreadID, InitialNice, NewNice, errno);
	}
