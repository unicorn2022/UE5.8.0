...
namespace AndroidProfiler
{
		...
		FConsoleCommandWithArgsDelegate::CreateLambda([](const TArray<FString>& Args)
			{
				...
				FString ProfileName = FAndroidProfiler::StartCapture(CombinedArgs, [](const FAndroidProfiler::FProfileResults& Results)
				{
					if (!Results.FilePath.IsEmpty())
					{
						...
						UE_CLOGF(!Results.Error.IsEmpty(), LogAndroidProfiling, Warning, "Profile %ls completed with log: %ls", *Results.ProfileName, *Results.Error);
						...
						UE_LOGF(LogAndroidProfiling, Log, "Profile %ls complete, retrieve via 'adb pull %ls'", *Results.ProfileName, *DestPath);
					}
					else
					{
						UE_LOGF(LogAndroidProfiling, Error, "Profile %ls failed: %ls", *Results.ProfileName, *Results.Error);
					}
				});
			}));
	...
	static void InitJNI()
	{
		...
		UE_CLOGF(!IsJNIAvailable(), LogAndroidProfiling, Error, "JNI Could not find ProfilerAccessor class or methods. ProfilerAccessor is not supported when Compile SDK < 35.");
	}
	...
	static FString CallProfiler(FString& ProfileNameOUT, const FString& CombinedArgs)
	{
		if (IsJNIAvailable())
		{
			...
			UE_CLOGF(bException, LogAndroidProfiling, Error, "Profiler failed due to java exception.");

			if(ProfilerResults.Num() == 2)
			{
				ProfileNameOUT = ProfilerResults[0];
				const FString& ProfilerMessage = ProfilerResults[1];
				UE_LOGF(LogAndroidProfiling, Log, "Profiler : %ls ", ProfilerMessage.IsEmpty() ? TEXT("profile issued.") : *ProfilerMessage);
				return ProfilerMessage;
			}
			else
			{
				UE_LOGF(LogAndroidProfiling, Error, "Profiler failed.");
			}
		}
		else
		{
			UE_LOGF(LogAndroidProfiling, Error, "Profile failed, java profiler not found.");
		}
		return FString();
	}
	...
	static bool CancelProfile(const FString& ProfileName)
	{
		...
		UE_LOGF(LogAndroidProfiling, Error, "Profile cancel failed, java profiler not found.");
		return false;
	}
}
...
void JNICALL UE::Jni::AndroidProfiling::FProfilerAccessor::nativeOnProfileFinish(JNIEnv* jenv, jclass clazz, jstring ProfileNameJava, jstring ProfileErrorJava, jstring ProfileFilepathJava)
{
	...
	UE_LOGF(LogAndroidProfiling, Log, "nativeOnProfileFinish (%ls, %ls, %ls)",*Results.ProfileName, *Results.Error, *Results.FilePath);
...
void FAndroidProfiler::StopCapture(const FString& ProfileName)
{
	...
	{
		UE_LOGF(LogAndroidProfiling, Error, "StopCapture ignored, profile %ls is not active.", *ProfileName);
	}
}
#else
