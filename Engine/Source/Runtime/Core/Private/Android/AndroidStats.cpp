...
// We want to mirror the android enum values, so check some of them.
static_assert((int)EDeviceThermalState::None == (int)ATHERMAL_STATUS_NONE);
static_assert((int)EDeviceThermalState::Severe == (int)ATHERMAL_STATUS_SEVERE);
EDeviceThermalState FAndroidStats::GetThermalStatus()
{
	return static_cast<EDeviceThermalState>(GThermalStatus);
}
...
#endif //#if CSV_PROFILER
		}
#endif //#if CSV_PROFILER || STATS
...
static void GPUStatsLogCallback(uint8_t Level, const char* Message)
{
	switch (static_cast<libGPUCountersLogLevel>(Level))
	{
		...
		{
			UE_LOGF(LogAndroidStats, Error, "%s", Message);
			break;
		}
		case libGPUCountersLogLevel::Log:
		default:
		{
			UE_LOGF(LogAndroidStats, Log, "%s", Message);
			break;
		}
	}
}
...
static void InitGPUStats()
{
	UE_LOGF(LogAndroidStats, Log, "HWCPipe: GAndroidHWCPipeStatsEnabled=%d (set on commandline)", GAndroidHWCPipeStatsEnabled);
