...
jclass AndroidJavaEnv::FindJavaClass(const char* name)
{
	...
	jclass FoundClass = UE::Jni::Java::Lang::FClassLoader::loadClass(ClassLoader, ClassNameObj).Leak();
...
jclass AndroidJavaEnv::FindJavaClassGlobalRef(const char* name)
{
	...
	jstring ClassNameObj = Env->NewStringUTF(name);
	auto FoundClass = UE::Jni::Java::Lang::FClassLoader::loadClass(ClassLoader, ClassNameObj);
...
namespace UE::Jni
{
	...
	{
		FScopedJavaObject<jclass> Class = Java::Lang::FClassLoader::loadClass(ClassLoader, *FScopedJavaObject{Value->NewStringUTF(ClassName)});
	...
	FScopedJavaObject<Java::Lang::FString*> FEnv::Marshal(FStringView Param) const
	{
		return NewScopedJavaObject(Value, Value->NewString(reinterpret_cast<const jchar*>(Param.GetData()), Param.Len()));
	}
	
	FString FEnv::Marshal(Java::Lang::FString* Param) const
	{
		FString String;

		if (Param)
		{
			if (jsize Len = Value->GetStringLength(Param))
			{
				String.GetCharArray().AddUninitialized(Len + 1);
				TCHAR* Buffer = String.GetCharArray().GetData();
				Value->GetStringRegion(Param, 0, Len, reinterpret_cast<jchar*>(Buffer));
				Buffer[Len] = '\0';
			}
		}
	...
	extern "C" void (FEnv::*const __start_UE_Jni_Initialize [[gnu::weak]])() const;
    extern "C" void (FEnv::*const __stop_UE_Jni_Initialize [[gnu::weak]])() const;
	void FEnv::Initialize() const
	{
		...
		Initialize<FInteger>();
		Initialize<Java::Util::FHashMap>();

		for (auto* Initialize = &__start_UE_Jni_Initialize; Initialize < &__stop_UE_Jni_Initialize; ++Initialize)
		{
			(this->**Initialize)();
		}
	}
