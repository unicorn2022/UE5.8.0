...
namespace AndroidVKQuality
{
	...
	bool LoadVkQuality(const char* GLESVersionString, void* VulkanPhysicalDeviceProperties)
	{
		...
		if (!VKQualityLib)
		{
			UE_LOGF(LogAndroid, Warning, "VKQuality: libvkquality.so could not be loaded");
			return false;
		}
		...
		{
			UE_LOGF(LogAndroid, Warning, "VKQuality: not all entrypoints were loaded correctly");
			// unload .so
			UnloadVkQuality();
			return false;
		}
		...
		if (Result != kSuccess)
		{
			UE_LOGF(LogAndroid, Warning, "VKQuality: initialization failed with error %d", Result);		
			// unload .so
			UnloadVkQuality();
			return false;
		}
	...
	FString GetVKQualityRecommendation()
	{
		...
		switch (Recommendation)
		{
			...
			default:
				UE_LOGF(LogAndroid, Warning, "VKQuality: unrecognized recommendation %d", Recommendation);
				return TEXT("");
		}
	}
