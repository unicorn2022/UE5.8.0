...
bool FPreloadableArchive::StartPreload()
{
	...
	if (!bInitialized)
	{
		UE_LOGF(LogCore, Error, "Attempted FPreloadableArchive::StartPreload when uninitialized. Call will be ignored.");
		return false;
	}
	if (!AllocateCache())
	{
...
bool FPreloadableArchive::AllocateCache()
{
	...
	if (!bInitialized)
	{
		UE_LOGF(LogCore, Error, "Attempted FPreloadableArchive::AllocateCache when uninitialized. Call will be ignored.");
		return false;
	}
	if (Size < 0)
	{
...
bool FPreloadableArchive::ResumePreloadNonRecursive()
{
	...
	if (!ReadRequest)
	{
		...
		UE_LOGF(LogCore, Warning, "ReadRequest returned null");
...
void FPreloadableArchive::OnReadComplete(bool bCanceled, IAsyncReadRequest* ReadRequest)
{
	...
	while (true)
	{
		...
		{
			UE_LOGF(LogCore, Warning, "Precaching failed for %ls: %ls.", *GetArchiveName(), (bCanceled ? TEXT("Canceled") : TEXT("GetReadResults returned null")));
...
void FPreloadableArchive::Serialize(void* V, int64 Length)
{
	...
	{
		UE_LOGF(LogCore, Warning, "FPreloadableArchive::Serialize Mismatch on %ls. BytesMatch=%ls, PosMatch=%ls, WasPreloading=%ls",
...
void FPreloadableArchive::SerializeInternal(void* V, int64 Length)
{
	...
	{
		SetError();
		UE_LOGF(LogCore, Error, "Attempted to Serialize from FPreloadableArchive when not initialized.");
		return;
	}
	if (Pos + Length > Size)
	{
...
void FPreloadableArchive::SerializeSynchronously(void* V, int64 Length)
{
	if (SynchronousArchive)
	{
		...
		if (SynchronousArchive->IsError())
		{
			if (!IsError())
			{
				UE_LOGF(LogCore, Warning, "Failed to seek to offset %lld in %ls.", Pos, *GetArchiveName());
				SetError();
			}
		}
		else
		{
			...
			{
				UE_LOGF(LogCore, Warning, "Failed to read %lld bytes at offset %lld in %ls.", Length, Pos, *GetArchiveName());
				SetError();
			}
		}
		Pos += Length;
	...
	else if (AsynchronousHandle)
	{
		...
		if (!ReadRequest)
		{
			if (!IsError())
			{
				UE_LOGF(LogCore, Warning, "Failed to create ReadRequest to offset %lld in %ls.", Pos, *GetArchiveName());
				SetError();
			}
		}
		else
		{
			if (!ReadRequest->WaitCompletion())
			{
				if (!IsError())
				{
					UE_LOGF(LogCore, Warning, "Failed to WaitCompletion on ReadRequest to offset %lld in %ls.", Pos, *GetArchiveName());
					SetError();
				}
			}
			else
	...
	else
	{
		if (!IsError())
		{
			UE_LOGF(LogCore, Warning, "No InnerArchive available for serialization in %ls", *GetArchiveName());
			SetError();
		}
	}
	return;
