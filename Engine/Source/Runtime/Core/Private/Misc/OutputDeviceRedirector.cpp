...
struct FOutputDeviceRedirectorState
{
	...
	TMpscQueue<FOutputDeviceItem, FOutputDeviceLinearAllocator> BufferedItems;
	std::atomic<uint32> BufferedItemCount = 0;
	...
	FORCENOINLINE bool EnableBacklog(bool bEnable);
...
void FOutputDeviceRedirectorState::ThreadLoop()
{
	...
	for (;;)
	{
		...
		while (BufferedItemCount.load(std::memory_order_relaxed) && IsPrimaryThread(ThreadId))
		{
			if (FOutputDevicesPrimaryScopeLock Lock(*this); Lock.IsLocked())
			{
				FlushBufferedItems();
...
{
	if (!BufferedItemCount.load(std::memory_order_relaxed))
	{
		return;
	}
...
void FOutputDeviceRedirectorState::FlushBufferedItemsTo(VisitorType&& Visitor)
{
	...
	for (uint32 Count = BufferedItemCount.exchange(0, std::memory_order_relaxed); Count; --Count)
	{
		for (;;)
		{
			if (!Visitor.bIsPanicThread && HasPanicThread())
			{
				...
				BufferedItemCount.fetch_add(Count, std::memory_order_relaxed);
				return;
			}
			
			// Dequeue can return NullOpt even though it's not empty due to races of setting Prev->Next	in Enqueue		
			if (TOptional<FOutputDeviceItem> Item = BufferedItems.Dequeue())
			{
				Visit([this, &Visitor]<class ValueType>(const ValueType& Value)
				{
...
uint64 FOutputDeviceRedirectorState::CreateFence()
{
	...
	BufferedItems.Enqueue(Index);
	if (!BufferedItemCount.fetch_add(1, std::memory_order_relaxed))
	{
		ThreadWakeEvent.Notify();
	}
	return Index;
...
UE_AUTORTFM_NOAUTORTFM
bool FOutputDeviceRedirectorState::EnableBacklog(bool bEnable)
{
	...
	else if (!bWasEnabled)
	{
			...
			PrimaryLock.IsLocked() && IsPrimaryThread(ThreadId) && BufferedOutputDevices.IsEmpty())
		{
			...
			FlushBufferedItemsTo(FVisitor{*this, IsPanicThread(ThreadId)});
		}
	}
	return bWasEnabled;
}
...
UE_AUTORTFM_NOAUTORTFM // Not currently supported in a closed transaction
bool FOutputDeviceRedirector::EnableBacklog(bool bEnable)
{
	return State->EnableBacklog(bEnable);
}
...
void FOutputDeviceRedirector::SerializeRecord(const UE::FLogRecord& Record)
{
	...
	State->BufferedItems.Enqueue(Record);
	if (!State->BufferedItemCount.fetch_add(1, std::memory_order_relaxed))
	{
...
void FOutputDeviceRedirector::Serialize(const TCHAR* const Data, const ELogVerbosity::Type Verbosity, const FName& Category, const double Time)
{
	...
	State->BufferedItems.Enqueue(Data, Category, Verbosity, RealTime);
	if (!State->BufferedItemCount.fetch_add(1, std::memory_order_relaxed))
	{
		State->ThreadWakeEvent.Notify();
	}
