...
bool FURLRequestFilter::IsRequestAllowed(FStringView InURL) const
{
	...
	{
		UE_LOGF(LogCore, Log, "Scheme for URL %.*ls not allowed.", InURL.Len(), InURL.GetData());
		return false;
	}
	...
	UE_CLOGF(!bFoundDomain, LogCore, Log, "Domain for URL %.*ls not allowed.", InURL.Len(), InURL.GetData());
