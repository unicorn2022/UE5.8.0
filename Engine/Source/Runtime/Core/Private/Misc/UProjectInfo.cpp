...
void FUProjectDictionary::Refresh()
{
	...
	IFileManager::Get().FindFiles(ProjectDirsFiles, *(RootDir / FString("*.uprojectdirs")), true, false);
		...
		if(FFileHelper::LoadANSITextFileToStrings(*(RootDir / ProjectDirsFile), &IFileManager::Get(), Lines))
		{
			for(const FString& Line: Lines)
			{
				...
				if(!Entry.IsEmpty() && !Entry.StartsWith(";"))
				{
					...
					{
						UE_LOGF(LogUProjectInfo, Warning, "Project search path '%ls' is not under root directory, ignoring.", *Entry);
					}
				}
			}
		}
	...
	for(const FString& ProjectRootDir: ProjectRootDirs)
	{
		...
		IFileManager::Get().FindFiles(ProjectDirs, *(ProjectRootDir / FString("*")), false, true);
...
FUProjectDictionary& FUProjectDictionary::GetDefault()
{
	...
	{
		UE_LOGF(LogUProjectInfo, Verbose, "Found projects:");
		for(TMap<FString, FString>::TConstIterator Iter(DefaultDictionary.ShortProjectNameDictionary); Iter; ++Iter)
		{
			UE_LOGF(LogUProjectInfo, Verbose, "    %ls: \"%ls\"", *Iter.Key(), *Iter.Value());
		}
