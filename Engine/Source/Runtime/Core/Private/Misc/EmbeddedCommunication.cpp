...
void FEmbeddedCommunication::ForceTick(int ID, float MinTimeSlice, float MaxTimeSlice)
{
	...
	{
		UE_LOGF(LogInit, Display, "###ForceTick %d: no messages", ID);
		return;
	}
	...
	{
		UE_LOGF(LogInit, Display, "  ###ForceTick %d timed out", ID);
	}
#endif
}
