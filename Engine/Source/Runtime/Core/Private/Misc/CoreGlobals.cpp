...
CSV_DEFINE_CATEGORY(EngineInit, true);
...
UE::CoreUObject::Private::FStoreBase* const* GCoreFixedTagStoresForDebugVisualizers = nullptr;
...
void CORE_API RequestEngineExit(const TCHAR* ReasonString)
{
	...
	UE_LOGF(LogCore, Log, "Engine exit requested (reason: %ls%ls)", ReasonString, GShouldRequestExit ? TEXT("; note: exit was already requested") : TEXT(""));
	GShouldRequestExit = true;
#else
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_LOGF(LogCore, Log, "Engine exit requested (reason: %ls%ls)", ReasonString, GIsRequestingExit ? TEXT("; note: exit was already requested") : TEXT(""));
	GIsRequestingExit = true;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
void ToggleGDebugPUCrashedFlag(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogCore, Log, "Gpu crashed flag forcibly set to: %i", GIsGPUCrashed ? 1 : 0);
}
...
#define CSV_BOOT_PROFILING (CSV_PROFILER_STATS && !UE_BUILD_SHIPPING)
...
#if CSV_BOOT_PROFILING
	if (!GBootTimingCompleted && IsInGameThread() && FCsvProfiler::IsCapturing())
	{
		Message = InMessage;
		if (Message.Len())
		{
			CSV_REGION_BEGIN(EngineInit, *Message);
		}
	}
#endif
}
...
FScopedBootTiming::~FScopedBootTiming()
{
	...
	if (!Message.IsEmpty())
	{
		CSV_REGION_END(EngineInit, *Message);
	}
#endif
...
static void DumpBootTimingString(const TCHAR* Message)
{
	if (0)
	{
		UE_LOGF(LogCore, Display, "%ls", Message);
	}
	else
	{
...
bool IsThisNotNull(const void* This, const ANSICHAR* Function)
{
	...
	UE_LOGF(LogCore, Warning, "'%s' was called with a null 'this' pointer - this is no longer supported.", Function);
	return false;
}
