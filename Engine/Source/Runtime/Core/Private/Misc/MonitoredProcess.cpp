...
bool FMonitoredProcess::RunProcessSynchronous( const FString& InURL, const FString& InParams, TArray<FString>& OutOutput, float Timeout/*0.f*/ )
{
	FMonitoredProcess MonitoredProcess(InURL, InParams, /*InHidden=*/true);

	MonitoredProcess.OnOutput().BindLambda([&OutOutput](const FString& Output)
		{
			OutOutput.Add(Output);
		});
	...
	// Add any remaining unprocessed output.
	const FString& RemainingBuffer = MonitoredProcess.GetFullOutputWithoutDelegate();
	if (RemainingBuffer.Len() > 0)
	{
		OutOutput.Add(RemainingBuffer);
	}

	return MonitoredProcess.GetReturnCode() == 0;
}
...
//		Params += TEXT(" -nocompile");
	...
	UE_LOGF(LogMonitoredProcess, Log, "Running Serialized UAT: [ %ls %ls ]", *URL, *Params);
...
{
#if PLATFORM_WINDOWS
	FString RunUATScriptName = "RunUAT.bat";
#elif PLATFORM_LINUX
	FString RunUATScriptName = "RunUAT.sh";
#else
	FString RunUATScriptName = "RunUAT.command";
#endif
