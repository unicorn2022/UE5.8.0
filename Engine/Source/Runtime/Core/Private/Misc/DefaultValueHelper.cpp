...
bool FDefaultValueHelper::ParseInt(const TCHAR* Source, int32& OutVal)
{
	int32 Base;
	const int32 Len = (Source && *Source) ? FCString::Strlen(Source) : 0;
	if( Len > 0 && IsStringValidInteger( Source, Source + Len, Base ) )
	{
		const bool bHasWhitespace = HasWhitespaces(Source);
		const FString NoWhitespace = bHasWhitespace ? RemoveWhitespaces(Source) : FString();
