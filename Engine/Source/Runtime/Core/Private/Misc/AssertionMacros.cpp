...
bool GEnsureBreakEnabled = true;
FAutoConsoleVariableRef CVarEnsureBreakEnabled(
	TEXT("core.EnsureBreakEnabled"),
	GEnsureBreakEnabled,
	TEXT("Set to false to turn off the debugger catching ensures"),
	ECVF_Default);
...
FAutoConsoleCommand CVarResetEnsureState(
	...
	FConsoleCommandDelegate::CreateLambda([]()
		{
			// Increment and wrap to 0-255
			GEnsureResetState = (uint8)(GEnsureResetState + 1);
		}));

FAutoConsoleVariableRef CVarEnsureStateRef(
    TEXT("core.EnsureState"),
    GEnsureResetState,
    TEXT("Set the underlying value/counter for ensure, its initial value is 1"));
...
public:
	...
	void AppendV(const UTF8CHAR* Format, va_list Args)
	{
		if (const int32 Len = TCString<UTF8CHAR>::GetVarArgs(Utf8Scratch, UE_ARRAY_COUNT(Utf8Scratch), Format, Args); Len > 0)
		{
			Append(Utf8Scratch);
		}
	}

	UE_REWRITE void AppendV(const ANSICHAR* Format, va_list Args)
	{
		AppendV((const UTF8CHAR*)Format, Args);
	}
...
private:
	...
	// Scratch space for formatting as UTF-8.
	// This is static to avoid stack overflows.
	// This is safe to be static because its use is guarded by FailDebugMutex.
	inline static UTF8CHAR Utf8Scratch[Capacity] = {};
};
...
FORCENOINLINE void StaticFailDebugV(
	...
	void* ProgramCounter,
	const TCHAR* Message,
	const FormatCharType* DescriptionFormat,
...
{
	va_list DescriptionArgs{};
	StaticFailDebugV(Error, /*Expression*/ "", File, Line, /*bIsEnsure*/ false, ProgramCounter, Message, /*DescriptionFormat*/ (const TCHAR*)nullptr, DescriptionArgs);
}
...
FORCENOINLINE void VARARGS StaticFailDebugV(const TCHAR* Error, const ANSICHAR* File, int32 Line, void* ProgramCounter, const ANSICHAR* DescriptionFormat, va_list DescriptionArgs)
{
	StaticFailDebugV(Error, /*Expression*/ "", File, Line, /*bIsEnsure*/ false, ProgramCounter, /*Message*/ nullptr, DescriptionFormat, DescriptionArgs);
}

FORCENOINLINE void VARARGS StaticFailDebugV(const TCHAR* Error, const ANSICHAR* File, int32 Line, void* ProgramCounter, const UTF8CHAR* DescriptionFormat, va_list DescriptionArgs)
{
	StaticFailDebugV(Error, /*Expression*/ "", File, Line, /*bIsEnsure*/ false, ProgramCounter, /*Message*/ nullptr, DescriptionFormat, DescriptionArgs);
}
...
void FDebug::LogFormattedMessageWithCallstack(const FName& InLogName, const ANSICHAR* File, int32 Line, const TCHAR* Heading, const TCHAR* Message, ELogVerbosity::Type Verbosity)
{
	...
	if (bWriteUATMarkers)
	{
		UE::Logging::Private::LogDebugMessageLine(File, Line, LogName, Verbosity, TEXT("begin: stack for UAT"));
	}
	...
	{
		UE::Logging::Private::LogDebugMessageLine(File, Line, LogName, Verbosity, Heading);
		UE::Logging::Private::LogDebugMessageLine(File, Line, LogName, Verbosity, TEXT(""));
	}
	...
	for (const TCHAR* LineStart = Message;; )
	{
		...
		*SingleLineWritePos = TEXT('\0');
		...
		{
			UE::Logging::Private::LogDebugMessageLine(File, Line, LogName, Verbosity, SingleLine, Prefix);
		}
	...
	if (bWriteUATMarkers)
	{
		UE::Logging::Private::LogDebugMessageLine(File, Line, LogName, Verbosity, TEXT("end: stack for UAT"));
	}
}
...
FORCENOINLINE void FDebug::EnsureFailed(const ANSICHAR* Expr, const ANSICHAR* File, int32 Line, void* ProgramCounter, const TCHAR* Msg)
{
	...
	va_list DescriptionArgs{};
	StaticFailDebugV(TEXT("Ensure condition failed: "), Expr, File, Line, /*bIsEnsure*/ true, ProgramCounter, Msg, (const TCHAR*)nullptr, DescriptionArgs);

	// Is there a debugger attached?  If not we'll submit an error report.
	if (FPlatformMisc::IsDebuggerPresent() && !GAlwaysReportCrash)
	{
		const FString CombinedErrorMsg = FString::Printf(TEXT("Ensure condition failed: %hs" FILE_LINE_DESC_ANSI "\n%s\n"), Expr, File, Line, Msg);
		FCoreDelegates::OnEnsureFailed.Broadcast(Expr, File, Line, Msg, *CombinedErrorMsg);

#if !NO_LOGGING
		if (GEnsuresAreErrors)
		{
			UE_LOGFMT(LogOutputDevice, Error, "{CombinedErrorMsg}", CombinedErrorMsg);
		}
		else
		{
			UE_LOGFMT(LogOutputDevice, Warning, "{CombinedErrorMsg}", CombinedErrorMsg);
		}
#endif
	}
	else
	{
		...
		TStringBuilder<1024> ErrorMsg;
		ErrorMsg.Appendf(TEXT("Ensure condition failed: %hs " FILE_LINE_DESC_ANSI LINE_TERMINATOR_ANSI "%s" LINE_TERMINATOR_ANSI "Stack: " LINE_TERMINATOR_ANSI), Expr, File, Line, Msg);
		...
		if (StackTrace != NULL)
		{
			...
			FCoreDelegates::OnEnsureFailed.Broadcast(Expr, File, Line, Msg, *ErrorMsg);
		...
		else
		{
			...
			if (GEnsuresAreErrors)
			{
				UE_LOGFMT(LogOutputDevice, Error, "Ensure condition failed: {Expr} [File:{File}] [Line: {Line}]", Expr, File, Line);
			}
			else
			{
				UE_LOGFMT(LogOutputDevice, Warning, "Ensure condition failed: {Expr} [File:{File}] [Line: {Line}]", Expr, File, Line);
			}
#endif
		}
...
void UE_COLD UE_DEBUG_SECTION VARARGS UE::Assert::Private::ProcessLowLevelFatalError(const ANSICHAR* File, int32 Line, const ANSICHAR* Format, ...)
{
	va_list Args;
	va_start(Args, Format);
	StaticFailDebugV(TEXT("LowLevelFatalError"), /*Expression*/ "", File, Line, /*bIsEnsure*/ false, PLATFORM_RETURN_ADDRESS(), /*Message*/ nullptr, (const UTF8CHAR*)Format, Args);
	va_end(Args);
...
void UE_COLD UE_DEBUG_SECTION VARARGS UE::Assert::Private::ProcessLowLevelFatalError(const ANSICHAR* File, int32 Line, const UTF8CHAR* Format, ...)
{
	va_list Args;
	va_start(Args, Format);
	StaticFailDebugV(TEXT("LowLevelFatalError"), /*Expression*/ "", File, Line, /*bIsEnsure*/ false, PLATFORM_RETURN_ADDRESS(), /*Message*/ nullptr, Format, Args);
...
UE_AUTORTFM_ALWAYS_OPEN
bool UE_COLD UE_DEBUG_SECTION UE::Assert::Private::CheckEnsureFailed(bool bAlways, std::atomic<uint8>& bExecuted, const ANSICHAR* File, int32 Line, const ANSICHAR* Expr)
{
	if (((bAlways && GEnsureAlwaysEnabled) || bExecuted.load(std::memory_order_relaxed) != GEnsureResetState) && FPlatformMisc::IsEnsureAllowed())
	{
		if (bExecuted.exchange(static_cast<uint8>(GEnsureResetState), std::memory_order_release) == GEnsureResetState && !bAlways)
		{
			return false;
		}
		...
		return bResult && GEnsureBreakEnabled;
	}
...
{
	if (bExecuted.exchange(static_cast<uint8>(GEnsureResetState), std::memory_order_release) == GEnsureResetState && !(Ensure->bAlways && GEnsureAlwaysEnabled))
	{
		return false;
	}
