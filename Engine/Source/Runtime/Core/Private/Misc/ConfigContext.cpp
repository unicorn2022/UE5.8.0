...
#define LOCTEXT_NAMESPACE "ConfigSystem"

namespace
{
	FString VersionName = "Version";
	FString PreserveName = "Preserve";
	FString LegacyIniVersionString = "IniVersion";
	FString LegacyEngineString = "Engine.Engine";
	FString CurrentIniVersionString = "CurrentIniVersion";
	...
	constexpr int32 KeyFlag_UseGlobalCache = 1 << 0;
	constexpr int32 KeyFlag_UsePluginCache = 1 << 1;
	constexpr int32 KeyFlag_AssumeExists = 1 << 2;
	constexpr int32 KeyFlag_Deprecated_Plugin = 1 << 3;
	constexpr int32 KeyFlag_Deprecated_Other = 1 << 4;
	// if you add a KeyFlag, make sure to increase NumFlagsBits below if needed

	constexpr int32 NumStaticLayerBits = 8; // needs to hold the length of GConfigLayers (or any custom layers, which is highly unlikely to ever be longer than GConfigLayers)
	constexpr int32 NumExpansionBits = 5; // needs to hold the length of GConfigExpansions
	constexpr int32 NumPlatformBits = 5; // needs to hold the length of the longest IniPlatformParent chains (which are generally going always be < 4)
	constexpr int32 NumFlagsBits = 8; // needs to hold KeyFlags above
	...
	static_assert(UE_ARRAY_COUNT(GConfigLayers) < (1 << NumStaticLayerBits), "Need more NumStaticLayerBits");
	static_assert(UE_ARRAY_COUNT(GConfigExpansions) < (1 << NumExpansionBits), "Need more NumExpansionBits");
	static_assert(NumStaticLayerBits + NumExpansionBits + NumPlatformBits + NumFlagsBits <= 32, "All bits combined is greater than 32");
...
UE_TRACE_EVENT_BEGIN(Cpu, FConfigContextLoad, NoSync)
UE_TRACE_EVENT_FIELD(UE::Trace::WideString, BaseName)
UE_TRACE_EVENT_END()
...
bool FConfigContext::Load(const TCHAR* InBaseIniName, FString& OutFinalFilename)
{
	...
	UE_TRACE_LOG_SCOPED_T(Cpu, FConfigContextLoad, CpuChannel)
		<< FConfigContextLoad.BaseName(InBaseIniName);
	...
	if (bIsForPluginModification)
	{
		...
		if (Branch == nullptr)
		{
			...
			UE_LOGF(LogConfig, Warning, "Unable to find branch %ls, making a new one to read plugin layers into. This isn't expected, tell JoshA if you see this", InBaseIniName);
	...
	// when we return, the context is likely to go out of scope, so the TempBranch will be deleted, and the 
	// file better not have a reference to it!
	if (ExistingFile && ExistingFile->Branch == TemporaryBranch)
	{
		ExistingFile->Branch = nullptr;
	}
	return bSuccess;
}
...
static bool LoadAnIniFile(const FString& FilenameToLoad, FConfigFile& ConfigFile)
{
	...
	//UE_LOGF(LogConfig, Warning, "LoadAnIniFile was unable to find FilenameToLoad: %ls ", *FilenameToLoad);
	return false;
}
...
bool FConfigContext::PerformSingleFileLoad()
{
	...
	{
		UE_LOGF(LogConfig, Display, "Tracking dest ini %ls", *DestIniFilename);
...
bool FConfigContext::PrepareForLoad(bool& bPerformLoad)
{
	...
	if (Branch == nullptr)
	{
		...
		if (Branch == nullptr)
		{
			...
			// we allow plugin branches to get wiped if they had no static layers - non-plugins we keep just in case
			// a plugin wants to modify it (a plugin modifying a plugin would require that the modified plugin has
			// actual config data in it)
			if (bIsForPlugin)
			{
				static bool bRemoveEmptyPluginBranches = !FParse::Param(FCommandLine::Get(), TEXT("NoIniPluginBranchClean"));
				bCanRemoveBranchIfEmpty = bRemoveEmptyPluginBranches;
			}
		}
	}
...
bool FConfigContext::PerformLoad()
{
	...
	if (bIsForPluginModification)
	{
		...
		for (FString& File : GatheredFiles)
		{
			...
			UE_LOGF(LogConfig, Verbose, "Loading plugin modification file %ls", *File);
		}
	...
	// if we wiped a branch that had no static files, we cannot continue
	if (bCanRemoveBranchIfEmpty && !bGeneratedFile)
	{
		return false;
	}
...
#define BASIC(x) 	UE_LOGF(LogConfig, Display, "  %s: %ls", #x, *PerformBasicReplacements("{" #x "}", InBaseIniName));
#define EXTRA(x) 	UE_LOGF(LogConfig, Display, "  %s: %ls", #x, *PerformFinalExpansions("{" #x "}", InPlatform));

	UE_LOGF(LogConfig, Display, "Variables for expansion:");
	BASIC(TYPE);
	BASIC(USERSETTINGS);
	BASIC(USER);
	BASIC(CUSTOMCONFIG);
...
void FConfigContext::AddStaticLayersToHierarchy(TArray<FString>* GatheredLayerFilenames, bool bIsForLogging)
{
	...
	for (int32 LayerIndex = 0; LayerIndex < NumLayers; LayerIndex++)
	{
		...
		if (EnumHasAnyFlags(Layer.Flag, EConfigLayerFlags::Deprecated))
		{
			if (Layers == GPluginLayers)
			{
				FlagPartOfKey |= KeyFlag_Deprecated_Plugin;
			}
			else
			{
				FlagPartOfKey |= KeyFlag_Deprecated_Other;
			}
		}
...
// NOTE: For tirst checkin, it will just log a warning, then once we have settled, we can checkin
// more in-your-face behavior
static void ProcessDeprecation(const FString& File, const FText& MessageText)
{
	if (!IsInGameThread())
	{
		AsyncTask(ENamedThreads::GameThread, [File, MessageText]() { ProcessDeprecation(File, MessageText); });
		return;
	}

	FFormatNamedArguments Arguments;
	Arguments.Add(TEXT("File"), FText::FromString(File));
	Arguments.Add(TEXT("Message"), MessageText);
	FText Msg = FText::Format(LOCTEXT("DeprecatedConfigFile", "Found a deprecated '{File}' - {Message}"), Arguments);
	...
	//	{
	//		Message->AddToken(FURLToken::Create(FString::Printf(TEXT("file://%s"), *AbsPath), LOCTEXT("DeprecatedGFPConfig_URLCLick", "Click to open file")));
	//	}
	...
	//ensureAlwaysMsgf(false, TEXT("%s"), *Msg.ToString());
}
...
{
	static bool bDumpIniLoadInfo = FParse::Param(FCommandLine::Get(), TEXT("dumpiniloads"));
	static bool bRemoveMissingLayersFromHierarchy = !FParse::Param(FCommandLine::Get(), TEXT("NoIniHierarchyClean"));
	...
	{
		UE_LOGF(LogConfig, Display, "Branch %ls had commandline override(s), so override NoReplay mode. This will use a bit more memory, but is needed for plugin/GFP handling.", *BaseIniName);
		Branch->ReplayMethod = EBranchReplayMethod::DynamicLayerReplay;
	}
#endif

	bool bReadAnyFile = false;
	TSet<int32> KeysToRemove;
	...
	for (const TPair<int32, FUtf8String>& HierarchyIt : Branch->Hierarchy)
	{
		...
		UE_CLOGF(bDumpIniLoadInfo, LogConfig, Display, "Looking for file: %ls", *IniFileName);
		...
		{
			if (bRemoveMissingLayersFromHierarchy)
			{
				KeysToRemove.Add(HierarchyIt.Key);
			}
			continue;
		}
		...
		UE_CLOGF(bDumpIniLoadInfo, LogConfig, Display, "   Found %ls!", *IniFileName);

		if (HierarchyIt.Key & KeyFlag_Deprecated_Plugin)
		{
			ProcessDeprecation(IniFileName, LOCTEXT("RenameToDefault", "Rename to Default<PluginName>.ini'"));
		}
		if (HierarchyIt.Key & KeyFlag_Deprecated_Other)
		{
			ProcessDeprecation(IniFileName, LOCTEXT("UnknownConfigDeprecation", "Unknown reason, check ConfigHierarchy.h for EConfigLayerFlags::Deprecated that might trigger this."));
		}

		bool bDoEmptyConfig = false;
		//UE_LOGF(LogConfig, Log,  "Combining configFile: %ls", *IniList(IniIndex) );
	...
	// look for some standard dynamic layers that we know to look for now (and not safe reloading, which will already handle this
	// since it will reload all dynamic layers already)
	if (!bIsForPlugin && !ProjectConfigDir.IsEmpty() && !bIsSafeReloading)
	{
		FString ProjectPluginsOverride = FPaths::Combine(ProjectConfigDir, FString::Printf(TEXT("PluginOverride%s.ini"), *BaseIniName));

		// use the DynamicLayers version so we can specify the priority
		if (Branch->AddDynamicLayersToHierarchy({ { ProjectPluginsOverride, NAME_None, (uint16)DynamicLayerPriority::ProjectPluginOverrides } }, nullptr, StagedGlobalConfigCache, nullptr))
		{
			bReadAnyFile = true;
		}
	}
...
bool FConfigContext::GenerateDestIniFile()
{
	if (!bIsFixingUpAfterBinaryConfig)
	{
		...
		if (!LoadIniFileHierarchy())
		{
			if (bCanRemoveBranchIfEmpty)
			{
				ConfigSystem->Remove(DestIniFilename);
				Branch = TemporaryBranch = nullptr;
				return false;
			}
		}
	}
...
#endif
	{
		...
		{
			static bool bDumpIniLoadInfo = FParse::Param(FCommandLine::Get(), TEXT("dumpiniloads"));
			UE_CLOGF(bDumpIniLoadInfo, LogConfig, Display, "Looking for saved user ini file: %ls", *DestIniFilename);
			if (DoesConfigFileExistWrapper(*DestIniFilename, nullptr))
			{
				UE_CLOGF(bDumpIniLoadInfo, LogConfig, Display, "   Found!");
	...
	if (ExistingFile == nullptr && Branch->InMemoryFile.Num() > 0)
	{
		...
		if (Branch->SavedLayer.Num() > 0)
		{
			...
			if (Branch->CombinedStaticLayers.GetInt(*CurrentIniVersionString, *VersionName, SourceConfigVersionNum))
			{
				...
				{
					UE_LOGF(LogInit, Log, "%ls version has been updated. It will be regenerated.", *FPaths::ConvertRelativePathToFull(DestIniFilename));
					bVersionChanged = true;
				}
				else if (SourceConfigVersionNum < CurrentIniVersion)
				{
					UE_LOGF(LogInit, Warning, "%ls version is later than the source. Since the versions are out of sync, nothing will be done.", *FPaths::ConvertRelativePathToFull(DestIniFilename));
				}
			}
