...
static bool appCompressMemoryZLIB(void* CompressedBuffer, int64& CompressedSize, const void* UncompressedBuffer, int64 UncompressedSize, int32 BitWindow, int32 CompLevel)
{
	...
	if (BitWindow == 0 || BitWindow == DEFAULT_ZLIB_BIT_WINDOW)
	{
		...
		{
			UE_LOGF(LogCompression, Error, "Requested a ZLIB compression that doesn't fit in uLong bits");
			return false;
		}
	...
	else
	{
		...
		{
			UE_LOGF(LogCompression, Error, "Requested a ZLIB compression that doesn't fit in uInt bits");
			return false;
		}
...
static bool appCompressMemoryGZIP(void* CompressedBuffer, int64& CompressedSize, const void* UncompressedBuffer, int64 UncompressedSize, int CompressionLevel)
{
	...
	if (InitError != Z_OK)
	{
		UE_LOGF(LogCompression, Error, "Failed to init ZLIB - error code %d", InitError);
		return false;
	}
	ON_SCOPE_EXIT
	{
		...
		if (EndError != Z_OK)
		{
			UE_LOGF(LogCompression, Warning, "Failed to shut down ZLIB - error code %d", EndError);
		}
	};
...
static bool appUncompressMemoryGZIP(void* UncompressedBuffer, int64 UncompressedSize, const void* CompressedBuffer, int64 CompressedSize)
{
	...
	{
		UE_LOGF(LogCompression, Error, "GZIP compression: can't fit in unsigned long: 0x%llx or 0x%llx", UncompressedSize, CompressedSize);
		return false;
	}
	...
	UE_CLOGF(Result == Z_MEM_ERROR, LogCompression, Warning, "appUncompressMemoryGZIP failed: Error: Z_MEM_ERROR, not enough memory!");
	UE_CLOGF(Result == Z_BUF_ERROR, LogCompression, Warning, "appUncompressMemoryGZIP failed: Error: Z_BUF_ERROR, not enough room in the output buffer!");
	UE_CLOGF(Result == Z_DATA_ERROR, LogCompression, Warning, "appUncompressMemoryGZIP failed: Error: Z_DATA_ERROR, input data was corrupted or incomplete!");
	...
	{
		UE_LOG(LogCompression, Warning, TEXT("appUncompressMemoryGZIP failed: Mismatched uncompressed size. Expected: %" INT64_FMT ", Got:%ld. Result: %d"), UncompressedSize, ZUncompressedSize, Result);
...
static bool appUncompressMemoryZLIB( void* UncompressedBuffer, int64 UncompressedSize, const void* CompressedBuffer, int64 CompressedSize, int32 BitWindow )
{
	...
	if (!IntFitsIn<decltype(z_stream::avail_in)>(UncompressedSize) ||
		!IntFitsIn<decltype(z_stream::avail_out)>(CompressedSize))
	{
		UE_LOGF(LogCompression, Error, "ZLIB compression: can't fit in unsigned long: 0x%llx or 0x%llx", UncompressedSize, CompressedSize);
		return false;
	}
	...
	// These warnings will be compiled out in shipping.
	UE_CLOGF(Result == Z_MEM_ERROR, LogCompression, Warning, "appUncompressMemoryZLIB failed: Error: Z_MEM_ERROR, not enough memory! (%ls)", UTF8_TO_TCHAR(stream.msg));
	UE_CLOGF(Result == Z_BUF_ERROR, LogCompression, Warning, "appUncompressMemoryZLIB failed: Error: Z_BUF_ERROR, not enough room in the output buffer! (%ls)", UTF8_TO_TCHAR(stream.msg));
	UE_CLOGF(Result == Z_DATA_ERROR, LogCompression, Warning, "appUncompressMemoryZLIB failed: Error: Z_DATA_ERROR, input data was corrupted or incomplete! (%ls)", UTF8_TO_TCHAR(stream.msg));
	...
	if( UncompressedSize != ZUncompressedSize )
	{
		UE_LOG( LogCompression, Warning, TEXT("appUncompressMemoryZLIB failed: Mismatched uncompressed size. Expected: %" INT64_FMT ", Got:%ld. Result: %d"), UncompressedSize, ZUncompressedSize, Result );
		bOperationSucceeded = false;
	}
	return bOperationSucceeded;
}
...
static bool appUncompressMemoryStreamZLIB(void* UncompressedBuffer, int64 UncompressedSize, IMemoryReadStream* Stream, int64 StreamOffset, int64 CompressedSize, int32 BitWindow)
{
	...
	{
		UE_LOGF(LogCompression, Error, "ZLIB compression: can't fit in unsigned long: 0x%llx or 0x%llx", UncompressedSize, ChunkSize);
		return false;
	}
	...
	while (Result == Z_OK)
	{
		if (stream.avail_in == 0u)
		{
			...
			if (!IntFitsIn<decltype(z_stream::avail_in)>(ChunkSize))
			{
				UE_LOGF(LogCompression, Error, "ZLIB compression: can't fit in unsigned long: 0x%llx", ChunkSize);
				return false;
			}
	...
	// These warnings will be compiled out in shipping.
	UE_CLOGF(Result == Z_MEM_ERROR, LogCompression, Warning, "appUncompressMemoryStreamZLIB failed: Error: Z_MEM_ERROR, not enough memory!");
	UE_CLOGF(Result == Z_BUF_ERROR, LogCompression, Warning, "appUncompressMemoryStreamZLIB failed: Error: Z_BUF_ERROR, not enough room in the output buffer!");
	UE_CLOGF(Result == Z_DATA_ERROR, LogCompression, Warning, "appUncompressMemoryStreamZLIB failed: Error: Z_DATA_ERROR, input data was corrupted or incomplete!");
...
void FCompression::Init(void)
{
	FScopeLock Lock(&CompressionFormatsCriticalSection);
	
	// CompressionFormatInitOnFirstUseFromLock is safe to call multiple times
...
ICompressionFormat* FCompression::GetCompressionFormat(FName FormatName, bool bErrorOnFailure)
{
	...
	if (ExistingFormat == nullptr)
	{
		...
		{
			// CompressionFormatInitOnFirstUseFromLock is a nop if already done
			// this should now usually be done by FCompression::Init from LaunchEngineLoop
			// but we need to keep this init-on-first-use here for the test harness
		...
		if (ExistingFormat == nullptr)
		{
			if (bErrorOnFailure)
			{
				UE_LOGF(LogCompression, Error, "FCompression::GetCompressionFormat - Unable to find a module or plugin for compression format %ls", *FormatName.ToString());
			}
			else
			{
				UE_LOGF(LogCompression, Display, "FCompression::GetCompressionFormat - Unable to find a module or plugin for compression format %ls", *FormatName.ToString());
			}
			return nullptr;
		}
	}
...
bool FCompression::GetMaximumCompressedSize(FName FormatName, int64& OutMaxCompressedSize, int64 UncompressedSize, uintptr_t CompressionData)
{
	...
	if (UncompressedSize < 0)
	{
		...
		UE_LOGF(LogCompression, Error, "Negative value passed to GetMaximumCompressedSize (0x%llx)", UncompressedSize);
		return false;
	}
...
bool FCompression::CompressMemoryBound(FName FormatName, int64& OutBufferSizeRequired, int64 UncompressedSize, uintptr_t CompressionData)
{
	...
	if (UncompressedSize < 0)
	{
		UE_LOGF(LogCompression, Error, "Negative value passed to CompressMemoryBound (0x%llx)", UncompressedSize);
		return false;
	}
	...
	else if (FormatName == NAME_Zlib)
	{
		...
		{
			UE_LOGF(LogCompression, Error, "Zlib doesn't support >32 bit sizes (0x%llx)", UncompressedSize);
			return false;
		}
		...
		else
		{
			...
			{
				UE_LOGF(LogCompression, Error, "Zlib CompressMemoryBound calculated negative value 0x%llx -> 0x%llx", UncompressedSize, OutBufferSizeRequired);
				return false;
			}
		}
		return true;
	...
	else if (FormatName == NAME_Gzip)
	{
		...
		{
			UE_LOGF(LogCompression, Error, "Gzip doesn't support >32 bit sizes (0x%llx)", UncompressedSize);
			return false;
		}
		...
		if (OutBufferSizeRequired < 0)
		{
			UE_LOGF(LogCompression, Error, "Gzip CompressMemoryBound calculated negative value 0x%llx -> 0x%llx", UncompressedSize, OutBufferSizeRequired);
			return false;
		}
		return true;
	}
	else if (FormatName == NAME_LZ4)
	{
		...
		{
			UE_LOGF(LogCompression, Error, "LZ4 doesn't support >32 bit sizes (0x%llx) max is 0x%x", UncompressedSize, LZ4_MAX_INPUT_SIZE);
			return false;
		}
		OutBufferSizeRequired = LZ4_compressBound((int)UncompressedSize);
		if (OutBufferSizeRequired < 0)
		{
			UE_LOGF(LogCompression, Error, "LZ4 CompressMemoryBound calculated negative value 0x%llx -> 0x%llx", UncompressedSize, OutBufferSizeRequired);
			return false;
		}
		return true;
	}
	...
	{
		UE_LOGF(LogCompression, Error, "GetCompressedBufferSize for format %ls failed to return compression bound: check bits needed? (0x%llx)", *FormatName.ToString(), UncompressedSize);
		return false;
	}

	if (OutBufferSizeRequired < 0)
	{
		UE_LOGF(LogCompression, Error, "%ls CompressMemoryBound calculated negative value 0x%llx -> 0x%llx", *FormatName.ToString(), UncompressedSize, OutBufferSizeRequired);
		return false;
	}
...
int32 FCompression::CompressMemoryBound(FName FormatName, int32 UncompressedSize, ECompressionFlags Flags, int32 CompressionData)
{
	...
	{
		UE_LOGF(LogCompression, Fatal, "CompressMemoryBound failed, check sizes/format (%d, %ls)", UncompressedSize, *FormatName.ToString());
		return -1;
	}
...
bool FCompression::CompressMemoryIfWorthDecompressing(FName FormatName, int32 MinBytesSaved, int32 MinPercentSaved, void* CompressedBuffer, int32& CompressedSize, const void* UncompressedBuffer, int32 UncompressedSize, ECompressionFlags Flags, int32 CompressionData)
{
	...
	{
		UE_LOGF(LogCompression, Fatal, "CompressMemoryIfWorthDecompressing failed, check sizes/format (%d, %ls)", UncompressedSize, *FormatName.ToString());
		return false;
	}
...
bool FCompression::CompressMemoryIfWorthDecompressing(FName FormatName, bool& bOutWasCompressed, int64 MinBytesSaved, int32 MinPercentSaved, void* CompressedBuffer, int64& CompressedSize, const void* UncompressedBuffer, int64 UncompressedSize, ECompressionFlags Flags, uintptr_t CompressionData)
{
	...
	if (UncompressedSize < -1 ||
		CompressedSize < -1)
	{
		UE_LOGF(LogCompression, Error, "Negative value passed to CompressMemoryIfWorthDecompressing (0x%llx / 0x%llx)", UncompressedSize, CompressedSize);
		return false;
	}
...
bool FCompression::CompressMemory(FName FormatName, void* CompressedBuffer, int32& CompressedSize, const void* UncompressedBuffer, int32 UncompressedSize, ECompressionFlags Flags, int32 CompressionData)
{
	...
	UE_AUTORTFM_OPEN
	{
		bSucceeded = CompressMemory(FormatName, CompressedBuffer, CompressedSize64, UncompressedBuffer, UncompressedSize, Flags, CompressionData);
	};
	if (!IntFitsIn<int32>(CompressedSize64))
	{
		UE_LOGF(LogCompression, Error, "Compressing 32 bit memory size ended up a 64 bit size! %d -> %lld", UncompressedSize, CompressedSize64);
		return false;
	}
	CompressedSize = (int32)CompressedSize64;
	return bSucceeded;
...
bool FCompression::CompressMemory(FName FormatName, void* CompressedBuffer, int64& CompressedSize, const void* UncompressedBuffer, int64 UncompressedSize, ECompressionFlags Flags, uintptr_t CompressionData)
{
	...
	else if (FormatName == NAME_LZ4)
	{
		...
		if (UncompressedSize > LZ4_MAX_INPUT_SIZE)
		{
			UE_LOGF(LogCompression, Error, "LZ4 can't compress larger than 0x%x (passed 0x%llx)", LZ4_MAX_INPUT_SIZE, UncompressedSize);
			return false;
		}
...
bool FCompression::UncompressMemory(FName FormatName, void* UncompressedBuffer, int64 UncompressedSize, const void* CompressedBuffer, int64 CompressedSize, ECompressionFlags Flags, uintptr_t CompressionData)
{
	...
	else if (FormatName == NAME_LZ4)
	{
		...
		{
			UE_LOGF(LogCompression, Error, "LZ4 can't fit in int: 0x%llx or 0x%llx", CompressedSize, UncompressedSize);
			return false;
		}
