...
// if set to 1, this will make sure FConfigCacheIni::GetString (and friends) is thread safe from dynamic layer loading, and SafeReload
// if set to 0, this will behave as 2025 and before (no safety for GetString)
// note that if you bypass FConfigCache::Get* and use FConfigFile::Get*, then it will not be threadsafe
#define UE_THREAD_SAFETY_PROTECT_GETSTRING 1

// if set to 1, GetString (GetInt, etc) will be slower, but SafeUnload will unload more memory
// if set to 0, SafeUnload will free less memory, but Get* will be faster
#define UE_THREAD_SAFETY_USE_WRITE_LOCKS 1

namespace
{
	const FString CurrentIniVersionStr = "CurrentIniVersion";
	const FName VersionSectionName = "Version";
	const FString SectionsToSaveStr = "SectionsToSave";
...
static int GSkipSaveIfPreSaveSucceeded = 1;
static FAutoConsoleVariableRef CVarSkipSaveIfPreSaveSucceeded(
	TEXT("ini.SkipSaveIfPreSaveSucceeded"),
	GSkipSaveIfPreSaveSucceeded,
	TEXT("If true, then the standard config file save will be skipped if the PreSave delegate indicates someone saved."));
...
/**
 * Stores some values that are helpful for logging but would be cumbersome to pass around to every internal function.
 */
struct FConfigLogContext
{
	FConfigLogContext* OuterContext = nullptr;
	int32 ContextDepth = 0;
...
thread_local static FConfigLogContext ConfigLogContext;

/**
 * Pushes a new set of logging context values for the current scope.
 */
struct FConfigLogContextScope
{
	FConfigLogContext PrevLogContext;

	FConfigLogContextScope(const TCHAR* InAction, const TCHAR* InSection, const TCHAR* InKey)
	{
		PrevLogContext = ConfigLogContext;

		// Only set the outer pointer if this isn't the first scope
		if (ConfigLogContext.ContextDepth > 0)
		{
			ConfigLogContext.OuterContext = &PrevLogContext;
		}

		ConfigLogContext.ContextDepth++;
		ConfigLogContext.Action = InAction;
		ConfigLogContext.Section = InSection;
		ConfigLogContext.Key = InKey;
	}

	~FConfigLogContextScope()
	{
		// Verify OuterContext pointer was not modified
		check((ConfigLogContext.OuterContext == nullptr) || (ConfigLogContext.OuterContext == &PrevLogContext));

		ConfigLogContext = PrevLogContext;
	}
};

/**
 * Gets the "outermost" log context. Useful for determining which function was called from outside code.
 * e.g. FindConfigFile can be called directly, but is also called by Find and FindPlatformConfig.
 */
FConfigLogContext* GetOutermostConfigLogContext()
{
	FConfigLogContext* Result = &ConfigLogContext;
	while (Result->OuterContext)
	{
		Result = Result->OuterContext;
	}
...
static void CheckLongSectionNames(const TCHAR* Section, const FConfigFile* File)
{
	if (!FPlatformProperties::RequiresCookedData())
	{
		...
		if (FCString::Strnicmp(Section, TEXT("/Script/"), 8) == 0)
		{
			...
			{
				UE_LOGF(LogConfig, Fatal, "Short config section found while looking for %ls", Section);
			}
		}
		else
		{
			// Section is a short name
			FString LongName = FString("/Script/") + Section;
			if (File->FindSection(*LongName))
			{
				UE_LOGF(LogConfig, Fatal, "Short config section used instead of long %ls", Section);
			}
		}
	}
}
...
bool DoesConfigFileExistWrapper(const TCHAR* IniFile, const TSet<FString>* IniCacheSet, const TSet<FString>* PrimaryConfigFileCache, const TSet<FString>* SecondaryConfigFileCache)
{
	...
	UE_LOGF(LogConfig, VeryVerbose, "Looking for a config file without a staged cache for %ls", IniFile);
...
static bool SaveConfigFileWrapper(const TCHAR* IniFile, const FString& Contents)
{
	...
	// save if the delegate didn't, or it did and we don't want to skip the normal save
	bool bLocalWriteSucceeded = false;
	if (SavedCount == 0 || GSkipSaveIfPreSaveSucceeded == false)
	{
...
bool FConfigFile::Combine(const FString& Filename, bool bHandleSymbolCommands)
{
	return FillFileFromDisk(Filename, bHandleSymbolCommands);
}
...
void LogOrEditorWarning(const FText& Msg, const FString& PartialKey, const FString& File)
{
	...
	// always spit to log
	UE_LOGF(LogConfig, Warning, "%ls", *Msg.ToString());
}
		...
		if (FPaths::IsUnderDirectory(RelativePath, FPaths::RootDir()))
		{
			...
			for (int32 Index = 0; Index < Files.Num(); Index++)
			{
				...
				if (NormalizedOverride.EndsWith(RelativePath))
				{
					...
					UE_LOGF(LogConfig, Warning, "Loading override ini file: %ls ", *Files[Index]);
					return true;
				}
			}
		}
...
void FillFileFromBuffer(FileType* File, FStringView Buffer, bool bHandleSymbolCommands, const FString& FileHint)
{
	...
	static const FName ConfigFileClassName = "ConfigFile";
				...
				if (Value != Start)
				{
					...
					*Value++ = TEXT('\0');
					...
					if (bHandleSymbolCommands)
					{
						...
						if (Lookup == nullptr)
						{
							UE_LOGF(LogConfig, Log, "Found unknown ini command %c in an ini", Cmd);
							continue;
						}
						Command = *Lookup;
					}
...
void FConfigFile::CombineFromBuffer(const FString& Buffer, const FString& FileHint, bool bHandleSymbolCommands)
{
	::FillFileFromBuffer(this, Buffer, bHandleSymbolCommands, FileHint);
}
...
bool FConfigFile::Write(const FString& Filename, bool bDoRemoteWrite/* = true*/, const FString& PrefixText/*=FString()*/)
{
	...
	return WriteInternal(Filename, bDoRemoteWrite, SectionTexts, SectionOrder, nullptr);
}
...
bool FConfigFile::WriteInternal(const FString& Filename, bool bDoRemoteWrite, TMap<FString, FString>& InOutSectionTexts, const TArray<FString>& InSectionOrder, FString* OriginalContents)
{
	int32 IniCombineThreshold = MAX_int32;
	...
	for( TIterator SectionIterator(*this); SectionIterator; ++SectionIterator )
	{
		...
		if (Branch != nullptr && Branch->FinalCombinedLayers.Num() > 0)
		{
			...
			if (!SourceConfigSection && FPlatformProperties::RequiresCookedData() == false && SectionName.StartsWith(TEXT("/Script/")))
			{
				...
				{
					UE_LOGF(LogConfig, Fatal, "Short config section found while looking for %ls", *SectionName);
				}
			}
#endif
		}
...
void FConfigFile::SaveSourceToBackupFile()
{
	...
	{
		UE_LOGF(LogConfig, Warning, "Failed to saved backup for config[%ls]", *Filename);
	}
}
...
bool FConfigBranch::AddDynamicLayerToHierarchy(const FString& Filename, FConfigModificationTracker* ModificationTracker, const TSet<FString>* GlobalConfigFileCache, const TSet<FString>* PluginConfigFileCache)
{
...
bool FConfigBranch::AddDynamicLayersToHierarchy(const TArray<FDynamicLayerInfo>& Layers, FConfigModificationTracker* ModificationTracker, const TSet<FString>* GlobalConfigFileCache, const TSet<FString>* PluginConfigFileCache, bool bForceFullDynamicLayerUpdate)
{
#if UE_THREAD_SAFETY_PROTECT_GETSTRING
	// don't allow a read while modifying it here
	UE::TUniqueLock FileWriteLock(InMemoryFile.SharedMutex);
#endif
	...
	{
		UE_CLOGF(bDumpIniLoadInfo, LogConfig, Display, "Looking for file: %ls", *Layer.Filename);

		UE_LOGF(LogConfig, Verbose, "Adding Dynamic layer %ls to Branch %ls", *Layer.Filename, *IniName.ToString());

		if (!DoesConfigFileExistWrapper(*Layer.Filename, nullptr, GlobalConfigFileCache, PluginConfigFileCache))
		{
			UE_LOGF(LogConfig, Verbose, "  .. doesn't exist!");
			continue;
		}

		bFoundAFile = true;
		UE_CLOGF(bDumpIniLoadInfo, LogConfig, Display, "   Found %ls!", *Layer.Filename);

		if (AddedLayers.Num() == 0 && !bForceFullDynamicLayerUpdate)
		{
			Patch = CalculateDiff(FinalCombinedLayers, InMemoryFile);
			UE_LOGF(LogConfig, Verbose, "  .. calculating diff on first file");
		}
		...
		{
			UE_LOGF(LogConfig, Verbose, "  .. no replay, so just adding at end");
		}
		else
		{
			...
			for (DynamicLayerList::TIterator Node(DynamicLayers.GetHead()); Node; ++Node)
			{
				...
				{
					UE_LOGF(LogConfig, Verbose, "  .. inserted in middle of dynamic layers");
			...
			if (!bInserted)
			{
				UE_LOGF(LogConfig, Verbose, "  .. inserting at end of layers");
		...
		if (ModificationTracker != nullptr)
		{
			...
			{
				UE_LOGF(LogConfig, Verbose, "  .. tracking sections:");
				for (const TPair<FString, FConfigCommandStreamSection>& Pair : *DynamicLayer)
				{
					...
					if (FConfigModificationTracker::FCVarTracker* CVarTracker = ModificationTracker->CVars.Find(Pair.Key))
					{
						...
						{
							UE_LOGF(LogConfig, Verbose, "  .. .. .. .. %ls = %ls", *CVarPair.Key.ToString(), *CVarPair.Value.GetSavedValue());
	...
	if (AddedLayers.Num() > 0 || bForceFullDynamicLayerUpdate)
	{
		...
		if (!bInsertedBeforeEnd && !bForceFullDynamicLayerUpdate && CommandLineOverrides.Num() == 0)
		{
			...
			{
				UE_LOGF(LogConfig, Verbose, "  .. reapplying layer with %d sections", NewLayer->Num());
		...
		else
		{
			...
			UE_LOGF(LogConfig, Verbose, "  .. reapplying all dynamic layers");
			for (DynamicLayerList::TIterator Node(DynamicLayers.GetHead()); Node; ++Node)
			{
...
bool FConfigBranch::RemoveDynamicLayersFromHierarchy(const TArray<FString>& Filenames, FConfigModificationTracker* ModificationTracker)
{
...
#if UE_THREAD_SAFETY_PROTECT_GETSTRING
	// this can modify InMemoryFile, so we need to protect it 
	UE::TUniqueLock WriteFileLock(InMemoryFile.SharedMutex);
#endif
	...
	if (ReplayMethod == EBranchReplayMethod::NoReplay)
	{
		UE_LOGF(LogConfig, Warning, "Attempted to remove dynamic layer(s) from branch %ls, but it is using NoReplay mode, so this cannot work. Skipping.", *IniName.ToString());
		return false;
	}
...
void FConfigBranch::RemoveTagsFromHierarchy(const TArray<FName>& Tags, FConfigModificationTracker* ModificationTracker)
{
	...
	for (DynamicLayerList::TIterator Node(DynamicLayers.GetHead()); Node; ++Node)
	{
		...
		{
			UE_LOGF(LogConfig, Verbose, "Removing dynamic layer %ls from branch %ls with tag %ls", *Node->Filename, *IniName.ToString(), *Node->Tag.ToString());
...
void FConfigBranch::SafeUnload()
{
#if UE_THREAD_SAFETY_PROTECT_GETSTRING
	// this can modify InMemoryFile, so we need to protect it 
	UE::TUniqueLock WriteLock(InMemoryFile.SharedMutex);
#endif
	
	bIsSafeUnloaded = true;
	InactiveTimer = 0;

#if UE_THREAD_SAFETY_USE_WRITE_LOCKS
	// if we are not using full Write Locks, then we we no longer unload the final combined data
	// this makes thread safety much simpler at the expense of some memory. because we don't clear 
	// the InMemoryFile, we don't actually need to SafeReload to get the final value, only when we 
	// add/remove layers or similar. 
	// This block is the low-memory, slower threading, mode
	// advanced behavior
	...
	FCoreDelegates::TSOnConfigBranchUnloaded().Broadcast(*IniName.ToString());
}
...
void FConfigBranch::SafeReload()
{
	...
	// in case another thread reloaded while we were blocked, skip this
	if (!bIsSafeUnloaded)
	{
		return;
	}
	...
	const FString IniNameStr = IniName.ToString();
	const float ReloadTimeMs = (float)(FPlatformTime::Seconds() - StartTime) * 1000.0f;
	UE_LOGF(LogConfig, Log, "Branch '%ls' had been unloaded. Reloading on-demand took %.2fms", *IniNameStr, ReloadTimeMs);

	const FConfigLogContext* LogContext = GetOutermostConfigLogContext();
	const TCHAR* ContextAction = LogContext->Action ? LogContext->Action : TEXT("");
	const TCHAR* ContextSection = LogContext->Section ? LogContext->Section : TEXT("");
	const TCHAR* ContextKey = LogContext->Key ? LogContext->Key : TEXT("");
	UE_LOGF(LogConfig, Log, "Reload Context for thread %ls: Action=%ls Section=%ls Key=%ls", *FThreadManager::GetThreadName(FPlatformTLS::GetCurrentThreadId()), ContextAction, ContextSection, ContextKey);

	FCoreDelegates::TSOnConfigBranchReloaded().Broadcast(*IniNameStr, ReloadTimeMs);
}

void FConfigBranch::UpdateInactiveTimer()
{
	if (InactiveTimer >= 0 && GTimeToUnloadConfig > 0)
	{
		InactiveTimer = FPlatformTime::Seconds();
		UE_LOGF(LogConfig, Verbose, "Resetting InactiveTimer for %ls", *IniName.ToString());
	}
}
...
void FConfigCacheIni::Tick(float DeltaSeconds)
{
	...
	for (FConfigBranch* Branch : BranchesToCheck)
	{
		...
		else if (Branch->InactiveTimer > 0)
		{
			...
			{
				UE_LOGF(LogConfig, Log, "Unloading %ls due to inactivity", *Branch->IniPath);
...
FConfigBranch* FConfigCacheIni::FindBranchWithNoReload(FName BaseIniName, const FString& Filename)
{
	...
	FConfigBranch* Branch = BaseIniName == NAME_None ? nullptr : KnownFiles.GetBranch(BaseIniName);
	...
	if (Branch)
	{
		...
		Branch->UpdateInactiveTimer();
	}

	return Branch;
}

FConfigBranch* FConfigCacheIni::FindBranchWithDynamicLayerTag(FName BaseIniName, FName Tag)
{
	FConfigBranch* Branch = FindBranch(BaseIniName, "");
...
{
	FConfigLogContextScope LogScope(TEXT("FindBranch"), nullptr, nullptr);
...
{
	FConfigLogContextScope LogScope(TEXT("FindConfigFile"), nullptr, nullptr);
...
{
	FConfigLogContextScope LogScope(TEXT("Find"), nullptr, nullptr);
	...
	if (!Result && !bAreFileOperationsDisabled)
	{
		...
		if (!Result)
		{
			if (DoesConfigFileExistWrapper(*UnrealFileName))
			{
				...
				UE_LOGF(LogConfig, Verbose, "GConfig::Find is looking for file:  %ls", *UnrealFileName);
				{
					...
					UE_LOGF(LogConfig, Verbose, "GConfig::Find has loaded file:  %ls", *UnrealFileName);
				}
			}
		}
		else
		{
			...
			UE_LOGF(LogConfig, Warning, "GConfig::Find attempting to access config with non-normalized path %ls. Please use FConfigCacheIni::NormalizeConfigIniPath (which would make generate %ls) before accessing INI files through ConfigCache.", *Filename, *UnrealFileName);
		}
	}
...
{
	FConfigLogContextScope LogScope(TEXT("FindConfigFileWithBaseName"), nullptr, nullptr);
...
bool FConfigCacheIni::Flush(bool bRemoveFromCache, const FString& Filename)
{
	...
	if (Type != EConfigCacheType::Temporary)
	{
		...
		if (!bAreFileOperationsDisabled)
		{
			if (Filename.Len() > 0)
			{
				...
				{
					bSuccess = SaveBranch(*Branch) && bSuccess;
				}
			}
			else
			{
				...
				{
					bSuccess = SaveBranch(*Pair.Value) && bSuccess;
				}
	...
	if (bRemoveFromCache)
	{
		...
		if (bAreFileOperationsDisabled)
		{
			UE_LOGF(LogConfig, Warning, "Tried to flush the config cache and read it back in, but File Operations are disabled!!");
		}
...
{
	FConfigLogContextScope LogScope(TEXT("Parse1ToNSectionOfNames"), Section, KeyOne);
...
{
	FConfigLogContextScope LogScope(TEXT("Parse1ToNSectionOfStrings"), Section, KeyOne);
...
void FConfigCacheIni::LoadFile( const FString& Filename, const FConfigFile* Fallback, const TCHAR* PlatformString )
{
	...
	if( !IsUsingLocalIniFile(*Filename, nullptr) || DoesConfigFileExistWrapper(*Filename) )
	{
		...
		UE_LOGF(LogConfig, Verbose, "GConfig::LoadFile has loaded file:  %ls", *Filename);
	}
	else if( Fallback )
	{
		Add( *Filename, *Fallback );
		UE_LOGF(LogConfig, Verbose, "GConfig::LoadFile associated file:  %ls", *Filename);
	}
	else
	{
		UE_LOGF(LogConfig, Warning, "FConfigCacheIni::LoadFile failed loading file as it was 0 size.  Filename was:  %ls", *Filename);
	}
}
...
static FConfigBranch* FindOrLoadNoSafeReload(FConfigCacheIni* ConfigSystem, const FString& Filename)
{
	// check for non-filenames
	if(Filename.Len() == 0)
	{
	...
	// we need to get the branch without reloading here so that we can put the Reload and Get() be protected
	// by the same block, which is held buy the the Branch/File we are looking up
	FConfigBranch* Branch = ConfigSystem->FindBranchWithNoReload(NAME_None, Filename);
	if (Branch == nullptr)
	{
		// this will load if possible, with Normalized path (and log a warning if it wasn't match)
		ConfigSystem->Find(*Filename);
		
		// need to find with Normalized path
		Branch = ConfigSystem->FindBranchWithNoReload(NAME_None, ConfigSystem->NormalizeConfigIniPath(Filename));
	}
...
{
	FConfigLogContextScope LogScope(TEXT("GetString"), Section, Key);

	// get the file without a SafeReload possible, so no lock is needed
	...
	// due to some rare-but-possible edge cases, we need to either keep InMemoryFile around when we 
	// SafeUnload, and _not_ call SafeReload here, or we need a writelock around both the Unload and the
	// GetString functionality (or we would Unload between the Reload call and the Get). A ReadLock isn't
	// enough, and was causing deadlocks
#if UE_THREAD_SAFETY_USE_WRITE_LOCKS
	// don't read while something may be modifying the File (generally a Branch's InMmeoryFile)
	// frustratingly we need 
	UE::TUniqueLock WriteLock(Branch->InMemoryFile.SharedMutex);
...
#else
	// we do need a ReadLock because dynamiclayers can still modify InMemoryFile
	// (if we did a SafeReload, we would need a WriteLock which makes this function ~2-4x slower!)
	UE::TSharedLock ReadLock(Branch->InMemoryFile.SharedMutex);	
#endif
	
	FRemoteConfig::Get()->FinishRead(*Filename); // Ensure the remote file has been loaded and processed

	
	const FConfigSection* Sec = Branch->InMemoryFile.FindSection( Section );
	if( !Sec )
	{
#if !UE_BUILD_SHIPPING
		CheckLongSectionNames( Section, &Branch->InMemoryFile );
#endif
		return false;
	}
...
{
	FConfigLogContextScope LogScope(TEXT("GetText"), Section, Key);
...
{
	FConfigLogContextScope LogScope(TEXT("GetSection"), Section, nullptr);
...
{
	FConfigLogContextScope LogScope(TEXT("DoesSectionExist"), Section, nullptr);
...
bool FConfigCacheIni::EmptySectionsMatchingString( const TCHAR* SectionString, const FString& Filename )
{
	...
	if (File)
	{
		...
		TArray<FString> SectionsToEmpty;
		for (FConfigFile::TConstIterator It = File->CreateConstIterator(); It; ++It)
		{
			if (It.Key().Contains(SectionString))
			{
				SectionsToEmpty.Add(It.Key());
			}
		}
				
		for (const FString& SectionNameString : SectionsToEmpty)
		{
			bEmptied |= EmptySection(*SectionNameString, Filename);
		}
...
{
	FConfigLogContextScope LogScope(TEXT("GetSectionNames"), nullptr, nullptr);
...
{
	FConfigLogContextScope LogScope(TEXT("GetPerObjectConfigSections"), *SearchClass, nullptr);
...
bool FConfigCacheIni::CreateGConfigFromSaved(const TCHAR* Filename)
{
	...
	FString BinaryConfigScalabilityOverride;
	FParse::Value(FCommandLine::Get(), TEXT("BinaryConfigScalabilityOverride="), BinaryConfigScalabilityOverride);
	bool bIsConfigScalabilityOverride = !BinaryConfigScalabilityOverride.IsEmpty();
	TArray<uint8> FileContent;
	if (bIsConfigScalabilityOverride)
	{
		FString BinaryConfig = FString::Printf(TEXT("%sBinaryConfig%s.ini"), *FPaths::ProjectConfigDir(), *BinaryConfigScalabilityOverride);
		if (FFileHelper::LoadFileToArray(FileContent, *BinaryConfig, FILEREAD_Silent))
		{
			// Free immediately the default Binary Config preloaded Data
			if (PreloadedData != nullptr)
			{
				FMemory::Free(PreloadedData);
			}

			// Set the Preload Data to the Override Scalability Binary Config file
			PreloadedData = FileContent.GetData();
			Size = FileContent.Num();
		}
		else
		{
			UE_LOGF(LogConfig, Log, "BinaryConfigScalabilityOverride was set to %ls but BinaryConfig %ls could not be loaded. Reverting to default BinaryConfig.ini file.",
				*BinaryConfigScalabilityOverride, *BinaryConfig);
			bIsConfigScalabilityOverride = false;
		}
	}
	...
	UE_LOGF(LogInit, Display, "Loaded binary GConfig from %lld bytes of data...", Size);
	if (!bIsConfigScalabilityOverride)
	{
		FMemory::Free(PreloadedData);
	}
...
#endif
	{
		...
		if (!bIsGamelessExe)
		{
			...
			if (GIsGameAgnosticExe && !bEngineConfigCreated)
			{
				...
				UE_LOGF(LogInit, Fatal, "%ls", *Message.ToString());
			}
		}

		// load editor, etc config files
		LoadRemainingConfigFiles(Context);
	#endif // UE_ENABLE_CONFIG_SYSTEM
	}
...
const FString& FConfigCacheIni::GetCustomConfigString()
{
	...
	if (!bInitialized)
	{
		...
		if (FParse::Value(FCommandLine::Get(), CommandlineOverrideSpecifiers::CustomConfigIdentifier, CustomConfigString))
		{
			...
			UE_LOGF(LogConfig, Log, "Overriding CustomConfig from %ls to %ls using -customconfig cmd line param", TEXT(CUSTOM_CONFIG), *CustomConfigString);
		}
#endif

#ifdef UE_USE_COMMAND_LINE_PARAM_FOR_CUSTOM_CONFIG
		FString CustomName = UE_STRINGIZE(UE_USE_COMMAND_LINE_PARAM_FOR_CUSTOM_CONFIG);
		if (!bCustomConfigOverrideApplied && FParse::Param(FCommandLine::Get(), *CustomName))
		{
			bCustomConfigOverrideApplied = true;
			CustomConfigString = CustomName;
			UE_LOGF(LogConfig, Log, "Overriding CustomConfig from %ls to %ls using a custom cmd line param", TEXT(CUSTOM_CONFIG), *CustomConfigString);
		}
#endif

		if (!bCustomConfigOverrideApplied && !CustomConfigString.IsEmpty())
		{
			UE_LOGF(LogConfig, Log, "Using compiled CustomConfig %ls", *CustomConfigString);
		}
	}
...
{
	FConfigLogContextScope LogScope(TEXT("FindPlatformConfig"), nullptr, nullptr);
...
{
	FConfigLogContextScope LogScope(TEXT("FindOrLoadPlatformConfig"), nullptr, nullptr);
...
void FConfigFile::UpdateSections(const TCHAR* DiskFilename, const TCHAR* IniRootName/*=nullptr*/, const TCHAR* OverridePlatform/*=nullptr*/)
{
	...
	// pass in DiskFile, so it will not touch the disk if nothing changes
	WriteInternal(DiskFilename, true, SectionTexts, SectionOrder, &DiskFile);
}
...
void FConfigCacheIni::AddPluginsToBranches(const TArray<FName>& PluginNames, FConfigModificationTracker* ModificationTracker)
{
	...
	for (FName PluginName : PluginNames)
	{
		...
		FPluginInfo* PluginInfo = nullptr;
		{
			...
			{
				UE_LOGF(LogConfig, Warning, "Attempting to load a dynamic plugin (%ls) that was not registered ahead of time!", *PluginName.ToString());
				return;
			}
		}
		...
		{
			// make sure this file isn't in a subdir (like Tags), this will make the StagedPluginConfigCache case match the SlowPluginConfigs case
			FString DirName = FPaths::GetCleanFilename(FPaths::GetPath(ConfigFilename));
			if (!DirName.IsEmpty() && DirName != TEXT("Config") && DirName != PlatformNameStr)
			{
				UE_LOGF(LogConfig, Verbose, "Skipping config file %ls in plugin dir %ls, since it's in a subdirectory.", *ConfigFilename, *PluginInfo->PluginDir);
				continue;
			}
			UE_LOGF(LogConfig, Verbose, "Found config file %ls in plugin dir %ls", *ConfigFilename, *PluginInfo->PluginDir);
			...
			if (ModificationTracker && ModificationTracker->bSkipLoadingAlreadyLoadedFiles)
			{
				...
				{
					UE_LOGF(LogConfig, Verbose, "Skipping already loaded file %ls", *FullPath);
					continue;
				}
			}
			...
			if (Branch == nullptr)
			{
				...
				UE_CLOGF(PlatformName == CurrentPlatform, LogConfig, Verbose, "Found unknown .ini file %ls in plugindir %ls", *ConfigFilename, *PluginInfo->PluginDir);
				continue;
			}
			
			UE_LOGF(LogConfig, Verbose, "Modifying branch %ls with plugin ini %ls", *BranchName.ToString(), *ConfigFilename);
...
void FConfigCacheIni::AsyncInitializeConfigForPlatforms()
{
	...
	for (const TPair<FName, FDataDrivenPlatformInfo>& Pair : AllPlatformInfos)
	{
		...
		GetPlatformConfigFutures()[PlatformName] = Async(EAsyncExecution::ThreadPool, [PlatformName]
		{
			...
			UE_LOGF(LogConfig, Display, "Loading %ls ini files took %.2f seconds", *PlatformName.ToString(), FPlatformTime::Seconds() - Start);
		});
	}
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FConfigCacheIni::ForPlatform);
	...
	if (PlatformConfig == nullptr)
	{
		...
		UE_LOGF(LogConfig, Display, "Read in platform %ls ini files took %.2f seconds", *PlatformName.ToString(), FPlatformTime::Seconds() - Start);
	}
...
#else
	UE_LOGF(LogConfig, Error, "FConfigCacheIni::ForPlatform cannot be called when not in a developer tool");
	return nullptr;
#endif
}
				...
				UE::DynamicConfig::PerformDynamicConfig("TrackedDyn", [Branch, Filename](FConfigModificationTracker* ChangeTracker)
					{
						...
						ChangeTracker->CVars.Add(TEXT("PluginCVars_LowPriority")).CVarPriority = (int)ECVF_SetByPluginLowPriority;
						ChangeTracker->CVars.Add(TEXT("PluginCVars_HighPriority")).CVarPriority = (int)ECVF_SetByPluginHighPriority;
