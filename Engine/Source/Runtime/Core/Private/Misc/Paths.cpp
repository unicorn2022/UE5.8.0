...
{
	auto IsSlashOrBackslash    = [](auto C) { return C == '/' || C == '\\'; };
	auto IsNotSlashOrBackslash = [](auto C) { return C != '/' && C != '\\'; };
...
FString FPaths::ProjectUserDir()
{
	...
	if (ShouldSaveToUserDir())
	{
		...
		return FPaths::Combine(FPlatformProcess::UserSettingsDir(), TEXT(UE_STRINGIZE(UE_SAVED_DIR_OVERRIDE))) + TEXT("/");
#else
...
#if UE_ENABLE_ICU
			UE_CLOGF(!StaticData.EngineLocalizationPaths.Num(), LogInit, Warning, "No paths for engine localization data were specifed in the engine configuration.");
#endif
...
const TArray<FString>& FPaths::GetEditorLocalizationPaths()
{
	...
	if(!StaticData.bEditorLocalizationPathsInitialized)
	{
		if(GConfig && GConfig->IsReadyForUse())
		{
			...
			{
				UE_LOGF(LogInit, Warning, "No paths for editor localization data were specifed in the editor configuration.");
			}
...
const TArray<FString>& FPaths::GetPropertyNameLocalizationPaths()
{
	...
	if(!StaticData.bPropertyNameLocalizationPathsInitialized)
	{
		if(GConfig && GConfig->IsReadyForUse())
		{
			...
			{
				UE_LOGF(LogInit, Warning, "No paths for property name localization data were specifed in the editor configuration.");
			}
...
const TArray<FString>& FPaths::GetToolTipLocalizationPaths()
{
	...
	if(!StaticData.bToolTipLocalizationPathsInitialized)
	{
		if(GConfig && GConfig->IsReadyForUse())
		{
			...
			{
				UE_LOGF(LogInit, Warning, "No paths for tooltips localization data were specifed in the editor configuration.");
			}
...
#if UE_ENABLE_ICU
			UE_CLOGF(!StaticData.GameLocalizationPaths.Num(), LogPaths, Warning, "No paths for game localization data were specifed in the game configuration.");
#endif
...
FString FPaths::GetExtension(const FString& InPath, UE::Paths::EFlags Flags)
{
	using namespace UE::Paths;

	const FString CleanFilename = GetCleanFilename(InPath);
	int32 DotPos = !EnumHasAnyFlags(Flags, EFlags::AllowCompoundExtension)
		? CleanFilename.Find(TEXT("."), ESearchCase::CaseSensitive, ESearchDir::FromEnd)
		: CleanFilename.Find(TEXT("."), ESearchCase::CaseSensitive, ESearchDir::FromStart);
	if (DotPos != INDEX_NONE)
	{
		return CleanFilename.Mid(DotPos + (EnumHasAnyFlags(Flags, EFlags::IncludeDot) ? 0 : 1));
	}
...
FString FPaths::GetExtension(const FString& InPath, bool bIncludeDot)
{
	...
	return GetExtension(InPath, bIncludeDot ? EFlags::IncludeDot : EFlags::None);
}
...
FString FPaths::CreateStandardFilename(const FString& InPath)
{
	...
	if (!FullPath.StartsWith(RootDirectoryView))
	{
		...
		if (FullPath.Contains(RootDirectoryView))
		{
			...
			UE_LOGF(LogPaths, Warning,
				"CreateStandardFilename: Unexpected: InPath '%ls' transformed to '%ls' which contains but does not start with '%.*ls'.",
...
FString FPaths::ConvertToSandboxPath( const FString& InPath, const TCHAR* InSandboxName )
{
	...
	{
		UE_LOGF(LogInit, Fatal, "%ls does not start with %ls so this is not a valid sandbox path.", *SandboxPath, *RootDirectory);
	}
