...
static FString GetParentIniFromDDPI(const FString& DDPIPath)
{
	FString IniParent;
	FString IniContents;
	if (FFileHelper::LoadFileToString(IniContents, *DDPIPath))
	{
		FConfigFile IniFile;
		IniFile.ProcessInputFileContents(IniContents, DDPIPath);
...
static const TArray<FString> GetAllDataDrivenIniFilenames()
{
	TArray<FString> DataDrivenIniFilenames;
...
static const TArray<FString> GetHostAndTargetDataDrivenIniFilenames()
{
	// grab all we find on disk, and filter only the Host/Target, likely a better way to only search on disk for our host/target but start with easier for now
	TArray<FString> DataDrivenIniFilenames = GetAllDataDrivenIniFilenames();

	FString TargetPlatformCmdLine;
	if (FParse::Value(FCommandLine::Get(), TEXT("TargetPlatform="), TargetPlatformCmdLine) && TargetPlatformCmdLine != TEXT("All"))
	{
		TSet<FString> HostAndTargetPlatforms;
		HostAndTargetPlatforms.Add(FString::Printf(TEXT("/%s/"), ANSI_TO_TCHAR(FPlatformProperties::IniPlatformName()))); // always add Host

		TArray<FString> TargetPlatformNames;
		TargetPlatformCmdLine.ParseIntoArray(TargetPlatformNames, TEXT("+"), true);

		for (const FString& TargetPlatformName : TargetPlatformNames)
		{
			FString Stripped = TargetPlatformName;
			Stripped.RemoveFromEnd("Client");
			Stripped.RemoveFromEnd("Server");
			Stripped.RemoveFromEnd("Editor");
			// Cook flavours, for e.g. Android, are defined as Android_ASTC etc.
			int32 UnderscoreIdx = INDEX_NONE;
			if (Stripped.FindChar(TEXT('_'), UnderscoreIdx))
			{
				Stripped.LeftInline(UnderscoreIdx);
			}

			// add any TargetPlatforms
			HostAndTargetPlatforms.Add(FString::Printf(TEXT("/%s/"), *Stripped));
		}

		TSet<FString> AllPlatforms(HostAndTargetPlatforms);
		TSet<FString> PlatformsToCheck = HostAndTargetPlatforms;

		while (PlatformsToCheck.Num() > 0)
		{
			TSet<FString> NewParentPlatforms;

			for (const FString& DataDrivenIniFilename : DataDrivenIniFilenames)
			{
				for (const FString& Platform : PlatformsToCheck)
				{
					if (DataDrivenIniFilename.Contains(Platform))
					{
						FString ParentIniPlatform = GetParentIniFromDDPI(DataDrivenIniFilename);
						if (!ParentIniPlatform.IsEmpty() && !AllPlatforms.Contains(ParentIniPlatform))
						{
							FString ParentIniWithSlash = FString(TEXT("/") + ParentIniPlatform + TEXT("/"));
							AllPlatforms.Add(ParentIniWithSlash);
							NewParentPlatforms.Add(ParentIniWithSlash);
						}
						break;
					}
				}
			...
			PlatformsToCheck = MoveTemp(NewParentPlatforms);
		}

		// remove any paths that are not part our Host or TargetPlatform list
		DataDrivenIniFilenames.RemoveAll([&AllPlatforms](const FString& Filename)
		{
			for (const FString& Component : AllPlatforms)
			{
...
static const TArray<FString>& GetDataDrivenIniFilenames()
{
	...
	if (bHasSearchedForFiles == false)
	{
		...
		if (FParse::Param(FCommandLine::Get(), TEXT("-LoadOnlyHostAndTargetPlatformModules")))
		{
			DataDrivenIniFilenames = GetHostAndTargetDataDrivenIniFilenames();
		}
		else
		{
			DataDrivenIniFilenames = GetAllDataDrivenIniFilenames();
		}
	}
...
static FString DDPITryRedirect(const FConfigFile& IniFile, const TCHAR* Key, bool* OutHadBang=nullptr)
{
	...
	if (GCommandLinePrefix.Len() > 0)
	{
		...
		{
			UE_LOGF(LogTemp, Display, "--> Overriding DDPI setting %ls for to %ls (via prefix %ls)", Key, *CmdLineValue, *GCommandLinePrefix);
			return CmdLineValue;
		}
	}
...
#endif // DDPI_HAS_EXTENDED_PLATFORMINFO_DATA
...
static void LoadDDPIIniSettings(const FConfigFile& IniFile, FDataDrivenPlatformInfo& Info, FName PlatformName)
{
	...
	DDPIGetString(IniFile, TEXT("ICUDataVersion"), Info.ICUDataVersion);
	...
	// Skip Preview Platform parsing if the Platforms are disabled
	// Exception: Fake Platforms that have no real ShaderPlatform and only PreviewShaderPlatform for previewing or fake Platforms (e.g. VulkanPC).
	auto IniHasRealShaderPlatforms = [&IniFile]()
	{
		for (const auto& Section : IniFile)
		{
	...
	if (Info.bEnabledForUse || Info.bIsFakePlatform || !IniHasRealShaderPlatforms())
	{
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FDataDrivenPlatformInfoRegistry::GetAllPlatformInfos);
