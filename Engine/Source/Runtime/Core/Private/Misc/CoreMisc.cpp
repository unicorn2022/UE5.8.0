...
FDerivedDataCacheInterface& GetDerivedDataCacheRef()
{
	...
	if (!Cache)
	{
		UE_LOGF(LogInit, Fatal, "Derived Data Cache was requested, but not available.");
...
class ITargetPlatformManagerModule* GetTargetPlatformManager(bool bFailOnInitErrors)
{
	...
	if (!FPlatformProperties::RequiresCookedData())
	{
		...
		if (!bInitialized)
		{
			check(IsInGameThread());
			bInitialized = true;
			SingletonInterface = ITargetPlatformManagerModule::Get();
...
class ITargetPlatformManagerModule& GetTargetPlatformManagerRef()
{
	...
	{
		UE_LOGF(LogInit, Fatal, "Target platform manager was requested, but not available.");
...
{
	UE_CLOGF(!GIsRetrievingVTablePtr, LogCore, Fatal, "The %ls constructor is for internal usage only for hot-reload purposes. Please do NOT use it.", CtorSignature);
}
...
void NotifyLoadingStateChanged(bool bState, const TCHAR *Message)
{
	...
	{
		RecursiveCount++;
		UE_LOGF(LogStreaming, Display, "Loading Interval Starting %ls", Message);
	}
	else
	{
		RecursiveCount--;
		check(RecursiveCount >= 0);
		UE_LOGF(LogStreaming, Display, "Loading Interval Ending   %ls", Message);
	}
	...
	{
		LoadCount++;
		UE_LOGF(LogStreaming, Display, "Loading Interval  %5d loading time intervals   %7.2fs spent loading    %7.2fs spent not loading", LoadCount, TotalActiveTime, TotalInactiveTime);
	}
...
namespace UEAsserts_Private
{
	...
	{		
		...
		UE_LOGF(LogCore, Error, "%ls", TempStr);
	}
}
#endif
