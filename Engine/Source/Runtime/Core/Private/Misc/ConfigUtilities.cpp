...
TMulticastDelegate<void(const FName&)> RemovePluginHotfixesFromDynamicBranch;
	...
	TFunction<void(class FConfigModificationTracker*)> PostModification)
{
	...
	{
		UE_LOGF(LogConfig, Verbose, "Modified Files:");
		for (FString& File : ChangeTracker.LoadedFiles)
		{
			UE_LOGF(LogConfig, Verbose, "  %ls", *File);
		}
	}
	
	if (ChangeTracker.bTrackModifiedSections)
	{
		UE_LOGF(LogConfig, Verbose, "Modified sections:");
		for (auto Pair : ChangeTracker.ModifiedSectionsPerBranch)
		{
			UE_LOGF(LogConfig, Verbose, "  Branch: %ls", *Pair.Key.ToString());
			for (FString& Section : Pair.Value)
			{
				UE_LOGF(LogConfig, Verbose, "    %ls", *Section);
			}
		}
		
		if (ChangeTracker.CVars.Num() > 0)
		{
			UE_LOGF(LogConfig, Verbose, "Modified Cvars:");
			for (const auto& SectionPair : ChangeTracker.CVars)
			{
				const FConfigModificationTracker::FCVarTracker& CVars = SectionPair.Value;

				UE_LOGF(LogConfig, Verbose, "  Branch: %ls, SetBy Priority: %d", *SectionPair.Key, CVars.CVarPriority);
				...
				{
					UE_LOGF(LogConfig, Verbose, "    Tag %ls:", *TagPair.Key.ToString());
					for (const auto& BranchPair : TagPair.Value)
					{
						UE_LOGF(LogConfig, Verbose, "      Section %ls:", *BranchPair.Key.ToString());
						const FConfigSection& Section = BranchPair.Value;
						
						for (const auto& CVarPair : Section)
						{
							UE_LOGF(LogConfig, Verbose, "        %ls = %ls", *CVarPair.Key.ToString(), *CVarPair.Value.GetValue());
						}
					}
				}
			}
...
void LoadCVarsFromFileForNextBoot(TMap<FString, FString>& OutCVars)
{
	...
	{
		UE_LOGF(LogConfig, Log, "No project persistent download dir available for boot hotfix");
		return;
	}
	...
	{
		UE_LOGF(LogConfig, Log, "No local boot hotfix file found at: [%ls]", *FullPath);
		return;
	}
	...
	{
		UE_LOGF(LogConfig, Error, "Failed to load local boot hotfix file: [%ls]", *FullPath);
		return;
	}
	...
	{
		UE_LOGF(LogConfig, Error, "Failed to delete local boot hotfix file [%ls]", *FullPath);
	}

	UE_LOGF(LogConfig, Log, "Local boot hotfix file [%ls] loaded and deleted", *FullPath);
...
void SaveCVarForNextBoot(const TCHAR* Key, const TCHAR* Value)
{
	...
	if (!FPaths::HasProjectPersistentDownloadDir())
	{
		UE_LOGF(LogConfig, Log, "No persistent download dir, ignoring CVar %ls hotfix for next boot", Key);
		return;
	}
	...
	AsyncPool(*GIOThreadPool, [StrKey, StrValue] {
		...
		UE_LOGF(LogConfig, Log, "Local boot hotfix file [%ls] saved with hotfixed CVar: %ls=%ls", *FullPath, *StrKey, *StrValue);
		});
#else
...
#if !UE_BUILD_SHIPPING
	// Support passing boot hotfix cvars via the commandline for testing purposes
	FString BootCvars;
	if (FParse::Value(FCommandLine::Get(), TEXT("BootHotfixCvars="), BootCvars))
	{
		TArray<FString> Pairs;
		BootCvars.ParseIntoArray(Pairs, TEXT(","), true);
		for (const FString& Pair : Pairs)
		{
			FString Key, Value;
			if (Pair.Split(TEXT("="), &Key, &Value))
			{
				CVarsToApply.Add(Key, Value);
				UE_LOGF(LogConfig, Log, "Applied commandline boot hotfix cvar %ls=%ls", *Key, *Value);
			}
		}
	}
...
void OnSetCVarFromIniEntry(const TCHAR *IniFile, const TCHAR *Key, const TCHAR* Value, uint32 SetBy, bool bAllowCheating, bool bNoLogging, FName Tag)
{
	...
	if(CVar)
	{
		...
		if(bAllowChange)
		{
			...
			UE_CLOGF(!bNoLogging && bChanged, LogConfig, Log, "Set CVar [[%ls:%ls]]", Key, Value);
#endif
		}
		else
		{
		...
		{
			UE_LOGF(LogConfig, Log, "Saving %ls for boot hotfix", Key);
	...
	else
	{
		if (Tag != NAME_None)
		{
			UE_LOGF(LogConfig, Warning, "Setting unregistered CVar %ls with Tag %ls. This will not be unloaded when the Taqg is unloaded, and it won't be usable until registered in code.", Key, *Tag.ToString());
		}
...
#if !UE_BUILD_SHIPPING
		UE_LOGF(LogConfig, Log, "CVar [[%ls:%ls]] deferred - dummy variable created", Key, Value);
#endif //!UE_BUILD_SHIPPING
	}
}
...
void ApplyCVarSettingsFromIni(const TCHAR* InSectionName, const TCHAR* InIniFilename, uint32 SetBy, bool bAllowCheating, FName Tag)
{
	...
	UE_LOGF(LogConfig,Log,"Applying CVar settings from Section [%ls] File [%ls]",InSectionName,InIniFilename);
		...
		for (const FCVarIniHistory& IniHistory : CVarIniHistory)
		{
			...
			if (const FConfigSection* Section = GConfig->GetSection(*SectionName, false, *IniFilename))
			{
				for (FConfigSectionMap::TConstIterator It(*Section); It; ++It)
				{
					...
					{
						UE_LOGF(LogConfig, Warning, "Failed to change Readonly CVAR value %ls %ls -> %ls Config %ls %ls",
							*KeyString, *CurrentValue, HumanFriendlyValue, *IniFilename, *SectionName);

						continue;
					}

					UE_LOGF(LogConfig, Display, "Applied changed CVAR value %ls %ls -> %ls Config %ls %ls", 
						*KeyString, *CurrentValue, HumanFriendlyValue, *IniFilename, *SectionName);
...
public:
	...
	void DumpHistory()
	{
		...
		{
			UE_LOGF(LogConfig, Display, "%ls", *Line);
			FTCHARToUTF8 UTF8String(*(MoveTemp(Line) + LINE_TERMINATOR));
			Writer->Serialize((UTF8CHAR*)UTF8String.Get(), UTF8String.Length());
		};

		UE_LOGF(LogConfig, Display, "Dumping History of Config Reads to %ls", *SavePath);
		UE_LOGF(LogConfig, Display, "Begin History of Config Reads");
		UE_LOGF(LogConfig, Display, "------------------------------------------------------");
		...
		UE_LOGF(LogConfig, Display, "------------------------------------------------------");
		UE_LOGF(LogConfig, Display, "End History of Config Reads");
