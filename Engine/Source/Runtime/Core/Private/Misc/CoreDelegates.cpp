...
TMulticastDelegate<void(const ANSICHAR*, const ANSICHAR*, int32, const TCHAR*, const TCHAR*)> FCoreDelegates::OnEnsureFailed;
TMulticastDelegate<void()> FCoreDelegates::OnHandleSystemEnsure;
TMulticastDelegate<void()> FCoreDelegates::OnHandleSystemError;
TMulticastDelegate<void()> FCoreDelegates::OnHandleSystemHang;
...
TDelegate<void(TArray<uint8>&, TArray<uint8>&)>& FCoreDelegates::GetPakSigningKeysDelegate()
{
	...
	FCoreDelegates::FPrePostSlateModalWithContextDelegate FCoreDelegates::PreSlateModalWithContext;
	FCoreDelegates::FPrePostSlateModalWithContextDelegate FCoreDelegates::PostSlateModalWithContext;
...
TMulticastDelegate<void(bool)> FCoreDelegates::AudioFocusChangedDelegate;

FSimpleMulticastDelegate& FCoreDelegates::GetOnPostEngineInit()
{
...
TTSMulticastDelegate<void(const TCHAR*)>& FCoreDelegates::TSOnConfigBranchUnloaded()
{
	static TTSMulticastDelegate<void(const TCHAR*)> Singleton;
	return Singleton;
}

TTSMulticastDelegate<void(const TCHAR*, float)>& FCoreDelegates::TSOnConfigBranchReloaded()
{
	static TTSMulticastDelegate<void(const TCHAR*, float)> Singleton;
	return Singleton;
}
...
TTSMulticastDelegate<void()> FCoreDelegates::ApplicationHeartbeat;
...
TDelegate<void(TArrayView<const FShaderHash>, FCoreDelegates::FIoChunkIdResolvedFunc)> FCoreDelegates::ResolvePackageShaderMaps;
TDelegate<void(TArrayView<const FShaderHash>, FCoreDelegates::FAttachShaderReadRequestFunc)> FCoreDelegates::PreloadPackageShaderMaps;
TDelegate<void(TArrayView<const FShaderHash>)> FCoreDelegates::ReleasePreloadedPackageShaderMaps;
...
FSimpleMulticastDelegate FCoreDelegates::OnLowLevelMemTrackerTick;
