...
namespace AutomationTest
{
	...
	*/
	...
	static inline bool GetTestClassAndParameters(const FString& InTest, FString* OutTestClass, FString* OutTestParameters)
	{
		return InTest.Split(TEXT(" "), OutTestClass, OutTestParameters, ESearchCase::CaseSensitive);
	}
};
	...
	: bComplexTask( bInComplexTask )
{
	...
	TestName = InName.Replace(TEXT(" "), TEXT("_"));
	// Register the newly created automation test into the automation testing framework
	const bool bRegistered = FAutomationTestFramework::Get().RegisterAutomationTest( TestName, this );
	if (!bRegistered)
	{
		UE_LOGF(LogAutomationTest, Warning, "Failed to register test with the name '%ls'. Test with the same name is already registered and will not be overridden.", *TestName);
	}
}
...
void FAutomationTestFramework::FAutomationTestOutputDevice::Serialize( const TCHAR* V, ELogVerbosity::Type Verbosity, const class FName& Category )
{
	...
	if (LocalCurTest)
	{
		...
		else
		{
			...
			FString AnalyticsString = "AUTOMATIONANALYTICS";
...
{
	const FString DefaultAutomationSubFolder = "Unreal Automation";
...
bool FAutomationTestFramework::RunSmokeTests()
{
		...
		if ( TestInfo.Num() > 0 )
		{
			...
			for ( int TestIndex = 0; TestIndex < TestInfo.Num(); ++TestIndex )
			{
				...
				if (!!(TestInfo[TestIndex].GetTestFlags() & EAutomationTestFlags::SmokeFilter))
				{
					...
					const bool CurTestSuccessful = GIsAutomationTesting && StopTest(CurExecutionInfo);
			...
			if (bAllSuccessful && !FPlatformMisc::IsDebuggerPresent() && TotalDuration > 2.0)
			{
				...
				UE_LOGF(LogAutomationTest, Warning, "Smoke tests took >2s to run (%.2fs). '%ls' took %dms. "
					"SmokeFilter tier tests should take less than 1ms. Please optimize or move '%ls' to a slower tier than SmokeFilter.", 
	...
	{
		UE_LOGF( LogAutomationTest, Log, "Skipping unit tests for the cooked build and commandlet." );
	}
	else if (!FPlatformProperties::IsProgram())
	{
		UE_LOGF(LogAutomationTest, Error, "Skipping unit tests." );
...
void FAutomationTestFramework::StartTestByName( const FString& InTestToRun, const int32 InRoleIndex, const FString& InFullTestPath )
{
	...
	FString TestName;
	FString Params;
	if (!AutomationTest::GetTestClassAndParameters(InTestToRun, &TestName, &Params))
	{
	...
	if ( !GIsSlowTask && !GIsPlayInEditorWorld )
	{
		...
		{
			UE_LOGF(LogAutomationTest, Error, "Test %ls does not exist and could not be run.", *TestPath);
		}
	}
	else
	{
		UE_LOGF(LogAutomationTest, Error, "Test %ls is too slow and could not be run.", *TestPath);
	}
}
...
bool FAutomationTestFramework::ExecuteLatentCommands()
{
	...
	while (!LatentCommands.IsEmpty())
	{
		...
		if (bComplete)
		{
			...
			{
				UE_LOGF(LogAutomationTest, Verbose, "Tail of latent command queue is not removed, because last completed automation latent command is not corresponding.");
			}
		}
		else
		{
...
void FAutomationTestFramework::LoadTestModules( )
{
	...
	if( !bRunningSmokeTests )
	{
		...
		for( int32 EngineModuleId = 0; EngineModuleId < EngineTestModules.Num(); ++EngineModuleId)
		{
			...
			{
				UE_LOGF(LogAutomationTest, Warning, "The automation test module ('%ls') doesn't have a valid name.", *ModuleName.ToString());
				continue;
			}
			if (!FModuleManager::Get().IsModuleLoaded(ModuleName))
			{
				UE_LOGF(LogAutomationTest, Log, "Loading automation test module: '%ls'.", *ModuleName.ToString());
	...
	for ( TMap<FString, FAutomationTestBase*>::TConstIterator TestIter( AutomationTestClassNameToInstanceMap ); TestIter; ++TestIter )
	{
		...
		if (bEnabled && bPassesApplicationRequirements && bPassesFeatureRequirements && bPassesFilterRequirement)
		{
			...
			if (TimeForGetTests > 10.0)
			{
				...
				UE_LOGF(LogAutomationTest, Warning, "Automation Test '%ls' took > 10 seconds to return from GetTests(...): %.2fs", *CurTest->GetTestName(), (float)TimeForGetTests);
			}
...
void FAutomationTestFramework::LoadTestTagMappings(const FString& BasePath, bool RestrictedPathsOnly)
{
	...
	for (const FString& Path : AllPaths)
	{
		...
		{
			UE_LOGF(LogAutomationTest, Log, "Loading tags from %ls.", *TagsSettingsFilePath);
			...
			{
				UE_LOGF(LogAutomationTest, Error, "Could not load mapping file %ls", *TagsSettingsFilePath);
			}
		}
	}
}
...
void FAutomationTestFramework::SaveTestTagMappings(const TArray<FString>& TestFullNames, const TArray<FString>& TestFilePaths, const FBeforeTagMappingConfigSaved& BeforeConfigSaved, const FAfterTagMappingConfigSaved& AfterConfigSaved) const
{
	...
	for (int i = 0; i < TestFilePaths.Num(); i++)
	{
		...
		if (TagsSettingsFilePath.IsEmpty())
		{
			UE_LOGF(LogAutomationTest, Error, "Test neither in project path or a restricted engine path, cannot be mapped. Path: %ls", *TestSourcePath);
			continue;
		}
	...
	{
		UE_LOGF(LogAutomationTestFramework, Log, "Saving tags to %ls.", *ConfigPath);
...
void FAutomationTestFramework::DumpAutomationTestExecutionInfo( const TMap<FString, FAutomationTestExecutionInfo>& InInfoToDump )
{
	...
	for ( TMap<FString, FAutomationTestExecutionInfo>::TConstIterator MapIter(InInfoToDump); MapIter; ++MapIter )
	{
		...
		UE_LOGF(LogAutomationTest, Log, "%ls: %ls (%.2fms)", *CurTestName, CurExecutionInfo.bSuccessful ? *SuccessMessage : *FailMessage, 1000.0 * CurExecutionInfo.Duration);
		...
		for ( const FAutomationExecutionEntry& Entry : CurExecutionInfo.GetEntries() )
		{
			switch (Entry.Event.Type )
			{
				case EAutomationEventType::Info:
					UE_LOGF(LogAutomationTest, Display, "%ls", *Entry.Event.Message);
					break;
				case EAutomationEventType::Warning:
					UE_LOGF(LogAutomationTest, Warning, "%ls", *Entry.Event.Message);
					break;
				case EAutomationEventType::Error:
					UE_LOGF(LogAutomationTest, Error, "%ls", *Entry.Event.Message);
					break;
			}
		}
	}
...
void FAutomationTestFramework::InternalStartTest( const FString& InTestToRun, const FString& InFullTestPath)
{
	...
	FString TestName;
	if (!AutomationTest::GetTestClassAndParameters(InTestToRun, &TestName, &Parameters))
	{
		TestName = InTestToRun;
	}
	...
	if ( ContainsTest( TestName ) )
	{
		...
		if (!!(RequestedTestFilter & NonSmokeTestFlags))
		{
			...
			{
				UE_LOGF(LogAutomationTestStateTrace, Log, "Test is about to start. Name={%ls}", *CurrentTestFullPath);
			}
			UE_LOGF(LogAutomationTest, Log, "%ls %ls is starting at %f", *CurrentTest->GetBeautifiedTestName(), *Parameters, StartTime);
		}
		...
		{
			{
				...
				{
					UE_LOGF(LogAutomationTest, Warning, "%ls %ls: Test failed while committing an AutoRTFM transaction!", *CurrentTest->GetBeautifiedTestName(), *Parameters);
					bTestSuccessful = false;
				}
			}
			{
				...
				{
					UE_LOGF(LogAutomationTest, Warning, "%ls %ls: Test failed while aborting an AutoRTFM transaction!", *CurrentTest->GetBeautifiedTestName(), *Parameters);
					bTestSuccessful = false;
				}
			}
			if (!bTestSuccessful)
			{
				UE_LOGF(LogAutomationTest, Warning, "%ls %ls: Test passes normally, but fails under AutoRTFM!", *CurrentTest->GetBeautifiedTestName(), *Parameters);
			}
		}
#endif
...
bool FAutomationTestFramework::InternalStopTest(FAutomationTestExecutionInfo& OutExecutionInfo)
{
	...
	if (!!(RequestedTestFilter & NonSmokeTestFlags))
	{
		UE_LOGF(LogAutomationTest, Log, "%ls %ls ran in %f", *CurrentTest->GetBeautifiedTestName(), *Parameters, TimeForTest);
		if (AutomationTest::bLogTestStateTrace)
		{
			UE_LOGF(LogAutomationTestStateTrace, Log, "Test has stopped execution. Name={%ls}", *CurrentTestFullPath);
		}
	}
...
bool FAutomationTestFramework::CanRunTestInEnvironment(const FString& InTestToRun, FString* OutReason, bool* OutWarn) const
{
	...
	if (!AutomationTest::GetTestClassAndParameters(InTestToRun, &TestClassName, &TestParameters))
	{
	...
	if (!ContainsTest(TestClassName))
	{
		...
		{
			*OutReason = FString::Printf(TEXT("Could not find test %s in test registry."), *TestClassName);
		}
...
void FAutomationTestFramework::AddAnalyticsItemToCurrentTest( const FString& AnalyticsItem )
{
	...
	{
		UE_LOGF( LogAutomationTest, Warning, "AddAnalyticsItemToCurrentTest() called when no automation test was actively running!" );
	}
}
...
{
	if (!Actual.EqualToCaseIgnored(Expected))
	{
		AddError(FString::Printf(TEXT("Expected '%s' to be %s, but it was %s."), What, *Expected.ToString(), *Actual.ToString()), 1);
		return false;
	}
...
bool FAutomationTestBase::TestGreaterThan(const TCHAR* What, const float Actual, const float Expected, float Tolerance)
{
	if (FMath::IsNearlyEqual(Actual, Expected, Tolerance))
	{
		AddError(FString::Printf(TEXT("Expected '%s' to be greater than %f, but it was %f and within equality tolerance %f."), What, Expected, Actual, Tolerance), 1);
		return false;
	}
	if (Actual > Expected)
	{
	...
	AddError(FString::Printf(TEXT("Expected '%s' to be greater than %f, but it was %f and outside equality tolerance %f."), What, Expected, Actual, Tolerance), 1);
	return false;
}
...
bool FAutomationTestBase::TestBetweenExclusive(const TCHAR* What, const int32 Actual, const int32 ExpectedLow, const int32 ExpectedHigh)
{
	if (Actual > ExpectedLow && Actual < ExpectedHigh)
	{
		return true;
	}
	AddError(FString::Printf(TEXT("Expected '%s' to be between %d and %d exclusive, but it was %d."), What, ExpectedLow, ExpectedHigh, Actual), 1);
	return false;
}

bool FAutomationTestBase::TestBetweenExclusive(const TCHAR* What, const int64 Actual, const int64 ExpectedLow, const int64 ExpectedHigh)
{
	if (Actual > ExpectedLow && Actual < ExpectedHigh)
	{
		return true;
	}
	AddError(FString::Printf(TEXT("Expected '%s' to be between %" PRId64 " and %" PRId64 " exclusive, but it was %" PRId64 "."), What, ExpectedLow, ExpectedHigh, Actual), 1);
	return false;
}

bool FAutomationTestBase::TestBetweenInclusive(const TCHAR* What, const int32 Actual, const int32 ExpectedLow, const int32 ExpectedHigh)
{
	if (Actual >= ExpectedLow && Actual <= ExpectedHigh)
	{
		return true;
	}
	AddError(FString::Printf(TEXT("Expected '%s' to be between %d and %d inclusive, but it was %d."), What, ExpectedLow, ExpectedHigh, Actual), 1);
	return false;
}

bool FAutomationTestBase::TestBetweenInclusive(const TCHAR* What, const int64 Actual, const int64 ExpectedLow, const int64 ExpectedHigh)
{
	if (Actual >= ExpectedLow && Actual <= ExpectedHigh)
	{
		return true;
	}
	AddError(FString::Printf(TEXT("Expected '%s' to be between %" PRId64 " and %" PRId64 " inclusive, but it was %" PRId64 "."), What, ExpectedLow, ExpectedHigh, Actual), 1);
	return false;
}
