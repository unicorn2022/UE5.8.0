...
const TCHAR* FLinuxPlatformProcess::BaseDir()
{
	...
	if (!bHaveResult)
	{
		...
		{
			int ErrNo = errno;
			UE_LOGF(LogHAL, Fatal, "fopen() for cmdline failed with errno = %d (%ls)", ErrNo,
...
{
	EWaitAndForkResult RetVal = FUnixPlatformProcess::WaitAndFork();
	if (RetVal == EWaitAndForkResult::Child)
	{
		extern void UELinuxKernelToolsConnection_Init();
		UELinuxKernelToolsConnection_Init();
	}
