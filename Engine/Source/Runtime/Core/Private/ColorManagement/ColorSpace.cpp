...
void PreloadWorkingColorSpace()
{
	...
	if (GConfig != nullptr)
	{
		...
		{
			UE_LOGF(LogCore, Warning, "Working color space in config is missing chromaticities, falling back to sRGB.");
		}
	}
	else
	{
		UE_LOGF(LogCore, Display, "Working color space in config is not available, falling back to sRGB.");
	}
...
const FColorSpace& FColorSpace::GetRec2020()
{
	static const FColorSpace ColorSpaceRec2020 = FColorSpace(EColorSpace::Rec2020);

	return ColorSpaceRec2020;
}

const FColorSpace& FColorSpace::GetP3D65()
{
	static const FColorSpace ColorSpaceP3D65 = FColorSpace(EColorSpace::P3D65);

	return ColorSpaceP3D65;
}
