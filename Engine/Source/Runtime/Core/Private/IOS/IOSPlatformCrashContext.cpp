...
void FIOSCrashContext::AddPlatformSpecificProperties() const
{
	...
	//Which additional system libraries should be loaded is tied to our OS version
	{
		...
		FString AdditionalSystemSymbolsVersion = FString::Printf(TEXT("[%s %s 64b]" ), *MajorVersion, *MinorVersion);
...
void FIOSApplicationInfo::Init()
{
	...
	NSOperatingSystemVersion CurrentOSXVersion = [[NSProcessInfo processInfo] operatingSystemVersion];
	OSVersion = FString::Printf(TEXT("%ld.%ld.%ld"), CurrentOSXVersion.majorVersion, CurrentOSXVersion.minorVersion, CurrentOSXVersion.patchVersion);
