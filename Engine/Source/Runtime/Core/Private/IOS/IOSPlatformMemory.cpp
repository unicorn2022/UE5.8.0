...
void FIOSPlatformMemory::OnOutOfMemory(uint64 Size, uint32 Alignment)
{
	auto HandleOOM = [&]()
	{
		...
		if (BackupOOMMemoryPool)
		{
			...
			UE_LOGF(LogMemory, Warning, "Freeing %d bytes from backup pool to handle out of memory.", FPlatformMemory::GetBackMemoryPoolSize());
		...
		UE_LOGF(LogMemory, Warning, "Ran out of memory allocating %llu bytes with alignment %u. Last error msg: %ls.", Size, Alignment, ErrorMsg);
