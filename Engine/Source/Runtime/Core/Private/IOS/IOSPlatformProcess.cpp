...
{
	UE_LOGF(LogIOS, Log,  "LaunchURL %ls %ls", URL, Parms?Parms:TEXT("") );
	...
	[[UIApplication sharedApplication] openURL: [NSURL URLWithString: CFUrl] options:@{} completionHandler:
		^(BOOL Success)
		{
			if (!Success)
			{
				UE_LOGF(LogIOS, Warning, "LaunchURL of %ls was NOT successful", URL);
			}
		}
	];
...
{
	UE_LOGF(LogIOS, Log, "LauncSynchURL %ls %ls", URL, Parms?Parms:TEXT("") );
