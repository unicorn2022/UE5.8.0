...
FDevSystemGuids::FDevSystemGuids()
	...
	, MaterialTranslationDDCVersion(0x086219B8, 0x6E3F435D, 0xA9D99249, 0xD681D5DD)
{
}
...
{
#if !UE_BUILD_SHIPPING
	UE_LOGF(LogDevObjectVersion, Log, "Number of dev versions registered: %d", GDevVersions.Num());
	for (FGuid& Guid : GDevVersions)
	{
		FCustomVersion Version = FCurrentCustomVersions::Get(Guid).GetValue();
		UE_LOGF(LogDevObjectVersion, Log, "  %ls (%ls): %d", *Version.GetFriendlyName().ToString(), *Version.Key.ToString(EGuidFormats::DigitsWithHyphens), Version.Version);
	}
#endif
}
