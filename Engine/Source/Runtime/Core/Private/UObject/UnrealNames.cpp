...
const TCHAR* LexToString(EName Ename)
{
	switch (Ename)
	{
#define REGISTER_NAME(num,name) case EName::name: return TEXT(UE_STRINGIZE(name));
...
#if defined(UE_FNAME_POOL_SHARD_COUNT_OVERRIDE)
static constexpr uint32 FNamePoolShards = UE_FNAME_POOL_SHARD_COUNT_OVERRIDE;
static constexpr uint32 FNamePoolShardBits = FPlatformMath::ConstExprCeilLogTwo(FNamePoolShards);
#else
#if WITH_CASE_PRESERVING_NAME
...
struct FNameHash
{
	...
	static constexpr uint32 ShardMask = (1U << FNamePoolShardBits) - 1U;
	static_assert((ShardMask & FNameSlot::ProbeHashMask) == 0, "Probe hash mask overlaps with bits used to calculate shard index");
...
{
	CharType LowerStr[NAME_SIZE];
	UE_CLOGF(Len > NAME_SIZE, LogCore, Fatal,
		"FName length is too long; HashLowerCase value is undefined. Length = %d, MaxLength = %d, Name=%.*ls...",
...
UE::FSharedString FNameEntry::GetPlainNameSharedString() const
{
	checkName(Header.Len != 0);
	FNameBuffer Temp;
	return UE::FSharedString(TStringView<TCHAR>(EntryToCString(*this, Temp), Header.Len));
}
...
#if DO_CHECK
void FName::ReportInvalidInternalNumber(FNameEntryId ComparisonId, int32 InternalNumber)
{
	UE_LOGF(LogCore, Fatal, "Trying to construct an invalid FName '%ls' with internal number: %d", *GetNamePool().Resolve(ComparisonId).GetPlainNameString(), InternalNumber);
	for (;;);
}
#endif
...
struct FNameHelper
{
	...
	static FName MakeDetectNumber(ViewType View, EFindName FindType)
	{
		...
		return MakeWithInternalNumber(View, FindType, InternalNumber);
	}

	static FName MakeWithInternalNumber(FNameAnsiStringView	 View, EFindName FindType, int32 InternalNumber)
	{
	...
	static FName MakeWithInternalNumber(FNameUtf8StringViewWithWidth View, EFindName FindType, int32 InternalNumber)
	{
		// Ignore the supplied number if the name string is empty
		// to keep the semantics of the old FName implementation
		if (View.Len == 0)
	...
	static FName MakeWithInternalNumber(const FWideStringViewWithWidth View, EFindName FindType, int32 InternalNumber)
	{
		// Ignore the supplied number if the name string is empty
		// to keep the semantics of the old FName implementation
		if (View.Len == 0)
	...
	static FName MakeWithInternalNumber(FNameEntryIds BaseIds, EFindName FindType, int32 InternalNumber)
	{
#if UE_FNAME_OUTLINE_NUMBER
#if WITH_CASE_PRESERVING_NAME
...
private:
	...
	static FName MakeInternal(FNameStringView View, EFindName FindType, int32 InternalNumber)
	{
		...
		if (FindType == FNAME_Find && !Ids.DisplayId)
		{
			...
			{
				return MakeWithInternalNumber(Ids, FindType, InternalNumber); // 'None' with suffix
			}
			else
			{
...
#if DO_CHECK
		if (!FName::IsValidInternalNumber(InternalNumber))
		{
			FName::ReportInvalidInternalNumber(Ids.ComparisonId, InternalNumber);
		}
#endif

		return MakeWithInternalNumber(Ids, FindType, InternalNumber);
	}
...
FName FName::FindNumberedName(FNameEntryId DisplayId, int32 InInternalNumber)
{
	return FNameHelper::MakeWithInternalNumber(FNameEntryIds{FName::GetComparisonIdFromDisplayId(DisplayId), DisplayId}, FNAME_Find, InInternalNumber);
}
...
void FName::SetNumber(int32 InInternalNumber) 
{
	FName NewName = FNameHelper::MakeWithInternalNumber(FNameEntryIds{GetComparisonIndex(), GetDisplayIndex()}, FNAME_Add, InInternalNumber);
	*this = NewName;
}
#endif // UE_FNAME_OUTLINE_NUMBER
...
FName::FName(const WIDECHAR* Name, int32 InInternalNumber)
	: FName(FNameHelper::MakeWithInternalNumber(MakeUnconvertedView(Name), FNAME_Add, InInternalNumber))
{}

FName::FName(const ANSICHAR* Name, int32 InInternalNumber)
	: FName(FNameHelper::MakeWithInternalNumber(MakeUnconvertedView(Name), FNAME_Add, InInternalNumber))
{}

FName::FName(const UTF8CHAR* Name, int32 InInternalNumber)
	: FName(FNameHelper::MakeWithInternalNumber(MakeUnconvertedView(Name), FNAME_Add, InInternalNumber))
{}

FName::FName(int32 Len, const WIDECHAR* Name, int32 InInternalNumber)
	: FName(InInternalNumber != NAME_NO_NUMBER_INTERNAL ? FNameHelper::MakeWithInternalNumber(MakeUnconvertedView(Name, Len), FNAME_Add, InInternalNumber)
		: FNameHelper::MakeDetectNumber(MakeUnconvertedView(Name, Len), FNAME_Add))
{}

FName::FName(int32 Len, const ANSICHAR* Name, int32 InInternalNumber)
	: FName(InInternalNumber != NAME_NO_NUMBER_INTERNAL ? FNameHelper::MakeWithInternalNumber(MakeUnconvertedView(Name, Len), FNAME_Add, InInternalNumber)
		: FNameHelper::MakeDetectNumber(MakeUnconvertedView(Name, Len), FNAME_Add))
{}

FName::FName(int32 Len, const UTF8CHAR* Name, int32 InInternalNumber)
	: FName(InInternalNumber != NAME_NO_NUMBER_INTERNAL ? FNameHelper::MakeWithInternalNumber(MakeUnconvertedView(Name, Len), FNAME_Add, InInternalNumber)
		: FNameHelper::MakeDetectNumber(MakeUnconvertedView(Name, Len), FNAME_Add))
{}

FName::FName(const TCHAR* Name, int32 InInternalNumber, bool bSplitName)
	: FName(InInternalNumber == NAME_NO_NUMBER_INTERNAL && bSplitName
		? FNameHelper::MakeDetectNumber(MakeUnconvertedView(Name), FNAME_Add)
		: FNameHelper::MakeWithInternalNumber(MakeUnconvertedView(Name), FNAME_Add, InInternalNumber))
{}
...
template <ESearchCase::Type SearchCase>
int32 FName::CompareInternal( const FName& Other ) const
{
#if UE_FNAME_OUTLINE_NUMBER
	if constexpr (SearchCase == ESearchCase::IgnoreCase)
	{
	...
	{
		if (GetDisplayIndexInternal() == Other.GetDisplayIndexInternal())
		{
			return 0; // Identical
		}
	}
	...
	if constexpr (SearchCase == ESearchCase::IgnoreCase)
	{
		A = GetComparisonIndex();
		B = Other.GetComparisonIndex();
	}
	else
	{
		A = GetDisplayIndexFast();
		B = Other.GetDisplayIndexFast();
	}
	...
	return CompareDifferentIdsAlphabetically<SearchCase>(A, B);
}

int32 FName::Compare( const FName& Other ) const
{
	return CompareInternal<ESearchCase::IgnoreCase>(Other);
}

int32 FName::CompareSensitive( const FName& Other ) const
{
	return CompareInternal<ESearchCase::CaseSensitive>(Other);
}
...
UE::FSharedString FName::GetPlainNameSharedString() const
{
	return GetDisplayNameEntry()->GetPlainNameSharedString();
}
...
{
	NAME_CHECK(FName(Literal) == FLazyName(Literal));
	NAME_CHECK(FLazyName(Literal) == FName(Literal));
	NAME_CHECK(FLazyName(Literal) == FLazyName(Literal));
	NAME_CHECK(FName(Literal) == FLazyName(Literal).Resolve());

	CharType Literal2[N];
	FMemory::Memcpy(Literal2, Literal);
	NAME_CHECK(FLazyName(Literal) == FLazyName(Literal2));
	...
	NAME_CHECK(WriteToString<64>(FLazyName(Literal).Resolve()).ToView().Equals(WriteToString<64>(Literal).ToView(), ComparisonMode));
}
...
struct FNameAutoTestTruncateHelper
{
	...
	static void TestInner()
	{
		...
		if constexpr (RequiredLen >= TestStrLen)
		{
			...
			NAME_CHECK(Len == TestStrLen);
		}
		else
		{
			// String should be truncated
			NAME_CHECK(Len == RequiredLen);
		}
		...
		for (uint32 Index = 0; Index <= RequiredLen; ++Index)
		{
			if (Index < Len)
			{
				NAME_CHECK(Buffer.Data[Index] == TestStr[Index]);
			}
			else if (Index == Len)
			{
				NAME_CHECK(Buffer.Data[Index] == 0); // null terminator
			}
			else
			{
				NAME_CHECK(Buffer.Data[Index] == CanaryValue); // should not have been written
			}
		}

		NAME_CHECK(Buffer.Canary == CanaryValue);
	}
...
TEST_CASE_NAMED(FNameTest, "System::Core::Name", "[ApplicationContextMask][SmokeFilter]")
{
	NAME_CHECK(FNameHash::IsAnsiNone("None", 4) == 1);
	NAME_CHECK(FNameHash::IsAnsiNone("none", 4) == 1);
	NAME_CHECK(FNameHash::IsAnsiNone("NONE", 4) == 1);
	NAME_CHECK(FNameHash::IsAnsiNone("nOnE", 4) == 1);
	NAME_CHECK(FNameHash::IsAnsiNone("None", 5) == 0);
	NAME_CHECK(FNameHash::IsAnsiNone(TEXT("None"), 4) == 0);
	NAME_CHECK(FNameHash::IsAnsiNone("nono", 4) == 0);
	NAME_CHECK(FNameHash::IsAnsiNone("enon", 4) == 0);
	...
	NAME_CHECK(AutoTest_1.GetNumber() == NAME_EXTERNAL_TO_INTERNAL(1));
	NAME_CHECK(autoTest_1.GetNumber() == NAME_EXTERNAL_TO_INTERNAL(1));
	NAME_CHECK(autoTeSt_1.GetNumber() == NAME_EXTERNAL_TO_INTERNAL(1));
	NAME_CHECK(AutoTest_2.GetNumber() == NAME_EXTERNAL_TO_INTERNAL(2));
	NAME_CHECK(AutoTestB_2.GetNumber() == NAME_EXTERNAL_TO_INTERNAL(2));

	NAME_CHECK(AutoTest_1 != AutoTest_2);
	NAME_CHECK(AutoTest_1 == autoTest_1);
	NAME_CHECK(AutoTest_1 == autoTeSt_1);

	NAME_CHECK(AutoTest_1.ToUnstableInt() == autoTest_1.ToUnstableInt());
	NAME_CHECK(AutoTest_1.ToUnstableInt() != AutoTest_2.ToUnstableInt());

	TCHAR Buffer[FName::StringBufferSize];

#if WITH_CASE_PRESERVING_NAME
	NAME_CHECK(!FCString::Strcmp(*AutoTest_1.ToString(), TEXT("AutoTest_1")));
	NAME_CHECK(!FCString::Strcmp(*autoTest_1.ToString(), TEXT("autoTest_1")));
	NAME_CHECK(!FCString::Strcmp(*autoTeSt_1.ToString(), TEXT("autoTeSt_1")));
	NAME_CHECK(!FCString::Strcmp(*AutoTestB_2.ToString(), TEXT("AutoTestB_2")));
	
	NAME_CHECK(FName("ABC").ToString(Buffer) == 3);
	NAME_CHECK(!FCString::Strcmp(Buffer, TEXT("ABC")));
	NAME_CHECK(FName("abc").ToString(Buffer) == 3);
	NAME_CHECK(!FCString::Strcmp(Buffer, TEXT("abc")));
	NAME_CHECK(FName(TEXT("abc")).ToString(Buffer) == 3);
	NAME_CHECK(!FCString::Strcmp(Buffer, TEXT("abc")));
	NAME_CHECK(FName("ABC_0").ToString(Buffer) == 5);
	NAME_CHECK(!FCString::Strcmp(Buffer, TEXT("ABC_0")));
	NAME_CHECK(FName("ABC_10").ToString(Buffer) == 6);
	NAME_CHECK(!FCString::Strcmp(Buffer, TEXT("ABC_10")));	
#endif

	NAME_CHECK(autoTest_1.GetComparisonIndex() == AutoTest_2.GetComparisonIndex());
	NAME_CHECK(autoTest_1.GetPlainNameString() == AutoTest_1.GetPlainNameString());
	NAME_CHECK(autoTest_1.GetPlainNameString() == AutoTest_2.GetPlainNameString());
	NAME_CHECK(*AutoTestB_2.GetPlainNameString() != *AutoTest_2.GetPlainNameString());
	NAME_CHECK(AutoTestB_2.GetNumber() == AutoTest_2.GetNumber());
	NAME_CHECK(autoTest_1.GetNumber() != AutoTest_2.GetNumber());
	NAME_CHECK(AutoTest_1.GetNumber() == autoTest_1.GetNumber());

	NAME_CHECK(FCStringAnsi::Strlen("None") == FName().GetStringLength());
	NAME_CHECK(FCStringAnsi::Strlen("ABC") == FName("ABC").GetStringLength());
	NAME_CHECK(FCStringAnsi::Strlen("ABC_0") == FName("ABC_0").GetStringLength());
	NAME_CHECK(FCStringAnsi::Strlen("ABC_9") == FName("ABC_9").GetStringLength());
	NAME_CHECK(FCStringAnsi::Strlen("ABC_10") == FName("ABC_10").GetStringLength());
	NAME_CHECK(FCStringAnsi::Strlen("ABC_2000000000") == FName("ABC_2000000000").GetStringLength());
	NAME_CHECK(FCStringAnsi::Strlen("ABC_4000000000") == FName("ABC_4000000000").GetStringLength());

	const FName NullName(static_cast<ANSICHAR*>(nullptr));
	NAME_CHECK(NullName.IsNone());
	NAME_CHECK(NullName == FName(static_cast<WIDECHAR*>(nullptr)));
	NAME_CHECK(NullName == FName(NAME_None));
	NAME_CHECK(NullName == FName());
	NAME_CHECK(NullName == FName(""));
	NAME_CHECK(NullName == FName(TEXT("")));
	NAME_CHECK(NullName == FName("None"));
	NAME_CHECK(NullName == FName("none"));
	NAME_CHECK(NullName == FName("NONE"));
	NAME_CHECK(NullName == FName(TEXT("None")));
	NAME_CHECK(FName().ToEName());
	NAME_CHECK(*FName().ToEName() == NAME_None);
	NAME_CHECK(NullName.GetComparisonIndex().ToUnstableInt() == 0);

	// Numbered nones
	FName None_7 = FName("None_7");
	NAME_CHECK(None_7.ToString() == "None_7");
	NAME_CHECK(None_7.GetComparisonIndex() == FNameEntryId::FromEName(EName::None));
	NAME_CHECK(None_7.GetNumber() == NAME_EXTERNAL_TO_INTERNAL(7));
	NAME_CHECK(FName(EName::None, NAME_EXTERNAL_TO_INTERNAL(7)) == None_7);
	NAME_CHECK(FName(FName(), NAME_EXTERNAL_TO_INTERNAL(7)) == None_7);
	NAME_CHECK(FName("None", NAME_EXTERNAL_TO_INTERNAL(7)) == None_7);
	NAME_CHECK(FName(FNameEntryId::FromEName(EName::None), FNameEntryId::FromEName(EName::None), NAME_EXTERNAL_TO_INTERNAL(7)) == None_7);
	NAME_CHECK(!None_7.IsNone());

	// "_[num]" is considered None to keep semantics of old implementation, unlike "None_[num]"
	NAME_CHECK(FName("_7").IsNone()); 
	NAME_CHECK(FName("_0").IsNone());
	NAME_CHECK(FName("_2147483646").IsNone());
	NAME_CHECK(!FName("_2147483647").IsNone()); // invalid number

	const FName Cylinder(NAME_Cylinder);
	NAME_CHECK(Cylinder == FName("Cylinder"));
	NAME_CHECK(Cylinder.ToEName());
	NAME_CHECK(*Cylinder.ToEName() == NAME_Cylinder);
	NAME_CHECK(Cylinder.GetPlainNameString() == TEXT("Cylinder"));

	// Test numbers
	NAME_CHECK(FName("Text_0") == FName("Text", NAME_EXTERNAL_TO_INTERNAL(0)));
	NAME_CHECK(FName("Text_1") == FName("Text", NAME_EXTERNAL_TO_INTERNAL(1)));
	NAME_CHECK(FName("Text_1_0") == FName("Text_1", NAME_EXTERNAL_TO_INTERNAL(0)));
	NAME_CHECK(FName("Text_0_1") == FName("Text_0", NAME_EXTERNAL_TO_INTERNAL(1)));
	NAME_CHECK(FName("Text_00") == FName("Text_00", NAME_NO_NUMBER_INTERNAL));
	NAME_CHECK(FName("Text_01") == FName("Text_01", NAME_NO_NUMBER_INTERNAL));

	// Test unterminated strings
	NAME_CHECK(FName("") == FName(0, "Unused"));
	NAME_CHECK(FName("Used") == FName(4, "UsedUnused"));
	NAME_CHECK(FName("Used") == FName(4, "Used"));
	NAME_CHECK(FName("Used_0") == FName(6, "Used_01"));
	NAME_CHECK(FName("Used_01") == FName(7, "Used_012"));
	NAME_CHECK(FName("Used_123") == FName(8, "Used_123456"));
	NAME_CHECK(FName("Used_123") == FName(8, "Used_123_456"));
	NAME_CHECK(FName("Used_123") == FName(8, TEXT("Used_123456")));
	NAME_CHECK(FName("Used_123") == FName(8, TEXT("Used_123_456")));
	NAME_CHECK(FName("Used_2147483646") == FName(15, TEXT("Used_2147483646123")));
	NAME_CHECK(FName("Used_2147483647") == FName(15, TEXT("Used_2147483647123")));
	NAME_CHECK(FName("Used_2147483648") == FName(15, TEXT("Used_2147483648123")));

	// Test wide strings
	FString Wide("Wide ");
	Wide[4] = 60000;
	FName WideName(*Wide);
	NAME_CHECK(WideName.GetPlainNameString() == Wide);
	NAME_CHECK(FName(*Wide).GetPlainNameString() == Wide);
	NAME_CHECK(FName(*Wide).ToString(Buffer) == 5);
	NAME_CHECK(!FCString::Strcmp(Buffer, *Wide));
	NAME_CHECK(Wide.Len() == WideName.GetStringLength());
	FString WideLong = FString::ChrN(1000, 60000);
	NAME_CHECK(FName(*WideLong).GetPlainNameString() == WideLong);
	...
	if (Once)
	{
		NAME_CHECK(FName("UniqueUnicorn!!", FNAME_Find) == FName());

		// Check that FNAME_Find can find entries
		const FName UniqueName("UniqueUnicorn!!", FNAME_Add);
		NAME_CHECK(FName("UniqueUnicorn!!", FNAME_Find) == UniqueName);
		NAME_CHECK(FName(TEXT("UniqueUnicorn!!"), FNAME_Find) == UniqueName);
		NAME_CHECK(FName("UNIQUEUNICORN!!", FNAME_Find) == UniqueName);
		NAME_CHECK(FName(TEXT("UNIQUEUNICORN!!"), FNAME_Find) == UniqueName);
		NAME_CHECK(FName("uniqueunicorn!!", FNAME_Find) == UniqueName);
	...
	NAME_CHECK(Names[0] == "FooA");
	NAME_CHECK(Names[1] == "FooAB");
	NAME_CHECK(Names[2] == "FooABC");
	NAME_CHECK(Names[3] == "FooABCD");
	NAME_CHECK(Names[4] == "FooB");
	NAME_CHECK(Names[5] == "FooC");
	NAME_CHECK(Names[6] == FooWide);
		
	NAME_CHECK(FName() == TEXTVIEW("None"));
	NAME_CHECK(FName() == TEXTVIEW("none"));
	NAME_CHECK(FName() == TEXTVIEW(""));
	NAME_CHECK(FName() == FStringView());
	NAME_CHECK(FName("a") == TEXTVIEW("a"));
	NAME_CHECK(FName("a") == TEXTVIEW("A"));
	NAME_CHECK(FName("A") == TEXTVIEW("a"));
	NAME_CHECK(FName("aa") != TEXTVIEW("a"));
	NAME_CHECK(FName("a") != TEXTVIEW("aa"));
	NAME_CHECK(FName("a") == TEXTVIEW("aa").Left(1)); // Unterminated
	NAME_CHECK(FName("aa") != TEXTVIEW("ab"));
	NAME_CHECK(FName("a_0") == TEXTVIEW("a_0"));
	NAME_CHECK(FName("a_1") != TEXTVIEW("a_0"));
	NAME_CHECK(FName("a_10") == TEXTVIEW("a_10"));
	NAME_CHECK(FName("a")    == TEXTVIEW("a_10").Left(1)); // Unterminated number
	NAME_CHECK(FName("a_")   == TEXTVIEW("a_10").Left(2)); // Unterminated number
	NAME_CHECK(FName("a_1")  == TEXTVIEW("a_10").Left(3)); // Unterminated number
	NAME_CHECK(FName("a_2147483646") == TEXTVIEW("a_2147483646")); // Largest valid number
	NAME_CHECK(FName("a_2147483647") == TEXTVIEW("a_2147483647")); // Smallest invalid number
	NAME_CHECK(FName("a_") == TEXTVIEW("a_"));
	NAME_CHECK(FName("_") == TEXTVIEW("_"));
	NAME_CHECK(FName("_3") == TEXTVIEW("_3")); // Special none
	NAME_CHECK(FName() == TEXTVIEW("_123")); // Special none
	NAME_CHECK(FName("_3") == TEXTVIEW("None")); // Special none
	NAME_CHECK(FName("none_1") == TEXTVIEW("none_1")); // Numbered none
	NAME_CHECK(WideName == FStringView(Wide));
	NAME_CHECK(FName("a") == ANSITEXTVIEW("a"));
	NAME_CHECK(FName("a") == ANSITEXTVIEW("A"));
	NAME_CHECK(FName("A") != ANSITEXTVIEW("b"));
	...
	NAME_CHECK(FLazyName("Hej") == FLazyName(UTF8TEXT("Hej")));
	NAME_CHECK(FLazyName("Hej") == FLazyName(WIDETEXT("Hej")));
	NAME_CHECK(FLazyName(UTF8TEXT("Hej")) == FLazyName("Hej"));
	NAME_CHECK(FLazyName(UTF8TEXT("Hej")) == FLazyName(WIDETEXT("Hej")));
	NAME_CHECK(FLazyName(WIDETEXT("Hej")) == FLazyName("Hej"));
	NAME_CHECK(FLazyName(WIDETEXT("Hej")) == FLazyName(UTF8TEXT("Hej")));
...

	{
		...
		for (uint32 Index : SetBits)
		{
			...
			NAME_CHECK(bWasSet == false);
		}

		const bool b32WasSet = TestSet.SetBitAtomic(34);
		NAME_CHECK(b32WasSet);

		uint32 Expected = 0;
		TestSet.ForEachSetBit([&](uint32 Index)
		{
			NAME_CHECK(Index == SetBits[Expected]);
			++Expected;
		});
		NAME_CHECK(Expected == UE_ARRAY_COUNT(SetBits));

		const bool b1023WasSet = TestSet.ClearBitAtomic(1023);
		NAME_CHECK(b1023WasSet == true);
		TestSet.ForEachSetBit([&](uint32 Index)
		{
			NAME_CHECK(Index != 1023);
		});
	}
}
...
FArchive& operator<<(FArchive& Ar, FNameEntrySerialized& E)
{
	if (Ar.IsLoading())
	{
		...
		{
			Ar.SetCriticalError();
			UE_LOGF(LogUnrealNames, Error, "String is too long");
			return Ar;
		}
...
#if UE_FNAME_OUTLINE_NUMBER
FName FName::CreateNumberedName(FNameEntryId ComparisonId, FNameEntryId DisplayId, int32 InInternalNumber)
{
	...
	return FNameHelper::MakeWithInternalNumber(FNameEntryIds{ ComparisonId, DisplayId }, FNAME_Add, InInternalNumber);
}
#endif // UE_FNAME_OUTLINE_NUMBER
...
#if UE_FNAME_OUTLINE_NUMBER
		return FNameHelper::MakeWithInternalNumber(FNameEntryIds{ ComparisonId, DisplayId }, FNAME_Add, Number);
#else // UE_FNAME_OUTLINE_NUMBER
...
struct FNameBatchLoader
{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("FNameBatchLoader::Read");
		uint32 Num = 0;
		Ar << Num;
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("FNameBatchLoader::Load");
		check(Headers.Num());
...
struct FNameBatchAsyncLoader : FNameBatchLoader
{
	...
	void DoWork()
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("FNameBatchAsyncLoader::DoWork");
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE_STR("FNameBatchAsyncLoader::GetResult");
		check(DoneEvent);
