...
public:
		...
		, TempMemory(nullptr)
	{
		...
		else
		{
			...
			if (bUserSuppliedMemory && (AlignedOffset != Offset || AlignedBytesToRead != BytesToRead))
			{
				...
				{
					NumMessages++;
					UE_LOGF(LogTemp, Log, "FMicrosoftReadRequest request was not aligned. This is expected with loose files, but not a pak file.");
				}
				else if (NumMessages == 10)
				{
					NumMessages++;
					UE_LOGF(LogTemp, Log, "LAST NOTIFICATION THIS RUN: FMicrosoftReadRequest request was not aligned.");
				}
			...
			{
				UE_LOGF(LogTemp, Fatal, "FMicrosoftReadRequest bogus request Offset = %lld BytesToRead = %lld AlignedOffset = %lld AlignedBytesToRead = %lld FileSize = %lld File = %ls", Offset, BytesToRead, AlignedOffset, AlignedBytesToRead, FileSize, GetFileNameForErrorMessagesAndPanicRetry());
			}
			...
			if (!ReadFile(FileHandle, TempMemory ? TempMemory : Memory, AlignedBytesToRead, (LPDWORD)&NumRead, &OverlappedIO))
			{
				...
				{
					UE_LOGF(LogTemp, Fatal, "FMicrosoftReadRequest ReadFile Failed! Error code = %x", ErrorCode);
				}
			}
	...
	void PerformRequest()
	{
		...
		if (bFailed)
		{
			UE_LOGF(LogTemp, Error, "Bad read, retrying %ls", *FailedMessage);
			...
			for (int32 Try = 0; bFailed && Try < 10; Try++)
			{
				...
				if (Handle != INVALID_HANDLE_VALUE )
				{
					...
					{
						uint32 ErrorCode = GetLastError();
						UE_LOGF(LogTemp, Error, "Failed to seek for retry. %x", ErrorCode );
					}
					...
					{
						uint32 ErrorCode = GetLastError();
						UE_LOGF(LogTemp, Error, "Failed to read for retry. %x", ErrorCode );
					}
					CloseHandle(Handle);
				}
				else
				{
					uint32 ErrorCode = GetLastError();
					UE_LOGF(LogTemp, Error, "Failed to open handle for retry. %x", ErrorCode );
				}
				if (bFailed && Try < 9)
				{
		...
		if (bFailed)
		{
			UE_LOGF(LogTemp, Fatal, "Unable to recover from a bad read: %ls", *FailedMessage);
		}
