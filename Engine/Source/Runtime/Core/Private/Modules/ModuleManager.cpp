...
void FModuleManager::WarnIfItWasntSafeToLoadHere(const FName InModuleName)
{
	if ( !IsInGameThread() )
	{
		UE_LOGF(LogModuleManager, Warning, "ModuleManager: Attempting to load '%ls' outside the main thread.  This module was already loaded - so we didn't crash but this isn't safe.  Please call LoadModule on the main/game thread only.  You can use GetModule or GetModuleChecked instead, those are safe to call outside the game thread.", *InModuleName.ToString());
	}
}
	...
	, bIsLoadingDynamicLibrary(false)
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("ModulesBootstrap="), ModulesBootstrapFilename))
	{
		...
		{
			UE_LOGF(LogModuleManager, Display, "Unable to bootstrap from archive %ls, will fallback on normal initialization", *ModulesBootstrapFilename);
		}
	}
#endif
}
...
{
	UE_LOGF(LogModuleManager, Log, "InternalLoadLibrary: '%ls' ('%ls')", *ModuleName.ToString(), *LibraryToLoad);
	...
	{
		UE_LOGF(LogModuleManager, Log, "InternalLoadLibrary: already loaded library '%ls'", *LibraryToLoad);
	...
	for (FName Dependency : Dependencies)
	{
		...
		{
			UE_LOGF(LogModuleManager, Log, "InternalLoadLibrary: loading dependency '%ls'", *Dependency.ToString());
	...
	UE_LOGF(LogModuleManager, Verbose, "InternalLoadLibrary: used about %llu KB", (FinalMemUsed - InitialMemUsed) / 1024);
...
{
	UE_LOGF(LogModuleManager, Verbose, "InternalFreeLibrary: '%ls'", *ModuleName.ToString());
	...
	if (ModuleName != NAME_None)
	{
		...
		UE_LOGF(LogModuleManager, Verbose, "InternalFreeLibrary: library '%ls' has %d users", *LibraryToUnload, ModuleCount);
	}
	...
	else if (Handle)
	{
		for (const TPair<FString, void*>& LibraryNameAndHandle : LibraryHandles)
		{
			if (LibraryNameAndHandle.Value == Handle)
			{
				...
				UE_LOGF(LogModuleManager, Verbose, "InternalFreeLibrary: library '%ls' was explicitly requested", *LibraryToUnload);
				break;
			}
		}
	}
	...
	{
		UE_LOGF(LogModuleManager, Fatal, "InternalFreeLibrary: no module name or valid handle was passed")
	}

	// Safety feature: allow disabling of merged library unloading, either wholly, or for specific libraries
	const auto CanUnloadLibrary = [&LibraryToUnload, this]()
	{
		...
		if (DelayUnloadLibraries.Contains(LibraryToUnload))
		{
			return false;
		}
	...
	{
		UE_LOGF(LogModuleManager, Log, "InternalFreeLibrary: preparing unload for library '%ls'", *LibraryToUnload);
		...
		for (TPair<FName, FInitializeStaticallyLinkedModule> OtherModuleNameAndInfo : StaticallyLinkedModuleInitializers)
		{
			...
			{
				UE_LOGF(LogModuleManager, Verbose, "InternalFreeLibrary: cleaning up for module '%ls'",
		...
		for (FName UnloadedModule : UnloadedModules)
		{
			StaticallyLinkedModuleInitializers.Remove(UnloadedModule);
		}
	...
	if (ModuleName != NAME_None)
	{
		for (const TPair<FString, void*>& LibraryNameAndHandle : LibraryHandles)
		{
			...
			{
				UE_LOGF(LogModuleManager, Log, "InternalFreeLibrary: '%ls' is now unused", *LibraryName);
...
void FModuleManager::LogMergedLibraryUsage()
{
	for (const TPair<FString, void*>& LibraryNameAndHandle : LibraryHandles)
	{
		...
		UE_LOGF(LogModuleManager, Log, "Merged library usage for '%ls'", *LibraryName);

		if (LoadedModules.Num())
		{
			UE_LOGF(LogModuleManager, Log, "  %d modules loaded for this library", LoadedModules.Num());
			for (const FName& Module : LoadedModules)
			{
				UE_LOGF(LogModuleManager, Log, "    '%ls'", *Module.ToString());
			}
		}

		if (SharedLibraries.Num())
		{
			UE_LOGF(LogModuleManager, Log, "  depends on %d other libraries", SharedLibraries.Num());
			for (const FName& Library : SharedLibraries)
			{
				UE_LOGF(LogModuleManager, Log, "    '%ls'", *Library.ToString());
			}
		}

		if (LoadedLibraries.Num())
		{
			UE_LOGF(LogModuleManager, Log, "  %d other libraries depend on this library", LoadedLibraries.Num());
...
void FModuleManager::OnObjectCleanup()
{
	...
	for (auto LibraryAndModulesIterator = DelayUnloadLibraries.CreateIterator(); LibraryAndModulesIterator; ++LibraryAndModulesIterator)
	{
		...
		if (ensure(CheckLiveObjectsInModulesCallback.IsBound()) && !CheckLiveObjectsInModulesCallback.Execute(LibraryAndModulesIterator.Value()))
		{
			...
			if (HandlePtr)
			{
				UE_LOGF(LogModuleManager, Log, "OnObjectCleanup: unloading dynamic library '%ls'", *LibraryAndModulesIterator.Key());

				AutoRTFM::UnreachableIfClosed();

				// Give various systems a last chance to clean up before removal
				OnLibraryUnmountCallback.Broadcast();

#if UE_DELEGATE_CHECK_LIFETIME
				FMulticastDelegateTracker::CheckForStaleDelegatesInModules(LibraryAndModulesIterator.Value());
				...
				UE_LOGF(LogModuleManager, Verbose, "OnObjectCleanup: freed about %llu KB", (InitialMemUsed - FinalMemUsed) / 1024);
...
IModuleInterface* FModuleManager::LoadModuleWithFailureReason(const FName InModuleName, EModuleLoadResult& OutFailureReason, ELoadModuleFlags InLoadModuleFlags)
{
	...
	UE_LOGF(LogModuleManager, Verbose, "LoadModuleWithFailureReason %ls", *InModuleName.ToString());
...
//	ensureMsgf(IsInGameThread(), TEXT("ModuleManager: Attempting to load '%s' outside the main thread.  Please call LoadModule on the main/game thread only.  You can use GetModule or GetModuleChecked instead, those are safe to call outside the game thread."), *InModuleName.ToString());
	...
	const auto InitializeModule = [this, InModuleName, InLoadModuleFlags, &ModuleInfo, &LoadedModule, &OutFailureReason, &EnsureModuleFilename](const FInitializeStaticallyLinkedModule* ModuleInitializerPtr)
	{
		...
		else
		{
			UE_CLOGF((InLoadModuleFlags & ELoadModuleFlags::LogFailures) != ELoadModuleFlags::None,
				LogModuleManager, Warning, "ModuleManager: Unable to load module '%ls' because InitializeModule function failed (returned nullptr.)", *InModuleName.ToString());
	...
	else
	{
		...
		UE_CLOGF((InLoadModuleFlags & ELoadModuleFlags::LogFailures) != ELoadModuleFlags::None,
			LogModuleManager, Warning, "ModuleManager: Module '%ls' not found - its StaticallyLinkedModuleInitializers function is null.", *InModuleName.ToString());
	...
	else
	{
		...
		UE_LOGF(LogModuleManager, Verbose, "ModuleManager: Load Module '%ls' DLL '%ls'", *InModuleName.ToString(), *ModuleInfo->Filename);
		...
		if (ModuleInfo->Filename.IsEmpty() || !FPlatformProcess::ModuleExists(ModuleInfo->Filename))
		{
			...
			{
				UE_CLOGF((InLoadModuleFlags & ELoadModuleFlags::LogFailures) != ELoadModuleFlags::None,
					LogModuleManager, Warning, "ModuleManager: Unable to load module '%ls'  - %d instances of that module name found.", *InModuleName.ToString(), ModulePathMap.Num());
		...
		if (FPlatformProcess::ModuleExists(ModuleFileToLoad))
		{
			...
			if (ModuleInfo->Handle != nullptr)
			{
				...
				{
					UE_CLOGF((InLoadModuleFlags & ELoadModuleFlags::LogFailures) != ELoadModuleFlags::None,
						LogModuleManager, Warning, "ModuleManager: Unable to load module '%ls' because InitializeModule function was not found.", *ModuleFileToLoad);
...
void FModuleManager::UnloadModulesAtShutdown()
{
	...
	for (FModulePair& ModuleToUnload : ModulesToUnload)
	{
		UE_LOGF(LogModuleManager, Verbose, "Shutting down and abandoning module %ls (%d)", *ModuleToUnload.ModuleName.ToString(), ModuleToUnload.LoadOrder);
		const bool bIsShutdown = true;
		UnloadModule(ModuleToUnload.ModuleName, bIsShutdown);
		UE_LOGF(LogModuleManager, Verbose, "Returned from UnloadModule.");
	}
}
...
IModuleInterface* FModuleManager::GetModule( const FName InModuleName )
{
	...
	UE_LOGF(LogModuleManager, Warning, "GetModule racing against IsReady: %ls", *InModuleName.ToString());
#endif
...
void FModuleManager::FindModulePaths(const TCHAR* NamePattern, TMap<FName, FString> &OutModulePaths) const
{
	if (ModulePathsCache.Num() == 0)
	{
		...
		if (!BuildId.IsSet())
		{
			...
			{
				UE_LOGF(LogModuleManager, Fatal, "Unable to read module manifest from '%ls'. Module manifests are generated at build time, and must be present to locate modules at runtime.", *FileName)
			}
...
void FModuleManager::ProcessPendingStaticallyLinkedModuleInitializers() const
{
	...
	{
		UE_LOGF(LogModuleManager, VeryVerbose, "ProcessPendingStaticallyLinkedModuleInitializers: '%ls'",
...
{
	UE_LOGFMT(LogModuleManager, Log, "StartProcessingNewlyLoadedObjects");
...
bool FModuleManager::DoesLoadedModuleHaveUObjects( const FName ModuleName ) const
{
	...
	{
		return IsPackageLoaded.Execute(*FString(FString("/Script/") + ModuleName.ToString()));
	}
