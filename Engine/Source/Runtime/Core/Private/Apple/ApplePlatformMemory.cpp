...
#if PLATFORM_MAC
	// Following Windows' lead on disabling program size. The default implementation
	// of program size (in FLLMGlobals::InitializeProgramSize) right now is 
	// FPlatformMemoryStats::TotalPhysical - FPlatformMemoryStats::AvailablePhysical.
	// Unless we are running in an environment primarily designed around one process,
	// this is incorrect as both of these numbers are system-level stats and could include
	// memory taken up by other processes.
	LLM(FLowLevelMemTracker::Get().SetProgramSize(0));
#endif
	
	const FPlatformMemoryConstants& MemoryConstants = FPlatformMemory::GetConstants();
	UE_LOGF(LogInit, Log, "Memory total: Physical=%.1fGB (%dGB approx) Pagefile=%.1fGB Virtual=%.1fGB",
	   float(static_cast<double>(MemoryConstants.TotalPhysical)/1024.0/1024.0/1024.0),
	   MemoryConstants.TotalPhysicalGB,
	   float(static_cast<double>(MemoryConstants.TotalVirtual-MemoryConstants.TotalPhysical)/1024.0/1024.0/1024.0),
	   float(static_cast<double>(MemoryConstants.TotalVirtual)/1024.0/1024.0/1024.0) );
	
}

#ifndef UE_APPLE_USE_MIMALLOC_MIN_FREE_RAM_MB
#	define UE_APPLE_USE_MIMALLOC_MIN_FREE_RAM_MB 0
#endif

void SelectAllocatorToUse(const FPlatformMemoryStats& MemStats)
{
    ...
    if(getenv("UE4_FORCE_MALLOC_ANSI") != nullptr)
    {
        ...
        FPlatformMemory::AllocatorToUse = FPlatformMemory::EMemoryAllocatorToUse::Ansi;
        return;
    }
	...
	if (!ENABLE_PGO_PROFILE && UE_APPLE_USE_MIMALLOC_MIN_FREE_RAM_MB && ((MemStats.AvailablePhysical / 1024 / 1024) >= UE_APPLE_USE_MIMALLOC_MIN_FREE_RAM_MB))
	{
		NSLog(@"Using mimalloc allocator.\n");
		FPlatformMemory::AllocatorToUse = FPlatformMemory::EMemoryAllocatorToUse::Mimalloc;
		return;
	}
	if (USE_MALLOC_BINNED3)
	{
		if (!FApplePlatformMemory::CanOverallocateVirtualMemory())
		{
		...
		FPlatformMemory::AllocatorToUse = FPlatformMemory::EMemoryAllocatorToUse::Binned3;
		return;
	}
    else if (USE_MALLOC_BINNED2)
    {
		NSLog(@"Using MallocBinned2 allocator.\n");
		FPlatformMemory::AllocatorToUse = FPlatformMemory::EMemoryAllocatorToUse::Binned2;
        return;
    }
    else
    {
...
#if !defined(UE_APPLE_DISABLE_MALLOC_ROUTER)
#	define UE_APPLE_DISABLE_MALLOC_ROUTER 0
#endif

// We need to create this decorator to route any calls to delete if that pointer was allocated from the crt allocator as some system libs mix allocator calls
template <class T>
struct FAppleMallocRouter final : public T
{
#if !UE_APPLE_DISABLE_MALLOC_ROUTER
	virtual void* Realloc(void* Ptr, SIZE_T NewSize, uint32 Alignment) override
	{
		if (Ptr && FPlatformMemory::PtrIsOSMalloc(Ptr))
		{
			return ::realloc(Ptr, NewSize);
		}

		return T::Realloc(Ptr, NewSize, Alignment);
	}
	...
	virtual void* TryRealloc(void* Ptr, SIZE_T NewSize, uint32 Alignment) override
	{
		...
		return T::TryRealloc(Ptr, NewSize, Alignment);
	}
...
FMalloc* FApplePlatformMemory::BaseAllocator()
{
    ...
    switch (AllocatorToUse)
    {
		...
		{
			Instance = new FAppleMallocRouter<FMallocMimalloc>();
			break;
		}

		case EMemoryAllocatorToUse::Binned3:
		{
			Instance = new FAppleMallocRouter<FMallocBinned3>();
			break;
		}

        case EMemoryAllocatorToUse::Binned2:
        {
            Instance = new FAppleMallocRouter<FMallocBinned2>();
            break;
        }
        ...
        case EMemoryAllocatorToUse::Binned:
        {
            ...
            host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info_t)&Stats, &StatsSize);
...
FPlatformMemoryStats FApplePlatformMemory::GetStats()
{
	...
	// Gather platform memory stats for the host and the specific process.
	vm_statistics64 Stats{};
	task_vm_info ProcessStats{};
	mach_msg_type_number_t StatsSize = HOST_VM_INFO64_COUNT;
	mach_msg_type_number_t ProcessStatsSize = TASK_VM_INFO_COUNT;
	FMemory::Memset(&Stats, 0, StatsSize);
	if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info_t)&Stats, &StatsSize))
	{
		UE_LOGF(LogTemp, Warning, "Failed to fetch vm statistics");
	}
	if (task_info(mach_task_self(), TASK_VM_INFO, (task_info_t)&ProcessStats, &ProcessStatsSize))
	{
		UE_LOGF(LogTemp, Warning, "Failed to fetch task-level vm statistics");
	}
	...
	MemoryStats.UsedVirtual = ProcessStats.virtual_size;
...
const FPlatformMemoryConstants& FApplePlatformMemory::GetConstants()
{
	...
	if( MemoryConstants.TotalPhysical == 0 )
	{
		...
		mach_msg_type_number_t HostSize = sizeof(vm_statistics64_data_t) / sizeof(integer_t);
		...
		if (host_statistics64(HostPort, HOST_VM_INFO64, (host_info_t)&vm_stat, &HostSize) != KERN_SUCCESS)
		{
			UE_LOGF(LogTemp, Warning, "Failed to fetch vm statistics");
...
#define UE_PLATFORM_SANITY_CHECK_OS_ALLOCATIONS			(UE_BUILD_DEBUG || (UE_BUILD_DEVELOPMENT && (UE_GAME || UE_SERVER) && !MALLOC_LEAKDETECTION))
...
void* _Nullable FApplePlatformMemory::BinnedAllocFromOS(SIZE_T Size)
{
...
#if UE_PLATFORM_SANITY_CHECK_OS_ALLOCATIONS
	...
	if (Ptr == (void*)-1)
	{
		UE_LOGF(LogTemp, Warning, "mmap failure allocating %zu, error code: %d", Size, errno);
		Ptr = nullptr;
	}
...
void FApplePlatformMemory::BinnedFreeToOS(void* Ptr, SIZE_T Size)
{
	...
	#if (UE_PLATFORM_SANITY_CHECK_OS_ALLOCATIONS)
	{
		...
		{
			UE_LOGF(LogHAL, Fatal, "BinnedFreeToOS() has been passed an address %p (size %llu) not allocated through it.", Ptr, (uint64)Size);
			// unreachable
			return;
		}
		...
		{
			UE_LOGF(LogHAL, Fatal, "BinnedFreeToOS(): info mismatch: descriptor ptr: %p, size %llu, but our pointer is %p and size %llu.", PointerToUnmap, SizeToUnmap, AllocDescriptor, (uint64)(SizeInWholePages + DescriptorSize));
			// unreachable
			return;
		}
		...
		{
			UE_LOGF(LogHAL, Fatal, "BinnedFreeToOS(): info mismatch: descriptor original size %llu, our size is %llu for pointer %p", AllocDescriptor->OriginalSizeAsPassed, Size, Ptr);
			// unreachable
			return;
		}
	...
	if (munmap(Ptr, Size) != 0)
	{
		...
		UE_LOGF(LogHAL, Fatal, "munmap(addr=%p, len=%llu) failed with errno = %d (%ls)", Ptr, Size,
...
FApplePlatformMemory::FPlatformVirtualMemoryBlock FApplePlatformMemory::FPlatformVirtualMemoryBlock::AllocateVirtual(size_t InSize, size_t InAlignment)
{
	...
	Result.Ptr = MmapWithTag(nullptr, Result.GetActualSize(), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_NORESERVE, MmapTag::AllocateVirtualMemoryBlock, 0);
...
void* LLMAlloc(size_t Size)
{
	...
	{
		const int ErrNo = errno;
		UE_LOGF(LogHAL, Error, "mmap(Size=%llu) failed with errno = %d (%ls)", Size,
			ErrNo, StringCast< TCHAR >(strerror(ErrNo)).Get());
	}
	else
	{
...
void LLMFree(void* Addr, size_t Size)
{
    ...
    {
        const int ErrNo = errno;
        UE_LOGF(LogHAL, Fatal, "munmap(addr=%p, len=%llu) failed with errno = %d (%ls)", Addr, Size,
               ErrNo, StringCast< TCHAR >(strerror(ErrNo)).Get());
    }
}
