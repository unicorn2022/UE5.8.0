...
FString FXxHash64::ToString() const
{
	FString Out;
	AppendString(Out);
	return Out;
...
void FXxHash64::FromString(FStringView Src)
{
	check(Src.Len() == GetStringLen());
	uint8 Bytes[sizeof(Hash)];
	UE::String::HexToBytes(Src, Bytes);
	*this = FromByteArray(Bytes);
}

void FXxHash64::AppendString(FString& Out) const
{
	uint8 Bytes[sizeof(Hash)];
	ToByteArray(Bytes);
	BytesToHex(Bytes, sizeof(Bytes), Out);
}

FString LexToString(const FXxHash64& InHash)
{
	return InHash.ToString();
}

void LexFromString(FXxHash64& InHash, const TCHAR* InString)
{
...
bool FXxHash64::LoadFromCompactBinary(const FCbFieldView& Field)
{
	FMemoryView BytesView = const_cast<FCbFieldView&>(Field).AsBinaryView();
	if (BytesView.GetSize() != sizeof(Hash))
	{
		*this = FXxHash64();
		return false;
	}
	const uint8 (*Bytes)[sizeof(Hash)] = reinterpret_cast<const uint8(*)[sizeof(Hash)]>(BytesView.GetData());
	*this = FromByteArray(*Bytes);
	return true;
}

FCbWriter& FXxHash64::WriteCompactBinary(FCbWriter& Writer) const
{
	uint8 Bytes[sizeof(Hash)];
	ToByteArray(Bytes);

	Writer.AddBinary(FMemoryView(Bytes, sizeof(Bytes)));
	return Writer;
}
