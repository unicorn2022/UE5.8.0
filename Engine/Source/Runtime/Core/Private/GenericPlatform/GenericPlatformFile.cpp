...
{
	// Note: cannot forward to OpenMappedEx2 here. OpenMappedEx has inconsistent behavior on different platforms,
	// so the call must be forwarded in the platform implementation

	return MakeError(TEXT("OpenMappedEx is not implemented on this platform"));
}

FOpenMappedResult IPlatformFile::OpenMappedEx2(const TCHAR* Filename, EOpenMappedFlags OpenOptions, int64 MaximumSize)
{
	return MakeError(TEXT("OpenMappedEx2 is not implemented on this platform"));
}

IFileHandle* IPlatformFile::OpenWrite(const TCHAR* Filename, bool bAppend, bool bAllowRead)
{
	EOpenWriteFlags Flags = EOpenWriteFlags::None;
	if (bAppend)
	{
		Flags |= EOpenWriteFlags::Append;
	}
	if (bAllowRead)
	{
		Flags |= EOpenWriteFlags::AllowRead;
	}

	FFileOpenResult Result = OpenWrite(Filename, Flags);
	if (Result.HasValue())
	{
...
bool IPlatformFile::CopyFile(const TCHAR* To, const TCHAR* From, EPlatformFileRead ReadFlags, EPlatformFileWrite WriteFlags)
{
	...
	{
		bool bFailed = true;
		if ((WriteFlags & EPlatformFileWrite::AttemptDeleteAndCreate) != EPlatformFileWrite::None)
		{
			if (DeleteFile(To))
			{
				ToFile = TUniquePtr<IFileHandle>(OpenWrite(To, false, (WriteFlags & EPlatformFileWrite::AllowRead) != EPlatformFileWrite::None));
				bFailed = (!ToFile);
			}
		}
		if (bFailed)
