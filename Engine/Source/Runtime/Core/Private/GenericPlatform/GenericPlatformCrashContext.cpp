...
static TAutoConsoleVariable<bool> CVarTraceSnapshotOnCrash(
	TEXT("CrashContext.TraceSnapshotOnCrash"),
	false,
	TEXT("Execute a trace snapshot and include in the crash report."),
	ECVF_ReadOnly
);

static TAutoConsoleVariable<int32> CVarTraceSnapshotOnCrashTailSize(
	TEXT("CrashContext.TraceSnapshotOnCrashTailSize"),
	0,
	TEXT("Max limit of tail size to write to snapshot. Zero indicates no limit."),
	ECVF_ReadOnly
);

static bool GCrashContextFormatEarlyCrashLog = false;
static FAutoConsoleVariableRef CVarCrashContextFormatEarlyCrashLog(
	TEXT("CrashContext.FormatEarlyCrashLog"),
	GCrashContextFormatEarlyCrashLog,
	TEXT("If true, log lines written to the crash report log will include timestamps and category tags,	matching the format of the on-disk log.\n"
		"Only affects crashes that occur before log output devices are fully initialized, i.e. before the log backlog is disabled. (default = false)"),
	ECVF_Default
);

/**
* Output device used to write the file log's backlog in the case of an early crash before the file log is ready.
*/
class FCrashBacklogOutputDevice : public FOutputDevice
{
public:
	FCrashBacklogOutputDevice(FArchive* InArchive)
		: LogAr(InArchive)
	{
		check(InArchive);
	}
	...
	CORE_API void Serialize(const TCHAR* Data, ELogVerbosity::Type Verbosity, const class FName& Category)
	{
		Serialize(Data, Verbosity, Category, -1.0);
	}

	CORE_API void Serialize(const TCHAR* Data, ELogVerbosity::Type Verbosity, const class FName& Category, const double Time)
	{
		if (Verbosity != ELogVerbosity::SetColor)
		{
			FOutputDeviceHelper::FormatCastAndSerializeLine(*LogAr, Data, Verbosity, Category, Time, bSuppressEventTag, bAutoEmitLineTerminator);
		}
	}
...
FAdditionalCrashContextDelegateEx FGenericCrashContext::AdditionalCrashContextDelegateEx;
...
void FGenericCrashContext::Initialize()
{
	...
	Symbols = FString::Printf(TEXT("%s-%s"), *Symbols, *BuildFlavor);
#else
	...
	UE_LOGF(LogInit, Log, "Session CrashGUID >====================================================\n         Session CrashGUID >   %ls\n         Session CrashGUID >====================================================", *GuidRoot);
	...
	FCoreDelegates::CrashOverrideParamsChanged.AddLambda([](const FCrashOverrideParameters& InParams)
	{
		...
		{
			NCached::Set(NCached::Session.GameName, *(FString("UE-") + FApp::GetProjectName() + InParams.GameNameSuffix));
		}
	...
	FCoreDelegates::OnPostFork.AddLambda([](EForkProcessRole Role)
	{
		...
		{
			UE_LOGF(LogCrashContext, VeryVerbose, "Updating forked child Session ProcessID: %u -> %u", NCached::Session.ProcessId, FPlatformProcess::GetCurrentProcessId());
	...
	SetEngineData(TEXT("BuildFlavor"), BuildFlavor);
...
void FGenericCrashContext::CopySharedCrashContext(FSharedCrashContext& Dst)
{
	...
	TStringBuilder<0> DynamicDataBuilder;
...
void FGenericCrashContext::InitializeFromConfig()
{
	...
	if (const FConfigSection* CRCConfigSection = GConfig->GetSection(ConfigSectionName, bForceGetSection, GEngineIni))
	{
...
#if WITH_EDITOR
	if (const FConfigSection* CRCEConfigSection = GConfig->GetSection(ConfigSectionName, bForceGetSection, GEditorIni))
	{
		for (FConfigSection::TConstIterator ConfigValueIt(*CRCEConfigSection); ConfigValueIt; ++ConfigValueIt)
		{
			CrashConfigFile.SetInSection(ConfigSectionName, ConfigValueIt.Key(), ConfigValueIt.Value().GetValue());
		}
...
void FGenericCrashContext::SerializeSessionContext(FString& Buffer)
{
	...
	AddCrashPropertyInternal(Buffer, TEXT("Misc.Is64bitOperatingSystem"), 1);
...
bool FGenericCrashContext::ShouldIncludeInPortableCallStackHash(const FCrashStackFrame& Frame) const
{
	// This may allocate if its the first time calling into this function
	static const TCHAR* ExeName = FPlatformProcess::ExecutableName();
	...
	return Frame.ModuleName == ExeName || Frame.ModuleName.Contains(UEEditorName);
}
...
void FGenericCrashContext::AddPortableCallStackHash() const
{
	...
	{
		if (ShouldIncludeInPortableCallStackHash(*It))
		{
...
void FGenericCrashContext::SetEngineData(const FString& Key, const FString& Value)
{
	if (Value.Len() == 0)
	{
		...
		UE_SUPPRESS(LogCrashContext, VeryVerbose, 
		{
			...
			{
				UE_LOGF(LogCrashContext, VeryVerbose, "FGenericCrashContext::SetEngineData(%ls, <RemoveKey>)", *Key);
			}
		});
	...
	else
	{
		...
		UE_SUPPRESS(LogCrashContext, VeryVerbose, 
		{
			if (OldVal != Value)
			{
				UE_LOGF(LogCrashContext, VeryVerbose, "FGenericCrashContext::SetEngineData(%ls, %ls)", *Key, *Value);
			}
		});
		OldVal = Value;
	}
...
void FGenericCrashContext::SetGameData(const FString& Key, const FString& Value)
{
	if (Value.Len() == 0)
	{
		...
		UE_SUPPRESS(LogCrashContext, VeryVerbose, 
		{
			...
			{
				UE_LOGF(LogCrashContext, VeryVerbose, "FGenericCrashContext::SetGameData(%ls, <RemoveKey>)", *Key);
			}
		});
	...
	else
	{
		...
		UE_SUPPRESS(LogCrashContext, VeryVerbose, 
		{
			if (OldVal != Value)
			{
				UE_LOGF(LogCrashContext, VeryVerbose, "FGenericCrashContext::SetGameData(%ls, %ls)", *Key, *Value);
			}
		});
		OldVal = Value;
	}
...
FString FGenericCrashContext::DumpLog(const FString& CrashFolderAbsolute)
{
	...
	if (bMemoryOnly || bBacklogEnabled)
	{
		...
		if (LogFile)
		{
			...
			{
				FCrashBacklogOutputDevice Wrapper(LogFile.Get());
				Wrapper.SetSuppressEventTag(!GCrashContextFormatEarlyCrashLog);
				TGuardValue GuardPrintLogTimes(GPrintLogTimes, ELogTimes::SinceGStartTime);
...
void FGenericCrashContext::GenerateFiles(const TCHAR* OutputDirectory)
{
	if (CVarTraceSnapshotOnCrash.GetValueOnAnyThread())
	{
		FString SnapshotPath = FPaths::Combine(OutputDirectory, TEXT("Snapshot.utrace"));
		FTraceAuxiliary::WriteSnapshot(*SnapshotPath, CVarTraceSnapshotOnCrashTailSize.GetValueOnAnyThread());
	}
}
...
void FGenericCrashContext::CopyPlatformSpecificFiles(const TCHAR* OutputDirectory, void* Context)
{
	...
	// Use the physical file system directly rather than IFileManager, to avoid 
	// being denied in scenarios where bAllowNonUFSIniWhenCooked=false
	IPlatformFile* PhysicalFile = FPlatformFileManager::Get().FindPlatformFile(IPlatformFile::GetPhysicalTypeName());
	if (PhysicalFile != nullptr && PhysicalFile->FileExists(CrashConfigSrcPath))
	{
		...
		PhysicalFile->CopyFile(*CrashConfigDstAbsolute, CrashConfigSrcPath);	// best effort, so don't care about result: couldn't copy -> tough, no config
	}
	...
	for (const FString& AbsoluteSource : NCached::FilePaths)
	{
		if (IFileManager::Get().FileExists(*AbsoluteSource))
		{
			...
			{
				UE_LOGF(LogCrashContext, Error, "Error moving crash context file '%ls' to '%ls'", *AbsoluteSource, *AbsoluteDest);
			}
		}
	}
}
...
struct FCrashContextExtendedWriterImpl : public FCrashContextExtendedWriter
{
	...
	/** MoveFile a file into the report */
	virtual bool MoveFile(const TCHAR* FromFilePath) override
	{
		TCHAR DestFilePath[1024] = { 0 };
		bool bFileMoved = false;
		FString ToFileName = *FPaths::GetCleanFilename(FromFilePath);
		int32 DestPathLength = FCString::Snprintf(DestFilePath, UE_ARRAY_COUNT(DestFilePath), TEXT("%s/%s"), OutputDirectory, *ToFileName);
		if (DestPathLength>0 && DestPathLength < UE_ARRAY_COUNT(DestFilePath))
		{
			IPlatformFile& PlatformFile = IPlatformFile::GetPlatformPhysical();
			bFileMoved = PlatformFile.MoveFile(DestFilePath, FromFilePath);
			if (!bFileMoved) // attempt copy the file if the move fails. (cant move across file systems etc)
			{
				bFileMoved = PlatformFile.CopyFile(DestFilePath, FromFilePath);
				if (bFileMoved)
				{
					PlatformFile.DeleteFile(FromFilePath);
				}
				else
				{
					FPlatformMisc::LowLevelOutputDebugStringf(TEXT("Crash Context FileMove failed. (%s , %s)"), FromFilePath, DestFilePath);
				}
			}
		}
		return bFileMoved;
...
void FGenericCrashContext::DumpAdditionalContext(const TCHAR* CrashFolderAbsolute)
{
	...
	DumpAdditionalContext(CrashFolderAbsolute, Params);
}

void FGenericCrashContext::DumpAdditionalContext(const TCHAR* CrashFolderAbsolute, const FAdditionalCrashContextParams& Params)
{
#if WITH_ADDITIONAL_CRASH_CONTEXTS 
	...
	if (!UE::IsGCLocked())
	{
		...
		AdditionalCrashContextDelegateEx.Broadcast(Writer, Params);
	}
