...
bool FGenericPlatformSymbolication::SaveSymbolDatabaseForBinary(FString TargetFolder, FString Name, FGenericPlatformSymbolDatabase& Database)
{
	...
	if(!DataArchive.GetError())
	{
		...
		{
			UE_LOGF( LogGenericPlatformSymbolication, Error, "Unable to write debug symbols to output file." );
		}
	}
	else
	{
		UE_LOGF( LogGenericPlatformSymbolication, Error, "Unable to serialise debug symbols." );
	}
