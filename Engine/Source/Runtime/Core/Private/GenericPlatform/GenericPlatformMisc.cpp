...
struct FLogNameEventStats
{
	...
	void Dump()
	{
		...
		UE_LOGF(LogGenericPlatformMisc, Log, "*********Log Named Events *********");
		UE_LOGF(LogGenericPlatformMisc, Log, "******* Top %i NamedEvents", LogCount);
		...
		for (const TPair<FName, FNameEventEntry>& Pair : NamedEventsMap)
		{
			if (Index <= LogCount)
			{
				UE_LOGF(LogGenericPlatformMisc, Log, "%ls : Max: %lu Total: %lu Avg: %f", *(Pair.Key.ToString()), Pair.Value.MaxFrameCount, Pair.Value.Total, (float)Pair.Value.Total / (CurrentFrameCount - (float)StartFrame));
			}
			Total += Pair.Value.Total;
			Index++;
		}
		UE_LOGF(LogGenericPlatformMisc, Log, "***********************************");
		UE_LOGF(LogGenericPlatformMisc, Log, "Total Unique NamedEvents: %i", NamedEventsMap.Num());
		UE_LOGF(LogGenericPlatformMisc, Log, "Average NamedEvents per frame: %f", (float)Total / (CurrentFrameCount - (float)StartFrame));
		UE_LOGF(LogGenericPlatformMisc, Log, "*********Log Named Events *********");
	}
	...
	void ParseSortType(const FString& Arg)
	{
		...
		{
			UE_LOGF(LogGenericPlatformMisc, Log, "LogNamadEvent: Bad Sort argument. Option: Avg, Spike or Total");
		}
	}
...
static FAutoConsoleCommand GLogNamedEventsCmd(
	...
	TEXT("Log named events Commands. LogNamedEvents Dump, Reset, Sort [Avg|Spike|Total], MaxOutput [int value]"),
		FConsoleCommandWithArgsDelegate::CreateStatic([](const TArray<FString>& Args)
		{
			...
			if (Args.Num() >= 1)
			{
				if (Args[0].Compare(FString("Reset"), ESearchCase::IgnoreCase) == 0)
				{
					LogNameEventStats.StartFrame = GFrameCounter;
					LogNameEventStats.Reset();
				}
				else if (Args[0].Compare(FString("Dump"), ESearchCase::IgnoreCase) == 0)
				{
					LogNameEventStats.Dump();
				}
				else if (Args[0].Compare(FString("Sort"), ESearchCase::IgnoreCase) == 0)
				{
					if (Args.Num() == 2)
					{
				...
				else if (Args[0].Compare(FString("MaxOutput"), ESearchCase::IgnoreCase) == 0)
				{
					if (Args.Num() == 2)
					{
				...
				{
					UE_LOGF(LogGenericPlatformMisc, Log, "LogNamadEvent: Bad argument. Option: Reset, Dump, Sort [Avg,Sort,Total] or MaxOutput [int]");
				}
			}
		}
	)
...
{
	UE_LOGF(LogGenericPlatformMisc, Error, "SetEnvironmentVar not implemented for this platform: %ls = %ls", VariableName, Value);
}
...
void FGenericPlatformMisc::SubmitErrorReport( const TCHAR* InErrorHist, EErrorReportMode::Type InMode )
{
	if ((!FPlatformMisc::IsDebuggerPresent() || GAlwaysReportCrash) && !FParse::Param(FCommandLine::Get(), TEXT("CrashForUAT")))
	{
		if ( GUseCrashReportClient )
		{
			...
			{
				UE_LOGF(LogGenericPlatformMisc, Error, "This platform does not implement KillAllPopUpBlockingWindows");
			}
		}
		else
		{
			UE_LOGF(LogGenericPlatformMisc, Error, "This platform cannot submit a crash report. Report was:\n%ls", InErrorHist);
		}
	}
}
...
uint32 FGenericPlatformMisc::GetFeatureBits_X86()
{
	//
	...
	bool bHasFMA = (CpuInfo[2] & (1u << 12)) != 0;
	...
	if (bAVXContextSupport)
	{
		...
		if (bHasFMA)
		{
			FeatureBits |= ECPUFeatureBits_X86::FMA;
		}
	}
...
bool FGenericPlatformMisc::HasAVX2InstructionSupport()
{
...
FString FGenericPlatformMisc::GetCPUVendor()
{
	...
	return FString( "GenericCPUVendor" );
}

FString FGenericPlatformMisc::GetCPUBrand()
{
	// Not implemented cross-platform. Each platform may or may not choose to implement this.
	return FString( "GenericCPUBrand" );
}
...
{
	// Not implemented cross-platform. Each platform may or may not choose to implement this.
	return FString( "GenericGPUBrand" );
}
...
{
	// Not implemented cross-platform. Each platform may or may not choose to implement this.
	out_OSVersionLabel = FString( "GenericOSVersionLabel" );
	out_OSSubVersionLabel = FString( "GenericOSSubVersionLabel" );
}
...
bool FGenericPlatformMisc::SetStoredValue(const FString& InStoreId, const FString& InSectionName, const FString& InKeyName, const FString& InValue)
{
	...
	const FString ConfigPath = FString(FPlatformProcess::ApplicationSettingsDir()) / InStoreId / FString("KeyValueStore.ini");
...
{
	UE_LOGF(LogGenericPlatformMisc, Log,  "FPlatformMisc::RequestExit(%i, %ls)", Force,
...
{
	UE_LOGF(LogInit, Display, "Restart application (with cmdnline) is not supported or implemented in current platform");
	return false;
}

bool FGenericPlatformMisc::RestartApplication()
{
	UE_LOGF(LogInit, Display, "Restart application is not supported or implemented in current platform");
	return false;
}
...
void FGenericPlatformMisc::RequestExitWithStatus(bool Force, uint8 ReturnCode, const TCHAR* CallSite)
{
	...
	UE_LOGF(LogGenericPlatformMisc, Warning, "FPlatformMisc::RequestExitWithStatus(%i, %d) - return code will be ignored by the generic implementation.", Force, ReturnCode);
...
EAppReturnType::Type FGenericPlatformMisc::MessageBoxExt( EAppMsgType::Type MsgType, const TCHAR* Text, const TCHAR* Caption )
{
	...
	UE_LOGF(LogGenericPlatformMisc, Warning, "MessageBox: %ls : %ls", Caption, Text);
...
const FString FGenericPlatformMisc::VersionCheckPlatformName()
{
	...
	if (!GConfig->GetString(TEXT("PatchConfig"), TEXT("PlatformOverrideName"), OverrideVersionCheckPlatformName, GEngineIni))
	{
		...
		FString PlatformNameSuffix;
		if (FParse::Value(FCommandLine::Get(), TEXT("PlatformSuffix="), PlatformNameSuffix))
		{
			OverrideVersionCheckPlatformName += PlatformNameSuffix;
		}
	}
...
static void MakeEngineDir(FString& OutEngineDir)
{
	...
	FString DefaultEngineDir = "../../../Engine/";
#if PLATFORM_DESKTOP
	...
	UE_LOGF(LogGenericPlatformMisc, Warning, "Failed to determine engine directory: Defaulting to %ls", *OutEngineDir);
#endif
...
bool FGenericPlatformMisc::IsPreferredInputDevice(const TCHAR* DeviceName)
{
	// If given a null treat it as always being enabled.
	// This ensures that users who have written their own IInputDeviceModule code will
	// continue working and they can add support for this feature if they want to.
	if (!DeviceName)
	{
		return true;
	}
	
	bool bIsPreferredAPICheckEnabled = false;
	
	GConfig->GetBool(TEXT("/Script/Engine.InputSettings"), TEXT("bEnablePreferredInputAPIPreferences"), OUT bIsPreferredAPICheckEnabled, GInputIni);
	
	// If the project setting is not enabled, always return true because we want every API to be enabled
	if (!bIsPreferredAPICheckEnabled)
	{
		return true;
	}

	const FString CurrentPreferredInputAPIs = FGenericPlatformMisc::GetCurrentPreferredInputDevices();

	UE_CLOGF(CurrentPreferredInputAPIs.IsEmpty(), LogGenericPlatformMisc, Warning, "bEnablePreferredInputAPIPreferences is true, but no default preferred APIs were provided. Please fill out DefaultPreferredInputAPIList in the input settings for this to have any affect");
	
	// If the given device is contained in our comma-seperated string value, then it is preferred
	return CurrentPreferredInputAPIs.Contains(DeviceName);
}

void FGenericPlatformMisc::SetPreferredInputDevices(const TCHAR* DeviceNames)
{
	GConfig->SetString(TEXT("GamepadAPIPreference"), TEXT("PreferredAPI"), DeviceNames, GGameUserSettingsIni);

	UE_LOGF(LogGenericPlatformMisc, Log, "Changed preferred gamepad API to '%ls'", DeviceNames);
}

void FGenericPlatformMisc::ResetPreferredInputDevicesToDefault()
{
	// Clear the user set value
	GConfig->RemoveKey(TEXT("GamepadAPIPreference"), TEXT("PreferredAPI"), GGameUserSettingsIni);

	// Set it to the default specified in the project if there is one
	FString DefaultPreferredAPIs = TEXT("");
	const bool bHasDefaultPreferredList = GConfig->GetString(TEXT("/Script/Engine.InputSettings"), TEXT("DefaultPreferredInputAPIList"), DefaultPreferredAPIs, GInputIni);
	if (bHasDefaultPreferredList && !DefaultPreferredAPIs.IsEmpty())
	{
		SetPreferredInputDevices(*DefaultPreferredAPIs);
	}

	UE_LOGF(LogGenericPlatformMisc, Log, "Changed preferred gamepad API to '%ls'", *DefaultPreferredAPIs);
}

FString FGenericPlatformMisc::GetCurrentPreferredInputDevices()
{
	// If the API check is enabled, we want to look at the game's user preference INI and see what the use has selected for their API 
	FString CurrentPreferredAPIs = TEXT("");

#if WITH_EDITOR
	// First, check the editor preference for overriden API preferences here allowing for easier testing
	if (GConfig->GetString(TEXT("/Script/UnrealEd.LevelEditorPlaySettings"), TEXT("PreferredInputAPIOverride"), OUT CurrentPreferredAPIs, GEditorPerProjectIni) && !CurrentPreferredAPIs.IsEmpty())
	{
		return CurrentPreferredAPIs;
	}
#endif

	// Next, check if the user has set a preference yet. If they have, use that
	if (GConfig->GetString(TEXT("GamepadAPIPreference"), TEXT("PreferredAPI"), OUT CurrentPreferredAPIs, GGameUserSettingsIni))
	{
		return CurrentPreferredAPIs;
	}
		
	// Lastly, use the project's default preferred list if there is one.
	if (GConfig->GetString(TEXT("/Script/Engine.InputSettings"), TEXT("DefaultPreferredInputAPIList"), OUT CurrentPreferredAPIs, GInputIni))
	{
		return CurrentPreferredAPIs;
	}
...
{
	return TEXT(UE_STRINGIZE(UBT_COMPILED_PLATFORM));
}

const TCHAR* FGenericPlatformMisc::GetUBTTarget()
{
	return TEXT(UE_STRINGIZE(UBT_COMPILED_TARGET));
}

const TCHAR* FGenericPlatformMisc::GetUBTArchitecture()
{
	return TEXT(UE_STRINGIZE(UBT_COMPILED_ARCHITECTURE));
}
...
using FUBTTargetNameArrayType = TCHAR[128];
	...
	static FUBTTargetNameArrayType& GetStaticUBTTargetName()
	{
		static struct FInitializer
		{
			FInitializer()
			{
				...
				FCString::Strcpy(UBTTargetName, TEXT("Unreal" UE_STRINGIZE(UBT_COMPILED_TARGET)));
			}
	...
	static FUBTTargetNameArrayType& GetStaticUBTTargetName()
	{
		/** The name of the UBT target that the current executable was built from. Defaults to the UE default target for this type to make content only projects work,
			but will be overridden by the primary game module if it exists */
		static FUBTTargetNameArrayType GUBTTargetName = TEXT("Unreal" UE_STRINGIZE(UBT_COMPILED_TARGET));
...
float FGenericPlatformMisc::GetDeviceTemperature()
{ 
	return -1.0f; 
}

EDeviceThermalState FGenericPlatformMisc::GetDeviceThermalState()
{ 
...
bool FGenericPlatformMisc::IsDeviceGamingHandheld()
{
	return false;
}
...
FDelegateHandle FGenericPlatformMisc::RegisterAudioFocusChangedEventListener(const FOnAudioFocusChangedDelegate Callback)
{
	// not implemented by default
	return FDelegateHandle();
}

void FGenericPlatformMisc::UnregisterAudioFocusChangedEventListener(const FDelegateHandle DelegateHandle)
{
	// not implemented by default
}
...
bool FGenericPlatformMisc::FileExistsInPlatformPackage(const FString& RelativePath)
{
	...
	{
		UE_LOGF(LogGenericPlatformMisc, Warning, "FileExistsInPlatformPackage: expected a relative path, but received %ls", *RelativePath);
		return false;
	}
...
void FGenericPlatformMisc::SanitizeSystemWideMutexName(FString& InName)
{
	// Remove all invalid filesystem characters from the provided name but also 
	// remove '.' since we don't want names with . or .. to be interpreted as 
	// paths for platforms that might require handle names to be valid paths
	const FString InvalidChars = FString::Printf(TEXT("%s."), *FPaths::GetInvalidFileSystemChars());

	int32 Index = INDEX_NONE;
	for (TCHAR& Char : InName)
	{
		if (InvalidChars.FindChar(Char, Index))
		{
			Char = TEXT('_');
		}
	}
...
{
#if !UE_BUILD_SHIPPING
	UE_LOGF(LogGenericPlatformMisc, Log, "Show console is not supported or implemented in current platform");
#endif
}
...
{
	UE_LOGF(LogGenericPlatformMisc, Warning, "FGenericPlatformMisc::AddNetworkListener not implemented for this platform");
...
{
	UE_LOGF(LogGenericPlatformMisc, Warning, "FGenericPlatformMisc::RemoveNetworkListener not implemented for this platform");
...
const FUtf8StringView FGenericPlatformMisc::GetConfigRulesVariableAsUtf8(const FUtf8StringView Key)
{
