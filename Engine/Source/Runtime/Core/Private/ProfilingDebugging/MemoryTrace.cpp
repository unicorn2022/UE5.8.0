...
#if UE_TRACE_ENABLED
UE_TRACE_CHANNEL_DEFINE(MemAllocChannel, "Traces memory allocations and displays them in the Memory Insights Profiler. Optional callstack data is associated using Module + Callstack channels. \
Optional asset metadata is associated using Metadata + AssetMetadata channels.", true)
#endif
...
static EMemoryTraceInit MemoryTrace_ShouldEnable(int32 ArgC, const ArgCharType* const* ArgV)
{
	...
	for (int32 ArgIndex = 1; ArgIndex < ArgC; ArgIndex++)
	{
		...
		int32 ArgStart = INDEX_NONE;
		while (ArgView.FindChar(CHARTEXT(ArgCharType, '-'), ArgStart))
		{
			ArgView.RightChopInline(ArgStart);

			int32 SpacePos = INDEX_NONE;
			ArgView.FindChar(CHARTEXT(ArgCharType, ' '), SpacePos);
			TStringView<ArgCharType> SubArg = (SpacePos == INDEX_NONE) ? ArgView : ArgView.Left(SpacePos);

			if (SubArg.Left(7).Equals(CHARTEXTVIEW(ArgCharType, "-trace="), ESearchCase::IgnoreCase))
			{
				TStringView<ArgCharType> ChannelsOrPresets = SubArg.RightChop(7);

				while (!ChannelsOrPresets.IsEmpty())
				{
					int32 CommaPos = INDEX_NONE;
					ChannelsOrPresets.FindChar(CHARTEXT(ArgCharType, ','), CommaPos);
					TStringView<ArgCharType> Channel = (CommaPos == INDEX_NONE) ? ChannelsOrPresets : ChannelsOrPresets.Left(CommaPos);

					if (Channel.Equals(CHARTEXTVIEW(ArgCharType, "memalloc"), ESearchCase::IgnoreCase))
					{
					...
					else if (Channel.Equals(CHARTEXTVIEW(ArgCharType, "callstack"), ESearchCase::IgnoreCase))
					{
						Mode |= EMemoryTraceInit::Callstacks;
					}
					else if (Channel.Equals(CHARTEXTVIEW(ArgCharType, "memtag"), ESearchCase::IgnoreCase))
					{
						// The "memtag" channel is only used to enable LLM tracing.
					}
					else if (Channel.Equals(CHARTEXTVIEW(ArgCharType, "probing"), ESearchCase::IgnoreCase))
					{
						Mode |= EMemoryTraceInit::Probing;
					}
					else if (Channel.Equals(CHARTEXTVIEW(ArgCharType, "memory"), ESearchCase::IgnoreCase))
					{
						Mode |= EMemoryTraceInit::Full;
					}
					else if (Channel.Equals(CHARTEXTVIEW(ArgCharType, "memory_light"), ESearchCase::IgnoreCase))
					{
...
FMalloc* MemoryTrace_CreateInternal(FMalloc* InMalloc, EMemoryTraceInit Mode)
{
	...
	// to be updated which we will do that in response to memory events.
