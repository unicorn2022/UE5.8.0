		...
		FString ActiveProfileName = FAndroidProfiler::StartCapture(ProfilerArgs, [](const FAndroidProfiler::FProfileResults& Results)
			{
				if (!Results.FilePath.IsEmpty())
				{
					...
					UE_CLOGF(!Results.Error.IsEmpty(), LogProfilingDebugging, Warning, "Profile %ls completed with log: %ls", *Results.ProfileName, *Results.Error);
					...
					UE_LOGF(LogProfilingDebugging, Log, "Profile %ls complete, retrieve via 'adb pull %ls'", *Results.ProfileName, *DestAbsolutePath);
				}
				else
				{
					UE_LOGF(LogProfilingDebugging, Error, "Profile %ls failed: %ls", *Results.ProfileName, *Results.Error);
				}
			});
