...
TAutoConsoleVariable<bool> CVarCsvUseLegacyFrameTime(
	TEXT("csv.UseLegacyFrameTime"),
	false,
	TEXT("If true uses the delta between EndFrame() calls to compute the frame time.")
	TEXT("If false, computes the frame time after the idle at the start of the frame (when using a frame rate limit)."),
	ECVF_Default
);
...
TAutoConsoleVariable<int32> CVarCsvShippingContinuousWrites(
	...
	-1,
	TEXT("Only applies in shipping buids. If != -1, overrides csv.ContinousWrites."),
	ECVF_Default
);

TAutoConsoleVariable<int32> CVarCsvShippingMaxFileSizeMB(
	TEXT("csv.Shipping.MaxFileSizeMB"),
	-1,
	TEXT("Only applies in shipping buids. If != -1, overrides csv.MaxFileSizeMB."),
	ECVF_Default
);
#endif

TAutoConsoleVariable<int32> CVarCsvMaxFileSizeMB(
	TEXT("csv.MaxFileSizeMB"),
	0,
	TEXT("Approximate maximum file size (in megabytes) for CSV profiler files.")
	TEXT("The profiler will stop writing values to the output file after this size is exceeded.")
	TEXT("Only supported when ContinuousWrites cvar is > 0.")
	TEXT("If <= 0, size is not limited."),
	ECVF_Default
);
...
TAutoConsoleVariable<float> CVarCsvProcessingThreadGtStallMaxUpdateLatencyMs(
	TEXT("csv.ProcessingThread.GtStallMaxUpdateLatencyMs"),
	0.0,
	TEXT("How far behind the processing thread can get (ms since last update) before we stall the GT. Detection is handled on the GT. Set to 0 to disable"),
	ECVF_Default
);
...
TAutoConsoleVariable<float> CVarCsvProcessingThreadGtStallUpdateTimeThresholdMs(
	...
	1000.0f,
	TEXT("Specifies the max time a processing thread update can take before we consider stalling the game thread. Used in combination with csv.ProcessingThread.GtStallUpdateEscalationThreshold\n")
...
const EThreadPriority GCsvThreadPriorities[] =
{
...
const TCHAR* GCsvThreadPriorityNames[] =
{
	TEXT("TPri_Lowest"),
...
static int32 GCsvThreadPriorityIndex = 0; 
FAutoConsoleVariableRef CVarCsvThreadPriority(
	TEXT("csv.ProcessingThread.Priority"),
	GCsvThreadPriorityIndex,
	TEXT("Thread priority of the CSV profiler thread: 0=Lowest, 1=BelowNormal, 2=SlightlyBelowNormal, 3=Normal, 4=AboveNormal, 5=Highest\n")
	TEXT("This is switchable at runtime"),
	ECVF_Default);

TAutoConsoleVariable<int32> CVarCsvTrackMemoryUse(
	TEXT("csv.TrackMemoryUse"),
	0,
	TEXT("if 1, tracks memory use via the CsvProfiler/MemUsedLastUpdateMB stat. This can take 0.5ms on the processing thread so is disabled by default"),
	ECVF_Default
);
...
// Set via config [CsvProfiler]:bEventTimestamps. Defaults to true
// If enabled, CSV events will include a timestamp, encoded with the following suffix : ##<time in seconds>
static bool GCsvEventTimestamps = true;
...
static bool GCsvProfilerInitialized = false;

static std::atomic<bool> GDelayProcessingThreadStart = false;
...
struct FCsvPersistentCustomStats
{
	...
	TCsvPersistentCustomStat<T>* GetOrCreatePersistentCustomStat(FName Name, int32 CategoryIndex, bool bResetEachFrame)
	{
		...
		if (FindStat)
		{
			...
			UE_LOGF(LogCsvProfiler, Fatal, "Error: Custom stat %ls was already registered with a different type", *Name.ToString());
		}
...
{
#if CSV_PROFILER_ALLOW_DEBUG_FEATURES
	static bool bContinuousWritesDisabled = FParse::Param(FCommandLine::Get(), TEXT("csvContinuousWritesDisabled"));
	if (bContinuousWritesDisabled)
	{
		return false;
	}
#endif
...
int32 GetMaxFileSizeMB(bool bGameThread)
{
	int32 CVarValue = -1;
#if UE_BUILD_SHIPPING
	CVarValue = bGameThread ? CVarCsvShippingMaxFileSizeMB.GetValueOnGameThread() : CVarCsvShippingMaxFileSizeMB.GetValueOnAnyThread();
	if (CVarValue == -1)
#endif
	{
		CVarValue = bGameThread ? CVarCsvMaxFileSizeMB.GetValueOnGameThread() : CVarCsvMaxFileSizeMB.GetValueOnAnyThread();
	}
	return CVarValue;
}
...
static CSV_PROFILER_INLINE void ValidateFName(const FName& StatName)
{
	...
	{
		UE_LOGF(LogCsvProfiler, Fatal, "Numbered FName stats (suffixed _<number>) are not supported. Stat name: '%ls'. Disable csv.NumberedFNamesStatsAreFatal to make this non-fatal", *StatName.ToString());
	}
#endif
}
...
public:
	...
	void AddCVarABData(const FString& CVarName, int32 Count)
	{
		...
		{
			UE_LOGF(LogCsvProfiler, Log, "Skipping CVar %ls - Not found", *CVarName);
		}
		else if (Count == 0)
		{
			UE_LOGF(LogCsvProfiler, Log, "Skipping CVar %ls - No value specified", *CVarName);
		}
	}
	...
	void InitFromCommandline()
	{
		...
		if (FParse::Value(FCommandLine::Get(), TEXT("csvABTest="), ABTestString, false))
		{
			...
			{
				UE_LOGF(LogCsvProfiler, Log, "Initialized CSV Profiler A/B test"); 
				...
				for (int32 Index = 0; Index < CVarABDataArray.Num(); ++Index)
				{
					...
					UE_LOGF(LogCsvProfiler, Log, "  CVar %ls [Original value: %ls] AB Test with values:", *CVarABData.CVarName, *CVarABData.OriginalValue);
					for (int32 i = 0; i < CVarABData.Count; ++i)
					{
						UE_LOGF(LogCsvProfiler, Log, "    [%d] : %.2f", i, CVarValues[CVarValuesIndex + i]);
					}
				...
				UE_LOGF(LogCsvProfiler, Log, "Stat Offset: %d frames", StatFrameOffset);
				UE_LOGF(LogCsvProfiler, Log, "Switch Duration : %d frames", SwitchDuration);
				UE_LOGF(LogCsvProfiler, Log, "Fast cvar set: %ls", bFastCVarSet ? TEXT("Enabled") : TEXT("Disabled"));

			}
			else
			{
				UE_LOGF(LogCsvProfiler, Log, "CSV Profiler A/B has not initialized");
			}
		}
	}
	...
	void BeginFrameUpdate(int32 FrameNumber, bool bCapturing)
	{
		...
		else if (bPrevCapturing == true)
		{
			...
			for (int32 Index = 0; Index < CVarABDataArray.Num(); ++Index)
			{
				...
				UE_LOGF(LogCsvProfiler, Log, "CSV Profiler A/B test - setting %ls=%ls", *CVarABDataArray[Index].CVarName, *CVarABDataArray[Index].OriginalValue);
			}
...
public:
	FCsvBenchmarkResult Run(int32 IterationCount)
	{
		...
		UE_LOGF(LogCsvProfiler, Log, "Csv profiler benchmark completed (%d iterations). Time taken: %.2fms", IterationCount, BenchTotalTimeMs );
		UE_LOGF(LogCsvProfiler, Log, "Timing stat ns: %.2fns", Result.TimingStatTimeNs);
		UE_LOGF(LogCsvProfiler, Log, "Custom stat ns: %.2fns", Result.CustomStatTimeNs);
...
public:
	...
	void UpdateCategoryFromConfig(int32 CategoryIndex)
	{
		...
		for (FString const& DisabledCategory : CategoriesDisabledInConfig)
		{
			...
			{
				UE_LOGF(LogCsvProfiler, Log, "Config disabled category %ls", *CategoryNames[CategoryIndex]);
		...
		for (FString const& EnabledCategory : CategoriesEnabledInConfig)
		{
			...
			{
				UE_LOGF(LogCsvProfiler, Log, "Config enabled category %ls", *CategoryNames[CategoryIndex]);
...
{
	check(GConfig);
	GConfig->GetBool(TEXT("CsvProfiler"), TEXT("bEventTimestamps"), GCsvEventTimestamps, GEngineIni);
...
static void HandleCSVDeleteDefaultDirectoryCommand()
{
	if ((GCsvProfilerIsCapturing && IsContinuousWriteEnabled(true)) || GCsvProfilerIsWritingFile)
	{
		UE_LOGF(LogCsvProfiler, Error, "CSV Profiler is writing file, ignoring command csv.DeleteDefaultDirectory");
	}
	else
	{
		IFileManager::Get().DeleteDirectory(*FCsvProfiler::GetDefaultDirectory(), false, true);
	}
}

static FAutoConsoleCommand HandleCSVDeleteDefaultDirectoryCmd(
	TEXT("csv.DeleteDefaultDirectory"),
	TEXT("Deletes the default directory for CSV Profiler files. Directory is currently only used by the CsvProfile command, and the command line argument: -ReplicationCSVCaptureFrames=X"),
	FConsoleCommandDelegate::CreateStatic(&HandleCSVDeleteDefaultDirectoryCommand)
);
...
public:
	...
	uint64 AddBeginFrameTimestamp(ECsvTimeline::Type Timeline, const bool bDoThreadCheck = true)
	{
#if DO_CHECK
		if (bDoThreadCheck)
		{
...
{
public:
	FCsvWriterHelper(const TSharedRef<FArchive>& InOutputFile, int32 InBufferSize, bool bInCompressOutput, bool bInContinuousWrites)
		...
		, MaxFileBytes(0)
	{
		...
		int32 MaxFileSizeMB = GetMaxFileSizeMB(false);
		if (MaxFileSizeMB > 0)
		{
			if (!bInContinuousWrites)
			{
				UE_LOGF(LogCsvProfiler, Error, "cvar csv.MaxFileSizeMB not supported unless the appropriate ContinuousWrites cvar is true. Ignoring MaxFileSizeMB.");
				MaxFileBytes = 0;
			}
			else
			{
				MaxFileBytes = MaxFileSizeMB;
				MaxFileBytes *= 1024 * 1024;
				if (MaxFileBytes > InBufferSize)
				{
					// reduce the threshold so that tuned value isn't exceeded quite as much once the file is done flushing
					MaxFileBytes -= InBufferSize;
				}
			}
		}
	}
...
private:
	...
	void SerializeInternal(void* Src, int32 NumBytes)
	{
		if (Buffer.Num() == 0)
		{
			SerializeToOutputFile(Src, NumBytes);
		}
		else
		{
	...
	void Flush()
	{
		if (BytesInBuffer > 0)
		{
			if (GZipBuffer.Num() > 0)
			{
				...
				int32 CompressedSize;
				{
					while (true)
					{
						...
						else
						{
							...
							UE_LOGF(LogCsvProfiler, Error, "CSV data compression failed.");
							BytesInBuffer = 0;
							return;
						}
					}
				...
				{
					SerializeToOutputFile(GZipBuffer.GetData(), CompressedSize);
				}
			}
			else
			{
				// No compression. Write directly to the output file
				SerializeToOutputFile(Buffer.GetData(), BytesInBuffer);
			}
	...
	void SerializeToOutputFile(void* Src, int32 NumBytes)
	{
		OutputFile->Serialize(Src, NumBytes);

		uint64 PreviousBytesWritten = BytesWritten.fetch_add(NumBytes, std::memory_order_relaxed);
		if (MaxFileBytes > 0)
		{
			uint64 CurrentBytesWritten = PreviousBytesWritten;
			CurrentBytesWritten += NumBytes;
			if (PreviousBytesWritten < MaxFileBytes && CurrentBytesWritten >= MaxFileBytes)
			{
				UE_LOGF(LogCsvProfiler, Log, "CSV Profiler will no longer write values due to exceeding csv.MaxFileSizeMB. %llu written of %llu max bytes)", CurrentBytesWritten, MaxFileBytes);
			}
		}
	}
	...
	std::atomic<uint64> BytesWritten;
	uint64 MaxFileBytes;
...
public:
	...
	inline uint64 GetNumBytesWritten() const
	{
		return BytesWritten.load(std::memory_order_relaxed);
	}

	inline bool HasReachedMaxFileBytes() const
	{
		return MaxFileBytes <= 0 ? false : GetNumBytesWritten() >= MaxFileBytes;
	}
};
...
struct FCsvProcessedEvent
{
	...
	{
		// If the event has a timestamp, output it after the text with a ## separator
		if (TimeOffsetSeconds != std::numeric_limits<double>::max())
		{
			if (CategoryIndex == 0)
			{
				return FString::Printf(TEXT("%s##%.6f"), *EventText, TimeOffsetSeconds);
			}
			return FString::Printf(TEXT("%s/%s##%.6f"), *FCsvCategoryData::Get()->GetCategoryNameByIndex(CategoryIndex), *EventText, TimeOffsetSeconds);
		}
		if (CategoryIndex == 0)
		{
			return EventText;
		}
		return FString::Printf(TEXT("%s/%s"), *FCsvCategoryData::Get()->GetCategoryNameByIndex(CategoryIndex), *EventText);
	}
	...
	, bDirty(false)
{
	...
	{
		UE_LOGF(LogCsvProfiler, Warning, "Stat name '%ls' contains invalid characters. Valid characters are: '%ls'", *Name, *GCsvStatNameValidator->GetValidCharacters());
...
FCsvStreamWriter::FCsvStreamWriter(const TSharedRef<FArchive>& InOutputFile, bool bInContinuousWrites, int32 InBufferSize, int64 InNumFramesToBuffer, bool bInCompressOutput, uint32 InRenderThreadId, uint32 InRHIThreadId, bool bAggregateTaskWorkerStats)
	: Stream(InOutputFile, InBufferSize, bInCompressOutput, bInContinuousWrites)
...
void FCsvStreamWriter::PushValue(FCsvStatSeries* Series, int64 FrameNumber, const FCsvStatSeriesValue& Value)
{
	...
	if (Stream.HasReachedMaxFileBytes())
	{
		return;
	}
...
void FCsvStreamWriter::FinalizeNextRow()
{
	...
	if (Row)
	{
		if (Row->Events.Num() > 0)
		{
			...
			{
				EventStrings.Add(Event.GetFullText());
			}
...
void FCsvStreamWriter::Finalize(const TMap<FString, FString>& Metadata)
{
	...
	Stream.WriteMetadataEntry((TEXT("EventTimestamps")), GCsvEventTimestamps ? TEXT("1") : TEXT("0") );
...
void FCsvStreamWriter::Process(FCsvProcessThreadDataStats& OutStats)
{
	...
	// Read the peak mem use, if configured
	OutStats.MemoryUseBytes = 0;
	if (CVarCsvTrackMemoryUse.GetValueOnAnyThread(true))
	{
		CSV_SCOPED_TIMING_STAT(CsvProfiler, GetMemoryUse);
		OutStats.MemoryUseBytes = GetAllocatedSize();
	}	
...
inline uint64 FCsvStreamWriter::GetNumBytesWritten() const
{
	return Stream.GetNumBytesWritten();
}

inline bool FCsvStreamWriter::HasReachedMaxFileBytes() const
{
	return Stream.HasReachedMaxFileBytes();
}


struct FProcessingThreadStats
{
	double LastUpdateStartTime = 0.0;
	double LastUpdateEndTime = 0.0;
	FCsvProcessThreadDataStats ProcessingStats;
};
...
class FCsvProfilerProcessingThread : public FRunnable
{
	...
	FProcessingThreadStats UpdateStats;
...
public:
		...
		: CsvProfiler(InCsvProfiler)
	{
		...
		{
			GCsvThreadPriorityIndex = 5;
			AffinityMask = FPlatformAffinity::GetTaskGraphThreadMask();
		}
#endif
		UE_LOGF(LogCsvProfiler, Display, "Starting CSV processing thread with priority %ls", GetThreadPriorityNameFromIndex(GCsvThreadPriorityIndex));
		CurrentThreadPriority = GetThreadPriorityFromIndex(GCsvThreadPriorityIndex);
		Thread = FForkProcessHelper::CreateForkableThread(this, TEXT("CSVProfiler"), 0, CurrentThreadPriority, AffinityMask);
	}
	...
	{
		// Sleep until the csv profiler is fully initialized. On some platforms, TLS is not ready until this point
		while (GDelayProcessingThreadStart)
		{
			FPlatformProcess::SleepNoStats(0.001f);
		}
		// Set initial LastUpdateTime to the time the thread started so we can compute processing latency on capture start
		UpdateStats.LastUpdateStartTime = UpdateStats.LastUpdateEndTime = FPlatformTime::Seconds();
		...
		while (StopCounter.GetValue() == 0)
		{
			...
			float ElapsedMs = CsvProfiler.ProcessStatData(&Stats);
			...
			HandleGtStalling(Stats, ElapsedMs);

			// Update stats
			{
				UE::TScopeLock Lock(StatsCs);
				UpdateStats.LastUpdateStartTime = UpdateStartTime;
				UpdateStats.LastUpdateEndTime = UpdateEndTime;
				UpdateStats.ProcessingStats = Stats;
			}
	...
	bool StallGameThreadIfNeeded()
	{
		check(IsInGameThread());

		// Handle stalling GT due to max update latency (too long since last update)
		const double MaxUpdateLatency = (double)CVarCsvProcessingThreadGtStallMaxUpdateLatencyMs.GetValueOnGameThread()/1000.0;
		if (MaxUpdateLatency > 0.0)
		{
			double GtTime = FPlatformTime::Seconds();
			double LastUpdateTime = GetStats().LastUpdateStartTime;
			double ElapsedSinceLastUpdate = GtTime - GetStats().LastUpdateStartTime;
			// If we're over the latency threshold, wait so the CSV profiler thread can catch up
			if (ElapsedSinceLastUpdate > MaxUpdateLatency)
			{
				bThreadPriorityBoost = true;
				CSV_EVENT(CsvProfiler, TEXT("GT Stall"));
				CSV_SCOPED_TIMING_STAT_EXCLUSIVE(CsvProfiler_Stall);
				CSV_SCOPED_TIMING_STAT(CsvProfiler, Stall);
				UE_LOGF(LogCsvProfiler, Warning, "Game thread stalled waiting for CSV processing thread to catch up!");
				// Stall until the next update
				do
				{
					FPlatformProcess::Sleep(0.f);
				} while (GetStats().LastUpdateStartTime < GtTime);
				return true;
			}
		}

		// Handle GT stalls triggered from the processing thread
		if (StallGameThreadCs.TryLock())
		{
			bThreadPriorityBoost = false;
			StallGameThreadCs.Unlock();
			return false;
		}
		CSV_SCOPED_TIMING_STAT_EXCLUSIVE(CsvProfiler_Stall);
	...
	FProcessingThreadStats GetStats()
	{
		UE::TScopeLock Lock(StatsCs);
		return UpdateStats;
	}
...
private:
	static EThreadPriority GetThreadPriorityFromIndex(int32 Index)
	{
		const int32 ClampedIndex = FMath::Clamp(Index, 0, (int32)UE_ARRAY_COUNT(GCsvThreadPriorities) - 1);
		return GCsvThreadPriorities[ClampedIndex];
	}

	static const TCHAR* GetThreadPriorityNameFromIndex(int32 Index)
	{
		const int32 ClampedIndex = FMath::Clamp(Index, 0, (int32)UE_ARRAY_COUNT(GCsvThreadPriorityNames) - 1);
		return GCsvThreadPriorityNames[ClampedIndex];
	}
	void UpdateThreadPriorityIfNeeded()
	{
		int32 ThreadPriIndex = GCsvThreadPriorityIndex;
		if (bThreadPriorityBoost)
		{
			ThreadPriIndex = 5; // TPri_Highest
		}
		EThreadPriority DesiredThreadPriority = GetThreadPriorityFromIndex(ThreadPriIndex);
		if (DesiredThreadPriority != CurrentThreadPriority)
		{
			UE_LOGF(LogCsvProfiler, Display, "Setting CSV processing thread priority to %ls%ls", GetThreadPriorityNameFromIndex(ThreadPriIndex), bThreadPriorityBoost ? TEXT(" - Boost active") : TEXT(""));
			FPlatformProcess::SetThreadPriority(DesiredThreadPriority);
			CurrentThreadPriority = DesiredThreadPriority;
		}
	}

	void HandleGtStalling(const FCsvProcessThreadDataStats& ProcessingStats, float ElapsedMs)
	{
		check(IsInCsvProcessingThread());
		const float TimeBetweenUpdatesMs = CVarCsvProcessingThreadTimeBetweenUpdates.GetValueOnAnyThread();

		// Detect well-of-despair (stats coming in faster than they can be processed)
		uint32 NumStatEntriesProcessed = ProcessingStats.CustomStatCount + ProcessingStats.TimestampCount;
		...
		else 
		{
			...
			if (StallThresholdMs > 0.0f && PrevElapsedMs > StallThresholdMs && ElapsedMs > PrevElapsedMs && NumStatEntriesProcessed > PrevNumStatEntriesProcessed)
			{
				...
				if (SlowUpdateEscalationCount >= CVarCsvProcessingThreadGtStallUpdateEscalationThreshold.GetValueOnAnyThread())
				{
					...
					bHasGtStallLock = true;
					StallCounter = 2;
					double StatEntriesProcessedPerSecond = (double)TotalStatEntriesProcessed / TotalProcessingTime;
					UE_LOGF(LogCsvProfiler, Warning, "Stats coming in faster than we can process them! GT stalled until we can catch up!");
					UE_LOGF(LogCsvProfiler, Warning, "Avg processing rate: %.0f stat entries per second (timestamps+custom stats)", StatEntriesProcessedPerSecond);
					UE_LOGF(LogCsvProfiler, Warning, "Check CsvProfiler/* stats to see current rates. Run with -csvStatCounts to report per-stat counts");
	...
	bool bHasGtStallLock = false;
	volatile bool bThreadPriorityBoost = false;
};
...

	{
		...
		for (int i = 0; i < Events.Num(); i++)
		{
			...
			if (FrameNumber >= 0)
			{
				...
				ProcessedEvent.TimeOffsetSeconds = GCsvEventTimestamps ? FPlatformTime::ToSeconds64(Event.Timestamp-GCaptureStartTimestamp) : std::numeric_limits<double>::max();
	...
	, FileWriteBlockingEvent(FPlatformProcess::GetSynchEventFromPool())
{
	...
	SetMetadataInternal(TEXT("CompactTSet"), UE_USE_COMPACT_SET_AS_DEFAULT ? TEXT("1") : TEXT("0"));
...
void FCsvProfiler::BeginCaptureInternal(const FCsvCaptureCommand& CurrentCommand)
{
	if (GCsvProfilerIsCapturing)
	{
		UE_LOGF(LogCsvProfiler, Warning, "Capture start requested, but a capture was already running");
		return;
	}

	UE_LOGF(LogCsvProfiler, Display, "Capture Starting");
	if (GConfig)
	{
	...
	FString DestinationFolder = CurrentCommand.DestinationFolder.IsEmpty() ? GetDefaultDirectory() : CurrentCommand.DestinationFolder + TEXT("/");
	FString Filename = CurrentCommand.Filename.IsEmpty() ? FString::Printf(TEXT("Profile(%s)"), *FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S"))) : CurrentCommand.Filename;
	Filename = FPaths::SetExtension(Filename, CsvExtension);
	...
	if (!OutputFile)
	{
		UE_LOGF(LogCsvProfiler, Error, "Failed to create CSV file \"%ls\". Capture will not start.", *OutputFilename);
		return;
	}
	...
	if (EnumHasAnyFlags(CurrentCommand.Flags, ECsvProfilerFlags::BootCapture))
	{
		// If we're capturing from boot, set the start time to GStartTime 
		// This ensures the first time and boot timings are relative to GStartTime
		check(GFrameNumber==1);
		UE_LOGF(LogCsvProfiler, Log, "Doing a boot time capture. Start time offset %.2f", FPlatformTime::Seconds()-GStartTime);
		CaptureStartTime = GStartTime;
		// Note: we have to remove the platform time offset when converting between absolute seconds and clock cycles
        // Warning: this won't work on unix (see comment in the else case below). First frame timing will be inaccurate there when capturing from boot
		GCaptureStartTimestamp = FPlatformTime::SecondsToCycles64(CaptureStartTime - FPlatformTime::GetSecondsTimeOffset());
	...
	{
		// Workaround for unix time inconsistencies. We can't convert between Cycles64 and Seconds because they can come from different clocks (see UnixPlatformTime.h)
		// Implementing FPlatformTime::GetSecondsTimeOffset() might help but we'd still get precision differences
		// Instead we'll record StartTime and CaptureStartTimestamp independently
		CaptureStartTime = FPlatformTime::Seconds();
		GCaptureStartTimestamp = FPlatformTime::Cycles64();
	}

	LastEndFrameTimestamp = GCaptureStartTimestamp;
	...
	if (GCsvUseProcessingThread && ProcessingThread == nullptr)
	{
		...
		{
			UE_LOGF(LogCsvProfiler, Error, "CSV Processing Thread could not be created due to being in a single-thread environment ");
			delete ProcessingThread;
			ProcessingThread = nullptr;
	...
	UE_LOGF(LogCsvProfiler, Display, "Capture started. CSV ID: %ls", *CsvIdString);
	...
	SetMetadataInternal(TEXT("HasGtStalls"), TEXT("0"));

	static IConsoleVariable* VSyncCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.VSync"));
	if (VSyncCVar)
	{
		SetMetadataInternal(TEXT("VSyncEnabled"), *FString::FromInt(VSyncCVar->GetInt()));
	}
	...
	if (ThermalState != EDeviceThermalState::Unsupported)
	{
		SetNonPersistentMetadata(TEXT("ThermalState_Start"), *FString::FromInt((int32)ThermalState));
	}
...
void FCsvProfiler::EndFrame()
{
	...
	if (GCsvProfilerIsCapturing)
	{
		if (ProcessingThread)
		{
			FProcessingThreadStats ProcessingThreadStats = ProcessingThread->GetStats();
			double ElapsedSinceLastUpdate = FPlatformTime::Seconds()-ProcessingThreadStats.LastUpdateStartTime;

			if (ProcessingThreadStats.ProcessingStats.MemoryUseBytes>0)
			{
				CSV_CUSTOM_STAT(CsvProfiler, MemUsedLastUpdateMB, (float)ProcessingThreadStats.ProcessingStats.MemoryUseBytes / 1024.0f / 1024.0f, ECsvCustomStatOp::Set);
			}
			CSV_CUSTOM_STAT(CsvProfiler, UpdateLatencyMs, (float)ElapsedSinceLastUpdate*1000.0f, ECsvCustomStatOp::Set);

			// Stall the GT if needed
			if ( ProcessingThread->StallGameThreadIfNeeded() )
			{
				CSV_METADATA(TEXT("HasGtStalls"), TEXT("1"));
			}
		}		
		...
		// If we're doing a boot capture, just use the legacy frametime method, otherwise the initial EngineInit frames won't be set
		if (EnumHasAnyFlags(CurrentFlags, ECsvProfilerFlags::BootCapture) || CVarCsvUseLegacyFrameTime->GetBool())
		{
			// Record the frametime (measured since the last EndFrame)
			uint64 ElapsedCycles = CurrentTimeStamp - LastEndFrameTimestamp;
			double ElapsedMs = FPlatformTime::ToMilliseconds64(ElapsedCycles);
		...
		// Note: we use the end frame timestamp even when CVarCsvUseLegacyFrameTime is false as 
		// this timestamp is the csv frame boundary and not necessarily the logical frame boundary.
...
bool FCsvProfiler::TryEndCaptureInternal(const FCsvCaptureCommand& CurrentCommand)
{
	if (GCsvProfilerIsCapturing || GCsvProfilerIsWritingFile)
	{
		...
		UE_LOGF(LogCsvProfiler, Display, "Capture Stop requested");
	...
	{
		UE_LOGF(LogCsvProfiler, Warning, "Capture Stop requested, but no capture was running!");
	}
	else
	{
...
void FCsvProfiler::UpdateFrameTime()
{
	// If we're doing a boot capture, just use the legacy frametime method, otherwise the initial EngineInit frames won't be set
	if (!EnumHasAnyFlags(CurrentFlags, ECsvProfilerFlags::BootCapture) && !CVarCsvUseLegacyFrameTime->GetBool())
	{
		// This is called right after UpdateTimeAndHandleMaxTickRate() in the engine loop which is the point at which
		// we've finished waiting when a frame rate limit is enabled. Computing the frame time here means it does not 
		// align with the csv frame boundary, but it does align with the logical frame boundary.
		// This avoids noise in the frame time stat introduced by the wait at the start of the frame as it cannot possibly
		// predict how long this frame will be and is instead based on the time of previous frames. Therefore this wait 
		// period and any time before it logically belong to the prior frame. Using this period for the frame time results
		// in almost 0 noise in the frame time calculation when a frame rate limit is enabled, and no difference to computing
		// it at the end of the frame when running uncapped or with vsync enabled (besides the phase shift).

		static double LastTimestamp = 0.0;
		const double CurrentTimestamp = FPlatformTime::Seconds();
		if (LastTimestamp > 0.0)
		{
			double FrameTimeMs = (CurrentTimestamp - LastTimestamp) * 1000.0;
			CSV_CUSTOM_STAT_MINIMAL_GLOBAL(FrameTime, FrameTimeMs, ECsvCustomStatOp::Set);
		}
	...
	ECsvProfilerFlags InFlags)
{
	...
	if (CommandQueue.Peek(CurrentCommand) && CurrentCommand.CommandType == ECsvCommandType::Start)
	{
		UE_LOGF(LogCsvProfiler, Warning, "BeginCapture() called, but there is already a pending start command. Ignoring!");
		return;
	}
	...
	{
		UE_LOGF(LogCsvProfiler, Error, "Failed to allocate TLS! Not starting the CSV capture");
	}
	else
	{
...
TSharedFuture<FString> FCsvProfiler::EndCapture(FGraphEventRef EventToSignal)
{
	...
	if (!IsCapturing())
	{
		UE_LOGF(LogCsvProfiler, Warning, "EndCapture() called, but no capture was in progress. Ignoring!");
		return {};
	}
	...
	if (CommandQueue.Peek(CurrentCommand) && CurrentCommand.CommandType == ECsvCommandType::Stop)
	{
		UE_LOGF(LogCsvProfiler, Warning, "EndCapture() called, but there is already a pending stop command. Ignoring!");
		return CurrentCommand.Future;
	}

	if (CommandQueue.Peek(CurrentCommand) && CurrentCommand.CommandType == ECsvCommandType::Start)
	{
		UE_LOGF(LogCsvProfiler, Warning, "EndCapture() called, but there is already a pending start command!");
	}
	...
	if (ThermalState != EDeviceThermalState::Unsupported)
	{
		SetNonPersistentMetadata(TEXT("ThermalState_End"), *FString::FromInt((int32)ThermalState));
	}
	...
	bool bHasReachedMaxFileBytes = CsvWriter == nullptr ? false : CsvWriter->HasReachedMaxFileBytes();
	SetNonPersistentMetadata(TEXT("CsvMaxFileBytesReached"), bHasReachedMaxFileBytes ? TEXT("1") : TEXT("0"));
...
void FCsvProfiler::FinalizeCsvFile()
{
	...
	UE_LOGF(LogCsvProfiler, Display, "Capture Ending");
	...
	UE_LOGF(LogCsvProfiler, Display, "Capture Ended. Writing CSV to file : %ls", *OutputFilename);
	UE_LOGF(LogCsvProfiler, Display, "  Frames : %d", CaptureEndFrameCount);
	UE_LOGF(LogCsvProfiler, Display, "  Peak memory usage  : %.2fMB", float(MemoryBytesAtEndOfCapture) / (1024.0f * 1024.0f));

	OnCSVProfileFinished().Broadcast(OutputFilename);

	float FinalizeDuration = float(FPlatformTime::Seconds() - FinalizeStartTime);
	UE_LOGF(LogCsvProfiler, Display, "  CSV finalize time : %.3f seconds", FinalizeDuration);
...
FString FCsvProfiler::GetDefaultDirectory()
{
	return FPaths::ProfilingDir() + TEXT("CSV/");
}

void FCsvProfiler::RecordEventfInternal(int32 CategoryIndex, bool bLog, const TCHAR* Fmt, ...)
{
	...
	if (bIsCsvRecording || GStartOnEvent)
	{
		...
		if (bIsCsvRecording)
		{
			RecordEvent(CategoryIndex, Str, bLog);
...
void FCsvProfiler::RecordEvent(int32 CategoryIndex, const FString& EventText, bool bLog, bool bAllowDelegate)
{
	if (GCsvProfilerIsCapturing && GCsvCategoriesEnabled[CategoryIndex])
	{
		LLM_SCOPE(ELLMTag::CsvProfiler);
		...
		if (bLog)
		{
			UE_LOGF(LogCsvProfiler, Display, "CSVEvent \"%ls\" [Frame %d]", *FullEventText, FCsvProfiler::Get()->GetCaptureFrameNumber());
		}
#if CSV_PROFILER_ALLOW_DEBUG_FEATURES
		if (bAllowDelegate)
		{
...
void FCsvProfiler::RecordEventAtFrameStart(int32 CategoryIndex, const FString& EventText, bool bLog)
{
	RecordEventAtTimestamp(CategoryIndex, EventText, FCsvProfiler::Get()->LastEndFrameTimestamp, bLog);
}

void FCsvProfiler::BeginRegion(int32 CategoryIndex, const TCHAR* RegionName, const TCHAR* SubCategoryName)
{
	if (GCsvProfilerIsCapturing && GCsvCategoriesEnabled[CategoryIndex])
	{
		RecordRegionEventInternal(CategoryIndex == CSV_CATEGORY_INDEX_GLOBAL ? nullptr : *FCsvCategoryData::Get()->GetCategoryNameByIndex(CategoryIndex), RegionName, SubCategoryName, true);
	}
}

void FCsvProfiler::EndRegion(int32 CategoryIndex, const TCHAR* RegionName, const TCHAR* SubCategoryName)
{
	if (GCsvProfilerIsCapturing && GCsvCategoriesEnabled[CategoryIndex])
	{
		RecordRegionEventInternal(CategoryIndex == CSV_CATEGORY_INDEX_GLOBAL ? nullptr : *FCsvCategoryData::Get()->GetCategoryNameByIndex(CategoryIndex), RegionName, SubCategoryName, false);
	}
}

void FCsvProfiler::RecordRegionEventInternal(const TCHAR* CategoryName, const TCHAR* RegionName, const TCHAR* SubCategoryName, bool bBegin)
{
	const TCHAR* Suffix = bBegin ? TEXT("Begin") : TEXT("End");
	FString EventText;
	if (CategoryName)
	{
		EventText = SubCategoryName ?
			FString::Printf(TEXT("Region: %s/%s/%s - %s"), CategoryName, SubCategoryName, RegionName, Suffix) :
			FString::Printf(TEXT("Region: %s/%s - %s"), CategoryName, RegionName, Suffix);
	}
	else
	{
		EventText = SubCategoryName ? 
			FString::Printf(TEXT("Region: %s/%s - %s"), SubCategoryName, RegionName, Suffix) : 
			FString::Printf(TEXT("Region: %s - %s"), RegionName, Suffix);
	}
	RecordEvent(CSV_CATEGORY_INDEX_GLOBAL, EventText, false, false);
}
...
void FCsvProfiler::SetMetadataInternal(const TCHAR* Key, const TCHAR* Value, bool bSanitize, EMetadataPersistenceType PersistenceType)
{
	...
	if (Value == nullptr)
	{
		...
		{
			UE_LOGF(LogCsvProfiler, Display, "Metadata unset : %ls", *KeyLower);
	...
	else
	{
		...
		if (bSanitize)
		{
			...
			{
				UE_LOGF(LogCsvProfiler, Warning, "Metadata value sanitized due to invalid characters: %ls=\"%ls\"", *KeyLower, Value);
			}
		}
		...
		{
			UE_LOGF(LogCsvProfiler, Display, "Metadata set : %ls=\"%ls\"", *KeyLower, *ValueStr);
		}
		CurrentMetadataMap.FindOrAdd(KeyLower) = ValueStr;
	}
}

void FCsvProfiler::RecordEventAtTimestamp(int32 CategoryIndex, const FString& EventText, uint64 Cycles64, bool bLog)
{
	if (GCsvProfilerIsCapturing && GCsvCategoriesEnabled[CategoryIndex])
	{
		LLM_SCOPE(ELLMTag::CsvProfiler);
		if (bLog)
		{
			UE_LOGF(LogCsvProfiler, Display, "CSVEvent [Frame %d] : \"%ls\"", FCsvProfiler::Get()->GetCaptureFrameNumber(), *EventText);
		}
		FCsvProfilerThreadData::Get().AddEventWithTimestamp(EventText, CategoryIndex,Cycles64);

		if (IsContinuousWriteEnabled(false))
		{
			UE_LOGF(LogCsvProfiler, Warning, 
				"RecordEventAtTimestamp is not compatible with continuous CSV writing. " "Some events may be missing in the output file. Set 'csv.ContinuousWrites' " "to 0 to ensure events recorded with specific timestamps are captured correctly.");
		}
	}
}
...
void FCsvProfiler::PreInit(const TCHAR * Commandline)
{
#if CSV_PROFILER_ALLOW_DEBUG_FEATURES
	// If we're doing boot profiling then we need to init early and make sure the commandline is set
	int32 NumCsvFrames = 0;
	if (FParse::Value(Commandline, TEXT("csvCaptureFrames="), NumCsvFrames))
	{
		if (!FCommandLine::IsInitialized())
		{
			FCommandLine::Set(Commandline);
		}
		// Delay processing thread start
		GDelayProcessingThreadStart = true;
		FCsvProfiler::Get()->InitInternal();
	}
#endif
}
...
{
	InitInternal();
	// It's now safe to start the processing thread
	GDelayProcessingThreadStart = false;
}

void FCsvProfiler::InitInternal()
{
	if (GCsvProfilerInitialized)
	{
		return;
	}
	GCsvProfilerInitialized = true;
	...
	{
		check(IsInGameThread());
		BeginCapture(NumCsvFrames, FString(), FString(), ECsvProfilerFlags::BootCapture);
		// Call BeginFrame() to start capturing a dummy first "frame"
		BeginFrame();
		// Add a region end marker so we can see where preinit and csv capture started
		EndRegion(CSV_CATEGORY_INDEX_GLOBAL, TEXT("EngineInit/Launch"));
	}
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("csvCompression="), CompressionMode))
	{
		...
		default:
			UE_LOGF(LogCsvProfiler, Warning, "Invalid command line compression mode \"%d\".", CompressionMode);
			break;
		}
	}
		...
		if (CsvExecCommandsStr.ParseIntoArray(CsvExecCommandsList, TEXT(","), true) > 0)
		{
			for (FString FrameAndCommand : CsvExecCommandsList)
			{
				...
				if (FrameAndCommand.FindChar(TEXT(':'), ColonIndex))
				{
					...
					if ( bModuloCommand )
					{
						UE_LOGF(LogCsvProfiler, Display, "Added CsvExecCommand (repeating) - Modulo: %d : %ls", Frame, *CommandStr);
					}
					else
					{
						UE_LOGF(LogCsvProfiler, Display, "Added CsvExecCommand - frame %d : %ls", Frame, *CommandStr);
					}
		...
		if (CsvExecCommandsStr.ParseIntoArray(CsvExecCommandsList, TEXT(","), true) > 0)
		{
			for (FString FrameAndCommand : CsvExecCommandsList)
			{
				...
				if (ColonIndex != INDEX_NONE)
				{
					...
					UE_LOGF(LogCsvProfiler, Display, "Added CsvEventExecCommand - event \"%ls\" : %ls", *EventStr, *CommandStr);
				}
			}
		}
	}
...
/*Get the current number of bytes written to file*/
uint64 FCsvProfiler::GetNumBytesWritten() const
{
	return CsvWriter == nullptr ? 0 : CsvWriter->GetNumBytesWritten();
}
...
bool FCsvProfiler::EnableCategoryByString(const FString& CategoryName) const
{
	...
	if (Category >= 0)
	{
		UE_LOGF(LogCsvProfiler, Log, "Enabled category %ls", *CategoryName);
		GCsvCategoriesEnabled[Category] = true;
		return true;
	}
	UE_LOGF(LogCsvProfiler, Warning, "Error: Can't find category %ls", *CategoryName);
	return false;
}
...
float FCsvProfiler::ProcessStatData(FCsvProcessThreadDataStats* OutStats)
{
	check(IsInCsvProcessingThread());
