...
UE_TRACE_CHANNEL_DEFINE(CpuChannel, "CPU profiler scoping events and timing data. Captures hierarchical timing information for CPU-bound operations on all threads.")
...
UE_TRACE_EVENT_BEGIN(CpuProfiler, MetadataSpec, NoSync | Important) // added in UE 5.6
	UE_TRACE_EVENT_FIELD(uint32, Id)
	UE_TRACE_EVENT_FIELD(UE::Trace::AnsiString, Name)
...
UE_TRACE_EVENT_BEGIN(CpuProfiler, Metadata, NoSync) // added in UE 5.6
	UE_TRACE_EVENT_FIELD(uint32, Id)
...
struct FCpuProfilerTraceInternal
{
	...
	static thread_local EThreadState ThreadState;
		...
		: public FTlsAutoCleanup
	{
		FThreadBuffer()
		{
			ThreadState = EThreadState::Created;
		}
		...
		virtual ~FThreadBuffer()
		{
			...
			ThreadState = EThreadState::Destroyed;
		}
...
thread_local FCpuProfilerTraceInternal::EThreadState FCpuProfilerTraceInternal::ThreadState = FCpuProfilerTraceInternal::EThreadState::Uninitialized;

FCpuProfilerTraceInternal::FThreadBuffer* FCpuProfilerTraceInternal::CreateThreadBuffer()
{
	if (FCpuProfilerTraceInternal::ThreadState == FCpuProfilerTraceInternal::EThreadState::Destroyed)
	{
		return nullptr;
	}
...
void FCpuProfilerTrace::OutputSuspendEvent()
{
	...
	*TimerScopeDepth = FCpuProfilerTraceInternal::ThreadDepth - SavedThreadDepth;
	...
	if ((FCpuProfilerTraceInternal::ThreadDepth == 0) ||
		(ThreadBuffer->BufferSize >= FCpuProfilerTraceInternal::FullBufferThreshold))
	{
		FCpuProfilerTraceInternal::FlushThreadBuffer(ThreadBuffer);
	}
}
