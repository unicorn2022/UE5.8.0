...
#if !defined(UE_CALLSTACK_TRACE_UNCOMPRESSED)
	// If enabled, callstack addresses aggregate without any compression
	#define UE_CALLSTACK_TRACE_UNCOMPRESSED 0
#endif

////////////////////////////////////////////////////////////////////////////////
UE_TRACE_CHANNEL_EXTERN(CallstackChannel, CORE_API)
...
UE_TRACE_EVENT_BEGIN_EXTERN(Memory, CallstackSpecXORAndRLE, NoSync)
	UE_TRACE_EVENT_FIELD(uint32, CallstackId)
	UE_TRACE_EVENT_FIELD(uint8[], CompressedFrames)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN_EXTERN(Memory, CallstackSpecDelta7bit, NoSync)
	UE_TRACE_EVENT_FIELD(uint32, CallstackId)
	UE_TRACE_EVENT_FIELD(uint8[], CompressedFrames)
UE_TRACE_EVENT_END()

UE_TRACE_EVENT_BEGIN_EXTERN(Memory, CallstackSpecDeltaVarInt, NoSync)
	UE_TRACE_EVENT_FIELD(uint32, CallstackId)
	UE_TRACE_EVENT_FIELD(uint8[], CompressedFrames)
UE_TRACE_EVENT_END()
...
#endif //UE_CALLSTACK_TRACE_RESERVE_GROWABLE
				KnownSet.Emplace(Hash, Id);
#if UE_CALLSTACK_TRACE_UNCOMPRESSED
...
#else
				uint8 Compressed[UE_CALLSTACK_TRACE_MAX_FRAMES * 9];
				const uint32 CompressedSize = FCallstackDeltaVarInt::Compress(
					MakeArrayView(Entry.Frames, Entry.FrameCount),
					Compressed
				);
				
				UE_TRACE_LOG(Memory, CallstackSpecDeltaVarInt, CallstackChannel)
					<< CallstackSpecDeltaVarInt.CallstackId(Id)
					<< CallstackSpecDeltaVarInt.CompressedFrames(Compressed, CompressedSize);
#endif //UE_CALLSTACK_TRACE_UNCOMPRESSED
			}
