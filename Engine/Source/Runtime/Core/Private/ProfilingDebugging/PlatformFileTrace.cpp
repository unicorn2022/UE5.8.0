...
UE_TRACE_CHANNEL(FileChannel, "Platform file I/O operations: open, close, read, and write events with file handle, offset, size, and cycle timestamps.")
...
void FPlatformFileTrace::EndClose(uint64 FileHandle)
{
	...
	if (bUnderflow)
	{
		UE_LOGF(LogCore, Error, "FPlatformFileTrace Close without an Open: FileHandle %llu.", FileHandle);
	}
#else
	int32 NewValue = --OpenFileHandleCount;
	if (NewValue == -1)
	{
		UE_LOGF(LogCore, Error, "FPlatformFileTrace Close without an Open");
