...
bool FPlatformEvents::CanEnable(FPlatformEventsTrace::EEventType Event, const TCHAR** OutReason)
{
	...
	if (!::OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &CurrentProcToken.Handle))
	{
		...
		UE_LOGF(LogCore, Log, "Unable to enable platform events: %ls", ErrProcAccess);
		return false;
	}
	...
	UE_LOGF(LogCore, Log, "Unable to enable platform events: %ls", ErrProcRights);
	return false;
#else
	return true;
#endif
...
void FPlatformEvents::Enable(FPlatformEventsTrace::EEventType Event)
{
	...
	else
	{
		...
		{
			UE_LOGF(LogPlatformEvents, Error, "Unable to update ETW trace: 0x%08x", Status);
		}
	}
...
bool FPlatformEvents::StartETW()
{
		...
		if (::OpenProcessToken(::GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &Token))
		{
			...
			if (::LookupPrivilegeValue(NULL, SE_SYSTEM_PROFILE_NAME, &Luid))
			{
				...
				if (!::AdjustTokenPrivileges(Token, 0, &TokenPrivileges, sizeof(TokenPrivileges), NULL, NULL))
				{
					...
					UE_LOGF(LogPlatformEvents, Warning, "Cannot enable profile privilege for process: 0x%08x", Error);
				}
			}
			::CloseHandle(Token);
		}
	...
	// set up sampling interval, must be done before StartTrace
	{
		...
		if (Status != ERROR_SUCCESS)
		{
			UE_LOGF(LogPlatformEvents, Warning, "Unable to set ETW sampling interval: 0x%08x", Status);
		}
	}
	...
	// start the trace
	{
		...
		{
			UE_LOGF(LogPlatformEvents, Error, "Administrator rights required for ETW");
			return false;
		}
		else if (Status != ERROR_SUCCESS)
		{
			UE_LOGF(LogPlatformEvents, Warning, "Unable to start ETW trace: 0x%08x", Status);
			return false;
		}
	}
	...
	// select for which events ETW should collect stack (StackWalkEvent), this is harmless if stack sampling is not enabled
	{
		...
		if (Status != ERROR_SUCCESS)
		{
			UE_LOGF(LogPlatformEvents, Error, "Cannot enable stack tracing for ETW: 0x%08x", Status);
			return false;
		}
	}
	...
	// open trace for receiving callbacks
	{
		...
		if (TraceHandle == INVALID_PROCESSTRACE_HANDLE)
		{
			UE_LOGF(LogPlatformEvents, Error, "Cannot open ETW trace");
			return false;
		}
	}
