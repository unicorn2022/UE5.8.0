...
private:
	...
	void ScanMemory()
	{
		...
		while (true)
		{
			...
			{
				Dl_info dl_info{};
				dladdr(reinterpret_cast<void*>(CurrAddr), &dl_info);
				
				if(dl_info.dli_fname != nullptr && 
				   FAnsiStringView(dl_info.dli_fname).EndsWith(TCHAR_TO_ANSI(FPlatformProcess::ExecutableName())))
				{
					ProgramSize += Info.pages_resident * vm_page_size;
				}
			}
	...
	#endif
		FGenericPlatformMemory::SetProgramSize(ProgramSize);
