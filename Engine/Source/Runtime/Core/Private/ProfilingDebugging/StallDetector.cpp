	...
	while (!StopThread)
	{
		...
		if (Seconds != InvalidSeconds)
		{
			...
			for (FStallDetector* Detector : StallScopes)
			{
				...
				if (OverageSeconds > 0.0 && !FPlatformMisc::IsDebuggerPresent())
				{
					TArray<uint64> Backtrace;
			...
			UE::Tasks::Launch(UE_SOURCE_LOCATION, [DetectedStalls = MoveTemp(DetectedStalls)]() {
				...
				for (const FDetectedStall& Stall : DetectedStalls)
				{
					...
					if (bSendReport)
					{
						...
						UE_LOGF(LogStall, Log, "Stall detector '%ls' exceeded budget of %fs, reporting...", Stall.Name, Stall.BudgetSeconds);
						...
						UE_LOGF(LogStall, Log, "Stall detector '%ls' report submitted, and took %fs", Stall.Name, ReportSeconds);
					}
					else if (ForceStallDetectedLog || Stall.ReportingMode != EStallDetectorReportingMode::Disabled)
					{
						UE_LOGF(LogStall, Log, "Stall detector '%ls' exceeded budget of %fs", Stall.Name, Stall.BudgetSeconds);
					}
				}
...
UE::FStallDetector::~FStallDetector()
{
	...
	if (Seconds != InvalidSeconds && bStarted && !bPersistent)
	{
		...
		if (OverageSeconds > 0.0)
		{
			...
			{
				UE_LOGF(LogStall, Log, "Stall detector '%ls' complete in %fs (%fs overbudget)", Stats.Name, Stats.BudgetSeconds + OverageSeconds, OverageSeconds);
			}
...
void UE::FStallDetector::Startup()
{
	...
	if (++InitCount == 1)
	{
		...
		UE_LOGF(LogStall, Log, "Startup complete.");
	}
}
...
void UE::FStallDetector::Shutdown()
{
	if (--InitCount == 0)
	{
		...
		UE_LOGF(LogStall, Log, "Shutdown complete.");
	}
	check(InitCount >= 0);
}
