...
void FABTest::ABTestCmdFunc(const TArray<FString>& Args)
{
	...
	if (Args.Num() == 1 && Args[0].Compare(FString("framelog"), ESearchCase::IgnoreCase) == 0)
	{
		Get().Stop();
		Get().StartFrameLog();
		return;
	}
	if (Args.Num() == 1 && Args[0].Compare(FString("stop"), ESearchCase::IgnoreCase) == 0)
	{
		Get().Stop();		
		return;
	}
	if (Args.Num() == 1 && Args[0].Compare(FString("scope"), ESearchCase::IgnoreCase) == 0)
	{
