...
public:
	bool Initialize()
	{
		...
		FString WindowsPixDll("WinPixEventRuntime.dll");
#endif
		UE_LOGF(LogProfilingDebugging, Log, "Loading %ls for PIX profiling (from %ls).", *WindowsPixDll, *WindowsPixDllRelativePath);
		...
		if (WinPixEventRuntimeHandle == nullptr)
		{
			...
			UE_LOGF(LogProfilingDebugging, Error, "Failed to get %ls handle: %ls (%d)", *WindowsPixDll, ErrorMsg, ErrorNum);
		}
#endif
