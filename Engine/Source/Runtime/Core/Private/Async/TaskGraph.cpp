...
CORE_API int32 GTaskThreadStackSize = 0;
static FAutoConsoleVariableRef CVarTaskThreadStackSize(
	TEXT("TaskGraph.ThreadStackSize"),
	GTaskThreadStackSize,
	TEXT("Thread Stack Size, 0 means platform default")
);

#if UE_ASYNC_WORK_STEALING_QUEUE_USES_RETRY_LIMIT
CORE_API int32 GWorkStealingQueueRetryLimit = -1;
static FAutoConsoleVariableRef WorkStealingQueueRetryLimitCVar(
	TEXT("TaskGraph.WorkStealingQueue.RetryLimit"),
	GWorkStealingQueueRetryLimit,
	TEXT("The maximum number of times (-1 = practically infinite) that a work-stealing attempt can fail due to a taken slot before it is forced to yield"),
	ECVF_Default);
#endif
	...
	static void ThreadSwitchForABTest(const TArray<FString>& Args)
	{
		...
		{
			UE_LOGF(LogConsoleResponse, Display, "This command requires two arguments, both 0 or 1 to control the use of high priority and background priority threads, respectively.");
		}
		UE_LOGF(LogConsoleResponse, Display, "High priority task threads: %d    Background priority threads: %d", ENamedThreads::bHasHighPriorityThreads, ENamedThreads::bHasBackgroundThreads);
	}
		...
		~FProfileRecScope()
		{
			if (SampleIndex < 1000)
			{
				...
				if (Target->NumSamplesFinished.Increment() == 1000)
				{
					...
					UE_LOGF(LogTemp, Display, "%6.4f ms / scope %ls",MsPer, Target->Name);
...
static void EnableRandomizedThreads(const TArray<FString>& Args)
{
	...
	{
		UE_LOGF(LogConsoleResponse, Display, "Random sleeps are enabled.");
	}
	else
	{
		UE_LOGF(LogConsoleResponse, Display, "Random sleeps are disabled.");
	}
}
...
private:
	...
	void WaitOnNamedThreadForTasks(const FGraphEventArray& Tasks, ENamedThreads::Type BaseNamedThread, ENamedThreads::Type NamedThreadWithFlags)
	{
		...
		if (!FTaskGraphInterface::IsMultithread())
		{
			...
			UE_LOGF(LogTaskGraph, Fatal, "Recursive waits are not allowed in single threaded mode.");
		}
	...
	bool ProcessUntilTasksComplete(const FGraphEventArray& Tasks, ENamedThreads::Type CurrentThreadIfKnown, const FProcessTasksUpdateCallback& IdleWorkUpdate) final override
	{
		...
		for (int32 TaskIndex = 0; TaskIndex < Tasks.Num(); TaskIndex++)
		{
			...
			if (!ensure(RemainingTasks[TaskIndex] != nullptr))
			{
				...
				UE_LOGF(LogTaskGraph, Error, "ProcessUntilTasksComplete was passed an invalid event!");
				return false;
			}
		}
...
public:
	...
	void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
	{
		auto LogWarningIfSlow = [this](const TCHAR* Msg)
		{
			...
			{
				UE_CLOGF(GPrintBroadcastWarnings, LogTaskGraph, Warning, "Task graph took %6.2fms for %ls to %ls", ThisTime * 1000.0, Name, Msg);
			}
		};
...
void FTaskGraphInterface::BroadcastSlow_OnlyUseForSpecialPurposes(bool bDoTaskThreads, bool bDoBackgroundThreads, TFunction<void(ENamedThreads::Type CurrentThread)>& Callback)
{
	...
	if (bDoTaskThreads)
	{
		...
		{
			UE_LOGF(LogTaskGraph, Log, "FTaskGraphInterface::BroadcastSlow_OnlyUseForSpecialPurposes Broadcast failed after three seconds. Ok during automated tests.");
		}
		...
		}
		{
			...
			FTaskGraphInterface::Get().WaitUntilTasksComplete(TaskThreadTasks, ENamedThreads::GameThread_Local);
			{
				...
				if (ThisTime > 0.02)
				{
					UE_CLOGF(GPrintBroadcastWarnings, LogTaskGraph, Warning, "Task graph took %6.2fms to wait for task thread broadcast.", ThisTime * 1000.0);
				}
			}
		}
	}
	...
	}
	{
		...
		if (ThisTime > 0.02)
		{
			UE_CLOGF(GPrintBroadcastWarnings, LogTaskGraph, Warning, "Task graph took %6.2fms to broadcast.", ThisTime * 1000.0);
		}
	}
}
...
static void HandleNumWorkerThreadsToIgnore(const TArray<FString>& Args)
{
	...
	UE_LOGF(LogConsoleResponse, Display, "Currently ignoring %d threads per priority bank", GNumWorkerThreadsToIgnore);
}
...
static void SetTaskThreadPriority(const TArray<FString>& Args)
{
	...
	if (Args.Num() && Args[0] == TEXT("abovenormal"))
	{
		...
		UE_LOGF(LogConsoleResponse, Display, "Setting task thread priority to above normal.");
	}
	else if (Args.Num() && Args[0] == TEXT("belownormal"))
	{
		Pri = TPri_BelowNormal;
		UE_LOGF(LogConsoleResponse, Display, "Setting task thread priority to below normal.");
	}
	else
	{
		UE_LOGF(LogConsoleResponse, Display, "Setting task thread priority to normal.");
	}
