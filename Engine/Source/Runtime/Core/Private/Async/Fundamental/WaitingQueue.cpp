...
void FWaitingQueue::ConditionalStandby(FWaitEvent* Node)
{
	...
	while (GetActiveThreadCount(LocalState) > ThreadCount + Oversubscription.load(std::memory_order_relaxed))
	{
		...
		{
			// After pushing ourselves onto the standby stack, recheck shutdown.
			// StartShutdown drains the stack then does a plain StandbyState = StackMask.
			// If we pushed after the drain but before that store, nobody will trigger
			// our event and StopWorkers will deadlock on Thread->Join().
...
bool FWaitingQueue::CommitStandby(FWaitEvent* Node, FOutOfWork& OutOfWork)
{
...

	{
		...
		{
			// Same shutdown race as ConditionalStandby — recheck after pushing onto the stack.
			if (bIsShuttingDown.load(std::memory_order_acquire))
			{
				Node->Event->Trigger();
				OutOfWork.Stop();
