...
bool FSharedRecursiveMutex::IsLockedByCurrentThread() const
{
	const uint32 CurrentThreadId = FPlatformTLS::GetCurrentThreadId();
	return ThreadId.load(std::memory_order_relaxed) == CurrentThreadId;
}
...
{
	Link.Pop();
	// C++ SharedMutex requires that UnlockShared() synchronizes-with subsequent Lock() operations.
	// This requirement necessitates the use of release memory order or stronger.
