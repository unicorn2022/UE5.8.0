...
{
	AutoRTFM::UnreachableIfClosed("FUniqueBuffer is not yet transactionally safe. Call FCbWriter::Save(const FMutableMemoryView Buffer) instead.");
...
void FCbWriter::AddBinary(FUtf8StringView Name, const FSharedBuffer& Value)
{
	SetName(Name);
	AddBinary(Value);
}
...
void FCbWriter::AddBinary(FUtf8StringView Name, const FCompositeBuffer& Value)
{
	SetName(Name);
	AddBinary(Value);
}
