...
void AppendQuotedJsonString(TStringBuilderBase<CharType>& Builder, FUtf8StringView Value)
{
	...
	while (!Value.IsEmpty())
	{
		...
		for (UTF8CHAR Char : Escape)
		{
			switch (Char)
			{
			case UTF8CHAR('\\'): Builder << FConstants::EscapeBackslash; break;
			case UTF8CHAR('\"'): Builder << FConstants::EscapeDoubleQuote; break;
			case UTF8CHAR('\b'): Builder << FConstants::EscapeBell; break;
			case UTF8CHAR('\f'): Builder << FConstants::EscapeFormFeed; break;
			case UTF8CHAR('\n'): Builder << FConstants::EscapeLineFeed; break;
			case UTF8CHAR('\r'): Builder << FConstants::EscapeCarriageReturn; break;
			case UTF8CHAR('\t'): Builder << FConstants::EscapeTab; break;
			default:
...
void LogFieldTooLargeForArrayWarning(uint64 ArraySize)
{
	UE_LOGFMT(LogSerialization, Warning, "Attempting to load a FCbFieldView that has too many elements ({ArraySize}) to fit in TArray.", ArraySize);
}
...
{
	return LoadAndValidateCompactBinary(Ar, Allocator, ECbValidateMode::Default);
}

FCbField LoadAndValidateCompactBinary(FArchive& Ar, FCbBufferAllocator Allocator, ECbValidateMode ValidateMode)
{
	...
	for (const int64 StartPos = Ar.Tell(); !Ar.IsError() && FieldSize > 0;)
	{
		...
		if (!Ar.IsError() && TryMeasureCompactBinary(MakeMemoryView(HeaderBytes), FieldType, FieldSize))
		{
			if (FieldSize <= uint64(Ar.TotalSize() - StartPos))
			{
				...
				FMutableMemoryView TailView = Buffer.GetView().CopyFrom(MakeMemoryView(HeaderBytes));
				if (!TailView.IsEmpty())
				{
					// Read the remainder of the field.
					Ar.Serialize(TailView.GetData(), static_cast<int64>(TailView.GetSize()));
				}

				if (!Ar.IsError() && (ValidateMode == ECbValidateMode::None || ValidateCompactBinary(Buffer, ValidateMode) == ECbValidateError::None))
				{
...
FCbFieldView LoadCompactBinaryView(FArchive& Ar, FCbMemoryAllocator Allocator)
{
	return LoadAndValidateCompactBinaryView(Ar, Allocator, ECbValidateMode::Default);
}

FCbFieldView LoadAndValidateCompactBinaryView(FArchive& Ar, FCbMemoryAllocator Allocator, ECbValidateMode ValidateMode)
{
	TArray<uint8, TInlineAllocator<64>> HeaderBytes;
	ECbFieldType FieldType;
	uint64 FieldSize = 1;
	...
	for (const int64 StartPos = Ar.Tell(); !Ar.IsError() && FieldSize > 0;)
	{
		...
		if (!Ar.IsError() && TryMeasureCompactBinary(MakeMemoryView(HeaderBytes), FieldType, FieldSize))
		{
			if (FieldSize <= uint64(Ar.TotalSize() - StartPos))
			{
				FMutableMemoryView View = Allocator(FieldSize);
				checkf(View.GetSize() == FieldSize, TEXT("Allocator returned a buffer of size %" UINT64_FMT " bytes "
					"when %" UINT64_FMT " bytes were requested."), View.GetSize(), FieldSize);

				FMutableMemoryView TailView = View.CopyFrom(MakeMemoryView(HeaderBytes));
				if (!TailView.IsEmpty())
				{
					// Read the remainder of the field.
					Ar.Serialize(TailView.GetData(), static_cast<int64>(TailView.GetSize()));
				...
				if (!Ar.IsError() && (ValidateMode == ECbValidateMode::None || ValidateCompactBinary(View, ValidateMode) == ECbValidateError::None))
				{
					return FCbFieldView(View.GetData());
				}
			}
			break;
		}
