...
FArchive& FArchiveFromStructuredArchiveImpl::operator<<(Verse::VCell*& Value)
{
	OpenArchive();

	if (InnerArchive.IsTextFormat())
	{
		// VCell references are not generally supported in structured text archives, but
		// ignoring them while saving makes things easier for use cases like debug printing.
		check(!IsLoading());
	}
	else
	{
