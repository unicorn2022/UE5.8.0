...
FArchiveBoundsScope::FArchiveBoundsScope(FArchive* InOwner, FArchiveBounds InOriginalBounds)
	: Owner(InOwner)
...
FArchiveBoundsScope::~FArchiveBoundsScope()
{
	check(Owner);
	Owner->SetBoundsImpl(OriginalBounds);
}

FArchiveBoundsScope FArchive::SetBounds(int64_t Size)
{
	FArchiveBounds PrevBounds = GetBoundsImpl();
	SetBoundsImpl({ Tell(), Size});
	return FArchiveBoundsScope(this, PrevBounds);
}

void FArchive::SetBoundsImpl(FArchiveBounds NewBounds)
{
}

FArchiveBounds FArchive::GetBoundsImpl() const
{
	return {};
}
...
#if UE_INTERNAL_LINKER_BOUNDS
void FArchiveState::ClearCriticalError()
{
	ForEachState([](FArchiveState& State) { State.ArIsError = State.ArIsCriticalError = false; });
}
#endif // UE_INTERNAL_LINKER_BOUNDS
...
FArchive& FArchive::operator<<(struct FLazyObjectPtr& Value)
{
	...
	UE_LOGF(LogSerialization, Fatal, "FArchive does not support FLazyObjectPtr serialization. Use FArchiveUObject instead.");
	return *this;
}
...
{
	// The base FArchive does not implement this method. Use FArchiveUObject instead.
	UE_LOGF(LogSerialization, Fatal, "FArchive does not support FObjectPtr serialization. Use FArchiveUObject instead.");
	return *this;
}
...
{
	// The base FArchive does not implement this method. Use FArchiveUObject instead.
	UE_LOGF(LogSerialization, Fatal, "FArchive does not support FSoftObjectPtr serialization. Use FArchiveUObject instead.");
	return *this;
}
...
{
	// The base FArchive does not implement this method. Use FArchiveUObject instead.
	UE_LOGF(LogSerialization, Fatal, "FArchive does not support FSoftObjectPath serialization. Use FArchiveUObject instead.");
	return *this;
}
...
{
	// The base FArchive does not implement this method. Use FArchiveUObject instead.
	UE_LOGF(LogSerialization, Fatal, "FArchive does not support FWeakObjectPtr serialization. Use FArchiveUObject instead.");
	return *this;
}
...
void FArchive::SerializeBool( bool& D )
{
	...
	if (OldUBoolValue > 1)
	{
		UE_LOGFMT(LogSerialization, Error, "Invalid boolean encountered while reading archive {ArchiveName} - stream is most likely corrupted.", GetArchiveName());
...
void FArchive::SerializeCompressedNew(void* V, int64 Length, FName CompressionFormatToEncode, FName CompressionFormatToDecodeOldV1Files,  ECompressionFlags Flags, bool bTreatBufferAsFileReader, int64 * OutPartialReadLength)
{
	...
	if( IsLoading() )
	{
		...
		*this << PackageFileTag;
		...
		{
			UE_LOGF(LogSerialization, Log, "ArchiveName: %ls", *GetArchiveName());
			UE_LOGF(LogSerialization, Log, "Archive UE Version: %d", UEVer().ToValue());
			UE_LOGF(LogSerialization, Log, "Archive Licensee Version: %d", LicenseeUEVer());
			UE_LOGF(LogSerialization, Log, "Position: %lld", Tell());
			UE_LOGF(LogSerialization, Log, "Read Size: %lld", Length);
			UE_LOGF(LogSerialization, Fatal, "BulkData compressed header read error. This package may be corrupt!");
		}
		...
		if ( ! bReadCompressionFormat )
		{
			...
			{
				UE_LOGF(LogSerialization, Warning, "Old style compression flags are being used with FAsyncCompressionChunk, please update any code using this!");
				PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		{		
			...
			if (CompressionFormatToDecode == NAME_Zlib && FPlatformProperties::GetZlibReplacementFormat() != nullptr)
			{
				...
				UE_LOGF(LogSerialization, Warning, "Archive v2 header with ZLib not ZlibReplacement: %ls", *CompressionFormatToDecode.ToString());
			}
		}
		...
		if ( ! FCompression::IsFormatValid(CompressionFormatToDecode) )
		{
			...
			UE_LOGF(LogSerialization, Log, "Position: %lld", Tell());
			UE_LOGF(LogSerialization, Log, "Read Size: %lld", Length);
			UE_LOGF(LogSerialization, Log, "CompressionFormatToDecode not found : %ls", *CompressionFormatToDecode.ToString());
			UE_LOGF(LogSerialization, Fatal, "BulkData compressed header read error. This package may be corrupt!");
		}
		...
		*this << Summary;
		...
		UE_CLOGF(Summary.CompressedSize < 0 || Summary.CompressedSize > (INT64_MAX/2) , LogSerialization, Fatal, " Archive SerializedCompressed CompressedSize (%lld) invalid", (int64)Summary.CompressedSize);
		UE_CLOGF(Summary.UncompressedSize < 0 || Summary.UncompressedSize > (INT64_MAX/2) , LogSerialization, Fatal, " Archive SerializedCompressed UncompressedSize (%lld) invalid", (int64)Summary.UncompressedSize);
		...
		UE_CLOGF(LoadingCompressionChunkSize >= INT32_MAX, LogSerialization, Fatal, " Archive SerializedCompressed LoadingCompressionChunkSize (%lld) >= INT32_MAX", LoadingCompressionChunkSize);

		// check Summary.UncompressedSize vs [V,Length] passed in
		if ( OutPartialReadLength == nullptr )
		{
			// UncompressedSize must == Length
			UE_CLOGF( Summary.UncompressedSize != Length, LogSerialization, Fatal, " Archive SerializedCompressed UncompressedSize (%lld) != Length (%lld)", (int64)Summary.UncompressedSize, (int64) Length );
		}
		else
		{
		...
		UE_CLOGF( TotalChunkCompressedSize != Summary.CompressedSize, LogSerialization, Fatal, " Archive SerializedCompressed TotalChunkCompressedSize (%lld) != Summary.CompressedSize (%lld)", (int64)TotalChunkCompressedSize, (int64) Summary.CompressedSize );
		UE_CLOGF( TotalChunkUncompressedSize != Summary.UncompressedSize, LogSerialization, Fatal, " Archive SerializedCompressed TotalChunkUncompressedSize (%lld) != Summary.UnompressedSize (%lld)", (int64)TotalChunkUncompressedSize, (int64) Summary.UncompressedSize );
		...
		for( int64 ChunkIndex=0; ChunkIndex<TotalChunkCount; ChunkIndex++ )
		{
			...
			if ( IsError() )
			{
				UE_LOGF(LogSerialization,Error, "Failed to serialize compress chunk in %ls, Chunk.CompressedSize=%d", *GetArchiveName(), (int)Chunk.CompressedSize);
				break;
			}
			...
			{
				UE_LOGF(LogSerialization,Error, "Failed to uncompress data in %ls, CompressionFormatToDecode=%ls", *GetArchiveName(), *CompressionFormatToDecode.ToString());
				SetError();
				break;
			}
...
void FArchive::SerializeIntPacked(uint32& Value)
{
	if (IsLoading())
	{
		...
		while(more)
		{
			...
			TempValue += (uint32)((uint64)NextByte << (7 * cnt++));	// Add to total value
		}
