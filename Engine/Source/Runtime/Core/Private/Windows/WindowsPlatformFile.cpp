...
class FFileHandleWindows : public IFileHandle
{
	...
	void UpdateFileSize()
	{
		...
		else
		{
			...
			UE_LOGF(LogTemp, Warning, "GetFileSizeEx: Failed to get file size for handle!");
		}
	}
...
#if UE_BUILD_SHIPPING
		UE_CLOGF(NumOutstandingRegions.load(std::memory_order_relaxed), LogHAL, Error, "Cleaning mapped file with alive mapped regions: %ls", *DebugFilename);
#else
		UE_CLOGF(NumOutstandingRegions.load(std::memory_order_relaxed), LogHAL, Fatal, "Cleaning mapped file with alive mapped regions: %ls", *DebugFilename);
#endif
...
{
	auto ConvertToUnrealPathInline = [](FString& Path)
	{
		// Remove the Windows device path prefix.
		if (Path.StartsWith(TEXT("\\\\?\\UNC\\"), ESearchCase::CaseSensitive))
		{
			Path.RemoveAt(2, 6); // remove ?\UNC\ to convert \\?\UNC\Path\... to \\Path\...
		}
		else
		{
		...
		Path.ReplaceCharInline(TEXT('\\'), TEXT('/'), ESearchCase::CaseSensitive);
	};

	FString Result = *FNormalizedFilename(Filename);
	FString NormalizedInputPath = Result;
	ConvertToUnrealPathInline(NormalizedInputPath);

	TRACE_PLATFORMFILE_BEGIN_OPEN(Filename);
	HANDLE hFile = CreateFile(*Result,
	...
	for (uint32 Length = Result.Len() + 10;;)
	{
		TArray<TCHAR, FString::AllocatorType>& CharArray = Result.GetCharArray();
		CharArray.SetNum(Length);

		Length = GetFinalPathNameByHandle(hFile, CharArray.GetData(), CharArray.Num(), FILE_NAME_NORMALIZED);
		if (Length == 0)
		{
			// Error - return normalized filename.
			return NormalizedInputPath;
		}
	...
	ConvertToUnrealPathInline(Result);

	// Check that the result only differs by case for the full path - otherwise do a slower approach which builds the path up by component.
	// This is to ensure symbolic links aren't followed.
	if (!Result.Equals(NormalizedInputPath, ESearchCase::IgnoreCase))
	{
		Result.Reset();

		FString AccumulatedFilename;
		AccumulatedFilename.Reserve(Result.Len());

		TArray<uint8> FileNameInfoTempBuffer;
		FileNameInfoTempBuffer.AddUninitialized(sizeof(FILE_NAME_INFO) + NormalizedInputPath.Len() * sizeof(TCHAR));
		FPathViews::IterateComponents(
			NormalizedInputPath,
			[&AccumulatedFilename, &FileNameInfoTempBuffer, &Result](FStringView Component)
			{
				AccumulatedFilename /= Component;
				...
				HANDLE hFile = CreateFile(
					*AccumulatedFilename,
					...
					NULL,
					OPEN_EXISTING,
					FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_BACKUP_SEMANTICS, // FILE_FLAG_OPEN_REPARSE_POINT prevents symlinks from being followed. FILE_FLAG_BACKUP_SEMANTICS is required for opening a directory, which AccumulatedFilename might be.
					NULL
				);
				if (hFile == INVALID_HANDLE_VALUE)
				{
					// File is probably missing - leave Component unchanged
					Result /= Component;
					return;
				}
				ON_SCOPE_EXIT
				...
				for (;;)
				{
					FILE_NAME_INFO* BufferPtr = (FILE_NAME_INFO*)FileNameInfoTempBuffer.GetData();
					BOOL bResult = GetFileInformationByHandleEx(hFile, FileNameInfo, BufferPtr, (DWORD)FileNameInfoTempBuffer.Num());
					if (bResult)
					{
						Result /= FPathViews::GetPathLeaf(MakeStringView(BufferPtr->FileName, BufferPtr->FileNameLength / sizeof(*BufferPtr->FileName)));
						return;
					}

					// If the error wasn't due to a too-small buffer - leave Component unchanged
					...
					FileNameInfoTempBuffer.SetNumUninitialized(sizeof(FILE_NAME_INFO) + BufferPtr->FileNameLength);
				}
			}
		);

		// If it's still not a case variant, just return the normalized input.
		if (!Result.Equals(NormalizedInputPath, ESearchCase::IgnoreCase))
		{
			return NormalizedInputPath;
		}
...
FOpenMappedResult FWindowsPlatformFile::OpenMappedEx(const TCHAR* Filename, EOpenReadFlags OpenOptions, int64 MaximumSize)
{
	...
	if (EnumHasAnyFlags(OpenOptions, EOpenReadFlags::AllowWrite))
	{
		...
		ActualOpenOptions |= EOpenMappedFlags::AllowRead;
	}
	if (EnumHasAnyFlags(OpenOptions, EOpenReadFlags::AllowDelete))
	{
		ActualOpenOptions |= EOpenMappedFlags::AllowDelete;
	}

	return OpenMappedEx2(Filename, ActualOpenOptions, MaximumSize);
}

FOpenMappedResult FWindowsPlatformFile::OpenMappedEx2(const TCHAR* Filename, EOpenMappedFlags OpenOptions, int64 MaximumSize)
{
	uint32  Access = GENERIC_READ;
	uint32  WinFlags = FILE_SHARE_READ |
		(EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::AllowWrite) ? FILE_SHARE_WRITE : 0) |
		(EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::AllowDelete) ? FILE_SHARE_DELETE : 0) |
		GetFileShareDeleteFlagIfEnabled();
	uint32  Create = OPEN_EXISTING;
	...
	if (EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::OpenForWrite))
	{
		...
		WinFlags = 
			(EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::AllowRead) ? FILE_SHARE_READ : 0) |
			(EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::AllowDelete) ? FILE_SHARE_DELETE : 0) |
			GetFileShareDeleteFlagIfEnabled();
		Create = EnumHasAnyFlags(OpenOptions, EOpenMappedFlags::OpenExisting) ? OPEN_EXISTING : CREATE_ALWAYS;
		...
		if (MaximumSize < 1)
		{
			...
			{
				const int64 Size = FileSize(Filename);
				if (Size < 1)
				{
					return MakeError(TEXT("File size is 0 or the file does not exist and no maximum size provided."));
				}
	...
	else
	{
		...
		if (Size < 1)
		{
			return MakeError(TEXT("File size is 0 or the file does not exist"));
		}

		if (MaximumSize < 1 || MaximumSize > Size)
		{
			MaximumSize = Size;
		}
	}
	...
	return MakeValue(new FMappedFileHandleWindows(Handle, MappingHandle, MaximumSize, Filename));
}
...
bool FWindowsPlatformFile::IterateDirectoryCommon(const TCHAR* Directory, const TFunctionRef<bool(const WIN32_FIND_DATAW&)>& Visitor)
{
	...
	{
		// FindFirstFileWithWildcard can return an invalid handle even for directories that exist on disk, due to flags
		// on the directory that instruct the operating system to hide them from various listings. e.g.
		...
		// 
		// But unfortunately, FindNextFileW on the parent directory will still report those hidden directories, and we
		// therefore report them to our caller. IPlatformFile::IterateDirectoryRecursively on the parent
		// directory reasonably expects that for these directories we report, calling IterateDirectory on them does
		// not return false (unless the Visitor returns false).
		//
		// To workaround this issue, we return true for these hidden directories.
		// But we still need to return false if the directory does not exist.
		// DirectoryExists uses the function GetFileAttributes to decide whether the directory exists.
