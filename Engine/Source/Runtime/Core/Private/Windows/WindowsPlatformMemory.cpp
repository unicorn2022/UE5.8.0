...
static bool IsRunningAsJob()
{
	static bool bIsJob = []() {
		...
		if (!IsProcessInJob(GetCurrentProcess(), NULL, &bIsRunningInJob))
		{
			...
			UE_LOGF(LogHAL, Warning, "IsProcessInJob(GetCurrentProcess(), NULL, &bIsRunningInJob) failed with GetLastError() = %d",
				ErrNo
			);
			return false;
		}
...
void FWindowsPlatformMemory::Init()
{
	...
	if (IsRunningAsJob())
	{
		UE_LOGF(LogMemory, Log, "Process is running as part of a Windows Job with separate resource limits");
	}

	const FPlatformMemoryConstants& MemoryConstants = FPlatformMemory::GetConstants();
	UE_LOGF(LogMemory, Log, "Memory total: Physical=%.1fGB (%dGB approx) Virtual=%.1fGB", 
...
{	
	...
	{
		DWORD ErrNo = GetLastError();
		UE_LOGF(LogMemory, Error, "QueryInformationJobObject(NULL, JobObjectLimitViolationInformation, &LimitInformation, sizeof(LimitInformation), &LengthWritten) failed with GetLastError() = %d",
			ErrNo
		);
		return 0;
	}
...
const FPlatformMemoryConstants& FWindowsPlatformMemory::GetConstants()
{
	...
	if( MemoryConstants.TotalPhysical == 0 )
	{
		...
		if (IsRunningAsJob())
		{
			...
			{
				DWORD ErrNo = GetLastError();
				UE_LOGF(LogMemory, Error, "QueryInformationJobObject(NULL, JobObjectExtendedLimitInformation, &JobInfo, sizeof(JobInfo), &LengthWritten) failed with GetLastError() = %d",
					ErrNo
				);
			}
			else
			{
				...
				{
					UE_LOGF(LogMemory, Display, "Running as part of a job but no memory limit has been set.");
				}
				else
				{
					...
					if ((JobInfo.BasicLimitInformation.LimitFlags & JOB_OBJECT_LIMIT_JOB_MEMORY) == JOB_OBJECT_LIMIT_JOB_MEMORY)
					{
						...
						UE_LOGF(LogMemory, Display, "Detected a job memory limit of %.1fGB for this job.",
					...
					if ((JobInfo.BasicLimitInformation.LimitFlags & JOB_OBJECT_LIMIT_PROCESS_MEMORY) == JOB_OBJECT_LIMIT_PROCESS_MEMORY)
					{
						...
						UE_LOGF(LogMemory, Display, "Detected a per-process memory limit of %.1fGB for this job.",
							(double)MemoryLimit / (1024 * 1024 * 1024)
						);
					}
...
FPlatformMemory::FSharedMemoryRegion* FWindowsPlatformMemory::MapNamedSharedMemoryRegion(const FString& InName, bool bCreate, uint32 AccessMode, SIZE_T Size, const void* pSecurityAttributes)
{
	...
	if (bCreate)
	{
		...
		{
			DWORD ErrNo = GetLastError();
			UE_LOGF(LogHAL, Warning, "CreateFileMapping(file=INVALID_HANDLE_VALUE, security=NULL, protect=0x%x, MaxSizeHigh=%d, MaxSizeLow=%d, name='%ls') failed with GetLastError() = %d", 
	...
	else
	{
		...
		{
			DWORD ErrNo = GetLastError();
			UE_LOGF(LogHAL, Warning, "OpenFileMapping(access=0x%x, inherit=false, name='%ls') failed with GetLastError() = %d", 
...
bool FWindowsPlatformMemory::UnmapNamedSharedMemoryRegion(FSharedMemoryRegion * MemoryRegion)
{
	...
	if (MemoryRegion)
	{
		...
		if (!UnmapViewOfFile(WindowsRegion->GetAddress()))
		{
			...
			UE_LOGF(LogHAL, Warning, "UnmapViewOfFile(address=%p) failed with GetLastError() = %d", 
