...
{
	if (!CanProcessStackWalk())
	{
		return TArray<FProgramCounterSymbolInfo>();
	}
	TGuardValue<bool> ReentryGuard(GStackWalkInProgress, true);
...
UE_AUTORTFM_ALWAYS_OPEN void FWindowsPlatformStackWalk::ThreadStackWalkAndDump(ANSICHAR* HumanReadableString, SIZE_T HumanReadableStringSize, int32 IgnoreCount, uint32 ThreadId)
{
	...
	if (ThreadHandle)
	{
		...
		ContextWapper.Context.ContextFlags = CONTEXT_ALL;
...
bool FWindowsPlatformStackWalk::UploadLocalSymbols()
{
	...
	{
		UE_LOGF(LogWindows, Log, "Uploading to symbol storage disabled by command line flag");
		return true;
	}
	// Prepare string
	...
	for (int32 ModuleIndex = 0; ModuleHandlePointer[ModuleIndex]; ModuleIndex++)
	{
		...
		if (PathRenameExtensionW( DebugName, L".pdb" ))
		{
			...
			if (PathFileExistsW( DebugName ))
			{
				...
				UE_LOGF( LogWindows, Log, "Uploading to symbol storage: %ls", ImageName );
				if (!SymSrvStoreFileW( ProcessHandle, *SymbolStorage, ImageName, SYMSTOREOPT_PASS_IF_EXISTS ))
				{
					...
					UE_LOGF(LogWindows, Warning, "Uploading to symbol storage failed. Error Code %u: %ls", Result, ErrorBuffer);
					// Calling SymSrvStoreFileW can crash if called after failing, so ditch out of the loop on error
					break;
				}

				// Upload debug symbols
				UE_LOGF( LogWindows, Log, "Uploading to symbol storage: %ls", DebugName );
...
UE_AUTORTFM_ALWAYS_OPEN void LoadSymbolsForModule(HMODULE ModuleHandle, const FString& RemoteStorage)
{
	...
	// Load module.
	const DWORD64 BaseAddress = SymLoadModuleExW(ProcessHandle, NULL, ImageName, ModuleName, (DWORD64)ModuleInfo.lpBaseOfDll, ModuleInfo.SizeOfImage, NULL, 0);
	if (!BaseAddress)
	{
		ErrorCode = GetLastError();
		...
		if (ErrorCode != ERROR_SUCCESS)
		{
			...
			UE_LOGF(LogWindows, Verbose, "Failed to load module '%ls' with SymLoadModuleExW. Error %d: %ls", ModuleName, ErrorCode, ErrorMsg);
		}
	}
}
...
UE_AUTORTFM_ALWAYS_OPEN int32 FWindowsPlatformStackWalk::GetProcessModuleSignatures(FStackWalkModuleInfo *ModuleSignatures, const int32 ModuleSignaturesSize)
{
	...
	for( int32 ModuleIndex = 0; ModuleHandlePointer[ModuleIndex] && SignatureIndex < ModuleSignaturesSize; ModuleIndex++ )
	{
		...
		else
		{
			...
			UE_LOGF(LogWindows, Verbose, "SymGetModuleInfoW64 failed, rich module information unavailable. Error Code: %u", GetLastError());
		}
