...
int64 FVirtualAllocPageStatus::MarkChangedAndReturnDeltaSize(void* InStartAddress, SIZE_T InSize, bool bCommitted)
{
	...
	if (StartIndexInGroup != 0)
	{
		...
		if (bCommitted)
		{
			...
			NumChangedPages += NumModifiedBits - FMath::CountBits64(PageBits & ModifiedBitsMask);
		...
		else
		{
			...
			if (PageBits)
			{
				NumChangedPages += FMath::CountBits64((*PageBits) & ModifiedBitsMask);
				*PageBits &= ~ModifiedBitsMask;
				if (*PageBits == 0)
				{
	...
	while (Group < EndFullGroups)
	{
		if (bCommitted)
		{
			...
			uint64& PageBits = GroupToPageBitsMap.FindOrAdd(Group, 0);
			NumChangedPages += PagesPerGroup - FMath::CountBits64(PageBits);
			PageBits = ~(0ULL);
		}
		else
		{
			...
			NumChangedPages += FMath::CountBits64(PageBits);
		}
