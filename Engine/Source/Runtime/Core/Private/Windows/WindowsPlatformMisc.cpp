...
// Collect extended storage information asynchronously. A Sync call can take in the order of 1 second on the calling thread
bool GCollectStorageInformationAsync = false;
static FAutoConsoleVariableRef CVarCollectStorageInformationAsync(
	TEXT("g.CollectStorageInformationAsync"),
	GCollectStorageInformationAsync,
	TEXT("Collects extended storage information async"),
	ECVF_ReadOnly);
...
{
	static TAutoConsoleVariable<bool> CVarDisableWindowsMessageBox(
		TEXT("WindowsPlatform.DisableMessageBox"),
		false,
		TEXT("If this is true, no MessageBox will be displayed and calls to MessageBoxExt will return EAppReturnType::Cancel by default."),
		ECVF_Default);
	
	static TAutoConsoleVariable<bool> CVarWindowsRaiseExceptionsOnFaultTermination(
		TEXT("WindowsPlatform.RaiseExceptionOnFaultTermination"),
		false,
		TEXT("If true, regular faults will raise an exception (1) prior to terminating the process, useful for external crash handlers"),
		ECVF_Default);
}
...
int32 GetOSVersionsHelper( TCHAR* OutOSVersionLabel, int32 OSVersionLabelLength, TCHAR* OutOSSubVersionLabel, int32 OSSubVersionLabelLength )
{
	...
	FString OSVersionLabel = "Windows (unknown version)";
...
namespace
{
	...
	#endif
		{
			FCString::Snprintf(OutString, Length, TEXT("%d.%d.%d.%d.%d.64bit"), OsVersionInfo.dwMajorVersion, OsVersionInfo.dwMinorVersion, OsVersionInfo.dwBuildNumber, OsVersionInfo.wProductType, OsVersionInfo.wSuiteMask);
			return true;
		}
		return false;
	}
	...
	// NOTE: This is populated async (if GCollectStorageInformationAsync)
	// Usage must be synchronized with WaitOnCollectStorageInformationAsync
	...
	static void LogStorageInformationWarning(HRESULT HRes,  const TCHAR* message)
	{
		...
		if (ErrorInfo)
		{
			...
			{
				UE_LOGF(LogWindows, Log, "%ls [%ls]", message, ErrorMessage);
			}
			else
			{
				UE_LOGF(LogWindows, Log, "%ls [error code %x]", message, HRes);
			}
	...
	static void CollectStorageInformationImpl()
	{
	...
	struct FCollectStorageInformationAsyncThread
	{
		HANDLE HThread;
		DWORD HThreadId;

		FCollectStorageInformationAsyncThread()
			: HThread(NULL)
			, HThreadId(0)
		{
		}
	...
	static FCollectStorageInformationAsyncThread GCollectStorageInformationAsyncThread;
	
	static DWORD WINAPI CollectStorageInformationAsyncProc(LPVOID Arg)
	{
		CollectStorageInformationImpl();
		return 0;
	}

	static void CollectStorageInformationAsync()
	{
		GCollectStorageInformationAsyncThread.HThread = ::CreateThread(NULL, 0, CollectStorageInformationAsyncProc, NULL, 0, &GCollectStorageInformationAsyncThread.HThreadId);
		if (GCollectStorageInformationAsyncThread.HThread == NULL)
		{
			UE_LOGF(LogWindows, Error, "Unable to create thread for CollectStorageInformationAsync, running synchronous...");
			CollectStorageInformationImpl();
		}
	}
#endif // ALLOW_COLLECT_STORAGE_INFORMATION

	static void WaitOnCollectStorageInformationAsync()
	{
#if ALLOW_COLLECT_STORAGE_INFORMATION
		if (GCollectStorageInformationAsync && ::GetCurrentThreadId() != GCollectStorageInformationAsyncThread.HThreadId)
		{
			::WaitForSingleObject(GCollectStorageInformationAsyncThread.HThread, INFINITE);
		}
#endif
	}

	static void CollectStorageInformation()
	{
#if ALLOW_COLLECT_STORAGE_INFORMATION
		if (GCollectStorageInformationAsync)
		{
			CollectStorageInformationAsync();
		}
		else
		{
			CollectStorageInformationImpl();
...
const TCHAR* LexToString(ETDRLevel TDRLevel)
{
	switch (TDRLevel)
	{
	case ETDRLevel::Disabled:
		return TEXT("Disabled");
	case ETDRLevel::BugCheck:
		return TEXT("BugCheck");
	case ETDRLevel::RecoverVGA:
		return TEXT("RecoverVGA");
	case ETDRLevel::Recover:
		return TEXT("Recover");
	case ETDRLevel::Invalid:
		[[fallthrough]];
	default:
		return TEXT("Invalid");
	}
...
static void PureCallHandler()
{
	...
	else
	{
		...
		UE_LOGF(LogWindows, Fatal,"Pure virtual function being called" );
	}
}
...
void PGO_WriteFile()
{
	...
	FString PGOSweepPath = FPaths::Combine(ExeFolder, TEXT("arm64ec"), TEXT("pgosweep.exe"));
#elif PLATFORM_CPU_ARM_FAMILY
	FString PGOSweepPath = FPaths::Combine(ExeFolder, TEXT("arm64"), TEXT("pgosweep.exe"));
#else
	...
	if (ProcHandle.IsValid())
	{
		...
		{
			UE_LOGF(LogWindows, Log, "pgosweep.exe exit code %d", ExitCode);
		}
		FPlatformProcess::CloseProc(ProcHandle);
	}
	else
	{
		UE_LOGF(LogWindows, Error, "Failed to launch pgosweep.exe");
	}
}
...
#if WITH_APPLICATION_CORE && WITH_WINDOWS_CONFIGRULES
	{
		SCOPED_BOOT_TIMING("FWindowsConfigRules::Init");
		FWindowsConfigRules::Init();
	}
	{
		SCOPED_BOOT_TIMING("FWindowsConfigRules::Parse");
		FWindowsConfigRules::SetStandardPredefinedVars();
		FWindowsConfigRules::Parse();
	}
#endif // WITH_APPLICATION_CORE && WITH_WINDOWS_CONFIGRULES
...
void FWindowsPlatformMisc::PlatformInit()
{
	...
	UE_LOGF(LogInit, Log, "Gaming Handheld Device: %d", FPlatformMisc::IsDeviceGamingHandheld() ? 1 : 0);
	...
	// Start listening for network connectivity changes via NLM events.
	StartNetworkConnectionMonitoring();
}

void FWindowsPlatformMisc::PlatformTearDown()
{
	// StopNetworkConnectionMonitoring is now called via FCoreDelegates::OnEnginePreExit as this function is called too late in the exit sequence
...
static BOOL WINAPI ConsoleCtrlHandler(DWORD CtrlType)
{
	...
	if (!IsRequestingExit)
	{
		UE_LOGF(LogCore, Warning, "*** INTERRUPTED *** : SHUTTING DOWN");
		UE_LOGF(LogCore, Warning, "*** INTERRUPTED *** : CTRL-C TO FORCE QUIT");
	}
...
void FWindowsPlatformMisc::SetEnvironmentVar(const TCHAR* VariableName, const TCHAR* Value)
{
	...
	if (Error == 0)
	{
		UE_LOGF(LogWindows, Warning, "Failed to set EnvironmentVariable: %ls to : %ls", VariableName, Value);
	}
}
...
static void HardKillIfAutomatedTesting()
{
	...
	if(( FApp::IsUnattended() == true ) && ( FromCommandLine != 0 ) && ( FParse::Param(FCommandLine::Get(), TEXT("KillAllPopUpBlockingWindows")) == true ))
	{
		...
		UE_LOGF(LogWindows, Warning, "Attempting to run KillAllPopUpBlockingWindows");
...
{
	UE_LOGF(LogWindows, Log,  "FPlatformMisc::RequestExit(%i, %ls)",
...
{
	UE_LOGF(LogWindows, Log, "FPlatformMisc::RequestExitWithStatus(%i, %i, %ls)", Force, ReturnCode,
	...
	if (Force)
	{
		...
		if (UE::Core::Private::CVarWindowsRaiseExceptionsOnFaultTermination->GetBool())
		{
			if (ReturnCode != 0)
			{
				RaiseException(1);
...
private:
	...
	EAppReturnType::Type Show(HWND ParentWindowHandle, int Template)
	{
		...
		if (!DialogHwnd)
		{
			...
			UE_LOGF(LogWindows, Error, "Failed to create dialog. %ls Error: 0x%X (%u)", ErrorBuffer, LastError, LastError);
			return Result;
		}
...
{
	if (UE::Core::Private::CVarDisableWindowsMessageBox.GetValueOnAnyThread() == true)
	{
		UE_LOGF(LogWindows, Warning, "FWindowsPlatformMisc::MessageBoxExt: A popup creation was auto-cancelled (%ls)", Caption);
		return EAppReturnType::Cancel;
	}
				...
				while (InfoPtr < BufferPtr + BufferBytes)
				{
					...
					if (ProcessorInfo->Relationship == RelationProcessorCore)
					{
							...
							for (int GroupIdx = 0; GroupIdx < ProcessorInfo->Processor.GroupCount; ++GroupIdx)
							{
								if (FilterGroupAffinity.Group == ProcessorInfo->Processor.GroupMask[GroupIdx].Group)
								{
									...
									if (Intersection > 0)
									{
										...
										OutLogicalProcessorCount += FMath::CountBits64(Intersection);
									}
								}
							}
						}
						else
						{
							...
							for (int GroupIdx = 0; GroupIdx < ProcessorInfo->Processor.GroupCount; ++GroupIdx)
							{
								OutLogicalProcessorCount += FMath::CountBits64(ProcessorInfo->Processor.GroupMask[GroupIdx].Mask);
							}
						}
					}
...
const TCHAR* FWindowsPlatformMisc::GetPlatformFeaturesModuleName()
{
	...
	{
		static FString PlatformFeaturesName = "WindowsPlatformFeatures";
		...
		{
			UE_LOGF(LogWindows, Log, "%ls enabled", *PlatformFeaturesName);
			return *PlatformFeaturesName;
		}
	}

	UE_LOGF(LogWindows, Log, "WindowsPlatformFeatures disabled or dedicated server build");
	return nullptr;
}
...
bool FWindowsPlatformMisc::SetStoredValue(const FString& InStoreId, const FString& InSectionName, const FString& InKeyName, const FString& InValue)
{
	...
	FString FullRegistryKey = FString("Software") / InStoreId / InSectionName;
...
public:
		...
		: bHasCPUIDInstruction(CheckForCPUIDInstruction()), Vendor(), CPUInfo(0), CacheLineSize(PLATFORM_CACHE_LINE_SIZE)
	{
		...
		if(bHasCPUIDInstruction)
		{
			...
			UE_CLOGF(bHasTimedPauseInstruction, LogWindows, Log, "Enabling Tpause support");
		}
	}
...
#pragma comment(lib, "tbs.lib")
		...
		if (void* hTbs = FPlatformProcess::GetDllHandle(TEXT("tbs.dll")))
		{
			...
			if (const pfnTbsiGetDeviceInfo Tbsi_GetDeviceInfo = reinterpret_cast<pfnTbsiGetDeviceInfo>(FPlatformProcess::GetDllExport(hTbs, TEXT("Tbsi_GetDeviceInfo"))))
			{
				...
				UE_LOGF(LogWindows, Warning, "Failed to retrieve TPM device information");
			}
			else
			{
				UE_LOGF(LogWindows, Warning, "Failed to load proc address for Tbsi_GetDeviceInfo");
			}
			FPlatformProcess::FreeDllHandle(hTbs);
		}
		else
		{
			UE_LOGF(LogWindows, Warning, "Failed to load tbs.dll");
		}
#endif
		return false;
	};
	...
	auto HasSecureBootSupportInternal = []() -> bool
	{
		if (void* hNtDll = FPlatformProcess::GetDllHandle(TEXT("ntdll.dll")))
		{
			...
			if (const pfnNtQuerySystemInformation fnNtQuerySystemInformation = reinterpret_cast<pfnNtQuerySystemInformation>(FPlatformProcess::GetDllExport(hNtDll, TEXT("NtQuerySystemInformation"))))
			{
				...
				UE_LOGF(LogWindows, Warning, "Failed to retrieve secure boot information");
			}
			else
			{
				UE_LOGF(LogWindows, Warning, "Failed to load proc address for NtQuerySystemInformation");
			}
			FPlatformProcess::FreeDllHandle(hNtDll);
		}
		else
		{
			UE_LOGF(LogWindows, Warning, "Failed to load ntdll.dll");
		}
		return false;
	};
...
static void GetVideoDriverDetailsFromSetup(const FString& DeviceName, bool bVerbose, FGPUDriverInfo& Out)
{
	...
	if (hDevInfo != INVALID_HANDLE_VALUE)
	{
		...
		for (int32 Idx = 0; SetupDiEnumDeviceInfo(hDevInfo, Idx, &DeviceInfoData); Idx++)
		{
				...
				(PBYTE)Buffer, sizeof(Buffer), nullptr, 0))
			{
				if (DeviceName.Compare(Buffer) == 0)
				{
					...
					{
						UE_CLOGF(bVerbose, LogWindows, Log, "Failed to retrieve driver registry key for device %d", Idx);
					}
			...
			{
				UE_CLOGF(bVerbose, LogWindows, Log, "Failed to retrieve driver description for device %d", Idx);
			}
		}
		...
		if (bFound)
		{
			...
			{
				UE_CLOGF(bVerbose, LogWindows, Log, "Failed to find provider name");
			}
			...
			{
				UE_CLOGF(bVerbose, LogWindows, Log, "Failed to find internal driver version");
			}
			// Get the driver date
			FILETIME FileTime;
			...
			{
				UE_CLOGF(bVerbose, LogWindows, Log, "Failed to find driver date");
			}
		}
		else
		{
			UE_CLOGF(bVerbose, LogWindows, Log, "Unable to find requested device '%ls' using Setup API.", *DeviceName);
		}
	...
	{
		UE_CLOGF(bVerbose, LogWindows, Log, "Failed to initialize Setup API");
	}
	...
	else if(Out.IsAMD() && !RegistryKey.IsEmpty())
	{
		...
		{
			Out.UserDriverVersion = FString("Catalyst ") + Out.UserDriverVersion;
		}
...
static BOOL CALLBACK MonitorEnumProc(HMONITOR Monitor, HDC MonitorDC, LPRECT Rect, LPARAM UserData)
{
	...
	*NumMonitors += 1;
	...
	UE_LOGF(LogWindows, Log, "    resolution: %dx%d, work area: (%d, %d) -> (%d, %d), device: '%ls'%ls",
...
FGPUDriverInfo FWindowsPlatformMisc::GetGPUDriverInfo(const FString& DeviceDescription, bool bVerbose)
{
	if (bVerbose)
	{
		...
		UE_LOGF(LogWindows, Log, "Attached monitors:");
		int NumMonitors = 0;
		EnumDisplayMonitors(nullptr, nullptr, MonitorEnumProc, (LPARAM)&NumMonitors);
		UE_LOGF(LogWindows, Log, "Found %d attached monitors.", NumMonitors);
	}
	...
	if (Method == 5)
	{
		UE_CLOGF(bVerbose, LogWindows, Log, "Gathering driver information using Windows Setup API");
		...
		UE_CLOGF(bVerbose, LogWindows, Log, "Failed to get driver data for device '%ls' using Setup API. Switching to fallback method.", *DeviceDescription);
		Method = 4; // Switch to method 4 as a fallback if method 5 fails
	}
	
	if(Method == 3 || Method == 4)
	{
		UE_CLOGF(bVerbose, LogWindows, Log, "EnumDisplayDevices:");
		...
		for(uint32 i = 0; i < 256; ++i)
		{
			...
			UE_CLOGF(bVerbose, LogWindows, Log, "   %d. '%ls' (P:%d D:%d), name: '%ls'",
				i,
				Device.DeviceString,
			...
			{
				DriverLocation = FString("\\HKEY_LOCAL_MACHINE\\") + DriverLocation.RightChop(18);
			}
		...
		{
			UE_CLOGF(bVerbose, LogWindows, Log, "DebugString: %ls", *DebugString);
		}
...
FTDRInfo FWindowsPlatformMisc::GetTDRInfo()
{
	FTDRInfo TDRInfo;
	TDRInfo.Level = ETDRLevel::Recover;
	TDRInfo.Delay = FTDRInfo::DefaultDelay;
	TDRInfo.DdiDelay = FTDRInfo::DefaultDdiDelay;

	const TCHAR* GraphicsDriverKey = TEXT("System\\CurrentControlSet\\Control\\GraphicsDrivers");

	FString TDRLevel;
	if (FWindowsPlatformMisc::QueryRegKey(HKEY_LOCAL_MACHINE, GraphicsDriverKey, TEXT("TdrLevel"), TDRLevel))
	{
		int32 Value = FCString::Atoi(*TDRLevel);
		TDRInfo.Level = (Value >= 0 && Value < int32(ETDRLevel::Invalid)) ? ETDRLevel(Value) : ETDRLevel::Invalid;
	}

	FString TDRDelay;
	if (FWindowsPlatformMisc::QueryRegKey(HKEY_LOCAL_MACHINE, GraphicsDriverKey, TEXT("TdrDelay"), TDRDelay))
	{
		LexFromString(TDRInfo.Delay, *TDRDelay);
	}

	FString TDRDdiDelay;
	if (FWindowsPlatformMisc::QueryRegKey(HKEY_LOCAL_MACHINE, GraphicsDriverKey, TEXT("TdrDdiDelay"), TDRDdiDelay))
	{
		LexFromString(TDRInfo.DdiDelay, *TDRDdiDelay);
	}
...
bool FWindowsPlatformMisc::IsDeviceGamingHandheld()
{
...
{
	WaitOnCollectStorageInformationAsync();
...
void FWindowsPlatformMisc::ApplyWindowsCompatConfigRules(void)
{
#if WITH_APPLICATION_CORE && WITH_WINDOWS_CONFIGRULES

	bool bSkipAllowList = false;
#if !UE_BUILD_SHIPPING
	bSkipAllowList = FParse::Param(FCommandLine::Get(), TEXT("SkipConfigRulesAllowList"));
#endif

	TArray<FString> WindowsCompatCVars;
	if (bSkipAllowList || GConfig->GetArray(TEXT("WindowsCompatCVars"), TEXT("CVars"), WindowsCompatCVars, GEngineIni))
	{
		TSet<FString> AllowedCompatCVars(WindowsCompatCVars);
		const TMap<FString, FString>& ConfigRuleVars = FPlatformMisc::GetConfigRuleVars();
		for (const TTuple<FString, FString>& Pair : ConfigRuleVars)
		{
			const FString& Key = Pair.Key;
			const FString& Value = Pair.Value;
			static const TCHAR WindowsCompat[] = TEXT("WindowsCompat.");
			if (Key.StartsWith(WindowsCompat))
			{
				FString CVarName = Key.Mid(UE_ARRAY_COUNT(WindowsCompat) - 1);
				if (bSkipAllowList || AllowedCompatCVars.Contains(CVarName))
				{
					...
					if (CVar)
					{
						...
						UE_LOGF(LogConfigRules, Log, "Compat Setting %ls = %ls", *CVarName, *Value);
					}
				}
			}
		}
...
const class TMap<FString, FString>& FWindowsPlatformMisc::GetConfigRuleVars()
{
#if WITH_APPLICATION_CORE && WITH_WINDOWS_CONFIGRULES
	return FWindowsConfigRules::GetConfigRulesMap();
#else 
	return FGenericPlatformMisc::GetConfigRuleVars();
#endif // WITH_APPLICATION_CORE && WITH_WINDOWS_CONFIGRULES
}

static INetworkListManager* GNetworkListManager = nullptr;
static bool GNetworkMonitoringInitializedCOM = false;

/** Cached network connection type, updated by NLM events and initial query.
  * Under the current STA apartment model, both the ConnectivityChanged callback and all reads are serialized on the game thread via window message marshaling. */
static ENetworkConnectionType GCachedNetworkConnectionType = ENetworkConnectionType::Unknown;

/**
 * Queries NLM network connections to determine if the active connection is WiFi or wired Ethernet
 */
static ENetworkConnectionType DetermineConnectionTypeFromNLM(INetworkListManager* NetworkListManager)
{
	// Get an enumerator over all NLM network connections
	IEnumNetworkConnections* EnumConnections = nullptr;
	HRESULT Result = NetworkListManager->GetNetworkConnections(&EnumConnections);
	if (FAILED(Result) || EnumConnections == nullptr)
	{
	...
	// Iterate each connection and check only those that are currently connected
	INetworkConnection* Connection = nullptr;
	while (EnumConnections->Next(1, &Connection, nullptr) == S_OK)
	{
		VARIANT_BOOL IsConnected = VARIANT_FALSE;
		if (SUCCEEDED(Connection->get_IsConnected(&IsConnected)) && IsConnected != VARIANT_FALSE)
		{
			// Get the adapter GUID that backs this NLM connection
			GUID AdapterId;
			if (SUCCEEDED(Connection->GetAdapterId(&AdapterId)))
			{
				// Convert adapter GUID -> LUID -> interface index.
				// We need the index because MIB_IFROW (used by GetIfEntry) is keyed by dwIndex
				
				NET_LUID Luid;
				NET_IFINDEX IfIndex;

				if (ConvertInterfaceGuidToLuid(&AdapterId, &Luid) == NO_ERROR &&
					ConvertInterfaceLuidToIndex(&Luid, &IfIndex) == NO_ERROR)
				{
					// Query the interface row to read the physical adapter type
					...
					if (GetIfEntry(&IfRow) == NO_ERROR)
					{
						...
						else if (IfRow.dwType == IF_TYPE_ETHERNET_CSMACD)
						{
							bHasEthernet = true;
						}
					}
	...
	EnumConnections->Release();

	// When both WiFi and Ethernet are active (e.g. docked laptop), prefer Ethernet, as the wired connection is typically the primary route
	...
	// Fallback: no recognized adapter type was found (e.g. VPN or cellular)
	return ENetworkConnectionType::Unknown;
}

/**
 * Queries NLM for the current network connection type. Expensive as it involves COM calls and interface enumeration.
 * Should only be called during init and from NLM event callbacks, not per-frame.
 * The result is stored in GCachedNetworkConnectionType.
 */
static ENetworkConnectionType QueryNetworkConnectionTypeFromNLM()
{
	if (GNetworkListManager == nullptr)
	{
		return ENetworkConnectionType::Unknown;
	}

	NLM_CONNECTIVITY Connectivity;
	HRESULT GetConnectivityResult = GNetworkListManager->GetConnectivity(&Connectivity);
	if (SUCCEEDED(GetConnectivityResult))
	{
		...
		{
			return DetermineConnectionTypeFromNLM(GNetworkListManager);
		}
		else
		{
...
/**
 * COM event listener that receives network connectivity change notifications from NLM.
 * Registered in StartNetworkConnectionMonitoring, unregistered in StopNetworkConnectionMonitoring.
 */
class FNLMEventListener : public INetworkListManagerEvents
{
public:
	FNLMEventListener() : RefCount(1), Cookie(0), ConnectionPoint(nullptr) {}
	virtual ~FNLMEventListener() = default;
	...
	{
		// We use the PlatformAtomics methods to avoid having to add this to AllowWindowsPlatformTypes
	...
	{
		// We ignore the raw NLM_CONNECTIVITY bitmask and re-query NLM to also resolve adapter types.
		const ENetworkConnectionType ConnectionType = QueryNetworkConnectionTypeFromNLM();
		const ENetworkConnectionType PreviousType = GCachedNetworkConnectionType;
		GCachedNetworkConnectionType = ConnectionType;

		// NLM can fire ConnectivityChanged for events that don't affect the resolved type (e.g. a VPN connecting while Ethernet stays primary). Skip redundant broadcasts.
		if (ConnectionType == PreviousType)
		{
			return S_OK;
		}

		UE_LOGF(LogInit, Verbose, "Network connectivity changed (ConnectionType=%d)", static_cast<int32>(ConnectionType));
		...
		{
			// Task graph not running yet (early startup). Set the status synchronously.
			FPlatformMisc::SetNetworkConnectionStatus(FPlatformMisc::NetworkConnectionTypeToStatus(ConnectionType));
		}
		return S_OK;
	}
	...
	/** Subscribe to NLM connectivity events. Returns true on success. */
	bool Advise(INetworkListManager* NetworkListManager)
	{
		IConnectionPointContainer* Container = nullptr;
		HRESULT Result = NetworkListManager->QueryInterface(IID_IConnectionPointContainer, (void**)&Container);
		if (FAILED(Result) || Container == nullptr)
		{
			return false;
		}

		Result = Container->FindConnectionPoint(IID_INetworkListManagerEvents, &ConnectionPoint);
		Container->Release();
		if (FAILED(Result) || ConnectionPoint == nullptr)
		{
			return false;
		}
	...
	/** Unsubscribe from NLM connectivity events. */
	void Unadvise()
	{
...
private:
	// Used by the IUnknown interface, type required by InterlockedIncrement/Decrement methods
	volatile int32 RefCount;

	// Token returned by Advise which identifies our registration
	DWORD Cookie;

	// Intermediary between the event source (INetworkListManager) and this listener
	IConnectionPoint* ConnectionPoint;
};

static FNLMEventListener* GNLMEventListener = nullptr;

ENetworkConnectionType FWindowsPlatformMisc::GetNetworkConnectionType()
{
	return GCachedNetworkConnectionType;
}

void FWindowsPlatformMisc::StartNetworkConnectionMonitoring()
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FWindowsPlatformMisc::StartNetworkConnectionMonitoring);

	check(GNetworkListManager == nullptr && GNLMEventListener == nullptr);

	// COM must be initialized on this thread before we can create COM objects.
	// This defaults to a single-threaded apartment (COINIT_APARTMENTTHREADED), which means NLM callbacks are marshaled to this thread via window messages.
	// Callbacks will only fire when the game thread pumps messages (FWindowsPlatformApplicationMisc::PumpMessages), so there may be latency under heavy game-thread load.
	// This also guarantees that callbacks and teardown are serialized on the same thread, avoiding races.
	if (!FWindowsPlatformMisc::CoInitialize())
	{
		UE_LOGF(LogInit, Log, "Failed to initialize COM for network monitoring");
		return;
	}
	
	GNetworkMonitoringInitializedCOM = true;

	HRESULT CoCreateInstanceResult = CoCreateInstance(CLSID_NetworkListManager, nullptr, CLSCTX_ALL, IID_INetworkListManager, (void**)&GNetworkListManager);
	if (FAILED(CoCreateInstanceResult) || GNetworkListManager == nullptr)
	{
		UE_LOGF(LogInit, Log, "Failed to create INetworkListManager for network monitoring (HRESULT=0x%08X)", CoCreateInstanceResult);
		GNetworkListManager = nullptr;
		FWindowsPlatformMisc::CoUninitialize();
		GNetworkMonitoringInitializedCOM = false;
		return;
	}

	GNLMEventListener = new FNLMEventListener();
	if (!GNLMEventListener->Advise(GNetworkListManager))
	{
		UE_LOGF(LogInit, Log, "Failed to subscribe to INetworkListManagerEvents");
		GNLMEventListener->Release();
		GNLMEventListener = nullptr;
		GNetworkListManager->Release();
		GNetworkListManager = nullptr;
		FWindowsPlatformMisc::CoUninitialize();
		GNetworkMonitoringInitializedCOM = false;
		return;
	...
	const ENetworkConnectionType InitialConnectionType = QueryNetworkConnectionTypeFromNLM();
	GCachedNetworkConnectionType = InitialConnectionType;
	SetNetworkConnectionStatus(NetworkConnectionTypeToStatus(InitialConnectionType));

	// Register cleanup on OnEnginePreExit - this fires before OnPreExit and OnExit.
	// We must clean it up earlier than PlatformTearDown because its callbacks can crash at that point
	FCoreDelegates::OnEnginePreExit.AddStatic(StopNetworkConnectionMonitoring);
}

void FWindowsPlatformMisc::StopNetworkConnectionMonitoring()
{
	// Safe to access GNetworkListManager without a lock here: under the single-threaded COM apartment (COINIT_APARTMENTTHREADED), both the ConnectivityChanged callback and this teardown run on the same thread (the game thread), so they are serialized.
	// If the apartment model is ever changed to multithreaded (COINIT_MULTITHREADED), a mutex or ref-counting guard would be needed.
	if (GNLMEventListener != nullptr)
	{
		GNLMEventListener->Unadvise();
		GNLMEventListener->Release();
		GNLMEventListener = nullptr;
	}
	...
	if (GNetworkMonitoringInitializedCOM)
	{
		FWindowsPlatformMisc::CoUninitialize();
		GNetworkMonitoringInitializedCOM = false;
	}
