...
void FWindowsErrorOutputDevice::Serialize( const TCHAR* Msg, ELogVerbosity::Type Verbosity, const class FName& Category )
{
	...
	{   
		...
		if (LastError == 0)
		{
			UE_LOGF(LogWindows, Log, "Windows GetLastError: %ls (%i)", FPlatformMisc::GetSystemErrorMessage(ErrorBuffer, 1024, LastError), LastError);
		}
		else
		{
			UE_LOGF(LogWindows, Error, "Windows GetLastError: %ls (%i)", FPlatformMisc::GetSystemErrorMessage(ErrorBuffer, 1024, LastError), LastError);
		}
	...
	{
		UE_LOGF(LogWindows, Error, "Error reentered: %ls", Msg );
	}
...
void FWindowsErrorOutputDevice::HandleError()
{
	...
	if (NewCallCount != 1)
	{
		UE_LOGF(LogWindows, Error, "HandleError re-entered." );
		return;
	}
