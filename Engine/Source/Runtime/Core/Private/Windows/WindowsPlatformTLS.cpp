...
namespace WindowsPlatformTLS_Private
{
	...
	public:
		...
		bool TryAlloc(uint32& OutIndex)
		{
			if (!Queue.try_pop(OutIndex))
			{
				return false;
			}
			OutIndex -= 1;
...
{
	uint32 Result;
	if (WindowsPlatformTLS_Private::FSingleton::Get().Slots.TryAlloc(Result))
	{
		return Result;
	}
	UE_LOGFMT(LogCore, Fatal, "Maximum number of TlsSlots (WINDOWS_MAX_NUM_TLS_SLOTS == {MaxSlots}) exceeded when calling AllocTlsSlot.",
		WindowsPlatformTLS_Private::MaxSlots);
}
