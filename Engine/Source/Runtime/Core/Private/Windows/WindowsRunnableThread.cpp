...
int FRunnableThreadWin::TranslateThreadPriority(EThreadPriority Priority)
{
	...
	default: UE_LOGF(LogHAL, Fatal, "Unknown Priority passed to TranslateThreadPriority()"); return THREAD_PRIORITY_NORMAL;
	}
}
...
uint32 FRunnableThreadWin::GuardedRun()
{
	...
	// Initialize random number generator. srand() is thread-local in windows.
	...
	else
	{
		...
		__except (FPlatformMisc::GetCrashHandlingType() == ECrashHandlingType::Default ? ReportCrash(GetExceptionInformation()) : EXCEPTION_CONTINUE_SEARCH)
		{
			__try
			{
				...
				UE_LOGF( LogThreadingWindows, Error, "Runnable thread %ls crashed.", *ThreadName );
				GWarn->Flush();
			...
			__except(EXCEPTION_EXECUTE_HANDLER)
			{
				...
				TerminateProcess(GetCurrentProcess(), (UINT)ECrashExitCodes::CrashHandlerCrashed);
			}
		}
#endif // !PLATFORM_SEH_EXCEPTIONS_DISABLED
	}
...
bool FRunnableThreadWin::SetThreadAffinity(const FThreadAffinity& Affinity)
{
	...
	if (SetThreadGroupAffinity(Thread, &GroupAffinity, &PreviousGroupAffinity) == 0)
	{
		...
		UE_LOGF( LogThreadingWindows, Warning, "Runnable thread %ls call to SetThreadAffinity failed with: 0x%x", *ThreadName, LastError);
		return  false;
	}
