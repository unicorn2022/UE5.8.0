...
void FWindowsPlatformCrashContext::CopyOptionalAttachments(const TCHAR* BaseOutputDirectory) const
{
	...
	{
		UE_LOGF(LogWindows, Error, "Error creating crash report's optional attachments directory %ls", *OptionalAttachmentsDirectory);
		return;
	}
	...
	for (const FString& OptionalAttachmentFilepath : OptionalAttachmentFilepaths)
	{
		...
		{
			UE_LOGF(LogWindows, Error, "Error moving crash report's optional attachment file %ls to output directory", *OptionalAttachmentFilepath);
		}
	}
...
void FWindowsPlatformCrashContext::WriteOptionalAttachmentsXML(const FString& Filepath) const
{
	...
	{
		UE_LOGF(LogWindows, Error, "Error writing optional attachments XML file %ls", *Filepath);
	}
}
...
void FWindowsPlatformCrashContext::AddPlatformSpecificProperties() const
{
	...
	AddCrashProperty(TEXT("HostArchitecture"), FWindowsPlatformMisc::GetHostArchitecture());
	AddCrashProperty(TEXT("IsDeviceHandheld"), FPlatformMisc::IsDeviceGamingHandheld());
...
void FWindowsPlatformCrashContext::CopyPlatformSpecificFiles(const TCHAR* OutputDirectory, void* Context)
{
	...
	auto GetRecordCrashDump = [this](const TCHAR* Key)
		{
			bool bRecordCrashDump = true;

			static const TCHAR* SectionName = TEXT("CrashReportClient");
			...
			if (IFileManager::Get().FileExists(CrashConfigFilePath))
			{
				...
				CrashConfigFile.Read(CrashConfigFilePath);
				if (CrashConfigFile.GetBool(SectionName, Key, bRecordCrashDump))
				{
					return bRecordCrashDump;
				}
			}

#if UE_EDITOR
			if (GConfig->GetBool(SectionName, Key, bRecordCrashDump, GEditorIni))
			{
				return bRecordCrashDump;
			}
#endif

			GConfig->GetBool(SectionName, Key, bRecordCrashDump, GEngineIni);
			return bRecordCrashDump;
		};
	...
	switch (Type)
	{
		case ECrashContextType::Stall:
			bRecordCrashDump = GetRecordCrashDump(TEXT("Stall.RecordDump"));
			break;
		case ECrashContextType::Ensure:
			bRecordCrashDump = GetRecordCrashDump(TEXT("Ensure.RecordDump"));
			break;
...
FProcHandle LaunchCrashReportClient(void** OutWritePipe, void** OutReadPipe, uint32* OutCrashReportClientProcessId)
{
	...
	*OutReadPipe = PipeChildOutRead;
	...
	if (CreateCrashReportClientPath(CrashReporterClientPath, CR_CLIENT_MAX_PATH_LEN))
	{
		...
		UE_LOGF(LogWindows, Log, "Started CrashReportClient (pid=%d)", std::underlying_type_t<UE::HAL::EProcessId>(ProcessId));
 ...
 */
void GuardedDumpAdditionalContext(const TCHAR* CrashDirectoryAbsolute, const FAdditionalCrashContextParams& Params)
{
#if !PLATFORM_SEH_EXCEPTIONS_DISABLED
	__try
#endif
	{
		FGenericCrashContext::DumpAdditionalContext(CrashDirectoryAbsolute, Params);
	}
#if !PLATFORM_SEH_EXCEPTIONS_DISABLED
	__except (TRUE)
	{
	...
	EErrorReportUI ReportUI)
{
	...
	if (FGenericCrashContext::CreateCrashReportDirectory(SharedContext->SessionContext.CrashGUIDRoot, ReportCallCount++, CrashDirectoryAbsolute))
	{
		...
		// Generate some files that needs to be done in the crashing process
		FGenericCrashContext::GenerateFiles(*CrashDirectoryAbsolute);

		// Dump additional context
		FAdditionalCrashContextParams Params =
		{ 
			.CrashedThreadId = SharedContext->CrashingThreadId
		};
		GuardedDumpAdditionalContext(*CrashDirectoryAbsolute, Params);
	}
...
int32 ReportCrashUsingCrashReportClient(FWindowsPlatformCrashContext& InContext, EXCEPTION_POINTERS* ExceptionInfo, EErrorReportUI ReportUI)
{
	...
	if( bCanRunCrashReportClient )
	{
		...
		if (IFileManager::Get().MakeDirectory(*CrashFolderAbsolute, true))
		{
			...
			// Generate some files that needs to be done in the crashing process
			FGenericCrashContext::GenerateFiles(*CrashFolderAbsolute);

			// Generate and copy platform specific files (e.g. minidump) to output directory
			InContext.CopyPlatformSpecificFiles(*CrashFolderAbsolute, (void*) ExceptionInfo);

			// Dump additional context
			FAdditionalCrashContextParams Params =
			{
				.CrashedThreadId = InContext.GetCrashedThreadId()
			};
			GuardedDumpAdditionalContext(*CrashFolderAbsolute, Params);
			...
			CrashReportClientArguments += FString(" -AppName=") + AppName;
			CrashReportClientArguments += FString(" -CrashGUID=") + CrashGUID;

			const FString DownstreamStorage = FWindowsPlatformStackWalk::GetDownstreamStorage();
			if (!DownstreamStorage.IsEmpty())
			{
				CrashReportClientArguments += FString(" -DebugSymbols=") + DownstreamStorage;
			}
		...
		{
			UE_LOGF(LogWindows, Log, "Could not start crash report client using %ls", CrashReporterClientPath);
...
private:
	...
	FORCENOINLINE uint32 Run()
	{
...
#endif
		{
			FPlatformProcess::SetThreadName(TEXT("CrashReportingThread"));
		...
		__except(EXCEPTION_EXECUTE_HANDLER)
		{
			...
			TerminateProcess(GetCurrentProcess(), (UINT)ECrashExitCodes::CrashReporterCrashed);
		}
#endif
		return 0;
	}
...
public:
	...
	void RegisterAbortSignalHandler()
	{
		...
		{
			UE_LOGF(LogWindows, Log, "Custom abort handler registered for crash reporting.");
		}
		else
		{
			UE_LOGF(LogWindows, Error, "Unable to register custom abort handler for crash reporting.");
		}
#endif
	}
#endif
