...
bool TTransform<T>::DebugEqualMatrix(const TMatrix<T>& Matrix) const
{
	...
	if (!Equals(TestResult))
	{
		...
		{
			UE_LOGF(LogTransform, Log, "Matrix(S)\t%ls", *TestResult.Scale3D.ToString());
			UE_LOGF(LogTransform, Log, "VQS(S)\t%ls", *Scale3D.ToString());
		}

		// see now which one isn't equal
		if (!Rotation.Equals(TestResult.Rotation))
		{
			UE_LOGF(LogTransform, Log, "Matrix(R)\t%ls", *TestResult.Rotation.ToString());
			UE_LOGF(LogTransform, Log, "VQS(R)\t%ls", *Rotation.ToString());
		}

		// see now which one isn't equal
		if (!Translation.Equals(TestResult.Translation, 0.01f))
		{
			UE_LOGF(LogTransform, Log, "Matrix(T)\t%ls", *TestResult.Translation.ToString());
			UE_LOGF(LogTransform, Log, "VQS(T)\t%ls", *Translation.ToString());
		}
		return false;
	}
