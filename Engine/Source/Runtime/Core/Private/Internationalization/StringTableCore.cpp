...
FStringTableEntry::FStringTableEntry(FStringTableConstRef InOwnerTable, FString InSourceString, FTextId InDisplayStringId, FString InDevNotes)
#else
...
{
	static const FString MissingSourceString = "<MISSING STRING TABLE ENTRY>";
...
#if WITH_EDITORONLY_DATA
			KeyToEntryPair.Value = FStringTableEntry::NewStringTableEntry(AsShared(), OldEntry->GetSourceString(), FTextId(TableNamespace, KeyToEntryPair.Key), OldEntry->GetDevNotes());
#else
...
#if WITH_EDITORONLY_DATA
void FStringTable::SetSourceString(const FTextKey& InKey, const FString& InSourceString, const FString& InDevNotes)
#else
...
#if WITH_EDITORONLY_DATA
	TableEntry = FStringTableEntry::NewStringTableEntry(AsShared(), InSourceString, FTextId(TableNamespace, InKey), InDevNotes);
#else
...
#if WITH_EDITORONLY_DATA
void FStringTable::EnumerateSourceStrings(const TFunctionRef<bool(const FString&, const FString&, const FString&)>& InEnumerator) const
{
	EnumerateKeysAndSourceStrings([&InEnumerator](const FTextKey& InKey, const FString& InSourceString, const FString& InDevNotes) -> bool
	{
		return InEnumerator(InKey.ToString(), InSourceString, InDevNotes);
	});
}

void FStringTable::EnumerateKeysAndSourceStrings(const TFunctionRef<bool(const FTextKey&, const FString&, const FString&)>& InEnumerator) const
{
	UE::TScopeLock KeyMappingLock(KeyMappingCS);

	for (const auto& KeyToEntryPair : KeysToEntries)
	{
		if (!InEnumerator(KeyToEntryPair.Key, KeyToEntryPair.Value->GetSourceString(), KeyToEntryPair.Value->GetDevNotes()))
		{
			break;
		}
	}
	...
	if (Ar.IsSaving())
	{
		// Save entries
		{
			...
			for (const auto& KeyToEntryPair : KeysToEntries)
			{
				...
				{
					FString DevNotes = KeyToEntryPair.Value->GetDevNotes();
					Ar << DevNotes;
				}
#endif
			}
...
#if WITH_EDITORONLY_DATA
				FString DevNotes;
				if (Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) >= FFortniteMainBranchObjectVersion::AddDevNotesToFText && !Ar.IsFilterEditorOnly())
				{
					Ar << DevNotes;
				}
				
				FStringTableEntryRef TableEntry = FStringTableEntry::NewStringTableEntry(AsShared(), SourceString, FTextId(TableNamespace, Key), DevNotes);
#else
...
{
	return ExportStringsToCSVFile(InFilename);
}

FString FStringTable::ExportStringsToCSVString() const
{
...
#if WITH_EDITORONLY_DATA
			FString ExportedDevNotes = KeyToEntryPair.Value->GetDevNotes().ReplaceCharWithEscapedChar();
			ExportedDevNotes.ReplaceInline(TEXT("\""), TEXT("\"\""));
			
			ExportedStrings += TEXT(",");

			ExportedStrings += TEXT("\"");
			ExportedStrings += ExportedDevNotes;
			ExportedStrings += TEXT("\"");
#endif
...
bool FStringTable::ExportStringsToCSVFile(const FString& InFilename) const
{
	FString ExportedStrings = ExportStringsToCSVString();
	return FFileHelper::SaveStringToFile(ExportedStrings, *InFilename);
}

bool FStringTable::ImportStrings(const FString& InFilename)
{
	return ImportStringsFromCSVFile(InFilename);
}

bool FStringTable::ImportStringsFromCSVString(const FString& InCSVString, const TCHAR* InErrorContext)
{
	const FCsvParser ImportedStringsParser(InCSVString);
	const FCsvParser::FRows& Rows = ImportedStringsParser.GetRows();

	// Must have at least 2 rows (header and content)
	if (Rows.Num() <= 1)
	{
		UE_LOGF(LogStringTable, Warning, "Failed to import string table from '%ls'. Incorrect number of rows (must be at least 2).", InErrorContext);
		return false;
	}
	...
	// Validate header
	{
...
#if WITH_EDITORONLY_DATA
			else if(FCString::Stricmp(Cell, TEXT("DevNotes")) == 0 && DevNotesColumn == INDEX_NONE)
			{
				DevNotesColumn = CellIdx;
			}
#endif
			else
			{
		...
		{
			bValidHeader = false;
			UE_LOGF(LogStringTable, Warning, "Failed to import string table from '%ls'. Failed to find required column 'Key'.", InErrorContext);
		}
		if (SourceStringColumn == INDEX_NONE)
		{
			bValidHeader = false;
			UE_LOGF(LogStringTable, Warning, "Failed to import string table from '%ls'. Failed to find required column 'SourceString'.", InErrorContext);
		}
		if (!bValidHeader)
		{
			return false;
...
#if WITH_EDITORONLY_DATA
				FString DevNotes = Cells.IsValidIndex(DevNotesColumn) ? Cells[DevNotesColumn] : TEXT("");
				DevNotes = DevNotes.ReplaceEscapedCharWithChar();
				
				FStringTableEntryRef TableEntry = FStringTableEntry::NewStringTableEntry(AsShared(), SourceString, FTextId(TableNamespace, Key), MoveTemp(DevNotes));
#else
				FStringTableEntryRef TableEntry = FStringTableEntry::NewStringTableEntry(AsShared(), SourceString, FTextId(TableNamespace, Key));
#endif
				KeysToEntries.Emplace(Key, TableEntry);
...
bool FStringTable::ImportStringsFromCSVFile(const FString& InFilename)
{
	FString ImportedStrings;
	if (!FFileHelper::LoadFileToString(ImportedStrings, *InFilename))
	{
		UE_LOGF(LogStringTable, Warning, "Failed to import string table from '%ls'. Could not open file.", *InFilename);
		return false;
	}
	return ImportStringsFromCSVString(ImportedStrings, *InFilename);
}
...
void FStringTableRedirects::InitStringTableRedirects()
{
	...
	if (CoreStringTableSection)
	{
		...
		{
			static const FName StringTableRedirectsName = "StringTableRedirects";
			if (It.Key() == StringTableRedirectsName)
			{
				...
				if (FParse::Value(*ConfigValue, TEXT("OldStringTable="), OldStringTable))
				{
					...
					UE_CLOGF(NewStringTable.IsNone(), LogStringTable, Warning, "Failed to parse string table redirect '%ls'. Missing or empty 'NewStringTable'.", *ConfigValue);
				...
				else if (FParse::Value(*ConfigValue, TEXT("StringTable="), OldStringTable))
				{
					...
					UE_CLOGF(OldKey.IsEmpty(), LogStringTable, Warning, "Failed to parse string table redirect '%ls'. Missing or empty 'OldKey'.", *ConfigValue);

					FParse::Value(*ConfigValue, TEXT("NewKey="), NewKey);
					UE_CLOGF(NewKey.IsEmpty(), LogStringTable, Warning, "Failed to parse string table redirect '%ls'. Missing or empty 'NewKey'.", *ConfigValue);
				...
				{
					UE_LOGF(LogStringTable, Warning, "Failed to parse string table redirect '%ls'. Expected 'OldStringTable' and 'NewStringTable' for a table ID redirect, or 'StringTable', 'OldKey', 'NewKey' for a key redirect.", *ConfigValue);
				}
			}
		}
	}
