...
static FAutoConsoleCommand CmdDumpLiveTable(
	...
	FConsoleCommandWithArgsDelegate::CreateStatic([](const TArray<FString>& Args)
	{
		...
		{
			UE_LOGF(LogLocalization, Warning, "Unknown argument passed to Localization.DumpLiveTable!");
		}
		...
		{
			UE_LOGF(LogLocalization, Display, "No arguments provided, this would dump every string. Consider dumping to a file instead, or providing filter(s) for Namespace, Key, or DisplayString");
			return;
		}
...
static FAutoConsoleCommand SetDisplayString(
	...
	FConsoleCommandWithArgsDelegate::CreateStatic([](const TArray<FString>& Args)
	{
		...
		{
			UE_LOGF(LogLocalization, Display, "Missing argument(s): Key and/or DisplayString");
			return;
		}
		// An empty DisplayString is allowed, but the argument for it must be provided: -DisplayString=""
		if (Key.GetValue().IsEmpty())
		{
			UE_LOGF(LogLocalization, Display, "Empty argument: Key");
			return;
		}
...
FString ConditionDisplayStringForLog(const FString& DisplayString)
{
	static const TArray<TCHAR> CharsToReplace = { TEXT('\n'), TEXT('\r'), TEXT('\t'), TEXT('"'), TEXT('\\') };
	return DisplayString.ReplaceCharWithEscapedChar(&CharsToReplace);
}
...
void ApplyDefaultCultureSettings(const ELocalizationLoadFlags LocLoadFlags)
{
	...
	auto LogCultureOverride = [](const TCHAR* InResult, const TCHAR* InOptionDisplayName, const ERequestedCultureOverrideLevel InOverrideLevel)
	{
		...
		case ERequestedCultureOverrideLevel::CommandLine:
			UE_LOGF(LogInit, Log, "Overriding %ls with command-line option (%ls).", InOptionDisplayName, InResult);
			break;
		case ERequestedCultureOverrideLevel::EditorSettings:
			UE_LOGF(LogInit, Log, "Overriding language with editor %ls configuration option (%ls).", InOptionDisplayName, InResult);
			break;
		case ERequestedCultureOverrideLevel::GameUserSettings:
			UE_LOGF(LogInit, Log, "Overriding language with game user settings %ls configuration option (%ls).", InOptionDisplayName, InResult);
			break;
		case ERequestedCultureOverrideLevel::GameSettings:
			UE_LOGF(LogInit, Log, "Overriding language with game %ls configuration option (%ls).", InOptionDisplayName, InResult);
			break;
		case ERequestedCultureOverrideLevel::EngineSettings:
			UE_LOGF(LogInit, Log, "Overriding language with engine %ls configuration option (%ls).", InOptionDisplayName, InResult);
			break;
		case ERequestedCultureOverrideLevel::Defaults:
			UE_LOGF(LogInit, Log, "Using OS detected %ls (%ls).", InOptionDisplayName, InResult);
			break;
		}
	};
...
#endif
		{
			...
			if (!ValidCultureName.IsEmpty())
			{
				if (bRequireExactMatch && InRequestedCulture != ValidCultureName)
				{
					...
					UE_LOGF(LogTextLocalizationManager, Log, "No specific localization for '%ls' exists, so '%ls' will be used for the %ls.", *InRequestedCulture, *TargetCultureName, InLogDesc);
				}
			}
			else if (!ValidFallbackCultureName.IsEmpty())
			{
				TargetCultureName = ValidFallbackCultureName;
				UE_LOGF(LogTextLocalizationManager, Log, "No localization for '%ls' exists, so '%ls' will be used for the %ls.", *InRequestedCulture, *TargetCultureName, InLogDesc);
			}
			else
			{
...
void InitEngineTextLocalization()
{
	...
	FTextLocalizationManager::Get().CacheLocalizedLanguageName(LocLoadFlags);
...
void FTextLocalizationManager::CompactDataStructures()
{
	...
	UE_LOGF(LogTextLocalizationManager, Log, "Compacting localization data took %6.2fms", 1000.0 * (FPlatformTime::Seconds() - StartTime));
}
...
void FTextLocalizationManager::DumpLiveTable(const FString* NamespaceFilter, const FString* KeyFilter, const FString* DisplayStringFilter, const FLogCategoryBase* CategoryOverride) const
{
	...
	{
		UE_LOG_REF(Category, Display, TEXT("LiveTableEntry: Namespace: '%s', Key: '%s', DisplayString: '%s'"), *Id.GetNamespace().ToString(), *Id.GetKey().ToString(), *TextLocalizationManager::ConditionDisplayStringForLog(*DisplayString));
	});
...
void FTextLocalizationManager::DumpLiveTable(const FString& OutputFilename, const FString* NamespaceFilter, const FString* KeyFilter, const FString* DisplayStringFilter) const
{
	...
	{
		DumpString += FString::Printf(TEXT("LiveTableEntry: Namespace: '%s', Key: '%s', DisplayString: '%s'"), *Id.GetNamespace().ToString(), *Id.GetKey().ToString(), *TextLocalizationManager::ConditionDisplayStringForLog(*DisplayString));
...
void FTextLocalizationManager::AddOrUpdateDisplayStringInLiveTable(const FString& Namespace, const FString& Key, const FString& DisplayString, const FString* const SourceStringPtr)
{
	...
	if (FDisplayStringEntry* LiveEntry = DisplayStringLookupTable.Find(TextId))
	{
		if (SourceStringPtr)
		{
			LiveEntry->SourceStringHash = FTextLocalizationResource::HashString(*SourceStringPtr);
		}
		LiveEntry->DisplayString = MakeTextDisplayString(CopyTemp(DisplayString));
		DirtyLocalRevisionForTextId(TextId);

		UE_LOGF(LogConsoleResponse, Display, "Updated string for Namespace='%ls', Key='%ls' to DisplayString='%ls'", *Namespace, *Key, *TextLocalizationManager::ConditionDisplayStringForLog(DisplayString));
	}
	else if (SourceStringPtr)
	{
		...
		UE_LOGF(LogConsoleResponse, Display, "Added string for Namespace='%ls', Key='%ls' to DisplayString='%ls'", *Namespace, *Key, *TextLocalizationManager::ConditionDisplayStringForLog(DisplayString));
	}
	else
	{
		UE_LOGF(LogConsoleResponse, Display, "String not found for Namespace='%ls', Key='%ls'", *Namespace, *Key);
	}
}
...
FString FTextLocalizationManager::GetLocalizedLanguageName() const
{
...
void FTextLocalizationManager::RefreshResources(TUniqueFunction<void()>&& Notification)
{
	...
	CacheLocalizedLanguageName(LocLoadFlags);
...
void FTextLocalizationManager::LoadLocalizationResourcesForPrioritizedCultures_Async(TArrayView<const FString> PrioritizedCultureNames, const ELocalizationLoadFlags LocLoadFlags, TUniqueFunction<void()>&& Notification)
{
	QueueAsyncTask([AvailableTextSources = LocalizedTextSources, PrioritizedCultureNames = TArray<FString>(PrioritizedCultureNames), LocLoadFlags, Notification = MoveTemp(Notification)]()
	{
...
void FTextLocalizationManager::LoadLocalizationTargetsForPrioritizedCultures_Sync(TArrayView<const TSharedPtr<ILocalizedTextSource>> AvailableTextSources, TArrayView<const FString> LocalizationTargetPaths, TArrayView<const FString> PrioritizedCultureNames, const ELocalizationLoadFlags LocLoadFlags)
{
	...
	for (const FString& LocalizationTargetPath : LocalizationTargetPaths)
	{
		UE_LOGF(LogTextLocalizationManager, Verbose, "Loading LocRes data from '%ls'", *LocalizationTargetPath);
	}
	{
		LLM_SCOPE_BYNAME(TEXT("Localization/DisplayStrings"));
	...
	if (bNeedsFullRefresh)
	{
		UE_LOGF(LogTextLocalizationManager, Verbose, "Patching LocRes data failed, performing full refresh");
	...
	else
	{
		...
		UE_LOGF(LogTextLocalizationManager, Verbose, "Patching LocRes data for %d entries", TextLocalizationResource.Entries.Num());
		UpdateLiveTable(MoveTemp(TextLocalizationResource), UpdateOptions);
	}
}
...
void FTextLocalizationManager::LoadChunkedLocalizationResources_Sync(TArrayView<const TSharedPtr<ILocalizedTextSource>> AvailableTextSources, const int32 ChunkId, const FString& PakFilename)
{
	...
	UE_LOGF(LogTextLocalizationManager, Verbose, "Request to load localization data for chunk %d (from PAK '%ls')", ChunkId, *PakFilename);
	...
	{
		UE_LOGF(LogTextLocalizationManager, Verbose, "Skipped loading localization data for chunk %d (from PAK '%ls') as this chunk has already been processed", ChunkId, *PakFilename);
		return;
	}
	...
	if (!FTextLocalizationManager::IsDisplayStringSupportEnabled())
	{
		...
		UE_LOGF(LogTextLocalizationManager, Verbose, "Skipped loading localization data for chunk %d (from PAK '%ls') as display strings are disabled", ChunkId, *PakFilename);
		return;
	}
	...
	if (!IsInitialized())
	{
		...
		UE_LOGF(LogTextLocalizationManager, Verbose, "Skipped loading localization data for chunk %d (from PAK '%ls') as the localization manager isn't ready", ChunkId, *PakFilename);
		return;
	}
	...
	for (const FString& LocalizationTarget : ChunkedLocalizationTargets)
	{
		...
		{
			UE_LOGF(LogTextLocalizationManager, Verbose, "Skipped loading localization data for chunk %d (from PAK '%ls') as the localization directory for '%ls' was not yet available", ChunkId, *PakFilename, *ChunkedLocalizationTargetName);
			return;
		}
		...
		FTextLocalizationMetaDataResource LocMetaResource;
		{
			...
			{
				UE_LOGF(LogTextLocalizationManager, Verbose, "Skipped loading localization data for chunk %d (from PAK '%ls') as the LocMeta file for '%ls' was not yet available", ChunkId, *PakFilename, *ChunkedLocalizationTargetName);
				return;
			}
			if (!LocMetaResource.LoadFromFile(LocMetaFilename))
			{
				UE_LOGF(LogTextLocalizationManager, Verbose, "Skipped loading localization data for chunk %d (from PAK '%ls') as the LocMeta file for '%ls' failed to load", ChunkId, *PakFilename, *ChunkedLocalizationTargetName);
				return;
			}
		}
		...
		for (const FString& CompiledCulture : LocMetaResource.CompiledCultures)
		{
			...
			{
				UE_LOGF(LogTextLocalizationManager, Verbose, "Skipped loading localization data for chunk %d (from PAK '%ls') as the '%ls' LocRes file for '%ls' was not yet available", ChunkId, *PakFilename, *CompiledCulture, *ChunkedLocalizationTargetName);
				return;
			}
		}
...
void FTextLocalizationManager::CacheLocalizedLanguageName(const ELocalizationLoadFlags InLoadFlags)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FTextLocalizationManager::CacheLocalizedLanguageName);

	{
		const TArray<FString> AvailableCultures = GetLocalizedCultureNames(InLoadFlags);
		const TArray<FString> PrioritizedCulturesToMatch = FInternationalization::Get().GetPrioritizedCultureNames(FInternationalization::Get().GetCurrentLanguage()->GetName());
		for (const FString& PrioritizedCulture : PrioritizedCulturesToMatch)
		{
			if (AvailableCultures.Contains(PrioritizedCulture))
			{
				LocalizedLanguageName = PrioritizedCulture;
				return;
			}
		}
	}
	...
	{
		// If this is a game, use the native culture of the game as the fallback
		LocalizedLanguageName = GetNativeCultureName(ELocalizedTextSourceCategory::Game);
...
void FTextLocalizationManager::EnableGameLocalizationPreview(const FString& CultureName)
{
	...
	const bool bWillGameLocalizationPreviewBeEnabled = PreviewCulture != NativeGameCulture;
	// We always need to enable this, even when disabling, so that the native language data reloads correctly for plugins (see ShouldForceLoadGameLocalization in FLocalizationTargetDescriptor::ShouldLoadLocalizationTarget)
	// The correct value is set once the loc data has loaded, via the async notification below
	bIsGameLocalizationPreviewEnabled = true;
	bIsLocalizationLocked = IsLocalizationLockedByConfig() || bWillGameLocalizationPreviewBeEnabled;

	TArray<FString> PrioritizedCultureNames;
	if (bWillGameLocalizationPreviewBeEnabled)
	{
	...
	LocLoadFlags |= (bWillGameLocalizationPreviewBeEnabled ? ELocalizationLoadFlags::Native : ELocalizationLoadFlags::None);

	LoadLocalizationResourcesForPrioritizedCultures_Async(PrioritizedCultureNames, LocLoadFlags, 
		[bWillGameLocalizationPreviewBeEnabled]()
		{
			FTextLocalizationManager::Get().bIsGameLocalizationPreviewEnabled = bWillGameLocalizationPreviewBeEnabled;
		});
}
