...
UE_TRACE_EVENT_BEGIN(Cpu, LoadTextLocalizationMetaDataResourceFromFile, NoSync)
...
bool FTextLocalizationMetaDataResource::LoadFromFile(const FString& FilePath)
{
	...
	UE_TRACE_LOG_SCOPED_T(Cpu, LoadTextLocalizationMetaDataResourceFromFile, CpuChannel)
		<< LoadTextLocalizationMetaDataResourceFromFile.Path(*FilePath);
	...
	if (!Reader)
	{
		UE_LOGF(LogTextLocalizationResource, Log, "LocMeta '%ls' could not be opened for reading!", *FilePath);
		return false;
	}
...
bool FTextLocalizationMetaDataResource::LoadFromArchive(FArchive& Archive, const FString& LocMetaID)
{
	...
	// Verify header
	{
		...
		{
			UE_LOGF(LogTextLocalizationResource, Warning, "LocMeta '%ls' failed the magic number check!", *LocMetaID);
			return false;
		}
	...
	{
		UE_LOGF(LogTextLocalizationResource, Error, "LocMeta '%ls' is too new to be loaded (File Version: %d, Loader Version: %d)", *LocMetaID, (int32)VersionNumber, (int32)FTextLocalizationResourceVersion::ELocMetaVersion::Latest);
		return false;
	}
...
bool FTextLocalizationMetaDataResource::SaveToFile(const FString& FilePath)
{
	...
	if (!Writer)
	{
		UE_LOGF(LogTextLocalizationResource, Log, "LocMeta '%ls' could not be opened for writing!", *FilePath);
		return false;
	}
...
{
	AddEntry(InNamespace, InKey, InSourceStringHash, InLocalizedString, InPriority, InLocResID, TextLocalizationResourceUtil::GetLocalizationTargetPathIdFromLocResId(InLocResID));
}

void FTextLocalizationResource::AddEntry(const FTextKey& InNamespace, const FTextKey& InKey, const FString& InSourceString, const FString& InLocalizedString, const int32 InPriority, const FTextKey& InLocResID, const int32 InLocalizationTargetPathID)
{
	AddEntry(InNamespace, InKey, HashString(InSourceString), MakeTextDisplayString(CopyTemp(InLocalizedString)), InPriority, InLocResID, InLocalizationTargetPathID);
}

void FTextLocalizationResource::AddEntry(const FTextKey& InNamespace, const FTextKey& InKey, const uint32 InSourceStringHash, const FTextConstDisplayStringRef& InLocalizedString, const int32 InPriority, const FTextKey& InLocResID, const int32 InLocalizationTargetPathID)
{
	FEntry NewEntry;
	NewEntry.LocResID = InLocResID;
	NewEntry.LocalizationTargetPathId = InLocalizationTargetPathID;
...
bool FTextLocalizationResource::LoadFromFile(const FString& FilePath, const int32 Priority)
{
	...
	if (!Reader)
	{
		UE_LOGF(LogTextLocalizationResource, Log, "LocRes '%ls' could not be opened for reading!", *FilePath);
		return false;
	}
...
bool FTextLocalizationResource::LoadFromArchive(FArchive& Archive, const FTextKey& LocResID, const int32 Priority)
{
	...
	else
	{
		...
		Archive.Seek(0);
		UE_LOGF(LogTextLocalizationResource, Warning, "LocRes '%ls' failed the magic number check! Assuming this is a legacy resource (please re-generate your localization resources!)", *LocResID.ToString());
	}

	// Is this LocRes file too new to load?
	if (VersionNumber > FTextLocalizationResourceVersion::ELocResVersion::Latest)
	{
		UE_LOGF(LogTextLocalizationResource, Error, "LocRes '%ls' is too new to be loaded (File Version: %d, Loader Version: %d)", *LocResID.ToString(), (int32)VersionNumber, (int32)FTextLocalizationResourceVersion::ELocResVersion::Latest);
		return false;
	}
	...
	for (uint32 i = 0; i < NamespaceCount; ++i)
	{
		...
		for (uint32 j = 0; j < KeyCount; ++j)
		{
			...
			if (VersionNumber >= FTextLocalizationResourceVersion::ELocResVersion::Compact)
			{
				...
				{
					UE_LOGF(LogTextLocalizationResource, Warning, "LocRes '%ls' has an invalid localized string index for namespace '%ls' and key '%ls'. This entry will have no translation.", *LocResID.ToString(), *Namespace.ToString(), *Key.ToString());
				}
			}
			else
			{
...
bool FTextLocalizationResource::SaveToFile(const FString& FilePath) const
{
	...
	if (!Writer)
	{
		UE_LOGF(LogTextLocalizationResource, Log, "LocRes '%ls' could not be opened for writing!", *FilePath);
		return false;
	}
	...
	// Equal priority entries won't replace, but may log a conflict
	{
		...
		if (bDidConflict)
		{
			...
			if (bLoggingConflictsRequested)
			{
				...
				UE_LOGF(LogTextLocalizationResource, Warning, "%ls", *(SummaryMessage + TEXT(" ") + DetailsMessage));
			}
			else if (!GIsBuildMachine)
			{
				// Log just the summary as a warning and hide the rest in the log.
				UE_LOGF(LogTextLocalizationResource, Warning, "%ls", *SummaryMessage);
				UE_LOGF(LogTextLocalizationResource, Log, "%ls", *DetailsMessage);
			}
			else
			{
				// Hide the entire message in the log.
				UE_LOGF(LogTextLocalizationResource, Log, "%ls", *(SummaryMessage + TEXT(" ") + DetailsMessage));
			}
		}
	}
#endif
