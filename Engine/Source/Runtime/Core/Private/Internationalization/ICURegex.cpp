...
FString FRegexMatcher::ReplaceFirst(FStringView Replacement)
{
	TSharedPtr<icu::RegexMatcher> ICURegexMatcher = Implementation->GetInternalRegexMatcher();
	if (ICURegexMatcher.IsValid())
	{
		UErrorCode ICUStatus = U_ZERO_ERROR;
		icu::UnicodeString ReplacementString = ICUUtilities::ConvertString(Replacement);
		icu::UnicodeString Result = ICURegexMatcher->replaceFirst(ReplacementString, ICUStatus);
		return ICUUtilities::ConvertString(Result);
	}

	return FString(Replacement);
}

FString FRegexMatcher::ReplaceAll(FStringView Replacement)
{
	TSharedPtr<icu::RegexMatcher> ICURegexMatcher = Implementation->GetInternalRegexMatcher();
	if (ICURegexMatcher.IsValid())
	{
		UErrorCode ICUStatus = U_ZERO_ERROR;
		icu::UnicodeString ReplacementString = ICUUtilities::ConvertString(Replacement);
		icu::UnicodeString Result = ICURegexMatcher->replaceAll(ReplacementString, ICUStatus);
		return ICUUtilities::ConvertString(Result);
	}
