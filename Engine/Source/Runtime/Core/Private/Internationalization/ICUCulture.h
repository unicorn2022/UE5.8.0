...
public:
	...
	// We run this in the open which is safe so long as we cannot create `FICUCultureImplementation` in the closed.
	AUTORTFM_OPEN virtual const FDecimalNumberFormattingRules& GetDecimalNumberFormattingRules() override;

	// We run this in the open which is safe so long as we cannot create `FICUCultureImplementation` in the closed.
	AUTORTFM_OPEN virtual const FDecimalNumberFormattingRules& GetPercentFormattingRules() override;

	// We run this in the open which is safe so long as we cannot create `FICUCultureImplementation` in the closed.
	AUTORTFM_OPEN virtual const FDecimalNumberFormattingRules& GetCurrencyFormattingRules(const FString& InCurrencyCode) override;
