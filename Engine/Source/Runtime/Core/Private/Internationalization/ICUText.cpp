...
FString FTextTransformer::ToLower(const FString& InStr)
{
	...
	// We run this code in the open because it calls into ICU which is not transactionally instrumented.
	// We get away with this because we use the explicit `AutoRTFM::Open` which knows how to take open
	// constructed `FString`'s and make them transactionally safe (see `AutoRTFMReturnFromOpenMode`).
...
ETextDirection ComputeTextDirection(UBiDi* InICUBiDi, const icu::UnicodeString& InICUString)
{
	...
	{
		UE_LOGF(LogCore, Warning, "Failed to set the string data on the ICU BiDi object (error code: %d). Text will assumed to be left-to-right", static_cast<int32>(ICUStatus));
	}
...
ETextDirection ComputeTextDirection(const TCHAR* InString, const int32 InStringStartIndex, const int32 InStringLen)
{
	...
	{
		UE_LOGF(LogCore, Warning, "Failed to create ICU BiDi object (error code: %d). Text will assumed to be left-to-right", static_cast<int32>(ICUStatus));
	}
