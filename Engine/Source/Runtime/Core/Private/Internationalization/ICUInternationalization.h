...
private:
...
#if WITH_ICU_V78
	static UBool OpenDataFile(const void* InContext, void** OutFileContext, void** OutContents, const char* InPath, int32_t* OutLength);
#else
	...
	struct FICUCachedFileData
	{
		...
		// Stored as int32 because ICU's UDataMemory::length (udatamem.h) is int32_t — the value written
		// here is ultimately assigned to pData->length in uprv_mapFileFromCallback, so widening to int64
		// would not eliminate the narrowing; it would just move it into ICU's own internals. int32 also
		// avoids the 4-byte alignment padding that int64 would introduce after ReferenceCount, increasing
		// every cache entry by 8 bytes. No ICU data file approaches 2GB — the largest full locale set
		// is order of tens of MB.
		int32 FileSize;
		void* Buffer;
	};
	...
	/**
	 * Running tally of memory across all cached ICU data files, in bytes.
	 * MUST only be read or written while holding PathToCachedFileDataMapCS.
	 * All access sites in OpenDataFile and CloseDataFile already hold that lock.
	 */
	int64 TotalCachedFileDataBytes = 0;
};
