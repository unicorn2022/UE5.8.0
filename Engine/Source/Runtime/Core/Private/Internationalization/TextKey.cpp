...
bool LoadKeyString(FArchive& Ar, FInlineStringBuffer& OutStrBuffer)
{
	...
	if (SaveNum)
	{
		...
		UE_CLOGF(SaveNum > InlineStringSize, LogTextKey, VeryVerbose, "Key string '%ls' was larger (%d) than the inline size (%d) and caused an allocation!", OutStrBuffer.GetData(), SaveNum, InlineStringSize);
	}
