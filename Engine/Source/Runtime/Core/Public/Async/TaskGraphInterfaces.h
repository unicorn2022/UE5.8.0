...
public:
	...
	virtual ~FTaskGraphInterface() = default;
...
public:
	...
	{	// only called for named thread tasks, normal tasks are executed using `FTaskBase` API directly (see `TGraphTask`)
		...
		TryExecuteTask(); // It's possible retraction already executed this task
...
public:
	...
	private:
		...
		inline TGraphTask* ConstructAndHoldImpl(T&&... Args)
		{
			...
			// Only use this when TTask implements GetTaskFlags().
			if constexpr (TModels_V<CGetTaskFlagsProvider, TTask>)
			{
				Task->Init(Pri, ExtPri, TaskObject->GetTaskFlags());
			}
			else
			{
...
private:
	...
	void Init(UE::Tasks::ETaskPriority InPriority, UE::Tasks::EExtendedTaskPriority InExtendedPriority, UE::Tasks::ETaskFlags InTaskFlags = UE::Tasks::ETaskFlags::None)
	{
		FBaseGraphTask::Init(TEXT("GraphTask"), InPriority, InExtendedPriority, InTaskFlags);
	}
	...
	virtual void ExecuteTask() override final
	{
		...
		{
			SCOPE_CYCLE_COUNTER_STATID(TaskObject->GetStatId());
