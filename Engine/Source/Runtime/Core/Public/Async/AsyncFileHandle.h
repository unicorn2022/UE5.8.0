...
protected:
	...
	std::atomic<bool> bDataIsReady = false;
	std::atomic<bool> bCompleteAndCallbackCalled = false;
	std::atomic<bool> bCompleteSync = false;
...
public:
	...
	virtual ~IAsyncReadRequest()
	{
		...
		UE_CLOGF(!(bCompleteAndCallbackCalled.load(std::memory_order_relaxed) && (bSizeRequest || !Memory)),
			LogHAL, Fatal, "IAsyncReadRequests must not be deleted until they are completed."
		);
	...
	**/
	...
	{
		// fast path to avoid a barrier until the value returns true
		if (bCompleteAndCallbackCalled.load(std::memory_order_relaxed))
		{
			// Once we return true, we need a barrier to guarantee the memory
			// writes that have happened before the release barrier can be seen.
			// Because for all we know, PollCompletion() could be the only thing
			// that is used to synchronize with the writing thread.
			return bCompleteAndCallbackCalled.load(std::memory_order_acquire);
		}
	...
	*/
	...
	inline void Cancel()
	{
		...
		{
			bCanceled.store(true, std::memory_order_release);
			bDataIsReady.store(true, std::memory_order_release);
	...
	**/
	...
	{
		check(bDataIsReady.load(std::memory_order_relaxed) && bSizeRequest);
		return bCanceled.load(std::memory_order_relaxed) ? -1 : Size;
	}
	...
	**/
	inline uint8* GetReadResults()
	{
		check(bDataIsReady.load(std::memory_order_relaxed) && !bSizeRequest);
		uint8* Result = Memory;
		if (bCanceled.load(std::memory_order_relaxed) && Result)
		{
			Result = nullptr;
		}
		else
...
protected:
	...
	**/
	...
	void SetDataComplete()
	{
		...
		if (Callback)
		{
			Callback(bCanceled.load(std::memory_order_relaxed), this);
		}
	}

	void SetAllComplete()
	{
		bCompleteAndCallbackCalled.store(true, std::memory_order_release);
	}
