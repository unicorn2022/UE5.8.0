...
#if !defined(UE_ASYNC_WORK_STEALING_QUEUE_USES_RETRY_LIMIT)
#define UE_ASYNC_WORK_STEALING_QUEUE_USES_RETRY_LIMIT 0
#endif

#if UE_ASYNC_WORK_STEALING_QUEUE_USES_RETRY_LIMIT
extern CORE_API int32 GWorkStealingQueueRetryLimit;
#endif
...
protected:
	...
	{
#if UE_ASYNC_WORK_STEALING_QUEUE_USES_RETRY_LIMIT
		int32 Retries = GWorkStealingQueueRetryLimit;
#endif
		...
		do
		{
			...
			{
				if (ItemSlots[Idx].Value.compare_exchange_weak(Slot, uintptr_t(ESlotState::Taken), std::memory_order_acq_rel))
				{
					if(IdxVer == Tail.load(std::memory_order_acquire))
					{
			...
			else if (Retries-- == 0)
			{
				...
				Retries = GWorkStealingQueueRetryLimit;
			}
#endif
		} while(true);
	}
...
public:
	...
	public:
		...
		inline FTask* StealLocal(int32 PriorityIndex)
		{
			FTask* Item;
		...
		inline FTask* StealLocal(bool bPermitBackgroundTasks)
		{
			const int32 MaxPriority = bPermitBackgroundTasks ? int32(ETaskPriority::Count) : int32(ETaskPriority::ForegroundCount);
		...
		inline FTask* Dequeue(int32 PriorityIndex)
		{
			FTask* Item;
			if (LocalQueues[PriorityIndex].Get(Item))
			{
		...
		inline FTask* Dequeue(bool bPermitBackgroundTasks)
		{
			const int32 MaxPriority = bPermitBackgroundTasks ? int32(ETaskPriority::Count)   : int32(ETaskPriority::ForegroundCount);
		...
		inline FTask* DequeueSteal(bool bPermitBackgroundTasks)
		{
			...
			FTask* Result = Registry->StealItem(CachedRandomIndex, CachedPriorityIndex, bPermitBackgroundTasks);
			if (Result)
			{
				return Result;
			}
...
private:
	...
	void AddLocalQueue(TLocalQueue* QueueToAdd)
	{
		...
		UE_CLOGF(Index >= MaxLocalQueues, LowLevelTasks, Fatal, "Attempting to add more than the maximum allowed number of queues (%d)", MaxLocalQueues);
