...
public:
	...
	inline TFunctionWithContext(CallableType&& Callable UE_LIFETIMEBOUND)
		requires (!std::is_same_v<std::decay_t<CallableType>, TFunctionWithContext>) &&
			std::is_invocable_r_v<ReturnType, std::decay_t<CallableType>, ArgTypes...>
		: Function(&Call<std::remove_pointer_t<std::decay_t<CallableType>>>)
		, Context((void*)&Callable)
	{
	}

	/** Assign from a lambda or a function pointer. */
	template <typename CallableType>
	inline TFunctionWithContext& operator=(CallableType&& Callable UE_LIFETIMEBOUND)
		requires (!std::is_same_v<std::decay_t<CallableType>, TFunctionWithContext>) &&
			std::is_invocable_r_v<ReturnType, std::decay_t<CallableType>, ArgTypes...>
	{
		Function = &Call<std::remove_pointer_t<std::decay_t<CallableType>>>;
...
private:
	...
	{
		return Invoke(*(CallableType*)Callable, (ArgTypes&&)Args...);
	}
