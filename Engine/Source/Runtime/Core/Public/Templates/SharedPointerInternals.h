...
namespace SharedPointerInternals
{
	...
	template <class SharedReferencerType, class ObjectType>
	void EnableSharedFromThis(SharedReferencerType& InSharedReferencer, ObjectType const* InObject);
	...
	public:
		...
		/** Constructor that references the specified controller, without incrementing the shared count */
	...
	private:
		...
		friend void EnableSharedFromThis(SharedReferencerType& InSharedReferencer, OtherType const* InObject);
	...
	public:
		...
		/** Constructor that references the specified controller, without incrementing the weak count */
		explicit FWeakReferencer(TReferenceControllerBase<Mode>* InReferenceController)
			: ReferenceController{InReferenceController}
		{
		}
...
namespace SharedPointerInternals
{
	/** Templated helper function (const) that creates a shared reference or pointer from an object instance */
	template <class SharedReferencerType, class ObjectType>
	void EnableSharedFromThis(SharedReferencerType& InSharedReferencer, ObjectType const* InObject)
	{
		///////////////////////////////////////////////////////////////////////////////////////////////
		...
		if constexpr (IsDerivedFromSharedFromThis<ObjectType>())
		{
			if (InObject)
			{
				InObject->UpdateWeakReferenceInternal(InSharedReferencer.ReferenceController);
			}
		}
	}
}
