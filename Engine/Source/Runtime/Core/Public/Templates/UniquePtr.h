...
#include "Concepts/UnboundedArray.h"
...
struct TDefaultDelete<T[]>
{
	...
	template <typename U>
		requires UE::CConvertibleTo<U(*)[], T(*)[]>
	TDefaultDelete(const TDefaultDelete<U[]>&)
	{
	}
...
public:
	 ...
	 */
	template <UE::CPointerConvertibleTo<T> OtherT, typename OtherDeleter>
		requires (!UE::CArray<OtherT>)
	 ...
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT std::add_lvalue_reference_t<T> operator*() const
		requires requires { *(T*)nullptr; } // this construct means that operator* is only considered for overload resolution if T is dereferenceable
	{
		return *Ptr;
	}
...
public:
	 ...
	 */
	template <typename OtherT, typename OtherDeleter>
		requires UE::CConvertibleTo<OtherT(*)[], T(*)[]>
	inline TUniquePtr(TUniquePtr<OtherT[], OtherDeleter>&& Other)
		: Deleter(MoveTemp(Other.GetDeleter()))
		, Ptr    (Other.Ptr)
	{
...
template <typename T, typename... ArgTypes>
	requires (!UE::CArray<T>)
[[nodiscard]] UE_FORCEINLINE_HINT TUniquePtr<T> MakeUnique(ArgTypes&&... Args)
{
