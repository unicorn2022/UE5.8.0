...
public:
	...
	>
	UE_NODEBUG explicit TSharedRef( OtherType* InObject )
		: Object( InObject )
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef( OtherType* InObject, DeleterType&& InDeleter )
		: Object( InObject )
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef( SharedPointerInternals::TRawPtrProxy< OtherType > const& InRawPtrProxy )
		...
		, SharedReferenceCount( SharedPointerInternals::NewDefaultReferenceController< Mode >( InRawPtrProxy.Object ) )
	{
		...
		SharedPointerInternals::EnableSharedFromThis(SharedReferenceCount, InRawPtrProxy.Object);
	}
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType > const& InRawPtrProxy )
		: Object( InRawPtrProxy.Object )
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType >&& InRawPtrProxy )
		: Object( InRawPtrProxy.Object )
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef( TSharedRef< OtherType, Mode > const& InSharedRef )
	...
	UE_NODEBUG inline TSharedRef( TSharedRef< OtherType, Mode > const& InSharedRef, SharedPointerInternals::FStaticCastTag )
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TSharedRef( TSharedRef< OtherType, Mode > const& InSharedRef, SharedPointerInternals::FConstCastTag )
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TSharedRef( TSharedRef< OtherType, Mode > const& OtherSharedRef, ObjectType* InObject )
		: Object( InObject )
	...
	UE_NODEBUG inline TSharedRef( TSharedRef const& InSharedRef )
		: Object( InSharedRef.Object )
		, SharedReferenceCount( InSharedRef.SharedReferenceCount )
	{
	}

	UE_NODEBUG inline TSharedRef( TSharedRef&& InSharedRef )
		: Object( InSharedRef.Object )
		, SharedReferenceCount( InSharedRef.SharedReferenceCount )
	{
	 ...
	 */
	UE_NODEBUG inline TSharedRef& operator=( TSharedRef const& InSharedRef )
	{
	...
	UE_REWRITE TSharedRef& operator=( TSharedRef&& InSharedRef )
	{
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef& operator=( SharedPointerInternals::TRawPtrProxy< OtherType > const& InRawPtrProxy )
	{
		// If the following assert goes off, it means a TSharedRef was initialized from a nullptr object pointer.
		// Shared references must never be nullptr, so either pass a valid object or consider using TSharedPtr instead.
		check( InRawPtrProxy.Object != nullptr );
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef& operator=( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType > const& InRawPtrProxy )
	{
		// If the following assert goes off, it means a TSharedRef was initialized from a nullptr object pointer.
		// Shared references must never be nullptr, so either pass a valid object or consider using TSharedPtr instead.
		check( InRawPtrProxy.Object != nullptr );
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedRef& operator=( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType >&& InRawPtrProxy )
	{
		// If the following assert goes off, it means a TSharedRef was initialized from a nullptr object pointer.
		// Shared references must never be nullptr, so either pass a valid object or consider using TSharedPtr instead.
		check( InRawPtrProxy.Object != nullptr );
	...
	/** TSharedRef can be returned from AutoRTFM::Open() via a copy-construction in the closed. */
	 ...
	 */
	[[nodiscard]] UE_REWRITE TSharedPtr<ObjectType, Mode> ToSharedPtr() const
	{
	 ...
	 */
	[[nodiscard]] UE_REWRITE TWeakPtr<ObjectType, Mode> ToWeakPtr() const
	{
	 ...
	 */
	[[nodiscard]] UE_REWRITE ObjectType& Get() const
	{
	 ...
	 */
	[[nodiscard]] UE_REWRITE ObjectType& operator*() const
	{
		// Should never be nullptr as TSharedRef is never nullable
		checkSlow( IsValid() );
		return *Object;
	 ...
	 */
	[[nodiscard]] UE_REWRITE int32 GetSharedReferenceCount() const
	{
	 ...
	 */
	[[nodiscard]] UE_REWRITE bool IsUnique() const
	{
	...
	UE_REWRITE explicit TSharedRef(FIntrusiveUnsetOptionalState)
		: Object(nullptr)
	{
	}
	UE_REWRITE bool UEOpEquals(FIntrusiveUnsetOptionalState) const
	{
		return !IsValid();
	}
	/////////////////////////////////////////////////
	...
	template <typename OtherType>
	[[nodiscard]] UE_NODEBUG bool UEOpEquals(const TSharedRef<OtherType, Mode>& Rhs) const
	{
		return this->Object == &Rhs.Get();
	}

	template <typename OtherType>
	[[nodiscard]] UE_NODEBUG bool UEOpEquals(const TSharedPtr<OtherType, Mode>& Rhs) const
	{
	...
	template <typename OtherType>
	[[nodiscard]] UE_NODEBUG bool UEOpEquals(const TWeakPtr<OtherType, Mode>& Rhs) const
	{
		return this->Object == Rhs.Pin().Get();
	}

	template <typename OtherType>
	[[nodiscard]] UE_NODEBUG bool UEOpLessThan(const TSharedRef<OtherType, Mode>& Rhs) const
	{
		return this->Object < &Rhs.Get();
	}
...
private:
	...
	void Init(OtherType* InObject)
	{
		...
		// If the object happens to be derived from TSharedFromThis, the following method
		// will prime the object with a weak pointer to itself.
		SharedPointerInternals::EnableSharedFromThis(SharedReferenceCount, InObject);
	}
...
private:
	...
	UE_NODEBUG inline explicit TSharedRef(ObjectType* InObject, SharedPointerInternals::TReferenceControllerBase<Mode>* InSharedReferenceCount)
		: Object(InObject)
...
public:
	...
	UE_REWRITE TSharedPtr( SharedPointerInternals::FNullTag* = nullptr )
		: Object( nullptr )
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline explicit TSharedPtr( OtherType* InObject )
		: Object( InObject )
		, SharedReferenceCount( SharedPointerInternals::NewDefaultReferenceController< Mode >( InObject ) )
	{
		// If the object happens to be derived from TSharedFromThis, the following method
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedPtr( OtherType* InObject, DeleterType&& InDeleter )
		: Object( InObject )
		, SharedReferenceCount( SharedPointerInternals::NewCustomReferenceController< Mode >( InObject, Forward< DeleterType >( InDeleter ) ) )
	{
		// If the object happens to be derived from TSharedFromThis, the following method
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedPtr( SharedPointerInternals::TRawPtrProxy< OtherType > const& InRawPtrProxy )
		: Object( InRawPtrProxy.Object )
		, SharedReferenceCount( SharedPointerInternals::NewDefaultReferenceController< Mode >( InRawPtrProxy.Object ) )
	{
		// If the object happens to be derived from TSharedFromThis, the following method
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedPtr( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType > const& InRawPtrProxy )
		: Object( InRawPtrProxy.Object )
		, SharedReferenceCount( SharedPointerInternals::NewCustomReferenceController< Mode >( InRawPtrProxy.Object, InRawPtrProxy.Deleter ) )
	{
		// If the object happens to be derived from TSharedFromThis, the following method
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedPtr( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType >&& InRawPtrProxy )
		: Object( InRawPtrProxy.Object )
		, SharedReferenceCount( SharedPointerInternals::NewCustomReferenceController< Mode >( InRawPtrProxy.Object, MoveTemp( InRawPtrProxy.Deleter ) ) )
	{
		// If the object happens to be derived from TSharedFromThis, the following method
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedPtr( TSharedPtr< OtherType, Mode > const& InSharedPtr )
		: Object( InSharedPtr.Object )
		, SharedReferenceCount( InSharedPtr.SharedReferenceCount )
	{
	}

	UE_NODEBUG inline TSharedPtr( TSharedPtr const& InSharedPtr )
		: Object( InSharedPtr.Object )
		, SharedReferenceCount( InSharedPtr.SharedReferenceCount )
	{
	}

	UE_NODEBUG inline TSharedPtr( TSharedPtr&& InSharedPtr )
		: Object( InSharedPtr.Object )
		, SharedReferenceCount( MoveTemp(InSharedPtr.SharedReferenceCount) )
	{
		InSharedPtr.Object = nullptr;
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TSharedPtr( TSharedRef< OtherType, Mode > const& InSharedRef )
		: Object( InSharedRef.Object )
		, SharedReferenceCount( InSharedRef.SharedReferenceCount )
	{
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TSharedPtr( TSharedPtr< OtherType, Mode > const& InSharedPtr, SharedPointerInternals::FStaticCastTag )
	...
	template <typename OtherType>
	UE_NODEBUG TSharedPtr(TSharedPtr<OtherType, Mode>&& InSharedPtr, SharedPointerInternals::FStaticCastTag)
		: Object{static_cast<ObjectType*>(InSharedPtr.Object)}
		, SharedReferenceCount{MoveTemp(InSharedPtr.SharedReferenceCount)}
	{
		InSharedPtr.Object = nullptr;
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TSharedPtr( TSharedPtr< OtherType, Mode > const& InSharedPtr, SharedPointerInternals::FConstCastTag )
	...
	template <typename OtherType>
	UE_NODEBUG TSharedPtr(TSharedPtr<OtherType, Mode>&& InSharedPtr, SharedPointerInternals::FConstCastTag)
		: Object{const_cast<ObjectType*>(InSharedPtr.Object)}
		, SharedReferenceCount{MoveTemp(InSharedPtr.SharedReferenceCount)}
	{
		InSharedPtr.Object = nullptr;
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TSharedPtr( TSharedPtr< OtherType, Mode > const& OtherSharedPtr, ObjectType* InObject )
		: Object( InObject )
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TSharedPtr( TSharedPtr< OtherType, Mode >&& OtherSharedPtr, ObjectType* InObject )
		: Object( InObject )
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TSharedPtr( TSharedRef< OtherType, Mode > const& OtherSharedRef, ObjectType* InObject )
		: Object( InObject )
		, SharedReferenceCount( OtherSharedRef.SharedReferenceCount )
	{
	}
	 ...
	 */
	// NOTE: The following is an Unreal extension to standard shared_ptr behavior
	UE_REWRITE TSharedPtr& operator=( SharedPointerInternals::FNullTag* )
	{
		Reset();
		return *this;
	}
	 ...
	 */
	UE_NODEBUG inline TSharedPtr& operator=( TSharedPtr const& InSharedPtr )
	{
	...
	UE_NODEBUG inline TSharedPtr& operator=( TSharedPtr&& InSharedPtr )
	{
		if (this != &InSharedPtr)
		{
			Object = InSharedPtr.Object;
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_REWRITE TSharedPtr& operator=( SharedPointerInternals::TRawPtrProxy< OtherType > const& InRawPtrProxy )
	{
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_REWRITE TSharedPtr& operator=( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType > const& InRawPtrProxy )
	{
		*this = TSharedPtr< ObjectType, Mode >( InRawPtrProxy );
		return *this;
	}
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_REWRITE TSharedPtr& operator=( SharedPointerInternals::TRawPtrProxyWithDeleter< OtherType, DeleterType >&& InRawPtrProxy )
	{
		*this = TSharedPtr< ObjectType, Mode >( MoveTemp( InRawPtrProxy ) );
		return *this;
	}

	/** TSharedPtr can be returned from AutoRTFM::Open() via a copy-construction in the closed. */
	static constexpr AutoRTFM::EReturnFromOpenMode AutoRTFMReturnFromOpenMode = AutoRTFM::EReturnFromOpenMode::CopyConstructInClosed;
	 ...
	 */
	// NOTE: The following is an Unreal extension to standard shared_ptr behavior
	[[nodiscard]] UE_NODEBUG inline TSharedRef< ObjectType, Mode > ToSharedRef() const&
	{
		// If this assert goes off, it means a shared reference was created from a shared pointer that was nullptr.
		// Shared references are never allowed to be null.  Consider using TSharedPtr instead.
		check( IsValid() );
	 ...
	 */
	 // NOTE: The following is an Unreal extension to standard shared_ptr behavior
	[[nodiscard]] UE_NODEBUG inline TSharedRef< ObjectType, Mode > ToSharedRef() &&
	{
		// If this assert goes off, it means a shared reference was created from a shared pointer that was nullptr.
		// Shared references are never allowed to be null.  Consider using TSharedPtr instead.
		check( IsValid() );
	 ...
	 * @return  True if the shared pointer is valid and can be dereferenced
	 */
	[[nodiscard]] UE_REWRITE const bool IsValid() const
	{
		return Object != nullptr;
	}
	 ...
	 * @return  Reference to the object
	 */
	[[nodiscard]] UE_NODEBUG inline std::add_lvalue_reference_t<ObjectType> operator*() const
		requires requires (ObjectType* Ptr) { *Ptr; }
	{
		check( IsValid() );
		return *Object;
	}
	 ...
	 */
	[[nodiscard]] UE_NODEBUG inline ObjectType* operator->() const
	{
		check( IsValid() );
		return Object;
	}
	...
	UE_REWRITE explicit TSharedPtr(FIntrusiveUnsetOptionalState)
		: Object((ObjectType*)-1)
	{
	}
	UE_REWRITE bool UEOpEquals(FIntrusiveUnsetOptionalState) const
	...
	template <typename OtherType>
	[[nodiscard]] UE_REWRITE bool UEOpEquals(const TSharedPtr<OtherType, Mode>& Rhs) const
	{
		return this->Object == Rhs.Get();
	}
	...
	template <typename OtherType>
	[[nodiscard]] UE_REWRITE bool UEOpLessThan(const TSharedPtr<OtherType, Mode>& Rhs) const
	{
		return this->Object < Rhs.Get();
	}
...
private:
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline explicit TSharedPtr( TWeakPtr< OtherType, Mode > const& InWeakPtr )
		: Object( nullptr )
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline explicit TSharedPtr(TWeakPtr< OtherType, Mode >&& InWeakPtr)
		: Object(nullptr)
	 ...
	 * another shared object, but pointing to a different object, typically a subobject.
	 *
	 * @param  InSharedReferenceCount  The shared referencer whose reference count should be shared.
	 * @param  InObject                The object pointer to use
	 */
	UE_NODEBUG TSharedPtr(SharedPointerInternals::FSharedReferencer<Mode> InSharedReferenceCount, ObjectType* InObject)
		: Object{InObject}
		, SharedReferenceCount{MoveTemp(InSharedReferenceCount)}
	{
	}
...
public:
	...
	// NOTE: FNullTag parameter is an Unreal extension to standard shared_ptr behavior
	UE_NODEBUG inline TWeakPtr( SharedPointerInternals::FNullTag* = nullptr )
		: Object( nullptr )
		, WeakReferenceCount()
	{
	}
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr( TSharedRef< OtherType, Mode > const& InSharedRef )
		: Object( InSharedRef.Object )
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr( TSharedPtr< OtherType, Mode > const& InSharedPtr )
		: Object( InSharedPtr.Object )
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TWeakPtr(TWeakPtr< OtherType, Mode > const& InWeakPtr, SharedPointerInternals::FStaticCastTag)
	...
	template <typename OtherType>
	UE_NODEBUG TWeakPtr(TWeakPtr<OtherType, Mode>&& InWeakPtr, SharedPointerInternals::FStaticCastTag)
		: Object{static_cast<ObjectType*>(InWeakPtr.Object)}
	 ...
	 */
	template <typename OtherType>
	UE_NODEBUG inline TWeakPtr(TWeakPtr< OtherType, Mode > const& InWeakPtr, SharedPointerInternals::FConstCastTag)
	...
	template <typename OtherType>
	UE_NODEBUG TWeakPtr(TWeakPtr<OtherType, Mode>&& InWeakPtr, SharedPointerInternals::FConstCastTag)
		: Object{const_cast<ObjectType*>(InWeakPtr.Object)}
		, WeakReferenceCount{MoveTemp(InWeakPtr.WeakReferenceCount)}
	{
		InWeakPtr.Object = nullptr;
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr( TWeakPtr< OtherType, Mode > const& InWeakPtr )
		: Object( InWeakPtr.Object )
		, WeakReferenceCount( InWeakPtr.WeakReferenceCount )
	{
	}
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr( TWeakPtr< OtherType, Mode >&& InWeakPtr )
		: Object( InWeakPtr.Object )
		, WeakReferenceCount( MoveTemp(InWeakPtr.WeakReferenceCount) )
	{
		InWeakPtr.Object = nullptr;
	...
	UE_NODEBUG inline TWeakPtr( TWeakPtr const& InWeakPtr )
		: Object( InWeakPtr.Object )
		, WeakReferenceCount( InWeakPtr.WeakReferenceCount )
	{
	}

	UE_NODEBUG inline TWeakPtr( TWeakPtr&& InWeakPtr )
		: Object( InWeakPtr.Object )
		, WeakReferenceCount( MoveTemp(InWeakPtr.WeakReferenceCount) )
	{
		InWeakPtr.Object = nullptr;
	 ...
	 */
	// NOTE: The following is an Unreal extension to standard shared_ptr behavior
	UE_REWRITE TWeakPtr& operator=( SharedPointerInternals::FNullTag* )
	{
		Reset();
		return *this;
	}
	 ...
	 */
	UE_NODEBUG inline TWeakPtr& operator=( TWeakPtr const& InWeakPtr )
	{
	...
	UE_NODEBUG inline TWeakPtr& operator=( TWeakPtr&& InWeakPtr )
	{
		if (this != &InWeakPtr)
		{
			Object             = InWeakPtr.Object;
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr& operator=( TWeakPtr<OtherType, Mode> const& InWeakPtr )
	{
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr& operator=( TWeakPtr<OtherType, Mode>&& InWeakPtr )
	{
		Object             = InWeakPtr.Object;
		InWeakPtr.Object   = nullptr;
		WeakReferenceCount = MoveTemp(InWeakPtr.WeakReferenceCount);
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr& operator=( TSharedRef< OtherType, Mode > const& InSharedRef )
	{
		Object = InSharedRef.Object;
		WeakReferenceCount = InSharedRef.SharedReferenceCount;
		return *this;
	...
	template <
		...
		UE_REQUIRES(std::is_convertible_v<OtherType*, ObjectType*>)
	>
	UE_NODEBUG inline TWeakPtr& operator=( TSharedPtr< OtherType, Mode > const& InSharedPtr )
	{
		Object = InSharedPtr.Object;
		WeakReferenceCount = InSharedPtr.SharedReferenceCount;
		return *this;
	...
	/** TWeakPtr can be returned from AutoRTFM::Open() via a copy-construction in the closed. */
	static constexpr AutoRTFM::EReturnFromOpenMode AutoRTFMReturnFromOpenMode = AutoRTFM::EReturnFromOpenMode::CopyConstructInClosed;
	 ...
	 */
	[[nodiscard]] UE_REWRITE TSharedPtr< ObjectType, Mode > Pin() const&
	{
		return TSharedPtr< ObjectType, Mode >( *this );
	}
	 ...
	 * @return  Shared pointer for this object (will only be valid if still referenced!)
	 */
	[[nodiscard]] UE_REWRITE TSharedPtr< ObjectType, Mode > Pin() &&
	{
	 ...
	 */
	[[nodiscard]] UE_NODEBUG inline bool IsValid() const
	{
	 ...
	 */
	[[nodiscard]] UE_REWRITE bool HasSameObject( const void* InOtherPtr ) const
	{
		return Pin().Get() == InOtherPtr;
	}

	UE_REWRITE uint32 GetWeakPtrTypeHash() const
	{
	...
	UE_REWRITE explicit TWeakPtr(FIntrusiveUnsetOptionalState)
		: Object((ObjectType*)-1)
	{
	}
	UE_REWRITE bool UEOpEquals(FIntrusiveUnsetOptionalState) const
	...
	template <typename OtherType>
	[[nodiscard]] UE_REWRITE bool UEOpLessThan(const TWeakPtr<OtherType, Mode>& Rhs) const
	{
		return this->Pin().Get() < Rhs.Pin().Get();
	}
...
private:
	/**
	 * Aliasing constructor used to create a weak pointer which shares its reference count with
	 * another shared object, but pointing to a different object, typically a subobject.
	 *
	 * @param  InWeakReferenceCount  The weak referencer whose reference count should be shared.
	 * @param  InObject              The object pointer to use
	 */
	UE_NODEBUG TWeakPtr(SharedPointerInternals::FWeakReferencer<Mode> InWeakReferenceCount, ObjectType* InObject)
		: Object{InObject}
		, WeakReferenceCount{MoveTemp(InWeakReferenceCount)}
	{
	}
...
public:
	 ...
	 */
	[[nodiscard]] UE_NODEBUG TSharedRef< ObjectType, Mode > AsShared()
	{
		//
		...
		//
		check(DoesSharedInstanceExist());
		
		ReferenceController->AddSharedReference();
		TSharedPtr<ObjectType, Mode> SharedThis{SharedPointerInternals::FSharedReferencer<Mode>{ReferenceController}, static_cast<ObjectType*>(this)};
	 ...
	 */
	[[nodiscard]] UE_NODEBUG TSharedRef< ObjectType const, Mode > AsShared() const
	{
		//
		// If the following assert goes off, it means one of the following:
		//
		...
		ReferenceController->AddSharedReference();
		TSharedPtr<ObjectType const, Mode> SharedThis{SharedPointerInternals::FSharedReferencer<Mode>{ReferenceController}, static_cast<ObjectType const*>(this)};
	...
	[[nodiscard]] UE_REWRITE TSharedRef< SubobjectType, Mode > AsSharedSubobject( SubobjectType* SubobjectPtr ) const
	{
	 ...
	 */
	[[nodiscard]] UE_NODEBUG TWeakPtr< ObjectType, Mode > AsWeak()
	{
		//
		// If the following assert goes off, it means one of the following:
		//
		...
		TWeakPtr<ObjectType, Mode> Result{SharedPointerInternals::FWeakReferencer<Mode>{ReferenceController}, static_cast<ObjectType*>(this)};

		// Now that we've verified the pointer is valid, we'll return it!
		return Result;
	}
	[[nodiscard]] UE_NODEBUG TWeakPtr< ObjectType const, Mode > AsWeak() const
	{
		//
		// If the following assert goes off, it means one of the following:
		//
		...
		ReferenceController->AddWeakReference();
		TWeakPtr<ObjectType const, Mode> Result{SharedPointerInternals::FWeakReferencer<Mode>{ReferenceController}, static_cast<ObjectType const*>(this)};
	 ...
	 */
	template <typename SubobjectType>
	[[nodiscard]] UE_REWRITE TWeakPtr< SubobjectType, Mode > AsWeakSubobject( SubobjectType* SubobjectPtr ) const
	{
...
protected:
	 ...
	 */
	template< class OtherType >
	[[nodiscard]] UE_REWRITE static TSharedRef< OtherType, Mode > SharedThis( OtherType* ThisPtr )
	{
	 ...
	 */
	template< class OtherType >
	[[nodiscard]] UE_REWRITE static TSharedRef< OtherType const, Mode > SharedThis( const OtherType* ThisPtr )
	{
...
private:
	...
	/**
	 * Freshens the internal weak pointer object using the authoritative shared reference to the object.
	 * Note that until this function is called, calls to AsShared() will result in an empty pointer.
	 */
	template <ESPMode OtherMode>
	UE_NODEBUG void UpdateWeakReferenceInternal(SharedPointerInternals::TReferenceControllerBase<OtherMode>* InReferenceController) const
	{
		static_assert(OtherMode == Mode, "You cannot create a shared reference or pointer of one mode with a type which inherits TSharedFromThis of another mode.");

		if (!DoesSharedInstanceExist() && ReferenceController != InReferenceController)
		{
			if (InReferenceController)
			{
				InReferenceController->AddWeakReference();
			}
...
public:
	 ...
	 */
	[[nodiscard]] UE_REWRITE bool DoesSharedInstanceExist() const
	{
...
protected:
	...
	UE_REWRITE TSharedFromThis& operator=( TSharedFromThis const& )
	{
		return *this;
	}
...
private:
	...
	template <class SharedReferencerType, class OtherType>
	friend void SharedPointerInternals::EnableSharedFromThis(SharedReferencerType& InSharedReferencer, OtherType const* InObject);

	/** Note this is declared mutable only so that UpdateWeakReferenceInternal() can update it. */
	mutable SharedPointerInternals::TReferenceControllerBase<Mode>* ReferenceController = nullptr;
};
 ...
 */
template< class CastToType, class CastFromType, ESPMode Mode >
[[nodiscard]] UE_REWRITE TSharedPtr< CastToType, Mode > StaticCastSharedPtr(TSharedPtr< CastFromType, Mode > const& InSharedPtr)
{
	return TSharedPtr< CastToType, Mode >(InSharedPtr, SharedPointerInternals::FStaticCastTag());
}

template <class CastToType, class CastFromType, ESPMode Mode>
[[nodiscard]] UE_REWRITE TSharedPtr<CastToType, Mode> StaticCastSharedPtr(TSharedPtr<CastFromType, Mode>&& InSharedPtr)
{
	return TSharedPtr<CastToType, Mode>{MoveTemp(InSharedPtr), SharedPointerInternals::FStaticCastTag{}};
}
 ...
 */
template< class CastToType, class CastFromType, ESPMode Mode >
[[nodiscard]] UE_REWRITE TWeakPtr< CastToType, Mode > StaticCastWeakPtr(TWeakPtr< CastFromType, Mode > const& InWeakPtr)
{
	return TWeakPtr< CastToType, Mode >(InWeakPtr, SharedPointerInternals::FStaticCastTag());
}

template <class CastToType, class CastFromType, ESPMode Mode>
[[nodiscard]] UE_REWRITE TWeakPtr<CastToType, Mode> StaticCastWeakPtr(TWeakPtr<CastFromType, Mode>&& InWeakPtr)
{
	return TWeakPtr<CastToType, Mode>{MoveTemp(InWeakPtr), SharedPointerInternals::FStaticCastTag{}};
}
 ...
 */
template< class CastToType, class CastFromType, ESPMode Mode >
[[nodiscard]] UE_REWRITE TSharedRef< CastToType, Mode > ConstCastSharedRef( TSharedRef< CastFromType, Mode > const& InSharedRef )
{
	return TSharedRef< CastToType, Mode >( InSharedRef, SharedPointerInternals::FConstCastTag() );
}

template <class CastToType, class CastFromType, ESPMode Mode>
[[nodiscard]] UE_REWRITE TSharedRef<CastToType, Mode> ConstCastSharedRef(TSharedRef<CastFromType, Mode>&& InSharedRef)
{
	return TSharedRef<CastToType, Mode>{MoveTemp(InSharedRef), SharedPointerInternals::FConstCastTag{}};
}
 ...
 */
template< class CastToType, class CastFromType, ESPMode Mode >
[[nodiscard]] UE_REWRITE TSharedPtr< CastToType, Mode > ConstCastSharedPtr( TSharedPtr< CastFromType, Mode > const& InSharedPtr )
{
	return TSharedPtr< CastToType, Mode >( InSharedPtr, SharedPointerInternals::FConstCastTag() );
}

template <class CastToType, class CastFromType, ESPMode Mode>
[[nodiscard]] UE_REWRITE TSharedPtr<CastToType, Mode> ConstCastSharedPtr(TSharedPtr<CastFromType, Mode>&& InSharedPtr)
{
	return TSharedPtr<CastToType, Mode>{MoveTemp(InSharedPtr), SharedPointerInternals::FConstCastTag{}};
}
 ...
 */
template< class CastToType, class CastFromType, ESPMode Mode >
[[nodiscard]] UE_REWRITE TWeakPtr< CastToType, Mode > ConstCastWeakPtr(TWeakPtr< CastFromType, Mode > const& InWeakPtr)
{
	return TWeakPtr< CastToType, Mode >(InWeakPtr, SharedPointerInternals::FConstCastTag());
}

template <class CastToType, class CastFromType, ESPMode Mode>
[[nodiscard]] UE_REWRITE TWeakPtr<CastToType, Mode> ConstCastWeakPtr(TWeakPtr<CastFromType, Mode>&& InWeakPtr)
{
	return TWeakPtr<CastToType, Mode>{MoveTemp(InWeakPtr), SharedPointerInternals::FConstCastTag{}};
}
...
*/
template <typename ObjectType, ESPMode Mode>
[[nodiscard]] UE_REWRITE uint32 GetTypeHash( const TSharedRef< ObjectType, Mode >& InSharedRef )
{
...
*/
template <typename ObjectType, ESPMode Mode>
[[nodiscard]] UE_REWRITE uint32 GetTypeHash( const TSharedPtr< ObjectType, Mode >& InSharedPtr )
{
...
*/
template <typename ObjectType, ESPMode Mode>
[[nodiscard]] UE_REWRITE uint32 GetTypeHash( const TWeakPtr< ObjectType, Mode >& InWeakPtr )
{
