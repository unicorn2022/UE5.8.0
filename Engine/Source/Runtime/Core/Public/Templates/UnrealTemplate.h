...
/**
 * Forwards to an unqualified GetData(), but can be called from within a scope
 * where GetData() is a member and hides any other
 */
template <typename T>
[[nodiscard]] UE_REWRITE constexpr auto GetDataHelper(T&& Arg) -> decltype(GetData((T&&)Arg))
{
	return GetData((T&&)Arg);
}
...
#define FGuardValue_Bitfield(ReferenceValue, NewValue) \
	const bool UE_JOIN(TempBitfield, __LINE__) = ReferenceValue; \
	ReferenceValue = NewValue; \
	const TGuardValue_Bitfield_Cleanup<TFunction<void()>> UE_JOIN(TempBitfieldCleanup, __LINE__)([&](){ ReferenceValue = UE_JOIN(TempBitfield, __LINE__); });
...
/**
 * ForwardLike will cast a reference to an object with the cv qualifiers and value category
 * of another type.  This allows perfect forwarding of subobjects with the same cvrefs as
 * the outer type.  Qualifiers and refs from each type are composed, so const will never
 * be cast away, nor will an lvalue be cast to an rvalue.
 *
 * Example:
 *
 * template <typename T>
 * void GetInnerA(T&& Outer)
 * {
 *     // Has type const TypeOfA& if Outer is a const lvalue.
 *     // Has type TypeOfA&& if Outer is an rvalue.
 *     // etc.
 *     auto&& OuterA = ForwardLike<T>(Outer.A);
 * }
 */
template <typename T, typename U>
UE_INTRINSIC_CAST UE_REWRITE decltype(auto) ForwardLike(U&& Obj)
{
	return (TCopyQualifiersAndRefsFromTo_T<T&&, std::remove_reference_t<U>>)Obj;
}
