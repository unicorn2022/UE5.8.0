...
//
// The bug is reported here: https://developercommunity.visualstudio.com/t/Bad-codegen-when-a-function-returns-an-a/10984053
...
namespace UE::Core::Private::Tuple
{
	...
	template <typename TupleType, uint32 Num>
	struct TTuplePopFront
	{
	};

	template <typename Type0, typename... Types, uint32 Num>
	struct TTuplePopFront<TTuple<Type0, Types...>, Num> : TTuplePopFront<TTuple<Types...>, Num - 1>
	{
	};

	template <typename Type0, typename... Types>
	struct TTuplePopFront<TTuple<Type0, Types...>, 0>
	{
		using Type = TTuple<Type0, Types...>;
	};

	template <>
	struct TTuplePopFront<TTuple<>, 0>
	{
		using Type = TTuple<>;
	};

	template <typename TupleType, uint32 Num> struct TTuplePopFront<const          TupleType, Num> { using Type = const          typename TTuplePopFront<TupleType, Num>::Type; };
	template <typename TupleType, uint32 Num> struct TTuplePopFront<      volatile TupleType, Num> { using Type =       volatile typename TTuplePopFront<TupleType, Num>::Type; };
	template <typename TupleType, uint32 Num> struct TTuplePopFront<const volatile TupleType, Num> { using Type = const volatile typename TTuplePopFront<TupleType, Num>::Type; };

	template <typename TupleType, uint32 Num, typename ResultTuple = TTuple<>>
	struct TTuplePopBack
	{
	};

	template <typename Type0, typename... Types, uint32 Num, typename... ResultTypes>
	struct TTuplePopBack<TTuple<Type0, Types...>, Num, TTuple<ResultTypes...>> : TTuplePopBack<TTuple<Types...>, Num - 1, TTuple<ResultTypes..., Type0>>
	{
	};

	template <typename Type0, typename... Types, typename... ResultTypes>
	struct TTuplePopBack<TTuple<Type0, Types...>, 0, TTuple<ResultTypes...>>
	{
		using Type = TTuple<ResultTypes...>;
	};

	template <typename... ResultTypes>
	struct TTuplePopBack<TTuple<>, 0, TTuple<ResultTypes...>>
	{
		using Type = TTuple<ResultTypes...>;
	};

	template <typename TupleType, uint32 Num> struct TTuplePopBack<const          TupleType, Num> { using Type = const          typename TTuplePopBack<TupleType, Num>::Type; };
	template <typename TupleType, uint32 Num> struct TTuplePopBack<      volatile TupleType, Num> { using Type =       volatile typename TTuplePopBack<TupleType, Num>::Type; };
	template <typename TupleType, uint32 Num> struct TTuplePopBack<const volatile TupleType, Num> { using Type = const volatile typename TTuplePopBack<TupleType, Num>::Type; };
	...
	struct TGetElementByIndex
	{
		...
		constexpr auto operator()(TupleType&& Tuple) const -> decltype(((TupleType&&)Tuple).template Get<Index>())
		{
			return ((TupleType&&)Tuple).template Get<Index>();
		}
	};

	template <typename Type>
	struct TGetElementByType
	{
		template <typename TupleType>
			requires (TIsTuple_V<std::decay_t<TupleType>>)
		constexpr auto operator()(TupleType&& Tuple) const -> decltype(((TupleType&&)Tuple).template Get<Type>())
		{
			return ((TupleType&&)Tuple).template Get<Type>();
		}
	};
}
...
/**
 * Traits class which pops a number of element types from the start of a tuple type.
 *
 * Example: TTuplePopFront<TTuple<A, B, C, D, E>, 2>::Type == TTuple<C, D, E>
 */
template <typename TupleType, uint32 Num = 1>
struct TTuplePopFront : UE::Core::Private::Tuple::TTuplePopFront<TupleType, Num>
{
};

template <typename TupleType, uint32 Num = 1>
using TTuplePopFront_T = typename TTuplePopFront<TupleType, Num>::Type;


/**
 * Traits class which pops a number of element types from the end of a tuple type.
 *
 * Example: TTuplePopBack<TTuple<A, B, C, D, E>, 2>::Type == TTuple<A, B, C>
 */
template <typename TupleType, uint32 Num = 1>
struct TTuplePopBack : UE::Core::Private::Tuple::TTuplePopBack<TupleType, TTupleArity<TupleType>::Value - Num>
{
};

template <typename TupleType, uint32 Num = 1>
using TTuplePopBack_T = typename TTuplePopBack<TupleType, Num>::Type;
...
// Overload for std::swap - to support swapping of references
template <typename... Types>
void swap(const TTuple<Types&...>& Lhs, const TTuple<Types&...>& Rhs)
{
	VisitTupleElements(
		[](auto& LhsElement, auto& RhsElement)
		{
			using std::swap;
			swap(LhsElement, RhsElement);
		},
		Lhs,
		Rhs
	);
...
template <typename... Types>
UE_REWRITE void swap(const TTuple<Types&...>& Lhs, TTuple<Types&...>& Rhs)
{
	std::swap(Lhs, AsConst(Rhs));
}
template <typename... Types>
UE_REWRITE void swap(TTuple<Types&...>& Lhs, const TTuple<Types&...>& Rhs)
{
	std::swap(AsConst(Lhs), Rhs);
}
template <typename... Types>
UE_REWRITE void swap(TTuple<Types&...>& Lhs, TTuple<Types&...>& Rhs)
{
	std::swap(AsConst(Lhs), AsConst(Rhs));
}

// Overload for UE Swap - to support swapping of references
template <typename... Types>
void Swap(const TTuple<Types&...>& Lhs, const TTuple<Types&...>& Rhs)
{
	VisitTupleElements(
...
UE_REWRITE void Swap(const TTuple<Types&...>& Lhs, TTuple<Types&...>& Rhs)
{
	Swap(Lhs, AsConst(Rhs));
}
template <typename... Types>
UE_REWRITE void Swap(TTuple<Types&...>& Lhs, const TTuple<Types&...>& Rhs)
{
	Swap(AsConst(Lhs), Rhs);
}
template <typename... Types>
UE_REWRITE void Swap(TTuple<Types&...>& Lhs, TTuple<Types&...>& Rhs)
{
	Swap(AsConst(Lhs), AsConst(Rhs));
}

template <uint32 Index>
inline constexpr UE::Core::Private::Tuple::TGetElementByIndex<Index> GetElementByIndex;

template <typename Type>
inline constexpr UE::Core::Private::Tuple::TGetElementByType<Type> GetElementByType;
