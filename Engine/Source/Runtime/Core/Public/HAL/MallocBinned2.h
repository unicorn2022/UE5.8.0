...
// Double-free detection for small pool allocations. Writes a pointer-derived canary into freed
// bins and checks for it on subsequent frees. Near-zero overhead (same cache line as free list
// writes). Can be disabled if profiling shows unexpected impact.
#ifndef UE_MB2_DOUBLE_FREE_DETECTION
#	define UE_MB2_DOUBLE_FREE_DETECTION		0
#endif
...
public:
	...
	struct FPoolInfo
	{
		...
		void* AllocateBin(uint32 BinSize);
...
private:
	...
	struct FFreeBlock
	{
		inline FFreeBlock(uint32 InPageSize, uint16 InBinSize)
			: NextFreeBlock(0)
		{
		...
		{
			return (FFreeBlock*)NextFreeBlock;
		}

		FORCEINLINE void SetNextFreeBlock(FFreeBlock* Next)
		{
			NextFreeBlock = uint64(Next);
		}

		inline void* AllocateBin(uint32 BinSize)
		{
			--NumFreeBins;
		...
		constexpr static uint32 MaxBitsForFreeBins = 32 - FMath::CountLeadingZeros((UE_MB2_LARGE_ALLOC - 16) / 16);	// max number of free bins in the 64kb page - 4095, fits into 12 bits
		static_assert(MaxBitsForFreeBins <= 16, "NextFreeBlockIndex should fit in 16 bits");

		uint64 NumFreeBins : 16;		// Number of consecutive free bins here, at least 1
		uint64 NextFreeBlock : 48;		// Next free block in another pool
	};

	struct FPoolHeader : public FFreeBlock
	{
		inline FPoolHeader(uint32 InPageSize, uint16 InBinSize, uint8 InPoolIndex, EBlockCanary InCanary)
			: FFreeBlock(InPageSize, InBinSize)
			, BinSize(InBinSize)
...
#	endif
	...
	static FORCEINLINE FPoolHeader* GetPoolHeaderFromPointer(void* Ptr)
	{
		return (FPoolHeader*)AlignDown(Ptr, UE_MB2_LARGE_ALLOC);
	}

#if UE_MB2_DOUBLE_FREE_DETECTION
	static void SetAndValidateDoubleFreeCanary(void* Ptr);
	static void ClearDoubleFreeCanary(void* Ptr);
	static void ReportSmallDoubleFree(void* Ptr);
#endif
...
public:
	...
	void* MallocNoTLS(SIZE_T Size, uint32 Alignment);
	...
	void FreeNoTLS(void* Ptr);

	inline bool GetSmallAllocationSize(void* Ptr, SIZE_T& SizeOut) const
	{
		if (!IsOSAllocation(Ptr))
		{
			const FPoolHeader* Header = GetPoolHeaderFromPointer(Ptr);
			CanaryTest(Header);
			SizeOut = Header->BinSize;
			return true;
		}
		return false;
	}
	...
	void CanaryFail(const FPoolHeader* Header) const;
	inline void CanaryTest(const FPoolHeader* Header) const
	{
#if BINNED2_FORK_SUPPORT
		// When we support forking there are two valid canary values.
		if (Header->CanaryAndForkState != CurrentCanary && Header->CanaryAndForkState != OldCanary)
#else
