...
class FVirtualAllocator
{
	...
	void FreeVirtualByBlock(void* Ptr, FPerBlockSize& Block, size_t AlignedSize)
	{
		...
		if (!RecycledLinks)
		{
			...
			checkSlow(RecycledLinks); // To suppress a static analysis warning about RecycledLinks being null below.
		}
...
protected:
	...
	virtual uint8* AllocNewVM(size_t AlignedSize)
	{
		...
		if (SpaceConsumed > SpaceConsumedPeak)
		{
			SpaceConsumedPeak = SpaceConsumed;
		}
		return Result;
	}
...
public:
	...
	void GetStats(FVirtualAllocatorStats& OutStats)
	{
		...
		OutStats.VMSpaceConsumedPeak = SpaceConsumedPeak;
