...
#include "HAL/CriticalSection.h"
	...
	#define PLATFORM_SUPPORTS_ALL_THREAD_BACKTRACES (PLATFORM_WINDOWS || PLATFORM_MAC || PLATFORM_UNIX)
#endif
...
public:
	...
	*/
	...
	/** Specifies the type of thread event. */
	enum class EThreadEventType
	{
		Added,
		Removed
	};
	...
	/**
	  * Used internally to broadcast about a real thread object events.
	  * A thread-safe version, broadcast might be done from any thread.
	  * 
	  * @see EThreadEventType
	  * @see AddThread
	  * @see RemoveThread
	  */
	DECLARE_TS_MULTICAST_DELEGATE_ThreeParams(FThreadEvent, const uint32 /*ThreadId*/, class FRunnableThread* /*Thread*/, EThreadEventType /*EventType*/);
	...
	{
		static FString GameThreadName("GameThread");
		static FString RenderThreadName("RenderThread");
...
private:
	...
	/** Broadcasts whenever a thread has been added or removed. See EThreadEventType for details */
