...
#elif UE_MBC_ALLOW_RUNTIME_TWEAKING
#	ifndef UE_MBC_SUPPORT_RUNTIME_TLC_TOGGLE
#		define UE_MBC_SUPPORT_RUNTIME_TLC_TOGGLE		1
#	endif
#endif
...
protected:
	...
	{
		static thread_local FPerThreadFreeBlockLists* ThreadSingleton;
	...
	virtual void ClearAndDisableTLSCachesOnCurrentThread() override
	{
		...
		if (UE_MBC_SUPPORT_RUNTIME_TLC_TOGGLE || GMallocBinnedPerThreadCaches)
		{
			// we need to flush cache even if it's disabled in case of run time tweaking disabled it, but it wasn't cleaned up
	...
	virtual void MarkTLSCachesAsUsedOnCurrentThread() override
	{
		...
		if (UE_MBC_SUPPORT_RUNTIME_TLC_TOGGLE || GMallocBinnedPerThreadCaches)
		{
			// when UE_MBC_SUPPORT_RUNTIME_TLC_TOGGLE is set to 1 we still need to lock cache even if it is disabled, so we will clean it up properly
