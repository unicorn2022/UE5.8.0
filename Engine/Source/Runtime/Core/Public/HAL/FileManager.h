...
public:
	...
	*
	* @param Path	Path to check if it's a symlink
	*
	* @return	True if symlink, false if not or the symlink check is not implemented. 
	*/
	virtual bool IsSymlink(const TCHAR* Path) = 0;
};
