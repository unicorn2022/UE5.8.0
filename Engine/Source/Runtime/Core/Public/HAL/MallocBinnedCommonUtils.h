...
public:
	...
	static void FlushCurrentThreadCache(AllocType& Allocator, bool bNewEpochOnly = false)
	{
		if (typename AllocType::FPerThreadFreeBlockLists* Lists = AllocType::FPerThreadFreeBlockLists::Get())
		{
			if (Lists->UpdateEpoch(Allocator.MemoryTrimEpoch.load(std::memory_order_relaxed)) || !bNewEpochOnly)
			{
				...
				{
					UE_LOGF(LogMemory, Warning, "FMalloc%ls took %6.2fms to wait for mutex AND trim.", Allocator.GetDescriptiveName(), WaitForTrimTime * 1000.0f);
				}
			}
		}
	}
	...
	template <class AllocType>
	static void UpdateStatsCommon(const AllocType& Alloc)
	{
		...
		constexpr int32 NumSmallPools = AllocType::NUM_SMALL_POOLS;
		...
		CSV_CUSTOM_STAT(MallocBinned, RequestedSmallPoolMemoryMB, (float)Alloc.GetTotalAllocatedSmallPoolMemory() / (1024.0f * 1024.0f), ECsvCustomStatOp::Set);
		CSV_CUSTOM_STAT(MallocBinned, TotalAllocatedSmallPoolMemoryMB, (float)Alloc.AllocatedOSSmallPoolMemory.load(std::memory_order_relaxed) / (1024.0f * 1024.0f), ECsvCustomStatOp::Set);
		CSV_CUSTOM_STAT(MallocBinned, RequestedLargeAllocsMemoryMB, (float)Alloc.AllocatedLargePoolMemory.load(std::memory_order_relaxed) / (1024.0f * 1024.0f), ECsvCustomStatOp::Set);
		CSV_CUSTOM_STAT(MallocBinned, TotalAllocatedLargeAllocsMemoryMB, (float)Alloc.AllocatedLargePoolMemoryWAlignment.load(std::memory_order_relaxed) / (1024.0f * 1024.0f), ECsvCustomStatOp::Set);
#endif
	}
};
