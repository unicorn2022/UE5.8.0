...
#if !defined(UE_WITH_CVAR_HELP_TEXT)
	#define UE_WITH_CVAR_HELP_TEXT 1
#endif

#if UE_WITH_CVAR_HELP_TEXT
	#define UE_CVAR_HELP_TEXT_PARAM(HelpText) HelpText
#else
	#define UE_CVAR_HELP_TEXT_PARAM(HelpText) nullptr
#endif
...
enum EConsoleVariableFlags
{
	...
	// Used for temporarily replacing a variable.  THIS IS AN ARRAY TYPE meaning multiple vales with this SetBy are stored in the history
	ECVF_SetByTemp =                0x0F000000,
	// Highest priority used via editor UI or or game/editor interactive console
	ECVF_SetByConsole =				0x10000000,
...
#else
	const TCHAR* GetHelp() const { return TEXT(""); }
	void SetHelp(const TCHAR*) {}
	FText GetDetailedHelp() const { return FText::FromString(TEXT("")); }
#endif
...
public:
	...
	/**
	 * Remove the callback function set to this console variable.
	 */
	virtual void ClearOnChangedCallback(){}
...
public:
	...
	{
		static const FName Name = "ConsoleCommandExecutor";
		return Name;
	}
...
struct IConsoleManager
{
	...
	*/
	...
	/**
	* Create a console variable that delegates get/set to external functions.
	* The Getter is called on every read; the Setter is called on every write.
	* ECVF_RenderThreadSafe is NOT supported with delegate variables.
	*
	* @param Name must not be 0, must not be empty, must not contain spaces
	* @param DefaultValue initial value passed to Setter during registration
	* @param Getter called to read the current value
	* @param Setter called to write a new value
	* @param Help must not be 0, must not be empty
	* @param Flags bitmask combined from EConsoleVariableFlags
	*/
	virtual IConsoleVariable* RegisterDelegatedConsoleVariable(
		const TCHAR* Name, int32 DefaultValue,
		TFunction<int32()> Getter, TFunction<void(const int32&)> Setter,
		const TCHAR* Help, uint32 Flags = ECVF_Default) = 0;

	virtual IConsoleVariable* RegisterDelegatedConsoleVariable(
		const TCHAR* Name, float DefaultValue,
		TFunction<float()> Getter, TFunction<void(const float&)> Setter,
		const TCHAR* Help, uint32 Flags = ECVF_Default) = 0;

	virtual IConsoleVariable* RegisterDelegatedConsoleVariable(
		const TCHAR* Name, bool DefaultValue,
		TFunction<bool()> Getter, TFunction<void(const bool&)> Setter,
		const TCHAR* Help, uint32 Flags = ECVF_Default) = 0;

	virtual IConsoleVariable* RegisterDelegatedConsoleVariable(
		const TCHAR* Name, const FString& DefaultValue,
		TFunction<FString()> Getter, TFunction<void(const FString&)> Setter,
		const TCHAR* Help, uint32 Flags = ECVF_Default) = 0;
	...
	*/
	 ...
	 * @return 0 if the object wasn't found
	 */
	TConsoleVariableData<FString>* FindTConsoleVariableDataString(const TCHAR* Name) const
	{
		IConsoleVariable* P = FindConsoleVariable(Name);

		return P ? P->AsVariableString() : 0;
	}
...
struct TGuardConsoleVariable
{
	...
	IConsoleVariable::FSetContext SetContext;
	
	TGuardConsoleVariable(const TCHAR* ConsoleVariableName, const T& NewValue, EConsoleVariableFlags Flags = ECVF_SetByCode, FName Tag = NAME_None)
		: TGuardConsoleVariable(IConsoleManager::Get().FindConsoleVariable(ConsoleVariableName), NewValue, Flags, Tag)
	{
	...
	TGuardConsoleVariable(IConsoleVariable* ConsoleVariable, const T& NewValue, EConsoleVariableFlags Flags = ECVF_SetByCode, FName Tag = NAME_None)
		: ConsoleVariable(ConsoleVariable)
		, SetContext(Flags, Tag)
	{
		if (ConsoleVariable)
		{
			ConsoleVariable->GetValue(PreviousValue);
			ConsoleVariable->Set(NewValue, SetContext);
		}
	}
	...
	~TGuardConsoleVariable()
	{
		if (ConsoleVariable)
		{
			ConsoleVariable->Set(PreviousValue, SetContext);
		}
	}
};
...
public:
	...
	FAutoConsoleVariableSink(FConsoleCommandDelegate&& InCommand)
	...
	virtual ~FAutoConsoleVariableSink() = default;
...
public:
	...
	virtual ~FAutoConsoleObject()
	{
		...
		const bool bKeepCVarObject = false; // do not keep the console object in memory, as it may cause crashes if we reload the object later
#else
		const bool bKeepCVarObject = true; // do not delete console objects by default as some existing code assumes they're kept
#endif
		IConsoleManager::Get().UnregisterConsoleObject(Target, bKeepCVarObject);
	}
...
{
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, bool DefaultValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, int32 DefaultValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, float DefaultValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, const TCHAR* DefaultValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, bool DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, int32 DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, float DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);
	CORE_API FAutoConsoleVariable(uint32 Flags, const TCHAR* Name, const TCHAR* DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);
public:
	/**
	 * Create a bool console variable
	 * @param Name must not be 0
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, bool DefaultValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	/**
	 * Create a int console variable
	 * @param Name must not be 0
	 * @param Help must not be 0
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, int32 DefaultValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	/**
	 * Create a float console variable
	 * @param Name must not be 0
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, float DefaultValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	/**
	 * Create a string console variable
	 * @param Name must not be 0
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, const TCHAR* DefaultValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	/**
	 * Create a bool console variable
	 * @param Name must not be 0
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, bool DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}
	/**
	 * Create a int console variable
	 * @param Name must not be 0
	 * @param Help must not be 0
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, int32 DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}
	/**
	 * Create a float console variable
	 * @param Name must not be 0
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, float DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}
	/**
	 * Create a string console variable
	 * @param Name must not be 0
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariable(const TCHAR* Name, const TCHAR* DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}

	virtual ~FAutoConsoleVariable() {}
...
/**
 * Disable deprecation warnings for access to cvars for the duration of the current scope, enabling internal use of deprecated cvars without generating warnings / ensures.
 * Analogous to the use case for the pragmas for deprecated code.
 */
struct FCVarDisableDeprecationWarningScope
{
	/**
	 * if bInCondition is false, the scope does nothing such that it can be conditionally used.
	 */
	CORE_API FCVarDisableDeprecationWarningScope(bool bInCondition = true);
	CORE_API ~FCVarDisableDeprecationWarningScope();

	CORE_API static bool IsDisabled();

private:
	const bool bCondition = true;
};

struct FCVarDeprecationArgs
{
	// The first UE version when this was deprecated
	const TCHAR* Version = TEXT("");
...
{
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, int32& RefValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, float& RefValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, bool& RefValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, FString& RefValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, FName& RefValue, const TCHAR* Help);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, int32& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, float& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, bool& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);
	CORE_API FAutoConsoleVariableRef(uint32 Flags, const TCHAR* Name, FString& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback);

	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, int32& RefValue, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, float& RefValue, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, bool& RefValue, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FString& RefValue, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FName& RefValue, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, int32& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, float& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, bool& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags);
	CORE_API FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FString& RefValue, const FConsoleVariableDelegate& Callback, uint32 Flags);
...
public:
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, int32& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, int32& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Flags) {}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, float& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, float& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Flags) {}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, bool& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, bool& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Flags) {}
	/**
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, FString& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FString& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Flags) {}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, FName& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help)) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FName& RefValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Flags) {}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, int32& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, int32& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Callback, Flags) {}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, float& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, float& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Callback, Flags) {}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, bool& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, bool& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Callback, Flags) {}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE FAutoConsoleVariableRef(const TCHAR* Name, FString& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(Flags, Name, RefValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback) {}
	FORCEINLINE FAutoConsoleVariableRef(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, FString& RefValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: FAutoConsoleVariableRef(DeprecationArgs, Name, RefValue, Callback, Flags) {}

	virtual ~FAutoConsoleVariableRef()
	{
		AsVariable()->ClearOnChangedCallback();
	}
...
{
	FORCENOINLINE TAutoConsoleVariable(uint32 Flags, const TCHAR* Name, const T& DefaultValue, const TCHAR* Help)
		: TAutoConsoleVariable(IConsoleManager::Get().RegisterConsoleVariable(Name, DefaultValue, Help, Flags))
	{
	}

	CORE_API TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const T& DefaultValue, uint32 Flags);

	FORCENOINLINE TAutoConsoleVariable(uint32 Flags, const TCHAR* Name, const T& DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback)
		: TAutoConsoleVariable(Flags, Name, DefaultValue, Help)
	{
		AsVariable()->SetOnChangedCallback(Callback);
	}

	FORCENOINLINE TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const T& DefaultValue, const FConsoleVariableDelegate& Callback, uint32 Flags)
		: TAutoConsoleVariable(DeprecationArgs, Name, DefaultValue, Flags)
	{
		AsVariable()->SetOnChangedCallback(Callback);
	}

	FORCENOINLINE TAutoConsoleVariable(uint32 Flags, const TCHAR* Name, const T& DefaultValue, TFunction<T()> Getter, TFunction<void(const T&)> Setter, const TCHAR* Help)
		: TAutoConsoleVariable(IConsoleManager::Get().RegisterDelegatedConsoleVariable(Name, DefaultValue, MoveTemp(Getter), MoveTemp(Setter), Help, Flags))
	{
	}
...
public:
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE TAutoConsoleVariable(const TCHAR* Name, const T& DefaultValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: TAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help))
	{
	}

	FORCEINLINE TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const T& DefaultValue, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: TAutoConsoleVariable(DeprecationArgs, Name, DefaultValue, Flags)
	{
	}
	 ...
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE TAutoConsoleVariable(const TCHAR* Name, const T& DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: TAutoConsoleVariable(Flags, Name, DefaultValue, UE_CVAR_HELP_TEXT_PARAM(Help), Callback)
	{
	}

	FORCEINLINE TAutoConsoleVariable(const FCVarDeprecationArgs& DeprecationArgs, const TCHAR* Name, const T& DefaultValue, const TCHAR* Help, const FConsoleVariableDelegate& Callback, uint32 Flags = ECVF_Default)
		: TAutoConsoleVariable(DeprecationArgs, Name, DefaultValue, Callback, Flags)
	{
	}
	 ...
	 *
	 * @param Name must not be 0
	 * @param DefaultValue initial value; the Getter is authoritative after construction
	 * @param Getter called to read the current value
	 * @param Setter called to write a new value
	 * @param Help must not be 0
	 * @param Flags bitmask combined from EConsoleVariableFlags
	 */
	FORCEINLINE TAutoConsoleVariable(const TCHAR* Name, const T& DefaultValue, TFunction<T()> Getter, TFunction<void(const T&)> Setter, const TCHAR* Help, uint32 Flags = ECVF_Default)
		: TAutoConsoleVariable(Flags, Name, DefaultValue, MoveTemp(Getter), MoveTemp(Setter), UE_CVAR_HELP_TEXT_PARAM(Help))
	{
	}

	virtual ~TAutoConsoleVariable()
	{
		AsVariable()->ClearOnChangedCallback();
	}
...
private:
	TConsoleVariableData<T>* Ref = nullptr;
};
...
{
	CORE_API FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandDelegate& Command);
	CORE_API FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsDelegate& Command);
	CORE_API FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithOutputDeviceDelegate& Command);
	CORE_API FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command);
	CORE_API FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldDelegate& Command);
	CORE_API FAutoConsoleCommand(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command);
public:
	/**
	 * Register a console command that takes no arguments
	 *
	 ...
	 * @param	Flags		Optional flags bitmask
	 */
	FORCEINLINE FAutoConsoleCommand(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommand(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}
	 ...
	 * @param	Flags		Optional flags bitmask
	 */
	FORCEINLINE FAutoConsoleCommand(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommand(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}
	...
	* @param	Flags		Optional flags bitmask
	*/
	FORCEINLINE FAutoConsoleCommand(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithOutputDeviceDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommand(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}
    ...
    * @param	Flags		Optional flags bitmask
    */
	FORCEINLINE FAutoConsoleCommand(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommand(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}
	...
	* @param	Flags		Optional flags bitmask
	*/
	FORCEINLINE FAutoConsoleCommand(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommand(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}
	...
	* @param	Flags		Optional flags bitmask
	*/
	FORCEINLINE FAutoConsoleCommand(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommand(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}

	virtual ~FAutoConsoleCommand() {}
};
#else
...
{
	CORE_API FAutoConsoleCommandWithWorld(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldDelegate& Command);
public:
	/**
	 * Register a console command that takes a world argument
	 *
	 ...
	 * @param	Flags		Optional flags bitmask
	 */
	FORCEINLINE FAutoConsoleCommandWithWorld(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommandWithWorld(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}

	virtual ~FAutoConsoleCommandWithWorld() {}
};
...
{
	CORE_API FAutoConsoleCommandWithWorldAndArgs(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldAndArgsDelegate& Command);
public:	
	/**
	 ...
	 * @param	Flags		Optional flags bitmask
	 */
	FORCEINLINE FAutoConsoleCommandWithWorldAndArgs(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldAndArgsDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommandWithWorldAndArgs(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}

	virtual ~FAutoConsoleCommandWithWorldAndArgs() {}
};
...
{
	CORE_API FAutoConsoleCommandWithArgsAndOutputDevice(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command);
public:
	
	FORCEINLINE FAutoConsoleCommandWithArgsAndOutputDevice(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommandWithArgsAndOutputDevice(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}

	virtual ~FAutoConsoleCommandWithArgsAndOutputDevice() {}
};
...
{
	CORE_API FAutoConsoleCommandWithOutputDevice(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithOutputDeviceDelegate& Command);
public:
	/**
	 * Register a console command that takes an output device
	 *
	 ...
	 * @param	Flags		Optional flags bitmask
	 */
	FORCEINLINE FAutoConsoleCommandWithOutputDevice(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithOutputDeviceDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommandWithOutputDevice(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}

	virtual ~FAutoConsoleCommandWithOutputDevice() {}
};
...
{
	CORE_API FAutoConsoleCommandWithWorldArgsAndOutputDevice(uint32 Flags, const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command);
public:
	/**
	 * Register a console command that takes an output device
	 *
	 ...
	 * @param	Flags		Optional flags bitmask
	 */
	FORCEINLINE FAutoConsoleCommandWithWorldArgsAndOutputDevice(const TCHAR* Name, const TCHAR* Help, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command, uint32 Flags = ECVF_Default)
		: FAutoConsoleCommandWithWorldArgsAndOutputDevice(Flags, Name, UE_CVAR_HELP_TEXT_PARAM(Help), Command) {}

	virtual ~FAutoConsoleCommandWithWorldArgsAndOutputDevice() {}
};
...
/**
 * A class that can be used to deprecate a cvar, but without making a new one to replace it.
 */
template <typename T>
class TAutoConsoleVariableDeprecated_NoReplace
{
public:
	/**
	 * @param Name The name of this cvar
	 * @param DefaultValue The value this cvar has by default
	 * @param DeprecatedAtVersion The first UE version when this was deprecated
	 * @param UsageBehavior How to handle uses of the deprecated cvar  - defaults to ensure
	 * @param DeprecationMessage Optional deprecation message - defaults to empty, auto-generated.
	 */
	CORE_API TAutoConsoleVariableDeprecated_NoReplace(const TCHAR* Name, const T& DefaultValue, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior=EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
};

/**
 * A class that can be used to deprecate a ConsoleCommand, but without making a new one to replace it.
 */
class FAutoConsoleCommandDeprecated_NoReplace
{
public:
	/**
	 ...
	 * @param DeprecationMessage Optional deprecation message - defaults to empty, auto-generated.
	 */
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithArgsDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithOutputDeviceDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithArgsAndOutputDeviceDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithWorldDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithWorldArgsAndOutputDeviceDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
	CORE_API FAutoConsoleCommandDeprecated_NoReplace(const TCHAR* Name, const FConsoleCommandWithWorldAndArgsDelegate& Command, const TCHAR* DeprecatedAtVersion, EShadowCVarBehavior UsageBehavior = EShadowCVarBehavior::Ensure, const TCHAR* DeprecationMessage = TEXT(""));
};
...
{
	CORE_API TAutoRangedConsoleVariable(IConsoleVariable* Parent, const TCHAR* Name, T DefaultValue, T MinValue, T MaxValue, const TCHAR* Help, uint32 Flags);
public:
	FORCEINLINE TAutoRangedConsoleVariable(const TCHAR* Name, T DefaultValue, T MinValue, T MaxValue, const TCHAR* Help, uint32 Flags = ECVF_Default, IConsoleVariable* Parent = nullptr)
		: TAutoRangedConsoleVariable(Parent, Name, Forward(DefaultValue), Forward(MinValue), Forward(MaxValue), UE_CVAR_HELP_TEXT_PARAM(Help), Flags) {}
};
