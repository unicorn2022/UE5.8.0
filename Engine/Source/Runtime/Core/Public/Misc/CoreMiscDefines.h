...
#define WARNING_LOCATION(Line) __FILE__ "(" UE_STRINGIZE(Line) ")"
...
//
// due to limitations of resource compiler.
#ifndef RC_INVOKED
...
// Push and pop macro definitions
#ifdef __clang__
	#define UE_PUSH_MACRO(name) _Pragma(UE_STRINGIZE(push_macro(name)))
	#define UE_POP_MACRO(name) _Pragma(UE_STRINGIZE(pop_macro(name)))
#else
	...
	#define ANONYMOUS_VARIABLE( Name ) UE_JOIN(Name, __COUNTER__)
#else
	// Created a variable with a unique name.
	// Less reliable than the __COUNTER__ version.
	#define ANONYMOUS_VARIABLE( Name ) UE_JOIN(Name, __LINE__)
#endif
 ...
 */
	...
	#define UE_DEPRECATED_FORGAME UE_EMPTY_FUNCTION
#endif
 ...
 */
	...
	#define UE_DEPRECATED_HEADER(Version, Message)
    	...
    	#define UE_DEPRECATED_HEADER(Version, Message) __pragma(message(__FILE__ "(" UE_STRINGIZE(__LINE__) "): error C4996: " Message " Please update your code to the new API before upgrading to the next release, otherwise your project will no longer compile."))
    #else
    	#define UE_DEPRECATED_HEADER(Version, Message) __pragma(message(__FILE__ "(" UE_STRINGIZE(__LINE__) "): warning C4996: " Message " Please update your code to the new API before upgrading to the next release, otherwise your project will no longer compile."))
    #endif
#else
	#define UE_DEPRECATED_HEADER(Version, Message) _Pragma(UE_STRINGIZE(message(Message " Please update your code to the new API before upgrading to the next release, otherwise your project will no longer compile.")))
#endif
...
#define UE_STATIC_DEPRECATE(Version, bExpression, Message) \
	struct UE_JOIN(FDeprecationMsg_, __LINE__) { \
	...
	enum class UE_JOIN(EDeprecationMsg_, __LINE__) { Value = UE_JOIN(FDeprecationMsg_, __LINE__)::condition(TStaticDeprecateExpression<!!(bExpression)>()) }
...
#define UE_STATIC_ASSERT_WARN(bExpression, Message) \
	struct UE_JOIN(FStaticWarningMsg_, __LINE__) { \
		[[deprecated(Message)]] \
		static constexpr int condition(TStaticDeprecateExpression<true>) { return 1; } \
		static constexpr int condition(TStaticDeprecateExpression<false>) { return 1; } \
	}; \
	enum class UE_JOIN(EStaticWarningMsg_, __LINE__) { Value = UE_JOIN(FStaticWarningMsg_, __LINE__)::condition(TStaticDeprecateExpression<!(bExpression)>()) }
