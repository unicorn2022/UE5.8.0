...
// Note: Threshold of 0 disables the timeguard
  ...
  #define LIGHTWEIGHT_TIME_GUARD_BEGIN( Name, ThresholdMS ) \
	float UE_JOIN(__TimeGuard_ThresholdMS_, Name) = ThresholdMS; \
	uint64 UE_JOIN(__TimeGuard_StartCycles_, Name) = ( ThresholdMS > 0.0f ) ? FPlatformTime::Cycles64() : 0;

  #define LIGHTWEIGHT_TIME_GUARD_END( Name, NameStringCode ) \
	if ( UE_JOIN(__TimeGuard_ThresholdMS_, Name) > 0.0f ) \
	{\
		float UE_JOIN(__TimeGuard_MSElapsed_,Name) = FPlatformTime::ToMilliseconds64( FPlatformTime::Cycles64() - UE_JOIN(__TimeGuard_StartCycles_,Name) ); \
		if ( UE_JOIN(__TimeGuard_MSElapsed_,Name) > UE_JOIN(__TimeGuard_ThresholdMS_, Name) ) \
		{ \
			FString ReportName = NameStringCode; \
			UE_LOGF(LogCore, Warning, "LIGHTWEIGHT_TIME_GUARD: %ls - %ls took %.2fms!", TEXT(#Name), *ReportName, UE_JOIN(__TimeGuard_MSElapsed_,Name)); \
		} \
	}
#else
  #define LIGHTWEIGHT_TIME_GUARD_BEGIN( Name, ThresholdMS ) 
