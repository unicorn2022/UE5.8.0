...
public:
	...
	CORE_API ~FQueuedThreadPoolWrapper() override;
	...
	/**
	 *  Return the estimated memory currently used by this thread pool, if supported, or -1 if not.
	 */
	virtual int64 GetEstimatedMemory() const
	{
		return -1ll;
	}
...
protected:
	...
	private:
		...
		FReturnedRefCountValue Release() const override
		{
			...
			return FReturnedRefCountValue{ Refs };
		}

		FReturnedRefCountValue GetRefCount() const override
		{
			return FReturnedRefCountValue{ static_cast<uint32>(NumRefs.GetValue()) };
		}
	...
	// Can be overridden to ignore specific priority levels if required.
	CORE_API virtual bool CanSchedule(EQueuedWorkPriority Priority) const;
private:
...
/** ThreadPool wrapper implementation allowing to schedule thread-pool tasks on the task graph.
  */
class UE_DEPRECATED(5.8, "FQueuedThreadPoolTaskGraphWrapper is obsolete and will be removed") FQueuedThreadPoolTaskGraphWrapper final : public FQueuedThreadPool
{
public:
	/**
