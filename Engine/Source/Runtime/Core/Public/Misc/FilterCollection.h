...
public:
	...
	{
		for (const TSharedPtr<IFilter<ItemType>>& Filter : ChildFilters)
		{
	...
	using RangedForIteratorType = TArray<TSharedPtr<IFilter<ItemType>>>::RangedForIteratorType;
	using RangedForConstIteratorType = TArray<TSharedPtr<IFilter<ItemType>>>::RangedForConstIteratorType;

	/** STL-like iterators to enable range-based for loop support. */
	[[nodiscard]] UE_REWRITE constexpr RangedForIteratorType      begin()       { return ChildFilters.begin(); }
	[[nodiscard]] UE_REWRITE constexpr RangedForConstIteratorType begin() const { return ChildFilters.begin(); }
	[[nodiscard]] UE_REWRITE constexpr RangedForIteratorType      end()         { return ChildFilters.end(); }
	[[nodiscard]] UE_REWRITE constexpr RangedForConstIteratorType end()   const { return ChildFilters.end(); }
