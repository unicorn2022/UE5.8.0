...
#define UE_RETURN_ON_ERROR(Condition, Message) const bool UE_JOIN(UE____bCondition_Line_, __LINE__) = (Condition); AddErrorIfFalse(UE_JOIN(UE____bCondition_Line_, __LINE__), (Message)); if(!UE_JOIN(UE____bCondition_Line_, __LINE__)) return false
#endif
...
public:
	...
	virtual ~FThreadedAutomationLatentCommand() = default;
...
public:
	/* virtual destructor */
	virtual ~IAutomationNetworkCommand() = default;
...
private:
	...
	TQueue<TSharedPtr<IAutomationLatentCommand>, EQueueMode::SingleThreaded> LatentCommands;

	/** Queue of deferred commands */
	TQueue<TSharedPtr<IAutomationNetworkCommand>, EQueueMode::SingleThreaded> NetworkCommands;
...
public:
	...
	FAutomationTestTags(const FString& InTestFullName, const FString& InTags)
	{
		...
		if (!Registered)
		{
			UE_LOGF(LogAutomationTestFramework, Warning, "Tag registration already exists for test '%ls', existing value was not overridden", *TestFullName);
		}
	}
...
public:
	...
	struct FHasToString
	{
		...
		auto Requires(ValueType& InElem) -> decltype(InElem.ToString());
	};

	template<typename ValueType> 
	inline void ReportError(const TCHAR* What, const ValueType& Actual, const ValueType& Expected)
	{
		if constexpr (TModels_V<FHasToString, ValueType>)
		{
			ReportError(What, *Actual.ToString(), *Expected.ToString());
		}
		else
		{
	...
	template<> 
	inline void ReportError(const TCHAR* What, const FString& Actual, const FString& Expected)
	{
		AddError(FString::Printf(TEXT("%s: The two values are not equal (actual=%s, expected=%s)."), What, *Actual, *Expected));
	}
	...
	inline bool TestEqual(const TCHAR* What, const ValueType& Actual, const ValueType& Expected)
	{
		if (Actual != Expected)
		{
			ReportError(What, Actual, Expected);
			return false;
		}
		return true;
	}
	...
	CORE_API bool TestBetweenExclusive(const TCHAR* What, const int32 Actual, const int32 ExpectedLow, const int32 ExpectedHigh);
	CORE_API bool TestBetweenExclusive(const TCHAR* What, const int64 Actual, const int64 ExpectedLow, const int64 ExpectedHigh);
	CORE_API bool TestBetweenInclusive(const TCHAR* What, const int32 Actual, const int32 ExpectedLow, const int32 ExpectedHigh);
	CORE_API bool TestBetweenInclusive(const TCHAR* What, const int64 Actual, const int64 ExpectedLow, const int64 ExpectedHigh);

	bool TestBetweenExclusive(const FString& What, const int32 Actual, const int32 ExpectedLow, const int32 ExpectedHigh)
	{
		return TestBetweenExclusive(*What, Actual, ExpectedLow, ExpectedHigh);
	}

	bool TestBetweenExclusive(const FString& What, const int64 Actual, const int64 ExpectedLow, const int64 ExpectedHigh)
	{
		return TestBetweenExclusive(*What, Actual, ExpectedLow, ExpectedHigh);
	}

	bool TestBetweenInclusive(const FString& What, const int32 Actual, const int32 ExpectedLow, const int32 ExpectedHigh)
	{
		return TestBetweenInclusive(*What, Actual, ExpectedLow, ExpectedHigh);
	}

	bool TestBetweenInclusive(const FString& What, const int64 Actual, const int64 ExpectedLow, const int64 ExpectedHigh)
	{
		return TestBetweenInclusive(*What, Actual, ExpectedLow, ExpectedHigh);
	}
...
private:
	...
	public:
		...
		virtual ~FSingleExecuteLatentCommand() = default;
	...
	public:
		...
		virtual ~FUntilDoneLatentCommand() = default;
		...
		virtual bool Update() override
		{
			...
			else if ((FDateTime::UtcNow() >= StartedRunning + Timeout) && !FPlatformMisc::IsDebuggerPresent())
			{
				Reset();
	...
	public:
		...
		virtual ~FAsyncUntilDoneLatentCommand() = default;
	...
	public:
		...
		virtual ~FAsyncLatentCommand() = default;
...
public:
	...
	void xDescribe(const TArray<FString> InDescriptions, TFunction<void(int32 /*Iteration*/)> DoWork)
	{
		//disabled
	}
...
#define IMPLEMENT_SIMPLE_AUTOMATION_TEST_PRIVATE( TClass, TBaseClass, PrettyName, TFlags, FileName, LineNumber ) \
	...
	protected: \
		...
		AUTORTFM_DISABLE_IF(!EnumHasAnyFlags(TFlags, EAutomationTestFlags::SupportsAutoRTFM)) \
		virtual bool RunTest(const FString& Parameters) override; \
...
#define BEGIN_DEFINE_SPEC_PRIVATE( TClass, PrettyName, TFlags, FileName, LineNumber ) \
	class AUTORTFM_DISABLE_IF(!EnumHasAnyFlags(TFlags, EAutomationTestFlags::SupportsAutoRTFM)) TClass : public FAutomationSpecBase \
	{ \
	public: \
		TClass( const FString& InName ) \
		: FAutomationSpecBase( InName, false ) {\
...
public:
	...
	virtual ~FFunctionLatentCommand() = default;
...
public:
	...
	FUntilCommand(TFunction<bool()> InCallback, float InTimeout = 5.0f)
		: Callback(MoveTemp(InCallback))
		, TimeoutCallback([]() { return true; })
		, Timeout(InTimeout)
	{}
...
{
public:
	virtual ~IAutomationLatentCommandWithRetriesAndDelays() = default;
	...
	virtual void CommandFailedDueToError(const FString& ErrorMessage)
	{
		...
		UE_LOGF(LogLatentCommands, Error, "%ls", *ErrorMessage);
	...
	virtual bool Update() override
	{
		...
		if (bHasUnlimitedRetries)
		{
			UE_LOGF(LogLatentCommands, Log, "%ls Executing Attempt %d.", *GetTestAndCommandName(), CurrentIteration);
		}
		else
		{
			UE_LOGF(LogLatentCommands, Log, "%ls Executing Attempt %d of %d.", *GetTestAndCommandName(), CurrentIteration, MaxRetries);
		}

		if (Execute())
		{
			// completion log message, logs total time taken.
			UE_LOGF(LogLatentCommands, Log, "%ls Completed Successfully, total run time: %f seconds.", *GetTestAndCommandName(), GetCurrentRunTime());
...
protected:
		...
		, DelayTimeInSeconds(InWaitTimeBetweenRuns)
	{
		...
		{
			// Must log here so that Gauntlet can also pick up the error for its report. Otherwise, it will only show up in the Automation log.
			UE_LOGF(LogLatentCommands, Warning,
				"%ls has been set to Unlimited retries. Will be using MaxTotalRunTime to prevent " "running forever. Default time is set at 300 seconds. If this is not enough time, " "make sure to override this in your Execute() loop.", *GetTestAndCommandName());
		}
	}
