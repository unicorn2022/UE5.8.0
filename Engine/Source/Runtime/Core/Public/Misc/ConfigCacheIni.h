...
using FConfigSystemMutex = UE::FTransactionallySafeSharedRecursiveMutex;
...
public:
	...
	bool bIsUnloadingPlugin = false;
	// if this is true and bTrackLoadedFiles is set, then we ignore files in LoadedFiles array
	bool bSkipLoadingAlreadyLoadedFiles = false;
...
private:
	...
	// thread safety for reading while something is modifying this file
	FConfigSystemMutex SharedMutex;
...
public:
	...
	CORE_API bool Combine(const FString& Filename, bool bHandleSymbolCommands = true);
	CORE_API void CombineFromBuffer(const FString& Buffer, const FString& FileHint, bool bHandleSymbolCommands = true);
...
private:
	 ...
	 * @param OriginalContents - [optional] If set, and the string matches the generated string to write out, we skip the write to disk (it will return true)
	 * @return TRUE if the write was successful
	 */
	bool WriteInternal(const FString& Filename, bool bDoRemoteWrite, TMap<FString, FString>& InOutSectionTexts, const TArray<FString>& InSectionOrder, FString* OriginalContents);
...
// this is the ordering within dynamic layers - for any given priority, the order is indeterminate (controlled by discovery of the 
// file on disk, or pulled down from hotfix server, etc)
enum class DynamicLayerPriority : uint16
{
	// unknown priority, lowest priority
	Unknown = 0,
	// plugins built into UE (a project can disable them, but these may want to override settings in a project plugin)
	EnginePlugin = 10,
	Plugin UE_DEPRECATED(5.8, "DynamicLayerPriority::Plugin is deprecated, use EnginePlugin or ProjectPlugin instead") = 20,
	// plugins in Project/Plugins, or external locations, that the project has enabled
	ProjectPlugin = 30,
	// this special layer allows for a .ini file specified in the project to override any plugin settings, just in cast that is needed
	ProjectPluginOverrides = 49,
	// GameFeaturePlugins tend to come and go, and expect to override other plugin settings
	GameFeature = 50,
	// hotfixes are generally going to the highest priority to be able to override any setting
	Hotfix = 80,
};
...
public:
	 ...
	 */
	CORE_API bool AddDynamicLayerToHierarchy(const FString& Filename, FConfigModificationTracker* ModificationTracker=nullptr, const TSet<FString>* GlobalConfigFileCache=nullptr, const TSet<FString>* PluginConfigFileCache=nullptr);
	CORE_API bool AddDynamicLayersToHierarchy(const TArray<FDynamicLayerInfo>& Layers, FConfigModificationTracker* ModificationTracker=nullptr, const TSet<FString>* GlobalConfigFileCache=nullptr, const TSet<FString>* PluginConfigFileCache=nullptr, bool bForceFullDynamicLayerUpdate=false);
	...
	/**
	 * Updates the inactive timer to the current time
	 */
	CORE_API void UpdateInactiveTimer();
...
private:
	...
	TSAN_ATOMIC(double) InactiveTimer = -1;
};
...
public:
	...
	*/
	...
	/** Same as FindBranch, but only if the provided tag is found for one of its dynamic layers */
	CORE_API FConfigBranch* FindBranchWithDynamicLayerTag(FName BaseIniName, FName Tag);
	...
	CORE_API bool Flush(bool bRemoveFromCache, const FString& Filename=TEXT(""));
