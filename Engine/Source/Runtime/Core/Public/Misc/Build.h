...
// Enables the universal placeholders feature, which allows all imported objects to be replaced with 
// placeholder objects. This flag is internal and will dispear in the future without warning.
// It is used to facilitate fast visualization of tagged property .uasset data by satisfying the 
// following needs:
// 
//	1.	Assume no imported packages need to actually be loaded, and can therefore be universally mocked, 
//		hence 'universal placeholders' - see FLinkerLoad::CreateUniversalPlaceholders
//	2.	Provide safe behavior to placeholders which would need their type to 'emerge' from usage. 
//		Currently placeholders are mocked as plain objects, but that cannot accomodate native types with 
//		native invariants - see ObjectPtr.h
//	3.	Relatedly avoids discarding placeholders that don't conform to emergent non-native type 
//		constraints see FLinkerLoad::operator<<(FObjectPtr&)
//	4.	Relatedly avoids hashing of placeholder types that are not hashable - see PropertySet.cpp,
//		PropertyMap.cpp
//  5.	Assumes a read operation, avoiding expensive mutators - see PropertyBagRepository.cpp
//	6.	Allows serialization routines to expect unreachable objects (in the long run all loaded 
//		data should be made reachable) - see JsonStringifyImpl.cpp
//
// All of these usages will be accomodated more elegantly by future work. Do not use this define.
#ifndef UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
	#define UE_INTERNAL_UNIVERSAL_PLACEHOLDERS 0
#endif

// Universal placeholders are incompatible with full editor functionality, preventing use in that
// configuration for safety:
#if UE_EDITOR && UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
	#error UE_INTERNAL_UNIVERSAL_PLACEHOLDERS cannot be used in the full UE_EDITOR
#endif

// Universal placeholders are also incompatible with pure cooked configurations, as universal placeholders
// rely on tagged serialization, IDOs, placeholders and other editor only FLinkerLoad functionality:
#if !WITH_EDITOR && UE_INTERNAL_UNIVERSAL_PLACEHOLDERS
	#error UE_INTERNAL_UNIVERSAL_PLACEHOLDERS cannot be used in cooked configurations
#endif

// This define adds more robust bounds checking to FLinkerLoad:
#if WITH_EDITOR
	#define UE_INTERNAL_LINKER_BOUNDS 1
#else 
	#define UE_INTERNAL_LINKER_BOUNDS 0
#endif
