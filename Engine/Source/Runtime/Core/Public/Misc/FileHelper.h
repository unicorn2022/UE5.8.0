...
struct FFileHelper
{
	...
	struct FLoadFileAsyncOptions
	{
		...
		IPlatformFile::EOpenReadFlags FileOpenFlags = IPlatformFile::EOpenReadFlags::None;
	...
	*/
	 ...
	 *
	 * @param Filename     Name of the file to load
	 * @param Options	   Additional options to control behavior, use designated initializers for customization.
	 *
	 * e.g. 
	 * @code
	 * FFileHelper::LoadFileToStringAsync(TEXT("foo.txt"), {});
	 * FFileHelper::LoadFileToStringAsync(TEXT("foo.txt"),
	 * 		{ .OverridePlatformFile = MyPlatformFile, .FileOpenFlags = IPlatformFile::EOpenReadFlags::NoBuffering });
	 * @endcode  
	 */
	static CORE_API UE::Tasks::TTask<TValueOrError<FString, FFileSystemError>> LoadFileToStringAsync(const TCHAR* Filename, FLoadFileAsyncOptions Options);
