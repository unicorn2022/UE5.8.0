...
#include "Templates/SharedPointer.h"
	...
	// If enabled FSerializedShaderArchive, FIoStoreShaderCodeArchive and FMetalShaderLibrary will use a mmapping of the on-disk shader archive instead of normal serialization.
	// The on-disk format must match the expected in memory layout.
...
public:
	...
	static CORE_API TMulticastDelegate<void(const ANSICHAR* Expr, const ANSICHAR* File, int32 Line, const TCHAR* Msg, const TCHAR* CombinedErrorMsg)> OnEnsureFailed;

	// Callback when an ensure error report is being submitted
	...
	// Callback when a fatal hang has occurred.
	// WARNING: Use AddRaw/AddStatic for this delegate. Other bindings could be unsafe and cause deadlocks, e.g. AddUObject invoked during GC.
	static CORE_API TMulticastDelegate<void()> OnHandleSystemHang;
	...
	struct FModalWindowContext
	{
		...
		TOptional<bool> bIsSlowTaskWindow;

		// Guaranteed to match between PreSlateModalWithContext and PostSlateModalWithContext
		typedef std::intptr_t FWindowHandle;
		FWindowHandle WindowIdentifier = 0;
	};

	DECLARE_MULTICAST_DELEGATE_OneParam(FPrePostSlateModalWithContextDelegate, const FModalWindowContext&);
	...
	UE_DEPRECATED(5.8, "PreSlateModal has been deprecated. Please use PreSlateModalWithContext instead.")
	...
	static CORE_API FPrePostSlateModalWithContextDelegate PreSlateModalWithContext;
    
    // Called before the editor displays a Slate (non-platform) modal window, allowing other windows the opportunity to disable themselves to avoid reentrant calls
	UE_DEPRECATED(5.8, "PostSlateModal has been deprecated. Please use PostSlateModalWithContext instead.")
	...
	static CORE_API FPrePostSlateModalWithContextDelegate PostSlateModalWithContext;
	...
	static CORE_API FSimpleMulticastDelegate& GetOnPostEngineInit();

	// Called at the end of UEngine::Init, right before loading PostEngineInit modules for both normal execution and commandlets
	UE_DEPRECATED(5.8, "OnPostEngineInit has been deprecated. Please use GetOnPostEngineInit() instead.")
	...
	static CORE_API TTSMulticastDelegate<void(const TCHAR* IniFilename)>& TSOnConfigBranchUnloaded();
	static CORE_API TTSMulticastDelegate<void(const TCHAR* IniFilename, float ReloadTimeMs)>& TSOnConfigBranchReloaded();
	...
	// Called periodically from within slow operations in order to indicate the application is still alive.
	// Progress must be made for this to be called, or it could prevent the application from detecting soft-locks and unresponsiveness.
	// Subscribers should be lightweight and thread-safe.
	static CORE_API TTSMulticastDelegate<void()> ApplicationHeartbeat;
	...
	// Called when the application audio focus changed. Only called if there's audio focus event listeners
	static CORE_API TMulticastDelegate<void(bool bHasAudioFocus)> AudioFocusChangedDelegate;
	...
	/** Callback triggered when a shader map hash has been resolved to an I/O store chunk ID. */
	using FIoChunkIdResolvedFunc = TFunctionRef<void(const class FIoChunkId&)>;

	/**
	 * Callback to resolve I/O store chunk IDs from shader code hashes.
	 * Used when installing content on-demand.
	 *
	 * @param	Hashes		Array of hashes that should be resolved.
	 * @param	Callback	Callback triggered for each resolved chunk ID.
	 */
	static CORE_API TDelegate<void(TArrayView<const FShaderHash>, FIoChunkIdResolvedFunc)> ResolvePackageShaderMaps;
	...
	using FShaderReadRequestResult = TPair<class FIoRequest, TIoStatusOr<struct FIoMappedRegion>>;
#else
	using FShaderReadRequestResult = class FIoRequest;
#endif
	using FAttachShaderReadRequestFunc = TFunctionRef<FShaderReadRequestResult(const class FIoChunkId&, FGraphEventRef)>;
	static CORE_API TDelegate<void(TArrayView<const FShaderHash>, FAttachShaderReadRequestFunc)> PreloadPackageShaderMaps;
	static CORE_API TDelegate<void(TArrayView<const FShaderHash>)> ReleasePreloadedPackageShaderMaps;
	...
	// Called during each FLowLevelMemTracker tick.
	static CORE_API FSimpleMulticastDelegate OnLowLevelMemTrackerTick;
