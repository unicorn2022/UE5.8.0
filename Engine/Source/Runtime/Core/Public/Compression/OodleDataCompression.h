...
* The output made by this function is a single compressed stream of the same format as Compress()
...
* NOTE: CompressIndependentChunks and small MinChunkSize do hurt compression ratio (compared to non-parallel Compress)
*
* @param	OutCompressedData			output buffer where compressed data is written
* @param	CompressedBufferSize		bytes available to write in OutCompressedData
* @param	InUncompressedData			input buffer containing data to compress
...
* @param	Level						ECompressionLevel to encode with (this is not saved in the stream)
* @param	CompressIndependentChunks	(optional) should chunks be made independent (allows for parallel decode)
* @param	MinChunkSize				minimum chunk size; making this larger (eg. 16 MB) increases compression ratio but hurts parallelism; only used if CompressIndependentChunks is false
* @return								Compressed size written or zero for failure
*
* Calling Compress() or CompressParallel() does not affect whether or not the data can be decoded parallel.
* That is controlled by CompressIndependentChunks.
* The choice of Compress vs CompressParallel affects encode speed and compression ratio.
* CompressParallel lets you sacrifice some compression ratio for faster encodes.
*/
