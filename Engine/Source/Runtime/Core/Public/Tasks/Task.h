...
/** 
 * This namespace provides utilities for creating and accessing the state of tasks that run on separate threads.
 * You can do this by using one of the Launch functions to start a task and return a handle for later use. For example:
 *
 *		UE::Tasks::TTask<FReturnValue> TaskHandle = UE::Tasks::Launch(TEXT("TaskDebugName"), []() {
 *			DoSomethingSlow();
 *			return FReturnValue();
 *		});
 * 
 * In this example, the task will run on any worker thread and return a result payload that can be accessed
 * on the calling thread when the task has completed. Tasks can be launched with a list of prerequisites,
 * which will delay execution of the task until the prerequisites have completed. UE::Tasks::FTaskEvent can 
 * be used as a prerequisite to support manual triggering of task execution.
 * 
 * Read the full documentation for an explanation of the different parameters to the launch function and
 * the different ways to use the task handle after it has been launched.
 */
namespace UE::Tasks
{
	...
	namespace Private
	{
		/** Base implementation used by TTask to provide handle functionality */
		class FTaskHandle
		{
		public:
			...
			/** True if the task has been correctly initialized and not yet destroyed */
			bool IsValid() const
			{
			 ...
			 * @param ExtendedPriority - priority options for integrating with other system like named threads
	 		 * @param Flags - task config options
			...
			/** Returns true if it is valid to Wait on this task */
			bool IsAwaitable() const
			{
				return IsValid() && Pimpl->IsAwaitable();
			}

			/** Gets the priority this task was initialized with */
			ETaskPriority GetPriority() const
			{
				return Pimpl->GetPriority();
			}

			/** Gets the extended priority this task was initialized with */
	...
	/** Generic task type, can be either a real task or event */
	using FTask = Private::FTaskHandle;

	/** A templated asynchronous task that returns a result payload when complete */
	...
	/** A task that does not return a result payload */
	template<>
	...
	public:
		...
		/** Waits until task is completed, there is no result payload */
		void GetResult()
		{
	...
	public:
		...
		/** Add tasks to wait on, all prerequisites must be added before triggering the event */
		...
		/** Activates the task event, which will complete if the prerequisites have completed */
		void Trigger()
		{
	...
	/** Returns prerequisites for an array of tasks */
	...
	/** Utility function to wait on a specific task */
	inline void Wait(FTask& Task)
	{
		Task.Wait();
	}
