...
public:
	...
	// If app has a Shared File Container via the AppGroups capability, return the
	// path or an empty string if it's not found
	static FString GetSharedContainerPath(const TCHAR* AppGroupID);
	
protected:
	virtual FString ConvertToPlatformPath(const FString& Filename, bool bForWrite, bool bIsPublicWrite) override;
	virtual int32 OpenFileForMemMap(const FString& NormalizedFilename, EOpenMappedFlags OpenOptions, FString& OutFinalPath) override;
