...
struct FMicrosoftPlatformMathBase : public TUnrealPlatformMathSSE4Base<FGenericPlatformMath>
{
	...
	static constexpr inline uint32 CeilLogTwo( uint32 Arg )
	{
	...
	[[nodiscard]] static constexpr inline int32 CountBits(uint32 Bits)
	{
		UE_IF_CONSTEVAL
		{
			return FGenericPlatformMath::CountBits(Bits);
		}
		else
		{
			return (int32)__popcnt(Bits);
		}
	}

	[[nodiscard]] static constexpr inline int32 CountBits64(uint64 Bits)
	{
		UE_IF_CONSTEVAL
		{
			return FGenericPlatformMath::CountBits64(Bits);
		}
		else
		{
			return (int32)__popcnt64(Bits);
		}
	}
