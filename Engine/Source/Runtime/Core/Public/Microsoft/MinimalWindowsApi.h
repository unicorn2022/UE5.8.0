...
namespace Windows
{
	...
	MINIMAL_WINDOWS_API BOOL WINAPI CancelIoEx(HANDLE hFile, LPOVERLAPPED lpOverlapped);
	// Timing
	...
	UE_FORCEINLINE_HINT BOOL WINAPI CancelIoEx(HANDLE hFile, OVERLAPPED* lpOverlapped)
	{
		return CancelIoEx(hFile, (LPOVERLAPPED)lpOverlapped);
	}
