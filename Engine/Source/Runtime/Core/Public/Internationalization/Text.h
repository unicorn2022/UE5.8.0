 ...
 * @note Redeclared in KismetTextLibrary for meta-data extraction purposes, be sure to update there as well.
 */
enum class EMemoryUnit : uint8
{
	Unit_Bytes,

	/* Auto-select based on magnitude using IEC */
	Auto_IEC,

	/* IEC (1024-based) */
	Unit_KiB, Unit_MiB, Unit_GiB, Unit_TiB, Unit_PiB, Unit_EiB,

	/* Auto-select based on magnitude using SI */
	Auto_SI,

	/* SI (1000-based) */
	Unit_KB, Unit_MB, Unit_GB, Unit_TB, Unit_PB, Unit_EB
};
...
public:
	...
	// FText can be returned from AutoRTFM::Open() via a copy-construction in the closed.
	...
	/**
	 * Generate an FText that represents the passed number as a memory size in the current culture, expressed in a specific unit.
	 * Use EMemoryUnit::Auto_IEC or EMemoryUnit::Auto_SI to preserve the default magnitude-based selection behavior.
	 */
	[[nodiscard]] static CORE_API FText AsMemory(uint64 NumBytes, EMemoryUnit Unit, const FNumberFormattingOptions* const Options = nullptr, const FCulturePtr& TargetCulture = nullptr);
	...
	[[nodiscard]] static CORE_API FText AsLocalizable_Advanced(const FTextKey& Namespace, const FTextKey& Key, const TCHAR* String, FStringView InDevNotes = TEXTVIEW(""));
	[[nodiscard]] static CORE_API FText AsLocalizable_Advanced(const FTextKey& Namespace, const FTextKey& Key, FStringView String, FStringView InDevNotes = TEXTVIEW(""));
	[[nodiscard]] static CORE_API FText AsLocalizable_Advanced(const FTextKey& Namespace, const FTextKey& Key, FString&& String, FStringView InDevNotes = TEXTVIEW(""));
	[[nodiscard]] static CORE_API FText AsLocalizable_Advanced_LocText(const TCHAR* Namespace, const TCHAR* Key, const TCHAR* String, FStringView InDevNotes = TEXTVIEW("")); // For use by LOCTEXT to avoid constructing temp FTextKeys in every caller
#else
	...
	[[nodiscard]] static CORE_API bool ShouldGatherForLocalization(FStringView SourceString);
	 ...
	 */
	[[nodiscard]] static CORE_API FText ChangeKey( const FTextKey& Namespace, const FTextKey& Key, const FText& Text, const FString& DevNotes );

	CORE_API const FString& GetDevNotes() const;
#endif
...
#if WITH_EDITORONLY_DATA
	CORE_API FText( FString&& InSourceString, const FTextKey& InNamespace, const FTextKey& InKey, FString InDevNotes, uint32 InFlags=0 );
#endif
