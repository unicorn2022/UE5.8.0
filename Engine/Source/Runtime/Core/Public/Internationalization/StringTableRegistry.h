...
public:
	...
	/** Internal function called by LOCTABLE_SETSTRING and LOCTABLE_SETNOTESTRING to set the entry denoted by the given key to the given source string, within the given string table (table must have been registered already) */
#if WITH_EDITORONLY_DATA
	CORE_API void Internal_SetLocTableEntry(const FName InTableId, const FTextKey& InKey, const FString& InSourceString, const FString& InDevNotes = TEXT(""));
#else
...
#if WITH_EDITORONLY_DATA
/** Add a string table entry with the given key, source string, and developer translation context note. */
#define LOCTABLE_SETNOTESTRING(ID, KEY, SRC, NOTE) \
	FStringTableRegistry::Get().Internal_SetLocTableEntry(TEXT(ID), TEXT(KEY), TEXT(SRC), TEXT(NOTE))
#else
#define LOCTABLE_SETNOTESTRING(ID, KEY, SRC, NOTE) LOCTABLE_SETSTRING(ID, KEY, SRC)
#endif
