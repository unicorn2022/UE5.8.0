...
public:
	...
	static FStringTableEntryRef NewStringTableEntry(FStringTableConstRef InOwnerTable, FString InSourceString, FTextId InDisplayStringId, FString InDevNotes)
	{
		return MakeShared<FStringTableEntry, ESPMode::ThreadSafe>(MoveTemp(InOwnerTable), MoveTemp(InSourceString), MoveTemp(InDisplayStringId), MoveTemp(InDevNotes));
	}
#else
	...
	/** Create a new string table entry using the given data */
#if WITH_EDITORONLY_DATA
	CORE_API FStringTableEntry(FStringTableConstRef InOwnerTable, FString InSourceString, FTextId InDisplayStringId, FString InDevNotes);
#else
...
#if WITH_EDITORONLY_DATA
	/** Get the translation context notes */
	const FString& GetDevNotes() const { return DevNotes; }
#endif
...
#if WITH_EDITORONLY_DATA
	/** Notes from the developer to provide additional context to translators */
	FString DevNotes;
#endif
};
...
public:
...
#if WITH_EDITORONLY_DATA
	/** Set the source string and dev notes used by the given entry (will replace any existing data for that entry) */
	CORE_API void SetSourceString(const FTextKey& InKey, const FString& InSourceString, const FString& InDevNotes);
#else
	...
	CORE_API void EnumerateSourceStrings(const TFunctionRef<bool(const FString&, const FString&, const FString&)>& InEnumerator) const;
	CORE_API void EnumerateKeysAndSourceStrings(const TFunctionRef<bool(const FTextKey&, const FString&, const FString&)>& InEnumerator) const;
#endif
	...
	UE_DEPRECATED(5.8, "ExportStrings is deprecated in favor of ExportStringsToCSVFile")
	CORE_API bool ExportStrings(const FString& InFilename) const;

	/** Export the key, string, and meta-data information in this string table to a CSV string (does not export the namespace) */
	CORE_API FString ExportStringsToCSVString() const;

	/** Export the key, string, and meta-data information in this string table to a CSV file (does not export the namespace) */
	CORE_API bool ExportStringsToCSVFile(const FString& InFilename) const;

	/** Import key, string, and meta-data information from a CSV file to this string table (does not import the namespace) */
	UE_DEPRECATED(5.8, "ImportStrings is deprecated in favor of ImportStringsFromCSVFile")
	CORE_API bool ImportStrings(const FString& InFilename);

	/** Import key, string, and meta-data information from a CSV string to this string table (does not import the namespace) */
	CORE_API bool ImportStringsFromCSVString(const FString& InCSVString, const TCHAR* InErrorContext = TEXT("<unknown>"));

	/** Import key, string, and meta-data information from a CSV file to this string table (does not import the namespace) */
	CORE_API bool ImportStringsFromCSVFile(const FString& InFilename);
