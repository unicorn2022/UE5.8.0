...
public:
	...
	/**
	 * Get the language that we last loaded localization data for.
	 * eg) If the available languages were [en, fr, de] and the current language was "en-US", this function would return "en".
	 */
	CORE_API FString GetLocalizedLanguageName() const;
...
private:
	...
	void LoadLocalizationResourcesForPrioritizedCultures_Async(TArrayView<const FString> PrioritizedCultureNames, const ELocalizationLoadFlags LocLoadFlags, TUniqueFunction<void()>&& Notification);
	...
	/** Cache the language that we last loaded localization data for. */
	void CacheLocalizedLanguageName(const ELocalizationLoadFlags InLoadFlags);
	...
	/** The language that we last loaded localization data for. */
	FString LocalizedLanguageName;
};
