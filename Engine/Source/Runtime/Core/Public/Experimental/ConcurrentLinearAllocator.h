...
class TBlockAllocationCache
{
	...
	{
		UE_MODULE_SAFE_THREAD_LOCAL(FTLSCleanup, Tls);
		void* Ret = Tls.Block;
		Tls.Block = NewBlock;
		return Ret;
	}
...
class TLinearAllocatorBase
{
	...
	FORCENOINLINE static void AllocateBlock(FBlockHeader*& Header)
	{
		if constexpr (!BlockAllocationTag::InlineBlockAllocation)
		{
			...
			UE_MODULE_SAFE_THREAD_LOCAL(FTLSCleanup, Cleanup);
...
public:
	...
	static inline void* Malloc(SIZE_T Size, uint32 Alignment)
	{
		...
		{
			void* const Pointer = AutoRTFM::Open([Size, Alignment]
			{
				return Malloc(Size, Alignment);
			});

			AutoRTFM::OnAbort([Pointer]
			{
				Free(Pointer);
			});
		...
		UE_MODULE_SAFE_THREAD_LOCAL(FBlockHeader*, Header);
		if (Header == nullptr)
		{
		AllocateNewBlock:
			if constexpr (BlockAllocationTag::InlineBlockAllocation)
	...
	AUTORTFM_OPEN static inline SIZE_T GetAllocationSize(void* Pointer)
	{
		if (Pointer)
		{
			if constexpr (SupportsFastPath)
...
public:
	...
	// In a transaction because the bulk allocator only has bulk delete, on abort we're just wasting the memory.
	// So we run this code in the open (non-transactionally) always.
	AUTORTFM_OPEN inline void* Malloc(SIZE_T Size, uint32 Alignment)
	{
