...
public:
	...
	TIntrusiveDoubleLinkedList(TIntrusiveDoubleLinkedList&& Other)
	{
		AddHead(Forward<TIntrusiveDoubleLinkedList&&>(Other));
	}
	...
	{
		check(Element && !Element->IsInList());
...
private:
	...
	[[nodiscard]] UE_FORCEINLINE_HINT ElementType*       GetSentinel()       { return static_cast<ElementType*>(&Sentinel); }
	[[nodiscard]] UE_FORCEINLINE_HINT const ElementType* GetSentinel() const { return static_cast<const ElementType*>(&Sentinel); }
