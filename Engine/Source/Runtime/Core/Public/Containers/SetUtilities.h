...
// These divisors will determine at what point it's more optimal to individually remove elements from the hash table vs just using memset to reset the whole thing at once
// Sparse set stores the hashtable index so it's fairly cheap to remove vs compact set that has to recalculate the hash key which is more expensive
#define UE_SET_UNHASH_THRESHOLD_STORED_KEY 4
#define UE_SET_UNHASH_THRESHOLD_CALCULATE_KEY 32

/**
 * Traits which determine whether or not a type is a TSet.
 */
template <typename T>
inline constexpr bool TIsTSet_V = false;

template <typename T> inline constexpr bool TIsTSet_V<const          T> = TIsTSet_V<T>;
template <typename T> inline constexpr bool TIsTSet_V<      volatile T> = TIsTSet_V<T>;
template <typename T> inline constexpr bool TIsTSet_V<const volatile T> = TIsTSet_V<T>;

template <typename T>
struct TIsTSet
{
	static inline constexpr bool Value = TIsTSet_V<T>;
};
...
struct BaseKeyFuncs
{
	...
	static inline constexpr bool bAllowDuplicateKeys = bInAllowDuplicateKeys;
};
