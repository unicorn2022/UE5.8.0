...
public:
	...
	[[nodiscard]] UE_FORCEINLINE_HINT SizeType GetMaxIndex() const
	{
		return NumElements;
	}
...
protected:
	...
	[[nodiscard]] UE_FORCEINLINE_HINT constexpr TCompactSetBase() = default;
	...
	/* Calculate the size of the hash table from the number of elements in the set */
	[[nodiscard]] UE_FORCEINLINE_HINT static constexpr HashCountType GetHashCount(uint32 NumElements)
	{
	...
	[[nodiscard]] UE_FORCEINLINE_HINT static constexpr size_t GetTotalMemoryRequiredInBytes(uint32 NumElements, HashCountType HashCount, const FCompactSetLayout Layout)
	{
	...
	[[nodiscard]] static constexpr SizeType GetMaxElementsForAvailableSpace(size_t TotalBytes, HashCountType HashCount, uint32 MinElementCount, const FCompactSetLayout Layout)
	{
		...
		const size_t MaxElements = AvailableBytes / (Layout.Size + TypeSize * KeyStorageMultiplier); // Calculate the max available ignoring the fact that the hash data has to be aligned
		const size_t RealAvailableBytes = AlignDown(AvailableBytes - (MaxElements << TypeShift) * KeyStorageMultiplier, UE::Core::CompactHashTable::GetMemoryAlignment()); // Remove the max required indexes and align down
		return (SizeType)FMath::Min<size_t>(MaxElements, RealAvailableBytes / Layout.Size); // Now we can get the true number of elements we could potentially use within the aligned space
	}

	[[nodiscard]] SizeType AllocatorCalculateSlackGrow(SizeType NewMaxElements, const FCompactSetLayout& Layout) const
	{
		const HashCountType OldHashCount = MaxElements > 0 ? *(HashCountType*)(Elements.GetAllocation() + GetElementsSizeInBytes(MaxElements, Layout)) : 0;
		const size_t OldSize = GetTotalMemoryRequiredInBytes(MaxElements, OldHashCount, Layout);

		const HashCountType NewHashCount = GetHashCount(NewMaxElements);
		const size_t NewSize = GetTotalMemoryRequiredInBytes(NewMaxElements, NewHashCount, Layout);
		size_t NewSlackSize = 0;

		checkf(NewSize > 0 && NewSize <= MAX_int32, TEXT("Invalid size for TSet[%zd]: NewMaxElements[%d] ElementSize[%d] HashCount[%d]"), NewSize, NewMaxElements, Layout.Size, NewHashCount);

		if constexpr (TAllocatorTraits<AllocatorType>::SupportsElementAlignment)
		{
			NewSlackSize = Elements.CalculateSlackGrow((SizeType)NewSize, (SizeType)OldSize, 1, Layout.Alignment);
		}
		else
		{
			NewSlackSize = Elements.CalculateSlackGrow((SizeType)NewSize, (SizeType)OldSize, 1);
		}
		...
		HashCountType SlackHashCount = GetHashCount(SlackNumElements);
	...
	void ResizeAllocation(const SizeType NewMaxElements, const FCompactSetLayout& Layout)
	{
		(void)ResizeAllocationPreserveData(NewMaxElements, Layout, false);
	}

	// Use this is you'll be keeping the element data
	[[nodiscard]] bool ResizeAllocationPreserveData(const SizeType NewMaxElements, const FCompactSetLayout& Layout, bool bPreserve = true)
	{
		...
		if (NewMaxElements != MaxElements)
		{
			...
			const HashCountType NewHashCount = NewMaxElements > 0 ? UE::Core::CompactHashTable::GetHashCount(NewMaxElements) : 0;
			const size_t NewSize = NewMaxElements > 0 ? GetTotalMemoryRequiredInBytes(NewMaxElements, NewHashCount, Layout) : 0;

			checkf(NewSize >= 0 && NewSize <= MAX_int32, TEXT("Invalid size for TSet[%lld]: NewMaxElements[%d] ElementSize[%d] HashCount[%d]"), NewSize, NewMaxElements, Layout.Size, NewHashCount);
			...
			if (bPreserve && NewMaxElements > MaxElements && OldHashCount == NewHashCount)
			{
				...
				if constexpr (TAllocatorTraits<AllocatorType>::SupportsElementAlignment)
				{
					Elements.ResizeAllocation((SizeType)OldSize, (SizeType)NewSize, 1, Layout.Alignment);
				}
				else
				{
					Elements.ResizeAllocation((SizeType)OldSize, (SizeType)NewSize, 1);
				}
				...
				const uint8* OldHashLocation = OldHashTable + ((size_t)MaxElements << NewTypeShift) * KeyStorageMultiplier + HashCountSize;
				uint8* NewHashLocation = NewHashTable + ((size_t)NewMaxElements << NewTypeShift) * KeyStorageMultiplier + HashCountSize;

				// WARNING: Order matters here since we're moving data around in the same buffer. Make sure we're moving the tail data first
				// since we're expanding in size. So HashTable, then (optional) key storage, then hashcount and next indexes

				// Copy hash lookup table (first so to free up the space for the other data to move)
				FMemory::Memmove(NewHashLocation, OldHashLocation, ((size_t)NewHashCount << NewTypeShift));

#if UE_ENABLE_COMPACTHASHTABLE_KEYSTORAGE
				// Copy key storage if enabled
				const uint8* OldKeyStorage = OldHashTable + ((size_t)MaxElements << NewTypeShift) + HashCountSize;
				uint8* NewKeyStorage = NewHashTable + ((size_t)NewMaxElements << NewTypeShift) + HashCountSize;
				FMemory::Memmove(NewKeyStorage, OldKeyStorage, ((size_t)NumElements << NewTypeShift));
#endif
			...
			else
			{
				...
				if constexpr (TAllocatorTraits<AllocatorType>::SupportsElementAlignment)
				{
					Elements.ResizeAllocation(NumElements * Layout.Size, (SizeType)NewSize, 1, Layout.Alignment);
				}
				else
				{
					Elements.ResizeAllocation(NumElements * Layout.Size, (SizeType)NewSize, 1);
				}

				// Update hash size
				if (NewMaxElements > 0)
				{
					HashCountType* NewHashTable = (HashCountType*)(Elements.GetAllocation() + GetElementsSizeInBytes(NewMaxElements, Layout));
