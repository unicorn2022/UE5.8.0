...
#define TSETPRIVATEFRIEND UE_JOIN(UE_TSPARSE_SET, PrivateFriend)

#if !defined(UE_USE_SPARSE_SET_STRICT_MODE)
#define UE_USE_SPARSE_SET_STRICT_MODE 0
#endif

#define DO_SPARSE_SET_ITERATOR_CHECKS DO_CHECK && UE_USE_SPARSE_SET_STRICT_MODE
...
public:
	...
	*/
	...
	[[nodiscard]] UE_TSPARSE_SET SymmetricDifference(const UE_TSPARSE_SET& OtherSet) const
	{
		UE_TSPARSE_SET Result;
		Result.Reserve(Num() + OtherSet.Num()); // Worst case is no overlap
		...
		for(TConstIterator SetIt(OtherSet);SetIt;++SetIt)
		{
			if(!this->Contains(KeyFuncs::GetSetKey(*SetIt)))
			{
				Result.Add(*SetIt);
			}
		}
...
private:
	...
	void UnhashElements()
	{
		...
		if (Num() < (HashSize / UE_SET_UNHASH_THRESHOLD_STORED_KEY))
		{
	...
	{
	private:
		using SetType = std::conditional_t<bConst, const UE_TSPARSE_SET, UE_TSPARSE_SET>;
	...
	public:
		...
		[[nodiscard]] UE_FORCEINLINE_HINT explicit TBaseIterator(SetType& InSet)
			: ElementIt(InSet.Elements.begin())
			, Set(InSet)
#if DO_SPARSE_SET_ITERATOR_CHECKS
			, InitialNum(InSet.Num())
#endif
		{
		}

		[[nodiscard]] UE_FORCEINLINE_HINT explicit TBaseIterator(SetType& InSet, const ElementItType& InElementIt)
		...
		UE_FORCEINLINE_HINT TBaseIterator& operator++()
		{
			...
			ensureMsgf(SetNum >= InitialNum, TEXT("Sets/Maps should never have elements removed during iteration outside of Iterator.RemoveCurrent(). InitialNum %d CurrentNum %d"), InitialNum, SetNum);
	...
	public:
		...
		/** Initialization constructor. */
		[[nodiscard]] inline explicit TBaseKeyIterator(SetType& InSet, KeyArgumentType InKey)
			: Set  (InSet)
			, Key  (InKey) //-V1041
			, Index(INDEX_NONE)
#if DO_SPARSE_SET_ITERATOR_CHECKS
...
public:
	...
	public:
		...
		{
#if DO_SPARSE_SET_ITERATOR_CHECKS
			ensureMsgf(this->LastRemoved == INDEX_NONE, TEXT("RemoveCurrent was already called for the current iteration index of %d"), this->LastRemoved);
			this->LastRemoved = this->ElementIt.GetIndex();
			--this->InitialNum;
#endif

			this->Set.RemoveByIndex(this->ElementIt.GetIndex());
		}
	};
...
public:
	...
	{
		return TRangedForIterator(*this, Elements.end());
	}
	[[nodiscard]] UE_FORCEINLINE_HINT TRangedForConstIterator end() const
	{
		return TRangedForConstIterator(*this, Elements.end());
	}
...
template <typename ElementType, typename KeyFuncs, typename Allocator>
inline constexpr bool TIsTSet_V<UE_TSPARSE_SET<ElementType, KeyFuncs, Allocator>> = true;
