...
/** A TArray allocator (32-bit sizetype only) that allocates from FMemory and allows specifying alignment for all allocations. */
...
/**
 * BaseClass for TArray Allocators.
 * Templated to support the container's sizetype (e.g. int32 (IndexSize==32) or int64 (IndexSize==64)).
 * Uses non-virtual inheritance via the CuriouslyRecurringTemplatePattern; concrete subclasses must pass
 * their own type in as the SubClass template parameter.
 */
template <int IndexSize, typename SubClass>
class TSizedAllocatorBase
{
public:
...
public:
	...
	class ForAnyElementType
	{
		...
		friend class TSizedAllocatorBase;

	public:
		/** Default constructor. */
		UE_NODEBUG constexpr ForAnyElementType()
			: Data(nullptr)
		{
		}

		/** Explicitly consteval constructor. */
		UE_NODEBUG explicit consteval ForAnyElementType(EConstEval)
			: Data(nullptr)
		{}
		...
		UE_NODEBUG UE_FORCEINLINE_HINT void MoveToEmptyFromOtherAllocator(typename OtherAllocator::ForAnyElementType& Other)
		{
			checkSlow((void*)this != (void*)&Other);

			if (Data)
			{
				// Call subclass Free using CuriouslyRecurringTemplatePattern.
				static_cast<SubClass::ForAnyElementType*>(this)->CallFree(Data);
			}
		...
		/**
		 * Moves the state of another allocator into this one. The allocator is the same class.
		 *
		 * Assumes that the allocator is currently empty, i.e. memory may be allocated but any existing elements have already been destructed (if necessary).
		 * @param Other - The allocator to move the state from.  This allocator should be left in a valid empty state.
		 */
		UE_NODEBUG UE_FORCEINLINE_HINT void MoveToEmpty(SubClass::ForAnyElementType& Other)
		{
			this->MoveToEmptyFromOtherAllocator<SubClass>(Other);
		}

		/** Destructor. */
		UE_NODEBUG UE_FORCEINLINE_HINT ~ForAnyElementType()
		{
			if (Data)
			{
				// Call subclass Free using CuriouslyRecurringTemplatePattern.
		...
		// FContainerAllocatorInterface
		UE_NODEBUG UE_FORCEINLINE_HINT FScriptContainerElement* GetAllocation() const
		{
			return Data;
		}
		UE_NODEBUG void ResizeAllocation(SizeType CurrentNum, SizeType NewMax, SIZE_T NumBytesPerElement)
		{
			// Avoid calling FMemory::Realloc( nullptr, 0 ) as ANSI C mandates returning a valid pointer which is not what we want.
			if (Data || NewMax)
			{
				...
				// Call subclass Realloc using CuriouslyRecurringTemplatePattern.
				Data = (FScriptContainerElement*)static_cast<SubClass::ForAnyElementType*>(this)->CallRealloc(Data, NewMax, NumBytesPerElement);
			}
		}
		UE_NODEBUG void ResizeAllocation(SizeType CurrentNum, SizeType NewMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement)
		{
			// Avoid calling Realloc( nullptr, 0 ) as ANSI C mandates returning a valid pointer which is not what we want.
			if (Data || NewMax)
			{
				static_assert(sizeof(SizeType) <= sizeof(SIZE_T), "SIZE_T is expected to handle all possible sizes");
				...
				// Call subclass Realloc using CuriouslyRecurringTemplatePattern.
				Data = (FScriptContainerElement*)static_cast<SubClass::ForAnyElementType*>(this)->CallRealloc(Data, NewMax, NumBytesPerElement, AlignmentOfElement);
			}
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackReserve(SizeType NewMax, SIZE_T NumBytesPerElement) const
		{
			return DefaultCalculateSlackReserve(NewMax, NumBytesPerElement, true);
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackReserve(SizeType NewMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement) const
		{
			return DefaultCalculateSlackReserve(NewMax, NumBytesPerElement, true, (uint32)AlignmentOfElement);
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackShrink(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement) const
		{
			return DefaultCalculateSlackShrink(NewMax, CurrentMax, NumBytesPerElement, true);
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackShrink(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement) const
		{
			return DefaultCalculateSlackShrink(NewMax, CurrentMax, NumBytesPerElement, true, (uint32)AlignmentOfElement);
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackGrow(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement) const
		{
			return DefaultCalculateSlackGrow(NewMax, CurrentMax, NumBytesPerElement, true);
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackGrow(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement) const
		{
			return DefaultCalculateSlackGrow(NewMax, CurrentMax, NumBytesPerElement, true, (uint32)AlignmentOfElement);
		}

		UE_NODEBUG SIZE_T GetAllocatedSize(SizeType CurrentMax, SIZE_T NumBytesPerElement) const
		{
			return CurrentMax * NumBytesPerElement;
		}

		UE_NODEBUG bool HasAllocation() const
		{
			return !!Data;
		}

		UE_NODEBUG constexpr SizeType GetInitialCapacity() const
		{
			return 0;
		}

	public:
		// Optional template functions to override on ForAnyElementType on the SubClass.
#if UE_ENABLE_ARRAY_SLACK_TRACKING
		UE_NODEBUG UE_FORCEINLINE_HINT void SlackTrackerLogNum(SizeType NewNumUsed)
		{
			// Base implementation does not report to SlackTracking; subclasses must override if they need to report
			// to slacktracking, @see TSizedHeapAllocator. 
		}

		// Suppress slack tracking on an allocation -- should be called whenever the container may have been resized.
		// Useful for debug allocations you don't want to show up in slack reports.
		UE_NODEBUG UE_FORCEINLINE_HINT void DisableSlackTracking()
		{
			// Base implementation does not report to SlackTracking; subclasses must override if they need to report
			// to slacktracking, @see TSizedHeapAllocator. 
		}
		...
		// Required template functions to override on ForAnyElementType on the SubClass.
		void* CallRealloc(void* InData, SizeType NewMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT) = delete;
		void CallFree(void* InData) = delete;
	...
	public:
		...
		UE_NODEBUG UE_FORCEINLINE_HINT ElementType* GetAllocation() const
		{
			return (ElementType*)ForAnyElementType::GetAllocation();
		}
	};
...
/**
 * A TArray allocator that allocates from FMemory.
 * The BaseMallocType template parameter is deprecated and passing it in will be a compile error in a future release.
 * Use TSizedExternalAllocator instead to provide an ExternalMallocType,
 */
template <int IndexSize, typename BaseMallocType = FMemory>
class TSizedHeapAllocator : public TSizedAllocatorBase<IndexSize, TSizedHeapAllocator<IndexSize, BaseMallocType>>
{
public:
	using MyClass = TSizedHeapAllocator<IndexSize, BaseMallocType>;
	using SuperClass = TSizedAllocatorBase<IndexSize, MyClass>;
	using BaseMalloc = BaseMallocType;
	using SizeType = SuperClass::SizeType;

	class ForAnyElementType : public SuperClass::ForAnyElementType
	{
	public:
		// Declare we are using constructors from ForAnyElementType on SuperClass
		using SuperClass::ForAnyElementType::ForAnyElementType;

		// Declare we are using Data pointer on the superclass; the compiler can't figure this out automatically due
		// to two-phase template compile.
		using SuperClass::ForAnyElementType::Data;

		// Declare our constructor so we can add deprecation handling for BaseMallocType.
		UE_NODEBUG constexpr ForAnyElementType()
		{
			UE_STATIC_DEPRECATE(5.8, (!std::is_same_v<BaseMallocType, FMemory>), "Overriding the MallocType in TSizedHeapAllocator is deprecated. Use TSizedExternalAllocator<int IndexSize, typename ExternalMallocType> instead");
		}

		// Override the ResizeAllocation functions so they can be defined as exported to avoid bloat
#if !UE_MERGED_MODULES && !defined(UE_HEADER_UNITS)
		UE_NODEBUG void ResizeAllocation(SizeType CurrentNum, SizeType NewMax, SIZE_T NumBytesPerElement)
		{
			SuperClass::ForAnyElementType::ResizeAllocation(CurrentNum, NewMax, NumBytesPerElement);
		}
		UE_NODEBUG void ResizeAllocation(SizeType CurrentNum, SizeType NewMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement)
		{
			SuperClass::ForAnyElementType::ResizeAllocation(CurrentNum, NewMax, NumBytesPerElement, AlignmentOfElement);
		}
#endif
	...
	public:
		...
		UE_NODEBUG UE_FORCEINLINE_HINT void* CallRealloc(void* InData, SuperClass::SizeType NewMax,
			SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT)
		{
#if UE_ENABLE_ARRAY_SLACK_TRACKING
			AlignmentOfElement = AlignmentOfElement == 0
				? 0
				: (AlignmentOfElement > alignof(FArraySlackTrackingHeader)
					? AlignmentOfElement : alignof(FArraySlackTrackingHeader));
			return (FScriptContainerElement*)FArraySlackTrackingHeader::Realloc(InData, NewMax, NumBytesPerElement, AlignmentOfElement);
#else
			return (FScriptContainerElement*)BaseMalloc::Realloc(InData, NewMax * NumBytesPerElement, AlignmentOfElement);
#endif
		}
		UE_NODEBUG UE_FORCEINLINE_HINT void CallFree(void* InData)
		{
#if UE_ENABLE_ARRAY_SLACK_TRACKING
			FArraySlackTrackingHeader::Free(InData);
#else
	...
	// Special implementation of ForElementType that implements the consteval constructor.
	template<typename ElementType>
	class ForElementType : public ForAnyElementType
	{
	public:
		UE_NODEBUG ForElementType() = default;
		UE_NODEBUG explicit consteval ForElementType(EConstEval)
...
{
public:
	using MyClass = TSizedExternalAllocator<IndexSize, ExternalMallocType>;
	using SuperClass = TSizedAllocatorBase<IndexSize, MyClass>;
	using ExternalMalloc = ExternalMallocType;
	...
	public:
		UE_FORCEINLINE_HINT void* CallRealloc(void* InData, SuperClass::SizeType NewMax,
			SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT)
		{
			return (FScriptContainerElement*)ExternalMalloc::Realloc(InData, NewMax * NumBytesPerElement, AlignmentOfElement);
		}
		UE_FORCEINLINE_HINT void CallFree(void* InData)
		{
			ExternalMalloc::Free(InData);
		}
	};
};
 ...
 * TArrays using this allocator must be constructed with the TArray constructors that take an ElementAllocatorType&&
 * argument; attempting to construct them without that argument will intentionally fail due to lack of a default constructor
 * on ForAnyElementType.
*/
template <int IndexSize>
class TSizedMallocAllocator : public TSizedAllocatorBase<IndexSize, TSizedMallocAllocator<IndexSize>>
{
public:
	using MyClass = TSizedMallocAllocator<IndexSize>;
	using SuperClass = TSizedAllocatorBase<IndexSize, MyClass>;
	...
	public:
		...
		explicit ForAnyElementType(FMalloc* InMalloc)
			: Malloc(InMalloc)
		{
		}
		ForAnyElementType(ForAnyElementType&& Other)
			: Malloc(Other.Malloc)
		{
			Data = Other.Data;
			Other.Data = nullptr;
	...
	public:
		UE_FORCEINLINE_HINT void* CallRealloc(void* InData, SuperClass::SizeType NewMax,
			SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT)
		{
			return Malloc->Realloc(InData, NewMax * NumBytesPerElement, AlignmentOfElement);
		}
		UE_FORCEINLINE_HINT void CallFree(void* InData)
		{
			Malloc->Free(InData);
	...
	// Declare our own ForElementType so we can support our constructor parameters.
	template<typename ElementType>
	class ForElementType : public ForAnyElementType
	{
	public:
		explicit ForElementType(FMalloc* InMalloc)
			: ForAnyElementType(InMalloc)
		{
		}
		ForElementType(ForElementType&& Other)
			: ForAnyElementType(MoveTemp(Other))
		{
		}
...
using FDefaultMallocAllocator = TSizedMallocAllocator<32>;
...
public:
	...
	public:
		...
		void ResizeAllocation(SizeType CurrentNum, SizeType NewMax, SIZE_T NumBytesPerElement)
			requires (!TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment)
		{
		...
		void ResizeAllocation(SizeType CurrentNum, SizeType NewMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT)
			requires (!!TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment)
		{
			// Make sure the number of live elements is still withing the allocation since we only memmove, not destruct
			checkSlow(CurrentNum <= NewMax);
			...
			else
			{
				...
				{
					// Allocate new indirect memory for the data.
					SecondaryData.ResizeAllocation(0, NewMax, NumBytesPerElement, AlignmentOfElement);
				...
				{
					// Reallocate the indirect data for the new size.
					SecondaryData.ResizeAllocation(CurrentNum, NewMax, NumBytesPerElement, AlignmentOfElement);
				}
			}
		}
		...
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackReserve(SizeType NewMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT) const
			requires (!!TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment)
		{
			// If the elements use less space than the inline allocation, only use the inline allocation as slack.
			return NewMax <= NumInlineElements ?
				NumInlineElements :
				SecondaryData.CalculateSlackReserve(NewMax, NumBytesPerElement, AlignmentOfElement);
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackShrink(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement) const
			requires (!TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment)
		{
		...
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackShrink(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT) const
			requires (!!TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment)
		{
			// If the elements use less space than the inline allocation, only use the inline allocation as slack.
			return NewMax <= NumInlineElements ?
				NumInlineElements :
				SecondaryData.CalculateSlackShrink(NewMax, CurrentMax, NumBytesPerElement, AlignmentOfElement);
		}
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackGrow(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement) const
			requires (!TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment)
		{
		...
		UE_NODEBUG UE_FORCEINLINE_HINT SizeType CalculateSlackGrow(SizeType NewMax, SizeType CurrentMax, SIZE_T NumBytesPerElement, uint32 AlignmentOfElement = DEFAULT_ALIGNMENT) const
			requires (!!TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment)
		{
			// If the elements use less space than the inline allocation, only use the inline allocation as slack.
			// Also, when computing slack growth, don't count inline elements -- the slack algorithm has a special
			...
			return NewMax <= NumInlineElements ?
				NumInlineElements :
				SecondaryData.CalculateSlackGrow(NewMax, CurrentMax <= NumInlineElements ? 0 : CurrentMax, NumBytesPerElement, AlignmentOfElement);
		}

		UE_NODEBUG UE_FORCEINLINE_HINT SIZE_T GetAllocatedSize(SizeType CurrentMax, SIZE_T NumBytesPerElement) const
		{
		...
		UE_REWRITE bool HasAllocation() const
		{
	...
	private:
		...
		UE_REWRITE ElementType* GetInlineElements() const
		{
...
{
	enum { SupportsElementAlignment = TAllocatorTraits<SecondaryAllocator>::SupportsElementAlignment };
	enum { SupportsSlackTracking = true };
};
...
// If enabled store a copy of the 'key' (key % hashcount - 1, so lookup index) for quick removal
// This takes more space but allows compact sets to be used with garbage reference removal
#if !defined UE_ENABLE_COMPACTHASHTABLE_KEYSTORAGE
#define UE_ENABLE_COMPACTHASHTABLE_KEYSTORAGE 1
#endif

namespace UE::Core::Private::CompactSetAllocatorHelpers
{
	static constexpr uint8 KeyStorageMultiplier = UE_ENABLE_COMPACTHASHTABLE_KEYSTORAGE ? 2 : 1;
	...
	constexpr int32 CalculateRequiredBytes()
	{
		...
		constexpr uint32 IndexStorageSize = NumInlineElements * KeyStorageMultiplier;
		return Align(NumInlineElements*ElementSize, 4) + 4 + (IndexStorageSize + HashSize) * TypeSize;
	}
}
