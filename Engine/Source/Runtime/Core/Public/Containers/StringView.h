...
template <typename T> inline constexpr bool TIsTStringView_V                 = false;
template <typename T> inline constexpr bool TIsTStringView_V<TStringView<T>> = true;

template <typename T> inline constexpr bool TIsTStringView_V<const          T> = TIsTStringView_V<T>;
template <typename T> inline constexpr bool TIsTStringView_V<      volatile T> = TIsTStringView_V<T>;
template <typename T> inline constexpr bool TIsTStringView_V<const volatile T> = TIsTStringView_V<T>;

template <typename T>
struct TIsTStringView
{
	static constexpr bool Value = TIsTStringView_V<T>;
...
public:
	...
	template <UE::CNotCVRefTo<ViewType> CharRangeType>
		// !std::is_pointer_v is used here to force the pointer constructor to be used when passed an array or pointer.
		requires (!std::is_pointer_v<std::decay_t<CharRangeType>> && UE::CCompatibleStringViewable<CharRangeType, CharType> && !TIsTStringView_V<std::decay_t<CharRangeType>>)
	[[nodiscard]] constexpr inline TStringView(const CharRangeType& InRange UE_LIFETIMEBOUND)
		: DataPtr((const CharType*)GetDataHelper(InRange))
	...
	// !std::is_pointer_v is used here to force the pointer constructor to be used when passed an array or pointer.
		requires (!std::is_pointer_v<std::decay_t<CharRangeType>> && UE::CCompatibleStringViewable<CharRangeType, CharType> && TIsTStringView_V<std::decay_t<CharRangeType>>)
	[[nodiscard]] constexpr inline TStringView(const CharRangeType& InRange)
		: DataPtr((const CharType*)GetDataHelper(InRange))
		, Size(IntCastChecked<int32>(GetNum(InRange)))
	{
	}
	...
	/**
	 * We need to explicitly implement assignment operators, even though they're equivalent to copying an implicitly-
	 * constructed temporary, because UE_LIFETIMEBOUND isn't transitive across implicit construction.
	 */

	/** Construct a view of the null-terminated string pointed to by InData. */
	UE_REWRITE constexpr TStringView& operator=(const CharType* InData UE_LIFETIMEBOUND)
	{
		return *this = TStringView(InData);
	}

	/** Construct a view of the null-terminated string pointed to by InData. */
	template <UE::CCompatibleCharType<CharType> OtherCharType>
	UE_REWRITE constexpr TStringView& operator=(const OtherCharType* InData UE_LIFETIMEBOUND)
	{
		return *this = TStringView(InData);
	}
	...
	template <UE::CNotCVRefTo<ViewType> CharRangeType>
		// !std::is_pointer_v is used here to force the pointer constructor to be used when passed an array or pointer.
		requires (!std::is_pointer_v<std::decay_t<CharRangeType>> && UE::CCompatibleStringViewable<CharRangeType, CharType> && !TIsTStringView_V<std::decay_t<CharRangeType>>)
	UE_REWRITE constexpr TStringView& operator=(const CharRangeType& InRange UE_LIFETIMEBOUND)
	{
		return *this = TStringView(InRange);
	}
	...
	// !std::is_pointer_v is used here to force the pointer constructor to be used when passed an array or pointer.
		requires (!std::is_pointer_v<std::decay_t<CharRangeType>> && UE::CCompatibleStringViewable<CharRangeType, CharType> && TIsTStringView_V<std::decay_t<CharRangeType>>)
	UE_REWRITE constexpr TStringView& operator=(const CharRangeType& InRange)
	{
		return *this = TStringView(InRange);
	}
...
public:
	...
	template <UE::CConvertibleTo<TStringView> CharRangeType>
	[[nodiscard]] UE_REWRITE auto UEOpLessThan(CharRangeType&& Rhs) const
	{
		return TStringView::PrivateLess(*this, ImplicitConv<TStringView>(Forward<CharRangeType>(Rhs)));
	}

	template <UE::CConvertibleTo<TStringView> CharRangeType>
	[[nodiscard]] UE_REWRITE auto UEOpGreaterThan(CharRangeType&& Rhs) const
	{
...
[[nodiscard]] constexpr inline FUtf8StringView operator""_PrivateU8SV(const UTF8CHAR* String, size_t Size)
{
	return FUtf8StringView(String, (int32)Size);
}
...
#define WIDETEXTVIEW(str) UE_JOIN(WIDETEXT(str), _PrivateWSV)
#define UTF8TEXTVIEW(str) UE_JOIN(UTF8TEXT(str), _PrivateU8SV)

namespace UE::Core::Private
{
	template <typename CharType>
	void CharTextViewStaticAssert()
	{
		static_assert(sizeof(CharType) == 0, "Unsupported char type passed to CHARTEXTVIEW");
	}
}

// Expands out to ANSITEXTVIEW(x), TEXTVIEW(x), WIDETEXTVIEW(x) or UTF8TEXTVIEW(x) depending on CharType
#define CHARTEXTVIEW(CharType, x) \
	( \
		[]() -> decltype(auto) \
		{ \
			/* We expect <type_traits> to already have been included any time the CHARTEXTVIEW macro is used */ \
			using UnqualifiedCharType = std::remove_cv_t<CharType>; \
			if constexpr (std::is_same_v<UnqualifiedCharType, ANSICHAR>) \
			{ \
				return ANSITEXTVIEW(x); \
			} \
