...
public:
	...
	UE_NODEBUG void RemoveCurrent(EAllowShrinking AllowShrinking = UE::Core::Private::AllowShrinkingByDefault<ContainerAllocatorType>())
	{
		Container.RemoveAt(Index, AllowShrinking);
		Index--;
	}
	 ...
	 */
	UE_NODEBUG void RemoveCurrentSwap(EAllowShrinking AllowShrinking = UE::Core::Private::AllowShrinkingByDefault<ContainerAllocatorType>())
	{
		Container.RemoveAtSwap(Index, AllowShrinking);
		Index--;
	}
...
UE_NODEBUG [[nodiscard]] UE_FORCEINLINE_HINT TIndexedContainerIterator<ContainerType, ElementType, SizeType> operator+(SizeType Offset, TIndexedContainerIterator<ContainerType, ElementType, SizeType> RHS)
{
	...
	struct TCheckedPointerIterator
	{
		...
		UE_NODEBUG UE_FORCEINLINE_HINT TCheckedPointerIterator operator++(int)
		{
			TCheckedPointerIterator Result = *this;
			++*this;
			return Result;
		}
		...
		UE_NODEBUG [[nodiscard]] UE_FORCEINLINE_HINT TCheckedPointerIterator operator+(ptrdiff_t N) const
		{
			TCheckedPointerIterator Result = *this;
			if constexpr (bReverse)
			{
		...
		UE_NODEBUG UE_FORCEINLINE_HINT TCheckedPointerIterator& operator+=(ptrdiff_t N)
		{
			if constexpr (bReverse)
			{
				this->Ptr -= N;
		...
		UE_NODEBUG [[nodiscard]] UE_FORCEINLINE_HINT ptrdiff_t operator-(const TCheckedPointerIterator& Rhs) const
		{
			if constexpr (bReverse)
			{
				return Rhs.Ptr - Ptr;
		...
		UE_NODEBUG [[nodiscard]] UE_FORCEINLINE_HINT ElementType& operator[](ptrdiff_t N) const
		{
			if constexpr (bReverse)
			{
				return *(Ptr - (N + 1));
		...
		UE_NODEBUG [[nodiscard]] UE_FORCEINLINE_HINT bool UEOpEquals(const TCheckedPointerIterator& Rhs) const
		{
			// We only need to do the check in comparison operators, because no other operator will be
			// called until after they return.
			//
			// Also, we should only need to check one side of this comparison - comparing iterators from
			// different containers is undefined.

			// This should use *CurrentNum rather than CurrentNum[0], but it works around the following compiler issue:
			// https://developercommunity.visualstudio.com/t/False-C6269-during-static-analysis/10989722
			ensureMsgf(!CurrentNum || CurrentNum[0] == InitialNum, TEXT("Array has changed during ranged-for iteration!"));
			return Ptr == Rhs.Ptr;
		}

		UE_NODEBUG  [[nodiscard]] UE_FORCEINLINE_HINT bool UEOpLessThan(const TCheckedPointerIterator& Rhs) const
		{
			// We only need to do the check in comparison operators, because no other operator will be
			// called until after they return.
			//
 ...
 *
 * A dynamically sized array of typed elements.  Makes the assumption that your elements are trivially relocatable; 
...
public:
...
#if UE_AUTORTFM
	// TArray can be returned from AutoRTFM::Open() via a copy-construction in the closed,
	// so long as the element type also supports copy-construction in the closed.
	static constexpr AutoRTFM::EReturnFromOpenMode AutoRTFMReturnFromOpenMode =
		(AutoRTFM::ReturnFromOpenModeFor<ElementType> == AutoRTFM::EReturnFromOpenMode::CopyConstructInClosed) ?
	...
	/**
	 * Constructor to create an empty array, that also initializes the ElementAllocatorType from a moved-in parameter.
	 * The input InAllocatorInstance must be empty; it is only meant to pass parameters to the AllocatorInstance.
	 * 
	 * This constructor and the others that take ElementAllocatorType&& are intended to allow AllocatorTypes that need
	 * to be parameterized, such as one that allocates from a specified FMalloc*.
	 * The input AllocatorInstance must be empty because we currently do not have a way to store the correct ArrayMax
	 * for the Allocator's current allocation; that would require adding an additional InArrayMax argument or extending
	 * the API of AllocatorInstance to be able to report it.
	 */
	[[nodiscard]] UE_NODEBUG UE_FORCEINLINE_HINT explicit TArray(ElementAllocatorType&& InAllocatorInstance)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
		, ArrayNum(0)
		, ArrayMax(AllocatorInstance.GetInitialCapacity())
	{
		check(!AllocatorInstance.HasAllocation());
	}
	...
	/**
	 * Constructor from a raw array of elements, that also initializes the ElementAllocatorType from a moved-in parameter.
	 * The input InAllocatorInstance must be empty; it is only meant to pass parameters to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	[[nodiscard]] UE_NODEBUG UE_FORCEINLINE_HINT explicit TArray(ElementAllocatorType&& InAllocatorInstance, const ElementType* Ptr, SizeType Count)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		Construct(Ptr, Count);
...
private:
	UE_FORCEINLINE_HINT void Construct(const ElementType* Ptr, SizeType Count)
	{
		if (Count < 0)
		{
...
public:
	...
	template <typename OtherElementType, typename OtherSizeType>
	[[nodiscard]] UE_NODEBUG explicit TArray(ElementAllocatorType&& InAllocatorInstance, const TArrayView<OtherElementType, OtherSizeType>& Other);
	...
	/**
	 * Constructor from an initializer list, that also initializes the ElementAllocatorType from a moved-in parameter.
	 * The input InAllocatorInstance must be empty; it is only meant to pass parameters to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	[[nodiscard]] UE_NODEBUG explicit TArray(ElementAllocatorType&& InAllocatorInstance, std::initializer_list<InElementType> InitList)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		CopyToEmpty(InitList.begin(), (SizeType)InitList.size(), 0);
	...
	/**
	 * Copy Constructor from a TArray with a different AllocatorType, that also initializes this TArray's
	 * ElementAllocatorType from a moved-in parameter before doing the copy.
	 * The input InAllocatorInstance must be empty; it is only meant to pass parameters to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	template <
		...
		UE_REQUIRES(UE::Core::Private::TArrayElementsAreCompatible_V<ElementType, const OtherElementType&>)
	>
	[[nodiscard]] UE_NODEBUG UE_FORCEINLINE_HINT explicit TArray(ElementAllocatorType&& InAllocatorInstance, const TArray<OtherElementType, OtherAllocator>& Other)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		CopyToEmpty(Other.GetData(), Other.Num(), 0);
	...
	/**
	 * Copy Constructor from a TArray with the same AllocatorType, that also initializes this TArray's
	 * ElementAllocatorType from a moved-in parameter before doing the copy.
	 * The input InAllocatorInstance must be empty; this function is only meant to pass parameters to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	[[nodiscard]] UE_NODEBUG UE_FORCEINLINE_HINT explicit TArray(ElementAllocatorType&& InAllocatorInstance, const TArray& Other)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		CopyToEmpty(Other.GetData(), Other.Num(), 0);
	...
	/**
	 * Copy constructor from TArray with same AllocatorType, with ExtraSlack, that also initializes this TArray's
	 * ElementAllocatorType from a moved-in parameter before doing the copy.
	 * The input InAllocatorInstance must be empty; this function is only meant to pass parameters to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	[[nodiscard]] UE_NODEBUG UE_FORCEINLINE_HINT explicit TArray(ElementAllocatorType&& InAllocatorInstance, const TArray& Other, SizeType ExtraSlack)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		CopyToEmptyWithSlack(Other.GetData(), Other.Num(), 0, ExtraSlack);
...
public:
	...
	/**
	 * Move constructor from same AllocatorInstance type that also initializes this TArray's ElementAllocatorType from
	 * a moved-in parameter before moving the elements (or entire data pointer) from the other TArray's AllocatorInstance.
	 * The input InAllocatorInstance must be empty; this function is only meant to pass parameters to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	[[nodiscard]] UE_NODEBUG UE_FORCEINLINE_HINT explicit TArray(ElementAllocatorType&& InAllocatorInstance, TArray&& Other)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		MoveOrCopy(*this, Other, 0);
	...
	/**
	 * Move constructor from different AllocatorInstance type that also initializes this TArray's ElementAllocatorType from
	 * a moved-in parameter before moving the elements from the other TArray's AllocatorInstance.
	 * The input InAllocatorInstance must be empty; this function is only meant to pass parameters to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	template <
		...
		UE_REQUIRES(UE::Core::Private::TArrayElementsAreCompatible_V<ElementType, OtherElementType&&>)
	>
	[[nodiscard]] UE_NODEBUG UE_FORCEINLINE_HINT explicit TArray(ElementAllocatorType&& InAllocatorInstance, TArray<OtherElementType, OtherAllocator>&& Other)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		MoveOrCopy(*this, Other, 0);
	...
	/**
	 * Move constructor from same or different AllocatorInstance type, with ExtraSlack, that also initializes this
	 * TArray's ElementAllocatorType from a moved-in parameter before moving the elements (or entire data pointer) from the other
	 * TArray's AllocatorInstance. The input InAllocatorInstance must be empty; this function is only meant to pass parameters
	 * to the AllocatorInstance.
	 * @see TArray(ElementAllocatorType&&) for further comments on the InAllocatorInstance.
	 */
	template <
		typename OtherElementType
		UE_REQUIRES(UE::Core::Private::TArrayElementsAreCompatible_V<ElementType, OtherElementType&&>)
	>
	[[nodiscard]] UE_NODEBUG explicit TArray(ElementAllocatorType&& InAllocatorInstance, TArray<OtherElementType, AllocatorType>&& Other, SizeType ExtraSlack)
		: AllocatorInstance(MoveTemp(InAllocatorInstance))
	{
		check(!AllocatorInstance.HasAllocation());
		MoveOrCopyWithSlack(*this, Other, 0, ExtraSlack);
	...
	/**
	 * Returns maximum number of elements in array before reallocation.
	 *
	 * @returns Maximum number of elements in array before reallocation.
	 * @see GetSlack
	 */
...
public:
	 ...
	 *                       Use FNonshrinkingAllocator or pass EAllowShrinking::No to prevent this.
	 */
	UE_FORCEINLINE_HINT void RemoveAt(SizeType Index, SizeType Count, EAllowShrinking AllowShrinking = UE::Core::Private::AllowShrinkingByDefault<AllocatorType>())
	{
...
public:
	 ...
	 *                       Use FNonshrinkingAllocator or pass EAllowShrinking::No to prevent this.
	 */
	UE_FORCEINLINE_HINT void RemoveAtSwap(SizeType Index, SizeType Count, EAllowShrinking AllowShrinking = UE::Core::Private::AllowShrinkingByDefault<AllocatorType>())
	{
		RangeCheck(Index, Count);
		if (Count)
		{
	 ...
	 * @return The number of elements that were added to the array, or 0 if Num() >= DesiredNum.
	 */
	SizeType PadToNumDefaulted(SizeType DesiredNum)
	{
		SizeType Diff = DesiredNum - Num();
		if (Diff <= 0)
		{
			return 0;
		}
	 ...
	 * @return The number of elements that were added to the array, or 0 if Num() >= DesiredNum.
	 */
	SizeType PadToNum(SizeType DesiredNum, const ElementType& PadValue)
	{
		SizeType Diff = DesiredNum - Num();
		if (Diff <= 0)
		{
		...
		SizeType Pos = AddUninitialized(Diff);
		ElementType* Data = GetData() + Pos;
		for (SizeType Index = 0; Index != Diff; ++Index, ++Data)
		{
			(void)new ((void*)Data) ElementType(PadValue);
		}
	...
	void Append(RangeType&& Source)
	{
		...
		// Allocate memory for the new elements.
		SizeType Pos = AddUninitialized(SourceCount);
		ConstructItems<ElementType>((void*)(GetData() + Pos), GetDataHelper(Source), SourceCount);
	}
...
public:
	...
	{
		return HeapPush(MoveTempIfPossible(InItem), TLess<>());
	}
	...
	{
		HeapPop(OutItem, TLess<>(), AllowShrinking);
	}
	...
	{
		HeapPopDiscard(TLess<>(), AllowShrinking);
	}
	...
	{
		HeapRemoveAt(Index, TLess<>(), AllowShrinking);
	}
...
{
	static constexpr bool Value = ::TAllocatorTraits<AllocatorType>::IsZeroConstruct;
};
