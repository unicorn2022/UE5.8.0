...
public:
	...
	typedef typename BaseMapType::KeyInitType KeyInitType;
...
#undef WITH_STRIPEDMAP_DEBUGGABLE_MUTEX
	...
	template <typename FunctionType>
	decltype(auto) ApplyUnlocked(KeyInitType InKey, FunctionType&& InFunction)
	{
	...
	template <typename LockType, typename FunctionType>
	decltype(auto) Apply(KeyInitType InKey, FunctionType&& InFunction) const
	{
	...
	template <typename LockType, typename FunctionType>
	decltype(auto) Apply(KeyInitType InKey, FunctionType&& InFunction)
	{
		return ApplyByHash<LockType>(KeyFuncsType::GetKeyHash(InKey), Forward<FunctionType>(InFunction));
	}

	template <typename FunctionType>
	decltype(auto) Write(KeyInitType InKey, FunctionType&& InFunction)
	{
		return Apply<ExclusiveLockType>(InKey, Forward<FunctionType>(InFunction));
	}

	template <typename FunctionType>
	decltype(auto) WriteByHash(uint32 InHash, KeyInitType InKey, FunctionType&& InFunction)
	{
		return ApplyByHash<ExclusiveLockType>(InHash, InKey, Forward<FunctionType>(InFunction));
	}

	template <typename FunctionType>
	decltype(auto) Read(KeyInitType InKey, FunctionType&& InFunction)
	{
		return Apply<SharedLockType>(InKey, Forward<FunctionType>(InFunction));
	}

	template <typename FunctionType>
	decltype(auto) ReadByHash(uint32 InHash, KeyInitType InKey, FunctionType&& InFunction)
	{
		return ApplyByHash<SharedLockType>(InHash, InKey, Forward<FunctionType>(InFunction));
	}

	template <typename FunctionType>
	decltype(auto) Read(KeyInitType InKey, FunctionType&& InFunction) const
	{
		return Apply<SharedLockType>(InKey, Forward<FunctionType>(InFunction));
	}

	template <typename FunctionType>
	decltype(auto) ReadByHash(uint32 InHash, KeyInitType InKey, FunctionType&& InFunction) const
	{
		return ApplyByHash<SharedLockType>(InHash, InKey, Forward<FunctionType>(InFunction));
	}
	...
	template <typename FunctionType>
	void ParallelForEachMap(FunctionType&& InFunction)
	{
		ParallelFor(
			BucketCount,
			[this, &InFunction](int32 BucketIndex)
			{
	...
	template <typename FunctionType>
	void ParallelForEachMap(FunctionType&& InFunction) const
	{
		ParallelFor(
			BucketCount,
			[this, &InFunction](int32 BucketIndex)
...
public:
	 ...
	 */
	ValueType FindRef(KeyInitType InKey) const
	{
	 ...
	 */
	template <typename FunctionType>
	bool FindAndApply(KeyInitType InKey, FunctionType&& InFunction) const
	{
	...
	bool FindAndApply(KeyInitType InKey, UpdateFunctionType&& InUpdateFunction)
	{
	 ...
	 */
	[[nodiscard]] bool Contains(KeyInitType InKey) const
	{
	...
	*/
	...
	void FindOrProduceAndApplyForWrite(const KeyType& InKey, ProduceFunctionType&& InProduceFunction, ApplyFunctionType&& InApplyFunction)
	{
		return Write(InKey, [&InKey, &InProduceFunction, &InApplyFunction](uint32 Hash, MapType& Map)
		{
			...
			{
				InApplyFunction(Map.EmplaceByHash(Hash, InKey, InProduceFunction()));
			}
		});
	}
	...
	*/
	 ...
	 */
	int32 Remove(KeyInitType InKey)
	{
	...
	int32 RemoveByHash(uint32 InKeyHash, KeyInitType InKey)
	{
	...
	int32 RemoveIf(KeyInitType InKey, PredicateType&& InPredicate)
	{
		return Write(InKey,
	 ...
	 * Removes all elements where the predicate returns true.
	 *
	 * @param InPredicate  A thread-safe predicate to call to determine if a pair should be removed.
	 * @return The number of pairs that were removed.
	 */
	template <typename PredicateType>
	int32 ParallelRemoveIf(PredicateType&& InPredicate)
	{
		std::atomic<int32> RemovedCount = 0;
		ParallelForEachMap(
			[&RemovedCount, &InPredicate](MapType& Map)
			{
				int32 LocalRemoveCount = 0;
				for (auto It = Map.CreateIterator(); It; ++It)
				{
					if (InPredicate(*It))
					{
	 ...
	 */
	bool RemoveAndCopyValue(KeyInitType InKey, ValueType& OutRemovedValue)
	{
	 ...
	 */
	ValueType FindAndRemoveChecked(KeyInitType InKey)
	{
	...
	[[nodiscard]] SIZE_T GetAllocatedSize() const
	{
		SIZE_T Total = 0;
		ForEachMap(
			[&Total](const MapType& Map)
			{
				Total += Map.GetAllocatedSize();
			}
		);
	 ...
	 *
	 * @param Number The number of elements that the map should be able to contain after allocation.
	 * @see Shrink
	 */
	void Reserve(int32 Number)
	{
		int32 NumberPerMap = Number / BucketCount;
		ForEachMap(
			[NumberPerMap](MapType& Map)
			{
				Map.Reserve(NumberPerMap);
			}
		);
	}
	 ...
	 * Calls a function on all elements of the map with exclusive access (elements can be modified).
	 *
	 * @param InFunction A thread-safe callback to run on each Pair of the map.
	 */
	template <typename FunctionType>
	void ParallelForEach(FunctionType&& InFunction)
	{
		ParallelForEachMap(
			[&InFunction](MapType& Map)
			{
	 ...
	 */
	template <typename FunctionType>
	void ParallelForEach(FunctionType&& InFunction) const
	{
		ParallelForEachMap(
			[&InFunction](const MapType& Map)
			{
...
public:
	 ...
	 */
	template<typename Allocator>
	void MultiFind(typename Super::KeyInitType InKey, TArray<ValueType, Allocator>& OutValues, bool bMaintainOrder = false) const
	{
