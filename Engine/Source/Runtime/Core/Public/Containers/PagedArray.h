...
public:
	...
	/**
	 * Returns maximum number of pages before reallocation.
	 *
	 * @returns Returns maximum number of pages before reallocation.
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT SizeType MaxPages() const
	{
		return Pages.Num();
	}

	/**
	 * Returns number of pages used for the number of elements in array.
	 *
	 * @returns Returns number of pages used for the number of elements in array.
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT SizeType NumPages() const
	{
		SizeType NumPages = NumRequiredPages(Count);
		checkSlow(static_cast<int32>(NumPages) <= Pages.Num());
		return NumPages;
	}
...
private:
	TArray<PageType, AllocatorType> Pages;
	SizeType Count = 0;
