...
public:
	...
	[[nodiscard]] TSpscQueue() : TSpscQueue(::new(AllocatorType::Malloc(sizeof(FNode), alignof(FNode))) FNode)
	{
	}
	...
	{
		UE_AUTORTFM_REPORT_HAZARD_IF_CLOSED(SpscQueue);
...
private:
	...
	[[nodiscard]] explicit TSpscQueue(FNode* Node) : Tail(Node), Head(Node), First(Node), TailCopy(Node)
	{
	}
