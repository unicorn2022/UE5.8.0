...
namespace UE::Core::ArrayView::Private
{
	...
	struct TIsCompatibleRangeType
	{
		...
		{
			return GetDataHelper(Forward<T>(Arg));
		}
	};
...
public:
	...
	[[nodiscard]] constexpr TArrayView(std::initializer_list<ElementType> List UE_LIFETIMEBOUND)
		requires UE::CConst<ElementType>
		: DataPtr(GetDataHelper(List))
	...
	>
	UE_REWRITE constexpr TArrayView& operator=(OtherRangeType&& Other)
	{
		return *this = TArrayView(Forward<OtherRangeType>(Other));
	}
	template <
		typename OtherRangeType,
		typename CVUnqualifiedOtherRangeType = std::remove_cv_t<std::remove_reference_t<OtherRangeType>>
	...
	>
	UE_REWRITE constexpr TArrayView& operator=(OtherRangeType&& Other UE_LIFETIMEBOUND)
	{
		return *this = TArrayView(Forward<OtherRangeType>(Other));
	}
	 ...
	 * The caller is responsible for ensuring that the view does not outlive the initializer list.
	 */
	UE_REWRITE constexpr TArrayView& operator=(std::initializer_list<ElementType> List UE_LIFETIMEBOUND)
		requires UE::CConst<ElementType>
	{
		return *this = TArrayView(List);
	}
...
public:
	...
	template <typename RangeType>
		requires (std::is_convertible_v<decltype(GetDataHelper(std::declval<RangeType&>())), const ElementType*>)
	[[nodiscard]] bool UEOpEquals(RangeType&& Rhs) const
	{
		auto Count = GetNum(Rhs);
		return Count == ArrayNum && CompareItems(DataPtr, GetDataHelper(Rhs), Count);
	}
...
template <
	...
	UE_REQUIRES(TIsContiguousContainer<CVUnqualifiedOtherRangeType>::Value && TIsTArrayView_V<CVUnqualifiedOtherRangeType>)
>
[[nodiscard]] constexpr auto MakeArrayView64(OtherRangeType&& Other)
{
	return TArrayView64<std::remove_pointer_t<decltype(GetData(DeclVal<OtherRangeType&>()))>>(Forward<OtherRangeType>(Other));
}
...
template <
	...
	UE_REQUIRES(TIsContiguousContainer<CVUnqualifiedOtherRangeType>::Value && !TIsTArrayView_V<CVUnqualifiedOtherRangeType>)
>
[[nodiscard]] constexpr auto MakeArrayView64(OtherRangeType&& Other UE_LIFETIMEBOUND)
{
	return TArrayView64<std::remove_pointer_t<decltype(GetData(DeclVal<OtherRangeType&>()))>>(Forward<OtherRangeType>(Other));
}
...
template<typename ElementType>
[[nodiscard]] constexpr TArrayView64<ElementType> MakeArrayView64(ElementType* Pointer UE_LIFETIMEBOUND, int64 Size)
{
	return TArrayView64<ElementType>(Pointer, Size);
}
...
template <typename ElementType>
[[nodiscard]] constexpr TArrayView64<const ElementType> MakeArrayView64(std::initializer_list<ElementType> List UE_LIFETIMEBOUND)
{
	return TArrayView64<const ElementType>(List.begin(), List.size());
}
...
template <
	...
	UE_REQUIRES(TIsContiguousContainer<CVUnqualifiedOtherRangeType>::Value && TIsTArrayView_V<CVUnqualifiedOtherRangeType>)
>
[[nodiscard]] constexpr auto MakeConstArrayView64(OtherRangeType&& Other)
{
	return TArrayView64<const std::remove_pointer_t<decltype(GetData(DeclVal<OtherRangeType&>()))>>(Forward<OtherRangeType>(Other));
}
...
template <
	...
	UE_REQUIRES(TIsContiguousContainer<CVUnqualifiedOtherRangeType>::Value && !TIsTArrayView_V<CVUnqualifiedOtherRangeType>)
>
[[nodiscard]] constexpr auto MakeConstArrayView64(OtherRangeType&& Other UE_LIFETIMEBOUND)
{
	return TArrayView64<const std::remove_pointer_t<decltype(GetData(DeclVal<OtherRangeType&>()))>>(Forward<OtherRangeType>(Other));
}
...
template<typename ElementType>
[[nodiscard]] constexpr TArrayView64<const ElementType> MakeConstArrayView64(const ElementType* Pointer UE_LIFETIMEBOUND, int64 Size)
{
	return TArrayView64<const ElementType>(Pointer, Size);
}
...
template <typename ElementType>
[[nodiscard]] constexpr TArrayView64<const ElementType> MakeConstArrayView64(std::initializer_list<ElementType> List UE_LIFETIMEBOUND)
{
	return TArrayView64<const ElementType>(List.begin(), List.size());
}
...
template<typename InElementType, typename InAllocatorType>
template<typename OtherElementType, typename OtherSizeType>
[[nodiscard]] UE_FORCEINLINE_HINT TArray<InElementType, InAllocatorType>::TArray(ElementAllocatorType&& InAllocatorInstance, const TArrayView<OtherElementType, OtherSizeType>& Other)
