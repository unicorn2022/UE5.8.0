...
/**
 * Traits which determine whether or not a type is a TMap.
 */
template <typename T>
inline constexpr bool TIsTMap_V = false;

template <typename T> inline constexpr bool TIsTMap_V<const          T> = TIsTMap_V<T>;
template <typename T> inline constexpr bool TIsTMap_V<      volatile T> = TIsTMap_V<T>;
template <typename T> inline constexpr bool TIsTMap_V<const volatile T> = TIsTMap_V<T>;

template <typename T>
struct TIsTMap
{
	static inline constexpr bool Value = TIsTMap_V<T>;
};
