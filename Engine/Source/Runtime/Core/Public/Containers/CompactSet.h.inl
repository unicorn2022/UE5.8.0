...
// Array view is a unique feature for compact sets since the data is not allowed to have holes like sparse set
// However to maintain compatibilty with sparse set this feature is not available by default
#if !defined(UE_TCOMPACT_SET_ARRAY_VIEW)
#define UE_TCOMPACT_SET_ARRAY_VIEW 0
#endif

#if UE_TCOMPACT_SET_ARRAY_VIEW || defined(UE_TCOMPACT_SET_FORCE_ARRAY_VIEW)
#define UE_TCOMPACT_SET_ARRAY_VIEW_ENABLED 1
#else
#define UE_TCOMPACT_SET_ARRAY_VIEW_ENABLED 0
#endif

#define TSETPRIVATEFRIEND UE_JOIN(UE_TCOMPACT_SET, PrivateFriend)
 ...
 *  ____________ ___________ ________________ _____________ ____________
 * |            |           |                | (Optional)  |            |
 * | Data Array | Hash Size | Collision List | Key Storage | Hash Table |
 * |____________|___________|________________|_____________|____________|
 * 
 ...
 * (Optional) Key Storage - If enabled, stores "GetHashKey(Item) % (HashSize - 1)" so we can remove an item without having to rehash it.
...
private:
	...
	public:
		...
		[[nodiscard]] explicit UE_FORCEINLINE_HINT TBaseIterator(SetType& InSet)
		: TBaseIterator(InSet, 0)
		{
		}

		[[nodiscard]] explicit inline TBaseIterator(SetType& InSet, SizeType StartIndex)
		: Set(InSet)
		, Index(StartIndex)
#if DO_CHECK
		...
		{
#if DO_CHECK
			checkf(LastRemoved == INDEX_NONE, TEXT("Accessing iterator after it was removed from the set and before it was incremented to the next valid elements."));
#endif
...
#if DO_CHECK
		SizeType InitialNum;
		SizeType LastRemoved = INDEX_NONE;
#endif
	};
	...
	public:
		...
		[[nodiscard]] explicit inline TBaseKeyIterator(SetType& InSet, KeyArgumentType InKey)
		: Set  (InSet)
		, Key  (InKey)
...
public:
	...
	public:
		...
		{
#if DO_CHECK
			checkf(this->LastRemoved == INDEX_NONE, TEXT("RemoveCurrent was already called for the current iteration index of %d"), this->LastRemoved);
			this->LastRemoved = this->Index;
			--this->InitialNum;
#endif
	...
	public:
		...
		inline void RemoveCurrent()
		{
			...
			if (this->NextIndex == this->Set.Num())
			{
	...
	void Reset()
	{
		if (this->NumElements > 0)
		{
			...
			if (this->NumElements < (SizeType)(HashTable.GetHashCount() / UE_SET_UNHASH_THRESHOLD_STORED_KEY))
			{
				for (int32 Index = 0; Index < this->NumElements; ++Index)
				{
					HashTable.RemoveHashElement(HashTable.GetStoredKey(Index, this->NumElements));
				}
			}
			#else
			if (this->NumElements < (SizeType)(HashTable.GetHashCount() / UE_SET_UNHASH_THRESHOLD_CALCULATE_KEY))
			{
				// if we've only used a portion of the full allocated size then it can be faster
				// to only clear out the used hash elements instead of memsetting the whole of the memory
				// The exact number can be hard to determine since we have to recalculate the hash value to clear it

				for (const ElementType& Element : *this)
				{
					const KeyInitType Key = KeyFuncs::GetSetKey(Element);
					const uint32 KeyHash = KeyFuncs::GetKeyHash(Key);

					HashTable.RemoveHashElement(KeyHash);
				}
			}
			#endif
			else
	...
	void Compact()
	{
		// Data is always compacted, but this is useful to be able to more easily swap between sparse and compact sets
		// We can at least perform a shrink here to maintain memory parity with sparse set
		Shrink();
	}
	...
	inline void RangeCheck(FSetElementId Id) const
	{
		...
		{
			checkf(IsValidId(Id), TEXT("Array index out of bounds: %d into an array of size %d"), Id.AsInteger(), this->NumElements);
		}
	}
	...
	{
		const uint32 KeyHash = KeyFuncs::GetKeyHash(KeyFuncs::GetSetKey(InElement));
		return FindOrAddByHash(KeyHash, MoveTempIfPossible(InElement), bIsAlreadyInSetPtr);
	}
	...
	void Dump(FOutputDevice& Ar) const
	{
		if (this->MaxElements == 0)
		{
			Ar.Log(TEXT("UE_TCOMPACT_SET: empty"));
		}
		else if (this->MaxElements > 0)
		{
			FConstCompactHashTableView HashTable = this->GetConstHashTableView();

			Ar.Logf(TEXT("UE_TCOMPACT_SET: %d elements, %d max elements, %d hash slots"), this->NumElements, this->MaxElements, HashTable.GetHashCount());
	...
	/** @return the symmetric difference of two sets. (A not in B and B not in A where A is this and B is Other) */
	[[nodiscard]] UE_TCOMPACT_SET SymmetricDifference(const UE_TCOMPACT_SET& OtherSet) const
	{
		UE_TCOMPACT_SET Result;
		Result.Reserve(this->NumElements + OtherSet.NumElements); // Worst case: no overlap

		// Add all elements from A that are not in B
		for (const ElementType& Element : *this)
		{
			if (!OtherSet.Contains(KeyFuncs::GetSetKey(Element)))
			{
		...
		// Add all elements from B that are not in A
		for (const ElementType& Element : OtherSet)
		{
			if (!this->Contains(KeyFuncs::GetSetKey(Element)))
			{
				Result.Add(Element);
			}
		}
	...
	/** @return a TArray that's a copy of all the elements in this set, prefer using ArrayView if available. */
	[[nodiscard]] TArray<ElementType> Array() const
	{
		return TArray<ElementType>(GetData(), this->NumElements);
	}

#if UE_TCOMPACT_SET_ARRAY_VIEW_ENABLED
...
private:
	...
	[[nodiscard]] SizeType FindIndexByHash(uint32 KeyHash, const ComparableKey& Key) const
	{
		...
		#define UE_COMPACTHASHTABLE_EXECUTEBYTYPE(Type) \
				...
				const Type* HashIndices = NextIndices + this->MaxElements * this->KeyStorageMultiplier; \
	...
	{
		checkf(ElementIndex >= 0 && ElementIndex < this->NumElements, TEXT("Invalid ElementIndex (%d) passed to UE_TCOMPACT_SET::RemoveByIndex. NumElements=%d"), ElementIndex, this->NumElements);

		ElementType* ElementsData = GetData();
		FCompactHashTableView HashTable = GetHashTableView();

		#if UE_ENABLE_COMPACTHASHTABLE_KEYSTORAGE
		RemoveByIndexAndHash<IsStable>(ElementIndex, HashTable.GetStoredKey(ElementIndex, this->NumElements), ElementsData, HashTable);
		#else
		RemoveByIndexAndHash<IsStable>(ElementIndex, KeyFuncs::GetKeyHash(KeyFuncs::GetSetKey(ElementsData[ElementIndex])), ElementsData, HashTable);
		#endif
	}

	template <bool IsStable = false>
	SizeType RemoveByIndexAndHash(const SizeType ElementIndex, const uint32 KeyHash, ElementType* ElementsData, FCompactHashTableView& HashTable)
	{
		const SizeType LastElementIndex = this->NumElements - 1;
		SizeType NextIndex; 
		
		// Note: Destructing/moving elements for better debugability
		if (ElementIndex == LastElementIndex)
		{
			NextIndex = HashTable.Remove(ElementIndex, KeyHash, ElementIndex, KeyHash);
			
			ElementsData[LastElementIndex].~ElementType();
		}
		else if constexpr (IsStable)
		{
			NextIndex = HashTable.RemoveStable(ElementIndex, KeyHash);
		...
		{
			#if UE_ENABLE_COMPACTHASHTABLE_KEYSTORAGE
			const uint32 LastElementHash = HashTable.GetStoredKey(LastElementIndex, this->NumElements);
			#else
			const uint32 LastElementHash = KeyFuncs::GetKeyHash(KeyFuncs::GetSetKey(ElementsData[LastElementIndex]));
			#endif
			NextIndex = HashTable.Remove(ElementIndex, KeyHash, LastElementIndex, LastElementHash);
	...
	int32 RemoveImpl(uint32 KeyHash, const ComparableKey& Key)
	{
		...
		{
			checkf(ElementIndex >= 0 && ElementIndex < this->NumElements, TEXT("Invalid ElementIndex (%d) passed to UE_TCOMPACT_SET::RemoveByIndexAndHash. NumElements=%d"), ElementIndex, this->NumElements);
			if (KeyFuncs::Matches(KeyFuncs::GetSetKey(ElementsData[ElementIndex]), Key))
			{
				if constexpr (!KeyFuncs::bAllowDuplicateKeys)
				{
					RemoveByIndexAndHash<IsStable>(ElementIndex, KeyHash, ElementsData, HashTable);
					// If the hash disallows duplicate keys, we're done removing after the first matched key.
					return 1;
				}
				else
				{
					ElementIndex = RemoveByIndexAndHash<IsStable>(ElementIndex, KeyHash, ElementsData, HashTable);
					NumRemovedElements++;
				}
			}
			else
			{
				ElementIndex = HashTable.GetNext(ElementIndex, this->NumElements);
			}
		}
...
struct TSETPRIVATEFRIEND
{
	...
	static FArchive& Serialize(FArchive& Ar, UE_TCOMPACT_SET<ElementType, KeyFuncs, Allocator>& Set)
	{
		...
		else
		{
			...
			// Modifying refs is similar to load, in that we need to rehash the values after serializing.
			// Ex: See FArchiveReplaceObjectRef, which is often used to patch references via serialization.
			// This is reflecting the same logic being used in TScriptSparseSet for now to ensure correctness.
			// @todo - Maybe this could be optimized later to only rehash when the elements are pointer types.
			if (Ar.IsModifyingWeakAndStrongReferences() && !Ar.IsSaving())
			{
				Set.Rehash();
			}
		}
...
template <typename ElementType, typename KeyFuncs, typename Allocator>
inline constexpr bool TIsTSet_V<UE_TCOMPACT_SET<ElementType, KeyFuncs, Allocator>> = true;

#undef TSETPRIVATEFRIEND
#undef UE_TCOMPACT_SET_ARRAY_VIEW_ENABLED