...
public:
	...
	TLruCache(const TLruCache& Other)
	{
		*this = Other;
	}

	TLruCache& operator=(const TLruCache& Other)
	{
		Empty(Other.MaxNumElements);

		// Copy from least recent to most so that we keep the usage state through copy
		const FCacheEntry* CurrentEntry = Other.LeastRecent;
		while (CurrentEntry)
		{
	...
	TLruCache(TLruCache&& Other) = default;
	TLruCache& operator=(TLruCache&& Other) = default;
...
public:
	 ...
	 * 
	 * @param InMaxNumElements The maximum number of elements this cache can hold
	 * @param InAllowShrinking If InMaxNumElements is smaller than current MaxNumElements, indicates if we want to compact internal memory
	 */
	void SetMaxNumElements(int32 InMaxNumElements, EAllowShrinking InAllowShrinking = EAllowShrinking::Default)
	{
		if (InMaxNumElements < MaxNumElements)
		{
			while (LookupSet.Num() > InMaxNumElements)
			{
				RemoveLeastRecent();
			}

			if (InAllowShrinking == EAllowShrinking::Yes)
			{
				LookupSet.Shrink();
			}
		}

		LookupSet.Reserve(InMaxNumElements);
		MaxNumElements = InMaxNumElements;
	}
