...
public:
	...
	[[nodiscard]] inline T* GetItem(uint32 Index)
	{
		...
		checkLockFreePointerList(Index < (uint32)NextIndex.GetValue() && Index < MaxTotalItems && BlockIndex < MaxBlocks && Pages[BlockIndex].load(std::memory_order_relaxed));
		return Pages[BlockIndex].load(std::memory_order_relaxed) + SubIndex;
	}
...
private:
	[[nodiscard]] void* GetRawItem(uint32 Index)
	{
		...
		if (!Pages[BlockIndex].load(std::memory_order_relaxed))
		{
			...
			if (!Pages[BlockIndex].compare_exchange_strong(ExpectedValue, NewBlock))
			{
				// we lost discard block
				checkLockFreePointerList(Pages[BlockIndex].load(std::memory_order_relaxed) && Pages[BlockIndex].load(std::memory_order_relaxed) != NewBlock);
				LockFreeFreeLinks(ItemsPerPage * sizeof(T), NewBlock);
			}
			else
			{
				checkLockFreePointerList(Pages[BlockIndex].load(std::memory_order_relaxed));
			}
		}
		return (void*)(Pages[BlockIndex].load(std::memory_order_relaxed) + SubIndex);
	}

	alignas(PLATFORM_CACHE_LINE_SIZE) FThreadSafeCounter NextIndex;
	alignas(PLATFORM_CACHE_LINE_SIZE) std::atomic<T*> Pages[MaxBlocks];
};
