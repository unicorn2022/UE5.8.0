...
public:
	...
	friend uint32 GetTypeHashWithNumber(FNameEntryId Id, uint32 Number)
	{
		const uint32 UnstableInt = Id.ToUnstableInt();
		return HashCombineFast(UnstableInt + Number, UnstableInt);
	}
#endif
...
public:
	...
	/** Get name without number part as a dynamically allocated shared string */
	[[nodiscard]] CORE_API UE::FSharedString GetPlainNameSharedString() const;
...
public:
	...
	friend FORCEINLINE uint32 GetTypeHash(FMinimalName Name)
	{
		return GetTypeHashWithNumber(Name.Index, Name.Number);
	}
#endif
};
...
public:
	...
	friend FORCEINLINE uint32 GetTypeHash(FScriptName Name)
	{
		return GetTypeHashWithNumber(Name.ComparisonIndex, Name.Number);
	}
#endif
};
...
public:
...
#if UE_FNAME_OUTLINE_NUMBER
	// This should really be named GetInternalNumber().
	[[nodiscard]] CORE_API int32 GetNumber() const;
	// This should really be named SetInternalNumber().
	...
	// This should really be named SetInternalNumber().
	FORCEINLINE void SetNumber(const int32 NewInternalNumber)
	{
		Number = NewInternalNumber;
	}
#endif //UE_FNAME_OUTLINE_NUMBER
...
public:
	...
	/**
	 * Checks to see if an internal number is valid, such as those provided to the FName(Name, Number) constructors.
	 *
	 * @param	InInternalNumber	The number to check.
	 *
	 * @return	true if the number is valid.
	 */
	[[nodiscard]] static constexpr bool IsValidInternalNumber(int32 InInternalNumber)
	{
		return InInternalNumber >= NAME_NO_NUMBER_INTERNAL;
	}
	...
	* @return the new FName based upon the calling FName, but with any Whitespace characters potentially problematic for
	...
	/**
	 * Compares name to passed in one. Sort is case-sensitive and alphabetical ascending.
	 *
	 * @param	Other	Name to compare this against
	 * @return	< 0 is this < Other, 0 if this == Other, > 0 if this > Other
	 */
	[[nodiscard]] CORE_API int32 CompareSensitive( const FName& Other ) const;
	 ...
	 * @param InInternalNumber The hardcoded value for the number portion of the name, in internal format
	 */
	FORCEINLINE FName(EName Ename, int32 InInternalNumber)
		: FName(CreateNumberedNameIfNecessary(FNameEntryId::FromEName(Ename), InInternalNumber))
	{
	}
	 ...
	 * @param InInternalNumber The hardcoded value for the number portion of the name, in internal format
	 */
	FORCEINLINE FName(FName Other, int32 InInternalNumber)
		: FName(CreateNumberedNameIfNecessary(Other.GetComparisonIndex(), Other.GetDisplayIndex(), InInternalNumber))
	{
	}
	 ...
	 */
	FORCEINLINE FName(FNameEntryId InComparisonIndex, FNameEntryId InDisplayIndex, int32 InInternalNumber)
		: FName(CreateNumberedNameIfNecessary(InComparisonIndex, InDisplayIndex, InInternalNumber))
	{
	}
	 ...
	 * Only call this if you *really* know what you're doing
	 */
	[[nodiscard]] static FName CreateFromDisplayId(FNameEntryId DisplayId, int32 InInternalNumber)
	{
#if UE_FNAME_OUTLINE_NUMBER
...
#endif
		return FName(GetComparisonIdFromDisplayId(DisplayId), DisplayId, InInternalNumber);
	}

#if UE_FNAME_OUTLINE_NUMBER
	CORE_API static FName FindNumberedName(FNameEntryId DisplayId, int32 InInternalNumber);
#endif //UE_FNAME_OUTLINE_NUMBER
	 ...
	 *
	 * @param Name Value for the string portion of the name
	 * @param InInternalNumber Value for the number portion of the name, in internal format
	 */
	CORE_API FName(const WIDECHAR* Name, int32 InInternalNumber);
	CORE_API FName(const ANSICHAR* Name, int32 InInternalNumber);
	CORE_API FName(const UTF8CHAR* Name, int32 InInternalNumber);
	CORE_API FName(int32 Len, const WIDECHAR* Name, int32 InInternalNumber);
	CORE_API FName(int32 Len, const ANSICHAR* Name, int32 InInternalNumber);
	CORE_API FName(int32 Len, const UTF8CHAR* Name, int32 InInternalNumber);

	inline FName(TStringView<ANSICHAR> View, int32 InInternalNumber)
		: FName(NoInit)
	{
		*this = FName(View.Len(), View.GetData(), InInternalNumber);
	}
	inline FName(TStringView<WIDECHAR> View, int32 InInternalNumber)
		: FName(NoInit)
	{
		*this = FName(View.Len(), View.GetData(), InInternalNumber);
	}
	inline FName(TStringView<UTF8CHAR> View, int32 InInternalNumber)
		: FName(NoInit)
	{
		*this = FName(View.Len(), View.GetData(), InInternalNumber);
	}
	 ...
	 */
	CORE_API FName(const TCHAR* Name, int32 InInternalNumber, bool bSplitName);
	...
	*/
	...
	UE_DEPRECATED(5.8, "Tests for FName are registered with both test frameworks now.")
	inline static void AutoTest()
	{
	}
...
private:
	...
	FORCEINLINE int32 CompareInternal(const FName& Other) const;
...
#if UE_FNAME_OUTLINE_NUMBER
	[[nodiscard]] CORE_API static FName CreateNumberedName(FNameEntryId ComparisonId, FNameEntryId DisplayId, int32 InInternalNumber);
#endif

	[[nodiscard]] FORCEINLINE static FName CreateNumberedNameIfNecessary(FNameEntryId ComparisonId, FNameEntryId DisplayId, int32 InInternalNumber)
	{
#if UE_FNAME_OUTLINE_NUMBER
		if (InInternalNumber != NAME_NO_NUMBER_INTERNAL)
		{
			// We need to store a new entry in the name table
			return CreateNumberedName(ComparisonId, DisplayId, InInternalNumber);
		}

#if DO_CHECK
		if (!IsValidInternalNumber(InInternalNumber))
		{
			ReportInvalidInternalNumber(ComparisonId, InInternalNumber);
		}
#endif
...
#endif
#if !UE_FNAME_OUTLINE_NUMBER
		Out.Number = InInternalNumber;
#endif
		return Out;
	}

	[[nodiscard]] UE_REWRITE static FName CreateNumberedNameIfNecessary(FNameEntryId ComparisonId, int32 InInternalNumber)
	{
		return CreateNumberedNameIfNecessary(ComparisonId, ComparisonId, InInternalNumber);
	}
	...
	[[nodiscard]] friend FORCEINLINE uint32 GetTypeHash(FName Name)
	{
		return GetTypeHashWithNumber(Name.GetComparisonIndex(), Name.GetNumber());
	}
#endif

#if DO_CHECK
	[[noreturn]] CORE_API static void ReportInvalidInternalNumber(FNameEntryId ComparisonId, int32 InternalNumber);
#endif
