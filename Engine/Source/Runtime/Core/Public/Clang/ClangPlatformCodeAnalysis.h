...
//
	...
	#define USING_UNDEFINED_BEHAVIOR_SANITISER 1
#else
	#define USING_UNDEFINED_BEHAVIOR_SANITISER 0
#endif

#if defined(__has_feature) && __has_feature(memory_sanitizer)
	#define USING_MEMORY_SANITISER 1
#else
	#define USING_MEMORY_SANITISER 0
#endif

#if USING_UNDEFINED_BEHAVIOR_SANITISER
	#define UE_DISABLE_UBSAN [[clang::no_sanitize("undefined")]]
#endif
...
// We do want all the additional annotations for the instrumentation too.
	...
	// Thread-sanitiser annotation functions.
		...
		// both open and closed code call directly, no closed variant or trampoline generated.
		#include "AutoRTFM/Constants.h"
		#define ANNOTATE_FUNC_ATTRIBUTES __attribute__((autortfm(autortfm_mode_internal)))
	#else
		#define ANNOTATE_FUNC_ATTRIBUTES
	#endif
	#ifdef __cplusplus
	extern "C" {
	#endif
		ANNOTATE_FUNC_ATTRIBUTES void AnnotateHappensBefore(const char *f, int l, void *addr);
		ANNOTATE_FUNC_ATTRIBUTES void AnnotateHappensAfter(const char *f, int l, void *addr);
	#ifdef __cplusplus
	}
	#endif
	#undef ANNOTATE_FUNC_ATTRIBUTES
