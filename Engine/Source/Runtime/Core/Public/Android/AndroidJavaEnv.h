...
namespace UE::Jni
{
	...
	struct FMember
	{
		...
		template <typename T, typename... TArgs>
		UE_DEPRECATED(5.8, "TConstructor<T(TArgs...) instead.")
	...
	inline thread_local const struct FEnv
	{
		...
		void Initialize() const
		{
			...
			else
			{
				...
				{
					FScopedJavaObject<Java::Lang::TClass<typename T::PartialClass>*> LocalClass = Find<typename T::PartialClass>();
					...
					Class<typename T::PartialClass> = NewGlobalRef(*LocalClass);
				}
		...
		template <CContiguousRange T> requires(!std::is_convertible_v<const T&, FStringView>)
		auto Marshal(const T& Params) const
		{
			using U = decltype(Marshal(Params[0]))::Type;
			
			int32 Count = GetNum(Params);			
			FScopedJavaObject<Java::Lang::TArray<U>*> OutArray{reinterpret_cast<Java::Lang::TArray<U>*>(Value->NewObjectArray(Count, Class<std::remove_pointer_t<U>>, nullptr))};
			
			for (int32 Index = 0; Index < Count; ++Index)
			{
				Value->SetObjectArrayElement(*OutArray, Index, *Marshal(Params[Index]));
			}
			
			return OutArray;
		}

		FScopedJavaObject<Java::Lang::FString*> Marshal(FStringView Param) const;
		FString Marshal(Java::Lang::FString* Param) const;
	...
	{
	private:
		[[gnu::retain, gnu::used, gnu::section("UE_Jni_Initialize")]]
		static constexpr void (FEnv::*Function)() const = &FEnv::Initialize<T>;
	};

	template <typename T, typename... TArgs> requires(!std::is_function_v<T>)
	struct UE_DEPRECATED(5.8, "TConstructor<T(TArgs...) instead.") TConstructor<T, TArgs...>: TConstructor<T(TArgs...)>
	{
	};

	template <typename T, typename... TArgs>
	struct TConstructor<T(TArgs...)>
	{
	...
	namespace Java
	{
		namespace Lang
		{
			struct FClassLoader: FObject
			{
				...
				inline static TMember<FClassLoader*, jclass (jstring name)> loadClass;

				static constexpr FMember Members[]
				{
					UE_JNI_MEMBER(loadClass)
				};
			};
			
			struct FInteger: FObject
			{
				static constexpr FAnsiStringView ClassName = "java/lang/Integer";
		
				inline static TConstructor<FInteger(jint value)> New;
		...
		namespace Util
		{
			struct FHashMap: Lang::FObject
			{
				static constexpr FAnsiStringView ClassName = "java/util/HashMap";
		
				inline static TConstructor<FHashMap()> New;
				inline static TMember<FHashMap*, Lang::FObject* (Lang::FObject* key, Lang::FObject* value)> put;
