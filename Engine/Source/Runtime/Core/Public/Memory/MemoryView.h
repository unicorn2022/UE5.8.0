...
public:
	...
	/** Construct a view from a contiguous range of any type with compatible const/volatile qualifiers. */
	template <UE::CContiguousRange RangeType>
		requires std::is_convertible_v<decltype(GetDataHelper(DeclVal<RangeType>())), DataType*>
	constexpr inline TMemoryView(RangeType& Range)
		: TMemoryView(GetDataHelper(Range), (uint64)GetNum(Range) * sizeof(*GetDataHelper(Range)))
	{
	}
...
private:
	...
	template <UE::CVoid OtherDataType>
