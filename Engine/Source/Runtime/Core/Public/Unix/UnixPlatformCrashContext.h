...
struct FUnixCrashContext : public FGenericCrashContext
{
	...
	/**
	 * Captures callstacks from all threads for inclusion in crash reports.
	 * Must be called after CaptureStackTrace() so the crashing thread's stack is available.
	 */
...
private:
	/** Accumulated XML string for all thread contexts, populated by CaptureAllThreadContexts() */
	mutable FString AllThreadContextsString;
};
