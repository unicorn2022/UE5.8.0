...
struct FProcHandle
{
	...
	/** Process start time captured during OpenProcess(), used with OpenedPid to
	  * uniquely identify the process and detect PID reuse. Value is clock ticks
	  * since boot from /proc/<pid>/stat field 22. Zero if unavailable.
	  */
	uint64				OpenedStartTime;

	/** Pid of external process opened with OpenProcess() call. */
	pid_t				OpenedPid;

	constexpr FProcHandle()
	:	ProcInfo(nullptr), OpenedStartTime(0), OpenedPid(-1)
	{
	}

	FProcHandle(FProcState* InHandle)
	:	ProcInfo(InHandle), OpenedStartTime(0), OpenedPid(-1)
	{
	}

	FProcHandle(pid_t InProcPid)
	:	ProcInfo(nullptr), OpenedStartTime(0), OpenedPid(InProcPid)
	{
	}
