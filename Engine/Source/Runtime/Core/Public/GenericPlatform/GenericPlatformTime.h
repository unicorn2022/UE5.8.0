...
struct FGenericPlatformTime
{
	...
	/** Updates current thread CPU utilization, calling is user defined per-thread (unused float parameter, is for FCookTicker compatibility). */
	...
	/** Gets the platform time offset, if there is one. This is added to the value returned from Seconds() */
	static double GetSecondsTimeOffset()
	{
		return 0.0;
	}
