...
struct FGenericPlatformProcess
{
	...
	/** Get Default Thread Stack Size in bytes, 0 means to use the platform OS default size **/
	static uint32 GetDefaultThreadStackSize()
	{
		return 0;
	}

	/** Get Default Pool Thread Stack Size in bytes **/
	static uint32 GetDefaultPoolThreadStackSize()
	{
		return 128 * 1024;
	}
	...
	*/
	 ...
	 */
	// Message to others in the future, don't try to delete this function as it isn't exactly deprecated,
	// but it should only ever be called from TEventPool::GetEventFromPool()/FSafeRecyclableEvent::GetRawEvent()
