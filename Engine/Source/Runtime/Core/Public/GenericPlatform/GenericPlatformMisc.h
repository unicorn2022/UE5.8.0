...
/**
 * Generic Thermal State Enum returned by platform implementations of FGenericPlatformMisc::GetDeviceThermalState.
 * This is a direct mapping to Android getThermalStatus.  This also means that historical android status data matches.
 * IOS NSProcessInfoThermalState, which provides less detail, will be mapped onto the first 4 values.
 * Unless a platform provides enough to supply at least the None and Severe states GetDeviceThermalState should be left Unsupported.
 * In all cases platform specific stats may have better information.
 */
enum class EDeviceThermalState : int32
{
	Unsupported = -1,	// Feature is unsupported or an error is returned from the platform (which may also means unsupported on this device).

	None = 0,		// No thermal perf impacts.								If this function is supported on a platform this value should be meaningful.
	Light = 1,		// Device likely warm, but little or no throttling.
	Moderate = 2,	// Device likely warmer, but little or no throttling.
	Severe = 3,		// Serious throttling engaged, Device likely hot.		If this function is supported on a platform this value should be meaningful.
	Critical = 4,	// Fully throttling.
	Emergency = 5,	// Device functionality limited.
	Shutdown = 6,	// Device should shutdown.
};
...
namespace ECPUFeatureBits_X86
{
	...
	inline constexpr uint32 FMA = 1U << 12;
}

typedef TDelegate<void(bool), FDefaultDelegateUserPolicy> FOnAudioFocusChangedDelegate;
...
struct FGenericPlatformMisc
{
	...
	*/
	...
	UE_DEPRECATED(5.8, "Replace HasNonoptionalCPUFeatures() with false.")
	static inline bool HasNonoptionalCPUFeatures() { return false; }
	/** @return whether to check for specific CPU compatibility or not */
	UE_DEPRECATED(5.8, "Replace NeedsNonoptionalCPUFeaturesCheck() with false.")
	static inline bool NeedsNonoptionalCPUFeaturesCheck() { return false; }
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "FPlatformMisc::Is64bitOperatingSystem is deprecated as UE only supports 64-bit.")
	...
	*/
	...
	/**
	 * If bEnablePreferredInputAPIPreferences is enabled, then check if the given input device is
	 * "Preferred" by the user. If a null device name is given, then this function will always treat 
	 * that as a preferred API and return true.
	 * 
	 * @param DeviceName	The name of the input device API to check if it is preferred. This should come from
	 *						an IInputDeviceModule::GetPreferredDeviceAPIString call.
	 * @return True if the given input device API is preferred.
	 */
	static CORE_API bool IsPreferredInputDevice(const TCHAR* DeviceName);

	/**
	 * Sets the [GamepadAPIPreference]PreferredAPI config value in the GGameUserSettingsIni ini to the
	 * given value. This will then be used to determine if an input API is preferred on the next boot.
	 *
	 * NOTE: This will not take affect immediately. An engine reboot is required to determine which input APIs should be created.
	 * 
	 * @param DeviceName The full comma-separated list of preferred input APIs this application should use.
	 */
	static CORE_API void SetPreferredInputDevices(const TCHAR* DeviceName);

	/**
	 * Resets the users [GamepadAPIPreference]PreferredAPI config value
	 * and calls SetPreferredInputDevices with the project's default preferred
	 * input API list if available.
	 */
	static CORE_API void ResetPreferredInputDevicesToDefault();

	/**
	 * @return A comma-separated string value which represents a list of the preferred input APIs.
	 */
	static CORE_API FString GetCurrentPreferredInputDevices();
...
#	endif
	 ...
	 */
	UE_DEPRECATED(5.8, "GetDeviceTemperatureLevel() has been superseded by GetDeviceTemperature() and/or GetDeviceThermalState() because it's behavior was poorly defined (on android it returned temperature, on ios it returned a thermal status enum value).")
	static CORE_API float GetDeviceTemperatureLevel();

	/**
	 * Returns the current device temperature in degrees Celcius.
	 * Keep in mind that no specific temperature probe position can be assumed.  It will vary across platforms and devices.
	 * Inspecting the platform implementation might yeild more information about this stat or more detailed platform specific stats.
	 * -1.0 is returned for a platform that does not support this measure at all.
	 * A platform that does support it, but is not currently providing data may return -1 or 0.  
	 * Theoretically -1 and 0 could be real temperatures... 
	 */
	static CORE_API float GetDeviceTemperature();

	/**
	 * Returns the current device thermal state, which is a cross platform qualitative representation of the device's heat status.
	 * Briefly EDeviceThermalState::None (0) is the ideal state, EDeviceThermalState::Severe+ (3) is where serious throttling will start.
	 * See EDeviceThermalState and device specific implemetations for details.
	 * -1.0 is returned for a platform that does not support this measure at all.
	 */
	static CORE_API EDeviceThermalState GetDeviceThermalState();
	...
	/**
	 * Converts a network connection type to a network connection status.
	 * Maps None/AirplaneMode to Disabled, all known connection types to Connected.
	 */
	static ENetworkConnectionStatus NetworkConnectionTypeToStatus(ENetworkConnectionType ConnectionType)
	{
		switch (ConnectionType)
		{
		case ENetworkConnectionType::None:
		case ENetworkConnectionType::AirplaneMode:
			return ENetworkConnectionStatus::Disabled;
		...
		default:
			return ENetworkConnectionStatus::Connected;
		}
	}
	...
	/**
	 * Returns whether the device is considered a gaming handheld device. It's not a dynamic property, the status is cached at startup.
	 */
	static CORE_API bool IsDeviceGamingHandheld();
	...
	*/
	...
	/**
	* Register an audio focus change listener
	*/
	static CORE_API FDelegateHandle RegisterAudioFocusChangedEventListener(const FOnAudioFocusChangedDelegate Callback);

	/**
	* Unregister an audio focus change listener
	*/
	static CORE_API void UnregisterAudioFocusChangedEventListener(const FDelegateHandle DelegateHandle);
	...
	*/
	...
	/*
	* Returns the utf8 view of the config rule variable or empty if not found.
	*/
	static CORE_API const FUtf8StringView GetConfigRulesVariableAsUtf8(const FUtf8StringView Key);
	...
	*/
	...
	/*
	* Utility to sanitize names provided to FPlatformSystemWideMutex. 
	* Sanitization is done in-place and will not change the string length.
	*/
	static void SanitizeSystemWideMutexName(FString& InName);
