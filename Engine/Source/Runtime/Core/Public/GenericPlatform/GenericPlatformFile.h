...
enum class EPlatformFileWrite : uint8
{
	...
	AttemptDeleteAndCreate = 0x02 // if the target file cannot be open for write, attempt to delete it and create a new one in it splace
};
...
public:
	...
	*/
	...
	/** Return ESymlinkResult::Symlink if the file is a symbolic link, if unimplemented, try to go down a level */
	virtual ESymlinkResult IsSymlink(const TCHAR* Filename) 
	{ 
		if (GetLowerLevel())
		{
			return GetLowerLevel()->IsSymlink(Filename);
		}
		return ESymlinkResult::Unimplemented; 
	}
	...
	enum class EOpenWriteFlags : uint8
	{
		/** No additional flags. */
		None		= 0,
		/** Allow the returned handle to be read from. On certain platforms allow other handles/processes to read from this file as well. */
		AllowRead	= 1 << 0,
		/** Append to file. */ 
		Append		= 1 << 1,
	};
	 ...
	 * NOTE: Physical IPlatformFile Implementations should override exactly one overload of OpenWrite, preferrably the version with flags.
	 * This overload will be removed in a future revision.
	 * This default implementation calls the Flags overload.
	 * 
	 * @param Filename		The file to be opened
	 * @param bAppend		Append to file.
	 * @param bAllowRead	Allow the returned handle to be read from. On certain platforms allow other handles/processes to read from this file as well.
	 * 
	 * @return	If successful will return a non-nullptr pointer. Close the file by deleting the handle. 
	 **/
	CORE_API virtual IFileHandle* OpenWrite(const TCHAR* Filename, bool bAppend = false, bool bAllowRead = false);
	/**
	 * Open a file handle for writing.
	 * NOTE: Physical IPlatformFile Implementations should override exactly one overload of OpenWrite, preferrably this one.
	 * This default implementation calls the Bools overload.
	 * 
	 * @param Filename	The file to be opened
	...
	{
	public:
		virtual ~FDirectoryStatVisitor() = default;
	...
	*/
	...
	/** Flags to be used when opening a file for via IPlatformFile::OpenMappedEx */
	enum class EOpenMappedFlags : uint8
	{
		None			= 0,
		/** Allow the handle to be written to via the memory map. */
		OpenForWrite	= 1 << 0,
		/** If the file already exists, do not replace it. Only applies if OpenForWrite is set. */
		OpenExisting	= 1 << 1,
		/** Allow other handles/processes to write to this file. 
		  * On certain platforms this flag is needed to open files that are currently being written to as well. 
		  * Only applies if OpenForWrite is not set.
		  */
		AllowWrite		= 1 << 2,
		/** On certain platforms allow other handles/processes to read from this file as well.
		  * Only applies if OpenForWrite is set.
		  */
		AllowRead		= 1 << 3,
		/** Allow the file to be deleted or renamed while keeping this handle valid for reading */
		AllowDelete		= 1 << 4,
	...
	* @param OpenOptions	Allows specifying map to be writable via AllowWrite or be ReadOnly by default.
	...
	* The exact file open behavior may vary per-platform when using AllowWrite.
	* Prefer OpenMappedEx2 which has more consistent behavior across all supported platforms. 
	* OpenMappedEx is not currently deprecated so as not to change any platform specific behavior that may be implicitly relied upon.
	*/
	...
	*
	* @param Filename		File to be mapped. This doesn't actually map anything, just opens the file.
	* @param OpenOptions	Allows specifying map to be writable via OpenForWrite or be ReadOnly by default.
	* @return Close the file by deleting the handle. A non-null return value does mean the file exists.
	* Null can be returned for many reasons even if the file exists. Perhaps this platform does not support mapped files, or this file is compressed in a pak file.
	* Generally you attempt to open mapped, and if that fails, then use other file operations instead.
	* Prefer OpenMappedEx2 which has consistent behavior across all supported platforms.
	*/
	CORE_API virtual FOpenMappedResult OpenMappedEx2(const TCHAR* Filename, EOpenMappedFlags OpenOptions = EOpenMappedFlags::None, int64 MaximumSize = 0);
	...
	**/
	...
	{
	public:
		virtual ~IFileServerMessageHandler() = default;
...
ENUM_CLASS_FLAGS(IPlatformFile::EOpenMappedFlags);
