...
struct FGenericPlatformString : public FGenericPlatformStricmp
{
	 ...
	 *
	 * Writes the formatted output to Dest, up to a maximum of DestSize-1 characters, excluding the null terminator.
	 * Content of Dest is undefined when the return value is outside of the range [0, DestSize).
	 * A return value is calculated even when DestSize is 0, in which case Dest may be null.
	 * A null terminator is always written unless DestSize is 0.
	 * Success is indicated by ReturnValue >= 0 && ReturnValue < DestSize.
	 *
	 * %hs is always treated as UTF-8.
	 * %ls is always treated as UTF-16.
	 * %s is treated as UTF-8 for narrow format strings and UTF-16 for wide format strings.
	 *
	 * @param Dest   Pointer to the destination buffer to write to.
	 * @param DestSize   Maximum number of characters to write to Dest, including the null terminator.
	 * @param Format   Format string with which to format the output.
	 * @param Args   Arguments containing the data referenced by the format string.
	 * @return Number of characters, excluding the null terminator, that would be
	 *         written to Dest if DestSize was ignored, or -1 on error.
	 */
	[[nodiscard]] CORE_API static int32 FormatV(ANSICHAR* Dest, SIZE_T DestSize, const ANSICHAR* Format, va_list Args);
	[[nodiscard]] CORE_API static int32 FormatV(WIDECHAR* Dest, SIZE_T DestSize, const WIDECHAR* Format, va_list Args);
	[[nodiscard]] UE_REWRITE static int32 FormatV(UTF8CHAR* Dest, SIZE_T DestSize, const ANSICHAR* Format, va_list Args)
	{
		return FormatV((ANSICHAR*)Dest, DestSize, Format, Args);
	}
	[[nodiscard]] UE_REWRITE static int32 FormatV(UTF8CHAR* Dest, SIZE_T DestSize, const UTF8CHAR* Format, va_list Args)
	{
		return FormatV((ANSICHAR*)Dest, DestSize, (const ANSICHAR*)Format, Args);
	}

	UE_REWRITE static int32 GetVarArgs(ANSICHAR* Dest, SIZE_T DestSize, const ANSICHAR*& Fmt, va_list ArgPtr)
	{
		int32 Result = FormatV(Dest, DestSize, Fmt, ArgPtr);
		return (Result >= 0 && Result < (int32)DestSize) ? Result : -1;
	}

	UE_REWRITE static int32 GetVarArgs(WIDECHAR* Dest, SIZE_T DestSize, const WIDECHAR*& Fmt, va_list ArgPtr)
	{
		int32 Result = FormatV(Dest, DestSize, Fmt, ArgPtr);
		return (Result >= 0 && Result < (int32)DestSize) ? Result : -1;
	}

	UE_REWRITE static int32 GetVarArgs(UTF8CHAR* Dest, SIZE_T DestSize, const UTF8CHAR*& Fmt, va_list ArgPtr)
	{
		int32 Result = FormatV((ANSICHAR*)Dest, DestSize, (const ANSICHAR*)Fmt, ArgPtr);
		return (Result >= 0 && Result < (int32)DestSize) ? Result : -1;
	}
...
protected:
#if DO_CHECK
	static CORE_API void ReportZeroStrncpySize();
#endif
};
