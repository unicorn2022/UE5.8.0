 ...
 * These values generally don't change over the life of the executable, except for memory hotplugging and the addition or removal of swap space.
 */
...
struct FGenericPlatformMemory
{
	...
	enum EMemoryAllocatorToUse
	{
		...
		Binned4, // New experimental binned allocator
	...
	*/
	...
	{
#if UE_PLATFORM_REQUIRES_NONNULL_MEMOP_ARGS
		// On this platform `memmove` is required to not have `nullptr` as `Dest` or `Src`.
		// 
		// This will eventually go away when `n3322` is adopted, but for now we workaround
		// this by checking for `Count == 0` and bailing early (the only defined behavior
		// way going forward).
		if (UNLIKELY(Count == 0))
		{
			return Dest;
	...
	{
#if UE_PLATFORM_REQUIRES_NONNULL_MEMOP_ARGS
		// On this platform `memcmp` is required to not have `nullptr` as `Dest` or `Src`.
		// 
		// This will eventually go away when `n3322` is adopted, but for now we workaround
		// this by checking for `Count == 0` and bailing early (the only defined behavior
		// way going forward).
	...
	{
#if UE_PLATFORM_REQUIRES_NONNULL_MEMOP_ARGS
		// On this platform `memset` is required to not have `nullptr` as `Dest`.
		//
		// This will eventually go away when `n3322` is adopted, but for now we workaround
		// this by checking for `Count == 0` and bailing early (the only defined behavior
		// way going forward).
	...
	{
#if UE_PLATFORM_REQUIRES_NONNULL_MEMOP_ARGS
		// On this platform `memcpy` is required to not have `nullptr` as `Dest` or `Src`.
		// 
		// This will eventually go away when `n3322` is adopted, but for now we workaround
		// this by checking for `Count == 0` and bailing early (the only defined behavior
		// way going forward).
...
public:
	...
	*/
	 ...
	 */
	UE_DEPRECATED(5.8, "This workaround is now obsolete")
