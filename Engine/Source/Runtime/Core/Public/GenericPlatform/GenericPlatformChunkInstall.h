...
public:
	...
	/** Virtual destructor */
	virtual ~IPlatformChunkInstall() = default;
	...
	/**
	 * Cancel an in-progress install of the given named chunk
	 * @param NamedChunk	The name of the chunk
	 * @return				whether the cancellation task has been kicked
	 **/
	virtual bool CancelNamedChunkInstall(const FName NamedChunk) = 0;

	/**
	 * Cancel an in-progress install of the given set of named chunks
	 * @param NamedChunks	The names of the chunks to cancel
	 * @return				whether the cancellation task has been kicked
	 **/
	virtual bool CancelNamedChunksInstall(const TArrayView<const FName>& NamedChunks) = 0;
...
public:
	...
	virtual bool CancelNamedChunkInstall(const FName NamedChunk) override
	{
		return CancelNamedChunksInstall(MakeArrayView(&NamedChunk, 1));
	}

	virtual bool CancelNamedChunksInstall(const TArrayView<const FName>& NamedChunks) override
	{
		return false;
	}
