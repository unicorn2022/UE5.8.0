...
struct alignas(uint32) FBlake3Hash
{
public:
	...
	friend CORE_API FArchive& operator<<(FArchive& Ar, FBlake3Hash& Value);
...
/** Construct a hash from a 64-character hex string. */
template <typename CharType>
inline bool TryLexFromString(FBlake3Hash& OutHash, TStringView<CharType> String)
{
	if (String.Len() == sizeof(FBlake3Hash) * 2)
	{
		OutHash = FBlake3Hash(String);
		return true;
	}
	return false;
}
