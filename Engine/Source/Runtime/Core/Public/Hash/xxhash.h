...
struct FXxHash64
{
	...
	[[nodiscard]] static inline FXxHash64 FromByteArray(const uint8(&Bytes)[sizeof(uint64)])
	{
	...
	static constexpr int32 GetStringLen() { return sizeof(uint64) * 2; }

	/** Convert the hash to a 16-character hex string. */
	[[nodiscard]] CORE_API FString ToString() const;

	/** Append the 16-character hex representation to the given string. */
	CORE_API void AppendString(FString& Out) const;

	/** Parse the hash from a 16-character hex string. */
	CORE_API void FromString(FStringView Src);
	...
	CORE_API friend FString LexToString(const FXxHash64& InHash); 
	CORE_API friend void LexFromString(FXxHash64& InHash, const TCHAR* InString);

	friend FCbWriter& operator<<(FCbWriter& Writer, const FXxHash64& InHash) { return InHash.WriteCompactBinary(Writer); }
	friend bool LoadFromCompactBinary(const FCbFieldView& Field, FXxHash64& OutHash) { return OutHash.LoadFromCompactBinary(Field); }

	CORE_API bool LoadFromCompactBinary(const FCbFieldView& Field);
	CORE_API FCbWriter& WriteCompactBinary(FCbWriter& Writer) const;
};

DECLARE_INTRINSIC_TYPE_LAYOUT(FXxHash64);
...
struct FXxHash128
{
	...
	/** Load the hash from its canonical (big-endian) representation. */
	[[nodiscard]] static inline FXxHash128 FromByteArray(const uint8 (&Bytes)[sizeof(uint64[2])])
	{
