...
{
	const FormatCharType* Format = nullptr;
	...
	TStaticBasicLogRecord* operator&() const = delete;
#endif
};

// Pass TStaticBasicLogRecord by value when the record is not static, because in that
// configuration it is simply a wrapper for the format string pointer.
#if UE_PRIVATE_LOG_USE_STATIC_RECORD
template <UE::CCharType FormatCharType>
using TStaticBasicLogRecordParam = const TStaticBasicLogRecord<FormatCharType>&;
#else
template <UE::CCharType FormatCharType>
using TStaticBasicLogRecordParam = TStaticBasicLogRecord<FormatCharType>;
#endif

template <ELogVerbosity::Type Verbosity, UE::CCharType FormatCharType>
CORE_API void BasicLog(TStaticBasicLogRecordParam<FormatCharType> Log, const FLogCategoryBase* Category, ...);
template <UE::CCharType FormatCharType>
CORE_API void BasicFatalLog(TStaticBasicLogRecordParam<FormatCharType> Log, const FLogCategoryBase* Category, ...);
template <UE::CCharType FormatCharType>
CORE_API void BasicFatalLogWithProgramCounter(TStaticBasicLogRecordParam<FormatCharType> Log, const FLogCategoryBase* Category, void* ProgramCounter, ...);
	...
	**/
	 ...
	 * Prefer UE_LOGF because its ASCII or UTF-8 format strings require less memory. 
	 *
	 ...
	 * @param Format         Format string literal in the style of printf. Specify as TEXT("...")
	 */
	#define UE_LOG(CategoryName, Verbosity, Format, ...) \
	{ \
	 ...
	 * @param CategoryName   Name of the log category as provided to DEFINE_LOG_CATEGORY.
	 * @param Verbosity      Verbosity level of this message. See ELogVerbosity.
	 * @param Format         Format string literal in the style of printf. Specify as "..." or UTF8TEXT("...")
	 */
	#define UE_LOGF(CategoryName, Verbosity, Format, ...) \
	{ \
		static_assert(::TIsArrayOrRefOfTypeByPredicate<decltype(Format), ::TIsCharEncodingCompatibleWithUTF8CHAR>::Value, "Formatting string must be a UTF8CHAR or ANSICHAR array."); \
		UE_PRIVATE_LOG(UE_EMPTY, constexpr, CategoryName, Verbosity, Format, ##__VA_ARGS__) \
	}
	...
	{ \
		static_assert(::TIsArrayOrRefOfTypeByPredicate<decltype(Format), ::TIsCharEncodingCompatibleWithUTF8CHAR>::Value, "Formatting string must be a UTF8CHAR or ANSICHAR array."); \
		UE_PRIVATE_LOG(if (Condition), constexpr, CategoryName, Verbosity, UTF8TEXT(Format), ##__VA_ARGS__) \
	}
	...
	{ \
		...
		UE_IF(UE_PRIVATE_LOG_USE_STATIC_RECORD, static,) constexpr ::UE::Logging::Private::TStaticBasicLogRecord<TElementType_T<decltype(Format)>> LOG_Static(Format, __builtin_FILE(), __builtin_LINE(), UE_IF(LOGTRACE_ENABLED, &LOG_Dynamic, nullptr)); \
