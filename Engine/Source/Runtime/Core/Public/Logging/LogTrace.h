...
#define TRACE_LOG_MESSAGE(Category, Verbosity, Format, ...) \
	...
	{ \
		static bool UE_JOIN(__LogPoint, __LINE__); \
		if (!UE_JOIN(__LogPoint, __LINE__)) \
		{ \
			FLogTrace::OutputLogMessageSpec(&UE_JOIN(__LogPoint, __LINE__), &Category, ELogVerbosity::Verbosity, __FILE__, __LINE__, TEXT("%s")); \
			UE_JOIN(__LogPoint, __LINE__) = true; \
		} \
		FLogTrace::OutputLogMessageSimple(&UE_JOIN(__LogPoint, __LINE__), Format, ##__VA_ARGS__); \
	}
#else
#define TRACE_LOG_MESSAGE(Category, Verbosity, Format, ...) \
	...
	{ \
		...
		if (!UE_JOIN(__LogPoint, __LINE__)) \
		{ \
			FLogTrace::OutputLogMessageSpec(&UE_JOIN(__LogPoint, __LINE__), &Category, ELogVerbosity::Verbosity, __FILE__, __LINE__, (const TCHAR*)Format); \
			UE_JOIN(__LogPoint, __LINE__) = true; \
		} \
		FLogTrace::OutputLogMessage(&UE_JOIN(__LogPoint, __LINE__), ##__VA_ARGS__); \
	}
#endif
