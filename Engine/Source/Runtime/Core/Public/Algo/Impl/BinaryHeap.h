...
// Workaround for a codegen bug that was discovered related to member function pointer projections that are virtual
	...
	#define UE_VIRTUAL_PROJECTION_WORKAROUND 1
#else
	#define UE_VIRTUAL_PROJECTION_WORKAROUND 0
#endif
...
namespace AlgoImpl
{
	 ...
	 *
	 * @param	Heap		Random access iterator to the first element of a binary heap.
	 * @param	Index		Node index.
	...
	inline void HeapSiftDown(IteratorType Heap, IndexType Index, const IndexType Count, const ProjectionType& InProj, const PredicateType& Predicate)
	{
	 ...
	 */
	template <class IteratorType, typename IndexType, typename ProjectionType, class PredicateType>
	inline IndexType HeapSiftUp(IteratorType Heap, IndexType RootIndex, IndexType NodeIndex, const ProjectionType& InProj, const PredicateType& Predicate)
	{
#if UE_VIRTUAL_PROJECTION_WORKAROUND
		const ProjectionType& Proj = InProj;
#else
	 ...
	 *
	 * @param	First		Random access iterator to the first element to heapify.
	 * @param	Num			The number of items to heapify.
	 * @param	Proj		The projection to apply to the elements.
	 * @param	Predicate	A binary predicate object used to specify if one element should precede another.
	 */
	template <typename IteratorType, typename IndexType, typename ProjectionType, typename PredicateType>
	inline void HeapifyInternal(IteratorType First, IndexType Num, ProjectionType Proj, PredicateType Predicate)
	{
	 ...
	 * @param	Predicate	A binary predicate object used to specify if one element should precede another.
	 */
	template <typename IteratorType, typename IndexType, typename ProjectionType, class PredicateType>
	void HeapSortInternal(IteratorType First, IndexType Num, ProjectionType Proj, PredicateType Predicate)
	{
		if constexpr (std::is_signed_v<IndexType>)
		{
