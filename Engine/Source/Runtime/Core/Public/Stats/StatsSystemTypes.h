...
struct TStatId
{
	// TStatId can be returned from AutoRTFM::Open() via a copy-construction in the closed.
...
class FEndOfPipeStats : private FThreadStats
{
	...
	// Serializes producers writing to the singleton's inherited Packet.
	// Multiple workers (RHI submission thread + task-graph EOP tasks) call
	// AddMessage on this instance; without this lock the chunked array writes
	// race and produce torn FStatMessages that fail the DummyAlwaysOne check
	// in FStatsThreadState::ProcessNonFrameStats.
	FTransactionallySafeCriticalSection PacketCriticalSection;
...
public:
	...
	void AddMessage(FName InStatName, EStatOperation::Type InStatOperation, TValue Value, bool bIsCycle = false)
	{
		if (!InStatName.IsNone() && WillEverCollectData() && IsThreadingReady())
		{
			UE::TScopeLock Lock(PacketCriticalSection);
...
public:
	...
	inline void Start(TStatId InStatId, EStatFlags InStatFlags, bool bAlways = false, UE::FSourceLocation SourceLocation = UE::FSourceLocation::Current())
	{
		...
		if ((bAlways && FThreadStats::WillEverCollectData()) || FThreadStats::IsCollectingData())
		{
			...
			EmittedEvent |= ThreadStatsEventFlag;
		}
	}
	...
	inline void Stop()
	{
		if (EmittedEvent & NamedEventFlag)
		{
			FPlatformMisc::EndNamedEvent();
		}

#if CPUPROFILERTRACE_ENABLED
		if (EmittedEvent & TraceEventFlag)
		{
			FCpuProfilerTrace::OutputEndEvent();
		}
#endif

		if (EmittedEvent & ThreadStatsEventFlag)
		{
...
public:
	...
	/** virtual destructor. **/
	virtual ~IStatGroupEnableManager() = default;
...
struct AUTORTFM_OPEN FThreadSafeStaticStatInner : public FThreadSafeStaticStatBase
{
