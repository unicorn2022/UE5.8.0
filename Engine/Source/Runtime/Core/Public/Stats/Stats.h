...
#define SCOPE_CYCLE_COUNTER_STATID(StatId) \
	FScopeCycleCounter UE_JOIN(CycleCount_FromStatId_, __LINE__)(StatId);
...
#define SCOPE_CYCLE_COUNTER_TO_TRACE(StatString, StatName, Condition) \
	TRACE_CPUPROFILER_EVENT_DECLARE(UE_JOIN(__Decl_, StatName, __LINE__)); \
	TRACE_CPUPROFILER_EVENT_SCOPE_USE(UE_JOIN(__Decl_, StatName, __LINE__), StatString, UE_JOIN(__Scope_, StatName, __LINE__), Condition && GCycleStatsShouldEmitNamedEvents);
...
#define SCOPE_CYCLE_COUNTER_STATID(StatId) \
	UE::Stats::FHitchTrackingStatScope UE_JOIN(HitchTrackingStatScope_FromStatId_, __LINE__)(ANSI_TO_PROFILING("Lightweight StatId Scope"));
