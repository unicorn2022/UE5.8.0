...
public:
	...
	/** Comparison operator for FBoxCenterAndExtent */
	[[nodiscard]] bool operator==(const FBoxCenterAndExtent& Other) const
	{
	...
	[[nodiscard]] friend inline uint32 GetTypeHash(const FBoxCenterAndExtent& Box)
	{
		return HashCombine(GetTypeHash(Box.Center), GetTypeHash(Box.Extent));
	}
};
...
public:
	...
	*/
	...
	void DumpStats() const
	{
		...
		UE_LOGF(LogGenericOctree, Log, "Octree overview:");
		UE_LOGF(LogGenericOctree, Log, "\t%i nodes", NumNodes);
		UE_LOGF(LogGenericOctree, Log, "\t%i leaves", NumLeaves);
		UE_LOGF(LogGenericOctree, Log, "\t%i elements", NumElements);
		UE_LOGF(LogGenericOctree, Log, "\t%i >= elements per node", MaxElementsPerNode);
		UE_LOGF(LogGenericOctree, Log, "Octree node element distribution:");
		for (int32 i = 0; i < NodeElementDistribution.Num(); i++)
		{
			if (NodeElementDistribution[i] > 0)
			{
				UE_LOGF(LogGenericOctree, Log, "\tElements: %3i, Nodes: %3i", i, NodeElementDistribution[i]);
			}
		}
	}
...
private:
	...
	)
	{
		...
		UE_LOGF(LogGenericOctree, Fatal,
