...
public:
	...
	/** Performs a safe division, which returns the largest possible value when divisor is nearly zero */
	template <class T, class U> 
	[[nodiscard]] static constexpr auto SafeDivide(T Dividend, U Divisor) -> decltype(Dividend / Divisor)
	{
		return FMath::IsNearlyZero(Divisor) ? TNumericLimits<decltype(Dividend / Divisor)>::Max() : (Dividend / Divisor);
	};
...
#undef FASTASIN_HALF_PI
	...
	>
	[[nodiscard]] static constexpr auto FindDeltaAngleDegrees(T A1, T2 A2) -> decltype(A2 - A1)
	{
		// Find the difference
		auto Delta = A2 - A1;
	...
	template <
		...
		UE_REQUIRES(std::is_floating_point_v<T> || std::is_floating_point_v<T2>)
	>
	[[nodiscard]] static constexpr auto FindDeltaAngleRadians(T A1, T2 A2) -> decltype(A2 - A1)
	{
		// Find the difference
		auto Delta = A2 - A1;
	 ...
	 *
	 * @Note: 'Azimuth' (.X) sign is changed to represent left/right and not front/behind. front/behind is the function's return value.
	 *
	...
	*/
		...
		const float InSmoothingTime)
	{
		if (InSmoothingTime > UE_KINDA_SMALL_NUMBER)
		{
			...
			//
			// The target velocity turns into an offset to the position
