...
#include <atomic>
 ...
 #if WITH_EDITOR && ( PLATFORM_WINDOWS || PLATFORM_UNIX ) && !UE_BUILD_SHIPPING && COUNTERSTRACE_ENABLED && !USING_THREAD_SANITISER && !USING_INSTRUMENTATION
...
#define SCOPE_STALL_COUNTER(InName, InBudgetSeconds)                                                                                                                                 \
	static_assert(UE_ARRAY_COUNT(UE_STRINGIZE(InName)) <= UE::StallDetectorScopeMaxNameLength, "Scope name must be less than UE::StallDetectorScopeMaxNameLength characters long "); \
	static UE::FStallDetectorStats UE_JOIN(StallDetectorStats, __LINE__)(TEXT(#InName), InBudgetSeconds, UE::EStallDetectorReportingMode::Disabled);                                 \
	UE::FStallDetector UE_JOIN(ScopeStallDetector, __LINE__)(UE_JOIN(StallDetectorStats, __LINE__));
...
#define SCOPE_STALL_REPORTER(InName, InBudgetSeconds)                                                                                                                                \
	static_assert(UE_ARRAY_COUNT(UE_STRINGIZE(InName)) <= UE::StallDetectorScopeMaxNameLength, "Scope name must be less than UE::StallDetectorScopeMaxNameLength characters long "); \
	static UE::FStallDetectorStats UE_JOIN(StallDetectorStats, __LINE__)(TEXT(#InName), InBudgetSeconds, UE::EStallDetectorReportingMode::First);                                    \
	UE::FStallDetector UE_JOIN(ScopeStallDetector, __LINE__)(UE_JOIN(StallDetectorStats, __LINE__));
...
#define SCOPE_STALL_REPORTER_ALWAYS(InName, InBudgetSeconds)                                                                                                                         \
	static_assert(UE_ARRAY_COUNT(UE_STRINGIZE(InName)) <= UE::StallDetectorScopeMaxNameLength, "Scope name must be less than UE::StallDetectorScopeMaxNameLength characters long "); \
	static UE::FStallDetectorStats UE_JOIN(StallDetectorStats, __LINE__)(TEXT(#InName), InBudgetSeconds, UE::EStallDetectorReportingMode::Always);                                   \
	UE::FStallDetector UE_JOIN(ScopeStallDetector, __LINE__)(UE_JOIN(StallDetectorStats, __LINE__));
...
#define SCOPE_STALL_DETECTOR_PAUSE() \
	UE::FStallDetectorPause UE_JOIN(ScopeStallDetectorPause, __LINE__);
