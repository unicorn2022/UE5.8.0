...
{ \
	static bool UE_JOIN(__BookmarkPoint, __LINE__); \
	if (!UE_JOIN(__BookmarkPoint, __LINE__)) \
	{ \
		...
		FMiscTrace::OutputBookmarkSpec(&UE_JOIN(__BookmarkPoint, __LINE__), __FILE__, __LINE__, Format); \
		UE_JOIN(__BookmarkPoint, __LINE__) = true; \
	} \
	FMiscTrace::OutputBookmark(CallstackTrace_GetCurrentId(), &UE_JOIN(__BookmarkPoint, __LINE__), ##__VA_ARGS__); \
}
...
{ \
	...
	FMiscTrace::OutputBookmarkCycles(CallstackTrace_GetCurrentId(), Cycles, &UE_JOIN(__BookmarkPoint, __LINE__), ##__VA_ARGS__); \
}
