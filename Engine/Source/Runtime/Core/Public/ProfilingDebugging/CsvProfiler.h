...
struct FCsvDeclaredStat;
	...
	#define CSV_EVENT(Category, Format, ...) \
		...
		TRACE_BOOKMARK(TEXT(UE_STRINGIZE(Category)) TEXT("/") Format, ##__VA_ARGS__)
	...
	#define CSV_EVENT_NOLOG(Category, Format, ...) \
		FCsvProfiler::RecordEventfNoLog( CSV_CATEGORY_INDEX(Category), Format, ##__VA_ARGS__ ); \
		TRACE_BOOKMARK(TEXT(UE_STRINGIZE(Category)) TEXT("/") Format, ##__VA_ARGS__)

	#define CSV_EVENT_GLOBAL_NOLOG(Format, ...) \
		FCsvProfiler::RecordEventfNoLog( CSV_CATEGORY_INDEX_GLOBAL, Format, ##__VA_ARGS__ ); \
		TRACE_BOOKMARK(Format, ##__VA_ARGS__)

	#define CSV_REGION_BEGIN(Category, RegionName) \
		FCsvProfiler::BeginRegion(CSV_CATEGORY_INDEX(Category), RegionName);

	#define CSV_REGION_END(Category, RegionName) \
		FCsvProfiler::EndRegion(CSV_CATEGORY_INDEX(Category), RegionName);

	#define CSV_REGION_BEGIN_GLOBAL(RegionName) \
		FCsvProfiler::BeginRegion(CSV_CATEGORY_INDEX_GLOBAL, RegionName);

	#define CSV_REGION_END_GLOBAL(RegionName) \
		FCsvProfiler::EndRegion(CSV_CATEGORY_INDEX_GLOBAL, RegionName);
...
// Shared macros. These emit Trace and CSV instrumentation. Can be used in place of TRACE_BEGIN_REGION and TRACE_END_REGION
// NOTE: CSV_TRACE_REGION_END requires a category name, unlike TRACE_END_REGION. This is in order to avoid potential naming clashes
#if CSV_PROFILER
	#define CSV_TRACE_REGION_BEGIN(RegionName, ...) \
		TRACE_BEGIN_REGION(RegionName __VA_OPT__(,) __VA_ARGS__) \
		FCsvProfiler::BeginRegion(CSV_CATEGORY_INDEX_GLOBAL, RegionName __VA_OPT__(,) __VA_ARGS__);

	#define CSV_TRACE_REGION_END(RegionName, ...) \
		FCsvProfiler::EndRegion(CSV_CATEGORY_INDEX_GLOBAL, RegionName __VA_OPT__(,) __VA_ARGS__); \
...
struct FCsvProcessThreadDataStats
{
	...
	uint32 CustomStatCount = 0;
	uint32 EventCount = 0;
	uint64 MemoryUseBytes = 0;
};
...
public:
	...
	CORE_API static void PreInit(const TCHAR* Commandline);
	...
	CORE_API static void RecordEvent(int32 CategoryIndex, const FString& EventText, bool bLog=true, bool bAllowDelegate=true);
	CORE_API static void RecordEventAtFrameStart(int32 CategoryIndex, const FString& EventText, bool bLog = true);
	CORE_API static void RecordEventAtTimestamp(int32 CategoryIndex, const FString& EventText, uint64 Cycles64, bool bLog = true);
	...
	inline static void RecordEventf(int32 CategoryIndex, const FmtType& Fmt, Types... Args)
	{
		UE_AUTORTFM_OPEN
		{
			...
			RecordEventfInternal(CategoryIndex, true, (const TCHAR*)Fmt, Args...);
		};
	}

	template <typename FmtType, typename... Types>
	inline static void RecordEventfNoLog(int32 CategoryIndex, const FmtType& Fmt, Types... Args)
	{
		UE_AUTORTFM_OPEN
		{
			static_assert(TIsArrayOrRefOfTypeByPredicate<FmtType, TIsCharEncodingCompatibleWithTCHAR>::Value, "Formatting string must be a TCHAR array.");
			static_assert((TIsValidVariadicFunctionArg<Types>::Value && ...), "Invalid argument(s) passed to FCsvProfiler::RecordEventfNoLog");
			RecordEventfInternal(CategoryIndex, false, (const TCHAR*)Fmt, Args...);
		};
	}
	...
	CORE_API static void BeginRegion(int32 CategoryIndex, const TCHAR* RegionName, const TCHAR* SubCategoryName = nullptr);
	CORE_API static void EndRegion(int32 CategoryIndex, const TCHAR* RegionName, const TCHAR* SubCategoryName = nullptr);

	CORE_API static FString GetDefaultDirectory();

	CORE_API static bool IsCapturing();
	CORE_API static bool IsCapturing_Renderthread();
	...
	CORE_API uint64	GetNumBytesWritten() const;
	 ...
	 */
	CORE_API TSharedFuture<FString> EndCapture(FGraphEventRef EventToSignal = {});
	...
	/** Called from the engine tick at the point where we should compute the current frame time. */
	CORE_API void UpdateFrameTime();
	...
	/** Called when an event is emitted. 
		NOTE: this will only fire if CSV_PROFILER_ALLOW_DEBUG_FEATURES is enabled */
...
private:
	CORE_API static void VARARGS RecordEventfInternal(int32 CategoryIndex, bool bLog, const TCHAR* Fmt, ...);

	void InitInternal();
	
	static void RecordRegionEventInternal(const TCHAR* CategoryName, const TCHAR* RegionName, const TCHAR* SubCategoryName, bool bBegin);
	...
	float ProcessStatData(FCsvProcessThreadDataStats* OutNumStatEntriesProcessed = nullptr);
