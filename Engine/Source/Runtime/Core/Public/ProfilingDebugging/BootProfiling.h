...
struct FBootProfiling
{
	...
	/** Accumulate time spent in blocking dialogs (e.g. driver denylist) during boot. */
	static CORE_API void AddBlockingDialogDuration(double Seconds);

	/** Returns total time spent in blocking dialogs during boot. */
	static CORE_API double GetBlockingDialogDuration();
