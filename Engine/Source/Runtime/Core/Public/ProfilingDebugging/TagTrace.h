...
////////////////////////////////////////////////////////////////////////////////
#define UE_MEMSCOPE(InTag)					FMemScope UE_JOIN(MemScope,__LINE__)(InTag);
// UE_MEMSCOPE_PTR(InPtr) or UE_MEMSCOPE_PTR(InPtr, InRootHeap)
#define UE_MEMSCOPE_PTR(InPtr, ...)			FMemScopePtr UE_JOIN(MemPtrScope,__LINE__)((uint64)InPtr, ##__VA_ARGS__);
#define UE_MEMSCOPE_DEFAULT(InTag)			FDefaultMemScope UE_JOIN(MemScope,__LINE__)(InTag);
#define UE_MEMSCOPE_UNINITIALIZED(Line)		FMemScope UE_JOIN(MemScope,Line);
#define UE_MEMSCOPE_ACTIVATE(Line, InTag)	UE_JOIN(MemScope,Line).SetTagAndActivate(InTag);
