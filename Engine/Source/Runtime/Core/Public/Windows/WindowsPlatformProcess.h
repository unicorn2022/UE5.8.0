...
public:
	...
	/** Test-only: overwrite GBaseDirectory so we can force paths that would normally fail. Not thread safe. */
	static CORE_API void SetBaseDirForTesting(const TCHAR* InBaseDir);
#endif
