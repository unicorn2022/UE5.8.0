...
enum class ETDRLevel
{
	/** TDR was disabled explicitly */
	Disabled = 0,

	/** Don't attempt to recover from a GPU or driver timeout and display a bugcheck screen instead */
	BugCheck,

	/** Not implemented */
	RecoverVGA,

	/** Recover adapter/driver on timeout (Default) */
	Recover,

	Invalid
};

CORE_API const TCHAR* LexToString(ETDRLevel TDRLevel);


/**
 *	Information about Windows' Timeout Detection and Recovery (TDR) mechanism for detecting display adapter hangs
 */
struct FTDRInfo
{
	static constexpr uint32 DefaultDelay = 2;
	static constexpr uint32 DefaultDdiDelay = 5;

	ETDRLevel Level;
	uint32 Delay;		// GPU timeout threshold in seconds after which TDR will kick in
	uint32 DdiDelay;	// Graphics driver timeout threshold in seconds after which TDR will kick in
};
	...
	: public FGenericPlatformMisc
{
	...
	static CORE_API struct FTDRInfo GetTDRInfo();
	...
	*/
	...
	/**
	 * Returns the current network connection type using NLM (Network List Manager).
	 */
	static CORE_API ENetworkConnectionType GetNetworkConnectionType();

	/**
	 * Starts event-driven network connection monitoring via INetworkListManagerEvents.
	 * Called from PlatformInit(). Updates network connection status on connectivity changes.
	 */
	static CORE_API void StartNetworkConnectionMonitoring();

	/**
	 * Stops network connection monitoring and releases NLM COM resources.
	 * Called from PlatformTearDown().
	 */
	static CORE_API void StopNetworkConnectionMonitoring();
	 ...
	 * NOTE: This call will block (if GCollectStorageInformationAsync is enabled) until stats are available
	 */
	...
	static CORE_API void ApplyWindowsCompatConfigRules(void);
