...
private:
	...
	struct BufCore
	{
		...
		{
			return uint32(NumRefs.fetch_add(1, std::memory_order_seq_cst) + 1);
		}

		inline uint32 Release() const
		{
			const int32 Refs = NumRefs.fetch_sub(1, std::memory_order_seq_cst);
#if DO_CHECK
			check(Refs > 0);
#endif
			if (Refs == 1)
		...
		{
			return uint32(NumRefs.load(std::memory_order_relaxed));
		}
