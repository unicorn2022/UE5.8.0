...
public:
	...
	[[nodiscard]] inline static FIoHash HashBuffer(FMemoryView View);
	[[nodiscard]] inline static FIoHash HashBuffer(const void* Data, uint64 Size);
	[[nodiscard]] CORE_API static FIoHash HashBuffer(const FCompositeBuffer& Buffer);
...
private:
	...
	friend CORE_API FArchive& operator<<(FArchive& Ar, FIoHash& InHash);
};
...
/** Construct a hash from a 40-character hex string. */
template <typename CharType>
inline bool TryLexFromString(FIoHash& OutHash, TStringView<CharType> String)
{
	if (String.Len() == sizeof(FIoHash) * 2)
	{
		OutHash = FIoHash(String);
		return true;
	}
	return false;
}
