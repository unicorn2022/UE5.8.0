...
{
	// Retaining this version with no branching until both have been profiled on ARM.
	// This is equivalent to Min(FloorLog2(N) / 7 + 1, 9)
	//return FPlatformMath::Min(((uint32(FPlatformMath::FloorLog2NonZero(InValue | 1)) * 37) >> 8) + 1, 9u);

	// This branching implementation is faster than branchless alternatives in real-world benchmarks.
	return (InValue < (uint32(1) << 28))
		? ((InValue < (uint32(1) << 14))
			? ((InValue < (uint32(1) <<  7)) ? 1 : 2)
			: ((InValue < (uint32(1) << 21)) ? 3 : 4))
		: 5;
}
...
FORCEINLINE uint32 MeasureVarUInt(uint64 InValue)
{
	// Retaining this version with no branching until both have been profiled on ARM.
	// This is equivalent to Min(FloorLog2(N) / 7 + 1, 9)
	//return FPlatformMath::Min(((uint32(FPlatformMath::FloorLog2NonZero_64(InValue | 1)) * 37) >> 8) + 1, 9u);

	// This branching implementation is faster than branchless alternatives in real-world benchmarks.
	return (InValue < (uint64(1) << 28))
		? ((InValue < (uint64(1) << 14))
			? ((InValue < (uint64(1) <<  7)) ? 1 : 2)
			: ((InValue < (uint64(1) << 21)) ? 3 : 4))
		: ((InValue < (uint64(1) << 42))
			? ((InValue < (uint64(1) << 35)) ? 5 : 6)
			: ((InValue < (uint64(1) << 49)) ? 7 : ((InValue < (uint64(1) << 56)) ? 8 : 9)));
}
