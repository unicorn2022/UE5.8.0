...
namespace UE::IDOHelpers
{
	...
	UE_INTERNAL extern CORE_API TFunction<bool(TNotNull<FProperty*>)> GIsUnknownPropertyFunc;
...
public:
	...
	void PushProperty(class FProperty* InProperty, const bool bIsEditorOnlyProperty)
	{
		...
		PRAGMA_DISABLE_INTERNAL_WARNINGS
		if (UE::IDOHelpers::GIsUnknownPropertyFunc.IsSet() && UE::IDOHelpers::GIsUnknownPropertyFunc(InProperty))
		{
			++UnknownPropertyStack;
		}
		PRAGMA_ENABLE_INTERNAL_WARNINGS
	...
	/**
	 * Check to see whether there are any properties on the stack is unknown
	 */
	bool HasUnknownProperty() const
	{
#if WITH_EDITORONLY_DATA
		PRAGMA_DISABLE_INTERNAL_WARNINGS
		checkf(UE::IDOHelpers::GIsUnknownPropertyFunc.IsSet(), TEXT("There was no function pointer provided to retrieve this information, so the result will not be correctly returned."))
		PRAGMA_ENABLE_INTERNAL_WARNINGS
		return UnknownPropertyStack > 0;
...
private:
	...
	/** Non-zero if on the current stack of serialized properties is a unknown property */
	int32 UnknownPropertyStack;
