...
struct FTypeLayoutDesc
{
	...
	uint8 IsIntrinsic : 1;
	UE_DEPRECATED(5.8, "IsInitialized is not used anymore")
...
static void InternalInitializeBaseHelper(FTypeLayoutDesc& TypeDesc)
{
	...
	FieldDesc.WriteFrozenMemoryImageFunc = TGetFreezeImageFieldHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::Do();
...
#define INTERNAL_LAYOUT_FIELD(T, InName, InOffset, InFlags, InNumArray, InBitFieldSize, Counter) \
	...
	UE_DECLARE_INTERNAL_LINK_SPECIALIZATION(InternalLinkType, Counter - CounterBase) { \
		...
		static void Initialize(FTypeLayoutDesc& TypeDesc) { \
			...
			FieldDesc.Type = &StaticGetTypeLayoutDesc<UE_REMOVE_OPTIONAL_PARENS(T)>(); \
			FieldDesc.WriteFrozenMemoryImageFunc = TGetFreezeImageFieldHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::Do(); \
...
#define INTERNAL_LAYOUT_FIELD_WITH_WRITER(T, InName, InFunc, Counter) \
	...
	UE_DECLARE_INTERNAL_LINK_SPECIALIZATION(InternalLinkType, Counter - CounterBase) { \
		...
		static void CallWriteFrozenField(FMemoryImageWriter& Writer, const void* Object, const void* FieldObject, const FTypeLayoutDesc& TypeDesc, const FTypeLayoutDesc& DerivedTypeDesc) { \
			static_cast<const DerivedType*>(Object)->InFunc(Writer, *(UE_REMOVE_OPTIONAL_PARENS(T)*)FieldObject); \
		} \
		static void Initialize(FTypeLayoutDesc& TypeDesc) { \
			InternalLinkType<Counter - CounterBase + 1>::Initialize(TypeDesc); \
			alignas(FFieldLayoutDesc) static uint8 FieldBuffer[sizeof(FFieldLayoutDesc)] = { 0 }; \
...
#define LAYOUT_FIELD(T, Name, ...) UE_REMOVE_OPTIONAL_PARENS(T) Name; INTERNAL_LAYOUT_FIELD(T, Name, STRUCT_OFFSET(DerivedType, Name), EFieldLayoutFlags::MakeFlags(__VA_ARGS__), 1u, 0u, __COUNTER__)
#define LAYOUT_MUTABLE_FIELD(T, Name, ...) mutable UE_REMOVE_OPTIONAL_PARENS(T) Name; INTERNAL_LAYOUT_FIELD(T, Name, STRUCT_OFFSET(DerivedType, Name), EFieldLayoutFlags::MakeFlags(__VA_ARGS__), 1u, 0u, __COUNTER__)
#define LAYOUT_FIELD_INITIALIZED(T, Name, Value, ...) UE_REMOVE_OPTIONAL_PARENS(T) Name = Value; INTERNAL_LAYOUT_FIELD(T, Name, STRUCT_OFFSET(DerivedType, Name), EFieldLayoutFlags::MakeFlags(__VA_ARGS__), 1u, 0u, __COUNTER__)
#define LAYOUT_MUTABLE_FIELD_INITIALIZED(T, Name, Value, ...) mutable UE_REMOVE_OPTIONAL_PARENS(T) Name = Value; INTERNAL_LAYOUT_FIELD(T, Name, STRUCT_OFFSET(DerivedType, Name), EFieldLayoutFlags::MakeFlags(__VA_ARGS__), 1u, 0u, __COUNTER__)
#define LAYOUT_ARRAY(T, Name, NumArray, ...) UE_REMOVE_OPTIONAL_PARENS(T) Name[NumArray]; INTERNAL_LAYOUT_FIELD(T, Name, STRUCT_OFFSET(DerivedType, Name), EFieldLayoutFlags::MakeFlags(__VA_ARGS__), NumArray, 0u, __COUNTER__)
#define LAYOUT_MUTABLE_BITFIELD(T, Name, BitFieldSize, ...) mutable UE_REMOVE_OPTIONAL_PARENS(T) Name : BitFieldSize; INTERNAL_LAYOUT_FIELD(T, Name, ~0u, EFieldLayoutFlags::MakeFlags(__VA_ARGS__), 1u, BitFieldSize, __COUNTER__)
#define LAYOUT_BITFIELD(T, Name, BitFieldSize, ...) UE_REMOVE_OPTIONAL_PARENS(T) Name : BitFieldSize; INTERNAL_LAYOUT_FIELD(T, Name, ~0u, EFieldLayoutFlags::MakeFlags(__VA_ARGS__), 1u, BitFieldSize, __COUNTER__)
#define LAYOUT_FIELD_WITH_WRITER(T, Name, Func) UE_REMOVE_OPTIONAL_PARENS(T) Name; INTERNAL_LAYOUT_FIELD_WITH_WRITER(T, Name, Func, __COUNTER__)
#define LAYOUT_MUTABLE_FIELD_WITH_WRITER(T, Name, Func) mutable UE_REMOVE_OPTIONAL_PARENS(T) Name; INTERNAL_LAYOUT_FIELD_WITH_WRITER(T, Name, Func, __COUNTER__)
...
#define LAYOUT_FIELD_EDITORONLY(T, Name, ...) UE_REMOVE_OPTIONAL_PARENS(T) Name; INTERNAL_LAYOUT_FIELD(T, Name, STRUCT_OFFSET(DerivedType, Name), EFieldLayoutFlags::MakeFlagsEditorOnly(__VA_ARGS__), 1u, 0u, __COUNTER__)
#define LAYOUT_ARRAY_EDITORONLY(T, Name, NumArray, ...) UE_REMOVE_OPTIONAL_PARENS(T) Name[NumArray]; INTERNAL_LAYOUT_FIELD(T, Name, STRUCT_OFFSET(DerivedType, Name), EFieldLayoutFlags::MakeFlagsEditorOnly(__VA_ARGS__), NumArray, 0u, __COUNTER__)
#define LAYOUT_BITFIELD_EDITORONLY(T, Name, BitFieldSize, ...) UE_REMOVE_OPTIONAL_PARENS(T) Name : BitFieldSize; INTERNAL_LAYOUT_FIELD(T, Name, ~0u, EFieldLayoutFlags::MakeFlagsEditorOnly(__VA_ARGS__), 1u, BitFieldSize, __COUNTER__)
#else
...
#define INTERNAL_LAYOUT_INTERFACE_PREFIX(Type) UE_JOIN(INTERNAL_LAYOUT_INTERFACE_PREFIX_, Type)
...
#define INTERNAL_LAYOUT_INTERFACE_SUFFIX(Type) UE_JOIN(INTERNAL_LAYOUT_INTERFACE_SUFFIX_, Type)
...
#define INTERNAL_LAYOUT_INTERFACE_INLINE_IMPL(Type) UE_JOIN(INTERNAL_LAYOUT_INTERFACE_INLINE_IMPL_, Type)

#define INTERNAL_DECLARE_TYPE_LAYOUT_COMMON(T, InInterface) \
	static constexpr int CounterBase = __COUNTER__; \
	public: using DerivedType = UE_REMOVE_OPTIONAL_PARENS(T); \
...
#define INTERNAL_DECLARE_INLINE_TYPE_LAYOUT(T, InInterface) \
	private: static void InternalDestroy(void* Object, const FTypeLayoutDesc&, const FPointerTableBase* PtrTable, bool bIsFrozen) { \
		Freeze::DestroyObject(static_cast<UE_REMOVE_OPTIONAL_PARENS(T)*>(Object), PtrTable, bIsFrozen); \
	} \
	public: static FTypeLayoutDesc& StaticGetTypeLayout() { \
		static_assert(TValidateInterfaceHelper<UE_REMOVE_OPTIONAL_PARENS(T), ETypeLayoutInterface::InInterface>::Value, #InInterface " is invalid interface for " #T); \
		static FTypeLayoutDesc& StaticTypeDesc = []() -> FTypeLayoutDesc& \
		{ \
			...
			TypeDesc.WriteFrozenMemoryImageFunc = TGetFreezeImageHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::Do(); \
			...
			TypeDesc.Size = sizeof(UE_REMOVE_OPTIONAL_PARENS(T)); \
			TypeDesc.Alignment = alignof(UE_REMOVE_OPTIONAL_PARENS(T)); \
			TypeDesc.Interface = ETypeLayoutInterface::InInterface; \
			TypeDesc.SizeFromFields = ~0u; \
			TypeDesc.GetDefaultObjectFunc = &TGetDefaultObjectHelper<UE_REMOVE_OPTIONAL_PARENS(T), ETypeLayoutInterface::InInterface>::Do; \
			InternalLinkType<1>::Initialize(TypeDesc); \
			InternalInitializeBases<UE_REMOVE_OPTIONAL_PARENS(T)>(TypeDesc); \
			FTypeLayoutDesc::Initialize(TypeDesc); \
			return TypeDesc; \
		}(); \
		return StaticTypeDesc; } \
	public: INTERNAL_LAYOUT_INTERFACE_PREFIX(InInterface)(UE_EMPTY) const FTypeLayoutDesc& GetTypeLayout() const INTERNAL_LAYOUT_INTERFACE_INLINE_IMPL(InInterface) \
...
#define INTERNAL_DECLARE_LAYOUT_BASE(T) \
	private: using InternalBaseType = typename TGetBaseTypeHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::Type; \
...
#define DECLARE_TYPE_LAYOUT(T, Interface) INTERNAL_DECLARE_LAYOUT_BASE(T); INTERNAL_DECLARE_TYPE_LAYOUT(T, Interface, UE_EMPTY)
...
#define DECLARE_TYPE_LAYOUT_EXPLICIT_BASES(T, Interface, ...) INTERNAL_DECLARE_LAYOUT_EXPLICIT_BASES(T, __VA_ARGS__); INTERNAL_DECLARE_TYPE_LAYOUT(T, Interface, UE_EMPTY)
...
#define INTERNAL_IMPLEMENT_TYPE_LAYOUT_COMMON(TemplatePrefix, T) \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) void UE_REMOVE_OPTIONAL_PARENS(T)::InternalDestroy(void* Object, const FTypeLayoutDesc&, const FPointerTableBase* PtrTable, bool bIsFrozen) { \
		Freeze::DestroyObject(static_cast<UE_REMOVE_OPTIONAL_PARENS(T)*>(Object), PtrTable, bIsFrozen); \
	} \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) FTypeLayoutDesc& UE_REMOVE_OPTIONAL_PARENS(T)::StaticGetTypeLayout() { \
		static_assert(TValidateInterfaceHelper<UE_REMOVE_OPTIONAL_PARENS(T), InterfaceType>::Value, "Invalid interface for " #T); \
		static FTypeLayoutDesc& StaticTypeDesc = []() -> FTypeLayoutDesc& \
		{ \
			alignas(FTypeLayoutDesc) static uint8 TypeBuffer[sizeof(FTypeLayoutDesc)] = { 0 }; \
			FTypeLayoutDesc& TypeDesc = *(FTypeLayoutDesc*)TypeBuffer; \
			...
			TypeDesc.SizeFromFields = ~0u; \
			TypeDesc.GetDefaultObjectFunc = &TGetDefaultObjectHelper<UE_REMOVE_OPTIONAL_PARENS(T), InterfaceType>::Do; \
			InternalLinkType<1>::Initialize(TypeDesc); \
			InternalInitializeBases<UE_REMOVE_OPTIONAL_PARENS(T)>(TypeDesc); \
			FTypeLayoutDesc::Initialize(TypeDesc); \
			return TypeDesc; \
...
//#define INTERNAL_REGISTER_TYPE_LAYOUT(T) static const FRegisterTypeLayoutDesc ANONYMOUS_VARIABLE(RegisterTypeLayoutDesc)(UE_REMOVE_OPTIONAL_PARENS(T)::StaticGetTypeLayout())
...
#define IMPLEMENT_UNREGISTERED_TEMPLATE_TYPE_LAYOUT(TemplatePrefix, T) \
	...
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) const FTypeLayoutDesc& UE_REMOVE_OPTIONAL_PARENS(T)::GetTypeLayout() const { return StaticGetTypeLayout(); }
...
#define REGISTER_INLINE_TYPE_LAYOUT(T) \
	static const FDelayedAutoRegisterHelper ANONYMOUS_VARIABLE(DelayedAutoRegisterHelper)(EDelayedRegisterRunPhase::ShaderTypesReady, [] \
		{ \
			UE_REMOVE_OPTIONAL_PARENS(T)::StaticGetTypeLayout().Name = TEXT(#T); \
			FTypeLayoutDesc::Register(UE_REMOVE_OPTIONAL_PARENS(T)::StaticGetTypeLayout()); \
		} \
	);
...
#define DECLARE_TEMPLATE_INTRINSIC_TYPE_LAYOUT(TemplatePrefix, T) \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) struct THasTypeLayout<UE_REMOVE_OPTIONAL_PARENS(T)> { UE_STATIC_ONLY(THasTypeLayout); static const bool Value = true; }; \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) struct TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)> { \
		UE_STATIC_ONLY(TStaticGetTypeLayoutHelper); \
		static void CallWriteMemoryImage(FMemoryImageWriter& Writer, const void* Object, const FTypeLayoutDesc& TypeDesc, const FTypeLayoutDesc& DerivedTypeDesc) { \
			Freeze::IntrinsicWriteMemoryImage(Writer, *static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(Object), TypeDesc); \
		} \
		static uint32 CallUnfrozenCopy(const FMemoryUnfreezeContent& Context, const void* Object, const FTypeLayoutDesc& TypeDesc, void* OutDst) { \
			return Freeze::IntrinsicUnfrozenCopy(Context, *static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(Object), OutDst); \
		} \
		static uint32 CallAppendHash(const FTypeLayoutDesc& TypeDesc, const FPlatformTypeLayoutParameters& LayoutParams, FSHA1& Hasher) { \
			return Freeze::IntrinsicAppendHash(static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(nullptr), TypeDesc, LayoutParams, Hasher); \
		} \
		static uint32 CallGetTargetAlignment(const FTypeLayoutDesc& TypeDesc, const FPlatformTypeLayoutParameters& LayoutParams) { \
			return Freeze::IntrinsicGetTargetAlignment(static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(nullptr), TypeDesc, LayoutParams); \
		} \
		static void CallToString(const void* Object, const FTypeLayoutDesc& TypeDesc, const FPlatformTypeLayoutParameters& LayoutParams, FMemoryToStringContext& OutContext) { \
			return Freeze::IntrinsicToString(*static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(Object), TypeDesc, LayoutParams, OutContext); \
		} \
		...
		static const FTypeLayoutDesc& Do() { \
			...
			{ \
				...
				TypeDesc.SizeFromFields = sizeof(UE_REMOVE_OPTIONAL_PARENS(T)); \
				return TypeDesc; \
			}(); \
			return StaticTypeDesc; } }; \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) struct TGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)> { \
		UE_STATIC_ONLY(TGetTypeLayoutHelper); \
		static const FTypeLayoutDesc& Do(const UE_REMOVE_OPTIONAL_PARENS(T)&) { return TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::Do(); }}
...
#define IMPLEMENT_EXPORTED_INTRINSIC_TYPE_LAYOUT(T) \
		void TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::CallWriteMemoryImage(FMemoryImageWriter& Writer, const void* Object, const FTypeLayoutDesc& TypeDesc, const FTypeLayoutDesc& DerivedTypeDesc) { \
			Freeze::IntrinsicWriteMemoryImage(Writer, *static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(Object), TypeDesc); \
		} \
		uint32 TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::CallUnfrozenCopy(const FMemoryUnfreezeContent& Context, const void* Object, const FTypeLayoutDesc& TypeDesc, void* OutDst) { \
			return Freeze::IntrinsicUnfrozenCopy(Context, *static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(Object), OutDst); \
		} \
		uint32 TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::CallAppendHash(const FTypeLayoutDesc& TypeDesc, const FPlatformTypeLayoutParameters& LayoutParams, FSHA1& Hasher) { \
			return Freeze::IntrinsicAppendHash(static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(nullptr), TypeDesc, LayoutParams, Hasher); \
		} \
		uint32 TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::CallGetTargetAlignment(const FTypeLayoutDesc& TypeDesc, const FPlatformTypeLayoutParameters& LayoutParams) { \
			return Freeze::IntrinsicGetTargetAlignment(static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(nullptr), TypeDesc, LayoutParams); \
		} \
		void TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::CallToString(const void* Object, const FTypeLayoutDesc& TypeDesc, const FPlatformTypeLayoutParameters& LayoutParams, FMemoryToStringContext& OutContext) { \
			return Freeze::IntrinsicToString(*static_cast<UE_REMOVE_OPTIONAL_PARENS(T) const*>(Object), TypeDesc, LayoutParams, OutContext); \
		} \
		void TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::CallDestroy(void* Object, const FTypeLayoutDesc&, const FPointerTableBase* PtrTable, bool bIsFrozen) { \
			Freeze::DestroyObject(static_cast<UE_REMOVE_OPTIONAL_PARENS(T)*>(Object), PtrTable, bIsFrozen); \
		} \
		const FTypeLayoutDesc& TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)>::Do() { \
			static FTypeLayoutDesc& StaticTypeDesc = []() -> FTypeLayoutDesc& \
			{ \
				alignas(FTypeLayoutDesc) static uint8 TypeBuffer[sizeof(FTypeLayoutDesc)] = { 0 }; \
				FTypeLayoutDesc& TypeDesc = *(FTypeLayoutDesc*)TypeBuffer; \
...
#define ALIAS_TEMPLATE_TYPE_LAYOUT(TemplatePrefix, T, Alias) \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) struct TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)> : public TStaticGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(Alias)> { UE_STATIC_ONLY(TStaticGetTypeLayoutHelper); }; \
	UE_REMOVE_OPTIONAL_PARENS(TemplatePrefix) struct TGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(T)> : public TGetTypeLayoutHelper<UE_REMOVE_OPTIONAL_PARENS(Alias)> { UE_STATIC_ONLY(TGetTypeLayoutHelper); }

#define ALIAS_TYPE_LAYOUT(Type, Alias) \
	static_assert(sizeof(UE_REMOVE_OPTIONAL_PARENS(Type)) == sizeof(UE_REMOVE_OPTIONAL_PARENS(Alias)), "Using a type alias but the sizes don't match!"); \
