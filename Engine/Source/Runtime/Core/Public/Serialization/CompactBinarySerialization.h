...
CORE_API void LogFieldTooLargeForArrayWarning(uint64 ArraySize);
 ...
 * to load into once its size has been determined. Validated with ECbValidateMode::Default.
 *
...
/** Load a compact binary field view from an archive. Allocator must manage lifetime. */
CORE_API FCbFieldView LoadCompactBinaryView(FArchive& Ar, FCbMemoryAllocator Allocator);
 ...
 *
 * @param Ar Archive to read the field from. An error state is set on failure.
 * @param Allocator Allocator for the buffer that the field is loaded into. May be FUniqueBuffer::Alloc.
 * @param ValidateMode Validation mode in which to validate the loaded field. May be None.
 * @return A field with a reference to the allocated buffer, or a default field on failure.
 */
CORE_API FCbField LoadAndValidateCompactBinary(FArchive& Ar, FCbBufferAllocator Allocator, ECbValidateMode ValidateMode);
/** Load a compact binary field view from an archive. Allocator must manage lifetime. */
CORE_API FCbFieldView LoadAndValidateCompactBinaryView(FArchive& Ar, FCbMemoryAllocator Allocator, ECbValidateMode ValidateMode);
...
{
	const uint64 Size = Field.AsArrayView().Num();
	if (Size <= MAX_int32)
	{
		OutValue.Reset((int32)Size);
		bool bOk = !Field.HasError();
		for (const FCbFieldView& ElementField : Field)
		{
			bOk &= LoadFromCompactBinary(ElementField, OutValue.Emplace_GetRef());
		}
		return bOk;
	}
	else
	{
		UE::Serialization::Private::LogFieldTooLargeForArrayWarning(Size);
		return false;
	}
}
