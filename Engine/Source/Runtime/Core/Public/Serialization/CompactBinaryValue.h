...
{
public:
	constexpr FCbValue() = default;
	FCbValue(const FCbFieldView& View);
	...
	FORCEINLINE explicit operator bool() const { return Type != ECbFieldType::None; }
...
FORCEINLINE FCbValue::FCbValue(const FCbFieldView& View)
	: FCbValue(View.GetValue())
{
}
