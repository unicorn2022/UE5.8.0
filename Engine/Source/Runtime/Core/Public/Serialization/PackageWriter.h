...
class UObject;
namespace UE::Cook { class ICookSandbox; }
...
namespace UE::Cook::Artifact
{
	...
	enum EFieldStorage : uint8
	{
		/**
		* The bytes of the field will be stored in the compact binary of the package's root entry rather than as a separate attachment.
		* This is useful for fields used as filters since no additional attachment needs to be fetched, but it makes every read of the oplog more expensive.
		*/
		Inline,
		/**
		* The bytes of the field will be store in an attachment, and the field on the package's root entry will be an
		* ObjectAttachment - a 20byte pointer to the attachment that holds the bytes of the field.
		* This should be the default for most added fields, to keep the oplog entries small.
		*/
		Attachment,
	};
}
...
public:
	...
	struct FCommitAttachmentInfo
	{
		FName Key;
		FCbObject Value;
		UE::Cook::Artifact::EFieldStorage FieldStorage;
	};
...
struct FPackageHashes : FRefCountedObject
{
...
public:
	...
	virtual UE::Cook::ICookSandbox* GetCookSandbox() = 0;
...
public:
	...
	/**
	 * Called from UCookOnTheFlyServer::PopulateCookedPackages. Load the oplog of previously cooked packages, if not
	 * already done, and return the number of them.
	 */
	virtual void PopulateOplog(const FAssetRegistryState& PreviousState, int32& OutNumPackagesInOplog) = 0;
	/**
	 * Called during CookByTheBookFinished. Remove Ops from the Oplog that have not been referenced recently
	 * and should be removed to save space in the oplog and in artifacts that cache data about packages.
	 * 
	 * @param OplogContext Information that describes which packages were cooked or their previously cooked op
	 *        inspected (by e.g. incremental cook) in the current cook. The PackageWriter should update the
	 *        LastReferencedDate for these packages before checking for ops that have not recently been referenced.
	 */
	virtual void UpdateLastReferenceDateAndPruneStaleOps(
		UE::Cook::Artifact::FUpdateOplogPackagesContext& OplogContext) = 0;
	/**
	 * Called during CookByTheBookFinished, after UpdateLastReferenceDateAndPruneStaleOps. Return the list of all
	 * packages that have cooked data from the current or previous cooks. CookArtifacts use this to cull their data
	 * when previously cooked packages are pruned.
	 */
	virtual TArray<FName> GetOplogPackageNames() = 0;
	/**
	...
	*/
	...
	/**
	* Store a value under a given key that can be read during staging and in future incremental cooks.
	* KeyName must not start with /, which identifies packages, and must not equal any existing special op names.
	*/
	virtual void AppendOp(FName KeyName, FCbObject Payload)
	{
	}
};
