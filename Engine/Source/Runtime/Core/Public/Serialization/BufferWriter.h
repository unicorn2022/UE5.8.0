	...
	void Serialize(void* Data, int64 Num)
	{
		...
		if (NumBytesToAdd > 0)
		{
			if (bAllowResize)
			{
				...
				{
					UE_LOGF(LogSerialization, Fatal, "FBufferWriter does not support data larger than 2GB. Archive name: %ls.", *GetArchiveName());
				}
			...
			{
				UE_LOGF(LogSerialization, Fatal, "FBufferWriter overflowed. Archive name: %ls.", *GetArchiveName());
			}
		}
