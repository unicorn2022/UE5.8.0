...
struct FIoStoreTocResource
{
	...
	[[nodiscard]] CORE_API FIoStoreTocResourceView AsView() const;
};

/**
 * This is the next level up from FIoStoreTocResource - it loads the toc but also resolves the directory index so you get 
 * filenames. If you don't need that, use FIoStoreTocResource.
*/
class IIoStoreTocReader
{
public:
	virtual const FIoStoreTocResourceView& GetTocResource() const = 0;
	[[nodiscard]] virtual FIoStoreTocChunkInfo GetTocChunkInfo(int32 TocEntryIndex) const = 0;
	CORE_API virtual ~IIoStoreTocReader();

	// Creates an instance for accessing the TOC. The TocFilePath should be a path to the
	// .utoc file (not the ucas or undecorated). 
	CORE_API static TIoStatusOr<TUniquePtr<IIoStoreTocReader>> ReadFromDisk(const TCHAR* TocFilePath, const TMap<FGuid, FAES::FAESKey>& DecryptionKeys);
};