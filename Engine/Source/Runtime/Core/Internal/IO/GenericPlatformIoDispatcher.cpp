...
FIoStatus FGenericPlatformIoDispatcher::OnInitialize()
{
	...
	UE_LOGF(LogPlatformIoDispatcher, Log, "Initializing, Platform='%ls', ReadSize=%dKB, ReadMemory=%dMB, FileCache=%dMB, RequestSorting=%ls, MaxConcurrentDecodeJobs=%d",
	...
	if (FileBlockRequest->ErrorCode != EIoErrorCode::Cancelled)
	{
		...
		if (FileBlockCache.Get(*FileBlockRequest) == false)
		{
			...
			while (RetryCount++ < 10)
			{
				...
				{
					UE_LOGF(LogPlatformIoDispatcher, Warning, "Failed to seek to offset %lld", int64(FileBlockRequest->FileOffset));
					continue;
				}
				if (FileHandle.Handle->Read(reinterpret_cast<uint8*>(FileBlockRequest->Buffer), int64(FileBlockRequest->Size)) == false)
				{
					UE_LOGF(LogPlatformIoDispatcher, Warning, "Failed to read %lld bytes at offset %lld", int64(FileBlockRequest->Size), int64(FileBlockRequest->FileOffset));
					continue;
				}
				FileBlockRequest->ErrorCode = EIoErrorCode::Ok;
