...
namespace OldTaskGraphTests
{
	...
	{
		UE_LOGF(LogConsoleResponse, Display, "Total %6.3fms   %6.3fms queue   %6.3fms wait   %6.3fms work   : %ls"
	...
	static void TaskGraphBenchmark(const TArray<FString>& Args)
	{
		...
		{
			UE_LOGF(LogConsoleResponse, Display, "WARNING: TaskGraphBenchmark disabled for non multi-threading platforms");
			return;
		}
	...
	{
		UE_LOGF(LogTemp, Display, "Starting latency test....");
		...
		auto ForegroundTask = [](uint64 StartCycles)
		{
			...
			//UE_LOGF(LogTemp, Display, "Latency %6.2fus", Latency);
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		...
		{	// check ref count for named thread tasks
			FGraphEventRef LocalQueueTask = FFunctionGraphTask::CreateAndDispatchWhenReady(
				[] 
				{
			...
			check(LocalQueueTask->IsComplete());
			check(LocalQueueTask.GetRefCount() >= 1);
			// Now that the wait above can perform retraction, it will not necessarily pump the game thread queue and will just leave an empty task with a refcount on it
			// so pump it ourself to verify that once the game thread queue is empty, the refcount should have been cleaned up as well.
			FTaskGraphInterface::Get().ProcessThreadUntilIdle(ENamedThreads::GameThread_Local);
			check(LocalQueueTask.GetRefCount() == 1);
		}
		...
		{	// a particular real-life case that doesn't work in the old TaskGraph if run in single-threaded mode.
			...
			bool bHasRun = false;
			FGraphEventRef AnyTask = FFunctionGraphTask::CreateAndDispatchWhenReady([&bWorkerDone] { bWorkerDone = true; });
			FGraphEventRef LocalQueueTask = FFunctionGraphTask::CreateAndDispatchWhenReady(
				[&bHasRun]
				{
					check(bHasRun == false);
					bHasRun = true;
					check(IsInGameThread());
				}, TStatId{}, AnyTask, ENamedThreads::GameThread_Local);
			...
			// Now that the wait above can perform retraction, it will not necessarily pump the game thread queue and will just leave an empty task with a refcount on it
			// so pump it ourself to verify that once the game thread queue is empty, the refcount should have been cleaned up as well.
			// The reason we do it in a loop, is that the task on the game thread could be queued after the wait returns from a successful retraction.
			while (LocalQueueTask.GetRefCount() > 1)
			{
				FTaskGraphInterface::Get().ProcessThreadUntilIdle(ENamedThreads::GameThread_Local);
			}
			check(LocalQueueTask.GetRefCount() == 1);
		...
		{	// launch a GT task, then an any-thread task that depends on it. wait for the any-thread task. this was a deadlock on the new frontend
			FGraphEventRef GTTask = FFunctionGraphTask::CreateAndDispatchWhenReady([] { check(IsInGameThread()); }, TStatId{}, nullptr, ENamedThreads::GameThread);
	...
	void Fib()
	{
		...
		ResEvent->Wait();
		UE_LOGF(LogTemp, Display, "Fibonacci(%d) = %d", N, *Res);
	}
