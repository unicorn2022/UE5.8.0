...
TEST_CASE("Core::Async::ParkingLot", "[Core][Async]")
{
	SECTION("FIFO, WakeToken")
	{
		...
		for (int32 Index = 0; Index <= TaskCount; ++Index)
		{
			...
			ParkingLot::WakeOne(&WaitCount, [&](ParkingLot::FWakeState WakeState) -> uint64
			{
				++WakeCount;
				bDidWake = WakeState.bDidWake;
				bHasWaitingThreads = WakeState.bHasWaitingThreads;
				return ++Sequence;
			});
			// The callback must be invoked exactly once.
			CHECK(WakeCount == 1);
			CHECK(bDidWake == (Index < TaskCount));
			CHECK(bHasWaitingThreads >= (Index + 1 < TaskCount));
		}
