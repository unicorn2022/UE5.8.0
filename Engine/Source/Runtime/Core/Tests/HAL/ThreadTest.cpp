...
namespace
{
	void TestIsJoinableAfterCreation()
	{
		...
		Thread.Join();
		UE_LOGF(LogTemp, Log, "%ls completed", StringCast<TCHAR>(__FUNCTION__).Get());
	}
	...
	void TestThreadSingleton()
	{
		...
		{	// check that ThreadSingleton entries don't point to invalid memory after cleanup, and that destruction occurs in the reverse order of construction.
			...
			{
				virtual ~FThreadSingletonFirst()
				{
					// By the time we reach this destructor, the second singleton's destructor should have been executed already.
	...
	void TestTypicalUseCase()
	{
		...
		FThread WorkerThread(TEXT("Test.Thread.TestTypicalUseCase"), [&bQuitRequested, &WorkQueue, WorkQueuedEvent]() 
		{
			while (!bQuitRequested)
			{
				...
				// do work
				UE_LOGF(LogTemp, Log, "Work #%d consumed", Work);
			}

			UE_LOGF(LogTemp, Log, "Quit");
		});
		...
		for (FWork Work = 0; Work != WorkNum; ++Work)
		{
			...
			UE_LOGF(LogTemp, Log, "Work #%d produced", Work);
		}

		UE_LOGF(LogTemp, Log, "Request to quit");
		bQuitRequested = true;
...
{
	UE_LOGF(LogTemp, Log, "%ls", StringCast<TCHAR>(__FUNCTION__).Get());
