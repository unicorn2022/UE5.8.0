...
namespace UE { namespace TasksTests
{
	...
	TEST_CASE_NAMED(FTasksBasicTest, "System::Core::Tasks::Basic", "[.][ApplicationContextMask][EngineFilter]")
	{
		...
		{	// a basic test for multiple named thread tasks
			// This will exercise retraction on named thread
			TArray<FTask> GTTasks;
	...
	{
		{
			...
			//UE_LOGF(LogTemp, Display, "Spawning %d empty trackable tasks took %f secs", NumTasks, Duration);
		...
		}
		{
			...
			//double Duration = FPlatformTime::Seconds() - StartTime;
			//UE_LOGF(LogTemp, Display, "Spawning %d empty non-trackable tasks took %f secs", NumTasks, Duration);
		}
	}
	...
	void TestBatchSpawning()
	{
		...
		//UE_LOGF(LogTemp, Display, "Spawning %d empty non-trackable tasks took %f secs total, %f secs spawning and %f secs dispatching", NumTasks, EndTime - StartTime, SpawnedTime - StartTime, EndTime - SpawnedTime);
	...
	template<uint32 RepeatCount>
	void TaskConcurrencyLimiter_WaitingRetractionStressTest()
	{
		for (uint32 RepeatIndex = 0; RepeatIndex < RepeatCount; ++RepeatIndex)
		{
			// Schedule more blocking work than we have task threads to run them on so we are forced to retract work in the limiter to proceed
			const uint32 NumWaitingTasks = LowLevelTasks::FScheduler::Get().GetMaxNumWorkers() * 2;
			TArray<FEventRef> Blockers;
			TArray<FTask> WaitingTasks;
			Blockers.SetNum(NumWaitingTasks);
			WaitingTasks.Reserve(NumWaitingTasks);
			...
			{
				WaitingTasks.Add(Launch(UE_SOURCE_LOCATION,
					[&Blockers, WaitIndex, &WaitCount]
					{
						TRACE_CPUPROFILER_EVENT_SCOPE(Waiting);
						WaitCount.fetch_add(1, std::memory_order_relaxed);
						Blockers[WaitIndex]->Wait();
					}));
			}

			// Ensure we have waited long enough that we have started blocking tasks on all worker threads
			while (WaitCount.load(std::memory_order_relaxed) < (int32)LowLevelTasks::FScheduler::Get().GetNumWorkers())
			{
			...
			{
				TaskConcurrencyLimiter.Push(UE_SOURCE_LOCATION,
					[&Blockers, WaitIndex](uint32)
					{
						TRACE_CPUPROFILER_EVENT_SCOPE(Blocked);
						Blockers[WaitIndex]->Trigger();
					}
				);
			}

			// Without retractions we will never run any of the queued task work
			TaskConcurrencyLimiter.Wait();
			Wait(WaitingTasks);
		}
	}

	TEST_CASE_NAMED(FTasksConcurrencyLimiterWaitingRetractionTest, "System::Core::Async::Tasks::TaskConcurrencyLimiter::WaitingRetraction", "[.][ApplicationContextMask][EngineFilter]")
	{
		UE_BENCHMARK(5, TaskConcurrencyLimiter_WaitingRetractionStressTest<10'000>);
	}
	...
	// StressTest RestartWorkers while a standby worker is actively executing tasks.
	// ConditionalStandby is called after each task execution in StandbyLoop.
	// The race: a standby worker can push itself onto the standby stack (via CAS)
	// after StartShutdown has drained the stack but before it does StandbyState = StackMask.
	// The worker blocks on Event->Wait() with nobody to wake it, and StopWorkers hangs on Join().
	TEST_CASE_NAMED(FConditionalStandbyShutdownRace, "System::Core::Async::Tasks::ConditionalStandbyShutdownRace", "[.][ApplicationContextMask][EngineFilter]")
	{
		for (int32 Index = 0; Index < 100; ++Index)
		{
			// Single foreground worker so oversubscription reliably creates a standby worker.
			LowLevelTasks::FScheduler::Get().RestartWorkers(1, 0);

			UE::FManualResetEvent BlockerReady;
			UE::FManualResetEvent BlockerRelease;

			// Block the only normal worker and hold oversubscription open.
			// This forces creation of a standby worker that must handle subsequent tasks.
			FTask Blocker = Launch(TEXT("Blocker"),
				[&]()
				{
					LowLevelTasks::FOversubscriptionScope _;
					BlockerReady.Notify();
					BlockerRelease.Wait();
					// Scope ends here: oversubscription decrements.
					// The standby worker's next ConditionalStandby will enter
					// the while loop (activeCount > ThreadCount + 0).
				});

			// Wait for oversubscription to be active and the standby worker to exist.
			BlockerReady.Wait();

			// Feed tasks for the standby worker. After each execution it calls
			// ConditionalStandby. Once oversubscription ends, that call will
			// try to push itself onto the standby stack.
			for (int32 i = 0; i < 8; ++i)
			{
				Launch(TEXT("StandbyWork"), []() {});
			}

			// Release the blocker (ends oversubscription) and immediately restart.
			// The standby worker may be mid-ConditionalStandby when StartShutdown runs.
			BlockerRelease.Notify();
			LowLevelTasks::FScheduler::Get().RestartWorkers(0, 4);
	...
	TEST_CASE_NAMED(FTaskGraphLongEventChain, "System::Core::Async::Tasks::LongEventChain", "[ApplicationContextMask][EngineFilter]")
	{
		UE::Tasks::FTaskEvent RootEvent(TEXT("RootEvent"));

		UE::Tasks::FPipe Pipe(TEXT("Pipe"));

		std::atomic<int32> CurrentIndex = 0;

		// Launch a long event chain with RootEvent not triggered yet.
		// Each event will unlock a task to put in the pipe, and then we can observe the unlock order.
		UE::Tasks::FTaskEvent LastEvent = RootEvent;
		for (int32 Index = 0; Index < 100000; ++Index)
		{
			UE::Tasks::FTaskEvent Event(TEXT("Event"));
			Event.AddPrerequisites(LastEvent);
			...
			// RootEvent is not triggered yet, so this does launch the task yet
			Event.Trigger();
			LastEvent = Event;
		}

		// Without the overflow handling in Close(), this would probably cause a stack overflow
		RootEvent.Trigger();
