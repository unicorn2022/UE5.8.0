	...
	{
		{
				...
				TestTrue(FString::Printf(TEXT("SaveCompactBinary(%s)->EqualBytes"), Test), ExpectedData.EqualBytes(MakeMemoryView(WriteAr))))
			{
				...
				{
					TArray<uint8, TInlineAllocator<256>> FieldBuffer;
					FMemoryReader ReadAr(WriteAr);
					FCbFieldView FieldView = LoadCompactBinaryView(ReadAr, [&FieldBuffer](uint64 Size)
					{
						FieldBuffer.SetNumUninitialized(IntCastChecked<int32>(Size));
						return MakeMemoryView(FieldBuffer);
					});
					CHECK_MESSAGE(FString::Printf(TEXT("LoadCompactBinaryView(%s)->EqualBytes"), Test), ExpectedData.EqualBytes(MakeMemoryView(FieldBuffer)));
				}
			}
		}
		{
