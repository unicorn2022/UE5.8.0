...
TEST_CASE_NAMED(FCompactHashTableViewTest, "System::Core::Containers::CompactHashTable::Dynamic", "[Core][Containers][CompactHashTable]")
{
	SECTION("Constructor Behavior")
	{
		const uint8 Sentinel = 0xf0;
		...
		auto TestTableType = []<uint32 Size, uint32 HashSize = Size>()
		{
			...
			constexpr uint32 IndexStorageSize = Size * UE::Core::Private::CompactSetAllocatorHelpers::KeyStorageMultiplier;
			static_assert(RequiredSize == sizeof(typename TStaticCompactHashTable<Size>::IndexType) * (IndexStorageSize + HashSize));
			...
			{
				if (Index < IndexStorageSize * sizeof(IndexType))
				{
					CHECK(TestMemory[Index] == Sentinel);
				}
				else
