...
TEST_CASE_NAMED(FPathTests, "System::Core::Misc::Paths", "[ApplicationContextMask][SmokeFilter]")
{
	...
	// Extension texts
	{
		auto RunGetTest = [](const TCHAR* InPath, const TCHAR* InExpectedExt, const TCHAR* InExpectedExtDot)
			{
				FString Ext = FPaths::GetExtension(FString(InPath));
				if (Ext != InExpectedExt)
				{
					FAIL_CHECK(FString::Printf(TEXT("Path '%s' failed to get the extension (got '%s', expected '%s')."),
						InPath, *Ext, InExpectedExt));
				}
				Ext = FPaths::GetExtension(FString(InPath), false /* bIncludeDot */);
				if (Ext != InExpectedExt)
				{
					FAIL_CHECK(FString::Printf(TEXT("Path '%s' failed to get the extension (got '%s', expected '%s')."),
						InPath, *Ext, InExpectedExt));
				}
				Ext = FPaths::GetExtension(FString(InPath), true/* bIncludeDot */);
				if (Ext != InExpectedExtDot)
				{
					FAIL_CHECK(FString::Printf(TEXT("Path '%s' failed to get the extension (got '%s', expected '%s')."),
						InPath, *Ext, InExpectedExtDot));
				}
				Ext = FPaths::GetExtension(FString(InPath), EFlags::IncludeDot);
				if (Ext != InExpectedExtDot)
				{
					FAIL_CHECK(FString::Printf(TEXT("Path '%s' failed to get the extension (got '%s', expected '%s')."),
						InPath, *Ext, InExpectedExtDot));
		...
		auto RunGetTestFull = [](const TCHAR* InPath, const TCHAR* InExpectedExt, const TCHAR* InExpectedExtDot,
			const TCHAR* InExpectedExtCompound, const TCHAR* InExpectedExtDotCompound)
			{
				...
				Ext = FPaths::GetExtension(FString(InPath), EFlags::None);
				if (Ext != InExpectedExt)
				{
					FAIL_CHECK(FString::Printf(TEXT("Path '%s' failed to get the extension (got '%s', expected '%s')."),
						InPath, *Ext, InExpectedExt));
				...
				Ext = FPaths::GetExtension(FString(InPath), EFlags::AllowCompoundExtension);
				if (Ext != InExpectedExtCompound)
				{
					FAIL_CHECK(FString::Printf(TEXT("Path '%s' failed to get the extension (got '%s', expected '%s')."),
						InPath, *Ext, InExpectedExtCompound));
				}
				Ext = FPaths::GetExtension(FString(InPath), EFlags::IncludeDot | EFlags::AllowCompoundExtension);
				if (Ext != InExpectedExtDotCompound)
				{
					FAIL_CHECK(FString::Printf(TEXT("Path '%s' failed to get the extension (got '%s', expected '%s')."),
						InPath, *Ext, InExpectedExtDotCompound));
				}
			};



		RunGetTest(TEXT("file"),								TEXT(""),	TEXT(""));
		RunGetTest(TEXT("file.txt"),							TEXT("txt"),TEXT(".txt"));
		RunGetTest(TEXT("file.tar.gz"),							TEXT("gz"),	TEXT(".gz"));
		RunGetTest(TEXT("C:/Folder/file"),						TEXT(""),	TEXT(""));
		RunGetTest(TEXT("C:/Folder/file.txt"),					TEXT("txt"),TEXT(".txt"));
		RunGetTestFull(TEXT("C:/Folder/file.tar.gz"),			TEXT("gz"),	TEXT(".gz"),	TEXT("tar.gz"), TEXT(".tar.gz"));
		RunGetTestFull(TEXT("C:/Folder/First.Last/file"),		TEXT(""),	TEXT(""),		TEXT(""),		TEXT(""));
		RunGetTestFull(TEXT("C:/Folder/First.Last/file.txt"),	TEXT("txt"),TEXT(".txt"),	TEXT("txt"),	TEXT(".txt"));
		RunGetTestFull(TEXT("C:/Folder/First.Last/file.tar.gz"),TEXT("gz"),	TEXT(".gz"),	TEXT("tar.gz"), TEXT(".tar.gz"));
