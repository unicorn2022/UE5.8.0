...
void FAutomationWorkerModule::SendTests( const FMessageAddress& ControllerAddress )
{
	...
	UE_LOGF(LogAutomationWorker, Log, "Set %d tests to %ls", TestInfo.Num(), *ControllerAddress.ToString());
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received FindWorkersMessage from %ls", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received NextNetworkCommandReplyMessage from %ls", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received StartTestSession from %ls", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received StopTestSession from %ls", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received RequestTestsMessage from %ls", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received ScreenShotCompared from %ls", *Context->GetSender().ToString());

	if (Message.UniqueId != ActiveScreenshotComparisonId)
	{
		UE_LOGF(LogAutomationWorker, Log, "Ignoring unexpected screenshot comparison result for %ls", *Message.ScreenshotPath);
		return;
	}
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received TestDataRetrieved from %ls", *Context->GetSender().ToString());
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received PerformanceDataRetrieved from %ls", *Context->GetSender().ToString());
...
void FAutomationWorkerModule::HandleScreenShotAndTraceCapturedWithName(const TArray<FColor>& RawImageData, const TArray<uint8>& CapturedFrameTrace, const FAutomationScreenshotData& Data)
{
	...
	if (TestRequesterAddress.IsValid())
	{
		...
		UE_LOGF(LogAutomationWorker, Log, "Sending screenshot %ls to %ls", *Message->ScreenShotName, *TestRequesterAddress.ToString());
	...
	else
	{
		...
		UE_LOGF(LogAutomationWorker, Log, "Saving screenshot %ls as %ls",*Data.ScreenshotPath, *LocalFile);

		if (!IFileManager::Get().MakeDirectory(*DirectoryPath, bTree))
		{
			UE_LOGF(LogAutomationWorker, Error, "Failed to create directory %ls for incoming screenshot", *DirectoryPath);
			return;
		}
		...
		if (!FFileHelper::SaveArrayToFile(CompressedBitmap, *LocalFile))
		{
			...
			UE_LOGF(LogAutomationWorker, Warning, "Fail to save screenshot to %ls. WriteError: %u (%ls)", *LocalFile, WriteErrorCode, WriteErrorBuffer);
			return;
		}
		...
		if (CapturedFrameTrace.Num() > 0)
		{
			if (!FFileHelper::SaveArrayToFile(CapturedFrameTrace, *LocalTraceFile))
			{
				...
				TCHAR WriteErrorBuffer[2048];
				FPlatformMisc::GetSystemErrorMessage(WriteErrorBuffer, 2048, WriteErrorCode);
				UE_LOGF(LogAutomationWorker, Warning, "Failed to save frame trace to %ls. WriteError: %u (%ls)", *LocalTraceFile, WriteErrorCode, WriteErrorBuffer);
			}
		}
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received RunTests %ls from %ls", *Message.BeautifiedTestName, *Context->GetSender().ToString());
	...
	if (TestRequesterAddress.IsValid() && !TestName.IsEmpty())
	{
		...
		{
			UE_LOGF(LogAutomationWorker, Log, "Worker is already running test '%ls' from %ls. Request is ignored.", *BeautifiedTestName, *TestRequesterAddress.ToString());
			return;
		}
		...
		UE_LOGF(LogAutomationWorker, Warning, "%ls", *LogMessage);
		...
		|| IsTestExcluded(Message.FullTestPath, &SkipReason, &bWarn))
	{
		...
		if (bWarn)
		{
			UE_LOGF(LogAutomationWorker, Warning, "%ls", *SkippingMessage);
		}
		else
		{
			UE_LOGF(LogAutomationWorker, Display, "%ls", *SkippingMessage);
		}
...
{
	UE_LOGF(LogAutomationWorker, Log, "Received StopTests from %ls", *Context->GetSender().ToString());
...
void FAutomationWorkerModule::HandleTelemetryData(const FString& StorageName, const FString& InTestName, const TArray<FAutomationTelemetryData>& InItems)
{
	...
	UE_LOGF(LogAutomationWorker, Log, "Sending Telemetry Data for %ls", *Message->TestName);
