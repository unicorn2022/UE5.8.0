...
void FSkeletalMeshLODModelToDynamicMesh::Convert(const FSkeletalMeshLODModel* MeshIn, FDynamicMesh3& MeshOut, bool bCopyTangents)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "FSkeletalMeshLODModelToDynamicMesh:FSkeletalMeshLODModel verts %d  instances %d", ModelWrapper.NumVerts(), 3*ModelWrapper.NumTris());
	}
	...
	if (bPrintDebugMessages)
	{
		...
		UE_LOGF(LogTemp, Warning, "FSkeletalMeshLODModelToDynamicMesh:  Conversion Timing: Triangles %fs   Attributes %fs",
		...
		UE_LOGF(LogTemp, Warning, "FSkeletalMeshLODModelToDynamicMesh:  FDynamicMesh verts %d triangles %d (primary) uvs %d normals %d", MeshOut.MaxVertexID(), MeshOut.MaxTriangleID(), NumUVs, NumNormals);
	}
