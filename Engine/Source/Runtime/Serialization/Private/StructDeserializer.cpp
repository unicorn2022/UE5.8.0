		...
		case EStructDeserializerBackendTokens::ArrayEnd:
			{
				...
				if (StateStack.Num() == 0)
				{
					UE_LOGF(LogSerialization, Verbose, "Malformed input: Found ArrayEnd without matching ArrayStart");
		...
		case EStructDeserializerBackendTokens::ArrayStart:
			{
				...
				else
				{
					...
					{
						UE_LOGF(LogSerialization, Verbose, "The array property '%ls' does not exist", *PropertyName);
					}
		...
		case EStructDeserializerBackendTokens::Property:
			{
				...
				if (PropertyName.IsEmpty() && (CurrentState.Property != nullptr) && (CurrentState.Property->GetClass() == FSetProperty::StaticClass()))
				{
					...
					{
						UE_LOGF(LogSerialization, Verbose, "An item in Set '%ls' could not be read (%ls)", *PropertyName, *Backend.GetDebugString());
					}
				}
				...
				else if (PropertyName.IsEmpty())
				{
					...
					if (Property == nullptr)
					{
						...
						if (Policies.MissingFields != EStructDeserializerErrorPolicies::Ignore)
						{
							UE_LOGF(LogSerialization, Verbose, "Failed to serialize array element %i", CurrentState.ArrayIndex);
						}
					...
					{
						UE_LOGF(LogSerialization, Verbose, "The array element '%ls[%i]' could not be read (%ls)", *PropertyName, CurrentState.ArrayIndex, *Backend.GetDebugString());
					}
				...
				else if ((CurrentState.Property != nullptr) && (CurrentState.Property->GetClass() == FMapProperty::StaticClass()))
				{
					...
					{
						UE_LOGF(LogSerialization, Verbose, "An item in map '%ls' could not be read (%ls)", *PropertyName, *Backend.GetDebugString());
					}
				}
				else if ((CurrentState.Property != nullptr) && (CurrentState.Property->GetClass() == FOptionalProperty::StaticClass()))
				{
					...
					{
						UE_LOGF(LogSerialization, Verbose, "An item in optional '%ls' could not be read (%ls)", *PropertyName, *Backend.GetDebugString());
					}
				}
				else
				{
					...
					if (Property != nullptr)
					{
						...
						{
							UE_LOGF(LogSerialization, Verbose, "The property '%ls' could not be read (%ls)", *PropertyName, *Backend.GetDebugString());
						}
					}
					else
					{
						...
						if (Policies.MissingFields != EStructDeserializerErrorPolicies::Ignore)
						{
							UE_LOGF(LogSerialization, Verbose, "The property '%ls' does not exist", *PropertyName);
						}
		...
		case EStructDeserializerBackendTokens::StructureStart:
			{
				...
				if (PropertyName.IsEmpty())
				{
					...
					{
						UE_LOGF(LogSerialization, Verbose, "Found unnamed value outside of array or set.");
						return false;
					}
				}
				...
				else
				{
					...
					if (NewState.Property == nullptr)
					{
						...
						if (Policies.MissingFields != EStructDeserializerErrorPolicies::Ignore)
						{
							UE_LOGF(LogSerialization, Verbose, "Map, Set, or struct property '%ls' not found", *PropertyName);
						}
				...
				else
				{
					...
					if (Policies.MissingFields != EStructDeserializerErrorPolicies::Ignore)
					{
						UE_LOGF(LogSerialization, Verbose, "Structured property '%ls' not found", *PropertyName);
					}
	...
	while (Backend.GetNextToken(Token))
	{
		...
		case EStructDeserializerBackendTokens::ArrayEnd:
		{
			...
			else if (CurrentState.Property->ArrayDim > 1 && CurrentState.ArrayIndex < CurrentState.Property->ArrayDim)
			{
				...
				if (Policies.MissingFields != EStructDeserializerErrorPolicies::Ignore)
				{
					UE_LOGF(LogSerialization, Verbose, "The static array '%ls' of size %d only had %d entries", *CurrentState.Property->GetFName().ToString(), CurrentState.Property->ArrayDim, CurrentState.ArrayIndex);
				}
			...
			else if (PropertyName.IsEmpty())
			{
				...
				else
				{
					...
					else
					{
						...
						UE_LOGF(LogSerialization, Verbose, "Static array %ls has dimension of %d and trying to read element %d", *CurrentState.Property->GetFName().ToString(), CurrentState.Property->ArrayDim, CurrentState.ArrayIndex);
						continue;
					}
				}
			...
			else
			{
				...
				if (Property != nullptr)
				{
					...
					else if (FOptionalProperty* OptionalProperty = CastField<FOptionalProperty>(Property))
					{
						...
						{
							// Not set
							UE_LOGF(LogSerialization, Verbose, "TOptional %ls is not set and is trying to be read", *OptionalProperty->GetFName().ToString());
							Backend.SkipStructure();
							continue;
						}
					}
		...
		case EStructDeserializerBackendTokens::StructureStart:
		{
			...
			else
			{
				...
				else
				{
					...
					{
						//Index out of bound
						UE_LOGF(LogSerialization, Verbose, "Static array %ls has dimension of %d and trying to read element %d", *NewState.Property->GetFName().ToString(), NewState.Property->ArrayDim, CurrentState.ArrayIndex);
						Backend.SkipStructure();
						continue;
					}
