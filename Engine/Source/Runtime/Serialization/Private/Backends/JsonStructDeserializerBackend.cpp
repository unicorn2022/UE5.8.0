	...
	case EJsonNotation::Number:
		{
			...
			UE_LOGF(LogSerialization, Verbose, "Numeric field %ls with value '%ls' is not supported in FProperty type %ls (%ls)", *Property->GetFName().ToString(), *NumericValueString, *Property->GetClass()->GetName(), *GetDebugString());
