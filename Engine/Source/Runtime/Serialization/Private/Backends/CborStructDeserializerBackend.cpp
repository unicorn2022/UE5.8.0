...
namespace UE::CborStructDeserializerBackend::Private
{

FString ConvertCborTypeToString(ECborCode CborCode)
{
	switch (CborCode)
	{
		case ECborCode::Uint:
...
bool FCborStructDeserializerBackend::GetNextToken(EStructDeserializerBackendTokens& OutToken)
{
	...
	// We've read an odd number of items from this map so far. So the next value is the key and it must be a string.
	if (CborReader.GetContext().MajorType() == ECborCode::Map && (CborReader.GetContext().AsLength() & 1))
	{
		// This code is designed to deserialize CBOR data into Unreal Engine structs. If this is not a string then we are somehow not dealing with UE
		// structs and this would be an error. Previously this code would just assert and crash.  This gracefully handles this case and passes the
		// failure down to the caller. 		
		if (LastContext.MajorType() == ECborCode::TextString)
		{
		...
		else
		{
			...
			FString CborMajorTypeAsString = UE::CborStructDeserializerBackend::Private::ConvertCborTypeToString(LastContext.MajorType());
			uint8 CborMajorTypeAsInt = static_cast<uint8>(LastContext.MajorType());
			UE_LOGF(LogSerialization, Error, "Encountered invalided context while parsing CBOR map. Expected a string type but got code %d (%ls)", CborMajorTypeAsInt, *CborMajorTypeAsString);
			return false;
		}
	}
...
bool FCborStructDeserializerBackend::ReadProperty(FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex)
{
	...
	case ECborCode::TextString:
	{
		...
		UE_LOGF(LogSerialization, Verbose, "String field %ls with value '%ls' is not supported in FProperty type %ls (%ls)", *Property->GetFName().ToString(), *StringValue, *Property->GetClass()->GetName(), *GetDebugString());
	...
	case ECborCode::ByteString:
	{
		...
		UE_LOGF(LogSerialization, Verbose, "Error while deserializing field %ls. Unexpected UProperty type %ls. Expected a UByteProperty/UInt8Property to deserialize a TArray<uint8>/TArray<int8>", *Property->GetFName().ToString(), *Property->GetClass()->GetName());
		return false;
	}
	...
	case ECborCode::Prim:
	{
		...
		case ECborCode::False:
		{
			...
			UE_LOGF(LogSerialization, Verbose, "Boolean field %ls with value '%ls' is not supported in FProperty type %ls (%ls)", *Property->GetFName().ToString(), LastContext.AsBool() ? *(CoreTexts.True.ToString()) : *(CoreTexts.False.ToString()), *Property->GetClass()->GetName(), *GetDebugString());
			return false;
		}
			// Null
		case ECborCode::Null:
		...
		case ECborCode::Value_4Bytes:
			...
			UE_LOGF(LogSerialization, Verbose, "Float field %ls with value '%f' is not supported in FProperty type %ls (%ls)", *Property->GetFName().ToString(), LastContext.AsFloat(), *Property->GetClass()->GetName(), *GetDebugString());
			return false;
			// Double
		case ECborCode::Value_8Bytes:
			...
			UE_LOGF(LogSerialization, Verbose, "Double field %ls with value '%f' is not supported in FProperty type %ls (%ls)", *Property->GetFName().ToString(), LastContext.AsDouble(), *Property->GetClass()->GetName(), *GetDebugString());
			return false;
		default:
			UE_LOGF(LogSerialization, Verbose, "Unsupported primitive type for %ls with value '%f' in FProperty type %ls (%ls)", *Property->GetFName().ToString(), LastContext.AsDouble(), *Property->GetClass()->GetName(), *GetDebugString());
			return false;
		}
	}
	}
