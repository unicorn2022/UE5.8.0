...
void FCborStructSerializerBackend::WriteProperty(const FStructSerializerState& State, int32 ArrayIndex)
{
	...
	{
		UE_LOGF(LogSerialization, Verbose, "FCborStructSerializerBackend: Property %ls cannot be serialized, because its type (%ls) is not supported", *State.ValueProperty->GetFName().ToString(), *State.ValueType->GetFName().ToString());
	}
