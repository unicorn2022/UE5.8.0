...
void FLiveLinkProvider::ClearTrackedSubject(const FName& SubjectName)
{
	...
	SubjectFrameIds.Remove(SubjectName);
}
...
bool FLiveLinkProvider::UpdateSubjectFrameData(const FName SubjectName, FLiveLinkFrameDataStruct&& FrameData, const TMap<FName, FString>& ExtraAnnotations)
{
	...
	// Assign a FrameId to this subject

	if (FLiveLinkBaseFrameData* BaseData = FrameData.GetBaseData())
	{
		int32& NextFrameId = SubjectFrameIds.FindOrAdd(SubjectName, 0);

		if (BaseData->FrameId < 0)
		{
			BaseData->FrameId = NextFrameId;
			NextFrameId = int32(uint32(NextFrameId) + 1u);
		}
		else
		{
			// Provider manually set FrameId, so honor it and update our internal one with it.
			NextFrameId = FMath::Max(NextFrameId, int32(uint32(BaseData->FrameId) + 1u));
		}
	}
	...
	if (ConnectedAddresses.Num() > 0)
	{
		...
		bool bCanCompressData = CVarLiveLinkCompressAnimData.GetValueOnAnyThread() && FrameData.GetStruct() == FLiveLinkAnimationFrameData::StaticStruct();
		...
		if (bCanCompressData && FrameData.GetStruct() == FLiveLinkAnimationFrameData::StaticStruct())
		{
...
void FLiveLinkProvider::HandlePingMessage(const FLiveLinkPingMessage& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	{
		UE_LOGF(LogLiveLinkMessageBus, Warning, "A unsupported version of LiveLink is trying to communicate. Requested version: '%d'. Supported version: '%d'.", Message.LiveLinkVersion, LIVELINK_SupportedVersion)
		return;
	}
...
void FLiveLinkProvider::HandleConnectMessage(const FLiveLinkConnectMessage& Message, const TSharedRef<IMessageContext, ESPMode::ThreadSafe>& Context)
{
	...
	if (Message.LiveLinkVersion < LIVELINK_SupportedVersion)
	{
		UE_LOGF(LogLiveLinkMessageBus, Error, "A unsupported version of LiveLink is trying to connect. Requested version: '%d'. Supported version: '%d'.", Message.LiveLinkVersion, LIVELINK_SupportedVersion)
		return;
	}
	...
	if (!ConnectedAddresses.ContainsByPredicate([=](const FTrackedAddress& Address) { return Address.Address == ConnectionAddress; }))
	{
		...
		for (const auto& Subject : Subjects)
		{
			...
			FPlatformProcess::Sleep(0.1f); //HACK: Try to help these go in order, editor needs extra buffering support to make sure this isn't needed in future.
