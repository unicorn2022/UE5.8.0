...
void InitDebugContext()
{
	...
	{
		UE_LOGF(LogRHI,Warning,"OpenGL debug output extension not supported!");
	}
	else
	{
		UE_LOGF(LogRHI, Log, "OpenGL debug output extension enabled!");
	}
...
#endif
		UE_LOGF(LogRHI,Verbose,"disabling reporting back of debug groups and markers to the OpenGL debug output callback");
	}
...
void ApplyExtensionsOverrides(FString& ExtensionsString)
{
	// Strip extensions
	{
		...
		for (FString& ExtName : ExtList)
		{
			...
			{
				UE_LOGF(LogRHI, Log, "Stripped extension: %ls", *ExtName);
			}
		}
	}
	...
	// Add extensions
	{
		...
		for (FString& ExtName : ExtList)
		{
			...
			if (!ExtensionsString.Contains(ExtName))
			{
				...
				UE_LOGF(LogRHI, Log, "Added extension: %ls", *ExtName);
			}
		}
	}
}
...
static void InitRHICapabilitiesForGL()
{
...
#endif
	UE_LOGF(LogRHI, Log, "Initializing OpenGL RHI");
...
#endif
	{
		...
		UE_LOGF(LogRHI, Log, "OpenGL Extensions:");
		...
		{
			UE_LOGF(LogRHI, Log, "  %ls", *GLExtensionArray[ExtIndex]);
		}
...
#undef LOG_AND_GET_GL_INT_TEMP
	...
	{
		FString CompressedFormatsString = "  GL_COMPRESSED_TEXTURE_FORMATS:";
	...
	UE_LOGF(LogRHI, Log, "OpenGL MajorVersion = %d, MinorVersion = %d, ShaderPlatform = %ls, FeatureLevel = %ls", MajorVersion, MinorVersion, *ShaderPlatformName, *FeatureLevelName);
#if PLATFORM_ANDROID
	UE_LOGF(LogRHI, Log, "PLATFORM_ANDROID");
#endif
...
FOpenGLDynamicRHI::FOpenGLDynamicRHI()
{
...

	{
		...
		{
#if PLATFORM_ANDROID
			if(!AndroidThunkCpp_IsXRImmersiveHMDApplication())
#endif
			{
				CVar->Set(false);
			}
	...
	if (GRHIVendorId == 0x5143)
	{
		...
		if (CVar)
		{
			...
			{
				CVar->Set(false);
				UE_LOGF(LogRHI, Log, "GPU particles are disabled on this device because the driver version %d is less than 415", DriverVersion);
			}
		}
	}
#endif
...
static bool VerifyCompiledShader(GLuint Shader, const ANSICHAR* GlslCode, bool IsFatal)
{
	...
	if (!GOpenGLShaderHackLastCompileSuccess)
	{
		...
		if (GlslCode)
		{
			UE_LOGF(LogRHI, Warning, "Shader:\n%ls", ANSI_TO_TCHAR(GlslCode));
			...
			for (int i = 0; i < 30 && (*Temp != '\0'); ++i)
			{
				...
				UE_LOGF(LogRHI, Display, "%ls", *Converted);
				Temp += Converted.Len();
			}
		}
#endif
		UE_LOGF(LogRHI, Warning, "Failed to compile shader.");
...
void FOpenGLProfiler::Setup()
{
	...
	{
		UE_LOGF(LogOpenGL, Warning, "OpenGL RHI profiler not supported.");
	}
	else
	{
		...
		if (bEnabled)
		{
			UE_LOGF(LogOpenGL, Display, "OpenGL RHI profiler detected reliable timestamps. GPU profiler is enabled. GPU clock: 0x%llx, CPU Clock: 0x%llx", GPU, CPU);
		}
		else
		{
			UE_LOGF(LogOpenGL, Warning, "OpenGL RHI profiler detected unreliable timestamps. GPU profiler is disabled. GPU clock: 0x%llx, CPU Clock: 0x%llx", GPU, CPU);
		}
	}
...
void FOpenGLProfiler::EndFrame(FRHIEndFrameArgs const& Args)
{
	if (bEnabled)
	{
	...
	Current->FrameBoundaryEvent = &EmplaceEvent<UE::RHI::GPUProfiler::FEvent::FFrameBoundary>(
		ERHIPipeline::Graphics,
		Args,
		NewFrame->Calibration.CPUTimestamp
	);
