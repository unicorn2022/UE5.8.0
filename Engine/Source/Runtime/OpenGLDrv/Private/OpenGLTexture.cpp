	...
	, bMultisampleRenderbuffer(EnumHasAnyFlags(InDesc.Flags, TexCreate_Memoryless) && InDesc.NumSamples > 1)
{
	...
	{
		UE_CLOGF(!bCanCreateAsEvicted, LogRHI, Warning, "CanDeferTextureCreation:%d, Flags:%x Mask:%x, Target:%x",
	...
	, bMultisampleRenderbuffer(Other.bMultisampleRenderbuffer)
{
	...
	RHICmdList.EnqueueLambda([this, &Other](FRHICommandListImmediate&)
	{
	...
	RHICmdList.EnqueueLambda([this, HardwareBuffer](FRHICommandListBase& CmdList)
		{
			...
			{
				UE_LOGF(LogRHI, Warning, "eglGetNativeClientBufferANDROID - Could not get native client buffer!");
				return;
			}
			...
			if (EglImage == nullptr)
			{
				UE_LOGF(LogRHI, Warning, "eglCreateImageKHR - Could not create EGLimage from native client buffer! B=0x%llx E=0x%x", NativeClientBuffer, eglGetError());
				return;
			}
...
void FOpenGLDynamicRHI::InitializeGLTextureInternal(FOpenGLTexture* Texture, void const* BulkDataPtr, uint64 BulkDataSize)
{
	...
	{
		UE_LOGF(LogRHI, Fatal,"Texture format '%ls' not supported (sRGB=%d).", GPixelFormats[Desc.Format].Name, bSRGB);
	}
...
void* FOpenGLTexture::Lock(uint32 InMipIndex,uint32 ArrayIndex,EResourceLockMode LockMode,uint32& DestStride)
{
	...
	{
		check(ArrayIndex == 0);
		EvictionParamsPtr->SetMipData(InMipIndex, nullptr, MipBytes);
...
void FOpenGLTexture::Unlock(uint32 MipIndex,uint32 ArrayIndex)
{
	...
	{
		UE_LOGF(LogRHI, Warning, "Converting texture from PF_FloatRGBA to RGBA8!  Only supported for limited cases of 0.0 to 1.0 values (clamped)");
...
bool FOpenGLTextureDesc::CanDeferTextureCreation()
{
	...
	if (!bDeferTextureCreationConfigRulesChecked)
	{
		...
		if (ConfigRulesDeferOpenGLTextureCreationStr)
		{
			...
			UE_LOGF(LogRHI, Log, "OpenGL deferred texture creation, set by config rules: %d", (int)bConfigRulesCanDeferTextureCreation.GetValue());
		}
		else
		{
			UE_LOGF(LogRHI, Log, "OpenGL deferred texture creation, no config rule set: %d", (int)bCanDeferTextureCreation);
		}
...
void FOpenGLDynamicRHI::RHICopyTexture(FRHITexture* SourceTextureRHI, FRHITexture* DestTextureRHI, const FRHICopyTextureInfo& CopyInfo)
{
	...
	if (DestTexture->IsEvicted()
		&& SourceTexture->EvictionParamsPtr.IsValid()
		&& SourceTexture->GetFormat() == DestTexture->GetFormat()
		&& CopyInfo.SourceSliceIndex == 0
		&& CopyInfo.DestSliceIndex == 0
		&& CopyInfo.NumSlices == 1
		&& CopyInfo.SourcePosition.IsZero()
		&& CopyInfo.DestPosition.IsZero())
	{
		// Both source and destination textures are candidates for eviction.
		// We might be able to achieve the copy entirely offline by copying the CPU-side mip data, rather than involving the GPU, leaving the destination evicted.

		// Check the dimensions of the copy are compatible
		FUintVector3 SrcMipExtent = UE::RHITextureUtils::CalculateMipExtents(SourceTexture->GetDesc(), CopyInfo.SourceMipIndex);
		FUintVector3 DstMipExtent = UE::RHITextureUtils::CalculateMipExtents(DestTexture->GetDesc(), CopyInfo.DestMipIndex);

		if (SrcMipExtent == DstMipExtent && (CopyInfo.Size.IsZero() || CopyInfo.Size == FIntVector3(SrcMipExtent)))
		{
			// Check all the required mips from the source are available to copy from
			bool bRequiredSrcMipsPresent = true;
			const TArray<TArray<uint8>>& SrcMipData = SourceTexture->EvictionParamsPtr->MipImageData;

			for (uint32 Mip = CopyInfo.SourceMipIndex, Index = 0; Index < CopyInfo.NumMips; ++Mip, ++Index)
			{
				if (!SrcMipData.IsValidIndex(Mip) || SrcMipData[Mip].IsEmpty())
				{
					bRequiredSrcMipsPresent = false;
					break;
				}
			}

			if (bRequiredSrcMipsPresent)
			{
				// We're replacing entire mips in the destination with copies from the source.
				// Clone the offline mip data directly from the source.
				DestTexture->EvictionParamsPtr->CloneMipData(*SourceTexture->EvictionParamsPtr, CopyInfo.NumMips, CopyInfo.SourceMipIndex, CopyInfo.DestMipIndex);

				// Leave the destination texture evicted.
				return;
			}
		}
	}
...
FTextureRHIRef FOpenGLDynamicRHI::RHICreateTexture2DFromAndroidHardwareBuffer(FRHICommandListBase& RHICmdList, AHardwareBuffer* HardwareBuffer)
{
	...
	// UE_LOGF(LogRHI, Log, "[Media] hardware buffer format: 0x%x", HardwareBufferDesc.format & 0xff);
...
void LogTextureEvictionDebugInfo()
{
	...
	{
		UE_LOGF(LogRHI, Warning, "txdbg: Texture mipmem %d. GTotalTexStorageSkipped %d, GTotalCompressedTexStorageSkipped %d, Total noncompressed = %d", GTotalEvictedMipMemStored, GTotalTexStorageSkipped, GTotalCompressedTexStorageSkipped, GTotalTexStorageSkipped - GTotalCompressedTexStorageSkipped);
		UE_LOGF(LogRHI, Warning, "txdbg: Texture GTotalEvictedMipMemDuplicated %d", GTotalEvictedMipMemDuplicated);
		UE_LOGF(LogRHI, Warning, "txdbg: Texture GTotalMipRestores %d, GTotalMipStoredCount %d", GTotalMipRestores, GTotalMipStoredCount);
		UE_LOGF(LogRHI, Warning, "txdbg: Texture GAvgRestoreTime %f (%d), GMaxRestoreTime %f, TotalRestoreTime %f", GAvgRestoreCount ? (float)(GAvgRestoreTime / GAvgRestoreCount) : 0.f, GAvgRestoreCount, (float)GMaxRestoreTime, (float)GAvgRestoreTime);
		UE_LOGF(LogRHI, Warning, "txdbg: Texture LRU %d", FTextureEvictionLRU::Get().Num());
