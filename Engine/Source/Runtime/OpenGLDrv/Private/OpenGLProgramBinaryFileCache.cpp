	...
	namespace OpenGL
	{
		...
		struct FBinaryCacheFileHeader
		{
			...
			static FBinaryCacheFileHeader CreateHeader(const FGuid& BinaryCacheGuidIn, uint32 NumPrograms, uint32 ValidSize)
			{
				...
				NewHeader.CapabilitiesCRC = GetCapabilitiesCRC();
			...
			bool IsValid(const FGuid* OptionalWantedGuid) const
			{
				return IsValidVersion()
					&& CapabilitiesCRC == GetCapabilitiesCRC()
			...
			{
				return FString::Format(TEXT("{0}, {1}, {2}, {3}, {4}"), { Version, bCacheUsesCompressedBinaries, *BinaryCacheGuid.ToString(), CapabilitiesCRC, ProgramCount });
			}
		};
	}
}
...
class FOpenGLProgramBinaryMapping : public FRefCountedObject
{
public:
	...
	, CurrentBinaryFileState(EBinaryFileState::Uninitialized)
{
	...
	for(FString& FoundFile : FoundFiles)
	{
		...
		if (FoundFile != CacheSubDir || !bIsDir)
		{
			...
			UE_LOGF(LogRHI, Verbose, "FOpenGLProgramBinaryCache Deleting %ls %ls", bIsDir ? TEXT("dir") : TEXT("file"), *FullPath );
			UE_CLOGF(!bSuccess, LogRHI, Warning, "FOpenGLProgramBinaryCache Failed to delete %ls", *FullPath);
		}
	}
}
...
void FOpenGLProgramBinaryCache::Initialize()
{
	...
	{
		UE_LOGF(LogRHI, Log, "FOpenGLProgramBinaryCache disabled by r.ProgramBinaryCache.Enable=0");
		return;
	}

	if (!FOpenGL::SupportsProgramBinary())
	{
		UE_LOGF(LogRHI, Warning, "FOpenGLProgramBinaryCache disabled as devices does not support program binaries");
		return;
	}
	...
	if (FOpenGL::HasBinaryProgramRetrievalFailed())
	{
		if (FOpenGL::SupportsProgramBinary())
		{
			UE_LOGF(LogRHI, Warning, "FOpenGLProgramBinaryCache: Device has failed to emit program binary despite SupportsProgramBinary == true. Disabling binary cache.");
			return;
		}
	}
#endif
	...
	// Remove entire ProgramBinaryCache folder if either -ClearOpenGLBinaryProgramCache or -clearPSODriverCache are specified on command line
	if (FParse::Param(FCommandLine::Get(), TEXT("ClearOpenGLBinaryProgramCache")) || FParse::Param(FCommandLine::Get(), TEXT("clearPSODriverCache")))
	{
		UE_LOGF(LogRHI, Log, "ClearOpenGLBinaryProgramCache specified, deleting binary program cache folder: %ls", *CacheFolderPathRoot);
	...
	{
		UE_LOGF(LogRHI, Log, "Moving program binary cache: %ls -> %ls", *OldCacheFolderPathRoot, *CacheFolderPathRoot);
	...
	UE_LOGF(LogRHI, Log, "Enabling program binary cache dir at %ls", *CachePtr->GetProgramBinaryCacheDir());
	...
	else
	{
		...
		UE_LOGF(LogRHI, Log, "FOpenGLProgramBinaryCache will be initialized when ShaderPipelineCache opens its file");
	...
	namespace OpenGL
	{
		static bool IsBinaryCacheValid(const FString& CachePath, const FGuid* OptionalGuidCheck, bool bLogWhenInvalid, UE::OpenGL::FBinaryCacheFileHeader* HeaderOUT)
		{
			...
			if (BinaryProgramReader.IsValid())
			{
				...
				UE_CLOGF(bLogWhenInvalid, LogRHI, Warning, "FOpenGLProgramBinaryCache: %ls is an invalid binary cache (%ls)", *CachePath, *BinaryCacheHeader.ToString());
			}
			else
			{
				UE_CLOGF(bLogWhenInvalid, LogRHI, Warning, "FOpenGLProgramBinaryCache: %ls not found.", *CachePath);
			}
...
{
	UE_LOGF(LogRHI, Log, "FOpenGLProgramBinaryCache: using precache system.");

	FScopeLock Lock(&GProgramBinaryFileCacheCS);

	const FString PreCacheName("PrecacheBinaries");
	...
	UE_LOGF(LogRHI, Log, "InitPrecaching : Beginning new cache = %ls", *GShaderCachePrecompileContext.GetCacheName());
...
void FOpenGLProgramBinaryCache::UpdatePrecacheMapping()
{
	...
	bool bBinaryCacheFileInitiallyValid = !(DestAr.IsError() || DestAr.IsCriticalError());

	static bool bOneTime = true;
	UE_CLOGF(!bBinaryCacheFileInitiallyValid && bOneTime, LogRHI, Error, "UpdatePrecacheMapping : archive failed (%d, %d)", DestAr.IsError(), DestAr.IsCriticalError());
	bOneTime = bOneTime && bBinaryCacheFileInitiallyValid;

	if (bBinaryCacheFileInitiallyValid && DestPos >= CurrentShaderPipelineProperties.LastMappedPosition + (GBinaryCacheMMapAfterEveryMB * 1024 * 1024))
	{
		...
		bool bSuccess = BinaryProgramReader.IsValid();
		if(bSuccess)
		{
			...
			bSuccess = ReadProgramFile_Internal(ProgramsToRead, DestPos, GetProgramBinaryCacheFilePath(), *BinaryProgramReader);
			if(bSuccess)
			{
			...
			{
				UE_LOGF(LogRHI, Log, "UpdatePrecacheMapping : failed to re-read from program cache.");
			}
		}
		else
		{
			UE_LOGF(LogRHI, Log, "UpdatePrecacheMapping : Mapping failed to open %ls", *GetProgramBinaryCacheFilePath());
		}
...
void FOpenGLProgramBinaryCache::OnShaderPipelineCacheOpened(FString const& Name, EShaderPlatform Platform, uint32 Count, const FGuid& VersionGuid, FShaderPipelineCache::FShaderCachePrecompileContext& ShaderCachePrecompileContext)
{
	...
	{		
		...
		UE_LOGF(LogRHI, Verbose, "OnShaderPipelineCacheOpened, Ignoring empty PSO cache. %ls (%ls)", *CurrentShaderPipelineProperties.PipelineCacheName, *CurrentShaderPipelineProperties.CacheVersionGuid.ToString());
		return;
	}
	 
	UE_LOGF(LogRHI, Log, "Scanning Binary program cache, using Shader Pipeline Cache %ls (%ls)", *CurrentShaderPipelineProperties.PipelineCacheName, *CurrentShaderPipelineProperties.CacheVersionGuid.ToString());
...
void FOpenGLProgramBinaryCache::OnShaderPipelineCachePrecompilationComplete(uint32 Count, double Seconds, const FShaderPipelineCache::FShaderCachePrecompileContext& ShaderCachePrecompileContext)
{
	...
	UE_LOGF(LogRHI, Log, "OnShaderPipelineCachePrecompilationComplete: %ls(%ls) - %ls %d program binaries (%d requested)", *CurrentShaderPipelineProperties.PipelineCacheName, *CurrentShaderPipelineProperties.CacheVersionGuid.ToString(), CacheStatusText, ProgramsCached, Count );
	...
	if (bIsBuildingCache)
	{
		...
		if(bSuccess)
		{
			...
			if (IsBuildingCache_internal())
			{
				UE_LOGF( LogRHI, Error, "Failed to load just completed cache! : %ls(%ls)", *CurrentShaderPipelineProperties.PipelineCacheName, *CurrentShaderPipelineProperties.CacheVersionGuid.ToString());
...
void FOpenGLProgramBinaryCache::ScanProgramCacheFile()
{
	...
	UE_LOGF(LogRHI, Log, "OnShaderScanProgramCacheFile");
	...
	{
		UE_LOGF(LogRHI, Log, "OnShaderScanProgramCacheFile : %ls was not found, recreating.", *ProgramCacheFilename);
	}
	else
	{
		UE::OpenGL::FBinaryCacheFileHeader BinaryCacheHeader;
		...
		{
			UE_LOGF(LogRHI, Warning, "ScanProgramCacheFile - invalid binary cache file encountered (%ls, %lld). Rebuilding binary program cache.", *BinaryCacheHeader.ToString(), Ar.TotalSize());
			BinaryProgramReader->Close();
			PlatformFile.DeleteFile(*ProgramCacheFilenameTemp);
		}
		else
	...
	if (!bBinaryFileIsValid)
	{
		...
		else
		{
			...
			UE_LOGF(LogRHI, Fatal, "ScanProgramCacheFile - Failed to open binary cache.");
		}
	}
	else
	{
...
bool FOpenGLProgramBinaryCache::ReadProgramFile_Internal(uint32 ProgramsToRead, int64 EndOffset, const FString& ProgramCacheFilename, FArchive& Ar)
{
	...
	UE_LOGF(LogRHI, Log, "OnShaderScanProgramCacheFile : %ls %ls", CurrentMapping->HasValidMapping() ? TEXT("mapped") : TEXT("opened"), *ProgramCacheFilename);
	...
	while (Ar.Tell() < EndOffset)
	{
		...
		if (ensure(ProgramBinarySize > 0))
		{
			...
			UE_LOGF(LogRHI, VeryVerbose, " scan found PSO %ls - %d", *ProgramKey.ToString(), ProgramBinarySize);
	...
	UE_LOGF(LogRHI, VeryVerbose, "Program Binary cache: Found %d cached programs", ProgramIndex);
	UE_CLOGF(ProgramIndex != ProgramsToRead, LogRHI, Error, "Program Binary cache: Mismatched program count! expected: %d", ProgramsToRead);
	return true;
}
...
bool FOpenGLProgramBinaryCache::OpenCacheWriteHandle(const FString& ProgramCacheFilenameToWrite, bool bAppendToExisting)
{
	...
	UE_CLOGF(BinaryCacheWriteFileHandle, LogRHI, Log, "Opened binary cache for write (%ls)", *ProgramCacheFilenameToWrite);
	UE_CLOGF(BinaryCacheWriteFileHandle == nullptr, LogRHI, Warning, "Failed to open OGL binary cache output file. (%ls)", *ProgramCacheFilenameToWrite);
	UE_CLOGF(BinaryCacheWriteFileHandle && (BinaryCacheWriteFileHandle->IsError() || BinaryCacheWriteFileHandle->IsCriticalError()), LogRHI, Error, "OGL binary cache output archive error (%ls, %d,%d)", *ProgramCacheFilenameToWrite, BinaryCacheWriteFileHandle->IsError(), BinaryCacheWriteFileHandle->IsCriticalError());
...
bool FOpenGLProgramBinaryCache::MarkValidContent(int32 NumPrograms)
{
	...
	if (FailCode == 0)
	{
		...
		UE_LOGF(LogRHI, Verbose, "MarkValidContent, file valid at %d Programs, %lld bytes.", NumPrograms, CurrPos);
		Ar << OutHeader;
		Ar.Seek(CurrPos);
		Ar.Flush();
	...
	if (FailCode != 0)
	{
		...
		UE_LOGF(LogRHI, Fatal, "MarkValidContent - FArchive error bit set, failed to write binary cache. %d, %d", NumPrograms, FailCode);
	}
...
bool FOpenGLProgramBinaryCache::CloseCacheWriteHandle(int32 NumProgramsAdded)
{
	...
	{
		RHIGetPanicDelegate().ExecuteIfBound(FName("FailedBinaryProgramArchiveWrite"));
		UE_LOGF(LogRHI, Fatal, "CloseCacheWriteHandle - FArchive error bit set, failed to write binary cache. %d", FailCode);
	}
...
void FOpenGLProgramBinaryCache::AddProgramBinaryDataToBinaryCache(const FOpenGLProgramKey& ProgramKey, const FOpenGLProgramBinary& BinaryProgramData)
{
	...
	FArchive& Ar = *BinaryCacheWriteFileHandle;
	const bool bBinaryCacheFileInitiallyValid = !(Ar.IsError() || Ar.IsCriticalError());
	...
	const bool bBinaryCacheValidAfterwards = !(Ar.IsError() || Ar.IsCriticalError());
	UE_CLOGF(!bBinaryCacheValidAfterwards, LogRHI, Error, "AddProgramBinaryDataToBinaryCache : archive failed (%d, %d, %d, %d, %d, %d, %d)", Ar.IsError(), Ar.IsCriticalError(), Start, End, ProgramBinarySize, bBinaryCacheFileInitiallyValid, FPlatformMisc::GetLastError());
	UE_LOGF(LogRHI, VeryVerbose, "AddProgramBinaryDataToBinaryCache : added %d bytes to cache", End-Start);
	...
	if (UE::OpenGL::AreBinaryProgramsCompressed())
	{
		...
		UE_LOGF(LogRHI, Verbose, "AddProgramBinaryDataToBinaryCache: total Uncompressed: %d, total Compressed %d, Total saved so far: %d", TotalUncompressed, TotalCompressed, TotalUncompressed - TotalCompressed);
	}
	UE_LOGF(LogRHI, VeryVerbose, "AddProgramBinaryDataToBinaryCache: written Program %ls to cache (%d bytes)", *ProgramKey.ToString(), BinaryProgramDataView.Num());

	ProgramsInCurrentCache.Add(ProgramKey);

 	if (IsPrecachingEnabled() && bBinaryCacheValidAfterwards)
 	{
...
void FOpenGLProgramBinaryCache::CheckPendingGLProgramCreateRequests_internal()
{
	if (PendingGLContainerPrograms.Num() > 0)
	{
		...
		UE_LOGF(LogRHI, Verbose, "CheckPendingGLProgramCreateRequests : iter count = %d, time taken = %f ms (remaining %d)", Count, TimeTaken, PendingGLContainerPrograms.Num());
	}
}
