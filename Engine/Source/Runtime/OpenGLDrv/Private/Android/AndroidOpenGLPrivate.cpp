...
FAndroidGPUInfo::FAndroidGPUInfo()
{
	...
	EGL->SetCurrentRenderingContext(nullptr); // a windowed surface is not required here.
...
void FAndroidGPUInfo::ReadGPUFamily()
{
	...
	{
		UE_LOGF(LogAndroid, Log, "FAndroidGPUInfo ANGLE detected with GL_RENDERER string %ls", *GPUFamily);
		GPUFamily = FAndroidMisc::GetVulkanDeviceNameString();
		UE_LOGF(LogAndroid, Log, "FAndroidGPUInfo Using Vulkan deviceName string %ls", *GPUFamily);
	}
	...
	if (ArmGPUInfoInstance)
	{
		...
		if (RegexMatcher.FindNext() && ArmGPUInfo.num_shader_cores > 0)
		{
			...
			if (Capture.IsEmpty())
			{
				...
				UE_LOGF(LogAndroid, Log, "FAndroidGPUInfo renaming GPUFamily: %ls -> %ls", *GPUFamily, *ARMLibName);
				GPUFamily = ARMLibName;
			}
			else if ((uint32)FCString::Atoi64(*Capture) != ArmGPUInfo.num_shader_cores)
			{
				UE_LOGF(LogAndroid, Warning, "FAndroidGPUInfo GPUFamily core count mismatch: %ls, expected MP%d", *GPUFamily, ArmGPUInfo.num_shader_cores);
			}
		}
	}
}
