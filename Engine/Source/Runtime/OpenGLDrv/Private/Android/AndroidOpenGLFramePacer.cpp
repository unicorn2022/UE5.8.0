...
bool FAndroidOpenGLFramePacer::SupportsFramePaceInternal(int32 QueryFramePace, int32& OutRefreshRate, int32& OutSyncInterval)
{
	...
	if (FAndroidPlatformRHIFramePacer::CVarUseSwappyForFramePacing.GetValueOnAnyThread() == 1)
	{
		...
		UE_LOGF(LogRHI, Log, "FAndroidOpenGLFramePacer -> Supported Refresh Rates:%ls", *RefreshRatesString);
		...
		if (FAndroidPlatformRHIFramePacer::CVarSupportNonVSyncMultipleFrameRates.GetValueOnAnyThread() == 1)
		{
			for (int32 Rate : RefreshRates)
			{
				...
				{
					UE_LOGF(LogRHI, Log, "Supports %d using refresh rate %d with naive frame pacing", QueryFramePace, Rate);
					OutRefreshRate = Rate;
					OutSyncInterval = 0;
					return true;
				}
...
extern bool AndroidThunkCpp_IsStandaloneApplication();

bool FAndroidOpenGLFramePacer::SwapBuffers(FPlatformOpenGLSurfaceContext* SurfaceContext, bool bLockToVsync)
{
	...
	EGLSurface eglSurface = SurfaceContext->eglSurface;
	int32 SyncInterval = FAndroidPlatformRHIFramePacer::GetLegacySyncInterval();

	if (SurfaceContext->IsUsingWindowedSurface() == false && !AndroidThunkCpp_IsXRImmersiveHMDApplication() && !AndroidThunkCpp_IsStandaloneApplication())
	{
	...
	{
		ANativeWindow* CurrentNativeWindow = SurfaceContext->Window;
		if (CurrentNativeWindow != CachedNativeWindow)
		{
			CachedNativeWindow = CurrentNativeWindow;
			UE_LOGF(LogRHI, Verbose, "Swappy - setting native window %p", CachedNativeWindow);
...
#endif
	{
		if (DesiredSyncIntervalRelativeTo60Hz != SyncInterval)
		{
			...
			if (eglGetCompositorTimingANDROID_p)
			{
				...
				UE_LOGF(LogRHI, Log, "AndroidEGL:SwapBuffers eglGetCompositorTimingANDROID EGL_COMPOSITE_DEADLINE_ANDROID=%lld, EGL_COMPOSITE_INTERVAL_ANDROID=%lld, EGL_COMPOSITE_TO_PRESENT_LATENCY_ANDROID=%lld",
			...
			UE_LOGF(LogRHI, Log, "JNI Display getRefreshRate=%f",
				RefreshRate
			);
			...
			UE_LOGF(LogRHI, Log, "Final display timing metrics: DriverRefreshRate=%7.4f  DriverRefreshNanos=%lld",
				DriverRefreshRate,
				DriverRefreshNanos
			);
			...
			UE_LOGF(LogRHI, Log, "AndroidEGL:SwapBuffers Min=%d, Max=%d, Request=%d, ClosestDriver=%d, SetDriver=%d", MinSwapInterval, MaxSwapInterval, DesiredSyncIntervalRelativeTo60Hz, DesiredSyncIntervalRelativeToDevice, DesiredDriverSyncInterval);
			...
			if (DesiredDriverSyncInterval != DriverSyncIntervalRelativeToDevice)
			{
				...
				UE_LOGF(LogRHI, Log, "Called eglSwapInterval %d", DesiredDriverSyncInterval);
		...
		{
			{
				UE_CLOGF(bPrintMethod, LogRHI, Display, "Using niave method for frame pacing (possible with timestamps method)");
		...
		{
			UE_CLOGF(bPrintMethod, LogRHI, Display, "Using eglGetFrameTimestampsANDROID method for frame pacing");
			...
			{
				//UE_CLOGF(bPrintOnce, LogRHI, Log, "eglGetFrameTimestampsSupportedANDROID retured true for EGL_FIRST_COMPOSITION_START_TIME_ANDROID");
				...
				else
				{
					...
					{
						UE_LOGF(LogRHI, Log, "eglGetFrameTimestampsANDROID_p failed for %d consecutive frames, reverting to naive frame pacer.", GGetTimeStampsRetryCount);
					}
				}
			}
			else
			{
				//UE_CLOGF(bPrintOnce, LogRHI, Log, "eglGetFrameTimestampsSupportedANDROID doesn't exist or retured false for EGL_FIRST_COMPOSITION_START_TIME_ANDROID, discarding eglGetNextFrameIdANDROID_p and eglGetFrameTimestampsANDROID_p");
			}
			//bPrintOnce = false;
		}
		...
		if (DesiredSyncIntervalRelativeToDevice > 0 && eglGetFrameTimestampsANDROID_p && FAndroidPlatformRHIFramePacer::CVarSpewGetFrameTimestamps.GetValueOnAnyThread())
		{
			...
			UE_CLOGF(FAndroidPlatformRHIFramePacer::CVarSpewGetFrameTimestamps.GetValueOnAnyThread() > 1, LogRHI, Log, "************************************  frame %d   base time is %lld", NextFrameIDSlot - 1, FirstRealValue);

			for (int32 Index = int32(NextFrameIDSlot) - NUM_FRAMES_TO_MONITOR; Index < int32(NextFrameIDSlot); Index++)
			{

				UE_CLOGF(FAndroidPlatformRHIFramePacer::CVarSpewGetFrameTimestamps.GetValueOnAnyThread() > 1, LogRHI, Log, "eglGetFrameTimestampsANDROID_p  frame %d", Index);
				for (int32 IndexInner = 0; IndexInner < 9; IndexInner++)
				{
					...
					UE_CLOGF(FAndroidPlatformRHIFramePacer::CVarSpewGetFrameTimestamps.GetValueOnAnyThread() > 1, LogRHI, Log, "     %8d    %ls", MsVal, TimestampStrings[IndexInner]);
				}
			}
			...
			if (Results[IndexLast % NUM_FRAMES_TO_MONITOR][3] > 1 && Results[IndexLastNext % NUM_FRAMES_TO_MONITOR][3] > 1)
			{
				...
				if (NumRecordedFrameInterval == 100)
				{
					...
					UE_LOGF(LogRHI, Log, "%3d fast  %3d ok  %3d slow   %ls", NumBelowTarget, NumOnTarget, NumAboveTarget, *All);
