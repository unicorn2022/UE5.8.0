...
static bool GbInjectFragmentControlFlowHint = false;
static FAutoConsoleVariableRef CVarInjectFragmentControlFlowHint(
	TEXT("android.OpenGL.InjectFragmentControlFlowHint"),
	GbInjectFragmentControlFlowHint,
	TEXT("true - Injects a benign control flow branch into the fragment shader to trigger alternative code generation with some compilers.\n")
	TEXT("false - fragment shaders are unchanged. (default)")
	,
	ECVF_ReadOnly
);
...
public:
	...
	void MarkCacheMissedTime(const FOpenGLProgramKey& ProgramKey, bool bLogOnFirstUse)
	{
		...
		if (!ProgramTimes.UsedTime)
		{
			...
			{
				TotalMisses++;
				UE_CLOGF(bLogOnFirstUse, LogRHI, Log, "BinaryCacheUsage: Program %ls was not in the binary cache when first used.", *ProgramKey.ToString());
			}
			else if (!ProgramTimes.CompileTime)
			{
				...
				UE_CLOGF(bLogOnFirstUse, LogRHI, Log, "BinaryCacheUsage: Program %ls was used too early, binary compile was not ready when first used. Span between compile and use: %f", *ProgramKey.ToString(), (float)TimeToUse);
			}
		}
	}
	...
	void LogStats()
	{
		...
		UE_CLOGF(bLogMe, LogRHI, Log, "BinaryCacheUsage: %d programs seen, %d preloaded, %d used in time, %d used before compile finished (avg early miss time span %f), %d programs used were not in the cache. %f avg compile time",
			KeyToTimes.Num(),
			PreloadedBinaries,
			TotalHits,
			TotalEarlyUses,
...
static bool VerifyLinkedProgram(GLuint Program)
{
	...
	if (LinkStatus != GL_TRUE)
	{
		if (ReportProgramLinkFailures())
		{
			...
			UE_LOGF(LogRHI, Error, "Failed to link program. Current total programs: %d program binary bytes, last gl error 0x%X, drvalloc %d\n  log:\n%ls",
				GNumPrograms,
				LastGLError,
		...
		{
			UE_LOGF(LogRHI, Error, "Failed to link program. Current total programs:%d", GNumPrograms);
		}
...
public:
	...
	void CompressShader(const TArray<ANSICHAR>& InGlslCode)
	{
		...
		StatTotalUncompressedSize += UncompressedSize == -1 ? GlslCode.Num() : UncompressedSize;
 		...
 		//UE_LOGF(LogRHI, Warning, "Shader sizes: %d %d", StatTotalStoredSize.Load(EMemoryOrder::Relaxed), StatTotalUncompressedSize.Load(EMemoryOrder::Relaxed));
	}
...
namespace
{
	...
	inline void ReplaceCString(TArray<ANSICHAR> & Dest, const ANSICHAR * Source, const ANSICHAR * Replacement)
	{
		...
		for (const ANSICHAR * FoundPointer = FCStringAnsi::Strstr(Dest.GetData(), Source);
			...
			FoundPointer = FCStringAnsi::Strstr(Dest.GetData()+FoundIndex+ReplacementLen, Source))
		{
	...
	SCOPE_CYCLE_COUNTER(STAT_OpenGLShaderCompileVerifyTime);
	{
		...
		if (CompileStatus != GL_TRUE)
		{
			if (ReportShaderCompileFailures())
			{
				...
				if (GlslCodeString)
				{
					UE_LOGF(LogRHI,Error,"Shader:\n%ls", ANSI_TO_TCHAR(GlslCodeString));
				}

				UE_LOGF(LogRHI,Error,"Failed to compile shader. Compile log:\n%ls", ANSI_TO_TCHAR(CompileLog));
				if (LogLength > 1)
				{
					FMemory::Free(CompileLog);
				}
...
void OPENGLDRV_API GetCurrentOpenGLShaderDeviceCapabilities(FOpenGLShaderDeviceCapabilities& Capabilities)
{
	...
	if (FOpenGL::IsAndroidGLESCompatibilityModeEnabled())
	{
		...
		Capabilities.bRequiresControlFlowOptimizerWorkaround = GbInjectFragmentControlFlowHint;
#elif PLATFORM_IOS
...
void OPENGLDRV_API GLSLToDeviceCompatibleGLSL(FAnsiCharArray& GlslCodeOriginal, const FString& ShaderName, GLenum TypeEnum, const FOpenGLShaderDeviceCapabilities& Capabilities, FAnsiCharArray& GlslCode)
{
	...
	if (TypeEnum == GL_FRAGMENT_SHADER)
	{
		...
		if (Capabilities.bRequiresControlFlowOptimizerWorkaround)
		{
			ReplaceCString(GlslCodeOriginal, "void main()\n{\n", "void main()\n{\nif (gl_FragCoord.x < 0.0) return;\n");
		}
	}
...
FOpenGLShader::FOpenGLShader(TArrayView<const uint8> Code, GLenum TypeEnum, FShaderResourceTable& SRT, FRHIShader* RHIShader)
{
		...
		)
	{
		UE_LOGF(LogRHI, Fatal,
			"Corrupt shader bytecode. GlslMarker=0x%08x FrequencyMarker=0x%04x",
			Header.GlslMarker,
	...
	FAnsiCharArray GlslCodeFinal;
	{
		...
		else
		{
			...
			{
				UE_LOGF(LogRHI, Fatal, "SHADER CRC CLASH!");
			}
		}
#endif
	}
...
FOpenGLVertexShader::FOpenGLVertexShader(TArrayView<const uint8> Code)
	: FOpenGLShader(Code, GL_VERTEX_SHADER, ShaderResourceTable, this)
{}
...
FOpenGLPixelShader::FOpenGLPixelShader(TArrayView<const uint8> Code)
	: FOpenGLShader(Code, GL_FRAGMENT_SHADER, ShaderResourceTable, this)
{}
...
FOpenGLGeometryShader::FOpenGLGeometryShader(TArrayView<const uint8> Code)
	: FOpenGLShader(Code, GL_GEOMETRY_SHADER, ShaderResourceTable, this)
{}
...
FOpenGLComputeShader::FOpenGLComputeShader(TArrayView<const uint8> Code)
	: FOpenGLShader(Code, GL_COMPUTE_SHADER, ShaderResourceTable, this)
{}
...
FVertexShaderRHIRef FOpenGLDynamicRHI::RHICreateVertexShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return new FOpenGLVertexShader(CreateShaderDesc.Code);
}

FPixelShaderRHIRef FOpenGLDynamicRHI::RHICreatePixelShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return new FOpenGLPixelShader(CreateShaderDesc.Code);
}

FGeometryShaderRHIRef FOpenGLDynamicRHI::RHICreateGeometryShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return new FOpenGLGeometryShader(CreateShaderDesc.Code);
}
...
namespace UE
{
	namespace OpenGL
	{
        static bool CreateGLProgramFromUncompressedBinary(GLuint& ProgramOUT, const TArrayView<const uint8>& ProgramBinary)
        {
	        ...
	        //	UE_LOGF(LogRHI, Warning, "LRU: CreateFromBinary %d, binary format: %x, BinSize: %d", GLProgramName, ((GLenum*)ProgramBinaryPtr)[0], BinarySize - sizeof(GLenum));
...
class FGLProgramCacheLRU
{
	...
	public:
		...
		void RestoreGLProgramFromBinary(const FOpenGLProgramKey& ProgramKey)
		{
			...
			else
			{
				...
				UE_LOGF(LogRHI, Log, "[%ls, %d, %d, crc 0x%X]", *ProgramKey.ToString(), LinkedProgram->Program, GetProgramBinaryView().Num(), ProgramCRC );
				// dump first 32 bytes..
				if (GetProgramBinaryView().Num() >= 32)
				{
					...
					{
						UE_LOGF(LogRHI, Log, "[%d :  0x%08X]", Dump, *MemPtr++);
					}
				}
				RHIGetPanicDelegate().ExecuteIfBound(FName("FailedBinaryProgramCreate"));
				UE_LOGF(LogRHI, Fatal, "RestoreGLProgramFromBinary : Failed to restore GL program from binary data! [%ls]", *ProgramKey.ToString());
			}
		}
	...
	FOpenGLLinkedProgram* FindEvictedAndUpdateLRU(const FOpenGLProgramKey& ProgramKey)
	{
		FOpenGLLinkedProgram* LinkedProgram;
		{
			...
			// UE_LOGF(LogRHI, Warning, "LRU: found and recovered EVICTED program %ls", *ProgramKey.ToString());
	...
	void EvictFromLRU(FOpenGLLinkedProgram* LinkedProgram)
	{
		...
		//UE_LOGF(LogRHI, Warning, "LRU: Evicting program %d", LinkedProgram->Program);
...
public:
	...
	void Add(const FOpenGLProgramKey& ProgramKey, FOpenGLLinkedProgram* LinkedProgram)
	{
		...
		// UE_LOGF(LogRHI, Warning, "LRU: adding program %ls (%d)", *ProgramKey.ToString(), LinkedProgram->Program);
	...
	void AddOrReplaceEvicted(const FOpenGLProgramKey& ProgramKey, TUniqueObj<FOpenGLProgramBinary>&& ProgramBinaryData)
	{
		...
		// UE_LOGF(LogRHI, Warning, "LRU: adding EVICTED program %ls", *ProgramKey.ToString());
	...
	FOpenGLLinkedProgram* Find(const FOpenGLProgramKey& ProgramKey, bool bFindAndCreateEvictedProgram)
	{
		...
		if (Found)
		{
			...
			//UE_LOGF(LogRHI, Warning, "LRU: ::Find program %d exists in LRU!", (*Found)->Program);
			return *Found;
		}
...
public:
	...
	static bool IsUsingLRU()
	{
		if (UseLRUCacheStatus == -1)
		{
			...
			{
				UE_LOGF(LogRHI, Warning, "Requesting OpenGL program LRU cache, but program binary is not supported by driver. Falling back to non-lru cache.");
			}

			UseLRUCacheStatus = CVarEnableLRU.GetValueOnAnyThread() == 1 && FOpenGLProgramBinaryCache::IsEnabled();
			UE_LOGF(LogRHI, Log, "OpenGL program LRU cache active = %d (%d, %d)", UseLRUCacheStatus, CVarEnableLRU.GetValueOnAnyThread(), FOpenGLProgramBinaryCache::IsEnabled());
		}
	...
	: Bindings(Shader.Bindings)
{
	...
	for (int32 SamplerIndex = 0; SamplerIndex < Bindings.NumSamplers; ++SamplerIndex)
	{
		...
		if (Location != -1)
		{
			...
			{
				UE_LOGF(LogShaders, Error, "%ls has a shader using too many textures (idx %d, max allowed %d) at stage %d", *ProgramBase.ProgramKey.ToString(), SamplerIndex, MaxTextureUnit[Stage] - 1, Stage);
				checkNoEntry();
			}
		}
	}
...
#endif //#if ENABLE_UNIFORM_BUFFER_LAYOUT_NAME_MANGLING_CL1862097
{
	...
	if(!Uniforms.Num())
	{
		for (TLinkedList<FShaderParametersMetadata*>::TIterator StructIt(FShaderParametersMetadata::GetStructList()); StructIt; StructIt.Next())
		{
			...
			UE_LOGF(LogRHI, Log, "UniformBufferStruct %ls %ls %d",
...
#if ENABLE_UNIFORM_BUFFER_LAYOUT_DUMP
				UE_LOGF(LogRHI, Log, "  +%d %ls%dx%d %ls[%d]",
					Member.GetOffset(),
					*BaseTypeName,
					Member.GetNumRows(),
	...
	*/
	...
	if(!(RequestedUniformName.StartsWith("Material_") || RequestedUniformName.StartsWith("MaterialCollection")))
	{
		...
		{
			UE_LOGF(LogRHI, Fatal, "uniform buffer member %ls in the GLSL source doesn't match it's declaration in it's FShaderParametersMetadata", *RequestedUniformName);
		}
	}
}
...
#if ENABLE_UNIFORM_BUFFER_LAYOUT_DUMP
	UE_LOGF(LogRHI, Log, "program %d has %d uniform blocks", Program, NumBlocks);
#endif  // #if ENABLE_UNIFORM_BUFFER_LAYOUT_DUMP
...
#if ENABLE_UNIFORM_BUFFER_LAYOUT_DUMP
		UE_LOGF(LogRHI, Log, "  [%d] uniform block (%ls) = %ls, %d active uniforms, %d bytes {",
			BlockIndex,
			*ReferencedBy,
...
#if ENABLE_UNIFORM_BUFFER_LAYOUT_DUMP
				UE_LOGF(LogRHI, Log, "    [%d] +%d %ls %ls %d elements %d array stride",
					UniformIndex,
	...
	: FOpenGLLinkedProgramBase(FOpenGLProgramKey(VertexShader, PixelShader, GeometryShader), 0)
{
	...
	if (!VerifyLinkedProgram(Program))
	{
		...
		//if (VertexShader)
		{
			UE_LOGF(LogRHI, Error, "Vertex Shader:\n%ls", ANSI_TO_TCHAR(VertexShader->GlslCode.GetData()));
		}
		//if (PixelShader)
		{
			UE_LOGF(LogRHI, Error, "Pixel Shader:\n%ls", ANSI_TO_TCHAR(PixelShader->GlslCode.GetData()));
		}
		if (GeometryShader)
		{
			UE_LOGF(LogRHI, Error, "Geometry Shader:\n%ls", ANSI_TO_TCHAR(GeometryShader->GlslCode.GetData()));
		}
#endif //DEBUG_GL_SHADERS
		RHIGetPanicDelegate().ExecuteIfBound(FName("FailedProgramLink"));
		UE_LOGF(LogRHI, Fatal, "Failed to link graphics program [%ls]. Current total programs: %d", *ProgramKey.ToString(), GNumPrograms);
	}
	...
	: FOpenGLLinkedProgramBase(FOpenGLProgramKey(ComputeShader), 0)
{
	...
	{
#if DEBUG_GL_SHADERS
		UE_LOGF(LogRHI, Error, "Compute Shader:\n%ls", ANSI_TO_TCHAR(ComputeShader->GlslCode.GetData()));
#endif //DEBUG_GL_SHADERS
		...
		UE_LOGF(LogRHI, Fatal, "Failed to link compute program [%ls]. Current total programs: %d", *ProgramKey.ToString(), GNumPrograms);
	}
...
void FOpenGLDynamicRHI::LinkComputeProgram(FRHIComputeShader* ComputeShaderRHI)
{
	...
	VERIFY_GL_SCOPE();
	check(ComputeShaderRHI->GetHash() != FShaderHash());
...
FComputeShaderRHIRef FOpenGLDynamicRHI::RHICreateComputeShader(const FRHICreateShaderDesc& CreateShaderDesc)
{
	return new FOpenGLComputeShader(CreateShaderDesc.Code);
}
...
static FOpenGLProgramBinary ExternalProgramCompile(const FOpenGLProgramKey& ProgramKey, FGraphicsPipelineStateInitializer::EPSOPrecacheCompileType PSOCompileType, FRHIVertexShader* VertexShaderRHI, FRHIPixelShader* PixelShaderRHI)
{
	...
	{
		UE_LOGF(LogRHI, Error, "External compile of program %ls failed: %ls ", *ProgramKey.ToString(), *FailLog);
#if DEBUG_GL_SHADERS
		if (VSCode.Num())
		{
			UE_LOGF(LogRHI, Error, "Vertex Shader:\n%ls", ANSI_TO_TCHAR(VSCode.GetData()));
		}
		if (PSCode.Num())
		{
			UE_LOGF(LogRHI, Error, "Pixel Shader:\n%ls", ANSI_TO_TCHAR(PSCode.GetData()));
		}
#endif //DEBUG_GL_SHADERS
	}
#else
...
void FOpenGLDynamicRHI::PrepareGFXBoundShaderState(const FGraphicsPipelineStateInitializer& Initializer)
{
	...
	if (!bIsPreCachePSO || !FOpenGLProgramBinaryCache::IsEnabled() || !bCanCreateExternally)
	{
		...
		{
			UE_LOGF(LogRHI, Warning, "Ignoring precache PSO, external compiler not active.");
			bOneTime = false;
		}
		return;
	}
	...
	if (FOpenGLProgramBinaryCache::RequiresCaching(ProgramKey))
	{
		if (FOpenGLProgramBinaryCache::IsBuildingCache())
		{
			...
			{
				UE_LOGF(LogRHI, Warning, "Program binary generation failed (%ls), omitted from binary cache.", *ProgramKey.ToString());
			}
		}
		else
		{
			...
			UE_LOGF(LogRHI, Warning, "New PSO encountered during precompile %ls", *ProgramKey.ToString());
		}
	}
}
...
void FOpenGLShaderParameterCache::CommitPackedUniformBuffers(FOpenGLLinkedProgram* LinkedProgram, CrossCompiler::EShaderStage Stage, FRHIUniformBuffer** RHIUniformBuffers, const TArray<CrossCompiler::FUniformBufferCopyInfo>& UniformBuffersCopyInfo)
{
	...
	else
	{
		...
		for (int32 BufferIndex = 0; BufferIndex < Bindings.NumUniformBuffers; ++BufferIndex)
		{
			...
			UE_CLOGF(UniformBuffer == nullptr && EmulatedUniformBufferSet.IsValidIndex(BufferIndex), LogRHI, Fatal, "CommitPackedUniformBuffers null UB stage %d, idx %d (%d), %ls", Stage, BufferIndex, EmulatedUniformBufferSet.Num(), *LinkedProgram->ProgramKey.ToString());
#endif
...
// 			FScopeLock Lock(&GProgramBinaryCacheCS);
			...
			else
			{
				if (!bProgramExists)
				{
					...
					if (!bSuccess)
					{
						UE_LOGF(LogRHI, Log, "[%ls, %d, %d]", *ProgramKey.ToString(), GLProgramId, ProgramBinaryData->GetDataView().Num());
						RHIGetPanicDelegate().ExecuteIfBound(FName("FailedBinaryProgramCreateLoadRequest"));
						UE_LOGF(LogRHI, Fatal, "CompleteLoadedGLProgramRequest_internal : Failed to create GL program from binary data! [%ls]", *ProgramKey.ToString());
					}
