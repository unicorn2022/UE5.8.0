...
static void PlatformInitPixelFormatForDevice(HDC DeviceContext, bool bTryIsDummyContext)
{
	...
	{
		UE_LOGF(LogRHI, Fatal, "Failed to set pixel format for device context.");
	}
}
...
void PlatformResizeGLContext(FPlatformOpenGLDevice* Device, FPlatformOpenGLContext* Context, FPlatformOpenGLSurfaceContext* SurfaceContext, uint32 SizeX, uint32 SizeY, bool bFullscreen, bool bWasFullscreen, GLenum BackBufferTarget, GLuint BackBufferResource)
{
	FScopeLock ScopeLock(Device->ContextUsageGuard);
	{
		...
		if (bFullscreen)
		{
			...
			HMONITOR hMonitor = MonitorFromWindow(Context->WindowHandle, MONITOR_DEFAULTTONEAREST);
...
// Create a dummy context so that wglCreateContextAttribsARB can be initialized.
		...
		if (bOpenGLSupported)
		{
			...
			if (!OpenGLDLL)
			{
				UE_LOGF(LogRHI, Fatal, "Couldn't load opengl32.dll");
			}
...
#define CHECK_GL_ENTRYPOINTS(Type,Func) if (Func == NULL) { bFoundAllEntryPoints = false; UE_LOGF(LogRHI, Warning, "Failed to find entry point for %ls", TEXT(#Func)); }
