...
//			checkf(Data != NULL, TEXT("FOpenGL::MapBufferRange Failed, glError %d (0x%x)"), glGetError(), glGetError());
		...
		if (Data == nullptr)
		{
			UE_LOGF(LogRHI, Fatal, "Failed to lock buffer: Resource %u, Size %u, Offset %u, bReadOnly %d, bUseMapBuffer %d, glError (0x%x)", 
				(uint32)Resource, 
				InSize, 
				InOffset, 
				bReadOnly ? 1:0, 
	...
	uint8* LockWriteOnlyUnsynchronized(uint32 InOffset, uint32 InSize, bool bDiscard)
	{
		...
		if (Data == nullptr)
		{
			UE_LOGF(LogRHI, Fatal, "Failed to lock buffer (write only): Resource %u, Size %u, Offset %u, bUseMapBuffer %d, glError (0x%x)", 
				(uint32)Resource, 
				InSize, 
				InOffset, 
				bUseMapBuffer ? 1:0, 
	...
	void ReleaseOwnership()
	{
		...
		if (Resource != 0)
		{
			if (LockBuffer != NULL)
			{
				...
				{
					UE_LOGF(LogRHI, Warning, "Destroying TOpenGLBuffer without returning memory to the driver; possibly called RHIMapStagingSurface() but didn't call RHIUnmapStagingSurface()? Resource %u", Resource);
				}
			}
...
public:
	...
	virtual ~TOpenGLStreamedBufferArray( void ) = default;
...
struct FOpenGLProfiler
{
	...
	void EndFrame(FRHIEndFrameArgs const& Args);
...
public:
	...
	struct FPlatformOpenGLSurfaceContext* GetGLSurfaceContext() const { return OpenGLSurfaceContext; }
...
private:
	...
	FPlatformOpenGLSurfaceContext* OpenGLSurfaceContext;
	uint32 SizeX;
	uint32 SizeY;
	bool bIsFullscreen;
