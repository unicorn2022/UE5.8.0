...
void Linux_ContextMakeCurrent(SDL_HWindow hWnd, SDL_HGLContext hGLDC)
{
	...
	if (Result != 0)
	{
		...
		UE_LOGF(LogLinux, Warning, "SDL_GL_MakeCurrent() failed, SDL error: '%ls'", *SdlError);
	}
}
...
void Linux_PlatformCreateOpenGLContextCore(FPlatformOpenGLContext* OutContext)
{
	...
	if (OutContext->hGLContext == nullptr)
	{
		...
		UE_LOGF(LogInit, Error, "Linux_PlatformCreateOpenGLContextCore - Could not create OpenGL %d.%d context, SDL error: '%ls'",
...
struct FPlatformOpenGLSurfaceContext* PlatformCreateOpenGLSurfaceContext(FPlatformOpenGLContext* SourceContext, void* InWindowHandle)
{
	return nullptr;
}
	...
	bool bLockToVsync)
{
...

	{
		...
		if (Viewport.GetCustomPresent())
		{
			...
			bool bShouldPresent = Viewport.GetCustomPresent()->Present(&Viewport, RHICmdContext, SyncInterval);
		...
		if (bPresent)
		{
			...
			if (Context->SyncInterval != RealSyncInterval)
			{
				...
				UE_LOGF(LogLinux, Log, "Setting swap interval to '%ls'", PlatformDescribeSyncInterval(RealSyncInterval));
				...
				if (SetSwapResult == -1)
				{
					if (RealSyncInterval == -1)
					{
						...
						UE_LOGF(LogLinux, Log, "Unable to set desired swap interval, falling back to '%ls'", PlatformDescribeSyncInterval(FallbackInterval));
				...
				if (SetSwapResult == -1)
				{
					UE_LOGF(LogLinux, Warning, "Unable to set desired swap interval '%ls'", PlatformDescribeSyncInterval(RealSyncInterval));
				}
...
void PlatformResizeGLContext(FPlatformOpenGLDevice* Device,
	...
	struct FPlatformOpenGLSurfaceContext* SurfaceContext,
...
bool PlatformInitOpenGL()
{
	...
	{
		UE_LOGF(LogInit, Error, "PlatformInitOpenGL() : InitSDL() failed, cannot initialize OpenGL.");
		// unreachable
		return false;
	}
	...
	if (!bInitialized)
	{
		...
		{
			UE_LOGF(LogLinux, Fatal, "SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, %d) failed: %ls", MajorVersion, UTF8_TO_TCHAR(SDL_GetError()));
		}

		if (SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, MinorVersion) != 0)
		{
			UE_LOGF(LogLinux, Fatal, "SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, %d) failed: %ls", MinorVersion, UTF8_TO_TCHAR(SDL_GetError()));
		}

		if (SDL_GL_SetAttribute(SDL_GL_CONTEXT_FLAGS, DebugFlag) != 0)
		{
			UE_LOGF(LogLinux, Fatal, "SDL_GL_SetAttribute(SDL_GL_CONTEXT_FLAGS, %d) failed: %ls", DebugFlag, UTF8_TO_TCHAR(SDL_GetError()));
		}

		if (SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE) != 0)
		{
			UE_LOGF(LogLinux, Fatal, "SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE) failed: %ls", UTF8_TO_TCHAR(SDL_GetError()));
		}
		...
		if (FParse::Param(FCommandLine::Get(), TEXT("quad_buffer_stereo")))
		{
			...
			{
				UE_LOGF(LogLinux, Fatal, "SDL_GL_SetAttribute(SDL_GL_STEREO, 1) failed: %ls", UTF8_TO_TCHAR(SDL_GetError()));
			}
		}
		...
		{
			UE_LOGF(LogRHI, Error, "OpenGL %d.%d not supported by driver", MajorVersion, MinorVersion);
		}
...
#define CHECK_GL_ENTRYPOINTS(Type,Func) if (Func == NULL) { bFoundAllEntryPoints = false; UE_LOGF(LogRHI, Fatal, "Failed to find entry point for %ls", TEXT(#Func)); }
