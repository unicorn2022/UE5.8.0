...
static void RemapBuckets()
{
	...
	else
	{
		...
		UE_LOGF(LogRHI,Log,"Configured UBO bucket pool to %d buckets based on alignment of %d bytes", UniformBufferSizeBuckets.Num(), Alignment);
	}
}
...
static void SuballocateUBO( uint32 Size, GLuint& Resource, uint32& Offset, uint8*& Pointer)
{
	...
	UE_LOGF(LogRHI,Log,"Allocated new buffer for uniform Pool %d buffers with %d bytes",UBOPool.Num(), UBOPool.Num()*GetUBOPoolSize());
}
...
void UpdateUniformBufferContents(FOpenGLUniformBuffer* UniformBuffer, const void* Contents, uint32 ConstantBufferSize)
{
	if (ConstantBufferSize > 0)
	{
		...
		{
			UE_LOGF(LogRHI, Fatal, "RHIUpdateUniformBuffer doesn't support PersistentlyMappedBuffer yet!");
		}
		else
		{
