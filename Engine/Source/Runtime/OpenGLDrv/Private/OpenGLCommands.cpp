				...
				if(TextureStage.LimitMip != -1)
				{
					for( int32 TexIndex = 0; TexIndex <= MaxProgramTexture; ++TexIndex )
					{
						if(TexIndex != TextureStageIndex && ShaderState->NeedsTextureStage(TexIndex))
						{
							...
							{
								UE_LOGF(LogRHI, Warning, "Texture SRV fallback requires that each texture SRV be bound with the same mip-range restrictions. Expect rendering errors.");
							}
						}
					}
				}
			...
			else
			{
				if( bABlendWasSet )
				{
					...
					if( CachedRenderTargetBlendState.bSeparateAlphaBlendEnable != RenderTargetBlendState.bSeparateAlphaBlendEnable
						...
						UE_LOGF(LogRHI, Fatal, "OpenGL state on draw requires setting different blend operation or factors to different render targets. This is not supported on Mac OS X!");
				}
				else
				{
...
void FOpenGLDynamicRHI::SetupVertexArrays(uint32 BaseVertexIndex, FOpenGLStream* Streams, uint32 NumStreams, uint32 MaxVertices)
{
	...
	for (uint32 StreamIndex = 0; StreamIndex < NumStreams && StreamMask; StreamIndex++)
	{
		if (StreamMask & 0x1)
		{
			...
			{
				UE_LOGF(LogRHI, Error, "Stream %d marked as in use, but vertex buffer provided is NULL (Mask = %x)", StreamIndex, StreamMask);
...
void FOpenGLDynamicRHI::RHIDrawPrimitiveIndirect(FRHIBuffer* ArgumentBufferRHI, uint32 ArgumentOffset)
{
	...
	{
		UE_LOGF(LogRHI, Fatal, "OpenGL RHI does not yet support indirect draw calls.");
		return;
	}
...
{
	UE_LOGF(LogRHI, Fatal, "OpenGL Render path does not support multiple Viewports!");
}
...
{
	UE_LOGF(LogRHI, Fatal, "FOpenGLDynamicRHI::RHIGetCommandContext should never be called. OpenGL RHI does not implement parallel command list execution.");
	return nullptr;
}

FDynamicRHI::FRHIFinalizeContextsResult FOpenGLDynamicRHI::RHIFinalizeContexts(FRHIFinalizeContextsArgs&& Args)
{
	checkf(Args.UploadContext == nullptr, TEXT("OpenGL RHI does not implement upload contexts."));
	...
	for (ERHIPipeline Pipeline : MakeFlagsRange(ERHIPipeline::All))
	{
	    ...
	    {
		    // "Context" will always be the default graphics context, since we don't implement parallel execution.
		    check(Pipeline == ERHIPipeline::Graphics && Context == this);
	    }
    }
#endif
