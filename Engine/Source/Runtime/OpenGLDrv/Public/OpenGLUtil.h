...
extern bool PlatformOpenGLThreadHasRenderingContext();
	...
	#define CHECK_EXPECTED_GL_THREAD() \
		...
		{ \
			UE_LOGF(LogRHI, Fatal, "Potential use of GL context from incorrect thread. [IsInGameThread() = %d, IsInRenderingThread() = %d, IsInRHIThread() = %d, IsRunningRHIInSeparateThread() = %d]", IsInGameThread(), IsInRenderingThread(), IsInRHIThread(), IsRunningRHIInSeparateThread()); \
		}
#else
	#define CHECK_EXPECTED_GL_THREAD() 
#endif
	...
	struct FOpenGLErrorScope
	{
			...
			, Line(InLine)
		{
			...
			UE_LOGF(LogRHI, Log, "log before %ls(%d): %ls", InFilename, InLine, ANSI_TO_TCHAR(InFunctionName));
#endif
			CheckForErrors(0);
		}
		...
		{
#if ENABLE_VERIFY_GL_TRACE
			UE_LOGF(LogRHI, Log, "log after  %ls(%d): %ls", Filename, Line, ANSI_TO_TCHAR(FunctionName));
