...
public:
	...
	FOpenGLShader(TArrayView<const uint8> Code, GLenum TypeEnum, FShaderResourceTable& SRT, FRHIShader* RHIShader);
...
public:
	...
	FOpenGLVertexShader(TArrayView<const uint8> Code);
...
public:
	...
	FOpenGLPixelShader(TArrayView<const uint8> Code);
...
public:
	...
	FOpenGLGeometryShader(TArrayView<const uint8> Code);
...
public:
	...
	FOpenGLComputeShader(TArrayView<const uint8> Code);
...
public:
	...
	{
		FString retme;
		if(ShaderHashes[CrossCompiler::SHADER_STAGE_VERTEX] != FShaderHash())
		{
		...
		else if(ShaderHashes[CrossCompiler::SHADER_STAGE_COMPUTE] != FShaderHash())
		{
	...
	FShaderHash ShaderHashes[CrossCompiler::NUM_SHADER_STAGES];
};
