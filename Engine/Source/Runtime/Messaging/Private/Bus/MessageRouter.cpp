	...
	if (Context->IsValid())
	{
		...
		if (RecipientCount > 0)
		{
			...
			if (UE_GET_LOG_VERBOSITY(LogMessaging) >= ELogVerbosity::Verbose)
			{
				...
				UE_LOGF(LogMessaging, Verbose, "Dispatching %ls from %ls to %ls", *Context->GetMessageTypePathName().ToString(), *Context->GetSender().ToString(), *RecipientStr);
			}
#endif
			...
			{
				UE_LOGF(LogMessaging, Verbose, "%d recipients were filtered out", RecipientCount - Recipients.Num());
			}
		}
		...
		else
		{
			...
			UE_SUPPRESS(LogMessaging, Verbose,
			{
				const FString RecipientStr = FString::JoinBy(Recipients, TEXT("+"),
					[](const TSharedPtr<IMessageReceiver>& Receiver)
					{
						return Receiver ? Receiver->GetRecipientId().ToString() : TEXT("");
					});

				UE_LOGF(LogMessaging, Verbose, "Dispatching %ls from %ls to %i subscriber(s): %ls"
...
{
	UE_LOGF(LogMessaging, Verbose, "Adding %ls as intereceptor for %ls messages", *Interceptor->GetDebugName().ToString(), *MessageType.ToString());
...
void FMessageRouter::HandleAddRecipient(FMessageAddress Address, TWeakPtr<IMessageReceiver, ESPMode::ThreadSafe> RecipientPtr)
{
	...
	if (Recipient.IsValid())
	{
		UE_LOGF(LogMessaging, Verbose, "Adding %ls on %ls as recipient", *Recipient->GetDebugName().ToString(), *Address.ToString());
...
void FMessageRouter::HandleAddSubscriber(TSharedRef<IMessageSubscription, ESPMode::ThreadSafe> Subscription)
{
	...
	if (Subscriber.IsValid())
	{
		UE_LOGF(LogMessaging, Verbose, "Adding %ls as a subscriber for %ls messages", *Subscriber->GetDebugName().ToString(), *Subscription->GetMessageTypePathName().ToString());
	}
...
{
	UE_LOGF(LogMessaging, Verbose, "Removing %ls as intereceptor for %ls messages", *Interceptor->GetDebugName().ToString(), *MessageType.ToString());
...
void FMessageRouter::HandleRemoveRecipient(FMessageAddress Address)
{
	...
	if (Recipient.IsValid())
	{
		UE_LOGF(LogMessaging, Verbose, "Removing %ls on %ls as recipient", *Recipient->GetDebugName().ToString(), *Address.ToString());
...
void FMessageRouter::HandleRemoveSubscriber(TWeakPtr<IMessageReceiver, ESPMode::ThreadSafe> SubscriberPtr, FTopLevelAssetPath MessageType)
{
	...
	for (auto& SubscriptionsPair : ActiveSubscriptions)
	{
		...
		for (int32 SubscriptionIndex = 0; SubscriptionIndex < Subscriptions.Num(); ++SubscriptionIndex)
		{
			...
			{
				UE_LOGF(LogMessaging, Verbose, "Removing %ls as a subscriber for %ls messages", *Subscriber->GetDebugName().ToString(), *Subscription->GetMessageTypePathName().ToString());
...
{
	UE_LOGF(LogMessaging, Verbose, "Routing %ls message from %ls", *Context->GetMessageTypePathName().ToString(), *Context->GetSender().ToString());
	...
	for (auto& Interceptor : Interceptors)
	{
		...
		{
			UE_LOGF(LogMessaging, Verbose, "Message was intercepted by %ls", *Interceptor->GetDebugName().ToString());
	...
	{
		UE_LOGF(LogMessaging, Verbose, "Queued message for dispatch");
