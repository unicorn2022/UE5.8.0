...
)
{
	...
	if (UE_GET_LOG_VERBOSITY(LogMessaging) >= ELogVerbosity::Verbose)
	{
		...
		UE_LOGF(LogMessaging, Verbose, "Forwarding %ls from %ls to %ls",
...
void FMessageBus::Intercept(const TSharedRef<IMessageInterceptor, ESPMode::ThreadSafe>& Interceptor, const FTopLevelAssetPath& MessageType)
{
	...
	{
		UE_LOGF(LogMessaging, Verbose, "Adding invterceptor %ls", *Interceptor->GetDebugName().ToString());
...
{
	UE_LOGF(LogMessaging, Verbose, "Publishing %ls from sender %ls", *TypeInfo->GetName(), *Publisher->GetSenderAddress().ToString());
...
{
	UE_LOGF(LogMessaging, Verbose, "Registering %ls", *Address.ToString());
...
{
	UE_LOGF(LogMessaging, Verbose, "Sending %ls to %d recipients", *TypeInfo->GetName(), Recipients.Num());
...
)
{
	if (!MessageType.IsNull())
	{
		...
		{
			UE_LOGF(LogMessaging, Verbose, "Subscribing %ls", *Subscriber->GetDebugName().ToString());
...
void FMessageBus::Unintercept(const TSharedRef<IMessageInterceptor, ESPMode::ThreadSafe>& Interceptor, const FTopLevelAssetPath& MessageType)
{
	if (!MessageType.IsNull())
	{
		UE_LOGF(LogMessaging, Verbose, "Unintercepting %ls", *Interceptor->GetDebugName().ToString());
...
{
	UE_LOGF(LogMessaging, Verbose, "Attempting to unregister %ls", *Address.ToString());
	if (!RecipientAuthorizer.IsValid() || RecipientAuthorizer->AuthorizeUnregistration(Address))
	{
		UE_LOGF(LogMessaging, Verbose, "Unregistered %ls", *Address.ToString());
...
void FMessageBus::Unsubscribe(const TSharedRef<IMessageReceiver, ESPMode::ThreadSafe>& Subscriber, const FTopLevelAssetPath& MessageType)
{
	if (!MessageType.IsNull())
	{
		...
		{
			UE_LOGF(LogMessaging, Verbose, "Unsubscribing %ls", *Subscriber->GetDebugName().ToString());
