...
void TMeshTangents<RealType>::ComputeSeparatePerTriangleTangents(const FDynamicMeshNormalOverlay* NormalOverlay, const FDynamicMeshUVOverlay* UVOverlay, const FComputeTangentsOptions& Options)
{
	...
	const EParallelForFlags ParallelForFlags = Options.bParallel ? EParallelForFlags::None : EParallelForFlags::ForceSingleThread;
...
void TMeshTangents<RealType>::ComputeMikkTStyleTangents(const FDynamicMeshNormalOverlay* NormalOverlay, const FDynamicMeshUVOverlay* UVOverlay, const FComputeTangentsOptions& Options)
{
	...
	ParallelFor(Mesh->MaxVertexID(), [&](int32 VertexID)
	{
		...
		for (const FIndex3i& TriEntry : TriSet)
		{
			...
			//{
			//	UE_LOGF(LogTemp, Warning, "TriVertNormal is %f %f %f", TriVertNormal.X, TriVertNormal.Y, TriVertNormal.Z);
			//	UE_LOGF(LogTemp, Warning, "GroupTangent is %f %f %f", GroupTangent.X, GroupTangent.Y, GroupTangent.Z);
			//	UE_LOGF(LogTemp, Warning, "Orig Bitangent is %f %f %f", OrigBitangent.X, OrigBitangent.Y, OrigBitangent.Z);
			//	UE_LOGF(LogTemp, Warning, "Dots are %f %f %f", TriVertNormal.Dot(GroupTangent), TriVertNormal.Dot(OrigBitangent), GroupTangent.Dot(OrigBitangent));
			//	UE_LOGF(LogTemp, Warning, "BinormalSign is %f", BinormalSign);
			//	UE_LOGF(LogTemp, Warning, "Recons Bitangent is %f %f %f", BTResult.X, BTResult.Y, BTResult.Z);
			//}
		}
	}, ParallelForFlags);
