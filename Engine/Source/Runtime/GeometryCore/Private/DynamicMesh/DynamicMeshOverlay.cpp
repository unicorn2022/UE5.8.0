...
{
	ClearElements(); // deletes all elements and initializes triangles to be 1:1 w/ parentmesh IDs
	TStaticArray<RealType, ElementSize> DefaultElement;
	...
	for (int32 VertexID : ParentMesh->VertexIndicesItr())
	{
		...
		{
			ElementID = VertexID;
			EMeshResult Result = InsertElement(VertexID, DefaultElement.GetData());
			checkSlow(Result == EMeshResult::Ok); // because we allocate in increasing sequential order, shouldn't be possible InsertElement to return failure
		}
		else
		{
...
template<typename RealType, int ElementSize>
bool TDynamicMeshOverlay<RealType, ElementSize>::FindAnyElementIDAtVertex(int32 VertexID, int32& OutElementID) const
{
