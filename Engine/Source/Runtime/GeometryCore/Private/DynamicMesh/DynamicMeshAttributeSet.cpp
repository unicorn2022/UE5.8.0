...
void FDynamicMeshAttributeSet::Copy(const FDynamicMeshAttributeSet& Copy)
{
	...
	TriangleLabelAttributes.Reset();
	for (const TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& AttribPair: Copy.TriangleLabelAttributes)
	{
		AttachTriangleLabelAttribute(AttribPair.Key,
			static_cast<FDynamicMeshTriangleLabelAttribute*>(AttribPair.Value->MakeCopy(ParentMesh)));
	}
...
void FDynamicMeshAttributeSet::CompactCopy(const FCompactMaps& CompactMaps, const FDynamicMeshAttributeSet& Copy)
{
	...
	{
		AttachTriangleLabelAttribute(AttribPair.Key,
			static_cast<FDynamicMeshTriangleLabelAttribute *>(AttribPair.Value->MakeCompactCopy(CompactMaps, ParentMesh)));
	}
...
void FDynamicMeshAttributeSet::Append(const FDynamicMeshAttributeSet& ToAppend, const FDynamicMesh3::FAppendInfo& AppendInfo)
{
	...
	for (TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& AttribPair: TriangleLabelAttributes)
	{
		AppendHelper(*AttribPair.Value, ToAppend.FindTriangleLabelAttribute(AttribPair.Key));
	}
...
void FDynamicMeshAttributeSet::AppendDefaulted(const FDynamicMesh3::FAppendInfo& AppendInfo)
{
	...
	for (const TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& AttribPair : TriangleLabelAttributes)
	{
...
void FDynamicMeshAttributeSet::CompactInPlace(const FCompactMaps& CompactMaps)
{
	...
	for (auto& [AttrName, TriangleLabelAttr]: TriangleLabelAttributes)
	{
		TriangleLabelAttr->CompactInPlace(CompactMaps);
	}
...
void FDynamicMeshAttributeSet::EnableMatchingAttributes(const FDynamicMeshAttributeSet& ToMatch, bool bClearExisting, bool bDiscardExtraAttributes)
{
	...
	for (int32 k = bClearExisting ? 0 : ExistingPolygroupLayers; k < NumPolygroupLayers(); k++)
	{
		...
		if (k < ToMatch.NumPolygroupLayers() && PolygroupLayers[k].GetName() == NAME_None)
		{
	...
	for (int32 k = bClearExisting ? 0 : ExistingWeightLayers; k < NumWeightLayers(); k++)
	{
		...
		if (k < ToMatch.NumWeightLayers() && WeightLayers[k].GetName() == NAME_None)
		{
	...
	if (bClearExisting)
	{
		...
		for (const TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& AttribPair: ToMatch.TriangleLabelAttributes)
		{
			AttachTriangleLabelAttribute(AttribPair.Key, 
				static_cast<FDynamicMeshTriangleLabelAttribute *>(AttribPair.Value->MakeNew(ParentMesh)));
		}
	...
	else
	{
		...
		{
			TriangleLabelAttributeMap ExistingTriangleLabels = MoveTemp(TriangleLabelAttributes);
			...
			ResetRegisteredAttributes();
			for (TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& AttribPair : ExistingTriangleLabels)
			{
				if ( ToMatch.TriangleLabelAttributes.Contains(AttribPair.Key) )
				{
					AttachTriangleLabelAttribute(AttribPair.Key, AttribPair.Value.Release());
				}
			}
		...
		// add any new triangle label attribute that did not previously exist
		for (const TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& AttribPair : ToMatch.TriangleLabelAttributes)
		{
			if (!TriangleLabelAttributes.Contains(AttribPair.Key))
			{
				AttachTriangleLabelAttribute(AttribPair.Key,
					static_cast<FDynamicMeshTriangleLabelAttribute *>(AttribPair.Value->MakeNew(ParentMesh)));	
			}
...
void FDynamicMeshAttributeSet::AttachTriangleLabelAttribute(FName InName, FDynamicMeshTriangleLabelAttribute* InAttribute)
{
	RemoveTriangleLabelAttribute(InName);

	// Ensure proper ownership.
	InAttribute->Reparent(ParentMesh);
	InAttribute->SetName(InName);
	TriangleLabelAttributes.Add(InName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>(InAttribute));

	RegisterExternalAttribute(InAttribute);
}

void FDynamicMeshAttributeSet::RemoveTriangleLabelAttribute(FName InName)
{
	if (TriangleLabelAttributes.Contains(InName))
	{
		UnregisterExternalAttribute(TriangleLabelAttributes[InName].Get());
		TriangleLabelAttributes.Remove(InName);
	}
}

void FDynamicMeshAttributeSet::RemoveAllTriangleLabelAttributes()
{
	for (const auto&[Name, AttrUniquePtr]: TriangleLabelAttributes)
	{
		UnregisterExternalAttribute(AttrUniquePtr.Get());
	}
	TriangleLabelAttributes.Reset();
}

bool FDynamicMeshAttributeSet::HasTriangleLabelAttribute(FName InName) const
{
	return TriangleLabelAttributes.Contains(InName);
}

FDynamicMeshTriangleLabelAttribute* FDynamicMeshAttributeSet::FindTriangleLabelAttribute(FName InName) const
{
	const TUniquePtr<FDynamicMeshTriangleLabelAttribute>* Found = TriangleLabelAttributes.Find(InName);
	return Found && Found->IsValid() ? Found->Get() : nullptr;
}
	
const FDynamicMeshAttributeSet::TriangleLabelAttributeMap& FDynamicMeshAttributeSet::GetTriangleLabelAttributes() const
{
	return TriangleLabelAttributes;
}
...
void FDynamicMeshAttributeSet::OnNewTriangle(int TriangleID, bool bInserted)
{
	...
	const FName NewLabel = NAME_None;
	for (auto &[Name, TriangleLabelAttr]: TriangleLabelAttributes)
	{
		TriangleLabelAttr->SetNewValue(TriangleID, &NewLabel);
	}
}
...
void FDynamicMeshAttributeSet::OnRemoveTriangle(int TriangleID)
{
	...
	for (auto &[Name, TriangleLabelAttr]: TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnRemoveTriangle(TriangleID);
	}
}
...
void FDynamicMeshAttributeSet::OnSplitEdge(const FDynamicMesh3::FEdgeSplitInfo& SplitInfo)
{
	...
	for (auto& [Name, TriangleLabelAttr] : TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnSplitEdge(SplitInfo);
	}
}
...
void FDynamicMeshAttributeSet::OnFlipEdge(const FDynamicMesh3::FEdgeFlipInfo & flipInfo)
{
	...
	for (auto& [Name, TriangleLabelAttr] : TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnFlipEdge(flipInfo);
	}
}
...
void FDynamicMeshAttributeSet::OnCollapseEdge(const FDynamicMesh3::FEdgeCollapseInfo & collapseInfo)
{
	...
	for (auto& [Name, TriangleLabelAttr] : TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnCollapseEdge(collapseInfo);
	}
}
...
void FDynamicMeshAttributeSet::OnPokeTriangle(const FDynamicMesh3::FPokeTriangleInfo & pokeInfo)
{
	...
	for (auto& [Name, TriangleLabelAttr] : TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnPokeTriangle(pokeInfo);
	}
}
...
void FDynamicMeshAttributeSet::OnMergeEdges(const FDynamicMesh3::FMergeEdgesInfo & mergeInfo)
{
	...
	for (auto& [Name, TriangleLabelAttr] : TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnMergeEdges(mergeInfo);
	}
}
...
void FDynamicMeshAttributeSet::OnMergeVertices(const DynamicMeshInfo::FMergeVerticesInfo& MergeInfo)
{
	...
	for (auto& [Name, TriangleLabelAttr] : TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnMergeVertices(MergeInfo);
	}
}
...
void FDynamicMeshAttributeSet::OnSplitVertex(const DynamicMeshInfo::FVertexSplitInfo& SplitInfo, const TArrayView<const int>& TrianglesToUpdate)
{
	...
	for (auto& [Name, TriangleLabelAttr] : TriangleLabelAttributes)
	{
		TriangleLabelAttr->OnSplitVertex(SplitInfo, TrianglesToUpdate);
	}
}
...
bool FDynamicMeshAttributeSet::IsSameAs(const FDynamicMeshAttributeSet& Other, bool bIgnoreDataLayout) const
{
	if (UVLayers.Num() != Other.UVLayers.Num() ||
		...
		TriangleLabelAttributes.Num() != Other.TriangleLabelAttributes.Num())
	{
		return false;
	}
	...
	if (!TriangleLabelAttributes.IsEmpty())
	{
		for (const auto&[Name, LabelAttribute]: TriangleLabelAttributes)
		{
			const TUniquePtr<FDynamicMeshTriangleLabelAttribute>* Found = Other.TriangleLabelAttributes.Find(Name);
			const FDynamicMeshTriangleLabelAttribute* OtherLabelAttribute = Found ? Found->Get() : nullptr;
			if (!OtherLabelAttribute)
			{
				return false;
			}
			if (!LabelAttribute->IsSameAs(*OtherLabelAttribute, bIgnoreDataLayout))
			{
				return false;
			}
		}
...
void SerializeLabels(FArchive& Ar, FDynamicMeshTriangleLabelAttribute& Labels)
{
	enum class EIndexSize : uint8 { U8 = 0, U16 = 1, U32 = 2 };
	...
	if (Ar.IsLoading())
	{
		...
		constexpr uint32 ArrayMax = static_cast<uint32>(std::numeric_limits<TArray<FName>::SizeType>::max());
		if (!ensure(NumNames <= ArrayMax))
		{
			Ar.SetError();
			return;
		}
		...
		// ret unique names
		uint32 NumUniqueNames = 0;
		Ar << NumUniqueNames;
		
		if (!ensure(NumUniqueNames <= ArrayMax))
		{
			Ar.SetError();
			return;
		}
		...
		UniqueNames.Reserve(NumUniqueNames);
		
		for (uint32 Index = 0; Index < NumUniqueNames; ++Index)
		{
			FString NameStr;
			Ar << NameStr;
			UniqueNames.Add(FName(NameStr));
		}

		// single FName so assign the same for all triangles
		// or all names are unique so just assign them 
		if (NumUniqueNames == 1 || NumUniqueNames == NumNames)
		{
			const bool bSingle = NumUniqueNames == 1;	
			for (uint32 Index = 0; Index < NumNames; ++Index)
			{
				Labels.SetValue(Index, bSingle ? UniqueNames[0] : UniqueNames[Index]);
			}
			return;
		}

		// read index size
		EIndexSize IndexSize = EIndexSize::U8;
		Ar << IndexSize;

		// read indices relative to UniqueNames 
		for (uint32 Index = 0; Index < NumNames; ++Index)
		{
			uint32 NameIndex = 0;
			if (IndexSize == EIndexSize::U8)
			{
				uint8 v = 0;
				Ar << v;
				NameIndex = v;
			}
			else if (IndexSize == EIndexSize::U16)
			{
				uint16 v = 0;
				Ar << v;
				NameIndex = v;
			...
			if (ensure(UniqueNames.IsValidIndex(NameIndex)))
			{
				Labels.SetValue(Index, UniqueNames[NameIndex]);
			}
		}
	}
	else
	{
		// write names count
		uint32 NumNames = Labels.GetParentMesh()->MaxTriangleID();
		Ar << NumNames;
		...
		TArray<uint32> NameIndices;
		NameIndices.Init(0, NumNames);

		for (uint32 Index = 0; Index < NumNames; ++Index)
		{
			const FName Name = Labels.GetValue(Index);
			if (const uint32* Found = NameToIndex.Find(Name))
			{
				NameIndices[Index] = *Found;
			}
			else
			{
				NameIndices[Index] = UniqueNames.Num();
				NameToIndex.Add(Name, UniqueNames.Num());
				UniqueNames.Emplace(Name.ToString());
			}
		}
		
		// write unique count and names
		uint32 NumUniqueNames = UniqueNames.Num();
		Ar << NumUniqueNames;
		for (uint32 Index = 0; Index < NumUniqueNames; ++Index)
		{
			Ar << UniqueNames[Index];
		}
		
		// single FName or all names are unique so nothing else to do
		if (NumUniqueNames == 1 || NumUniqueNames == NumNames)
		{
			return;
		}

		// choose a convenient index size and write it
		EIndexSize IndexSize = (NumUniqueNames <= MAX_uint8) ? EIndexSize::U8 : (NumUniqueNames <= MAX_uint16) ? EIndexSize::U16 : EIndexSize::U32;
		Ar << IndexSize;

		// write indices relative to UniqueNames 
		for (uint32 Index = 0; Index < NumNames; ++Index)
		{
			if (IndexSize == EIndexSize::U8)
			{
				uint8 v = static_cast<uint8>(NameIndices[Index]);
				Ar << v;
			}
			else if (IndexSize == EIndexSize::U16)
			{
				uint16 v = static_cast<uint16>(NameIndices[Index]);
				Ar << v;
			}
			else
			{
				Ar << NameIndices[Index];
			}
		}
	}
}
...
void FDynamicMeshAttributeSet::Serialize(FArchive& Ar, const FCompactMaps* CompactMaps, bool bUseCompression)
{
	...
	const bool bSerializeTriangleLabels = !Ar.IsLoading() ||
			Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) >= FFortniteMainBranchObjectVersion::DynamicMeshAttributesTriangleLabels;
	if (bSerializeTriangleLabels)
	{		
		int32 NumTriangleLabelAttributes = TriangleLabelAttributes.Num();
		Ar << NumTriangleLabelAttributes;
		
		FNameAsStringProxyArchive ProxyArchive(Ar);
		
		if (Ar.IsLoading())
		{
			RemoveAllTriangleLabelAttributes();

			for (int32 Index = 0; Index < NumTriangleLabelAttributes; ++Index)
			{
				FName Key;
				ProxyArchive << Key;
				...
				if (IsValid)
				{
					TUniquePtr<FDynamicMeshTriangleLabelAttribute>& Value =
						TriangleLabelAttributes.Emplace(Key, new FDynamicMeshTriangleLabelAttribute(ParentMesh));
					SerializeLabels(Ar, *Value.Get());
					Value->SetName(Key);
					RegisterExternalAttribute(Value.Get());
				}
			}
		...
		{
			for (TriangleLabelAttributeMap::TIterator It(TriangleLabelAttributes); It; ++It)
			{
				ProxyArchive << It.Key();

				const TUniquePtr<FDynamicMeshTriangleLabelAttribute>& Value = It.Value();
				bool bIsValid = Value.IsValid();
				Ar << bIsValid;
				if (bIsValid)
				{
...
SIZE_T FDynamicMeshAttributeSet::GetByteCount() const
{
	...
	for (const TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& TriangleLabelAttribute: TriangleLabelAttributes)
	{
		ByteCount += TriangleLabelAttribute.Value->GetByteCount();
	}
...
bool FDynamicMeshAttributeSet::CheckValidity(bool bAllowNonmanifold, EValidityCheckFailMode FailMode) const
{
	...
	for (const TPair<FName, TUniquePtr<FDynamicMeshTriangleLabelAttribute>>& Pair : TriangleLabelAttributes)
	{
		if (Pair.Value)
		{
			bValid = Pair.Value->CheckValidity(bAllowNonmanifold, FailMode) && bValid;

			// If non-null, triangle label attributes are in the RegisteredAttributes array
			int32 FoundIndex;
