...
// Generate a semi-random point ordering (via shuffling) 
// Note this does create some bias in favor of more-densely
// triangulated regions, which can have more points in the
// initial dense sampling.
// Version 1 and earlier also ignore the RandomSeed, so will tend to draw
// samples from the same triangles across differently-seeded runs.
void GetShuffledPointOrdering(TArray<int32>& PointOrdering, int32 NumDensePoints, int32 RandomSeed, int32 Version)
{
	bool bUseSeed = Version > 1 || Version == INDEX_NONE;

	// as of Version 2, we use the random stream to generate a shuffled order
	if (bUseSeed)
	{
		PointOrdering.SetNumUninitialized(NumDensePoints);
		FRandomStream Stream(RandomSeed);
		for (int32 Index = 0; Index < NumDensePoints; ++Index)
		{
			// Insert Index at a random position via swap
			const int32 SwapIndex = Stream.RandRange(0, Index);
			if (SwapIndex < Index)
			{
				PointOrdering[Index] = PointOrdering[SwapIndex];
			}
			PointOrdering[SwapIndex] = Index;
		}
	}
	else // older version; use modulo iteration w/out random seed
	{
	...
	int32 SamplingMethodVersion)
{
	...
	//UE_LOGF(LogTemp, Warning, "TotalArea %f  SampleArea %f EstSamples %d  DenseSamples %d   DenseArea %f  NumDensePoints %d",
	...
	FProgressCancel* Progress)
{
	...
	if (bShuffle)
	{
		GetShuffledPointOrdering(PointOrdering, NumDensePoints, RandomSeed, SamplingMethodVersion);
	}
	else
	{
		PointOrdering.Reserve(NumDensePoints);
