...
{
	Storage.Add(0);
	SourceData = TArrayView64<const uint8>(Storage);
}
	
FDisplacementMap::FDisplacementMap( 
		TArray64<uint8>&& InSourceStorage, 
	...
	, AddressX( InAddressX )
	, AddressY( InAddressY )
	, Storage(MoveTemp(InSourceStorage))
	, SourceData(Storage)
{
	Init();
}
...
FDisplacementMap::FDisplacementMap( 
		TArrayView64<const uint8> InSourceData, 
		ETextureSourceFormat InSourceFormat,
		const int32 InSizeX,
		const int32 InSizeY,
		float InMagnitude, 
...
int FDisplacementMap::GetBytesPerPixel(const ETextureSourceFormat SourceFormat) 
{
	switch (SourceFormat)
	{
	case TSF_BGRA8:
...
void FDisplacementMap::Init()
{
	if ((BytesPerPixel = GetBytesPerPixel(SourceFormat)) == 0)
	{
		ensureMsgf(false, TEXT("Unsupported texture format."));
		InitDefault();
		return;
	}
		
	if (SizeX > MAX_RESOLUTION || SizeY > MAX_RESOLUTION)
	{
		ensureMsgf(false, TEXT("Maximum displacement map resolution exceeded."));
		InitDefault();
		return;
	}

	// replace void textures by 1x1 black pixel
	if (SizeX == 0 || SizeY == 0)
	{
		InitDefault();
		return;
	...
	check(SourceData.Num() == SizeX * SizeY * BytesPerPixel);

	if (SizeX == 1 && SizeY == 1)
	{
		// don't add mipmaps for 1x1 image
		check(NumLevels == 1);
		return;
	}
...
void FDisplacementMap::InitDefault()
{
	SourceFormat = TSF_G8;
	BytesPerPixel = 1;
	SizeX = 1;
...
{
	// Half texel
	UV.X = UV.X * static_cast<float>(SizeX) - 0.5f;
	UV.Y = UV.Y * static_cast<float>(SizeY) - 0.5f;
	...
	float wx1 = UV.X - static_cast<float>(x0);
	float wy1 = UV.Y - static_cast<float>(y0);
	float wx0 = 1.0f - wx1;
	float wy0 = 1.0f - wy1;
...
{
	// Half texel
	MinUV = MinUV * FVector2f( (float)SizeX, (float)SizeY ) - FVector2f( 0.5f );
	MaxUV = MaxUV * FVector2f( (float)SizeX, (float)SizeY ) - FVector2f( 0.5f );
...
float FDisplacementMap::SampleEWA(FVector2f UV, FVector2f Axis0, FVector2f Axis1) const
{
	...
	float LODLevelf  = FMath::Max(0.f, static_cast<float>(NumLevels) - 1.f + FMath::Log2(Len1));
	uint32 LODLeveli = FMath::FloorToInt(LODLevelf);
	float d = LODLevelf - static_cast<float>(LODLeveli);
...
float FDisplacementMap::EWA(uint32 Level, FVector2f UV, FVector2f Axis0, FVector2f Axis1) const
{
	...
	FVector2f MipScale((float)MipSizeX, (float)MipSizeY);
	...
	for (int32 Vi = V0; Vi <= V1; ++Vi)
	{
		const float Vf = static_cast<float>(Vi) - UV.Y;
		for (int32 Ui = U0; Ui <= U1; ++Ui)
		{
			const float Uf = static_cast<float>(Ui) - UV.X;
			...
			if (RSqr < 1.f)
			{
				...
				const int32 X = TMathUtil<int32>::Clamp(Ui, 0, MipSizeX - 1);
				const int32 Y = TMathUtil<int32>::Clamp(Vi, 0, MipSizeY - 1);
				if (Level == 0)
				{
					Sum += Weight * Load(X, Y);
				}
