...
void FDynamicMeshEditor::AppendTriangles(const FDynamicMesh3* SourceMesh, const TArrayView<const int>& SourceTriangles, FMeshIndexMappings& IndexMaps, FDynamicMeshEditResult& ResultOut, bool bComputeTriangleMap)
{
	...
	{
		if (SourceMesh->IsTriangle(SourceTriangleID))
		{
			NumTriangles++;
		}
	}
	check(NumTriangles == SourceTriangles.Num());
	
	if (NumTriangles != SourceTriangles.Num())
	{
		return;
	}
	...
	// reserve data
	{
		if (SourceMesh->HasAttributes() && Mesh->HasAttributes())
		{
			const int32 MaxNumTriangles = FMath::Max(Mesh->TriangleCount() + NumTriangles, Mesh->MaxTriangleID());
			
			const FDynamicMeshAttributeSet* SrcAttributes = SourceMesh->Attributes();
			FDynamicMeshAttributeSet* Attributes = Mesh->Attributes();
		
			IndexMaps.GetTriangleMap().Reserve(NumTriangles);
			ResultOut.NewTriangles.Reserve(NumTriangles);
			
			const int NumUVLayers = FMath::Min(SrcAttributes->NumUVLayers(), Attributes->NumUVLayers());
			for (int UVLayerIndex = 0; UVLayerIndex < NumUVLayers; UVLayerIndex++)
			{
				IndexMaps.GetUVMap(UVLayerIndex).Reserve(NumTriangles);
				Attributes->GetUVLayer(UVLayerIndex)->InitializeTriangles(MaxNumTriangles);
			}

			const int NumNormalLayers = FMath::Min(SrcAttributes->NumNormalLayers(), Attributes->NumNormalLayers());
			for (int NormalLayerIndex = 0; NormalLayerIndex < NumNormalLayers; NormalLayerIndex++)
			{
				IndexMaps.GetNormalMap(NormalLayerIndex).Reserve(NumTriangles);
				Attributes->GetNormalLayer(NormalLayerIndex)->InitializeTriangles(MaxNumTriangles);
			}

			if (Attributes->HasPrimaryColors())
			{
				IndexMaps.GetColorMap().Reserve(NumTriangles);
				Attributes->PrimaryColors()->InitializeTriangles(MaxNumTriangles);
			}
		}
	}

	const bool bHasTriangleGroups = Mesh->HasTriangleGroups();
	const bool bSourceHasTriangleGroups = SourceMesh->HasTriangleGroups();
	...
	for (int SourceTriangleID : SourceTriangles)
	{
		...
		if (bHasTriangleGroups)
		{
			if (bSourceHasTriangleGroups)
			{
		...
		FIndex3i NewTri;
		const FIndex3i& Tri = SourceMesh->GetTriangleRef(SourceTriangleID);
		for (int j = 0; j < 3; ++j)
		{
...
// Note: This is the legacy / direct TArray<FDynamicMesh3> version of SplitMesh
bool FDynamicMeshEditor::SplitMesh(const FDynamicMesh3* SourceMesh, TArray<FDynamicMesh3>& SplitMeshes, TFunctionRef<int(int)> TriIDToMeshID, int DeleteMeshID, TArray<int32>* MeshIDsOut, bool bSortByID)
{
	...
	if (NumMeshes == 0) // full delete case, just leave the empty SplitMeshes array
	{
		return true;
	}
...
bool FDynamicMeshEditor::SplitMesh(const FDynamicMesh3* SourceMesh, TArray<TUniquePtr<FDynamicMesh3>>& SplitMeshes, bool bSplitIfSingle, TFunctionRef<int(int)> TriIDToMeshID, int DeleteMeshID, TArray<int32>* MeshIDsOut, bool bSortByID)
{
	using namespace UE::DynamicMeshEditorInternals;
	...
	if (!bAlsoDelete && NumMeshes < 2 && !bSplitIfSingle)
	{
		return false; // nothing to do, so don't bother filling the split meshes array
	}
	...
	if (!bAlsoDelete && NumMeshes == 1)
	{
		SplitMeshes[0] = MakeUnique<FDynamicMesh3>(*SourceMesh);
		return true;
	}
	...
	{
		SplitMeshes[Idx] = MakeUnique<FDynamicMesh3>();
		FDynamicMesh3& M = *SplitMeshes[Idx];
		M.EnableMeshComponents(SourceMesh->GetComponentsFlags());
		if (SourceMesh->HasAttributes())
		{
			M.EnableAttributes();
	...
	{
		FMeshIndexMappings& Map = Mappings.Emplace_GetRef();
		Map.Initialize(SplitMeshes[Idx].Get());
	}
	...
	for (int Idx = 0; Idx < NumMeshes; Idx++)
	{
		AppendVertexAttributes(SourceMesh, SplitMeshes[Idx].Get(), Mappings[Idx]);
	}
