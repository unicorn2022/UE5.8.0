	...
	bool bFailOnMissingObjects) const
{
	...
	if (bVerbose)
	{
		UE_LOGF(LogTemp, Warning, "FSparseDynamicOctree3::CheckValidity: MaxLevel %d  SpillCount %d  MissingCount %d", MaxLevel, SpillObjectCount, MissingObjectCount);
		for (uint32 k = 0; k <= MaxLevel; ++k)
		{
			UE_LOGF(LogTemp, Warning, "    Level %4d  Cells %4d  Objects %4d", k, CellsAtLevels[k], ObjectsAtLevel[k]);
		}
	}
}
