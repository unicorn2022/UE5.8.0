...
private:
	...
	void make_grid(FVector3f origin, float dx, int ni, int nj, int nk, FDenseGrid3f& winding)
	{
		...
		TArray<bool> bits;
		check(winding.Num() <= MAX_int32);
		bits.SetNumZeroed(static_cast<int32>(winding.Num()));
		...
		while (true)
		{
			...
			ParallelFor(cur_front.Num(), [this, origin, dx, ni, nj, nk, &winding, &distances, &bounds, &bits, &cur_front, &queue, &QueueSection, &abort](int FrontIdx)
			{
				...
				FVector3i ijk = winding.GetCoords(cur_front[FrontIdx]);
				...
				for (int K = 0; K < 26; ++K) {
					...
					checkSlow(winding.GetIndex(nijk) <= MAX_int32);
					int LinearNIJK = static_cast<int32>(winding.GetIndex(nijk));
	...
	void make_grid_dense(FVector3f origin, float dx, int ni, int nj, int nk, FDenseGrid3f& winding)
	{
		...
		bool abort = false;
		check(winding.Num() <= MAX_int32);
		ParallelFor(static_cast<int32>(winding.Num()), [this, origin, dx, &winding, &abort](int LinearIdx)
		{
			if (LinearIdx % 100 == 0)
			{
				abort = CancelF();
			...
			FVector3i ijk = winding.GetCoords(LinearIdx);
