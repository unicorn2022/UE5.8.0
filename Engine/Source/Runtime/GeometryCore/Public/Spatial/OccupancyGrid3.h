...
struct FOccupancyGrid3
{
		...
		)
	{
		...
		if (!ensure(Occupancy.Num() <= (int64)MAX_int32))
		{
			// TODO: support occupancy grids of larger sizes
			return;
		}
		ParallelFor(static_cast<int32>(Occupancy.Num()), [&](const int32 OccupancyId)
		{
			const FVector3i OccupancyIndex(Occupancy.GetCoords(OccupancyId));
			const SIZE_T Count = Algo::CountIf(CornerOffsets, [&WindingGrid, OccupancyIndex](FVector3i CornerOffset)
			{
