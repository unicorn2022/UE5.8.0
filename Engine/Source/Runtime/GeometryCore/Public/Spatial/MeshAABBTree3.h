...
public:
		...
		FBoxesSet& Tris, FBoxesSet& Nodes, FAxisAlignedBox3d& Box)
	{
		...
		if (interval.Length() > FMathd::ZeroTolerance)
		{
			...
			while (l < r)
			{
				// Float rounding can collapse midpoint onto an extremum, in which case
				// every value matches one of the conditions; the scans must be bounded.
				while (l < ICount && Centers[IStart + l][axis] <= midpoint)
				{
					l++;
				}
				while (r >= 0 && Centers[IStart + r][axis] > midpoint)
				{
					r--;
				}
				if (l >= r)
			...
			if (n0 == 0 || n0 == ICount)
			{
				// Partition degenerated to a single side; fall back to half-split.
				n0 = ICount / 2;
				n1 = ICount - n0;
			}
			checkSlow(n0 >= 1 && n1 >= 1);
