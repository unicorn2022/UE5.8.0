...
private:
	...
	void make_level_set3_parallel_spatial(FVector3f Origin, CellSizeType DX, int NI, int NJ, int NK, FDenseGrid3f& Distances, int ExactBand)
	{
		...
		check(Grid.Num() <= MAX_int32);
		ParallelFor(static_cast<int32>(Grid.Num()), [this, &Origin, DX, NI, NJ, NK, &Distances, max_dist, upper_bound, &closest_tri](int LinearIdx)
		{
			FVector3i Idx = Grid.GetCoords(LinearIdx);
			if (Distances[Idx] == 1) {
	...
	void make_level_set3_parallel_floodfill(FVector3f Origin, CellSizeType DX, int NI, int NJ, int NK, FDenseGrid3f& Distances)
	{
		...
		{
			int64 CellLinearIndex = Distances.GetIndex(CellGridIndex);
		...
		TArray<bool> done; 
		check(Distances.Num() <= MAX_int32);
		done.SetNumZeroed(static_cast<int32>(Distances.Num()));
		...
		bool bAbort = false;
		{
			...
			ParallelFor(Mesh->MaxVertexID(), [&](int vid)
			{
...

				{
					...
					closest_tri[Idx] = near_tid;
					int idx_linear = static_cast<int32>(Distances.GetIndex(Idx));
		...
		double max_query_dist = max_dist + GetDiagLength(DX);
		{
			...
			while (PendingCellQueue.Num() > 0)
			{
				ParallelFor(PendingCellQueue.Num(), [&](int QIdx)
				{
					...
					FVector3i cur_idx = Distances.GetCoords(cur_linear_index);
					for (FVector3i idx_offset : IndexUtil::GridOffsets26) 
					{
						...
						int nbr_linear_idx = static_cast<int32>(Distances.GetIndex(nbr_idx));
