...
public:
	...
	*/
	...
	/**
	 * NOTE: Please use the TArray<TUniquePtr<FDynamicMesh3>> version of this method, below, instead of this one, because storing FDynamicMesh3 in TArray directly is not safe.
	 * This method will not directly cause memory corruption, but any subsequent manipulation of the array will be unsafe.
	 * 
	 ...
	 */
	UE_DEPRECATED(5.8, "Use the version of SplitMesh that takes a TArray<TUniquePtr<FDynamicMesh3>> array instead")
	 ...
	 * @param SourceMesh The mesh to split
	 * @param SplitMeshes An array with one mesh per unique Mesh ID found in SourceMesh (optionally excluding the DeleteMeshID)
	 * @param bSplitIfSingle Whether to still split if the split would just create a single mesh with no deletion.
	 * @param TriIDToMeshID A function mapping Triangle IDs to Mesh IDs
	 * @param DeleteMeshID If non-negative, triangles with this Mesh ID will be removed rather than being placed in a split mesh
	 * @param MeshIDPerSplitMesh If non-null, will be filled with the Mesh ID for each of the SplitMeshes generated
	 * @param bSortByMeshID If true, SplitMeshes will be sorted in order of increasing Mesh ID
	 * @return true if needed split, false if there were not multiple mesh ids, and bSplitIfSingle was false. If false, the SplitMeshes array will not be updated.
	 */
	static GEOMETRYCORE_API bool SplitMesh(const FDynamicMesh3* SourceMesh, TArray<TUniquePtr<FDynamicMesh3>>& SplitMeshes, bool bSplitIfSingle, TFunctionRef<int(int)> TriIDToMeshID, int DeleteMeshID = -1, TArray<int32>* MeshIDPerSplitMesh = nullptr, bool bSortByMeshID = false);
