...
public:
	...
	// Note: Only filled for operations that combine both meshes (i.e., when OperatesOnSingleMesh(Operation) == false)
...
public:
	...
	// Helper to test if an operation will incorporate/edit both meshes, or just one
	// Note: If the operation works on a single mesh, then TrackPerTriangleSourceMesh will not be filled
	static bool OperatesOnSingleMesh(EBooleanOp Operation)
	{
		return Operation == EBooleanOp::TrimInside || Operation == EBooleanOp::TrimOutside || Operation == EBooleanOp::NewGroupInside || Operation == EBooleanOp::NewGroupOutside;
	}
