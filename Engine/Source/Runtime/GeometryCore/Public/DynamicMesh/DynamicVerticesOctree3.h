...
public:
	...
	/**
	 * Reinsert Vertices into the tree
	 */
	template<typename EnumerableType>
	void ReinsertVertices(const EnumerableType& Vertices)
	{
		for (int VertexID : Vertices)
		{
			FVector3d Point = VertexSource->GetVertex(VertexID);
	...
	/**
	 * Reinsert Vertices into the tree, unless ShouldSkip(VertexID) returns true
	 */
	template<typename EnumerableType>
	void ReinsertFilteredVertices(const EnumerableType& Vertices, TFunctionRef<bool(int32)> ShouldSkip)
	{
		for (int32 VertexID : Vertices)
		{
			if (!ShouldSkip(VertexID))
			{
				FVector3d Point = VertexSource->GetVertex(VertexID);
				ModifiedBounds.Contain(Point);
				FSparseDynamicPointOctree3::ReinsertPoint(VertexID, Point);
