...
struct FComputeTangentsOptions
{
	...
	/** if true, the computation will be done in parallel (via ParallelFor) */
	bool bParallel = true;
};
...
public:
	 ...
	 * in the mesh, then 3*N tangents are generated. These tangents are optionally computed in parallel.
	 */
	...
	/**
	 * Compute per-triangle tangents for the given UV Overlay. Computation is done in parallel.
