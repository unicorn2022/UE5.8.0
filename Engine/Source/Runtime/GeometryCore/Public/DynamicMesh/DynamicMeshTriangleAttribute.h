...
public:
	...
	virtual ~FDynamicMeshTriangleAttributeChange() = default;
...
public:
	...
	*/
	virtual bool CheckValidity(bool bAllowNonmanifold, EValidityCheckFailMode FailMode) const override
	{
		...
		if (!ParentMesh || ParentMesh->MaxTriangleID() < 0 || static_cast<size_t>(ParentMesh->MaxTriangleID())*AttribDimension > AttribValues.Num())
		{
			switch (FailMode)
			{
...
using FDynamicMeshTriangleLabelAttribute = TDynamicMeshSingleTriangleAttribute<FName>;
