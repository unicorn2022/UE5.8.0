...
public:
	...
	*/
	...
	/**
	 * Utility function to be able to modify a vertex in place. Will assert if out of bounds.
	 * @param Vertex       Vertex value to update
	 * @param HoleIndex    Hole Index, -1 if targeting Outer
	 * @param VertexIndex Index of the vertex to update in the Hole/Outer polygon.
	 */
	void UpdateVertexInPlace(const TVector2<T>& Vertex, int32 HoleIndex, int32 VertexIndex)
	{
		check(HoleIndex == -1 || Holes.IsValidIndex(HoleIndex));
		TPolygon2<T>& Polygon = HoleIndex == -1 ? Outer : Holes[HoleIndex];
		check(Polygon.GetVertices().IsValidIndex(VertexIndex));
		Polygon[VertexIndex] = Vertex;
	}
	...
	{
		SIZE_T AllocatedSize = Outer.GetVertices().GetAllocatedSize();

		AllocatedSize += Holes.GetAllocatedSize();
		for (const TPolygon2<T>& Hole : Holes)
		{
			AllocatedSize += Hole.GetVertices().GetAllocatedSize();
		}
