...
{ 
    static inline const FString Name = "Foo";
};
...
void CreatePolyBezierWithFooAttributes()
{
	...
	Spline.GetSpline().AppendAnchor( FPolyBezierSpline3d::FAnchor(
		FVector(150, -50, 0),  // In Handle
		FVector(200, 0, 0),    // Anchor Point
		FVector(200, 100, 0))  // Out Handle
	);
    ...
    if (FooSpline)
    {
        ...
        UE_LOGF(LogTemp, Display, "Attribute channel has %d values", NumFooValues);
    ...
    for (float t = 0.0f; t <= 2.0f; t += 0.1f)
	{
		...
		UE_LOGF(LogTemp, Display, "At t=%.1f: Position=(%.1f, %.1f, %.1f), Foo=%.2f",
