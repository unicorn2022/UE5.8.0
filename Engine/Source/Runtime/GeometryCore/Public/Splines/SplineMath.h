...
namespace Math
{
	...
	struct FSplineValidation
	{
		...
		static bool IsValidWindow(const TArrayView<const T* const>& Window)
		{
			for (int32 i = 0; i < N; ++i)
			{
				if (!Window.IsValidIndex(i) || Window[i] == nullptr)
				{
					return false;
				}
			}
		...
		TArray<float>& OutBasis)
	{
		...
		for (int32 j = 1; j <= Degree; ++j)
		{
			...
			{
				UE_LOGF(LogTemp, Warning, "Invalid knot indices in ComputeBasisFunctions: LeftIndex=%d, RightIndex=%d",
	...
	struct TGenericDerivativeHelper
	{
		...
		{
			// Basic validation
			if (!FSplineValidation::IsValidWindow<T, Order+1>(Window))
			{
	...
	/**
	 * Bezier derivative calculator.
	 * Computes derivatives and applies segment scaling.
	 * 
	 * @tparam T Value type (FVector, FQuat, FTransform, etc.)
	 * @tparam Order Derivative order (0=position, 1=velocity, 2=acceleration, etc.)
	 */
	template <typename T, int32 Order>
	struct TBezierDerivativeCalculator
	{
		static T Compute(TArrayView<const T* const> Window, float Parameter, float SegmentScale)
		{
			...
			if (!FSplineValidation::IsValidWindow<T, 4>(Window) || // Cubic Bezier needs 4 points
			...
			if constexpr (Order > 0)
			{
				// If L == 0, the segment is degenerate in parameter space
				// return the zero derivative (well-defined limit)
				if (SegmentScale == 0.0f)
				{
					return FSplineValidation::GetDefaultValue<T>();
				}
				
				// Calculate scaling factor: 1 / SegmentScale^Order
				float ScalingFactor = FMath::Pow(1.0f / SegmentScale, static_cast<float>(Order));
