...
public:
        ...
        FactoryFunction Factory)
    {
		...
		UE::TScopeLock Lock(GetRegistryLock());
        ...
        UE_LOGF(LogSpline, Log, "Registered spline type: TypeId=0x%08X, Impl=%ls, Value=%ls",
    ...
    static inline TUniquePtr<ISplineInterface> CreateSpline(TypeId TypeId)
    {
        ...
        if (Names)
        {
            UE_LOGF(LogSpline, Warning, "No factory for type ID 0x%08X (Impl=%ls, Value=%ls)",
                TypeId, *Names->Key, *Names->Value);
        }
        else
        {
            UE_LOGF(LogSpline, Warning, "Unknown type ID 0x%08X", TypeId);
        }
        ...
        const FString& ValueTypeName)
    {
        ...
        UE_LOGF(LogSpline, Warning, "No registered type for Impl=%ls, Value=%ls",
...
private:
     ...
     */
    static inline FTransactionallySafeCriticalSection& GetRegistryLock()
    {
        static FTransactionallySafeCriticalSection RegistryLock;
        return RegistryLock;
    }
