...
struct FKnot
{
	...
	explicit FKnot(float InValue, uint32 InMultiplicity)
		: Value(InValue)
...
template <typename InValueType, int32 InDegree>
class TBSpline
	: public TSplineInterface<InValueType>,
	  private TSelfRegisteringSpline<TBSpline<InValueType, InDegree>, InValueType>
{
public:
	using ValueType = InValueType;

	static constexpr int32 Degree = InDegree;
	static constexpr int32 WindowSize = Degree + 1;
	using FWindow = TStaticArray<const ValueType*, WindowSize>;
	using FWindowStore = TStaticArray<ValueType, WindowSize>;
	...
	void Dump() const
	{
		...
		{
			UE_LOGF(LogSpline, Verbose, "Values:");
			for (int32 Index = 0; Index < Values.Num(); ++Index)
			{
				UE_LOGF(LogSpline, Verbose, "  [%d] %f", Index, Values[Index]);
			}
		}
		else if constexpr (THasToString<ValueType>::value)
		{
			...
			for (int32 Index = 0; Index < Values.Num(); ++Index)
			{
				UE_LOGF(LogSpline, Verbose, "  [%d] %ls", Index, *Values[Index].ToString());
			}
		}

		UE_LOGF(LogSpline, Verbose, "Knots:");
		PrintKnotVector();
	}
	...
	virtual ValueType EvaluateImpl(float Parameter) const override
	{
		...
		if (!Math::FSplineValidation::IsValidWindow<ValueType, WindowSize>(Window) ||
	...
	{
		UE_LOGF(LogSpline, Warning, "SetParameter is disabled. Use Reparameterize() or a linear spline if you need manual control.");
		return INDEX_NONE;
	}
	...
	virtual TArray<FKnot> GetKnotVector() const
	{
		return PairKnots;
	}

	bool ValidateKnotVector(const TArray<FKnot>& InKnots) const
	{
		...
		for (const FKnot& Knot : InKnots)
		{
			if (!ensureAlwaysMsgf(Knot.Multiplicity > 0,
								  TEXT("Invalid knot with zero multiplicity.")))
			{
				return false;
			}
		...
		if (!ensureAlwaysMsgf(TotalExpandedKnots >= RequiredKnots,
							  TEXT("Invalid knot vector size. Need %d knots, got %d."),
							  RequiredKnots,
							  TotalExpandedKnots))
		{
			return false;
		...
		for (int32 i = 1; i < InKnots.Num(); ++i)
		{
			if (!ensureAlwaysMsgf(InKnots[i].Value >= InKnots[i - 1].Value,
								  TEXT("Invalid knot vector: knot values must be non-decreasing.")))
			{
				return false;
			}
		}
	 ...
	 */
	virtual bool SetKnotVector(const TArray<FKnot>& NewKnots)
	{
		if (!ValidateKnotVector(NewKnots))
		{
			return false;
		}
		...
		UE_LOGF(LogSpline, Verbose, "Set knot vector (Direct) - Points: %d, Unique Knots: %d",
			NumKeys(), PairKnots.Num());
	...
	virtual void Reparameterize(EParameterizationPolicy ParameterizationPolicy)
	{
		...
		{
			UE_LOGF(LogSpline, Warning, "Not enough points for B-spline: %d (need >= %d)",
				NumValues, Degree + 1);
			return;
		}
		...
		UE_LOGF(LogSpline, Verbose, "Updated knot vector - Points: %d, Degree: %d, Knots: %d",
	...
	{
		// Clamping only meaningful for non-closed curves
		return bClampEnds && !bIsClosedLoop;
	}
...
protected:
	...
	**/
	 ...
	 * @return index of new knot if knot insertion was successful, INDEX_NONE otherwise.
	 */
	int32 InsertKnot(FKnot InKnot)
	{
		...
		while (Low <= High)
		{
			...
			{
				UE_LOGF(LogSpline, Warning, "Knot insertion failed, knot already exists!");
				return INDEX_NONE;
			}
		...
		MarkFlatKnotsCacheDirty();
		return PairKnots.Insert(InKnot, Low);
	}
	...
	struct FKnotSearchCache
	{
		explicit FKnotSearchCache(const TArray<FKnot>& PairKnots)
		{
	 ...
	 */
	float GetNearestAvailableKnotValue(const FValidKnotSearchParams& InSearchParams, const FKnotSearchCache& SearchCache) const
	{
		...
		if (!SearchCache.KnotValuesSet.Contains(Param::NormalizeKey(DesiredParameter)))
		{
			return DesiredParameter;
		}
		...
		for (;;)
		{
			...
			if (bSearchLeft)
			{
				...
				else
				{
					Left = NewLeft;
					AnyProgress = true;
					if (!SearchCache.KnotValuesSet.Contains(Param::NormalizeKey(Left)))
					{
						return Left;
					}
			...
			if (bSearchRight)
			{
				...
				else
				{
					Right = NewRight;
					AnyProgress = true;
					if (!SearchCache.KnotValuesSet.Contains(Param::NormalizeKey(Right)))
					{
						return Right;
					}
		...
		// Fallback: never return the conflicting original. Pick a deterministic, distinct nudge.
		{
			...
			if (SearchCache.KnotValuesSet.Contains(Probe))
			{
			...
			ensureMsgf(!SearchCache.KnotValuesSet.Contains(Probe), TEXT("Fallback still conflicts; check knot packing logic."));
			return Fallback;
		}
	}
	...
	void GenerateChordLengthKnots(int32 KnotCount)
	{
		...
		if (this->IsClosedLoop())
		{
			...
			for (int32 i = 0; i < NumKnots; ++i)
			{
				PairKnots.Add(FKnot(static_cast<float>(i) * TotalLength / static_cast<float>(n + d), 1));
			}
		}
		else if (bClampEnds)
		{
		...
		else
		{
			...
			for (int32 i = 0; i < KnotCount; ++i)
			{
				...
				PairKnots.Add(FKnot(Param * TotalLength, 1));
			}
		}
		MarkFlatKnotsCacheDirty();
	}
	 ...
	 * This ensures that the first and last knots have Degree + 1 multiplicity if the ends are clamped.
	 * For unclamped splines (including closed splines), first and last knots will have a multiplicity of Degree.
	 */
	void ApplyClampedKnotsMultiplicity()
	{
		...
		{
			PairKnots[0].Multiplicity = Degree;
			PairKnots.Last().Multiplicity = Degree;
		}
	...
	{
		TArray<FKnot> KnotVector = GetKnotVector();

		for (int32 Index = 0; Index < KnotVector.Num(); ++Index)
		{
			const FKnot& Knot = KnotVector[Index];
			UE_LOGF(LogSpline, Verbose, "  [%d] Value: %f, Multiplicity: %u", Index, Knot.Value, Knot.Multiplicity);
		}

		for (int32 j = 0; j < this->FlatKnots.Num(); ++j)
		{
			UE_LOGF(LogSpline, Verbose, "  FlatKnot[%d] = %f", j, this->FlatKnots[j]);
		}
	}
...
private:
	...
	int32 FindKnotSpan(float Parameter) const
	{
		...
		if (NumValues <= 0)
		{
			UE_LOGF(LogSpline, Error, "FindKnotSpan: Not enough knots to define a valid span.");
		...
		UE_LOGF(LogSpline, Warning, "Fallback in FindKnotSpan for t=%f", Parameter);
		return Degree;
	}

	virtual void FindWindow(float Parameter, FWindow& OutWindow, FWindowStore& Scratch) const
	{
		OutWindow = {};
		if (NumKeys() < Degree + 1)
		{
