...
public:
	...
	struct FComputedPoint
	{
		...
		bool bSeedPoint = false;
	};

	
	void EnumerateComputedPoint(TFunction<void(FComputedPoint)> InFunction)
	{
		for (const FGraphNode& Node : AllocatedNodes)
		{
			if (Node.bFrozen)
			{
				FComputedPoint PointInfo;
				PointInfo.PointID = Node.PointID;
				PointInfo.SeedIndex = Node.SeedPointID;
				PointInfo.GraphDistance = Node.GraphDistance;
				PointInfo.bSeedPoint = Node.ParentPointID == INDEX_NONE;
				
				InFunction(MoveTemp(PointInfo));
			}
		}	
	}
