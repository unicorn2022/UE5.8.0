		...
		for (int32 Level(0); Level < 32; ++Level)
		{
			...
			auto TagForRefine = [&](const int32 Idx)
			{
				...
				if (ShouldRefine(TriIndex, Level))
				{
					...
					{
						// for more gradual mesh regularity, tag neighbors for refinement.
						for (int NbIdx=0; NbIdx<3; ++NbIdx)
						{
							const int32 AdjTriIndex = Mesh.GetAdjTriangle(TriIndex, NbIdx);
							if (AdjTriIndex != IndexConstants::InvalidID && RefinementPolicy.AllowRefine(AdjTriIndex, Level) && 
								!ConcurrencyState.TriSplitRecords[AdjTriIndex].bClaimed.test_and_set())
							{
