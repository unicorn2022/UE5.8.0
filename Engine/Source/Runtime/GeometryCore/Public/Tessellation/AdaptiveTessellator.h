...
enum class EAdaptiveTessellationRefinementMethod
{
	// builtin Nanite refinement strategy, with optimized/sampled split location
	HierarchicalSplit,
	
	// custom callback for refinement and refinement location
	Custom
};
...
struct FAdaptiveTessellatorOptions
{
	...
	// one-dimensional rate (edge-length) at which features are sampled. triangles with edge-length smaller than
	...
	// upper bound on the number of triangles of the tessellated mesh. Note that reaching this limit, the algorithm
	// will not guarantee an optimal use of the triangle budget. It is purely for safe-guard purposes.
	//
	uint32 MaxTriangles { 10'000'000u };

	EAdaptiveTessellationRefinementMethod RefinementMethod { EAdaptiveTessellationRefinementMethod::HierarchicalSplit };
...
private:
	...
	inline static uint32 HashPosition(const FVector3d& Position)
	{
		...
		return static_cast<uint32>(Murmur64({
  ...
  public:
	...
	TAdaptiveTessellator(
		...
		const FAdaptiveTessellatorOptions& InOptions)                   
		: Mesh(InMesh)
...
private:
	inline bool CouldFlipEdge( int32 TriIndex, int32 TriEdgeIndex ) const 
	{
		...
		return ( TriNormal | AdjNormal ) > RealType(Options.CosineThreshold);
	}
...
inline void TAdaptiveTessellator<MeshT, DisplacementPolicyT>::FindSplitBVH( uint32 TriIndex )
{
	...
	const RealType SampleArea = static_cast<RealType>(EquilateralArea( Options.SampleRate ));
			...
			*/
			...
			if( ChildNode.ErrorMax > BestError * 1.25f + Options.TargetError )
			{
				...
				if( !bStopTraversal )
				{
					...
					const bool bSmallEnough = ChildArea < SampleArea * RealType(16.);
					NumSamples = FMath::Min( NumSamples, static_cast<int32>(FMath::CeilToInt( ChildArea / SampleArea )) );
...
inline void TAdaptiveTessellator<MeshT, DisplacementPolicyT>::CheckSplitAndEnqueue( const int32 TriIndex, const int32 Level )
{
	...
	if ( Options.RefinementMethod == EAdaptiveTessellationRefinementMethod::HierarchicalSplit )
	{
