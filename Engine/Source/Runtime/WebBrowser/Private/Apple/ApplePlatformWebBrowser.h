...
#endif
{
	...
	bool bUsingIndirectRendering;
	bool bAllowScreenshots;
	// needs to be atomic as it is set on the game thread
	// but read from AppKit/UIKit's UI thread during WebKit
	// callbacks
	std::atomic<bool> bShuttingDown;
	bool bReadyToAddToNativeLayer;
}
#if !PLATFORM_TVOS
...
@property(strong) NSImage* LastWebViewScreenshot;
#else
@property(strong) WebViewCloseButton* CloseButton;
@property(strong) UIView* WebViewContainer;
@property(strong) UIImage* LastWebViewScreenshot;
#endif
