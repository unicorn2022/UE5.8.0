...
void FCEFBrowserApp::OnBeforeCommandLineProcessing(const CefString& ProcessType, CefRefPtr< CefCommandLine > CommandLine)
{
	...
	{
		UE_LOGF(LogCEFBrowser, Log, "CEF GPU acceleration enabled");
	...
	{
		UE_LOGF(LogCEFBrowser, Log, "CEF GPU acceleration disabled");
...
bool FCEFBrowserApp::TickMessagePump(float DeltaTime, bool bForce)
{
	...
	if (bPump || bForce)
	{
		...
		{
			UE_LOGF(LogCEFBrowser, Log, "CefDoMessageLoopWork call Frame Rate %0.2f", NumFrames / std::max(SecondsFrameRate, 0.01f));
			SecondsFrameRate = 0;
			NumFrames = 0;
		}
		...
		UE_LOGF(LogCEFBrowser, Verbose, "FCEFBrowserApp::TickMessagePump %u ms %u ms %ls", static_cast<uint32_t>((BeforeSec - LastTickTime) * 1000), static_cast<uint32_t>((AfterSec - BeforeSec) * 1000), bForce ? TEXT("FORCED") : TEXT(""));
