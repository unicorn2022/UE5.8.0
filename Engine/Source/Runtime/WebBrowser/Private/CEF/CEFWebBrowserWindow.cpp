...
FCEFWebBrowserWindow::FCEFWebBrowserWindow(CefRefPtr<CefBrowser> InBrowser, CefRefPtr<FCEFBrowserHandler> InHandler, FString InUrl, TOptional<FString> InContentsToLoad, FString InUserAgentProduct, bool bInShowErrorMessage, bool bInThumbMouseButtonNavigation, bool bInUseTransparency, bool bInJSBindingToLoweringEnabled, bool bInUsingAcceleratedPaint)
...
FCEFWebBrowserWindow::~FCEFWebBrowserWindow()
{
	...
	UE_LOGF(LogWebBrowser, Log, "Deleting browser for Url=%ls.", *CurrentUrl);
}
...
#define LETTER_KEY_MACRO(val, vkey) else if (Key == EKeys::val) \
	...
	//UE_LOGF(LogWebBrowser, Log, "Modifiers: %i %i %i" , OutKeyEvent.unmodified_character, OutKeyEvent.windows_key_code, OutKeyEvent.modifiers);
...
bool FCEFWebBrowserWindow::OnFileDialog(CefDialogHandler::FileDialogMode Mode, const CefString& DialogTitle, const CefString& DefaultFilePath, const std::vector<CefString>& AcceptFilters, const std::vector<CefString>& AcceptExtensions, const std::vector<CefString>& AcceptDescriptions, CefRefPtr<CefFileDialogCallback> Callback)
{
#if WITH_EDITOR
	return false;
#else
	bool bAllowFileDialogsInClientBuilds = false;
	GConfig->GetBool(TEXT("Browser"), TEXT("bAllowFileDialogsInClientBuilds"), bAllowFileDialogsInClientBuilds, GEngineIni);
	if (!bAllowFileDialogsInClientBuilds)
	{
		UE_LOGF(LogWebBrowser, Warning, "FileDialogs are prevented by default in client builds. Set bAllowFileDialogsInClientBuilds=true under [Browser] in Engine.ini to enable.");
		return true;
	}
	return false;
#endif
	...
	if (IsValid())
	{
		...
		if (bBlockTillClosed && ensure(FSlateApplication::IsInitialized()))
		{
			...
			while (InternalCefBrowser != nullptr)
			{
				...
				{
					UE_LOGF(LogWebBrowser, Error, "CloseBrowser - took more than %0.2f second to close. Abandoning wait...", CloseWaitTimeout);
...
void FCEFWebBrowserWindow::OnPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const void* Buffer, int Width, int Height)
{
	...
	if (Type == PET_VIEW)
	{
		...
		{
			UE_LOGF(LogWebBrowser, Verbose, "FCEFWebBrowserWindow::OnPaint: Frame size still wrong after %zu frames: %dx%d -> %dx%d (%d,%d %dx%d)", NumFramesSinceViewportResize, ViewportRect.Width(), ViewportRect.Height(), Width, Height, DirtyRect.Min.X, DirtyRect.Min.Y, DirtyRect.Width(), DirtyRect.Height());
		}
		else if (!bWasCorrectNativeCefBuffer)
		{
			UE_LOGF(LogWebBrowser, Verbose, "FCEFWebBrowserWindow::OnPaint: Frame size OK again after %zu frames: %dx%d -> %dx%d (%d,%d %dx%d)", NumFramesSinceViewportResize, ViewportRect.Width(), ViewportRect.Height(), Width, Height, DirtyRect.Min.X, DirtyRect.Min.Y, DirtyRect.Width(), DirtyRect.Height());
		}
		...
		if (CapturedCefFrame.IsValid())
		{
			...
			{
				UE_LOGF(LogWebBrowser, Verbose, "FCEFWebBrowserWindow::OnPaint: Detected corrupted CEF frame!");
	...
	if (bUsingAcceleratedPaint)
	{
		UE_LOGF(LogWebBrowser, Error, "Accelerated CEF rendering selected but OnPaint called. Disabling accelerated rendering for this browser window.");
...
void FCEFWebBrowserWindow::OnAcceleratedPaint(CefRenderHandler::PaintElementType Type, const CefRenderHandler::RectList& DirtyRects, const CefAcceleratedPaintInfo& Info)
{
	...
	if (!bUsingAcceleratedPaint)
	{
		UE_LOGF(LogWebBrowser, Error, "Accelerated CEF rendering NOT selected but OnAcceleratedPaint called. Enabling accelerated rendering for this browser window.");
...
#  endif
	...
	if (Type == PET_VIEW)
	{
		...
		if (!bHasCorrectNativeCefBuffer)
		{
			UE_LOGF(LogWebBrowser, Verbose, "FCEFWebBrowserWindow::OnAcceleratedPaint: Frame size still wrong after %zu frames: %dx%d -> %dx%d (%d, %d)", NumFramesSinceViewportResize, ViewportRect.Width(), ViewportRect.Height(), SourceRect.Width(), SourceRect.Height(), SourceRect.Min.X, SourceRect.Min.Y);
		}
		else if (!bWasCorrectNativeCefBuffer)
		{
			UE_LOGF(LogWebBrowser, Verbose, "FCEFWebBrowserWindow::OnAcceleratedPaint: Frame size OK again after %zu frames: %dx%d -> %dx%d (%d, %d)", NumFramesSinceViewportResize, ViewportRect.Width(), ViewportRect.Height(), SourceRect.Width(), SourceRect.Height(), SourceRect.Min.X, SourceRect.Min.Y);
		}
...
void FCEFWebBrowserWindow::SetCaptureCefFrames(const bool bValue)
{
#if UE_CEF_HAS_RESIZE_BUG
	if (bValue)
	{
		if (!CapturedCefFrame.IsValid())
		{
			CapturedCefFrame = MakeShared<FCEFCapturedFrame>();
		}
	}
	else
	{
...
bool FCEFWebBrowserWindow::IsCefResizeNeeded() const
{
	if (!bHasCorrectNativeCefBuffer && NumFramesSinceViewportResize >= MIN_FRAMES_FOR_CEF_RESIZE)
	{
		UE_LOGF(LogWebBrowser, Verbose, "FCEFWebBrowserWindow::IsCefResizeNeeded: Forcing CEF resize");
		return true;
	}
	return false;
}

void FCEFWebBrowserWindow::RequestCefResize()
{
	if (!IsValid())
	{
		return;
...
bool FCEFWebBrowserWindow::OnProcessMessageReceived(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> frame, CefProcessId SourceProcess, CefRefPtr<CefProcessMessage> Message)
{
	if (IsClosing())
	{
		UE_LOGF(LogWebBrowser, Warning, "Received a process message for a browser window in closing state, ignoring.");
		return false;
	}
	if (!IsValid())
	{
		UE_LOGF(LogWebBrowser, Warning, "Received a process message for a browser window in invalid state, ignoring.");
		return false;
	}
	if (!Browser || !Browser->IsValid())
	{
		UE_LOGF(LogWebBrowser, Warning, "Received a process message with an invalid browser state, ignoring.");
		return false;
	}
...
void FCEFWebBrowserWindow::OnTextSelectionChanged(
	CefRefPtr<CefBrowser> Browser,
...
void FCEFWebBrowserWindow::OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	if (!IsValid())
	{
		return;
...
FReply FCEFWebBrowserWindow::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	if (!IsValid())
	{
...
void FCEFWebBrowserWindow::OnDragLeave(const FDragDropEvent& DragDropEvent)
{
	if (!IsValid())
	{
		return;
...
FReply FCEFWebBrowserWindow::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	if (!IsValid())
	{
		return FReply::Unhandled();
