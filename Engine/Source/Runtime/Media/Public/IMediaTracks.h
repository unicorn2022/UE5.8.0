...
public:
	 ...
	 *
	 * If a media file contains multiple tracks each track may have different duration and
	 * the overall media duration is typically reported as that from the longest track.
	 *
	 * For some applications it may be required to know the exact track duration for processing,
	 * which is what this method returns if the media container, as well as the media player
	 * used for playback has the ability to provide it.
	 *
	 * @param TrackType The type of track to query.
	 * @param TrackIndex The index of the track to query.
	 * @return The duration of the track if known.
	 */
	virtual TOptional<FTimespan> GetTrackDuration(EMediaTrackType TrackType, int32 TrackIndex) const
