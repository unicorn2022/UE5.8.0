...
{
	/* UE's historical reference white level for mapping scene-referred colors to nits (see TonemapCommon.ush). */
	UE_DEPRECATED(5.8, "Use UE::Color::GetReferenceWhiteLinearScale for pipeline normalization; any additional white-level adjustment should happen in the consuming material.")
	...
	UE_DEPRECATED(5.8, "Use UE::Color::GetReferenceWhiteLinearScale(EEncoding, EReferenceWhite::BT2408) for pipeline normalization; any additional white-level adjustment should happen in the consuming material.")
	static constexpr float kLinearToNitsScale_BT2408 = 203.0f;
}

UE_DEPRECATED(5.8, "Use UE::Color::GetReferenceWhiteLinearScale(EEncoding, EReferenceWhite) for pipeline normalization; any additional white-level adjustment should happen in the consuming material.")
...
public:
	...
	/**
	 * Extra ETextureCreateFlags to OR into the intermediate render target for this
	 * sample. Lets samples opt into create-time policies (e.g. ReservedResource for
	 * sparse residency) without an RHI-wide engine change. Default: no extra flags.
	 */
	virtual ETextureCreateFlags GetIntermediateTargetCreateFlagsHint() const
	{
		return static_cast<ETextureCreateFlags>(0);
	}
	...
	*/
	...
	/**
	 * Linear-side scale that consumers may apply to normalize decoded transfer-function
	 * output to UE scene-linear (diffuse white = 1.0). Consumers typically multiply the
	 * color-space conversion matrix by this factor; how and when it is applied is up to
	 * the consumer.
	 *
	 * Subclasses should override this when the source metadata indicates a specific
	 * reference white. The canonical implementation calls
	 * UE::Color::GetReferenceWhiteLinearScale(GetEncodingType(), EReferenceWhite::XXX)
	 * so the scale resolves per encoding (e.g. 1/203 for PQ + BT2408, 1/DecodeHLG(0.75)
	 * for HLG + BT2408). The base default returns 1.0 so projects without tagging see
	 * spec-literal decoded values.
	 */
...
public:
	...
	/** Virtual destructor. */
	virtual ~IMediaTextureSample() = default;
};
