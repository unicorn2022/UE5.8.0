...
static const uint32						GPoolLimitTimeoutMs	= 4 * 1000;
#undef T_ALIGN
...
////////////////////////////////////////////////////////////////////////////////
FWriteBuffer* Writer_GetPanicPool()
{
	static FWriteBuffer* SPanicBuffer = []()
	{
		uint8 *Allocation = (uint8 *)Writer_MemoryAllocate(
			GPoolBlockSize, PLATFORM_CACHE_LINE_SIZE);
#if TRACE_PRIVATE_STATISTICS
		AtomicAddRelaxed(&GTraceStatistics.BlockPoolAllocated, uint64(GPoolBlockSize));
#endif
		uint32 BufferSize = GPoolBlockSize - sizeof(FWriteBuffer);
		FWriteBuffer* PanicBuffer = (FWriteBuffer *)(Allocation + BufferSize);
		static_assert(GPoolBlockSize < UINT16_MAX);
		PanicBuffer->CursorMaskInv = 0;
		PanicBuffer->Size = uint16(BufferSize);
		PanicBuffer->NextBuffer = nullptr;
		PanicBuffer->NextThread = nullptr;
		PanicBuffer->SetCursor(Allocation);
		PanicBuffer->Reaped = Allocation;
		PanicBuffer->EtxOffset = 0 - int32(sizeof(FWriteBuffer));
		// This mask will cause the cursor to stay at the start of
		// the buffer, without ever increasing
		PanicBuffer->CursorMaskInv = ~0;
		return PanicBuffer;
	}();
	return SPanicBuffer;
...
FWriteBuffer* Writer_AllocateBlockFromPool()
{
	...
	while (true)
	{
		...
		if ((AtomicLoadAcquire(&GPoolTotalAllocated) >= GPoolMaxSize))
		{
			...
			StallStart = StallStart ? StallStart : Ts;
			uint32 DurationMs = uint32(1000.f*(float(Ts - StallStart) / float(TimeGetFrequency())));
			if (DurationMs > GPoolLimitTimeoutMs)
			{
				// At this point we have timed out waiting for a free block.
				// This likely means the worker thread cannot keep up with
				// tracing bandwidth or that I/O is stalled.
				Panic();
				Ret = Writer_GetPanicPool();
				break;
			}
			continue;
		}
