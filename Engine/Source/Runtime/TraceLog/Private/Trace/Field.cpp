...
static void Field_WriteAuxData(uint32 Index, int32 Size, CallbackType&& Callback)
{
	...
	bool bCommit = (Buffer->ReadCursor() == Buffer->Committed);

	// Do we have enough space to write at least [header][overflow-1] bytes of payload?
	int32 Remaining = int32(ptrdiff_t((uint8*)Buffer - Buffer->ReadCursor()));
	auto NextBuffer = [&Buffer, &Remaining, &bCommit] ()
	{
		if (bCommit)
		{
			AtomicStoreRelease(&(uint8* volatile&)(Buffer->Committed), Buffer->ReadCursor());
		}
		bCommit = true;

		Buffer = Writer_NextBuffer(Buffer);
		Remaining = int32(ptrdiff_t((uint8*)Buffer - Buffer->ReadCursor()));
	};
	...
	while (true)
	{
		...
		memcpy(Buffer->ReadCursor(), &Pack, sizeof(uint32)); /* FAuxHeader::Pack */
		*Buffer->ReadCursor() = uint8(EKnownEventUids::AuxData) << EKnownEventUids::_UidShift; /* FAuxHeader::Uid */
		Buffer->MoveCursor(sizeof(FAuxHeader));

		// Write payload data
		Callback(Buffer->ReadCursor(), SegmentSize);
		Buffer->MoveCursor((uint16)SegmentSize);
