...
////////////////////////////////////////////////////////////////////////////////
static bool TcpSocketSetNonBlockingInternal(int Socket, bool bNonBlocking)
{
...
{
	int Inner = TcpSocketNative(Socket);
...
UPTRINT TcpSocketConnect(const ANSICHAR* Host, uint16 Port)
{
	...
	if (!TcpSocketSetNonBlockingInternal(Socket, false))
	{
		close(Socket);
		return 0;
	}
...
UPTRINT TcpSocketListen(uint16 Port)
{
	...
	if (!TcpSocketSetNonBlockingInternal(Socket, true))
	{
		close(Socket);
		return 0;
	}
...
bool TcpSocketPeerName(UPTRINT Socket, char* Name, uint32 NameSize)
{
	...
	getpeername(Inner, (sockaddr*) &Addr, &AddrLen);
