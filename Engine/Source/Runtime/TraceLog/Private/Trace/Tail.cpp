...
public:
	...
	void			IterateRanges(CallbackType&& Callback, uint32 MaxSize);
...
////////////////////////////////////////////////////////////////////////////////
template <typename CallbackType>
void FPacketRing::IterateRanges(CallbackType&& Callback, uint32 MaxSize)
{
	FPacketRing::FRange Ranges[] = { GetBackPackets(), GetFrontPackets() };

	int32 BytesToSkip = 0;
	if (MaxSize != 0 && MaxSize < INT32_MAX)
	{
		const uint32 TotalSize = Ranges[0].Size + Ranges[1].Size;
		const uint32 TotalBytesToSkip = MaxSize > TotalSize ? 0 : TotalSize - MaxSize;
		BytesToSkip = TotalBytesToSkip;
	}
	...
	for (const auto& Range : Ranges)
	{
		...
		uint32 RangeSkipped = 0;
		uint8* RangeOffset = (uint8*) Range.Data;
		while (BytesToSkip > 0 && RangeOffset < ((uint8*)Range.Data + Range.Size))
		{
			// We need to skip over entire packages
			FTidPacketBase* Packet = (FTidPacketBase*) RangeOffset;
			BytesToSkip -= Packet->PacketSize;
			RangeSkipped += Packet->PacketSize;
			RangeOffset = RangeOffset + Packet->PacketSize;
		} 

		FPacketRing::FRange RangeView = {RangeOffset, Range.Size - RangeSkipped};
		Callback(RangeView);
	}
}
...
void Writer_TailAppend(uint32 ThreadId, uint8* __restrict Data, uint32 Size)
{
	...
	// We expect supplied thread id to not contain any flags
	...
	if (Size > 384)
	{
		...
		FTidPacketEncoded* Packet = GPacketRing.Append<FTidPacketEncoded>(EncodeMaxSize);
		if (Packet == nullptr)
		{
			// FPacketRing::AppendImpl resets if the packet is too large,
			return Writer_SendData(ThreadId, Data, Size);
		}
		...
		int32 EncodedSize = EncodeInstr(Data, Size, Packet->Data, EncodeMaxSize);
		if (EncodedSize > 0)
		{
...
////////////////////////////////////////////////////////////////////////////////
void Writer_TailOnConnect(uint32 MaxBytes /*= 0*/)
{
	FProfilerScope _(__func__);
