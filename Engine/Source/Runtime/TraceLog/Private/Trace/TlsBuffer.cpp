...
FWriteBuffer*		Writer_NextBuffer(FWriteBuffer* CurrentBuffer);
...
static FWriteBuffer						GNullWriteBuffer	= { {}, static_cast<uint16>(~0u), 0, 0, 0, 0, nullptr, nullptr, nullptr, nullptr, (uint8*)&GNullWriteBuffer };
thread_local FTlsState					GTlsState = {&GNullWriteBuffer, nullptr};
...
{
	FWriteBuffer* OnConnectBuffer = FOnConnectRedirectScope::GetBuffer();
	if (OnConnectBuffer == nullptr)
	{
		return false;
	}
	OutPrevState = GTlsState;
	GTlsState.WriteBuffer = OnConnectBuffer;
	GTlsState.NextBufferFn = &FOnConnectRedirectScope::NextBuffer;
	return true;
}
...
static FWriteBuffer* Writer_NextBufferInternal(FWriteBuffer* CurrentBuffer)
{
	...
	TBoundedRedirectScope<2048> TraceData;
	...
	NextBuffer->SetCursor((uint8*)NextBuffer - NextBuffer->Size);
	NextBuffer->Reaped = NextBuffer->ReadCursor();
	...
	{
		memcpy(NextBuffer->ReadCursor(), TraceData.GetData(), RedirectSize);
		NextBuffer->MoveCursor((uint16)RedirectSize);
	}
	...
	NextBuffer->Committed = NextBuffer->ReadCursor();
	...
	else
	{
		...
		const uint64 Used = CurrentBuffer->ReadCursor() - ((uint8*)CurrentBuffer - CurrentBuffer->Size);
		...
		int32 EtxOffset = int32(PTRINT((uint8*)(CurrentBuffer) - CurrentBuffer->ReadCursor()));
...
////////////////////////////////////////////////////////////////////////////////
TRACELOG_API FWriteBuffer* Writer_NextBuffer(FWriteBuffer* CurrentBuffer)
{
	// Allow us to redirect next buffer function (e.g. growable redirects). If
	// not set fall back to the default implementation.
	if (GTlsState.NextBufferFn)
	{
		return GTlsState.NextBufferFn(CurrentBuffer);
	}
...
{
	if (GTlsState.WriteBuffer == &GNullWriteBuffer)
	{
		return;
	}

	const uint32 LocalThreadId = GTlsState.WriteBuffer->ThreadId;
...
////////////////////////////////////////////////////////////////////////////////
void Writer_DrainBufferList(uint32 ThreadId, FWriteBuffer* List)
{
	FWriteBuffer* Buffer = List;
	do {
		uint8 *Committed = AtomicLoadAcquire((uint8 **)&Buffer->Committed);
		...
		{
#if TRACE_PRIVATE_STATISTICS
			AtomicAddRelaxed(&GTraceStatistics.BytesTraced, uint64(SizeToReap));
#endif

			Writer_SendData(ThreadId, Buffer->Reaped, SizeToReap);
			Buffer->Reaped = Committed;
		}
	} while ((Buffer = Buffer->NextBuffer));
}
...
void Writer_EndThreadBuffer()
{
	...
	const PTRINT CurrentCursor = (PTRINT)GTlsState.WriteBuffer->ReadCursor();
	int32 EtxOffset = int32(PTRINT((uint8*)GTlsState.WriteBuffer - CurrentCursor));
	AtomicStoreRelease(&(GTlsState.WriteBuffer->EtxOffset), EtxOffset);
}
