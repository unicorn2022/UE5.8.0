...
#endif // TRACE_PRIVATE_FULL_ENABLED
...
FChannel::Iter::~Iter()
{
	...
	{
		FChannel* OldHead = AtomicLoadRelaxed(&GHeadChannel);
		AtomicStoreRelaxed(&(Node->Next), OldHead);
		if (AtomicCompareExchangeRelaxed(&GHeadChannel, (FChannel*)Inner[1], OldHead))
		{
			break;
		}
	}
...
const FChannel* FChannel::Iter::GetNext()
{
	...
	auto* Ret = (FChannel*)Inner[0];
	if (Ret != nullptr)
	{
		Inner[0] = AtomicLoadRelaxed(&Ret->Next);
		if (Inner[0] != nullptr)
		{
			Inner[2] = Inner[0];
		}
...
void FChannel::Setup(const ANSICHAR* InChannelName, const InitArgs& InArgs)
{
	...
	for (;; PlatformYield())
	{
		...
		AtomicStoreRelaxed(&Next, HeadChannel);
...
void FChannel::ToggleAllInternal(bool bEnabled, bool bIssueCallback)
{
	...
	{
		for (; Channel != nullptr; Channel = AtomicLoadRelaxed(&Channel->Next))
		{
