...
/**
 * Control tracing to secure socket connections.
 */
#if !defined(UE_TRACE_ALLOW_SECURE_TRACING)
#	define UE_TRACE_ALLOW_SECURE_TRACING 0
#	define TRACE_PRIVATE_ALLOW_SECURE_TRACING 0
#else
#	define TRACE_PRIVATE_ALLOW_SECURE_TRACING UE_TRACE_ALLOW_SECURE_TRACING
#endif
...
/**
 * Default Tail buffer size in bytes. Defaults to 0 when Minimal Tracing.
 */
#if !defined(UE_TRACE_DEFAULT_TAIL_SIZE)
#   if TRACE_PRIVATE_FULL_ENABLED
#       define UE_TRACE_DEFAULT_TAIL_SIZE 4 << 20
#   else
#       define UE_TRACE_DEFAULT_TAIL_SIZE 0
#   endif
#endif
 ...
 */
#if !defined(UE_TRACE_PACKET_VERIFICATION)
#	define UE_TRACE_PACKET_VERIFICATION 0
#endif

/**
 * Enable tracking of memory and bandwidth statistics.
 */
#if !defined(UE_TRACE_STATISTICS_ENABLED)
#	define UE_TRACE_STATISTICS_ENABLED (!(UE_BUILD_SHIPPING || USING_THREAD_SANITISER)) 
#endif

/**
 * Enable built in events to send statistics. Enabled by default
 * when statistics tracking is enabled.
 */
#if !defined(UE_TRACE_EMIT_STATISTICS)
#	define UE_TRACE_EMIT_STATISTICS UE_TRACE_STATISTICS_ENABLED
#endif

/**
 * Assert functionality. When integrating TraceLog define this to let it use 
 * built in assert functionality. The format string is specified using wide char.
 *
 * @note: Asserts must only be used in inlined code due linking.
 * @note: See UE checkf for expected format
 * @note: In UE with non-monolithic targets, TraceLog module cannot use asserts. So
 * we use the suppress macro to reset any global defines.
 */
#if defined(UE_TRACE_SUPPRESS_ASSERT)
#	undef UE_TRACE_ASSERT_FMT
#endif
#if !defined(UE_TRACE_ASSERT_FMT)
#	define	UE_TRACE_ASSERT_FMT(Cond, Fmt, ...)
#endif
