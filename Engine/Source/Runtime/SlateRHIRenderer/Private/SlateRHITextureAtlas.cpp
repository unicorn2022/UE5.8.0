...
void FSlateTextureAtlasRHI::UpdateTexture_RenderThread(FRHICommandListImmediate& RHICmdList, FSlateTextureData* RenderThreadData)
{
	check(IsInRenderingThread());

	if (!AtlasTexture->IsInitialized())
	{
		AtlasTexture->InitResource(RHICmdList);
	}
	...
	{
		uint32 DestStride;
		uint8* TempData = (uint8*)RHICmdList.LockTexture2D(AtlasTexture->GetTypedResource(), 0, RLM_WriteOnly, /*out*/ DestStride, false);
		...
		if ( TempData != nullptr )
		{
			...
			RHICmdList.UnlockTexture2D(AtlasTexture->GetTypedResource(), 0, false);
		}
	}
...
void FSlateTextureAtlasRHI::ConditionalUpdateTexture( )
{
	...
	if (bNeedsUpdate)
	{
		...
		ENQUEUE_RENDER_COMMAND(SlateUpdateAtlasTextureCommand)(
			...
			{
				Atlas->UpdateTexture_RenderThread(RHICmdList, RenderThreadData);
			});
