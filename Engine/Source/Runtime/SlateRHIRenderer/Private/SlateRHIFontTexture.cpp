...
void FSlateFontAtlasRHI::ConditionalUpdateTexture()
{
	if( bNeedsUpdate )
	{
		if (IsInRenderingThread())
		{
			...
			FontTexture->InitResource(RHICmdList);

			uint32 DestStride;
			uint8* TempData = (uint8*)RHICmdList.LockTexture2D( FontTexture->GetTypedResource(), 0, RLM_WriteOnly, /*out*/ DestStride, false );
			...
			RHICmdList.UnlockTexture2D( FontTexture->GetTypedResource(),0,false );
		}
		else
		{
			checkSlow( IsThreadSafeForSlateRendering() );
			...
			ENQUEUE_RENDER_COMMAND(SlateUpdateFontAtlasTextureCommand)(
				...
				{
					uint32 DestStride;
					uint8* TempData = (uint8*)RHICmdList.LockTexture2D( Atlas->FontTexture->GetTypedResource(), 0, RLM_WriteOnly, /*out*/ DestStride, false );
					// check( DestStride == Atlas.BytesPerPixel * Atlas.AtlasWidth ); // Temporarily disabling check
					...
					RHICmdList.UnlockTexture2D( Atlas->FontTexture->GetTypedResource(),0,false );
				});
		}
	...
	: FontTexture(new FSlateFontTextureRHIResource(InWidth, InHeight, InContentType))
{
	if (IsInRenderingThread())
	{
		...
		FontTexture->InitResource(RHICmdList);
		UpdateTextureFromSource(RHICmdList, InWidth, InHeight, InRawData);
	}
	else
	{
		checkSlow(IsThreadSafeForSlateRendering());
		...
		ENQUEUE_RENDER_COMMAND(SlateUpdateFontTextureCommand)(
			...
			{
				This->UpdateTextureFromSource(RHICmdList, This->PendingSourceData->SourceWidth, This->PendingSourceData->SourceHeight, This->PendingSourceData->SourceData);
...
void FSlateFontTextureRHI::UpdateTextureFromSource(FRHICommandListImmediate& RHICmdList, const uint32 SourceWidth, const uint32 SourceHeight, const TArray<uint8>& SourceData)
{
	const uint32 BytesPerPixel = GetSlateFontAtlasContentBytesPerPixel(GetContentType());

	uint32 DestStride;
	uint8* LockedTextureData = static_cast<uint8*>(RHICmdList.LockTexture2D(FontTexture->GetTypedResource(), 0, RLM_WriteOnly, /*out*/ DestStride, false));
