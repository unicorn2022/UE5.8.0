	...
	uint32& OutWidth, uint32& OutHeight, TArray<uint8>& OutDecodedImage )
{
	...
	{
		UE_CLOGF(!IsRunningCommandlet(), LogSlate, Log, "Could not load file for Slate resource: [%ls] file: [%ls]", *TextureName.ToString(),*ResourcePath );
		return false;
	}
...
static void LoadUObjectForBrush( const FSlateBrush& InBrush )
{
	...
	if (!Path.IsEmpty() && Path.StartsWith(FSlateBrush::UTextureIdentifier()))
	{
		...
		if( !TextureObject )
		{
			UE_LOGF(LogSlate, Warning, "Error loading UTexture from path: %ls not found", *Path);
		...
		UE_LOGF(LogSlate, Warning, "The texture:// method of loading UTextures for use in Slate is deprecated.  Please convert %ls to a Brush Asset", *Path);
	}
}
...
FSlateShaderResourceProxy* FSlateRHIResourceManager::GetShaderResource(const FSlateBrush& InBrush, FVector2f LocalSize, float DrawScale)
{
	...
	{
		UE_LOGF(LogSlate, Warning, "Attempted to access resource for %ls which is pending kill, unreachable or pending destroy", *ResourceObject->GetName());
	}
	else
	{
...
FSlateShaderResource* FSlateRHIResourceManager::GetFontShaderResource( int32 InTextureAtlasIndex, FSlateShaderResource* FontTextureAtlas, const class UObject* FontMaterial )
{
	...
	else if (!FontMaterial->IsA<UMaterialInterface>())
	{
		...
		{
			UE_LOGF(LogSlate, Fatal, "%ls", *ErrorMsg);
		}
		else
		{
