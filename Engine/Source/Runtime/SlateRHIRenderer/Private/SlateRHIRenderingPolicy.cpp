...
static TAutoConsoleVariable<int32> CVarSlateMaterialShaderMode(
	TEXT("slate.MaterialShaderMode"),
	0,
	TEXT("Whether to compile fully specialized slate material shaders or use dynamic branches. Changing this variable affects which shaders are compiled during a cook and are available at runtime.")
	TEXT(" 0: Fully static specialized shaders (best GPU performance, default)\n")
	TEXT(" 1: Hybrid approach, with specialization for Default shader type and dynamic branch for the rest (balance between performance and shader count)\n")
	TEXT(" 2: Only use dynamic branch version of the shaders (lowest shader count)\n"),
	ECVF_ReadOnly);

static TAutoConsoleVariable<int32> CVarSlateMobileMaterialShaderMode(
	TEXT("slate.Mobile.MaterialShaderMode"),
	-1,
	TEXT("Mobile renderer specific override for slate.MaterialShaderMode.")
	TEXT(" -1: No mobile override. Uses value from slate.MaterialShaderMode (default).\n")
	TEXT(" [0-2]: See mode descriptions from slate.MaterialShaderMode.\n"),
	ECVF_ReadOnly);

static int32 GetSlateMaterialShaderMode(EShaderPlatform ShaderPlatform)
{
	static FShaderPlatformCachedIniValue<int32> PerPlatformCVar(TEXT("slate.MaterialShaderMode"));
	const int32 Mode = PerPlatformCVar.Get(ShaderPlatform);
	static FShaderPlatformCachedIniValue<int32> MobilePerPlatformCVar(TEXT("slate.Mobile.MaterialShaderMode"));
	const int32 MobileMode = MobilePerPlatformCVar.Get(ShaderPlatform);

	const bool bIsMobile = IsMobilePlatform(ShaderPlatform);
	return bIsMobile && MobileMode >= 0 ? MobileMode : Mode;
}

bool IsValidSlateMaterialShaderType(ESlateShader ShaderType, EShaderPlatform ShaderPlatform)
{
	const int32 SlateMaterialShaderMode = GetSlateMaterialShaderMode(ShaderPlatform);
	switch (ESlateMaterialShaderMode(SlateMaterialShaderMode))
	{
	case ESlateMaterialShaderMode::Static:
		return ShaderType != ESlateShader::Dynamic;
	case ESlateMaterialShaderMode::Hybrid:
		return  ShaderType == ESlateShader::Default
			 || ShaderType == ESlateShader::Dynamic;
	case ESlateMaterialShaderMode::Dynamic:
		return ShaderType == ESlateShader::Dynamic;
	default:
		return true;
	}
...
bool ChooseMaterialShaderTypes(ESlateShader ShaderType, EShaderPlatform ShaderPlatform, bool bUseInstancing, FMaterialShaderTypes& OutShaderTypes)
{
	const int32 SlateMaterialShaderMode = GetSlateMaterialShaderMode(ShaderPlatform);
	switch (ESlateMaterialShaderMode(SlateMaterialShaderMode))
	{
	...
	case ESlateMaterialShaderMode::Hybrid:
		...
		{
			OutShaderTypes.AddShaderType<TSlateMaterialShaderPS<ESlateShader::Dynamic>>();
		}
		break;
	case ESlateMaterialShaderMode::Dynamic:
		OutShaderTypes.AddShaderType<TSlateMaterialShaderPS<ESlateShader::Dynamic>>();
...
FRHIBlendState* GetMaterialBlendState(FSlateShaderResource* TextureMaskResource, const FMaterial* Material)
{
	if (TextureMaskResource && IsOpaqueOrMaskedBlendMode(*Material))
	{
		...
		return TStaticBlendState<CW_RGBA, BO_Add, BF_SourceAlpha, BF_InverseSourceAlpha, BO_Add, BF_One, BF_InverseSourceAlpha>::GetRHI();
	}
	else
	{
	...
	const FSlateClippingOp* ClippingStateOp)
{
	...
	if (ResourceType == ESlateShaderResource::Material)
	{
		...
		const EShaderPlatform SceneShaderPlatform = GetFeatureLevelShaderPlatform(SceneFeatureLevel);
		...
		if (!ChooseMaterialShaderTypes(ShaderType, SceneShaderPlatform, bUseInstancing, ShaderTypesToGet))
		{
		...
		PixelShader->SetDrawFlags(*PixelBindings, ShaderType, bDrawDisabled);
		...
		{
			PixelShader->SetAdditionalTexture(*PixelBindings, GBlackTexture->TextureRHI, TStaticSamplerState<SF_Bilinear, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI());
		}
	...
	const FSlateRenderBatchOp& RenderBatchOp)
{
	...
	State.GraphicsPSOInit.StatePrecachePSOHash = 0;
	if (RenderBatchOp.ShaderResourceType == ESlateShaderResource::Material)
	{
		...
		const bool bIsRequired = false; //TODO FORT-1096704: Setting bIsRequired to false is necessary to avoid triggering a fatal log in FShaderMapResource_SharedCode::CreateRHIShaderOrCrash, until the bug is fixed properly.
		State.GraphicsPSOInit.BoundShaderState.VertexShaderRHI = RenderBatchOp.VertexBindings->Shader.GetVertexShader(bIsRequired);
		State.GraphicsPSOInit.BoundShaderState.PixelShaderRHI  = RenderBatchOp.PixelBindings->Shader.GetPixelShader(bIsRequired);
		State.GraphicsPSOInit.PrimitiveType = GetRHIPrimitiveType(RenderBatch.DrawPrimitiveType);
		
		if (!State.GraphicsPSOInit.BoundShaderState.VertexShaderRHI || !State.GraphicsPSOInit.BoundShaderState.PixelShaderRHI)
		{
			static bool bLoggedOnce = false;
			UE_CLOGF(!bLoggedOnce, LogSlate, Error, "Unable to create shaders to render a Slate material");
			bLoggedOnce = true;
			return;
		}

		if (UPSOPrecacheSettingsManager::GetSettings().bSkipSlateMaterialDrawOnPSOTooLate && GetPSOPrecacheProxyCreationStrategy() != EPSOPrecacheProxyCreationStrategy::AlwaysCreate)
		{
			// Early out draw when slate material PSO is still compiling for Precache
			State.GraphicsPSOInit.StatePrecachePSOHash = RHIComputeStatePrecachePSOHash(State.GraphicsPSOInit);
			FGraphEventRef CompletionEvent;
			FPSOPrecacheRequestID PSOPrecacheRequestID;
			EPSOPrecacheResult PSOPrecacheResult = PipelineStateCache::CheckPipelineStateInCache(State.GraphicsPSOInit, CompletionEvent, PSOPrecacheRequestID);
			if (PSOPrecacheResult == EPSOPrecacheResult::Active)
			{
				PipelineStateCache::BoostPrecachePriority(EPSOPrecachePriority::Highest, PSOPrecacheRequestID);
				return;
			}
		}

#if PSO_PRECACHING_VALIDATE
		if (PSOCollectorStats::IsFullPrecachingValidationEnabled() && UPSOPrecacheSettingsManager::GetSettings().bSlateMaterials)
		{
...
#if PSO_PRECACHING_VALIDATE
		if (PSOCollectorStats::IsFullPrecachingValidationEnabled() && UPSOPrecacheSettingsManager::GetSettings().GlobalShaderMode > 0)
		{			
    ...
    TInterval<int32> LayerRange)
{
	...
	*RenderBatchDrawInputs =
	...
	while (NextRenderBatchIndex != INDEX_NONE)
	{
		...
		int32 NextRenderBatchLayer = NextRenderBatch.GetLayer();
		
		NextRenderBatchIndex = NextRenderBatch.NextBatchIndex;
		
		if(NextRenderBatchLayer < LayerRange.Min || (NextRenderBatchLayer >= LayerRange.Max && LayerRange.Max != std::numeric_limits<int32_t>::max()))
		{
			continue;
		}
		...
		case ESlateRenderBatchType::PostProcess:
		{
			...
			BlurInputs.HDRPaperWhiteInNits = Inputs.HDRPaperWhiteInNits;
...
TConstArrayView<ESlateShader> GetSlateShaderTypesToPrecache(EShaderPlatform ShaderPlatform)
{
	static const ESlateShader StaticShaderTypes[] =
	{
	...
	static const ESlateShader HybridShaderTypes[] =
	{
		ESlateShader::Default,
		ESlateShader::Dynamic
	};

	static const ESlateShader DynamicShaderTypes[] =
	{
		ESlateShader::Dynamic
	};
	...
	{
	case ESlateMaterialShaderMode::Static:
		return TConstArrayView<ESlateShader>(StaticShaderTypes);
	case ESlateMaterialShaderMode::Hybrid:
		return TConstArrayView<ESlateShader>(HybridShaderTypes);
	case ESlateMaterialShaderMode::Dynamic:
		return TConstArrayView<ESlateShader>(DynamicShaderTypes);
	default:
		return TConstArrayView<ESlateShader>();
	}
}
	...
	TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	auto AddPSOInitializer = [&](EPixelFormat RenderTargetFormat)
	{
		FGraphicsPipelineStateInitializer GraphicsPSOInit;	
		...
		AddRenderTargetInfo(RenderTargetFormat, ETextureCreateFlags::RenderTargetable, RenderTargetsInfo);
		...
		PSOPrecacheData.InMemoryMode = UPSOPrecacheSettingsManager::GetSettings().bKeepSlatePSOsInMemoryForActiveMaterials ? EPSOPrecacheInMemoryMode::Cached : EPSOPrecacheInMemoryMode::Uncached;
		#if PSO_PRECACHING_VALIDATE
		PSOPrecacheData.ResourceName = Material ? Material->GetAssetName() : FString(TEXT(""));
	...
	AddPSOInitializer(GetDefaultBackBufferPixelFormat());
	AddPSOInitializer(PF_B8G8R8A8); //< For render to texture
}
...
void SlateGlobalPSOCollector(const FSceneTexturesConfig& SceneTexturesConfig, int32 GlobalPSOCollectorIndex, TArray<FPSOPrecacheData>& PSOInitializers)
{
	...
	for (int32 ShaderTypeIndex = 0; ShaderTypeIndex <= (int32)ESlateShader::Dynamic; ++ShaderTypeIndex)
	{
		...
		{
			ESlateDrawEffect DrawEffects = (ESlateDrawEffect) DrawEffectsIndex;
	...
	for (FRHIPixelShader* SlateElementPixelShader : SlateElementPixelShaders)
	{
		for (FBlendStateRHIRef& BlendState : SlateElementBlendStates)
		{
			...
			AddSlatePSOInitializer(BlendState, bInstanced, ESlateDrawPrimitive::TriangleList, RHIVertexShader, SlateElementPixelShader, SceneTexturesConfig, GlobalPSOCollectorIndex, nullptr, PSOInitializers);
			AddSlatePSOInitializer(BlendState, bInstanced, ESlateDrawPrimitive::LineList, RHIVertexShader, SlateElementPixelShader, SceneTexturesConfig, GlobalPSOCollectorIndex, nullptr, PSOInitializers);
		}
	}
}
...
{
	if (!Material.IsUIMaterial() || !UPSOPrecacheSettingsManager::GetSettings().bSlateMaterials)
	{
		return;
	}
	...
	for (ESlateShader ShaderType : GetSlateShaderTypesToPrecache(ShaderPlatform))
	{
		...
		for (int32 UseInstancingIndex = 0; UseInstancingIndex < UseInstancingCount; ++UseInstancingIndex)
		{
			...
			FMaterialShaderTypes ShaderTypesToGet;
			if (!ChooseMaterialShaderTypes(ShaderType, ShaderPlatform, bUseInstancing, ShaderTypesToGet))
			{
				continue;
			}
			...
			AddSlatePSOInitializer(BlendState, bUseInstancing, ESlateDrawPrimitive::TriangleList, RHIVertexShader, RHIPixelShader, SceneTexturesConfig, PSOCollectorIndex, &Material, PSOInitializers);
		}
	}
}
