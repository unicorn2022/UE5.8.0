...
#define VERSE_ENUM_DIAGNOSTICS(v) \
    ...
    v(2305, Warning, WarnSemantic_MissingEntitlementDefinition,     "") \
    v(2306, Warning, WarnSemantic_CaseInsensitiveNames,             "") \
    v(2307, Warning, WarnSemantic_DeprecatedOctalLiteral,           "Octal literals are deprecated, and will be interpreted as a decimal literal in future versions of Verse. " \
                                                                    " To preserve the current meaning of this code in future versions of Verse, you should rewrite it as a decimal literal without any leading zeroes.") \
    /* Errors (3000+) */ \
    ...
    v(3110, Error,   ErrSyntax_CharacterLiteralIsNotUnicodeScalar,  "Character literals must be Unicode scalars, which exclude the surrogate code points 0uD800..0uDFFF.") \
    /* Semantic errors */ \
    ...
    v(3551, Error,   ErrSemantic_MayNotSkipOutOfSpawn,              "`return` and `break` are disallowed in `spawn`.") \
    ...
    v(3556, Error,   ErrSemantic_MayNotSkipOutOfBranch,             "`return` and `break` are disallowed in `branch`.") \
    ...
    v(3566, Error,   ErrSemantic_MayNotSkipOutOfDefer,              "`return` and `break` are disallowed in `defer`.") \
 ...
 /* v(3575, Error,   **UNUSED**,                                    "") */ \
    ...
    v(3626, Error,   ErrSemantic_MayNotSkipOutOfArchetype,          "`return` and `break` are disallowed in archetype instantiation.") \
    ...
    v(3683, Error,   ErrSemantic_MayNotSkipOutOfWhen,               "`return` and `break` are disallowed in `when`.") \
    v(3684, Error,   ErrSemantic_MayNotSkipOutOfUpon,               "`return` and `break` are disallowed in `upon`.") \
    v(3685, Error,   ErrSemantic_MayNotSkipOutOfSetLive,            "`return` and `break` are disallowed in `set live`.") \
    v(3686, Error,   ErrSemantic_MayNotSkipOutOfVarLive,            "`return` and `break` are disallowed in `var live`.") \
    ...
    v(3691, Error,   ErrSemantic_MayNotSkipOutOfRace,               "`return` and `break` are disallowed in `race`.") \
    v(3692, Error,   ErrSemantic_MayNotSkipOutOfRush,               "`return` and `break` are disallowed in `rush`.") \
    v(3693, Error,   ErrSemantic_MayNotSkipOutOfSync,               "`return` and `break` are disallowed in `sync`.") \
    v(3694, Error,   ErrSemantic_MissingWebDataSyncServiceAttribute, "") \
    /* Assembler errors */ \
    ...
    v(9003, Error,   ErrAssembler_RuntimeError,                     "Runtime error while evaluating global.") \
...
struct SGlitchLocus
{
    ...
    Verse::Vst::TNodePtr<Verse::Vst::Node> _VstIdentifier;

    SGlitchLocus() {}
    SGlitchLocus(const CUTF8String& SnippetPath, const STextRange& Range, const Verse::Vst::TNodePtr<Verse::Vst::Node> VstIdentifier) : _SnippetPath(SnippetPath), _Range(Range), _VstIdentifier(VstIdentifier) {}
    SGlitchLocus(const CUTF8String& SnippetPath, const STextRange& Range) : _SnippetPath(SnippetPath), _Range(Range) {}
...
struct SGlitch : public CSharedMix
{
    ...
    // Methods 
        ...
        SGlitch(SGlitchResult&& Result, SGlitchLocus&& Locus)
         ...
         **/
                     CUTF8String        AsFormattedString() const  { return FormattedString(_Result._Message.AsCString(), _Locus._SnippetPath.AsCString(), _Locus._Range, _Result.GetInfo().Severity, _Result._Id); }
