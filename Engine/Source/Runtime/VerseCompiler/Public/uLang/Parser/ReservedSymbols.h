...
#define VISIT_RESERVED_SYMBOL(Name, Symbol, Reservation, VerseVersion, \
	FNVersion)                                                         \
	Name,
/// Represents the set of reserved symbols in the semantic analyzer (not the
/// parser!)
...
VERSECOMPILER_API CUTF8StringView
GetReservedSymbol(const EReservedSymbol Identifier);

/// Gets the type of reservation this symbol has.
VERSECOMPILER_API EIsReservedSymbolResult GetReservationForSymbol(
	const EReservedSymbol Identifier, const uint32_t CurrentVerseVersion,
	const uint32_t CurrentUploadedAtFNVersion);
/// @overload
VERSECOMPILER_API EIsReservedSymbolResult GetReservationForSymbol(
	const CSymbol& Identifier, const uint32_t CurrentVerseVersion,
	const uint32_t CurrentUploadedAtFNVersion);

/// Gets all reserved tokens (both the parser and semantic analyzer).
VERSECOMPILER_API TSet<CUTF8String>
