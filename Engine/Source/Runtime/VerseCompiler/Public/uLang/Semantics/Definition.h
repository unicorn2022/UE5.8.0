 ...
 */
class CDefinition : public CNamed, public TAstNodeRef<CExpressionBase>, public CSharedMix
{
public:
    ...
    // If this definition is in the given visit set, return false.
    // Otherwise, add this definition *and all definitions it overrides* to the visit set and
    // return true.
    bool TryMarkOverriddenAndConstrainedDefinitionsVisited(CVisitSet& VisitSet) const
    {
        if (!VisitSet.TryVisit(_PrototypeDefinition->_LastOverridingVisitKey))
        {
            return false;
        }
        ...
        {
            VisitSet.TryVisit(OverriddenDefinition->GetPrototypeDefinition()->_LastOverridingVisitKey);
        }

        // TODO? Should this one also use _PrototypeDefinition-> instead of this-> as above?
        if (_ConstrainedDefinition)
        {
            VisitSet.TryVisit(_ConstrainedDefinition->GetPrototypeDefinition()->_LastOverridingVisitKey);
        }
    ...
    {
        ULANG_ENSUREF(GetPrototypeDefinition() == this, "GetAttributes must not be called on an instantiated definition, which inherits its attributes from its prototype definition.");
        return GetPrototypeDefinition()->_Attributes;
    }

    CAttributable& GetAttributes()
    {
        ULANG_ENSUREF(GetPrototypeDefinition() == this, "GetAttributes must not be called on an instantiated definition, which inherits its attributes from its prototype definition.");
        return const_cast<CAttributable&>(GetPrototypeDefinition()->_Attributes);
    }

    bool HasAttributeClass(const CClass* AttributeClass, const CSemanticProgram& Program) const
    {
        return GetAttributes().HasAttributeClass(AttributeClass, Program);
    }

    bool HasAttributeSubclass(const CClass* AttributeClass, const CSemanticProgram& Program) const
    {
        return GetAttributes().HasAttributeSubclass(AttributeClass, Program);
    }

    void AddAttributeClass(const CClass* AttributeClass)
    {
        GetAttributes().AddAttributeClass(AttributeClass);
    }
    ...
    UE_API bool IsFinal(uint32_t UploadedAtFNVersion) const;
    ...
    virtual bool IsEntitlementCompatConstraint() const = 0;
    ...
    UE_API void SetInstantiatedOverriddenDefinition(const CNormalType& InstType, const CDefinition& GeneralizedDefinition);

    virtual void InstantiateDefinition(CScope& InstPositiveScope, CScope& InstNegativeScope, const CNormalType& InstType, const TArray<STypeVariableSubstitution>& Substitutions) const
    {
        ULANG_ERRORF("InstantiateDefinition unimplemented for %s", AsNameCString());
    }

    virtual void CreateNegativeDefinition(CScope& NegativeScope) const
    {
        ULANG_ERRORF("CreateNegativeDefinition unimplemented for %s", AsNameCString());
    }
...
private:
    ...
    // The visit key used to identify whether this definition or an overriding definition is in a visit set.
    CVisitKey _LastOverridingVisitKey{};
    ...
    // The attributes for the definition. Only valid if GetPrototypeDefinition() == this.
    CAttributable _Attributes;
};
