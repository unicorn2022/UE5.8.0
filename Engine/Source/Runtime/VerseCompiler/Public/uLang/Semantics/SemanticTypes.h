...
private:
    ...
    // We introduce a custom type that just contains a unique pointer to tuple types,
    // but compares on the pointee rather than the pointer. While we could use `TURefSet`,
    // that requires us to deeply copy the contents of `CTupleType` to provide a `Key`,
    // which includes a `TArray`. Yuck! Much faster to just introduce a new type.
    struct CUniqueTupleTypeWithPointeeEquality;

    mutable TSet<CUniqueTupleTypeWithPointeeEquality> _TupleTypesStartingWithThisType;

    // We introduce a custom type that just contains a unique pointer to function types,
    // but compares on the pointee rather than the pointer. While we could use `TURefSet`,
    // that requires us to deeply copy the contents of `CFunctionType` to provide a `Key`,
    // which includes a `TArray`. Yuck! Much faster to just introduce a new type.
    struct CUniqueFunctionTypeWithPointeeEquality;

    mutable TSet<CUniqueFunctionTypeWithPointeeEquality> _FunctionTypesWithThisParameterType;
};
...
public:
    ...
    virtual SmallDefinitionArray FindInstanceMember(const CSymbol& MemberName, EMemberOrigin Origin, const SQualifier& Qualifier, const CAstPackage* ContextPackage, CVisitSet& VisitSet) const { return {}; }
    ...
    virtual SmallDefinitionArray FindTypeMember(const CSymbol& MemberName, EMemberOrigin Origin, const SQualifier& Qualifier, CVisitSet& VisitSet) const { return {}; }
...
public:
    ...
    {
        return PositiveType()->GetNormalType().FindTypeMember(MemberName, Origin, Qualifier, VisitSet);
    }
...
public:
    ...
    virtual SmallDefinitionArray FindInstanceMember(const CSymbol& MemberName, EMemberOrigin Origin, const SQualifier& Qualifier, const CAstPackage* ContextPackage, CVisitSet& VisitSet) const override
    {
        return _SuperType->GetNormalType().FindInstanceMember(MemberName, Origin, Qualifier, ContextPackage, VisitSet);
    }
    virtual SmallDefinitionArray FindTypeMember(const CSymbol& MemberName, EMemberOrigin Origin, const SQualifier& Qualifier, CVisitSet& VisitSet) const override
    {
        return _SuperType->GetNormalType().FindTypeMember(MemberName, Origin, Qualifier, VisitSet);
    }
...
public:
    ...
    virtual bool CanBePredictsVarDataType() const override { return GetElementType()->GetNormalType().CanBePredictsVarDataType(); }
...
public:
    ...
    bool TryMarkVisited(CVisitSet& VisitSet) const { return VisitSet.TryVisit(_LastVisitKey); }

    int32_t Num() const { return _Elements.Num(); }
...
public:
    ...
    CFunctionType(
        ...
        bool bImplicitlySpecialized = false)
        ...
        , _bImplicitlySpecialized(bImplicitlySpecialized)
    {
        for (const CTypeVariable* TypeVariable : _TypeVariables)
        {
            ULANG_ASSERT(TypeVariable);
        }
    }
...
struct STypeVariableSubstitution
{
    ...
    friend bool operator<(const STypeVariableSubstitution& Left, const STypeVariableSubstitution& Right)
    {
        if (Left._TypeVariable < Right._TypeVariable)
        {
            return true;
        }
        else if (Right._TypeVariable < Left._TypeVariable)
        {
            return false;
        }

        if (Left._NegativeType < Right._NegativeType)
        {
            return true;
        }
        else if (Right._NegativeType < Left._NegativeType)
        {
            return false;
        }

        return Left._PositiveType < Right._PositiveType;
    }
};
...
public:
    ...
    struct Key final
    {
        const FIntOrNegativeInfinity& _MinInclusive;
        const FIntOrPositiveInfinity& _MaxInclusive;

        friend bool operator==(const Key& Left, const Key& Right)
        {
            return
                (Left._MinInclusive == Right._MinInclusive) && (Left._MaxInclusive == Right._MaxInclusive);
        }
        ...
        friend bool operator<(const Key& Left, const Key& Right)
        {
            if (Left._MinInclusive < Right._MinInclusive)
            {
                return true;
            }
            else if (Right._MinInclusive < Left._MinInclusive)
            {
                return false;
            }

            return Left._MaxInclusive < Right._MaxInclusive;
        }
    };
...
public:
    ...
    struct Key final
    {
        ...
        {
            return
                (Left._MinRanking == Right._MinRanking) && (Left._MaxRanking == Right._MaxRanking);
        }
        ...
        friend bool operator<(const Key& Left, const Key& Right)
        {
            if (Left._MinRanking < Right._MinRanking)
            {
                return true;
            }
            else if (Right._MinRanking < Left._MinRanking)
            {
                return false;
            }

            return Left._MaxRanking < Right._MaxRanking;
        }
    };

    operator Key() const
    {
        return { _MinRanking, _MaxRanking };
    }
...
struct CTypeBase::CUniqueTupleTypeWithPointeeEquality final
{
    template<typename... CtorArgsType>
    CUniqueTupleTypeWithPointeeEquality(CtorArgsType&&... CtorArgs) :
        Payload(TUPtr<CTupleType>::New(uLang::ForwardArg<CtorArgsType>(CtorArgs)...)) {
    }

    operator CTupleType& () const
    {
        return *Payload;
    }

    bool operator==(const CUniqueTupleTypeWithPointeeEquality& Other) const
    {
        return
            Other.Payload->GetElements() == Payload->GetElements() &&
            Other.Payload->GetFirstNamedIndex() == Payload->GetFirstNamedIndex();
    }

    friend uint32_t GetTypeHash(const CUniqueTupleTypeWithPointeeEquality& TupleType)
    {
        uint32_t SmooshedHash = TupleType.Payload->GetFirstNamedIndex();

        for (const CTypeBase* const Element : TupleType.Payload->GetElements())
        {
            SmooshedHash = HashCombineFast(SmooshedHash, GetTypeHash(Element));
        }
...
private:
    TUPtr<CTupleType> Payload;
};

struct CTypeBase::CUniqueFunctionTypeWithPointeeEquality final
{
    template<typename... CtorArgsType>
    CUniqueFunctionTypeWithPointeeEquality(CtorArgsType&&... CtorArgs) :
        Payload(TUPtr<CFunctionType>::New(uLang::ForwardArg<CtorArgsType>(CtorArgs)...)) {
    }

    operator CFunctionType& () const
    {
        return *Payload;
    }

    bool operator==(const CUniqueFunctionTypeWithPointeeEquality& Other) const
    {
        return
            Other.Payload->GetEffects() == Payload->GetEffects() &&
            Other.Payload->IsClosed() == Payload->IsClosed() &&
            &Other.Payload->GetParamsType() == &Payload->GetParamsType() &&
            &Other.Payload->GetReturnType() == &Payload->GetReturnType() &&
            Other.Payload->GetTypeVariables() == Payload->GetTypeVariables() &&
            Other.Payload->ImplicitlySpecialized() == Payload->ImplicitlySpecialized();
    }

    friend uint32_t GetTypeHash(const CUniqueFunctionTypeWithPointeeEquality& FunctionType)
    {
        const uint32_t Hashes[] = {
            GetTypeHash(FunctionType.Payload->GetEffects()),
            GetTypeHash(FunctionType.Payload->IsClosed()),
            GetTypeHash(&FunctionType.Payload->GetParamsType()),
            GetTypeHash(&FunctionType.Payload->GetReturnType()),
            GetTypeHash(FunctionType.Payload->GetTypeVariables()),
            GetTypeHash(FunctionType.Payload->ImplicitlySpecialized()),
        };

        uint32_t SmooshedHash = 0;
...
namespace SemanticTypeUtils
{
    ...
    VERSECOMPILER_API const CTypeBase& Canonicalize(const CTypeBase&, ETypePolarity);

    VERSECOMPILER_API const CFunctionType& Canonicalize(const CFunctionType&);

    VERSECOMPILER_API const CFunctionType& Canonicalize(const CFunctionType&, ETypePolarity);

    VERSECOMPILER_API const CTupleType& Canonicalize(const CTupleType&);

    VERSECOMPILER_API const CTupleType& Canonicalize(const CTupleType&, ETypePolarity);
    ...
    VERSECOMPILER_API void ForEachType(
        const CTypeBase*, ETypePolarity,
        const TFunction<void(const CTypeBase*, ETypePolarity)>&);

    VERSECOMPILER_API void ForEachTypeRecursive(
    ...
    bool IsMessageType(const CTypeBase*, uint32_t UploadedAtFnVersion);
    ...
    bool IsLeaderboardType(const CTypeBase*, uint32_t UploadedAtFnVersion);

    // Returns whether the type is an entitlement type
    bool IsEntitlementType(const CTypeBase*, uint32_t UploadedAtFnVersion);
    ...
    VERSECOMPILER_API const CTypeBase& StripCastableAndConcrete(const CTypeBase&, ETypeConstraintFlags&);
