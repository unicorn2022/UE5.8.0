...
public:
    ...
    virtual CScope* AsScope() override { return this; }
    ...
    bool GetTrackLinkDependecies() const { return _bTrackLinkDependencies; }
    void SetTrackLinkDependecies(bool bTrackLinkDependecies) { _bTrackLinkDependencies = bTrackLinkDependecies; }

    bool GetDontCall() const { return _bDontCall; }
    void SetDontCall(bool DontCall) const { _bDontCall = DontCall; }
...
private:
    ...
    /// `true` if this function must track link dependencies, i.e., it is used to create a default value in a class/interface. 
    bool _bTrackLinkDependencies{ false };

    /// Only used if _bTrackLinkDependencies is `true` and the function can not be found. 
    /// Will be true for the default function of a data field with accessors in an interface read from a digest.
    mutable bool _bDontCall{ false };
