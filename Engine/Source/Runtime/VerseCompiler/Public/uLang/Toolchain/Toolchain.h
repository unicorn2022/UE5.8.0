...
namespace uLang
{
    ...
    inline bool IsCompileFailure(ECompilerResult E)    { return Enum_HasAnyFlags(E, ECompilerResult::CompileMask_Failure); }
    inline bool IsAbortedCompile(ECompilerResult E)    { return Enum_HasAnyFlags(E, ECompilerResult::CompileMask_Aborted); }
    inline bool IsCompileIncomplete(ECompilerResult E) { return !E || IsAbortedCompile(E); }
    inline bool IsCompileComplete(ECompilerResult E)   { return !IsCompileIncomplete(E); }
    ...
    public:
         ...
         */
        UE_API ECompilerResult ParseSnippet(const Verse::Vst::TNodeRef<Verse::Vst::Snippet>& OutVst,
         ...
         */
        UE_API ECompilerResult CompileVst(const Verse::Vst::TNodeRef<Verse::Vst::Project>& Vst, const SBuildContext& BuildContext, const SProgramContext& ProgramContext);
         ...
         */
        UE_API ECompilerResult SemanticAnalyzeVst(TOptional<TSRef<CSemanticProgram>>& OutProgram, const Verse::Vst::TNodeRef<Verse::Vst::Project>& Vst, const SBuildContext& BuildContext, const SProgramContext& ProgramContext);
        ...
        */
        ...
        const Verse::Vst::TNodePtr<Verse::Vst::Project>&               GetProjectVst() const { return _ProjectVst; }
         ...
         */
        void SetProjectVst(const Verse::Vst::TNodeRef<Verse::Vst::Project>& NewProject)
        {
        ...
        // Add a storage for modified source code, e.g., auto qualified code.
        // If set then the modified source will be added to the map, keyed by path.
        // No need to set unless the modified source is needed after compiling, 
        // e.g., by the assert_roundtrip macro in VerseTestScriptCmd.
        UE_API void SetPathToSource(uLang::TMap<uLang::CUTF8String, uLang::CUTF8String>* PathToSource);
    ...
    private:
        ...
        // Extend VstProject with the vst created from SourceProject.
        void FillInVst(Verse::Vst::Project* VstProject, SBuildResults& BuildResults, const CSourceProject& SourceProject, const SBuildContext& BuildContext, const SProgramContext& ProgramContext);
        ...
        Verse::Vst::TNodePtr<Verse::Vst::Project> _ProjectVst; 

        // Localization and string information stored here, see SetPathToSource for usage.
        TArray<FSolLocalizationInfo> _LocalizationInfo;
        TArray<FSolLocalizationInfo> _StringInfo;

        // Storage for modified source, keyed by path.
        // The source stored here takes precedence over the one in SourceSnippet. 
        uLang::TMap<uLang::CUTF8String, uLang::CUTF8String>* _PathToSource{ nullptr };
    };
}
