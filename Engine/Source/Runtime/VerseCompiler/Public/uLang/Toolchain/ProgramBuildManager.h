...
struct SToolchainOverrides
{
    ...
    TOptional<TSRefArray<IPostLinkInjection>>         PostLinkInjections;
};
...
public:
    ...
    void SetPathToSource(uLang::TMap<uLang::CUTF8String, uLang::CUTF8String>* PathToSource) { _Toolchain->SetPathToSource(PathToSource); }

    UE_API SBuildResults BuildProject(const CSourceProject& SourceProject, const SBuildContext& BuildContext);
    UE_API ECompilerResult ParseSnippet(const Verse::Vst::TNodeRef<Verse::Vst::Snippet>& OutVst, const CUTF8String& TextSnippet, const SBuildContext& BuildContext);
    UE_API ECompilerResult SemanticAnalyzeVst(TOptional<TSRef<CSemanticProgram>>& OutProgram, const Verse::Vst::TNodeRef<Verse::Vst::Project>& Vst, const SBuildContext& BuildContext);
    ...
    const Verse::Vst::TNodePtr<Verse::Vst::Project>& GetProjectVst() const { return _Toolchain->GetProjectVst(); }
