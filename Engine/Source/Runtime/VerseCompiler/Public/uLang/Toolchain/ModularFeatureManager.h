...
class AUTORTFM_DISABLE TModularFeatureRegHandle
{
public:
    template<typename... Args_t>
    TModularFeatureRegHandle(Args_t&&... Args);
    ...
    TModularFeatureRegHandle(TModularFeatureRegHandle&& Other)
        : _ModularFeatureRef(Move(Other._ModularFeatureRef))
    {
    }

    TModularFeatureRegHandle& operator=(TModularFeatureRegHandle&& Other)
    {
        _ModularFeatureRef = Move(Other._ModularFeatureRef);
        return *this;
    }
    ...
    // Conversion methods
    operator TSRef<FeatureType>() const             { return _ModularFeatureRef; }
    operator FeatureType*() const                   { return (FeatureType*)_ModularFeatureRef; }
    FeatureType&                operator*() const   { return *_ModularFeatureRef; }
    FeatureType*                operator->() const  { return _ModularFeatureRef.operator->(); }

    //---------------------------------------------------------------------------------------

    TSRef<FeatureType>&         ToSharedRef()       { return _ModularFeatureRef; }
    const TSRef<FeatureType>&   ToSharedRef() const { return _ModularFeatureRef; }
