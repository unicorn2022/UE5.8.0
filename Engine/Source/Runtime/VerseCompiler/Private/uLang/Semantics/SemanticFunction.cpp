...
uLang::TSPtr<uLang::CExprClassDefinition> CFunction::GetBodyClassDefinitionAst() const
{
...
uLang::TSPtr<uLang::CExprInterfaceDefinition> CFunction::GetBodyInterfaceDefinitionAst() const
{
    ULANG_ASSERTF(!GetIrNode(true), "Called AST function on when IR available");
    if (TSPtr<CExpressionBase> BodyAst = GetBodyAst())
    {
...
{
    return GetPrototypeDefinition()->HasAttributeClass(GetProgram()._nativeClass, GetProgram());
}
...
void CFunction::InstantiateDefinition(CScope& InstPositiveScope, CScope& InstNegativeScope, const CNormalType& InstType, const TArray<STypeVariableSubstitution>& Substitutions) const
{
    TSRef<CFunction> InstFunction = InstPositiveScope.CreateFunction(GetName());
    InstFunction->_ExtensionFieldAccessorKind = _ExtensionFieldAccessorKind;
    InstFunction->SetPrototypeDefinition(*GetPrototypeDefinition());
    InstFunction->SetInstantiatedOverriddenDefinition(InstType, *this);
    const CFunctionType* NegativeFunctionType = _NegativeType;
    const CFunctionType* FunctionType = _Signature.GetFunctionType();
    ...
    InstFunction->SetSignature(
        SSignature(
            ...
            TArray<CDataDefinition*>(_Signature.GetParams())),
        GetSignatureRevision());
    InstFunction->CreateNegativeDefinition(InstNegativeScope);
}

void CFunction::CreateNegativeDefinition(CScope& NegativeScope) const
{
    TSRef<CFunction> NegativeFunction = NegativeScope.CreateFunction(GetName());
    NegativeFunction->_ExtensionFieldAccessorKind = _ExtensionFieldAccessorKind;
    NegativeFunction->SetPrototypeDefinition(*GetPrototypeDefinition());
}
