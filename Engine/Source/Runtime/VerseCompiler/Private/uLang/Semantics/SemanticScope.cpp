...
void CScope::GetScopePath(uLang::CUTF8StringBuilder& OutBuilder, uLang::UTF8Char SeparatorChar, EPathMode Mode) const
{
    ...
    if (Mode == EPathMode::PackageRelative || Mode == EPathMode::PackageRelativeWithRoot)
    {
        ...
        {
            // Fall back to using using the module as the package root.
            ULANG_ERRORF("Did not expect null package for %s", GetScopePath('/', EPathMode::PrefixSeparator).AsCString());
    ...
    for (Index = 0; Index < NumSymbols; ++Index)
    {
        ...
        if (OutBuilder.IsFilled() || Mode == EPathMode::PrefixSeparator)
        {
            OutBuilder.Append(SeparatorChar);
        }

        OutBuilder.Append(ScopeName.AsStringView());
    }
}

CUTF8String CScope::GetScopePath(uLang::UTF8Char SeparatorChar, EPathMode Mode) const
{
    uLang::CUTF8StringBuilder Path;
...
T* CScope::GetCaptureScope(U* This)
{
    for (U* Scope = This; Scope; Scope = Scope->_Parent)
    {
        ...
        if (Scope->_Kind == uLang::Cases<EKind::Class, EKind::Interface, EKind::Type>)
        {
            continue;
        }
        return nullptr;
...
static const CScope& RemoveModulePart(const CScope& Scope)
{
    if (Scope.GetKind() == CScope::EKind::ModulePart)
    {
        return *static_cast<const CModulePart&>(Scope).GetModule();
    }
    return Scope;
}
...
bool CScope::IsSameOrChildOf(const CScope* Other) const
{
    for (const CScope* Scope = this; Scope; Scope = Scope->_Parent)
    {
        if (&RemoveModulePart(*Scope) == &RemoveModulePart(*Other))
        {
            return true;
        }
    }
...
SResolvedDefinitionArray CScope::ResolveDefinition(const CSymbol& Name, const SQualifier& Qualifier, const CAstPackage* ContextPackage) const
{
    ...
    // Every time you call FindInstanceMember it should use a new visit set, which ensures that
    ...
    {
        const CLogicalScope* LogicalScope = Scope->AsLogicalScopeNullable();
        if (LogicalScope && LogicalScope->TryMarkVisited(VisitSet))
        {
            SmallDefinitionArray FoundDefinitions = LogicalScope->FindDefinitions(Name, EMemberOrigin::InheritedOrOriginal, Qualifier, ContextPackage, VisitSet);
        ...
        if (Qualifier._Type != SQualifier::EType::Local)
        {
            ...
            {
                if (Using->TryMarkVisited(VisitSet))
                {
                    ResolvedDefnsAppend(&Result, Using->FindDefinitions(Name, EMemberOrigin::InheritedOrOriginal, Qualifier, ContextPackage, VisitSet));
                }
            }
        }
    }
    ...
    if (Qualifier._Type != SQualifier::EType::Local)
    {
        ...
        for (const CScope* Scope = this; Scope; Scope = Scope->GetParentScope())
        {
            ...
            for (const CDataDefinition* UsingContext : Scope->GetUsingInstances())
            {
                ...
                // Note that each call to `FindInstanceMember()` uses a new visit set and it returns
...
bool CScope::CanAccess(const CDefinition& Definition, const SAccessLevel& DefinitionAccessLevel) const
{
    ...
    {        
        for (const CScope* I = Scope; I; I = I->GetParentScope())
        {
            if (I->GetKind() == Cases<CScope::EKind::Class, CScope::EKind::Interface>)
            {
                return static_cast<const CLogicalScope*>(I);
            }
        }
        return nullptr;
    };
    ...
    case SAccessLevel::EKind::Internal:
    {
        ...
        const CModule* DefinitionModule = Definition._EnclosingScope.GetModule();
    ...
    {
        const CLogicalScope* ReferencingClassOrInterface = GetScopeClassOrInterface(this);
        if (!ReferencingClassOrInterface)
        {
            return false;
        }
        const CScope* DefinitionScope = Definition.DefinitionAsLogicalScopeNullable();
        if (!DefinitionScope)
        {
            DefinitionScope = &Definition._EnclosingScope;
        }
        const CLogicalScope* DefinitionClassOrInterface = GetScopeClassOrInterface(DefinitionScope);
        if (!DefinitionClassOrInterface)
        {
            return false;
        }

        if (ReferencingClassOrInterface->GetKind() == CScope::EKind::Class)
        {
            if (DefinitionClassOrInterface->GetKind() == CScope::EKind::Class)
            {
                return static_cast<const CClass*>(ReferencingClassOrInterface)->IsClass(*static_cast<const CClass*>(DefinitionClassOrInterface));
            }
            if (DefinitionClassOrInterface->GetKind() == CScope::EKind::Interface)
            {
                return static_cast<const CClass*>(ReferencingClassOrInterface)->ImplementsInterface(*static_cast<const CInterface*>(DefinitionClassOrInterface));
            }
        }
        else if (ReferencingClassOrInterface->GetKind() == CScope::EKind::Interface)
        {
            if (DefinitionClassOrInterface->GetKind() == CScope::EKind::Interface)
            {
                return static_cast<const CInterface*>(ReferencingClassOrInterface)->IsInterface(*static_cast<const CInterface*>(DefinitionClassOrInterface));
            }
        }
        return false;
    }
    case SAccessLevel::EKind::Private:
    {
        ...
        return ReferencingClassOrInterface == DefinitionClassOrInterface;
    }
...
namespace {
    // This is to compensate for the reference from the data field in the interface to the default function.
    unsigned int ExtraReference(const CDefinition* Definition)
    {
        if (const CFunction* Function = Definition->AsNullable<CFunction>()) {
            return Function->GetTrackLinkDependecies() ? 1 : 0;
        }
        return 0;
    }
}
...
CLogicalScope::~CLogicalScope()
{
    ...
    {
        ULANG_ASSERTF(Definition->GetRefCount() == 1 + ExtraReference(Definition.Get()), "Unexpectedly freeing %s scope while there's an external reference to its %s definition %s",
...
SmallDefinitionArray CLogicalScope::FindDefinitions(const CSymbol& Name, EMemberOrigin Origin, const SQualifier& Qualifier, const CAstPackage* ContextPackage, CVisitSet& VisitSet) const
{
        ...
        if (const SmallDefinitionArray* DefinitionArray = _DefinitionNameMap.Find(Name))
        {
            for (CDefinition* Definition : *DefinitionArray)
            {
                if (Qualifier.IsUnspecified() ||
                    Qualifier == Definition->GetBaseOverriddenDefinition().GetPrototypeDefinition()->GetImplicitQualifier())
                {
                    ...
                    if (Definition->TryMarkOverriddenAndConstrainedDefinitionsVisited(VisitSet))
                    {
...
void CLogicalScope::ForAllDefinitions(
...
bool CLogicalScope::HasVar() const
{
    for (const uLang::TSRef<uLang::CDataDefinition>& DataMember : GetDefinitionsOfKind<uLang::CDataDefinition>())
    {
        if (DataMember->IsVar())
        {
            return true;
...
void CLogicalScope::InstantiateMembers(
...
{
    for (const CDefinition* Definition : GetDefinitions())
    {
        Definition->InstantiateDefinition(InstantiatedPositiveScope, InstantiatedNegativeScope, InstantiatedType, Substitutions);
    }

    InstantiatedPositiveScope.SetNegativeMemberTypes(InstantiatedNegativeScope);
}

void CLogicalScope::SetNegativeMemberTypes(CLogicalScope& InstantiatedNegativeScope) const
{
    const TArray<TSRef<CDefinition>>& PositiveDefinitions = GetDefinitions();
    const TArray<TSRef<CDefinition>>& NegativeDefinitions = InstantiatedNegativeScope.GetDefinitions();
    ULANG_ASSERT(PositiveDefinitions.Num() == NegativeDefinitions.Num());

    for (auto I = PositiveDefinitions.begin(), J = NegativeDefinitions.begin(), Last = PositiveDefinitions.end();
        I != Last;
        ++I, ++J)
    {
        ...
        {
            CFunction& NegativeFunction = (*J)->AsChecked<CFunction>();
        ...
        {
            ULANG_ERRORF("Unexpected definition kind %s while setting negative type for %s",
                DefinitionKindAsCString((*I)->GetKind()),
                GetQualifiedNameString(**I).AsCString());
        }
    }
}
