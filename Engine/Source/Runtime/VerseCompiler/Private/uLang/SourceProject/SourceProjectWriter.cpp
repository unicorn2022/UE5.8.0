...
bool CSourceProjectWriter::WriteProject(const CSourceProject& Project, const CUTF8String& DestinationDir, CUTF8String* ResultProjectFilePath /*= nullptr*/, const EWriteFlags WriteFlags) const
{
    ...
    for (const CSourceProject::SPackage& Package : Project._Packages)
    {
        ...
        // NOTE: (yiliang.siew) For any user-scoped package, we are forcing its role to be a source package -
        // because there is no use case today for the project descriptor to treat the user package as cooked alongside
        // its digest.
        if (PackageDesc._Settings._VerseScope ==
            uLang::Cases<uLang::EVerseScope::InternalUser, uLang::EVerseScope::PublicUser>)
        {
            PackageDesc._Settings._Role = uLang::EPackageRole::Source;
        }
