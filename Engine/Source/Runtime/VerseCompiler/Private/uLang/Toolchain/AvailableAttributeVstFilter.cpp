...
namespace
{
    const uLang::CUTF8String* GetIdentifierString(const Verse::Vst::Node* Node)
    {
        if (Node != nullptr)
        {
    ...
    uLang::TOptional<int64_t> FindIntegerInitValueByName(const char* TargetIdentifier, const Verse::Vst::Clause* ParentClause)
    {
        for (const Verse::Vst::Node* ChildNode : ParentClause->GetChildren())
        {
            ...
            {
                const uLang::CUTF8String* IdentStr = GetIdentifierString(ChildDef->GetOperandLeft());
                    ...
                    if (const Verse::Vst::Node* ValueNode = ChildDef->GetOperandRight())
                    {
                        ...
                        if (!ValueClause->IsEmpty())
                        {
                            if (const Verse::Vst::IntLiteral* IntValueLiteral = ValueClause->GetChildren()[0]->AsNullable<Verse::Vst::IntLiteral>())
                            {
                                ...
                                return uLang::TOptional<int64_t>(value);
                            }
                        }
                    }
                }
        ...
        return uLang::TOptional<int64_t>();
    }

    bool ExtractAvailableAttribute(Verse::Vst::TNodePtr<Verse::Vst::Node> DefinitionVstNode, const uLang::SBuildVersionInfo& BuildVersion)
    {
        // Prepended @attributes are in the Aux data
        if (Verse::Vst::TNodePtr<Verse::Vst::Clause> Aux = DefinitionVstNode->GetAux())
        {
            ...
            for (const Verse::Vst::Node* Child : Aux->GetChildren())
            {
                if (const Verse::Vst::Clause* ChildClause = Child->AsNullable<Verse::Vst::Clause>())
                {
                    ...
                    if (!ChildClause->IsEmpty())
                    {
                        ...
                        {
                            const uLang::CUTF8String* VersionIdentStr = GetIdentifierString(VersionAttribMacro->GetName());
                            if (VersionIdentStr && VersionIdentStr->ToStringView() == "available")
                            {
                                if (Verse::Vst::Clause* VersionBodyClause = VersionAttribMacro->GetClause(0))
                                {
                                    ...
                                    if (uLang::TOptional<int64_t> MinUploadedAtFNVersion = FindIntegerInitValueByName("MinUploadedAtFNVersion", VersionBodyClause))
                                    {
...
namespace uLang
{
void CAvailableAttributeVstFilter::StaticFilter(const Verse::Vst::TNodeRef<Verse::Vst::Node>& VstNode, const SBuildContext& BuildContext)
{
...
void CAvailableAttributeVstFilter::StaticFilterHelper(
    const Verse::Vst::TNodeRef<Verse::Vst::Node>& VstNode,
    ...
    const SBuildVersionInfo& InBuildVersion)
{
    ...
    {
        TNodeRef<Node> ChildNode = VstNode->GetChildren()[NodeChildIndex];
