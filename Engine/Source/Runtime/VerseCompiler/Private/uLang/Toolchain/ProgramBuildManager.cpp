...
static SToolchainParams MakeToolchainParams(const SToolchainOverrides& ToolchainOverrides)
{
    ...
    //
    // Post-Link Injections
    if (ToolchainOverrides.PostLinkInjections.IsSet())
    {
        ToolchainParams.LayerInjections._PostLinkInjections = *ToolchainOverrides.PostLinkInjections;
    }
    else
    {
        ToolchainParams.LayerInjections._PostLinkInjections = GetModularFeaturesOfType<IPostLinkInjection>();
    }
...
ECompilerResult CProgramBuildManager::ParseSnippet(const Verse::Vst::TNodeRef<Verse::Vst::Snippet>& OutVst, const CUTF8String& TextSnippet, const SBuildContext& BuildContext)
{
    return _Toolchain->ParseSnippet(OutVst, TextSnippet, BuildContext);
}

ECompilerResult CProgramBuildManager::SemanticAnalyzeVst(TOptional<TSRef<CSemanticProgram>>& OutProgram, const Verse::Vst::TNodeRef<Verse::Vst::Project>& Vst, const SBuildContext& BuildContext)
{
