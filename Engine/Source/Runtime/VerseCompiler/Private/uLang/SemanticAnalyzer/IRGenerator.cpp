...
private:
	...
	{
	public:
		EffectCollector(EffectCollector* Parent, CExpressionBase& IrNode)
			: Parent(Parent)
			, IrNode(IrNode)
		{ }
		...
		~EffectCollector()
		{
			IrNode.Effects |= Effects;
			if (Parent)
			{
				Parent->Effects |= IrNode.Effects;
			}
		}

	private:
		EffectCollector* Parent;
    ...
    void InitIrMemberDefinitions(CMemberDefinitions& IrNode, CMemberDefinitions& AstNode)
    {
        ...
        for (int32_t Ix = 0, NumMembers = AstNode.Members().Num(); Ix < NumMembers; ++Ix)
        {
			...
			CollectEffects(IrNode.Members()[Ix]);
        }
    }
	...
	void CollectEffects(CAstNode* IrNode)
	{
		if (IrNode)
		{
			CollectEffects(*IrNode);
		}
	}

	void CollectEffects(CAstNode& IrNode)
    {
		// This analysis is conservative. For example, it bubbles up <decides> beyond
		// constructs that consume the <decides> effect. Right now, we don't need it to
		// be precise. But it would be easy to do if we ever wanted to do it.
		if (CExpressionBase* Expression = IrNode.AsExpression())
		{
			EffectCollector Collector(CurrentEffectCollector, *Expression);
			TGuardValue<EffectCollector*> ScopeGuard(CurrentEffectCollector, &Collector);

			Expression->VisitChildrenLambda([&] (const SAstVisitor&, CAstNode& Node)
			{
				CollectEffects(Node);
			});
		}
    ...
    TSRef<CExpressionBase> MakeIrNode(CExpressionBase& AstNode)
    {
        ...
        case EAstNodeType::Flow_First:             // CExprFirst     - Find first value over an iterable type - first(Num:Nums; Pred(Num)) {DoStuff(Num)}
            return GenFirst(static_cast<CExprFirst&>(AstNode));
        ...
        case EAstNodeType::Concurrent_Batch:         // CExprBatch        - batch {X}
            return GenBatch(static_cast<CExprBatch&>(AstNode));
        ...
        case EAstNodeType::Concurrent_When:         // CExprWhen          - when(E1) {E2}
            return GenBinaryAwaitOp(static_cast<CExprWhen&>(AstNode));
        ...
        case EAstNodeType::Noop_:
            return GenNoop(static_cast<CNoop&>(AstNode));
    ...
    TSRef<CExpressionBase> GenNode(CExpressionBase& AstNode)
    {
		...
		IrNode->Effects = AstNode.Effects;
        return IrNode;
    }
    ...
    TSPtr<CExpressionBase> WithElements(TSRef<CExpressionBase>&& Expr, const CNormalType& Type, bool bBindElementsToTemporary, Function F)
    {
        ...
        if (const CTupleType* TupleType = Type.AsNullable<CTupleType>())
        {
            ...
            if (!bBindElementsToTemporary && Expr->GetNodeType() == EAstNodeType::Invoke_MakeTuple)
            {
                ...
                for (const TSRef<CExpressionBase>& SubExpr : MakeTuple.GetSubExprs())
                {
            ...
            else
            {
                ...
                CodeBlock->AppendSubExpr(Move(Result.AsRef()));
                return CodeBlock;
            }
        }
    ...
    TSPtr<CExpressionBase> MaybeCoerceArrayToTuple(TSRef<CExpressionBase>&& Value, const CArrayType& SourceArrayType, const CTupleType& ResultTupleType)
    {
        ...
        for (const CTypeBase* ResultElementType : ResultTupleType.GetElements())
        {
            ...
            MakeTuple->AppendSubExpr(Move(CoercedElement.AsRef()));
            ++I;
        }
    ...
    TSRef<CIrAllOne> WrapIteration(TSRef<CExprCodeBlock>& ForCodeBlock, const CTypeBase* ResultType)
    {
        ForCodeBlock->IrSetResultType(ResultType);

        bool bCanFail = ForCodeBlock->CanFail(ForCodeBlock->_AssociatedScope->GetPackage());
        TSRef<CIrAllOne> IrAll = NewIrNode<CIrAllOne>(Move(ForCodeBlock));
        IrAll->_bCanFail = bCanFail;
        IrAll->IrSetResultType(ResultType);
        IrAll->_bGenerateResult = true;
        return IrAll;
    }

    TSRef<CIrAllOne> SetupIteration(STempBinding& SourceBinding, const CTypeBase* ElementType, const CTypeBase* ResultType, CDataDefinition** OutElementDefinition, CIrFor** OutIteratedBody)
    {
        TSRef<CControlScope> ForCodeBlockScope = SourceBinding.CodeBlock->_AssociatedScope->CreateNestedControlScope();
        TSRef<CExprCodeBlock> ForCodeBlock = NewIrNode<CExprCodeBlock>(1);
        ForCodeBlock->_AssociatedScope = ForCodeBlockScope;

        TSRef<CControlScope> ForScope = ForCodeBlock->_AssociatedScope->CreateNestedControlScope();
        TSRef<CDataDefinition> ElementDefinition = ForScope->CreateDataDefinition(_Program.GetSymbols()->AddChecked("Item", true));
        ElementDefinition->SetType(ElementType);
        *OutElementDefinition = ElementDefinition.Get();
        ...
        *OutIteratedBody = For.Get();
        ForCodeBlock->AppendSubExpr(Move(For));

        return WrapIteration(ForCodeBlock, ResultType);
    }
        ...
        if (NeedsCoercion(*Value, ResultNormalType, SourceNormalType))
        {
            ...
            else if (ResultNormalType.IsA<CArrayType>() && SourceNormalType.IsA<CTupleType>())
            {
                ...
                for (const CTypeBase* SourceElementType : SourceTupleType.GetElements())
                {
                    ...
                    MakeArray->AppendSubExpr(Move(CoercedElement.AsRef()));
                    ++I;
                }
            ...
            else if (ResultNormalType.GetKind() == SourceNormalType.GetKind())
            {
                ...
                else if (CommonKind == ETypeKind::Array)
                {
                    ...
                    CDataDefinition* ElementDefinition;
                    CIrFor* ForExpr;
                    TSRef<CIrAllOne> IrAll = SetupIteration(SourceBinding, SourceArrayType.GetElementType(), &ResultArrayType, &ElementDefinition, &ForExpr);
                    ...
                    ForExpr->SetBody(NewIrNode<CIrIteratedBody>(NewIrNode<CIrArrayAdd>(Move(CoercedElement.AsRef()))));
                    SourceBinding.CodeBlock->AppendSubExpr(Move(IrAll));
                ...
                else if (CommonKind == ETypeKind::Generator)
                {
                    ...
                    CDataDefinition* ElementDefinition;
                    CIrFor* ForExpr;
                    TSRef<CIrAllOne> IrAll = SetupIteration(SourceBinding, SourceGeneratorType.GetElementType(), &ResultGeneratorType, &ElementDefinition, &ForExpr);
                ...
                else if (CommonKind == ETypeKind::Map)
                {
                    ...
                    For->SetBody(NewIrNode<CIrIteratedBody>(NewIrNode<CIrMapAdd>(Move(CoercedMapKey.AsRef()), Move(CoercedMapValue.AsRef()))));

                    ForCodeBlock->AppendSubExpr(Move(For));

                    TSRef<CIrAllOne> IrAll = WrapIteration(ForCodeBlock, &ResultMapType); // Body is included in Filters
    ...
    TSRef<CExpressionBase> GenIdentifierData(CExprIdentifierData& AstNode)
    {
        ...
        TSRef<CExpressionBase> IrNode = NewIrNode<CExprIdentifierData>(
            _Program,
            OverriddenPrototypeDefinition,
            Move(IrContext),
            GenNode(AstNode.Qualifier()));
        if (const CReferenceType* ResultReferenceType = ResultType->GetNormalType().AsNullable<CReferenceType>())
        {
            IrNode->IrSetResultType(&_Program.GetOrCreateReferenceType(
                OverriddenPrototypeDefinition._NegativeType,
                OverriddenPrototypeDefinition.GetType()));
            if (_TargetVM == SBuildParams::EWhichVM::BPVM)
            {
                // Type used by `ApplyTypeToExpression` when analyzing data
                // definitions and used as the underlying representation.
                const CNormalType& PositiveSourceType = SemanticTypeUtils::Canonicalize(*OverriddenPrototypeDefinition.GetType()).GetNormalType();
                // Type possible given all writes.
                const CNormalType& PositiveResultType = SemanticTypeUtils::Canonicalize(*ResultReferenceType->PositiveValueType()).GetNormalType();
                // If the representation boxes but the use does not, unbox to a
                // pointer into the boxed value.
                if (PositiveSourceType.GetKind() == ETypeKind::Any && PositiveResultType.GetKind() != ETypeKind::Any)
                {
                    IrNode = NewIrNode<CIrDataOfDynamic>(Move(IrNode));
                    IrNode->IrSetResultType(ResultType);
                }
            }
            return IrNode;
        }
        IrNode->IrSetResultType(OverriddenPrototypeDefinition.GetType());
    ...
    TSRef<CExpressionBase> GenInvocation(CExprInvocation& AstNode, const CExprIdentifierFunction& AstIdentifierFunction)
    {
        ...
        TSRef<CExpressionBase> IrCallee = NewIrNode<CExprIdentifierFunction>(
            ...
            GenNodeUnlessModule(AstIdentifierFunction.Context()),
            GenNode(AstIdentifierFunction.Qualifier()),
            AstIdentifierFunction._bSuperQualified);
        TSPtr<CExpressionBase> IrArgument;
        {
            CScope* Scope = AstIdentifierFunction._ConstructorCaptureScope ?
                AstIdentifierFunction._ConstructorCaptureScope->AsScope() :
                _Scope;
            TGuardValue<CScope*> ScopeGuard(_Scope, Scope);
            IrArgument = GenNode(*AstNode.GetArgument());
        }
        ...
        TSRef<CExprInvocation> IrNode = NewIrNode<CExprInvocation>(
            AstNode._CallsiteBracketStyle,
            Move(IrCallee),
            Move(IrArgument.AsRef()));
		IrNode->bResultUsed = AstNode.bResultUsed;
        IrNode->SetResolvedCalleeType(AstNode.GetResolvedCalleeType());
    ...
    TSRef<CExpressionBase> GenInvocation(CExprInvocation& AstNode)
    {
        ...
        TSRef<CExpressionBase> IrArgument = CoerceToType(GenNode(AstNode.GetArgument()), &CalleeType.GetParamsType());
        TSRef<CExprInvocation> IrNode = NewIrNode<CExprInvocation>(
            AstNode._CallsiteBracketStyle,
            Move(IrCallee),
            Move(IrArgument));
    ...
    TSRef<CExprMakeArray> GenMakeArray(CExprMakeArray& AstNode)
    {
        ...
        const TArray<TSRef<CExpressionBase>>& SubExprs = AstNode.GetSubExprs();
        TSRef<CExprMakeArray> IrNode = NewIrNode<CExprMakeArray>(SubExprs.Num());
        for (const TSRef<CExpressionBase>& ElementAst : AstNode.GetSubExprs())
        {
    ...
    TSRef<CExprMakeMap> GenMakeMap(CExprMakeMap& AstNode)
    {
        ...
        for (const TSRef<CExpressionBase>& PairAst : AstNode.GetSubExprs())
        {
    ...
    TSRef<CExpressionBase> GenInvokeType(CExprInvokeType& AstNode)
    {
        ...
        return CoerceToType(Move(IrNode), AstNode.GetResultType(_Program));
    }

    TSRef<CExpressionBase> GenPointerToReference(CExprPointerToReference& AstNode)
    {
        TSRef<CExpressionBase> IrNodeOperand = GenNode(*AstNode.Operand());
        const CPointerType& SourcePointerType = IrNodeOperand->IrGetResultType()->GetNormalType().AsChecked<CPointerType>();

        TSRef<CExpressionBase> IrNode = NewIrNode<CExprPointerToReference>(Move(IrNodeOperand));
        IrNode->IrSetResultType(&_Program.GetOrCreateReferenceType(
            SourcePointerType.NegativeValueType(),
            SourcePointerType.PositiveValueType()));

        if (_TargetVM == SBuildParams::EWhichVM::BPVM)
        {
            const CTypeBase* ResultType = AstNode.GetResultType(_Program);
            const CReferenceType& ResultReferenceType = ResultType->GetNormalType().AsChecked<CReferenceType>();
            // Type used by `ApplyTypeToExpression` when analyzing data
            // definitions and used as the underlying representation.
            const CNormalType& PositiveSourceType = SemanticTypeUtils::Canonicalize(*SourcePointerType.PositiveValueType()).GetNormalType();
            // Type possible given all writes, i.e. the instantiated type.
            const CNormalType& PositiveResultType = SemanticTypeUtils::Canonicalize(*ResultReferenceType.PositiveValueType()).GetNormalType();
            // If the representation boxes but the use does not, unbox to a
            // pointer into the boxed value.
            if (PositiveSourceType.GetKind() == ETypeKind::Any && PositiveResultType.GetKind() != ETypeKind::Any)
    ...
    {
        CScope* Scope = AstNode._LiveScope ? AstNode._LiveScope : _Scope;
        TGuardValue<CScope*> ScopeGuard(_Scope, Scope);
        TSRef<CExpressionBase> IrNodeOperand = GenNode(*AstNode.Operand());
        const CReferenceType& SourceType = IrNodeOperand->IrGetResultType()->GetNormalType().AsChecked<CReferenceType>();
        TSRef<CExprSet> IrNode = NewIrNode<CExprSet>(AstNode._bIsLive, AstNode._LiveScope, Move(IrNodeOperand));
        IrNode->IrSetResultType(&SourceType);
        return IrNode;
    }
    ...
    {
        TSRef<CExpressionBase> IrNodeOperand = GenNode(*AstNode.Operand());
        const CReferenceType& IrNodeOperandResultType = IrNodeOperand->IrGetResultType()->GetNormalType().AsChecked<CReferenceType>();
        TSRef<CExprReferenceToValue> IrNode = NewIrNode<CExprReferenceToValue>(Move(IrNodeOperand));
        IrNode->IrSetResultType(IrNodeOperandResultType.PositiveValueType());
        return IrNode;
    }
    ...
    TSRef<CExprCodeBlock> GenCodeBlock(CExprCodeBlock& AstNode)
    {
        ...
        for (const TSRef<CExpressionBase>& SubExpr : AstNode.GetSubExprs())
        {
    ...
    bool IsGenerator(const CExpressionBase& Expr)
    {
        if (Expr.GetNodeType() == EAstNodeType::Definition_Data
            || Expr.GetNodeType() == EAstNodeType::Definition_IterationPair)
        {
            const CExprDefinition& Definition = static_cast<const CExprDefinition&>(Expr);
            if (Definition.Value())
            {
    ...
    * 
    *  ResultDestination is created by the code generator when compiling the all expression.
    */
    ...
    TSRef<CIrAllOne> GenIteration(CExprIteration& AstNode)
    {
        TGuardValue<CScope*> ScopeGuard(_Scope, AstNode._Filters->_AssociatedScope.Get());
        ULANG_ASSERT(_Scope);
        ...
        for (const TSRef<CExpressionBase>& Filter : AstNode._Filters->GetSubExprs())
        {
            ...
            { // Generate CIrFor
                ...
                TSRef<CExprCodeBlock> IteratedBody = NewIrNode<CExprCodeBlock>(1);
                For->SetBody(IteratedBody);
                For->IrSetResultType(AstNode.IrGetResultType());

                CurrentBlock->AppendSubExpr(Move(For));
                CurrentBlock = Move(IteratedBody);
            }
            else
            {
        ...
        CurrentBlock->AppendSubExpr(NewIrNode<CIrIteratedBody>(NewIrNode<CIrArrayAdd>(GenNode(*AstNode._Body))));

        TSRef<CIrAllOne> IrAll = NewIrNode<CIrAllOne>(Move(IrNode));
        IrAll->_bCanFail = AstNode.CanFail(ScopePackage);
        IrAll->IrSetResultType(AstNode.IrGetResultType());

        return IrAll;
    }

    TSRef<CIrAllOne> GenFirst(CExprIteration& AstNode)
    {
        TGuardValue<CScope*> ScopeGuard(_Scope, AstNode._Filters->_AssociatedScope.Get());
        ULANG_ASSERT(_Scope);
        ...
        ULANG_ASSERTF(AstNode._Body.IsValid(), "Missing body in first");
        CurrentBlock->AppendSubExpr(NewIrNode<CIrIteratedBody>(GenNode(*AstNode._Body), false));

        TSRef<CIrAllOne> IrOne = NewIrNode<CIrAllOne>(Move(IrNode), false);
        IrOne->_bCanFail = AstNode.CanFail(ScopePackage);
        IrOne->IrSetResultType(AstNode.IrGetResultType());
    ...
    TSRef<CExprSync> GenSync(CExprSync& AstNode)
    {
        ...
        for (const TSRef<CExpressionBase>& SubExpr : SubExprs)
        {
            IrNode->AppendSubExpr(GenNode(*SubExpr));
        }
        return IrNode;
    ...
    {
        CScope* Scope = AstNode._Scope ? AstNode._Scope : _Scope;
        TGuardValue<CScope*> ScopeGuard(_Scope, Scope);
        TSRef<CExprBranch> IrNode = NewIrNode<CExprBranch>();
        IrNode->SetExpr(GenNode(AstNode.Expr()));
        IrNode->_Scope = AstNode._Scope;
        return IrNode;
    }
    ...
    TSRef<CExprBatch> GenBatch(CExprBatch& AstNode)
    {
        const TArray<TSRef<CExpressionBase>>& SubExprs = AstNode.GetSubExprs();
        TSRef<CExprBatch> IrNode = NewIrNode<CExprBatch>(SubExprs.Num());
        for (const TSRef<CExpressionBase>& SubExpr : AstNode.GetSubExprs())
        {
            IrNode->AppendSubExpr(GenNode(*SubExpr));
        }
    ...
    TSRef<CExprInterfaceDefinition> GenInterfaceDefinition(CExprInterfaceDefinition& AstNode)
    {
        ...
        if (_TargetVM == SBuildParams::EWhichVM::BPVM)
        {
            ...
            // If data member has value then add a new method to enclosing definition that calculates the value. 
            // i := interface:
            //        V:t = e 
            // =>
            // V_defun_XXXXXX():t = e     (XXXXXX is calculated from the qualified name to make it more unique in the module)  
            // i := interface
            //        V:t = e
            // In addition an expression for calling the new function is generated to be used by implementing classes to create the default value for the data member.
            CScope* EnclosingScope = GetModuleScopeForBindings(&SemanticInterface._EnclosingScope);
            if (!EnclosingScope)
            {
                AppendGlitch(
                    AstNode,
                    EDiagnostic::ErrSemantic_Internal,
                    CUTF8String("No enclosing scope while processing interface definition for %s", SemanticInterface.AsNameCString()));
                return IrNode;
            }
            for (CDataDefinition* DataMember : SemanticInterface.GetDefinitionsOfKind<CDataDefinition>())
            {
                ...
                // This doesn't work for digests since they don't tell if there are accessors or not.
                // Take care of that problem in SolarisCodeGenerator.
                if (DataMember->_OptionalAccessors)
                {
                    continue;
                }
                // Create definition for init value of property, if any
                // If this is a digest then the following is incorrect if the data field has accessors defined in the interface.
                // We will catch that error later.
                if (DataMember->HasInitializer())
                {
                    const Verse::Vst::Node* MappedVstNode = DataMember->GetIrNode()->GetMappedVstNode();

                    // Create IR for the function that creates the default value
                    // V_defun_XXXXXX():t = e
                    // where e is the default value of the data member
                    CUTF8String HashName = GetQualifiedNameString(*DataMember);
                    const uint32 Hash = UE::HashStringFNV1a32(FUtf8StringView(HashName.AsCString(), HashName.ByteLen()));
                    CSymbol NewName = _SemanticProgram->GetSymbols()->AddChecked(CUTF8String("%s_defun_%x", DataMember->GetName().AsCString(), Hash), true);

                    // V_defun_XXXXX()
                    TSRef<CExprIdentifierUnresolved> ElementCallee = TSRef<CExprIdentifierUnresolved>::New(NewName);
                    ElementCallee->SetIrMappedVstNode(MappedVstNode);
                    TSRef<CExprMakeTuple> ElementArgument = TSRef<CExprMakeTuple>::New();
                    ElementArgument->SetIrMappedVstNode(MappedVstNode);
                    CTupleType::ElementArray ElementArray;
                    const CTypeBase& ArgumentType = _SemanticProgram->GetOrCreateTupleType(Move(ElementArray));
                    ElementArgument->SetResultType(&ArgumentType);
                    const CFunctionType* FunctionType = &_SemanticProgram->GetOrCreateFunctionType(
                        ArgumentType,
                        *DataMember->GetType());
                    TSRef<CExprInvocation> Element = TSRef<CExprInvocation>::New(CExprInvocation::EBracketingStyle::Parentheses, Move(ElementCallee), Move(ElementArgument), FunctionType, DataMember->GetType());
                    Element->SetIrMappedVstNode(MappedVstNode);

                    // V_defun_XXXXX() := e
                    TSPtr<CExpressionBase> ValueDomain; // Ok to ignore this, since we are late in the compiler pipeline.
                    TSPtr<CExpressionBase> Value = DataMember->GetIrNode()->Value();
                    TSRef<CFunction> Function = EnclosingScope->CreateFunction(NewName);
                    Function->SetTrackLinkDependecies(true);
                    // Make the function internal. This prevents it from being included in digests
                    // (only packages with EVersePackageType::Assets includes internal, and there are no interfaces there),
                    // and the code that uses the function is created after verification of valid scope.
                    Function->SetAccessLevel({ SAccessLevel::EKind::Internal });
                    Function->MapSignature(*FunctionType, _SemanticProgram->GetRevision()); // Or should it be GetNextRevision?
                    TSRef<CExprFunctionDefinition> DefaultFunction = TSRef<CExprFunctionDefinition>::New(Function, Move(Element), Move(ValueDomain), Move(Value), EVstMappingType::Ir);
                    DefaultFunction->SetIrMappedVstNode(MappedVstNode);

                    // Would be nice to add to enclosing AST instead of this hack
                    DataMember->DefaultFunction = DefaultFunction;

                    // Create a call to default function, used by classes that implements this interface
                    // V_defun_XXXX()
                    TSRef<CExprIdentifierFunction> Callee = TSRef<CExprIdentifierFunction>::New(*Function, FunctionType);
                    Callee->SetIrMappedVstNode(MappedVstNode);
                    TSRef<CExprMakeTuple> Argument = TSRef<CExprMakeTuple>::New();
                    Argument->SetIrMappedVstNode(MappedVstNode);
                    Argument->SetResultType(&ArgumentType);
                    TSRef<CExprInvocation> DefaultValue = TSRef<CExprInvocation>::New(CExprInvocation::EBracketingStyle::Parentheses, Move(Callee), Move(Argument), FunctionType, DataMember->GetType());
                    DefaultValue->SetIrMappedVstNode(MappedVstNode);
                    DataMember->DefaultValue = DefaultValue;

                    // Rewrite data value in field to external{}
                    TSRef<CExprExternal> External = TSRef<CExprExternal>::New(_Program);
                    DataMember->GetIrNode()->SetValue(External);
                }
            }
        }
        return IrNode;
    ...
    TSRef<CNoop> GenNoop(CNoop& AstNode)
    {
        TSRef<CNoop> IrNode = NewIrNode<CNoop>(_Program);
        return IrNode;
    }
	...
	EffectCollector* CurrentEffectCollector{nullptr};
};
