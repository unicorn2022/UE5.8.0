...
#define RHI_VALIDATION_CONTEXT_CHECK(InExpression, Context, InMessage) \
	do \
	{ \
		if(UNLIKELY(!(InExpression))) \
		{ \
			FValidationRHI::ReportValidationFailure(InMessage, Context); \
		} \
	}while(0)
...
public:
	...
	// FlushType: Wait RHI Thread
	virtual FPixelShaderRHIRef RHICreatePixelShader(const FRHICreateShaderDesc& CreateShaderDesc) override final
	{
		return RHI->RHICreatePixelShader(CreateShaderDesc);
	}

	// FlushType: Wait RHI Thread
	virtual FVertexShaderRHIRef RHICreateVertexShader(const FRHICreateShaderDesc& CreateShaderDesc) override final
	{
		return RHI->RHICreateVertexShader(CreateShaderDesc);
	}

	// FlushType: Wait RHI Thread
	virtual FGeometryShaderRHIRef RHICreateGeometryShader(const FRHICreateShaderDesc& CreateShaderDesc) override final
	{
		check(RHISupportsGeometryShaders(GMaxRHIShaderPlatform));
		return RHI->RHICreateGeometryShader(CreateShaderDesc);
	}
	...
	virtual FMeshShaderRHIRef RHICreateMeshShader(const FRHICreateShaderDesc& CreateShaderDesc)
	{
		...
		return RHI->RHICreateMeshShader(CreateShaderDesc);
	}
	...
	{
		check(RHISupportsMeshShadersTier0(GMaxRHIShaderPlatform));
		return RHI->RHICreateAmplificationShader(CreateShaderDesc);
	}
	...
	// FlushType: Wait RHI Thread
	virtual FComputeShaderRHIRef RHICreateComputeShader(const FRHICreateShaderDesc& CreateShaderDesc) override final
	{
		return RHI->RHICreateComputeShader(CreateShaderDesc);
	}
	...
	*/
	...
	virtual FViewportRHIRef RHICreateViewport(void* WindowHandle, uint32 SizeX, uint32 SizeY, bool bIsFullscreen, EPixelFormat PixelFormat) override final
	{
		check(PixelFormat != PF_Unknown);
		return RHI->RHICreateViewport(WindowHandle, SizeX, SizeY, bIsFullscreen, PixelFormat);
	}

	virtual void RHIResizeViewport(FRHIViewport* Viewport, uint32 SizeX, uint32 SizeY, bool bIsFullscreen, EPixelFormat PixelFormat) override final
	{
		check(PixelFormat != PF_Unknown);
		RHI->RHIResizeViewport(Viewport, SizeX, SizeY, bIsFullscreen, PixelFormat);
	}

	virtual void RHIEndDrawingViewport(FRHICommandListImmediate& RHICmdList, FRHIViewport* Viewport, const FRHIPresentArgs& PresentArgs) override final
	{
		RHI->RHIEndDrawingViewport(RHICmdList, Viewport, PresentArgs);
	}
	...
	*/
	...
	virtual IRHIComputeContext* RHIGetCommandContext(ERHIPipeline Pipeline) override final;
	virtual IRHIUploadContext* RHIGetUploadContext() final override
	{
		return RHI->RHIGetUploadContext();
	}

	virtual FRHIFinalizeContextsResult RHIFinalizeContexts(FRHIFinalizeContextsArgs&& Args) override final;
	virtual FRHICloseTranslateChainResult RHICloseTranslateChain(FRHICloseTranslateChainArgs&& Args) override final;

	virtual void RHISubmitCommandLists(FRHISubmitCommandListsArgs&& Args) override final;
	
	virtual IRHICommandContext* RHIGetParallelCommandContext(FRHIParallelRenderPassInfo const& ParallelRenderPass) override final;
	...
	virtual FRayTracingShaderRHIRef RHICreateRayTracingShader(const FRHICreateShaderDesc& CreateShaderDesc, EShaderFrequency ShaderFrequency) override final
	{
		return RHI->RHICreateRayTracingShader(CreateShaderDesc, ShaderFrequency);
	}
	...
	{
		return RHI->RHIWriteResourceGPUAddresses(RHICmdList, GPUIndex, Resources, AddressesPtr);
	}

#if PLATFORM_SUPPORTS_BINDLESS_RENDERING
	RHI_API static void ValidateDescriptorRangeResources(const FRHIDescriptorRangeDesc& RangeDesc, TConstArrayView<FRHIDescriptorResource> Resources);

	virtual FDescriptorRangeRHIRef RHICreateDescriptorRange(FRHICommandListBase& RHICmdList, const FRHIDescriptorRangeDesc& Desc, TConstArrayView<FRHIDescriptorResource> InitialResources) override final
	{
		ValidateDescriptorRangeResources(Desc, InitialResources);
		return RHI->RHICreateDescriptorRange(RHICmdList, Desc, InitialResources);
	}
	virtual void RHIUpdateDescriptorRange(FRHICommandListBase& RHICmdList, FRHIDescriptorRange* Range, uint32 StartIndex, TConstArrayView<FRHIDescriptorResource> Resources) override final
	{
		RHI_VALIDATION_CHECK((uint64)StartIndex + Resources.Num() <= Range->GetDesc().Count,
			*FString::Printf(TEXT("UpdateDescriptorRange would write past end of range. StartIndex=%u, ResourceCount=%d, RangeCapacity=%u."),
				StartIndex, Resources.Num(), Range->GetDesc().Count));
		ValidateDescriptorRangeResources(Range->GetDesc(), Resources);
		RHI->RHIUpdateDescriptorRange(RHICmdList, Range, StartIndex, Resources);
	}
#endif

//protected:
	static void ReportValidationFailure(const TCHAR* InMessage, RHIValidation::FTracker* Tracker);
	static void ReportValidationFailure(const TCHAR* InMessage, FValidationComputeContext* Context);
	static void ReportValidationFailure(const TCHAR* InMessage, FValidationContext* Context);
	static void ReportValidationFailure(const TCHAR* InMessage, FRHICommandListBase* CmdList = nullptr);
