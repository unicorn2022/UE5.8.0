...
#include "Containers/Map.h"
	...
	#define UE_INPUT_LATENCY_TRACKING 1
#else
	#define UE_INPUT_LATENCY_TRACKING 0
#endif
...
public:
	...
	virtual ~FRHIFrameInfo() {}

	std::atomic<float> AverageInputLatencyMs = 0.0f;
	
public:
	void SetInputSampleMarker(uint64 FrameIndex) ;
	void SetPresentCounter(uint64 FrameIndex, uint64 PresentCounter);
	void SetVsyncDetails(const FRHIFlipDetails& NewFlipFrame);

	float GetAverageInputLatencyInMs()  { return AverageInputLatencyMs; }

private:
	void ComputeHistory(uint64 FrameIndex);
	void Trim();

	TMap<uint64,FRHIFrameInfoMarker> Frames;
	UE::FMutex FramesMutex;
};
...
#else
inline RHI_API void RHIFrameInfoSetInputSampleMarker(uint64 FrameNumber) {}
inline RHI_API void RHIFrameInfoSetPresentCounter(uint64 FrameNumber, uint64 PresentCounter) {}
inline RHI_API void RHIFrameInfoSetVsyncDetails(const FRHIFlipDetails& NewFlipFrame) {}
#endif 
