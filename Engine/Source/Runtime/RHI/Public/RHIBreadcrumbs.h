...
#define WITH_RHI_BREADCRUMBS (UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT || WITH_PROFILEGPU || HAS_GPU_STATS)
#endif
...
#define RHI_BREADCRUMBS_EMIT_LOCATION (WITH_RHI_BREADCRUMBS_FULL && CPUPROFILERTRACE_ENABLED && 1)
...
namespace UE::RHI::GPUProfiler
{
	...
	public:
		...
		// Random assigned color, based on static name
		FColor const Color;

		template <size_t N>
		FRHIBreadcrumbData(TCHAR const(&StaticName)[N], ANSICHAR const* File, uint32 Line)
			: FRHIBreadcrumbData_Location(File, Line)
			, StaticName(StaticName)
			, Color(FLinearColor::IntToDistinctColor(UE::HashStringFNV1a<uint32>(MakeStringView(StaticName, N - 1))).ToFColor(true))
		{}
	};
	...
	public:
...
#if HAS_GPU_STATS
		UE::RHI::GPUProfiler::FGPUStat* GPUStat = nullptr;
#endif
		...
		FRHIBreadcrumbNode(FRHIBreadcrumbData const& Data, FRHIBreadcrumbAllocator& Allocator, UE::RHI::GPUProfiler::FGPUStat* InGPUStat = nullptr)
			: Allocator(&Allocator)
			, Data(Data)
#if HAS_GPU_STATS
			, GPUStat(InGPUStat)
	...
	using TRHIBreadcrumbInitializer = std::tuple<TDesc const*, std::tuple<TValues...>, UE::RHI::GPUProfiler::FGPUStat*>;
	...
	public:
		...
		inline bool Contains(FRHIBreadcrumbAllocator* Allocator);
	};

	class FRHIBreadcrumbAllocator : public TSharedFromThis<FRHIBreadcrumbAllocator>
	{
		static thread_local FRHIBreadcrumbAllocatorArray PendingDestruction;
		static thread_local int32 DestructionDepth;
	...
	public:
		RHI_API ~FRHIBreadcrumbAllocator();
	...
	{
		if (!Contains(Allocator))
		{
			Add(Allocator->AsShared());
		}
	}

	inline bool FRHIBreadcrumbAllocatorArray::Contains(FRHIBreadcrumbAllocator* Allocator)
	{
	...
	namespace UE::RHI::Breadcrumbs::Private
	{
		...
		public:
			...
			uint32 GetTraceCpuSpec() const
			{
				...
				if (TraceCpuSpecId == 0 && bool(CpuChannel))
				{
					FScopeLock Lock(&Cs);
		...
		public:
			...
			TRHIBreadcrumb(FRHIBreadcrumbAllocator& Allocator, TDesc const& Desc, UE::RHI::GPUProfiler::FGPUStat* InGPUStat, TValues&&... Values)
				: FRHIBreadcrumbNode(Desc, Allocator, InGPUStat)
	...
	#define RHI_BREADCRUMB_PRIVATE_DEFINE(StaticName, FormatString, ValueType, GPUStat)	\
		...
		{																				\
			...
			> static const Desc(														\
				  FRHIBreadcrumbData(StaticName, __FILE__, __LINE__)					\
				, FormatString															\
				, Values...																\
			);																			\
																						\
	...
	#if HAS_GPU_STATS
		#define RHI_GPU_STAT_ARGS(StatName) (static_cast<UE::RHI::GPUProfiler::FGPUStat*>(&GPUStat_##StatName))
		#define RHI_GPU_STAT_ARGS_NONE      (static_cast<UE::RHI::GPUProfiler::FGPUStat*>(nullptr))
	#else
		#define RHI_GPU_STAT_ARGS(StatName) (static_cast<UE::RHI::GPUProfiler::FGPUStat*>(nullptr))
		#define RHI_GPU_STAT_ARGS_NONE      (static_cast<UE::RHI::GPUProfiler::FGPUStat*>(nullptr))
	#endif
	...
	#define RHI_BREADCRUMB_EVENT_PRIVATE_IMPL(RHICmdList_Or_RHIContext, Stat, Condition, StaticName, Format, ...)\
		TOptional<FRHIBreadcrumbScope> UE_JOIN(BreadcrumbScope, __LINE__);										 \
		do																										 \
		{																										 \
			if (Condition)																						 \
			{																									 \
