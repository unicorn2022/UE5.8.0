...
static FName NAME_PLATFORM_WINDOWS("Windows");
static FName NAME_PLATFORM_ANDROID("Android");
static FName NAME_PLATFORM_IOS("IOS");
static FName NAME_PLATFORM_MAC("Mac");
static FName NAME_PLATFORM_TVOS("TVOS");

static FName InvalidFeatureLevelName("InvalidFeatureLevel");
static FName InvalidShadingPathName("InvalidShadingPath");
...
{
	// Special case for alternate Qualcomm vendor id.
	if (static_cast<int32>(VendorId) == 0x4D4F4351)
	{
		return TEXT("Qualcomm");
	}
...
FString LexToString(EShaderPlatform Platform, bool bError)
{
	...
	case SP_METAL_SM6_IOS: return TEXT("METAL_SM6_IOS");
...
FString GetBufferUsageFlagsName(EBufferUsageFlags BufferUsage)
{
	return BuildEnumNameBitList<EBufferUsageFlags>(BufferUsage, [](EBufferUsageFlags BufferUsage)
	{
		...
		case BUF_Volatile:				return TEXT("BUF_Volatile");
		...
		ensureMsgf(false, TEXT("Unexpected buffer usage flag bit: %x"), uint32{EnumToUnderlyingType(BufferUsage)});
		return TEXT("BUF_Unknown");
	});
}
...
FString GetTextureCreateFlagsName(ETextureCreateFlags TextureCreateFlags)
{
	return BuildEnumNameBitList<ETextureCreateFlags>(TextureCreateFlags, [](ETextureCreateFlags TextureCreateFlags)
	{
		...
		case ETextureCreateFlags::Atomic64Compatible:				return TEXT("Atomic64Compatible");
		...
		ensureMsgf(false, TEXT("Unexpected texture creation flag bit: %" UINT64_x_FMT), uint64{EnumToUnderlyingType(TextureCreateFlags)});
		return TEXT("Unknown");
	});
}
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	RHI_RESOURCE_TYPE_DEF(RayTracingGeometry),
	RHI_RESOURCE_TYPE_DEF(RayTracingScene),
...
const TCHAR* GetBufferUsageFlagString(EBufferUsageFlags BufferUsage)
{
	...
	case EBufferUsageFlags::ReservedResource:
		return TEXT("ReservedResource");
	}
	return TEXT("");
}
...
const TCHAR* GetUniformBufferBaseTypeString(EUniformBufferBaseType BaseType)
{
	...
	case UBMT_RESOURCE_COLLECTION:
		return TEXT("UBMT_RESOURCE_COLLECTION");
	case UBMT_DESCRIPTOR_RANGE:
		return TEXT("UBMT_DESCRIPTOR_RANGE");
	}
	return TEXT("");
}
...
const TCHAR* GetShaderCodeResourceBindingTypeName(EShaderCodeResourceBindingType BindingType)
{
	...
	case EShaderCodeResourceBindingType::Texture2DMultiView:					return TEXT("Texture2DMultiView");
	...
	case EShaderCodeResourceBindingType::RWTexture2DMultiView:					return TEXT("RWTexture2DMultiView");
	...
	case EShaderCodeResourceBindingType::ResourceCollection:				return TEXT("ResourceCollection");
	case EShaderCodeResourceBindingType::DescriptorRange:					return TEXT("DescriptorRange");
	default:																checkf(false, TEXT("Missing EShaderCodeResourceBindingType %d"), int32{EnumToUnderlyingType(BindingType)});
	}
	return TEXT("");
}
...
const TCHAR* GetDescriptorTypeString(ERHIDescriptorType InDescriptorType)
{
	switch (InDescriptorType)
	{
	case ERHIDescriptorType::BufferSRV:             return TEXT("BufferSRV");
	case ERHIDescriptorType::BufferUAV:             return TEXT("BufferUAV");
	case ERHIDescriptorType::TypedBufferSRV:        return TEXT("TypedBufferSRV");
	case ERHIDescriptorType::TypedBufferUAV:        return TEXT("TypedBufferUAV");
	case ERHIDescriptorType::TextureSRV:            return TEXT("TextureSRV");
	case ERHIDescriptorType::TextureUAV:            return TEXT("TextureUAV");
	case ERHIDescriptorType::CBV:                   return TEXT("CBV");
	case ERHIDescriptorType::Sampler:               return TEXT("Sampler");
	case ERHIDescriptorType::AccelerationStructure: return TEXT("AccelerationStructure");
	case ERHIDescriptorType::Invalid:               return TEXT("Invalid");
	default:
		checkf(false, TEXT("Unknown DescriptorType: %d"), static_cast<uint32>(InDescriptorType));
		return TEXT("");
	}
}

FString GetDescriptorMaskString(ERHIDescriptorTypeMask InTypeMask)
{
	TStringBuilder<128> Result;

	if (InTypeMask == ERHIDescriptorTypeMask::None)
	{
		Result.Append(TEXT("<empty>"));
	}
	else
	{
		uint32 IterMask = static_cast<uint32>(InTypeMask);
		while (IterMask)
		{
			const uint32 TypeIndex = FBitSet::GetAndClearNextBit(IterMask);
			const ERHIDescriptorType DescriptorType = static_cast<ERHIDescriptorType>(TypeIndex);
			...
			Result.Append(GetDescriptorTypeString(DescriptorType));
		}
	}
