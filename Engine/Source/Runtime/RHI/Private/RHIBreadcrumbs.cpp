...
thread_local FRHIBreadcrumbAllocatorArray FRHIBreadcrumbAllocator::PendingDestruction;
thread_local int32 FRHIBreadcrumbAllocator::DestructionDepth = 0;

FRHIBreadcrumbAllocator::~FRHIBreadcrumbAllocator()
{
	//
	// There can be frames, particularly in the editor, where there are hundreds of RHI breadcrumb allocators
	// chained together via the Parents field. Limit the recursive stack depth of the destructor to prevent
	// stack overflow in these cases.
	// 
	// Allowing some recursion avoids paying the cost of using the PendingDestruction array in the general
	// gameplay case where the chain is only a few allocators long.
	//
	if (DestructionDepth > 2)
	{
		if (PendingDestruction.IsEmpty())
		{
			PendingDestruction = MoveTemp(Parents);
		}
		else
		{
			for (TSharedRef<FRHIBreadcrumbAllocator>& Item : Parents)
			{
				PendingDestruction.Emplace(Item);
			}
		}
	}
	else
	{
		TGuardValue Guard(DestructionDepth, DestructionDepth + 1);
		Parents.Reset();

		while (PendingDestruction.Num())
		{
			FRHIBreadcrumbAllocatorArray ToDelete = MoveTemp(PendingDestruction);
		}
	}
}
...
RHI_API FRHIBreadcrumbNode* const FRHIBreadcrumbNode::Sentinel = []()
{
	...
	static TRHIBreadcrumbDesc<0> StaticData(FRHIBreadcrumbData(TEXT("Sentinel"), __FILE__, __LINE__), nullptr);
...
RHI_API void FRHIBreadcrumbNode::WriteCrashData(struct FCrashContextExtendedWriter& Writer, const TCHAR* ThreadName) const
{
	...
	UE_LOGF(LogRHI, Error, "%ls", String);
}
...
RHI_API void FRHIBreadcrumbState::DumpActiveBreadcrumbs(TMap<FQueueID, TArray<FRHIBreadcrumbRange>> const& QueueRanges, EVerbosity Verbosity) const
{
	...
	switch (Verbosity)
	{
	case EVerbosity::Log    : UE_LOGF(LogRHI, Log    , "Active GPU breadcrumbs:%ls\r\n", *Tree); break; 
	case EVerbosity::Warning: UE_LOGF(LogRHI, Warning, "Active GPU breadcrumbs:%ls\r\n", *Tree); break;
	case EVerbosity::Error  : UE_LOGF(LogRHI, Error  , "Active GPU breadcrumbs:%ls\r\n", *Tree); break;
	}
