...
int32 GVsyncInformationInsights = 0;
static FAutoConsoleVariableRef CVarVsyncInformationInsights(
	...
	TEXT("0 - No information (default)")
	TEXT("1 - Input / Vsync frame counter information")
	TEXT("2 - Input / Vsync frame counter and present counter information")
);

static FRHIFrameInfo GRHIFrameInfo;
#endif
...
void FDumpTransitionsHelper::DumpResourceTransition(const FName& ResourceName, const ERHIAccess TransitionType)
{
	...
	if ((ResourceDumpName != NAME_None) && (ResourceDumpName == ResourceName))
	{
		...
		UE_LOGF(LogRHI, Log, "%ls transition to: %ls", *ResourceDumpName.ToString(), *GetRHIAccessName(TransitionType));
		UE_LOGF(LogRHI, Log, "%ls", ANSI_TO_TCHAR(DumpCallstack));
	}
}
...
static void OnEnableConsole120FpsCVarRHIChanged(IConsoleVariable* Variable)
{
	...
	if (InternalEnableConsole120Fps != GEnableConsole120Fps)
	{
		...
		UE_LOGF(LogRHI, Log, "Console 120Fps = %d", GEnableConsole120Fps);
	}
}
...
void DecodeRenderTargetMode(ESimpleRenderTargetMode Mode, ERenderTargetLoadAction& ColorLoadAction, ERenderTargetStoreAction& ColorStoreAction, ERenderTargetLoadAction& DepthLoadAction, ERenderTargetStoreAction& DepthStoreAction, ERenderTargetLoadAction& StencilLoadAction, ERenderTargetStoreAction& StencilStoreAction, FExclusiveDepthStencil DepthStencilUsage)
{
	...
	case ESimpleRenderTargetMode::EExistingColorAndDepthAndClearStencil:
		...
		break;
	default:
		UE_LOGF(LogRHI, Fatal, "Using a ESimpleRenderTargetMode that wasn't decoded in DecodeRenderTargetMode [value = %d]", (int32)Mode);
	}
...
void FRHIFrameInfo::SetInputSampleMarker(uint64 FrameIndex)
{
	uint64 Cycles64 = FPlatformTime::Cycles64(); 
		
	{
		UE::TUniqueLock ScopeLock(FramesMutex);

		FRHIFrameInfoMarker& FrameMarker = Frames.FindOrAdd(FrameIndex);
		FrameMarker.InputCycle = Cycles64;
		Trim();
	}	
}

void FRHIFrameInfo::SetPresentCounter(uint64 FrameIndex, uint64 PresentCounter)
{
	{
		UE::TUniqueLock ScopeLock(FramesMutex);

		FRHIFrameInfoMarker& FrameMarker = Frames.FindOrAdd(FrameIndex);
		FrameMarker.PresentCounter = PresentCounter;
		Trim();
	}
...
void FRHIFrameInfo::SetVsyncDetails(const FRHIFlipDetails& NewFlipFrame)
{ 
	uint64 LatestFrameCounter = 0;

	{
		UE::TUniqueLock ScopeLock(FramesMutex);

		for (TPair<uint64, FRHIFrameInfoMarker>& Elem : Frames)
		{
			if (Elem.Value.PresentCounter == NewFlipFrame.PresentIndex)
			{
				//if VBlankTimeInCycles is not set we compute it from VBlankTimeInSeconds
				uint64 VBlankTimeInCycle = (NewFlipFrame.VBlankTimeInCycles != 0) ? NewFlipFrame.VBlankTimeInCycles : ((uint64)(NewFlipFrame.VBlankTimeInSeconds / FPlatformTime::GetSecondsPerCycle64()));
				float Hz = NewFlipFrame.Hz;

				Elem.Value.VSyncCycle = VBlankTimeInCycle;
				//LatestFrameCounter = Elem.Value.FrameIndex;
				LatestFrameCounter = Elem.Key; 
			
				uint64 CurrentInputLatency = 0;

				if (VBlankTimeInCycle > Elem.Value.InputCycle)
				{
					CurrentInputLatency = VBlankTimeInCycle - Elem.Value.InputCycle;
				}

				if (CurrentInputLatency)
				{
					GInputLatencyTime = CurrentInputLatency;
				}

				Elem.Value.InputLatencyMS = FPlatformTime::ToMilliseconds64(CurrentInputLatency);

#if UE_INPUT_LATENCY_TRACKING
				if (GVsyncInformationInsights == 1)
				{
					TRACE_BOOKMARK_CYCLES(Elem.Value.InputCycle, TEXT("Input Frame: %" UINT64_FMT), LatestFrameCounter)

					if (Hz)
					{
						TRACE_BOOKMARK_CYCLES(VBlankTimeInCycle, TEXT("Vsync (%.2f Hz) Frame: %" UINT64_FMT), Hz, LatestFrameCounter)
					}
					else
					{
						TRACE_BOOKMARK_CYCLES(VBlankTimeInCycle, TEXT("Vsync Frame: %" UINT64_FMT), LatestFrameCounter)
					}
				}
				else if (GVsyncInformationInsights == 2)
				{
					TRACE_BOOKMARK_CYCLES(Elem.Value.InputCycle, TEXT("Input P:%" UINT64_FMT " F:%" UINT64_FMT), Elem.Value.PresentCounter, LatestFrameCounter)

					if (Hz)
					{
						TRACE_BOOKMARK_CYCLES(VBlankTimeInCycle, TEXT("Vsync (%.2f Hz) P:%" UINT64_FMT " F:%" UINT64_FMT " Lat:%.2f"), Hz, Elem.Value.PresentCounter, LatestFrameCounter, Elem.Value.InputLatencyMS)
					}
					else
					{
						TRACE_BOOKMARK_CYCLES(VBlankTimeInCycle, TEXT("Vsync P:%" UINT64_FMT " F:%" UINT64_FMT " Lat:%.2f"), Elem.Value.PresentCounter, LatestFrameCounter, Elem.Value.InputLatencyMS)
					}
				}
#endif
				break;
	...
	ComputeHistory(LatestFrameCounter);
}

void FRHIFrameInfo::ComputeHistory(uint64 FrameNumber)
{
	UE::TUniqueLock ScopeLock(FramesMutex);

	const uint64 FrameThreshold = FrameNumber < FrameInfoHistoryCount ? FrameInfoHistoryCount : FrameNumber - FrameInfoHistoryCount;

	uint32 ElementCount = 0;
	double AccumulatedInputLatency = 0.0;

	for (auto It = Frames.CreateIterator(); It; ++It)
	{
		if (It->Value.InputLatencyMS != 0.0)
		{
			AccumulatedInputLatency += It->Value.InputLatencyMS;
			ElementCount++;
		}

		if (It->Key <= FrameThreshold)
		{
			It.RemoveCurrent();
		}
	}
	...
	if (ElementCount)
	{
		AverageInputLatencyMs = AccumulatedInputLatency / ((double)ElementCount);
	}

	Trim();
}

void FRHIFrameInfo::Trim()
{
	// Array should not grow indefinitely 
	if (Frames.Num() > (FrameInfoHistoryCount * 2))
	{
		Frames.CreateIterator().RemoveCurrent();
	}
}

#if UE_INPUT_LATENCY_TRACKING
extern RHI_API void RHIFrameInfoSetVsyncDetails(const FRHIFlipDetails& NewFlipFrame)
{
	GRHIFrameInfo.SetVsyncDetails(NewFlipFrame);
}

extern RHI_API void RHIFrameInfoSetInputSampleMarker(uint64 FrameNumber)
{
	GRHIFrameInfo.SetInputSampleMarker(FrameNumber);
}

extern RHI_API void RHIFrameInfoSetPresentCounter(uint64 FrameNumber, uint64 PresentCounter)
{
	GRHIFrameInfo.SetPresentCounter(FrameNumber, PresentCounter);
}
#endif
