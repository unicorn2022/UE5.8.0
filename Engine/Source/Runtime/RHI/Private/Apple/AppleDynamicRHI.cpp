...
static inline bool ValidateAppleDynamicRHIOptions(FAppleDynamicRHIOptions* Options)
{
	...
	{
		UE_LOGF(LogRHI, Fatal, "-sm5 and -sm6 are mutually exclusive options but more than one was specified on the command line.");
		return false;
	}
	if (0 != (Options->ForceMTL & Options->ForceSM6))
	{
		UE_LOGF(LogRHI, Warning, "-mtl and -sm6 are incompatible options, using MetalRHI with SM5.");
		Options->ForceSM5 = 1;
		Options->ForceSM6 = 0;
		Options->ForceMTL = 1;
	}
	if(Options->ForceSM6)
	{
		...
		if(!bSupportsSM6)
		{
			...
			UE_LOGF(LogRHI, Fatal, "-sm6 is selected but Mac requires OS 15 to support SM6");
			return false;
		}
	}
	return true;
