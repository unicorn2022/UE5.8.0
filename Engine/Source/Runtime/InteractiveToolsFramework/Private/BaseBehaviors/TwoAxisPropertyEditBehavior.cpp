...
bool UTwoAxisPropertyEditInputBehavior::IsKeyboardPressed(const FInputDeviceState& InputState) const
{
	...
	if (!Target->GetCaptureKey().IsModifierKey())
	{
		// Explicitly guarding against unused modifiers here so that modified same-key hotkeys still get the input.
		// In this case,  If B is the capture key, Shift+B will not be captured and passes through to the next input handler
		// This is assuming this behavior only support a single key for capture key.
		const bool bModifierKeyDown = 
			FInputDeviceState::IsShiftKeyDown(InputState) ||
			FInputDeviceState::IsCtrlKeyDown(InputState) ||
			FInputDeviceState::IsCmdKeyDown(InputState) ||
			FInputDeviceState::IsAltKeyDown(InputState);
		if (bModifierKeyDown)
		{
			return false;
		}
