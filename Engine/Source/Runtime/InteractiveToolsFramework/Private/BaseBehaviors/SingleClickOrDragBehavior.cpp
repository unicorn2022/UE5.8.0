...
FInputCapturePriority USingleClickOrDragInputBehavior::GetPriority()
{
	const FInputCapturePriority BasePriority = Super::GetPriority();
	
	if (UE::ITF::Private::UseInputBindingComplexity())
	{
		return BasePriority.ApplyBindingComplexity(Modifiers.GetActiveModifierCount());
	}
...
FInputCaptureRequest USingleClickOrDragInputBehavior::WantsCapture(const FInputDeviceState& Input)
{
	...
	if (IsPressed(Input) && (ModifierCheckFunc == nullptr || ModifierCheckFunc(Input)) )
	{
		...
		{
			Modifiers.CacheModifierState(Input);
