...
void UInteractiveToolManager::DeactivateToolInternal(EToolSide Side, EToolShutdownType ShutdownType)
{
	if (Side == EToolSide::Left)
	{
		...
		// Note that the deregister is done because a tool that generates a modal window during Shutdown
		// could otherwise re-trigger input recapture, since modals continue pumping input messages.
...
void UInteractiveToolManager::CancelUndoTransaction()
{
	...
	bool bShowUnexpectedShutdownMessage, const FText& UnexpectedShutdownMessage)
{
	if (bShowUnexpectedShutdownMessage)
	{
		if (OnToolUnexpectedShutdownMessage.IsBound() == false )
		{
			...
			{
				FString ToolName = (Tool != nullptr) ? Tool->GetToolInfo().ToolDisplayName.ToString() : FString("(Null Tool)");
				if ( bInToolSetup )
				{
					UE_LOGF(LogTemp, Error, "[InteractiveToolManager] Tool %ls Could not be Initialized", *ToolName);
				}
				else
				{
					UE_LOGF(LogTemp, Error, "[InteractiveToolManager] Tool %ls was Shut Down Automatically, no message provided", *ToolName);
				}
			}
			else
			{
				UE_LOGF(LogTemp, Error, "[InteractiveToolManager] %ls", *UnexpectedShutdownMessage.ToString());
			}
		}
		else
		{
