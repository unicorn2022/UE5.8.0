...
namespace GizmoElementBaseLocals
{
	...
	static TAutoConsoleVariable<bool> CVarAllowFlattenedRendering(
		TEXT("ITF.Gizmos.AllowFlattenedRendering"),
		true,
		TEXT("When enabled, gizmo rendering will be flattened when possible, improving display at very large orthographic zoom levels"),
...
bool UGizmoElementBase::GetViewAlignRot(const FVector& InViewLocation, const FVector& InViewDirection, const FVector& InViewUp, bool bInPerspectiveView, const FTransform& InLocalToWorldTransform, const FVector& InLocalCenter, FQuat& OutAlignRot) const
{
	...
	else if (ViewAlignType == EGizmoElementViewAlignType::PointEye || ViewAlignType == EGizmoElementViewAlignType::PointScreen)
	{
		...
		FVector LocalViewUp = WorldToLocalTransform.TransformVectorNoScale(InViewUp);
...
void UGizmoElementBase::ApplyViewDepthOffset(const FVector& InViewLocation, const FVector& InViewDirection, const bool bInIsPerspectiveView, const double& InViewDepthOffset, FTransform& InOutTransform) const
{
	...
	{
		// Perspective views need to move the object away from the camera while also scaling so that they have the correct size.
		FVector CameraToObject = InOutTransform.GetLocation() - InViewLocation;
		const double DistanceToObject = CameraToObject.Length();
		...
		if (DistanceToObject < MinDistance)
		{
			...
			InOutTransform.SetLocation(InViewLocation + InViewDirection * InViewDepthOffset * InOutTransform.GetScale3D().X);
			return;
		}
		...
		InOutTransform.SetLocation(InViewLocation + CameraToObject * TargetDistance);
		InOutTransform.SetScale3D(InOutTransform.GetScale3D() * ScaleFactor);
	}
	else
	{
		// Orthographic cameras can simply shift the location along the camera forward axis without any scaling applied
		InOutTransform.SetLocation(InOutTransform.GetLocation() + InViewDirection * InViewDepthOffset);
	}
}
...
bool UGizmoElementBase::VerifyUniformScale(const FVector& Scale) const
{
	if (!FMath::IsNearlyEqual(Scale.X, Scale.Y, KINDA_SMALL_NUMBER) || !FMath::IsNearlyEqual(Scale.X, Scale.Z, KINDA_SMALL_NUMBER))
	{
		...
		{
			UE_LOGF(LogGizmoElementBase, Warning, "Gizmo element library view-dependent alignment does not currently support non-uniform scale (%f %f %f).",
...
bool UGizmoElementBase::IsVisible(
	...
	const FVector& InLocalCenter) const
{
	return (GetVisibleState() &&
		(!IsVisibleFunction.IsSet() || IsVisibleFunction(View, InCurrentViewInteractionState, InCurrentInteractionState, InLocalToWorldTransform, InLocalCenter)) &&
...
{
	// Call with "All" view interaction state, as this (deprecated) call path is unaware of the actual state
	return IsVisible(View, EViewInteractionState::All, InCurrentInteractionState, InLocalToWorldTransform, InLocalCenter);
}
...
bool UGizmoElementBase::IsHittable(const UGizmoViewContext* ViewContext, const FTransform& InLocalToWorldTransform, const FVector& InLocalCenter) const
{
	return (GetHittableState() &&
		(!IsHittableFunction.IsSet() || IsHittableFunction(ViewContext, InLocalToWorldTransform, InLocalCenter)) &&
...
bool UGizmoElementBase::UpdateRenderState(IToolsContextRenderAPI* RenderAPI, const FVector & InLocalCenter, FRenderTraversalState & InOutRenderState, bool& bOutHasAlignRot, FQuat& OutAlignRot)
{
	...
	if (IsVisible(View, RenderAPI->GetViewInteractionState(), InOutRenderState.InteractionState, InOutRenderState.LocalToWorldTransform, InLocalCenter))
	{
		bool bIsPerspectiveView;
		FVector ViewLocation, ViewDirection, ViewUp;
		GizmoElementBaseLocals::GetViewInfo<FSceneView>(View, ViewLocation, ViewDirection, ViewUp, bIsPerspectiveView);
...
void UGizmoElementBase::FRenderTraversalState::Initialize(const UE::GizmoRenderingUtil::ISceneViewInterface& InView, const FTransform& InTransform, const double InDPIScale)
{
	LocalToWorldTransform = InTransform;
	PixelToWorldScale = UE::GizmoRenderingUtil::CalculateViewDependentScaleAndFlatten(&InView, InTransform.GetLocation(), 0.5, FlattenScale) * InDPIScale;
}

PRAGMA_DISABLE_DEPRECATION_WARNINGS
void UGizmoElementBase::FRenderTraversalState::Initialize(const FSceneView* InSceneView, FTransform InTransform, const double InDPIScale)
{
	UE::GizmoRenderingUtil::FSceneViewWrapper Wrapper(*InSceneView);
	Initialize(Wrapper, InTransform, InDPIScale);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

FScaleMatrix UGizmoElementBase::FRenderTraversalState::SafeFlattenMatrix() const
{
	return SafeFlattenMatrix(LocalToWorldTransform);
}

FScaleMatrix UGizmoElementBase::FRenderTraversalState::SafeFlattenMatrix(const FTransform& InLocalToWorldTransform) const
{
	if (GizmoElementBaseLocals::CVarAllowFlattenedRendering.GetValueOnAnyThread() && InLocalToWorldTransform.GetRotation().IsIdentity())
	{
		return FScaleMatrix(FlattenScale);
	}
	return FScaleMatrix(FVector::One());
}

void UGizmoElementBase::FLineTraceTraversalState::Initialize(const UGizmoViewContext* InGizmoViewContext, FTransform InTransform)
{
	...
	PixelToWorldScale = UE::GizmoRenderingUtil::CalculateLocalPixelToWorldScale<double>(InGizmoViewContext, InTransform.GetLocation(), bUseDPIScaleFromViewContext);
}
...
void UGizmoElementBase::SetIsVisibleFunction(
	TUniqueFunction<bool(const FSceneView* InView, EViewInteractionState InCurrentViewInteractionState, EGizmoElementInteractionState InCurrentInteractionState, const FTransform&
	InLocalToWorldTransform, const FVector& InLocalCenter)>&& InIsVisibleFunction)
{
	IsVisibleFunction = MoveTemp(InIsVisibleFunction);
}

void UGizmoElementBase::SetIsHittableFunction(
	TUniqueFunction<bool(const UGizmoViewContext* InViewContext, const FTransform& InLocalToWorldTransform, const FVector& InLocalCenter)>&& InIsHittableFunction)
{
	IsHittableFunction = MoveTemp(InIsHittableFunction);
}
