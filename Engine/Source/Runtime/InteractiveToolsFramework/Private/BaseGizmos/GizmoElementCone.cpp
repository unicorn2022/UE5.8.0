...
namespace GizmoConeComponentLocals
{
	...
	void DrawDisc(FPrimitiveDrawInterface* InPDI, const FMatrix& InLocalToWorld, double InRadius, int32 InNumSides, const UMaterialInterface* InMaterial)
	{
		check(InNumSides >= 3);
		...
		{
			const FVector Vertex = (AxisX * FMath::Cos(AngleDelta * (SideIndex)) + AxisY * FMath::Sin(AngleDelta * (SideIndex))) * InRadius;
		...
		MeshBuilder.Draw(InPDI, InLocalToWorld, InMaterial->GetRenderProxy(), SDPG_Foreground,0.0f);
	}
}
...
void UGizmoElementCone::Render(IToolsContextRenderAPI* RenderAPI, const FRenderTraversalState& RenderState)
{
	...
	if (bVisibleViewDependent)
	{
		if (const UMaterialInterface* UseMaterial = CurrentRenderState.GetCurrentMaterial())
		{
			...
			DrawCone(PDI, RenderLocalToWorldMatrix, Angle, Angle, NumSides, false, FColor::White, UseMaterial->GetRenderProxy(), SDPG_Foreground);
			...
			if (bEndCaps)
			{
				...
				RenderLocalToWorldMatrix.RemoveScaling();
				RenderLocalToWorldMatrix.SetOrigin(Location);
				GizmoConeComponentLocals::DrawDisc(PDI, RenderLocalToWorldMatrix, Radius * HeightOverConeSide * CurrentRenderState.LocalToWorldTransform.GetScale3D().X, NumSides, UseMaterial);
			}
		}
	}
}
