...
TUniquePtr<FToolCommandChange> FTransformProxyChangeSource::EndChange()
{
	if (Proxy.IsValid())
	{
		...
		if (!ActiveChange)
		{
			...
			UE_LOGF(LogInteractiveToolsTransformProxy, Error, "FTransformProxyChangeSource: Re-entrant EndChange invalidated ActiveChange. Did you delete a gizmo on EndPivotEditSequence/EndTransformEditSequence?");
			return TUniquePtr<FToolCommandChange>();
		}
...
void FTransformProxyChangeSource::CancelChange()
{
	if (Proxy.IsValid() && ensureMsgf(ActiveChange, TEXT("FTransformProxyChangeSource: CancelChange called without a pending change.")))
	{
		if (ActiveChange->bSetPivotMode)
		{
			Proxy->EndPivotEditSequence();
