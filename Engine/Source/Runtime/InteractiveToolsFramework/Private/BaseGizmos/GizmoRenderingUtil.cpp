...
RealType UE::GizmoRenderingUtil::CalculateLocalPixelToWorldScale(
	...
	const TOptional<double>& InDPIScale)
{
	...
	// when the camera is very far from the location if the distance is relatively small.
	//
	// For perspective projection, the "W" value from WorldToScreen gives us a sense of distance, so we 
	// make the offset relative to that.
	//
	// For orthographic projection, W is constant (~1.0) regardless of zoom level, so instead we derive
	// the offset from the inverse of the projection matrix scale (M[0][0]), which encodes the ortho width.
	// This provides a zoom-proportional offset that maintains numerical stability at extreme zoom levels.
	...
	{
		// Perspective: offset proportional to distance (W component)
		FVector4 LocationScreenPoint = View->WorldToScreen(Location);
		OffsetDelta = LocationScreenPoint.W * Offset;
	}
	else
	{
		// Orthographic: offset proportional to ortho scale (inverse of M[0][0])
		// M[0][0] = 2 / OrthoWidth, so 1/M[0][0] = OrthoWidth/2 gives us a world-space measure
		const FMatrix& ProjMatrix = View->GetProjectionMatrix();
		OffsetDelta = FMath::Abs(1.0 / ProjMatrix.M[0][0]) * Offset;
	}
	...
	constexpr double MinPixelToWorldScale = (1.e-32); // Tiny, but non-zero. Built-in "SMALL_NUMBER" variants aren't small enough for micro-zoom levels.
	constexpr double MaxPixelToWorldScale = UE_DOUBLE_BIG_NUMBER; // Arbitrary large value to prevent division by zero or very small scale

	double PixelDeltaSqr = FMath::Max(MinPixelToWorldScale, VectorDifferenceSqr(PixelA, PixelB));
	double WorldDeltaSqr = VectorDifferenceSqr(Location, OffsetPointWorld);

	double Result = FMath::Sqrt(WorldDeltaSqr / PixelDeltaSqr);
	Result = FMath::Clamp(Result, MinPixelToWorldScale, MaxPixelToWorldScale);
	if (bInWithDPIScale)
	{
...
template INTERACTIVETOOLSFRAMEWORK_API float UE::GizmoRenderingUtil::CalculateLocalPixelToWorldScale<float>(
	const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, bool, const TOptional<double>&);

template INTERACTIVETOOLSFRAMEWORK_API double UE::GizmoRenderingUtil::CalculateLocalPixelToWorldScale<double>(
	const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, bool, const TOptional<double>&);
	...
	FVector& OutFlattenScale)
{
	...
	UE::GizmoRenderingUtil::FSceneViewWrapper Wrapper(*View);
	return CalculateViewDependentScaleAndFlatten(&Wrapper, Location, InScale, OutFlattenScale);
}

template <typename RealType>
RealType UE::GizmoRenderingUtil::CalculateViewDependentScaleAndFlatten(
	const UE::GizmoRenderingUtil::ISceneViewInterface* View, 
	const FVector& Location, 
	const RealType InScale, 
	FVector& OutFlattenScale)
{
	const FMatrix& ViewMatrix = View->GetViewMatrix();
	bool bIsPerspective = View->GetProjectionMatrix().M[3][3] < 1.0;
	bool bIsOrthoXY = !bIsPerspective && FMath::Abs(ViewMatrix.M[2][2]) > 0.0;
	bool bIsOrthoXZ = !bIsPerspective && FMath::Abs(ViewMatrix.M[1][2]) > 0.0;
	bool bIsOrthoYZ = !bIsPerspective && FMath::Abs(ViewMatrix.M[0][2]) > 0.0;
	RealType UniformScale = static_cast<RealType> (InScale * View->WorldToScreen(Location).W * (4.0 / View->GetUnscaledViewRect().Width() / View->GetProjectionMatrix().M[0][0]));
	...
	RealType MinimumScale = static_cast<RealType>(UE_DOUBLE_SMALL_NUMBER);
	if (FMath::Abs(UniformScale) < MinimumScale)
	{
		UniformScale = MinimumScale * (UniformScale < 0.0 ? -1.0 : 1.0);
	}

	if (bIsPerspective)
	{
		OutFlattenScale = FVector::OneVector;
	}
	else
	{
		...
		if (bIsOrthoXY)
		{
			OutFlattenScale = FVector(1.0, 1.0, 1.0 / UniformScale);
		}
		else if (bIsOrthoXZ)
		{
			OutFlattenScale = FVector(1.0, 1.0 / UniformScale, 1.0);
		}
		else if (bIsOrthoYZ)
		{
			OutFlattenScale = FVector(1.0 / UniformScale, 1.0, 1.0);
		}
	}
	return UniformScale;
}

template INTERACTIVETOOLSFRAMEWORK_API float UE::GizmoRenderingUtil::CalculateViewDependentScaleAndFlatten<float>(
	const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, float, FVector&);

template INTERACTIVETOOLSFRAMEWORK_API double UE::GizmoRenderingUtil::CalculateViewDependentScaleAndFlatten<double>(
	const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, double, FVector&);
