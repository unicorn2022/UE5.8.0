...
void UGizmoElementTorus::Render(IToolsContextRenderAPI* RenderAPI, const FRenderTraversalState& RenderState)
{
	...
	if (bVisibleViewDependent)
	{
		if (const UMaterialInterface* UseMaterial = CurrentRenderState.GetCurrentMaterial())
		{
			...
			FMatrix RenderLocalToWorldMatrix = CurrentRenderState.LocalToWorldTransform.ToMatrixWithScale();
			RenderLocalToWorldMatrix = RenderLocalToWorldMatrix * CurrentRenderState.SafeFlattenMatrix();
			RenderLocalToWorldMatrix.SetOrigin(CurrentRenderState.LocalToWorldTransform.GetTranslation()); // Restore origin

			DrawTorus(PDI, RenderLocalToWorldMatrix,
