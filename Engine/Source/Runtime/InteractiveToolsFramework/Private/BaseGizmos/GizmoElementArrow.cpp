...
FInputRayHit UGizmoElementArrow::LineTrace(const UGizmoViewContext* ViewContext, const FLineTraceTraversalState& LineTraceState, const FVector& RayOrigin, const FVector& RayDirection, FLineTraceOutput& OutLineTraceOutput)
{
	...
	// If we didn't hit the cylinder, OR it was hit, but not a surface hit, try the arrow head
	if (!Hit.bHit || !OutLineTraceOutput.bIsSurfaceHit)
	{
		check(HeadElement);
		FLineTraceOutput HeadLineTraceOutput = OutLineTraceOutput;
		FInputRayHit HeadHit = HeadElement->LineTrace(ViewContext, CurrentLineTraceState, RayOrigin, RayDirection, HeadLineTraceOutput);
		if (HeadHit.bHit && HeadLineTraceOutput.bIsSurfaceHit)
		{
			// If we're here, the cylinder was hit, but not on it's surface - but the head was hit directly on the surface.
			// This means we'll take priority over another element with a higher hit priority
			OutLineTraceOutput = HeadLineTraceOutput;
			Hit = HeadHit;
		}
	}
