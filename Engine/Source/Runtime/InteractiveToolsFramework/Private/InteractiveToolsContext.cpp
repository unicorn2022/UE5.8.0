...
UInteractiveToolsContext::UInteractiveToolsContext()
{
	...
	SetCreateToolGroupManagerFunc(  [](const FContextInitInfo& ContextInfo)
	{
		UToolGroupManager* NewToolGroupManager = NewObject<UToolGroupManager>(ContextInfo.ToolsContext);
		return NewToolGroupManager;
	});

	SetShutdownToolGroupManagerFunc( [] (UToolGroupManager* ToolGroupManagerIn)
	{
		ToolGroupManagerIn->Shutdown();	
	});
}
...
void UInteractiveToolsContext::SetCreateToolGroupManagerFunc(TUniqueFunction<UToolGroupManager*(const FContextInitInfo&)> Func)
{
	CreateToolGroupManagerFunc = MoveTemp(Func);
}
...
void UInteractiveToolsContext::SetShutdownToolGroupManagerFunc(TUniqueFunction<void(UToolGroupManager*)> Func)
{
	ShutdownToolGroupManagerFunc = MoveTemp(Func);
}
...
void UInteractiveToolsContext::Initialize(IToolsContextQueriesAPI* QueriesAPI, IToolsContextTransactionsAPI* TransactionsAPI)
{
	...
	ToolGroupManager = CreateToolGroupManagerFunc(InitInfo);
}


void UInteractiveToolsContext::Shutdown()
{
	ShutdownToolGroupManagerFunc(ToolGroupManager);
	ToolGroupManager = nullptr;
...
bool UInteractiveToolsContext::StartTool(EToolSide WhichSide, const FString& ToolTypeIdentifier)
{
	...
	{
		UE_LOGF(LogTemp, Warning, "ToolManager: Unknown Tool Type %ls", *ToolTypeIdentifier);
		return false;
	}
	else
	{
