...
struct FMouseInputDeviceState
{
	...
	/** The 2D position of the mouse, unconstrained by screen real-estate. Computed by summing unbound deltas. */
	UPROPERTY(transient, BlueprintReadWrite, Category = MouseInputDeviceState)
	FVector2D UnboundedPosition2D;

	/** Change in 2D mouse position, valid also for movements outside the display bounds */
	UPROPERTY(transient, BlueprintReadWrite, Category = MouseInputDeviceState)
	FVector2D UnboundedMouseDelta2D;

	/** Ray into current 3D scene at current unbounded 2D mouse position */
	UPROPERTY(transient, BlueprintReadWrite, Category = MouseInputDeviceState)
	FRay UnboundedWorldRay;
	...
	FMouseInputDeviceState()
	{
		...
		WorldRay = FRay();
		UnboundedPosition2D = FVector2D::ZeroVector;
		UnboundedMouseDelta2D = FVector2D::ZeroVector;
		UnboundedWorldRay = FRay();
	}
};
...
struct FInputDeviceState
{
	...
	UE_EXPERIMENTAL(5.8, "Experimental extension that allows for generic access to state via FKey.")
	DECLARE_DELEGATE_RetVal_OneParam(FDeviceButtonState, FGetDeviceButtonState, const FKey&);
	...
	/** @return The state for the current button. */
	UE_EXPERIMENTAL(5.8, "Experimental extension that allows for generic access to state via FKey.")
	INTERACTIVETOOLSFRAMEWORK_API FDeviceButtonState GetButtonState(const FKey& Key) const;

PRAGMA_DISABLE_EXPERIMENTAL_WARNINGS
	UE_EXPERIMENTAL(5.8, "Experimental extension that allows for generic access to state via FKey.")
	FGetDeviceButtonState OnGetButtonStateFallback;
