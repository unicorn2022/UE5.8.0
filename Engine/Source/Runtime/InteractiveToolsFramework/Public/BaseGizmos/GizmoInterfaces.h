...
public:
	...
	/** Sets the optional transform adjuster to apply to the tested geometry. */
	virtual void SetTransformAdjuster(const TSharedPtr<UE::GizmoRenderingUtil::IViewBasedTransformAdjuster>& InAdjuster) { /** Empty default implementation. */ }

	/** Get the (optional) transform adjuster applied to this Target. */
	virtual TSharedPtr<UE::GizmoRenderingUtil::IViewBasedTransformAdjuster> GetTransformAdjuster() const { return nullptr; /** No adjuster by default. */ }
};
...
public:
	...
	/**
	 * CancelUpdate is called when a gizmo cancels and reverts the changes to a parameter (via a ParameterSource)
	 */
	UFUNCTION()
