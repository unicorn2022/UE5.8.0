...
namespace UE::GizmoRenderingUtil
{
	...
	public:
		...
		virtual const FMatrix& GetProjectionMatrix() const { return View->ViewMatrices.GetViewToClip(); }
		virtual const FMatrix& GetViewMatrix() const { return View->ViewMatrices.GetWorldToView(); }
	...
	INTERACTIVETOOLSFRAMEWORK_API RealType CalculateLocalPixelToWorldScale(
	...
	extern template float CalculateLocalPixelToWorldScale<float>(const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, bool, const TOptional<double>&);

	extern template double CalculateLocalPixelToWorldScale<double>(const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, bool, const TOptional<double>&);
	 ...
	 */
	template <typename RealType = float>
	INTERACTIVETOOLSFRAMEWORK_API RealType CalculateViewDependentScaleAndFlatten(
		const UE::GizmoRenderingUtil::ISceneViewInterface* View,
		const FVector& Location,
		const RealType Scale,
		FVector& OutWorldFlattenScale);

	extern template float CalculateViewDependentScaleAndFlatten<float>(const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, float, FVector&);

	extern template double CalculateViewDependentScaleAndFlatten<double>(const UE::GizmoRenderingUtil::ISceneViewInterface*, const FVector&, double, FVector&);
