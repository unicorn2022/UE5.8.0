...
public:
	...
	*/
	...
	/**
	* If true, behavior will update modifiers also when not capturing. If false, modifiers will be updated only while capturing.
	*/
	bool bAlwaysUpdateModifiers = false;

protected:
	INTERACTIVETOOLSFRAMEWORK_API virtual void OnKeyPressedInternal(const FInputDeviceState& Input);
	INTERACTIVETOOLSFRAMEWORK_API virtual void OnKeyReleasedInternal(const FInputDeviceState& Input);
