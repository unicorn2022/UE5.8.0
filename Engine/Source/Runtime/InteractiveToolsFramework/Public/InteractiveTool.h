...
protected:
	 ...
	 */
	UE_DEPRECATED(5.8, "Use FindDynamicPropertyCache or FindOrAddDynamicPropertyCache instead.")
	...
	/**
	 * FindDynamicPropertyCache returns class-internal objects that subclasses can use to save/restore properties, or nullptr if
	 * the cache entry does not exist.
	 * @param CacheIdentifier multiple versions of the UInteractiveToolPropertySet can be stored, CacheIdentifier indicates which one to use
	 * @return instance of the current subclass that can be used to save/restore values, or nullptr if the entry does not exist.
	 */
	INTERACTIVETOOLSFRAMEWORK_API TObjectPtr<UInteractiveToolPropertySet> FindDynamicPropertyCache(const FString& CacheIdentifier) const;
	
	/**
	 * FindOrAddDynamicPropertyCache returns class-internal objects that subclasses can use to save/restore properties. Creates
	 * a cache entry if one does not yet exist.
	 * @param CacheIdentifier multiple versions of the UInteractiveToolPropertySet can be stored, CacheIdentifier indicates which one to use
	 * @return instance of the current subclass that can be used to save/restore values
	 */
	INTERACTIVETOOLSFRAMEWORK_API TObjectPtr<UInteractiveToolPropertySet> FindOrAddDynamicPropertyCache(const FString& CacheIdentifier) const;
