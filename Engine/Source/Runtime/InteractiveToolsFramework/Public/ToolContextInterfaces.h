...
{
	/** Whether to perform any kind of snapping */
	bool bIsSnappingActive = true;
	...
	/** 
	 * @return true if position grid snapping is active (snapping enabled AND position grid snapping enabled)
	 */
	bool IsPositionGridSnappingActive() const { return bIsSnappingActive && bEnablePositionGridSnapping; }
	...
	/** 
	 * @return true if rotation grid/angular snapping is active (snapping enabled AND rotation grid snapping enabled)
	 */
	bool IsRotationGridSnappingActive() const { return bIsSnappingActive && bEnableRotationGridSnapping; }
	...
	/** 
	 * @return true if scale grid snapping is active (snapping enabled AND scale grid snapping enabled)
	 */
	bool IsScaleGridSnappingActive() const { return bIsSnappingActive && bEnableScaleGridSnapping; }
	...
	{
		/** Specify whether this snapping type is enabled */
		bool bEnable = false;

		/** The distance/radius around a candidate transform to automatically snap to. 0.0 requires precise placement. */
		float SnapDistance = 0.0f;
	} ObjectTransform;
	
	/** 
	 * @return true if object transform snapping is active (snapping enabled AND object transform snapping enabled)
	 */
	bool IsObjectTransformSnappingActive() const { return bIsSnappingActive && ObjectTransform.bEnable; }
};
...
private:
	...
	TOptional<bool> UseSoftwareCursorOverride;
...
public:
	...
	void SetUseSoftwareCursor(const TOptional<bool>& InUseSoftwareCursorOverride)
	{
		UseSoftwareCursorOverride = InUseSoftwareCursorOverride;
	}
	
	TOptional<bool> GetUseSoftwareCursorOverride() const
	{
		return UseSoftwareCursorOverride;
	}
	...
	virtual void ClearCursorOverride() 
	{
		...
		CursorOverrideShape = EMouseCursor::Default;
		CursorOverrideWidget = nullptr;
		UseSoftwareCursorOverride.Reset();
	}
};
...
{
public:
	virtual ~IToolsContextQueriesAPI() = default;
...
{
public:
	virtual ~IToolsContextTransactionsAPI() = default;
	...
	/**
	 * Cancel and revert a started Transaction.
	 */
	virtual void CancelUndoTransaction() = 0;
...
{
public:
	virtual ~IToolsContextRenderAPI() = default;
