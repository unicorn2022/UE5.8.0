...
TSharedPtr<IAnalyticsProvider> FAnalytics::CreateAnalyticsProvider( const FName& ProviderModuleName, const FAnalyticsProviderConfigurationDelegate& GetConfigValue )
{
	...
	if (ProviderModuleName != NAME_None)
	{
		...
		if (Module != NULL)
		{
			UE_LOGF(LogAnalytics, Log, "Creating configured Analytics provider %ls", *ProviderModuleName.ToString());
			return Module->CreateAnalyticsProvider(GetConfigValue);
		}
		else
		{
			UE_LOGF(LogAnalytics, Warning, "Failed to find Analytics provider named %ls.", *ProviderModuleName.ToString());
		}
	}
	else
	{
		UE_LOGF(LogAnalytics, Warning, "CreateAnalyticsProvider called with a module name of None.");
	}
	return NULL;
}
...
FString FAnalytics::GetConfigValueFromIni( const FString& IniName, const FString& SectionName, const FString& KeyName, bool bIsRequired )
{
	...
	{
		UE_LOGF(LogAnalytics, Verbose, "Analytics missing Key %ls from %ls[%ls].", *KeyName, *IniName, *SectionName);
	}
	return Result;
}
