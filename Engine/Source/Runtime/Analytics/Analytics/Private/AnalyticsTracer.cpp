...
void FAnalyticsSpan::Start(const TArray<FAnalyticsEventAttribute>& AdditionalAttributes)
{
	...
	// Always add the Span GUID to the attributes so we can join this span to other events
	Attributes.Add(FAnalyticsEventAttribute(TEXT("Span_GUID"), Guid.ToString()));
}
...
void FAnalyticsSpan::End(const TArray<FAnalyticsEventAttribute>& AdditionalAttributes)
{
	...
	const FString SpanEventName = "Span";
