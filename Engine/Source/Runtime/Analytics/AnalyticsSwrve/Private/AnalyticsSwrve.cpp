...
TSharedPtr<IAnalyticsProvider> FAnalyticsSwrve::CreateAnalyticsProvider(const FAnalyticsProviderConfigurationDelegate& GetConfigValue) const
{
	...
	{
		UE_LOGF(LogAnalytics, Warning, "CreateAnalyticsProvider called with an unbound delegate");
	}
	return NULL;
}
...
TSharedPtr<IAnalyticsProvider> FAnalyticsSwrve::CreateAnalyticsProvider(const Config& ConfigValues) const
{
	...
	{
		UE_LOGF(LogAnalytics, Warning, "CreateAnalyticsProvider config did not contain required parameter %ls", *Config::GetKeyNameForAPIKey());
		return NULL;
	}
...
static void ApplyABTestDiffs(const FString& ResourceDiffPayload)
{
	...
	FString ResourceDiffPayloadJson = FString("{\"Tests\":") + ResourceDiffPayload + TEXT("}");
	...
	if ( FJsonSerializer::Deserialize( Reader, JsonObject ) )
	{
		...
		for (TArray<TSharedPtr<FJsonValue> >::TConstIterator it(Tests); it; ++it)
		{
			...
			if (IniTokens.Num() < 2)
			{
				UE_LOGF(LogAnalytics, Warning, "Failed to parse resource name %ls into an INI file and section", *IniKey);
				continue;
			}
			TSharedPtr<FJsonObject> TestDiff = TestObj->GetObjectField(TEXT("diff"));
			for (TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>::TConstIterator ResIt(TestDiff->Values); ResIt; ++ResIt)
			{
				FString NewValue = TestDiff->GetObjectField(ResIt.Key())->GetStringField(TEXT("new"));
				UE_LOGF(LogAnalytics, VeryVerbose, "Got an ABTtest resource for Engine[%ls]%ls=%ls", *IniTokens[1], *ResIt.Key(), *NewValue);
				...
				{
					UE_LOGF(LogAnalytics, Warning, "Didn't understand INI resource name %ls for ABTest resource", *IniTokens[0]);
				}
			}
		}
	}
	...
	{
		UE_LOGF(LogAnalytics, Warning, "Failed to parse Swrve AB test resource diff payload '%ls'. Errors: %ls", *ResourceDiffPayload, *Reader->GetErrorMessage());
	}
}
...
{
	UE_LOGF(LogAnalytics, Verbose, "Initializing Swrve Analytics provider");

	APIKey = ConfigValues.APIKeySwrve;
	if (APIKey.IsEmpty())
	{
		UE_LOGF(LogAnalytics, Warning, "AnalyticsSwrve missing APIKey. No events will be processed.");
	}
	...
	UE_LOGF(LogAnalytics, Log, "Swrve APIKey = %ls. APIServer = %ls. AppVersion = %ls", *APIKey, *APIServer, *AppVersion);
...
{
	UE_LOGF(LogAnalytics, Verbose, "Destroying Swrve Analytics provider");
	EndSession();
}
...
{
	UE_LOGF(LogAnalytics, VeryVerbose, "Swrve Method: %ls. Params: %ls. Payload:\n%ls", *MethodName, *OptionalParams, *Payload);

	if (UserID.IsEmpty())
	{
		UE_LOGF(LogAnalytics, Warning, "%ls called without a valid UserID. Ignoring.", *MethodName);
		return false;
	}
	...
	UE_SUPPRESS(LogAnalytics, Warning, 
	{
		...
		if (Now >= NextEventRateDetectionWindowTimeSec)
		{
			...
			{
				UE_LOGF(LogAnalytics, Warning, "Sending too many events to Swrve (%d) in the past %.2lf seconds (Rate of %.2lf / sec, Max Rate is %.2lf / sec). You may be exceeding Swrve quotas.", EventRateDetectionWindowCount, TimePassedSec, ActualRate, LimitRate);
			}
...
{
	UE_LOGF(LogAnalytics, Log, "AnalyticsSwrve::StartSession [%ls]",*APIKey);
...
void FAnalyticsProviderSwrve::SetUserID(const FString& InUserID)
{
	...
	{
		UE_LOGF(LogAnalytics, Log, "SetUserId %ls", *InUserID);
		UserID = InUserID;
	}
	else if (UserID != InUserID)
	{
		UE_LOGF(LogAnalytics, Log, "Overriding SetUserId %ls with cmdline UserId of %ls.", *InUserID, *UserID);
	}
}
...
void FAnalyticsProviderSwrve::RecordEvent(const FString& EventName, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	FString EventParams = "swrve_payload=";
	...
	if (Attributes.Num() > 0)
	{
		...
		for (int Ndx = 0; Ndx < DefaultEventAttributes.Num(); ++Ndx)
		{
			...
			EventParams += FString("\"") + DefaultEventAttributes[Ndx].GetName() + TEXT("\": \"") + DefaultEventAttributes[Ndx].GetValue() + TEXT("\"");
		}
		...
		for (int Ndx=0;Ndx<Attributes.Num();++Ndx)
		{
			...
			EventParams += FString("\"") + Attributes[Ndx].GetName() + TEXT("\": \"") + Attributes[Ndx].GetValue()+ TEXT("\"");
		}
		EventParams += TEXT("}");
	}
	...
	FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
	...
	{
		UE_LOGF(LogAnalytics, VeryVerbose, "Swrve response for [%ls]. Code: %d. Payload: %ls", 
			*HttpRequest->GetURL(), HttpResponse->GetResponseCode(), *HttpResponse->GetContentAsString());
	}
	else
	{
		UE_LOGF(LogAnalytics, VeryVerbose, "Swrve response for [%ls]. No response", 
			*HttpRequest->GetURL());
	}
}
	...
	FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
	if (bSucceeded &&
		HttpResponse.IsValid())
	{
		UE_LOGF(LogAnalytics, VeryVerbose, "Swrve ABTest response for [%ls]. Code: %d. Payload: %ls", 
			*HttpRequest->GetURL(), HttpResponse->GetResponseCode(), *HttpResponse->GetContentAsString());
		...
		{
			UE_LOGF(LogAnalytics, Warning, "Swrve returned failure for AB test resources request [%ls]. Code: %d. Payload: %ls", 
				*HttpRequest->GetURL(), HttpResponse->GetResponseCode(), *HttpResponse->GetContentAsString());
		}
	}
	else
	{
		UE_LOGF(LogAnalytics, Warning, "Swrve ABTest response for [%ls]. No response", 
			*HttpRequest->GetURL());
	}
}
