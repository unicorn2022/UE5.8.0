	...
	, EventCache(ConfigValues.MaximumPayloadSize, ConfigValues.UseLegacyProtocol ? 0 : ConfigValues.PreallocatedPayloadSize)
{
	...
	{
		UE_LOGF(LogAnalytics, Fatal, "AnalyticsET: APIKey (%ls) cannot be empty!", *Config.APIKeyET);
	}
	...
	UE_LOGF(LogAnalytics, Verbose, "[%ls] Initializing ET Analytics provider", *Config.APIKeyET);
	...
	{
		UE_LOGF(LogAnalytics, Warning, "AnalyticsET: APIServerET is empty for APIKey (%ls), creating as a NULL provider!", *Config.APIKeyET);
	}

#if !UE_HTTP_SUPPORT_UNIX_SOCKET
	if (!Config.APIUnixSocketPathET.IsEmpty())
	{
		UE_LOGF(LogAnalytics, Warning, "[%ls] Specified UnixSocketPath '%ls' but that is not supported on this platform", *Config.APIKeyET, *Config.APIUnixSocketPathET);
	}
...
bool FAnalyticsProviderET::Tick(float DeltaSeconds)
{
	...
	if (bShouldFlush)
	{
		...
		{
			UE_LOGF(LogAnalytics, Verbose, "[%ls] Tried to flush, but another analytics provider has already flushed this frame. Deferring until next frame.", *Config.APIKeyET);
		}
		else
		{
...
{
	LLM_SCOPE_BYNAME(TEXT("Analytics"));
	UE_LOGF(LogAnalytics, Display, "[%ls] AnalyticsET::StartSession ( APIServer = %ls%ls. AppVersion = %ls )", *Config.APIKeyET, *Config.APIServerET, *Config.APIEndpointET, *Config.AppVersionET);
...
void FAnalyticsProviderET::EndSession()
{
	...
	if (bSessionInProgress)
	{
		...
		UE_LOGF(LogAnalytics, Display, "[%ls] AnalyticsET::EndSession", *Config.APIKeyET);
	}
...
TSharedRef<IHttpRequest, ESPMode::ThreadSafe> FAnalyticsProviderET::CreateRequest(EAnalyticsRecordEventMode Mode)
{
	...
	{
		UE_LOGF(LogAnalytics, Display, "[%ls] ET Analytics provider tried to create a new HTTP request when HTTP was shutdown", *Config.APIKeyET);
	}
...
void FAnalyticsProviderET::ExecuteRequest(TArray<uint8>& Payload, OUT int32& PayloadSize, OUT int32& EventCount, EAnalyticsRecordEventMode Mode)
{
	...
	UE_LOGF(LogAnalytics, VeryVerbose, "[%ls] AnalyticsET URL:%ls?SessionID=%ls&AppID=%ls&AppVersion=%ls&UserID=%ls&AppEnvironment=%ls&UploadType=%ls. Payload:%.*s",
		*Config.APIKeyET,
...
void FAnalyticsProviderET::SetAppVersion(FString&& InAppVersion)
{
	...
	{
		UE_LOGF(LogAnalytics, Log, "[%ls] Updating AppVersion to %ls from old value of %ls", *Config.APIKeyET, *InAppVersion, *Config.AppVersionET);
...
void FAnalyticsProviderET::SetUserID(const FString& InUserID)
{
	...
	{
		UE_LOGF(LogAnalytics, Log, "[%ls] SetUserId %ls", *Config.APIKeyET, *InUserID);
	...
	{
		UE_LOGF(LogAnalytics, Log, "[%ls] Overriding SetUserId %ls with cmdline UserId of %ls.", *Config.APIKeyET, *InUserID, *UserID);
	}
}
...
bool FAnalyticsProviderET::SetSessionID(const FString& InSessionID)
{
	if (SessionID != InSessionID)
	{
		...
		UE_LOGF(LogAnalytics, Log, "[%ls] Forcing SessionID to %ls.", *Config.APIKeyET, *SessionID);
	}
	return true;
}
...
void FAnalyticsProviderET::EventRequestComplete(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool)
{
	...
	{
		UE_LOGF(LogAnalytics, VeryVerbose, "[%ls] ET response for [%ls]. Code: %d. Payload: %ls", *Config.APIKeyET, *HttpRequest->GetURL(), HttpResponse->GetResponseCode(), *HttpResponse->GetContentAsString());
	...
	{
		UE_LOGF(LogAnalytics, VeryVerbose, "[%ls] ET response for [%ls]. No response", *Config.APIKeyET, *HttpRequest->GetURL());
	}
}
...
void FAnalyticsProviderET::SetUrlDomain(const FString& Domain, const TArray<FString>& AltDomains)
{
	...
	if (Config.APIServerET.IsEmpty())
	{
		UE_LOGF(LogAnalytics, Warning, "AnalyticsET: APIServerET is empty for APIKey (%ls), converting to a NULL provider!", *Config.APIKeyET);
	}
	else
	{
		UE_LOGF(LogAnalytics, Log, "AnalyticsET: Set APIServerET to %ls", *Config.APIServerET);
	}
}
...
void FAnalyticsProviderET::FlushEventLegacy(const FString& EventName, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	UE_AUTORTFM_ONCOMMIT(this, EventName = EventName, Attributes = Attributes)
	{
		...
		if (ensure(FModuleManager::Get().IsModuleLoaded("HTTP")))
		{
			...
			UE_LOGF(LogAnalytics, VeryVerbose, "[%ls] AnalyticsET URL:SendEvent.1?SessionID=%ls&AppID=%ls&AppVersion=%ls&UserID=%ls&EventName=%ls%ls",
				*Config.APIKeyET,
				*SessionID,
				*Config.APIKeyET,
				*Config.AppVersionET,
