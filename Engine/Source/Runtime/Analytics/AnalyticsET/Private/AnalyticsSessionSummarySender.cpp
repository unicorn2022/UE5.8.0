...
bool FAnalyticsSessionSummarySender::SendSessionSummary(const FString& UserId, const FString& AppId, const FString& AppVersion, const FString& SessionId, const TArray<FAnalyticsEventAttribute>& Properties)
{
	...
	//{
	//	UE_LOGF(LogAnalyticsSessionSummarySender, Display, "Attr %ls = %ls", *Att.GetName(), *Att.GetValue());
	//}

	UE_LOGF(LogAnalyticsSessionSummarySender, Log, "Sent summary session report for: AppId=%ls SessionId=%ls", *AppId, *SessionId);
	return true;
}
