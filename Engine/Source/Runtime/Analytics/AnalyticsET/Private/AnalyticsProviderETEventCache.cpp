...
void FAnalyticsProviderETEventCache::AddToCache(FString EventName, const TArray<FAnalyticsEventAttribute>& Attributes)
{
	...
	{
		UE_LOGF(LogAnalytics, VeryVerbose, "AddToCache for event (%ls) may overflow MaximumPayloadSize (%d). Payload is currently (%d) bytes, and event will use an estimated (%d) bytes. Queuing up existing payload for flush before adding this event.", *EventName, MaximumPayloadSize, CachedEventUTF8Stream.Num(), EventSizeEstimate);
		QueueFlush();
	}
...
void FAnalyticsProviderETEventCache::QueueFlush()
{
	...
	if (bPlayloadTooLarge)
	{
		...
		UE_LOGF(LogAnalytics, Warning, "EventCache payload exceeded the maximum allowed size (%.3f KB > %.3f KB), containing %d events. Listing events in the payload for investigation:",
		...
		for (const FAnalyticsEventEntry& Entry : CachedEventEntries)
		{
			UE_LOGF(LogAnalytics, Warning, "    %ls,%d", *Entry.EventName, Entry.EventSizeChars);
		}
	}
	...
	{
		UE_LOGF(LogAnalytics, Display, "EventCache took too long to flush (%.3f ms > %.3f ms). Payload size: %.3f KB, %d events. Listing events in the payload for investigation:",
		...
		for (const FAnalyticsEventEntry& Entry : CachedEventEntries)
		{
			UE_LOGF(LogAnalytics, Display, "    %ls,%d", *Entry.EventName, Entry.EventSizeChars);
		}
	}
...
bool FAnalyticsProviderETEventCacheTest::RunTest(const FString& Parameters)
{
	...
	{
		FString TheTestName = "BasicStrings";
		...
		cache.AddToCache(TheTestName, MakeAnalyticsEventAttributeArray(
			...
			TEXT("FStringStringAttribute"), FString("FStringValue")
		));

		FString ExpectedResult = "{\"Events\":[{\"EventName\":\"BasicStrings\",\"DateOffset\":\"+00:00:00.000\",\"ConstantStringAttribute\":\"ConstantStringValue\",\"FStringStringAttribute\":\"FStringValue\"}]}";
		TestEqual(TheTestName, cache.FlushCache(), ExpectedResult);
	}

	{
		FString TheTestName = "UnicodeEvent";
		FAnalyticsProviderETEventCache cache;
	...
	{
		FString TheTestName = "NumericalEvent";
		FAnalyticsProviderETEventCache cache;
		cache.AddToCache(TheTestName, MakeAnalyticsEventAttributeArray(
		...
		FString ExpectedResult = "{\"Events\":[{\"EventName\":\"NumericalEvent\",\"DateOffset\":\"+00:00:00.000\",\"IntAttr\":-2147483648,\"LongAttr\":-9223372036854775808,\"UIntAttr\":4294967295,\"ULongAttr\":18446744073709551615,\"FloatAttr\":3.402823466e+38,\"DoubleAttr\":1.797693135e+308,\"IntAttr2\":0,\"FloatAttr2\":0.0,\"DoubleAttr2\":0.0,\"BoolTrueAttr\":true,\"BoolFalseAttr\":false,\"INFAttr\":null,\"NANAttr\":null}]}";
		TestEqual(TheTestName, cache.FlushCache(), ExpectedResult);
	}

	{
		FString TheTestName = "JsonEvent";
		FAnalyticsProviderETEventCache cache;
		cache.AddToCache(TheTestName, MakeAnalyticsEventAttributeArray
		(
		...
		FString ExpectedResult = "{\"Events\":[{\"EventName\":\"JsonEvent\",\"DateOffset\":\"+00:00:00.000\",\"NullAttr\":null,\"FragmentAttr\":{\"Key\":\"Value\",\"Key2\":\"Value2\"}}]}";
		TestEqual(TheTestName, cache.FlushCache(), ExpectedResult);
