...
void UGameplayTask::OnDestroy(bool bInOwnerFinished)
{
	if (ensureMsgf(IsValidChecked(this), TEXT("OnDestroy called on invalid gameplay task")))
	{
		...
		ensureMsgf(TaskState != EGameplayTaskState::Finished, TEXT("%s OnDestroy called, current state: %i, owner name: %s"), *GetName(), EnumToUnderlyingType(TaskState), *OwnerName);
	}
	TaskState = EGameplayTaskState::Finished;
...
void UGameplayTask::Resume()
{
	...
	UE_CLOGF(TasksComponent.IsValid() == false, LogGameplayTasks, Error
		, "%ls Resume called while TasksComponent is no longer valid", *GetName());
