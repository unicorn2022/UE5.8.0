...
void UGameplayTaskResource::UpdateAutoResourceID()
{
	...
	if (AutoResourceID == INDEX_NONE)
	{
		...
		{
			UE_LOGF(LogGameplayTasks, Error, "AutoResourceID out of bounds (probably too much GameplayTaskResource classes, consider manually assigning values if you can split all classes into non-overlapping sets");
		}
	}
}
