...
{
	UE_LOGF(WindowsVideoRecordingSystem, Verbose, "%ls", __FUNCTIONW__);
...
void FWindowsVideoRecordingSystem::EnableRecording(bool bEnableRecording)
{
	...
	{
		UE_LOGF(WindowsVideoRecordingSystem, Warning, "Can't enable recording because this App can't render.");
		return;
	}
...
bool FWindowsVideoRecordingSystem::NewRecording(const TCHAR* DestinationFileName, FVideoRecordingParameters InParameters)
{
	...
	if (!IsEnabled())
	{
		UE_LOGF(WindowsVideoRecordingSystem, Warning, "%ls : can't open a new recording. Recording is disabled", __FUNCTIONW__);
		return false;
	}

	if (RecordState != EVideoRecordingState::None)
	{
		UE_LOGF(WindowsVideoRecordingSystem, Warning, "FWindowsVideoRecordingSystem::NewRecording: can't open a new recording, one is already in progress.");
		return false;
	}
...
void FWindowsVideoRecordingSystem::StartRecording()
{
	...
	{
		UE_LOGF(WindowsVideoRecordingSystem, Warning, "%ls: can't start recording, invalid state", __FUNCTIONW__);
		return;
	}

	UE_LOGF(WindowsVideoRecordingSystem, Log, "%ls: starting a recording", __FUNCTIONW__);
...
void FWindowsVideoRecordingSystem::PauseRecording()
{
	...
	{
		UE_LOGF(WindowsVideoRecordingSystem, Warning, "%ls: can't pause recording, invalid state.", __FUNCTIONW__);
		return;
	}

	UE_LOGF(WindowsVideoRecordingSystem, Log, "%ls: pausing a recording", __FUNCTIONW__);
...
float FWindowsVideoRecordingSystem::GetCurrentRecordingSeconds() const
{
	...
	UE_LOGF(WindowsVideoRecordingSystem, Verbose, "%ls: reporting %f", __FUNCTIONW__, Ret);
	return Ret;
}
...
void FWindowsVideoRecordingSystem::FinalizeRecording(const bool bSaveRecording, const FText& Title, const FText& Comment, const bool bStopAutoContinue/* = true*/)
{
	...
	if (RecordState == EVideoRecordingState::None)
	{
		UE_LOGF(WindowsVideoRecordingSystem, Warning, "%ls: can't finalize recording, invalid state.", __FUNCTIONW__);
		return;
	}
	...
	if (bSaveRecording)
	{
		...
		if (!bRet)
		{
			UE_LOGF(WindowsVideoRecordingSystem, Warning, "%ls: can't finalize recording.", __FUNCTIONW__);
			return;
		}
