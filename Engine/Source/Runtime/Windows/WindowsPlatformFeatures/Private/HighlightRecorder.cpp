...
{
	Stop();
	UE_LOGF(HighlightRecorder, Log, "destroyed");
}
...
bool FHighlightRecorder::Start(double RingBufferDurationSecs)
{
	...
	{
		UE_LOGF(HighlightRecorder, Error, "cannot start recording, invalid state: %d", static_cast<int32>(State.Load()));
		return false;
	}
	...
	UE_LOGF(HighlightRecorder, Log, "recording started, ring buffer %.2f secs", RingBufferDurationSecs);
...
bool FHighlightRecorder::Pause(bool bPause)
{
	...
	if (State == EState::Stopped)
	{
		UE_LOGF(HighlightRecorder, Error, "cannot pause/resume recording, recording is stopped");
		return false;
	}
	...
	if (bPause && PauseTimestamp == 0)
	{
		...
		State = EState::Paused;
		UE_LOGF(HighlightRecorder, Log, "paused");
	}
	...
	else if (!bPause && (PauseTimestamp != 0 || PauseTimestamp == GetRecordingTime()))
	{
		...
		State = EState::Recording;
		UE_LOGF(HighlightRecorder, Log, "resumed after %.3f s", LastPausedDuration.GetTotalSeconds());
	}
...
void FHighlightRecorder::Stop()
{
	...
	UE_LOGF(HighlightRecorder, Log, "recording stopped");
}
...
bool FHighlightRecorder::SaveHighlight(const TCHAR* Filename, FDoneCallback InDoneCallback, double MaxDurationSecs)
{
	...
	if (State == EState::Stopped)
	{
		UE_LOGF(HighlightRecorder, Error, "cannot save clip when recording is stopped");
		return false;
	}

	if (bSaving)
	{
		UE_LOGF(HighlightRecorder, Error, "saving is busy with the previous clip");
		return false;
	}

	UE_LOGF(HighlightRecorder, Log, "start saving to %ls, max duration %.3f", Filename, MaxDurationSecs);
...
bool FHighlightRecorder::SaveHighlightInBackground(const FString& Filename, double MaxDurationSecs)
{
	...
	UE_LOGF(HighlightRecorder, Log, "saving to %ls %ls, took %.3f secs", *Filename, bRes ? TEXT("succeeded") : TEXT("failed"), PassedSecs);
...
bool FHighlightRecorder::SaveHighlightInBackgroundImpl(const FString& Filename, double MaxDurationSecs)
{
	...
	if (Samples.Num()==0)
	{
		UE_LOGF(HighlightRecorder, Error, "no samples to save to .mp4");
		return false;
	}
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UE_LOGF(HighlightRecorder, Verbose, "writting %d samples to .mp4, %.3f s, starting from %.3f s, index %d",
...
bool FHighlightRecorder::InitialiseMp4Writer(const FString& FullFilename, bool bHasAudio)
{
	...
	if (!PlatformFile.DirectoryExists(*DirName))
	{
		...
		if (!bRes)
		{
			UE_LOGF(HighlightRecorder, Error, "Can't create directory %ls", *DirName);
			return false;
		}
	}
	...
	if (bHasAudio)
	{
		...
		if (AudioConfig.Key == "")
		{
			UE_LOGF(HighlightRecorder, Error, "Could not get audio config");
			return false;
		}
	...
	if (VideoConfig.Key == "")
	{
		UE_LOGF(HighlightRecorder, Error, "Could not get video config");
		return false;
	}
...
// timestamp > than ("cur time" - "max duration to save")
{
	...
	{
		StartIndex = 0;
		UE_LOGF(HighlightRecorder, Error, "No samples to write to .mp4, max duration: %.3f", MaxDuration.GetTotalSeconds());
		return false;
	}
	...
	if (!bFound)
	{
		UE_LOGF(HighlightRecorder, Error, "No samples found to write to .mp4, max duration: %.3f", MaxDuration.GetTotalSeconds());
		return false;
	}
