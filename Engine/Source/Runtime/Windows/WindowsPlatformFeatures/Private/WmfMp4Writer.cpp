...
bool FWmfMp4Writer::Initialize(const TCHAR* Filename)
{
	...
	UE_LOGF(WMF, Verbose, "Initialised Mp4Writer for %ls", Filename);
	return true;
}
...
TOptional<DWORD> FWmfMp4Writer::CreateAudioStream(const FString& Codec, const AVEncoder::FAudioConfig& Config)
{
	...
	{
		UE_LOGF(WMF, Error, "FWmfMp4Writer doesn't support codec %ls", *Codec);
		return {};
	}
...
bool FWmfMp4Writer::Write(const AVEncoder::FMediaPacket& InSample, DWORD StreamIndex)
{
	...
	if(InSample.IsVideoKeyFrame() || InSample.Type == AVEncoder::EPacketType::Audio)
	{
		Sample.SetIsKeyFrame();
	}

	CHECK_HR(Writer->WriteSample(StreamIndex, const_cast<IMFSample*>(Sample.GetSample())));

	UE_LOGF(MP4, VeryVerbose, "stream #%d: time %.3f, duration %.3f%ls", StreamIndex, Sample.GetTime().GetTotalSeconds(), Sample.GetDuration().GetTotalSeconds(), InSample.IsVideoKeyFrame() ? TEXT(", key-frame") : TEXT(""));
...
bool FWmfMp4Writer::Finalize()
{
	...
	UE_LOGF(WMF, VeryVerbose, "Closed .mp4");
