...
void FD3D11DynamicRHI::FinalizeCreateBufferInternal(FD3D11Buffer* Buffer, TConstArrayView<uint8> InitialData)
{
	...
	TRefCountPtr<ID3D11Buffer> BufferResource;
	{
		...
		if (FAILED(hr))
		{
			...
			UE_LOGF(LogD3D11RHI, Error, "Failed to create buffer '%ls' with ByteWidth=%u, Usage=%d, BindFlags=0x%x, CPUAccessFlags=0x%x, MiscFlags=0x%x, StructureByteStride=%u, InitData=0x%p",
