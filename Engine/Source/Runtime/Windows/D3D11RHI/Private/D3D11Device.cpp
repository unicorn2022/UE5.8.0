...
{
	UE_LOGF(LogD3D11RHI, Log, "Shutdown");
...
void FD3D11DynamicRHI::SetupAfterDeviceCreation()
{
	...
	{
		UE_LOGF(LogD3D11RHI, Log, "Async texture creation enabled");
	}
	else
	{
		UE_LOGF(LogD3D11RHI, Log, "Async texture creation disabled: %ls",
	...
	}
	{
		...
		if (const HRESULT Result = Direct3DDevice->CheckFeatureSupport(D3D11_FEATURE_D3D11_OPTIONS, &Data, sizeof(Data)); SUCCEEDED(Result))
		{
			...
			{
				UE_LOGF(LogD3D11RHI, Log, "D3D11_MAP_WRITE_NO_OVERWRITE for dynamic buffer SRVs is supported");
			}
		}
	}

	{
		...
		{
			UE_LOGF(LogD3D11RHI, Log, "Stencil ref from pixel shader is supported");
		}
#else
...

	{
		...
		{
			UE_LOGF(LogD3D11RHI, Log, "Array index from any shader is supported");
		}
	}
...
{
	UE_LOGF(LogD3D11RHI, Error, "Crashed freeing up the D3D11 device.");
	if (GDynamicRHI)
	{
...
{
	UE_LOGF(LogD3D11RHI, Log, "CleanupD3DDevice");
...
static bool CanFormatBeDisplayed(const FD3D11DynamicRHI* InD3DRHI, EPixelFormat InPixelFormat)
{
	...
	if (FAILED(FormatSupportResult))
	{
		...
		UE_LOGF(LogD3D11RHI, Warning, "CheckFormatSupport(%ls) failed: 0x%08x", D3DFormatString, FormatSupportResult);
		return false;
	}
