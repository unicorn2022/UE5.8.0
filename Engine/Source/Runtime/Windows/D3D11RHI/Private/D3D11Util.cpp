...
FString GetD3D11ErrorString(HRESULT ErrorCode, ID3D11Device* Device)
{
	...
	if(ErrorCode == DXGI_ERROR_DEVICE_REMOVED && Device)
	{
		...
		ErrorCodeText += FString(" ") + GetD3D11DeviceHungErrorString(hResDeviceRemoved);
	}
...
static void TerminateOnDeviceRemoved(HRESULT D3DResult, ID3D11Device* Direct3DDevice)
{
	if (D3DResult == DXGI_ERROR_DEVICE_REMOVED)
	{
		...
		if (Direct3DDevice)
		{
			const HRESULT hRes = Direct3DDevice->GetDeviceRemovedReason();
			if (hRes != S_OK)
			{
				FGenericCrashContext::SetEngineData(TEXT("GPUCrash.D3DDeviceRemovedReason"), FString::FromInt(hRes));
			}
		}
		...
		if (Direct3DDevice)
		{
			...
			UE_LOGF(LogD3D11RHI, Fatal, "Unreal Engine is exiting due to D3D device being lost. (Error: 0x%X - '%ls')", hRes, Reason);
		}
		else
		{
			UE_LOGF(LogD3D11RHI, Fatal, "Unreal Engine is exiting due to D3D device being lost. D3D device was not available to determine DXGI cause.");
		}
...
void GetAndLogMemoryStats(const FD3D11Adapter& InAdapter, uint64& OutVRAMBudgetBytes, uint64& OutVRAMUsageBytes)
{
	...
	{
		UE_LOGF(LogD3D11RHI, Error, "\tLocal Budget:\t%7.2f MB", MemoryStats.BudgetLocal / (1024.0f * 1024));
		UE_LOGF(LogD3D11RHI, Error, "\tLocal Used:\t%7.2f MB", MemoryStats.UsedLocal / (1024.0f * 1024));
		UE_LOGF(LogD3D11RHI, Error, "\tSystem Budget:\t%7.2f MB", MemoryStats.BudgetSystem / (1024.0f * 1024));
		UE_LOGF(LogD3D11RHI, Error, "\tSystem Used:\t%7.2f MB", MemoryStats.UsedSystem / (1024.0f * 1024));
...
static void TerminateOnOutOfMemory(HRESULT D3DResult, bool bCreatingTextures)
{
	if (D3DResult == E_OUTOFMEMORY)
	{
		...
		{
			UE_LOGF(LogD3D11RHI, Fatal, "Out of video memory trying to allocate a rendering resource");
		}
		else
		{
...
void VerifyD3D11Result(HRESULT D3DResult,const ANSICHAR* Code,const ANSICHAR* Filename,uint32 Line, ID3D11Device* Device)
{
	...
	UE_LOGF(LogD3D11RHI, Error, "%ls failed with error %ls\n at %ls:%u", ANSI_TO_TCHAR(Code), *ErrorString, ANSI_TO_TCHAR(Filename), Line);

	TerminateOnDeviceRemoved(D3DResult, Device);
	TerminateOnOutOfMemory(D3DResult, false);

	UE_LOGF(LogD3D11RHI, Fatal,"%ls failed with error %ls\n at %ls:%u",ANSI_TO_TCHAR(Code), *ErrorString, ANSI_TO_TCHAR(Filename), Line);
}
...
void VerifyD3D11ShaderResult(FRHIShader* Shader, HRESULT D3DResult, const ANSICHAR* Code, const ANSICHAR* Filename, uint32 Line, ID3D11Device* Device)
{
	...
	UE_LOGF(LogD3D11RHI, Error, "%ls failed trying to create shader '%ls' with error %ls\n at %ls:%u", ANSI_TO_TCHAR(Code), Shader->GetShaderName(), *ErrorString, ANSI_TO_TCHAR(Filename), Line);
	TerminateOnDeviceRemoved(D3DResult, Device);
	TerminateOnOutOfMemory(D3DResult, false);

	UE_LOGF(LogD3D11RHI, Fatal, "%ls failed trying to create shader '%ls' with error %ls\n at %ls:%u", ANSI_TO_TCHAR(Code), Shader->GetShaderName(), *ErrorString, ANSI_TO_TCHAR(Filename), Line);
}
	...
	const void* SubResPtr, uint32 SubResPitch, uint32 SubResSlicePitch, ID3D11Device* Device, const TCHAR* DebugName)
{
	...
	UE_LOGF(LogD3D11RHI, Error,
		"%ls failed with error %ls\n at %ls:%u\n Size=%ix%ix%i PF=%d D3DFormat=%ls(0x%08X), NumMips=%i, Flags=%ls, Usage:0x%x, CPUFlags:0x%x, MiscFlags:0x%x, SampleCount:0x%x, SampleQuality:0x%x, SubresPtr:0x%p, SubresPitch:%i, SubresSlicePitch:%i, Name:'%ls', DebugInfo: %ls",
		ANSI_TO_TCHAR(Code),
		*ErrorString,
	...
	UE_LOGF(LogD3D11RHI, Fatal,
		"%ls failed with error %ls\n at %ls:%u\n Size=%ix%ix%i PF=%d Format=%ls(0x%08X), NumMips=%i, Flags=%ls, Usage:0x%x, CPUFlags:0x%x, MiscFlags:0x%x, SampleCount:0x%x, SampleQuality:0x%x, SubresPtr:0x%p, SubresPitch:%i, SubresSlicePitch:%i",
		ANSI_TO_TCHAR(Code),
		*ErrorString,
		ANSI_TO_TCHAR(Filename),
		Line,
	...
	ID3D11Device* Device)
{
	...
	UE_LOGF(LogD3D11RHI, Error,
		"%ls failed with error %ls\n at %ls:%u\n (Size=%ix%i Fullscreen=%d Format=%ls(0x%08X)) -> (Size=%ix%i Fullscreen=%d Format=%ls(0x%08X))",
		ANSI_TO_TCHAR(Code),
		*ErrorString,
		ANSI_TO_TCHAR(Filename),
		Line,
...
void VerifyD3D11CreateViewResult(HRESULT D3DResult, const ANSICHAR* Code, const ANSICHAR* Filename, uint32 Line, ID3D11Device* Device, const FString& ResourceName, const D3D11_UNORDERED_ACCESS_VIEW_DESC& Desc)
{
	...
	UE_LOGF(LogD3D11RHI, Error, "%ls failed with error %ls (Name='%ls', Format='%ls' (0x%08X), FormatSupport=0x%08X, FormatSupport2=0x%08X)\n at %ls:%u",
	...
	UE_LOGF(LogD3D11RHI, Fatal, "%ls failed with error %ls (Name='%ls', Format='%ls' (0x%08X), FormatSupport=0x%08X, FormatSupport2=0x%08X)\n at %ls:%u",
		ANSI_TO_TCHAR(Code), *ErrorString,
		*ResourceName, ViewFormat, Desc.Format, FormatSupport.OutFormatSupport, FormatSupport2.OutFormatSupport2,
		ANSI_TO_TCHAR(Filename), Line);
}
...
{
	const FString TextureName = Texture ? Texture->GetName().ToString() : FString("<Unknown>");
...
{
	FString BufferName = FString("<Unknown>");
