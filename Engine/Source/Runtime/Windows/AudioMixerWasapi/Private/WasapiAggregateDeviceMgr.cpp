...
namespace Audio
{
	...
	bool FWasapiAggregateDeviceMgr::InitializeHardware(const TArray<FWasapiRenderStreamParams>& InParams, const TFunction<void()>& InCallback)
	{
		...
		for (int32 DeviceIndex = 0; DeviceIndex < InParams.Num(); ++DeviceIndex)
		{
			...
			{
				UE_LOGF(LogAudioMixer, Error, "FWasapiAggregateDeviceMgr::InitializeHardware failed to create RenderStreamDevice: %d", DeviceIndex);
				return false;
			}

			if (!RenderStreamDevices[DeviceIndex]->InitializeHardware(InParams[DeviceIndex]))
			{
				UE_LOGF(LogAudioMixer, Error, "FWasapiAggregateDeviceMgr::InitializeHardware InitializeHardware failed for RenderStreamDevice: %d", DeviceIndex);
				return false;
			}
		}
	...
	bool FWasapiAggregateDeviceMgr::OpenAudioStream(const TArray<FWasapiRenderStreamParams>& InParams)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Error, "FWasapiAggregateDeviceMgr::OpenAudioStream no render devices found");
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixer, Error, "Unable to create RenderDeviceThread");
			return false;
		}

		if (EventHandles.Num() != NumRenderDevices)
		{
			UE_LOGF(LogAudioMixer, Error, "OpenAudioStream error creating EventHandles");
			return false;
		}
		...
		for (uint32 Index = 0; Index < NumRenderDevices; ++Index)
		{
			...
			{
				UE_LOGF(LogAudioMixer, Error, "OpenAudioStream failed to open main audio device");
				return false;
			}
		}
	...
	bool FWasapiAggregateDeviceMgr::StartAudioStream()
	{
		if (RenderStreamDevices.IsEmpty())
		{
			UE_LOGF(LogAudioMixer, Error, "FWasapiAggregateDeviceMgr::StartAudioStream no devices available to start");
			return false;
		}
		...
		for (TUniquePtr<FAudioMixerWasapiRenderStream>& Device : RenderStreamDevices)
		{
			if (Device.IsValid())
			{
				...
				{
					UE_LOGF(LogAudioMixer, Error, "FWasapiAggregateDeviceMgr::StartAudioStream unable to start render device");
					return false;
				}
			}
		}
		...
		{
			UE_LOGF(LogAudioMixer, Error, "FWasapiAggregateDeviceMgr::StartAudioStream failed to start device thread");
			return false;
		}
	...
	bool FWasapiAggregateDeviceMgr::IsDeviceLost() const
	{
		// Any stream observing device loss invalidates the aggregate.
		for (const TUniquePtr<FAudioMixerWasapiRenderStream>& Device : RenderStreamDevices)
		{
			if (Device.IsValid() && Device->IsDeviceLost())
			{
				return true;
			}
		}
	...
	void FWasapiAggregateDeviceMgr::SetDeviceLost()
	{
		for (TUniquePtr<FAudioMixerWasapiRenderStream>& Device : RenderStreamDevices)
		{
			if (Device.IsValid())
	...
	void FWasapiAggregateDeviceMgr::ClearDeviceLost()
	{
		for (TUniquePtr<FAudioMixerWasapiRenderStream>& Device : RenderStreamDevices)
		{
			if (Device.IsValid())
			{
				Device->ClearDeviceLost();
			}
		}
	}
}
