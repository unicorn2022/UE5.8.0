...
namespace Audio
{
	...
	bool FAudioMixerWasapiRenderStream::InitializeHardware(const FWasapiRenderStreamParams& InParams)
	{
		...
		if (!MMDevice)
		{
			UE_LOGF(LogAudioMixerWasapi, Error, "InitializeHardware null MMDevice");
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioMixerWasapi, Error, "OpenAudioStream failed IMMDevice::Activate %ls", *AudioClientErrorToFString(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			...
			UE_LOGF(LogAudioMixerWasapi, Warning, "InitializeHardware failed SetClientProperties %ls", *AudioClientErrorToFString(Result));
		}
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Error, "OpenAudioStream failed IAudioClient::GetMixFormat MixFormat: 0x%llx Result: %ls", (uint64)MixFormat, *AudioClientErrorToFString(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioMixerWasapi, Error, "OpenAudioStream failed IAudioClient::GetDevicePeriod %ls", *AudioClientErrorToFString(Result));
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Error, "OpenAudioStream failed DefaultDevicePeriod = %d", DefaultDevicePeriod);
			return false;
		}
		...
		UE_LOGF(LogAudioMixerWasapi, Log, "FAudioMixerWasapiRenderStream::InitializeHardware DefaultDevicePeriod: %d requesting buffer size: %d", DefaultDevicePeriod, BufferFramesToRequest);
		...
		if (InParams.SampleRate != InParams.HardwareDeviceInfo.SampleRate)
		{
			...
			UE_LOGF(LogAudioMixerWasapi, Warning, "Sample rate mismatch. Engine sample rate: %d Device sample rate: %d", InParams.SampleRate, InParams.HardwareDeviceInfo.SampleRate);
			UE_LOGF(LogAudioMixerWasapi, Warning, "Device level sample rate conversion will be used.");
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioMixerWasapi, Error, "OpenAudioStream failed IAudioClient::Initialize %ls", *AudioClientErrorToFString(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioMixerWasapi, Error, "OpenAudioStream failed IAudioClient::GetBufferSize %ls", *AudioClientErrorToFString(Result));
			return false;
		}
		...
		UE_LOGF(LogAudioMixerWasapi, Display, "FAudioMixerWasapiRenderStream::InitializeHardware audio mixer requested num channels: %d, Wasapi render stream hardware supports num channels: %d", AudioMixerNumChannels, RenderStreamHardwareChannels);
		...
		UE_LOGF(LogAudioMixerWasapi, Display, "FAudioMixerWasapiRenderStream::InitializeHardware succeeded with sample rate: %d, buffer period: %d", InParams.SampleRate, InParams.NumFrames);
	...
	bool FAudioMixerWasapiRenderStream::TeardownHardware()
	{
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::TeardownHardware failed...not initialized. ");
			return false;
		}
		...
		UE_LOGF(LogAudioMixerWasapi, Verbose, "FAudioMixerWasapiRenderStream::TeardownHardware succeeded");
	...
	{
		UE_LOGF(LogAudioMixerWasapi, Display, "FAudioMixerWasapiRenderStream: Initializing mono downmix buffer");
	...
	bool FAudioMixerWasapiRenderStream::OpenAudioStream(const FWasapiRenderStreamParams& InParams, HANDLE InEventHandle)
	{
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::OpenAudioStream stream not in closed sate. CurrStreamState: %d", CurrStreamState);
			return false;
		}
		...
		if (InParams.HardwareDeviceInfo.DeviceId != RenderStreamParams.HardwareDeviceInfo.DeviceId)
		{
			...
			{
				UE_LOGF(LogAudioMixerWasapi, Warning, "OpenAudioStream failed InitAudioClient");
				return false;
			}
		}
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "OpenAudioStream null EventHandle");
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "OpenAudioStream failed IAudioClient::SetEventHandle %ls", *AudioClientErrorToFString(Result));
			return false;
		}
		...
		if (FAILED(Result))
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "OpenAudioStream failed IAudioClient::GetService IAudioRenderClient %ls", *AudioClientErrorToFString(Result));
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::OpenAudioStream StreamState modified during open. OriginalState: %d CurrStreamState: %d", OriginalState, CurrStreamState);
			return false;
		}
		
		UE_LOGF(LogAudioMixerWasapi, Verbose, "FAudioMixerWasapiRenderStream::OpenAudioStream succeeded with SampeRate: %d, NumFrames: %d", InParams.SampleRate, InParams.NumFrames);
	...
	bool FAudioMixerWasapiRenderStream::CloseAudioStream()
	{
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::CloseAudioStream stream in unexpected state. bIsInitialized: %d CurrStreamState: %d",
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::CloseAudioStream stream appears to be running. StopAudioStream() must be called prior to closing.");
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::CloseAudioStream StreamState modified during close. OriginalState: %d CurrStreamState: %d", OriginalState, CurrStreamState);
			return false;
		}
	...
	bool FAudioMixerWasapiRenderStream::StartAudioStream()
	{
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::StartAudioStream stream not in open state. bIsInitialized: %d, CurrStreamState: %d",
				bIsInitialized.load(std::memory_order_relaxed), CurrStreamState);
			return false;
		}
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "StartAudioStream failed invalid audio client");
			return false;
		}

		AudioClient->Start();
		UE_LOGF(LogAudioMixerWasapi, Verbose, "FAudioMixerWasapiRenderStream::StartAudioStream stream started");
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::StartAudioStream StreamState modified during start. OriginalState: %d CurrStreamState: %d", OriginalState, CurrStreamState);
			return false;
		}
	...
	bool FAudioMixerWasapiRenderStream::StopAudioStream()
	{
		if (!bIsInitialized.load(std::memory_order_acquire))
		{
			UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::StopAudioStream() not initialized");
			return false;
		}
		...
		if (CurrStreamState != EAudioOutputStreamState::Stopped && CurrStreamState != EAudioOutputStreamState::Closed)
		{
			...
			{
				UE_LOGF(LogAudioMixerWasapi, Warning, "FAudioMixerWasapiRenderStream::StopAudioStream StreamState modified during stop. OriginalState: %d CurrStreamState: %d", OriginalState, CurrStreamState);
				return false;
			}
		}
		...
		{
			UE_LOGF(LogAudioMixerWasapi, Display, "FAudioMixerWasapiRenderStream::StopAudioStream render stream reported %d callback buffer errors (can be normal if preceded by device swap).", CallbackBufferErrors);
