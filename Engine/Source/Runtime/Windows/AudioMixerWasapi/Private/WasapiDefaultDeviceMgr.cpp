...
namespace Audio
{
	...
	{ 
		...
		{
			UE_LOGF(LogAudioMixer, Error, "OpenAudioStream null EventHandle");
			return false;
		}
	...
	{ 
		...
		{
			UE_LOGF(LogAudioMixer, Error, "FWasapiDefaultDeviceMgr::StartAudioStream failed to start main audio device");
			return false;
		}

		if (!RenderDeviceThread->Start())
		{
			UE_LOGF(LogAudioMixer, Error, "FWasapiDefaultDeviceMgr::StartAudioStream failed to start device thread");
			return false;
		}
	...
	bool FWasapiDefaultDeviceMgr::IsDeviceLost() const
	{
		return MainRenderStreamDevice.IsValid() && MainRenderStreamDevice->IsDeviceLost();
	}

	void FWasapiDefaultDeviceMgr::SetDeviceLost()
	{
		if (MainRenderStreamDevice.IsValid())
		{
			MainRenderStreamDevice->SetDeviceLost();
		}
	}

	void FWasapiDefaultDeviceMgr::ClearDeviceLost()
	{
		if (MainRenderStreamDevice.IsValid())
		{
			MainRenderStreamDevice->ClearDeviceLost();
		}
	}
}
