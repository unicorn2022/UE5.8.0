...
#define XAUDIO2_LOG_AND_HANDLE_ON_FAIL(FunctionName, Result, OnError)\
	if (FAILED(Result))\
	{\
		UE_LOGF(LogAudioMixer, Error, "XAudio2 Error: %ls -> 0x%X: '%ls', called in '%s' (%ls:%d)",\
...
namespace Audio
{
	...
	bool FMixerPlatformXAudio2::PreDeviceSwap()
	{
		...
		if (DeviceSwapContext.IsValid())
		{
			...
			UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::PreDeviceSwap - Starting swap to [%ls]", DeviceSwapContext->RequestedDeviceId.IsEmpty() ? TEXT("[System Default]") : *DeviceSwapContext->RequestedDeviceId);

			return true;
		}
		else
		{
			UE_LOGF(LogAudioMixer, Warning, "FMixerPlatformXAudio2::PreDeviceSwap - null device swap context");
			return false;
		}
	...
	{
		UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::EnqueueAsyncDeviceSwap - enqueuing async device swap");
		FScopeLock Lock(&DeviceSwapCriticalSection);
		...
		if (DeviceSwapResult && DeviceSwapResult->IsNewDeviceReady())
		{
			...
			if (XAudio2System && OutputAudioStreamSourceVoice && OutputAudioStreamMasteringVoice)
			{
				...
				if (SUCCEEDED(Result))
				{
					...
					UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::PostDeviceSwap - successful Swap new Device is (NumChannels=%u, SampleRate=%u, DeviceID=%ls, Name=%ls), Reason=%ls, InstanceID=%d, DurationMS=%.2f",
	...
	void FMixerPlatformXAudio2::ResumePlaybackOnNewDevice()
	{
		if (DeviceSwapContext.IsValid())
		{
			...
			UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::ResumePlaybackOnNewDevice - device swap requested during previous device swap...not resuming audio");
		}
		else
		{
	...
	TUniquePtr<FDeviceSwapResult> FMixerPlatformXAudio2::PerformDeviceSwap(TUniquePtr<FXAudio2DeviceSwapContext>&& InDeviceContext)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Error, "FMixerPlatformXAudio2::PerformDeviceSwap - failed due to invalid DeviceSwapContext");
			return {};
		}

		UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::PerformDeviceSwap - AsyncTask Start. Because=%ls", *InDeviceContext->DeviceSwapReason);
	...
	bool FMixerPlatformXAudio2::InitializeHardware()
	{
		...
		{
			UE_LOGF(LogAudioMixer, Error, "Failed to register for callbacks.");
		}
	...
	static bool GetMMDeviceInfo(IMMDevice* MMDevice, FAudioPlatformDeviceInfo& OutInfo)
	{
		...
		if (WaveFormatEx->wFormatTag == WAVE_FORMAT_EXTENSIBLE)
		{
			...
			{
				UE_LOGF(LogAudioMixer, Warning, "Did not find the channel type flags for audio device '%ls'. Reverting to a default channel ordering.", *OutInfo.Name);
	...
	bool FMixerPlatformXAudio2::GetOutputDeviceInfo(const uint32 InDeviceIndex, FAudioPlatformDeviceInfo& OutInfo)
	{
		...
		if (DeviceCount == 0)
		{
			UE_LOGF(LogAudioMixer, Warning, "No available audio device");
			goto Cleanup;
		}
		...
		{
			UE_LOGF(LogAudioMixer, Error, "Requested device index (%d) is larger than the number of devices available (%d)", InDeviceIndex, DeviceCount);
			goto Cleanup;
		}
		else
		{
	...
	{
		UE_LOGF(LogAudioMixer, Log, "FMixerPlatformXAudio2::StartAudioStream() called. InstanceID=%d", InstanceID);
	...
	bool FMixerPlatformXAudio2::StopAudioStream()
	{
		...
		UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::StopAudioStream() called. InstanceID=%d, StreamState=%d",
	...
	void FMixerPlatformXAudio2::SubmitBuffer(const uint8* Buffer)
	{
		...
		if (OutputAudioStreamSourceVoice)
		{
			...
			{
				UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::SubmitBuffer() called for the first time. InstanceID=%d", InstanceID);
	...
	bool FMixerPlatformXAudio2::InitializeDeviceSwapContext(const FString& InRequestedDeviceID, const TCHAR* InReason)
	{
		...
		if (TOptional<FAudioPlatformDeviceInfo> TempDeviceInfo = GetDeviceInfoCache()->FindActiveOutputDevice(NewDeviceName))
		{
			if (TempDeviceInfo.IsSet())
			{
				...
				{
					UE_LOGF(LogAudioMixer, Warning, "Ignoring attempt to switch to device with unsupported params: Channels=%u, SampleRate=%u, Id=%ls, Name=%ls",
	...
	bool FMixerPlatformXAudio2::InitDeviceSwapContextInternal(const FString& InRequestedDeviceID, const TCHAR* InReason, const TOptional<FAudioPlatformDeviceInfo>& InDeviceInfo)
	{
		...
		if (DeviceSwapContext.IsValid())
		{
			UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2::InitDeviceSwapContextInternal DeviceSwapContext in-flight, ignoring");
			return false;
		}
		...
		if (!DeviceSwapContext.IsValid())
		{
			UE_LOGF(LogAudioMixer, Warning, "FMixerPlatformXAudio2::InitDeviceSwapContextInternal failed to create DeviceSwapContext");
			return false;
		}
	...
	IXAudio2MasteringVoice* FMixerPlatformXAudio2::CreateMasteringVoice(IXAudio2& InXAudio2System, const FAudioPlatformDeviceInfo& NewDevice, bool bUseDefaultDevice)
	{
		...
		if (FAILED(Result))
		{
			...
			UE_LOGF(LogAudioMixer, Error, "CreateMasteringVoice failed with result 0x%X: %ls (line: %d) with Args (NumChannels=%u, SampleRate=%u, DeviceID=%ls, Name=%ls)",
	...
	void FMixerPlatformXAudio2::OnCriticalError(HRESULT Error)
	{
		...
		UE_LOGF(LogAudioMixer, Warning, "FMixerPlatformXAudio2::OnCriticalError: 0x%X: %ls", Error, *Audio::ToErrorFString(Error));
	}
