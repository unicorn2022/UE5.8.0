...
{	
	...
	{		
		...
		{
			UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2: Changing default audio render device to new device: Role=%ls, DeviceName=%ls, InstanceID=%d", 
	...
	void FMixerPlatformXAudio2::OnDeviceAdded(const FString& DeviceId, bool bIsRenderDevice)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2: Original audio device re-added. Moving audio back to original audio device: DeviceName=%ls, bRenderDevice=%d, InstanceID=%d", 
	...
	void FMixerPlatformXAudio2::OnDeviceRemoved(const FString& DeviceId, bool bIsRenderDevice)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2: Audio device removed [%ls], falling back to other windows default device. bIsRenderDevice=%d, InstanceID=%d", 
	...
	void FMixerPlatformXAudio2::OnDeviceStateChanged(const FString& DeviceId, const EAudioDeviceState InState, bool bIsRenderDevice)
	{
		...
		{
			UE_LOGF(LogAudioMixer, Display, "FMixerPlatformXAudio2: Audio device not available [%ls], falling back to other windows default device. InState=%d, bIsRenderDevice=%d, InstanceID=%d", 
