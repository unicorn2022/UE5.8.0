...
void UTimecodeRegressionProvider::ReinitRegressionBuffer()
{
	...
	UE_CLOGF(Timestep && !ClockedTimeStep, LogTimeManagement, Error,
		"Current timestep %ls must implement the IClockedTimeStep in order to be used with UTimecodeRegressionProvider",
		*Timestep->GetName()
		);
