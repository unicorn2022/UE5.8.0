	...
	)
{
	...
	{
		OutSlope = 0.0;
		// The slope is undefined. The set of all possible linear curves is infinite. However, all of them must pass through the same point. 
		// Therefore, we'll choose the linear function parallel to the x-axis.
		OutOffset = InArgs.Num == 0 ? 0 : InArgs.SumY / InArgs.Num;
		return false;
	}
