...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFrameNumberInterfaceRelativeFrameMathTest, "System.Core.Time.FrameNumberInterface.RelativeFrameMathPreservesSubframes", EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::SmokeFilter | EAutomationTestFlags::HighPriority)
bool FFrameNumberInterfaceRelativeFrameMathTest::RunTest(const FString& Parameters)
{
	constexpr EFrameNumberDisplayFormats DisplayFormat = EFrameNumberDisplayFormats::Frames;
	constexpr uint8 ZeroPadFrames = 0;
	const FFrameRate TickResolution(24000, 1);
	const FFrameRate DisplayRate(24, 1);

	const FFrameNumber ExistingTickFrame = FFrameRate::TransformTime(FFrameTime::FromDecimal(1.5), DisplayRate, TickResolution).RoundToFrame();
	const FFrameNumber ExpectedTickFrame = FFrameRate::TransformTime(FFrameTime::FromDecimal(2.5), DisplayRate, TickResolution).RoundToFrame();

	FFrameNumberInterface Interface(DisplayFormat, ZeroPadFrames, TickResolution, DisplayRate);
	const TOptional<double> Result = Interface.FromString(TEXT("+=1"), ExistingTickFrame.Value);

	TestTrue(TEXT("Relative frame math should parse"), Result.IsSet());
	if (Result.IsSet())
	{
		TestEqual(TEXT("Relative frame math should preserve the display-rate subframe"), static_cast<int32>(Result.GetValue()), ExpectedTickFrame.Value);
	}
