...
{
	if (IsInGameThread() && FModuleManager::Get().IsModuleLoaded(TEXT("Sockets")))
	{
...
bool ISocketSubsystem::GetLocalAdapterAddresses(TArray<TSharedPtr<FInternetAddr>>& OutAddresses)
{
	...
	UE_LOGF(LogSockets, Warning, "Falling back to generic GetLocalAdapterAddresses implementation. Consider implementing a Platform specific version.");
...
bool ISocketSubsystem::GetMultihomeAddress(TSharedRef<FInternetAddr>& Addr)
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("MULTIHOME="), Home, UE_ARRAY_COUNT(Home)))
	{
		...
		{
			UE_LOGF(LogSockets, Log, "Invalid multihome IP address %ls", Home);
			return false;
		}
		else
		{
...
{
	UE_LOGF(LogSockets, Warning, "RecvMulti is not supported by current socket subsystem.");
...
{
	UE_LOGF(LogSockets, Warning, "TranslatePacketTimestamp is not supported by current socket subsystem.");
...
void ISocketSubsystem::QuerySocketStateMany(TArrayView<FSocket*> Sockets, TArray<ESocketStateFlags>& InOutSocketStates, FTimespan WaitTime) const
{
	check(!IsSocketWaitSupported());
	
	InOutSocketStates.SetNum(Sockets.Num());

	// clear the returned states
	for (int i = 0; i < InOutSocketStates.Num(); i++)
	{
		InOutSocketStates[i] = (ESocketStateFlags)0;
	}
}
...
{
	UE_LOGF(LogSockets, Log, "Got %d GAI Results for hostname %ls. Error Code: %ls [%d]", GAIResult.Results.Num(), *GAIResult.QueryHostName,
		SocketSub->GetSocketError(GAIResult.ReturnCode), GAIResult.ReturnCode);

	for (int i = 0; i < GAIResult.Results.Num(); ++i)
	{
		UE_LOGF(LogSockets, Log, "Result #%d Address: %ls Type: %ls", i, *GAIResult.Results[i].Address->ToString(false),
...
static void RunGAIQuery(const FString& HostStr)
{
	if (HostStr.IsEmpty())
	{
		UE_LOGF(LogSockets, Warning, "SOCKETSUB GAI requires an input string to test with.");
		return;
	}
	...
	if (SocketSub)
	{
		...
		{
			UE_LOGF(LogSockets, Warning, "Did not get results!");
		}
		return;
	}

	UE_LOGF(LogSockets, Warning, "Failed to get socket subsystem!");
}
...
static void RunAsyncGAIQuery(const FString& HostStr)
{
	if (HostStr.IsEmpty())
	{
		UE_LOGF(LogSockets, Warning, "SOCKETSUB ASYNCGAI requires an input string to test with.");
		return;
	}
	...
	if (SocketSub)
	{
		...
		{
			UE_LOGF(LogSockets, Log, "Async GAI Request returned after %f seconds, started at %f", FPlatformTime::Seconds() - AsyncRequestStartTime, AsyncRequestStartTime);
...
static void RunAddressSerialize(const FString& InputStr)
{
	if (InputStr.IsEmpty())
	{
		UE_LOGF(LogSockets, Warning, "SOCKETSUB Serialize requires an ip address to test with.");
		return;
	}
	...
	if (SocketSub)
	{
		...
		if (NewAddr.IsValid())
		{
			UE_LOGF(LogSockets, Log, "Result Address: %ls Type: %ls", *NewAddr->ToString(false), *NewAddr->GetProtocolType().ToString());
		}
		else
		{
			UE_LOGF(LogSockets, Warning, "Did not get results!");
	...
	if (FParse::Command(&Cmd, TEXT("SOCKETSUB")))
	{
		...
		else
		{
			...
			if (FParse::Command(&Cmd, TEXT("GETBINDADDRESSES")))
			{
				...
				UE_LOGF(LogSockets, Log, "Got Binding Addresses:");
				for (const auto& BindAddress : BindAddresses)
				{
					UE_LOGF(LogSockets, Log, "# Bind Address: %ls", *BindAddress->ToString(false));
				}
			}
			else if (FParse::Command(&Cmd, TEXT("GETADAPTERS")))
			{
				...
				{
					UE_LOGF(LogSockets, Log, "Got Local Adapter Addresses:");
					for (const auto& AdapterAddress : AdapterAddresses)
					{
						UE_LOGF(LogSockets, Log, "# Adapter Address: %ls", *AdapterAddress->ToString(false));
					}
				}
				else
				{
					UE_LOGF(LogSockets, Warning, "Could not get the local adapter addresses!");
				}
			}
			else if (FParse::Command(&Cmd, TEXT("GETLOCALHOST")))
			{
				...
				UE_LOGF(LogSockets, Log, "LocalHostAddr is %ls (this function auto adds multihome) Bind all: %d", *LocalHostAddr->ToString(false), bCanBindAll);
			}
			else if (FParse::Command(&Cmd, TEXT("GETBINDADDR")))
			{
				TSharedRef<FInternetAddr> BindAddr = SocketSub->GetLocalBindAddr(Ar);
				UE_LOGF(LogSockets, Log, "The Local binding addr is %ls", *BindAddr->ToString(false));
			}
		}
