...
FSocket* FSocketSubsystemBSD::CreateSocket(const FName& SocketType, const FString& SocketDescription, const FName& ProtocolType)
{
	...
	if (ProtocolType != FNetworkProtocolTypes::IPv4 && ProtocolType != FNetworkProtocolTypes::IPv6)
	{
		...
		{
			UE_LOGF(LogSockets, Warning, "Provided socket protocol type is unsupported! Returning null socket");
			return nullptr;
		}
	}
	...
	if (!NewSocket)
	{
		UE_LOGF(LogSockets, Warning, "Failed to create socket %ls [%ls]", *SocketType.ToString(), *SocketDescription);
	}
	...
	EAddressInfoFlags QueryFlags, const FName ProtocolTypeName, ESocketType SocketType)
{
	...
	{
		UE_LOGF(LogSockets, Warning, "GetAddressInfo was passed with both a null host and service, returning empty result");
	...
	UE_LOGF(LogSockets, Verbose, "Executed getaddrinfo with HostName: %ls ServiceName: %ls Return: %d", HostName ? HostName : TEXT("null"), ServiceName ? ServiceName : TEXT("null"), ErrorCode);
	if (AddrQueryResult.ReturnCode == SE_NO_ERROR)
	{
		...
		if (AddrInfo != nullptr && AddrInfo->ai_canonname != nullptr)
		{
			...
			UE_LOGF(LogSockets, Verbose, "getaddrinfo ai_canonname returns: %ls", *AddrQueryResult.CanonicalNameResult);
			...
			if (AddrInfo->ai_family == AF_INET || (AddrInfo->ai_family == AF_INET6 && bCanUseIPv6))
			{
				...
				if (AddrData != nullptr)
				{
					...
					if (AddrQueryResult.Results.Add(NewAddressData) != INDEX_NONE)
					{
						...
						UE_LOGF(LogSockets, Verbose, "# Family: %ls Address: %ls And scope %d", ((AddrInfo->ai_family == AF_INET) ? TEXT("IPv4") : TEXT("IPv6")), *(NewAddress->ToString(true)), ((AddrInfo->ai_family == AF_INET) ? -1 : IPv6AddrData->sin6_scope_id));
						...
						if (bTryGetFQDN && (AddrData->ss_family == AF_INET || AddrData->ss_family == AF_INET6))
						{
								...
								NI_NAMEREQD) == 0)
							{
								...
								if (FAnsiStringView(HostBuffer).FindChar(TEXT('.'), DotPosition))
								{
									...
									UE_LOGF(LogSockets, Verbose, "Using getnameinfo hostname '%ls'", *AddrQueryResult.CanonicalNameResult);
								}
							}
						}
#endif
	...
	{
		UE_LOGF(LogSockets, Warning, "GetAddressInfo failed to resolve host with error %ls [%d]", GetSocketError(AddrQueryResult.ReturnCode), ErrorCode);
	}
#else
	UE_LOGF(LogSockets, Error, "Platform has no getaddrinfo(), but did not override FSocketSubsystem::GetAddressInfo()");
#endif
	return AddrQueryResult;
}
...
TSharedPtr<FInternetAddr> FSocketSubsystemBSD::GetAddressFromString(const FString& InAddress)
{
	...
	UE_LOGF(LogSockets, Warning, "Could not serialize %ls, got error code %ls [%d]", *InAddress, GetSocketError(LastError), LastError);
	return nullptr;
}
...
bool FSocketSubsystemBSD::GetHostName(FString& HostName)
{
	...
	return bRead;
#else
	UE_LOGF(LogSockets, Error, "Platform has no gethostname(), but did not override FSocketSubsystem::GetHostName()");
	return false;
#endif
}
...
ESocketErrors FSocketSubsystemBSD::TranslateErrorCode(int32 Code)
{
	...
	UE_LOGF(LogSockets, Warning, "Unhandled socket error! Error Code: %d. Returning SE_EINVAL!", Code);
	return SE_EINVAL;
}
...
ESocketErrors FSocketSubsystemBSD::TranslateGAIErrorCode(int32 Code) const
{
	...
	default:
		UE_LOGF(LogSockets, Warning, "Unhandled getaddrinfo() socket error! Code: %d", Code);
		return SE_EINVAL;
	}
...
TSharedRef<FInternetAddr> FSocketSubsystemBSD::GetLocalHostAddr(FOutputDevice& Out, bool& bCanBindAll)
{
	...
	UE_LOGF(LogSockets, VeryVerbose, "GetLocalHostAddr: %ls", *HostAddr->ToString(true));
	...
	if (bIsValid)
	{
		...
		if (TestSocket.IsValid())
		{
			if (TestSocket->Connect(*ConnectAddr))
			{
				...
				UE_LOGF(LogSockets, VeryVerbose, "GetLocalHostAddrViaConnect: %ls", *HostAddr->ToString(true));
			}
...
void FSocketSubsystemBSD::QuerySocketStateMany(TArrayView<FSocket*> Sockets, TArray<ESocketStateFlags>& InOutSocketStates, FTimespan WaitTime) const
{
	check(IsSocketWaitSupported());
	check(InOutSocketStates.Num() == Sockets.Num());
	...
	TArray<struct pollfd> FDSet;
	FDSet.SetNum(Sockets.Num());

	for (int32 SocketIndex = 0; SocketIndex < Sockets.Num(); SocketIndex++)
	{
		uint8 Request = (uint8)InOutSocketStates[SocketIndex];
		FSocket* Socket = Sockets[SocketIndex];

		struct pollfd FD;
		FD.fd = static_cast<FSocketBSD*>(Socket)->GetNativeSocket();
		FD.events = 0;
		FD.revents = 0;

		if (Request & (uint8)ESocketStateFlags::Readable)
		{
			FD.events |= POLLIN;
		}

		if (Request & (uint8)ESocketStateFlags::Writable)
		{
			FD.events |= POLLOUT;
		}

		// POLLERR is output-only and always reported in revents; no need to set it in events.

		FDSet[SocketIndex] = FD;
	}

	int PollResult = ::poll(&FDSet[0], FDSet.Num(), WaitTime.GetTicks() >= 0 ? (int)WaitTime.GetTotalMilliseconds() : -1);
	...
	if (PollResult >= 0)
	{
		for (int32 SocketIndex = 0; SocketIndex < Sockets.Num(); SocketIndex++)
		{
			...
			struct pollfd FD = FDSet[SocketIndex];

			if ((Request & (uint8)ESocketStateFlags::Readable) && (FD.revents & (POLLIN | POLLHUP)))
			{
				Result |= (uint8)ESocketStateFlags::Readable;
			}

			if ((Request & (uint8)ESocketStateFlags::Writable) && (FD.revents & POLLOUT))
			{
				Result |= (uint8)ESocketStateFlags::Writable;
			}

			if ((Request & (uint8)ESocketStateFlags::Error) && (FD.revents & (POLLERR | POLLNVAL)))
			{
				Result |= (uint8)ESocketStateFlags::Error;
			}

			InOutSocketStates[SocketIndex] = (ESocketStateFlags)Result;
		}
	}
	else
	{
		for (int32 SocketIndex = 0; SocketIndex < InOutSocketStates.Num(); SocketIndex++)
		{
			InOutSocketStates[SocketIndex] = (ESocketStateFlags)0;
		}
	}

#elif PLATFORM_HAS_BSD_SOCKET_FEATURE_SELECT

	check(Sockets.Num() <= FD_SETSIZE);	
	...
	for (int32 SocketIndex = 0; SocketIndex < Sockets.Num(); SocketIndex++)
	{
		...
		FSocket* Socket = Sockets[SocketIndex];
		SOCKET PlatformSocket = static_cast<FSocketBSD*>(Socket)->GetNativeSocket();

		if (Request & (uint8)ESocketStateFlags::Readable)
		{
			FD_SET(PlatformSocket, &SocketSetRead);
		}

		if (Request & (uint8)ESocketStateFlags::Writable)
		{
			FD_SET(PlatformSocket, &SocketSetWrite);
		}

		if (Request & (uint8)ESocketStateFlags::Error)
		{
			FD_SET(PlatformSocket, &SocketSetError);
		}

		MaxFd = FMath::Max(MaxFd, (int)PlatformSocket);
	}

	timeval* TimePointer = WaitTime.GetTicks() >= 0 ? &Time : nullptr;

	// Check the status of the state
	int32 SelectStatus = select(MaxFd+1, &SocketSetRead, &SocketSetWrite, &SocketSetError, TimePointer);
	...
	if (SelectStatus >= 0)
	{
		for (int32 SocketIndex = 0; SocketIndex < Sockets.Num(); SocketIndex++)
		{
			...
			if (FD_ISSET(PlatformSocket, &SocketSetRead))
			{
				Result |= (uint8)ESocketStateFlags::Readable;
			}

			if (FD_ISSET(PlatformSocket, &SocketSetWrite))
			{
				Result |= (uint8)ESocketStateFlags::Writable;
			}

			if (FD_ISSET(PlatformSocket, &SocketSetError))
			{
				Result |= (uint8)ESocketStateFlags::Error;
			}
...
#else
	UE_LOGF(LogSockets, Fatal, "This platform doesn't support poll() or select(), but FSocketSubsystemBSD::QuerySocketStateMany was not implemented");
#endif
}
