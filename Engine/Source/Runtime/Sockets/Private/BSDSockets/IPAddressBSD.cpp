...
void FInternetAddrBSD::SetIp(const TCHAR* InAddr, bool& bIsValid)
{
	...
	{
		UE_LOGF(LogSockets, Verbose, "SocketSubsystem pointer is null, cannot resolve the stringed address");
	}
}
...
void FInternetAddrBSD::SetAnyAddress()
{
	...
	{
		UE_LOGF(LogSockets, Warning, "Could not determine the default protocol to use in SetAnyAddress!");
	}
}
...
void FInternetAddrBSD::SetBroadcastAddress()
{
	...
	{
		UE_LOGF(LogSockets, Warning, "Could not determine the default protocol to use in SetBroadcastAddress!");
	}
}
...
void FInternetAddrBSD::SetLoopbackAddress()
{
	...
	{
		UE_LOGF(LogSockets, Warning, "Could not determine the default protocol to use in SetLoopbackAddress!");
	}
}
...
#else
		UE_LOGF(LogSockets, Error, "Platform has no getnameinfo(), but did not override FInternetAddrBSD::ToString()");
#endif
		return ReturnVal;
	});
}
