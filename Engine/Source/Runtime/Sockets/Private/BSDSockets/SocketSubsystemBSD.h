...
public:
	...
	/** Polls multiple sockets for readability, writability, and error states using poll() or select().
	 *  InOutSocketStates is used as both input (requested flags) and output (resulting flags).
	 */
	void QuerySocketStateMany(TArrayView<FSocket*> Sockets, TArray<ESocketStateFlags>& InOutSocketStates, FTimespan WaitTime) const override;
