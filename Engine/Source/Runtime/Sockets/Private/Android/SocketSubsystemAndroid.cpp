...
{
	SocketSubsystemModule.UnregisterSocketSubsystem(FName("ANDROID"));
	FSocketSubsystemAndroid::Destroy();
}
...
/**
 * Creates a socket and ensures IPv6 sockets are dual-stack capable (IPV6_V6ONLY=false).
 * This matches the behavior of iOS, Unix, and Windows socket subsystems, allowing a
 * single IPv6 socket to accept both IPv4 and IPv6 connections.
 */
FSocket* FSocketSubsystemAndroid::CreateSocket(const FName& SocketType, const FString& SocketDescription, const FName& ProtocolType)
{
	...
	if (TempSocket)
	{
		...
		if (Result == 0)
		{
			...
			for (const auto& SortedAddress : SortedAddresses)
			{
				...
				UE_LOGF(LogSockets, Log, "(%ls) Address %ls", GetSocketAPIName(), *SortedAddress.ToString());
			}
			bSuccess = true;
		}
		else
		{
			int ErrNo = errno;
			UE_LOGF(LogSockets, Warning, "ioctl( ,SIOGCIFCONF, ) failed, errno=%d (%ls)", ErrNo, ANSI_TO_TCHAR(strerror(ErrNo)));
			bSuccess = false;
		}
