...
//	NETWORK_PROFILER(GNetworkProfiler.TrackSocketSendTo(this,Data,BytesSent,Destination));
	UE_LOGF(LogSockets, Verbose, "Socket '%ls' SendTo %i Bytes", *SocketDescription, BytesSent );
	return true;
}
...
//	NETWORK_PROFILER(GNetworkProfiler.TrackSocketSend(this,Data,BytesSent));
	UE_LOGF(LogSockets, Verbose, "Socket '%ls' Send %i Bytes", *SocketDescription, BytesSent );
	return true;
}
...
bool FSocket::RecvFrom(uint8* Data, int32 BufferSize, int32& BytesRead, FInternetAddr& Source, ESocketReceiveFlags::Type Flags)
{
	if( BytesRead > 0 )
	{
		UE_LOGF(LogSockets, Verbose, "Socket '%ls' RecvFrom %i Bytes", *SocketDescription, BytesRead );
	}
	return true;
}
...
bool FSocket::Recv(uint8* Data, int32 BufferSize, int32& BytesRead, ESocketReceiveFlags::Type Flags)
{
	if( BytesRead > 0 )
	{
		UE_LOGF(LogSockets, Verbose, "Socket '%ls' Recv %i Bytes", *SocketDescription, BytesRead );
	}
	return true;
}
...
bool FSocket::SetSendSegmentationOffloadSize(uint16 InSegmentPayloadBytes)
{
	return false;
}
