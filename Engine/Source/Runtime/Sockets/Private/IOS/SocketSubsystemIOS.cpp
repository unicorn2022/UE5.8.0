...
{
	UE_LOGF(LogIOS, Log, " FSocketSubsystemIOS::InternalBSDSocketFactory");
...
{
	SocketSubsystemModule.UnregisterSocketSubsystem(FName("IOS"));
	FSocketSubsystemIOS::Destroy();
}
	...
	if (getifaddrs(&Interfaces) == 0)
	{
		...
		for (ifaddrs* Travel = Interfaces; Travel != nullptr; Travel = Travel->ifa_next)
		{
			...
			if (NewAddress.Address->IsValid())
			{
				...
				UE_LOGF(LogIOS, Verbose, "Added address %ls with interface %ls", *NewAddress.ToString(), *NewAddress.InterfaceName);
		...
		{
			UE_LOGF(LogIOS, Verbose, "Ordered Address: %ls", *AddressInterface.ToString());
...
TArray<TSharedRef<FInternetAddr>> FSocketSubsystemIOS::GetLocalBindAddresses()
{
	...
	if (!GetMultihomeAddress(MultihomeAddr))
	{
		...
		{
			UE_LOGF(LogIOS, Warning, "Unable to grab the local adapters on this device in order to write scope id on binding addreesses!");
		}
	}
	else
	{
	...
	UE_LOGF(LogIOS, Verbose, "Using scope id %u for binding addresses", AdapterScopeId);
