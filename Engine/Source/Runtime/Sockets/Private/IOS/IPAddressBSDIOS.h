...
public:
	...
	virtual void SetIPv6BroadcastAddress() override
	{
		...
		if (SocketSubsystemIOS)
		{
			...
			{
				UE_LOGF(LogSockets, Warning, "Could not resolve the broadcast address for iOS, this address will just be blank");
			}
			else
			{
				...
				{
					UE_LOGF(LogSockets, Warning, "Could not get binding addresses to set the internal scope id for the broadcast address");
				}
			}
		}
		else
		{
			UE_LOGF(LogSockets, Warning, "Could not get the socketsubsystem for querying the scope id of the broadcast address");
		}
		SetPort(0);
	}
};
