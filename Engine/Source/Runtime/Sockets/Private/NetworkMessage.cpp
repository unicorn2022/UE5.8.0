...
bool FNFSMessageHeader::WrapAndSendPayload(const TArray<uint8>& Payload, const FSimpleAbstractSocket& Socket)
{
	...
	{
		UE_LOGF(LogSockets, Error, "Unable to send.");
		return false;
	}
	return true;
}
...
bool FNFSMessageHeader::ReceivePayload(FArrayReader& OutPayload, const FSimpleAbstractSocket& Socket)
{
	...
	{
		UE_LOGF(LogSockets, Error, "Unable to read full network header");
		return false;
	}
	...
	{
		UE_LOGF(LogSockets, Error, "Bad network header magic");
		return false;
	}
	if (!Header.PayloadSize)
	{
		UE_LOGF(LogSockets, Error, "Empty payload");
		return false;
	}
	...
	{
		UE_LOGF(LogSockets, Error, "Unable to read full payload");
		return false;
	}
	...
	{
		UE_LOGF(LogSockets, Error, "Payload Crc failure.");
		return false;
	}
