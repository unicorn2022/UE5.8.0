...
// as other subsystems (ie Steam) might override the default
	...
	#if PLATFORM_WINDOWS
		#define PLATFORM_SOCKETSUBSYSTEM FName("WINDOWS")
	#elif PLATFORM_MAC
		#define PLATFORM_SOCKETSUBSYSTEM FName("MAC")
	#elif PLATFORM_IOS
		#define PLATFORM_SOCKETSUBSYSTEM FName("IOS")
	#elif PLATFORM_UNIX
		#define PLATFORM_SOCKETSUBSYSTEM FName("UNIX")
	#elif PLATFORM_ANDROID
		#define PLATFORM_SOCKETSUBSYSTEM FName("ANDROID")
	#else
...
public:
	...
	/**
	 * Returns true if FSocket::Wait and QuerySocketStateMany is supported by this socket subsystem.
	 */
	...
	enum class ESocketStateFlags : uint8
	{
		Readable = 1 << 0,
		Writable = 1 << 1,
		Error = 1 << 2,
	...
	/**
	 * Queries the state of multiple sockets in a single call (equivalent to poll/select).
	 * Not supported on all platforms: check IsSocketWaitSupported() before calling.
	 *
	 * @param Sockets            The sockets to query.
	 * @param InOutSocketStates  On input, the flags to query per socket (Readable, Writable, Error).
	 *                           On output, the flags that are actually set. Must be the same length as Sockets.
	 * @param WaitTime           Maximum time to block waiting for any socket to become ready.
	 *                           Pass FTimespan(-1) to block indefinitely, or FTimespan::Zero() to poll without blocking.
	 */
	SOCKETS_API virtual void QuerySocketStateMany(TArrayView<FSocket*> Sockets, TArray<ESocketStateFlags>& InOutSocketStates, FTimespan WaitTime) const;
