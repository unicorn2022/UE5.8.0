		...
		Widget.GetAllChildren()->ForEachWidget([&, this](SWidget& NewWidget)
			{
				...
				if (bShouldAdd)
				{
					if (NewWidget.GetParentWidget().Get() != &Widget)
					{
						TStringBuilder<512> Builder;
						Builder << TEXT("The widget '") << FReflectionMetaData::GetWidgetDebugInfo(NewWidget) << TEXT("' has the widget '")
						<< FReflectionMetaData::GetWidgetPath(NewWidget.GetParentWidget().Get()) << TEXT("' as parent, but is also a child of widget '")
						<< FReflectionMetaData::GetWidgetPath(Widget) << ("' This is not supported and may eventually crash. Please fix.");
#if UE_BUILD_SHIPPING
						UE_LOGF(LogSlate, Error, "%ls", Builder.ToString());
#else
						UE_LOGF(LogSlate, Fatal, "%ls", Builder.ToString());
#endif
					}
...
void FSlateInvalidationWidgetList::LogWidgetsList(bool bOnlyVisible) const
{
	...
	for (FSlateInvalidationWidgetIndex Index = FirstIndex(); Index != FSlateInvalidationWidgetIndex::Invalid; Index = IncrementIndex(Index))
	{
		...
		if (!bOnlyVisible || InvalidateWidget.Visibility.IsVisible())
		{
			...
			UE_LOGF(LogSlate, Log, "%ls", Builder.ToString());
		}
	}
}
...
bool FSlateInvalidationWidgetList::VerifyWidgetsIndex() const
{
	...
	for (FSlateInvalidationWidgetIndex Index = FirstIndex(); Index != FSlateInvalidationWidgetIndex::Invalid; Index = IncrementIndex(Index))
	{
		...
		if (SWidget* Widget = InvalidateWidget.GetWidget())
		{
			...
			{
				UE_LOGF(LogSlate, Warning, "Widget '%ls' at index [%d,%d] is set to [%d,%d]."
			...
			{
				UE_LOGF(LogSlate, Warning, "Widget '%ls' at index [%d,%d] is set to the correct proxy index [%d,%d]."
					, *FReflectionMetaData::GetWidgetDebugInfo(Widget)
					, Index.ArrayIndex, Index.ElementIndex
					, WidgetIndex.ArrayIndex, WidgetIndex.ElementIndex);
				bResult = false;
		...
		{
			UE_LOGF(LogSlate, Warning, "Widget at index [%d,%d] is [null]", Index.ArrayIndex, Index.ElementIndex);
			bResult = false;
		}
	}
	return bResult;
...
bool FSlateInvalidationWidgetList::VerifyProxiesWidget() const
{
	...
	for (const FArrayNode& Node : Data)
	{
		...
		for (int32 ElementIndex = 0; ElementIndex < Node.StartIndex; ++ElementIndex)
		{
			...
			{
				UE_LOGF(LogSlate, Warning, "Element '%d' in the array of sort value '%d' as a valid widget '%ls' when it should be set to none."
		...
		for (int32 ElementIndex = Node.StartIndex; ElementIndex < Node.ElementList.Num(); ++ElementIndex)
		{
			...
			if (!Widget)
			{
				UE_LOGF(LogSlate, Warning, "Element '%d' in the array of sort value '%d' does not have a valid widget."
					, ElementIndex, Node.SortOrder);
				bResult = false;
			}
		}
...
bool FSlateInvalidationWidgetList::VerifySortOrder() const
{
	...
	if (FirstArrayIndex != INDEX_NONE)
	{
 		...
 		for (int32 ArrayIndex = Data[FirstArrayIndex].NextArrayIndex; ArrayIndex != INDEX_NONE; ArrayIndex = Data[ArrayIndex].NextArrayIndex)
		{
			...
			{
				UE_LOGF(LogSlate, Warning, "Array '%d' has a bigger sort order than previous array node '%d'."
...
bool FSlateInvalidationWidgetList::VerifyElementIndexList() const
{
	...
	if (bResult)
	{
		...
		if (!bResult)
		{
			UE_LOGF(LogSlate, Warning, "The 2 lists are not identical. With Attribute: %d. With ForEach: %d", WidgetListWithIterator.Num(), WidgetListWithForEachWidget.Num());
		}
	}
	...
	if (bResult)
	{
		for (const FArrayNode& ArrayNode : Data)
		{
			for (IndexType ElementIndex : ArrayNode.ElementIndexList_WidgetWithRegisteredSlateAttribute)
			{
				...
				{
					UE_LOGF(LogSlate, Warning, "ElementIndex '%d' in the array of sort value '%d' is invalid."
				...
				if (const SWidget* Widget = InvalidationWidget.GetWidget())
				{
					...
					{
						UE_LOGF(LogSlate, Warning, "ElementIndex '%d' in the array of sort value '%d' should not be there since widget '%ls' doesn't have registered SlateAttribute."
							, ElementIndex, ArrayNode.SortOrder
							, *FReflectionMetaData::GetWidgetDebugInfo(Widget));
						bResult = false;
					}
					...
					{
						UE_LOGF(LogSlate, Warning, "The widget '%ls' was not updated.", *FReflectionMetaData::GetWidgetDebugInfo(Widget));
						bResult = false;
					}
				}
			}
