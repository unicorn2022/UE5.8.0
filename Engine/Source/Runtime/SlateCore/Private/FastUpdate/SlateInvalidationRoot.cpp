...
FSlateInvalidationRoot::FSlateInvalidationRoot()
	...
	, bDeferInvalidateAllWidgetsAndClearResources(TOptional<bool>{})
#if WITH_SLATE_DEBUGGING
...
FSlateInvalidationResult FSlateInvalidationRoot::PaintInvalidationRoot(const FSlateInvalidationContext& Context)
{
	...
	if (!bNeedsSlowPath)
	{
		// In case widgets were added during Slow Path or PrePass, we need to recheck if we can call ProcessPreUpdate
		bNeedsSlowPath = !CanCacheThisFrame();
	}
#endif
...
bool FSlateInvalidationRoot::PaintFastPath(const FSlateInvalidationContext& Context)
{
	...
	{
		SCOPED_NAMED_EVENT(Slate_PaintSlowPath, FColor::Red);
		Context.WindowElementList->PopCachedElementData(); // to ensure we are not caching anymore
		ClearAllFastPathData(true);
		CachedMaxLayerId = PaintSlowPath(Context);
	}
	...
	{
		{
			...
			{
				UE_LOGF(LogSlate, Log, "Dumping Pre Invalidation List");
...
void FSlateInvalidationRoot::ProcessPrepassUpdate()
{
	...
	{
		UE_LOGF(LogSlate, Log, "Dumping Prepass Invalidation List");
		UE_LOGF(LogSlate, Log, "-------------------");
	}
#endif
...
bool FSlateInvalidationRoot::ProcessPostUpdate()
{
	...
	if (GSlateInvalidationRootDumpPostInvalidationList)
	{
		UE_LOGF(LogSlate, Log, "Dumping Post Invalidation List");
		UE_LOGF(LogSlate, Log, "-------------------");
	}
#endif
...
bool FSlateInvalidationRoot::ProcessInvalidation()
{
	...
	if (bDeferInvalidateAllWidgetsAndClearResources.IsSet())
	{
		HandleInvalidateAllWidgets(bDeferInvalidateAllWidgetsAndClearResources.GetValue());
		bDeferInvalidateAllWidgetsAndClearResources = {};
	}
...
{
	if (bProcessingPreUpdate | bProcessingAttributeUpdate | bProcessingPrepassUpdate | bProcessingPostUpdate)
	{
		bDeferInvalidateAllWidgetsAndClearResources = bClearResourcesImmediately;
	}
	else
	{
...
{
	UE_LOGF(LogSlate, Log, "Dumping Update List");
	UE_LOGF(LogSlate, Log, "-------------------");
	...
	for (int32 ListIndex = FinalUpdateList.Num() - 1; ListIndex >= 0; --ListIndex)
	{
		...
		if (WidgetProxy.Visibility.IsVisible() && WidgetPtr)
		{
			...
			{
				UE_LOGF(LogSlate, Log, "Volatile Repaint %ls", *FReflectionMetaData::GetWidgetDebugInfo(WidgetPtr));
			}
			else if (WidgetPtr->HasAnyUpdateFlags(EWidgetUpdateFlags::NeedsRepaint))
			{
				UE_LOGF(LogSlate, Log, "Repaint %ls", *FReflectionMetaData::GetWidgetDebugInfo(WidgetPtr));
			}
			else
			{
				if (WidgetPtr->HasAnyUpdateFlags(EWidgetUpdateFlags::NeedsActiveTimerUpdate))
				{
					UE_LOGF(LogSlate, Log, "ActiveTimer %ls", *FReflectionMetaData::GetWidgetDebugInfo(WidgetPtr));
				}

				if (WidgetPtr->HasAnyUpdateFlags(EWidgetUpdateFlags::NeedsTick))
				{
					UE_LOGF(LogSlate, Log, "Tick %ls", *FReflectionMetaData::GetWidgetDebugInfo(WidgetPtr));
				}
			}
		}
	}
...
void LogPreInvalidationItem(const FSlateInvalidationWidgetList& FastWidgetPathList, FSlateInvalidationWidgetIndex WidgetIndex)
{
	...
	{
		UE_LOGF(LogSlate, Log, "  AttributeRegistration %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
	else if (EnumHasAnyFlags(Proxy.CurrentInvalidateReason, EInvalidateWidgetReason::ChildOrder))
	{
		UE_LOGF(LogSlate, Log, "  Child Order %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
	else
	{
		UE_LOGF(LogSlate, Log, "  [?] %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
}
...
{
	const FWidgetProxy& Proxy = FastWidgetPathList[WidgetIndex];
	UE_LOGF(LogSlate, Log, "  Prepass %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
}
...
void LogPostInvalidationItem(const FSlateInvalidationWidgetList& FastWidgetPathList, FSlateInvalidationWidgetIndex WidgetIndex)
{
	...
	{
		UE_LOGF(LogSlate, Log, "  Layout %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
	else if (EnumHasAnyFlags(Proxy.CurrentInvalidateReason, EInvalidateWidgetReason::Visibility))
	{
		UE_LOGF(LogSlate, Log, "  Visibility %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
	else if (EnumHasAnyFlags(Proxy.CurrentInvalidateReason, EInvalidateWidgetReason::Volatility))
	{
		UE_LOGF(LogSlate, Log, "  Volatility %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
	else if (EnumHasAnyFlags(Proxy.CurrentInvalidateReason, EInvalidateWidgetReason::RenderTransform))
	{
		UE_LOGF(LogSlate, Log, "  RenderTransform %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
	else if (EnumHasAnyFlags(Proxy.CurrentInvalidateReason, EInvalidateWidgetReason::Paint))
	{
		UE_LOGF(LogSlate, Log, "  Paint %ls", *FReflectionMetaData::GetWidgetDebugInfo(Proxy.GetWidget()));
	}
...
void VerifyWidgetList(TSharedRef<SWidget> RootWidget, FSlateInvalidationRootHandle InvalidationRootHandle, FSlateInvalidationWidgetList& WidgetList)
{
	...
	if (!bIsIdentical)
	{
		UE_LOGF(LogSlate, Log, "**-- New Build List --**");
		List.LogWidgetsList(true);
		UE_LOGF(LogSlate, Log, "**-- Invaliation Root List --**");
		...
		if (!HittestGridSortData.Widget->GetProxyHandle().IsValid(HittestGridSortData.Widget))
		{
			...
			if (ParentWidetIndex != FSlateInvalidationWidgetIndex::Invalid)
			{
				UE_SLATE_LOG_ERROR_IF_FALSE(WidgetList.IsValidIndex(ParentWidetIndex) && EnumHasAllFlags(WidgetList[ParentWidetIndex].CurrentInvalidateReason, EInvalidateWidgetReason::ChildOrder)
					, CVarSlateInvalidationRootVerifyHittestGrid
					, TEXT("The parent widget doesn't have ChildOrder invalidation. The hittest grid has an invalid widget."));
			}
		}
	}
}
