...
void FSlateApplicationBase::UpdateAnyActiveTimersArePending()
{
	...
	{
		auto ActiveTimerInfoPinned = (*It).Pin();

		// remove any tick handles that may have become invalid.
		...
		if(!ActiveTimerInfoPinned.IsValid())
		{
			It.RemoveCurrent();
			continue;
		}
...
void FSlateApplicationBase::InvalidateAllWidgets(bool bClearResourcesImmediately) const
{
	...
	if (DoesThreadOwnSlateRendering())
	{
		...
		UE_LOGF(LogSlate, Log, "InvalidateAllWidgets triggered.  All widgets were invalidated");
