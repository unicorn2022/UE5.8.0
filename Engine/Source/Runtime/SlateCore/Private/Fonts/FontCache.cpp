	...
	, CachedFontSkew(GlyphsToRender.Num() > 0 ? GlyphsToRender[0].FontFaceData->FontSkew : 0.f)
{
	...
	for (int32 CurrentGlyphIndex = 0; CurrentGlyphIndex < NumGlyphsToRender; ++CurrentGlyphIndex)
	{
		...
		if (!SourceIndexToGlyphData)
		{
			...
			UE_LOGF(LogSlate, Warning, "No Glyph! Index %i. Valid %i, %i, Valid Glyph %i, Visible %i, Num %i, Grapheme %i, Dir %u",
			...
			if (CurrentGlyph.FontFaceData.IsValid())
			{
				if (TSharedPtr<FFreeTypeFace> FontFacePtr = CurrentGlyph.FontFaceData->FontFace.Pin())
				{
					...
					UE_LOGF(LogSlate, Warning, "Font missing Gylph. Valid %i, Loading %i",
...
void FShapedGlyphSequence::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	if (GSlateCheckUObjectShapedGlyphSequence)
	{
		...
		if (!DebugFontMaterialName.IsNone())
		{
			...
			{
				UE_LOGF(LogSlate, Fatal, "Material % ls has become invalid. This means the FShapedGlyphSequence::FontMaterial was garbage collected while slate was using it", *DebugFontMaterialName.ToString());
			}
		}
		if (!DebugOutlineMaterialName.IsNone())
		{
			...
			{
				UE_LOGF(LogSlate, Fatal, "Material %ls has become invalid. This means the FShapedGlyphSequence::OutlineSettings::OutlineMaterial was garbage collected while slate was using it", *DebugOutlineMaterialName.ToString());
			}
		}
	}
#endif
...
bool FSlateFontCache::AddNewEntry( const FCharacterRenderData InRenderData, uint8& OutTextureIndex, uint16& OutGlyphX, uint16& OutGlyphY, uint16& OutGlyphWidth, uint16& OutGlyphHeight, uint8& OutPaddingOffset )
{
	...
	if (InRenderData.SizeX > FontAtlasSize.X || InRenderData.SizeY > FontAtlasSize.Y)
	{
		...
		if (NonAtlasedTexture.IsValid())
		{
			...
			UE_LOGF(LogSlate, Warning, "SlateFontCache - Glyph texture is too large to store in the font atlas, so we're falling back to a non-atlased texture for this glyph. This may have SERIOUS performance implications. Atlas page size: { %d, %d }. Glyph render size: { %d, %d }",
			...
			{
				UE_LOGF(LogSlate, Warning, "SlateFontCache - Glyph texture has more than 256 textures.");
				return false;
			}
		...
		UE_LOGF(LogSlate, Warning, "SlateFontCache - Glyph texture is too large to store in the font atlas, but we cannot support rendering such a large texture. Atlas page size: { %d, %d }. Glyph render size: { %d, %d }",
			FontAtlasSize.X, FontAtlasSize.Y,
			InRenderData.SizeX, InRenderData.SizeY);
	...
	if (AllFontTextures.Num() >= std::numeric_limits<uint8>::max())
	{
		UE_LOGF(LogSlate, Warning, "SlateFontCache - Atlas has more than 256 textures.");
		return false;
	}
...
FShapedGlyphSequenceRef FSlateFontCache::ShapeBidirectionalText( const FString& InText, const FSlateFontInfo &InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InBaseDirection, const ETextShapingMethod InTextShapingMethod , TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces) const
{
	return ShapeBidirectionalText(*InText, 0, InText.Len(), InFontInfo, InFontScale, InBaseDirection, InTextShapingMethod, OutLoadingFontFaces);
}

FShapedGlyphSequenceRef FSlateFontCache::ShapeBidirectionalText( const TCHAR* InText, const int32 InTextStart, const int32 InTextLen, const FSlateFontInfo &InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InBaseDirection, const ETextShapingMethod InTextShapingMethod , TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces) const
{
	return TextShaper->ShapeBidirectionalText(InText, InTextStart, InTextLen, InFontInfo, InFontScale, InBaseDirection, InTextShapingMethod, OutLoadingFontFaces);
}

FShapedGlyphSequenceRef FSlateFontCache::ShapeUnidirectionalText( const FString& InText, const FSlateFontInfo &InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InTextDirection, const ETextShapingMethod InTextShapingMethod , TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces) const
{
	return ShapeUnidirectionalText(*InText, 0, InText.Len(), InFontInfo, InFontScale, InTextDirection, InTextShapingMethod, OutLoadingFontFaces);
}

FShapedGlyphSequenceRef FSlateFontCache::ShapeUnidirectionalText( const TCHAR* InText, const int32 InTextStart, const int32 InTextLen, const FSlateFontInfo &InFontInfo, const float InFontScale, const TextBiDi::ETextDirection InTextDirection, const ETextShapingMethod InTextShapingMethod , TArray<TWeakPtr<FFreeTypeFace>>* OutLoadingFontFaces) const
{
	return TextShaper->ShapeUnidirectionalText(InText, InTextStart, InTextLen, InFontInfo, InFontScale, InTextDirection, InTextShapingMethod, OutLoadingFontFaces);
}
...
FSdfGlyphFontAtlasData FSlateFontCache::GetSdfGlyphFontAtlasData(const FShapedGlyphEntry& InShapedGlyph, const FFontOutlineSettings& InOutlineSettings)
{
	...
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("FSlateFontCache::GetSdfGlyphFontAtlasData_AddEntry");
	...
	else
	{
		...
		NewAtlasData->bSupportsSdf = false;
		UE_LOGF(LogSlate, Warning, "SlateFontCache - Requested SDF cannot fit font atlas.");
	}
	SdfGlyphToAtlasData.Add(GlyphKey, NewAtlasData);
...
void FSlateFontCache::RequestFlushCache(const FString& FlushReason)
{
	if (!bFlushRequested)
	{
		...
		UE_LOGF(LogSlate, Log, "FontCache flush requested. Reason: %ls", *FlushReason);
#else
		UE_LOGF(LogSlate, Warning, "FontCache flush requested. Reason: %ls", *FlushReason);
#endif
...
void FSlateFontCache::ConditionalDumpFontCacheStats() const
{
	if (bDumpFontCacheStats)
	{
		...
		//Display statistics
		UE_LOGF(LogSlate, Log, "%d glyphs in cache, using %d pixels on %d total. Using %f%% of all font textures.", GlyphCount, PixelsUsedInAtlasCount, FontAtlasPixelTotalCount, static_cast<float>(PixelsUsedInAtlasCount) * 100.0f / static_cast<float>(FontAtlasPixelTotalCount));
		UE_LOGF(LogSlate, Log, "FontFace & Style | Size (72DPI) | OutlineSize | Mitered | Sep. Fill a | Skew || Glyphs Count |");
		...
		for (const TPair<FFontCacheStatsKey, int>& Kvp : Stats)
		{
			...
			UE_LOGF(LogSlate, Log, "%ls %ls | %d | %d | %ls | %ls | %.2f || %d |",
...
#if !WITH_EDITOR
		UE_LOGF(LogSlate, Log, "Slate font cache was flushed");
#endif
