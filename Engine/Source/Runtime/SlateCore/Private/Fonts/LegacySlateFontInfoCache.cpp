...
TSharedRef<const FCompositeFont> FLegacySlateFontInfoCache::GetDefaultFont()
{
	if (!DefaultFont.IsValid())
	{
		...
		// Devanagari  (editor-only)
		if (GIsEditor)
		{
			FCompositeSubFont& SubFont = MutableDefaultFont->SubTypefaces.AddDefaulted_GetRef();
			APPEND_RANGE(SubFont, Devanagari);
			APPEND_RANGE(SubFont, DevanagariExtended);
			APPEND_EDITOR_FONT(SubFont.Typeface, "Regular", "NotoSansDevanagari-Regular.ttf", EFontHinting::Default);
		}

		// Gurmukhi  (editor-only)
		if (GIsEditor)
		{
			FCompositeSubFont& SubFont = MutableDefaultFont->SubTypefaces.AddDefaulted_GetRef();
			APPEND_RANGE(SubFont, Gurmukhi);
			APPEND_EDITOR_FONT(SubFont.Typeface, "Regular", "NotoSansGurmukhi-Regular.ttf", EFontHinting::Default);
		}
...
const FFontData& FLegacySlateFontInfoCache::GetLastResortFontData(const FFallbackContext& InContext)
{
	...
	if (!LastResortFontData.IsValid())
	{
		...
		UE_LOGF(LogSlate, Log, "Last resort fallback font was requested. %ls", *InContext.ToString());
	}
