...
/**
 * Enable or disable detection of async loading font faces. This impacts whether or not FTextLayout, FSlateTextLayout etc would be able to detect if font faces are loading asynchronously and respond accordingly.
 * See FTextLayout::AreAnyFontFacesLoading() and FSlateTextLayout::OnPaint() to see how the detection of async loading font faces for a text widget has implications on rendering of text and execution of delegates.
 */
static bool GDetectAsyncLoadingFontFaces = false;
FAutoConsoleVariableRef CVarDetectAsyncLoadingFontFaces(
	TEXT("Slate.Font.DetectAsyncLoadingFontFaces"),
	GDetectAsyncLoadingFontFaces,
	TEXT("If set to true, text widgets will be able to detect if any of its font faces are loading asynchornously and respond accordingly.. This does nothing if Slate.Font.AsyncLazyLoad is set to false."));
...
bool FSlateFontRenderer::CanLoadCodepoint(const FFontData& InFontData, const UTF32CHAR InCodepoint, EFontFallback MaxFallbackLevel) const
{
	...
	bReturnVal = FaceGlyphData.FaceAndMemory.IsValid() && !FaceGlyphData.FaceAndMemory->IsFaceLoading() && FaceGlyphData.GlyphIndex != 0;
#endif // WITH_FREETYPE
...
FFreeTypeFaceGlyphData FSlateFontRenderer::GetFontFaceForCodepoint(const FFontData& InFontData, const UTF32CHAR InCodepoint, EFontFallback MaxFallbackLevel) const
{
	...
	if (ReturnVal.FaceAndMemory.IsValid())
	{
		...
		// If we want to detect and handle still loading font faces, we avoid clearing the face in memory so text widgets are able to react accordingly when font faces are detected to still be loading.
		if (ReturnVal.FaceAndMemory->IsFaceLoading())
		{
			// This would prevent the detection of fonts that are still loading while text is being passed through the shaping pass
			// By clearing the face in memory, there is no way for text widgets to detect which font faces are loading and thus react accordingly  
			if (!GDetectAsyncLoadingFontFaces)
			{
	...
	if (!ReturnVal.FaceAndMemory.IsValid() || (InCodepoint != 0 && ReturnVal.GlyphIndex == 0))
	{
		...
		{
			UE_LOGF(LogSlate, Warning, "Could not find Glyph Index %u with codepoint U+%x, getting last resort font data %ls", ReturnVal.GlyphIndex, InCodepoint, *InFontData.GetFontFilename());
...
bool FSlateFontRenderer::GetRenderDataInternal(const FFreeTypeFaceGlyphData& InFaceGlyphData, const float InScale, const FFontOutlineSettings& InOutlineSettings, FCharacterRenderData& OutRenderData) const
{
	...
	if((ScaledOutlineSize > 0.0f || OutlineFontRenderMethod == 1) && Slot->format == FT_GLYPH_FORMAT_OUTLINE)
	{
		...
		if (Error != 0)
		{
			UE_LOGF(LogSlate, Warning, "FreeType error %x while trying to render glyph index %u from face %ls %ls (from outline path)", Error, Slot->glyph_index, UTF8_TO_TCHAR(Face->family_name), UTF8_TO_TCHAR(Face->style_name));
			return false;
		}
	}
	else
	{
		...
		if (Error != 0)
		{
			UE_LOGF(LogSlate, Warning, "FreeType error %x while trying to render glyph index %u from face %ls %ls (from standard path)", Error, Slot->glyph_index, UTF8_TO_TCHAR(Face->family_name), UTF8_TO_TCHAR(Face->style_name));
			return false;
		}

		FT_Bitmap* Bitmap = nullptr;
		FT_Bitmap TmpBitmap;
// Available from FreeType2-2.10.0 onwards 
...
FT_Face FSlateFontRenderer::GetFontFace(const FFontData& InFontData) const
{
	...
	return (FaceAndMemory.IsValid() && !FaceAndMemory->IsFaceLoading()) ? FaceAndMemory->GetFace() : nullptr;
}
