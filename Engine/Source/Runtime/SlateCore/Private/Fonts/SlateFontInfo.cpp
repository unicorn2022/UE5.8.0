...
#endif
{
	...
	{
		UE_LOGF(LogSlate, Warning, "FSlateFontInfo was constructed with a null FCompositeFont. Slate will be forced to use the fallback font path which may be slower.");
	}
}
...
#endif
{
	...
	if (InFontObject)
	{
		...
		{
			UE_LOGF(LogSlate, Verbose, "'%ls' does not provide a composite font that can be used with Slate. Slate will be forced to use the fallback font path which may be slower.", *InFontObject->GetName());
		}
	}
	else
	{
		UE_LOGF(LogSlate, Warning, "FSlateFontInfo was constructed with a null UFont. Slate will be forced to use the fallback font path which may be slower.");
	}
}
