...
bool PreprocessFontGeometry(FPreprocessedFontGeometry& OutPreprocessedFontGeometry, FT_Face InFreeTypeFace)
{
	...
	UE_LOGF(LogSlate, Log, "Preprocessed geometry for font %s is %d bytes, containing %d paths and %d windings out of %d total glyphs.", InFreeTypeFace->family_name, int(OutPreprocessedFontGeometry.GetDataSize()), StatNumFullGeometry, StatNumWindings, int(InFreeTypeFace->num_glyphs));
