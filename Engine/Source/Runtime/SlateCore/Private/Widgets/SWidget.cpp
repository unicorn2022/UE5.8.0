...
#include "Types/SlateToolTipForceFieldExpansionMetaData.h"
...
SWidget::SWidget()
	...
	, bIsProjectContent(false)
	, bIsProjectContentParent(false)
...
{
#if WITH_SLATE_DEBUGGING
	UE_CLOGF(Debug_DestroyedTag != 0xDC, LogSlate, Fatal, "The widget is already destroyed.");
...
{
	return OnNavigation(MyGeometry, InNavigationEvent.GetNavigationType());
}

FNavigationReply SWidget::OnNavigation(const FGeometry& MyGeometry, const EUINavigation Type)
{
...
void SWidget::AssignParentWidget(TSharedPtr<SWidget> InParent)
{
	...
	const TSharedPtr<SWidget> PreviousParent = ParentWidgetPtr.Pin();
	...
	{
		// We want to be sure that the previous parent gets a ChildOrder invalidation for Dynamic Invalidation,
		// otherwise we might not be able to catch widgets having the wrong parents
		PreviousParent->Invalidate(EInvalidateWidgetReason::ChildOrder);
	}
	if (InParent.IsValid())
	{
...
#if WITH_SLATE_DEBUGGING
	const FSlateInvalidationRoot* InvalidationRoot = FastPathProxyHandle.GetInvalidationRootHandle().GetInvalidationRoot();
	if (FastPathProxyHandle.IsValid(this) && InvalidationRoot && !InvalidationRoot->NeedsSlowPath())
	{
...
void SWidget::SetFastPathProxyHandle(const FWidgetProxyHandle& Handle, FSlateInvalidationWidgetVisibility InvalidationVisibility, bool bParentVolatile)
{
	...
	if (FSlateInvalidationRoot* Root = FastPathProxyHandle.GetInvalidationRootHandle().GetInvalidationRoot())
	{
		if (FHittestGrid* HittestGrid = Root->GetHittestGrid())
		{
			HittestGrid->UpdateWidget(this, Handle.GetWidgetSortOrder());
		}
	}
...
void SWidget::SetToolTipForceFieldExpansion(const TAttribute<TOptional<FSlateRect>>& InExpansionRect)
{
	const TSharedRef<FSlateToolTipForceFieldExpansionMetaData> DefaultMetaData = FindOrAddMetaData<FSlateToolTipForceFieldExpansionMetaData>();
	DefaultMetaData->Attribute = InExpansionRect;
}

TOptional<FSlateRect> SWidget::GetToolTipForceFieldExpansion() const
{
	TOptional<FSlateRect> Result;
	for (const TSharedRef<FSlateToolTipForceFieldExpansionMetaData>& ExpansionMetaData : GetAllMetaData<FSlateToolTipForceFieldExpansionMetaData>())
	{
		if (ExpansionMetaData->Attribute.IsSet())
		{
			TOptional<FSlateRect> SlateRect = ExpansionMetaData->Attribute.Get();
			if (SlateRect.IsSet())
			{
				if (!Result.IsSet())
				{
				...
				{
					Result = Result->Expand(SlateRect.GetValue());
				}
			}
		}
	}
...
void SWidget::Invalidate(EInvalidateWidgetReason InvalidateReason)
{
	...
	{
		// This is done here rather than in the FSlateInvalidationRoot to help catching all cases of widgets having wrong parents
		InvalidateCachedSupportsInvalidation();
	}
#endif
...
FSlateRect SWidget::GetHitTestBoundingRect() const
{
	return GetPaintSpaceGeometry().GetRenderBoundingRect();
}
...
bool SWidget::SupportsInvalidationRecursive(EInvalidationStrategy InInvalidationStrategy) const
{
	if ((InInvalidationStrategy == EInvalidationStrategy::UseCachedValue) &
		(CachedSupportsInvalidationRecursive != ECachedInvalidationResult::NeedsRefresh))
	{
		return CachedSupportsInvalidationRecursive == ECachedInvalidationResult::SupportsInvalidation;
	}

	if (!SupportsInvalidation())
	{
		check(CachedSupportsInvalidationRecursive == ECachedInvalidationResult::ClassNotSupportingInvalidation);
		return false;
	}
	check(CachedSupportsInvalidationRecursive != ECachedInvalidationResult::ClassNotSupportingInvalidation);


	const ECachedInvalidationResult PreviousCachedValue = CachedSupportsInvalidationRecursive;
	// Sets the value to Refreshing to avoid checking this widget multiple times
	CachedSupportsInvalidationRecursive = ECachedInvalidationResult::Refreshing;

	FChildren* Children = const_cast<SWidget*>(this)->GetAllChildren();
	for (int32 ChildIndex = 0; ChildIndex < Children->Num(); ++ChildIndex)
	{
		...
		if (!ChildWidget->ParentWidgetPtr.HasSameObject(this))
		{
			// Here the child  widget has another widget as parent, so we need to mark them as not supporting invalidation
			CachedSupportsInvalidationRecursive = ECachedInvalidationResult::ChildrenNotSupportingInvalidation;
			if (ChildWidget->CachedSupportsInvalidationRecursive != ECachedInvalidationResult::ClassNotSupportingInvalidation)
			{
				ChildWidget->CachedSupportsInvalidationRecursive = ECachedInvalidationResult::InvalidParent;
			}
			break;
		}

		// If we had an invalid parent but we don't have one anymore, we recheck if we are invalidation compatible, otherwise we use the cached value
		const EInvalidationStrategy Strategy = ChildWidget->CachedSupportsInvalidationRecursive == ECachedInvalidationResult::InvalidParent ?
			EInvalidationStrategy::RecomputeSelf : EInvalidationStrategy::UseCachedValue;

		if (!ChildWidget->SupportsInvalidationRecursive(Strategy))
		{
			CachedSupportsInvalidationRecursive = ECachedInvalidationResult::ChildrenNotSupportingInvalidation;
			break;
		}
	...
	// If we have checked all the children and none of them were invalid, then we are supporting invalidation
	if (CachedSupportsInvalidationRecursive == ECachedInvalidationResult::Refreshing)
	{
		CachedSupportsInvalidationRecursive = ECachedInvalidationResult::SupportsInvalidation;
	}

	// If the value has changed, we invalidate the parent unless it is currently recomputing
	if (CachedSupportsInvalidationRecursive != PreviousCachedValue)
	{
		const TSharedPtr<SWidget> ParentWidget = GetParentWidget();
		if (ParentWidget && ParentWidget->CachedSupportsInvalidationRecursive != ECachedInvalidationResult::Refreshing)
		{
			if (CachedSupportsInvalidationRecursive != ECachedInvalidationResult::SupportsInvalidation)
			{
				...
				ParentWidget->SetSelfAndParentsAsNotSupportingInvalidation();
			}
			else
			{
				// If we now support invalidation, ask the parent to recompute
				ParentWidget->SupportsInvalidationRecursive(EInvalidationStrategy::RecomputeSelf);
			}
		}
	}
...
void SWidget::SetSelfAndParentsAsNotSupportingInvalidation()
{
	TSharedPtr<SWidget> Widget = (this == &SNullWidget::NullWidget.Get()) ? GetParentWidget() : AsShared();
	while (Widget)
	{
		...
		if (Widget->CachedSupportsInvalidationRecursive == ECachedInvalidationResult::SupportsInvalidation)
		{
			Widget->CachedSupportsInvalidationRecursive = ECachedInvalidationResult::ChildrenNotSupportingInvalidation;
...
UE_TRACE_EVENT_BEGIN(Cpu, SWidget_InvalidateCachedSupportsInvalidation, NoSync)
	UE_TRACE_EVENT_FIELD(UE::Trace::WideString, CreatedInLocation)
UE_TRACE_EVENT_END()

void SWidget::InvalidateCachedSupportsInvalidation()
{
...
#if UE_SLATE_TRACE_ENABLED
	UE_TRACE_LOG_SCOPED_T(Cpu, SWidget_InvalidateCachedSupportsInvalidation, SlateChannel)
		<< SWidget_InvalidateCachedSupportsInvalidation.CreatedInLocation(*GetCreatedInLocation().ToString());
#endif // UE_SLATE_TRACE_ENABLED

	TSharedPtr<SWidget> Widget = (this == &SNullWidget::NullWidget.Get()) ? GetParentWidget() : AsShared();
	while (Widget)
	{
		if ((Widget->CachedSupportsInvalidationRecursive == ECachedInvalidationResult::ClassNotSupportingInvalidation)
			| (Widget->CachedSupportsInvalidationRecursive == ECachedInvalidationResult::NeedsRefresh))
		{
			return;
		}

		if (GSlateIsOnFastUpdatePath && Widget->Advanced_IsInvalidationRoot())
		{
			if (!Widget->SupportsInvalidationRecursive(EInvalidationStrategy::RecomputeSelf))
			{
				// If we are currently in fast path, we want to stop and do a slow path as we might have invalid widgets that have been added
				const_cast<FSlateInvalidationRoot*>(Widget->Advanced_AsInvalidationRoot())->InvalidateRootLayout(this);
			}
			// We do not need to refresh the parents of this widget as `SupportsInvalidationRecursive` would have taken care of it if necessary.
			return;
		}
...
void SWidget::SetClassSupportsInvalidation(bool bClassSupportsInvalidation)
{
	bSupportsInvalidation = bClassSupportsInvalidation;
	CachedSupportsInvalidationRecursive = bSupportsInvalidation ?
		ECachedInvalidationResult::NeedsRefresh : ECachedInvalidationResult::ClassNotSupportingInvalidation;
}

#endif // ~UE_SLATE_WITH_DYNAMIC_INVALIDATION
