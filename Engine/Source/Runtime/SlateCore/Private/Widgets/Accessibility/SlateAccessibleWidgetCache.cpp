...
void FSlateAccessibleWidgetCache::DumpAccessibilityStats()
{
	...
	UE_LOGF(LogAccessibility, Log, "Dumping Slate accessibility stats:");
	UE_LOGF(LogAccessibility, Log, "Number of cached widgets and Ids: %i, %i", NumWidgets, NumIds);
	UE_LOGF(LogAccessibility, Log, "Size of FSlateAccessibleWidget: %u", SizeOfWidget);
	UE_LOGF(LogAccessibility, Log, "Size of id map: %u", SizeOfIdMap);
	UE_LOGF(LogAccessibility, Log, "Approximate memory stored in cache: %u kb", CacheSize / 1000);
}
