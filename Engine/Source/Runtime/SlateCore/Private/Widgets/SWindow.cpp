...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
		FCoreDelegates::PostSlateModalWithContext.Broadcast({ .WindowIdentifier = (intptr_t)this });
	}
#endif
	...
	if (IsRegularWindow())
	{
		UE_LOGF(LogSlate, Log, "Window '%ls' being destroyed", *GetTitle().ToString());
	}
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(SWindow::ShowWindow);
...
{
	InvalidateRootChildOrder();
	UE_LOGF(LogSlate, Log, "Toggling fast path.  New State: %d", bGlobalInvalidationEnabled);
}
...
EWindowZone::Type SWindow::GetCurrentWindowZone(UE::Slate::FDeprecateVector2DParameter LocalMousePosition)
{
	...
	if (FSlateApplicationBase::Get().GetPlatformCursor()->GetType() == EMouseCursor::None)
	{
		return EWindowZone::ClientArea;
	}
	if (LocalMousePosition.X >= 0 && LocalMousePosition.X < Size.X && LocalMousePosition.Y >= 0 && LocalMousePosition.Y < Size.Y)
	{
		int32 Row = 1;
		int32 Col = 1;
		EWindowMode::Type Mode = GetWindowMode();
		const bool bFullscreen = Mode == EWindowMode::Fullscreen || Mode == EWindowMode::WindowedFullscreen;
		const bool bResizable = SizingRule == ESizingRule::UserSized && !bFullscreen && !NativeWindow->IsMaximized();
		if (bResizable)
		{
		...
		if (InZone == EWindowZone::ClientArea)
		{
			...
			constexpr bool bIgnoreEnabledStatus = false;
			FWidgetPath HitTestResults = FSlateApplicationBase::Get().GetHitTesting()
				.LocateWidgetInWindow(GetPositionInScreen() + LocalMousePosition, SharedThis(this), bIgnoreEnabledStatus, INDEX_NONE);
			if( HitTestResults.Widgets.Num() > 0 )
			{
				...
				if (ZoneOverride != EWindowZone::Unspecified && (bResizable || !EWindowZone::IsBorder(ZoneOverride)))
				{
