...
uint32 FSlateDebugging::NoInvalidationPanels = 0;
uint32 FSlateDebugging::NoDynamicInvalidationPanels = 0;
uint32 FSlateDebugging::NoDynamicInvalidationPanelsSupportingInvalidation = 0;

void FSlateDebugging::IncrementInvalidationPanelCount(bool bCanCache, bool bCanUseDynamic, bool bSupportsInvalidation)
{
	if (!bCanCache)
	{
		return;
	...
	++NoInvalidationPanels;
	if (bCanUseDynamic)
	{
		++NoDynamicInvalidationPanels;
		if (bSupportsInvalidation)
		{
			++NoDynamicInvalidationPanelsSupportingInvalidation;
		}
	}
}

void FSlateDebugging::ResetInvalidationPanelCount()
{
	NoInvalidationPanels = 0;
	NoDynamicInvalidationPanels = 0;
	NoDynamicInvalidationPanelsSupportingInvalidation = 0;
}
