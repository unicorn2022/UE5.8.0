...
FConsoleSlateDebuggerPaint::FConsoleSlateDebuggerPaint()
	...
	, OnlyProjectContentRefCVar(
		TEXT("SlateDebugger.Paint.OnlyProjectContent")
		, bDebugProjectContentOnly
		, TEXT("Only debug the project's contents (exclude editor, chrome, diagnostic output)")
		, FConsoleVariableDelegate::CreateRaw(this, &FConsoleSlateDebuggerPaint::RebuildWidgetListOnVariableChanged))
	...
	, OnlyInvalidationRootRefCVar(
		TEXT("SlateDebugger.Paint.OnlyInvalidationRoot")
...
void FConsoleSlateDebuggerPaint::AddUpdatedWidget(const SWidget& Widget, const FConsoleSlateDebuggerUtility::TSWindowId WindowId, bool bIncrementUpdateCount)
{
	...
	{
		UE_LOGF(LogSlateDebugger, Warning, "'%ls' got painted more than once on frame %u.", *(WidgetInfo.WidgetName), GFrameNumber);
	}
