...
static FAutoConsoleVariableRef CVarApplyDisabledEffectOnWidgets(
	...
	TEXT("If true, disabled widgets will have alpha multiplied by 0.45 (applies to project content only, not editor)."),
...
static bool IsResourceObjectValid(UObject*& InObject)
{
	...
	// Catch GC'd-but-not-yet-collected UObjects via the engine's standard flag/state checks.
	// NOTE: these still dereference InObject (to read flags / internal index), so they do NOT survive
	// a fully freed/unmapped backing page. Such case must be fixed upstream by ensuring the FSlateBrush isn't dangling.
	if (!IsValidChecked(InObject) || InObject->IsUnreachable() || InObject->HasAnyFlags(RF_BeginDestroyed))
	{
		UE_LOGF(LogSlate, Warning, "[%ls] Attempted to access resource for %ls which is pending kill, unreachable or pending destroy - Platform=%ls",
			ANSI_TO_TCHAR(__FUNCTION__), *InObject->GetName(), ANSI_TO_TCHAR(FPlatformProperties::PlatformName()));
		return false;
	}
...
{
	if (ElementList.IsInProjectContent() && !bApplyDisabledEffectOnWidgets)
	{
...
void FSlateDrawElement::MakeText( FSlateWindowElementList& ElementList, uint32 InLayer, const FPaintGeometry& PaintGeometry, const FString& InText, const FSlateFontInfo& InFontInfo, ESlateDrawEffect InDrawEffects, const FLinearColor& InTint )
{
	...
	{
		UE_LOGF(LogSlate, Log, "MakeText: '%ls'.", *InText);
	}
#endif
...
namespace SlateDrawElement
{
	...
	void CheckInvalidUMaterial(const UObject* InMaterialResource, const TCHAR* MaterialMessage)
	{
		if (InMaterialResource && GSlateCheckUObjectRenderResources)
		{
			...
			{
				UE_LOGF(LogSlate, Error, "Material '%ls' is not valid. PendingKill:'%d'. ValidLowLevelFast:'%d'. InvalidClass:'%d'"
					, MaterialMessage
				...
				if (bIsValidLowLevel)
				{
					UE_LOGF(LogSlate, Error, "Material name: '%ls'", *InMaterialResource->GetFullName());
				}
				...
				{
					UE_LOGF(LogSlate, Fatal, "%ls", Message);
				}
				else
				{
...
void FSlateDrawElement::MakeCustomVerts(FSlateWindowElementList& ElementList, uint32 InLayer, const FSlateResourceHandle& InRenderResourceHandle, const TArray<FSlateVertex>& InVerts, const TArray<SlateIndex>& InIndexes, ISlateUpdatableInstanceBuffer* InInstanceData, uint32 InInstanceOffset, uint32 InNumInstances, ESlateDrawEffect InDrawEffects, ESlateBatchDrawFlag InBatchFlags)
{
	...
	Element.AddBatchFlags(InBatchFlags);
	Element.RenderTransform = FSlateRenderTransform();
}
