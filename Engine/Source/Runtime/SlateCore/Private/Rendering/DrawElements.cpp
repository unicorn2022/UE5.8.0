...
{
	bIsProjectContent = bInGameLayer;
}
...
void FSlateWindowElementList::SetIsInProjectContent(bool bIsInProjectContent)
{
	bIsProjectContent = bIsInProjectContent;
}
...
{
	if (!CachedElementDataListStack.IsEmpty())
	{
...
void FSlateCachedElementData::Empty()
{
	...
	{
		UE_LOGF(LogSlate, Verbose, "Resetting cached element data.  Num: %d", CachedElementLists.Num());
	}
