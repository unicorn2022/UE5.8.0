...
void FSlateVectorGraphicsCache::UpdateCache()
{
	...
	if (PendingRequests.Num() > 0)
	{
		UE_LOGF(LogSlate, Verbose, "SVG raster took: %fms", Time * 1000.f);
	}
...
void FSlateVectorGraphicsCache::RequestFlushCache(const FString& Reason)
{
	...
	UE_LOGF(LogSlate, Log, "Vector cache flush requested. Reason: %ls", *Reason);
#else
	UE_LOGF(LogSlate, Warning, "Vector cache flush requested. Reason: %ls", *Reason);
#endif
