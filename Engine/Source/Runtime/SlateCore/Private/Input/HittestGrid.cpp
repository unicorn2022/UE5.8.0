...
FHittestGrid::FHittestGrid()
	...
	, PaintToHitTestTransform() // Default identity
...
bool FHittestGrid::IsWidgetDataInLayerRange(const FWidgetData& WidgetData, const TInterval<int32>& LayerRange)
{
	// top 32 bits are InBatchPriorityGroup, which we don't worry about
	int32 WidgetLayer = static_cast<int32>(WidgetData.PrimarySort);
	
	return LayerRange.Min <= WidgetLayer && LayerRange.Max > WidgetLayer;
}

TArray<FWidgetAndPointer> FHittestGrid::GetBubblePath(UE::Slate::FDeprecateVector2DParameter DesktopSpaceCoordinate, float CursorRadius, bool bIgnoreEnabledStatus, int32 UserIndex, TInterval<int32> LayerRange)
{
		...
		if (BestHit.IsValid())
		{
			...
			if (FirstHitWidget.IsValid() && IsCompatibleUserIndex(UserIndex, BestHitWidgetData.UserIndex))
			{
				...
				while (CurWidget.IsValid())
				{
					...
					if(IsWidgetDataInLayerRange(BestHitWidgetData, LayerRange))
					{
				...
				if (Path.IsEmpty() || !Path.Last().Widget->Advanced_IsWindow())
				{
...
{
	/*extern*/ const FText Valid = LOCTEXT("StateValid", "Valid"); //~ The widget is valid will be consider as the result
	/*extern*/ const FText NotCompatibleWithUserIndex = LOCTEXT("StateNotCompatibleWithUserIndex", "User Index not compatible"); //~ The widget is not compatible with the requested user index
	/*extern*/ const FText DoesNotIntersect = LOCTEXT("StateDoesNotIntersect", "Does not intersect"); //~ The widget rect is not in the correct direction or is not intersecting with the "swept" rectangle
	/*extern*/ const FText PreviousWidgetIsBetter = LOCTEXT("StatePreviousWidgetIsBetter", "Previous Widget was better"); //~ The widget would be valid but the previous valid is closer
	/*extern*/ const FText NotADescendant = LOCTEXT("StateNotADescendant", "Not a descendant"); //~ We have a non escape boundary condition and the widget isn't a descendant of our boundary
	/*extern*/ const FText Disabled = LOCTEXT("StateNotEnabled", "Disabled"); //~ The widget is not enabled
	/*extern*/ const FText ParentDisabled = LOCTEXT("StateParentNotEnabled", "ParentDisabled"); //~ A parent of the widget is disabled
	/*extern*/ const FText DoesNotSuportKeyboardFocus = LOCTEXT("StateDoesNotSuportKeyboardFocus", "Keyboard focus unsupported"); //~ THe widget does not support keyboard focus
}
	...
	while (CurrentCellPoint[AxisIndex] >= 0 && CurrentCellPoint[AxisIndex] < NumCells[AxisIndex])
	{
		...
		for (StrideCellPoint[StrideAxis] = StrideAxisMin; StrideCellPoint[StrideAxis] <= StrideAxisMax; ++StrideCellPoint[StrideAxis])
		{
			...
			for (int32 i = WidgetIndexes.Num() - 1; i >= 0; --i)
			{
				...
				{
					const float BestDestSide = DestSideFunc(BestWidgetRect);
					const float TestDestSide = DestSideFunc(TestCandidateRect);

					// If the test candidate is further on the navigation axis, keep the current best.
					if (!CompareFunc(BestDestSide, TestDestSide))
					{
						AddToNextFocusableWidgetCondidateDebugResults(TestWidget, HittestGridDebuggingText::PreviousWidgetIsBetter);
						continue;
					}

					// Equidistant on the navigation axis: prefer the candidate whose center
					// is closest to the source center.
					if (CompareFunc(TestDestSide, BestDestSide))
					{
						const float BestDistSq = FVector2f::DistSquared(WidgetRect.GetCenter(), BestWidgetRect.GetCenter());
						const float TestDistSq = FVector2f::DistSquared(WidgetRect.GetCenter(), TestCandidateRect.GetCenter());
...
FSlateRect FHittestGrid::GetWidgetRenderBoundingRect(const TSharedPtr<SWidget>& Widget) const
{
	...
	return Widget->GetHitTestBoundingRect().OffsetBy(-GridWindowOrigin);
}
...
void FHittestGrid::SetPaintToHitTestTransform(const FSlateRenderTransform& InTransform)
{
	PaintToHitTestTransform = InTransform;
	if (PaintToHitTestTransform.IsSet() && PaintToHitTestTransform.GetValue().IsIdentity())
	{
		PaintToHitTestTransform = TOptional<FSlateRenderTransform>();
	}
}
...
void FHittestGrid::AddWidget(const SWidget* InWidget, int32 InBatchPriorityGroup, int32 InLayerId, FSlateInvalidationWidgetSortOrder InSecondarySort)
{
	...
	FSlateRect BoundingRect = GridSpaceGeometry.GetRenderBoundingRect();

	if (PaintToHitTestTransform.IsSet())
	{
		// Ensure the cells check correspond to the hittest to paint remapping
		const FSlateRotatedRect WindowOrientedClipRect = TransformRect(
			Concatenate(
				Inverse(GridSpaceGeometry.GetAccumulatedLayoutTransform()),
				Concatenate(GridSpaceGeometry.GetAccumulatedRenderTransform(), PaintToHitTestTransform.GetValue())),
			FSlateRotatedRect(GridSpaceGeometry.GetLayoutBoundingRect())
		);
		BoundingRect = WindowOrientedClipRect.ToBoundingRect();
	}
		...
		for (int32 i = WidgetIndexes.Num() - 1; i >= 0; --i)
		{
			...
			if (bIsValidWidget)
			{
				...
				if (bPointInsideClipMasks)
				{
					...
					FSlateRotatedRect WindowOrientedClipRect;
					if (WidgetIndexes[i].GetGrid()->PaintToHitTestTransform.IsSet())
					{
						// If the hittestgrid contains an additional hittest rotation, apply this.
						const FTransform2D GridPaintToHitTestTransform = WidgetIndexes[i].GetGrid()->PaintToHitTestTransform.GetValue();
						WindowOrientedClipRect = TransformRect(
							Concatenate(
								Inverse(TestGeometry.GetAccumulatedLayoutTransform()),
								Concatenate(TestGeometry.GetAccumulatedRenderTransform(), GridPaintToHitTestTransform)),
...
void FHittestGrid::LogGrid() const
{
	...
	UE_LOGF(LogHittestDebug, Warning, "\n%ls", *TempString);
	...
	for (TSparseArray<FWidgetData>::TConstIterator It(WidgetArray); It; ++It)
	{
		...
		UE_LOGF(LogHittestDebug, Warning, "  [%d][%lld] => %ls @ %ls",
			It.GetIndex(),
		...
		for (TSparseArray<FWidgetData>::TConstIterator It(HittestGrid->WidgetArray); It; ++It)
		{
			...
			if (ensure(CachedWidget.IsValid())) // Widget should always be valid
			{
				...
				FSlateDrawElement::MakeBox(
					...
					GetUserColorFromIndex(CurWidgetData.UserIndex).CopyWithNewOpacity(.005f)
				);
			}
		}
	};
