...
UObject* FSlateSound::GetResourceObject( ) const
{
	...
	if (LegacyResourceName_DEPRECATED != NAME_None)
	{
		...
		if (!LegacyResourceObject)
		{
			if (!IsInGameThread())
			{
				UE_LOGF(LogSlate, Warning, "Can't find/load sound %ls because Slate is being updated in another thread! (loading screen?)", *LegacyResourceName_DEPRECATED.ToString());
			}
			else
			{
