 ...
 * If you need to update this enum, keep it in sync with the LexToString overload below.
 */
...
SLATECORE_API const TCHAR* LexToString(ESlateTextureAtlasThreadId ThreadId);
...
/**
 * Convenience function to get the correct atlas thread ID based on the thread we're currently in as a string.
 */
SLATECORE_API const TCHAR* GetCurrentSlateTextureAtlasThreadIdAsString();
...
{
public:
	virtual ~ISlateTextureAtlasFactory() = default;
...
public:
	...
	virtual ~FSlateFlushableAtlasCache() = default;
...
public:
	/** Virtual destructor */
	virtual ~ISlateAtlasProvider() = default;
