...
public:
	...
	/** @return if the InvalidationRoot is currently updating the attributes. */
	bool IsProcessingAttributeUpdate() const { return bProcessingAttributeUpdate; }
	...
	/**
	 * @return if the InvalidationRoot can cache this frame. This can return false for a Dynamic Invalidation Panel
	 * not supporting invalidation, or a disabled Invalidation Panel.
	 */
	virtual bool CanCacheThisFrame() const { return true; };
...
private:
	...
	/**
	 * If set, HandleInvalidateAllWidgets will be called at the end of ProcessInvalidation when it is safe to do so.
	 * The value of this variable will be passed as the `bClearResourcesImmediately` parameter for HandleInvalidateAllWidgets
	 */
	TOptional<bool> bDeferInvalidateAllWidgetsAndClearResources;
