...
public:
	 ...
	 */
	SLATECORE_API FNavigationReply OnNavigation(const FGeometry& MyGeometry, const EUINavigation Type);
...
public:
	...
	/** Update all the Slate Attributes of this widget. */
	void UpdateAllAttributes()
	{
		FSlateAttributeMetaData::UpdateAllAttributes(*this, FSlateAttributeMetaData::EInvalidationPermission::AllowInvalidation);
	}
	...
	/**
	 * @return the SlateRect that FHittestGrid would use to test for hit bounds
	 */
	SLATECORE_API virtual FSlateRect GetHitTestBoundingRect() const;
	...
	/** Whether the widget is a part of the project's content (as opposed to editor, chrome, diagnostic output).
	 * Dynamic: updated during pre-pass, inherited from parent but may be overwritten in CustomPrepass.
	 * Used to filter which widgets can be navigated to, which are considered by diagnostic tools.
	 */
...
protected:
	...
	/**
	 * Call from CustomPrepass to change whether this widget and its children are a part of the project's content (as opposed to editor, chrome,
	 * diagnostic output).
	 */
	void SetIsProjectContent(bool bInIsProjectContent)
	{
		bIsProjectContent = bInIsProjectContent;
	}

	/**
	 * Call from CustomPrepass to change whether this widget's children are a part of the project's content.
	 */
	void SetIsProjectContentParent(bool bInIsProjectContent)
	{
		bIsProjectContentParent = bInIsProjectContent;
	}
...
public:
	...
	/**
	 * Checks is this widget can ever support keyboard focus.
	 *
	 * @return  True if this widget can ever support keyboard focus
	 */
...
public:
	...
	/** Sets a custom repel zone for the tooltip, bToolTipForceFieldEnabled must be true */
	SLATECORE_API void SetToolTipForceFieldExpansion(const TAttribute<TOptional<FSlateRect>>& InExpansionRect);
	
	/** @return FSlateRect that combines all FSlateToolTipForceFieldExpansionMetaData attribute values */
	SLATECORE_API TOptional<FSlateRect> GetToolTipForceFieldExpansion() const;
...
public:
	...
	int32 RemoveMetaData(const TSharedRef<MetaDataType>& RemoveMe)
	{
		...
		OnRemovedMetadata(RemoveMe);
		return 1;
	}
	
	/**
	 * Finds or creates the metadata based on the type provided.
	 * @return the first metadata of the type supplied that we encounter or a newly created one
	 */
    template<typename MetaDataType>
    TSharedRef<MetaDataType> FindOrAddMetaData()
    {
    	if (TSharedPtr<MetaDataType> FoundMetaData = GetMetaData<MetaDataType>())
    	{
    		return FoundMetaData.ToSharedRef();
    	}

    	TSharedRef<MetaDataType> NewMetaData = MakeShared<MetaDataType>();
    	AddMetadataInternal(NewMetaData);
    	return NewMetaData;
    }

protected:
	virtual void OnAddedMetadata(const TSharedRef<ISlateMetaData>& AddedMetaData)
	{
	}

	virtual void OnRemovedMetadata(const TSharedRef<ISlateMetaData>& RemovedMetaData)
	{
	}
	...
	int32 RemoveAllMetaData()
	{
		...
		for (int32 Index = NumBefore - 1; Index >= 0; --Index)
		{
			...
			if (MetaDataEntry->IsOfType<MetaDataType>())
			{
				...
				OnRemovedMetadata(MetaDataEntry);
			}
		}
...
public:
	...
	virtual bool IsTableView() const { return false; }
	...
	/** Returns true if this widget's class supports invalidation. This value never changes. */
	SLATECORE_API bool SupportsInvalidation() const;

	enum class EInvalidationStrategy : uint8
	{
		/** Use the last cached value unless it needs refreshing. */
		UseCachedValue,
		/** Check again if this widget and its direct children support invalidation. */
		RecomputeSelf,
	};

	enum class ECachedInvalidationResult : uint8
	{
		/** The last result has been invalidated and needs to be refreshed. This will happen in the next call to SupportsInvalidationRecursive. */
		NeedsRefresh,
		 ...
		 * It is likely the case that this widget has been reused in multiple slots which is not allowed with invalidation.
		 */
		InvalidParent,
	};
	 ...
	 * This value is cached, and will be reused if InInvalidationStrategy is set to UseCachedValue.
	 */
	SLATECORE_API virtual bool SupportsInvalidationRecursive(EInvalidationStrategy InInvalidationStrategy) const;
private:
	/** Fast path to invalidate all ancestors when a descendant widget is not supporting invalidation. */
	void SetSelfAndParentsAsNotSupportingInvalidation();

	/** Sets this widget and its ancestors as needing to have their SupportsInvalidationRecursive refreshed. */
	void InvalidateCachedSupportsInvalidation();

	/** Store the value from the TWidgetTypeTraits<>::SupportsInvalidation just after the widget is created */
	SLATECORE_API void SetClassSupportsInvalidation(bool bClassSupportsInvalidation);

public:
	ECachedInvalidationResult Debug_GetCachedInvalidationResult() const { return CachedSupportsInvalidationRecursive; }
...
private:
	...
	uint8 bIsProjectContent : 1;
	uint8 bIsProjectContentParent : 1;
...
protected:
	...
	mutable ECachedInvalidationResult CachedSupportsInvalidationRecursive = ECachedInvalidationResult::ChildrenNotSupportingInvalidation;
#endif
...
inline const TCHAR* LexToString(SWidget::EInvalidationStrategy Strategy)
{
#define INVALIDATION_STRATEGY_LEXTOSTRING(Name) case SWidget::EInvalidationStrategy::Name: return TEXT(#Name);
	switch (Strategy)
	{
		INVALIDATION_STRATEGY_LEXTOSTRING(UseCachedValue)
		INVALIDATION_STRATEGY_LEXTOSTRING(RecomputeSelf)
	default:
		return TEXT("Unknown");
	}
#undef INVALIDATION_STRATEGY_LEXTOSTRING
}

inline const TCHAR* LexToString(SWidget::ECachedInvalidationResult Result)
{
#define INVALIDATION_RESULT_LEXTOSTRING(Name) case SWidget::ECachedInvalidationResult::Name: return TEXT(#Name);
	switch (Result)
	{
		INVALIDATION_RESULT_LEXTOSTRING(NeedsRefresh)
		INVALIDATION_RESULT_LEXTOSTRING(Refreshing)
		INVALIDATION_RESULT_LEXTOSTRING(SupportsInvalidation)
		INVALIDATION_RESULT_LEXTOSTRING(ClassNotSupportingInvalidation)
		INVALIDATION_RESULT_LEXTOSTRING(ChildrenNotSupportingInvalidation)
		INVALIDATION_RESULT_LEXTOSTRING(InvalidParent)
	default:
		return TEXT("Unknown");
	}
#undef INVALIDATION_RESULT_LEXTOSTRING
}
