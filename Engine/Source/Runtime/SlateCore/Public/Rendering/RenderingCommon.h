...
enum class ESlateShader : uint8
{
	...
	MsdfFont = 9,
	/** Dynamic branching version of all the shader types above */
	Dynamic = 10,
};
...
{
	/**
	 * ISlateViewport::Paint must return this interface.  By default, it is a no-op.  If a Slate Viewport needs to establish some global state while
	 * its host SWidget and while that SWidget's child widget(s) are painted, then it should clean up that state in the destructor of the PaintScope
	 * that it returns.
	 */
	class FPaintScope
	{
	public:
		virtual ~FPaintScope() = default;
	};
}
...
public:
	 ...
	 */
	UE_DEPRECATED(5.8, "Override Paint instead; perform any teardown in the returned scope's destructor so it runs after child widgets paint.")
	virtual void OnDrawViewport( const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, class FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) final { }

	/**
	 * Called by SViewport at the start of its OnPaint. The returned scope object lives until SViewport::OnPaint
	 * returns.
	 */
	virtual TUniquePtr<UE::Slate::FPaintScope> Paint(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, const int32 LayerId, const FWidgetStyle& InWidgetStyle, const bool bParentEnabled)
	{
		return nullptr;
	}
	...
	/**
	* Does this frame identify as a Camera Cut ?
	*/
	virtual bool IsCameraCut() const
	{
		return false;
	}
...
{
public:
	virtual ~ICustomSlateElement() = default;
...
{
protected:
	virtual ~ISlateUpdatableInstanceBufferRenderProxy() = default;
...
{
public:
	virtual ~ISlateUpdatableInstanceBuffer() = default;
