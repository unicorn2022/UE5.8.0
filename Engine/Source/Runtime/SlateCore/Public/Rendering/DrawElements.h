...
public:
	...
	UE_DEPRECATED(5.8, "Use SetIsProjectContent from CustomPrepass instead")
	SLATECORE_API void SetIsInGameLayer(bool bInGameLayer);

	UE_DEPRECATED(5.8, "Renamed to IsInProjectContent")
	SLATECORE_API bool GetIsInGameLayer();

	void SetIsInProjectContent(bool bIsInProjectContent);

	/** Whether the stack frame is drawing the project's content (as opposed to editor, chrome, diagnostic output). */
	bool IsInProjectContent() const
	{
...
private:
	...
	*/
	...
	/** Caches whether the current widget constitutes "content" (as opposed to editor, chrome, diagnostic output). */
	bool bIsProjectContent;
