...
struct FWindowStyle : public FSlateWidgetStyle
{
	...
	static SLATECORE_API const FWindowStyle& GetBorderless();
...
/** A structure used to help the user identify deprecated FInvalidatableBrushAttribute that are now TSlateAttribute. */
class FDeprecatedInvalidatableBrushAttribute
{
public:
	FDeprecatedInvalidatableBrushAttribute() { }
	FDeprecatedInvalidatableBrushAttribute(const TAttribute< const FSlateBrush* >& InImage) {}

	bool IsBound() const { return false; }
	const FSlateBrush* Get() const { return nullptr; }
	TAttribute< const FSlateBrush* > GetImage() const { return {}; }
	void SetImage(SWidget& ThisWidget, const TAttribute< const FSlateBrush* >& InImage) {}
};

/**
 * Class similar to FInvalidatableBrushAttribute but to be used with TSlateAttribute.
 * Note that this class does not invalidate on value change unless used inside a TSlateAttribute
 */
class FSlateBrushCached
{
public:
	FSlateBrushCached() : Image(nullptr), ImageCache(FSlateBrush()) {}
	FSlateBrushCached(const FSlateBrush* InImage)
		: Image(InImage), ImageCache(InImage ? *InImage : FSlateBrush())
	{
	}
	...
	{
		Image = InImage;
		ImageCache = InImage ? *InImage : FSlateBrush();
	}
	
	bool operator==(const FSlateBrushCached& Rhs) const
	{
		return Image == Rhs.Image;
	}
	bool operator==(const FSlateBrush* Rhs) const
	{
		if (Image == Rhs)
		{
			const FSlateBrush NewImageToCache = Image ? *Image : FSlateBrush();
			return ImageCache == NewImageToCache;
		}
		return false;
	}
private:
	/** The slate brush to draw for the image. */
