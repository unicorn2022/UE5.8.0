...
public:  
	...
	const FSlateWidgetStyle* GetStyleChecked( const FName DesiredTypeName ) const 
	{
		if ( CustomStyle == nullptr )
		{
			UE_LOGF( LogSlateStyle, Error, "USlateWidgetStyleAsset::GetStyle : No custom style set for '%ls'.", *GetPathName() );
			return nullptr;
		}
		...
		if ( Style == nullptr )
		{
			UE_LOGF( LogSlateStyle, Error, "USlateWidgetStyleAsset::GetStyle : No style found in custom style set for '%ls'.", *GetPathName() );
			return nullptr;
		}

		if ( Style->GetTypeName() != DesiredTypeName )
		{
			UE_LOGF( LogSlateStyle, Error, "USlateWidgetStyleAsset::GetStyle : The custom style is not of the desired type. Desired: '%ls', Actual: '%ls'", *DesiredTypeName.ToString(), *Style->GetTypeName().ToString() );
			return nullptr;
		}
