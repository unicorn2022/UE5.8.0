...
public:
	...
	FPointerEvent()
		...
		, GesturePhase(EGesturePhase::None)
	...
	FPointerEvent(
		...
		const UE::Slate::FDeprecateVector2DParameter& InLastScreenSpacePosition,
		const float InForce,
		const bool bPressLeftMouseButton,
		const bool bInIsForceChanged,
		const bool bInIsFirstMove,
		const FModifierKeysState& InModifierKeys,
		const uint32 InTouchpadIndex
	)
		: FInputEvent(InModifierKeys, InUserIndex, false)
		, ScreenSpacePosition(InScreenSpacePosition)
		, LastScreenSpacePosition(InLastScreenSpacePosition)
	...
	FPointerEvent(
		...
		const float InForce,
		const bool bPressLeftMouseButton,
		const bool bInIsForceChanged = false,
		const bool bInIsFirstMove = false,
	...
	/** A constructor for gesture events with a specific input device */
	FPointerEvent(
		FInputDeviceId InDeviceId,
		const UE::Slate::FDeprecateVector2DParameter& InScreenSpacePosition,
		const UE::Slate::FDeprecateVector2DParameter& InLastScreenSpacePosition,
...
public:
	...
	EGesturePhase GetGesturePhase() const { return GesturePhase; }
