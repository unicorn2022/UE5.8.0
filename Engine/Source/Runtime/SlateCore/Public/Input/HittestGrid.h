...
{
	extern SLATECORE_API const FText Valid;
	extern SLATECORE_API const FText PreviousWidgetIsBetter;
	extern SLATECORE_API const FText NotCompatibleWithUserIndex;
	extern SLATECORE_API const FText DoesNotIntersect;
	extern SLATECORE_API const FText NotADescendant;
	extern SLATECORE_API const FText Disabled;
	extern SLATECORE_API const FText ParentDisabled;
	extern SLATECORE_API const FText DoesNotSuportKeyboardFocus;
};
...
{
public:
	virtual ~ICustomHitTestPath()= default;
...
public:
	 ...
	 * and return the path along which the corresponding event would be bubbled, optionally filtering
	 * the hittest to widgets in a range of layers.
	 */
	SLATECORE_API TArray<FWidgetAndPointer> GetBubblePath(UE::Slate::FDeprecateVector2DParameter DesktopSpaceCoordinate, float CursorRadius, bool bIgnoreEnabledStatus, int32 UserIndex = INDEX_NONE, TInterval<int32> LayerRange = TInterval<int32>(std::numeric_limits<int32>::min(), std::numeric_limits<int32>::max()));
	...
	SLATECORE_API void SetPaintToHitTestTransform(const FSlateRenderTransform& InTransform);
...
private:
	...
	bool IsWidgetDataInLayerRange(const FWidgetData& WidgetData, const TInterval<int32>& LayerRange);
...
private:
	...
	/** Optional delta transform applied during hit-testing.
		Used by RetainerWidget to reconcile hittest space with the child’s paint space
		when the retainer is non-grid aligned (i.e rotation) */
	TOptional<FSlateRenderTransform> PaintToHitTestTransform;
