 ...
 *
 * If bManagedByWidget is false, the children will not be attached or detached from the parent and no invalidation will be triggerred
 */
template<typename ChildType, bool bManagedByWidget = true>
...
public:
	TSlotlessChildren(SWidget* InOwner, bool InbChangesInvalidatePrepass = true) requires (bManagedByWidget)
		: FChildren(InOwner)
		, bChangesInvalidatePrepass(InbChangesInvalidatePrepass)
	{
	}

	TSlotlessChildren(SWidget* InOwner) requires (!bManagedByWidget)
		: FChildren(InOwner)
		, bChangesInvalidatePrepass(false)
	{
	}
...
template <typename ChildType>
using TUnmanagedSlotlessChildren = TSlotlessChildren<ChildType, false>;


/** Required to implement GetChildren() in a way that can dynamically return the currently active child. */
template<typename SlotType, typename Attr>
class TOneDynamicChildBase final : public FChildren
{
public:
	TOneDynamicChildBase(SWidget* InOwner, TPanelChildren<SlotType>* InAllChildren, const Attr* InWidgetIndex)
		: FChildren(InOwner)
	...
	TOneDynamicChildBase(SWidget* InOwner, TPanelChildren<SlotType>* InAllChildren, std::nullptr_t) = delete;
	TOneDynamicChildBase(SWidget* InOwner, std::nullptr_t, const Attr* InWidgetIndex) = delete;
	TOneDynamicChildBase(SWidget* InOwner, std::nullptr_t, std::nullptr_t) = delete;
	TOneDynamicChildBase(std::nullptr_t, TPanelChildren<SlotType>* InAllChildren, std::nullptr_t) = delete;
	TOneDynamicChildBase(std::nullptr_t, std::nullptr_t, const Attr* InWidgetIndex) = delete;
	TOneDynamicChildBase(std::nullptr_t, std::nullptr_t, std::nullptr_t) = delete;
...
private:
	...
	const Attr* WidgetIndex;
};

template <typename SlotType>
using TOneDynamicChild = TOneDynamicChildBase<SlotType, TAttribute<int32>>;
