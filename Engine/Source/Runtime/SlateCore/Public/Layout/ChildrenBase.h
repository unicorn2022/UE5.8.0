...
protected:
	...
	friend class TOneDynamicChildBase;
...
protected:
	...
	virtual ~FChildren()
	{
		...
		UE_CLOGF(Debug_bIsIteratingChildren, LogSlate, Fatal,
			"Destroying widget while iterating children! Owner: %ls [%ls]",
	...
	void Debug_TestDestroyTag() const
	{
		...
		UE_CLOGF(Debug_DestroyedTag != 0xDC, LogSlate, Fatal, "The FChildren is destroyed. You probably have 1 widget owned by 2 different FChildren.");
	}
