...
public:
	 ...
	 * See StandardizeCaseInPlace() for an in-place equivalent.
	 */
	[[nodiscard]] static JSONUTILITIES_API FString StandardizeCase(const FString &StringIn);

	static JSONUTILITIES_API void StandardizeCaseInPlace(FString& String);
...
public: // UStruct -> JSON
	...
	static bool UStructToFormattedJsonObjectString(const UStruct* StructDefinition, const void* Struct, FString& OutJsonString, int64 CheckFlags = 0, int64 SkipFlags = 0, int32 Indent = 0, const CustomExportCallback* ExportCb = nullptr, EJsonObjectConversionFlags ConversionFlags = EJsonObjectConversionFlags::None)
	{
		...
		if (UStructToJsonObject(StructDefinition, Struct, JsonObject, CheckFlags, SkipFlags, ExportCb, ConversionFlags))
		{
			...
			{
				UE_LOGF(LogJson, Warning, "UStructToFormattedObjectString - Unable to write out json");
				JsonWriter->Close();
			}
		}
	 ...
	 * @return False if any properties failed to write
	 */
	static JSONUTILITIES_API bool UStructToJsonAttributes(const UStruct* StructDefinition, const void* Struct, TSharedRef<FJsonObject> OutJsonObject, int64 CheckFlags = 0, int64 SkipFlags = 0, const CustomExportCallback* ExportCb = nullptr, EJsonObjectConversionFlags ConversionFlags = EJsonObjectConversionFlags::None);
...
public: // JSON -> UStruct
	 ...
	 * @return False if any properties matched but failed to deserialize
	 */
	static JSONUTILITIES_API bool JsonAttributesToUStruct(const TMap<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& JsonAttributes, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags = 0, int64 SkipFlags = 0, const bool bStrictMode = false, FText* OutFailReason = nullptr, const CustomImportCallback* ImportCb = nullptr);
	...
	static bool JsonObjectStringToUStruct(const FString& JsonString, OutStructType* OutStruct, int64 CheckFlags = 0, int64 SkipFlags = 0, const bool bStrictMode = false, FText* OutFailReason = nullptr, const CustomImportCallback* ImportCb = nullptr)
	{
		...
		{
			UE_LOGF(LogJson, Warning, "JsonObjectStringToUStruct - Unable to parse. json=[%ls]", *JsonString);
			if (OutFailReason)
			{
		...
		{
			UE_LOGF(LogJson, Warning, "JsonObjectStringToUStruct - Unable to deserialize. json=[%ls]", *JsonString);
			if (OutFailReason)
			{
	...
	static bool JsonArrayStringToUStruct(const FString& JsonString, TArray<OutStructType>* OutStructArray, int64 CheckFlags = 0, int64 SkipFlags = 0, const bool bStrictMode = false, FText* OutFailReason = nullptr, const CustomImportCallback* ImportCb = nullptr)
	{
		...
		{
			UE_LOGF(LogJson, Warning, "JsonArrayStringToUStruct - Unable to parse. json=[%ls]", *JsonString);
			if (OutFailReason)
			{
		...
		{
			UE_LOGF(LogJson, Warning, "JsonArrayStringToUStruct - Error parsing one of the elements. json=[%ls]", *JsonString);
			if (OutFailReason)
			{
	...
	static bool JsonArrayToUStruct(const TArray<TSharedPtr<FJsonValue>>& JsonArray, TArray<OutStructType>* OutStructArray, int64 CheckFlags = 0, int64 SkipFlags = 0, const bool bStrictMode = false, FText* OutFailReason = nullptr, const CustomImportCallback* ImportCb = nullptr)
	{
		...
		for (int32 i = 0; i < JsonArray.Num(); ++i)
		{
			...
			{
				UE_LOGF(LogJson, Warning, "JsonArrayToUStruct - Array element [%i] was not an object.", i);
				if (OutFailReason)
				{
			...
			{
				UE_LOGF(LogJson, Warning, "JsonArrayToUStruct - Unable to convert element [%i].", i);
				if (OutFailReason)
				{
