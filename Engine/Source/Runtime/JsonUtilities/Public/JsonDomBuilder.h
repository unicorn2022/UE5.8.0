...
public:
	...
	public:
		...
		template <template <class> class TPrintPolicy = TPrettyJsonPrintPolicy>
		FUtf8String ToUtf8String() const
		{
			FUtf8String Result;
			auto JsonWriter = TJsonWriterFactory<UTF8CHAR, TPrintPolicy<UTF8CHAR>>::Create(&Result);
			FJsonSerializer::Serialize(Object, JsonWriter);
			return Result;
		}
		...
		template <class StringType
			UE_REQUIRES(std::is_constructible_v<FString, StringType>)>
		FObject& Set(const FString& Key, const StringType& Str)        { Object->SetField(Key, MakeShared<FJsonValueString>(FString(Str)));   return *this; }
		...
		void CopyIf(const FJsonObject& Src, TFunctionRef<bool (const FJsonObject::FStringType&, const FJsonValue&)> Pred)
		{
			for (const TPair<FJsonObject::FStringType, TSharedPtr<FJsonValue>>& KV: Src.Values)
			{
	...
	public:
		...
		template <class StringType
			UE_REQUIRES(std::is_constructible_v<FString, StringType>)>
		FArray& Add(const StringType& Str)        { Array.Emplace(MakeShared<FJsonValueString>(FString(Str))); return *this; }

		template <class NumberType
			UE_REQUIRES(!std::is_same_v<NumberType, bool> && (std::is_integral_v<NumberType> || std::is_floating_point_v<NumberType>))>
		FArray& Add(NumberType Number)            { Array.Emplace(MakeShared<FJsonValueNumber>(Number));   return *this; }

		template <class BoolType
			UE_REQUIRES(std::is_same_v<BoolType, bool>)>
		FArray& Add(BoolType Boolean)             { Array.Emplace(MakeShared<FJsonValueBoolean>(Boolean)); return *this; }
