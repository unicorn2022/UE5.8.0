...
namespace Audio
{
		...
		if (!AudioCapture.IsStreamOpen())
		{
			FOnAudioCaptureFunction OnCapture = [this](const void* AudioData, int32 NumFrames, int32 NumChannels, int32 SampleRate, double StreamTime, bool bOverFlow)
			{
				...
				if (bIsCapturing)
				{
					...
					{
						UE_LOGF(LogAudioCaptureCore, Warning, "Attempt to write past end of buffer in OpenDefaultStream [%u]", AudioCaptureData.Num() + NumSamples);
					}
				}
			};
			...
			if (AudioCapture.OpenAudioCaptureStream(Params, MoveTemp(OnCapture), 1024))
			{
				...
				{
					UE_LOGF(LogAudioCaptureCore, Error, "FAudioCaptureSynth::OpenDefaultStream: Failed to start audio capture stream.");
	...
	bool FAudioCaptureSynth::GetAudioData(TArray<float>& OutAudioData)
	{
		...
		if (CaptureDataSamples > 0)
		{
			...
			{
				UE_LOGF(LogAudioCaptureCore, Warning, "Attempt to write past end of buffer in GetAudioData");
				return false;
			}
