...
namespace CEF3Utils
{
	...
	void* LoadDllCEF(const FString& Path)
	{
		...
		if (!Handle)
		{
			...
			UE_LOGF(LogCEF3Utils, Error, "Failed to get CEF3 DLL handle for %ls: %ls (%d)", *Path, ErrorMsg, ErrorNum);
		}
		return Handle;
	}
	...
	bool LoadCEF3Modules(bool bIsMainApp)
	{
	...
	#else
		DllPath = FPaths::Combine(*FPaths::EngineDir(), TEXT("Binaries/ThirdParty/CEF3/Win64"), CEF_VERSION_DIR CEF_VERSION_SUFFIX);
	#endif
		...
		if (bIsMainApp)
		{
			...
			{
				UE_LOGF(LogCEF3Utils, Error, "Chromium loader initialization failed");
			}
		}
		else
		{
			...
			if (!bLoaderInitialized)
			{
				UE_LOGF(LogCEF3Utils, Error, "Chromium helper loader initialization failed");
			}
		}
	...
	void BackupCEF3Logfile(const FString& LogFilePath)
	{
		...
		if (FileManager.FileSize(*Cef3LogFile) > 0) // file exists and is not empty
		{
			...
			{
				UE_LOGF(LogCEF3Utils, Warning, "Failed to backup cef3.log");
			}
		}
	}
};
