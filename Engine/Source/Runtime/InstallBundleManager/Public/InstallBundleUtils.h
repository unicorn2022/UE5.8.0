...
namespace InstallBundleUtil
{
	...
	INSTALLBUNDLEMANAGER_API const FString& GetInstallBundleSubsetSectionPrefix();
	...
	// Returns an ordered regex list used to map files to bundle subsets
	INSTALLBUNDLEMANAGER_API TArray<TPair<FString, TArray<FRegexPattern>>> LoadBundleSubsetRegexFromConfig(
		const FConfigFile& InstallBundleConfig);
	...
	// Return true for a matching install bundle subset for a given path
	INSTALLBUNDLEMANAGER_API bool MatchBundleSubsetRegex(
	...
	namespace PersistentStats
	{
		...
		public:
			...
			virtual ~FBundlePersistentStats() = default;
		...
		public:
			...
			virtual ~FSessionPersistentStats() = default;
	...
	{
		virtual ~IBundleSourceContentRequestSharedContext() = default;
	};
	...
	struct FContentRequestSharedContext
	{
		...
		// (Optional) - can be used to pass a list of requested bundles to IInstallBundleSource implementations
		TArray<FName> RequestedBundles;	
	};
