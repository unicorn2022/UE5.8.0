...
enum class EInstallBundleRequestFlags : uint32
{
	...
	SkipMount										= (1 << 5),
	SkipOnDemandTocMount							= (1 << 6),
	AsyncMount										= (1 << 7),
	UpdateBundleReports								= (1 << 8), // will update the BundleReport for the involved bundles (see InstallBundleManagerReporting.h and IInstallBundleManager::ReportingDelegate)
	ExplicitUpdateList								= (1 << 9),
	CheckForLimitedDataUsage						= (1 << 10),
	Defaults										= None,
};
...
// TODO: These details shouldn't be in engine code. If you find yourself adding to this struct, please consider refactoring this 
// to pass back a list of FAnalyticsEventAttribute's instead.
struct FBundleDecompressionInfo
{
	uint64 TotalCompressedBytes{};				// total amount of bytes in compressed files (incl headers)
	uint64 TotalDecompressedBytes{};			// total amount of bytes in decompressed files (incl headers)
	uint64 TotalActuallyReadBytes{};			// total amount of compressed bytes actually read this session (excl headers)
	uint64 TotalActuallyWrittenBytes{};			// total amount of decompressed bytes actually written this session (excl headers)
	double TotalDecompressionReadTime{};		// seconds spent reading compressed bytes this session
	double TotalDecompressionDecompressTime{};	// seconds spent decompressing this session
	double TotalDecompressionWriteTime{};		// seconds spent writing decompressed bytes this session
};
...
struct FBundleRequestCompleteInfo
{
	...
	FString DownloadType;
	FBundleDecompressionInfo BundleDecompressionInfo;
	double AsyncStartupDelay{};
	double AsyncExitDelay{};
	bool AsyncPlacedInBackground{};
};
