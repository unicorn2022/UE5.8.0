...
public:
	...
	virtual ~IInstallBundleManager() = default;
	...
	// Note: Some bundles sources may call this from a background task, so it should be threadsafe
	virtual int32 SelectBundleSubset(const FString& BundleName, TConstArrayView<FString> BundleSubsets) const { return INDEX_NONE; }
	...
	virtual EInstallBundleResult GetAllMountedBundles(TArray<FName>& OutBundleNames) const = 0;
	virtual EInstallBundleResult GetAllBundlesWithState(EInstallBundleInstallState InstallState, TArray<FName>& OutBundleNames) const = 0;
