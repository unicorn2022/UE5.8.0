...
{
public:
	virtual ~IInstallBundleSource() = default;
	...
	// Currently only CheckForCellularDataUsage/CheckForLimitedDataUsage are supported
	...
	virtual void SetLimitedDataPreference(int32 Value) {}
	...
	// Called on the game thread whenever a new analytics provider is to be used than the one passed to Init()
	virtual void SetAnalyticsProvider(TSharedPtr<IAnalyticsProviderET> AnalyticsProvider) = 0;
