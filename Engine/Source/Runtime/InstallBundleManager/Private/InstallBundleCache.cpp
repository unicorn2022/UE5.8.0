...
FInstallBundleCacheStats FInstallBundleCache::GetStats(EInstallBundleCacheStatsFlags Flags /*= EInstallBundleCacheStatsFlags::None*/, bool bVerbose /*= false*/) const
{
	...
	if (bDumpToLog)
	{
		UE_LOGF(LogInstallBundleManager, Display, "");
		UE_LOGF(LogInstallBundleManager, Display, "* Install Bundle Cache Stats %ls", *CacheName.ToString());
...
#define INSTALLBUNDLECACHE_DEFAULT_LOG(Verbosity) \
	...
	{ \
		UE_LOGF(LogInstallBundleManager, Verbosity, "* \tbundle %ls", *BundleName.ToString()) \
		...
		UE_LOGF(LogInstallBundleManager, Verbosity, "* \t\treserved: %ls", (Info.State == ECacheState::Reserved) ? TEXT("true") : TEXT("false")) \
		UE_LOGF(LogInstallBundleManager, Verbosity, "* \t\ttimestamp: %ls", *Info.TimeStamp.ToString()) \
		UE_LOGF(LogInstallBundleManager, Verbosity, "* \t\tage scale: %f", Info.AgeScalar) \
	}

#define INSTALLBUNDLECACHE_CSV_HEADER_LOG(Verbosity) \
	UE_LOGF(LogInstallBundleManager, Verbosity, "* \tbundle, full size, overhead size, current size, diff, reserved, timestamp, age scale")
