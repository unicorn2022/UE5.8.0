...
namespace Audio
{
	...
	public:
		...
		virtual FName GetDefaultOutputDevice(EAudioDeviceRole InRole) const = 0;
		...
		/** Mark a device inactive after a caller observed it unreachable at the OS level. */
		virtual void InvalidateDevice(FName InDeviceId) = 0;
	};
	...
	struct FAudioOutputStreamInfo
	{
		...
		~FAudioOutputStreamInfo() = default;
	...
	public: // Virtual functions
		...
		/** Protects device swap related resources during device swaps */
		virtual void LockDeviceSwap() {}

		/** Unprotects device swap resources. Balance with LockDeviceSwap() */
		virtual void UnlockDeviceSwap() {}
	...
	protected:
			...
			if (!LogHistory.Contains(Hash))
			{
				switch (InVerbosity)
				{
					case ELogVerbosity::Fatal:
						UE_LOGF(LogAudioMixer, Fatal, "%ls", *Message);
						break;

					case ELogVerbosity::Error:
						UE_LOGF(LogAudioMixer, Error, "%ls", *Message);
						break;

					case ELogVerbosity::Warning:
						UE_LOGF(LogAudioMixer, Warning, "%ls", *Message);
						break;

					case ELogVerbosity::Display:
						UE_LOGF(LogAudioMixer, Display, "%ls", *Message);
						break;

					case ELogVerbosity::Log:
						UE_LOGF(LogAudioMixer, Log, "%ls", *Message);
						break;

					case ELogVerbosity::Verbose:
						UE_LOGF(LogAudioMixer, Verbose, "%ls", *Message);
						break;

					case ELogVerbosity::VeryVerbose:
						UE_LOGF(LogAudioMixer, VeryVerbose, "%ls", *Message);
						break;
		...
		/** Non-blocking: signals the render thread to stop (sets Stopping state, triggers event)
		 *  without waiting for the thread to exit. Use TryFinishStopGeneratingAudio() to complete cleanup. */
		AUDIOMIXERCORE_API void SignalStopGeneratingAudio();

		/** Checks if the render thread has stopped after SignalStopGeneratingAudio() was called.
		 *  If the thread has stopped (or a timeout is reached), cleans up thread/events and returns true.
		 *  If the thread is still running, returns false. */
		AUDIOMIXERCORE_API bool TryFinishStopGeneratingAudio();
	...
	protected:
		...
		int32 CurrentUnderrunCount = 0;
		uint64 TimeLastWarningCycles = 0;
		...
		/** Timestamp (seconds) when SignalStopGeneratingAudio was called, for timeout detection. */
		double StopGeneratingAudioSignalTime = 0.0;
	...
	public:
		...
		virtual void LockDeviceSwap() override { DeviceSwapCriticalSection.Lock(); }
		virtual void UnlockDeviceSwap() override { DeviceSwapCriticalSection.Unlock(); }
	...
	private:
		...
		/** True when we've signaled the render thread to stop but haven't finished cleanup yet.
		 *  Only read/written on the audio thread (inside CheckThreadedDeviceSwap). */
		bool bWaitingForRenderThreadStop = false;

		/** If a force device swap is requested while bWaitingForRenderThreadStop is true, the
		 *  request is deferred here so it can be replayed after the current swap completes.
		 *  Empty string means no deferred swap; non-empty or explicit default ("") is stored
		 *  as a TOptional to distinguish "no request" from "request for default device". */
		TOptional<FString> DeferredDeviceSwapId;
		FString DeferredDeviceSwapReason;
	};

	class FScopedDeviceSwapLock final
	{
	public:
		FScopedDeviceSwapLock() = delete;
		explicit FScopedDeviceSwapLock(IAudioMixerPlatformInterface* InPlatform)
			: AudioMixerPlatform(InPlatform)
		{
			if (AudioMixerPlatform)
			{
				AudioMixerPlatform->LockDeviceSwap();
			}
		}

		~FScopedDeviceSwapLock()
		{
			if (AudioMixerPlatform)
			{
				AudioMixerPlatform->UnlockDeviceSwap();
			}
		}

	private:
		IAudioMixerPlatformInterface* AudioMixerPlatform = nullptr;
	};
}
