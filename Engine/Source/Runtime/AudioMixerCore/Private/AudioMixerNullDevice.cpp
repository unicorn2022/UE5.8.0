	...
	uint32 FMixerNullCallback::Run()
	{
		...
		while (!bShouldShutdown)
		{
			...
			if (WakeupEvent)
			{
				...
				UE_CLOGF(!bShouldShutdown && !bShouldRecyle, LogAudioMixer, Display, "FMixerNullCallback: Simulating a h/w device callback at [%dms], ThreadID=%u",  (int32)(CallbackTime * 1000.f), CallbackThread->GetThreadID() );
			...
			while (!bShouldRecyle && !bShouldShutdown)
			{
				...
				if (RealClock - LastLog > 30.f)
				{
					...
					{
						UE_LOGF(LogAudioMixer, Warning, "FMixerNullCallback: Large time delta between simulated audio clock and realtime [%dms], ThreadID=%u", (int32)(SleepTime * 1000.f), CallbackThread->GetThreadID());
						LastLog = RealClock;
					}
				}
#endif //!NO_LOGGING
