...
namespace Audio
{
	...
	bool CheckBufferValidity(const FResamplingParameters& InParameters, FResamplerResults& OutData)
	{
		...
		{
			UE_LOGF(LogAudioResampler, Error, "Please specify an output buffer when using Resample().");
			return false;
		}

		if (InParameters.SourceSampleRate <= 0.0f || InParameters.DestinationSampleRate <= 0.0f)
		{
			UE_LOGF(LogAudioResampler, Error, "Please use non-zero, positive sample rates when calling Resample().");
			return false;
		}

		if (OutData.OutBuffer->Num() < GetOutputBufferSize(InParameters))
		{
			UE_LOGF(LogAudioResampler, Error, "Insufficient space in output buffer: Please allocate space for %d samples.", GetOutputBufferSize(InParameters));
			return false;
		}
	...
	bool Resample(const FResamplingParameters& InParameters, FResamplerResults& OutData)
	{
		...
		{
			UE_LOGF(LogTemp, Error, "Error creating sample converter: %s", src_strerror(Error));
			return false;
		}
		...
		if (Error != 0)
		{
			UE_LOGF(LogTemp, Error, "Error on Resampling process: %s", src_strerror(Error));
			return false;
		}
	...
	void FResamplerImpl::Init(EResamplingMethod ResamplingMethod, float StartingSampleRateRatio, int32 InNumChannels) 
	{
		...
		if (LibSRCState)
		{
			if (InNumChannels == src_get_channels(LibSRCState))
			{
				...
				if (ErrorResult != 0)
				{
					...
					UE_LOGF(LogAudioResampler, Error, "Failed to reset sample converter state: %s.", ErrorString);
					return;
				}
			}
			else
		...
		if (nullptr == LibSRCState)	
		{
			...
			{
				const char* ErrorString = src_strerror(ErrorResult);
				UE_LOGF(LogAudioResampler, Error, "Failed to create a sample rate convertor state object: %s.", ErrorString);
			}
		}
		...
		if (LibSRCState)
		{
			...
			{
				const char* ErrorString = src_strerror(ErrorResult);
				UE_LOGF(LogAudioResampler, Error, "Failed to set sample rate ratio: %s.", ErrorString);
			}
		}
	...
	int32 FResamplerImpl::ProcessAudio(float* InAudioBuffer, int32 InSamples, bool bEndOfInput, float* OutAudioBuffer, int32 MaxOutputFrames, int32& OutNumFrames)
	{
		if (LibSRCState)
		{
			...
			{
				const char* ErrorString = src_strerror(ErrorResult);
				UE_LOGF(LogAudioResampler, Error, "Failed to process audio: %s.", ErrorString);
				return ErrorResult;
			}
