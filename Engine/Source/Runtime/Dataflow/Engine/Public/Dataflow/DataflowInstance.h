...
private:
	DATAFLOWENGINE_API void SetVariableOverrideAndNotify(const FGuid& PropertyID, bool bOverrideState);
...
struct FDataflowInstance
{
	...
	DATAFLOWENGINE_API FDataflowInstance& operator=(const FDataflowInstance& Other);
	
	// Delete these to avoid complications with Bind/UnbindEditorDelegates.
	//
	// They can be implemented if needed.
	FDataflowInstance(const FDataflowInstance& Other) = delete;
	FDataflowInstance(FDataflowInstance&& Other) = delete;
...
public:
	...
	/** Get the preview blueprint variable values for this asset. */
	FInstancedPropertyBag& GetPreviewBlueprintVariableOverrides()
	{
		return PreviewBlueprintVariableOverrides;
	}
	/** Get the preview blueprint variable values for this asset. Const version. */
	const FInstancedPropertyBag& GetPreviewBlueprintVariableOverrides() const
	{
		return PreviewBlueprintVariableOverrides;
	}
#endif
	...
	*/
	...
	/** Call as part of the PostLoad of any UObject containing a FDataflowInstance member */
	DATAFLOWENGINE_API void PostLoad();
...
private:
	// IMPORTANT: Any properties added here must be handled in the assignment operator
	...
	/** Dataflow variables to override */
	UPROPERTY(EditAnywhere, Category = "Dataflow", meta=(ShowOnlyInnerProperties))
	FDataflowVariableOverrides VariableOverrides;

#if WITH_EDITORONLY_DATA
	/** Preview blueprint variables to override */
	UPROPERTY()
	FInstancedPropertyBag PreviewBlueprintVariableOverrides;
#endif
...
{
	/** Weather or not a dataflow asset is assigned to an UObject */
	...
	/** Get the Dataflow instance interface from a UObject */
	DATAFLOWENGINE_API IDataflowInstanceInterface* GetDataflowInterfaceFromObject(UObject* Obj);

	/** Get the Dataflow attachment from a UObject */
	DATAFLOWENGINE_API UDataflowAttachment* GetDataflowAttachmentFromObject(const UObject* Obj);

	/** Get the Dataflow instance interface from a const UObject */
	DATAFLOWENGINE_API const IDataflowInstanceInterface* GetDataflowInterfaceFromObject(const UObject* Obj);

	/** 
	* Set a Dataflow asset/object on a specific object
	* The object type must be compatible with dataflow
	* Returns true if the dataflow object was set successfuly
	*/
	DATAFLOWENGINE_API bool SetDataflowAssetOnObject(UDataflow* DataflowAsset, UObject* Obj);
	...
	// Register tags for an asset using Dataflow
	DATAFLOWENGINE_API void GetAssetRegistryTags(const UObject* BoundAsset, FAssetRegistryTagsContext& Context);
