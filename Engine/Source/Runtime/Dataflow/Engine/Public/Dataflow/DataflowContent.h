...
public:
	...
	virtual ~UDataflowSkeletalContent() override= default;
...
protected:
#if WITH_EDITOR
	UFUNCTION()
	bool ShouldFilterAnimation(const FAssetData& AssetData) const;
#endif
	...
	UPROPERTY(EditAnywhere, Category = "Preview", Transient, SkipSerialization)
	bool bListCompatibleAnimationsOnly = true;

	/** Animation asset to be used to preview simulation */
	UPROPERTY(EditAnywhere, Category = "Preview", Transient, SkipSerialization, meta = (GetAssetFilter = "ShouldFilterAnimation"))
