...
#define LOCTEXT_NAMESPACE "DataflowSubGraphNodes"
...
namespace UE::Dataflow::SubGraphNodes::Private
{
	...
	{
		static constexpr int32 MaxSubGraphNestingDepth = 20;
	...
	public:
			...
			, SubGraphCallback(InSubGraphCallback)
		{
			...
			// Record the subgraph nesting depth
			if (const FContextSubGraph* ParentSubGraphContext = ParentContext.AsType<FContextSubGraph>())
			{
				SubGraphNestingDepth = ParentSubGraphContext->SubGraphNestingDepth + 1;
			}
		}
		...
		virtual void BroadCastLog(EMessageSeverity::Type Severity, const FText& InMessage, const FDataflowNode* InNode, const FDataflowOutput* InOutput) override
		{
			// make sure to forward to the parent context
			if (Severity == EMessageSeverity::Error)
			{
				ParentContext.Error(InMessage, InNode, InOutput);
			}
			else if (Severity == EMessageSeverity::Warning)
			{
				ParentContext.Warning(InMessage, InNode, InOutput);
			}
			else
			{
				ParentContext.Info(InMessage, InNode, InOutput);
			}

			FEngineContext::BroadCastLog(Severity, InMessage, InNode, InOutput);
		}
		...
		{
			if (SubGraphNestingDepth >= MaxSubGraphNestingDepth)
			{
				ParentContext.Error("Maximum nesting depth reached - skipping evaluation (Potential recursive subgraph call)", &CallerNode, &CallerOutput);
				CallerOutput.SetNullValue(*this);
				return false;
			}
			...
			if (const FDataflowInput* InputToPull = SubGraphOutputNode.FindInput(CallerOutput.GetName()))
			{
				if (const FDataflowOutput* OutputToEvaluate = InputToPull->GetConnection())
				{
					...
					{
						TAtomic<bool> bHasWarning = false;
						TAtomic<bool> bHasError = false;
						FDelegateHandle OnLogHandle = ParentContext.GetOnContextLogMulticast().AddLambda(
							[&bHasWarning, &bHasError](const FLogMessage& InMessage)
							{
								if (InMessage.Severity == EMessageSeverity::Warning)
								{
									bHasWarning = true;
								}
								else if (InMessage.Severity == EMessageSeverity::Error)
								{
									bHasError = true;
								}
							});
						...
						ParentContext.GetOnContextLogMulticast().Remove(OnLogHandle);

						if (bHasError)
						{
							const FText ErrorMessage(LOCTEXT("SubgraphContext_EvaluationError", "Evaluation of SubGraph returned errors : see the details above"));
							ParentContext.Error(ErrorMessage, &CallerNode, &CallerOutput);
						}
						if (bHasWarning)
						{
							const FText WarningMessage(LOCTEXT("SubgraphContext_EvaluationWarning", "Evaluation of SubGraph returned warnings: see the details above"));
							ParentContext.Warning(WarningMessage, &CallerNode, &CallerOutput);
						}
					}
				}
			}
...
{
	if (InSubGraphGuid == SubGraphGuid && InDataflowAsset == DataflowAssetWeakPtr)
	{
		switch (InReason)
		{
...
{
	if (InSubGraph.GetOuter() == DataflowAssetWeakPtr.Get())
	{
