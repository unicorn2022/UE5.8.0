...
#if WITH_EDITOR
FDataflowAssetDelegates::FOnDataflowPostEditUndo FDataflowAssetDelegates::OnDataflowPostEditUndo;
#endif
...
const FString UDataflow::SimulationTag = TEXT("DataflowSimulationTag");
...
void UDataflow::PostEditUndo()
{
	Super::PostEditUndo();

	// Refresh the graph panel so widgets for nodes restored by the transaction
	// system (e.g. comment nodes whose own PostEditUndo doesn't notify) get rebuilt.
	if (Schema)
	{
		NotifyGraphChanged();
	}

	FDataflowAssetDelegates::OnDataflowPostEditUndo.Broadcast(*this);
}
...
TSharedPtr<const UE::Dataflow::FCompiledGraph> UDataflow::CompileGraphIfNeeded()
{
	const FGuid GraphTopolyGuid = Dataflow ? Dataflow->GetGraphTopologyGuid() : FGuid();
	if (LastCompiledTopologyGUID != GraphTopolyGuid)
	{
		CompiledGraphs.Reset();

		UE::Dataflow::FCompilerParameters Params;
		Params.DataflowObjectToCompile = this;

		UE::Dataflow::FCompilerResults Results;
		if (UE::Dataflow::FCompiler::Compile(Params, Results))
		{
			CompiledGraphs = Results.GetCompiledGraphs();
			UE_LOGF(LogTemp, Warning, "Dataflow Compiled : time = %.3f ms (%d compiled graphs)", (float)Results.GetCompilationTimeMs(), CompiledGraphs.Num());
		}
		else
		{
			ensureMsgf(false, TEXT("Graph compilation failed, check the result error log"));
		}
		// store the guid even if the compilation was not successful so that we don't keep on trying to recompile
		LastCompiledTopologyGUID = GraphTopolyGuid;
	}
	const TSharedRef<UE::Dataflow::FCompiledGraph>* OutCompiledGraph = CompiledGraphs.Find(this);
	return OutCompiledGraph 
		? OutCompiledGraph->ToSharedPtr()
		: nullptr;
}
