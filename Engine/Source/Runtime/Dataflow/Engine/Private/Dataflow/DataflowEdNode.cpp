...
{
	UE_LOGF(DATAFLOWNODE_LOG, Verbose, "UDataflowEdNode::UpdatePinsFromDataflowNode()");
...
{
	UE_LOGF(DATAFLOWNODE_LOG, Verbose, "UDataflowEdNode::UpdatePinsConnectionsFromDataflowNode()");
	// called on node creation from UI. 
...
{
	UE_LOGF(DATAFLOWNODE_LOG, Verbose, "UDataflowEdNode::AllocateDefaultPins()");
	// called on node creation from UI. 
...
TSharedPtr<UE::Dataflow::FGraph> UDataflowEdNode::GetDataflowGraphFromPin(const UEdGraphPin& Pin)
{
...
TSharedPtr<const FDataflowNode> UDataflowEdNode::GetDataflowNodeFromPin(const UEdGraphPin& Pin)
{
	if (UDataflowEdNode* EdNode = Cast<UDataflowEdNode>(Pin.GetOwningNode()))
	{
...
const FDataflowInput* UDataflowEdNode::GetDataflowInputFromPin(const UEdGraphPin& Pin)
{
...
const FDataflowOutput* UDataflowEdNode::GetDataflowOutputFromPin(const UEdGraphPin& Pin)
{
	if (TSharedPtr<const FDataflowNode> DataflowNode = GetDataflowNodeFromPin(Pin))
	{
...
bool UDataflowEdNode::CanConnectPins(const UEdGraphPin& OutputPin, const UEdGraphPin& InputPin)
{
...
bool UDataflowEdNode::CanConvertPins(const UEdGraphPin& OutputPin, const UEdGraphPin& InputPin)
{
	ensure(OutputPin.Direction == EEdGraphPinDirection::EGPD_Output && InputPin.Direction == EEdGraphPinDirection::EGPD_Input);
...
/*static*/ FSlateIcon UDataflowEdNode::GetIconFromNode(const FDataflowNode& DataflowNode)
{
	FSlateIcon Icon;
	if (const FString* IconName = DataflowNode.TypedScriptStruct()->FindMetaData("Icon"))
	{
		FName StyleSheetName = FAppStyle::GetAppStyleSetName();
		if (const FString* StyleName = DataflowNode.TypedScriptStruct()->FindMetaData("Style"))
		{
			StyleSheetName = FName(*StyleName);
		}
		Icon = FSlateIcon(StyleSheetName, FName(*IconName));
	}
	else
	{
		// no icon defined by the node look for one in the category
		Icon = UE::Dataflow::FIconCategoryRegistry::Get().GetCategoryIcon(DataflowNode.GetCategory());
	}
	return Icon;
}
...
FSlateIcon UDataflowEdNode::GetIconAndTint(FLinearColor& OutColor) const
{
	...
	if (TSharedPtr<const FDataflowNode> DataflowNode = GetDataflowNode())
	{
		Icon = UDataflowEdNode::GetIconFromNode(*DataflowNode);
	}
	return Icon;
}
	...
	if (UEdGraphNode* FromGraphNode = FromPin->GetOwningNode())
	{
		if (FromPin->Direction == EEdGraphPinDirection::EGPD_Output)
		{
			for (UEdGraphPin* InputPin : this->GetAllPins())
			{
				...
				{
					if (UDataflowEdNode::CanConnectPins(*FromPin, *InputPin))
					{
		...
		if (FromPin->Direction == EEdGraphPinDirection::EGPD_Input)
		{
			for (UEdGraphPin* OutputPin : this->GetAllPins())
			{
				...
				{
					if (UDataflowEdNode::CanConnectPins(*OutputPin, *FromPin))
					{
...
void UDataflowEdNode::PostEditUndo()
{
	...
	if (UEdGraph* const ParentGraph = GetGraph())
	{
		if (IsValid(ParentGraph) && ParentGraph->Schema)
		{
			ParentGraph->NotifyGraphChanged();
		}
	}
	...
	if (Pin && SupportsEditablePinType(*Pin))
	{
		if (TSharedPtr<FDataflowNode> DataflowNode = GetDataflowNode())
		{
			if (const FDataflowInput* Input = DataflowNode->FindInput(Pin->PinName))
			{
				...
				{
					bEditablePinReentranceGuard = true;
					ON_SCOPE_EXIT{ bEditablePinReentranceGuard = false; };
					...
					if (PropertyType == Pin->PinType.PinCategory)
					{
						...
						// we cannot invalidate the node right away because invalidation cause delegate to invalidate the pin 
...
{
#if WITH_EDITOR
	ensure(!OnNodeInvalidatedDelegateHandle.IsValid());
	OnNodeInvalidatedDelegateHandle = FDataflowNodeDelegates::OnNodeInvalidated.AddWeakLambda(this, [this](FDataflowNode& InDataflowNode)
		{
			if (IsInGameThread() || IsInSlateThread())
			{
				if (InDataflowNode.GetGuid() == GetDataflowNodeGuid())
				{
					if (UEdGraph* EdGraph = GetGraph())
					{
						EdGraph->NotifyNodeChanged(this);
...
void UDataflowEdNode::UnRegisterDelegateHandle()
{
	...
	{
		FDataflowNodeDelegates::OnNodeInvalidated.Remove(OnNodeInvalidatedDelegateHandle);
