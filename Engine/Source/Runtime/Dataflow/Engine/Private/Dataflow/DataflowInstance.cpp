...
namespace UE::Dataflow::InstanceUtils
{
	...
	UDataflowAttachment* GetDataflowAttachment(const UObject* InAsset)
	{
		if (const IInterface_AssetUserData* UserDataInterface = Cast<IInterface_AssetUserData>(InAsset))
		{
			// The asset user interface does not provide a const API, we need to work around it 
			IInterface_AssetUserData* MutableUserDataInterface = const_cast<IInterface_AssetUserData*>(UserDataInterface);
			return MutableUserDataInterface->GetAssetUserData<UDataflowAttachment>();
		}
		return nullptr;
	}

	void AddDataflowAttachmentIfNeeded(UObject* InAsset)
	{
		if (const IDataflowInstanceInterface* DataflowInstanceInterface = Cast<IDataflowInstanceInterface>(InAsset))
		{
			// no attachement needed
			return;
		}

		if (IInterface_AssetUserData* UserDataInterface = Cast<IInterface_AssetUserData>(InAsset))
		{
			if (!UserDataInterface->HasAssetUserDataOfClass(UDataflowAttachment::StaticClass()))
			{
				if (UDataflowAttachment* Attachement = FDataflowAttachmentFactory::Get().Create(InAsset))
				{
					UserDataInterface->AddAssetUserData(Attachement);
				}
			}
		}
	}

	UDataflowAttachment* GetDataflowAttachmentFromObject(const UObject* Obj)
	{
		return GetDataflowAttachment(Obj);
	}

	IDataflowInstanceInterface* GetDataflowInterfaceFromObject(UObject* Obj)
	{
		// is the owning asset itself implement the dataflow instance interface
		if (IDataflowInstanceInterface* Interface = Cast<IDataflowInstanceInterface>(Obj))
		{
			return Interface;
		}
		// or is that using a dataflow attachement ? 
		if (UDataflowAttachment* Attachment = GetDataflowAttachment(Obj))
		{
			return Attachment;
		}
		return nullptr;
	...
	const IDataflowInstanceInterface* GetDataflowInterfaceFromObject(const UObject* Obj)
	{
		// is the owning asset itself implement the dataflow instance interface
		if (const IDataflowInstanceInterface* Interface = Cast<const IDataflowInstanceInterface>(Obj))
		{
			return Interface;
		}
		// or is that using a dataflow attachement ? 
		if (const UDataflowAttachment* Attachment = GetDataflowAttachment(Obj))
		{
			return Attachment;
		}
		return nullptr;
	...
	bool HasValidDataflowAsset(UObject* Obj)
	{
		if (Obj)
		{
			...
			if (const IDataflowInstanceInterface* Interface = GetDataflowInterfaceFromObject(Obj))
			{
				return true;
			}
			// Legacy - check if the class contains a dataflow asset and terminal node name properties
	...
	bool SetDataflowAssetOnObject(UDataflow* DataflowAsset, UObject* Obj)
	{
		AddDataflowAttachmentIfNeeded(Obj);

		// check if the object implements IDataflowGeneratedAssetInterface
		if (IDataflowInstanceInterface* Interface = GetDataflowInterfaceFromObject(Obj))
		{
			Interface->GetDataflowInstance().SetDataflowAsset(DataflowAsset);
			return true;
		}
		return false;
	}
	...
	void GetAssetRegistryTags(const UObject* BoundAsset, FAssetRegistryTagsContext& Context)
	{
		const UDataflow* DataflowObject = UE::Dataflow::InstanceUtils::GetDataflowAssetFromObject(BoundAsset);

		const bool bHasDataflow = (DataflowObject != nullptr);
		Context.AddTag(UObject::FAssetRegistryTag(TEXT("HasDataflow"), bHasDataflow ? TEXT("True") : TEXT("False"), UObject::FAssetRegistryTag::TT_Alphabetical));

		const bool bIsDataflowEmbedded = DataflowObject ? (!DataflowObject->IsAsset()) : false;
		Context.AddTag(UObject::FAssetRegistryTag(TEXT("IsDataflowEmbedded"), bIsDataflowEmbedded ? TEXT("True") : TEXT("False"), UObject::FAssetRegistryTag::TT_Alphabetical));
	}
...
FDataflowInstance& FDataflowInstance::operator=(const FDataflowInstance& Other)
{
	DataflowAsset = Other.DataflowAsset;
	DataflowTerminal = Other.DataflowTerminal;
	VariableOverrides = Other.VariableOverrides;
#if WITH_EDITORONLY_DATA
	PreviewBlueprintVariableOverrides = Other.PreviewBlueprintVariableOverrides;
#endif

	// Note that Owner is deliberately not copied, since it should be set to the owning object of 
	// this instance.
	//
	// OnOwnerPropertyChangedHandle should also not be copied, as each instance of 
	// FDataflowInstance needs to have its own unique OnOwnerPropertyChangedHandle to ensure that
	// the delegate is unregistered properly when the instance is destroyed.
...
void FDataflowInstance::PostLoad()
{
#if WITH_EDITOR
	BindEditorDelegates();
#endif
...
void FDataflowInstance::UnbindEditorDelegates()
{
	if (Owner)
	{
	...
	OnOwnerPropertyChangedHandle.Reset();
}
...
{
	bool bSuccess = false;
	if (DataflowAsset && DataflowAsset->Dataflow)
	{
