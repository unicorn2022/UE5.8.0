...
#if WITH_EDITOR
bool UDataflowSkeletalContent::ShouldFilterAnimation(const FAssetData& AssetData) const
{
	if (!SkeletalMesh || !SkeletalMesh->GetSkeleton() || !bListCompatibleAnimationsOnly)
	{
		return false; // nothing to filter against — show everything
	}

	const FString TargetSkeleton = FAssetData(SkeletalMesh->GetSkeleton()).GetExportTextName();
	const FString AssetSkeleton = AssetData.GetTagValueRef<FString>(TEXT("Skeleton"));
	return AssetSkeleton != TargetSkeleton; // true = exclude
}
#endif