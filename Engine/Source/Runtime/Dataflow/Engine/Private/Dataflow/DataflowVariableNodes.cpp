...
namespace GetDataflowVariableNode::Private
{
	static const FName EnableCategoriesName = TEXT("EnableCategories");
	static const FName CategoryMetaDataName = TEXT("Category");
	static const FName TooltipMetaDataName = TEXT("ToolTip");
}
...
#if WITH_EDITOR
			Category = Desc->GetMetaData(GetDataflowVariableNode::Private::CategoryMetaDataName);
			Description = Desc->GetMetaData(GetDataflowVariableNode::Private::TooltipMetaDataName);
#endif
...
void FGetDataflowVariableNode::NotifyAssetVariablesChanged(UDataflow* DataflowAsset)
{
	if (DataflowAsset)
	{
...
void FGetDataflowVariableNode::SetVariableMetaData(FName MetaDataName, const FString& InValue)
{
#if WITH_EDITOR
	if (const TStrongObjectPtr<UDataflow> DataflowAsset = WeakDataflowPtr.Pin())
	{
		...
		{
			TArray<FPropertyBagPropertyDesc> Descs(PropertyBag->GetPropertyDescs());
			if (FPropertyBagPropertyDesc* PropertyDesc = Descs.FindByPredicate([this](const FPropertyBagPropertyDesc& Desc) { return Desc.Name == VariableName; }))
			{
				if (MetaDataName == GetDataflowVariableNode::Private::CategoryMetaDataName)
				{
					PropertyDesc->SetMetaData(GetDataflowVariableNode::Private::EnableCategoriesName, FString());
				}
				PropertyDesc->SetMetaData(MetaDataName, InValue);
				...
				NotifyAssetVariablesChanged(DataflowAsset.Get());
			}
		}
	}
#endif
...
void FGetDataflowVariableNode::OnPropertyChanged(UE::Dataflow::FContext& Context, const FPropertyChangedEvent& InPropertyChangedEvent)
{
	...
	const FName CategoryMemberName = GET_MEMBER_NAME_CHECKED(FGetDataflowVariableNode, Category);
	const FName DescriptionMemberName = GET_MEMBER_NAME_CHECKED(FGetDataflowVariableNode, Description);

	if (PropertyName == VariablePropertyBagMemberName || MemberPropertyName == VariablePropertyBagMemberName || PropertyName == VariableName || MemberPropertyName == VariableName)
	{
	...
	else if (PropertyName == CategoryMemberName || MemberPropertyName == CategoryMemberName)
	{
		SetVariableMetaData(GetDataflowVariableNode::Private::CategoryMetaDataName, Category);
	}
	else if (PropertyName == DescriptionMemberName || MemberPropertyName == DescriptionMemberName)
	{
		SetVariableMetaData(GetDataflowVariableNode::Private::TooltipMetaDataName, Description);
	}
#endif
}
...
void FGetDataflowVariableNode::Evaluate(UE::Dataflow::FContext& Context, const FDataflowOutput* Out) const
{
	if (Out->IsA(&Value))
	{
		if (EvaluateFunction)
		{
			...
			{
				if (IDataflowInstanceInterface* Interface = UE::Dataflow::InstanceUtils::GetDataflowInterfaceFromObject(EngineContext->Owner))
				{
