...
namespace UE::Dataflow
{
	...
	{ 
		...
		{
			TUniquePtr<FDataflowNode> NewNode = ClassMap[Param.Type](Param);
			if(NewNode->HasValidConnections())
			{
				return Graph.AddNode(TSharedPtr<FDataflowNode>(NewNode.Release()));
			}
			
			const FText ErrorTitle = FText::FromString("Node Factory");
			const FString ErrorMessageString = FString::Printf(TEXT("Cannot create Node %s. Node Type %s is not well defined."), *NewNode->GetName().ToString(), *NewNode->GetDisplayName().ToString());
		...
		if (ClassMap.Contains(NewParameters.TypeName))
		{
			if (ParametersMap[NewParameters.TypeName].DisplayName.IsEqual(NewParameters.DisplayName))
			{
				UE_LOG(LogDataflowFactory, Warning,
					...
					*NewParameters.DisplayName.ToString());
			}
