...
#if WITH_EDITOR
FDataflowNodeDelegates::FOnNodeInvalidated FDataflowNodeDelegates::OnNodeInvalidated;
#endif
...
void FDataflowNode::Invalidate(const UE::Dataflow::FTimestamp& InModifiedTimestamp)
{
	...
	if (LastModifiedTimestamp < InModifiedTimestamp)
	{
		...
		{
			if (FDataflowNodeDelegates::OnNodeInvalidated.IsBound())
			{
				FDataflowNodeDelegates::OnNodeInvalidated.Broadcast(*this);
			}
		}
#endif
...
bool FDataflowNode::EvaluateNode(UE::Dataflow::FContext& Context) const
{
	// TODO(dataflow) : in the future we should also have the possibility to evaluate the watched output if needed by the context 
	bool bWasEvaluatedOrForwarded = false;
	if (IsActive())
	{
		if (ExpandedOutputs.IsEmpty())
		{
			Evaluate(Context, nullptr);
			bWasEvaluatedOrForwarded = true;
		}
		else if (!IsFrozen())
		{
			for (const TPair<UE::Dataflow::FConnectionKey, FDataflowOutput*>& OutputByKey : ExpandedOutputs)
			{
				if (const FDataflowOutput* const Output = OutputByKey.Value)
				{
					// todo(ccaillaud) : shoudl we have an option to skip the non connected outputs ? 
					if (!Context.HasData(Output->CacheKey(), GetTimestamp()))
					{
	...
	else
	{
		for (const TPair<UE::Dataflow::FConnectionKey, FDataflowOutput*>& OutputByKey : ExpandedOutputs)
		{
			if (const FDataflowOutput* const Output = OutputByKey.Value)
			{
				if (const FDataflowInput* const PassthroughInput = Output->GetPassthroughInput())
				{
					Output->ForwardInput(PassthroughInput, Context);
					bWasEvaluatedOrForwarded = true;
				}
			}
		}
	}
	return bWasEvaluatedOrForwarded;
}
...
FText FDataflowNode::GetPropertyDisplayNameText(const TArray<const FProperty*>& PropertyChain, int32 ContainerIndex)
{
	...
	{
		const FProperty* const Property = PropertyChain[Index];
		if (!Property->HasMetaData(FName("SkipInDisplayNameChain")))
		{
...
{
	//UE_LOGF(LogChaos, Warning, "%ls", *GetName().ToString())
			...
			for (FPropertyValueIterator PropertyIt(FProperty::StaticClass(), Struct, this); PropertyIt; ++PropertyIt)
			{
				if (const FProperty* const Property = PropertyIt.Key())
				{
					...
					if (Property->PropertyFlags & CPF_HasGetValueTypeHash)
					{
						...
						// UE_LOGF(LogChaos, Warning, "( %lu \t%ls", (unsigned long)CrcHash, *Property->GetName())
					...
					if (const FStructProperty* StructProperty = CastField<FStructProperty>(Property))
					{
						if (!StructProperty->Struct || !StructProperty->Struct->GetCppStructOps())
						{
							...
							UE_LOGF(LogChaos, Warning, 
								"Dataflow: Context caching disable for graphs with node '%ls' due to non-hashed UPROPERTY '%ls'.", 
		...
		if (const UStruct* const Struct = ScriptOnStruct->GetStruct())
		{
			for (FPropertyValueIterator PropertyIt(FProperty::StaticClass(), Struct, ScriptOnStruct->GetStructMemory()); PropertyIt; ++PropertyIt)
			{
				...
				if (Property->HasMetaData(FDataflowNode::DataflowInput))
				{
					...
					{
						UE_LOGF(LogChaos, Warning, "Missing dataflow RegisterInputConnection in constructor for (%ls:%ls)", *GetName().ToString(), *PropName.ToString())
				...
				if (Property->HasMetaData(FDataflowNode::DataflowOutput))
				{
					...
					if(!OutputConnection)
					{
						UE_LOGF(LogChaos, Warning, "Missing dataflow RegisterOutputConnection in constructor for (%ls:%ls)", *GetName().ToString(),*PropName.ToString());
						bHasValidConnections = false;
						ensure(false);
					}
					...
					else if (const FString* PassthroughName = Property->FindMetaData(FDataflowNode::DataflowPassthrough))
					{
						...
						{
							UE_LOGF(LogChaos, Warning, "Missing DataflowPassthrough registration for (%ls:%ls)", *GetName().ToString(), *PropName.ToString());
							bHasValidConnections = false;
							ensure(false);
						...
						{
							UE_LOGF(LogChaos, Warning, "Mismatch in declared and registered DataflowPassthrough connection; (%ls:%ls vs %ls)", *GetName().ToString(), *FullPassthroughName, *PassthroughConnectionInput->GetName().ToString());
							bHasValidConnections = false;
							ensure(false);
						}
						...
						{
							UE_LOGF(LogChaos, Warning, "Incorrect DataflowPassthrough Connection set for (%ls:%ls)", *GetName().ToString(), *PropName.ToString());
							bHasValidConnections = false;
							ensure(false);
						}
						...
						{
							UE_LOGF(LogChaos, Warning, "DataflowPassthrough connection types mismatch for (%ls:%ls)", *GetName().ToString(), *PropName.ToString());
							bHasValidConnections = false;
							ensure(false);
						}
					}
					...
					{
						UE_LOGF(LogChaos, Warning, "Missing DataflowPassthrough declaration for (%ls:%ls)", *GetName().ToString(), *PropName.ToString());
						bHasValidConnections = false;
						ensure(false);
					}
				}
			...
			for (const TPair<UE::Dataflow::FConnectionKey, FDataflowInput*>& ExpandedInput : ExpandedInputs)
			{
				...
				if (const FProperty* Property = Input->GetProperty())
				{
					if (!Property->HasMetaData(FDataflowNode::DataflowInput))
					{
						...
						UE_LOGF(LogChaos, Warning, "Missing dataflow DataflowInput declaration for (%ls:%ls)", *GetName().ToString(), *Property->GetFName().ToString());
					}
				}
			}
			...
			for (const TPair<UE::Dataflow::FConnectionKey, FDataflowOutput*>& Output : ExpandedOutputs)
			{
				...
				if (const FProperty* Property = OutputVal->GetProperty())
				{
					if (!Property->HasMetaData(FDataflowNode::DataflowOutput))
					{
						...
						UE_LOGF(LogChaos, Warning, "Missing dataflow DataflowOutput declaration for (%ls:%ls)", *GetName().ToString(), *Property->GetFName().ToString());
					}
				}
			}
#endif	
...
{
	const UE::Dataflow::FFactoryParameters& FactoryParameters = UE::Dataflow::FNodeFactory::GetInstance()->GetParameters(GetType());
...
bool FDataflowNode::SetOutputConnectionConcreteType(FDataflowConnection* Output, FName NewType)
{
	if (Output && Output->GetOwningNode() == this && Output->GetDirection() == UE::Dataflow::FPin::EDirection::OUTPUT)
	{
		return SetConnectionConcreteType(Output, NewType);
	}
	return false;
}
