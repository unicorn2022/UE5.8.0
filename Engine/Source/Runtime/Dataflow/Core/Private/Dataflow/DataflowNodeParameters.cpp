...
namespace UE::Dataflow
{
	...
	void FContext::Info(const FText& InInfo, const FDataflowNode* InNode, const FDataflowOutput* InOutput)
	{
		BroadCastLog(EMessageSeverity::Info, InInfo, InNode, InOutput);
	}

	void FContext::Info(const FString& InInfo, const FDataflowNode* InNode, const FDataflowOutput* InOutput)
	{
		FText Message = FText::FromString(InInfo);
		Info(Message, InNode, InOutput);
	}

	void FContext::Warning(const FText& InWarning, const FDataflowNode* InNode, const FDataflowOutput* InOutput)
	{
		BroadCastLog(EMessageSeverity::Warning, InWarning, InNode, InOutput);
	}

	void FContext::Warning(const FString& InWarning, const FDataflowNode* InNode, const FDataflowOutput* InOutput)
	{
		FText Message = FText::FromString(InWarning);
		Warning(Message, InNode, InOutput);
	}

	void FContext::Error(const FText& InError, const FDataflowNode* InNode, const FDataflowOutput* InOutput)
	{
		BroadCastLog(EMessageSeverity::Error, InError, InNode, InOutput);
	}

	void FContext::Error(const FString& InError, const FDataflowNode* InNode, const FDataflowOutput* InOutput)
	{
		FText Message = FText::FromString(InError);
		Error(Message, InNode, InOutput);
	}

	void FContext::BroadCastLog(EMessageSeverity::Type Severity, const FText& InMessage, const FDataflowNode* InNode, const FDataflowOutput* InOutput)
	{
		FLogMessage LogMessage
		{
			.Severity = Severity,
			.Node = InNode ? InNode->AsWeak() : TWeakPtr<const FDataflowNode>{},
			.OutputGuid = (InOutput) ? InOutput->GetGuid() : FGuid(),
			.Text = InMessage,
		};

		auto BroadCastLogOnGameThread =
			[WeakDelegates = Delegates.ToWeakPtr(), Message = MoveTemp(LogMessage)]()
			{
				check(IsInGameThread());
				if (TSharedPtr<FDelegates> Delegates = WeakDelegates.Pin())
				{
					if (Delegates->OnContextLog.IsBound())
					{
						Delegates->OnContextLog.Broadcast(Message);
					}
				}
			};
		...
		if (IsInGameThread())
		{
			BroadCastLogOnGameThread();
		}
		else
		{
			Async(EAsyncExecution::TaskGraphMainThread, MoveTemp(BroadCastLogOnGameThread));
		}
	}
	...
	FContext::FContext() 
		: AsyncEvaluator(*this) 
		, Delegates(MakeShared<FDelegates>())
	{}
	...
	void FContext::BeginContextEvaluation(const FDataflowNode * Node, const FDataflowOutput * Output)
	{
		...
		else if (Node)
		{
			...
			else if (Node->IsActive() && !Node->IsFrozen())
			{
				...
				UE_LOGF(LogChaosDataflow, Verbose, "FDataflowNode::Evaluate(): Node [%ls], Output [nullptr], NodeTimestamp [%llu]", *Node->GetName().ToString(), Node->GetTimestamp().Value);
	...
	bool FContext::Evaluate(const FDataflowOutput& Connection)
	{
		...
		else
		{
			...
			UE_LOGF(LogChaosDataflow, VeryVerbose, "FContext::Evaluate(): Node [%ls], Output [%ls]", *Connection.GetOwningNode()->GetName().ToString(), *Connection.GetName().ToString());
			bool RetValue = false;
			{
			...
			/*const bool bHasError = NodeHasError(Connection.GetOwningNode());
