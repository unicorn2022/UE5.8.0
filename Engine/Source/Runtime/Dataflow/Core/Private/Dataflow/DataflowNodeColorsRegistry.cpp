...
namespace UE::Dataflow
{
	...
	FIconCategoryRegistry& FIconCategoryRegistry::Get()
	{
		static FIconCategoryRegistry Instance;
		return Instance;
	}

	void FIconCategoryRegistry::RegisterCategoryIcon(const TCHAR* Category, const TCHAR* IconName, const TCHAR* StyleName)
	{
		if (Category && IconName)
		{
			FName CategoryName(Category);
			if (!IconsByCategory.Contains(CategoryName))
			{
				FIconInfo IconInfo;
				IconInfo.IconName = FName(IconName);
				IconInfo.StyleName = StyleName ? FName(StyleName) : FAppStyle::GetAppStyleSetName();
				IconsByCategory.Add(Category, IconInfo);
			}
		}
	}


	FSlateIcon FIconCategoryRegistry::GetCategoryIcon(const FName Category) const
	{
		if (const FIconInfo* IconInfo = IconsByCategory.Find(Category))
		{
			return FSlateIcon(IconInfo->StyleName, IconInfo->IconName);
		}
		return FSlateIcon();
	}
}
