...
UENUM()
enum class EDataflowConvertToResolutionMethod : int32
{
	UseClosest = 0 UMETA(DisplayName = "Closest resolution"),
	UsePrevious = 1 UMETA(DisplayName = "Previous resolution"),
	UseNext = 2 UMETA(DisplayName = "Next resolution"),
};
...
*/
USTRUCT()
struct FDataflowConvertToResolutionNode : public FDataflowNode
{
	GENERATED_USTRUCT_BODY()
	DATAFLOW_NODE_DEFINE_INTERNAL(FDataflowConvertToResolutionNode, "ConvertToResolution", DATAFLOW_IMAGE_NODES_CATEGORY, "")

public:
	FDataflowConvertToResolutionNode(const UE::Dataflow::FNodeParameters& InParam, FGuid InGuid = FGuid::NewGuid());

private:
	/** Arbitrary value to convert to a image resolution type */
	UPROPERTY(EditAnywhere, Category = Options, meta = (DataflowInput));
	FDataflowNumericTypes Value;

	/** Method to use to match the resolution */
	UPROPERTY(EditAnywhere, Category = Options);
	EDataflowConvertToResolutionMethod Method = EDataflowConvertToResolutionMethod::UseClosest;
