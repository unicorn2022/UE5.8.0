	...
	*/
	...
	public:
		...
		void SetData(FContextCacheKey InKey, const FProperty* InProperty, T&& InValue, const FGuid& InNodeGuid, uint32 InNodeHash, const FTimestamp& InTimestamp)
		{
			...
			{
				if (const FContextCacheElementBase* CacheEntryToClone = InValue.GetCacheEntryPrivate())
				{
		...
		DATAFLOWCORE_API void Info(const FText& InInfo, const FDataflowNode* InNode = nullptr, const FDataflowOutput* InOutput = nullptr);
		...
		DATAFLOWCORE_API void Warning(const FText& InWarning, const FDataflowNode* InNode = nullptr, const FDataflowOutput* InOutput = nullptr);
		...
		DATAFLOWCORE_API void Error(const FText& InError, const FDataflowNode* InNode = nullptr, const FDataflowOutput* InOutput = nullptr);
		...
		struct FLogMessage
		{
			...
			TWeakPtr<const FDataflowNode> Node;
			FGuid OutputGuid;
			FText Text;
		};

		DECLARE_MULTICAST_DELEGATE_OneParam(FOnContextLogMulticast, const FLogMessage& Message);
		FOnContextLogMulticast& GetOnContextLogMulticast() { return Delegates->OnContextLog; }
		...
		UE_DEPRECATED(5.8, "Deprecated method, context no longer count the Info calls")
		int32 GetNumInfo() const { return 0; }

		UE_DEPRECATED(5.8, "Deprecated method, context no longer count the Warning calls")
		int32 GetNumWarnings() const { return 0; }

		UE_DEPRECATED(5.8, "Deprecated method, context no longer count the Error calls")
		int32 GetNumErrors() const { return 0; }

		UE_DEPRECATED(5.8, "Deprecated method, context does not store the warning state of a node, this is the responsability of the code using the context")
		bool NodeHasWarning(const FDataflowNode* InNode) const { return false; }

		UE_DEPRECATED(5.8, "Deprecated method, context does not store the error state of a node, this is the responsability of the code using the context")
		bool NodeHasError(const FDataflowNode* InNode) const { return false; }

		UE_DEPRECATED(5.8, "Deprecated state of a node, failure state is not longer reported")
		bool NodeFailed(const FDataflowNode* InNode) const { return false; }

		UE_DEPRECATED(5.8, "ClearNodesData is deprecated as it is no longer needed")
		void ClearNodesData() {}

		UE_DEPRECATED(5.8, "ClearNodeData is deprecated as it is no longer needed")
		void ClearNodeData(const FDataflowNode* InNode) {}
		...
		UE_DEPRECATED(5.8, "Use OnContextLogMulticast instead.")
	...
	protected:
		...
		DATAFLOWCORE_API virtual void BroadCastLog(EMessageSeverity::Type Severity, const FText& InMessage, const FDataflowNode* InNode, const FDataflowOutput* InOutput);

	private:
		// have the delegate in a shared ref so we can keep a weakpointer on them when scheduling on the gamethread from another thread 
		struct FDelegates
		{
			FOnContextLogMulticast OnContextLog;
		};
		TSharedRef<FDelegates> Delegates;
