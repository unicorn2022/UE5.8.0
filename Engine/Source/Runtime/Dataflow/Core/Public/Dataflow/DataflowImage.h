...
struct FDataflowImage
{
	...
	DATAFLOWCORE_API void CreateR32F(EDataflowImageResolution Resolution, TArrayView64<float> InPixels = {});

	/** Create a single channel float format image */
	DATAFLOWCORE_API void CreateR32F(int32 Width, int32 Height, TArrayView64<float> InPixels = {});

	/** Create a four channels float format image */
	DATAFLOWCORE_API void CreateRGBA32F(EDataflowImageResolution Resolution, TArrayView64<FVector4f> InPixels = {});

	/** Create a four channels float format image */
	DATAFLOWCORE_API void CreateRGBA32F(int32 Width, int32 Height, TArrayView64<FVector4f> InPixels = {});
	...
	/**
	* Copy R32F pixels to the image
	* Number of pixels must match and format must be R32F
	* return false if the copy could not be done
	*/
	DATAFLOWCORE_API bool CopyPixels(TArrayView64<float> Pixels);
	...
	* return false if the copy could not be done
	*/
	DATAFLOWCORE_API bool CopyPixels(TArrayView64<FVector4f> Pixels);
