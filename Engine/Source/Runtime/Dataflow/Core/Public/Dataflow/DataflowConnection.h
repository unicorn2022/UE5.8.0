...
public:
	...
	virtual ~FDataflowConnection() = default;
	...
	*/
	...
	/**
	* return the short list of supported types
	* if the type of the connection is a broad anytype and that the list of supported type is not finite, then nothing is returned
	*/
	DATAFLOWCORE_API void GetSupportedTypes(TArray<FName>& OutSupportedTypes) const;
